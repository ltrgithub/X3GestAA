		...
		if (!session.isAuthenticated()) {
			...
			if (options.forceAuth) {
				...
				if (options.authType == "sage-id") {
					...
					if (session.expireDue) {
						...
						require("../../src/auth/sage-id").verifyUser(request, response, authData, _);
					} else {
						...
						if (authData.sessionNotify) {
							...
							var extend = require("../../src/auth/sage-id").extendSession(request, response, session, authData, _);
			...
			if (authRes) {
				...
				if (options.authType == "sage-id") {
					...
					session = require("../../src/auth/sage-id").updateSessionWithID(request, response, _sessions, cookie, authData, _);
				}
...
exports.getAuthModuleObsolete = function(request, redirect, _) {
	...
	if (request.connection.authorized) return require("../../src/auth/certificate").create(checkUser.fromCertificate);
	// certificate auth <<<
	var result;
	...
	if (regexResult && regexResult[1].indexOf("oauth2") < 0 && regexResult[3]) { // login with token
		var user = require("../../src/auth/changePassword").getTempLogin(regexResult[3]);
		if (user) {
			return {
	...
	if (authmethod === "basic") {
		return require("../../src/auth/basic").create(checkUser.fromLoginPage);
	} else if (authmethod === "digest") {
		return require("../../src/auth/digest").create(checkUser.fromLoginPage);
	} else if (authmethod === "sageerpx3") {
		return require("../../src/auth/sageerpx3").create(checkUser.fromLoginPage);
		...
		return require("../../src/auth/oauth2").create(checkUser.fromLoginPage, authserver);
	}
};
...
function dispatcherObsolete(config) {
	return function(_, request, response) {
		...
		if (request.url.indexOf("/auth/pwd--") === 0) {
			...
			var newUrl = "/";
			var changePasswordModule = require('../../src/auth/changePassword');
