...
function loadResource(_, path, name) {
	var res = JSON.parse(fs.readFile(path, 'utf8', _));
	res.id = name.replace(/\.json$/, '');
	return res;
}
...
var workingCopyActions = {
	$save: function(_, entity, wc, result) {
		fs.writeFile(ROOT + "/partner-license/data/" + entity + "/" + wc.$uuid + ".json", JSON.stringify(wc, null, '\t'), 'utf8', _);
...
function updateWorkingCopy(_, entity, wc, data) {
	Object.keys(data).forEach(function(key) {
		if (key !== "$actions" && key !== "$url") wc[key] = data[key];
	});
	var result = {};
...
function dataDispatcher(_, request, response) {
	...
	var path = fspath.join(ROOT, 'partner-license', 'data', entity);
	var posted = request.readAll(_, 'utf8');
	try {
		if (key === '$workingCopies') {
			var trackingId = request.parsedUrl.query.trackingId,
				wc;
			if (request.method === 'POST') {
				var uuid = segments[4];
				wc = workingCopies[trackingId] = uuid //
				? loadResource(_, path + '/' + uuid + '.json', uuid + '.json') //
				: {
...
ez.devices.http.server(function(request, response, _) {
	if (request.url === '/') {
		return redirect(_, 307, response, '/partner-license/html/main.html?url=' + encodeURIComponent("/home?representation=home.$page"));
	}
