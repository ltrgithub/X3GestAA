...
module.exports = {
	...
	reader: function(_, connection, sql, args, ord) {
		var received = [],
			error, callback, done;
...
*/
		...
		function push(record) {
			Object.keys(record).forEach(function(key)
			{
				...
				{
					// Convert byte arrays to buffers
		...
		function send(err, result) {
			//trace && trace('send(' + (err ? err.message : "undefined") + "," + (result ? "result" : "undefined") + ")");
			var cb = callback;
			callback = null;
			if (cb) {
		...
		function sendResult(result) {
			/*console.error("RESULT1", result.map(function(col) {
				return typeof col.value + ", type=" + require('util').inspect(col.metadata.type);
			}));*/
			result = dru.tdsColumnsToRow(result);
			//console.error("RESULT2", result);
			return send(null, result);
		}

		var reader = generic.reader(function(cb) {
			if (error) {
				tracer.debug && tracer.debug("Reader call 1");
				return cb(error);
			}
			if (received.length) {
				// Dequeue the first available row.
				tracer.debug && tracer.debug("Reader call 2");
				return cb(null, shift());
			}
			if (done) {
				tracer.debug && tracer.debug("Reader call 3");
			...
			callback = cb;
				...
				console.error("SQL SERVER STOP", done);
				if (!cb) console.error("CB MISSING IN STOP", new Error().stack);
				if (done) return cb();
				cancelled = true;
				connection.cancel();
				callback = cb;
		...
		tracer.debug && tracer.debug("In reader 0 state" + connection.state.name);
		reader.context = {};
		tracer.debug && tracer.debug("reader initialized : " + sql);

		var request = tds_query(connection, sql, args, function(err, rowCount, rows) {
			//console.error("tds_query complete", !!err, cancelled);
			if (err && cancelled) err = null;
			send(err);
		});
		tracer.debug && tracer.debug("In reader 1 state" + connection.state.name);

		request.on('row', sendResult);

		tracer.debug && tracer.debug("In reader 2 state" + connection.state.name);
		...
		/*request.on('error', function(err) {
			console.error("TEDIOUS ON ERROR", cancelled, err.code);
			if (cancelled && err.code === ECANCEL) err = null;
			send(err);
		});*/
		tracer.debug && tracer.debug("In reader 3 state" + connection.state.name);		
		done = false ;
		connection.execSql(request);
		tracer.debug && tracer.debug("In reader 4 state" + connection.state.name);		
