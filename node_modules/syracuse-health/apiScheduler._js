...
var tests = {
	...
	elasticSearchResponseTime: function(_) {
		var url = 'http://' + config.searchEngine.hostname + ':' + config.searchEngine.port;
		var result = healthCheck(_, url);
		var retVal;
		if (result !== undefined) {
	...
	mongoDbResponseTime: function(_) {
		var server = new mongo.Server(config.collaboration.hostname, config.collaboration.port);
...
flows.setInterval(function(_) {
	try {
		console.debug("API Health Check interval");
