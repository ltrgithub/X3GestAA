...
var tests = {
	...
	dbResponseTime: function(_, site) {
		...
		var configuration = {
		    user: user,
		    password: pwd,
		    server: site.ipORA + '\\' + config.apiHealthScheduler.instance,
		    database: config.apiHealthScheduler.catalog,
		    connectionTimeout: config.apiHealthScheduler.timeout,
		    pool: {
...
flows.setInterval(function(_) {
	try {
		...
		var enc = config.apiHealthScheduler.healthId;
