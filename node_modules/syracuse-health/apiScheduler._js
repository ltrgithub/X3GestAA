...
var tests = {
	...
	x3ServerResponseTime: function(_, site) {
		...
		var instId = getInstanceIdFromIP(ipX3, _);
		if (instId == undefined){
			throw new Error('X3 Server does not exist');
			return;
		}
	...
	elasticSearchResponseTime: function(_) {
		...
		//Check if instance exists before attempting to connect
		var instId = getInstanceIdFromIP(config.apiHealthScheduler.esHostName, _);
		if (instId == undefined){
			throw new Error('ElasticSearch Server does not exist');
			return;
		}
	...
	mongoDbResponseTime: function(_) {
		//Check if instance exists before attempting to connect
		var instId = getInstanceIdFromIP(config.apiHealthScheduler.mongoHostName, _);
		if (instId == undefined){
			throw new Error('MongoDB Server does not exist');
			return;
		}
	...
	dbResponseTime: function(_, site) {
		...
		//Check if instance exists before attempting to connect
		var instId = getInstanceIdFromIP(site.ipORA, _);
		if (instId == undefined){
			throw new Error('Database Server does not exist');
			return;
		}
...
var usageTests = {
	CPUUtilization: function(_, instId) {
		var usage = getUsage(instId, 'CPUUtilization', _);
		return usage;
	},
	NetworkIn: function(_, instId) {
		var usage = getUsage(instId, 'NetworkIn', _);
		return usage;
	},
	NetworkOut: function(_, instId) {
		var usage = getUsage(instId, 'NetworkOut', _);
		return usage;
	},
}
...
function getTenantUsage(_, site, instanceType) {
	...
	var instId = getInstanceIdFromIP(ip, _);
	var t = new Date();
	try {
		return {
			type: instanceType,
...
function getInstanceIdFromIP (instanceIP, _) {
  var params = {
    Filters: [
      {
  ...
  //console.log('Inst IP =', instanceIP);
...
flows.setInterval(function(_) {
	try {
		...
		var db2 = new mongo.Db('production', server2, {
			fsync: true
		});
		var result;
		var result2;
		try {
			...
			//console.log('Site Cursor Length = ' + cursor.length);
			//console.log('Site Cursor = ' ,cursor);

			for (var i = 0; i < cursor.length; i++) {
				//if(cursor[i].site === 'QA01' || cursor[i].site === 'QA02'){
