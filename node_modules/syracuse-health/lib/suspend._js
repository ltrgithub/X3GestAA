...
function getSiteStatus(_, request, response) {
	...
	var url = options.baseUrl + "sites?representation=site.%24query&count=5&where=(site%20like%20'%25" + (request.headers['x-forwarded-host'] || request.headers.host) + "%25')&startIndex=1";
	...
	if (sites.length === 0) throw new Error("site not found: " + (request.headers['x-forwarded-host'] || request.headers.host));
