...
var loadOptions = exports.loadOptions = {
	cache: true,
}
...
function _loadFile(_, dep) {
	...
	_watch(path);
	return compiler.loadFile(_, path, loadOptions);
}
...
function _missingDependencies(_, root, path, known) {
	...
	function _explore(_, dep, missingMap) {
		...
		var dependencies = _directDependencies(_, dep);
	...
	known.forEach_(_, function(_, cur) {
		_explore(_, { root: root, resolvedAs: fspath.join(root, cur), requiredAs: cur  }, null);
	});
	// then fill missing map
	_explore(_, { root: root, resolvedAs: fspath.join(root, path), requiredAs: path }, missingMap);
