...
function _loadFile(_, path) {
	...
	if (_exists(_, _js)) {
		_watch(_js);
		return require("streamline/lib/compiler/compile").loadFile(_, _js, loadOptions);
	} else if (_exists(_, js_)) {
		_watch(js_);
		return require("streamline/lib/compiler/compile").loadFile(_, js, loadOptions);
...
function _missingDependencies(_, path, known) {
	...
	function _explore(_, dep, missingMap) {
		var path = _extendPath(_, dep.path);
	...
	known.forEach_(_, function(_, cur) {
		_explore(_, { path: cur }, null);
	});
	// then fill missing map
	_explore(_, { path: path }, missingMap);
