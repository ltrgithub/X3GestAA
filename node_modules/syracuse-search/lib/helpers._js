...
exports.indexExists = function(_, baseUrl, indexName, options) {
	...
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index indexExists: url: " + par.url);
	// TODO: configure if elasticSearch really exists for this server
	try {
		var resp = streams.httpRequest(par).end().response(_);
		tracer.trace.debug() && tracer.trace.debug("elasticIndex.index exists status: " + resp.statusCode);
		resp.readAll(_);
...
exports.getIndexFacets = function(_, baseUrl, indexName, options) {
	...
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index getIndexFacets: url: " + par.url);
	var resp = streams.httpRequest(par).end().response(_);
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index getIndexFacets status: " + resp.statusCode);
	var content = resp.readAll(_);
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index getIndexFacets content: " + content);
	...
	try {
		...
		if (resp.statusCode === 200) {
			if (!onlyError) {
				...
				tracer.trace.info() && tracer.trace.info("Search engine is ok " + baseUrl);
			}
		} else {
			diag.push({
			...
			tracer.trace.error() && tracer.trace.error("Search engine is not ok " + "\n [" + resp.statusCode + "]");
		}
	} catch (e) {
		diag.push({
			$severity: asInfo ? "info" : "error",
