...
exports.debugTrace = function(opt, mesg, fine) {
	if (opt && opt.tracer) {
		if (!fine || opt.tracer.fine) {
			console.log(mesg);
		}
	}
};
...
exports.indexExists = function(_, baseUrl, indexName, options) {
	...
	exports.debugTrace(opt, "elasticIndex.index indexExists: url: " + par.url);
	// TODO: configure if elasticSearch really exists for this server
	try {
		var resp = streams.httpRequest(par).end().response(_);
		exports.debugTrace(opt, "elasticIndex.index exists status: " + resp.statusCode);
		resp.readAll(_);
...
exports.getIndexFacets = function(_, baseUrl, indexName, options) {
	...
	exports.debugTrace(opt, "elasticIndex.index getIndexFacets: url: " + par.url);
	var resp = streams.httpRequest(par).end().response(_);
	exports.debugTrace(opt, "elasticIndex.index getIndexFacets status: " + resp.statusCode);
	var content = resp.readAll(_);
	exports.debugTrace(opt, "elasticIndex.index getIndexFacets content: " + content);
