...
exports.indexExists = function(_, baseUrl, indexName, options) {
	...
	logger.debug(_) && logger.debug(_, "elasticIndex.index indexExists: url: " + par.url);
	// TODO: configure if elasticSearch really exists for this server
	try {
		var resp = streams.httpRequest(par).end().response(_);
		logger.debug(_) && logger.debug(_, "elasticIndex.index exists status: " + resp.statusCode);
		resp.readAll(_);
...
exports.getIndexFacets = function(_, baseUrl, indexName, options) {
	...
	logger.debug(_) && logger.debug(_, "elasticIndex.index getIndexFacets: url: " + par.url);
	var resp = streams.httpRequest(par).end().response(_);
	logger.debug(_) && logger.debug(_, "elasticIndex.index getIndexFacets status: " + resp.statusCode);
	var content = resp.readAll(_);
	logger.debug(_) && logger.debug(_, "elasticIndex.index getIndexFacets content: " + content);
	...
	try {
		...
		if (resp.statusCode === 200) {
			if (!onlyError) {
				...
				logger.info(_) && logger.info(_, "Search engine is ok " + baseUrl);
			}
		} else {
			diag.push({
			...
			logger.error(_) && logger.error(_, "Search engine is not ok " + "\n [" + resp.statusCode + "]");
		}
	} catch (e) {
		diag.push({
			$severity: asInfo ? "info" : "error",
