 ...
 */
function _updateResource(_, hit, properties, resource, right, queryString) {
...
function _makeFunctionResource(_, endpoint, mapping, hit, isMultiEP, right, queryString) {
	...
	var prop = {};
	var isDisplay = _updateResource(_, hit, prop, r, right, queryString);
...
function _makeResource(_, mapping, hit, representation, resFormat, right, queryString) {
	var title = _getTypeTitle(mapping, hit._type);
	// items base url
...
function preProcessSearch(_, q, locale, searchType) {
	...
	for (var t = 0; t < terms.length; t++) {
		...
		var termDate = _convertDateToString(_, term, locale);
...
exports.search = function(_, context, configOverride) {
	...
	q = preProcessSearch(_, q, selectedLocale, searchType);
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		if (hit._type === "function") {
			...
			r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
				resource = _makeFunctionResource(_, e, mappings, hit, endpoints.length > 1, right, q);
				if (resource) {
			...
			resource = _makeResource(_, mappings, hit, context.representation ? context.representation.entity : context.searchEntityName, context.representation.type, right, q);
			if (resource) {
				resultRes.$resources.push(resource);
			} else {
				_removeFacet(hit, context, respData);
