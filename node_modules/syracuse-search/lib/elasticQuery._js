...
exports.search = function(_, context, configOverride) {
	...
	if (mapKeys && mapKeys.length) {
		...
		logger.info() && logger.info("elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	if (!hasNoAccess) {
		...
		logger.debug() && logger.debug("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		logger.info() && logger.info("search " + JSON.stringify(search, null, 2));
