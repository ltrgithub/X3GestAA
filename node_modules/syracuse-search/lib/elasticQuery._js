...
function _updateResource(_, hit, properties, resource) {
	...
	if (hit._type != "function") {
		...
		var prototype = JSON.parse(req.end().response(_).readAll(_));
		prop = prototype._source && prototype._source.$prototype && prototype._source.$prototype.$properties.$resources.$item.$properties;
				...
				if (typeof propItem === "object") {
					if (propItem.$title) {
						var p = {
							...
							$isHighlight: (sHighlight[hh] ? true : false),
							$title: propItem.$title
						};
						var r = sHighlight[hh] ? (sHighlight[hh].length > 1 ? sHighlight[hh] : sHighlight[hh][0]) : sSource[hh.indexOf(".") !== -1 ? hh.split(".").slice(-1) : hh];
						res.properties = p,
						res.resource = r;
					} else {
