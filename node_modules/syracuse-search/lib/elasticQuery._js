...
function _makeFilterFromSdata(where, isRootFilter) {
	...
	function mergeAndOrResult(tab, left, right) {
		...
		case "operator":
			switch (where.value.code) {
				case "and":
					...
					var left = _makeFilterFromSdata(where.children[0], true);
					var right = _makeFilterFromSdata(where.children[1], true);
					var tab;
					if (isRootFilter) {
						result = {
							and: []
					...
					result = mergeAndOrResult(tab, left, right);
...
function _addSdataWhere(query, where) {
	...
	var filter = _makeFilterFromSdata(where);
	filter.map(function(item) {
		if (query.filtered.filter[item]) {
			Object.keys(filter[item]).map(function(k) {
				query.filtered.filter[item].push(filter[item][k]);
			});
		} else {
			query.filtered.filter[item] = filter[item];
		}
	});
	//
	/*var filter = query.filtered.filter = query.filtered.filter || {};
...
function _addX3RightFilter(_, allRight, search, user, endpoint, isFunction) {
	...
	if (repRight && Object.keys(repRight).length > 0) {
		Object.keys(repRight).forEach(function(key) {
			...
			var filter = _makeFilterFromSdata(repRight.condition);
		});
	}
}
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach(function(it) {
			...
			if (right[it].condition) { // can access if condition is ok
				_addSdataWhere(search.query, right[it].condition);
