		...
		if (arr.length === 3) {
			if (arr[2].length === 4) {
				...
				try {
					...
					tracer.trace.debug() && tracer.trace.debug("Parsed date: " + v);
					var d = new Date(v);
					tracer.trace.debug() && tracer.trace.debug("Date: " + d);
					var date = d.getDate();
...
function preProcessSearch(_, q, locale, searchType) {
	...
	for (t = 0; t < terms.length; t++) {
		...
		tracer.trace.debug() && tracer.trace.debug("Date converted to X3 format=> " + termDate);
		if (termDate) {
			term = termDate;
		} else {
...
exports.search = function(_, context, configOverride) {
	...
	tracer.trace.info() && tracer.trace.info("elasticQuery.indexNames: " + sys.inspect(indexNames));
	...
	if (mapKeys && mapKeys.length) {
		...
		tracer.trace.debug() && tracer.trace.debug("elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	tracer.trace.debug() && tracer.trace.debug("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
	...
	if (!hasNoAccess) {
		...
		tracer.trace.debug() && tracer.trace.debug("elasticQuery.search facetsMap: " + sys.inspect(facetsMap, null, 4));
		//
		...
		tracer.trace.info() && tracer.trace.info("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		tracer.trace.debug() && tracer.trace.debug("search " + JSON.stringify(search, null, 2));
		...
		tracer.trace.debug() && tracer.trace.debug("elasticQuery.search response: " + sys.inspect(respData, null, 5));
	}
	//
	if (respData.error) {
