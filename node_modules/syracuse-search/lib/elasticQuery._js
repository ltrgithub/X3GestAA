...
exports.search = function(_, context) {
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		} else resultRes.$resources.push(_makeResource(mappings, hit, context.representation.type));
	});
	...
	function membersMap(item) {
		...
		// filters
		if (filters && filters[facet]) if (Array.isArray(filters[facet]) && (filters[facet].indexOf(item.term) >= 0)) facetMember.$selected = true;
		// links
		return facetMember;
	}
	var facet;
