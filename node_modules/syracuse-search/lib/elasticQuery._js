		...
		if (arr.length === 3) {
			if (arr[2].length === 4) {
				...
				try {
					...
					logger.debug() && logger.debug("Parsed date: " + v);
					var d = new Date(v);
					logger.debug() && logger.debug("Date: " + d);
					var date = d.getDate();
...
function preProcessSearch(_, q, locale, searchType) {
	...
	for (t = 0; t < terms.length; t++) {
		...
		logger.debug() && logger.debug("Date converted to X3 format=> " + termDate);
		if (termDate) {
			term = termDate;
		} else {
...
exports.search = function(_, context, configOverride) {
	...
	logger = loghelper.getTracer("search");
	...
	logger.info() && logger.info("elasticQuery.indexNames: " + sys.inspect(indexNames));
	...
	if (mapKeys && mapKeys.length) {
		...
		logger.debug() && logger.debug("elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	logger.debug() && logger.debug("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
	...
	if (!hasNoAccess) {
		...
		logger.debug() && logger.debug("elasticQuery.search facetsMap: " + sys.inspect(facetsMap, null, 4));
		//
		...
		logger.info() && logger.info("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		logger.debug() && logger.debug("search " + JSON.stringify(search, null, 2));
		...
		logger.debug() && logger.debug("elasticQuery.search response: " + sys.inspect(respData, null, 5));
	}
	//
	if (respData.error) {
