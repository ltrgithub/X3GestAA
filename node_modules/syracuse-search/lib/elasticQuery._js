...
function _addSyracuseRightFilter(_, allRight, search, baseUrlIndex, isFunction) {
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach_(_, function(_, it) {
			var proto = _getPrototypeCache(_, baseUrlIndex + "_entities" + "/$entities/" + it);
			var cond;
...
exports.search = function(_, context, configOverride) {
	...
	if (right && !right.$entities) {
		...
		_addSyracuseRightFilter(_, right, search, urlSegs.join("/"), isFunction);
	}
	var respData = {};
