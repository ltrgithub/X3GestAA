 ...
 */
function _addRightFilterExpression(exp, filter, user, endpoint) {
	var hasNoChildrenOperator = false;
	if (exp.children.length === 2) {
		var term;
		exp.children.forEach(function(item) {
			if (item.type === "operator") {
				term = _addRightFilterExpression(item, filter, user, endpoint);
			}
			if (term.isComplete) { // add to filter
	...
	if (exp.children.length === 2) {
		if (exp.type === "operator") {
			...
			if (exp.value.text === ".") {
				termFilter.filter = {
					termId: exp.children[0].value + "." + exp.children[1].value
				};
			} else {
				termFilter.filter = {
					termOp: exp.value.text,
					termId: exp.children[0].value,
					termValue: exp.children[1].value
				};
				termFilter.isComplete = true;
			}
			return termFilter;
		}
		return null;
...
function _addX3RightFilter(_, allRight, search, user, endpoint, isFunction) {
	...
	if (repRight && Object.keys(repRight).length > 0) {
		Object.keys(repRight).forEach_(_, function(_, key) {
			var item = {};
			item["_type"] = key;
			// filter on type
			search.query.filtered.filter.or.filters.push({
			...
			_addRightFilterCondition(_, allRight, search, user, endpoint, isFunction);


		});
	}
	/*if (search.query.filtered.filter && search.query.filtered.filter.or.filters.length > 0) {
		search.query.filtered.filter.or["_cache"] = true;
		search.query.filtered.filter.or["_cacheKey"] = user.getEndpointLogin(_, endpoint.$uuid) + "_" + (isFunction ? "function" : "data");
	}*/
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach_(_, function(_, it) {
			...
			if (right[it].condition) { // can access if condition is ok
				_addRightFilterCondition(_, right[it].condition, filter, user, endpoint, isFunction);
...
exports.search = function(_, context, configOverride) {
	...
	if (!searchConfig.prototype) {
		...
		// must get all source where $access is empty
		...
		// Combine the results of two queries using the should clause of a bool query
		// to allow exact matches to be boosted above fuzzy matches.
		search = {
			from: context.parameters.startIndex - 1,
			size: context.parameters.count,
			query: {
