...
function preProcessSearch(_, q, locale, searchType) {
	...
	for (t = 0; t < terms.length; t++) {
		...
		exports.tracer && exports.tracer.fine && exports.tracer.trace("Date converted to X3 format=> " + termDate);
		if (termDate) {
			term = termDate;
		} else {
...
function defineExactQuery(searchConfig, q) {
	...
	if (["OR", "AND"].indexOf(q) !== -1) { // apply a query without boolean , we have to search on prefix phase
...
exports.search = function(_, context, configOverride) {
	...
	exports.tracer && exports.tracer.fine && exports.tracer.trace("elasticQuery.indexNames: " + sys.inspect(indexNames));
	...
	if (mapKeys && mapKeys.length) {
		...
		//
		// exports.tracer && exports.tracer.fine && exports.tracer("elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	exports.tracer && exports.tracer.fine && exports.tracer.trace("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
	...
	if (!hasNoAccess) {
		...
		exports.tracer && exports.tracer.fine && exports.tracer.trace("elasticQuery.search facetsMap: " + sys.inspect(facetsMap, null, 4));
		//
		...
		exports.tracer && exports.tracer.fine && exports.tracer.trace("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		exports.tracer && exports.tracer.info && exports.tracer.trace("search " + JSON.stringify(search, null, 2));
		...
		exports.tracer && exports.tracer.fine && exports.tracer.trace("elasticQuery.search response: " + sys.inspect(respData, null, 5));
	}
	//
	if (respData.error) {
