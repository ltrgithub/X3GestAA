...
function _makeFilterFromSdata(where) {
	function _left(where) {
		...
		switch (where.type) {
			...
			default:
				if (where.value === "$id")
					result = where.value.replace('$id', '_id');
				else if (where.value === "$type")
					result = where.value.replace('$type', '_type');
				else
					result = where.value;
...
function preProcessSearch(_, q, up, searchType) {
	...
	for (var t = 0; t < terms.length; t++) {
		...
		var term = terms[t];
		if (term.indexOf("[0-9]\d[\/\-]+[0-9]\d[\/\-]+(19|20)\d\d") > 0) {
			// Regex that should catch all numerical date representations
			term = _convertDateToString(_, term, up);
			exports.tracer && exports.tracer("Date converted to X3 format=> " + term);
		} else {
...
function _addSyracuseRightFilter(_, allRight, search, user, endpoint, isFunction) {
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach(function(it) {
			...
			if (right[it].condition) { // can access if condition is ok
				// replace of $uuid
				var cond = "( " + right[it].condition.replace(".$uuid", ".$key").replace("$uuid", "$id") + " ) and $type eq " + it;
	...
	/*if (search.query.filtered.filter && search.query.filtered.filter.length > 0) {
		search.query.filtered.filter["_cache"] = true;
		search.query.filtered.filter["_cacheKey"] = endpoint.dataset(_) + "_" + user.getEndpointLogin(_, endpoint.$uuid) + "_" + (isFunction ? "function" : "data");
	}*/
...
exports.tsearch = function(_, context, configOverride) {
	//>>>>>>> fb5e54a65d270008543712021be9e1fb645b7d8a
