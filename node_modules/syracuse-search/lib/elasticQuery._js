...
exports.search = function(_, context, configOverride) {
	...
	if (configOverride.recOptions) {
		streamsRecorder.createRecorder(_, streams, configOverride.recOptions, config);
	}
	...
	if (right && !right.cache) { // purge after reload right (new http session)
		...
		options = {
			url: urlSegs.join("/") + "/_cache/clear?filter_keys=syracuse_" + user.login(_) + "_" + (isFunction ? "function" : "data"),
			method: "GET"
		};
