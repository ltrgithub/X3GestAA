...
function _updateResource(_, hit, properties, resource) {
	...
	var options = {
		url: baseUrl + "/" + hit._index + "/$entities/" + hit._type,
		method: "GET",
	};
	var req = streams.httpRequest(options);
	var prototype = JSON.parse(req.end().response(_).readAll(_));
	var prop = prototype._source && prototype._source.$prototype && prototype._source.$prototype.$properties.$resources.$item.$properties;
