...
function _replaceSearchDollar(value) {
	...
	if (value === "$id") {
		...
		result = value.replace('$site', '_site');
		...
		case "operator":
			switch (where.value.code) {
				...
				case "or":
					...
					var left = _makeFilterFromSdata(where.children[0]);
					var right = _makeFilterFromSdata(where.children[1]);
					result = {
						or: [left, right]
					};
					break;
...
function _addFunctionsFilter(_, query, endpoints) {
	...
	endpoints.forEach_(_, function(_, endpoint) {
		where = where + " or (application.$key eq '" + endpoint.$uuid + "' and endpoint eq null) or (endpoint.$key eq '" + endpoint.$uuid + "' and application.$key eq '" + endpoint.applicationRef(_).$uuid + "')";
	});
...
function _addX3RightFilter(_, allRight, search, user, endpoint, isFunction) {
	...
	if (false && rep && Object.keys(rep).length > 0) {
		Object.keys(rep).forEach(function(key) {
			...
			if (Object.keys(repRight).length > 0) {
				//filter with condition
				var cond = "$type eq " + key; //TODO replace key by entity name
	...
	if (queryFilter) {
		...
		if (filter && (keyFilter === "or" || keyFilter === "and")) {
			search.query.filtered.filter[keyFilter] = {
				filters: filter,
				_cache: true,
				_cacheKey: endpoint.dataset(_) + "_" + user.getEndpointLogin(_, endpoint.$uuid) + "_" + (isFunction ? "function" : "data") + (suffixCache ? "_" + suffixCache : "")
...
exports.search = function(_, context, configOverride) {
	...
	if (right && !right.notPurgeCache) { // purge after reload right (new http session)
		options = {
			url: urlSegs.join("/") + "/_cache/clear?filter_keys=" + adminEP.dataset(_) + "_" + user.login(_) + "_" + (isFunction ? "function" : "data"),
			method: "GET"
		};
		httpClient.httpRequest(_, options).end();
		options = {
	...
	if (right && !right.$entities) {
		hasNoAccess = !_addX3RightFilter(_, right, search, user, currentEndpoint, isFunction);
	} else {
	...
	if (!hasNoAccess) {
		...
		suffix = where ? (suffix ? suffix + "_" + where : where) : suffix;
