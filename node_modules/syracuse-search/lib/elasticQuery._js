...
function preProcessSearch(_, q, up, wild) {
...
exports.search = function(_, context, configOverride) {
	...
	var attr = "$access";
	/*
	if (context.parameters.source === "functions") {
		right = up.getFunctionsAuthorizations(_).$functions;
	} else {
		right = up.getAccessCodeAuthorizations(_).$accessCodes;
	}*/
	...
	if (searchConfig.prototype) {
		...
		if (q.indexOf(":") !== 0) {
			...
			// Special case appends wild card to all terms
			var wild = false;
			if (sp[0] === "s") {
				searchType = "x";
				wild = true;
		...
		q = preProcessSearch(_, q, up, wild);
	} else {
		queryDate = _convertDateToString(_, q, up);
	}
	...
	// crnit: this produces a general shard failure beacause of parser error: _source not known
	/*	search._source = {
		exclude: ["$access"]
	};*/
	if (right && Object.keys(right).length) {
		...
		if (attr === "$access") {
			if (context.parameters.source !== "functions") {
				var item = {};
				item[attr] = "all";
				search.query.filtered.filter.or.push({
					term: item
				});
			}
		}
