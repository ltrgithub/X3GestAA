	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		if (hit._type === "function") {
			...
			r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
				var resource = _makeFunctionResource(_, e, mappings, hit, endpoints.length > 1, right);
				if (resource) {
					resultRes.$resources.push(resource);
				} else {
					for (var i = 0; i < respData.facets.type.length && respData.facets.type.terms[i].term === hits._type; i++) {}
					respData.facets.type.terms[i].count--;
					context.totalCount--;
				}
			});
		} else {
			var resource = _makeResource(_, mappings, hit, context.representation ? context.representation.entity : context.searchEntityName, context.representation.type, right);
			if (resource) {
				resultRes.$resources.push(resource);
			} else {
				for (var i = 0; i < respData.facets.type.length && respData.facets.type.terms[i].term === hits._type; i++) {}
