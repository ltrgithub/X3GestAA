...
function _updateResource(_, hit, properties, resource, right, queryString, context) {
	...
	if (type === "function") {
		type = "menuItem";
		index = elasticHelpers.getIndexNameWithMultitenantOrCloudV2Prefix(adminEP.application(_) + "." + adminEP.contract(_) + "." + adminEP.dataset(_) + locale);
...
function _selectIndexes(_, elasticBaseUrl, selectedEndpoints, sources, selectedIndexName) {
	...
	function _addIndex(_, indexName, localeCode) {
		var idx = elasticHelpers.getIndexNameWithMultitenantOrCloudV2Prefix(indexName + "." + localeCode.toLowerCase());
	...
	function _addEP(_, ep, localeCode) {
		var idx = elasticHelpers.getIndexNameWithMultitenantOrCloudV2Prefix(ep.getIndexName(_, localeCode || locale.current));
		if (elasticHelpers.indexExists(_, elasticBaseUrl, idx)) {
			indexes.push(idx);
		} else if (localeCode !== elasticHelpers.fallbackLocale) {
