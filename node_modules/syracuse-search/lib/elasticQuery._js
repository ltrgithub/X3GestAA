		...
		case "operator":
			switch (where.value.code) {
				...
				case "=":
					...
					if (exports.elasticVersion && exports.elasticVersion[0] === 1) {
						// key = "query_string";
						if (val == null) {
							result.missing = {
							...
							result.query = {
								query_string: {
									default_field: _left(where.children[0]),
									query: (val && val === "$all" ? "_all" : val)
								}
							};
...
function _addX3RightFilter(_, allRight, search, isFunction) {
	...
	if (rep && Object.keys(rep).length > 0) {
		...
		Object.keys(listEntity).forEach(function(key) {
			if (!isFunction) {
				var c = attr + " eq " + listEntity[key].type;
				...
				functionRegexp = !functionRegexp ? listEntity[key].type.toLowerCase() : functionRegexp + "|" + listEntity[key].type.toLowerCase();
			}
		});

		if (isFunction) {
			_addRegexpFilter(search.query, attr, functionRegexp);
...
exports.search = function(_, context, configOverride) {
	...
	if (diag && diag.length === 0) {
		...
		try {
			...
			exports.elasticVersion = elasticVersion.getElasticVersion(_, urlSegs.join("/"));
		} catch (e) {
			diag && diag.push({
				$severity: "error",
				$message: e.message
	...
	var indexNames = _selectIndexes(_, elasticBaseUrl, up, source, (filters && filters.index && filters.index[0]) || context.searchIndexName);
	...
	if (!hasNoAccess) {
		...
		if (applyFilter && currentEndpoint) {
			...
			_addCacheFilter(_, search, user, currentEndpoint, isFunction, right, urlSegs, suffix);
		}
