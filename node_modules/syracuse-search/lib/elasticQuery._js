...
function _updateResource(_, hit, properties, resource, right, queryString, context) {
	...
	function _fromSource(src, kk) {
		...
		if (i === ktab.length && srcVal) {
			...
			// hack right on column will be manage directly in elastic search
			var resStr = res;
			if (typeof res !== "string" && typeof res !== "object" && typeof res !== "object") {
				resStr = String(res);
			}
			if (resStr === queryString) { // hack
				res = res;
				hasHighlightProperty = true;
			}
			return res;
	...
	if (prop) {
		...
		Object.keys(prop).map(function(k) {
			...
			function getRecord(propItem, hh, isInArray, context) {
				var res = {
					properties: {},
					resource: {}
				};
					...
					switch (propItem.$type) {
						...
						case "application/x-object":
							if (propItem.$item.$type === "application/x-reference") {
								res = getRecord(propItem.$item, hh, true, context);
								...
								if (psub) {
									Object.keys(psub).map(function(key) {
										...
										var resp = getRecord(psub[key], hl, true, context);
										if (resp) {
				...
				if (context.parameters && context.parameters.noHighlight) {
					if (res.resource) {
						if (typeof res.resource === "string") {
							res.resource.replace('<em>', '').replace('</em>', '');
						} else if (res.resource.length) {
							res.resource.forEach(function(item, i) {
								res.resource[i] = item.replace('<em>', '').replace('</em>', '');
							});
						}
					}
				}
			...
			var result = getRecord(prop[k], hh, false, context);
			if (result) {
...
function _addX3RightFilter(_, allRight, search, isFunction) {
	...
	if (rep && Object.keys(rep).length > 0) {
		...
		Object.keys(rep).forEach(function(key) { // watch all representation and define condition for entity
			...
			if (repRight.$conditions && repRight.$conditions.length > 0 && !isFunction) {
			...
			if (repRight.$accessCodes && repRight.$accessCodes.length > 0 && !isFunction) {
			...
			if (repRight.$sites && repRight.$sites.length > 0 && !isFunction) {
...
exports.search = function(_, context, configOverride) {
	...
	var edRight = context.endpoint || currentEndpoint;
		...
		respData.hits.hits.forEach_(_, function(_, hit) {
			...
			if (hit._type === "function") {
				...
				r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
					resource = _makeFunctionResource(_, e, mappings, hit, endpoints.length > 1, right, q, context);
					if (resource) {
				...
				resource = _makeResource(_, mappings, hit, context.representation ? context.representation.entity : context.searchEntityName, context.representation.type, right, q, context);
				if (resource) {
					resultRes.$resources.push(resource);
				} else {
					_removeFacet(hit, context, respData);
