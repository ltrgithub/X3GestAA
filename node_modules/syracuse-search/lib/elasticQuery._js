...
function _updateResource(_, hit, properties, resource) {
	...
	if (hit._type != "function") {
		options.url = baseUrl + "/" + hit._index + "/$entities/" + hit._type;
		if (_prototype[options.url]) {
			...
			prop = proto._source && proto._source.$prototype && proto._source.$prototype.$properties.$resources.$item.$properties;
		...
		Object.keys(prop).map(function(k) {
			...
			function getRecord(propItem, hh) {
				...
				if (typeof propItem === "object") {
					if (propItem.$title) {
						var p = {
							$isSummary: propItem.$isSummary ? true : false,
						...
						Object.keys(propItem.$item.$properties).map(function(key) {
							hh += hh !== "" ? "." + key : key;
							var resp = getRecord(propItem.$item.$properties[key], hh);
...
exports.search = function(_, context) {
	...
	var editDistance = config.searchEngine && config.searchEngine.editDistance ? config.searchEngine.editDistance : null;
	...
	if (q.indexOf("*") >= 0 || q.indexOf("[") >= 0) {
		...
		for (var t = 0; t < terms.length; t++) {
			...
			var tlower = terms[t].toLowerCase();
			if (tlower == "or" || tlower == "and") {
				...
				if (!isAroundQuote && terms[t].length > 0) {
					qstr += "~" + (editDistance ? editDistance : ""); //fuzzy each word
				}
			}
		}
	}
	...
	search.query.filtered.query.query_string = {
		default_field: "_all",
		query: isAroundQuote ? qstr + "~" + (editDistance ? editDistance : "") : qstr, // fuzzy all string if around quote
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		if (hit._type !== "$entities") {
			...
			} else resultRes.$resources.push(_makeResource(_, mappings, hit, context.representation.entity, context.representation.type));
		}
	});
