...
exports.search = function(_, context) {
	...
	var fuzzyDistance = config.searchEngine && config.searchEngine.fuzzyDistance ? config.searchEngine.fuzzyDistance : 1;
	...
	if (q.indexOf("*") >= 0 || q.indexOf("[") >= 0) {
		...
		for (var t = 0; t < terms.length; t++) {
			...
			if (tlower == "or" || tlower == "and") {
				...
				qstr += terms[t];
				if (!isAroundQuote) {
					qstr += "~" + fuzzyDistance; //fuzzy each word
				}
			}
		}
	}
	...
	search.query.filtered.query.query_string = {
		default_field: "_all",
		query: isAroundQuote ? qstr + "~" + fuzzyDistance : qstr, // fuzzy all string if around quote
