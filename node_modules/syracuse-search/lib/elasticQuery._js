...
function _makeFilterFromSdata(where) {
	...
	function getBranchElement(operator, left, right) {
		...
		if (left) {
			if (left[operator]) {
				getBranchElement(operator, left[operator][0], left[operator][1]).forEach(function(item) {
					tab.push(item);
				});
			} else {
				tab.push(left);
		...
		if (right) {
			if (right[operator]) {
				getBranchElement(operator, right[operator][0], right[operator][1]).forEach(function(item) {
					tab.push(item);
				});
			} else {
				tab.push(right);
		...
		case "operator":
			switch (where.value.code) {
				case "and":
					...
					var res = getBranchElement(where.value.code, _makeFilterFromSdata(where.children[0]), _makeFilterFromSdata(where.children[1]));
					result = {
						and: res
					};
					break;
				...
				case "=":
					...
					break;
				case "like":
					right = where.children[1] && _format(where.children[1].value, isDate);
