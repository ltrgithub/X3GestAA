...
function _addRightFilter(_, allRight, search, user, endpoint, isFunction) {
	var attr = "$access";
	...
	if (repRight && Object.keys(repRight).length > 0) {
		Object.keys(repRight).map(function(key) {
			var item = {};
			item["_type"] = key;
			// filter on type
			search.query.filtered.filter.or.filters.push({
...
exports.search = function(_, context, configOverride) {
	...
	/*if (context.parameters.source === "functions") {
		right = up.getFunctionsAuthorizations(_);
	} else {
		right = up.getAccessCodeAuthorizations(_);
	}*/
	...
	_addRightFilter(_, right, search, up.user(_), up.selectedEndpoint(_), context.parameters.source === "functions");
