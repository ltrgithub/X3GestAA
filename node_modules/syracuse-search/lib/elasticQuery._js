...
//var indexMeta = require('./indexMeta');
...
// for unit test 
exports.setElasticVersion = function(vers) {
	elasticVersionFilter = vers;
};
...
function _getTypeTitle(mapping, typeName) {
	...
	Object.keys(mapping).forEach(function(idx) {
		var map;
		if (elasticVersionFilter && elasticVersionFilter[0] < 1) {
			map = mapping[idx][typeName]
		} else {
			map = mapping[idx]["mappings"][typeName];
		}
...
function _addFacets(query, contract) {
	...
	forEachKey(contract.searchFacets, function(key, facet) {
		if (!elasticVersionFilter || elasticVersionFilter[0] < 2) {
			...
			query.facets[key] = {
				terms: {
					fields: [key == "type" ? "_type" : "facet_" + key]
				}
			};
		} else {
			...
			query.aggs[key] = {
				terms: {
					field: key == "type" ? "_type" : "facet_" + key
				}
			};
		}
	});
...
function _getQueryObject(query) {
	...
	var q = query;
	if (!elasticVersionFilter || elasticVersionFilter[0] < 2) {
		q.filtered = q.filtered || {};
		q.filtered.query = q.filtered.query || {};
		return q.filtered;
...
function _makeFilterFromSdata(where) {
		...
		switch (where.type) {
			case "operator":
				switch (where.value.code) {
					case ".":
						var r = _left(where.children[1]);
						var l = _left(where.children[0]);
						if (elasticVersionFilter && elasticVersionFilter[0] >= 2) {
							l += "_ref";
						}
						result = l + "." + r;
	...
	function _format(child, isDate) {
		...
		if (child.type === "identifier")
			return value.replace("$all", "_all");
		return ((typeof value === "object") && value.toString()) ? value.toString() : value;
	}

	var result = {};
	// isDate seems to be always false. Is it normal?
	var isDate = false;
	var r, res, val;
	switch (where.type) {
		case "operator":
			switch (where.value.code) {
				case "and":
					...
					res = getBranchElement(where.value.code, where.children[0], where.children[1]);
					result = {
						and: res
					};
					break;
				case "or":
					...
					break;
				case "=":
					val = _format(where.children[1], isDate);
					// check version in order to generate the right filter - breaking change betwenn 0.90 and 1.0.0
					if (elasticVersionFilter && elasticVersionFilter[0] >= 1) { // version > 1.0.0
						if (val == null) {
							result.missing = {
								field: _left(where.children[0]) + (elasticVersionFilter && elasticVersionFilter[0] >= 2 ? "_ref" : "")
							};
						} else {
							...
							result.query.term[_left(where.children[0])] = v;
						}
					} else { // version < 1.0.0
						if (val == null) {
							result.missing = {
					...
					break;
				case "like":
					// Avoid format on regexp - case sensitive
					if (where.children[1]) {
						if (where.children[1].type == "literal") r = where.children[1].value;
						else r = _format(where.children[1], isDate);
					}
				...
				case "<>":
					val = _format(where.children[1], isDate);
					if (val === null) { // can be field or a object. in both case null value is not indexed by elastic
						result.not = {
							missing: {
								field: _left(where.children[0])
							}
						};
					} else { // it's normally a field on the structure
						result.not = {
							term: {}
						};
						result.not.term[_left(where.children[0])] = val;
					}
					break;
				case "in":
					r = where.children.slice(1).map(function(c) {
						return c && c.value && _format(c, isDate);
					});
					result.terms = {};
				...
				default:
					...
					if (["<", "<=", ">=", ">", "between"].indexOf(where.value.code) > 0) {
						...
						r = _format(where.children[1], isDate);
						switch (where.value.code) {
							case "<":
								l.lt = r;
								break;
							...
							case "between":
								l.gte = r;
								l.lte = _format(where.children[2], isDate);
								break;
						}
					}
			}
...
function _addFunctionsFilter(_, query, endpoints) {
	...
	var where = app + " eq null";

	// Ensure that the functions returned are valid for the user
	endpoints.forEach_(_, function(_, endpoint) {
		where = where + " or (" + app + ".$key eq '" + endpoint.applicationRef(_).$uuid + "' and " + ed + " eq null) or (" + ed + ".$key eq '" + endpoint.$uuid + "' and " + app + ".$key eq '" + endpoint.applicationRef(_).$uuid + "')";
	});
	var adminEP = adminHelper.getCollaborationEndpoint(_);
	where = where + " or (" + app + ".$key eq '" + adminEP.applicationRef(_).$uuid + "' and " + ed + " eq null) or (" + ed + ".$key eq '" + adminEP.$uuid + "' and " + app + ".$key eq '" + adminEP.applicationRef(_).$uuid + "')";
...
function _formatValueDecimal(_, val, item, locale) {
	...
	if (val && item.$maxLength) {
		...
		return decimal.format(val, format) + "";
	}
	return val;
}
...
function _updateResource(_, hit, properties, resource, right, queryString, context) {
	...
	function _fromSource(src, kk) {
		...
		if (i === ktab.length && srcVal) {
			...
			if (res.toString().toLowerCase().indexOf(queryString.toLowerCase()) !== -1) { // hack
				hasHighlightProperty = true;
			}
			return res;
		}
	...
	var sHighlight = {};
	hit && hit.highlight && Object.keys(hit.highlight).forEach(function(item) {
		sHighlight[item.replace("_ref", "")] = hit.highlight[item];
	});


	var baseUrl = [searchEngine.getBaseUrl()].join('/');
	...
	if (type === "function") {
		type = "menuItem";
		index = elasticHelpers.getFullIndexName(adminEP.application(_) + "." + adminEP.contract(_) + "." + adminEP.dataset(_) + locale);
	...
	if (prop) {
		...
		Object.keys(prop).forEach_(_, function(_, k) {
			...
			// Hack to avoid display of securityLevel 
			if (hh == "securityLevel" && type == "user") return null;
			...
			function getRecord(_, propItem, hh, isInArray, context) {
				...
				if (typeof propItem === "object") {
					...
					switch (propItem.$type) {
						case "application/x-reference":
							...
							res.properties = {
								$isSummary: propItem.$isSummary || propItem.$item.$type === "application/x-date" || isGlobalSummary ? true : false,
								$isHighlight: (sHighlight && (sHighlight[hh] || sHighlight[hh + ".$value"]) ? true : false),
								$title: propItem.$title
							};
							resVal = _fromHighlight(sHighlight, hh, isInArray) || _fromHighlight(sHighlight, hh + ".$value", isInArray) || _fromSource(sSource, hh) || _fromSource(sSource, hh + ".$value");
						...
						case "application/x-object":
							...
							if (propItem.$item.$type === "application/x-reference") {
								res = getRecord(_, propItem.$item, hh, propItem.$type === "application/x-array", context);
								...
								if (psub) {
									Object.keys(psub).forEach_(_, function(_, key) {
										...
										var resp = getRecord(_, psub[key], hl, propItem.$type === "application/x-array", context);
										if (resp) {
						...
						default:
							...
							resVal = _fromHighlight(sHighlight, hh, isInArray) || _fromSource(sSource, hh);
							res.resource = mustFormatServerSide ? _formatValue(_, resVal, propItem, context) : resVal;
				...
				if (context.parameters && context.parameters.noHighlight) {
					if (res.properties.$isHighlight) {
						var reg1 = /<em>/g; // new RegExp('<em>', 'g');
						var reg2 = /<\/em>/g; // new RegExp('</em>', 'g');
...
function _makeFunctionResource(_, endpoint, mapping, hit, isMultiEP, right, queryString, context) {
	...
	function _getMethod() {
		return ((hit._source.linkType === "$representation") && (["$create", "$edit"].indexOf(hit._source.facet) >= 0)) ? "POST" : "GET";
	}
	...
	var appKey = hit._source.application && hit._source.application.$key;
	if (elasticVersionFilter && elasticVersionFilter[0] >= 2) {
		appKey = hit._source.application_ref && hit._source.application_ref.$key;
	}
	var r = {
		$url: hit._source.$url,
		$method: _getMethod(),
		$baseUrl: appKey === endpoint.applicationRef(_).$uuid ? endpoint.getBaseUrl(_) : adminEP.getBaseUrl(_),
		$uuid: hit._id,
...
function _selectIndexes(_, elasticBaseUrl, selectedEndpoints, sources, selectedIndexName) {
	var indexes = [];

	function _addIndex(_, indexName, localeCode) {
		var idx = elasticHelpers.getFullIndexName(indexName + "." + localeCode.toLowerCase());
	...
	function _addEP(_, ep, localeCode) {
		var idx = elasticHelpers.getFullIndexName(ep.getIndexName(_, localeCode || locale.current));
		if (elasticHelpers.indexExists(_, elasticBaseUrl, idx)) {
			indexes.push(idx);
		} else if (localeCode !== elasticHelpers.fallbackLocale) {
	...
	if (sources.indexOf("data") >= 0) {
		if (selectedIndexName) {
			...
			if (selectedEndpoints) {
				selectedEndpoints.forEach_(_, function(_, ep) {
					_addEP(_, ep);
				});
			}
		}
...
function _selectEndpoints(_, userProfile) {
	...
	if (userProfile) {
		if (userProfile.selectedEndpoint(_)) {
			eps.push(userProfile.selectedEndpoint(_)); // send only the selectedEndpoint
		}
	}
	return eps;
}
		...
		if (arr.length === 3) {
			if (arr[2].length === 4) {
				...
				try {
					...
					tracer.debug && tracer.debug("Parsed date: " + v);
					var d = new Date(v);
					tracer.debug && tracer.debug("Date: " + d);
					var date = d.getDate();
...
function _addX3RightFilter(_, allRight, search, isFunction) {
	...
	var accessRegexp = "";
	var entityRegexp = "";
	var q = _getQueryObject(search.query);
	if (right && right !== "*" && Object.keys(right).length > 0) { //simple right
		...
		if (accessRegexp) {
			q.filter = {};
			accessRegexp = accessRegexp + "|_all|all"; // add all for manage chinese that use a special analyzer. With luncene 5 chinese will use the standard analyser that will solve the problem
		} else {
			// can't access to data except those that are accessible by all
			accessRegexp = "_all|all"; // add all for manage chinese that use a special analyzer. With luncene 5 chinese will use the standard analyser that will solve the problem
		}
	}
	...
	if (rep && Object.keys(rep).length > 0) {
		...
		Object.keys(rep).forEach(function(key) { // watch all representation and define condition for entity
			...
			if (Object.keys(listEntity).indexOf(entityName) === -1) { // entity name not treate add default type condition
				...
				listEntity[entityName] = {
					type: attr + " eq " + (isFunction ? entityName.toLowerCase() : entityName),
					site: "",
					condition: "",
					access: ""
				};
		...
		/* if( isFunction ){// change representationREf.entity as a webex - no condition added
            Object.keys(listEntity).forEach(function (key) {
                var c = key.toLowerCase();
                entityRegexp = entityRegexp + (!entityRegexp ? c : "|" + c);
            });
        }else {*/
		Object.keys(listEntity).forEach(function(key) {
	...
	if (entityRegexp) {
		_addRegexpFilter(search.query, attr, entityRegexp);
	}
	if (accessRegexp) {
...
function preProcessSearch(_, q, locale, searchType) {
	...
	for (t = 0; t < terms.length; t++) {
		...
		tracer.debug && tracer.debug("Date converted to X3 format=> " + termDate);
		if (termDate) {
			term = termDate;
		} else {
...
function defineStartWithQuery(query, searchConfig, q, search) {
	var allowLeadifFng = (searchConfig.allowLeadingWildcard) ? searchConfig.allowLeadingWildcard : false;
	var query;
	if (q.indexOf("OR") === -1 && q.indexOf("AND") === -1) { // apply a query without boolean , we have to search on prefix phase
		if (elasticVersionFilter && elasticVersionFilter[0] >= 2) {
			query.must = {
				"multi_match": {
					"query": q,
			...
			query.multi_match.type = "phrase_prefix";
		}
...
function defineExactQuery(searchConfig, q) {
	...
	if (q.indexOf("OR") === -1 && q.indexOf("AND") === -1) { // apply a query without boolean , we have to search on prefix phase
		query.query_string.type = "phrase_prefix";
...
function defineExactPlusFuzzyQuery(query, searchConfig, q) {
	var minSimilarity = (searchConfig.minSimilarity) ? searchConfig.minSimilarity : 0.5;
	var ignoreFrequency = (searchConfig.ignoreFrequency) ? searchConfig.ignoreFrequency : true;
	var allowLeading = (searchConfig.allowLeadingWildcard) ? searchConfig.allowLeadingWildcard : false;
	...
	// TODO will be remove when we will decide to don't support elastic search < 1.5, maybe we can add a diagnoses to ask admin to update the elastic search server
	if (elasticVersionFilter && (elasticVersionFilter[0] === 1 && elasticVersionFilter[1] >= 5)) { // if > 1.5 use function_score key else custome_boost_factor
		query.bool.should.push({
			function_score: functionScore
		});
	} else if (elasticVersionFilter && elasticVersionFilter[0] < 1) {
		query.bool.should.push({
			custom_boost_factor: functionScore
		});
	}
...
function _addCacheFilter(_, search, user, endpoint, isFunction, right, urlSegs, suffixCache) {
	function purgeCache(_, cacheKey) {
		httpClient.httpRequest(_, {
			url: urlSegs.join("/") + "/_cache/clear?filter_keys=" + cacheKey,
			method: "GET"
		}).end().response(_);
	}

	var q = _getQueryObject(search.query);
	var queryFilter = q.filter;
	if (queryFilter) {
		...
		if (filter && (keyFilter === "or" || keyFilter === "and")) {
			...
			var ref = right.$etag; // last stamp fo right change for the endpoint
			var mustPurgeCache = !(cacheFilterUser[userLog][cacheKey]) || cacheFilterUser[userLog][cacheKey] != ref;

			if (mustPurgeCache) {
				cacheFilterUser[userLog][cacheKey] = ref;
				// purge cache
				purgeCache(_, cacheKey);
			}
...
function _addFilterMustNot(_, search, where) {
	//TODO create json term query *
	var cond = _makeFilterFromSdata(parser.parse(where));
	//TODO add to must_not property in the query
	search.query.bool.must_not = search.query.bool.must_not || [];
	search.query.bool.must_not.push(cond.not);
}
...
function _addSyracuseRightFilter(_, allRight, search, baseUrlIndex, isFunction, prototypeUnit) {
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach_(_, function(_, it) {
			var its = isFunction ? pluralize(it) : it;
			...
			if (right[it].condition) { // can access if condition is ok
				...
				if (!isFunction) {
					...
					cond = "( " + filterElastic + " ) and " + attr + " eq " + its;
				} else {
					cond = attr + " eq " + its; // can access to the function
				...
				var condnot = " not ( " + attr + " eq " + its + " )";
				if (elasticVersionFilter && elasticVersionFilter[0] >= 2) {
					_addFilterMustNot(_, search, condnot);
				} else {
					cond = condnot;
				}
			}
...
exports.search = function(_, context, opts) {
	// Allow config from nodelocal.js to be overridden to make sure that
	// unit tests have the correct config settings

	var searchConfig = helpers.object.extend(searchEngine.config(), opts || {}, true);
	var keepInContext = searchConfig.keepInContext;
	var urlSegs = [searchEngine.getBaseUrl()];
	var diag = [];
	var req, options = {};

	elasticHelpers.checkStartSearchEngine(_, searchEngine.getBaseUrl(), diag, true, true);
	if (diag && diag.length === 0) {
		// Check the version of elastic search
		try {
			elasticVersion.checkVersion(_, urlSegs.join("/"));
			elasticVersionFilter = elasticVersion.getElasticVersion(_, urlSegs.join("/"));
		} catch (e) {
			diag && diag.push({
				$severity: "error",
				$message: e.message
	...
	if (diag && diag.length !== 0) {
		return context.reply(_, httpHelpers.httpStatus.NotFound, diag[0].$message);
	}
	...
	if (!searchConfig.allowLeadingWildcard) {
		if (hasLeadingWildcard(q)) {
			return context.reply(_, httpHelpers.httpStatus.NotFound, locale.format(module, "badSearchString", q));
		}
	}
	// Sort out the parameters etc
	var source = context.parameters.source || "data";
	...
	if (context.parameters.profile) {
		var parsedUserProfile = jsurl.parse(context.parameters.profile);
		var db = adminHelper.getCollaborationOrm(_);
		up = db.model.getEntity(_, "userProfile").factory.createInstance(_, null, db);
		up.loadUserProfile(_, user, selectedLocale, parsedUserProfile);
		user = up.user(_);
		selectedLocale = up.selectedLocale(_);
	}
	...
	var endpoints = _selectEndpoints(_, up); // TODO normally only one endpoint for a search - clean call of selectEndpoints

	// get url app/cnt/dataset to check if the endpoint is the selectEndpoint of user profile
	var suburl = context.request && context.request._request && context.request._request.url;
	if (suburl && suburl.indexOf("/sdata/syracuse/search/syracuse") === -1) { // check if the selectedEndpoint correspond the current used
		var urlApps = suburl.substring(suburl.indexOf("/sdata/") + 7).split("/");
		if (urlApps.length >= 3) {
			var epUrl = adminHelper.getEndpoint(_, {
				dataset: urlApps[2]
			});
			if (epUrl) {
				endpoints = [epUrl];
	...
	} // else fulltext search use selectedEndpoint
	if (!endpoints.length) { // no selected endpoint we disable this feature for prevent security issue about right
		return context.reply(_, httpHelpers.httpStatus.BadRequest, locale.format(module, "noSelectedEndpoint"));
	}

	var indexNames = _selectIndexes(_, elasticBaseUrl, endpoints, source, (filters && filters.index && filters.index[0]) || context.searchIndexName);
	tracer.info && tracer.info("elasticQuery.indexNames: " + sys.inspect(indexNames));
	if (!indexNames.length) {
		return context.reply(_, httpHelpers.httpStatus.NotFound, locale.format(module, isFunction ? "noFunctionIndexAvailable" : "noDataIndexAvailable", up.selectedEndpoint(_).description(_), locale.current));
	}
	// get authorized function and authorization from x3
	// issue SAM 101135
	var isCollaboration = context.request && context.request._request && context.request._request.url.indexOf("/sdata/syracuse/collaboration/syracuse/") !== -1;
	var edRight = endpoints[0];

	var right = up.getAccessRightAuthorizations(_, edRight);
	if (right && right.$diagnoses) {
		return context.reply(_, httpHelpers.httpStatus.BadRequest, locale.format(module, "X3Right", right.$diagnoses[0].$message));
	}
	if (right && right.purgeCache) { // purge after reload right (new http session)

		var userLog = user.getEndpointLogin(_, edRight.$uuid);
		if (cacheFilterUser[userLog]) { // clear all filter cached for current user - user relogged
			Object.keys(cacheFilterUser[userLog]).forEach_(_, function(_, key) {
				options = {
					url: urlSegs.join("/") + "/_cache/clear?filter_keys=" + key,
					method: "GET"
				};
				httpClient.httpRequest(_, options).end().response(_);
			});
			delete cacheFilterUser[userLog]; // no more cache executed
	...
	if (mapKeys && mapKeys.length) {
		...
		tracer.debug && tracer.debug("elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	if (elasticVersionFilter && elasticVersionFilter[0] >= 2) {
		if (searchType != "fuzzy") {
			...
			// match only the query
			search.highlight.fields["*"] = queryHigh;
		}

		search.highlight.require_field_match = false;
	...
	indexNames.forEach_(_, function(_, indexName) {
		facetsMap = elasticHelpers.getIndexFacets(_, elasticBaseUrl, indexName, null).reduce(function(prev, f) {
			prev[f.code] = f;
			return prev;
		}, facetsMap);
	});
	...
	switch (searchType) {
		case "fuzzy":
			...
			defineExactPlusFuzzyQuery(q, searchConfig, qProcessed.fulltext);
			break;
		case "exact":
			defineExactQuery(q, searchConfig, qProcessed.fulltext);
		...
		default:
			...
			defineStartWithQuery(q, searchConfig, qProcessed.fulltext, search);
			break;
	}
	tracer.debug && tracer.debug("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
	...
	if (!hasNoAccess) {
		...
		tracer.debug && tracer.debug("elasticQuery.search facetsMap: " + sys.inspect(facetsMap, null, 4));
		//
		...
		if (!elasticVersionFilter || elasticVersionFilter[0] < 2) {
			if (applyFilter && right) {
				var suffix = suffixFilterCache ? suffixFilterCache : null;
				_addCacheFilter(_, search, user, edRight, isFunction, right, urlSegs, suffix);
			}
		}
		tracer.debug && tracer.debug("elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		tracer.info && tracer.info("search " + JSON.stringify(search, null, 2));
		...
		tracer.debug && tracer.debug("elasticQuery.search response: " + sys.inspect(respData, null, 5));
	}
	//
	if (respData.error) {
		// manage error stackOverflowError in order to explain to increase stack size value regarding the filter # elastic search issue #9127
		if (Array.isArray(respData.error) && respData.error.indexOf("StackOverflowError") >= 0) {
			return context.reply(_, respData.status, locale.format(module, "stackOverflowError"));
		} else if (respData.error && respData.error.reason) {
			return context.reply(_, respData.status, locale.format(module, "searchEngineError", respData.error.reason));
		} else {
	...
	function _removeFacet(hit, context, respData) {
		Object.keys(respData.facets).forEach(function(facetKey) {
			// remove multiple facet
			if (hit._source["facet_" + facetKey] && respData.facets[facetKey] && respData.facets[facetKey] && respData.facets[facetKey].terms && respData.facets[facetKey].terms.length >= 1) {
				var toDel = [];
				for (var i = 0; i < respData.facets[facetKey].terms.length; i++) {
					if (respData.facets[facetKey].terms[i].term === hit._source["facet_" + facetKey]) {
						respData.facets[facetKey].terms[i].count--;
						if (respData.facets[facetKey].terms[i].count === 0) {
							toDel.push(i);
						}
					}
				}
				toDel.forEach(function(i) {
					respData.facets[facetKey].terms.splice(i, 1);
				});
		...
		var toDel = [];
		for (var i = 0; i < respData.facets.type.terms.length; i++) {
			if (respData.facets.type.terms[i] && respData.facets.type.terms[i].term === hit._type) {
				respData.facets.type.terms[i].count--;
				if (respData.facets.type.terms[i].count === 0) {
					toDel.push(i);
				}
			}
		...
		toDel.forEach(function(i) {
			respData.facets.type.terms.splice(i, 1);
		});
	...
	function membersMap(item, facet) {
		var facetMember = {
			$title: item.term || item.key,
			$value: item.term || item.key,
			$count: item.count || item.doc_count,
			$links: {}
		};
		if (facet === "type") {
			facetMember.$title = _getTypeTitle(mappings, facetMember.$value);
		}
		// filters
		if (filters && filters[facet]) {
			if (Array.isArray(filters[facet]) && (filters[facet].indexOf(facetMember.$value) >= 0)) {
	...
	function addFacets() {
		...
		for (var facet in respData.facets) {
			resultRes.$searchFacets.push({
				$facet: facet,
				$title: facetsMap[facet].description,
				$missing: respData.facets[facet].missing,
				$members: (respData.facets[facet].terms || []).map(item => membersMap(item, facet))
			});
		}
	...
	function addAggregations() {
		resultRes.$searchFacets = [];
		for (var agg in respData.aggregations) {
			resultRes.$searchFacets.push({
				$facet: agg,
				$title: facetsMap[agg].description,
				$missing: respData.aggregations[agg].missing,
				$members: (respData.aggregations[agg].buckets || []).map(item => membersMap(item, agg))
			});
		}
	}
