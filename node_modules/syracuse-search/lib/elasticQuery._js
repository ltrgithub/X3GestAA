...
function _addX3RightFilter(_, allRight, search, isFunction) {
	...
	var listEntity = [];
	if (false && rep && Object.keys(rep).length > 0) {
		Object.keys(rep).forEach(function(key) {
			...
			if (listEntity.indexOf(entityName) === -1 && Object.keys(repRight).length > 0) {
				//filter with condition
				var cond = "$type eq " + entityName;
				repRight.$conditions && repRight.$conditions.forEach(function(item) {

					if (item.$type === "restriction") {
						cond = cond + " and not (" + item.$where + ")";
					} else if (item.$type === "authorization") {
						cond = cond + " and " + item.$where;
					}
				});

				//filter on access code
				cond = cond + " or $access eq $all";
				repRight.$accessCodes && repRight.$accessCodes.split(",").forEach(function(item) {
					cond = cond + " or $access eq " + item;
				});

				// add restriction on site only if there are some in database
				repRight.$sites && repRight.$sites.split(",").forEach(function(item) {
					cond = cond + " or $site eq " + item;
				});

				condition = condition ? condition + " or (" + cond + ")" : "(" + cond + ")";
			}
			listEntity.push(entityName);
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach(function(it) {
			...
			if (right[it].condition && !isFunction) { // can access if condition is ok
				// replace of $uuid
				cond = "( " + right[it].condition.replace(".$uuid", ".$key").replace("$uuid", "$id") + " ) and " + attr + " eq " + it;
...
exports.search = function(_, context, configOverride) {
	...
	if (!indexNames.length) {
		return context.reply(_, httpHelper.HttpStatus.NotFound, locale.format(module, isFunction ? "noFunctionIndexAvailable" : "noDataIndexAvailable", locale.current, elasticHelpers.fallbackLocale));
	}
	//
