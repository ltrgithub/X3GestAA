...
/*
 * transform  condition filter parsed by Parser in profile.json into elastic search object filter
 */
function _addRightFilterExpression(exp, filter, user, endpoint, current) {
	var hasNoChildrenOperator = false;
	if (exp.children) {
		var currentTerm = current || {};
		exp.children.forEach(function(item) {
			var res = _addRightFilterExpression(item, filter, user, endpoint, currentTerm);

			if (currentTerm && currentTerm.isComplete) { // add to filter
				var obj = {};

				filter[item.value.text] = filter[item.value.text] || [];
				obj[currentTerm.termId] = currentTerm.termValue;
				filter[item.value.text].push(obj);

				console.log("filter " + JSON.stringify(filter, null, 2));
				console.log("term " + JSON.stringify(currentTerm, null, 2));
				currentTerm = {};
			} else if (currentTerm && !currentTerm.isComplete) {
				//merge if necessary lastTerm with term
				if (res) {
					if (exp.value.text === ".") {
						currentTerm.termId = (!currentTerm.termId ? res.termId : currentTerm.termId + "." + res.termId);
					} else {
						currentTerm.termId = (currentTerm.termId || res.termId);
					}
					currentTerm.termValue = currentTerm.termValue || res.termValue;
					currentTerm.isComplete = currentTerm.termValue && currentTerm.termId;
					console.log("currentTerm " + JSON.stringify(currentTerm, null, 2));
				}

			}
		});
	} else { // child that is literal or identifier
		var termFilter = {};
		if (exp.type === "identifier") {
			termFilter.termId = exp.value;

		} else if (exp.type === "literal") {
			termFilter.termValue = exp.value;
...
/*
 * transform  condition filter in profile.json into elastic search object filter
 */
exports._addRightFilterCondition = function(condition, filter, user, endpoint) {
	var exp = parser.Parser.parse(condition);
	//console.log("exp "+JSON.stringify(exp,null,2));
	_addRightFilterExpression(exp, filter, user, endpoint);
	console.log("filter " + JSON.stringify(filter, null, 2));
			...
			switch (where.value.code) {
				...
				case "=":
					...
					if (val == null) {
						...
						result.query.field[_left(where.children[0])] = val;
					}
					break;
				case "like":
...
function _updateResource(_, hit, properties, resource) {
	...
	if (hit._type != "function") {
		...
		console.log("req " + require('util').inspect(options, null, 2));
		...
		if (_prototype[options.url]) {
			...
			prop = mapping[hit._index].mappings["function"].properties;
			_prototype[options.url] = prop;
	...
	if (prop) {
		...
		Object.keys(prop).map(function(k) {
			...
			/*
			 * key : key of the current properties
			 * propItem : object that represent the current prototype property
			 * hh : the highlight key is associate to the current property
			 */
...
var elasticDataFormat = ["yyyy", "mm", "dd"];
...
function _addX3RightFilter(_, allRight, search, user, endpoint, isFunction) {
	var attr = "$access";
	var right = (allRight ? (isFunction ? allRight.$functions : allRight.$accessCode) : null);
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		if (!isFunction) {
			var item = {};
			item[attr] = "all";
			search.query.filtered.filter.or.filters.push({
				term: item
			});
		}
	...
	if (repRight && Object.keys(repRight).length > 0) {
		Object.keys(repRight).forEach(function(key) {
			...
			_addRightFilterCondition(allRight, search, user, endpoint, isFunction);
		});
	}
}
...
function defineDefaultQuery(searchConfig, q, queryDate) {
	var query = {
		filtered: {
			query: {}
		}
	...
	var editDistance = searchConfig.editDistance ? searchConfig.editDistance : 2;
...
function _addSyracuseRightFilter(_, allRight, search, user, endpoint, isFunction) {
	var attr = (!isFunction ? "_type" : "representationRef.entity");
	var right = allRight.$entities;

	console.log("entities " + JSON.stringify(right, null, 2));
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		var filter = search.query.filtered.filter;
		...
		Object.keys(right).forEach(function(it) {
			...
			if (right[it].condition) { // can access if condition is ok
				_addRightFilterCondition(right[it].condition, filter, user, endpoint, isFunction);
			} else { // can't access to the entity
				var item = {};
				item[attr] = it;
	...
	/*if (search.query.filtered.filter && search.query.filtered.filter.bool.must_not.length > 0) {
        search.query.filtered.filter.bool["_cache"] = true;
        search.query.filtered.filter.bool["_cacheKey"] = endpoint.dataset(_)+"_"+user.getEndpointLogin(_, endpoint.$uuid) + "_" + (isFunction ? "function" : "data");
    }*/
...
exports.search = function(_, context, configOverride) {
	...
	right = null; // up.getAccessRightAuthorizations(_);
	...
	var queryDate = _convertDateToString(_, q, up);
	var searchType = "default";

	// Prototype code only, allow the search type to be overridden by adding
	// a prefix to the search string x: f: p:
	if (searchConfig.prototype) {
		searchType = searchConfig.protoVersion;

		// Allow prototype version to be overridden by search string
		if (q.indexOf(":") !== 0) {
			var sp = q.split(":");
	...
	switch (searchType) {
		case "x":
		case "exact":
			search.query = defineExactQuery(searchConfig, q);
			break;
		case "f":
		case "fuzzy":
		...
		case "p":
		case "exactPlusFuzzy":
			search.query = defineExactPlusFuzzyQuery(searchConfig, q);
			break;
		default:
			search.query = defineDefaultQuery(searchConfig, q, queryDate);
	}
	...
	/*if (right && !right.$entities){
	    _addX3RightFilter(_, right, search, up.user(_), up.selectedEndpoint(_), context.parameters.source === "functions");
    }else{
        _addSyracuseRightFilter(_, right, search, up.user(_), up.selectedEndpoint(_), context.parameters.source === "functions");
