...
function _updateResource(_, hit, properties, resource, right, queryString) {
	...
	function _fromHighlight(hglt, kk, isInArray) {
		var result = hglt && hglt[kk] && kk !== "$url" ? (isInArray || hglt[kk].lenght > 1 ? hglt[kk] : hglt[kk][0]) : null;
		if (result) {
	...
	if (prop) {
		...
		Object.keys(prop).map(function(k) {
			...
			function getRecord(propItem, hh, isInArray) {
				var res = {
					properties: {},
					resource: {}
				};
				...
				if (typeof propItem === "object") {
					switch (propItem.$type) {
						case "application/x-reference":
							...
							res.resource = _fromHighlight(sHighlight, hh, isInArray) || _fromHighlight(sHighlight, hh + ".$value", isInArray) || _fromSource(sSource, hh) || _fromSource(sSource, hh + ".$value");

							break;
						case "application/x-array":
						case "application/x-object":
							if (propItem.$item.$type === "application/x-reference") {
								res = getRecord(propItem.$item, hh, true);
								...
								if (psub) {
									Object.keys(psub).map(function(key) {
										...
										var resp = getRecord(psub[key], hl, true);
										if (resp) {
						...
						default:
							...
							res.resource = _fromHighlight(sHighlight, hh, propItem, isInArray) || _fromSource(sSource, hh);
			...
			var result = getRecord(prop[k], hh, false);
			if (result) {
...
function _addSyracuseRightFilter(_, allRight, search, baseUrlIndex, isFunction, prototypeUnit) {
	...
	if (config.searchEngine.desactivateRight) {
		return;
	}
