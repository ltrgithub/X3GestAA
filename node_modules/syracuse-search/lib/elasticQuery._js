...
exports.search = function(_, context) {
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		if (hit.highlight) {
			Object.keys(hit.highlight).forEach_(_, function(_, hl) {
				console.log("--------------------------------------------");
				console.log("source before" + JSON.stringify(hit._source, null, 2));
				if (hl.indexOf(".") === -1) { //elem in root of source
					hit._source[hl] = hit.highlight[hl][0];
				} else {
					var val = hit._source;
					var isArray = hit.highlight[hl].length > 1;
					if (isArray) {
						var spl = hl.split(".");
						spl.slice(0, -1).map(function(k) {
							val = val[k];
						});
						hit.highlight[hl].map(function(item, i) {
							val[i][spl[spl.length - 1]] = item;
						});
					} else {
						hl.split(".").map(function(k) {
							val = val[k];
						});
						val = hit.highlight[hl][0];
					}

				}
				console.log("source after " + JSON.stringify(hit._source, null, 2));
			});
		}
