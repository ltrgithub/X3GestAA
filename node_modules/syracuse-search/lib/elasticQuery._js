...
function _addFunctionsFilter(_, query, endpoints) {
	var where = "endpoint eq null or application eq null";

	// Ensure that the functions returned are valid for the user
	endpoints.forEach_(_, function(_, endpoint) {
		where = where + " or (endpoint eq '" + endpoint.$uuid + "' and application eq '" + endpoint.applicationRef(_).$uuid + "')";
	});
...
exports.search = function(_, context, configOverride) {
	...
	if (!hasNoAccess) {
		...
		// apply cache filter
		if (applyFilter) { // use cache filter only if only right filter is apply to prevent bad result // TODO manage cache filter in any case
			_addCacheFilter(_, search, user, currentEndpoint, isFunction);
		}
