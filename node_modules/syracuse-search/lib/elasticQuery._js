...
exports.search = function(_, context, contractName, applicationName, datasetName, entityName) {
	...
	if(searchIndexName) 
		...
		if(userProfile) {
			if(userProfile.selectedEndpoint(_)) {
				indexNames.push(userProfile.selectedEndpoint(_).getIndexName(_, locale.current));
				endpoints.push(userProfile.selectedEndpoint(_));
			} else {
				userProfile.endpoints(_).toArray(_).forEach_(_, function(_, endpoint) {
					if(endpoint.enableSearch(_)) {
						indexNames.push(endpoint.getIndexName(_, locale.current));
	...
	// TODO: how to manage search of functions ? for now just add the new index
	indexNames.push(functionIndexName + "." + locale.current.toLowerCase());
