...
function _addFilters(query, filters) {
	...
	*/
	...
	forEachKey(filters, function(key, facet) {
		if (key === "type") return;
		var cond = "facet_" + key + " like '" + facet.slice(0) + "'"; // add like for generate regexp because fields filter doesn't work with pace
...
function _makeFunctionResource(_, endpoint, mapping, hit, isMultiEP, right) {
	...
	var prop = {};
	_updateResource(_, hit, prop, r, right);
	r.$properties = prop;
...
function _addSyracuseRightFilter(_, allRight, search, isFunction) {
...
exports.search = function(_, context, configOverride) {
	...
	var right = up.getAccessRightAuthorizations(_);
	...
	if (!indexNames.length) {
		return context.reply(_, httpHelper.HttpStatus.NotFound, locale.format(module, isFunction ? "noFunctionIndexAvailable" : "noDataIndexAvailable", userProfile.selectedEndpoint(_), locale.current));
	}
	//
	...
	if (right && !right.$entities) {
		...
		_addSyracuseRightFilter(_, right, search, isFunction);
	}
	var respData = {};
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		...
		if (hit._type === "function") {
			...
			r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
				resultRes.$resources.push(_makeFunctionResource(_, e, mappings, hit, endpoints.length > 1, right));
			});
		} else resultRes.$resources.push(_makeResource(_, mappings, hit, context.representation ? context.representation.entity : context.searchEntityName, context.representation.type, right));
