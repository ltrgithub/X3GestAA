...
function _addFilters(query, filters) {
	...
	forEachKey(filters, function(key, facet) {
		...
		for (var i = 0; i < facet.length; i++) {
			var cond = "facet_" + key + " like '" + facet[i] + "'"; // add like for generate regexp because fields filter doesn't work with space
 ...
 */
function _updateResource(_, hit, properties, resource, right) {
	// get prototype store in elasticSearch
	var hasHighlightProperty = false;

	function _fromHighlight(hglt, kk) {
		var result = hglt && hglt[kk] && kk !== "$url" ? (hglt[kk].length > 1 ? hglt[kk] : hglt[kk][0]) : null;
		if (result) {
	...
	function _fromSource(src, kk) {
		...
		if (ktab.length === 0) {
			...
			var srcVal = src;
			var found = false;
			for (var i = 0; i < ktab.length && srcVal[ktab[i]]; i++) {
				srcVal = srcVal[ktab[i]];
			}
			if (i === ktab.length && srcVal) {
				if (typeof srcVal !== "object") {
					return srcVal.$value ? srcVal.$value : srcVal;
	...
	if (prop) {
		...
		Object.keys(prop).map(function(k) {
			...
			function getRecord(propItem, hh) {
				...
				if (propItem.$access) {
					if (right.$X3entities && right.$X3entities[type].accessCodes && right.$X3entities[type].accessCodes.length && right.$X3entities[type].accessCodes.indexOf(propItem.$access) === -1) {
						return null; // do nothin
					}
					if (right.$accessCodes && Object.keys(right.$accessCodes).indexOf(propItem.$access) === -1) {
						return null; // do nothin
					}
				}
				if (typeof propItem === "object") {
					switch (propItem.$type) {
						case "application/x-reference":
							...
							res.resource = _fromHighlight(sHighlight, hh) || _fromHighlight(sHighlight, hh + ".$value") || _fromSource(sSource, hh) || _fromSource(sSource, hh + ".$value");
						...
						default:
							...
							res.resource = _fromHighlight(sHighlight, hh) || _fromSource(sSource, hh);
			...
			var result = getRecord(prop[k], hh);
			if (result) {
...
function _makeFunctionResource(_, endpoint, mapping, hit, isMultiEP, right) {
	...
	var prop = {};
	var isDisplay = _updateResource(_, hit, prop, r, right);
...
function _makeResource(_, mapping, hit, representation, resFormat, right) {
	var title = _getTypeTitle(mapping, hit._type);
	// items base url
...
function _addX3RightFilter(_, allRight, search, isFunction) {
	...
	if (rep && Object.keys(rep).length > 0 && !isFunction) {
		...
		Object.keys(rep).forEach(function(key) { // watch all representation and define condition for entity
			...
			if (repRight.$conditions && repRight.$conditions.length > 0) {
				repRight.$conditions.forEach(function(item) {
					if (item.$type === "restriction") {
						...
						cond.condition = cond.condition ? " and " + cd : cd;
					} else if (item.$type === "authorization") {
						cond.condition = cond.condition ? " and " + item.$where : item.$where;
					}
				});
			}
			...
			if (repRight.$sites && repRight.$sites.length > 0) {
				...
				repRight.$sites.split(",").forEach(function(it) {
					cond.site = cond.site + (cond.site[cond.site.length - 1] === '(' ? it.toLowerCase() : "," + it.toLowerCase());
				});
				cond.site = cond.site + ")";
			}
		});
...
function preProcessSearch(_, q, up, searchType) {
	...
	for (var t = 0; t < terms.length; t++) {
		...
		var numbers = /[0-9]*,[0-9]*/g.exec(terms[t]);
		...
		if (termDate) {
			...
			var tlower = typeof term === "string" ? term.toLowerCase() : term;
			...
			/*if (tlower !== "or" && tlower !== "and") {
				...
				if (searchType === "startsWith") {
					term += "*"; // start wth term and also include term
				}
			}*/
		}
...
function _addSyracuseRightFilter(_, allRight, search, baseUrlIndex, prototypes, isFunction) {
	...
	if (config.searchEngine.desactivateRight) return;
	...
	if (right && Object.keys(right).length > 0) { //simple right
		...
		Object.keys(right).forEach(function(it) {
			var proto = prototypes[baseUrlIndex + "_entities" + "/$entities/" + it];
			var cond;
...
exports.search = function(_, context, configOverride) {
	...
	if (right && !right.notPurgeCache) { // purge after reload right (new http session)
	...
	var edRight = currentEndpoint;
	if (indexNames && indexNames.length && indexNames[0].indexOf("syracuse") !== -1) {
		edRight = adminEP;
	}
	...
	q = preProcessSearch(_, q, up, searchType);
	...
	if (!context.parameters.noHighlight) {
		search.highlight = {
			fields: {
				"*": {}
			}
	...
	switch (searchType) {
		...
		default:
			...
			search.query.filtered.query = defineStartWithQuery(searchConfig, q);
			break;
	}
	...
	if (right && !right.$entities) {
		...
		_addSyracuseRightFilter(_, right, search, urlSegs.join("/"), _prototype, isFunction);
	}
	var respData = {};
	...
	if (!hasNoAccess) {
		...
		while (idx < buf.length) {
			...
			req = httpClient.httpRequest(_, options);
			req.write(_, buf.slice(idx, newIdx).toString('utf8'));
			idx = newIdx;
		}
		...
		if (keepInContext) context.searchRequest = search;
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		 ...
		 */
		function removeFacet(hit, context, respData) {
			Object.keys(respData.facets).forEach(function(facetKey) {
				if (hit._source["facet_" + facetKey]) {
					for (var i = 0; respData.facets[facetKey] && i < respData.facets[facetKey].terms.length && respData.facets[facetKey].terms[i].term === hit._source["facet_" + facetKey]; i++) {}
					if (respData.facets[facetKey] && respData.facets[facetKey].terms[i].term === hit._source["facet_" + facetKey]) {
			...
			for (var i = 0; respData.facets.type && i < respData.facets.type.length && respData.facets.type.terms[i].term === hit._type; i++) {}
			if (respData.facets.type && respData.facets.type.terms[i - 1].term === hit._type) {
		...
		if (hit._type === "function") {
			...
			r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
				resource = _makeFunctionResource(_, e, mappings, hit, endpoints.length > 1, right);
				if (resource) {
					resultRes.$resources.push(resource);
				} else {
					removeFacet(hit, context, respData);
				}
			});
		} else {
			resource = _makeResource(_, mappings, hit, context.representation ? context.representation.entity : context.searchEntityName, context.representation.type, right);
			if (resource) {
				resultRes.$resources.push(resource);
			} else {
				removeFacet(hit, context, respData);
