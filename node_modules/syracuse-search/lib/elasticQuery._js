...
function _formatValueDecimal(_, val, item, locale) {
	var sepChar = locale.numberDecimalSeparator(_);
	var groupChar = locale.numberGroupSeparator(_);
	...
	if (res) {
		...
		res = decimal.format(res, format);
		return res.split(".")[0].replace(",", groupChar) + sepChar + res.split(".")[1];
	}
	return val;
}
				...
				if (typeof propItem === "object") {
					switch (propItem.$type) {
						case "application/x-reference":
							...
							res.resource = _formatValue(_, _fromHighlight(sHighlight, hh, isInArray) || _fromHighlight(sHighlight, hh + ".$value", isInArray) || _fromSource(sSource, hh) || _fromSource(sSource, hh + ".$value"), propItem, context);
						...
						default:
							...
							res.resource = _formatValue(_, _fromHighlight(sHighlight, hh, isInArray) || _fromSource(sSource, hh), propItem, context);
...
exports.search = function(_, context, configOverride) {
	...
	if (!hasNoAccess) {
		...
		exports.tracer && exports.tracer.info && exports.tracer.trace("elasticQuery.search response: " + sys.inspect(respData, null, 5));
	}
	//
	if (respData.error) {
