...
exports.search = function(_, context, configOverride) {
	...
	if (mapKeys && mapKeys.length) {
		...
		logger.debug(_) && logger.debug(_, "elasticQuery.search mappings. Url: " + urlSegs.join("/") + "/_mapping ; result" + sys.inspect(mappings, null, 5));
		// Search
	}
	...
	if (!hasNoAccess) {
		...
		logger.info(_) && logger.info(_, "elasticQuery.search query: " + JSON.stringify(search.query, null, "\t"));
		logger.debug(_) && logger.debug(_, "search " + JSON.stringify(search, null, 2));
