	...
	if (prop) {
		...
		Object.keys(prop).map(function(k) {
			...
			function getRecord(propItem, hh) {
				...
				if (propItem.$access) {
					if (right.$X3entities && right.$X3entities[type] && right.$X3entities[type].accessCodes && right.$X3entities[type].accessCodes.length && right.$X3entities[type].accessCodes.indexOf(propItem.$access) === -1) {
		...
		if (hit._type === "function") {
			...
			r && _selectEndpoints(_, up).forEach_(_, function(_, e) {
				...
				if (resource) {
					...
					_removeFacet(hit, context, respData);
				}
			});
		} else {
