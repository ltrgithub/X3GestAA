...
function _getDescriptionResult(source) {
	...
	//			return ((k[0] != "$") && (Array.isArray(source[k]) || (typeof source[k] !== "object")));
			...
			r = (typeof source === "string" ? source : source.toString());
...
function _makeFunctionResource(_, endpoint, mapping, hit, isMultiEP) {
	...
	var r = {
		...
		title: locale.format(module, isMultiEP ? "functionMultiDescription" : "functionMonoDescription", hit._source.$description, endpoint.description(_)),
...
function _makeResource(mapping, hit, resFormat) {
	var title = _getTypeTitle(mapping, hit._type);
	// items base url
...
exports.search = function(_, context) {
	...
	var indexNames = _selectIndexes(_, elasticBaseUrl, context.getUserProfile(_), sources, (filters && filters.index && filters.index[0]) || context.searchIndexName);
	...
	//
	var entityName = context.parameters.entity || (context.representation.entity !== "queryResult" ? context.representation.entity : "");
...
/*	if((allEntities.length === 1) && (indexNames.length === 1))
	...
	if (respData.hits && respData.hits.hits) respData.hits.hits.forEach_(_, function(_, hit) {
		if(hit._type === "function") {
			...
			resultRes.$resources.push(_makeResource(mappings, hit, context.representation.type));
	});
