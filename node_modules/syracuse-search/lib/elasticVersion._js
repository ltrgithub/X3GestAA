...
exports.checkVersion = function(_, searchBaseUrl, opts) {
	...
	debugTrace("Installed version: " + instVersion);

	if (!compatibleVersion(instVersion, opts.minVersion || minVersion, true)) {
	...
	if (!compatibleVersion(instVersion, opts.maxVersion || maxVersion, false)) {
	...
	return isRecVersion(instVersion, opts.recVersion || recVersion);
};