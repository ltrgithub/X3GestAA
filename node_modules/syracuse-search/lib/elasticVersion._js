...
exports.getElasticVersion = function(_, searchBaseUrl, opts) {
	...
	} else if (opts && opts.version) {
		installedVersion = opts.version.split(".").map(function(v) {
			return +v;
		});
	}
...
exports.hasNoIndexes = function(_, searchBaseUrl, opts) {

	// Query the elastic search base URL to get the about information
	var options = {
		url: searchBaseUrl + "/_cat/indices",
		method: "GET",
...
exports.checkVersion = function(_, searchBaseUrl, opts) {
	...
	tracer.debug && tracer.debug("Installed version: " + instVersion);
