...
function _createInstanceIndexResource(_, instance, endpoint, options) {
	...
	// specific for menuItems: index also the target link. TODO: generalize ?
	if(entity.name === "menuItem") {
		data.$targetLink = instance.getLink(_, endpoint);
	}
	//
	return data;
}
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	updateInstance: function(_, instance, options) {
		...
		var reqOptions = {
			url: [elasticBaseUrl, _indexName, entity.name, instance.computeKey()].join("/"),
			method: 'PUT'
		};
...
//					console.log("post at count: " + cnt);
	...
	updateIndex: function(_, differential, options) {
		...
		var entities = (options && options.representation) ? [model.getEntity(_, options.representation, "$query")] : model.getIndexedEntities(_);
...
function _indexMappingUpdate(_, context, indexName, entity, searchFacets, opt) {
	...
	mapping[entityName] = {
		properties: {},
		_meta: {
			$classTitle: entity.$titleTemplate && entity.$titleTemplate.expression 
		}
	};
		...
		if(prototype[prop]) {
			switch(prototype[prop].$type) {
				case "application/x-choice":
					val = (((prototype[prop].$value.$enum || []).filter(function(enumItem) {return enumItem.$value === item[prop]}) || [])[0] || {}).$title;
					break;
				default:
					val = item[prop];
			}
