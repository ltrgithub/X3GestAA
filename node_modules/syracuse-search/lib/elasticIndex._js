...
var logger = require("syracuse-trace/lib/helper").getTracer(_, "search");
...
function _createInstanceIndexResource(_, instance, endpoint, options, prototype) {
	...
	function _computeUrl(_, inst) {
		...
		logger.debug(_) && logger.debug(_, "compute url: " + sys.inspect(entity.$urlTemplate));
	...
	var opt = options || {};
	logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: enter: " + instance.$uuid);
	...
	(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: compute field: " + field + "; value: " + val);
		//
		if (val) {
			data[field] = val;
			...
			if (facets && facets[field]) facets[field].forEach_(_, function(_, f) {
				logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: field facet: " + f);
	...
	(opt.rawData || []).forEach_(_, function(_, relName) {
		logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: set rawData for relation: " + relName);
		...
		if (rel.isPlural) relData.toArray(_).forEach_(_, function(_, item) {
			...
			logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: rawData found for " + relName + ": " + relData.$uuid);
		}
	});
	...
	logger.debug(_) && logger.debug(_, "ElasticIndex.createResource: compute templates");
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntities: function(_, entityName, data, options) {
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateInstance: index response: " + sys.inspect(diag));
	...
	updateInstance: function(_, instance, options) {
		var opt = options || {};
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateInstance: enter: " + instance._meta.name + "." + instance.$uuid);
		//
		var self = this;
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateInstance: url: " + reqOptions.url);
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateInstance: index data: " + sys.inspect(data));
	...
	updateEntity: function(_, entity, differential, options) {
		...
		if (differential) {
			...
			if (lastIndex) {
				...
				logger.debug(_) && logger.debug(_, "ElasticIndex.updateEntity: differential where: " + where.sdataWhere);
			}
		}
		//
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateEntity: fetching data");
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateEntity: endpoint " + require('util').inspect(options));
		...
		if (inst) {
			do {
				...
				do {
					...
					logger.debug(_) && logger.debug(_, "ElasticIndex.updateEntity: action : " + sys.inspect(action));
					req.write(_, JSON.stringify(action) + "\n");
					logger.debug(_) && logger.debug(_, "write ok ");

					var data = _createInstanceIndexResource(_, inst, self._endpoint, opt, prototype);
					logger.debug(_) && logger.debug(_, "ElasticIndex.updateEntity: index data: " + sys.inspect(data));
					req.write(_, JSON.stringify(data) + "\n");
					cnt++;
		...
		logger.debug(_) && logger.debug(_, "entity " + entity.name);
	...
	updateIndex: function(_, differential, options) {
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateIndex enter: locale=" + opt.locale);
		if (opt.locale) {
		...
		//
		logger.debug(_) && logger.debug(_, "ElasticIndex.updateIndex creating index");
	...
	getPrototype: function(_, entity, options) {
		...
		var opt = options || {};
		logger.debug(_) && logger.debug(_, "ElasticIndex.updatePrototype: enter: " + entity.name);
		//
		var self = this;

		logger.debug(_) && logger.debug(_, "ElasticIndex.updatePrototype: url: " + options.url);
		...
		logger.debug(_) && logger.debug(_, "ElasticIndex.updatePrototype: index data: " + sys.inspect(data));
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	var mapping = {};
	// logger.debug(_) && logger.debug(_, "_indexMappingUpdate: entityName=" + entityName + ", entity=" + sys.inspect(entity));
	...
	logger.debug(_) && logger.debug(_, "ElasticIndex _indexMappingUpdate: contract: " + (entity.contract && entity.contract.application) + ", entityName=" + entityName + ", title=" + entityTitle + ", classTitle=" + classTitle);
	if (opt.tracer) {
		entity && Object.keys(entity).forEach_(_, function(_, key) {
			if ((key[0] === '$')) {
				logger.debug(_) && logger.debug(_, "   " + key + "=" + entity[key]);
			}
		});
	}

	// logger.debug(_) && logger.debug(_, "...title=" + entityTitle + ", classTitle=" + classTitle);
	mapping[entityName] = {
		properties: {},
		_meta: {
	...
	logger.debug(_) && logger.debug(_, "elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	logger.debug(_) && logger.debug(_, "elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
...
function _indexCreate(_, indexName, localeCode, model, options, isFunctionIndex) {
	...
	if (!elasticHelpers.indexExists(_, elasticBaseUrl, indexName, opt)) {
		...
		var resp = req.end().response(_);
		logger.debug(_) && logger.debug(_, "elasticIndex.index put settings status: " + resp.statusCode);
		//
		...
		if (facets && facets.length) {
			logger.debug(_) && logger.debug(_, "ElasticIndex.updateFacets");
			//
			...
			facets.forEach_(_, function(_, f) {
				...
				req.write(_, JSON.stringify(action) + "\n");
				logger.debug(_) && logger.debug(_, "ElasticIndex.updateFacets: index data: " + sys.inspect(f));
