...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	updateInstance: function(_, instance, options) {
		...
		// 
		var self = this;
		var _indexName = opt.indexName || self._endpoint.getIndexName(_);
	...
	updateEntity: function(_, entity, searchFacets, differential, options) {
		...
		// update mapping
		_indexMappingUpdate(_, {}, _indexName, entity, searchFacets, opt);
		// update data
			...
			if(inst) {
				do {
					...
					do {
						...
						req.write(_, JSON.stringify(data) + "\n");
						cnt++;
//						console.log("CLE: " + inst.CLE(_) + "; count: " + cnt);
						inst = cursor.next(_);
					} while(inst && (cnt % 1000 !== 0))
					var resp = req.end().response(_);
//					console.log("post at count: " + cnt);
					//
	...
	updateIndex: function(_, differential, options) {
		...
		var entities = (options && options.representation) ? [model.getEntity(_, options.representation, "$bulk")] : model.getIndexedEntities(_);
		...
		adminHelpers.AdminHelper.logServerMessage(_, "Start index update", []);
		...
		try {
			entities.forEach_(_, function(_, entity) {
				...
				if(entity.name === "menuItem") {
					self.updateFunctions(_, entity, facets, differential, opt);
				}
			});
		} catch(e) {
			opt.diagnoses.push({severity: "error", message: e.message});
			opt.diagnoses.push({severity: "error", message: e.stack});
			adminHelpers.AdminHelper.logServerMessage(_, "End index update", opt.diagnoses);
			throw e;
		}
		opt.diagnoses && adminHelpers.AdminHelper.logServerMessage(_, "End index update", opt.diagnoses)
		//
		self._endpoint.lastIndexDate(_, indexTime);
		self._endpoint.save(_);
	},
	updateFunctions: function(_, menuItemEntity, facets, differential, options) {
		var self = this;
		var entity = menuItemEntity;
		// ensure functions index
		_indexCreate(_, null, functionIndexName, options);
		//
		...
		self.updateEntity(_, entity, facets, differential, o);
	},
	updateFunctionInstance: function(_, instance, options) {
		...
		self.updateInstance(_, instance, o);
	}
});
...
function _indexMappingUpdate(_, context, indexName, entity, searchFacets, opt) {
	var opt = opt || {};
...
/*	mapping[entityName].properties.$targetLink = {
			type: "object",
			index: "no",
			include_in_all: false
	};*/
...
//	console.log("fields mapping "+entityName+"; "+sys.inspect(searchFacets)+"; "+sys.inspect(mapping));
	...
	//
	_processElasticResp(_, resp, context.response, diagnoses, locale.format(module, "mappingUpdated", entityName));
}
...
function _indexCreate(_, context, indexName, options) {
	...
	if(resp.statusCode == 404) {
		...
		//
		_processElasticResp(_, resp, context && context.response, diagnoses, locale.format(module, "indexCreated", indexName));
	}
}
...
exports.deleteEndpointIndex = function(_, context, endpoint, entityName, diagnoses) {
	...
	var resp = req.end().response(_);
	//
	_processElasticResp(_, resp, context.response, diagnoses, locale.format(module, "indexDeleted", endpoint.getIndexName(_)));
}
...
exports.deleteIndex = function(_, context, indexName, diagnoses) {
	...
	var resp = req.end().response(_);
	//
	_processElasticResp(_, resp, context && context.response, diagnoses, locale.format(module, "indexDeleted", indexName));
}
