...
var logger = require("syracuse-trace/lib/helper").getTracer("search");
...
function _createInstanceIndexResource(_, instance, endpoint, options, prototype) {
	...
	function _computeUrl(_, inst) {
		...
		logger.debug() && logger.debug("compute url: " + sys.inspect(entity.$urlTemplate));
	...
	var opt = options || {};
	logger.debug() && logger.debug("ElasticIndex.createResource: enter: " + instance.$uuid);
	...
	(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
		...
		logger.debug() && logger.debug("ElasticIndex.createResource: compute field: " + field + "; value: " + val);
		//
		if (val) {
			data[field] = val;
			...
			if (facets && facets[field]) facets[field].forEach_(_, function(_, f) {
				logger.debug() && logger.debug("ElasticIndex.createResource: field facet: " + f);
	...
	(opt.rawData || []).forEach_(_, function(_, relName) {
		logger.debug() && logger.debug("ElasticIndex.createResource: set rawData for relation: " + relName);
		...
		if (rel.isPlural) relData.toArray(_).forEach_(_, function(_, item) {
			...
			logger.debug() && logger.debug("ElasticIndex.createResource: rawData found for " + relName + ": " + relData.$uuid);
		}
	});
	...
	logger.debug() && logger.debug("ElasticIndex.createResource: compute templates");
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntities: function(_, entityName, data, options) {
		...
		logger.debug() && logger.debug("ElasticIndex.updateInstance: index response: " + sys.inspect(diag));
	...
	updateInstance: function(_, instance, options) {
		var opt = options || {};
		logger.debug() && logger.debug("ElasticIndex.updateInstance: enter: " + instance._meta.name + "." + instance.$uuid);
		//
		var self = this;
		...
		logger.debug() && logger.debug("ElasticIndex.updateInstance: url: " + reqOptions.url);
		...
		logger.debug() && logger.debug("ElasticIndex.updateInstance: index data: " + sys.inspect(data));
	...
	updateEntity: function(_, entity, differential, options) {
		...
		if (differential) {
			...
			if (lastIndex) {
				...
				logger.debug() && logger.debug("ElasticIndex.updateEntity: differential where: " + where.sdataWhere);
			}
		}
		//
		logger.debug() && logger.debug("ElasticIndex.updateEntity: fetching data");
		logger.debug() && logger.debug("ElasticIndex.updateEntity: endpoint " + require('util').inspect(options));
		...
		if (inst) {
			do {
				...
				do {
					...
					logger.debug() && logger.debug("ElasticIndex.updateEntity: action : " + sys.inspect(action));
					req.write(_, JSON.stringify(action) + "\n");
					logger.debug() && logger.debug("write ok ");

					var data = _createInstanceIndexResource(_, inst, self._endpoint, opt, prototype);
					logger.debug() && logger.debug("ElasticIndex.updateEntity: index data: " + sys.inspect(data));
					req.write(_, JSON.stringify(data) + "\n");
					cnt++;
		...
		logger.debug() && logger.debug("entity " + entity.name);
	...
	updateIndex: function(_, differential, options) {
		...
		logger.debug() && logger.debug("ElasticIndex.updateIndex enter: locale=" + opt.locale);
		if (opt.locale) {
		...
		//
		logger.debug() && logger.debug("ElasticIndex.updateIndex creating index");
	...
	getPrototype: function(_, entity, options) {
		...
		var opt = options || {};
		logger.debug() && logger.debug("ElasticIndex.updatePrototype: enter: " + entity.name);
		//
		var self = this;

		logger.debug() && logger.debug("ElasticIndex.updatePrototype: url: " + options.url);
		...
		logger.debug() && logger.debug("ElasticIndex.updatePrototype: index data: " + sys.inspect(data));
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	var mapping = {};
	// logger.debug() && logger.debug("_indexMappingUpdate: entityName=" + entityName + ", entity=" + sys.inspect(entity));
	...
	logger.debug() && logger.debug("ElasticIndex _indexMappingUpdate: contract: " + (entity.contract && entity.contract.application) + ", entityName=" + entityName + ", title=" + entityTitle + ", classTitle=" + classTitle);
	if (opt.tracer) {
		entity && Object.keys(entity).forEach_(_, function(_, key) {
			if ((key[0] === '$')) {
				logger.debug() && logger.debug("   " + key + "=" + entity[key]);
			}
		});
	}

	// logger.debug() && logger.debug("...title=" + entityTitle + ", classTitle=" + classTitle);
	mapping[entityName] = {
		properties: {},
		_meta: {
	...
	logger.debug() && logger.debug("elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	logger.debug() && logger.debug("elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
...
function _indexCreate(_, indexName, localeCode, model, options, isFunctionIndex) {
	...
	if (!elasticHelpers.indexExists(_, elasticBaseUrl, indexName, opt)) {
		...
		var resp = req.end().response(_);
		logger.debug() && logger.debug("elasticIndex.index put settings status: " + resp.statusCode);
		//
		...
		if (facets && facets.length) {
			logger.debug() && logger.debug("ElasticIndex.updateFacets");
			//
			...
			facets.forEach_(_, function(_, f) {
				...
				req.write(_, JSON.stringify(action) + "\n");
				logger.debug() && logger.debug("ElasticIndex.updateFacets: index data: " + sys.inspect(f));
