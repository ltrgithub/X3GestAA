	...
	(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
		...
		if (p = entity.$properties[field]) {
			...
			if (p = entity.$relations[field]) {
				...
				if(rel.isChild) {
					childOpt = _unfixFunctionsOptions(options);
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateInstance: function(_, instance, options) {
		...
		if(!elasticHelpers.indexExists(_, elasticBaseUrl, _indexName, {})) return;
		//
		var entity = instance.getEntity();
		...
		if(options && options.diagnoses) {
			if(diag.ok)
				options.diagnoses.push({severity:"info", message: locale.format(module, "instanceUpdated", instance.getValue(_))});
			else
				if(diag.error)
	...
	updateIndex: function(_, differential, options) {
		...
		entities.some_(_, function(_, entity) {
			_track(locale.format(module, "indexingEntity"), entity.name, Math.ceil(idx++*100/entities.lenth));
			try {
...
//				adminHelpers.AdminHelper.logServerMessage(_, locale.format(module, "endIndexUpd", indexName), opt.diagnoses);
			...
			if(opt.tracker && opt.tracker.abortRequested) {
				opt.diagnoses.push({severity: "info", message: locale.format(module, "indexUpdAborted") });
				return true;
			} else
				return false;
		});
		opt.diagnoses.push({
			severity: "success",
			message: locale.format(module, "endIndexUpd", indexName)
		});
		_track(locale.format(module, "endIndexUpd", indexName), "", 100);
	...
	updatePrototype: function(_, entity, options) {
		...
		if(options && options.diagnoses) {
			if(diag.ok)
				options.diagnoses.push({severity:"info", message: locale.format(module, "prototypeUpdated", entity.name)});
			else
				if(diag.error)
					options.diagnoses.push({severity:"error", message: diag.error});
		}
...
exports.registerModelEvents = function(_, config) {
	...
	eps.forEach_(_, function(_, e) {
		...
		if(!elasticHelpers.indexExists(_, elasticBaseUrl, indexName, {})) return;
		//
