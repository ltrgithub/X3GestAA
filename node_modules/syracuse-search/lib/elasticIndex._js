...
var minimal = config.searchEngine && config.searchEngine.stemmer === "minimal";
// TODO: map all locales codes to supported elasticsearch analyzers
var _analyzersMap = {
	"de-de": {
		"stemmer": minimal ? "minimal_german" : "german",
		"stop": "_german_"
	},
	"en-us": {
		"stemmer": minimal ? "minimal_english" : "english",
		"stop": "_english_"
	},
	"es-es": {
		"stemmer": minimal ? "light_spanish" : "spanish",
		"stop": "_spanish_"
	},
	"fr-fr": {
		"stemmer": minimal ? "minimal_french" : "french",
		"stop": "_french_"
	},
	"it-it": {
		"stemmer": minimal ? "light_italian" : "italian",
		"stop": "_italian_"
	},
	"pl-pl": {
		"stemmer": minimal ? "light_polish" : "polish"
	},
	"pt-br": {
		"stemmer": minimal ? "light_brazilian" : "brazilian",
		"stop": "_brazilian_"
	},
	"pt-pt": {
		"stemmer": minimal ? "minimal_portuguese" : "portuguese",
		"stop": "_portuguese_"
	},
	"ru-ru": {
		"stemmer": minimal ? "light_russian" : "russian",
		"stop": "_russian_"
	},
	"zh-cn": {
		"analyzer": "chinese"
...
function _createInstanceIndexResource(_, instance, endpoint, options) {
		...
		if (field !== "$access" && (p = entity.$properties[field])) {
			...
			if (rel.isPlural) {
				relData.toArray(_).forEach_(_, function(_, item) {
					(val = val || []).push(rel.getIsChild() ? _createInstanceIndexResource(_, item, endpoint, childOpt) : {
						$key: item.computeKey(),
				...
				if (relData) {
					val = rel.getIsChild() ? _createInstanceIndexResource(_, relData, endpoint, childOpt) : {
	...
	if (data && Object.keys(data).length !== 0) { // only if data is not empty to don't store unuseful info in index
		if (!options.isFunctionIndex) {
			data["$access"] = inst.$access ? inst.$access : "_all";
		} else {
			data["$access"] = data.fusionFunction ? data.fusionFunction : "_all";
		}
	}
...
exports.checkStartSearchEngine = function(_, diag) {
	...
	try {
		...
		if (resp.statusCode === 200) {
			...
			_processElasticResp(_, resp, null, diag, locale.format(module, "searchEngineOK", elasticBaseUrl));
			return;
		} else {
			diag.push({
				$severity: "error",
				$message: "can't join searchEngine server http status [" + resp.statusCode + "]"
			});
			_processElasticResp(_, resp, null, diag, locale.format(module, "searchEngineNotOK"));
		}
	} catch (e) {

		diag.push({
			$severity: "error",
			$message: "can't join searchEngine server error : [" + e.stack + "]"
		});
	}
};
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntity: function(_, entity, differential, options) {
		...
		opt.tracer && opt.tracer("ElasticIndex.updateEntity: endpoint " + require('util').inspect(options));
		...
		if (inst) {
			...
			do {
				...
				do {
					var mustIndex = true;
					if (options.isFunctionIndex) {
						//check if menuItem has to be index for the endpoint in options
						mustIndex = ((!inst.endpointName(_) && !inst.applicationName(_)) || (inst.applicationName(_) === e.application(_) && (!inst.endpointName(_) || inst.endpointName(_) === e.dataset(_))));
					}
					if (mustIndex) {
						...
						opt.tracer && opt.tracer("ElasticIndex.updateEntity: action : " + sys.inspect(action));
						req.write(_, JSON.stringify(action) + "\n");
						opt.tracer && opt.tracer("write ok ");
	...
	updateFunctionIndex: function(_, differential, options) {
		...
		var indexName = (functionIndexName + "." + e.application(_).toLowerCase() + "." + e.contract(_).toLowerCase() + "." + e.dataset(_).toLowerCase()).replace(" ", "_");

		_indexCreate(_, self.getIndexName(_, {
			indexName: indexName
		}), self._locale, endpointSyracuse.getModel(_), options);
		//
		var foptions = _fixFunctionsOptions(e, indexName, options);
		foptions.fendpoint = endpointSyracuse;
		self.updateEntity(_, entity, differential, foptions);
	},
	...
	updateFunctionInstance: function(_, instance, options) {
		...
		endpoints.forEach_(_, function(_, e) {
			...
			var indexName = functionIndexName + "." + e.application(_).toLowerCase() + "." + e.contract(_).toLowerCase() + "." + e.dataset(_).toLowerCase();
			_indexCreate(_, self.getIndexName(_, {
				indexName: indexName
	...
	getPrototype: function(_, entity, options) {
		...
		var data = entity.getPrototype(_, entity.name, this._endpoint.protocol(_) === "syracuse" ? "$summary" : "$search"); // TODO
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	exports.tracer && exports.tracer("elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	exports.tracer && exports.tracer("elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
