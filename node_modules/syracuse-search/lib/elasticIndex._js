...
function _createInstanceIndexResource(_, instance, endpoint, options, prototype) {
	...
	if (data && Object.keys(data).length !== 0) { // only if data is not empty to don't store un
		...
		if (!options.isFunctionIndex) {
			...
			if (siteVal) {
				data["$site"] = siteVal.toLowerCase(); // regexp only work on lowercase
			}
		}
	}
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateIndex: function(_, differential, options) {
		...
		//
		opt.tracer && opt.tracer.trace("ElasticIndex.updateIndex creating index");
	...
	getPrototype: function(_, entity, options) {
		...
		var opt = options || {};
		options && options.tracer && options.tracer.trace("ElasticIndex.updatePrototype: enter: " + entity.name);
		//
		var self = this;

		options && options.tracer && options.tracer.trace("ElasticIndex.updatePrototype: url: " + options.url);
		...
		options && options.tracer && options.tracer.trace("ElasticIndex.updatePrototype: index data: " + sys.inspect(data));
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	opt.tracer && opt.tracer.trace("ElasticIndex _indexMappingUpdate: contract: " + (entity.contract && entity.contract.application) + ", entityName=" + entityName + ", title=" + entityTitle + ", classTitle=" + classTitle);
	opt.tracer && entity && Object.keys(entity).forEach(function(key) {
		(key[0] === '$') && opt.tracer.trace("   " + key + "=" + entity[key]);
	});
	...
	exports.tracer && exports.tracer.trace("elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	exports.tracer && exports.tracer.trace("elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
...
function _indexCreate(_, indexName, localeCode, model, options, isFunctionIndex) {
	...
	if (!elasticHelpers.indexExists(_, elasticBaseUrl, indexName, opt)) {
		...
		var resp = req.end().response(_);
		opt.tracer && opt.tracer.trace("elasticIndex.index put settings status: " + resp.statusCode);
		//
		...
		if (facets && facets.length) {
			opt.tracer && opt.tracer.trace("ElasticIndex.updateFacets");
			//
			...
			facets.forEach_(_, function(_, f) {
				...
				req.write(_, JSON.stringify(action) + "\n");
				opt.tracer && opt.tracer.trace("ElasticIndex.updateFacets: index data: " + sys.inspect(f));
