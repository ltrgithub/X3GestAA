...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateIndex: function(_, differential, options) {
		...
		var idx = 0;
		var aborted = opt.tracker && opt.tracker.abortRequested;
...
//				adminHelpers.AdminHelper.logServerMessage(_, locale.format(module, "endIndexUpd", indexName), opt.diagnoses);
		...
		var msg = aborted ? locale.format(module, "indexUpdAborted") : locale.format(module, "endIndexUpd", indexName);
		opt.diagnoses.push({
			severity: aborted ? "warning" : "success",
			message: msg
		});
		_track("", msg, 100);
