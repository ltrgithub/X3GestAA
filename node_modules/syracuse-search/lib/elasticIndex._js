...
function _createInstanceIndexResource(_, instance, endpoint, options, prototype) {
	...
	function _computeUrl(_, inst) {
		...
		tracer.trace.debug() && tracer.trace.debug("compute url: " + sys.inspect(entity.$urlTemplate));
	...
	var opt = options || {};
	tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: enter: " + instance.$uuid);
	...
	(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: compute field: " + field + "; value: " + val);
		//
		if (val) {
			data[field] = val;
			...
			if (facets && facets[field]) facets[field].forEach_(_, function(_, f) {
				tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: field facet: " + f);
	...
	(opt.rawData || []).forEach_(_, function(_, relName) {
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: set rawData for relation: " + relName);
		...
		if (rel.isPlural) relData.toArray(_).forEach_(_, function(_, item) {
			...
			tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: rawData found for " + relName + ": " + relData.$uuid);
		}
	});
	...
	tracer.trace.debug() && tracer.trace.debug("ElasticIndex.createResource: compute templates");
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntities: function(_, entityName, data, options) {
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateInstance: index response: " + sys.inspect(diag));
	...
	updateInstance: function(_, instance, options) {
		var opt = options || {};
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateInstance: enter: " + instance._meta.name + "." + instance.$uuid);
		//
		var self = this;
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateInstance: url: " + reqOptions.url);
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateInstance: index data: " + sys.inspect(data));
	...
	updateEntity: function(_, entity, differential, options) {
		...
		if (differential) {
			...
			if (lastIndex) {
				...
				tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateEntity: differential where: " + where.sdataWhere);
			}
		}
		//
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateEntity: fetching data");
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateEntity: endpoint " + require('util').inspect(options));
		...
		if (inst) {
			do {
				...
				do {
					...
					tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateEntity: action : " + sys.inspect(action));
					req.write(_, JSON.stringify(action) + "\n");
					tracer.trace.debug() && tracer.trace.debug("write ok ");

					var data = _createInstanceIndexResource(_, inst, self._endpoint, opt, prototype);
					tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateEntity: index data: " + sys.inspect(data));
					req.write(_, JSON.stringify(data) + "\n");
					cnt++;
		...
		tracer.trace.debug() && tracer.trace.debug("entity " + entity.name);
	...
	updateIndex: function(_, differential, options) {
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateIndex enter: locale=" + opt.locale);
		if (opt.locale) {
		...
		//
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateIndex creating index");
	...
	updateFunctionIndex: function(_, differential, options) {
		...
		var endpointSyracuse = adminHelpers.AdminHelper.getEndpoints(_, {
			dataset: "syracuse"
		})[0];
		...
		try {
			...
			continu = self.updateEntity(_, entity, false, options);
	...
	getPrototype: function(_, entity, options) {
		...
		var opt = options || {};
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updatePrototype: enter: " + entity.name);
		//
		var self = this;

		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updatePrototype: url: " + options.url);
		...
		tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updatePrototype: index data: " + sys.inspect(data));
...
function _processElasticResp(_, elasticResp, contextResp, diagnoses, okMessage) {
	if (diagnoses) {
		...
		if (diag.ok) diagnoses.push({
			...
			if (diag.error.indexOf("MergeMappingException") >= 0) {
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	var mapping = {};
	// tracer.trace.debug() && tracer.trace.debug("_indexMappingUpdate: entityName=" + entityName + ", entity=" + sys.inspect(entity));
	...
	tracer.trace.debug() && tracer.trace.debug("ElasticIndex _indexMappingUpdate: contract: " + (entity.contract && entity.contract.application) + ", entityName=" + entityName + ", title=" + entityTitle + ", classTitle=" + classTitle);
	if (opt.tracer) {
		entity && Object.keys(entity).forEach_(_, function(_, key) {
			if ((key[0] === '$')) {
				tracer.trace.debug() && tracer.trace.debug("   " + key + "=" + entity[key]);
			}
		});
	}

	// tracer.trace.debug() && tracer.trace.debug("...title=" + entityTitle + ", classTitle=" + classTitle);
	mapping[entityName] = {
		properties: {},
		_meta: {
	...
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	tracer.trace.debug() && tracer.trace.debug("elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
...
function _indexCreate(_, indexName, localeCode, model, options, isFunctionIndex) {
	...
	if (!elasticHelpers.indexExists(_, elasticBaseUrl, indexName, opt)) {
		...
		var resp = req.end().response(_);
		tracer.trace.debug() && tracer.trace.debug("elasticIndex.index put settings status: " + resp.statusCode);
		//
		...
		if (facets && facets.length) {
			tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateFacets");
			//
			...
			facets.forEach_(_, function(_, f) {
				...
				req.write(_, JSON.stringify(action) + "\n");
				tracer.trace.debug() && tracer.trace.debug("ElasticIndex.updateFacets: index data: " + sys.inspect(f));
