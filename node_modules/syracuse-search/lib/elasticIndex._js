	...
	function _createInstanceIndexResource(_, instance, endpoint, options) {
		...
		(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
			...
			if (p = entity.$properties[field]) {
				...
				else if (relData) val = rel.isChild ? _createInstanceIndexResource(_, relData, endpoint, options) : decodeTemplate(_, relData, rel.targetEntity.$valueTemplate);
		...
		if (opt.representations && opt.representations.length) {
			opt.representations.forEach_(_, function(_, r) {
				...
				if (ent) entities.push(ent);
					...
					severity: "warning",
					message: locale.format(module, "reprNotFound", r)
				});
			});
...
function _addFacetFieldsMapping(searchFacets, entityName, mapping) {
	...
	forEachKey(searchFacets, function(key, facet) {
		if (facet.$fields && facet.$fields[entityName]) mapping[entityName].properties["facet_" + key] = {
			type: "string",
			analyzer: "keyword"
		};
	});
...
exports.registerModelEvents = function(_, config) {
	...
	eps.forEach_(_, function(_, e) {
		...
		model && model.registerEvent("$afterSave", function(_, instance) {
			...
			if (instance.getEntity().name === "menuItem") indexer.updateFunctionInstance(null, instance, config);
