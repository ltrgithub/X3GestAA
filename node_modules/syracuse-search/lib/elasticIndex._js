			...
			if (p = entity.$relations[field]) {
				...
				if (rel.isPlural)
					relData.toArray(_).forEach_(_, function(_, item) {
						(val = val || []).push(rel.isChild ? _createInstanceIndexResource(_, item, endpoint, options) : decodeTemplate(_, item, rel.targetEntity.$valueTemplate));
					});
				else
					if(relData) 
						val = rel.isChild ? _createInstanceIndexResource(_, relData, endpoint, options) : decodeTemplate(_, relData, rel.targetEntity.$valueTemplate);
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateIndex: function(_, differential, options) {
		...
		_indexCreate(_, self.getIndexName(_, opt), self._locale, opt);
		//
		try {
...
function _indexCreate(_, indexName, localeCode, options) {
	var opt = options || {};
