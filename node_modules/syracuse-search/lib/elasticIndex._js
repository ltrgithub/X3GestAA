...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntities: function(_, entityName, data, options) {
		...
		elasticHelpers.debugTrace(options, "ElasticIndex.updateInstance: index response: " + sys.inspect(diag));
	...
	updateInstance: function(_, instance, options) {
		var opt = options || {};
		elasticHelpers.debugTrace(options, "ElasticIndex.updateInstance: enter: " + instance._meta.name + "." + instance.$uuid);
		//
		var self = this;
		...
		elasticHelpers.debugTrace(options, "ElasticIndex.updateInstance: url: " + reqOptions.url);
		...
		elasticHelpers.debugTrace(options, "ElasticIndex.updateInstance: index data: " + sys.inspect(data));
	...
	updateEntity: function(_, entity, differential, options) {
		...
		if (differential) {
			...
			if (lastIndex) {
				...
				elasticHelpers.debugTrace(opt, "ElasticIndex.updateEntity: differential where: " + where.sdataWhere);
			}
		}
		//
		elasticHelpers.debugTrace(opt, "ElasticIndex.updateEntity: fetching data");
		elasticHelpers.debugTrace(opt, "ElasticIndex.updateEntity: endpoint " + require('util').inspect(options));
		...
		if (inst) {
			do {
				...
				do {
					...
					elasticHelpers.debugTrace(opt, "ElasticIndex.updateEntity: action : " + sys.inspect(action));
					req.write(_, JSON.stringify(action) + "\n");
					elasticHelpers.debugTrace(opt, "write ok ");

					var data = _createInstanceIndexResource(_, inst, self._endpoint, opt, prototype);
					elasticHelpers.debugTrace(opt, "ElasticIndex.updateEntity: index data: " + sys.inspect(data));
					req.write(_, JSON.stringify(data) + "\n");
					cnt++;
		...
		elasticHelpers.debugTrace(opt, "entity " + entity.name);
	...
	updateIndex: function(_, differential, options) {
		...
		elasticHelpers.debugTrace(opt, "ElasticIndex.updateIndex enter: locale=" + opt.locale);
		if (opt.locale) {
		...
		//
		elasticHelpers.debugTrace(opt, "ElasticIndex.updateIndex creating index");
		...
		function _updateAllEntity(_) {
			...
			entities.some_(_, function(_, entity) {
				...
				try {
					continu = self.updateEntity(_, entity, differential, opt);
					...
					if (e.message === "MergeMappingException") { // throw error to stop the process if mergeMapping we have to restart
						...
						continu = self.updateEntity(_, entity, false, opt);
						return;
					} else {
						opt.diagnoses.push({
							$severity: "error",
	...
	updateFunctionIndex: function(_, differential, options) {
		...
		try {
			continu = self.updateEntity(_, entity, differential, foptions);
		} catch (e) {
			// delete index and restart
	...
	getPrototype: function(_, entity, options) {
		...
		var opt = options || {};
		elasticHelpers.debugTrace(options, "ElasticIndex.updatePrototype: enter: " + entity.name);
		//
		var self = this;

		elasticHelpers.debugTrace(options, "ElasticIndex.updatePrototype: url: " + options.url);
		...
		elasticHelpers.debugTrace(options, "ElasticIndex.updatePrototype: index data: " + sys.inspect(data));
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	var mapping = {};
	// elasticHelpers.debugTrace(opt, "_indexMappingUpdate: entityName=" + entityName + ", entity=" + sys.inspect(entity));
	...
	elasticHelpers.debugTrace(opt, "ElasticIndex _indexMappingUpdate: contract: " + (entity.contract && entity.contract.application) + ", entityName=" + entityName + ", title=" + entityTitle + ", classTitle=" + classTitle);
	if (opt.tracer) {
		entity && Object.keys(entity).forEach(function(key) {
			if ((key[0] === '$')) {
				elasticHelpers.debugTrace(opt, "   " + key + "=" + entity[key]);
			}
		});
	}

	// elasticHelpers.debugTrace(opt, "...title=" + entityTitle + ", classTitle=" + classTitle);
	mapping[entityName] = {
		properties: {},
		_meta: {
	...
	elasticHelpers.debugTrace(opt, "elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	elasticHelpers.debugTrace(opt, "elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
