...
function _createInstanceIndexResource(_, instance, endpoint, options) {
	...
	(entity.getSearchFields(_) || []).forEach_(_, function(_, field) {
		...
		if (p = entity.$properties[field]) {
			if(p.$isLocalized) {
				var v = inst.getPropAllLocales(_, field);
				if(v) {
					// normalize
					val = {};
...
function _indexMappingUpdate(_, context, indexName, entity, searchFacets, opt) {
	...
	entity.$properties && Object.keys(entity.$properties).forEach(function(p) {
		...
		if(prop.$isLocalized) {
			...
			localeCodes.forEach(function(l) {
				l = l.toLowerCase();
/*				pm[p].properties[l] = {
					type: "string",
					index_analyzer: _analyzersMap[l] || "english",
					search_analyzer: _analyzersMap[l] || "english",
					index: "analyzed"
				}*/
				//
				pm[p+"_"+l] = {
					type: "string",
//					analyzer: _analyzersMap[l] || "english",
					index_analyzer: _analyzersMap[l] || "english",
//					search_analyzer: _analyzersMap[l] || "english",
					index: "analyzed",
					include_in_all: true
