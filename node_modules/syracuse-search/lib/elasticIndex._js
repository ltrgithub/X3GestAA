...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntity: function(_, entity, searchFacets, options) {
		var opt = options || {};
		...
		// update data
		var where = (self._endpoint.lastIndexDate(_) ? {
			jsonWhere: {
				$updDate: {
		...
		if(inst) {
			...
			do {
				...
				var data = _createInstanceIndexResource(_, inst, options);
				options && options.tracer && options.tracer("ElasticIndex.updateEntity: index data: " + sys.inspect(data));
				req.write(_, JSON.stringify(data) + "\n");
	...
	updateIndex: function(_, options) {
		var opt = options || {};
		var self = this;
		...
		//
		_indexCreate(_, null, self._endpoint.getIndexName(_), opt.diagnoses);
		// 
		entities.forEach_(_, function(_, entity) {
			self.updateEntity(_, entity, facets, opt);
		});
		//
