...
function _indexMappingUpdate(_, context, indexName, entity, searchFacets, opt) {
	...
	mapping[entityName] = {
		properties: {},
		_meta: {
			$classTitle: entity.$titleTemplate.expression 
		}
	};
...
function _restUpdateEntityIndex(_, context, indexName, baseUrl, entityName, diagnoses) {
	...
	var itemProto = prototype.$properties.$resources.$item;
	while (dataUrl) {
	...
	//	console.log("index data: "+sys.inspect(data));
		...
		data.$resources.forEach_(_, function(_, item) {
			...
			req.write(_, JSON.stringify(action) + "\n");
			var data = {};
			flows.eachKey(_, itemProto.$properties, function(_, key, field) {
				var val = null;
				switch(field.$type) {
	...
	//		data.$title = item.$title;
			data.$description = _getDescription(_, indexName, entityName, item, itemProto.$properties);
