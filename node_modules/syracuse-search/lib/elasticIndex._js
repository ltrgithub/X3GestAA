...
function _createInstanceIndexResource(_, instance, endpoint, options, prototype) {
	...
	function _computeUrl(_, inst) {
		...
		opt.tracer && opt.tracer("compute url: " + sys.inspect(entity.$urlTemplate));
...
exports.IndexHelper = helpers.defineClass(ElasticIndex, null, {
	...
	updateEntity: function(_, entity, differential, options) {
		...
		exports.tracer && exports.tracer.trace("entity " + entity.name);
...
function _indexMappingUpdate(_, indexName, entity, searchFacets, opt) {
	...
	opt.tracer && opt.tracer("elasticIndex.index write mapping; entity: " + entityName + "; " + sys.inspect(mapping, null, 4));
	req.write(_, JSON.stringify(mapping));
	var resp = req.end().response(_);
	opt.tracer && opt.tracer("elasticIndex.index mapping update status: " + resp.statusCode + "; entity: " + entityName + "; diagnoses: " + sys.inspect(diagnoses));
	//
