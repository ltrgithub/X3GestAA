...
describe(module.id, () => {
	...
	it('test syracuse right filter  ', function(_) {
		...
		strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"missing":{"field":"representationRef.entity"}},{"query":{"field":{"representationRef.entity":"typeTests"}}}]}]}', "syracuse right filter : " + JSON.stringify(search.query.filtered.filter));
		...
		elasticQuery.addSyracuseRightFilter(_, syraRight, search, baseUrlIndex, true, prototypeUnitTest);
		strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"missing":{"field":"representationRef.entity"}},{"not":{"query":{"field":{"representationRef.entity":"typeTests"}}}}]}]}', "syracuse right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	it('test x3 right filter  ', function(_) {
		var search = {
			query: {
				filtered: {}
			}
		...
		resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
		strictEqual(resBool, true, "only $representation without condition,site has  right");
		console.error(JSON.stringify(search.query.filtered.filter));
	...
	it('test facet filter  ', function(_) {
		var search = {
			query: {
				filtered: {}
			}
	...
	// search is case insentitive, so values must be in lower case
