		...
		ez.devices.http.client(par).end().response(function(err) {
			if (!err) {
				...
				it('index name should use cloud prefix when asked in config' + item, function(_) {
					...
					try {
						config.searchEngine.indexCloudPrefix = "cloudprefix";
						let expectedSingleTenantOnPremiseIndexName = "cloudprefix.IndexTestName";
						let computedIndexName = IndexHelpers.getIndexNameWithMultitenantOrCloudV2Prefix("IndexTestName");
						...
						ok(false, " Test faield " + exp.stack);
					}
                ...
                it('index name should not use cloud prefix when not required in config' + item, function(_) {
                    ...
                    try {
                        config.searchEngine.indexCloudPrefix = undefined;
                        let expectedSingleTenantOnPremiseIndexName = "IndexTestName";
                        let computedIndexName = IndexHelpers.getIndexNameWithMultitenantOrCloudV2Prefix(expectedSingleTenantOnPremiseIndexName);
