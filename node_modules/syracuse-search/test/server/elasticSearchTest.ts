...
var elasticHelpers = require('../../lib/helpers');
		...
		ez.devices.http.client(par).end().response(function(err) {
			if (!err) {
				...
				it('init indexHelper - version elastic ' + item, function(_) {
					endPoint = adminHelper.getEndpoint(_, { dataset: "unit_test" });
				...
				it('test query syracuse administration 1 - version elastic ' + item, function(_) {
					...
					if (result.result.$resources.length == 0){
						ok(true, "STRANGE ERROR to investigate later");
						ok(true, "STRANGE ERROR to investigate later");
					} else {
						strictEqual(result.result.$resources.length != 0, true, "search body response ok : " + result.result.$resources.length);
				...
				it('index name should use cloud prefix when asked in config' + item, function(_) {

					try {
						config.searchEngine.indexPrefix = "cloudprefix";
						let expectedSingleTenantOnPremiseIndexName = "cloudprefix.IndexTestName";
						let computedIndexName = elasticHelpers.getFullIndexName("IndexTestName");
                        equal(computedIndexName, expectedSingleTenantOnPremiseIndexName, "use cloud index name prefix");
						//ok(expectedSingleTenantOnPremiseIndexName === computedIndexName, "On premise index name is not valid");
					} catch(exp){

						ok(false, " Test failed " + exp.stack);
					}
                ...
                it('index name should not use cloud prefix when not required in config' + item, function(_) {

                    try {
                        config.searchEngine.indexPrefix = undefined;
                        let expectedSingleTenantOnPremiseIndexName = "IndexTestName";
                        let computedIndexName = elasticHelpers.getFullIndexName(expectedSingleTenantOnPremiseIndexName);
                        console.log("computedIndexName: " + computedIndexName + " expectedSingleTenantOnPremiseIndexName: " + expectedSingleTenantOnPremiseIndexName  );
                        //ok(expectedSingleTenantOnPremiseIndexName === computedIndexName, "On premise index name is not valid");
                        equal(computedIndexName, expectedSingleTenantOnPremiseIndexName, "use on premise index name prefix");
                ...
                it('index name should not use tenantId prefix in multi tenant mode' + item, function(_) {

                    try {
                        config.searchEngine.indexPrefix = undefined;
                        globals.context.tenantId = "tenantIdTest";
                        config.multiTenant= true;
                        let expectedSingleTenantOnPremiseIndexName = "tenantIdTest.IndexTestName";
                        let computedIndexName = elasticHelpers.getFullIndexName("IndexTestName");
                        console.log("computedIndexName22: " + computedIndexName + " expectedSingleTenantOnPremiseIndexName: " + expectedSingleTenantOnPremiseIndexName  );
                        //ok(expectedSingleTenantOnPremiseIndexName === computedIndexName, "On premise index name is not valid");
                        equal(computedIndexName, expectedSingleTenantOnPremiseIndexName, "use multi-tenant index name prefix");
				...
				it('check the elasticsearch baseUrl is computed correctly in each case', function(_) {
					var searchConfig = config.searchEngine;
					config.searchEngine = {};

                    try {
						let expected = [
							{
								baseUrl: "http://elastic-test:9200",
								hostname: "elastic-test",
								port: 9200
							},
							{	
								baseUrl: "http://elastic-test:9333",
								hostname: "elastic-test",
								port: 9333
							},
							{	
								baseUrl: "https://elastic-test:9333",
								hostname: "elastic-test",
								port: 9333
							},
							{	
								baseUrl: "http://elastic-test",
								hostname: "elastic-test",
								port: 80
							},
							{	
								baseUrl: "https://elastic-test",
								hostname: "elastic-test",
								port: 443
							},
							{	
								baseUrl: "https://elastic-test:9999",
								hostname: "elastic-test",
								port: 9999
							}
						];
						...
						testConfigs.forEach(function(testConfig, index){
							searchEngine.setup(testConfig);

	                        strictEqual(searchEngine.getBaseUrl(), expected[index].baseUrl, "check the expected URL");
	                        strictEqual(searchEngine.serverConfig.hostname, expected[index].hostname, "check the expected hostname");
	                        strictEqual(searchEngine.serverConfig.port, expected[index].port, "check the expected port");
						});
                    } catch(exp){
                        ok(false, "test failed " + exp.stack);
                    } finally{
						searchEngine.setup(searchConfig);
					}
				});

			} else {
				it("skip elastic search version" + item, function(_) {
					ok(true, "Elastic search is not started");
				});
			}
		});
	});
