...
var IndexHelpers = require('../../lib/helpers');
			...
			if (!err) {
				...
				it('index name should use cloud prefix when asked in config' + item, function(_) {
					...
					try {
						...
						let computedIndexName = IndexHelpers.getFullIndexName("IndexTestName");
                ...
                it('index name should not use cloud prefix when not required in config' + item, function(_) {
                    ...
                    try {
                        ...
                        let computedIndexName = IndexHelpers.getFullIndexName(expectedSingleTenantOnPremiseIndexName);
				...
				it('check the elasticsearch baseUrl is computed correctly in each case', function(_) {
                    try {
						let expectedUrl = [
							"http://elastic-test:9200",
							"http://elastic-test:9333",
							"https://elastic-test:9200",
							"https://elastic-test:9333",
							"https://elastic-test",
							"http://elastic-test"
						];
						var testConfigs = [
						...
						testConfigs.forEach(function(testConfig, index){
	                        strictequal(IndexHelpers.getElasticBaseUrl(testConfig), expectedUrl[index], "check the expected URL");
						});
                    } catch(exp){
