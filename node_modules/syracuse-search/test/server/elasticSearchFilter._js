...
asyncTest("test syracuse right filter  with elasticsearch version < 1.x ", function(_) {
	var search = {
		query: {
			filtered: {}
		}
...
asyncTest("test x3 right filter with elasticsearch version < 1.x ", function(_) {
	var search = {
		query: {
			filtered: {}
		}
	...
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, false, "test with function authorized only to all function has  right ");
	strictEqual(!search.query.filtered.filter, true, "x3right filter : " + JSON.stringify(search.query.filtered.filter));
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
	strictEqual(resBool, false, "test with function authorized only to all function has no right ");
	strictEqual(!search.query.filtered.filter, true, "x3right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, true, "test with function but specified only few has right ");
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"regexp":{"$access":"666|gesaus|numberofthebeast|ironmaiden|_all"}}]}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
	strictEqual(resBool, false, "test with function but specified only few has no right ");
	strictEqual(!search.query.filtered.filter, true, "x3right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
	strictEqual(resBool, false, "empty access code only has no right ");
	...
	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
	strictEqual(resBool, false, "no right properties has no right");
	strictEqual(!search.query.filtered.filter, true, "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, false, "no right properties has no right");
	strictEqual(!search.query.filtered.filter, true, "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, true, "only $representation without condition,site or access code has right");
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"regexp":{"representationRef.entity":"test1|test2|test3"}}]},{"missing":{"field":"representationRef.entity"}}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"regexp":{"representationRef.entity":"testentity1|test2|testentity3"}}]},{"missing":{"field":"representationRef.entity"}}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"regexp":{"representationRef.entity":"test1"}}]},{"missing":{"field":"representationRef.entity"}}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, false);
	strictEqual(resBool, true, "only $representation without condition,accessCode with site and multiple representation has right");
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"and":[{"query":{"field":{"_type":"TEST1"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"and":[{"query":{"field":{"_type":"TEST2"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"and":[{"query":{"field":{"_type":"TEST3"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"query":{"field":{"_type":"TEST4"}}}]},{"or":[{"regexp":{"$access":"level1|test1|test2|test3|_all"}}]}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));

	search.query.filtered = {};
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, true, "only $representation without condition,accessCode with site and multiple representation has right");

	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"regexp":{"representationRef.entity":"test1|test2|test3|test4"}}]},{"missing":{"field":"representationRef.entity"}},{"or":[{"regexp":{"$access":"666|gesaus|numberofthebeast|test1|ironmaiden|bottleneck|swipping|_all"}}]}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
...
asyncTest("test right filter  with elasticsearch version >= 1.x ", function(_) {
	var search = {
		query: {
			filtered: {}
		}
	...
	var baseUrlIndex = "http://localhost:9200/syracuse.collaboration.syracuse.en-us";
	elasticQuery.elasticVersion = [1, 2, 1];
	// test syracuse filter generation on data
	syraRight.$entities.typeTest.condition = "author.$uuid eq test and toto.$uuid eq tst"; // test replace author in by  .$uuid $key;
	elasticQuery.addSyracuseRightFilter(_, syraRight, search, baseUrlIndex, false, prototypeUnitTest);
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"and":[{"query":{"query_string":{"default_field":"author.$key","query":"test"}}},{"query":{"query_string":{"default_field":"toto.$key","query":"tst"}}},{"query":{"query_string":{"default_field":"_type","query":"typeTest"}}}]}]}', "syracuse right filter : " + JSON.stringify(search.query.filtered.filter));
	...
	strictEqual(resBool, true, "only $representation without condition,accessCode with site and multiple representation has right");
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"and":[{"query":{"query_string":{"default_field":"_type","query":"TEST1"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"and":[{"query":{"query_string":{"default_field":"_type","query":"TEST2"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"and":[{"query":{"query_string":{"default_field":"_type","query":"TEST3"}}},{"terms":{"$site":["_all","a1","a2","a3"]}}]},{"query":{"query_string":{"default_field":"_type","query":"TEST4"}}}]},{"or":[{"regexp":{"$access":"level1|test1|test2|test3|_all"}}]}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
