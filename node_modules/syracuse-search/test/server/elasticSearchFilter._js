...
asyncTest("test x3 right filter  ", function(_) {
	...
	resBool = elasticQuery.addX3RightFilter(_, x3Right, search, true);
	strictEqual(resBool, true, "only $representation without condition,accessCode with site and multiple representation has right");
	strictEqual(JSON.stringify(search.query.filtered.filter), '{"and":[{"or":[{"query":{"field":{"representationRef.entity":"TEST1"}}},{"query":{"field":{"representationRef.entity":"TEST2"}}},{"query":{"field":{"representationRef.entity":"TEST3"}}},{"query":{"field":{"representationRef.entity":"TEST4"}}},{"missing":{"field":"representationRef.entity"}}]},{"or":[{"regexp":{"$access":"666|gesaus|numberofthebeast|test1|ironmaiden|bottleneck|swipping|_all"}}]}]}', "x3right right filter : " + JSON.stringify(search.query.filtered.filter));
