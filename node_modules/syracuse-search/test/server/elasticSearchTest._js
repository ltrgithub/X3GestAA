...
asyncTest("test indexation syracuse administration", function(_) {
	try {
		...
		strictEquals(resp.statusCode, 200, "index created ok");
...
asyncTest("test query syracuse administration", function(_) {


	start();
});

asyncTest("test delete index function syracuse administration", function(_) {
	try {
		var req = {
			url: elasticBaseUrl + "/syracuse.collaboration.unit_test.en-us/_mapping",
			method: 'GET'
		...
		var resp = streams.httpRequest(req).end().response(_);
		strictEquals(resp.statusCode, 404, "index not exists ok");
