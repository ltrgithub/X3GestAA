...
asyncTest("import demo database", 11, function(_) {
	...
	strictEqual(count, 40, "roles count ok");
	// check endpoints
...
asyncTest("test format resource type (date,decimal...)", 21, function(_) {
	var propItem = {
		$type = "application/x-decimal",
		$maxLength: 6.2
	};
	var resVal = formatValue("300.00", propItem); // already correct format passed in string
	strictEqual(JSON.stringify(resVal), "300,00", " already correct format passed in string");

	var resVal = formatValue("300", propItem); // not a decimal pass in string
	strictEqual(JSON.stringify(resVal), "300,00", " not a decimal pass in string");

	var resVal = formatValue(300.00, propItem); // not a decimal pass as a float
	strictEqual(JSON.stringify(resVal), "300,00", " not a decimal pass as a float");


	var resVal = formatValue(300, propItem); // not a decimal pass as a integer
	strictEqual(JSON.stringify(resVal), "300,00", " not a decimal pass as a integer");

	var resVal = formatValue(300.001111, propItem); // not a decimal pass as a float with too many decimal number
	strictEqual(JSON.stringify(resVal), "300,00", " not a decimal pass as a float with too many decimal number");

	var resVal = formatValue("<em>300.001111</em>", propItem); // with highlight
	strictEqual(JSON.stringify(resVal), "300,00", " with highlight");

	var resVal = formatValue("test", propItem); // non numeric but type decimal
	strictEqual(JSON.stringify(resVal), "test", " pass a non numeric value");

	var resVal = formatValue("<em>test<em>", propItem); // non numeric with highlight but type decimal
	strictEqual(JSON.stringify(resVal), "<em>test<em>", " pass a non numeric value");
