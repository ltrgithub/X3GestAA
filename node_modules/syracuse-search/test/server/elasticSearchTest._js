...
asyncTest("test query syracuse administration 2", 21, function(_) {
	...
	if (opts.result.$resources[0].$representation === 'user') {
		strictEqual(opts.result.$resources[0].$searchItemTitle, "mr  <em>Guest</em>", "resource that match the most ok : return " + opts.result.$resources[0].$searchItemTitle);
	} else if (opts.result.$resources[0].$representation === 'role') {
		strictEqual(opts.result.$resources[0].$searchItemTitle, "<em>Guest</em>", "resource that match the most ok : return " + opts.result.$resources[0].$searchItemTitle);
	} else if (opts.result.$resources[0].$representation === 'group') {
		strictEqual(opts.result.$resources[0].$searchItemTitle, "<em>Guests</em>", "resource that match the most ok : return " + opts.result.$resources[0].$searchItemTitle);
	} else if (opts.result.$resources[0].$representation === 'team') {
		strictEqual(opts.result.$resources[0].members[0], "<em>guest</em>", "resource that match the most ok : return " + opts.result.$resources[0].members);
	} else {
		strictEqual(opts.result.$resources[0].code, "<em>GUEST</em>", "resource that match the most ok : return " + opts.result.$resources[0].code);
	}

	debugTrace("==================Normal startsWith Query=======================");
...
asyncTest("test creation elastic search filter", function(_) {
	...
	debugTrace("==========================================");
	debugTrace("Quert :" + JSON.stringify(search));
