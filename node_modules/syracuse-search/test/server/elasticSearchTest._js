...
asyncTest("test query syracuse administration 1", 24, function(_) {
	...
	strictEqual(ctx.searchRequest.query.filtered.query.query_string.query, "group*", "check query string ok : " + ctx.searchRequest.query.filtered.query.query_string.query);
	hl = _extractHighlight(result.result.$resources);
	strictEqual(hl.length != 0, true, "check highlight " + hl.length);
...
asyncTest("test query syracuse administration 2", 19, function(_) {
	...
	strictEqual(result.result.$resources.length != 0, true, "search body response ok : " + result.result.$resources.length);
	var res = '{"filtered":{"query":{"query_string":{"query":"guest*","default_operator":"AND","analyze_wildcard":true,"allow_leading_wildcard":false}}}}';
	...
	strictEqual(res, JSON.stringify(ctx.searchRequest.query), "request ok ");
	console.log(JSON.stringify(result.result.$resources[0], null, 2));
	strictEqual(result.result.$resources.length != 0, true, "Search body response ok : " + result.result.$resources.length);
...
asyncTest("test delete index  syracuse administration", function(_) {
	try {
		...
		// delete all to be sure no remains index exists
		diag = [];

		elasticIndex.deleteAllIndex(_, diag);
