...
asyncTest("test query syracuse administration 2", function(_) {
	var result = {};
	var ctx = _makeContextQuerySearch("admin", result);
	elasticQuery.search(_, ctx);
	strictEqual(result.status, 200, "search ok ");
	strictEqual(result.result.$resources[0].code, "<em>ADMIN</em>", "resource that match the most ok : return " + result.result.$resources[0].code);
	// check first result is the right one 

	//test fuzzy request wit test
	ctx = _makeContextQuerySearch("test", result);
	elasticQuery.search(_, ctx);
	strictEqual(result.status, 200, "search ok ");
	strictEqual(result.result.$resources[0].login, "<em>guest</em>", "resource that match the most ok : return " + result.result.$resources[0].login);
	start();
});
