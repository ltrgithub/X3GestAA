...
///        - claimedRoles: optional array of claimed roles (e. g. ["emisor"])
///        - certifiedRoles: optional array of certified roles 
///         
...
exports.sign = function(_, xml, path, name, options) {
	...
	try {
		...
		if (options.includeKeyInfo) {
			...
			keyInfo += "<" + currentPrefix + "X509Data><" + currentPrefix + "X509Certificate>" + strippedCertificate.replace(/\r\n?/g, "\n") + "</" + currentPrefix + "X509Certificate></" + currentPrefix + "X509Data>" +
...
exports.verify = function(_, xml, name, options) {
	...
	// compare with certificate from XML
	if (options.chain) {
		var certificate = select(doc, "//*[local-name(.)='X509Certificate' and namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']")[0];
		if (certificate) {
			certificate = certificate.toString().replace(/<[^>]+>/g, "").replace(/[^A-Za-z0-9\/\+\=]/g, "");
			// test whether certificates are equal
			var tmp = cert.replace(/(BEGIN|END) CERTIFICATE/g, "").replace(/[^A-Za-z0-9\/\+\=]/g, "");
			console.log("TEMP " + tmp)
			console.log("TEM2 " + certificate)

			if (tmp != certificate) { // test whether certificate has been issued by given certificate
				var tmp = "-----BEGIN CERTIFICATE-----\n";
				var i = 0;
				for (i = 0; i < certificate.length - 64; i += 64) // insert line breaks in base64
					tmp += certificate.substr(i, 64) + '\n';
				tmp += certificate.substr(i) + '\n-----END CERTIFICATE-----\n';
				certificate = tmp;
				var check = jsx509.integrity(certificate, null, null, [cert]);
				if (check.error) sig.validationErrors.push("Certificate in XML has not been signed with provided certificate");
				cert = certificate; // now validate with certificate taken from XML
			}
		}
	}
...
//Return value: if it exists, an object containing the validity date, empty string otherwise
function certificateExists(_, name, withKey) {
	...
	var inst = db.fetchInstance(_, db.model.getEntity(_, "certificate"), {
		jsonWhere: condition
	});
	if (inst) {
		return {
			notAfter: inst.notAfter(_).toString()
		}
	} else {
		return "";
	}
