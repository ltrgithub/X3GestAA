...
exports.sign = function(_, xml, path, name, options) {
	...
	try {
		...
		if (options.xades) {
			...
			if (options.xades.hash && options.xades.hashAlgorithm) {
				hashAlgorithm = options.xades.hashAlgorithm;
			...
			// hack - should be done within addReferences
			sigOpts.attrs = {
				"Id": sigId
			};
...
exports.verify = function(_, xml, name, options) {
	...
	options = options || {};
	if (!options.notNormalizeLineEndings) xml = xml.replace(/\x0d\x0a/g, "\x0a")
	// console.log("JJJJJJJJJJJJJJJJJJJJJJJJJ" + xml);
	var doc = new Dom().parseFromString(xml);
