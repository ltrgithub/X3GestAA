...
///     canonicalizationAlgorithm: canonicalization algorithm (default: "http://www.w3.org/2001/10/xml-exc-c14n#") 
...
///  options: Object with additional information for signing:
///     signatureAlgorithm: signature algorithm
///     utf8: XML data (with UTF8 encoding) have been read in UTF8 mode, not in binary mode. Output also in UTF8 mode
///     includeKeyInfo: include key info into signature with certificate and details of public key
///     canonicalizationAlgorithm: canonicalization algorithm (default: "http://www.w3.org/2001/10/xml-exc-c14n#") 
///     xades: include xades information. Must be object with information:
///        - identifier: identifier, e. g. http://www.facturae.es/politica_de_firma_formato_facturae/politica_de_firma_formato_facturae_v3_1.pdf
...
exports.sign = function(_, xml, path, name, options) {
	...
	var sigOpts = {}; // options for signing
	...
	if (!options.utf8) {
		...
		if (encoding != "utf8" && encoding != "utf-8" && encoding.indexOf("iso-8859-") > 0) throw new Error("Unsupported encoding " + r[2]);
		if (encoding.indexOf("utf") >= 0)
			xml = new Buffer(xml, 'binary').toString("utf8");
	}
	// transform XML to canonical format before computing signature
	// xml = xml.replace(/\x0d\x0a?/g, "\x0a")
	/*	if (xml.indexOf("xmlns:") > 0) {
			var node = new Dom().parseFromString(xml);
			var can = new SignedXml.CanonicalizationAlgorithms["http://www.w3.org/2001/10/xml-exc-c14n#"]();
			xml = can.process(node.documentElement).toString();
	...
	try {
		...
		if (options.includeKeyInfo) {
			...
			keyInfo += "<X509Data><X509Certificate>" + strippedCertificate + "</X509Certificate></X509Data>" +
		...
		if (options.xades) {
			...
			objectInfo += '<etsi:CertDigest><DigestMethod Algorithm="' + sigAlgorithm + '"></DigestMethod><DigestValue>' + hash + '</DigestValue></etsi:CertDigest>';
			objectInfo += '<etsi:IssuerSerial><X509IssuerName>' + cert.issuerDn + '</X509IssuerName><X509SerialNumber>' + cert.serialDecimal + '</X509SerialNumber></etsi:IssuerSerial>';
			...
			objectInfo += '<etsi:SigPolicyHash><DigestMethod Algorithm="' + _makeAlgorithmURI(hashAlgorithm) + '"></DigestMethod><DigestValue>' + hash + '</DigestValue></etsi:SigPolicyHash>';
