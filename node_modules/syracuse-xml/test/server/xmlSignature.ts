...
describe(module.id, () => {
    ...
    it('check certificate existence', function(_) {
    	...
    	strictEqual(ex, false, "No certificate yet");
    });
    ...
    it('check certificate existence 2', function(_) {
    	...
    	strictEqual(ex, true, "Certificate exists");
    });
	...
	it('verify Facturae example', function(_) {
        ...
        let certInst = entity.createInstance(_, db, null);
        certInst.name(_, "facturae_ex_test");
        certInst.internal(_, false);
        certInst.keyExists(_, false);

        let caString = fs.readFileSync(fsp.join(__dirname, 'data/facturae.crt'), "utf8");
        let caBuf = new Buffer(caString);
        let caStore = certInst.certificate(_);
        let stream = caStore.createWritableStream(_, {
            length: caBuf.length
        ...
        let certName = "facturae_ex_test";
        let input = fs.readFile(__dirname + '/data/factura_ejemplo2_32v1.xml', 'utf8', _);
            
        let verif = xmlHelper.verify(_, input, certName, {addSignatureCanonicalization: true});
        // this XML file is not correct, therefore test first error
        strictEqual(verif.length && verif[0].indexOf("invalid signature: for uri  calculated digest is"), 0, "Error detected");
