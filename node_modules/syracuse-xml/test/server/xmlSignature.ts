...
const certtools = require('syracuse-load/lib/certTools');
...
describe(module.id, () => {
    ...
    it('sign and verify xml', function(_) {
        ...
        var CustomKeyInfo = function(_, keyPath, pp) {
            let prefix = 'ds';
            this.key = fs.readFile(keyPath, "utf8", _);


            this.getKeyInfo = function(key) {
                console.log("getKeyInfo called with key:", key);
                prefix = prefix || '';
                prefix = prefix ? prefix + ':' : prefix;
                return `<${prefix}X509Data><${prefix}X509Certificate></${prefix}X509Certificate></${prefix}X509Data>`;
            };
        ...
        for (let i in certificates) {
            ...
            let keyPath = fsp.join(jsx509Dir, "test/server/fixtures/" + certificates[i].name + ".crt");
            let input = fs.readFile(__dirname + '/data/input.xml', 'utf8', _);
            let myKeyInfo = new CustomKeyInfo(_, keyPath, certificates[0].passphrase);
            
            let signedXml = xmlHelper.sign(_, input, 'Facturae', {
                certificateName: certName,
                algorithm: 'RSA-SHA1',
                keyInfoProvider: myKeyInfo
            });
            console.log("signedXml:",signedXml);

            let verif = xmlHelper.verify(_, signedXml, myKeyInfo);
            strictEqual(verif, true, "Signature validated");
