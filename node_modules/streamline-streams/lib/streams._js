	...
	if (options.proxy) {
		...
		if (options.proxy.auth){
			if (options.proxy.auth.toLowerCase() === "basic"){
				...
				if (!ntlmEngine.proxyAuthenticate)
					throw new Error("NTLM Engine module MUST provide 'proxyAuthenticate' functions");
				options.proxyAuthenticate = ntlmEngine.proxyAuthenticate;
...
function HttpClientRequest(options) {
	...
	self.proxyAuthenticate = function(_) {
		if (options && options.proxyAuthenticate) options.proxyAuthenticate(_, options);
		return self;
	}
}
