...
function ReadableStream(emitter, options) {
	...
	function readChunk(callback) {
		if (options.timeout) {
			if (typeof emitter.setTimeout !== 'function') {
				callback(new Error('setTimeout function missing'));
				callback = null;
				return;
			}
			emitter.setTimeout(options.timeout, function() {
				if (callback) {
...
function _fixHttpClientOptions(options) {
	...
	if (options.proxy) {
		...
		if (options.host !== os.hostname()) {
			...
			// Check excludes
			if (options.proxy.excludes && options.proxy.excludes.indexOf(options.host) !== -1) {
				// Do nothing
			} else {
...
function HttpClientRequest(options) {
	...
	if (!options.proxyAuthenticate) _init();

	self.proxyAuthenticate = function(_) {
		if (options && options.proxyAuthenticate) {
