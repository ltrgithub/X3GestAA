...
function _getEncoding(headers, options) {
	...
	switch (options.detectEncoding) {
		case 'strict': return _getEncodingStrict(headers);
		case 'disable': return null;
		default: return _getEncodingDefault(headers);
	}
}
	...
	if (options.proxy) {
		...
		if (options.proxy.auth){
			if (options.proxy.auth.toLowerCase() === "basic"){
				...
				var ntlmEngine = options.proxy.ntlmEngine;
				if (!ntlmEngine)
					throw new Error("NTLM Engine module required");
				
				if (!ntlmEngine.generateNegociateMessage || !ntlmEngine.generateAuthenticateMessage)
					throw new Error("NTLM Engine module MUST provide 'generateNegociateMessage' and 'generateAuthenticateMessage' functions");
				
				options.ntlmEngine = ntlmEngine;
...
exports.httpRequest = function(options) {
	...
	if (options && options.ntlmEngine) {
		var msg1 = options.ntlmEngine.generateNegociateMessage("connectionOriented", options.proxy.domain, options.proxy.workstation);
		...
		msg1.on("done", function() {
			...
			var _negRequest = options.module.request(options, function(res) {
				...
				res.on('end', function () {
					...
					if (proxyAuth && proxyAuth.indexOf('NTLM ') === 0){
						...
						var msg3 = options.ntlmEngine.generateAuthenticateMessage("connectionOriented", msg1, options.proxy.user, options.proxy.password, options.proxy.domain, options.proxy.workstation, msg2);
						msg3.on("done", function() {
							...
							_authRequest.end();
							// HOW TO RETURN HTTP CIENT REQUEST HERE ?
							//new HttpClientRequest(options);
			...
			_negRequest.on('error', function(e) {
				console.log('problem with negociate request: ' + e.message);
			});
			_negRequest.end();
		});
...
wrapChainMethods(NetStream, ["setTimeout", "setNoDelay", "setKeepAlive", "ref", "unref"]);
...
function NetServer(serverOptions, connectionListener, streamOptions) {
	...
	var emitter = net.createServer(serverOptions, function(connection) {
		(function(_){
			globals.context = {};
			connectionListener(new NetStream(connection, streamOptions || {}));			
		})(void _);
	});
	Server.call(self, emitter);
}
