...
exports.decorate = function(proto) {
	...
	proto.fork = function(consumers) {
		...
		var sources = consumers.map(function(consumer) {
			...
			function read(cb) {
				trace && trace("FORK: inside read: pos=" + pos + ", position=" + position);
				var i = pos - position;
				var result;
				if (i < 0) throw new Error("invalid state 1 in fork: i=" + i);
				if (i >= 0 && i < results.length) {
					trace && trace("FORK: found in results");
					result = results[i];
					...
					if (next) {
						...
						trace && trace("FORK: at end");
						cb(null);
					}
				} else {
					trace && trace("FORK: queuing");
					q.push(cb);
				}
			}
			...
			return new Decorated(function(_) {
				...
				trace && trace("FORK: end of chain: " + val);
				if (val === undefined) {
					done++;
				}
				return val;
