...
exports.extendContract = function(contract, re) {
	function copy(src, dst) {
		Object.keys(src).forEach(function(name) {
			dst[name] = src[name];
		});
	}
	registry.scanPackages(function(elt, sub) {
		var exts = elt['contract-extensions'];
		if (exts && re.test(sub)) exts.forEach(function(ext) {
			extend(contract, require(ext.module));
		});
	});
};