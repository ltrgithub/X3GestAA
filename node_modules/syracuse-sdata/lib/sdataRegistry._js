...
function extend(contractRef, contractSec) {
	if (contractRef.application !== contractSec.application || contractRef.contract !== contractSec.extends) {
		console.error("MISMATCH ", contractSec);
		return;
	}

	function copy(src, dst) {
		Object.keys(src).forEach(function(name) {
			console.error("EXTENDS1: " + name);
			if (src[name].$extends) {
				console.error("EXTENDS: " + src[name]);
