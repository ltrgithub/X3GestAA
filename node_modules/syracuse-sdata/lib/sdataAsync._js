...
var asyncRepliesTimeout = nodeconfig.config && nodeconfig.config.session && nodeconfig.config.session.asyncTimeout;
...
var Tracker = helpers.defineClass(function(context, trackerResponse, trackngId, dispatch) {
	...
	flush: function(_, response) {
		if (this.response) {
			...
			this.response.flush(_, response);
			if (!this.response.managePaging && !this.replyLink) {
				this.response = null;
			}
		} else {
			var headers = {
	...
	start: function(_) {
		var self = this;
		// make sure request content is read before send 202 response
		if (this.context.request.reader) {
			// new ez-streams reader compatibility
			var body = this.context.request.reader.readAll(_);
			this.context.request.reader = this.context.request.reader.peekable();
			this.context.request.reader.unread(body);
		} else {
			// legacy reader
			var body = this.context.request.readAll(_);
			this.context.request.unread(body);
		}
		//
