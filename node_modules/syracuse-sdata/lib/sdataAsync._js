...
var TrackerResponse = helpers.defineClass(function(managePaging, trackingId) {
	...
	flush: function(_, response) {
		...
		if (this.managePaging) {
			...
			var length = this.managePaging.resourcesObject.length;
			if (this.managePaging.parameters) {
				var startIndex = this.managePaging.parameters.startIndex;
				var count = this.managePaging.parameters.count;
				var url = "/sdata/$trackers('" + this.managePaging.trackngId + "')";
			} else {
				var startIndex = 1;
				var count = this.managePaging.count || 20;
				var url = "--";
			}
			if (length > count) {
...
function _makeLink(url, startIndex, count) {
	return {
		"$url": url + "?startIndex=" + startIndex + "&count=" + count,
...
function _serialize(tracker) {
	var t = {
		...
		$links: tracker.$links,
		startDate: date.fromJsDate(new Date(tracker.startTime)).toString(),
		startTime: time.fromJsDate(new Date(tracker.startTime)).toString()
	};
...
exports.track = function(_, context, id) {
	...
	switch (context.request.method.toLowerCase()) {
		case 'get':
			if (tracker) {
				if (tracker.response.managePaging) {
