...
var Tracker = helpers.defineClass(function(context, trackerResponse, trackngId, dispatch) {
	...
	reply: function(_, response) {
		...
		if (!this.done) {
			...
			if (this.abortRequested) this.phase = locale.format(module, "aborted");
			else this.phase = locale.format(module, "completed");

			if (this.response) this.response.flush(_, response);
			else {
