...
function _logBox(box, prefix, linePrefix) {
	var s;
	debug.log((linePrefix ? linePrefix : "") + (prefix ? prefix + " " : "") + box.ident + " '" + box.data + "', parent=" + (box.parent && box.parent.ident) + ", before=" + (box.before && box.before.ident) + ", above=" + (box.above && box.above.ident));
	s = (linePrefix ? linePrefix : "") + "...rect={" + box.x + "," + box.y + "," + box.width + "," + box.height + "}";
	...
	var Box = helpers.defineClass(function(clas, meta, data) {
		...
		fill: function(_, outerWidth) {
			if (debug && debug.box) {
				...
				_logBox(this, "\t...before: this");
				_logBox(this.parent, "\t...before: parent");
			...
			if (this.meta === 'application/x-string') {
				...
				// debug && debug.box && debug.log("\t...workDoc=" + sys.inspect(workDoc, true, null));
				setFontStyle(_, this.style);
				workDoc.text(this.data, x, y, {
					width: inner,
				...
				if (image) {
					...
					workDoc.image(_, image.data, x, y, {
						fit: [image.width, image.height]
					}).link(x, y, image.width, image.height, image.url);
				}
			}
			this.adjustHeight(Math.max(this.height, workDoc.y - this.y + padding.bottom + border.bottom.width));
			...
			// }
			debug.log("\t...y=" + workDoc.y + ", height=" + this.height + ", parentHeight=" + this.parent.height);
			debug && debug.box && _logBox(this, "\t...after: this");
			debug && debug.box && _logBox(this.parent, "\t...after: parent");
			return this;
		},
		ident: {
			get: function() {
