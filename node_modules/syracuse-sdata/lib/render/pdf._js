...
// A shallow version of a pdf document that helps to compute the size of strings.
...
exports.render = function(_, context, data, proto) {
	...
	function(clas, meta, data, isLeaf) {
		...
		debug && debug.box && debug.log("new Box: " + clas + "(" + this.id + "), data=" + data + ", meta=" + meta + " , data.$title=" + (data ? data.$title : "") + ", meta.$title=" + (meta ? meta.$title : ""));
		...
		fill: function(_, outerWidth) {
			if (debug && debug.box) {
				...
				debug.log("\t\t\tshallow.rect={" + doc.shallow.x + "," + doc.shallow.y + "," + doc.shallow.width + "," + doc.shallow.height + "}");
			}
