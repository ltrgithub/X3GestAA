...
exports.render = function(_, context, data, proto) {
	var debug = (context.pdfReport && context.pdfReport.debug) || (config.pdfReport && config.pdfReport.debug);
	...
	if (debug) {
		...
		debug.output && debug.output.data && fs.writeFile(".out/" + debug.output.data, JSON.stringify(data), "utf8", _);
		debug.output && debug.output.proto && fs.writeFile(".out/" + debug.output.proto, JSON.stringify(proto), "utf8", _);
	}
	var options = {
		size: 'A4',
	...
	function getImage(_, prop, val) {
		...
		var url = format(val.$url || prop.$url, prop, val);
		debug && debug.image && debug.log("LOADING IMAGE: " + val)
...
/*
	...
	function(clas, meta, data, unbreakable) {
	...
	function buildBox(article, meta, data, clas, title) {
		...
		if (box.layoutType === 'table') {
			child = new Box('header', box.meta, box.data);
			box.items = [child];
		}
	...
	function valueBox(_, box) {
		...
		if (val != null && prop) {
			...
			case 'application/x-string':
				// Values might be localized at the application level
				// val = localizedText(val.toString());
				// break;
		...
		case 'table':
			...
			var rows = resources.map_(_, function(_, r) {
				...
				row.items = header.items.map_(_, function(_, col) {
					var prop = col.meta;
					var cell = new Box('cell', prop, r[col.bind], true);
