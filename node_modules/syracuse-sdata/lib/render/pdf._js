...
var debug = config.pdfReport && config.pdfReport.debug;
...
// A shallow version of a pdf document that helps for computing size of strings.
// The source code was generate by the CoffeeScript compiler from the pdfkit files, then it was slightly adapted.
...
exports.render = function(_, context, data, proto) {
	...
	function(clas, meta, data, isLeaf) {
		...
		// breadCrumb: {
		// 	get: function() {
		// 		return this.parent == null ? this.ident : this.parent.breadCrumb + " > " + this.ident;
		// 	}
		// },
		// getRootBox: function() {
		// 	var r;
	...
	function buildBox(article, meta, data, clas, title) {
		...
		if (box.layoutType === 'table') {
			child = new Box('row', box.meta, box.data);
			box.items = [child];
		}
	...
	function computeBox(_, box) {
		...
		case 'table':
			...
			var headerRow = new Box('row', box.meta, box.data);
			headerRow.items = box.items;

			// create data rows
			// box.rows = resources.map_(_, function(_, r) {
			box.items = resources.map_(_, function(_, r) {
				var row = new Box('row', m, r);
				// headerRow.items are columns
				row.items = headerRow.items.map_(_, function(_, col) {
					var prop = col.meta;
					var cell = new Box('cell', prop, r[col.bind]);
					cell.value = valueBox(_, cell);
					// cell.place(row, prev, box.title).fill(_, inner / count);
			...
			var colStyle = sst.boxStyle('column');
			...
			// row headers
			headerRow.items.forEach_(_, function(_, cell) {
				cell.place(headerRow, prev, above).fill(_, w);
			...
			box.items.forEach_(_, function(_, row, i) {
				...
				prevRow = row;
				row.items.forEach_(_, function(_, cell, i) {
					var header = box.items[i];
					// cell.place(row, prev, above).fill(_, header.inner);
					cell.place(row, prev, null).fill(_, header.inner);
			...
			// add header to the items list for drawing
			box.items.unshift(headerRow);
