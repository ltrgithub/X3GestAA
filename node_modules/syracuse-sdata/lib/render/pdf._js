...
exports.render = function(_, context, data, proto) {
	...
	function createPdfDocument(_, options) {
		...
		doc.computeTextRect = function(text, x, y, options) {
			return this.shallow.computeTextRect(text, x, y, options)
		};

		doc.computeImageRect = function(_, src, x, y, options) {
			return this.shallow.computeImageRect(_, src, x, y, options)
		};
	...
	function(clas, meta, data, isLeaf) {
		...
		debug && debug.box && debug.log("new Box: class=" + clas + ", data=" + data + ", meta=" + meta + " , data.$title=" + (data ? data.$title : "") + ", meta.$title=" + (meta ? meta.$title : ""));
		...
		draw: function(_, context, index) {
			...
			var bg = this.style.background,
				...
				bw = border.top.width,
				bc = border.top.color;
			debug && debug.box && debug.log("\tBox.draw: border=" + JSON.stringify(border));
			doc.lineWidth(bw);
	...
	function buildBox(article, meta, data, clas, title) {
		debug && debug.buildBox && debug.log("buildBox: data=" + data + ", meta=" + meta + ", class=" + clas + ", data.$title=" + (data ? data.$title : "") + ", article.$title=" + article.$title + ", meta.$title=" + (meta ? meta.$title : ""));
	...
	function valueBox(_, box) {
		...
		debug && debug.valueBox && debug.log("valueBox: prop=" + (prop ? prop.$type : "undefined"));
		if (val != null && prop) {
			switch (prop.$type) {
			case 'application/x-string':
