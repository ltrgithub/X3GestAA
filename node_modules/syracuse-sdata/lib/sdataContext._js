...
function Context(request, response, options) {
	...
	// valora
	self._mobileIronHeaders = {};
	for (var name in self.request.headers) {
		/*if (name.indexOf("x-mobileiron") === 0 || name === "x-forwarded-for" || name === "origin"){
			httpHelper.tracer && httpHelper.tracer("save header: " + name + " - len:" + self.request.headers[name].length);
			self._mobileIronHeaders[name] = self.request.headers[name];
			if (name === "origin"){
				self._mobileIronHeaders["Access-Control-Allow-Origin"] = "*";
			}
		}*/
		self._mobileIronHeaders[name] = self.request.headers[name];
	}
...
exports.Context = helpers.defineClass(Context, HttpContext, {
	checkAlreadyProcessRequest: function() {
	...
	reply: function(_, statusCode, result, headers) {
		...
		if (statusCode >= 400 && typeof result === "string" && config.hosting.multiTenant) {
			var genericMessage = httpHelper.statusMessages[statusCode] || ("Error " + statusCode);
		...
		// Valora
		for (var name in self._mobileIronHeaders) {
			if (headers[name] == null) {
				headers[name] = self._mobileIronHeaders[name];
				httpHelper.tracer && httpHelper.tracer("Restore header: " + name + " - len:" + headers[name].length);
			}
		}
		httpHelper.tracer && httpHelper.tracer("HTTP RESPONSE: " + statusCode + " " + JSON.stringify(headers));
	...
	getRepresentationSelect: function(contract, representation, facetName) {
	...
	exception: function(statusCode, message) {
		...
		var ex = new Error(httpHelper.statusMessages[statusCode] || ("Error " + statusCode));
