...
/*									for(var i = template.matches.length - 1; i >= 0; i--)
	...
	reply: function(_, statusCode, result, headers) {
		...
		case "docx":
			// TODO: Review also, see xlsx 
			headers["content-type"] = httpHelper.mediaTypes.docx;
			headers["cache-control"] = "no-cache,must-revalidate";

			self.response.writeHead(statusCode, headers);
			var docxUrl = self.url + "?representation=" + self.parameters.representation;
			require('msoffice/lib/word/docx').render(_, self.response, docxUrl);
			self.response.end();
			return;
				...
				if (rel.isPlural) {
					...
					case "children":
						...
						if (representation.type === "$edit") {
							...
							if(rel.$select) {
								$r.$actions.$create.$properties = $r.$actions.$create.$properties || {};
								var selectEnt = model.getEntity(rel.$select.$type);
								$r.$actions.$create.$properties.$ = $r.$actions.$create.$properties.$ || {};
								$r.$actions.$create.$properties.$.$select = {
...
//									$r.$actions.$create.$properties.$select.$selectField = rel.$select.$selectField;
							}
						}
						break;
					default:
			...
			if ((representation.type !== "$child") && !isChild) {
				...
				if (queryRepr.indexOf(representation.type) >= 0) {
					...
					// WORDHACK / TODO: NLS
					resource.$links.$word = self.getLink("", "{$pluralType}", representation.type, "Word");
					resource.$links.$word.$type = httpHelper.mediaTypes.docx;
...
//		if (!mainEntity) throw self.notFound(locale.format(module, "entityNotFound", model.singularize(entityName)));
		...
		if (addSectionInfo) {
			...
			if (repMeta) {
				...
				if(stringRes[representationName]) res.$localization = stringRes[representationName];
				// merge meta
