...
*/
		...
		if (!nocompress && self.request.headers['accept-encoding'] && self.request.headers['accept-encoding'].split(',').indexOf('gzip') !== -1) {
			result = zlib.gzip(new Buffer(result, encoding), ~_);
	...
	replyPrototype: function(_) {
		...
		if (reqEtag && (reqEtag === locEtag)) return self.reply(_, 304, {});
