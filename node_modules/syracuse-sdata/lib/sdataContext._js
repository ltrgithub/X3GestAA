...
function _formatFeed(context, url, results) {
	...
	feed.$links.$wordmailmerge = {
			$url: feed.$url + "&createMode={creationMode}&doc_uuid={document}",
		...
		if (params.trackingId && params.binding) {
			...
			if (rel.$lookupFilter) {
				if (typeof rel.$lookupFilter !== "function")
					...
					processFilter(_, instance, filter, rel.$lookupFilter(_));
			}
			//
			if (rel.isPlural) {
...
/*			if (userProfile.selectedEndpoint(_) && !userProfile.selectedEndpoint(_).isSame(_, self.application && self.application.name, self.contract && self.contract.name, self.dataset)) {
	...
	getPrototypeResource: function(_, prototypeId, addSectionInfo, applicationName, contractName, datasetName) {
		...
		function _fillThumb($thumb, targetEntity, rel, prefix) {
			...
			$thumb.$representation = targetEntity.name;
			$thumb.$links = {
		...
		function _getEntityPrototype(_, entity, isChild, childPrefix) {
			...
			resource.$prototype = "{$baseUrl}/$prototypes('{$representation}." + representation.type + "')";
			if (!isChild) {
...
//				}
			...
			flows.eachKey(_, entity.$relations, function(_, name, rel) {
				...
				$r.$isDisabled = rel.$isDisabled;
				//
				var cap = (rel.$capabilities && rel.$capabilities.split(",")) || [];
				var i_add = cap.indexOf("add");
				var i_del = cap.indexOf("delete");
				// ad standard caps only if the class caps is not defined
				if(rel.$capabilities == null) {
					if(cap.indexOf("sort") < 0) cap.push("sort");
					if(representation.type === "$edit") {
						if(i_add < 0) cap.push("add");
				...
				if (rel.isPlural) {
					...
					case "link":
						if (rel.$assoType) {
							...
							($r.$links = $r.$links || {}).$select = self.getLink(rel.targetEntity.name, rel.targetEntity.plural, "$select", 
									locale.format(module, "createTableActionTitle"), "trackingId={$trackingId}" + (rel.name ? "&binding=" + thumbRelName : ""));
						}
						break;
					case "children":
...
//									$r.$actions.$create.$parameters.$actions.$select.$selectField = rel.$select.$selectField;
			...
			if ((representation.type !== "$child") && !isChild) {
				entity.fillLinksResource(representation.type, resource);
				//
				self.addWordLinks(resource, representation);
				self.addPptLinks(resource, representation);
				//
				if (queryRepr.indexOf(representation.type) >= 0) {
					...
					// EXCELHACK
					resource.$links.$excel = self.getLink("", "{$pluralType}", representation.type, locale.format(module, "excel"));
...
//		console.log("order by: "+sys.inspect(orderBy));
	...
	addWordLinks: function(resource, representation)
	{
		var self = this;
		if (queryRepr.indexOf(representation.type) >= 0) {
			resource.$links.$wordmailmerge = {
					"$title": locale.format(module, "mailMerge"),
					"$url": self.getLink("", "{$pluralType}", representation.type).$url + "&createMode={creationMode}&doc_uuid={document}",
					"$type": httpHelper.mediaTypes.word_mailmerge,
		...
		resource.$links.$wordreport = {
			...
			"$url": self.getLink("", "{$pluralType}", representation.type).$url + "&reportMode={reportMode}&doc_uuid={document}",
			"$type": httpHelper.mediaTypes.word_report,
			
			"$parameters": {
				"$url": "{$baseUrl}/msoReportModes/$template/$workingCopies?representation=msoReportMode.$edit&role={$role}",
				"$method": "POST",
				"$properties": {
					"dummy": { 
						"$title": "dummy",
	...
	addPptLinks: function(resource, representation)
	{
		var self = this;
		if (queryRepr.indexOf(representation.type) >= 0) {
			resource.$links.$pptslide = {
				"$title": locale.format(module, "createNewSlide"),
				"$url": self.getLink("", "{$pluralType}", representation.type).$url + "&pptMode=newSlide",
