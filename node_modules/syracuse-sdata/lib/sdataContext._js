...
*/
	...
	replyProperty: function(_, instance, propertyName) {
		...
		if (prop.isExternalStorage()) {
			...
			var headers = {
				...
				//"expires": (new Date(0)).toString(), // will be add in syracuse._js
	...
	replyPrototype: function(_) {
		...
		if (proto) this.reply(_, 200, proto, {
			etag: locEtag,
			//expires: (new Date()).toGMTString() // will be add in syracuse._js
		});
	...
	exception: function(statusCode, message) {
		...
		console.error("SData status " + statusCode + ": " + message);
	...
	replyResource: function(_, statusCode, resource, etag) {
		...
		var headers = etag ? {
			etag: etag,
			//			expires: (new Date()).toGMTString() // IE9 needs this header in order to manage ETag; is added in syracuse._js, not here
		} : null;
