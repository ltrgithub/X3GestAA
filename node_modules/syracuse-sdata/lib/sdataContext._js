...
function _formatFeed(_, context, url, results) {
	...
	if (context.entity && context.entity.isCapableOf("excelReport")) {
		feed.$links.$excelreport = {
			$url: feed.$url + "&reportMode={reportMode}&doc_uuid={document}&docx=application/syracuse-excel-report",
			$title: locale.format(module, "excelReport"),
			$type: httpHelper.mediaTypes.excel_report
		};
	}
...
exports.Context = helpers.defineClass(Context, HttpContext, {
	...
	addComplementFilter: function(_, params) {
		...
		if (params.trackingId && params.binding) {
			...
			if (rel.$lookupFilter) {
				if (typeof rel.$lookupFilter !== "function") processFilter(_, instance, filter, rel.$lookupFilter);
				else processFilter(_, instance, filter, rel.$lookupFilter(_, instance));
			}
			//
			if (rel.isPlural) {
...
*/
	...
	getPrototypeResource: function(_, prototypeId, addSectionInfo, applicationName, contractName, datasetName) {
		...
		function _getEntityPrototype(_, entity, isChild, childPrefix) {
			...
			p.$baseUrl = url;
			p.$baseHelpUrl = url.replace("/sdata/", "/help/" + locale.current.replace('-', '_') + "/");
			return p;
		}
		//
		var self = this;
