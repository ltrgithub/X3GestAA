...
function _formatKeyPagerFeed(context, url, results) {
	...
	if (!context.isLastPage) {
		parameters.key = "lt.\"zzzzzzzzzzzzzzzz\"";
		feed.$links.$last = {
			$url: url + _queryString(parameters),
			$type: "application/json; vnd-sage=syracuse"
		}
...
function _formatFeed(context, url, results) {
	...
	feed.$links.$wordmailmerge = {
			$url: feed.$url + "&createMode={createMode}.&doc_uuid={doc_uuid}.&docx=mailmerge",
...
/*									for(var i = template.matches.length - 1; i >= 0; i--)
	...
	reply: function(_, statusCode, result, headers) {
		...
		case "docx": 
			...
			self.response.writeHead(statusCode, headers);
			var proto = self.getPrototypeResource(_, self.parameters.representation, true);
			require('msoffice/lib/word/docx').render(_, self, result, proto);
			self.response.end();
			return;
...
//				}
			...
			flows.eachKey(_, entity.$relations, function(_, name, rel) {
				...
				$r.$capabilities = (rel.$capabilities != null) ? rel.$capabilities : (isChild ? "sort,add,delete" : "sort,filter,add,delete");
...
//									$r.$actions.$create.$parameters.$actions.$select.$selectField = rel.$select.$selectField;
			...
			if ((representation.type !== "$child") && !isChild) {
				...
				if (["$lookup", "$select"].indexOf(representation.type) >= 0) {
					resource.$links = {
						$details: self.getLink("", "{$pluralType}", "$details", locale.format(module,"detailsLinkTitle"))		 
					}
				}
...
//				console.log("prop: "+prop+"; a: "+a[prop](_)+"; b: "+b[prop](_));
	...
	{
		var self = this;
		/* TODO: Review later / seems not to work, should be used for both links 
		resource.$links.$word = {
					"$title": "Word",
		...
		*/
		...
		resource.$links.$wordreport_tpl = {
				"$title": locale.format(module, "wordReportTpl"),
				"$url": self.getLink("", "{$pluralType}", representation.type).$url,
				"$type": httpHelper.mediaTypes.word_report_tpl
		};

		resource.$links.$wordreport = {
				"$title": locale.format(module, "wordReport"),
				"$url": self.getLink("", "{$pluralType}", representation.type).$url + "&createMode={createMode}.&doc_uuid={doc_uuid}.&docx=print",
		...
		if (queryRepr.indexOf(representation.type) >= 0) {
			resource.$links.$wordmailmerge = {
				"$title": locale.format(module, "mailMerge"),
				"$url": self.getLink("", "{$pluralType}", representation.type).$url + "&createMode={createMode}.&doc_uuid={doc_uuid}.&docx=mailmerge",
				"$type": httpHelper.mediaTypes.word_mailmerge,
