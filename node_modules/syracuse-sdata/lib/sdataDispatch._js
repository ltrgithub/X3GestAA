	...
	calendarQuery: function(_, context) { // format query response that map x-calendar to calendar response expect by the client
		function getPropertyValue(listProp, object) {
			...
			listProp && listProp.forEach(function(cur) {
				array.push(object[cur]);
			});
			return array;
		}

		function getPropertyDescription(listProp, object) { // must create a field with ul , li
			var descr = "<ul>";
			listProp && listProp.forEach(function(cur) {
						...
						$resources.forEach(function(r) {
							...
							if (!start || !end || !summary || !description) {
								...
								events.push({
									...
									start: {
										datetime: start.length === 2 ? start[0] + "T" + start[1] + "Z" : ""
									},
									end: {
										datetime: end.length === 2 ? end[0] + "T" + end[1] + "Z" : ""
									},
								});
							}
						});
...
var _syncSourceMap = {
	post: function(_, context) {
		try {
			...
			tracker.location = "/sdata/$trackers('" + context.parameters.trackngId + "')?reply=true";
			tracker.start(_);
		} catch (err) {
