...
var ignoreTimeHit = ["userBookmarkProxy", "pageLayoutProxy"];

function sendTimeToGA(_, context, catPrefix, startTime) {
	if (context.parameters.representation) {
		var parts = context.parameters.representation.split('.');
		if (ignoreTimeHit.indexOf(parts[0]) === -1) {
			ga.hitTiming(_, context, catPrefix + " loading time", parts[1], (new Date().getTime()) - startTime, parts[0]);
		}
	}
}
	...
	walk: function(_, context, name, id) {
		...
		if (context.parameters.search) {
			...
			if (context.path && context.path.indexOf('/$batchGet') > 0) {
				...
				var res = x3pool.send(_, context);
				sendTimeToGA(_, context, "X3", startTime);
				return res;
			}
		}
	},
...
var _entityMap = {
	get: function(_, context) {
		...
		if (!context.contract && stubsPath) {
			...
			var instances = factory.replyInstances(_, context);
			// stats
			sendTimeToGA(_, context, "Collaboration", startTime);
			return instances;
		}
	},
	post: function(_, context) {
