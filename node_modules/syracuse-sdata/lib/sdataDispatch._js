...
var _x3Hacks = {
	...
	// Mobile #3369
	// Temporarilly - Implements X3 object method calls in batch mode because not implemented in X3 side
	batchAction: function(_, context, representation, id) {
		var respBody, wrkCpyData, error, warning, tracer;
		tracer = null; //console.log;
		tracer && tracer("batchAction - representation=" + representation);
		try {
			...
			tracer && tracer("batchAction - actParams\n" + JSON.stringify(actParams, null, 2));
			// -1- Create a working copy
			tracer && tracer("batchAction - Create working copy  - url -> " + url);
			...
			tracer && tracer("batchAction - Create working copy - statusCode=" + reply.head.statusCode);
			tracer && tracer("batchAction - Create working copy - returnedData\n" + JSON.stringify(reply.body, null, 2));
			if (reply.head.statusCode == 200) {
				...
				tracer && tracer("batchAction - Update working copy - wrkCpyUrl -> " + wrkCpyUrl);
				actParams.$data.$uuid = wrkCpyData.$uuid;
				actParams.$data.$url = wrkCpyUrl;
				tracer && tracer("batchAction - Update working copy - postData\n" + JSON.stringify(actParams.$data, null, 2));
				var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
					head: {
						"accept": "application/json;vnd.sage=syracuse",
						"content-type": "application/json; charset=UTF-8",
				...
				tracer && tracer("batchAction - Update working copy - statusCode=" + reply.head.statusCode);
				tracer && tracer("batchAction - Update working copy - returnedData\n" + JSON.stringify(reply.body, null, 2));
				if (reply.head.statusCode == 200) {
					// -3- Call method with parameters given by mobile client - actParams
					var body = {
						$uuid: reply.body.$uuid,
						$etag: reply.body.$etag,
						$actions: actParams.$actionParams,
						$url: wrkCpyUrl
					};
					tracer && tracer("batchAction  - ExecAction - url =" + wrkCpyUrl);
					tracer && tracer("batchAction  - ExecAction - postData =" + JSON.stringify(body, null, 2));
					var ref = "http://127.0.0.1:8124/syracuse-main/html/main.html?url=" + encodeURIComponent(wrkCpyUrl);
					var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
						head: {
							"accept": "application/json;vnd.sage=syracuse",
							"content-type": "application/json; charset=UTF-8",
					...
					tracer && tracer("batchAction - ExecAction statusCode = " + reply.head.statusCode);
					tracer && tracer("batchAction - ExecAction DATA\n" + JSON.stringify(reply.body, null, 2));
					if (reply.head.statusCode == 200) {
						var diag = reply.body.$diagnoses;
						// -4- Read working copy and return result
						// -> In batch mode we have to return all object data (not only delta returned by action call) to update mobile client local working copy
						var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
							head: {
								"accept": "application/json;vnd.sage=syracuse",
								"content-type": "application/json; charset=UTF-8",
						...
						tracer && tracer("batchAction - NEW working copy - statusCode=" + reply.head.statusCode);
						tracer && tracer("batchAction - NEW working copy - returnedData\n" + JSON.stringify(reply.body, null, 2));
						if (reply.head.statusCode == 200) {
							// batch protocol returns $httpStatus per resource
							reply.body.$httpStatus = 200;
							if (diag) {
								// Add action diagnoses to working copy
								reply.body.$diagnoses = diag;
							}
							respBody = reply.body;
						} else {
							error = {
								$message: "Error reading new working copy",
								$stackTrace: "Status : " + reply.head.statusCode + "\nUrl : " + url + "\nBody:" + JSON.stringify(reply.body, null, 2)
							};
						}
					...
					error = {
						$message: "Error updating workingcopy",
						$stackTrace: "Status : " + reply.head.statusCode + "\nUrl : " + url + "\nBody:" + JSON.stringify(reply.body, null, 2)
					};
				}
			} else {
			...
			error = {
				$message: "Javascript Error executing batch action",
				$stackTrace: context.request.url + "\n" + e.stack
			};
		} finally {
			if (wrkCpyData) {
				tracer && tracer("batchAction - DELETE  Working copy");
				// -3- Delete workingcopy if any
				var url = wrkCpyData.$url.replace("{$baseUrl}", wrkCpyData.$baseUrl);
				tracer && tracer("batchAction -DELETE url " + url);
				var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
					head: {
						"accept": "application/json;vnd.sage=syracuse",
						"content-type": "application/json; charset=UTF-8",
				...
				tracer && tracer("batchAction -DELETE statusCode = " + reply.head.statusCode);
				tracer && tracer("batchAction -DELETE response " + (reply.body ? "\n" + JSON.stringify(reply.body, null, 2) : "EMPTY"));
				if (reply.head.statusCode != 200) {
					warning = {
						$severity: "warning",
					...
					tracer && tracer("batchAction -DELETE OK");
				}
			}
		}
		if (error) {
...
var _propertyLocalizeMap = {
	get: function(_, context) {
		...
		return factory.replyLocalizedProperty(_, context, instance, context.propertyName, true);
	},
	put: function(_, context) {
		var instance = context.instance;
		if (!instance) return context.reply(_, 404, "The requested instance was not found");
		return factory.updateLocalizedProperty(_, context, context.initialInstance, instance, context.propertyName, context.propertyName, JSON.parse(context.request.readAll(_)));
	}
};
...
var _instanceMap = {
	walk: function(_, context, name, id) {
		...
		if (!context.instance) context.initialInstance = inst;
			...
			if (take) {
				...
				if (!resource.$isDeleted) {
					...
					if (endpointNumber <= 2) { // always delete special instance for deleted instances when there are 2 endpoints 
						...
						delInst.endpoint(_, resource.$endpoint);
...
var _syncSourceMap = {
	post: function(_, context) {
		try {
			...
			tracker.location = "/sdata/$trackers('" + context.parameters.trackngId + "')";
			tracker.start(_);
		} catch (err) {
...
function _doIt(_, context) {
	try {
		...
		if ("phone" === context.parameters.device) {
			...
			return context.reply(_, ex.httpStatus || 500, {
				$diagnoses: diags
			});
			/*			if (ex.httpStatus) {
...
exports.dispatcher = function(options) {
	...
	return function(_, request, response) {
		if ((options.etna && /^\/sdata\/x3/.test(request.url)) || /^\/sdata\/etna/.test(request.url)) return require("etna-supervisor/lib/server").httpDispatch(_, request, response);
