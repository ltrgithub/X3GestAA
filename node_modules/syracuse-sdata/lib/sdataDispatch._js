...
var uploadHelpers = require('streamline-upload/lib/helpers');
...
var _items = {
	...
	"LNKIMG": {
		...
		"$type": "image",
		"$isReadOnly": true,
		"$url": "{$baseUrl}/BDOC('R_BPARTNER~%C2%A8~LNKIMG')"
	},
	...
	"LNKEXPCOL": {
		...
		"$item": {
			...
			"$instanceUrl": "{$instanceUrl}/LNKEXPCOL('{$uuid}')",
			"$properties": {
				"LNKEXPCPY": {
					"$title": "Company",
					"$shortTitle": "Company",
...
var _linkedInData = {
	"firstName": "Dominique",
	"headline": "Software Evangelist, Sage ERP X3 at Sage",
	"id": "EazAXw3Ufk",
	...
	"positions": {
		...
		"values": [{
			...
			"title": "Software Evangelist, Sage ERP X3"
		}, {
			"company": {
				"id": 2802,
				"name": "Sage Software"
...
function getLinkedInProfile(_, email) {

	// return _linkedInData;

	var map = {
		"AO001": "http://fr.linkedin.com/in/dbopp",
		"AO002": "http://de.linkedin.com/pub/mathias-walz/97/206/354",
		"AO003": "http://de.linkedin.com/pub/norbert-butzke/85/b87/138",
		"AO004": "http://fr.linkedin.com/in/sophierebeyrotte/de",
		"AO005": "http://www.linkedin.com/pub/richard-burns/29/a4b/303"
	};
// Mathias
//	var map = linkedin.syncMap;

	var url = map[email];
	if (!url) return null;
	return linkedin.getUserByUrl(_, url);
}

function transformPayload(_, linkedInPayload, x3Payload) {
	linkedInPayload = JSON.parse(linkedInPayload);
	getPosition(_, linkedInPayload, x3Payload);
	getSkills(_, linkedInPayload, x3Payload);
	getGeneral(_, linkedInPayload, x3Payload);
	return x3Payload;
}

function getPosition(_, linkedInPayload, x3Payload) {
	var positions = linkedInPayload.positions;
	if (positions == null) {
		return;
	}
	...
	positions.forEach(function(pos) { 	
		...
		var x3 = {};
			...
			x3.LNKEXPCPY = cpy.name;
			x3.LNKEXPTITLE = pos.title;

			x3.LNKEXPDAT = pos.startDate.year + "-";
			if (pos.startDate.month == null) {
				x3.LNKEXPDAT += "01";
			} else {
				var m = pos.startDate.month +"";
				if (m.length == 1) {
					x3.LNKEXPDAT += "0";
				}
				x3.LNKEXPDAT += m;
		...
		x3Payload.LNKEXPCOL.push(x3);
	});
}

function getSkills(_, linkedInPayload, x3Payload) {
	x3Payload.LNKSKLCOL = ["skill1", "skill2"];
}

function getGeneral(_, linkedInPayload, x3Payload){
	if (linkedInPayload.pictureUrl && linkedInPayload.pictureUrl.length > 0) {
		x3Payload.LNKIMG = {"$url" : linkedInPayload.pictureUrl};
	} else {
		x3Payload.LNKIMG = {"$url" : "http://img1.wikia.nocookie.net/__cb20120511110342/lou/images/a/ac/No_image_available.svg"};
	}

	x3Payload.LNKCRY = "FRA";
	x3Payload.LNKSUM = linkedInPayload.summary || "";
}
// STUBS
...
var _x3ContractMap = {
	walk: function(_, context, name, id) {
		...
		if (context.applicationRef) { // TEST to allow stubs
			...
			// if (ep.useEtna(_)) return require("etna-supervisor/lib/server").httpDispatch(_, ep.getEtnaConfig(_), context.request, context.response);
...
var _x3Hacks = {
	...
	protoHack: function(_, context) {
		...
		try {
			...
			var x3Body = reply.body; //JSON.parse(reply.body);
			
			for (var p in _items) {
				x3Body.$properties[p] = _items[p];
			}

			x3Body.$article.$layout.$items.push(_article);
			//x3Body.$properties.AUSPIC.$url = "http://www.hdwallpapersimages.com/wp-content/uploads/2014/01/Winter-Tiger-Wild-Cat-Images.jpg";

			context.reply(_, 200, x3Body, {
	...
	bpartnerHack: function(_, context) {
		...
		try {
			...
			var x3Body = reply.body; //JSON.parse(reply.body);
			var profile = getLinkedInProfile(_, x3Body.BPRNUM);

			transformPayload(_, profile, x3Body);
		...
		if (!resp || !resp.body) {
			...
			if (resp.head.status === 200) {
				try {
					...
					context.reply(_, 500, respCalendar, {
						"content-type": "application/json"
					});
				}
...
var _x3DatasetMap = {
	walk: function(_, context, name, id) {
		...
		if (context.path.indexOf("SEED/BPARTNER") > 0 && context.rawQuery.indexOf("details") > 0) {
			return _x3Hacks.bpartnerHack(_, context);
		} else
		if (context.parameters.search) {
			...
			if (context.path && context.path.indexOf('/$batchGet') > 0) {
				// http://localhost:8124/sdata/x3/erp/SEED/BPARTNER('AO001')?representation=BPARTNER.$details&count=50
			...
			} else if ((/\(/.test(context.request.url.split('/')[5])) && (context.request.headers && context.request.headers.accept && context.request.headers.accept.indexOf("application/json") >= 0)) {
				console.log("ok hack 1");
				var response = context.response.pre.transform(function(_, reader, writer) {
					console.log("TRANSFORM");
					var str = reader.readAll(_).toString('utf8');
					console.log("TRANSFORM: " + str);
					var obj = JSON.parse(str);
					factory.addDiscussionFeed(_, obj);
					console.log("OBJ=" + JSON.stringify(obj, null, 2));
					writer.write(_, new Buffer(JSON.stringify(obj), 'utf8'));
				});
				context.response = Object.create(context.response);
				context.response.write = response.write.bind(response);
				context.response.end = function() {
					response.write(!_);
				};
				console.log("ok hack 2");
	...
	prototypes: function(_, context, name, id) {
		// delegate to x3pool
//<<<<<<< HEAD
		// console.log("BILLY3" + context.url);
	//	if (context.path.indexOf("SEED/$prototypes") > 0 && context.url.indexOf("AUTILIS.$details") > 0) {
// 		if (false && context.path.indexOf("SEED/$prototypes") > 0 && context.url.indexOf("BPARTNER.$details") > 0) {
// // =======
// // 		console.log("BILLY3" + context.url);
// // 		//	if (context.path.indexOf("SEED/$prototypes") > 0 && context.url.indexOf("AUTILIS.$details") > 0) {
// // 		if (context.path.indexOf("SEED/$prototypes") > 0 && context.url.indexOf("BPARTNER.$details") > 0) {
// // >>>>>>> 7294876c6a9916e82b50556fa7ea71e8fd83c984
// 			return _x3Hacks.protoHack(_, context);
// 		} else {
			x3pool.send(_, context);
		//}
	},
...
var _propertyMap = {
	...
	put: function(_, context) {
		...
		if (!uploadHelpers.allowContentType(options.contentType)) return context.reply(_, 415, "content type not allowed: " + options.contentType);

		context.request.setEncoding(null);
		var stream = store.createWritableStream(_, options);
		var reader = context.request;
		if (/html/.test(options.contentType)) reader = reader.transform(htmlTransforms.escaper({
			preserve: true,
		}));
		var buf;
...
var _wcPropertyMap = {
	...
	put: function(_, context) {
		...
		// handling case of multipart/form-data post
		if (boundary) {
			var multipart = ez.transforms.multipart.parser(content.request.headers);
			context.request.transform(multipart).forEach(_, function(_, part) {
				// TODO: test with multiple files
				var reader = part;
				if (/html/.test(part.headers['content-type'])) reader = reader.transform(htmlTransforms.escaper({
					preserve: true,
				}));
			...
			var writer = store.createWritableStream(_, options);
			var reader = context.request;
			if (/html/.test(contentType)) reader = reader.map(function(_, buf) {
			...
			if (take) {
				...
				if (!resource.$isDeleted) {
					if (!localInst) {
						...
						localInst.$syncUuid = resource.$uuid;
					}
						...
						if (error && error.code === "UNIQUE_KEY_VIOLATION" && error.propname in resource) {
							...
							if (testInstance) {
								if (!testInstance.$syncUuid || testInstance.$syncUuid.toUpperCase() < resource.$uuid.toUpperCase()) {
									testInstance.$syncUuid = resource.$uuid; // take "larger" UUID
								}
...
var _linkedMap = {
	...
	"delete": function(_, context) { // delete global UUID of an instance (do not delete instance itself)
		if (context.syncUuid) {
			...
			return context.reply(_, httpHelper.HttpStatus.BadRequest);
		}
	},
...
function _doIt(_, context) {
	try {
		...
		if (sdata !== "sdata" && sdata !== "api") throw new Error("Bad URL: expected 'sdata', got '" + sdata + "'");
