...
function _hackCpwPrototype(_, context) {
	var pp = {
	...
	for (var ii = 0; ii < 30; ii++) {
		pp.$properties.$resources.$item.$properties["date" + ii] = {
			"$title": "" + (ii + 1) + "/03",
			"$type": "application/x-real"
		};
		pp.$properties.$resources.$treeview.$bindings.$data.push("date" + ii);
	}
	context.reply(_, 200, pp);
}

function _hackCpw(_, context) {
	var coll = context.db.db.collection("Transaction", _);
	...
	//
	console.log("Got cursor: ", (new Date()).getTime() - start, "length", datas.length);
	var start = (new Date()).getTime();
	var results = [];
	var crtParent;
	datas.reduce_(_, function(_, prev, crt) {
		//        if (prev) console.log(prev.account, crt.amount);
		if (!prev || prev.p_account != "A" + crt._id.account || prev.transactionType != "T" + crt._id.transactionType) {
			if (prev) results.push(prev);
			// create group line
			if (!prev || prev.p_account != "A" + crt._id.account) {
				crtParent = {};
				crtParent.$uuid = helpers.uuid.generate();
				crtParent.account = "A" + crt._id.account;
				crtParent.open = false;
				results.push(crtParent);
			}
			//
			prev = {};
			prev.$uuid = helpers.uuid.generate();
			//prev.account = "A" + crt._id.account;
			prev.p_account = "A" + crt._id.account;
			prev.transactionType = "T" + crt._id.transactionType;
		}
		var dateCol = "date" + ((new Date(crt._id.date)).getDate() - 1);
		prev[dateCol] = crt.amount;
		crtParent[dateCol] = (crtParent[dateCol] || 0) + crt.amount;
		return prev;
	}, null);
	console.log("Data prepared: ", (new Date()).getTime() - start, "length", results.length);
	context.reply(_, 200, {
		$resources: results
	});
}
...
var _syracuseDatasetMap = {
	...
	prototypes: function(_, context, id) {
		if (id) {
			...
			if (id === "cpw.$query") return _hackCpwPrototype(_, context);
...
var _entityMap = {
	get: function(_, context) {
		...
		if (!context.contract && stubsPath) {
			...
			console.log("entity name", context.entity.name);
			if (context.entity.name === "cpw") return _hackCpw(_, context);
...
function _doIt(_, context) {
	try {
		...
		if (sdata !== "sdata" && sdata !== "api" && sdata != "mobile1") throw new Error("Bad URL: expected 'sdata,api,mobile1', got '" + sdata + "'");
