...
var _operationMap = {
	...
	post: function(_, context) {
		var payload = context.request.readAll(_);
		try {
			if (payload) payload = JSON.parse(payload);
		} catch (e) {
			throw new Error("Bad JSON data");
		}
		return context.executeOperation(_, payload);
	}
};
...
function _doIt(_, context) {
	try {
		...
		if (!/^(sdata|mobile1|api|api1)$/.test(sdata)) throw new Error("Bad URL: expected 'sdata,api,mobile1', got '" + sdata + "'");
