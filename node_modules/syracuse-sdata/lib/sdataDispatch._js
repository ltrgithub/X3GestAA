...
var workingCopy = require("./workingCopy");
...
var _syracuseDatasetMap = {
	walk: function(_, context, name, id) {
		...
		if (id) {
			...
			if(context.entity) {
				context.instance = context.db.fetchInstance(_, context.entity, id);
...
//				if(!context.instance) throw context.notFound(locale.format(module, "instanceNotFound", name, id));
	...
	reset: function(_, context) {
		if (helpers.useInstances) throw Error("sdataDispatch.reset: NYI");
		context.withTransaction(_, function(_) {
			context.model.reset(_, context.transaction);
			// resync the schema to recreate tables
			context.session.schemaSync(context.transaction, _, null);
...
var _propertyMap = {
	get: function(_, context) {
		...
		var instance = context.db.fetchInstance(_, context.entity, context.instanceId);
...
//			console.log("buffer: "+typeof buf+";   "+sys.inspect(new Buffer(buf, "binary")));
		}
		store.close(_);
		//
		context.reply(_, 204);
...
var _entityMap = {
	get: function(_, context) {
		...
		if (!context.contract && helpers.stubsPath) {
			...
			return factory.fetchInstances(_, context);
	},
	post: function(_, context) {
		context.setMeta(true);
...
var _instanceMap = {
	walk: function(_, context, name, id) {
		...
		context.setMeta(false);
		var inst = context.db.fetchInstance(_, context.entity, context.instanceId, true);
	...
	get: function(_, context) {
		if (!context.contract && helpers.stubsPath) {
			...
			factory.fetchInstance(_, context);
		}
	},
	put: function(_, context) {
		context.setMeta(true);
...
var _prototypeMap = {
	get: function(_, context) {
		...
		if ((!context.application || !context.contract) && helpers.stubsPath) {
			...
			context.entity = context.model.getEntity(keys[0]);
			if (!context.entity) throw context.notFound("entity not found: " + keys[0]);
			context.setMeta(true);
...
var _templateMap = {
	get: function(_, context) {
		context.setMeta(true);
		if (helpers.useInstances) context.replyTemplate(_);
		else {
			context.withTransaction(_, function(_) {
...
var _searchMap = {
	...
	"delete": function(_, context) {
		return searchEngine.deleteIndex(_, context);
	}
}
