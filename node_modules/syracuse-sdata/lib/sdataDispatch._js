...
function getLinkedInProfile(_, email) {
	...
	// 	"Mathias.Walz@sage.com": "http://de.linkedin.com/pub/mathias-walz/97/206/354",
	...
	// 	"Richard.Burns@sage.com": "http://www.linkedin.com/pub/richard-burns/29/a4b/303"
	// };
// Mathias
	...
	//return linkedin.getUserByUrlOnline(_, url);
}

function transformPayload(_, linkedInPayload, x3Payload) {
	//console.log(JSON.stringify(x3Payload));
...
function getGeneral(_, linkedInPayload, x3Payload){
	if (linkedInPayload.pictureUrl && linkedInPayload.pictureUrl.length > 0) {
		console.log("PICTURE: " + linkedInPayload.pictureUrl);
...
var _x3Hacks = {
	...
	bcustomerHack: function(_, context) {
		...
		try {
			...
			var x3Body = reply.body; //JSON.parse(reply.body);
			var profile = getLinkedInProfile(_, x3Body.BPC_WEB);
			//var profile = getLinkedInProfile(_, "sophie.rebeyrotte@sage.com");
...
var _x3DatasetMap = {
	walk: function(_, context, name, id) {
		...
		if (context.path.indexOf("SEED/BPCUSTOMER") > 0 && context.rawQuery.indexOf("details") > 0) {
			return _x3Hacks.bcustomerHack(_, context);
		} else
		if (context.parameters.search) {
