...
var _x3Hacks = {
	...
	batchAction: function(_, context, representation, id) {
		...
		try {
			...
			// -3- Get working copy - Not necessary but action call returns working data that are not used by client - see SPRICESEAM
			ts = new Date().getTime();
			reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
				head: {
					"accept": "application/json;vnd.sage=syracuse",
			...
			report.getWrkCpy = new Date().getTime() - ts;
			if (reply.head.statusCode != 200) {
				throw new Error("Error reading working copy\nStatus : " + reply.head.statusCode + "\nUrl : " + url + "\nBody:" + JSON.stringify(reply.body, null, 2));
			}
			respBody = reply.body;
		} catch (e) {
			error = {
