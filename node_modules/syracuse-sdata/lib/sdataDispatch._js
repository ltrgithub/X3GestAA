...
var _x3Hacks = {
	...
	batchAction: function(_, context, representation, id) {
		...
		try {
			...
			report.createWrkCpy = new Date().getTime() - start;
			if (reply.head.statusCode == 200) {
				...
				report.updtWrkCpy = new Date().getTime() - start - report.createWrkCpy;
				if (reply.head.statusCode == 200) {
					...
					report.callMethod = new Date().getTime() - start - report.updtWrkCpy;
					if (reply.head.statusCode == 200) {
						respBody = reply.body;
						// TODO - Return the entire working copy and update the mobile client working copy
					} else {
						error = {
