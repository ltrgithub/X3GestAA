...
function _saveDiscussionFeed(_, context, instance) {
	console.log("Save !!");
	var adminDb = adminHelper.getCollaborationOrm(_).db;
	var feed = {
		resourceId: instance.$uuid,
		//message: context.query.message, //JSON.parse(context.request.readAll(_) || '{ "foo": "bar" }'),
		message: JSON.parse(context.request.readAll(_) || '{ "foo": "bar" }'),
	};
	adminDb.collection('discFeed', _).update({
...
var _instanceMap = {
	walk: function(_, context, name, id) {
		...
		if (name === 'discussionFeed') return _saveDiscussionFeed(_, context, context.instance || factory.fetchInstance(_, context));
	...
	put: function(_, context) {
		//console.log("Property "+context.propertyName);
		//if (context.propertyName === 'discussionFeed') return _saveDiscussionFeed(_, context, context.instance || factory.fetchInstance(_, context));
		console.log("Put");
