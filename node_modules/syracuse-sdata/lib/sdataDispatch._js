...
var _syncDigestMap = {
	get: function(_, context) {
		var info = context.entity._syncInfo;
		if (!info) context.reply(_, 400, "No synchronization possible on this entity");
		context.reply(_, 200, info.makeDigest(_));
	},
	post: function(_, context) {
		...
		try {
			...
			brequest.$resources.forEach_(_, function(_, entry) {
				...
				try {
					...
					switch (method) {
						case "get":
							if (entry.$location.indexOf("?select=") > 0) {
								context.sendUrlOnly = true;
							}
							factory.replyInstance(_, context);
							break;
