...
var _x3DatasetMap = {
	walk: function(_, context, name, id) {
		if (context.parameters.search) {
			...
			} else if (false && _endsWith(context.path, '/$template') && context.method == "get") {
	...
	try {
		...
		var notOK = data.$resources.some_(_, function(_, resource) { // true result only in case of error
			...
			if (endpoint === resource.$endpoint) {
				...
				// source synchronization digest and the tick for that endpoint is at least the value of $tick of 
				...
				// synchronization digest and the tick for that endpoint is at least the value of $tick of the source 
				...
				if (sourceData.$conflictPriority !== localData.$conflictPriority) {
					...
					console.log("Conflict: conflict priorities " + localData.$conflictPriority + " " + sourceData.$conflictPriority);
					...
					if (resourceTime !== stamp) {
						console.log("Conflict: time stamps " + resource.$stamp + " " + stamp);
						take = (resourceTime > stamp);
					} else {
						// compare endpoint URLs 
						console.log("Conflict: endpoint names " + resource.$endpoint + " " + endpoint);
					...
					try {
						...
						if (error && error.code === "UNIQUE_KEY_VIOLATION" && error.propname in resource) {
							...
							if (testInstance) {
								...
								console.log("UUUU1 " + testInstance.$syncUuid);
...
var _syncSourceMap = {
	post: function(_, context) {
		try {
			...
			var tracker = sdataAsync.create(context, _doSyncSource);
...
exports.dispatcher = function(options) {
	...
	return function(_, request, response) {
		if (/^\/sdata\/etna/.test(request.url)) return require("etna/").httpDispatch(_, request, response);
