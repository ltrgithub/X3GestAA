...
var _x3Hacks = {
	...
	batchAction: function(_, context, representation, id) {
		...
		try {
			...
			if (wrkCpyData) {
				_mergeActions(respBody, wrkCpyData);
				var ts = new Date().getTime();
...
function _mergeActions(dest, src) {
	if (src.$actions) {
		dest.$actions = dest.$actions || src.$actions;
		Object.keys(src.$actions).forEach(function(act) {
			var action = src.$actions[act];
			var destact = dest.$actions[act] = dest.$actions[act] || action;
			Object.keys(action).forEach(function(p) {
				destact[p] = destact[p] || action[p];
			});
		});
	}
}
