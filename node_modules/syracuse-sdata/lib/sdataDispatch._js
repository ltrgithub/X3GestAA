...
function _dispatch(_, context, map){
    ...
    if (seg == null) {
        ...
        if (pair.name[0] == '$') {
            ...
            if (map[name]) {
            	if(map.$canExecute && !map.$canExecute(_, context, name))
            		return context.reply(_, httpHelper.HttpStatus.Forbidden, locale.format(module, "forbiddenSegment", pair.name, context.method));
            	else
...
var _applicationMap = {
    walk: function(_, context, name, id){
        ...
        if (!app && !context.application && !helpers.stubsPath) 
            throw context.notFound(locale.format(module, "applicationNotFound", context.applicationName, context.contractName));
...
var _syracuseContractMap = {
    walk: function(_, context, name, id){
        ...
        if (context.contract) {
            ...
            if (!ds) 
                throw context.notFound("dataset not found: " + name);
...
var _x3ContractMap = {
    walk: function(_, context, name, id){
        ...
        if (context.applicationRef) { // TEST to allow stubs
            ...
            if (!ep) 
                throw context.notFound(locale.format(module, "endpointNotFound", context.applicationName, context.contractName, name));
            context.endpoint = ep;
            //
            context.baseUrl = context.walked();
        }
    ...
    walk: function(_, context, name, id){
    	if(context.parameters.search) {
            ...
            if (model) { // TEST to allow stubs
                ...
                if (!context.entity) 
                    throw context.notFound(locale.format(module, "entityNotFound", name));
            }
            if (id) {
	...
	write: function(cb, data) {
		...
		if (this.batchResponse) {
			try { 	
    			...
    			d.push({$severity: "warning",$message:ex.message, $stackTrace:ex.stack + "\n" + this.data})
			}			
		}		
	}
}
var _x3Hacks = {
	...
	template:function(_, context, name, id){
    	var resp  = context.response, respS = 200, respB, respH;
    	try{
    		...
    		if(respS == 200){ 
        		...
        		var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, { 
        				...
        				body: {
        					"$etag":respB.$etag,
        					"$actions":{"$abort":{"$isRequested":true}},
        					"$uuid":respB.$uuid
        				} 
        			}
        		);
        		if(reply.head.statusCode != 200){
        			...
        			d.push({$severity: "warning",$message:"Can't delete working copie",$stackTrace:url + "\n" + JSON.stringify(reply.body)})
        		}
    		} 		
    	}catch(e){
    		respS = 500;
			respB = {$diagnoses:[{$severity: "error", $message: "Error creating working copy", $stackTrace: e.stack}]};
		}
...
var _propertyMap = {
    get: function(_, context){
        ...
        if (!instance) 
            return context.reply(_, 404, "The requested instance was not found");
...
var _wcPropertyMap = {
    get: function(_, context){
        ...
        if (!instance) 
            return context.reply(_, 404, "The requested working copy is not available (session expired)");
        context.replyProperty(_, instance, context.propertyName);
    },
    put: function(_, context){
        var instance = context.httpSession[context.workingCopyId];
...
var _instanceMap = {
    walk: function(_, context, name, id){
        ...
        if (!rel) {
            ...
            if (prop) 
                return _dispatch(_, context, _propertyMap);
...
var _workingCopiesMap = {
    ...
    get: function(_, context){
        ...
        if (!instance && !(entity && entity.$autoRecreateWorkingCopy)) 
            return context.reply(_, 404, "The requested working copy is not available (session expired)");
...
var _serviceMap = {
    walk: function(_, context, name, id){
        ...
        if (!operation) 
            throw context.notFound("service operation not found: " + name);
        var hasInstance = context.instanceId != null;
        
        if (hasInstance != operation.isMethod) 
            throw context.badRequest("isMethod mismatch on operation");
    ...
    get: function(_, context){
        ...
        return context.replyDictionaryPage(_, context.entity.$services, "operation", function(name, operation){
            if (operation.isMethod != hasInstance) 
                return;
            return {
                $url: baseUrl + "/" + name,
                $key: name,
...
var _importMap = {
    post: function(_, context){
        ...
        context.reply(_, 201, _importTools[ct] &&
        _importTools[ct](_, context.db, r, {
            importMode: "insert",
            createSession: true
        }));
    },
    put: function(_, context){
        var r = context.request;
        var ct = httpHelper.parseHeaders(r.headers)["content-type"] || "application/json";
        context.reply(_, 200, _importTools[ct] &&
        _importTools[ct](_, context.db, r, {
            importMode: "update",
            createSession: true
        }));
...
function _logRequest(_, context){
    if (!context.request.headers["x-history-title"]) 
        return;
    //
...
function _doIt(_, context){
    try {
        ...
        if (sdata != "sdata") 
            throw new Error("Bad URL: expected 'sdata', got '" + sdata + "'");
       ...
       if("phone" === context.parameters.device){
       	 context.reply(_, ex.httpStatus ==null ? 500 : ex.httpStatus, {
				$diagnoses: [{
					$severity: "error",
