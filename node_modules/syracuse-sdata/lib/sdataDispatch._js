...
var _applicationMap = {
	walk: function(_, context, name, id) {
		...
		//
		switch ((app && app.protocol(_)) || "syracuse") {
			case "syracuse":
				return _dispatch(_, context, _syracuseContractMap);
			case "x3":
				return _dispatch(_, context, _x3ContractMap);
			case "proxy":
				return _dispatch(_, context, _proxyContractMap);
			default:
				throw new Error("NIY protocol");
		}
	},
	get: function(_, context) {
...
var _proxyContractMap = {
	walk: function(_, context, name, id) {
		...
		var serviceUrl = ep.appServiceUrl(_);
		if (!/\/$/.test(serviceUrl)) serviceUrl += '/';
		serviceUrl += context.path.split('/').slice(5).join('/');
		// TODO: authentication + maybe some header tweaks
		var client = require('syracuse-httpclient/lib/httpClient').httpRequest(_, {
			url: serviceUrl,
			method: context.request.method,
			headers: context.request.headers,
		});
		var resp = context.request.pipe(_, client).response(_);
		context.response.writeHead(resp.statusCode, resp.headers);
		resp.pipe(_, context.response);
	}
};
