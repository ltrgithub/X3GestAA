...
var _x3Hacks = {
	...
	batchAction: function(_, context, representation, id) {
		...
		var actParams = JSON.parse(context.request.readAll(_));
		tracer && tracer("batchAction  - actParams\n" + JSON.stringify(actParams, null, 2));
		var respStatus = 200;
		var respBody, respHeader;
		var wrkCpyData = null;
		try {
			// -1- Create a working copy
			var url = actParams.$workingCopyUrl + '&trackingId=' + helpers.uuid.generate();
			...
			if (reply.head.statusCode == 200) {
				...
				if (reply.head.statusCode == 200) {
					// -2- Call method with parameters given by mobile client - actParams
					var body = {
					...
					if (reply.head.statusCode == 200) {
						// -2- Call action ok - Return response status and body 
						respStatus = 200;
						respBody = reply.body;
						var reply = x3pool.jsonSend(_, context.request.session, context.endpoint, {
							head: {
			...
			tracer && tracer("batchAction - JS ERROR \n" + e.stackTrace);
			respStatus = 500;
			respBody = {
				$diagnoses: [{
					$severity: "error",
			...
			if (wrkCpyData) {
				...
				tracer && tracer("batchAction -DELETE response\n" + JSON.stringify(reply.body, null, 2));
				if (reply.head.statusCode != 200) {
					if (!respBody) {
						respBody = {};
					}
					if (!respBody.$diagnose) {
						respBody.$diagnose = [];
					}
					respBody.$diagnose.push({
						$severity: "warning",
		...
		if (!respHeader) respHeader = {};
		if (!respHeader["content-type"]) {
			respHeader["content-type"] = "application/json";
		}
		context.reply(_, respStatus, respBody, respHeader);
	}
};
