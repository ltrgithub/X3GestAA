...
exports.parseHeaders = function(headers) {
	...
	var result = {
		isJSON : function() {
			return (this["content-type"] === "application/json")
					|| !this["content-type"];
		},
		toNodeCharset : function() {
			// TODO: have a proper list of node supported charsets
			return (this.charset === "utf-8" || this.charset === "utf8") ? "utf8"
					: "binary";
		}
	};
	// content type
	...
	if (contentType[0])
		result["content-type"] = _normalize(contentType[0]);
	contentType.slice(1).forEach(function(param) {
		param = param.split("=");
		if (param[0])
			result[_normalize(param[0])] = _normalize(param[1]);
		else
			return;
	});
	//
...
exports.HttpContext = function(request, response, options) {
	...
	var _urlBegin = (('authorized' in request.connection) ? "https" : "http") + "://" + request.headers.host;
	this.secure = ('authorized' in request.connection);
