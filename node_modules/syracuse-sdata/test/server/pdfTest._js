...
asyncTest("simple", 4, function(_) {

	var res = _load("simple", _)
	ok(res.data != null, "data not null");
	equal(res.data.login, "erbou", "login ok");

	ok(res.proto != null, "proto not null");
	equal(res.proto.$type, "application/json", "proto $type is application/json");

	pdfRenderer.render(_, context, res.data, res.proto);
...
function _inject() {
	...
	aspect.around(PdfDocument, ["roundedRect", "text", "fillColor", "lineWidth", "widthOfString", "currentLineHeight"], function(context, args, result) {
		if (context.pointCut === "before") {
			tracer(context.name + " " + args);
		} else {
			tracer(context.name + " ==> " + result);
		}
	});
}