...
asyncTest(
	...
	25,
	function(_) {
		var body = get(_, cookie, "$prototypes('prototypeTest.$details')",
			200);
		tracer && tracer("Body (1040): " + sys.inspect(body, null, 4));
		// reference
		strictEqual(body.$properties.reference.$type,
			"application/x-reference", "Reference type ok");
		strictEqual(body.$properties.reference.$item.$value, "{$key}",
			"Reference $value ok");
		// child
		strictEqual(body.$properties.child.$type, "application/x-object",
			"Child type ok");
		ok(body.$properties.child.$item.$properties.string != null,
			"Child has properties ok");
		// list
		strictEqual(body.$properties.references.$type,
			"application/x-array", "List type ok");
		strictEqual(body.$properties.references.$item.$type,
			"application/x-reference", "List item type ok");
		strictEqual(body.$properties.references.$item.$item.$value,
			"{$key}", "List item $value ok");
		// children
		strictEqual(body.$properties.children.$type, "application/x-array",
			"Child type ok");
		strictEqual(body.$properties.children.$item.$type,
			"application/json", "Child $item type ok");
		ok(body.$properties.children.$item.$properties.string != null,
			"Child has properties ok");
		// poly reference
		strictEqual(body.$properties.polyRef.$type,
			"application/x-variant", "Poly Reference type ok");
		strictEqual(body.$properties.polyRef.$variants.string.$type,
			"application/x-reference", "Poly Reference type ok");
		strictEqual(body.$properties.polyRef.$variants.string.$item.$value,
			"{$key}", "Poly Reference $value ok");
		// poly child
		strictEqual(body.$properties.polyChild.$type,
			"application/x-variant", "Poly Child type ok");
		strictEqual(body.$properties.polyChild.$variants.string.$type,
			"application/x-object", "Poly Child type ok");
		ok(
			body.$properties.polyChild.$variants.string.$item.$properties.string != null,
			"Poly Child has properties ok");
		// poly list
		strictEqual(body.$properties.polyRefs.$type, "application/x-array",
			"Poly List type ok");
		strictEqual(body.$properties.polyRefs.$item.$type,
			"application/x-variant", "Poly List item type ok");
		strictEqual(body.$properties.polyRefs.$item.$variants.string.$type,
			"application/x-reference", "Poly List item type ok");
		strictEqual(
			body.$properties.polyRefs.$item.$variants.string.$item.$value,
			"{$key}", "Poly List item $value ok");
		// children
		strictEqual(body.$properties.polyChildren.$type,
			"application/x-array", "Poly Children type ok");
		strictEqual(body.$properties.polyChildren.$item.$type,
			"application/x-variant", "Poly Children $item type ok");
		strictEqual(
			body.$properties.polyChildren.$item.$variants.string.$type,
			"application/x-object", "Poly Children $item type ok");
		ok(
			body.$properties.polyChildren.$item.$variants.string.$item.$properties.string != null,
			"Poly Children has properties ok");
