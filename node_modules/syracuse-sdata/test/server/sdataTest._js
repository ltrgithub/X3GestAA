...
asyncTest("concurrency test", 31, function(_) {
	requestCount++;
	...
	strictEqual(body.$actions.$save.$diagnoses[0].severity, "error", "Has an error");
	//
...
asyncTest("where as details key test", 4, function(_) {
	cookie = getCookie(_);
	var body = post(_, cookie, "strings", {
	...
	body = get(_, cookie, "strings(string eq \"where_as_details_test\")?representation=string.$details", 200);
	strictEqual(body.$uuid, testUuid, "Detail fetched ok");
	
	start();
});

asyncTest("sdata filters test", 4, function(_) {
	var body = post(_, cookie, "strings", {
		string: "T1",
		stringTiny: "01234567890123456789",
		stringNormal: "01234567890123456789",
	...
	// in operator
	body = get(_, cookie, "strings?representation=string.$query&where=string in (\"T1\",\"T2\")", 200);
	strictEqual(body.$resources.length, 2, "In operator ok");
	
	start();
});

asyncTest("proxy class and inline reference storage", 6, function(_) {
	...
	var resUrl = body.$url;
	strictEqual(body.proxyClass.$key, "CODE1~DESC1", "proxyClass return key ok");
	...
	strictEqual(body.proxyClass.$key, "CODE1~DESC1", "proxyClass return key ok (from get)");
	strictEqual(body.proxyClass.code, "CODE1", "proxyClass has code ok (from get)");
...
asyncTest("prototypes test", 25, function(_) {
	var body = get(_, cookie, "$prototypes('prototypeTest.$details')", 200);
	...
	// children
	strictEqual(body.$properties.children.$type, "application/x-array", "Child type ok");
	strictEqual(body.$properties.children.$item.$type, "application/json", "Child $item type ok");
