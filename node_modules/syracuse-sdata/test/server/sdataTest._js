...
function postasync(_, cookie, url, data, statusCode) {
	...
	var body = undefined;
	while (!body) {
		console.log("Vor timeout get2");
		setTimeout(~_, 500);
		console.log("Vor get2 " + location + " " + statusCode);
		body = get2(_, cookie, baseUrl + location, undefined, statusCode);
		console.log("BODYx " + sys.format(body) + " ");
	}
...
function get2(_, cookie, url, headers, statusCode) {
	...
	if (response.statusCode !== 202) {
		console.log("RESPPPP " + response.statusCode);
...
asyncTest("Sync protocol", 171, function(_) {
	...
	console.log("xNACHHER " + sys.format(body));
	...
	console.log("VIERTENS " + sys.format(body));
	var key1 = body.$key;
	var uuid1 = body.$uuid;
	...
	console.log("POST-RE5S " + sys.format(body));
	...
	console.log("UIOP " + sys.format(body));
	...
	// conflict handling
	console.log("ZEITEN " + date_n4 + " " + (Date.parse(date_n4)) + " ");
...
asyncTest("prototypes test", 37, function(_) {
	...
	strictEqual(body.$properties.polyChild.$variants.tree.$type, "application/x-object", "Poly Child tree item ok");
	// poly list
