...
program
	...
	.option('-m, --initMenus <initMenus>', 'init menus in the texts server')
	.option('-t, --initTexts <initTexts>', 'init texts in the texts server')
	...
	.option('-z, --test', 'Test')
	.option('-l, --language <language>', 'specify a language for initMenus')
	.parse(process.argv);
...
function run(_: _) {
	...
	} else if (program.initMenus) {
		sync.initMenus(_, program.initMenus, program.product, program.language);
	} else if (program.initTexts) {
		sync.initTexts(_, program.initTexts, program.product);
