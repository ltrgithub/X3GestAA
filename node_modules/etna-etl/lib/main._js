...
var tmpRoot = config.etna.tmpRoot || path.resolve(__dirname, '../../../tmp');
...
function importMetadata(_) {
	var importer = require("etna-etl/lib/importer").newImporter(_, {
		trace: config.etna.trace,
		dir: tmpRoot + "/META",
		mongo: config.etna.SUPERV.mongoStore,
	}).open(_);
	if (!program.test) importer.createTables(_);
	entities = getResource(_, "entities");
	entities.forEach_(_, function(_, entityResource) {
		console.log("entityResource.entity " + entityResource.entity);
...
function git(_) {
	require("etna-etl/lib/nodegit").newGit(config.etna).gitify(_);
}
