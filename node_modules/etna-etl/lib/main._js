...
function runExporter(_) {
	var exporter = require("etna-etl/lib/exporter").newExporter(config.metadata);
...
function runImporter(_) {
	var importer = require("etna-etl/lib/importer").newImporter(config.metadata);
...
if (gitR) {
	var git = require("etna-etl/lib/" + gitR).newGit(config.metadata);
	git.gitify(_);
}
