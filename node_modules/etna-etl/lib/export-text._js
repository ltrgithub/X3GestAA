...
exports.run = function(_, exporter, text) {
	...
	elts.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		console.log("BEGIN: " + i + "/" + elts.length);
		trace && !((i + 1) % 100) && trace((i + 1) + "/" + elts.length + ": exporting " + text.title + ": " + exporter.objVals(_, elt, text.objectTypes));
		...
		switch (text.tableName) {
			case 'ATEXTRA':
				meta = buildExecute(_, 'MODULE,CODACT', 'ATABLE', null, elt, 'CODFIC');
				if (!meta) return;
				res = {
			...
			case 'ATEXTE':
				...
				if (entities[elt.TYPOBJ] && fs.exists("./entities/" + entities[elt.TYPOBJ] + "._js")) {
					...
					//meta = buildExecute(_, mainKey, entity.tableName, null, elt, entity.primaryKey)[0];
				}
				res = {
		...
		console.log("END:   " + i + "/" + elts.length);
	});
console.log("END OF LOOP");
	trace && trace(text.title + ": " + elts.length + " exported in " + Math.round((Date.now() - t0) / 1000) + " seconds");
};
