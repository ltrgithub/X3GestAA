...
export function newSync(_: _, endpoint: any, tracker?: Tracker) {
	...
	function _initTexts(_:_,baseUrl: string, product: string) {
		return _initTextsServer(_,baseUrl, product, "ATEXTE", "SYNC",
			(driver, args) => "select NUMERO_0, LAN_0, LANORI_0, TEXTE_0, COMMENT_0 from ATEXTE  where LAN_0= " + driver.param(0) + " order by LANCHP_0, LANNUM_0");
	}

	function _initMenus(_:_, baseUrl: string, product: string) {
		return _initTextsServer(_,baseUrl, product, "APLSTD", "SYNC_APLSTD",
			(driver, args) => {
				args.push(product ? 2 : 1);
				return "select a.LANCHP_0, a.LANNUM_0, a.LAN_0, a.LANMES_0, a.LANORI_0 from APLSTD a"
					+ " inner join AMENLOC b on a.LANCHP_0 = b.MENLOC_0 and a.LAN_0= " + driver.param(0) + " and b.FLGPRO_0= " + driver.param(1)
					+ " order by a.LANCHP_0, a.LANNUM_0";
			});
	}

	function _initTextsServer(_:_, baseUrl: string, product: string, type: string, post: string, getSql: (driver, args)) {
