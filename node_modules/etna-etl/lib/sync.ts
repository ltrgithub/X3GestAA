...
export function newSync(_: _, endpoint: any, tracker?: Tracker) {
		...
		return _initTextsServer(_, baseUrl, product, "APLSTD", "SYNC_APLSTD",
			(driver:Driver, args:any) => {
				...
				if( !product ||product === "SU") {
					...
					sql += " and b.FLGPRO_0= " + driver.param(1);
				}
			...
			(driver:Driver,context:any,lan:string) => {
				context.chapters = [0,200].reduce((r,menu) => {
					...
					driver.reader(sql, args).toArray().forEach( (record:any) =>  {
						if(r[record.LANNUM_0] !== undefined) {
							r[record.LANNUM_0].LANMES = record.LANMES_0; 
							r[record.LANNUM_0].LANORI = record.LANORI_0; 
						}
					});
					return r;
				},context.chapters);
	...
	function _initTextsServer(_: _, baseUrl: string, product: string, type: string, post: string, getSql: (driver:Driver, args:any) => string, transform:(context:any,record:any) => void, afterTransform?:(driver:Driver,context:any,lan:string) => any[]) {
