...
export function newSync(_: _, endpoint: any, tracker?: Tracker) {
	...
	function exportMetadataToJSONs(_: _, entityType: string, entityNames: string[], options: Options): FileStruct[] {
		...
		var dbUrl = mongoConfig.url || ("mongodb://" + (mongoConfig.host || "localhost") + ":" + (mongoConfig.port || 27017));
		var mongoDb = mongodb.MongoClient.connect(dbUrl).then(_, _).db(mongoConfig.database);
	...
	function _initMenus(_: _, baseUrl: string, product: string, language?:string) {
			...
			(context:any,record:any) => {
				...
				if(record.MENLOCAL === 2) {
					if(record.LANNUM === 0) {
						...
						record.CODE = context.codes[''+record.LANNUM] || (record.LANNUM % 10).toString();
					}
				}
				else if(record.LANNUM === 0) {
					context.codes = {};
					context.chapters[record.LANCHP].LANORI = record.LANORI;
					context.chapters[record.LANCHP].LANMES = record.LANMES;
				}
	...
	function _initTextsServer(_: _, baseUrl: string, product: string, type: string, post: string, getSql: (driver:Driver, args:any) => string, transform:(context:any,record:any) => void, afterTransform?:(driver:Driver,context:any,lan:string) => any[], language?:string) {
	...
	return {
		...
		removeMetadata: function (_: _, entityType: string, entityNames: string[] | string = [], options: Options) {
			...
			var mongoConfig = etnaConfig.mongo || {};
			const dbUrl = mongoConfig.url || ("mongodb://" + (mongoConfig.host || "localhost") + ":" + (mongoConfig.port || 27017));
			// options.exportToMongo or endpoint.useEtna ?
			var db = options.exportToMongo ? mongodb.MongoClient.connect(dbUrl).then(_, _).db(mongoConfig.database) : null;
