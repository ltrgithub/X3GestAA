...
exports.run = function(_, exporter, entity, tracker, entityNames) {
	...
	var childrenCols = Object.keys(entity.children).reduce_(_, function(_, r, childName) {
		...
		r[childName] = exporter.getTableColumns(_, child.tableName, excludedChildColumns).join(',');
		return r;
	}, {});
	...
	elts.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		...
		var data = buildExecute(_, context, mainColumns, entity.tableName, null, elt, null, entity.textLinks)[0];
		...
		if (exporter.folderType == 0)
			...
			modulePart = "CHAPTERS/" + data.CHAPITRE;
		else
			modulePart = "GLOBAL";
