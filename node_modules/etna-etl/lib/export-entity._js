...
exports.run = function(_, exporter, entity, tracker, entityNames, options) {
	...
	elts.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		...
		if (entity.children) {
			...
			Object.keys(entity.children).forEach_(_, function(_, childName) {
				...
				Object.keys(subResults).forEach(function(key) {
					data[key] = data[key] || {};
					data[key][childName] = subResults[key].map(function(result) {
						return jsonHelper.createGroups(child, result);
					});
				});
			});
		}
		...
		Object.keys(data).forEach_(_, function(_, activityCode) {
			...
			dataPart = jsonHelper.createGroups(entity, jsonHelper.sortJsonObject(_, dataPart, entity));
