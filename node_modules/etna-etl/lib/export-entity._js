...
exports.run = function(_, exporter, entity, tracker, entityNames, options) {
	...
	function queryData(_, tableMeta, entityDescriptor, pkValue) {
		...
		if (!data['']) {
			// Make sure that the entity is decribed
			var val = {};
			Object.keys(pkValue).forEach(function(pkColumn) {
				val[pkColumn] = data[Object.keys(data)[0]][0][pkColumn];
			});
			data[''] = [val];
		}
	...
	// Here, pkValues contains all the keys of the entityType taht should be exported
	...
	pkValues.forEach_(_, function(_, pkValue, idx) {
		...
		trace && trace("Processing " + entity.title + " (" + (idx + 1) + " / " + pkValues.length + ") : " + JSON.stringify(pkValue));
	...
	trace && trace(entity.title + ": " + pkValues.length + " resources exported in " + Math.round((Date.now() - t0) / 1000) + " seconds");
