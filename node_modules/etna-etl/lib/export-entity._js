...
exports.run = function(_, exporter, entity, tracker, entityNames, options) {
	...
	pkValues.forEach_(_, function(_, pkValue, idx) {
		...
		if (exporter.productId === productsHelper.knownProducts.supervisor) {
			...
			modulePart = "SUPERV";
		} else if (exporter.moduleNames[mainObjectData.MODULE]) {
			if (options.specialExport) {
				modulePart = mainObjectData.MODULE + "-" + exporter.moduleNames[mainObjectData.MODULE];
			} else {
				modulePart = exporter.moduleNames[mainObjectData.MODULE];
			}
		} else if (mainObjectData.CHAPITRE) {
			var chapter = mainObjectData.CHAPITRE;
