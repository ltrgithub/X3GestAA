...
exports.run = function(_, exporter, entity, tracker, entityNames, options) {
	...
	function queryData(_, tableMeta, entityDescriptor, pkValue, defaultActivityCode) {
		...
		function queryTable(_, tableMeta, keys, entityDescriptor, parentDescriptor, queryOptions) {
			...
			var sqlResults = buildExecute(_, tableMeta, colsToQuery, tableMeta.name, entityDescriptor.orderBy, keys, where, entityDescriptor.textLinks, queryOptions);
		...
		Object.keys(entityDescriptor.children || {}).forEach_(_, function(_, childName) {
			...
			// Reminder : data is indexed by activityCode. Each data[activityCode] is an array on items
	...
	if (options.incrementalDate)
		colsToQuery.push('UPDDATTIM');
	if (options.specialExport)
		colsToQuery.push('MODULE');
	...
	var pkValues = buildExecute(_, null, colsToQuery, entity.tableName, entity.primaryKey).filter(function(s) {
		if (entityNames) {
			if (entityNames.indexOf(s[entity.primaryKey[0]]) == -1) {
		...
		if (options.specialExport) {
			return [2, 3, 4, 5, 6, 7, 8, 9, 12, 15, 20].indexOf(s.MODULE) != -1;
		}
	...
	pkValues.forEach_(_, function(_, pkValue, idx) {
		...
		Object.keys(data).forEach_(_, function(_, activityCode) {
			...
			absoluteFilenames.push(exporter.writeResource(_, {
				...
				name: _formatPkValues(entity.primaryKey, pkValue) + (activityCode ? "." + activityCode : ""),
				data: dataPart
			}));
	...
	if (absoluteFilenames.length)
		_trace(entity.title + ": " + JSON.stringify(pkValues.length) + " resources exported in " + Math.round((Date.now() - t0) / 1000) + " seconds", "info");
