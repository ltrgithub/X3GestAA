...
exports.run = function(_, exporter, entity, tracker, entityNames, options) {
	...
	// First let's list enitites stored in the repo in order to detect ones that aren't present in the X3 database anymore:
	if (!entityNames) var deletedEntities = exporter.list(_, entity);
	...
	var pkValues = buildExecute(_, null, colsToQuery, entity.tableName, entity.primaryKey, null, null,
		...
		if (deletedEntities) {
			const idxKey = deletedEntities.indexOf(_formatPkValues(entity.primaryKey, s));
			if (idxKey >= 0) deletedEntities.splice(idxKey, 1);
		}
	...
	pkValues.forEach_(_, function(_, pkValue, idx) {
		...
		mainObjectData = jsonHelper.sortJsonObject(_, mainObjectData, entity);
	...
	if (absoluteFilenames.length) {
		if (filteredFilesCount)
			_trace(locale.format(module, "exportDoneWithFilteredFiles", entity.title, generatedFilesCount, filteredFilesCount, Math.round((Date.now() - t0) / 1000)), "info");
		else
			_trace(locale.format(module, "exportDone", entity.title, generatedFilesCount, Math.round((Date.now() - t0) / 1000)), "info");
	}

	// Eventually delete obsoletes entities
	if (deletedEntities.length) {
		deletedEntities.forEach_(_, function(_, deletedEntitiy) {
			absoluteFilenames.push(exporter.deleteResource(_, entity, deletedEntitiy));
		});
	}
