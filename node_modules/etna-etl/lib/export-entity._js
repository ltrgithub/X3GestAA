...
exports.run = function(_, exporter, entity, tracker, entityNames) {
	...
	var mainColumns = exporter.getTableColumns(_, entity.tableName, entity.excludedColumns);
	...
	var childrenCols = Object.keys(entity.children).reduce_(_, function(_, r, childName) {
		...
		var excludedChildColumns = child.excludedColumns || entity.excludedColumns;
	...
	elts.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		...
		trace && trace((i + 1) + "/" + elts.length + ": exporting " + entity.title + ": " + exporter.objVals(_, elt));
