...
exports.newExporter = function(config) {
	...
	function writeResource(_, res) {
		var relativeFolder = config.metaFolder;
		res.path.split('/').forEach_(_, function(_, s) {
			relativeFolder += "/" + s;
			mkdir(_, config.solutionPath + "/" + relativeFolder);
		});
		var relativeFilename = relativeFolder + "/" + res.name + ".json";
		fs.writeFile(config.solutionPath + "/" + relativeFilename, JSON.stringify(res.data, null, '\t'), "utf8", _);
	...
	return {
		...
		trace: trace,
		init: function(_) {
			trace && trace("creating output: " + config.solutionPath + "/" + config.metaFolder);
			mkdir(_, config.solutionPath + "/" + config.metaFolder);
		},
		clean: function(_) {
			trace && trace("cleaning output: " + config.solutionPath + "/" + config.metaFolder);
			fs.readdir(config.solutionPath + "/" + config.metaFolder, _).forEach_(_, function(_, name) {
				if (name !== '.git') rm(_, config.solutionPath + "/" + config.metaFolder + "/" + name);
			});
		},
		check: function(_) {
