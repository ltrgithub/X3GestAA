			...
			results.forEach_(_, function(_, result) {
				...
				driver.withConnection(_, function(_, cnx) {
					...
					texts.forEach(function(text) {
						var langCode = text.LANG;
						var atextObj = result[MARK_extraTables].ATEXTRA[text.ZONE];
						if (!atextObj) {
							atextObj = {
								ident1: idents.ident1,
								ident2: idents.ident2,
								texts: {},
							}
							result[MARK_extraTables].ATEXTRA[text.ZONE] = atextObj;
						}
						atextObj.texts[langCode] = text.TEXT;
					});
				});
			});
		}
