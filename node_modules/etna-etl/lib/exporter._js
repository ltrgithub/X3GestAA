	...
	function trim(obj, tableMeta, options) {
		...
		var result = Object.keys(obj).reduce(function(o, k) {
			...
			if (isArray) {
				...
				o[colShortName].push(options.formatValues ? transformValue(v, columnMeta) : v);
			} else {
				...
				o[colShortName] = options.formatValues ? transformValue(v, columnMeta) : v;
			}
			return o;
		}, {});
		/*		
	...
	return {
		...
		getTableMeta: function(_, tableName, excludedColumnNames) {
			...
			return buildExecute(_, null, "CODZONE,DIME,CODTYP", "ATABZON", "CODZONE", {
				...
				if (excludedColumnNames && ~excludedColumnNames.indexOf(keys.CODZONE))
					return;
				var arr = [];
				for (var i = 0; i < keys.DIME; i++) {
					var s = keys.CODZONE + '_' + i;
					if (s === "AUUID_0" && sqlConfig.driver === "oracle") s = "CAST(AUUID_0 as varchar2(32))"; // hack to get around unsupported datatype in driver					
					if (["AX1", "AX2", "AX3"].indexOf(keys.CODTYP) < 0) arr.push(s);
				}
				return arr.join(',');
			}).filter(function(s) {
				return s;
