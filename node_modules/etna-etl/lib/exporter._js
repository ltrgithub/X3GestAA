			...
			results.forEach_(_, function(_, result) {
				...
				driver.withConnection(_, function(_, cnx) {
					...
					if (pkColumns.length > 1) {
						// Composite PK : based on 2 columns
						txtQuery += " AND IDENT2_0 = " + driver.param(2);
						// Force it to string (for instance ATABTAB has a numeric PK ... (NUMTAB))
						idents.ident2 = "" + keys[pkColumns[1]];
						txtParams.push(idents.ident2);
					} else {
						if (options.entityDescriptor && options.entityDescriptor.distinctKey) {
							// Use the first column of the distinctKey to filter the right atextras
							txtQuery += " AND IDENT2_0 = " + driver.param(2);
							// Force it to string (for instance ATABTAB has a numeric PK ... (NUMTAB))
							var dk = options.entityDescriptor.distinctKey;
							if (!Array.isArray(dk)) {
								dk = dk.split(',');
							}
							idents.ident2 = "" + decodeValueFromJsonString(undefined, result[dk[0]]).value;
							txtParams.push(idents.ident2);
						}
					}
