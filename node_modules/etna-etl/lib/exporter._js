...
var locale = require('streamline-locale'),
	...
	decodeValueFromJsonString = require('scm-helper/lib/jsonHelper').decodeValueFromJsonString,
...
exports.newExporter = function(_, config) {
		...
		if (tableMeta && tableMeta.extraTables && tableMeta.extraTables.ATEXTRA) {
			results.forEach_(_, function(_, result) {
				...
				driver.withConnection(_, function(_, cnx) {
					...
					var keysToUse = options.entityDescriptor.primaryKey || options.entityDescriptor.distinctKey;
					if (!Array.isArray(keysToUse))
						keysToUse = keysToUse.split(',');
					// Force it to string (for instance ATABTAB has a numeric PK ... (NUMTAB))
					sqlParams.push("" + decodeValueFromJsonString(undefined, result[keysToUse[0]]).value);
					var txtQuery = "select ZONE_0 ZONE, LANGUE_0 LANG, TEXTE_0 TEXT, IDENT1_0 IDENT1, IDENT2_0 IDENT2 " +
						"from ATEXTRA where CODFIC_0 = " + driver.param(0) + " AND IDENT1_0 = " + driver.param(1);

					if (keysToUse.length > 1) {
						// Force it to string (for instance ATABTAB has a numeric PK ... (NUMTAB))
						sqlParams.push("" + decodeValueFromJsonString(undefined, result[keysToUse[1]]).value);
					} else {
					...
					if (sqlParams.length === 3)
						txtQuery += " AND IDENT2_0 = " + driver.param(2);
					txtQuery += " ORDER BY ZONE, IDENT1, IDENT2, LANG";
					...
					var currentZoneName = undefined;
					texts.forEach(function(text) {
						if ((currentZoneName === text.ZONE) && currentZone && (currentZone.ident1 === text.IDENT1) && (currentZone.ident2 === text.IDENT2)) {
							...
							result[MARK_extraTables].ATEXTRA[text.ZONE].push(currentZone);

						}
						currentZone.texts[text.LANG] = text.TEXT;
					});
					// Now, sort the ATEXTRAs by (ident1, ident2)
					Object.keys(result[MARK_extraTables].ATEXTRA).forEach(function(zoneName) {
						result[MARK_extraTables].ATEXTRA[zoneName].sort(function(i1, i2) {
	...
	return {
		...
		init: function(_, tracker) {
			...
			// Retrieve the productId (0=SUPERV, ...)
			this.productId = productsHelper.getProductIdFromFolderType(
				parseInt(buildExecute(_, null, "VALEUR", "ADOVAL", null, {
					"PARAM": "TYPDOS"
				})[0].VALEUR));
			return true;
		},
