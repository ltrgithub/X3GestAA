...
exports.newExporter = function(_, config) {
	...
	function executeSql(_, sql, args, tableMeta, options) {
		...
		try {
			return driver.withConnection(_, function(_, cnx) {
				return driver.reader(_, cnx, sql, args).map(
					function(_, record) {
						return trim(record, tableMeta, options);
					}).toArray(_);
			});
		} catch (ex) {
	...
	/// Write a JSON file and returs the RELATIVE filename that was written (relative to metaFolder)
	function writeResource(_, res) {
		var relativeFolder = "";
		res.path.split('/').forEach_(_, function(_, s) {
			relativeFolder += "/" + s;
			mkdir(_, path.join(config.solutionPath, config.metaSubFolder, relativeFolder));
		});
		var relativeFilename = relativeFolder + "/" + res.name + ".json";
		var absoluteFilename = path.join(config.solutionPath, config.metaSubFolder, relativeFilename);
		try {
