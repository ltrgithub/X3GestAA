...
function encodeValueToJsonString(value, columnMeta, sqlDriver) {
	if (columnMeta) {
		switch (columnMeta.type) {
			...
			case 8: // date
				...
				if (sqlDriver && sqlDriver.isNullDate(date))
					value = "D|NULL";
				else
					value = "D|" + value;
				break;
			...
			default:
				if (value instanceof String)
					throw new Error("Unhandled type " + columnMeta.type);
				break;
		}
	} else {
...
function decodeValueFromJsonString(key, val) {
	if (/^##\w+##$/.test(key)) {
	...
	if (Array.isArray(val)) {
		...
		val.forEach(function(arrayItem, itemIdx) {
			result.value.push(decodeValueFromJsonString(key + "[" + itemIdx + "]", arrayItem)).value;
		});
...
exports.encodeValueToJsonString = encodeValueToJsonString;
exports.decodeValueFromJsonString = decodeValueFromJsonString;