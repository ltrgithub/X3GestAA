...
exports.newExporter = function(_, config) {
	...
	function writeResource(_, res) {
		...
		try {
			...
			if (fs.exists(absoluteFilename, _)) {
				var prevMeta = JSON.stringify(require(absoluteFilename), null, '\t');
		...
		list(_, entity) {
			...
			return fs.readdir(entityDir, _).reduce_(_, function(_, r, f) {
				...
				if (fs.stat(filename, _).isFile() && f.toLowerCase().endsWith('.json')) {
					r.push(path.basename(filename, '.json').substring(1).toUpperCase());
				}
				return r;
			}, []);
		},
		/*
		deleteResource doesn't actually delete the resource. This is performed by the import feature
		*/
		deleteResource(_, entity, entityKey) {
			const resourcePath = path.join(config.outputFolder, this.product.name, entity.subdir, '_' + entityKey + '.json');
			trace && trace("delete resource " + resourcePath);
			const newResourceName = resourcePath + ".delete";
			fs.rename(resourcePath, newResourceName, _);
