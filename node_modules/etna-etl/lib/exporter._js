...
*/
	...
	function buildExecute(_, select, tableName, orderBy, elt, where, textLinks) {
		...
		var results = executeSql(_, sqlQuery, args);
		console.log(results);
		if (textLinks) {
			results.forEach_(_, function(_, result) {
				textLinks.forEach_(_, function(_, textLink) {
					...
					if (result[textLink]) {
						...
						var textResults = buildExecute(_, "LAN_0, TEXTE_0, COMMENT_0", "ATEXTE", null, {
							"NUMERO": result[textLink]
						});
						textResults.forEach(function(langItem) {
							result["##texts##"][textLink][langItem["LAN"]] = langItem["TEXTE"];
							if (langItem["COMMENT"]) {
								if (!result["##comments##"])
									result["##comments##"] = {};
								if (!result["##comments##"][textLink])
									result["##comments##"][textLink] = {};
								result["##comments##"][textLink][langItem["LAN"]] = langItem["COMMENT"];
							}
						});
					} else {
						console.log("**************************");
						...
						// for instance, this information will be used wehn building patches from json files.
						result["##texts##"][textLink] = {};
					}
				});
			});
	...
	return {
		...
		init: function(_, tracker) {
			...
			this.folderType = parseInt(buildExecute(_, "VALEUR", "ADOVAL", null, {
				"PARAM": "TYPDOS"
			})[0].VALEUR);
		},
