...
exports.exportMetadata = function(_, config, tracker) {
	...
	var entityDescriptors = getAllEntityDescriptors(_);
	entityDescriptors.forEach_(_, function(_, entityDescriptor) {
		if (tracker && tracker.abortRequested) return;
		if (tracker) tracker.phase = "exporting " + entityDescriptor.title;
		if (entityDescriptor.title === "contexts") {
			entityExporter.run(_, exporter, entityDescriptor, tracker);
		}
	});
