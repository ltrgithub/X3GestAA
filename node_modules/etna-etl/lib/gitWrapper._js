...
exports.git = function(globalOptions) {
	globalOptions = globalOptions || {};
	var trace = globalOptions.trace;
	...
	return {
		...
		/// ``` javascript
		/// { 
		///		out: xxxx : the branch name (when no error)
		///		err: xxxx (the stderr of the git command)
		///		exitCode: nnn  (the exitCode of the git command)
		/// }
		/// ```
		getBranch: function(_) {
			return launchGitCommand(_, "symbolic-ref", ["--short", "HEAD"]);
		},
		...
		///		diffs : [
		///			{
		///				filename : xxx (ABSOLUTE filename)
		///				touch : A(dd) | D(elete) | (M)odified | ... (refer to git doc for more details)
		/// 		},
		///			...
		///		]
		/// }
		...
		diffStat: function(_, fromSha1, toSha1) {
			function parseDiffStatsResult(gitResult, withStatus) {
				...
				allText.split('\n').forEach(function(item) {
					...
					if (!withStatus) {
						// Consider all the files as new (A stands for added)
						item = "A\t" + item;
					}
					var result = reg.exec(item);
					if (result) {
						files.diffs.push({
							filename: path.join(globalOptions.folder, result[2]),
							touch: result[1]
						});
					}
				});
			...
			if (fromSha1) {
				// List only the files that were modified in the range fromSha1..toSha1
				toSha1 = toSha1 || "HEAD";
		...
		listModifiedFiles: function(_, fromSha1, toSha1) {
			...
			if (fromSha1) {
				...
				result = this.diffStat(_, fromSha1, toSha1 || this.getHead(_));
			} else {
				// Retrieve all versionned files
				result = this.diffStat(_);
			}
