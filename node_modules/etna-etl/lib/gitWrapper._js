...
exports.git = function(options) {
	...
	function parseDiffStatsResult(gitResult) {
		var result = {
			diffs: [],
		};
		...
		allText.split('\n').forEach(function(item) {
			...
			if (reg) {
				var filename = reg[1];
				var diffType = reg[2];
				var added = /\+/.test(diffType);
				var removed = /\-/.test(diffType);
				var logType = added ? (removed ? 'update' : 'add') : 'remove';
				result.diffs.push({
					filename: filename,
					touch: logType
				});
	...
	return {
		...
		diffStat: function(_, fromSha1, toSha1) {
			toSha1 = toSha1 || "HEAD";
			var gitResult = launchGitCommand(_, "diff", ['--stat', fromSha1 + '..' + toSha1]);
			return parseDiffStatsResult(gitResult);
		},
