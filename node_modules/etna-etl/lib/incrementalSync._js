...
exports.newIncrementalSync = function(config, tracker) {
	...
	function syncType(_, itemHandler, options) {
		_trace("Synchronize Mongo collection '" + itemHandler.mongo.colName + "'", "info");
		var t0 = Date.now();
		...
		if (fullSync) {
			...
			_trace("Full synchronization : the mongoDb collection has been dropped.", "info");
		} else {
			...
			_trace("Incremental synchronization : will only process changes from " + lastSyncDate, "info");
			lastSyncDateFilters = [{
		...
		_trace("Processed " + count + " records in " + Math.round((Date.now() - t0) / 1000) + " seconds", "info");
		...
		if (options.skipIndexes) {
			_trace("Creation of indexes skipped by options.", "info");
		} else {
			...
			indexesToCreate.forEach_(_, function(_, index) {
				...
				_trace("Index " + JSON.stringify(idxKeys) + " created (unique = " + index.unique + ")", "info");
			});
		}
