...
exports.newIncrementalSync = function(config, tracker) {
	...
	function syncType(_, itemHandler, options) {
		...
		if (fullSync) {
			...
			lastSyncDateFilters = [{
				...
				operator: '>',
				value: superv.sqlDriver.toSqlDate(lastSyncDate)
			}];
		}
		var count = 0;
		var tableDef = superv.sqlDriver.getTableDef(_, itemHandler.sql.tableName);
		...
		tableReader.forEach(_, function(_, row) {
			count++;
			var filter = {};
			itemHandler.sql.distinctKeys.forEach(function(key) {
				filter[key] = row[key];
			});
			if ((count % 100) == 0)
		...
		itemHandler.sql.distinctKeys.forEach(function(key) {
			// Remove the trailing '_x'
			key = key.substring(0, key.lastIndexOf('_'));
			distinctIndex.keys.push(key);
		});
	...
	return {
		sync: function(_, itemType, options) {
			var itemHandler = require("./sync/" + itemType).text;
			return syncType(_, itemHandler, options);
		},
	};
