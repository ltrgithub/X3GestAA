...
function readAndMergeJsonFiles(_, paths, entityDescriptor, options) {
	options = options || {};

	function mergeObjects(object1, object2, entityDescriptor) {
		function indexChildren(entityDescriptor, children) {
			var result = {};
			var pk = entityDescriptor.primaryKey || entityDescriptor.patchPK || entityDescriptor.orderBy;
		...
		Object.keys(object1).forEach(function(key) {
			...
			if (!object2.hasOwnProperty(key)) {
				// The value only exists in object1
				result[key] = val1;
				return;
			}
			...
			delete object2[key];
			if (val1 === val2) {
				// That's easy : the 2 objects have the same value
				result[key] = val1;
				return;
			}
			if (entityDescriptor.children && entityDescriptor.children.hasOwnProperty(key)) {
				var childEntity = entityDescriptor.children[key];
				// This is a list of sub-objects
				// We have to index all the children by their PK and merge the good pairs
				var children1 = indexChildren(childEntity, val1);
				var children2 = indexChildren(childEntity, val2);
				var children = mergeObjects(children1, children2, childEntity);
				// Now, we can rebuild an array that will contain all the sub-objects from object1 and object2
				// Note : the sub-objects are sorted by their PK (Object.keys(..).sort())
				result[key] = Object.keys(children).sort().map(function(key) {
					return children[key];
				});
				return;
			}
			throw new Error("Could not resolve conflicts on key " + key);
		});
		// Here, object2 only contains the values that were present in object2 but not in object2
	...
	if (!Array.isArray(paths))
		paths = [paths];
	var data;
	paths.forEach_(_, function(_, path, index) {
		console.log("Reading " + path);
		var dataPart = JSON.parse(fs.readFile(path, 'utf8', _));
		if (options.cleanData)
			dataPart = cleanData(dataPart);
		if (0 === index) {
			// Main file
			data = dataPart;
		} else {
			// We are processing a secondary file, the dataPart must be merged with the dataPart of the main file
			data = mergeObjects(data, dataPart, entityDescriptor);
		}
	});
	return data;
}
...
function cleanData(data) {
	if ("string" === typeof(data)) {
		...
		if (!parts)
			throw new Error("Wrong format. Value = " + data);
		switch (parts[1]) {
			case 'D':
			case 'T':
	...
	return {
		...
		fillTables: function(_, entity, tracker) {
			...
			groups.forEach_(_, function(_, group, i) {
				...
				var data = readAndMergeJsonFiles(_, allFiles, entity, {
					cleanData: true
				});
				var filter = {};
...
exports.readAndMergeJsonFiles = readAndMergeJsonFiles;