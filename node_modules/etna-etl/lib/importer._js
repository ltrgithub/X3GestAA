...
exports.newImporter = function(_, config) {
	...
	var jsondir = config.rootFolder + "/" + config.metaFolder;
	...
	var dbUrl = "mongodb://" + (mgConf.host || "localhost") + ":" + (mgConf.port || 27017) + "/" + (mgConf.database || "x3meta");
				...
				fs.readdir(moduleDir, _).forEach_(_, function(_, type) {
					...
					if (fs.stat(typeDir, _).isDirectory()) {
						...
						entries.filter(function(name) {
							var fn = config.metaFolder + "/" + module + "/" + type + "/" + name;
							if (!(/\.json$/).test(name))
								return false;
							if (config.filenames) {
						...
						entries.filter(function(name) {
							...
							fs.readdir(typeDir + "/" + lan, _).filter(function(name) {
								...
								var relativeFilename = config.metaFolder + "/" + module + "/" + type + "/" + lan + "/" + name;
								...
								dest.push(config.rootFolder + "/" + relativeFilename);
							});
						});
					}
				});
	...
	return {
		...
		createTables: function(_) {
			...
			var t0 = Date.now();
			var tableNames;
			tableNames = db.collectionNames(_).map(function(o) {
...
*/
		...
		/*
		persistMetadataInMongo: function(_, collectionName, meta) {
			var coln = db.collection(collectionName, _);
			var filter = {};
			filter[meta.mainKey] = meta.mainKeyValue;
			var existingMeta = coln.find(filter, _).toArray(_)[0];
			if (existingMeta)
				coln.update(filter, meta, _);
			else
				coln.insert(meta, _);
		},
*/
		fillTables: function(_, entity, tracker) {
			...
			names.forEach_(_, function(_, name, i) {
				...
				trace && trace(entity.title + ": " + (i + 1) + "/" + names.length + " importing " + name);
				var data = readFile(_, files[entity.subdir][name]);
				var filter = {};
				filter[entity.primaryKey] = data[entity.primaryKey];
				var existingData = coln.find(filter, _).toArray(_);
				if (existingData && existingData.length > 0 && existingData[0]) {
					// trace && trace("UPDATE " + JSON.stringify(filter));
					coln.update(filter, data, _);
				} else {
					// trace && trace("INSERT " + JSON.stringify(filter));
					coln.insert(data, _);
				}
			});
			if (tracker && tracker.abortRequested) return;
