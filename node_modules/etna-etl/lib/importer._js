				...
				fs.readdir(moduleDir, _).forEach_(_, function(_, type) {
					...
					if (fs.stat(typeDir, _).isDirectory()) {
						...
						entries.filter(function(name) {
							var fn = "/" + module + "/" + type + "/" + name;
							if (!(/\.json$/).test(name))
								return false;
							if (config.relativeFilenames) {
								// The file must belong to the provided list of files to process
								if (config.relativeFilenames.indexOf(fn) == -1)
									return false;
							}
							return true;
						...
						entries.filter(function(name) {
							...
							fs.readdir(typeDir + "/" + lan, _).filter(function(name) {
								...
								var relativeFilename = "/" + module + "/" + type + "/" + lan + "/" + name;
								if (config.relativeFilenames) {
									// The file must belong to the provided list of files to process
									if (config.relativeFilenames.indexOf(relativeFilename) == -1)
										return;
								}
								dest.push(config.metaFolder + "/" + relativeFilename);
							});
						});
					}
				});
