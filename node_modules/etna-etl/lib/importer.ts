...
export function newImporter(_: _, resourceManager : ResourceManager, config: ExporterConfig) {
	...
	var dbUrl = mgConf.url || ("mongodb://" + (mgConf.connectionString || ((mgConf.host || "localhost") + ":" + (mgConf.port || 27017))));

	trace && trace(locale.format(module, "importToMongoWithUrl", dbUrl));
	var db = mongodb.MongoClient.connect(dbUrl, mgConf.options as mongodb.MongoClientOptions || {}).then(_, _).db(mgConf.database);
