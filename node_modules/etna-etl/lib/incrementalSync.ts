...
export function newIncrementalSync(config: ExporterConfig, tracker: Tracker) {
	...
	function syncType(_: _, itemHandler: ItemHandler, options: SyncOptions) {
		...
		var t0 = Date.now();
		var superv = supervisor.create(config);
		...
		var count = 0;
		var tableDef = superv.sqlDriver.getTableDef(superv.folderName, itemHandler.sql.tableName);
		var tableReader = superv.sqlDriver.createTableReader<any>(tableDef, lastSyncDateFilters);
		// if (lastSyncDateFilters != null)
		// 	tableReader = tableReader.filter(lastSyncDateFilters);

		tableReader.forEach(row => {
			count++;
			var filter = {} as any;
