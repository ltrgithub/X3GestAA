...
var fs = require('streamline-fs'),
	...
	resourceManager = require('scm-helper/lib/resourceManager');
...
exports.newSync = function(etnaConfig, tracker) {
	...
	var solutionPath = nodeLocalConfig.etl.metaRootFolder || etnaConfig.solutionPath;
	solutionPath = solutionPath || path.resolve(__dirname, '../../../tmp');

	etnaConfig.metaSubFolder = path.join(etnaConfig.folderName, nodeLocalConfig.etl.metaSubFolder);
	...
	function extractMetadata(_, entityType, entityName) {
		...
		var entityResource = resourceManager.getEntityDescriptor(_, entityType);
	...
	function exportMetadataToJSONs(_, entityType, entityNames, options) {
		...
		var exporter = require("etna-etl/lib/exporter").newExporter(_, {
			...
			metaSubFolder: etnaConfig.metaSubFolder,
			force: true, // for now!
			parallel: 1,
		});
		...
		if (options.incrementalDate)
			_trace("Incremental export from " + options.incrementalDate, "info");
		else
			_trace("Full export", "info");
		...
		var entityExporter = require("etna-etl/lib/export-entity");
		if (entityType)
			entities = [resourceManager.getEntityDescriptor(_, entityType)];
		else
			entities = resourceManager.getAllEntityDescriptors(_);
		...
		entities.forEach_(_, function(_, entityResource) {
			var generatedFilenames = entityExporter.run(_, exporter, entityResource, tracker, entityNames, options);
	...
	function getGitWrapper() {
		if (!gitWrapperInstance) {
			_trace("git wrapper initialized, local repo path = " + etnaConfig.solutionPath);
			var options = {
	...
	function importMetadataToMongo(_, absoluteFilenamesToimport, entityType, fullSync) {
		...
		if (tracker) {
			tracker.phase = "Import type '" + entityType + "' to mongo db ...";
			tracker.phaseDetail = "Import type '" + entityType + "' to mongo db ...";
		}

		var t0 = Date.now();
		var importer = require("etna-etl/lib/importer").newImporter(_, {
			trace: etnaConfig.trace,
			metaFolder: path.join(solutionPath, etnaConfig.metaSubFolder),
		...
		if (entityType)
			...
			entities = resourceManager.getAllEntityDescriptors(_);
		entities.forEach_(_, function(_, entityResource) {
			importer.fillTables(_, entityResource, tracker, fullSync);
		});

		// _trace(absoluteFilenamesToimport.length + " JSON files imported in " + Math.round((Date.now() - t0) / 1000) + " seconds", "info");
	}
	...
	return {
		...
		exportMetadataAndPush: function(_, entityType, entityNames, options) {
			if (tracker) {
				tracker.phase = "Export metadata to JSON files";
				tracker.phaseDetail = "Export metadata to JSON files";
			}
			...
			process.chdir(etnaConfig.solutionPath);
			var t0 = Date.now();
			var absoluteFilenames = exportMetadataToJSONs(_, entityType, entityNames, options);
			_trace(absoluteFilenames.length + " JSON files generated in " + Math.round((Date.now() - t0) / 1000) + " seconds", "info");
			_trace("Generated files = " + absoluteFilenames);
			...
			importMetadataToMongo(_, absoluteFilenames, entityType);
