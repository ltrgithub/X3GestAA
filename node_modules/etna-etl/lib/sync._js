...
exports.extractMetadataFromX3 = function(_, filters, options) {
	...
	try {
		...
		config.solutionPath = nodeLocalConfig.etl.solutionPath || config.solutionPath;
		config.metaSubFolder = nodeLocalConfig.etl.metaSubFolder;
...
exports.newSync = function(config, tracker) {
	...
	function exportMetadataToJSONs(_, entityType, entityNames) {
		...
		var exporter = require("etna-etl/lib/exporter").newExporter(_, {
			...
			metaSubFolder: config.metaSubFolder,
			force: true, // for now!
			parallel: 1,
		});
		...
		entities.forEach_(_, function(_, entityResource) {
			...
			relativeFilenames = relativeFilenames.concat(generatedFilenames);
		});
	...
	function importMetadataToMongo(_, relativeFilenamesToimport, entityType) {
	...
	return {
		...
		exportMetadataAndPush: function(_, entityType, entityNames, options) {
			...
			var relativeFilenames = exportMetadataToJSONs(_, entityType, entityNames);
			trace("Generated files = " + JSON.stringify(relativeFilenames));
			...
			var absoluteFilenames = relativeFilenames.map(function(filename) {
				return path.join(config.metaSubFolder, filename);
			});
		...
		importTest: function(_) {
			require("etna-etl/lib/sqldump").importTest(_, config, tracker);
		},
		...
		syncText: function(_, textType, options) {
			var incSync = require("./incrementalSync").newIncrementalSync(config, tracker);
