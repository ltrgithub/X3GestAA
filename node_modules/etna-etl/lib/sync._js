...
exports.newSync = function(config, tracker) {
	...
	console.log(JSON.stringify(config.sql));
	if (!nodeLocalConfig.etl || !nodeLocalConfig.etl.metaSubFolder)
		throw new Error("Missing etl.metaSubFolder in nodelocal.js.");
	...
	function getGitWrapper() {
		if (!gitWrapperInstance) {
			...
			if (config.git)
				options.trace = config.git.trace;
	...
	function importMetadataToMongo(_, filenamesToimport, entityType) {
	...
	return {
		...
		exportMetadataAndPush: function(_, entityType, entityNames, options) {
			...
			console.log("Generated files = " + JSON.stringify(filenames));
			...
			importMetadataToMongo(_, filenames, entityType);
