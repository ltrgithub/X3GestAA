...
exports.newSync = function(_, endpoint, tracker) {
	...
	function exportMetadataToJSONs(_, entityType, entityNames, options) {
		...
		var exporter = require('./exporter').newExporter(_, {
...
*/
	...
	return {
		...
		exportData: function(_, s3Cfg) {
			try {
				require('./sqldump').exportAll(_, etnaConfig, tracker, s3Cfg);
			} catch (err) {
		...
		importData: function(_) {
			try {
				require('./sqldump').importAll(_, etnaConfig, tracker);
			} catch (err) {
				console.error(err.message);
				throw new Error(err.message);
			}
...
*/
		...
		importTest: function(_) {
			require('./sqldump').importTest(_, etnaConfig, tracker);
		},

		importFilesFromS3: function(_, s3Cfg) {
			require('./sqldump').importFilesFromS3(_, etnaConfig, tracker, s3Cfg);
		},
*/
		syncText: _syncText,
