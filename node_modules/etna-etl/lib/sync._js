...
exports.newSync = function(config, tracker) {
	...
	function exportMetadataToJSONs(_, entityNames, entityType) {
		var filenames = [];
		if (tracker)
		...
		entities.forEach_(_, function(_, entityResource) {
			var f = entityExporter.run(_, exporter, entityResource.entity, tracker, entityNames);
			// Note : f is an array 
...
*/
	...
	function executeGitCommand(_, title, cmd) {
		...
		child.stderr.on("data", function(txt) {
			result.stderr += txt;
		});
		...
		// We have to use an intermediate function to handle both 'close' and 'error' events
		// and forward the params of these 2 params to our single '_' callback
		function waitOnChild(cb) {
		...
		_log("'" + title + "' done, exitCode = " + result.exitCode);
		return result;
	}
	...
	function getGitHead(_) {
		...
		executeGitCommand(_, "git log -1", function(git) {
			var child = git.spawn("log", [-1, '--oneline', '--no-abbrev-commit']);
			child.stdout.on("data", function(txt) {
				text += txt;
			});
			return child;
	...
	return {
		...
		copyMetaFromSqlToMongo: function(_, entityType, entityName) {
			...
			var filenames = exportMetadataToJSONs(_, entityName, entityType);
			return;
		...
		exportMetadataAndPush: function(_, entityType, entityNames, options) {
			...
			if (!entityType && !entityNames)
				...
				if (entityNames)
					commitMessage += ", name = [" + entityType + "]";
			}

			if (!options.skipGit)
				gitPull(_);

			var filenames = exportMetadataToJSONs(_, entityNames, entityType);
