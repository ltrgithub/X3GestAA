...
exports.newSync = function(etnaConfig, tracker) {
	...
	function importMetadataToMongo(_, absoluteFilenamesToimport, entityType, fullSync, options) {
		...
		entities.forEach_(_, function(_, entityResource) {
			importedFilesCount += importer.fillTables(_, entityResource.entity, tracker, fullSync);
		});

		_trace(importedFilesCount + " JSON files imported in " + Math.round((Date.now() - t0) / 1000) + " seconds", "info");
		if (!options.skipTexts) {
			_syncText(_, "aplstd", options);
			_syncText(_, "atexte", options);
		}
