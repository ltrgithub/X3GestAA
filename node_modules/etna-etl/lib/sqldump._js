...
exports.exportAll = function(_, config, tracker, s3Cfg) {
	...
	Object.keys(superv).forEach(function(k) {
		console.log(" - " + k + " : " + superv[k]);
	});
	...
	if (fs.exists(folderFilename, _)) {
		var reader = file.text.reader(folderFilename).transform(jsonTrans.parser());

		folderConfig = reader.read(_);
		...
		superv.sqlDriver.withConnection(_, function(_, cnx) {
			var dosRef = superv.sqlDriver.reader(_, cnx, "select DOSREF_0 from X3.ADOSSIER where DOSSIER_0=" + superv.sqlDriver.param(0), [superv.folderName]).toArray(_)[0];
			console.log(dosRef.DOSREF_0 + " / " + superv.folderName);
			folderConfig.isRoot = (dosRef && (dosRef.DOSREF_0 == superv.folderName));
		});
		folderConfig.tables = {};
	}
	...
	superv.sqlDriver.readTables(_, superv.folderName).forEach_(_, function(_, tableDef, i) {
		try {
			...
			if ("ADOSSIER" === tableDef.tableName) {
				// We skip the ADOSSIER table because it contains the hierarchy of folder. This hierarchy will be re-created*
			...
			var previousTableExport = folderConfig.tables[tableDef.tableName];
			...
			// }
			folderConfig.tables[tableDef.tableName] = {
			...
			// Write the folder configuration file so that next export will not process again the same tables
			var writer = file.text.writer(folderFilename);
			var reader = ez.devices.array.reader([folderConfig]);
