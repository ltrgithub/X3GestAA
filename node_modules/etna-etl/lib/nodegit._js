...
*/
	author: {
		name: "Stephane Deniaud",
		email: "stephane.deniaud@sage.com"
	},
	committer: {
		name: "Stephane Deniaud",
		email: "stephane.deniaud@sage.com"
...
exports.newGit = function(config) {
	...
	var path = pathJoin(config.solutionPath, '.git');
	...
	return {
		gitify: function(_, commit) {
			...
			var repo, parent, init = false;
			if (!fs.exists(path, _)) {
				repo = git.Repo.init(config.solutionPath, false, _);
			...
			if (init) {
				...
				diffList.patches().forEach_(_, function(_, patch) {
					...
					if (patch.newFile().path() !== patch.oldFile().path()) index.addByPath(patch.oldFile().path(), _);
				});
				if (diffList.size() === 0) return;

				//get latest commit (will be the parent commit)
				parent = [repo.getCommit(git.Reference.oidForName(repo, 'HEAD', _), _)];
			}
			index.write(_);
