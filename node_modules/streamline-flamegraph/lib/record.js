...
function Writer(options) {
	...
	setImmediate(function() {
		...
		output = options.output || ez.devices.file.binary.writer(process.cwd() + '/perf-recorded.gz');
...
exports.create = function(options) {
	...
	glob.emitter.on('resume', function(f) {
		...
		if (f.yielded && f.yielded < cpu) {
			writer.write(perfStack(f, cpu - f.yielded, options, true));
		}
	});
	...
	glob.emitter.on('enter', function(f) {
		...
		if (cpu < now) {
			if (f.prev) writer.write(perfStack(f.prev, now - cpu, options));
			cpu = now;
		}
	});
	glob.emitter.on('exit', function(f) {
		...
		if (cpu < now) {
			writer.write(perfStack(f, now - cpu, options));
			cpu = now;
		}
	});
