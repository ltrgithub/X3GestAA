...
function locationUrl(file, line, options) {
	var values = {
		fullpath: file,
		relpath: fsp.relative(options.sourceRoot, file.replace(/\\/g, '/')),
		line: line,
	};
...
exports.convert = function(_, streams, options) {
	...
	function genGraph(_, input, output) {
		var p = /^win/.test(process.platform) ?
				fsp.join(__dirname, '../bin/win-flamegraph.cmd') :
				fsp.join(__dirname, '../deps/flamegraph.pl');
		var child = require('child_process').spawn(p, [ //
		"--nameattr", nameattr, //
		"--palette", palette, //
		"--cp"]);
		flows.collect(_, [
