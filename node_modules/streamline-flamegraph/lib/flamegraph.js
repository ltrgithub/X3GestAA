...
exports.makesvg = function(data, options) {
	...
	var countname = "samples";      // what are the counts in the data?
	var colors = "hot";             // color theme
...
if (require.main === module) {
	...
	function load(filename) {
		...
		titletext = "Flame Graph "+new Date();  
		return samples0;
	}
	...
	if (port) {
		...
		http.createServer(function (req, res) {
		  ...
		  if (req.url.substr(0,7) === "/reload") {
			  ...
			  if (restr) {
				  var include = restr[2].replace("S", "^");
				  include = new RegExp(include+";");
				  marklevel = +restr[1];
			  }		  
		  }
		  res.end(exports.makesvg(samples, {palette: palette, frames: data.frames, frameformat: "{2}({0}:{1})", codes: data.codes,hash: 2, mode: "drill", include: include, exclude: exclude, marklevel: marklevel, titletext: titletext}));
		}).listen(+port, '127.0.0.1');
		console.log('Server running at http://127.0.0.1:'+port);
	} else {
		console.log(exports.makesvg(samples, {palette: palette, frames: data.frames, frameformat: "{2}({0}:{1})", codes: data.codes,hash: 2,
