...
exports.newGenerator = function(_, config) {
	...
	function _generatePatches(_, filesList, options) {
		...
		cleanFolder(_, config.patchSubFolder);
	...
	return {
		...
		synchronize: function(_, options) {
			...
			if (config.trace) {
				var currentBranchResult = getGitWrapper().command(_, "symbolic-ref", ["--short", "HEAD"]);
