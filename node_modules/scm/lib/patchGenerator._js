...
exports.newGenerator = function(_, config) {
	...
	function processElement(_, writer, elt, eltConfig) {
		...
		writer.write(_, "3,\"" + eltConfig.abbrev + "\"," + parseJSONValue(pk, elt[pk]).value + "\n");
		...
		if (elt["##texts##"]) {
			...
			Object.keys(texts).forEach_(_, function(_, columnName) {
				...
				console.log(columnName + " / " + textIds);
	...
	function processATableElement(_, writer, entityExportConfig, elt) {
		...
		if (elt["##texts##"]) {
			...
			title = parseJSONValue(entityExportConfig.titleProperty, elt[entityExportConfig.titleProperty]).value;
		}
