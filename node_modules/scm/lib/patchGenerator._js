...
exports.newGenerator = function(_, config) {
	...
	function _generatePatches(_, jsonFilesList, options) {
		...
		jsonFilesList.forEach_(_, function(_, filename) {
			...
			var elt;
			try {
				elt = JSON.parse(fs.readFile(filename, 'utf8', _));
			} catch (err) {
	...
	function _setLastSyncdSha1(_, sha1) {
		try {
			superv.sqlDriver.withConnection(_, function(_, cnx) {
				superv.sqlDriver.execute(_, cnx, "delete from " + config.settingsTable + " where ID = " + superv.sqlDriver.param(0), ["SYNCD_GIT_HEAD"]);
				var valueToWrite = JSON.stringify({
					sha1: sha1,
					date: new Date().toISOString()
				});
				superv.sqlDriver.execute(_, cnx, "insert into " + config.settingsTable + " (ID, VALUE) values (" + superv.sqlDriver.param(0) + "," + superv.sqlDriver.param(1) + ")", ["SYNCD_GIT_HEAD", valueToWrite]);
			});
		} catch (err) {
	...
	return {
		...
		getLastSyncdSha1: function(_) {
			...
			try {
				superv.sqlDriver.withConnection(_, function(_, cnx) {
					var row = superv.sqlDriver.reader(_, cnx, "select VALUE from " + config.settingsTable + " where ID = " + superv.sqlDriver.param(0), ["SYNCD_GIT_HEAD"]).toArray(_)[0];
					if (!row) {
						return null;
					}
		...
		resolveConflitcs: function(_, entity, jsonFile) {
			...
			var result = findDifferences(_, entity, "", "", prevObj, currentObj, {
				skipMatching: true,
				skipSpecials: true
			});
