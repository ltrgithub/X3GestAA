...
function registerTest(jsonFile, refPatchFile) {
	unitTests.push({
		json: jsonFile,
		refPatch: refPatchFile
	});
}

registerTest("SUPERV/CONTENTTYPES/ATYPE0.json", "ATYP_ATYPE0.dat");

registerTest("SUPERV/ACTIVITYCODES/ABI.json", "ACV_ABI.dat");

// tables
registerTest("SUPERV/TABLES/ABANK.json", "ATB_ABANK.dat");

registerTest("COMMON/CLASSES/TCAWRKHISSUI.json", "ACLA_TCAWRKHISSUI.dat");
registerTest("SUPERV/CLASSES/ACTXPARAM.json", "ACLA_ACTXPARAM.dat");
registerTest("SUPERV/WINDOWS/OACLA.json", "AWI_OACLA.dat");
registerTest("SUPERV/CONTEXTS/AFOLDER.json", "ACTX_AFOLDER.dat");
registerTest("SUPERV/CONTEXTS/AHISTO.json", "ACTX_AHISTO.dat");
registerTest("SUPERV/FUNCTIONS/ABATPAR.json", "AFC_ABATPAR.dat");
registerTest("FA/GLOBVARS/G300CPFI.json", "AGB_G300CPFI.dat");
registerTest("SUPERV/INQUIRIES/AJE.json", "ACN_AJE.dat");
registerTest("SUPERV/OBJECTS/AAA.json", "AOB_AAA.dat");
registerTest("CHAPTERS/AAS/PARAMETERS/AASAUTNUM.json", "ADP_AASAUTNUM.dat");
registerTest("FINANCE/PROCESSES/1099MISC.json", "ADC_1099MISC.dat");
registerTest("SUPERV/REPORTS/ABLOB.json", "ARP_ABLOB.dat");
registerTest("SUPERV/REPORTS/ACODIF.json", "ARP_ACODIF.dat");

// Representations
registerTest("SUPERV/REPRESENTATIONS/ABANK.json", "ASW_ABANK.dat");

// Representations
registerTest("SALES/REPRESENTATIONS/SQUOTE.json", "ASW_SQUOTE.dat");

// Representations : validate ASHWMENU sub table + validate ASHWLNK with empty CODLNK
registerTest("SUPERV/REPRESENTATIONS/CONTACTCRM.json", "ASW_CONTACTCRM.dat");

// Representations : validate ASHWMET sub table
registerTest("PURCHASES/REPRESENTATIONS/PPRICSEARCHM.json", "ASW_PPRICSEARCHM.dat");

// Representations : validate ASHWOPT sub table
registerTest("SALES/REPRESENTATIONS/SPRICCONF.json", "ASW_SPRICCONF.dat");

// Screens
registerTest("SUPERV/SCREENS/AAB1.json", "AMK_AAB1.dat");

// Screens : validate AMSKPAR sub table
registerTest("PURCHASES/SCREENS/PTH1.json", "AMK_PTH1.dat");

registerTest("SUPERV/VIEWS/ADICADX.json", "AVW_ADICADX.dat");

registerTest("COMMON/MISCTABLES/6.json", "ADV_6.dat");

registerTest("SUPERV/ACTIONS/ABIIMP.json", "ACT_ABIIMP.dat");
...
asyncTest('Generation of single patches, one patch file per JSON file', 2 * unitTests.length, function(_) {
	...
	unitTests.forEach_(_, function(_, unitTest) {
		var patchFile;
		try {
			patchFile = pg.generatePatches(_, [path.join(config.scm.metaSubFolder, unitTest.json)])[0];
		...
		try {
			var refPatchFile = path.join(config.scm.refPatchFolder || (__dirname + "/refPatchFiles"), unitTest.refPatch);
...
asyncTest('Generation of unique patch file, one patch file for many JSON file', 2, function(_) {
	...
	var files = unitTests.map(function(unitTest) {
		return path.join(config.scm.metaSubFolder, unitTest.json);
	});
	...
	try {
		var refPatchFile = path.join(config.scm.refPatchFolder || (__dirname + "/refPatchFiles"), "FULL.dat");
		comparePatchFiles(_, config, pg, patchFile, refPatchFile);
		ok(true, "Compare with reference patch file : " + refPatchFile);
	} catch (err) {
		ok(false, "Compare with reference patch file : " + refPatchFile + ", reason = " + err.message);
