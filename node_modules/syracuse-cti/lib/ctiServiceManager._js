...
function CtiServiceManager() {
	...
	this.setup = function() {
		...
		// registering multiple times can create a memory leak in socket.io
		...
		socketHandler.register("/sage-cti",
			...
			// custom incomming events
			{
				pair: function(socket, handshake, data) {
					...
					if (!self.isProviderAlive())
						return tracer.debug && tracer.debug("[SOCKET.IO] " + socket.id + " Pairing failed for userId=" + userId + " : No CTI service available");
					if (!userId) return;
	...
	this.registerAgent = function(agentNum) {
		return this.isProviderAlive() && this.provider.registerAgent && this.provider.registerAgent(agentNum);
	};

	this.unregisterAgent = function(agentNum) {
		return this.isProviderAlive() && this.provider.unregisterAgent && this.provider.unregisterAgent(agentNum);
	};

	this.identification = function(event, lastName, firstName, companyName) {
		return this.isProviderAlive() && this.provider.identification && this.provider.identification(event, lastName, firstName, companyName);
	};

	this.showData = function(ctiId, tel) {
		this.isProviderAlive() && this.provider.showData && this.provider.showData(ctiId, tel);
	};

	this.makeCall = function(ctiId, tel) {
		this.isProviderAlive() && this.provider.makeCall && this.provider.makeCall(ctiId, tel);
	};
}
