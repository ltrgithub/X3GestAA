...
function IsiComClient(manager, uri) {
	...
	this.listen = function(_) {
		tracer.debug && tracer.debug("IsiComClient: listen to " + uri);
		_ensureConnect(_);
		var buf;
		while ((buf = _readEvent(_))) {
			tracer.debug && tracer.debug("IsiComClient: read=" + buf.toString());
			var event;
			try {
			...
			switch (event.name) {
				case "callIn":
					...
					data.countryIsoAlpha2 = countryForPhoneNumber(data.tel);
					tracer.debug && tracer.debug("IsiComClient: call in event with data=" + JSON.stringify(data));
					// 2. emit callIn on user socket
					this.manager.notifyUser(data.user.$uuid, event.name, data);
					break;
			}
		}
		tracer.debug && tracer.debug("IsiComClient: listenning to " + uri + " ended");
	};
