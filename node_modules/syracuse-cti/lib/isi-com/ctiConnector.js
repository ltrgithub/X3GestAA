...
if (test) {
	console.log(typeof ctiServerMethods.srvRedirectAcd);
	console.log(typeof ctiServerEvents.GetOpcode);
	console.log(ctiServerMethods.pad3(52));
}
...
function Connector() {
	...
	this.connect = function(opt, cb) {
				...
				sock.on('data', function(data) {
					...
					if ((str.length > 3) && (str[0] == controlSequence.STX) && (str.indexOf(controlSequence.CRLF, str.length - controlSequence.CRLF.length) != -1)) {
						...
						while (str.indexOf(controlSequence.CRLF) > 0) {
							...
							if (debug) {
								console.log('<= opcode=' + opcode + '-' + opcodeStr);
							}
							...
							if (opcode == ctiServerEvents.AgentInformations) {
								if (fields.length > 4) {
									if (debug) {
										console.log('<= AgentInformations agentLogon = ' + fields[4]);
									}
							...
							if (opcode == ctiServerEvents.CSTAEvent) {
								...
								if (debug) {
									console.log('<= CstaEvent : (' + fields[0] + ') ' + opcodeStr);
								}
		...
		// TODO : the "binary" encoding will be removed in future versions
		sock.setEncoding("binary");
	};

	this.registerAll = function() {
		sock.write(ctiServerMethods.GetMessageCommand(ctiServerMethods._RegisterAll, null));
	};

	this.makeCall = function(agentNum, phoneNumberToDial) {
		sock.write(ctiServerMethods.GetMessageCommand(ctiServerMethods.srvMakeCall, [agentNum, phoneNumberToDial]));
	};

	this.sendIdentification = function(agentNum, refCom, lastName, firstName, company) {
		sock.write(ctiServerMethods.GetMessageCommand(ctiServerMethods.srvSendIdentification, [agentNum, refCom, lastName, firstName, company]));
	};
}
