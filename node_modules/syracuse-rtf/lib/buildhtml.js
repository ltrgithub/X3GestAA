...
function builder(documentModel) {
	...
	function characterFormatting(node) {
		switch (node.word) {
			...
			case "tab":
				//case "pnfs":
				return $("<span style='margin-left: 10pt'>");
	...
	function paragraphFormatting(elt, node) {
		...
		switch (node.word) {
			...
			case "li":
				...
				if (node.arg > 0) return elt.css("margin-left", twipsToPoint(node.arg) + "pt");
				else return elt;
			case "ri":
				// return elt.css("margin-right", twipsToPixel(node.arg) + "px");
				if (node.arg > 0) return elt.css("margin-right", twipsToPoint(node.arg) + "pt");
				else return elt;
			default:
				return elt;
		}
	...
	return {
		...
		getHtml: function() {
			...
			sections.map(function(i, e) {
				var $e = $(e);
				$("<ul>").append($e.children().detach()).appendTo($e);
			});
