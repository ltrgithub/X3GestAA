...
function builder(documentModel) {
	...
	function newWorker() {
		...
		// return $("<div>").addClass('s-rtf-worker').addClass('s-rtf-clean');
	}

	function newSection() {
		// return $("<div>").addClass('s-rtf-section').addClass('s-rtf-clean');
...
/*case "bullet":
	...
	function sectionFormatting(elt, node) {
		...
		case "li":
			// return elt.css("margin-left", (node.arg / 15) + "px");
			return elt.css("margin-left", twipsToPoint(node.arg) + "pt");
		case "ri":
			// return elt.css("margin-right", (node.arg / 15) + "px");
	...
	context = (function() {
		...
		function flattenFormat(fmt, ctx) {
			if (ctx === null) return fmt;
			ctx = (typeof(ctx) === "undefined") ? current : ctx;
			var f = ctx.format;
			fmt = fmt || {};
			...
			return flattenFormat(fmt, ctx.parent);
		}
	...
	return {
		...
		control: function(node) {
			...
			case "li":
			case "ri":
				sectionFormatting($section, node);
				break;
		...
		getHtml: function() {
			...
			$("<ul>").append(sections.children().detach()).appendTo(sections);

			sections = $rootdiv.find(".s-rtf-section");

			// replace <div> by <p>
			sections.find(".s-rtf-paragraph").wrap("<p>").removeAttr("class").contents().unwrap();

			// cleanup
			sections.find(".s-rtf-worker :empty").remove();
			sections.find(":empty").remove();
