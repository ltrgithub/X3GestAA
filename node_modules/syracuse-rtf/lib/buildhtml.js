...
function builder2(documentModel) {
	...
	// var r = $("<div>").append("<p>").append("<p>");
	// r.children().wrap("<li>");
	// $("<ul>").append(r.children().detach()).appendTo(r);
	...
	function newFormat() {
		return $("<div>").addClass('s-rtf-format');
	}
	...
	function appendWorker() {
		...
		$worker && $worker.appendTo($section);
		$worker = null;
	}

	function appendSection() {
		$section && $section.appendTo($rootdiv);
		$section = null;
	}
...
/*case "bullet":
	...
	function paragraphFormattingStyle(elt, node) {
		...
		case "li":
			return elt.css("left-margin", (node.arg / 15) + "px");
		case "ri":
			return elt.css("right-margin", (node.arg / 15) + "px");
		default:
			return elt;
		}
	};
	...
	return {
		...
		close: function(idx, parent) {
			this.context = this.context.parent;
			$current = $worker; //$rootdiv.find("p:last");
			if ($current.size() === 0) $current = $rootdiv;
			if (idx < parent.length - 1) recoverContext.apply(this);
		},
		...
		control: function(node, context) {
			...
			case "ri":
				...
				var el;
				if (parent.is('div.s-rtf-format')) {
					el = paragraphFormattingStyle(parent, node);
				} else {
					el = paragraphFormatting(node);
					el.addClass('s-rtf-format');
					$worker.appendTo(el);
				}
				break;
			...
			default:
				...
				if (el != null) {
					this._updateContext(node.word, el.clone());
					$current.append(el);
					$current = el;
				}
			}
		...
		getHtml: function() {
			...
			var list = $rootdiv.find(".s-rtf-section.s-rtf-list").removeAttr("class");
			var workers = list.find(".s-rtf-paragraph");
			workers.contents().unwrap().wrap("<li>");
			$("<ul>").append(list.children().detach()).appendTo(list)

			list = $rootdiv.find(".s-rtf-section").removeAttr("class");
			list.find(".s-rtf-paragraph").removeAttr("class").contents().unwrap().wrap("<p>");
			...
			// r.children().wrap("<li>");
			// $("<ul>").append(r.children().detach()).appendTo(r);
			// $rootdiv.find(".s-rtf-section").removeClass();
			var ss = $rootdiv.html();
			return $rootdiv.html();
		},
		_getColor: function(context) {
