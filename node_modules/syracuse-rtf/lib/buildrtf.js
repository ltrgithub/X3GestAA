...
function rtfContext() {
	return {
		...
		close: function() {
			this.current--;
			if (!this.current) this.pending = false;
...
exports.buildRtf = function(html) {
	...
	function processNode(n, i) {
		...
		if (!$(n).is("blockquote") && bld.context.pending) {
			if (bld.context.current == 1 && ($(n).prev() && !$(n).prev().html())) {
				//previous text alone 
				if (!bld.context.par) bld.control("par");
			}
			bld.control("li" + (bld.context.current * 600));
			bld.context.pending = false;
		}
		if (n.nodeType === 3) { // TEXT node
			...
			bld.fragment("\\'B7\\tab}");
			if (i === 0) {
				var uls = $(n).parents('ul').length + bld.context.current + bld.context.ulindent;
	...
	function visit(i, e) {
		...
		if ($e.is("div[align]") || $e.is('*[style*=text-align]') || $e.is('*[style*=margin]') && !$e.is("blockquote")) {
			processNode(e, i);
		...
		var grp, nl, sibling = e.nextSibling,
			...
			$contents = $e.contents();
		if (!$e.is("blockquote") && sibling && sibling.nodeName === "DIV") {
		...
		if ($e.is("blockquote") && bld.context.current) {
			if (bld.context.pending) {
				bld.context.closeAll();
