...
exports.buildRtf = function(html) {
	...
	function processNode(n, i) {
		...
		if (n.nodeType === 3) { // TEXT node
			...
			if (!$(n).parent || !$(n).parent().is("blockquote")) {
				var nb = $(n).find("blockquote").length + 1;
				bld.controlarg("li", toTwips("px", (nb * 40)));
			}
		} else {
			bld.htmltag(n.nodeName);
		}
	...
	function visit(i, e) {
		...
		var grp, nl, sibling = e.nextSibling,
			...
			$contents = $e.contents();
		if (sibling && sibling.nodeName === "DIV" || ($e.is("blockquote") && sibling && sibling.nodeName !== "P" /*&& sibling && sibling.nodeName === "BLOCKQUOTE"*/ )) {
		...
		if (($e.is("blockquote") && e.childElementCount > 0 && e.firstChild.nodeName !== "BLOCKQUOTE") || $e.is('div[style*=margin-left]')) {
			bld.control("pard");
		}
		if (grp) {
			bld.fragment("}");
