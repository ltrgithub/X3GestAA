...
var _defaultFontSizeTag = "fs24";
var _defaultFontFace = "MS Sans Serif";
...
function builder(documentModel) {
	...
	rtftext += "\\uc0\\pard\\" + _defaultFontSizeTag;
	...
	return {
		...
		text: function(str) {
			...
			while (pos < str.length) {
				...
				} else if (str.charCodeAt(pos) == 10 || str.charCodeAt(pos) == 160) {
					rtftext += (needSpace ? " " : "");
				}
				needSpace = false;
				if (pos < str.length) {
		...
		controlarg: function(word, arg) {
			if (!rtftext.endsWith(word + arg) || word === "u") {
				rtftext += "\\" + word + arg;
				needSpace = true;
			}
		},
...
exports.getDefaultFontSize = function(native) {
	var fs = _defaultFontSizeTag;
	if (!native) {
		fs = parseInt(fs.slice(2), 10) / 2 + "";
	}
	return fs;
};
exports.getDefaultFontFace = function(opts) {
...
exports.buildRtf = function(html) {
	...
	if (html.indexOf('MsoNormal') >= 0) {
		html = _cleanWordHTML(html);
	}
	...
	function _cleanWordHTML(str) {
		...
		str = str.replace(/<(\w[^>]*) style="margin([^\"]*)"([^>]*)/gi, "<$1$3");
		...
		// str = str.replace(/<H3([^>]*)>/gi, '');n
		// str = str.replace(/<H4([^>]*)>/gi, '');
		// str = str.replace(/<H5([^>]*)>/gi, '');
		// str = str.replace(/<H6([^>]*)>/gi, '');
		// str = str.replace(/<\/H\d>/gi, '<br>'); //remove this to take out breaks where Heading tags were 
		// str = str.replace(/<(U|I|STRIKE)>&nbsp;<\/\1>/g, '&nbsp;');
		str = str.replace(/<(u|U|i|I|STRIKE)><o:p>&nbsp;<\/o:p><\/u>/g, '&nbsp;');
		str = str.replace(/<(B|b)>&nbsp;<\/(b|B)>/g, '');
		//str = str.replace(/<(P|p)>&nbsp;<\/(p|P)>/g, '');
	...
	function processNode(n, i) {
		...
		if (n.nodeType === 3) { // TEXT node
			...
			rules.forEach(function(e) {
				...
				if (stylerule[0] != null) stylerule[0] = stylerule[0].replace(/^\s*/, '').replace(/\s*$/, '').replace(/"/g, "");
				if (stylerule[1] != null) stylerule[1] = stylerule[1].replace(/^\s*/, '').replace(/\s*$/, '').replace(/"/g, "");
				switch (stylerule[0]) {
					case 'color':
					...
					case 'background-color':
						if (stylerule[1] === 'transparent') {
							idx = 0;
						} else if (stylerule[1].substring(0, 3) === 'rgb') {
							clr = rgbColor(stylerule[1]);
							idx = findColor(documentModel.colors, clr);
						} else {
							idx = findColorInColortbl(stylerule[1]);
					...
					case 'text-decoration':
						if (stylerule[1].indexOf('underline') > -1) bld.control('ul');
						if (stylerule[1].indexOf('line-through') > -1) bld.control('strike');
						break;
					case 'font-style':
					...
					case 'font-size':
						...
						break;
					case 'font-family':
						var font = stylerule && stylerule[1];
						font = font.replace(/'/gi, "");
						bld.font(findFont(font));
						bld.context.ff = true;
						break;
					case 'margin-left':
						var m = /([0-9\.\-]+)([A-Za-z]*)/g.exec(stylerule[1]);
						var unit = m[2] && m[2].toLowerCase() || "px";
						var val = parseInt(m[1].startsWith('.') ? m[1].replace('.', '0.') : m[1], 10);
						if ($(n).is("ul")) {
					...
					case 'margin-bottom':
						...
						break;
					case 'margin':
						if ($(n).attr('style').search("text-align") < 0) { // special case : copy from word with alignement twice margin and text-align. The best way will be to clean html before but ...
							$(n).contents().each(visit);
							if (n.nodeName === "P") {
								bld.control("par");
							}
	...
	function extractFontsAndColors() {
		...
		addFont(_defaultFontFace); //default
		// IE Fireforx
		...
		// Chrome
		var fnts2 = $root.find('*[style*="font-family"]');
		fnts2.each(function(i, e) {
			var s = $(e).attr("style"),
				styles = s && s.replace(/[\n\r]*/g, '').split(';'),
				fontstyles = styles && styles.filter(function(e) {
					var rule = e.split(':');
					rule[0] = rule[0].replace(/^\s*/, '').replace(/\s*$/, '').replace(/'/g, "").replace(/"/g, "");
					return ['font-family'].indexOf(rule[0]) != -1;
				});

			var font = fontstyles && fontstyles[0] && fontstyles[0].split(":");
			if (font && font[1]) {
				addFont(font[1].replace(/'/g, "").replace(/"/g, "").replace(/^\s*/, ""));
			}
		});
	...
	function visit(i, e) {
		...
		if (e.nodeName === "S" || $e.is('*[style*=text-decoration]')) {
			bld.control("strike0");
		}
		if ($e.is('*[style*=font-size]') && bld.context.fs) {
			bld.context.fs = false;
			if (!grp) {
				bld.control(_defaultFontSizeTag);
			}
		}
		if (e.nodeName === "I" || $e.is('*[style*=font-style]')) {
			bld.control("i0");
		}
		if (($e.is('*[style*=color]') || (e.nodeName === "FONT" && $e.attr('color') && $e.attr('color').length > 0)) && !$e.is('*[style*=background-color]') && !grp) {
			bld.control("cf0");
		}
		if (($e.is('*[style*=font-family]') || $e.is("font[face]")) && bld.context.ff) {
			bld.controlarg("f", "0");
			bld.context.ff = false;
		}
		if ($e.is('*[style*=background-color]') && !grp) {
			bld.control("highlight0");
		}
		if (e.nodeName === "UL") {
