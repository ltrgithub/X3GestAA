...
exports.buildRtf = function(html) {
	...
	function processNode(n) {
		...
		if (n.nodeType === 3) { // TEXT node
			...
			var tmp ;
			var stylerule ;
			//var booltmp=$(n).is("span[style^=font-size]") ; //bizarrement booltm==false dans firebug
			if ($(n).is("span[style^='font-size']")){
				 tmp = $(n).attr("style");
				 stylerule = tmp.split(':');
				//if (stylerule[0]==="font-size"){
				//var match=/(..|.).?(..|.)?pt/.exec(stylerule[1]);
			...
			if ($(n).is("span[style^='background-color']") || $(n).is("span[style^='color']")) {
				tmp = $(n).attr("style");
				 stylerule = tmp.split(':');
				//if (stylerule[0]==="background-color" || stylerule[0]==="color"){
					var clr = rgbColor(stylerule[1]);
					var idx = findColor(documentModel.colors, clr);
					var colorid ;
					if (stylerule[0] === "background-color") colorid = "highlight" // + (idx + 1); // "cb"+(idx+1) ;
					else if (stylerule[0] === "color") colorid = "cf" ; //+ (idx + 1);
					bld.controlarg(colorid,idx + 1);
			}
		} else if ($(n).is("font")) {
			//if ($(n).is("font[size]"))
			if ($(n).is("font[face]")) {
		...
		else if ($(n).is("div[style^='margin']")){
			var margin=$(n).attr("style");
			var stylerule=margin.split(':') ;
			var px=parseInt(stylerule[1]) ;

			bld.control("par") ;
			bld.control("pard") ;

			if (stylerule[0]=="margin-left") 
				bld.controlarg("li",px*15) ;
			else if (stylerule[0]=="margin-right") 
				bld.controlarg("ri",px*15) ;
			else if (stylerule[0]=="margin-top") 
				bld.controlarg("sb",px*15) ;
			else if (stylerule[0]=="margin-bottom") 
	...
	function findColorInColortbl(colorstr) {
		var idx = findColor(documentModel.colors, parseColor(colorstr));
		return idx + 1;
	}
	...
	function extractFontsAndColors(el) {
		...
		var stylebgclrs = $(el).filter('span[style^="background-color"]');
		var styleclrs = $(el).filter('span[style^="color"]');
		//var styles=$(el).filter('span[style]');

		//TODO: refactoring styleclrs.add("$(el).filter('span[style^=background-color]'"))
		...
		function processColorStyle(coln) {
			coln.each(function(i, e) {
				...
				var stylerule = tmp.split(':');
				var clr = rgbColor(stylerule[1]);
				if (findColor(documentModel.colors, clr) == -1) {
					documentModel.colors.push(clr);
				}
			});
		}

		/*function processStyle(coln){
			coln.each(function(i, e) {
				var tmp = $(e).attr("style");
				var stylerule = tmp.split(':');
				if (stylerule[0]==="color" || stylerule[0]==="background-color" )
				{
					var clr = rgbColor(stylerule[1]);
				if ( findColor(documentModel.colors, clr) == -1) {
					documentModel.colors.push(clr);
