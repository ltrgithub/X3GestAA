...
exports.buildRtf = function(html) {
	...
	function visit(i, e) {
		...
		if (sibling && sibling.nodeName === "DIV") {
			if (!(sibling.childElementCount === 1 && sibling.firstChild.nodeName === "BR") && !crlf) {
				nl = true;
			}
		} else if ($contents.length > 0 && e.nextSibling && !$e.is("ul") && !$e.is("li") && !$e.is("ul") && !$e.is("p")) {
			grp = true;
			bld.fragment("{");
		}
		if ($e.is("p[align]")) {
		...
		if ($e.is('*[style*=color]') && !$e.is('*[style*=background-color]')) {
			bld.control("cf0");
		}
		if (e.nodeName === "UL") {
			bld.control("pard");
