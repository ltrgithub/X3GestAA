...
exports.buildRtf = function(html) {
	...
	function visit(i, e) {
		if ($(e).is("div[align]") || $(e).is('*[style*=text-align]')){
			processNode(e);
			return ;
		}

		var grp=($(e).contents().size() > 0 && e.nextSibling!=null && !$(e).is("ul") && !$(e).is("li") ) ;
 	/*if (grp ){ 
 		var tt=$(e).parent();
 		if( tt.length>0){
 		var test=$('span~font') ;
 		var test1=tt.contents() ;
 		var test3=test1.filter(e.localName+'~*') ;
 		var test2= e.nextSibling ;//$(e).next() ;
 	}*/

		//var grp1=$(e).clone().parent().contents().size().
		if (grp ) 
		 bld.text("{");
		processNode(e);
