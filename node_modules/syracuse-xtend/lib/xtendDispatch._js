...
function initHrm(_, context) {
	function hasChanged(_, opt) {
		...
		var s = getSite(_, context.httpSession.hrm.current);
	...
	var p = new proxy.Proxy(_, webServerUrl, trace);
	proxies[pId] = proxies[pId] || {};
	proxies[pId][host + "_" + port] = proxies[pId][host + "_" + port] || p;
...
function forward(_, context, path) {
	...
	var p = proxies[pId][context.httpSession.hrm[pId]];
	...
	walk: function(_, context, name, id) {
		switch (name) {
			case "page":
				...
				initHrm(_, context);
				return forward(_, context, "/xtend/" + name + "/" + context.toWalk());
			case "remote":
