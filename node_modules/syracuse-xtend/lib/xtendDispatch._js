...
function initHrm(_, context, suburl) {
	function authenticate(_, context, baseUrl, solution, folder, site, certificate, suburl) {
		...
		var ssoBaseUrl = baseUrl + "/xtend/xtendsso/" + solution + "/" + folder + "/" + site;
		...
		try {
			...
			console.log("An error occured while parsing connection response :\n" + connectBody);
			return getPortal(_);
		}
	}
	//
	...
	var result = authenticate(_, context, webServerUrl, params.SOL, params.FLDR, params.SITE, null, suburl);

	var resp = result.resp;
	var JSESSIONID = result.id;
	//console.log("JSESSIONID after auth: "+JSESSIONID);

	// Instanciate and store proxy if not already exist
	context.httpSession.hrmSessions = context.httpSession.hrmSessions || {};
	var p = context.httpSession.hrmSessions[JSESSIONID] = new proxy.Proxy(_, webServerUrl, trace);
...
function forward(_, context, path) {
	...
	if (p) {
		...
		throw new Error("No HRM proxy associated to your http session.");
	}
}
