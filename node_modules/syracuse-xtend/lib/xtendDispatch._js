...
function getProxyInstance(context, solution, folder, site, host, port, JSESSIONID) {
	...
	function findInSessionMap(proxies, isCurrent, _solution, _folder, _site) {
		if (proxies) {
			var keys = Object.keys(proxies);
			var match, _jsid;
			for (var i in keys) {
				_jsid = keys[i];
				//if (isCurrent || (_solution !== solution && _folder !== folder && _site !== site && _jsid !== JSESSIONID)) {
		...
		if (sites) {
			...
			for (var i in keys) {
				...
				if (isCurrent || (_solution !== solution && _folder !== folder && _site !== site)) {
					match = findInSessionMap(sites[_site], false, _solution, _folder, _site);
					if (match) break;
				}
			}
	...
	if (solution && folder && site) {
		...
		if (host && port) {
			...
			if (!p || p.baseUrl !== baseUrl) {
				...
				res = findInSessionMap(sitMap, true);
				if (res && res.proxy) {
					trace && trace("Matching proxy configuration found [" + res.jsid + "] on current site");
					if (!res.jsid) return;
					...
					// Try to find matching proxy previously instanciated for same folder but other sites
					res = findInSiteMap(folMap, true);
					if (res && res.proxy) {
						trace && trace("Matching proxy configuration found [" + res.jsid + "] on current folder ; site [" + res.site + "]");
						if (!res.jsid) return;
						res.solution = solution;
						res.folder = folder;
						res.site = site;
						...
						// Try to find matching proxy previously instanciated for same solution but other folders
						res = findInFolderMap(solMap, true);
						if (res && res.proxy) {
							trace && trace("Matching proxy configuration found [" + res.jsid + "] on current solution ; folder [" + res.folder + "] ; site [" + res.site + "]");
							if (!res.jsid) return;
							res.solution = solution;
							res.folder = folder;
							res.site = site;
							...
							// Try to find matching proxy previously instanciated for other solutions
							res = findInSolutionMap(globalMap, true);
							if (res && res.proxy) {
								trace && trace("Matching proxy configuration found [" + res.jsid + "] on  solution [" + res.solution + "] ; folder [" + res.folder + "] ; site [" + res.site + "]");
								if (!res.jsid) return;
								res.solution = solution;
								res.folder = folder;
								res.site = site;
