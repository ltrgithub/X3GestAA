...
_scripts[1] = function(_, db) {
	exports.tracer && exports.tracer("Executing update script to version: 1; add automatically on automateTask the user, role and locale");
	...
	if (elts && elts.length > 0) {
		elts.forEach_(_, function(_, e) {
			...
			e.automateTasks(_) && e.automateTasks(_).toArray(_).forEach_(_, function(_, task) {
				task.user(_, user);
				task.role(_, role);
			});
			e.save(_);
			exports.tracer && exports.tracer("Set default user, role " + code + " on security profile ");
		});
		// UPDATE
