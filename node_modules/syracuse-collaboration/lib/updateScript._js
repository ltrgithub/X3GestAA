...
_scripts[38] = function(_, db) {
	...
	if (settings.length < 1) {
		...
		var setting = db.model.getEntity("setting").factory.createInstance(_, null, db);
...
_scripts[42] = function(_, db) {
	...
	hosts.forEach_(_, function(_, h) {
		var host = db.fetchInstance(_, db.model.getEntity("host"), { jsonWhere: { "hostname": h.hostname}});
