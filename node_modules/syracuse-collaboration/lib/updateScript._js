...
_scripts[6] = function(_, db) {
	...
	[{
		...
		var app = db.fetchInstance(_, db.model.getEntity(_, "application"), {jsonWhere:{
...
_scripts[7] = function(_, db) {
	...
	eps.forEach_(_, function(_, ep) {
		// find a x3server
		var x3server = db.fetchInstance(_, db.model.getEntity(_, "x3server"), {jsonWhere:{
		...
		//
		var epInst = db.fetchInstance(_, db.model.getEntity(_, "endPoint"), {jsonWhere:{$uuid: ep._id}});
		if(epInst) {
...
_scripts[42] = function(_, db) {
	...
	hosts.forEach_(_, function(_, h) {
		var host = db.fetchInstance(_, db.model.getEntity(_, "host"), { jsonWhere: { "hostname": h.hostname}});
...
function _stringToLocalizedString(_, db, collectionName, properties) {
	...
	objects.forEach_(_, function(_, c){
		...
		db.db.collection(collectionName, _).update({ _id: c._id }, { $set: upd },{ safe: true, multi: true }, _);
	}) ;
}
