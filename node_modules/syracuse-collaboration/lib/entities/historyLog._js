...
function _propertyToText(_, instance, obj, path) {
	...
	if (_isCollection(obj)) {
		...
		for (var key in obj) {
			...
			if (_isProperty(obj[key]) || (prop && prop.$isArray)) {
				realName = prop.getTitle();
				v = (Array.isArray(obj[key].$value) ? obj[key].$value.join() : obj[key].$value);
				p = (Array.isArray(obj[key].$prev) ? obj[key].$prev.join() : obj[key].$prev);
...
function _getValueFromTemplate(_, instance, propName, obj) {
	...
	template.matches.forEach_(_, function(_, match) {
		...
		if (obj[pName]) res = res.replace(match, _getLocalizeValue(obj[pName]));
