...
function x3Test(_) {
	...
	while (--i >= 0) {
		console.log(i + "E " + endpoints[i].description(_));
		if (endpoints[i].description(_) !== "superv7") continue;
		var milli = Date.now();
		var startTime = milli;
		var orm = endpoints[i].getOrm(_);
		...
		var result = {
			...
			cpu: cpuCnt / Math.max(1, cpuTime), // difference between "find regex" and "find some", measures Mongo server CPU speed, the higher, the better
			file: fileCnt / Math.max(1, fileTime), // difference between "find regex" and "find some", measures Mongo server CPU speed, the higher, the better
			dbms: dbmsCnt / Math.max(1, dbmsTime), // difference between "find regex" and "find some", measures Mongo server CPU speed, the higher, the better
			dbmsLat: dbmsLatCnt / Math.max(1, dbmsLatTime) // difference between "find regex" and "find some", measures Mongo server CPU speed, the higher, the better
		};
		return result;
...
function callX3(_, client, type, limit, method) {
	...
	request.url = url;
	// console.log("Anfrage "+util.format(request))
	...
	// console.log("Antwort "+util.format(erg)+" "+response.toString());
}
