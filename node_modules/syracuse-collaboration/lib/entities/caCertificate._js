...
exports.entity = {
	...
	$valueTemplate: "{name}",
	$helpPage: "Administration-reference-CA-Certificates",
	$events: {
		$beforeSave: [
			function(_, instance) {
		...
		$afterSave: [
			function(_, instance) { // update nanny processes unless special marker property has been set
				if (config.mockServer) {
					...
					try {
						console.log(config.mockServer.mockClient.simpleRequest.simpleRequest(options, null, _));
					} catch (e) {
						console.log("Error " + e);
					}
				}
	...
	$actions: {
		$save: function(_, instance) {
			var r = {};
			if (config.mockServer && instance.$snapshot && !instance.$created) {
	...
	$functions: {
		...
		getPEMCertificate: function(_) {
			return certTools.getPEMCertificate(_, this.name(_), true);
		}
	},
	$defaultOrder: [
		["name", true]
