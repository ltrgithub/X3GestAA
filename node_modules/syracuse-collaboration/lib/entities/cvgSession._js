	...
	$properties: {
		...
		cpuGraph: {
			...
			$compute: function(_, instance) {
				return (instance.cpuGraphStore = (instance.cpuGraphStore || new x3Logs.LogStore("image/svg+xml", instance.x3user(_), instance.x3pid(_))));
			}
		},
		log: {
			$title: "Log",
			...
			$compute: function(_, instance) {
				return (instance.logStore = (instance.logStore || new x3Logs.LogStore("x-trace", instance.x3user(_), instance.x3pid(_))));
			}
		}
	},
	$relations: {},
				...
				try {
					...
					if (instance) {
						if (instance.reused(_)) {
							if (!cvg.cvgAdmReuseKill(_, instance.cid(_))) {
