...
exports.entity = {
	...
	$functions: {
		...
		replacePredefinedVars: function(_, sdata) {
			...
			r = r.replace("{$profile.selectedRole.endPoints}", "(" + up.selectedRole(_).endPoints(_).toUuidArray(_).map(function(rId) {
				return "\"" + rId + "\"";
			}).join(",") + ")");
			...
			// special section for security level restrictions
			//			var db = adminHelper.getCollaborationOrm(_);
			//			r = r.replace("{$authorized.roles}", )
		...
		canSaveSecLevel: function(_, instance) {
			...
			if (sp && sp.securityLevel(_) > secLevel) {
				instance.$addError(locale.format(module, 'secLevel', secLevel));
				return false;
			}
