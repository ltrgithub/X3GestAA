	...
	$properties: {
		...
		sageOwner: {
			...
			$isHidden: function(_, instance) {
				if (instance.authoringLevel(_) !== "admin" || !((globals.context.config || {}).system || {}).enableDevelopmentFeatures) {
					instance.factoryOwner(_, "");
					return true;
				}
				return false;
		...
		factoryOwner: {
			...
			$isHidden: function(_, instance) {
				return instance.authoringLevel(_) !== "admin" || (!((globals.context.config || {}).system || {}).enableDevelopmentFeatures && !(globals.context.config || {}).enablePartnerFeatures);
			},
	...
	$events: {
		$canSave: [
			...
			function(_, instance) {
				...
				if (!instance.canSaveSecLevel(_, instance, 0)) return false;
		...
		$beforeSave: [
			...
			function(_, instance) {
				...
				if (fId !== "") instance.factoryOwner(_, fId.toUpperCase());
			}
		]
	},
	$functions: {
		...
		hasFactoryRights: function(_) {
			return this.factoryOwner(_) && this.factoryOwner(_) !== "";
		},
		canUpdateFactoryInstance: function(_, instance, ignoreInstance) {
			if (ignoreInstance || instance.$factory) {
				...
				if (this.factoryOwner(_) && this.factoryOwner(_) !== "") {
					return instance.$factoryOwner ? instance.$factoryOwner === adminHelper.getFactoryOwner(_, this) : true;
				}
				return false;
			}
			return true;
		...
		canSaveSecLevel: function(_, instance, secLevel) {
			// verify security level against current user level security level
			// check against 'undefined' is for case secLevel = 0
			secLevel = typeof secLevel !== 'undefined' ? secLevel : this.securityLevel(_) || 0;
