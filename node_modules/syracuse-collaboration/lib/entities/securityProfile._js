...
exports.entity = {
	...
	$properties: {
		...
		authoringLevel: {
			...
			$enum: function(_, instance) {
				var values = [];
				if (((globals.context.config || {}).system || {}).enableDevelopmentFeatures || helpers.enableDeveloppementFeatures) values.push({
		...
		$canSave: [
			...
			function(_, instance) {
				...
				items.some_(_, function(_, item) {
					if (item.code(_) === "myProfile") {
						if (!item.canRead(_)) throw new Error(locale.format(module, "noProfile"));
						return true;
					}
				});
			}
	...
	$functions: {
		...
		canChangeFactoryInstance: function(_, instance) {
