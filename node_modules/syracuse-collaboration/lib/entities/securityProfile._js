...
var secProfileItem = require("syracuse-collaboration/lib/entities/securityProfileItem");
...
exports.entity = {
	...
	$init: function(_, instance) {
		function hasPropertiesActivable(_, itemCode) {
			var propsEnabled = false;
			secProfileItem.propsAvailable.forEach_(_, function(_, p) {
				if (secProfileItem.canModifyProperty(_, itemCode, p)) propsEnabled = true;
			});
			return propsEnabled;
		}
		...
		Object.keys(profile).forEach_(_, function(_, pName) {
			if (hasPropertiesActivable(_, pName)) {
	...
	$functions: {
		...
		replacePredefinedVars: function(_, sdata) {
			...
			r = r.replace("{$profile.securityProfile.level}", sp.securityLevel(_));
			r = r.replace("{$profile.securityProfile.code}", sp.code(_));
			return r;
		},
