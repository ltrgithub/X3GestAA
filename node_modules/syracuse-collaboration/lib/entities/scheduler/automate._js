...
function _automateRun(automate, tenantId) {
	// I'm here because of the timer event so remove the id
	automate._timerId = null;
	globals.context.tenantId = tenantId; // preserve tenantId
	//
	automate.run(!_);
}
...
exports.entity = {
	...
	$functions: {
		...
		scheduleNextRun: function(_) {
			...
			if (diff) {
				if (diff > 0) this._timerId = setTimeout(_automateRun, diff, this, globals.context.tenantId);
				else
				// small timer to be sure that the next run won't be computed the same ms
					this._timerId = setTimeout(_automateRun, 100, this, globals.context.tenantId);
				//this.run(_);
