...
exports.entity = {
	...
	$helpPage: "Administration-reference-Scheduler",
	$properties: {
		description: {
			$title: "Automate",
			$isMandatory: true,
	...
	$functions: {
		...
		run: function(_, diagnoses, track) { // track: track some information. If there is an error during automate execution, the error is returned
			...
			try {
				...
				console.log("SCH 1y " + (Date.now() - anfang));
				self.status(_, "error");
				error = e;
			...
			console.log("SCH ENDEENDE" + (Date.now() - anfang));
			return error;
		}
	},
	$events: {
