...
exports.entity = {
	...
	$properties: {
		...
		days: {
			...
			$enumCaptions: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"],
			$isArray: true,
			$type: "string",
	...
	$functions: {
		getNextRun: function(_, last) {
			...
			if(!times || !times.length || !days || !days.length)
				return null;
			//
			var nextTime;
			var i = 0;
...
//			console.log("diff: "+diff);
			if(diff < 0)
				res = last.addDays(1).addSeconds(diff);
			else
				res = last.addSeconds(diff);
			// check days list
