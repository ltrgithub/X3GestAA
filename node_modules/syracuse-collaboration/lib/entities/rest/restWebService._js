...
var tracer = config && config.rest.client && config.rest.client.tracer != null ? config.rest.client.tracer : false;
...
exports.entity = {
	...
	$functions: {
		exexRequest: function(_, method, subUrl, parameters, headers, data) {
			...
			var response;
			// data only for POST and PUT methods
			if (method === "POST" || method === "PUT") {
				trace("Data: " + JSON.stringify(data, null, 2));
				response = request.end(data).response(_);
			} else {
				response = request.end().response(_);
			}

			var content = response.readAll(_);
			if (this.contentType(_) === "json") content = JSON.parse(content);
	...
	$services: {
		get: {
			...
			$execute: function(_, context, instance) {
				...
				try {
					var res = instance.exexRequest(_, "GET");
