	...
	$events: {
		$beforeSave: [function(_, instance) {
			try{
				...
				stub.url(_, instance.url(_));
				//stub.description(_, "Tester");
				stub.authentication(_, 'basic');
				stub.authUser(_, "guest");
				stub.authPassword(_, "guest");
				stub.$generate(_, stub);
				console.log("STUB="+JSON.stringify(stub.serializeInstance(_),null,2));
				instance.stub(_, stub);
				console.log("Stub gen end");
