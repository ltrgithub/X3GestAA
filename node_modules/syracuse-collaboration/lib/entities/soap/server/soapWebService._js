	...
	$relations: {
		...
		representations: {
			...
			$select: {
				...
				$fieldMap: { name: "name", title: "title" , entityName: "entityName"},
		...
		$beforeSave: [function(_, instance) {
			try{
				...
				if (representations.length === 0)
					instance.$diagnoses.push({severity: "error", message: "At least one representation is required"});
				...
				if (instance.$diagnoses.length === 0){
					...
					var entity = db.model.getEntity("soapStub");
					var stub = entity.factory.createInstance(_, null, db);
					stub.name(_, instance.name(_));
					stub.url(_, null);
					stub.description(_, "Tester");
					stub.authentication(_, 'basic');
					stub.authUser(_, "guest");
					stub.authPassword(_, "guest");
					...
					forEachKey(stub.$diagnoses, function(key, diag) {
						instance.$diagnoses.push({severity: diag.severity, message: diag.message});
					});
