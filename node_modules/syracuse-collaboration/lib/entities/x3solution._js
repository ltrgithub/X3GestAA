...
exports.entity = {
	...
	$valueTemplate: "{solutionName} {serverHost}:{serverPort}",
		...
		createEndpoints: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				folders.forEach_(_, function(_, f) {
					console.error("to create: " + JSON.stringify(s, null, 2));
				});
			},
		},
	},
	...
	$events: {
		...
		$beforeSave: [
			...
			function(_, instance) {
				...
				var solName = instance.solutionName(_);
				var mainHost = instance.serverHost(_);
				var mainPort = instance.serverPort(_);

				if (!instance.runtimes(_).toArray(_).some_(_, function(_, r) {
					...
					x3srv.tag(_, "MAIN");
					instance.runtimes(_).set(_, x3srv);
				}


				var options = {
					method: "GET",
					url: instance.baseUrl(_) + "/Adonix_" + solName + "/solution.json",
					headers: {}
				};
				if (!instance.proxy(_)) options.ignoreProxy = true;
				var data;
				try {
					...
					if (resp.statusCode !== 200) {
						instance.$addDiagnose("warning", locale.format(module, "cantAccessSolutionJson", options.url));
					}
					data = resp.readAll(_);
				} catch (e) {
					instance.$addDiagnose("warning", locale.format(module, "cantAccessSolutionJson", options.url));
				...
				if (data) {
					...
					try {
						...
						instance.$addDiagnose("error", locale.format(module, "cantParseSolutionJson", e.message, options.url));
					}

					var runtimes = jsonSolution.runtimes;
					if (runtimes.length === 0) {
						instance.$addDiagnose("error", locale.format(module, "noRuntimes", solName));
					}

					// check consistency with main declaration
					if (!runtimes.some_(_, function(_, r) {
						if (r.mainPort === mainPort && r.server === mainHost) return true;
						if (r.mainPort === mainPort && r.server.indexOf('.') && r.server.split('.')[0] === mainHost) {
							instance.serverHost(_, r.server);
							return true;
						}
					})) instance.$addDiagnose("error", locale.format(module, "noMainRuntimeMatch", mainHost, mainPort, solName));

					// register every runtimes (x3servers)
					runtimes.forEach_(_, function(_, r) {

						var _runtimes = instance.runtimes(_).filter(_, {
							jsonWhere: {
								serverHost: r.server,
					...
					instance.runtimes(_).toArray(_).forEach_(_, function(_, r) {
						if (r.tag(_) === "MAIN" && r.serverHost(_) !== instance.serverHost(_)) {
							instance.runtimes(_).deleteInstance(_, r.$uuid);
						}
						if (r.serverHost(_) === instance.serverHost(_) && r.serverPort(_) === instance.serverPort(_)) {
							r.tag(_, "MAIN");
						}
