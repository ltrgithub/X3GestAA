...
var selectX3Server = function(_, solution, options) {
	...
	try {
		...
		var realList = servers.filter_(_, function(_, s) {
			// TODO add a checkServer to be sure he's still banned in order to don't oblige the user to change manually the value. The server can be banned because of network problem and comes back
			var isBanned = s.banned(_);
			if (isBanned) {
				if (solution.endpoints(_) && solution.endpoints(_).toArray(_).length) {
					isBanned = !s.checkServer(_, solution.endpoints(_).toArray(_).length && solution.endpoints(_).toArray(_)[0].x3ServerFolder(_));
				}
				if (isBanned) {
					// try again after check if the server is really down or not
					bannedList.push(s.$uuid);
				} else { // if the server is reachable, he's no longer in the bannish state
					s.banned(_, isBanned);
					s.save(_);
				}
			}
