...
var selectX3Server = function(_, solution, options) {
	...
	try {
		...
		var realList = servers.filter_(_, function(_, s) {
			...
			if (isBanned && s.errorTry(_) >= 5 && s.tag(_) !== "MAIN") {
				bannedList.push(s.$uuid); // this server have to really be bannish
			} else {
				//temporary banned to don't select it we have to reset the banned to false in order to allow to retry on it
				s.banned(_, false);
				var ins = solution.save(_, null, {
					clearDiagnoses: true
				});
				var err = hasErrors(ins);
				if (err) {
					throw new Error(err);
				}
			}
