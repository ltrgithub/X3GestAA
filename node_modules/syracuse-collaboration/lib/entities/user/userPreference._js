...
exports.entity = {
	...
	$functions: {
		...
		setAppConnection: function(_, selEp, inst) {
			...
			function cleanData(_, _data) {
				...
				if (_data && _data.$properties) {
					...
					forEachKey(_data.$properties, function(k, val) {
						...
						if (!mustResetConn && _data[k] != null && val.$resetConnection === true) {
					...
					if (mustResetConn) {
						globals.context.session.appConnection[dataset].$resetConnection = true;
					}
				}
				return _data;
			}
			...
			if (stored) {
				data = inst && inst.$snapshot && inst.$snapshot.data(_);
				if (data) data = JSON.parse(data);
				var newData = stored.data(_);
				if (newData) newData = cleanData(_, JSON.parse(newData));
				if (data && newData) {
				...
				if (inst.endpoint(_) && inst.endpoint(_).$uuid === selEp.$uuid) {
					data = JSON.stringify(cleanData(_, JSON.parse(inst.data(_))))
					trace && trace("new app connection data stored into mongo:", data);
					inst.data(_, data);
