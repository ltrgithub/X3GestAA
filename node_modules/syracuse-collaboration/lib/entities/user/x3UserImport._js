...
function _convertDiag(d) {
	return d.severity ? { $severity: d.severity, $message: d.message } : d;
}

function _addDiagnose(diagnoses, severity, message) {
	console.log("added diag message (22); "+message);
...
var _groupPolicyMap = {
	create: function(_, instance, menuProfile, options) {
		...
		grp.description(_, locale.format(module, "autoGroupDesc", datetime.now().toString("yyMMdd_HHmmss"), (role && role.description(_)) || "-", ep.dataset(_)));
		grp.role(_, role);
	...
	menuProfile: function(_, instance, menuProfile, options) {
		...
		if(role) {
			...
			_addDiagnose(options && options.$diagnoses, "warning", locale.format(module, "roleNotFound", menuProfile, ep.dataset(_)));
		//
		if(!grp && (instance.createGroupPolicy_menuProf(_) === "create"))
			return _groupPolicyMap.create(_, instance, menuProfile, options);
		else
			return grp;
	},
...
//			db.resetCache();
			...
			if(!entity)
				throw new Error(locale.format(module, "entityNotFound", this.endpoint(_).description(_)));
			var where = {
		...
		importUser: function(_, user, options) {
			...
			if(!keyVal)
				return _addDiagnose(opt.$diagnoses, "error", locale.format(module, "missingKey", self.keyProperty(_), user.USR(_), user.INTUSR(_)));
			...
			if(!syraUser) {
				...
				if(dupUser)
					return _addDiagnose(opt.$diagnoses, "error", locale.format(module, "duplicateLogin", user.LOGIN(_), self.keyProperty(_), keyVal))
				//
			...
			if(!grp)
				_addDiagnose(opt.$diagnoses, "warning", locale.format(module, "groupNotFound", user.PRFMEN(_), self.endpoint(_).dataset(_)));
			else
				syraUser.groups(_).set(_, grp);
			...
			if(options && options.$diagnoses) {
				...
				(diags || []).forEach(function(diag) {
					...
					if(diag.$severity === "warning")
						_addDiagnose(opt.$diagnoses, "warning", locale.format(module, "importWarn", keyVal, diag.$message));
				});
			...
			if(!hasError)
				_addDiagnose(opt.$diagnoses, "success", locale.format(module, created ? "importUserCreated" : "importUserModified", keyVal));
			//
		}
	},
	$services: {
