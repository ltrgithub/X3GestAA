...
var _groupPolicyMap = {
	create: function(_, instance, menuProfile, options) {
		...
		if(!role) instance.$addDiagnose("warning", locale.format(module, "roleNotFound", menuProfile, endpoint.dataset(_)));
		//
	...
	menuProfile: function(_, instance, menuProfile, options) {
		...
		if(role) {
			...
			instance.$addDiagnose("warning", locale.format(module, "roleNotFound", menuProfile, endpoint.dataset(_)));
		//
...
exports.entity = {
	...
	$relations: {
		...
		role: {
			...
			$type: "role",
			$isDefined: function(_, instance) {
				return instance.createGroupPolicy_create(_) === "existingRole";
			}
		}
	},
	$functions: {
...
//			db.resetCache();
		...
		importUser: function(_, user, options) {
			...
			if(!grp)
				instance.$addDiagnose("warning", locale.format(module, "groupNotFound", user.PRFMEN(_), self.endpoint(_).dataset(_)));
			else
			...
			if(!hasError)
				_addDiagnose(options.$diagnoses, "success", locale.format(module, created ? "importUserCreated" : "importUserModified", keyVal));
			//
		}
	},
	$services: {
