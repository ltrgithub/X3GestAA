...
function _createNotification(_, user, notificationCode, parameters) {
	var db = user._db;
	//
	var notification = db.fetchInstance(_, db.getEntity("notificationEvent"), {
		jsonWhere: {
			code: notificationCode
		}
	});
	notification && notification.schedule(_, user.$uuid, new Date().getTime(), parameters, "db");
}

var _eventActionsMap = {
	deactivate: function(_, user, parameters) {
		var pp = parameters;
		user.active(_, false);
		user.save(_, null, {
		...
		if (user.hasErrors(_)) {
			pp.notification && pp.notification.fail && _createNotification(_, user, pp.notification.fail, pp);
		} else {
			pp.notification && pp.notification.success && _createNotification(_, user, pp.notification.success, pp);
		}
	},
	activate: function(_, user, parameters) {
...
exports.entity = {
	...
	$services: {
		...
		testuser: {
			...
			$execute: function(_, context, instance, parameters) {
				try {
					var cu = require('syracuse-hrm/lib/userCreation').user;
					var res = cu(_, "asdf", instance.login(_), "mrs", "fn", "ln", "fn.ln@gmx.de", "R4Z9L", "123456789", "9876554", 2);
					console.log("OKx " + res + " <<");
				} catch (e) {
	...
	$functions: {
		...
		fire: function(_, key, parameters) {
			var pp = parameters || {};
			var action = pp.action || "deactivate";
			//
			_eventActionsMap[action] && _eventActionsMap[action](_, this, pp);
		}
	},
	$events: {
		$canSave: [
			...
			function(_, instance) {
				...
				// verify security level for groups against current user level security level
				if (instance.groups(_).toArray(_).some_(_, function(_, grp) {
					...
					if (grp.$isDeleted && isCurrent) {
						var spCode = grp.role(_).securityProfile(_).code(_);
