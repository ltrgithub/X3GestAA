...
exports.entity = {
	...
	$functions: {
		$serialize: function(_) {
			...
			// This cannot be handled by the profile.json file
			this._allowRead = this.allowRead(_);
			var res = this._internalSerialize(_);
			delete this._allowRead;

			return res;
		},
		$canReadProperty: function(_, key) {
			return key && (key[0] === "$") || this._allowRead;
		},
		...
		getSecurityLevel: function(_) {
			...
			grps.forEach_(_, function(_, grp) {
				...
				if (secLevel == null || secProfile && secProfile.securityLevel(_) < secLevel) secLevel = secProfile.securityLevel(_);
			});
			return secLevel;
		},
		allowRead: function(_) {
			...
			// console.error("user.allowRead: login='" + this.login(_) + "', secLevel=" + secLevel + ", current=" + spLevel);
			return secLevel > spLevel;
		},
