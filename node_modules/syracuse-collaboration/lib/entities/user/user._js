	...
	$events: {
		$canSave: [
			...
			function(_, instance) {
				...
				var secLevel = instance.groups(_).toArray(_).reduce_(_, function(_, r, grp) {
					var sl = grp.role(_) && grp.role(_).securityProfile(_) && grp.role(_).securityProfile(_).securityLevel(_);
					return r < sl ? r : sl;
				}, 10);
				var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
