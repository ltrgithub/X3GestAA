...
exports.entity = {
	...
	$properties: {
		...
		signature: {
			$title: "New signature code", // LOC: "New password" 
			$type: "password",
			$salt: "{login}:{$realm}",
			$capabilities: "confirm",
			$isHidden: function(_, instance) {
		...
		sfUserName: {
			...
			$isHidden: function(_, instance) {
				var mandatory = true;
				instance.groups(_).toArray(_).forEach_(_, function(_, grp) {
					if (grp.description(_) === 'Salesforce CRM' && !grp.$isDeleted) {
						mandatory = false;
						return;
					}
				});
		...
		$canSave: [
			...
			function(_, instance) {
				...
				if (!instance.$created && instance.$snapshot) {
					...
					tracer && tracer(console.log("OLD " + oldPassword + " inst " + instance.password(_)));
