	...
	$properties: {
		...
		enableSageID: {
			...
			$isHidden: function(_, instance) {
				...
				if (Array.isArray(methods) && methods.indexOf("sage-id") > -1)
					hidden = false;
				return hidden;
			}
		},
		enableBasic: {
			...
			$isHidden: function(_, instance) {
				var hidden = true;
				var methods = (config && config.session && config.session.auth) || "basic";
				if (Array.isArray(methods) && methods.indexOf("basic") > -1)
					hidden = false;
				return hidden;
			}
		},
		enableOAuth2: {
			...
			$isHidden: function(_, instance) {
				var hidden = true;
				var methods = (config && config.session && config.session.auth) || "basic";
				if (Array.isArray(methods) && methods.indexOf("oauth2") > -1)
					hidden = false;
				return hidden;
			}
		},
		enableLdap: {
			...
			$isHidden: function(_, instance) {
				var hidden = true;
				var methods = (config && config.session && config.session.auth) || "basic";
				if (Array.isArray(methods) && methods.indexOf("ldap") > -1)
					hidden = false;
				return hidden;
			}
		},
		enableSaml: {
			...
			$isHidden: function(_, instance) {
				var hidden = true;
				var methods = (config && config.session && config.session.auth) || "basic";
				if (Array.isArray(methods) && methods.indexOf("saml") > -1)
					hidden = false;
				return hidden;
			}
		}
