...
/*function _changeLogin(_, instance, parameters) {
	...
	switch (auth) {
		case "db":
			...
			var hash = crypto.createHash('MD5');
			var pwd = instance.password(_);
			var a1 = oldLogin + ":" + realm + ":" + password;
			if (pwd[0] === "U") { // hash has been obtained using UTF8 representation (new)
				pwd = pwd.substr(1);
				hash.update(a1, 'utf8');
			} else
				hash.update(a1, 'binary');
			if (hash.digest("hex") !== pwd) {
				instance.$addError(locale.format(module, "wrongPass"));
				return;
			}
			// set new password hash
			var a1 = parameters.newLogin + ":" + realm + ":" + password;
			var hash = crypto.createHash('MD5');
			hash.update(a1, 'utf8');
			instance.password(_, "U" + hash.digest("hex"));
			break;
		...
		default:
			var signature = instance.signature(_);
			if (signature) {
				var hash = "U" + _h(oldLogin + ":" + realm + ":" + password);
				if (hash !== instance.signature(_)) {
					instance.$addError(locale.format(module, "wrongSign"));
					return;
				}
			}
			var hash = "U" + _h(parameters.newLogin + ":" + realm + ":" + password);
			// set new password hash
			instance.signature(_, hash);
...
exports.entity = {
	...
	$properties: {
		...
		/*authentication: {
			...
			$propagate: function(_, instance, val) {
				if (val != "ldap") instance.ldap(_, undefined);
		...
		oldPassword: {
			...
			$isHidden: function(_, instance) {
				...
				return true;*/
				var authMethods = instance.getUserAuthenticationMethods(_);
				if (authMethods.indexOf("db"))
					return !noAdmin(_);
				return true;
			}
		},
		password: {
			...
			$isMandatory: function(_, instance) {
				...
				return instance.basicEnabled(_);
			},
			$isHidden: function(_, instance) {
				/*var auth = instance.authentication(_) || authHelper.getStandardSetting(_).source || "";
				return auth !== "db";*/
		...
		oldSignature: {
			...
			$isHidden: function(_, instance) {
				...
				return true;*/
				if (!instance.basicEnabled(_) && !instance.basicEnabled(_)) {
					return !noAdmin(_);
				}
				return true;
			}
		...
		signature: {
			...
			$isHidden: function(_, instance) {
				...
				return (instance.basicEnabled(_) || instance.ldapEnabled(_));
			}
		},
		email: {
		...
		sageIdEnabled: {
			$title: "Sage ID",
			$type: "boolean",
			$isHidden: authDefaultValue('sage-id')
		},
		basicEnabled: {
			$title: "Basic",
			$type: "boolean",
			$isHidden: authDefaultValue('basic')
		},
		oauth2Enabled: {
			$title: "OAuth2",
			$type: "boolean",
			$isHidden: authDefaultValue('oauth2')
		},
		ldapEnabled: {
			$title: "LDAP",
			$type: "boolean",
			$isHidden: authDefaultValue('ldap')
		},
		samlEnabled: {
			$title: "SAML",
			$type: "boolean",
			$isHidden: authDefaultValue('saml')
		}
	},
	$relations: {
		groups: {
	...
	$services: {
		...
		*/
		/*testlogin: {
			...
			$isDefined: function(_, instance) {
				...
				if (authMethods.indexOf("ldap") === -1)
					return false;
				return (authMethods.indexOf("ldap"));
			},
			$isMethod: true,
			$parameters: {
				$properties: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				try {
					ldapauth.ldapAuthentication(_, ldapName, password, cfg);
		...
		// service with 1 parameter for LDAP login
		changeLogin2: {
			$method: "post",
			$title: "Change login",
			$isMethod: true,
		...
		// service with 2 parameters for external login
		changeLogin3: {
			$method: "post",
			$title: "Change login",
			$isMethod: true,
	...
	$functions: {
		...
		mustChangePassword: function(_) {
			...
			var authMethods = this.getUserAuthenticationMethods(_);
			return authMethods.indexOf("db");
		...
		getUserAuthenticationMethods: function(_) {
			var authMethods = [];
			if (this.sageIdEnabled(_))
				authMethods.push("sage-id");
			if (this.basicEnabled(_))
				authMethods.push("basic");
			if (this.oauth2Enabled(_))
				authMethods.push("oauth2");
			if (this.ldapEnabled(_))
				authMethods.push("ldap");
			if (this.samlEnabled(_))
...
function authDefaultValue(providerType) {
	return function(_, instance) {
		var hidden = true;
		var methods = (config && config.session && config.session.auth) || "basic";
		if (Array.isArray(methods) && methods.indexOf(providerType) > -1)
			hidden = false;
		return hidden;
	};
}