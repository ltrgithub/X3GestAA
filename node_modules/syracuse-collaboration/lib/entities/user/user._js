	...
	$events: {
		$canSave: [
			...
			function(_, instance) {
				...
				// no need to check if there are no groups
				var grps = instance.groups(_).toArray(_);
				if (grps.length) {
					// verify security level for groups against current user level security level
					var secLevel = grps.reduce_(_, function(_, r, grp) {
						var sl = (grp.role(_) && grp.role(_).securityProfile(_) && grp.role(_).securityProfile(_).securityLevel(_)) || 0;
						return r < sl ? r : sl;
					}, 1000);
					var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
					...
					// lowest is strongest (admin is zero)
					if (sp && secLevel < sp.securityLevel(_)) {
						instance.$addError(locale.format(module, "secLevel"), "groups");
						return false;
					}
				}
