			...
			function(_, instance) {
				...
				if (auth === "oauth2" || auth === "saml2" || auth === "sage-id") {
					...
					if (useremail.indexOf('@') > 0) {
						var condition = '(authentication eq "' + auth + '"' + (auth === auth2 ? ' or authentication eq "")' : ')') + ' and (email like "' + useremail + '" or ((email eq null or email eq "") and (login eq "' + useremail + '"))) and $uuid ne "' + instance.$uuid + '"';
