...
function _processLdapGroups(_, user, ldapGroupNames, _track) {
	ldapGroupNames && ldapGroupNames.forEach_(_, function (_, ldapGroupName) {
		_processLdapGroup(_, user, ldapGroupName, _track);
	})
}

function _processLdapGroup(_, user, ldapGroupName, _track) {	
	var db = adminHelper.getCollaborationOrm(_);
	var entity = db.model.getEntity(_, "group");
	var groups = db.fetchInstances(_, entity, { jsonWhere: { ldapGroup: ldapGroupName } });
	if (groups) groups.forEach_(_, function (_, group) {
        _track && _track(locale.format(module, "msgProcessGroup"),
			locale.format(module, "msgGroupFound", group.description(_)),
			null);
		...
		group.endPoints(_).toArray(_).forEach_(_, function (_, endpoint) {
			...
			if (ues.length > 0) {
				ue = ues[0];
				_track && _track(null, locale.format(module, "msgUserExist", ue.login), null);
			} else {
				ue = user.endpoints(_).add(_);
				ue.endpoint(_, endpoint);
				_track && _track(null, locale.format(module, "msgCreateUser", ue.login), null);
			}
...
function updateUsers(_, instance, users, diagnoses, del, translateTable, _track) {
	...
	for (i = 0; i < instanceCount; i++) {
		...
		if (!newValues) { // LDAP entry does not exist any more
			...
			user.active(_, true);
			_processLdapGroups(_, user, ldapGroups, _track);
			user.save(_);
			user.getAllDiagnoses(_, diagnoses, {
				addEntityName: true,
				addPropName: true
	...
	for (var authenticationName in users) {
		...
		if (!inst.lastName(_)) {
			...
			_processLdapGroups(_, inst, ldapGroups, _track)
		...
		usersFromLdap: {
			...
			$execute: function (_, context, instance) {
				function _track(phase, detail, progress) {
					...
					if (phase) context.tracker.phase = phase;
					if (detail) context.tracker.phaseDetail = detail;
					if (progress) context.tracker.progress = progress;
				}
				var t = context && context.tracker;
				var diags = t ? (t.$diagnoses = t.$diagnoses || []) : (instance.$diagnoses = instance.$diagnoses || []);
				instance.$diagnoses = instance.$diagnoses || [];
				diags = instance.importUsers(_, _track);
