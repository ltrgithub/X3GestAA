...
function _addDiagnose(tracker, severity, message) {
	tracker && tracker.$diagnoses && tracker.$diagnoses.push({
		$severity: severity,
		$message: message
	});
};
...
function getAllUsers(instance, all, _, attribs) {
	...
	return getAllEntries(instance, all, _, attribs, filter, null);
}
...
function getAllEntries(instance, all, _, attribs, filter, keySearch) {
	...
	try {
		var authNameMapping;
		if (all) {
			authNameMapping = keySearch || instance.authenticationNameMapping(_);
			if (!authNameMapping) {
...
function _processLdapGroups(_, user, ldapGroupNames, tracker) {
	if ((ldapGroupNames) && (ldapGroupNames.length >0)) {
		ldapGroupNames.forEach_(_, function (_, ldapGroupName) {
			_processLdapGroup(_, user, ldapGroupName, tracker);
		})
	} else {
		_addDiagnose(tracker, "warning", locale.format(module, "msgNoGroupInfo", user.login(_)));
	}
}

function _processLdapGroup(_, user, ldapGroupName, tracker) {
	var db = adminHelper.getCollaborationOrm(_);
	var entity = db.model.getEntity(_, "group");
	var groups = db.fetchInstances(_, entity, { jsonWhere: { ldapGroup: ldapGroupName } });
	if (groups) {
		groups.forEach_(_, function (_, group) {
			_addDiagnose(tracker, "info", locale.format(module, "msgGroupFound", group.description(_), ldapGroupName));
			...
			group.endPoints(_).toArray(_).forEach_(_, function (_, endpoint) {
                ...
                ue.getAllDiagnoses(_, tracker.$diagnoses, {
					addEntityName: true,
					addPropName: true
				});
				if (ues.length > 0) {
					_addDiagnose(tracker, "info", locale.format(module, "msgUpdateUser", ue.login(_)));
				} else {
					_addDiagnose(tracker, "info", locale.format(module, "msgCreateUser", ue.login(_)));
				}

			})
		})
	} else {
		_addDiagnose(tracker, "info", locale.format(module, "msgGroupNotFound", ldapGroupName));
	}
}


function _getLdapGroupsFromValue(_, value) {
	if (!value) {
		return [];
	}
	else if (Array.isArray(value)) {
		return value.map_(_, function (_, item) { return item.toString("utf8"); })
	}
	else {
...
function updateUsers(_, instance, users, del, translateTable, tracker) {
	if (!translateTable) translateTable = _makeTranslateTable(instance, null, _);
	if (!translateTable.authenticationName) {
		_addDiagnose(tracker, "error", locale.format(module, "noAuthName"));
		return;
	}
	if (!translateTable.lastName) {
		_addDiagnose(tracker, "error", locale.format(module, "noLastNameMapping"));
		return;
	}
	...
	for (i = 0; i < instanceCount; i++) {
		...
		if ((tracker) && ((i % 200) === 0)) {
			tracker.phase = locale.format(module, "delUpdate");
			tracker.phaseDetail = locale.format(module, "count", i, userCount);
			tracker.progress = 20 + 80 * (i + instanceCount) / (instanceCount + userCount);
		}
		...
		if (!newValues) { // LDAP entry does not exist any more
			...
			for (var key in translateTable) {
				...
				if ((key === "groupName")) {
					ldapGroups = _getLdapGroupsFromValue(_, newValue);
					continue;
				}
			...
			user.active(_, true);
			_processLdapGroups(_, user, ldapGroups, tracker);
			user.save(_);
			user.getAllDiagnoses(_, tracker.diagnoses, {
				addEntityName: true,
				addPropName: true
			});
			users[name][USED] = 1;
	...
	for (var authenticationName in users) {
		if ((tracker) && ((i % 200) === 0)) {
			tracker.phase = locale.format(module, "insert");
			tracker.phaseDetail = locale.format(module, "count", i, userCount);
			tracker.progress = 20 + 80 * (i + instanceCount) / (instanceCount + userCount);
		}
		i++;
		...
		users[authenticationName] = undefined;
		if (!inst.lastName(_)) {
			tracker && tracker.addWarning(locale.format(module, "noLastName", authenticationName));
			inst.login(_, "");
			inst.deleteSelf(_);
		} else {
			...
			_processLdapGroups(_, inst, ldapGroups, tracker)
	...
	tracker && (tracker.phase = locale.format(module, "finished"));
	tracker && (tracker.phaseDetail = "");
	tracker && (tracker.progress = 100);
...
exports.entity = {
	...
	$functions: {
		...
		importUsers: function (_, tracker) {
			...
			updateUsers(_, instance, allUsers, false, translateTable, tracker);
	...
	$services: {
		...
		usersFromLdap: {
			...
			$execute: function (_, context, instance) {
				...
				if (tracker) tracker.$diagnoses = tracker.$diagnoses || [];
				instance.$diagnoses = instance.$diagnoses || [];
				instance.importUsers(_, tracker);
			}
		}
	},
	$searchIndex: {
