...
function getAllUsers(instance, all, _, attribs, diags) {
	...
	var ldapClient = require('syracuse-ldap').bindClient(clientOpts, instance.adminDn(_), instance.adminPassword(_), _);
	// console.log("bound");
	try {
		var authNameMapping;
		if (all) {
	...
	$services: {
		connectionTest: {
			...
			$execute: function(_, context, instance) {
				...
				var ldapClient;
				try {
					ldapClient = require('syracuse-ldap').bindClient(clientOpts, instance.adminDn(_), instance.adminPassword(_), _);
				} catch (e) {
					instance.$addError(locale.format(module, "connError", e.toString()));
					return;
				} finally {
					try {
						if (ldapClient) ldapClient.unbind();
					} catch (e) {
						instance.$addError(locale.format(module, "unbindError", e));
						return;
					}
				}
		...
		usersFromLdap: {
			...
			$execute: function(_, context, instance) {
				...
				if (userEntity.noAdmin(_)) {
					diags.$addError(locale.format(module, "noAdmin"));
					return;
				}
				...
				var allUsers = getAllUsers(instance, true, _, attribs, diags);
