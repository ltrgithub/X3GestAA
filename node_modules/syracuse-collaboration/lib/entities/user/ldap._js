	...
	$properties: {
		...
		searchFilter: {
			...
			$compute: function(_, instance) {
				return "(" + instance.authenticationNameMapping(_) + "={{username}})";
			}
		},
		syncSearchFilter: {
		...
		onlyKnownGroups: {
			$title: "Users belongs to known groups",
			$description: "Users belongs to LDAP groups linked to X3 groups",
			$type: "boolean",
			$default: false
		},
		globalSearchFilter: {
			...
			$isHidden: function(_, instance) {
				return !instance.onlyKnownGroups(_);
			},
			$compute: function(_, instance) {
				var groupFilter;
		...
		groupSearchFilter: {
			...
			$description: "Group search filter is used only for helping to link x3 groups with LDAP groups (LDAP group name lookup). It is not related to users syncronization.",
			$isMandatory: false
		},
