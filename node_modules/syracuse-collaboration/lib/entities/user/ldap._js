...
///  all: retrieve all users? If false, retrieve only one user (using searchFilter if syncSearchFilter is not set)
...
///   and the search filter is the sync search filter (if available) or the search filter with {{username}} replaced by "*"
	...
	// console.log("bound");
	try {
		var res = _ldapSearch(ldapClient, instance.authenticationNameMapping(_), {filter: filter, base: instance.searchBase(_)}, all ? 0 : 1, _);
		return res;
	} finally {
		ldapClient.unbind(_);
	}
...
function _ldapSearchInt(userEntries, ldapClient, loginKey, config,  cookie, size, cb) {
	...
	ldapClient.search(config.base, opts, control, function(error, res) {
		...
		res.on('searchEntry', function(entry) {
			...
			if (loginKey) {
				var key = obj[loginKey].toString("utf8");
				userEntries[key] = obj; 					
			} else {
				userEntries[""] = obj;
			}
