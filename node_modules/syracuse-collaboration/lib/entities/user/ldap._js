...
function updateUsers(_, instance, users, diagnoses, del) {
	var authMapping = instance.authenticationNameMapping(_);
	var lastNameMapping = instance.lastNameMapping(_);
	if (!authMapping) {
		diagnoses.push({
			severity: "error",
			message: locale.format(module, "noAuthNameMapping")
		});
		return;
	}
	if (!lastNameMapping) {
	...
	var translateTable = {
		authenticationName: authMapping,
	...
	for (var authenticationName in cloned) {
		...
		if (!inst.lastName(_)) {
			...
			for (var i = 0; i < disturbingUsers.length; i++) {
				...
				disturbingNames[disturbingUsers[i].login(_).toLowerCase()] = 1;
			}
			if (authenticationName.toLowerCase() in disturbingNames) {
				...
				for (var i = 1; i <= disturbingUsers.length + 1; i++) {
					if ((authenticationName + i).toLowerCase() in disturbingNames) continue;
