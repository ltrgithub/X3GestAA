		...
		groups.forEach_(_, function(_, group) {
			...
			group.endPoints(_).toArray(_).forEach_(_, function(_, endpoint) {
				...
				try {
					...
					if (ues.length > 0) {
						instance.$addDiagnose("info", locale.format(module, "msgUpdateUser", ue.login(_)));
					} else {
						instance.$addDiagnose("info", locale.format(module, "msgCreateUser", ue.login(_)));
					}
				} catch (error) {
					instance.$addDiagnose("warning", locale.format(module, "msgErrorCreateX3User", user.login(_)));
					if (ues.length = 0) ue.deleteSelf(_);
				}
			});
		});
	} else {
