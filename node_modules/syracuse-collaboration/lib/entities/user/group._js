...
exports.entity = {
	...
	$functions: {
		setUserSecurityLevel: function(_) {
			console.log("Group after save");
			this.users(_).toArray(_).forEach_(_, function(_, user) {
	...
	$events: {
		...
		$afterSave: [
			//propagate security level to user 
			function(_, instance) {
				if (instance.isDirty("role")) instance.setUserSecurityLevel(_);
			},
			function(_, instance) {
				instance.$diagnoses = instance.$diagnoses || [];
