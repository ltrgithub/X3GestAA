	...
	$events: {
		$canSave: [
			...
			function(_, instance) {
				...
				var prev = instance.$snapshot.role(_) && instance.$snapshot.role(_).securityProfile(_) ? instance.$snapshot.role(_).securityProfile(_).canSaveSecLevel(_, instance) : true;
				return curr && prev;
			}
		],
		$beforeSave: [
