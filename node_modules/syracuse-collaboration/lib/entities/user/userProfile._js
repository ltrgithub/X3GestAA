...
function _getDefaultTheme(_, themes) {
	var defaultTheme = null;
	for (var i = 0; i < themes.toArray(_).length && !defaultTheme; i++) {
		if (themes.toArray(_)[i].default(_))
			defaultTheme = themes.toArray(_)[i];
	}
	return defaultTheme;
}
...
exports.entity = {
	...
	$properties: {
		...
		authoringLevel: {
			...
			$compute: function(_, instance) {
				...
				return (sp && sp.authoringLevel(_)) || (((globals.context.config || {}).enableIndusFeatures) ? "sage" : "none");
			}
		},
		sitePreferences: {
	...
	$functions: {
		...
		loadUserProfile: function(_, user, defaultLocale, paramUserProfile) {
			...
			self.selectedTheme(_, userPrefs && userPrefs.lastTheme(_) || _getDefaultTheme(_, self.themes(_)));
