...
exports.entity = {
	...
	$functions: {
		...
		getFunctionsAuthorizations: function(_, endpoint) {
			var up = this;
			...
			return up._fctRights[selEp.dataset(_)] = selEp.getAuthorizedFunctions(_, up.user(_), up.selectedRole(_));
		},
		getAccessCodeAuthorizations: function(_, endpoint) {
			var up = this;
			var selEp = endpoint || up.selectedEndpoint(_);
			if (!selEp) return null;
			// cache management
			if (up._accessRights && up._accessRights[selEp.dataset(_)]) return up._accessRights[selEp.dataset(_)];
			up._accessRights = up._accessRights || {};
			return up._accessRights[selEp.dataset(_)] = selEp.getAuthorizedAccessCode(_, up.user(_), up.selectedRole(_));
		}
	},
	$services: {
		current: {
