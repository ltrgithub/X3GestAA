...
exports.entity = {
			...
			$compute: function(_, instance) {
				...
				if (userPrefs) {
					...
					if (selEp && appConn) {
						...
						if (data != null && globals.context.session.appConnection && globals.context.session.appConnection[dataset]) {
							data = JSON.parse(data);
							forEachKey(globals.context.session.appConnection[dataset], function(k, val) {
								data[k] = val;
							});
							data = JSON.stringify(data);
						}
						console.log("Computed app conn data: " + JSON.stringify(data));
						return data;
					}
				}
				return;
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			userPrefs.setAppConnection(_, this.selectedEndpoint(_), this.applicationConnection(_));
	...
	$actions: {
		$resetConnection: function(_, instance) {
			...
			if (selEp) {
				...
				var dataset = selEp.dataset(_);
				if (globals.context.session.appConnection && globals.context.session.appConnection[dataset] && globals.context.session.appConnection[dataset].$resetConnection) {
					...
					r.$isRequested = true;
					delete globals.context.session.appConnection[dataset].$resetConnection;
				}
			}
			return r;
		}
