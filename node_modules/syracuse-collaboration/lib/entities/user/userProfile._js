...
var page = require("syracuse-collaboration/lib/entities/page/page");
var syracuse;
...
function _changeProductName(_, instance, val) {
	var ed;
	if (!val) {
	...
	if (ed) {
		...
		var productNameListByland = adminHelper.getProductNameByEndpoint(ed.dataset(_));
...
exports.entity = {
	...
	$properties: {
		...
		gaContributor: {
			$title: "Google Analytic contributor",
			$type: "boolean",
			$compute: function(_, instance) {
				return true;
			}
		...
		selectedEndpoint: {
			...
			$propagate: function(_, instance, val) {
				...
				if (val && instance) {
					...
					var selectedEpId = val && val.$uuid;
					var adminEp = adminHelper.getCollaborationEndpoint(_);
					if (adminEp && selectedEpId !== adminEp.$uuid)
						instance.getAccessRightAuthorizations(_, val); // the connection will updat the productName of the instance
					// reset the product name regarding the map define in helper that is updated on each syracuse connection
					_changeProductName(_, instance, val);
				}
			}
		},
		endpoints: {
	...
	$functions: {
		...
		loadUserProfile: function(_, user, defaultLocale, paramUserProfile) {
			...
			// set selected role
			self.selectedRole(_, _pickFirst(_, [extUp.role, userPrefs && userPrefs.lastRole(_) && userPrefs.lastRole(_).$uuid], self.roles(_), true));
			// set endpoints
			_setEndpoints(_, this, this.selectedRole(_), extUp.ep);
			// locales
			var localeCode = extUp.loc || (userPrefs && userPrefs.lastLocaleCode(_)) || defaultLocale;
		...
		$save: function(_, saveRes) {
			...
			// collect stats
			var updPrefs = this.sitePreferences(_);
			var curPrefs = userPrefs.sitePreferences(_);
			var curSelLp = curPrefs && curPrefs.desktop && curPrefs.desktop.landingpage && curPrefs.desktop.landingpage.$selected;
			var updSelLp = updPrefs && updPrefs.desktop && updPrefs.desktop.landingpage && updPrefs.desktop.landingpage.$selected;
			if (this.gaContributor(_) && curSelLp && updSelLp && curSelLp !== updSelLp) {
				page.collectLandingStat(_, globals.context, this, {
					entity: "landingPage",
					$uuid: updSelLp
				}, selEndpoint);
			}
