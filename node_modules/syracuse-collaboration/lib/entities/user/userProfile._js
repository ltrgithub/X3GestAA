...
exports.entity = {
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			if (!oldRole || !newRole || (oldRole.$uuid !== newRole.$uuid && _badgeSet(_, oldRole) !== _badgeSet(_, newRole)) || (lastEndpoint && (!selEndpoint || lastEndpoint.dataset(_) !== selEndpoint.dataset(_)))) {
				...
				globals.context.request.session && globals.context.request.session.disconnectAllX3Sessions(_, "Endpoint/role change");
			}
			...
			var newAppConnection = this.applicationConnection(_);
			userPrefs.setAppConnection(_, this.selectedEndpoint(_), newAppConnection.proto(_), newAppConnection.data(_));
			userPrefs.save(_);
			// location
	...
	$links: function(_, instance) {
		...
		//
		var appConn = instance.applicationConnection(_);
		if (appConn) {
			lnks.$applicationConnection = {
				$url: appConn.genEditLink(_, instance.selectedEndpoint(_)),
				$method: "GET"
			};
		}
		//
