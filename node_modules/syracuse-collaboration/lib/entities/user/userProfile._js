...
function _fromCookie() {
	try {
		...
		if (cookie) cookie = helpers.http.parseCookie(cookie)["profile"];
...
function _pickFirst(_, candidates, validateList) {
	var res = null;
	candidates.some_(_, function(_, cId) {
		return cId && (res = validateList.get(_, cId));
	});
	if (!res) res = validateList && validateList.toArray(_)[0];
	return res;
}
...
function _setEndpoints(_, userProfile, role, cookieEpId) {
	...
	if (cookieEpId && (cookieEpId !== selEp.$uuid)) selEp = null;
...
exports.entity = {
	...
	$functions: {
		...
		loadUserProfile: function(_, user, defaultLocale) {
			...
			// set selected role
			self.selectedRole(_, _pickFirst(_, [cookieParams.role, userPrefs && userPrefs.lastRole(_) && userPrefs.lastRole(_).$uuid], self.roles(_)));
			// set endpoints
