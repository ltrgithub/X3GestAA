...
exports.entity = {
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			if (!oldRole || !newRole || oldRole.$uuid !== newRole.$uuid || (lastEndpoint && (!selEndpoint || lastEndpoint.dataset(_) !== selEndpoint.dataset(_)))) {
				syracuse = syracuse || require("syracuse-main/lib/syracuse");
				if (selEndpoint.protocol(_) !== "syracuse") {
					var liz = check.checkConcurrent(globals.context.session, newRole, null, _, syracuse.getDevice(), this.$diagnoses);
					if (!liz) {
