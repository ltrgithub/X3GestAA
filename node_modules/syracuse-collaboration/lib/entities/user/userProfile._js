	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			try {
				...
				console.error("LIZENSIERTxxx");
			} catch (e) {
				console.error("FFFFF" + e.stack);
			}
			console.error("DIAGDDD " + util.format(self.$diagnoses));
			if (!userPrefs) {
				// create prefs
		...
		getFunctionsAuthorizations: function(_) {
			var up = this;
			if (!up.selectedEndpoint(_)) return null;
			var selEp = up.selectedEndpoint(_);
			// cache management
