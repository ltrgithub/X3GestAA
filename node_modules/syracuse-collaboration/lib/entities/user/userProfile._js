...
function _setEndpoints(_, userProfile, role) {
	...
	if (userProfile.selectedEndpoint(_) && userProfile.endpoints(_).get(_, userProfile.selectedEndpoint(_).$uuid)) {
		...
		if (userPrefs && userPrefs.lastEndpoint(_) && userProfile.endpoints(_).get(_, userPrefs.lastEndpoint(_).$uuid)) userProfile.selectedEndpoint(_, userPrefs.lastEndpoint(_));
		else if (userProfile.endpoints(_).getLength() == 1) userProfile.selectedEndpoint(_, userProfile.endpoints(_).toArray(_)[0]);
	}
}
...
exports.entity = {
	...
	$functions: {
		loadUserProfile: function(_, user, defaultLocale) {
			...
			// locales
			var localeCode = (userPrefs && userPrefs.lastLocaleCode(_)) || defaultLocale;
