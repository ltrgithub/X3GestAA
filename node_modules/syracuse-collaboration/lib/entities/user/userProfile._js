...
function _setEndpoints(_, userProfile, role, cookieEpId) {
	...
	if (cookieEpId && selEp && (cookieEpId !== selEp.$uuid)) {
		selEp = userProfile.endpoints(_).get(_, cookieEpId);
	}
	...
	if (!userProfile.selectedEndpoint(_) && (endpoints.getLength() === 1)) {
...
exports.entity = {
	...
	$properties: {
		...
		authoringLevel: {
			...
			$compute: function(_, instance) {
				...
				return (sp && sp.authoringLevel(_)) || ((((globals.context.config || {}).system || {}).enableDevelopmentFeatures) ? "sage" : "none");
			}
		},
		sitePreferences: {
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			var liz = check.checkConcurrent(_, globals.context.session, this.selectedRole(_), null, globals.context.session.device, this.$diagnoses);
			if (!liz) {
			...
			if (!oldRole || !newRole || oldRole.$uuid !== newRole.$uuid) {
			...
			if (!disconnected && lastEndpoint && (!selEndpoint || lastEndpoint.dataset(_) !== selEndpoint.dataset(_))) {
