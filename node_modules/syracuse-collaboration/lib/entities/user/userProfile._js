...
exports.entity = {
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			saveRes.$links.$location.$url = "?representation=home.$landing&role={$role}";
			//
			if (globals.context.session) globals.context.session.userProfileCookie = self.toCookie(_);
			// diags
		...
		getAccessRightAuthorizations: function(_, endpoint) {
			...
			if (up._fctRights[selEp.dataset(_)]) {
				up._fctRights[selEp.dataset(_)].notPurgeCache = true;
				return up._fctRights[selEp.dataset(_)];
			}
