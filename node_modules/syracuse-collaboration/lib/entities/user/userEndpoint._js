...
exports.entity = {
	...
	$functions: {
		...
		_getNewX3UserCode: function(_, code) {
			var self = this;
			var x3user;
			// first try with the given code
			if (code) {
				x3user = self.getX3User(_, code);
				if (!x3user) return code;
			}
			// if user exist generate a random code
			do {
				code = _getRandomCode(_);
				x3user = self.getX3User(_, code);
			}
			while (x3user);
			return code;
		...
		computeLogin: function(_, length, withCode) {
			var self = this;
			var result = self.user(_).login(_).replace(/[^A-Za-z0-9]+/g, '')
				.substr(0, length);
			if (withCode) result = result.concat('-', self.code(_));
			return result.toUpperCase();
		},
		updateX3User: function(_, professionCode, auto) {
			var self = this;
			var endpoint = self.endpoint(_);
			var orm = endpoint.getOrm(_);
			var ent = orm.getEntity(_, "ASYRAUS");
			...
			if (!auto && x3user) {
				self.$addError(locale.format(module, "userExists", self.code(_)));
				return;
			}
			...
			var code;
			if (!self.code(_)) {
				code = self.computeLogin(_, 5, false);
				self.code(_, self._getNewX3UserCode(_, code));
			} else {
				code = self.code(_);
			}
			...
			if (!self.login(_)) {
				if (code == self.code(_)) {
					login = self.computeLogin(_, 20, false);
				} else {
					login = self.computeLogin(_, 14, true);
				}
			} else {
				login = self.login(_);
			}
	...
	$services: {
		createUser: {
			...
			$execute: function(_, context, instance, parameters) {
				instance.updateX3User(_, parameters.$select, false);
			}
		}
	}
};