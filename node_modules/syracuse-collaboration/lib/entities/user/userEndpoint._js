	...
	$functions: {
		...
		getX3User: function(_, code) {
			...
			if (code) {
				...
				self._orm = self._orm || endpoint.getOrm(_);
				self._ent = self._ent || self._orm.getEntity(_, "ASYRAUS");
				x3user = self._orm.fetchInstance(_, self._ent, {
					jsonWhere: {
						USR: code
					}
				});
		...
		updateX3User: function(_, professionCode) {
			...
			var login = self.login(_) || self.user(_).login(_).replace(/[^A-Za-z 0-9]+/g, '')
				.substr(0, 14)
			...
			var newUser = null;
			try {
				newUser = x3user || self._ent.createInstance(_, self._orm, null);
				newUser.USR(_, self.code(_));
				newUser.LOGIN(_, login);
				newUser.ENAFLG(_, self.user(_).active(_));
				newUser.ADDEML(_, self.user(_).email(_));
				newUser.FIRSTNAME(_, self.user(_).firstName(_));
				newUser.LASTNAME(_, self.user(_).lastName(_));
				...
				self.$addDiagnose("warning", locale.format(module, "msgErrorCreateX3User", login));
			}
