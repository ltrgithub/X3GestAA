...
exports.entity = {
	...
	$properties: {
		login: {
			$title: "User Login",
			$pattern: "^[A-Za-z0-9_]*$",
			$patternMessage: "X3 login name can only contain A to Z and 0 to 9 and underscore",
			$isMandatory: true,
			$displayLength: 10,
			$pattern: function(_, instance) {
				...
				return config.adminUserRestrict ? "^((?!" + regexParts.join('|') + "$).)([A-Za-z0-9])*$" : "^[A-Za-z0-9]*$";
			},
			$patternModifiers: "i",
			$patternMessage: !config.adminUserRestrict ? "X3 login name can only contain A to Z and 0 to 9" : "X3 login name can only contain A to Z and 0 to 9, and Sage factory users are protected"
		}
	},
