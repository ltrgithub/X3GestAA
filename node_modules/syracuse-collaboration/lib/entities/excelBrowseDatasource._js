		...
		computeServiceUrl: function(_) {
			if(this.endpoint(_) && this.entity(_) && this.representation(_)) {
				var params = ["representation=" + this.representation(_) + ".$query", 
					"count="+excelHelpers.defines.defaultFetchCount];
				var orderBys = this.orderBys(_).toArray(_);
				if(orderBys.length)
					params.push("orderBy=" + encodeURIComponent(orderBys.map_(_, function(_, orderBy) {
						return orderBy.name(_) + " " + orderBy.order(_);
					}).join(",")));
