	...
	$functions: {
		...
		computeServiceUrl: function(_) {
			if(this.endpoint(_) && this.entity(_) && this.representation(_))
				this.serviceUrl(_, ["", "sdata", this.endpoint(_).applicationRef(_).application(_), 
					this.endpoint(_).applicationRef(_).contract(_), 
					this.endpoint(_).dataset(_), 
					this.entity(_)].join("/") + "?representation=" + this.representation(_) + ".$query" + "&count="+excelHelpers.defines.defaultFetchCount);
