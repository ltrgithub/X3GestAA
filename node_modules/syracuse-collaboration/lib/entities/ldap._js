...
function getAllUsers(instance, translate, _) {
	...
	try {
	    // !!! _data is not OK
		var res = _ldapSearch(ldapClient, translate, instance._data, _);
		return res;
	} finally {
		ldapClient.unbind(_);
	}
...
function _ldapSearch(ldapClient, translate, config,  cb) {
		// search result: 
		var userEntries = {};
...
function updateUsers(_, instance, translate, users, diagnoses, del) {
...
exports.entity = {
	...
	$services: {
			...
			var translate = getTranslationTable(_, instance);
			var allUsers = getAllUsers(instance, translate, _);
			var diagnoses = [];
			updateUsers(_, instance, translate, allUsers, diagnoses, false);
