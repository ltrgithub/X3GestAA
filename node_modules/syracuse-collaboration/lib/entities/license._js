	...
	$services: {
		testFunction: {
			...
			$execute : function(_, context, instance) {
				...
				console.log("Test1");
				check.psi(_, "hugo", "get", "/licensetest", null)
			...
			$execute: function(_, context, instance) {
				...
				if (!content) {
					if (!fs.existsSync(instance.path(_))) {
						instance.$diagnoses.push({severity: "error", message: locale.format(module, "noLicense")})
						return;
					}
				...
				try {
					...
					if (instance.$diagnoses.length === 0) instance.$diagnoses.push({severity: "info", message: locale.format(module, "OK")})
				} catch (e) {
					console.log(e.stack)
