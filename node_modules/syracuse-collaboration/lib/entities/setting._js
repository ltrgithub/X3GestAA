	...
	$events: {
		$beforeSave: [
			...
			function(_, instance) {
				...
				if (!settingsOld) throw new Error("No global settings");
				...
				if (oldAuth != auth) {
					if (auth === "sage-id" || auth === "oauth2" || auth === "saml2") { // find duplicates
						...
						var cursor = instance._db.createCursor(_, instance._db.getEntity(_, "user"), {
							sdataWhere: 'authentication eq "" or authentication eq "' + auth + '"'
						});
						var inst;
						instance.$diagnoses = instance.$diagnoses || [];
					...
					} else { // just test admin user
						var user = instance._db.fetchInstance(_, instance._db.getEntity(_, "user"), {
							jsonWhere: {
								login: "admin",
								authentication: ""
							}
						});
						if (user && !_setAdminAuthentication(_, instance, user, settingsOld)) return false;
					}
