				...
				$execute : function(_, context, instance) {
					...
					try {
						...
						if (cl.fileExists(_)) {
							...
							if (!content) {
								instance.$diagnoses.push({severity: "warning", message: locale.format(module, "emptyPatchFile")})
								setLock(_, false, settings);
								return;
							}
							result = integrate.clusterPatch("-", content, {tryagain: true}, _);
						} else {
							if (instance.patchFile(_)) {
								result = integrate.clusterPatch(instance.patchFile(_), null, {tryagain:true}, _);								
							} else {
								instance.$diagnoses.push({severity: "warning", message: locale.format(module, "noPatchFile")})
								setLock(_, false, settings);
								return;
							}
						}
						...
						instance.$diagnoses.push({severity: "info", message: locale.format(module, "stopSession")});						
					} catch (e) {
						console.log("Error "+e+" "+e.stack+" "+e.trace);
			...
			$integrityCheck: {
				...
				$execute : function(_, context, instance) {
					...
					try {
						...
						var ownStatusFuture = patchtools.checkChecksumsAll();
						var foreignStatus;
						var statusValues = [];
						if (config.mockServer) {
							// get status of other servers
							var options = { path: "/nannyCommand/notifyOtherNannies/notifyOne/consistency", method: "POST"};
						...
						statusValues.push({ message: ownStatusFuture(_)});
						...
						console.error("ERROR "+e.stack+" "+e.trace)
						instance.$diagnoses.push({severity: "error", message: e});
					} finally {
