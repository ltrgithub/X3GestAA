...
exports.entity = {
	...
	$valueTemplate: "{name}",
	$helpPage: "Administration-reference-HRM-Sites",
	$properties: {
		name: {
			$title: "Name",
			$isMandatory: true,
		...
		secure: {
			$title: "SSL",
			$description: "Only SSL access is allowed",
			$type: "boolean",
			$default: false
		},
		solution: {
			...
			$compute: function(_, instance) {
				return instance.endpoint(_) && instance.endpoint(_).x3SolutionName(_);
			}
		},
		folder: {
			$title: "Folder",
			$compute: function(_, instance) {
				return instance.endpoint(_) && instance.endpoint(_).x3ServerFolder(_);
			}
		}
	},
	$relations: {
	...
	$functions: {
		toString: function(_) {
			return this.solution(_) + "_" + this.folder(_) + "_" + this.name(_);
		},
		install: function(_) {
			...
			function setMenuItem(_) {
				...
				function updateMenuItem(_, inst, code, name, params) {
					...
					if (params) {
						var port = self.secure(_) ? params.https : params.http;
						inst.description(_, site + " (" + solution + "/" + folder + ") - Devmode (" + params.host + ":" + port + ")");
						inst.parameters(_).set(_, createMenuItemParameter(_, "SRVHOST", locale.format(module, "miParamHost"), params.host));
						inst.parameters(_).set(_, createMenuItemParameter(_, "SRVPORT", locale.format(module, "miParamPort"), port));
					} else {
				...
				if (self.devmode(_)) {
					...
					for (var i in arr) {
						if (!(self.secure(_) && !arr[i].sslEnabled(_))) {
							var mName = menuItemCode + "_DEV" + i;
							var sName = site + " (" + arr[i].host(_) + ":" + arr[i].http(_) + ")";
							menuItem = createMenuItem(_, mName, sName, {
								host: arr[i].host(_),
								http: arr[i].http(_),
								sslEnabled: self.secure(_),
								https: arr[i].https(_)
							});
							menuItems.push(menuItem);
						}
			...
			var self = this;
			var solution = self.solution(_);
			var folder = self.folder(_);
			...
			_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "menuItemsAdded", _menuItems.length, ids)));
			//
			_menuBlock = setMenuBlock(_, _menuItems);
			_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "menuBlockAdded", _menuBlock.code(_))));
			//
			_menuModule = setMenuModule(_, [_menuBlock]);
			_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "menuModuleAdded", _menuModule.code(_))));
			if (!homePage) {
				_diagnoses.push(hrmHelper.formatDiagnosis("warning", locale.format(module, "homePageMissing")));
			} else {
				_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "installOk")));
			}
			return {
				nbItems: _menuItems.length,
		...
		uninstall: function(_) {
			...
			function mustRemoveEpFromBlock(_, menuBlock) {
				if (menuBlock) {
					...
					if (items.getLength() > 0) {
						...
						if (arr.some_(_, function(_, it) {
							return it.endpoint(_).$uuid === ep.$uuid;
						})) return false;
			...
			function mustRemoveEpFromModule(_, menuModule) {
				if (menuModule) {
					...
					if (submodules.getLength() > 0) {
						...
						if (arr.some_(_, function(_, sm) {
							return sm.endpoints(_).toUuidArray(_).indexOf(ep.$uuid) !== -1;
						})) return false;
			...
			if (itemsRemoved.length > 0)
				_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "menuItemsRemoved", itemsRemoved.length, itemsRemoved.join(','))));
			...
			if (mustRemoveEpFromBlock(_, b)) {
				...
				_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "epRemovedFromBlock", ep.description(_), MENU_BLOCK_NAME)));
			}
			...
			if (mustRemoveEpFromModule(_, m)) {
				...
				_diagnoses.push(hrmHelper.formatDiagnosis("info", locale.format(module, "epRemovedFromModule", ep.description(_), MODULE_NAME)));
			}
			return {
		...
		ping: function(_) {
			...
			var db = adminHelper.getCollaborationOrm(_);
			var entity = db.getEntity(_, "menuItem");
			var solution = this.solution(_);
			var folder = this.folder(_);
			var site = this.name(_);

			var menuItemCode = PREFIX_MENU_ITM + solution + "_" + folder + "_" + site;
			var menuItem = hrmHelper.getInstance(_, db, entity, "code", menuItemCode);
			if (!menuItem) {
				_diagnoses.push(hrmHelper.formatDiagnosis("error", locale.format(module, "notPublished")));
				return _diagnoses;
			}

			if (this.servers(_).getLength() > 0) {
				this.servers(_).toArray(_).forEach_(_, function(_, item) {
					_diagnoses = _diagnoses.concat(item.ping(_));
				});
			} else {
				_diagnoses.push(hrmHelper.formatDiagnosis("error", locale.format(module, "noServers")));
			}
			return _diagnoses;
		},
	},
		...
		install: {
			...
			$execute: function(_, context, instance) {
				...
				try {
					...
					instance.$diagnoses.push(hrmHelper.formatDiagnosis("error", e.message, e.stack));
				}
			}
		},
		uninstall: {
