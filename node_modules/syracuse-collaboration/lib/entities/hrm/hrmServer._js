	...
	$functions: {
		...
		isServerRunning: function(_) {
			try {
				var options = {
					method: "GET",
					url: "http://" + this.host(_) + ":" + this.port(_) + "/xtend/sitedispmenu.jsp",
					headers: {}
				};
		...
		ping: function(_) {
			...
			if (!this.banned(_)) {
				if (this.isServerRunning(_)) {
					_diagnoses.push({
						$severity: "info",
						$message: locale.format(module, "pingOk", this.toString(_))
					});
				} else {
					_diagnoses.push({
						$severity: "error",
						$message: locale.format(module, "pingFailed", this.toString(_))
					});
				}
			} else {
				_diagnoses.push({
					$severity: "error",
					$message: locale.format(module, "serverBanned", this.toString(_))
				});
			}
			return _diagnoses;
		},
