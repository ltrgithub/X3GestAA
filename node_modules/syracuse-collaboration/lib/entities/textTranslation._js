...
exports.entity = {
	...
	$services: {
	    ...
	    fromX3toSyracuse: {
			...
			$invocationMode: "async",
			$execute : function(_, context, instance) {
				//console.log("fromX3toSyracuse ") ;
				var opt = context.parameters.parameters && jsurl.parse(context.parameters.parameters);
				console.log("call updateResources ") ;
		...
		fromSyracuseToX3Admin: {
			$method : "GET",
			$isMethod : true,
			$title : "admin ressources, Syracuse -> X3",
			$invocationMode: "async",
			$type: "application/x-translation",
			/*$parameters: {
				$url: "{$baseUrl}/selectExportTargets/$template/$workingCopies?representation=selectExportTarget.$edit&role={$role}",
			...
			$execute : function(_, context, instance) {
				...
				instance.$diagnoses = instance.$diagnoses || [];
				opt.$diagnoses = instance.$diagnoses;
				translation.extractAdminResources(_, instance, opt ,context);
		...
		fromX3toSyracuseAdmin: {
			$method : "GET",
			$isMethod : true,
			$title : "admin ressources, X3 -> Syracuse",
			$type: "application/x-translation",
			$invocationMode: "async",
			$execute : function(_, context, instance) {
				//console.log("fromX3toSyracuse ") ;
				var opt = context.parameters.parameters && jsurl.parse(context.parameters.parameters);
				console.log("call updateResources ") ;
				translation.updateAdminResources(_, instance, opt ,context);
			}
		},
		deleteAll: {
			$method : "GET",
