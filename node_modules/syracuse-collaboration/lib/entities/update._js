...
exports.entity = {
		...
		fire: function(_, key, parameters) {
			...
			if (session === undefined) {
				...
				var users = db.fetchInstances(_, db.getEntity(_, "user"), {
					jsonWhere: {
						"$uuid": parameters.user
					}
				});

				var roles = db.fetchInstances(_, db.getEntity(_, "role"), {
					jsonWhere: {
						"$uuid": parameters.role
					}
				});

				var locales = db.fetchInstances(_, db.getEntity(_, "localePreference"), {
					jsonWhere: {
						"$uuid": parameters.locale
					}
				});

				var diagnoses = [];
				if (users.length && roles.length && locales.length) {
					session = sessionManager.createBatchSession(_, users[0], roles[0], locales[0], diagnoses);
				}
			}
	...
	$services: {
		...
		unschedule: {
			...
			$execute: function(_, context, instance) {
				var db = adminHelper.getCollaborationOrm(_);
				var events = db.fetchInstances(_, db.getEntity(_, "eventTime"), {
					jsonWhere: {
						"parameters.update": instance.$uuid
					}
				}).forEach_(_, function(_, evt) {
