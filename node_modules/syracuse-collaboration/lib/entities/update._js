	...
	return function(_, context, instance) {
		...
		for (var i = 0; i < patches.length; i++) {
			if (patches[i].type(_).toUpperCase() === "SYRACUSE") {
				...
				// remove this patch from the list of 'X3' patches: 
				patches.splice(i, 1);
			}
		}
		...
		// Set a global status:
		instance.status(_, locale.format(module, "updateSuccess"));
		endpoints.toArray(_, true).forEach_(_, function(_, epToUpdate) {
			...
			folders.forEach_(_, function(_, folder) {
				...
				if (folderStatus !== "success") {
					instance.status(_, locale.format(module, "updateFailed"));
				}
			});
		});
		instance.save(_);
...
function patchSyracuse(_, context, integrate, update, epToUpdate, patch) {
	var patchName = patch.name(_).toUpperCase();
	console.log("patchSyracuse:" + patchName);
	if (!integrate) return;
