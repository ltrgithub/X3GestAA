			...
			if (r.head.status === 200 && r.body.$diagnoses) {
				...
				if (done) {
					...
					try {
						...
						tracer.debug && tracer.debug("read trace:" + JSON.stringify(r.body));
	...
	$services: {
		...
		schedule: {
			...
			$execute: function(_, context, instance, parameters) {
				instance.scheduleUpdate(_, datetime.parse(parameters.executionDate + "T" + parameters.executionTime), true);
				/*				var dtm;
		...
		remoteApply: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				// - folder : folder code
				// - schedule : date time in ISO format
				...
				if (pars.folder) opt.jsonWhere.x3ServerFolder = pars.folder;
				...
				if (pars.schedule) update.scheduleUpdate(_, datetime.parse(pars.schedule), true);
				//
				return {
