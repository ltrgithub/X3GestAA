...
var zipm = require('streamline-zip');
...
var tracer = require('syracuse-core').getTracer('patch');
...
function addDiagnose(context, message, severity) {
	var t = context.tracker;
	severity = severity || "info";
	if (t) {
		tracer.debug && tracer.debug(t.phaseDetail + " - " + message);
		if (message) {
			t.$diagnoses = t.$diagnoses || [];
			var lastDiagnose = t.$diagnoses[t.$diagnoses.length - 1];
			if (lastDiagnose && (lastDiagnose.$message === message) && (lastDiagnose.$severity === severity)) {
				// Same diagnose as the previous ... skip it . This will occur when the patch is being integrated
				return;
			}
			t.$diagnoses.push({
				$severity: severity,
...
function copyPatch(_, context, update, endpoint, patch) {
	...
	addDiagnose(context, locale.format(module, "copyPatchTo", path));
	...
	if (store.fileExists(_)) {
		...
		var patchFiles = new zipm.Unzip(
			...
			function(filename, filecontent, headers, _) {
				...
				addDiagnose(context, locale.format(module, "copyFile", fname));
		...
		addDiagnose(context, locale.format(module, "cleanUp"));
		x3Files
			...
			.forEach_(_, function(_, file) {
				try {
					addDiagnose(context, locale.format(module, "unlinkFile", path, file));
					sadfs.unlink(_, {
						folder: endpoint.x3ServerFolder(_),
						path: path,
						name: file,
		...
		addDiagnose(context, locale.format(module, "copyFilesTo", path));
		var options = {
			folder: endpoint.x3ServerFolder(_),
			path: "",
		};
		path.split('/').forEach_(_, function(_, subdir) {
			options.path += subdir;
			addDiagnose(context, locale.format(module, "mkdir", options.path));
...
function applyUpdate(integrate, options) {
	options = options || {};
	return function(_, context, instance) {
		track(context, "Patch import");
		...
		endpoints.toArray(_, true).forEach_(_, function(_, epToUpdate) {
			fcts.push(updateEndpoint(!_, context, integrate, instance, epToUpdate, patches, options));
		});
...
var patchTypes = {
	X3: function(_, context, integrate, update, epToUpdate, patch, options) {
		var patchName = patch.name(_).toUpperCase();
		addDiagnose(context, locale.format(module, "applyX3Patch", patchName));


		function updateFolder(_, context, integrate, update, epToUpdate, patch, path, folder, options) {
			...
			if (options.forceUpdate) {
				...
				} else if ((update.version(_) === folder.release(_)) && (folder.patch(_) < patch.lastMaintenance(_))) {
					applyPatch = {
						version: update.version(_),
						folder: folder.name(_),
						directory: path,
			...
			if (applyPatch) {
				...
				addDiagnose(context, locale.format(module, "applyX3PatchTo", folder.name(_)));
				if (!options.skipStartBatchServer) {
					...
					for (var i = 0; i < 36000; i++) {
						...
						if (r && r.$diagnoses) {
							...
							r.$diagnoses.forEach(function(diag) {
								...
								addDiagnose(context, diag.$message);
							});
							if (done) {
								folder.status(_, done.$message);
								if (done.$severity === "success")
			...
			if (updateChildren) {
				updateFolders(_, context, integrate, update, epToUpdate, patch, path, options, folder.name(_));
			}
		}

		function updateFolders(_, context, integrate, update, epToUpdate, patch, path, options, parent) {
			parent = parent || "";
			...
			folders.forEach_(_, function(_, folder) {
				if (!folder.history(_) && parent === folder.parent(_)) {
					...
					fcts.push(updateFolder(!_, context, integrate, update, epToUpdate, patch, path, folder, options));
				}
			});
			// Wait for the execution of all futures :
			fcts.forEach_(_, function(_, fct) {
		...
		var patchLocalFilename;
		if (options.localFolder) {
			// The patch is already on the server (in the folder options.localFolder)
			// This will mainly occur when the service is called from the SCM WebServer
			patchLocalFilename = path.join(options.localFolder, patch.path(_));
		} else {
			// First copy the patch on the server :
			patchLocalFilename = copyPatch(_, context, update, epToUpdate.endpoint(_), patch);
		}
		//var path = "PATCH/X3V7_P3/FILPATCH";
		console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");

		// Apply the patch
		updateFolders(_, context, integrate, update, epToUpdate, patch, patchLocalFilename, options);
	...
	IND: function(_, context, integrate, update, epToUpdate, patch, options) {
		var patchName = patch.name(_).toUpperCase();
		addDiagnose(context, locale.format(module, "applyIndPatch", patchName));
...
function startBatchServer(_, context, epToUpdate, reserved) {
	...
	// Stop the server:
	addDiagnose(context, locale.format(module, "stopBatchServer"));
	...
	for (var i = 0; i < 600; i++) {
		...
		addDiagnose(context, locale.format(module, "batchServerStopping"));
	}

	// Start the server in the expected mode
	tracer.debug && tracer.debug("startBatch");
	addDiagnose(context, locale.format(module, "startBatchServer"));
	endpoint.postService(_, "startBatch", {
		flgpatch: (reserved || false)
	});

	status = endpoint.getService(_, "servBatch");
	addDiagnose(context, locale.format(module, "batchServerStarted"));
	tracer.debug && tracer.debug("servBatch:" + JSON.stringify(status));
}
...
function updateEndpoint(_, context, integrate, update, epToUpdate, patches, options) {
	addDiagnose(context, locale.format(module, "updateEndpoint", epToUpdate.endpoint(_).description(_)));
	...
	patches.forEach_(_, function(_, patch) {
		if (options.forceUpdate || patch.apply(_)) {
			var type = patch.type(_);
			type && patchTypes[type] && patchTypes[type](_, context, integrate, update, epToUpdate, patch, options);
		} else {
			addDiagnose(context, locale.format(module, "doesntApplyPatch", patch.name(_).toUpperCase()));
		}
	});
...
exports.entity = {
	...
	$functions: {
		...
		applyUpdate: function(_, context, instance, integrate, options) {
			applyUpdate(integrate, options)(_, context, instance);
		},
		test: function(_, context) {
			for (var i = 0; i < 5; i++) {
				addDiagnose(context, "Import patch", "Essai de trace " + i);
				setTimeout(~_, 1000);
			}
			throw new Error("C'est fini");
		}
	},
	$services: {
		test: {
