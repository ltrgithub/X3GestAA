...
function log2Html(_, title, log) {
	...
	html += '<p><h1>' + title + ' - ' + (new Date()).toISOString() + '</h1></p>' + '\n';
	if (errors.length) {
...
function updateFolder(_, context, integrate, update, epToUpdate, patch, path, folder) {
	...
	if (legislation && (!legislations || legislations.indexOf(legislation) < 0)) {
		updateChildren = true;
		trackMessage(context, "warning", locale.format(module, "legislationWarning", patch.legislation(_), folder.name(_)));
			...
			for (var i = 0; i < 36000; i++) {
				...
				if (r && r.$diagnoses) {
					...
					if (done) {
						...
						var htmlContents = log2Html(_, patch.name(_), traceContents);
						var trace = folder.trace(_);
						if (trace && trace.readAll) htmlContents += trace.readAll(_);
