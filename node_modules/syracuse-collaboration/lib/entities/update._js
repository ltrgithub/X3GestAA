		...
		var patchFiles = new unzip.Unzip(
			...
			function(filename, filecontent, headers, _) {
				try {
					...
					//dbg:console.log("Copy file " + fname+ " typeof :"+(typeof filecontent));
...
var patchTypes = {
	X3: function(_, context, integrate, update, epToUpdate, patch) {
		...
		startBatchServer(_, context, integrate, epToUpdate, true);
		...
		startBatchServer(_, context, integrate, epToUpdate);
...
function startBatchServer(_, context, integrate, epToUpdate, reserved) {
	if (!integrate) return;
	...
	// Stop the server:
	track(context, "Stop the batch server");
