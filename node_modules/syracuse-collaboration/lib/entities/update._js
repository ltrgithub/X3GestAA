...
function copyPatch(_, context, update, endpoint, patch) {
	...
	track(context, locale.format(module, "copyPatchTo", path));

	var sadfs = new SadFsqClient(_, x3server.serverHost(_), x3server.serverPort(_), null, null, false);
	var x3Files = [];
	var refFiles = [];
	try {
		//dbg:console.log("sadfs.readdir:"+[endpoint.x3ServerFolder(_),path].join(','));
	...
	if (store.fileExists(_)) {
		...
		var patchFiles = new unzip.Unzip(
			...
			function(filename, filecontent, headers, _) {
				...
				track(context, locale.format(module, "copyFile", fname));
				...
				for (var i = 0; i < 3; i++) {
					try {
						...
						console.log("sadfs.writeFile(" + fileOptions.path + ") failed");
					} catch (e) {
						console.log("exception in sadfs.writeFile:" + e.stack);
					}
					try {
						sadfs.unlink(_, fileOptions);
					} catch (e) {
						console.log("exception in sadfs.unlink:" + e.stack);
					}
				}
			}, {
		...
		track(context, locale.format(module, "cleanUp"));
		x3Files
			...
			.forEach_(_, function(_, file) {
				try {
					track(context, locale.format(module, "unlinkFile", path, file));
					sadfs.unlink(_, {
						folder: endpoint.x3ServerFolder(_),
						path: path,
						name: file,
		...
		track(context, locale.format(module, "copyFilesTo", path));
		var options = {
			folder: endpoint.x3ServerFolder(_),
			path: "",
		};
		path.split('/').forEach_(_, function(_, subdir) {
			options.path += subdir;
			track(context, locale.format(module, "mkdir", options.path));
...
var patchTypes = {
	X3: function(_, context, integrate, update, epToUpdate, patch) {
		...
		track(context, locale.format(module, "applyX3Patch", patchName));
		...
		function updateFolder(_, context, integrate, update, epToUpdate, patch, path, folder) {
			...
			if (applyPatch) {
				...
				track(context, locale.format(module, "applyX3PatchTo", folder.name(_)));
	...
	IND: function(_, context, integrate, update, epToUpdate, patch) {
		var patchName = patch.name(_).toUpperCase();
		track(context, locale.format(module, "applyIndPatch", patchName));
...
function startBatchServer(_, context, epToUpdate, reserved) {
	...
	// Stop the server:
	track(context, locale.format(module, "stopBatchServer"));
	...
	for (var i = 0; i < 600; i++) {
		...
		track(context, locale.format(module, "batchServerStopping"));
	}

	// Start the server in the expected mode
	tracer.debug && tracer.debug("startBatch");
	track(context, locale.format(module, "startBatchServer"));
	endpoint.postService(_, "startBatch", {
		flgpatch: (reserved || false)
	});

	status = endpoint.getService(_, "servBatch");
	track(context, locale.format(module, "batchServerStarted"));
	tracer.debug && tracer.debug("servBatch:" + JSON.stringify(status));
}
...
function updateEndpoint(_, context, integrate, update, epToUpdate, patches) {
	track(context, locale.format(module, "updateEndpoint", epToUpdate.endpoint(_).description(_)));
	...
	patches.forEach_(_, function(_, patch) {
		if (patch.apply(_)) {
			...
			track(context, locale.format(module, "doesntApplyPatch", patch.name(_).toUpperCase()));
		}
	});
