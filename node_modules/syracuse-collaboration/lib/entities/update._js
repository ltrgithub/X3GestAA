...
function updateTrace(_, context, folder, title, contents, diagnoses) {
	...
	console.log("udpate.createWritableStream options:", options);
	var writer = folder.trace(_).createWritableStream(_, options);
					...
					if (done) {
						try {
							...
							for (var i = 0; i < 3; i++) {
								...
								tracer.debug && tracer.debug("? exists " + r.trace);
								if (sadfs.stat(_, {
									path: r.trace
								}, ["size"]).size > 0) {
									tracer.debug && tracer.debug("read file " + r.trace);
									...
									if (diagnoses.length) {
										...
										diagnoses = diagnoses.sort(function(a, b) {
						...
						if (traceNotFound) {
							...
							done.$message = locale.format(module, "traceNotFound", r.trace);
							trackMessage(context, done.$severity, done.$message);
						}
					...
					new unzip.Unzip(
						...
						function(filename, filecontent, headers, _) {
							try {
								...
								releaseNote = (contents.releaseNote || "").toLowerCase();
