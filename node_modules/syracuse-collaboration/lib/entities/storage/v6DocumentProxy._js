			...
			if (params.what) {
				...
				switch (params.what) {
					...
					case "V7WORD":
						...
						delete params.documentUrl;
						// find V7 document
						var det = httpHelper.decodeDetailSegment(params.docUrl);
						if (det && det.id) {
							var d = self._db.fetchInstance(_, self._db.getEntity(_, "document"), det.id);
							d && self.deleteUrl(_, d.computeUrl());
						}

						var uuid = params.docUrl;
						uuid = uuid.split("documents('");
						if (uuid.length > 1) {
							uuid = uuid[1];
							uuid = uuid.split("')");
							uuid = uuid[0];
