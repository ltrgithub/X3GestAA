...
var notifHelper = require("syracuse-notification/lib/helper");
...
exports.entity = {
	...
	$helpPage: "Administration-reference-Documents",
	$properties: {
		expiration: {
	...
	$relations: {
		...
		teams: {
			...
			$propagate: function(_, instance, val) {
				var _users = [];
				var _admin = val.administrator(_);
				if (_admin) _users.push(_admin.login(_));
				var _members = val.members(_);
				if (_members) {
					_members.toArray(_).forEach_(_, function(_, _m) {
						_users.push(_m.login(_));
					});
				}
				var _authors = val.authors(_);
				if (_authors) {
					_authors.toArray(_).forEach_(_, function(_, _a) {
						_users.push(_a.login(_));
					});
				}
				console.log("USERS: " + JSON.stringify(_users, null, 2));

				var _link = {
					title: instance.description(_),
					url: "/sdata/syracuse/collaboration/syracuse/documents(%27" + instance.$uuid + "%27)/content"
				};
				var users = val.administrator(_);
				notifHelper.sendMessage(_, "message", instance.description(_), "A new document is available for your team " + val.description(_) + "!!!", _users, "info", _link, globals.context.session.getData('userLogin'));
			}
		},
		owner: {
			$title: "Owner",
