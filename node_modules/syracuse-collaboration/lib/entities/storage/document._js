	...
	$relations: {
		volume: {
			...
			$control: function(_, instance, val) {
				...
				if ((instance.volume(_) && !instance.volume(_).isCompatible(_, instance.$snapshot && instance.$snapshot.volume(_))) || !instance.volume(_)) if (instance.content(_) && instance.content(_).fileExists(_)) throw new Error(locale.format(module, "storageTypeChange"));
			},
			$propagate: function(_, instance, val) {
