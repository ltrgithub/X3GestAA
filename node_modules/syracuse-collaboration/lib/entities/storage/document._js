...
exports.entity = {
	...
	$init: function(_, instance) {
		...
		if (params.representation) {
			// add internal tags depending on representation
			var tags = templateDocRepTags[params.representation];
			if (tags) {
				Object.keys(tags).forEach_(_, function(_, tagKey) {
					var tagValue = tags[tagKey];
					var tag = instance.internalTags(_).add(_);
					tag.tagKey(_, tagKey);
					tag.tagValue(_, tagValue.replace(/\{(.*?)\}/g, function(match, p1) {
