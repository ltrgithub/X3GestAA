			...
			$execute: function(_, context, instance) {
				...
				if (errors.length > 0) {
					_sendError(_, context, 400,
						...
						locale.format(module, "errorInvalidPages", [Object.keys(applicationData.$pageErrors).join("\n")]));
				} else {
					// All is fine
...
function _processDashboard(_, helpers, applicationData, dashboard) {
	...
	var output = {
		...
		$vignettes: {},
		$article: _getDashboardArticle(_, helpers, dashboard)
	};
...
function _getDashboardArticle(_, helpers, dashboard) {
	var page;
	try {
		var opts = {
		...
		return page && page.$article;
	} catch (e) {
		throw new Error("Unable to load dashboard article: " + opts.representation, e);
	}
}
