	...
	$services: {
		...
		availableApplications: {
			...
			$execute: function(_, context) {
				...
				var parts = endpoint && endpoint.split(".");
				...
				apps.forEach_(_, function(_, mobileApplication) {
					...
					if (!parts) {
						...
						mobileApplication.endPoints(_).toArray(_).forEach_(_, function(_, ep) {
							if (ep.application(_) === parts[0] && ep.contract(_) === parts[1] && ep.dataset(_) === parts[2]) {
								valid = true;
							}
						});
					}
					if (valid) {
						...
						if (endpoint) {
							$metaDataUrl = $metaDataUrl + "?endpoint=" + endpoint;
						}
						$resources.push({
...
function _getHelpers(_, instance, context) {
	...
	if (globals.context && globals.context.session) {
		...
		if (!endpoint) {
			...
			endpoint = upEndpoint.application(_) + "." + upEndpoint.dataset("_") + "." + upEndpoint.contract("_");
		}
...
function _parseProto(_, helpers, applicationData, prototype, representation, facet, allowedLinks) {
	...
	if ($links) {
		_parseLinks(_, helpers, applicationData, $links, allowedLinks, prototype.$properties.$resources.$item);
	}
