...
exports.entity = {
	...
	$valueTemplate: "{code}",
	$helpPage: "Administration-reference-Personalization-management",
	$properties: {
		code: {
			$title: "Code",
			$isMandatory: true,
	...
	$services: {
		exportPerso: {
			...
			$execute: function(_, context, instance) {
				try {
					...
					opt.tracker = t;
					if (opt && opt.targetType === "download") t.replyLink = "$download";
		...
		downloadExportPerso: {
			$title: "Download exported personalizations",
			$method: "GET",
			$isMethod: true,
			$isHidden: true,
			$type: "application/x-export",
			$execute: function(_, context, instance) {
				...
				return {
					...
					body: instance.exportPersonalizations(_, opt)
				};
			}
		},
		schedule: {
			...
			$execute: function(_, context, instance, parameters) {
				if (!parameters) parameters = context.parameters.parameters && jsurl.parse(context.parameters.parameters);
