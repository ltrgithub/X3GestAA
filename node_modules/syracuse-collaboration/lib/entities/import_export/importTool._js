...
var Unzip = require('streamline-zip/lib/unzip/unzip').Unzip;
...
var _sourceMap = {
	...
	"clientjson": function(_, instance, options) {
		...
		if (!upload.fileExists(_)) {
			...
			var content = upload.createReadableStream(_).read(_, -1).toString("utf8");
	...
	"clientzip": function(_, instance, options) {
		...
		var content = upload.createReadableStream(_).read(_, -1);
		var counter = 0;
		var total = 1;
		var unzip = new Unzip(
			content,
