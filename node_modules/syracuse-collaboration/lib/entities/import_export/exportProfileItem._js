	...
	$properties: {
		...
		contract :{
			...
			$compute: function(_, instance) {
				return instance._parent.contractName(_) || "collaboration";
			}
		...
		application: {
			$isHidden: true,
			$compute: function(_, instance) {
				return instance._parent.applicationName(_)  || "syracuse";
			}
		...
		endpointName: {
			$isHidden: true,
			$compute: function(_, instance) {
				return instance._parent.endpointName(_) || "syracuse";
			}
			//defaultValue : "syracuse"
		},
		standardProfile: {
			...
			$isDisabled: function(_, instance) {
				return (instance.getEntity().$exportProfile == null);
			},
	...
	$functions: {
		...
		getKeyAttributeNames: function(_) {
			...
			if(self.standardProfile(_)) {
				...
				return self.entityKeyAttribute(_).toArray(_).map_(_, function(_, attr) {
					return attr.name(_);
				})
		},
		...
		getStandardProfile: function(_) {
			if(this._stdExportProfile)
				return this._stdExportProfile;
			else {
				var ent = this.getExportedEntity(_);
				return ent.$exportProfile;
			}
		}
	},
	$searchIndex: {
