...
function _fillData(_, context, instanceValue, keyValue) {
	...
	var today = syracuseDate.today().toString();
	for (var key in contents) {
		...
		inst.licenseeStreet(_, license.licensedTo.address.address1);
		...
		inst.valStart(_, syracuseDate.parse(license.validity[0]));
		inst.valEnd(_, syracuseDate.parse(license.validity[1]));
		...
		if (policy) {
			...
			inst.valid(_, (today <= license.validity[1] && today >= license.validity[0]));
			["modules", "badges", "activityCodes", "languages", "legislations", "parameterKits"].forEach_(_, function(_, item) {
				...
				policyParts.forEach_(_, function(_, part) {
					var from = "";
					var to = "";
					var max = "0";
					...
					switch (part.condition) {
						...
						case "always":
							...
							child.valStart(_, syracuseDate.parse(license.validity[0]));
							child.valEnd(_, syracuseDate.parse(license.validity[1]));
							break;
						default:
					...
					if (item === "modules") child.keyFunctions(_, part.keyFunctions.join(" "));
				});
			});
		} else {
			inst.valid(_, false);
