...
memoryStore.createReadableStream = function(_) {
	console.log("MemoryStore.createReadableStream");
	var dump = "";
	var up = globals.context.session && globals.context.session.getUserProfile(_);
	if (up) {
		var endpoint = up.selectedEndpoint(_);
	...
	$properties: {
		...
		cpuGraph: {
			...
			$compute: function(_, instance) {
				return (instance.cpuGraphStore = (instance.cpuGraphStore || new x3Logs.LogStore("image/svg+xml", instance.userName(_))));
			}
		},
		log: {
			$title: "Log",
			...
			$compute: function(_, instance) {
				return (instance.logStore = (instance.logStore || new x3Logs.LogStore("x-trace", instance.userName(_))));
			}
		},
		memoryDump: {
			$title: "Memory Dump",
