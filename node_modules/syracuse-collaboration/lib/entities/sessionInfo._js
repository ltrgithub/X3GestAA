...
exports.entity = {
	...
	$helpPage: "Administration-reference-Session-Infos",
	$properties: {
		sid: {
			$title: "Session id",
			$serialize: false
	...
	$services: {
		disconnect: {
			...
			$execute: function(_, context, instance) {
				...
				var logoutURL = context.url.split("/").splice(0, 3).join("/") + "/logout";
				var resp;
				try {
					resp = streams.httpRequest({
						url: logoutURL,
						method: "POST",
						headers: {
					...
					instance.$addDiagnose("warning", locale.format(module, "errorConnect"), null, null, e.stack);
				}
				//
				if (resp) {
