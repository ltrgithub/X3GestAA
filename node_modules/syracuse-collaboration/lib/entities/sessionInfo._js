...
exports.entity = {
	...
	$properties: {
		...
		serverName: {
			$title: "Server name",
			$default: function() { return os.hostname()+":"+(config.port || "default"); }
		}
	},
	$relations: {
		fusionSessions: {
			$title: "Transition sessions",
			$type: "fusionSessionInfos",
			$isChild: true
		}
	},
	$functions: {
	...
	$services: {
	    disconnect : {
			...
			$execute : function(_, context, instance) {
				sessionManager.deleteSession(_, instance.sid(_));
				context.reply(_, 200, "Disconnected", {location: context.baseUrl + "/sessionInfos?representation=sessionInfo.$query"});
			}
		}
	},
	$defaultOrder: [["sid", true]]
}