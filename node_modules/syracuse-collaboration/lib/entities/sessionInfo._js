...
memoryStore.getStorePath = function(_) {
	try {
		if (this.path === undefined) {
			var up = globals.context.session && globals.context.session.getUserProfile(_);
			if (up) {
				var endpoint = up.selectedEndpoint(_);
				this.path = endpoint.getService(_, "memoryDump").file;
			}
		}
		return this.path;
	} catch (e) {
		console.error("the service memoryDump isn't available");
	}
	return null;
};
...
memoryStore.getProperties = function(_) {
	...
	var path = this.getStorePath(_);
	if (path === null) return properties;
...
memoryStore.createReadableStream = function(_) {
	...
	return ez.devices.buffer.reader(dump);
};
memoryStore.fileExists = function(_) {
	return (this.getStorePath(_) !== null);
};
	...
	$properties: {
		...
		cpuGraph: {
			...
			$compute: function(_, instance) {
				return (instance.cpuGraphStore = (instance.cpuGraphStore || new x3Logs.LogStore(_, "image/svg+xml", instance.userName(_))));
			}
		},
		log: {
			$title: "Log",
			...
			$compute: function(_, instance) {
				return (instance.logStore = (instance.logStore || new x3Logs.LogStore(_, "x-trace", instance.userName(_))));
			}
		},
		memoryDump: {
			$title: "Memory Dump",
			...
			$compute: function(_, instance) {
				return (instance.memoryStore = (instance.memoryStore || new MemoryStore(instance.userName(_))));
			}
		},
		samlNameId: {
			$title: "SAML Name Id"
