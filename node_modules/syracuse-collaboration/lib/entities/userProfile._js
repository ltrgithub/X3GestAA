...
//			else if (user.defaultRole(_)) this.selectedRole(_, user.defaultRole(_));
			else if (this.roles(_).getLength() == 1) this.selectedRole(_, this.roles(_).toArray(_)[0]);
			// set endpoints
			_setEndpoints(_, this, this.selectedRole(_));
			// locales
/*			user.locales(_).toArray(_).forEach_(_, function(_, locale) {
				self.locales(_).set(_, locale);
			});
			*/
			// set selected locale
			// get selected locale code
/*			if (userPrefs && userPrefs.lastLocaleCode(_)) this.selectedLocale(_, user.getUserLocaleByCode(_, userPrefs.lastLocaleCode(_)));
//			else if (user.defaultLocale(_)) this.selectedLocale(_, user.getPersLocaleByCode(_, user.defaultLocale(_).code(_)) || user.defaultLocale(_));
			// if not any default locale, get one from the request header
			if(!this.selectedLocale(_) && defaultLocale) {
				var def = this._db.fetchInstance(_, this._db.model.getEntity("localePreference"), {jsonWhere:{code:{$regex: "^"+defaultLocale, $options:"i"}}});
				def && this.selectedLocale(_, def);
			}*/
		...
		$save: function(_, saveRes) {
			...
			var userPrefs = user.preferences(_);
			if (!check.checkConcurrent(this._context.request.session, this.selectedRole(_), _)) {
				if (userPrefs)
					this.selectedRole(_, userPrefs.lastRole(_)) // restore old role
	    ...
	    $logout : {
			...
			$execute : function(_, context, instance) {
				...
				res.$actions.$logout = {
					...
					$links: {
						$location: {
							// retourner a la page par défaut du serveur
							$url: "/"
						}
					}
				};
