...
var _listSolverMap = {
	...
	x3: function(_, ep, ent, context, parameters) {
		...
		var expr = parameters && filters.jsonToSdata(parameters);
		if(expr)
			context.parameters.where = Parser.parse(expr);
		//
		var params = _fixParameters(context.parameters);
		var ep_db = ep.getOrm(_);
		//
		var repr = ep_db.fetchInstances(_, ep_db.getEntity(_, "ACLAIDX", "$query"), params);
		return repr.map_(_, function(_, r) {
			...
			instance.canSearch(_, r.FLGSEARCH(_));
			instance.application(_, ep.applicationRef(_));
			instance.endpointRef(_, ep);
			return instance;
		});
		...
		entity: {
			...
			$propagate: function(_, instance, val) {
				if(instance._parent && instance._relation) {
					// force propagate
					if(instance._relation.$propagate)
						instance._relation.$propagate(_, instance._parent, instance);
				}
			}
		},
		canSearch: {
