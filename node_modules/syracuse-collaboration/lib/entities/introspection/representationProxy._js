...
//			instance.dataset(_, ep.dataset(_));
	...
	x3: function(_, ep, ent, context) {
		...
		return repr.map_(_, function(_, r) {
			...
			instance.representation(_, r.NAME(_));
			instance.title(_, r.TITLE(_));
			instance.entity(_, r.CLASSE(_));
//			instance.dataset(_, ep.dataset(_));
			instance.application(_, ep.applicationRef(_));
			instance.endpoint(_, ep);
			return instance;
...
//		inst.dataset(_, ep.dataset(_));
	...
	x3: function(_, ep, inst, key) {
		...
		if(r) {
			inst.representation(_, r.NAME(_));
			inst.title(_, r.TITLE(_));
			inst.entity(_, r.CLASSE(_));
//			inst.dataset(_, ep.dataset(_));
			inst.application(_, ep.applicationRef(_));
			inst.endpoint(_, ep);
		}
...
exports.entity = {
	...
	$key: "{application.$uuid}~{endpoint.$uuid}~{representation}",
	$isPersistent: false,
	$canEdit: false,
	$canDelete: false,
	$keyPager: true,
	...
	$defaultOrder: [["representation", true]],
	$searchIndex: {
