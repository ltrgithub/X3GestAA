...
exports.entity = {
	...
	$functions: {
		...
		generateToken: function(_, params) {
			...
			info.locale = up.selectedLocale(_) && up.selectedLocale(_).code(_);
			tokenInst.info(_, info)
			tokenInst.save(_);
			// for deletion by ttl
			tokenInst.schedule(_);
			// create and return the token
			let token = jwt.sign(info, this.secret(_));
			//			console.log("Secret:",this.secret(_));
...
site: SITE1`;
				...
				console.log("Token: " + require('syracuse-4gl-api/lib/connectedApp').getConnectedAppToken(_, instance.clientId(_), tokenParams1));
			}
		}
	},
	$events: {
