...
exports.entity = {
	...
	$functions: {
		...
		_canCreateFactory: function(_) {
			...
			return !this._existsFactoryVariant && (!sp || !sp.authoringLevel(_) || (sp.authoringLevel(_) === "sage") || (sp.authoringLevel(_) === "partner"));
		},
		_canCreateModel: function(_) {
			if (!this._modelRepresentation) return false;
			var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
			return !this._existsModelVariant && (!sp || !sp.authoringLevel(_) || (sp.authoringLevel(_) === "sage") || (sp.authoringLevel(_) === "partner"));
		},
		_canCreateGlobal: function(_) {
			var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
			return !sp || !sp.authoringLevel(_) || (sp.authoringLevel(_) === "sage") || (sp.authoringLevel(_) === "partner") || (sp.authoringLevel(_) === "admin");
		},
