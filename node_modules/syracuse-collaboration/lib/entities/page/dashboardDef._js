								...
								if (ca && application && datasets.length && (appName === ca.application(_))) {
									...
									vignettes.forEach_(_, function(_, vignette) {
										...
										if (portlet && (portlet.type(_) === "$menu")) {
											var mobApp = {};
											var epV = null; // FDB - Darina Todo - Set with vignette's endpoint - epV = vignette.endpoint(_);
											var epP = null; // FDB - Darina Todo - Set with portlet's endpoint - epP = portlet.endpoint(_);
	...
	$functions: {
		selectAllVariants: function(_, preferedVariantId, options) {
			...
			variants.forEach_(_, function(_, v) {
				...
				else if (epId && v.endpoints(_).get(_, epId)) score += endpointScore;
				//
				scores.push({
					$uuid: v.$uuid,
					score: score,
