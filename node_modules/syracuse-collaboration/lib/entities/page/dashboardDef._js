...
var PageHelper = require("./authoringHelpers").PageHelper;
...
exports.entity = {
	...
	$properties: {
		...
		dashboardName: {
			...
			$control: function(_, instance) {
				if(instance.dashboardName(_).indexOf(".") >= 0)
					instance.$addError(locale.format(module, "invalidCaracter", "."), "dashboardName");
			}
		},
		mobile: {
	...
	$services: {
	    mobilePortlets : {
			...
			$execute : function(_, context, instance) {
				...
				var up = null, uendpoints=[];
				if (globals.context && globals.context.session) 
					up = globals.context.session.getUserProfile(_);
				if (up) {
	...
	$functions: {
		selectAllVariants: function(_, userProfile, application, preferedVariantId) {
		...
		$relations: {
			variants: {
				...
				$relations: {
					...
					vignettes: {
						$key: ["allEndpoints", "portlet", "endpoint"],
						$properties: ["allEndpoints"],
						$relations: {
							portlet: {
								$key: "code",
