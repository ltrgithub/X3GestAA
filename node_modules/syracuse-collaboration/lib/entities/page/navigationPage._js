...
exports.entity = {
	...
	$links: function(_, instance) {
		...
		if (!instance.$factory || !sp || !sp.authoringLevel(_) || (sp.hasFactoryRights(_))) res.admin = {
	...
	$events: {
		/*
 * 		// This code is added to address SAM:xxxxxx on Navigation pages
 * 		// To allow user to edit home page again, the S_MOD_ADMIN is added by defaul 
 
		$beforeSave: [

			function(_, instance, params) {
				// Add automatically Admin module to do not have empty navigation page
				var db = instance._db;
				var modAdmin = db.fetchInstance(_, db.getEntity(_, "menuModule"), {
					jsonWhere: {
						code: "S_MOD_ADMIN"
					}
				});
				if (modAdmin) {
					var curMod = instance.modules(_).get(_, modAdmin.$uuid);
					if (!curMod) {
						instance.modules(_).add(_, modAdmin);
					}
				}
			}
		],
