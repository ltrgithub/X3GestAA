...
function _getMenuItem(_, vg, admDb, menuItemEnt, application, filter, options) {
	...
	if (options.canUpdate.menu || men._isCreated) {
				...
				if (self.importVignettes(_)) {
					...
					while (vg = crs.next(_)) {
						if (vg.TYP(_) && _menuTypesMap[vg.TYP(_)]) {
							...
							if (self.setAsFactory(_)) _updateProp(_, mi, "menu", "isFactory", true);
							//
							if (mi._isModified || mi._isCreated) {
								var res = mi.save(_, null, {
									shallowSerialize: true
								});
								var dd = [];
								...
								if (_processDiagsOk(dd)) {
									diags.push({
										$severity: "success",
										$message: mi._isCreated ? locale.format(module, "menuCreated", mi.title(_), mi.code(_)) : locale.format(module, "menuModified", mi.code(_), mi.title(_), "")
									});
								}
							}
						}
