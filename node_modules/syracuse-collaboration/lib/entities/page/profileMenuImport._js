	...
	$functions: {
		importMenuProfile: function(_, diags, tracker) {
			...
			function _makeModule(_, pp, menu) {
				...
				if (mod) {
					if (miscBlock && (miscBlock._isCreated || canUpdate.module) && mod) {
			...
			function _makeSubmodule(_, mod, menu) {
				...
				if (block && (block._isCreated || canUpdate.module) && mod && !mod.submodules(_).get(_, block.$uuid)) {
			...
			function _makeMenuEntry(_, parentBlock, menu, mod) {
				...
				if (mi) {
					...
					if (mi && (isCreated || canUpdate.block) && parentBlock && !parentBlock.items(_).get(_, mi.$uuid)) {
