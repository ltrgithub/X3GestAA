	...
	if (!men) {
		...
		if (options.canUpdate.menu &&
			(!men.factoryOwner || men.factoryOwner === "" ||
				options.$factoryOwner === adminHelper.sageFactoryName ||
...
function getX3FolderLangs(_, db, folderName) {
	var x3folderIsoLangs = [];
	var afolder = db.getEntity(_, "ADOSSIER", "$details");
	var folder = db.fetchInstance(_, afolder, folderName);
	// use _data because ADSALAN(_)
	folder && folder._data && folder._data.ADSALAN.forEach_(_, function(_, f) {
		var iso = locale.defaultIso(f.LAN);
		if (iso && iso.length === 2) iso = locale.longIso(iso);
		if (iso) {
			x3folderIsoLangs.push(iso.substring(0, 3) + iso.substring(3).toUpperCase());
		}
	});
		...
		locales: {
			...
			$lookupFilter: function(_, instance) {
				...
				if (ep) {
					var db = ep.getOrm(_);
					try {
						x3IsoLangs = getX3FolderLangs(_, db, ep.dataset(_));
		...
		importMenuProfile: function(_, diags, tracker) {
			...
			function _makeModule(_, pp, menu) {
				...
				if (mods.length === 1) {
					...
					throw new Error(locale.format(module, "severalModules", codeFilter));
				}
				...
				(menu._children || []).some_(_, function(_, submen) {
					if (submen.FONCTION(_) === "MENU") _makeSubmodule(_, mod, submen);
						...
						if (miscBlocks.length === 1) {
							...
							throw new Error(locale.format(module, "severalBlocks", codeFilter + "_MISC"));
						}
			...
			function _makeSubmodule(_, mod, menu) {
				...
				if (blocks.length === 1) {
					...
					throw new Error(locale.format(module, "severalBlocks", codeFilter));
				}
			...
			function _makeBlock(_, parentBlock, _submoduleBlocks, menu, mod) {
				...
				if (blocksKeys.length === 1) {
					...
					throw new Error(locale.format(module, "severalSubBlocks", codeFilter, parentBlock.code(_)));
				}
			...
			try {
				x3folderIsoLangs = getX3FolderLangs(_, db, self.endpoint(_).dataset(_));
			} catch (e) {
				diags.push({
					$severity: "warning",
					$message: locale.format(module, "repAdossierMissing", self.endpoint(_).dataset(_), e.message)
				});
			}
			...
			var locales = self.allLocales(_) ? admDb.fetchInstances(_, admDb.getEntity(_, "localePreference"), allLocalesFilter) : self.locales(_).toArray(_);
			var canCreate = {};
			var canUpdate = {};
