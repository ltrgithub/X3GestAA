	...
	$functions: {
		importMenuProfile: function(_, diags, tracker) {
			...
			function _makeModule(_, pp, menu) {
				...
				if (self.setAsFactory(_)) _updateProp(_, mod, "module", "isFactory", true);
				...
				(menu._children || []).some_(_, function(_, submen) {
					if (submen.FONCTION(_) === "MENU") _makeSubmodule(_, mod, submen);
						...
						if (self.setAsFactory(_)) _updateProp(_, miscBlock, "submodule", "isFactory", true);
			...
			function _makeSubmodule(_, mod, menu) {
				...
				if (block) {
					...
					if (self.setAsFactory(_)) _updateProp(_, block, "submodule", "isFactory", true);
				}
				//
				(menu._children || []).some_(_, function(_, submen) {
			...
			function _makeBlock(_, parentBlock, _submoduleBlocks, menu, mod) {
				...
				if (block) {
					...
					if (self.setAsFactory(_)) _updateProp(_, block, "block", "isFactory", true);
			...
			function _makeMenuEntry(_, parentBlock, menu, mod) {
				...
				if (mi) {
					if (canUpdate.menu || (isCreated && canCreate.menu)) {
						...
						if (self.setAsFactory(_)) _updateProp(_, mi, "menu", "isFactory", true);
					}
					//
			...
			if (self.pageName(_)) {
				...
				if (!page) {
					...
					if (self.setAsFactory(_)) page.isFactory(_, true);
				}
			}
			//
				...
				if (self.importVignettes(_)) {
					...
					while (vg = crs.next(_)) {
						if (vg.TYP(_) && _menuTypesMap[vg.TYP(_)]) {
							...
							if (mi && self.setAsFactory(_)) _updateProp(_, mi, "menu", "isFactory", true);
							//
