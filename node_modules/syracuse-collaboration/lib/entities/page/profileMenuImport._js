...
function _getMenuItem(_, vg, admDb, menuItemEnt, application, filter, options) {
	...
	// Always try to select the standard menu item with the standard name
	ft.jsonWhere.code = options.standardPrefix + vg.CODVI(_);
	if (application) ft.jsonWhere.application = application.$uuid;
	//
	var men = admDb.fetchInstance(_, menuItemEnt, ft);
	if (!men) {
		// Unable to find the menu item with the standard code, check for
		// a non-standard code
		ft.jsonWhere.code = null;
		men = admDb.fetchInstance(_, menuItemEnt, ft);
...
exports.entity = {
	...
	$functions: {
		importMenuProfile: function(_, diags, tracker) {
				...
				if (mi) {
					if (canUpdate.menu || (isCreated && canCreate.menu)) {
						if (mi.title(_) !== menu.LIBMENU(_)) {
							updateMessage = locale.format(module, "menuModified", mi.code(_), mi.title(_), menu.LIBMENU(_));
			...
			var stdPrefix = "STD_" + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
			var codePrefix = null;
			if (self.setAsFactory(_)) {
				codePrefix = (factoryOwner === "SAGE" ? "STD_" : "ADN_") + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
			} else {
