	...
	$properties: {
		...
		setAsFactory: {
			...
			$isHidden: function(_, instance) {
				...
				var hasFactoryRights = sp && sp.authoringLevel(_) && sp.authoringLevel(_) === "sage";
				if (hasFactoryRights) return false;
				var hasPartnerRights = config && config.enablePartnerFeatures;
				if (hasPartnerRights) return false;
				return true;
			}
		},
		importMenu: {
	...
	$functions: {
		importMenuProfile: function(_, diags, tracker) {
			function _updateFactory(_, inst, authProp) {
				if (inst && (canUpdate[authProp] || (inst._isCreated && canCreate[authProp])) && (inst.isFactory(_) !== true)) {
					inst.isFactory(_, true);
			...
			function _makeModule(_, pp, menu) {
				...
				if (mod && (canUpdate.module || (isCreated && canCreate.module)) && (_getProperty(_, mod, 'title') !== menu.LIBMENU(_))) {
					mod.title(_, menu.LIBMENU(_));
					mod._isModified = true;
				}
				if (self.setAsFactory(_) && hasFactoryRights) _updateFactory(_, mod, "module");
				...
				(menu._children || []).some_(_, function(_, submen) {
					if (submen.FONCTION(_) === "MENU") _makeSubmodule(_, mod, submen);
						...
						if (miscBlock && mod && (canUpdate.submodule || (miscBlock._isCreated && canCreate.submodule)) && (_getProperty(_, miscBlock, 'title') !== mod.title(_))) {
							miscBlock.title(_, mod.title(_));
							miscBlock._isModified = true;
						}
						if (self.setAsFactory(_) && hasFactoryRights) _updateFactory(_, miscBlock, "submodule");
				...
				if (mod) {
					if (miscBlock && miscBlock._isCreated && mod && canUpdate.module) {
			...
			function _makeSubmodule(_, mod, menu) {
				...
				if (block) {
					if ((canUpdate.submodule || (isCreated && canCreate.submodule)) && (_getProperty(_, block, 'title') !== menu.LIBMENU(_))) {
						block.title(_, menu.LIBMENU(_));
						block._isModified = true;
					}
					if (self.setAsFactory(_) && hasFactoryRights) _updateFactory(_, block, "submodule");
				}
				//
				(menu._children || []).some_(_, function(_, submen) {
				...
				if (block && block._isCreated && mod && canUpdate.module && !mod.submodules(_).get(_, block.$uuid)) {
			...
			function _makeBlock(_, parentBlock, _submoduleBlocks, menu, mod) {
				...
				if (block) {
					if ((canUpdate.block || (isCreated && canCreate.block)) && (_getProperty(_, block, 'title') !== menu.LIBMENU(_))) {
						block.title(_, menu.LIBMENU(_));
						block._isModified = true;
					}
					if (self.setAsFactory(_) && hasFactoryRights) _updateFactory(_, block, "block");
					if (parentBlock && canUpdate.block && block._isCreated && !parentBlock.items(_).get(_, block.$uuid)) {
			...
			function _makeMenuEntry(_, parentBlock, menu, mod) {
				...
				if (!miTitle) {
					...
					diags.push({
						$severity: "warning",
						$message: locale.format(module, "codeInsteadTitle", menu.FONCTION(_))
					});
				}
				if (mi) {
					if (canUpdate.menu || (isCreated && canCreate.menu)) {
						if (_getProperty(_, mi, 'title') !== miTitle) {
							...
							updateMessage = locale.format(module, "menuModified", mi.code(_), mi.title(_), miTitle);
							mi.title(_, miTitle);
							mi._isModified = true;
						}
						if (mod && (!mi.module(_) || (mi.module(_).$uuid !== mod.$uuid))) {
							moduleMessage = locale.format(module, "menuModuleModified", mi.code(_));
							mi.module(_, mod);
							mi._isModified = true;
						}
						if (self.setAsFactory(_) && hasFactoryRights) _updateFactory(_, mi, "menu");
					}
					//
					if (mi._isModified || isCreated) {
						...
						if (_processDiagsOk(dd)) {
							if (isCreated) {
								diags.push({
									$severity: "success",
									$message: locale.format(module, "menuCreated", mi.title(_), mi.code(_))
								});
							} else {
					...
					if (mi && mi._isCreated && parentBlock && canUpdate.block && !parentBlock.items(_).get(_, mi.$uuid)) {
			...
			var isHRM = (app.contract(_) === "hrm");
			var codePrefix = null;
			var modPrefix = "STD_" + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
			var blockPrefix = (isHRM ? "STD_" : "") + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
			if (self.setAsFactory(_)) {
				if (hasFactoryRights) {
					codePrefix = "STD_" + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
				} else {
					codePrefix = "VER_" + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
				}
			} else {
				codePrefix = "SPE_" + app.application(_).toUpperCase() + "_" + app.contract(_).toUpperCase() + "_";
			}
			// start
			...
			if (self.pageName(_)) {
				...
				if (!page) {
					...
					if (self.setAsFactory(_) && hasFactoryRights) {
						page.isFactory(_, true);
					}
				}
			}
						...
						if (vg.TYP(_) && _menuTypesMap[vg.TYP(_)]) {
							...
							if (mi && (mi._isModified || mi._isCreated)) {
								...
								if (_processDiagsOk(dd)) {
									diags.push({
										$severity: "success",
										$message: mi._isCreated ? locale.format(module, "menuCreated", mi.title(_), mi.code(_)) : locale.format(module, "menuModified", mi.code(_), mi.title(_), mi.title(_))
									});
								} else {
									diags.push({
										$severity: "error",
										$message: locale.format(module, "menuImportFailure", mi.title(_), mi.code(_))
									});
								}
							}
						}
	...
	$services: {
		import: {
			...
			$execute: function(_, context, instance, params) {
				...
				var diags = tt ? (tt.$diagnoses = tt.$diagnoses || []) : [];
		...
		schedule: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				parameters.$select.forEach_(_, function(_, s) {
					...
					t.description(_, locale.format(module, "menuProfileImportLabel"));
					t.logLevel(_, "error");
