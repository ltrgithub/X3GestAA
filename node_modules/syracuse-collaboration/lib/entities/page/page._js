	...
	return _fromPrototypeCache(_, cacheId, "navigationPage") || _toPrototypeCache(_, cacheId, "navigationPage", (function(_) {
		...
		np.modules(_).toArray(_).forEach_(_, function(_, mm) {
			...
			mm.submodules(_).toArray(_).forEach_(_, function(_, sm) {
				...
				var res_sm = sm.getNavigationPageResource(_, "{$menuBaseUrl}", isAdmin ? admFctRights : selEpFctRights, adminMode);
...
function _getLandingPage(_, context, params) {
	...
	return _fromPrototypeCache(_, cacheId, "landingPage") || _toPrototypeCache(_, cacheId, "landingPage", (function(_) {
		...
		// Prototype additions
		var pageProto = page.$prototype.$properties.rolePages = page.$prototype.$properties.userPages = {
