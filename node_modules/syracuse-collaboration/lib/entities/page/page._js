...
function _computeEtag(variantId, key1, key2) {
	var _etag = "";
	if (!variantId && !key1) {
		_etag += 'no_variant';
	}
...
function _computeNavigationPageEtag(_, context, params) {
	...
	return _computeEtag(null, lastModified.toISOString(), rightsKey || new Date(0));
}
...
/*	var cnt = context.db.getEntity(_, "pageLayoutProxy").getLayoutContentFromId(_, context.db, prefId);
    ...
    */
	...
	page._etag = _computeEtag(variant && variant.$uuid, variantUpdDate && variantUpdDate.toISOString(), fusionUpdDate && fusionUpdDate.toISOString());
	return page;
}
...
/*			var cnt = plProxyEnt.getLayoutContentFromId(_, db, "landingPage." + lp.$uuid);
            ...
            */
            res.stdLayout = "{$baseUrl}/pageLayoutProxies('landingPage." + lp.$uuid + "')?representation=pageLayoutProxy.$details";
            // crnit <<<
            //
