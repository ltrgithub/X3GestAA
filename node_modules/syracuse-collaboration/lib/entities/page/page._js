...
function _getNavigationPage(_, context, params) {
	...
	return _fromPrototypeCache(_, cacheId, "navigationPage") || _toPrototypeCache(_, cacheId, "navigationPage", (function(_) {
		...
		var subModEnt = navModEnt.$relations.submodules.getTargetEntity();
		...
		modProtoLks.$create = navModEnt.getLink("$create", null, "originPage={$uuid}");
		modProtoLks.$select = navPageEnt.$relations.modules.getLink("$select");
		...
		blockProtoLks.$create = subModEnt.getLink("$create", null, "originModule={$uuid}");
		blockProtoLks.$select = navModEnt.$relations.submodules.getLink("$select");
		// 
		//blockProto.$properties.items.$capabilities = "sort,reorder,delete";
		...
		blockItemsProtoLks.$select = subModEnt.$relations.items.getLink("$select");
...
//		var blockItemsMILks = blockItemsMIProto.$links = blockItemsMIProto.$links || {};
//		blockItemsMILks.$select = subModEnt.$relations.items.getLink("$select", "menuItem");
		...
		page.modules = np.modules(_).toArray(_).filter_(_, function(_, mm) {
			...
			return {
				$uuid: mm.$uuid,
				$menuBaseUrl: mmAppId === adminApp.$uuid ? adminEp.getBaseUrl(_) : selectedEp && selectedEp.getBaseUrl(_),
				title: mm.title(_),
