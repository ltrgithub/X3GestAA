...
// ignore some pages for GA hits
var hitPageIgnore = ["userProfile"];
// set special data for authoring pages hits
var hitPageAuthoring = ["authoringSaveParam", "pageAuth"];
...
function _getPage(_, context, params) {
	...
	if (referer) {
		...
		if (parsedUrl.pathname === "/syracuse-main/html/main.html") {
			var _query = parsedUrl.query;
			var _params = _query.url && querystring.parse(_query.url.substring(1));
			if (_params && _params.representation) {
				switch (_params.representation) {
					case "home.$landing":
						origin = "$landing";
						break;
		...
		} else if (parsedUrl.pathname === "/syracuse-mobile/html/mobile.html") {
			origin = "$dashboard";
		}
	}
	...
	var menuMap = adminHelper.getReverseMenuMap(_, _getNavPageComponentsLastModified(_, adminHelper.getCollaborationOrm(_)), key, facet, origin);
	// Breadcrumb
	page.$links.$breadcrumb = adminHelper.menuMapToBreadCrumb(menuMap);

	// Hit pages on server side
	// for now only desktop
	if (params && params.device === "desktop") {
		var itemTitle, itemLinkType, itemFacet, itemOwner, modTitle;
		//console.error("menuMap: "+JSON.stringify(menuMap,null,2));
		var pageName, idParts = params.id && params.id.split(',');
		if (idParts.length > 0) {
			var parts = idParts[0].split('.');
			pageName = parts && parts[3];
		}
		if (hitPageIgnore.indexOf(pageName) === -1) {
			if (menuMap && menuMap.length > 1) {
				modTitle = menuMap[1] && menuMap[1].title && (menuMap[1].title["en-us"] || menuMap[1].title["fr-fr"]);
				var item = menuMap.pop();
				// item.$stacked indicates that it's classic stacked window. We set title only for real objects
				if (item && item.$stacked && !fusionProto.$object) {
					itemTitle = pageName;
					// special cases
					if (pageName === "EXEFNC") {
						itemLinkType = "$goto";
					...
					itemTitle = item && item.title && (item.title["en-us"] || item.title["fr-fr"]);
					itemOwner = item && item.factoryOwner;
					itemLinkType = item && item.linkType;
				}
			}
			// special case for authoring pages
			if (hitPageAuthoring.indexOf(pageName) !== -1) {
				itemLinkType = "$personalize";
			}
			ga.hitPage(_, context, pageName, {
				selectedEp: selectedEp,
				title: itemTitle,
				type: itemLinkType,
				facet: params.facet,
				module: modTitle,
				variant: variant && variant.title(_),
				userId: userId,
				device: params.device,
				factoryOwner: itemOwner
			});
		}
	}
	//
...
function loadAdminModule(_, db) {
	if (_modAdmin) return;
	_modAdmin = db.fetchInstance(_, db.getEntity(_, "menuModule"), {
		jsonWhere: {
			code: "S_MOD_ADMIN"
		}
	});
...
function _fetchNavPageModules(_, np, lastModified) {
	...
	if (((lastModified || "").toString() !== (modulesCache.lastModified || "").toString()) || !modulesCache.lastModified) {
		...
		if (isFactorySageUser(_) && _modAdmin && moduleIds.indexOf(_modAdmin.$uuid) === -1) {
			//console.error("Force display of ADMIN module");
			modArr.splice(0, 0, _modAdmin);
		}
		return _modulesToCache(modArr, lastModified);
	}
	...
	if (isFactorySageUser(_) && _modAdmin && moduleIds.indexOf(_modAdmin.$uuid) === -1) {
		//console.error("Force display of ADMIN module");
		moduleIds.splice(0, 0, _modAdmin.$uuid);
	}
	//
	return moduleIds.map_(_, function(_, mid) {
		if (modulesCache && modulesCache.modules && modulesCache.modules[mid]) {
			...
			// new module, load it
			var mm = _modAdmin && mid === _modAdmin.$uuid ? _modAdmin : np.modules(_).get(_, mid);
...
exports.collectLandingStat = function(_, context, up, options, selectedEp) {
	// collect stats
	ga.hitPage(_, context, "home", {
		selectedEp: selectedEp,
		title: options.title, // if title is not set, pageInfo is mandatory to retrieve the title in mongo
		type: "$landing",
...
function _getLandingPage(_, context, params) {
	...
	return (function(_) {
		function _serializeLandingPage(_, lp) {
			...
			if (lpSelected && (lpSelected === lp.$uuid)) {
				selectedId = lp.$uuid;
				selectedTitle = lp.title(_);
				enTitle = lp._data && lp._data.title && (lp._data.title["en-us"] || lp._data.title["fr-fr"]);
			}
			//
		...
		var rr = db.fetchInstance(_, db.getEntity(_, "role"), role);
		var up = context.getUserProfile(_);
		var lpSelected = up && up.sitePreferences(_) && up.sitePreferences(_).desktop.landingpage && up.sitePreferences(_).desktop.landingpage.$selected;
		// desired page name
		var pageName = null;
		var selectedId = null;
		var selectedTitle = "";
		...
		if (adminMode) {
			...
			if (lpg) {
				...
				enTitle = lpg._data && lpg._data.title && (lpg._data.title["en-us"] || lpg._data.title["fr-fr"]);
			}
		} else {
		...
		//
		// collect stats
		exports.collectLandingStat(_, context, up, {
			title: enTitle
		}, selectedEp);
		//
		return page;
			...
			switch (params.facet) {
				...
				case "$navigation":
					if (!nocache) {
						...
						// collect stats
						ga.hitPage(_, context, params.representation, {
							selectedEp: params.endpoint && adminHelper.getEndpoint(_, {
								dataset: params.endpoint
							}),
							title: params.representation,
							type: "$navigation",
							userId: context.getUserProfile(_).user(_).$uuid,
							device: params.device,
							force: true
						});
					} else {
