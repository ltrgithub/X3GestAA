...
function _computeNavigationPageEtag(_, context, params) {
	...
	return _computeEtag(_, up.user(_).$uuid, lastModified.toISOString(), rightsKey || new Date(0));
}
...
function _fetchNavPageModules(_, np, lastModified) {
	if (modulesCache && modulesCache.lastModified) {
		if ((lastModified || "").toString() !== modulesCache.lastModified.toString()) return np.modules(_).toArray(_);
	}
