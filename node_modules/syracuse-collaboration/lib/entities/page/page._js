...
function _getNavigationPage(_, context, params) {
	...
	return _fromPrototypeCache(_, cacheId, "navigationPage") || _toPrototypeCache(_, cacheId, "navigationPage", (function(_) {
		...
		blockProtoLks.$create = navModEnt.getLink("$create", null, "originModule={$uuid}");
		...
		var blockItemsMILks = blockItemsProto.$variants.menuItem.$links = blockItemsProto.$variants.menuItem.$links || {};
		...
		page.modules = np.modules(_).toArray(_).filter_(_, function(_, mm) {
			...
			return {
				$uuid: mm.$uuid,
				$baseUrl: mmAppId === adminApp.$uuid ? adminEp.getBaseUrl(_) : selectedEp && selectedEp.getBaseUrl(_),
				title: mm.title(_),
				...
				submodules: mm.submodules(_).toArray(_).filter_(_, function(_, sm) {
					...
					return sm.getNavigationPageResource(_);
				})
			};
		});
		//
