...
function _loadAllMenus(_, db, lastModified) {
	if (modulesCache && modulesCache.lastModified && ((lastModified || "").toString() !== modulesCache.lastModified.toString())) {
...
function _fetchNavPageModules(_, np, lastModified) {
	...
	if (modulesCache && modulesCache.lastModified) {
		if ((lastModified || "").toString() !== modulesCache.lastModified.toString()) {
			np.fullLoad(_, _loadAllMenus(_, db, lastModified));
			return np.modules(_).toArray(_);
		}
	}
	...
	return moduleIds.map_(_, function(_, mid) {
		if (modulesCache && modulesCache.modules && modulesCache.modules[mid]) {
			...
			if (modulesCache && modulesCache.menus) mm.fullLoad(_, db, _loadAllMenus(_, db, lastModified));
			return mm;
		}
	});
}
