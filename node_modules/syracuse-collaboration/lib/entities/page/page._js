...
function _getPage(_, context, params) {
	...
	page.$authorUrl = context.baseUrl + "/" + authEntity.plural +
			...
			((pageData && pageData.$uuid) ? "" : "&pageContext="+params.pageContext+(params.device ? "&device="+params.device : ""));
	//
	var fromFile = false;
	if (!pageData) {
		var p = _getPath(id);
	...
	if((params.facet === "$dashboard") && !fromFile) {
		...
		if(pageData) {
			...
			page.$prototype = _fromPrototypeCache(_, pageData) || _toPrototypeCache(_, pageData, pageData.getPrototype(_, context.getUserProfile(_), {
				$diagnoses: page.$diagnoses
			}));
			var loc = pageData && pageData.pageData(_) && JSON.parse((pageData.pageData(_).localization(_) || "{}"));
			if(loc) 
	...
	if(context.parameters.fetchPrototype !== "false")
		page.$prototype = page.$prototype || (params.x3fusion ? _getFusionPrototype(_, context, protoIds) : context.getPrototypeResource(_, protoIds.slice(3,5).join("."), (pageData == null), protoIds[0], protoIds[1], protoIds[2])) || _getUrlPrototype(context.baseUrl, context.instanceId, params.x3fusion);
	//
