...
function _getPage(_, context, params) {
	...
	if (!pageDef) {
		if (params.x3fusion) {
			...
			if (context.parameters.modelRepresentation) {
				var new_r = context.parameters.modelRepresentation;
	...
	page.$authorUrl = context.baseUrl + "/" + authEntity.plural + ((variant && variant.$uuid) ? "('" + (variant && variant.$uuid) + "')" : "/$template") +
		"/$workingCopies?representation=" + authEntity.name + ".$edit" + ((variant && variant.$uuid) ? "" : "&pageContext=" + params.pageContext + (params.device ? "&device=" + params.device : ""));
	//
	var fromFile = false;
	if (!variant) {
		var p = _getPath(id);
	...
	if ((params.facet === "$dashboard") && !fromFile) {
		...
		if (variant) {
			...
			page.$prototype = _fromPrototypeCache(_, cacheId) || _toPrototypeCache(_, cacheId, variant.getPrototype(_, context.getUserProfile(_), {
				...
				withArticle: true // for now always send default article (page.$article == null)
			}));
