...
function _computeEtag(variantId, updDate, fusionProtoCacheInfos) {
	var _etag = "";
	if (!variantId && !updDate) {
		_etag += 'no_variant';
	}
	if (variantId) {
		_etag += variantId;
	...
	if (updDate) {
		if (_etag !== "") _etag += '~';
		_etag += updDate.toISOString();
	}
	if (fusionProtoCacheInfos) {
		if (_etag !== "") _etag += '~';
		_etag += (new Date(fusionProtoCacheInfos.lastModified)).toISOString();
	}
...
function _computeNavigationPageEtag(_, context, params) {
	function getLastModifiedInstance(_, entityName, binding) {
		var entity = context.db.getEntity(_, entityName);
		var _inst = context.db.fetchInstance(_, entity, {
			jsonWhere: {},
			orderBy: [{
				binding: binding,
				descending: true
	...
	lastModified = adminRightsDate > lastModified ? adminRightsDate : lastModified;
	_tracer && _tracer("Administration endpoint rights : " + adminRightsDate);

	var epRightsDate = selEpFctRights && selEpFctRights.$etag;
	lastModified = epRightsDate > lastModified ? epRightsDate : lastModified;
	_tracer && _tracer("Selected endpoint rights : " + epRightsDate);

	_tracer && _tracer("Last modified: " + lastModified);
	return _computeEtag(null, lastModified);
}
...
function _getPage(_, context, params) {
	...
	//
	timing.end();
	page._etag = _computeEtag(variant && variant.$uuid, variantUpdDate, fusionProtoCacheInfos);
	return page;
}
