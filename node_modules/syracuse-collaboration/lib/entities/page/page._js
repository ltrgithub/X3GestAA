...
function _toPrototypeCache(_, cacheId, cacheName, prototype) {
	return (globals.context.session.toCache && globals.context.session.toCache(cacheName, cacheId, prototype));
}
...
function _getNavigationPage(_, context, params) {
	var role = params.role || context.getSelectedRoleId(_);
	var adminMode = context.parameters && context.parameters.editMode === "admin";
	var app = params.application && params.contract && adminHelper.getApplication(_, params.application, params.contract);
	...
	return _fromPrototypeCache(_, cacheId, "navigationPage") || _toPrototypeCache(_, cacheId, "navigationPage", (function(_) {
		...
		/*		blockItemsProtoLks.$create = {
			$title: locale.format(module, "createMenuItemTitle"),
			$variants: {
				menuItem: menuItemEnt.getLink("$create", locale.format(module, "createMenuItemTitle"), "originBlock={$uuid}")
			}
		};
		blockItemsProtoLks.$create.$variants.menuItem.$target = "$edit";*/
		...
		var np = db.fetchInstance(_, navPageEnt, {
			jsonWhere: {
...
function _getLandingPage(_, context, params) {
	...
	return _fromPrototypeCache(_, cacheId, "landingPage") || _toPrototypeCache(_, cacheId, "landingPage", (function(_) {
		...
		//
		// landing page data
		var lp = db.fetchInstance(_, ldPageEnt, {
			jsonWhere: {
				pageName: params.representation
			}
		});
		...
		page = lp.serializeInstance(_);
		page.$selectedEPBaseUrl = selectedEp && selectedEp.getBaseUrl(_);
		//
		var ldPageProto = page.$prototype = ldPageEnt.getPrototype(_, "landingPage", "$details");
		ldPageProto.$baseUrl = adminEp.getBaseUrl(_);
		//
		return page;
	})(_));
}
