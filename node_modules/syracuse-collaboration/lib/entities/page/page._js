...
/*		if (!adminMode) {
		...
		np.modules(_).toArray(_).forEach_(_, function(_, mm) {
			...
			page_m.$transMenuBaseUrl = page_m.$menuBaseUrl.replace("/sdata/", "/trans/");
			mm.submodules(_).toArray(_).forEach_(_, function(_, sm) {
				...
				}, isAdmin ? admFctRights : selEpFctRights, adminMode);
	...
	return _fromPrototypeCache(_, cacheId, "landingPage") || _toPrototypeCache(_, cacheId, "landingPage", (function(_) {
		function _serializeLandingPage(_, lp) {
			var res = {
				...
				$selected: (pageName && (pageName === lp.pageName(_))) ? true : false,
		...
		var plEnt = db.getEntity(_, "pageLayout");
		//
		var rr = db.fetchInstance(_, db.getEntity(_, "role"), role);
		var up = context.getUserProfile(_);
		// desired page name
		...
		// Prototype additions
		page.$prototype.$properties.rolePages = _getPagePrototype(_, false, false);
		page.$prototype.$properties.userPages = _getPagePrototype(_, true, false);
		//
