...
function _getDashboardPrototype(_, pageData, profile) {
	function addPortletEndpoint(_, portlet, endpoint, isMultiEP, pagePortlet) {
		...
		if(p) {
			// TODO: properly manage title template
/*			if(!pagePortlet.allEndpoints(_) && !(profile && profile.selectedEndpoint(_))) {
			...
			var template = pagePortlet.titleTemplate(_) || "{portlet} ({endpoint})";
			template = template.replace("{portlet}", p.$title).replace("{endpoint}", (endpoint && endpoint.description(_)) || "");
			p.$title = template;
		}
...
//				if(!config || !config.pages || !config.pages.allowDashboardAuthoring)
