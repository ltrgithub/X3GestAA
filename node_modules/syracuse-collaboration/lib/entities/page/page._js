...
function _fromPrototypeCache(_, cacheId, cacheName) {
...
function _getPage(_, context, params) {
	...
	if ((params.facet === "$dashboard") && !fromFile && (_selected("$prototype") || _selected("$article"))) {
		...
		if (variant) {
			...
			page.$prototype = _fromPrototypeCache(_, cacheId, "dashboardPrototype") || _toPrototypeCache(_, cacheId, "dashboardPrototype", variant.getPrototype(_, context.getUserProfile(_), {
...
function _getNavigationPage(_, context, params) {
	var role = params.role || context.getSelectedRoleId(_);
	var editMode = (context.parameters && context.parameters.editMode) || "user";
	...
	//
	return _fromPrototypeCache(_, cacheId, "navigationPage") || _toPrototypeCache(_, cacheId, "navigationPage", (function(_) {
		...
		if (adminMode) {
			modProtoLks.$create = navModEnt.getLink("$create", locale.format(module, "createModuleTitle"), "originPage={$uuid}");
		...
		if (adminMode) {
			blockProtoLks.$create = subModEnt.getLink("$create", locale.format(module, "createSubmoduleTitle"), "originModule={$uuid}");
		...
		//		console.log("(563): " + sys.inspect(blockItemsMBProto, null, 6));
		...
		// rights structure
		var admFctRights = up.getFunctionsAuthorizations(_, adminEp);
		var selEpFctRights = (selectedEpId !== adminEp.$uuid) ? up.getFunctionsAuthorizations(_, selectedEp) : admFctRights;
		...
		page.modules = [];
		np.modules(_).toArray(_).forEach_(_, function(_, mm) {
			if (!_keep(_, mm.application(_), mm.endpoints(_).toArray(_))) return;
			...
			mm.submodules(_).toArray(_).forEach_(_, function(_, sm) {
				if (!_keep(_, sm.application(_), sm.endpoints(_).toArray(_))) return;
				var res_sm = sm.getNavigationPageResource(_, "{$menuBaseUrl}", isAdmin ? admFctRights : selEpFctRights);
				if (res_sm.items && res_sm.items.length) page_m.submodules.push(res_sm);
			});
			if (page_m.submodules && page_m.submodules.length) page.modules.push(page_m);
		});
		//
		return page;
	})(_));
...
function _getLandingPage(_, context, params) {
	...
	var selectedEpId = selectedEp && selectedEp.$uuid;
	var cacheId = [params.representation, (selectedEp && selectedEpId) || "all", role, locale.current].join("_");
	//
	return _fromPrototypeCache(_, cacheId, "landingPage") || _toPrototypeCache(_, cacheId, "landingPage", (function(_) {
		function _serializeLandingPage(_, lp) {
			if (pageName && (pageName === lp.pageName(_))) {
				var res = {
					$uuid: lp.$uuid,
					title: lp.title(_),
					vignettes: lp.vignettes(_).toArray(_).map_(_, function(_, lv) {
						var lvep = lv.endpoint(_);
						return {
							$uuid: lv.$uuid,
							bind: lv.vignette(_).code(_) + (lvep ? "-" + lvep.dataset(_) : ""),
							$links: {
		...
		// desired page name
		var pageName = null;
		var pref = up.getRepresentationPrefs(_, "landingPage", "$landing");
		if (params.representation === "*") {
			pageName = (pref && pref.lastLandingPageName(_));
		} else {
			pageName = params.representation;
			if (!pref) pref = up.createRepresentationPrefs(_, "landingPage", "$landing");
			pref.lastLandingPageName(_, params.pageName);
			pref.save(_);
		}
		//
		page.rolePages = rr && rr.landingPages(_).toArray(_).map_(_, _serializeLandingPage);
		page.userPages = up.user(_) && up.user(_).landingPages(_).toArray(_).map_(_, _serializeLandingPage);
		//
		page.$prototype = {
			$properties: {
				rolePages: {
		...
		// TODO
		page.$prototype.$properties.rolePages.$item.$item.$properties.vignettes.$bind = "{bind}";
		page.$prototype.$properties.userPages.$item.$item.$properties.vignettes.$bind = "{bind}";
		//
		page.$prototype.$baseUrl = adminEp.getBaseUrl(_);
		page.$selectedEpBaseUrl = selectedEp.getBaseUrl(_);
		//
		return page;
	})(_));
}
		...
		$setId: function(_, context, id) {
			...
			switch (params.facet) {
				...
				case "$landing":
					...
					break;
				default:
					page = _getPage(_, context, params);
			}
			this.$resource = page || {};
			this.$resource.$uuid = this.$resource.$uuid || helpers.uuid.generate();
