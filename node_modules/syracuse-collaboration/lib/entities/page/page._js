...
function _computeNavigationPageEtag(_, context, params) {
	...
	// add x3 login to etag because of the need of changes that can be done on login mapping
	var x3login = up.user(_).getEndpointLogin(_, selectedEpId);
	_tracer && _tracer("X3 login used in etag: " + x3login);
	var epRightsKey = x3login + (selEpFctRights ? "~" + selEpFctRights.$etag : "");
	_tracer && _tracer("Selected endpoint rights : " + epRightsKey);
	if (epRightsKey) rightsKey = rightsKey != null ? rightsKey + "~" + epRightsKey : epRightsKey;
