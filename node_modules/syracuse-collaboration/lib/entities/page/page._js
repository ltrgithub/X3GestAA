...
function _computeEtag(_, variantId, key1, key2) {
	var _etag = "";
	if (!variantId && !key1) {
		_etag += 'no_variant';
	}
...
function _computeNavigationPageEtag(_, context, params) {
	...
	return _computeEtag(_, null, lastModified.toISOString(), rightsKey || new Date(0));
}
...
function _getPage(_, context, params) {
    ...
    */
	...
	page._etag = _computeEtag(_, variant && variant.$uuid, variantUpdDate && variantUpdDate.toISOString(), fusionUpdDate && fusionUpdDate.toISOString());
	return page;
}

var modulesCacheFunnel = flows.funnel(1);
var modulesCache = { modules: {}};
function _fetchNavPageModules(_, np, lastModified) {
    if (modulesCache && modulesCache.lastModified) {
        if (lastModified > modulesCache.lastModified) return np.modules(_).toArray(_);
    }
    var moduleIds = np.modules(_).toUuidArray(_);
    return moduleIds.map_(_, function(_, mid) {
        if (modulesCache && modulesCache.modules && modulesCache.modules[mid]) return modulesCache.modules[mid];
        else return np.modules(_).get(_, mid);
    });
}
			...
			switch (params.facet) {
				...
				case "$navigation":
					if (!nocache) {
                        ...
                        // crnit : compute etag here instead of in cacheEtag function to be able to compare
                        var shasum = crypto.createHash('sha1');
                        shasum.update(etag, "utf8");
