...
exports.entity = {
	...
	$functions: {
		...
		$save: function(_, saveRes) {
			...
			if(!pageData) {
				// create, some props are mandatory for now
				pageData = variant.createChild(_, "pageData");
				variant.pageData(_, pageData);
				pageData.title(_, pageDef.title(_));
				pageData.facet(_, pageDef.facet(_));
				pageData.representation(_, pageDef.representation(_));
				pageData.application(_, pageDef.application(_));
			}
			pageData.content(_, this.content(_));
