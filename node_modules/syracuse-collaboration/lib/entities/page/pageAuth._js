...
exports.entity = {
	...
	$functions: {
		...
		createPageDef: function(_, pageContext, device, modelRepresentation) {
			...
			var title = pageContext + "." + (device || "desktop");
			pageDef.facet(_, parts[3]);
			pageDef.application(_, adminHelper.getApplication(_, parts[0], parts[1]));
			pageDef.representation(_, modelRepresentation ? modelRepresentation + '$MODEL' : parts[2]);
	...
	$services: {
		saveAs: {
			...
			$facets: ["$edit"],
			$parameters: {
				$url: "{$baseUrl}/authoringSaveParams/$template/$workingCopies?representation=authoringSaveParam.$edit&role={$role}&pageType=page&variantId={variantId}&modelRepresentation={modelRepresentation}",
				$method: "POST",
				$properties: {
					parameters: {
						$type: "application/x-string"
