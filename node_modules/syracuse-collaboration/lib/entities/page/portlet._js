...
var _contentSolverMap = {
	$menu: function(_, portlet, endpoint, options) {
		...
		items.forEach_(_, function(_, item) {
			...
			link.$vignettes = [portlet.code(_) || portlet.$uuid];
...
function _scanPageForRepresentations(facet, proto, list, added) {
	//don't scan lookup facet
	if (facet === "$lookup") return;
	// add links and lookups
	var prototypes = [], badLinks = [];
		...
		representation : {
			...
			$execute : function(_, context, instance) {	
				...
				var _sendError = function  (_, status, msg, stack, diagnoses){
					...
					var diags = [{"$severity" : "error","$message" : msg, "$stackTrace": stackTrace.join('\n')}]
					context.reply(_, status, {"$diagnoses":diagnoses ? diags.concat(diagnoses) : diags});	
				}			
				if (!instance) {
				...
				while (list.length) {
					...
					list.forEach_(_, function(_, representation) {
						if (!res.pages[representation]) {
							...
							var page  = pageEntity.pageContent(_, context, opts), proto = page && page.$prototype, ok = true;
							if (proto && proto.$diagnoses) {
								proto.$diagnoses.forEach_(_, function(_, diag) {
							...
							if (ok && (proto==null || typeof proto != 'object' || Object.keys(proto).length == 0 || !proto.$properties)){
								// Force Error 
								diagnoses.push({ "$message":locale.format(module, "badProto", representation), "$severity":"error"});
								ok = false;
							} 
							if (ok) {
								res.pages[representation] = page;
								var badLink = _scanPageForRepresentations(opts.facet, page.$prototype, cl, list);								
								if (badLink && badLink.length > 0) {
									// Force warning 
									var diag = {"$message" : locale.format(module, "badLink"), "$severity" : "warning"};
									diag.$stacktrace = "";
									badLink.forEach_(_, function(_, msg) {
										diag.$stacktrace += msg + "\n";
									});
									diagnoses.push(diag);
								}
				...
				if (res.dashboards){
					...
					for (var p in dashboards){
						...
						dash.forEach(function(d){
							if (!d.linkType  ||  !d.representation) 
								newDash.push(d); // add 
							else if ((d.linkType === "$representation") && res.entities[d.representation]) 
								newDash.push(d);
						});
				...
				var _empty=function(o){ return o == null || Object.keys(o).length == 0}
				if (_empty(res.entities) || _empty(res.pages) || _empty(res.dashboards)){
					var stack=[];
					if (_empty(res.entities)) stack.push("No entities");
					if (_empty(res.pages)) stack.push("No pages");
					if (_empty(res.dashboards)) stack.push("No dashboards");
