...
exports.entity = {
	...
	$services: {
		representation : {
			// get mobile application structure (list of used representations by this portlet)
			$method : "get",
			$isHidden : true,
			$isMethod : true,
			$title : "Mobile Application Structure",
			$execute : function(_, context, instance) {
				var res = {representations: [], home: {data: {$resources:[], representation:{}}}};
				if (instance.type(_) === "$menu") {
					...
					items.forEach_(_, function(_, item) {
						if (item.linkType(_) === "$representation") {
							...
							data.applicationName = item.applicationName(_);
							data.contractName = item.contractName(_);
							data.endpointName = item.endpointName(_);
							data.facet  = item.facet(_);
							res.home.data.$resources.push(data);
						}
						
					});
				}
				//inspect all menu items of portlet
