			...
			$execute : function(_, context, instance) {	
				...
				while (list.length) {
					...
					list.forEach_ (_, function(_, repInfo) {
						var representation = repInfo.rep, from = repInfo.from;
						if (!res.pages[representation]) {
							...
							if (proto && proto.$diagnoses) {
								var diag = { "$message":locale.format(module, "badRep", representation), "$severity":"warning", "$stackTrace":"From : " + from};
							...
							if (ok && (proto==null || typeof proto != 'object' || Object.keys(proto).length == 0 || !proto.$properties)){
								// Force Error 
								diagnoses.push({ "$message":locale.format(module, "badProto", representation), "$severity":"warning",  "$stackTrace": "From : " + from});
								ok = false;
							}else{
