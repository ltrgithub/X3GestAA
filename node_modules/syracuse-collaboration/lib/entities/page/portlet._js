	...
	$services: {
		representation : {
			...
			$execute : function(_, context, instance) {				
				if (!instance) {
					var e= new Error(locale.format(module, "noInstance"));
					e.httpStatus = 400;
					throw e;
				}
					...
					list.forEach_(_, function(_, representation) {
						if (!res.pages[representation]) {
							...
							if (proto==null || typeof proto != 'object' || Object.keys(proto).length == 0){
								errors.push(locale.format(module, "badRep", representation))
							}else{
				...
				if (errors.length>0) {
					errors.push("",locale.format(module, "checkRep"));
					errors.push ("","Application: " + params.applicationName.toUpperCase() + " - Contract: " + params.contractName.toUpperCase()  + " - Endpoint: " + params.endpointName.toUpperCase());
					context.reply(_, 500, {"$diagnoses":[{"$severity":"error","$message":locale.format(module, "badApp", instance.title(_)),"$stackTrace":errors.join('\n')}]});	
					return;
				}
				res.entities = {};
				...
				if (!res.entities || Object.keys(res.entities).length == 0 || !res.pages || Object.keys(res.pages).length ==0){
					throw new Error(locale.format(module, "noApp", instance.title(_), params.endpointName));					
				}else{
