...
function _scanPageForRepresentations(facet, proto, list, added, errors) {
	//don't scan lookup facet
	if (facet === "$lookup") return;
	// add links and lookups
	var prototypes = [];
	if (!proto || (typeof proto != "object")) return;
	prototypes.push({proto: proto});
	if (proto.$properties.$resources) 
		prototypes.push({proto: proto.$properties.$resources.$item});
	var rep = "representation=";
			...
			forEachKey(proto.$links, function(linkName, link) {
				...
				if (url.indexOf("{$baseUrl}")!=0 || ir<0){					
					...
					if (url.indexOf("/trans/")==0){
						msg = linkName + " :  unexpected convergence url\n[" + url + "]";
					}
...
/*				var appUuid = val && val.$uuid;
		...
		items: {
			...
			$propagate: function(_, instance, val) {
				globals.context.session.resetCache && globals.context.session.resetCache("dashboardPrototype");
			}
		},
		pageItem: {
			$title: "Page content",
	...
	$services: {
		representation : {
			...
			$execute : function(_, context, instance) {	
				var params = JSON.parse(context.request.readAll(_)), res, key;
				var _sendError = function  (_, status, msg, stack){
					...
					if (stack) stackTrace = stackTrace.concat(stack);
					context.reply(_, status, {"$diagnoses":[{"$severity" : "error","$message" : msg, "$stackTrace": stackTrace.join('\n')}]});	
				}	
				var _sendWarning = function  (_, msg, stack){
					if (!res["$diagnoses"])res["$diagnoses"]=[];
					var stackTrace=["Application: " + params.applicationName.toUpperCase() + " - Contract: " + params.contractName.toUpperCase()  + " - Endpoint: " + params.endpointName.toUpperCase()];
					if (stack) stackTrace = stackTrace.concat(stack);
					res["$diagnoses"].push({"$severity" : "$warning","$message" : msg, "$stackTrace": stackTrace.join('\n')});	
				}					
				if (!instance) {
				...
				while (list.length) {
					...
					list.forEach_(_, function(_, representation) {
						if (!res.pages[representation]) {
							...
							var proto = page && page['$prototype'];
							if (proto==null || typeof proto != 'object' || Object.keys(proto).length == 0){
								errors.badRep.push(locale.format(module, "badRep", representation))
							}
							res.pages[representation] = pageEntity.pageContent(_, context, opts);
							_scanPageForRepresentations(opts.facet, res.pages[representation].$prototype, cl, list, errors.badLink);
						}
					});
					list = cl;
      			}
				var diags=[];
				if (errors.badRep.length>0) {
					errors.badRep.push("",locale.format(module, "checkRep"));
					_sendError(_, 500, locale.format(module, "badApp", instance.title(_)), errors.badRep);
					context.reply(_, 500, {"$diagnoses":[{"$severity":"error","$message":locale.format(module, "badApp", instance.title(_)),"$stackTrace":errors.badRep.join('\n')}]});	
					return;
					//_sendWarning(_, locale.format(module, "badApp", instance.title(_)), errors.badRep);
				}
				if (errors.badLink.length>0) {
					var stack = [locale.format(module, "badLink")].concat(errors.badLink);
					stack.push("",locale.format(module, "checkTypeRep"));
					_sendWarning(_, locale.format(module, "badApp", instance.title(_)), stack);	
					return;
				}
				res.entities = {};
				...
				if (!res.entities || Object.keys(res.entities).length == 0 || !res.pages || Object.keys(res.pages).length ==0){
					_sendError(_, 400, locale.format(module, "noApp", instance.title(_), params.endpointName));
				}else{
