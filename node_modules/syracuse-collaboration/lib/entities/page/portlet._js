	...
	$services: {
		representation : {
			...
			$execute : function(_, context, instance) {	
				...
				var _addWarning = function  (_, diags, msg, stack){
					var diags = diags || [];
					diags.push({"$severity" : "$warning","$message" : msg, "$stackTrace": stack ? stack.join('\n') : ""});	
					return diags;
				}					
				if (!instance) {
					...
					list.forEach_(_, function(_, representation) {
						if (!res.pages[representation]) {
							...
							if (proto==null || typeof proto != 'object' || Object.keys(proto).length == 0){
								errors.badProto.push("Prototype null or not valid - " + representation)
							}
				...
				if (errors.badProto.length>0) {
					_addWarning(_, diagnoses, locale.format(module, "badProto", instance.title(_)), errors.badProto);
				}
				if (errors.badLink.length>0) {
					_addWarning(_, diagnoses, locale.format(module, "badLink"), errors.badLink);	
				}
				if (!res.entities || Object.keys(res.entities).length == 0 || !res.pages || Object.keys(res.pages).length ==0){
					_sendError(_, 400, locale.format(module, "badApp", instance.title(_)), null, diagnoses);
				}else{
