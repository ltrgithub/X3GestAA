...
var _computeLinkMap = {
	...
	$representation_$create: function(_, item, baseUrl) {
		return baseUrl + "/" + item.representationRef(_).entity(_) + "/$template/$workingCopies?representation=" + item.representationRef(_).representation(_) + "." + item.getLinkFacet(_);
	},
	$representation_$details: function(_, item, baseUrl) {
		// TODO: parameter
		return baseUrl + "/" + item.representationRef(_).entity(_) + "('" + item.keyParameter(_) + "')?representation=" + item.representationRef(_).representation(_) + "." + item.getLinkFacet(_);
	},
	$representation_$edit: function(_, item, baseUrl) {
		// TODO: parameter and test
		return baseUrl + "/" + item.representationRef(_).entity(_) + "('" + item.keyParameter(_) + "')/$workingCopies?representation=" + item.representationRef(_).representation(_) + "." + item.getLinkFacet(_);
	},
	...
	$stats: function(_, item, baseUrl) {
		...
		return name && (baseUrl + "/STATS?representation=STATS~" + name + "." + item.getLinkFacet(_));
	},
	$process: function(_, item, baseUrl) {
		...
		var level = item.requestLevel(_);
		return pars[1] && (baseUrl + "/PROCESS?representation=PROCESS~" + pars.join("~") + "." + item.getLinkFacet(_));
	}
};
...
exports.entity = {
	...
	$valueTemplate: "{title}",
	$helpPage: "Administration-reference-Menu-items",
	$properties: {
		code: {
			$title: "Code"
		},
		...
		requestName: {
			...
			$isDefined: function(_, instance) {
				return (instance.linkType(_) === "$request");
			},
			$isMandatory: function(_, instance) {
				return (instance.linkType(_) === "$request");
			}
	...
	$relations: {
		application: {
			...
			$inv: "menuItems",
			$isDefined: function(_, instance) {
				return (["$representation", "$function", "$process", "$request", "$stats"].indexOf(instance.linkType(_)) >= 0);
			},
			$isMandatory: function(_, instance) {
				return (["$representation", "$function", "$process", "$request", "$stats"].indexOf(instance.linkType(_)) >= 0);
			},
	...
	$functions: {
		...
		getExecuteLink: function(_, linkResName) {
			...
			if (!ep) {
				...
				if (ep && (ep.applicationRef(_).$uuid !== instance.application(_).$uuid)) return null;
			}
			if (!ep) return null;
			return instance.getLink(_, ep);
		},
		...
		getItemUrl: function(_, endpoint, baseUrlProp) {
			...
			return (_computeLinkMap[mapLinkEntry] && _computeLinkMap[mapLinkEntry](_, this, (endpoint && endpoint.getBaseUrl(_)) || baseUrlProp || "{$baseUrl}")) + params;
		},
		...
		authorized: function(_, fctAuth) {
			...
			switch (this.linkType(_)) {
				case "$function":
					...
					var isThere = fctAuth.$functions && (fctAuth.$functions[this.fusionFunction(_)] != null);
				...
				case "$representation":
					...
					if (!rep) return false;
					var isThere = fctAuth.$entities && (fctAuth.$entities[rep.toLowerCase()] != null);
					isThere = isThere || (fctAuth.$representations && (fctAuth.$representations[rep.toLowerCase()] != null));
					return ((fctAuth.$mode === "authorize") && isThere) || ((fctAuth.$mode != "authorize") && !isThere);
				default:
					return true;
			}
	...
	$searchIndex: {
		$fields: ["code", "title", "description", "linkType", "application", "representationRef", "facet", "fusionFunction", "fusionKey", "dashboard", "categories"]
	}
};