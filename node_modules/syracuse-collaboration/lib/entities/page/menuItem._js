	...
	$relations: {
		application: {
			...
			$lookup: function(_, instance) {
				return {
					$type: "application/json",
					$url: adminHelper.getCollaborationEndpoint(_).getBaseUrl(_) + "/applications?representation=application.$lookup" + (instance.linkType(_) === "$function" ? "&where=(protocol eq \"x3\")" : "")
				}
			}
		},
		endpoint: {
