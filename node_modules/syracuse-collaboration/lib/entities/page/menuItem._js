...
var _computeLinkMap = {
	...
	$hrm: function(_, item, baseUrl) {
		...
		var sol = ep && ep.x3SolutionName(_);
...
exports.entity = {
	...
	$allowFactory: true,
	$factoryExcludes: ["description", "endpoint", "module"],
	$properties: {
		code: {
			$title: "Code",
			$isUnique: true,
			...
			$isDisabled: function(_, instance) {
				if (!instance.$factory) return false;
	...
	$relations: {
		application: {
			...
			$inv: "menuItems",
			$isDefined: function(_, instance) {
				return (["$representation", "$function", "$process", "$request", "$stats", "$hrm"].indexOf(instance.linkType(_)) >= 0);
			},
			$isMandatory: function(_, instance) {
				return (["$representation", "$function", "$process", "$request", "$stats", "$hrm"].indexOf(instance.linkType(_)) >= 0);
			},
	...
	$functions: {
		...
		authorized: function(_, fctAuth) {
			...
			switch (this.linkType(_)) {
				case "$function":
					if (fctAuth.$disabledFunctions && (fctAuth.$disabledFunctions[this.fusionFunction(_)] != null)) return false;
