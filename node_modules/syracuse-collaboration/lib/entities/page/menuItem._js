...
var _computeLinkMap = {
	...
	$representation_$create: function(_, item, baseUrl) {
		return baseUrl + "/" + item.representationRef(_).entity(_) + "/$template/$workingCopies?representation=" + item.representationRef(_).representation(_) + "." + item.facet(_);
	},
...
exports.entity = {
	...
	$properties: {
		...
		referenceDataset: {
			...
			$isHidden: true,
			$compute: function(_, instance) {
				var ep = (instance.endpoint(_) || (instance.application(_) && instance.application(_).defaultEndpoint(_)));
	...
	$relations: {
		application: {
			...
			$propagate: function(_, instance, val) {
				...
				if(val && r && (r.application(_).$uuid !== val.$uuid))
