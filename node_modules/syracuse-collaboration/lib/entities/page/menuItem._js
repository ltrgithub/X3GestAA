...
var _computeLinkMap = {
	...
	$representation_$cube: function(_, item, endpoint) {
		return endpoint.getBaseUrl(_) + "/" + item.entity(_) + "('" + item.keyParameter(_) + "')?representation=" + item.representation(_) + "." + item.facet(_);
	},
...
exports.entity = {
	...
	$properties: {
		...
		facet: {
			$title: "Facet",
			$enum: ["$query", "$cube", "$details", "$edit" ,"$create"],
			$enumCaptions: ["Query", "Statistics", "Details", "Edit", "Create"],
			$isDefined: function(_, instance) {
				return (instance.linkType(_) === "$representation");
			},
			$isMandatory: function(_, instance) {
