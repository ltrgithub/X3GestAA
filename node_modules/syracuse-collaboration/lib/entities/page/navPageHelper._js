...
function _loadAllMenus(_, db, lastModified) {
	...
	for (var ii = 0; ii < cnt; ii++) {
		...
		var key = mm[ii].applicationName(_) + "." + mm[ii].contractName(_) + "." + (mm[ii].fusionFunction(_) ? mm[ii].fusionFunction(_) : mm[ii].representation(_));
		breadCrumbMap[key] = mm[ii].$uuid;
	}
	//_tracer && _tracer("MENUS: "+require('util').inspect(modulesCache && modulesCache.menus,null,2));
...
function _modulesToCache(modules, lastModified) {
	...
	modulesCache.modulesArr = modules;
	// chaining
	return modules;
}
...
var hasChanged = exports.hasChanged = function(lastModified) {
	return ((lastModified || "").toString() !== (modulesCache.lastModified || "").toString()) || !modulesCache.lastModified;
};
	...
	if (hasChanged(lastModified)) {
		return modulesCacheFunnel(_, function(_) {
			if (hasChanged(lastModified)) {
				...
				return modulesCache.modulesArr;
			}
		});
	}
...
exports.getLastChangeTime = function() {
	return modulesCache.lastModified;
};
...
exports.getBreadcrumb = function(_, lastModified, id, facet) {
	...
	if (hasChanged(lastModified)) {
		// call _fetchNavPageModules to be handled by the funnel as for nav page compute
		exports._fetchNavPageModules(_, null, lastModified);
	}

	var current = modulesCache.menus[breadCrumbMap[id]];
