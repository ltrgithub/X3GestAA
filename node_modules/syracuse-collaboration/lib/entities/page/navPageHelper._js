	...
	if (home) {
		...
		home.modules && home.modules.forEach_(_, function(_, m, idx) {
			...
			if (_m) {
				...
				var _multiplier = (_m._factory || _m.isFactory) ? 100 : 200;
				var newIdx = idx + (modulesOrder.indexOf(_c) * _multiplier);
				_tracer && _tracer("Module (" + _m.code + " : " + (_m.title && _m.title['default']) + ") had old index [" + idx + "] and have been positionned at [" + newIdx + "]");
		...
		if (reordered.length > 0) {
			...
			tracer && tracer("Modules reordered on navigation page 'home'");
		}
	}
};
...
exports.cleanNavPage = function(_, db, application, contract, options) {
	...
	exports.reorderModules(_, db, navPageName, options);
