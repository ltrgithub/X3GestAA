	...
	if (pageLayoutProxy && pageLayoutProxy._layoutId && pageLayoutProxy._layoutId.indexOf('.') !== -1) {
		...
		if (lp) {
				...
				if (loc[pref]) {
					...
					keys.forEach_(_, function(_, k) {
						if (loc[pref.toLowerCase()][k] !== loc[pref][k]) {
							if (content && content.$layout && content.$layout.$items && content.$layout.$items.some(function(itm) {
								return itm.$title && itm.$title.indexOf(k) !== -1;
							})) {
								var oldVal = loc[pref.toLowerCase()][k];
								var newVal = loc[pref][k];

								if (tracer && lp.isFactory(_) === true) {
			...
			if (items) {
				for (var i in loc) {
					for (var j in loc[i]) {
						if (!items.some(function(itm) {
							return itm.$title && itm.$title.indexOf(j) !== -1;
						})) {
							if (tracer && lp.isFactory(_) === true) {
								// display everything for factory elements
...
exports.entity = {
	...
	getLayoutEtagFromId: function(_, db, id) {
		var layout = _getLayout(_, db, id);
		if (!layout) return null;
		return layout.$updDate && layout.$updDate.toGMTString();
	}
};