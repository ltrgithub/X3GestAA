...
exports.entity = {
	...
	$valueTemplate: "{title}",
	$helpPage: "Administration-reference-Menu-submodules",
	$allowFactory: true,
			...
			items.forEach_(_, function(_, it) {
				...
				if (res_it && (!res_it.menuBlock && !res_it.menuItem)) {
					...
					try {
						...
						console.error("it meta properties : " + sys.inspect(it._meta._$properties, null, 4));
						console.error("it meta relations : " + sys.inspect(it._meta._$relations, null, 4));
						...
						console.error("it _relation : " + sys.inspect(it._relation, null, 4));
						console.error("it data : " + sys.inspect(it._data));
						console.error("it proto : " + sys.inspect(Object.getPrototypeOf(it), null, 4));
						console.error("res_it : " + sys.inspect(res_it, null, 4));
					...
					require("syracuse-session/lib/sessionManager").gentlyTerminate(_, "menuBlock", true);
				}
				if (!adminMode && res_it.menuBlock && res_it.menuBlock.items && !res_it.menuBlock.items.length) return;
				if (res_it.menuBlock) {
					var sousRes = res_it.menuBlock;
					if (adminMode && it.$factory && ff.authLevel && (ff.authLevel !== "sage")) {
						sousRes.$links = sousRes.$links || {};
						sousRes.$links.$edit = sousRes.$links.$edit || {};
						sousRes.$links.$edit.$isHidden = true;
						if (sm.$factory) {
		...
		fullLoad: function(_, menus) {
			...
			(self.items(_)._array || []).forEach_(_, function(_, it, pos) {
				if (it.$variantType === "menuItem") {
					...
					if (mm) self.items(_)._array[pos] = mm;
				} else {
					it.ensureLoaded(_);
					it.fullLoad(_, menus);
				}
