...
exports.entity = {
	...
	$functions: {
		...
		diagnoseFusionArticle: function(_, pageDef, variant, endpoint, version, opt) {
			...
			//
			var protoFields = Object.keys(p.$properties).filter(function(pName) {
				return (pName[0] !== "$") && !p.$properties[pName].$isExcluded;
			}).map(function(pName) {
				return p.$properties[pName].$X3Name;
			});
			// scan $bind
			var binds = {};
			...
			protoFields.length && diags.push({
				$severity: "info",
				$message: locale.format(module, "protoFieldNotInArticle", variant.code(_), variant.title(_), par.prototypeId, protoFields.join(","))
			});
			a_binds.length && diags.push({
				$severity: "info",
				$message: locale.format(module, "articleFieldNotInProto", variant.code(_), variant.title(_), par.prototypeId, a_binds.join(","))
			});
			g.length && diags.push({
				$severity: "info",
				$message: locale.format(module, "articleExcludedNotInProto", variant.code(_), variant.title(_), par.prototypeId, g.join(","))
			});
		}
	},
	$indexes: {},
