...
//			$type: "application",
	...
	$links: function(_, instance) {
		function _makeLink(title, type) {
			return {
				$title: title,
				$url: lkUrl.replace("%7BlinkType%7D", type),
				$method: "GET",
				$isDisabled: !enabled
			};
		}
		...
		lkUrl += "&where=" + encodeURIComponent("((type eq '$page') and (linkType eq '{linkType}') and (application eq '" + instance.applicationId(_) + "' or application eq null))");
