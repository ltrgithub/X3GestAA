	...
	$save: function(_, saveRes, parameters) {
		...
		if(this._pageId) {
			...
			if(this._pageContext) {
				pageDef = this.createPageDef(_, this._pageContext, this._device);
				if(!pageDef) return;
				// create a variant
		...
		if(v.isFactory(_)) {
			...
			if(sp && sp.authoringLevel(_) && (["admin", "user"].indexOf(sp.authoringLevel(_)) >= 0))
				throw new Error(locale.format(module, "notAuthorizedFactory"));
		}
		if(v._isGlobal(_)) {
			var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
			if(sp && sp.authoringLevel(_) && (sp.authoringLevel(_) === "user"))
				throw new Error(locale.format(module, "notAuthorizedGlobal"));
		}
		//
