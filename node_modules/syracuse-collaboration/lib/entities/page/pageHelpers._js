...
exports.AuthoringHelper = helpers.defineClass(AuthoringHelper, null, {
	...
	$save: function(_, saveRes, parameters) {
		...
		/*		if (this._authorVariantId && (saveAsOption !== "model_variant")) {
			variant = this.getVariant(_, saveAsOption);
			pageDef = variant && variant._parent;
		} else {
			if (this._pageContext) {
				pageDef = this.createPageDef(_, this._pageContext, this._device, saveAsModel && this.modelRepresentation && this.modelRepresentation(_));
				if (!pageDef) return;
				// create a variant
		...
		if (v.isFactory(_)) {
			...
			if (sp && sp.authoringLevel(_) && (["admin", "user"].indexOf(sp.authoringLevel(_)) >= 0)) throw new Error(locale.format(module, "notAuthorizedFactory"));
		}
		if (v._isGlobal(_)) {
			var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
...
/*		var thisDiags = this.$diagnoses = this.$diagnoses || [];
		...
		*/
		...
		if (variant && variant.isFactory(_)) saveRes.$confirm = locale.format(module, "modifyFactory");
	...
	getSaveVariant: function(_, saveAsOption, modelRepresentation) {
		...
		if (modelRepresentation && (saveAsOption === "model_variant")) {
			...
			parts[2] = modelRepresentation + "$MODEL";
			c = parts.join(".");
		}
		//
...
exports.Factory = {
	getPageFromContext: function(_, db, pageType, pageContext) {
