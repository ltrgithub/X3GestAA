...
exports.AuthoringHelper = helpers.defineClass(AuthoringHelper, null, {
	...
	$save: function(_, saveRes, parameters) {
		...
		if (v.$factory) {
			...
			if (!sp.canChangeFactoryInstance(_, v)) throw new Error(locale.format(module, "notAuthorizedFactoryNotOwned", v.$factoryOwner));
			//if (v.$factoryOwner && v.$factoryOwner !== adminHelper.getFactoryOwner(_)) throw new Error(locale.format(module, "notAuthorizedFactoryNotOwned", v.$factoryOwner));
		}
		if (v._isGlobal(_)) {
			var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
