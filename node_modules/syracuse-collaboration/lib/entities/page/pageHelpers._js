...
exports.AuthoringHelper = helpers.defineClass(AuthoringHelper, null, {
	...
	$save: function(_, saveRes, parameters) {
		...
		if (this._pageId) {
			...
			if (this._pageContext) {
				pageDef = this.createPageDef(_, this._pageContext, this._device, this.modelRepresentation && this.modelRepresentation(_));
				if (!pageDef) return;
				// create a variant
	...
	getVariant: function(_, saveAsOption) {
		...
		if (this._pageId) {
			...
			//
			return saveAsOption != null ? this.addVariant(_, pageDef, this._pageId) : pageDef.variants(_).get(_, this._pageId);
		}
		return null;
	}
});
