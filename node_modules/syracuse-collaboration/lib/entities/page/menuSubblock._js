...
exports.entity = {
	...
	$functions: {
		getNavigationPageResource: function(_, baseUrlProp, auth) {
			var sm = this;
			var items = sm.items(_).toArray(_);
			if (auth) items = items.filter_(_, function(_, it) {
				return !it.authorized || it.authorized(_, auth);
			});
			return {
				menuBlock: {
					$uuid: sm.$uuid,
					title: sm.title(_),
					items: items.map_(_, function(_, it) {
						return it.getNavigationPageResource(_, baseUrlProp, auth);
					})
				}
			};
		}
