...
function getX3Rights(_, ep, user, role, handler) {
	// ignore x3 rights
	if (config.system && config.system.enableDevelopmentFeatures && ep.x3RightsIgnore(_)) {
...
exports.entity = {
	...
	$properties: {
		...
		x3RightsIgnore: {
			...
			$type: "boolean",
			$isDefined: function(_, instance) {
				return config.system && config.system.enableDevelopmentFeatures;
			},
			$isNullable: true,
			$default: false
		},
...
*/
		...
		incrementalSyncExportMeta: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				require('etna-etl/lib/sync').newSync(instance.getEtnaConfig(_), context && context.tracker).exportMetadataAndPush(_, undefined, undefined, options);
			},
		},
		etnaImportMeta: {
