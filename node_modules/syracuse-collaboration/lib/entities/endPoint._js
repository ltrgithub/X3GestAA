...
exports.entity = {
	...
	$functions: {
		...
		getBoServer: function(_) {
			return this.x3server(_).boServer(_) || this.x3server(_).serverHost(_);
		},
		getBoServerRestPort: function(_) {
			return this.x3server(_).boServerRestPort(_);
		},
		getBoServerHttpPort: function(_) {
			return this.x3server(_).boServerPort(_);
		},
		getBoServerBaseUrl: function(_, secure) {
			var x3server = this.x3server(_);
			return (secure ? "https://" : "http://") + [(x3server.boServer(_) || x3server.serverHost(_)) + ":" + x3server.boServerPort(_)].join("/");
		},
		...
		getFusionPrototype: function(_, options) {
			...
			//
			var respData = response.readAll(_);
			//
			switch (response.statusCode) {
				case 200:
					return this.tryParsePrototype(_, respData.toString("utf8"));
				case 500:
					throw new Error(locale.format(module, "prototypeServerError", respData.toString("utf8")));
				default:
					return null;
			}
		},
