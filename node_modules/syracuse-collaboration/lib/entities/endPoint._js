...
var _authAccessCodeSolverMap = {
	...
	"x3": function(_, ep, user, role) {
		...
		if (cache) {
			...
			if (rights) rights.$etag = cache.etag(_);
		}
		var res = {
			$mode: "authorize"
		};
		try {
			...
			if (rights) {
				if (rights.$accessCodes && rights.$accessCodes !== "*") //
					res.$accessCodes = rights.$accessCodes.split(",").reduce(function(prev, key) {

						prev[key] = true;
						return prev;
					}, {});
				// TOTO: $representations, after patch2
			}
		} catch (e) {
...
var _authFunctionsSolverMap = {
	...
	"x3": function(_, ep, user, role) {
		...
		try {
			...
			// convert to functions structure
			if (rights) {
				if (rights.$authorizedFunctions && rights.$authorizedFunctions !== "*") //
...
var _authRepresentationSolverMap = {
	"syracuse": function(_, ep, user, role) {
		// compute forbidden representations from user rights
		var sp = role && role.securityProfile(_);
		return {
	...
	"x3": function(_, ep, user, role) {
		...
		try {
			...
			// convert to functions structure
			if (rights) {
				if (rights.authorizedRepresentations) //
...
exports.entity = {
	...
	$functions: {
		...
		getAuthorizedFunctions: function(_, user, role) {
			var p = this.protocol(_);
			return p && _authFunctionsSolverMap[p](_, this, user, role);
		},
		getAuthorizedAccessCode: function(_, user, role) {
			var p = this.protocol(_);
			return p && _authAccessCodeSolverMap[p](_, this, user, role);
		}
	},
	$services: {
		makeDefaultEndpoint: {
