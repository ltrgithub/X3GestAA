...
var _authFunctionsSolverMap = {
	...
	"x3": function(_, ep, user, role) {
		var adminDb = adminHelper.getAdministrationOrm(_);
		var db = ep.getOrm(_);
		...
		return {
			$mode: "authorize",
			$functions: ((rights || {}).$authorizedFunctions || "").split(",").reduce(function(prev, key) {
				prev[key] = true;
				return prev;
			}, {}),
			$representations: rights.$representations
		};
	}
};
		...
		getFusionPrototype: function(_, options) {
			...
			switch (response.statusCode) {
				case 200:
					...
					pp.$baseHelpUrl = [this.getBaseUrl(_, "help/" + locale.current.replace('-', '_')), "{$category}", "{$keyword}"].join("/");
					return pp;
				case 500:
