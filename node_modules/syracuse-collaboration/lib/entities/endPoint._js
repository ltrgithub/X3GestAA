	...
	$services: {
		...
		updateMenuProfiles: {
			...
			$parameters: {
				"$actions": {
					"$lookup": {
						"$url": "{$baseUrl}/roles?representation=role.$lookup"
					}
				}
			},
			$method: "POST",
			$execute: function(_, context, instance, parameters) {
				...
				mappings.forEach_(_, function(_, mapping) {
					...
					if (index >= 0) {
						...
						instance.$addMessage("info", locale.format(module, "mappingDeleted", mp));
					}
				});
				// add new instances
				if (codes.length) {
					var filter = ((parameters && parameters.$select) ? {
						jsonWhere: {
							$uuid: parameters.$select
						}
					} : {});
					console.error(JSON.stringify(filter));
					var role = instance._db.fetchInstance(_, instance._db.getEntity(_, "role"), filter);
					if (!role) instance.$addError(locale.format(module, "noRole"));
		...
		updateMappings: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				var codes = {};
				insts.forEach_(_, function(_, inst) {
					console.log("ZZ " + inst.CODMET(_) + " " + inst.PRFMEN(_));
