	...
	$properties: {
		...
		x3SolutionName: {
			...
			$compute: function(_, instance) {
				return instance.x3solution(_) && instance.x3solution(_).solutionName(_);
			}
		},
		transitionWebServer: {
	...
	$functions: {
		...
		getFusionDataServerBaseUrl: function(_, secure) {
			...
			if (!this.transitionWebServer || !this.transitionWebServer(_)) {
				var solutionDesc = _getSolutionDescriptor(_, this);
				solutionWebServer = this.transitionWebServer(_) || (solutionDesc && solutionDesc.webServers[0] && (solutionDesc.webServers[0].server + ":" + solutionDesc.webServers[0].mainPort)) || (x3solution && ((x3solution.webServer(_) || x3solution.serverHost(_)) + ":" + x3solution.webServerPort(_)));
			...
			$execute: function(_, context, instance) {
				...
				try {
					instance.$diagnoses = require("syracuse-x3/lib/pool").checkServerSettings(_, {
						...
						applicationServer: solution.serverHost(_)
					});
				} catch (e) {
				...
				var solutionName = instance.x3SolutionName(_);
