...
function _getSolutionDescriptor(_, endpoint) {
	...
	if(endpoint.x3SolutionName(_)) {
		...
		if(resp.statusCode === 200) {
			endpoint._solutionDescriptor = JSON.parse(resp.readAll(_));
		}
	}
	// compatibility, if solution.json not present
	if(endpoint._solutionDescriptor) return endpoint._solutionDescriptor;
	//
...
exports.entity = {
	...
	$functions: {
		...
		useProxy: function(_) {
			return (this.protocol(_) !== "syracuse");
		},
		getProxyMapEntry: function(_) {
			return _proxyMap[this.protocol(_)] && _proxyMap[this.protocol(_)](_, this);
//			return null;
		},
		//
