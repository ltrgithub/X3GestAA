...
function _getSolutionDescriptor(_, endpoint, silent) {
	...
	if (endpoint.getSolutionName(_)) {
		...
		if (!(endpoint.x3solution(_) && endpoint.x3solution(_).proxy(_))) options.ignoreProxy = true;
		try {
...
exports.entity = {
	...
	$properties: {
		...
		x3SolutionName: {
			...
			//				if (instance.x3solution(_)) {
			//					return instance.x3solution(_).solutionName(_);
			//				}
			//			}
		},
		x3Historic: {
	...
	$functions: {
		...
		getSolutionName: function(_) {
			if (this.x3solution(_)) return this.x3solution(_).solutionName(_);
		...
		getWebServerBaseUrl: function(_, withFolder, secure) {
			...
			if (x3solution)
				return (secure ? "https://" : "http://") + [
					(x3solution.webServer(_) || x3solution.serverHost(_)) + ":" + x3solution.webServerPort(_),
					"Adonix_" + this.getSolutionName(_)
				].join("/") + (withFolder ? "/" + this.x3ServerFolder(_) : "");
		},
		...
		postService: function(_, service, parameters) {
			var orm = this.getOrm(_, true);
			return (orm && orm.postService) ? orm.postService(_, service, parameters) : undefined;
		},
		...
		getFolderDescription: function(_) {
			...
			if (!(this.x3solution(_) && this.x3solution(_).proxy(_))) options.ignoreProxy = true;
			try {
				var req = httpClient.httpRequest(_, options);
				req.setTimeout(500);
				var resp = req.end().response(_);
					...
					if (folders.folders) {
						for (var i = 0; i < folders.folders.length; i++) {
							if (folders.folders[i] && folders.folders[i].name === x3ServerFolder) {
								console.error("FOLDER: " + JSON.stringify(folders.folders[i], null, 2));
	...
	$services: {
		...
		checkServer: {
			...
			$execute: function(_, context, instance) {
				...
				hasErrors = solution.$diagnoses && solution.$diagnoses.some(function(d) {
					return d.$severity === "error";
				});

				var solutionName = instance.getSolutionName(_);
				var des = _getSolutionDescriptor(_, instance);
				if (des.solution.name !== solutionName) instance.$addError(locale.format(module, "differentSolution", solutionName, des.solution.name));
				var url = instance.getWebServerBaseUrl(_) + "/FOLDERS.json";
				var options = {
					url: url,
					method: "GET"
				};
				if (!(instance.x3solution(_) && instance.x3solution(_).proxy(_))) options.ignoreProxy = true;
				var resp;
				try {
					var req = httpClient.httpRequest(_, options);
					req.setTimeout(500);
		...
		fullResyncAPLSTD: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/sync').newSync(instance.getEtnaConfig(_), context && context.tracker).syncText(_, "aplstd", {
					fullSync: true
				});
			},
		},
		fullResyncATEXTE: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/sync').newSync(instance.getEtnaConfig(_), context && context.tracker).syncText(_, "atexte", {
					fullSync: true
				});
			},
		},
	...
	$searchIndex: {
		$fields: ["description", "applicationRef", "dataset", "groups",
			"x3solution", "x3ServerFolder", "databaseDriver", "databaseHost"
		]
	}
};