...
var _modelMap = {
	"syracuse": function(_, ep, failIfNull) {
		//
		var contract = registry.getContract(
			ep.applicationRef(_).application(_), ep.applicationRef(_)
			.contract(_), failIfNull);
		if (!contract)
			return null;
		...
		if (rights) {
			if (rights.hasOwnProperty("$accessCodes")) {
				if (rights.$accessCodes === "*") { //
					...
					res.$accessCodes = rights.$accessCodes.split(",").reduce(
						function(prev, key) {
			...
			if (rights.hasOwnProperty("$authorizedFunctions")) {
				if (rights.$authorizedFunctions !== "*") { //
					res.$functions = rights.$authorizedFunctions.split(",")
			...
			if (rights.hasOwnProperty("$disabledFunctions")) {
				res.$disabledFunctions = rights.$disabledFunctions.split(",")
					.reduce(function(prev, key) {
						prev[key] = true;
						return prev;
					}, {}) || {};
...
function _getSolutionDescriptor(_, endpoint, silent) {
	...
	if (silent)
		return null;
	throw new Error(locale.format(module, "solutionDescriptorNotFound",
		endpoint.description(_)));
	// if(endpoint._solutionDescriptor) return endpoint._solutionDescriptor;
	//
	// var config = require("syracuse-main/lib/nodeconfig").config;
	// console.log("config.x3: " +sys.inspect(config));
	// return config && config.x3 && config.x3.solutions &&
	// (endpoint._solutionDescriptor =
	// config.x3.solutions[endpoint.x3server(_).serverHost(_)]);
}
...
exports.entity = {
	...
	$properties: {
		...
		databaseDriver: {
			...
			$isMandatory: function(_, instance) {
				return (instance.protocol(_) === "syracuse" && !instance
					.localDatabase(_));
			},
			$isDefined: function(_, instance) {
				return (instance.protocol(_) === "syracuse");
		...
		etnaDatabaseHost: {
			$title: "Database host",
			$isMandatory: function(_, instance) {
				return instance.useEtna(_) && instance.etnaDriver(_) === "oracle";
			},
			$isDefined: function(_, instance) {
				return instance.useEtna(_) && instance.etnaDriver(_) === "oracle";
			},
		...
		etnaSQLInstance: {
			...
			$isMandatory: function(_, instance) {
				return instance.useEtna(_) && instance.etnaDriver(_) === "sqlServer";
			},
			$isDefined: function(_, instance) {
				return instance.useEtna(_) && instance.etnaDriver(_) === "sqlServer";
			},
	...
	$functions: {
									...
									vigns
										...
										.forEach_(
											...
											function(_,
												...
												if (v
													.endpoint(_) && (v
														.endpoint(_).$uuid === self.$uuid)) {
													vigns
														.deleteInstance(
															_,
															v.$uuid);
													self
														.addRelatedInstance(d);
												}
											});
								});
					});
		...
		isSame: function(_, application, contract, dataset, host, port) {
			// TODO: host and port
			return ((application === this.application(_)) && (contract === this.contract(_)) && (dataset === this
				.dataset(_)));
		},
		getBaseUrl: function(_, prefix) {
			return ["/" + (prefix || "sdata"), this.application(_),
				this.contract(_), this.dataset(_)].join("/");
		},
		...
		getSolutionName: function(_) {
			...
			throw new Error(locale.format(module, "solutionNameNotFound", this
				.description(_)));
		...
		getFusionPrototype: function(_, options) {
			...
			try {
				...
				e.httpStatus = 500;
				e.message = locale.format(module, "prototypeServerError",
					options.prototypeId, e.message);
				throw e;
			}
		},
		...
		getFusionDataServerBaseUrl: function(_, secure) {
			...
			//
			return (secure ? "https://" : "http://") + [solutionWebServer, "sdata", "x3", "trans", "-"]
				.join("/");
		},
	...
	$services: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				if (app) {
					...
					if (!diag.some(function(d) {
						...
						instance.$addDiagnose("info", locale.format(module,
							"setAsDefault", app.description(_)));
				}
			}
		},
		checkServer: {
			...
			$execute: function(_, context, instance) {
				...
				if (!server)
					return;
				try {
					instance.$diagnoses = require("syracuse-x3/lib/pool")
						.checkServerSettings(_, {
				...
				if (des.solution.name !== solution)
					throw new Error(locale.format(module, "differentSolution",
						solution, des.solution.name));
				...
				if (resp.statusCode < 400) {
					...
					if (folders.solution !== solution)
						throw new Error(locale.format(module,
							"differentSolution2", solution,
							folders.solution));
					...
					if (i < 0)
						throw new Error(locale.format(module, "wrongFolder",
							x3ServerFolder));
				} else {
					var error;
					if (resp.statusCode === 404) {
						error = new Error(locale.format(module,
							"foldersNotFound", url));
					} else {
		...
		checkHelpBaseUrl: {
			...
			$execute: function(_, context, instance) {
				...
				if (response.statusCode !== 200) {
					throw new Error(locale.format(module, "helpUrlError",
						respData && respData.toString("utf8")));
				}
			}
		},
		etnaExportMeta: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/exporter').exportMetadata(_,
					instance.getEtnaConfig(_), context && context.tracker);
			},
		},
		etnaImportMeta: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/importer').importMetadata(_,
					instance.getEtnaConfig(_), context && context.tracker);
			},
		},
		etnaExportData: {
		...
		/*
		 * #4199: hide extra functionality updateMappings: { $title: "Update
		 * profession code mappings", $isMethod: true, $isDisabled: true, $method:
		 * "POST", $execute: function(_, context, instance, parameters) { if
		 * (instance.protocol(_) !== "x3") {
		 * instance.$addError(locale.format(module, "noX3Endpoint")); return; } var
		 * orm = instance.getOrm(_); var ent = orm.getEntity(_, "ASYRMET"); //
		 * console.error("T2 "+sys.format(ent)) var insts = orm.fetchInstances(_,
		 * ent, {
		 *
		 * }); var codes = []; insts.forEach_(_, function(_, inst) {
		 * codes.push(inst.CODMET(_)); }); var mappings0 =
		 * instance.menuProfileToRoles(_); var mappings = mappings0.toArray(_);
		 * mappings.forEach_(_, function(_, mapping) { var index =
		 * codes.indexOf(mapping.professionCode(_)); if (index >= 0) {
		 * codes.splice(index, 1); } else { // delete role
		 * mappings0.deleteInstance(_, mapping.$uuid); mapping.getAllDiagnoses(_,
		 * instance.$diagnoses); } }); // add new instances if (codes.length > 0) { //
		 * fetch arbitrary role var adminDb = adminHelper.getCollaborationOrm(_);
		 * var role = adminDb.fetchInstance(_, adminDb.getEntity(_, "role"), {
		 * jsonWhere: {} }); if (!role) { instance.$addError(locale.format(module,
		 * "noRole")); return; } codes.forEach_(_, function(_, code) { var mapping =
		 * mappings0.add(_); if ("menuProfile" in mapping) mapping.menuProfile(_,
		 * "Test"); mapping.professionCode(_, code); mapping.role(_, role);
		 * mapping.save(_); mapping.getAllDiagnoses(_, instance.$diagnoses); }); }
		 * instance.save(_); }, }
		 */
	},
	$searchIndex: {
