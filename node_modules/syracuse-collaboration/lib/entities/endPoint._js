...
function getX3Rights(_, ep, user, role, handler) {
	...
	try {
		...
		if (rights) {
			...
			// TOTO: $representations, after patch3
		}
	} catch (e) {
...
exports.entity = {
	...
	$properties: {
		...
		etnaDriver: {
			...
			$propagate: function(_, instance, val) {
				instance.etnaDatabaseUser(_, val === 'oracle' ? instance.x3ServerFolder(_) : "");
			},
		},
		etnaDatabaseHost: {
			$title: "Database host",
		...
		getEtnaConfig: function(_) {
			var config = {
				...
				sql: {
					...
					hostname: this.etnaDatabaseHost(_),
					port: this.etnaDatabasePort(_),
					user: this.etnaDatabaseUser(_),
					password: this.etnaDatabasePassword(_),
				}
			};
			if (config.sql.driver === "oracle") config.sql.database = this.etnaOracleSID(_);
			else config.sql.database = this.x3ServerFolder(_);
