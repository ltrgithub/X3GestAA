...
exports.entity = {
	...
	$properties: {
		...
		x3ServerFolder: {
			...
			$propagate: function(_, instance, val) {
				instance.etnaDatabaseUser(_, instance.etnaDriver(_) === 'oracle' ? val : "");
			},
			// $default: "superv",
			$lookup: {
	...
	$functions: {
		...
		getEtnaConfig: function(_) {
			...
			if (config.tenantId) config.mongo.database = config.tenantId + "-" + config.mongo.database;
			return config;
		},
	},
	$services: {
		...
		etnaExportMeta: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/exporter').exportMetadata(_, instance.getEtnaConfig(_), context && context.tracker);
			},
		},
		etnaImportMeta: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/importer').importMetadata(_, instance.getEtnaConfig(_), context && context.tracker);
			},
		},
		etnaExportData: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/sqldump').exportAll(_, instance.getEtnaConfig(_), context && context.tracker);
			},
		},
		importExportData: {
			...
			$execute: function(_, context, instance, parameters) {
				require('etna-etl/lib/sqldump').importAll(_, instance.getEtnaConfig(_), context && context.tracker);
			},
		},
