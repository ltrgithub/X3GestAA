	...
	$events: {
		$canSave: [function(_, instance) {
			...
			if (serverCert && !serverCert.keyExists(_)) {
				instance.$addError("noKeyServer$#$No private key in server certificate");
			}
			if (clientCert && !clientCert.keyExists(_)) {
				instance.$addError("noKeyClient$#$No private key in client certificate");
			}
			if (serverCert && clientCert) {
				...
				// get list of CA names of server certificate
					var names = serverCas.map_(_, function(_, serverCa) { return serverCa.name(_); });
