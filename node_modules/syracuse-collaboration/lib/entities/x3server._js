	...
	$properties: {
		...
		banned: {
			...
			$propagate: function(_, instance, val) {
				if (val) {
					instance.banTimeoutId(_, flows.setTimeout(function(_) {
					...
					instance.banTimeoutId && instance.banTimeoutId(_) && clearTimeout(instance.banTimeoutId(_));
				}
			}
		},
		errorTry: {
	...
	$functions: {
		...
		bannish: function(_) {
			...
			try {
				...
				if (this.errorTry(_) >= 5 && this.tag(_) !== "MAIN") {
					this.banned(_, true);
					...
					this.banned(_, true); // we bannish it temporary to don't reuse the same one on the second tentative
					var s = this._parent.save(_, null, {
						clearDiagnoses: true
					});
					var err = hasErrors(s);
