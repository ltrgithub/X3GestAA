	...
	$init: function(_, instance) {
		...
		if (instance.$diagnoses.length === 0) {
			...
			try {
				...
				var version = elasticVersion.getElasticVersion(_, _getUrlSearchEngine(_));
				instance.$diagnoses.push({
					$severity: "info",
					$message: locale.format(module, "elasticVersion", _getUrlSearchEngine(_), version)
				});
			} catch (e) {
				instance.$diagnoses.push({
					$severity: "error",
		...
		updateIndex: function(_, diags, tracker) {
			...
			function _update(_, helper, localeCode, isSyracuse) {
				...
				helper.updateIndex(_, instance.differentialUpdate(_), {
					...
					tracer: config && config.searchEngine && config.searchEngine.tracer && config.searchEngine.tracer.trace,
					entities: entis,
					locale: localeCode,
					tracker: tracker,
