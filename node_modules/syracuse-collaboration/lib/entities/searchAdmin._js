...
exports.entity = {
	...
	$init: function(_, instance) {
		...
		index.checkStartSearchEngine(_, instance.$diagnoses, true);
			...
			function _launchUpdate(_, ep, isSyracuse) {
				locArray.forEach_(_, function(_, loc) {
					...
					if (isSyracuse) {
						...
						var options = {
							url: index.serverConfig.baseUrl + "/sage.x3.functions.syracuse.collaboration.syracuse." + loc.code(_).toLowerCase() + "/_aliases",
							method: 'GET'
						};
						var req = streams.httpRequest(options);
						var resp = req.end().response(_);
	...
	$services: {
		...
		deleteFunctionsIndex: {
			...
			$confirm: "this operation will delete function index for selected endpoint, are you sure ?",
			$isMethod: true,
			$execute: function(_, context, instance) {
				elasticVersion.checkVersion(_, _getUrlSearchEngine(_));
				...
				var indexName = "sage.x3.functions" + "." + ep.application(_).toLowerCase() + "." + ep.contract(_).toLowerCase() + "." + ep.dataset(_).toLowerCase() + ".";
				instance.locales(_).toArray(_).forEach_(_, function(_, loc) {
					index.deleteIndex(_, null, indexName + loc.code(_).toLowerCase(), instance.$diagnoses);
				});

				indexName = "sage.x3.functions" + ".";
				instance.locales(_).toArray(_).forEach_(_, function(_, loc) {
					index.deleteIndex(_, null, indexName + loc.code(_).toLowerCase(), instance.$diagnoses);
				});
				if (instance.$diagnoses && instance.$diagnoses.length === 0) {
		...
		checkStartedSearchEngine: {
			$method: "PUT",
			$title: "Check search engine",
			$isMethod: true,
			$execute: function(_, context, instance) {
				index.checkStartSearchEngine(_, instance.$diagnoses);
			}
		},
		schedule: {
			$method: "POST",
