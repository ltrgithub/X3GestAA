...
exports.entity = {
	...
	$functions: {
		updateIndex: function(_, diags, tracker) {
			function _update(_, helper, localeCode) {
				helper.updateIndex(_, instance.differentialUpdate(_), {
					...
					entities: instance.entities(_).toArray(_).map_(_, function(_, r) {
						return r.entity(_);
					}),
					locale: localeCode,
					tracker: tracker,
					progressSlices: locArray.length,
			...
			console.log("endpoint " + endpoint.description(_));
			var locArray = instance.locales(_).toArray(_);
			var l_idx = 1;

			function _launchUpdate(_, ep) {
				locArray.forEach_(_, function(_, loc) {
					...
					_update(_, h, loc.code(_));
				});

			}

			_launchUpdate(_, endpoint);
			// endpoint choice 
			_launchUpdate(_, ep);
			//
