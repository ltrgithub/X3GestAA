...
exports.entity = {
	...
	$properties: {
		...
		representations: {
			...
			$isArray: true,
			$select: function(_, instance) {
				if(!instance.endpoint(_)) return;
				var options = {
					application: instance.endpoint(_).applicationRef(_),
					contract: instance.endpoint(_),
					representationField: "representation",
					entityField: "entity"
				}
				return adminHelper.getSelectRepresentations(_, options);
			}
/*			$select: {
