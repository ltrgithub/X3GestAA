...
exports.entity = {
	...
	$actions: {
		$save: function(_, instance) {
			...
			console.log("START"+connectionData.length)
			if (connectionData.some_(_, function(_, conn) {
				console.log("5II"+conn.$isDeleted+" "+conn.$created+" "+conn.active(_));
				//console.log("KKK "+util.format(Object.keys(conn)));
				if (conn.$isDeleted && !conn.$created && conn.active(_)) return true;
				console.log("5II"+conn.$isDeleted)
				if (!conn.$snapshot) return false;
				console.log("1II")
				var initConn = conn.$snapshot;						
				console.log("2II"+initConn.$isDeleted+" "+initConn.$created)
				if (!initConn.active(_)) return false;
				...
				if (cert && (!initCert || cert.$uuid !== initCert.$uuid) || !cert && initCert) return true;
