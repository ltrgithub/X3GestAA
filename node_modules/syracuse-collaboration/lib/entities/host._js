...
exports.entity = {
	...
	$services: {
		...
		deleteCertificate: {
			$method: "post",
			$title: "Remove server certificate",
			$isMethod: true,
			$execute: function(_, context, instance) {
				// delete certificate
				var db = adminHelper.getCollaborationOrm(_);
				var whereClause = "(name eq \"" + instance.hostname(_).toLowerCase() + "\")";
				var certs = db.fetchInstances(_, db.model.getEntity(_, "certificate"), {
					sdataWhere: whereClause
				});
				if (certs.length > 0) {
					certs[0].__syracuse_allow_delete__ = true;
					certs[0].deleteSelf(_);
				}
			}
		}
	},
	...
	$actions: {
		$save: function(_, instance) {
			...
			if (connectionData.some_(_, function(_, conn) {
				...
				if (conn.$isDeleted || !conn.active(_) || initConn.ssl(_) !== conn.ssl(_) || initConn.clientAuth(_) !== conn.clientAuth(_)) {
					return true;
				}
