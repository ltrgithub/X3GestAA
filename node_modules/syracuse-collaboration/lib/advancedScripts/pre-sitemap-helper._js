...
var reorderNavPageMod = function(_, db, navPageName, options) {
	...
	if (home) {
		...
		home.modules && home.modules.forEach_(_, function(_, m, idx) {
			...
			if (_m) {
				var _c = getApp(_, getMod(_, m._uuid).application._uuid).contract;
				reordered.splice(idx + (modulesOrder.indexOf(_c) * 100), 1, m);
			}
		});
...
exports.cleanNavPage = function(_, db, application, contract, options) {
	...
	//reorderNavPageMod(_, db, navPageName);
