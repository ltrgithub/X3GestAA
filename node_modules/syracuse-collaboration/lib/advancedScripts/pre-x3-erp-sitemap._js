...
_scripts[0] = function(_, db) {
	exports.tracer && exports.tracer("Executing pre script [0]; find X3 erp application");
	...
	if (!elts[0]) throw new Error("X3 hrm application not found");
	appUuid = elts[0]._id;
...
_scripts[2] = function(_, db) {
	...
	home.modules && home.modules.forEach(function(m) {
		if (modsUuids.indexOf(m._uuid) === -1) keep.push(m);
	});
	...
	// re import modules
	importHandler.jsonImport(_, db, "x3-erp-init.json", {
		importMode: "insert",
		$diagnoses: []
	});
