...
exports.getEndpoints = function(_, options) {
	...
	return db.fetchInstances(_, db.model.getEntity("endPoint"), options);
}
...
exports.getEndpoint = function(_, options) {
	...
	// get db endpoints
	if(options.jsonWhere)
		opt.jsonWhere = options.jsonWhere;
	else
		if(options.application && options.contract && options.dataset)
			// case insensitive search
			opt.jsonWhere = {
				application: {$regex: "^"+options.application+"$", $options: "i"},
				contract: {$regex: "^"+options.contract+"$", $options: "i"},
				dataset: {$regex: "^"+options.dataset+"$", $options: "i"}
			}
	var db = exports.getCollaborationOrm(_);
	return db.fetchInstance(_, db.model.getEntity("endPoint"), opt);
}
