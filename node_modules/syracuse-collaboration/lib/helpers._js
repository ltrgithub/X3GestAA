			...
			function addSubModule(_, subModule, father) {
				...
				subModule.items(_).toArray(_).forEach_(_, function(_, item) {
					if (item.$type === "menuSubblock" ||  item.$type === "menuBlock") {
						...
						var key = item.applicationName(_) + "." + item.contractName(_) + "." + (item.fusionFunction(_) ? item.fusionFunction(_) : item.representation(_));

						menuItemMap.menu[key] = menuItemMap.menu[key] ||  {
							instance: item,
							father: subModule.$uuid
						};
...
exports.AdminHelper = {
	...
	getBreadcrumb: function(_, lastModified, id, facet) {
		...
		if (current) {
			do {
				if (current.instance.$type !== "menuItem") { // don't send the last level

					// compute url of the menu
					if (current.instance) {
						var title = current.instance.title(_);
				...
				current = current.father ? menuItemMap.menu[current.father] : null;
