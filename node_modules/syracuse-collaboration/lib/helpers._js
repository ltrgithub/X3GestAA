...
function _buildMenuItemMap(_) {
	var model = exports.AdminHelper.getCollaborationModel(_);
...
exports.AdminHelper = {
	...
	getBreadcrumb: function(_, lastModified, endpoint, id, facet) {
		...
		if (current) {
			do {
				...
				if (current.instance) {
					...
					if (current.instance.$type === "menuItem" && ["$fusion", "$detail", "$edit"].indexOf(facet) !== -1) {
	...
	getCollaborationOrm: function(_) {
		...
		return require("syracuse-orm/lib/dataModel").getOrm(_, this.getCollaborationModel(), contract.datasets[dataset]);
	},
	...
	getEndpoints: function(_, options) {
		...
		if (options.jsonWhere) opt.jsonWhere = options.jsonWhere;
			...
			if (options.dataset) opt.jsonWhere.dataset = {
