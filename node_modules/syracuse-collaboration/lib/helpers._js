...
function _getProductDataByEndpoint(dataset, item) {
	if (multiTenant) {
		var arr = _productNameByEndpoint[globals.context.tenantId][dataset];
	} else {
...
exports.AdminHelper = {
	...
	getProductVersionByEndpoint: function(dataset) {
		return _getProductDataByEndpoint(dataset, 2);
	},
	getProductCodeByEndpoint: function(dataset) {
		return _getProductDataByEndpoint(dataset, 1);
	},
	setProductByEndpoint: function(productName, dataset, productCode, productVersion) { // call after connection
		if (multiTenant === undefined) {
			var hosting = require('config').hosting;
			multiTenant = (hosting && hosting.multiTenant) || false;
		}
		if (multiTenant)
			_productNameByEndpoint[globals.context.tenantId][dataset] = [productName, productCode, productVersion];
		else
			_productNameByEndpoint[dataset] = [productName, productCode, productVersion];
	},
	changeProductName: function(_, instance, val) {
		var ed;
		if (!val) {
	...
	getApplication: function(_, applicationName, contractName) {
		if (!_cache.applications[applicationName + "_" + contractName]) {
			var opt = {};
			...
			_cache.applications[applicationName + "_" + contractName] = db.fetchInstance(_, db.model.getEntity(_, "application"), opt);
		}
		return _cache.applications[applicationName + "_" + contractName];
	},
		...
		if (options && options.jsonWhere) opt.jsonWhere = options.jsonWhere;
			...
			if (options && options.dataset) {
				...
				if (enableDevelopmentFeatures) {
					if (_cache.endpoints[options.dataset] && (new Date().getTime() - _cache.endpoints[options.dataset].stamp > 15000)) {
						//console.error("Endpoint "+options.dataset+" cache expired");
						delete _cache.endpoints[options.dataset];
					}
					if (!_cache.endpoints[options.dataset]) {
						//console.error("Register endpoint "+options.dataset+" in cache");
						_cache.endpoints[options.dataset] = {
							stamp: new Date().getTime(),
							ep: db.fetchInstances(_, db.model.getEntity(_, "endPoint"), opt)
						};
					} else {
						//console.error("Get endpoint "+options.dataset+" from cache");
						_cache.endpoints[options.dataset].stamp = new Date().getTime();
					}
					return _cache.endpoints[options.dataset].ep;
				}
			}
		}
