...
exports.AdminHelper = {
	...
	getStandardSetting: function(_) {
		...
		if (authent === "basic" || authent === "digest") {
			result.method = authent;
			var ldap = setting ? setting.ldap(_) : _config.session.ldap;
			if (ldap) {
				result.source = "ldap";
				if (authent === "digest") result.method = "basic";
			...
			if (setting) {
				...
				if (!_config.session.oauth2) throw new Error("No oauth2 data in configuration file");
			}
		}
		return result;
	},
	...
	getFactoryOwner: function(_, sp) {
		...
		if (sp && sp.authoringLevel(_) === "sage") return "SAGE";
		if (sp && sp.authoringLevel(_) === "partner") {
			if (!config || !config.partnerId) throw new Error("Partner ID missing in nodelocal configuration");
