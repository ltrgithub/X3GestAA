...
exports.AdminHelper = {
		...
		if (options && options.jsonWhere) opt.jsonWhere = options.jsonWhere;
			...
			if (options && options.dataset) {
				...
				if (!_cache.endpoints[options.dataset]) {
					...
					_cache.endpoints[options.dataset] = db.fetchInstances(_, db.model.getEntity(_, "endPoint"), opt);
				}
				return _cache.endpoints[options.dataset];
	...
	removeEndpointFromCache: function(dataset) {
		if (_cache.endpoints[dataset]) {
			...
			console.error("Endpoint " + dataset + " has been removed from cache");
		}
	},
