...
var nodeQUnit = require("test-runner/deps/node-qunit/lib/index");
...
function _runFile(_, file, verbose, results) {
	...
	try {
		var child = spawn(process.execPath, nodeOpts.concat([__dirname + "/../../../test-runner/deps/node-qunit/lib/bootstrap.js", JSON.stringify(opts)]));
...
function _runServer(_, files, verbose) {
	...
	return results.filter(function(tt) {
		return tt.errorStack != null;
	});
}
...
function runIt(_) {
	var arg = process.argv[2] || 1;
	//    console.log(arg);
	var files = ["syracuse-collaboration/test/server-special/navigationPageTest"];
	var ii = 0;
	while (ii++ < arg) {
		var res = _runServer(_, files, true);
		if (res.length) {
			console.error("GOT ERROR on iteration " + ii + ": \n" + JSON.stringify(res, null, 2));
			break;
		}
	}
