...
exports.setTracesOn = function(_, name, options) {
	try {
		options = options || {};
		...
		tracer.records[helpers.uuid.generate()] = {
			queue: q,
			level: traceHelper.methods.indexOf(options.level || "debug")
		};
		q.reader = q.reader.map(function(_, n) {
			return n.stamp + " | " + n.severity + " | " + n.message;
		});
		q.reader.pipe(!_, ez.devices.console.log);
	} catch (e) {
