...
exports.getCookie = function(_, baseUrl, login, pass, fullResponseReturn, status) {
	var response = new ez.devices.http.client({
		//		url: baseUrl + "/syracuse-main/html/main.html",
		url: baseUrl + "/sdata/syracuse/collaboration/syracuse/users?representation=user.$query",
		user: login || "admin",
		password: pass || "admin",
		headers: {
...
exports.get = function(_, cookie, url, statusCode, fullResponseReturn, headers) {
	var hd = {
		cookie: cookie,
...
exports.post = function(_, cookie, url, data, statusCode, fullResponseReturn, headers) {
	var response = ez.devices.http.client({
		method: "post",
		url: url,
...
exports.initializeTestEnvironnement = function(_, datasetName, port, noCompress) {
	...
	endPoint.getBaseUrl = function(_) {
		return "http://localhost:3004/" + [endPoint.contract.application, endPoint.contract.contract, datasetName].join("/");
	};
...
exports.createX3Endpoint = function(_, adminDb, folder) {
	var x3server = adminDb.getEntity(_, "x3server").createInstance(_, adminDb);
	x3server.description(_, "x3");
	x3server.serverHost(_, "localhost");
	...
	ep.description(_, "x3");
	ep.applicationRef(_, adminHelper.getApplication(_, "x3", "erp"));
	ep.x3server(_, x3server);
	ep.dataset(_, folder);
	ep.x3ServerFolder(_, folder);
	ep.x3SolutionName(_, "STD");
...
exports.getX3TestEndpointData = function(_) {
	// TODO
