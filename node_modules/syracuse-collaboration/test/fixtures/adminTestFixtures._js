...
exports.setTracesOn = function(name, options) {
	try {
		...
		var mod;
		if (moduleName || options.mod) {
			mod = {
				name: options.mod.name || moduleName,
				level: options.mod.level || "debug",
				enabled: options.mod.enabled || true
			};
		}

		var tracer = traceHelper.getTracer(name);
		if (options) options.name = tracerName;
		tracer._loadConfig(options || {
			name: tracerName,
			enabled: true,
			level: "debug",
			transport: "console"
		}, mod);
		tracer.transport = new defLogger();
	} catch (e) {
		console.error("adminTestFixtures.setTracesOn error: " + e.stack);
		throw e;
	}
};