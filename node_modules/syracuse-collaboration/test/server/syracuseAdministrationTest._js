...
asyncTest("create objects", 65, function(_) {
	requestCount++;
	var body;
...
//	body = get(_, cookie, "users('"+_findUuid(users, "login", "rex")+"')?representation=user.$details", 200);
	...
	var resp = post(_, cookie, "userProfiles/$template/$workingCopies?trackingId=" + uuid.generate(), {
		selectedLocale: {
			$url: contractUrl + "localePreferences(code eq \"en-US\")"
		}
	}, 201, true);
	strictEqual(resp.body.selectedLocale.code, "en-US", "Locale set as en-US ok");
