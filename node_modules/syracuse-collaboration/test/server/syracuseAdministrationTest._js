...
asyncTest("create objects", 72, function(_) {
	requestCount++;
	var body;
	cookie = getCookie(_);
	// check init script
...
asyncTest("link user and groups", 35, function(_) {
	requestCount++;
...
asyncTest("connect without role", 3, function(_) {
	function _addRoleToGroup(_, role, group) {
		var groupUuid = _findUuid(groups, "description", group);
		// Fetch user WC
	...
	var testcookie = getCookie(_, "yoda", "yoda", 403);
...
asyncTest("user profile test", 23, function(_) {
	// language part========
	cookie = getCookie(_, "yoda", "yoda");
	...
	strictEqual(resp.headers["content-language"], "en-us", "Locale header ok");
