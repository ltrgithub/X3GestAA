...
asyncTest("create objects", 68, function(_) {
	...
	endPoints.forEach_(_, function(_, item) {
		...
		data.$key = item.$uuid;
		data.$etag = body.$etag;
		data.applicationRef = {$uuid: (item.protocol === "syracuse" ? applicationId : applicationX3Id) };
		if(item.protocol === "x3") {
...
//	strictEqual(body.$authorUrl, baseUrl + contractUrl + "dashboardAuths('" + v3uuid + "')/$workingCopies?representation=dashboardAuth.$edit", "Got Variant 3 ok");
	...
	body = get(_, cookie, "pages('syracuse.collaboration.syracuse.d1.$dashboard,$page')", 200);
	// check variant uuid
	strictEqual(body.$authorUrl, baseUrl + contractUrl + "dashboardAuths('" + v1uuid + "')/$workingCopies?representation=dashboardAuth.$edit", "Got Variant 1 ok");
	//
	// ROLES TEST
	//
	start();
