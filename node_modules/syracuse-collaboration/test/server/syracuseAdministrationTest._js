...
function getCookie(_, login, pass) {
	var resp = adminTestFixtures.getCookie(_, baseUrl, login, pass, true);
...
asyncTest("create objects", 69, function(_) {
	requestCount++;
	var body;
	cookie = getCookie(_);
	// check init script
...
*/
asyncTest("link user and groups", 30, function(_) {
	requestCount++;
	// var cookie = getCookie(_);
	var body = null;
	//
...
asyncTest("create page datas", 19, function(_) {
	requestCount++;
	var body;
	// var cookie = getCookie(_);
	...
	body = put(_, cookie, body.$url, data);
	console.log("create page datas.create page 1 (633):" + sys.inspect(body, null, 5));
	...
	console.log("create page datas.create page 1:" + sys.inspect(body, null, 4));
	strictEqual(body.$authorUrl, baseUrl + contractUrl + "pageAuths('" + page1VarUuid + "')/$workingCopies?representation=pageAuth.$edit&pageContext=syracuse.collaboration.group.$details&device=desktop", "Author url test");
...
asyncTest("Dashboards fetch test", 11, function(_) {
	var body;
	//
	cookie = getCookie(_, "yoda", "yoda");
	// get user profile
	...
	body = get(_, cookie, "pages('syracuse.collaboration.syracuse.d1.$dashboard,$page')", 200);
	// check variant uuid
	strictEqual(body.$authorUrl, baseUrl + contractUrl + "dashboardAuths('" + v1uuid + "')/$workingCopies?representation=dashboardAuth.$edit&pageContext=syracuse.collaboration.d1.$dashboard&device=desktop", "Got Variant 1 ok");
	//
	// ROLES TEST
	//
	start();
...
asyncTest("user profile test", 22, function(_) {
	// language part========
	cookie = getCookie(_, "yoda", "yoda");
	// accept language should be from default language
	strictEqual(acceptLanguage.toLowerCase(), "fr-fr", "Accept language after first connect test");
	// get userProfile WC
	...
	strictEqual(resp.headers["content-language"].toLowerCase(), "fr-fr", "Content-Language ok");
	...
	acceptLanguage = resp.headers["content-language"];
	// check application prototype "title"
	var body = get(_, cookie, "$prototypes('application.$detail')", 200);
...
asyncTest("group add passing a code (error mngmt)", 7, function(_) {
	cookie = getCookie(_, "yoda", "yoda");
	...
	strictEqual(body.groups[0].description, "Universe", "Group Universe added");
...
asyncTest("Scheduler test", 1, function(_) {
	ok(true, "NYI");
	start();
});
