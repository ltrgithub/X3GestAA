	...
	getUserProfile : function(_) {
		return {
			user : function(_) {
				...
				return db.fetchInstance(_, db.model.getEntity("user"), {
					jsonWhere : {
						login : user
					}
				});
			...
			selectedLocale : function(_) {
				var db = adminHelper.getCollaborationOrm(_);
				var locale = db.fetchInstance(_, db.model.getEntity("localePreference"), { });
				return locale;
			}
		}
	},
...
asyncTest("Test lookups X3", 1 + lookupEnts.length * 2,	function(_) {
	...
	flows.each(_, lookupEnts, function(_, entName) {
		var entity = db.model.getEntity(entName);
...
asyncTest("Test lookup templatePurpose", 3 + templatePurposes.length * 4, function(_) {
	var db = adminHelper.getCollaborationOrm(_);
	var entity = db.model.getEntity("msoWordTemplateDocument");
	var instances = [];
	...
	entity = db.model.getEntity("lookupTemplatePurposes");
	...
	flows.each(_, instances, function(_, purpSaved) {
		...
		flows.each(_, purposes, function(_, purpLookup) {
			if (purpLookup.name(_) === purpSaved.templatePurpose(_))
				match = true;
		});
	...
	flows.each(_, instances, function(_, purpSaved) {purpSaved.deleteSelf(_);});
