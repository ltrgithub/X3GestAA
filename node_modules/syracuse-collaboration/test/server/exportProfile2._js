...
asyncTest("very simple entity: entityAttribute", 16, function(_) {
	...
	coll.$resources.forEach_(_,function(_,el){
		...
		item.exportedObjects.push({$uuid : el.$uuid }) ;
	});
	...
	var profileInst = db.fetchInstance(_, entity, {jsonWhere:{code:"VSEP"}});
	var entityAttr=model.getEntity("entityAttribute") ;
	jsonExport.jsonExport(_,profileInst,"verySimpleExportProfile" );
	...
	testAttributes.forEach_(_,function(_,prop){
		...
		//var body = get(_, cookie, "entityAttributes('"+prop.$uuid +"')", 200);
		
		var obj=db.fetchInstance(_,entityAttr,{jsonWhere: { name: prop.name} }) ;
		strictEqual( obj,null,"record destroyed");

	});

	var diag = [];
	jsonImport.jsonImport(_, db, "very-simple-export-profile.json", {});
	...
	testAttributes.forEach_(_,function(_,prop){
		...
		//var body = get(_, cookie, "entityAttributes('"+prop.$uuid +"')", 200);
		//console.log("After import: get entity attribute: "+ sys.inspect(obj))  ;
	});
...
asyncTest("combined key export profile, key properties not included in property selection ", 16, function(_) {
	...
	var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
	var model=db.model ;
	jsonImport.jsonImport(_, db, "combined-key-import-profile.json", {});//{$diagnoses:diag, tracer:console.log}
	...
	var entity=model.getEntity("exportProfile") ;
	var profileInst = db.fetchInstance(_, entity, {jsonWhere:{code:"CKEP"}});
	var entityApp=model.getEntity("application") ;
	jsonExport.jsonExport(_,profileInst,"combined-key-export-profile" );

	originals.forEach_(_,function(_,app){
		if (app.application=="syracuse" && app.contract=="collaboration")
			return ;
		//console.log("deleting " + prop.name + prop.$uuid) ;
		var body = del(_, cookie, "applications('"+app.$uuid +"')", 200);
			
		var obj=db.fetchInstance(_,entityApp,{jsonWhere: { application: app.application,
	...
	var diag = [];
	jsonImport.jsonImport(_, db, "combined-key-export-profile.json", {});
	...
	originals.forEach_(_,function(_,app){
		...
		var obj=db.fetchInstance(_,entityApp,{jsonWhere: { application: app.application,contract: app.contract}}) ;
		//console.log("obj->application"+obj.application(_) + " protocol: "+obj.protocol(_)) ;
		
		//var apps =get(_,cookie,"applications?where=(application like '"+app.application+ "' and contract like '"+app.contract+"')") ;
...
asyncTest("single reference relation", 8, function(_) {
	
	cookie = getCookie(_);
	var diag = [];
	var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
	var model=db.model ;
	jsonImport.jsonImport(_, db, "single-ref-relation-import-profile.json", {});
	...
	var expProfile ={
		description: "Single Reference Relation Export Profile  ",
		code : "SRREP",
		applicationName: "syracuse",
		endpoint: adminEp,
		application : {
	...
	var entity=model.getEntity("exportProfile") ;
	var profileInst = db.fetchInstance(_, entity, {jsonWhere:{code:"SRREP"}});
	
	jsonExport.jsonExport(_,profileInst,"single-ref-relation-export-profile" );

	
	var input = JSON.parse(fs.readFile(fsp.join(__dirname, //
	 "../../../../import","single-ref-relation-export-profile.json"), //
	 "utf8", _));
...
asyncTest("plural reference relation", 16, function(_) {
...
asyncTest("single child relation", 16, function(_) {
	...
	var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
	var model=db.model ;
	/*jsonImport.jsonImport(_, db, "simple-child-relation-import-profile.json", {});
	...
	var expProfile ={
		description: "Simple Child Relation Export Profile  ",
		code : "SCREP",
		applicationName: "syracuse",
		endpoint: adminEp,
		application : {
	...
	var profileInst = db.fetchInstance(_, entity, {jsonWhere:{code:"CKEP"}});
	var entityApp=model.getEntity("application") ;
	jsonExport.jsonExport(_,profileInst,"simple-child-relation-export-profile" );
	...
	var input = JSON.parse(fs.readFile(fsp.join(__dirname, "../../../import",s"imple-child-relation-export-profile.json"), "utf8", _));
...
asyncTest(" plural child relation", 16, function(_) {
	...
	var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
	var model=db.model ;
	/*jsonImport.jsonImport(_, db, "combined-key-import-profile.json", {});
	...
	asyncTest("nested child relation", 16, function(_) {
		start() ;
	});
