...
asyncTest("very simple entity: entityAttribute", 10, function(_) {
	try{
	cookie = getCookie(_);
	...
	jsonExport.jsonExport(_,profileInst,{targetType: file, path : "../node_modules/syracuse-collaboration/test/fixtures/very-simple-export-profile.json"} );
...
asyncTest("combined key export profile, key properties not included in property selection ", 16, function(_) {
	...
	jsonExport.jsonExport(_,profileInst,{targetType :file, path :"combined-key-export-profile"} );
...
var body = post(_, cookie, "exportProfileItems/$template/$workingCopies?trackingId=" + uuid.generate(), {});
	...
	body = put(_, cookie, body.$url, data);
		...
		jsonExport.jsonExport(_,profileInst,{targetType : "file", path : exportFile,$diagnoses:diag, tracer:console.log} );
