...
var appli = adminHelper.getApplication(_, "syracuse", "collaboration");
	...
	var adminEp =_findUuid(endPoints,"description","Administration") ;
	var data = helpers.object.clone();
	adminEp.$uuid = body.$uuid;
	data.$key = adminEp.$uuid;
	data.$etag = body.$etag;
	...
	adminEp=get(_,cookie,"endPoints('"+_findUuid(endPoints, 'description', 'Administration')+"')") ;
} );



/*asyncTest("create dummy export profile: ", 27, function(_) {
	cookie = getCookie(_);
	var strings =[
	{
		stringNormal : "1st stringNormal",
		
	},
	{
		stringNormal : "2nd stringNormal",
		
	},

	] ;

	strings.forEach_(_, function(_, item) {
			var body = post(_, cookie, baseUrl + "/sdata/qunit/sdataTest/test/"+"strings/$template/$workingCopies?trackingId=" + uuid.generate(), {});
			...
			body = put(_, cookie, body.$url, data);
			console.log("STRINGS put "+JSON.stringify(body, null, 2 )) ;
	}); 
	...
	var dummyExpProfile ={
		description: "Dummy Export Profile",
		code : "Dummy",
		...
		exportProfileItem: exportItemsDummy,
	}


	exportItemsDummy.forEach_(_,function(_,item){
		console.log("ITEM CLASS NAME: "+item.className) ;
		var collection=dummyEntities[item.className] ;
		if (collection.length)
			item.exportedObjects=[] ;
		
		collection.forEach_(_,function(_,el){
			item.exportedObjects.push({$uuid : _findUuid(collection,"description",el.description)}) ;
		});
...
asyncTest("export dummy profile ", 12, function(_) {
	...
	var profileInst = db.fetchInstance(_, entity, {jsonWhere:{code:"Dummy"}});
	//console.log("profileInst: "+sys.inspect(profileInst));
	jsonExport.jsonExport(_,profileInst,"exportProfileDummy" );
