...
asyncTest("data setup", 13, function(_) {
	...
	// Create volume STD
	vol = db.getEntity(_, "storageVolume").createInstance(_, db);
	vol.code(_, "STD");
	vol.description(_, "STD");
	vol.save(_);



	//	var symulroles = users.symul.getUserRolesList(_);
	//	for (var i in symulroles) {
	//		console.log("ROLE: "+JSON.stringify(symulroles[i].serializeInstance(_),null,2));
	//	}
	//	
	//	
	//	var globalteamAuthors = teams.global.authors(_).toArray(_);
	//	for (var j in globalteamAuthors) {
	//		console.log("AUTHORS: "+JSON.stringify(globalteamAuthors[j].serializeInstance(_),null,2));
	//	}
	//	
	//	
	//	var globalteamUsers = teams.global.members(_).toArray(_);
	//	for (var k in globalteamUsers) {
	//		console.log("MEMBERS: "+JSON.stringify(globalteamUsers[k].serializeInstance(_),null,2));
	//	}
...
asyncTest("create restrictions tests", function(_) {
	// ========== Prints =============
	// try to create a print with phgat user, should get 201
	modifySampleDoc(_, users.phgat);
	var body = post(_, cookies.phgat, "printDocuments", sampleDoc, 201);
	...
	modifySampleDoc(_, users.cadbe, vol);
	...
	modifySampleDoc(_, users.phgat, vol);
	...
	modifySampleDoc(_, users.phgat, vol, teams.global);
	body = post(_, cookies.phgat, "documents", sampleDoc, 201);
...
asyncTest("read restrictions tests", function(_) {
	// ========== Prints =============
	var body = get(_, cookies.phgat, "printDocuments?representation=printDocument.$query", 200);
...
asyncTest("update restrictions tests", function(_) {

	// Who can update document that is not associated to a team
	var body = put(_, cookies.phgat, "documents('" + docs.phgat + "')?representation=document.$edit", {
...
asyncTest("delete restrictions tests", function(_) {
	...
	// ========== Documents =============	
	// Who can delete a document not assciated to a team ? phgat
