...
asyncTest("create sessions tests", 6, function(_) {
	...
	cookies.phgat = adminTestFixtures.getCookie(_, baseUrl, "phgat", "phgat");
	cookies.jglec = adminTestFixtures.getCookie(_, baseUrl, "jglec", "jglec");
	cookies.symul = adminTestFixtures.getCookie(_, baseUrl, "symul", "symul");
	cookies.cadbe = adminTestFixtures.getCookie(_, baseUrl, "cadbe", "cadbe");
	cookies.paric = adminTestFixtures.getCookie(_, baseUrl, "paric", "paric");
	start();

});

asyncTest("create restrictions on prints", function(_) {
	// ========== Prints =============
...
asyncTest("create restrictions on storage volumes", function(_) {
	// ========== Storage Volumes ===========
	// create a volume
	sampleVolume = {
		code: "SAMPLE",
...
asyncTest("create restrictions on teams", function(_) {
	// ========== Teams ===========
	...
	body = post(_, cookies.phgat, "teams", sampleTeam, 403);
	ok(!onlyInfo(body.$diagnoses), "Team creation forbidden with user that is administrator of the team (but no rights on users) OK (" + body.$diagnoses[0].$message + ")");

	body = post(_, cookies.jglec, "teams", sampleTeam, 403);
	ok(!onlyInfo(body.$diagnoses), "Team creation forbidden with user that is author of the team (but no rights on users) OK (" + body.$diagnoses[0].$message + ")");

	body = post(_, cookies.symul, "teams", sampleTeam, 403);
	ok(!onlyInfo(body.$diagnoses), "Team creation forbidden with user that is member of the team (but no rights on users) OK (" + body.$diagnoses[0].$message + ")");
...
asyncTest("create restrictions on documents", function(_) {
	// ========== Documents =============
	...
	ok(!onlyInfo(body.$diagnoses), "Document creation forbidden with paric that is not a member of the team but have rights on collaboration area OK (" + body.$diagnoses[0].$message + ")");
	...
	body = post(_, cookies.paric, "documents/$template/$workingCopies", sampleDoc, 403);
	ok(!onlyInfo(body.$diagnoses), "Document (WC) creation forbidden with paric that is not a member of the team but have rights on collaboration area OK (" + body.$diagnoses[0].$message + ")");
	...
	ok(!onlyInfo(body.$diagnoses), "Document (WC) save forbidden with paric that is not a member of the team but have rights on collaboration area OK (" + body.$diagnoses[0].$message + ")");
...
asyncTest("read restrictions on prints", function(_) {
	// ========== Prints =============
	...
	body = get(_, cookies.phgat, "printDocuments('" + prints.jglec + "')?representation=printDocument.$details", 404); // temporary 404
	strictEqual(body, null, "Print details not found with owner phgat OK");
	...
	body = get(_, cookies.jglec, "printDocuments('" + prints.phgat + "')?representation=printDocument.$details", 404); // temporary 404
	strictEqual(body, null, "Print details not found with owner jglec OK");

	start();
});


asyncTest("read restrictions on storage volumes", function(_) {
	// ========== Storage Volumes ===========
...
asyncTest("read restrictions on teams", function(_) {
	// ========== Teams ===========
	...
	body = get(_, cookies.paric, "teams('" + teams.sample + "')?representation=team.$details", 404);
	strictEqual(body, null, "Team details not found with user that is not member of the team (and have no rights on users) OK");
...
asyncTest("read restrictions on documents", function(_) {
	// ========== Documents =============	
	...
	body = get(_, cookies.paric, "documents('" + docs.phgat_admin + "')?representation=document.$details", 404); // temporary 404
	strictEqual(body, null, "Team's document (created by the administrator) details not found with user paric (not member) OK");
	...
	body = get(_, cookies.paric, "documents('" + docs.jglec_author + "')?representation=document.$details", 404); // temporary 404
	strictEqual(body, null, "Team's document (created by an author) details not found with user paric (not member) OK");
...
asyncTest("update restrictions on storage volumes", function(_) {
	// ========== Storage Volumes ===========
...
asyncTest("update restrictions on teams", function(_) {
	// ========== Teams =============
	// Who can read sample team ?
...
asyncTest("update restrictions on documents", function(_) {
	// ========== Documents =============
...
asyncTest("delete restrictions on prints", function(_) {
...
asyncTest("delete restrictions on storage volumes", function(_) {
	// ========== Storage Volumes ===========
...
asyncTest("delete restrictions on teams", function(_) {
	// ========== Teams =============	
...
asyncTest("delete restrictions on documents", function(_) {
	// ========== Documents =============	
	// Who can delete a document not associated to a team ? phgat
