...
asyncTest("data setup", 4, function(_) {
	...
	// get admin $uuid
	var admin = db.fetchInstance(_, db.getEntity(_, "user"), {
		jsonWhere: {
			login: "admin"
		}
	});
	...
	var properties = {
		...
		expiration: datetime.fromJsDate(new Date((new Date()).getTime() + 50000))
	};
	var document = sa.writeAll(_, properties, new Buffer("test", 'utf8'), "", null, "printDocument");
	printUuid = document.$uuid;
	ok(document != null && printUuid != null, "Print creation OK");

	start();
});

asyncTest("read restrictions tests", 16, function(_) {
	...
	var body = get(_, cookie, "printDocuments('" + printUuid + "')?representation=printDocument.$details", 200);
	strictEqual(body.description, "UnitTest", "Print description for admin ok");

	var cookie = adminTestFixtures.getCookie(_, baseUrl, "guest", "guest");
	...
	var resp = get(_, cookie, "printDocuments('" + printUuid + "')?representation=printDocument.$details", 403, true);
	strictEqual(resp.statusCode, 403, "Print description forbidden for guest ok");
...
asyncTest("delete restrictions tests", 13, function(_) {
	// full rights user - should pass
	var cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin");
	// create a group
	var body = post(_, cookie, "groups", {
	...
	var body = del(_, cookie, "groups(description eq 'Expendable')", 200);
	// check if deleted
	...
	var body = del(_, cookie, "groups(description eq 'Expendable')", 403);
	// check if deleted: must still be there
	body = get(_, cookie, "groups(description eq 'Expendable')", 200);

	// Try to delete print with guest - should fail
	var body = del(_, cookie, "printDocuments('" + printUuid + "')", 403);

	// Try to delete print with admin - should pass
	var cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin");
	var body = del(_, cookie, "printDocuments('" + printUuid + "')", 200);
...
asyncTest("Execute user rights test", 10, function(_) {
	...
	// entity service test
	var body = post(_, cookie, "importTools/$template/$workingCopies", {
		importSrc: "file",
	...
	var body = get(_, cookie, "exportProfiles(code eq \"TEST\")/$service/exportProfile", 403);
