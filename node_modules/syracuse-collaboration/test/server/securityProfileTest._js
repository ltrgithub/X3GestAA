...
asyncTest("read restrictions tests", 16, function(_) {
	...
	body = get(_, cookie, "printDocuments('" + printUuid + "')?representation=printDocument.$details", 200);
	strictEqual(body.description, "UnitTest", "Print description for admin ok");

	cookie = adminTestFixtures.getCookie(_, baseUrl, "guest", "guest");
	...
	resp = get(_, cookie, "printDocuments('" + printUuid + "')?representation=printDocument.$details", 403, true);
...
asyncTest("delete restrictions tests", 13, function(_) {
	...
	body = del(_, cookie, "printDocuments('" + printUuid + "')", 403);

	// Try to delete print with admin - should pass
	cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin");
	body = del(_, cookie, "printDocuments('" + printUuid + "')", 200);
