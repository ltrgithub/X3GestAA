...
function _getModel() {
	return dataModel.make(endPoint.contract, "mongodb_admin_test");
}
...
function putUser(target, login, givenName, email) {
  target[login] = { "sAMAccountName": login, "givenName": givenName, "sn": login+"_name", "mail": email, "l2": "l_"+login };
}
...
asyncTest("init database", 1, function(_) {
	...
	var _db = new mongodb.Db(endPoint.datasets["mongodb_admin_test"].database, server, {});
	_db = _db.open(_);
	_db.dropDatabase(_);
...
//		var db = adminHelper.getCollaborationOrm(_);
		...
		translationTable = ldap.getTranslationTable(_, ldapInstance);
	start();
});
...
asyncTest("add new users", 6, function(_) {
	...
	ldap.updateUsers(_, ldapInstance, translationTable, users, diagnoses, false);
...
asyncTest("change 1 user, remove one user", 4, function(_) {
	...
	ldap.updateUsers(_, ldapInstance, translationTable, users, diagnoses, true);
	strictEqual(diagnoses.length, 0, "No ORM messages");
	if (diagnoses.length > 0) {
		console.error("Diagnoses: "+ sys.format(diagnoses));
	}
...
// different settings
asyncTest("different attribute mappings", 9, function(_) {
//	var db = adminHelper.getCollaborationOrm(_);
	var entity = db.model.getEntity("ldap");
	var instance = entity.factory.createInstance(_, null, db);
	instance.name(_, "UNITUNITDIFF");
	instance.url(_, "ldap://ldap");
	instance.syncTranslationTable(_, "weiter:'wei,ter',login:l");
	var table = ldap.getTranslationTable(_, instance);
	strictEqual(table.login, "l", "login value changed");
	strictEqual(table.weiter, "'wei,ter", "Fixed value set");
	instance.syncTranslationTable(_, "login:l2, unsinn: test");
	table = ldap.getTranslationTable(_, instance);
	strictEqual(table.login, "l2", "login value changed");
	strictEqual(table.unsinn, "test", "test value changed");
	var diagnoses = [];
	instance.save(_);
	instance.getAllDiagnoses(_, diagnoses);
	strictEqual(diagnoses.length, 0, "ldap instance correctly saved");
	var users = {};
	putUser(users, "UNITUNIT6", "1", "email5@test"); 
	putUser(users, "UNITUNIT7", "1", "email6@test"); 
	ldap.updateUsers(_, instance, table, users, diagnoses, true);
	strictEqual(diagnoses.length, 1, "1 message after updateUsers");
	strictEqual(diagnoses[0].message, "Wrong attribute 'unsinn' will be ignored", "correct message value");
	
	// remove all users and new LDAP instance
	var ld = db.fetchInstances(_, entity, {jsonWhere: {name: "UNITUNITDIFF"}});
	instance = ld[0];
	
	var users2 = instance.users(_).toArray(_);
	strictEqual(users2.length, 2, "2 users in LDAP instance");
	
	for (var i=0; i < users2.length; i++) {
		users2[i].deleteSelf(_);
		users2[i].getAllDiagnoses(_, diagnoses);
	}
	
	instance.deleteSelf(_);
	instance.getAllDiagnoses(_, diagnoses);
	strictEqual(diagnoses.length, 1, "1 message");	
