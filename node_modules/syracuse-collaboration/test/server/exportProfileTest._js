	...
	function testCheck(_) {
			...
			var res = outdata.filter_(_, function(_, it) {
				...
				return keyattrs.every_(_, function(_, attr) {
					...
					if (initem[prop]) {
						...
						} else return (it[prop] === initem[prop]);
					} //else
		...
		function checkExportItem(_, it) {
				...
				function checkAttr(_, attr) {
					function checkProp(_, obj, idx) {
						...
						if (typeof obj === "Object") {
							...
							} else strictEqual(obj, outItem[prop], 'check item.prop: ' + prop);
						}
			...
			if (keyattrs.length === 1) {
				...
				prop = attr.name;
				if (isLocalized(inproto[entityName], prop)) ok(outproto[entityName].$key.inexOf(prop + '.' + locale.current) !== -1, "combined key check localized prop: " + prop);
				else ok(outproto[entityName].$key.inexOf(prop + '.' + locale.current) !== -1, "combined key check localized prop: " + prop);
			});
...
/*asyncTest("singular non child relation with localized key", 8,	function(_)  {
	var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
	var model = db.model;
	var entityRole= model.getEntity(_, "role");
	var entityGroup= model.getEntity(_, "group");
	var roledata = {
			description : {"en-us" : "first role","fr-fr" : "premier role"},//
			functionProfile :"aaaaaaaa" 
	} ;
	var roleinst = entityRole.createInstance(_, db, roledata);
	roleinst.save(_) ;
	var diags=[] ;
	roleinst.getAllDiagnoses(_, diags) ;
	ok(!hasErrors(diags),"role created");
	
	var groupdata = {
			description : {"en-us" : "first group","fr-fr" : "premier group"},//
			role :  roleinst
	};
	
	var groupinst = entityGroup.createInstance(_, db, groupdata);
	groupinst.save(_) ;
	diags=[] ;
	roleinst.getAllDiagnoses(_, diags) ;
	ok(!hasErrors(diags),"group created") ;
	
	roleinst=db.fetchInstance() ;
	
	var entityExpProfile = model.getEntity(_, "exportProfile");
	var entityExpProfileItem = model.getEntity(_, "exportProfileItem");
	
	var rolesProfItemData={
			className : "roles",
			title : "Roles",
			entityKeyAttribute : [],
			entityAttribute : [],
			exportedObjects : [roleinst ]
		} ;
	var rolesProfItem = entityExpProfileItem.createInstance(_, db, rolesProfItemData);
	rolesProfItem.save(_) ;
	
	
	//???????????? ai-je besoin du tout de cette entity? 
	//  var entityAttr=model.getEntity(_, "entityAttribute");
	//var attrInst=entityAttr.createInstance(db,{name: "description"}) ;
	//var attrInstrole=entityAttr.createInstance(db,{name: "role"}) ;
	//attrInst.save(_) ;
	
	
	var groupsProfItemData={
			className : "groups",
			title : "Groups",
			entityKeyAttribute : [],
			entityAttribute : [],
			exportedObjects : [groupinst ]
		} ;
	var groupsProfItem = entityExpProfileItem.createInstance(_, db, groupsProfItemData);
	
	groupsProfItem.save(_) ;
	...
	var expProfileData={ 
		description : "singular non child relation",
		code : "SRREP",
		applicationName: "syracuse",
		endpoint: adminEp,
		exportItems : [ rolesProfItem, groupsProfItem ]
	} ;

	var  expProfileInst=entityExpProfile.createInstance(_, db, expProfileData);
	expProfileInst.save(_) ;
