...
console.log("NEW DIR: "+resultsdir) ;
	...
	function testSetup(_) {
		...
		jsonImport.jsonImport(_, db, importFile, {$diagnoses:diag, tracer:console.log});// $diagnoses:diag, tracer:console.log
		console.log("Import done!") ;
		//var body = get(_, cookie, "portlets", 200);
		// console.log("exportProfileItem: "+ JSON.stringify(body,null,2)) ;
		//console.log("*********PORTLETS: "	+ JSON.stringify(body, null, 2));
		...
		tmp.forEach_(_,function(_,portl){
			...
			console.log("LOCALES "+sys.inspect(locs)) ;
		}) ;*/
		

		exportItems.forEach_(_, function(_, item) {
			console.log("exportItems.forEach") ;
			var coll = get(_, cookie, item.className, 200);
			...
			console.log("exportProfileItem post: "+ JSON.stringify(body,null,2)) ;
			...
			body = put(_, cookie, body.$url, data);
			 ...
			 //var body = post(_, cookie, "exportProfileItems", item);
			 console.log("exportProfileItem put: "+ JSON.stringify(body,null,2)) ;
		...
		//console.log("self.outproto "+sys.inspect(self.outproto)) ;
	...
	function testCheck(_) {
		...
		if (test.profileCode==='NSKEP') {
			...
			console.log("PATH(input):  "+infpath) ;
			var inputTxt=fs.readFile(infpath,"utf8",_) ;
			console.log("inputTxt: "+test.profileDescr +' >>>>>' +inputTxt) ;
			...
			//console.log("outfpath :"+outfpath) ;
			var outputTxt=fs.readFile(outfpath , "utf8",_);
			
			console.log("outputTxt "+test.profileDescr +' >>>>>' +outputTxt);
			var output = JSON.parse(outputTxt);
			console.log("OUTPUT: " +JSON.stringify(output,2,null) );
			var outproto = output.$prototypes || {};
			var outdata = output.$items || [];

			/*console.log("same (inproto, outproto) ? EXPORT PROFILE DESCRIPTION :" +expProfile.description) ;
			console.log("outproto: "+sys.inspect(outproto)) ;
			console.log("inproto: "+sys.inspect(inproto)) ;*/
			...
			flows.eachKey(_, inproto, function(_,key, entity){
				if (!selectedEntity(_,key)) 
					...
					console.log("checking the props of entity: "+key) ;
			...
			console.log("INPROTO après traitement: "+sys.inspect(inproto)) ;
