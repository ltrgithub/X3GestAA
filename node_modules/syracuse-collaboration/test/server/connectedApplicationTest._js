...
asyncTest("generate tokens", 3, function(_) {
...
site: SITE2`;
	...
	formattedUrl = connAppHelper.formatConnectedAppUrl(_, clientId, "/path/to/resource?site=SITE2", tokenParams2);
...
asyncTest("verify tokens", 26, function(_) {
	let db = adminHelper.getCollaborationOrm(_);
	let model = db.model;
	...
	ok(tokenInfos.length === 3, `Three tokens generated OK`);
