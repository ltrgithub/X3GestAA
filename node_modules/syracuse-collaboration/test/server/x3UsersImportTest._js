...
asyncTest("init database", 2, function(_) {
	...
	db.dropDatabase(_);
	var server = new mongodb.Server(testEndPoint.datasets.test.hostname, testEndPoint.datasets.test.port, {});
	var db = adminTestFixtures.newMongoDb("mongodb_demo", server, {});
	db = db.open(_);
	db.dropDatabase(_);
...
asyncTest("import user with group creation", 8, function(_) {
	// create sample user
	...
	ok(u.endpoints(_).toArray(_).some_(_, function(_, ue) {
		return (ue.endpoint(_).$uuid === stubEp.$uuid) && (ue.login(_) === "USR_LOGIN1");
	}), "Endpoint associated to USR1 ok");

	start();
});

asyncTest("import user with existing group detection", 8, function(_) {
	// create sample user
	var sUserEnt = samplesDb.getEntity(_, "AUTILISINI");
	var su = sUserEnt.createInstance(_, samplesDb);
	su.USR(_, "USR2");
	su.LOGIN(_, "USR_LOGIN2");
	su.INTUSR(_, "firstName, lastName");
	...
	// is endpoint login associated ?
	ok(u.endpoints(_).toArray(_).some_(_, function(_, ue) {
		return (ue.endpoint(_).$uuid === stubEp.$uuid) && (ue.login(_) === "USR_LOGIN2");
	}), "Endpoint associated to USR2 ok");

	start();
});

asyncTest("import user without endpoint map entry", 6, function(_) {
	// create sample user
	var sUserEnt = samplesDb.getEntity(_, "AUTILISINI");
	var su = sUserEnt.createInstance(_, samplesDb);
	su.USR(_, "USR3");
	su.LOGIN(_, "USR_LOGIN3");
	su.INTUSR(_, "firstName, lastName");
...
asyncTest("import several users with group creation", 6, function(_) {
	// create sample user
	var sUserEnt = samplesDb.getEntity(_, "AUTILISINI");
	var su = sUserEnt.createInstance(_, samplesDb);
	su.USR(_, "MU1");
	su.LOGIN(_, "MU_LOGIN1");
	su.INTUSR(_, "firstName, lastName");
