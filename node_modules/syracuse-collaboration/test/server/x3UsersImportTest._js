...
asyncTest("data setup", 1, function(_) {
	...
	var r = roleEnt.createInstance(_, adminDb);
	r.description(_, "R1");
	r.save(_);
	// endpoint
...
asyncTest("import user with group creation", 6, function(_) {
	// create sample user
	...
	imp.description(_, "GRP_CRE_ROLE_MAP");
	...
	// do it
	var diags = [];
	tracer && tracer("Before import user");
	imp.importUser(_, su, {
		$diagnoses: diags
	});
