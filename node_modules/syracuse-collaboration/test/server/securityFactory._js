...
asyncTest("data initialization", function(_) {
	...
	// try to create a main instance with user that have Partner factory profile but partner ID not configured
	main.id = "main2";
	body = post(_, cookies.partner, "mains", main, 201);
	ok(!onlyInfo(body.$actions.$save.$diagnoses), "'Main' instance creation fail with Partner factory profile if partner ID is not configured OK (" + body.$actions.$save.$diagnoses[0].$message + ")");
	...
	strictEqual(body.$factory, true, "Factory property set OK");
	strictEqual(body.$factoryOwner, config.partnerId, "Factory owner property set (" + body.$factoryOwner + ") OK");
...
asyncTest("Sage factory instance data updates with Partner factory profile", function(_) {
	...
	strictEqual(body.secondRel && body.secondRel.$uuid, seconds.second1, "Reject modification on singular relation OK");
	...
	ok(onlyInfo(body.$diagnoses), "Delta update using working copy on singular relation (factory allowed) granted OK");
	strictEqual(body.secondRelA && body.secondRelA.$uuid, seconds.second1, "Modify singular relation (factory allowed) thanks to $factoryExcludes OK");
