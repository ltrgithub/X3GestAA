...
asyncTest("standard profile export test", 9, function(_) {
...
asyncTest("navigation page scripts", 4, function(_) {
	var profileEnt = db.getEntity(_, "personalizationManagement");
	var profile = profileEnt.createInstance(_, db);
	profile.code(_, "P3");
	...
	strictEqual(res.$scriptsBefore[0].options.factoryOwner, "SAGE", "factory owner check ok");
...
asyncTest("landing pages import/export test", 14, function(_) {
	var diag = jsonImport.jsonImportFromJson(_, null, {
		$prototypes: lpProto,
		$items: [{
			"$type": "menuItem",
