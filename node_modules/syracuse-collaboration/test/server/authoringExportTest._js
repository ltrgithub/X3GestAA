...
asyncTest("navigation pages import test", 17, function(_) {
	...
	var impJson = JSON.parse(orgRes.replace(/B3/g, "B4").replace(/M2/g, "M3").replace("home2", "home3"));
...
asyncTest("landing pages import/export test", 14, function(_) {
	...
	//
	// lets change some codes, the exported data should reimport
	var impJson = JSON.parse(orgRes.replace(/home1/g, "home2").replace(/LP1/g, "LP2"));
	var diag = jsonImport.jsonImportFromJson(_, null, impJson, {
		tracer: tracer,
		importMode: "update"
	});
