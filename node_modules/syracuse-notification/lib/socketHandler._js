...
function getSocketByLogin(logins) {
	var res = [];
		...
		socket.on('connection', function(client) {
			...
			client.on('message', function(event) {
				(function(_) {
					...
					var message = eventManager.createMessage(_ >> _cb, event.title, event.message, event.to, event.severity);
					//
					if (event.to && Array.isArray(event.to)) {
						//						event.to.forEach(function(to) {
						//							if (clients[to]) clients[to].forEach(function(id) {
						//								tracer.info && tracer.info("* EMIT MESSAGE TO '" + to + "' on socket '" +id + "'");
						////console.log("SOCKETS: "+util.inspect(socket.sockets,null,4));
						//								var _s = socket.sockets.sockets.filter(function(s) {
						//									return s.id === id
						//								})[0];
						//								_s.emit('message', message);
						//							});
						//						});
						var _sockets = getSocketByLogin(event.to);
			...
			client.on('get events', function(count, fn) {
				(function(_) {
					tracer.info && tracer.info("* IO GET EVENTS");
					var events = eventManager.getEvents(_, count || 20);
					fn(events);
				})(_ >> _cb);
			});
