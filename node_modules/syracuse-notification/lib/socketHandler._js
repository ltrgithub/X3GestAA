		...
		socket.on('connection', function(client) {
			...
			client.on('get users', function(count, fn) {
				(function(_) {
					...
					var logins = Object.keys(clients);
					var users = [];
					logins.forEach_(_, function(_, login) {
						users.push({
							login: login,
							firstName: eventManager.getFirstName(_, login),
							connected: true
						});
					});

					eventManager.getOfflineUsers(_, logins).forEach_(_, function(_, u) {
						users.push({
							login: u.login,
			...
			client.on('disconnect', function(login) {
				tracer.info && tracer.info("* IO DISCONNECT: " + login);
