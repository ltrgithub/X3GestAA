...
function _deduceType(data) {
    ...
    var text = data;
    if (text[0] === "<") {
        if (text.slice(0, 4).toLowerCase() === "<xml") return "application/xml";
        if (text.slice(0, 16).toLowerCase() === "<!doctype html>") return "text/html";
    }
    return "text/plain";
}
            ...
            writer: function(_) {
                ...
                return {
                    write: function(_, data) {
                        ...
                        var type = _deduceType(data);
                        if (type) opt.headers["content-type"] = type;
                        if (!client) client = module.exports.client(opt);
                        if (data === undefined) {
                            return _end(_, client);
