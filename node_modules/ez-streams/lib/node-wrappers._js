...
exports.httpListener = function(listener, options) {
	...
	return function(request, response) {
		return globals.withContext(function(_) {
			return listener(new HttpServerRequest(request, options), new HttpServerResponse(response, options), _);
		})(_ >> function(err) {
			if (err) throw err;
		});
	};
...
function NetServer(serverOptions, connectionListener, streamOptions) {
	...
	var emitter = net.createServer(serverOptions, function(connection) {
			globals.withContext(function(_) {
				connectionListener(new NetStream(connection, streamOptions || {}), _);
			})(_ >> function(err) {
				if (err) throw err;
			});
		});
