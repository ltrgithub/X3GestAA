...
const enumDescriptor = require('etna/lib/x3js/enumDescriptor');
const codeGen = require('etna/lib/x3js/entity-codegen');
const ezm = require('ez-mongodb');

function fillInstance(_, superv, instance, metaData, fileData) {
	if (fileData) {
		if (!fileData.module) throw new Error(`${fileData.file}: no module`);
		instance.module(_, fileData.module || '');
		instance.name(_, fileData.name || '');
		instance.dictId(_, fileData.dictId || '');
		instance.title(_, fileData.title || '');
		instance.status(_, fileData.status || '4gl');
		instance.comment(_, fileData.comment || '');
		instance.source(_, fs.readFile(fileData.file, 'utf8', _));
	} else {
		instance.module(_, '');
		instance.name(_, '');
		...
		instance.comment(_, '');
		instance.source(_, '');
		entityHelper.fillFromMeta(_, instance, enumDescriptor, superv, metaData);
	}
}
...
function updateAll(_, instance, saving) {
	...
	if (instance.status(_) === '4gl') {
		instance.source(_, `// ${instance.dictId(_)}: source not available. X3JS enum name has not been set.`);
		// do not save to file.
		return;
	}
	...
	const filePath = entityHelper.sourceFilename(_, instance, 'enums', superv);
	...
	if (fs.exists(filePath, _, _)) {
		parsed = entityParser.parseFile(_, filePath, enumDescriptor);
	} else {
		const source = codeGen.template(_, enumDescriptor);
		parsed = entityParser.parseSource(_, source, filePath, enumDescriptor);
	}
	parsed.data = entityHelper.mergeData(parsed.data, enumDescriptor, instance.serializeInstance(_), '');
	const output = codeGen.generateSourceCode(_, parsed, enumDescriptor);
	...
	if (saving && instance.status(_) !== 'incomplete') codeGen.generateSourceFile(_, parsed.file, source);
}
	...
	$functions: {
		...
		missingMappings(_) {
			...
			this.entries(_).toArray(_).forEach_(_, (_, entry) => {
				if (!entry.name(_)) {
					missing.push(`${this.dictId(_)}.${entry.value(_)}: missing value mapping`);
				}
			});
			return missing;
		},
			...
			$execute(_, context) {
				...
				ezm.reader(superv.mongoStore.collection('APLSTD', _).find({
					...
					if (LANCHP !== metaData.LANCHP) {
						...
						fillInstance(_, superv, instance, metaData, byDictId[metaData.LANCHP]);
				...
				*/
				...
				enums.forEach_(_, (_, fileData) => {
					...
					instance.endpoint(_, endpoint);
					fillInstance(_, superv, instance, null, fileData);
					instance.save(_);
				});
...
entityHelper.addProperties(exports.entity, enumDescriptor);