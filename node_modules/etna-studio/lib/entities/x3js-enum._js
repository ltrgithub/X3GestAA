...
const x3jsUtil = require('etna/lib/api/util');
const entityHelper = require('etna/lib/api/entity-helper');
const entityParser = require('etna/lib/api/entity-parser');
const enumDescriptor = require('etna/lib/api/enum-descriptor').descriptor;
const codeGen = require('etna/lib/api/entity-codegen');
const ezm = require('ez-mongodb');

function updateAll(_, instance, saving) {
	entityHelper.fromCurrentSession(_, enumDescriptor).updateAll(_, instance, saving);
}
...
exports.entity = {
	...
	$canEdit: true,
	$capabilities: "",
	$titleTemplate: "Enum {name}",
	$listTitle: "List of enums",
	$descriptionTemplate: " ",
	$valueTemplate: " ",
	$properties: {
		module: {
	...
	$functions: {
		...
		missingMappings(_) {
			...
			const missing = [];
			this.values(_).toArray(_).forEach_(_, (_, entry) => {
				if (!entry.name(_)) {
