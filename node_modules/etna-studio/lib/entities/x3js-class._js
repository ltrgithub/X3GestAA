		...
		synchronize: {
			...
			$execute(_, context) {
				...
				ezm.reader(superv.mongoStore.collection('ACLASSE', _).find({})).forEach(_, (_, metaData) => {
					...
					try {
						fillInstance(_, helper, instance, metaData, byDictCode[metaData.CODCLA]);
						...
						console.error(`class ${metaData.CODCLA}: synchronization failed: ${ex.stack}`);
						tracker.$diagnoses.push({
							$severity: 'error',
							$message: `class ${metaData.CODCLA}: synchronization failed: ${ex.message}`,
						});
					}
