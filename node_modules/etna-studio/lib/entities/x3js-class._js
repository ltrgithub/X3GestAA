...
function sourceFilename(_, inst, superv) {
	const root = superv.directories.jsLocal || superv.directories.jsShared;
	return fsp.join(root, inst.module(_), 'classes', inst.name(_) + '.x3js');
}
	...
	Object.keys(desc.properties).forEach(k => {
		...
		switch (prop.type) {
			...
			case 'object':
				...
				break;
			case 'flags':
				Object.keys(prop.dictCodes).forEach(flag => {
					const name = k + util.pascalize(flag);
					if (d2[name] != null) d1[k] = d2[name];
				});
				break;
			case 'type':
				{
...
function generateCode(_, inst) {
	const superv = getSupervisor(_, inst.endpoint(_));
	const filePath = sourceFilename(_, inst, superv);
	var parsed;
	if (fs.exists(filePath, _, _)) {
		parsed = classParser.parseFile(_, filePath);
	} else {
		const source = codeGen.classTemplate(_, inst);
		parsed = classParser.parseSource(_, source, filePath);
	}
	parsed.data = mergeData(parsed.data, classDescriptor, inst.serializeInstance(_), '');
	codeGen.generateSourceFile(_, parsed);
}
...
exports.entity = {
	...
	$events: {
		$beforeSave(_, instance) {
			if (/^(4gl|incomplete|mapped)$/.test(instance.status(_))) {
