...
exports.entity = {
	...
	$properties: {
		...
		passed: {
			...
			$type: 'boolean',
			$compute(_, instance) {
				if (instance.globalError(_)) return false;
		...
		summary: {
			$title: "summary",
			$compute(_, instance) {
				const name = instance.unitTest(_).name(_);
				const err = instance.globalError(_);
				if (err) return err;
				...
				if (counters.caseErrors === 0) {
					return `${name}: ${counters.caseCount} test cases passed`;
				} else {
					return `${name}: ${counters.caseErrors}/${counters.caseCount} test cases failed`;
				}
			}
		},
	},
	...
	$functions: {
		fill(_, stamp, unitTest, result) {
			...
			this.stamp(_, stamp);
			this.duration(_, result ? result.stats.duration : 0);
			if (result) {
