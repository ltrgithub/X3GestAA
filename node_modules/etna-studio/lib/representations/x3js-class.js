'use strict';
const util = require('etna/lib/supervisor/util');
const classDescriptor = require('etna/lib/x3js/class-descriptor');

function block(list) {
	return {
		$items: list.map(name => ({
			$bind: name
		}))
	};
}

function flags(desc, name) {
	return Object.keys(desc.properties[name].dictCodes).map(k => name + util.pascalize(k));
}

function descNames(desc, name) {
	return Object.keys(desc.properties[name].properties).map(k => name + util.pascalize(k));
}

function events(names) {
	return names.map(k => ['events' + util.pascalize(k) + 'Enabled', 'events' + util.pascalize(k)])
		.reduce((res, elt) => res.concat(elt), []);
}

exports.representation = {
	$entityName: 'x3jsClass',
	$facets: {
		$details: {
			$layout: {
				$layoutType: 'tabs',
				$items: [{
					$category: 'section',
					$title: 'General',
					$layout: block(['module', 'name', 'title', 'dictCode', 'status', 'comment', 'endpoint',
						'activityCode', 'kind'
					]),
				}, {
					$category: 'section',
					$title: 'Behavior',
					$layout: {
						$layoutType: 'row',
						$items: [{
							$title: 'Capabilities',
							$layout: block(flags(classDescriptor, 'can')),
						}, {
							$title: 'Flags',
							$layout: block(flags(classDescriptor, 'is')),
						}, {
							$title: 'Contextual Properties',
							$layout: block(descNames(classDescriptor, 'contextProperties')),
						}],
					},
				}, {
					$category: 'section',
					$title: 'Mapping',
					$layout: {
						$layoutType: 'row',
						$items: [{
							$title: 'Table Mapping (4gl)',
							$layout: block(descNames(classDescriptor, 'table')),
						}, {
							$title: 'Table Definition (x3js)',
							$layout: block(['tableDef']),
						}],
					},
				}, {
					$category: 'section',
					$title: 'Properties',
					$layout: block(['properties']),
				}, {
					$category: 'section',
					$title: 'Collections',
					$layout: block(['collections']),
				}, {
					$category: 'section',
					$title: 'Events',
					$layout: {
						$layoutType: 'row',
						$items: [{
							$title: 'Global events',
							$layout: block(events(['init', 'read', 'control'])),
						}, {
							$title: 'Insert events',
							$layout: block(events(['insert', 'insertBefore', 'insertAfter', 'insertControlBefore', 'insertControlAfter', 'insertRollback'])),
						}, {
							$title: 'Update events',
							$layout: block(events(['update', 'updateBefore', 'updateAfter', 'updateControlBefore', 'updateControlAfter', 'updateRollback'])),
						}, {
							$title: 'Delete events',
							$layout: block(events(['delete', 'deleteBefore', 'deleteAfter', 'deleteControlBefore', 'deleteControlAfter', 'deleteRollback'])),
						}],
					},
				}, {
					$category: 'section',
					$title: 'Source',
					$layout: block(['source', 'scripts']),
				}, {
					$category: 'section',
					$title: 'Tests',
					$layout: block(['tests', 'unitTests']),
				}]
			}
		},
		$edit: {
			$copy: '$details'
		},
		/*
		$query: {
			$layout: {
				$items: [{
					$category: "section",
					$layout: {
						$items: [{
							$category: "section",
							$layout: {
								$items: [{
									$bind: "$resources",
									$layout: block(['module', 'name', 'title', 'dictCode', 'status', 'comment', 'endpoint']),
									$cardItem :{
										$position:"row",
										$layout :{
											$items: [{
												$layoutType: "row",
												$items: [{
													$bind: "activityCode"
												}, {
													$bind: "canRead"
												}]
											}]
										}
									}
								}]
							}
						}]
					}
				}]
			}
		}
		*/
	}
};
//console.error(JSON.stringify(exports.representation, null, 2));