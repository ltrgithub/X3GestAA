...
exports.sendMail = function(_, server, options) {
	...
	var smtpOptions = {
		host: server.host(_),
		secureConnection: server.secureConnection(_),
	};
	var port = server.port(_);
	if (port) smtpOptions.port = port;
	var name = server.clientHost(_);
	if (name) smtpOptions.name = name;
	if (server.authentication(_) === "password") {
		smtpOptions.auth = {
			user: server.user(_),
			password: server.password(_)
		};
	}
	// generate key
