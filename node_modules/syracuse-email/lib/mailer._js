...
/// To use this module do `var mailer = require('./mailer')`.  
...
/// ```
/// 
exports.send = function(_, data, options, transportOptions) {
	options = options || {};
	var db = adminHelper.getCollaborationOrm(_);
	var srvInst, theme;
	if (options.server) {
		var srvEntity = db.getEntity(_, "notificationServer");
		srvInst = db.fetchInstance(_, srvEntity, {
			sdataWhere: '(code eq "' + options.server + '")'
		});
		if (!srvInst) throw new Error("Notification server '" + options.server + "' doesn't exist");
	} else {
		var setEntity = db.model.getEntity(_, "setting");
		var settings = setEntity.fetchInstances(_, db)[0];
		srvInst = settings && settings.mailer(_);
		if (!srvInst) throw new Error("No default notification server set in global settings");
	}

	if (!options.noTheme) {
		var themeInst;
		if (options.theme) {
			var themeEntity = db.getEntity(_, "notificationTheme");
			themeInst = db.fetchInstance(_, themeEntity, {
				sdataWhere: '(name eq "' + options.theme + '")'
			});
			if (!themeInst) throw new Error("No notification theme named '" + options.theme + "'");
			theme = themeInst.name(_);
		} else {
			themeInst = srvInst.mailerTheme(_);
			theme = themeInst && themeInst.name(_);
		}
	} else {
		theme = null; // important to be null
	}

	srvInst.notify(_, data, options.view, {
		theme: theme,
