...
var tracer = require('@sage/syracuse-core').getTracer("email");
...
exports.entity = {
	...
	$valueTemplate: "{code}",
	$helpPage: "Administration-reference_Notification-Servers",
	$properties: {
		code: {
			$title: "Name",
			$isMandatory: true,
		...
		rejectUnauthorized: {
			$title: "Check server identity for TLS",
			$type: "boolean",
			$default: true,
	...
	$relations: {
		...
		cacerts: {
			$title: "CA certificates of mail server",
			$type: "caCertificates",
	...
	$functions: {
		notify: function(_, emailOptions, values, options) {
			options = options || {};
			...
			if (themeInst) {
				buildTheme(_, emailOptions, themeInst, options.isTemplate);
			}
			...
			var mailer = ezmailer.writer(this.mailerOptions(_), options.transportOptions);
		...
		mailerOptions: function(_) {
			...
			switch (this.authentication(_)) {
				...
				case 'password':
					options.auth = {};
					options.auth.user = this.user(_);
					options.auth.pass = this.password(_);
					break;
			}
			...
			$execute: function(_, context, instance, parameters) {
				...
				try {
					...
					instance.notify(_, {
						...
						signature: sig,
						link: {
							href: (context.secure ? "https://" : "http://") + (context.request.headers['x-forwarded-host'] || context.request.headers.host) + "/syracuse-main/html/main.html?url=" + context.baseUrl + "/notificationServers('" + instance.$uuid + "')?representation=notificationServer.$details",
