...
exports.entity = {
	...
	$valueTemplate: "{code}",
	$allowFactory: true,
	$helpPage: "Administration-reference_Notification-Events",
	$properties: {
		code: {
			$title: "Code",
			$isMandatory: true,
				...
				servers.forEach_(_, function(_, server) {
					try {
						...
						if (globals.context.request) {
							link = {
								href: (globals.context.secure ? "https://" : "http://") + (globals.context.request.headers['x-forwarded-host'] || globals.context.request.headers.host) + "/syracuse-main/html/main.html?url=/sdata/syracuse/collaboration/syracuse/notificationEvents('" + self.$uuid + "')?representation=notificationEvent.$details",
		...
		try {
			if (reg[2] === "user") {
				if (user[reg[3]]) {
					...
					tracer && tracer("User property not found: " + reg[3]);
			} else {
