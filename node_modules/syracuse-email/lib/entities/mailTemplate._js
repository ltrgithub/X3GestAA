...
exports.entity = {
	...
	$valueTemplate: "{name}",
	$allowFactory: true,
	$helpPage: "Administration-reference_Mail-Templates",
	$properties: {
		name: {
			$title: "Name",
			$isMandatory: true,
	...
	$relations: {
		theme: {
			...
			$description: "Any theme used by notification template must contains {{template}} mustache variable."
		},
		endpoint: {
			$title: "Endpoint",
			$type: "endPoint",
	...
	$functions: {
		sendMail: function(_, values, attachments) {
			var mailer = require('@sage/syracuse-lib/src/collaboration/entities/setting').getInstance(_, this._db).mailer(_);
			if (!mailer) throw new Error("mailer has not been configured");
			var theme = this.theme(_);
			var emailOptions = this.stringify(_);
			emailOptions.attachments = attachments;
			mailer.notify(_, emailOptions, values, {
				theme: theme && theme.name(_),
				isTemplate: true
			});
		},
...
exports.addMailTemplateService = function(entity) {
	entity.$services.mailWithTemplate = {
		...
		$execute: function(_, context, instance) {
			var id = context.parameters.$select;
			...
			function mapAttachments(_, atts, mandatory) {
				return atts.split(',').map_(_, function(_, k) {
					...
					if (!att.fileExists(_)) {
						...
						var rs = att.createReadableStream(_);
						rs.attName = k;
						return rs;
					}
			...
			try {
				...
				var att = mapAttachments(_, template.mandatoryAttachments(_), true) //
					.concat(mapAttachments(_, template.optionalAttachments(_), false));

				template.sendMail(_, instance.serializeInstance(_), att);
		...
		$parameters: {
			...
			$actions: {
				$lookup: {
					$url: "{$baseUrl}/mailTemplates?representation=mailTemplate.$lookup&where=(entityName eq '" + entity.name + "')",
				}
			},
			$properties: {
				$select: {},
