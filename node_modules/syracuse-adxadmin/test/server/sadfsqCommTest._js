...
test("read buffer SLBF : read a remote file", function() {
	...
	// get result buffer
	var buf = new Buffer([fs.readFileSync(__dirname + "/data/SLBF_read.json", {
		encoding: "utf-8"
	})]); // file that contains binary to check 
	console.log("-----> length " + buf.length);
	strictEqual(comm.hasError(buf.slice(0, 1)), false, "error missing ok"); // del byte that define if it's an error // not validate here 
	var len = comm._readBufSLBF(buf.slice(0, 4), 0, sadFsqFile);
	strictEqual(len, 0, " open File ");
	var buff = comm._readBufSLBF(buf, 1, sadFsqFile);
	strictEqual(sadFsqFile.binary.toString('hex'), buf, " open File ");
