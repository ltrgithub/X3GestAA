...
test("written buffer TOUF : open a remote file", function(_) {
	...
	buff = comm._writeBufTOUF(sadFsqFile, "r");
...
test("written buffer SLBF : read a remote file", function(_) {
	...
	var buff = comm._writeBufSLBF(2, 32768);
	var bytes = new Buffer([0x02, 0x00, 0x05, 0x05, 0x00, 0x00, 0x80, 0x00]);
	strictEqual(buff.toString('hex'), bytes.toString('hex'), "file TSLBF Buffer");
...
test("written buffer T_KUS : kill a remote process", function(_) {
	...
	var pid = 1223456;
	var buff = comm._writeBufTKUS(pid, 9);
...
test("written buffer SEBF : write remote file", function(_) {
	...
	var fd = 2;
	var buff = comm._writeBufSEBF(fd, filebin.slice(0, 150));
	...
	buff = comm._writeBufSEBF(fd, filebin.slice(151));
...
test("written buffer TPRM : get info of a remote file 2", function() {
	var sadFsqFile = new SadFsqFile({
		canonical: "@SUPERV.tmp/TCA_TEST$json"
	});
	var buff = comm._writeBufTPRM(sadFsqFile, "size");
	var bytes = new Buffer([0x00, 0x00, 0x1F, 0x07, 0x08, 0x00, 0x1B, 0x00, 0x19, 0x40, 0x53, 0x55, 0x50, 0x45, 0x52, 0x56, 0x2E, 0x74, 0x6D, 0x70, 0x2F, 0x54, 0x43, 0x41, 0x5F, 0x54, 0x45, 0x53, 0x54, 0x24, 0x6A, 0x73, 0x6F, 0x6E]);
	strictEqual(buff.toString('hex'), bytes.toString('hex'), "file " + sadFsqFile.canonical + " TPRM");
...
test("read buffer SLBF : read a remote file", function() {
	...
	var len = comm._readBufSLBF(buf.slice(1, 5), 0);
	strictEqual(len, 1418, " get length to read ok");
	var binary = new Buffer([]);
	var buff = comm._readBufSLBF(buf.slice(6), 1, binary);
...
test("read buffer TPAT : get path of a remote file", function() {
	...
	strictEqual(comm._hasError(buf.slice(0, 1)), false, "error missing ok"); // del byte that define if it's an error // not validate here 
	var len = comm._readBufTPAT(buf.slice(1, 2), 0); // size of buffer to read
	strictEqual(len, 65, "buffer size to read ok");

	var path = comm._readBufTPAT(buf.slice(2), 1);
...
test("read buffer TPRM : read file info", function() {
	...
	strictEqual(comm._hasError(buf.slice(0, 1)), false, "error missing ok"); // del byte that define if it's an error // not validate here 
	var size = comm._readBufTPRM(buf.slice(1, 5), "size"); // size of buffer to read
...
test("read buffer TERR : read error", function() {
	...
	var len = comm._readBufTERR(buf.slice(1, 2), 0); // size of buffer to read
	strictEqual(len, 53, "buffer size to read ok");

	var mess = comm._readBufTERR(buf.slice(2), 1);
