...
var sadMask = require('syracuse-adxadmin/lib/sadfsq/sadfsqClient').Mask;
...
function completeTest(param, i) {
	...
	asyncTest("rename file", function(_) {
		...
		try {
			sadfs.close(_, sadfs.open(_, fileOptions, "r"));
		} catch (e) {
			ok(true, e.message);
		}

		var fd = sadfs.open(_, fileOptionsBis, "r");
		ok(true, "file correctly renamed " + fileOptionsBis.name);
		sadfs.close(_, fd, "r");

		sadfs.rename(_, fileOptionsBis, fileOptions);
		try {
			sadfs.close(_, sadfs.open(_, fileOptionsBis, "r"));
		} catch (e) {
			ok(true, e.message);
		}

		fd = sadfs.open(_, fileOptions, "r");
		ok(true, "file correctly renamed " + fileOptions.name);
		sadfs.close(_, fd, "r");
	...
	asyncTest("open and close new file in system mode", 1, function(_) {
		var fd = sadfs.openSys(_, fileOptions, sadMod.O_APPEND | sadMod.O_WRONLY | sadMod.O_CREAT, 0x1c0);
		sadfs.close(_, fd);
		ok(true, "open and close file ok");
		start();
	});
