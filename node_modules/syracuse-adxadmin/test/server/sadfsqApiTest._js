...
function completeTest(param, i) {
	...
	asyncTest("client class instanciation", function(_) {
		...
		var recOptions = {
			...
			path: "node_modules/syracuse-adxadmin/lib/sadfsq/data"
		};
		...
		sadfs3 = new SadFsqClient(_, "ecvmdev2003x64", 1818, "SAGEFR\\apisu", "Sage-2010", false, recOptions);
	...
	asyncTest("write and flush", function(_) {
		...
		strictEqual(size, 69, "Size after flush: " + size);
		sadfs.close(_, fd);
	...
	asyncTest("write with lock and flush", function(_) {
		...
		var fd = sadfs.open(_, fileOptions, "w");
		sadfs.write(_, fd, buffer, 0, buffer.length, 0, true);
		ok(true, "Write...");
		var size = sadfs.stat(_, fileOptions, ["size"]).size;
		strictEqual(size, 0, "Size before flush: " + size);
		ok(true, "Flush...");
		...
		var buffer3 = new Buffer("buff", "utf-8");
		fd = sadfs.open(_, fileOptions, "r+");
		sadfs.write(_, fd, buffer2, 0, buffer2.length, 5, true);
		var buffer4 = new Buffer(4);
		sadfs.read(_, fd, buffer4, 0, 4, 5);
		console.log("buffer readed " + buffer4.toString('utf-8'));
		strictEqual(buffer4.toString("utf-8"), "test", "Write at the middle of the file OK");

		sadfs.write(_, fd, buffer3, 0, buffer3.length, 5, true);
		sadfs.read(_, fd, buffer4, 0, 4, 5);
		strictEqual(buffer4.toString("utf-8"), "buff", "Revert modification OK");
	...
	asyncTest("seek and get position", 14, function(_) {
		var fd = sadfs.open(_, fileOptions, "r");
		var position = sadfs.getPosition(_, fd);
		...
		sadfs.seek(_, fd, 1, 70, false);
		position = sadfs.getPosition(_, fd);
		strictEqual(position, 78, "Seek CUR positive too big: " + position);

		sadfs.seek(_, fd, 1, -38, false);
		position = sadfs.getPosition(_, fd);
		strictEqual(position, 40, "Seek CUR normal: " + position);
		try {
			sadfs.seek(_, fd, 1, 70);
		} catch (e) {
		...
		sadfs.seek(_, fd, 2, -8);
		position = sadfs.getPosition(_, fd);
		strictEqual(position, 61, "Seek EOF negative: " + position);

		sadfs.seek(_, fd, 2, 8, false);
		position = sadfs.getPosition(_, fd);
		strictEqual(position, 77, "Seek EOF positive: " + position);
		try {
			sadfs.seek(_, fd, 2, 8);
		} catch (e) {
