...
asyncTest("Client class instanciation", function(_) {
	...
	var recOptions = {
		recMode: sadUnitConfig.offline ? "PLAY" : "REC",
		fileName: "apiTest.json"
	};
	if (recOptions.recMode === "REC") recOptions["override"] = true; // always override file
	client = new SadFsqClient(_, connCtx.server, connCtx.port, connCtx.user, connCtx.pass, connCtx.secure);
...
asyncTest("Get file info", function(_) {
	...
	var res = client.getFileInfo(_, fileOptions[0], attributes);
