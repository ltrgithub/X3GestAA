...
exports.CommonClient = helpers.defineClass(function(_, options, hostname, port, user, password, secured) {
		...
		try {
			if (!this.stream) {
				if (this.recOptions && this.recOptions.recMode === "PLAY") {
					...
					this.stream = new OfflinePlayer(_, readJsonPlayer(_, configRecords.dumpPath + "/cvgRecords/" + this.recOptions.fileName), null);
				...
				this.communicator = new CommClass(_, this.stream);
	...
	__setSocketTimeout: function(prevent) {
