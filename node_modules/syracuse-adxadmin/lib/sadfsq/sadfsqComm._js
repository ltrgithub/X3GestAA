...
function getBuffers(order, params) {
	...
	buffers.lenUtil = {
		...
		buf: function() {
			...
			lenUtil.writeInt16BE(buffers.canonical.buf().length + order.len, 0);
			return lenUtil;
		}
	};
	...
	buffers.openModLen = {
		...
		buf: function() {
			...
			openModLen.writeInt16BE((params.openMod ? params.openMod.length : 0) + 2, 0);
			return openModLen;
		}
	};
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_getBufTOUP: function(sadFile, openMod) { //TODO
		...
		return bufferConcat([buffers.init, buffers.lenUtil, buffers.order, buffers.canonicalBufLen, buffers.canonicalLen, buffers.canonical, buffers.openModBufLenbuffers.openModLen, buffers.openModBufLenbuffers.openMod]);
