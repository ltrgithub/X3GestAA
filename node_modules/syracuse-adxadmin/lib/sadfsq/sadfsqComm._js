...
var T_SLBF = {
	descr: "read remote file",
	id: 5,
	len: 5,
	lenUtil: []
}; // read
var T_SEBF = {
	descr: "write remote file",
	id: 5,
	len: 5,
	lenUtil: []
}; // write 
...
function getBuffers(order, params) {
	...
	buffers.lenToRead = {
		...
		buf: function() {
			...
			buf.writeint32BE(params.lenToRead);
			return buf;
		}
	};
	buffers.binaryLen = {
		...
		buf: function() {
			var buf = new Buffer(4);
			buf.writeInt32BE(sadFile.binary.length);
			return buf;
		}
	};
	buffers.binary = {
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	processSEBF: function(_) { //TODO
		...
		return bufferConcat([buffers.fd, buffers.lenUtil, buffers.order, buffers.binaryLen, buffer.binary]);

	},

	// close remote open file
	_getBufSFER: function(sadFile) { // TODO
	...
	_getBufSLBF: function(_, sadFile, lenToRead) { // TODO
	...
	_getBufTKUS: function(signal) { // TODO
		var buffers = getBuffers(T_EFF, {
			sadFile: sadFile,
			signal: signal
		});
	...
	///
	///Req[0]		= 0
	///Req[1]+1		= Longueur totale à suivre
	///Req[3]		= T_OUP
	///Req[4]+1		= Longueur buffer nom de fichier + longueur du nombre de caratère (short)
	///Req[6]+1		= Nombre de caractère du nom de fichier (sans octet null)
	///Req[7]+		= Nom du fichier (forme cannonique)
	///Req[n]+1		= Longueur buffer mode d'ouverture + longueur du nombre de caratère (short)
	///Req[n+2]+1	= Nombre de caractère du mode (sans octet null)
	///Req[n+4]+	= mode d'ouverture (rw, a)
	_getBufTOUP: function(sadFile, openMod) { //TODO
	...
	/// Req[3] = T_PAT
	/// Req[4]+1 = Longueur buffer nom de fichier + longueur du nombre de caratère (short)
	/// Req[6]+1 = Nombre de caractère du nom de fichier (short)
	/// Req[8]+ = Nom du fichier (forme cannonique)
