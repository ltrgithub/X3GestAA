...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_processTOU: function(_, suf, sadFile, openMod) {
		...
		var suffix = suf;
		// write open order
		this.stream.write(_, this["_writeBuf" + suffix].apply(null, [sadFile, openMod]));
		tracer && tracer("END WRITE PROCESS TOU");
		// read result 
		var buf = this.stream.read(_, 1);
		tracer && tracer("END READ 1 PROCESS TOU");
		if (this._NoLogicalError(_, buf, sadFile)) {
			this["_readBuf" + suffix].apply(null, [buf, sadFile]);
		}
		tracer && tracer("END READ FINAL PROCESS TOU");
		tracer && tracer("END PROCESS TOU");
	},
	...
	_readBufTERR: function(buf, id, sadFile) {
		switch (id) {
			...
			case 1:
				sadFile.err.push(buf.toString("utf-8"));
				break;
			default:
				throw new Error("automate issue, can't read the right segment");
		}
	...
	processTERR: function(_, sadFile) { // TODO 
		...
		console.log("Size of error message: " + size);
