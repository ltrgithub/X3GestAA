...
function getBuffers(order, params) {
	...
	buffers.mask = {
		...
		buf: function() {
			var buf = new Buffer(4);
			buf.writeInt32BE(params.mask, 0);
			return buf;
		}
	};
	...
	buffers.offset = {
		...
		buf: function() {
			var buf = new Buffer(4);
			buf.writeInt32BE(params.offset, 0);
			return buf;
		}
	};
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_writeBufDPL: function(fd, cursor, offset) {
		...
		return bufferConcat([buffers.fd, buffers.lenUtil, buffers.order, buffers.cursor, buffers.offset]);
	},
