...
function getBuffers(order, params) {
	...
	buffers.binary = {
		...
		buf: function() {
			var len = params.lenToWrite > sadFile.binary.length ? sadFile.binary.length : params.lenToWrite;
			return sadFile.binary.slice(0, len);
		}
	};
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	/// #Connection
	/// 
	/// This chapter describes connection/disconnection sequences with AdxAdmin server component.  
	/// 
	/// 
	/// ##**Connection**
	/// 
	...
	_processTOU: function(_, suf, sadFile, openMod) {
		if (!this.stream) throw Error("stream null or undefined, need to process Connect");
		var suffix = suf;
		// write open order
	...
	_readBufSLBF: function(buf, sadFile) {},
