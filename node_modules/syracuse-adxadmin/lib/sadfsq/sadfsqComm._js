...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_processTOU: function(_, suf, sadFile, openMod) {
		...
		if (_NoLogicalError(_, buf, sadFile)) {
	...
	processSFER: function(_, sadFile) { // TODO
		this._checkStream();
		this.stream.write(_, _writeBufSFER(sadFile));;
		if (_NoLogicalError(_, buf, sadFile)) {
			// nothin
		}
	},
	...
	processTPAT: function(_, sadFile) { //TODO;
		this._checkStream();
		this.stream.write(_, _writeBufTPAT(sadFile));
		var buf;
	...
	processTPRM: function(_, sadFile, options) { //TODO
		this._checkStream();
		this.stream.write(_, _writeBufTPRM(sadFile, option));
		_NoLogicalError(_, this.stream.read(_, 1), sadFile);
	},
	...
	processSLBF: function(_, sadFile) { // TODO
		...
		if (this._NoLogicalError(_, buf, sadFile)) {
			...
			if (!size) throw Error("No file size found");
			// read file 
			var isError = false;
	...
	processSEBF: function(_, sadFile, byteByRequest) {
		...
		while (sadFile.binary.length != 0) {
			var buf = _writeBufSEBF(sadFile, byteToWriteByReq);
	...
	_readBufTERR: function(buf, id, sadFile) {
		var usefulBuf = buf.slice(1, buf.length);
		switch (id) {
			case 0:
				return buf.readUInt8(0);
			case 1:
				buf.slice(1, buf.length);
	...
	processTERR: function(_, sadFile) { // TODO 
		this._checkStream();
		this.stream.write(_, _writeBufTERR());
