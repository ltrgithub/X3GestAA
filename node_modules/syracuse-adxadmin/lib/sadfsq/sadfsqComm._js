...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_NoLogicalError: function(_, buf, sadFile) {
		if (this._hasError(buf)) {
			this.processTERR(_, sadFile, (256 - buf.readUInt8(0)));
	...
	_readBufTPAT: function(buf, id, sadFile) {
		switch (id) {
			...
			case 1:
				...
				sadFile.fullpath = buf.toString("utf-8");
				break;
			default:
	...
	processTPAT: function(_, sadFile) {
		...
		traceBuffer(0, "Error code", buf);
		if (this._NoLogicalError(_, buf, sadFile))
			this._readBufTPAT(this.stream.read(_, this._readBufTPAT(this.stream.read(_, 1), 0, sadFile)), 1, sadFile);
	...
	processSLBF: function(_, sadFile, lenToRead) {
		this._checkStream();
		this.stream.write(_, this._writeBufSLBF(sadFile, lenToRead));
		...
		while (sadFile.binary.length < size && !stop) {
			if ((i % 3) === 0) { // check logical Error 
				tracer && tracer("\n### Check Error  ###");
				stop = !this._NoLogicalError(_, this.stream.read(_, 1), sadFile);
			} else if ((i % 3) === 1) { // read size to read 
				len = this._readBufSLBF(this.stream.read(_, 4), 0, sadFile);
				tracer && tracer("\n### Read len : " + len + " ###");
			} else if ((i % 3) === 2 && len !== -1) { // read content
				tracer && tracer("\n### Read Stream : size to read " + len + " ###");
				buf = Buffer.concat([buf, this.stream.read(_, len)]);
				this._readBufSLBF(buf, 1, sadFile);
				len = -1;
			} else {
	...
	processTERR: function(_, sadFile, errId) { // TODO 
		tracer && tracer("\nCatch logical error");
		this._checkStream();
		...
		sadFile.err = x3errors.getError(errId) + " : ";
