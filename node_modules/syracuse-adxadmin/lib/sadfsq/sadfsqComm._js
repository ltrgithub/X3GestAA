...
var tracer = nodeconfig.config && nodeconfig.config.sadfsq && nodeconfig.config.sadfsq.protocol && nodeconfig.config.sadfsq.protocol.trace;
	...
	buffers.canonical = {
		...
		buf: function() {
			if (sadFile && sadFile.canonical) {
				...
				throw new Error("Canonical name must exists.");
			}
		}
	};
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_NoLogicalError: function(_, buf) {
		if (this._hasError(buf)) {
			var err = this.processTERR(_, (256 - buf.readUInt8(0)));
			throw new Error(err);
		}
		return true;
	},
	...
	processTOUF: function(_, sadFile, openMod) { //TODO
		return this._processTOU(_, "TOUF", sadFile, openMod);
	},
	/*
	...
	*/
	...
	/// ---
	/// 
	_writeBufTPAT: function(sadFile) {
	...
	processTPAT: function(_, sadFile) {
		this._checkStream();
		...
		traceBuffer(0, "Error code", buf);
		if (this._NoLogicalError(_, buf))
			return this._readBufTPAT(this.stream.read(_, this._readBufTPAT(buf.slice(1), 0)), 1);
	...
	/// ---
	/// 
	_writeBufTPRM: function(_, sadFile, info) { //TODO
	...
	_readBufTPRM: function(buf, info) {
	...
	processTPRM: function(_, sadFile, info) { //TODO
		this._checkStream();
		var b = this._writeBufTPRM(_, sadFile, info);
		//nconsole.log(b);
		this.stream.write(_, b);

		tracer && tracer("\n### Read buffer for order: T_PRM ###");
		var buf = this.stream.read(_, 5);
		traceBuffer(0, "Error code", buf.slice(0, 1));
		if (this._NoLogicalError(_, buf)) {
			return this._readBufTPRM(buf.slice(1), info);
		}
	},
	...
	/// ---
	/// 
	_writeBufSLBF: function(fd, lenToRead) { // TODO
	...
	_readBufSLBF: function(buf, id, binary) {
		switch (id) {
			case 0:
	...
	processSLBF: function(_, fd, lenToRead) {
		this._checkStream();
		this.stream.write(_, this._writeBufSLBF(fd, lenToRead));
		...
		while (buf.length < size) {
			if ((i % 3) === 0) { // check logical Error 
				tracer && tracer("\n### Check Error  ###");
				this._NoLogicalError(_, tmpbuf = this.stream.read(_, 5));
			} else if ((i % 3) === 1) { // read size to read 
				len = this._readBufSLBF(tmpbuf.slice(1), 0);
				tracer && tracer("\n### Read len : " + len + " ###");
			} else if ((i % 3) === 2 && len !== -1) { // read content
				tracer && tracer("\n### Read Stream : size to read " + len + " ###");
				buf = this._readBufSLBF(this.stream.read(_, len), 1, buf);
				len = -1;
			} else {
	...
	/// ---
	/// 
	_writeBufSEBF: function(fd, data) { //TODO
	...
	processSEBF: function(_, fd, data) {
		this._checkStream();
		// write all file 
		var buf = this._writeBufSEBF(fd, data);
	...
	_readBufTOUP: function(buf) { // TODO merge with TOUP , it's the same 
		traceBuffer(0, "File number", buf);
		// get file num 
		return buf.readInt8(0);
	},
	processTOUP: function(_, sadFile, openMod) { //TODO
		return this._processTOU(_, "TOUP", sadFile, openMod);
	},
	...
	/// ---
	/// 
	_writeBufTKUS: function(pid, signal) { // TODO
	...
	processTKUS: function(_, pid, signal) { // TODO
		this._checkStream();
		this.stream.write(_, this._writeBufTKUS(pid, signal));
	...
	processTERR: function(_, errId) { // TODO 
		tracer && tracer("Catch logical error");
		this._checkStream();
		tracer && tracer("\n### Create buffer for order T_ERR ###");
		var buf = this._writeBufTERR();
		traceBuffer(0, "Ask error", buf);
		...
		var size = this._readBufTERR(buf.slice(1), 0);
		err += this._readBufTERR(this.stream.read(_, size), 1);
