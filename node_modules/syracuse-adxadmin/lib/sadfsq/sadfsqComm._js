...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_checkRequireProcess: function(sadFile) {
		this._checkStream();
		if (sadFile.err || sadFile.err.length === 0) {
			throw new Error("error in automate process, sadFsqFile contains error");
		}
	},
	...
	_processTOU: function(_, suf, sadFile, openMod) {
		this._checkRequireProcess(sadFile);
		var suffix = suf;
		// write open order
	...
	_readBufTPRM: function(buf, sadFile, info) {
		var fileInfo = info ? info : "size";
		sadFile.fileInfo[fileInfo] = buf.readInt32BE(0);
	},
