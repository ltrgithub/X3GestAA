...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_writeBufConnect: function(params) { //TODO
		...
		tracer && tracer("length user " + bufLenUser.toString("hex"));
	...
	connect: function(_, user, password, secured) {
		...
		if (pid > -1) {
			...
			var errCode = buf.readInt32LE(1);
			var msg = this.stream.read(_, msgLen).toString('utf-8');
	...
	processTPRM: function(_, sadFile, options) { //TODO
		...
		if (this._NoLogicalError(_, this.stream.read(_, 1), sadFile)) {
			this._readBufTPRM(this.stream.read(_, 4), sadFile, options);
		}
	},
