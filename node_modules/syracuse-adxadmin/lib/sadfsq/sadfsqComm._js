...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	*/
	...
	processTPAT: function(_, sadFile) {
		...
		traceBuffer(0, "Error code", buf);
		if (this._NoLogicalError(_, buf))
			return this._readBufTPAT(this.stream.read(_, this._readBufTPAT(this.stream.read(_, 1), 0)), 1);
	...
	processSLBF: function(_, fd, lenToRead) {
		...
		while (buf.length < size) {
			if ((i % 3) === 0) { // check logical Error 
				tracer && tracer("\n### Check Error  ###");
				this._NoLogicalError(_, this.stream.read(_, 1));
			} else if ((i % 3) === 1) { // read size to read 
				len = this._readBufSLBF(this.stream.read(_, 4), 0);
				tracer && tracer("\n### Read len : " + len + " ###");
			} else if ((i % 3) === 2 && len !== -1) { // read content
				tracer && tracer("\n### Read Stream : size to read " + len + " ###");
				buf = this._readBufSLBF(this.stream(_, len), 1, buf);
				len = -1;
			} else {
