...
function bufferConcat(table) {
	...
	table.map(function(item, i) {
		if (item.buf) {
			var b = item.buf();
			tracer && tracer("Buffer[" + i + "] (" + item.descr + ") = " + b.toString('hex') + " len " + b.length);
...
exports.SadFsqComm = helpers.defineClass(function(_, stream) {
	...
	_hasError: function(buffer, isTERR) {
		if (isTERR) return (buffer[0] & 0x80) === 0;
	...
	_checkStream: function() {
		if (!this.stream) throw new Error("stream is nul of undefined");
	},
	...
	_writeBufConnect: function(params) { //TODO
		...
		var serviceId = SAD_ID;
		if (params.secure) serviceId = SAD_SECURE_ID;
		bufServiceId.writeInt8(serviceId, 0);
		tracer && tracer("service " + bufServiceId.toString("hex"));

		var bufUser = new Buffer(params.user, "utf-8");
		tracer && tracer("user " + bufUser.toString("hex"));

		var bufPass = new Buffer(params.pass, "utf-8");
		tracer && tracer("pass " + bufPass.toString("hex"));

		var bufLenUser = new Buffer(1);
		bufLenUser.writeInt8(bufUser.length, 0);
		tracer && tracer("lengt user " + bufLenUser.toString("hex"));
		var bufLenPass = new Buffer(1);
		bufLenPass.writeInt8(bufPass.length, 0);
		tracer && tracer("length pass " + bufLenPass.toString("hex"));

		var bufLenUtil = new Buffer(1);
		bufLenUtil.writeInt8(bufUser.length * 2 + bufPass.length + 3, 0);
		tracer && tracer("length util " + bufLenUtil.toString("hex"));
		return Buffer.concat([bufServiceId, bufLenUtil, bufLenUser, bufUser, bufLenUser, bufUser, bufLenPass, bufPass]);
	},
	...
	_processTOU: function(_, suf, sadFile, openMod) {
		...
		if (this._NoLogicalError(_, buf, sadFile)) {
			this["_readBuf" + suffix].apply(this, [buf, sadFile]);
		}
	...
	processSFER: function(_, sadFile) { // TODO
		...
		if (this._NoLogicalError(_, this.stream.read(_, 1), sadFile)) {
			// nothin
		}
	},
	...
	processTPAT: function(_, sadFile) { //TODO;
		...
		var buf;
		if (this._NoLogicalError(_, buf = this.stream.read(_, 1), sadFile))
	...
	/// ---
	/// 
	_writeBufTPRM: function(sadFile, option) { //TODO
	...
	_readBufTPRM: function(buf, sadFile, option) {
		sadFile.fileInfo[option] = buf.readInt32BE(0);
	},
	processTPRM: function(_, sadFile, options) { //TODO
		this._checkStream();
		this.stream.write(_, this._writeBufTPRM(sadFile, options));
		var buf;
		if (this._NoLogicalError(_, this.stream.read(_, 1), sadFile)) {
			this._readBufTPRM(this.stream.read(_, 4), sadFile, option);
		}
	},
	...
	_readBufSLBF: function(buf, id, sadFile) {
		switch (id) {
			case 0:
				tracer && tracer("### test read SLBF ###");
				var len = buf.readInt32BE(0); // length of buffer to read
				tracer && tracer("SLBF length in byte : " + buf.toString('hex') + " in integer " + len);
				return len;
			case 1:
	...
	processSEBF: function(_, sadFile, byteByRequest) {
		...
		while (sadFile.binary.length !== 0) {
			var buf = this._writeBufSEBF(sadFile, byteToWriteByReq);
			this.stream.write(_, buf); // write buf
			// read response 
		}
		this._NoLogicalError(_, this.stream.read(_, 1), sadFile);
	...
	_readBufTERR: function(buf, id, sadFile) {
		switch (id) {
			...
			case 1:
				var messBuf = buf;
				sadFile.err.push(messBuf.toString("utf-8"));
				break;
			default:
				throw new Error("automate issue, can't read the right segment");
		}
	...
	processTERR: function(_, sadFile) { // TODO 
		...
		if (this._hasError(stream.read(_, 1), true)) {

			this._readBufTERR(this.stream.read(_, this._readBufTERR(this.stream.read(_, 1), 0, sadFile)), 1, sadFile);
		} else {
			throw new Error("can't read a logical Error");
		}
	}
