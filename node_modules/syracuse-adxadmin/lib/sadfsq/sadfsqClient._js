...
exports.SadFsqClient = helpers.defineClass(function(_, hostname, port, user, password, secured, recOptions) {
	...
	close: function(_, fd) {
		...
		if (opened) {
			this.communicator.processSFER(_, opened.fd);
	...
	read: function(_, fd, buffer, offset, length, position) {
		...
		var lenToRead = fdOpened.offset + length > fdOpened.size ? fdOpened.size - fdOpened.offset : length;
		if (lenToRead > 0) {
			var buf = this.communicator.processSLBF(_, fdOpened.fd, lenToRead);
	...
	write: function(_, fd, buffer, offset, length, position) {
		this._connect(_);
		var opened = this.fdsOpen[fd];
		this.communicator.processSEBF(_, opened.fd, buffer);
	},
