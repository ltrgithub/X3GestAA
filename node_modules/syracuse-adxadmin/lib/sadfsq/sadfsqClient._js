...
exports.SadFsqClient = helpers.defineClass(function(_, hostname, port, user, password, secured, recOptions) {
	...
	/// ### write :  
	/// ``` javascript  
	/// sadfs.write(_, fd, buffer, offset, length, [position]);  
	/// ```
	...
	/// 
	/// * `path` destination absolute remote path
	/// 
	///
	/// return 1 if the directory is create , 0 else   
	///

	mkdir: function(_, fileOptions) {
		var cmd = "ae_mkdir";
		if (fileOptions && fileOptions.folder && fileOptions.volume && fileOptions.path) {
			// case ae_mkdir subpath volume application
			cmd += " " + fileOptions.path;
		} else if (fileOptions && fileOptions.folder && fileOptions.volume) {
			// case ae_mkdir application volume
			cmd += " " + fileOptions.path;
		} else if {
			fileOptions && fileOptions.path) {
			// case ae_mkdir fullpath
			cmd += " " + fileOptions.path;
		}
		this.exec(_, "ae_mkdir " + path);
	...
	_connect: function(_) {
		...
		try {
			...
			console.log("SadFsqClient connection error : " + e.stack);
