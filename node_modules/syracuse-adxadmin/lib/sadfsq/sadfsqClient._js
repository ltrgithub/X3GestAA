...
exports.SadFsqClient = helpers.defineClass(function(_, hostname, port, user, password, secured, recOptions) {
	...
	/// * `lock` boolean : determine if the write operation use lock. if it's not define, null or false , the lock and lock test is not applied   
	/// 
	...
	write: function(_, fd, buffer, offset, length, position, lock) {
		...
		if (opened) {
			if (lock) {
				if (!this.lock(_, fd, 2, this.fdsOpen[fd].size, 0)) {
			...
			if (lock) {
				if (!this.lock(_, fd, 0, this.fdsOpen[fd].size, 0)) {
