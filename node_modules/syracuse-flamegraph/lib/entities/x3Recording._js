		...
		start: {
			...
			$execute: function(_, context, instance) {
				try {
					...
					var dump = instance.memoryDump(_, endpoint);
					if (dump) {
						instance.heapMemoryUsed1(_, instance.getHeapMemoryUsed(dump));
						instance.writeStream(_, "memoryDump1", dump, "x-trace", "memory.dmp");
					}
				...
				try {
					if (context.httpSession.x3SessionConfig && context.httpSession.x3SessionConfig.runtime) {
						...
						if (tra && tra.length) {
							...
							var dump = instance.memoryDump(_, endpoint);
							if (dump) {
								instance.heapMemoryUsed2(_, instance.getHeapMemoryUsed(dump));

								instance.heapMemoryGrowth(_, instance.heapMemoryUsed2(_) - instance.heapMemoryUsed1(_));
								instance.writeStream(_, "memoryDump2", dump, "x-trace", "memory.dmp");
							}
			...
			$execute: function(_, context, instance) {
				try {
					if (context.httpSession.x3SessionConfig && context.httpSession.x3SessionConfig.runtime) {
						...
						var dump = memoryDump(_, endpoint);
						if (dump) instance.writeStream(_, "memoryDump2", dump, "x-trace", "memory.dmp");
