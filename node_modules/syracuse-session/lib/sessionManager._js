...
var url = require('url');
var config = require("syracuse-main/lib/nodeconfig");
...
function() {
	...
	this.setup = function(settings, authModule) {
	...
	function _internalCreateSession(_, request, response) {
		...
		var session = new Session(request, _allocSID());
		session.ignoreStoreSession = _settings.ignoreStoreSession;
		session.host = ((request.socket && request.socket.secure) ? "https" : "http") + "://" + request.headers.host;
...
//		session.updateSessionInfo(_);
	...
	function _setSessionCookie(request, response, _, options) {
		...
		if (options.authRequired && session && session.authModule) {
			if (!session.isAuthenticated()) {
				if (options.forceAuth) {
					return session.authModule.authenticate(request, response, _);
				} else if (options.forceAuth2 && session.authModule.authenticate2) {
                    return session.authModule.authenticate2(request, response, _);
				} else return session.authModule.forbidden(request, response);
			}
		}
		return true;
	}
	...
	this.sessionManagement = function(_, request, response, options) {
		...
		request.session && request.session.setLocales(_, (request.headers["accept-language"] || "").split(",")[0]);
		//
		return cookieResult;
	}
	//
	...
	this.cleanupSessionInfos = function(_) {
		var serverName = os.hostname()+":"+(config.port || "default");
		...
		tracer && tracer("session.close server name: "+serverName);
