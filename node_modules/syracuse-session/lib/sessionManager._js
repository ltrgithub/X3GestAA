...
exports.sessionManager = new function() {
	...
	function _getSession(_, request, response) {
		...
		if (request.headers['user-agent'].indexOf('MSIE') > -1)
	...
	function _setSessionCookie(_, request, response) {
		...
		function _secureCookie(value) {
			...
			// #SAM103877 : HttpOnly cookie parameter impact every hot changes about languagues, endpoints, roles...
			// Temporary disable it waiting for stable solution consisting to split the cookie to secure the sid but not other information.
			//value += '; HttpOnly';
			return value;
		}
