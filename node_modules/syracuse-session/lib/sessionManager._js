...
exports.sessionManager = new function() {
	...
	function _checkExpired() {
		...
		for (var key in _sessions) {
			...
			if (session && (session.lastAccess < expired)) {
				...
				_deleteSession(!_, key, false);
			} else {
				if (_mustTerminate) {
					if (!session.hasCvgSessions()) _deleteSession(!_, key, false);
	...
	function _deleteSession(_, key, forceDbDelete) {
		var session = _sessions[key];
		if (session) {
	...
	this.deleteSession = function(_, sid) {
		_deleteSession(_, sid, true);
	};
	this.destroy = function(_) {
		for (var key in _sessions) {
			_deleteSession(_, key, true);
		}
	};
	...
	function _getSession(_, request, response) {
		...
		if (createSesion) {
			...
			if (cookie) {
				...
				syra = syra || require('syracuse-main/lib/syracuse');
				syra.clearRedir(request);
	...
	function _setSessionCookie(request, response, _, options) {
		...
		response.writeHead = function(statusCode, headers) {
			if (request.session) {
				...
				var syraCookie = _settings.key + "." + port + "=" + request.session.id + '; path=/;';
	...
	this.sessionManagement = function(_, request, response, options) {
		...
		//
		// replace $selectedEpBaseUrl
		if (request.url.indexOf('$selectedEpBaseUrl') > 0) {
			try {
				var selEpUrl = request.session.getUserProfile(_).selectedEndpoint(_).getBaseUrl(_);
				request.url = request.url.replace(/(?:\{|\%7B)\$selectedEpBaseUrl(?:\}|\%7D)/g, selEpUrl);
			} catch (e) {
				console.error("No session - no selected endpoint: " + e.message);
			}
		}
	...
	this.logout = function(_, request, response) {
		...
		tracer && tracer((new Date()).toISOString() + "; sessionManager.Logout enter");
