...
//		session.updateSessionInfo(_);
	...
	this.cvgAdmSessInfos=function(_,sid){
		...
		keys.forEach_(_, function(_, sessId) {
			...
			if (syraSess){
				var cvgSessions=syraSess.getFusionSessions(_);
	...
	//Append syraSessioninfo to fusionSession instance
	this._getSessionInfo=function(_,syraSess, cvgSess){
		...
		i.userhost=syraSess.host;
		if (i.userhost){
			var p=i.userhost.indexOf('//');
			if (p>0) i.userhost=i.userhost.substr(p+2);
		}
		return i;
	},
	...
	this.cvgAdmSessInfo=function(_,cvgid){
		...
		for (var id in _sessions){
			...
			if (sess){
				var cvgSess=sess.getFusionSession(cvgid);
				if (cvgSess) {
