...
exports.sessionManager = new function() {
	...
	function _checkDropSession(key, session) { // return true if the session have to be killed
		...
		if (session.lastAccess < timeoutSendCheck) {
			...
			if (_listIo[key].nbAcks > 2) { // if more that 2 socket io message never receive a response, and another session exists with the same IP we considere the session is failed
