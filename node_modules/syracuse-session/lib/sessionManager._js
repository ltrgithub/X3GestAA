...
exports.sessionManager = new function() {
	...
	socketIoHandler.register("session", {
		path: "/session",
		onConnection: function(io, handshake) {
			// send a push to the client to check if he's alive or not
			var cookie = helpers.http.parseCookie(handshake.headers.cookie);
			var sessionId = cookie[_getSessionCookieName(handshake.headers.host.split(':')[1])];
			console.log("client address " + handshake.address);
			console.log("client address " + JSON.stringify(io.adapter.sids));

			_listIo[sessionId] = {
				address: handshake.address, // save address to detect several session on the same client
				lastCheck: (new Date()).getTime(),
				socketIo: io,
				nbCheck: 0
			};
			_listsessionbyIp[handshake.address] = _listsessionbyIp[handshake.address] ||  [];
			_listsessionbyIp[handshake.address].push(sessionId);
		},
		message: {
			"check": function(io, handshake, data) {
				// notif check
				console.log("check received");
				var cookie = helpers.http.parseCookie(handshake.headers.cookie);
				var sessionId = cookie[_getSessionCookieName(handshake.headers.host.split(':')[1])];
				if (_listIo[sessionId]) {
					_listIo[sessionId].lastCheck = (new Date()).getTime();
					_listIo[sessionId].nbCheck--;
				} //TODO else
			},
			"ipAddress": function(io, handshake, data) {

			},
			"disconnection": function(io, handshake, data) {
				//nothin
	...
	// check if a session is inactive we send a check message to the client
	// kill unused session where a client (ip addres is considere as active)
	function _checkDropSession(key, session) { // return true if the session have to be killed
		var timeoutSendCheck = +new Date() - (_settings.checkInterval) * 3 * 1000; //  after 3 checkInterval, if no action done we send a check request to check if client is alive

		// check if we have to send a a check  isAlive message
		if (session.lastAccess < timeoutSendCheck) {
			console.log("check send");
			_listIo[key].socketIo.emit("check", {
				action: "isAlive"
			}); // ask for isAlive
			if (_listIo[key].nbCheck > 2) { // if more that 2 socket io message never receive a response, and another session exists with the same IP we considere the session is failed
				console.log("3 check without response");
				return true;
			}
			_listIo[key].nbCheck++;
	...
	function _checkExpired() {
		...
		for (var tenantId in _sessions) {
			...
			for (var key in sessions) {
				...
				if (session && (_checkDropSession(key, session) || (session.lastAccess < expired))) {
	...
	function _deleteSocketIoSession(key) {
		if (_listIo[key]) {
			var ipClient = _listIo[key].address;
			var idx = _listsessionbyIp[ipClient] && _listsessionbyIp[ipClient].indexOf(key);
			if (idx !== -1) {
				_listsessionbyIp[ipClient].splice(idx, 1);
			}
			delete _listIo[key];
		}
	}
	...
	this.sessionByCookie = function(cookie) {
		...
		var key = cookies[_getSessionCookieName()];
	...
	function _getSession(_, request, response) {
		...
		var cookie = cookies[_getSessionCookieName(port)] || "";
		...
		if (cookie) {
			...
			if (session) {
				...
				tracer && tracer((new Date()).toISOString() + ": Session (" + cookie + ") not found.");
	...
	function _internalCreateSession(_, request, response, reuseSid) {
		...
		clientCookie = (clientCookie ? (clientCookie + "; ") : "") + _getSessionCookieName(port) + "=" + session.id;
