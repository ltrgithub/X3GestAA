...
exports.sessionManager = new function() {
	...
	var _settings;
	var self = this;
	var socketIoHandler = SocketHandler.getSocketHandler();
	socketIoHandler && socketIoHandler.register("session", "/session", function(io, handshake) {
	...
	function _checkDropSession(key, session) { // return true if the session have to be killed
		if (_listIo[key]) {
			if (!_listIo[key].isOpened()) { // if more that 2 socket io message never receive a response, and another session exists with the same IP we considere the session is failed
				return true;
			}
		}
		return false;
