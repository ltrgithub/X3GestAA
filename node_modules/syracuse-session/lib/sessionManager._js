...
exports.sessionManager = new function() {
		...
		socketIoHandler && socketIoHandler.register("/session", function(io, handshake) {
			...
			function dropSession(_) {
				...
				Object.keys(_sessions).forEach(function(tenantId) {
					if (key in _sessions[tenantId]) {
						session = _sessions[tenantId][key];
			...
			if (_listSocketIoBySession[sessionId].number === 0) { // check in 10 seconds if the number of socket io client is still 0 or to delete session
				...
				setTimeout(function() {
					if (_listSocketIoBySession[sessionId].number === 0) {
						dropSession(_ >> function(err) {
							if (err) {
								console.log("Drop session error " + err);
								throw err;
							}
						});
					}
				}, 10000); // check for 10 seconds TODO maybe make this value configurable
			}
	...
	function _checkExpired() {
		...
		for (var tenantId in _sessions) {
			...
			for (var key in sessions) {
				...
				if (session && (session.lastAccess < expired || (session.expirationStamp && session.expirationStamp <= Date.now()))) {
	...
	this.createBatchSession = function(_, user, role, locale, diagnoses) {
		...
		tracer && tracer("Create batch session for: " + login);
	...
	function _getSession(_, request, response, useSessionPool) {
	...
	this.ensureSession = function(_, request, response, useSessionPool) {
		if (useSessionPool) return sessionPool.allocSession(_, request, response, this);
		...
		_getSession(_, request, response, useSessionPool);
		//
