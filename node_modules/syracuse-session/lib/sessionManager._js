...
function getAuthModule(request, redirect, _) {
	...
	if (config.session && config.session.auth === "sage-id") {
		...
		if (path === "sage-id" && authMethods.indexOf(path) > -1) {
			authmethod = "sage-id";
			standardSetting = {};
	...
	if (authmethod === "basic") {
		...
		return require("streamline-auth/lib/sageerpx3").create(_userCheck, standardSetting);
		...
		return require("sage-id").create({
			...
			StartSignOnAttemptRequest: {
				SuccessUri: "http://" + request.headers.host + "/sage-id/startSignOnAttemptSuccess/{0}",
				FailureUri: "http://" + request.headers.host + "/sage-id/startSignOnAttemptFailure",
				CancelAllowed: "true",
				State: "test",
				SessionLengthMinutes: 10
			},
			StartNewUserRegistrationAttemptRequest: {
				SuccessUri: "http://" + request.headers.host + "/sage-id/startNewUserRegistrationAttemptSuccess/{0}",
				FailureUri: "http://" + request.headers.host + "/sage-id/startNewUserRegistrationAttemptFailure",
				CancelAllowed: "true",
				State: "test",
				SessionLengthMinutes: 10,
...
exports.sessionManager = new function() {
	...
	//
	this.logout = function(_, request, response, options) {
		if (!options)
			options = {
				sageIdBase: "https://services.sso.staging.services.sage.com/SSO",
				pfx_object: pfx_object,
		...
		if (cookie) {
			...
			var session = _sessions[cookie];
			if (session && session.EndSignOnAttemptResponse && !options.sessionEnd) {
				require('sage-id').create(options).logout(request, response, session, _);
			}
			// delete session
			if (options.sessionId) {
				_deleteSession(_, options.sessionId);
			} else
				_deleteSession(_, cookie);
		}
		var authModule = getAuthModule(request, false, _);
		var h = {
			location: (authModule && authModule.loginBaseUrl) || "/",
