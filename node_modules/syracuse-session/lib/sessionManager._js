...
//				request.headers.authorization = "";
		...
		response.writeHead = function(statusCode, headers) {
			if (request.session) {
				...
				if (!_hasCookie(cookie, "client.id=")) {
					...
					cookie.push("cclient.id=" + request.session.clientId + "; path=/; expires=" + clientExpire.toGMTString());
				}
				// user profile
				if (!_hasCookie(cookie, "user.profile") && request.session.userProfileCookie) {
					var profileExpire = new Date(+new Date + 200 * 86400 * 1000); // 200 days
					cookie.push("user.profile=" + request.session.userProfileCookie + "; path=/; expires=" + profileExpire.toGMTString());
				}
	...
	this.logout = function(_, request, response) {
		...
		authModule && authModule.setAuthRequestHeader && authModule.setAuthRequestHeader(h);
		response.writeHead(401, h);
