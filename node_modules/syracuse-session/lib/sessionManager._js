...
exports.sessionManager = new function() {
		...
		for (var tenantId in _sessions) {
			...
			for (var key in sessions) {
				...
				if (session && (session.lastAccess < expired)) {
					...
					_deleteSession(!_, key, session);
				} else {
					if (_mustTerminate) {
						if (!session.hasCvgSessions()) _deleteSession(!_, key);
					}
				}
			}
		}
	...
	function _deleteSession(_, key, session) {
		if (session) {
	...
	this.deleteSession = function(_, sid) {
		_deleteSession(_, sid, tenantSession(sid));
	};
	...
	this.destroy = function(_) {
		for (var tenantId in _sessions) {
			var sessions = _sessions[tenantId];
			for (var key in sessions) {
				_deleteSession(_, key, sessions[key]);
			}
		}
	};
		...
		response.writeHead = function(statusCode, headers) {
			if (request.session) {
				...
				if (!_hasCookie(cookie, _settings.key + "." + port)) {
					var syraCookie = self.secureCookie(secure, self.getSyracuseCookie(request.session, port) + '; path=/; expires='); //self.secureCookie(_settings.key + "." + port + "=" + request.session.id + '; path=/; expires=');//self.secureCookie(_settings.key + "." + port + "=" + request.session.id + '; path=/; expires=');//s
					cookie.push(syraCookie);
				}
				// client ID
				if (!_hasCookie(cookie, "client.id")) {
					var clientId = self.secureCookie(secure, "client.id=" + request.session.clientId + "; path=/; expires=" + expireGMTString);
					if (checkIfIE(request.headers['user-agent']))
						clientId = self.secureCookie(secure, "client.id=" + request.session.id + "; path=/; expires=" + expireGMTString);
	...
	this.logout = function(_, request, response, session) {
		...
		if (cookie) {
			...
			_deleteSession(_, cookie, session);
		}
