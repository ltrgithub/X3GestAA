...
exports.sessionManager = new function() {
	...
	function _getSessionCookieName() {
		return _settings.key + "." + (config.port || "8124");
	}

	function _getLoginCookieName() {
		return _settings.key + ".login." + (config.port || "8124");
	}
	...
	this.formatSessionCookie = function(sid, port) {
		return _getSessionCookieName() + "=" + sid + '; path=/';
	};
	...
	function _getSession(_, request, response) {
		...
		var cookie = cookies[_getSessionCookieName()] || "";
	...
	function _internalCreateSession(_, request, response, reuseSid) {
		...
		session.loginCookieName = _getLoginCookieName();
		if (checkIfIE(request.headers['user-agent']))
		...
		var cookie = _getSessionCookieName() + "=" + session.id;
		if (checkIfIE(request.headers['user-agent']))
		...
		clientCookie = (clientCookie ? (clientCookie + "; ") : "") + _getSessionCookieName() + "=" + session.id;
	...
	function _setSessionCookie(_, request, response) {
		...
		response.writeHead = function(statusCode, headers) {
			if (request.session) {
				...
				var expireGMTString = (new Date(+new Date + 200 * 86400 * 1000)).toGMTString(); // 200 days
				var port = request.connection.localPort;

				if (!_hasCookie(cookie, _getSessionCookieName())) {
					var syraCookie = _secureCookie(_getSessionCookieName() + "=" + request.session.id + '; path=/');
					cookie.push(syraCookie);
				}
				if (!_hasCookie(cookie, _getLoginCookieName()) && request.session.loginCookie) {
					var ck = _secureCookie(_getLoginCookieName() + "=" + request.session.loginCookie + '; path=/; expires=' + (new Date(+new Date + 4 * 7 * 86400 * 1000)).toGMTString()); // 4 weeks
					cookie.push(ck);
				}
				// client ID
	...
	this.logout = function(_, request, response, session) {
		...
		var cookie = pck[_getSessionCookieName()];
		...
		if (!errorExists) {
			...
			var h = {
				location: session && session.loginCookie ? "/auth/forgetMe/page" : "/auth/login/page",
				"content-type": "application/json"
			};
...
function checkIfIE(userAgent) {
	if (!userAgent) return false;
	var reIEOld = new RegExp("MSIE ([0-9]{1,}[\.0-9]{0,})");
	var reIENew = new RegExp("Trident/.*rv:([0-9]{1,}[\.0-9]{0,})");
	return reIEOld.test(userAgent) || reIENew.test(userAgent);
}