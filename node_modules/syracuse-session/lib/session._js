...
exports.Session = helpers.defineClass(Session, events.EventEmitter, {
	...
	setUserProfile: function(_, userProfile) {
		...
		// get right access
		userProfile.getAccessRightAuthorizations(_); // get right on initialisation of session
	...
	updateSessionInfo: function(_, timeCheck) {
		...
		return session.updateFunnel(_, function(_) {
			...
			// console.log("UPD SI6 "+require('util').format(session))
			//			console.log("ZZZx "+require('util').inspect(require('syracuse-x3/lib/pool').getClients(), { depth: 10 }));
			//			console.log("ZZZ1 "+require('util').inspect(session.getData("cvgSessions"), { depth: 4 }));
