...
exports.Session = helpers.defineClass(Session, events.EventEmitter, {
	...
	checkPersistentCookie: function(_, request, withTouch) {
		...
		if (loginCookie) {
			...
			if (!security.signIn.rememberMe || (loginToken.login(_) !== login) || (loginToken.token(_) !== token) || unauth) {
				if (!security.signIn.rememberMe || withTouch) {
	...
	setupPersistentCookie: function(_, request) {
		var self = this;
		self.loginCookie = "";
		if (!security.signIn.rememberMe || !self.authData || !self.authData.user) return;
		var db = adminHelper.getCollaborationOrm(_);
