	...
	destroy: function(_){
		...
		if(cvgSessions) 
			Object.keys(cvgSessions).forEach_(_, function(_, ssid) {
				...
				self.closeCvgSession(ssid, false, _);
			});
		//
...
/*		var up = this.getUserProfile(_);
	...
	updateCvgSession: function(cvgSession, _) {
		var cvgSessions = this.getData("cvgSessions");
	...
	closeCvgSession: function(fsid, reuse, _) {
		tracer && tracer("closeCvgSession - id=" + fsid +  "- reuse=" + reuse);
		var cvgSessions = this.getData("cvgSessions");
		if (cvgSessions && cvgSessions[fsid]){
			cvgSessions[fsid].close(_, reuse);
