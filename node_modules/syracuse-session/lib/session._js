...
exports.Session = helpers.defineClass(Session, events.EventEmitter, {
	...
	destroy: function(_) {
		...
		//
		// it's purge by time to live now
	...
	updateSessionInfo: function(_, timeCheck) {
		...
		try {
			...
			if (!sessionInfo) {
				...
				// 
				sessionInfo = db.model.getEntity(_, "sessionInfo").factory.createInstance(_, null, db);
			...
			sessionInfo.peerAddress(_, req.connection ? (req.connection.remoteAddress || "") : "-");
