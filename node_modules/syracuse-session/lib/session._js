...
exports.Session = helpers.defineClass(Session, events.EventEmitter, {
	...
	setUserProfile: function(_, userProfile) {
		...
		if(userProfile && userProfile.selectedRole(_) && userProfile.selectedRole(_).securityProfile(_))
			this.setData("securityProfile", userProfile.selectedRole(_).securityProfile(_));
		// update session info is asynchronous but we don't wait the result here
...
/*		var up = this.getUserProfile(_);
	...
	getFusionSessions: function(_) {
		var res=[];
		var fusionSessions = this.getData("fusionSessions");
		if(fusionSessions) {
			Object.keys(fusionSessions).forEach_(_, function(_, ssid) {
				res.push(fusionSessions[ssid])
			});
		}
		return res;
	},
	...
	updateFusionSession: function(fusionSession, _) {
		var fusionSessions = this.getData("fusionSessions");
		if(!fusionSessions) this.setData("fusionSessions", fusionSessions = {});
		fusionSessions[fusionSession.sid] = fusionSession;
		fusionSession.lastAccess=datetime.now();
	},
	closeFusionSession: function(fsid, reuse, _) {
		tracer && tracer("closeFusionSession - id=" + fsid +  "- reuse=" + reuse);
		var fusionSessions = this.getData("fusionSessions");
		if (fusionSessions && fusionSessions[fsid]){
			fusionSessions[fsid].close(_, reuse);
			delete fusionSessions[fsid];	
			tracer && tracer("\tSession deleted");		
		}else{
			tracer && tracer("\tcloseFusionSession - Session not found");
		}
	}
});
