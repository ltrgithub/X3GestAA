...
var sessionTracer = require('syracuse-core/lib/trace').getTracer("sessionManager.session");
var flows = require('streamline-runtime').flows;
var profileFunnel = flows.funnel(1);
...
exports.Session = helpers.defineClass(Session, events.EventEmitter, {
	...
	checkPersistentCookie: function(_, request, withTouch) {
		...
		if (loginCookie) {
			...
			var error;
			try {
				checkUser.checkUserLogin(_, self, login);
			} catch (e) {
				unauth = true;
				error = e;
			}
	...
	destroy: function(_, forceDelete) {
		...
		cvgListReuse.closeReuseSession(_, this.id);
	 ...
	 * To be reviewed by Boost team (same code as before but wrapped in a funnel)
	 */
	getUserProfile: function(_, failIfMissing) {
		var self = this;
		var up = self.getData("userProfile");
		if (up) return up;
		up = profileFunnel(_, function(_) {
	...
	updateSessionInfo: function(_, timeCheck, withCvgSessions) {
		...
		try {
			...
			if (!sessionInfo) {
				...
				tracer && tracer((new Date()).toISOString() + "; session.updateSessionInfo " + session.id + "; no need to update");
				return null;
			}
			var up = session.getData("userProfile");
			if (up) {
				sessionInfo.userName(_, up.user(_).login(_));
				sessionInfo.dataset(_, up.selectedEndpoint(_).dataset(_));
			} else {
			...
			if (withCvgSessions) {
				session.getCvgSessions(_).forEach_(_, function(_, cvgs) {
					...
					if (res === null) tracer && tracer((new Date()).toISOString() + "; session.updateCvgSessionInfo " + cvgsi.sid(_) + "; no need to update");
					else {
						if (res && res.$actions && res.$actions.$save && res.$actions.$save.$diagnoses) {
							tracer && tracer((new Date()).toISOString() + "; session.updateCvgSessionInfo " + cvgsi.sid(_) + "; result: " + sys.inspect(res && res.$actions && res.$actions.$save && res.$actions.$save.$diagnoses));
							_errorDiagnose(cvgsi.sid(_), res && res.$actions && res.$actions.$save && res.$actions.$save.$diagnoses);
						} else {
							tracer && tracer((new Date()).toISOString() + "; session.updateCvgSessionInfo(2) " + cvgsi.sid(_) + "; result: " + sys.inspect(res));
							tracer && tracer((new Date()).toISOString() + "; session.updateCvgSessionInfo(3) " + cvgsi.sid(_) + "; result: " + sys.inspect(cvgsi.getAllDiagnoses(_)));
	...
	closeCvgSession: function(fsid, params, reason, _) {
		...
		tracer && tracer("closeCvgSession - id=" + fsid + "- reuse=" + params.reuse + " - reason=" + reason);
		var cvgSessions = this.getData("cvgSessions");
	...
	statelessExecute: function(_, fn) {
		...
		try {
			...
			this.expirationStamp = Date.now() + Math.round(config.statelessTimeout * 60 * 1000);
		}
	},
});
