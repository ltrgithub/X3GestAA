...
function normalizedCookie(ckHeader) {
...
asyncTest("sessionManager - Authenticate with form", 21, function(_) {
	var response = new ez.devices.http.client({
	...
	resp = response.readAll(_);
	var respCookie = normalizedCookie(response.headers["set-cookie"]);
	var loginCk = extractCookie(respCookie, loginCookieKey);
	...
	resp = response.readAll(_);
	var respCookie2 = normalizedCookie(response.headers["set-cookie"]);
	strictEqual(response.statusCode, 200, "user authenticated login cookie");
	var loginCk = extractCookie(respCookie2, loginCookieKey);
