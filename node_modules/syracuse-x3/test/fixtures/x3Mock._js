...
function X3Mock() {
	...
	function checkBody(request, item) {
		if (item.data) {
			var check = true;
			Object.keys(item.data).forEach(function(propertieName) {
				if (check) {
					if (request.body && request.body[propertieName]) {
						var sameObj = JSON.stringify(item.data[propertieName]) === JSON.stringify(request.body[propertieName]);
						check = check && sameObj;
					} else {
						check = false;
					}
