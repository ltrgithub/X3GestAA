...
function X3Mock(_) {
	...
	function loadResponses(_) {
		...
		files.forEach_(_, function(_, file) {
			...
			var data = fs.readFile(pathFile, "utf8", _);
			try {
	...
	self.onCloseRequest = function(request) {
		return {
			head: {
				status: 204
			},
	...
	function checkBody(request, item) {
		if (item.data) {
			return Object.keys(item.data).every(function(propertyName) {
				if (request.body && request.body[propertyName]) {
					return JSON.stringify(item.data[propertyName]) === JSON.stringify(request.body[propertyName]);
				} else {
					return false;
				}
			});
	...
	function findItem(originalValue, item, request) {

		var urlOk = request.head.url.indexOf(item.url) === 0;
		var methodOk = request.head.method === item.method;
		var bodyOk = checkBody(request, item);

		if (urlOk && methodOk && bodyOk) {
			...
			if (!result) {
				if (item.$items) {
					item.$items.forEach(function(itemChild) {
						if (!result) {
							result = findItem(result, itemChild, request);
						}
					});
				} else {
	...
	function dispatchRequest(request) {
		...
		if (request.head.url) {
			...
			if ((url.indexOf("/$sessions") === 0 || url.indexOf("/$connections") === 0) && request.head.method === "DELETE") {
				return self.onCloseRequest(request);
			}

			var response = findRequest(request);
			if (response) {
				return response;
			}
	...
	self.close = function(_) {
		if (self.crtSocket) self.crtSocket.destroy();
		self.server.close(_);
	};
