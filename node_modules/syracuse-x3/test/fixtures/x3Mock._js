...
function X3Mock() {
	...
	function readRequest(_, c, data) {
		...
		var chunks = [];
		do {
			buf = new Buffer(c.read(_, 5));
			var bodyLen = buf.readUInt32BE(0) - 1;
			if(bodyLen)
				chunks.push(c.read(_, bodyLen, "utf8"));
		} while (buf[4] == 2);
//		_checkCode(frame.code, 3);
	...
	self.server = streams.createNetServer(function(c, _) {
		try {
			self.crtSocket = c;
			var data = c.read(_, 4);
			if ((new Buffer(data)).readUInt32BE(0) !== 0x00070300) throw new Error("connection refused: bad magic");
