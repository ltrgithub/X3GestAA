...
module(config.QLF.folder + ".x3Search", {
	setup: function() {},
	teardown: function() {
		if (doStop) {
			setTimeout(function() {
...
asyncTest("ABANK", 5, function(_) {
	_checkSearch(_, "ABANK");
	start();
});
asyncTest("ACLAIDX", 4, function(_) {
	_checkSearch(_, "ACLAIDX");
	start();
});
...
asyncTest("AFCTFCT", 4, function(_) {
	_checkSearch(_, "AFCTFCT");
	start();
});
