...
asyncTest("ZQAB", function(_) {
  ...
  putDelta(_,prototype,workingCopy,{"SHO":"A"});
  //20 characters string (for 10 max):
  putDelta(_,prototype,workingCopy,{"SHO":"ABCDEFGHIJ"});
  //21 characters string (for 10 max):
  putDelta(_,prototype,workingCopy,{"SHO":"ABCDEFGHIJK"});

  //modify a boolean property:
  putDelta(_,prototype,workingCopy,{"FLG":false});

  //modify a choice :
  var delta = {"MNL":1};
  var expected = JSON.parse(JSON.stringify(delta));
  expected.ENDDAT = "2012-11-01"; 
  putDelta(_,prototype,workingCopy,delta,expected);
