...
asyncTest("listenNode -  callilog - test with http request", function(_) {
	...
	// NEW GANTT ok
	stream.setToRead(binaryNode.ND_IHM_gantt_new);
	...
	strictEqual(!stream.writed, true, "not reply sent to runtime - wait notif plugin");
	...
	strictEqual(stream.writed && stream.writed.toString("hex"), "0000000015756e646566696e65643d01756e646566696e65640a0000000000", "write client reply , notif plugin and client ok");
	...
	stream.setToRead(binaryNode.ND_IHM_gantt_update);
	...
	stream.setToRead(binaryNode.NDOBOXI);
	flows.sleep(_, 1000); // wait proces of node
	...
	stream.setToRead(binaryNode.ND_IHM_gantt_delete);
	flows.sleep(_, 1000); // wait proces of node
