...
function getPlugin(_, cookie, suburl, sessionId, statusCode, fullResponse) {
	...
	return adminTestFixtures.get(_, cookie, baseUrl + "/trans/x3/erp/SUPERV/$plugin('" + sessionId + "')/" + suburl, statusCode, fullResponse);
}
...
asyncTest("listenNode -  callilog - test with http request", function(_) {
	...
	strictEqual(res, '{"sap":{"target":{"type":"plugin","plugin":{"class":"ILOG","name":"gantt","type":"asynchronous","id":null,"module":"Scheduling\\\\v6\\\\x3ilog.dll","method":"new","methodId":1}}},"session":{"settings":{"mnu":[],"datePivot":1957,"dateOrder":0,"dateSep":"/","thousandSep":",","decimalSep":".","timeSep":":"}},"srvop":{"request":{"id":0,"v":1025,"rqtdist":null,"connect":1,"server":""}}}', "generate reply to the client ND_IHM new ");
	stream2.writed = null;
...
asyncTest("listenNode -  http - new gantt  launch plugin with download/upload ", function(_) {
	...
	res = JSON.stringify(putPlugin(_, cookie, "requestSvc?act=524&trackngId=1", cvgSession.sid, {
		act: 523,
		param: {
			plugin: {
				return :{
	...
	putPlugin(_, cookie, "blobSvc", cvgSession.sid, 202, false),
	get(_, cookie, "requestSvc?$trackers=2", cvgSession.sid, 202, false);
