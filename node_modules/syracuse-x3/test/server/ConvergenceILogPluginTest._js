...
asyncTest("listenNode -  callilog - test with http request", function(_) {
	...
	res = JSON.stringify(putPlugin(_, cookie, "requestSvc?act=523&trackngId=3", cvgSession.sid, {
		act: 523,
		param: {
			plugin: {
				return :{
	...
	res = JSON.stringify(putPlugin(_, cookie, "requestSvc?act=524&trackngId=4", cvgSession.sid, {
		act: 524,
		param: {
			method: "new",
			plugin: {
	...
	getPlugin(_, cookie, "requestSvc?$trackers=4", cvgSession.sid, 202);

	ok(true, "send srv_next_filed in response of info box");
	put(_, cookie, "requestSvc?act=1025&trackngId=5", cvgSession.sid, {
		act: 1025,
		fld: {
			fmtKind: "EDIT",
			ist: {
	...
	res = JSON.stringify(getPlugin(_, cookie, "requestSvc?$trackers=4", cvgSession.sid, 200));
	...
	res = JSON.stringify(get(_, cookie, "requestSvc?$trackers=5", cvgSession.sid, 200));
