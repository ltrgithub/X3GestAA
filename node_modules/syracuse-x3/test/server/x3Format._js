...
module(config.unit_test.x3endpoint.folder + ".x3Format");

if (!config.unit_test || !config.unit_test.x3endpoint || !config.unit_test.run || !config.unit_test.run.x3Format) {
	//if (!config.unit_test || !config.unit_test.x3endpoint) {
	return test('X3 FORMATS UNIT TESTS DISABLED FOR NOW', 1, function() {
