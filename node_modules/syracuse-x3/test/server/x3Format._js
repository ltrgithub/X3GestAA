...
module(config.QLF.folder+".x3Format", {
	setup: function() {},
	teardown: function() {
		if (doStop) {
			setTimeout(function() {
...
var Representation = function(_repName) {
  ...
  this.update = function(_,_key,_delta) {
    ...
    //Create a new working copy:
    var url;
    /*dbg*///ok("update","typeof(_delta):"+typeof(_delta));
  ...
  this.setProperties = function(_,_delta,_isDisabled,_status) {
    ...
    /*dbg*///ok(true,"DBG setProperties:"+JSON.stringify(response));   
...
function testAQCQRY02(_) {
   asyncTest("AQCQRY02 - CRUD 0K", function(_) {
      ...
      representation.setProperties(_,{"FIELD01": FIELD01}); 
      representation.setProperties(_,{"FIELD02": FIELD02}); 
