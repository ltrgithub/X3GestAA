...
function get(_, cookie, url, statusCode, facet) {
	...
	try {
		...
		console.log("GET JSON error; response was: "+resp);
		throw e;
	}
}
...
asyncTest("init database", 1, function(_) {
	...
	var db = new mongodb.Db(endPoint.datasets["collaboration_test"].database, server, {});
	db = db.open(_);
	db.dropDatabase(_);
	//
...
asyncTest("x3 endpoint route", 4, function(_) {
	requestCount++;
	var body;
	cookie = getCookie(_);
...
/*	else {
	...
	*/
	...
	x3serverId = body.$uuid;
	// create endpoint
	//console.log("x3 endpoint route creating ep");
	body = post(_, cookie, "endPoints", {
		description: "X3 superv",
		applicationRef: {$uuid: appId},
		dataset: "SUPERV",
		enableSearch: false,
		protocol: "x3",
		x3server: {$uuid: x3serverId}
	}, 201);
	//console.log("x3 endpoint route creating ep body(1)"+sys.inspect(body, null, 4));
	epId = body.$uuid;
...
asyncTest("x3 user code", 9, function(_) {
	requestCount++;
	cookie = getCookie(_);
	var body;
	...
	cookie = getCookie(_);
	//
	//console.log("add endpoints test");
