...
var CvgClient = require("syracuse-x3/lib/fusion/client/CvgClient").CvgClient;

function compare(a, b, path) {
	if (a === b) {
		strictEqual(a, b, path + " OK [" + a + "]");
		return;
	}
	...
	Object.keys(b).forEach(function(k) {
		if (a[k] === undefined) {
			strictEqual(a, b, path + "." + k + ': missing ' + JSON.stringify(b[k]));
		}
	});
}
...
function getNonObjCount(obj) {
	var count = 0;
	for (var i in obj) {
		if (typeof obj[i] === 'object') {
			count += getNonObjCount(obj[i]);
...
function connect(_, expected) {
	try {
		var client = new CvgClient(stream, {
...
var expected = require('fs').readFileSync(__dirname + "/data/resp_oamk_connect_210113.json", "utf8");
expected = fix(JSON.parse(expected));
asyncTest("OAMK Connection", getNonObjCount(expected), function(_) {
	stream = new Stream();
	var dump = require(__dirname + "/data/dump_oamk_connect_210113.js").dump;
	dump(READ, WRITE);
