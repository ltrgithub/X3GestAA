...
asyncTest("listenNode -  callui CR Developper - test with http request", function(_) {
	...
	var res = JSON.stringify(put(_, cookie, "requestSvc?act=1025&trackngId=2", cvgSession.sid, {
		act: 1025,
		fld: {
			fmtKind: "EDIT",
			ist: {
	...
	strictEqual(res, JSON.stringify(resp), "generate reply to the client DevReport ");
	res = put(_, cookie, "requestSvc?act=532&trackngId=3", cvgSession.sid, {
		act: 532,
		param: {
			method: "new",
			plugin: {
...
asyncTest("listenNode -  callui CR Developper - test with http request and simulation buffer", function(_) {
	var id = "id";
	var clientId = "clientId";
	var session = new Session(null, id, clientId);
	var cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin").join(" ");
	...
	get(_, cookie, "requestSvc?act=532&$trackers=2", cvgSession.sid, 200);


	stream2.setToRead(binaryNode["callui-checkout"]);

	res = JSON.stringify(put(_, cookie, "requestSvc?act=1025&trackngId=3", cvgSession.sid, {
		act: 1025,
		fld: {
			fmtKind: "EDIT",
			ist: {
	...
	stream2.writed = null;
	res = put(_, cookie, "requestSvc?act=532&trackngId=4", cvgSession.sid, {
		act: 532,
		param: {
			method: "new",
			plugin: {
	...
	stream2.setToRead(binaryNode.ND_GIST);
	flows.sleep(_, 500);
	get(_, cookie, "requestSvc?act=532&$trackers=4", cvgSession.sid, 200);


	stream2.setToRead(binaryNode["callui-commit"]);

	res = JSON.stringify(put(_, cookie, "requestSvc?act=1025&trackngId=5", cvgSession.sid, {
		act: 1025,
		fld: {
			fmtKind: "EDIT",
			ist: {
	...
	res = put(_, cookie, "requestSvc?act=532&trackngId=6", cvgSession.sid, {
		act: 532,
		param: {
			method: "new",
			plugin: {
	...
	stream2.setToRead(binaryNode.ND_GIST);
	flows.sleep(_, 500);
	get(_, cookie, "requestSvc?act=532&$trackers=6", cvgSession.sid, 200);
	strictEqual(cvgClient.cvgPlugin && Object.keys(cvgClient.cvgPlugin).length, 0, " plugin deleted ok ");

	start();
});

asyncTest("error case send cr request without plugin instanciate ", function(_) {
	var id = "id";
	var clientId = "clientId";
	var session = new Session(null, id, clientId);
	var cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin").join(" ");
	...
	strictEqual(!stream2.writed, true, "send 532 request but no plugin exists ");


	start();
});

asyncTest("ILog and Crystal Report plugin test", function(_) {
	var id = "id";
	var clientId = "clientId";
	var session = new Session(null, id, clientId);
	var cookie = adminTestFixtures.getCookie(_, baseUrl, "admin", "admin").join(" ");
	...
	// add CR report during usage of ILog
	stream2.setToRead(binaryNode["callui-checkout"]);
	flows.sleep(_, 500);
	// no reply 200 so get again a polling
	ok(true, "get tracker session 202");
	var res = get(_, cookie, "requestSvc?$trackers=2", cvgSession.sid, 200, false);

	res = put(_, cookie, "requestSvc?act=532&trackngId=Cr3", cvgSession.sid, {
		act: 532,
		param: {
			method: "new",
			plugin: {
	...
	ok(true, "get tracker session 202");
	get(_, cookie, "requestSvc?$trackers=Cr3", cvgSession.sid, 202, false);
	...
	// check existance plugin
	res = JSON.stringify(get(_, cookie, "requestSvc?$trackers=Cr3", cvgSession.sid, 200, false));
	strictEqual(res, '{"sap":{"target":{"type":"plugin","plugin":{"class":"ILOG","name":"gantt","type":"asynchronous","module":"Scheduling\\\\v6\\\\x3ilog.dll","method":"delete","methodId":3,"id":1}}},"session":{"settings":{"mnu":[],"datePivot":1957,"dateOrder":0,"dateSep":"/","thousandSep":",","decimalSep":".","timeSep":":"}},"srvop":{"request":{"id":0,"v":1025,"rqtdist":null,"connect":1,"server":""}}}', "delete sent by server after sending a delete ");
	...
	strictEqual(Object.keys(cvgClient.cvgPlugin).length, 0, "no plugin in client after delete ");
