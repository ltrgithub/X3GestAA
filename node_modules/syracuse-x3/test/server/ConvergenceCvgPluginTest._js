...
asyncTest("initialize data demo", 1, function(_) {
	var opt = {
		$diagnoses: []
	};
	jsonImport.jsonImport(_, db, "syracuse-admin-demo.json", opt);
	//	tracer && tracer("import demo db diags (134): "+sys.inspect(diag));
	ok(adminTestFixtures.onlyInfo(opt.$diagnoses), "Demo database import ok");
	start();
});
...
asyncTest("listenNode -  callilog - test with http request", function(_) {
	...
	cvgClient = new CvgClient("150067").init(_, session, stream2, {}, null, context);
...
asyncTest("listenNode -  callilog - new gantt/scale", function(_) {
	...
	localConfig.plugin = localConfig.plugin || {};
	cvgClient = new CvgClient("150067").init(_, session, stream, {}, null, context);
