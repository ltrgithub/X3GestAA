...
asyncTest("MorePrinter", function(_) {
	...
	for (var i = 0; i < result.paperSizeList.length; i++) {
		strictEqual(result.paperSizeList[i] != null ,true,"paperSize["+i+"] "+JSON.stringify(result.paperSizeList[i]));
	} 
	for (var i = 0; i < result.backList.length; i++) {
		strictEqual(result.backList[i] != null ,true,"back["+i+"] "+JSON.stringify(result.backList[i]));
	} 
...
asyncTest("AllPrinter", function(_) {
	...
	for (var i = 0; i < result.length; i++) {
		strictEqual(result[i] != null ,true,"printer["+i+"] "+JSON.stringify(result[i]));
	} 
	setTimeout(function(_) {
		cli._disconnect(_);
		start();
...
asyncTest("ExtraPrinter", function(_) {
	...
	strictEqual(cli.pid != null && cli.pid >= 0 , true, "pid: "+cli.pid);
	var comm = cli.communicator;
	comm.send_ND_EXTRA_PRINTER(_,"\\\\172.28.8.4\\HP3390-grenoble","172.28.8.151","HP Universal Printing PCL 5",22);
