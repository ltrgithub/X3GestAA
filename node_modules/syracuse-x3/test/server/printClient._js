...
asyncTest("OuvImp; GetState; GetReport",  8, function(_) {
	...
	var cli = new PrintClient(_, hostname, port);
	//var cli = new PrintClient(_, "eccmichel", port);
...
asyncTest("MorePrinter", 1, function(_) {
	var cli = new PrintClient(_, hostname, port);
	strictEqual(cli.pid != null && cli.pid >= 0 , true, "pid: "+cli.pid);
	var comm = cli.communicator;
...
asyncTest("ExtraPrinter", 0, function(_) {
	...
	var comm = cli.communicator;
	console.log("send more");
	comm.send_ND_EXTRA_PRINTER(_,"\\\\172.28.8.4\\HP3390-grenoble","172.28.8.151","HP Universal Printing PCL 5","");
