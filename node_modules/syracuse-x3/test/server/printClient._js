...
asyncTest("MorePrinter", function(_) {
	...
	strictEqual(cli.pid != null && cli.pid >= 0 , true, "pid: "+cli.pid);
	stop();
	var result = cli.communicator.send_ND_MORE_PRINTER(_,"\\\\172.28.8.4\\HP3390-grenoble","172.28.8.151","HP Universal Printing PCL 5");
...
asyncTest("ExtraPrinter", function(_) {
	...
	strictEqual(cli.pid != null && cli.pid >= 0 , true, "pid: "+cli.pid);
	stop();
	var result = cli.communicator.send_ND_EXTRA_PRINTER(_,"\\\\172.28.8.4\\HP3390-grenoble", "172.28.8.151", "HP Universal Printing PCL 5", 9);
	strictEqual(result.$width, 2100, "A4 Paper size width: "+result.$width);
	strictEqual(result.$length, 2970, "A4 Paper size length: "+result.$width);
	cli._disconnect(_);
	start();
});
