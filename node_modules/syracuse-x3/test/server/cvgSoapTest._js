...
/*asyncTest("Ws Classic Soap Client -  connect / disconnect ", function(_) {
	...
	context.session.genericWsCtx = {
		endpoint: endpoints.endpoint(_),
		user: users.user(_),
		localePref: localePrefs["fr-FR"]
	};
	var cli = new CvgClientWS(_, context);
	...
	context.session.genericWsCtx.localePref = localePrefs["en-US"];
	cli = new CvgClientWS(_, context);
	strictEqual(cli.stream !== null, true, "client connected ok");
	...
	strictEqual(cli.stream !== null, false, "client disconnected ok");
	 ...
	 failed bad endpoint
	try {
		context.session.genericWsCtx.endpoint = endpoints.endpointBidon(_);
		cli = new CvgClientWS(_, context);
	 ...
	 failed bad user
	context.session.genericWsCtx.endpoint = endpoints.endpoint(_);
	context.session.genericWsCtx.user = users.userBidon(_);
	cli = new CvgClientWS(_, context);
...
asyncTest("Ws Classic Soap Client -  call sub program", function(_) {
	context.session.genericWsCtx.endpoint = endpoints.endpointSUPERV(_);
	context.session.genericWsCtx.user = users.user(_);
	var cli = new CvgClientWS(_, context);
...
asyncTest("Ws Classic Soap Client -  call technical sub program", function(_) {
	...
	var param = [{
		...
		"size": 8,
		"typ": "CLOB",
		"resu": "ADAPI:PUB:adonix.trace.level=3;adonix.trace.on=on;adonix.trace.size=8;adxwss.requestid=TSVC.3.20150210142403.3;"
	}, {
		"descr": "A",
		"dim": 1,
		"nb": 1,
	...
	var reply = cli.execAckCall(_, "WJQLFWSADD:ADDDECIMAL", 0, param);
