...
asyncTest("Ws Classic Soap Client -  connect / disconnect ", function(_) {
	...
	var cli = new CvgClientWS(_, context, recOptions);
	...
	cli = new CvgClientWS(_, context, recOptions);
	strictEqual(cli.stream !== null, true, "client connected ok");
...
asyncTest("Ws Classic Soap Client -  technical Subprogram", function(_) {
	...
	cli = new CvgClientWS(_, context, recOptions);
	try {
		var res = cli._changeLogin(_, "AP", "ENG");
	...
	cli = new CvgClientWS(_, context, recOptions);
	try {
		var res = cli._changeLogin(_, "TCA", "ENG");
	...
	cli = new CvgClientWS(_, context, recOptions);
	try {
		var res = cli._changeLogin(_, "AP", "FRI");
	...
	cli = new CvgClientWS(_, context, recOptions);
	try {
		var res = cli._changeLogin(_, "BIDON", "FRA");
	...
	try {
		var res = cli._recupHDAT(_, "QLFWSADD", stamp);
...
asyncTest("Ws Classic Soap Client -  execWs", function(_) {
	...
	strictEqual(JSON.stringify(jsxml.parse(reply)), '{"RESULT":{"GRP":{"$":{"DIM":"1","SIZE":"1","ID":"G1"},"FLD":[{"$":{"NAME":"A","TYPE":"Decimal"},"$value":"2.112"},{"$":{"NAME":"B","TYPE":"Decimal"},"$value":"97.887"},{"$":{"NAME":"R","TYPE":"Decimal"},"$value":"99.999"}]}}}', "result ok");
	...
	strictEqual(jsxml.parse(reply) && Object.keys(jsxml.parse(reply).RESULT).length > 1, true, "reply contain group ok");
	strictEqual(techInfo.changeLanguage, false, " no change language  ok ");
	strictEqual(techInfo.changeUserId, false, " no change  user ok ");
