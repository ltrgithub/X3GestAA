...
var longtext = "1234567890123456789";
for (var i = 0; i < 9; i++) longtext += longtext + longtext + longtext + "a";

// truncate output to 100 characters
function _output(res) {
	return (res.length > 100 ? res.substr(0, 100) + "..." : res);
}
...
var tests = [{
	...
	fileOptions: {
		folder: "SUPERV",
		path: "tmp",
		name: "test_5", // test_4 is already used for renaming test
		extension: "tst"
	}
}];
...
var connCtx = {
	server: sadUnitConfig.server !== undefined ? sadUnitConfig.server : "aws-x3-devsup.sagefr.adinternal.com",
...
function completeTest(param, i) {
	...
	asyncTest("readFile", function(_) {
		...
		strictEqual(res, param.str, "content ok: " + _output(res));
		start();
	});
	...
	asyncTest("read", function(_) {
		...
		try {
			len = sadfs.read(_, fd, buffer, 16, buffer.length - 15);
			strictEqual(true, false, "No error occurred when reading to much");
		} catch (e) {
			ok("Error when trying to read too much");
		}
		len = sadfs.read(_, fd, buffer, 16, buffer.length - 16);

		// len = sadfs.read(_, fd, buffer, 16, 64);

		strictEqual(len, Math.min(bufStr.length, buffer.length) - 16, "Size read 4 ok: " + len);
		strictEqual(buffer.slice(16, Math.min(bufStr.length, buffer.length)).toString('hex'), bufStr.slice(16, Math.min(bufStr.length, buffer.length)).toString('hex'), "Buffer content 4 ok");

		position = sadfs.getPosition(_, fd);
		strictEqual(position, Math.min(bufStr.length, buffer.length), "Position ok");
		...
		sadfs.close(_, fd);
		strictEqual(param.str.slice(0, Math.min(bufStr.length, buffer.length)), buffer.slice(0, Math.min(bufStr.length, buffer.length)).toString('utf-8'), "Content ok: " + _output(param.str));
		start();
	});
	...
	asyncTest("client disconnect", 4, function(_) {
		sadfs.disconnect(_);
		sadfs2.disconnect(_);
		sadfs3.disconnect(_);
		sadfs4.disconnect(_);
		...
		strictEqual(sadfs4.stream == null, true, "Stream is closed");
		start();
	});
