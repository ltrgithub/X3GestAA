...
function completeTest(param, i, _) {
	...
	asyncTest("readFile Not Exists", function(_) {
		try {
			var res = sadfs.readFile(_, fileOptionsNotExists, {
				flag: "r",
				encoding: "utf-8"
			});
			ok(false, "no error raise ");
		} catch (e) {
			ok(true, "error raise ok " + e.stack);
		}
		start();
	});
