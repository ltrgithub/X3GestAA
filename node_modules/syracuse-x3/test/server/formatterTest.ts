...
/*global QUnit, start, ok, test, equal, strictEqual */

var locale = require('streamline-locale');
var CvgFormatter = require('../../lib/convergence/types/CvgFormatter').CvgFormatter;
...
describe(module.id, () => {
	...
	function newFormatter(opts, lang) {
		...
		if (typeDate === "MDY") {
			options.dateOrder = FTM_X3DATE_MDY;
		} else if (typeDate === "DMY") {
			options.dateOrder = FTM_X3DATE_DMY;
		} else if (typeDate === "YMD") {
			options.dateOrder = FTM_X3DATE_YMD;
		} else {
	...
	function testOptions(fmt, raw, value, expected, opts) {
		...
		// Add actual format
		if (isCountryDate) {
			fmt += "{" + formatter.getRealFormat(format).format + "}";
		}
		var removeTime = null;
	...
	function testEditToDisplay(lang, fmt, value, expected, opts, srvVal) {
		...
		try {
			var got = formatter.formatEditToDisplay(fmt, value, srvVal);
	...
	function testFormatErr(lang, fmt, x3Val, message, strict, opts) {
		try {
			var got = newFormatter(opts, lang).formatField(fmt, x3Val, true, strict);
	...
	function testUnformatErr(lang, fmt, formatVal, message, strict, opts) {
		try {
			var got = newFormatter(opts, lang).unformatField(fmt, formatVal, true);
	...
	function testUnformat(lang, fmt, value, expected, strict, opts) {
		try {
			var got = newFormatter(opts, lang).unformatField(fmt, value, strict);
	...
	function testFormat(lang, fmt, value, expected, raw, strict, opts) {
		try {
			var got = newFormatter(opts, lang).formatField(fmt, value, raw, strict);
			strictEqual(got, expected, fmt + " : '" + value + "' -> '" + got + "'");
		} catch (ex) {
	...
	it('init', function(_) {
		locale.setCurrent(_, "fr");
		equal(locale.current, "fr");
	});

	it('basic formatting', function() {
		...
		testUnformat('fr', "K:[+(33)]#[.]##[.]##[.]##[.]##", "+(33) .  .  .  .", "");
		testBasic('fr', "K:[(+34)]#[.]##[.]##[.]##[.]##", true, "", "(+34) .  .  .  .");
		testBasic('fr', "K:[(+34)]9#", true, "123456789", "(+34)123456789");
		...
		testBasic('fr', "DZ:DD[/]MM[/]YY", false, "21/02", "21/02/" + year.substring(2));
		testUnformat('fr', "DZ:DD[/]MM[/]YY", "21/02/08", "20080221");
		testUnformat('fr', "DZ:DD[/]MM[/]YY", "21/02", year + "0221");
	...
	it('date basic', function() {
		...
		testOptions('DZ:DD[/]MM[/]YY', false, '10/12/05ddd', '10/12/05');
		// #1208 - strict format
		testFormat('fr', 'DZ:DD[/]MM[/]YY', '10/12/05ddd', '10/12/05', false, false, null);
		testFormatErr('fr', 'DZ:DD[/]MM[/]YY', '10/12/05ddd', 'Number expected', true, null);
		testFormatErr('fr', 'DZ:DD[/]MM[/]YY', '10//05', 'Number expected', true, null);
	});

	it('date with country format', function() {
		// DD1=MM/DD
	...
	it('date with missing components', function() {
		...
		testOptions('DZ:DD[_]MM[_]YY', false, '', '', opt);
		testOptions('DZ:DD[_]MM[_]YY', true, '', '', opt);
		testOptions('DZ:DD[_]MM', false, '', '', opt);
		testOptions('DZ:DD[_]MM', true, '', '', opt);
		testOptions('DZ:MM[_]YY', false, '', '', opt);
		testOptions('DZ:MM[_]YY', true, '', '', opt);
		testOptions('DZ:MM[_]DD[_]YY', true, '15991231', '', opt);
		testOptions('DZ:MM[-]DD[-]YYYY', false, '05/17/2006', '05-17-2006', opt);
		testOptions('DZ:MMM', false, '05/17/2006', 'MAY', opt);
		testOptions('DZ:MMM[-]YYYY', false, '0506', 'MAY-2006', opt);
	});

	it('number basic', function() {
	...
	it('number z3 (thousands sep)', function() {
	...
	it('number - (leading sign)', function() {
	...
	it('number z and v with . input', function() {
	...
	it('number z and v with 0 inputs', function() {
	...
	it('number z and v with spaces input', function() {
	...
	it('number z and v with empty input', function() {
		testOptions('Nv2z:7.6#', false, '', '');
		testOptions('Nv2z:7.6#', true, '', '');
		testOptions('Nv2:7.6#', false, '', '0,000000');
		testOptions('Nv2:7.6#', true, '', '0,000000');
	...
	it('number z and v with lots of zeros in input', function() {
	...
	it('string padding tests', function() {
	...
	it('string case conversions', function() {
	...
	it('string c directive', function() {
		var opt = {
			dateSep: '_',
			datePivot: 1950,
			dynFormats: {
		...
		testEditToDisplay('fr', 'K:3c', "111", "111", opt);
		testEditToDisplay('fr', 'K:3c', "---", "---", opt);
		testOptions('K:3c', false, 'ddd', 'DDD', opt);
		testOptions('K:3c', false, 'd-d', 'D-D', opt);
		...
		// #553 - skipBadFormat set to true by default - We added skipBadDynFmt (true by default) to skip or not skip bad dynamic format errors
		testFormatErr('fr', 'K:3c', '222', 'Test Unexpected format char \'c\'', true, {
			skipBadDynFmt: false
		});
		// skipBadDynFmt = true - returns parameter's value
		testOptions('K:3c', false, '222', '222', {
			skipBadDynFmt: true
		});
	});

	it('dynamic formats', function() {
		var opt = {
			dynFormats: {
				c: {
					subType: "A#",
	...
	it('date series 2', function() {
		...
		testOptions('D:[debut]DD[.]MM[.]YY[fin]', false, '16/03', "debut16.03." + year.substring(2) + "fin");
		testOptions('D:[debut]DD[.]MM[.]YY[fin]', false, '16/03/63', "debut16.03.63fin");
		testOptions('D:[debut]DD[.]MM[.]YYYY[fin]', false, '16/03/63', "debut16.03.1963fin");
	...
	it('number series 2', function() {
		testOptions('N:9.4#', true, '53,1336', '53,1336');
		testOptions('NPzc:9.2#', true, '5,4', '5,40');
		testOptions('NPzc:9.2#', true, '5,456789', '5,45');
		testOptions('NPzc:9.2#', true, '4349,012945123455656560000000000000000000000000000000', '4349,01');
		testOptions('NPzc:9.2#', true, '4349,250000000000000000000000000000000', '4349,25');
		testOptions('NPzc:9.2#', true, '4,053293300806905610808782135485', '4,05');
		testOptions('N3*:7.3#', false, '5,4', '* *** **5,400');
		testOptions('N3*:7.3#', true, '5,4', '* *** **5,400');
		testOptions('N+z:11F', false, '00012345678,005', '00012345678');
		testOptions('N+z:11F', true, '00012345678,005', '00012345678');
		testOptions('N+z:12.5', true, '123456789016,005', '123456789016,00500');
		testOptions('N+z:12.5', true, '123456789016,00565056', '123456789016,00565');
		...
		testOptions('N+z3:11F', true, '0,256', '0,256');
		testOptions('Nv+z3:11F', true, '0,256', '0,256');
		testOptions('Nv+z3:5.2#', true, '13,00', '13,00');
		testOptions('N3z:14#', false, '14 322', '14 322');
		testOptions('N3z:14#', true, '14 322', '14 322');
		...
		testOptions('N3zv1:14.0#', true, '14322', '14 322');
		testOptions('N+z3:11.2#', true, '345678911,88', '345 678 911,88');
		testOptions('N-D0:[debut]####8.[Fin]##5#[Fin]', false, '+21150,213', 'debut000000021150,Fin2130000Fin+');
		testOptions('N-D0:[debut]####8.[Fin]##5#[Fin]', true, '+21150,213', 'debut000000021150,Fin2130000Fin+');
		testOptions('N-D0:3.0#', false, '+150', '150,+');
		testOptions('N-D0:3.0#', true, '+150', '150,+');
		testOptions('N-D0:0.6#', false, '+,123', ',123000+');
		testOptions('N-D0:0.6#', true, '+,123', ',123000+');
		testOptions('N-0:0.6#', false, '+,123', '+,123000');
		testOptions('N-0:0.6#', true, '+,123', '+,123000');
		testOptions('N-:4.1#', true, '+150,3', '+150,3');
		testOptions('N-:4.0#', false, '+150', '+150');
		testOptions('N-:4.0#', true, '+150', '+150');
		testOptions('N-D0:[debut]####2.[Fin]##3#[Fin]', false, '+150,23', 'debut000150,Fin23000Fin+');
		testOptions('N-D0:[debut]####2.[Fin]##3#[Fin]', true, '+150,23', 'debut000150,Fin23000Fin+');
		//testOptions('N-:[debut]####2.[Fin]##3#[Fin]', false, '+150,23', '+debut150,Fin23000Fin');
		//testOptions('N-:[debut]####2.[Fin]##3#[Fin]', true, '+150,23', '+debut150,Fin23000Fin');
		testOptions('N3:9.2#', true, '+1234567,30', '1 234 567,30');
		testOptions('N:6.2#', true, '+23345,3', '23345,30');
		//testOptions('N+-AD0z53:5.2#', false, '-23345.3', null);
		//testOptions('N+-AD0z53:5.2#', true, '-23345.3', null);
		testOptions('N+z3:11.2#', true, '1234567911,88', '1 234 567 911,88');
		testOptions('N+z3*:11.2#', false, '150,88', '** *** *** 150,88');
		testOptions('N+z3*:11.2#', true, '150,88', '** *** *** 150,88');
		testOptions('N+z3*:4.2#', false, '150,88', '* 150,88');
		testOptions('N+z3*:4.2#', true, '150,88', '* 150,88');
		testOptions('N+z30:11.2#', false, '150,88', '00 000 000 150,88');
		testOptions('N+z30:11.2#', true, '150,88', '00 000 000 150,88');
		testOptions('N+z30:4.2#', false, '150,88', '0 150,88');
		testOptions('N+z30:4.2#', true, '150,88', '0 150,88');
		testOptions('Nz:7.6#', false, '  - 3,2', '-3,200000');
		testOptions('Nz:7.6#', true, '  - 3,2', '-3,200000');
		testOptions('N+z3:11.2#', true, '1234567911,88', '1 234 567 911,88');
		testOptions('N+z3*:11.2#', false, '150,88', '** *** *** 150,88');
		testOptions('N+z3*:11.2#', true, '150,88', '** *** *** 150,88');
		testOptions('N+z3*:4.2#', false, '150,88', '* 150,88');
	...
	it('X3 -> Edit -> Display Format', function() {
		var year = (new Date()).getFullYear().toString();
		...
		testX3ToEditToDisp("fr", "D:DD[-]MM", year + "0301", "01-03", "01/03");
		testX3ToEditToDisp("en", "D:DD[-]MM", year + "0301", "01-03", "03/01");
	});

	it('Parse user input value ', function() {
		...
		testEditToDisplay('en', "D:MM[/]DD[/]YY", "11282012", "11/28/12");
		// Special cases
		testEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "111103", "11/11/2003", {
			dateOrder: 'DMY'
		});
		testEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "111103", "11/11/2003", {
			dateOrder: 'MDY'
		...
		//testEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "111103", "03/11/2011", {
		//	dateOrder: 'YMD'
		//});
		// Test 4 digit date input with 2 digits in format
		testEditToDisplay('fr', "D:DD[/]MM[/]YY", "2011113", "03/11/11", {
			dateOrder: 'YMD'
		});
		testBadEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "02213", "Bad month 21", {
			dateOrder: 'DMY'
		});
		testEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "02213", "21/02/2003", {
			dateOrder: 'MDY'
		});
		testBadEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "02213", "Bad month 21", {
			dateOrder: 'YMD'
		});
		testBadEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "20213", "Bad month 21", {
			dateOrder: 'DMY'
		});
		testBadEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "20213", "Bad month 20", {
			dateOrder: 'MDY'
		});
		testBadEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "20213", "Bad month 21", {
			dateOrder: 'YMD'
		...
		//# 1218
		// YY acepts 2005
		testEditToDisplay('fr', "D:DD[/]MM[/]YY", "1/2/2005", "01/02/05");
		// YYYY acepts 5
		testEditToDisplay('fr', "D:DD[/]MM[/]YYYY", "1/2/5", "01/02/2005");
	...
	it('Strict mode', function() {
		...
		testOptions("X+-03z:8.2#", true, '12', '12', true);
		testOptions("X+-03z:8.2#", false, "+12", "+12");
		// #553 for dates
		...
		testOptions("D:JJ[/]MM[/]BBBB", true, '20121110', '20121110', true);
		testUnformat('fr', "D:JJ[/]MM[/]CCCCC", '10/11/12', "10/11/12");
		testFormat('fr', "D:JJ[/]MM[/]CCCCC", '10/11/12', "10/11/12");
	});
});