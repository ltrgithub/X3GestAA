...
var Batch = function(_code) {
	...
	this.post = function(_) {
		...
		var response = x3Helper.post(_, _getCookie(_), this.batch.$url, this.batch);
		/*dbg*/
...
var Representation = function(_repName) {
	...
	this.get_facet = function(_, _key, _facet) {
		...
		this.proDetails = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('" + this.repName + "." + _facet + "')"), 200);
		/*dbg*/ //ok(true,this.repName+".$details prototype");

		var data = x3Helper.get(_, _getCookie(_), x3Helper.x3Url(this.repName + "(%27" + _key + "%27)?representation=" + this.repName + "." + _facet + ""), 200);
		/*dbg*/
...
function testIssue1475(_) {
	...
	asyncTest("AQCQRY02 CRUD OK" + x3Helper.gitHubIssue(1475), function(_) {
		...
		strictEqual(400, response.$resources[3].$httpStatus, "$httpStatus:" + response.$resources[3].$httpStatus);
...
function testIssue2362(_) {
	asyncTest("AQMCRUDM Delete" + x3Helper.gitHubIssue(2362), function(_) {
		var batch = new Batch("AQMCRUDM");
		var data = {
			"CODE": 740
		};
...
//!testIssue2362(_); Test was changed ??
//!testIssue2306(_); representation removed ??
