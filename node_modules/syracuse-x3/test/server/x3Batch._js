...
var Batch = function(_class, _repName) {
	...
	this.create = function(_, _resource) {
		...
		_resource.$url = x3Helper.x3Url(className + "?representation=" + repName + ".$edit");
		batch.$resources.push(_resource);
	};
	...
	asyncTest("Issue 3207" + x3Helper.gitHubIssue(3207), function(_) {
		...
		var data = {
			...
			"ADEVADEVICEL": [{
				...
				"$uuid": "5a1fca00-7dcc-11e3-baa7-0800200c9a66"
			}]
		};
		...
		var response = representation.read(_, data.CODE);
		strictEqual(2, response["ADEVADEVICEL"].length, "ADEVADEVICEL size");
		for (var i = 0; i < response["ADEVADEVICEL"].length; i++) {
			checkProperty(response["ADEVADEVICEL"][i], "SUBCODE", 101 + i);
			ok("$properties" in response["ADEVADEVICEL"][i], "$properties");
			["ADEV_UNIT"].forEach(function(property) {
