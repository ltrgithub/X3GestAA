...
module(config.unit_test.x3endpoint.folder + ".x3Requester", {
	setup: function() {},
	teardown: function() {
		if (doStop) {
			setTimeout(function() {
...
if (!config.unit_test || !config.unit_test.x3endpoint) {
	return test('X3 PROTOTYPES and PAYLOAD UNIT TESTS DISABLED FOR NOW', 1, function() {
...
function _checkRequesterPayload3(_, query, facet) {
	var payload = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("QUERY('" + query + "')?representation=QUERY~" + query + "~3." + facet), 200);
	console.log(payload);
	ok("$totalResult" + "=" + payload.$totalResults, "$totalResult" + "=" + 13);
	ok("$recordsPerPage" + "=" + payload.$recordsPerPage, "$recordsPerPage" + "=" + 20);
	var i;
	for (i = 0; i < payload.$totalResults; i++) {
		ok("line_no(" + payload.$resources[i].line_no + ")", "line_no(" + i + 1 + ")");
	}
	ok("CRY(0)=" + payload.$resources[0].CRY, "CRY(0)=AN");
	ok("CRY(1)=" + payload.$resources[1].CRY, "CRY(1)=AR");
	ok("CRY(6)=" + payload.$resources[6].CRY, "CRY(6)=FR");
	ok("CRY(12)=" + payload.$resources[12].CRY, "CRY(12)=US");
}

function _checkRequesterPayload2(_, query, facet) {
	var payload = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("QUERY('" + query + "')?representation=QUERY~" + query + "~2." + facet), 200);
	console.log(payload);
	ok("$totalResult" + "=" + payload.$totalResults, "$totalResult" + "=" + 13);
	ok("$recordsPerPage" + "=" + payload.$recordsPerPage, "$recordsPerPage" + "=" + 20);
	var i;
...
function _checkRequesterPayload1(_, query, facet) {
...
function testPrototypeAQCREQ3() {
	asyncTest("Requester-Prototype" + x3Helper.gitHubIssue(5554), function(_) {
		//console.log("Test");
...
function testPayloadAQCREQ3() {
	asyncTest("Requester-Payload" + x3Helper.gitHubIssue(5554), function(_) {
		//console.log("Test");
		_checkRequesterPayload3(_, "AQCREQ3");
		_checkRequesterPayload2(_, "AQCREQ3");
		_checkRequesterPayload1(_, "AQCREQ3");
		start();
	});
}


testPrototypeAQCREQ3();
testPayloadAQCREQ3();
