...
function testAQCLNK01(_) {
	...
	asyncTest("AQCLNK01 - OPERATIONS Test", function(_) {
		...
		query.$resources.forEach_(_, function(_, resource) {
			wcount = wcount + 1;
			var numField01 = resource.REP_FIELD01.substr(3);
			var noField01 = parseInt(numField01);
			/*
			...
			/*
			strictEqual(response.$actions.OPERAT1_LIG44444.$return.$type, "application/x-date", "OPERAT1_LIG44444.$return.$type");
			/*
			var today = new Date();
			var date = new Date(Date.parse(response.$actions.OPERAT1_LIG44444.$return.$value));
			var expectedDate = new Date();
			expectedDate.setDate(today.getDate());
			strictEqual(date.toDateString(), expectedDate.toDateString(), "OPERAT1_LIG44444.$return.$value");
			*/
		});
		strictEqual(wcount, count, "number of line=" + count);
		start();
	});
...
testAQCQRY11(_);
testMISC(_);
//TODO implement SETUP testAQCPROC01(_);
testTABCOUNTRY(_);
testTABCOUNTRYUS(_);
//TODO use a dedicated table for this:testPOSCOD(_);
testAQCLNK01(_);
//TODO implement SETUP testAQCPROC06(_);
testAQCPROC07(_);
testAQCPROT1(_);
testAQCBPR(_);
