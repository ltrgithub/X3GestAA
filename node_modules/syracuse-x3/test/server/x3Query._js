...
function testTABCOUNTRY(_) {
	...
	asyncTest("TABCOUNTRY Second page", function(_) {
		...
		strictEqual(query.$resources[1].CRYDES, 'Biélorussie', "Country description"); //French // returning Biélorussie. FF payload and dispaly are both ' '. // GH 5661 
		...
		strictEqual(query.$resources[1].CUR_REF.$title, "Rouble biélo", "Currency title"); //French // Rouble biélo returned in payload though not displayed.  // GH 5661 
		strictEqual(query.$resources[1].CUR_REF.$description, "Rouble Biélorusse", "Currency description"); //French // Rouble Biélorusse returned in payload though not displayed.  // GH 5661 
...
function testTABCOUNTRYUS(_) {
	...
	asyncTest("TABCOUNTRYUS Second page", function(_) {
		...
		strictEqual(query.$itemsPerPage, 20, "$itemsPerPage"); // truly 50
		strictEqual(query.$resources[1].CRY, 'BY', "Country"); //DZ
		strictEqual(query.$resources[1].CRYDES, '', "Country description"); //English  // returning Biélorussie. FF payload and dispaly are both ' '. // GH 5661 
		ok(query.$resources[1].EECFLG === false, true, "EECFLAG = true");
		strictEqual(query.$resources[1].EECDAT, '0000-00-00', "EEU Date"); // D data type should resolve 1599-12-30 // GH 5661 
		strictEqual(query.$resources[1].CUR, 'BYB', "Currency");
		strictEqual(query.$resources[1].CUR_REF.$title, '', "Currency title"); //English  // Rouble biélo returned in payload though not displayed.  // GH 5661 
		...
		strictEqual(query.$resources[0].CUR_REF.$scale, 2, "Currency scale");
		strictEqual(query.$resources[1].LAN, 'BEL', "Language"); // FRA
		strictEqual(query.$resources[1].LAN_REF.$title, 'Biélorusse', "Language title"); //English 
		strictEqual(query.$resources[1].LAN_REF.$description, 'Biélorusse', "Languge description"); //English  
		start();
	});
}
