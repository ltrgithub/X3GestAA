...
function testMISC(_) {
	asyncTest("AQCPROC01", function(_) {
		...
		var query = representation.query(_);
		var count = 20;
		ok(query.$resources.length != 0, "resources ength");
		start();
	});
	...
	asyncTest("AQCPROC01", function(_) {
		...
		query.$resources.forEach(function(resource) {
			if (resource.C01P01 === "QUNIT_001") {
				strictEqual(resource.C01P02, "R01P01_02", "resource.C01P02");
				ok(resource.C01P02_REF !== undefined, true, "resource.C01P02_REF");
				strictEqual(resource.C01P02_REF.$title, "INTSHO 2", "resource.C01P02 $title");
				strictEqual(resource.C01P02_REF.$description, "INTDES 2", "resource.C01P02 $description");
			}
		});
		start();
	});
...
function testAQCLNK01(_) {
	...
	asyncTest("AQCLNK01 = F1_079", function(_) {
		...
		//ok(query.$resources[0].CRY_REF !== undefined, true, "resource.CRY_REF");  // available through CRY data-type from REP CRY Key & Lookup check mark.          		 		                                                                                               		 
		start();
	});
	...
	//        ok(query.$resources[11].REP_FIELD02 !== undefined, false, "resource.REP_FIELD02 unavailable");
	//        start();
	// });
...
function testTABCOUNTRY(_) {
	...
	asyncTest("TABCOUNTRY First page", function(_) {
		...
		//strictEqual(query.$resources[0].CRYDES, 'ZZZZZZddd', "Country description");   // GH 5661        
		...
		//strictEqual(query.$resources[0].CUR_REF.$description, 'EURO', "Currency description");  // EURO eur   works, though FF Payload says just EURO // GH 5661 
	...
	asyncTest("TABCOUNTRY Second page", function(_) {
		...
		strictEqual(query.$resources[1].CRY, 'BY', "Country"); //DZ
		//strictEqual(query.$resources[1].CRYDES, '', "Country description");  // returning Biélorussie. FF payload and dispaly are both ' '. // GH 5661 
		ok(query.$resources[1].EECFLG === false, true, "EECFLAG = true");
		//strictEqual(query.$resources[1].EECDAT, '0000-00-00', "EEU Date");   // D data type should resolve 1599-12-30 // GH 5661 
		strictEqual(query.$resources[1].CUR, 'BYB', "Currency");
		//strictEqual(query.$resources[1].CUR_REF.$title, '', "Currency title");  // Rouble biélo returned in payload though not displayed.  // GH 5661 
		//strictEqual(query.$resources[1].CUR_REF.$description, '', "Currency description");  // Rouble Biélorusse returned in payload though not displayed.  // GH 5661 
		strictEqual(query.$resources[0].CUR_REF.$symbol, null, "Currency symbol");
		strictEqual(query.$resources[0].CUR_REF.$precision, 11, "Currency precision");
		strictEqual(query.$resources[0].CUR_REF.$scale, 2, "Currency scale");
		strictEqual(query.$resources[1].LAN, 'BEL', "Language"); // FRA
		strictEqual(query.$resources[1].LAN_REF.$title, 'Biélorusse', "Language title"); // French
		strictEqual(query.$resources[1].LAN_REF.$description, 'Biélorusse', "Languge description"); // French  
		start();
	});
}
