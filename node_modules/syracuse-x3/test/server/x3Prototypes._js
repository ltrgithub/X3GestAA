...
function _checkPrototype(_, _representation, _facet, _statusCode) {
	...
	function _strictEqual(actual, expected, message) {
		...
		try {
			...
			test.nok++;
			test.messages.push("NEQ: " + message + " (" + actual + "!=" + expected + ")");
...
*/
	...
	function cutBaseUrl(s) {
		return s.substring(s.indexOf('/sdata'));
	}
	_strictEqual(cutBaseUrl(prototype.$baseUrl), "/sdata/x3/erp/" + config.QLF.dataset, "$baseUrl");
	...
	strictEqual(test.nok, 0, test.nok + " assertions failed:\n" + test.messages.join('\n'));
	return test;
}
