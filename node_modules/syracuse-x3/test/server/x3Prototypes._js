...
*/
	...
	if (rewriteFixturesProto === true && config.unit_test.x3endpoint.dataset === "SUPERV") {
		console.error("rewriteFixturesProto " + __dirname + "/../fixtures/" + _representation.toLowerCase() + "_" + _facet + ".json");
		try {
		...
		var test = _compareObjects(JSON.stringify(p2, null, 2), p2);
	} else {
		var p1 = {};
		try {
...
function _compareJSONs(_, _representation, Detail, Edit, Query, Lookup, Summary, Bulk) {
	var facets = [];
...
function testCompare() {
	...
	asyncTest("ACLAIDX    compare", 6, function(_) {
		_compareJSONs(_, "ACLAIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("AFCTIDX    compare", 6, function(_) {
		_compareJSONs(_, "AFCTIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});

	asyncTest("APSADX     compare", 6, function(_) {
		_compareJSONs(_, "APSADX", Detail, false, Query, false, false, Bulk);
		start();
	});
	asyncTest("AREPIDX    compare", 6, function(_) {
		_compareJSONs(_, "AREPIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("ATABLE     compare", 12, function(_) {
		_compareJSONs(_, "ATABLE", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("ATABDIV    compare", 10, function(_) {
		_compareJSONs(_, "ATABDIV", Detail, Edit, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("ATRACE     compare", 6, function(_) {
		_compareJSONs(_, "ATRACE", Detail, false, Query, false, false, Bulk);
		start();
	});
	asyncTest("AVOLUME    compare", 10, function(_) {
		_compareJSONs(_, "AVOLUME", Detail, Edit, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("COMPANY    compare", 12, function(_) {
		_compareJSONs(_, "COMPANY", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("POSCOD     compare", 12, function(_) {
		_compareJSONs(_, "POSCOD", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("TABLAN     compare", 12, function(_) {
		_compareJSONs(_, "TABLAN", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("TABUNIT    compare", 12, function(_) {
		_compareJSONs(_, "TABUNIT", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});

	//	asyncTest("ZQREQH     compare", 8, function(_) {
	//		_compareJSONs(_, "ZQREQH",Detail,false,Query,Lookup,false,Bulk);
	//		start();
	//	});
	//	asyncTest("ZZSERVICE  compare", 8, function(_) {
	//		_compareJSONs(_, "ZZSERVICE",Detail,Edit,Query,false,false,Bulk);
	//		start();
	//	});
	...
	asyncTest("AQCPROT1  compare", 12, function(_) {		
		_compareJSONs(_, "AQCPROT1", Detail, Edit, Query, Lookup, Summary,Bulk);				  
		start();
	});  ////// Failing at 402 EDIT, so add all EDITS to Detail properties.
*/

	asyncTest("ATABCUR    compare", 10, function(_) {
		_compareJSONs(_, "ATABCUR", Detail, Edit, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("TABCOUNTRY compare", 12, function(_) {
		_compareJSONs(_, "TABCOUNTRY", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});

	asyncTest("AQCLNK01  compare", 12, function(_) {
		_compareJSONs(_, "AQCLNK01", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
}
