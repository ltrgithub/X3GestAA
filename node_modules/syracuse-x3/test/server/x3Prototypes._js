...
function _compareJSON(_, _representation, _facet) {
	var name = _representation + ".$" + _facet;
	var exceptions = ["AQCLNK01/query/Type/$properties.$resources.$item.$properties/REP_CREUSR_REF",
	...
	function _compareObjects(o1, o2) {
		...
		function _compare(o1, o2, path, message) {
			...
			for (var p in o1) {
				...
				if (["$x3Format", "$baseUrl", "$collaborationUrl", "$localization", "$title", "$shortTitle", "$actxLanDef", "$actxUser", "$actxLogin", "$actxLegCur", "$actxNbLeg", "$actxFolder", "$actxSolution"].indexOf(p) < 0) {
...
function testCompare() {
	...
	*/
	...
	asyncTest("AQCLNK01  compare", 10, function(_) {
...
function testIssue5554() {
	asyncTest("QUERIES" + x3Helper.gitHubIssue(5554), function(_) {
		var prototype = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('QUERY~TESTREQ1~1.$query')"), 200);
		prototype = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('QUERY~DOC~1.$query')"), 200);
		prototype = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('QUERY~AIDES~1.$query')"), 200);
		start();
	});
}
