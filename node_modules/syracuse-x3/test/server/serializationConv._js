...
module('x3 convergence serialisation test');

var done;
module("X3 - Convergence Serialization", {
	setup: function(_) {},
	teardown: function(_) {
		if (done) {
			setTimeout(function() {
...
asyncTest("CvgDecimal",  function(_) {
	...
	var buf = x3decimal.toBytes("101010.10");
	strictEqual('c30a0a0a0a000000000000000000000000' === buf.toString('hex'),true,"101010.10 ["+buf.toString('hex')+"");
	var buf = x3decimal.toBytes("-101010.10");
	strictEqual('3c5959595a000000000000000000000000' === buf.toString('hex'),true,"-101010.10 ["+buf.toString('hex')+"");
	var buf = x3decimal.toBytes("3.1415926536");
	strictEqual('c1030e0f5b412400000000000000000000' === buf.toString('hex'),true,"-3.1415926536 ["+buf.toString('hex')+"");
});
