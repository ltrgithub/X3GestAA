		...
		try {
			...
			try {
				...
				try {
					...
					ok( false,"Ko : formatField Error : " + ex.message);
				}
			} catch (ex) {
...
function testEditFormatField(fmt, value ,srvVal, expected, opts) {
	var formatter =newFormatter(opts);
	var format = new CvgFormat(fmt)
	try{
		var got=formatter.formatEditToDisplay(format, value, srvVal, false);
...
function testEditFormat(fmt, expected, opts) {
	var formatter =newFormatter(opts);
	var editFmt=formatter.getFormatEdit(fmt);
	var format = new CvgFormat(fmt)
	strictEqual(editFmt, expected, format + " : Edit: " + editFmt);
}
...
function testEditToDisp (lang, fmt, x3Val, dispVal, editVal, opts) {
	...
	var got2;
	try {
		got2=formatter.formatEditToDisplay(format, got1, x3Val, false);
	} catch (ex) {
		console.error(ex);
		got2 = ex.message;
		error=true;
...
function testFormatErr(lang, fmt, x3Val, message, strict, opts){
	...
	var got;
	try {
		got =formatter.formatField(format, x3Val, true, strict);
...
function testUnformatErr(lang, fmt, formatVal, message, strict, opts){
	...
	var got;
	try {
		got =formatter.unformatField(format, formatVal);
...
function testParse(fmt, value, expected, opts) {
	var formatter = newFormatter(opts);
	var format = new CvgFormat(fmt)
	var got;
	try {
		got = formatter.unformatField(format, value);
	} catch (ex) {
		console.error(ex);
		got = ex.message;
	}		
...
test("basic formatting", 64 , function() {
	...
	testBasic('fr', "K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", false, "2500613564564", "2 50 06 13 564 564");
	testBasic('fr', "K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", true, "2500613564564", "2 50 06 13 564 564");
	testParse("K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", "2 50 06 13 564 564", "2500613564564");
	testParse("Kz:##[:]##[:]##","15:12:28","151228");
	...
	testParse("DZ:DD[/]MM[/]YY", "21/02/08", "20080221");
	testParse("DZ:DD[/]MM[/]YY", "21/02", "20130221");
...
test("date basic", 38 , function() {
	testOptions('D:DD[/]MM[/]YYYY', false, '01102', '01/10/2002');
	testOptions('D:DD[/]MM[/]YYYY', true, '01102', '02/10/2001');
	testOptions('D:DD[/]MM[/]YY', false, '01/02/08', '01/02/08');
	testOptions('D:DD[/]MM[/]YY', true, '01/02/08', '08/02/01');
	testOptions('D:DD[/]MM[/]YY', true, '31/04/04', '04/04/31');
	testOptions('D:DD[/]MM[/]YY', true, '42/04/04', '04/04/42');
	testOptions('D:DD[/]MM[/]YYYY', false, '01011601', '01/01/1601');
	testOptions('D:DD[/]MM[/]YYYY', true, '631001', '01/10/1963');
	...
	testOptions('D:DD[/]MM[/]YY', true, '02/2/06', '06/02/02');
	...
	testOptions('D:DD[/]MM[/]YY', true, '20/02', '20/02/13');
});
...
test("date Z", 12, function() {
	...
	testOptions('DZ:DD[/]MM[/]YY', true, '15991231', '  /  /  ');
	testParse('DZ:DD[/]MM[/]YY', '', '15991231');
	testParse('DZ:DD[/]MM[/]YYYY', '', '15991231');
	testParse('DZ:DD[/]MM[/]YY', '   ', '15991231');
	testParse('DZ:DD[/]MM[/]YYYY', '  ', '15991231');
});
...
test("date with missing components", 70, function() {
	testOptions('DZ:YYYY', true, '2017', '2017');
...
test("number * (filler)", 13, function() {
	testParse("N-3zv*:14.2", '+*** *14 322,3', '14322.3', null);
...
test("date series 2", 41 , function() {
	...
	testOptions('D:DD[/]MM[/]YY', true, '2/3/6', '06/03/02');
	testOptions('DZ:DD[/]MM[/]YY', true, '15991231', '  /  /  ');
...
test("X3 -> Edit -> Display Format", 22, function() {
	...
	testEditFormatField("DZ:DD[/]MM[/]YY", "21/02/08", "20090102", "21/02/08"); 
