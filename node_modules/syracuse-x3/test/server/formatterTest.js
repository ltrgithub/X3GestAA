		...
		try {
			...
			try {
				...
				try {
					...
					ok( false,"Ko : formatField Error : " + ex.message +  " : expected:'" + expected + "'");
				}
			} catch (ex) {
				ok( false,"Ko : unformatField Error : " + ex.message);
			}
		} catch (ex) {
			ok( false,"Ko : formatField Error : " + ex.message + " : expected:'" + expected + "'");
		}		
...
function testEditFormatField(lang, fmt, value, expected, opts, srvVal) {
	try{
		var formatter =newFormatter(opts,lang);
		var got=formatter.formatEditToDisplay(new CvgFormat(fmt), value, srvVal);
		strictEqual(got, expected, fmt + " : Edit: " + formatter.getFormatEdit(fmt) +": '" + value + "' -> '" + got + "'");
	} catch (ex) {
		ok( false,"Ko : formatEditToDisplay Error : " + ex.message);
	}	
}
...
function testEditToDisp (lang, fmt, x3Val, dispVal, editVal, opts) {
	var formatter =newFormatter(opts,lang);
	var format = new CvgFormat(fmt)
	...
	strictEqual(got2, dispVal, formatter.getFormatEdit(fmt) + " formatEditToDisplay - '" + got1 + "' -> '" + got2 + "'");
	if (error) return;
	var got3;
	try {
...
function testUnformatErr(lang, fmt, formatVal, message, strict, opts){
	try {
		...
		ok( true, fmt + " : " + formatVal + " : unformatField Ok : Error expected : "  + message + " - errMsg: " + e.message);
	}
}
function testSimple(fmt, raw, value, expected) {
	testOptions(fmt, raw, value, expected);
}

function testParse(lang,fmt, value, expected, opts) {
	var got;
	try {
		var got = newFormatter(opts,lang).unformatField(new CvgFormat(fmt), value);
...
test("basic formatting", 64 , function() {	
	...
	testBasic('fr', "K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", false, "2500613564564", "2 50 06 13 564 564");
	testBasic('fr', "K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", true, "2500613564564", "2 50 06 13 564 564");
	testParse('fr', "K:#[ ]##[ ]##[ ]#B[ ]###[ ]###[ ]##", "2 50 06 13 564 564", "2500613564564");
	testParse('fr', "Kz:##[:]##[:]##","15:12:28","151228");
	...
	testParse('fr',"DZ:DD[/]MM[/]YY", "21/02/08", "20080221");
	testParse('fr',"DZ:DD[/]MM[/]YY", "21/02", "20130221");
...
test("date Z", 12, function() {
	...
	testOptions('DZ:DD[/]MM[/]YY', true, '15991231', '  /  /  ');
	testParse('fr','DZ:DD[/]MM[/]YY', '', '15991231');
	testParse('fr','DZ:DD[/]MM[/]YYYY', '', '15991231');
	testParse('fr','DZ:DD[/]MM[/]YY', '   ', '15991231');
	testParse('fr','DZ:DD[/]MM[/]YYYY', '  ', '15991231');
});
...
test("number * (filler)", 13, function() {
	testParse('fr',"N-3zv*:14.2", '+*** *14 322,3', '14322.3', null);
...
test("string c directive", 15, function() {
	var opt= { 	
		dateSep: '_', 
		datePivot: 1950,
		dynFormats:{
...
test("dynamic formats", 18, function() {
	var opt={
			dynFormats:{
				c: { subType:"A#",complement:"-/_"},
				d: { subType:"c",complement:"*$?%"},
...
test("date series 2", 41 , function() {
	...
	testSimple('D:[debut]DD[.]MM[.]YY[fin]', false, '16/03', "debut16.03.13fin");
	testSimple('D:[debut]DD[.]MM[.]YY[fin]', false, '16/03/63', "debut16.03.63fin");
	testSimple('D:[debut]DD[.]MM[.]YYYY[fin]', false, '16/03/63', "debut16.03.1963fin");
...
test("string series 2", 2, function() {
	testSimple('K:5X>5X', false, 'abcdefgh~~ijklmnopq', 'abcdefgh~~');
	/*
...
test("X3 -> Edit -> Display Format", 21, function() {
	testEditToDisp("fr",  'N+z3:15.5#', '12345678911.03205', '12 345 678 911,03205', '12345678911,03205');
	testEditToDisp("fr",  "D:[debut]DD[.]MM[.]YY[fin]", '20090301', 'debut01.03.09fin', '01/03/09');
	testEditToDisp ("fr", "D:DD[-]MM[-]YY", "20090301", "01-03-09", "01/03/09");
	testEditToDisp ("en", "D:DD[-]MM[-]YY", "20090301", "01-03-09", "03/01/09");
	testEditToDisp("fr",  "N-3zv*:10.3", "1254.256", "+* **1 254,256", "1254,256"); 
	...
	// TODO testEditToDisp ("fr", "D:DD[-]MM", "20090301", "01-03", "01/03");
	// TODO testEditToDisp ("en", "D:DD[-]MM", "20090301", "01-03", "03/01");
	// These case are working because yera=2013 (current year)
	testEditToDisp ("fr", "D:DD[-]MM", "20130301", "01-03", "01/03");
	testEditToDisp ("en", "D:DD[-]MM", "20130301", "01-03", "03/01");
});

test("Parse user input value ", 32 , function() {
	// full date input with out separators
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "281112", "28/11/12");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "112812", "11/28/12");
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "28112012", "28/11/12");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "11282012", "11/28/12");
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "021112", "02/11/12");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "11282012", "11/28/12");
	// full date input with separators
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "28/11/12", "28/11/12");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "11/28/12", "11/28/12");
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "28/11/2012", "28/11/12");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "11/28/2012", "11/28/12");
	// Date with undefined values - take into account serverValue to fill udefined values (year) instead of current year
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "2811", "28/11/99",null, "19990502");
	testEditFormatField('fr', "D:DD[/]MM[/]YY", "28/11", "28/11/99",null, "19990502");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "1128", "11/28/99",null, "19990502");
	testEditFormatField('en', "D:MM[/]DD[/]YY", "11/28", "11/28/99",null, "19990502");
	// Dates
	testEditFormatField('fr', "D:MM[/]YY", "122005", "12/05");
	testEditFormatField('en', "D:MM[/]YY", "122005", "12/05");
	testEditFormatField('fr', "D:MM[/]YY", "125", "12/05");
	testEditFormatField('en', "D:MM[/]YY", "125", "12/05");
	// Use case with display format dateOrder different from edit format dateOrder (country format)
	// Date displayed in English but input by a french user
	testEditFormatField('fr', "D:MM[/]DD[/]YYYY", "12/31/05", "31/12/2005");
	testEditFormatField('en', "D:DD[/]MM[/]YYYY", "31/12/05", "12/31/2005");
	testEditFormatField('fr', "D:MM[/]DD", "311205", "12/31");
	testEditFormatField('en', "D:DD[/]MM", "123105", "31/12");
	// Hours
	testEditFormatField('fr', "K:##[:]##", "12:15", "12:15");
	testEditFormatField('fr', "K:##[:]##", "1215", "12:15");
	// Phone numbers
	testEditFormatField('fr', "K:##[.]##[.]##[.]##[.]##", "0123456789", "01.23.45.67.89");
	testEditFormatField('fr', "K:##[.]##[.]##[.]##[.]##", "01.23.45.67.89", "01.23.45.67.89");
	// Bad date value
