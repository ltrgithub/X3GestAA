...
var config = require('config').x3print || {};
var flows = require('streamline/lib/util/flows');

var PrintClient = require('syracuse-x3/lib/clients/print/PrintClient');

var hostname = config && config.server && config.server.qunit && config.server.qunit.hostname || "ecvmdev2003x64";
var port = config && config.server && config.server.qunit && config.server.qunit.port || 1890;
...
asyncTest("ChangePriority and ListJobs", function(_) {
	...
	for (var j = 0; j < jobs.length; j++) {
		...
		if (j === 2) strictEqual(job.order, 2, "Check priority OK for rank " + job.rank + " and order '+'");
		if (j === 5) strictEqual(job.order, 7, "Check priority OK for rank " + job.rank + " and order '-'");
		if (j === 9) strictEqual(job.order, 1, "Check priority OK for rank " + job.rank + " and order 'MAX'");
		if (j === 0) strictEqual(job.order, 10, "Check priority OK for rank " + job.rank + " and order 'MIN'");
...
var filesName = ["test1.txt", "test2.txt"],
	filesLoc = ["sandbox", "SUPERV@SOLSUPV6$FRA!10"];
var cnxId = "FILE_OPE";
recOptions.fileName = "printApiTest_FILE-1.json";
asyncTest("putFile", function(_) {
