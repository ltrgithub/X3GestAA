...
var Spy = helpers.defineClass(function(_, stream) {
	...
	this.dumpFile = __dirname + "/convergence/data/dump_" + (new Date().toISOString()).replace(/\:/g,'-') + ".js";
...
sessionProto.connect = function(_, context, config) {
	console.log("Direct Fusion Connect");
	...
	if (listReuse && listReuse.length !== 0) {
		...
		var i=0;
		for(i=0;i<listReuse.length && CvgClient!==null ;i++){
			Cvgclient = listReuse[i].isSameContext(reuseContext) ? Cvgclient = listReuse[i] : Cvgclient = null;
		}
		// remove client from reuse list
		if(Cvgclient){
			console.log("find reuse");
			listReuse.splice(i,1);
		}
	}
	var response =null;
	if (!Cvgclient){
		...
		console.log("SESSION OK: " + this.sid);

		// Activate dump
		if (nodeconfig.config && nodeconfig.config.x3fusion && nodeconfig.config.x3fusion.dump)
			this._spy = new Spy(_, client.getStream());
	
		this._fusionClient = new CvgClient(client.getStream(), {
			adxHttpPub: adxHttpPub,
			adxFolder: adxFolder,
		}, {
			syraPreferences: preferences,
		...
		this._fusionClient = Cvgclient;
		console.log("pass reuse");
...
sessionProto.processFormat = function(_, context) {
	...
	var formater = this._fusionClient.formater;
...
sessionProto.close = function(_, reuse) {
	...
	try {
		if (reuse){
			...
			this._fusionClient.isReuse=true;
			listReuse.push(this._fusionClient);
			console.log("add reuse : size "+listReuse.length)
		}
