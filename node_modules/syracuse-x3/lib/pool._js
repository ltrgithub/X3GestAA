...
var cacheClass = require('syracuse-sdata/lib/cacheHelper').CacheClass;
var _configs = {};
...
function _isCached(entityName) {
	return entityName === "SQUOTE";
}

function getCacheClass(_, entityName) {
	if (!dataCacheClass[entityName]) {
		dataCacheClass[entityName] = new cacheClass(_, null, entityName);
	}
	return dataCacheClass[entityName];
}

// create cache action for hits or miss
function _addCacheAction(_, edp, entityName, id, time, isMiss) {
	var inst = getCacheClass(_, "cacheAction").createInstance(_);
	inst.entityName(_, entityName);
	inst.entityID(_, id);
	inst.cacheHit(_, !isMiss);
	inst.endpoint(_, edp);
	inst.responseTime(_, time);

	var t = inst.save(_);
	console.log(sys.inspect(t.$actions.$save.$diagnoses));
}

function _fromCache(_, entityName, id) {
	// fetch me from mongo
	// return { etag: "something", data: ""}
	var inst = getCacheClass(_).getInstance(_, {
		"jsonWhere": {
			"id": id,
			"entity": entityName
		}
	...
	if (inst) {
		return {
			etag: inst.etag(_),
			data: inst.cacheData(_)
		};
	}
	return {};
}

function _toCache(_, entityName, id, cache) {
	var inst = getCacheClass(_).createInstance(_);
	inst.etag(_, cache.etag);
	inst.entity(_, entityName);
	inst.id(_, id);
	inst.cacheData(_, cache.data);
	inst.save(_);
...
*/
	...
	//
	var cache = context.instanceId && (context.method === "get") && _isCached(context.entityName) && _fromCache(_, context.entityName, context.instanceId);
	var time = (new Date()).getTime();
	var clientResp = client.sendRequest(_, request, context.response, cache);
	if (cache && cache.isInvalid) {
		_toCache(_, context.entityName, context.instanceId, cache);
	}
	// for stats
	cache && _addCacheAction(_, context.endpoint, context.entityName, context.instanceId, (new Date()).getTime() - time, cache && cache.isInvalid);
	if (clientResp.statusCode == 500) {
		// destroy client
		request = context.request;
		var endpoint = context.endpoint;
		tracerPool.info && tracerPool.info("X3session: Error 500 so destroying client");
		// _updateSession(!_, request.session.id);
