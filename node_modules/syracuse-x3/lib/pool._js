...
function _updateSession(_, ssid) {
	...
	if (sessionInfo) sessionInfo.x3Sessions(_, exports.allClientIds(ssid).join(", "));
	// shouldn't be saved here !!!
	//sessionInfo.save(_);
}
...
function poolLog(sessid, clientId, created) {
	...
	if (created) {
		tracer && tracer(date + " - connect syrapedia client : session [" + sessid + "] , dataset id [" + clientId + "]");
	} else {
		tracer && tracer(date + " - disconnect syrapedia client : [" + sessid + "] , dataset Id [" + clientId + "]");
	}
}
