...
var tracerPool = require('@sage/syracuse-core').getTracer("x3Comm.pool");
...
function _areSameLocalePreferences(_, pref1, pref2) {
	if (!pref1 || pref1 && !pref2)
		return false;
	return x3helpers.dateFormatToX3Format(pref1.shortDate(_)) === x3helpers.dateFormatToX3Format(pref2.shortDate(_)) && //
		x3helpers.dateFormatToX3Format(pref1.shortDate(_)) === x3helpers.dateFormatToX3Format(pref2.shortDate(_)) && //
		x3helpers.dateFormatToX3Format(pref1.longDate(_)) === x3helpers.dateFormatToX3Format(pref2.longDate(_)) && //
		x3helpers.dateFormatToX3Format(pref1.shortTime(_)) === x3helpers.dateFormatToX3Format(pref2.shortTime(_)) && //
		x3helpers.dateFormatToX3Format(pref1.longTime(_)) === x3helpers.dateFormatToX3Format(pref2.longTime(_)) && //
		x3helpers.dateFormatToX3Format(pref1.shortDatetime(_)) === x3helpers.dateFormatToX3Format(pref2.shortDatetime(_)) && //
		x3helpers.dateFormatToX3Format(pref1.longDatetime(_)) === x3helpers.dateFormatToX3Format(pref2.longDatetime(_)) && //
		pref1.numberDecimalSeparator(_) === pref2.numberDecimalSeparator(_) && //
		pref1.numberGroupSeparator(_) === pref2.numberGroupSeparator(_) && //
		pref1.twoDigitYearMax(_) === pref2.twoDigitYearMax(_) && //
		pref1.firstDayOfWeek(_) === pref2.firstDayOfWeek(_) && //
		pref1.firstWeekOfYear(_) === pref2.firstWeekOfYear(_);
}
	...
	return session.x3funnel(_, function(_) {
		...
		if (client) {
			if (!_isSameRuntimeLogConf(client.activateRuntimeLog, runtimeCfg, endpoint.dataset(_))) {
				client.disconnect(_);
			} else if (!_areSameLocalePreferences(_, client.localePreferences, userProfile.selectedLocale(_))) {
				var applied = client.updateSession(_, {
					preferences: x3helpers.getX3Preferences(_, userProfile.selectedLocale(_))
				});
				if (applied) {
					return client;
				} else {
		...
		client.localePreferences = userProfile.selectedLocale(_);
		//
		client.connect(_, {
		...
		client.createSession(_, {
			sessionType: "primary",
			locale: locale.current,
			localePreferences: client.localePreferences,
...
function _send(_, context) {
	...
	if (clientResp && clientResp.statusCode == 500) {
		// destroy client
