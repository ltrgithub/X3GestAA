...
function _makeClient(_, session, endpoint, wcId) {
	...
	return session.x3funnel(_, function(_) {
		...
		//
		var client = (_clients[session.id] || {})[endpoint.dataset(_)];
		if (client && client.isAlive()) return client;
		//
		if (client) {
			tracer && tracer("dead client: session id=" + session.id);
		...
		//
		(_clients[session.id] = _clients[session.id] || {})[endpoint.dataset(_)] = client;
		//
		return client;
	});
}
...
exports.disconnect = function(_, sessionId) {
	if(_clients[sessionId])
		Object.keys(_clients[sessionId]).forEach_(_, function(_, clientId) {
			_clients[sessionId][clientId].disconnect(_);
		});
