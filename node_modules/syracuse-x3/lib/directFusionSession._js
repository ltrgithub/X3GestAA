...
var protocol = require("./automata/x3Protocol");
...
sessionProto.connect = function(_, context, config) {
	try {
		...
		var ctx = handlers.newContext(client.getStream());
		protocol.process(_, ctx);

		console.log("*** FINISHED CONNECTION SEQUENCE !!! ***");


		throw new Error("FUSION DIRECT INCOMPLETE");
...
/*
		...
		if (x3server.serverName(_)) r.server = x3server.serverName(_);
