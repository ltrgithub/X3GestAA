...
var Spy = helpers.defineClass(function(_, stream) {
	...
	this.dumpFile = __dirname + "/fusion/data/dump_" + new Date().toISOString() + ".js";
...
sessionProto.connect = function(_, context, config) {
	...
	var sid = client.createSession(_, {
		userName: userProfile && userProfile.user(_) && userProfile.user(_).getEndpointLogin(_, endpoint.$uuid),
		adxtyp: 25,
		fusionTech: {
			protocol: "150067",
	...
	//uncomment next line to activate spy
	...
	this._fusionClient = new CAdapiClient(client.getStream(), {
		...
		getUrlServerBlob: function() {return "";}
	});
	this._fusionClient.start(_);
...
sessionProto.processRequest = function(_, context) {
	try {
		...
		if (post.act !== parseInt(context.parameters.act, 10)) throw new Error("act mismatch: expected " + context.parameters.act + ", got " + post.act);
		this._spy && this._spy.dumpJSON(_, "REQUEST", post);
		this._fusionClient.processRequest(_, post);
