...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
		...
		if (cliCfg.recmode == null) {
			...
			if (_listReuse && _listReuse.length !== 0) {
				...
				if (reuseClient) {
					...
					this.sessionInfo = reuseClient.sessionInfo;
					this.sessionInfo.reused(_, false);
	...
	_doAsync: function(_, context, connectAct, bodyStr) {
		...
		if (x3SessionConfig.cvgLog) self._cvgClient.tracer.config = x3SessionConfig.convergence;
		//
		...
		if (trackAct == null) {
			...
			if ('C' === connectAct) { // connection mode
				self._cvgClient.connectSart(_, context);
			...
			context.reply(_, 200, self._cvgClient.generateResponse(_, 'C' === connectAct), self.cvgClient.response.head);
			...
			try {
				if (trackAct == "START") {
					...
					try {
						...
						self._cvgClient.generateResponse(_, 'C' === connectAct);
