...
var convAsync = require("syracuse-x3/lib/convergence/convAsync").convAsync;
...
function _doAsync(_,context,caller){
	...
	if (id && id!=="") { // tracking is passed as query string parameter
		...
		var tracker = caller.async.create(context,_exec);
		...
		if ( id ){
			return caller.async.track(_,context,id);
		}
