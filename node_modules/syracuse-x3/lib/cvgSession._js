...
sessionProto.connect = function(_, context, config) {
	...
	try {
		if (!reuseClient){
			...
			this._cvgClient = new CvgClient(_, client.getStream(), adxInfo, config);
			this._cvgClient.sessionConv = this;
			response = this._cvgClient.start(_);
		}else{
		...
		sendResponse(_, context, 500, getErrDiagnose(e, this._cvgClient, "connectionFailed"));
	}
}
...
sessionProto.processRequest = function(_, context) {
	...
	try{
		...
		data = this._cvgClient.processRequest(_, post);
		//console.log("RESPONSE: " + JSON.stringify(response, null, 2));
		this._spy && this._spy.dumpJSON(_, "RESPONSE", data);

	}catch(e){
		statusCode = 500;
		data = getErrDiagnose(e, this._cvgClient);	
	}
...
sessionProto.proxy = function(_, context, config) {
	try {
		...
		sendResponse(_, context, 500, getErrDiagnose(e, this._cvgClient, "proxyFailed"));
	}	
}
