...
_sessionProto._doAsync = function (_, context,isConnect){
	...
	if (action == null) throw new Error("Unexpected tracking url - trackngId or $trackers is missing");
	try{
		if (action == "START") { 
			// Start tracking request
			if (self._trackers[trackId]) throw new Error("Tracker already exist. trackId[" + trackId + "]");
			tracker = self._trackers[trackId] = new convTracker(_, context, trackId,self._cvgClient);
			...
			try{			
				...
				if( !isConnect ){
					...
					data = self._cvgClient.connectSart(_);
				}
				statusCode = 200;
			}catch(e){
