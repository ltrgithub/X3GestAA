		...
		if (!reuseClient || cliCfg.recmode){
			...
			if (player == null){
				...
				if (cliCfg.recmode == "REC"){
					var opts={fileName:context.parameters.recfile,overwrite:false,x3Func:this.sessInfo.x3Func};
					["x3User","x3Host","x3Port","x3Lang","x3Folder","x3Solution"].forEach(function(p){
						opts[p] = cliCfg[p];
					});
					recorder = recordMgr.newRecorder(_, client.getStream(), opts);
...
_sessionProto._doAsync = function (_, context,isConnect){
	...
	if (action == null) {
		...
		if( isConnect && isConnect === 'C' ){// connection mode
			...
			data =this._cvgClient.execOpenFunction(_,context.parameters.f);
		}else{
