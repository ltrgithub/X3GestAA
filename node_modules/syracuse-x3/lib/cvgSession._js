...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
	...
	_doAsync: function(_, context, actionToDo, bodyStr) {
		var self = this,
			data = null,
			tracker = null,
			excep = null,
		...
		if (trackAct == null) {
			...
			if ("connect" === actionToDo) { // connection mode
				self._cvgClient.connectStart(_, context);
			} else if ("reuse" === actionToDo) { // reuse mode
				self._cvgClient.execOpenFunction(_, context);
			} else if ("requestSvc" === actionToDo) {
				observer.processRequest(_, context, bodyStr);
			} else if ("blobSvc" === actionToDo) {
				observer.services.blobSvc.processRequest(_, context, bodyStr);
			}

			context.reply(_, 200, self._cvgClient.generateResponse(_, 'C' === actionToDo || 'R' === actionToDo), self._cvgClient.response.head);
			if (self._cvgClient.recorder) {
			...
			try {
				if (trackAct == "START") {
					...
					tracker = self._trackers[context.observer][trackId] = new ConvTracker(_, context, trackId, observer, actionToDo != null);
					data = null;
					var statusCode = 500;
					try {
		...
		try {
			...
			switch (svc) {
				...
				default:
					// connection case
					bodyStr = context.request.readAll(_) || "{}";
					this._doAsync(_, context, "connect", bodyStr);
