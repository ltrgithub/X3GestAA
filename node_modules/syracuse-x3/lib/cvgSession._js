...
var CvgClient = require('./convergence/client/CvgClient').CvgClient;
...
var cvgListReuse = require('./cvgListReuse');
var tracerSession = require('@sage/syracuse-core').getTracer("classic.session");
var OfflinePlayer = require('./clients/common/offlinePlayer').OfflinePlayer;
...
var mainFunctions = ["ADSVAL", "GESADS", "CREHISTO", "APATCH", "APATCHA", "PATCH", "ABICREUNV", "ABIVALBAS", "ABIVALTOT", "ABIMAJBAS", "ABIMOVETA", "BATCHRUN", "BATCHSTOP"];
...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
	...
	connect: function(_, context, config) {
		...
		var cliCfg = {
			...
			appHost: cvghelpers.config("x3fusion.appHostOverride") || x3solution.serverHost(_),
		...
		if (cliCfg.recmode == null && !context.parameters.protocolVersion) {
		...
		// override httpSession.closeReuse
		httpSession.closeReuse = function(_, reason) {
			cvgListReuse.closeReuseSession(_, httpSession.id, reason);
		}
		...
		try {
			...
			if (!reuseClient || isActivateRLog) {
				...
				if (player == null) {
					...
					var sid = this.client.createSession(_, {
						...
						fusionTech: {
							protocol: context.parameters.protocolVersion || currentProtocol,
							wkf: context.parameters.f
						},
						localePreferences: userProfile.selectedLocale(_),
						applicationConnectionData: userProfile.applicationConnectionData(_),
				...
				this._cvgClient = new CvgClient(context.parameters.protocolVersion).init(_, this, (player ? player : this.client.getStream()), cliCfg, recorder, context);
			...
			console.error("Connection error " + e)
				// do not respond here but propagate an error to let the cvg service cleaning up the connection
			throw new Error(_getRsrc("connectionFailed"));
		}
	},
	...
	_doAsync: function(_, context, connectAct, bodyStr) {
		...
		if (context.observer && context.observer === "plugin") {
			if (self._cvgClient.cvgPlugin && self._cvgClient.cvgPlugin) {
				...
				// no plugin anymore, we just send a delete to don't open the plugin client side
				context.reply(_, 200, _DEFAULTPLUGINTARGET, self._cvgClient.response.head);
				return;
		...
		if (trackAct == null) {
			...
			try {
				...
				if (excep != null || (tracker && tracker.isFlushed())) {
					...
					if (context.observer && self._trackers && self._trackers[context.observer] && self._trackers[context.observer][trackId]) {
						delete self._trackers[context.observer][trackId];
					}
				}
			}
	...
	cvgUpdateSessionInfo: function(_) {
		...
		if (!cvgSessionInfo) {
			...
			cvgSessionInfo.protocolVersion(_, res.protocolVersion);
			// add cli info
	...
	cvgAdmSessInfo: function(_) {
		...
		res.protocolVersion = this._cvgClient ? this._cvgClient.protocolVersion : "";
