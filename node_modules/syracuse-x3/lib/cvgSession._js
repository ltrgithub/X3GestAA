...
_sessionProto.connect = function(_, context, config) {
	...
	var cliCfg = {
		...
		clientSettings : _readClientSettings(_,requestBody)
	}
...
function _doAsync(_,context,caller){
	...
	if (id && id!=="") { // tracking is passed as query string parameter
		...
		var tracker = sdataAsync.create(context,_exec);
		tracker.location = context.path+"?$trackers=" + tracker.trackngId + "";
		tracker.convStep = 30;
		tracker.caller = caller;
		var convMaxTime = _nodeCfg.x3fusion && _nodeCfg.x3fusion.tracker && _nodeCfg.x3fusion.tracker.maxTimeBeforeSend ? _nodeCfg.x3fusion.tracker.maxTimeBeforeSend : 10000 ;
		...
		return caller.processRequest(_,context);
	}
}
...
_sessionProto.proxy = function(_, context, config) {
	try {
		...
		switch (svc) {
			case 'requestSvc':
				return _doAsync(_,context,this);
			case 'formatSvc':
