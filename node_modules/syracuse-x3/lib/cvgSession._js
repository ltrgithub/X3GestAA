...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
	...
	connect: function(_, context, config) {
		...
		var cliCfg = {
			...
			timeoutReuse: cvghelpers.config("x3fusion.reuseTimeout", null, 20 * 60 * 1000),
		...
		if (cliCfg.recmode == null) {
			...
			funnelReuse(_, function(_) {
				tracerSession.info && tracerSession.info("Create session [Nb reuse: " + _listReuse.length + "]");
				if (_listReuse && _listReuse.length !== 0) {
					...
					if (reuseClient) {
						...
						tracerSession.info && tracerSession.info("\t _listReuse.length" + _listReuse.length + "]");
					} else {
						tracerSession.info && tracerSession.info("Reuse not found !");
					}
				}
			});
		}
	...
	destroyReuseClient: function(_, cvgClient) {
		...
		var i;
		funnelReuse(_, function(_) {
			tracerSession.info && tracerSession.info("destroyReuseClient - nbReused:" + _listReuse.length + " - id:" + cvgClient.uuid);
	...
	_doAsync: function(_, context, connectAct, bodyStr) {
		...
		if (context.observer && context.observer === "plugin") {
			var id = "ilog-default"; // id by default for upload , use the last ilog plugin
			if (bodyStr) {
			...
			if (id) {
				observer = self._cvgClient.cvgPlugin.ND_IHM[id] = self._cvgClient.cvgPlugin.ND_IHM[id] || self._cvgClient.cvgPlugin.ND_IHM["ilog-default"];
				if (id !== "ilog-default")
					observer.id = id;
			} else {
				observer = self._cvgClient.cvgPlugin.ND_IHM["ilog-default"];
			}
		} else {
		...
		if (trackAct == null) {
			...
			try {
				if (trackAct == "START") {
					...
					tracker = self._trackers[context.observer][trackId] = new ConvTracker(_, context, trackId, observer, connectAct === "reuse" || connectAct === "connect");
					data = null;
					var statusCode = 500;
					try {
	...
	close: function(_, params, reason) {
		...
		try {
			if (params.reuse && !this._cvgClient.activateRuntimeLog) {
				...
				funnelReuse(_, function(_) {
					// store CvgClient in reuse list
					self._cvgClient.setInReuseMode(_, true);
					self.sessionInfo.reused(_, true);
					self.cvgUpdateSessionInfo(_);
					self._cvgClient.sessionInfo = self.sessionInfo; // save sessionInfo
					_listReuse.push(self._cvgClient);
					tracerSession.info && tracerSession.info("Add reuse session [size: " + _listReuse.length + "] - id=" + self._cvgClient.uuid + " - reason=" + reason);
				});
