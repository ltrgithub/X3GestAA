		...
		if (trackAct == null) {
			...
			try {
				if (trackAct == "START") {
					...
					tracker = self._trackers[trackId] = new ConvTracker(_, context, trackId, self[observer], connectAct != null);
					data = null;
					var statusCode = 500;
					try {
						if (self._cvgClient.recorder) {
						...
						self[observer].generateResponse(_, 'C' === connectAct || 'R' === connectAct);
						data = self[observer].response;
						statusCode = 200;
					} catch (e) {
						statusCode = 500;
