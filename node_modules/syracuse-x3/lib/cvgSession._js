...
function _sendResponse(_, context, statusCode, data, head) {
	var headers = head || {};
...
_sessionProto.connect = function(_, context, config) {
	...
	try {
		...
		var recorder = null;
		var isActivateRLog = runtimeCfg.enableLog && !runtimeCfg.dataset || endpoint.dataset(_) === runtimeCfg.dataset;
		if (!reuseClient || isActivateRLog) {
			...
			this._cvgClient.activateRuntimeLog = runtimeCfg.enableLog;
