...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
	...
	_doAsync: function(_, context, connectAct, bodyStr) {
		...
		if (trackAct == null) {
			...
			context.reply(_, 200, self._cvgClient.generateResponse(_, 'C' === connectAct || 'R' === connectAct), self.cvgClient.response.head);
			if (self._cvgClient.recorder) {
	...
	close: function(_, reuse, reason) {
		tracerSession.info() && tracerSession.info("Close session [reuse:" + (reuse === true) + "] : " + this.sid);
		try {
			if (reuse && !this._cvgClient.activateRuntimeLog) {
				...
				tracerSession.info() && tracerSession.info("Add reuse session [size: " + _listReuse.length + "] - id=" + this._cvgClient.uuid + " - reason=" + reason);
			} else if (this._cvgClient) {
				this.sessionInfo.deleteSelf(_);
	...
	cvgUpdateSessionInfo: function(_) {
		...
		return session.updateFunnel(_, function(_) {
			//tracerSession.info() && tracerSession.info("cvgSession.updateSessionInfo at " + new Date());
