...
_sessionProto.connect = function(_, context, config) {
	...
	try {			
		...
		if (!reuseClient){
			...
			if (player == null){
				...
				if (cliCfg.recmode == "REC"){
					var opts={fileName:context.parameters.recfile,overwrite:false,x3Func:this.sessInfo.x3Func, cliCfg : {}};		
		...
		if (this._cvgClient && this._cvgClient.clientConfig.modePLAY){
			...
			var diag = _getErrDiagnose(e, this._cvgClient);
		}else{
			var diag = _getErrDiagnose(e, this._cvgClient, "connectionFailed");
		}
		// we set location in order to let client to retreive cvgSessionId even if error
		_sendResponse(_, context, 500, diag, {
...
_sessionProto._doAsync = function (_, context,isConnect){
	var self = this, tracker = null, excep = null;
	var trackId = context.parameters.trackngId || "", action=null;
	if (trackId.length>0){
		...
		if (trackId.length>0) action = "TRACK";
	}	
	if (action == null) {
		// execute request in synchronous mode
		var data ;
		if( isConnect && isConnect === 'C' ){// connection mode
			data =this._cvgClient.connectSart(_,context);
		}else if( isConnect && isConnect === 'R' ){ // reuse mode
			data =this._cvgClient.execOpenFunction(_,context);
		}else{
		...
		try{
			if (action == "START") { 
				...
				tracker = self._trackers[trackId] = new convTracker(_, context, trackId,self._cvgClient,isConnect);
				...
				try{			
					...
					if( !isConnect ){
						data =  self._cvgClient.processRequest(_, context);
					}else{
						data = self._cvgClient.connectSart(_,context);
					}
					statusCode = 200;
				}catch(e){
					// console.log("\tprocessRequest.error");
					data = _getErrDiagnose(e, self._cvgClient);	
					statusCode = 500;
				}finally{
					// console.log("\tprocessRequest.end")
