...
_sessionProto.connect = function(_, context, config) {
	...
	try {
		if (!reuseClient){
			...
			if (context.parameters.recmode == "REC") this._recStream = recordMgr.newStreamRec(_, client.getStream(),{fileName:context.parameters.recfile,overwrite:true});
			if (context.parameters.recmode == "PLAY") this._playStream = recordMgr.newStreamPlay(_, client.getStream());
			this._cvgClient = new CvgClient(_, client.getStream(), cliCfg, this._recStream);
