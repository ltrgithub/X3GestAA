...
_sessionProto._doAsync = function (_, context, connectAct, bodyStr){	
	var self = this, data = null, tracker = null, excep = null, trackId = context.parameters.trackngId || "", trackAct=null;
	if (trackId.length>0){
		...
		try{
			if (trackAct == "START") { 
				...
				try{
					...
					statusCode = 500;
					context.reply(_,statusCode , data.body, data.head);			
				}finally{
			...
			excep = e;
			var body= e.toJSON ? e.toJSON():new CvgException(self._cvgClient,e,"").diagnose.toJSON()
