...
function _doAsync(_,context,caller){
	...
	if (id && id!=="") { // tracking is passed as query string parameter
		...
		var convMaxTime = _nodeCfg.x3fusion && _nodeCfg.x3fusion.tracker && _nodeCfg.x3fusion.tracker.maxTimeBeforeSend ? _nodeCfg.x3fusion.tracker.maxTimeBeforeSend : 4000 ;
	...
	try {
		...
		switch (svc) {
			case 'requestSvc':
				...
				break;
			case 'formatSvc':
				this._cvgClient.services.formatSvc.processRequest(_, context);
				break;
			case 'blobSvc':
