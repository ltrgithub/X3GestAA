...
var offlinePlayer = require("syracuse-x3/lib/clients/common/offlinePlayer");
...
var CvgSession = helpers.defineClass(function _cvgSession(fsid) {
	...
	connect: function(_, context, config) {
		...
		function getOfflinePlayer(_, fileName) {
			...
			return new OfflinePayler(JSON.parse(fs.readFile(_defDataDir + "/" + fileName)));
		}
		var startConnectTime = new Date().getTime();
		...
		try {
			...
			if (!reuseClient || isActivateRLog) {
				...
				ga.hitTiming(_, context, "Classic connection loading time", "First", (new Date().getTime()) - startConnectTime, this.sessInfo.x3Func);
				...
				ga.hitTiming(_, context, "Classic connection loading time", "Reuse", (new Date().getTime()) - startConnectTime, this.sessInfo.x3Func);
			}
			if (requestBody) {
