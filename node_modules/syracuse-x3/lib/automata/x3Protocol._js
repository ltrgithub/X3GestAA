...
function readList(_, ctx, letter) {
	...
	while (true) {
		...
		var req = new X3Reader(ctx, head[0], ctx.stream.read(_, len));
...
function processNode(_, ctx, req, node) {
	var handler = exports.defaultHandler;
	var args = [req, node];
	...
	trace && console.log("NODE HANDLER: " + node.name + JSON.stringify(args.slice(2)));
...
function readRequest(_, ctx) {
	...
	return new X3Reader(ctx, head[0], len ? ctx.stream.read(_, len) : new Buffer(0));
}
