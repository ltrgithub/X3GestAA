...
var _applicationMap = {
	walk: function(_, context, name, id) {
		...
		//
		var res = _dispatch(_, context, _contractMap);
		//console.log("time application "+(Date.now()-time));
		return res;
	}
};
...
var _contractMap = {
	walk: function(_, context, name, id) {
		if (context.applicationRef) { // TEST to allow stubs
			...
			//console.log("get endpoint "+((new Date().getTime())-time));
		...
		var res = _dispatch(_, context, _datasetMap);
		//console.log("time contract "+((new Date().getTime())-time));
...
var _datasetMap = {
	...
	sessions: function(_, context, id) {
		...
		context.observer = "client";
		// TEMPORARY WAITING FOR CLIENT ENHANCEMENTS
		context.parameters.timing = context.parameters.timing || true;
		var res = _dispatch(_, context, _connectMap);
		//console.log("timeSession "+ ((new Date()).getTime() - time));
		return res;
	},
	files: function(_, context, id) {
		cvgServices.webProxy(_, context, context.endpoint.getX3FolderName(_) + "/" + context.toWalk(), _config);
	},
...
exports.x3dispatcher = function(config) {
	...
	return function(_, request, response) {
		var timeFull = (new Date()).getTime();

		var context = new Context(request, response);
		request.context = context;
		_doIt(_, context);
		//console.log("timefull "+((new Date()).getTime() - timeFull));
