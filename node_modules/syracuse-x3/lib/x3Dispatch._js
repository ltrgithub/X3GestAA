...
var _datasetMap = {
	...
	studio: function(_, context, id) {
		var ses = studio.getStudioSession(context);
		if (!ses || !ses.$uuid) {
			console.error("Studio session not registered: $uuid=" + (ses || {}).$uuid);
			throw context.forbidden("Not registered");
		}
		context.cvgSessionId = id;
		context.observer = "studio";
		context.httpSession.setData("adxtyp", 30); // 6 % 8
	...
	plugin: function(_, context, id) {
		...
		context.observer = "plugin"; // id of plugin is defined in the body of the request
		return _dispatch(_, context, _connectMap);
	},
...
function getErroReply(ex) {
	...
	if (ex.toJson) {
		...
		bdy = {
			...
			"$diagnoses": [{
				message: x3helpers.errShortPath(ex.stack ? ex.safeStack : ex.toString())
			}],
...
function _doIt(_, context) {
	try {
		...
		if (ex.toJson) {
			// ex is a fusion error
			bdy = ex.toJson();
		} else if (ex.diagnose && ex.diagnose.toJSON) {
			// ex is a fusion error
			bdy = ex.diagnose.toJSON();
		} else {
			bdy = {
				$message: x3helpers.errShortPath(ex.message ? ex.message : ex.toString()),
				"$diagnoses": [{
