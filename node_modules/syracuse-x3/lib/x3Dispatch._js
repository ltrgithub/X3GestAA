...
function getProtoUrl(_, context) {
	...
	if (_config && _config.prototypesLocalServerRoot) url = [_config.prototypesLocalServerRoot];
...
var _prototypeMap = {
	get: function(_, context) {
		function getPrototypeResp(_, protoUrl) {
			...
			if (helpers.stubsPath && _config.prototypesLocalServerRoot && protoUrl.indexOf(_config.prototypesLocalServerRoot) === 0) {
...
var _connectMap = {
	...
	post: function(_, context) {
		var result = cvgServices.connect(_, context, _config);
		var b = result.body;
		if (b) {
			...
			try {
				...
				if (!b.srvop.sessionInfo) {
					b.srvop.sessionInfo = {};
				}
				if (nodeconfig.config.x3fusion && !nodeconfig.config.x3fusion.undirect) {
					b.srvop.sessionInfo.nodeWS = true;
					b.srvop.sessionInfo.sid = result.sid;
				}
				var nodeCtx = b.srvop.sessionInfo.node = {
					proto: getProtoUrl(_, context),
					fusion: context.endpoint.getFusionDataServerBaseUrl(_),
					sessSettings: sessionManager.getSettings(_)
				}
				var x3server = context.endpoint.x3server(_);
				// add more technical info
				var runtimeCtx = b.srvop.sessionInfo.runtime;
				runtimeCtx.x3host = x3server.serverHost(_);
				runtimeCtx.x3port = x3server.serverPort(_);


				var up = context.httpSession.getUserProfile(_);
				if (up && up.user(_)) {
					nodeCtx.x3user = up.user(_).getEndpointLogin(_, context.endpoint.$uuid);
					runtimeCtx.x3user = nodeCtx.x3user;	
				}
				if (x3server) {
					nodeCtx.x3host = x3server.serverHost(_);
					nodeCtx.x3port = x3server.serverPort(_);
					nodeCtx.x3name = x3server.serverName(_);
				}
				try {
					// Last modif until availabilty of Syracuse version number
					var stat = fs.statSync(__dirname + "/../../../.git/FETCH_HEAD");
					nodeCtx.gitlastmodif = stat.mtime.toLocaleString();
				} catch (e) {}
			} catch (e) {}
		}
	...
	"delete": function(_, context) {
			for (var i in context.parameters) {
				if (i.indexOf("sid_") > -1) {
					...
					context.httpSession.closeCvgSession(sid, false, _);
				}
			}
			context.httpSession.closeCvgSession(context.cvgSessionId, context.parameters.reuse, _);
			if (!context.parameters.panic)
...
exports.x3dispatcher = function(config) {
	...
	return function(_, request, response) {
		...
		try {
			...
			return context.reply(_, ex.httpStatus == undefined ? 500 : ex.httpStatus, bdy);
		}
	}
}
