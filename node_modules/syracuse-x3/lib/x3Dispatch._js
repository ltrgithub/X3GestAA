...
*/		
	...
	files: function(_, context, id) {
		x3FusionPool.webProxy(_, context, context.endpoint.x3ServerFolder(_) + "/" + context.toWalk());
	},
	shared: function(_, context, id) {
		x3FusionPool.webProxy(_, context, "X3_ROOT/" + context.toWalk());
	}
}
...
var _connectMap = {
	...
	post: function(_, context) {
		var result = x3FusionPool.connect(_, context);
		context.reply(_, result.statusCode, result.body, result.headers);
	},
	"delete": function(_, context) {
		context.session.closeFusionSession(context.fusionSessionId, context.parameters.reuse, _);
	}
}
