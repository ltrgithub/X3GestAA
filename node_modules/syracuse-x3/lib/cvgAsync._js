	...
	track: function(_, response) {
		...
		try {
			if (self.isFinished()) {
				...
				self.client.generateResponse(_, self.isConnect, false);

				if (self.client.response.head.statusCode > statusCode) {
					statusCode = self.client.response.head.statusCode;
				}
				...
				var body = JSON.stringify({
					...
					remainingSeconds: 0,
					// We don't send partial responses in record mode
					reply: self.client.recorder == null ? self.client.response.body : {}
				});
