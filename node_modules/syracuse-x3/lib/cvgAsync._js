...
exports.cvgTracker = helpers.defineClass(function(_, context, trackId,cvgClient) {
	this._trackId = trackId;
	this.client = cvgClient;
	this.partialResponse ;
    this._location = context.path + "?$trackers=" + trackId + "";
    ...
    track: function(_, response) {
		...
		if (self._timeoutId != null){
    		_log && _log(this, "\tclearTimeout");
			clearTimeout(self._timeoutId);
			self._timeoutId = null;
		}
		self._tracked = true;
		_log && _log(this, "tracker.TRACK - isFinished[" + self.isFinished() + "]");
    	try{
			if (self.isFinished()) {
				...
				this.client.razModification();
				response.write(_, JSON.stringify(body), "utf8");				
				response.end();
			} 
    	}catch(e){
    		_log && _log(this, "\ttracker.TRACK.ERROR - " + (e.stack ? e.stack : e.message ? e.message : e));
    		throw e;
    	}
    },
