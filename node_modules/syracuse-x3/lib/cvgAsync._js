	...
	track: function(_, response) {
		...
		try {
			if (self.isFinished()) {
				...
				self[self.observer].generateResponse(_, self.isConnect, false);


				response.writeHead(202, head);
				// We don't send partial responses in record mode
				if (self[self.observer].recorder == null) self[self.observer].razModification();
				var body = JSON.stringify({
					...
					remainingSeconds: 0,
					// We don't send partial responses in record mode
					reply: self[self.observer].recorder == null ? self[self.observer].response.body : {}
				});
