...
var nocompress = config && config.hosting && config.hosting.nocompress;

var TrackerResponse = helpers.defineClass(function(mustCompress) {
	this.status = 202;
	this.head = {
	...
	this.mustCompress = mustCompress && !nocompress;
}, null, {
	...
	flush: function(_, response) {
		this.isFlushed = true;
		if (this.mustCompress) this.head["content-encoding"] = "gzip";
		response.writeHead(this.status, this.head);

		for (var i = 0; i < this.chunks.length; i++) {
			var data = this.mustCompress ? zlib.gzip(new Buffer(this.chunks[i], this.encoding), ~_) : this.chunks[i];
			response.write(_, data);
		}
		response.end();
	},
	end: function(data) {
...
exports.CvgTracker = helpers.defineClass(function(_, context, trackId, cvgClient, isConnect) {
	...
	track: function(_, response) {
		...
		try {
			if (self.isFinished()) {
				...
				var mustCompress = !nocompress && self.context.request.headers['accept-encoding'] && self.context.request.headers['accept-encoding'].split(',').indexOf('gzip') !== -1;
				...
				if (mustCompress) head["content-encoding"] = "gzip";

				response.writeHead(202, head);
				var respJson = self.client.getResponse(_, self.isConnect, false);
				...
				var body = JSON.stringify({
					...
					pollingMillis: self._pollingMillis ? self._pollingMillis : 1000,
					// unused
					phaseDetail: "",
					progress: null,
					remainingSeconds: 0,
				...
				if (mustCompress) {
					response.end(zlib.gzip(new Buffer(body, "utf8"), ~_));
				} else {
					response.end(_, body, "utf8");
				}
			}
		} catch (e) {
	...
	start: function(_, context, waitMs) {
		...
		var mustCompress = self.context.request.headers['accept-encoding'] && self.context.request.headers['accept-encoding'].split(',').indexOf('gzip') !== -1;
		context.response = self._trackResponse = new TrackerResponse(mustCompress);
