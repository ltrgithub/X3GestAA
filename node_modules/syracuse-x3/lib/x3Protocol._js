...
var DESCS = [
	...
	[87, "NDOWIN", "IUUU[UF"], /*  Ouverture Fenetre	*/
...
function processNode(_, ctx, req, node) {
	...
	console.log("NODE HANDLER: " + node.name + JSON.stringify(args.slice(3)));
...
function readRequest(_, ctx) {
	...
	return new Request(head[0], len, len ? ctx.stream.read(_, len) : new Buffer(0));
}
