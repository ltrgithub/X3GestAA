...
var Request = exports.Request = helpers.defineClass(function(ctx, wnd, data) {
	this.ctx = ctx;
	this.wnd = wnd;
	this.pos = 0;
	this.data = data;
	...
	nextIst: function() {
		return this.pos < this.data.length - 5 ? new CStructIst(this.nextLong(), this.nextShort()) : 0;
	},
	nextString: function() {
	...
	nextDefs: function() {
		...
		var sub = new Request(this.ctx, this.wnd, buf);
		var n = sub.nextByte();
		var defs = [];
		for (var i = 0; i < n; i++) defs.push(CStructResu.read(sub));
		return defs;
	},
	...
	nextDate3: function() {
		// TODO: check this
		return (this.nextShort() << 8) + this.nextByte();
	},
