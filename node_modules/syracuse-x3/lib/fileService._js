...
var tracer = ((config.x3sadfsq || {}).fileService || {}).trace;
	...
	return {
		getClient: function(_, endpoint) {
			var x3server = endpoint.x3server(_),
				key = (x3server.serverName(_) ? x3server.serverName(_) : x3server.serverHost(_)) + ":" + x3server.serverPort(_),
				q = pools[key] || (pools[key] = []),
				entry = q[0],
				client;
			if (entry) {
				...
				tracer && tracer("[sadfsqPool] getClient create new client");
				client = new SadFsqClient(_, (x3server.serverName(_) ? x3server.serverName(_) : x3server.serverHost(_)), x3server.serverPort(_), null, null, false);
				if (client.release)
...
function SadfsqWritableStream(endpoint, fileOptions, options) {
	...
	this.endpoint = endpoint;
	this.fileOptions = fileOptions;
	tracer && tracer("[SadfsqWritableStream] new instance name=" + (this.fileOptions.path + "/" + this.fileOptions.name + "." + this.fileOptions.extension));
}

SadfsqWritableStream.prototype = {
	write: function(_, data, enc) {
		if (data) tracer && tracer("[SadfsqWritableStream] receive data len=" + data.length);
		if (this.endpoint.protocol(_) !== "x3")
			throw new Error("Bad endpoint protocol");
		if (!this.fd) {
			this.client = sadfsqPool.getClient(_, this.endpoint);
			var fullname = this.fileOptions.path + "/" + this.fileOptions.name + "." + this.fileOptions.extension;
			tracer && tracer("[SadfsqWritableStream] start writing to name=" + fullname + " on " + this.client.hostname + ":" + this.client.port);
			this.fd = this.client.open(_, this.fileOptions, "w");
		}
		if (data == null) {
			tracer && tracer("[SadfsqWritableStream] final write fd=" + this.fd + ", length=" + this.position);
			...
			tracer && tracer("[SadfsqWritableStream] write fd=" + this.fd + ", enc=" + enc + " [" + this.position + ":" + data.length + "]");
			try {
				this.client.write(_, this.fd, buffer, 0, buffer.length, null, this.options.lock);
				this.position += buffer.length;
			} catch (e) {
				tracer && tracer("EXCEPTION: " + e.message + "\nat" + e.stack);
				// do not reuse it
...
exports.deleteFile = function(_, endpoint, path) {
	...
	try {
		var fileOptions = _getFileOptions(_, endpoint, path);
		client = sadfsqPool.getClient(_, endpoint);
