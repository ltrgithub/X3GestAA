...
var Dispatcher = helpers.defineClass(function(context) {
...
//					console.log("Content="+ content);
		...
		try{
			...
			if (isProxyDispatch() == true){
				...
				this.context.response.writeHead(response.statusCode, response.headers);
				
				var buf;
				while(buf = response.read(_)){
					//console.log("RESP="+buf.toString('utf8'));
					this.context.response.write(_, buf, "utf8");
				}

				this.context.response.end();
			}else{
				this.context.response.write(_, this.content);
				this.context.response.end();
			}
...
exports.create = function(response){
	return new Dispatcher(response);
}