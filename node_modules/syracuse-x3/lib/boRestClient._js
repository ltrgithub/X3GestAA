...
exports.proxy = function(_, context, boServerPath) {
	...
	var boServer = context.endpoint.boServer(_);
	if (boServer) {
...
exports.boRestClient = helpers.defineClass(function(_, profile, server, context) {
	...
	init: function(_) {
		...
		if (!this.boServer && !this.profile) {
			this.boServer = getBoServer(_, this.context.endpoint);
		}
	...
	logon: function(_, withAdmin) {
		...
		if (!withAdmin && !this.profile) {
			this.profile = getBoProfile(_, this.context);
		}
...
var getBoServer = function(_, endpoint) {
	var boserver = endpoint.boServer(_);
	if (!boserver) throw new Error(locale.format(module, "noBoServer", endpoint.description(_)));
	return boserver;
};
...
var getBoProfile = function(_, context) {
	...
	var boserver = getBoServer(_, context.endpoint);
	...
	if (!profile) throw new Error(locale.format(module, "noBoProfile", role.description(_), context.endpoint.description(_)));
	return profile;
};