...
exports.boRestClient = helpers.defineClass(function(_, profile, context) {
	...
	logon: function(_) {
		var prof = null;
		if (!this.profile) {
			prof = getBoProfile(_, this.context);
		} else {
			prof = this.profile;
		}

		this.bosec = prof.security(_);
		this.bouser = prof.user(_);
		this.bopwd = prof.password(_);
		var boServer = prof.boServer(_);
		...
		if (!this.token) {
			throw Error("logon failed " + (this.fail ? "(" + this.fail + ")" : ""));
		}
		return this.token;
	},
	/*
