...
exports.format = function(settings, fmt, val) {
	...
	var len = fmt.length,
		j = 0,
		result = "",
		pwc, now, integ, decim = "", fill,
		tail = "",
		cur, n3 = 0,
		replaceBad = false,
		numPart = 0, 
		extra = 0; // number of chars added to integral part

	function badFormat() {
		return "Unexpected format character [" + ch + "] - Position[" + (j - 1) + "] - Format='" + fmt + "'";
	}

	function _fmtInt(val, rep, forbid0) {
		var s;
		...
		K: function() {
			for (; rep > 0; rep--) {
				...
				default:
					...
					if (!/[c-z]/.test(ch)) throw new Error(badFormat());
				}
...
/*
			...
			function flush() {
				...
				if (cur.length < rep + extra) {
					...
					result += cur;
				} else if (cur.length === rep + extra) {
					result += cur;
				} else {
					result += cur.substring(0, rep);
					if (numPart === 0) valid = false;
				}
				extra = 0;
				cur = null;
			}
			...
			case '#':
				if (numPart === 0) {
					...
					if (cur.length < rep) {
						...
						result += cur;
						while (cur.length < rep--) result += '0';
						cur = '';
					} else {
						result += cur.substring(0, rep);
						cur = cur.substring(rep);
					}
				}
				numPart++;
				break;
			case '.':
				if (numPart > 1) throw new Error("too many . directives in format");
				flush();
				result += special[3]
				cur = decim;
				numPart = 1;
		...
		D: function() {
			switch (ch) {
			case 'D':
				result += _fmtInt(val.day, rep, true);
				break;
			case 'M':
				if (rep <= 2) {
					result += _fmtInt(val.month, rep, true);
				} else if (rep === 3) {
					result += _months3[val.month - 1] || (fill + fill + fill);
				} else {
					result += _monthName(val.month, rep);
				}
				break;
			case 'Y':
				result += _fmtInt(rep <= 2 ? val.year % 100 : val.year, rep, val.year === 0);
				break;
			case 'h':
	...
	try {
		...
		if (fn) {
			for (i = 1; i < fmt.length && (ch = fmt[i]) !== ':'; i++) {
				...
				case 'v':
					...
					if (tch !== 'N' && _isDigit(fmt[i + 1])) options.v = parseInt(fmt[++i]);
					break;
				default:
					options[ch] = true;
					break;
		...
		fill = options['*'] ? special[5] : (options['0'] ? '0' : ' ');
