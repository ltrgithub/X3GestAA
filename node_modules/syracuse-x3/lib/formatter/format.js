...
function process(settings, fmt, val, parse) {
	...
	function _throwBadFormat(txt) {
		badFormat=true;
		throw new Error(txt ? txt :"Unexpected format character [" + fmtChar + "] - Position[" + (srcPos - 1) + "] - Format='" + fmt + "'");
	}
			...
			if (dyn) {
				...
				for (var i = 0; i < dyn.subType.length; i++) {
					...
					if (dynTypes.indexOf(dynChar) < 0){
						...
						if (valid) { valid = savValid; return ch; }
					}
				}
				if (dyn.complement && dyn.complement.indexOf(valChar) >= 0) { valid = savValid; return valChar; }
				...
				if (depth === 0) {
					...
					_throwBadFormat("Unexpected character - Val[" + val + "] - Position[" + (srcPos - 1) + "] - dynamicFormat='" + fmt + "{"+resolveFmt(fmt.substr(fmt.indexOf(':')+1))+"}");
				}
				return valChar;
			}else _throwBadFormat();	
		}
		return valChar;
	}
	...
	function numberFormatter() {
		function flush() {
			...
			if (partStr.length < repeat + numExtra) {
				...
				if (options['3'] && options['*'] && integ==partStr && partStr.length < repeat){
					var i=integ.search(/\d/);
					var tmp=integ,head='';
					if (i>=0) {
						// remove non digit leading chars and restore after
						tmp=integ.substring(i);
						head=integ.substring(0,i);
					}
					var r=repeat;
					while (partStr.length < r){
						if (tmp.length % (Math.floor(tmp.length/4)*4+3) == 0){
							tmp = special[2] + tmp;
							r--;
						}
						tmp = fillChar + tmp;	
				...
				if (partNum === 0) {
					...
					throw Error("Number of digit overflow - ["+partStr+"] - Expected length["+ (repeat + numExtra) +"]");
				}
			}
			numExtra = 0;
			partStr = null;
		...
		case '#':
			if (partNum === 0) {
				...
				if (partStr.length < repeat) {
					...
					if (settings.strict && partStr.length>repeat){
						throw Error("Bad decimal part [" + partStr + "] - Expected[" + repeat + "]");
					}
					result += partStr.substring(0, repeat);
	...
	function numberParser() {
		...
		if (result === '') {
			...
			while ((ch = val[srcPos]) && (ch === fillChar || ch === special[2]) && options['*']) srcPos++; 
		}
		while (ch = val[srcPos]) {
		...
		}else if (ch === special[3] && settings.strict){
	...
	try {
		...
		if (typeChar === 'N' && !parse) {
			...
			if (options["3"]) {
				...
				// FDB - No thousand sep if 3 digit 
				var l=integ.length - (integ[0] === '+' || integ[0] === '-' ? 1 : 0);
				if (l>3){
		...
		if (typeChar === 'N' && settings.strict && decim && decim.length>0 && partNum==1){
			// FDB - Strict
			throw Error("No decimal part expected");
		}
		if (val && typeChar === 'K' && settings.strict && val.length>maxLength){
			// FDB - Strict
			throw Error("Size overflow - nbRead[" + val.length + "] - Expected[" + maxLength + "]");
		}
		if (parse) {
			if (fmt[0] === 'D') {
			...
			if (typeChar === 'N' && result.length>0 && result[result.length-1]==special[3]){
		...
		if (!parse && badFormat===true && settings.skipBadFormat===true)	{
			// I order to log a diagnose - This error could occur if X3 field contains a wrong format
			// -> User can enter a value and is not stuck on the field
			if (settings.tracer) settings.tracer("Bad X3 format - " + ex.message);
			return val;
		}
		//console.log("FORMAT ERROR-->" + ex.message);
		throw ex;
	}
}
