			...
			if (partNum === 0) {
				...
				if (partStr.length < repeat) {
					...
					if (settings.strict && partStr.length>repeat){
						throw new Error("Bad decimal part [" + partStr + "] - Expected[" + repeat + "]");
					}
					result += partStr.substring(0, repeat);
	...
	function numberParser() {
		...
		if (result === '') {
			...
			if (ch === '-' && result === '') {
				...
				if (options['-']) srcPos++;
				else if (settings.strict) throw new Error("Unexpected positive sign"); //+ sign only if options['-'] 
	...
	try {
		...
		if (parse) {
			...
			if (typeChar === 'N'){
				...
				// if (!valid) result = result.substring(0, lastValid);
			}
