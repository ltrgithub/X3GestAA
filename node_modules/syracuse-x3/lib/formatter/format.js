		...
		default:
			...
			if (dyn) {
				...
				if (depth === 0) {
					...
					throw new Error("Unexpected character - Val[" + val + "] - Position[" + (srcPos - 1) + "] - dynamicFormat='" + fmt + "{"+resolveFmt(fmt.substr(fmt.indexOf(':')+1))+"}"); //_mismatch(valChar, "custom format " + fmtChar, valChar === ' ');
				}
				return valChar;
			}else throw new Error(badFormat());	
		}
		return valChar;
	}
...
exports.format = function(settings, fmt, val) {
	...
	// console.log("\tformat - format:" +fmt + " -  value:'"+val+"' - res='"+res+"'");
	return res;
}
