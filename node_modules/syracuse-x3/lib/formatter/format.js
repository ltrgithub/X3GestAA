...
function process(settings, fmt, val, parse) {
	...
	function formatChar(fmtChar, valChar, depth, ignore) {
		...
		case 'A':
			if (!_isUpper(valChar)) {
				...
				else _mismatch(valChar, "uppercase letter", ignore || valChar === ' ');
			}
			break;
		case 'a':
			if (!_isLower(valChar)) {
				if (_isUpper(valChar) && !options.A) valChar = valChar.toLowerCase();
				else _mismatch(valChar, "lowercase letter", ignore || valChar === ' ');
			}
			break;
		case 'B':
			if (!_isUpper(valChar) && !_isDigit(valChar)) {
				if (_isLower(valChar) && !options.A) valChar = valChar.toUpperCase();
				else _mismatch(valChar, "uppercase letter or digit", ignore || valChar === ' ');
			}
			break;
		case 'b':
			if (!_isLower(valChar) && !_isDigit(valChar)) {
				if (_isUpper(valChar) && !options.A) valChar = valChar.toLowerCase();
				else _mismatch(valChar, "lowercase letter or digit", ignore || valChar === ' ');
			}
			break;
		case 'C':
			if (!_isLower(valChar) && !_isUpper(valChar) && !_isDigit(valChar)) {
				_mismatch(valChar, "letter or digit", ignore || valChar === ' ');
			}
			break;
		case 'L':
			if (!_isLower(valChar) && !_isUpper(valChar)) {
				_mismatch(valChar, "lowercase letter", ignore || valChar === ' ');
			}
			break;
		case 'H':
			if (!_isHexa(valChar)) {
				_mismatch(valChar, "hexadecimal character", ignore || valChar === ' ');
			}
			break;
		case ' ':
			if (fmtChar !== ' ') {
				_mismatch(valChar, "space", ignore || valChar === ' ');
			}
			break;
		case '#':
			if (!_isDigit(valChar)) {
				_mismatch(valChar, "digit", ignore || valChar === ' ');
			}
			break;
		default:
			...
			if (dyn) {
				...
				if (depth === 0) _mismatch(valChar, "custom format " + fmtChar, valChar === ' ');
				return valChar;
			}
			// we get lower case letter in unit tests - accept them silently for now
			else if (!/[c-z]/.test(fmtChar)) throw new Error(badFormat());			
		}
		return valChar;
	}
	...
	function stringWalker() {
		...
		for (; repeat > 0; repeat--) {
			...
			valChar = formatChar(fmtChar, valChar || ' ', 0);
