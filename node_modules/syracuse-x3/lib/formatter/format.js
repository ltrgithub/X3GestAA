...
function _monthName(month, repeat) {
	...
	s += ' ';
	return s.length > repeat ? s.substring(0, len) : s;
}
...
exports.format = function(settings, fmt, val) {
	...
	var len = fmt.length,
		srcPos = 0,
		result = "",
		secretChar, now, integ, decim = "", fillChar,
		tail = "",
		cur, n3 = 0,
		replaceBad = false,
		numPart = 0, 
	...
	var formatters = {
		K: function() {
			for (; repeat > 0; repeat--) {
				...
				if (secretChar && defined) vch = secretChar;
				result += vch;
			}
		},
		N: function() {
			...
			function flush() {
				if (!cur) return;
				if (cur.length < repeat + extra) {
					if (fillChar === '0' && cur[0] === '-') {
						result += '-';
						cur = cur.substring(1);
						repeat--;
					}
					while (cur.length < repeat--) result += fillChar;
					result += cur;
				} else if (cur.length === repeat + extra) {
					result += cur;
				} else {
					result += cur.substring(0, repeat);
			...
			case '#':
				if (numPart === 0) {
					...
					if (cur.length < repeat) {
						// X3 engine does not follow spec: Z option is now set by default
						result += cur;
						while (cur.length < repeat--) result += '0';
						cur = '';
					} else {
						result += cur.substring(0, repeat);
						cur = cur.substring(repeat);
					}
				}
				numPart++;
				break;
			...
			case 'F':
				...
				if (cur.length <= repeat) {
					if (decim) {
						...
						cur = (cur + special[3] + decim).substring(0, repeat);
					}
				}
				integ = null;
				decim = null;
	...
	try {
		...
		if (fn) {
			for (fmtPos = 1; fmtPos < fmt.length && (fmtChar = fmt[fmtPos]) !== ':'; fmtPos++) {
				...
				case 'v':
					...
					if (tch !== 'N' && _isDigit(fmt[fmtPos + 1])) options.v = parseInt(fmt[++fmtPos]);
					break;
				default:
		...
		if (!valid) {
			...
			if (replaceBad) result = result.replace(/./g, badChar);
