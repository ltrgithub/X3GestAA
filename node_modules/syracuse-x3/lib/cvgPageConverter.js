...
var _normalizerMap = {
    $X3Name: function(key, prop){
        if (!prop.$X3Name && (key[0] === "b")) 
            prop.$X3Name = "LeftList_" + key;
    },
    ...
    return function(proto, $article){
        ...
        function fillMap(elt){
            Object.keys(elt.$properties).forEach(function(key){
                ...
                if (prop[from]) {
                    ...
                    if (prop.$item && prop.$type === "application/x-array") {
        ...
        function fixArticle(art){
            ...
            if (Array.isArray(art)) 
                art.forEach(fixArticle);
            else 
                if (typeof art === 'object') {
                    if (art.$bind != null) {
                        if (map[art.$bind]) 
                            art.$bind = map[art.$bind];
                        else 
                            throw new Error("name missing for  " + art.$bind);
                    }
                    Object.keys(art).forEach(function(key){
                        if (key === '$fusionBar' || key === '$menus') 
                            return;
                        fixArticle(art[key]);
                    })
                }
