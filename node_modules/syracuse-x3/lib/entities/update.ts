...
var sadFsq = require('../clients/sadfsq/sadfsqClient');
...
var importTool = require("@sage/syracuse-lib/src/import/jsonImport");
...
function applyUpdate(integrate) {
	return function(_, context) {
		...
		var instance = this;
		instance.updateInProgress(_, true);
		// Save updateInProgress
		instance.save(_);
		...
		var batchServers = null;
		if (integrate) {
			batchServers = endpoints.toArray(_, true)
		                            .map_(_,function(_,ep){
			                                return ep.endpoint(_).x3solution(_).batchServer(_);
										})
		                            .filter_(_,function(_,bs) {
										    return bs.status(_)=="running"
										})
		                            .filter_(_,function(_,bs,pos,self){
										   return self.findIndex(b =>b.$uuid==bs.$uuid) == pos;
									});
		    batchServers.forEach_(_, function(_,bs){

			if(tracker) {
				tracker.$diagnoses = tracker.$diagnoses || [];
				tracker.phaseDetail = locale.format(module, "stoppingBatchServer", bs.code(_));
			    tracker.$diagnoses.push({$severity: "info",
					   				 $message: locale.format(module, "stoppingBatchServer", bs.code(_))});		
			}
			bs.stopAll(_);
			if(tracker) {
			    tracker.$diagnoses = tracker.$diagnoses.concat(bs.$diagnoses); 
			} 
			if(bs.hasErrors(_)) return;
			bs.$diagnoses = [];
		   })
		}
        
		if(tracker) tracker.phaseDetail = locale.format(module, "phase" + (integrate ? "Apply" : "Test"));
		...
		var statuses = null;
		endpoints.toArray(_, true).forEach_(_, function(_, epToUpdate) {
			statuses = statuses || {error: 0,warning: 0,success: 0};
		...
		if (statuses && !Object.keys(statuses).some_(_, function(_, status) {
				if (statuses[status]) {
		...
		statuses && trackMessage(context, instance.status(_), instance.detailedStatus(_));
		instance.updateInProgress(_, false);
		instance.save(_);
		...
		if (integrate) {
			...
			if(tracker) {
				tracker.$diagnoses = tracker.$diagnoses || [];
				tracker.phaseDetail = locale.format(module, "startingBatchServer", bs.code(_));
			    tracker.$diagnoses.push({$severity: "info",
					   				 $message: locale.format(module, "startingBatchServer", bs.code(_))});		
			}
			bs.start(_);
			if(tracker) {
			    tracker.$diagnoses = tracker.$diagnoses.concat(bs.$diagnoses); 
...
function updateFolder(_, context, integrate, update, epToUpdate, patch, path, folder) {
	...
	if (legislation && (!legislations || legislations.indexOf(legislation) < 0)) {
		...
		updateChildren = false; // SAM 121158
	...
	if (store.fileExists(_)) {
		...
		var patchFiles = new unzip.Unzip(
			...
			function(filename, filecontent, headers, _) {
                ...
                var importContext;
				if (update.factoryOwner(_) && update.factoryOwner(_).toLowerCase() == "sage") 
				  importContext = {tenantId: globals.context.tenantId}
				else
                  importContext = globals.context;
				if (filename.substring(filename.lastIndexOf('.')).toLowerCase() === ".zip") {
					...
					var jsonFile = new unzip.Unzip(
						...
						function(jsonName, jsonContent, headers, _) {
							 globals.withContext( _ => {importTool.jsonImportFromJson(_, null, jsonContent.toString('utf8'), options);}, 
					               importContext)(_);							
						});
					jsonFile.unzip(_);
				} else {
					globals.withContext( _ => {importTool.jsonImportFromJson(_, null, filecontent.toString('utf8'), options);}, 
					               importContext)(_);
...
function updateEndpoint(_, context, integrate, update, epToUpdate, patches) {
	...
	patches.forEach_(_, function(_, patch) {
		if (patch.apply(_)) {
			...
			if (type !== "SYRACUSE") {
				var handler = type && patchTypes[type];
				if (handler) handler(_, context, integrate, update, epToUpdate, patch);
				else trackMessage(context, "error", locale.format(module, "patchTypeUnknown", type));
			}
		} else {
...
exports.entity = {
					...
					new unzip.Unzip(
						...
						function(filename, filecontent, headers, _) {
							try {
								...
								instance.factoryOwner(_, contents.factoryOwner);
	...
	$services: {
		...
		apply: {
			...
			$title: "Apply update",
			$isMethod: true,
			$confirm:"This operation will stop the batch server and the associated running tasks.",
			$invocationMode: "async",
			$capabilities: "abort",
			$facets: ["$details"],
			$isDisabled: function(_, instance) {
