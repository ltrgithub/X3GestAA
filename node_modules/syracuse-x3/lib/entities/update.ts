...
function copyPatch(_, context, update, endpoint, patch, copyOptions) {
	...
	var x3Files = [];
	var refFiles = [];
	if (!copyOptions.scmMode) {
		// We are copying the files into a folder that may already contain some files
		// New files will be copied while obsolete files will be removed
		try {
		...
		var patchFiles = new unzip.Unzip(
			...
			function(filename, filecontent, headers, _) {
				...
				filter: function(filename, headers, _) {
					if (copyOptions.scmMode) {
						// We are copying to a new folder (it was empty when we entered the function)
						// We want to copy everything (including catalog.json/update.json).
						nbKept++;
						return true;
					}
...
function patchSCM(_, context, integrate, update, epToUpdate, patch) {
	...
	var toFile = "PATCH/" + fsp.basename(path);
...
function _readPropertiesFromArchive(_, instance, forcedValues) {
	var uploaded = false;
			...
			function(filename, filecontent, headers, _) {
				try {
					...
					contents.patches.forEach_(_, function(_, p) {
						...
						patch.name(_, p.name);
						patch.type(_, forcedValues.patchType || p.type);
	...
	$properties: {
		...
		file: {
			...
			$uploadDone: function(_, instance) {
				instance.$diagnoses = [];
				if (!_readPropertiesFromArchive(_, instance))
					instance.$addError(locale.format(module, "invalidUpdate"));
			}
		},
		version: {
			$title: "Version",
...
exports.applyZipPatchFromSCM = function(_, context, name, description, endpointId, archiveName, archiveAbsoluteFilename) {
	var result = {};
	try {
		// Create an 'update' entity in mongoDb
		var filecontent = fs.readFileSync(archiveAbsoluteFilename);
		...
		// Write the archive in the file of the update
		var store = update.file(_);
		var writer = store.createWritableStream(_, {
			contentType: "application/x-zip-compressed",
		...
		// Associate the endpoint to the update :
		...
		// Now, extract all the informations (version, patches, ...) from the 
		// 'update.json' file existing in the archive
		if (!_readPropertiesFromArchive(_, update, {
				patchType: "SCM", // force the type of the patch to SCM
			})) {
			throw new Error(locale.format(module, "invalidUpdate"));
		}
