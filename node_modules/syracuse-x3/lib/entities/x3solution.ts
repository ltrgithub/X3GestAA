			...
			$execute: function(_, context, instance, parameters) {
				...
				if (folders.length > 0) {
					folders.forEach_(_, function(_, f) {
						tracker && tracker.$diagnoses.push({$severity: "info",
						 									$message: locale.format(module, "createEndpoint",f) });		
						var ep = instance.createEndpoint(_, f);
						if (ep) {
						   tracker && tracker.$diagnoses.push({$severity: "info",
						   									   $message: locale.format(module, "createEndpointDone", ep.description(_))});		
						   ep.importSyrDefFolder(_,tracker,folders.length);	
						} else {
						  tracker && ( tracker.$diagnoses.push(instance.$diagnoses.pop()));
						}
	...
	$functions: {
		...
		checkRuntime: function(_, context, filterFunc) {
			...
			servers.forEach_(_, function(_, s) {
				try {
					self.$diagnoses.push(require('../pool').checkServerSettings(_, {
						x3server: s,
		...
		createEndpoint: function(_, folderName) {
			...
			// check first whether there is already an endpoint with same folder name
			if (this.endpoints(_).toArray(_).some_(_, function(_, ep) {
				...
				this.$diagnoses.push({
					...
					$message: locale.format(module, "epAlreadySet", folderName)
				});
				return;
			}
			var srvEnt = adminDb.getEntity(_, "endPoint");
			var ep = srvEnt.createInstance(_, adminDb);
			// WARNING: please do not change these naming conventions - 
			// they are important for Cloud V2!
...
var propagateToBatchController = function(_, instance) {
	...
	if (serversMap) {
		...
		for (var i = 0; i < Object.keys(serversMap).length; i++) {
			...
			// if line deleted
			if (srv.old && (!srv.new || srv.new.$isDeleted)) {
				_trace && _trace("Line deleted: " + JSON.stringify(srv.old, null, 2));
				var conf = srv.new && _getBatchConf(srv.new.$uuid);
				if (conf) {
