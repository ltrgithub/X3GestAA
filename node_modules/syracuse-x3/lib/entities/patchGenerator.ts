	...
	$functions: {
		generate:function(_,context:any, parameters:any) {
			...
			try {
				...
				catalog.files = catalog.files.slice().filter_(_, function(_,file) {
					...
					if(scmFiles[patchFilename]) {
						...
						if(!file.filename.endsWith('.rpt')) {
							...
							lines.some( line => {
								if(/^2,/.test(line)) {
									...
									keep = !(file.type === "TRT" && /^[WXYZ]/.test(file.id))
											&& (["ADO","ADZ","ADF"].indexOf(file.type) < 0) // Don't deliver documentation 
											&& !(file.type === "ADP" && ["ATXTETAG","RIGHTSETAG"].indexOf(file.id) >= 0); // Exclusions
							...
							file.type = "ETA";
							const match = file.filename.match(/([A-Z]*).rpt/);
							file.id = match[1];
						}
				...
				let patchIndex = Object.keys(catalog.files.map(file => fileKey(file))
				...
				while((index = patchIndex.splice(0,25)) && index.length) {
					scmFiles['index_'+("00"+(++count)).slice (-2)+'.lst'] = index.concat('').join('\n');
				}
		...
		remoteCreation: {
			...
			$overridesReply: true,
  			$execute: function(_, context, instance, _parameters) {
				...
				if(	   !(parameters.fromSha && parameters.toSha)
					&& !(parameters.fromBranch && parameters.toBranch)) _error("Range is missing");
