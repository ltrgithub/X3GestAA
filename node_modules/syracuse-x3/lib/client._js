...
function X3Client(poolConfig) {
	...
	function _send(_, header, request, response, licenseData) {
		...
		timing.end();
		// license check
		if (licenseData && inputLength > 0) { // just measure the end of the invocation
	...
	function setProductName(_, params, x3licData) {
		...
		for (var i = 0; i < validLicense.length && !params.userProfile.productName(_); i++) {
			if (validLicense[i].productCode === x3licData.product) {
				adminHelper.setProductnameByEndpoint(validLicense[i].productTitle, params.userProfile.selectedEndpoint(_).dataset(_));
			}
		}
	}
		...
		try {
			...
			if (x3licData && x3licData.product && !params.noCheckLicense) {
				...
				if (lk) {
					...
					var diags = [];
					var license = check.getX3LicenseInfo(_, x3licData.product, x3licData.version, globals.context.session, diags);
					if (!license) {
						this.disconnect(_);
