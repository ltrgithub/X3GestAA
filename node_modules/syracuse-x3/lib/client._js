...
function X3Client(config) {
	...
	var _readTimeout = (config.x3Pool && config.x3Pool.timeout) || 600000;
	...
	function _readFromStream(_, options) {
		try {
			return _stream.read(_, options);
		} catch(e) {
			if (e.code === "ETIMEOUT") e.message = locale.format(module, "x3ReadTimeout");
			// destroy stream
			_stream.destroy();
			_stream = null;
			//
