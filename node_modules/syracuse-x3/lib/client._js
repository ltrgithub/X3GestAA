...
//			data: (len > 1) ? (encoding ? data && data.toString(encoding) : data) : null
	...
	function _send(_, header, request, response) {
		...
		if (request.url && /.*\/IMAGE\(.*\).*word_document=true/.test(request.url))
		{
			...
			handleDocx(_, request, response, head);
		}
		else
		{
			...
			do {
				...
				frame = _readFrame(_);
				// BEGIN BERLIN HACK
	/*			if (request.url && /AWRKHISSUI.*query/.test(request.url)) { 
					var body = JSON.parse(frame.data.toString("utf8"));
					body.$resources.forEach(function(res) {
						res.$title = res.$value = res.CLEDEC;
						res.$hint = res.VALCTX7
						res.$description = res.VALCTX2 + " - " + res.VALCTX3;
					})
					frame.data = new Buffer(JSON.stringify(body), "utf8");
				}*/
				// END BERLIN HACK
		
	//			_tracer && _tracer("received body frame " + frame.code + ": " + sys.inspect(frame.data));
	...
	function handleDocx(_, request, response, head) {
		var document;
		var frame;
		var urlParts = request.url.split("?");
		var params   = helpers.url.parseQueryString(urlParts[1] || "");
		
		var filename = params.filename || "";
		
		// TODO: Will not work if content is split into more than 1 frame!
		
		do {
			frame = _readFrame(_);
			
			var docUrl = urlParts[0];
			
			document = docx.handleDocument(_, frame.data, docUrl);
			frame.data = document.data;
			
			head["content-type"] = document.type;
