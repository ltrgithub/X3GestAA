...
function X3Client(poolConfig) {
	...
	function setProductName(_, params, x3licData) {
		var validLicense = licenseCheck.validLicenses();
		var localeSelected = params.userProfile.selectedLocale(_).code(_).toLowerCase();
		for (var i = 0; i < validLicense.length && !params.userProfile.productName(_); i++) {
			if (validLicense[i].productCode === x3licData.product) {
				var productName = (validLicense[i].productTitle ? (validLicense[i].productTitle[localeSelected] ? validLicense[i].productTitle[localeSelected] : validLicense[i].productTitle[Object.keys(validLicense[i].productTitle)[0]]) : '').replace(/^Sage/, '');
				adminHelper.setProductnameByEndpoint(productName, params.userProfile.selectedEndpoint(_).dataset(_));
			}
		}
	}
