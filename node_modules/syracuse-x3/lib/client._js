...
function WritableMemoryStream() {
	...
	self.toString = function(enc) {
		return chunks.map(function(chunk) { return chunk.toString(enc); }).join("");
	}
	self.end = function() {
		// ?
	}
...
function X3Client(config) {
	...
	function _jsonSend(_, params) {
		...
		var result = {
			...
			body: JSON.parse(response.toString("utf8"))
		}
...
//			console.log("nodeconf: "+require("util").inspect(nodeconfig.config));
			...
			sendParams.head.authorization = params.authToken || "auth-token";
		if(params.localePreferences)
			sendParams.body.preferences = {
