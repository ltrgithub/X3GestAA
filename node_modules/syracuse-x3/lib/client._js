...
//			data: (len > 1) ? (encoding ? data && data.toString(encoding) : data) : null
	...
	function handleSpecialX3Calls(_, head) {
		...
		function isSpecial(location) {
			...
			for (var i in specials) {
				if (location.indexOf(specials[i] + ":") === 0)
					return true;
			}
			return false;
		}
		...
		while (head.location != null && isSpecial(head.location)) {
			...
			if (typeof res.body === 'object') {
				bodyStr = util.inspect(JSON.stringify(res.body));
			} else {
	...
	function _send(_, header, request, response) {
		...
		if (request.url && /\/IMAGE\(.*\).*mime=/.test(request.url))
		{
	...
	this.connect = function(_, params) {
		...
		try {
			_stream = _client.connect(_);
			_stream.emitter.setNoDelay(true);
		} catch (e) {
	...
	function BulkReader() {
		...
		bulk.open = function(_, params) {
			...
			if (head.status === 500) {
				...
				if (_frame.$diagnoses && _frame.$diagnoses.length) throw new Error(_frame.$diagnoses[0].message);
				else throw new Error(head.message);
			} else
			// future
			_frame = _readFrame(null, "utf8");
			// 
			return bulk;
		}
...
/*			var response = _jsonSend(_, {
			...
			if (response.head.statusCode >= 400) {
				_tracer && _tracer("queryReader.open received error" + JSON.stringify(response.body));
				if (response.body.$diagnoses && response.body.$diagnoses.length) throw new Error(response.body.$diagnoses[0].message);
				else throw new Error(response.head.message);
			}
			_parts = response.body.$resources;
			var url = response.body.$links && response.body.$links.$next && response.body.$links.$next.$url;
			if(url)
				// future
				_next = _jsonSend(null, {
					head: {
						...
						"accept-language": locale.current || "en-US",
						method: "GET",
						url: response.body.$links.$next.$url
					},
					body: null
				});*/
			// 
		...
		bulk.next = function(_) {
			...
			while (!_parts.length) {
				...
				if (r.head.statusCode >= 400) {
					...
					if (r.body.$diagnoses && r.body.$diagnoses.length) throw new Error(r.body.$diagnoses[0].message);
				...
				if(!_parts.length || !url) {
					...
					// future
					url = url.replace("$baseUrl", _baseUrl);
	...
	this.createBulkReader = function(_, params) {
		try {
			var reader = (params.facet && params.facet === "$query") ? new QueryReader() : new BulkReader();
