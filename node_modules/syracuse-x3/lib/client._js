...
var uploadHelpers = require('@sage/syracuse-lib/src/upload/helpers');
...
function X3Client(poolConfig) {
	...
	function _jsonSend(_, params, funnel) {
		...
		return funnel.call(null, _, function(_) {
			...
			//
			if (respStr && respStr.$diagnoses) {
				respStr.$diagnoses.forEach(x3helpers.sanitizeDiagnosis);
			}
			var result = {
	...
	this.createSession = function(_, params) {
		...
		if (_x3.auth && (_x3.auth.method === "X3RSA")) {
			...
			poolConfig.certificate = poolConfig.certificate || (poolConfig.x3solution && poolConfig.x3solution.certificate(_));
	...
	this.disconnect = function(_, funnel) {
		...
		try {
			...
			//Check all success status codes
			//if (response.head.statusCode != 204) throw new Error("disconnect session failed: " + JSON.stringify(response));
			if (~~(response.head.statusCode / 100) != 2) throw new Error("disconnect session failed: " + JSON.stringify(response));
			...
			if (~~(response.head.statusCode / 100) != 2) throw new Error("disconnect connection failed: " + JSON.stringify(response));
