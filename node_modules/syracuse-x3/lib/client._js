...
function X3Client(config) {
	...
	this.createSession = function(_, params) {
		...
		// if user does not have any badges at all, no X3 session may be started
		var badgeString = globals.context.session.sessionInfo.badge(_);
		if (!badgeString) throw new Error(locale.format(module, "noBadges"));
