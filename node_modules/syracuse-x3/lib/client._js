...
function X3Client(poolConfig) {
	...
	this.connect = function(_, params) {
		...
		try {
			...
			var response = _jsonSend(_, {
				...
				body: {
					server: (params.server && (this.x3server.serverHost(_) !== params.server)) ? params.server : "", // MAIN server MUST NOT be set
					folder: params.folder || "SUPERV",
	...
	this.createSession = function(_, params) {
		...
		if (params.localePreferences) sendParams.body.preferences = {
			...
			firstWeekOfYear: params.localePreferences.firstWeekOfYear(_)
		};

		sendParams.body.fusionTech = params.fusionTech;

		// manage application connection parameters
		if (params.applicationConnection) {
			var appData = params.applicationConnection.data(_);
			if (appData != null) {
				// temporary comment waiting for runtime fix
				//appData = typeof appData === "string" ? JSON.parse(appData) : appData;
				sendParams.body.applicationConnectionData = appData;
			}
		}
