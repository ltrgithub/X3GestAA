	...
	function BulkReader() {
		...
		bulk.open = function(_, params) {
			...
			if (head.status === 500) {
				...
				if (_frame.$diagnoses && _frame.$diagnoses.length) throw new Error(_getDiagMessage(_frame.$diagnoses[0]));
		...
		bulk.next = function(_) {
			...
			while (!_parts.length) {
				...
				if (r.head.statusCode >= 400) {
					...
					if (r.body.$diagnoses && r.body.$diagnoses.length) throw new Error(_getDiagMessage(r.body.$diagnoses[0]));
