...
function _getSrvErrorFromResponse(mainMess, response) {
	...
	} else if (response.body && response.body.$diagnosis) {
		diagnoses = [response.body.$diagnosis];
	} else {
	...
	this.connect = function(_, params) {
		...
		if (x3licData && x3licData.product) {
			...
			if (lk) {
				...
				if (diag.length) {
					var err = new Error(locale.format("licenseError", x3licData.product, x3licData.version));
					err.$diagnoses = diag;
					throw err;
				}
				var response = _jsonSend(_, {
				...
				if (response.head.statusCode >= 400) {
					throw _getSrvErrorFromResponse(locale.format(module, "licenseError"), response);
				}
			}
		}
		//
