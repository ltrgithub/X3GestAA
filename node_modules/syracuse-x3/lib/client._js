...
function X3Client(poolConfig) {
	...
	var _client = streams.tcpClient(poolConfig.port, poolConfig.address, {
		read: {
	...
	function _send(_, header, request, response, cache) {
		...
		if (request.url && (/\/IMAGE\(.*\).*mime=/.test(request.url) || /\/BDOC\(.*\).*mime=/.test(request.url))) {
			...
			if (head.statusCode !== 304 || !cache) {
				response.writeHead(head.statusCode, head);
				var cachePart = new WritableMemoryStream();
				//var parts = [];
				//		var total = 0;
				do {
					...
					if (cache && head.statusCode === 200) cachePart.write(_, frame.data);
					//			parts.push(frame.data);
					//			total += frame.data.length;
				} while (frame.code == 2);
				_checkCode(frame.code, 3);
				if (cache) {
					cache.data = cachePart.toString("utf8");
				...
				response.write(_, cache.data, "utf8");
			}
		}
	...
	this.sendRequest = function(_, request, response, cache) {
		return _funnel(_, function(_) {
			...
			if (cache && cache.etag) head.etag = cache.etag;
			return _send(_, head, request, response, cache);
		});
	};
