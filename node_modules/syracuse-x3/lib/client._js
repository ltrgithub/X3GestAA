...
function X3Client(config) {
	...
	function _outBuffer(code, obj) {
		...
		_tracer.info() && _tracer.info("X3.client.outBuffer: sending length: " + len);
	...
	function _readFrame(_, encoding) {
		_tracer.info() && _tracer.info("reading frame from X3");
		var intro = self.read(_, 5, true);
		if (intro == null) throw new Error("error reading frame: stream closed");
		var len = _readInt(intro);
		_tracer.info() && _tracer.info("reading frame from X3: got len=" + (len - 1));
		var data = self.read(_, len - 1, true);
		_tracer.info() && _tracer.info("frame read ok");
		return {
			code: intro[4],
	...
	function _jsonSend(_, params, funnel) {
		...
		return funnel.call(null, _, function(_) {
			...
			_tracer.info() && _tracer.info("_jsonSend got response: " + respStr);
			if (respStr) try {
			...
			switch (special) {
				case "js":
					...
					try {
						...
						_tracer.error() && _tracer.error("Parse error : " + e.stack);
						_tracer.error() && _tracer.error(frame.data.toString('utf8'));
						res.body = '';
						res.header = {
							"statusCode": 500,
					...
					try {
						...
						_tracer.error() && _tracer.error("JS Error: " + e.stack);
						res.body = '';
						res.header = {
							"statusCode": 500,
							"message": e.message,
				...
				case "future":
					...
					try {
						...
						_tracer.error() && _tracer.error("Future retrieve Error: " + e.stack);
						res.body = '';
						res.header = {
							"statusCode": 500,
							"message": e.message,
	...
	function _send(_, header, request, response) {
		_tracer.info() && _tracer.info("sending header: " + JSON.stringify(header));
		...
		while (buf = request.read(_)) {
			...
			_tracer.info() && _tracer.info("sending chunk: " + (buf ? buf : "null"));
			_stream.write(_, _outBuffer(2, buf));
		}
		_tracer.info() && _tracer.info("write end frame");
		_stream.write(_, _outBuffer(3, ""));
		//
		var frame = _readFrame(_, "binary");
		_tracer.info() && _tracer.info("received header frame: " + frame.data);
		_checkCode(frame.code, 1);
		var head = JSON.parse(frame.data);
		head.statusCode = head.status; // compat stuff -- fix protocol
		_tracer.info() && _tracer.info("head.location: " + head.location);
		//
		...
		if (request.url && (/\/IMAGE\(.*\).*mime=/.test(request.url) || /\/BDOC\(.*\).*mime=/.test(request.url))) {
			...
			do {
				...
				frame = _readFrame(_);
				_tracer.info() && _tracer.info("received body frame " + frame.code + ": " + frame.data.toString("utf8"));
				//			_tracer.info() && _tracer.info("received body frame " + frame.code);
				if (head.statusCode === 500) {
					_tracer.error() && _tracer.error("ERROR 500: head=" + JSON.stringify(head) + "\nbody=" + frame.data);
		...
		_tracer.info() && _tracer.info("ending response");
		response.end();
		timing.end();
	...
	this.checkServerSettings = function(_, params) {
		_tracer.info() && _tracer.info("x3client.checkServerSettings enter");
		...
		_tracer.info() && _tracer.info("connected: pid=" + _x3.pid);
	...
	this.connect = function(_, params) {
		var self = this;
		_tracer.info() && _tracer.info("x3client.connect enter");
		var lic = check.getParsedLicense(_);
		if (lic && !lic.namedUsers) {
		...
		try {
			...
			_tracer.info() && _tracer.info("connected: pid=" + _x3.pid + ", params=" + JSON.stringify(params));
	...
	this.createSession = function(_, params) {
		_tracer.info() && _tracer.info("x3client.createSession enter");
		...
		if (_x3.auth && (_x3.auth.method === "X3RSA")) {
			...
			_tracer.info() && _tracer.info("pubkeyname " + pubkeyName);
	...
	this.sendRequest = function(_, request, response) {
		return _funnel(_, function(_) {
			...
			_tracer.info() && _tracer.info("X3 client requested url=" + request.url + "\n\theader=" + JSON.stringify(head));
	...
	function BulkReader(readerBaseUrl) {
		...
		bulk.open = function(_, params) {
			...
			//
			diagnoses = [];
			_tracer.info() && _tracer.info("opening bulk reader");
			// make request
			var x3head = {
				headers: {
					"accept": "application/json;vnd.sage=syracuse",
			...
			//
			_tracer.info() && _tracer.info("bulk reader header params: " + sys.inspect(x3head, null, 4));
			...
			if (head.status === 500) {
				...
				_tracer.info() && _tracer.info("bulkReader.open received error; frame " + JSON.stringify(_frame));
		...
		bulk.next = function(_) {
			_tracer.info() && _tracer.info("bulkReader.next enter");
			while (!_parts.length) {
				if (_done) return null;
				//
				var f = _frame(_);
				_tracer.info() && _tracer.info("bulkReader.next received body frame " + f.code + ": " + f.data);
				if (f.code === 3) {
					_done = true;
					_busy = false;
				} else
				...
				res.forEach(function(r) {
					...
					r = r.replace(/,$/, "");
					_tracer.info() && _tracer.info("bulkReader.next resource :" + r);
	...
	function QueryReader(readerBaseUrl) {
		...
		bulk.open = function(_, params) {
			...
			//
			diagnoses = [];
			_tracer.info() && _tracer.info("opening query reader; url=" + params.url);
			// make request
			_next = _jsonSend(!_, {
				head: {
					"accept": "application/json;vnd.sage=syracuse",
		...
		bulk.next = function(_) {
			_tracer.info() && _tracer.info("queryReader.next enter");
			while (!_parts.length) {
				if (_done) return null;
				//
				var r = _next(_);
				if (r.head.statusCode >= 400) {
					_tracer.info() && _tracer.info("queryReader.next received error" + JSON.stringify(r.body));
				...
				if (!_parts.length || !url) {
					...
					_tracer.info() && _tracer.info("queryReader.next future; url=" + url);
					_next = _jsonSend(!_, {
						head: {
							"accept": "application/json;vnd.sage=syracuse",
							"accept-language": locale.current || "en-US",
