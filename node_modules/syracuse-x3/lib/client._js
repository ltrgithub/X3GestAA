...
//			data: (len > 1) ? (encoding ? data && data.toString(encoding) : data) : null
	...
	function _send(_, header, request, response) {
		...
		while (head.location != null && head.location.indexOf('js:') == 0) {
			...
			var x3json = null, result = {};

			try {
				console.log("Data: "+frame.data.toString('utf8'));
			...
			try {
				if (x3json) {
					result = jsRunner.execute(_, modul, x3json);
				}
			} catch (e) {
			...
			_stream.write(_, _outBuffer(1, JSON.stringify(result.header))); 
			_stream.write(_, _outBuffer(2, ""));
			_stream.write(_, _outBuffer(3, JSON.stringify(result.body))); 
