	...
	try {
		...
		if (resp.statusCode !== 201) {
			...
			if(resp.headers["content-language"]) {
        		...
        		locale.current = resp.headers["content-language"];
        		// TODO: user profile modification ???
        	}
			tracer && tracer("fusionSession.connect session sid: " + this.sid + " created; response: " + JSON.stringify(respJson,null,"   "));
		...
		var msg=locale.format(module, "connectionFailed");
		if (ex.message && ex.message.search(/ECONNREFUSED/i)>=0){
			var url=this._baseUrl?parseUrl(this._baseUrl):null
			msg+= "\n" + locale.format(module, "bridgeNotAvail",url?url.host:"");
		}
		throw new x3helpers.Error(msg,ex, ex.statusCode);
	}
}
...
sessionProto.proxy = function(_, context, config){
	try {
		...
		throw new x3helpers.Error(locale.format(module, "proxyFailed"),ex);
    }
}
...
sessionProto.close = function(_, reuse){
	...
	try {
		...
		throw new x3helpers.Error(locale.format(module, "closeFailed"),ex, ex.statusCode);
	}
}
