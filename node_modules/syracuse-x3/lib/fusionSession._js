...
sessionProto.proxy = function(_, context, config){
	try {
        ...
        tracer && tracer("sessionProto.proxy ERROR\n" + ex.toString())
		throw "Proxy exception\n" + ex.toString();
		if (context.response) {
			context.response.writeHead(ex.httpStatus!=undefined ? ex.httpStatus : 500, null);
			context.response.write(null, x3helpers.err2ClientBody(ex,"Syracuse proxy exception"));
		}
    }
}
