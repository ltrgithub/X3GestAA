...
sessionProto.connect = function(_, context) {
	...
	return JSON.parse(resp.readAll(_).toString("utf8"));
}

sessionProto.proxy = function(_, context) {
//	console.log("session proxy context.toWalk: "+sys.inspect(context.toWalk()));
	console.log("session proxy context: "+sys.inspect(context));
...
//	var params = Object.keys(context.parameters).map(function(key) {return key + "=" + encodeURIComponent(context.parameters[key]); }).join("&");
	...
	var dis = dispatcher.create(context);
