...
var CSDataEntryNoPayload = Object; //require('syracuse-x3/lib/convergence/CSDataEntryNoPayload').CSDataEntryNoPayload;

var SDataEntryExcep = exports.SDataEntryExcep = helpers.defineClass(function(schema, t) {
	CvgReply.call(this, schema);
	this.exep = t;
...
var FusionExcep = exports.FusionExcep = helpers.defineClass(function(info, reason, s, e) {
		...
		if (e instanceof CXSDataException) {
			...
			if (se.listDiagnosis != null) {
				for (var d in se.listDiagnosis) {
					this.diag.addDiagnosis(se.listDiagnosis[d]);
				}
			}
		} else {
	...
	getMessage: function() {
		...
		if (s.length === 0) {
			s += FusionClientInfo.NOCATCH.msgDef;
		}
		return s;
	},
	toJSon: function() {
...
FusionExcep.err2Diagnoses = function(t, cause) {
	...
	var cur = t;
	//	while (cur != null) {
	var diag = cur instanceof CvgException ? cur.diagnose : null;
	if (diag == null) {
		diag = new CDiagnosis();
...
FusionExcep.excep2IContent = function(req, info, t) {
	if (t == null) {
		throw new Error("Unexpected null exception");
	}
	if (req == null || req.adapter == null || req.adapter.schema == null) {
		throw new Error("Bad IRequest - No schema");
	}
	...
	try {
		console.log("FUSION EXCEPTION\n" + JSON.stringify(x.toJSON()));
	} catch (e) {
		throw new Error("excep2IContent error", e);
	}
	return x;
}

FusionExcep.throwExcep = function(reason, i, t) {
