...
var CvgBlobSvc = exports.CvgBlobSvc = helpers.defineClass(function(_, client) {
	...
	getBlob: function(uuid) {
		...
		var field = screen && screen[uuid.substring(1,uuid.length)]
		var binary = screen && field && field.binary;
	...
	processRequest: function(_, context) {
			...
			try{
				...
				case 'get':
					...
					var blob = this.blobs[id[0]] && this.blobs[id[0]][id.substring(1,id.length)];
