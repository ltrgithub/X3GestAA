...
var CvgFormatSvc = exports.CvgBlobSvc = helpers.defineClass(function(_, client) {
	...
	processRequest: function(_, context) {
			...
			if (!val) {
				result = "";
			} else if (fmt.format == null || fmt.format.length < 2) {
				result = val;
			} else {
				try{
					switch(act) {
					...
					case 'toEdit':
						...
						// »
						result = this.client.formater.formatEdit(fmt, val, typ, false);
						break;
					case 'rawToEdit':
						// a partir d’une valeur au format interne x3 (YYYYMMDD),
						// renvoyer la valeur mise en forme selon le format « Edit »
						result = this.client.formater.formatEdit(fmt, val, typ, true);
						break;
					case 'rawToDisp':
						// a partir d’une valeur au format interne x3 (YYYYMMDD),
						// renvoyer la valeur mise en forme selon le format «
	...
	formatDisp: function(val, fmt, raw) {
		...
		if (!raw) {
			...
			return this.client.formater.formatEditToDisplay(fmt, val);
		} else {
	...
	formatRaw: function(val, fmt) {
		try {
			...
			// valeur X3
			// Sinon erreur
			return this.client.formater.unformatField(fmt, val);
		} catch (e) {
		}
