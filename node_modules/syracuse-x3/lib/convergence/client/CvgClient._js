...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, stream, adxInfos, aConfig, aType) {
	...
	this.services = CvgServices.create(_);
}, null, {
	newTrackLists: function(keepPrevious) {
		return {
			SESSION: [],
			ACT: [],
			TARGETACT: [],
			...
			ENTITY: [],
			FUNC: [],
			RPC: (this.trackLists && this.trackLists.RPC) || [],
			WINDOW: [],
			DIAGNOSE: [],
		};
	},
	setInReuseMode: function(bool){
		if (bool) {
			this.isReuse = true;
			this.timeToReuse = (new Date()).getTime();
			console.log("set timeout");	
			var timeoutReuse = nodeconfig.config.x3fusion.reuseTimeout;
			console.log("timeout reuse "+timeoutReuse);
			this.reuseTimeoutId = setTimeout(function(cvgCli){
			// get current time and check timeout of reuse
				console.log("reuse "+ cvgCli.isReuse);	
				if (cvgCli.isReuse) {// only if it's reuse 
					// reuse session need to be destroy
					cvgCli.sessionConv.destroyReuseClient(cvgCli);
				}
			},timeoutReuse,this);
		}else{
			this.isReuse = false;
	...
	processAction: function(_, body) {
		...
		if (!resu) resu = new CvgResu(CvgResuTdo["NULL"]);
...
/*
	...
	track: function(obj) {
		...
		if (obj.getObservableId) {
			...
			var old = list[key];
			if (old != null){
				console.error("Remove tracked object: "+obj.jsonType);
				list[old] = undefined;
			}
	...
	getResponse: function() {
		...
		if (lists.RPC.length !== 0 && lists.RPC[0].reply){

			sap.rpc = lists.RPC.pop();
		}
