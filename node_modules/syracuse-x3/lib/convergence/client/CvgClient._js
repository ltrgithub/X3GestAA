...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	processSuperAction: function(_, body) {
		...
		for (var i = 0; i < superAction.actions.length && !stop; i++) {
			...
			do {
				...
				var t1 = target && target.type === 'ist' && target.ist;
				if (!t1) stop = true;
				if (action.setTarget) {
	...
	processRequest: function(_, context, bodyStr) {
		var timing = perfmonEnabled && perfmon.start(module, 'cvgClient.processRequest', act);
		...
		var httptiming = perfmonEnabled && perfmon.start(module, 'cvgClient.HttpRequest');
		...
		if (perfmonEnabled) {
			...
			if (perfmonDetail) {
				this.timing.action[act].readHttpRequest = this.timing.action[act].readHttpRequest || [];

				httptiming.getTiming() && this.timing.action[act].readHttpRequest.push({
					duration: (httptiming.getTiming().duration) || 0
				});
			}
		}
	...
	traverseSettable: function(ist, aModifier) {
        ...
        // normally elsif is not necessary
        }else */
	...
	generateResponse: function(_, connect, isTrack) {
		var timing = perfmon.start(module, "cvgClient.generateResponse");
		...
		if (localConfig.perfmon) {
			...
			timing.end();
			response.timing[].generateResponse = {
				duration: timing.getTiming().duration
			};
		}
