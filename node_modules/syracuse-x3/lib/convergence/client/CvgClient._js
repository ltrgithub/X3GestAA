...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	processAsyncAction: function(_, body, connect, mergeMode, noResp) {
		...
		try {
			...
			RazRequest = ((new Date()).getTime() - RazRequest) / 1000;
			...
			if (body.fld) {
				if (body.fld.v != null) {
					...
					timeResuFromAction = ((new Date().getTime) - timeResuFromAction) / 1000;
				}
			...
			listenSendingArg = ((new Date()).getTime() - listenSendingArg) / 1000;
	...
	razModification: function(actionCode) {
		...
		var entity = this._trackLists.ENTITY;
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (this.traceTiming && this.perfTimer) {
			...
			response.srvop.timing = {
				...
				toJSON: ((new Date()).getTime() - generateTime) / 1000,
				timeNode: {
