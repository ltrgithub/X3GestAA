...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder,context) {
	...
	processSuperAction: function(_, body) {		
		...
		for (var i = 0; i < superAction.actions.length ; i++) {
			...
			do{
				...
				resp = this.processAction(_, newBody,false,i !== 0);
	...
	loadWindowDescription: function(_, ist, winId, funcId, stamp) {
		...
		function getWindowXml(_, winId) {
			//if (true) return require('streamline-fs').readFile(__dirname + "/../data/" + winId + ".xml", "utf8", _);
			...
			var proxyConf = httpHelper.getDefaultProxyConf(_);
			if (proxyConf)
				opt.proxy = proxyConf;

			var request = streams.proxyAuthenticate(_).httpRequest(opt);
