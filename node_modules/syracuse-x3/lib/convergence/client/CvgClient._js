...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	this.askKill = false;
	this.killPluginTimoeut = localConfig.plugin && localConfig.plugin.killTimeoutOnCreate;
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				case "CLIENT_PLUGIN_REPLY":
					...
					this._clientReplyPlugin(_, "ilog", body);
					automata.process(_, this, this.activeRequest, {
						title: "special",
						detail: "CLIENT_PLUGIN_REPLY"
					});
					break;
				default: // it's the plugin that catch the request and all node
					var mustDelete = this.cvgPlugin && this.cvgPlugin.ND_IHM && this.cvgPlugin.ND_IHM["ilog"].processRequest(_, specialAction.id, body);
					if (mustDelete) { // ok acknoledgement delete , must remove the plugin controller
						delete this.cvgPlugin.ND_IHM["ilog"];
					}
			}
		} else {
	...
	_clientReplyPlugin: function(_, klass, body) {
		this.cvgPlugin.ND_IHM[klass]._clientReplyPlugin(_, this._buildPluginResponse(_, body && body.param && body.param.plugin));
	},
	_buildPluginResponse: function(_, paramPlug) {

		if (paramPlug && paramPlug.return &&paramPlug.return .property) {
			var ret = {};
			ret[paramPlug.return .property.name] = paramPlug.return .property.value;
		}
		return ret;
	},
