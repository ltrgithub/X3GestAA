...
var serializers = {
	...
	D: function(val) {
		traceProtocol && traceProtocol("Serialize.D: "+val);
		if (!val) return;
	...
	S: function(val) {
		traceProtocol && traceProtocol("Serialize.S: "+val);
	...
	L: function(val) {
		traceProtocol && traceProtocol("Serialize.L: "+val);
		if (typeof val !== 'number') throw new Error("action parameter type mismatch, expected number, got " + typeof val);
...
var CvgClient = exports.CvgClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	this.newActiveRequest(new CvgReply(this, CONNECT_ACTION), null,{"reqNum":0} );
	...
	processAction: function(_, body) {
		...
		this.newActiveRequest(reply, null, {"ctxNDSLB": ctx, "reqNum": body.tech.id ? body.tech.id : 0});
			...
			if (body.fld.v){
				if (!body.fld.ist) {
					...
					if (entity && entity instanceof CvgVariable) {
						...
						resu = CvgResu.fromAction(entity.dataType, entity.unFormatData(body.fld.v));
	...
	sendAction: function(_, resu, extras, params) {
		...
		traceAction && traceAction("SEND ACTION: resu=" + resu + ", extras=[" + extras + "], params=[" + params + "]");
...
/*
	...
	processSuperAction: function(_, body) {
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			// Delete target if not needed
			if (!action.setTarget)
				delete newBody.param.target; 
			
			// Retrieve sudo params
			if (sudo[i])
				newBody.param.std = [sudo[i]];
			
			// Use other action sudo params
			if (action.propagate)
	...
	configFormater: function() {
		 ...
		 */
		this.formater.dateOrder = this.config.params.IDate;
		this.formater.dateSep = this.config.params.SDate;
		this.formater.pivotDate = this.config.params.uiParamPIVOTDATE;
		this.formater.decimalSep = this.config.params.SDecimal;
		this.formater.thousandSep = this.config.params.SThousand;
		// pour optilmiser le flux xml vers le browser: suprime les espaces de cadrage a droite pour les valeur alpha.
		this.formater.vOn = this.config.getFormaterVon();
		this.formater.zOn = true;
	},
	...
	executeCallUiOpenFile: function(aListOfProperties) {
		...
		var properties = Object.keys(aListOfProperties);
		var wMax = properties.length;
		if (wMax > 2) {
			var wLocalDir = aListOfProperties.UILocalDir;
			var wLocalFile = aListOfProperties.UILocalFile;
			 ...
			 */
			if (wLocalDir === "HTTP") {
				wUrl += this.translateLocalFileUrl(wLocalFile, aListOfProperties);
			}
			/*
			...
			var wCallUiOpenFile = {url: wUrl.toString()}
			this.activeRequest.reply.addAskOpenFile(wCallUiOpenFile);

			for (var wI in properties) {
				var wPropertyId = properties[wI];
				var wProperty = aListOfProperties[wPropertyId];
				
				if (wPropertyId !== "UIAction" && wPropertyId !== "UILocalDir" && wPropertyId !== "UILocalFile") {
					wCallUiOpenFile[wPropertyId.toLowerCase()] = wProperty.toString();
				}
			}
		}
		return {OpenFile: "1"};
	},
	executeCallUiGetFile: function(always, aListOfProperties) {
		return {GetFile: "1"};
	},
	executeCallUiPutFile: function(always, aListOfProperties) {
		return {PutFile: "1"};
	},
	...
	/* Wait to see use case to uncomment it - Not sure about implementation !!!!
	listenNodeND_INT: function(_, reader, node) {
		try {
			this.sendAction(_, new CvgResu(CvgResu.TDO_SHORT, 0), [], []);
			//aSegmentNode.setAutoReplied();
		} catch (e) {
			this.connected = false;
			var wErrMess = this.getMessage('CANT_SEND_AUTOMATIC_NODE_REPLY', "ND_INT");
			// 16w_104 - refonte interface de CException
			throw new CvgException(this, e, wErrMess);
		}
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		try {
			...
			this.sendAction(_, new CvgResu(CvgResu.TDO_NULL), [], wActionArgs);
		} catch (e) {
			this.connected = false;
			throw e;
		}
...
/* TODO overload
	...
	translateLocalFileUrl: function(aUrlFile, aListOfProperties) {
		 ...
		 */
		if (!aListOfProperties["UIAdxPubSubFile"] != null) {
			wSB += "http://";
			wSB += aUrlFile;
		}
		/*
		 ...
		 */
			...
			if (wMalformedUrl) {
				throw new Error("Malformed url, not an AdxPubSubFile ! [" + aUrlFile + "]");
			}
			wSB += aUrlFile.substring(wPos);
		}
		return wSB;
	},
