...
exports.CvgClient = helpers.defineClass(function() {
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				case "CLIENT_PLUGIN_REPLY":
					...
					this._clientReplyPlugin(_, context, "ilog-default", body);
	...
	_clientReplyPlugin: function(_, context, defaultId, body) {
		if (this.cvgPlugin.ND_IHM) {
			...
			if (this.cvgPlugin.ND_IHM[id]) {
				...
				this.cvgPlugin.ND_IHM[id].clientReplyPlugin(_, context, body);
			}
		}
	},
		...
		try {
			...
			if (this.isProcessingRequest) { // process already oin progrss
				...
				if (act >= 1024) {
					...
					this.processSpecialAction(_, context, body);
				}
			}
			...
			if (self.clientConfig.modePLAY) {
				...
				try {
					...
					if (+e.STATUS_CODE === 404) { // not found
						...
						var orm = self.endpoint.getOrm(_);
						var tablanQuery = orm.getEntity(_, "TABLAN", "$query");
						var tablan = orm.fetchInstances(_, tablanQuery, {
							sdataWhere: "LANISO ne '' and LANCON eq true"
						});
						var isos = [];
						if (!tablan.some_(_, function(_, l) {
							var iso = l.LANISO(_);
							if (iso === language) return true;
							isos.push(iso);
						})) {
							// prepare error so that _doAsync method of cvgSession is convinced to transmit a nice error message
							var error = new Error(locale.format(module, "languageNotInFolder", isos.sort().join(" ")));
							error.diagnose = {
								toJSON: function() {
									return {
										$diagnoses: [{
	...
	newILogPlugin: function(_) {
		...
		var plug = this.cvgPlugin.ND_IHM["ilog-default"] = new CvgPlugin(_, this, "ilog-default", localConfig.plugin && localConfig.plugin.killTimeoutOnCreate || 20000);
		return plug;
	},
