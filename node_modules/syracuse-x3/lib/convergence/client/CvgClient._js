...
exports.CvgClient = helpers.defineClass(function(protocolVersion) {
	...
	isSameContext: function(_, config, context) {
		...
		var b = this.protocolVersion === config.protocolVersion && this.clientConfig.isSameContext(config, tracerSession);
		if (b) {
	...
	processSuperAction: function(_, body) {
		...
		for (i = 0; i < superAction.actions.length && !stop; i++) {
			...
			do {
				...
				if (!this.x3writer.descriptors.actions[action.name]) {
					throw new Error("Bad sub-action: " + action.name);
				}
				processNext = true;
				newBody.act = this.x3writer.descriptors.actions[action.name];
				if (nextFld) {
				...
				if (action.setTarget) {
					...
					processNext = superAction.processNextAction(this.x3writer.descriptors.actions[action.name], t0, t1, action.checkNumLine, self);
				}
