...
var perfmonEnabled = x3helpers.config("perfmon", localConfig);
...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	_calculTiming: function(timing, key) {
		...
		if (Array.isArray(timingk)) {
			...
			var sumX3Node = 0;
			var sumX3Autoack = 0;
			var sumX3SendAction = 0;
			var sumCvg = 0;
			timingk.forEach(function(item) {
				if (item.sendAction) {
					sumX3SendAction += item.sendAction;
				} else if (item.x3read) {
					sumX3Read += item.x3read;
				} else {
					sumCvg += item.cvg;
					sumX3Node += item.x3node;
					sumX3Autoack += item.x3autoack;
				}
			});
			timing['Sum_' + key] = {
				x3: sumX3Read + sumX3Node + sumX3Autoack + sumX3SendAction,
				cvg: sumCvg
			};
