...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, stream, adxInfos, aConfig) {
	...
	isSameContext: function(context){// check context for reuse session
		this.clientConfig.addPrefsIfMissing(context.syraPreferences);
		if (!context.user  || context.user !== this.clientConfig.user) {
			traceSessions && traceSessions("Username is not the same : ["+context.user+"] !== ["+this.clientConfig.user+"]");
			return false;
		}
		if (!context.adxHttpPub || context.adxHttpPub !== this.adxInfos.adxHttpPub) {
			traceSessions && traceSessions("Adx HTTP publication folder is not the same : ["+context.adxHttpPub+"] !== ["+this.clientConfig.adxHttpPub+"]");
			return false;
		}
		if (!context.server || context.server !== this.clientConfig.server) {
			traceSessions && traceSessions("Server host is not the same : ["+context.server+"] !== ["+this.clientConfig.server+"]");
			return false;
		}
		if (!context.port || context.port !== this.clientConfig.port) {
			traceSessions && traceSessions("Server port is not the same : ["+context.port+"] !== ["+this.clientConfig.port+"]");
			return false;
		}
		if (context.codeLangIso !== this.clientConfig.codeLangIso) {
			traceSessions && traceSessions("Code lang ISO is not the same : ["+context.codeLangIso+"] !== ["+this.clientConfig.codeLangIso+"]");
			return false;
		}
		for (var i in context.syraPreferences) {
			if (context.syraPreferences[i] !== this.clientConfig.syraPreferences[i]) {
				traceSessions && traceSessions("Preference: ["+i+"] is not the same : ["+context.syraPreferences[i]+"] !== ["+this.clientConfig.syraPreferences[i]+"]");
				return false;
			}
		}
		return true;
	...
	processAction: function(_, body) {
		...
		try {
			...
			var connectOrReuse = (body.act === CvgAction.CONNECT_ACTION || (body.act === 1077 && (body.param.call && (body.param.call.json && body.param.call.json.spgm === "GESUSER:CONNECT_FONCTION") || (body.param.call.xml && body.param.call.xml.indexOf("GESUSER:CONNECT_FONCTION")!=-1))))? true: false; 
					...
					if (body.fld.ist) {
						...
						if (entity && entity instanceof CvgVariable) {
							...
							try {
								...
								var val = entity.unFormatData(value);
								if (val !== undefined) {
	...
	processSpecialAction: function(_, body) {
		...
		if (specialAction.proxy === "srvImp") {
			switch(action.id) {
				case "MORE_PRINTER":
					var printerName = body.param && body.param.selpr && param.selpr.printerName;
					if (!printerName)
						throw new CvgException(this.client, null, "printerName is required.", null, "srvImpSvc");
					
					var printClient = new PrintClient(_, hostname, port);
					var infos = printClient.getAdvancedParameters(_, printerName);
					new CvgImpMore(this.client, infos);
					break;
				case "CANCEL_IMP":
					...
					break;
				case "VALID_IMP":
					console.log("Param: "+JSON.stringify(body.param,null,2));
					var printerName = body.param && body.param.selpr && param.selpr.printerName;
					if (!printerName)
						throw new CvgException(this.client, null, "printerName is required.", null, "srvImpSvc");
					
					var printClient = new PrintClient(_, hostname, port);
					var extra = printClient.getExtraParameters(_, printerName);
					...
					break;
				default:
					throw new Error("Unknow SrvImp action [" + action.id + "]");
			}
			return this.getResponse();
		}
		else {
	...
	processRequest: function(_, body) {
		...
		else if (body.act >= 768)return this.processSuperAction(_, body);
		else return this.processSpecialAction(_, body);
	},
	getMessage: function() {
...
//	},
	...
	configFormater: function() {
		...
		// console.error("configFormater");
		// console.error(this.formater.settings);
	},
	...
	executeAllSessionSettings: function(values) {
		...
		if (this.appSrvSessionSettings.DefaultDatePivot != null){
			this.formater.settings.datePivot = parseInt(this.appSrvSessionSettings.DefaultDatePivot,10);
		}
		return {
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		if (this.adxIstsOfPickedLines.length > 0) {
			...
			if (wLeftListLine != null) {
				...
				var wMess = this.getMessage('ERROR_CANT_GET_PICKED_LINE_KEY', wArgIst.idx.toString());
		...
		x3writer.sendAction(_, this, new CvgResu(CvgResuTdo["NULL"], null), [], wActionArgs);
	},
...
//				};
		...
		if (this.activeRequest.sendContext.connectAction) {
			...
			response.srvop.sessionInfo.runtime.folder = this.adxInfos.adxFolder;
		}
