...
exports.CvgClient = helpers.defineClass(function() {
	...
	processSuperAction: function(_, body) {
		...
		for (i = 0; i < superAction.actions.length && !stop; i++) {
			...
			do {
				...
				if (!this.x3writer.actions.actions[action.name]) {
					throw new Error("Bad sub-action: " + action.name);
				}
				processNext = true;
				newBody.act = this.x3writer.actions.actions[action.name];
				if (nextFld) {
				...
				if (std[j]) {
					...
					if (action.propagate) {
						newBody.param.std.push(this.x3writer.actions.actions[action.propagate]);
					}
				}
				...
				if (action.setTarget) {
					...
					processNext = superAction.processNextAction(this.x3writer.actions.actions[action.name], t0, t1, action.checkNumLine, self);
				}
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		if (this._pickedLines.length > 0) {
			...
			actArgs = [this.x3writer.actions.actions.SRV_ABANDON];
		}
