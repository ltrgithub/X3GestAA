...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	processRequest: function(_, context, bodyStr) {
		...
		if (this.isProcessingRequest) { // process already oin progrss
			console.log("a request is already processing");
		} else {
	...
	deconnectClient: function(_, params, reason) {
		...
		try {
			...
			this.askKill = true;
			// TODO replace by alog
			console.log("ask Kill client uuid=[" + this.uuid + "] reasons=" + reason + " panic=" + params.panic);
			...
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					...
					var self = this;
					console.log("wait funnel action");

					this.funnelAction(_, function(_) {
						while (self.activeRequest && self.activeRequest.workingCtx.pendingBlockingNode) {
							console.log("action for blocking node  " + JSON.stringify(self.activeRequest.workingCtx.pendingBlockingNode, null, 2));
						...
						console.log("processAction 1075");
			...
			console.log("complete kill client uuid=[" + this.uuid + "]");
