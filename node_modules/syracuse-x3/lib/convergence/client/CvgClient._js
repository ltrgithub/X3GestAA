...
var disconnectLoop = x3helpers.config("disconnectLoopDetect", localConfig);
...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	this.newActiveRequest(CONNECT_ACTION, new CvgReply(this, CONNECT_ACTION), {
		"reqNum": 0
	});
	...
	this.tracer = new Tracer(this._cvgSession.sid, "Classic Server", "Cvg");
	this.tracer.enabled = x3helpers.config("tracesEnabled", localConfig);
}, null, {
	...
	read: function(_, len, withTimeout) {
		try {
			var x3TimeRead = this.traceTiming && perfmon.start(module, "read", this.currentAction);
			var data = this.stream.read(_, len, withTimeout);
			if (x3TimeRead && this.activeRequest) {
				x3TimeRead.end();
				var t = x3TimeRead.getTiming();
				this.activeRequest.rtsTimer.read += (t && t.duration) || 0;
			}
			return data;
		} catch (e) {
			if (e.code === "ETIMEOUT") {
			...
			this.stream = null;
			//
			this.tracer.enabled && this.tracer.trace(_, e.stack, traceLog);
			throw e;
		}
	},
	write: function(_, data) {
		var x3TimeWrite = this.traceTiming && perfmon.start(module, "write", this.currentAction);
		this.stream.write(_, data);
		if (x3TimeWrite && this.activeRequest) {
			x3TimeWrite.end();
			var t = x3TimeWrite.getTiming();
			this.activeRequest.rtsTimer.write += (t && t.duration) || 0;
		}
	},
	...
	setInReuseMode: function(_, yes) {
		this.tracer.enabled && this.tracer.trace(_, "session", "setInReuseMode + " + yes, traceSessions);
		...
		if (yes) {
			...
			this.tracer.enabled && this.tracer.trace(_, "session", "set timeout", traceSessions);
			var timeoutReuse = this.clientConfig.getParam("timeoutReuse");
			var self = this;
			this.tracer.enabled && this.tracer.trace(_, "session", "timeout reuse " + timeoutReuse, traceSessions);
			this.reuseTimeoutId = flows.setTimeout(function(_) {
				// get current time and check timeout of reuse
				self.tracer.enabled && self.tracer.trace(_, "session", "setTimeout - isReuse:" + self.isReuse + " - id:" + self.uuid, traceSessions);
			...
			this.isReuse = false;
			this.timeToReuse = -1;
			this.tracer.enabled && this.tracer.trace(_, "session", "clear timeout", traceSessions);
	...
	setParentFolder: function(_, parentFolderName) {
		...
		this.clientConfig._params.x3Host = x3server.serverHost(_);
		this.clientConfig._params.x3Port = x3server.serverPort(_);
		...
		this.clientConfig._params.x3Solution = ep.x3SolutionName(_);
		this.clientConfig._params.appHost = x3server.serverName(_);
	},
	...
	connectStart: function(_, context) {
		...
		this.tracer.enabled && this.tracer.trace(_, "session", "START CONNECTING OK:\n" + JSON.stringify(this.response, null, 2), traceSessions);
	...
	connectStop: function(_) {
		this.tracer.enabled && this.tracer.trace(_, "action", "STOP CONNECTING OK", traceAction);
		this.connecting = false;
	},
	...
	processAsyncAction: function(_, body, connect, mergeMode, noResp) {
		...
		var timeAnaReq = (new Date()).getTime();
		var RazRequest = 0;
		try {
			var pendingNode;
			self.tracer.enabled && self.tracer.trace(_, "action", "************************************", traceAction);
			self.tracer.enabled && self.tracer.trace(_, "action", "Begin Process Action:\n" + JSON.stringify(body, null, 2), traceAction);
			if (self.recorder) {
			...
			RazRequest = ((new Date()).getTime() - RazRequest);
			var timeResuFromAction = 0;
			var resu, extras = [];
			if (body.fld) {
				if (body.fld.v != null) {
					var entity = null;
					if (body.fld.ist) {
						...
						self.tracer.enabled && self.tracer.trace(_, "action", "ACTION IST=" + JSON.stringify(ist, null, 2), traceAction);
					...
					timeResuFromAction = (new Date()).getTime();
					...
					timeResuFromAction = ((new Date().getTime) - timeResuFromAction);
				}
			...
			self.tracer.enabled && self.tracer.trace(_, "protocol", "Send true value: " + resu.toString(), traceProtocol);
			var listenSendingArg = (new Date()).getTime();
			self.activeRequest.listenSendingArgs(body, self, params);
			listenSendingArg = ((new Date()).getTime() - listenSendingArg);

			//self.activeRequest.buildIstDatasOfAction(params[0]);
			...
			self.tracer.enabled && self.tracer.trace(_, "action", "************************************", traceAction);
			self.tracer.enabled && self.tracer.trace(_, "action", "End Process Action:\n" + JSON.stringify(self.response, null, 2), traceAction);
			//return resp;

			this.timeAnalysisReq = {
				RazRequest: RazRequest,
				listenSendingArg: listenSendingArg,
				timeResuFromAction: timeResuFromAction,
				totalBeforeSend: ((new Date()).getTime() - timeAnaReq),
			};
		} catch (e) {
			self.tracer.enabled && self.tracer.trace(_, "processAction error\n" + (e.stack || e), traceLog);
	...
	processSuperAction: function(_, body) {
		var self = this;
		this.tracer.enabled && this.tracer.trace(_, "action", "===================================", traceAction);
		this.tracer.enabled && this.tracer.trace(_, "action", "Process Super Action:\n" + JSON.stringify(body, null, 2), traceAction);
		...
		for (i = 0; i < superAction.actions.length && !stop; i++) {
			...
			do {
				...
				if (!action.setTarget) {
					this.tracer.enabled && this.tracer.trace(_, "action", "DELETE TARGET !!!", traceAction);
	...
	processSpecialAction: function(_, body) {
		this.tracer.enabled && this.tracer.trace(_, "action", "===================================", traceAction);
		this.tracer.enabled && this.tracer.trace(_, "action", "Process Special Action:\n" + JSON.stringify(body, null, 2), traceAction);
		...
		if (specialAction.proxy === "srvImp") {
			...
			this.newActiveRequest(body.act, reply, {
	...
	closeClient: function(_, e, reason) {
		...
		this.tracer.enabled && this.tracer.trace(_, "CloseClient Convergence Client\n\tReason[" + reason + "]\n\tAlreadyClosed[" + !(this.connClient && this.connClient.hasSocket()) + "] - Exception[" + mesg + "]", traceLog);
	...
	deconnectClient: function(_, params, reason) {
		var pendingNode;
		this.stopRecord();
		try {
			...
			this.tracer.enabled && this.tracer.trace(_, "Deconnect Convergence client - isAlive" + this.isAlive() + "\n\tReason[" + (reason || "none") + "]\n\tAsk X3 server to close ", traceLog);
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					this.closeSocket();
				} else {
					...
					this.funnelAction(_, function(_) {
						var node = {};
						var loopDetection = disconnectLoop || 10;
			...
			this.activeRequest = null;
			this.tracer && this.tracer.close(_);
	...
	razModification: function(actionCode) {
		this.callUiOpenFile = null;
		//this._stackedWin.razModifications();
		for (var i = 0; i < this._stackedWin.length; i++) {
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (this.traceTiming && this.perfTimer) {
			...
			reqTime = (reqTime && reqTime.duration) || 0;
			var rtsTime = (this.activeRequest.rtsTimer.read + this.activeRequest.rtsTimer.write);
			response.srvop.timing = {
				rts: Math.round(rtsTime * 1000),
				ws: Math.round((reqTime - rtsTime) * 1000),
				toJSON: ((new Date()).getTime() - generateTime),
				timeNode: {
					detail: this.timeNode,
					total: this.totalNode
				},
				timeAnalysisRequest: this.timeAnalysisReq
			};
		}
		if (!isTrack) {
			//console.log("time node ", this.timeNode);
			this.timeNode = {};
			this.timeAnalysisReq = 0;
			this.totalNode = 0;
		}
