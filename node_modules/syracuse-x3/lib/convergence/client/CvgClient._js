...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder,context) {
	...
	execOpenFunction: function(_,context){
		...
		var data = this.processAction(_,ctx,request, true);
		return {
			head : {
	...
	processSuperAction: function(_,context, body) {		
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			resp = this.processAction(_,context, newBody,false,i !== 0);
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch(specialAction.id) {
				case "PRINTER_SEL_OPTS":
					...
					printClient = PrintClient.createClient(_, srvImp.hostname, srvImp.port);
