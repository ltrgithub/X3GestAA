...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	this.listBadge = [];
	var self = this;
	context.httpSession.getUserProfile(_).selectedRole(_).badges(_).toArray(_).forEach_(_, function(_, item) {
	...
	this.cvgWinStubPath = "/../data/cvgWins/";
	this.askKill = false;
	...
	readOld: function(_, len, withTimeout) {
	...
	write: function(_, data) {
		return this.stream.write(_, data);
	},
	...
	isSameContext: function(_, config, context) {
		...
		var b = this.clientConfig.isSameContext(config, traceSessions);
		if (b) {
	...
	loadWindowDescription: function(_, ist, winId, funcId, stamp) {
		...
		if (this.clientConfig.modePLAY) {
			...
			try {
				...
				var filePath = __dirname + this.cvgWinStubPath + winId + ".json";
				...
				if (!file) {
					...
					jsonDescr = cvgCacheManager.getCacheManager().getResource(_, url, null, {
						timestamp: stamp,
						ignoreProxy: !this.clientConfig._params.useProxy,
						xmlToJson: true
					});
				} else {
					jsonDescr = JSON.parse(file);
				}
				descr = jsonDescr.WIND;
				if (url && descr.$.TIM) {
					cvgCacheManager.getCacheManager().cache[url].timestamp = descr.$.TIM;
				}
			} catch (e) {
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (this.traceTiming) {
			var perfEnd = Date.now();
			var reqTime = perfEnd - this.perfTimer;
