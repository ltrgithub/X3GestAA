...
var serializers = {
	...
	D: function(val) {// table of cvgresu
		console.log("serializer.D : "+val);
	...
	T: function(val){ // table of short
		console.log("serializer.T : "+val);
		if (!Array.isArray(val)) throw new Error("action parameter type mismatch, expected array, got " + typeof val);
		var countBuf = new Buffer([val.length]);
		var valsBuf = Buffer.concat(val.map(function(v) {
...
function serializeActionParams(act, params) {
	...
	function serialize(p,d){
		return Buffer.concat(p.slice(0,d.length).map(function(param, i) {
			console.log("serializer "+d[i]+" param "+param);
	...
	if (paramSplited.length > 1) {
		...
		console.log("extends detected "+JSON.stringify(paramSplited,null,2));
		// serialized extends 
		...
		for (var i=1;i<paramSplited.length;i++) {
			...
			for (var j in paramSplited[i]) {
				console.log("desc "+paramSplited[i][j]);
				console.log("offset "+offset);
				console.log("params "+JSON.stringify(params,null,2));

				var b = serialize(params.slice(offset++,offset),paramSplited[i][j]);
				console.log("b "+b.toString('hex'));
				buffExt ? buffExt = Buffer.concat([buffExt,b]): buffExt = b; // concat element of extends
				console.log("bufExt "+buffExt.toString('hex'));
			}
			...
			console.log("allbuf "+allBuffExt.toString('hex'));
		}
		// concat std buf with all extends 
		allBuffExt= Buffer.concat([buf,allBuffExt]);
		console.log("finish extends "+allBuffExt.toString('hex'));
		return allBuffExt;
	}else{
		return buf;
	}
...
var CvgClient = exports.CvgClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	execOpenFunction: function(_,context){
		console.log("openNewFunction");
		var xml  = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><spgm name=\"GESUSER:CONNECT_FONCTION\">";
	 	xml += "<param dim=\"1\" size=\""+context.length+"\" nb=\"1\" typ=\"STRU\">";
		xml += "<resu>"+context+"</resu>";
		xml += "</param>";
		xml += "<param dim=\"1\" size=\"1\" nb=\"0\" typ=\"STRU\"/>";
		xml += "</spgm>";
		...
		return this.processAction(_,req);
	},
	start: function(_) {
		this.connected = true;
	...
	processAction: function(_, body) {
		...
		this.newActiveRequest(reply, null, {"ctxNDSLB": ctx, "reqNum": body.tech && body.tech.id ? body.tech.id : 0});
		...
		console.log("param before sendAction "+JSON.stringify(params,null,2));
