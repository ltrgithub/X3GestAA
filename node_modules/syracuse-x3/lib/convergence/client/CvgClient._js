...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				...
				case "PRINTER_SEL_CANCEL":
					...
					delete this.activeRequest.workingCtx.pendingBlockingNode.NDGTIM;
	...
	deconnectClient: function(_, reason) {
		var pendingNode, pendingNodeArr, resp;
		this.stopRecord();
		try {
			...
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					...
					if ((pendingNodeArr = this.activeRequest ? Object.keys((pendingNode = this.activeRequest.workingCtx.pendingBlockingNode)) : null) && pendingNodeArr.length > 0) {
						...
						resp = this[pendingNode[pendingNodeArr[0]].proxy](_, {
							act: pendingNode[pendingNodeArr[0]].act,
							fld: {
								notModified: true
							},
							param: pendingNode[pendingNodeArr[0]].param
						});
	...
	listenNodeNDOTXSC: function(_, reader, node, ist, numTxt, imgId, txt, dummyTxt, actId) {
		...
		for (var i = 0; i < this._stackedWin.length && winFound == null; i++) {
			...
			if (item.getAdxIst().winAlphaId === ist.winAlphaId) {
				winFound = item;
			}
		}
		if (winFound != null) {
	...
	popRequesterWindow: function() {
		var win = this._stackedWin.popStackedWindow();
		win.setOpened(false);
		win.jsonType = "FUNCC";
		this.track(win);
		return win;
	...
	pushRequesterWindow: function(winId, funcId, descr, stamp, winIdx) {
		...
		this._stackedWin.pushStackedWindow(win);
	...
	getResponse: function(connect, isTrack) {
