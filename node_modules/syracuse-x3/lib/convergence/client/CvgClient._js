	...
	executeGetUiBO: function(_, key) {
		try {
			if (key === "GetBoToken") {
				...
				if (!this.boClient.boserv) {
					this.boClient.init(_);
				}
				this.clientConfig._params.GetBoHost = this.boClient.boserv;
			} else if (key === "GetBoPort") {
				if (!this.boClient.boport) {
					this.boClient.init(_);
				}
				this.clientConfig._params.GetBoPort = this.boClient.boport;
			}
		} catch (e) {
