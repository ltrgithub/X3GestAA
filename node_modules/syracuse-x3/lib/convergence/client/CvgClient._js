...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder) {
	...
	execOpenFunction: function(_,context){
		...
		return this.processAction(_,context,request, true);
	},
	connectSart: function(_,context) {
		...
		var resp = this.getResponse(_,context,true);
	...
	processAction: function(_,context, body, connect,mergeMode) {
		try {
			...
			this.newActiveRequest(reply, {"ctxNDSLB": ctx, "reqNum": body.tech && body.tech.id ? body.tech.id : 0},mergeMode);	
			var resu, extras = [];
			if (body.fld) {
				if (body.fld.v != null){
					var entity =null;
					if (body.fld.ist) {					
						...
						if (entity && entity instanceof CvgVariable) {
							...
							try {
								...
								return this.getResponse(_,context);
							}	
						}
					}
			...
			automata.process(_, this);
			var resp = this.getResponse(_,context,connect);
			traceAction && traceAction("************************************");
	...
	processSuperAction: function(_,context, body) {		
		var excludeList = ['target', 'previous'];
		var eraseNotExists = ['$rcd']; // if it's not exists in the last response and it exists in previous one, must erase 
		traceAction && traceAction("===================================");
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			var resp = this.processAction(_,context, newBody,false,i!=0);
	...
	processSpecialAction: function(_,context, body) {
		traceAction && traceAction("===================================");
	...
	processRequest: function(_, context) {
		...
		if (this.recorder){
			this.recorder.dumpHttpReq(_, context, bodyStr);
		}
		if (act >= 1024) {
			resp = this.processAction(_,context, body);
		}else if (act >= 768) {
			resp = this.processSuperAction(_,context, body);
		}else {
			resp = this.processSpecialAction(_,context, body);	
		}		
		if (this.recorder){
			this.recorder.dumpHttpResp(_, resp);
		}
		return {
			body : resp,
		};
	...
	getResponse: function(_,context,connect) {
