...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	var secure = (context.request.hosting && context.request.hosting.https) || ('authorized' in context.request.connection);
	this.syraSessionCookie = sessionManager.secureCookie(secure, sessionManager.getSyracuseCookie(context.request.session, context.request.connection.localPort));
	...
	_clientReplyPlugin: function(_, klass, body) {
		this.cvgPlugin.ND_IHM[klass]._clientReplyPlugin(_, body);
	},
	...
	getILogPlugin: function(_) {
		this.cvgPlugin.ND_IHM = this.cvgPlugin.ND_IHM || {};
		var plug;
		if (!this.cvgPlugin.ND_IHM["ilog"]) {
			plug = this.cvgPlugin.ND_IHM["ilog"] = new CvgPlugin(_, this, this.killPluginTimoeut || 20000); // default 120s if it's not defined
		} else {
			plug = this.cvgPlugin.ND_IHM["ilog"];
		}
		return plug;
	},
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (lists.RPC.length !== 0) {
			...
			if (this.lastAckCallSpgmId === "GESUSER:CONNECT_FONCTION") { // if reuse ack_call return
				if (rpc.json.result.errm !== null && sap.target.type === "portal") {
