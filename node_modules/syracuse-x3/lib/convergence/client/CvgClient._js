...
var fusionConfig=nodeconfig.config && nodeconfig.config.x3fusion;
var traceAction = fusionConfig && fusionConfig.actions && fusionConfig.actions.trace;
var traceProtocol = fusionConfig && fusionConfig.protocol && fusionConfig.protocol.trace;
var traceSessions = fusionConfig && fusionConfig.sessions && fusionConfig.sessions.trace;
...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, stream, adxInfos, aConfig) {
	// Activate dump
	if (nodeconfig.config && fusionConfig && fusionConfig.dump) this._spy = new Spy(_, stream);
	...
	this.newActiveRequest(new CvgReply(this, CONNECT_ACTION),{"reqNum":0,"connectAction":true} );
	...
	setInReuseMode: function(_,yes){
		...
		if (yes) {
			...
			var timeoutReuse =this.clientConfig.params.timeoutReuse;
			var self = this;
	...
	isSameContext: function(config, adxInfo){// check context for reuse session
		if (!this.clientConfig.isSameContext(config, traceSessions)) return false;
		if (!adxInfo.adxHttpPub || adxInfo.adxHttpPub !== this.adxInfos.adxHttpPub) {
			traceSessions && traceSessions("\tAdx HTTP publication folder is not the same : ["+adxInfo.adxHttpPub+"] !== ["+this.adxInfos.adxHttpPub+"]");
			return false;
		}
		return true;
	...
	processAction: function(_, body) {
		...
		try {
			...
			this.newActiveRequest(reply, {"ctxNDSLB": ctx, "reqNum": body.tech && body.tech.id ? body.tech.id : 0,"connectAction":connectOrReuse});
		...
		if (specialAction.proxy === "srvImp") {
			switch(specialAction.id) {
				case "PRINTER_SEL_OPTS":
					...
					var printClient = new PrintClient(_, srvImp.hostname, srvImp.port);
	...
	executeAllSessionSettings: function(values) {
		...
		if (values.DefaultDatePivot != null){
			this.formater.setDatePivot(this.appSrvSessionSettings.DefaultDatePivot);
		}
		return {
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		if (this.adxIstsOfPickedLines.length > 0) {
			...
			if (wLeftListLine != null) {
				...
				new CvgDiagnose(this.client, 'WARNING', 99, wMess,this.getMessage('ERROR_CANT_GET_PICKED_LINE_KEY', wArgIst.idx.toString()), null, "Protocol" , null, true);
			}
		} else {
	...
	filterSessionSettings: function() {
		...
		this.formater.addSessSettings(res);
		return res;
	},
