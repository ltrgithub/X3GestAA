...
exports.CvgClient = helpers.defineClass(function(protocolVersion) {
	...
	processSuperAction: function(_, body) {
		...
		var i, newBody, action, j, processNext, t0, t1, target, respTarget, stop = false,
			descActions = this.x3writer.descriptors.actions;
		for (i = 0; i < superAction.actions.length && !stop; i++) {
			...
			do {
				...
				if (!descActions[action.name]) {
					throw new Error("Bad sub-action: " + action.name);
				}
				processNext = true;
				newBody.act = descActions[action.name];
				if (nextFld) {
				...
				if (std[j]) {
					...
					if (action.propagate) {
						newBody.param.std.push(descActions[action.propagate]);
					}
				}
				...
				if (action.setTarget) {
					...
					processNext = superAction.processNextAction(descActions[action.name], t0, t1, action.checkNumLine, self);
				}
