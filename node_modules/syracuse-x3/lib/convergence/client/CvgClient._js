...
var CvgStackedWindows = require('../model/CvgStackedWindows').CvgStackedWindows;
var CvgStackedWindow = require('../model/CvgStackedWindow').CvgStackedWindow;
var CvgReply = require('./CvgReply').CvgReply;
var CvgRequest = require('./CvgRequest').CvgRequest;
var CvgInstance = require('../model/CvgInstance').CvgInstance;
var CvgStatusBar = require('../model/CvgStatusBar').CvgStatusBar;
var CvgCallUiOpenFile = require('../model/CvgCallUiOpenFile').CvgCallUiOpenFile;
var CvgDataSrc = require('../model/CvgDataSrc').CvgDataSrc;
var CvgDataSrcLine = require('../model/CvgDataSrcLine').CvgDataSrcLine;
...
var CvgSuperAction = require('./CvgAction').SuperActions;
...
var CvgMenuAct = require('../model/CvgMenuAct');
var CvgMenuCtx = require('../model/CvgMenuCtx');
var CvgClose = require('../model/CvgClose');
var CvgImp = require('../model/CvgImp');
var CvgServices = require('./CvgClientServices');
...
var CvgClientAbs = require('../../clients/common/cvg/CvgClientAbs').CvgClientAbs;
var globals = require('streamline/lib/globals');

var Tracer = require('../../entities/x3SessionConfig').Tracer;
var CvgObserver = require('./CvgObserver').CvgObserver;
var CvgPluginILog = require('./plugin/CvgPluginILog').CvgPluginILog;
var CvgPluginCR = require('./plugin/CvgPluginCR').CvgPluginCR;
...
exports.CvgClient = helpers.defineClass(function(protocolVersion) {
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				...
				case "PRINTER_SEL_CANCEL":
					...
					this.x3writer.props1Serializer.write(_, this, {});
				...
				case "PRINTER_SEL_OK":
					...
					this.x3writer.props1Serializer.write(_, this, props);
					this.automata.process(_, this, this.activeRequest, {
						title: "special",
	...
	loadWindowDescription: function(_, ist, winId, funcId, stamp) {
		...
		function loadResource(_, url, subpath, winName, cacheOptions) {
			...
			// try / except to track down error
			if (typeof jsonDescr === "string")
				try {
					jsonDescr = JSON.parse(jsonDescr);
				} catch (e) {
					console.error(new Date().toISOString(), "JSON parse error:", e.message, "url:", url, "Content:", jsonDescr);
					throw e;
				}
			return jsonDescr;
		}
	...
	executeCallUiOpenFile: function(values) {
		...
		if (localFile && ["HTTP", "HTTPS", "APP", "CLASSIC_PAGE"].indexOf(localDir) >= 0) {
			if (localDir !== "APP") {
				localFile = localFile.replace("\\", "/");
			}
			if (localDir === "HTTP" || localDir === "HTTPS") {
				localFile = localFile.replace(/\#/g, "%23");
			}
	...
	generateResponse: function(_, connect, isTrack, isAccepted) {
		...
		for (i = 0; i < lists.GLDIAGNOSE.length; i++) {
			...
			response.sap.$4glDiagnoses.push(x3helpers.sanitizeDiagnosis(JSON.parse(JSON.stringify(lists.GLDIAGNOSE[i]))));
		}
