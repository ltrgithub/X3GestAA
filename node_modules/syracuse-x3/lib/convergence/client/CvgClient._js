...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (lists.RPC.length !== 0 && sap.target && sap.target.type !== 'portal') {
			...
			//TODO check branche failure catch by runtime -  add diagnose to portal
			var result = lists.RPC[0].result;
			sap.target.portal = {
				$diagnoses: [{
					"$message": "x3 Error : errm:" + result.errm + " errn:" + result.errn + " errt:" + result.errt + " errl:" + result.errl,
					"$severity": 'ERROR',
					"appCode": 500,
