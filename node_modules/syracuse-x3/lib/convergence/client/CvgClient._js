...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	setInReuseMode: function(_, yes) {
		...
		if (yes) {
			...
			this.reuseTimeoutId = flows.setTimeout(function(_) {
				...
				self.tracer.trace(_, "session", "setTimeout - isReuse:" + self.isReuse + " - id:" + self.uuid, traceSessions);
	...
	connectSart: function(_, context) {
		...
		if (perfmonEnabled) {
			var act = "connect";
			timing.end();
			var t = timing && timing.getTiming();
			this.timing.action = this.timing.action || {};
			this.timing.action[act] = this.timing.action[act] || {};
			this.timing.action[act].connectSart = this.timing.action[act].connectSart || [];

			t && this.timing.action[act].connectSart.push({
	...
	processAction: function(_, body, connect, mergeMode, noResp) {
		...
		try {
			...
			if (perfmonEnabled) {
				...
				var t = timing && timing.getTiming();
				this.timing.action = this.timing.action || {};
				this.timing.action[params[0]] = this.timing.action[params[0]] || {};
				this.timing.action[params[0]].processAction = this.timing.action[params[0]].processAction || [];

				t && this.timing.action[params[0]].processAction.push({
					duration: (t && t.duration) || 0
				});
			}
			this.tracer.trace(_, "action", "************************************", traceAction);
			...
			this.tracer.trace(_, "processAction error\n" + (e.stack ? e.stack : e), traceLog);
	...
	processRequest: function(_, context, bodyStr) {
		...
		if (perfmonEnabled) {
			httptiming.end();
			var t = httptiming && httptiming.getTiming();
			this.timing.action = this.timing.action || {};
			this.timing.action[act] = this.timing.action[act] || {};
			this.timing.action[act].processRequest = this.timing.action[act].processRequest || [];

			t && this.timing.action[act].processRequest.push({
				duration: (t && t.duration) || 0
			});
	...
	loadWindowDescription: function(_, ist, winId, funcId, stamp) {
		...
		if (this.clientConfig.modePLAY) {
			...
			try {
				...
				var file, jsonDescr;
				try {
					file = sfs.readFile(__dirname + this.cvgWinStubPath + winId + ".xml", "utf8", _);
				} catch (e) {
	...
	closeClient: function(_, e, reason) {
		this.stopRecord();
		this.tracer.trace(_, "CloseClient Convergence Client\n\tReason[" + reason + "]\n\tAlreadyClosed[" + (this.connClient == null || !this.connClient.hasSocket()) + "] - Exception[" + (e == null ? "None" : e.message ? e.message : e) + "]", traceLog);
	...
	deconnectClient: function(_, reason) {
		...
		this.stopRecord();
		try {
			this.tracer.trace(_, "Deconnect Convergence client - isAlive" + this.isAlive() + "\n\tReason[" + (reason == null ? "none" : reason) + "]\n\tAsk X3 server to close ", traceLog);
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					this.closeSocket();
				} else {
	...
	_calculTiming: function(timing, key) {
		...
		if (Array.isArray(timingk)) {
			...
			!perfmonDetail && timing[key] && delete timing[key];
	...
	generateResponse: function(_, connect, isTrack) {
			...
			if (win) {
				...
				if (entity instanceof CvgStatusBar) {
					...
					if (winNode.entities[jsonKey] == null) {
						var timing = perfmonEnabled && perfmon.start(module, entity.getAdxIst() + ".toJSON");
		...
		response.srvop.request = {
			...
			"server": this.baseUrl // change by baseUrl - more interesting if the server public name is not the same and for unit test
		};
