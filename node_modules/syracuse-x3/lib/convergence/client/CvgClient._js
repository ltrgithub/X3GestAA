	...
	processSpecialAction: function(_, body) {
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				...
				case "PRINTER_SEL_OK":
					var selected = body.param && body.param.selpr && body.param.selpr.selected, extra;
					delete this.activeRequest.workingCtx.pendingBlockingNode.NDGTIM;
			...
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					...
					if((pendingNodeArr = this.activeRequest ? Object.keys((pendingNode = this.activeRequest.workingCtx.pendingBlockingNode)) : null) && pendingNodeArr.length > 0) {
						...
						resp = this[pendingNode[pendingNodeArr[0]].proxy](_, {act: pendingNode[pendingNodeArr[0]].act, fld: {notModified: true}, param: pendingNode[pendingNodeArr[0]].param});
