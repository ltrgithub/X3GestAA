...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	processAction: function(_, body, connect, mergeMode, noResp) {
		var timing = perfmonEnabled && perfmon.start(module, 'cvgClient.processRequest', body.act);
		try {
			var pendingNode;
			this.tracer.enabled && this.tracer.trace(_, "action", "************************************", traceAction);
			this.tracer.enabled && this.tracer.trace(_, "action", "Begin Process Action:\n" + JSON.stringify(body, null, 2), traceAction);
			if (this.recorder) {
				this.recorder.dumpAction(_, body.act, false);
			}
			...
			if (body.fld) {
				if (body.fld.v != null) {
					...
					if (body.fld.ist) {
						...
						this.tracer.enabled && this.tracer.trace(_, "action", "ACTION IST=" + JSON.stringify(ist, null, 2), traceAction);
			...
			this.tracer.enabled && this.tracer.trace(_, "protocol", "Send true value: " + resu.toString(), traceProtocol);
			...
			if (perfmonEnabled) {
				this.recordTiming(timing, params[0], "processAction");
			}
			this.tracer.enabled && this.tracer.trace(_, "action", "************************************", traceAction);
			this.tracer.enabled && this.tracer.trace(_, "action", "End Process Action:\n" + JSON.stringify(this.response, null, 2), traceAction);
			//return resp;
		} catch (e) {
			this.tracer.enabled && this.tracer.trace(_, "processAction error\n" + (e.stack || e), traceLog);
