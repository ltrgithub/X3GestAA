...
var CvgClient = exports.CvgClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	actionParams: function(body) {
		...
		if (body.param.std && body.param.std[0]) params = params.concat(body.param.std);
		return params;
	},
	processAction: function(_, body) {
		...
		if (body.fld) {
			...
			if (crsr != null) extras.push(new CvgResu(CvgResu.TDO_INT, parseInt(body.fld.ctx.crsr)));
		}
	...
	processSuperAction: function(_, body) {
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			if (std[i]){
				...
				if (action.iterate){
					if (action.iterate === 'line'){
						for (var j in std[i]){
							newBody.param.target.nl = std[i][j];
							resp = this.processAction(_, newBody);
						}
					}
					else {
						throw new Error("Iterate '"+action.iterate+"' not yet implemented.");
					}
