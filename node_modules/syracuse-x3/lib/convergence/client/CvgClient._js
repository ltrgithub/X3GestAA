...
var allDescr = {}; // map of all descriptions loaded
...
function formatKeyValues(result) {
	return [Object.keys(result).map(function(key) {
		// TODO: Split entries to 250 characters maximum
...
var CvgClient = exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	this.boClient = new boRestClient(_, null, context);
	...
	setInReuseMode: function(_, yes) {
		...
		if (yes) {
			...
			this.reuseTimeoutId = setTimeout(_ << function(_) {
	...
	connectSart: function(_, context) {
		...
		var timeact = {};
		timeact["connect"] = t1;
		this.activeRequest.timing.server.action.push(timeact);
	...
	processAction: function(_, body, connect, mergeMode) {
		try {
			...
			var ctx, target, reply = new CvgReply(this, body.act);
							...
							try {
								if (value !== undefined) {
									...
									if (entity.dataType === CvgResuTdo['BLOB']) { // blob case the value is not send by the client but is store in blobSvc
										value = this.services.blobSvc.getBlob(value).binary;
									}
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				case "PRINTER_SEL_OPTS":
					...
					try {
						printClient = PrintClient.getClient(_, srvImp.hostname, srvImp.port);
						...
						var diag = new CvgDiagnose(this, 'INFO', 99, locale.format(module, "GET_PRINTER_OPT_1"), e.message || "", e, "Protocol [processSpecialActionPRINTER_SEL_OPTS]", null, false);
				...
				case "PRINTER_SEL_OK":
					...
					if (selected.duplex) props._PrtDescDuplex = selected.duplex;
					// Check paper size/source
					if (printerName && ((selected.paperSize !== undefined && selected.paperSize !== null) || (selected.paperSource !== undefined && selected.paperSource !== null))) {
						try {
							printClient = PrintClient.getClient(_, srvImp.hostname, srvImp.port);
						} catch (e) {
							if (printClient) {
						...
						if (printClient) {
							...
							paperProps = printClient.checkPrinterPaper(_, printerName, selected.port, selected.driver, selected.paperSize, selected.paperSource);
							if (paperProps) {
								//console.log("\t Papers props OUT" + JSON.stringify(paperProps,null, 2));
								if (paperProps.paperSource) props._PrtDescSource = paperProps.paperSource.id;
								if (paperProps.paperSize) {
									props._PrtDescPaperSize = paperProps.paperSize.id;
									if (paperProps.paperSize.height) props._PrtDescPaperHeight = paperProps.paperSize.height;
									if (paperProps.paperSize.width) props._PrtDescPaperWidth = paperProps.paperSize.width;
								}
							}
						}
					}
	...
	processRequest: function(_, context, bodyStr) {
		...
		var bodyStr = bodyStr != null ? bodyStr : context.request.readAll(_);
		...
		var timing = perfmon.start(module, 'cvgClient.processRequest', act);
		if (act >= 1024) {
	...
	loadWindowDescription: function(_, ist, winId, funcId, stamp) {
		...
		if (stamp) {
			...
			//console.log("\n Last modified " + lastmodified.toGMTString() + " . So max age : " + cacheCtrl + "\n");
		}
		...
		function getWindowXml(_, winId) {
			try {
				var file = null;
				try {
					file = require('streamline-fs').readFile(__dirname + "/../data/" + winId + ".xml", "utf8", _);
				} catch (e) {
					//console.log("failed to read local file (try remote file) "+e.stack);
				}
				if (!file) {
					var url = self.winDescrUrl(winId);
					var header = {};
					if (cacheCtrl) {
						header["Cache-Control"] = cacheCtrl;
					}
					if (allDescr && allDescr[url]) {
						header["If-None-Match"] = allDescr[url].etag;
				...
				throw new CvgException(self, e, "can't get window description " + winId, "KILL", "");
	...
	getFormulaItem: function(_, win, file, target) {
		...
		if (response.statusCode === 200 && body) {
			...
			for (var i = 0; i < items.Main.LINE.length; i++) {
				...
				this.activeRequest.listenNodeNDOFLD(_, null, null, win.getAdxIst(), item.COL[0]["$cdata"], item.COL[1]["$cdata"], item.COL[2]["$cdata"], item.COL[3]["$cdata"], item.COL[4]["$cdata"], item.COL[5]["$cdata"], target);
			}
		}
	},
		...
		if (keys.length >= 2) {
			...
			if (localDir === "HTTP" || localDir === "HTTPS") {
				...
				if (this.callUiOpenFile) {
					this.callUiOpenFile.addOpenFile(localDir.toLowerCase(), url);
				} else {
					this.callUiOpenFile = new CvgCallUiOpenFile(this, localDir.toLowerCase(), url);
				}
			}
			return {};
		}
	...
	executeAllSessionSettings: function(values) {
		...
		for (var i = 1; i < keys.length; i++) {
			...
			/*if( keys[i] === "BOSEC" ){
				this.boClient.bosec = values[keys[i]];
			}else if( keys[i] === "BOSERV" ){
				this.boClient.boserv = values[keys[i]];
			}else if( keys[i] === "BOUSER" ){
				this.boClient.bousr = values[keys[i]];
			}else if( keys[i] === "BOPWD" ){
				this.boClient.bopwd = values[keys[i]];
			}*/
		}
	...
	listenNodeND_GFD: function(_, reader, node, properties) {
		...
		if (key === "GetBoToken") {
			try {
				this.clientConfig._params.GetBoToken = this.boClient.token ? this.boClient.token : this.boClient.logon(_);
			} catch (e) {
				this.sendNotify(null, 2, e.message);
			}
		}
		...
		var res = formatKeyValues(result);
		return res;
	},
	...
	listenNodeNDCTSU: function(_, reader, node, type, subType, compl) {
		 ...
		 */
		var type = String.fromCharCode(type);
	...
	listenNodeNDPKAR: function(_, reader, node) {
		if (this._pickedLines.length > 0) {
			...
			var actArgs = [CvgAction.SRV_ABANDON];
		}
	...
	getResponse: function(_, connect, isTrack) {
		...
		for (var i = 0; i < lists.ENTITY.length; i++) {
			...
			var win = entity instanceof CvgStackedWindow ? entity : (entity instanceof CvgStatusBar ? entity.stackedWindow : this._stackedWin.findWin(entity.getFatherWindowId().charCodeAt(0) - '@'.charCodeAt(0)));
			if (win) {
		...
		for (var i = 0; i < lists.FUNCO.length; i++) {
			sap.func = sap.func || {};
			var win = lists.FUNCO[i];
		...
		for (var i = 0; i < lists.FUNCC.length; i++) {

			sap.func = sap.func || {};
			var win = lists.FUNCC[i];
			var key = win && win.entityIdxAlpha;
		...
		for (var i = 0; i < lists.GLDIAGNOSE.length; i++) {
		...
		this.activeRequest.timing.server.response.push(t1);
		// add sessionInfo
		if (true === connect) {
		...
		if (this.srvTrace) {
			if (this.srvTrace.all) {
				response.timing = this.activeRequest.timing;
			} else {

				if (this.srvTrace.active && !Array.isArray(this.srvTrace.active)) {
					var arr = [this.srvTrace.active];
				} else {
					var arr = this.srvTrace.active;
				}
				var treat = false;
				for (var i = 0; i < arr.length; i++) {
					if (arr.indexOf('.') === -1) {
						response.timing[arr] = this.activeRequest.timing[arr];
					} else { // need to be test
						var keys = arr.split('.');
						var curTarg = response.timing;
						var curFrom = this.activeRequest.timing;
						for (var j = 0; j < keys.length && curFrom; j++) {
							curTarg = curTarg[keys[j]];
							curTarg = {};
							curFrom = curFrom[keys[j]];
						}
						curTarg = curFrom;
					}
				}
