...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	getTarget: function() {
		...
		return lists.TARGET[lists.TARGET.length - 1];
	},
	...
	generateResponse: function(_, connect, isTrack) {
		...
		if (lists.SESSIONOPEN && lists.SESSIONOPEN.length !== 0) {
			response.session.open = lists.SESSIONOPEN[0];
		}
		var sap = response.sap;
		if (lists.MISC && lists.MISC.length !== 0) {
			sap.misc = lists.MISC[0];
		}
		...
		if (lists.RPC.length !== 0) {
			...
			if (this.lastAckCallSpgmId === "GESUSER:CONNECT_FONCTION") { // if reuse ack_call return
				if (rpc.errm !== null && sap.target.ist.ist === CvgIst.levelZeroStructIst.ist) {
					sap.target.portal = {
						$diagnoses: [{
							"$stackTrace": "x3 Error; num:" + rpc.errn + ", message:" + rpc.errm + " line:" + rpc.errl + " type:" + rpc.errt,
							"$severity": 'ERROR',
							"appCode": 500,
							"$origin": "NodeJS: Convergence Server ",
							"$message": "x3 Error - " + rpc.errm,
						}]
					};
				}
		...
		for (i = 0; i < lists.REPORT.length; i++) {
			...
			sap.jobs.report[kind].push(report);
		}
		if (true === connect) {
		...
		for (i = 0; i < lists.DIAGNOSE.length; i++) {
			...
			response.srvop.$diagnoses.push(lists.DIAGNOSE[i]);
		}
		for (i = 0; i < lists.GLDIAGNOSE.length; i++) {
			response.sap.$4glDiagnoses = response.srvop.$4glDiagnoses || [];
			response.sap.$4glDiagnoses.push(lists.GLDIAGNOSE[i]);
		}
