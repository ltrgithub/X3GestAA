...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
		...
		if (specialAction.proxy === "srvImp") {
			...
			switch (specialAction.id) {
				case "REPLY_PLUGIN":
					...
					x3writer.props1Serializer.write(_, this, this._buildPluginResponse(_, body && body.param && body.param.plugin));
					automata.process(_, this, this.activeRequest, {
						title: "special",
						detail: "REPLY_PLUGIN"
					});
					break;
				default:
					throw new Error("Unknow plugin action [" + specialAction.id + "]");
			}
		} else {
	...
	_buildPluginResponse: function(_, paramPlug) {
		// get result to construct property reply
		//var result = paramPlug.result; // can trace it maybe TODO
		if (!paramPlug || !paramPlug.return ||!paramPlug.return .name)
			var ret = {};
