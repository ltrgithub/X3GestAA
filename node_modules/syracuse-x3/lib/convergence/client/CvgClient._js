...
exports.CvgClient = helpers.defineClass(function(_, cvgSession, stream, aConfig, recorder, context) {
	...
	recordTiming: function(timer, action, funcName) {
		timer.end();
		var t = timer.getTiming();
	...
	processRequest: function(_, context, bodyStr) {
		...
		if (this.isProcessingRequest) { // process already oin progrss
			...
			CvgDiagnose.create(this, null, 409, "a request is already processing", null, error, origin, "BUSY", true, null);
	...
	deconnectClient: function(_, params, reason) {
		...
		try {
			...
			console.log("ask Kill client uuid=[" + this.uuid + "] reasons=" + reason + " panic=" + (params.panic ? params.panic : false));
			...
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					...
					this.logSession(_, "INFO", "ask Kill client uuid=[" + self.uuid + "] reasons=" + reason + " panic=" + (params.panic ? params.panic : false));
					var diag = CvgDiagnose.create(this, null, 500, "a request is already processing", null, null, null, "KILL", true);
					diag.statusCode = 500;
					this.funnelAction(_, function(_) {
						...
						function isLoopState(listNodes) {
							...
							var b = Object.keys(listNodes).length > 10;
							if (!b) {
								b = Object.keys(listNodes).reduce(function(prev, cur, idx, arr) {
									return prev || listNodes[cur] >= 10;
								}, false);
							}
							return b;
			...
			this.logSession(_, "INFO", "disconnect client failed " + e.stack);
		} finally {
			this.activeRequest = null;

			this.tracer && this.tracer.close(_);

			this.logSession(_, "INFO", "disconnect client uuid=[" + this.uuid + "] completed ");
	...
	generateResponse: function(_, connect, isTrack) {
		...
		var wins = {}, win, key, i, entity, winNode, gridParent, line, jsonKey, jsonEntity;
