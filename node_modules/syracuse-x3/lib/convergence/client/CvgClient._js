...
var traceProtocol = nodeconfig.config && nodeconfig.config.x3fusion && nodeconfig.config.x3fusion.actions && nodeconfig.config.x3fusion.protocol.trace;
...
/*
	...
	processSuperAction: function(_, body) {
		...
		function getActionIndex(name){
			for (var idx in superAction.actions){
				if (superAction.actions[idx].name === name)
					return idx;
			}
			return;
		}
