...
var CvgLeftList = require('../model/CvgLeftList').CvgLeftList;
var CvgModifier = require('../model/CvgModifier').CvgModifier;
var CvgDataSrc = require('../model/CvgDataSrc').CvgDataSrc;
var CvgEntity = require('../model/CvgEntity').CvgEntity;
var CvgFormat = require('../model/CvgFormat').CvgFormat;
var CvgResu = require('../types/CvgResu').CvgResu;
var CvgResuTdo = require('../types/CvgResu').TDO;
var CvgResuTag = require('../types/CvgResu').TAG;
var CvgIst = require('../types/CvgIst').CvgIst;
var CvgLeftListLineNode = require('../model/CvgLeftListLineNode').CvgLeftListLineNode;
var CvgException = require('../model/CvgException').CvgException;
var CvgWindow = require('../model/CvgWindow').CvgWindow;
var CvgMessage = require('../model/CvgMessage').CvgMessage;
var CvgVariable = require('../model/CvgVariable').CvgVariable;

var CvgVariableCombo = require('../model/CvgVariableCombo').CvgVariableCombo;
var CvgPickedLine = require('../model/CvgPickedLine').CvgPickedLine;
var CvgReplyAckCall = require('../model/CvgReplyAckCall');
var CvgInstanceExeci = require('../model/CvgInstanceExeci');
var CvgDiagnose = require('../model/CvgDiagnose');
var CvgSessionOpen = require('../model/CvgSessionOpen');
var CvgAskUiAction = require('../model/CvgAskUiAction');
var CvgReboot = require('../model/CvgReboot');
var PrintClient = require('../../clients/print/PrintClient');
var PrintHelpers = require('../../clients/print/PrintHelpers');
var CvgReport = require('../model/CvgReport');
var CvgImp = require('../model/CvgImp');
var CvgPluginTarget = require('../model/CvgPluginTarget').CvgPluginTarget;
var CvgRequestAbs = require('../../clients/common/cvg/CvgRequestAbs').CvgRequestAbs;

var CvgHelp = require('../model/CvgHelp').CvgHelp;
...
function _getSrvImpId(srvImpName, printConfig) {
	var ret = {},
		edtSrvPart = srvImpName ? srvImpName.split(':') : null;
...
var CDataSrcDeleter = helpers.defineClass(function(cli, aTargetLine, aNbLines) {
	...
	setModifiable: function(ist, obj) {
		var o = obj;
		if (!(obj instanceof CvgDataSrc)) {
			o = obj.srcLine && obj.srcLine.dataSrc || obj.dataSrc;
		}
		if (o) {
			if (this.nbLines === -1) {
				o.serverRemoveAllLines();
			} else {
				o.serverRemoveLines(this.targetLine, this.nbLines);
			}
		}
		return this.done;
	}
...
var CDataSrcInserter = helpers.defineClass(function(cli, aTargetLine, aNbLines) {
	...
	setModifiable: function(ist, obj) {
		...
		if (o) {
			o.serverInsertLines(this.zeroBasedIdx, this.nbLines);
			if (o.hasDimOverFlow()) {
				this.setLogicalErrMess(this.cli.getMessage('ERROR_DIM_OVERFLOW', o.dim.toString(), this.nbLines.toString(), o.lines.length.toString()));
			}
		}
		return this.done;
	}
...
var CLeftListAppender = helpers.defineClass(function(aIdx, aBranche, aTxt, aVal, aClf, aClp) {
	...
	setModifiable: function(ist, obj) {
		...
		o.appendLeftListLine(this.idx, this.branche, this.txt, this.val, this.clf, this.clp);
		this.done = true;
		return this.done;
	}
});
...
var CLeftListColumnsModifier = helpers.defineClass(function(aWhat, aDefs) {
	...
	setModifiable: function(ist, obj) {
		var o = obj;
		if (!(obj instanceof CvgLeftList)) { // must modified on the leftList it self and not the first element
			o = obj.srcLine && obj.srcLine.dataSrc || obj.dataSrc;
		}
		if (this.whatModification === NDCOLAR_MODIF_FORMATS) {

			o.changeColumnsFormats(this.defs);
		} else {
			o.changeColumnsTitles(this.defs);
		}
		this.done = true;
		return this.done;
	}
...
var CLeftListSelecterByKey = helpers.defineClass(function(selected, aCle) {
	...
	setModifiable: function(ist, obj) {
		...
		this.selLineIdx = o.setServerSelectLine(this.selected, this.cle);
...
exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	 ...
	 * colonnes
	 */
	listenActionArgsBRWRAPIDESEL: function(aClient, opt) {
		var aParams = opt.params;
	...
	listenActionArgsBRWCHANGELINE: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsBRWSELECTLINE: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsTREEEXPANDLINE: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsTREESELECTLINE: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsTREECHANGELINE: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsSLST: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsDABR: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	listenActionArgsCABR: function(aClient, opt) {
		this.listenActionArgsDOLEFTLISTLINE(aClient, opt);
	},
	/**
	 ...
	 */
	listenActionArgsDOLEFTLISTLINE: function(aClient, opt) {
		var aParams = opt.params;
		if (aParams.length >= 3) {
			...
			this.listenActionArgsDOLEFTLISTLINE2(aClient, opt);
		}
		return aParams;

	},
	listenActionArgsDOLEFTLISTLINE2: function(aClient, opt) {
		var aParams = opt.params;
	 ...
	 */
	listenActionArgsLIST_PAGE_DOWN: function(aClient, opt) {
		var aParams = opt.params;
	 ...
	 */
	listenActionArgsLIST_PAGE_UP: function(aClient, opt) {
		var aParams = opt.params;
		var leftLst = this.retrieveLeftList(aClient, aParams);
	 ...
	 * Ajoute : - la cle apres l'argument IST
	 */
	listenActionArgsPICKING: function(aClient, opt) {
		var aParams = opt.params;
		var ist = aParams[1],
		...
		function registerPickedLine(key) {
			...
			var pickedLine = new CvgPickedLine(self.client, ist, key, leftlist, actToPropagate);
	...
	listenNodeND_ASKUI: function(_, reader, node, properties) {
		try {
			...
			this.workingCtx.pendingBlockingNode = {
				...
				"act": this.client.x3writer.descriptors.actions.SRV_ASKUI_RESPONSE,
				"param": {},
				"fld": {
					"ist": null,
	...
	listenNodeNDINED: function(_, reader, node) {
		this.client.x3writer.sendAction(_, this.client, new CvgResu(CvgResuTdo.STRU, ""), [], [this.client.x3writer.descriptors.actions.SRV_EXECUTE]);
	...
	listenNodeND_OCX: function(_, reader, node, properties) {
		// TODO: translate message
		if (properties) {
			var prop = properties;
	...
	listenNodeND_IHM: function(_, reader, node, properties) {
		...
		try {
			if (properties) {
				...
				if (prop && (prop.IAction === 1 || prop.IAction === 6) && this.client.getILogPlugin().id !== "ilog-default") {
					this.client.newILogPlugin();
				}
				var plug = this.client.getILogPlugin();
	...
	listenNodeNDCHGR: function(_, reader, node, ist, state) {
		// 2 "SRV_GRIZO" - 1 "SRV_ACTZO - 0 "SRV_DISZO";
