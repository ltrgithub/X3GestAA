...
var CvgReplyAckCall = require('syracuse-x3/lib/convergence/model/CvgReplyAckCall').CvgReplyAckCall;
var CvgReboot = require('syracuse-x3/lib/convergence/model/CvgReboot').CvgReboot;
var CvgDiagnose = require('syracuse-x3/lib/convergence/model/CvgDiagnose').CvgDiagnose;
var CvgInstanceExeci = require('syracuse-x3/lib/convergence/model/CvgInstanceExeci').CvgInstanceExeci;
var PrintClient = require('syracuse-print/lib/PrintClient');
var CvgReportSubmitted = require('syracuse-x3/lib/convergence/model/CvgReport').CvgReportSubmitted;
...
function _postDiag(req,msg, detail){
	new CvgDiagnose(req.client, 'WARNING', 99, msg, detail, null, "Protocol" , null, true);			
}
...
var CLeftListPickingSelecter = exports.CLeftListPickingSelecter = helpers.defineClass(function(selected) {
	...
	setModifiable: function(ist, obj) {
		obj.setLeftListLineSelected(this.selected, true)
		this.done = true;
		return this.done;
	}
...
var CLeftListSelecterByKey = exports.CLeftListSelecterByKey = helpers.defineClass(function(selected, aCle, request) {
	...
	setModifiable: function(ist, obj) {
		obj.setServerSelectLine(this.selected, this.cle);
		this.request.setLefListLineSelected(obj.findLineIdx(this.cle));
		this.done = true;
		return this.done;
	}
});

var CFormatSetter = exports.CFormatSetter = helpers.defineClass(function(cli, aFormat) {
	CvgModifier.call(this);
	this.format = aFormat;
	...
	setModifiable: function(ist, obj) {
		...
		this.newVsisibleState = obj.setServerFormat(new CvgFormat(this.format));
		this.done = true;
		return this.done;
	}
});

var CVisibleStateSetter = exports.CVisibleStateSetter = helpers.defineClass(function(aEntityState) {
	CvgModifier.call(this);
	...
	setModifiable: function(ist, obj) {
		 ...
		 */
		// console.log("CVisibleStateSetter")
		obj.stateSet(this.state, true);
		this.done = true;
		return this.done;
	}
});
...
var CActiveStateSetter = exports.CActiveStateSetter = helpers.defineClass(function(aState) {
	CvgModifier.call(this);
	this.entitryState = aState;
}, CvgModifier, {
	setModifiable: function(ist, obj) {
		// console.log("CActiveStateSetter")
...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	this.client = aClient;
	...
	loadWindowDescription: function(_, winId) {
		...
		return jsxml.parse(getWindowXml(_, winId)).WIND;
	},
	...
	listenActionArgsPICKING: function(aClient, aParams) {
		var ist = aParams[1],actToPropagate = aParams[3],self = this;
		function registerPickedLine(line) {
			if (typeof ist === "string")
				ist = new CvgIst(ist, line);
			else if (ist instanceof CvgIst) 
				ist =  new CvgIst(ist.ist, line);
			else
				throw new CvgException(self.client, null, "ARG_WRONG - need a 'CvgIst' or a 'String' !");
			var pickedLine = new CvgPickedLine(ist, actToPropagate);
			//console.log(">>> Picked line IST: "+pickedLine.ist.toString());
			self.client.traverseSettable(ist, new CLeftListPickingSelecter(pickedLine.isSelection()));
			self.client.addAdxIstOfPickedLines(pickedLine);
		}
		/*
	 ...
	 * "AskGetFile"
	 */
	listenNodeND_ASKUI: function(_, reader, node) {
		x3writer.sendAction(_, this.client, new CvgResu(CvgResuTdo["STRU"], "-99,Not supported"), [], [CvgAction.SRV_ASKUI_RESPONSE]);
		// TODO: translate message
		_postDiag(this, "ASKUI node not supported", "ND_ASKUI");
	},
	...
	listenNodeND_ASKUISendAutoReply: function(_, aNumError, aExplaination) {
		var resp = new StringBuilder();
		resp.append(aNumError);
		resp.append(',');
		resp.append(aExplaination);
		var istDatas = new CvgIstDatas(CvgResuTag["STRU"], resp.toString());
		var protocol = this.adonixClient.remoteProtocolNum;
		var segAct = CSegmentActionFactory.buildSegmentAction(true, protocol, CvgAction.SRV_ASKUI_RESPONSE, istDatas);
		if (this.client.typeClient === ETypeClient.JULIET) {
			this.connClient.writeSegment(segAct);
		} else {
			this.adonixClient.writeSegment(segAct, false);
		}
	},
	...
	listenNodeND_CLF: function(_, reader, node, ist) {
		...
		var stackedWin = this.client.popRequesterWindow();
		var reply = this.reply;
		reply.addPopedWindow(stackedWin);
	},
	...
	listenNodeND_REB: function(_, reader, node, str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13, str14, str15, str16, str17) {
		// Notify observer
		new CvgReboot(this.client, str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13, str14, str15, str16, str17);
	...
	listenNodeNDCHFM: function(_, reader, node, ist, fmt) {
		...
		if (setter.hasNewVisibleState() && ist.isCellIst()) {
			// pour designer la colonne
			ist.idx = -1;
			var setter = new CVisibleStateSetter(setter.newVsisibleState);
			this.client.traverseSettable(ist, setter);
		}
		if (!setter.done) {
	...
	listenNodeNDEXECI: function(_, reader,node,ist,spgm,dbgflg,paramDefs,paramVal) {
		new CvgInstanceExeci(_, ist,spgm,dbgflg,paramDefs[0],paramVal[0],this.client);
	...
	listenNodeNDGLAR: function(_, reader, node, ist, key) {
		var selector = new CLeftListSelecterByKey(true, key, this);
	...
	listenNodeNDULAR: function(_, reader, node, ist, key) {
		this.client.traverseSettable(ist, new CLeftListSelecterByKey(false, key, this));
	},
	 ...
	 * trait� dans la methode "listenNodeNDOBOXI" de la classe "CAdonixRequest"
	 *
	...
	listenNodeNDOBOXI: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		if (typBox === OU_BIF) {
			var p = msg.indexOf(Cvg.AFF_SYNCHRONE_HEADER);
		...
		var typMsg = -1, defBtn = -1;
		if (msg.length > 0) msg.replace('\r', ' ');
		if (ttl.length > 0) ttl.replace('\r', ' ');
		switch (typBox) { 
		/* Boite de Question */
		case OU_BQU:
			typMsg = MSG_QUESTION;
		...
		var replyAuto = this.silent || (this.reply.actionId === CONNECT_ACTION) && (typMsg === MSG_STOP || this.client.isInteractiveSession);
		var msg = new CvgMessage(typMsg, 0, msg, ttl, defBtn, wSleep, btnNum,this.client);
	 ...
	 */
	listenNodeNDOFLD: function(_, reader, node, ist, txt1, txt2, txt3, int1, txt4, txt5) {
		ist.screen = 34;
		ist.bloc = 1;
		ist.idx = -1;
		var line = [];
		line.push(CvgResuTag["STRU"], txt1);
		line.push(CvgResuTag["STRU"], txt2);
		line.push(CvgResuTag["STRU"], txt3);
		line.push(CvgResuTag["STRU"], parseInt(int1, 10));
		line.push(CvgResuTag["STRU"], txt4);
		line.push(CvgResuTag["STRU"], txt5);
		var clf = [];
		this.client.traverseSettable(ist, new CLeftListAppender(-1, 0, "", line, clf, null));
	},
	...
	listenNodeNDOUVIMP: function(_, reader, node, properties) {
		...
		function replyError(e) {
			var report = new CvgReportSubmitted(self.client, {}, e);
			return [0, 0];
		}
		var self = this;
		if (properties.STATE && properties.STATE === "LAUNCHED") {			
			...
			var report = new CvgReportSubmitted(this.client, props);			
			return [0, 1];
		} else {
			...
			try{
				...
				var edtSrvPart = properties['+X3IMP'] ? properties['+X3IMP'].split(':') : null;
				var hostname = (edtSrvPart && edtSrvPart[0]) || this.client.printConfig.server.hostname;
				var port = (edtSrvPart && edtSrvPart[1]) || this.client.printConfig.server.port;
				this.setSrvImp(hostname, port);				
				printClient = PrintClient.createClient(_, hostname, port);
				var seqId = printClient.launchReport(_, properties);				
				if (seqId === 0) {
					// TODO: translate message
					return replyError(new CvgException(this.client, null, "An error has occured while submitting report to report server.", null, "SrvImp Server"));
				}				
				var report = new CvgReportSubmitted(this.client, {
					host: hostname + ":" + port,
					trackId: jobId,
					title: properties.X3TIT,
					rank: seqId,
				...
				return replyError(new CvgException(this.client, e, null, null, "SrvImp Client"));
			}finally{
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		//console.log("NDGTIM Properties: "+JSON.stringify(properties,null,2));
		var printClient;
		try {
			var edtSrvPart = properties._PrinterServer ? properties._PrinterServer.split(':') : null;
			var hostname = (edtSrvPart && edtSrvPart[0]) || this.client.printConfig.server.hostname;
			var port = (edtSrvPart && edtSrvPart[1]) || this.client.printConfig.server.port;
			this.setSrvImp(hostname, port);
			printClient = PrintClient.createClient(_, hostname, port);
			...
			var selectedIdx, selected = properties._PrinterName && properties._PrinterName.length !== 0 ? properties._PrinterName : null;
			var records = [];
			...
			for (var i in selPr) {
				...
				if (properties._PrinterCopies) {
					records[i].copies = parseInt(properties._PrinterCopies, 10);
				}
				if (properties._Orientation) {
					records[i].orientation = parseInt(properties._Orientation, 10);
				}
				if (properties._PrinterCollate) {
					records[i].collate = properties._PrinterCollate === "0" ? true : false;
				}
				if (properties._PrinterStartPage) {
					records[i].startpage = parseInt(properties._PrinterStartPage, 10);
				}
				if (properties._PrinterStopPage){
					records[i].stoppage = parseInt(properties._PrinterStopPage, 10);
				}
				var capabilities = [];
				if (selPr[i].collate) capabilities.push("collate");
				if (selPr[i].copies) capabilities.push("copies");
				if (selPr[i].orientation) capabilities.push("orientation");
				if (selPr[i].duplex) capabilities.push("duplex");
				records[i].$capabilities = capabilities.join(',');
			}
			if (selected) {
				var sel = records.splice(i, 1);
				records.splice(0, 0, sel[0]);
			}
			var config = new CvgImpConfig(this.client, records);
			
		}catch(e) {
			console.error("NDGTIM error: "+e.stack);
			throw e;
		}finally{
			if (printClient) printClient.disconnect(_);
			return;
		}
	...
	listenNodeNDOWIN: function(_, reader, node, ist, winId, funcId, stamp, extInfos) {
		...
		var descr = this.loadWindowDescription(_, winId);
		stackedWin = this.client.pushRequesterWindow(winId, funcId, descr, stamp, ist.win);
	...
	listenNodeNDRESULT: function(_, reader, node, ist, errn, errl, errp, errt, errm, resu, tblParams, tblResus) {
		// Instanciate CvgReplyAckCall and store it in client.trackList.RPC
		new CvgReplyAckCall(this.client, errn, errl, errp, errt, errm, resu[0], tblParams[0], tblResus[0]);
	},
	 ...
	 */
	listenSegment: function(segment) {
		if (!validSegmentNode(segment)) {
			return false;
		}
		var processed = false;
		switch (segment.nodeId) {
		case CvgNode.NDOLAR:
			processed = listenNodeNDOLAR(segment);
			break;
		case CvgNode.NDCHVL:
			processed = listenNodeNDCHVL(segment);
			break;
		case CvgNode.NDCHVB:
			processed = listenNodeNDCHVB(segment);
			break;
		case CvgNode.NDCHFM:
			processed = listenNodeNDCHFM(segment);
			break;
		case CvgNode.NDCHGR:
			processed = listenNodeNDCHGR(segment);
			break;
		case CvgNode.NDCHCR:
			processed = listenNodeNDCHCR(segment);
			break;
		case CvgNode.NDOFLD:
			processed = listenNodeNDOFLD(segment);
			break;
		case CvgNode.NDCTIT:
			processed = listenNodeNDCTIT(segment);
			break;
		case CvgNode.NDCSTL:
			processed = listenNodeNDCSTL(segment);
			break;
		case CvgNode.NDGLAR:
			processed = listenNodeNDGLAR(segment);
			break;
		case CvgNode.NDINSLI:
			processed = listenNodeNDINSLI(segment);
			break;
		case CvgNode.NDOCAR:
			processed = listenNodeNDOCAR(segment);
			break;
		case CvgNode.NDSUPLI:
			processed = listenNodeNDSUPLI(segment);
			break;
		case CvgNode.NDAFTB:
			processed = listenNodeNDAFTB(segment);
			break;
		case CvgNode.NDRZAR:
			processed = listenNodeNDRZAR(segment);
			break;
		case CvgNode.NDULAR:
			processed = listenNodeNDULAR(segment);
			break;
		case CvgNode.ND_CLF:
			processed = listenNodeND_CLF(segment);
			break;
		case CvgNode.NDDSLB:
			processed = listenNodeNDDSLB(segment);
			break;
		case CvgNode.ND_NAP:
			processed = listenNodeND_NAP(segment);
			break;
		case CvgNode.NDOMNAO:
		case CvgNode.NDOMNBO:
			processed = listenNodeNDOMNBO(segment);
			break;
		case CvgNode.NDOUVIMP:
			processed = listenNodeNDOUVIMP(segment);
			break;
		case CvgNode.NDOFENH:
			processed = listenNodeNDOFENH(segment);
			break;
		case CvgNode.ND_ASKUI:
			// AskUI_Call]
			processed = listenNodeND_ASKUI(segment);
			break;
		default:
		}
		if (processed) {
			this.receivedSegments.add(segment);
			// noeud pour rendre la main a l'utilisateur => traitement du noeud
			// complet et fin d'attente
			if (segment.isNodeWaitUser()) {
			...
			return TODO_SUPER.listenSegment(segment);
		}
	},
	...
	listenSendingArgsAckResi: function(aCommClient,aParams, ist, rpc){
		...
		var callParams = rpc.xml ? jsxml.parse(rpc.xml) : rpc.json.params;
		// push errn,l,p,t,m
	...
	listenSendingArgsAckCall: function(aCommClient,aParams, ist, call){
		// Remove all params except action id
		aParams.splice(1, aParams.length - 1);		
		// Instanciate IST and push it in params
		ist = CvgIst.fromTarget(ist);
...
//		console.log("JSON:\n"+JSON.stringify(call.json,null,2));
		...
		for(var i=0;i<paramDefs.length;i++){
			var listResu = paramDefs[i].resu;
			var attributes = call.xml ? paramDefs[i].$ : paramDefs[i];
			var x3typ = CvgResuTag[attributes.typ];
			// add defs param i
			defs.push(parseInt(attributes.dim,10));//DIM
			defs.push(parseInt(attributes.nb));//NbVal
			defs.push(parseInt(attributes.size));//Size
			defs.push(x3typ + 256);//x3type
			//add value
			if (listResu){
				if (Array.isArray(listResu)) {
					for(var j=0;j<listResu.length;j++){
						var v = typeof listResu[j] === 'object' ? (call.xml ? listResu[j].$cdata : listResu[j]) : listResu[j];
						values.push(new CvgResu.fromX3Type(x3typ,v));	
					}
				}else{
					var v = typeof listResu === 'object' ? (call.xml ? listResu.$cdata : listResu) : listResu;
					values.push(new CvgResu.fromX3Type(x3typ,v));	
				}
			}
	   }
	...
	listenSendingArgs: function(body, aCommClient, aParams) {
		...
		case CvgAction.SRV_EXECI_RESPONSE:			
			if (body.param.reply && (body.param.reply.json || body.param.reply.xml)) this.listenSendingArgsAckResi(this, aParams, body.fld.ist, body.param.reply);
			else _postDiag(this,"SRV_EXECI_RESPONSE - Can't perform remote call", "Json or Xml description missing");
		case CvgAction.SRV_CALLREMOTEFUNC:
		case CvgAction.SRV_CALLREMOTESPGM:
			if (body.param.call && (body.param.call.json || body.param.call.xml)) this.listenSendingArgsAckCall(this, aParams, body.fld.ist, body.param.call);
			else _postDiag(this, "SRV_CALLREMOTESPGM - Can't perform remote call", "Json or Xml description missing");
			return ;
		default:
			break;
		}
	...
	setSrvImp:function (hostname, port){
		this.srvImp = {
			hostname: hostname,
			port: port
		}
