...
var CvgSessionOpen = require('syracuse-x3/lib/convergence/model/CvgSessionOpen').CvgSessionOpen;
...
var CFormatSetter = exports.CFormatSetter = helpers.defineClass(function(cli, aFormat,fmt) {
	CvgModifier.call(this);
	this.format = aFormat;
...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	listenNodeND_NAP: function(_, reader, node, txt) {
		new CvgSessionOpen(this.client,txt);
	},
	listenNodeND_REB: function(_, reader, node, str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11, str12, str13, str14, str15, str16, str17) {
		...
		if (this.client) this.client.closeSocket();
	},
	...
	listenNodeNDCHFM: function(_, reader, node, ist, fmt) {
		...
		var obj = this.client.traverseSettable(ist, setter);
	...
	listenNodeNDEXECI: function(_, reader,node,ist,spgm,dbgflg,paramDefs,paramVal) {
		var serverUrl = reader.ctx._cvgSession.sessInfo.serverUrl;
		new CvgInstanceExeci(_, ist,spgm,dbgflg,paramDefs[0],paramVal[0],this.client, serverUrl);
	...
	listenNodeNDOBOXI: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		...
		var replyAuto = this.silent || (this.reply.actionId === CONNECT_ACTION);
	...
	listenNodeNDOUVIMP: function(_, reader, node, properties) {
		...
		var self = this, printClient, srvId;
		...
		try{
			...
			this.setSrvImp(srvId.host, srvId.port);
			printClient = PrintClient.createClient(_, srvId.host, srvId.port, true);	
			printClient.launchReport(_, properties, true);
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		...
		try {
			...
			var cfg=this.client.printConfig && this.client.printConfig.server;
			var hostname = (edtSrvPart && edtSrvPart[0]) || (cfg ? cfg.hostname : "");
			var port = (edtSrvPart && edtSrvPart[1]) ||(cfg ? cfg.port : 0);
			this.setSrvImp(hostname, port);
			printClient = PrintClient.createClient(_, hostname, port);
			...
			console.log("NDGTIM ERROR: " + e.stack);
			throw e;
		}finally{
	...
	listenNodeNDRESULT: function(_, reader, node, ist, errn, errl, errp, errt, errm, resu, tblParams, tblResus) {
		...
		if(errn !== 0 &&  this.sendContext.connect ) {
			// add diagnosis 
			new CvgDiagnose(this.client, 'ERROR', 500, "errt["+errt+"] errp["+errp+"] errl["+errl+"] errn["+errn+"] errm["+errm+"]", null, null, "x3 runtime", true,true,'4GLDIAGNOSE');
		}else{
