...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, aParams, sendContext) {
	...
	listenActionArgsPICKING: function(aClient, aParams) {
		...
		for (var i in wLines){
			wArgIst.idx = wLines[i];
			var wPickedLine = new CvgPickedLine(wArgIst, wActionToPropagate);
			this.client.traverseSettable(wArgIst, new CLeftListPickingSelecter(wPickedLine.isSelection()));
	...
	listenNodeNDEXECI: function(_, reader,node,ist,spgm,dbgflg,paramDefs,paramVal) {
		var execi = new CvgInstanceExeci(ist,spgm,dbgflg,paramDefs[0],paramVal[0],this.client);
		this.client.processAction(_, execi.process());
	},
