...
var CDataSrcAffStateSetter = exports.CDataSrcAffStateSetter = helpers.defineClass(function(cli, aAftbTyp, aNbLines) {
	...
	setModifiable: function(aStructIst, aModifiable) {
		...
		if (this.done) {
			...
			//console.log(util.inspect(aModifiable));
			this.setLogicalErrMess(this.cli.getMessage('ERROR_NOT_RIGHT_MODIFIABLE', aModifiable.getAdxId(), 'CvgDataSrc'));
		}
		return this.done;
	}
});
...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	buildListenNodeMessage: function(aKeyMess, aValue1, aValue2, aModifier) {
		var wMess = aValue2 ? this.client.getMessage(aKeyMess, aValue1, aValue2) : this.client.getMessage(aKeyMess, aValue1);
	...
	listenActionArgsDOLEFTLISTLINE: function(aClient, aParams) {
		...
		aParams.splice(2, 1, wLeftListLine.cle);
	...
	listenActionArgsPICKING: function(aClient, aParams) {
		...
		function registerPickedLine(line) {
			var ist;
			if (typeof wArgIst === "string")
				ist = new CvgIst(wArgIst, line);
			else if (wArgIst instanceof CvgIst) 
				ist =  new CvgIst(wArgIst.ist, line);
			else
				throw new CvgException(self.client, null, "ARG_WRONG - need a 'CvgIst' or a 'String' !");

			var wPickedLine = new CvgPickedLine(ist, wActionToPropagate);
			//console.log(">>> Picked line IST: "+wPickedLine.ist.toString());
			self.client.traverseSettable(ist, new CLeftListPickingSelecter(wPickedLine.isSelection()));
			self.client.addAdxIstOfPickedLines(wPickedLine);
		}
		var self = this;
		/*
		...
		var wAction = parseInt(aParams[0], 10);
		var wArgIst = aParams[1];
		var wLines = aParams[2];
		var wActionToPropagate = aParams[3];

		if (Array.isArray(wLines)) {
			for (var i in wLines){
				registerPickedLine(wLines[i]);
			}
		} else {
			registerPickedLine(wLines);
		}
		aParams.splice(2, 2);
	},
	...
	listenNodeNDGIST: function(_, reader, node, wIst) {
		this.reply.setTargetAdxId(wIst, this);
	},
	...
	listenNodeND_ASKUISendAutoReply: function(_, aNumError, aExplaination) {
		...
		var wIstDatas = new CvgIstDatas(CvgResuTag["STRU"], wResponse.toString());
	...
	listenNodeNDAFTB: function(_, reader, node, wArgIst, wAffTbState, wNbLines) {
		...
		if (!wDataSrcAffStateSetter.done) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_AFFSTATE', CvgDataSrc.aftbTypToString(wAffTbState), null, wDataSrcAffStateSetter), "NDAFTB - " + wArgIst.toString());	
		}
	},
	...
	listenNodeNDCHFM: function(_, reader, node, wIst, wFormat) {
		...
		if (!wFormatSetter.done) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_SETTER_FORMAT', wFormat, null, wFormatSetter), "NDCHFM - " + wIst.toString());	
		}
	},
	...
	listenNodeNDCHGR: function(_, reader, node, wArgIst, wState) {
		...
		if (!wStateSetter.done) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_SETTER_STATE', CvgEntity.x3StateToString(wState), null, wStateSetter), 'NDCHGR - ' + wArgIst.toString());
		}
	},

	listenNodeNDCHVB: function(_, reader, node, wArgIst, wResus) {
		var wExtendResu = wResus[0];
		var wValueSetter = new CServerValueSetter(wExtendResu);
		this.client.traverseSettable(wArgIst, wValueSetter);
		if (!wValueSetter.done) {
			var wBlobSize = wExtendResu.value.length;
			_postDiag(this, this.buildListenNodeMessage('ERROR_SETTER_BLOB', wBlobSize, null, wValueSetter), 'NDCHVB - ' + wArgIst.toString());
		}
	},
	...
	listenNodeNDCHVL: function(_, reader, node, ist, resu) {
		...
		if (!wValueSetter.done) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_SETTER_VALUE', resu.toString(), null, wValueSetter), 'NDCHVL - ' + ist.toString());
		}
	},
	...
	listenNodeNDCSTL: function(_, reader, node, wArgIst, sty) {
		...
		if (!wStyleSetter.done) {
			_postDiag(this,this.buildListenNodeMessage('ERROR_SETTER_STYLE', sty, null, wStyleSetter), 'NDCSTL - ' + wArgIst.toString());
		}
	},
	...
	listenNodeNDDSLB: function(_, reader, node, wArgIst, value) {
		...
		if (v == null) {
			_postDiag(this, "Variable not found[" + wArgIst.toString() + "]", 'NDDSLB');
		}
	},
	...
	listenNodeNDGLAR: function(_, reader, node, wArgIst, wCle) {
		...
		if (!wLeftListLineSelecter.done) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_LLLINE_SELECTER', wCle.toString(), null, wLeftListLineSelecter), 'NDGLAR - ' + wArgIst.toString());
		}
	},
	...
	listenNodeNDINSLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcInserter.hasLogicalErrMess()) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_INSERTER', wNbLines.toString(), wTargetLine.toString(), wDataSrcInserter), 'NDINSLI - ' + wArgIst.toString());
		}
	},
	...
	listenNodeNDOBOXI: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		...
		var wReplyAuto = this.silent || (this.reply.actionId === CONNECT_ACTION) && (typMsg === MSG_STOP || this.client.isInteractiveSession);
		var wNewMess = new CvgMessage(typMsg, 0, msg, ttl, defBtn, wSleep, btnNum,this.client);
	...
	listenNodeNDOBOXISendAutoReply: function(_, aNumButton) {
		var resu = new CvgResu(CvgResuTdo["INT"], aNumButton);
	...
	listenNodeNDSATAN: function(_, reader, node, wArgIst) {
		...
		this.reply.setTargetAdxId(wArgIst,this);
	},
	 ...
	 */
	listenNodeNDOFLD: function(_, reader, node, wIst, wTxt1, wTxt2, wTxt3, wInt1, wTxt4, wTxt5) {
		wIst.screen = 34;
		wIst.bloc = 1;
		wIst.idx = -1;
		var wLine = [];
		wLine.push(CvgResuTag["STRU"], wTxt1);
		wLine.push(CvgResuTag["STRU"], wTxt2);
		wLine.push(CvgResuTag["STRU"], wTxt3);
		wLine.push(CvgResuTag["STRU"], parseInt(wInt1, 10));
		wLine.push(CvgResuTag["STRU"], wTxt4);
		wLine.push(CvgResuTag["STRU"], wTxt5);
		var wClf = [];
	...
	listenNodeNDOWIN: function(_, reader, node, wIst, wWindowsId, wFunctionId, wStamp, wExtendedInfos) {
		...
		wStackedWindow.setNDOWinComplementDescr(wExtendedInfos.toString('utf8'));
	},
	...
	listenNodeNDSUPLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			_postDiag(this, this.buildListenNodeMessage('ERROR_DELETER', wNbLines.toString(), wTargetLine.toString(), wDataSrcDeleter),'NDSUPLI' + wArgIst.toString());
		}
	},
	...
	listenSegment: function(aSegment) {
		...
		case CvgNode.ND_ASKUI:
			// AskUI_Call]
			wProcessed = listenNodeND_ASKUI(wSegmentNode);
			break;
		default:
		}
		if (wProcessed) {
