	...
	listenNodeND_IHM: function(_, reader, node, properties) {
		...
		try {
			if (properties) {
				...
				if (!this.client.cvgPlugin.ND_IHM) {
					this.client.cvgPlugin.ND_IHM = new CvgPlugin(_, this.client, localConfig.plugin && localConfig.plugin.killTimeoutOnCreate || 120000); // default 120s if it's not defined
				}
				ilog = this.client.cvgPlugin.ND_IHM.call(_, "ND_IHM", prop);
			}
		} finally {
			this.workingCtx.pendingBlockingNode = {
				"id": "ND_IHM",
