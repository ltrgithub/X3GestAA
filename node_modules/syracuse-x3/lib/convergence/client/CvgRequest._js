...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	listenActionArgsDOLEFTLISTLINE: function(aClient,aParams){
		if( aParams.length >= 3 ){
			...
			var ist = new CvgIst(aParams[1]); // retrieve instance as expect by x3
			if( aParams.length < 2 && typeof aParams[2] === "string" ){
				var key = aParams[2].split("^");
				var resus = [];
				for(var i=0;i<key.length;i++){
					var t = key.split("~");
					resus.push(new CvgResu(t[0],t[1]));
				}
