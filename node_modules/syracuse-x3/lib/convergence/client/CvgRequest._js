...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
		...
		if (aParams.length >= 3) {
			...
			if (typeof aParams[2] === "string") {
				...
				for (var i = 0; i < key.length; i++) {
					var t = key[i].split("~");
					var resu = new CvgResu.fromAction(parseInt(t[0], 10), t[1]);
					resus.push(resu);
				}
	...
	listenNodeNDOUVIMP: function(_, reader, node, properties) {
		...
		var self = this,
			jobRank, printClient, srvId, report;
		if (properties['+STATE'] && properties['+STATE'] === "LAUNCHED") {
			try {
				...
				report = new CvgReportSubmitted(this.client, props);
				return [0, 1];
			} catch (e) {
			...
			try {
				...
				report = new CvgReportSubmitted(this.client, {
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		...
		try {
			...
			var diag = new CvgDiagnose(this.client, 'INFO', 99, locale.format(module, "LISTEN_NDGTIM_1"), '"' + srvId.host + ":" + srvId.port + '" : ' + (e.message || ""), e, "Protocol [listenNodeNDGTIM]", null, false);
