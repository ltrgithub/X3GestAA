...
var CServerFormatSetter = exports.CServerFormatSetter = helpers.defineClass(function(cli, aFormat) {
	...
	setModifiable: function(ist, obj) {
		this.inferredState = obj.setServerFormat(new CvgFormat(this.format));
		this.done = true;
		return this.done;
	}
});
...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	loadWindowDescription: function(_, winId) {
		...
		function getWindowXml(_, winId) {
			...
			var opt = {
				method: "GET",
				url: adxHttpPub + "/" + adxFolder + "/GEN/" + lang + "/FENS/" + winId + ".xml",
				headers: {}
			}
	...
	listenNodeNDCHFM: function(_, reader, node, ist, fmt) {
		var setter = new CServerFormatSetter(this.client, fmt);
		this.client.traverseSettable(ist, setter);
		if (setter.hasInferredState() && ist.isCellIst()) {
			// pour designer la colonne
			ist.idx = -1;
			var setter = new CEntityStateSetter(setter.inferredState);
			this.client.traverseSettable(ist, setter);
		}
		if (!setter.done) {
	...
	listenNodeNDCHGR: function(_, reader, node, ist, state) {
		...
		var setter = new CServerStateSetter(CvgEntity.x3State2EntityState(state));
		this.client.traverseSettable(ist, setter);
		if (!setter.done) {
	...
	listenNodeNDOMENA: function(_, reader, node, profil) {
		...
		if (this.client.appSrvSessionSettings.usrProfile !== profil) this.client.appSrvSessionSettings.usrProfile = profil;
	},
	/**
