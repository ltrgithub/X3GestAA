...
exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	listenNodeND_IHM: function(_, reader, node, properties) {
		...
		try {
			if (properties) {
				...
				this.client.cvgPlugin.ND_IHM = this.client.cvgPlugin.ND_IHM || {};
				var plug;
				if (!this.client.cvgPlugin.ND_IHM["ilog"]) {
					plug = this.client.cvgPlugin.ND_IHM["ilog"] = new CvgPlugin(_, this.client, this.client.killPluginTimoeut || 20000); // default 120s if it's not defined
				} else {
					plug = this.client.cvgPlugin.ND_IHM["ilog"];
				}
				ilog = plug.call(_, "ND_IHM", prop);
				if (ilog.method === "Update") {
					console.log("nd_ihm _update");
					// stop the plugin tracker but not the client one
					this.client.orchestraTracker = this.client.cvgPlugin.ND_IHM.ilog;

				} else if (ilog.method === "Delete") {
					// stop tracker plugin and client we apply the stop directly here
					var stopTrackerClient = this.client.stopTracker;
					this.client.orchestraTracker = [this.client.cvgPlugin.ND_IHM.ilog, this.client];
	...
	listenNodeNDEXECI: function(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal) {
		...
		// TODO change observer in case of ilog NDEXECIs
