...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	listenActionArgsDOLEFTLISTLINE: function(aClient, aParams) {
		if (aParams.length >= 3) {
			...
			var ist = aParams[1];
			if (typeof ist === "string") ist = new CvgIst(aParams[1]);
			else if (!(ist instanceof CvgIst)) throw new CvgException(this.client, null, "ARG_WRONG - need a 'CvgIst' or a 'String' !");
	...
	listenActionArgsPICKING: function(aClient, aParams) {
		...
		function registerPickedLine(key) {
			if (typeof ist === "string") ist = new CvgIst(ist);
			else if (ist instanceof CvgIst) ist = new CvgIst(ist.ist);
			else throw new CvgException(self.client, null, "ARG_WRONG - need a 'CvgIst' or a 'String' !");
	...
	listenNodeNDEXECI: function(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal) {
		...
		if (!serverUrl)
		{
			serverUrl = this.client && this.client._cvgSession && this.client._cvgSession.sessInfo && this.client._cvgSession.sessInfo.serverUrl;
		}
	...
	listenNodeNDOFLD: function(_, reader, node, ist, txt1, txt2, txt3, int1, txt4, txt5, target) {
		...
		var ist = new CvgIst(ist.winAlphaId + ",AH," + list);
		//ist.screen = 34;
		//ist.bloc = 1;
		//ist.idx = -1;
		var line = [];
		...
		var r4 = new CvgResu(CvgResuTag["STRU"], parseInt(int1, 10));
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		...
		try {
			...
			for (var i in selPr) {
				...
				if (selPr[i].printerName === selected) selectedIdx = i;
	...
	listenSendingArgsAckResi: function(aCommClient, aParams, ist, rpc) {
		...
		// push return 
		var returnVal = rpc.xml ? jsxml.parse(rpc.xml) : rpc.json.
		return;
