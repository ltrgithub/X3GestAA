...
exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, sendContext) {
	...
	listenNodeNDCHVB: function(_, reader, node, ist, resus) {
		...
		if (this.client.boClient.variable && !this.client.boClient.token) {
			try {
				...
				this.client.tracer.enabled && this.client.tracer.trace(_, e.stack, traceLog);
	...
	listenNodeNDEXECIAutoReply: function(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal) {
		// create a body error type
		// TODO replace by alog
		console.log("autoReply EXECI, client uuid=[" + this.client.uuid + "]");
		...
		var params = [];
		this.listenSendingArgs(body, this.client, params);
		x3writer.sendAction(_, this.client, new CvgResu(CvgResuTdo.NULL, null), null, params);

	},
	listenNodeNDEXECIStd: function(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal) {
		var self = this;
	...
	listenNodeNDEXECI: function(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal) {
		if (this.client && this.client.askKill) {
			// auto reply
			this.listenNodeNDEXECIAutoReply(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal);
		} else {
			return this.listenNodeNDEXECIStd(_, reader, node, ist, spgm, dbgflg, paramDefs, paramVal);
		}
	},
	 ...
	 */
	listenNodeNDOBOXIStd: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		if (typBox === OU_BIF) {
			var p = msg.indexOf("$$");
	...
	listenNodeNDOBOXIAutoReply: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		console.log("autoReply NDBOXI, client uuid=[" + this.client.uuid + "]");

		this.listenNodeNDOBOXIStd(_, reader, node, typBox, btnNum, msg, ttl, wSleep);
		// réponse automatique à tous les message si "SRV_CALLREMOTESPGM" (ACK_CALL) ou open connexion
		switch (typBox) { //TODO must replace by a map
			/* Boite de Question */
			case OU_BQU:
				typMsg = MSG_QUESTION;
				defBtn = 2;
		...
		if (btnNum !== 0 && btnNum !== defBtn) defBtn = btnNum;
		this.listenNodeNDOBOXISendAutoReply(_, defBtn);


	},

	listenNodeNDOBOXI: function(_, reader, node, typBox, btnNum, msg, ttl, wSleep) {
		if (this.client && this.client.askKll) {
			this.listenNodeNDOBOXIAutoReply(_, reader, node, typBox, btnNum, msg, ttl, wSleep);
		} else {
			this.listenNodeNDOBOXIStd(_, reader, node, typBox, btnNum, msg, ttl, wSleep);
		}
	},
	...
	listenNodeNDOFLD: function(_, reader, node, ist, txt1, txt2, txt3, int1, txt4, txt5, target) {
		...
		if (!done) {
			this.client.tracer.enabled && this.client.tracer.trace(_, "line not appened :  NDOFLD to list " + ist.toJSON(), traceLog);
		}
	},
	...
	listenNodeNDGTIMAutoReply: function(_, reader, node, properties) {
		console.log("autoReply EXECI, client uuid=[" + this.client.uuid + "]");

	},

	listenNodeNDGTIMStd: function(_, reader, node, properties) {
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		if (this.client && this.client.askKill) {
			this.listenNodeNDGTIMAutoReply(_, reader, node, properties);
		} else {
			this.listenNodeNDGTIMStd(_, reader, node, properties);
		}
	},
