...
/* TODO overload
	...
	listenActionArgsBRWRAPIDESEL: function(aClient, aParams) {
		...
		while (wI < wNbArgs) {
			...
			wTabOfResus.push(new CvgResu(CvgResu.TDO_ALPHA, wColValue));
			wI++;
		}
		wI = wNbArgs - 1;
		while (wI >= 2) {
	...
	listenNodeND_ASKSendAutoReply: function(_, aNumError, aExplaination) {
		...
		var wIstDatas = new CvgIstDatas(CvgResu.TAG_STRU, wResponse.toString());
	...
	listenNodeNDCHVB: function(_, reader, node, wArgIst, wResus) {
		var wExtendResu = wResus[0];//.toString('utf8');
	...
	listenNodeNDOFLD: function(_, reader, node, wIst, wTxt1, wTxt2, wTxt3, wInt1, wTxt4, wTxt5) {
		...
		wIst.idx = -1;
		var wLine = [];
		wLine.push(CvgResu.TAG_STRU, wTxt1);
		wLine.push(CvgResu.TAG_STRU, wTxt2);
		wLine.push(CvgResu.TAG_STRU, wTxt3);
		wLine.push(CvgResu.TAG_STRU, parseInt(wInt1, 10));
		wLine.push(CvgResu.TAG_STRU, wTxt4);
		wLine.push(CvgResu.TAG_STRU, wTxt5);
		var wClf = [];
	...
	// "ist" "l'instance ou se positionner" "ist"
	// "wErrn (short)" "la variable errn valorisée par le moteur " "errn"
	// "wErrl (short)" "contient la variable errl valorisée par le moteur" "errl"
	// "wErrp (string)"
	// "id du traitement dans la variable errp valorisée par le moteur"
	// "errp"
	// "wErrt (car)" "contient  la variable errtyp valorisée par le moteur"
	// "messtyp"
	// "wErrm (string)" "contient  la variable errm valorisée par moteur" "mess"
	listenNodeNDRESULT: function(_, reader, node, ist, wErrn, wErrl, wErrp, wErrt, wErrm, wResus, wTblParams, wTblResus) {

		this.setTargetAdxId(ist);

		// TODO

//
//		// 16w_115 - Fiche 54186 - evolution du protocol X3 ack_call / NDRESULT
//		CAdonixSpgmResult wSpgmResult = new CAdonixSpgmResult(pTracer, wErrn, wErrl, wErrp, wErrt, wErrm, wResu, wTblParam, wTblResu);
//		// CAdonixSpgmResult wSpgmResult = new CAdonixSpgmResult(pTracer,wErrn,
//		// wErrl, wErrp, wErrt, wErrm, wResu, wTblParam);
//
//		pAdonixReply.setSpgmResult(wSpgmResult);
	...
	listenSendingArgsAckCall: function(aCommClient,aParams, ist, xml){
		ist = CvgIst.fromTarget(ist);
		aParams.push(ist);
		var jsCall = jsxml.parse(xml);
	   // name of subprog
	   aParams.push(jsCall.spgm.$.name);// push name of su.program
	   // debug flag 
	   var flag = parseInt(jsCall.spgm.$.trace ,10)<< 3 | parseInt(jsCall.spgm.$.execduration,10) << 2 | parseInt(jsCall.spgm.$.totalduration,10) << 1 | parseInt(jsCall.spgm.$.exceptiondetail,10)			
	   ...
	   var paramDefs = jsCall.spgm.param;
	   for(var i=0;i<paramDefs.length;i++){
			var listResu = paramDefs[i].resu;
			var x3typ = CvgResu.getTypeFromString(paramDefs[i].$.typ);
			// add defs param i
			defs.push(parseInt(paramDefs[i].$.dim,10));//DIM
			defs.push(parseInt(paramDefs[i].$.nb));//NbVal
			defs.push(parseInt(paramDefs[i].$.size));//Size
			defs.push(x3typ + 256);//x3type
			//add value
			if (listResu){
				if (Array.isArray(listResu)) {
					for(var j=0;j<listResu.length;j++){
						values.push(new CvgResu.fromX3Type(x3typ,listResu[j]));	
					}
				}else{
					values.push(new CvgResu.fromX3Type(x3typ,listResu));	
				}
			}
	   }
	   // tab def
	...
	listenSendingArgs: function(body, aCommClient, aParams) {
		...
		case CvgAction.SRV_CALLREMOTESPGM:
			if (body.param.call && body.param.call.xml)
				this.listenSendingArgsAckCall(this, aParams, body.fld.ist, body.param.call.xml);
			else
				this.reply.addLogicalErrorMess(this, 'SRV_CALLREMOTESPGM', "Can't perform AckCall", "Xml description missing");
			return ;
		default:
			break;
		}
