...
var CvgRequest = exports.CvgRequest = helpers.defineClass(function(aClient, aAdonixReply, aParams, sendContext) {
	...
	listenNodeND_ASKUI: function(_, reader, node) {
		...
		new CvgDiagnose(this.client, 'WARNING', 99, "ASKUI node not supported", "ND_ASKUI", null, "Protocol" , null, true);
	},
	...
	listenNodeNDINED: function(_, reader, node) {
		...
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "NDINED node not supported", "NDINED", null, "Protocol" , null, true);
	},

	listenNodeND_INT: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "ND_INT node not supported", "ND_INT", null, "Protocol" , null, true);
	...
	listenNodeNDSYSTEM: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "NDSYSTEM node not supported", "NDSYSTEM", null, "Protocol" , null, true);
		return [0];
	},
	
	listenNodeNDEDBK: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "NDEDBK node not supported", "NDEDBK", null, "Protocol" , null, true);
		return [0, 0];
	},
	
	listenNodeND_OCX: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "ND_OCX node not supported", "ND_OCX", null, "Protocol" , null, true);
		return [0, 0, 0, 0, 0];
	},
	listenNodeND_IHM: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "ND_IHM node not supported", "ND_IHM", null, "Protocol" , null, true);
		return [0, 0, 0, 0, 0];
	},
	listenNodeNDGTDT: function(_, reader, node) {
		// TODO: translate message
		new CvgDiagnose(this.client, 'WARNING', 99, "NDGTDT node not supported", "NDGTDT", null, "Protocol" , null, true);
		return [0, 0, 0, 0, 0];
	},
	...
	listenNodeNDAFTB: function(_, reader, node, wArgIst, wAffTbState, wNbLines) {
		...
		if (!wDataSrcAffStateSetter.done) {
			...
			var wMessLE = this.buildListenNodeMessage('ERROR_AFFSTATE', wState, null, wDataSrcAffStateSetter);
			this.reply.addLogicalErrorMess(this, "lib=\"NDAFTB\" x3func=\"Grid_setRedraw\"", wArgIst.toString(), wMessLE);
	...
	listenNodeNDCHFM: function(_, reader, node, wArgIst, wFormat) {
		...
		if (!wFormatSetter.done) {
			var wMessLE = this.buildListenNodeMessage('ERROR_SETTER_FORMAT', wFormat, null, wFormatSetter);
	...
	listenNodeNDCHGR: function(_, reader, node, wArgIst, wState) {
		...
		if (!wStateSetter.done) {
			this.reply.addLogicalErrorMess(this, 'NDCHGR', wArgIst.toString(), buildListenNodeMessage('ERROR_SETTER_STATE', CvgEntity.x3StateToString(wState), null, wStateSetter));
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDCHVB: function(_, reader, node, wArgIst, wResus) {
		...
		if (!wValueSetter.done) {
			...
			var wMessLE = this.buildListenNodeMessage('ERROR_SETTER_BLOB', wBlobSize, null, wValueSetter);
	...
	listenNodeNDCHVL: function(_, reader, node, ist, resu) {
		...
		if (!wValueSetter.done) {
			var wMessLE = this.buildListenNodeMessage('ERROR_SETTER_VALUE', resu.toString(), null, wValueSetter);
	...
	listenNodeNDCSTL: function(_, reader, node, wArgIst, sty) {
		...
		if (!wStyleSetter.done) {
			var wMessLE = this.buildListenNodeMessage('ERROR_SETTER_STYLE', sty, null, wStyleSetter);
	...
	listenNodeNDGLAR: function(_, reader, node, wArgIst, wCle) {
		...
		if (!wLeftListLineSelecter.done) {
			var wMessLE = this.buildListenNodeMessage('ERROR_LLLINE_SELECTER', wCle.toString(), null, wLeftListLineSelecter);
	...
	listenNodeNDINSLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcInserter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage('ERROR_INSERTER', wNbLines.toString(), wTargetLine.toString(), wDataSrcInserter);
		...
		case OU_BEN:
			{
				wTypeMess = MSG_STOP;
				wDefaultButton = 0;
				// Who is closing syracuse session ?
//				if (this.client.connecting)
//					this.client.closeClient(_, null, 3);
				break;
			}
	...
	listenNodeNDOUVIMP: function(_, reader, node, properties) {
		...
		if (properties.STATE && properties.STATE === "LAUNCHED") {
			...
			try{
				...
				var seqId = printClient.launchPrint(_, properties);
	...
	listenNodeNDGTIM: function(_, reader, node, properties) {
		...
		try {
			...
			if (printClient) printClient._disconnect(_);
			return;
		}
	...
	listenNodeNDSUPLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage('ERROR_DELETER', wNbLines.toString(), wTargetLine.toString(), wDataSrcDeleter);
...
//		console.log("JSON:\n"+JSON.stringify(call.json,null,2));
	...
	listenSendingArgs: function(body, aCommClient, aParams) {
		...
		case CvgAction.SRV_EXECI_RESPONSE:
			...
			if (body.param.reply && (body.param.reply.json || body.param.reply.xml)){
				...
				this.reply.addLogicalErrorMess(this, body.act, "Can't perform remote call", "Json or Xml description missing");
			}
		case CvgAction.SRV_CALLREMOTEFUNC:
		case CvgAction.SRV_CALLREMOTESPGM:
			if (body.param.call && (body.param.call.json || body.param.call.xml))
				this.listenSendingArgsAckCall(this, aParams, body.fld.ist, body.param.call);
			else
				this.reply.addLogicalErrorMess(this, body.act, "Can't perform remote call", "Json or Xml description missing");
			return ;
		default:
	...
	setTargetAdxId: function(ist) {
		if (this.reply /*instanceof CvgReply*/ ) {
			...
			reply.status = this.reply.status;
