...
var CvgReply = exports.CvgReply = helpers.defineClass(function(client, actionId) {
	...
	setTargetAdxId: function(ist, request) {
		...
		if (request != null){		
			...
			try {
				if (isVar && target.acceptEditFormat() && !this.client.clientConfig.fmtLocalFormat) {				
					// No value returned if data formatted by client (fmtLocalFormat) 	
					if (target.format == null || !target.format.isValid()) throw new Error (this.client.getMessage('ERROR_CURFORMAT_NOFORMAT', ist.toString()));
					fmt = target.format.format;
					fmtValue = target.formatX3ToEdit();
					srvValue = target.getServerValStr();
				}
