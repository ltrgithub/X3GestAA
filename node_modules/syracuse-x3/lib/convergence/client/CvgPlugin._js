...
exports.CvgPlugin = helpers.defineClass(function(_, client, id, killTimeoutOnCreate) {
	...
	processRequest: function(_, context, bodyStr, tracker) {
		tracerAction.debug && tracerAction.debug("process Plugin request " + JSON.stringify(bodyStr));

		tracerAction.debug && tracerAction.debug("tracker " + !tracker);
		...
		switch (specialAction.id) {
			...
			case "DELETE":
				...
				this.writeAckIlogReply(_, body && body.param && body.param.plugin);
				this.createDeleteTargetPlugin(_);
				this.stopTracker(_, context, this.client); // stop tracker of the client and target plugin
	...
	call: function(_, nodeName, property) {
		tracerAction.debug && tracerAction.debug("treat node " + nodeName + " with property " + JSON.stringify(property));
	...
	clientReplyPlugin: function(_, context, body) {
	...
	_writeRegisterIlogReply: function(_, context) {
		...
		if (this.noTimeoutApply) { // just stop tracker to notify client
			...
			this.stopTracker && this.stopTracker(_, context, this.client);
	...
	writeAckIlogReply: function(_, plugin) {
	...
	_processPluginIlogNew: function(_) {
		...
		this.timeout = flows.setTimeout(function(_) {
			...
			self.stopTracker && self.stopTracker(_, self.context, self.client);
		}, this.killTimeout);
		CvgPluginTarget.create(this.client, this); // notify only classic page client

	},
	_processPluginIlogUpdate: function(_) {
		tracerAction.debug && tracerAction.debug("create plugin target and force update ");
	...
	_readPluginND_IHM: function(_, property) {
		...
		tracerAction.debug && tracerAction.debug("analyse ND_IHM node");
