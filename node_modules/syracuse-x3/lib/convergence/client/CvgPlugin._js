	...
	processRequest: function(_, context, bodyStr, tracker) {
		...
		switch (specialAction.id) {
			...
			case "DELETE":
				...
				this.stopTracker && this.stopTracker(_, context, this.client.orchestraTracker); // stop tracker of the client and target plugin
				break;
			case "PLUGIN_ACK":
				this.writeAckIlogReply(_, body && body.param && body.param.plugin);
			...
			param: {
				...
				reply: {
					json: {
						...
						"return": {
							typ: "INT",
							resu: context.request.headers["content-length"] || 0
						},
						params: [{
							typ: "BLOB",
							num: 0,
