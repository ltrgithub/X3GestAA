...
exports.CvgPlugin = helpers.defineClass(function(_, client, killTimeoutOnCreate) {
	...
	razModification: function() {
		...
		this.download = null;
		this.upload = null;
		this.target && this.target.razModifications();
	},

	getPluginTarget: function() {
		return this.target || CvgPluginTarget.create(this, this);
	},
	...
	processRequest: function(_, context, bodyStr, tracker) {
		...
		switch (specialAction.id) {
			...
			case "PLUGIN_ACK":
				...
				break;
			case "PLUGIN_ILOG_UPLOAD":
				// TOOD parse element to generate response to the runtime as ACK_RESI
				var binary = new Buffer(bodyStr);
				var req = {
				...
				this.client.processAsyncAction(_, req);
				automata.process(_, this.client, this.client.activeRequest, {
					title: "special",
					detail: "PLUGIN_ILOG_UPLOAD"
				});
				break;
			default:
	...
	_processPluginIlogUpdate: function(_) {
		this.target = this.getPluginTarget(); // notify only plugin client
	},
	_processPluginIlogDelete: function(_) {
		this.target = CvgPluginTarget.create(this, this); // notify plugin client
