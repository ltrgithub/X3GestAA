...
exports.CvgPlugin = helpers.defineClass(function(_, client, killTimeoutOnCreate) {
		...
		switch (specialAction.id) {
			case "REGISTER": // notif that the plugin ilog client is launched and operational
				...
				if (body.$diagnose && body.$diagnoses.length) {
					...
					replyRegister = body && body.param && body.param.plugin;
	...
	_writeRegisterIlogReply: function(_, replyRegister) {
		// compare reply Register and reply client

		if (this.notifPlugin && this.replyClient && !this.noTimeoutApply) {
			var prop = {};
			var reply = this.replyClient;
			if (replyRegister.name !== this.replyClient.name) {
				reply = replyRegister;
			}
	...
	_writeAckIlogReply: function(_, prop) {
		if (prop && prop.return &&prop.return .property) {
			var prop = {};
			prop[prop.return .property.name] = prop.return .property.value;
			x3writer.props1Serializer.write(_, this.client, prop);
		}
	},
	...
	sendIlogErrorToX3: function(_, client, ilog, errorMess, errorCode) {
		...
		// notify tracker to write an error response to the plugin client
		this.stopTracker && this.stopTracker(_, this.context, false); // exception error need to stop tracker
