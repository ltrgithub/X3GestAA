...
var _mgr = exports.recordMgr = new function() {
	 ...
	 function _init(_){
			...
			try{
				if(exists(_, _self._dataDir)){
					...
					_tracer("cvgRecordMgr _init OK - path :" + _self._dumpDir);						
				}else{
					throw "cvgRecordMgr data directory not found\n" + _self._dataDir;
				}
			} catch(e) {
				logerr(e, "cvgRecordMgr._init - ERROR - Recorder/Player are disabled");	
				throw e;
			}
	 }
	...
	this._createFile = function(_, opts){
		...
		try{
			...
			_tracer("cvgRecordMgr._createFile :" + path);
			return path;
		}catch(e){
			logerr(e, "cvgRecordMgr._createFile - ERROR  - File '" + fileName + ".js'");
			fileName =null;
		}
	}
	...
	this.newRecorder = function(_, stream, opts){
		try{
			_initFuture(_);
			var _recClass=require('./CvgRecorder').CvgRecorder;
			return new _recClass(_, stream, _self._createFile (_, opts), _config);
		}catch(e){
			logerr(e, "cvgRecordMgr - ERROR - Can't create recorder");
		}
		return null;
	}
	...
	this.cvgRecordPlay = function(_, filename, context){
		try{
			...
			var path=this._getFullPath(filename);
			if (exists(_, path)){
				var _playClass=require('./CvgPlayer').CvgPlayer;
