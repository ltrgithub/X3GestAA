...
function _readFileData (_, path, readContent){
	var stats=ufs.stat(path, _), info= null, fd = null, content = null;
	try {
		...
		if (pos==-1) throw "First line not found";
		line = line.substring(0, pos);
		info = JSON.parse(line);
		if (info.type == "RECINFO") return { stats:stats, info: info.data, content:content };
		else throw "Unexpected first line - First line doesn't contain record info (type:'RECINFO'";
	}finally{
...
var _mgr = exports.recordMgr = new function() {
		...
		try{
			...
			if (exists(_, path)){
				if(fileName==null || opts.overwrite){
					...
					var r=new RegExp(fileName + "_([\\d]+).json"), max=-1;
	...
	// create a recorder
	this.newRecorder = function(_, stream, opts, cliCfg){
		try{
			_initFuture(_);
			// Record client config in order to restore them in player
			opts.cliCfg = {};
	...
	this.cvgRecordGet=function(_, fileName, readContent){
		...
		try{
			...
			return  {
				...
				x3host : data.info.x3Host || "",
				x3port : data.info.x3Port != null ? data.info.x3Port.toString() : "",
				x3solution : data.info.x3Solution || "",
				x3folder : data.info.x3Folder || "",
				x3user : data.info.x3User || "",
				x3lang : data.info.x3Lang || "",
	...
	this.cvgRecordPlay = function(_, filename, context){
		try{
			...
			if (exists(_, path)){
				...
				var player = new klass(_, path, _readJson(_, path), _config);
