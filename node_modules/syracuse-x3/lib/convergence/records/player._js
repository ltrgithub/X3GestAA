...
exports.player = helpers.defineClass(function(_, filePath, json, config) {
	...
	_processJson: function(_, json){
		...
		var lineNum = 0;
		try {
			var step = null, newEndPoint = null, curEndPoint;
			self._steps = [];
					...
					if (epCrit){
						...
						if (!ep){
							...
							if (ep ==  null || ep.length == 0) {
								throw "Endpoint " +  curEndPoint + " not found in database and no '"+ epCrit.contract + "/" + epCrit.application + "' endpoint has been found in current database";
							}
							newEndPoint = ep[0].dataset(_);
							self._log("New EndPoint found -> " +  newEndPoint)
						}else {self._log("Endpoint " +  e.data.endpoint.dataset + " found in database")}
					}
