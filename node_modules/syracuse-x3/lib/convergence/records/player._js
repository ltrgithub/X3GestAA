...
exports.player = helpers.defineClass(function(_, filePath, config) {
	...
	start:function(_, context){
		var self=this, ex=null, x3Func="", x3Act = "", respHttp = null, connectReq = null;
		self._curStep=null;
		self._curStepIdx=0;
		try{
			...
			try{
				...
				if (ex!=null){
					...
					var message="Error playing X3 function " + x3Func + " - file[" + self.fileName + "]";
					if (self._curStep) {
	...
	_processFile: function(_){
		var self = this;
		try {
			var json = ufs.readFile(self.filePath, 'utf8', _) || "";
			json = json.trim();
			if (json.length == 0) throw "Empty file"
			if (json[json.length-1] == ',') json =  json.substring(0, json.length-1);
			json = "[" + json + "]";
		}catch(e){
			throw "Error reading record file [" + self.filePath + "]\n" + (e.message ? e.message : e)
		}
		try {
			json = JSON.parse(json);
		}catch(e){
			throw "Error parsing record file [" + self.filePath + "]\n" + (e.message ? e.message : e)
		}
		var lineNum = 0;
		try {
			var step = null;
