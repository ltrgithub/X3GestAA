...
var _report=function(){
		...
		this.toJSON=function(){
			...
			if (this.recInfo){
				msgs.push("X3 folder : " + this.recInfo.x3Solution + " / " +  this.recInfo.x3Folder);
				msgs.push("X3 host : " + this.recInfo.x3Host + " / " + this.recInfo.x3Port);
				msgs.push("X3 user : " + this.recInfo.x3User + " / " + this.recInfo.x3Lang);
			}
			...
			for (var i in this.times){
				...
				if(!hash[step]) hash[step] = {n:0, time:0};
				hash[step].n++;
			...
			return {
			    "$diagnoses": [{
			            severity: "info",
			            message: ("<b><red>Test succeeded</b><br>" + indent + msgs.join("<br>" + indent))
			     }]
			}					
		}	
}
exports.player = helpers.defineClass(function(_, filePath, json, config) {
	...
	this._bytesInfo=[];
	this._closed= false;
	this._report= new _report();
	this.x3Func = "";
	this._processJson(_, json);
}, null, {
	start:function(_, context){
		var self=this, ex=null, x3Act = "", respHttp = null, connectReq = null;
		self._curStep=null;
		self._curStepIdx=0;
		try{
			...
			self._steps.forEach_(_, function(_, e){
				...
				self._report.times.push({step: e.connect ? "Connect" : "Act" + x3Act, time: new Date().getTime() - startTime});
	...
	_sendHttpReq:function(_, req){
		...
		if (self._httpCookie){
			...
			var usr = self._config.user;
			if (usr==null || usr.length==0) throw "No user found in config file";
			self._log("\tNo cookie -> headers.Authorization -> user=" +  usr);
			options.headers.Authorization = 'Basic ' + new Buffer(usr + ":" + self._config.password).toString('base64') 
		}
	...
	_processJson: function(_, json){
		...
		try {
			...
			throw "Error processing record file [" + self.fileName + "] - Line[" + lineNum + "]\n" + (e.message ? e.message : e) + "\nfullPath" + self.filePath;
		}		
	},
	_log:function(txt){
