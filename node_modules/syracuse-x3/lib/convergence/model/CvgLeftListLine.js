...
var CvgLeftListLine = exports.CvgLeftListLine = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, localIdx) {
	...
	setColumns: function(aIdx, aVal, aClf) {
		...
		while (wI < aVal.length && wI < this.vars.length) {
			...
			if (resu.isDate() && !(v instanceof CvgVariableDate)) {
				var newVar = new CvgVariableDate(this, v.entityIndexBaseOne, v.name, v.id, v.dataType, null, v.state);
			...
			if (wI < aVal.length && this.dataSrc.entrySup.vars[wI].dataType !== aVal[wI].type) {
	...
	toJSON: function() {
		...
		var mykey = "";
		for (var i = 0; this.cle && i < this.cle.length; i++) {
			if (i > 0) {
				mykey += "^";
			}
		...
		for (var i = 0; this.realListelem && i < this.realListelem.length; i++) {
