...
var CvgInstanceExeci = exports.CvgInstanceExeci = helpers.defineClass(function(_, ist,spgm,dbgflag,paramDefs,paramVal, client) {
	...
	process: function(_) {
		if (this.spgm === "PROXY_DISPATCH"){
			// exec js 
			var header = JSON.parse(this.values[0].value);
			console.log("value 1 "+JSON.stringify(this.values[1].value,null,2));
			var body = JSON.parse(this.values[1].value);
			...
			try {
	            ...
	            // Http Request
	                ...
	                if (header.method != null) {
	                     if (header.url.indexOf('http://') == 0 || header.url.indexOf('https://') == 0) {
	                         ...
	                         tracer && tracer("Header=" + JSON.stringify(result.header, null, 2));
	                     } else {
	         ...
	         return this.genExeciNode(_, new CvgResu(CvgResuTdo["CLOB"],result), errl, errm, errn, errp, errt);
		}else{
			// construction of XML 
			// id 
			this.xml = "<execi id=\""+this.spgm+"\" ist=\""+this.instance+"\" exceptiondetail=\""+this.exceptionDet+"\" totalduration=\""+this.totalDuration+"\" execduration=\""+this.execDuration+"\" trace=\""+this.trace+"\"> ";
			// params 
			var j =0;
			var i = 0;
