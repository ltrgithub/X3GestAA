...
var CvgExcelExport = require('syracuse-x3/lib/convergence/model/CvgExcelExport').CvgExcelExport;
var jsRunner = require('syracuse-x3/lib/jsRunner');

var CvgInstanceExeci = exports.CvgInstanceExeci = helpers.defineClass(function(_, ist, spgm, dbgflag, paramDefs, paramVal, client, serverUrl) {
			...
			while (i < this.defs.length) {
				...
				if (nb != 0) { // if there is some resu
					...
					if (this.spgm === "setDataTableExtractToExcel") {
						try {
							if (item.resu.length !== 1) throw new Error("Only one resu can be accepted by Excel export.");
							...
							return this.genExeciNode(_, {
								typ: "NULL",
								resu: null
							}, 1, e.message, 1, "CvgInstanceExeci._js", 2, e);
						}
					}
				}
