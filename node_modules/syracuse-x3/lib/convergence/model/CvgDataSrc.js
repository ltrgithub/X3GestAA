				...
				if( item < stamp )
					...
					if( metadata === 'STY' ){
						if( map[item].style !== this.style ){
							if( !(map[item] instanceof CvgDataSrcLine) 
							    && !map[item].isEntrySup
								&&( map[item].style === map[item].srcLine.style 
								|| map[item].style == this.entrySup.vars[i].style) ){
								delete map[item];
							}
						}
					}
	...
	optimizeMetaStt: function(map){
		...
		for(var i=0;i< map.length;i++){
			...
			if( map[i] instanceof CvgDataSrc ){// if it's data source we need to add 
				...
				if( map[i] instanceof CvgDataSrcLine 
					|| (map[i].isEntrySup) ){// if it's column or line we add only 
					mapStt[stamp]= map[i];
				}else{// if state is the same as column or lines 
					if(i<this.entrySup.vars.length &&  state !== this.entrySup.vars[i].state 
						|| state !== map[i].srcLine.state ){
							mapStt[stamp]= map[i];
					}
				}
			}
		...
		if (this.isBlocGrid()) {
			...
			for (var i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					this.manageMetadata(mapPCol, line, 'PCOL', lastLine);
			...
			if (!emptyJson(stypCol)) {// add all pCol as style
				if( metadata.sty && metadata.sty.length )
					metadata.sty.pushValues(stypCol);
				else
