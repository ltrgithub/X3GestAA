...
// modules that we load on demand (probably to avoid pb with circular dependencies)
...
var CvgDataSrc = exports.CvgDataSrc = helpers.defineClass(function(aRequester, aScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	...
	getJsonMetadata: function(map, metadata) {
		// not sure that these requires are necessary - but keep them in case...
		CvgDataSrcLine = CvgDataSrcLine || require('syracuse-x3/lib/convergence/model/CvgDataSrcLine').CvgDataSrcLine;
		CvgVariable = CvgVariable || require('syracuse-x3/lib/convergence/model/CvgVariable').CvgVariable;
		...
		if (this.isBlocGrid()) {
			...
			for (i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					if (this.metaScope && (this.metaScope.full || this.metaScope.lines && this.metaScope.lines[line.uuid] !== null)) {
	...
	traverseSettable: function(aStructIst, aSetter) {
		...
		try {
			...
			// inferieure designee
			if (aStructIst.isFieldIst() || aStructIst.isLineIst()) {
			...
			if (aStructIst.isBlocIst() || aStructIst.isAllLinesOfBlocIst()) {
				this.metaScope.full = true;
				// Application sur le bloc
