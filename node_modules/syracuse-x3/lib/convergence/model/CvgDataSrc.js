...
var CvgDataSrc = exports.CvgDataSrc = helpers.defineClass(function(aRequester, aScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	...
	manageMeta: function(mapMeta, line, lastLine) {
		...
		var self = this;
		if (self.entrySup != null) {

			colEntity = self.entrySup.getListEntityOlderThanMeta(stampLine, true);
			for (metaKey in colEntity) {
				for (key in colEntity[metaKey]) {
					mapMeta[metaKey][key] = colEntity[metaKey][key];
				}
			}
		...
		// on current line
		colDiff = line.getListEntityDifferentValueMeta();
		for (metaKey in colDiff) {
			for (key in colDiff[metaKey]) {
				mapMeta[metaKey][key] = colDiff[metaKey][key];
			}
		}
		if (lastLine) {
			if (self.entrySup != null) {

				colEntity = self.entrySup.getListEntityOlderThanMeta(stampLine, false);
				for (metaKey in colEntity) {
					for (key in colEntity[metaKey]) {
						mapMeta[metaKey][key] = colEntity[metaKey][key];
					}
	...
	manageMetadata: function(map, line, metadata, lastLine) {
		...
		if (this.entrySup != null) {
			...
			for (key in colEntity) map[key] = colEntity[key];
		}
		...
		for (key in colDiff) map[key] = colDiff[key];
	...
	setEntrySup: function(aDataSrcEntry) {
		...
		for (var i = 0; i < this.entrySup.vars.length; i++) {
		...
		if (this.isBlocGrid()) {
			...
			for (var i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					/*  this.manageMetadata(mapMeta, line, 'STY', lastLine);
					this.manageMetadata(mapMeta, line, 'PCOL', lastLine);
					this.manageMetadata(mapMeta, line, 'SEL', lastLine);
					this.manageMetadata(mapMeta, line, 'VIS', lastLine);
					this.manageMetadata(mapMeta, line, 'ACT', lastLine);
					this.manageMetadata(mapMeta, line, 'FMT', lastLine);
					this.manageMetadata(mapMeta, line, 'TIT', lastLine);*/
					// DCH - cell by cell
			...
			if (!emptyJson(stypCol)) { // add all pCol as style
				if (!metadata.sty) metadata.sty = stypCol;
			}
			...
			if (this.hasModifTitle()) {
				...
				if (this.titleStyle != null) object.stytit = this.titleStyle;
			}
			// TODO on client side
			if (this.hasModifStyle() && this.style != null) object.sty = this.style;
			if (this.hasModifState()) object.stt = this.state;
		}
		return object;
	},
