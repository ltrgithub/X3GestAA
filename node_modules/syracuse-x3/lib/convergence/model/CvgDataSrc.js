		...
		for (var i in keys) {
			...
			switch (metadata) {
				...
				case 'STT':
					...
					itemJs.push(item.getReadableState()); // style
					break;
				case 'FMT':
					if (item.format != null) {
	...
	manageMetadata: function(map, line, metadata, lastLine) {
		...
		switch (metadata) {
			case 'STY':
				...
				break;
			case 'STT':
				if( metadata === line.kindStateChange ){
		...
		if (this.isBlocGrid()) {
			...
			for (var i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					this.manageMetadata(mapSttVisible, line, 'STT-visible', lastLine);
			...
			this.manageTableMetadata(mapStt, 'STT');
			// add all state come from set visible and set selected 
			for(item in mapSttVisible){
				mapStt.push(item);
			}
			for(item in mapSttSelected){
				mapStt.push(item);
			}
			...
			// FDB - Only titles are implemented on client side
