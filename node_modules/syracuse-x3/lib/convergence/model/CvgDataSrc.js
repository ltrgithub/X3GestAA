...
var CvgDataSrc = exports.CvgDataSrc = helpers.defineClass(function(aRequester, aScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	...
	isEmptyLine: function(lineIndex) {
		return this.lines && this.lines[lineIndex].vars.every(function(v) {
			return (v.serverValue && v.serverValue.value) == null;
		});
	},

	// insert "apres" aTargetLine
	serverInsertLines: function(startX3Idx, nbLines) {
		var timeInst = Date.now();
		var i, setFormat = false;

		// If only one line in datasource and is empty line the first insert start before the empty line 
		if ((this.lines.length == 1) && (startX3Idx == 1) && this.isEmptyLine(0)) {
			startX3Idx = 0;
		}
		this.fusionMvtsInsert(nbLines, startX3Idx);
		var indexAfterInsert = startX3Idx;
		...
		if (this.entrySup) {
			for (i = 0; i < nbLines; i++) {
				...
				setFormat = true;
				srcLine.setZeroBaseIdx(startX3Idx + i);
				this.lines.splice(startX3Idx + i, 0, srcLine);
				indexAfterInsert++;
			}
		} else {
			tracerStd.info && tracerStd.info((new Date().toISOString()) + " entrySup is null - do not propagate format");
		}
		this.updateIdxInsert(startX3Idx + i, indexAfterInsert);
	...
	serverRemoveLines: function(startX3Idx, nbLines) {
		...
		if (startX3Idx === 0) {
			...
			tracerStd.info && tracerStd.info((new Date().toISOString()) + " startX3Idx is not > 0 - do nothing");
		} else {
	...
	setServerStyle: function(aStyle, applyOn) {
		...
		if ('CHILDS' === applyOn && this.isBlocFiche()) {
			children = this.blocFicheGetChilds();
		} else {
	...
	traverseSettable: function(aStructIst, aSetter) {
		CActiveStateSetter = CActiveStateSetter || require('../client/CvgRequest').CActiveStateSetter;
		var wContinue;
		//try {
		var CActiveStateSetter = CActiveStateSetter || require('../client/CvgRequest').CActiveStateSetter;
	...
	updateIdx: function(startIdx) {
		...
		for (i = startIdx, l = this.lines.length; i < l; i++) {
			...
			// Update datsource line cache after index update
			// Get the old cache
			var cache, oldindex;
			var split = this.lines[i].getAdxIst().toString().split(",");
			if (split.length == 4) {
				cache = this.win && this.win.cache &&
					this.win.cache[split[0]] &&
					this.win.cache[split[0]][split[1]];
				oldindex = split[3];
			}
			...
			// Update datsource line cache after index update
			// Change the cache index
			if (cache) {
				cache[this.lines[i].zeroBasedIndex + 1] = cache[oldindex];
				delete cache[oldindex];
			}
		}
