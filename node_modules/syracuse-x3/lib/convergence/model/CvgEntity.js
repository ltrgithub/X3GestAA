...
var CvgEntity = exports.CvgEntity = helpers.defineClass(function(aClient, aId, aEntityIndexBaseOne, aEntityTyp) {
	...
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  +-- Â« Etat Â» ==> 0 : inactif(Grizo) â€“ 1 : actif (actzo)
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  |  +----- Â« Lecture seule Â» (Diszo)==> 0 : non  â€“ 1 : oui
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  +-------- Â« Apparence Â» ==> 0 : invisible â€“ 1 : visible
	//        |  |  |  |  |  |  |  |  |  |  |  |  +----------- Â« Statut Â» ==> 0 : dÃ©sÃ©lectionnÃ© â€“ 1 : sÃ©lectionnÃ©
	//Bits    31 30 29 28 27 26 25 24 23 22 21 20 19 18 17 16
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  +-- Notion Â« Etat Â» ==> 0 : absente â€“ 1 : prÃ©sente
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  |  +----- Notion Â« Lecture seule Â» ==> 0 : absente â€“ 1 : prÃ©sente
	//        |  |  |  |  |  |  |  |  |  |  |  |  |  +-------- Notion Â« Apparence Â» ==> 0 : absente â€“ 1 : prÃ©sente
	//        |  |  |  |  |  |  |  |  |  |  |  |  +----------- Notion Â« Statut Â» ==> 0 : absente â€“ 1 : prÃ©sente
	...
	stateSet: function(aNewState, force) {
		...
		if (force || old !== niou) {
			if (aNewState === STATE_VISIBLE || aNewState === STATE_NOT_VISIBLE ){
				this.setLastStampMod(getTick(), 'STT-visible');
			}else if(aNewState === STATE_SELECT || aNewState === STATE_NOT_SELECT){
				this.setLastStampMod(getTick(), 'STT-select');
			}else{
				this.setLastStampMod(getTick(), 'STT');
			}
			this.state = niou;
			this.stateModified = true
			//console.log("\tstateSet: " + this.getAdxId()+ " - New:"+niou+" - old:"+old);
		}
	},
	...
	// formatRefCell - Cellule de rÃ©fÃ©rence pour toutes celles de la colonne
	setColumnFormater: function(formatRefCell) {
		if (formatRefCell==null) return;
		this.columnFormater = formatRefCell;
		if (this.format == null || this.format.format != this.columnFormater.format.format) {
			this.setLastStampMod(getTick(), 'FMT');
		}
		...
		//console.log("\tsetColumnFormater" + this.getAdxId() + " - Ref:" + formatRefCell.format.format + " - me:" + this.format.format)
	},
	...
	// State utilise auusi  pour sÃ©lectionner une ligne
	...
	setServerStyle: function(aStyle, applyOn) {
		if (applyOn != null && applyOn === 'TITLE') {
			...
			if (!this.styleModified) {
				this.setLastStampMod(getTick(), 'STY');
			}
			this.style = aStyle;
	...
	setServerTitle: function(aTitle) {
		...
		if (wNewTitle) {
			this.title = aTitle;
			this.isTitleModified = true;
			this.setLastStampMod(getTick(), 'TIT');
		}
	},
	setServerValue: function(aStructResu) {
		...
		if (!this.formatModified && this.columnFormater != null && this.columnFormater !== this.format) {
			this.storeFormat(this.columnFormater.format);
		}
		this.forceChange();
	},
