...
var CvgObservable = require('./CvgObservable').CvgObservable;
var CvgIst = require('../types/CvgIst').CvgIst;
var CvgResu = require('../types/CvgResu').CvgResu;
var CvgResuTdo = require('../types/CvgResu').TDO_LIB;
var CvgResuTag = require('../types/CvgResu').TAG_LIB;
...
var FusionEntityState = helpers.defineClass(function(maskVal, maskAtt, maskNot) {
...
exports.FusionEntityState = FusionEntityState;
...
function _readableState(state) {
	var r = "";
	if (FusionEntityState.VISIBLE.isNotOn(state)) {
		r += "!V";
	}
	if (FusionEntityState.VISIBLE.isOn(state)) {
		r += "V";
	}
	if (FusionEntityState.ACTIF.isNotOn(state)) {
		r += "!E";
	}
	if (FusionEntityState.ACTIF.isOn(state)) {
		r += "E";
	}
	if (FusionEntityState.SELECTED.isNotOn(state)) {
		r += "!S";
	}
	if (FusionEntityState.SELECTED.isOn(state)) {
		r += "S";
	}
	if (FusionEntityState.READONLY.isNotOn(state)) {
		r += "!R";
	}
	if (FusionEntityState.READONLY.isOn(state)) {
		r += "R";
	}
	return r;
}
...
exports.FusionMetaData = FusionMetaData;
...
var DISABLED = 0; //var X3_SRV_GRISO = 0;
var ENABLED = 1; //var X3_SRV_ACTZO = 1;
var READONLY = 2; //var X3_SRV_DISZO = 2;
...
var CvgEntity = exports.CvgEntity = helpers.defineClass(function(aClient, aId, aEntityIndexBaseOne, aEntityTyp, windows) {
	CvgObservable.call(this, aClient);
	this.alternativeId = null;
	this.id = aId;
	this.state = STATE_DEFAULT;
	this.stateToJson = STATE_DEFAULT;
	this.visible = null;
	this.selected = false;
	this.actzo = STATE_ENABLE;

	this.isTitleModified = false;
	this.format = null;
	this.formatStr = "";
	this.formatStrToJson = "";
	...
	// set by default a tick for the format
	...
	forceChange: function() {
		CvgObservable.prototype.forceChange.call(this);
		//console.log("change ",this.getAdxIst().toString());

		if (this.win && (!this.srcLine || this.srcLine.isEntrySup)) {
			this.win.addCache(this.getAdxIst(), this);
		}
	},
	removeCache: function() {
		this.win && this.win.deleteCache(this.getAdxIst());
	},
	...
	calculateState: function() {
		...
		if (this.lastStampModActzo != -1) {
			this.state = _calcJsonNewEntityState(this.state, this.actzo);
		}
		return this.lastStampModState;
	},
	setVisible: function(bool) {
		var oldState = bool ? STATE_VISIBLE : STATE_NOT_VISIBLE;
		if (this.visible !== oldState) {
			if (bool) {
				if (this.visible !== null) // null is considere as visible. don't notify te client because the state doesn't change in client side
	...
	getObservableId: function() {
		...
		function computeObservableId() {
			var ist = self.getAdxIst();
			if (ist != null) {
				var fullId = "";
				fullId += ist.winAlphaId;
				if (self.entityTyp !== 'WINDOW') {
					fullId += ist.screenAlphaIdFusion;
					if (self.entityTyp !== 'SCREEN') {
						fullId += ist.blocAlphaId;
						if (self.entityTyp === 'FIELD') {
							fullId += ist.field;
						}
					}
				}
		...
		if (!this.observerableId) {
			var fullId = computeObservableId();
			if (fullId && fullId.indexOf(0) === (this.win && this.win.entityAdxAlpha)) {
				return (this.observerableId = fullId);
			} else {
				return fullId;
			}
		}
		return this.observerableId;
	...
	getAdxIst: function() {
		this.ist = new CvgIst(this.getAdxId());
		//this._addInMapIst(ist);
		return this.ist;
	},
	...
	getJsonKey: function() {
		if (this.jsonKey === undefined) {
			...
			return (this.jsonKey = jsonKey);
		}
		// updae cache
		//this.win && this.win.addCache(this.getAdxIst(), this);
		return this.jsonKey;
	},
	...
	razModification: function() {
		...
		this.formatStrToJSon = this.formatStr;
		this.styleToJson = this.style;
		this.pcolorToJson = this.pColor;
		...
		if (this.hasColumnFormater()) {
			this.columnFormater.optimChvl = -1;
		}
	},
	...
	setLastStampMod: function(lastStampMod, metadata) {
		// inform the dataSrc if the entity is link to a datasrc that the meta has been modified
		var value;
		if (this.srcLine && this.srcLine.dataSrc) {
			this.srcLine.dataSrc.metaChange = true;
		} else if (this.dataSrc) {
			this.dataSrc.metaChange = true;
		}
		var meta = metadata;
		var objVal = {};
		switch (metadata) {
			...
			case 'PCOL':
				...
				this.lastStampModState = lastStampMod;
				objVal.value = "$pcolor_" + this.pColor;
				meta = "STY";
				break;
			case 'VIS':
			...
			case 'STY':
				...
				objVal.value = this.style;

				break;
			case 'FMT':
				this.lastStampModFmt = lastStampMod;
				objVal.value = this.formatStr;

				break;
			case 'TIT':
				this.lastStampModTit = lastStampMod;
				objVal.value = this.title;
		...
		this.notifyStampValueMeta && this.notifyStampValueMeta(meta, objVal, lastStampMod, metadata);
	...
	setServerFormat: function(aFormat, fmt) {
		...
		if (this.formatStr !== fmt) {
			this.formatStr = fmt;
		}
		this.srvValueModified = true; // SAM 119099 - always send data to client while we change the format in order to allow the client to apply new format
		if (fmt && fmt.charAt(0) !== '-')
			this.updateFormatTick();

		if (this.srcLine && this.srcLine.dataSrc) {
			var idxVar = this.srcLine.vars.indexOf(this);
			this.srcLine.dataSrc.entrySup && this.srcLine.dataSrc.entrySup.vars[idxVar].setVisible(isVisible);
		}
	...
	setServerResu: function(aStructResu) {
		// check if we set a resu that correspond to the type

		this.serverValue = aStructResu;
	},
	// Style pour les variables - Noeud NDCSTL
	setServerStyle: function(aStyle, applyOn) {
		//console.log("setServerStyle->" + this.getAdxId() + " - style=" + aStyle + " - this.style=" + this.style + " - applyOn=" + applyOn);
		if (applyOn != null && applyOn === 'TITLE') {
			...
			if (this.style !== aStyle ||  applyOn === "CHILDS") {
				this.style = aStyle;
...
CvgEntity.x3AffModeToEntityState = function(mode) {
	switch (mode) {
		...
		case X3VAR_MAF_AFFICHE:
			return STATE_VISIBLE + CvgEntity.x3State2EntityState(DISABLED);
