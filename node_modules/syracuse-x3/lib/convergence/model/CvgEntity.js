...
var FusionEntityState = exports.FusionEntityState = helpers.defineClass(function(maskVal, maskAtt, maskNot) {
	...
	isOn: function(status) {
		console.log("maskVal "+this.maskVal+" status "+status);
...
function _calcJsonNewEntityState (curFusionState, stateToAdd) {
	...
	if (addNotVisible) {
		console.log("state to Add "+stateToAdd);
...
var CvgEntity = exports.CvgEntity = helpers.defineClass(function(aClient, aId, aEntityIndexBaseOne, aEntityTyp) {
	...
	this.selected = STATE_NOT_SELECT;
	this.status = X3_SRV_ACTZO; 
	...
	calculateState: function(){
		this.state =0;
		console.log("calculate state "+this.state+" visible "+this.visible+" id "+this.getObservableId());
		this.state = _calcJsonNewEntityState(this.state,this.visible );
		this.state = _calcJsonNewEntityState(this.state,this.selected);
		this.state = _calcJsonNewEntityState(this.state,this.actzo);
		console.log("calculate state "+this.state+" id "+this.getObservableId());
	...
	setVisible : function(bool){
		...
		console.log("change visible "+this.visible+" id "+this.getObservableId());
	},
	...
	setLeftListLineSelected: function(selected, force) {
		...
		this.setSelected(selected == STATE_SELECT ? true:false);
	},
