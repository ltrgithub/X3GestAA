...
var _log=true;
var CvgDiagnose = exports.CvgDiagnose = helpers.defineClass(function(client, severity, appCode, message, details, error, origin, recovery, track,jsonType) {
	...
	this.recovery = alive ? recovery : 'ABORT';
	// En dernier
	if (track) this.setJsonType(jsonType ? jsonType : 'DIAGNOSE');
}, CvgObservable, {
	toJSON: function() {
		var diag = {
		...
		if (this.details) diag.$details = this.details;		
        if (this.recovery) diag.$recoveryCode = this.recovery;        
		diag.$links = [{
