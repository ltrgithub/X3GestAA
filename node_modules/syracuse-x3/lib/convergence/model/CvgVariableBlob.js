...
var CvgVariableBlob = exports.CvgVariableBlob = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aSize, aFormat, aEntityState, blobSvc) {
	...
	controlBlobSize: function(aBlob) {
		...
		if (aBlob.length > wMaxSize) {
			throw new CvgException(this.client, null, this.client.getMessage('ERROR_UPLOAD_BLOB', aBlob.length.toString(), this.id, wMaxSize.toString(), this.size.toString()));
		}
	},
	...
	setBrowserUnformattedValue: function(aBuffer) {
		var resu = new CvgResu(CvgResuTag["BLOB"], aBuffer);
		console.log("RESU= "+resu.toString());
	...
	setServerValue: function(aResu) {
		if (aResu.value) {
			this.uuid = this.srcLine.dataSrc.screen.requesterWindow.entityIdxAlpha + this.srcLine.dataSrc.screen.entityIdxAlpha + this.srcLine.dataSrc.entityIdxAlpha + this.id;
	...
	unFormatData: function(aValue) {
		return this.client.services.blobSvc.getBlob(aValue);
	},
