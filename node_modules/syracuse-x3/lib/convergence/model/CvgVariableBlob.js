...
var CvgVariableBlob = exports.CvgVariableBlob = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aSize, aFormat, aEntityState, blobSvc) {
	...
	controlBlobSize: function(aBlob) {
		...
		if (aBlob.length > wMaxSize) {
			throw new CvgFormatException(this.client, null, this.client.getMessage('ERROR_UPLOAD_BLOB', aBlob.length.toString(), this.id, wMaxSize.toString(), this.size.toString()));
		}
	},
	...
	unFormatData: function(aValue) {
		var binary = this.client.services.blobSvc.getBlob(aValue);
		if (binary) {
			return new CvgResu(CvgResuTdo["BLOB"], binary);
		}else {
