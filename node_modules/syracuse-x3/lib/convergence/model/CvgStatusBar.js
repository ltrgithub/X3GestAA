...
var CvgStatusIcon = exports.CvgStatusIcon = helpers.defineClass(function(aImgId, aActionId, aVisible) {
	...
	toJSON: function() {
		// this.actionId = 0 pour les icone supprimés
...
var CvgStatusBar = exports.CvgStatusBar = helpers.defineClass(function() {
	...
	setText: function(aNumText, aText) {
		...
		if(aNumText < notifyOffset) {
			...
			this.notify.push({"t": aText, "l": aNumText - notifyOffset});
		}
	},
	toJSON: function() {
		var statusbar = {t: [], ico: []}, i, len, max = _notifySeverity.length;
		...
		if ((len = this.notify.length) > 0) {
			...
			for (i = 0; i < this.notify.length; i++) {
				if(this.notify[i].l < max) {
					statusbar.notify[i] = {"$message": this.notify[i].t, "$severity": _notifySeverity[this.notify[i].l]};
				}
			}
			// RAZ notify!
			this.notify = [];
