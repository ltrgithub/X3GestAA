...
var CvgDataSrc = require('./CvgDataSrc').CvgDataSrc;
	...
	setServerSelectLine: function(selected, selKeys) {
		...
		if (sel != null) {
			...
			if (this.isListFormat(CvgLeftList.FMT_PICKING_FLAT)) {
				tracerStd.info && tracerStd.info((new Date().toISOString()) + " set selected " + sel.line.getObservableId() + " selected " + selected);
			...
			var sel = this.getLine(this.curLineIdx);
			sel.setLeftListLineSelected(false);
	...
	overrideProto: function() {
		...
		if (this.hasColumnTitleModified() || this.isColomnFormatChange()) {
			...
			for (var it in this.entrySup.vars) {
				...
				if (item.dataType != 1 && item.dataType != 7 && item.format && item.format.format) {
					// Issue 106575 : test on string type, just to improve perf, and avoid to force the client fo format string in left list!
					itemJs.$X3Fmt = item.format.format;
				}
