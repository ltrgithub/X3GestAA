	...
	toJSON: function() {
		...
		if (this.hasServerValueModification()) {
			...
			if (this.isBo) {
				...
				value = this.client.baseUrl.replace('trans', 'bo') + "/$proxy" + value.substring(idx2 + 5, idx - 1).replace("businessobjects", "BOE") + "&token=" + encodeURIComponent(this.client.boClient.token);
