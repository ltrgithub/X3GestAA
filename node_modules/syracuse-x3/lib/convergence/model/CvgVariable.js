...
var CvgVariable = exports.CvgVariable = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVariableName, aVariableId, aDataType, aFormat, aEntityState) {
	...
	buildMsgFormatterException: function(aMessKey, aWhatFormatKey, value, e) {
	...
	callUnFormater: function(aValue) {
		...
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				try {
					//console.log("before format: '"+aValue+"'");
					aValue = fmt.formatField(this.format, aValue, false);
					//console.log("after format: '"+aValue+"'");
				} catch (e) {
					// aValue peut etre une valeur formatee au format X3 --> On
					// tente le unformate
				}
				// Déformate - 01/02/03 -> 20030201
				var res = fmt.unformatField(this.format, aValue, true);
				//console.log("after unformat: '"+res+"'");

				return res;
				
			} catch (e) {
				console.error("Error callUnFormater:\n"+e.stack);
				throw new CvgFormatException(this.client, e, this.buildMsgFormatterException('ERROR_UNFORMAT_EXCEPTION', 'UNFORMAT_STOR', aValue, e), 'CLOSESESS');
			}
		}
		// No format
		...
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				try {
					...
					// Seems the value is already raw
				}
				try {
					res = this.client.formater.formatField(this.format, aValue, true, true);
				} catch (e3) {
					console.error("Error formatEditToDisplay:\n"+e.stack);
					throw new CvgFormatException(this.client, e3, this.buildMsgFormatterException('ERROR_FORMAT_EXCEPTION', 'FORMAT_ETOS', aValue, e3), 'CLOSESESS');
				}
			}
			return res;
		} else {
