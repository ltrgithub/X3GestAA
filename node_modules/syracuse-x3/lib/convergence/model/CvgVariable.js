...
var CvgVariable = exports.CvgVariable = helpers.defineClass(function(srcLine, idx1, name, id, type, fmt, state) {
	...
	toJSON: function() {
		...
		if (this.hasServerValueModification()) {
			...
			if (this.isBo && this.client.boClient.token) {
				var idx = value.indexOf('</ADR>');
				var idx2 = value.indexOf('<ADR>');
				// add hack for replace businessobjects by BOE
				//value = this.client.baseUrl.replace('trans', 'bo') + "/$proxy" + value.substring(idx2 + 5, idx - 1).replace("businessobjects", "BOE") + "&sWindow=ifr&token=" + this.client.boClient.token;

				var docId = "AW4AVT1AUhVAogA6P7OQv9c";
				var encodedToken = encodeURIComponent(this.client.boClient.token);

				value = this.client.baseUrl.replace('trans', 'bo') + "/$proxy" + "/BOE/OpenDocument/opendoc/openDocument.jsp?iDocID=" + docId + "&sIDType=CUID&token=" + encodedToken;
			}
			if (value != null) {
				object.v = value;
			} else {
