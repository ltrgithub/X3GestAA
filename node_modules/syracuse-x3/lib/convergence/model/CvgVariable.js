...
var CvgResuTdoLib = require('../types/CvgResu').TDO_LIB;
var CvgResuTag = require('../types/CvgResu').TAG;
var CvgDiagnose = require('./CvgDiagnose');
...
var CvgVariable = exports.CvgVariable = helpers.defineClass(function(srcLine, idx1, name, id, type, fmt, state, windows) {
	CvgEntity.call(this, srcLine && srcLine.client, id, idx1, 'FIELD', windows);
	this.isBo = false;
	...
	forceChange: function() {
		...
		if (this.srcLine != null && !this.srcLine.isLeftListLine) {
			...
			if (dataSrc && dataSrc.isBlocGrid()) {
				...
				//this.srcLine.varsModified[this.uuid] = this;
				return;
			}
		}
	},
	notifyStampValueMeta: function(metadata, objVal, lastStampMod, realMeta) {
		if (this.srcLine && this.srcLine.dataSrc) {
			if (!this.srcLine.isEntrySup && this.srcLine.zeroBaseLineIdx !== undefined && this.srcLine.zeroBaseLineIdx !== -1 && realMeta !== "VIS") { // visibile is only on column
				this.srcLine.dataSrc.notifyStampMeta(metadata, "line," + (this.srcLine.zeroBaseLineIdx + 1) + "," + this.id, objVal, lastStampMod, this.getObservableId().substring(1));
			} else {
				if (this.srcLine.isEntrySup) // check to only set the meta to the column (identified by entrySup)
					this.srcLine.dataSrc.notifyStampMeta(metadata, "col," + this.id, objVal, lastStampMod, this.getObservableId().substring(1));
			}
		}
	},
	...
	// override add cache to add only variable that are not in a grid. for entity in the grid it will be added when the set the index line
	...
	hasGuiModModifications: function() {
		return this.hasModifState() || this.hasModifTitle() || this.hasModifStyle() || this.hasModifFormat();
	},
