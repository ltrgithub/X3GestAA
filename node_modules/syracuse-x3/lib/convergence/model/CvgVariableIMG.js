...
var CvgVariableIMG = exports.CvgVariableIMG = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aSize, aFormat, aEntityState, window) {
	CvgVariableBlob.call(this, srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aSize, aFormat, aEntityState, window);
}, CvgVariableBlob, {
	clone: function(line) {
		var wVariable = new CvgVariableIMG(line, this.entityIndexBaseOne, this.name, this.id, this.dataType, this.size, this.format, this.state, this.win);
		wVariable.mustBeFormatted = this.mustBeFormatted;
		if (this.indirection != null) {
			wVariable.setIndirection(this.indirection.clone(line));
		}
