...
var CvgFormat = exports.CvgFormat = helpers.defineClass(function(format) {
	...
	getNbDecimal: function() {
		var wPos = this.format.indexOf('.');
		if (wPos > -1)
			return CvgFormat.extractIntegerFromString(this.format.substring(wPos + 1));
		else
			return 0;
	},
	isWithoutZero: function() {
		if (this.isValid() && this.format.indexOf(':'))
			return this.format.split(':')[0].indexOf('z') !== -1;
		return false;
	},
	isWhithSymbolThousand: function() {
		if (this.isValid() && this.format.indexOf(':'))
			return this.format.split(':')[0].indexOf('3') !== -1;
		return false;
	},
...
CvgFormat.extractIntegerFromString = function(aString) {
	...
	var int = parseInt(aString.substring(wStart, wStop), 10);
	return isNaN(int) ? -1 : int;
}

CvgFormat.newEmptyFormat = function() {
	return new CvgFormat("");
}
