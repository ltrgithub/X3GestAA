...
var CvgLeftList = require('./CvgLeftList').CvgLeftList;
var CvgListOfLocalMenus = require('./CvgListOfLocalMenusFactory').CvgListOfLocalMenus;
var CvgListOfLocalMenusFactory = require('./CvgListOfLocalMenusFactory').CvgListOfLocalMenusFactory;
var CvgListOfScreens = require('./CvgListOfScreens').CvgListOfScreens;
var CvgListOfDataSrcs = require('./CvgListOfDataSrcs').CvgListOfDataSrcs;
var CvgListOfDataSrcsFactory = require('./CvgListOfDataSrcsFactory').CvgListOfDataSrcsFactory;
var CvgListOfLeftLists = require('./CvgListOfLeftLists').CvgListOfLeftLists;
var CvgWindow = require('./CvgWindow').CvgWindow;
var CvgScreen = require('./CvgScreen').CvgScreen;
var CvgVariable = require('./CvgVariable').CvgVariable;
var CvgDataSrcLine = require('./CvgDataSrcLine').CvgDataSrcLine;

var CvgStackedWindow = require('./CvgStackedWindow').CvgStackedWindow;
var CvgListOfLeftListsFactory = require('./CvgListOfLeftListsFactory').CvgListOfLeftListsFactory;
var CvgStatusBar = require('./CvgStatusBar').CvgStatusBar;
var navPageHelper = require("@sage/syracuse-lib/src/collaboration/entities/page/navPageHelper");
...
var CvgStackedWindow = exports.CvgStackedWindow = helpers.defineClass(function(aClient, aWindowId, aFunctionId, aDescr, aStamp, aEnityIndexBaseOne) {
	...
	// do not compute breadcrumb for special windows
	if (['TEMPON', '_CHOOSE', '_FORMULA', '_SELBOX'].indexOf(this.id) === -1) {
		// make the association between function code and screen name in order to build the breadcrumb
	...
	addCache: function(ist, entity) {
		...
		if (split.length && split[0] !== "@" && split[1] !== "@" && split[2] !== "@") {
			var current = this.cache[split[0]] = this.cache[split[0]] ||  {};
			if (entity instanceof CvgVariable && (entity.srcLine instanceof CvgDataSrcLine) && split.length === 4 && !entity.isEntrySup) {
				for (var i = 1; i < split.length - 2; i++) {
					current = current[split[i]] = current[split[i]] || {};
				}
				...
				if (!entity.srcLine || entity.srcLine.isEntrySup) {
					for (var i = 1; i < split.length - 1; i++) {
						current = current[split[i]] = current[split[i]] || {};
					}
	...
	setOpened: function(opened) {
		...
		if (this.opened === false) {
			...
			// raz the target setted if we close the windows 
			// check if the target is on the current windows 
			var target = this.client.getTarget();
			if (target && target.ist && target.ist.win === this.getAdxId())
				this.client.razTarget();
		}
	...
	traverseSettable: function(aStructIst, aSetter) {
		...
		aSetter.setModifiable = function(ist, obj) {
			if (!obj.srcLine || obj.srcLine.isEntrySup) {
