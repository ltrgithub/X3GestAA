			...
			if (i !== 0) {
				...
				switch (meta) {
					...
					case 'TIT':
						...
						break;
					case 'FMT':
						if (item.formatStr != null){


							valueItem = item.formatStr;
							if (this.format != null) {
								value = this.formatStr;
							}
							if (colVar != null) {
								if (colVar.formatStr != null) {
									colVal = colVar.formatStr; // value column
								}
		...
		for (var vv in this.vars) {
			...
			switch (metaData) {
				case 'FMT':
					...
					/*if( v.getObservableId().indexOf("CA2")){
						console.log("format "+v.formatStr);
						console.log("id "+v.getObservableId());
						console.log("tick "+v.lastStampModFmt);
					}*/

					hasValue = v.formatStr != null;
					break;
				case 'ACT':
