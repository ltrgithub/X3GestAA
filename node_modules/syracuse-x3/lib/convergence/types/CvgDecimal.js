...
var CvgDecimal = exports.CvgDecimal = {
	...
	toBytes: function(str, buf, pos) {
		...
		if (intg.length > 0) {
			...
			mant = dec1.length % 2 ? '0'+ dec2 :dec1+ dec2;
		} else { // all zeros
			buf[0] = 0x80;
			buf.fill(0, 1, 17);
			return buf;
		}
		var exp = ((intg.length+dec2.length)-(dec1.length+dec2.length));
		var power = exp /2 ;
		power = exp % 2 !==0 ? Math.floor(power)+1:power;
		//power = (exp % 2) !==0 ? power +1:power;
		buf[pos++] = neg ? (63 - power) : (power + 192);
		mant =  mant.length % 2 ? mant+ '0':mant;
		for (var i = 0; 2*i+1 < mant.length; i++) {
			var val =  parseInt(mant.charAt(2* i))*10+parseInt(mant.charAt(2*i+1));
			buf[pos + i] =val;
		}
