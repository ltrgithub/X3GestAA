...
var CvgFormatter = exports.CvgFormatter = helpers.defineClass(function(values) {
	...
	_parseEditDate :function (fmt, str, srvVal,y4Digits){
		...
		console.log(str + " - "  + JSON.stringify(dat))
	...
	typeDate: function(){
		...
		ord = ord == Cvg.FTM_X3DATE_MDY ? 'MDY' : ord == Cvg.FTM_X3DATE_DMY ? 'DMY' :  ord == Cvg.FTM_X3DATE_YMD ? 'YMD' : null;
		if (ord) return ord;
		throw new Error("unsupported date order : " + ord) ;		
	},
	// Add settings to res - used to send settings to client
	addUiCliSettings: function(settings){
	    settings.datePivot 		= this.settings.datePivot;
      	settings.dateOrder 		= this.settings.dateOrder;
		settings.dateSep		= this.settings.dateSep;
		settings.thousandSep	= this.settings.thousandSep;
		settings.decimalSep 	= this.settings.decimalSep;
		settings.timeSep		= this.settings.timeSep;	
	}
});
