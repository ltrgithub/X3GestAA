...
//				return res;
	...
	transform: function(format) {
		...
		if (format.length === 5 && format.indexOf('D:DD') === 0){
			...
			console.error("Transform date format from '"+format+"' to 'D:"+s+"'");
			return "D:" + s;
		}
		return format;
	},
	getFormatEdit: function(fmt) {
		...
		switch(type) {
			...
			case 'D':
				var t = type + 'z:' + (parts[1] || '').replace(/\[[^\]]*\]|\>/g,'');
				console.log("EDIT FORMAT TRANSFORMED: "+t);
				
				
				//return type + 'z' + (parts[1] || '').replace(/D*/g, 'D').replace(/M*/g, 'M').replace(/Y*/g, 'Y');
			default:
				return fmt
		}
	},
