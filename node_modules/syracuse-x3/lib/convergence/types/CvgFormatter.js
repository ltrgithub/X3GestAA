...
var CvgFormatter = exports.CvgFormatter = helpers.defineClass(function(settings) {
	...
	formatField: function(format, value, raw, isEdit) {
		...
		case 'D':
			...
			if ((raw && parts.length !== 2) || parseInt(this.settings.dateOrder, 10) === Cvg.FTM_X3DATE_YMD) {
				...
				if (parseInt(this.settings.dateOrder, 10) === Cvg.FTM_X3DATE_MDY && d !== 0) { // d test to avoid swap in valude.length <= 4 case
					var tmp = d;
					d = m;
					m = tmp;
				}
			...
			if (y === 1599 && m === 12 && d === 31) {
				...
				y = this.calcCentury(y, value, fmt);
		...
		switch (fmt[0]) {
			...
			case 'D':
					...
					if (dat.year !== 1599 && dat.month !== 12 && dat.day !== 31) {
						var realYear = this.calcCentury(dat.year, val, fmt);
						if (dat.year !== realYear)
							dat = date.make(realYear, dat.month, dat.day);
					}
					var res = zeroPad(dat.year, 4) + zeroPad(dat.month, 2) + zeroPad(dat.day, 2);
					return res;
			case 'N':
				if (val.length === 0) {
					return "";
	...
	calcCentury: function(y, value, fmt) {
	...
	buildDateFormat: function(day, month, year) {
		var s = '';
		var sep = "[" + this.settings.dateSep + "]";
		if (parseInt(this.settings.dateOrder, 10) === Cvg.FTM_X3DATE_DMY) {
			s += day + sep + month;
			if (year.length > 0) {
				s += sep + year;
			}
		} else if (parseInt(this.settings.dateOrder, 10) === Cvg.FTM_X3DATE_YMD) {
