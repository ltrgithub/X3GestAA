"use strict";
var formatter = require('../../formatter/format');
var date = formatter.dateHelpers;
var template = {
	decimalSep: '.',
	dateOrder: 0,
	thousandSep: ',',
...
CvgFormatter.prototype.constructor = CvgFormatter;
var _CvgFormatterMenbers = {
	// convergence client - called at sessionSettings
	setDatePivot: function(v) {
		v = parseInt(v, 10);
		if (!isNaN(v)) {
			this.settings.datePivot = v;
		}
	},
	...
	addDynamicFormat: function(dyn) {
		if (!this.settings.dynFormats) {
			this.settings.dynFormats = {};
		}
	...
	formatField: function(format, value, raw, strict) {
		...
		// #553
		var strValue = value,
			thousandGroup, dec, lastD, lenN, thousandSep, fmtR, decPart;
		...
		switch (fmt[0]) {
			case 'D':
				...
				if (raw) {
					...
					if (parts.length !== 1) {
						throw new Error("Bad x3 date[" + value + "] - No separator expected - format YYYYMMDD");
					}
					if (value.length === 0) {
						dat.y = 1599;
						dat.m = 12;
						...
						dat.y = parseInt(value.substr(0, len) || '0', 10);
						dat.m = parseInt(value.substr(len, 2) || '0', 10);
						dat.d = parseInt(value.substr(len + 2, 2) || '0', 10);
						if (dat.y === 0 && dat.m === 0 && dat.d === 0) {
							dat.y = 1599;
							dat.m = 12;
							dat.d = 31;
						}
					...
					if (rightPart && rightPart.length > 0) {
						...
						if (this.settings.skipBadFormat === true) {
							return strValue;
						}
					...
					if (parts.length > 1) {
						...
						if (value.length > 8 & id == 'y') {
							dat.y = parseInt(value.substr(pos - l) || '0', 10);
						}
					}
				}
				...
				break;
			case 'N':
				thousandSep = this.settings.thousandSep;
				if (thousandSep && !raw) {
					// If it's not a raw value, thousand sep can't be egual to dec sep
					value = value.split(thousandSep).join('');
				} else if (raw && thousandSep == ".") {
					lenN = value.length;
					lastD = value.charAt(lenN - 1);
					if (lastD == thousandSep) {
						value = value.substr(0, lenN - 1);
					}
					fmtR = (fmt.split(":"))[1];
					dec = fmtR.indexOf(".") >= 0 || fmtR.indexOf("F") >= 0 ? true : false;
					thousandGroup = value.split(thousandSep);
					if (dec && thousandGroup.length > 2) {
						// Not really normal regarding protocol!!! We assume that the last sep is for decimal... no choice
						decPart = thousandGroup[thousandGroup.length - 1];
						thousandGroup.splice(thousandGroup.length - 1, 1);
						value = thousandGroup.join("") + "." + decPart;
					}
				}
				if (value.length > 0) {
		...
		try {
			...
			switch (fmt[0]) {
				case 'K':
					...
					result = this.settings.blankStrict && fmt.indexOf('[') < 0 ? result.replace(/\f\n\r\t\v+$/, '') : result.replace(/\s+$/, '');
					break;
				case 'D':
					if (val != null) {
					...
					if (dat.year !== 1599 || dat.month !== 12 || dat.day !== 31) {
						...
						if (dat.year !== realYear) {
							dat = date.make(realYear, dat.month, dat.day);
						}
						if (check === true) {
							this._checkDate(dat, val);
						}
					}
				...
				case 'N':
					if (val.length === 0) {
						...
						if (this.settings.thousandSep) {
							val = val.split(this.settings.thousandSep).join('');
						}
	...
	formatEdit: function(format, val, raw) {
		...
		var wResult = this.formatField(this.getFormatEdit(format), rawVal, true, false);
		return wResult;
	},
	...
	_readDigit: function(dat, x, t, y4Digits, dateRef) {
		...
		if (t == 'M' || t == 'D') {
			...
			if (isNaN(dat[t])) {
				if (dateRef && dateRef[t] != null) {
					dat[t] = dateRef[t];
				} else {
					throw new Error((t == 'M' ? "Month" : "Day") + " expected");
				}
			}
			digVal = x.substr(2);
		} else {
	...
	_parseEditDate: function(fmt, str, srvVal, y4Digits) {
		...
		if (split.length == 1) {
			...
			var i = 0,
				y4 = false,
				dDefined = ord.indexOf("D") >= 0,
				mDefined = ord.indexOf("M") >= 0,
				yDefined = ord.indexOf("Y") >= 0,
				ordl = ord.length;
			if (yDefined && mDefined && dDefined && str.length > 6) {
				y4 = true;
			} else if (yDefined && (mDefined || dDefined) && str.length > 4) {
				y4 = true;
			} else if (yDefined && !mDefined && !dDefined && str.length == 4) {
				y4 = true;
			}
			for (i = 0; i < ordl; i++) {
				tmp = this._readDigit(dat, tmp, ord.charAt(i), y4, dateRef);
			}
			if (tmp.length !== 0) {
				if (i > 3 || isNaN(tmp)) {
		...
		var p;
		for (p in dat) {
			if (dat[p] == null || isNaN(dat[p])) {
				dat[p] = dateRef[p];
			}
		}
		// #1218 -JMK - we accept 2 or 4 digits dates only
		if ((dat.Y >= 100 && dat.Y <= 999) || dat.Y > 9999) {
			throw new Error("Bad year number");
		}
		// Date X3
		...
		if (dDefined !== false && (dat.D <= 0 || dat.D > 31)) {
			throw new Error("Bad day number");
		}
		if (dat.M <= 0 || dat.M > 12) {
			throw new Error("Bad month number");
		}
		if (ord.length == 3) {
			this._checkDate(date.make(dat.Y, dat.M, dat.D), str);
		}
		var res = '';
		res += String(dat.Y);
	...
	formatEditToDisplay: function(format, val, srvVal) {
		var editFmt = this.getFormatEdit(format);
	...
	_calcCentury: function(y, value, fmt) {
		...
		if ((y < 100) && value.length > 0) {
			century = Math.floor(new Date().getFullYear() / 100) * 100;
			y = century + y;
			if (y - 100 >= this.settings.datePivot) {
				y -= 100;
			}
	...
	getRealFormat: function(format) {
		...
		var res = {
			format: format,
			countryDate: format.search(/D:DD[1-4]$/) === 0
		};
		if (res.countryDate) {
			res.dateType = res.format[4];
			var type = res.format[4];
			var day = 'DD';
	...
	_cleanFormat: function(fmt, edit) {
		...
		if (res.left.length === 0 || res.right.length === 0) {
			throw new Error("Bad format");
		}
		...
		if (res.left[0] == 'D') {
			...
			res.right.replace(/(M+)|(D+)|(Y+)/g, function(p1) {
				...
				if (x == "M") {
					res.month = edit ? "MM" : p1;
				}
				if (x == "Y") {
					res.year = edit ? p1.length > 2 ? "YYYY" : "YY" : p1;
				}
				return "";
			});
		}
...
for (var name in _CvgFormatterMenbers) {
	CvgFormatter.prototype[name] = _CvgFormatterMenbers[name];
}
exports.CvgFormatter = CvgFormatter;