...
var CvgFormatter = exports.CvgFormatter = helpers.defineClass(function(settings) {
	...
	formatField: function(format, value, raw, strict) {
		...
		case 'D':
			...
			if ((raw && parts.length !== 2) || parseInt(this.settings.dateOrder, 10) === Cvg.FTM_X3DATE_YMD) {
				if (parts.length > 1) {
	            ...
	            if (parts.length > 1) {
	            	var idx={d:-1,m:-1,y:-1},i=0;
		            fmt.substr(fmt.indexOf(':')).replace(/(Y+|M+|D+)/g,function(p1){
		                idx[p1[0].toLowerCase()]=i++;
		            });
					d = idx.d>=0 &&  parts[idx.d] != null ? parseInt(parts[idx.d] || '0', 10) : 0;
					m = idx.m>=0 &&  parts[idx.m] != null ? parseInt(parts[idx.m] || '0', 10) : 0;
					y = idx.y>=0 &&  parts[idx.y] ? parseInt(parts[idx.y], 10) : new Date().getFullYear();
				} else {
					if (value.length <= 4) {
			...
			if (y === 1599 && m === 12 && d === 31) {
				...
				y = this._calcCentury(y, value, fmt);
				value = date.make(y, m, d);
			}
			break;
		case 'N':
	...
	unformatField: function(format, val) {		
		...
		switch (fmt[0]) {
			...
			case 'D':
				...
				var f=this._cleanFormat(format.format,false)
	...
	// date is input with current this.settings.dateOrder
	...
	formatEditToDisplay: function(format, val, srvVal) {
		...
		if (format.format[0]=='D'){
			val=this._parseEditDate(editFmt, val, srvVal ? srvVal.value : null);
		}else{
			var editFmt=new CvgFormat(editFmt);
			var tmp = this.formatField(editFmt, val, false, true);
			if (tmp != null) {
				var tmp = this.unformatField(editFmt, tmp);
			}
		}
