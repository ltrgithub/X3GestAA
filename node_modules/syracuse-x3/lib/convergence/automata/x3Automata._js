	...
	for (var i = 0; i < vals.length; i++) {
		...
		switch (letter) {
			case 'U':
				...
				client.stream.write(_, buf);
				client.stream.write(_, new Buffer(val, "utf8"));
				break;
			default:
				throw new Error("bad ack char: " + letter);
		}
...
function processNode(_, client, request, reader, node, phase) {
	...
	if (!node.skipHandler) { // no need to call dummyHandler!
		...
		var fn = client[nam];
		client.timeNode = client.timeNode || {};
		...
		if (client.timeNode[node.name] === undefined) {
			client.timeNode[node.name] = 0;
		}
		var timeSpent = ((new Date()).getTime() - time) / 1000;
		client.timeNode[node.name] = +timeSpent;
		if (client.totalNode === undefined)
			client.totalNode = 0;
		client.totalNode += timeSpent;
	}
