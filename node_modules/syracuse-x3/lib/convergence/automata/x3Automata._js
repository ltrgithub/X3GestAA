...
function processNode(_, client, reader, node) {
	...
	if (t1) { // trace only read that take at least 1 ms
		timeread = {};
		timeread[node.name] = t1;
		client.activeRequest.timing.binary.read.push(timeread);
	}
	...
	if (t1) { // time to treat the node in local
		timeread = {};
		timeread[node.name] = t1;
		client.activeRequest.timing.server.listenNode.push(timeread);
	}
	timing.end();
