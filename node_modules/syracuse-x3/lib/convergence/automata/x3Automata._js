...
function processNode(_, client, request, reader, node, phase) {
	...
	var timing = perfmon.start(module, "x3Automata.processNode-x3", node.name);
	...
	client.tracer.trace(_, "protocol", "NODE TIMING: " + node.name + ": x3=" + timingInfo.x3 + "s ; cvgSrv=" + timingInfo.cvg + "s", trace);
