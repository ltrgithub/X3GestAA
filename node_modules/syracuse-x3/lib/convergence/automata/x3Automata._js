...
var DESCS = [
	...
	[113, "NDCHVB", "IxUF"], /*  Modif Valeur Bin	*/
...
function processNode(_, client, reader, node) {
	...
	for (var i = 0; i < params.length; i++) {
		if ('AUD'.indexOf(params[i]) >= 0) {
			reader.ensure(_, 2); // ensure that peekShort works
			reader.ensure(_, reader.peekShort() + 2);
			args.push(readVal(_, reader, params[i]));
		}
		else if ('x'.indexOf(params[i]) >= 0) {
			reader.ensure(_, 4); // ensure that peekLong works
			reader.ensure(_, reader.peekLong() + 4);
