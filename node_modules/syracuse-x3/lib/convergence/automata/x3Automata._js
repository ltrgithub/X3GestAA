...
var Desc67 = helpers.defineClass(function() {}, null, {
	init: function() {
		this.DESCS = [
			[0, "ND_RCN", null],
			[1, "ND_GFD", "[pF[U"],
		...
		this.NODES = [];
		this.MAP = {};
		this.ACTION_ENDING = ['NDGIST', 'NDOBOXI', 'NDEXECI', 'ND_ASKUI', 'NDGTIM', 'ND_LIM', 'ND_IHM'];
		var self = this;
		this.DESCS.forEach(function(node) {
			var pat = node[2] || "";
...
var DESC = {
	150067: new Desc67().init()
};


exports.X3Automata = helpers.defineClass(function() {
	this.protocolVersion = 150067; // by default
	this.desc = DESC[150067]; // by default
}, null, {
	setProtocolVersion: function(protocolVersion) {
		this.protocolVersion = protocolVersion;
		// TODO switch to another description
	},
	// !: error
	// F: call function
	// B: read byte
	...
	processNode: function(_, client, request, reader, node, phase) {
		...
		for (i = 0; i < params.length; i++) {
			...
			if ('x'.indexOf(params[i]) >= 0) {
				...
				var extend = this.readVal(_, reader, params[i]);
				var subArgs = [];
				var j = i + 1;
				...
				args.push(this.readVal(_, reader, params[i]));
			}
		...
		var fn = node.skipHandler ? this.dummyHandler : client["listenNode" + node.name];
