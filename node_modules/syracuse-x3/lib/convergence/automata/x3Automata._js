...
function processNode(_, client, reader, node) {
	...
	var results;
	var timing = perfmon.start(module, "x3Automata.processNode", node.name);
...
function process(_, client) {
	var timing = perfmon.start(module, "x3Automata.process");
	var reader, node = NODES[0];
	do {
		var readTiming = perfmon.start(module, "x3Automata.readRequest", "lastNode=" + (client.lastListenNode && client.lastListenNode.name));
