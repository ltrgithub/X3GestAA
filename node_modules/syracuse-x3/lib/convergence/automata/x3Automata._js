...
var DESCS = [
	...
	[177, "NDEXECI", "IULxTxW"], ]; /* FuncIu request */

var NODES = [];
var MAP = {}
var CONNECTING_ENDING = ['NDGIST', 'NDOBOXI', 'NDEXECI', 'ND_ASKUI', 'NDGTIM', 'ND_LIM'];
...
//MAP.ND_ASKUI.waitUser = true; // AskUi replied automatically because not supported in convergence
...
function readPropertiesVal(_, reader, letter) {
	switch (letter) {
		case 'p':
...
function readProperties(_, client, letter) {
	console.log("readProperties");
	var list = [];
	while (true) {
		...
		list.push(readPropertiesVal(_, reader, letter));
	}
}

function readProperties2(_, client, letter) {
	console.log("readProperties2");
	var list = {};
	while (true) {
		var len = client.stream.read(_, 1).readInt8(0);;
		console.log("Len: "+len);
		if (len < 0) throw new Error("bad property2 len: " + len);
		if (len === 0) return list;
		var reader = new X3Reader(client, null, client.stream.read(_, len));
		var prop = readPropertiesVal(_, reader, letter);
		var key = Object.keys(prop)[0];
		list[key] = prop[key];
	}
	client.stream.read(_, 4);
}
...
function processNode(_, client, reader, node) {
	...
	for (var i = 0; i < properties.length; i++) {
		args.push(readProperties(_, client, properties[i]));
	}
	...
	if (client.connecting && CONNECTING_ENDING.indexOf(node.name) > -1) {
		trace && trace("END CONNECTING");
