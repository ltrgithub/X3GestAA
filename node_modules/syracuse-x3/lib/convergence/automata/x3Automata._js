...
function processNode(_, client, reader, node) {
	var timing = perfmon.start(module, "x3Automata.processNode-read", node.name);
	...
	t && client.activeRequest.timing.binary.read.push(t);
	timing = perfmon.start(module, "x3Automata.processNode-apply", node.name);
	...
	timing.end();
	var t = timing.getTiming();
	t && client.activeRequest.timing.server.listenNode.push(t);
