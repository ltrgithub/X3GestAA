...
function processNode(_, client, request, reader, node, phase) {
	...
	var timing = perfmon.start(module, "x3Automata.processNode-x3read", node.name);
	...
	timingInfo.x3read = (t && t.duration) || 0;

	//
	timing = perfmon.start(module, "x3Automata.processNode-cvg", node.name);
	...
	timing.end();
	t = timing.getTiming();
	timingInfo.cvg = (t && t.duration) || 0;
	client.lastListenNode = node;

	timing = perfmon.start(module, "x3automata.autoack");
	var acks = node.acks;
	var ack = 0;
	...
	timing.end();
	t = timing.getTiming();
	timingInfo.x3autoack = (t && t.duration) || 0;

	t && client.timing[phase.title][phase.detail].push(timingInfo);
	client.tracer.trace(_, "protocol", "NODE TIMING: " + node.name + ": x3read=" + timingInfo.x3 + "s ; cvgSrv=" + timingInfo.cvg + "s", trace);
}
...
function process(_, client, request, phase) {
	...
	var timing = perfmon.start(module, "x3Automata.process", phase.title + "_" + phase.detail);
	var reader, node = NODES[0];
	do {
		var readTiming = perfmon.start(module, "x3Automata.readRequest", "lastListenNode " + (client.lastListenNode && client.lastListenNode.name));
		reader = readRequest(_, client);
		readTiming.end();
		var processTiming = perfmon.start(module, "x3Automata.processRequest", "lastListenNode " + (client.lastListenNode && client.lastListenNode.name));
