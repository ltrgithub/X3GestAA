...
function processNode(_, client, request, reader, node, phase) {
	...
	client.tracer && client.tracer.trace(_, "protocol", "Process Node: " + node.name, trace);
	...
	client.tracer && client.tracer.trace(_, "protocol", "NODE HANDLER: " + node.name + ": args=" + JSON.stringify(args.slice(2)), trace);
