...
var traceProtocol = config.x3fusion && config.x3fusion.protocol && config.x3fusion.protocol.trace;
...
function sendAction(_, client, resu, extras, params) {
	...
	client.tracer.enabled && client.tracer.trace(_, "protocol", "Send Action: " + act, traceProtocol);
...
function serializeActionParams(_, client, act, params) {
	...
	function serialize(_, p, d, isExtend) {
		return Buffer.concat(p.slice(0, d.length).map_(_, function(_, param, i) {
			client.tracer.enabled && client.tracer.trace(_, "protocol", "\t- index: " + i + "\n\t- serializer: " + d[i] + "\n\t- param: " + util.inspect(param) + "\n\t- is extend: " + (isExtend === true) + "---\n", traceProtocol);
	...
	client.tracer.enabled && client.tracer.trace(_, "protocol", "Parameters to serialize: " + JSON.stringify(params, null, 2), traceProtocol);
	...
	client.tracer.enabled && client.tracer.trace(_, "protocol", "===\nSerialization detail\n===", traceProtocol);
	...
	if (paramSplited.length > 1) {
		...
		client.tracer.enabled && client.tracer.trace(_, "protocol", "\nProcess extends params one by one (nb=" + _extendsDescr.length + "; descr=" + JSON.stringify(_extendsDescr, null, 2) + ")", traceProtocol);
		// serialized extends 
...
exports.props1Serializer = {
	...
	"write": function(_, client, props) {
		props = exports.props1Serializer.build(props, true);
		if (props.length !== 0) {
			props.forEach_(_, function(_, p) {
				if (p.length !== 0) {
					var bufContent = new Buffer(p[0], "utf8");
					var bufLen = new Buffer(4);
					bufLen.writeInt32BE(bufContent.length, 0);
					client.stream.write(_, Buffer.concat([new Buffer([0]), bufLen, bufContent]));
				}
			});
		}
		var bufEnd = new Buffer(4);
		bufEnd.writeInt32BE(0, 0);
		client.stream.write(_, Buffer.concat([new Buffer([0]), bufEnd]));
	}
};
