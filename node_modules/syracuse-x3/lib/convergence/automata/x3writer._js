...
function serializeActionParams(_, client, act, params) {
	...
	client.tracer.trace(_, "protocol", "Serialize params: " + JSON.stringify(params, null, 2), traceProtocol);
	...
	if (paramSplited.length > 1) {
		var _extends = paramSplited.slice(1);
		client.tracer.trace(_, "protocol", "Number of extends to serialize: " + _extends.length, traceProtocol);
		// serialized extends 
		var offset = std.length;
		var allBuffExt = null;
		for (var i = 0; i < _extends.length; i++) {
			var buffExt = null;
			for (var j in _extends[i]) {
				var b = serialize(_, params.slice(offset++, offset), _extends[i][j], true);
