...
exports.props1Serializer = {
	"build": function(props, arrayOuput, maxProperty1Len) {
		var propsArr = [],
			maxLen, len, nbPack, j, prop, idPack;
		...
		for (var i in props) {
			if (maxProperty1Len > 0) {
				...
				for (j = 0; j < nbPack; j++) {
					idPack = j == 9 ? 11 : j + 1;
					propsArr.push(i + "=" + String.fromCharCode(idPack) + prop.substr(j * maxLen, maxLen) + "\n");
				}
			} else {
				propsArr.push(i + "=" + String.fromCharCode(1) + prop + "\n");
			}
		}
		return arrayOuput ? propsArr : propsArr.join('');
	},
	"write": function(_, client, props) {
		var buf, propsStr = exports.props1Serializer.build(props, false, 35);
