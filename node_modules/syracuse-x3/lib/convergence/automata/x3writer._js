...
exports.X3Writer = helpers.defineClass(function(protocolVersion) {
	...
	this.serializers = {
		...
		X: function(val, isExtend) { // table of cvgParam
			var countBuf = self.serializers.getCvgParamLenBuffer(val, isExtend, 2);
			var valsBuf = self.serializers.getCvgParamValueBuffer(val, false);
			//console.log("VALSBUF=" + valsBuf.toString('hex'));
		...
		Y: function(val, isExtend) { // table of cvgParam
			var countBuf = self.serializers.getCvgParamLenBuffer(val, isExtend, 2);
			var valsBuf = self.serializers.getCvgParamValueBuffer(val, true);
			//console.log("VALSBUF=" + valsBuf.toString('hex'));
			var b = Buffer.concat([countBuf, valsBuf]);
			console.log(b);
			return b;
		...
		Z: function(val, isExtend) { // table of cvgParam
			var countBuf = self.serializers.getCvgParamLenBuffer(val, isExtend, 4);
			var valsBuf = self.serializers.getCvgParamValueBuffer(val, true);
			//console.log("VALSBUF=" + valsBuf.toString('hex'));
			return Buffer.concat([countBuf, valsBuf]);
		},
		getCvgParamValueBuffer: function(val, addIndex) {
			return Buffer.concat(val.map(function(v) {
				if (addIndex)
					return Buffer.concat([self.serializers.S(v.num), self.serializers.S(v.poste), self.serializers.S(0), v.resu.getBytes()]);
				else
					return Buffer.concat([self.serializers.S(v.num), self.serializers.S(v.poste), v.resu.getBytes()]);
			}));
		},

		getCvgParamLenBuffer: function(val, isExtend, buffLength) {
			if (!Array.isArray(val)) throw new Error("action parameter type mismatch, expected array, got " + typeof val);
			var countBuf;
			if (isExtend) {
				countBuf = new Buffer(buffLength);
				if (buffLength >= 4)
