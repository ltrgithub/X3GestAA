...
function sendAction(_, client, resu, extras, params) {
	...
	client.tracer && client.tracer.trace(_, "protocol", "Send Action: " + act, traceProtocol);
...
function serializeActionParams(_, client, act, params) {
	...
	function serialize(_, p, d, isExtend) {
		return Buffer.concat(p.slice(0, d.length).map_(_, function(_, param, i) {
			client.tracer && client.tracer.trace(_, "protocol", "\t- index: " + i + "\n\t- serializer: " + d[i] + "\n\t- param: " + util.inspect(param) + "\n\t- is extend: " + (isExtend === true) + "---\n", traceProtocol);
	...
	client.tracer && client.tracer.trace(_, "protocol", "Parameters to serialize: " + JSON.stringify(params, null, 2), traceProtocol);
	...
	client.tracer && client.tracer.trace(_, "protocol", "===\nSerialization detail\n===", traceProtocol);
