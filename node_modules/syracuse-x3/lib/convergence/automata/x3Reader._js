...
var X3Reader = exports.X3Reader = helpers.defineClass(function(client, wnd, data) {
	...
	nextByte: function(lenient) {
		var d = this.data;
		if (this.pos < d.length) {
			return d[this.pos++];
		} else {
			if (!lenient) throw new Error("nextByte");
			return 0;
		}
	},
	nextShort: function() {
	...
	nextResu: function(_) {
		var tag = this.nextByte();
		var len = this.nextByte();
		var tdo = CvgResuTagTdo[tag];
		if (tdo !== undefined) {
			switch (tdo) {
			case 0: // NULL
				return new CvgResu(tdo, null);
			case 2: // SHORT
				return new CvgResu(tdo, this.nextShort());
			case 3: // INT
				return new CvgResu(tdo, this.nextLong());
			case 4: // DCB
				if (len !== 17) throw new Error("bad bcd len: " + len);
				return new CvgResu(tdo, this.nextBcd());
			case 1: // LIB:
			case 7: // STRU:
				return new CvgResu(CvgResuTdo.STRU, this.nextUnicode(_, len));
			case 8: // DATE:
				if (len !== 3) throw new Error("bad date len: " + len);
				return new CvgResu(tdo, this.nextDate());
			case 9: // BLOB:
				return new CvgResu(tdo, this.nextBlob(_));
			case 10: // CLOB
				return new CvgResu(tdo, this.nextClob(_));
			}
		}
		throw new Error("bad resu tag: " + tag + " (" + String.fromCharCode(tag) + ")");
	},
