...
var CvgDecimal = require('../types/CvgDecimal').CvgDecimal;
var CvgDate = require('../types/CvgDate').CvgDate;
...
var X3Reader = exports.X3Reader = helpers.defineClass(function(client) {
	...
	nextProperties: function(map, len, isProperties2) {
		...
		var applyOn = "key";
		var nextIsIdx = false;
		for (var i = this.pos; i < this.pos + len; i++) {
	...
	nextResuTable2: function(_, deserialize) {
		var len = 0;
		if (deserialize) {
			len = this.nextByte();
		} else {
	...
	nextStruct: function(_) {
		return this.getStructs(_, this.nextShort(), "getStructShort");

	},
	nextStruct2: function(_) {
		return this.getStructs(_, this.nextShort(), "getStructShort2");

	},
	nextStruct3: function(_) {
		return this.getStructs(_, this.nextLong(), "getStructShort2");
	},
	getStructShort: function(_) {
		return {
			num: this.nextShort(),
			poste: this.nextShort()
		};
	},
	getStructShort2: function(_) {
		var level = this.nextShort();
		var res = this.getStructShort(_);
		res.level = level;
		return res;
	},
	getStructs: function(_, len, handlerStructShort) {
		var tbl = [];
		for (var i = 0; i < len; i++) {
			var params = this[handlerStructShort](_);
