...
var X3Reader = exports.X3Reader = helpers.defineClass(function(ctx, wnd, data) {
	...
	nextResu: function(_) {
		...
		switch (tag) {
			case CvgResu.TAG_NULL:
				return new CvgResu(CvgResu.TDO_NULL, null);
			case CvgResu.TAG_LIBELLE:
				throw new Error("Resu LIBELLE not yet implemented !");
			case CvgResu.TAG_SHORT:
				return new CvgResu(CvgResu.TDO_SHORT, this.nextShort());
			case CvgResu.TAG_INT:
				return new CvgResu(CvgResu.TDO_INT, this.nextLong());
			case CvgResu.TAG_BCD:
				if (len !== 17) throw new Error("bad bcd len: " + len);
				return new CvgResu(CvgResu.TDO_BCD, this.nextBcd());
			case CvgResu.TAG_STRU:
				return new CvgResu(CvgResu.TDO_ALPHA, this.nextUnicode(_, len));
			case CvgResu.TAG_DATE:
				if (len !== 3) throw new Error("bad date len: " + len);
				return new CvgResu(CvgResu.TDO_DATE, this.nextDate());
			case CvgResu.TAG_BLOB:
				return new CvgResu(CvgResu.TDO_BLOB, this.nextBlob());
			case CvgResu.TAG_CLOB:
				return new CvgResu(CvgResu.TDO_CLOB, this.nextClob());
			default:
				throw new Error("bad resu tag: " + tag + " (" + String.fromCharCode(tag) + ")");
		}
	},
	nextResuTable: function(_) {
		var len = this.nextShort();
		var tbl = [];
		for (var i = 0 ; i < len ; i++) {
	...
	nextClob: function() {
		...
		if (missing > 0) this.data = Buffer.concat([this.data, this.ctx.stream.read(_, missing)]);
	},
});