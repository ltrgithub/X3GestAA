...
var CAdapiDataSrc = helpers.defineClass(function(aAdapiRequester, aAdapiScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	...
	getJsonMetadata: function(map, metadata) {
		...
		for (var key in keys) {
			...
			if (item instanceof CAdapiVariable) {
				if (item.getSrcLine().equals(this.entrySup)) {
					// it's a colomn
					itemJs.put(0);
				} else {
					// it's a cell  retrinve line number
					var lineNumber = -1;
					if ( item.getSrcLine() instanceof CAdapiDataSrcLine) {
						lineNumber = item.getSrcLine().getOneBaseLineIdx();
					}
					itemJs.put(lineNumber);
				}
			} else if (item instanceof CAdapiDataSrcLine) {
				// it's a line
				itemJs.put(item.getOneBaseLineIdx());
			}
	...
	toJson: function() {
		...
		// Modif DataSrc
		if (hasModifTitle()) {
			object.put('tit', getTitle());
			if (getTitleSty() != null) {
				object.put('stytit', getTitleSty());
			}
		}
		if (hasModifStyle()) {
			object.put('sty', getStyle());
		}
		return object;
	}
