...
var AdapiLineMove = helpers.defineClass(function(lefList) {
...
var CAdapiDataSrc = helpers.defineClass(function(aAdapiRequester, aAdapiScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	dataSrcType = dataSrcType !== undefined ? dataSrcType : CAdapiDataSrc.TYPE_SCREEN;
	TODO_SUPER(aAdapiRequester.adapiClient, aDataSourceID, aEntityIndexBaseOne, EntityType.BLOC);
	...
	// En dernier
	setTypeJsonNode('ENTITY');
}, CAdapiEntity, {
	_traceLines: function() {
		var trace = false;
		if (trace) {
			Utils.toConsole("\tSize[" + this.id + "]=" + size());
			for (var l in this.lines) {
				Utils.toConsole("\t" + l.zeroBaseLineIdx + " - " + l.dataSrcAdxId);
			}
		}
	},
	...
	buildMessErrorTraverse: function(aStructIst, aAdapiSetter, aKeyOfMessSuffix) {
		...
		wMess = CStringFacilities.sprintf(wMess, aStructIst.alphanumCodage, this.adxId, CStringFacilities.getClasseName(aAdapiSetter.class));
		return wMess + this.adapiClient.adapiResources.getMessage(aKeyOfMessSuffix);
	},
	fusionMvtsDelete: function(nbLines, idx) {
		// Utils.toConsole(this.id + " - Mvt delete - nbLines[" + nbLines + "] - idx[" + idx + "]");
	...
	fusionMvtsInsert: function(nbLines, idx) {
		// Utils.toConsole(this.id + " - Mvt insert - nbLines[" + nbLines + "] - idx[" + idx + "]");
		this.listEventLine.addInsert(nbLines, idx);
		_traceLines();
	},
	fusionMvtsRemoveAll: function() {
		// Utils.toConsole(this.id + " - Mvt RemoveAll");
	...
	fusionMvtsReset: function() {
		// Utils.toConsole(this.id + " - Mvt Reset");
		this.listEventLine.reset();
	},
	getAdxId: function() {
		return this.adapiScreen == null ? null : new StringBuilder().append(this.adapiScreen.adxId).append(',').append(this.entityIndexBaseOneInAlphanumFormat).toString();
	},
	...
	getJsonMetadata: function(map, metadata) {
		...
		for (var key in keys) {
			...
			var obsId = item.adapiObservableId;
			if (item instanceof CAdapiDataSrcLine) {
				obsId = this.adapiObservableId;
			}
			...
			if (item instanceof CAdapiVariable) {
				if (item.srcLine.equals(this.entrySup)) {
					...
					if ( item.srcLine instanceof CAdapiDataSrcLine) {
						lineNumber = item.srcLine.oneBaseLineIdx;
					}
					itemJs.put(lineNumber);
				}
			} else if (item instanceof CAdapiDataSrcLine) {
				// it's a line
				itemJs.put(item.oneBaseLineIdx);
			}
			// indice colomn, cellule
			switch (metadata) {
			case STY:
				itemJs.put(item.style); // style
				break;
			case STT:
				itemJs.put(item.state); // style
				break;
			case FMT:
				if (item.format != null) {
					itemJs.put(item.format.string); // style
				}
				break;
			case TIT:
				if (item.title != null) {
					itemJs.put(item.title); // style
				}
				break;
			}
			meta.put(itemJs);
		...
		try {
			...
			// inferieure designee
				...
				if (wI < 0 || wI >= this.lines.size()) {
					...
					wMess = CStringFacilities.sprintf(wMess, String.valueOf(wI), String.valueOf(this.lines.size()), this.adxId);
