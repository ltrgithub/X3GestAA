...
var X3Date = require('syracuse-x3/lib/fusion/types/X3Date').X3Date;
...
var CStructResu = exports.CStructResu = helpers.defineClass(function(type, val) {
	...
	isBCD: function() {
		return this.type === CStructResu.TDO_BCD;
	},
	isInt: function() {
	...
	getBytes: function() {
		...
		switch (this.type) {
			...
			case CStructResu.TDO_BCD:
				var bufDcb = BCD.toBytes(val);
				var buf = new Buffer(2);
				buf[0] = 0x2f;
				buf[1] = 0x11;
				return Buffer.concat([buf, bufDcb], 19);
			case CStructResu.TDO_DATE:
				var days = X3Date.toDays(val);
				return new Buffer([CStructResu.TAG_DATE, 3, (days >> 16) & 0xff, (days >> 8) & 0xff, days & 0xff]);
			default:
				throw new Error("NIY resu to bytes: " + this.type);
		}
	},
	clone: function() {
		return new CStructResu(this.type, this.value)
	},
});
...
CStructResu.read = function(reader) {
	...
	switch (tag) {
		...
		case CStructResu.TAG_LIBELLE:
			throw new Error("Resu LIBELLE not yet implemented !");
		case CStructResu.TAG_SHORT:
			return new CStructResu(CStructResu.TDO_SHORT, reader.nextShort());
		case CStructResu.TAG_INT:
			return new CStructResu(CStructResu.TDO_INT, reader.nextLong());
		case CStructResu.TAG_BCD:
			if (len !== 17) throw new Error("bad bcd len: " + len);
			return new CStructResu(CStructResu.TDO_BCD, reader.nextBcd());
		case CStructResu.TAG_STRU:
			return new CStructResu(CStructResu.TDO_ALPHA, reader.nextUnicode(len));
		case CStructResu.TAG_DATE:
			if (len !== 3) throw new Error("bad date len: " + len);
			return new CStructResu(CStructResu.TDO_DATE, reader.nextDate());
		case CStructResu.TAG_BLOB:
			return new CStructResu(CStructResu.TDO_BLOB, reader.nextBlob());
		case CStructResu.TAG_CLOB:
			return new CStructResu(CStructResu.TDO_CLOB, reader.nextClob());
		default:
