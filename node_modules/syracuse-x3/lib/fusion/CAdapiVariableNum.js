...
var CAdapiVariableNum = exports.CAdapiVariableNum = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aFormat, aEntityState) {
	...
	formatThousand: function(aEntier, aSymbolThousand) {
		var wResult = null;
		if (aEntier.length > 3) {
			wResult = integer.format(aEntier, "#,###")
		} else {
			wResult = aEntier;
		}
		return wResult;
	...
	formatX3ToDisplay: function(aData) {
		if (aData.isNum()) {
			...
			if (!aData.isNull()) {
				var wMess = getRsrcMess(CAdapiResources.KEYMESS_ERROR_RESU_NOT_NUMERIC, this.id, CStructResu.x3typeToString(aData.resuTyp), this.format.string);
	...
	unformatHimself: function(aValue) {
		...
		for (wI = 0; wI < wLen; wI++) {
			...
			if (CAdapiVariable.isDigit(wCurChar)) {
				wSB += wCurChar;
