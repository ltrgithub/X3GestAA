...
var AdapiMessageObersable = helpers.defineClass(function(message, client) {
	...
	getTypeJsonNode: function() {
		return 'TARGET'; // mess superviseur/X3
	},
	toJson: function() {
		...
		box.put("type", this.message.typ);
		box.put("def", this.message.defBtn);
		// box.put("to",get());
		box.put("tit", this.message.title);
		var li = new JSONArray();
		li.put(this.message.mess);
		box.put("li", li);
		box.put("to", this.message.userMaxSleep);
		target.put("box", box);
		return target;
	}
...
var CAdapiMessage = helpers.defineClass(function(aType, clientObs, aMessage, notify) {
	TODO_SUPER(clientObs.tracer, aType, aMessage);
	this.action = -1;
...
/* TODO overload
	public CAdapiMessage(aType, aAction, aMessage, aTitre, aDefBtn, aSleep, aChx, clientObs) {
		TODO_SUPER(clientObs.tracer, aType, aMessage, aTitre, aDefBtn, aSleep, aChx);
		this.action = aAction;
	...
	getTypeInString: function() {
		var wLibType = CAdapi.affSynchroneTypeToString(this.typ);
		if (wLibType != null) {
			return wLibType;
		}
		return TODO_SUPER.typeInString;
	},
