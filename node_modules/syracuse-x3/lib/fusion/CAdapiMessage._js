"use strict"; var helpers = require('syracuse-core/lib/helpers');
	...
	var AdapiMessageObersable = helpers.defineClass(function() {}, AdapiObservable, {
		private CAdapiMessage message = null;

		private AdapiMessageObersable(message, client) {
			TODO_SUPER(client);
			...
			return ETypeJsonNode.TARGET;// mess superviseur/X3
				...
				box.put("type", message.getTyp());
				box.put("def", message.getDefBtn());
				// box.put("to",get());
				box.put("tit", message.getTitle());
				var li = new JSONArray();
				li.put(message.getMess());
				box.put("li", li);
				box.put("to", message.getUserMaxSleep());
				target.put("box", box);
				return target;
		}
...
var CAdapiMessage = helpers.defineClass(function() {}, CAdonixMessage, {
	...
	private int this.action = -1;
	private ArrayList<String> this.webServerLocicalMess = null;

	public CAdapiMessage(aType, clientObs, aMessage,notify) {
	...
	void addWebServerLocicalMess(ArrayList<String> aListOfLocicalMess) {
		if (aListOfLocicalMess != null) {
			if (this.webServerLocicalMess == null) {
				this.webServerLocicalMess = new ArrayList<String>();
			}
		...
		return new CAdapiMessage(getTyp(), this.action, getMess(), this.title, this.btn, this.sleep, this.chx, clientObs);
	}

	protected ArrayList<String> getListOfWebServerLocicalMess() {
