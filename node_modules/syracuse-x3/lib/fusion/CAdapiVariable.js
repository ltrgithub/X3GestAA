...
var CAdapiVariable = exports.CAdapiVariable = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVariableName, aVariableId, aDataType, aFormat, aEntityState) {
	CAdapiEntity.call(this, srcLine && srcLine.client, aVariableId, aEntityIndexBaseOne, 'FIELD');
	this.srcLine = null;
	...
	buildMessFormterExcetion: function(aMessKey, aWhatFormatKey, aValue, e) {
		...
		return this.buildMessFormterExcetion(aMessKey, aWhatFormatKey, aValue, e);
	},
/* TODO overload
	...
	callFormater: function(aValue) {
		try {
			...
			this.client.addLogicalErrorMess(this, "callFormater", this.getAdxId(), buildMessFormterExcetion(CAdapiResources.KEYMESS_ERROR_FORMAT_EXCEPTION, CAdapiResources.KEYMESS_FORMAT_RTOS, aValue, e));
		}
	...
	callUnFormater: function(aValue) {
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				throw new CAdapiException(this.client, e, buildMessFormterExcetion(CAdapiResources.KEYMESS_ERROR_UNFORMAT_EXCEPTION, CAdapiResources.KEYMESS_UNFORMAT_STOR, aValue, e), true);
			}
		}
		// No format
	...
	cloneValues: function(v) {
		CAdapiEntity.prototype.cloneValues.call(this, v);
		...
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				try {
					...
					throw new CAdapiException(this.client, e, buildMessFormterExcetion(CAdapiResources.KEYMESS_ERROR_FORMAT_EXCEPTION, CAdapiResources.KEYMESS_FORMAT_ETOS, aValue, e), true);
				}
			}
			return res;
		} else {
	...
	formatX3ToEdit: function() {
		...
		var res = val;
		if (this.acceptEditFormat() && this.mustBeFormated && val != null) {
			if (this.hasValidFormat() && this.client.hasValidFormater()) {
				try {
					res = this.client.formater.inputGetFocusInValue(this.format, val, true);
				} catch (e) {
					this.client.addLogicalErrorMess(this, "callUnformaterToEdit", this.id, buildMessFormterExcetion(CAdapiResources.KEYMESS_ERROR_UNFORMAT_EXCEPTION, CAdapiResources.KEYMESS_UNFORMAT_STOE, val, e));
				}
			}
		}
		return res;
	...
	getIdxInBloc: function() {
		var wIdx = this.id;
		var wPos = 0;
		while (!CAdapiVariable.isDigit(wIdx.charAt(wPos))) {
			wPos++;
		}
	...
	setServerFormat: function(aFormat) {
		var wIndferredState = CAdapiEntity.prototype.setServerFormat.call(this, aFormat);
		this.mustBeFormated = this.hasValidFormat();
	...
	setServerValue: function(aResu) {
		if (this.browserValue == null || this.browserValue !== aResu) {
			if (!this.hasServerValueModification() || !this.isServerValueEquals(aResu)) {
				CAdapiEntity.prototype.setServerValue.call(this, aResu);
			} else {
				this.forceServerValueModification(true);
			}
		} else if (this.mustBeFormated) {
			CAdapiEntity.prototype.setServerValue.call(this, aResu);
			this.resetServerModified();

		} else if (this.hasServerValueModification()) {
			CAdapiEntity.prototype.setServerValue.call(this, this.browserValue);
			this.resetServerModified();
		}
	},
	setUnModifiableServerValue: function(aResu) {
		this.storeBrowserValue(null);
		return CAdapiEntity.prototype.setUnModifiableServerValue.call(this, aResu);
	},
...
CAdapiVariable.isDigit = function(ch) {
	return ch >= '0' && ch <= '9';
}