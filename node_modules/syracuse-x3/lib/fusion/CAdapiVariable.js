...
/* TODO overload
	...
	callFormater: function(aValue) {
		try {
			...
			this.client.addLogicalErrorMess(this, "callFormater", this.getAdxId(), buildMsgFormatterException(CAdapiResources.KEYMESS_ERROR_FORMAT_EXCEPTION, CAdapiResources.KEYMESS_FORMAT_RTOS, aValue, e));
		}
	...
	callUnFormater: function(aValue) {
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				throw new CAdapiException(this.client, e, buildMsgFormatterException(CAdapiResources.KEYMESS_ERROR_UNFORMAT_EXCEPTION, CAdapiResources.KEYMESS_UNFORMAT_STOR, aValue, e), true);
			}
		}
		// No format
		...
		if (this.client.hasValidFormater() && this.hasValidFormat()) {
			try {
				...
				try {
					...
					throw new CAdapiException(this.client, e, buildMsgFormatterException(CAdapiResources.KEYMESS_ERROR_FORMAT_EXCEPTION, CAdapiResources.KEYMESS_FORMAT_ETOS, aValue, e), true);
				}
			}
			return res;
		} else {
		...
		if (this.acceptEditFormat() && this.mustBeFormatted && val != null) {
			if (this.hasValidFormat() && this.client.hasValidFormater()) {
				try {
					...
					this.client.addLogicalErrorMess(this, "callUnformaterToEdit", this.id, buildMsgFormatterException(CAdapiResources.KEYMESS_ERROR_UNFORMAT_EXCEPTION, CAdapiResources.KEYMESS_UNFORMAT_STOE, val, e));
				}
			}
		}
		return res;
	...
	setServerValue: function(aResu) {
		//if (aResu && typeof aResu.value  === "string" && aResu.value.substring(0, 4) == 'Comm') throw new Error(aResu.value + "#" + this.serverValue);
