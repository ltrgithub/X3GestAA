...
var CAdapiVariable = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVariableName, aVariableId, aDataType, aFormat, aEntityState) {
	TODO_SUPER(srcLine.adapiClient, aVariableId, aEntityIndexBaseOne, EntityType.FIELD);
	this.srcLine = null;
	...
	this.resuType = CStructResu.tdoNumToResuTyp(this.tracer, this.dataType);
	setFormat(aFormat);
	// En dernier
	...
	callFormater: function(aValue) {
		try {
			...
			this.adapiClient.addLogicalErrorMess(this, "callFormater", this.adxId, buildMessFormterExcetion(CAdapiResources.KEYMESS_ERROR_FORMAT_EXCEPTION, CAdapiResources.KEYMESS_FORMAT_RTOS, aValue, e));
		}
	...
	formatX3ToEdit: function() {
		var val = this.serverValResu != null ? this.serverValResu.value : null;
		var res = val;
	...
	getAdxId: function() {
		...
		sb.append(this.srcLine.dataSrcAdxId).append(this.idxInBloc).append(',').append(String.valueOf(this.srcLine.oneBaseLineIdx));
		return sb.toString();
	...
	getDisplayValue: function() {
		var res = this.serverValResu != null ? formatX3ToDisplay(this.serverValResu) : EMPTY;
	...
	razModification: function() {
		storeBrowserValue(this.serverValResu);
	...
	toJson: function() {
		...
		if (hasModifState()) {
			object.put('stt', this.state);
		}
		if (hasModifStyle() && this.style != null && this.style.length() != 0) {
			object.put('sty', this.style); // style presentation
		}
		if (hasModifTitle()) {
