...
var CAdapiEntity = helpers.defineClass(function() {}, AdapiObservable, {

	public static enum EntityType {
		BLOC, FIELD, ROW, SCREEN, WINDOW;
	}

	public enum FusionEntityState {
		ACTIF(0x0001, 0x00010000, 0xFFFFFFFE), READONLY(0x0002, 0x00020000, 0xFFFFFFFD), VISIBLE(0x0004, 0x00040000, 0xFFFFFFFB), SELECTED(0x0008, 0x00080000, 0xFFFFFFF7);

		public static int razAtt(status) {
			return status & 0x0000FFFF;
		}

		private int this.maskVal;
		private int this.maskAtt;
		private int this.maskNot;

		private FusionEntityState(maskVal, maskAtt, maskNot) {
	...
	private static int STATE_NOT_VISIBLE = 2;
	private static int STATE_ENABLE = 4;
	private static int STATE_NOT_ENABLE = 8;
	private static int STATE_SELECT = 16;
	private static int STATE_NOT_SELECT = 32;
	private static int STATE_READONLY = 64;
	private static int STATE_NOT_READONLY = 128;
	public final static int STATE_DEFAULT = 0;
	//private static int STATE_GRIZO = 4;
	// Etats reçu du serveur X3
	private static int X3_SRV_GRIZO = 0;
	private static int X3_SRV_ACTZO = 1;
	private static int X3_SRV_DISZO = 2;
	// Variables X3 - attribut MAF du xml
	private static int X3VAR_MAF_AFFICHE = 2;
	private static int X3VAR_MAF_INVISIBLE = 3;
	protected final static int X3VAR_MAF_SAISI = 1;
	private static int X3VAR_MAF_TECHNIQUE = 4;

	// Variables - Mode d'affichage - Reprise Web126
	public static int x3AffModeToEntityState(mode) {
		switch (mode) {
	...
	// Testé avec CMichel
	public static int x3State2EntityState(aX3State) {
		switch (aX3State) {
		case X3_SRV_GRIZO:
	...
	public static String x3StateToString(aServerState) {
		switch (aServerState) {
		case X3_SRV_GRIZO:
			return "SRV_GRIZO";
		case X3_SRV_ACTZO:
	...
	private int this.entityIndexBaseOne;
	private int this.entityState = STATE_DEFAULT;
	private boolean this.isTitleModified = false;
	private EntityType this.entityTyp;
	private CAdapiFormat this.format = null;
	private CAdapiEntity this.columnFormater = null;
	private String this.id;
	private boolean this.srvValueModified = false; // true by default
	private int this.lastState = STATE_DEFAULT;
	private boolean this.formatModified =false;
	private boolean this.styleModified = false;
	private CStructResu this.serverValue;
	private boolean this.stateModified = false;
	private String this.style = EMPTY;
	// Label champ, titre fenetre
	private String this.title = null;
	private String this.titleStyle = null;
	private long lastStampModState = -1;
	private long lastStampModSty = -1;
	private long lastStampModFmt = -1;
	private long lastStampModTit = -1;
	private String this.entityIdxAlpha = null;

	public CAdapiEntity(aAdapiClient, aId, aEntityIndexBaseOne, aEntityTyp) {
		TODO_SUPER(aAdapiClient);
		...
		var w26E1 = this.entityIndexBaseOne / 26;
		if (w26E1 > 0) {
			wSB.append((char) ('@' + w26E1));
			wSB.append((char) ('@' + this.entityIndexBaseOne % 26));
		} else {
			wSB.append((char) ('@' + this.entityIndexBaseOne));
		}
		this.entityIdxAlpha = wSB.toString();
	...
	}, cloneValues: function(var) {
		var.this.style = this.style;
		var.setLastStampMod(System.currentTimeMillis(), FusionMetaData.STY);
		if (this.serverValue != null) {
			var.this.serverValue = (CStructResu) this.serverValue.clone();
		}
	}, dispose: function() {
		TODO_SUPER.dispose();
	...
	}, entityStateSet: function(aNewState, force) {
		var old = this.entityState;
		var niou = calcJsonNewEntityState(this.entityState, aNewState);
		if (niou != old) {
			setLastStampMod(System.nanoTime(), FusionMetaData.STT);
		}
		if (force || this.entityState != niou) {
			this.entityState = niou;
			this.stateModified = niou != this.lastState;
		}
	}, forceServerFormat: function(aServerFormat) {
		if (this.format == null || aServerFormat.getString().equals(this.format.getString())) {
	...
	, getAdapiObservableId: function() {
		...
		if (ist != null) {
			...
			if (this.entityTyp != EntityType.WINDOW) {
				ist.getScreenAlphaIdFusion(fullId);
				if (this.entityTyp != EntityType.SCREEN) {
					ist.getBlocAlphaId(fullId);
					if (this.entityTyp == EntityType.FIELD) {
						fullId.append(ist.getField());
					}
				}
			}
	...
	public , getAdxId: function();

	// Todo - Optimiser sur modif adxId
	, getAdxIst: function() {
		return new CStructIst(getTracer(), getAdxId());
	}, getEntityIndexBaseOne: function() {
		return this.entityIndexBaseOne;
	}, getEntityIndexBaseOneInAlphanumFormat: function() {
		return this.entityIdxAlpha;
	}, getEntityTyp: function() {
		return this.entityTyp;
	}, getFatherWindowId: function() {
		// using identity id to find the windows
		return getAdxId() == null ? null : getAdxIst().getWinAlphaId();
	}, getFormat: function() {
		return this.format;
	}, getId: function() {
		return this.id;
	...
	, getJsonKey: function() {
		...
		if (this.entityTyp == EntityType.WINDOW) {
			return ist.getWinAlphaId();
		}
		if (this.entityTyp == EntityType.SCREEN) {
			return ist.getScreenAlphaIdFusion();
		}
		var jsonKey = new StringBuilder();
		ist.getScreenAlphaIdFusion(jsonKey);
		ist.getBlocAlphaId(jsonKey);
		if (this.entityTyp == EntityType.FIELD) {
			jsonKey.append(ist.getField());
		}
		return jsonKey.toString();
	}, getLastStampModFmt: function() {
		return lastStampModFmt;
	}, getLastStampModState: function() {
		return lastStampModState;
	}, getLastStampModSty: function() {
		return lastStampModSty;
	}, getLastStampModTit: function() {
		return lastStampModTit;
	}, getRsrcMess: function(id) {
		return this.adapiClient.getAdapiResources().getMessage(id);
	}, getServerValInt: function() {
		return this.serverValue != null ? this.serverValue.getIntValue() : -1;
	}, getServerValResu: function() {
		return this.serverValue;
	}, getServerValStr: function() {
		return this.serverValue != null ? this.serverValue.getValue() : EMPTY;
	}, getState: function() {
		...
		return this.title;
	}, getTitleSty: function() {
		return this.titleStyle;
	}, getTracer: function() {
		return this.adapiClient != null ? this.adapiClient.getTracer() : CTracerNull.getInstance();
	...
	, hasServerValueModification: function() {
		...
		return FusionEntityState.ACTIF.isOn(this.entityState);
	}, isStateVisible: function() {
		return FusionEntityState.VISIBLE.isOn(this.entityState);
		...
		this.entityState = FusionEntityState.razAtt(this.entityState);
		this.lastState = this.entityState;
	...
	, setColumnFormater: function(formatRefCell) {
		if (this.format == null || this.format.getString().equals(formatRefCell.getFormat().getString())) {
			setLastStampMod(System.nanoTime(), FusionMetaData.FMT);
		}
		...
		if (hasColumnFormater()) {
			...
			this.format = this.columnFormater.getFormat();
		}
		lastStampModFmt = System.nanoTime();
	...
	, setLeftListLineSelected: function(selected, force) {
		entityStateSet(selected ? STATE_SELECT : STATE_NOT_SELECT, true);
	}, setServerFormat: function(aFormat) {
		this.formatModified=true;
		if (this.format == null || !aFormat.getString().equals(this.format.getString())) {
			setLastStampMod(System.nanoTime(), FusionMetaData.FMT);
		}
		this.format = aFormat;
		...
		if (wChangeVisibility) {
			...
			entityStateSet(wInferredState, false);
			if (hasColumnFormater()) {
				this.columnFormater.entityStateSet(wInferredState, false);
	...
	, setServerStyle: function(aStyle, applyOn) {
		...
		if (!this.formatModified && this.columnFormater != null && !this.columnFormater.equals(this.format)) {
			forceServerFormat(this.columnFormater.getFormat());
		}
		forceChange();
	}

	// State  pour les variables - Attribut MAF
	, setStateFromDescr: function(entityState) {
		this.entityState = calcJsonNewEntityState(this.entityState, entityState);
	}
	...
	, setStyleFromDescr: function(aStyle) {
		...
		if (this.format == null || !this.format.getString().equals(aFormat.getString())) {
			lastStampModFmt = System.nanoTime();
		}
		this.format = aFormat;
	}
