...
var CAdapiLeftListLineNode = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, idx) {
	...
	clone: function() {
		...
		var wClone = new CAdapiLeftListLineNode(getAdapiRequester(), getDataSrc(), getId(), getNbVar(), idx);
		cloneVars(wClone);
		return wClone;
	},
	...
	setColumns: function(aIdx, aText, aClf) {
		...
		var v = this.getVar(0);
		if (v != null) {
			v.setUnModifiableServerValue(new CStructResu(getTracer(), CStructResu.X3TYP_STRU, aText));
		}
	},
