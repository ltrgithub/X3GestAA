...
var CAdapiStackedWindow = exports.CAdapiStackedWindow = helpers.defineClass(function(aAdapiClient, aWindowId, aFunctionId, aStamp, aEnityIndexBaseOne) {
	...
	if (hasDescrDom()) {
		var timestamp = this.descrDom.documentElement.getAttribute("TIM");
		if (timestamp == null || !timestamp.equals(aStamp)) {
			//reload description
			loadDesrDom(aWindowId, true);
			aStamp = this.descrDom.documentElement.getAttribute("TIM");
		}
	...
	getAdxId: function() {
		return this.entityIndexBaseOneInAlphanumFormat;
	},
	hasDescrDom: function() {
		return this.descrDom != null;
	},
	initAdapiScreens: function() {
		newScreens(this.adapiClient, this, this.descrDom, this.adapiScreens, this.listOfDataSrcs);
	},
	...
	setOpened: function(opened) {
		this.opened = opened;
		// En dernier
		setJsonType('FUNC'); // catch close ou open fonction
	},
	...
	toJson: function() {
		...
		if (isOpened()) {
			winO.put('name', this.id);
			winO.put('scrnNum', this.screenToShow >= 1 ? this.screenToShow : 1);
			winO.put('stamp', this.timeStamp);
			winO.put('extend', this.ndOWinComplementDescr);
			winO.put('func', this.functionId);
			winO.put('type', this.adapiWindow.type);
			...
			if (extModel != null) {
				...
				for (var item in extModel.entrySup.vars) {
					...
					itemJs.put('tit', item.title);
					itemJs.put('fmt', item.format != null ? item.format.string : "");
	...
	whichListnerAmI: function() {
		return CStringFacilities.getClasseName(this.class);
	}
});

CAdapiStackedWindow.newScreens = function(aAdapiClient, aAdapiRequesterWindow, aWindowDescription, aAdapiScreens, aListOfDataSrcs) {
	var wScreensNodes = aWindowDescription.getElementsByTagName(TAG_SCRN);
	var wMax = wScreensNodes.length;
	var wScreenNode;
	var wAdapiScreen;
	// String wValue;
	var wI = 0;
