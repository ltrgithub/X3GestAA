...
var CAdapiLeftListLine = require('syracuse-x3/lib/fusion/model/CAdapiLeftListLine').CAdapiLeftListLine;
var CAdapiLeftListLineLevel = require('syracuse-x3/lib/fusion/model/CAdapiLeftListLine').CAdapiLeftListLineLevel;
var CAdapiVariableFactory = require('syracuse-x3/lib/fusion/model/CAdapiVariableFactory').CAdapiVariableFactory;
var CAdapiLeftListRecursiv = require('syracuse-x3/lib/fusion/model/CAdapiLeftListRecursiv').CAdapiLeftListRecursiv;
var CAdapiLeftListHierarchic = require('syracuse-x3/lib/fusion/model/CAdapiLeftListHierarchic').CAdapiLeftListHierarchic;
var CAdapiLeftListLineNode = require('syracuse-x3/lib/fusion/model/CAdapiLeftListLineNode').CAdapiLeftListLineNode;

var CAdapiScreenLeftList = exports.CAdapiScreenLeftList = helpers.defineClass(function(aRequesterWindow, aListOfLeftLists) {
	CAdapiScreen.call(this, aRequesterWindow, CAdapiScreen.SCREEN_LEFTLIST_NUM_ZERO_BASED + 1, "_SCRNLEFT", aListOfLeftLists);
}, CAdapiScreen, {});
...
function instanciateLeftListLineFlat(aStackedWindow, aLeftList, aId, aLeftListNode) {
	...
	var wLeftListLine = new CAdapiLeftListLine(aStackedWindow, aLeftList, aId, wNbField, aLeftList.lines.length);
...
function instanciateLeftListLineLevel(aStackedWindow, aLeftList, aId, aLeftListNode) {
	var wNbField = aLeftListNode.$.NBZ;
	var wLeftListLine = new CAdapiLeftListLineLevel(aStackedWindow, aLeftList, aId, wNbField, aLeftList.lines.length);
	var wListOfX3Variables = aLeftListNode.FLD;
...
function instanciateLeftListLineNode(aStackedWindow, aLeftList, aId, aLeftListNode) {
	var wLeftListNode = new CAdapiLeftListLineNode(aStackedWindow, aLeftList, aId, 1, aLeftList.lines.length);
...
function instanciateVarsOfLeftListLine(aStackedWindow, aLeftListLine, aId, aListOfX3Variables, aForcedTDO) {
	...
	if (wMax == 0) {
		...
		throw new CAdapiException(aStackedWindow.client, wMess);
	}
	while (wI < wMax) {
...
var CAdapiListOfLeftListsFactory = exports.CAdapiListOfLeftListsFactory = {
	...
	instanciateNewLeftListEntrySup: function(aStackedWindow, aLeftList, aNbField) {
		...
		var wLeftListLine = new CAdapiLeftListLine(aStackedWindow, aLeftList, aLeftList.id, aNbField, aLeftList.lines.length);
		var wVariable;
		var wPrefixId = aLeftList.id;
		wPrefixId = wPrefixId.substring(wPrefixId.length - 1);
		var wI = 0;
	...
	instanciateOneLeftList: function(aStackedWindow, aScreen, aEntityIndexBaseOne, aId, aLeftListNode) {
		...
		if (CAdapiLeftList.isHierarchicFormat(wListFormat)) {
			...
			wAdapiLeftList = new CAdapiLeftListHierarchic(aStackedWindow, aScreen, aEntityIndexBaseOne, aId, wListFormat, wNbField, wNbKeyParts);
		} else if (CAdapiLeftList.isRecursiveFormat(wListFormat)) {
			wAdapiLeftList = new CAdapiLeftListRecursiv(aStackedWindow, aScreen, aEntityIndexBaseOne, aId, wListFormat, wNbField);
		} else {
			wAdapiLeftList = new CAdapiLeftList(aStackedWindow, aScreen, aEntityIndexBaseOne, aId, wListFormat, wNbField);
		}
