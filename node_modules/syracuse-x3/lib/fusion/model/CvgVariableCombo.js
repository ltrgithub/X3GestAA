...
var CvgVariableCombo = exports.CvgVariableCombo = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aFormat, aEntityState, aLocalMenu) {
	...
	clone: function() {
		var wVariable = new CvgVariableCombo(this.srcLine, this.entityIndexBaseOne, this.name, this.id, this.dataType, this.format, this.state, this.localMenu);
	...
	toDchJSON: function(arr) {
		if (this.disableModif) {
			if (this.disableList == null || this.disableList.length === 0) {
				...
				for (var it in this.disableList) {
					arr.push(this.disableList[it]);
				}
				return arr;
			}
		}
