...
var X3Reader = require('syracuse-x3/lib/fusion/automata/x3Reader').X3Reader;
...
var DESCS = [
	...
	[113, "NDCHVB", "IEF"], /*  Modif Valeur Bin	*/
...
function readVal(reader, letter) {
	switch (letter) {
		case 'B':
		...
		case 'E':
			return reader.nextExtend();
		case 'D':
			return reader.nextDefs();
		case 'V':
			return reader.nextResu();
		default:
...
function readListVal(reader, letter) {
	switch (letter) {
		case 'p':
...
function readList(_, client, letter) {
	...
	while (true) {
		...
		list.push(readListVal(reader, letter))
	}
}
...
function processNode(_, client, reader, node) {
	trace && console.log(node.name);
	var args = [reader, node];
	var params = node.params;
	for (var i = 0; i < params.length; i++) {
		if ('AUD'.indexOf(params[i]) >= 0) reader.ensure(_, client, reader.peekShort() + 2);
		else if ('E'.indexOf(params[i]) >= 0) {
			reader.ensure(_, client, 4); // ensure that peekLong works
			reader.ensure(_, client, reader.peekLong() + 4);
		}
		args.push(readVal(reader, params[i]))
	}
	var lists = node.lists;
	for (var i = 0; i < lists.length; i++) {
		args.push(readList(_, client, lists[i]));
	}
	trace && console.log("NODE HANDLER: " + node.name + ": args=" + JSON.stringify(args.slice(2)));
...
function processRequest(_, client, reader, node) { /*  Caractere a Traite  */
	...
	while (node) {
		...
		if (node === node.def) return node;//throw new Error("infinite loop")
