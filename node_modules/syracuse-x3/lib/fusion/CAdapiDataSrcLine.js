...
var CAdapiDataSrcLine = exports.CAdapiDataSrcLine = helpers.defineClass(function(requester, dataSrc, id, nbVars, zeroBaseIdx) {
	CAdapiEntity.call(this, dataSrc && dataSrc.client, id, 0, 'ROW');
	...
	clone: function() {
		var srcLine = new CAdapiDataSrcLine(this.requester, this.dataSrc, this.id, this.vars.length, this.zeroBaseLineIdx);
			...
			if (i != 0) {
				...
				if (stamp != -1) {
					if (lineStamp > colStamp) { // la ligne est plus recente
						if (valueItem != null && valueItem !== value && stamp > lineStamp) {
							map[stamp] = item;
						}
					} else {
						if (valueItem != null && valueItem !== colVal && stamp > colStamp) {
							map[stamp] = item;
						}
					}
				}
	...
	traverseSettable: function(aStructIst, aSetter) {
		...
		if (aStructIst.isFieldIst() || aStructIst.isAllLinesOfColumnIst()) {
			...
			if (wI < 0 || wI >= this.vars.length) {
				var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_VAR_UNKNOWN, wI.toString(), this.vars.length.toString(), this.getAdxId());
				aSetter.setLogicalErrMess(wMess);
				throw new Error(wMess)
				return false;
			}
			var wVariable = this.vars[wI];
			return aSetter.setAdapiModifiable(aStructIst, wVariable);
		}
		/*
		 ...
		 */
			...
			var wMess = this.getRsrcMess(CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE, aStructIst.toString(), this.getAdxId(), module.id);
			// TODO TCA
			// module.id is not corresponding to aSetter.class
			//var wMess = getRsrcMess(CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE, aStructIst.toString(), this.getAdxId(), CStringFacilities.getClasseName(aSetter.class));
			aSetter.setLogicalErrMess(wMess);
			return false;
		}
	},
	traverseSettableFields: function(aStructIst, aSetter) {
		...
		while (wI < wMax && wContinue) {
			...
			wContinue = aSetter.setAdapiModifiable(aStructIst, wVariable);
