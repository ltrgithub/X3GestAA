	...
	formatHimself: function(aValue) {
		...
		if (this.withoutZero) {
			try {
				...
				this.srcLine.adapiClient.tracer.trace(this, CStringFacilities.toStringDescr(" FormatData WithoutZero Value ", aValue), e);
				return aValue;
			}
		}
		var wEntier = null;
		...
		if (wHasDecimal) {
			wDecimals = this.formatDecimals(wDecimals, this.nbDecimals);
		}
		if (this.whithSymbolThousand) {
	...
	formatX3ToDisplay: function(aData) {
		if (aData.isNum()) {
			...
			if (!aData.isNull()) {
				...
				this.adapiClient.addLogicalErrorMess(this, "formatData", this.adxId, wMess);
			}
			return aData.value;
		}
	},
