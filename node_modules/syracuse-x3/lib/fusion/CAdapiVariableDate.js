...
var CAdapiVariableDate = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aFormat, aEntityState, aLocalFormatAndSep, aPivotDate) {
	...
	this.pivotDate;
	this.pivotYear = 50;
	if (aLocalFormatAndSep !== undefined) initLocalFormatAndSep(aLocalFormatAndSep);
	if (aPivotDate !== undefined) initPivotDate(aPivotDate);
}, CAdapiVariable, {
	/**
	...
	formatX3ToDisplay: function(aData) {
		...
		if (this.getServerValue().isDateNull()) {
			return EMPTY;
		}
		if (wValue == null) {
			return null;
		...
		if (wValue.length() != 8) {
			...
			this.adapiClient.addLogicalErrorMess(this, "callFormater", this.getAdxId(), wMess);
	...
	setServerResu: function(resu) {
		if (resu != null && CStructResu.DATE_ZERO.equals(resu.value)) {
			resu = null;
		}
	...
	unformatHimself: function(aValue) {
		...
		if (wYear.length() == 0) {
			if (this.browserValue.valueLength == 8) {
				...
				var wRightNow = Calendar.instance;
