...
var CAdapiVariableBlob = helpers.defineClass(function(srcLine, aEntityIndexBaseOne, aVarName, aID, aTdo, aSize, aFormat, aEntityState, aUrl) {
	...
	controlBlobSize: function(aBlob) {
		...
		if (aBlob.size() > wMaxSize) {
			throw CAdapiException.newFmtException(this.adapiClient, null, this.adapiClient.adapiResources.getMessage(CAdapiResources.KEYMESS_ERROR_UPLOAD_BLOB, String.valueOf(aBlob.size()), this.id, String.valueOf(wMaxSize), String.valueOf(this.size)));
		}
	},
	//getBlob: function();
	...
	hasValueBlob: function() {
		return this.serverValResu != null && this.serverValResu.hasBlobValue();
	},
	setBrowserUnformatedValue: function(aBuffer) {
		this.setBrowserUnformatedValue(new CStructResu(this.tracer, CStructResu.X3TYP_BLOB, aBuffer));
	},
	/* TODO overload
