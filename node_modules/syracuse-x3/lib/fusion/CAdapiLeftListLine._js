...
var CAdapiLeftListLineLevel = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, localIdx) {
	...
	clone: function() {
		...
		var wClone = new CAdapiLeftListLineLevel(this.adapiRequester, this.dataSrc, this.id, this.nbVar, idx);
		cloneVars(wClone);
		return wClone;
	}
});
...
var CAdapiLeftListLine = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, localIdx) {
	...
	clone: function() {
		...
		var wClone = new CAdapiLeftListLine(this.adapiRequester, this.dataSrc, this.id, this.nbVar, localIdx);
		cloneVars(wClone);
		return wClone;
	},
	...
	setColumns: function(aIdx, aVal, aClf) {
		...
		while (wI < nbVar && wI < wNbResu) {
			...
			if (resu.isDate() && !(v instanceof CAdapiVariableDate)) {
				...
				var wFormat = new CAdapiFormat(this.tracer, CAdapiEntity.EMPTY);
	...
	toJson: function() {
		...
		for (var item in this.vars) {
			if (item.hasServerValueModification()) {
				object.put(item.jsonKey, item.valueForJson);
				i++;
			}
		}
		if (i <= 0) {
