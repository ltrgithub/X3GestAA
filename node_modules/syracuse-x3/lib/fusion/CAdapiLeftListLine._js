...
var CAdapiLeftListLineLevel = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, localIdx) {
	...
	clone: function() {
		var idx = getDataSrc().size();
		var wClone = new CAdapiLeftListLineLevel(getAdapiRequester(), getDataSrc(), getId(), getNbVar(), idx);
		cloneVars(wClone);
		return wClone;
	}
});
...
var CAdapiLeftListLine = helpers.defineClass(function(aStackedWindow, aLeftList, aId, aNbVariables, localIdx) {
	...
	clone: function() {
		var localIdx = getDataSrc().size();
		var wClone = new CAdapiLeftListLine(getAdapiRequester(), getDataSrc(), getId(), getNbVar(), localIdx);
		cloneVars(wClone);
		return wClone;
	},
	getCle: function() {
	...
	setColumns: function(aIdx, aVal, aClf) {
		...
		while (wI < nbVar && wI < wNbResu) {
			...
			if (resu.isDate() && !(v instanceof CAdapiVariableDate)) {
				var vv = v;
				var client = getAdapiClient();
				var wConfig = client.getAdapiClientConfig();

				var wFormat = new CAdapiFormat(getTracer(), CAdapiEntity.EMPTY);
				...
				var newVar = new CAdapiVariableDate(this, vv.getEntityIndexBaseOne(), vv.getName(), vv.getId(), vv.getDataType(), wFormat, vv.getState(), wLocalformat, wDatePivot);
				setVar(wI, newVar);
	...
	setColumnsFormats: function(aFormats) {
		...
		while (wI < nbVar && wI < wNbResu) {
			...
			if (wFormatDef != null && wFormatDef.isNotNullResu()) {
				var wFormat = new CAdapiFormat(wFormatDef.getValue());
				if (wFormat.isValid()) {
					var v = getVar(wI);
					if (v != null) {
	...
	setColumnsTitles: function(aTitles) {
		...
		while (wI < nbVar && wI < wNbResu) {
			...
			if (wTitre != null && wTitre.isNotNullResu()) {
				var v = getVar(wI);
				if (v != null) {
					v.setServerTitle(wTitre.getValue());
				}
			}
			wI++;
		}
	...
	toJson: function() {
		...
		object.put(JSON_DOLLAR_UUID, getZeroBaseLineIdx());
		// line number
		var i = 0;
		for (var item in getVars()) {
			if (item.hasServerValueModification()) {
				object.put(item.getJsonKey(), item.getValueForJson());
				i++;
			}
		}
		if (i <= 0) {
