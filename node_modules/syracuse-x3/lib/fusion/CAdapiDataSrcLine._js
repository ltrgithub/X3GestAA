...
var CAdapiDataSrcLine = helpers.defineClass(function(requester, dataSrc, id, nbVars, zeroBaseIdx) {
	TODO_SUPER(dataSrc.adapiClient, id, 0, EntityType.ROW);
	...
	addObserver(requester.adapiClient);
	// En dernier
	setTypeJsonNode('ENTITY');
}, CAdapiEntity, {
	addOneVar: function(aVariable) {
		this.hVars.put(aVariable.id, aVariable);
	...
	clone: function() {
		var srcLine = new CAdapiDataSrcLine(this.adapiRequester, this.dataSrc, this.id, this.nbVar, this.zeroBaseLineIdx);
		cloneVars(srcLine);
		return srcLine;
	},
	...
	getAdapiObservableId: function() {
		var fullId = TODO_SUPER.adapiObservableId;
		fullId += "-L" + this.oneBaseLineIdx;
		return fullId;
	},
	getAdxId: function() {
		var wSB = new StringBuilder(16);
		wSB.append(this.dataSrcAdxId);
		wSB.append('0');
		wSB.append(',');
		wSB.append(this.oneBaseLineIdx);
		return wSB.toString();
	}
	...
	getDataSrcAdxId: function() {
		return this.adapiDataSrc == null ? null : this.adapiDataSrc.adxId;
	},
		...
		for (var item in this.vars) {
			if (i != 0) {
				...
				case STY:
					...
					break;
				case STT:
					valueItem = Integer.toString(item.state);
					value = Integer.toString(this.state);
					stamp = item.lastStampModState;
					lineStamp = this.lastStampModState;
					if (colVar != null) {
						colStamp = colVar.lastStampModState;
						colVal = Integer.toString(colVar.state); // value column
					}
					break;
				case TIT:
	...
	traverseSettable: function(aStructIst, aAdapiSetter) {
		...
		if (aStructIst.isFieldIst() || aStructIst.isAllLinesOfColumnIst()) {
			...
			if (wI < 0 || wI >= this.vars.length) {
				var wMess = this.adapiClient.adapiResources.getMessage(CAdapiResources.KEYMESS_ERROR_VAR_UNKNOWN);
				wMess = CStringFacilities.sprintf(wMess, String.valueOf(wI), String.valueOf(this.vars.length), this.adxId);
