...
var CAdapiScreenLeftList = exports.CAdapiScreenLeftList = helpers.defineClass(function(aAdapiRequesterWindow, aListOfLeftLists) {
	CAdapiScreen.call(this, aAdapiRequesterWindow, SCREEN_LEFTLIST_NUM_ZERO_BASED + 1, SCREEN_ID_LEFTLIST, aListOfLeftLists);
}, CAdapiScreen, {});
...
function extactType(aValue) {
	...
	return parseInt(aValue.substring(0, wPos));
}
...
function instanciateVarsOfLeftListLine(aStackedWindow, aLeftListLine, aId, aListOfX3Variables, aForcedTDO) {
	...
	var wVariableNode;
	var wI = 0;
	var wMax = aListOfX3Variables.length;
	if (wMax == 0) {
	...
	while (wI < wMax) {
		wVariableNode = aListOfX3Variables.item(wI);
...
var CAdapiListOfLeftListsFactory = {
	instanciateLeftLists: function(aStackedWindow, aAdapiScreens, aWindowDescription, aListOfLeftLists) {
		...
		var wMax = wLeftListNodes.length;
		if (wMax > 0) {
			...
			var wI = 0;
			while (wI < wMax) {
				wLeftListNode = wLeftListNodes.item(wI);
	...
	instanciateNewLeftListEntrySup: function(aStackedWindow, aAdapiLeftList, aNbField) {
		...
		var wLeftListLine = new CAdapiLeftListLine(aStackedWindow, aAdapiLeftList, aAdapiLeftList.id, aNbField, aAdapiLeftList.size());
		var wVariable;
		var wPrefixId = aAdapiLeftList.id;
		wPrefixId = wPrefixId.substring(wPrefixId.length - 1);
		var wI = 0;
