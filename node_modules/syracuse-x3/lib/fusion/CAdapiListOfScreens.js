...
var CAdapiScreenDummy = exports.CAdapiScreenDummy = helpers.defineClass(function(aRequesterWindow, aEntityIndexbaseOne, aScreenId) {
	...
	traverseSettable: function(aStructIst, aSetter) {
		return aSetter.setAdapiModifiable(aStructIst, this);
	}
});
...
var CAdapiListOfScreens = exports.CAdapiListOfScreens = helpers.defineClass(function(aRequesterWindow) {
	...
	this.screenButtons = new CAdapiScreenDummy(aRequesterWindow, CAdapiScreen.SCREEN_BUTTONS_NUM_ZERO_BASED + 1, CAdapiScreen.SCREEN_ID_BUTTONS);
	this.screenTabTitles = new CAdapiScreenDummy(aRequesterWindow, CAdapiScreen.SCREEN_TABTITLES_NUM_ZERO_BASED + 1, CAdapiScreen.SCREEN_ID_TABTITLES);
}, CAdapiListOfEntity, {
	get: function(aIdx) {
	...
	traverseSettable: function(aStructIst, aSetter) {
		...
		if (wAdapiScreen == null) {
			var wMess = this.getAdapiClient().resources.getMessage(CAdapiResources.KEYMESS_ERROR_SCRN_UNKNOWN, CAdapiScreen.screenZeroBaseIdxToWhatScreen(wI), this.length.toString(), this.requesterWindow.id);
		...
		if (!aStructIst.isScreenIst()) {
			...
			return aSetter.setAdapiModifiable(aStructIst, wAdapiScreen);
		}
	}
});