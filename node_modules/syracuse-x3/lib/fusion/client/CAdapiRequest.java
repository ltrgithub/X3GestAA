"use strict"; var helpers = require('syracuse-core/lib/helpers');
	...
	var CDataSrcAffStateSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.aftbTyp;
		private int this.nbLines;
		private IAdapiClient this.cli;

		CDataSrcAffStateSetter(cli, aAftbTyp, aNbLines) {
			TODO_SUPER(cli.getTracer());
			this.aftbTyp = aAftbTyp;
			this.nbLines = aNbLines;
			this.cli = cli;
		}, setAdapiModifiable: function(aStructIst,
				IAdapiModifiable aModifiable) {
			this.done = aModifiable instanceof CAdapiDataSrc;
			if (this.done) {
				((CAdapiDataSrc) aModifiable).setAffState(this.aftbTyp, this.nbLines);
			} else {
				setLogicalErrMess(this.cli.getAdapiResources().getMessage(
						CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE,
						aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
			}
			return this.done;
		}
	}

	var CDataSrcDeleter = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.nbLines;
		private int this.targetLine;
		private IAdapiClient this.cli;

		CDataSrcDeleter(cli, aTargetLine, aNbLines) {
			TODO_SUPER(cli.getTracer());
			this.cli = cli;
			...
			this.done = aModifiable instanceof CAdapiDataSrc;
			if (this.done) {
				var ds = (CAdapiDataSrc) aModifiable;
				if (this.nbLines == -1) {
	...
	var CDataSrcInserter = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.nbLines;
		private int this.zeroBasedIdx;
		IAdapiClient this.cli;

		CDataSrcInserter(cli, aTargetLine, aNbLines) {
			TODO_SUPER(cli.getTracer());
			this.cli = cli;
			...
			this.nbLines = aNbLines;
				...
				IAdapiModifiable aModifiable) {
			this.done = aModifiable instanceof CAdapiDataSrc;
			Utils.toConsole("\tsetMod=" + aStructIst.getAlphanumCodage()
					+ " - " + this.zeroBasedIdx + " - " + this.nbLines);
			if (this.done) {
				var wAdapiDataSrc = (CAdapiDataSrc) aModifiable;
				wAdapiDataSrc.serverInsertLines(this.zeroBasedIdx, this.nbLines);
				if (wAdapiDataSrc.hasDimOverFlow()) {
					setLogicalErrMess(this.cli.getAdapiResources().getMessage(
							CAdapiResources.KEYMESS_ERROR_DIM_OVERFLOW,
							String.valueOf(wAdapiDataSrc.getDim()),
							String.valueOf(this.nbLines),
							String.valueOf(wAdapiDataSrc.size())));
				}
			} else {
				setLogicalErrMess(this.cli.getAdapiResources().getMessage(
						CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE,
	...
	var CDownloadListner = helpers.defineClass(function() {}, null, {
		private CAdapiClient this.adapiClient;

		CDownloadListner(aAdapiClient) {
	...
	var CEntityStateSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.entityState;

		CEntityStateSetter(aTracer, aEntityState) {
			TODO_SUPER(aTracer);
	...
	var CLeftListAppender = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.branche;
		private CNodeArgTblResu this.clf;
		private CNodeArgTblResu this.clp;
		private int this.idx;
		private String this.txt;
		private CNodeArgTblResu this.val;

		CLeftListAppender(aTracer, aIdx, aBranche, aTxt,
				CNodeArgTblResu aVal, aClf, aClp) {
			TODO_SUPER(aTracer);
			this.idx = aIdx;
			...
			this.clp = aClp;
				...
				IAdapiModifiable aModifiable) {
			((CAdapiLeftList) aModifiable).appendLeftListLine(this.idx, this.branche,
					this.txt, this.val, this.clf, this.clp);
			this.done = true;
			return this.done;
		}
	}

	var CLeftListColumnsModifier = helpers.defineClass(function() {}, CAdapiModifier, {
		// modif des formats
		private static final int NDCOLAR_MODIF_FORMATS = 1;
		// modif des titres
		private CNodeArgTblResu this.defs;
		private int this.whatModification;

		CLeftListColumnsModifier(aTracer, aWhat,
				CNodeArgTblResu aDefs) {
			TODO_SUPER(aTracer);
			...
			if (this.whatModification == NDCOLAR_MODIF_FORMATS) {
				((CAdapiLeftList) aModifiable).changeColumnsFormats(this.defs);
			} else {
				((CAdapiLeftList) aModifiable).changeColumnsTitles(this.defs);
			}
			this.done = true;
			return this.done;
		}
	...
	var CLeftListDeleter = helpers.defineClass(function() {}, CAdapiModifier, {
		CLeftListDeleter(aTracer) {
			TODO_SUPER(aTracer);
		}, setAdapiModifiable: function(aStructIst,
				IAdapiModifiable aModifiable) {
			((CAdapiLeftList) aModifiable).serverRemoveAllLines();
			this.done = true;
			return this.done;
		}
	}

	var CLeftListPickingSelecter = helpers.defineClass(function() {}, CAdapiModifier, {
		private boolean this.selected;

		CLeftListPickingSelecter(aTracer, selected) {
			TODO_SUPER(aTracer);
			this.selected = selected;
		}, setAdapiModifiable: function(aStructIst,
				IAdapiModifiable aModifiable) {
			((CAdapiLeftListLine) aModifiable)
					.setLeftListLinePickingSel(this.selected);
			this.done = true;
			return this.done;
		}
	...
	var CLeftListSelecterByKey = helpers.defineClass(function() {}, CAdapiModifier, {
		private CNodeArgTblResu this.cle;
		private boolean this.selected;
		private CAdapiRequest request;

		CLeftListSelecterByKey(aTracer, selected,
				CNodeArgTblResu aCle, request) {
			TODO_SUPER(aTracer);
			this.selected = selected;
			this.cle = aCle;
			this.request = request;
				...
				IAdapiModifiable aModifiable) {
			((CAdapiLeftList) aModifiable).setServerSelectLine(this.selected, this.cle);
			request.setLefListLineSelected(((CAdapiLeftList) aModifiable)
					.findLineIdx(this.cle));
			this.done = true;
			return this.done;
		}
	}

	var CServerFormatSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private String this.format;
		private int this.inferredState = CAdapiEntity.STATE_DEFAULT;

		CServerFormatSetter(cli, aFormat) {
			TODO_SUPER(cli.getTracer());
			this.format = aFormat;
		}

		int getInferredState() {
			return this.inferredState;
		}

		boolean hasInferredState() {
	...
	var CServerStateSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private int this.entitryState;

		private CServerStateSetter(aTracer, aState) {
			TODO_SUPER(aTracer);
	...
	var CServerStyleSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private String this.style;
		private StyleApply this.applyOn;

		CServerStyleSetter(aTracer, aStyle, applyOn) {
			TODO_SUPER(aTracer);
			this.style = aStyle;
	...
	var CServerTitleSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private String this.title;

		CServerTitleSetter(aTracer, aTitle) {
			TODO_SUPER(aTracer);
			this.title = aTitle;
		}, setAdapiModifiable: function(aStructIst,
				IAdapiModifiable aModifiable) {
	...
	var CServerValueSetter = helpers.defineClass(function() {}, CAdapiModifier, {
		private CStructResu this.resu;

		CServerValueSetter(aTracer, aResu) {
			TODO_SUPER(aTracer);
			this.resu = aResu;
		}, setAdapiModifiable: function(aStructIst,
				IAdapiModifiable aModifiable) {
...
var CAdapiRequest = helpers.defineClass(function() {}, CAdonixRequest, {

	public final static String SENDCTX_PARAM_TARGET = "payload.param.target";
	private static char ARG_VALUES_SEPARATOR = CStructResu.SEPARATOR_TDO_DATA;
	private static String LABEL_CLASS_DATASRC = "CAdapiDataSrc";
	private static String LIB_ACTION_ID = "ActId";
	private static String LIB_FORMAT = "Format";
	private static String LIB_FUNCTION_ID = "FunctionId";
	private static String LIB_IST = "Ist";
	private static String LIB_LIB = "Lib";
	private static String LIB_MENU_ID = "MenuId";
	private static String LIB_MESS_ACT = "MessAction";
	private static String METH_ARGS_DOLEFTLISTLINE = "listenActionArgsDOLEFTLISTLINE";
	private static String METH_ND_ASK_GETFILE = "ASKGetFile";
	private static String METH_ND_CLF = "ND_CLF";
	private static String METH_ND_NAP = "ND_NAP";
	private static String METH_NDCHCR = "NDCHCR";
	private static String METH_NDCHFM = "NDCHFM";
	private static String METH_NDCHGR = "NDCHGR";
	private static String METH_NDCHST = "NDCHST";
	private static String METH_NDCHVB = "NDCHVB";
	private static String METH_NDCHVL = "NDCHVL";
	private static String METH_NDCSTL = "NDCSTL";
	private static String METH_NDCTIT = "NDCTIT";
	private static String METH_NDDSLB = "NDDSLB";
	private static String METH_NDGLAR = "NDGLAR";
	private static String METH_NDINSLI = "NDINSLI";
	private static String METH_NDOCAR = "NDOCAR";
	private static String METH_NDOFENH = "NDOFENH";
	private static String METH_NDOLAR = "NDOLAR";
	private static String METH_NDOMNBO = "NDOMNBO";
	private static String METH_NDOWIN = "NDOWIN";
	private static String METH_NDSUPLI = "NDSUPLI";
	private static String LIB_STATE = "State";
	private static String LIB_STYLE = "Style";
	private static String LIB_TEXT = "Txt";
	private static String LIB_TITLE = "Title";
	private static String LIB_WINDOW_ID = "WindowsId";
	private static String LIB_WINDOW_STAMP = "Stamp";
	private static String WINDOW_TEMPON_ID = "TEMPON";
	private IAdapiClient this.adapiClient;
	private int this.nbNodeOpenTempon = 0;
	private int lefListLineSelected = -1;

	public CAdapiRequest(aAdapiClient, aAdonixReply,
			ArrayList<Object> aParams, Map<String, Object> sendContext) {
		TODO_SUPER(aAdapiClient, aAdonixReply, aParams, sendContext);
		this.adapiClient = aAdapiClient;
	}

	protected synchronized void breakWaiting(aCompleteReply) {
		...
		if (CStructIstDatas.isIstDatas(aParam)) {
			istData = (CStructIstDatas) aParam;
		}
		...
		try {
			...
			if (var.acceptEditFormat() && istData.isEditFormat()) {
				...
				getX3Reply().addAdapiInstance(
						CAdapiInstance.newprevious(cli, istData.getIst(), var
								.getFormat().getString(), istData
								.getEditedValue(), this.adonixReply == null ? false
								: this.adonixReply.getStayInCommandMode()));
			}
			// deformatage : format presentation => format brut
			istData.setValueProtected(var.getResuTyp(),
					var.unFormatData(istData.getEditedValue()));
			var.setBrowserUnformatedValue(istData);
			return istData;
		} catch (e) {
			// Récupération de l'erreur de formatage pour info client
			var adapi = CAdapiInstance.newtarget(cli, istData
					.getIst(), var.getFormat().getString(), istData
					.getEditedValue(), this.adonixReply == null ? false
					: this.adonixReply.getStayInCommandMode());
			adapi.setException(e);
			getX3Reply().addAdapiInstance(adapi);
			if (e instanceof RuntimeException) {
				throw (RuntimeException) e;
			}
			throw new RuntimeException(e);
		}
	}, buildListenNodeMessage: function(aKeyMess, aValue1,
			CAdapiModifier aModifier) {
		var wMess = getAdapiClient().getAdapiResources().getMessage(
				aKeyMess, aValue1);

		if (aModifier.hasLogicalErrMess()) {
			return wMess.concat(aModifier.getLogicalErrMess());
		} else {
			return wMess;
		}
	}, buildListenNodeMessage: function(aKeyMess, aValue1,
			String aValue2, aModifier) {
		var wMess = getAdapiClient().getAdapiResources().getMessage(
		...
		return 0;
	}, getLefListLineSelected: function() {
		return lefListLineSelected;
	}, getX3Reply: function() {
		return (CX3Reply) TODO_SUPER.this.reply;
	}
	 ...
	 * colonnes
	 */
	void listenActionArgsBRWRAPIDESEL(aAdapiClient,
			ArrayList<Object> aParams) {
		var wNbArgs = aParams.size();
		ArrayList<CStructResu> wTabOfResus = new ArrayList<CStructResu>();
		var wI = 2;
		var wColValue;
		while (wI < wNbArgs) {
			wColValue = (String) aParams.get(wI);
			// ajoute dans la table un resu deformate construit a partir de la
			// chaine "adxid^tdo^value"
			wTabOfResus.add(new CStructResu(this.tracer, CStructResu.X3TYP_STRU,
					wColValue));
			wI++;
		}
		wI = wNbArgs - 1;
	 ...
	 */
	void listenActionArgsDOLEFTLISTLINE(aAdapiClient,
			ArrayList<Object> aParams) {
		/*
		...
		if (wParam instanceof CStructIst) {
			wArgIst = (CStructIst) wParam;
		} else if (wParam instanceof String) {
			wArgIst = new CStructIst(this.tracer, (String) wParam);
		} else {
			throw new CAdapiException(getAdapiClient(),
					"ARG_WRONG - need a \"CStructIst\" or a \"String\" !");
		}
		if (!wArgIst.isLineIst()) {
			throw new CAdapiException(getAdapiClient(),
					"ARG_WRONG - need a \"line\" ist !");
		}
		var wLeftListLine = (CAdapiLeftListLine) aAdapiClient
				.findModifiable(wArgIst);
		...
		if (wLeftListLine instanceof CAdapiLeftListLineNode) {
			var wLeftListNode = (CAdapiLeftListLineNode) wLeftListLine;
			/*
			 * Stockage dans la liste gauche r�cursive de l'id du Node �
			 * "expandre"
			 */
			wLeftListNode.getLeftListRecursiv().setNodeIdToExpanded(
					wArgIst.getIdx());
		}
		aParams.add(wLeftListLine.getCle());
		wArgIst.setLine(0);
		...
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
			...
			if (wMax > 0) {
				var wI = 0;
				while (wI < wMax) {
					wTB.appendDescr("ParamClass", CStringFacilities
							.getClasseName(aParams.get(wI).getClass()));
					wI++;
				}
			}
	 ...
	 */
	void listenActionArgsLIST_PAGE_DOWN(aAdapiClient,
			ArrayList<Object> aParams) {
		var wLeftList = retrieveAdapiLeftList(aAdapiClient, aParams);
		var wLeftListLine = (CAdapiLeftListLine) wLeftList
				.getLastLine();
	 ...
	 */
	void listenActionArgsLIST_PAGE_UP(aAdapiClient,
			ArrayList<Object> aParams) {
		var wLeftList = retrieveAdapiLeftList(aAdapiClient, aParams);
		var wLeftListLine = (CAdapiLeftListLine) wLeftList
				.getFirstLine();
	 ...
	 */
	void listenActionArgsPICKING(aAdapiClient,
			ArrayList<Object> aParams) {
		/*
		 ...
		 */
		var wSTAdxids = new StringTokenizer(
				(String) aParams.get(2), String.valueOf(ARG_VALUES_SEPARATOR));
		/*
		 * attribut "d" : action de selection ou de d�s�lection 1052
		 * SRV_BRWSELECTLINE ou 1067 SRV_BRWDESELECTLINE stockage au niveau du
		 * client
		 */
		var wSTActions = new StringTokenizer(
				(String) aParams.get(3), String.valueOf(ARG_VALUES_SEPARATOR));
		// vidage de la liste de ligne pickees
		getAdapiClient().clearAdxIstsOfPickedLines();
		...
		while (wSTAdxids.hasMoreTokens()) {
			...
			getAdapiClient().traverseSettable(
					wStructIst,
					new CLeftListPickingSelecter(this.tracer, wAdapiPickedLine
							.isSelection()));
			getAdapiClient().addAdxIstOfPickedLines(wAdapiPickedLine);
		}
		aParams.remove(3);
		aParams.remove(2);
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
		...
		if (this.tracer.isTraceParametersOn()) {
			...
			wTB.appendDescr("Throwable", e.getClass());
	 ...
	 */
	, listenNodeND_ASKGetFile: function(
			CNodeExtendProperties aExtendProperties) {
		var wRemoteFileDescriber = aExtendProperties
				.getOnePropertyValue(CAdonixClient.ASKUI_PARAM_REMOTEPATH);
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var wDownloadListner = new CDownloadListner(
				getAdapiClient());
		return getAdapiClient().getNewAdonixFileManager(wDownloadListner)
				.getFile(wRemoteFileDescriber);
	}, listenNodeND_ASKSendAutoReply: function(aNumError,
			String aExplaination) {
		var wResponse = new StringBuilder();
		wResponse.append(aNumError);
		wResponse.append(',');
		wResponse.append(aExplaination);
		var wIstDatas = new CStructIstDatas(this.tracer,
				CStructResu.X3TYP_STRU, wResponse.toString());
		var wProtocolNum = getAdonixClient().getRemoteProtocolNum();
		var wSegReplyAction = CSegmentActionFactory
				.buildSegmentAction(this.tracer, true, wProtocolNum,
						CDefAction.SRV_ASKUI_RESPONSE, wIstDatas);
		try {
			if (getAdapiClient().getTypeClient() == ETypeClient.JULIET) {
				this.connClient.writeSegment(wSegReplyAction);
			} else {
				getAdonixClient().writeSegment(wSegReplyAction, false);
			}
		} catch (e) {
			var wErrMess = this.adonixClient.getAdonixResources()
					.getMessCantSendAutomaticReply("ND_ASK");
	...
	, listenNodeND_CLF: function(aSegmentNode) {
		...
		var wAdapiStackedWindow = getAdapiClient()
				.popRequesterWindow();
		var wX3Reply = getX3Reply();
		...
		if (this.tracer.isTraceDebugOn()) {
			...
			wTB.appendDescr("getNbPopedWindows", wX3Reply.getNbPopedWindows());
			wTB.appendDescr("LastPopedWindowId",
					wX3Reply.getLastPopedWindowId());
	...
	, listenNodeND_NAP: function(aSegmentNode) {
		var wTxt = aSegmentNode.getNodeArgString(0).getStringValue();
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		getX3Reply().setAskOpenSession(wTxt);
		return true;
	}, listenNodeND_REB: function(aSegmentNode) {
		// Notify observer
		new AdapiReboot(getAdapiClient());
	...
	, listenNodeNDAFTB: function(aSegmentNode) {
		...
		var wAffTbState = aSegmentNode.getNodeArgByte(1).getIntValue();
		var wNbLines = aSegmentNode.getNodeArgShort(2).getIntValue();
		var wDataSrcAffStateSetter = new CDataSrcAffStateSetter(
				getAdapiClient(), wAffTbState, wNbLines);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				wDataSrcAffStateSetter);
		if (!wDataSrcAffStateSetter.done()) {
			...
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_AFFSTATE, wState,
					wDataSrcAffStateSetter);
			((CAdapiReply) getReply()).addLogicalErrorMess(
					this,
					CProtocolDefs.getProtocoleDefs()
							.getProtocolDef(getConn().getRemoteProtocolNum())
							.findNode(this.tracer, CDefNode.NDAFTB)
							.getFullIdInString(), wArgIst.getStructIst()
							.toStringDescr(), wMessLE);
	...
	, listenNodeNDCHCR: function(aSegmentNode) {
		if (this.tracer.isTraceDebugOn()
				&& this.tracer
						.traceModuleFilter(CAdapiClientConfig.TRACER_MODULE_MODIFIER)) {
			var wTB = this.tracer.popTraceBuffer();
	...
	, listenNodeNDCHFM: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wFormat = aSegmentNode.getNodeArgString(1).getStringValue();
		if (this.tracer.isTraceDebugOn()
				&& this.tracer
						.traceModuleFilter(CAdapiClientConfig.TRACER_MODULE_MODIFIER)) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var wFormatSetter = new CServerFormatSetter(
				getAdapiClient(), wFormat);
		var wIst = wArgIst.getStructIst();
		getAdapiClient().traverseSettable(wIst, wFormatSetter);
		if (wFormatSetter.hasInferredState()
				&& wArgIst.getStructIst().isCellIst()) {
			// pour designer la colonne
			wIst.setLine(-1);
			var wStateSetter = new CEntityStateSetter(this.tracer,
					wFormatSetter.getInferredState());
			getAdapiClient().traverseSettable(wIst, wStateSetter);
		}
		if (!wFormatSetter.done()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_SETTER_FORMAT, wFormat,
					wFormatSetter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDCHFM,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	, listenNodeNDCHGR: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wState = aSegmentNode.getNodeArgByte(1).getIntValue();
		// 0 "SRV_GRIZO";
		// 1 "SRV_ACTZO"
		// 2 "SRV_DISZO";
		if (this.tracer.isTraceDebugOn()
		...
		var wStateSetter = new CServerStateSetter(this.tracer,
				entityState);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wStateSetter);
		if (!wStateSetter.done()) {
			((CAdapiReply) getReply())
					.addLogicalErrorMess(
							this,
							METH_NDCHGR,
							wArgIst.getStructIst().toStringDescr(),
							buildListenNodeMessage(
									CAdapiResources.KEYMESS_ERROR_SETTER_STATE,
									CAdapiEntity.x3StateToString(wState),
									wStateSetter));
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
		...
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wExtendResu = ((CSegmentNodeExtended) aSegmentNode)
				.getNodeExtendResu(0);
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
			wTB.appendMethodName(METH_NDCHVB);
			wTB.appendDescr(wArgIst);
			wTB.appendDescr(wExtendResu.getValueResu());
			this.tracer.trace(this, ITracer.LEVEL_DEBUG, wTB);
			this.tracer.pushTraceBuffer(wTB);
		}
		var wValueSetter = new CServerValueSetter(this.tracer,
				wExtendResu.getValueResu());
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wValueSetter);
		if (!wValueSetter.done()) {
			var wBlobSize = String.valueOf(wExtendResu.getValueResu()
					.getValueLength());
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_SETTER_BLOB, wBlobSize,
					wValueSetter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDCHVB,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	...
	, listenNodeNDCHVL: function(aSegmentNode) {
		...
		var ist = aSegmentNode.getNodeArgIst(0).getStructIst();
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var wValueSetter = new CServerValueSetter(this.tracer,
				resu.getResu());
		getAdapiClient().traverseSettable(ist, wValueSetter);
		if (!wValueSetter.done()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_SETTER_VALUE,
					resu.toStringDescr(), wValueSetter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDCHVL,
					ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	, listenNodeNDCSTL: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var sty = aSegmentNode.getNodeArgString(1).getStringValue();
		if (this.tracer.isTraceDebugOn()
				&& this.tracer
						.traceModuleFilter(CAdapiClientConfig.TRACER_MODULE_MODIFIER)) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var wStyleSetter = new CServerStyleSetter(this.tracer, sty,
				StyleApply.getApply(wArgIst));
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wStyleSetter);
		if (!wStyleSetter.done()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_SETTER_STYLE, sty,
					wStyleSetter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDCHST,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	...
	, listenNodeNDCTIT: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wTitle = aSegmentNode.getNodeArgString(1).getStringValue();
		if (this.tracer.isTraceDebugOn()
				&& this.tracer
						.traceModuleFilter(CAdapiClientConfig.TRACER_MODULE_MODIFIER)) {
			var wTB = this.tracer.popTraceBuffer();
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				new CServerTitleSetter(this.tracer, wTitle));
		return true;
	}
	...
	, listenNodeNDDSLB: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var value = aSegmentNode.getNodeArgByte(1).getIntValue();
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var var = null;
		if (getSendContext() != null) {
			var currentIst = (CStructIst) getSendContext().get(
					SENDCTX_PARAM_TARGET);
			if (currentIst != null) {
				var = getAdapiClient().findModifiable(currentIst);
				if (var != null && var instanceof CAdapiVariableCombo) {
					((CAdapiVariableCombo) var).setServerDisabledChoice(value);
				}
			}
		}
		if (var == null) {
			Utils.toConsole("NDDSLB - Variable not found["
					+ wArgIst.getStructIst().getAlphanumCodage() + "]");
		}
		return true;
	}
	...
	, listenNodeNDEXECI: function(aSegmentNode) {
		...
		var ack_call = this.adonixReply.getExecIstArgs();
		// FDB - ON passe getAdapiClient().getCurrentField() pour le
		// notiyObservers
		new CAdapiInstanceExeci(ack_call.getXml(), getAdapiClient(),
				getAdapiClient().getCurrentField());
	...
	, listenNodeNDGLAR: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				wLeftListLineSelecter);
		if (!wLeftListLineSelecter.done()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER,
					wCle.toStringDescr(), wLeftListLineSelecter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDGLAR,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	...
	, listenNodeNDINSLI: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wTargetLine = aSegmentNode.getNodeArgShort(1).getIntValue();
		var wNbLines = aSegmentNode.getNodeArgShort(2).getIntValue();
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		var wDataSrcInserter = new CDataSrcInserter(
				getAdapiClient(), wTargetLine, wNbLines);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				wDataSrcInserter);
		if (wDataSrcInserter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_INSERTER,
					String.valueOf(wNbLines), String.valueOf(wTargetLine),
					wDataSrcInserter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDINSLI,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	...
	, listenNodeNDOBOXI: function(aSegmentNode) {
		var wTypeBox = aSegmentNode.getNodeArgByte(0).getByteValue();
		if (wTypeBox == OU_BIF) {
			var wMessText = aSegmentNode.getNodeArgString(2)
					.getStringValue();
			...
			if (wPos == 0 || wPos == 1) {
				var wMessTitre = aSegmentNode.getNodeArgString(3)
						.getStringValue();
				var wSleep = aSegmentNode.getNodeArgByte(4).getByteValue();
				wMessText.replace('\r', ' ');
				...
				if (wAffSynchroneType == CAdapi.AFF_SYNCHRONE_TYPE_SESSION) {
					wSleep = ((CAdapiClient) this.commClient)
							.getMaxTimeBeforeAutomaticHangUp();
				...
				if (this.tracer.isTraceDebugOn()) {
					...
					wTB.append(aSegmentNode.getNodeIdInString());
					var wLibType = CAdapi
							.affSynchroneTypeToString(wAffSynchroneType);
					wTB.appendDescr(LIB_MESS_TYP, wAffSynchroneType,
							wLibType != null ? wLibType : "unknown type");
					var wLibAction = CAdapi
							.affSynchroneActionToString(wAffSynchroneAction);
					wTB.appendDescr(LIB_MESS_ACT, wAffSynchroneAction,
							wLibAction != null ? wLibAction : "unknown action");
					wTB.appendDescr(LIB_MESS_TEXT, wMessText.replace('\n', '.'));
					wTB.appendDescr(LIB_MESS_TITRE,
							wMessTitre.replace('\n', '.'));
				...
				var wAdapiMessage = new CAdapiMessage(
						wAffSynchroneType, wAffSynchroneAction, wMessText,
						wMessTitre, -1, wSleep, -1, this.adapiClient);
				getAdapiClient().pushAffSynchroneMessage(wAdapiMessage);
	...
	, listenNodeNDOCAR: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wWhat = aSegmentNode.getNodeArgByte(1).getIntValue();
		...
		if (!wDefs.isComplete()) {
			var wMessMdl = getAdapiClient().getAdapiResources().getMessage(
					CAdapiResources.KEYMESS_ERROR_ARG_RESU_TBL);
			var wMess = CStringFacilities.sprintf(wMessMdl, "Defs",
					String.valueOf(wDefs.size()),
					String.valueOf(wDefs.getNbReceivedElements()));
			wMessMdl = getAdapiClient().getAdapiResources().getMessage(
					CAdapiResources.KEYMESS_NDOCAR_WHAT);
			wMess += CStringFacilities.sprintf(wMessMdl, String.valueOf(wWhat));
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDOCAR,
					wArgIst.getStructIst().toStringDescr(), wMess);
		}
		var wModifier = new CLeftListColumnsModifier(
				this.tracer, wWhat, wDefs);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wModifier);
		return true;
	}
	...
	, listenNodeNDOFENH: function(aSegmentNode) {
		...
		getX3Reply().setAskOpenHelpParam(wTxt);
		return true;
	}
	...
	, listenNodeNDOFLD: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wTxt1 = aSegmentNode.getNodeArgString(1).getStringValue();
		var wTxt2 = aSegmentNode.getNodeArgString(2).getStringValue();
		var wTxt3 = aSegmentNode.getNodeArgString(3).getStringValue();
		var wInt1 = aSegmentNode.getNodeArgShort(4).getIntValue();
		var wTxt4 = aSegmentNode.getNodeArgString(5).getStringValue();
		var wTxt5 = aSegmentNode.getNodeArgString(6).getStringValue();
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		getAdapiClient().traverseSettable(wIst,
				new CLeftListAppender(this.tracer, -1, 0, "", wLine, wClf, null));
		return true;
	}
	...
	, listenNodeNDOLAR: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wIdx = aSegmentNode.getNodeArgShort(1).getIntValue();
		var wBranche = aSegmentNode.getNodeArgByte(2).getIntValue();
		...
		if (!wVal.isComplete() || !wClf.isComplete() || !wClp.isComplete()) {
			...
			var wMessMdl = getAdapiClient().getAdapiResources().getMessage(
					CAdapiResources.KEYMESS_ERROR_ARG_RESU_TBL);
			if (!wVal.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Val",
						String.valueOf(wVal.size()),
						String.valueOf(wVal.getNbReceivedElements()));
			}
			if (!wClf.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Clf",
						String.valueOf(wVal.size()),
						String.valueOf(wClf.getNbReceivedElements()));
			}
			if (!wClp.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Clp",
						String.valueOf(wVal.size()),
						String.valueOf(wClp.getNbReceivedElements()));
			}
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDOLAR,
					wArgIst.getStructIst().toStringDescr(), wMess);
			this.tracer.trace(this, 1, "####" + wMess);
		}
		getAdapiClient().traverseSettable(
				wArgIst.getStructIst(),
				new CLeftListAppender(this.tracer, wIdx, wBranche, wTxt, wVal,
						wClf, wClp));
		return true;
	}
	...
	, listenNodeNDOMNBO: function(aSegmentNode) {
		var wIdm = aSegmentNode.getNodeArgInt(0).getIntValue();
		var wRng = aSegmentNode.getNodeArgShort(1).getIntValue();
		var wIde = aSegmentNode.getNodeArgInt(2).getIntValue();
		var wFlg = aSegmentNode.getNodeArgByte(3).getIntValue();
		var wActionId = aSegmentNode.getNodeArgShort(4).getIntValue();
		var wTxt = aSegmentNode.getNodeArgString(5).getStringValue();
		var wAcc = aSegmentNode.getNodeArgShort(6).getIntValue();
		if (this.tracer.isTraceDebugOn()) {
			var wTB = this.tracer.popTraceBuffer();
		...
		getX3Reply().addMenuWin(wFlg, wActionId, wTxt, wIdm, wRng, wIde, wAcc);
		return true;
	}
	...
	, listenNodeNDOUVIMP: function(aSegmentNode) {
		...
		// FDB Non utilisé -
		// getX3Reply().addAdapiReport(wExtendProperties.getProperties());
		var wSegReplyBinShort = new CSegmentReplyBinShort(
				this.tracer, true, (byte) 1);
		try {
			if (getAdapiClient().getTypeClient() == ETypeClient.JULIET) {
				this.connClient.writeSegment(wSegReplyBinShort);
			} else {
				getAdapiClient().writeSegment(wSegReplyBinShort, false);
			}
		} catch (e) {
			this.adonixClient.setServerNotConnected();
			var wErrMess = this.adonixClient.getAdonixResources()
					.getMessCantSendAutomaticReply("NDOUVIMP");
	...
	, listenNodeNDOWIN: function(aSegmentNode) {
		var wIst = aSegmentNode.getNodeArgIst(0);
		var wWindowsId = aSegmentNode.getNodeArgString(1).getStringValue();
		var wFunctionId = aSegmentNode.getNodeArgString(2).getStringValue();
		var wStamp = aSegmentNode.getNodeArgString(3).getStringValue();
		...
		try {
			...
			wAdapiStackedWindow = getAdapiClient().pushRequesterWindow(
					wWindowsId, wFunctionId, wStamp,
					wIst.getStructIst().getWin());
			wX3Reply.addPushedWindow(wAdapiStackedWindow);
			wX3Reply.setStamp(wStamp);
			if (aSegmentNode instanceof CSegmentNodeExtended) {
				var wSegNodeExtended = (CSegmentNodeExtended) aSegmentNode;
				if (wSegNodeExtended.hasExtends()) {
					var wExtendedInfos = ((CNodeExtendTextUtf8) wSegNodeExtended
							.getNodeExtend(0)).getValue();
					wAdapiStackedWindow
							.setNDOWinComplementDescr(wExtendedInfos);
				}
			}
			if (this.tracer.isTraceDebugOn()) {
				var wTB = this.tracer.popTraceBuffer();
				...
				wTB.appendDescr("NbPushedWindows",
						wX3Reply.getNbPushedWindows());
				wTB.appendDescr("LastPushedWindowId",
						wX3Reply.getLastPushedWindowId());
				aSegmentNode.addDescrInTB(wTB);
				this.tracer.trace(this, ITracer.LEVEL_DEBUG, wTB);
				this.tracer.pushTraceBuffer(wTB);
			}
	...
	, listenNodeNDRZAR: function(aSegmentNode) {
		getAdapiClient().traverseSettable(
				aSegmentNode.getNodeArgIst(0).getStructIst(),
				new CLeftListDeleter(this.tracer));
		return true;
	}
	...
	, listenNodeNDSUPLI: function(aSegmentNode) {
		...
		var wDataSrcDeleter = new CDataSrcDeleter(getAdapiClient(),
				wTargetLine, wNbLines);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				wDataSrcDeleter);
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage(
					CAdapiResources.KEYMESS_ERROR_DELETER,
					String.valueOf(wNbLines), String.valueOf(wTargetLine),
					wDataSrcDeleter);
			((CAdapiReply) getReply()).addLogicalErrorMess(this, METH_NDSUPLI,
					wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	...
	, listenNodeNDULAR: function(aSegmentNode) {
		...
		var wCle = aSegmentNode.getNodeArgTblResuOld(1);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(),
				new CLeftListSelecterByKey(this.tracer, false, wCle, this));
		return true;
	}
	...
	, listenSegment: function(aSegment) {
		...
		case CDefNode.NDOFENH:
			...
			break;
		case CDefNode.ND_ASK: // AskUI_Call]
		...
		if (wProcessed) {
			...
			if (wSegmentNode.isNodeWaitUser()) {
				if (getAdonixClient().getTypeClient() == ETypeClient.JULIET) {
					breakWaiting(true);
				}
			}
			return wProcessed;
	...
	}, listenSendingArgs: function(aRequestId, aCommClient,
			ArrayList<Object> aParams) {
		switch (aRequestId) {
		...
		case CDefAction.SRV_TREESELECTLINE:
			listenActionArgsDOLEFTLISTLINE((CAdapiClient) aCommClient, aParams);
			break;
		case CDefAction.SRV_BRWRAPIDESEL:
			listenActionArgsBRWRAPIDESEL((CAdapiClient) aCommClient, aParams);
			break;
		case CDefAction.SRV_LIST_PAGE_DOWN:
			listenActionArgsLIST_PAGE_DOWN((CAdapiClient) aCommClient, aParams);
			break;
		case CDefAction.SRV_LIST_PAGE_UP:
			listenActionArgsLIST_PAGE_UP((CAdapiClient) aCommClient, aParams);
			break;
		case CDefAction.SRV_PICKING:
			// listenActionArgsPICKING((CAdapiClient) aCommClient, aParams);
			break;
		default:
	...
	}, retrieveAdapiLeftList: function(aAdapiClient,
			ArrayList<Object> aParams) {
		return (CAdapiLeftList) aAdapiClient.findModifiable(CStructIst
				.newStructIst(this.tracer, aParams.get(1)));
		...
		if (this.adonixReply instanceof CAdapiReply) {
			((CAdapiReply) this.adonixReply).setTargetAdxId(ist, this);
		} else {
			var reply = new CAdapiReply(getAdapiClient(), this);
			reply.setOk(this.adonixReply.getStatus());
			reply.setValue(this.adonixReply.getBinValue());
