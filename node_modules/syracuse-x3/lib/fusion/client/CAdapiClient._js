...
var CDefSuperAction = require('syracuse-x3/lib/fusion/CDefAction').SuperActions;
...
var executeSuperAction = function(_, client, body, superAct){
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	processAction: function(_, body) {
		...
		if (body.fld) {
			...
			resu = CStructResu.fromAction(entity.dataType, body.fld.v);
	...
	sendAction: function(_, resu, extras, params) {
		...
		if (!resu) resu = new CStructResu(CStructResu.TDO_NULL);
	...
	processSuperAction: function(_, body) {
		...
		var superAction;
		for (var i in CDefSuperAction){
			if (CDefSuperAction[i].code === body.act){
				superAction = CDefSuperAction[i];
				break;
			}
		}
		...
		var t0 = body.fld && body.fld.ist;
		console.log("TO="+JSON.stringify(t0,null,2));
		var resp;
		for (var i = 0; i < superAction.actions.length; i++) {
			var action = superAction.actions[i];
			var processNext = true;
			
			if (action.checkTarget){
				body.act = action;
				resp = this.processAction(_, body);
				console.error("RESP "+i+": "+JSON.stringify(resp,null,2));
				var t1 = resp.sap.target;
				console.log("T1 "+i+"="+JSON.stringify(t0,null,2));
				processNext = superAction.processNextAction(CDefAction[action.name], t0, t1, action.checkNumLine);
			}
			if (!processNext)
				break;
		}
