...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, aConfig, aType) {
	...
	this.newActiveRequest(new CAdapiReply(this, CDefAction.LOC_CONNECT), null, null);
}, null, {
	...
	beginRequest: function(_, body) {
		...
		this.stream.write(_, resu.getBytes());
		var extra = [];
		var crsr = body.fld.ctx && body.fld.ctx.crsr;
		if (crsr != null) extra.push(new CStructResu(CStructResu.TDO_INT, parseInt(body.fld)));
		data = Buffer.concat([data, new Buffer([exta.length])]);
		extra.forEach(function(ext) {
			data = Buffer.concat([data, ext.getBytes()])
		});
		params = this.activeRequest.listenSendingArgs(body.act, this);
		var lenBuf = new Buffer(2);
		lenBuf.writeInt16BE(2 + params.length);
		data = Buffer.concat([data, lenBuf, params]);
	...
	open: function(aAdonixContext) {
		...
		try {
			 ...
			 */
			var aX3Reply = checkFailed(connect(aAdonixContext, newActiveRequest(new CAdapiReply(this, CDefAction.LOC_CONNECT), null, null)), "X3 Open Connexion failed");
