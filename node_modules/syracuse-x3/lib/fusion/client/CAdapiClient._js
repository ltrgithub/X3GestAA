...
var CAdapiClientConfig = require("syracuse-x3/lib/fusion/model/CAdapiClientConfig").CAdapiClientConfig;
var CAdapiModifier = require('syracuse-x3/lib/fusion/model/CAdapiModifier').CAdapiModifier;
var CAdapiEntity = require('syracuse-x3/lib/fusion/model/CAdapiEntity').CAdapiEntity;
var CAdapiStackedWindows = require('syracuse-x3/lib/fusion/model/CAdapiStackedWindows').CAdapiStackedWindows;
var CAdapiStackedWindow = require('syracuse-x3/lib/fusion/model/CAdapiStackedWindow').CAdapiStackedWindow;
...
var CAdapiInstance = require('syracuse-x3/lib/fusion/model/CAdapiInstance').CAdapiInstance;
var AdapiMenuAct = require('syracuse-x3/lib/fusion/model/AdapiMenuAct').AdapiMenuAct;
var AdapiMenuCtx = require('syracuse-x3/lib/fusion/model/AdapiMenuCtx').AdapiMenuCtx;
var CAdapiDataSrc = require('syracuse-x3/lib/fusion/model/CAdapiDataSrc').CAdapiDataSrc;
var CAdapiDataSrcLine = require('syracuse-x3/lib/fusion/model/CAdapiDataSrcLine').CAdapiDataSrcLine;
var CAdapiVariable = require('syracuse-x3/lib/fusion/model/CAdapiVariable').CAdapiVariable;
var CStructIst = require('syracuse-x3/lib/fusion/types/CStructIst').CStructIst;
var CStructResu = require('syracuse-x3/lib/fusion/types/CStructResu').CStructResu;
var CDefAction = require('syracuse-x3/lib/fusion/client/CDefAction').Actions;
var CDefSuperAction = require('syracuse-x3/lib/fusion/client/CDefAction').SuperActions;
var ActionDescriptors = require('syracuse-x3/lib/fusion/client/CDefAction').Descriptors;
var nodeconfig = require('syracuse-main/lib/nodeconfig');
var formatter = require('syracuse-x3/lib/formatter/format');
var CONNECT_ACTION = require('syracuse-x3/lib/fusion/client/CDefAction').CONNECT_ACTION;
...
var serializers = {
	I: function(val) {
		if (!(val instanceof CStructIst)) throw new Error("action parameter type mismatch, expected IST, got " + val);
		return val.getBytes();
	},
	U: function(val) {
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	actionParams: function(body) {
		...
		if (target && target.xid != null) {
			params.push(new CStructIst(this.targetToXid(target)));
		}
	...
	processAction: function(_, body) {
		traceAction("************************************");
		traceAction("Begin Process Action:\n" + JSON.stringify(body, null, 2));
		...
		if (body.param && body.param.target){
			var target = body.param.target;
			if (target && target.xid != null) {
				ctx = new CStructIst(this.targetToXid(target));
			}
		}
		...
		if (body.fld) {
			if (body.fld.v){
				if (!body.fld.ist) {
					var type = typeof body.fld.v === 'string' ? CStructResu.TDO_ALPHA : CStructResu.TDO_INT;
					resu = CStructResu.fromAction(type, body.fld.v);
	
				}else{
					// Retrieve the instance to obtain the data type of the action's resu
					var ist = CStructIst.fromAction(body.fld.ist);
					traceAction("ACTION IST=" + JSON.stringify(ist,null,2));
					...
					if (entity && entity instanceof CAdapiVariable) {
						...
						try {
							if (entity.acceptEditFormat() && body.fld.fmtKind === 'EDIT') {
								...
								this.activeRequest.reply.addInstance(CAdapiInstance.newprevious(this, ist, entity.format.format, value, this.activeRequest.reply == null ? false : this.activeRequest.reply.stayInCommandMode));
							}

						} catch(e) {
							console.error("Err: "+e.stack);
							// Récupération de l'erreur de formatage pour info client
							var adapi = CAdapiInstance.newtarget(this, ist, entity.format.format, body.fld.v, this.activeRequest.reply == null ? false : this.activeRequest.reply.stayInCommandMode);
							adapi.setException(e);
							this.activeRequest.reply.addInstance(adapi);
						}
...
/*
	...
	processSuperAction: function(_, body) {
		traceAction("===================================");
		traceAction("Process Super Action:\n" + JSON.stringify(body, null, 2));
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			if (!CDefAction[action.name]) throw new Error("Bad sub-action: " + action.name);

			var processNext = true;
			newBody.act = CDefAction[action.name];		
	...
	getMessage: function() {
		...
		for (var i = 1; i < arguments.length; i++){
			args.push(arguments[i] ? arguments[i].toString() : "");
		}
...
/* TODO overload
	...
	removeCtxButtons: function() {
		for (var i in this.trackLists) {
			if (this.trackLists[i] instanceof AdapiMenuCtx) {
				console.error("instanceof AdapiMenuCtx");
			}
		}
	},
