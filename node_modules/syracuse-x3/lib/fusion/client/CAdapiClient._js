...
var AdapiMenuCtx = require('syracuse-x3/lib/fusion/AdapiMenuCtx').AdapiMenuCtx;
...
var ActionDescriptors = require('syracuse-x3/lib/fusion/CDefAction').Descriptors;
...
var CFormatterJS = exports.CFormatterJS = helpers.defineClass(function(settings) {
	...
	formatField: function(format, value, raw) {
		...
		case 'D':
			...
			if (raw || this.settings.dateOrder === 'YMD') {
				if (parts.length > 1) {
					...
					if (str.length <= 6) {
						y = parseInt(str.substring(0, 2) || '0', 10);
						m = parseInt(str.substring(2, 4) || '0', 10);
						d = parseInt(str.substring(4) || '0', 10);
					} else {
						y = parseInt(str.substring(0, 4) || '0', 10);
						m = parseInt(str.substring(4, 6) || '0', 10);
						d = parseInt(str.substring(6) || '0', 10);
					}
				}
			} else {
				if (parts.length > 1) {
					...
					str = value.replace(/[^\d]/g, '');
					d = parseInt(str.substring(0, 2) || '0', 10);
					m = parseInt(str.substring(2, 4) || '0', 10);
					y = parseInt(str.substring(4) || '0', 10);
				}
				if (this.settings.dateOrder === 'MDY') {
					var tmp = d;
					d = m;
					m = tmp;
				}
...
function serializeActionParams(act, params) {
	...
	if (desc.params.length !== params.length) throw new Error(desc.name + ": param count mismatch: expected " + desc.params.length + ', got ' + params.length);
	
	
	return Buffer.concat(params.map(function(param, i) {
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	processAction: function(_, body) {
		...
		if (body.fld) {
			if (body.fld.v){
				if (!body.fld.ist) {
					...
					console.log("ACTION IST=" + JSON.stringify(ist,null,2));
		...
		//console.error("RESPONSE ACTION: "+JSON.stringify(resp,null,2));
		return resp;
	},
...
/*
	...
	processSuperAction: function(_, body) {
		console.log("SUPERACTION BODY: " + JSON.stringify(body, null, 2));
...
/* TODO overload
	...
	getResponse: function() {
		...
		for (var i = 0; i < lists.ENTITY.length; i++) {
			...
			if (Object.keys(wins[win.entityIdxAlpha].entities).length === 0)
