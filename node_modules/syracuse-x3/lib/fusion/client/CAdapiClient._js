...
var automata = require("syracuse-x3/lib/fusion/automata/x3Automata");
var CAdapiClientConfig = require("syracuse-x3/lib/fusion/CAdapiClientConfig").CAdapiClientConfig;
...
var CAdapiReply = require('syracuse-x3/lib/fusion/client/CAdapiReply').CAdapiReply;
...
var CAdapiDataSrcLine = require('syracuse-x3/lib/fusion/CAdapiDataSrcLine').CAdapiDataSrcLine;
...
var CFormaterJava = exports.CFormaterJava = helpers.defineClass(function() {}, null, {
	...
	formatField: function(format, value, bool) {
	...
	// hack to get timestamp formatter ok
	dateOrder: 1,
});
...
var CMotherWindow = exports.CMotherWindow = helpers.defineClass(function(aClient) {
	CAdapiEntity.call(this, aClient, "&ROOT", 1, 'WINDOW');
}, CAdapiEntity, {
	getAdxId: function() {
...
var ignoredKeys = ['UIAction', 'Id', //
'ODBCType', 'ODBCName', 'ODBCDriver', 'ODBCBase', 'ODBCServer', 'ODBCPort', //
'ADXTypportal', 'ADXWtitle', 'ADXPortalTheme', 'ADXProductSuff', //
'UIAllowDevRpt', 'ChangeUserContext', 'ADXWebsite', 'Userconnect', //
'AboutWarningFile', ];

var SuperActions = {};

var SuperAction = helpers.defineClass(function(name, code, actions, stableTarget, checkFn) {
	this.name = name;
	this.code = code;
	this.actions = actions.map(function(act) {
		if (!CDefAction[act]) throw new Error("bad sub-action: " + act);
		return CDefAction[act];
	})
	this.stableTarget = stableTarget;
	this.checkFn = checkFn;
	SuperActions[this.code] = this;
}, null, {
	execute: function(_, client, body) {
		var t0 = body.fld && body.fld.ist;
		var resp;
		for (var i = 0; i < this.actions.length; i++) {
			var action = this.actions[i];
			body.act = action;
			resp = client.processAction(_, body);
			var t1 = resp.sap.target;
			/* TODO: see the exact condition.
			if (i < this.actions.length - 1 && this.stableTarget && //
			((t0.win && t0.win !== t1.win) || //
			(t0.xid && t0.xid !== t1.xid) || //
			(t0.nl && t0.nl !== t1.nl)) || //
			(this.checkFn && !this.checkFn(body, resp))) {
				// temporary throw, for initial debugging
				throw new Error("superaction failed, t0=" + JSON.stringify(t0) + ", t1=" + JSON.stringify(t1));;
			};
			*/
		}
		return resp;
...
new SuperAction("SUP_LIST_PREV", 779, ['SRV_GETFOCUS', 'SRV_LIST_PAGE_UP'], true);
new SuperAction("SUP_LIST_NEXT", 780, ['SRV_GETFOCUS', 'SRV_LIST_PAGE_DOWN'], true);
new SuperAction("SUP_LIST_CHANGELINE", 781, ['SRV_GETFOCUS', 'SRV_BRWCHANGELINE'], true); // same name as 784 in vm file!
new SuperAction("SUP_LIST_FILTER", 782, ['SRV_GETFOCUS', 'SRV_BRWRAPIDESEL'], true);
new SuperAction("SUP_LIST_SELLINEEX", 784, ['SRV_GETFOCUS', 'SRV_BRWSELECTLINE'], true);
// example with check function
new SuperAction("SUP_LIST_PICKING_FOCUS_SELECT", 798, ['SRV_GETFOCUS', 'SRV_PICKING', 'SRV_TREESELECTLINE'], true, function(req, resp) {
	throw new Error("NIY superaction check");
});
// more to come.

var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	this.stream = stream;
	this.adxInfos = adxInfos;
	this.clientConfig = new CAdapiClientConfig(aConfig);
	...
	this.newActiveRequest(new CAdapiReply(this, CONNECT_ACTION), null, null);
}, null, {
	...
	processAction: function(_, body) {
		...
		var reply = new CAdapiReply(this, body.act);
		this.newActiveRequest(reply, null, null);

		var resuBuf, extrasLenBuf, extrasBuf;

		if (body.fld) {
			if (!body.fld.ist) throw new Error("ist not found: " + JSON.stringify(body));
			// Retrieve the instance to obtain the data type of the action's resu
			var ist = CStructIst.fromAction(body.fld.ist);
			console.log("ACTION IST=" + ist);
			var entity = this.findModifiable(ist);
			if (entity == null) throw new Error("modifiable not found: " + JSON.stringify(body));
			console.log("ENTITY TYP: " + entity.entityTyp);
			if (entity.dataType == null) throw new Error("data type not found: " + JSON.stringify(body));

			// Build the resu and serialize it
			var resu = CStructResu.fromAction(entity.dataType, body.fld.v);
			resuBuf = resu.getBytes();
			...
			if (crsr != null) extras.push(new CStructResu(CStructResu.TDO_INT, parseInt(body.fld)));

			// serialize the number of extras on 1 byte + the extras
			extrasLenBuf = new Buffer([extras.length]);
			extrasBuf = Buffer.concat(extras.map(function(extra) {
				return extra.getBytes();
			}));
		}
		else {
			throw new Error("fld missing");
			// see later CSDataPayloadFusionRequest/getActionsParamsToSend 
			// should add a NULL CStructResu
		}

		// serialize the action on 2 bytes
		var actBuf = new Buffer([(body.act >> 8) & 0xff, body.act & 0xff]);
		// some action have resu parameter, get them.
		...
		var paramsBuf = Buffer.concat(params.slice(1).map(function(p) {
			return new CStructResu(CStructResu.TDO_ALPHA, p).getBytes();
		}));
	...
	processSuperAction: function(_, body) {
		var superAction = SuperActions[body.act];
		if (!superAction) throw new Error("SUPERACTION MISSING: " + body.act);
		return superAction.execute(_, this, body);
	},
	...
	addLogicalErrorMess: function(aObject, aMethod, aIst, aMessage) {
		...
		if (this.hasActiveRequest()) {
			...
			if (this.logicalErrorMessage == null) {
				...
				this.logicalErrorMessage = new CAdapiMessage(CAdapiReply.WEBSERVER, this, wMess, false);
			}
			this.logicalErrorMessage.addWebServerLogicalMess(CAdapiReply.buildLogicalErrorMess(aObject, aMethod, aIst, aMessage));
		}
	},
	...
	deconnectClient: function(reason) {
		try {
			...
			return checkFailed(deconnect(new CAdapiReply(this, CDefAction.SRV_FIN_APPL)), "X3 deconnexion failed");
		} finally {
	...
	executeCallUiOpenFile: function(aListOfProperties) {
		...
		if (wMax > 2) {
			 ...
			 */
				...
				wUrl += this.clientConfig.params[CAdapiClientConfig.USERHOMEVIRTUALPATH];
				wUrl += '/';
				wUrl += wLocalDir;
				wUrl += '/';
				wUrl += wLocalFile;
	...
	getNewStackedWindows: function(aWindowId, aFunctionId, aDescr, aStamp, aWindowIdxBaseOne) {
		return new CAdapiStackedWindow(this, aWindowId, aFunctionId, aDescr, aStamp, aWindowIdxBaseOne);
	},
	...
	uiActionSetTechnoVers: function(_, reader, values) {
		this.sessionSettings.Version = values.Version;
		this.sessionSettings.Protocol = values.Protocol;
		//for (var key in values) if (key !== "UIAction") this.sessionSettings[key] = values[key];
		// bad values, but they keep our replay test happy 
		values.Id = "2";
		values.Version = "null.null"; // for now"
		return values;
	},
	uiActionSessionSettings: function(_, reader, values) {
		for (var key in values) {
			if (ignoredKeys.indexOf(key) < 0) this.sessionSettings[key] = values[key];
		}
		return {
			UIAction: "SessionSettings"
		};
	},
	uiActionSetWindFeatures: function(_, reader, values) {
		// strange!
		return {
			UIAction: "SessionSettings"
		};
	},
	uiActionSethangup: function(_, reader, values) {
		return {
			Sethangup: 1
		};
	},
	...
	listenNodeND_SLF: function(_, reader, node, properties) {
		...
		var action = "uiAction" + values.UIAction;
		var fn = this[action];
		if (!fn) throw new Error("bad UI action: " + action);
		var result = fn.apply_(_, this, [reader, values], 0);
		var self = this;

		if (values.UIAction === "SessionSettings") {
			var keys = Object.keys(values);
			for (var i in values) {
				if (keys[i] !== "UIAction" && values[i] !== "SessionSettings") {
					self.appSrvSessionSettings[i] = values[i];
				}
			}
		}
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		try {
			...
			throw e;
			//throw new CCommException(CCommException.CANT_SEND, e, this.getMessCantSendAutomaticReply("NDPKAR"));
		}
	},
	...
	newActiveRequest: function(reply, aParams, sendContext) {
		...
		this.activeRequest = new CAdapiRequest(this, reply, aParams, sendContext);;
	...
	open: function(aAdonixContext) {
		...
		try {
			 ...
			 */
			var aX3Reply = checkFailed(connect(aAdonixContext, newActiveRequest(new CAdapiReply(this, CONNECT_ACTION), null, null)), "X3 Open Connexion failed");
	...
	sendActionCallRemote: function(aX3ActionArgs) {
		var wX3Reply = new CAdapiReply(this, CDefAction.SRV_CALLREMOTESPGM, CReply.OK);
	...
	sendX3Action: function(args, sendContext) {
		try {
			...
			return checkFailed(sendAction(new CAdapiReply(this, actionCode), args, sendContext), "X3 send action failed");
		} finally {
			releaseAllAffSynchroneMessages();
		}
	},
	...
	storeUrlPortalSessionSetting: function(aId, aUrlPortal) {
		...
		if (wPos == -1) {
			 ...
			 */
			wSB.append(this.clientConfig.getParam(CAdapiClientConfig.VIRTUALPATHPAGES));
	...
	getResponse: function() {
		...
		var wins = {};
		//	Map<String, CAdapiEntity> root = new HashMap<String, CAdapiEntity>(); // listdump
		for (var i = 0; i < lists.ENTITY.length; i++) {
			...
			if (!win) throw new Error("win not found: " + entity.getFatherWindowId());
			...
			if (entity instanceof CAdapiEntity) {
				var gridParent = null;
				var line = null;
				if (entity instanceof CAdapiVariable) {
					line = entity.srcLine;
				} else if (entity instanceof CAdapiDataSrcLine) {
					line = entity;
				} else if (entity instanceof CAdapiDataSrc) {
					gridParent = entity;
				}
				if (line != null) {
		...
		var response = {
			sap: {
				acts: lists.ACT.filter(isDefined),
				target: lists.TARGET[0],
				wins: wins,
			},
		...
		response.session = {
			settings: this.filterSessionSettings()
		};
		response.srvop = this.srvop;
		response.toJsonTime = (Date.now() - t0);
...
CAdapiClient.checkFailed = function(adxReply, info, reason) {
	if (adxReply == null || !adxReply.isOk()) {
		...
		if (t instanceof CAdapiException && t.formatException) {
			return adxReply;
		}
