...
function serializeActionParams(act, params) {
	...
	if (desc.params === '?') throw new Error("params serialization missing for " + desc.name);
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	actionParams: function(body) {
		...
		if (target && target.xid != null) {
			...
			params.push(new CStructIst(xid));
		}
	...
	processAction: function(_, body) {
		...
		if (body.fld) {
			...
			console.log("ACTION ENTITY=" + JSON.stringify(entity,null,2));
	...
	processSuperAction: function(_, body) {
		...
		for (var i = 0; i < superAction.actions.length; i++) {
			...
			if (action.checkTarget) {
				body.act = CDefAction[action.name];
				var t0 = body.param && body.param.target;
				...
				if (!t1) throw new Error("ist missing in resp: " + JSON.stringify(resp.sap.target));
				//console.log("T1 " + i + "=" + JSON.stringify(t1, null, 2));
				processNext = superAction.processNextAction(CDefAction[action.name], t0, t1, action.checkNumLine);
			}
			console.log("ProcessNext=" + processNext);
