...
var CAdapiClient = helpers.defineClass(function(aTraceCli, aAdapiConfig, aType) {
	...
	addLogicalErrorMess: function(aObject, aMethod, aIst, aMessage) {
		if (hasActiveRequest()) {
			getActiveRequest().getReply().addLogicalErrorMess(aObject, aMethod, aIst, aMessage);
		} else {
	...
	executeCallUiOpenFile: function(aListOfProperties) {
		...
		if (wMax > 2) {
			...
			var wReply = getActiveRequest().getAdonixReply();
	...
	open: function(aAdonixContext) {
		try {
			...
			if (isServerConnected() && aX3Reply.isOk()) {
				getConnClientNode().setLogicalErrorListner(this);
			}
	...
	if (adxReply == null || !adxReply.isOk()) {
		...
		if (adxReply != null) {
			...
			if (adxReply.hasMessages()) {
				for (var msg in adxReply.getMessages()) {
