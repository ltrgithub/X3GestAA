...
var SRVOP = {
	"request": {
		"id": 0,
		"v": 0,
		"session": "1f44ca4c-2616-4412-9aa9-0e9a66508612_1820599980",
		"status": 0,
		"server": "SOSYRSUPERV"
	},
	"tech": {
	...
	"sessinfo": {
		"cnxid": "b882e88b-a358-4018-9ff4-8ef604b3ab5e_1820599980",
		"sessid": "1f44ca4c-2616-4412-9aa9-0e9a66508612_1820599980",
		"config": {
			"jettyPort": "27880",
			"systemarch": "x86",
			"installvers": "JAV",
			"tools": "C:/SAGE/SAFEX3/JAVV1/BRIDGE/BridgeTools",
			"host": "SOSYRSUPERV",
			"systembits": "32",
			"sdataPort": "27080",
			"data": "C:/SAGE/SAFEX3/JAVV1/BRIDGE/BridgeData",
			"systemtype": "windows",
			"installversname": "JULIET",
			"installversnum": "16j",
			"installpatch": "115"
		},
		"bridge": {
			"port": 27080,
			"ipserver": "SOSYRSUPERV.sagefr.adinternal.com",
			"ipclient": "172.16.140.62"
		},
		"x3": {
			"folder": "SUPERV",
			"port": 17000,
			"x3profile": "ADMIN",
			"host": "172.28.16.106",
			"http": "http://172.28.16.106:80/Adonix_SOLSUPV6",
			"x3user": "CMI",
			"lang": "ENG"
		}
	},
	"sessionInfo": {
		"node": {
			...
			"x3port": 17000,
			"gitlastmodif": "Fri Jan 11 2013 19:10:36 GMT+0100 (CET)"
		}
	}
};
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, aConfig, aType) {
	this.stream = stream;
	this.clientConfig = aConfig;
	this.trackLists = this.newTrackLists();
	this.sessionSettings = aConfig.sessionSettings || {};
	this.srvop = SRVOP;
	...
	beginRequest: function(_, body) {
		...
		var data = resu.getBytes();
		var extra = [];
		var crsr = body.fld.ctx && body.fld.ctx.crsr;
		if (crsr != null) extra.push(new CStructResu(CStructResu.TDO_INT, parseInt(body.fld)));
		data = Buffer.concat([data, new Buffer([extra.length])]);
		extra.forEach(function(ext) {
			data = Buffer.concat([data, ext.getBytes()])
		});
		...
		lenBuf.writeInt16BE(2 + params.length, 0);
		var paramsBuf = Buffer.concat(params.map(function(p) {
			return p.getBytes();
		}));
		data = Buffer.concat([data, lenBuf, actBuf, paramsBuf]);
	...
	traverseSettable: function(aStructIst, aModifier) {
		if (aStructIst.isWindowIst() && aStructIst.winZeroBase == -1) {
			return aModifier.setAdapiModifiable(aStructIst, this.motherWindow);
		} else {
		...
		for (var i = 0; i < lists.ENTITY.length; i++) {
			...
			if (entity instanceof CAdapiEntity) {
				...
				if (winNode.entities[jsonKey] == null) {
					...
					if (Object.keys(jsonEntity).length !== 0) winNode.entities[jsonKey] = jsonEntity;
				}
			}
		}
