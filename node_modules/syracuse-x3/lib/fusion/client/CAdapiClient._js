...
var CAdapiResourcesHelper = require('syracuse-x3/lib/fusion/CAdapiResources').CAdapiResourcesHelper;
...
var CFormaterJava = exports.CFormaterJava = helpers.defineClass(function() {}, null, {
	...
	formatField: function(format, value, bool) { return "" + value; }
});
...
var CMotherWindow = exports.CMotherWindow = helpers.defineClass(function(aAdapiClient) {
	...
	toJson: function() {
		throw new Error("Ba tojson MotherWin");
	}
});
...
var UIActions = {
	SetTechnoVers: function(_, reader, values) {
		// bad values, but they keep our replay test happy 
		values.Id = "2";
		values.Version = "null.null"; // for now
		return values;
	},
	SessionSettings: function(_, reader, values) {
		return {
			UIAction: "SessionSettings"
		};
	},
	SetWindFeatures: function(_, reader, values) {
		// strange!
		return {
			UIAction: "SessionSettings"
		};
	},
	Sethangup: function(_, reader, values) {
		return {
			Sethangup: 1
		};
	},
...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, aAdapiConfig, aType) {
	this.stream = stream;
	this.clientConfig = aAdapiConfig;
	this.trackLists = this.newTrackLists();

	this.activeRequest = null;
	this.resources = new CAdapiResourcesHelper();
	...
	addLogicalErrorMess: function(aObject, aMethod, aIst, aMessage) {
		if (this.hasActiveRequest()) {
			...
			if (this.logicalErrorMessage == null) {
				var wMess = this.resources.getMessage(CAdapiResources.KEYMESS_ERROR_LOGICAL);
	...
	executeCallUiOpenFile: function(aListOfProperties) {
		...
		if (wMax > 2) {
			 ...
			 */
			if (wLocalDir === CALLUI_LOCALDIR_HTTP) {
			...
			while (wI < wMax) {
				...
				if (wPropertyId !== CALLUI_OPENFILE_LOCALDIR && wPropertyId !== CALLUI_OPENFILE_LOCALFILE) {
	...
	executeOneSessionSetting: function(aStructProperty, areply) {
		...
		if (aStructProperty.id.toUpperCase() === CHG_MONTH_LIBS.toUpperCase()) {
		...
		} else if (wId.toUpperCase() === CHG_DATE_FORMAT_SERVER.toUpperCase()) {
		...
		} else if (aStructProperty.id.toUpperCase() === CHG_PIVOT_DATE.toUpperCase()) {
	...
	initAdapiResources: function(aLanguage) {
		var wOldLang = this.resources != null ? this.resources.askedAdonxiLang : null;
		var wMustReinit = wOldLang == null || wOldLang !== aLanguage;
		if (wMustReinit) {
			//this.resources = new CAdapiResources(aLanguage);
		}
	},
	...
	listenNodeNDPKAR: function(_, reader, node) {
		...
		if (this.adxIstsOfPickedLines.length > 0) {
			...
			if (wLeftListLine != null) {
				...
				wActionArgs.add(wArgIst);
				var wMess = this.resources.getMessage(CAdapiResources.KEYMESS_ERROR_CANT_GET_PICKED_LINE_KEY, String.valueOf(wArgIst.idx));
...
/* TODO overload
	...
	validAdonixContext: function(aAdonixContext) {
		...
		try {
			...
			var wMess = this.resources.getMessage(CAdapiResources.KEYMESS_ERROR_CONTEXT_NOT_VALID, aAdonixContext.toString());
	...
	getResponse: function() {
		...
		for (var i = 0; i < lists.ENTITY.length; i++) {
			...
			if (!win) {
				win = this.stackedWindows.findWin(entity.getFatherWindowId().charCodeAt(0) - '@'.charCodeAt(0));
				if (!win) throw new Error("win not found: " + entity.getFatherWindowId());
				if (win.statusBar) lists.WINDOW[0].statBar = win.statusBar.toJSON();
				if (win.title) lists.WINDOW[0].tit = win.title;
			}	
			if (entity instanceof CAdapiEntity) {
				...
				var entityObsId = entity.getAdapiObservableId();

				if (Object.keys(lists.WINDOW[0].entities).indexOf(entityObsId) === -1) {
					var jsonEntity = entity.toJSON();
					
					if (Object.keys(jsonEntity).length !== 0)
						lists.WINDOW[0].entities[entityObsId] = jsonEntity;
				}
			}
		}
