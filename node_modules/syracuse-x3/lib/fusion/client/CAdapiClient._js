...
var CAdapiClient = exports.CAdapiClient = helpers.defineClass(function(stream, adxInfos, aConfig, aType) {
	...
	processAction: function(_, body) {
		...
		// Sanity checks
		if (!body.fld) throw new Error("fld not found: " + JSON.stringify(body));
		...
		var params = this.activeRequest.listenSendingArgs(body.act, this);
		var paramsBuf = Buffer.concat(params.map(function(p) {
			return p.getBytes();
		}));
	...
	executeCallUiOpenFile: function(aListOfProperties) {
		...
		if (wMax > 2) {
			 ...
			 */
				...
				wUrl.append(this.clientConfig.getParam(CAdapiClientConfig.USERHOMEVIRTUALPATH));
				wUrl.append('/');
	...
	executeOneSessionSetting: function(aStructProperty, areply) {
		...
		if (aStructProperty.id.toUpperCase() === "SetMonthList".toUpperCase()) {
		...
		} else if (wId.toUpperCase() === "DefaultDateFormat".toUpperCase()) {
			this.clientConfig.setServerDateFormat(aStructProperty.strValue);
			setOneSessionSetting(aStructProperty.id, aStructProperty.strValue);
		} else if (aStructProperty.id.toUpperCase() === "DefaultDatePivot".toUpperCase()) {
		...
		if (values.UIAction === "SessionSettings") {
			...
			for (var i in values) {
				if (keys[i] !== "UIAction" && values[i] !== "SessionSettings") {
					self.sessionSettings[i] = values[i];
				}
			}
		}
