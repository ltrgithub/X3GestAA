...
var CAdonixClient = function() {};

var CDynFormat = helpers.defineClass(function(aType, aFormater) {
...
var CModifiableFinder = helpers.defineClass(function(aTracer) {
	TODO_SUPER(aTracer);
...
var CMotherWindow = helpers.defineClass(function(aAdapiClient) {
...
var LIB_IST = "ist";
var LIB_LIB = "Lib";
var LIB_METHOD_LISTEN_NDACTOP = "NDACTOP";
var LIB_METHOD_LISTEN_NDCTSU = "NDCTSU";
var LIB_METHOD_LISTEN_NDOMNCO = "NDOMNCO";
var LIB_METHOD_LISTEN_NDOTXSC = "NDOTXSC";
...
var CAdapiClient = helpers.defineClass(function(aTraceCli, aAdapiConfig, aType) {
	TODO_SUPER(aAdapiConfig.openTrace(aTraceCli), aAdapiConfig, aType);

	this.activeRequest = null;
	this.adapiResources = null;
	this.adxIstsOfPickedLines = new ArrayList();
	this.affSynchroneMessages = new Vector();
	this.applicationID = null;
	this.sessDynFormat = new ArrayList();
	...
	this.adapiConfig.switchTracer(this.tracer);
	initFormater();
	...
	this.withLogicalErrDetails = this.adapiClientConfig.getBoolParam(CAdonixConfig.WITH_LOGICAL_ERR_DETAILS);
}, CAdonixClient, {
	...
	addStatusIconDef: function(instance, aNumText, aTxt, aImgId, aActionId) {
		...
		for (i = 0; i < this.stackedWindows.size() && winFound == null; i++) {
			...
			if (item.adxIst.winAlphaId.equals(instance.winAlphaId)) {
				winFound = item;
			}
		}
		if (winFound != null) {
	...
	changePivotDate: function(prop) {
		try {
			this.formater.setUserDateFormat(this.adapiConfig.uiParamIDATE, this.adapiConfig.uiParamSEPDATE, Integer.parseInt(prop.strValue));
		} catch (e) {
	...
	closeTrace: function() {
		if (this.tracer instanceof CTraceCliChanel) {
			if (this.traceCli != null) {
				this.traceCli.destroyChanel(this.tracer);
			}
			this.tracer = null;
			this.traceCli = null;
		}
	...
	configFormater: function() {
		try {
			...
			this.formater.setVOn(this.adapiConfig.formaterVon);
	...
	getConnClientNode: function() {
		return TODO_SUPER.connClientNode;
	},
	...
	listenNodeNDACTOBJ: function(node) {
		var WIN_NOHIDDEN = 4;
		var num = node.getNodeArgByte(1).intValue;
		if (num > WIN_NOHIDDEN) {
			var win = findModifiable(node.getNodeArgIst(0).structIst);
	...
	listenNodeNDACTOP: function(aSegmentNode) {
		var wActionId = aSegmentNode.getNodeArgInt(0).intValue;
		var wFlag = aSegmentNode.getNodeArgByte(1).intValue != 0;
		// /Trace
	...
	listenNodeNDCTSU: function(aSegmentNode) {
		var wType = aSegmentNode.getNodeArgByte(0).intValue;
		var wSubType = aSegmentNode.getNodeArgString(1).stringValue;
		var wComplement = aSegmentNode.getNodeArgString(2).stringValue;
		// /Trace
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
			var wTB = this.tracer.popTraceBuffer();
			wTB.appendMethodName(LIB_METHOD_LISTEN_NDCTSU);
			wTB.appendDescr(LIB_FORMAT_TYPE, wType);
	...
	listenNodeNDOMNCO: function(aSegmentNode) {
		var wFlg = aSegmentNode.getNodeArgByte(0).intValue;
		var wActionId = aSegmentNode.getNodeArgShort(1).intValue;
		var wTxt = aSegmentNode.getNodeArgString(2).stringValue;
		/*
	...
	listenNodeNDOTXSC: function(aSegmentNode) {
		var wIst = aSegmentNode.getNodeArgIst(0).structIst;
		var wNumText = aSegmentNode.getNodeArgByte(1).intValue;
		var wImgId = aSegmentNode.getNodeArgShort(2).intValue;
		var wTxt = aSegmentNode.getNodeArgString(3).stringValue;
		// String wTxt = aSegmentNode.getNodeArgString(4).stringValue;
		var wActionId = aSegmentNode.getNodeArgShort(5).intValue;
		// /Trace
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG) && this.tracer.traceModuleFilter(CAdapiClientConfig.TRACER_MODULE_BUTTONS)) {
			var wTB = this.tracer.popTraceBuffer();
	...
	listenNodeNDPKAR: function(aSegmentNode) {
		...
		if (this.adxIstsOfPickedLines.size() > 0) {
			...
			wSegReplyAction = new CSegmentAction(this.tracer, this.remoteProtocolNum, wPickedLine.action.intValue());
			// /Trace
			if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
				var wTB = this.tracer.popTraceBuffer();
			...
			wSegReplyAction = new CSegmentAction(this.tracer, this.remoteProtocolNum, CDefAction.SRV_ABANDON);
		}
		try {
	...
	releaseAllAffSynchroneMessages: function() {
		var wNbMess = this.nbServerTempMess;
		if (wNbMess > 0) {
	...
	removeActiveRequest: function(aAdonixReply) {
		...
		if (hasActiveRequest()) {
			wX3Reply = this.activeRequest.x3Reply;
			realeaseActiveRequest();
		}
		return wX3Reply;
	},
	...
	setpTraceCli: function(traceCli) {
		this.traceCli = traceCli;
	},
	...
	toStringDescr: function(aSeparator) {
		...
		if (this.tracer instanceof CTraceCliChanel) {
			...
			CStringFacilities.addInStringDescr(wSB, "Canal", this.tracer.toString());
		}
		if (aSeparator != null) {
			wSB.append(aSeparator);
		}
