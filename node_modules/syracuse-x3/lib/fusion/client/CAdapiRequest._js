...
var CAdapiRequest = exports.CAdapiRequest = helpers.defineClass(function(aClient, aAdonixReply, aParams, sendContext) {
	...
	buildIstDatasOfAction: function(aParam) {
		...
		try {
			...
			if (v.acceptEditFormat() && istData.isEditFormat()) {
				...
				this.reply.addInstance(CAdapiInstance.newprevious(cli, istData.ist, v.format.format, istData.editedValue, this.reply == null ? false : this.reply.stayInCommandMode));
			}
			...
			var adapi = CAdapiInstance.newtarget(cli, istData.ist, v.format.format, istData.editedValue, this.reply == null ? false : this.reply.stayInCommandMode);
	...
	listenActionArgsDOLEFTLISTLINE: function(aClient, aParams) {
		 ...
		 * param�tre
		 */
		if (typeof aParams[1] !== "string") throw new CAdapiException(this.client, "ARG_WRONG - need a \"CStructIst\" or a \"String\" !");
		var wArgIst = new CStructIst(aParams[1]);
		...
		aParams.push(wLeftListLine.cle.toString());
		wArgIst.idx = 0; //wArgIst.setLine(0);
		aParams[1] = wArgIst.toString();
	},
	...
	listenNodeNDOBOXI: function(_, reader, node, wTypeBox, wChx, wMessText, wMessTitre, wSleep) {
		...
		case OU_BSE: /* Boite de Selection ??? */
		default:
			break;
		}
		...
		var wNewMess = this.reply.addMessage(wTypeMess, 0, wMessText, wMessTitre, wDefaultButton, wSleep, wChx);
		...
		if (wReplyAuto) {
			...
			this.listenNodeNDOBOXISendAutoReply(wDefaultButton); // bouton par défaut
			// défini par
			// l'utilisateur
			/*
	...
	listenNodeNDOBOXISendAutoReply: function(aNumButton) {
		/*
		 * 15w_002 Fiche 37099 - Protocole: formalisation du segment champ
		 * courant
		 */

		var wIstDatas = new CStructResu(CStructResu.TAG_INT, aNumButton);
		var wProtocolNum = this.client.protocolNum;
		// 16j_000
		...
		try {
			...
			pAdonixClient.setServerNotConnected();
			var wErrMess = pAdonixClient.getAdonixResources().getMessage(CAdonixResources.KEYMESS_CANT_SEND_AUTOMATIC_REPLY, "NDOBOXI");
	...
	listenNodeNDOMENA: function(_, reader, node, wProfil) {
		...
		if (this.client.appSrvSessionSettings.usrProfile !== wProfil)
			this.client.appSrvSessionSettings.usrProfile = wProfil;
	},
	/**
