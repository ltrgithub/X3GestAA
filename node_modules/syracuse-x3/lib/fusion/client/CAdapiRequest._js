...
var CDataSrcAffStateSetter = exports.CDataSrcAffStateSetter = helpers.defineClass(function(cli, aAftbTyp, aNbLines) {
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		...
		if (this.done) {
			...
			setLogicalErrMess(this.cli.resources.getMessage(
...
var CAdapiRequest = exports.CAdapiRequest = helpers.defineClass(function(aAdapiClient, aAdonixReply, aParams, sendContext) {
	...
	buildIstDatasOfAction: function(aParam) {
		...
		if (istData == null) {
			var wMess = cli.resources.getMessage(
	...
	buildListenNodeMessage: function(aKeyMess, aValue1, aModifier) {
		var wMess = this.client.resources.getMessage(
		aKeyMess, aValue1);
...
//			return require('fs').readFileSync("./data/OAMK.xml", 'utf8');
	...
	listenNodeNDAFTB: function(_, reader, node, wArgIst, wAffTbState, wNbLines) {
		...
		this.client, wAffTbState, wNbLines);
		...
		if (!wDataSrcAffStateSetter.done) {
			...
			this, CProtocolDefs.protocoleDefs.getProtocolDef(this.conn.remoteProtocolNum).findNode(CDefNode.NDAFTB).fullIdInString, wArgIst.toString(), wMessLE);
	...
	listenNodeNDCHGR: function(_, reader, node, wArgIst, wState) {
		...
		if (!wStateSetter.done) {
			this.reply.addLogicalErrorMess(
			this, 'NDCHGR', wArgIst.toString(), buildListenNodeMessage(
	...
	listenNodeNDCHVL: function(_, reader, node, ist, resu) {
		...
		if (!wValueSetter.done) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_SETTER_VALUE, resu.toString(), wValueSetter);
	...
	listenNodeNDGLAR: function(_, reader, node, wArgIst, wCle) {
		...
		if (!wLeftListLineSelecter.done) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER, wCle.toString(), wLeftListLineSelecter);
	...
	listenNodeNDOBOXI: function(_, reader, node, wTypeBox) {
		if (wTypeBox == OU_BIF) {
			...
			if (wPos == 0 || wPos == 1) {
				...
				wAffSynchroneType, wAffSynchroneAction, wMessText, wMessTitre, -1, wSleep, -1, this.client);
