...
var CDataSrcInserter = exports.CDataSrcInserter = helpers.defineClass(function(cli, aTargetLine, aNbLines) {
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		this.done = aModifiable instanceof CAdapiDataSrc;
		console.log("\tsetMod=" + aStructIst.alphanumCodage + " - " + this.zeroBasedIdx + " - " + this.nbLines);
		if (this.done) {
			...
			if (wAdapiDataSrc.hasDimOverFlow()) {
				setLogicalErrMess(this.cli.adapiResources.getMessage(
				CAdapiResources.KEYMESS_ERROR_DIM_OVERFLOW, String.valueOf(wAdapiDataSrc.dim), String.valueOf(this.nbLines), String.valueOf(wAdapiDataSrc.lines.length)));
			}
		} else {
			setLogicalErrMess(this.cli.adapiResources.getMessage(
			CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE, aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
...
var CDownloadListner = exports.CDownloadListner = helpers.defineClass(function(aAdapiClient) {
...
var CAdapiRequest = exports.CAdapiRequest = helpers.defineClass(function(aAdapiClient, aAdonixReply, aParams, sendContext) {
	CAdonixRequest.call(this, aAdapiClient, aAdonixReply, aParams, sendContext);
	this.adapiClient = aAdapiClient;
	...
	buildIstDatasOfAction: function(aParam) {
		...
		try {
			...
			var adapi = CAdapiInstance.newtarget(cli, istData.ist, v.format.string, istData.editedValue, this.adonixReply == null ? false : this.adonixReply.stayInCommandMode);
			adapi.setException(e);
			this.reply.addAdapiInstance(adapi);
			throw e;
		}
	},
...
/* TODO overload
	...
	getAffSynchroneAction: function(aMess, aPos) {
		...
		if (wAction === CAdapi.AFF_SYNCHRONE_LIB_ACTION_MODIFY) {
			return CAdapi.AFF_SYNCHRONE_ACTION_MODIFY;
		}
		if (wAction === CAdapi.AFF_SYNCHRONE_LIB_ACTION_OPEN) {
			return CAdapi.AFF_SYNCHRONE_ACTION_OPEN;
		}
		if (wAction === CAdapi.AFF_SYNCHRONE_LIB_ACTION_CLOSE) {
	...
	getAffSynchroneType: function(aMess, aPos) {
		...
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_PROGRESS) {
			return CAdapi.AFF_SYNCHRONE_TYPE_PROGRESS;
		}
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_VALIDCAR) {
			return CAdapi.AFF_SYNCHRONE_TYPE_VALIDCAR;
		}
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_SESSION) {
			return CAdapi.AFF_SYNCHRONE_TYPE_SESSION;
		}
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_TEMPON) {
			return CAdapi.AFF_SYNCHRONE_TYPE_TEMPON;
		}
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_DEBUG) {
			return CAdapi.AFF_SYNCHRONE_TYPE_DEBUGON;
		}
		if (wType === CAdapi.AFF_SYNCHRONE_LIB_TYPE_VOICE) {
	...
	loadWindowDescription: function(_, wWindowsId) {
		...
		function getWindowXml(_, wWindowsId){
			...
			var opt = {
				method: "GET",
				url: "http://172.28.16.106/Adonix_SOLSUPV6/SUPERV/GEN/ENG/FENS/"+wWindowsId+".xml",
				headers: {}
			}
			if (helpers.proxy){
...
//			return require('fs').readFileSync("./data/OAMK.xml", 'utf8');
	...
	listenActionArgsDOLEFTLISTLINE: function(aAdapiClient, aParams) {
		...
		if (wLeftListLine instanceof CAdapiLeftListLineNode) {
			 ...
			 * "expandre"
			 */
			wLeftListNode.dataSrc.setNodeIdToExpanded(
			wArgIst.idx);
		}
	...
	listenActionArgsPICKING: function(aAdapiClient, aParams) {
		...
		while (wSTAdxids.hasMoreTokens()) {
			...
			wStructIst, new CLeftListPickingSelecter(wAdapiPickedLine.isSelection()));
	...
	/* from CAdonixRequest */
	listenNodeNDGIST: function(_, req, node, wIst) {
	 ...
	 * "AskGetFile"
	 */
	listenNodeND_ASK: function(_, req, node) {
	 ...
	 * fld=[0] idx=[0] ]
	 */
	listenNodeND_CLF: function(_, req, node, wIst) {
	 ...
	 */
	listenNodeND_DCL: function(_, req, node) {},
	 ...
	 * blc=[1] fld=[0] idx=[0]]
	 */
	listenNodeND_FCL: function(_, req, node) {},
	 ...
	 * srn=[34] blc=[1] fld=[0] idx=[0]]
	 */
	listenNodeND_ICL: function(_, req, node) {
		return [1];
	},
	 ...
	 */
	listenNodeND_NAP: function(_, req, node, wTxt) {
		this.reply.setAskOpenSession(wTxt);
	},
	listenNodeND_REB: function(_, req, node) {
		// Notify observer
	 ...
	 */
	listenNodeNDAFTB: function(_, req, node, wArgIst, wAffTbState, wNbLines) {
		var wDataSrcAffStateSetter = new CDataSrcAffStateSetter(
		this.adapiClient, wAffTbState, wNbLines);
		this.adapiClient.traverseSettable(wArgIst, wDataSrcAffStateSetter);
		if (!wDataSrcAffStateSetter.done) {
			...
			this, CProtocolDefs.protocoleDefs.getProtocolDef(this.conn.remoteProtocolNum).findNode(CDefNode.NDAFTB).fullIdInString, wArgIst.toStringDescr(), wMessLE);
	 ...
	 */
	listenNodeNDCHCR: function(_, req, node) {},
	 ...
	 */
	listenNodeNDCHFM: function(_, req, node, wArgIst, wFormat) {
		...
		if (!wFormatSetter.done) {
			...
			this.reply.addLogicalErrorMess(this, 'NDCHFM', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 */
	listenNodeNDCHGR: function(_, req, node, wArgIst, wState) {
		// 0 "SRV_GRIZO";
		// 1 "SRV_ACTZO"
		// 2 "SRV_DISZO";
		// state - status Java
		...
		this.adapiClient.traverseSettable(wArgIst, wStateSetter);
		if (!wStateSetter.done) {
			this.reply.addLogicalErrorMess(
			this, 'NDCHGR', wArgIst.toStringDescr(), buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_SETTER_STATE, CAdapiEntity.x3StateToString(wState), wStateSetter));
			// aSegmentNode.setLogicalErrFlag();
		}
	},

	listenNodeNDCHVB: function(_, req, node, wArgIst, wResus) {
		var wExtendResu = wResus[0]
		var wValueSetter = new CServerValueSetter(wExtendResu.valueResu);
		this.adapiClient.traverseSettable(wArgIst, wValueSetter);
		if (!wValueSetter.done) {
			...
			this.reply.addLogicalErrorMess(this, 'NDCHVB', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 * .x00.x54.x00.x00]
	 */
	listenNodeNDCHVL: function(_, req, node, ist, resu) {
		var wValueSetter = new CServerValueSetter(resu.resu);
		...
		if (!wValueSetter.done) {
			...
			this.reply.addLogicalErrorMess(this, 'NDCHVL', ist.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 */
	listenNodeNDCSTL: function(_, req, node, wArgIst, sty) {
		var wStyleSetter = new CServerStyleSetter(sty, StyleApply.getApply(wArgIst));
		this.adapiClient.traverseSettable(wArgIst, wStyleSetter);
		if (!wStyleSetter.done) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_SETTER_STYLE, sty, wStyleSetter);
			this.reply.addLogicalErrorMess(this, 'NDCHST', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 */
	listenNodeNDCTIT: function(_, req, node, wArgIst, wTitle) {
		this.adapiClient.traverseSettable(wArgIst, new CServerTitleSetter(wTitle));
	},

	/**
	 * Dynamisme des combos des menus locaux
	 */
	listenNodeNDDSLB: function(_, req, node, wArgIst, value) {
		var v = null;
		...
		if (v == null) {
			console.log("NDDSLB - Variable not found[" + wArgIst.alphanumCodage + "]");
		}
	},
	 ...
	 */
	listenNodeNDEXECI: function(_, req, node) {
	 ...
	 * arbre
	 */
	listenNodeNDGLAR: function(_, req, node, wArgIst, wCle) {
		var wLeftListLineSelecter = new CLeftListSelecterByKey(true, wCle, this);
		this.adapiClient.traverseSettable(wArgIst, wLeftListLineSelecter);
		console.log("DONE: " + wLeftListLineSelecter.done);
		if (!wLeftListLineSelecter.done) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER, wCle.toStringDescr(), wLeftListLineSelecter);
			this.reply.addLogicalErrorMess(this, 'NDGLAR', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 */
	listenNodeNDINSLI: function(_, req, node, wArgIst, wTargetLine, wNbLines) {
		console.log("INSLI - Start=" + wTargetLine + " - Nb=" + wNbLines);
		var wDataSrcInserter = new CDataSrcInserter(this.adapiClient, wTargetLine, wNbLines);
		this.adapiClient.traverseSettable(wArgIst, wDataSrcInserter);
		if (wDataSrcInserter.hasLogicalErrMess()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDINSLI', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 */
	listenNodeNDOBOXI: function(_, req, node, wTypeBox) {
		if (wTypeBox == OU_BIF) {
			...
			if (wPos == 0 || wPos == 1) {
				...
				if (wAffSynchroneType == CAdapi.AFF_SYNCHRONE_TYPE_SESSION) {
					wSleep = this.adapiClient.maxTimeBeforeAutomaticHangUp;
	 ...
	 * simple et arbre
	 */
	listenNodeNDOCAR: function(_, req, node, wArgIst, wWhat, wDefs) {
		var wModifier = new CLeftListColumnsModifier(wWhat, wDefs);
		this.adapiClient.traverseSettable(wArgIst, wModifier);
	},

	/**
	 * NDOFENH 0E0103 (Help_Direct) Demande affichage aide
	 */
	listenNodeNDOFENH: function(_, req, node, wTxt) {
	 ...
	 */
	listenNodeNDOFLD: function(_, req, node, wArgIst, wTxt1, wTxt2, wTxt3, wInt1, wTxt4, wTxt5, wIst) {
		...
		this.adapiClient.traverseSettable(wIst, new CLeftListAppender(-1, 0, "", wLine, wClf, null));
	},
	 ...
	 */
	listenNodeNDOLAR: function(_, req, node, wArgIst, wIdx, wBranche, wTxt, wVal, wClf, wClp) {
		this.adapiClient.traverseSettable(wArgIst, new CLeftListAppender(wIdx, wBranche, wTxt, wVal, wClf, wClp));
	},
	 ...
	 */
	listenNodeNDOMNBO: function(_, req, node, wIdm, wRng, wIde, wFlg, wActionId, wTxt, wAcc) {
	 ...
	 */
	listenNodeNDOUVIMP: function(_, req, node) {
		...
		// FDB Non utilisé -
		// this.reply.addAdapiReport(wExtendProperties.properties);
	 ...
	 */
	listenNodeNDOWIN: function(_, req, node, wIst, wWindowsId, wFunctionId, wStamp, wExtendedInfos) {
		...
		//try {
			...
			wAdapiStackedWindow = this.adapiClient.pushRequesterWindow(wWindowsId, wFunctionId, descr, wStamp, wIst.winStr);
	 ...
	 */
	listenNodeNDRZAR: function(_, req, node, ist) {
		this.adapiClient.traverseSettable(ist, new CLeftListDeleter());
	},

	/**
	 * NDSUPLI 0E0A00 (Adxcpl_delGridLines) Suppression lignes dans tableau
	 */
	listenNodeNDSUPLI: function(_, req, node, wArgIst, wTargetLine, wNbLines) {
		var wDataSrcDeleter = new CDataSrcDeleter(this.adapiClient, wTargetLine, wNbLines);
		this.adapiClient.traverseSettable(wArgIst, wDataSrcDeleter);
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDSUPLI', wArgIst.toStringDescr(), wMessLE);
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	 ...
	 * ou arbre
	 */
	listenNodeNDULAR: function(_, req, node, wArgIst, wCle) {
		this.adapiClient.traverseSettable(wArgIst, new CLeftListSelecterByKey(false, wCle, this));
	},
	...
	retrieveAdapiLeftList: function(aAdapiClient, aParams) {
		return aAdapiClient.findModifiable(CStructIst.newStructIst(aParams.get(1)));
	},
