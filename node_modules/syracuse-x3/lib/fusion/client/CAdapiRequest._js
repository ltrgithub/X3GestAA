...
var CDataSrcAffStateSetter = exports.CDataSrcAffStateSetter = helpers.defineClass(function(cli, aAftbTyp, aNbLines) {
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		...
		if (this.done) {
			...
			setLogicalErrMess(this.cli.resources.getMessage(CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE, aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
		}
		return this.done;
	}
});
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		...
		if (this.done) {
			...
			if (wAdapiDataSrc.hasDimOverFlow()) {
				setLogicalErrMess(this.cli.resources.getMessage(CAdapiResources.KEYMESS_ERROR_DIM_OVERFLOW, String.valueOf(wAdapiDataSrc.dim), String.valueOf(this.nbLines), String.valueOf(wAdapiDataSrc.lines.length)));
			}
		} else {
			setLogicalErrMess(this.cli.resources.getMessage(CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE, aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
		}
...
var CAdapiRequest = exports.CAdapiRequest = helpers.defineClass(function(aAdapiClient, aAdonixReply, aParams, sendContext) {
	...
	buildIstDatasOfAction: function(aParam) {
		...
		if (istData == null) {
			var wMess = cli.resources.getMessage(CAdapiResources.KEYMESS_ERROR_NO_ISTDATA, aParam.class.name);
		...
		try {
			...
			if (v.acceptEditFormat() && istData.isEditFormat()) {
				...
				this.reply.addAdapiInstance(CAdapiInstance.newprevious(cli, istData.ist, v.format.string, istData.editedValue, this.adonixReply == null ? false : this.adonixReply.stayInCommandMode));
			}
	...
	buildListenNodeMessage: function(aKeyMess, aValue1, aModifier) {
		var wMess = this.client.resources.getMessage(aKeyMess, aValue1);
...
/* TODO overload
	buildListenNodeMessage: function(aKeyMess, aValue1, aValue2, aModifier) {
		var wMess = this.client.resources.getMessage(aKeyMess, aValue1, aValue2);
...
//			return require('fs').readFileSync("./data/OAMK.xml", 'utf8');
	...
	listenNodeNDAFTB: function(_, reader, node, wArgIst, wAffTbState, wNbLines) {
		...
		if (!wDataSrcAffStateSetter.done) {
			...
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_AFFSTATE, wState, wDataSrcAffStateSetter);
	...
	listenNodeNDCHFM: function(_, reader, node, wArgIst, wFormat) {
		...
		if (!wFormatSetter.done) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_SETTER_FORMAT, wFormat, wFormatSetter);
	...
	listenNodeNDCHGR: function(_, reader, node, wArgIst, wState) {
		...
		if (!wStateSetter.done) {
			this.reply.addLogicalErrorMess(this, 'NDCHGR', wArgIst.toString(), buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_SETTER_STATE, CAdapiEntity.x3StateToString(wState), wStateSetter));
			// aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDCHVB: function(_, reader, node, wArgIst, wResus) {
		...
		if (!wValueSetter.done) {
			...
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_SETTER_BLOB, wBlobSize, wValueSetter);
	...
	listenNodeNDCHVL: function(_, reader, node, ist, resu) {
		...
		if (!wValueSetter.done) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_SETTER_VALUE, resu.toString(), wValueSetter);
	...
	listenNodeNDCSTL: function(_, reader, node, wArgIst, sty) {
		...
		if (!wStyleSetter.done) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_SETTER_STYLE, sty, wStyleSetter);
	...
	listenNodeNDDSLB: function(_, reader, node, wArgIst, value) {
		...
		if (this.sendContext != null) {
			var currentIst = this.sendContext.get(SENDCTX_PARAM_TARGET);
			if (currentIst != null) {
	...
	listenNodeNDGLAR: function(_, reader, node, wArgIst, wCle) {
		...
		if (!wLeftListLineSelecter.done) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER, wCle.toString(), wLeftListLineSelecter);
	...
	listenNodeNDINSLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcInserter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_INSERTER, String.valueOf(wNbLines), String.valueOf(wTargetLine), wDataSrcInserter);
	...
	listenNodeNDSUPLI: function(_, reader, node, wArgIst, wTargetLine, wNbLines) {
		...
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			var wMessLE = this.buildListenNodeMessage(CAdapiResources.KEYMESS_ERROR_DELETER, String.valueOf(wNbLines), String.valueOf(wTargetLine), wDataSrcDeleter);
