...
//			return require('fs').readFileSync("./data/OAMK.xml", 'utf8');
	...
	listenNodeNDAFTB: function(_, req, node, wArgIst, wAffTbState, wNbLines) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wDataSrcAffStateSetter);
		if (!wDataSrcAffStateSetter.done()) {
			...
			this, CProtocolDefs.protocoleDefs.getProtocolDef(this.conn.remoteProtocolNum).findNode(CDefNode.NDAFTB).fullIdInString, wArgIst.ist.toStringDescr(), wMessLE);
	...
	listenNodeNDCHFM: function(_, req, node, wArgIst, wFormat) {
		...
		var wIst = wArgIst.ist;
		this.adapiClient.traverseSettable(wIst, wFormatSetter);
		if (wFormatSetter.hasInferredState() && wArgIst.ist.isCellIst()) {
		...
		if (!wFormatSetter.done()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDCHFM', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDCHGR: function(_, req, node, wArgIst, wState) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wStateSetter);
		if (!wStateSetter.done()) {
			this.reply.addLogicalErrorMess(
			this, 'NDCHGR', wArgIst.ist.toStringDescr(), buildListenNodeMessage(
	...
	listenNodeNDCHVB: function(_, req, node, wArgIst, wResus) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wValueSetter);
		if (!wValueSetter.done()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDCHVB', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDCSTL: function(_, req, node, wArgIst, sty) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wStyleSetter);
		if (!wStyleSetter.done()) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_SETTER_STYLE, sty, wStyleSetter);
			this.reply.addLogicalErrorMess(this, 'NDCHST', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDDSLB: function(_, req, node, wArgIst, value) {
		...
		if (v == null) {
			console.log("NDDSLB - Variable not found[" + wArgIst.ist.alphanumCodage + "]");
		}
	},
	...
	listenNodeNDGLAR: function(_, req, node, wArgIst, wCle) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wLeftListLineSelecter);
		if (!wLeftListLineSelecter.done()) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER, wCle.toStringDescr(), wLeftListLineSelecter);
			this.reply.addLogicalErrorMess(this, 'NDGLAR', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDINSLI: function(_, req, node, wArgIst, wTargetLine, wNbLines) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wDataSrcInserter);
		if (wDataSrcInserter.hasLogicalErrMess()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDINSLI', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDOCAR: function(_, req, node, wArgIst, wWhat, wDefs) {
		if (!wDefs.isComplete()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDOCAR', wArgIst.ist.toStringDescr(), wMess);
		}
		var wModifier = new CLeftListColumnsModifier(
		wWhat, wDefs);
		this.adapiClient.traverseSettable(wArgIst.ist, wModifier);
	},
	...
	listenNodeNDOLAR: function(_, req, node, wArgIst, wIdx, wBranche, wTxt, wVal, wClf, wClp) {
		if (!wVal.isComplete() || !wClf.isComplete() || !wClp.isComplete()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDOLAR', wArgIst.ist.toStringDescr(), wMess);
		}
		this.adapiClient.traverseSettable(wArgIst.ist, new CLeftListAppender(wIdx, wBranche, wTxt, wVal, wClf, wClp));
	},
	 ...
	 */
	listenNodeNDOWIN: function(_, req, node, wIst, wWindowsId, wFunctionId, wStamp, wExtends) {
	...
	listenNodeNDSUPLI: function(_, req, node, wArgIst, wTargetLine, wNbLines) {
		...
		this.adapiClient.traverseSettable(wArgIst.ist, wDataSrcDeleter);
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			...
			this.reply.addLogicalErrorMess(this, 'NDSUPLI', wArgIst.ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
	},
	...
	listenNodeNDULAR: function(_, req, node, wArgIst, wCle) {
		this.adapiClient.traverseSettable(wArgIst.ist, new CLeftListSelecterByKey(false, wCle, this));
	},
