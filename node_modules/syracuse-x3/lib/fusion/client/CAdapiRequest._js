...
var CAdapiLeftListLineNode = require('syracuse-x3/lib/fusion/model/CAdapiLeftListLineNode').CAdapiLeftListLineNode;
var CAdapiException = require('syracuse-x3/lib/fusion/model/CAdapiException').CAdapiException;
var CAdapiWindow = require('syracuse-x3/lib/fusion/model/CAdapiWindow').CAdapiWindow;
var CAdapiMessage = require('syracuse-x3/lib/fusion/model/CAdapiMessage').CAdapiMessage;
...
var CAdapiRequest = exports.CAdapiRequest = helpers.defineClass(function(aClient, aAdonixReply, aParams, sendContext) {
	...
	buildIstDatasOfAction: function(aParam) {
		...
		if (aParam && aParam instanceof CStructIst) {
			ist = aParam;
		}
		...
		if (ist == null) {
			...
			throw new CAdapiException(this.client, wMess);
		}
		...
		if (ist.editedResuTyp == CStructResu.X3TYP_NULL) {
			return ist;
		}
		...
		try {
			...
			if (v.acceptEditFormat() && ist.isEditFormat()) {
				...
				this.reply.addInstance(CAdapiInstance.newprevious(cli, ist.ist, entity.format.format, ist.editedValue, this.reply == null ? false : this.reply.stayInCommandMode));
			}
			...
			var adapi = CAdapiInstance.newtarget(cli, ist.ist, v.format.format, ist.editedValue, this.reply == null ? false : this.reply.stayInCommandMode);
...
/* TODO overload
	...
	listenActionArgsDOLEFTLISTLINE: function(aClient, aParams) {
		...
		if (typeof wArgIst === "string")
			wArgIst = new CStructIst(aParams[1]);
		else if (!(wArgIst instanceof CStructIst)) throw new CAdapiException(this.client, "ARG_WRONG - need a \"CStructIst\" or a \"String\" !");
	...
	listenActionArgsPICKING: function(aClient, aParams) {
		...
		for (var i = 0; i < wSTAdxids.length; i++) {
			wStructIst = new CStructIst(wSTAdxids[i]);
	...
	listenNodeNDOBOXI: function(_, reader, node, wTypeBox, wChx, wMessText, wMessTitre, wSleep) {
		...
		if (wTypeBox == OU_BIF) {
			...
			if (wPos == 0 || wPos == 1) {
				...
				if (wAffSynchroneType == CAdapi.AFF_SYNCHRONE_TYPE_DEBUGOFF) {
					this.reply.addMessage(wAdapiMessage);
				}
		...
		var wNewMess = this.reply.addMessage(new CAdapiMessage(wTypeMess, 0, wMessText, wMessTitre, wDefaultButton, wSleep, wChx,this.client));
		/**
	...
	listenNodeNDOBOXISendAutoReply: function(_, aNumButton) {
		...
		var resu = new CStructResu(CStructResu.TDO_INT, aNumButton);
		try {
			//var wSegReplyAction = CSegmentActionFactory.buildSegmentAction(pTracer, true, wProtocolNum, CDefAction.SRV_CHAMP_SUIVANT, wIstDatas);
			// pCommClient.writeSegment(wSegReplyAction, false);
			this.client.sendAction(_, resu, [], [CDefAction.SRV_CHAMP_SUIVANT]);
		}
		catch (e) {
	...
	retrieveLeftList: function(aClient, aParams) {
		return aClient.findModifiable(new CStructIst(aParams[1]));
	},
