...
var CDataSrcAffStateSetter = helpers.defineClass(function(cli, aAftbTyp, aNbLines) {
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		...
		if (this.done) {
			...
			CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE, aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
		}
		return this.done;
	}
});
...
var CDataSrcInserter = helpers.defineClass(function(cli, aTargetLine, aNbLines) {
	...
	setAdapiModifiable: function(aStructIst, aModifiable) {
		this.done = aModifiable instanceof CAdapiDataSrc;
		Utils.toConsole("\tsetMod=" + aStructIst.getAlphanumCodage() + " - " + this.zeroBasedIdx + " - " + this.nbLines);
		if (this.done) {
			...
			if (wAdapiDataSrc.hasDimOverFlow()) {
				setLogicalErrMess(this.cli.getAdapiResources().getMessage(
				CAdapiResources.KEYMESS_ERROR_DIM_OVERFLOW, String.valueOf(wAdapiDataSrc.getDim()), String.valueOf(this.nbLines), String.valueOf(wAdapiDataSrc.size())));
			}
		} else {
			setLogicalErrMess(this.cli.getAdapiResources().getMessage(
			CAdapiResources.KEYMESS_ERROR_NOT_RIGHT_MODIFIABLE, aModifiable.getAdxId(), LABEL_CLASS_DATASRC));
...
var CAdapiRequest = helpers.defineClass(function(aAdapiClient, aAdonixReply, aParams, sendContext) {
	...
	buildIstDatasOfAction: function(aParam) {
		...
		if (istData == null) {
			...
			CAdapiResources.KEYMESS_ERROR_NO_ISTDATA, aParam.getClass().getName());
		...
		try {
			...
			if (v.acceptEditFormat() && istData.isEditFormat()) {
				...
				istData.setEditedValue(v.formatEditToDisplay(istData.getEditedValue()));
				...
				CAdapiInstance.newprevious(cli, istData.getIst(), v.getFormat().getString(), istData.getEditedValue(), this.adonixReply == null ? false : this.adonixReply.getStayInCommandMode()));
			}
			// deformatage : format presentation => format brut
			istData.setValueProtected(v.getResuTyp(), v.unFormatData(istData.getEditedValue()));
			v.setBrowserUnformatedValue(istData);
			return istData;
		} catch (e) {
			// Récupération de l'erreur de formatage pour info client
			var adapi = CAdapiInstance.newtarget(cli, istData.getIst(),
			v.getFormat().getString(), istData.getEditedValue(), this.adonixReply == null ? false : this.adonixReply.getStayInCommandMode());
	...
	getLefListLineSelected: function() {
		return this.lefListLineSelected;
	},
	getX3Reply: function() {
	...
	listenActionArgsDOLEFTLISTLINE: function(aAdapiClient, aParams) {
		...
		if (wParam instanceof CStructIst) {
			...
			throw new CAdapiException(getAdapiClient(), "ARG_WRONG - need a \"CStructIst\" or a \"String\" !");
		}
		if (!wArgIst.isLineIst()) {
			throw new CAdapiException(getAdapiClient(), "ARG_WRONG - need a \"line\" ist !");
		}
		...
		if (this.tracer.traceLevelFilter(ITracer.LEVEL_DEBUG)) {
			...
			if (wMax > 0) {
				var wI = 0;
				while (wI < wMax) {
					wTB.appendDescr("ParamClass", CStringFacilities.getClasseName(aParams.get(wI).getClass()));
					wI++;
				}
			}
	...
	listenNodeND_ASKGetFile: function(aExtendProperties) {
		...
		getAdapiClient());
		return getAdapiClient().getNewAdonixFileManager(wDownloadListner).getFile(wRemoteFileDescriber);
	},
	listenNodeND_ASKSendAutoReply: function(aNumError, aExplaination) {
		...
		try {
			...
			var wErrMess = this.adonixClient.getAdonixResources().getMessCantSendAutomaticReply("ND_ASK");
	...
	listenNodeND_CLF: function(aSegmentNode) {
		...
		var wAdapiStackedWindow = getAdapiClient().popRequesterWindow();
		...
		if (this.tracer.isTraceDebugOn()) {
			...
			wTB.appendDescr("LastPopedWindowId", wX3Reply.getLastPopedWindowId());
	...
	listenNodeNDAFTB: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wDataSrcAffStateSetter);
		if (!wDataSrcAffStateSetter.done()) {
			...
			(getReply()).addLogicalErrorMess(
			this, CProtocolDefs.getProtocoleDefs().getProtocolDef(getConn().getRemoteProtocolNum()).findNode(this.tracer, CDefNode.NDAFTB).getFullIdInString(), wArgIst.getStructIst().toStringDescr(), wMessLE);
	...
	listenNodeNDCHFM: function(aSegmentNode) {
		...
		if (wFormatSetter.hasInferredState() && wArgIst.getStructIst().isCellIst()) {
			// pour designer la colonne
			wIst.setLine(-1);
			var wStateSetter = new CEntityStateSetter(this.tracer, wFormatSetter.getInferredState());
		...
		if (!wFormatSetter.done()) {
			...
			(getReply()).addLogicalErrorMess(this, METH_NDCHFM, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDCHGR: function(aSegmentNode) {
		...
		if (!wStateSetter.done()) {
			(getReply()).addLogicalErrorMess(
			this, METH_NDCHGR, wArgIst.getStructIst().toStringDescr(), buildListenNodeMessage(
	...
	listenNodeNDCHVB: function(aSegmentNode) {
		...
		var wValueSetter = new CServerValueSetter(this.tracer, wExtendResu.getValueResu());
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wValueSetter);
		if (!wValueSetter.done()) {
			var wBlobSize = String.valueOf(wExtendResu.getValueResu().getValueLength());
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_SETTER_BLOB, wBlobSize, wValueSetter);
			(getReply()).addLogicalErrorMess(this, METH_NDCHVB, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDCHVL: function(aSegmentNode) {
		...
		var wValueSetter = new CServerValueSetter(this.tracer, resu.getResu());
		...
		if (!wValueSetter.done()) {
			...
			(getReply()).addLogicalErrorMess(this, METH_NDCHVL, ist.toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDCSTL: function(aSegmentNode) {
		...
		if (!wStyleSetter.done()) {
			...
			(getReply()).addLogicalErrorMess(this, METH_NDCHST, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDCTIT: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), new CServerTitleSetter(this.tracer, wTitle));
		return true;
	}
	...
	listenNodeNDDSLB: function(aSegmentNode) {
		...
		if (getSendContext() != null) {
			var currentIst = getSendContext().get(
			SENDCTX_PARAM_TARGET);
			if (currentIst != null) {
				v = getAdapiClient().findModifiable(currentIst);
		...
		if (v == null) {
			Utils.toConsole("NDDSLB - Variable not found[" + wArgIst.getStructIst().getAlphanumCodage() + "]");
		}
		return true;
	}
	...
	listenNodeNDEXECI: function(aSegmentNode) {
		...
		// notiyObservers
		new CAdapiInstanceExeci(ack_call.getXml(), getAdapiClient(), getAdapiClient().getCurrentField());
	...
	listenNodeNDGLAR: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wLeftListLineSelecter);
		if (!wLeftListLineSelecter.done()) {
			var wMessLE = this.buildListenNodeMessage(
			CAdapiResources.KEYMESS_ERROR_LLLINE_SELECTER, wCle.toStringDescr(), wLeftListLineSelecter);
			(getReply()).addLogicalErrorMess(this, METH_NDGLAR, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDINSLI: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wDataSrcInserter);
		if (wDataSrcInserter.hasLogicalErrMess()) {
			...
			(getReply()).addLogicalErrorMess(this, METH_NDINSLI, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDOBOXI: function(aSegmentNode) {
		...
		if (wTypeBox == OU_BIF) {
			var wMessText = aSegmentNode.getNodeArgString(2).getStringValue();
			...
			if (wPos == 0 || wPos == 1) {
				var wMessTitre = aSegmentNode.getNodeArgString(3).getStringValue();
				...
				if (wAffSynchroneType == CAdapi.AFF_SYNCHRONE_TYPE_SESSION) {
					wSleep = (this.commClient).getMaxTimeBeforeAutomaticHangUp();
	...
	listenNodeNDOCAR: function(aSegmentNode) {
		...
		if (!wDefs.isComplete()) {
			...
			var wMess = CStringFacilities.sprintf(wMessMdl, "Defs", String.valueOf(wDefs.size()), String.valueOf(wDefs.getNbReceivedElements()));
			...
			(getReply()).addLogicalErrorMess(this, METH_NDOCAR, wArgIst.getStructIst().toStringDescr(), wMess);
		}
	...
	listenNodeNDOFLD: function(aSegmentNode) {
		...
		getAdapiClient().traverseSettable(wIst, new CLeftListAppender(this.tracer, -1, 0, "", wLine, wClf, null));
		return true;
	}
	...
	listenNodeNDOLAR: function(aSegmentNode) {
		...
		if (!wVal.isComplete() || !wClf.isComplete() || !wClp.isComplete()) {
			...
			CAdapiResources.KEYMESS_ERROR_ARG_RESU_TBL);
			if (!wVal.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Val", String.valueOf(wVal.size()), String.valueOf(wVal.getNbReceivedElements()));
			}
			if (!wClf.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Clf", String.valueOf(wVal.size()), String.valueOf(wClf.getNbReceivedElements()));
			}
			if (!wClp.isComplete()) {
				wMess += CStringFacilities.sprintf(wMessMdl, "Clp", String.valueOf(wVal.size()), String.valueOf(wClp.getNbReceivedElements()));
			}(getReply()).addLogicalErrorMess(this, METH_NDOLAR, wArgIst.getStructIst().toStringDescr(), wMess);
			this.tracer.trace(this, 1, "####" + wMess);
		}
		getAdapiClient().traverseSettable(
		wArgIst.getStructIst(), new CLeftListAppender(this.tracer, wIdx, wBranche, wTxt, wVal, wClf, wClp));
		return true;
	}
	...
	listenNodeNDOUVIMP: function(aSegmentNode) {
		...
		try {
			...
			var wErrMess = this.adonixClient.getAdonixResources().getMessCantSendAutomaticReply("NDOUVIMP");
	...
	listenNodeNDOWIN: function(aSegmentNode) {
		...
		try {
			...
			wWindowsId, wFunctionId, wStamp, wIst.getStructIst().getWin());
			...
			if (aSegmentNode instanceof CSegmentNodeExtended) {
				...
				if (wSegNodeExtended.hasExtends()) {
					var wExtendedInfos = (wSegNodeExtended.getNodeExtend(0)).getValue();
			...
			if (this.tracer.isTraceDebugOn()) {
				...
				wTB.appendDescr("NbPushedWindows", wX3Reply.getNbPushedWindows());
				wTB.appendDescr("LastPushedWindowId", wX3Reply.getLastPushedWindowId());
				aSegmentNode.addDescrInTB(wTB);
				this.tracer.trace(this, ITracer.LEVEL_DEBUG, wTB);
				this.tracer.pushTraceBuffer(wTB);
			}
	...
	listenNodeNDRZAR: function(aSegmentNode) {
		getAdapiClient().traverseSettable(
		aSegmentNode.getNodeArgIst(0).getStructIst(), new CLeftListDeleter(this.tracer));
		return true;
	}
	...
	listenNodeNDSUPLI: function(aSegmentNode) {
		...
		var wDataSrcDeleter = new CDataSrcDeleter(getAdapiClient(), wTargetLine, wNbLines);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), wDataSrcDeleter);
		if (wDataSrcDeleter.hasLogicalErrMess()) {
			...
			(getReply()).addLogicalErrorMess(this, METH_NDSUPLI, wArgIst.getStructIst().toStringDescr(), wMessLE);
			aSegmentNode.setLogicalErrFlag();
		}
		return true;
	}
	...
	listenNodeNDULAR: function(aSegmentNode) {
		var wArgIst = aSegmentNode.getNodeArgIst(0);
		var wCle = aSegmentNode.getNodeArgTblResuOld(1);
		getAdapiClient().traverseSettable(wArgIst.getStructIst(), new CLeftListSelecterByKey(this.tracer, false, wCle, this));
		return true;
	}
