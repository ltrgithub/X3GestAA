...
var CAdapiReply = exports.CAdapiReply = helpers.defineClass(function(aAdapiClient, aOrderId, aSTAT, aReplyMessages) {
	...
	addAdapiInstance: function(aAdapiInstance) {
		...
		this.instances.add(aAdapiInstance);
		if (aAdapiInstance.hasException()) {
			this.setTargetAdxId(aAdapiInstance.ist, aAdapiInstance.entity);
		}
	...
	addLogicalErrorMess: function(aObject, aMethod, aIst, aMessage) {
		...
		if (this.client.isWithLogicalErrDetails()) {
			...
			if (wMessage == null) {
				var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_LOGICAL);
	...
	*/
	addTargetInstance: function() {
		...
		try {
			...
			wAdapiInstance = CAdapiInstance.newtarget(this.client, this.targetIst, wFormatString, wFormatedValue, this.stayInCommandMode);
	...
	getTargetEntity: function() {
		if (this.targetEntity == null) {
			...
			if (wTargetEntity == null) {
				var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_CURFORMAT_NOIST, this.targetIst == null ? "NULL" : this.targetIst.toString());
				this.client.addLogicalErrorMess(this, "getTargetFormat", this.targetIst == null ? "NULL" : this.targetIst.alphanumCodage, wMess);
			} else {
	...
	initAdapiInstances: function() {
		if (this.instances == null) {
			this.instances = new CAdapiInstances();
		}
	},
	...
	validTargetFormat: function(aFormat) {
		if (aFormat == null || !aFormat.isValid()) {
			var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_CURFORMAT_NOFORMAT, this.targetIst.toString());
			this.client.addLogicalErrorMess(this, "getTargetFormat", this.targetIst.alphanumCodage, wMess);
			return wMess;
		} else {
	...
	addPopedWindow: function(aAdapiRequesterWindow) {
		...
		this.popedWindows.push(aAdapiRequesterWindow);
		setChangeWindow();
	},
	addPushedWindow: function(aAdapiRequesterWindow) {
		...
		this.pusheddWindows.push(aAdapiRequesterWindow);
