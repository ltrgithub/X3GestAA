	...
	*/
		...
		try {
			if (getTargetEntity() instanceof CAdapiVariable) {
				...
				if (wAdapiVariable.acceptEditFormat()) {
					wFormatString = validTargetFormat(getTargetEntity().format);
					wFormatedValue = wAdapiVariable.formatX3ToEdit();
					wServerValue = wAdapiVariable.getServerValStr();
				}
			}
			wAdapiInstance = CAdapiInstance.newtarget(this.adapiClient, getTargetIst(), wFormatString, wFormatedValue, this.stayInCommandMode);
			if (getTargetEntity() instanceof CAdapiVariableDate) {
				wAdapiInstance.setRoughValue((getTargetEntity()).getServerValStr());
			}
		} catch (e) {
			...
			wAdapiInstance = CAdapiInstance.newtarget(this.adapiClient, getTargetIst(), wFormatString, wServerValue, this.stayInCommandMode);
		}
	...
	findTargetEntity: function() {
		...
		if (wTargetEntity == null) {
			...
			this.adapiClient.addLogicalErrorMess(this, LIB_METHOD_GET_TARGET_FORMAT, getTargetIst() == null ? "NULL" : getTargetIst().alphanumCodage, wMess);
		}
	...
	validTargetFormat: function(aFormat) {
		if (aFormat == null || !aFormat.isValid()) {
			...
			this.adapiClient.addLogicalErrorMess(this, LIB_METHOD_GET_TARGET_FORMAT, getTargetIst().alphanumCodage, wMess);
			return wMess;
		} else {
