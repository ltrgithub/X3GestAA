...
var CAdapiStackedWindow = helpers.defineClass(function() {}, CAdapiEntity, {
	private static String ATT_NAM = "NAM";
	private static String TAG_SCRN = "SCRN";
	private static String ATT_NCO = "NCO"; // nombre de colonne du premier �cran
	private static String ATT_NLI = "NLI"; // nombre de ligne du premier �cran
	private static String ATT_TYF = "TYF"; // type de fen�tre adonix : girl=1, own=2 (modal), comment=3 (fen�tre sans bouton), dialogue=4
	private static String ATT_WTY = "WTY";
	private static String TAG_WIND = "WIND";

	private static CAdapiWindow instanciate(aWindowDescription) {
	...
	protected static void newScreens(aTracer, aAdapiClient, aAdapiRequesterWindow, aWindowDescription, aAdapiScreens, aListOfDataSrcs) {
		...
		var wI = 0;
		while (wI < wMax) {
			wScreenNode = (Element) wScreensNodes.item(wI);
	...
	// barre de status associ�e a l'ecran
	private AdapiStatusBar this.adapiStatusBar = null;
	private boolean opened = false;
	private CAdapiListOfScreens this.adapiScreens;
	private CAdapiWindow this.adapiWindow;
	protected Document this.descrDom;
	// nom de la fonction ayant ouvert la fen�tre
	private String this.functionId;
	private CAdapiListOfDataSrcs this.listOfDataSrcs;
	//  DataSource Left associ�es � l'objet
	private CAdapiListOfLeftLists this.listOfLeftLists;
	// index de la fen�tre empil�e dans la pile (en base 0)
	private int this.localIndexBaseZero;
	private AdapiMenuWin.AdapiWinMenuList this.menuList = null;
	// Table des menus locaux associ�es au requester
	private CAdapiListOfLocalMenus this.localMenus;
	private String this.ndOWinComplementDescr = null;
	// Identifiant Serveur de la DataSource "Liste Gauche" standard
	public String this.primaryLeftListId;
	private int this.screenToShow = -1;
	private String this.timeStamp = "";
	private long this.timeStampMilliSec = -1;
	private int stackLevel = -1;

	public CAdapiStackedWindow(aAdapiClient, aWindowId, aFunctionId, aStamp, aEnityIndexBaseOne) {
	...
	}, setMenuList: function(AdapiMenuWin.AdapiWinMenuList menuList) {
		...
		this.opened = opened;
		// En dernier
		setTypeJsonNode(ETypeJsonNode.FUNC);// catch close ou open fonction
		...
		try {
			...
			if (isOpened()) {
				winO.put(JSON_NAME, getId());
				winO.put(JSON_SCRNNUM, this.screenToShow >= 1 ? this.screenToShow : 1);
				winO.put(JSON_STAMP, getTimeStampInString());
				winO.put(JSON_EXTEND, getNdOWinComplementDescr());
				winO.put(JSON_FUNC, getFunctionId());
				winO.put(JSON_TYPE, getWindowTypeInString());
				// check if the left list have change fmt and/or title to add appendModel
				var extModel = null;
				for (item : this.listOfLeftLists) {
					if (item instanceof CAdapiLeftList) {
						var list = (CAdapiLeftList) item;
				...
				if (extModel != null) {
					...
					for (item : extModel.getEntrySup().getVars()) {
						var itemJs = new JSONObject();
						itemJs.put(JSON_TIT, item.getTitle());
						itemJs.put(JSON_FMT, item.getFormat() != null ? item.getFormat().getString() : "");
						appendModel.put(item.getJsonKey(), itemJs);
					}
					var dolaritem = new JSONObject();
					dolaritem.put(JSON_DOLLAR, appendModel);
					var leftListApp = new JSONObject();
					leftListApp.put(JSON_DOLLAR_ITEM, dolaritem);
					var leftListId = new JSONObject();
					leftListId.put(extModel.getJsonKey(), leftListApp);
					winO.put(JSON_APPPENDMODEL, leftListId);
				}
				winO.put(JSON_MNU, this.menuList == null ? new JSONArray() : this.menuList.toJson());
			}
			return winO;
