...
var FusionMetaData = require('syracuse-x3/lib/fusion/CAdapiEntity').FusionMetaData;
...
function copyOf(arr, len) {
	var a = arr.slice(0, len);
...
var CAdapiDataSrc = exports.CAdapiDataSrc = helpers.defineClass(function(aAdapiRequester, aAdapiScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	...
	CAdapiEntity.call(this, aAdapiRequester && aAdapiRequester.client, aDataSourceID, aEntityIndexBaseOne, 'BLOC');
	...
	blocFicheGetChilds: function() {
		...
		if (l == null || l.vars == null) {
			throw new Error("List boloc - Unexpected null first line");
		}
		return l.vars;
	},
	buildMessErrorTraverse: function(aStructIst, aAdapiSetter, aKeyOfMessSuffix) {
		var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE, aStructIst.toString(), this.getAdxId(), module.id);
		// TODO TCA
		// aStructIst.toString() doesn't return correct value
		// this.getAdxId() doesn't return correct value
		// module.id isn't corresponding to aAdapiSetter.class
		//wMess = locale.format(wMess, aStructIst.alphanumCodage, this.getAdxId(), CStringFacilities.getClasseName(aAdapiSetter.class));
		return wMess + this.client.resources.getMessage(aKeyOfMessSuffix);
	},
	...
	isBlocFiche: function() {
		return this.dataSrcType == CAdapiDataSrc.TYPE_SCREEN && this.dim == 1 && !this.hasVarBasPage();
	},
	isBlocGrid: function() {
		return this.isLeftList() || isBlocTableau();
	},
	isBlocTableau: function() {
		return this.dataSrcType == CAdapiDataSrc.TYPE_SCREEN && this.hasVarBasPage(); // this.dim>1;
	},
	isLeftList: function() {
	...
	serverInsertLines: function(zeroBasedIdx, nbLines) {
		for (i = 0; i < nbLines; i++) {
			...
			var srcLine = this.entrySup.slice(0);
	...
	setServerStyle: function(aStyle, applyOn) {
		if (StyleApply.CHILDS == applyOn && isBlocFiche()) {
			...
			CAdapiEntity.prototype.setServerStyle.call(this,aStyle, applyOn);
		}
	},
	size: function() {
		...
		if (this.isBlocGrid()) {
			...
			for (var i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					this.manageMetadata(mapSty, line, FusionMetaData.STY, lastLine);
					this.manageMetadata(mapStt, line, FusionMetaData.STT, lastLine);
					this.manageMetadata(mapFmt, line, FusionMetaData.FMT, lastLine);
					this.manageMetadata(mapTit, line, FusionMetaData.TIT, lastLine);
					// DCH - cell by cell
			...
			var sty = this.getJsonMetadata(mapSty, FusionMetaData.STY);
			if (!emptyJson(sty)) {
				metadata.sty = sty;
			}
			var fmt = this.getJsonMetadata(mapFmt, FusionMetaData.FMT);
			if (!emptyJson(fmt)) {
				metadata.fmt = fmt;
			}
			var stt = this.getJsonMetadata(mapStt, FusionMetaData.STT);
			if (!emptyJson(stt)) {
				metadata.stt = stt;
			}
			var tit = this.getJsonMetadata(mapTit, FusionMetaData.TIT);
			if (!emptyJson(tit)) {
				metadata.tit = tit;
			}
		...
		try {
			...
			// inferieure designee
				...
				if (wI < 0 || wI >= this.lines.length) {
					var wMess = this.client.resources.getMessage(CAdapiResources.KEYMESS_ERROR_LINE_UNKNOWN, wMess, wI, this.lines.length, this.getAdxId());
			...
			// application sur le bloc.
				...
				aAdapiSetter.setLogicalErrMess(this.buildMessErrorTraverse(aStructIst, aAdapiSetter, CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE_CASE));
				return true;
			}
		} catch (e) {
			var wMess = this.buildMessErrorTraverse(aStructIst, aAdapiSetter, CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE_EXCEPT);
