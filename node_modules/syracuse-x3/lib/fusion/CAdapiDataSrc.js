...
var CAdapiDataSrc = exports.CAdapiDataSrc = helpers.defineClass(function(aRequester, aScreen, aEntityIndexBaseOne, aDataSourceID, aDim, aNbField, aVarBasPage, dataSrcType) {
	dataSrcType = dataSrcType !== undefined ? dataSrcType : CAdapiDataSrc.TYPE_SCREEN;
	CAdapiEntity.call(this, aRequester && aRequester.client, aDataSourceID, aEntityIndexBaseOne, 'BLOC');
		...
		if (this.isBlocGrid()) {
			...
			for (var i = 0; i < this.lines.length; i++) {
				...
				if (line != null && !line.isDisposed()) {
					...
					this.manageMetadata(mapStt, line, 'STT', lastLine);
	...
	traverseSettable: function(aStructIst, aSetter) {
		try {
			...
			if (this.hasColumn() && aStructIst.isColumnIst() || aStructIst.isAllLinesOfColumnIst()) {
				...
				if (wContinue && aSetter instanceof CAdapiRequest.CServerStateSetter) {
					wContinue = traverseSettableLines(aStructIst, aSetter);
				}
				return wContinue;
			}
			...
			// application sur le bloc.
				...
				// Application sur le bloc
				var wContinue = aSetter.setAdapiModifiable(aStructIst, this);
				...
				aSetter.setLogicalErrMess(this.buildMessErrorTraverse(aStructIst, aSetter, CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE_CASE));
				return true;
			}
		} catch (e) {
			var wMess = this.buildMessErrorTraverse(aStructIst, aSetter, CAdapiResources.KEYMESS_ERROR_TRAVERSE_SETTABLE_EXCEPT);
