...
exports.proxy = function(_, context, boServerPath, config) {
	...
	if (cookie) {
		if (typeof cookie === 'string') {
			cookie = cookie.replace("Path=", "Path=/bo/x3/erp/" + context.endpoint.x3SolutionName(_) + "/$proxy");
		} else {
			for (var i = 0; i < cookie.length; i++) {
				cookie[i] = cookie[i].replace("Path=", "Path=/bo/x3/erp/" + context.endpoint.x3SolutionName(_) + "/$proxy");
			}
		}
	}
