...
exports.proxy = function(_, context, boServerPath, config) {
	function findCookies(cookies, key) {
		...
		for (var i = 0; i < cook.length; i++) {
			if (cook[i].indexOf(key + "=") === 0) {
				return cook[i].substring(key.length + 1);
			}
		}
		return;
	}
	...
	var baseUrl = context.endpoint.x3server(_).boServer(_).getBoServerBaseUrl(_, false);
...
exports.boRestCli = helpers.defineClass(function(_, profile, context) {

	//var profile = getBoProfile(_, context.getUser(_));

	if (!profile && context) profile = getBoProfile(_, context);

	this.bosec = profile.security(_);
	this.bouser = profile.user(_);
	this.bopwd = profile.password(_);
	var boServer = profile.boServer(_);
	this.boserv = boServer && boServer.host(_);
	this.boport = boServer && boServer.portRest(_);
...
//
var getBoProfile = function(_, context) {
	...
	if (!boserver) throw new Error("No Business Objects Server is assigned to X3 Server '" + x3server.description(_) + "'");

	var profiles = user.boProfiles(_).toArray(_);
	for (var i in profiles) {
		var inst = profiles[i];
		if (inst.boServer(_).$uuid === boserver.$uuid) {
			console.log("Server found: " + inst.boServer(_).name(_));
			profile = inst.profile(_);
		}
	}


	if (!profile) throw new Error("Can't find Business objects profile for user '" + user.login(_) + "' and X3 Server '" + x3server.description(_) + "'");
