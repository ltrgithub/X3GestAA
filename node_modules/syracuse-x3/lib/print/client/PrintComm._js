...
function buildProperties(props, node) {
	...
	return Buffer.concat([serializers['B'](0), serializers['S'](2), nodeId,  buf, serializers['B'](0), serializers['L'](0)]);
}
...
var printComm = exports.printComm = helpers.defineClass(function(_, stream) {
	...
	connect: function(_) {
		...
		this.pid = fromBytes(buf, "INT");
		traceProtocol && traceProtocol("Connected: "+this.pid);

		this.stream.write(_, buildProperties(this.technoVers, ND_SLF));
		var head = this.stream.read(_, 5);
		var headLen = head.readInt32BE(1);
		var redProps = this.stream.read(_, headLen);
		this.srvTechnoVers = getProperties(redProps);

	},
	send_ND_MORE_PRINTER: function(){
	},
	send_ND_GET_ALLPRINTERS: function(){
	},
	send_ND_EXTRA_PRINTER: function(){
	},
	send_ND_MORE_PRINTER: function(){
	},
