...
function buildProperties(props, node) {
	...
	return Buffer.concat([nodeId, buf, serializers['B'](0), serializers['L'](0)]);
}
...
var printComm = exports.printComm = helpers.defineClass(function(_, stream) {
	...
	send_ND_GETSTATEREPORT: function(_, jobId){
		var nodeId = new Buffer(GETSTATEREPORT);
		var buf = Buffer.concat([nodeId,  serializers['B'](jobId.length), getBytes(jobId, "UTF8")]);
