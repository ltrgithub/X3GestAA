...
var tracer = config.x3adxwhat && config.x3adxwhat.protocol && config.x3adxwhat.protocol.trace;
...
exports.AdxWhatComm = helpers.defineClass(function(_, client) {
	...
	_writeBufAsk: function(infoId) {
		...
		tracer && commonComm.traceBuffer(tracer, 0, "InfoID length (" + lenInfo + ")", buf);
		if (lenInfo > 0) {
			var bufStr = new Buffer(infoId, "utf-8");
			tracer && commonComm.traceBuffer(tracer, 1, "InfoID (" + infoId + ")", bufStr);
	...
	_readBufAsk: function(buf, id) {
		switch (id) {
			case 0:
				tracer && commonComm.traceBuffer(tracer, id, "Length of value", buf);
				return buf.readInt32BE(0);
			case 1:
				buf = buf.slice(0, buf.length - 1);
				tracer && commonComm.traceBuffer(tracer, id, "Value", buf);
