...
exports.AdxWhatComm = helpers.defineClass(function(_, client) {
	...
	_writeBufAsk: function(infoId) {
		...
		commonComm.traceBuffer(tracer, 0, "InfoID length (" + lenInfo + ")", buf);
		if (lenInfo > 0) {
			var bufStr = new Buffer(infoId, "utf-8");
			commonComm.traceBuffer(tracer, 1, "InfoID (" + infoId + ")", bufStr);
	...
	_readBufAsk: function(buf, id) {
		switch (id) {
			case 0:
				commonComm.traceBuffer(tracer, id, "Length of value", buf);
				return buf.readInt32BE(0);
			case 1:
				buf = buf.slice(0, buf.length - 1);
				commonComm.traceBuffer(tracer, id, "Value", buf);
