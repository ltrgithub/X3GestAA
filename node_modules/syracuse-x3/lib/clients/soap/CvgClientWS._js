...
var CvgClientAbs = require('../common/cvg/CvgClientAbs').CvgClientAbs;
var CvgRequestWS = require('./CvgRequestWS').CvgRequestWS;
var CvgWsException = require('./CvgWsException').CvgWsException;
var datetime = require('@sage/syracuse-core').types.datetime;
var CvgWsCriticalException = require('./CvgWsException').CvgWsCriticalException;
var forEachKey = helpers.object.forEachKey;
var OfflinePlayer = require('../common/offlinePlayer').OfflinePlayer;
var upath = require('path');
var recordMgr = require('../../convergence/records/recordMgr').recordMgr;
var ufs = require('fs');
var currentProtocol = require('../../convergence/automata/x3Descriptors').CURRENTPROTOCOL;
...
var cvgCacheManager = require('../../cvgCacheManager');
var poolManager = require("syracuse-soap/lib/generic/poolManager");

var tracer = require('@sage/syracuse-core').getTracer("soap-generic.ackcall");
...
function _wsTracer(wsCall) {
	var wrappedTracer = tracer;
	if (wsCall.wsTraces) {
		wrappedTracer = {};
		["info", "warn", "error", "debug"].forEach(function(level) {
			wrappedTracer[level] = function(message) {
...
exports.CvgClientWS = helpers.defineClass(function(protocolVersion) {
	...
	init: function(_, context, callContext, recOptions, options) {
		...
		if (recOptions && recOptions.path) {
			configRecords = {
				dumpPath: upath.join(recOptions.rootPath || process.mainModule.filename.substring(0, process.mainModule.filename.indexOf("node_modules")), recOptions.path)
			};
		}
		this.cliCfg = {};
		...
		try {
			...
			// try disconnection because even if connection failed somewhere, the socket could be opened
			try {
				this.disconnectClient(_, {}, "failed :" + e.message + "\n" + e.safeStack);
			} catch (err) {
				// DO NOTHING IF DISCONNECT FAILS
			}
			throw e;
		}
	...
	_connect: function(_, context, callContext, recOptions) {
		...
		this.sid = client.createSession(_, {
			...
			fusionTech: {
				protocol: this.protocolVersion ? this.protocolVersion : currentProtocol,
			},
	...
	execWs: function(_, context, wsCall, nbtry, ignoreCache) {
		...
		var wsTracer = this.wsTracer = _wsTracer(wsCall);
		//console.error("NB TRY: " + nbtry);
		try {
			...
			wsTracer.info && wsTracer.info("---------------------------------------");
			wsTracer.info && wsTracer.info("Call X3 subprogram '" + subprog + "'");

			// Add request traces
			wsTracer.debug && wsTracer.debug("### Begin Input parameters");
			wsTracer.debug && wsTracer.debug("*** Wrapper arguments: \r\n" + JSON.stringify(wsCall.callTechParams, null, 2));
			wsTracer.debug && wsTracer.debug("*** Sub program arguments: \r\n" + JSON.stringify(wsCall.callParams, null, 2));
			wsTracer.debug && wsTracer.debug("### End Input parameters");
			...
			var self = this;
			// Add request traces
			wsTracer.debug && wsTracer.debug("### Begin Output result");
			wsTracer.debug && wsTracer.debug("*** Technical parameters: \r\n" + JSON.stringify(wsCall.resTechParams, null, 2));
			if (wsCall.resTechParams[11]) {
				wsTracer.debug && wsTracer.debug("------------------------------------------------------------------------------------------");
				wsTracer.debug && wsTracer.debug("*** Adonix traces: \r\n");
				forEachKey(wsCall.resTechParams[11], function(key, value) {
					if (value.resu) {
						wsTracer.debug && wsTracer.debug(value.resu);
					}
				});
				wsTracer.debug && wsTracer.debug("------------------------------------------------------------------------------------------");
			}

			wsTracer.debug && wsTracer.debug("*** Result parameters: \r\n" + JSON.stringify(resultParams, null, 2));
			wsTracer.debug && wsTracer.debug("### End Output result");
			//
			...
			wsTracer.info && wsTracer.info("Result (" + ww_ok + ")");
			wsTracer.info && wsTracer.info("---------------------------------------");
			switch (ww_ok) {
				case 0:
					this.setFree(wsCall.context);
				...
				case 6: // database closed
					var err = new CvgWsException(locale.format(module, "sgbdClosed"));
					if (nbtry <= maxNbTry) {
						this.disconnectClient(_, {}, "failed :" + err.message + "\n" + err.safeStack);
						wsCall.addMessage(3, err.message);
						tracer.warn && tracer.warn(err.message + " : try to reconnect client and execute call again...");
				...
				case 9:
					...
					break;
				default:
					throw new CvgWsException(locale.format(module, "spgmNoResult", wsCall.name, ww_ok));
			}

		} catch (e) {
			if (e instanceof CvgWsCriticalException) {
				this.disconnectClient(_, {}, "failed :" + e.message);
				this.setUnavailable(_, context);
				return wsCall.getSoapResponse(0);
			} else if (e instanceof CvgWsException) {
				if (nbtry <= maxNbTry) {
	...
	_validReply: function(reply, wsCall, msgMessIdx, msgTypeIdx) {
		...
		if (!result) {
			...
			throw new CvgWsCriticalException();
	...
	_getDataXmlSchema: function(_, context, wsCall) {
		...
		res = cvgCacheManager.getCacheManager().getResource(_, url, null, {
			ignoreProxy: !this.useProxy
		}, this.recOptions);
		wsCall.techInfo.loadWebsDuration.stop();
		return res;
	...
	_recupHDAT: function(_, wsCall, stamp) {
		...
		var wsTracer = this.wsTracer || tracer;
		wsTracer.info && wsTracer.info("---------------------------------------");
		wsTracer.info && wsTracer.info("Call X3 subprogram '" + RECUPHDAT + "' with timestamp '" + stamp + "'");
		...
		wsTracer.info && wsTracer.info("Result (" + ok + ")");
		wsTracer.info && wsTracer.info("---------------------------------------");
		if (ok === 1) {
			return;
		} else if (ok === 0) {
	...
	_changeLogin: function(_, receiveUser, receiveLang, wsCall) {
		...
		var wsTracer = this.wsTracer || tracer;
		wsTracer.info && wsTracer.info("---------------------------------------");
		wsTracer.info && wsTracer.info("Call X3 subprogram '" + CHGLOGINXTEND + "'");
	...
	_flush: function(_, wsCall) {
		...
		var wsTracer = this.wsTracer || tracer;
		wsTracer.info && wsTracer.info("---------------------------------------");
		wsTracer.info && wsTracer.info("Call X3 subprogram '" + FLUSHADX + "'");
