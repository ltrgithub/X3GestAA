...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	execWS: function(_, context, params, techInfo, beautify, nbtry) {
		try {
			...
			var reply = CvgClientAbs.prototype.execAckCall.call(this, _, description.$.WRP + ":" + subprogName, wsCall.debugInfo, params, techInfo);
			...
			if (!(result && !result.errn && !result.errm)) {
				...
				switch (ww_ok) {
					case 0:
						throw new Error("Must handle bad status");
					case 1:
						...
						if (wsCall.action === "LIST") {
							paramsResult.filter(function(p) {
								return p.resu != null;
							});
						}
						wsCall.computeResult(description, paramsResult, this._x3SessionSettings.cdLang, beautify);
						return wsCall.result;
						// abnormal status
					...
					case 9:
						...
						this._recupHDAT(_, description.$.TIM, techInfo, wsCall.debugInfo);
						if (nbtry < maxNbTry)
							return this.execWs(_, context, params, techInfo, nbtry++);
						break;
					default:
	...
	_recupHDAT: function(_, wsName, stamp, debugInfo, techInfo) {
		techInfo && techInfo.poolExecDuration.start();
