...
exports.CvgClientWS = helpers.defineClass(function(_, context) {
	...
	this.callbackDisconnect = context.httpSession.genericWsCtx.callbackDisconnect;
	...
	disconnectClient: function(_, params, reason, tracker) {
		...
		this.callbackDisconnect(this.uuid);
	},
	execWS: function(_, context, params, techInfo) {
		try {
			...
			if (!(result && !result.errn && !result.errm)) {
				...
				var ww_ok = reply.params && reply.params.length && reply.params[0].resu;
				switch (ww_ok) {
					case -1:
						break;
						// ok status
