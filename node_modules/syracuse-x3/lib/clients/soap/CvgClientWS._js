...
exports.CvgClientWS = helpers.defineClass(function(_, endpoint, user, lang, context) {
	...
	execWS: function(_, ctx, wsCall) {

		if (!this._checkContext(_, ctx)) {
			this._changeLogin(_, ctx.user, ctx.lang);
		}

		var description = jsxml.parse(this._getDescription(_, wsCall.name));
		...
		params.splice(param.length, 0, wsCall.getAckCallParams(description));
		...
		if (result && !result.errn && !result.errm) {
			...
			throw new Error(result ? "X3 Error:" + result.errp ", message:" + result.errm + ", line :" + result.errl + ", type:" + result.errt);
		}


	},
	_getDescription: function(_, wsName) {
		var x3HttpPub = this.clientConfig.getParam("x3HttpPub");
		var x3Folder = this.clientConfig.getParam("x3Folder");
		return cacheMgr.getResource(_, x3HttpPub + "/" + x3Folder + "/GEN/WEBS/" + winId + ".xml");
	}
