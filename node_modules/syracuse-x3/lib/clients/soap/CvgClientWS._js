...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	execWs: function(_, context, params, techInfo, options, nbtry, ignoreCache) {
		...
		var wsCall = new CvgWSCall(techInfo, locale.x3Language(context.httpSession.genericWsCtx.localePref.code(_)), params, options.requestConfig);
			...
			switch (ww_ok) {
				...
				case 8: // flush adx wrapper in memory is not coherent
					if (this._flush(_, wsCall)) {
						if (nbtry < maxNbTry) return this.execWs(_, context, params, techInfo, requestConfig, nbtry++);
					}
					break;
				case 9:
					// get hdat and check consistency of description cache and stamp in x3
					if (nbtry < maxNbTry) {
						return this.execWs(_, context, params, techInfo, requestConfig, nbtry++, true);
					}
					break;
				default:
