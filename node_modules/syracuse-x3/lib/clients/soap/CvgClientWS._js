...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	execWs: function(_, context, wsCall, nbtry, ignoreCache) {
		...
		try {
			...
			//
			if (wsCall.wkeys === "XML3" || wsCall.wkeys === "XML4" || wsCall.wkeys === "XML5" || wsCall.wkeys === "XML16" || wsCall.wkeys === "XML17" || wsCall.wkeys === "XML18" || wsCall.wkeys === "XML19") {
				console.error("WKEY:" + JSON.stringify(wsCall.wkeys, null, 2));
	...
	_validReply: function(reply, wsCall, msgMessIdx, msgTypeIdx) {
		...
		if (msgMessIdx && wsCall.resTechParams && wsCall.resTechParams[msgMessIdx]) {
			for (var i = 0; i < Object.keys(wsCall.resTechParams[msgMessIdx]).length; i++) {
				if (wsCall.resTechParams[msgMessIdx] && wsCall.resTechParams[msgMessIdx][i]) {
