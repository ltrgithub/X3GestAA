...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	init: function(_, context, wsCall, recOptions) {
		...
		this.poolContext = wsCall.context;
		...
		if (recOptions && recOptions.recMode === 'PLAY') {
			...
			stream = this._connect(_, context, wsCall, recOptions).getStream();
	...
	_connect: function(_, context, wsCall, recOptions) { // TODO refactor to set that method in super class
		if (recOptions && recOptions.recmode === 'REC') {
		...
		var sysAuthToken;
		if (wsCall.context && wsCall.context.user && wsCall.context.user.infov6(_)) {
			sysAuthToken = new Buffer(wsCall.context.user.userv6(_) + ":" + wsCall.context.user.passwordv6(_), "utf8").toString("base64");
		}
		...
		this.sid = client.createSession(_, {
			...
			localePreferences: wsCall.context.localePref,
		...
		//this.isFree = true; // no necessary since we have queue
	...
	execWs: function(_, context, wsCall, nbtry, ignoreCache) {
		...
		//var wsCall = new WsCall(techInfo, locale.x3Language(context.httpSession.genericWsCtx.localePref.code(_)), params, options.requestConfig);
		...
		try {
			...
			var reply = CvgClientAbs.prototype.execAckCall.call(this, _, subprog, wsCall.debugInfo, ackCallParams);
			...
			this.isFree = true;
			cliManager.getManager(this.cliCfg.dataset).free(this);
		}
	},
	...
	_recupHDAT: function(_, wsCall, stamp, techInfo) {
		...
		var reply = this.execAckCall(_, RECUPHDAT, wsCall.debugInfo, params);
