...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	setUnavailable: function(_) {
		var pool = poolManager.getPool(this.cliCfg.poolAlias);
		if (pool) {
			pool.destroy(_, this.pid);
		}
	},
	// Override super
	...
	_setContextConfig: function(_, callContext) { // TODO refactor to set that method in super class
		...
		this.cliCfg.x3Host = endpoint.x3server(_).serverHost(_);
		this.cliCfg.x3Port = endpoint.x3server(_).serverPort(_);
		...
		this.cliCfg.x3Solution = endpoint.x3SolutionName(_);
		this.cliCfg.appHost = endpoint.x3server(_).serverName(_);
	...
	execWs: function(_, context, wsCall, nbtry, ignoreCache) {
		...
		try {
			...
			// Add request traces
			if (wsCall.wsTraces) {
				wsCall.addWsTrace("### Begin X3 call");
				wsCall.addWsTrace("*** Wrapper arguments: \r\n" + JSON.stringify(wsCall.callTechParams, null, 2));
				wsCall.addWsTrace("*** Sub program arguments: \r\n" + JSON.stringify(wsCall.callParams, null, 2));
				wsCall.addWsTrace("### End X3 call");
			}
			...
			// Add request traces
			if (wsCall.wsTraces) {
				wsCall.addWsTrace("### Begin X3 result");
				wsCall.addWsTrace("*** Technical parameters: \r\n" + JSON.stringify(wsCall.resTechParams, null, 2));
				if (wsCall.resTechParams[11]) {
					wsCall.addWsTrace("------------------------------------------------------------------------------------------");
					wsCall.addWsTrace("*** Adonix traces: \r\n");
					forEachKey(wsCall.resTechParams[11], function(key, value) {
						if (value.resu) {
							wsCall.addWsTrace(value.resu);
						}
					});
					wsCall.addWsTrace("------------------------------------------------------------------------------------------");
				}

				wsCall.addWsTrace("*** Result parameters: \r\n" + JSON.stringify(wsCall.resParams, null, 2));
				wsCall.addWsTrace("### End X3 result");
			}
	...
	_validReply: function(reply, wsCall, msgMessIdx, msgTypeIdx) {
		...
		// Manage messages
		if (msgMessIdx && wsCall.resTechParams[msgMessIdx]) {
			for (var i = 0; i < Object.keys(wsCall.resTechParams[msgMessIdx]).length; i++) {
				if (wsCall.resTechParams[msgMessIdx][i]) {
