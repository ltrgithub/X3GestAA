...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	_connect: function(_, context, callContext, recOptions) { // TODO refactor to set that method in super class
		if (recOptions && recOptions.recmode === 'REC') {
		...
		this.sid = client.createSession(_, {
			...
			collaborationBaseUrl: adminEP ? (context.httpSession || context.session).host + adminHelper.getCollaborationEndpoint(_).getBaseUrl(_) : "",
			peerAddress: ((context.httpSession || context.session).getData("requestInfo") || {}).peerAddress
		});
		poolManager.getPool(this.cliCfg.poolAlias).free(this, callContext.reserved);
		return client;
	},
