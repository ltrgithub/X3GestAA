...
exports.CvgClientWS = helpers.defineClass(function() {
	...
	init: function(_, context, wsCall, recOptions) {
		...
		this.cliCfg = this._getContextConfig(_, wsCall);
		var stream = null;
	...
	_getContextConfig: function(_, wsCall) { // TODO refactor to set that method in super class
		var endpoint = wsCall.context.endpoint;
		return {
			x3User: wsCall.context.user.getEndpointLogin(_, endpoint.$uuid),
			x3Host: endpoint.x3server(_).serverHost(_),
			x3Port: endpoint.x3server(_).serverPort(_),
			x3Lang: wsCall.context.localePref.code(_),
