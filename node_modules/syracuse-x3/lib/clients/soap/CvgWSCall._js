	...
	generateResult: function(description, params) {
					...
					flds.forEach(function(fld) {
						...
						if (resultParamMap[currentidx]) {
							...
							resultParamMap[currentidx].forEach(function(pp) {
								...
								paramsOutputGrp[grp.$.NAM][pp.poste][currentidx].xml.start += (fld.$.MENU ? "MENULOCAL='" + fld.$.MENU + "' MENULAB='TODO' >" : ">") + pp.resu;
							});
						}
					});
				});
		...
		if (!this.result) {
			...
			params.forEach(function(p) {
				resParamsMap[p.num - 11] = resParamsMap[p.num - 11] || [];
				resParamsMap[p.num - 11].push(p);
			});
