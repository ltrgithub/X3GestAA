...
exports.CvgWSCall = helpers.defineClass(function(opt) {
	...
	getResult: function(description, params, lang, beautify) {
		var self = this;
		/*
		...
		function getParamFieldOutputXml(resultParamMap) {
			...
			function _createField(elt, field, val) {

				function _createItem(_elt, itmVal) {
					_elt.ITM = _elt.ITM || [];
					var _itm = {
						$: {
							MENULAB: findMenuLabel(field.$.MEN, itmVal)
						},
						$value: itmVal
					};
					_elt.ITM.push(_itm);
				...
				if (field.$.DIM > 1 && field.$.MEN) {
					...
					for (var i = 0; i < field.$.DIM; i++) {
						var _p = resultParamMap[currentIdx][i];
						if (_p) {
							_createItem(_lst, _p.resu);
						}
					}
					elt.LST.push(_lst);
					...
					if (field.$.TYP === "Blob") {
						_fld.$.BYTES = val.length;
						_fld.$.MIMETYPE = "TODO";
						_fld.$value = new Buffer(_fld.$value).toString('base64');
					}
					if (field.$.MEN) {
						_fld.$.MENULOCAL = field.$.MEN;
						_fld.$.MENULAB = findMenuLabel(field.$.MEN, val);
					}
					elt.FLD.push(_fld);
			...
			function _createGroup(elt, grp) {
				...
				flds.forEach(function(_fld) {
					var _p = resultParamMap[currentIdx] && resultParamMap[currentIdx][0];
					if (_p) {
						_createField(_grp, _fld, _p.resu);
					}
				});
			...
			function _createLine(elt, num, flds) {
				...
				flds.forEach(function(_fld) {
					var _p = resultParamMap[currentIdx][num];
					_createField(_lin, _fld, (_p && _p.resu) || "");
			...
			function _createTable(elt, grp) {
				elt.TAB = elt.TAB || [];
				// Get size param
				var size = resultParamMap[currentIdx][0].resu || 0;
				currentIdx++;
			...
			var descripData = description.ADXDATA;
			if (self.action === "LIST") {
				descripData = description.ADXKEY;
			}
			...
			grps.forEach(function(grp) {
				...
				if (grp.$.NAM === "LEFTLIST") {
					...
					output.RESULT.$.SIZE = resultParamMap.reduce(function(prev, cur) {
						var len = Object.keys(cur).length;
						return len > prev ? len : prev;
					}, 0);
					var flds = Array.isArray(grp.FLD) ? grp.FLD : [grp.FLD];
					for (var i = 0; i < output.RESULT.$.SIZE - 1; i++) {
