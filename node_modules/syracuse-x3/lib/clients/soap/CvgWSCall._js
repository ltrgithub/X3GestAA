...
exports.CvgWSCall = helpers.defineClass(function(opt) {
	...
	generateResult: function(description, params, lang, isAlign) {
		var self = this;
		/*
		...
		function getParamFieldOutputXml(resultParamMap, isAlign) {
			...
			if (descripData) {
				var currentidx = 12;
				var paramsOutput = null;
				var paramsOutputGrp = {};

				var grps = Array.isArray(descripData.GRP) ? descripData.GRP : [descripData.GRP];
				grps.forEach(function(grp) {
					if (grp.$.DIM > 1 && grp.$.NAM !== "LEFTLIST") { // parameter describe number of element
						currentidx++;
					}
					if (!paramsOutput) {
						if (grp.$.NAM === "LEFTLIST") {
							paramsOutput = '\n<RESULT DIM="' + grp.$.DIM + '" SIZE="' + self.nb + '">';
						} else {
							paramsOutput = "\n<RESULT>";
						}
					}
					...
					flds.forEach(function(fld) {
						if (resultParamMap[currentidx]) {
							...
							if (!paramsOutputGrp[grp.$.NAM]) {
								if (grp.$.NAM !== "LEFTLIST") { // don't create the grp or tab xml
									var att = grpName === "TAB" ? ' DIM="' + grp.$.DIM + '" SIZE="' + resultParamMap[currentidx].length + '"' : '';
							...
							resultParamMap[currentidx].forEach(function(pp) {
								...
								if (fld.$.MEN && pp.resu || !fld.$.MEN) {
									// specific for leftlist

									if (!paramsOutputGrp[grp.$.NAM][pp.poste]) {
										if (grp.$.TYB === "Table" || grp.$.NAM === "LEFTLIST") {
									...
									if (fld.$.DIM > 1 && fld.$.MEN) {
										...
										paramsOutputGrp[grp.$.NAM][pp.poste][currentidx] = {
											xml: {
												start: '<FLD ' + (extra ? extra + ' ' : '') + (menuLab ? menuLab + ' ' : '') + (menuLocal ? menuLocal + ' ' : '') + 'NAME="' + fld.$.NAM + '" TYPE="' + fld.$.TYP + '" >' + val,
												end: '</FLD>'
											}
										};
									}
				...
				Object.keys(paramsOutputGrp).forEach(function(grpId) {
					// group level
					paramsOutput += (paramsOutputGrp[grpId].xml ? (isAlign ? "\n\t" : "") + paramsOutputGrp[grpId].xml.start : "");
		...
		if (!this.result) {
			...
			console.log(JSON.stringify(resParamsMap, null, 2));
			this.result = getParamFieldOutputXml(resParamsMap, isAlign);
		}
		return this.result;
	},
