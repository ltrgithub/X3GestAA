...
exports.CvgWSCall = helpers.defineClass(function(opt) {
	...
	// for objecg or list
	console.log("optkeys " + JSON.stringify(opt.wkeys));
	...
	generateResult: function(description, params) {
		...
		function getParamFieldOutputXml(resultParamMap) {
			...
			function getIdGroupXml(grp, params) {
				if (grp.$.TYB === "List")
					return "GRP";
				return "TAB DIM=" + grp.$.DIM + " SIZE=" + params.length;
			}

			if (description.ADXDATA) {
				var currentidx = 1;
				var paramsOutput = "<RESULT>";
				var paramsOutputGrp = {};
				var paramsOutputField = {};
				var paramsOutputLine = {};
				...
				grps.forEach(function(grp) {
					if (resultParamMap[currentidx]) {
						...
						paramsOutputGrp[grp.$.ID] = {
							xml: {
								start: "<" + grpName + " ID=" + grp.$.ID + ">",
								end: "</" + grpName + ">"
							}
						};
						...
						flds.forEach(function(fld) {
							currentidx = parseInt(fld.$.IDX);
							paramsOutputField[currentidx] = {};

							// create LIN
							resultParamMap[currentidx].forEach(function(pp) {
								if (grpName === "TAB") {
									paramsOutputGrp[grp.$.ID][pp.poste] = {
										xml: {
											start: "<LIN NUM" + pp.poste + ">",
											end: "</LIN>"
										}
									};
								}

								paramsOutputGrp[grp.$.ID][pp.poste][currentidx] = {
									xml: {
										start: "\t<FLD NAME=" + fld.$.NAM + " TYPE= " + fld.$.TYP + " ",
										end: "</FLD>"
									}
								};
								// check if it's local menu,
								paramsOutputGrp[grp.$.ID][pp.poste][currentidx].xml.start += (fld.$.MENU ? "MENULOCAL=" + fld.$.MENU + " MENULAB='TODO' >" : ">") + pp.resu;
							});
				...
				Object.keys(paramsOutputGrp).forEach(function(grpId) {
					// group level
					paramsOutput += "\t" + paramsOutputGrp[grpId].xml.start;
					Object.keys(paramsOutputGrp[grpId]).forEach(function(poste) {
						// poste level if xml exists
						if (paramsOutputGrp[grpId][poste].xml) {
							paramsOutput += "\t\t" + paramsOutputGrp[grpId][poste].xml.start;
						}
						Object.keys(paramsOutputGrp[grpId][poste]).forEach(function(field) {
							// field level
							paramsOutput += "\t\t\t" + paramsOutputGrp[grpId][poste][field].xml.start + paramsOutputGrp[grpId][poste][field].xml.end;
						});

						if (paramsOutputGrp[grpId][poste].xml) {
							paramsOutput += "\t\t" + paramsOutputGrp[grpId][poste].xml.end;
						}
		...
		if (!this.result) {
			...
			params.forEach(function(p) {
				resParamMap[p.num] = resParamMap[p.num] || [];
				resParamMap[p.num].push(p);
			});

			this.result = getParamFieldOutputXml(resParamMap);
		}
		return this.result;
	},
			...
			if (description.ADXDATA) {
				...
				grps.forEach(function(grp) {
					...
					flds.forEach(function(fld) {
						...
						currentidx = parseInt(fld.$.IDX);
						paramsInput[parseInt(fld.$.IDX, 10)] = {
							"index": parseInt(fld.$.IDX, 10),
			...
			grps.forEach(function(grp) { // check if the group exists in the
				...
				grp.FLD && grp.FLD.forEach(function(fld) { // TODO manage the order of field regarding description
					...
					if (descrFld) {
						...
						paramsMap[parseInt(descrFld.$.IDX, 10)] = {
							"index": parseInt(descrFld.$.IDX, 10),
							"descr": fld.$.NAM || fld.$.NAME,
							"dim": descrGrp.$.DIM,
