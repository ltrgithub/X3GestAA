...
var clientModule = require('../../client').module;
var tracerProtocol = require("syracuse-trace/lib/helper").getTracer("classic.protocol");

var RawBuffer = exports.RawBuffer = {
	ZERO_BYTE: new Buffer([0x00]),
	NODE_ID_LEN: new Buffer([0x00, 0x02])
};

var CommError = exports.CommError = helpers.defineClass(function(message) {
	Error.call(this);
	Error.captureStackTrace(this, this);
	this.message = x3helpers.sanitizeDiagnosis(message).$message;
}, Error, {});
...
exports.CommonComm = helpers.defineClass(function(_, stream, tracer) {
	...
	_connect: function(_, serviceId, user, password, secured) {
		...
		if (!buf) throw new CommError(locale.format(module, "connectionFailed", -1, locale.format(module, "checkConfMsg")));
	...
	read: function(_, len) {
		try {
			...
			// destroy stream
			if (this.stream && this.stream.destroy) this.stream.destroy();
			this.stream = null;
			//
			throw e;
		}
	...
	_authenticate: function(_, pid) {
		...
		function _outBuffer(code, obj) {
			...
			self.tracer.debug && self.tracer.debug("X3.client.outBuffer: sending length: " + len);
		...
		function _readFrame(_, encoding) {
			self.tracer.debug && self.tracer.debug("reading frame from X3");
			var intro = self.read(_, 5, true);
			if (intro == null) throw new Error("error reading frame: stream closed");
			var len = _readInt(intro);
			self.tracer.debug && self.tracer.debug("reading frame from X3: got len=" + (len - 1));
			var data = self.read(_, len - 1, true);
			self.tracer.debug && self.tracer.debug("frame read ok");
			return {
				code: intro[4],
		...
		function _send(_, header, request, response) {
			self.tracer.debug && self.tracer.debug("sending header: " + JSON.stringify(header));
			...
			while ((buf = request.read(_))) {
				self.tracer.debug && self.tracer.debug("sending chunk: " + (buf ? buf : "null"));
				self.stream.write(_, _outBuffer(2, buf));
			}
			self.tracer.debug && self.tracer.debug("write end frame");
			self.stream.write(_, _outBuffer(3, ""));
			//
			var frame = _readFrame(_, "binary");
			self.tracer.debug && self.tracer.debug("received header frame: " + frame.data);
			_checkCode(frame.code, 1);
			var head = JSON.parse(frame.data);
			head.statusCode = head.status; // compat stuff -- fix protocol
			self.tracer.debug && self.tracer.debug("head.location: " + head.location);

			response.writeHead(head.statusCode, head);
			do {
				frame = _readFrame(_);
				self.tracer.debug && self.tracer.debug("received body frame " + frame.code + ": " + frame.data.toString("utf8"));
				//			this.tracer.info && this.tracer.info("received body frame " + frame.code);
				if (head.statusCode === 500) {
					self.tracer.debug && self.tracer.debug("ERROR 500: head=" + JSON.stringify(head) + "\nbody=" + frame.data);
			...
			self.tracer.debug && self.tracer.debug("ending response");
			response.end();
		...
		function _jsonSend(_, params, funnel) {
			...
			return funnel.call(null, _, function(_) {
				...
				self.tracer.debug && self.tracer.debug("_jsonSend : " + JSON.stringify(params.body));
				...
				self.tracer.debug && self.tracer.debug("_jsonSend got response: " + respStr);
				if (params.raw) {
					// no transformation
				} else if (respStr) try {
		...
		try {
			...
			if (_x3.auth && (_x3.auth.method === "X3RSA")) {
				...
				self.tracer.debug && self.tracer.debug("pubkeyname " + pubkeyName);
...
exports.props1Serializer = {
	...
	rawWrite: function(_, client, propsStr, node) {
		var buf;
		...
		if (node) {
			buf = Buffer.concat([RawBuffer.ZERO_BYTE, RawBuffer.NODE_ID_LEN, new Buffer(node), buf]);
		}
		var bufEnd = new Buffer(4).fill(0);
	...
	write: function(_, client, props) {
		return this.rawWrite(_, client, this.build(props, false));
	}
};