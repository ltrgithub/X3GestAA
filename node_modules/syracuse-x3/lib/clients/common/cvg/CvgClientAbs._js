...
exports.CvgClientAbs = helpers.defineClass(function() {
	...
	this.automata = new X3Automata(_);
	this.x3writer = new X3Writer(_);
}, CvgObserver, {
	...
	listenNodeND_SLF: function(_, reader, node, properties) {
		...
		if (result && result.reply) { // auto reply
			this.x3writer.props1Serializer.write(_, this, result.pValues);
	...
	listenNodeND_GFD: function(_, reader, node, properties) {
		...
		return this.x3writer.props1Serializer.build(result, true);
	},
	...
	processAsyncAction: function(_, body, connect, mergeMode, noResp) {
		...
		try {
			...
			body.act && this.x3writer.actions.Descriptors[body.act] && this.x3writer.actions.Descriptors[body.act].listenHandler && this.x3writer.actions.Descriptors[body.act].listenHandler(_, body, self, params);
			...
			this.x3writer.sendAction(_, self, resu, extras, params);
