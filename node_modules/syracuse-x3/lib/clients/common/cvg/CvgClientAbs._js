...
var CvgObserver = require('../../../convergence/client/CvgObserver').CvgObserver;
var x3helpers = require('../../../helpers');
var flows = require('streamline-runtime').flows;
var datetime = require('@sage/syracuse-core').types.datetime;
var locale = require('streamline-locale');
var props1Serializer = require('../commonComm').props1Serializer;
var CvgClientConfig = require('../../../convergence/model/CvgClientConfig').CvgClientConfig;
var CONNECT_ACTION = require('../../../convergence/client/CvgAction').CONNECT_ACTION;
var CvgRequestAbs = require('./CvgRequestAbs').CvgRequestAbs;
var CvgException = require('../../../convergence/model/CvgException').CvgException;
var CvgDiagnose = require('../../../convergence/model/CvgDiagnose');
var adminHelper = require("@sage/syracuse-lib/src/collaboration/helpers").AdminHelper;
var CvgClose = require('../../../convergence/model/CvgClose');
var CvgIst = require('../../../convergence/types/CvgIst').CvgIst;
var localConfig = x3helpers.config("x3fusion", null, {});
var X3Writer = require('../../../convergence/automata/x3writer').X3Writer;
var CvgResu = require('../../../convergence/types/CvgResu').CvgResu;
var CvgResuTdo = require('../../../convergence/types/CvgResu').TDO;
...
var CvgReply = require('../../../convergence/client/CvgReply').CvgReply;
var perfmon = require('@sage/syracuse-lib/src/perfmon/record');
var X3Automata = require('../../../convergence/automata/x3Automata').X3Automata;
var X3Reader = require('../../../convergence/automata/x3Reader').X3Reader;
...
exports.CvgClientAbs = helpers.defineClass(function(protocolVersion) { // allow to force the protocol to use else we take the last implemented one
	...
	this.setProtocolVersion(protocolVersion ? protocolVersion : currentprotocol);
	...
	setProtocolVersion: function(protocolVersion) {
		if (!this.automata)
			...
			this.automata.setProtocolVersion(protocolVersion);
	...
	disconnectClient: function(_, params, reason, tracker, nbTry) {
		var pendingNode;
		nbTry = nbTry || 0;
			...
			if (this.isAlive()) {
				if (this.clientConfig.modePLAY) {
					...
					if (nbTry < loopDetection) {
						...
						diag.statusCode = 500;
						if (self.execProcessAction) {
							console.log("FORCE DISCONNEECT");
							this.closeSocket();
							return;
						}
						this.funnelAction(_, function(_) {
							...
							if (isLoopState(node)) {
								...
								self.closeSocket(); // we force the close of the socket.
							}
						});

						self.logSession(_, "FINE", "process close");
						self.processAction(_, {
							act: 1075,
							fld: {},
							param: {},
							tech: {}
						});
						self.disconnectClient(_, params, reason, tracker, nbTry++);
					} else { // loop detected, we kill the process by closing the scoker
						self.logSession(_, "INFO", "detect loop on ending node ");
						this.closeSocket(); // we force the close of the socket.
					}
				}
			}
		} catch (e) {
	...
	logSession: function(_, level, message) {
		if (localConfig.logSession && localConfig.logSession.level === level || level === "INFO") {
			...
			console.error((new Date()).toString() + " : " + "[sid:" + (this.context.httpSession && this.context.httpSession.id) + "] " + level + " : " + message);
		}
	},
	...
	closeClient: function(_, e, reason) {
		tracerStd.info && tracerStd.info("CloseClient Convergence Client\n\tReason[" + reason + "]\n\tAlreadyClosed[" + !(this.connClient && this.connClient.hasSocket()) + "] - Exception[" + e && e.message + "]");
		this.stopRecord();
		var mesg = "None";
		if (e) {
			mesg = e.message || e;
	...
	executeCallUiSetTechnovers: function(values) {
		if (parseInt(values.Protocol, 10) <= parseInt(this.protocolVersion, 10)) this.setProtocolVersion(values.Protocol);
	...
	processAsyncAction: function(_, body, connect, mergeMode, noResp, noRecurse) {
		...
		try {
			...
			if (params) {
				pendingNode = self.activeRequest && self.activeRequest.workingCtx && self.activeRequest.workingCtx.pendingBlockingNode;
			...
			if (noRecurse) {
				...
				self.automata.process(_, self, self.activeRequest, {
					title: "action",
					detail: p[0]
				});
			}
	...
	processAction: function(_, body, connect, mergeMode, noResp) {
		...
		function exec(_) {
			self.execProcessAction = true;
			self.processAsyncAction(_, body, connect, mergeMode, noResp);
			self.execProcessAction = false;
