...
var OfflinePlayer = require('./offlinePlayer').OfflinePlayer;
...
exports.CommonClient = helpers.defineClass(function(_, options, hostname, port, user, password, secured) {
	...
	this.CommClass = options.CommClass;
	if (this.recOptions && this.recOptions.path) {
		this.recOptions = options.recOptions ? options.recOptions : null;
		this.configRecords = {
			dumpPath: upath.join(this.recOptions.rootPath || process.mainModule.filename.substring(0, process.mainModule.filename.indexOf("node_modules")), this.recOptions.path)
		};
	}
		...
		try {
			if (!this.stream || !this.communicator) {
				...
				if (this.recOptions && this.recOptions.recMode === "PLAY") {
					this.stream = new OfflinePlayer(_, readJsonPlayer(_, this.configRecords.dumpPath + "/cvgRecords/" + this.recOptions.fileName));
				} else {
					this.stream = ez.devices.net.tcpClient(this.port, this.hostname).connect(_);
					this.stream.setNoDelay(true);
					if (this.recOptions && this.recOptions.recMode === "REC") {
						this.recorder = recordMgr.newRecorder(_, this.stream, this.recOptions, {}, this.configRecords);

					}
				}

				this.communicator = new this.CommClass(_, this);
