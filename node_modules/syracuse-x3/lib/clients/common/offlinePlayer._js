...
exports.OfflinePlayer = helpers.defineClass(function(_, json) {
	...
	read: function(_, size) {
		if (this.step <= this.json.length) {
			if (this.curStep.type !== "READ") throw new Error("expected READ, got " + this.curStep.type);
			if (size == null) size = this.curStep.buf && this.curStep.buf.length;
			var buf;
	...
	write: function(_, buf) {
		if (this.step <= this.json.length) {
			if (this.curStep.type === "WRITE") {
				...
				while (this.curStep.buf && this.curStep.writeBuf && this.curStep.writeBuf.length > 0) {
					var len = Math.min(this.curStep.writeBuf.length, this.curStep.buf.length);
					if (this.curStep.buf && this.curStep.writeBuf.slice(0, len).toString('hex') !== this.curStep.buf.slice(0, len).toString('hex')) {
						...
						throw new Error("bytes written not correspond to buffer saved written buffer[" + this.curStep.writeBuf.slice(0, len).toString('hex') + "] saved buffer[" + this.curStep.buf.slice(0, len).toString('hex') + "]");
					}
					if (this.curStep.buf && this.curStep.writeBuf.length === this.curStep.buf.length) {
						this._nextStep();
						break;
					} else {
						this.curStep.writeBuf = this.curStep.writeBuf.slice(len);
					}
				}
				return;
			}
