...
var props1Serializer = require('../common/commonComm').props1Serializer;
var RawBuffer = require('../common/commonComm').RawBuffer;
var tracer = require('@sage/syracuse-core').getTracer("x3Comm.print");
...
var ND_FILEPUT = [0x12, 0x6E];
var ND_FILEDELETE = [0x12, 0x6F];

var ND_ALLPRINT = [0x42, 0x01];
var ND_MOREPRINT = [0x42, 0x03];
var ND_EXTRAPRINT = [0x42, 0x04];

var ZERO_BYTE = RawBuffer.ZERO_BYTE;
var NODE_ID_LEN = RawBuffer.NODE_ID_LEN;

var trace;
var trcr = tracer; // just to avoid commit hook to fail

if (config && config.x3print && config.x3print.dump || trcr.info) {
	trace = function(_, spy, str) {
		tracer.info && tracer.info(str);
...
exports.PrintComm = helpers.defineClass(function(_, client) {
	...
	/// ---
	connect: function(_) {
		trace && this.dumpHeader(_, "CONNECT");
	...
	send_ND_NDOUVIMP: function(_, properties) {
		trace && this.dumpHeader(_, "ND_NDOUVIMP", properties);
		// Build request
	...
	send_ND_DELETEREPORT: function(_, seqId) {
		...
		trace && this.dumpHeader(_, "ND_DELETEREPORT", {
			seqId: seqId
		});
		// Build request
	...
	send_ND_GETSTATEREPORT: function(_, jobId) {
		trace && this.dumpHeader(_, "ND_GETSTATEREPORT", {
			jobId: jobId
		});
		// Build request
	...
	/// ---
	/// 
	send_ND_GETRESULTREPORT: function(_, jobId, stream) {
		trace && this.dumpHeader(_, "ND_GETRESULTREPORT", {
			jobId: jobId
		});
		// Build request
		var bufJob = new Buffer(jobId.toString(), "utf8");
		...
		var bin;
		if (!stream) bin = new Buffer(lenBin);
		var pos = 0;
		while (true) {
			var lenBlock = this.stream.read(_, 4).readInt32BE(0);
			if (lenBlock < 0) throw new Error("bad block len: " + lenBlock);
			if (lenBlock === 0) break;
			if (stream)
				stream.write(_, this.stream.read(_, lenBlock), "binary")
			else
				this.stream.read(_, lenBlock).copy(bin, pos);
			pos += lenBlock;
		}
		var res = {
			length: lenBin,
	...
	send_ND_LIM: function(_) {
		...
		trace && this.dumpHeader(_, "ND_LISTPRINTER", p);
		props1Serializer.rawWrite(_, this.stream, p, ND_LIM);
		var props = readProperties1(_, this.stream);
		var n = parseInt(props._NbPrinters.charCodeAt(0), 10);
		props._NbPrinters = n;
		trace && this.dumpResult(_, props);
		return props;
	},
	...
	send_ND_GET_ALLPRINTERS: function(_) {
		...
		trace && this.dumpHeader(_, "ND_ALLPRINT");
		...
		trace && this.dumpResult(_, result);
		return result;
	},
	...
	send_ND_MORE_PRINTER: function(_, prName, prPort, prDriver) {
		trace && this.dumpHeader(_, "ND_MORE_PRINTER", {
			printerName: prName,
			printerPort: prPort,
			driverName: prDriver
		});
	...
	send_ND_EXTRA_PRINTER: function(_, prName, prPort, prDriver, prPaperSizeId) {
		trace && this.dumpHeader(_, "ND_EXTRA_PRINTER", {
			printerName: prName,
			printerPort: prPort,
			driverName: prDriver,
	...
	/// ---
	///
	send_ND_FILEDELETE: function(_, name, location, meta) {
		trace && this.dumpHeader(_, "ND_FILEDELETE", {
			fileName: name,
			fileLocation: location
		});
		// Build request
		var buf = Buffer.concat([ZERO_BYTE, NODE_ID_LEN, new Buffer(ND_FILEDELETE)]);
		this.stream.write(_, buf);
		this._serializeFileDesc(_, name, location, meta);
		// Get response
		return this._getFileOpeRes(_);
	},
	/// ##**Node FILEPUT**
	...
	/// ---
	///
	send_ND_FILEPUT: function(_, name, location, meta, data, size) {
		trace && this.dumpHeader(_, "ND_FILEPUT", {
			fileName: name,
			fileLocation: location
		});
		// Build request
		var bufSize = new Buffer(4),
			buf = Buffer.concat([ZERO_BYTE, NODE_ID_LEN, new Buffer(ND_FILEPUT)]);
		bufSize.writeInt32BE(size, 0);
		this.stream.write(_, buf);
		this._serializeFileDesc(_, name, location, meta);
		this.stream.write(_, bufSize);
	...
	send_ND_END_PRINTER: function(_) {
		trace && this.dumpHeader(_, "ND_END_PRINTER");
	...
	_serializeFileDesc: function(_, name, location, meta) {
		// Build request
		var bufFlN = new Buffer(name, "utf8");
		var bufFlL = new Buffer(location, "utf8");
		var bufMeta = new Buffer(4);
		bufMeta.writeInt32BE(0, 0); // 0 because not yet used
		var buf = Buffer.concat([new Buffer([bufFlN.length]), bufFlN, new Buffer([bufFlL.length]), bufFlL, bufMeta]);
		this.stream.write(_, buf);

	},
	_getFileOpeRes: function(_) {
		var res = {},
			nExtSize;
		res.code = this.stream.read(_, 4).readInt32BE(0);
		nExtSize = this.stream.read(_, 4).readInt32BE(0);
		if (nExtSize > 0) {
			res.ext = this.stream.read(_, nExtSize);
		}
		trace && this.dumpResult(_, res);
		return res;
	}
