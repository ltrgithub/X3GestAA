...
var _reportMap = {
	get: function(_, context) {
		...
		var dd = session.withElevatedPrivilege(_, function(_) {
			return sa.open(_, filter, null, "printDocument");
		});
		if (!dd) {
			headers = {
			...
			var inst = session.withElevatedPrivilege(_, function(_) {
				return sa.getDocumentInstance(_, filter, null, "printDocument");
			});
			...
			var buf;
			session.withElevatedPrivilege(_, function(_) {
	...
	"delete": function(_, context) {
		...
		var result = globals.context.session.withElevatedPrivilege(_, function(_) {
			return sa.remove(_, filter, "printDocument");
		});
		if (result === 1) {
			context.reply(_, 204);
		} else {
...
var _serviceMap = {
	...
	getReport: function(_, context, id) {
		...
		var cli = getClient(_, context, id);
		var report;
		if (enableStream) report = cli.getReportInfo(_, id)
		else report = cli.getReport(_, id, null)
		...
		var document = globals.context.session.withElevatedPrivilege(_, function(_) {
			if (enableStream) {
				return sa.writeStream(_, properties, "", null, "printDocument", function(_, stream) {
					cli.getReport(_, id, stream);
				})
			} else {
				return sa.writeAll(_, properties, Buffer.isBuffer(report.binary) ? report.binary : new Buffer(report.binary, 'binary'), "", null, "printDocument");
			}
		});
		report.binary = undefined;
