...
var _serviceMap = {
	...
	getReport: function(_, context, id) {
		...
		var cli = getClient(_, context, id);
		var report;
		report = cli.getReportInfo(_, id)
			//report = cli.getReport(_, id, null)
		...
		var document = globals.context.session.withElevatedPrivilege(_, function(_) {
			var result = sa.getWritableStream(_, properties, "", null, "printDocument");
			cli.getReport(_, id, result.stream);
