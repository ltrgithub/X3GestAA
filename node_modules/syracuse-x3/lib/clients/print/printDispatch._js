...
var _serviceMap = {
	filePut: function(_, context, id) {
		// TODO call putFile of print client
		var cli = getClient(_, context, id);
		var name = context.request.headers["x-file-name"]; // header x-file-name
		var location = context.request.headers["x-file-location"]; // header x-file-location
		var meta = null;
		var buffer = null;
		//read buffer
		while (buffer = context.request.read(_)) {}
		cli.filePut(_, name, location, meta, buffer, buffer.length);
	},
	fileDelete: function(_, context, id) {
		// TODO call deleteFile of print client
		getClient(_, context, id);
