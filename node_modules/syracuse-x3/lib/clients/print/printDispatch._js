...
var _serviceMap = {
	filePut: function(_, context, id) {
		...
		//read buffer
		if (buffer != null) {
			_responseFileOperaton(_, context, cli.filePut(_, name, location, meta, buffer, buffer && buffer.length));
		} else {
	...
	fileDelete: function(_, context, id) {
		...
		_responseFileOperaton(_, context, cli.fileDelete(_, name, location, meta));
