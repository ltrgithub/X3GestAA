...
var PrintComm = require('./PrintComm').PrintComm;
var config = require('config');
var mime = require('streamline-static').mime;
...
var PrintClient = exports.PrintClient = helpers.defineClass(function(_, hostname, port, id, recOptions) {
	...
	tracer.info && tracer.info("Connect on srvimp --> " + hostname + ":" + port);
	...
	/// Returns nothing.
	///
	cancelAllReport: function(_) {
		var self = this;
		stack[this.printServer].map_(_, function(_, seqId) {
			self.communicator.send_ND_DELETEREPORT(_, seqId);
		});
		delete stack[this.printServer];
	},
	...
	/// ```
	///
	fileDelete: function(_, name, location, meta) {
		if (this._isGoodLocationPath(location)) {
			throw new Error("bad location");
		}
		if (this._isGoodLocationPath(name)) {
			throw new Error("bad fileName");
		}
		return this.communicator.send_ND_FILEDELETE(_, name, location, meta);
	},
	...
	/// ```
	///
	filePut: function(_, name, location, meta, buffer, size) {
		// check location for security reasons
		if (this._isGoodLocationPath(location)) {
			throw new Error("bad location");
		}
		if (this._isGoodLocationPath(name)) {
		...
		return this.communicator.send_ND_FILEPUT(_, name, location, meta, buffer, size);
	},

	_isGoodLocationPath: function(location) {
		var locUnix = location.replace(/\\/g, "/");
		return locUnix.indexOf("../") !== -1;
	},
	/// -------------
	/// ## getReport :
	/// ``` javascript
	...
	/// ```
	/// 
	getReport: function(_, jobId, stream) {
		try {
			...
			if (job.report && !job.report.binary) {
				var result = this.communicator.send_ND_GETRESULTREPORT(_, jobId, stream);
	...
	getReportInfo: function(_, jobId) {
		var jobs = getStackedJobs(this.printServer);
		var job = jobs[jobId];
		if (!job) throw new Error("Job [" + jobId + "] was not found on stack");
		return job.report;
	...
	__panicRelease: function(_) {
		...
		try {
			...
			console.error("panicRelease ERROR: " + e.stack);
			try {
				if (!dis && this.stream) {
					this.stream.end();
				}
			} catch (e) {
				console.error("CLOSING STREAM ERROR: " + e.stack);
			}
		} finally {
			this.stream = null;
	...
	__setSocketTimeout: function(_, prevent) {
		...
		this.socketTimeout = flows.setTimeout(function(_) {
			try {
				...
				console.error("Print client [" + self._id + "] destroyed cause Timeout");
			}
		}, (config.x3print && config.x3print.timeout) || 180000);
	}
});