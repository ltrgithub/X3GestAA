...
function bufferConcat(table) {
	...
	table.map(function(item, i) {
		if (item.buf) {
			var b = item.buf();
			tracer && commonComm.traceBuffer(tracer, i, item.descr, b);
...
exports.SadFsqComm = helpers.defineClass(function(_, client) {
	...
	_writeBufCLS: function() {
		...
		tracer && commonComm.traceBuffer(tracer, 0, "disconnect", buffer);
		return buffer;
	},
	...
	_readBufOUF: function(buf) {
		// get file num 
		tracer && commonComm.traceBuffer(tracer, 0, "File number", buf);
	...
	_processOU: function(_, suf, sadFile, openMod) {
		...
		if (this._NoLogicalError(_, buf)) {
			...
			tracer && commonComm.traceBuffer(tracer, 0, "Error code", buf);
		}
	},
	...
	_readBufTFF: function(buf) {
		tracer && commonComm.traceBuffer(tracer, 0, "is End Of File", buf);
		return buf.readUInt8(0);
	},
	...
	_readBufPAT: function(buf, id) {
		switch (id) {
			case 0:
				tracer && commonComm.traceBuffer(tracer, id + 1, "Length of path", buf);
				return buf.readInt8(0);
			case 1:
				tracer && commonComm.traceBuffer(tracer, id + 1, "Path", buf);
	...
	_readBufPRM: function(buf, info) {
		tracer && commonComm.traceBuffer(tracer, 1, info, buf);
	...
	processPRM: function(_, sadFile, info) {
		...
		tracer && commonComm.traceBuffer(tracer, 0, "Error code", buf.slice(0, 1));
		if (this._NoLogicalError(_, buf)) {
	...
	_readBufLBF: function(buf, id, binary) {
		switch (id) {
			case 0:
				tracer && commonComm.traceBuffer(tracer, 0, "Length", buf);
				var len = buf.readInt32BE(0); // length of buffer to read
				return len;
			case 1:
				tracer && commonComm.traceBuffer(tracer, 1, "Binary", buf);
	...
	_readBufERR: function(buf, id) {
		switch (id) {
			case 0:
				tracer && commonComm.traceBuffer(tracer, id + 1, "Message length", buf);
				return buf.readUInt8(0);
			case 1:
				tracer && commonComm.traceBuffer(tracer, id + 1, "Message", buf);
				return buf.toString("utf-8");
			default:
	...
	processERR: function(_, errId) {
		...
		tracer && commonComm.traceBuffer(tracer, 0, "Ask error", buf);
		this.stream.write(_, buf);
		// read first by that is normally an error
		this.tracer.info && this.tracer.info("\n### Read buffer for order: ERR ###");
		buf = this.stream.read(_, 2);
		tracer && commonComm.traceBuffer(tracer, 0, "Error code and size", buf);
