...
var x3errors = require('syracuse-x3/lib/clients/common/x3errors');
var commonComm = require('syracuse-x3/lib/clients/common/commonComm');
var CommError = require('syracuse-x3/lib/clients/common/commonComm').CommError;
var tracer = require("syracuse-trace/lib/helper").getTracer("x3Comm.sadfsq");
var nodeconfig = require('config');
var protocolConfig = ((nodeconfig.config || {}).x3sadfsq || {}).protocol || {};
	...
	buffers.canonical = {
		...
		buf: function() {
			if (sadFile && sadFile.canonical) {
				...
				throw new CommError(locale.format(module, "canonicalNotExists"));
			}
		}
	};
...
function bufferConcat(table) {
	...
	table.map(function(item, i) {
		if (item.buf) {
			var b = item.buf();
			tracer.debug && tracer.debug(commonComm.traceBuffer(i, item.descr, b));
			buf = Buffer.concat([buf, b]);
		} else {
			tracer.info && tracer.info("no method buf for item ");
		}
	});
	tracer.debug && tracer.debug(buf);
	return buf;
}
...
/// 
/// ---
exports.SadFsqComm = helpers.defineClass(function(_, client) {
	...
	_NoLogicalError: function(_, buf) {
		if (!buf) {
			throw new CommError(locale.format(module, "errorIdNotFound"));
		} else if (this._hasError(buf)) {
			var err = this.processERR(_, (256 - buf.readUInt8(0)));
			throw new CommError(err);
		}
		return true;
	},
	...
	_writeBufCLS: function() {
		...
		tracer.info && tracer.info(commonComm.traceBuffer(0, "disconnect", buffer));
		return buffer;
	},
	...
	_readBufOUF: function(buf) {
		// get file num 
		tracer.info && tracer.info(commonComm.traceBuffer(0, "File number", buf));
	...
	_processOU: function(_, suf, sadFile, openMod) {
		...
		if (this._NoLogicalError(_, buf)) {
			...
			tracer.info && tracer.info(commonComm.traceBuffer(0, "Error code", buf));
		}
	},
	...
	/// ---
	/// 
	_writeBufOUS: function(sadFile, mod, mask) {
		var _mod = {
			descr: "mod Buffer",
			buf: function() {
				var buf = new Buffer(4);
	...
	_readBufTFF: function(buf) {
		tracer.info && tracer.info(commonComm.traceBuffer(0, "is End Of File", buf));
		return buf.readUInt8(0);
	},
	...
	_readBufPAT: function(buf, id) {
		switch (id) {
			case 0:
				tracer.info && tracer.info(commonComm.traceBuffer(id + 1, "Length of path", buf));
				return buf.readInt8(0);
			case 1:
				tracer.info && tracer.info(commonComm.traceBuffer(id + 1, "Path", buf));
				return buf.toString("utf-8");
			default:
				throw new CommError("Can't read, reply not expected");
		}
	},
	...
	_writeBufPRM: function(sadFile, info) {
		...
		if (info == null || fileInfo === -1) {
			throw new CommError("The file attribute '" + info + "' is not allowed.");
		}
	...
	_readBufPRM: function(buf, info) {
		tracer.info && tracer.info(commonComm.traceBuffer(1, info, buf));
	...
	processPRM: function(_, sadFile, info) {
		...
		tracer.info && tracer.info(commonComm.traceBuffer(0, "Error code", buf.slice(0, 1)));
		if (this._NoLogicalError(_, buf)) {
	...
	_readBufLBF: function(buf, id, binary) {
		switch (id) {
			case 0:
				tracer.info && tracer.info(commonComm.traceBuffer(0, "Length", buf));
				var len = buf.readInt32BE(0); // length of buffer to read
				return len;
			case 1:
				tracer.info && tracer.info(commonComm.traceBuffer(1, "Binary", buf));
	...
	processLBF: function(_, fd, lenToRead) {
		this._checkStream();
		var size = lenToRead ? lenToRead : 1024 * (protocolConfig.LBFChunkSize || 64);
		if (size < 0) throw new CommError(locale.format(module, "sizeToReadNegative"));
		...
		do {
			...
			header = this.stream.read(_, 5);
			if (!header) break;
			this._NoLogicalError(_, header);
			len = this._readBufLBF(header.slice(1), 0);
			if (len) {
				buf = this._readBufLBF(this.stream.read(_, len), 1, buf);
			}
			if (lenToRead) lenToRead -= len;
		} while (!(len != size || (lenToRead != null && lenToRead <= 0))); // read to the end or the requested size.
		return buf;
	},
	...
	_readBufERR: function(buf, id) {
		switch (id) {
			case 0:
				tracer.info && tracer.info(commonComm.traceBuffer(id + 1, "Message length", buf));
				return buf.readUInt8(0);
			case 1:
				tracer.info && tracer.info(commonComm.traceBuffer(id + 1, "Message", buf));
				return buf.toString("utf-8");
			default:
				throw new CommError("automate issue, can't read the right segment");
		}
	},
	processERR: function(_, errId) {
		this.tracer.info && this.tracer.info("Catch logical error");
		this._checkStream();
		this.tracer.info && this.tracer.info("\n### Create buffer for order ERR ###");
		var buf = this._writeBufERR();
		tracer.info && tracer.info(commonComm.traceBuffer(0, "Ask error", buf));
		this.stream.write(_, buf);
		// read first by that is normally an error
		this.tracer.info && this.tracer.info("\n### Read buffer for order: ERR ###");
		buf = this.stream.read(_, 2);
		tracer.info && tracer.info(commonComm.traceBuffer(0, "Error code and size", buf));
		var err = x3errors.getError(errId) + " : ";
		var size = this._readBufERR(buf.slice(1), 0);
		err += this._readBufERR(this.stream.read(_, size), 1);
		//this.tracer.info && this.tracer.info(JSON.stringify(err, null, 2));
		return err;
	}
});