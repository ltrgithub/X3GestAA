...
exports.SadFsqClient = helpers.defineClass(function(_, hostname, port, user, password, secured, recOptions) {
	...
	/// var sadfs = new SadFsqClient(_, "server", "port");
	...
	open: function(_, options, flags) {
		...
		try {
			...
			tracer && tracer("failed to open " + e.message + "- retry connect and open");
			this.stream = null;
			this.__connect(_);
			fd = this.communicator.processOUF(_, sadFile, flags);
		}
	...
	path: function(_, options) {
		...
		try {
			...
			this.stream = null;
			tracer && tracer("failed to get the path " + e.message + " - retry");
			this.__connect(_);
			fullPath = this.communicator.processPAT(_, sadFile);
		}
		return fullPath;
	...
	rename: function(_, oldOptions, newOptions) {
		...
		try {
			...
			tracer && tracer("failed to rename " + e.message + "  - retry");
			this.stream = null;
			this.__connect(_);
			this.communicator.processREN(_, oldSadFile, newSadFile);
		}
	...
	readFile: function(_, fileOptions, options) {
		...
		tracer && tracer("buff readed " + buffer.toString("hex"));
		return buffer;
	},
	...
	_unlink: function(_, sadFile) {
		...
		try {
			...
			tracer && tracer("can't remove file " + e.message + " - retry");
	...
	exec: function(_, cmd) {
		...
		try {
			...
			this.stream && this.stream.end;
			this.steam = null;
			tracer && tracer("can't execute command " + e.message + " - retry");
			this.__connect(_);
			pid = this.communicator.processOUP(_, sadFile, "r");
		}
		if (pid) {
