...
exports.SadFsqClient = helpers.defineClass(function(_, context, user, password, secured, recOptions) {
	var options = {
	...
	if (context && context.host && context.port) {
		commonClient.CommonClient.call(this, _, options, context.host, context.port, user, password, secured);

	} else if (context.x3solution) {
		// always use MAIN x3 server
		commonClient.CommonClient.call(this, _, options, context.x3solution(_).serverHost(_), context.x3solution(_).serverPort(_), user, password, secured);
	} else {
