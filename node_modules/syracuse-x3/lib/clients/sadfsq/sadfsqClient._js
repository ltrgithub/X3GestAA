...
var commonClient = require('../common/commonClient');
var SadFsqFile = require('./sadfsqFile').SadFsqFile;
var sadFsqComm = require('./sadfsqComm');
...
exports.SadFsqClient = helpers.defineClass(function(_, context, user, password, secured, options, recOptions) {
	options = options || {};
	var opt = {
		...
		config: {
			timeout: options.timeout || (config.x3sadfsq && config.x3sadfsq.timeout)
		}
	};
	if (context && context.host && context.port) {
		commonClient.CommonClient.call(this, _, opt, context.host, context.port, user, password, secured);
	} else {
		var x3sol = context.x3solution(_);
		if (x3sol) { // always use MAIN x3 server
			commonClient.CommonClient.call(this, _, opt, x3sol.serverHost(_), x3sol.serverPort(_), user, password, secured);
		} else {
			throw new Error("bad context : should contain an endpoint instance or an object with host and port properties");
		}
	}
	...
	/// To use this module do require('./sadfsqClient').SadFsqClient.  
	/// All the methods are asynchronous.  
	/// ``` javascript
	/// var SadFsqClient = require('./sadfsqClient').SadFsqClient;
	...
	open: function(_, options, flags) {
		...
		try {
			...
			tracer.error && tracer.error("failed to open stack:" + e.stack);
			this.stream = null;
			this.__connect(_);
			fd = this.communicator.processOUF(_, sadFile, flags);
		}
	...
	readFile: function(_, fileOptions, options) {
		...
		tracer.info && tracer.info("buff read " + buffer.toString("hex"));
		return buffer;
	},
	...
	read: function(_, fd, buffer, offset, length, position) {
		var self = this;
		if (offset + length > buffer.length) throw new RangeError("Length extends beyond buffer");
		self.__connect(_);
		var fdOpened = this.fdsOpen[fd];

		if (length > 0) {
			if (position != null) {
				self.seek(_, fd, 0, position);
			}
			var inputLength = 0;
			do { // read chunks of at most CHUNKSIZE bytes
				var size = Math.min(CHUNKSIZE, length - inputLength);
				if (!self.communicator) return inputLength;
				var buf = self.communicator.processLBF(_, fdOpened.fd, size);
				buf.copy(buffer, offset);
				fdOpened.offset += buf.length;
				inputLength += buf.length;
				// could chunk be read completely? If not, it does not make sense to continue
				if (buf.length < size) {
					return inputLength;
				}
				offset += size;
				self.__setSocketTimeout(_, true); // avoid timeout for long files
			} while (length > inputLength);
			return inputLength; // return size of bytes read
		} else {
			return 0;
		}
	},
	...
	write: function(_, fd, buffer, offset, length, position, lock) {
		...
		if (opened) {
			...
			while (end - begin > CHUNKSIZE) { // write in chunks of at most CHUNKSIZE bytes
				this.communicator.processEBF(_, opened.fd, buffer.slice(begin, begin + CHUNKSIZE));
				begin += CHUNKSIZE;
				this.__setSocketTimeout(_, true); // avoid timeout for long files
			}
	...
	/// Returns the output generated by the command `ae_rmdir`.
	///
	mv: function(_, fileOptions1, fileOptions2) {
		return this._execbin(_, "ae_mv", this.path(_, fileOptions1) + " " + this.path(_, fileOptions2));
		// an exception is raise if the launch of process failed
	},
