...
exports.SadFsqClient = helpers.defineClass(function(_, endpoint, user, password, secured, recOptions) {
	var options = {
	...
	commonClient.CommonClient.call(this, _, options, endpoint.x3solution(_).serverHost(_), endpoint.x3solution(_).serverPort(_), user, password, secured);
	...
	/// var sadfs = new SadFsqClient(_, endpoint, "user", "password", secure);
	/// ```
	/// * `endpoint` is the endpoint instance to use for connection.  
