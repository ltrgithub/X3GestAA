...
var trace = require("syracuse-trace/lib/helper").getTracer("jsRunner");
...
var Runner = helpers.defineClass(function() {
	...
	execute: function(_, modul, body) {
		...
		trace.info() && trace.info("Mode: " + body.mode.toUpperCase() + " : Execute JS function from 4GL process...");
		trace.info() && trace.info("******************");
		trace.info() && trace.info("Module: " + JSON.stringify(modul, null, 2));
		trace.info() && trace.info("Body: " + JSON.stringify(body, null, 2));
		try {
			var args = [];
				...
				body.args.forEach_(_, function(_, elt, idx) {
					if (body.encodings != null && body.encodings[idx] > 0) {
						if (body.encodings[idx] === 1) {
							...
							trace.info() && trace.info("Decrypted argument [" + idx + "]: " + JSON.stringify(args[idx], null, 2));
						} else {
	...
	buildResponse: function(_, result, body, future) {
		...
		trace.info() && trace.info("Function Result: " + JSON.stringify(res, null, 2));
		return res;
	},

	httpRequest: function(_, header, body) {
		trace.info() && trace.info("******************");
		trace.info() && trace.info("Execute HTTP request from 4GL process...");
		trace.info() && trace.info("******************");
		trace.info() && trace.info("Header: " + JSON.stringify(header, null, 2));
		trace.info() && trace.info("Body: " + JSON.stringify(body, null, 2));
		var res;
		try {
		...
		trace.info() && trace.info("HTTP Result: " + JSON.stringify(res, null, 2));
		return res;
...
exports.getResult = function(_, uuid) {
	trace.info() && trace.info("******************");
	trace.info() && trace.info("Retrieve future: " + uuid);
	trace.info() && trace.info("******************");
