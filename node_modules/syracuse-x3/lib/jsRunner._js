...
var helpers = require('@sage/syracuse-core').helpers,
	...
	httpClient = require("@sage/syracuse-lib/src/http-client/httpClient");
var fs = require('streamline-fs');
var path = require('path');

var tracerJs = require('@sage/syracuse-core').getTracer("x3Comm.jsRunner");
var perfmon = require('@sage/syracuse-lib/src/perfmon/record');

var compatibilityBox = {
	"syracuse-httpclient/lib/httpApi": "@sage/syracuse-lib/src/http-client/httpApi",
	"syracuse-auth/lib/signatureAuth": "@sage/syracuse-lib/src/auth/signatureAuth",
	"syracuse-import/lib/jsonImport": "@sage/syracuse-lib/src/import/jsonImport",
	"syracuse-orm/lib/storageArea": "@sage/syracuse-lib/src/orm/storageArea",
	"syracuse-sdata/lib/asyncTrack": "@sage/syracuse-lib/src/sdata/asyncTrack",
};
...
var Runner = helpers.defineClass(function() {
	...
	execute: function(_, modul, body) {
		modul = compatibilityBox[modul] || modul;
		var timing = perfmon.start(module, "jsRunner.execute", modul + "." + body.function);

		tracerJs.info && tracerJs.info("******************");
		tracerJs.info && tracerJs.info("Mode: " + (body.mode ? body.mode.toUpperCase() : "NOMOD") + " : Execute JS function from 4GL process...");
		tracerJs.info && tracerJs.info("******************");
	...
	buildResponse: function(_, result, body, future) {
		...
		try {
			...
			if (result && result.statusCode && result.header && result.body !== undefined) {
				...
				res.header.message = require('http').STATUS_CODES[result.statusCode];
			} else {
				res = {
					header: {
						"statusCode": "200",
	...
	httpRequest: function(_, header, body) {
		...
		// remove headers with an empty name
		Object.keys(header).forEach(k => {
			if (!k) delete header[k];
		});
		var res;
		try {
			var accept = header.accept || header.Accept;
			var opt = {
				url: header.url,
			...
			res = {
				header: resp.headers,
				body: Math.floor(resp.statusCode / 100) === 2 ? (accept === "application/json" ? JSON.parse(content) : content) : ""
			};
			res.header.message = require('http').STATUS_CODES[resp.statusCode];
		} catch (e) {
