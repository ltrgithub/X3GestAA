...
var streams = require("streamline/lib/streams/streams"),
	...
	httpHelper = require("syracuse-httpclient/lib/helper");
...
var Runner = helpers.defineClass(function() {
	...
	buildResponse: function(result, body, future) {
		...
		try {
			...
			if (result != null && typeof result === "object" && result.header != null && result.body!= null)
				res = result;
	...
	httpRequest: function(_, header, body){
		...
		try {
			...
			if (!header.proxy && proxyConf) {
				header.proxy = proxyConf;
			}
			...
			var resp = request.proxyAuthenticate(_).end().response(_);
...
exports.getResult = function(_, uuid) {
	...
	if (future == null)
		return genErrorHead(500, "The 'future' corresponding to uuid '"+uuid+"' doesn't exist", 'no stack');
