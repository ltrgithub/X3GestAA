...
var trace = require("syracuse-trace/lib/helper").getTracer(_, "jsRunner");
...
function genErrorHead(_, status, message, stack) {
	...
	trace.error(_) && trace.error(_, stack);
	return {
		header: head,
		body: result
	};
...
var Runner = helpers.defineClass(function() {
	...
	execute: function(_, modul, body) {
		...
		trace.info(_) && trace.info(_, "Mode: " + body.mode.toUpperCase() + " : Execute JS function from 4GL process...");
		trace.info(_) && trace.info(_, "******************");
		trace.info(_) && trace.info(_, "Module: " + JSON.stringify(modul, null, 2));
		trace.info(_) && trace.info(_, "Body: " + JSON.stringify(body, null, 2));
		try {
			var args = [];
				...
				body.args.forEach_(_, function(_, elt, idx) {
					if (body.encodings != null && body.encodings[idx] > 0) {
						if (body.encodings[idx] === 1) {
							...
							trace.info(_) && trace.info(_, "Decrypted argument [" + idx + "]: " + JSON.stringify(args[idx], null, 2));
						} else {
	...
	buildResponse: function(_, result, body, future) {
		...
		trace.info(_) && trace.info(_, "Function Result: " + JSON.stringify(res, null, 2));
		return res;
	},

	httpRequest: function(_, header, body) {
		trace.info(_) && trace.info(_, "******************");
		trace.info(_) && trace.info(_, "Execute HTTP request from 4GL process...");
		trace.info(_) && trace.info(_, "******************");
		trace.info(_) && trace.info(_, "Header: " + JSON.stringify(header, null, 2));
		trace.info(_) && trace.info(_, "Body: " + JSON.stringify(body, null, 2));
		var res;
		try {
		...
		trace.info(_) && trace.info(_, "HTTP Result: " + JSON.stringify(res, null, 2));
		return res;
...
exports.getResult = function(_, uuid) {
	trace.info(_) && trace.info(_, "******************");
	trace.info(_) && trace.info(_, "Retrieve future: " + uuid);
	trace.info(_) && trace.info(_, "******************");
