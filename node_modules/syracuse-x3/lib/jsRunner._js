...
function genErrorHead(_, status, message, stack) {
	...
	tracer.error() && tracer.error(stack);
	return {
		header: head,
		body: result
	};
...
var Runner = helpers.defineClass(function() {
	...
	execute: function(_, modul, body) {
		...
		tracer.info() && tracer.info("Mode: " + body.mode.toUpperCase() + " : Execute JS function from 4GL process...");
		tracer.info() && tracer.info("******************");
		tracer.info() && tracer.info("Module: " + JSON.stringify(modul, null, 2));
		tracer.info() && tracer.info("Body: " + JSON.stringify(body, null, 2));
		try {
			var args = [];
				...
				body.args.forEach_(_, function(_, elt, idx) {
					if (body.encodings != null && body.encodings[idx] > 0) {
						if (body.encodings[idx] === 1) {
							...
							tracer.info() && tracer.info("Decrypted argument [" + idx + "]: " + JSON.stringify(args[idx], null, 2));
						} else {
	...
	buildResponse: function(_, result, body, future) {
		...
		tracer.info() && tracer.info("Function Result: " + JSON.stringify(res, null, 2));
		return res;
	},

	httpRequest: function(_, header, body) {
		tracer.info() && tracer.info("******************");
		tracer.info() && tracer.info("Execute HTTP request from 4GL process...");
		tracer.info() && tracer.info("******************");
		tracer.info() && tracer.info("Header: " + JSON.stringify(header, null, 2));
		tracer.info() && tracer.info("Body: " + JSON.stringify(body, null, 2));
		var res;
		try {
		...
		tracer.info() && tracer.info("HTTP Result: " + JSON.stringify(res, null, 2));
		return res;
...
exports.getResult = function(_, uuid) {
	tracer.info() && tracer.info("******************");
	tracer.info() && tracer.info("Retrieve future: " + uuid);
	tracer.info() && tracer.info("******************");
