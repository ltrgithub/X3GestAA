...
var tracer = require("syracuse-trace/lib/helper").getTracer("jsRunner", "Javascript calls from 4GL");
...
function genErrorHead(_, status, message, stack) {
	...
	tracer.trace.error() && tracer.trace.error(stack);
	return {
		header: head,
		body: result
	};
...
var Runner = helpers.defineClass(function() {
	...
	execute: function(_, modul, body) {
		...
		tracer.trace.info() && tracer.trace.info("Mode: " + body.mode.toUpperCase() + " : Execute JS function from 4GL process...");
		tracer.trace.info() && tracer.trace.info("******************");
		tracer.trace.info() && tracer.trace.info("Module: " + JSON.stringify(modul, null, 2));
		tracer.trace.info() && tracer.trace.info("Body: " + JSON.stringify(body, null, 2));
		try {
			var args = [];
				...
				body.args.forEach_(_, function(_, elt, idx) {
					if (body.encodings != null && body.encodings[idx] > 0) {
						if (body.encodings[idx] === 1) {
							...
							tracer.trace.info() && tracer.trace.info("Decrypted argument [" + idx + "]: " + JSON.stringify(args[idx], null, 2));
						} else {
			...
			if (!$module.$exported) throw new Error("module forbidden - $exported flag is not set");
	...
	buildResponse: function(_, result, body, future) {
		...
		tracer.trace.info() && tracer.trace.info("Function Result: " + JSON.stringify(res, null, 2));
		return res;
	},

	httpRequest: function(_, header, body) {
		tracer.trace.info() && tracer.trace.info("******************");
		tracer.trace.info() && tracer.trace.info("Execute HTTP request from 4GL process...");
		tracer.trace.info() && tracer.trace.info("******************");
		tracer.trace.info() && tracer.trace.info("Header: " + JSON.stringify(header, null, 2));
		tracer.trace.info() && tracer.trace.info("Body: " + JSON.stringify(body, null, 2));
		var res;
		try {
		...
		tracer.trace.info() && tracer.trace.info("HTTP Result: " + JSON.stringify(res, null, 2));
		return res;
...
exports.getResult = function(_, uuid) {
	tracer.trace.info() && tracer.trace.info("******************");
	tracer.trace.info() && tracer.trace.info("Retrieve future: " + uuid);
	tracer.trace.info() && tracer.trace.info("******************");
