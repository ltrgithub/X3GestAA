...
var tracer; //= require('@sage/syracuse-core').getTracer("x3Comm.gateway");
// var tracer = {
// 	error: console.log,
// 	info: console.log,
// 	debug: console.log
	...
	try {
		...
		if (up) {
			...
			var sid = client.createSession(_, {
				...
				baseUrl: context && context.baseUrl || session ? session.host + client.baseUrl : "",
				collaborationBaseUrl: adminEP && session ? session.host + adminHelper.getCollaborationEndpoint(_).getBaseUrl(_) : "",
				peerAddress: peer,
				noCheckLicense: false //https://github.com/Sage-ERP-X3/Syracuse/issues/9922
			});
...
exports.gateway = function(_, script, operation, parameters) {
	tracer = tracer || require('@sage/syracuse-core').getTracer("x3Comm.gateway");
