...
exports.getSignature = function(_, challenge, userName, certificate) {
	...
	if (certificate) {
		...
		if (nodeconfig.config.x3key) {
			result = require('syracuse-load/lib/certTools').sign(_, null, nodeconfig.config.sdata.signAlgorithm, toSign, { "output_encoding": "base64"})
		} else {
			var signer = crypto.createSign(nodeconfig.config.sdata.signAlgorithm);
			signer.update(toSign);
			var result = signer.sign(fs.readFile(fsp.join(__dirname, "../key.pem"), "ascii", _), "base64");			
		}
		return result;
	}
};
