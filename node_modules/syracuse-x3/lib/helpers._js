...
exports.getSignature = function(_, challenge, userName, certificate) {
	...
	var result;
	if (certificate) {
		result = certificate.sign(_, nodeconfig.config.sdata.signAlgorithm, toSign, {
			"output_encoding": "base64"
		});
		return result;
	} else {
		if (nodeconfig.config.x3key) {
			result = require('syracuse-load/lib/certTools').sign(_, null, nodeconfig.config.sdata.signAlgorithm, toSign, {
				"output_encoding": "base64"
			});
		} else {
