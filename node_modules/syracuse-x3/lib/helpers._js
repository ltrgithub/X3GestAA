...
exports.getSignature = function(_, challenge, userName, certificate) {
	...
	//TODO maybe special handling of internal certificates
	if (nodeconfig.collaboration && nodeconfig.collaboration.certdir && !certificate)
		throw new Error("Set certificate for X3 server");
	if (certificate) {
		result = certificate.sign(_, nodeconfig.config.sdata.signAlgorithm, toSign, { "output_encoding": "base64"});
		return result;
	} else {
		var signer = crypto.createSign(nodeconfig.config.sdata.signAlgorithm);
		signer.update(toSign);
		var result = signer.sign(fs.readFile(fsp.join(__dirname, "../key.pem"), "ascii", _), "base64");		console.log("SIGNER "+util.format(result));
		return result;
	}
};
...
exports.dateFormatToX3Format = function(dateFormat) {
	...
	dateUtil.walkFormat(dateFormat, {
		...
		d: function(repeat) {
			if (repeat > 2) fullDayPos = result.length;
			else shortDayPos = result.length;
			result.push("DD");
		},
		M: function(repeat) {
			switch (repeat) {
...
exports.addLocaleConfig = function(_, cliCfg, userProfile) {
	...
	if (selLocale && selLocale.shortDate(_)) {
		...
		dateUtil.walkFormat((cliCfg.SShortDate = selLocale.shortDate(_)), {
			...
			literal: function(literal) {
				if (dateSep == null) dateSep = literal;
			},
			d: function(repeat) {
...
exports.getTimeSep = function(timeFormat) {
	...
	if (timeFormat) {
		dateUtil.walkFormat(timeFormat, {
			// We assume that first literal is the date separator
			literal: function(literal) {
				if (timeSep == null) timeSep = literal;
			},
			H: dummy,
			h: dummy,
			m: dummy,
