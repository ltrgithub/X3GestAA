...
function testUpdtSyrFunc(_) {
	...
	var rec = db.fetchInstance(_, entity, {
		sdataWhere: "LANCHP eq " + dotnetChapter + "  and LAN eq 'ENG' and LANNUM eq 3" /*+(count+1).toString()*/
	});
	ok(rec != null, "third resource injected");
	msg = rec.LANMES(_);
	strictEqual(msg, "third resource", "third resource");
	var rec = db.fetchInstance(_, entity, {
		sdataWhere: "LANCHP eq " + dotnetChapter + " and LAN eq 'ENG' and LANNUM eq 4" /*+(count+2).toString()*/
	});
...
asyncTest("long message", 10, testLongMessage);
