...
function testUpdtSyrFunc(_) {
   //tracer= trace_dotnet_extract ;
    ...
    context.request.headers={
        ...
        var count=db.count(_,entity, {sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG'"}) ;
    ...
    var msg ;
    var res=db.fetchInstances(_,entity/*,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM ne 0"}*/);
   
    tracer && tracer("RES: ") ;
    res.forEach_(_,function(_,r){ 
        tracer && tracer ("rec: "+ JSON.stringify(r.serializeInstance(_),null,2)) }) ;
        var rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+"  and LAN eq 'ENG' and LANNUM eq 3"/*+(count+1).toString()*/});
        ok(rec != null, "third resource injected");
        msg=rec.LANMES(_) ;
        strictEqual(msg, "third resource" , "third resource") ;
         var rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM eq 4"/*+(count+2).toString()*/});
        ...
        var db = dataModel.getOrm(_, _getModel(), endPoint.datasets.mongodb_admin_test);
        var model = db.model;
        var techEntity = model.getEntity(_, "translationTool");
        ...
        '<value>forth resource+</value>\n'+ // 
        '</data>\n'+ //
        /*'<data name="key5" xml:space="preserve">\n' + //
        '<value>Un message beaucoup beaucoup beaucoup beaucoup beaucoup beaucoup beaucoup beaucoup beaucoup trop trop trop trop trop trop trop trop long+</value>\n'+ // 
        '</data>\n'+ //*/
        "after" ;  
        ...
        fs.writeFile(fpath, inputXml, "utf8", _);
       fs.close(fd,_);
        var rec4=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM eq 4"});
        ...
        var rec3=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM eq 3"});
...
function testUpdtX3Func(_) {
    ...
    context.request.headers={
        ...
        var entity=db.getEntity(_,"APLSTD") ;
        var rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM eq 3"});
        if (rec){
            var expected=rec.LANMES(_)+" updated on X3 side";
            rec.LANMES(_, expected) ;
            rec.save(_)
        }
        rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'ENG' and LANNUM eq 3"});
       ...
       var recFR= entity.createInstance(_,db) ;
        ...
        recFR=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq "+dotnetChapter+" and LAN eq 'FRA' and LANNUM eq 3"});
