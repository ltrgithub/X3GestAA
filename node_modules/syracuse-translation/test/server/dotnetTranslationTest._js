...
function testUpdtSyrFunc(_) {
    ...
    try {
       ...
       tracer && tracer("1st extraction");
        ...
        var rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq 10033 and LAN eq 'ENG' and LANNUM eq "+(count+1).toString()});
        ok(rec != null, "third resource injected");
        msg=rec.LANMES(_) ;
        strictEqual(msg, "third resource" , "third resource") ;
         var rec=db.fetchInstance(_,entity,{sdataWhere: "LANCHP eq 10033 and LAN eq 'ENG' and LANNUM eq "+(count+2).toString()});
        ...
        '</data>\n' + // 
        '<data name="key4" xml:space="preserve">\n' + //
        '<value>forth resource changed in syracuse</value>\n'+ // 
        '</data>\n'+ //
        "after" ;  
       ...
       fs.close(fd,_);
        ...
        rec4.LANMES(_,"forth resource changed in X3") ;
        rec4.save(_) ;
        ...
        strictEqual(rec3.LANMES(_), "third resource changed", "third resource modification done") ;
        var diag=body.$diagnoses;
        console.log("***********diag[0].message: "+diag[0].message) ;
        strictEqual(diag[0].severity, "warning" , "conflict warning raised") ;
        strictEqual(diag[0].message, 'update conflict for Syracuse/node_modules/syracuse-translation/test/server/dotnet/unitTestExtraction.resx:key4 : Forth key changed in syracuse vs. Forth key changed in X3',"warning message") ;
