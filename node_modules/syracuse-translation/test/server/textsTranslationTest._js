...
function testUpdtSyrFunc(_) {
    ...
    try {
        ...
        var rec=db.fetchInstance(_,entity,{sdataWhere: "lanchp eq 10031 and LAN eq 'ENG' and LANNUM eq 1"/*+obj["key1"].id*/});
        if (!rec)
            console.log("NON CREE "+relpath+' '+key1) ;
        else{
            var msg=rec.LANMES(_) ;
            strictEqual(msg,inputJson["key1"] , "before update") ;
        }
        
        var fpath = fsp.join(__dirname, "../server/resources/unitTest-en.json");
        var input = fs.readFile(fpath, "utf8", _);
        var parsed = JSON.parse(input);
        inputJson["key1"] = inputJson["key1"] + " changed";
        fs.writeFile(fpath, JSON.stringify(inputJson), "utf8", _);
        ...
        rec=db.fetchInstance(_,entity,{sdataWhere: "lanchp eq 10031 and LAN eq 'ENG' and LANNUM eq "+obj["key1"].id});
        if (!rec)
            console.log("NON CREE "+relpath+" key1") ; 
        else {
        var msg=rec.LANMES(_) ;
        console.log("msg="+msg) ;
        strictEqual(msg,inputJson["key1"] , "after update") ;
        }
        //console.log("CALL: deleteFolderRecursive!!!!!!!") ;
        //deleteFolderRecursive(_,dpath) ;


        var tmp=inputJson["key1"] ;
        inputJson["key1"] = inputJson["key1"] + " conlict";
        fs.writeFile(fpath, JSON.stringify(inputJson), "utf8", _);
        ...
        data.LANMES= data.LANMES+" conflict1";
        ...
        strictEqual(diag[0].message, 'update conflict for resource Syracuse/node_modules/syracuse-translation/test/server/resources/unitTest-en.json:key1, choose between  First key changed conlict(syracuse version)  and   First key changed conflict1(X3 version) ',"warning message") ;
        //restore
        inputJson["key1"] = tmp;
        fs.writeFile(fpath, JSON.stringify(inputJson), "utf8", _);
...
function testUpdtX3Func(_) {
    ...
    try {
       ...
       // var rec = db.fetchInstance(_, entity, {
        ...
        var body = get(_, cookie, "ZZAPLSTDs?sdataWhere=(lanchp eq 10031 and LAN eq 'ENG' and LANNUM eq 2)");
        ...
        body = get(_, cookie, "ZZAPLSTDs?sdataWhere=(lanchp eq 10031 and LAN eq 'ENG' and LANNUM eq 2)");
        console.log("GET: "+JSON.stringify(body,null,2)) ;
       ...
       body = put(_, cookie, body.$url, dataX3);
        ...
        body = get(_, cookie, "ZZAPLSTDs?sdataWhere=(lanchp eq 10031 and LAN eq 'FRA' and LANNUM eq 2)");
       ...
       strictEqual(expectedFr , body.$resources[0].LANMES,"french translation created on X3 ") ;
        ...
        strictEqual(expected , parsed["key2"] ,"Syracuse resource updated: eng") ;
        
        fpath = fsp.join(dpath, unitTest+"-fr.json");
        input = fs.readFile(fpath, "utf8", _);
        parsed = JSON.parse(input);
        
        strictEqual(expectedFr , parsed["key2"] ,"Syracuse resource created: fr") ;
...
asyncTest("update of the message in syracuse", 11, testUpdtSyrFunc);
