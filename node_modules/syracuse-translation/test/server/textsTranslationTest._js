...
asyncTest("index construction", 8, function (_) {
    ...
    try {
        ...
        var relpath=fsp.relative(rootpath,abspath) ;
        console.log("relpath: "+relpath) ;
        var key1=fsp.join(relpath,"key1").replace(/\\/g,'\/') ;


        strictEqual(index[key1].text,inputJson["key1"] , "key1") ;
        var key2=fsp.join(relpath,"key2").replace(/\\/g,'\/') ;
        strictEqual(index[key2].text,inputJson["key2"] , "key2") ;
        var key3=fsp.join(relpath,"key3").replace(/\\/g,'\/') ;
        strictEqual(index[key3].text,inputJson["key3"] , "key3") ;
...
//setTimeout(function(){
    asyncTest("update of the message in syracuse", 5, function (_) {
    var context={request :{}} ;
    cookie = getCookie(_);
    context.headers={
          "accept" : "application/json",
    ...
    try {
        ...
        var rec=db.fetchInstance(_,entity,{sdataWhere: "lanchp eq 10031 and lan eq 'ENG' and lannum eq "+idx[key1].id});
        if (!rec)
            console.log("NON CREE "+key1) ;
        else{
