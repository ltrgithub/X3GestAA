...
exports.MessageStore = helpers.defineClass(MessageStore, null, {
	...
	setMessage: function(message, language, refkey) {
		return this.set.setMessage(message, language, refkey);
	},
...
exports.X3ClobMessageStore = helpers.defineClass(X3ClobMessageStore, MessageStore, {
	...
	updateAll: function(_) {
		...
		this.diagnosis.logInfo("Creating workingCopie");
		...
		this.diagnosis.logInfo("Updating workingCopie with\n" + data);
		url = writer.cdoc2(id);
...
function MessageSet() {
	...
	function _setText(chp, id, lan, text, defState, stamp, lanori) {
		...
		if (text === placeholders.equal || lan === defaultLanguage.x3) {
	...
	this.reassembleRecord = function(nchp, id) {
		var chp = col[nchp];
		if (!chp)
			return chp;
		var i, res = {
		...
		function reassamble(r) {
			...
			Object.keys(r).forEach(function(lang) {
				if (lang === "XXX") {
					...
					t[lang] = (t[lang] || "") + replaceTerminator(r[lang]);
				}
			});
		}
	...
	this.traverse = function(cb, opt, context) {
		...
		colkeys.forEach(function(k0) {
			...
			chpkeys.forEach(function(k1) {
				...
				idkeys.forEach(function(k2) {
					if (k2[0] === "$") return;
					var meta = self.getMeta(k0, k1, k2);
	...
	this.dumpToPlainFile = function(_, file, opt) {
		...
		this.traverse(function(context, chp, id, lan, text, meta) {
			...
			contents += lan + "\t" + chp + "\t" + id + "\t" + (meta.lanori == null ? "" : meta.lanori) + '\t' + (opt.quoted ? _quoted(txt) : txt) + '\n';
