...
exports.X3ClobMessageStore = helpers.defineClass(X3ClobMessageStore, MessageStore, {
	...
	smartCopy: function(_, chp1, chp2) {
		...
		var inputFile = this.options.targetDir + "/output/messages-smart-input.txt",
			outputFile = this.options.targetDir + "/output/messages-smart-" + chp1 + "-" + chp2 + "-" + (datetime.now().toString(fileDateFormat)),
			data = fs.exists(inputFile, _) && fs.readFile(inputFile, "utf8", _),
			fetched = this.load(data);
		...
		this.set.traverseKeys(function(key, entry) {
			entry.forEach(function(e, i) {
				tu1 = e.text || [];
				t1 = _cleanString(tu1["ENG"]);
				m = memory[t1] || (memory[t1] = {
					$key: t1,
					$count: 0,
					text: {}
				});
				...
				Object.keys(tu1).forEach(function(l) {
					if (l === "XXX")
						return;
					t1 = _cleanString(mt[l]);
					t2 = _cleanString(tu1[l]);
					if (t1 !== undefined && t2 !== t1 && t2 !== m.$key) {
						// console.error("smartCopy various translations for '" + m.$key + "' [" + l + "] count=" + m.$count + "\n--- " + t2 + "\n+++ " + t1);
					} else {
						mt[l] = t2;
					}
				});
	...
	this.dumpToPlainFile = function(_, file, opt) {
		...
		this.traverse(function(context, chp, id, lan, text, meta) {
			...
			var lanori = meta ? (meta.lanori == null ? "" : meta.lanori) : (lan === defaultLanguage.x3 ? "" : defaultLanguage.x3),
				line = lan + "\t" + chp + "\t" + id + "\t" + lanori + '\t' + (opt.quoted ? _quoted(txt) : txt);
			// if (txt) {
			contents += line + "\n";
			// }
...
function normalizeString(s, convertNewline) {
	...
	s = s && s.replace(/[\s\u00b7]+$/g, "\u00b7") || "";
	return _fixCurly(_cleanString(convertNewline ? s.replace(/\n/g, "\\n") : s));
}

function _fixCurly(s) {
	if (s && s.charAt(0) === "{") {
		var closing = s.indexOf("}", 1);
		var opening = s.indexOf("{", 1);
		if (closing === -1 || (opening !== -1 && closing > s.indexOf("{", 1))) {
			s = s.substring(1) + "}";
		}
	}
	return s;
