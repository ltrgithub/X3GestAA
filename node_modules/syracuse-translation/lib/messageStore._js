...
exports.AplStdMessageStore = helpers.defineClass(AplStdMessageStore, MessageStore, {
	init: function(_) {
		...
		this.entityName = "APLSTD";
		this.entity = this.db.getEntity(_, this.entityName, "$edit");
	...
	_getquery: function(_, chapter, opt) {
		var entity = this.db.getEntity(_, this.entityName, "$bulk"),
			// entity = db.getEntity(_, this.entityName, "$query"),
			lang = opt.lang,
			params = {};
		opt = opt || {};
		if (typeof(chapter) === "object") {
	...
	deleteAll: function(_, chp1, chp2, lan, stamp) {
		var entity = this.db.getEntity(_, this.entityName, "$query"),
			count = 0;

		chp2 = chp2 || chp1;

		var x3Records = this.db.fetchInstances(_, entity, {
			sdataWhere: "LANCHP ge " + chp1 + " LANCHP le" + chp2 + " and LANNUM gt 0"
		});
		count += x3Records.length;
		x3Records.forEach_(_, function(_, rec) {
			rec.deleteSelf(_);
		});
		this.diagnosis.info(localizedText("deleteChapterRecords", chp1, x3Records.length), _);
