...
exports.X3ClobMessageStore = helpers.defineClass(X3ClobMessageStore, MessageStore, {
	...
	smartCopy: function(_, chp1, chp2) {
		...
		console.error("smartCopy: " + chp1 + ", " + chp2);
		...
		this.set.traverseKeys(function(key, entry) {
			entry.forEach(function(e, i) {
				...
				if (e2 && exclude.indexOf(key) === -1) {
					e2 = e2[i] || {};
					origin = e2.$origin && e2.$origin.split("/");
					tu1 = e.text || [];
					tu2 = e2.text || [];
					t1 = tu1["ENG"];
					t2 = tu2["ENG"];
					// if (t1 != t2 && t2 !== undefined && origin) {
					if (origin) {
						Object.keys(tu1).forEach(function(l) {
							...
							tu2[l] = tu1[l];
							// set2.setText(origin[0], origin[1], lan, text, state, stamp, lanori)
							// console.error(l + "\t" + origin[0] + "\t" + origin[1] + "\t" + (meta.lanori || "") + "\t" + normalizeString(tu1[l], true));
						});
					}
				}
		...
		this.dumpToPlainFile(_, this.options.file + "-smart", set2);
		// this.set.traverse(function(context, chp, id, lan, text, meta) {
		// 	if (chp == null || id == null || lan != null)
		// 		return;
		// 	tu1 = context.$id;
