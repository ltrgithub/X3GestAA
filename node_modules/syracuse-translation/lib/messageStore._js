...
var localizedText = require('./helpers').localizedText;
...
function MessageSet() {
			...
			entry.forEach(function(e, i) {
				...
				if (tmu) {
					Object.keys(tu).forEach(function(l) {
						...
						mtl = tmu.getText(l);
						if (mtl != null) {
							if (mtl !== tu[l] && mtl !== tref && (opt.agressive || tu[l] === tref)) {
								cb(e, l, tu, mtl);
							}
						}
					});
	...
	this.upsertMessage = function(message, language, refkey, opt) {
		...
		entries.forEach(function(e, i) {
			...
			m.state = oldText == null ? "created" : (e.text[x3lang] != oldText ? "modified" : "clean");
			if (!e.$meta["XXX"]) {
				e.$meta["XXX"] = {
					lanori: "",
					state: "clean"
...
function _cleanString(s) {
	return s && s.replace(/([^\s])[\s\u00b7]+$/g, "$1").replace(/\u0085.../g, "...").replace(/\u0085/g, "");
}
...
function _appendChunk(res, t) {
	var chunk = _replaceTerminator(t);
	if (chunk)
		return (res ? res + " " : "") + chunk;
	return res;
}
...
function Dictionary() {
	...
	this.smartAssign = function(key, lan, text, ek) {
		...
		// must not be a placeholder string, must have at least 2 letters and must not be equal to the en-us text
		if (!text || exports.isEqualPlaceHolder(text) || text === key) {
			return; // text is not relevant
			// } else if (!(/(\w|[u0080-\uFFEF]){2}/.test(text))) {
			// 	return console.error("Dictionary.smartAssign, text is not relevant: [" + [key, lan, ek].join("/") + "] text='" + text + "'");
		}
