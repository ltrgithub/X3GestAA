...
function MessageSet() {
	...
	this.upsertMessage = function(message, language, refkey, opt) {
		...
		message = _fixMissspelling(message, x3lang);
...
function _fixMissspelling(text, lan) {
	if (lan === "ENG" || lan === "BRI") {
		if (/^titel$/i.test(text))
			text = "Title";
	}
	return text;
}
...
function Dictionary() {
	...
	this.smartAssign = function(key, lan, text, ek) {
		if (!key) return;
		text = _cleanString(text);
		text = _fixMissspelling(text, lan);
...
DictionaryEntry.prototype = {
	...
	setText: function(lan, text, ek) {
		var log = (this.$key === "You are logged as" && lan === "POR");
		text = _cleanString(text);
		...
		if (mtl == null) {
			...
			// if (this.$key === "You are logged as" && lan === "POR")
			// 	console.error("  various translations for '" + this.$key + "' [" + lan + "] " + JSON.stringify(mtl));
		}
	}
};
