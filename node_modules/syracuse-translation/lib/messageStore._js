		...
		if (lan === defaultLanguage.x3) {
			Object.keys(tu).forEach(function(l) {
				if (l === "XXX" && l === defaultLanguage.x3)
					return;
				dictionary.smartAssign(mkey, l, tu[l]);
			});
		} else if (mkey && lan !== "XXX") {
			dictionary.smartAssign(mkey, lan, text);
		}
...
function Dictionary() {
	...
	this.smartAssign = function(key, lan, text) {
		if (!key) return;
		text = _cleanString(text);
		if (!text || exports.isEqualPlaceHolder(text))
			return; // not relevant
...
DictionaryEntry.prototype = {
	getText: function(lan) {
		return this.text[lan] && this.text[lan].primary.text;
	},
	setText: function(lan, text) {
		text = _cleanString(text);
		var mtl = this.text[lan];
		var primary = mtl && mtl.primary;
		if (mtl == null) {
			primary = {
				text: text,
				count: 1
		...
		} else if (text !== mtl.primary.text && text !== this.$key) {
			if (!mtl.alt.some(function(e) {
				if (e.text === text) {
					if (++e.count > mtl.primary.count) {
						mtl.primary = e;
					}
					return true;
				}
...
var dictionary = exports.dictionary = new Dictionary();