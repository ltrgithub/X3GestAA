...
exports.MessageStore = helpers.defineClass(MessageStore, null, {
	...
	createEntry: function(_, chp, id, message, language, refkey) {
		...
		var localizedText = this.world.localizedText;
		if (Array.isArray(id)) {
	...
	smartCopy: function(_, chp1, chp2) {
		...
		set2.traverseKeys(function(key, entry) {
			...
			entry.forEach(function(e, i) {
				...
				var tmu = dictionnary.getEntry(t2),
					mtl;
				if (tmu) {
...
function MessageSet() {
		...
		if (lan === defaultLanguage.x3) {
			Object.keys(tu).forEach(function(l) {
				if (l === "XXX" && l === defaultLanguage.x3)
					return;
				dictionnary.smartAssign(mkey, l, tu[l]);
			});
		} else if (mkey && lan !== "XXX") {
			dictionnary.smartAssign(mkey, lan, text);
		}
	...
	this.autoFix = function() {
		...
		this.traverseKeys(function(key, entry) {
			...
			entry.forEach(function(e, i) {
				...
				var tmu = dictionnary.getEntry(tref),
					mtl;
				if (tmu) {
					Object.keys(tu).forEach(function(l) {
						if (l === "XXX" || l === defaultLanguage.x3)
	...
	this.upsertMessage = function(message, language, refkey, opt) {
		var entries = this.getByKey(refkey),
			...
			x3lang = typeof(language) === "object" ? language.x3 : language;
		...
		entries.forEach(function(e, i) {
			...
			m.lanori = (x3lang === defaultLanguage.x3) ? "" : defaultLanguage.x3;
	...
	this.dumpToPlainFile = function(_, file, opt) {
		...
		this.traverse(function(context, chp, id, lan, text, meta) {
			...
			var line = lan + "\t" + chp + "\t" + id + "\t" + lanori + '\t' + (opt.quoted ? _quoted(txt) : txt);
			// if (txt) {
			contents += line + "\n";
			// }
			// txt += "[" + lan + ":" + chp + ":" + id + "] " + escapeSpecialChars(n[lan]) + "\n";
			if (["XXX", defaultLanguage.x3].indexOf(lan) === -1 && !exports.isEqualPlaceHolder(txt)) {
				var text0 = normalizeString(context.$id[defaultLanguage.x3], true);
				var re = /(\{[^\}]+\})/g,
					vars0 = [],
					vars1 = [],
					v;
				while ((v = re.exec(text0)))
					vars0.push(v[1]);
				while ((v = re.exec(txt)))
					vars1.push(v[1]);
				vars0.some(function(v) {
					if (vars1.indexOf(v) === -1) {
						if (txt) {
							...
							if (!err) {
								err = errors[errk] = [];
								err.push(defaultLanguage.x3 + "\t" + chp + "\t" + id + "\t\t" + (opt.quoted ? _quoted(text0) : text0));
							}
							err.push(line);
						}
						return true;
...
function Dictionnary() {
	...
	this.smartAssign = function(key, lan, text) {
		if (!key) return;
		var tmu = _tm[key] || (_tm[key] = new DictionnaryEntry(key)),
			mtl = tmu.getText(lan);
		// tmu.$count++;
		text = _cleanString(text);
		tmu.setText(lan, text);
	};
...
function DictionnaryEntry(key) {
	this.$key = key;
	this.$count = 0;
	this.text = {};
}

DictionnaryEntry.prototype = {
	getText: function(lan) {
		return this.text[lan];
	},
	setText: function(lan, text) {
		text = _cleanString(text);
		var mtl = this.text[lan];
		if (mtl !== undefined && text !== mtl && text !== this.$key) {
			// mtl.alt = mtl.alt || (mtl.alt = []);
			// mtl.alt.push(text);
			// console.error("  various translations for '" + this.$key + "' [" + lan + "] count=" + this.$count + "\n--- " + text + "\n+++ " + mtl);
			// various translations for '" + this.$key + "' [" + l + "] count=" + this.$count + "\n--- " + text + "\n+++ " + mtl);
		} else if (this.text[lan] == null) {
			this.text[lan] = text;
		}
	}
};

var dictionnary = exports.Dictionnary = new Dictionnary();