...
function get_tracer(fileName) {
	...
	return function(text) {
		var strn = text + "\n";
		fs.appendFileSync(internalFileName, new Buffer(strn, 'utf8'), options);
	};
}
...
function _getLanguages(_, x3endpoint) {
	...
	if (tabkey.length === 0) {
		throw new Error("TABLAN entities is empty, please fill the language association with X3 Code lang and ISO code (exemple : FRA, fr-FR)");
	} else {
		...
		function _checkEntry(_, key, message) {
			...
			if (seg && seg[key]) {
				...
				if (chp > chapterSyracuse + chapterBoundings) {
					...
					else instance.$addDiagnose('error', locale.format(module, "chapterFull", chapterSyracuse + chapterBoundings, "syracuse texts"));
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			...
			if (index[fname] && index[fname][key]) {
				...
				if (chp > chapterAdmin + chapterBoundings) {
					return _addDiagnose('error', locale.format(module, "chapterFull", chapterAdmin + chapterBoundings, "admin resources"));
				}
				if ((index.count + 1) % maxNbRecords === 0) id = maxNbRecords;
				else id = (index.count + 1) % maxNbRecords;

				if (val && (val.length > aplStdMaxLength)) incr = _injectLongMessage(_, endPoint, val, index.count, chapterAdmin, isoLangCode);
				else {
		...
		function _checkEntry(_, key, message, fpath) {
			...
			if (seg && seg[key]) {
				...
				if (chp > chapterDotnet + chapterBoundings) {
					return _addDiagnose('error', locale.format(module, "chapterFull", chapterDotnet + chapterBoundings, "dotnet resources"));
				}
