...
var maxNbRecords = 120; //2000 ;//1999 ;
...
function getX3Records(_, db, whereClause) {
	...
	while (inst = cursor.next(_)) {
		...
		if (!x3Records[inst.LANCHP(_)][inst.LANNUM(_)] && x3Chunks[inst.LANCHP(_)][inst.LAN(_)].indexOf(inst.LANNUM(_)) === -1) x3Records[inst.LANCHP(_)][inst.LANNUM(_)] = {};
		if (x3Records[inst.LANCHP(_)][inst.LANNUM(_)]) {
			...
			} else tracer && tracer("no match text: " + sys.inspect(text));
			rec[inst.LAN(_)] = text;
