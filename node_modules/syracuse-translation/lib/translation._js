...
/*var fsstream=fs.createWriteStream( "C:\\trace\\toto.txt", { flags: 'a+', encoding: "utf8",mode: "0666" });
    var stream=new streams.WritableStream(fsstream );   
    var tracer =stream.write ;
*/ 
...
function _createEntry(_, endpoint, contractUrl, chapter, id, message, ut) {
        ...
        function _checkEntry(_, key, message) {
            ...
            if (index[dirkey] && index[dirkey][key]) {
                ...
                if (x3message) {
                    ...
                    try {            
                        success = _createEntry(_, instance.endpoint(_), contractUrl, 10031, index[dirkey][key].id, message, unitTestFlg);
                    } catch (ex) {
                ...
                if (!(typeof message=="string" )){
                    console.log("**********Value is not a string ; Christie should fix the format") ;
                ...
                try {
                    ...
                    //self.context.request.headers,

                    var id = index.count + 1;
                    success = _createEntry(_, instance.endpoint(_), contractUrl, 10031, id, message, unitTestFlg);
                } catch (ex) {
                ...
                if (success){
                    ...
                    //console.log("index[dirkey]: " + sys.inspect(index[dirkey]));
                    ...
                    if (rec[lang]) {
                       tracer && tracer("lang: " + lang);
                        ...
                        if (lang != 'ENG')
                            ...
                            tracer && tracer("modifyed in X3 " + k + ' ' + key+' txt: '+index[k][key].text);
                            tracer && tracer("modifyed in X3 new: " + k + ' ' + key+' txt: '+rec[lang]);
       ...
       /* if (!index[fname])
        ...
        flows.eachKey(_, localizationRef, function (_, key, val) {
            ...
            if (x3message) {
                ...
                //self.context.request.headers,
                _createEntry(_, instance.endpoint(_), contractUrl, 10032, index[fname][key].id, val, unitTestFlg);
            }
        }
        else{
            tracer && tracer("inject message " + key);
            var id = index.count + 1 ;
                //self.context.request.headers,
               success= _createEntry(_, instance.endpoint(_), contractUrl, 10032, id, val, unitTestFlg);
