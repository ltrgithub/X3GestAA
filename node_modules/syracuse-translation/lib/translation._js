...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	updateEntry: function(_, el, entry) {
		...
		if (x3message && x3message === placeholders.equal) {
	...
	initFrame: function(_, category, msgkey, opt) {
		...
		languageMap = this.getLanguages(_).isoToX3;
	...
	saveRejected: function(_) {
		...
		if (contents) {
			...
			fs.writeFile(fsp.join(this.targetDir, "messages-rejected-" + stamp + ".txt"), contents, "utf8", _);
		}
	},
	...
	extractAdminResources: function(_, handler) {
		...
		function _extractAllLang(_, fpath, fname, fsModule) {
			var destfile = self.getAdminTranslationDestDir(fpath),
				...
				jsonContent = _filterJson(_, fullContent, _importfilters[basename]),
...
var _filterImport = {
	landingPage: function(data, proto) {
		var items = data.filter(function(item) {
			...
			return item.$type === "pageLayout" || (item.$type === "landingPage");
		});
