...
var categories = {
	...
	syracuse: new Category("Syracuse", "Syracuse resources", [10031, 10035], {
		$exclude: [/shadow-modules\//, /node_modules\/syracuse-core\/[^\/]+\/types\/resources\/(date|decimal)/, /node_modules\/[^\/]+\/dist\//]
	}),
			...
			Object.keys(g).forEach(function(ek) {
				...
				if (refText && dictionary) {
					Object.keys(x3ToIso).forEach(function(x3Lang) {
						...
						var message;
						cutText = texts[iso];
						if (cutText == null || messageStore.isEqualPlaceHolder(cutText)) {
							...
							if (dictionary) {
								...
								if (proposals && proposals.length > 0 && (message = proposals[0][0]) !== cutText) {
									...
									if (proposals.length === 1 || proposals[0][1] > proposals[1][1]) {
										...
										texts[iso] = message;
										fix.applied = true;
										// console.error("Autofix mismatching translation: " + x3Lang + " index:[" + cutText + "] vs. dic:[" + message + "](" + occurs + ") ref:[" + refText + "]");
										...
										message = texts[iso];
										fix.applied = false;
										console.error("Multiple proposal for mismatching translation: " + x3Lang + " [" + fixes.length + "] index:[" + cutText + "] vs. dic:[" + message + "](" + occurs + ") ref:[" + refText + "]");
									}
									fixes.push(fix);
									self.upsertMessage(message, x3Lang, refkey, defopt);
								}
