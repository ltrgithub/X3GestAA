	...
	records.forEach_(_, function(_, inst) {
		...
		if (x3Chunks[inst.LANCHP(_)][inst.LAN(_)].indexOf(inst.LANNUM(_)) === -1) {
			...
			while (ancestor) {
				...
				//text=ancestor.text ;
				ancestor = ancestors[ancestor.chapter][inst.LAN(_)][ancestor.id];
				tracer && tracer("ancestor" + sys.inspect(ancestor));
			}
		...
		function _checkEntry(_, key, message) {
			...
			if (seg && seg[key]) {
				...
				if (typeof message !== "string") {
					if (options && options.$diagnoses) options.$diagnoses.push({
						severity: 'error',
						message: locale.format(module, "incorrectType")
					});
					else instance.$addDiagnose('error', locale.format(module, "incorrectType"));
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			if (index[fname] && index[fname][key]) {
				...
				if (x3message !== "") {
					...
					_createEntry(_, instance.endpoint(_), chapterAdmin, el.id, val);
				}
			} else {
				tracer && tracer("inject message " + key);
				var id = index.count + 1;
				success = _createEntry(_, instance.endpoint(_), chapterAdmin, id, val);
				if (!success) if (options && options.$diagnoses) options.$diagnoses.push({
					severity: "error",
				...
				if (!index[fname] && success) index[fname] = {};
	...
	while (inst = cursor.next(_)) {
		if (!inst.LANNUM(_)) {
			...
			if (options && options.$diagnoses) {
				options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "incorrectRecord", inst.LANCHP(_), inst.LAN(_), inst.LANMES(_))
				});
		...
		var rec;
		if (!x3Records[inst.LANNUM(_)]) rec = x3Records[inst.LANNUM(_)] = {};
	...
	function extract(_, file) {
		...
		function _checkEntry(_, key, message, fpath) {
			...
			if (seg && seg[key]) {
				...
				if (typeof message !== "string") {
					instance.$addDiagnose('error', locale.format(module, "incorrectType"));
