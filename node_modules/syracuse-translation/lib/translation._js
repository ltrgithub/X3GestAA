...
var chapterDotnet = "10033";
var chapterAdmin = "10032";
var chapterSyracuse = "10031";
var fetchCountX3 = 10000;
var unusedTextString = "-----old text not used any more-----";

function trace(str) {
	var f = traceFilePrefix + "\trace_client.log";
	var strn = str + "\n";
	var options = {
		flag : 'a+'
	};
...
function _deleteAndCreateEntry(_, endPoint, key, text, chapter, lannum, newId) {
	
	// to use old update behavior with .net resources
	// _updateEntry(_, endPoint, key, text, chapter, lannum);
	...
	if (_hasErrors(_, msg)) {
		tracer && tracer("id: " + lannum);
		tracer && tracer("deleteEntry: diagnose after delete : " + sys.inspect(diags));
		// return false;
	} else {
		tracer && tracer("success");
	}
	*/
	// delete not supported by APLSTD, so make text unuseable
	_updateEntry(_, endPoint, key, unusedTextString, chapter, lannum);
	
	msg = entity.createInstance(_, db);
	msg.LANCHP(_, chapter);
	msg.LANNUM(_, newId);
	msg.LAN(_, "ENG");
	msg.LANMES(_, text);
	...
	msg.save(_);
	if (_hasErrors(_, msg)) {
		tracer && tracer("id: " + newId);
		tracer && tracer("createEntry: diagnose after save : " + sys.inspect(diags));
		// return false;
	} else {
		tracer && tracer("success");
	}
		...
		function _checkEntry(_, key, message) {
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				if (x3message) {
					...
					if (parsed[key] !== seg[key].text) {
						...
						if (x3message !== index[dirkey][key].text) {
							...
							_updateEntry(_, endPoint, key, parsed[key], chapterSyracuse, seg[key].id);
						}
					}
				} else {
					...
					try {
						success = _createEntry(_, instance.endpoint(_), chapterSyracuse, index[dirkey][key].id, message, unitTestFlg);
					} catch (ex) {
				...
				try {
					...
					success = _createEntry(_, instance.endpoint(_), chapterSyracuse, id, message, unitTestFlg);
				} catch (ex) {
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			if (index[fname] && index[fname][key]) {
				...
				if (x3message) {
					...
					_createEntry(_, instance.endpoint(_), chapterAdmin, index[fname][key].id, val, unitTestFlg);
				}
			} else {
				tracer && tracer("inject message " + key);
				var id = index.count + 1;
				success = _createEntry(_, instance.endpoint(_), chapterAdmin, id, val, unitTestFlg);
				if (!success)
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				if (x3message) {
					...
					if (message !== seg[key].text) {
						tracer && tracer("modifyed in Syracuse" + index[dirkey] + ' ' + index[dirkey][key]);
						if (x3message != unusedTextString && x3message !== index[dirkey][key].text) {
							tracer && tracer(" update conflict for " + dirkey + ":" + key);
							instance.$addDiagnose("warning", locale.format(module, "updateConflict", dirkey + ":" + key, index[key], x3message));
						} else {
							index[dirkey][key].text = message;
							var id = ++index.minIdx;
							_deleteAndCreateEntry(_, endPoint, key, message, chapterDotnet, seg[key].id, id);
							seg[key].id = id;
						}
					}
				} else {
					tracer && tracer("inject message " + dirkey + ":" + key);
					try {
						success = _createEntry(_, instance.endpoint(_), chapterDotnet, index[dirkey][key].id, message, unitTestFlg);
					} catch (ex) {
						console.log(" exception in createEntry: " + ex.stack);
						// console.log("index: "+JSON.stringify(index,null,2)) ;
						var fd = fs.open(indexpath, "w", "0666", _);
				...
				try {
					var id = ++index.minIdx;
					success = _createEntry(_, instance.endpoint(_), chapterDotnet, id, message, unitTestFlg);
				} catch (ex) {
					tracer && tracer(_, "exception in createEntry: " + ex.stack);
				}
				if (success) {
