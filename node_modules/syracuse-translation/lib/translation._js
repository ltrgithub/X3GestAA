...
function _createEntry(_, endpoint, contractUrl,headers,chapter, id, message,ut) {
 ...
 var data = {
        ...
        var entity = db.getEntity(_, "ZZAPLSTD", "$edit");
        ...
        function extract(_, file) {
            ...
            function _checkEntry(_, key, message) {
                ...
                if (x3message) {
                    ...
                    try{
                    _createEntry(_, instance.endpoint(_), contractUrl,self.context.request.headers,10031 ,index[dirkey][key].id, message,unitTestFlg);
                    } 
                    catch(ex){
                    ...
                    if (x3message) {
                       ...
                       // console.log("index["+fname+"]["+key+"].text :"+index[fname][key].text) ;
                        ...
                        tracer && tracer("inject message "  + key);
                         ...
                         _createEntry(_, instance.endpoint(_), contractUrl,self.context.request.headers, 10032 ,index[fname][key].id, val,unitTestFlg);
                    }
        });
    }
