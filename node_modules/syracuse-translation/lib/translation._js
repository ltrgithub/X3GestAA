...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	createEntry: function(_, chp, id, message, language, refkey) {
		...
		// this.store.setMessage(_, message, language, refkey);
	},
	...
	var handlers = {
		...
		"admin": {
			...
			save: function(file, data, base, language, _) {
				...
				var destfile = fsp.join(context.destImportDir, fsp.basename(file));
				this.saved[file] = destfile;
				data = _filterLocalizations(data.raw || data);
...
function _filterLocalizations(data) {
	var json = {}, $prototypes = data.$prototypes,
		items;
	if (!data.$prototypes || !data.$localization || !data.$items || data.$items.length === 0)
		return null;
	json.$prototypes = data.$prototypes;
	items = json.$items = [];
	json.$localization = data.$localization;

	// filter items
	_filterItemList(items, data.$items, $prototypes);
	return json;
}

function _filterItemList(items, collec, proto) {
	if (!Array.isArray(collec))
		return;
	collec.forEach(function(e) {
		var p = e.$type && proto[e.$type] || proto;
		if (p) {
			e = _shrinkItem(e, p);
			e && items.push(e);
		}
...
function _shrinkItem(e, proto) {
	var key = proto.$key,
		localized = proto.$localized || [],
		min = {
			$type: e.$type
		}, items;
	...
	localized.forEach(function(p) {
		min[p] = e[p];
	});

	forEachKey(proto, function(pk, pv) {
		if (pk[0] === "$") return;
		if (typeof(pv) === "object") {
			items = [];
			_filterItemList(items, e[pk], pv);
			if (items.length > 0)
				min[pk] = items;
		}
	});

	// hack for landingPage
	if (e.$type === "pageLayout") {
		min.content = e.content;
		min.page = e.page;
		return min;
	} else if (Object.keys(min).length > 2)
		return min;
}
