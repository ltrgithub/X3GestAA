...
exports.extractAdminResources = function(_, instance, options, context) {
	...
	function _extract(_, fpath, fname) {
		...
		var localizationRef;
		if (localization) localizationRef = localization['en-us'];
		if (!localizationRef) {
			if (options && options.$diagnoses) {
				options.$diagnoses.push({
					severity: "error",
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			if (index[fname] && index[fname][key]) {
				...
				if (x3message !== "") {
					...
					if (localizationRef[key] !== index[fname][key].text) {
						...
						if (x3message !== index[fname][key].text) {
							...
							var chunks = [];
							var nb = _updateEntry(_, endPoint, key, localizationRef[key], chapterAdmin, chapterAdmin, el.id, chunks, index.count);
							index.count += nb;
						}
					}
				} else {
				...
				if (val.length > maxLength) 
					incr = _injectLongMessage(_, instance.endpoint(_), val, index.count, chapterAdmin);
				else {
					success = _createEntry(_, instance.endpoint(_), chp, id, val);
					if (success) 
						incr += 1;
					else {
						if (options && options.$diagnoses) 
