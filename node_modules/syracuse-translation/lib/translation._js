...
function TranslationProcess(_, context, options) {
	...
	this.rootDir = fsp.join(config.system.root, this.options.dir || "");
	this.targetDir = this.options.indexdir || fsp.join(config.system.root, "translation-indexes");
...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	endExtract: function(_) {
		...
		this.appendToMessagesfile(_, $chapter);
	},

	appendToMessagesfile: function(_, $chapter) {
		this.store.appendTofile(_, $chapter);
	},
	...
	tryParseJsonFile: function(_, file, opt) {
		try {
			...
			return this.diagnosis.fatal(localizedText("incorrectJsonFormat", file), e.safeStack, _);
		}
	},
	...
	extractRes: function(_, handler) {
		try {
			...
			this.diagnosis.fatal(e.message || e, e.safeStack, _);
		}
	},
	...
	extractAdminResources: function(_, opt) {
		...
		function _extract(_, localizationRef, fpath, language) {
			...
			Object.keys(localizationRef).some_(_, function(_, key) {
				if (options.extractFilter && !options.extractFilter(fpath, key, lang, localizationRef[key]))
					return false;
				if (!options.extractFilter && !refKeys[key]) {
		...
		var frame = this.initFrame(_, categories.admin, "startExtractResources", opt),
			...
			path = options.exportFile ? options.dir || fsp.join(__dirname, "../../") : fsp.join(config.system.root, "import");

		if (opt.landingPageTabOnly) {
			include = ["x3-erp-homepages.json"];
			options.extractFilter = function(file, key, lan, value) {
				tracer && tracer("extractFilter: " + key.substring(0, 11));
				return key.substring(0, 11) === "pageLayout.";
			};
		}
				...
				flows.eachKey(_, updateCtx, function(_, lang, src) {
					...
					if (src.dirty) {
						try {
							...
							diagnosis.fatal(localizedText("fileWritingError", src.file), ex.safeStack, _);
						}
					}
				...
				if (rec) {
					...
					Object.keys(langMap).sort(function(a, b) {
						...
						if (targetText) {
							...
							if (lang !== "ENG" && refText === targetText) {
...
function indexDir(options) {
	return options && options.indexdir || fsp.join(config.system.root, "translation-indexes");
}
