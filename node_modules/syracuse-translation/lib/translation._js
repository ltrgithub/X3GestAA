...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	extractAdminResources: function(_, handler) {
		...
		var frame = this.initFrame(_, categories.admin, "startExtractResources", opt),
			index = frame.index,
			options = frame.options,
			exclude = categories.admin.$exclude || [],
								...
								if (src.exists && indexText && sourceText) {
									if (!messageStore.isSimilar(sourceText, indexText) && !messageStore.isEqualPlaceHolder(indexText)) {
										if (messageStore.isSimilar(targetText, sourceText)) {
											...
											if (conflictFix == null && !messageStore.isSimilar(targetText, indexText)) {
...
function _checkEntry(_, translation, resources, index, grpkey, language) {
	...
	return function(_, key) {
		...
		tracer && tracer("_checkEntry: key=" + grpkey + ':' + key + ", language=" + language + "message='" + resources[key] + "', indexEntry=" + sys.inspect(el || {}));
	...
	var handlers = {
		"syracuse": {
			...
			matchGroup: function(group) {
				return ((this.category.$exclude || []).indexOf(group) === -1);
			},
		},
		"dotnet": {
...
var adminIncludes = transCfg.include || [ //
	// "x3-erp-menus.json", ERP menus translation are imported from X3 menu profile
	"x3-erp-homepages.json", //
	"x3-erp-sitemap.json", //
	"x3-global-sitemap.json", //
	"x3-global-modules.json", //
	"x3-init.json",
	"syracuse-admin-templates.json", //
	// x3-pages.json

	"x3-mobile-dashboards.json"
	// "x3-hrm-menus.json", // HRM menus translation are imported from X3 menu profile
];
...
// var traceFilter = console.error;
var traceFilter;

function _simplifyImport(_, proto, data, localization) {
	var db = currentWorld.getCollaborationOrm(_);
	var model = db.model;
	var _pointerTypes = {};
	var items = [];
...
function _simplifyItem(_, entity, item, itemProto, localization, _pointerTypes) {
	...
	traceFilter && traceFilter("_simplifyItem: item=" + sys.inspect(item) + "\nentity=" + sys.inspect(entity));
	itemProto = itemProto || {};
	flows.eachKey(_, item, function(_, key, value) {
		...
		traceFilter && traceFilter("_simplifyItem: key=" + key);

		var relMeta, propMeta;
		if (entity.$relations && (relMeta = entity.$relations[key])) {
			// relation
			traceFilter && traceFilter("_simplifyItem relation: key=" + key + ", value=" + sys.inspect(value));
			var relProto = itemProto[key];
			if (relMeta.isPlural) {
				traceFilter && traceFilter("_simplifyItem: key=" + key + ", value=" + sys.inspect(value));
				traceFilter && traceFilter("entity=" + sys.inspect(entity));
				traceFilter && traceFilter("itemProto=" + sys.inspect(itemProto));
			...
			// traceFilter && traceFilter("entity=" + sys.inspect(entity));
			// traceFilter && traceFilter("itemProto=" + sys.inspect(itemProto));
			// traceFilter && traceFilter("propMeta=" + sys.inspect(propMeta));
...
function _simplifyCollection(_, entity, relName, relMeta, relProto, value, localization, _pointerTypes) {
	...
	// traceFilter && traceFilter("_simplifyCollection: name=" + relName + ", relMeta=" + JSON.stringify(relMeta) + //
