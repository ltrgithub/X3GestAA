			...
			Object.keys(g).forEach_(_, function(_, ek) {
				...
				if (texts) {
					if (texts[frCulture.iso] == null) {
						...
						diagnosis.logInfo("Create default value in index for " + gk + "/" + ek + "/" + frCulture.iso, _);
					}
					Object.keys(texts).forEach_(_, function(_, lk) {
						if (lk !== frCulture.iso && lk !== defaultLanguage.iso && texts[lk] === placeholders.equal) {
							store.setToDefault(e.chp, e.id, langMaps.isoToX3[lk]);
							diagnosis.logInfo("Create default value in store for [" + e.chp + "/" + e.id + "/" + langMaps.isoToX3[lk] + "] " + gk + "/" + ek + "/" + lk, _);
						}
					});
				}
			});
