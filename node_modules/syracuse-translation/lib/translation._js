...
function _findMessage(_, coll,   lanchp, lannum , chunks) {
	...
	/*if ( lannum===5){
	coll.forEach_(_,function(_, inst){
				tracer && tracer ("coll inst: "+JSON.stringify(inst.serializeInstance(_),null,2) );
				});
				}*/
		...
		function _checkEntry(_, key, message) {
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				if (x3message!=="") {
					...
					id=index[dirkey][key].id ;
					try {
						//var chp=chapterSyracuse+Math.floor((index.count+1)/maxNbRecords) ;
						
						success = _createEntry(_, instance.endpoint(_), index[dirkey][key].chp, id, message);
						if (success)
							incr+=1 ;
					} catch (ex) {
...
exports.updateResources = function(_, instance, options, context) {
	function updFile(_, k) {
		...
		function saveData(_, rec, dataBuffers, filePaths /*, fileHandlers*/) {
					...
					//tracer && tracer("save data fileHandlers: " + JSON.stringify(fileHandlers,null,2));
		...
		function _checkEntry(_, key, message, fpath) {
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				if (x3message!=="") {
					...
					try {
					//DARY
						chp=chapterDotnet+Math.floor((index.count+1)/maxNbRecords) ;
						success = _createEntry(_, instance.endpoint(_), chp, index[dirkey][key].id, message);
						if (success)
							incr+=1 ;
					} catch (ex) {
						console.log(" exception in createEntry: " + ex.stack);
				...
				if (message.length > maxLength) {
					...
					for (i=0; i<nbChunks; i++){
						...
						chp= chapterDotnet+Math.floor((index.count+nbCreatedChuncks+1)/maxNbRecords) ;
						success = _createEntry(_, instance.endpoint(_), chp, id, chunk);
						if (success)
							nbCreatedChuncks+=1 ;
							//incr+=1 ;
						if (id===maxNbRecords)
					...
					try {
						//var id = ++index.minIdx;
						if ((index.count+ 1) <=maxNbRecords){
							id = (index.count+1) ;
							chp=chapterDotnet ;
						}
						else if  ((index.count+ 1) >maxNbRecords)
						{
							id = (index.count+ 1)%maxNbRecords ;
							chp= chapterDotnet+Math.floor((index.count+incr+1)/maxNbRecords)+1 ;
						}
