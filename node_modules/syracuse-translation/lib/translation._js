...
function readIndex(_,indexpath,diags){
...
function getX3Records(_, db, whereClause) {
	...
	records.forEach_(_, function(_, inst) {
		...
		if (options && options.$diagnoses) options.$diagnoses.push({
								severity: "warning",
								message: locale.format(module, "incorrectRecord", inst.LANMES(_))
							});
							else
							 instance.$addDiagnose("warning", locale.format(module, "incorrectRecord", inst.LANMES(_)));
							return ;
...
function _updateEntry(_, endPoint, key, text, firstChapter, chp, lannum, chunks, idxcount) {
...
exports.extractResources = function(_, instance, options, context) {
			...
			if (seg && seg[key]) {
				...
				if (x3message !== "") {
					...
					if (options && options.$diagnoses) options.$diagnoses.push({
						...
						instance.$addDiagnose('error', locale.format(module, "indexMess", dirkey, key));
				}
				...
				if (chp>chapterSyracuse+chapterBoundings) {
					...
					instance.$addDiagnose('error',module, "chapterFull", chapterSyracuse+chapterBoundings, "syracuse texts");
					
					return ;
				}

				if (message.length > maxLength) incr = _injectLongMessage(_, instance.endpoint(_), message, index.count, chapterSyracuse);
				else {
	...
	var index= readIndex(_,indexpath , diags ) ;
...
//Syracuse->X3
exports.extractAdminResources = function(_, instance, options, context, isoLangCode) {
	...
	function _extract(_, fpath, fname) {
		...
		try {
			...
			else instance.$addDiagnose('error', locale.format(module, "incorrectJsonFormat", fpath));
			return ;
		}
		
		var localizationRef = jsonContent.$localization && jsonContent.$localization[isoLangCode];
		if (!localizationRef) {
			if (options && options.$diagnoses) {
				options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "noLocalizationRef", fname)
				});

			} else instance.$addDiagnose('error', locale.format(module, "noLocalizationRef", fname));
			return;
		}
			...
			if (index[fname] && index[fname][key]) {
				...
				if (x3message !== "") {
					...
					if (localizationRef[key] !== index[fname][key].text) {
						...
						if (x3message !== index[fname][key].text) {
							...
							if (options && options.$diagnoses) options.$diagnoses.push({
								severity: "warning",
								message: locale.format(module, "updateConflict", fname + ":" + key, x3message)
							});
							else instance.$addDiagnose("warning", locale.format(module, "updateConflict", fname + ":" + key, x3message));
						} else {
							tracer && tracer("update entry");
					...
					if (options && options.$diagnoses) options.$diagnoses.push({
						severity: 'error',
						message: locale.format(module, "indexMess", fname, key)
					});
					else 
						instance.$addDiagnose('error', locale.format(module, "indexMess", fname, key));
				...
				if (chp>chapterAdmin+chapterBoundings) {
					if (options && options.$diagnoses) 
						options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "chapterFull", chapterAdmin+chapterBoundings,"admin resources")
				});
				else 
					instance.$addDiagnose('error',module, "chapterFull", chapterAdmin+chapterBoundings, "admin resources");
				...
				if (val.length > maxLength) 
					incr = _injectLongMessage(_, instance.endpoint(_), val, index.count, chapterAdmin, isoLangCode);
				else {
					success = _createEntry(_, instance.endpoint(_), chp, id, val, isoLangCode);
					if (success) 
						incr += 1;
					else {
						if (options && options.$diagnoses) 
							options.$diagnoses.push({
							severity: "error",
							message: locale.format(module, "createEntryfail", key, fname)
							});
						else 
	...
	if (!isoToX3LangMap[isoLangCode]) return instance.$addDiagnose("error", "unknownLangMap", isoLangCode);
	//
	var self = this;
	self.context = context;
	...
	var whereClause = "LANCHP ge " +chapterAdmin+ " and LANCHP le "+ (chapterAdmin + 4)+ " and LANNUM ne 0 and LAN eq \"" + isoToX3LangMap[isoLangCode] + "\"";
	...
	var indexpath = fsp.join(indexdir, "indexAdmin" + isoLangCode + ".json");
	var diags = options && options.$diagnoses ;
	var index = readIndex(_, indexpath, diags) ; 
	if (!index) return ;
...
exports.extractDotnetResources = function(_, instance, options, context) {
			...
			if (seg && seg[key]) {
				...
				if (chp>chapterDotnet+chapterBoundings) {
					if (options && options.$diagnoses) 
						options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "chapterFull", chapterDotnet+chapterBoundings,"dotnet resources")
				});
				else 
					instance.$addDiagnose('error',module, "chapterFull", chapterDotnet+chapterBoundings, "dotnet resources");
	...
	var indexdir;
	if (options.dir) dir = fsp.join(__dirname, "../../", options.dir);
	else dir = fsp.join(__dirname, "../../../dotnet");
