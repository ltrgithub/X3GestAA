...
var categories = {
	...
	syracuse: new Category("Syracuse", "Syracuse resources", [10031, 10035], {
		$exclude: [/node_modules\/syracuse-core\/[^\/]+\/types\/resources\/(date|decimal)/, /node_modules\/[^\/]+\/dist\//]
	}),
...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	fillDefault: function(_) {
		...
		var dictionary = this.options.dictionary || messageStore.dictionary;
			...
			Object.keys(g).forEach(function(ek) {
				...
				if (refText && dictionary) {
					Object.keys(x3ToIso).forEach(function(x3Lang) {
						...
						if (texts[iso] == null || messageStore.isEqualPlaceHolder(texts[iso])) {
							var message = dictionary.getText(refText, x3Lang);
							// if (!message)
...
function _checkEntry(_, translation, resources, index, grpkey, language) {
	...
	return function(_, key) {
		...
		if (el) {
			...
			if (options.rebuild) {
				var proposals, refText = texts[defaultLanguage.iso],
					dicText;
				if (dictionary) {
					proposals = dictionary.get(refText, language.x3);
					if (proposals && proposals.length > 0 && (dicText = proposals[0][0]) !== message) {
						var arity = proposals.map(function(p) {
							return p[1];
						});
						if (proposals.length === 1 || proposals[0][1] > proposals[1][1] || dicText.split(/\s/).length > 1) {
							// use dic value
							texts[isoLang] = entry.val = dicText;
							console.error("autofix mismatching translation: " + language.x3 + " source:[" + message + "] vs. dic:[" + dicText + "](" + arity + ") ref:[" + texts[defaultLanguage.iso] + "]");
						} else {
							translation.multipleProposal = (translation.multipleProposal || 0) + 1;
							console.error("multiple proposal for mismatching translation: " + language.x3 + " [" + translation.multipleProposal + "] source:[" + message + "] vs. dic:[" + dicText + "](" + arity + ") ref:[" + texts[defaultLanguage.iso] + "]");
						}
					}
				}
			}
