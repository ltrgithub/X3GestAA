...
function getX3Records(_, db, whereClause) {
	...
	records.forEach_(_, function(_, inst) {
		...
		if (!inst.LANNUM(_)) {
			...
			else instance.$addDiagnose("warning", locale.format(module, "incorrectRecord", inst.LANMES(_)));
			return;
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			if (index[fname] && index[fname][key]) {
				...
				if (x3message !== "") {
					...
					else instance.$addDiagnose('error', locale.format(module, "indexMess", fname, key));
				...
				if (chp > chapterAdmin + chapterBoundings) {
					...
					else instance.$addDiagnose('error', module, "chapterFull", chapterAdmin + chapterBoundings, "admin resources");
				...
				if (val.length > maxTotalLength) incr = _injectLongMessage(_, instance.endpoint(_), val, index.count, chapterAdmin);
				else {
					...
					if (success) incr += 1;
						...
						else instance.$addDiagnose('error', locale.format(module, "createEntryfail", key, fname));
						return;
					}
				}
		...
		function _checkEntry(_, key, message, fpath) {
			...
			if (seg && seg[key]) {
				...
				if (chp > chapterDotnet + chapterBoundings) {
					...
					else instance.$addDiagnose('error', module, "chapterFull", chapterDotnet + chapterBoundings, "dotnet resources");
