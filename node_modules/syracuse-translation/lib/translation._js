					...
					if (parsed[key] !== seg[key].text) {
						...
						if (x3message !== index[dirkey][key].text) {
							...
							if (options && options.$diagnoses)
								options.$diagnoses.push({ severity: "warning", message: locale.format(module, "updateConflict", dirkey + ":" + key, message, x3message) });
							else
								instance.$addDiagnose("warning", locale.format(module, "updateConflict", dirkey + ":" + key, message, x3message));
						} else {
...
exports.extractAdminResources = function(_, instance, options, context) {
 //tracer=trace_extract_admin ;
				...
				if (x3message) {
					...
					if (localizationRef[key] !== index[fname][key].text) {
						...
						if (x3message !== index[fname][key].text) {
							...
							if (options && options.$diagnoses)
								options.$diagnoses.push({ severity: "warning", message: locale.format(module, "updateConflict", key, localizationRef[key], x3message) });
							else
	...
	var index;
	options = options || {};
	console.log("opts: "+sys.inspect(options)) ;
