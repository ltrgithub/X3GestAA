...
exports.deleteAll = function (_, instance, options, context) {
    ...
    var ent = db.getEntity(_, "ZZAPLSTD", "$query");
    var x3Records = db.fetchInstances(_, ent, {
...
exports.extractResources = function (_, instance, options, context) {
    ...
    function _createEntry(_, dirkey, key, message) {
        ...
        var putBody = put(_, self.context.request.headers, contractUrl + "/$workingCopies('" + trackingId + "')?representation=ZZAPLSTD.$edit", {
            lanchp: 10031,
        ...
        var savBody = put(_, self.context.request.headers, contractUrl + "/$workingCopies('" + trackingId + "')?representation=ZZAPLSTD.$edit", {
        ...
        console.log("RESULT: " + JSON.stringify(savBody, null, 2));
        ...
        function _checkEntry(_, key, message) {
            ...
            if (x3message) {
                ...
                if (parsed[key] !== index[dirkey][key].text) {
                    tracer && tracer("modifyed in Syracuse" + idxkey);
