...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	// 	// 	}
	// 	// }
	// 	return this.upsertMessage(entry.val, entry.language, entry.$key);
	...
	extractAdminResources: function(_, opt) {
		...
		if (opt.filterAdmin) {
			extractFilters = _extractFilters;
		}
	...
	update: function(_, handler) {
		...
		Object.keys(index).filter(function(k) {
			return k[0] !== "$" || handler && handler.match && handler.match(k);
		}).forEach_(_, updFile);
		this.endUpdate(_);
	},
...
function _getLanguage(isolang) {
	return languageMap[isolang] || (languageMap[isolang] = {
		iso: isolang,
		x3: locale.x3Language(isolang, true)
	});
}
...
var _filterExtract = {
	landingPage: function(json) {
		...
		items.forEach(function(item) {
			...
			if (item.$type === "landingPage" && item.$factoryOwner === "SAGE") {
	...
	sitemap: function(json) {
		...
		items.forEach(function(item) {
			...
			if (item.$type === "menuItem" && item.linkType === "$representation" && item.$factoryOwner === "SAGE") {
				...
				if (item.code) menuBlocks[item.code] = item;
			}
		});
		menuModules && menuModules.forEach(function(m) {
			...
			(mb.items || []).forEach(function(e) {
				if (e.$type === "menuSubblock") {
					if (e.title) keys[e.title] = mb;
					if (e.description) keys[e.description] = mb;
				}
			});
		});
		menuBlocks = null;
	...
	var handlers = {
		...
		"admin": {
			...
			load: function(_, fullName, language) {
				...
				if (!data) {
					data = context.tryParseJsonFile(_, fullName),
					tracer && tracer("[LOAD] Simplify: " + fullName);
					data = _filterLocalizations(_, data, _importfilters[fsp.basename(fullName)]);
			...
			match: function(file) {
				...
				return adminIncludes.indexOf(name) >= 0;
			},
		}
	};
...
function _filterLocalizations(_, data, filter) {
...
var _filterImport = {
	landingPage: function(data, proto) {
		return data.filter(function(item) {
			var isAdmin = _isAdmin(item);
			return item.$type === "pageLayout" || (item.$type === "landingPage" && isAdmin);
		});
	},
	sitemap: function(data, proto) {
		var items = [],
			menuBlocks = {},
			menuModules = [];
		...
		data.forEach(function(item) {
			...
			// 	console.log("menuBlock: " + item.code);
...
var _extractFilters = {
	"x3-erp-homepages.json": _filterExtract.landingPage,
	"x3-erp-sitemap.json": _filterExtract.sitemap,
	"x3-hrm-sitemap.json": _filterExtract.sitemap,
	"syracuse-collaboration-sitemap.json": _filterExtract.sitemap,
	"syracuse-support-sitemap.json": _filterExtract.sitemap,
};

var _importfilters = {
	"x3-erp-homepages.json": _filterImport.landingPage,
	"x3-erp-sitemap.json": _filterImport.sitemap,
	"x3-hrm-sitemap.json": _filterImport.sitemap,
	"syracuse-collaboration-sitemap.json": _filterImport.sitemap,
	"syracuse-support-sitemap.json": _filterImport.sitemap,
};

function _filter(fpath, opt) {
	opt = opt || {};
	var extractFilters = opt.extractFilters || _extractFilters,
		name = fsp.basename(fpath);
	return (transCfg.extractFilters && transCfg.extractFilters[name]) || extractFilters[name] || extractFilters["*"];
}
