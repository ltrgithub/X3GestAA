...
var transHelpers = require('./helpers');
var localizedText = transHelpers.localizedText;
var _normalizeGroup = transHelpers.normalizeGroup;
var _getMapping = transHelpers.getMapping;
var _walk = transHelpers.walk;
...
if (log.warn && !log.warning) log.warning = log.warn;
...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	stopTiming: function(name) {
		if (this.timings[name]) {
			this.timings[name].t1 = Date.now();
			this.timings[name].elapsed = this.timings[name].t1 - this.timings[name].t0;
			console.log("[TIMING]", name, this.timings[name].elapsed, "ms");
	...
	initFrame: function(_, category, msgkey, opt) {
		...
		indexes = readIndex(_, indexpath, category, frame.options);
		index = indexes.index;
		...
		frame.masterIndex = indexes.masterIndex;
		frame.id2key = {};

		var mkeys = frame.masterIndex && frame.masterIndex.keys;
		forEachKey(mkeys, function(mk, mv) {
			frame.id2key[mv.uid || mv.gid] = mk;
		});
		return this.frame;
	},
	...
	updateAll: function(_) {
		...
		try {
			...
			["syracuse"].forEach_(_, function(_, resType) {
				resourcesOpts[resType] && !skip[resType] && self.updateUsingTree(_, resourceHandler(_, resType, self, opt));
			});
			["dotnet", "admin"].forEach_(_, function(_, resType) {
				resourcesOpts[resType] && !skip[resType] && self.update(_, resourceHandler(_, resType, self, opt));
			});
		} finally {
			this.stopTiming("updateAll");
		}
	},
	...
	update: function(_, handler) {
		...
		}).forEach_(_, function(_, k) {
			self.updateFile(_, k, context);
		});
		this.endUpdate(_);
	},

	updateFile: function(_, k, context) {
		var fpath = k,
			id = k;
		var diagnosis = context.diagnosis,
			handler = context.handler,
			x3RecordSet = context.x3RecordSet,
			frame = context.frame,
			index = context.index,
		...
		var opt = handler.opt,
			...
			masterIndex = frame.masterIndex,
			grp = groups && groups[id] || {};

		if (context.hint === "tree") {
			fpath = grp.path;
			if (!fpath) return tracer && tracer("updateFile no path, skip id:" + id);
			if (!grp.lang) return tracer && tracer("updateFile no 'en' or 'en-us' resource file, skip id:" + id);
		}

		diagnosis.logInfo(localizedText("updateTextsOf", handler.desc, fpath));
		log.info && log.info("updateFile: " + fpath + ", " + id);

		function saveData(_) {
			tracer && tracer("save data updateCtx=" + sys.inspect(updateCtx));
			flows.eachKey(_, updateCtx, function(_, lang, src) {
				log.silly && log.silly("save data src of lang " + lang + ": " + JSON.stringify(src, null, 2));
				if (!fs.exists(fsp.dirname(src.file), _)) {
					log.info && log.info("Folder : " + fsp.dirname(src.file) + " does not exist");
					return;
				}
				...
				if (src.dirty && !data.saved) {
					if (resources && Object.keys(resources).length > 0) {
						try {
							log.silly && log.silly("write to file: " + src.file + " content: " + JSON.stringify(data));
						...
						log.info && log.info("delete file: " + src.file + " exists: " + src.exists);
		...
		function updData(_, key) {
			var el = segment[key],
				content, resources;
			log.silly && log.silly("updData: " + key + ' ' + JSON.stringify(el, null, 2));
			...
			if (rec && rec.text) {
				...
				if (!ref && !ignoreChecksum) {
					return diagnosis.fatal(localizedText("missingReference", fpath, key), null);
				}
				Object.keys(langMap).sort(refLangSorter(refLang)).forEach_(_, function(_, lang) {
					...
					if (targetText) {
						...
						if (lang !== refLang) {
							...
							if (equalsToRefLang) {
								tracer && tracer("Text equal to default ENG " + fpath + "/" + key + "[" + lang + "] value=" + targetText);
						...
						if (refText == null) {
							...
							tracer && tracer("Modified in X3 " + fpath + "/" + key + "[" + lang + "] old value=" + indexText + " new value=" + targetText);
							...
							if (src.exists && indexText && sourceText) {
								if (!messageStore.isSimilar(sourceText, indexText) && !messageStore.isEqualPlaceHolder(indexText)) {
									if (messageStore.isSimilar(targetText, sourceText)) {
										tracer && tracer("update index " + fpath + "/" + key + ": '" + el.text[isolang] + "' => '" + targetText + "'");
									} else if (lang === refLang) {
										diagnosis.error("Reference language text has been modified: " + rec.$key + " - " + fpath + "/" + key + " - " +
											e0.$origin + "/" + lang + ", s=" + sourceText + ", t=" + targetText + ", i=" + indexText);
										targetText = null;
									} else {
										tracer && tracer("update conflict for " + fpath + "/" + key + "[" + lang + "]\ns=" + sourceText + "\nt=" + targetText + "\ni=" + indexText);
										if (conflictFix == null && !messageStore.isSimilar(targetText, indexText) && !useTheir) {
											diagnosis.error(localizedText("updateConflict", rec.$key + " - " + fpath + "/" + key, e0.$origin + "/" + lang, sourceText, targetText, indexText));
											targetText = null;
										}
									}
								}
						...
						if (targetText) {
							if (conflictFix != null) {
								...
								tracer && tracer("force value of " + fpath + "/" + key + "[" + isolang + "]' to '" + targetText + "'");
							}
							...
							if (targetText != sourceText) {
								// console.error("update " + fpath + "/" + key + "[" + isolang + "]'\n  from '" + sourceText + "'\n  to   '" + targetText + "'")
								frame.updated++;
							}
						}
					}
		...
		var base = fpath.replace(/\//g, fsp.sep),
			updateCtx = {},
			segment = index[id];
		tracer && tracer("index[" + id + "]=" + JSON.stringify(segment, null, 2));

		if (!segment) {
			console.error("*** TODO: group index for ", id);
			return;
		}
	...
	updateUsingTree: function(_, handler) {
		function loadRessources(_, file, lang) {
			return handler.load(_, file, _getLanguage(lang)).resources;
		}

		function addMissingKeys(_, g, gn) {
			try {
				var idxgrp, idxel;
				idxgrp = index[gn] || (index[gn] = {
					$key: frame.id2key[gn] || sha1(gn)
				});
				if (!g.file) {
					return console.error("[FATAL] Cannot add missing key. no file for:", gn);
				}
				var data = loadRessources(_, g.file, g.lang);
				if (g.lang != "en" && g.lang != "en-us") {
					return console.error("[FATAL] Cannot add missing key. bad language:", g.lang, g.path);
				}
				if (!data) {
					return console.error("[FATAL] Cannot add missing key. no data:", g.lang, g.path);
				}
				// traverse all localized properties and add them in index if necessary
				Object.keys(data).forEach(function(k) {
					...
					if (!idxel) {
						idxel = idxgrp[k] = {
							...
							$key: frame.id2key[gn + "/" + k] || sha1(idxgrp.$key + "/" + k)
						};
						console.error("[ADD] Missing index property:", gn + "/" + k);
					}
				});
			} catch (e) {
				return console.error("[FATAL] Cannot add missing key. error:", e.message, e.stack);
			}
		}

		try {
			this.startTiming("updateUsingTree");
			var diagnosis = this.diagnosis,
				opt = handler.opt,
				ignoreChecksum = opt.ignoreChecksum,
				self = this,
			...
			tracer && tracer("updateUsingTree: rootDir=" + this.rootDir + ", rootpath=" + handler.rootpath);

			var visitor, rootpath = handler.rootpath || this.rootDir;
			// Collect files to proceed with
			this.startTiming("updateUsingTree.collect");
			_walk(_, rootpath, visitor = {
				init: function(_) {
					this.$groups = {};
					this.$skipped = [];
					this.$uniqueKeys = {};
				},
				visitDir: function(_, file, path, level) {
					var relpath = path.substring(this._rootDir.length + 1);
					var seg = relpath.split("/"),
						n = seg.length;
					if (file[0] === ".") {
						return false;
					}
					if (seg[0] != "" && seg[0] != "node_modules" && seg[0] != "shadow-modules") {
						return false;
					}

					if (seg[0] === "shadow-modules" && n > 1) {
						if (seg[1] != "win32-x64-v8-5.0" || (n > 2 && seg[2] != "node_modules") || (n > 3 && seg[3] != "@sage") || (n > 5 && seg[5] != "src")) {
							// console.error("===> skip shadow-modules:", relpath);
							this.$skipped.push(path);
							return false;
						}
					}

					if (/^(dist|deps|test|fixtures)$/.test(file)) {
						// console.error("===> skip: " + relpath);
						this.$skipped.push(path);
						return false;
					}
					if (/node_modules\/.+?\/node_modules\//.test(relpath)) {
						// console.error("skip: " + path);
						// console.error("===> skip node_modules:", relpath);
						this.$skipped.push(path);
						return false;
					}

					if (/node_modules\/(@sage\/)?etna/.test(relpath)) {
						this.$skipped.push(path);
						return false;
					}

					// console.error("===> visit folder:", relpath);
					return true;
				},
				visitFile: function(_, file, path, level) {
					// console.error("visitFile: " + path + "/" + file);
					var relpath = path.substring(this._rootDir.length + 1);
					var seg = relpath.split("/"),
						n = seg.length;
					if (seg[n - 2] != "resources" || !/\.json$/.test(file)) {
						return;
					}
					var m = /(.+?)-(\w\w(-\w\w)?)\.json$/.exec(file);
					// m && console.error("  ===> visit group:", m[1], m[2]);
					var lang = m && m[2] && m[2].toLowerCase();
					var g, gn, gpath;
					if (lang) {
						gpath = seg.slice(0, -1).join("/") + "/" + m[1];

						if (/^(date|decimal)-/.test(file) && /node_modules\/(@sage\/)?syracuse-core\/[^\/]+\/types\//.test(gpath)) {
							this.$skipped.push(path);
							return false;
						}
						gn = _normalizeGroup(gpath);
						// console.error("  ===> visit group:" + gn + " >>>", gpath, lang);
						// console.error("  ===> ok");

						g = this.$groups[gn] || (this.$groups[gn] = {
							path: gpath
						});
						if (lang === "en-us" || lang === "en") {
							if (g.lang)
								log.warn("default language ambiguity: " + lang + ", current=" + g.lang);
							g.lang = lang;
							g.file = relpath;
						}
						if (g.path !== gpath)
							console.error("duplicate keys in index:\n  " + m[1] + "\n  " + g.path);
						this.$maxGroupKeyLength = gn.length > ~this.$maxGroupKeyLength ? gn.length : ~this.$maxGroupKeyLength;
						this.$resCount = (this.$resCount || 0) + 1;
					}
				},
			});
			this.stopTiming("updateUsingTree.collect");

			// console.error("groups:\n", Object.keys(visitor.$groups).join("\n"));
			console.error("groups:\n", Object.keys(visitor.$groups).length);
			// console.error(visitor.$groups);
			console.error("count:", visitor.$resCount);
			// console.error("skipped:", visitor.$skipped.length, "\n", visitor.$skipped);
			console.error("skipped:", visitor.$skipped.length);
			// console.error(visitor.$skipped);
			console.error("maxLen:", visitor.$maxGroupKeyLength);

			var groups = visitor.$groups;
			var context = {
				diagnosis: diagnosis,
				handler: handler,
				langMap: langMap,
			...
			Object.keys(groups).forEach_(_, function(_, k) {
				addMissingKeys(_, groups[k], k);
				self.updateFile(_, k, context);
			});
			this.endUpdate(_);
		} finally {
			this.stopTiming("updateUsingTree.collect");
			this.stopTiming("updateUsingTree");
		}
	},
	...
	updateIndexfromCsv: function(_, file) {
		...
		var index = readIndex(_, indexpath, category, opt).index;
...
function readIndex(_, indexpath, chapter, options) {
	...
	var path = options.masterIndexPath || "../translations/data/masterIndex.json";
	if (!fsModule.exists(path, _)) {
		path = fsp.join(options.indexdir, "masterIndex.json");
	}
	var masterIndex;
	var data;
	if (!fsModule.exists(path, _)) {
		log.warn("Master index node found: " + path);
	} else {
		data = fsModule.readFile(path, "utf8", _);
	}
	try {
		masterIndex = data && JSON.parse(data);
		// tracer && tracer("index data " + JSON.stringify(index, null, 2));
	} catch (e) {
		throw new Error(localizedText("incorrectJsonFormat", indexpath));
	}

	var mapping = masterIndex && masterIndex.mapping || {};
	var mkeys = masterIndex && masterIndex.keys;
	!mkeys && console.error("**** No master index keys *****");

	data = fsModule.exists(indexpath, _) && fsModule.readFile(indexpath, "utf8", _);
	if (!data) {
		index = {};
		...
		_traverse(index, function(context, gv, ev, tv) {
			if (mkeys) {
				var mi, id;
				// console.error("traverse:", gv && gv.$key, ev && ev.$key, context.groupKey, id);
				if (!ev && !tv) {
					mi = mkeys[gv.$key];
					if (mi) {
						id = mi.gid;
						// context.groupKeyNorm = _getMapping(mapping, _normalizeGroup(context.groupKey));
						context.groupKeyNorm = id;
						if (id != context.groupKey) {
							// console.error("reassign group:", context.groupKey, id);
							// gv.$id = id;
							gv.$oldId = context.groupKey;
							index[context.groupKeyNorm] = gv;
							delete index[context.groupKey];
						}
					}
				} else if (!tv) {
					mi = mkeys[ev.$key];
					if (mi) {
						id = mi.uid;
						// ev.$oldId = context.entryKey;
						context.entryKeyNorm = id;

						var eid = id.split("/").slice(-1);
						if (eid != context.entryKey) {
							// console.error("reassign entry:", context.entryKey, id);
							gv[eid] = ev;
							delete gv[eid];
						}
						// context.groupKeyNorm = _getMapping(mapping, _normalizeGroup(context.groupKey));
	...
	function _newEntry(index, group, ident, val, language) {
		var segment = index[group] || (index[group] = {}),
			e;
		segment.$key = sha1(group);
		// segment.$id = _getMapping(mapping, _normalizeGroup(group));
		...
		e = {
			...
			$id: _getMapping(mapping, _normalizeGroup(group + "/" + ident))
		};
	...
	return {
		index: _initIndexChapter(index, chapter),
...
function _fileInfo(file, basedir, handler) {
	...
	var g0 = m && fsp.join(dirname, m[1]).replace(/\\/g, '\/');
	var gn = m && m[1].replace(/\\/g, '\/');
	gn = gn.replace(/(.+\/)?@sage\//, "");
	gn = gn.replace(/^syracuse-/, "s-");
	gn = gn.replace(/\/(lib|src)\//, "/");
	gn = gn.replace(/\/resources\/([^/]+)$/, "/$1");
	gn = gn.replace(/\/resources\/([^/]+)$/, "/$1");
	...
	if (handler.skipFile && handler.skipFile(fi)) {
		tracer && tracer("_fileInfo " + file + " skipped");
		return;
	}
...
var itemTypesToKeep = ["landingPage", "menuCategory", "portlet", "notificationEvent", "notificationTheme", "msoWordTemplateDocument", "mobileGadget", "mobileApplication"];
...
function _filterLocalizations(json) {
	...
	items.forEach(function(item) {
		...
		var keyName = (proto[item.$type] || {}).$key || "code";
		var code = item[keyName];
		// if (item.$type === "menuItem" && item.linkType === "$representation" && item.$factoryOwner === "SAGE") {
		if (itemTypesToKeep.indexOf(item.$type) !== -1) {
			addKey(item);
		} else if (item.$type === "menuItem" && item.linkType === "$representation") {
			addKey(item);
		} else if (item.$type === "menuModule") {
			if (["STD_X3_HRM_SITES", "STD_X3_ERP_EDI", "STD_X3_ERP_CCM", "Viewpages"].indexOf(code) != -1 || isAdmin) {
				addKey(item);
			...
			tracer && tracer("_filterLocalizations excluding type=" + item.$type + ", code=" + code);
		}
	});
	...
	var handlers = {
		"syracuse": {
			...
			skipFile: function(fi) {
				// tracer && tracer("handler.skip: " + JSON.stringify(file));
				var skip;
				if (/^shadow-modules/.test(fi.groupOld)) {
					skip = !/^shadow-modules\/win32-x64-v8-5\.0\/node_modules\/(@sage\/)?[^\/]+\/src\//.test(fi.groupOld);
					if (!skip) {
						fi.groupOld = fi.groupOld.substring("shadow-modules/win32-x64-v8-5.0/".length);
						console.error("keep group: " + fi.groupOld);
					}
				}
				// skip && console.error("skipFile: " + fi.group);
				return skip || this.category.isExcluded(fi.groupOld);
			},
			match: function(file) {
...
function _filterImport(data, proto) {
	...
	function addItem(item, code) {
		code = code || item[(proto[item.$type] || {}).$key || "code"];
		if (itemsMap[code])
			return;
		itemsMap[code] = item;
		items.push(item);
	...
	data.forEach(function(item) {
		var isAdmin = _isAdmin(item);
		var key = (proto[item.$type] || {}).$key || "code";
		var code = item[key];
		// if (item.$type === "menuItem" && item.linkType === "$representation" && item.$factoryOwner === "SAGE") {
		if (itemTypesToKeep.concat(["pageLayout", "dashboardDef"]).indexOf(item.$type) !== -1) {
			addItem(item, code);
			...
			if (["STD_X3_ERP_EDI", "STD_X3_ERP_CCM", "Viewpages"].indexOf(code) != -1 || isAdmin) {
				addItem(item);
				if (item.submodules && item.submodules.length > 0) {
					item.submodules.forEach(function(submodule) {
						submodules[submodule.code] = submodule;
