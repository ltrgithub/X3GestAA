...
World.prototype = {
	submit: function httpSubmit(_, ep, params) {
		params = params || {};
	...
	createMessageStore: function(_, endpointRead, endpointWrite) {
		...
		if (options.noDBStore && this.messagesFilePrefix) {
			if (this.messagesFilePrefix) {
				mstore = new messageStore.FileMessageStore(world, endpointRead, endpointWrite, opts);
			}
			//
		} else {
			mstore = new messageStore.X3ClobMessageStore(world, endpointRead, endpointWrite, opts);
		}
						...
						if (targetText) {
							...
							if (refText == null) {
								...
								if (src.exists && indexText && sourceText) {
									if (sourceText === indexText && lang === refLang) {
									...
									if (sourceText != indexText && !messageStore.isEqualPlaceHolder(indexText)) {
...
function _simplifyItem(_, entity, item, itemProto, localization, _pointerTypes) {
	...
	// console.error("_simplifyItem: " + item.code);
	itemProto = itemProto || {};
	flows.eachKey(_, item, function(_, key, value) {
		...
		if (entity.$relations && (relMeta = entity.$relations[key])) {
			...
			if (relMeta.isPlural) {
				// console.error("_simplifyCollection: key=" + key + ", value=" + sys.inspect(value));
				// console.error("entity=" + sys.inspect(entity));
				// console.error("itemProto=" + sys.inspect(itemProto));
...
function _simplifyCollection(_, entity, relName, relMeta, relProto, value, localization, _pointerTypes) {
	...
	// console.error("_simplifyCollection: " + relName);
