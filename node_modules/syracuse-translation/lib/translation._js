...
function getX3Records(_, db, whereClause) {
	...
	records.forEach_(_, function(_, inst) {
		...
		if (!inst.LANNUM(_)) tracer && tracer("ERROR: incorrect record,lannum is missing");
	...
	function extract(_, file) {
		...
		function _checkEntry(_, key, message) {
			...
			if (seg && seg[key]) {
				...
				if (chp>chapterSyracuse+5) {
					if (options && options.$diagnoses) 
						options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "chapterFull", chapterSyracuse+5,"syracuse texts")
				});
				else 
					instance.$addDiagnose('error',module, "chapterFull", chapterSyracuse+5, "syracuse texts");
...
exports.extractAdminResources = function(_, instance, options, context) {
	...
	function _extract(_, fpath, fname) {
		...
		flows.eachKey(_, localizationRef, function(_, key, val) {
			if (index[fname] && index[fname][key]) {
				...
				if (chp>chapterAdmin+5) {
					if (options && options.$diagnoses) 
						options.$diagnoses.push({
					severity: "error",
					message: locale.format(module, "chapterFull", chapterAdmin+5,"admin resources")
				});
				else 
					instance.$addDiagnose('error',module, "chapterFull", chapterAdmin+5, "admin resources");
	...
	var entity = db.getEntity(_, "APLSTD", "$bulk");
	var whereClause= "LANCHP in("+chapterAdmin + "," + (chapterAdmin + 1) + //
	"," + (chapterAdmin + 2) + "," + (chapterAdmin + 3) + //
	"," + (chapterAdmin + 4) + ") and LANNUM ne 0";
