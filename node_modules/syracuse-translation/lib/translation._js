...
function _getLanguages(_,instance) {
   ...
   /*var ent=db.model.getEntity("endPoint") ;
    
    var epX3=db.fetchInstance(_, ent,{jsonWhere: {"dataset": "SUPERV"}}); // instance.endpoint(_).getOrm(_);
   tracer && tracer("X3 EP: "+epX3.getBaseUrl(_)) ;
    var dbX3 = epX3.getOrm(_) ;
    var entity=dbX3.getEntity(_, 'TABLAN', "$query");
   tracer && tracer("TABLAN: "+sys.inspect(entity)) ;*/
    var where = "LANISO in  (" ;
 ...
 where+=")";
    ...
    var langs = db.fetchInstances(_, entity, {sdataWhere : where}) ; //' /*+sys.inspect(x3langs)*/});
...
exports.extractResources = function (_, instance, options, context) {
   ...
   //tracer && tracer("db: "+sys.inspect(db));
    ...
    var entity ;
    /*if(unitTestFlg )
      entity = db.getEntity(_, "APLSTDs", "$query");
    else*/
