...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	injectLongMessage: function(_, entry) {
		...
		if (chunks.length > 1 && (this.options.longMessagesStrategy === "truncate")) {
			...
			this.diagnosis.logWarning(localizedText("msgTooLong", entry.group, entry.ident, aplStdMaxLength, message));
		}
		...
		if (chunks.length > 0) {
			...
			if (x3message !== message) {
				...
				if (x3message !== indexText && message !== indexText) {
					if (conflictFix == null) {
						diagnosis.error(localizedText("updateConflict", entry.group + "/" + entry.ident, el.chp + "," + el.id + "," + x3Lang, message, x3message, indexText));
						return;
					}
				}
	...
	initFrame: function(_, category, msgkey, opt) {
		...
		if (opt.fetchRecords) {
			if (opt.loadFromIndex) {
				...
				this.store.dumpToPlainFile(_, this.messagesFilePrefix + "-fetched");
			}
		}
		frame.index = index;
	...
	verifyExtract: function(_) {
		...
		diagnosis.info(localizedText("foundRecordRange", x3Records.$count, chapter.$min, chapter.$max));
			...
			function updData(_, key) {
				...
				if (rec && rec.text) {
					...
					if (eRef && ((eRef.chp !== chapter) || (eRef.id !== el.id))) {
						return diagnosis.fatal(localizedText("invalidBinding", ref, chapter, el.id, eRef.chp, eRef.id), null);
					}
							...
							if (targetText != indexText || !src.exists) {
								...
								if (src.exists && indexText && sourceText && sourceText != indexText) {
									if (targetText === sourceText) {
										...
										if (conflictFix == null) {
											diagnosis.error(localizedText("updateConflict", k + "/" + key, el.chp + "," + el.id + "," + lang, sourceText, targetText, indexText));
											targetText = null;
										}
									}
								}
	...
	downloadToFile: function(_, parameters) {
		var file = fsp.join(this.targetDir, parameters.fileName),
			set,
			opt = {};
		...
		this.store.dumpToPlainFile(_, set.getAll(), file);
	},
...
function makeDiagnosisCollectors(options) {
	...
	function appendMessage(severity, message) {
		nfs.appendFile(logName, datetime.now() + " [" + severity.toUpperCase() + "] " + message + "\n", "utf8", function(err) {
	...
	function logger(severity, message) {
		...
		if (!fd) {
			fd = nfs.open(logName, "w+", "0666", function(err) {
				if (err) return console.error(err);
