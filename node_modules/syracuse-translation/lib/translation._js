...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	getLanguages: function(_) {
		...
		var adminDb = adminHelper.getCollaborationOrm(_),
			localePrefs = adminDb.fetchInstances(_, adminDb.model.getEntity(_, "localePreference")),
			x3langs = {},
			i;
		...
		try {
			...
			this.diagnosis.logWarning("Cannot get language from endpoint. A default list has been set. " + e.message, _);
			x3langs = {
				"GER": "de-de",
				"ENG": "en-us",
				"BRI": "en-gb",
	...
	injectLongMessage: function(_, entry) {
		...
		if (chunks.length > 1 && (this.options.longMessagesStrategy === "truncate")) {
			...
			this.diagnosis.logWarning(localizedText("msgTooLong", entry.group, entry.ident, aplStdMaxLength, message), _);
		}
	...
	createEntry: function(_, chp, id, message, language, refkey) {
		var i, ii, chunks = _splitMessage(message),
			placeHolder = message === placeholders.equal ? placeholders.equal : placeholders.empty;

		if (!Array.isArray(id)) {
			id = [id, id];
		}
		for (ii = id[0], i = 0; ii <= id[1]; ii++, i++) {
			this.store.createEntry(_, chp, ii, chunks[i] || placeHolder, language, refkey);
		}
	},
	...
	updateEntry: function(_, el, entry) {
		...
		tracer && tracer("updateEntry  index message='" + (indexText || "") + "'");
		tracer && tracer("updateEntry   conflict fix='" + conflictFix + "'");
		if (x3message === message && indexText === message)
			return;

		if (conflictFix) {
			message = el.text[isoLang] = conflictFix;
		}
		if (chunks.length > 0) {
			el.text = el.text || {};
			...
			if (x3message !== message) {
				...
				if (x3message !== indexText && message !== indexText) {
					if (conflictFix == null) {
						diagnosis.error(localizedText("updateConflict", entry.group + "/" + entry.ident, el.chp + "," + el.id + "," + x3Lang, message, x3message, indexText), _);
						return;
					}
				}
			...
			this.createEntry(_, el.chp, el.id, message, entry.language, entry.$key);
			diagnosis.logWarning(localizedText("indexMess", entry.group, entry.ident), _);
		}
	...
	fillDefault: function(_) {
		var store = this.store,
			...
			frCulture = {
				iso: "fr-fr",
				x3: locale.x3Language("fr-fr", true)
			},
			g, e, entry;

		diagnosis.info("Filling default values of " + chapter.$name, _);
		...
		Object.keys(index).forEach_(_, function(_, gk) {
			...
			Object.keys(g).forEach_(_, function(_, ek) {
				...
				if (ek[0] === "$") return;
				e = g[ek];
				if (e.text && e.text[frCulture.iso] != null) return;
				diagnosis.logInfo("Create default values for " + gk + "/" + ek, _);
				e.text[frCulture.iso] = e.text[defaultLanguage.iso];
				store.dupEntry(_, e.chp, e.id, defaultLanguage, frCulture);
			});
		});
		diagnosis.info("End of filling default values of " + chapter.$name, _);
	},
	...
	extractAdminResources: function(_, opt) {
		...
		function _filter(fpath) {
			return (transCfg.extractFilters && transCfg.extractFilters[fsp.basename(fpath)]) || extractFilters[fsp.basename(fpath)] || extractFilters["*"];
		}
		...
		function _extractAllLang(_, fpath, fname, fsModule) {
			...
			refKeys = !filter && !transCfg.ignoreRefKeys && localizationKeys(jsonContent);
		...
		var frame = this.initFrame(_, categories.admin, "startExtractResources", opt),
			...
			include = transCfg.include || ["syracuse-security-init.json", "syracuse-admin-template.json", "syracuse-admin-categories.json", "x3-erp-sitemap.json", "x3-hrm-sitemap.json", "x3-erp-homepages.json", "syracuse-collaboration-sitemap.json", "syracuse-support-sitemap.json", "x3-mobile-dashboards.json"],
			importDir = "../../../import/" + (opt.filterAdmin ? "" : (transCfg.adminSubDir || "translation")),
			path = options.exportFile ? options.dir || fsp.join(__dirname, "../../") : fsp.join(__dirname, importDir),
			extractFilters = {};

		if (opt.filterAdmin) {
			extractFilters = {
				"x3-erp-homepages.json": filters.pageLayout,
				"x3-erp-sitemap.json": filters.globalSitemap,
				"x3-hrm-sitemap.json": filters.globalSitemap,
				"syracuse-collaboration-sitemap.json": filters.globalSitemap,
				"syracuse-support-sitemap.json": filters.globalSitemap,
				// "*": filters.others
				"*": null // no filtering is the default
			};
		}
				...
				function loadLocalData(_, langs) {
					langs.forEach_(_, function(_, lang) {
						...
						if (src.exists) {
							...
							diagnosis.logWarning(localizedText("missingFile", file, lang), _);
							src.isDefault = true;
	...
	deleteAll: function(_) {
		...
		Object.keys(categories).forEach_(_, function(_, k) {
			...
			categ = categories[k];
			diagnosis.info("Deleting " + categ.$min + ".." + categ.$max, _);
...
function _fileInfo(file, basedir, handler) {
	...
	// 	return;
	// }
	language = languageMap[lang] || (languageMap[lang] = {
		iso: lang,
		x3: locale.x3Language(lang, true)
	});
...
function _checkEntry(_, translation, resources, index, grpkey, language) {
	...
	return function(_, key) {
		...
		if (isoLang != "en-us" && isoLang != "fr-fr" && texts["en-us"] === message) {
		...
		if (message === null) {
			message = "";
			diagnosis.logWarning(localizedText("nullValue", entry.keyString), _);
		} else if (typeof(message) !== "string") {
			return diagnosis.logWarning(localizedText("incorrectType", entry.keyString, typeof(message), JSON.stringify(message)), _);
		}
		if (message && (options.longMessagesStrategy === "truncate") && (message.length > aplStdMaxLength)) {
			...
			diagnosis.logWarning(localizedText("msgTooLong", grpkey, key, aplStdMaxLength, message), _);
		}
		if (el) {
			...
			if (texts[isoLang] == null) {
				...
				translation.createEntry(_, el.chp, el.id, entry.val, entry.language, entry.$key);
			} else {
...
var filters = {
	...
	globalSitemap: function(json) {
		...
		items.forEach(function(item) {
			if (item.$type === "menuItem" && item.linkType === "$representation") {
				keys[item.title] = item;
				keys[item.description] = item;
			} else if (item.$type === "menuModule" && item.code === "Viewpages") {
				keys[item.title] = item;
				keys[item.description] = item;
				menuModules = item.submodules;
			} else if (item.$type === "menuBlock" && item.application.application === "x3") {
				menuBlocks[item.code] = item;
			}
		});
		menuModules && menuModules.forEach(function(m) {
			var mb = menuBlocks[m.code];
			if (mb) {
				keys[mb.title] = mb;
				keys[mb.description] = mb;
			}
			(mb.items || []).forEach(function(e) {
				var menuSubblocks;
				if (e.$type === "menuSubblock") {
					(e.items || []).forEach(function(sb) {
						keys[sb.title] = mb;
						keys[sb.description] = mb;
					});
				}
			});
		});
	...
	syracuseMenuItem: function(json) {
		var localization = json.$localization || {},
			items = json.$items || [],
			langs = Object.keys(localization),
			res = {},
			keys = {},
			entries,
			e;
...
function _updateProjectFile(_, iso, base, opt) {
	...
	if (!exists) {
		diagnosis && diagnosis.logWarning && diagnosis.logWarning(localizedText("fileNotFound", projPath), _);
...
function reassembleRecord(chp, id) {
	if (!chp)
		return chp;
	var i, res = {
			id: id
	...
	function reassamble(r) {
		if (!r) return;

		var t = res.text || (res.text = {});
		Object.keys(r).forEach(function(lang) {
			if (lang === "XXX") {
				res.$key = res.$key || r[lang];
			} else {
				t[lang] = (t[lang] || "") + r[lang].replace(/(\s*_\s*)$/, " ");
			}
		});
	}
