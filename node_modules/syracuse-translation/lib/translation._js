...
function _createEntry(_, contractUrl,headers,chapter, id, message) {
        ...
        var data = {
            '$uuid' : uid ,
            'LANCHP': chapter ,//10031,
            'LANNUM': id, //index[dirkey][key].id,
            'LAN': 'ENG',
            'LANMES': message,
            
        };
        var body = post(_, headers, contractUrl + "/ZZAPLSTD?representation=ZZAPLSTD.$edit" , data);
...
function _updateEntry(_, endPoint,key, text, chapter ,lannum) {
    console.log("UPDATE ENTRY " + lannum+ ' new value: '+text);
    ...
    console.log("old value: "+msg.LANMES(_)) ;
    msg.LANMES(_, text);
    msg.save(_);
...
exports.extractResources = function (_, instance, options, context) {
        ...
        function extract(_, file) {
            ...
            function _checkEntry(_, key, message) {
                ...
                if (x3message) {
                    ...
                    _createEntry(_, contractUrl,self.context.request.headers,10031 ,index[dirkey][key].id, message);
    ...
    exports.updateResources = function (_, instance, options, context) {
        ...
        //var x3eps=get(_,cookie,bUrl+cntrUrl+"endPoints?sdataWhere=(application eq 'x3' and contract eq 'erp')") ;

        //var langFname={} ;


        langFname=_getLanguages(_) ;
        console.log("langFname: "+JSON.stringify(langFname)) ;
        var index;
        ...
        flows.eachKey(_, index, function (_, k) {
            ...
            flows.eachKey(_, index[k], function (_, key, el) {
                ...
                if (rec) {
                    ...
                    console.log("DATA BUFFERS: " + sys.inspect(dataBuffers));
                }
            });
    ...
    //console.log("X3RECORDS: " + sys.inspect(x3Records));
        ...
        var fdRes = fs.open(resourcesPath, "a+", 0666, _);
        var contentStr = fs.readFile(resourcesPath, "utf8", _);
        fs.close(fdRes, _);
        ...
        flows.eachKey(_, localizationRef, function (_, key, val) {
          if (!index[key]){
            index[key]={} ;
            index[key].text= val ;
            index[key].id=index.count+1 ;
            index.count++ ;
          }
           var x3message = _findMessage(_, x3Records, "LANNUM", index[key].id);
                if (x3message) {
                    //console.log("x3message " + x3message)
                    ...
                    //console.log("index[idxkey].text :"+index[idxkey].text) ;
                    if (localizationRef[key] !== index[key].text) {
                        tracer && tracer("modifyed in Syracuse " + index[key].text + ' ' + localizationRef[key]);
                        if (x3message !== index[key].text) {
                          ...
                          index[key].text=localizationRef[key] ;
                            _updateEntry(_, endPoint,key, localizationRef[key], 10032, index[key].id);
                      }
                    }
                   } else {
                    console.log("not found");
                    console.log("inject message "  + key);
                     _createEntry(_, contractUrl,self.context.request.headers, 10032 ,index[key].id, val);
                }
        });
...
exports.updateAdminResources = function (_, instance, options, context) {
        console.log("UPDATE Admin RESOURCES");
        var self = this;
        self.context = context;
        options = options || {};
        var burl = (options.baseurl) ? options.baseurl : "http://localhost:8124";
        ...
        while (inst = cursor.next(_)) {
            ...
            console.log("x3 record "+inst.LANNUM(_) +'-'+inst.LAN(_) +'-'+ inst.LANMES(_)) ;
            if (!x3Records[inst.LANNUM(_)])
                var rec = x3Records[inst.LANNUM(_)] = {};
            rec[inst.LAN(_)] = inst.LANMES(_);
        };
        ...
        flows.eachKey(_, index, function (_, locCode, el) {
      ...
      fs.writeFile(resourcesPath, JSON.stringify(jsonContent,null,2), "utf8", _);
