...
function TranslationProcess(_, context, options) {
	...
	this.messagesFile = options.messagesFile || fsp.join(this.targetDir, "messages.txt");
...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	endExtract: function(_) {
		...
		this.store.appendTofile(_, this.messagesFile);
	},

	clearMessagesFile: function(_) {
		fs.writeFile(this.messagesFile, "", "utf8", _);
	},
...
function store(_, endpoint, options) {
	...
	function _appendTofile(_, messagesFile) {
		...
		fs.appendFile(messagesFile, txt, "utf8", _);
	}
