		...
		if (num && num == lannum && chp && chp == lanchp) {
			...
			if (match ){
				chunks.push(item.LANMES(_)) ;
					var line = match[0];
					var chapter = match[2]? parseInt(match[2]) : item.LANCHP(_);
	...
	if (text.length>maxLength){
		...
		tracer && tracer ("i: "+i) ;
			if (oldChunks[i]){
				...
				if (match ){
					var line = match[0];
					var chapter = match[2]? parseInt(match[2]) : msg.LANCHP(_);
					var linenumber = parseInt(match[3]);
					tracer && tracer ("chapter: "+chapter) ;
					tracer && tracer ("linenumber: "+linenumber) ;

					var newValue=newChunks[i]+line ;
					...
					msg=nextmsg ; 
					/*msg =  db.fetchInstance(_, entity, {
						sdataWhere : "LANCHP eq " + nextchapter + " and LAN eq 'ENG' and LANNUM eq " + nextid
					});
					tracer && tracer("fetch nextchapter: " + nextchapter);
					tracer && tracer("fetch nextid: " + nextid);
					tracer && tracer("fetched: "+sys.inspect(msg));*/
		...
		function _checkEntry(_, key, message) {
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				if (x3message) {
					...
					if (message !== seg[key].text) {
						...
						if (x3message !== index[dirkey][key].text) {
							...
							var nb=_updateEntry(_, endPoint, key,message, currentChapter,seg[key].chp, seg[key].id,chunks,index.count);
					...
					try {
						var incr=0 ;
						success = _createEntry(_, instance.endpoint(_), currentChapter, index[dirkey][key].id, message);
						if (success)
							incr+=1 ;
					} catch (ex) {
				...
				try {
					...
					success = _createEntry(_, instance.endpoint(_), currentChapter, id, message);
					if (success)
					 incr+=1 ;
				} catch (ex) {
					if (options && options.$diagnoses)
				...
				//if (success) {
					...
					tracer && tracer("Math.floor((index.count)/maxNbRecords):  " +Math.floor((index.count+1)/maxNbRecords)) ;
						index[dirkey][key] = {
							chp : 10031+Math.floor((index.count)/maxNbRecords) ,
							id : index.count%maxNbRecords + 1,
							text : message
						};
						index.count+=incr ;
...
exports.updateResources = function(_, instance, options, context) {
	...
	while (inst = cursor.next(_)) {
		...
		if (x3Records[inst.LANCHP(_)][inst.LANNUM(_)]){
			...
			if (match ){
					var line = match[0];
					var chapter = match[2]? parseInt(match[2]) : inst.LANCHP(_);
					var linenumber = parseInt(match[3]);
					tracer && tracer ("chapter: "+match[2]) ;
			...
			rec[inst.LAN(_)] = text;			
				...
				var ancestor=ancestors[inst.LANCHP(_)][inst.LAN(_)][inst.LANNUM(_)] ;
					var prevRec=x3Records[ancestor.chapter][ancestor.id] ;
					prevRec[inst.LAN(_)]+=inst.LANMES(_) ;
		}
	...
	flows.eachKey(_, index, function(_, k) {
		...
		function loadLocalData(_, rec, dataBuffers) {
			//tracer && tracer("loadLocalData dataBuffers" + sys.inspect(dataBuffers));
		...
		var filePaths = {};
		var dataBuffers = {};
		var currentChapter=chapterSyracuse ;
		tracer && tracer("index[" + k + "]=" + JSON.stringify(index[k], null, 2));
		if (k === "count")
			return;
		flows.eachKey(_, index[k], function(_, key, el) {
			if (el.id>maxNbRecords)
				++currentChapter ;
			var rec = x3Records[currentChapter][el.id]
			if (rec) {
					...
					if (rec[lang]) {
						...
						if (lang != 'ENG')
							...
							if (!dataBuffers[lang][key]){
								if (options && options.$diagnoses)
									options.$diagnoses.push({ severity: "error", message: ocale.format(module, "missingReference", k + ":" + key)});
								else
				...
				if (x3message) {
					...
					if (localizationRef[key] !== index[fname][key].text) {
						...
						if (x3message !== index[fname][key].text) {
							...
							var nb=_updateEntry(_, endPoint, key, localizationRef[key], chapterAdmin,chapterAdmin, index[fname][key].id,index.count);
							index.count+=nb ;
						}
					}
				} else {
	...
	function extract(_, file) {
		...
		function _checkEntry(_, key, message, fpath) {
			...
			if (index[dirkey] && index[dirkey][key]) {
				...
				var x3message = _findMessage(_, x3Records, index[dirkey][key].chp, index[dirkey][key].id);
				if (x3message) {
					...
					if (message !== seg[key].text) {
						...
						if (x3message != index[dirkey][key].text ) {
							...
							index[dirkey][key].text = message;
							_updateEntry(_, endPoint, key, message, chapterDotnet, chapterDotnet, seg[key].id,incr,index.count);
						}
					}
				} else {
					tracer && tracer("inject message " + dirkey + ":" + key);
