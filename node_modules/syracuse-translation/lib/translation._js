...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
		...
		function _extractAllLang(_, file) {
			...
			function _extractLang(_, lang) {
				...
				function _extract(_, finfo) {
					...
					var data = handler.load(_, finfo.file),
			...
			files.forEach_(_, function(_, f) {
				var fi = _fileInfo(f, resdir, handler.ext),
			...
			function _walk(_, path) {
				...
				if (stat.isDirectory()) {
					file = {
						name: path,
						fullName: fPath,
						relativeName: fsp.relative(rootpath, fPath)
					};
					if (!handler.skip(file)) {
						fullPath.push(path);
	...
	extractAll: function(_) {
		var opt = {
			...
			resetIndex: this.instance.resetIndex(_)
		},
			skip = transCfg.skip || {};

		tracer && tracer("extractAll");
		var l = this.getLanguages(_);
		!skip.syracuse && this.extractRes(_, resourceHandler(_, "syracuse", this, opt));
		!skip.dotnet && this.extractRes(_, resourceHandler(_, "dotnet", this, opt));
		!skip.admin && this.extractAdminResources(_, opt);
	},
	...
	updateAll: function(_) {
		...
		!skip.syracuse && this.update(_, resourceHandler(_, "syracuse", this, opt));
		!skip.dotnet && this.update(_, resourceHandler(_, "dotnet", this, opt));
		!skip.admin && this.updateAdminResources(_, opt);
	},
	...
	endUpdate: function(_) {
		...
		if (frame.updated > 0) {
			diagnosis.info(localizedText("endUpdateResources", index.$chapter.$name, endpoint.description(_), frame.updated), _);
		} else {
					...
					Object.keys(langMap).forEach_(_, function(_, lang) {
						...
						if (targetText) {
							...
							if (targetText !== indexText) {
								...
								if (indexText && resources[key] !== indexText) {
									if (targetText === resources[key]) {
										tracer && tracer("update index " + k + "/" + key + ": '" + el.text[isolang] + "' => '" + targetText + "'");
										el.text[isolang] = targetText;
										frame.updated++;
										// diagnosis.info(localizedText("updateConflict", k + "/" + key, el.chp + "," + el.id + "," + lang, resources[key], targetText, indexText), _);
									} else {
										tracer && tracer("update conflict for " + k + "/" + key + "[" + lang + "]\ns=" + resources[key] + "\nt=" + targetText + "\ni=" + indexText);
										diagnosis.error(localizedText("updateConflict", k + "/" + key, el.chp + "," + el.id + "," + lang, resources[key], targetText, indexText), _);
									}
								} else {
				...
				if (rec) {
					flows.eachKey(_, langMap, function(_, lang, isolang) {
						...
						if (rec[lang]) {
							...
							localization[isolang] = localization[isolang] || {};
...
function store(_, endpoint, options) {
	...
	function _getX3Records(_, chapter, opt) {
		...
		if (typeof(chapter) === "object") {
			params.sdataWhere = "LANCHP ge " + chapter.$min + " and LANCHP le " + chapter.$max + " and LANNUM ne 0";
		} else {
			params.sdataWhere = "LANCHP eq " + chapter + " and LANNUM ne 0";
		}
		if (opt && opt.metaOnly) {
			params.sdataWhere += ' and LAN eq "XXX"';
		} else {
			params.sdataWhere += (opt && opt.noMeta ? ' and LAN ne "XXX"' : "") + (lang ? ' and LAN eq "' + (locale.x3Language(lang) || lang) + '"' : "");
		}
		if (opt && opt.lastUpdate) {
			params.sdataWhere += ' and UPDDATTIM ge "' + opt.lastUpdate + '"';
		}
		...
		do {
			records = db.fetchInstances(_, entity, params) || [];
			records.forEach_(_, fillRecord);
			tracer && tracer("getX3Records: length=" + records.length + ", params=" + JSON.stringify(params));
			params.startIndex += records.length;
		} while (records.length === fetchCount);
		fetched = params.startIndex;
		x3Records.$count = fetched;
		tracer && tracer("getX3Records count=" + fetched + ", params=" + JSON.stringify(params));
...
function makeDiagnosisCollectors(_, instance, options) {
	...
	if (options && options.$diagnoses) {
		tracer && tracer("makeDiagnosisCollectors with options.$diagnoses");
		...
		tracer && tracer("makeDiagnosisCollectors without options.$diagnoses");
		collector = function(severity, message) {
			hasError = hasError || severity.toLowerCase() === "error";
...
function _fileInfo(file, basedir, extFilter) {
	var ext = fsp.extname(file);
...
function resourceHandler(_, type, context, opt) {
	var handlers = {
		"syracuse": {
			category: categories.syracuse,
			rootDir: fsp.join(context.rootDir, "node_modules"),
			ext: ".json",
			desc: "file",
			load: function(_, fullName) {
			...
			skip: function(file) {
				...
				return file.name && file.name === "test";
			},
			match: function(file) {
