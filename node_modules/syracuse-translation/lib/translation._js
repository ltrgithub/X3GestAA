...
function store(_, endpoint, options) {
	...
	function _dbFlush(_, force) {
		...
		var timing = perfmon.start(module, "translation._dbFlush", list.length);
...
function resourceHandler(_, type, context, opt) {
	var handlers = {
		...
		"admin": {
			...
			skip: function(file) {
				var exclude = ((this.category.$exclude || []).indexOf(file.name) > 0);
				tracer && tracer("admin.handler.skip: " + exclude + ", file=" + JSON.stringify(file));
				return exclude;
			},
			match: function(file) {
				return true;
