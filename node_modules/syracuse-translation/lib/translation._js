...
function _updateEntry(_, endPoint, key, text, chp, lannum,chunks) {
...
//DARY
	if (text.length>maxLength){
		...
		for (i=0; i<(newChunks.length-1);i++){
			...
			//last chunk
			if (msg && oldChunks[newChunks.length-1]){
				msg.LANMES(_, newChunks[newChunks.length-1]);
					msg.save(_);
			}
				...
				if (x3message) {
					...
					if (message !== seg[key].text) {
						...
						if (x3message !== index[dirkey][key].text) {
							...
							_updateEntry(_, endPoint, key,message,  seg[key].chp, seg[key].id,chunks);
						}
					}
				} else {
