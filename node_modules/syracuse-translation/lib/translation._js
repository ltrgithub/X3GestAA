...
exports.TranslationProcess = helpers.defineClass(TranslationProcess, null, {
	...
	endExtract: function(_) {
		...
		this.store.flush(_);
		rec = this.store.getRecords(_, $chapter, {
			count: 1,
			orderBy: {
				name: "UPDDATTIM",
				descending: true
		...
		$chapter.$lastModified = rec[0] && rec[0].UPDDATTIM(_).toString(isoDateFormat) || "";
	...
	updateAll: function(_) {
		...
		resourcesOpts.admin && !skip.admin && this.updateAdminResources(_, opt);
	},
