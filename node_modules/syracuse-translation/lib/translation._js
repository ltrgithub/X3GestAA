	...
	extractAdminResources: function(_, handler) {
		...
		function _extractAllLang(_, fpath, fname, fsModule) {
			var destfile = self.getAdminTranslationDestDir(fpath),
				...
				jsonContent = _filterLocalizations(_, self.tryParseJsonFile(_, fpath), _importfilters[basename]),
...
function _filterLocalizations(_, data, filter) {
	...
	// _simplifyImport(_, proto, items, data.$localization, json);
...
var _filterImport = {
	landingPage: function(data, proto) {
		var items = data.filter(function(item) {
			return item.$type === "pageLayout" || (item.$type === "landingPage" && item.$factoryOwner === "SAGE");
		});
