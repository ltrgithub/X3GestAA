...
exports.getMessages = function(_, product, chapter, lan, from, to) {
	...
	var range = resp.body.reduce(function(r, message) {
		if (message.LAN === lan && message.LANNUM >= from && message.LANNUM <= to) {
			r.push({
				LANNUM: message.LANNUM,
				LANMES: message.LANMES
			});
		}
		return r;
	}, []);
...
exports.createMessage = function(_, product, lanori, languages, chapter, text) {
	// console.log("createMessage:"+ [product, lanori,languages, chapter, text].join(','));
	var data = {
		"LANMES": text,
		"LANORI": lanori,
		"LAN": languages.split(',')
