...
exports.getTextOriginalLanguage = function(_, product, num, lan) {
	...
	if (resp.status === 200) {
		resp.body.some(function(text) {
			...
			lanori2 = (!lanori2 && text.LANORI.length) ? text.LANORI : lanori2;
			return false;
		});
	}
	return _wrapResult(lanori1 || lanori2 || "", resp.status);
};
...
/* getLastModifiedTexts returns texts from a lastModified date
 */
exports.getLastModifiedTexts = function(_, product, lastModified) {
	// console.log("getText:"+ [product, num, lan].join(','));
	var resp = _get(_, _apiPath("SYNC", [product, lastModified]));
	return _wrapResult(resp.body, resp.status);
};
