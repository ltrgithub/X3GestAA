...
exports.getMessages = function(_, product, chapter, lan, from, to) {
	...
	var messages = resp.body.reduce(function(r, message) {
		if (message.LANNUM >= from && message.LANNUM <= to) {
			...
			r[message.LANNUM].codes[message.LAN] = message.CODE;
		}
		return r;
	}, {});
	...
	var range = Object.keys(messages).reduce(function(r, menu) {
		var message = "";
		var code = "";
		// CCC : first language to find at index 0
		[lan, "ENG", Object.keys(messages[menu].messages)[0]].some(
			language => {
				// CCC : get code for the language
				code = messages[menu].codes[language] || "";
				message = messages[menu].messages[language];
