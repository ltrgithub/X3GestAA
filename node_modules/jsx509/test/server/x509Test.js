...
var oids = require("jsx509").oids;
var x509 = require("jsx509").x509;
var Certificate = require("jsx509").Certificate;
var stripEncryption = require("jsx509").stripEncryption;
var integrity = require("jsx509").integrity;
var fs = require('fs');

module('jsx509');

test("oids rountrip", 141, function() {
...
test("strip private key encryption", 2, function() {
	...
	equal(nonEncrypted, stripEncryption(encrypted, "test"));
	equal(nonEncrypted, stripEncryption(nonEncrypted, "test"));
})
...
test("certificate verification", 3, function() {
	...
	equal(cert.verify(certString), false, "Certificate is not self signed")
})
...
test("verification of key and certificates", 9, function() {
	...
	equal(integrity(certString, encrypted, "serv", [caString]).error, "Error when decrypting private key (maybe wrong passphrase): TypeError: DecipherFinal fail", "wrong passphrase")
	...
	equal(integrity(certString, encrypted, "server", [certString]).error, "None of the given certificates of certification authorities fits to the certificate", "no fitting CA certificate")
	equal(integrity(certString, encrypted, "server", [encrypted]).error, "None of the given certificates of certification authorities fits to the certificate", "invalid CA certificate")
	equal(integrity(certString, encrypted, "server", ["blabla"]).error, "None of the given certificates of certification authorities fits to the certificate", "CA certificate not in PEM format")
