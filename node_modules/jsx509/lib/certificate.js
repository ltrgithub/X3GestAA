...
exports.integrity = function(certificate, key, passphrase, cacerts) {
	...
	if (certificate) {
		try {
			var cert = new exports.Certificate(certificate);
			result.cert = cert;
		} catch (e) {
			return {error: ""+e};
		}		
	...
	if (key) {
		...
		if (certificate) {
			...
			try {
				...
				console.log(e);
				return {error: locale.format(module, "errorSign", e)};
			}
			var verify = crypto.createVerify("RSA-SHA1");
			verify.update(testBuffer);
			try {
				if (!verify.verify(certificate, signature)) return {error: locale.format(module, "wrongKey")};
			} catch (e) {
				console.log(e);
				return {error: locale.format(module, "errorVerify", e)};
			}					
		}
	}
	if (cacerts && cacerts.length) {
		if (!cacerts.some(function(cacert) {
			try {
				return cert.verify(cacert);				
			} catch (e) {
				return {error:locale.format(module, "errorCA", e)};
			}
		})) return {error: locale.format(module, "noCA")};
	}
	// OK
	return result;
}
...
exports.stripEncryption = function(key, passphrase, test) {
	...
	if (r) {
		...
		if (r[1] === '4,ENCRYPTED') {
			if (r[3]) {
				var iv = new Buffer(r[3], "hex");
				passphrase = passphrase || "";
				var keyLength = 24;
				switch(r[2]) {
				case 'DES-EDE3-CBC': keyLength = 24; break;
				case 'DES-CBC': keyLength = 8; break;
				default: throw "Wrong cipher";
				}
				var ds = "";
				...
				var cipher = crypto.createDecipheriv(r[2], keypass, iv);
			} else {
				var cipher = crypto.createDecipher(r[2], keypass);
			}
			var buffer = new Buffer(r[4].replace(/\s+/g,""), "base64");
			try {
				var b1 = cipher.update(buffer, 'binary')
				var b2 = cipher.final('binary');				
			} catch(e) {
				throw locale.format(module, "errorDecrypt", e);
			}
			buffer = new Buffer(b1+b2, 'binary');
			if (test) {
				try {
					var node = asn1.fromBuffer(buffer);
				} catch (e) {
					throw locale.format(module, "wrongFormat");
				}
			}
			key = expandToPem(buffer, "RSA PRIVATE KEY");
		}
		return key;
	} else
		throw locale.format(module, "noPEM");
};


function _getDn(info) {
	var result = "C="+info.countryName+(info.stateOrProvinceName ? ", ST="+info.stateOrProvinceName : "")+
		(info.localityName ? ", L="+info.localityName : "")+", O="+info.organizationName+", ";
...
exports.Certificate = helpers.defineClass(function(buffer) {
	if (!Buffer.isBuffer(buffer)) {
		...
		if (startIndex < 0 || endIndex < 0) throw locale.format(module, "certNoPEM");
	...
	verify: function(certificate) {
		...
		if (algorithm.type === asn1.types.OID) {
			...
			if (certificate instanceof exports.Certificate) {
				certificate = expandToPem(asn1.toBuffer(certificate.parsed), "CERTIFICATE");
			}
			return verify.verify(certificate, this.parsed.children[2].getData())
		} else 
			throw "Algorithm is not an OID";
	}
})
