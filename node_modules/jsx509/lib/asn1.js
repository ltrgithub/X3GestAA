...
NP.getData = function() {
	if (this.buf) {
		...
		if (this.type === types.BITS || this.type === types.INTEGER && this.len > 1 && this.buf[this.pos] === 0)
			return this.buf.slice(this.pos+1, this.pos+this.len);
		else
			return this.buf.slice(this.pos, this.pos + this.len);
	} else
		return new Buffer(0);
}
...
NP.getMillis = function() {
	if (this.type === types.TIMESTAMP) {
		...
		throw "Invalid datetime string "+timestamp;
	}			
	throw "No datetime";
}
...
NP.toString = function(pfx) {
	...
	else if (this.type === types.TIMESTAMP) return head(name)+new Date(this.getMillis()).toGMTString();
