...
(function() {
	...
	function formatChildren(children, sessionId, start, end) {
	...
	function sum(vals) {
		return vals.reduce(function(r, v) { return r + v; }, 0);
	}

	function toHtml(data) {
		if (!data.children || !data.children.length) return 'Sorry, no data available. <a href="/syracuse-main/html/main.html?url=%3Frepresentation%3Dhome.%24dashboard">Navigate first!</a>';
		...
		s += '<h2>Distribution of CPU slices</h2><ul>' + data.cpu.distribution.map(function(v, i) {
			return '<li><b>&lt; ' + pow2(i) + ' ms</b>: ' + v + ' - ' + (Math.floor(v * 1000000 / tot) / 10000) + '%</li>';
		}).join('') + '</ul>';
		...
		s += '<p>uptime: ' + Math.floor(data.uptime / 3600) + ' hr ' + Math.floor(data.uptime / 60) % 60 + ' min ' + data.uptime % 60 + ' s</p>';
		return s;
	}
