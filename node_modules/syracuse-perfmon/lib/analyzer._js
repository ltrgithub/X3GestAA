...
var PerfStat = helpers.defineClass(function() {
	this.perfStat = {};
}, null, {
	...
	save: function writeStat(_, file) {
		this.perfStat.top.sort(sortTimingDesc);
	...
	_analyze: function anaylizeTiming(_, t, tp) {
		var self = this,
...
exports.analyze = function(path, file) {
	_analyze(_ >> _error, path, file);
};

function _analyze(_, path, file) {
	var perf = new PerfStat();
	perf.compute(_, path);
	perf.save(_, file);
}
