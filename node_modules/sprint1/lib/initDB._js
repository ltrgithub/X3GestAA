...
function createDatabase(_) {
  ...
  try {
    client = new Client();
    client.user = endPoint.datasets.sprint1.login;
    client.password = endPoint.datasets.sprint1.password;
    client.connect(_);
    client.query('DROP DATABASE IF EXISTS sprint1 ', _);
    client.query('CREATE DATABASE sprint1', _);
    client.end(_);
    // force database sync
    model.connect(_, "sprint1");
...
function importCompanies(_) {
  var context = new DataContext(model, "sprint1", true);
  companies.forEach_(_, function(_, company, index) {
    context.create(_, "Company", company);
  });
  context.commit(_);
...
function importCountries(_) {
  ...
  countries.forEach_(_, function(_, country, index) {
    context.create(_, "Country", country);
  });
  context.commit(_);
