...
test("with resource values", function() {
	var emp1 = _getEmpData();	
	equal(emp1.$.country.$lookup.$url, "http://localhost:8124/sdata/sprint1/settings/sprint1/countriesDiff", "should preserve values and subvalues");
});
...
test("with added properties by default (not in proto, not in resource) like $diagnoses", function() {
	...
	strictEqual(emp1.$.firstName.$diagnoses, undefined, "undefined by default");
	//setting value
	emp1.$.firstName.$diagnoses = [{ "severity": "warning", "message": "order date cannot be after shipping date" }];
	equal(emp1.$.firstName.$diagnoses.length, 1, "when setted should contains value");
    equal(emp1.$.firstName.$diagnoses[0].severity, "warning", "when setted should contains value");
	strictEqual(emp2.$.firstName.$diagnoses, undefined, "should not pollute other childs");
	strictEqual(employeeProto.$.firstName.$diagnoses, undefined, "should not pollute proto")
	
	emp1.$.firstName.$diagnoses.push({ "severity": "error", "message": "not found" });
	equal(emp1.$.firstName.$diagnoses.length, 2, "should work");
    equal(emp1.$.firstName.$diagnoses[0].severity, "warning", "should work");
	strictEqual(employeeProto.$.firstName.$diagnoses, undefined, "should not pollute proto")
	
	delta = helpers.object.clone(employeesDelta.$resources[0], true);
	delta2 = helpers.object.clone(delta, true);
	delta.$.firstName = {};
	delta.$.firstName.$diagnoses = [{ "severity": "info", "message": "just an info" }];
	emp1 = methodForMerging(delta, employeeProto);
	emp2 = methodForMerging(delta2, employeeProto);
	equal(emp1.$.firstName.$diagnoses.length, 1, "when already setted in resource, should find it");
    equal(emp1.$.firstName.$diagnoses[0].severity, "info", "when already setted in resource, should find it");
	strictEqual(emp2.$.firstName.$diagnoses, undefined, "should not pollute other childs");
	emp1.$.firstName.$diagnoses.push({"severity" : "nothing", "message" : "nothing"});
	equal(emp1.$.firstName.$diagnoses.length, 2, "when already setted in resource, should find it");
	equal(emp1.$.firstName.$diagnoses[1].severity, "nothing", "when already setted in resource, should find it");
	strictEqual(emp2.$.firstName.$diagnoses, undefined, "should not pollute other childs");
	emp1.$.firstName.$diagnoses = undefined;
	strictEqual(emp1.$.firstName.$diagnoses, undefined, "should able to set it to undefined")
