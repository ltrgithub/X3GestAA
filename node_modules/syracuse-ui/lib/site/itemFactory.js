...
helpers.defineClass(ItemFactory, null, {
    initializeItem: function(item, $item, boxParent){
        item.$item = $item;
        item.id = $item.$id || helpers.uuid.generate();
        if (boxParent) {
    ...
    loadPage: function($itemPage, pageCategoryClass){
        ...
        var item = new (pageCategoryClass || this._pageCategories[$pageCategory] || this._defaultPageCategory)();
        ...
        this.initializeItem(item, this.ensureDefaultArticle($article, item.$prototype), $itemPage.boxParent);
    ...
    createLayout: function(box, $layout){
        ...
        var layout = new this._layouts[$layout.$layoutType]();
        layout.box = box;
