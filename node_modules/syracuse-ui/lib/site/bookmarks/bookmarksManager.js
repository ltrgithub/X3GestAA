...
function _onNavigationMenuItemPickerClick(navigationMenuItem) {
	if (!navigationMenuItem.isBookmark) {
		...
		if (!_findMenuItem(urlSegment.$url)) {
		...
		_findMenuItem(navigationMenuItem.codeMenu.$url, null, true);
	}
	_toggleNavigationMenuItemBookMark(navigationMenuItem);
	exports.notifyChangeToServer();
};

function _findMenuItem($url, items, deleteIfFound) {
	var menuItem;
	var urlMaker = document.site.urlMaker;
	...
	for (var ii = 0, jj = items.length; !menuItem && ii < jj; ii++) {
		...
		if (item.menuItem) {
			...
			if (item.menuBlock && item.menuBlock.items) {
				menuItem = _findMenuItem($url, item.menuBlock.items, deleteIfFound);
			}
		}
	}
	return menuItem;
...
function _findByBunctionCode(functionCode, items, deleteIfFound) {
	var menuItem;
	items = items || _barArea.dataset.items;
	for (var ii = 0, jj = items.length; !menuItem && ii < jj; ii++) {
		var item = items[ii];
		if (item.menuItem) {
			if (!item.menuItem.$isSeparator && item.menuItem.urlSegment.fusionParams) {
				if (item.menuItem.urlSegment.fusionParams.functionCode == functionCode) {
					menuItem = item.menuItem;
					if (deleteIfFound) {
						items.splice(ii, 1);
						break;
			...
			if (item.menuBlock && item.menuBlock.items) {
				menuItem = _findByBunctionCode(functionCode, item.menuBlock.items, deleteIfFound);
			}
		}
	}
	return menuItem;
...
exports.checkNavigationMenuItemBookMark = function(navigationMenuItem) {
	if (_findMenuItem(navigationMenuItem.codeMenu.$url)) {
		navigationMenuItem.isBookmark = true;
		_toggleNavigationMenuItemBookMark(navigationMenuItem);
	}
};
...
function _onAddClick() {
	...
	if ($url) {
		if (!_findMenuItem($url, null, true)) {
			_barArea.dataset.items.push({
				menuItem: {
					$url: $url,
					title: title
...
exports.checkMainPage = function() {
	...
	if (activePage != _lastCheckedPage) {
		...
		if (activePage) {
			if (activePage.isFusionPage) {
				...
				if (functionfusion) {
					menuItem = _findByBunctionCode(functionfusion.name);
				}
			} else {
				menuItem = _findMenuItem(document.site.history.getSdataUrl(activePage.openerUrlSegments.$url));
			}
		}
		isOn = !! menuItem;
		for (var ii = 0, jj = _barArea.links.length; ii < jj; ii++) {
