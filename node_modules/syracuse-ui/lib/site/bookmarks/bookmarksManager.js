		...
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			...
			if (item.menuItem) {
				if (item.menuItem.$url) {
					...
					var seg = item.menuItem.urlSegment = urlMaker.parse(item.menuItem.$url);
					...
					if (seg.isFusion) {
						...
						(_syracuseMenuItems[seg.uri] = _syracuseMenuItems[seg.uri] || []).push(item.menuItem);
					}
...
exports.checkMainPageStatus = function() {
	...
	if (activePage && !activePage.disposed && activePage != _lastCheckedPage) {
		...
		syra_menus.setButtonTitle(_addBtn, isOn ? syra_local.bookmarksRemove : syra_local.bookmarksAdd);
		syra_site.dom.toggleClass(_addBtn, "s-bookmark-on", isOn);
		exports.onResize();
	}
};
...
exports.load = function() {
	...
	_links = [];
	_addBtn = syra_menus.addIconButton(syra_local.bookmarksAdd, "s-bookmarks-bar-add", "onAddClick");
	_addBtn.syraSiteObserver = "bookmarksManager";

	_manageBtn = syra_menus.addIconButton(syra_local.bookmarksManage, "s-bookmarks-bar-manage", "onManagerClick");
