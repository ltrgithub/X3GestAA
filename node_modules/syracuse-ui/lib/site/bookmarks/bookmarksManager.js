...
exports.refreshNavigationPage = function() {
	if (document.site.mainPage && document.site.mainPage.isNavigationPage) {
		...
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			...
			if (item.menuItem) {
				if (item.menuItem.$isSeparator) {
					...
					link.syraOnClick = "onBarLinkClick";
					link.syraSiteObserver = "bookmarksManager";
					link.setAttribute("href", "#");
					link.textContent = item.menuItem.title || item.menuItem.description || "???";
...
function _toggleNavigationMenuItemBookMark(navigationMenuItem) {
	document.site.dom.toggleClass(navigationMenuItem.bookmarkPicker, "s-bookmark-on", navigationMenuItem.isBookmark);
}
...
exports.checkNavigationMenuItemBookMark = function(navigationMenuItem) {
	navigationMenuItem.urlSegment = document.site.urlMaker.parse(navigationMenuItem.codeMenu.$url);
	document.site.urlMaker.replaceSegmentEndpoint(navigationMenuItem.urlSegment, document.site.userProfile.dataset.selectedEndpoint, true);
	if (_findMenuItem(navigationMenuItem.urlSegment)) {
		navigationMenuItem.isBookmark = true;
		_toggleNavigationMenuItemBookMark(navigationMenuItem);
	} else {
...
exports.refresh = function() {
	document.site.dom.empty(_barArea.linksSlot);
	document.site.dom.empty(_barArea.moreItemsSlot);
	...
	if (page) {
		...
		if (fct.name && (page.$fusionPageMeta.winModel._mdata.type == 1 || page.$fusionPageMeta.winModel._mdata.type == 2)) {
			if (page.openerUrlSegments.fusionParams.functionCode != fct.name) {
				...
				document.site.urlMaker.replaceFusionCode(urlSegment, fct.name);
				document.site.urlMaker.build(urlSegment);
				title = page.getTitle();
			} else {
				urlSegment = page.openerUrlSegments;
				title = fct.title;
			}
			description = page.localize.nvpFunction + ": " + page.openerUrlSegments.fusionParams.functionCode;
		}
	} else {
		...
		if (page && page.openerUrlSegments) {
			...
			if (page.$prototype.$representation) {
				description = page.localize.nvpEntity + ": " + page.$prototype.$representation;
				/*if (page.$prototype.$representation != helpers.string.pluralize(page.$prototype.$representation || "")) {
                 description += " (" + page.localize.nvpRepresentation + ": " + page.$prototype.$representation + ")";
                 }*/
			}
		}
	}
...
exports.checkMainPageStatus = function() {
	...
	if (activePage && !activePage.disposed && activePage != _lastCheckedPage) {
		...
		for (var ii = 0, jj = _barArea.links.length; ii < jj; ii++) {
			...
			if (link.menuItem) {
				document.site.dom.toggleClass(link, "s-bookmark-on", link.syraBookMarkOn = menuItem && link.menuItem == menuItem);
				document.site.dom.disableItem(link, (activePage && activePage.isFusionPage) && !link.menuItem.urlSegment.isFusion);
			}
		}
		_barArea.managePicker.style.visibility = _count ? "visible" : "hidden";
		var max = document.site.siteOptions.$item.$maxBookmarks || 25;
		if (_count >= max) {
			isBookmarkEnabled = false;
		}
		document.site.dom.disableItem(_barArea.addPicker, !isBookmarkEnabled);
		document.site.dom.toggleClass(_barArea.addPicker, "s-bookmark-on", isOn);
		exports.onResize();
	}
};
		...
		for (var ii = _barArea.links.length - 1; ii >= 0; ii--) {
			...
			if (!link.syraIsInMore) {
				...
				if (link.syraIsDisabled) {
					document.site.dom.disableItem(link, true);
				}
...
exports.onBarLinkClick = function(picker, event) {
	exports.onLinkLinkClick(picker.menuItem);
};

exports.onNavigationBookMarkClick = function(picker, event) {
	_onNavigationMenuItemPickerClick(repository[picker.syraFieldId]);
};
...
exports.load = function(slot) {
	...
	var link = _barArea.addPicker = document.createElement("a");
	link.className = "s-bookmarks-bar-add";
	link.syraOnClick = "onAddClick";
	link.syraSiteObserver = "bookmarksManager";
	link.setAttribute("href", "#");
	link.title = document.site.localize.bookmarksAdd;

	link = _barArea.managePicker = document.createElement("a");
	link.className = "s-bookmarks-bar-manage";
	link.syraOnClick = "onManagerClick";
	link.syraSiteObserver = "bookmarksManager";
	link.setAttribute("href", "#");
	link.title = document.site.localize.bookmarksManage;
	...
	var link = document.createElement("a");
	link.className = "s-bookmarks-more-picker";
	link.syraOnClick = "onMoreClick";
	link.syraSiteObserver = "bookmarksManager";
	link.setAttribute("href", "#");
	link.textContent = document.site.localize.bookmarksMore;
	link.title = document.site.localize.bookmarksMoreTooltip;
	_barArea.morePickerSlot.appendChild(link);
