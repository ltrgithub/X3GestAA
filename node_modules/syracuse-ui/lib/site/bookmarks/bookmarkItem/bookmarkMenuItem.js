		...
		if (this.codeMenu) {
			...
			if (value) {
				if (value.$isSeparator) {
					...
					value.$title = "-";
					document.site.dom.toggleClass(this.articleParent.domItem, this.$item.$skin + "-item", false);
				...
				if (value.$url) {
					...
					if (value.convergenceFunction) {
						value.$description = this.page.localize.nvpFunction + ": " + value.convergenceFunction;
					} else {
						if (value.entity) {
							value.$description = this.page.localize.nvpEntity + ": " + value.entity;
							if (value.entity != helpers.string.pluralize(value.representation || "")) {
								value.$description += " (" + this.page.localize.nvpRepresentation + ": " + value.representation + ")";
							}
						}
					}
