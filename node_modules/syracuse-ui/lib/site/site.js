...
var _checkerManager = require('syracuse-ui/lib/site/checker/checkerManager');
...
exports.Site = helpers.defineClass(Site, RawPage, {
			...
			callback: function() {
				if (!self.hasLogonFailed) {
					...
					locale.setCurrent(function(err) {
						...
						_checkerManager.onAfterLogon(onAfterLogon);
	...
	notifyRequestSurvey: function(count) {
		...
		syra_checker.robot.logRequestCount(count);
	...
	loadBox: function() {
		...
		var tutut = escape("?representation=excelconfig.$query&format=application/syracuse-excel-worksheet");


		window.syra_checker = _checkerManager;
	...
	addLogo: function(slot) {
		...
		btn.appendChild(this.productName);
	...
	ensureRecordUniqueSelection: function(list, onSelect) {
		...
		if (this._selectedList && this._selectedList.selector) {
			if (list != this._selectedList || (record && !record.$isSelected && !this._selectedList.selector.isMulti)) {
