...
var _tip = require('syracuse-ui/lib/over/tip');
var _overManager = require('syracuse-ui/lib/over/manager');
var ExternalAdapter = require("./aside/externalAdapter").ExternalAdapter;

var _alert = require('syracuse-ui/lib/alert/manager');
...
var _router = require('syracuse-ui/lib/controller/router');
var _ajax = require('syracuse-ui/lib/controller/ajax');

var _form = require('syracuse-ui/lib/controller/form');
...
var _dom = require('syracuse-ui/lib/utility/dom');
var _expression = require('syracuse-ui/lib/utility/expression');
var _locker = require('syracuse-ui/lib/utility/locker');
var _maximizer = require('syracuse-ui/lib/utility/maximizer');

var _bookmarks = require("syracuse-ui/lib/site/aside/bookmarks");
var _helpCenter = require('syracuse-ui/lib/help/helpCenter');
var _menusHelper = require('syracuse-ui/lib/menus/menusHelper');
var _picker = require('syracuse-ui/lib/menus/picker');
var _button = require('syracuse-ui/lib/menus/button');
...
var _qualityController = require('syracuse-ui/lib/quality/qualityController');
...
var _preference = require('syracuse-ui/lib/controller/preference');
var _icon = require("syracuse-ui/lib/utility/icon");
...
exports.Site = _helpers.defineClass(Site, DesktopPage, {
	...
	onMenuClick: function(options) {
		switch (options.menu.$bind) {
			...
			case "$documentation":
				...
				break;
			case "$help":
				_helpCenter.openPopup(options.menu);
				return false;
			case "$legal":
				_helpCenter.openLegalPage(options.menu);
				return false;
			default:
	...
	_focusBox: function(box, hasFocus) {
		if (box) {
			...
			if (target) {
				if (box != box.page) {
					_dom.toggleClass(target, "s-selected-box", hasFocus);
				}
			}
		}
	},
	...
	disableRequestSurvey: function(disabled) {
		...
		if (this.requestSurvey) {
			if (disabled) {
				_dom.toggleClass(this.requestSurvey, "s-on", false);
			} else {
		...
		if (!this._isdRequestSurveyDisabled) {
			if (this.requestSurvey.syraCount !== count) {
				if (count && !this.requestSurvey.syraCount) {
					_dom.toggleClass(this.requestSurvey, "s-on", true);
				}
				if (!count) {
					_dom.toggleClass(this.requestSurvey, "s-on", false);
				} else {
					_dom.setZIndex(this.requestSurvey, true);
				}
			}
		}
	...
	toggleMainPage: function(page, show) {
		_overManager.activateAll(page, show);
	...
	onMainPageChange: function($itemPage, onLoaded) {
		...
		syra_pageBuilder.load($itemPage, function(page) {
			...
			syra_form.showLastSavingDiagnoses(site.mainPage);
	...
	_onAfterLogon: function() {
		_dom.hide(this.layoutSlot, false);
		this.resizeItem();
		_url.history.start();
	...
	closePopups: function(box, event) {
		...
		event && syra_dd.stop();
		_overManager.closePopups(box, event);
	},
	...
	_renderHeader: function() {
		...
		if (this.$isLicenceTool) {
			_dom.hide(this.searcher && this.searcher.domItem, true);
	...
	addPageDesignerOpener: function() {
		this.pageDesignerBtn = syra_button.add({
			parent: this,
			slot: this.headerTop,
			...
			click: function() {
				...
				if (site.mainPage && site.mainPage.isFusionPage) {
					...
					var over = _overManager.getMostOverPage();
					page = over && over.page;
				}
		...
		syra_button.visibility(this.pageDesignerBtn, false);
	},
			...
			if (role || endpoint) {
				...
				for (var ii = 0, jj = pages.length; ii < jj; ii++) {
					...
					if (!page.isSecurityViewHidden && page.securityBreadcrumbSlot) {
						...
						_dom.empty(page.securityViewSlot);
						syra_button.remove(page.securityRoleFlag);
						syra_button.remove(page.securityEndpointFlag);
						if (role) {
							page.securityRoleFlag = syra_button.add({
								isIndicator: true,
								parent: page,
						...
						_dom.hide(page.securityBreadcrumbSlot, false);
					}
				}
			}
	...
	loadBox: function() {
		...
		_dom.hide(this.layoutSlot, true);
		...
		if (obj) {
			...
			if (obj.buttons && obj.buttons.length) {
				for (var ii = 0, jj = obj.buttons.length; ii < jj; ii++) {
					_button.dispose(obj.buttons[ii]);
				}
				delete obj.buttons;
			}
