...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	loadBox: function(siteOptions) {
		...
		//session event
		var idSession = this.getSessionId();
		var self = this;
		this.ioSocket = this.socketHandler.register({
			id: idSession,
			path: "/session",
			message: {
				"check": function(checkTest) {
					switch (checkTest.action) { // answer what server session want to check
						case "isAlive":
							self.socketHandler.emit(idSession, 'check', {
								"ok": true
							});
							break;
						default:
