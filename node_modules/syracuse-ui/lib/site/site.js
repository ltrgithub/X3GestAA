...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		this.history = this.urlMaker = this.screenDesigners = this.fusionHeaderFunctionLink = this.homeLink = null;
	...
	registerFlashWidget: function(field) {
		var boxParent = field.boxParent;
		field.page.hasFlashChildField = true;
		while (boxParent) {
		...
		if (show && page.domItem && page.displayed !== true) {
			...
			if (!show && page.displayed !== false) {
				...
				if (page.hasFlashChildField) {
					...
					syra_dom.removeChild(page.domItem);
				}
			}
		}
	},
	...
	checkUniqueSelection: function(hotspot) {
		if (this.selectedList) {
			if (this.selectedList.checkUniqueSelection === false) {
				...
				if (this.selectedList.selector) {
					var record;
					while (hotspot && !hotspot.isList) {
						if (hotspot.isRecordArticle) {
							record = hotspot;
						}
						hotspot = hotspot.articleParent;
					}
					if (hotspot && !hotspot.isList) {
						hotspot = null;
					}
