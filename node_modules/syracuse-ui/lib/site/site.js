...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		this._childPages = this.siteFunctions = this.browser = this.uiLocker = this.dom = this.layoutUpdater = this.layoutValidator = null;
	...
	onUserProfileChange: function(reloadMainPage) {
		...
		if (reloadMainPage) {
			...
			siteFunctions.showPageSecurity();
			if (self.mainPage) {
	...
	unregisterPage: function(page) {
		if (!page.isVignettePage) {
			var found = this._childPages.indexOf(page);
			if (found >= 0) {
				this._childPages.splice(found, 1);
			}
		}
	...
	resize: function(onBoxToggle) {
		...
		for (var ii = 0, jj = this._childPages.length; ii < jj; ii++) {
			this._childPages[ii].resizePage();
		}
		if (this.bookmarksManager) {
