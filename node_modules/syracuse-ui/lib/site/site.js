...
var FavoritesProvider = require('./favorites/favoritesProvider').FavoritesProvider;
...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		$(window).unbind();
		this.favoritesProvider = this.dragDropManager = this._uiLocker = this._uiLockerOverlay = this._uiLockerTimeout = null;
		...
		this.wcReqArray = this._favorites = null;
		...
		if (item && item.page) {
			...
			if (this.diagnosesPanel) {
				...
				if (this.mainPage) {
					...
					options.$message = (onlyOneDiagnose && message.$diagnoses[0].$message) || this.localize.siteMsgboxMsg;
					options.$type = "alert";
					options.$buttons = "ok";
	...
	logon: function(onAfterLogon) {
		...
		document.controller.loadWorkingCopy({
			...
			callback: function() {
				...
				//Hack lv for testing
				self._lvbuildFavorites();
				self.updateFavorites();
				onAfterLogon();
			}
		});

		// run jobs viewer if any trackers left in trackers list
		self._runJobsViewer();
		/*
         if (sessionStorage) {
         var trackersList = trackersList = sessionStorage.getItem("trackersList") !== "" ? JSON.parse(sessionStorage.getItem("trackersList")) || {} : {};
         var uuids = Object.keys(trackersList);
         for (var ii = 0, jj = uuids.length; ii < jj; ii++) {
         var params = trackersList[uuids[ii]];
         document.site.registerJobSyra(params.$location, undefined, params.$title, params.uuid);
         }
         }*/
	},
	...
	_lvbuildFavorites: function() {
		this.userProfile.dataset.favorites = [];
		var favorite = {};
		favorite.$uuid = helpers.uuid.generate();
		favorite.$serverIndex = 0;
		favorite.description = "Dictionnaire des écrans";
		favorite.$url = "?url=%2Ftrans%2Fx3%2Ferp%2FSEED%2F%24sessions%3Ff%3DGESAMK%252F2%252F%252FM%252F";
		this.userProfile.dataset.favorites.push(favorite);

		favorite = {};
		favorite.$uuid = helpers.uuid.generate();
		favorite.$serverIndex = 1;
		favorite.description = "Vignettes";
		favorite.$url = "?url=%2Fsdata%2Fsyracuse%2Fcollaboration%2Fsyracuse%2Fportlets%3Frepresentation%3Dportlet.%24query";
		this.userProfile.dataset.favorites.push(favorite);

	},
	updateFavorites: function() {
		var favorite;
		this.emptyDom(this._favorites);
		if (this.userProfile.dataset && this.userProfile.dataset.favorites && this.userProfile.dataset.favorites.length > 0) {
			for (var ii = 0; ii < this.userProfile.dataset.favorites.length; ii++) {
				favorite = document.createElement("a");
				favorite.setAttribute("data-s-picker", "s-site-favorite-link");
				favorite.setAttribute("href", "#");
				favorite.textContent = this.userProfile.dataset.favorites[ii].description;
				favorite.syraLink = this.userProfile.dataset.favorites[ii].$url;
				favorite.className = "s-site-favorite-link";
				this._favorites.appendChild(favorite);
			}
		}
	},
	...
	updateDocumentTitle: function() {
		...
		if (title) {
			...
			if (this.favoritesProvider && this.mainPage.openerHttpQuery) {
				this.favoritesProvider.save(this.mainPage.openerHttpQuery.fullUrl, title);
			}
		}
	},
	...
	resize: function(onBoxToggle) {
		...
		if (this.$$body && this.body) {
			if (this.authorPage && this.authorPage.onBeforeSiteResize) {
				this.authorPage.onBeforeSiteResize(this);
			} else {
				var newHeight = 0;
				if (this.header) {
					this._$$header = this._$$header || $(this.header);
					newHeight = this._$$header.outerHeight(true);
				}
				this.body.style.height = (this.$$layoutSlot.height() - newHeight) + "px";
			}
		}
	...
	_renderHeader: function() {
		...
		top.id = "s-site-header-top";
		this.header.appendChild(top);

		var bottom = document.createElement("div");
		bottom.id = "s-site-header-bottom";
		this.header.appendChild(bottom);
		...
		this.header.appendChild(this._sageLink);
		...
		bottomLeft.id = "s-site-header-bottom-left";
		bottom.appendChild(bottomLeft);

		div = document.createElement("div");
		div.id = "s-site-favorite-add";
		this.loadNewItem(bottomLeft.appendChild(div), {
			$bind: "$addfavorite",
			$category: "link",
			$noText: true,
			$skin: "s-site-favorite-add-link"
		});
		//Hack lv
		//div.firstChild.style.display = "";

		this._favorites = document.createElement("div");
		this._favorites.id = "s-site-header-favorites";
		bottom.appendChild(this._favorites);

		var bottomRight = document.createElement("div");
		bottomRight.id = "s-site-header-bottom-right";
		bottom.appendChild(bottomRight);

		// jobs viewer opener
		this._appendJobsViewerOpener(top);
	},
	...
	loadBox: function(siteOptions) {
		...
		this.$prototype.$links.$addfavorite = {
			$title: this.localize.siteAddFavorite
		};
