...
var expressionParser = require('./tools/expressionParser');
...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		this.history = this.urlMaker = this.screenDesigners = this.fusionHeaderFunctionLink = this.fusionHeaderHomeLink = this.homeLink = null;
		...
		this.fusionGateway = this._topPanelClass = this._topPanel = this._topPanelContainer = this.spyGateway = this.spy = null;
		this.headerLogo = this.headerTop = this.wcReqArray = this.bookmarks = null;
		RawPage.prototype.dispose.call(this);
		this.expressionParser = this.deltaManager = this.localizer = window.onerror = window.onbeforeunload = window.onunload = null;
	},
	...
	setZIndexValue: function(item, value) {
		if (item) {
	...
	onMainPageChange: function($itemPage) {
		...
		if (this.bookmarksBar) {
			this.bookmarksBar.checkMainPage(this.mainPage);
		}
	...
	_onLogon: function() {
		if (!this._isLocalChanging) {
			...
			if (this.userProfile) {
				...
				nameDiv.textContent = (this.userProfile.dataset.user.firstName ? this.userProfile.dataset.user.firstName + " " : "") + (this.userProfile.dataset.user.lastName ? this.userProfile.dataset.user.lastName : "");

				this._topPanelOpener.appendChild(nameDiv);

				// photo info
				var photoDiv = document.createElement("div");
				photoDiv.className = "s-site-user-photo";
				if (this.userProfile.dataset.user.photo) {
					this.loadNewItem(photoDiv, {
						$inplace: true,
						$bind: "photo",
						$category: "field",
						$field: {
							$type: "image",
							$url: this.userProfile.dataset.user.photo.$url
						}
					});
					var prototypeBackup = helpers.object.clone(this.$prototype);
					this.$prototype.$pluralType = "users";
					this.$prototype.$baseUrl = this.userProfile.$prototype.$baseUrl;
					this.$prototype.$key = this.userProfile.$prototype.$key;
					this.applyChange({
						$uuid: this.userProfile.dataset.user.$uuid,
						photo: this.userProfile.dataset.user.photo
					});
					delete this.dataset.$uuid;
					delete this.dataset.photo;
					this.$prototype = prototypeBackup;
				} else {
					var mn = document.createElement("a");
					mn.className = "s-site-user-nophoto";
					photoDiv.appendChild(mn);
				}
				this._topPanelOpener.appendChild(photoDiv);
			}
			this.resize();
		}
	},
	...
	_renderHeader: function() {
		...
		this.headerTop.appendChild(this._jobsViewerOpener);
		...
		this.landingHomeLink = this.loadNewItem(this.headerTop, {
			$bind: "$landingHome",
			$category: "link",
			$noText: true,
			$skin: "s-site-landing-home"
		});
	...
	appendLogoutButton: function() {
		this._logoutPicker = document.createElement("a");
		this._logoutPicker.className = "s-site-logout";
		this._logoutPicker.setAttribute("href", "#");
		this._logoutPicker.setAttribute("data-s-picker", "s-site-logout");
		this._logoutPicker.setAttribute("title", this.localize.siteLogoutLink);
		this.headerTop.appendChild(this._logoutPicker);
	},
	appendLogo: function() {
	...
	loadSiteLocalize: function() {
		...
		if (this.siteHelpLink) {
			this.siteHelpLink.mn.title = this.localize.siteHelpLink;
			this.homeLink.mn.title = this.localize.siteHomeLink;
			this.landingHomeLink.mn.title = this.localize.siteLandingHomeLink;
			this.navigationLink.mn.title = this.localize.siteNavigationLink;
	...
	loadNewPage: function($itemPage) {
		...
		item.$userPreferencesLink = $representation.$links && $representation.$links.$userPreferences;
		item.$userPreferences = $representation.userPreferences && $representation.userPreferences.content;
		...
		item.$pageRepresentationURL = $representation.$pageRepresentationURL;
		...
		this.localizer.applyPageLocalization(item);
		...
		item.loadBox($itemPage.initData, $representation.$diagnoses);
