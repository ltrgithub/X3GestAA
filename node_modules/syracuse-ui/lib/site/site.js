...
exports.Site = helpers.defineClass(Site, RawPage, {
    ...
    getFieldWidth: function($field, $isEditMode){
        ...
        if (!$width) {
            $width = this._fieldWidths.$default;
        }
        return $width;
    },
    setFieldWidth: function(field){
        ...
        if (field.$field.$scale) {
            $maxLength = field.$field.$scale + 2 * (field.$field.$scale / 3); //separator
        }
        if (field.$field.$precision) {
            $maxLength += field.$field.$precision + 1; //1for separator
        }
        if (!$maxLength) {
            $maxLength = field.$field.$maxLength || $width.$maxLength;
        }
        if ($maxLength) {
