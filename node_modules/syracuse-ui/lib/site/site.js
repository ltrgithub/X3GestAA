...
var _deltaManager = require('syracuse-ui/lib/site/tools/deltaManager');
...
var _maximizer = {
	getTopParentPage: function(page) {
		if (page.vignetteField) {
			page = this.getTopParentPage(page.vignetteField.page);
		}
		if (page.inlinePageHost) {
			page = this.getTopParentPage(page.inlinePageHost.record.page);
		}
	...
	applyToBar: function(bar, maximized) {
		if (bar) {
			if (maximized) {
	...
	maximizeItem: function(item, picker) {
		var page = this.getTopParentPage(item.page);
		...
		this.applyToBar(page.menuBar, item.isMaximized);
		if (page.fusionBar) {
			item.$item && item.$item.$isNavigationList && page.fusionBar.toggleBar(!item.isMaximized);
			this.applyToBar(page.fusionBar, item.isMaximized);
		}
		if (this.landingPageMaster) {
			this.applyToBar(syra_site.landingPageMaster.bar, item.isMaximized);
...
exports.Site = helpers.defineClass(Site, RawPage, {
	ensureArticleVisibility: function(article, resizeIfNeed, boxParent) {
		...
		layoutContent && this.layoutValidator.validateLayout(layoutContent, true);
	...
	onMainPageChange: function($itemPage) {
		...
		syra_url.history.addBackButton(this.mainPage);
	...
	_renderHeader: function() {
		...
		if (this.$isLicenceTool) {
			_domHelper.display(this.searcher && this.searcher.domItem, false);
	...
	refreshMainPageSecurity: function(page) {
		if (page && !page.disposed) {
			...
			if (syra_site.pageDesignerOpener && page.mainPageDesignerAccess != "unknow") {
	...
	loadBox: function() {
		...
		this.deltaManager = _deltaManager;
