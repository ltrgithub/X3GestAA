...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	showMessage: function(options, displayDisabled) {
		...
		this._msgBox.displayDisabled = displayDisabled; // property necessary for htmleditor msgbox
	...
	gotoHome: function() {
		var self = this;
		setTimeout(function() {
			var homeMenu = self._getHomeMenu();
			if (homeMenu) {
				homeMenu.click();
			}
		}, 20);
	...
	_renderHeader: function() {
		...
		this._sageLink.setAttribute("id", "s-site-sage");
	...
	loadBox: function(siteOptions) {
		...
		if (this._sageLink) {
			...
			if (homeMenu) {
				this._sageLink.setAttribute("title", homeMenu.mn.getAttribute("title"));
	...
	onUncaughtError: function(errorMsg, url, lineNumber, error) {
		if (errorMsg && errorMsg.indexOf("s-is-reload-requested") >= 0) {
			...
			if (self.mainPage) {
				...
				if (self.mainPage.externalAdapter) self.mainPage.externalAdapter.onUncaughtError(options);
			} else {
				//debugger; //TODO  
			}
		}
	...
	loadNewPage: function($itemPage) {
		...
		/*if (item.$prototype && item.$prototype.$localization) {
