...
var legalPageOpener = require('./legal/legalPageOpener');
...
exports.Site = helpers.defineClass(Site, RawPage, {
    ...
    _displayHelpLinks: function(menuItem){
        var self = this;
        if (!self.helpPopup) {
            self.helpPopup = document.site.dialogManager.openPopup(document.site, {
                content: self,
                slot: self.helpPopupSlot,
                position: {
                    my: "left top",
                    at: "left bottom",
                    of: $(menuItem.mn)
    ...
    onMenuClick: function(menuItem){
        ...
        if (menuItem.$bind && menuItem.$bind == '$help') {
            this._displayHelpLinks(menuItem);
            res = false;
        }
        if (menuItem.$bind == "$legal") {
            // open legal and technical information page
            legalPageOpener.openLegalPage(menuItem);
            res = false;
        }
        return res;
    },
    dispose: function(){
        ...
        this.browser = this.uiLocker = this.dom = this.layoutUpdater = this.layoutValidator = null;
        ...
        this.bookmarksManager = this.dragDropManager = null;
    ...
    onMainPageChange: function($itemPage){
        ...
        this.mainPage = this.loadNewPage($itemPage, true);
    ...
    resize: function(onBoxToggle){
        if (this.body) {
            var siteRect = this.dom.getBoundingClientRect(this.layoutSlot);
    ...
    toggleTopPanel: function(show, event){
        if (show !== false && !this._topPanel) {
            ...
            var rect = this._topPanelOpener.getBoundingClientRect();
            if (document.dir === 'rtl') {
                this._topPanelContainer.style.left = rect.left + "px";
            }
            else {
                this._topPanelContainer.style.right = (this.layoutSlot.getBoundingClientRect().right - rect.right) + "px";
            }
            this._topPanel = new (this._topPanelClass || SiteTopPanel)();
            this._topPanel.$prototype = this.$prototype;
            this._topPanel.page = this;
            this.dom.empty(this._topPanelContainer);
    ...
    _appendPicker: function(url, css, text, title){
    ...
    _renderHeader: function(){
        ...
        // help links
        this.helpPopupSlot = document.createElement("div");
        this.helpPopupSlot.className = "s-site-help-links-slot";
        for (var ii = 0, jj = this.$prototype.$helpLinks.length; ii < jj; ii++) {
            this.loadNewItem(this.helpPopupSlot, {
                $bind: this.$prototype.$helpLinks[ii],
                $category: "link",
        ...
        middleCell.appendChild(this._jobsViewerOpener);
        
        this._appendTopPanelOpener(middleCell);
        ...
        this.bookmarksManager.load(bookMarsSlot);
    ...
    loadBox: function(siteOptions){
        ...
        document.controller = new (this.siteOptions.controllerClass || Controller)();
    ...
    loadNewPage: function($itemPage, onMainPagechange){
