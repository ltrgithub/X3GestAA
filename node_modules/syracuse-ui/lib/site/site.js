...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	onUncaughtError: function(errorMsg, url, lineNumber, error) {
		if (errorMsg && errorMsg.indexOf("s-is-reload-requested") >= 0) {
			if (!sessionStorage.getItem("onSourceLoading")) {
				sessionStorage.setItem("onSourceLoading", "1");
				document.location.reload(true);
			} else {
				document.site.showDiagnoses({
					$diagnoses: [{
						$severity: "fatal",
						$message: "Automatic reloading of application has failed.You have to clear your cache (CTRL + SHIFT + DEL) and refresh (F5)"
					}]
				});
			}
		} else {
