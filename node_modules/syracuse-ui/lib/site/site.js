...
exports.Site = _helpers.defineClass(Site, DesktopPage, {
			...
			success: function() {
				if (!self.hasLogonFailed) {
					...
					_locale.setCurrent(function(err) {
						...
						self.$menus.site_sitemap.$title = syra_local.site_btn_sitemap;
	...
	onMainPageChange: function($itemPage, onLoaded) {
		...
		_manager.pageBuilder.load({
			...
			success: function(page) {
				...
				syra_nav.toggleMegaMenus(!(page.$isEditMode && !page.isLandingPage));
	...
	_renderHeader: function() {
		...
		if (!this.isOfficeSite) {
			...
			if (this.$device == "tablet") {
				this.btns.sitemap = this.addItem(this.headerBar, {
					$bind: "site_sitemap",
					$noText: true,
					$category: "link",
					$skin: "s-header-link s-header-sitemap",
		...
		if (this.$device !== "tablet") {
			this.btns.site_sitemap = this.addItem(this.footer, {
				$bind: "site_sitemap",
				$category: "link",
				$skin: "s-footer-sitemap",
				$shortCutTip: _manager.shortCuts.tip.goToNavigationPage
			});
		}


		!this.isOfficeSite && syra_ajax.survey.addFlag(this.footer);
		...
		this.megaMenusBar = syra_dom.ul("s-nav-mega-modules", (!this.isOfficeSite && this.$device == "desktop") ? this.header : undefined);
	},
	...
	switchGPDC: function() {
		if (!this.btns.gpdc) {
			this.btns.gpdc = syra_button.add({
				parent: this,
				slot: this.footer,
				text: "gpdc - on",
				css: "s-footer-gpdc s-selected",
				click: function() {
					syra_dom.toggleClass(this.link, "s-selected", syra_site.gpdc = !syra_site.gpdc);
					this.link.textContent = syra_site.gpdc ? "gpdc - on" : "gpdc - off";
				}
			});
		}
	},
