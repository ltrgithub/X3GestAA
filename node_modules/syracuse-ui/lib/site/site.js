...
var _manager = {
	...
	dataset: require('syracuse-ui/lib/controller/dataset'),
	tip: require('syracuse-ui/lib/over/tip'),
	widgets: require('syracuse-ui/lib/site/widget/widgets'),
	...
	layoutValidator: require("syracuse-ui/lib/article/layout/layoutValidator"),
	layoutUpdater: require("syracuse-ui/lib/article/layout/layoutUpdater"),
	maximizer: require('syracuse-ui/lib/utility/maximizer'),
	expression: require('syracuse-ui/lib/utility/expression'),
	article: require("syracuse-ui/lib/article/article"),
...
var _config = {
	size: {
		minWidth: 650,
		overResizeMinWidth: 400,
		overResizeMinHeight: 300
	}
};
...
exports.Site = _helpers.defineClass(Site, DesktopPage, {
	...
	validateLayoutContent: function(article, boxParent) {
		...
		layoutContent && _manager.layoutValidator.validate(layoutContent, true);
	},
	...
	onMenuClick: function(options) {
		switch (options.menu.$bind) {
			...
			case "$legal":
				...
				return false;
			default:
				if (this.bookmarks && options.menu.$bind && options.menu.$bind.split("-")[0] === "bookmark") {
					this.bookmarks.onLinkClick(options.menu);
					return false;
				}
				break;
		}
	...
	disableRequestSurvey: function(disabled) {
		...
		if (this.requestSurvey) {
			if (disabled) {
				_manager.dom.toggleClass(this.requestSurvey, "s-on", false);
			} else {
		...
		if (!this._isdRequestSurveyDisabled) {
			if (this.requestSurvey.syraCount !== count) {
				if (count && !this.requestSurvey.syraCount) {
					_manager.dom.toggleClass(this.requestSurvey, "s-on", true);
				}
				if (!count) {
					_manager.dom.toggleClass(this.requestSurvey, "s-on", false);
				} else {
					_manager.dom.setZIndex(this.requestSurvey, true);
				}
			}
		}
	...
	onMainPageChange: function($itemPage, onLoaded) {
		...
		_manager.pageBuilder.load($itemPage, function(page) {
			var site = syra_site;
			site.mainPage = page;
			...
			_manager.url.history.addBackButton(site.mainPage);
			_manager.form.showLastSavingDiagnoses(site.mainPage);
	...
	_renderHeader: function() {
		...
		this.siteHelpLink = this.addItem(this.headerTop, {
		...
		_manager.trackers.addOpener(middleCell);

		_topPanel.addOpener(middleCell);

		this.addLogoutButton();

		this.homeLink = this.addItem(this.headerTop, {
			$bind: "$home",
			$category: "link",
			$noText: true,
			$skin: "s-site-home",
		...
		this.navigationLink = this.addItem(this.headerTop, {
			$bind: "$navigation",
			$category: "link",
			$noText: true,
	...
	addPageDesignerOpener: function() {
		this.pageDesignerBtn = _manager.button.add({
			parent: this,
			slot: this.headerTop,
		...
		_manager.button.visibility(this.pageDesignerBtn, false);
	},
	...
	refreshMainPageSecurity: function(page) {
		if (page && !page.disposed) {
			...
			if (role || endpoint) {
				...
				var pages = page ? [page] : _manager.pageBuilder.getLoaded();
				for (var ii = 0, jj = pages.length; ii < jj; ii++) {
					...
					if (!page.isSecurityViewHidden && page.securityBreadcrumbSlot) {
						...
						_manager.dom.empty(page.securityViewSlot);
						_manager.button.remove(page.securityRoleFlag);
						_manager.button.remove(page.securityEndpointFlag);
						if (role) {
							page.securityRoleFlag = _manager.button.add({
								isIndicator: true,
								parent: page,
						...
						if (endpoint) {
							page.securityEndpointFlag = _manager.button.add({
								isIndicator: true,
								parent: page,
								slot: page.securityViewSlot,
								text: endpoint,
						...
						_manager.dom.hide(page.securityBreadcrumbSlot, false);
					}
				}
			}
	...
	loadBox: function() {
		...
		this.config = _config;
		window.syra_router = _manager.router;
		window.syra_help = _manager.help;
		window.syra_icon = _manager.icon;
		window.syra_ajax = _manager.ajax;
		window.syra_preference = _manager.preference;
		window.syra_dom = _manager.dom;
		window.syra_item = _manager.item;
		window.syra_article = _manager.article;
		window.syra_expression = _manager.expression;
		window.syra_maximizer = _manager.maximizer;
		window.syra_localizer = _manager.localizer;
		window.syra_locker = _manager.locker;
		window.syra_layoutValidator = _manager.layoutValidator;
		window.syra_layoutUpdater = _manager.layoutUpdater;
		window.syra_over = _manager.over;
		window.syra_dataset = _manager.dataset;
		window.syra_url = _manager.url;
		window.syra_tip = _manager.tip;
		window.syra_form = _manager.form;
		window.syra_trackers = _manager.trackers;
		window.syra_alert = _manager.alert;
		window.syra_pageBuilder = _manager.pageBuilder;
		window.syra_shortCuts = _manager.shortCuts;
		window.syra_button = _manager.button;
		window.syra_menus = _manager.menus;
		window.syra_picker = _manager.picker;
		window.syra_fields = _manager.fields;
		window.syra_site = this;
		window.syra_local = _locale.resources(module)();
		window.syra_widgets = _manager.widgets;
	...
	onUncaughtError: function(errorMsg, url, lineNumber, error) {
		...
		if (self.mainPage) {
			var options = {
				...
				error: error,
				doEvent: function() {
					var message = options.errorMsg;
					message += error ? "\nStack:\n" + error.stack : "";
					_manager.alert.error(message);
				}
			};
			if (self.mainPage.externalAdapter)
				self.mainPage.externalAdapter.onUncaughtError(options);
		} else {
			_manager.alert.error(errorMsg);
		}
	},
