...
exports.Site = helpers.defineClass(Site, RawPage, {
    ...
    loadBox: function(siteOptions){
        ...
        window.onbeforeunload = this.onBeforeUnload; // handler for session closing in case of logout or browser close
        return this;
    },
    onBeforeUnload: function(event){
        // getting convergence workbooks
        var books;
        if (document.controller && document.controller.fusionGateway && document.controller.fusionGateway._books) {
            books = document.controller.fusionGateway._books;
            if (books.length > 0) {
                for (var ii = 0; ii < books.length; ii++) {
                    if (books[ii].fusionSite && books[ii].fusionSite.onNotifyLogout) {
                        //TODO
                    }
                }
            }
            //            if (event.preventDefault) {
            //				console.log("before eventpreventdefault");
            //                event.preventDefault();
            //				console.log("after eventpreventdefault");
            //            }
