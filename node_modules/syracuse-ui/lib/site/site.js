...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	logon: function(onAfterLogon) {
		...
		var urlSeg = syra_site.history.getUrlSegment();
		var $userProfileUrl = self.userProfile.getLogonUrl();
		var $userProfileSeg = syra_site.history.getUrlSegment($userProfileUrl);
		...
		if (!prevdataset && (selectedEndpoint || selectedRole)) {
			var $baseUrl = syra_site.urlMaker.getBaseUrl($userProfileSeg);
			prevdataset = {
				$etag: 0
			};
			if (selectedRole) {
	...
	disableRequestSurvey: function(disabled) {
		...
		if (this.requestSurvey) {
			if (disabled) {
				syra_site.dom.toggleClass(this.requestSurvey, "s-on", false);
			} else {
		...
		if (!this._isdRequestSurveyDisabled) {
			if (this.requestSurvey.syraCount !== pendingRequests) {
				if (pendingRequests && !this.requestSurvey.syraCount) {
					syra_site.dom.toggleClass(this.requestSurvey, "s-on", true);
				}
				if (!pendingRequests) {
					syra_site.dom.toggleClass(this.requestSurvey, "s-on", false);
				} else {
	...
	_renderHeader: function() {
		...
		syra_site.searcher = _searcher.load(this.headerTop);
	...
	loadBox: function(siteOptions, loader) {
		...
		this.isTabletDevice = this.$device == "tablet";
		...
		this.$navigation = {
			...
			office: {
				$title: "Office",
				$url: "/msoffice/lib/excel/html/config.html?url=%3Frepresentation%3Dexcelconfig.%24dashboard"
			},
			devsos: {
		...
		window.syra_controller = new Controller();
		...
		this.requestControllers = this.requestControllers || {};
		this.requestControllers.http = require("syracuse-ui/lib/controller/httpController");
		this.agents = {};
