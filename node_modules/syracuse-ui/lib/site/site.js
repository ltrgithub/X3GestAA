...
exports.Site = helpers.defineClass(Site, RawPage, {
    dispose: function(){
        ...
        $(window).unbind();
        this._topPanelClass = this._topPanel = this._topPanelContainer = this.awPageSlot = this.spyGateway = this.spy = this.lastFocusField = null;
    ...
    showDiagnoses: function(message, item, options){
        ...
        if (item && item.page && item.page.diagnosesPanel) {
            ...
            if (item && item.page && !item.page.diagnosesPanel) {
                item.page.showDiagnoses(message, options);
            }
                ...
                if (this.diagnosesPanel) {
                    ...
                    if (this.mainPage) {
                        ...
                        options.$message = localize.s_msgbox_msg;
                        options.$type = "alert";
                        options.$buttons = "ok";
                        options.$title = localize.s_msgbox_title;
        ...
        if (useSmartMode) {
            ...
            if ($titleLength > $maxLength) {
                if (($titleLength - $maxLength) <= 4) {
                    ...
                    if (isSortable) {
                        $maxLength += 2;
                        $maxLength = Math.max(7, $maxLength);
                    }
                    else {
    ...
    appendJobsViewer: function(jobsViewer){
        if (this.layoutSlot) {
            this.layoutSlot.appendChild(document.site.jobsViewer ? document.site.jobsViewer._item : jobsViewer._item);
        }
    },
    resize: function(onBoxToggle){
        if (this.$$body && this.body) {
            ...
            if (this._header) {
                ...
                var $$header = (this.authorPage) ? $(this.authorPage.layoutSlot) : this._$$header;
        ...
        if (this.resizeListeners) {
            ...
            for (var ii = 0, jj = ids.length; ii < jj; ii++) {
                this.resizeListeners[ids[ii]].onWindowResize(onBoxToggle);
            }
        }
    },
    ...
    toggleTopPanel: function(show){
        if (show !== false && !this._topPanel) {
            ...
            $(this._topPanel.layoutSlot = this._topPanelContainer).empty().show();
    ...
    _appendTopPanelOpener: function(layoutSlot, topPanelClass){
        ...
        this._topPanelOpener.setAttribute("id", "s-site-top-pn-opener-link");
    ...
    loadBox: function(siteOptions){
        ...
        window.onbeforeunload = disconnectUtils.onBeforeUnload; // handler for session closing in case of page leaving or refresh (triggered before unload, cancelable)
        window.onunload = disconnectUtils.onUnload; // handler in case of page leaving or refresh (triggered after onbeforeunload, not cancelable)
        return this;
    },
    ...
    setClientTimer: function(){
        /*
         this.clientTimeOut = this.clientTimeOut || "TODO";
         var options = {};
         setTimeout(function(){
    ...
    loadNewPage: function($itemPage){
        ...
        if ($itemPage.$representation.$views) {
            item.$prototype.$links = item.$prototype.$links || {};
            for (var ii = 0, jj = $itemPage.$representation.$views.length; ii < jj; ii++) {
                var $view = $itemPage.$representation.$views[ii];
                $view.$isViewVariant = true;
                item.$prototype.$links["views-" + ii] = $view;
            }
        }
        ...
        $itemPage.$pageCategoryClass = $itemPage.boxParent = $itemPage.layoutSlot = $itemPage.$representation = $itemPage.onBeforeLoadPage = null;
        return item;
    },
