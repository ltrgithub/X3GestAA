...
_helpers.defineClass(Site, DesktopPage, {
	clone: function(obj) {
			...
			success: function() {
				if (!self.hasLogonFailed) {
					...
					_locale.setCurrent(function(err) {
						...
						self.$menus.$navigation.$title = syra_local.site_btn_sitemap;
	...
	closePopups: function(box, event) {
		this.userProfile.toggle(false, event, true);
		event && syra_dd.stop();
	...
	_renderHeader: function() {
		...
		this.btns.logout = _manager.button.add({
			...
			iconOnly: true,
			css: "s-header-link s-header-logout",
			click: function() {
				syra_site.logout();
			}
		});
		...
		this.searcher = _searcher.load(this.headerBar);
