...
exports.Site = helpers.defineClass(Site, RawPage, {
	ensureArticleVisibility: function(article, resizeIfNeed, boxParent) {
		...
		layoutContent && this.layoutValidator.validate(layoutContent, true);
	...
	onMainPageChange: function($itemPage) {
		...
		syra_site.dom.hide(this.body, true);
		...
		syra_workingCopy.showLastSavingDiagnoses(this.mainPage);
	...
	registerScreenDesigner: function(add, designer) {
		if (add) {
			...
			syra_site.dom.hide(this.header, true);
			...
			if (this.screenDesigners) {
				...
				if (this.screenDesigners.length == 0) {
					...
					syra_site.dom.hide(this.header, false);
				} else {
					var top = this.screenDesigners && this.screenDesigners.length && this.screenDesigners[this.screenDesigners.length - 1];
					if (top) {
	...
	loadBox: function() {
		...
		this.dom = _domHelper;
		this.layoutUpdater = _layoutUpdater;
		this.layoutValidator = _layoutValidator;
		...
		syra_site.dom.hide(this.layoutSlot, true);
