	...
	addPageDesignerOpener: function() {
		this.pageDesignerBtn = syra_menus.button.add({
			...
			btnclick: function() {
				...
				if (page) {
					if (site.currentQuickEdit &&
						site.currentQuickEdit.needCloseConfirm(page, function() {
							exports.onOpenerClick();
						})) {
						return;
					}
					site.switchItemDesigner(page, true);
				}
			}
		});
	...
	refreshMainPageSecurity: function(page) {
		if (page && !page.disposed) {
			...
			if (role || endpoint) {
				...
				var pages = page ? [page] : this.pageLoader.getLoaded();
				for (var ii = 0, jj = pages.length; ii < jj; ii++) {
					page = pages[ii];
					if (page.securityViewSlot) {
						this.dom.empty(page.securityViewSlot);
