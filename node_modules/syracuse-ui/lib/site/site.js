...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		window.syra_menus = window.syra_local = window.syra_site = window.syra_controller = null;
	},
		...
		syra_controller.loadWorkingCopy({
			...
			callback: function() {
				if (!self.userProfile.hasLogonFailed) {
					...
					var localePreferences = globals.context.localePreferences = dataset.selectedLocale;
					self.currentLangCode = localePreferences && localePreferences.code || "en";
					self.userProfile.setEncodedValue();
					locale.setCurrent(function(err) {
						...
						self._renderHeader();
						self.siteFunctions.showUserIdentity(self.userProfile);
						onAfterLogon();
						delete self.userProfile.onLogonProcessing;
					}, self.currentLangCode, localePreferences);
				}
			}
		});
