...
var _navigator = require('syracuse-ui/lib/page/navigation/navigator');
...
exports.Site = helpers.defineClass(Site, DesktopPage, {
	...
	getSize: function() {
		if (this.layoutSlot && this.size == undefined) {
			...
			this.size.dlg = {
				maxHeight: Math.max((this.size.height - (this.body ? this.size.body.top : 0)) - 50, 100),
	...
	addPageDesignerOpener: function() {
		...
		syra_menus.button.disable(this.pageDesignerBtn, true);
	},
	...
	addRoleEndpoint: function() {
		...
		if (endpoint && endpoint.description) {
			var flag = (this.securityEndpointFlag = syra_menus.button.add({
				isIndicator: true,
				parent: this,
				slot: this.headerTop,
			...
			if (endpoint.nature) {
				flag.insertBefore(syra_dom.addDiv("s-security-edp-nature s-nature-" + endpoint.nature), flag.lastChild);
			}
		}
	},
		...
		if (page && !page.disposed) {
			...
			if (role || endpoint) {
				...
				for (var ii = 0, jj = pages.length; ii < jj; ii++) {
					...
					if (!page.isSecurityViewHidden && page.securityBreadcrumbSlot) {
						...
						if (endpoint && endpoint.description) {
							var flag = (page.securityEndpointFlag = syra_menus.button.add({
								isIndicator: true,
								parent: page,
								slot: page.securityViewSlot,
								text: endpoint.description,
			...
			*/
			if (this.pageDesignerBtn && page.mainPageDesignerAccess != "unknow") {
				...
				if (this.userProfile.hasDesignRight()) {
					var show = page.mainPageDesignerAccess != "disabled";
					syra_menus.button.disable(this.pageDesignerBtn, !show);

					//this.pageDesignerBtn.link.style.visibility = show ? "" : "hidden";					
				} else {
					this.pageDesignerBtn.link.style.display = "none";
				}
			}
		}
	},
	...
	addLogo: function(slot) {
		...
		var css = "s-site-header-logo";
		if (this.productName) {
			css += "-" + this.productName.textContent.trim().toLowerCase();
		}
