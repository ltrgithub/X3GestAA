...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	dispose: function() {
		...
		this.desktopPages = this._authoringLink = this._newTabLink = null;
		this._pageDialogs = this.autoCloseDialogs = this.fusionGateway = this._topPanelClass = this._topPanel = this._topPanelContainer = this.awPageSlot = this.spyGateway = this.spy = null;
	...
	onMainPageChange: function($itemPage) {
		...
		this.setAuthoringVisibility(!this.mainPage.isDashBoard);
	...
	applyLocalize: function() {
		this._authoringLink.title = this.localize.siteAuthoringOpen;
		if (this._newTabLink) {
	...
	openAuthoringPage: function(open, selectedItem) {
		if (open) {
			this.toggleTopPanel(false);
			...
			if (!this.awPageSlot) {
				...
				this.awPageSlot.id = "s-aw-page";
				this.awPageSlot.style.display = "none";
				this._header.parentNode.insertBefore(this.awPageSlot, this._header.nextSibling);
			}
			this.emptyDom(this.awPageSlot);
			this.awPageSlot.style.display = "";
			this.authorPage.beginAuthoring(this.awPageSlot, selectedItem);
		} else {
			if (this.authorPage && this.authorPage.targetPage) {
				this.authorPage.targetPage.showDiagnoses({
					$diagnoses: null
				});
			}
	...
	setAuthoringVisibility: function(isVisible) {
		if (this._authoringLink) {
			if (isVisible == undefined) {
				isVisible = !(this.userProfile.getAuthoringLevel() === "none");
			}
			this._authoringLink.style.display = isVisible ? "" : "none";
		}
	},
	_renderHeader: function() {
		this._header = document.createElement("header");
		this._header.id = "s-site-header";
		...
		this._authoringLink.style.display = "none";
	...
	ensureUILocker: function() {
		if (this._uiLocker && this._uiLocker.style.display !== "none") {
			...
			var top = 0;
			if (this._header && this._header.clientHeight) {
				top = this._header.clientHeight;
				height -= top;
			}
