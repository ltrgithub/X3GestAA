...
var diagnoseCenter = require('syracuse-ui/lib/diagnoses/diagnoseCenter');

var RawPage = require("syracuse-ui/lib/page/rawPage").RawPage;
var _controller = require('syracuse-ui/lib/controller/controller');
var eventListener = require('syracuse-ui/lib/events/eventListener');
var locale = require('syracuse-core/lib/locale');

var siteFunctions = require("syracuse-ui/lib/site/tools/siteFunctions");
var pageLoader = require("syracuse-ui/lib/page/pageLoader");
var pageHelper = require('syracuse-ui/lib/page/pageHelper');
...
var FusionGateway = require('syracuse-ui/lib/fusion/fusionGateway').FusionGateway;
...
var _bookmarksManager = require("syracuse-ui/lib/page/bookmarks/bookmarksManager");

var _store = require('syracuse-ui/lib/site/tools/store');
...
var _topPanel = require('syracuse-ui/lib/site/tools/topPanel');
var _searcher = require('syracuse-ui/lib/search/searcher');
var _testManager = require('syracuse-ui/lib/tests/testManager');
var jsurl = require('jsurl/lib/jsurl');

var _widgetsLibrary = require('./widgetsLibrary');
...
function _ensureSectionVisibility(section) {
	if (section.isVignetteField) {
		return true;
	}
	var isVisible = false;
	if (section.$item && section.$item.$isEmptyVisible) {
		...
		if (section.childrenSection) {
			for (var ii = 0, jj = section.childrenSection.length; ii < jj; ii++) {
				if (_ensureSectionVisibility(section.childrenSection[ii])) {
					isVisible = true;
				}
			}
		}
	...
	if (isVisible == section.isInvisible || section.isInvisible === undefined) {
		...
		if ((section.isInvisible !== section.$isHidden) || (!section.isInvisible && !section.$isHidden && section.isItemHidden)) {
			section.showItem(!(section.isInvisible || section.$isHidden));
		}
	}
	return isVisible;
}
...
exports.Site = helpers.defineClass(Site, RawPage, {
		...
		if (article.childrenSection) {
			for (var mm = 0, kk = article.childrenSection.length; mm < kk; mm++) {
				if (!boxParent || boxParent.isChild(article.childrenSection[mm])) {
					_ensureSectionVisibility(article.childrenSection[mm]);
				}
			}
		}
		if (article.records) {
			for (var ii = 0, jj = article.records.length; ii < jj; ii++) {
				this.ensureArticleVisibility(article.records[ii]);
			}
		}
	...
	loadStyleSheet: function(name) {
		if (!_styleSheets[name]) {
			...
			link.setAttribute("href", _styleSheetPath + name);
			if (_themeStyleSheet) {
				syra_site.documentHeader.insertBefore(link, _themeStyleSheet);
			} else {
				syra_site.documentHeader.appendChild(link);
			}
			return link;
		}
	},
	...
	checkMainPageBookmarkStatus: function() {
		this.bookmarksManager && this.bookmarksManager.checkMainPageStatus();
	},
	...
	logout: function(doPost) {
		if (!doPost && this.fusionGateway) {
			this.fusionGateway.onUserActionUnload(function() {
	...
	onMenuClick: function(menuItem) {
		switch (menuItem.$bind) {
			...
			case "$legal":
				...
				return false;
			default:
				if (this.bookmarksManager && menuItem.$bind && menuItem.$bind.split("-")[0] === "bookmark") {
					this.bookmarksManager.onLinkClick(menuItem);
					return false;
				}
				break;
		}
	...
	dispose: function() {
		...
		this.bookmarksManager && this.bookmarksManager.dispose();
		_trackersManager.dispose();
		_topPanel.dispose();
		_dialogManager.dispose();
		this.uiLocker && this.uiLocker.dispose();
		siteFunctions.dispose(this);
		syra_diagnose && syra_diagnose.dispose();
		syra_controller && syra_controller.dispose();
		_helpCenter.dispose();
		RawPage.prototype.dispose.call(this);
		syra_store && syra_store.dispose();
		window.onerror = window.onbeforeunload = window.onunload = null;
		window.syra_page = window.syra_menus = window.syra_local = window.syra_site = window.syra_controller = null;
		window.syra_diagnose = window.syra_store = window.syra_fields = window.syra_trackers = null;
	...
	showDiagnoses: function(message, item, options) {
		syra_diagnose.showDiagnoses(message, item, options);
	},
	logon: function(onAfterLogon) {
		...
		var urlSeg = self.history.getUrlSegment();
		var $userProfileUrl = self.userProfile.getLogonUrl();
		var $userProfileSeg = self.history.getUrlSegment($userProfileUrl);
		...
		if (!prevdataset && (selectedEndpoint || selectedRole)) {
			var $baseUrl = self.urlMaker.getBaseUrl($userProfileSeg);
			prevdataset = {
				$etag: 0
			};
			if (selectedRole) {
			...
			callback: function() {
				if (!self.hasLogonFailed) {
					...
					locale.setCurrent(function(err) {
						window.syra_local = locale.resources(module)();
	...
	disableRequestSurvey: function(disabled) {
		...
		if (this.requestSurvey) {
			if (disabled) {
				this.dom.toggleClass(this.requestSurvey, "s-on", false);
			} else {
	...
	notifyRequestSurvey: function(pendingRequests) {
		...
		pendingRequests = Math.max(pendingRequests || 0, 0);
		this.layoutSlot.setAttribute("data-s-q-request-count", pendingRequests);
		this.requestSurvey.textContent = pendingRequests;
		if (!this._isdRequestSurveyDisabled) {
			if (this.requestSurvey.syraCount !== pendingRequests) {
				if (pendingRequests && !this.requestSurvey.syraCount) {
					this.dom.toggleClass(this.requestSurvey, "s-on", true);
				}
				if (!pendingRequests) {
					this.dom.toggleClass(this.requestSurvey, "s-on", false);
				} else {
					this.dom.setZIndex(this.requestSurvey, true);
				}
			}
		}
		this.requestSurvey.syraCount = pendingRequests;
	},
	toggleMainPage: function(page, show) {
		...
		if (show && page.domItem && page.displayed !== true) {
			...
			if (!show && page.displayed !== false) {
				page.displayed = show;
				this.dom.removeChild(page.domItem);
			}
		}
	},
	...
	onMainPageChange: function($itemPage) {
		...
		this.refreshBrowserTitle(this.mainPage);
	...
	drawBox: function() {
		...
		self.notifyRequestSurvey(); //inject on loading for test robot
		self.logon(function() {
			self._onAfterLogon();
		});
	},
	_onAfterLogon: function() {
		...
		this.resize();
		this.history.start();
		this.bookmarksManager && this.bookmarksManager.loadBookmarks();
	},
	...
	onTopPanelClick: function() {
		_topPanel.toggle(true);
	},
	resize: function(onBoxToggle) {
		this.layoutRect = null;
		this.autoViewMode = (window.innerWidth < 900 || (window.innerHeight > window.innerWidth)) ? "float" : "dock";
		if (this.body) {
		...
		this.bookmarksManager && this.bookmarksManager.onResize();
		this.uiLocker.ensureLock();
		this.ensureLogoSize();
	},
	getProductName: function() {
		if (!this.productName) {
			this.productName = document.createElement("span");
			this.productName.className = 's-m-product s-m-product-font';
			this.productName.textContent = ""; // to determine with the name of the product that will come back from the server
		}
		return this.productName;
	},
	updateProductName: function(productName) {
		// remove size for logo
		this.removeLogoSize();
		var productName = this.getProductName().textContent = productName;
		this.ensureLogoSize();
	},
	removeLogoSize: function() {
		if (this.logoButton)
			$(this.logoButton).width("");
	},
	ensureLogoSize: function() {
		if (this.logoButton && this.brandName && this.productName)
			$(this.logoButton).width(this.brandName.offsetWidth + this.productName.offsetWidth);
	},
	_renderHeader: function() {
		...
		/*this.designerHelpLink = this.loadNewItem(null, {
		...
		this.bookmarksManager = _bookmarksManager;
		this.bookmarksManager.load();

		if (this.$isLicenceTool) {
			domHelper.display(this.searcher && this.searcher.domItem, false);
	...
	addPageDesignerOpener: function() {
		this.pageDesignerOpener = syra_menus.addIconButton(syra_local.userProfile_siteAuthoringOpen, "s-page-designer-opener", "onPageDesignerOpenerClick");
		this.pageDesignerOpener.syraItem = this.id;
		this.pageDesignerOpener.style.visibility = "hidden";
		this.headerTop.appendChild(this.pageDesignerOpener);
	},
	showPageDesignerOpener: function(show) {
		if (this.pageDesignerOpener) {
			this.pageDesignerOpener.style.visibility = show ? "" : "hidden";
		}
	},
	...
	onPageDesignerOpenerClick: function() {
		var page;
		this.topPanel.toggle(false);
		if (this.mainPage && this.mainPage.isFusionPage) {
			page = this.fusionGateway.activatedBook.selectedSheet;
		} else {
			page = this.dialogManager.getTopDialogPage();
			if (page) {
				page = page._content;
			}
		}
		...
		if (page) {
			if (this.currentQuickEdit &&
				this.currentQuickEdit.needCloseConfirm(page, function() {
					exports.onOpenerClick();
				})) {
				return;
			}
			this.switchItemDesigner(page, true);
		}
	},
	loadBox: function() {
		this.documentHeader = document.getElementsByTagName("head")[0];
		this.isDocumentRTL = document.dir === 'rtl';
		...
		window.syra_test = _testManager;
		window.syra_trackers = _trackersManager;
		window.syra_diagnose = diagnoseCenter;
		window.syra_page = pageHelper;
		...
		this.$navigation = {
			...
			mobileDesktopAuthoring: {
				$title: "Mobile Desktop Personalization",
				"$description": "Mobile personalization with desktop client",
				$url: "/syracuse-main/html/main-mobile.html?url=%3Frepresentation%3Dhome.%24dashboard"
			},
			devsos: {
				$title: "Developer samples/SOS",
				$url: "?representation=devsos.$dashboard"
			},
			arrayTest: {
				$title: "UI test Pages",
		...
		window.syra_store = _store;
		this.widgetsLibrary = _widgetsLibrary;
		...
		if (!this.$officeSite) {
			this.fusionGateway = new FusionGateway();
		...
		this.layoutSlot.setAttribute("data-s-q-request-count", 0);
		this.layoutSlot.setAttribute("data-s-q-uilock", 0);

		eventListener.load();


		this.initializeNewItem(this, this.$item);
	...
	addLogo: function(slot) {
		...
		btn.appendChild(this.brandName);
		btn.appendChild(this.getProductName());
		this.logoButton = btn;
		slot.appendChild(btn);
		this.onLogoClick = function() {
			syra_menus.click.home();
		};
	},
	addLogoutButton: function() {
		...
		btn.syraItem = this.id;
		this.headerTop.appendChild(btn);
	...
	ensureRecordUniqueSelection: function(list, onSelect) {
		var record;
		...
		if (this._selectedList && this._selectedList.selector) {
			if (list != this._selectedList || (record && !record.$isSelected)) {
				this._selectedList.selector.unSelectAll();
			}
		}
		if (onSelect) {
