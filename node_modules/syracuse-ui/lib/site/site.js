...
exports.Site = helpers.defineClass(Site, RawPage, {
	...
	loadNewPage: function($itemPage) {
		...
		if (item.openerUrlSegments.params) {
			if (item.openerUrlSegments.params.where) {
				item.$prototype.$where = escape(item.openerUrlSegments.params.where);
			}
			item.$prototype.$filter = item.openerUrlSegments.params.filter;
		}
