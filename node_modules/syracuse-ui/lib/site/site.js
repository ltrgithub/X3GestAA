...
exports.Site = _helpers.defineClass(Site, DesktopPage, {
			...
			success: function() {
				if (!self.hasLogonFailed) {
					...
					_locale.setCurrent(function(err) {
						...
						self.$menus.$navigation.$title = syra_local.site_btn_navigation;
	...
	_renderHeader: function() {
		...
		(this.productNameBtn = this.addItem(this.headerTop, {
			$bind: "$home",
			$noText: true,
			$category: "link",
			$skin: "s-site-logo"
		})).setProductName = function(name) {
			this.domItem.className = this.$skin = "s-site-logo" + (name ? (" s-site-logo-" + name.trim().toLowerCase()) : "");
		};
		this.productName && this.productNameBtn.setProductName(this.productName);

		var middleCell = syra_dom.div("s-site-header-middle-cell", this.headerTop);

		_manager.trackers.addOpener(middleCell);

		_topPanel.addOpener(middleCell);

		this.btns.logout = _manager.button.add({
			parent: this,
			slot: middleCell,
			text: syra_local.site_btn_logout,
			iconOnly: true,
			css: "s-site-btn-logout",
			click: function() {
				syra_site.logout();
			}
		});
		...
		if (role) {
			...
			this.securityRoleFlag = syra_button.add({
				isIndicator: true,
				parent: this,
				slot: middleCell,
				text: role,
		...
		if (endpoint && endpoint.description) {
			var flag = (this.securityEndpointFlag = syra_button.add({
				isIndicator: true,
				parent: this,
				slot: middleCell,
		...
		this.btns.home = this.addItem(middleCell, {
			$bind: "$home",
			$category: "link",
			$skin: "s-site-btn-home",
			$shortCutTip: _manager.shortCuts.tip.goToDefaultHome
		});

		this.btns.favorites = _manager.button.add({
			parent: this,
			slot: middleCell,
			text: syra_local.site_btn_favorites,
			css: "s-site-btn-favorites",
			click: function() {}
		});

		this.btns.navigation = this.addItem(middleCell, {
			$bind: "$navigation",
			$category: "link",
			$skin: "s-site-btn-navigation",
		...
		this.btns.help = this.addItem(middleCell, {
			$bind: "site_help",
			$category: "link",
			$skin: "s-site-btn-help"
		});

		this.searcher = _searcher.load(middleCell);
