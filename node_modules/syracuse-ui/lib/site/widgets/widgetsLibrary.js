...
var _widgetFactory = require('syracuse-ui/lib/site/widgets/widgetFactory');
...
function _walk(obj, missingTypes, prop) {
	if (obj[prop])
		Object.keys(obj[prop]).forEach(function(k) {
			_collect(obj[prop][k], missingTypes);
		});
}

function _collect(obj, missingTypes) {
	if (obj) {
		_walk(obj, missingTypes, '$properties');
		_walk(obj, missingTypes, '$variants');
		_collect(obj.$item, missingTypes);
		if (obj.$type && !_hasWidget(obj.$type)) {
...
exports.ensureMediaTypes = function($prototype, finish) {
	var missingTypes = {};
	_collect($prototype, missingTypes);
			...
			function requireMissing(i) {
				if (i < data.length) {
					...
					require.async(data[i].$module, function(err, api) {
						...
						_fieldTypes[$type] = _widgetFactory.makeClass($type, api);
						requireMissing(i + 1);
					});
				} else {
					finish();
