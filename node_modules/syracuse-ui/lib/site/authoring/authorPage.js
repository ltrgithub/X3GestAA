...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    open: function(){
        ...
        this.$defaultItem = helpers.object.clone(document.itemFactory.ensureDefaultArticle(this.targetPage.$prototype.$article, this.targetPage.$prototype), true);
    ...
    buildAutoLayout: function($prototype, $sourceArticle){
        ...
        if ($rootItems[0].$category != "section") {
            ...
            if ($rootItems.length < 3) {
                ...
                $newArticle.$layout.$items.push({
                    $layoutType: "stack",
                    $items: [$rootItems[$rootItems.length - 1]]
                });
                for (var ii = 1, jj = $rootItems.length - 1; ii < jj; ii++) {
        ...
        $rootItems.forEach(function($parent){
            ...
            if ($children.length > 0) {
                if ($children[0].$category == "block") {
                    self.buildAutoBlock($prototype, $parent, $children);
                }
                else {
                    self.buildAutoFields($prototype, $parent, $children);
                }
            }
        });
        return $newArticle;
    },
    buildAutoBlock: function($prototype, $parent, $blocks){
        var $row;
        var self = this;
        if ($blocks.length > 1) {
            $parent.$layout.$items = [];
            $blocks.forEach(function($block){
                ...
                if ($fields.length > 0) {
                    ...
                    if (addAuto) {
                        self.buildAutoFields($prototype, $block, $fields);
                    }
            ...
            if ($blocks.length == 1) {
                ...
                self.buildAutoFields($prototype, $blocks[0], $fields);
            }
    },
    buildAutoFields: function($prototype, $parent, $children){
        $parent.$layout.$items = [{
            $layoutType: "columns",
            $layoutSubType: "50,50",
            $items: [{
    ...
    reloadPage: function($item){
        this.targetPage.diposeContent();
        this.targetPage.$item = $item;
        this.targetPage.$$body.empty();
        this.targetPage.loaded = false;
        this.targetPage.boundFields = {};
        this.targetPage.idMap = {};
        this.targetPage.menuItems = {};
        this.targetPage.$menus = {};
        this.targetPage.menuBoxes = [];
        this.targetPage.openBox(true);
    },
    onMenuClick: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            case "$autoLayout":
                debugger;
                var $newLayout = self.buildAutoLayout(this.targetPage.$prototype, helpers.object.clone(this.$defaultItem, true));
                self.reloadPage($newLayout);
                break;
            case "$defaultLayout":
                var dataset = self.dataset;
                self.reloadPage(helpers.object.clone(self.$sourceItem, true));
                break;
            case "$resetLayout":
                self.reloadPage(helpers.object.clone(self.$defaultItem, true));
                break;
            case "$close":
                if (self.isUpdated) {
    ...
    _drawStep: function(node){
        ...
        dom.setAttribute("title", this.localize["aw_" + (node.isPageItem() ? "page" : node.$authoringLevel) + "Step"]);
        node.addNodeId(dom);
