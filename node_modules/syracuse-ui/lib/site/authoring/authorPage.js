...
var LayoutAuthoring = require("syracuse-ui/lib/article/layouts/authoring/layoutAuthoring").LayoutAuthoring;
var ItemPalette = require("./palette/itemPalette").ItemPalette;

var paletteSettings = {
    article: require("./palette/pageSetting"),
    section: require("./palette/sectionSetting"),
    block: require("./palette/blockSetting"),
    field: require("./palette/fieldSetting")
};
...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    open: function(){
        if (!document.site.layoutAuthoring) {
            (document.site.layoutAuthoring = new LayoutAuthoring()).bindEvents();
        }
    ...
    drawBox: function(){
        ...
        self._panel.className = "s-aw-panel"
        self.$$authorBody.append(self._panel);
        self._ensurePanelVisibility();
        document.site.resize();
        
        this.rootNode = new TreeNode(self.targetPage, null, self);
        this.rootNode.load();
        this.rootNode.selectItem();
    },
    _ensurePanelVisibility: function(){
        this._panel.style.display = this.$isPaletteExpanded ? "" : "none";
    },
    ...
    localizePalette: function($localization){
        var self = this;
        Object.keys($localization).forEach(function(key){
            $localization[key] = self.localize["aw_" + key.slice(1)] || $localization[key];
        });
        /*   
         $isSeparatorsVisible: {
         $title: self.localize.aw_isSeparatorsVisible
         $fieldsIsTitleHidden: {
         $title: self.localize.aw_isLabelHidden,
         }
         //for block section
         this._formatLayout();
         */
    },
    _initializePalette: function($authoringLevel){
        this.palette = new ItemPalette();
        var settings = paletteSettings[$authoringLevel];
        this.palette.localize = this.localize;
        this.palette.authorPage = this;
        this.palette.externalAdapter = document.site.externalAdapter;
        this.palette.$prototype = settings.$prototype;
        this.palette.$item = settings.$item;
        this.localizePalette(this.palette.$prototype.$localization);
        this.palette._initializePage();
        var saveDisplay = this._panel.style.display;
        this._panel.style.display = "none";
        this.palette.$$container = $(this._panel).empty();
        this.$skin = "s-aw-designer-item";
        this.$defaultSkinBlock = "s-aw-designer-h2";
        this.$defaultSkinSection = "s-aw-designer-h1";
        this.palette.loadBox();
        this._panel.style.display = saveDisplay;
    },
    selectPathStep: function(node){
        ...
        setTimeout(function(){
            ...
            self._initializePalette(self.designedNode.$authoringLevel);
    ...
    showTargetItem: function(itemNode){
        ...
        while (node && node.item) {
            if (node.item._tabTitle && !node.item.$item.$opened) {
                node.item.openBox(true);
            }
            node = node.parentNode
        }
        var item = itemNode.item;
        if (item._tabTitle && !item.$item.$opened) {
            item.openBox(true);
    ...
    dispose: function(){
        ...
        delete this.page;
        if (document.site.layoutAuthoring) {
            document.site.layoutAuthoring.dispose();
            delete document.site.layoutAuthoring;
        }
        this._panel = null;
