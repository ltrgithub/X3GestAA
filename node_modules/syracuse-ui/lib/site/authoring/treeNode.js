...
exports.TreeNode = helpers.defineClass(TreeNode, null, {
    ...
    setDesignableItem: function(item){
        ...
        this.ensureItemCss(item, true);
    },
    ensureItemCss: function(item, enable){
        if (item._tabTitle) {
            document.site.toggleClass(item._tabTitle, "s-aw-item", enable);
            document.site.toggleClass(item._body, "s-aw-item", enable);
        }
        else {
            if (item.dataSlot || item.$$item) {
                document.site.toggleClass((item.dataSlot || item.$$item[0]), "s-aw-item", enable);
            }
        }
    },
    ...
    toggleBoxLightMode: function($isLightMode){
        ...
        if ($isLightMode) {
            ...
            oldSkin = this.fullSkin;
            newSkin = this.item.$skin = skinLight;
        }
        else {
            oldSkin = skinLight;
            newSkin = this.item.$skin = this.fullSkin || this.item.$skin;
        }
        this.item.ensureLayoutMode();
    },
            ...
            layoutContent.items.forEach(function(child){
                if (child.$layout) {
                    ...
                    if (node.$authoringLevel == "field") {
                        ...
                        node.ensureItemCss(node.item, true);
    ...
    toggleItemCss: function(status, show, css){
        ...
        if (this.item && this.item.$$item) {
            if (this.item._tabTitle) {
                document.site.toggleClass(this.item._tabTitle, css, show);
                document.site.toggleClass(this.item._body, css, show);
            }
            else {
    ...
    dispose: function(){
        if (this.item) {
            ...
            this.ensureItemCss(this.item, false);
