...
exports.TreeNode = helpers.defineClass(TreeNode, null, {
    ...
    setDesignableItem: function(item){
        if (item.$isTabLayout) {
            document.site.toggleClass(item.$$header[0], "s-aw-item", true);
            item.$$header[0].setAttribute("data-s-item", item.id);
            if (item.boxParent) {
                item.$$header[0].setAttribute("data-s-article", item.boxParent.getArticle().id);
            }
        }
        document.site.toggleClass(item.$$item[0], "s-aw-item", true);
