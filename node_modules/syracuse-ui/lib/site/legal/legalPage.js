...
var $eulaUrl = "/syracuse-ui/lib/site/legal/SAGE ERP X3 EULA v201404F & GB.pdf";
...
exports.LegalPage = helpers.defineClass(LegalPage, DesktopPage, {
	loadBox: function($prototype) {
		...
		this.$prototype.$links.$eula = {
			$title: syra_site.localize.legalEula,
			$url: ""
		};
		...
		this.$item = {
			$category: "page",
			$css: $skin + "-header",
			$title: syra_site.localize.legalPageTitle,
			$layout: {
				$css: $skin + "-body",
				$layout: {
					$items: [{
	...
	applyChange: function(newData) {
		...
		if (this.dataset && !this.dataset.$copyright) {
			newData.$copyright = syra_site.localize.legalCopyright;
		}

		if (newData && newData.$resources && newData.$resources.length > 0) {
			for (var ii = 0, jj = newData.$resources.length; ii < jj; ii++) {
				newData.$resources[ii].productVersion = syra_site.localize.legalProductVersionLabel + " " + newData.$resources[ii].productVersion;
			}
		}
