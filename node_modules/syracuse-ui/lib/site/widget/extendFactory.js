...
var ExtendField = _helpers.defineClass(function() {}, Field, {
	loadBox: function(initData) {
		...
		if (this.$type.indexOf("x-array") >= 0) {
			...
			this._dataValue = this.layoutSlot;
	...
	render: function(slot) {
		...
		self.extendWidget = self.api.create({
			...
			field: self,
			page: self.page,
			$prototype: self.$prototype, //article
			$field: self.$field, //pas article
			div: self._dataValue,
			$item: self.$item,
			...
			fetch: function(options, success, error) {
				options = options || {};
				if (options.$url) {
					options.$url = this.parseExpression(options.$url);
				}
			...
			getPagePreferences: function() {
				return syra_pagePreferences.getField(this);
			},
			savePagePreferences: function(data) {
				syra_pagePreferences.saveField(this, data);
			},
			registerFlashWidget: function() {
				self.layoutSlot.className += " s-field-flash-slot";
				var boxParent = (self.attachedField || self).boxParent;
				while (boxParent) {
	...
	validateInput: function(errors, value) {
		if (this.extendWidget && this.extendWidget.validateInput) {
			this.extendWidget.validateInput(errors, value);
		}
	...
	setDataValue: function(value, metaData) {
		...
		if (this.extendWidget && this.extendWidget.setValue)
			this.extendWidget.setValue(value, metaData);
	},
	...
	/*setDataBind: function(value, record, metaData, $bind) {
     if (this.extendWidget && this.extendWidget.setDataBind) {
     if (this.isArticle) {
     syra_dataset.ensure(this);
     }
     this.extendWidget.setDataBind(value, record, metaData, $bind);
     } else {
     Field.prototype.setDataBind.call(this, value, record, metaData, $bind);
     }
     },*/
	applyDesignMetaData: function(metadata) {
		if (this.extendWidget && this.extendWidget.applyDesignMetaData)
			this.extendWidget.applyDesignMetaData(metadata);
	},
	resizeArticle: function() {
		if (this.extendWidget && this.extendWidget.resize) {
			if (syra_dom.isVisible(this.extendWidget.container.div)) {
