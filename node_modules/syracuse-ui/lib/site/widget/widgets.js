...
var _categories = {
			...
			if ($field.$decorator && $field.$decorator.$type) {
				...
				if ($stereotype) {
					var decocatorClass = {
						...
						//"cube$flash": flashWidget.FlashWidget, //CubeChart,
	...
	field: {
		create: function($item, boxParent) {
					...
					if (field) {
						...
						if (field) {
							if (field.onReloading) {
								...
								syra_item.initializeNewItem(article.page, field, $item, boxParent);
			...
			if ($field) {
				var $class = $field.$formatType || $field.$type;
				...
				if (!fieldClass) {
					switch ($class) {
						case "application/x-array":
							...
							if (article.page.$prototype.$isCalendarView) {
								$field.$decorator = {
									$type: "application/x-array-calendar"
								};
...
exports.addItem = function(page, $item, boxParent, newItem) {
	...
	if ($item) {
		...
		if ($item.$layoutType) {
			...
			if (item) {
				syra_item.initializeNewItem(page, item, $item, boxParent);
			}
		}
	}
	return item;
	...
	if (obj) {
		...
		if (obj.$type) {
			...
			if ($type == "application/x-array") {
				...
				switch ($stereotype) {
					case "orgview":
						$type = "application/x-array-chart-org";
						break;
					case "agenda":
						$type = "application/x-array-chart-agenda";
						break;
					case "gantt":
						$type = "application/x-array-chart-gantt";
						break;
					default:
						if (obj.$cube) {
							$type = "application/x-array-chart-cube";
						}
						break;
				}
			}
			...
			function requireMissing(i) {
				if (i < data.length) {
					...
					require.async(data[i].$module, function(err, api) {
						...
						if ($type.indexOf("x-array") >= 0) {
							_fieldTypes[$type] = _extendFactory.makeArticleClass($type, api);
						} else {
