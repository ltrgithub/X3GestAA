		...
		while (target && target != root) {
			if (target.syrainout) {
				if (!related || (related !== target && !target.contains(related))) {
					item = repository[target.syrainout];
...
exports.load = function() {
	...
	_docHandlers.keyup = document.addEventListener("keypress", function(event) {
		keyboard[event.type](event);
	}, false);
		...
		_layoutHandlers.click = syra_site.layoutSlot.addEventListener("click", function(event) {
			try {
				if (!event.target.syraIsMouseEventDisabled) {
								...
								if (observer && event.syraOnClickTarget) {
									if (observer[event.syraOnClickTarget.syraOnClick]) {
										if (!observer.$isDisabled) {
											observer[event.syraOnClickTarget.syraOnClick](event.syraOnClickTarget, event);
										}
										done = true;
									}
									if (done) {
					...
					if (!done) {
						syra_site.onBeforeClick(undefined, event);
	...
	window.onbeforeunload = function(event) {
		return syra_site.fusionGateway && syra_site.fusionGateway.onWindowBeforeUnload();
	};
