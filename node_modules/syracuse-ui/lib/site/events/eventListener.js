...
var _winResizeHandler, _winOrientationHandler;
...
function _bindResize(bind) {
	if (bind) {
		_winResizeHandler = window.addEventListener("resize", function(event) {
			document.site && document.site.onSiteResizeEvent();
		});
		_winOrientationHandler = window.addEventListener("orientationchange", function(event) {
			document.site && document.site.onSiteResizeEvent();
		});
	} else {
		if (_winResizeHandler) {
			window.removeEventListener("resize", _winResizeHandler);
		}
		if (_winOrientationHandler) {
			window.removeEventListener("orientationchange", _winOrientationHandler);
		}
		_winResizeHandler = _winOrientationHandler = null;
	}
}
	...
	while (target && target != root) {
		if (target.syrainout) {
			if (!related || (related !== target && !target.contains(related))) {
				target.syrainout.onItemEnterLeave(onEnter, event);
...
var _mouseOverHandler, _mouseOutHandler;

function _bindMouseEnterLeave(bind) {
	if (document.site.layoutSlot) {
		if (bind) {
			_mouseOverHandler = document.site.layoutSlot.addEventListener("mouseover", function(event) {
				_onMouseEnterLeave(document.site.layoutSlot, event);
			}, false);
			_mouseOutHandler = document.site.layoutSlot.addEventListener("mouseout", function(event) {
				_onMouseEnterLeave(document.site.layoutSlot, event);
			}, false);
		} else {
			if (_mouseOverHandler) {
				document.site.layoutSlot.removeEventListener("mouseover", _mouseOverHandler);
			}
			if (_mouseOutHandler) {
				document.site.layoutSlot.removeEventListener("mouseout", _mouseOutHandler);
			}
		}
	}
}
...
exports.load = function() {
	...
	_bindResize(true);
	_bindListEvents();
	_bindMouseEnterLeave(true);
	_bindMobile();
};

exports.dispose = function() {
	keyboardManager.dispose();
	_bindMouseEnterLeave(false);
