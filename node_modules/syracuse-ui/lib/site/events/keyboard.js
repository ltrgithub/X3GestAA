...
function _getTargetItems(target) {
	...
	while (target != null && !field && !article) {
		if (!field && target.syraItem) {
			field = repository[target.syraItem];
			if (field && !field.isField) {
				field = null;
			}
			article = field && field.article;
		}
		if (target.syraarticle) {
			article = repository[target.syraarticle];
		}
...
exports.keydown = function(event) {
	...
	if (syra_site.messageBox && !syra_site.messageBox.displayDisabled) {
		isShorcutApplyed = syra_site.messageBox.applyShortCuts(_shortcurts);
