...
exports.FavoritesProvider = helpers.defineClass(FavoritesProvider, null, {
	...
	save: function($url, $title, $description, params) {
		if ($url) {
			...
			if (!this.map[$url]) {
				...
				if (params) item.menuItem.params = params;
	...
	onClickPicker: function(target, event) {
		...
		} else if (target.syraLink && target.syraLink !== "") {
	...
	onMenuClick: function(menuItem) {
		switch (menuItem.$bind) {
			case "$addfavorite":
				var title = menuItem.page.mainPage.getTitle(),
					params = this._params(menuItem),
					url = this._url(menuItem, params);
	...
	updateSiteFavorites: function() {
		...
		site.more.style.visibility = (more || this.items.length) ? "visible" : "hidden";
	},
	dispose: function() {
		if (this.favoritesPage) this.favoritesPage.dispose();
