...
exports.EventListener = helpers.defineClass(EventListener, null, {
	...
	_bindShortCut: function() {
		...
		document.site.$$layoutSlot.delegate("*", "keydown keyup", function(event) {
			...
			if (event.type == "keydown") {
				...
				if ((self.keysDown.length === 2 || (self.keysDown.length === 3 && event.shiftKey)) && ((keyboardShortCut.getModifier() === event.keyCode) || (self.keysDown.length !== 0 && self.keysDown.indexOf(keyboardShortCut.getModifier()) !== -1 && event))) { // if modifier exists try perform shortcut
					self.applyShortCut = keyboardShortCut.applyKeyboardShortCut2({
						isFusion: elem.fusionGateway && elem.fusionGateway._books && elem.fusionGateway._books[0].selectedSheet != null,
						keysDown: self.keysOk,
						event: event,
						eventListener: self
					});
					return false;
				} else if (self.keysDown.length > 2) {
					return false;
				}
			}
			if (event.type == "keyup") {
					...
					switch (pickerId) {
						...
						case "s-bar-title":
							...
							if (isFusionBar) {
								...
								bar = (event.currentTarget.className.indexOf("s-aw-toolbar") >= 0) ? article.awContext.toolsBar : article.menuBar;
							}
	...
	_bindListEvents: function() {
		document.site.$$layoutSlot.delegate("div[data-s-sort]", "click", function(event) {
			...
			event.stopPropagation();
			article.list.treeList.onNodeEvent(event);
			return false;
		});
	},
