...
exports.EventListener = helpers.defineClass(EventListener, null, {
    ...
    findBox: function($$item){
        var article = this.findArticle($$item);
        var boxId = $$item.attr("data-s-box");
        if (!boxId) {
            boxId = $$item.closest("[data-s-box]").attr("data-s-box");
        }
        return article ? article.idMap[boxId] : null;
    },
...
//            }
    ...
    _bindListEvents: function(){
        ...
        document.site.$$container.delegate("a[data-s-sort]", "click", function(event){
            ...
            var $$link = $(this);
            self.findArticle($$link).sortCapability.onSortAlphaTab($$link);
            return false;
            ...
            var $$link = $(this);
            self.findArticle($$link).pagingCapability.onPageClick($$link);
            return false;
        }).delegate(".s-list-selector-row", "click mouseenter mouseleave", function(event){
            var list = self.findArticle($(this)).list;
            if (event.type == "click") {
    ...
    _bindBox: function(){
        ...
        document.site.$$container.delegate("a[data-s-box]", "click", function(event, data){
            ...
            if (box.$isTabLayout) {
                ...
                var show = this.className.indexOf("s-open") < 0;
                var isFirstTime = !box.loaded || false;
                if (show) {
                    document.site.resize();
                }
                box._collapse(show, isFirstTime);
            }
            return false;
        });
    },
