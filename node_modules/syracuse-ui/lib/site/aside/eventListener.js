			...
			if (this.syraFieldId) {
				...
				if (field) {
					...
					if (event.type == "click" && document.site.authorPage) {
						doEvent = document.site.authorPage.designedPage.designer.onClickItem(field, this);
					}
					if (doEvent) {
			...
			try {
				...
				if (boxId) {
					...
					if (box) {
						var doEvent = true;
						if (document.site.authorPage) {
							doEvent = document.site.authorPage.designedPage.designer.onClickItem(box, this);
						}
						if (doEvent) {
									...
									if (!article.onClickPicker || article.onClickPicker(event.currentTarget, event)) {
										switch (pickerId) {
											...
											case "list-card-opener":
												article.builder.onCardOpenerClick(this);
												break;
											case "list-page":
