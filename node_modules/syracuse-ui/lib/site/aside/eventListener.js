...
exports.EventListener = helpers.defineClass(EventListener, null, {
	...
	_bindShortCut: function() {
		...
		document.site.$$layoutSlot.delegate("*", "keydown keyup", function(event) {
			...
			if (event.type == "keydown") {
				...
				if (((self.keysDown.length > 1 && self.keysDown.indexOf(keyboardShortCut.getModifier()) !== -1) || (self.keysDown.length > 0 && keyboardShortCut.getNoModShortcut().indexOf(self.keysDown[0]) !== -1) || (self.keysDown.length === 3 && event.shiftKey)) && event) { // if modifier exists try perform shortcut
	...
	_bindFieldEvents: function() {
		...
		document.site.$$layoutSlot.delegate("textarea,input,select", "mouseenter mouseleave mousedown click change keydown keypress keyup focusin focusout", function(event) {
			if (this.$syraFieldId) {
	...
	_bindListEvents: function() {
		document.site.$$layoutSlot.delegate("[data-s-btn],[data-s-menu],[data-s-picker]", "click", function(event) {
			...
			try {
				...
				if (article.arrayLevel == "array") {
					document.site.lastFocusGrid = article;
				}
				return retValue;
			}
