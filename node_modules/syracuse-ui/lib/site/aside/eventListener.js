...
var mobileListener = require('syracuse-ui/lib/mobile/mobileEventListener').EventListener;

function EventListener() {}

exports.EventListener = helpers.defineClass(EventListener, null, {
	load: function() {
		(this._mobileListener = new mobileListener()).load();
	...
	_bindFieldEvents: function() {
		...
		document.site.$$layoutSlot.delegate("textarea,input,select,[contenteditable]", "mouseenter mouseleave mousedown click change focusin focusout", function(event) {
			if (this.syraFieldId) {
				...
				if (field) {
					...
					if (field.page.designer) {
						doEvent = field.page.designer.onClickItem(field, this, field.article);
									...
									if (!article.onClickPicker || article.onClickPicker(event.currentTarget, event)) {
										switch (pickerId) {
											case "list-sort":
												...
												break;
											case "list-search":
												article.searchCapability.onSearchClick(event, this.syraBind);
												break;
											case "list-alphatab":
												article.sortCapability.onSortAlphaTab(this);
												break;
											case "grid-inlinecard":
												article.builder.onInlineCardOpenerClick(this);
												break;
											case "list-page":
