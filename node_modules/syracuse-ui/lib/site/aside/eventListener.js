...
exports.EventListener = helpers.defineClass(EventListener, null, {
    ...
    _bindFieldEvents: function(){
        document.site.$$container.delegate("textarea[data-s-field],input[data-s-field],select[data-s-field]", "click change keydown keypress keyup focusin focusout", function(event){
            ...
            var pickerId = event.target.getAttribute("data-s-picker");
            if (!article.onClickPicker || article.onClickPicker(pickerId, event)) {
                switch (pickerId) {
                    ...
                    case "s-bar-next":
                    case "s-bar-title":
                        var isFusionBar = event.target.className.indexOf("s-fusion-bar") >= 0;
                ...
                if (doEvent) {
                    if (box._tabTitle) {
                        ...
                        if (box.$item.$isBoxCollapsable) {
                            ...
                            if (box.contextField) {
                                if (!box.contextField.$isDisabled) {
                                    box.contextField.page.externalAdapter.onFieldClickPicker({
                                        field: box.contextField,
                                        pickerType: "menus",
                                        doEvent: function(){
                                            box.doOpenPicker();
                                        }
