...
exports.EventListener = helpers.defineClass(EventListener, null, {
	...
	_bindShortCut: function(){
		...
		document.site.$$layoutSlot.delegate("*","keydown keyup",function(event){
			...
			if( event.type == "keydown" && !event.originType){
				if( self.keysDown.indexOf(event.keyCode)=== -1){
					if( keyboardShortCut.getModifier() === event.keyCode ){ // fire modifier keydown
						self.keysDown.push(event.keyCode);
						return false;
					}else if( self.keysDown.length!==0 && self.keysDown.indexOf(keyboardShortCut.getModifier())!==-1 &&  event ){
						self.keysDown.push(event.keyCode);
						// manage short cut
						self.applyKeyboardShortCut = true;
						return keyboardShortCut.applyKeyboardShortCut(event.target,elem.mainPage.$pageCategory==="fusion",self.keysDown,event);
					}
				}
			}
			if( event.type == "keyup" ){
				if( self.keysDown.length === 1 ){
					 // we just keydown ESCAPE
				 	event.type = 'keydown';
					if( document.site.lastFocusField && document.site.lastFocusField.$$item ){
						document.site.lastFocusField.onFieldInputEvent(event);
					}
				}
				if( self.keysDown.indexOf(event.keyCode) !==-1){
					//delete self.keysDown[self.keysDown.indexOf(event.keyCode)];
					if( event.keyCode === keyboardShortCut.getModifier() ){
						self.keysDown = [];
					}
					if(  event.keyCode == 9 && keyboardShortCut.getModifier() === 9 ){ // if it's tabs and tab is modifier 
						var target = event.target;
        ...
        document.site.$$layoutSlot.delegate("textarea[data-s-field],input[data-s-field],select[data-s-field]", "click change keydown keypress keyup focusin focusout", function(event){
            ...
            if (field) {
				...
				if( field.articleParent && field.articleParent.$$item[0].localName === 'tr'  && field.articleParent.articleParent ){
					document.site.lastFocusGrid = field.articleParent.articleParent;
				}
                    ...
                    switch (pickerId) {
                        ...
                        case "s-bar-title":
                            ...
                            if (isFusionBar) {
                                ...
                                bar = (event.currentTarget.className.indexOf("s-aw-toolbar") >= 0) ? article.toolsBar : article.menuBar;
                            }
                            bar.onClickPicker(pickerId, event);
                            break;
                        default:
