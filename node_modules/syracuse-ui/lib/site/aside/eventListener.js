...
exports.EventListener = helpers.defineClass(EventListener, null, {
    ...
    bindAuthoringEvent: function(bind){
        if (bind) {
            document.site.$$container.delegate(".s-aw-content-tree", "mousedow.authoring click.authoring", function(event){
            ...
            document.site.$$container.undelegate(".authoring");
        }
    ...
    _bindFieldEvents: function(){
        document.site.$$container.delegate("textarea[data-s-field],input[data-s-field],select[data-s-field]", "click change keydown keypress keyup focusin focusout", function(event){
            ...
            if (field) {
                var doEvent = true;
                if (event.type == "click" && document.site.authorPage && document.site.authorPage.layoutAuthoring) {
                    doEvent = document.site.authorPage.layoutAuthoring.onClickItem(field, this);
                }
                if (doEvent) {
    ...
    _bindBox: function(){
        document.site.$$container.delegate("a[data-s-box]", "click", function(event, data){
            ...
            if (box) {
                var doEvent = true;
                if (document.site.authorPage && document.site.authorPage.layoutAuthoring) {
                    doEvent = document.site.authorPage.layoutAuthoring.onClickItem(box, this);
                }
                if (doEvent) {
                    if (box._tabTitle) {
