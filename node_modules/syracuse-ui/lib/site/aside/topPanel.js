...
function _addDashPanel() {
	if (!_top.dashPanel) {
		if (syra_site.$navigation) {
			...
			_top.dashPanel = syra_dom.div("s-toppn-dash");
...
exports.showUserIdentity = function(profile) {
	if (_top.openerBtn) {
		if (profile.dataset.user) {
			...
			_top.openerBtn.link.textContent = (profile.dataset.user.firstName ? profile.dataset.user.firstName + " " : "") + (profile.dataset.user.lastName ? profile.dataset.user.lastName : "");
			...
			if (profile.dataset.user.photo) {
				profile.addItem(photo, {
					...
					$url: syra_expression.parse(profile, profile.dataset.user.photo.$url || "{$shortUrl}/photo", {
						$shortUrl: profile.dataset.user.$shortUrl
					})
				});
			} else {
				syra_dom.anchor("s-site-user-photo-no", photo);
			}
			_top.openerBtn.link.insertBefore(photo, _top.openerBtn.link.firstChild);
		}
	}
};
...
exports.toggle = function(show, event, onClosePopups) {
	if (_top.openerBtn) {
		if (show && !_top.slot) {
			_top.slot = syra_dom.div("s-site-top-pn", syra_site.layoutSlot);
