...
function _addDashPanel() {
	if (!_top.dashPanel) {
		if (syra_site.$navigation) {
			...
			_top.dashPanel = document.createElement("div");
			_top.dashPanel.className = "s-toppn-dash";
			...
			var ids = Object.keys(syra_site.$navigation);
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				syra_site.loadNewItem(_top.dashPanel, {
					$category: "link",
...
exports.addOpener = function(slot) {
	_top.openerBtn = syra_menus.button.add({
		parent: syra_site,
		slot: slot,
...
exports.showUserIdentity = function(profile) {
	if (_top.openerBtn) {
		if (profile.dataset.user) {
			syra_site.dom.empty(_top.openerBtn.link);
...
exports.toggle = function(show, event, onClosePopups) {
	if (_top.openerBtn) {
		if (show && !_top.slot) {
			_top.slot = document.createElement("div");
			_top.slot.className = "s-site-top-pn";
			_top.slot.syrainout = syra_site.id;
			syra_site.layoutSlot.appendChild(_top.slot);
			syra_site.userProfile.toggle(_top.slot, !(syra_site.fusionGateway && syra_site.fusionGateway.activatedBook) && _addDashPanel());
			...
			syra_site.dom.setZIndex(_top.slot);
		} else {
			...
			syra_site.dom.empty(_top.slot);
			syra_site.dom.removeChild(_top.slot);
			_top.slot = null;
		}
		syra_site.dom.toggleClass(_top.openerBtn.link, "s-open", !! _top.slot);
	}
};

exports.onItemInOut = function(onEnter, event, target) {
	if (!syra_quality.enableTestRobot && _top.slot && target == _top.slot) {
		if (onEnter) {
			_top.onOutSlot && clearTimeout(_top.onOutSlot);
			_top.onOutSlot = null;
		} else {
			if (!syra_dlg.getTopDialogPage()) {
...
exports.dispose = function() {
	if (_top) {
		_top.onOutSlot && clearTimeout(_top.onOutSlot);
		syra_site.dom.removeChild(_top.slot);
		syra_menus.button.dispose(_top.openerBtn);
