...
exports.Resizer = helpers.defineClass(Resizer, null, {
    setResizable: function(options){
        ...
        if (self.options.dragSpot) {
            ...
            (self.$$dragSpot = $(self.options.dragSpot)).bind("mousedown." + self.id, function(event){
                if (!document.site.dragDropInstance && event.target == self.$$dragSpot[0]) {
                    ...
                    var $$boundary = $(document.site.mainPage.scrollview);
        ...
        if (self.options.isResizingEnabled !== false && self.options.direction !== null) {
            ...
            self.$$resizerSpot.bind("mousedown." + self.id, function(event){
                if (!document.site.dragDropInstance && event.target == self.$$resizerSpot[0]) {
                    if (self.isEnabled) {
    ...
    _startDrag: function(){
        ...
        $(document).bind("mouseup." + self.id, function(){
            ...
            if ((self.boundary.top < (position.top - 10)) && (self.boundary.left < (position.left - 10)) &&
            (self.boundary.bottom > (position.top + self.height + 10)) &&
            (self.boundary.right > (position.left + self.width + 10))) {
