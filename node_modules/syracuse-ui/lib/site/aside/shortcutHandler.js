    ...
    for (i = 0; i < keys.length; i++) {
        if (document.site.mainPage.idMap[keys[i]].tabTitle) {
            ...
            if (focus && document.site.mainPage.idMap[keys[i]].id === focus.id) {
                ...
                if ((!focus || !focus.tabTitle) && tab.length === 1) {
...
exports.navPageGrid = function(ctx){
    ...
    if (lastGrid.pagging) {
        var pagging = lastGrid.pagging;
        if (ctx.isFusion) {
            // looking for the next page 
            var pageNumber = Math.floor(pagging.$startIndex / pagging.$itemsPerPage) + 1;
            ...
            if (!isPageDown) {
                if (isShift) {
                    $(pagging.pageLinks[0]).click();
                }
                else 
                    if (0 < pageNumber - 1) {
                        $(pagging.pageLinks[pageNumber - 1]).click();
                    }
            }
            else {
                if (isShift) {
                    $(pagging.pageLinks.length - 1).click();
                }
                else 
                    if (pagging.pageLinks.length >= pageNumber + 1) {
                        $(pagging.pageLinks[pageNumber + 1]).click();
                    }
            }
