...
var GridRecord = require('syracuse-ui/lib/field/array/gridBuilder/gridRecord').GridRecord;
...
exports.colFitToWitdh = function(ctx){
	...
	if( lastGrid ){
		var colResizer = lastFocus.builder.colResizer;
	}
};

exports.navPageGrid = function(ctx){
		// get the grid
	var isPageDown = ctx.isPageDown;
	var isShift = ctx.isShift;
	var lastFocus = document.site.lastFocusField;
	var lastGrid = document.site.lastFocusGrid && document.site.lastFocusGrid.$$item ? document.site.lastFocusGrid:null;
	if(!lastGrid && lastFocus.$$item && lastFocus.$$item[0].localName === 'td' ){
		lastGrid = lastFocus.articleParent.articleParent;
	}
	if( lastGrid.pagingCapability ){
		var pcap = lastGrid.pagingCapability ;
		if( ctx.isFusion ){
			...
			if( !isPageDown ){
				if( isShift ){
					$(pcap.pageLinks[0]).click();
				}else if( 0 < pageNumber-1 ){
					$(pcap.pageLinks[pageNumber-1]).click();
				}
			}else{
				if( isShift ){
					$(pcap.pageLinks[pcap.pageLinks.length-1]).click();
				}else if( pcap.pageLinks.length >= pageNumber+1 ){
					$(pcap.pageLinks[pageNumber+1]).click();
				}
			}
			// passe in edit mod on the first cell of the page
			var keys = Object.keys(lastGrid.idMap);
			var found = null;
			for(var i=0;i<keys.length && !found;i++){
				if( lastGrid.idMap[keys[i]] instanceof GridRecord ){
					found = lastGrid.idMap[keys[i]];
				}
			}
			if( found ){
				$(found._rowIndexCell).click();
			}
		}else{
			var steps = pcap.serverSteps;
			var bind = isPageDown ? (isShift ? '$last' : '$next') : (isShift ? '$first': '$previous'); 

			var found = null;
			for(var i=0;i<steps.length && !found ; i++ ){
				if( steps[i].$bind === bind ){
					found = steps[i];
				}
			}
			if( found ){
...
exports.navRecord = function(ctx){
	...
	var found = null;
	for(var i=0;i<keys.length && !found;i++){
		if( document.site.mainPage.idMap[keys[i]].$bind === ctx.action ){
			found = document.site.mainPage.idMap[keys[i]];
		}
	}
	if( found ){
...
exports.actionGridLine = function(ctx){
	...
	if( lastFocus.menuItems[ctx.action] )
		lastFocus.menuItems[ctx.action][0].click();
};
...
exports.actionPage = function(ctx){
	...
	if( document.site.mainPage.menuItems[ctx.action] )
		document.site.mainPage.menuItems[ctx.action][0].click();
};
