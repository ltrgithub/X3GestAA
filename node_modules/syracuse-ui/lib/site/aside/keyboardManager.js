...
exports.KeyboardManager = helpers.defineClass(KeyboardManager, null, {
	...
	load: function() {
		...
		self.keyup = document.addEventListener("keyup", function(event) {
			...
			var page = self.getPage(focusField, article);
			if (self.shortcurts) {
				console.log("keyup self.shortcurts");
				if (event.keyCode == 27) {
					console.log("keyup event.keyCode == 27");
					if (!self.shortcurts.isEnabled) {
						console.log("keyup !self.shortcurts.isEnabled");
						var isEscApplyed;
						if (focusField && focusField.applyEscape) {
							console.log("keyup focusField && focusField.applyEscape");
						...
						if (!isEscApplyed && page) {
							...
							console.log("keyup page.applyEscape");
