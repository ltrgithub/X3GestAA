	...
	load: function() {
		...
		self.keydown = document.addEventListener("keydown", function(event) {
			...
			if (!isShorcutApplyed && page) {
				...
				//hack : for exemple after grid paging, ficusfield exist but in "ugly mode". so to avoid crach or unexpected behaviour :
				if (focusField && !focusField.articleParent) isShorcutApplyed = true;
			...
			if (!isShorcutApplyed && page) {
				...
				} else if (page.getCurrNavigationList) {
			...
			if (!isShorcutApplyed && page) {
				isShorcutApplyed = page.applyShortCuts(self.shortcurts, event);
				if (focusField && !isShorcutApplyed && page && page.$isFusionPage && event.keyCode !== 27) {
