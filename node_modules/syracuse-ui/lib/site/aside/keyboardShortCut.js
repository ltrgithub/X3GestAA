...
exports.applyKeyboardShortCut= function(targetField,isFusion,keysDown,event){
	...
	try{
		...
		if(keysDown.length>=2 && keysDown.indexOf(_keyboardMap[modifier])!==-1){
			if( keysDown.indexOf(_keyboardMap['p'])!==-1){// print 
			...
			}else if( keysDown.indexOf(_keyboardMap['a'])!==-1){// selection all rows of page grid
			...
			}else if( keysDown.indexOf(_keyboardMap['right'])!==-1){// tab navigation
				navTabFusion(true);		
			}else if( keysDown.indexOf(_keyboardMap['left'])!==-1){// tab navigation
				navTabFusion(false);		
			...
			}else if( keysDown.indexOf(_keyboardMap['pageup'])!== -1  ){// next page
			...
			}else if( keysDown.indexOf(_keyboardMap['pagedown'])!== -1  ){// prev page
				// check if we are in a grid
				var lastFocus = bodypage.lastFocusField;
				if( isFusion ){
					var lastGrid = bodypage.lastFocusGrid && bodypage.lastFocusGrid.$$item ? bodypage.lastFocusGrid:null;
			...
			}else if( keysDown.indexOf(_keyboardMap['k'])!==-1 && event.shiftKey){ // last record
				navRecord('$last');
			}else if( keysDown.indexOf(_keyboardMap['j'])!==-1 && event.shiftKey ){ // first record
				navRecord('$first');
			}else if( keysDown.indexOf(_keyboardMap['j'])!==-1){ // previous record
				navRecord('$previous');
			}else if( keysDown.indexOf(_keyboardMap['k'])!==-1){ // next record 
				navRecord('$next');
			}else if( keysDown.indexOf(_keyboardMap['f11'])!==-1){ // fullscreen mode
			...
			}else if( keysDown.indexOf(_keyboardMap['l'])!==-1 ||  keysDown.indexOf(_keyboardMap['f12'])!==-1){ // lookup picker
				...
				if( lastFocus.$$input && lastFocus.boxPickers && lastFocus.boxPickers.children.length > 0 ){
					...
					|| keysDown.indexOf(_keyboardMap['f1'])!==-1 
