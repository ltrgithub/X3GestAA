...
exports.applyKeyboardShortCut= function(targetField,isFusion,keysDown){
	...
	if(keysDown.length>=2 && keysDown.indexOf(_keyboardMap['tab'])!==-1){
		if( keysDown.indexOf(_keyboardMap['p'])!==-1){
			if( protoMenu.$print ){
				// get title to retrieve the menu button
				applyClickMenu(bodypage,protoMenu.$print.$title);
			}
			...
			if( protoMenu.$create ){
				// get title to retrieve the menu button
				applyClickMenu(bodypage,protoMenu.$create.$title);
			}
...
function applyClickMenu(bodypage,title){
	var items = bodypage.$$body.find("a[title='"+title+"'][data-s-menu]");
	if( items.length >= 1 ){
		var click = false;
