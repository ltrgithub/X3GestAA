...
exports.applyKeyboardShortCut= function(targetField,isFusion,keysDown){
	...
	if(keysDown.length>=2 && keysDown.indexOf(_keyboardMap[modifier])!==-1){
		if( keysDown.indexOf(_keyboardMap['p'])!==-1){// print 
		...
		}else if( keysDown.indexOf(_keyboardMap['n'])!==-1){// create
			var menu = "$create";
			if( isFusion ){
				menu = "1115";
			}
		...
		}else if( keysDown.indexOf(_keyboardMap['m'])!==-1 ){ // menu ctx
			...
			if( bodypage.mainPage.idMap[id] && bodypage.mainPage.idMap[id]._menusBox &&bodypage.mainPage.idMap[id]._menusBox.id===id+"-menu-picker"  ){
				var inputs =bodypage.mainPage.idMap[id]._menusBox.$$item.find('a');
			...
			var input = null;
			for(var i=0;i<items.length && !input;i++){
				if( items[i].$input ){
					input = item[i].$input;
				}
			}
