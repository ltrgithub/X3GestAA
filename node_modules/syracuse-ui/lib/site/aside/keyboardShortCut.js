...
exports.applyKeyboardShortCut= function(targetField,isFusion,keysDown,event){
	...
	try{
		...
		if(keysDown.length>=2 && keysDown.indexOf(_keyboardMap[modifier])!==-1){
			...
			}else if(isFusion && keysDown.length === 2 && keysDown.indexOf(_keyboardMap['f7'])!==-1){ // got to function 
					menu = "1186"; // x3 action 
					if( bodypage.mainPage.menuItems[menu] )
						bodypage.mainPage.menuItems[menu][0].click();
			...
			}else if(keysDown.length === 2 && ( keysDown.indexOf(_keyboardMap['l'])!==-1 ||  keysDown.indexOf(_keyboardMap['f12'])!==-1)){ // lookup picker
