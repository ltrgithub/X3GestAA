...
helpers.defineClass(BookmarksPage, DesktopPage, {
	...
	updateWorkingCopy: function() {
		_updateWorkingCopy(syra_dataset.cleanServerIndex(syra_site.clone(this.dataset)));
	},
...
exports.load = function() {
	...
	_manager.buttons = [];
	_manager.addBtn = syra_button.add({
		parent: syra_site,
	...
	_manager.manageBtn = syra_button.add({
		parent: syra_site,
		slot: syra_site.headerBottom,
		...
		iconOnly: true,
		fontIcon: "expand_l",
		click: _onManagerClick
	});
	syra_button.visibility(_manager.manageBtn, false);
	...
	_manager.moreBtn = syra_button.add({
		parent: syra_site,
		slot: syra_site.headerBottom,
		...
		click: function() {
			if (!_manager.morePopupDlg) {
				_manager.morePopupDlg = syra_over.openPopup(syra_site, {
	...
	syra_button.visibility(_manager.moreBtn, false);
...
exports.onNavigationMenuItemAddClick = function() {
	...
	if (!menuItem.isBookmark) {
		if (!_findMenuItem(menuItem.urlSeg)) {
		...
		var mn = _findMenuItem(menuItem.urlSeg);
...
exports.checkNavigationMenuItemBookMark = function(menuItem) {
	menuItem.urlSeg = syra_url.parse(menuItem.codeMenu.$url);
	syra_url.replaceSegmentEndpoint(menuItem.urlSeg, syra_site.userProfile.dataset.selectedEndpoint, true);
	if (_findMenuItem(menuItem.urlSeg)) {
		menuItem.isBookmark = true;
		_toggleButton(menuItem.bookmarkBtn, menuItem.isBookmark);
	} else {
		...
		if (activePage) {
			...
			if (activePage.isFusionPage) {
				...
				if (functionfusion) {
					...
					//(urlSeg.fusionParams.transaction == item.menuItem.urlSeg.transaction)) {
				}
			} else {
				urlSeg = syra_url.history.getUrlSegment(activePage.urlSeg.$url);
			}
			menuItem = urlSeg && _findMenuItem(urlSeg);
		}
		isOn = !! menuItem;
		for (var ii = 0, jj = _manager.buttons.length; ii < jj; ii++) {
			...
			if (btn.menuItem) {
				...
				syra_button.disable(btn, !btn.isBookmarkEnabled || ((activePage && activePage.isFusionPage) && !btn.menuItem.urlSeg.isFusion));
			}
		}
		syra_button.visibility(_manager.manageBtn, _manager.count);
		if (_manager.count >= syra_site.$maxBookmarks) {
			isBookmarkEnabled = false;
		}
		syra_button.disable(_manager.addBtn, !isBookmarkEnabled);
...
exports.onResize = function() {
	...
	syra_button.visibility(_manager.moreBtn, _manager.morePopupBody.children.length > 0);
};
exports.dispose = function() {
	_resetLinks();
	_manager.modal && _manager.modal.dispose();
	syra_button.dispose(_manager.moreBtn);
	syra_button.dispose(_manager.addBtn);
	syra_button.dispose(_manager.manageBtn);
...
function _refreshManager() {
	if (_manager.modal) {
		var items = syra_site.clone(_manager.dataset.items);
		_manager.modal.page.applyChange({
			items: items
		});
		_manager.modal.resize();
	}
}

function _onManagerClick() {
	_manager.modal = syra_over.openModal(syra_site, {
...
function _onAddClick() {
	...
	if (urlSeg) {
		var menuItem = _findMenuItem(urlSeg);
		if (menuItem) {
...
function _updateWorkingCopy(dataset) {
			...
			if (item.menuItem) {
				if (item.menuItem.$url) {
					...
					if (seg.isFusion) {
						...
						(_manager.syracuseMenuItems[item.menuItem.urlSeg.uri] = _manager.syracuseMenuItems[seg.uri] || []).push(item.menuItem);
					}
...
function _findMenuItem(urlSeg, items, deleteIfFound) {
	var menuItem;
	if (_manager.count) {
		if (urlSeg.fusionParams) {
			menuItem = _manager.fusionMenuItems[urlSeg.fusionParams.fullCode];
		} else {
			var menuItems = _manager.syracuseMenuItems[urlSeg.uri];
			if (menuItems) {
				for (var ii = menuItems.length - 1; !menuItem && ii >= 0; ii--) {
					var seg2 = menuItems[ii].urlSeg;
...
function _resetLinks() {
	...
	for (var ii = 0, jj = _manager.buttons.length; ii < jj; ii++) {
		syra_button.dispose(_manager.buttons[ii]);
	}
	_manager.buttons = [];
}

function _toggleButton(btn, isOn) {
	syra_button.setText(btn, isOn ? syra_local.bookmarksRemove : syra_local.bookmarksAdd, isOn ? "bookmark_on" : "bookmark_off");
}
