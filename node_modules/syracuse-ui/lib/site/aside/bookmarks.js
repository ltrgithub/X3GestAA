...
helpers.defineClass(BookmarkMenuField, ReferenceField, {
	...
	_renderCurrentValue: function() {
		if (this.codeMenu) {
			...
			var value = syra_fields.getDatasetValue(this);
			if (value) {
				if (value.$url) {
...
helpers.defineClass(BookmarksPage, DesktopPage, {
	...
	switchBookmarkEditTitle: function(record, isEdit) {
		...
		if (isEdit) {
			...
			record.editTitleField.setDataValue(syra_fields.getDatasetValue(record.singleField.variantItem).title);
		} else {
			syra_dom.hide(cell, false);
			if (record.editTitleField) {
				syra_article.removeItem(record.editTitleField, true, true);
