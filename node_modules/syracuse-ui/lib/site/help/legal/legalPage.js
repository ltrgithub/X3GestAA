...
var _$eulaDepsUrl = "/syracuse-main/html/main.html?url=/sdata/syracuse/collaboration/syracuse/copyrights?representation=copyright.$query&count&orderBy=name ASC";
...
exports.LegalPage = helpers.defineClass(LegalPage, DesktopPage, {
	loadBox: function($prototype) {
			...
			$layout: {
				$items: [{
					...
					renderRecordContent: function(record) {
						var head = syra_dom.addDiv("s-legal-item-head");
						syra_dom.addTextInfo("s-legal-item-product", record.dataset.product, head);
						syra_dom.addTextInfo("s-legal-item-iddn", record.dataset.productIddn, head);
						syra_dom.addTextInfo("s-legal-item-version", record.dataset.productVersion, head);
						...
						for (var ii = 0, jj = $rows.length; ii < jj; ii++) {
							...
							for (var mm = 0, kk = $cells.length; mm < kk; mm++) {
								if (mm > 0) {
									syra_dom.addDiv("s-legal-item-field-sep", row);
								}
								var $bind = $cells[mm];
								syra_dom.addTextInfo("s-legal-item-field-title", record.$prototype.$properties[$bind].$title, row);
								record.page.loadNewItem(syra_dom.addDiv("s-legal-item-field-value", row), {
									$bind: $bind,
									$inplace: true
								}, record);
		...
		this.loadNewItem(syra_dom.addDiv("s-legal-footer", this.layoutContent.domItem), {
			$category: "link",
			$skin: "s-site-legal-copyright",
			$bind: "$eulaDeps"
		});
	...
	onMenuClick: function(menuItem) {
		...
		if (options.$bind == "$eulaDeps") {
			window.open(_$eulaDepsUrl, "_blank");
			return false;
		}
		return true;
	}
