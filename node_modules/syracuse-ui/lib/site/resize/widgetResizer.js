...
var WidgetResizerDrop = require("./widgetResizerDrop").WidgetResizerDrop;
...
exports.WidgetResizer = helpers.defineClass(WidgetResizer, null, {
	...
	onDragMove: function(target, event) {
		if (this.isDraggable(target)) {
			if (document.site.ddManager.dropableItem) {
				if (document.site.ddManager.dropableItem == this) {
	...
	setDraggable: function() {
		...
		(self.$$dragSpot = $(self.options.dragSpot)).bind("mousedown." + self.id, function(event) {
			if (!document.site.ddManager.ddAgent && event.target == self.$$dragSpot[0]) {
