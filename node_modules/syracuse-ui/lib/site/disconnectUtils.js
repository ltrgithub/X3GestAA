...
exports.onUnload = function(){
    ...
    if (site && site.books && site.books.length > 0) {
        document.controller.fusionGateway.detachAllBooks(function(diag, client, clientRef, cbck){
            return true;
        }, false, true, true, {
            "panic": true
        });
...
exports.onUserActionUnload = function(callback, cancelCallback){
    ...
    function promptUser(diag, client, clientRef, cbck){
        ...
        options.callback = function(response, closedBy){
            ...
            if (!close) {
                ...
                document.controller.fusionGateway.activateBook(clientRef);
                if (cancelCallback) {
                    cancelCallback();
                }
            }
    ...
    if (booksNb > 0) {
        ...
        document.controller.fusionGateway.detachAllBooks(notifyHoster, false, false, true);
    }
    else 
        callback();
}
...
exports.updateTimer = function(){
    ...
    function callbackConfirm(){
        ...
        var $userProfileUrl = {
            $url: "/sdata/syracuse/collaboration/syracuse/userProfiles/$template/$workingCopies?representation=userProfile.$query"
        };
        ...
        page.formatMenuUrl($userProfileUrl, page.dataset, function($url, isCanceled){
            options.$location = {
                $url: $url
            };
            document.controller.sendRequest(page, options, function(data){
    ...
    function callbackCancel(){
        //TODO
        document.controller.fusionGateway.detachAllBooks(notifyHoster, true, true, false);
    }
    ...
    if (booksNb > 0) {
        ...
        300000 - 180000; // 5 minutes default
        ...
        document.site.serverTimeOutHandler = setTimeout(function(){
            if (document.controller.fusionGateway.canDetachBooks()) {
