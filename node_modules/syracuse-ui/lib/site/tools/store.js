...
exports.findItem = function(dom) {
	...
	while (dom != null) {
		...
		dom = _checkIfInIframe(dom);
		dom = dom.parentNode;
	}
	return item;
};

function _checkIfInIframe(dom) {
	if (dom.nodeType && dom.nodeType == 9) {
		var ifrms = document.getElementsByTagName("iframe");
		if (ifrms && ifrms.length > 0) {
			for (var ii = 0, jj = ifrms.length; ii < jj; ii++) {
				var doc = ifrms[ii].contentDocument ? ifrms[ii].contentDocument : ifrms[ii].contentWindow.document;
				if (dom === doc) {
					dom = ifrms[ii];
					break;
				}
