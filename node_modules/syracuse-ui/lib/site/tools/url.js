...
exports.history = {
	...
	addBackButton: function(page) {
		if (!page.isBackButtonDisabled && page.menuBar) {
			if (window.history.length > 1) {
				var hasBack, hasForward;
				var state = window.history.state;
				if (state && state.sageErpX3) {
					hasBack = true;
					hasForward = this.state.sageErpX3 && (state.sageErpX3 != this.state.sageErpX3);
				}
	...
	changeMainPage: function(onHistoryChange) {
		...
		if (href != document.location.href) {
			...
			syra_controller.changeMainPage(_getSdataUrl(href), null, onHistoryChange);
		}
	},
	load: function(opt) {
		...
		if (opt.target == "blank") {
			...
			window.history[(opt.editByFusion || opt.replaceState) ? "replaceState" : "pushState"](this.state = {
				editByFusion: opt.editByFusion,
				sageErpX3: opt.$url
			}, param, param);
			if (!opt.timeout) {
				syra_controller.changeMainPage(_getSdataUrl(opt.$url), opt.$itemPage);
			} else {
				syra_site.logout();
			}
		}
...
exports.parse = function($url, isSyracuse) {
	...
	//}
	segments.isPageRepresentation = !! (segments.isFusion || segments.params && segments.params.representation);
	if (segments.isPageRepresentation) {
		segments.isPageRepresentation = ($url.indexOf("/$files/") < 0) && ($url.search("/$shared/") < 0);
	}
	return segments;
};
...
exports.formatParameters = function(article, $parameters, record, callback) {
	...
	var expressionMaker = syra_expression;
...
exports.buildPageCollaborationUrl = function(article, urlSegment) {
	return syra_expression.parse(article, syra_site.$pageTemplateUrl || ('/page?url=' + encodeURIComponent(urlSegment.$url)), {
...
exports.getUrlParam = function(urlSegments, paramName) {
	var params, paramValue = null;
	if (urlSegments && (params = urlSegments.params) && params[paramName] !== undefined) {
