...
exports.history = {
	addBackButton: function(page) {
		if (page.menuBar && !page.isFusionPage) {
			if (window.history.length && document.referrer) {
				var segment = exports.parse(document.referrer);
				if (segment.isSyracuse) {
					var btn = syra_menus.addTextButton("Back", "s-mn-back-btn", "onBackHistory", "", "reload_back");
					btn.syraItem = page.id;
					page.menuBar.barBody.insertBefore(btn, page.menuBar.barBody.firstChild);
					page.onBackHistory = function() {
						window.history.back();
					};
				}
			}
		...
		save: function(page) {
			if (sessionStorage) {
				...
				if (segment && segment.$facet == "$query") {
					/*var index = _getSdataIndex(segment.uri);
                     if (index >= 0) {
                     (this.lastQueryUrls = this.lastQueryUrls || {})[segment.uri.slice(index)] = segment;
                     }*/
					sessionStorage.setItem(segment.uri, JSON.stringify(segment));
					//(this.lastQueryUrls = this.lastQueryUrls || {})[segment.uri] = segment;
				}
			}
		},
