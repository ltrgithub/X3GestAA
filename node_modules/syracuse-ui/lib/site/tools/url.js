...
exports.history = {
	stateKey: "sage_erp_x3",
	addBackButton: function(page) {
		if (!page.isBackButtonDisabled && page.menuBar) {
			if (window.history.length > 1 && window.history.state === this.stateKey) {
				var btn = syra_menus.button.add({
					parent: page,
					text: syra_local.site_back_link,
					css: "s-mn-back-btn",
				...
				page.menuBar.body.insertBefore(btn.link, page.menuBar.body.firstChild);
			}
		}
	},
	...
	setHref: function(menu, record, isExternal) {
		...
		if (!menu.isNoSyracuseUrl && menu.$url.indexOf("query") >= 0) {
	...
	loadMainPage: function(onHistoryChange) {
		...
		if (href != document.location.href) {
			...
			this._changeMainPage(href, null, onHistoryChange);
		}
	},
	load: function(opt) {
		syra_site.lastHref = document.location.href; // save last url;
		if (!syra_site.firstUrl) // load the first url loaded to determine if we have in certain case to force history.back
			syra_site.firstUrl = document.location.href;
		...
		if (opt.target == "blank") {
			...
			if (window.history.pushState) {
				window.history[opt.replaceState ? "replaceState" : "pushState"](this.stateKey, param, param);
				this._changeMainPage(opt.$url, opt.$itemPage);
			} else {
	...
	_changeMainPage: function($url, $itemPage, onHistoryChange) {
		if (!window.history.pushState) {
	...
	start: function() {
		...
		window[window.history.pushState ? "onpopstate" : "onhashchange"] = function(event) {
			if (this._updateHitory) {
				this._updateHitory = false;
			} else {
				if (syra_site.mainPage) {
					if (syra_site.forcehistoryback) {
						exports.history.forceBack();
						syra_site.forcehistoryback = false;

					} else {
						syra_site.lastHref = window.location.href;
						syra_site.mainPage.setHistoryBack(true);
						exports.history.loadMainPage(true);
					}
				}
	...
	forceBack: function(b) {
		syra_site.mainPage && syra_site.mainPage.setHistoryBack(true);
		syra_site.forcehistoryback = !b;
		window.history.back();
	},
	update: function(mainPage, $url) {
		...
		mainPage.openerUrlSegments = exports.parse($url);
		syra_site.checkMainPageBookmarkStatus();
	},
	queryPageSession: {
		save: function(page) {
			if (sessionStorage) {
				var segment = page.openerUrlSegments;
				if (segment && segment.$facet == "$query") {
					sessionStorage.setItem(segment.uri, JSON.stringify(segment));
				}
			}
		},
...
var _parameters = {
	openDialog: function(article, values, $parameters, record, callback) {
		var $itemPage;
		if (!$parameters.$url) {
			var $prototype = _helpers.object.clone($parameters);
		...
		syra_site.dialogManager.openModal(article, {
			...
			onOpened: function(page) {
				page.isDialogParameter = true;
				if ($parameters && $parameters.$url) {
					var keys = Object.keys(values);
					for (var ii = 0, jj = keys.length; ii < jj; ii++) {
						var prop = keys[ii];
						var $defaultValue = values[prop];
						if (page.boundFields && page.boundFields[prop] && page.boundFields[prop][0]) {
							var field = page.boundFields[prop][0];
			...
			onValidate: function(page, dialog) {
				if (!($parameters && $parameters.$url)) {
					if (syra_workingCopy.validateAllInputs(page)) { //no working pah, so no save server
						dialog.options.onServerSaved(null, dialog);
						return false;
					}
					return false;
				}
	...
	openSelectDialog: function(article, values, $parameters, record, callback) {
...
exports.formatParameters = function(article, $parameters, record, callback) {
	...
	if (callback) {
		if ($parameters.$actions) {
			_parameters.openSelectDialog(article, values, $parameters, record, callback);
			return;
		}
		if ($parameters.$properties) {
			_parameters.openDialog(article, values, $parameters, record, callback);
			return;
		}
		callback(values);
	}
...
exports.buildPageCollaborationUrl = function(article, urlSegment) {
	return syra_site.expressionMaker.parse(article, syra_site.$pageTemplateUrl || ('/page?url=' + encodeURIComponent(urlSegment.$url)), {
