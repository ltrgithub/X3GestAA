...
var _zIndex = 110;
var _styleSheetPath = "/syracuse-ui/themes/desktop/sage/";

exports.loadStyleSheet = function(id, sheets) {
	syra_site._styleSheets = syra_site._styleSheets || {};
	if (!syra_site._styleSheets[id]) {
		syra_site._styleSheets[id] = [];
		for (var ii = 0, jj = sheets.length; ii < jj; ii++) {
			var sheet = _styleSheetPath + sheets[ii];
			var link = document.createElement("link");
			link.setAttribute("rel", "stylesheet");
			link.setAttribute("type", "text/css");
			link.setAttribute("href", sheet);
			syra_site._styleSheets[id].push(link);
			//link.title = id;
...
exports.unloadStyleSheet = function(id) {
	if (syra_site._styleSheets) {
		var sheets = syra_site._styleSheets[id];
		if (sheets) {
			while (sheets.length) {
				syra_site.documentHeader.removeChild(sheets.shift());
			}
		}
		delete syra_site._styleSheets[id];
	}
};
