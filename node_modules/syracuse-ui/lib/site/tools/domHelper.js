...
exports.hide = function(dom, isHidden) {
	if (dom) {
		var display = isHidden ? "none" : "";
		if (dom.style.display != display) {
			dom.syraIsHidden = isHidden;
...
exports.isVisible = function(dom) {
	...
	if (top) {
		while (dom && dom != top) {
			if (dom.syraIsHidden || (dom.style && dom.style.display == "none")) {
				return false;
			}
			dom = dom.parentNode;
		}
...
exports.escape = function(message, br) {
	return _escaper(br ? message.replace(/\n/g, "<BR/>") : message);
};
