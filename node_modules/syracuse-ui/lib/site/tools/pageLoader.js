...
var autoFetchCategories = ["$query", "$bulk", "$details", "$search", "$lookup", "$select"];
...
exports.load = function($itemPage, onMainPagechange) {
	...
	item.localize = syra_site.localize;
	...
	if (item.$autoFetch === undefined) {
		item.$autoFetch = autoFetchCategories.indexOf(item.$facet) >= 0;
	}
