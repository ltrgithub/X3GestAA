...
exports.initialize = function(page) {
	...
	if (page.$facet == "$dashboard") { //office compatibility
	...
	if (!page.id) {
		...
		repository[page.id] = page;
	}
	if (!page.localize) {
		page.localize = syra_site.localize;
	}
};

exports.resize = function() {
	for (var ii = 0, jj = _childPages.length; ii < jj; ii++) {
		_childPages[ii].resizePage(true);
	}
};

exports.showPageSecurity = function(page) {
	if (syra_site.$item.$isPageSecurityView !== false) {
		var dataset = syra_site.userProfile && syra_site.userProfile.dataset;
		if (dataset) {
			...
			var pages = page ? [page] : _childPages;
...
exports.load = function($itemPage, onMainPagechange) {
	...
	var item = new($itemPage.$pageCategoryClass || site.siteOptions.widgetsLibrary.pageCategories[$pageCategory] || site.siteOptions.widgetsLibrary.defaultPageCategory)();
	...
	item.id = site._childPageOffset++;
	item.localize = site.localize;
	...
	site.initializeNewItem(item, $article, $itemPage.boxParent);
	site.localizer.applyPageLocalization(item);
	...
	if (onMainPagechange) {
		site.updateDocumentTitle();
		site.body.style.display = "";
	}
