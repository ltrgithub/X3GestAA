...
var _helpPopupSlot, _helpPopup, _pageDesignerOpener;
...
exports.addLogoutButton = function() {
	...
	picker.setAttribute("href", "#");
	picker.setAttribute("title", syra_site.localize.siteLogoutLink);
...
exports.addPageDesignerOpener = function() {
	_pageDesignerOpener = document.createElement("a");
	_pageDesignerOpener.syraSiteObserver = "siteFunctions";
	_pageDesignerOpener.className = "s-page-designer-opener";
	_pageDesignerOpener.syraOnClick = "onPageDesignerClick";
	_pageDesignerOpener.style.visibility = "hidden";
	_pageDesignerOpener.setAttribute("href", "#");
	_pageDesignerOpener.title = syra_site.localize.userProfile_siteAuthoringOpen;
...
function _showPageSecurity(page) {
...
exports.refreshBrowserTitle = function(page) {
...
exports.refreshMainPageSecurity = function(page) {
	if (page && !page.disposed) {
		_showPageSecurity(page);
		(page == syra_site.mainPage) && exports.refreshBrowserTitle(page);
...
exports.showTip = function(item, show, text, target) {
	if (show) {
		if (!item.titleTipText) {
			item.titleTipText = document.createElement("div");
			item.titleTipText.className = "s-popup-info";
		}
		item.titleTipText.textContent = text;
		item.titleTip = syra_site.dialogManager.openPopup(item.page, {
			content: {},
			slot: item.titleTipText,
			position: {
				my: "left bottom",
				at: "left top",
				of: $(target || item.domItem)
			}
		});
	} else {
		if (item.titleTip) {
			item.titleTip.close();
			item.titleTipText = item.titleTip = null;
		}
	}
};
...
exports.dispose = function() {
	if (syra_site) {
		...
		_pageDesignerOpener = _topPanel = _helpPopupSlot = _helpPopup = null;
	}
};