...
var searchFieldPage = require('syracuse-ui/lib/search/searchField/searchFieldPage');

var _pageDesignerOpener, _titleTip;
...
exports.toggleTopPanel = function(show, event, onBeforeClick) {
	if (_topPanel) {
		...
		if (_topPanel.container) {
			isVisible && syra_site.dom.setZIndex(_topPanel.container);
	...
	if (text) {
		...
		_titleTip.timeOut = setTimeout(function() {
			...
			if (text.indexOf("<em") >= 0) {
				_titleTip.textItem.innerHTML = text.replace(/<em>/g, "<em class='s-title-tip-em'>");
			} else {
...
exports.addSearchField = function(slot) {
	syra_site.searcher = searchFieldPage.load(slot);
};
...
function _moveToDomChildren(source, target) {
	syra_site.dom.empty(target);
	var nodes = source.children;
	for (var ii = 0, jj = nodes.length; ii < jj; ii++) {
		target.appendChild(nodes[ii]);
	}
}
...
exports.onMaximizeBoxClick = function(box) {
	...
	if (box.isMaximized) {
		if (!box._memBody) {
			box._memBody = document.createElement("div");
		}
		_moveToDomChildren(slot, box._memBody);
		if (box.tabTitle) {
		...
		_moveToDomChildren(box._memBody, slot);
		syra_site.dom.removeChild(box._memBody);
		delete box._memBody;
		_setMaximizeBtn(box);
	}
