...
exports.toggleTopPanel = function(show, event, onBeforClick){
    ...
    if (show) {
        ...
        site.dom.isParentNode(_topPanel.opener, event.target))) {
            return;
        }
        if (_topPanel.panel) {
            var panel = _topPanel.panel;
            if (event) {
                var isNavigationLink = panel.navigationBar && site.dom.isParentNode(panel.navigationBar.domItem, event.target);
                if (!isNavigationLink && site.dom.isParentNode(panel.domItem, event.target)) {
                    return;
                }
            }
        }
