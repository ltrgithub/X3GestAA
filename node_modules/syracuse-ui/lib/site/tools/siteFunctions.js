    ...
    if (show) {
        if (!_topPanel.panel) {
            if (!_topPanel.container) {
                ...
                document.site.layoutSlot.appendChild(_topPanel.container);
            }
            ...
            if (document.dir === 'rtl') {
                ...
                _topPanel.container.style.right = (site.layoutSlot.getBoundingClientRect().right - rect.right) + "px";
            }
            _topPanel.panel = new TopPanel();
            _topPanel.panel.$prototype = site.$prototype;
            _topPanel.panel.page = site;
            site.dom.empty(_topPanel.container);
            _topPanel.container.style.display = "";
        ...
        document.site.dom.isParentNode(_topPanel.opener, event.target))) {
            return;
        }
        if (_topPanel.panel) {
            if (event && $(event.target).closest(_topPanel.panel.domItem).length) {
                return;
            }
            _topPanel.panel.closePanel();
        }
        _topPanel.panel = null;
    }
    if (_topPanel.opener) {
        site.dom.toggleClass(_topPanel.opener, "s-open", _topPanel.panel != null);
    }
};

exports.showUserIdentity = function(userProfile, newData){
    if (_topPanel.opener && userProfile.dataset.user && !document.site.nameDiv) {
