...
function _maximizeItem(btn) {
	...
	if (syra_site.landingPageMaster) {
		_applyToBar(syra_site.landingPageMaster.bar, item.isMaximized);
		syra_site.landingPageMaster.resizeArticle(true);
	} else {
		item.page.resizeArticle(true);
	}
	var state = item.isMaximized ? "minimize" : "maximize";
	syra_menus.updateButtonIcon(btn.link, syra_local["box_" + state], state);
}
	...
	if (this.parent.isSection) {
		...
		if (section.isMaximized) {
			if (section.tabTitle) {
				...
				section.layoutParent.openTab(section);
			}
		} else {
			if (section.tabTitle) {
