...
function _matchProperty(instance, expression, property, record, properties, $subRecordKey) {
	var value = undefined;
...
exports.render = function(box, expression, layoutSlot, $fieldCss, $isEditMode) {
	if (expression[1] == "@") {
		var text = exports.parse(box.getArticle(), expression);
		...
		for (var ii = 0, jj = matches.length; ii < jj; ii++) {
			var match = syra_site.dom.formatHTMLText(matches[ii], false);
...
function _localize(article, text) {
	...
	if (!localized) {
		...
		if ($allLocalization) {
			...
			if (!localized) {
				var langs = Object.keys($allLocalization);
				for (var ii = 0, jj = langs.length; ii < jj && !localized; ii++) {
					localized = $allLocalization[langs[ii]][text];
				}
			}
		}
	}
			...
			for (var ii = 0, jj = matches.length; ii < jj; ii++) {
				...
				if (matchResult && matchResult.value != null) {
					if (matchResult.value.indexOf && matchResult.value.indexOf("{") >= 0) {
						...
						matchResult.value = exports.parse(matchResult.article, matchResult.value, record, properties, $subRecordKey, ignoreCurrent);
					} else {
