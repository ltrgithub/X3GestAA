...
exports.isSearchUrl = function($url) {
	return $url && (decodeURIComponent($url).indexOf("$search?representation=queryResult.$search&q=") >= 0 ||
		decodeURIComponent($url).indexOf("$xsearch?representation=queryResult.$search&q=") >= 0 ||
		decodeURIComponent($url).indexOf("$fsearch?representation=queryResult.$search&q=") >= 0);
};
			...
			"$items": [{
				...
				onOpen: function(block) {
					if (block.page && block.page.searchFacets) {
						block.page.searchFacets.searchFacetsBody.style.display = "";
					}
				}
			}, {
				"$category": "section",
				...
				onOpen: function(block) {
					if (block.page && block.page.searchFacets) {
						block.page.searchFacets.searchFacetsBody.style.display = "none";
					}
				}
			}]
		}
