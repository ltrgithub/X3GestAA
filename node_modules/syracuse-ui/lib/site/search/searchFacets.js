...
exports.SearchFacets = helpers.defineClass(SearchFacets, Article, {
	...
	setDataBind: function($searchFacets, record, metaData) {
		...
		if (this.$searchFacets = $searchFacets) {
			...
			// clean
			document.site.dom.empty(this.domItem);
			...
			searchFacetsTitle.textContent = document.site.localize.searchFacetFilters;
			searchFacetsHeader.appendChild(searchFacetsTitle);
			this.domItem.appendChild(searchFacetsHeader);
			...
			for (var ii = 0, jj = $searchFacets.length; ii < jj; ii++) {
				...
				if ($searchFacets[ii].$members && $searchFacets[ii].$members.length > 0) {
					...
					for (var mm = 0, pp = $searchFacet.$members.length; mm < pp; mm++) {
						...
						document.site.setSpecificAttributes(checked);
						member.appendChild(checked);

						var $member = $searchFacet.$members[mm];
						if ($member.$selected) {
							checked.setAttribute('checked', true);
							this._updateFilters($searchFacet.$facet, $member.$value || $member.$title, true);
						}
						...
						if ($member.$links) {
							...
							var links = this.page.loadNewItem(links, {
								$category: "menus",
								$skin: "s-list-menus",
								$layout: {
					...
					headerTitle.className = this.$itemSkin + "-header-title";
					...
					this.searchFacetsBody.appendChild(facet);
				}
			}
		}
	},
