...
var SearchVignettePageBrowse = require('./searchVignettePageBrowse').SearchVignettePageBrowse;
...
exports.SearchVignettePage = helpers.defineClass(SearchVignettePage, DesktopPage, {
    ...
    onClickPicker: function(picker){
        ...
        switch ($bind) {
            case "sv-module-picker":
                var modId = this.selectedModId = picker.getAttribute("sv-module-id");
                if (!this.selectedModId || (this.selectedModId && this.selectedId != modId)) {
                    document.site.emptyDom(this.categoriesLayout);
                    this.selectedModId = picker.getAttribute("sv-module-id");
                    ...
                    for (var ii = 0, jj = catKeys.length; ii < jj; ii++) {
                        ...
                        categoryHeaderTitle.textContent = this.vignetteData.modules[modId].categories[catKeys[ii]].title;
                        ...
                        this.categoriesLayout.appendChild(categoryHeader);
                        this.categoriesLayout.appendChild(categoryBody);
                    }
                }
                break;
    ...
    _updatePage: function(data){
        ...
        if (data.$resources && data.$resources.length > 0) {
            ...
            this.scrollview.removeChild(this.body);
            this.scrollview.appendChild(this.browseView);
        }
        else {
        ...
        for (var ii = 0, jj = data.$resources.length; ii < jj; ii++) {
            ...
            if (!hasCategories) {
                ...
                for (var mm = 0, pp = resource.categories.length; mm < pp; mm++) {
                    category = resource.categories[mm];
    ...
    _buildBrowseView: function(){
        ...
        item.appendChild(fieldSlot);
        
        // browse content slot
        var browseLayout = document.createElement("div");
        browseLayout.className = "s-sv-browse-layout";
        item.appendChild(browseLayout);
        
        // modules layout
        var modulesLayout = document.createElement("div");
        modulesLayout.className = "s-sv-browse-modules";
        browseLayout.appendChild(modulesLayout);
