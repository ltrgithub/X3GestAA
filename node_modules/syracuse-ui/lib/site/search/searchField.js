...
exports.SearchField = helpers.defineClass(SearchField, Field, {
	...
	_showPicker: function(onOut) {
		...
		if (show) {
			this.picker.style.opacity = onOut ? 0.4 : 1;
	...
	onClearClick: function() {
		...
		this._autoCompletePopup && this._autoCompletePopup.close();
	...
	toggleAutoCompleteList: function(show) {
		...
		if (self.getDataValue() !== "") {
			...
			if (self._autoCompletePopup) {
				...
				self._autoCompletePopup = syra_site.dialogManager.openPopup(self.boxParent, {
					content: self,
	...
	_appendLinksPicker: function() {
		if (this.$rootLinks.$links && this.$rootLinks.$links.$suggest) {
			this._isAutoComplete = true;
			this.picker = this.addPicker("onClearClick", "clear");
			this.picker.style.display = "none";
		}
