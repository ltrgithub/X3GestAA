...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	...
	onMenuClick: function(menu) {
		switch (menu.$bind) {
			...
			case "$searchSimilar":
				var isDataSource = this.idMap["searchDataTab"].tabTitle.className.indexOf("s-tab-open") >= 0;
				this["searchMode" + (isDataSource ? "Data" : "Functions")] = menu.$bind;
				this._sendOneSearchRequest({
					$url: searchUtils.getSearchModeLinks()[this._getSearchMode(isDataSource)].$url + (isDataSource ? "&source=data" : "&source=functions")
				}, this.dataset, {}, isDataSource);
				return false;
			default:
				return true;
		}
	...
	fetch: function(options) {
		var $queryFunctionsLink = {
			$url: searchUtils.getSearchModeLinks()["$searchStartingWith"].$url + "&source=functions"
		};
		var $queryDataLink = {
			$url: searchUtils.getSearchModeLinks()["$searchStartingWith"].$url + "&source=data"
		};
	...
	_sendOneSearchRequest: function($query, dataset, options, isDataSource, selectTab) {
		...
		self.formatMenuUrl($query, dataset, function($url, isCanceled) {
			...
			document.controller.sendRequest(self, options, function(data) {
				...
				data.$searchModeDescription = self._getSearchModeDescription(self._getSearchMode(isDataSource), data.query);
				self._updateLinksQueryText(data, isDataSource);

				//self._appendDescriptionData(data[isDataSource ? "$queryData" : "$queryFunctions"]);

				// update search result in each vignette (tab)
				if (isDataSource) {
					self.boundFields.$vignetteData[0].vignette.applyChange(data);
				} else {
					self.boundFields.$vignetteFunctions[0].vignette.applyChange(data);
				}
	...
	_updateLinksQueryText: function(data, isDataSource) {
		...
		for (var ii = 0, jj = linksToUpdate.length; ii < jj; ii++) {
			...
			data.$links[mode] = {
				$title: this._getSearchModeDescription(mode, data.query)
			};
			data.$links[mode].$isHidden = mode == this._getSearchMode(isDataSource);
		}
	},
	_getSearchMode: function(isDataSource) {
		return this["searchMode" + (isDataSource ? "Data" : "Functions")] = this["searchMode" + (isDataSource ? "Data" : "Functions")] || searchUtils.getInitSearchMode();
	},
	_getSearchModeDescription: function(mode, description) {
		return searchUtils.getSearchModeLinks()[mode].$title.replace('{$search}', description);
	},
