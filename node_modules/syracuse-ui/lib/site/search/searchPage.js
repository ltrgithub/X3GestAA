...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	fetch: function(options) {
		...
		// options
			...
			if (options.jsonParams) {
				this._sendOneSearchRequest($queryDataLink, this.dataset, options);
			}
			// pager step navigation (pagination)
				...
				if (options.$location) {
					...
					this._sendOneSearchRequest($location, this.dataset, options, isFunctionsQuery);
				}
				...
				// get corresponding tab information, update content accordingly
					...
					if (options.field && options.field.$item && options.field.$item.$bind) {
						...
						var isFunctionsQuery = options.field.$item.$bind == "$queryData" ? false : true;
						this._sendOneSearchRequest(isFunctionsQuery ? $queryFunctionsLink : $queryDataLink, this.dataset, options, isFunctionsQuery);
					} else {
						this._sendAllSearchRequests($queryFunctionsLink, $queryDataLink, this.dataset, options);
					}
				}
	...
	_sendAllSearchRequests: function($functionsQuery, $dataQuery, dataset, options) {
		...
		self.formatMenuUrl($functionsQuery, dataset, function($url, isCanceled) {
			...
			document.controller.sendRequest(self, options, function(data) {
				...
				self._updateTabTitle(false, data.$totalResults);
		...
		self.formatMenuUrl($dataQuery, dataset, function($url, isCanceled) {
			...
			document.controller.sendRequest(self, options, function(data) {
				...
				//data.$searchFacets
				delete data.$resources;
				self._updateTabTitle(true, data.$totalResults);
				self.applyChange(data);
			});
		});
	},
	...
	_updateTabTitle: function(isData, recordsNb) {
		this.idMap[isData ? "searchDataTab" : "searchFunctionsTab"].setTitle((isData ? "Data" : "Functions") + " (" + recordsNb + ")");
	},
