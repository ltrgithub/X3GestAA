...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	...
	_sendOneSearchRequest: function($query, dataset, options, isDataSource, selectTab) {
		...
		self.formatMenuUrl($query, dataset, function($url, isCanceled) {
			...
			document.controller.sendRequest(self, options, function(data) {
				...
				self._appendDescription(data[isDataSource ? "$queryData" : "$queryFunctions"]);
	...
	_appendDescription: function($resources) {
		for (var ii = 0, jj = $resources.length; ii < jj; ii++) {
			$resources[ii].$description = [];
			var $binds = Object.keys($resources[ii].$properties);
			for (var mm = 0, pp = $binds.length; mm < pp; mm++) {
				var property = $resources[ii].$properties[$binds[mm]];
				if (property.$isHighlight || property.$isSummary) {
					$resources[ii].$description.push(property.$title + ": " + $resources[ii][$binds[mm]]);
				}
			}
		}
	},
