...
exports.SearchRequest = helpers.defineClass(SearchRequest, RawPage, {
	...
	_bindEvents: function(bind) {
		...
		if (self._autoCompleteList) {
			if (bind) {
				$(self._autoCompleteList).delegate(".s-search-rq-field-cell-record", "click", function() {
				...
				}).delegate(".s-search-rq-field-cell-record", "mouseenter", function() {
	...
	unloadAutoCompleteList: function() {
		if (this._autoCompleteList) {
			...
			this.cleanBoundFields();
			this.removeItem(this.searchItem, true);
			this.searchItem = null;
			this.searchField.$isArrowClicked = this.searchField.$isRecordTriggered = null;
		}
	},
	cleanBoundFields: function() {
		if (this.boundFields) {
			this.boundFields.$query1.splice(0, 1);
			this.boundFields.$query2.splice(0, 1);
		}
	},
	dispose: function() {
...
exports.SearchField = helpers.defineClass(SearchField, Field, {
	...
	onInputKeydown: function(input, event) {
		...
		if (self._isAutoComplete) {
			...
			if (event.keyCode !== 13) {
				...
				if (self._autoCompletePopup) { // handle navigation in drop down search result list (down(40) and up(38))
					if ((event.keyCode == 40 || event.keyCode == 38) && self._autoCompletePopup) {
						var searchResRecords = self.page.boundFields.$query1[0].records.concat(self.page.boundFields.$query2[0].records);
						var index = self._getFocusRecIndex(searchResRecords);
						var jj = searchResRecords.length;
						if (index != undefined) {
							searchResRecords[event.keyCode == 40 ? ((index + 1) <= (jj - 1) ? (index + 1) : 0) : ((index - 1) >= 0 ? (index - 1) : jj - 1)].$$item.addClass("s-search-rq-field-cell-record-hover");
						} else {
							searchResRecords[event.keyCode == 40 ? 0 : jj - 1].$$item.addClass("s-search-rq-field-cell-record-hover");
						}
	...
	dispose: function() {
		this.$isArrowClicked = null;
		this.$isRecordTriggered = null;
