...
exports.SearchRequest = helpers.defineClass(SearchRequest, RawPage, {
	...
	createNewItem: function($item, boxParent) {
		if ($item.$bind == "$search") {
			...
			searchField.localize = syra_site.localize;
	...
	notifyDataChange: function(field, value) {
		if (field.$item.$bind == "$search") {
			...
			if (value.replace(/\s+/g, '') !== "") {
				...
				syra_controller.executeMenu(this.menuItems["$searchLink"][0]);
			}
		}
	},
	...
	loadAutoCompleteList: function(searchField) {
		...
		syra_site.urlMaker.formatMenuUrl(self, $queryLink1, self.dataset, function($url, isCanceled) {
		...
		syra_site.urlMaker.formatMenuUrl(self, $queryLink2, self.dataset, function($url, isCanceled) {
			syra_controller.sendRequest(self, {
				$location: {
					$url: $url
				}
	...
	unloadAutoCompleteList: function() {
		if (this._autoCompleteList) {
			syra_site.dom.empty(this._autoCompleteList);
	...
	updateLocalization: function() {
		this.applyChange({
			$properties: {
				$search: {
					$title: syra_site.localize.searchFieldTitle
				}
			},
			$links: {
				$searchLink: {
					$title: syra_site.localize.siteSearchLink,
				},
				$advancedSearch: {
					$title: syra_site.localize.searchAdvanceLabel
				}
			}
		});
	},
