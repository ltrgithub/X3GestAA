...
exports.SearchRequest = helpers.defineClass(SearchRequest, RawPage, {
    ...
    notifyDataChange: function(field, value){
        if (field.$item.$bind == "$search") {
            var self = this;
            self.dataset[field.$item.$bind] = value;
            self.menuItems["$searchLink"][0].$url = this.$prototype.$links.$searchLink.$url + "";
    ...
    loadAutoCompleteList: function(searchField){
        ...
        self.loadNewItem($(self._autoCompleteList), {
            ...
            $layout: {
                $items: [{
                    $skin: searchRqSkin+"test1",
                    $css: searchRqSkin+"test2",
                    $title: "Functions",
                    $bind: "$query1",
                    $isPagerHidden: true,
