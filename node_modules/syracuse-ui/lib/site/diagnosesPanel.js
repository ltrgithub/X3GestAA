...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    showDiagnosesPanel: function(){
        ...
        if (self._fieldDiagnoses.length > 0) {
            self.$$nextLink.show();
            if (self._fieldDiagnoses.length > 1) {
                self.$$prevLink.show();
            }
            else {
                self.$$prevLink.hide();
    ...
    _initializeCore: function(){
        ...
        html.className = "s-diag-view-nav-prev";
        html.textContent = self.localize.diag_prev;
        self.$$diagNavBar.append(self.$$prevLink = $(html));
        self.$$prevLink.hide();
        ...
        /*
         self.$$prevLink.bind("click", function(){
         self.numFocusedField = self.numFocusedField || 0;
         if (self._fieldDiagnoses.length && self._fieldDiagnoses.length > 1) {
         if (self.numFocusedField == 0) {
         self.numFocusedField = (self._fieldDiagnoses.length - 1);
         }
         //                else {
         //                    self.numFocusedField--;
         //                }
         self.focusField(self._fieldDiagnoses[self.numFocusedField].field);
         self.focusDiag(self.numFocusedField, self._fieldDiagnoses[self.numFocusedField].label);
         self.numFocusedField--;
         }
         });
         ...
         if (self.numFocusedField == self._fieldDiagnoses.length) {
        ...
        self.$$item.delegate("a.s-diag-view-msg-link", "click", function(){
        ...
        }).delegate("a.s-diag-view-nav-next,a.s-diag-view-nav-prev", "click", function(){// bind actions for navigation links
            var index;
            var next = $(this)[0].className.indexOf("next") > -1;
            ...
            if (index != undefined) {
                if (next) {
                    ...
                    if (index == 0) {
                        index = self._fieldDiagnoses.length - 1;
                    }
                    else {
                        index--;
                    }
        ...
        self.$$diagOpener = document.site.$$diagViewOpener;
        if (self.$$diagOpener) {
    ...
    _onDiagOpenerClick: function(){
        ...
        if (self.$$diagOpener) {
            if (self.$$item) {
                self.$$item.toggle();
                self.$$diagOpener.toggleClass("s-diag-view-close").toggleClass("s-diag-view-open");
                self._fitPageToContainer();
            }
        }
    },
