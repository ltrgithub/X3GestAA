...
helpers.defineClass(Group, null, {
    ...
    drawDiagnose: function($diagnose, $$fieldLink){
        ...
        if ($$label.find("br")[0]) {//$diagnose.message.indexOf("\n") > 0
            ...
            self._wrapTextToHide(expandedText, $$label, $$diagView);
        }
        else {
            ...
            if ($$label[0].textContent.length > diagMaxLength) {
                ...
                self._wrapTextToHide(expandedText, $$label, $$diagView, true);
            }
        }
        
        // close button
        //self._appendDiagnoseCloseButton($$diagView);
        
        $$diagView[0].style.display = "none";
        self.$$group[0].appendChild($$diagView[0]);
    },
    _appendDiagnoseCloseButton: function($$diagView){
        var html = document.createElement("a");
        html.className = "s-diag-view-msg-close";
        $$diagView[0].appendChild(html);
    },
    _wrapTextToHide: function(textToHide, $$label, $$diagView, lineBreak){
...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    pushFieldDiagnose: function(field){
        var self = this;
        if (self._fieldDiagnoses) {
            var isInArray = false;
            for (var ii = 0; ii < self._fieldDiagnoses.length; ii++) {
                isInArray = self._fieldDiagnoses[ii].id == field.id ? true : isInArray;
                if (isInArray) {
                    break;
                }
            }
            if (!isInArray) {
                self._fieldDiagnoses.push(field);
            }
        }
    },
    _removeFieldDiagnose: function(field){
        ...
        if (self._fieldDiagnoses) {
            ...
            for (var ii = 0; ii < self._fieldDiagnoses.length; ii++) {
                index = self._fieldDiagnoses[ii].id == field.id ? ii : index;
                if (index != undefined) {
                    break;
                }
            }
