...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    _refreshViewer: function(showAll){
        if (!this.disableViewer) {
            ...
            self.groups._groups.forEach(function(group){
                ...
                var domCount = document.createElement("a");
                domCount.className = "s-diag-view-count";
                domCount.setAttribute("id", 's-diag-view-count-' + group.severity);
                if (count) {
                    total += count;
                    countSeverity++;
                    group.$$group.appendTo(self.$$body);
                    domCount.className += " s-open";
                    self.$$header[0].appendChild(domCount);
                    $(domCount).bind("click", function(){
                        self.$$body.empty();
                        group.$$group.appendTo(self.$$body);
                        self.$$header.children().toggleClass("s-open", false);
                        $(this).toggleClass("s-open", true);
                    }).text(count + " " + localize["diag_" + group.severity]);
                }
                if (count == 0) {
            ...
            if (total) {
                if (countSeverity > 1) {
                    ...
                    all.className = "s-diag-view-count s-open";
                    all.setAttribute("id", "s-diag-view-count-diagnoses");
                    self.$$header[0].appendChild(all);
                    ...
                    $(all).bind("click", function(){
                        ...
                        self.$$header.children().toggleClass("s-open", false);
                        $(this).toggleClass("s-open", true);
                    }).text(total + " Diagnose(s)");
                }
                
            }
            
            if (total > 0) {
                document.site.setZIndex(self.$$item.show());
            }
            else {
                self.$$item.hide();
            }
    ...
    onWindowResize: function(setMinimumDimensions){
        ...
        this.$$core[0].style.maxWidth = (bodyWidth - this.$$header.outerWidth() - 200) + "px";
        this.$$body[0].style.maxHeight = (bodyHeight - this.$$header.outerHeight() - 200) + "px";
        //this.$$body[0].style.maxWidth = (bodyWidth - this.$$header.outerWidth() - 200) + "px";
    ...
    _initializeCore: function(){
        ...
        self.$$container = self.$$item; // container for dragging property
        html = document.createElement("a");
        html.className = "s-diag-view-close";
        self.$$item.append(html);
        
        // grab zone to drag the diagnose
        html = document.createElement("div");
        html.className = "s-diag-view-title";
        var $$diagViewTitle = $(html);
        $$diagViewTitle.text(localize.diag_title);
        ...
        // making diagnose draggable
        document.site.setDraggable(self, $$diagViewTitle);
        ...
        self.$$core.append($$diagViewTitle).append(self.$$header).append(self.$$body);
    ...
    drawLinks: function(message, page){
        ...
        var self = this;
        if (!self._menus) {
            self._menus = document.itemFactory.load(self.$$item, {
                $category: "links",
