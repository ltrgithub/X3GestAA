...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    showDiagnosesPanel: function(){
        ...
        if (self.total > 1) { //(self.$$body.find("label").length > 1)
            var moreText = (self.total) - 1 == 0 ? self.localize.diag_expand : self.localize.diag_expand + (self.total - 1);
    ...
    _initializeCore: function(){
        ...
        self.$$item.delegate("a.s-diag-view-msg-link", "click", function(){
            ...
            if (field) {
                ...
                self._onNavigationClick(0, field);
            }
            return false;
            ...
            self._onNavigationClick($(this)[0]);
        });
    ...
    _onNavigationClick: function(navLink, field){
        var self = this;
        var index;
        ...
        if (!navLink && field) {
            ...
            var next = navLink.className.indexOf("next") > -1;
            if (index != undefined) {
                if (next) {
        ...
        if (isLastDialog) {
            ...
            if (self.$$expandLink[0].textContent.indexOf(self.localize.diag_expand) != -1) {
                ...
                if (self.total == 1) {
                    ...
                    self.$$expandLink[0].textContent = self.localize.diag_expand + (self.total - 1);
                }
            }
            else {
                if (self.total == 1) {
