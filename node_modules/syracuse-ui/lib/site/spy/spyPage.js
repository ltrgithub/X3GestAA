...
exports.SpyPage = helpers.defineClass(SpyPage, DesktopPage, {
	loadBox: function($fieldsPrototype, initData, $initDiagnoses) {
		...
		if (window.opener && window.opener.document.site) {
			window.opener.document.site.spyer.onSpyPageLoaded(this);
		}
	},
		...
		if (value.$notifyData || value.$receiveData) {
			try {
				if (value.$receiveData) {
					window.opener.document.site.spyer.getSpyedPage().applyChange(JSON.parse(this.boundFields.$fieldReceive[0].getDataValue()));
				}
				if (value.$notifyData) {
					window.opener.document.site.spyer.getSpyedPage().notifyChangeToServer(JSON.parse(this.boundFields.$fieldNotify[0].getDataValue()));
				}
			} catch (error) {
