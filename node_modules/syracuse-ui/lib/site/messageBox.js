...
exports.MessageBox = helpers.defineClass(MessageBox, null, {
    open: function(options){
        ...
        self.$$container = self.$$item = $(domItem);
        ...
        $(document).bind("keypress.msgbox", function(evt){
            var btn = self.$$defaultBtn, btnArray, i, len;
            switch (evt.keyCode) {
                ...
                case keyHelper.keyCode.TAB:
                    btnArray = Object.keys(self.$$btns);
                    if (btnArray.length > 1) {
                        for (i = 0, len = btnArray.length; i < len; i++) {
                            if (self.$$btns[btnArray[i]] == self._$$focusBtn) {
                                break;
                            }
                        }
                        if (i < len) {
                            i = evt.shiftKey ? (i == 0 ? len - 1 : i - 1) : (i + 1) % 2;
                            self._$$focusBtn.removeClass("s-msgbox-button-default");
                            self._$$focusBtn = self.$$btns[btnArray[i]];
                            self._$$focusBtn.addClass("s-msgbox-button-default");
                            setTimeout(function(){
                                self._$$focusBtn.focus();
                            }, 5);
                        }
                    }
                    break;
        ...
        $(document).bind("keydown.msgbox", function(evt){
            switch (evt.keyCode) {
                ...
                case keyHelper.keyCode.ALPHAPAD_A:
                    if (self.keys && self.keys[keyHelper.keyCode.CTRL]) {
                        // select text
                        selectText(self.$$coreFormatted.is(":visible") ? self.$$coreFormatted[0] : self.$$coreRaw[0]);
                        return false;
                    }
                    else {
                        return true;
        ...
        $(document).bind("keyup.msgbox", function(evt){
            if (self.keys && self.keys[evt.keyCode]) {
                delete self.keys[evt.keyCode];
                return false;
            }
        });
        if (options.$showDetail) {
            this.$$bodyPicker.trigger("click");
        }
        self.$$defaultBtn.focus();
        self._$$focusBtn = self.$$defaultBtn;
    },
    ...
    _appendStackTrace: function(){
        ...
        self.$$bodyPicker.bind("click", function(){
            ...
            self._appendSelectPicker(self.$$bodyPicker.hasClass("s-msgbox-picker-opened"))
    ...
    _appendSelectPicker: function(isCoreVisible){
        var self = this;
        var div = document.createElement("div");
        if (isCoreVisible) {
            if (self.$$selectPicker) {
                self.$$selectPicker.show();
            }
            else {
                self.$$selectPicker = $(document.createElement("a")).addClass("s-msgbox-select-picker");
                self.$$selectPicker.text(self.localize.msgbox_select_picker);
                self.$$selectPicker.bind("click", function(){
                    selectText(self.$$coreFormatted.is(":visible") ? self.$$coreFormatted[0] : self.$$coreRaw[0]);
                });
                div = document.createElement("div");
                div.className = "s-msgbox-picker";
                self.$$body.find("div.s-msgbox-picker").after($(div).append(self.$$selectPicker));
            }
        }
        else {
            if (self.$$selectPicker) {
    ...
    drawLinks: function(message){
        ...
        var self = this;
        if (!self._menus) {
            self._menus = page.loadNewItem(self.$$links, {
                $category: "links",
                $skin: "s-msgbox-links",
                $isBindDisabled: true,
                $links: {},
    ...
    dispose: function(){
        ...
        $(document).unbind("keyup.msgbox");
        if (this.$$bodyPicker) {
            this.$$bodyPicker.unbind();
        }
        if (this.$$selectPicker) {
            this.$$selectPicker.unbind();
        }
    }
});
...
helpers.defineClass(Group, null, {
    create: function(severity, viewer){
        ...
        group.className = 's-msgbox-group-' + (this.severity = severity);
    ...
    bindEvents: function(bind){
        var self = this;
        if (bind) {
            self.$$group.delegate(".s-msgbox-diag-picker-close,.s-msgbox-diag-picker-open", "click", function(){
                $(this).toggleClass("s-msgbox-diag-picker-close").toggleClass("s-msgbox-diag-picker-open");
    ...
    drawLabel: function($diagnose){
        ...
        var message = {
            ...
            appCode: this.localize.msgbox_diag_appCode + ($diagnose.appCode || $diagnose.sdataCode)
        };
        this._messages.push(message);
        message.label.className = "s-msgbox-label-" + this.severity;
        ...
        if ($diagnose.stackTrace) {
            ...
            picker.className = "s-msgbox-diag-picker-close";
...
function Groups(viewer){
    ...
    self._groups = (["error", "fatal", "warning", "info"]).map(function(severity){
        (self.severities[severity] = new Group()).create(severity, viewer);
