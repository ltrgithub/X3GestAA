...
exports.MessageBox = helpers.defineClass(MessageBox, null, {
    open: function(options){
        ...
        if (options.$autoClose) {
            ...
            $(self.$$header.children()[0]).text(options.$title + "  (" + count + ")");
            self.autoCloseTimeOut = setInterval(function(){
                var countTxt = "(" + (--count) + ")";
                $(self.$$header.children("div.s-msgbox-header-title")[0]).text(options.$title + " " + countTxt)
                //self.$$header.text(options.$title + " " + countTxt)
                if (count == 0) {
