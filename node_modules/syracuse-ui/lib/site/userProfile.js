...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
    ...
    onMenuClick: function(menuItem){
        if (menuItem.$bind == "$logout") {
            ...
            disconnectUtils.onUserActionUnload(options);
        }
        else {
            return true;
        }
    ...
    onNotifyDataChange: function(field, value){
        ...
        if (field.$item.$bind == "selectedLocale") {
            //debugger;
            // callback : self.notifyDataChange(self, self.currentValue);
            return false;
        }
        return true;*/
    },
    onClickPicker: function(pickerId, event){
        ...
        var onSelectRecord = function(selectedRecords){
            records = helpers.object.clone(selectedRecords);
            var options = {
                fieldData: {
                    field: field,
                    $lookup: field.$lookup || field.$rootLinks.$links.$lookup,
                    records: records
                }
            }
            disconnectUtils.onUserActionUnload(options);
        ...
        if (field && field.$item && field.$item.$bind && field.$item.$bind == "selectedLocale") {
            field.onClickPicker(event.currentTarget, onSelectRecord);
        }
        else {
            return true;
        }*/
