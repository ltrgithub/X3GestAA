		...
		if (self.openerBtn) {
			if (show && !self._popup) {
				if (!self.lists) {
					self.lists = [];
					self.addTopList();
					self.applicationConnection.showList(self.domItem);
					self.addMiddleList();
					self.addBottomList();
				}
				self._popup = syra_over.openPopup(syra_site, {
					...
					onresize: function() {
						if (self.lists) {
							...
							self.lists[1].resize(syra_site.size.headerPopupMaxHeight - self.lists[0].scrollSlot.clientHeight - height - self.lists[2].scrollSlot.clientHeight);
						}
					},
					close: function() {
						setTimeout(function() {
