...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
	...
	onClickPicker: function(btn, event) {
		var field = document.site.findField(event.target);
	...
	getRegionalPreferences: function(success) {
		...
		document.controller.sendRequest(this, {
			$location: {
				$url: document.site.expressionMaker.parse(this, this.boundFields.selectedLocale[0].getLookupSettings().$url)
			}
		}, function(data) {
	...
	ensureFormBlock: function() {
		...
		if (!self.formBlock) {
			self.formBlock = self.loadNewItem(self.domItem, {
				$category: "section",
				$title: document.site.localize.userProfile_up_welcome,
				$skin: "s-user-form",
				$layout: {
					$items: [{
						$bind: "$save",
						...
						$isEditMode: true,
						$isHidden: document.site.siteOptions.isUserEndpointChangeDisabled,
						$isTopLabelAlignment: false,
						$isReferenceTitleVisible: false,
						$skin: "s-user-profile-ref",
						$css: "s-user-profile-selectedRole",
						$bind: "selectedRole",
						$title: document.site.localize.userProfile_up_role
					}, {
						$isEditMode: true,
						$isTopLabelAlignment: false,
						$skin: "s-user-profile-ref",
						...
						$title: document.site.localize.userProfile_up_endpoint
					}, {
						$isEditMode: true,
						$isTopLabelAlignment: false,
						$skin: "s-user-profile-ref",
						...
						$bind: "selectedLocale",
						$title: document.site.localize.userProfile_up_locale
					}]
				}
			});
	...
	showDiagnoses: function(message) {
		...
		if (message.$diagnoses && Object.keys(message.$diagnoses).length > 0) {
			...
			options.$message = document.site.localize.userProfile_siteMsgboxMsg;
			options.$type = "alert";
			options.$buttons = "ok";
			options.$title = document.site.localize.userProfile_upDiagMsgboxTitle;
	...
	loadBookmarks: function(onsuccess) {
		var self = this;
		document.controller.sendRequest(self, {
			$location: {
				$url: document.site.expressionMaker.parse(self, self.$menus.$bookmarks.$url)
			},
	...
	saveBookmarks: function(dataset, onsuccess) {
		var self = this;
		document.controller.sendRequest(self, {
			$location: {
				$url: document.site.expressionMaker.parse(self, self.$menus.$bookmarks.$url)
	...
	saveSitePreferences: function() {
		this.ensureSendBag().sitePreferences = this.dataset.sitePreferences;
		this.notifyActionChange(null, this, {
			$save: {
				$isRequested: true
			}
	...
	applyChange: function(newData) {
		var isFusionPage = document.site.mainPage && document.site.mainPage.isFusionPage;
		...
		if (document.site._topPanelOpener && this.dataset.user && !document.site.nameDiv) {
			...
			document.site.nameDiv.textContent = (this.dataset.user.firstName ? this.dataset.user.firstName + " " : "") + (this.dataset.user.lastName ? this.dataset.user.lastName : "");

			document.site._topPanelOpener.appendChild(document.site.nameDiv);
			...
			if (this.dataset.user.photo) {
				this.loadNewItem(photoDiv, {
					...
					$field: {
						$type: "image",
						$url: document.site.expressionMaker.parse(this, this.dataset.user.photo.$url, {
							$uuid: this.dataset.user.$uuid,
							$pluralType: "users",
							$key: this.$prototype.$key,
							$baseUrl: this.$prototype.$baseUrl
						})
					}
				});

				var newPhoto = helpers.object.clone(this.dataset.user.photo);
				newPhoto.$url = document.site.expressionMaker.parse(this, this.dataset.user.photo.$url, {
					$pluralType: "users",
					$key: this.dataset.user.$uuid,
					$baseUrl: this.$prototype.$baseUrl
				});
			...
			document.site._topPanelOpener.appendChild(photoDiv);
		}

		RawPage.prototype.applyChange.call(this, newData);

		document.site.onUserProfileChange(reloadMainPage);

		if (changeEndpoint && !reloadMainPage) {
			var url = document.site.history.getCurrentSdataUrl();
			if (document.site.urlMaker.parse(url).isFusion) {
				document.site.history.updateCurrentEndpoint(newData.selectedEndpoint);
				if (document.site.bookmarksBar) {
					document.site.bookmarksBar.refresh();
				}
			} else {
				document.site.gotoNavigationPage();
			}
		}
