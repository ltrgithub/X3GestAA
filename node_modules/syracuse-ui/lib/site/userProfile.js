...
exports.UserProfile = _helpers.defineClass(UserProfile, DesktopPage, {
	...
	addTopList: function() {
		...
		self.themeField = new syra_style.Theme(self, list.addLine("s-popup-primary-bottom-sep"));
		...
		if (self.openerBtn) {
			if (show && !self._popup) {
				...
				self._popup = syra_over.openPopup(syra_site, {
					...
					onresize: function() {
						if (self.lists) {
							self.lists[1].resize(syra_site.size.headerPopupMaxHeight - self.lists[0].scrollSlot.clientHeight - self.lists[2].scrollSlot.clientHeight);
						}
					},
					close: function() {
						setTimeout(function() {
