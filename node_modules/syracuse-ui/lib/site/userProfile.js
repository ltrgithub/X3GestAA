...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
    ...
    ensureFormBlock: function(){
        ...
        if (!self.formBlock) {
            ...
            document.site.emptyDom(document.site.logoutSlot);
            document.site.logoutSlot.appendChild(self.idMap.$logoutTop.mn);
        }
    },
    ...
    drawBox: function(){
        ...
        this._item.setAttribute("id", "s-user-profile");
    ...
    applyChange: function(newData){
        ...
        if (newData && newData.serverETag !== null) {
            if (module.etag != newData.serverETag) {
                var error = new Error("reload");
                error.isReloadRequest = true;
                throw error;
                return;
            }
        }
        ...
        if (Object.keys(this.dataset).length > 0) {
            ...
            if (this.dataset.selectedEndpoint && newData.selectedEndpoint) {
                reloadMainPage = this.dataset.selectedEndpoint.$uuid != newData.selectedEndpoint.$uuid
            }
            else {
                reloadMainPage = this.dataset.selectedEndpoint || newData.selectedEndpoint
            }
            if (!reloadMainPage) {
                if (this.dataset.selectedRole && newData.selectedRole) {
                    reloadMainPage = this.dataset.selectedRole.$uuid != newData.selectedRole.$uuid
                }
                else {
                    reloadMainPage = this.dataset.selectedRole || newData.selectedRole
                }
            }
        }
