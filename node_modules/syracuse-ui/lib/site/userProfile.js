...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
	...
	getLogonUrl: function() {
		...
		if (!profile) {
			var seg = syra_site.history.getUrlSegment();
	...
	onNotifyDataChange: function(field, value) {
		if (field && field.$item.$bind == "choiceTheme") {
			this.page.boundFields.selectedTheme[0].notifyFieldChange(value == 0 ? null : {
				$uuid: value
			});
			return false;
		}
	...
	getRegionalPreferences: function(success) {
		...
		syra_controller.callServer(this, {
			$location: {
	...
	ensureFormBlock: function() {
		...
		if (!self.formBlock) {
				...
				$layout: {
					$items: [{
						...
						onServerRequest: function(menu) {
							...
							if (menu.$diagnoses && menu.$diagnoses.length) {
								if (menu.$diagnoses[0].$severity == "error") {
									syra_site.showDiagnoses({
										$diagnoses: menu.$diagnoses
									});
								}
							}
							return false;
			...
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if ($bind == "selectedRole") {
					$item.$isHidden = syra_site.siteOptions.isUserEndpointChangeDisabled;
				}
				if ($bind == "selectedTheme") {
					$item.$isHidden = true;
				}
				if ($bind == "choiceTheme") {
					icon = icon.replace("choice", "");
					$item.$format = "$combo";
					$item.$isHidden = !(self.dataset.themes && self.dataset.themes.length > 0);
				}
				var field = self.loadNewItem(self.formBlock.body, $item, self.formBlock);
				field.domTitle.insertBefore(syra_menus.addFontIconFlag("s-user-profile-flag", icon), field.domTitle.firstChild);
			}
	...
	loadBox: function(initData) {
		if (this.$prototype && this.$prototype.$properties) {
			...
			syra_site.showDiagnoses(initData, this);
	...
	notifyDataChange: function(field, value, doNotify) {
		...
		if (!doNotify && syra_site.fusionGateway) {
			if (["donotuse", "selectedLocale", "selectedRole", "selectedEndpoint"].indexOf(field.$item.$bind) >= 0) {
	...
	applyChange: function(newData) {
		...
		// change logo
		!newData.productName ? syra_site.updateProductName("") : syra_site.updateProductName(newData.productName);
		...
		if (!this.currentLangCode) {
			globals.context.localePreferences = newData.selectedLocale;
	...
	_applyNewSecurity: function(newEndpoint) {
		...
		syra_test.robot.logPreferences();
		...
		if (page && !page.disposed) {
			...
			if (newEndpoint) {
				urlSegments = syra_site.history.getUrlSegment();
	...
	onLogonSucees: function() {
		...
		syra_test.enableTestRobot = this.dataset.enableTestRobot;
