...
function _addFlagToField(page, $bind, flag) {
	var label = page.boundFields[$bind][0].domTitle;
	label.insertBefore(syra_menus.addFontIconFlag("s-user-profile-flag", flag), label.firstChild);
}
...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
	...
	ensureFormBlock: function() {
		...
		if (!self.formBlock) {
			...
			if (self.dataset.themes && self.dataset.themes.length > 0) {
				items.push({
					$isEditMode: true,
					$isReferenceTitleVisible: false,
					$css: "s-user-profile-field",
					$bind: "selectedTheme",
					$title: syra_local.userProfile_up_theme
				});
			}
			...
			_addFlagToField(this, "selectedRole", "role");
			_addFlagToField(this, "selectedEndpoint", "endpoint");
			_addFlagToField(this, "selectedLocale", "local");
			_addFlagToField(this, "selectedTheme", "theme");
