...
exports.UserProfile = _helpers.defineClass(UserProfile, DesktopPage, {
	dispose: function() {
		syra_dom.remove(this.popupSlot);
	...
	drawPage: function() {
		this.domItem = syra_dom.div(this.$skin);
	...
	loadFields: function() {
		if (!this._isFieldsloaded) {
			this.addItem(this.domItem, {
				...
				$bind: "selectedRole",
				$title: syra_local.userProfile_selectedRole
			});
			this.addItem(this.domItem, {
				$isEditMode: true,
				$inplace: true,
				...
				$title: syra_local.userProfile_selectedEndpoint
			});
			this.addItem(this.domItem, {
				$isEditMode: true,
				$inplace: true,
	...
	toggle: function(show, event, onClosePopups) {
		if (this.openerBtn) {
			if (show && !this.popupSlot) {
				this.popupSlot = syra_dom.div("s-user-profile-page", syra_site.layoutSlot);
				this.popupSlot.syrainout = syra_site.id;
				this.popupSlot.appendChild(this.domItem);
				this.loadFields();

				if (!this.technicalSlot) {
					this.technicalSlot = syra_dom.div("s-site-top-technical", this.domItem);
					...
					for (var ii = 0, jj = syra_config.$tecnhicalLinks.length; ii < jj; ii++) {
						...
						syra_site.addItem(this.technicalSlot, {
							$category: "link",
							$skin: "s-mn-link",
							$bind: $link.$bind
						}, syra_site);
				...
				syra_dom.hide(this.technicalSlot, syra_fusion && syra_fusion.activatedBook);

				syra_quality.page_onAfterLoad(this);
				this.resizeItem(true);

				var rect = this.openerBtn.link.getBoundingClientRect();
				if (syra_context.isRTL) {
					this.popupSlot.style.left = rect.left + "px";
				} else {
					this.popupSlot.style.right = (syra_site.layoutSlot.getBoundingClientRect().right - rect.right) + "px";
				}
				syra_dom.setZIndex(this.popupSlot);
			} else {
				//return;
				if (onClosePopups && event &&
					((event.target == this.openerBtn.link) ||
						this.openerBtn.link.contains(event.target))) {
					return;
				}
				if (event && this.popupSlot) {
					if (this.popupSlot.contains(event.target)) {
						return;
					}
				}
				syra_dom.empty(this.popupSlot);
				syra_dom.remove(this.popupSlot);
				this.popupSlot = null;
			}
			syra_dom.toggleClass(this.openerBtn.link, "s-open", !! this.popupSlot);
		}
	},
	showIdentity: function() {
		if (this.openerBtn) {
			if (this.dataset.user) {
				...
				this.openerBtn.link.textContent = (this.dataset.user.firstName ? this.dataset.user.firstName + " " : "") + (this.dataset.user.lastName ? this.dataset.user.lastName : "");

				var photo = syra_dom.div("s-site-user-photo");
				if (this.dataset.user.photo) {
					...
					syra_dom.anchor("s-site-user-photo-no", photo);
				}
				this.openerBtn.link.insertBefore(photo, this.openerBtn.link.firstChild);
			}
