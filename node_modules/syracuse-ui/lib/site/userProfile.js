...
exports.UserProfile = _helpers.defineClass(UserProfile, DesktopPage, {
	...
	drawPage: function() {
		...
		this.domItem.className = "s-user-profile s-over-popup";
	...
	addOpener: function() {
		...
		self.addItem(syra_site.headerBar, {
			$isEditMode: true,
		...
		}).input.syraShortCutTitle = syra_local.site_role_tip;
		...
		}).input.syraShortCutTitle = syra_local.site_endpoint_tip;
		...
		if (self.openerBtn) {
			if (show && !self._popup) {
				...
				if (!self.technicalSlot) {
					self.technicalSlot = syra_dom.ul("s-user-profile-links", self.domItem);
					...
					for (var ii = 0, jj = syra_config.$tecnhicalLinks.length; ii < jj; ii++) {
						...
						syra_site.addItem(syra_dom.li("s-user-profile-link-item", self.technicalSlot), {
							$category: "link",
							$skin: "s-user-profile-link",
							$bind: $link.$bind
						}, syra_site);
					}
				}
