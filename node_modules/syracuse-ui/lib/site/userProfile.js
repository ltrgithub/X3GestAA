		...
		if (self.openerBtn) {
			if (show && !self._popup) {
				...
				if (!self.updatseProfileBtn) {
					self.updatseProfileBtn = list.addButton({
						parent: self,
						text: syra_local.site_btn_profile,
						css: "s-popup-primary-btn-primary",
						click: function() {
					...
					self.themeField = new syra_style.Theme(self, list.addLine());
					list.addSeparator();

					self.techList = new List({
						parentNode: self.domItem,
						scroll: true
					});
					...
					for (var ii = 0, jj = syra_config.$tecnhicalLinks.length; ii < jj; ii++) {
						...
						self.techList.addItem(syra_site, {
							$category: "link",
							$skin: "s-popup-primary-btn-default",
							$bind: $link.$bind
						});
					}
					self.techList.addSeparator();
					var list = new List({
						parentNode: self.domItem
					});
					list.addSeparator();
				...
				self.techList.hide(syra_fusion && syra_fusion.activatedBook);
