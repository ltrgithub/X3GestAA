...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
	...
	switchToNewSession: function(onsuccess) {
		...
		syra_controller.postQuery({
			$url: syra_site.$item.$userProfileUrl
		...
		if (hasLocalizationChanged) {
			...
			if (newData.selectedRole !== undefined || newData.selectedEndpoint !== undefined) {
				if (newData.selectedRole !== undefined) {
					syra_site.$prototype.$role = this.selectedRoleUiid = newData.selectedRole === null ? null : newData.selectedRole.$uuid;
				}
			...
			if (syra_site.mainPage) {
				syra_site.pageLoader.showPageSecurity();
				syra_site.updateDocumentTitle();
				syra_site.mainPage.enableDesignerOpener();
			}
	...
	setEncodedValue: function() {
		...
		if (this.dataset.selectedRole) {
			value.role = this.dataset.selectedRole.$uuid;
		}
		if (this.dataset.selectedEndpoint) {
			value.ep = this.dataset.selectedEndpoint.$uuid;
		}
