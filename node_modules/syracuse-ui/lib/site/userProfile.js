...
exports.UserProfile = helpers.defineClass(UserProfile, RawPage, {
	...
	ensureFormBlock: function() {
		...
		if (!self.formBlock) {
			...
			var $binds = ["selectedRole", "selectedEndpoint", "selectedLocale", "selectedTheme"];
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if ($bind == "selectedTheme") {
					$isHidden = !(self.dataset.themes && self.dataset.themes.length > 0);
				}
				var field = self.loadNewItem(self.formBlock.body, {
					$isEditMode: true,
					$isHidden: $isHidden,
				...
				field.domTitle.insertBefore(syra_menus.addFontIconFlag("s-user-profile-flag", $bind.replace("selected", "").toLowerCase()), field.domTitle.firstChild);
			}
	...
	applyChange: function(newData) {
		...
		if (this._selectedThemeUiid !== undefined && this._selectedThemeUiid && newData.selectedTheme) {
			//syra_site.applyTheme(theme);
		}
