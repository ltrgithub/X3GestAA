...
exports.UserProfile = _helpers.defineClass(UserProfile, DesktopPage, {
	...
	drawPage: function() {
		...
		this.domItem.className = "s-over-popup";
		this.domItem.syrainout = this.domItem.syraPage = this.domItem.syraItem = this.id = "s-user-profile";
	...
	loadFields: function() {
		if (!this._isFieldsloaded) {
			var ul = syra_dom.ul("s-user-profile-fields", this.domItem);
			this.addItem(syra_dom.li("s-head-popup-li", ul), {
				$isEditMode: true,
				$inplace: true,
				$isReferenceTitleVisible: false,
				$skin: "s-user-profile-field",
				$bind: "selectedLocale",
				$title: syra_local.userProfile_selectedLocale
			});
			this.addItem(syra_dom.li("s-head-popup-li", ul), {
				$isEditMode: true,
				$inplace: true,
				...
				$popupRight: true,
				$bind: "selectedTheme",
				$skinListItem: "s-head-popup-btn-default",
				$title: syra_local.userProfile_selectedTheme
			});
			this.addItem(syra_dom.li("s-head-popup-li", ul), {
				$isEditMode: true,
				$inplace: true,
				...
				$format: "$combo",
				$skinListItem: "s-head-popup-btn-default",
				$isHidden: !(this.dataset.themes && this.dataset.themes.length > 0),
				$title: syra_local.userProfile_choiceTheme
			});
	...
	onLogonSucees: function() {
		syra_context.enableUILog = this.dataset.enableUILog;
		syra_context.developpementMode = this.dataset.developpementMode;
		this._applyNewSecurity();
		syra_culture.format.initialize(this.dataset.selectedLocale);
	},
	...
	addOpener: function() {
		...
		self.openerBtn = syra_button.add({
			parent: syra_site,
			slot: syra_site.headerBar,
			iconOnly: true,
			text: syra_local.site_btn_opener,
			css: "s-header-link s-header-profile",
			click: function() {
				self.toggle(true);
			}
		});

		var roleItem = self.addItem(syra_site.headerBar, {
			$isEditMode: true,
			...
			$isReferenceTitleVisible: false,
			$skin: "s-security-user-field",
			$bind: "selectedRole"
		});
		roleItem.input.removeAttribute("contenteditable");
		...
		var endpointItem = self.addItem(syra_site.headerBar, {
			$isEditMode: true,
			$contentEditable: true,
			$isReadOnly: true,
			$coreId: true,
		...
		endpointItem.input.removeAttribute("contenteditable");
	...
	resizeItem: function() {
		if (this._popup) {
			if (this.technicalSlot) {
				this.technicalSlot.style.maxHeight = syra_site.size.popupMaxHeight + "px";
			}
			this._popup.resize();
		}
	},
	toggle: function(show, event) {
		...
		if (self.openerBtn) {
			if (show && !self._popup) {
				if (!self.updatseProfileBtn) {
					self.updatseProfileBtn = syra_button.add({
						parent: self,
						slot: self.domItem,
						text: syra_local.site_btn_profile,
						css: "s-head-popup-btn-primary",
						click: function() {
				...
				self.loadFields();
				if (!self.technicalSlot) {
					self.technicalSlot = syra_dom.ul("s-head-popup-ul", self.domItem);
					...
					for (var ii = 0, jj = syra_config.$tecnhicalLinks.length; ii < jj; ii++) {
						...
						syra_site.addItem(syra_dom.li("s-head-popup-li", self.technicalSlot), {
							$category: "link",
							$skin: "s-head-popup-btn-default",
							$bind: $link.$bind
						}, syra_site);
					}
				}
				if (!self.logoutBtn) {
					self.logoutBtn = syra_button.add({
						parent: self,
						slot: self.domItem,
				...
				syra_dom.hide(self.technicalSlot, syra_fusion && syra_fusion.activatedBook);
				syra_quality.page_onAfterLoad(self);
				self.resizeItem();
				syra_dom.toggleClass(this.openerBtn.link, "s-open", true);
			} else {
				if (self._popup) {
					self._popup.close();
					syra_dom.toggleClass(this.openerBtn.link, "s-open", false);
				}
			}
		}
	},
			...
			if (this.dataset.user) {
				...
				if (this.dataset.user.photo) {
					this.addItem(syra_dom.div(), {
						imageArea: syra_dom.span("s-header-user-photo", this.openerBtn.link),
						$skin: "s-header-user-photo",
						$inplace: true,
						$imageWidth: " ",
						$imageHeight: " ",
						$bind: "photo",
				...
				syra_dom.span("s-header-user-name", this.openerBtn.link).textContent = (this.dataset.user.firstName ? this.dataset.user.firstName + " " : "") + (this.dataset.user.lastName ? this.dataset.user.lastName : "");
			}
