...
function _changeMainPage($url, $itemPage) {
	if (!window.history.pushState) {
	...
	document.controller.changeMainPage(exports.getSdataUrl($url));
};

exports.load = function($url, target, $itemPage) {
	var param = _setUrlParameter($url);
	if (target == "blank") {
...
exports.setHref = function(menu, record, isExternal) {
	...
	menu.mn.setAttribute("href", menu.href = _setUrlParameter(menu.$url));
	if (isExternal || (menu.$url.indexOf("http") >= 0 && menu.$url.indexOf(window.location.host) < 0)) {
		menu.mn.setAttribute("target", menu.$target = "blank");
	}
};
