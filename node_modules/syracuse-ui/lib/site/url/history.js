	...
	window[window.history.pushState ? "onpopstate" : "onhashchange"] = function(event) {
		if (_updateHitory) {
			...
			if (syra_site.mainPage) {
				if (syra_site.lastHref === window.location.href && syra_site.forceback) {
					// call back again we have already gone back to that page using back or close button
					syra_site.mainPage.setHistoryBack(false);
					syra_site.forceback = false;
					exports.forceBack();
				} else {
