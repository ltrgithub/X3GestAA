...
exports.replaceEndpoint = function($url, newEndpoint, check) {
	if (newEndpoint) {
		...
		if (segments.endpoint) {
			exports.replaceSegmentEndpoint(segments, newEndpoint, check);
			$url = segments.$url;
		}
	}
	return $url;
...
exports.compareSegment = function(seg1, seg2) {
	if (seg1.uri == seg2.uri) {
		if (!seg1.params && !seg2.params) {
			return true;
		}
		if (seg1.params && seg2.params) {
			var keys1 = Object.keys(seg1.params);
			var keys2 = Object.keys(seg2.params);
			if (keys1.length == keys2.length) {
				for (var ii = 0, jj = keys1.length; ii < jj; ii++) {
					if (seg1.params[keys1[ii]] != seg2.params[keys1[ii]]) {
						return false;
					}
				}
				return true;
		...
		switch (prop) {
			...
			default:
				...
				if (expression && typeof(expression) == "string") {
					expression = document.site.expressionMaker.parse(article, expression, record);
				}
		...
		_formatParameters(article, $menu.$parameters, record, function(values, isCanceled) {
			if (isCanceled) {
				...
				if ($menu.$sourceType && $menu.$sourceType.indexOf("{") == 0) {
					$menu.$type = document.site.expressionMaker.parse(article, $menu.$sourceType, record, values);
				...
				callback(document.site.expressionMaker.parse(article, $menu.$sourceUrl || $menu.$url, record, values));
			}
		});
	} else {
		callback(document.site.expressionMaker.parse(article, $menu.$sourceUrl || $menu.$url, record, undefined, $menu.$subRecordKey, undefined, true));
	}
}
...
exports.formatMenuUrl = function(article, $menu, record, callback) {
	if (callback) {
		...
		return document.site.expressionMaker.parse(article, $menu.$sourceUrl || $menu.$url, record, properties, $menu.$subRecordKey, undefined, true);
	}
};
...
exports.buildPageCollaborationUrl = function(article, urlSegment) {
	return document.site.expressionMaker.parse(article, document.site.getPageTemplateUrl(urlSegment.$url), {
