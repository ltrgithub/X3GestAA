	...
	if ($menu.$parameters) {
		...
		_formatParameters(article, $menu.$parameters, record, function(values, isCanceled) {
			if (isCanceled) {
				...
				callback(document.site.expressionMaker.parseUrl(article, $menu.$sourceUrl || $menu.$url, record, values));
			}
		});
	} else {
		callback(document.site.expressionMaker.parseUrl(article, $menu.$sourceUrl || $menu.$url, record, undefined, $menu.$subRecordKey, undefined, true));
	}
}
...
exports.formatMenuUrl = function(article, $menu, record, callback) {
	if (callback) {
		...
		return document.site.expressionMaker.parseUrl(article, $menu.$sourceUrl || $menu.$url, record, properties, $menu.$subRecordKey, undefined, true);
	}
};
