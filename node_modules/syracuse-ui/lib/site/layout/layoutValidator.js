    ...
    for (var ii = 0, jj = layout.items.length; ii < jj; ii++) {
        ...
        if (ii > 0) {
            ...
            if (layout.isDesigned) {
                ...
                document.site.ddManager.setDragSpot(prevItem.rowSeparator, layout.isRow);
                if (prevItem.isLayout) {
                    prevItem.rowSeparator.syraDesignedLayoutId = prevItem.id;
                }
                else {
                    prevItem.rowSeparator.syraDesignedItemId = prevItem.id;
                }
            }
    ...
    tabs: function(layout){
        ...
        if (layout.items.length > 0) {
            ...
            for (var ii = 0, jj = layout.items.length; ii < jj; ii++) {
                var item = layout.items[ii];
                if (item.$opened) {
                    if (!item.$item.$opened || (item.isInvisible && !layout.isDesigned)) {
                        item.openBox(false);
                    }
                }
                if (item.$item.$opened) {
                    if (!item.isLayout && !item.isInvisible) {
                        tabOpened = item;
                    }
                    else {
