...
function _deleteRowColumn(layout, deletedIndex) {
	...
	if (layout.items.length == 1) {
		...
		syra_site.layoutValidator.validate(updated.layoutParent, true);
	}
}

exports.onDeleteLayoutClick = function(designer, event, btn) {
	if (!syra_dd.ddAgent) {
		syra_dd.cancel();
...
exports.cleanEmptyTabs = function($item) {
	if ($item) {
		delete $item.$tabIndex;
		if ($item.$layout) {
			exports.cleanEmptyTabs($item.$layout);
		}
		if ($item.$items) {
