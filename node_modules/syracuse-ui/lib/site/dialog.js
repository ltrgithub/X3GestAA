...
exports.Dialog = helpers.defineClass(Dialog, null, {
    open: function(options){
        ...
        if ((this.options = options).content) {
            ...
            this.$$dialogSlot = this.options.$$dialog || this.options.content.$$item;
            this.dialogSlot = this.$$dialogSlot[0];
    ...
    _openDialogPage: function($itemPage){
        ...
        if ($itemPage || self.options.page) {
            ...
            /*if (self._content.$facet != "$lookup") {
             self.dialogSlot.style.width = document.site.mainPage.scrollview.offsetWidth   + "px";
             }
             if (document.site.mainPage.fusionBar) {
             self.dialogSlot.style.left = document.site.mainPage.fusionBar._slot.offsetWidth + "px";
             self._xMarginPercent = 0.1;
             }
             else {
             self._initWidth = true;
        ...
        if (!this.disposed) {
            ...
            if (this.options.position) {
                if (this.options.position.left) {
                    this.dialogSlot.style.left = this.options.position.left;
                    this.dialogSlot.style.top = this.options.position.top;
                }
                else {
                ...
                var moveDiag = true;
                var diagWidth = this.$$dialogSlot.outerWidth(true);
                var diagHeight = this.$$dialogSlot.outerHeight(true);
                if (!resize) {
                    var offset = this.$$dialogSlot.offset();
                    if (this.draggedPosition || document.site.isResizing) {
                        moveDiag = (((offset.top + diagHeight) >= this._bodyHeight) || ((offset.left + diagWidth) >= this._bodyWidth));
                    }
                    if (moveDiag) {
                        delete this.draggedPosition;
                        if (diagWidth > this._maxWidth) {
                            this.dialogSlot.style.width = (diagWidth = this._maxWidth) + "px";
                        }
                        if (diagHeight > this._maxHeight) {
                            this.dialogSlot.style.height = (diagHeight = this._maxHeight) + "px";
                        }
                        if (this.options.$dialogSize == "content") {
                            this.dialogSlot.style.left = ((this._bodyWidth - diagWidth) / 2) + "px";
                            this.dialogSlot.style.top = ((this._bodyHeight - diagHeight) / 2) + "px";
                        }
                        else {
                            if (this._initWidth || ((offset.left + diagWidth) >= this._bodyWidth)) {
                                this.dialogSlot.style.left = ((this._bodyWidth - diagWidth) / 2) + "px";
                            }
