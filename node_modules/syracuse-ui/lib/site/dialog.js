						...
						if (scroller && scroller.bodySlot) {
							if (this.widgetResizing || document.site.isResizing) {
								...
								if (dialogRect.bottom > areaRect.bottom) {
									this.dialogSlot.style.height = (dialogRect.height - (dialogRect.bottom - areaRect.bottom)) + "px";
								}
							}
		...
		if (this._content) {
			...
			if (!this.disposed && document.site.$$body && document.site._$$header) {
				...
				if (this.options.position) {
					...
					this._maxHeight = Math.floor(this._maxHeight * 0.5);
								...
								if (!this.widgetResizing) {
									...
									if (space) {
										if (!document.site.isResizing) {
											this.dialogSlot.style.height = Math.min(this.dialogSlot.clientHeight - space, this._maxHeight) + "px";
										} else {
											if (space > 0) {
