...
exports.Dialog = helpers.defineClass(Dialog, null, {
	...
	_openContent: function() {
		var self = this;
		self.$isPageDocked = self.options.$isPageDocked;
		self.$isWidthDocked = self.options.$isWidthDocked;
	...
	_openDialogPage: function($itemPage) {
		...
		if ($itemPage || self.options.page) {
			...
			if (self._content.$isPageDocked || (self._content.$item && self._content.$item.$isPageDocked)) {
				self.$isPageDocked = true;
			}
			if (self._content.$isWidthDocked ||
				(self._content.$item &&
					(self._content.$item.$isWidthDocked ||
						(self._content.$item.$layout && self._content.$item.$layout.$isWidthDocked)))) {
				self.$isWidthDocked = true;
			}
			...
			self._endOpen();
			if (!self.$isPageDocked && !self.$isWidthDocked) {
				self._widgetResizer = document.site.setWidgetResizable({
					source: self,
					slot: self.dialogSlot,
	...
	onWindowResize: function(resize, onScroll) {
		if (this._content) {
			...
			if (!this.disposed && document.site.$$body && document.site._$$header) {
				...
				this._maxWidth = this.bodyWidth = $$area.width();
				this._xMargin = this._maxWidth * 0.05;
				this._maxWidth = this._maxWidth - (this._xMargin * 2);
				//this._yMargin = document.site._$$header.outerHeight(true);
				this._yMargin = $$area.offset().top;
				this._maxHeight = (this.bodyHeight = $$area.height()) - this._yMargin;
				this._diagWidth = this.$$dialogSlot.outerWidth(true);
				this._diagHeight = this.$$dialogSlot.outerHeight(true);
				if (this.$isPageDocked) {
					this.dialogSlot.style.top = this._yMargin + "px";
					this.dialogSlot.style.left = this._xMargin + "px";
					this.dialogSlot.style.width = this._maxWidth + "px";
					...
					if (this.options.position) {
						...
						if (!resize) {
							...
							if (this.draggedPosition || document.site.isResizing) {
								moveDiag = (((offset.top + this._diagHeight) >= this.bodyHeight) || ((offset.left + this._diagWidth) >= this.bodyWidth));
							}
							if (moveDiag) {
								...
								if (this.options.$dialogSize == "content") {
									this.dialogSlot.style.left = ((this.bodyWidth - this._diagWidth) / 2) + "px";
									this.dialogSlot.style.top = ((this.bodyHeight - this._diagHeight) / 2) + "px";
								} else {
									if (this._initWidth || ((offset.left + this._diagWidth) >= this.bodyWidth)) {
										this.dialogSlot.style.left = ((this.bodyWidth - this._diagWidth) / 2) + "px";
									}
									this.dialogSlot.style.top = this._yMargin + "px";
								}
