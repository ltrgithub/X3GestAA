	...
	_bindEvents: function(bind) {
		...
		if (bind) {
			$(document).bind("keypress" + namespace, function(event) {
				...
				if (event.keyCode == keyHelper.keyCode.ESCAPE && self.keydEsc) {
					self.close();
					return false;
				}
			});
	...
	_openDialogPage: function($itemPage) {
		...
		if ($itemPage || self.options.page) {
			...
			if (self.options.$itemPage) {
				...
				if (self.options.$method && (self.options.$method != "GET")) document.controller.postQuery(self.options, null, self.options.article, function($location, data) {
