...
exports.Dialog = helpers.defineClass(Dialog, null, {
    open: function(options){
        ...
        if ((this.options = options).content) {
            ...
            this.$$container = this.options.$$dialog || this.options.content.$$item;
            var dialog = this.$$container[0];
    ...
    _appendAutoClose: function(append){
        ...
        if ((self.options.$autoClose === undefined) ? true : self.options.$autoClose) {
            if (append) {
                self._autoCloseNamespace = self.$$container.attr("id") || self.id;
                self.$$container.attr("id", self._autoCloseNamespace);
    ...
    _endOpen: function(){
        this.$$container.appendTo(document.site.$$container);
        ...
        document.site.setZIndex(this.$$container[0]);
        this.onWindowResize();
    ...
    _openDialogPage: function($itemPage){
        ...
        if ($itemPage || self.options.page) {
            if (self.options.page) {
                ...
                self.$$container = self.options.page.$$container || $(document.createElement("div"));
                self._ensureMode();
            }
            else {
                self.$$container = $itemPage.$$container = $(document.createElement("div"));
                ...
                self._ensureMode();
                self.$$container[0].className = self.$skin + "-page-container";
    ...
    _moveToPositionNew: function(onBoxToggle){
        if (this.options.position) {
            ...
            if (this.options.$dialogSize == "content") {
                ...
                left = (body.clientWidth - this.$$container.outerWidth()) / 2;
                top = (body.clientHeight - this.$$container.outerHeight()) / 2;
            }
            else {
                ...
                width = Math.max(200, width - (margin.left * 2));
                style.width = width + "px";
                var height = body.clientHeight - (margin.top * 2);
                height = Math.max(200, height);
                // style.height = height + "px";
                
                var contentBody = this._content.$$body[0];
                var root = this.$$container[0];
                var top = 0;
                var start = contentBody;
                ...
                if (height > (contentBody.scrollHeight + 20)) {
                    style.height = (contentBody.scrollHeight + top + 20) + "px";
                    contentBody.style.height = contentBody.scrollHeight + "px";
                }
                else {
                    contentBody.style.height = height + "px";
                }
            }
            style.left = left + "px";
            style.top = top + "px";
    ...
    _resizeDialogPage: function(){
        ...
        body.style.height = (this.$$container.height() - top) + "px";
