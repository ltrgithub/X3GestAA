...
exports.unFormatValue = function(operator, value) {
	var op = _operators[operator];
	return op && op.unFormat ? op.unFormat(value) : value;
};

function _findExpression(exp, $bind) {
	var found;
	for (var ii = 0, jj = exp.children.length; !found && ii < jj; ii++) {
		var child = exp.children[ii];
		if (child.type == "identifier" && child.value == $bind) {
			found = exp;
		} else {
			if (child.children) {
				found = _findExpression(child, $bind);
			}
		}
	}
	return found;
...
function _findExpressionOld(exp, $bind) {
	var found;
	for (var ii = 0, jj = exp.children.length; !found && ii < jj; ii++) {
		var child = exp.children[ii];
		if (child.children) {
			if (child.children[0].type == "identifier" && child.children[0].value == $bind) {
				found = child;
			} else {
				found = _findExpression(child, $bind);
			}
...
function _ensureFilterField(options) {
...
exports.parseWhere = function(where, $bind) {
	var exp = Parser.parse(where);
	if (exp && $bind) {
		exp = _findExpression(exp, $bind);
	}
	return exp;
};
...
exports.getFieldValue = function(options) {
	if (options.field.page.isFusionPage) {
		return document.site.fusionGateway.filterMaker.getFieldValue(options);
	}
	...
	if (options.value != null) {
		...
		if (options.field && options.field.$field.$type == "application/x-reference") {
			_ensureFilterField(options);
		}
...
exports.getFieldOperators = function($type, field, $filterField) {
	if (field.page.isFusionPage) {
		return document.site.fusionGateway.filterMaker.getFieldOperators($type, field, $filterField);
	}
	switch ($type || field.$field.$type) {
		...
		case "application/x-datetime":
			var operators = ["none", "gt", "ge", "lt", "le", "eq", "ne", "empty", "notempty"];
			if (!field.$field.$type == "application/x-reference") {
				operators.push("between");
			}
			return operators;
		case "application/x-string":
			var operators = ["none", "gt", "ge", "lt", "le", "eq", "ne", "like_s", "like", "empty", "notempty"];
			if (!field.$field.$type == "application/x-reference") {
				operators.push("between");
			}
			return operators;
