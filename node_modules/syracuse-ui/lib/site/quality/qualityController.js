...
function _setPickers(item) {
	if (item.mnPickers) {
		var ids = Object.keys(item.mnPickers);
		for (var ii = 0, jj = ids.length; ii < jj; ii++) {
			var picker = item.mnPickers[ids[ii]];
...
var _field = {
	_setBind: function(field, target) {
	...
	_setValue: function(field, target) {
		if (target) {
			field.getDataValue && target.setAttribute("data-s-value", field.getDataValue());
	...
	setInfo: function(field) {
		if (field.domItem) {
			...
			this._setBind(field, field.domItem);
			...
			if (field.$isEditMode) {
				if (field.input) {
					this._setBind(field, field.input);
				}
			} else {
				if (field._dataValue) {
					this._setBind(field, field._dataValue);
				}
			}
			_setPickers(field);
			if (field.builder) {
	...
	_setChoice: function(field) {
		if (field.builder._choices) {
			...
			switch (field.$item.$format) {
				case "$combo":
					if (field.builder._list) {
						field.builder._list.setAttribute("data-s-choice-popup", field.id);
						for (var ii = 0, jj = field.builder._choices.length; ii < jj; ii++) {
							var choice = field.builder._choices[ii];
							this._setChoiceItem(choice.link, field.$enum[choice.$index || ii]);
						}
					}
					break;
				case "$button":
				...
				case "$radios":
					for (var ii = 0, jj = field.builder._choices.length; ii < jj; ii++) {
						this._setChoiceItem(field.builder._choices[ii].checkBox.input, field.$enum[ii]);
					}
					break;
			}
...
var _menus = {
	setInfo: function(menuItem) {
		if (menuItem) {
			...
			menuItem.domItem.setAttribute("data-s-menu-title", menuItem.$title);
		}
	},
	openPopupMenu: function(menusBox) {
		menusBox.body.setAttribute("data-s-menu-popup", menusBox.contextField.id);
		var ids = Object.keys(menusBox.childMenus);
		for (var ii = 0, jj = ids.length; ii < jj; ii++) {
			_menus.setInfo(menusBox.childMenus[ids[ii]]);
		}
	}
};
