...
var LandingVignetteSearchFacets = require('syracuse-ui/lib/page/landing/search/landingVignetteSearchFacets').LandingVignetteSearchFacets;
...
var vignetteFormats = {
	"$navigation": require('syracuse-ui/lib/page/landing/vignette/field/navigationVignetteField').NavigationVignetteField,
	"$html": require('syracuse-ui/lib/page/landing/vignette/field/htmlVignetteField').HTMLVignetteField,
	"$webi": require('syracuse-ui/lib/page/landing/vignette/field/boWebIVignetteField').BOWebIVignetteField,
	"$page": require('syracuse-ui/lib/page/landing/vignette/field/pageVignetteField').PageVignetteField,
	"$searchPage": require('syracuse-ui/lib/page/landing/vignette/field/searchPageVignetteField').SearchPageVignetteField,
	"$item": require('syracuse-ui/lib/page/landing/vignette/field/itemVignetteField').ItemVignetteField
};
...
var _fieldTypes = {
	...
	"application/x-quantity": require('syracuse-ui/lib/field/quantityField').QuantityField,
	...
	"application/wid": require('syracuse-ui/lib/page/landing/vignette/field/boWebIVignetteField').BOWebIVignetteField,
};
...
var landingSearchVignettePage = require('syracuse-ui/lib/page/landing/search/landingSearchVignettePage').LandingSearchVignettePage;
var searchVignettePage = require('syracuse-ui/lib/site/search/searchVignettePage').SearchVignettePage;
exports.pageCategories = {
	...
	dashboard: require('syracuse-ui/lib/page/dashboardPage').DashboardPage,
	portlet: vignettePage,
	vignette: vignettePage,
	searchVignette: searchVignettePage,
	landingSearchVignette: landingSearchVignettePage,
	soap: require('syracuse-ui/lib/soap/soapPage').SoapPage,
	fusion: require('syracuse-ui/lib/fusion/article/sheetPage').SheetPage,
	search: require('syracuse-ui/lib/site/search/searchPage').SearchPage,
	landingVignetteSelect: require('syracuse-ui/lib/page/landing/search/landingVignetteSelect').LandingVignetteSelect
};

var arrayTypes = {
	single: require("syracuse-ui/lib/field/array/singleBuilder/singleList").SingleList,
	grid: require("syracuse-ui/lib/field/array/gridBuilder/gridList").GridList,
	cards: require("syracuse-ui/lib/field/array/cardsBuilder/cardList").CardList,
};
...
exports.categories = {
	...
	landingVignetteSearchFacets: {
		create: function() {
			return new LandingVignetteSearchFacets();
		}
	},
	section: {
		create: function($item) {
	...
	field: {
		create: function($item, boxParent) {
			...
			if (article && article.reloadingFields) {
				field = article.reloadField($item, boxParent);
				if (field) {
					return field;
				}
			}
			...
			if ($field) {
				/*if ($field.$type == "application/x-pointer") {
                 if (article.page.$prototypeRepository && $field.$prototype) {
                 var index = $field.$prototype.indexOf("#");
                 if (index >= 0) {
                 $field = article.page.$prototypeRepository[$field.$prototype.slice(index + 1)];
                 >>> V7.0-patch2
                 if ($field) {
                 $field = article.$prototype.$properties[$item.$bind] = {
                 $type: "application/x-object",
                 $item: helpers.object.clone($field, true)
                 };
                 }
                 }
                 }
				...
				switch ($class) {
					case "application/x-array":
						if (!$field.$item.$properties || $field.$item.$type == "application/x-reference") {
							fieldClass = arrayTypes.single;
						} else {
							if ($item.$format != "cards") {
								$item.$format = "grid";
							}
							fieldClass = arrayTypes[$item.$format];
						}
						break;
				...
				if (field) {
					...
					field.$field = $field;
					field.localize = document.site.localize;
				}
			} else {
				field = new UnknowField();
