	...
	field: {
		create: function($item, boxParent) {
			...
			if (article && article.reloadingFields) {
				if ($item.$bind && article.reloadingFields[$item.$bind]) {
					field = article.reloadingFields[$item.$bind];
					field.isReloading = true;
					delete article.reloadingFields[$item.$bind];
				}
				if (field) {
					if (field.isList && field.$item.$format !== undefined && $item.$format != field.$item.$format) {
						article.removeItem(field, true, true);
						field = null;
					}
					if (field) {
						return field;
