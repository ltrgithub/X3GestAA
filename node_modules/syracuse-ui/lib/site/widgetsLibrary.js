    ...
    tabularChart: {
        ...
        create: function($item, boxParent) {
            var article = boxParent.getArticle(),
                ...
                $field = $item.$field || proto.$properties[$item.$bind],
                renderer = ($field.$cube && "flash") || $item.$renderer,
                // renderer = (proto.$cube && "highCharts") || $item.$renderer,
                $extensionType = ($field.$cube && "application/x-cube") || ($field.$extension && $field.$extension.$type);

            if ($extensionType) {
                var match = /application\/x-(\w+)/.exec($extensionType),
                    decorator = match && match[1],
                    decocatorClass = this.decocators[decorator + (renderer ? "$" + renderer : "")];
