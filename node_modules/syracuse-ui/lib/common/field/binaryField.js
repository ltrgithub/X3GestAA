...
exports.BinaryField = helpers.defineClass(BinaryField, Field, {
    ...
    render: function(){
        if (!this.$item.$filterMode) {
            var self = this;
            self.$$anchor = $("<a target='_blank'/>").appendTo(self.$$value);
            ...
            if (self.$isEditMode) {
                ...
                self.$$select = $("<a/>").addClass(this.$skin + "-img-select").text(_localize.selectFile).appendTo(self.$$value).bind("click", function(){
                    self.$$file.click();
                    return false;
                });
                self.$$remove = $("<a/>").addClass(this.$skin + "-img-remove").text(_localize.removeFile).appendTo(self.$$value).bind("click", function(){
                ...
                self.$$file.appendTo(self.$$value).bind("change", function(){
                    self._drawFiles(this.files);
                });
                self.$$fileName = $("<div style='display:none'/>").appendTo(self.$$value);
...
exports.ImageField = helpers.defineClass(ImageField, BinaryField, {
    _onUploadSuccess: function(data, response){
        var self = this;
        // called on succes only
        var imageUrl = self._getImageUrl();
        self.$$image.css("background-image", "url(\"" + imageUrl + "\")");
        self.$$anchor.attr("href", imageUrl);
    },
    ...
    _getImageUrl: function(){
        ...
        return (dataUrl && this._saltUrl(this.getArticle().parseExpression(dataUrl)));
    },
