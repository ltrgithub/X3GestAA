...
function _appendLookUp(field){
    ...
    return field.$$pickerBtn = $(btn).bind("click", function(){
        document.editFieldController.clickLookUp(field);
    });
}
...
exports.Field = helpers.defineClass(Field, null, {
    ...
    loadBox: function($$item){
        ...
        this.setMenus(this.$item.$help || this.$field.$help);
    ...
    _addValueBtns: function($$btn){
        if (!this.$$valueBtns) {
            var div = document.createElement("a");
            div.className = this._$cssField + "-value-btns";
            this.$$valueBtns = $(div).appendTo(this.$$dataValue);
        }
        this.$$valueBtns.append($$btn);
        this.$$dataValue[0].style.paddingRight = this.$$valueBtns.width() + "px";
    },
    setMenus: function($menus){
        var self = this;
        self.$menus = $menus;
        if (self.$$title && self.titleText.length > 0) {
            if (self.$menus) {
                if (!self.$$menusBtn) {
                    var div = document.createElement("a");
                    div.className = "s-field-menus-btn";
                    self._addValueBtns(self.$$menusBtn = $(div));
                    self.$$menusBtn.bind("click", function(){
                        if (!self.popupMore) {
                            self.$$menusBody = $("<div/>").addClass("s-field-menus-body").appendTo(self.$$item);
                            document.site.toggleClass(self.$$menusBtn, "s-close", false);
                            self.popupMore = document.site.createPopup({
                                page: self.getPage(),
                                $$popup: self.$$menusBody,
                                position: {
                                    my: "left top",
                                    at: "left bottom",
                                    of: self.$$menusBtn
                ...
                if (self.$$menusBtn) {
                    this.$$menusBtn.unbind().remove();
                    delete self.$$menusBtn;
                }
            }
        }
    },
            ...
            if (this.$help) {
                if (!this.$$helpFlag) {
                    /* var div = document.createElement("div");
                     ...
                     this.$$title.append(this.$$helpFlag = $(div));*/
                    var div = document.createElement("a");
                    div.className = "s-field-help-flag";
                    this._addValueBtns(this.$$helpFlag = $(div));
                }
            }
            else {
                if (this.$$helpFlag) {
    ...
    getDataValue: function(){
        return document.editFieldController.getDataValue(this);
    },
    applyMetaData: function(metaData){
        if (metaData) {
            if (metaData.$isEditMode !== undefined) {
                document.editFieldController.toggleEditMode(this, metaData.$isEditMode);
            }
            ...
            if (metaData.$help !== undefined) {
                this.setMenus(metaData.$help);
            }
            this.setState(metaData);
            if (metaData.$diagnoses !== undefined) {
                if (!this.$item.$isDiagnoseDisabled) {
    ...
    onInputValidate: function(event){
        document.editFieldController.onInputValidate(this, event);
    },
    onInputChange: function($$input, event){
        document.editFieldController.onInputChange(this, $$input, event);
    },
    onInputKeyup: function($$input, event){
        document.editFieldController.onInputKeyup(this, $$input, event);
    },
    dispose: function(){
