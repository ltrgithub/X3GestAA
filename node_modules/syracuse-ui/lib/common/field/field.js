...
exports.Field = helpers.defineClass(Field, null, {
    applyDesignMetaData: function(metadata, onAuthoring){
        ...
        if (onAuthoring !== false) {
            if (metadata.$isTopLabelAlignment !== undefined) {
                ...
                this.$$dataValue[0].className = this.$$dataValue[0].className.replace(curItemCss + "-value", newItemCss + "-value");
            }
        }
        if (this.$$title) {
    ...
    _createValueContainer: function(){
        var css = (this.$item.$css) ? (this.$item.$css + " " + this.$skinField) : this.$skinField;
        var topCss = (this.$item.$isTopLabelAlignment) ? css + "-top" : css;
        ...
        if (this.$item.$inplace) {
            ...
            var div = document.createElement("div");
            div.className = topCss + "-value-slot";
            this.$$valueSlot = $(div).append(this.$$dataValue);
            
            div = document.createElement("div");
            div.className = topCss + "-core";
    ...
    loadBox: function(){
        ...
        var itemCss = (this.$item.$css) ? (this.$item.$css + " " + this.$skinField) : this.$skinField;
        if (this.$item.$isTopLabelAlignment) {
            itemCss += "-top"
        }
        
        this.$$item[0].className += " " + itemCss;
        if (this.$isEditMode) {
            this.bindEvents();
        }
        if (this.initialize) {
        ...
        if (this.$$title && this.titleText.length > 0) {
            if (this.$help) {
                if (!this.$$helpFlag) {
                    ...
                    var div = document.createElement("div");
                    div.className = "s-field-help-flag";
                    this.$$valueSlot.append(this.$$helpFlag2 = $(div));
                }
            }
            else {
                if (this.$$helpFlag) {
    ...
    setDataValue: function(value, record){
        ...
        if (this.$isEditMode) {
            ...
            this.$$dataValue.empty().text(value);
        }
    },
