...
exports.Field = helpers.defineClass(Field, null, {
    applyDesignMetaData: function(metadata, onAuthoring){
        ...
        if (metadata.$width) {
            if (metadata.$width == "content") {
                var $width = document.site.getFieldWidth(this.$field.$type, this.$field.$format);
                if ($width.$charWidth) {
                    this.domValueSlot.style.maxWidth = ($width.$charWidth * ((this.$field.$constraints ? this.$field.$constraints.$size : null) || $width.$size)) + $width.$unit;
                }
                else {
                    this.domValueSlot.style.maxWidth = $width.$width;
                }
            }
            else {
    ...
    _createValueContainer: function(){
        ...
        if (this.$isEditMode) {
            ...
            this.$$textValue = $(document.createElement("div")).appendTo(this.$$dataValue);
        }
    },
    _initCss: function(){
    ...
    loadBox: function($$item){
        ...
        if (document.site.developpementMode) {
            var dataValue = this.$$dataValue[0];
            dataValue.setAttribute("data-s-q-bind", this.$item.$bind);
            dataValue.setAttribute("data-s-q-title", this.titleText);
            if (this.$$input) {
                this.$$input[0].setAttribute("data-s-q-bind", this.$item.$bind);
                this.$$input[0].setAttribute("data-s-q-title", this.titleText);
            }
        }
    },
    ...
    render: function(){
        if (this.$isEditMode) {
            ...
            if (this.$field.$links && (this.$field.$links.$lookup || this.$field.$links.$tunnel)) {
                ...
                this.$$inputValue[0].appendChild(input);
            }
    ...
    _appendPicker: function(pickerType, increment){
        ...
        if (!this._boxPicker) {
            ...
            this.$$inputValue[0].appendChild(this._boxPicker);
        }
                ...
                if (this.$$title && this.titleText.length > 0) {
                    ...
                    if (state.$isMandatory) {
                        if (!this._mandatoryFlag) {
                            ...
                            if (this.$$input) {
                                this.$$input[0].setAttribute("required", "required");
                            }
                        }
                    }
                    else {
    ...
    setDataValue: function(value, record){
        ...
        if (this.$isEditMode) {
            ...
            if (value && this.$field.$links && this.$field.$links.$details && !this.$item.$isMenusDisabled) {
                this.appendDetailLink(this.$$textValue, value, this.$field.$links.$details);
            }
            else {
