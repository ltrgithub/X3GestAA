...
exports.RawListBuilder = helpers.defineClass(RawListBuilder, null, {
    ...
    onReorder: function(sourceRecord, targetRecord, $insert){
        var newRecords = [];
        this.list._records.splice(sourceRecord.$recordIndex, 1);
        this.list._records.forEach(function(record){
        ...
        this.list._records = newRecords;
        this._notifyReorder(sourceRecord, targetRecord, $insert);
    },
    _notifyReorder: function(sourceRecord, targetRecord, $insert){
        sourceRecord.$$item[$insert](targetRecord.$$item);
        if (sourceRecord._menuRow) {
            $(sourceRecord._menuRow).insertAfter(sourceRecord.$$item);
        }
        if (targetRecord._menuRow) {
            $(targetRecord._menuRow).insertAfter(targetRecord.$$item);
        }
        /* var $resources = (this.list._records = newRecords).map(function(record){
