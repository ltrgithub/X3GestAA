...
exports.RawList = helpers.defineClass(RawList, Article, {
    ...
    drawBox: function(){
        ...
        div.className = this._makeSkinField(null, true);
    ...
    setDescription: function($description){
        ...
        if (this.descriptionText.length > 0) {
            ...
            if (!this.$item.$inplace) {
                if (!this.$$description) {
                    var div = document.createElement("div");
                    div.className = this._makeSkinField("-desc", true);
    ...
    _createValueContainer: function(){
        var div = document.createElement("div");
        div.className = this._makeSkinField("-value", true);
        this.$$value = $(div);
        if (this.$item.$inplace) {
            ...
            var div = document.createElement("div");
            div.className = this._makeSkinField("-core", true);
    ...
    appendTitle: function(){
        if (!this.$item.$inplace && !this.$item.$isTitleHidden) {
            ...
            label.className = this._makeSkinField("-title", true) + " s-field-title-complex";
    ...
    _makeSkinField: function(suffix, addCss){
        var css = this.$item.$inplace ? "s-inplace" : "s-field";
        if (suffix) {
            css += suffix;
        }
    ...
    applyDesignMetaData: function(metaData, onAuthoring){
        ...
        if (onAuthoring !== false) {
            if (metaData.$isTopLabelAlignment !== undefined) {
                ...
                this.$$item[0].className = this.$$item[0].className.replace(itemCss, this._makeSkinField());
                if (this.$$title) {
                    this.$$title[0].className = this.$$title[0].className.replace(titleCss, this._makeSkinField("-title"));
                }
                this.$$value[0].className = this.$$value[0].className.replace(valueCss, this._makeSkinField("-value"));
            }
        }
        if (this.$$title) {
            ...
            if (metaData.$isTitleHidden !== undefined) {
                this.$$title[0].style.display = (this.$item.$isTitleHidden = metaData.$isTitleHidden) ? "none" : "";
            }
        }
