...
exports.RawList = helpers.defineClass(RawList, Article, {
        ...
        if (metaData) {
            if (metaData.$capabilities !== undefined) {
                ...
                if (metaData.$capabilities) {
                    metaData.$capabilities.split(",").forEach(function($prop){
                        $capability[$prop] = true;
                    });
                }
                var articleParent = self.getArticleParent();
                if (articleParent.$facet == "$lookup" ||
                articleParent.$facet == "$select") {
                    delete self.$capability.reorder;
                    delete self.$capability.insert;
                    delete self.$capability["delete"];
                }
                if (self.$isEditMode && self._records) {
                    self._records.forEach(function(record, index){
                        self._appendCapabilities(record);
                    });
                }
            }
        }
    ...
    applyMetaData: function(metaData){
        ...
        if (metaData.$type === undefined) {
            ...
            this.applyCapabilities(metaData);
    ...
    _onSelectRecords: function(event){
        ...
        var uuidTarget = $$selector.hasClass("s-list-selector-all") ? null : self.findRecordUiid($$selector);
        var isSelected;
        if (self.recordSelector.$selectMode == "row") {
            isSelected = true;
        }
        else {
            isSelected = (self.recordSelector.$selectMode == "button" || $$selector.is(":checked"));
        }
        self.getPage().externalAdapter.onSelectRecordEvent({
            field: self,
            ...
            doEvent: function(){
                if (self.recordSelector.$selectMode == "row") {
                    if (self.selectedUiid) {
                        if (self.selectedUiid != uuidTarget) {
                            self.selectRecord(self.selectedUiid, false);
                        }
                    }
                    self.selectRecord(uuidTarget, true);
                }
                else {
                    if ($$selector.hasClass("s-list-selector-all")) {
                        self.selectedRecords = {};
                        Object.keys(self.dataset).forEach(function($uuid){
                            self.selectRecord($uuid, isSelected);
                        });
                    }
                    else {
                        ...
                        self.selectRecord(uuidTarget, isSelected);
                    }
                }
                var page = self.getPage();
            ...
            if (this.builder.setDataBind) {
                ...
                if ($resources !== undefined) {
                    ...
                    if (record.$isDelta) {
                        ...
                        $resources.forEach(function(record, $recordIndex){
                            ...
                            if (record.$isDeleted) {
                                ...
                                if (recordArticle) {
                                    ...
                                    self._records.splice(self.builder.appendRecord({
                                        record: record,
                        ...
                        if ($resources) {
                            $resources.forEach(function(record, $recordIndex){
                                self._records.push(self.builder.appendRecord({
                                    record: record,
                                    $recordIndex: $recordIndex
                                }));
                            });
