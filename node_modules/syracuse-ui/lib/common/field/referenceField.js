...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
    ...
    onInputChange: function($$input){
        var value = $$input.val();
        if (value == "") {
            document.editFieldController.notifyChange(this, null, true);
        }
        else {
            if (this.currentValue && this.currentValue.$value != value || !this.currentValue) {
                document.editFieldController.notifyChange(this, {
                    $value: value
                });
            }
        }
    ...
    render: function(){
        ...
        if (self.$isEditMode) {
            ...
            (self.$$input = $("<input type='text'/>"))[0].className = $skinInput + " " + self.$skinInput + "-ref-picker";
            
            var box = document.createElement("div");
            box.className = $skinInput + "-box";
            ...
            self.$$pickerBtn = $(btn).bind("click", function(){
                document.editFieldController.clickLookUp(self);
            }).appendTo($$box);
            self.$$dataValue.append($$box).append(self.$$refTitle);
        }
        else {
    ...
    setDataValue: function(value){
        ...
        if (value) {
            ...
            this.currentValue.$value = value[$valueField] || article.parseExpression(this.$field[$valueField], value) || "";
            var $keyField = this.$field.$key ? "$key" : "$value";
            this.currentValue.$key = value[$keyField] || article.parseExpression(this.$field[$keyField], value) || "";
        }
        else {
            this.currentValue = {
                $title: "",
