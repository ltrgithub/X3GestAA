...
exports.Article = helpers.defineClass(Article, Box, {
    loadBox: function(initData){
        ...
        this.boundFields = {};
        this.idMap = {};
        this.menusFacade = this.createMenusFacade();
        this.drawBox();
    ...
    createMenusFacade: function(){
        return new MenusFacade(this, this.dataset);
    },
    _bindFieldEvents: function(){
        ...
        if (self.$$item) {
            ...
            if (document.site.$isFieldEditOnflyEnabled || self.$item.$isFieldEditOnfly) {
                ...
                // TODO : to validate (CMI).
                self.$$item.on("click", "input[data-s-field],select[data-s-field]", null, function(event){
                    var $$target = $(event.target), field, result = true;
                    if ((field = self.idMap[$$target.attr("data-s-field")])) {
                        result = self.getPage().externalAdapter.onFieldEvent({
                            field: field,
                            event: event,
                            doEvent: function(){
                                if (field.onInputClick) {
                                    field.onInputClick($$target, event);
                                }
                            }
                        });
                    }
