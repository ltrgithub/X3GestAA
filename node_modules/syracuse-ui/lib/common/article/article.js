...
exports.Article = helpers.defineClass(Article, Box, {
    ...
    formatMenuUrl: function($menu, record, callback){
        if (callback) {
            if ($menu.$properties) {
                ...
                self.parseProperties($menu.$properties, record, function(values, isCanceled){
                    if (isCanceled) {
            ...
            var properties;
            if ($menu.$properties) {
                properties = this.parseProperties($menu.$properties, record);
            }
            return this.parseExpression($menu.$url, record, properties);
        }
    },
    _openPropertiesDialog: function(values, $properties, record, callback){
        var count = Object.keys($properties.$).length;
        if (count > 0) {
            if (count == 1 && $properties.$.$select) {
            
            }
            var $itemPage;
            if (!$properties.$url) {
                var $prototype = helpers.object.clone($properties);
                $prototype.$title = $properties.$title || "Parameters";
                $itemPage = {
                    $category: "page",
                    $urlParts: {
                        $facet: "$edit"
    ...
    parseProperties: function($properties, record, callback){
        var self = this;
        var values = {};
        Object.keys($properties).forEach(function(prop){
            switch (prop) {
                ...
                case "$actions":
                    break;
                default:
                    values[prop] = self.parseExpression($properties[prop], record);
                    break;
            }
        });
        if (callback) {
            if ($properties.$) {
                ...
                if (count > 0) {
                    if (!(count == 1 && $properties.$.$select)) {
                        self._openPropertiesDialog(values, $properties, record, callback);
                        return;
                    }
                }
            }
