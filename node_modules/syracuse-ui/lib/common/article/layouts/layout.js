...
exports.Layout = helpers.defineClass(Layout, null, {
    toggleAuthoring: function(isOn){
        var self = this;
        if (isOn) {
            if (!self.layoutAuthoring) {
                (self.layoutAuthoring = new layoutAuthoring.LayoutAuthoring()).begin(self);
            }
        }
        else {
            if (self.layoutAuthoring) {
                self.layoutAuthoring.end();
                document.controller.disposeObject(self.layoutAuthoring);
                delete self.layoutAuthoring;
            }
        }
    },
    ...
    _wrapInSlot: function(item){
        if (this.$layout.$layoutType == "side") {
            ...
            item.$$container = $$slot.appendTo(this.$$container);
        }
        else {
            if (item.$layout) {
