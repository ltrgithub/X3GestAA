    ...
    begin: function(layout, $$container, selector){
        ...
        self.$$container.delegate(self.selector, "click.author", function(event){
            ...
            if (document.site.DDAuthoring) {
                ...
                (document.site.DDAuthoring = new DragDrop()).start(item.authoringNode, document.site.authoringView.targetPage.$$body);
            }
            return false;
        }).delegate(self.selector, "mousemove.author", function(event){
            var authoringNode = document.controller.findItem($(this)).authoringNode;
            authoringNode.toggleOverItem(true);
            if (document.site.DDAuthoring) {
                document.site.DDAuthoring.moveDragImage(event, authoringNode);
            }
            return false;
        });
        self.$$container.delegate(".s-author-empty", "mouseup.author mousemove.author mouseleave.author mouseenter.author", function(event){
            ...
            if (dragDrop) {
                switch (event.type) {
                    case "mouseup":
                        var $action = dragDrop.getDropAction(event, self.layout.box.authoringNode);
                        if ($action) {
                            if ($action.$switch) {
                                dragDrop.sourceNode.item.switchCategory($action.$switch);
                            }
                            self.layout.appendNewItem({
                                newItem: dragDrop.sourceNode.item
                            });
                        ...
                        break;
                    case "mousemove":
                        dragDrop.moveDragImage(event, self.layout.box.authoringNode);
                        break;
                    case "mouseleave":
                        ...
                        break;
                    case "mouseenter":
                        if (dragDrop.getDropAction(event, self.layout.box.authoringNode)) {
    ...
    deleteLayout: function(){
        ...
        if (self.layout.$layout) {
            if (self.layout.layoutParent) {
                ...
                exports.onItemMovedOut(layoutParent, self.layout.box, true);
                if (self.layout.box.authoringNode) {
                    self.layout.box.authoringNode.parentNode.removeChildNode(self.layout.box.authoringNode);
                }
