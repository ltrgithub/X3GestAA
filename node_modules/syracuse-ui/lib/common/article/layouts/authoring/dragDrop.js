...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
    ...
    moveDragCue: function($position){
        ...
        this._dropCue.className = "s-author-drop-cue s-author-drop-cue-" + (this.targetNode ? this.targetNode.$authoringLevel : this.sourceNode.$authoringLevel);
        if ($position) {
    ...
    moveDragImage: function(event, targetNode, isOut){
        ...
        if (!isOut && this.$action) {
            ...
            if (isOut) {
                $(this._info).text(_localize.moveItem.replace("{0}", this.sourceNode.item.getTitle()));
            }
    ...
    toggleDraggedCss: function(show){
        if (this.sourceNode && this.sourceNode.item && this.sourceNode.item.$$item) {
            document.site.toggleClass(this.sourceNode.item.$$item[0], "s-author-dragged", show);
        }
    },
    dispose: function(){
