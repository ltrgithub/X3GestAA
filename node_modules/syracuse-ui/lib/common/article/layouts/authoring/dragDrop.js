...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
    _moveContent: function(sourceItem, targetItem){
        var self = this;
        ...
        while (count >= 0) {
            ...
            if (item.authoringNode.$authoringLevel == "field") {
                targetItem.layoutParent.appendNewItem({
                    ...
                    action: self.$drag.$insert,
                    doSelectItem: true
                });
            }
            else {
                if (item.layoutContent) {
                    self._moveContent(item, targetItem);
                }
            }
        }
    },
    dropItem: function(){
        ...
        if (this.$action.$appendNew) {
            ...
            if (this.$action.$moveContent) {
                this._moveContent(this.sourceNode.item, targetItem);
            }
            else {
        ...
        if (this.sourceNode != targetNode) {
            ...
            switch (this.sourceNode.$authoringLevel) {
                ...
                case "field":
                    ...
                    if (!this._isEventOnDropEmpty(event) && targetNode.$authoringLevel != "field") {
                        $action.$appendNew = targetNode.$authoringLevel;
                        text = (targetNode.$authoringLevel == "section") ? _localize.moveSection : _localize.moveBlock;
                    }
