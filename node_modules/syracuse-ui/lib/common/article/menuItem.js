...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
    ...
    setMenu: function($menu, record){
        ...
        if (self.$type && self.$type.indexOf('json') == -1) {
            ...
            self.domItem.setAttribute("href", self.$url = article.parseExpression(self.$url, record));
            self.domItem.className += " " + self.$skin + "-" + self.$type.replace("application/", "");
        }
        else {
            if (self.$url) {
                self.domItem.setAttribute("href", "?url=" + encodeURIComponent(self.$url = article.parseExpression(self.$url, record)));
            }
        }
    },
    dispose: function(){
