...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
    ...
    _setIconMenu: function(){
        ...
        div.className = this.mn.className + "-icon";
        var iconHeight = parseInt((div.style.height = this.$icon.$height || "32px").replace("px", ""));
        div.style.width = this.$icon.$width || "32px";
        this.$$item.prepend(this.iconValue = div);
        var $path = document.site.$item.$iconPath + (this.$icon.$path || "");
        this.iconValue.style.backgroundImage = "url('" + $path + this.$item.$bind.replace("$") + ".png')";
        if (this.$icon.$mode != "icon") {
            div = document.createElement("div");
            div.className = this.mn.className + "-label";
            this.$$label = $(div).appendTo(this.$$item);
        }
    },
    _createMenu: function(article, $menu){
        this.$$item = $(this.mn = document.createElement("a"));
