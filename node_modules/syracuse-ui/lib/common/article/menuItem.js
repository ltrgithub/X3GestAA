...
function _showDiagnoses(menu, $delta){
    ...
    var $menu = article.menusFacade.$menus[menu.$item.$bind];
    if ($menu && $menu.$links) {
...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
    loadBox: function(){
        ...
        if (this.$item.$bind) {
            this.getArticle().menusFacade.bindItem(this);
        }
        else 
            if (this.$item.$link) {
    ...
    hide: function($value){
        this.$isHidden = $value;
        this.$d$item.style.display = !$value ? "" : "none"
        if (this.$$liParent) {
    ...
    _createMenu: function(article, $menu){
        this.$$item = $(this.$d$item = document.createElement("a"));
        this.$d$item.className = this.$skin + ((this.$item.$css) ? (" " + this.$item.$css) : "");
        this.$d$item.setAttribute("data-s-menu", this.id);
        this.$d$item.setAttribute("data-s-article", article.id);
    ...
    setMenu: function($menu, record){
        ...
        if (self.$type && self.$type.indexOf('json') == -1) {
            if (self.$url && (self.$url.indexOf("format=") < 0)) { // TODO: improve this hack
                self.$url += "&format=" + article.parseExpression(self.$type, record);
            }
            self.$d$item.setAttribute("target", self.$target = "blank");
            self.$d$item.setAttribute("href", self.$url = article.parseExpression(self.$url, record));
            self.$d$item.className += " " + self.$skin + "-" + self.$type.replace("application/", "");
        }
        else {
            if (self.$url) {
                self.$d$item.setAttribute("href", "?url=" + encodeURIComponent(self.$url = article.parseExpression(self.$url, record)));
            }
        }
    },
    dispose: function(){
        if (this.menusFacade) {
            this.menusFacade.unbindItem(this, false);
        }
        delete this.page;
