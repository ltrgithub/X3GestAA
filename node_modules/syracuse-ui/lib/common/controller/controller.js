...
exports.Controller = helpers.defineClass(Controller, null, {
    ...
    openPage: function(menuArticle, menu, record){
        switch (menu.$target) {
            case "popup":
                ...
                break;
            default:
                var httpQuery = this.parseUrl(menuArticle.formatMenuUrl(menu, record));
                ...
                if (httpQuery.isSyracuseURL()) {
                    if (httpQuery.$urlParts.$facet == "$summary") {
                        ...
                        if (menu.$type && menu.$type.indexOf("json") == -1) {
                            ...
                            if (!httpQuery.$urlParts.isFusion && menu.$tags != "MOB") { //MOB=temp voir Bruno utilis� dans dashboard mobilehome
    ...
    _postQuery: function(menu, record, article, callback){
        ...
        menu.$url = menuArticle.formatMenuUrl(menu, record);
        var data;
        // = menu.parseProperties ? menu.parseProperties(record) : null;
    ...
    _executeAction: function(menu, record, article){
        ...
        var request = {};
        var $bindAction = request[menu.$item.$bind] = {
            $isRequested: true
        };
        if (menu.$properties) {
            if (menu.$properties.$select) {
                menu.boxParent.openDialog({
                    article: menuArticle,
                    $url: menu.$properties.$select.$url,
                    onValidate: function(page){
                        if ($bindAction.$properties && $bindAction.$properties.$select && $bindAction.$properties.$select.length) {
                            self.notifyChange(menuArticle, "$actions", request);
                        }
                    },
                    onSelectRecord: function(selectedRecords){
                        $bindAction.$properties = {
            ...
            var values = menuArticle.parseProperties(menu.$properties);
            Object.keys(menuArticle.parseProperties(menu.$properties)).forEach(function(prop){
                $bindAction[prop] = values[prop];
            });
        }
        self.notifyChange(menuArticle, "$actions", request);
    },
