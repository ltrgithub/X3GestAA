...
function ApplicationConnection() {
	...
	this._loadFields = function(edit) {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if (!$field.$isExcluded) {
				...
				if (edit) {
					field.isAlertPanelHidden = true;
				} else {
					field.isDiagnoseDisabled = true;
				}
				fields.push(field);
			}
		}
	...
	this._showList = function(parentNode) {
		if (this.page && !this.list) {
			...
			self.page.diagnoseSlot.className = "s-profile-diagnose-slot";
			slot.appendChild(self.page.diagnoseSlot);
			...
			self.saveBtn = syra_button.add({
				...
				click: function() {
					if (syra_form.validate(self.page)) {
						...
						var updated = syra_form.getSendBag(self.page);
						var $fields = self.page.$prototype.$properties;
						var $binds = Object.keys($fields);
						var resetRequested;
						for (var ii = 0, jj = $binds.length; !resetRequested && ii < jj; ii++) {
							if ($fields[$binds[ii]].$resetConnection !== undefined && updated[$binds[ii]]) {
								resetRequested = true;
							}
						}
						if (resetRequested && syra_fusion) {
							syra_fusion.onUserActionUnload(function() {
								self.commit(sendBag, updated);
							}, function() {}, "applicationConnection");
						} else {
							self.commit(sendBag, updated);
						}
	...
	this.commit = function(sendBag, updated) {
		var self = this;
		...
		delete self.page.dataset.$diagnoses;
		...
		wc.validate(function() {
			sendBag.applicationConnection = {
				data: self.buildSaveData(updated)
			};
			sendBag.$actions = {
				$save: {
					$isRequested: true
	...
	this.buildSaveData = function(updated) {
		var $meta;
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if ($field.$resetConnection !== undefined && updated[$bind]) {
				($meta = $meta || {})[$bind] = {
		...
		delete this.page.dataset.$diagnoses;
