...
function _sendOneSearchRequest(page, dataset, options, isDataSource) {
	...
	syra_site.urlMaker.formatMenuUrl(page, {
		...
		syra_controller.callServer(page, options, function(dataSrc) {
			// display data only when the two resquest are complete in order to open a function or not
			var queryCode = isDataSource ? "$queryData" : "$queryFunctions";
			page.checkResultSearch = page.checkResultSearch || {};
			page.checkResultSearch[queryCode] = dataSrc;
			var listKey = Object.keys(page.checkResultSearch);
			if (listKey.length === 2) {
				// set invisible the page that contain result
				listKey.forEach(function(datakey) {
					var data = page.checkResultSearch[datakey];
					page.applyChange({
						query: data.query,
				...
				// hack to open a function on search result - if there only one link in function or only functions and ctrl pressed we load the first function
				var resData = page.checkResultSearch["$queryData"].$queryData;
				var resFunction = page.checkResultSearch["$queryFunctions"].$queryFunctions;
				if ((!resData ||  resData.length === 0) && (resFunction && resFunction.length === 1 || (resFunction && resFunction.length >= 1 && (page.searchField && page.searchField.ctrl)))) {
					$(page.layoutSlot).hide();
					page.boundFields && _clickFirstFunction(page.boundFields.$vignetteFunctions);
					page.searchField.ctrl = false;
					$(page.layoutSlot).show();

				}
				delete page.checkResultSearch;
			}
		});
...
function _clickFirstFunction(vignetteFunction) {
	if (vignetteFunction) {
		var idMap = vignetteFunction.length > 0 && vignetteFunction[0].vignette && vignetteFunction[0].vignette.childLists && vignetteFunction[0].vignette.childLists[0].records[0].domItem.click();
	}
}
...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	loadBox: function(initData) {
		...
		this.searchField = syra_site.searcher && syra_site.searcher.searchField;
