...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	...
	_sendOneSearchRequest: function(dataset, options, isDataSource, selectTab) {
		var self = this;
		var $query = {
			$url: searchUtils.getSearchModeLinks()[this._getSearchMode()].$url + (isDataSource ? "&source=data" : "&source=functions") + "&profile={$userprofile}"
		};
		self._updateSearchModeLabels(isDataSource);
		syra_site.urlMaker.formatMenuUrl(syra_site, $query, dataset, function($url, isCanceled) {
		...
		for (var mm = 0, pp = $binds.length; mm < pp; mm++) {
			...
			if ((property.$isHighlight || property.$isSummary) && recordData[$binds[mm]]) {
				...
				if (data) {
					...
					if (data.replace) {
						value.innerHTML = data.replace(/<em>/g, "<em class='s-search-res-data-em'>");
					}
				}
