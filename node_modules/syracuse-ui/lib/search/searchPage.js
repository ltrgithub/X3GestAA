	...
	for (var ii = 0, pp = $binds.length; ii < pp; ii++) {
		...
		if (($prop.$isHighlight || $prop.$isSummary) && dataset[$bind]) {
			...
			if (data) {
				...
				if (data.replace) {
					value.innerHTML = syra_dom.formatHTMLText(data.replace(/<em>/g, "<em class='s-search-item-desc-em'>"), false);
				}
			}
...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	loadBox: function() {
		...
		self.searchField.setDataValue(self.searchCriteria = params.q);
		...
		self._modeBtn = syra_menus.button.add({
			parent: self,
			slot: slot,
			...
			btnclick: function() {
				...
				syra_menus.button.setText(this, self.searchField.isFuzzyMode ? syra_local.searchStartingWith : syra_local.searchSimilar);
