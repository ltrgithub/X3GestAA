...
var _queryLinks = {
	$query1: {
		$url: "/sdata/syracuse/search/syracuse/$search?representation=queryResult.$search&q={$search}&source=functions&count=5&profile={$userprofile}"
	},
	$query2: {
		$url: "/sdata/syracuse/search/syracuse/$search?representation=queryResult.$search&q={$search}&source=data&count=5&profile={$userprofile}"
	}
};
...
function _fillList(page, $bind) {
	syra_site.urlMaker.formatMenuUrl(syra_site, _queryLinks[$bind], page.dataset, function($url, isCanceled) {
		syra_controller.sendRequest(page, {
			$location: {
				$url: $url
			}
		}, function(data) {
...
exports.SearcherPage = helpers.defineClass(SearcherPage, RawPage, {
...
exports.load = function(layoutSlot) {
	...
	var $itemPage = {
		...
		$pageCategoryClass: SearcherPage,
		$representation: {
			$prototype: {
