...
exports.SearchFacets = helpers.defineClass(SearchFacets, Article, {
	...
	onItemInOut: function(onEnter, event, target) {
		if (event.target.syraFacet !== undefined) {
			var facet = this.facets[target.syraFacet];
			syra_site.dom.toggleClass(facet.members[target.syraMember].checkBox.icon, "s-enter", onEnter);
		}
	},
	...
	addMember: function(facet, index) {
		...
		member.checkBox.icon.syraFacet = member.checkBox.input.syraFacet = facet.index;
		member.checkBox.icon.syrainout = this.id;
