...
exports.FacetsBar = _helpers.defineClass(FacetsBar, null, {
	load: function(page, options, $criteria) {
		...
		if (options.slot) {
			...
			bar.slot.className = $skin + "-slot";
			self.slot = bar.body = document.createElement("div");
			bar.body.className = $skin + "-body";
			page.domItem.appendChild(bar.slot);
	...
	addMemberLinks: function(member) {
		...
		links.className = this.$facetSkin + "-member-links";
		member.item.appendChild(links);
		member.linksItem = this.page.addItem(links, {
			$category: "menus",
			$skin: "s-list-menus",
			$layout: {
	...
	addMember: function(facet, index) {
		...
		member.item.className = this.$facetSkin + "-member";
		member.title.className = this.$facetSkin + "-member-title";
		...
		member.count.className = this.$facetSkin + "-member-count";
	...
	addFacet: function($searchFacet) {
		...
		facet.item.className = this.$facetSkin;
		facet.header.className = this.$facetSkin + "-header s-search-facet-type-" + facet.dataset.$facet;
		...
		facet.body.className = this.$facetSkin + "-body";
	...
	fill: function(facets) {
		if (facets) {
			...
			if (!this.body) {
				...
				header.className = this.$facetSkin + "s-header";
				header.textContent = syra_local.searchFacetFilters;
				this.slot.appendChild(header);
				this.body = document.createElement("div");
				this.body.className = this.$facetSkin + "s-body";
