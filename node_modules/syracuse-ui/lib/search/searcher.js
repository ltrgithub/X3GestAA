...
helpers.defineClass(SearchField, Field, {
	...
	applyShortCuts: function(shortcuts) {
		this._isAutoComplete && _suggest.applyShortCuts(shortcuts);
	}
});
...
exports.SearchFieldPage = helpers.defineClass(SearchFieldPage, RawPage, {
	...
	createNewItem: function($item, boxParent) {
		if ($item.$bind == "$search") {
			...
			_searcherField.$field = this.$prototype.$properties[$item.$bind];
			return this.initializeNewItem(_searcherField, $item, boxParent);
		}
