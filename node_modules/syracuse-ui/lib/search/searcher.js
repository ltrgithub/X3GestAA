...
function SuggestBox(field, $source, slot) {
	...
	this.header = syra_dom.text("s-suggest-box-title", this.$title = syra_local["search_" + $source], this.slot);
	...
	this.fetch = function(value) {
		...
		self.field.fetchSearch({
			...
			options: {
				...
				error: function() {
					!self.disposed && self._setEmptyMessage(syra_local.search_no_res);
					return true;
				}
			}
		});
	...
	this.fill = function(data) {
		...
		if (this.$totalResults) {
			...
			for (var ii = 0, jj = this.$records.length; ii < jj; ii++) {
				...
				var row = syra_dom.anchor("s-suggest-box-item-link", li);
				this.rows.push(row);
...
helpers.defineClass(SearchField, Field, {
	getUrl: function($seach) {
		return syra_config.$searchUrl + ("&q=" + $seach) + (this.isFuzzyMode ? "&qt=fuzzy" : "") + ("&profile=" + syra_site.$prototype.$userprofile);
	},
	toggle: function(show) {
		syra_dom.hide(syra_site.searchFieldSlot, !show);
		show && this.focus();
	},
	...
	drawRecordText: function(css, data, dataset) {
		...
		div.innerHTML = syra_dom.escape(text.replace(/<em>/g, "<em class='" + css + "-em'>"));
		return div;
	},
	load: function() {
		this.results = {};
		...
		if (value !== "") {
			...
			if (self.searchValue !== value) {
				...
				if (!self.suggestPopup) {
					var slot = syra_dom.div("s-suggest s-over-popup");
					self.suggestBox = {
...
exports.load = function() {
	...
	syra_site.searchFieldSlot = syra_dom.div("", syra_site.headerBar);
	syra_dom.hide(syra_site.searchFieldSlot, true);
	return syra_site.addItem(syra_site.searchFieldSlot, {
		$bind: "$search",
		$isEditMode: true,
		$class: SearchField,
		$skin: "s-search-field",
