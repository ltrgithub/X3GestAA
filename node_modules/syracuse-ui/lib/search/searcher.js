...
function _callServer(page, box, options) {
	...
	var $url = _queryLinks + (box.isQueryData ? "&source=data" : "&source=functions");
...
function FullBox(page, $bind, slot) {
	var self = this;
	(this.page = page)[$bind] = this;
	this.$bind = $bind;
	this.isQueryData = $bind == "queryData";

	this.slot = slot.appendChild(document.createElement("div"));
	this.slot.className = "s-search-main-query s-search-" + $bind;

	this.header = document.createElement("header");
	this.header.className = "s-search-main-query-header";
	this.header.texContent = this.$title = syra_local["search_" + $bind];
	...
	this.fill = function(data) {
		...
		var queryData = {
			...
			$searchFacets: this.isQueryData ? data.$searchFacets : page.dataset.$searchFacets, // no search facet for 'functions' results
			$properties: {}
		};
...
function SuggestBox(field, $bind, slot) {
	...
	this.fill = function(data) {
		...
		if (data.$totalResults != undefined) {
			this.header.textContent = this.$title + " (" + (this.$totalResults = data.$totalResults) + ")";
		}
		if (this.$totalResults) {
			for (var ii = 0, jj = this.$records.length; ii < jj; ii++) {
				...
				this.isQueryData && row.appendChild(_addRecordText(this.field.page, "s-search-suggest-box-item-type", $record.$resultTypeTitle, $record));
				this.body.appendChild(row);
			}
		} else {
			this.body.textContent = syra_local.search_no_res;
		}
		var count = (this.field.queryFunctions.$totalResults + this.field.queryData.$totalResults);
		...
		if (value !== "") {
			...
			if (self.searchValue !== value) {
				...
				if (!self.suggestPopup) {
					...
					new SuggestBox(self, "queryFunctions", slot);
					new SuggestBox(self, "queryData", slot);
					...
					self.suggestPopup = syra_site.dialogManager.openPopup(self.boxParent, {
						...
						onClose: function() {
							...
							syra_site.disposeObject(self.queryFunctions);
							syra_site.disposeObject(self.queryData);
							slot = self.queryFunctions = self.queryData = self.suggestMore = self.suggestPopup = self.suggestSelected = null;
						}
					});
				}
				self.queryFunctions.$search = self.queryData.$search = value;
				_callServer(self.page, self.queryFunctions);
				_callServer(self.page, self.queryData);
			}
		} else {
			self.closeSuggest();
		}
	...
	applyShortCut: function(shortcuts) {
		...
		if (!shortcuts.enter) {
			...
			if (self.suggestPopup && (shortcuts.up || shortcuts.down)) {
				var searchResRecords = self.page.queryFunctions.field.records.concat(self.page.queryData.field.records);
...
function _addSearchField(page, slot) {
	page.$prototype.$properties.$search = {
		$type: "application/x-string",
		$title: syra_local.searchFieldTitle
	};
	return _searcherField = page.loadNewItem(slot, {
		$bind: "$search",
		$isEditMode: true,
		$class: SearcField,
...
exports.load = function(layoutSlot) {
	...
	return _addSearchField(syra_site, slot);
};
...
exports.SearchPage = helpers.defineClass(SearchPage, DesktopPage, {
	loadBox: function() {
		...
		new FullBox(this, "queryFunctions", this.layoutContent.domItem);
		new FullBox(this, "queryData", this.layoutContent.domItem);
		this.fetch();
	},
	...
	onModeClick: function() {
		...
		syra_menus.setButtonTitle(this._modeBtn, _isSimilar ? syra_local.searchStartingWith : syra_local.searchSimilar);
		_callServer(this, this.queryFunctions);
		_callServer(this, this.queryData);
	},
	...
	_sendAllRequests: function(options) {
		...
		_callServer(this, this.queryFunctions, functionsOptions);
		_callServer(this, this.queryData, dataOptions);
		...
		// pager step navigation (pagination)
			...
			// get corresponding tab information, update content accordingly
				...
				if (options.field && options.field.$item && options.field.$item.$bind) {
					...
					_callServer(this, this[options.field.$item.$bind], options);
				} else {
