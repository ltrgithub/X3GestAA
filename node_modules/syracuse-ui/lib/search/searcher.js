...
helpers.defineClass(SearchField, Field, {
	...
	onRunClick: function(event, target, results) {
	...
	onClearClick: function() {
		...
		this.closeSuggest();
		syra_workingCopy.onInputChange(this);
	},
	...
	applyShortCut: function(shortcuts) {
		...
		if (!shortcuts.enter) {
			...
			if (self.suggestBox && self.suggestPopup && (shortcuts.up || shortcuts.down)) {
				var rows = self.suggestBox.functions.rows.concat(self.suggestBox.data.rows).concat(self.suggestMore);
				if (rows.length > 1) {
					var next = 0;
						...
						filler: function(data) {
							if (!self.disposed) {
								...
								if (self.results.functions && self.results.data) {
									...
									self.onRunClick(null, null, self.results);
								}
							}
						}
					});
