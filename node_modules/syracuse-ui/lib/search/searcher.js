...
function SuggestBox(field, $source, slot) {
	...
	this.fill = function(data) {
		...
		syra_menus.button.hide(this.field.suggestMore, count <= 0);
		syra_menus.button.setText(this.field.suggestMore, count > 5 ? syra_local.searchAdvanceResults : syra_local.searchAdvanceDetails);
	};
...
helpers.defineClass(SearchField, Field, {
	getUrl: function($seach) {
		return _searchUrl + ("&q=" + $seach) + (this.isFuzzyMode ? "&qt=fuzzy" : "") + ("&profile=" + syra_site.$prototype.$userprofile);
	},
	...
	loadBox: function() {
		...
		this.clearBtn = syra_menus.button.add({
			...
			btnclick: function() {
				...
				field.closeSuggest();
				syra_workingCopy.onInputChange(field);
			}
		});
	...
	showPicker: function(onOut) {
		...
		syra_menus.button.hide(this.clearBtn, !show);
	},
	onKeyUp: function() {
		this.showPicker();
	},
		...
		if (value !== "") {
			...
			if (self.searchValue !== value) {
				...
				if (!self.suggestPopup) {
					...
					self.suggestMore = syra_menus.button.add({
						parent: self,
						slot: slot,
					...
					self.suggestPopup = syra_dlg.openPopup(self.boxParent, {
						content: self,
	...
	callServer: function(context) {
		...
		syra_url.formatMenuUrl(self.page, {
			...
			syra_controller.callServer(self.page, context.options, context.filler);
		});
	},
	dispose: function() {
		this.closeSuggest();
