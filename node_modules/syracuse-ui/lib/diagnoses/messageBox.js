...
function _addDiagnoses(messageBox, $diagnoses) {
	messageBox._ensureStackTrace();
	// add to formatted tab content
	var diagnoses = document.createElement("div");
	diagnoses.className = "s-msgbox-diagnoses";
	messageBox._coreFormatted.appendChild(diagnoses);
	...
	for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
		...
		if (severity) {
			...
			group = messageBox._groups[severity];
		}
		if (!group) {
			group = messageBox._groups[severity = "error"];
		}
		...
		if ($diagnose.$links || $diagnose.$actions) {
			messageBox.applyChange($diagnose);
		}
		diagnoses.appendChild(group.groupItem);
	}
	// add to raw tab content
	messageBox._coreRaw.innerHTML += messageBox.preformat(messageBox.detailToString($diagnoses));
}
...
function _onTabClick() {
	...
	syra_site.dom.toggle(msgBox._coreFormatted);
	syra_site.dom.toggle(msgBox._coreRaw);
	msgBox.resizeArticle();
}

function _onMessageClick() {
	var page = this.parent;
	this.isOpened = !this.isOpened;
	syra_menus.button.setText(this, this.isOpened ? syra_local.msgbox_diagnose_hide_detail : syra_local.msgbox_diagnose_show_detail, this.isOpened ? "collapse" : "expand");
	var message = this.message;
	if (this.isOpened) {
		if (!this.message.stackTraceView) {
			this.message.stackTraceView = document.createElement("div");
			this.message.stackTraceView.className = "s-msgbox-diag-stackTrace";
			this.message.stackTraceView.innerHTML = page.preformat(((this.message.appCode != undefined ? this.message.appCode + "\n" : "") + (this.message.stackTrace || this.message.$stackTrace)).replace(/\n/g, "<br/>"));
			this.message.label.appendChild(this.message.stackTraceView);
		}
	}
	this.message.stackTraceView.style.display = this.isOpened ? "" : "none";
	page.resizeArticle();
}
...
exports.MessageBox = helpers.defineClass(MessageBox, DesktopPage, {
	...
	ensureLayoutMode: function() {
		DesktopPage.prototype.ensureLayoutMode.call(this);
		if (this.domTitle) {
	...
	_ensureStackTrace: function() {
		if (!this._core) {
			...
			// tabs
			this._openedTab = syra_menus.button.add({
				parent: this,
				slot: this._coreTabsNav,
				text: syra_local.msgbox_nav_title_formatted,
				css: "s-msgbox-core-tab s-open",
				btnclick: _onTabClick
			});
			syra_menus.button.add({
				parent: this,
				slot: this._coreTabsNav,
			...
			this._bodyBtn = syra_menus.button.add({
				parent: this,
				slot: slot,
				...
				btnclick: function() {
					var msgBox = this.parent;
					syra_site.dom.toggle(msgBox._coreTabsNav);
					syra_site.dom.toggle(msgBox._core);
					this.isOpened = this.isOpened;
					syra_menus.button.setText(this, this.isOpened ? syra_local.msgbox_body_picker_opened : syra_local.msgbox_body_picker_closed, this.isOpened ? "collapse_row" : "expand_row");
					syra_menus.button.hide(msgBox._selectBtn, !this.isOpened);
					msgBox.resizeArticle();
				}
			});
			...
			slot.className = "s-msgbox-btn-slot";
			this._selectBtn = syra_menus.button.add({
				parent: this,
				slot: slot,
