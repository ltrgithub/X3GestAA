...
exports.MessageBox = helpers.defineClass(MessageBox, RawPage, {
	...
	onMessageClick: function(event, btn) {
		var show = btn.className.indexOf("s-open") < 0;
		syra_menus.setButtonTitle(btn, show ? syra_local.msgbox_diagnose_hide_detail : syra_local.msgbox_diagnose_show_detail);
		syra_site.dom.toggleClass(btn, "s-open", show);
	...
	_onBodyBtnClick: function() {
		...
		var show = this._bodyBtn.className.indexOf("s-open") < 0; //toggle
		syra_site.dom.toggleClass(this._bodyBtn, "s-open", show);
		this._bodyBtn.textContent = show ? syra_local.msgbox_body_picker_opened : syra_local.msgbox_body_picker_closed;
		this._selectBtn.style.display = show ? "" : "none";
		this.resizeArticle();
	},
	_ensureStackTrace: function() {
		if (!this._core) {
			...
			this._bodyBtn = syra_menus.addTextButton(syra_local.msgbox_body_picker_closed, "s-msg-box-body-btn", "_onBodyBtnClick");
	...
	_appendDiagnoses: function($diagnoses) {
		...
		for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
			...
			if ($diagnose.stackTrace || $diagnose.$stackTrace) {
				var btn = syra_menus.addIconButton(syra_local.msgbox_diagnose_show_detail, "s-msgbox-diag-btn", "onMessageClick");
	...
	dispose: function() {
		...
		this._bodyBtn = this._selectBtn = this._message = this._openedTab = null;
		this._widgetResizer = this.dialog = this.$itemButtons = this._groups = null;
