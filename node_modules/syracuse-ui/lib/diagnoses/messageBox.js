...
exports.MessageBox = helpers.defineClass(MessageBox, RawPage, {
	...
	drawBox: function() {
		...
		$(this.boundFields.$message[0].fieldValue).html((fHtmlEsc(msg)).replace(/\n/g, '<br/>'));
		...
		this._item.style.minWidth = this.$item.$minWidth ? this.$item.$minWidth + "px" : "350px"; // "400px" ? "100px" ?
		//this.body.style.minHeight = "30px";
		if (this._core) {
	...
	_appendDiagnoses: function($diagnoses) {
		...
		for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
			...
			message.label.innerHTML = document.site.formatHTMLMessage($diagnose.message || $diagnose.$message || this.localize.msgbox_diagnose_default_msg);
