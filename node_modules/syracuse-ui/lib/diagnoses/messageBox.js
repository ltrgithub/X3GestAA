...
function _addDiagnoses(messageBox, $diagnoses) {
	...
	var diagnoses = syra_dom.addDiv("s-msgbox-diagnoses", messageBox._coreFormatted);
	...
	for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
		...
		message.label.innerHTML = syra_dom.escape($diagnose.message || $diagnose.$message || syra_local.msgbox_diagnose_default_msg, true);
...
exports.MessageBox = helpers.defineClass(MessageBox, DesktopPage, {
	...
	preformat: function(txt) {
		return "<pre class='s-msgbox-pre'>" + syra_dom.escape(txt, true) + "</pre>";
	},
	...
	drawBox: function() {
		...
		var msg = ["<pre>", (fHtmlEsc(this.$item.$message || syra_local.msgbox_default_message)).replace(/\n/g, '</pre><pre>'), "</pre>"];
	...
	_ensureStackTrace: function() {
		if (!this._core) {
			...
			// tabs content
			this._coreFormatted = syra_dom.addDiv("s-msgbox-core-tab-formatted", this._core);
