...
exports.MessageBox = helpers.defineClass(MessageBox, RawPage, {
    ...
    selectText: function(element){
        var textItem = document.site.isDomDisplay(this._coreFormatted) ? this._coreFormatted : this._coreRaw;
    ...
    drawBox: function(){
        ...
        document.site.setDraggable(this, this.$$header);
        ...
        if (this.$item.$$container) {
            (this.$$container = this.$item.$$container).append(this.$$item);
        ...
        this.getMenuItem(this._focusBtnId = this._defaultBtnId).$$item.focus();
    },
            ...
            case "msg":
                ...
                if (show) {
                    if (!message.stackTraceView) {
                        ...
                        message.stackTraceView.innerHTML = this.preformat((message.appCode + "\n" + message.stackTrace).replace(/\n/g, "<br/>"));
    ...
    _bindEvents: function(bind){
        var self = this;
        if (bind) {
            $(document).bind("keypress.msgbox keydown.msgbox keyup.msgbox", function(event){
                switch (event.type) {
                    case "keypress":
                        switch (event.keyCode) {
                            case keyHelper.keyCode.ENTER:
                                self.onMenuClick({
                                    $bind: self._defaultBtnId
                                });
                                break;
                            ...
                            case keyHelper.keyCode.TAB:
                                if (self.$itemButtons.length > 1) {
                                    ...
                                    for (ii = 0, len = self.$itemButtons.length; ii < len; ii++) {
                                        if (self.$itemButtons[ii] == self._focusBtn.$item.$bind) {
                                            break;
                                        }
                                    }
                                    if (ii < len) {
                                        ii = event.shiftKey ? (ii == 0 ? len - 1 : ii - 1) : (ii + 1) % 2;
                                        document.site.toggleClass(self.getMenuItem(self._focusBtnId).mn, "s-msgbox-button-default", false);
                                        self._focusBtnId = self.$itemButtons[ii];
    ...
    _ensureStackTrace: function(){
        if (!this._core) {
            ...
            this._body.insertBefore(this._coreTabsNav, footer._item);
            
            this._core = document.createElement("div");
            this._core.className = "s-msgbox-core";
            this._core.style.display = "none";
            this._body.insertBefore(this._core, footer._item);
            ...
            this._body.insertBefore(slot, this._message);
    ...
    close: function(closedBy, response){
        var self = this;
        setTimeout(function(){
            if (self.$item.callback ? (self.$item.callback(response, closedBy) !== false) : true) {
                if (self.$$item) {
                    self.$$item.remove();
                }
