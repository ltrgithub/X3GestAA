...
exports.MessageBox = helpers.defineClass(MessageBox, RawPage, {
	...
	selectText: function(element) {
		if (this._coreFormatted) {
			var textItem = this._coreFormatted && document.site.dom.isDisplay(this._coreFormatted) ? this._coreFormatted : this._coreRaw;
	...
	onTabClick: function(picker) {
		...
		document.site.dom.toggle(this._coreFormatted);
		document.site.dom.toggle(this._coreRaw);
		this.resizePage();
	},
	onMessageClick: function(picker) {
		var show = picker.className.indexOf("s-open") < 0; //toggle
		document.site.dom.toggleClass(picker, "s-open", show);
		...
		if (shortcurts.tab) {
			if (self.$itemButtons.length > 1) {
				...
				if (ii < len) {
					...
					document.site.dom.toggleClass(self.getMenuItem(self._focusBtnId).domItem, "s-msgbox-button-default", false);
					self._focusBtnId = self.$itemButtons[ii].$bind;
					document.site.dom.toggleClass(self.getMenuItem(self._focusBtnId).domItem, "s-msgbox-button-default", true);
					setTimeout(function() {
	...
	onBodyPickerClick: function() {
		document.site.dom.toggle(this._coreTabsNav);
		document.site.dom.toggle(this._core);
		var show = this.bodyPicker.className.indexOf("s-open") < 0; //toggle
		document.site.dom.toggleClass(this.bodyPicker, "s-open", show);
	...
	resizePage: function(setMinimumDimensions) {
		if (this._core) {
