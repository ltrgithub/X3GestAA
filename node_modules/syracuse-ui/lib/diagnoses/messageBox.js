...
exports.MessageBox = helpers.defineClass(MessageBox, RawPage, {
	...
	loadBox: function(initData) {
		...
		if (!this.displayDisabled) {
			if (this.$item.layoutSlot) {
				...
				this.dialog = syra_site.dialogManager.openModal(syra_site, {
		...
		this.keys = {};
		this.$item.$showDetail && this.onBodyPickerClick();
	...
	onTabClick: function(event, picker) {
	...
	onMessageClick: function(event, picker) {
		...
		syra_site.dom.toggleClass(picker, "s-open", show);
		var message = this._groups[picker.syraSeverity].messages[picker.syraIndex];
		if (show) {
		...
		if (shortcurts.tab) {
			if (self.$itemButtons.length > 1) {
				...
				if (ii < len) {
					...
					syra_site.dom.toggleClass(self.getMenuItem(self._focusBtnId).domItem, "s-msgbox-button-default", false);
					self._focusBtnId = self.$itemButtons[ii].$bind;
					syra_site.dom.toggleClass(self.getMenuItem(self._focusBtnId).domItem, "s-msgbox-button-default", true);
					setTimeout(function() {
						self.getMenuItem(self._focusBtnId).domItem.focus();
					}, 5);
				}
			}
			return true;
	...
	onBodyPickerClick: function() {
		...
		syra_site.dom.toggleClass(this.bodyPicker, "s-open", show);
		this.bodyPicker.textContent = show ? syra_local.msgbox_body_picker_opened : syra_local.msgbox_body_picker_closed;
	...
	_ensureStackTrace: function() {
		if (!this._core) {
			...
			tab.syraOnClick = "onTabClick";
			tab.textContent = syra_local.msgbox_nav_title_formatted;
			this._coreTabsNav.appendChild(this._openedTab = tab);
			tab = document.createElement("a");
			tab.className = "s-msgbox-core-tab";
			tab.syraOnClick = "onTabClick";
			tab.textContent = syra_local.msgbox_nav_title_raw;
			...
			this.bodyPicker.syraOnClick = "onBodyPickerClick";
			this.bodyPicker.textContent = syra_local.msgbox_body_picker_closed;
			...
			this._selectPicker.syraOnClick = "selectText";
			this._selectPicker.className = "s-msgbox-select-picker";
			this._selectPicker.style.display = "none";
			this._selectPicker.textContent = syra_local.msgbox_select_picker;
	...
	_appendDiagnoses: function($diagnoses) {
		...
		for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
			...
			message.label.innerHTML = syra_site.formatHTMLText($diagnose.message || $diagnose.$message || syra_local.msgbox_diagnose_default_msg);
			if ($diagnose.stackTrace || $diagnose.$stackTrace) {
				var picker = document.createElement("a");
				picker.syraSeverity = severity;
				picker.syraIndex = group.messages.length - 1;
				picker.syraOnClick = "onMessageClick";
	...
	dispose: function() {
		...
		this.bodyPicker = this._selectPicker = this._message = this._openedTab = null;
		this.dialog = this.$itemButtons = this._groups = null;
		if (syra_site) {
			syra_site.messageBox = null;
		}
