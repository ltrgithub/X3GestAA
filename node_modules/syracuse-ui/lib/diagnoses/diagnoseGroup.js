...
exports.DiagnoseGroup = helpers.defineClass(DiagnoseGroup, null, {
    ...
    empty: function(field){
        if (field) {
			...
			document.site.emptyDom(this._groupItem);
        }
    },
    drawDiagnose: function($diagnose, options){
        ...
        if (options) {
            if (options.field && options.field.$item.$isExpressionChild) {
                ...
                dom.textContent = options.fieldLink.textContent.toUpperCase() + " ";
                label.insertBefore(dom, label.firstChild);
                options.field.diagnoseFieldSlot.style.display = "none";
            }
            else {
                label.insertBefore(options.fieldLink, label.firstChild);
            }
        }
