		...
		if (!field.isDiagnoseDisabled && message.$diagnoses && message.$diagnoses.length > 0) {
			if (field.isDiagnosePopup) {
				if (!field.diagnoseFlag && !syra_site.isTabletDevice) {
					...
					field.diagnoseFlag = syra_menus.button.add({
						isIndicator: true,
						parent: field,
				...
				syra_site.dom.toggleClass(field.refDescriptionItem || field.descriptionItem, "s-field-desc-diagnose", true);
			}
			var severities = {};
			for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
				...
				if (field.isDiagnosePopup) {
					...
					item.fieldText = (field.getTitle() || field.$field.$title || "") + text;
				} else {
					...
					if (!field.getTitle() && !field.$field.$title && field.arrayLevel && field.arrayLevel == "cell") {
						...
						item.fieldText = title + ":";
					} else {
						item.fieldText = (field.getTitle() || field.$field.$title || field.$item.$bind) + ":";
					}
				...
				if (field.$item.$isExpressionChild) {
					...
					iitem.msgItem.link.insertBefore(gotoBtn.link, item.msgItem.iconflag.nextSibling);
					if (field.diagsSlot) {
						syra_site.dom.hide(field.diagsSlot, true);
					}
				} else {
					item.msgItem.link.insertBefore(gotoBtn.link, item.msgItem.iconflag.nextSibling);
				}
			}
...
exports.clean = function(panel, field) {
	if (field.diagsSlot) {
		syra_site.dom.toggleClass(field.refDescriptionItem || field.descriptionItem, "s-field-desc-diagnose", false);
		syra_site.dom.removeChild(field.diagsSlot);
		delete field.diagsSlot;
	}
	syra_menus.button.remove(field.diagnoseFlag);
		...
		if (onEnter) {
			if (!field.diagsSlot) {
				if (items.length) {
					...
					field.diagsPopup = syra_dlg.openPopup(field.page, {
						content: {},
						slot: field.diagsSlot,
						position: {
							my: "left top",
