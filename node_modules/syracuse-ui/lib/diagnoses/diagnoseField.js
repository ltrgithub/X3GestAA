	...
	if (field) {
		...
		if (!field.isDiagnoseDisabled && message.$diagnoses && message.$diagnoses.length > 0) {
			...
			for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
				...
				if (!panel.goToField) {
					panel.goToField = function(event, btn) {
						var field = syra_store.get(btn.syraFieldId);
						if (field) {
							field.focus && field.focus();
							exports.onNavigationClick(this, 0, field);
						}
					};
				}
				...
				if (field.$item.$isExpressionChild) {
					...
					item.topLabel.insertBefore(topBtn, item.topLabel.firstChild);
				}
			}
...
exports.onNavigationClick = function(panel, btn, field) {
