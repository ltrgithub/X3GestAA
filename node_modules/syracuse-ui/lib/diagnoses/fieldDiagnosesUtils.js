...
exports.drawDiagnose = function(field, $diagnose, severityGroup){
    ...
    if (field.isDiagnoseInPlace) {
        ...
        if (!severityGroup.$$group.children().children().children("a[data-s-menu='" + field.id + "']")[0]) {
            ...
            appendArticleDiagnose(field, $diagnose);
...
helpers.defineClass(ArticleDiagnose, null, {
    ...
    appendDiagnose: function(severity){
        var self = this;
    ...
    removeDiagnose: function(severity){
        var self = this;
...
function appendArticleDiagnose(field, $diagnose){
    var f = field;
    while (f.articleParent) {
        (f.articleParent.articleDiagnose = f.articleParent.articleDiagnose || new ArticleDiagnose()).appendDiagnose($diagnose.severity)
        f = f.articleParent;
    }
};

function emptyArticleDiagnose(field){
    var f = field;
    if (field.diagSeverities) {
        while (f.articleParent) {
            if (f.articleParent.articleDiagnose) {
                for (var ii = 0; ii < field.diagSeverities.length; ii++) {
                    f.articleParent.articleDiagnose.removeDiagnose(field.diagSeverities[ii]);
                }
            }
            f = f.articleParent;
        }
