		...
		exports.box.show({
			...
			callback: function() {
				if (syra_site.lastOpenedMainPageUrlSegments) {
					syra_url.history.load(syra_site.lastOpenedMainPageUrlSegments.$url);
				} else {
...
exports.box = {
	show: function(options, displayDisabled) {
		...
		this.openedBox = new MessageBox();
		this.openedBox.$prototype = {};
		this.openedBox.displayDisabled = displayDisabled; // property necessary for htmleditor msgbox
		syra_site.initializeNewItem(this.openedBox, options);
		this.openedBox.loadBox();
		return this.openedBox;
	},
	close: function(closedBy, id) {
		if (this.openedBox && (!id || id == this.openedBox.id)) {
			this.openedBox.close(closedBy);
		}
	},
	dispose: function() {
		this.openedBox && this.openedBox.dispose();
		this.openedBox = null;
	}
};
