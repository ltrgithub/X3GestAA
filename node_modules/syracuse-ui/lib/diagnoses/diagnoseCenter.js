...
exports.showDiagnoses = function(message, item, options) {
	if (item && item.isField) {
		...
		if (message.field._tabletEscapePicker) { //temp hack for tablet
			// Hide contextual menu and display escape/reset button
			var ctxMenu = message.field.menusBox ? message.field.menusBox.domItem : null;
			if (message.$diagnoses && message.$diagnoses.length > 0) {
				message.field._tabletEscapePicker.style.display = "table-cell";
				if (ctxMenu)
					ctxMenu.style.display = "none";
			} else {
				message.field._tabletEscapePicker.style.display = "none";
				if (ctxMenu)
	...
	if (page && page != syra_site && page != syra_site.userProfile) {
		...
		if (message.$diagnoses && message.$diagnoses.length) {
			var onlyOne = message.$diagnoses.length == 1;
			exports.showBox({
				$type: "alert",
				$buttons: "ok",
				$title: syra_local.siteMsgboxTitle,
				$diagnoses: message.$diagnoses,
				$message: (onlyOne && message.$diagnoses[0].$message) || syra_local.userProfile_siteMsgboxMsg,
				$origin: onlyOne ? message.$diagnoses[0].$origin : "",
				$useLinkAsOk: (page && page == syra_site.userProfile) ? onlyOne && message.$diagnoses[0].$links : null //temp hack
			});
		}
	}
};