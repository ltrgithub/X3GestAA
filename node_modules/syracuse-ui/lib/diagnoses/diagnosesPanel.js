...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	ensureMessage: function(diagInfo) {
		...
		var textToHide;
		if (indexBr >= 0) {
			textToHide = labelText.substr(indexBr + "<br>".length);
			var textToShow = labelText.substr(0, indexBr);
			var diagMaxLength = 130;
			if (textToShow.length > diagMaxLength) {
				// getting understandable text from 130 first characters
				var maxLengthText = textToShow.substr(0, diagMaxLength);
		...
		if (textToHide && textToHide.match(/\S/g)) {
			diagInfo.label.innerHTML = diagInfo.label.innerHTML.replace(textToHide, "<span class='s-diagnose-msg-expanded-text'>" + textToHide + "</span>");
	...
	dispose: function() {
		...
		this.page = this.total = this.onSuccess = this.diagnoses = this._body = null;
	},
