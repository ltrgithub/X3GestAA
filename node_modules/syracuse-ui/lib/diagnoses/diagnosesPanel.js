...
helpers.defineClass(Group, null, {
    ...
    drawDiagnose: function($diagnose, options){
        ...
        if ($diagnose.$links || $diagnose.$actions) {
            self._drawDiagnoseLinks($diagnose, this.viewer.page, $$diagMsg[0]);
        }
        
        $$diagMsg[0].style.display = "none";
        self.$$group[0].appendChild($$diagMsg[0]);
    },
    _drawDiagnoseLinks: function($diagnose, page, diagContainer){
        var linksContainer = document.createElement("div");
        linksContainer.className = "s-diag-view-links-bar-" + $diagnose.severity;
        diagContainer.appendChild(linksContainer);
        page = page || this.page;
        var menus = page.loadNewItem($(linksContainer), {
            $category: "links",
...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    _removeFieldDiagnoseFromArray: function(field){
        var self = this;
        if (self._fieldDiagnoses) {
            var index;
    ...
    _updateFieldFocusHandling: function(field){
        ...
        this._removeFieldDiagnoseFromArray(field);
    ...
    emptyDiagnoseByFieldId: function(fieldId){
        ...
        // remove diagnoses
        self.$$body.find("a[data-s-menu=" + fieldId + "]").parent().parent().remove();
        
        // update diagnoses panel ui accordingly
        self._onDiagnoseRemoval();
    ...
    _cleanDiagnoses: function(field){
        var self = this;
        self.groups.empty(field);
        if (field && field.domDiagnose) {
            field.emptyDiagnoseSlot();
            self._updateFieldFocusHandling(field);
        }
    },
    dispose: function(){
        if (this.groups) {
