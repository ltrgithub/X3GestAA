...
function _appendFieldPopup(field, diags) {
	if (diags.length) {
		...
		field.popupDiagnoseDiag = document.site.dialogManager.openPopup(field.page, {
			...
			onClose: function() {
				document.site.removeDomChild(field.diagnoseFieldSlot);
...
function _appendBoxPopup(box) {
	if (box.severitiesCount) {
		...
		box.diagnosePopupDiag = document.site.dialogManager.openPopup(box.page, {
			...
			onClose: function() {
				document.site.removeDomChild(box.diagnosePopupSlot);
...
exports.onBoxItemEnterLeave = function(box, event) {
	if (box.severitiesCount) {
		switch (event.type) {
			case "mouseenter":
	...
	while (box) {
		...
		if (box.tabTitleHead || (box.header && box.$item.$isBoxCollapsable)) {
			if (add && box.prioritySeverity != prioritySeverity) {
				...
				picker.title = "";
				picker.setAttribute("data-s-inout", picker.syraBoxId = box.id);
			}
...
function _cleanField(panel, field, diags) {
	...
	var inputCss = field._input && field._input.className || "";
	...
	for (var ii = diags.length - 1; ii >= 0; ii--) {
		...
		inputCss.replace("s-" + diag.$diagnose.$severity, "");
		css.replace("s-" + diag.$diagnose.$severity, "");
	}
	if (field._input) {
		field._input.className = inputCss;
	}
	...
	if (field) {
		...
		if (!field.isDiagnoseDisabled && message.$diagnoses && message.$diagnoses.length > 0) {
			if (field.isDiagnosePopup) {
				if (!field.diagnoseFlag && !document.site.isTableDevice()) {
			...
			if (field._input) {
				document.site.toggleClass(field._input, "s-" + diagInfo.$diagnose.$severity, true);
			}
			document.site.toggleClass(field._dataValue, "s-" + diagInfo.$diagnose.$severity, true);
			if (field.diagnoseFieldSlot) {
...
exports.onFieldItemEnterLeave = function(field, event) {
...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	onNavigationClick: function(navLink, field) {
		if (_diagnoses && _diagnoses.length > 0) {
			...
			if (_diagnoseFocused) {
				document.site.toggleClass(_diagnoseFocused.topLabel, "s-diagnose-msg-focus", false);
			}
			document.site.toggleClass((_diagnoseFocused = currentDiagnose).topLabel, "s-diagnose-msg-focus", true);
