...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	appendToPage: function(page) {
		...
		this.$$item = $(this.page.diagnoseSlot.appendChild(html));
	},
	_refreshViewer: function(options, page) {
		...
		if (this._groups) {
			for (var ii = 0, jj = this._groups.length; ii < jj; ii++) {
				...
				if (count) {
					this.total += count;
					this.$$body[0].appendChild(group.$$group[0]);
				}
			}
		}
		...
		if (!message || (!message.field && !message.$diagnoses)) {
			...
			if (message.$diagnoses && message.$diagnoses.length > 0) {
				if (message.field) {
					...
					self.ensureFieldDiagnoseSlot(message.field);
				}
				var group;
					...
					if (message.field) {
						...
						if (field) { // there is already a diagnose for this field in the panel
							...
							if (field.$item.$isExpressionChild && !message.field.$item.$isExpressionChild) {
								...
								self.drawFieldDiagnose(message.field, $diagnose, group);
							} else {
								if (!field.$item.$isExpressionChild && !message.field.$item.$isExpressionChild) {
									self.drawFieldDiagnose(message.field, $diagnose, group);
								}
				...
				if (message.field) {
					...
					if (group) {
						self.toggleFieldDiagnose(message.field, "s-" + group.$severity, true);
					}
				}
			} else {
				self._cleanDiagnoses(message.field);
	...
	drawFieldDiagnose: function(field, $diagnose, severityGroup) {
		if (!field.isDiagnoseDisabled) {
			if (field.popupDiagnose || !severityGroup.$$group.children().children().children("a[data-s-menu='" + field.id + "']")[0]) {
		...
		if (field && field.diagnoseFieldSlot) {
			...
			if (field.diagSeverities) {
				for (var ii = 0; ii < field.diagSeverities.length; ii++) {
					this.toggleFieldDiagnose(field, "s-" + field.diagSeverities[ii]);
				}
			}
			if (field.diagSeverities) {
				for (var ii = 0; ii < field.diagSeverities.length; ii++) {
