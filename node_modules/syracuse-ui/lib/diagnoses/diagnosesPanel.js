...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
    ...
    createGroup: function($severity){
        ...
        return this._groupsMap[$severity] = {
            $severity: $severity,
            slot: this._body.appendChild(slot),
            diagnosesCount: 0
        };
    },
    _setPicker: function(picker, key){
        picker.setAttribute("data-s-diag-picker", key);
        if (this.isPageOwner) {
            picker.setAttribute("data-s-diag-page", this.page.id);
        }
    },
    _initialize: function(){
        ...
        this._body.className = "s-diagnose-msg-list";
        this.$$body = $(this._body);
        
        this._fieldDiagnosesCount = 0;
        this._fieldsMap = {};
        this._diagnoses = [];
        this._groupsMap = {};
            ...
            if (this._diagnoses.length > 0) {
                ...
                while (ii < this._diagnoses.length) {
                    if (!this._diagnoses[ii].field) {
                        this._cleanDiagnose(this._diagnoses[ii]);
                    }
                    else {
                        ii++;
                    }
    ...
    onClickPicker: function(picker){
        switch (picker.getAttribute("data-s-diag-picker")) {
            case "diag-expand":
    ...
    _setExpandState: function(refreshDiagnoses){
        if (refreshDiagnoses) {
            if (this.isCollapsed) {
                ...
                for (var ii = 0, jj = this._diagnoses.length; ii < jj; ii++) {
                    this.ensureMessage(this._diagnoses[ii]).style.display = "";
                }
            }
        }
        this._expand.style.display = (this.total == 1) ? "none" : "";
        if (this.isCollapsed) {
            var diagDisplay = (this.diagFocused != undefined) ? this.diagFocused : this._diagnoses[0];
            if (diagDisplay) {
    ...
    emptyDiagnoseByFieldId: function(fieldId){
        this._cleanFieldDiagnoses(fieldId);
    ...
    showDiagnoses: function(message, page, options){
        if (!message || (!message.field && !message.$diagnoses)) {
            ...
            if (!options || !options.noViewer) {
                ...
                if (this.total > 0) {
                    this._next.style.display = (this._fieldDiagnosesCount > 1) ? "" : "none";
                    this._setExpandState(true);
                    this._core.style.display = "inline-block";
    ...
    drawFieldDiagnose: function(field, $diagnose, group){
        if (!field.isDiagnoseDisabled) {
            ...
            if (field.popupDiagnose) {
                ...
                link.className = "s-field-diagnose-link s-field-diagnose-link-" + group.$severity;
            ...
            this.toggleDiagnoseIcon(field.boxParent, field.id, $diagnose.severity || $diagnose.$severity, true);
    ...
    _cleanDiagnose: function(diagInfo){
        ...
        var index = this._diagnoses.indexOf(diagInfo);
        if (index >= 0) {
    ...
    _cleanFieldDiagnoses: function(fieldId){
        ...
        if (diags && diags.length) {
            ...
            for (var ii = 0, jj = diags.length; ii < jj; ii++) {
                ...
                this._fieldDiagnosesCount--;
                severities[diagInfo.group.severity] = true;
            }
            this._fieldDiagnosesCount = Math.max(this._fieldDiagnosesCount, 0);
            if (field.diagnoseFieldSlot) {
                ...
                if (field.hasDiagnoses) {
                    ...
                    for (var ii = 0, jj = keys.length; ii < jj; ii++) {
                        this.toggleFieldDiagnose(field, "s-" + keys[ii], false);
                        this.toggleDiagnoseIcon(field.boxParent, field.id, keys[ii], false);
                    }
                }
    ...
    ensureMessage: function(diagInfo){
        ...
        var textToHide, textToInsert;
        if (indexBr >= 0) {
            textToInsert = "<br>";
            textToHide = labelText.substr(indexBr + textToInsert.length);
        }
        else {
            ...
            if (diagInfo.label.textContent.length > diagMaxLength) {
                ...
                var maxLengthText = diagInfo.label.textContent.substr(0, diagMaxLength);
                ...
                // wrap text to hide
                textToHide = labelText.substr(labelText.lastIndexOf(maxLengthText) + maxLengthText.length);
                textToInsert = "";
            }
        }
        if (textToHide && textToHide.match(/\S/g)) {
            diagInfo.label.innerHTML = diagInfo.label.innerHTML.replace(textToHide, textToInsert + "<span class='s-diagnose-msg-expanded-text'>" + textToHide + "</span>");
    ...
    dispose: function(){
        ...
        this.page = this.total = this.onSuccess = this._diagnoses = this._body = null;
    },
    refreshBoxDiagnoseIcon: function(box, prioritySeverity, add){
        if (true || box.isTabLayout || box.$item.$isBoxCollapsable) {
            if (box.tabTitleHead || box._header) {
    ...
    toggleDiagnoseIcon: function(box, fieldId, severity, add){
        if (box) {
            if (add) {
                ...
                var fieldSeverities = box._diagnosesFields[fieldId];
                if (!fieldSeverities) {
                    fieldSeverities = box._diagnosesFields[fieldId] = {};
                }
                ...
                if (box._diagnosesFields) {
                    if (fieldId === undefined) {
                        ...
                        for (var ii = 0, jj = fieldIds.length; ii < jj; ii++) {
                            ...
                            this.toggleDiagnoseIcon(box, fieldIds[ii], null, false);
                        }
                    ...
                    if (box._diagnosesFields) {
                        ...
                        if (fieldSeverities) {
                            if (severity !== null) {
                                ...
                                if (fieldSeverities.fatal) {
                                    fieldSeverities.fatal--;
                                    box.severitiesCount.fatal--;
                                }
                                if (fieldSeverities.error) {
                                    fieldSeverities.error--;
                                    box.severitiesCount.error--;
                                }
                                if (fieldSeverities.warning) {
                                    fieldSeverities.warning--;
                                    box.severitiesCount.warning--;
                                }
                                if (fieldSeverities.info) {
                                    fieldSeverities.info--;
                                    box.severitiesCount.info--;
                                }
                            }
                        }
                    }
            ...
            if (box._diagnosesFields) {
                if (box.severitiesCount.fatal) {
                    this.refreshBoxDiagnoseIcon(box, "fatal", add);
                }
                else {
                    if (box.severitiesCount.error) {
                        this.refreshBoxDiagnoseIcon(box, "error", add);
                    }
                    else {
                        if (box.severitiesCount.warning) {
                            this.refreshBoxDiagnoseIcon(box, "warning", add);
                        }
                        else {
                            if (box.severitiesCount.info) {
                                this.refreshBoxDiagnoseIcon(box, "info", add);
                            }
                        }
                    }
                }
                if (box.boxParent) {
                    this.toggleDiagnoseIcon(box.boxParent, fieldId, severity, add);
                }
            }
        }
    }
