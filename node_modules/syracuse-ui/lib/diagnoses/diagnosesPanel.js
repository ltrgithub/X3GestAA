...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	appendToPage: function(page) {
		...
		this.$$item = $(this.page.diagnoseSlot.appendChild(this.domItem));
	},
	createGroup: function($severity) {
		var slot = document.createElement("div");
		slot.className = 's-diagnose-group-' + $severity;
		slot.className += ' s-diagnose-group';
	...
	_initialize: function() {
		...
		this.$$core = $(this.domItem.appendChild(this._core));
	...
	cleanGridCardAlertDiagnose: function(gridRecordId) {
		if (this.body) {
			if (this.diagnoses.length > 0) {
				var ii = 0;
				while (ii < this.diagnoses.length) {
					if (!this.diagnoses[ii].field && this.diagnoses[ii].label.children[0].getAttribute("data-s-diag-field") == gridRecordId) {
	...
	onMessageClick: function(messageLink) {
		...
		if (id) {
			...
			if (messageLink.children[1] && messageLink.children[1].className == "s-diagnose-msg-hide") {
				...
				var diagPopup = document.site.dialogManager.openBoxDialog(self.page, {
					content: msgPopup,
					$dialogMode: "popup",
	...
	onClickPicker: function(picker) {
		switch (picker.syraDiagPicker) {
			case "diag-expand":
				...
				break;
			case "field-msg":
				var article = document.site.findArticle(picker);
				if (article) {
			...
			case "grid-card-diag":
				var article = document.site.findArticle(picker);
				if (article) {
					var gridRecordId = picker.getAttribute("data-s-diag-field");
					var gridRecord = article.idMap[gridRecordId];
					if (gridRecord) {
						// scroll to gridrecord
						gridRecord.list.scroller.scrollToItem(gridRecord.dataRow);
						// open card view
						if (gridRecord.rowCardOpener) {
							gridRecord.rowCardOpener.click();
						}
						// remove diagnose
						this.cleanGridCardAlertDiagnose(gridRecordId);
					}
				}
				break;
		}
	...
	_setExpandState: function(refreshDiagnoses) {
		...
		if (this.isCollapsed) {
			var diagDisplay = (this.diagFocused != undefined) ? this.diagFocused : this.diagnoses[this.diagnoses.length - 1];
			if (diagDisplay) {
	...
	drawLinks: function(diagObj, slot, message, page) {
		page = page || this.page;
		if (!diagObj._menus || diagObj._menus.isDisposed) {
			diagObj._menus = page.loadNewItem(slot, {
				$isAuthoringEnabled: false,
				$category: "menus",
				$isOwner: true,
				$skin: "s-diagnose-links"
			});
			diagObj._menus.isDiagnoseMenus = true;
		}
		diagObj._menus.ensurePrivateMenus(message);
	},
	...
	showDiagnoses: function(message, page, options) {
		if (!message || (!message.field && !message.$diagnoses)) {
			...
			if (message.$links || message.$actions) {
				this.drawLinks(this, this._diagViewLinks, message, page);
			}
			if (message.field) {
			...
			if (message.$diagnoses && message.$diagnoses.length > 0) {
				...
				for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
					...
					if (message.field) {
						...
						// in case of diagnose in inline grid card
						if (options && options.gridCardRecord) {
							var link = document.createElement("a");
							link.className = "s-diagnose-msg-link";
							options.gridCardRecord.list.setArticleId(link);
							link.setAttribute("data-s-diag-field", options.gridCardRecord.id);
							link.textContent = options.gridCardRecord.list.localize.diag_in_card_link.replace("{gridTitle}", options.gridCardRecord.list.titleText).replace("{gridIndex}", options.gridCardRecord.$serverIndex + 1);
							this._setPicker(link, "grid-card-diag");
							this.appendDiagnose(group, $diagnose, undefined, link);
							delete options.gridCardRecord;
						} else {
			...
			if (!options || !options.noViewer) {
				...
				if (this.total > 0) {
					...
					this._onLastDiagDisplay();
					document.site.setZIndexValue(this.domItem, document.site._zIndex - 1);
					document.site.setZIndexValue(this._close, document.site._zIndex - 1);
					this._opener.style.display = "none";
				}
				// 0 diagnose => clean
				else {
	...
	drawFieldDiagnose: function(field, $diagnose, group) {
		if (!field.isDiagnoseDisabled) {
			...
			field.articleParent.setArticleId(link);
			...
			if (field.popupDiagnose) {
				...
				if (!field.getTitle() && !field.$field.$title && field.arrayLevel && field.arrayLevel == "cell") {
					...
					link.textContent = (field.getTitle() || field.$field.$title || field.$item.$bind) + ":";
				}
			}
	...
	ensureMessage: function(diagInfo) {
		...
		if (diagInfo.$diagnose.$links || diagInfo.$diagnose.$actions) {
			...
			this.drawLinks(diagInfo, diagMsg.appendChild(container), diagInfo.$diagnose, this.page, diagMsg);
		}
		...
		diagInfo.group.slot.insertBefore(diagMsg, diagInfo.group.slot.firstChild);
	...
	appendDiagnose: function(group, $diagnose, field, fieldLink) {
		...
		// add link for gridCard case
		// add grid card property
		if (!field && fieldLink) {
			label.insertBefore(fieldLink, label.firstChild);
		}
	...
	dispose: function() {
		...
		this.$$core = this.$$diagViewLinks = this.$$item = null;
		...
		this.page = this.total = this.diagnoses = this.body = null;
	},
