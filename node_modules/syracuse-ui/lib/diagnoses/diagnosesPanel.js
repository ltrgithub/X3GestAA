...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	_initialize: function() {
		...
		this._close.setAttribute("data-s-picker", "diag-close");
		...
		this._next.setAttribute("data-s-picker", "next");
		...
		this._expand.setAttribute("data-s-picker", "diag-expand");
		...
		this._opener.setAttribute("data-s-picker", "diag-opener");
				...
				self.$$item.delegate("a[data-s-picker]", "click", function() {
					switch (this.getAttribute("data-s-picker")) {
						...
						case "diag-close":
							...
							break;
						case "diag-opener":
							if (self._opener && self._core) {
								self._core.style.display = "inline-block";
								self._opener.style.display = "none";
								self.page.onWindowResize();
							}
							break;
	...
	_setExpandState: function(refreshDiagnoses) {
		if (refreshDiagnoses) {
			if (this.isCollapsed) {
				...
				for (var ii = 0, jj = this._diagnoses.length; ii < jj; ii++) {
					this.drawMessage(this._diagnoses[ii]).style.display = "";
				}
			}
		}
		...
		if (this.isCollapsed) {
			...
			if (diagDisplay) {
				this.drawMessage(diagDisplay).style.display = "";
			}
			...
			if (field.diagnoseFieldSlot) {
				...
				if (field.hasDiagnoses) {
					...
					for (var ii = 0, jj = keys.length; ii < jj; ii++) {
						...
						field.boxParent.toggleDiagnoseIcon(field.id, keys[ii], false);
					}
				}
