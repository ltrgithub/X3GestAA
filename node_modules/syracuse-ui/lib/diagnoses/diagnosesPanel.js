	...
	refresh: function() {
		...
		if ((self.page == syra_site) || (self.page.isPageLoaded && !self.page._isDataChanging)) {
			...
			if (count) {
				...
				if (!self._expandBtn) {
					self._expandBtn = syra_menus.addIconButton(syra_local.diag_expand, "s-diagnose-expander", "onExpand");
					self._expandBtn.style.display = "none";
					self._expandBtn.syraItem = self.id;
					self.page.diagnoseSlot.appendChild(self._expandBtn);
				}
				syra_site.dom.display(self._expandBtn, count > 1);
				if (self.isCollapsed) {
					var item = self._focused || self.map[self.body.lastChild.syraId];
					item && item.showMessage(true);
					self._expandBtn.textContent = (count - 1) + syra_local.diag_expand;
				}
				self._expandBtn.textContent = self.isCollapsed ? ((count - 1) + " " + syra_local.diag_expand) : syra_local.diag_collapse;
