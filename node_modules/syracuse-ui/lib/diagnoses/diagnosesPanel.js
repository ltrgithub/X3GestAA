...
function _onOpenDetail() {
	...
	item._trace && syra_site.dom.hide(item._trace, !item.isExpanded);
	item.page.resizeArticle();
}
...
exports.DiagnoseItem = helpers.defineClass(DiagnoseItem, null, {
	showMessage: function(show) {
		syra_menus.button.hide(this.msgItem, !show);
	...
	addTopLabel: function() {
		...
		this.msgItem = syra_menus.button.add({
			isIndicator: true,
			parent: this,
		...
		if (this.$diagnose.$stackTrace) {
			...
			this._trace.innerHTML = syra_site.dom.textToHTML(this.$diagnose.$stackTrace, true);
			syra_site.dom.hide(this._trace, true);
	...
	clean: function() {
		if (this.msgItem) {
			syra_site.dom.removeChild(this.msgItem.link);
	...
	focus: function() {
		if (this.panel._focused) {
			syra_menus.button.remove(this.panel._focused.focuseFlag);
			delete this.panel._focused.focuseFlag;
		}
		this.panel._focused = this;
		if (!this.focuseFlag) {
			this.focuseFlag = syra_menus.button.add({
				isIndicator: true,
				parent: this,
				iconOnly: true,
...
helpers.defineClass(DiagnosesPanel, null, {
	...
	load: function(page) {
		...
		this._closeBtn = syra_menus.button.add({
			parent: this,
	...
	clean: function(closing) {
		if (this.domItem) {
			syra_menus.button.hide(this._expandBtn, true);
			syra_menus.button.hide(this._nextBtn, true);
			syra_menus.button.hide(this._closeBtn, true);
			syra_site.dom.hide(this.domItem, true);
	...
	showDiagnoses: function(message, page, options) {
		if (!message || (!message.$bind && !message.field && !message.$diagnoses)) {
			...
			if (message.field && (message.field.diagsSlot || message.field.picker_diagnose || (message.$diagnoses === null))) {
			...
			if (syra_site.landingPage && syra_site.landingPage.isPageLoaded && syra_site.dom.isVisible(this.domItem)) {
				syra_site.landingPage.resizeArticle();
			}
		}
	},
		...
		if ((self.page == syra_site) || (self.page.isPageLoaded && !self.page._isDataChanging)) {
			...
			if (count) {
				if (showNext && !self._nextBtn) {
					self._nextBtn = syra_menus.button.add({
						parent: self,
				...
				if (!self._expandBtn) {
					self._expandBtn = syra_menus.button.add({
						parent: self,
						isExpandBtn: true,
						slot: self.page.diagnoseSlot,
						...
						btnclick: function() {
							...
							panel.refresh();
							panel.page.resizeArticle();
						}
					});
				}
				syra_menus.button.hide(self._expandBtn, count <= 1);
				if (self.isCollapsed) {
					var item = self._focused || syra_menus.button.getButton(self.body.lastChild.syraBtn).parent;
				...
				self._nextBtn && syra_site.dom.setZIndex(self._nextBtn.link, false, zIndex);
				self._expandBtn && syra_site.dom.setZIndex(self._expandBtn.link, false, zIndex);
				self._closeBtn && syra_site.dom.setZIndex(self._closeBtn.link, false, zIndex);
				syra_site.dom.setZIndex(self.domItem, false, zIndex);
			} else {
				syra_menus.button.hide(self._expandBtn, true);
			}
			syra_menus.button.hide(self._nextBtn, !showNext);
			syra_menus.button.hide(self._closeBtn, count <= 0);
			syra_site.dom.hide(self.domItem, count <= 0);
			if (self.autoHide) {
				setTimeout(function() {
					self.clean();
