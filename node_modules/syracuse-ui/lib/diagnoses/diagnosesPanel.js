...
function _appendTopLabel(panel, diagInfo) {
	...
	if (diagInfo.$diagnose.$stackTrace) {
		diagInfo.$diagnose.$stackTrace = syra_site.formatHTMLText(diagInfo.$diagnose.$stackTrace);
		diagInfo.stackTrace = document.createElement("div");
		diagInfo.stackTrace.className = "s-diagnose-stacktrace";
		diagInfo.stackTrace.innerHTML = diagInfo.$diagnose.$stackTrace = syra_site.formatHTMLText(diagInfo.$diagnose.$stackTrace);
...
function _drawLinks(panel, diagInfo, slot, message, page) {
	...
	if (!diagInfo._menus || diagInfo._menus.disposed) {
	...
	syra_menus.setMenus(page, message);
}
...
function _appendFieldPopup(field, diags) {
	if (diags.length) {
		...
		field.popupDiagnoseDiag = syra_site.dialogManager.openPopup(field.page, {
			content: {},
			...
			onClose: function() {
				syra_site.dom.removeChild(field.diagnoseFieldSlot);
	...
	if (box.severitiesCount) {
		...
		for (var ii = 0, jj = severities.length; ii < jj; ii++) {
			...
			if (box.severitiesCount[severity]) {
				...
				label.textContent = box.severitiesCount[severity] + " " + syra_local["box_" + severity];
...
function _cleanField(panel, field, diags) {
	...
	syra_menus.diagnose.remove(field);
	...
	if (field) {
		...
		if (!field.isDiagnoseDisabled && message.$diagnoses && message.$diagnoses.length > 0) {
			if (field.isDiagnosePopup) {
				syra_menus.diagnose.add(field, message);
			} else {
				field.diagnoseFieldSlot = document.createElement("div");
			...
			for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
				...
				topLink.syraOnClick = "goToField";
				field.articleParent.setArticleId(topLink);
				topLink.syraFieldId = field.id;
...
function _showMessage(diagInfo, show) {
	...
	if (show && !diagInfo.isDetailChecked) {
		...
		if (diagInfo.$diagnose.$stackTrace || diagInfo.topLabel.scrollHeight > diagInfo.topLabel.clientHeight) {
			...
			diagInfo.detailLink.syraOnClick = "onOpenDetail";
			diagInfo.detailLink.textContent = syra_local.diag_stacktrace;
...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	appendToPage: function(page) {
		this.page = page;
		syra_map[this.id = (page.id + "diagnosepanel")] = this;
	...
	_initialize: function() {
		...
		this._close.syraOnClick = "onClose";
		...
		this._next.syraOnClick = "onNavigationClick	";
		this._next.style.display = "none";
		this._next.title = syra_local.diag_next_tooltip;
		this._next.textContent = syra_local.diag_next;
		...
		this._expand.syraOnClick = "onExpand";
		...
		this._opener.syraOnClick = "onOpen";
		this._opener.textContent = syra_local.diag_opener;
	...
	onFieldItemInOut: function(field, onEnter) {
		var diags = this._fieldsMap[field.id];
	...
	onMessageClick: function(event, picker) {
		var self = this;
		var id = picker.getAttribute("data-s-diagmsg-popup");
		if (id) {
			if (self.diagPopups && self.diagPopups[id]) {
				self.diagPopups[id].close();
			}
		} else {
			var label = picker.children[0];
			if (picker.children[1] && picker.children[1].className == "s-diagnose-msg-hide") {
				var msgPopup = new DiagnoseMessagePopup();
				msgPopup.loadBox(label.innerHTML, picker.getAttribute("data-s-severity"));
				var diagPopup = syra_site.dialogManager.openPopup(self.page, {
					...
					onClose: function() {
						msgPopup.dispose();
						picker.removeAttribute("data-s-diagmsg-popup");
						self.diagPopups[this.id] = null;
					},
					$isAutoClose: false
				});
				diagPopup.appendCloseButton(msgPopup.header);
				picker.setAttribute("data-s-diagmsg-popup", diagPopup.id);
	...
	goToField: function(event, picker) {
		var field = syra_map[picker.syraFieldId];
		if (field) {
	...
	onOpenDetail: function(event, picker) {
	...
	onNavigationClick: function(event, picker, field) {
		if (_diagnoses && _diagnoses.length > 0) {
			var currentDiagnose = _diagnoseFocused;
			if (!picker && field && this._fieldsMap[field.id]) {
			...
			if (_diagnoseFocused) {
				syra_site.dom.toggleClass(_diagnoseFocused.topLabel, "s-diagnose-msg-focus", false);
			}
			syra_site.dom.toggleClass((_diagnoseFocused = currentDiagnose).topLabel, "s-diagnose-msg-focus", true);
			if (_diagnoseFocused.msgItem) {
				syra_site.scrollToItem(_diagnoseFocused.msgItem, this._content, true);
			}
	...
	showDiagnoses: function(message, page, options) {
		if (!message || (!message.$bind && !message.field && !message.$diagnoses)) {
			...
			if (message.field && (message.field.diagnoseFieldSlot || message.field.mnPickers.diagnose)) {
				_cleanField(this, message.field, this._fieldsMap[message.field.id]);
			...
			if (message.$diagnoses) {
				for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
					...
					if ($diagnose.$message) {
						$diagnose.$message = syra_site.formatHTMLText($diagnose.$message);
					}
				}
			}
	...
	dispose: function() {
		...
		this._fieldsMap = this._close = this._content = this._core = this._diagViewLinks = this._expand = null;
		this.domItem = this._next = this._opener = this.page = this.body = null;
	}
});