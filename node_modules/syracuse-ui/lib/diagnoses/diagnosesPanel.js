...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	showDiagnosesPanel: function() {
		...
		var fieldDiag = this.$$body.find("a.s-diagnose-msg-link");
		this._nextLink.style.display = (this._fieldDiagnoses.length > 1) ? "" : "none";

		// diagnoses in an array 
		var diagMsg = this.$$body.find("div.s-diagnose-msg");
	...
	_applySuccessStyle: function() {
		...
		this._core.className = "s-diagnose-core-success";
		//this.$$core.find(".s-diagnose-links-body")[0].className = "s-diagnose-links-body-success";
		//this.$$core.find(".s-diagnose-links")[0].className = "s-diagnose-links-success";
		this._diagViewLinks.className = "s-diagnose-links-bar-success";
		var listArray = this.$$diagViewLinks.find(".s-diagnose-links-link-slot");
		var listArrayMain = this.$$diagViewLinks.find(".s-diagnose-links-link-slot");
		var anchorArray = this.$$diagViewLinks.find("a.s-diagnose-links-link");
		var anchorArrayMain = this.$$diagViewLinks.find("a.s-diagnose-links-main-link");

		if (listArray[0]) {
			for (var ii = 0; ii < listArray.length; ii++) {
				listArray[ii].className = "s-diagnose-links-link-slot"; //  "s-diagnose-links-link-slot-success";
			}
		} else if (listArrayMain[0]) {
			for (var ii = 0; ii < listArrayMain.length; ii++) {
				listArrayMain[ii].className = "s-diagnose-links-link-slot" //"s-diagnose-links-link-slot-success";
			}
		}

		if (anchorArray[0]) {
			for (var ii = 0; ii < anchorArray.length; ii++) {
				anchorArray[ii].className = "s-diagnose-links-link"; // "s-diagnose-links-link-success";
			}
		} else if (anchorArrayMain[0]) {
			for (var ii = 0; ii < anchorArray.length; ii++) {
				anchorArray[ii].className = "s-diagnose-links-main-link-success"; // "s-diagnose-links-main-link-success";
			}
		}
		this.page.onWindowResize();
	},
				...
				self.$$item.delegate("a[data-s-picker]", "click", function() {
					switch (this.getAttribute("data-s-picker")) {
						...
						case "diag-expand":
							...
							break;
						case "detail":
							$(this).parent().find("div.s-diagnose-stacktrace").toggle();
					...
					return false;
				}).delegate("div.s-diagnose-msg", "mouseenter", function() {
					var hide = $(this).find("div.s-diagnose-msg-hide");
					if (hide.length > 0) {
						var $$label = $(this).find("label");
						self._openMessagePopup(this, $$label);
					}
				}).delegate("div.s-diagnose-msg", "mouseleave", function() {
	...
	_onDiagExpand: function() {
		...
		var diagFocused;
		for (var ii = 0; ii < diagMsg.length; ii++) {
			if (diagMsg[ii].children[0].className.indexOf("s-diagnose-msg-focus") > -1) {
				diagFocused = ii;
			}
		}
	...
	onNavigationClick: function(navLink, field) {
		...
		if (this._fieldDiagnoses && this._fieldDiagnoses.length > 0) {
			...
			for (var ii = 0; ii < this._fieldDiagnoses.length; ii++) {
				if (this._fieldDiagnoses[ii].label.className.indexOf("s-diagnose-msg-focus") > -1) {
					index = ii;
					break;
				}
			}
			...
			for (var ii = 0; ii < this._fieldDiagnoses.length; ii++) {
				if (ii == index) {
					document.site.toggleClass(this._fieldDiagnoses[index].label, "s-diagnose-msg-focus", true);
				} else {
					document.site.toggleClass(this._fieldDiagnoses[ii].label, "s-diagnose-msg-focus");
				}
			}
			...
			if (this._expandLink.textContent.indexOf(this.localize.diag_expand) > -1) {
				// getting all diagnoses in diagnose panel
				var diagMsg = [];
				diagMsg = this.$$body.find("div.s-diagnose-msg");
				for (var ii = 0; ii < diagMsg.length; ii++) {
					diagMsg[ii].style.display = "none";
				}
	...
	_getFieldWithBind: function($bind) {
		...
		if (self._fieldDiagnoses) {
			for (var ii = 0, jj = self._fieldDiagnoses.length; ii < jj; ii++) {
				index = self._fieldDiagnoses[ii].field.$item.$bind == $bind ? ii : index;
			}
		}
		return index != undefined ? self._fieldDiagnoses[index].field : index;
	},
	cleanGlobalDiagnoses: function() {
		if (this.$$body) {
			...
			if (diagMsg.length > 0) {
				var globalDiag = [];
				for (var ii = 0; ii < diagMsg.length; ii++) {
					if ($(diagMsg[ii]).find("a.s-diagnose-msg-link").length < 1) {
