...
function _onOpenDetail() {
	...
	item._trace && syra_site.dom.display(item._trace, item.isExpanded);
...
exports.DiagnoseItem = helpers.defineClass(DiagnoseItem, null, {
	showMessage: function(show) {
		syra_menus.button.display(this.msgItem, show);
	...
	addTopLabel: function() {
		...
		this.msgItem = syra_menus.button.addIndicator({
			parent: this,
	...
	focus: function() {
		...
		if (!this.focuseFlag) {
			this.focuseFlag = syra_menus.button.addIndicator({
				parent: this,
				iconOnly: true,
		...
		this.msgItem && syra_page.scrollToItem(syra_site, this.msgItem.link, this.panel.domItem, true);
	}
});
...
helpers.defineClass(DiagnosesPanel, null, {
				...
				if (!box.diagnosePopupSlot) {
					...
					for (var ii = 0, jj = severities.length; ii < jj; ii++) {
						...
						if (box.severitiesCount[severity]) {
							syra_menus.button.addIndicator({
								parent: box.page,
	...
	clean: function(closing) {
		if (this.domItem) {
			syra_menus.button.display(this._expandBtn, false);
			syra_menus.button.display(this._nextBtn, false);
			syra_menus.button.display(this._closeBtn, false);
	...
	refresh: function() {
		...
		if ((self.page == syra_site) || (self.page.isPageLoaded && !self.page._isDataChanging)) {
			...
			if (count) {
				...
				syra_menus.button.display(self._expandBtn, count > 1);
				if (self.isCollapsed) {
				...
				syra_menus.button.display(self._expandBtn, false);
			}
			syra_menus.button.display(self._nextBtn, showNext);
			syra_menus.button.display(self._closeBtn, count > 0);
			syra_site.dom.display(self.domItem, count > 0);
			if (self.autoHide) {
				setTimeout(function() {
					self.clean();
