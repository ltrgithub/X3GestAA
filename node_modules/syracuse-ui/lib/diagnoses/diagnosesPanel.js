...
exports.DiagnosesPanel = helpers.defineClass(DiagnosesPanel, null, {
	...
	_onLastDiagDisplay: function() {
		...
		if (this.lastDiagAppended) {
			...
			if (document.site.isEllipsisActive(this.lastDiagAppended.label) && !this.lastDiagAppended.diagInfo.$hasExpander) {
			...
			if (message.$diagnoses && message.$diagnoses.length > 0) {
				...
				for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
					...
					if (message.field) {
						...
						var record = options && options.gridCardRecord;
						if (record) {
							var link = document.createElement("a");
							link.className = "s-diagnose-msg-link";
							record.list.setArticleId(link);
							link.setAttribute("data-s-diag-field", record.id);
							link.textContent = record.list.localize.diag_in_card_link.replace("{gridTitle}", record.list.getTitle()).replace("{gridIndex}", record.$serverIndex + 1);
