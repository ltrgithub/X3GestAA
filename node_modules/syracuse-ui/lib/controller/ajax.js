...
function Survey() {
	...
	function _show() {
		_count = Math.max(_count || 0, 0);
		syra_quality.logRequestCount(_count);
		if (_flag) {
			_flag.textContent = _count;
			syra_dom.hide(_flag, _disabled || _count <= 0);
		}
	}
	this.addFlag = function(slot) {
		_flag = document.createElement("div");
		_flag.id = "s-ajax-survey";
		slot.appendChild(_flag);
	};
	this.disable = function(disabled) {
		_disabled = disabled;
		if (_flag) {
			if (disabled) {
				syra_dom.hide(_flag, true);
			} else {
				_show();
			}
		}
...
var _wcQueue = {
	push: function(options) {
		...
		if (page && options.$etag && options.method == "PUT") {
			var previous = (page.wcQueue = page.wcQueue || {})[options.$url];
			page.wcQueue[options.$url] = options;
			return previous;
		}
	},
	pop: function(options, data) {
		var page = options.page;
		if (options.$etag && page && !page.disposed && page.wcQueue) {
			var queue = page.wcQueue[options.$url];
			delete page.wcQueue[options.$url];
			if (queue && queue != options) {
				queue.data.$etag = queue.$etag = data.$etag;
				exports.send(queue);
				return;
			}
		}
...
function Ajax(options) {
	...
	this.urlSeg = syra_url.parse(this.$url = options.$url);
	...
	if (!(this.options.headers && this.options.headers["accept-language"])) {
		...
		if (language) {
			...
			if (_ajaxLanguage != language) {
				//set default language for all ajax queries. Will be improved in react
				$.ajaxSetup({
					headers: {
						"Accept-Language": _ajaxLanguage = language
					}
				});
			}
		}
	...
	if (this.options.params && this.$url) {
...
Ajax.prototype._send = function(nbTry) {
	var self = this;
	self.$url = syra_url.history.ensureProxy(self.$url);
	exports.survey.send();

	self.lock && syra_locker.lock();
	...
	self.options.ajaxQuery = $.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			self.lock && syra_locker.unlock();
			if (!_wcQueue.pop(self.options, data)) {
		...
		error: function(jqXHR, textStatus, errorThrown) {
			self.lock && syra_locker.unlock();
			_wcQueue.remove(self.options);
			var response = _parseResponse(jqXHR, null);
			if (jqXHR.status === 401) {
				// read www-authenticate header to extract method
				var auth = response.headers && response.headers["www-authenticate"] || "";
				var mt = ((auth.split(" ")[0]) || "").toLowerCase();
				if (mt) {
					var dataObj = response.data && response.headers["content-type"] && response.headers["content-type"] === "application/json" ? JSON.parse(response.data) : {};
			...
			// check before normal response management if we can retry
			switch (jqXHR.status) {
				// retry error
				case 408:
				case 502:
			...
			var $diagnoses = _getDiagnoses(response);
			if (!self.options.error || self.options.error(response, ajax, $diagnoses)) {
				var ajax = this;
				switch (jqXHR.status) {
					case 0:
...
exports.put = function(options) {
	options.method = "PUT";
	if (options.data && options.data.$etag) {
		options.$etag = options.data.$etag;
	}
	exports.send(options);
};
...
exports.send = function(options) {
	...
	if (ajax.$url) {
		if (_wcQueue.push(options)) {
			return;
		}
		ajax._send(syra_site.userProfile.getRetryOnError() - 1);
	}
};
				...
				if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304 || xhr.status == 1223) {
					...
					if (options.error) {
						...
						if (syra_site.mainPage && !syra_site.mainPage.disposed) {
							...
							syra_alert.modal({
								$message: syra_local["http_" + error.status] || syra_local.default_err_msg,
								$title: syra_local["http_" + error.status + "_title"] || syra_local.siteMsgboxTitle,
								$type: "fatal",
								$buttons: "ok",
								$diagnoses: $diagnoses
							});
