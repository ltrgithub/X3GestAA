...
var _formatter = require('syracuse-ui/lib/field/helpers/format');
			...
			if (!field.onInputValueChanged || field.onInputValueChanged(event)) {
				...
				if (newValue != field.currentValue) {
					...
					if (field.$field.$x3Format) {
						newValue = _formatter.format(newValue, field.$field.$x3Format, field.$field.$type, field.page.$prototype.$x3FormatExt);
					}
					exports.notifyFieldChange(field, newValue);
				}
			}
...
exports.onBeforeUnloadPage = function(callback) {
	callback && callback();
	return; //temp disabling
	var page = syra_site.activeWorkingCopy;
	if (page && _storage.hasData(page)) {
		syra_diagnose.box.show({
			$title: page.getTitle() || "Question", //"syra_local.aw_updateMessageTitle,
...
function _onAfterActionMenuExecute(menuItem, $menu, isSuccess) {
	...
	if (page.inlinePageHost) {
		...
		if (isSaved || menuItem.$sourceBind == "$abort") {
			if (isSaved) {
		...
		if (isSaved && isSuccess) {
			if (page.dialogWrapper) {
				...
				if ($redirect) {
					...
					(_lastSavingDiagnoses = {})[$url] = page.$menus.$save.$diagnoses;
...
exports.onActionExecuted = function(menu, $menu) {
	...
	if (menu.articleParent == menu.page) {
		var res = menu.page.onAfterActionMenuExecute ? menu.page.onAfterActionMenuExecute(menu, $menu, isSuccess) : _onAfterActionMenuExecute(menu, $menu, isSuccess);
		if (res === false) {
