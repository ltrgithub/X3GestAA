...
exports.profile = {
	get: function(key, defaultValues, target) {
		...
		if (values === undefined && syra_site.$sitePreferences) {
			values = syra_site.$sitePreferences[key];
		}
		values = values || {};
		if (defaultValues) {
	...
	save: function(key, source, props) {
		...
		//clear old status
		if (values.desktop) {
			delete values.desktop.navigationpage;
		}
		if (values.tablet) {
			delete values.tablet.navigationpage;
		}
