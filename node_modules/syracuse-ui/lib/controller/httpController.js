	...
	if (syra_site.mainPage && !syra_site.mainPage.disposed) {
		syra_site.showDiagnoses({
			$diagnoses: (error.data.indexOf("$diagnoses") >= 0) ? JSON.parse(error.data).$diagnoses : [{
				$message: error.data ? error.data : syra_site.localize.http_empty_error
			}]
		});
	} else {
		//debugger; //TODO
		// messagebox
		var options = {
			$message: syra_site.localize["http_" + error.status] ? syra_site.localize["http_" + error.status] : syra_site.localize.default_err_msg,
			$title: syra_site.localize["http_" + error.status + "_title"] ? syra_site.localize["http_" + error.status + "_title"] : syra_site.localize.siteMsgboxTitle,
			$type: "fatal",
			$buttons: "ok",
			...
			error: function(jqXHR, textStatus, errorThrown) {
				...
				if (jqXHR.status === 401) {
					...
					if (mt) {
						...
						if (next) window.location.href = (next || "/index.html") + "?callbackUrl=" + encodeURIComponent(window.location.href);
					}
				}
					...
					switch (jqXHR.status) {
								...
								if (dataObj.$actions.$logout && dataObj.$actions.$logout.$isRequested) {
									...
									if (dataObj.$actions.$save && dataObj.$actions.$save.$isRequested) {
										...
										if (dataObj.$url && dataObj.$url.indexOf("userProfile") != -1) {
											if (sessionStorage && syra_site.userProfile) {
												var key = "userProfile";
						...
						case 412:
							if (options.page && options.page.sendBag) {
								...
								reqOptions = {
									$message: syra_site.localize.http_412_message,
									$title: syra_site.localize.http_412_title,
									$type: "alert",
									//$buttons: "yesno",
								...
								syra_site.showMessage(reqOptions);
							}
							break;
						default:
							self.showError(error);
