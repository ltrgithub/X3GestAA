...
helpers.defineClass(HttpQuery, null, {
	...
	replaceEndpoint: function(newEndpoint) {
		var parts = this.$url.split("/");
		for (var ii = 0, jj = parts.length; ii < jj; ii++) {
			switch (parts[ii].toLowerCase()) {
				case "sdata":
				case "trans":
					parts[ii + 1] = newEndpoint.application || "";
					parts[ii + 2] = newEndpoint.contract || "";
					parts[ii + 3] = newEndpoint.dataset || "";
					return (this.$url = parts.join("/"));
			}
		}
		return this.$url;
	},
	parse: function($url) {
		...
		for (var ii = 0, jj = params.length; ii < jj; ii++) {
			...
			this.$urlParts.params[hash[0]] = decodeURIComponent(hash[1]);
		}
		if (!(this.$urlParts.endpoint = this.$urlParts.params.endpoint)) {
			var sdataIndex = $url.search(/\/sdata\//i);
			if (sdataIndex >= 0) {
				this.$urlParts.endpoint = $url.slice(sdataIndex + ("/sdata/").length).split("/").slice(0, 3).join(".");
			} else {
				var x3Index = $url.search(/\/trans\//i);
				if (x3Index >= 0) {
					this.$urlParts.endpoint = $url.slice(x3Index + ("/trans/").length).split("/").slice(0, 3).join(".");
					this.$urlParts.target = "trans";
					this.$urlParts.isFusion = true;
				} else {
					this.$urlParts.endpoint = document.site.userProfile ? document.site.userProfile.getSelectedEndpointString() : null;
				}
			}
		}
		...
		// this.$urlParts.endpoint = this.$urlParts.endpoint || "syracuse.collaboration.syracuse";
		this.$urlParts.endpoint = this.$urlParts.endpoint;
		//var endpointParts = this.$urlParts.endpoint.split(".");
		var endpointParts = (this.$urlParts.endpoint || "syracuse.collaboration.syracuse").split(".");
		//
		if (endpointParts.length == 3) {
			this.$urlParts.endpointParts = {
		...
		if (this.$urlParts.params.representation) {
			var result = this.$urlParts.params.representation.split(".");
			this.$urlParts.representationRoot = result[0];
			this.$urlParts.$facet = result[1];
		}
	},
	applyParameters: function(params) {
		var paramObj = {};
		var paramString, key, ii, jj;
		var paramsStart = this.$url.lastIndexOf("?");
		if (paramsStart >= 0) {
			var parts = this.$url.slice(paramsStart + 1).split("&");
