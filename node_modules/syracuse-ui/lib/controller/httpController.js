...
function _onReceiveError(error) {
	if (syra_site.mainPage && !syra_site.mainPage.disposed) {
		...
		syra_diagnose.box.show(options);
	}
}
...
function _prepareRequest(httpQuery, options) {
	httpQuery.headers = httpQuery.headers || {};
	if (options && options.headers) {
		for (var prop in options.headers) {
			httpQuery.headers[prop] = options.headers[prop];
		}
	}
	httpQuery.method = options.method || "GET";
	/*if (options.$acceptType) {
     httpQuery.headers.Accept = options.$acceptType || "application/json;vnd.sage=syracuse";
     }*/
	httpQuery.headers.Accept = options.$acceptType || "application/json;vnd.sage=syracuse; charset=utf-8";
	if (options.$contentType) {
		httpQuery.headers["content-type"] = options.$contentType;
	}
	httpQuery.dataType = httpQuery.dataType ||
		function() {
			var dataType = httpQuery.headers["Accept"] || httpQuery.headers["content-type"] || "text/html";
	...
	// content type
	httpQuery.headers["content-type"] = httpQuery.headers["content-type"] || (httpQuery.dataType === "json" ? "application/json" : "");

	_setAcceptLanguage(httpQuery, options);

	if (options.data) {
		httpQuery.sendData = httpQuery.dataType === "json" ? JSON.stringify(options.data) : options.data;
	}
	...
	if (options.params) {
		...
		httpQuery.$url = syra_site.urlMaker.applyParameters(httpQuery.$url, options.params);
	}
	if (options.$etag) {
		httpQuery.headers["If-Match"] = options.$etag;
	}
	if (!BROWSER_VERBS_ALLOWED[httpQuery.method]) {
		httpQuery.headers["X-HTTP-Method-Override"] = httpQuery.method;
		httpQuery.method = "POST";
	}
}
...
helpers.defineClass(HttpQuery, null, {
		...
		sendNewRequest: function(options) {
			...
			function sendRequest(_, nbTry, self, initialSid) {
				...
				options.ajaxQuery = $.ajax({
						...
						error: function(jqXHR, textStatus, errorThrown) {
							...
							switch (jqXHR.status) {
								...
								case 12156:
									if (nbTry--) {
										sendRequest(_, nbTry, self, initialSid);
										return;
									}
							}
												...
												if (dataObj.$actions.$save && dataObj.$actions.$save.$isRequested) {
													...
													if (dataObj.$url && dataObj.$url.indexOf("userProfile") >= 0) {
														if (syra_site.userProfile) {
															...
															syra_controller.callServer(syra_site.userProfile, reqOptions);
														} else {
				...
				options = options || {};
				_prepareRequest(this, options);
				var retryId = this.method !== "GET" ? helpers.uuid.generate("") : null;
				if (retryId) {
					this.$url += (this.$url.indexOf('?') !== -1) ? "&retryId=" + retryId : "?retryId=" + retryId;
				}
				// send the request
				sendRequest(_, syra_site.userProfile.getRetryOnError() - 1, this, initialSid);
			...
			if (segments.workingCopyPreloadUrl) {
				...
				if (sessionStorage) {
					...
					if (sessionStorage.getItem(key)) {
						...
						var segs = syra_site.urlMaker.parse(reqOptions.$location.$url);
						segs.params.trackingId = segs.params.trackingId || helpers.uuid.generate();
						reqOptions.$location.$url = syra_site.urlMaker.build(segs);
