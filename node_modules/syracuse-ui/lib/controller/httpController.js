...
function _onReceiveError(error){
    ...
    if (document.site.mainPage && !document.site.mainPage.disposed) {
        ...
        // messagebox
        var options = {
            $message: localize["http_" + error.status],
            $title: localize["http_" + error.status + "_title"],
            $type: "fatal",
            $buttons: "ok"
        }
            ...
            error: function(jqXHR, textStatus, errorThrown){
                ...
                switch (jqXHR.status) {
                    ...
                    case 404:
                        ...
                        // user profile "logout" specific case
                        var dataObj = JSON.parse(ajax.data);
                        if (dataObj.$actions && dataObj.$actions.$logout && dataObj.$actions.$logout.$isRequested) {
                            var $url = "/";
                            setTimeout(function(){
                            ...
                            if (document.site && document.site.mainPage && document.site.mainPage.openerHttpQuery) {
                                ...
                                if (document.site.mainPage.openerHttpQuery.workingCopyPreloadUrl) {
                                    ...
                                    reqOptions.data = document.site.mainPage.dataset;
