...
exports.upload = function(options, responseCallback, progressCallback, errorCallback) {
	...
	if (options.file) {
		...
		reader.onload = function(evt) {
			if (self.isIe10) xhr.send(evt.target.result); //IE10
			else xhr.sendAsBinary(evt.target.result);
		};
		if (self.isIe10) reader.readAsArrayBuffer(options.file); //IE10
		else reader.readAsBinaryString(options.file);
	} else {
