...
exports.Controller = helpers.defineClass(Controller, null, {
	...
	changeMainPage: function($url, $itemPage, onHistoryChange, tempHackFusion) {
		var self = this;
		...
		if (true || self._currentUrl != $url) {
			...
			if ($itemPage) {
				...
				if (openerUrlSegments.isFusion) {
					...
					var workBook;
					if (!tempHackFusion && document.site.fusionGateway && (workBook = document.site.fusionGateway.activatedBook)) {
						workBook.onCloseAllSheets(function() {
							if (workBook.sheets.length != 0) {
								...
								self.changeMainPage($url, $itemPage, onHistoryChange, true);
							}
						});
						return false;
					}
					self.loadRepresentation(null, openerUrlSegments, function($itemPage) {
						$itemPage.initData = $itemPage.initData || (workingCopyPreload ? workingCopyPreload.initData : null);
						$itemPage.openerUrlSegments = openerUrlSegments;
	...
	loadRepresentation: function(article, segments, success, onError, onBeforeOpen, sendRqtOpts) {
		var self = this;
		var $representationUrl;
		if (typeof(segments) == "string") {
			this.loadRepresentation(article, document.site.urlMaker.parse(article ? article.parseExpression(segments) : segments), success, onError, onBeforeOpen, sendRqtOpts);
		} else {
			$representationUrl = segments.$url;
			//xhr.withCredentials = true;
			self.parseUrl((article || document.site).parseExpression(document.site.getPageTemplateUrl(segments.$url), {
				representation: segments.params.representation,
				endpoint: segments.endpoint,
				target: segments.target,
				pageview: segments.params.pageview,
				variant: segments.params.variant
			})).sendRequest(sendRqtOpts, function($representation, response, $url) {
				$representation.$pageRepresentationURL = $url;
				self._openLoadedRepresentation($representation, success, $representationUrl, segments, onError, onBeforeOpen, sendRqtOpts);
			}, onError);
		}
	},
	_openLoadedRepresentation: function($representation, success, $representationUrl, segments, onError, onBeforeOpen, sendRqtOpts) {
		var self = this;
	...
	loadWorkingCopy: function(options) {
		...
		self.postQuery(options.menu, options.record || null, null, function($location, data) {
			if (options.menuArticle) {
				var segments = document.site.urlMaker.parse($location.$url = options.menuArticle.parseExpression($location.$url, data));
