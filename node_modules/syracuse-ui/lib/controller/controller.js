...
exports.Controller = helpers.defineClass(Controller, null, {
		...
		if (menu.$target == "popup") {
			...
			menuArticle.formatMenuUrl(menu, record, function($url, isCanceled) {
				if (!isCanceled) {
					...
					if (httpQuery.$target != "blank" && menuArticle.page && menuArticle.page.$pageCategory == "vignette") {
		...
		if (menu.$isAction || menu.$url || menu.$sourceUrl) {
			...
			if (menu.$isAction) {
				...
				if (menu.$method && (menu.$method != "GET")) {
					...
					// REVIEW THIS >>>
					if (menuArticle && menuArticle.dataset && (menu.$invocationMode === "async")) menuArticle.dataset.$trackngId = helpers.uuid.generate();
					// <<<
	...
	findField: function($$item) {
		...
		if (article && article.idMap) return article.idMap[id];
		return null;
	},
