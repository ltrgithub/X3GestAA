...
exports.Controller = helpers.defineClass(Controller, null, {
	...
	_attachUrlWithRepresentation: function(segment, $url) {
		if (segment.params) {
	...
	_loadRepresentationContent: function($representation, options, userPreferenceLoading) {
		...
		if (typeof($representation.$prototype) == "string") {
			var $url = self._attachUrlWithRepresentation(options.segments, $representation.$prototype);
			self.parseUrl($url).sendRequest(options.sendRqtOpts, function($prototype) {
		...
		if ($representation.$prototype.$article && typeof($representation.$prototype.$article) == "string") {
			var $url = self._attachUrlWithRepresentation(options.segments, $representation.$prototype.$article);
			self.parseUrl($url).sendRequest(options.sendRqtOpts, function($article) {
		...
		if ($representation.$article && typeof($representation.$article) == "string") {
			var $url = self._attachUrlWithRepresentation(options.segments, $representation.$article);
			self.parseUrl($url).sendRequest(options.sendRqtOpts, function($article) {
	...
	sendRequest: function(page, options, onSuccess, onError) {
		...
		if (options.$location.$url) {
			...
			httpQuery.sendRequest(options, function(data, response, requestUrl) {
				if (options.$etag && options.page && !options.page.disposed && options.page.wcQueue) {
					var queue = options.page.wcQueue[options.$location.$url];
					delete options.page.wcQueue[options.$location.$url];
					if (queue && queue.options != options) {
						queue.options.data.$etag = queue.options.$etag = data.$etag;
						self.sendRequest(queue.options.page, queue.options, queue.onSuccess, queue.onError);
					}
				}
