...
exports.Controller = helpers.defineClass(Controller, null, {
	...
	changeMainPage: function(url, $itemPage) {
		...
		if (self._currentUrl != url) {
			self._currentUrl = document.site.urlMaker.getUrlParameter(url);
		...
		if (menu.$target == "popup") {
			...
			document.site.urlMaker.formatMenuUrl(menuArticle, menu, record, function($url, isCanceled) {
				...
				if (!isCanceled) {
					...
					if (document.site.urlMaker.isSyracuseURL(httpQuery.$url)) {
						if (httpQuery.$urlParts.uri.search(/mobile\.html/i) > 0) {
							httpQuery.$target = "blank";
							document.site.history.load(httpQuery.$url, httpQuery.$target);
							return;
						}
						if (httpQuery.$urlParts.$facet == "$summary") {
							...
							if (menu.$type && menu.$type.indexOf("json") == -1) {
								...
								if (isDefaultUrl) {
									...
									httpQuery.parse(document.site.urlMaker.setUrlParameter(httpQuery.$url));
								}
							}
						}
					} else {
	...
	loadWorkingCopy: function(options) {
		...
		self.postQuery(options.menu, options.record || null, null, function($location, data) {
			if (options.menuArticle) {
				var httpQuery = self.parseUrl(document.site.urlMaker.setUrlParameter($location.$url = options.menuArticle.parseExpression($location.$url, data)));
