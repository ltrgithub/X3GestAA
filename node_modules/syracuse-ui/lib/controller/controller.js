...
exports.Controller = helpers.defineClass(Controller, null, {
			...
			if ($itemPage) {
				...
				self.loadRepresentation({
					...
					onError: function(error, httpQuery) {
						...
						return syra_site.diagnosesPage[errorPage] && syra_site.diagnosesPage[errorPage](error, httpQuery.extractDiagnoses(error));
						/*           syra_site.show403(error);
                         syra_site.shwow404(error);
                         ...
                         return true; //do default error management*/
					}
				});
	...
	_loadRepresentationContent: function($representation, options, userPreferenceLoading) {
		...
		if (dataset && dataset.selectedRole) {
			$representation.$prototype.$role = dataset.selectedRole.$uuid; //important for url 
			$representation.$prototype.$userprofile = syra_site.userProfile.getEncodedValue();
		}
