...
var localizationPage = require('syracuse-ui/lib/page/localization/localizationPage');
...
exports.Controller = helpers.defineClass(Controller, null, {
	...
	executeMenu: function(menu, record, article, confirmed) {
		if (menu.$isAction || menu.$url || menu.$sourceUrl) {
			...
			// $localize (localizable properties)
			// TODO : change to if(menu.$bind == "$localize"){}
			if (menu.$item && menu.$item.$bind && menu.$item.$bind.indexOf("$localize") != -1) {

				var lPageBuilder = new localizationPageBuilder();
				lPageBuilder.loadLocalizationPage(menu);
			...
			if (menu.$isAction) {
				...
				if (menu.$method && (menu.$method != "GET")) {
					var isWorkingCopy = menu.$url.indexOf("$workingCopies") >= 0;
					if (!isWorkingCopy && menu.$sourceUrl) {
						isWorkingCopy = menu.$sourceUrl.indexOf("$workingCopies") >= 0;
					}
					if (isWorkingCopy) {
	...
	/**** File management ****/
	upload: function(file, url, article, requestCallback, progressCallback, errorCallback) {
	...
	findField: function(target) {
		...
		if (article && article.idMap) {
			while (target != null && !target.syraFieldId && !target.syraArticle) {
				target = target.parentNode;
			}
			if (target && target.syraFieldId) {
				return article.idMap[target.syraFieldId];
			}
		}
		return null;
	},
	findBox: function($$item) {
		...
		var id = $$item[0].syraBoxId || $$item.attr("data-s-box");
		if (!id) {
