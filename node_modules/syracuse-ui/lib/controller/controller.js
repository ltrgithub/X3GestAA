...
exports.Controller = helpers.defineClass(Controller, null, {
	...
	reloadMainPage: function() {
		...
		var qm = href.indexOf('?');
		if (rtl && !/rtl=true/.test(href)) href += (qm < 0 ? '?' : '&') + 'rtl=true';
		else if (!rtl) href = href.replace(/(\?|\&)rtl=true/, '');
		// reload page if writing direction changes
		if (href != document.location.href) document.location.href = href;
		else this.changeMainPage(href);
	},
	...
	_openLoadedRepresentation: function($representation, success, $representationUrl, httpQuery, onError, onBeforeOpen, sendRqtOpts) {
		...
		if (typeof($representation.$prototype) == "string") {
			...
			if (document.site.userProfile) {
				...
				if (document.site.userProfile.wrapLoadSuccess) success = document.site.userProfile.wrapLoadSuccess(success);
			}
	...
	_executeAction: function(menu, record, article) {
		...
		if (menu.$invocationMode === "async") r.$trackingId = helpers.uuid.generate();
		if (menu.$parameters) {
