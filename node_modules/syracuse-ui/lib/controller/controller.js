...
function _fetchPage(controller, $representation, options, userPreferenceLoading) {
	$representation.$prototype = $representation.$prototype || {};
	if (typeof($representation.$prototype) == "string") {
		var $url = _attachUrlWithRepresentation(options.segments, $representation.$prototype);
		options.sendRqtOpts.onSuccess = function($prototype) {
			$representation.$prototype = $prototype;
			_fetchPage(controller, $representation, options);
		};
		controller.parseUrl($url).sendNewRequest(options.sendRqtOpts);
		return;
	}
	if ($representation.$prototype.$article && typeof($representation.$prototype.$article) == "string") {
		var $url = _attachUrlWithRepresentation(options.segments, $representation.$prototype.$article);
		options.sendRqtOpts.onSuccess = function($article) {
	...
	if (!userPreferenceLoading) {
		...
		if ($userPreferencesLink) {
			if (!(options.segments && options.segments.representationRoot == "userProfile")) {
				options.sendRqtOpts.onSuccess = function(userPreferences) {
					$representation.userPreferences = userPreferences;
					_fetchPage(controller, $representation, options, true);
				};
				controller.parseUrl($userPreferencesLink.$url).sendNewRequest(options.sendRqtOpts);
				return;
			}
		}
	}
	if ($representation.$article && typeof($representation.$article) == "string") {
		var $url = _attachUrlWithRepresentation(options.segments, $representation.$article);
		options.sendRqtOpts.onSuccess = function($article) {
...
exports.Controller = helpers.defineClass(Controller, null, {
			...
			if ($itemPage) {
				...
				self.loadRepresentation({
					...
					onError: function(error, httpQuery) {
						...
						return syra_diagnose.showPage(errorPage, error, httpQuery.extractDiagnoses(error));
					}
				});
	...
	loadRepresentation: function(options) {
		...
		if (typeof(options.segments) == "string") {
			...
			var httpQuery = self.parseUrl(syra_site.urlMaker.buildPageCollaborationUrl(options.article || syra_site, options.segments));
			options.sendRqtOpts = options.sendRqtOpts || {};
			options.sendRqtOpts.onError = options.onError;
			options.sendRqtOpts.onSuccess = function($representation, response, $url) {
				$representation.$pageCollaborationUrl = $url;
				_fetchPage(self, $representation, options);
			};
			httpQuery.sendNewRequest(options.sendRqtOpts);
		}
	},
