...
exports.Controller = helpers.defineClass(Controller, null, {
    ...
    openResponseSoapPage: function(soapQuery, data){
        var self = this;
        setTimeout(function(){
            var $itemPage = {
                $$container: $("<div/>"),
    ...
    openSoapPage: function(httpQuery){
        var self = this;
        var soapQuery = self.parseUrl(httpQuery.$url);
                ...
                onValidate: function(page){
                    ...
                    page.formatMenuUrl(invokeMenu, null, function($url){
                        ...
                        self.sendRequest(page, {
                            ...
                            self.openResponseSoapPage(self.parseUrl(invokeMenu.$url), response.data);
                            dialog.close();
                        }, function(err){
                ...
                if (!isCanceled) {
                    ...
                    if (httpQuery.isSyracuseURL()) {
                        if (httpQuery.$urlParts.$facet == "$summary") {
                            ...
                            if (httpQuery.isSoapUrl()) {
                                self.openSoapPage(httpQuery);
                                return;
                            }
                            if (menu.$type && menu.$type.indexOf("json") == -1) {
                                ...
                                if (!httpQuery.$urlParts.isFusion && menu.$tags != "MOB") { //MOB=temp voir Bruno utilis� dans dashboard 
    ...
    notifyChange: function(article, $bind, value, isNotifyOnly){
        var self = this;
        var data = {};
        ...
        if (data) { //data == null =>onNotifyRecordChange disable notify
            ...
            if (articleParent) {
                articleParent.notifyChange(article.$item.$bind, data, isNotifyOnly);
            }
            else {
