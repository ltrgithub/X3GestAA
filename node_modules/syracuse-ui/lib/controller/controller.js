...
exports.loadRepresentation = function(context) {
	if (typeof(context.segments) == "string") {
		context.segments = syra_url.parse(context.article ? syra_site.expressionMaker.parse(context.article, context.segments) : context.segments);
	}
	...
	if (menu.$isAction || menu.$url || menu.$sourceUrl) {
		...
		if (!confirmed && menu.$confirm) {
			syra_diagnose.box.show({
				$title: syra_site.expressionMaker.parse(menuArticle, menuTitle || syra_local.siteDefaultConfirmTitle),
		...
		if (menu.$isAction) {
			...
			if (menu.$method && (menu.$method != "GET")) {
				...
				syra_workingCopy.onBeforeUnloadPage({
					success: function() {
...
exports.upload = function(options, responseCallback, progressCallback, errorCallback) {
	...
	// ERBOU Temporary fix: Daniel to clarify why no content-type is defined
