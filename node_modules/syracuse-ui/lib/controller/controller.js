...
function ensureMediaTypes(proto, finish) {
	...
	function collect(obj) {
		...
		function walk(prop) {
			if (obj[prop]) Object.keys(obj[prop]).forEach(function(k) {
				collect(obj[prop][k]);
			});
		}
		walk('$properties');
		walk('$variants');
		collect(obj.$item);
		if (obj.$type && !library.hasWidget(obj.$type)) missingTypes[obj.$type] = true;
	}
	collect(proto);
	...
	$.ajax({
		...
		success: function(data) {
			function requireMissing(i) {
				if (i < data.length) {
					if (!data[i].$module) return error("widget extension missing for " + data[i].$type);
				...
				syra_site.urlMaker.formatMenuUrl(menuArticle, menu, record, function($url, isCanceled) {
					if (!isCanceled) {
						if (menu.$invocationMode === "async" && menuArticle.dataset && menuArticle.dataset.$trackngId) {
							syra_jobs.registerJob($url, undefined, menu.$title, menuArticle.dataset.$trackngId);
							return;
						}
							...
							switch (page.$facet) {
								...
								case "$details":
									...
									if (menu.$invocationMode && menu.$invocationMode == "async" && menuArticle.dataset && menuArticle.dataset.$trackngId) {
										syra_jobs.registerJob($location.$url, undefined, menu.$title, menuArticle.dataset.$trackngId);
									}
									break;
								default:
									if (page) {
