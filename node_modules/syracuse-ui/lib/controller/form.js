...
//data.wcpreloadUrl = data.wcpreloadUrl || page.urlSeg.workingCopyPreloadUrl;

var _storage = {
	hasData: function(page) {
		var data = sessionStorage && sessionStorage.getItem(page.urlSeg.$url);
		if (data) {
	...
	write: function(page) {
		...
		if (page.isWorkingCopy && sessionStorage && page.urlSeg) {
			var key = page.urlSeg.$url;
	...
	clear: function(page) {
		sessionStorage && sessionStorage.removeItem(page.urlSeg.$url);
	}
};

exports.onWorkingCopyNotFound = function(ajax, $diagnoses, dataObj) {
	var segments = syra_site && syra_site.mainPage && syra_site.mainPage.urlSeg;
	var options = {};
	if (segments) {
		...
		options.success = ajax.options.success;
		options.error = ajax.options.error;
...
exports.updateWorkingCopy = function(page, sendBag) {
	if (!page.disposed) {
	...
	if (page) {
		if (field.articleParent.onFormUpdateDelta ? field.articleParent.onFormUpdateDelta(field, value) : true) {
			if (field.page.onFormUpdateDelta ? field.page.onFormUpdateDelta(field, value) : true) {
				...
				exports.updateWorkingCopy(page, sendBag);
				_storage.write(page);
			}
		}
...
exports.onBeforeUnloadPage = function(callback, activePage) {
	...
	if (wcPage && wcPage.$menus && wcPage.$menus.$save && _storage.hasData(wcPage)) {
		if (!activePage || (!activePage.isPageDesigner && !activePage.isAlertModal && wcPage == activePage)) {
			syra_alert.ask({
				$title: wcPage.getTitle() || syra_local.wc_warn_unload_title,
				$message: syra_local.wc_warn_unload_message,
				$newButtons: [{
					id: "yes",
					title: syra_local.wc_warn_unload_yes_button,
					click: function() {
					...
					id: "no",
					title: syra_local.wc_warn_unload_no_button
				}]
			});
			return;
		}
...
exports.load = function(options) {
	syra_router.postQuery({
		...
		article: options.menuArticle,
		success: function($url, data, requestUrl) {

			if (options.menuArticle) {
				var segments = syra_url.parse($url = syra_expression.parse(options.menuArticle, $url, data));
				...
				syra_router.loadRepresentation({
					...
					success: function($itemPage) {
						...
						options.article.load(data);
						options.success && options.success($itemPage, $url, data);
					}
				});
			}
...
exports.add = function(page) {
	if (exports.isFormUrl(page.urlSeg.$url)) {
...
function _onAfterActionMenuExecute(menuItem, $menu, isSuccess, $diagnoses) {
	...
	if (inlinePageHost) {
		...
		if (isSaved && isSuccess) {
			if (page.overPanel) {
				if (page.overPanel.options.onServerSaved) {
					if (page.overPanel.options.onServerSaved(menuItem, page.overPanel) === false) {
						return false;
					}
				}
			} else {
...
exports.showLastSavingDiagnoses = function(page) {
	if (_lastSavingDiagnoses) {
		var $diagnoses = _lastSavingDiagnoses[page.urlSeg.$url];
