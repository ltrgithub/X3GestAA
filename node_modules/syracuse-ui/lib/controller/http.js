...
function _onWorkingCopyNotFound(httpQuery, error, dataObj) {
...
HttpQuery.prototype.extractDiagnoses = function(error) {
	var $diagnoses;
	if (error) {
...
HttpQuery.prototype.showError = function(error) {
...
HttpQuery.prototype._send = function(nbTry, initialSid) {
	var self = this;
	...
	self.options.ajaxQuery = $.ajax({
		...
		error: function(jqXHR, textStatus, errorThrown) {
			...
			switch (jqXHR.status) {
				...
				case 12156:
					if (nbTry--) {
						self._send(nbTry, initialSid);
						return;
					}
					break;
			}
			if (!self.options.error || self.options.error(error, self)) {
				var ajax = this;
				switch (jqXHR.status) {
					case 0:
						!self.options.isAborted && self.showError(error);
						break;
					case 404:
						//---------------------------------------
						...
						if (dataObj && dataObj.$actions) {
							...
							if (dataObj.$actions.$logout && dataObj.$actions.$logout.$isRequested) {
								...
								_onWorkingCopyNotFound(self, error, dataObj);
							}
						} else {
							...
							if (ajax.url.indexOf("$lookup") != -1 && error.data) {
								...
								if (syra_workingCopy.isUrl(self.$url)) {
									_onWorkingCopyNotFound(self, error);
								} else {
									var errData = {};
									try {
					...
					default:
						if (jqXHR.status >= 500) {
							//to improve #5390
							if (self.options && self.options.page && self.options.page.dialogWrapper) {
								self.options.page.dialogWrapper.close();
							}
...
HttpQuery.prototype.send = function(options) {
	...
	this.headers.Accept = this.options.$acceptType || "application/json;vnd.sage=syracuse; charset=utf-8";
	...
	this._send(syra_site.userProfile.getRetryOnError() - 1, initialSid);
};

HttpQuery.prototype.sendCorsRequest = function(options, onSuccess, onError) {
	options = options || {};
...
exports.send = function(options) {
	...
	(new HttpQuery(options.$url)).send(options);
};

exports.sendCorsRequest = function(options) {
	var httpQuery = new HttpQuery();
	httpQuery.parse(options.$url);
	httpQuery.sendCorsRequest(options);
};
