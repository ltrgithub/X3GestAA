...
HttpQuery.prototype.showError = function(error) {
	syra_alert.error(this.extractDiagnoses(error));
};
					...
					case 404:
						...
						if (dataObj && dataObj.$actions) {
							...
							if (ajax.url.indexOf("$lookup") != -1 && error.data) {
								...
								syra_alert.error(errData.$diagnoses || syra_local.default_err_msg);
							} else {
								if (syra_form.isFormUrl(self.$url)) {
									...
									if (errData.$diagnoses) {
										syra_alert.error(errData.$diagnoses);
									} else {
										syra_form.onWorkingCopyNotFound(self, error);
									}
								}
...
HttpQuery.prototype.prepare = function(options) {
				...
				if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304 || xhr.status == 1223) {
					...
					if (options.error) {
						...
						if (syra_site.mainPage && !syra_site.mainPage.disposed) {
							syra_alert.error((error.data.indexOf("$diagnoses") >= 0) ? JSON.parse(error.data).$diagnoses : [{
