...
exports.changeMainPage = function($url, $itemPage, onHistoryChange) {
	...
	if (!syra_site.isOfficeSite) {
		syra_nav.showMegaMenus(null);
		urlSeg = syra_url.history.getUrlSegment($url);
		if (urlSeg.$facet === "$navigation" && document.location.href.search(/syracuse-main\/html\/main\.html/i) > -1) {
			/*
	...
	if (menu.$isAction || menu.$url || menu.$sourceUrl) {
		...
		if (menu.$isAction) {
			...
			if (menu.$method && (menu.$method != "GET")) {
				...
				var executed = exports.executeFusionFunc(menu, syra_fusion && syra_fusion.activatedBook) && menu.urlSeg && menu.urlSeg.isFusion;
				if (!executed) {
					syra_form.onBeforeUnloadPage(function() {
...
exports.executeFusionFunc = function(menuItem, workBook) {
	var executed = false;
	if (workBook) {
