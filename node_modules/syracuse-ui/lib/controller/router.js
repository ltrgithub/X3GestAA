...
exports.executeMenu = function(menu, article, confirmed) {
	if (menu.$isAction || menu.$url || menu.$sourceUrl) {
		/*if (menu.$url.indexOf("syracuse/application") > 0) {
		 //menu.$sourceUrl = menu.$url = "http://localhost:8124/plugin/anypdf.pdf";
		 // menu.$sourceUrl = menu.$url = "http://localhost:8124/trans/x3/erp/RELV7/$files/TMP/C128.pdf";
		 }*/
		...
		if (menu.$isAction) {
			...
			if (menu.$method && (menu.$method != "GET")) {
				...
				syra_fusion.executeFusionFunc(menu, syra_fusion && syra_fusion.activatedBook, function() {
					syra_form.onBeforeUnloadPage(function() {
		...
		default:
			syra_url.formatMenuUrl(menuArticle, menu, record, function($url, isCanceled) {
				if (!isCanceled) {
					...
					var isRepresentationUrl = segments.isPageRepresentation;
					...
					if (segments.isSyracuse) {
						...
						if (segments.$facet == "$summary") {
							...
							if (!isRepresentationUrl || (menu.$type && menu.$type.indexOf("json") == -1)) {
								target = "blank";
							} else {
