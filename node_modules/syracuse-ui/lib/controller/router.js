...
function _fetchPage($representation, context, userPreferenceLoading) {
	...
	if (typeof($representation.$prototype) == "string") {
		...
		syra_http.get(context.sendRqtOpts);
		return;
	}
		...
		if ($itemPage) {
			...
			exports.loadRepresentation(_mainPageFetching = {
				...
				error: function(error, httpQuery) {
					...
					return syra_alert.page(error, httpQuery.extractDiagnoses(error));
				}
			});
...
exports.postQuery = function(menu, record, article, callback, callbackError) {
	...
	syra_url.formatMenuUrl(menuArticle, menu, record, function($url) {
		...
		syra_http.send({
			page: menuArticle,
			url: $url,
			accept: menu.accept || menu.$acceptType || menu.$type,
			contentType: menu.contentType || menu.$contentType,
			params: params,
