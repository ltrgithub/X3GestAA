	...
	if (!userPreferenceLoading) {
		...
		if ($userPreferencesLink) {
			if (!(context.segments && context.segments.representationRoot == "userProfile")) {
				...
				context.sendRqtOpts.success = function($newPreferences) {
					$representation.$newPreferences = $newPreferences;
...
exports.loadRepresentation = function(context) {
	...
	context.sendRqtOpts.error = context.onError;
		...
		syra_ajax.send({
			...
			success: function(data, response, requestUrl) {
				...
				if ($location.$url) {
					...
					$location.$url = response.data.$url || option.menu.$url;
					$location.$type = response.data.$type || option.menu.$type;
				}
				if (option.menu.$invocationMode === "async") {
					_openTracker(option.menu, menuArticle, $location.$url, response.data);
				} else {
					option.success($location, response.data, requestUrl);
				}
			},
			error: option.error
		});
...
exports.executeMenu = function(menu, article, confirmed) {
	if (menu.$isAction || menu.$url || menu.$sourceUrl) {
		...
		if (!confirmed && menu.$confirm) {
			syra_alert.ask({
				$title: syra_expression.parse(menuArticle, menuTitle || syra_local.siteDefaultConfirmTitle),
				$message: syra_expression.parse(menuArticle, menu.$confirm),
