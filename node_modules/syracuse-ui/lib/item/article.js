...
exports.applyChange = function(article, newData) {
	if (newData) {
		syra_log.applyChange(article, newData);
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if (newFieldValue !== undefined || newFieldMetaData !== undefined) {
				...
				for (var kk = 0, mm = fields.length; kk < mm; kk++) {
					var field = fields[kk];
					syra_log.setValue(field, newFieldValue, newFieldMetaData);
					field.setValue(newFieldValue, newFieldMetaData, newData);
					field.isWidgetUpated = true;
