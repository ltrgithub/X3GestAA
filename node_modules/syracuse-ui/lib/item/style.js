...
function Theme(userProfile, slot) {
	...
	this.btn = syra_dom.anchor("s-head-popup-btn-default", syra_dom.li("s-head-popup-li", slot));
	this.btn.syraItem = this.id;
	this.btn.syraOnClick = "click";
	syra_dom.span("s-user-profile-theme-label", this.btn).textContent = syra_local.userProfile_theme;
	this.titleItem = syra_dom.span("s-user-profile-theme-value", this.btn);
	syra_dom.addFontIcon("s-user-profile-theme-flag", syra_icon.font.right_arrow, this.btn);
	...
	this.click = function(event, target) {
		var self = this;
		if (target == self.btn) {
			var ul = syra_dom.ul("s-over-popup");
			...
			for (var ii = 0, jj = themes.length; ii < jj; ii++) {
				var theme = themes[ii];
				var link = syra_dom.anchor("s-head-popup-btn-default", syra_dom.li("s-head-popup-li", ul));
			...
			if (target.syraTheme !== undefined) {
				syra_form.updateDelta({
					page: self.page,
					articleParent: self.page,
					$item: {
						$bind: "selectedTheme"
					}
