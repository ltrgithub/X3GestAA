...
exports.JobsViewer = helpers.defineClass(JobsViewer, RawPage, {
	...
	onMenuClick: function(menuItem) {
		...
		switch (menuItem.$bind) {
			...
			case "$details":
				...
				self.jobs[menuItem.articleParent.$uuid].onClientRelease(menuItem.$url, "DELETE");
				break;
			case "$download":
				window.open(menuItem.$url, "_blank");
				break;
		...
		// to prevent call to syra_controller.executeMenu
	...
	_hasJobInProgress: function(list) {
		for (var ii = 0, jj = list.length; ii < jj; ii++) {
			...
			for (var mm = 0, kk = keys.length; mm < kk; mm++) {
				...
				if (item.$item && item.$item.$bind == '$progressOn' && $(item.domItem).is(":visible")) {
					return true;
				}
			}
		}
	...
	_showMessage: function(menuItem, cbck) {
		var self = this;
		var options = {};
		options.$message = (syra_local['job_' + menuItem.$bind.slice(1) + '_msg'].replace("{jobTitle}", menuItem.articleParent.dataset.jobTitle)).replace("{jobKind}", menuItem.articleParent.dataset.jobType);
		options.$title = syra_local['job_' + menuItem.$bind.slice(1) + '_title'];
		...
		self.msgBoxId = syra_site.messageBox.id;
	},
	...
	applyChange: function(newData, closeMsgBox, newRecord) {
		...
		if (closeMsgBox && syra_site.messageBox && self.msgBoxId == syra_site.messageBox.id && !syra_site.messageBox.disposed) {
			syra_site.messageBox.close("no");
		}

		// hide record if necessary (according to jobs types list filtering)
		if (recordToHide) {
			self.getRecord(newData.$jobDetails[0].$uuid).domItem.style.display = "none";
		}
