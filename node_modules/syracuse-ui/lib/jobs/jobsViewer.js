...
exports.JobsViewer = helpers.defineClass(JobsViewer, RawPage, {
    ...
    loadBox: function(){
        ...
        var self = this;
        self.$prototype = jobsViewerPrototype.getPrototype();
        self.$prototype.$properties.$jobType.$value.$enum = self.getJobsTypesEnum();
        
        //$choiceLayout: self.selectedTypeValue || "1",
        self.$item = jobsViewerArticle.getItem();
    ...
    _removeJobView: function(record){
        ...
        var newData = {};
        var $enum = self.getJobsTypesEnum();
        if ($enum.length == 1) {
    ...
    applyChange: function(newData, closeMsgBox, newRecord){
        ...
        if (newRecord && newData.$jobDetails && newData.$jobDetails.length <= 1) {
            var $prevEnum = self.dataset && self.dataset.$properties && self.dataset.$properties.$jobType.$value.$enum && self.dataset.$properties.$jobType.$value.$enum.length > 0;
        ...
        if (newData.$jobDetails && newData.$jobDetails.length > 0) {
            ...
            for (var pp = 0, hh = newData.$jobDetails.length; pp < hh; pp++) {
                if (newData.$jobDetails[pp].diagnoseFatal || newData.$jobDetails[pp].diagnoseError || newData.$jobDetails[pp].diagnoseWarning || newData.$jobDetails[pp].diagnoseInfo || newData.$jobDetails[pp].diagnoseSuccess) {
        ...
        if (recordToHide) {
            self._getRecord(newData.$jobDetails[0].$uuid).$$item.hide();
        }
