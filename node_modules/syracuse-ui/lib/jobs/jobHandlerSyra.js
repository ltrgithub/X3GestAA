...
exports.JobHandlerSyra = helpers.defineClass(JobHandlerSyra, JobHandler, {
	register: function(params) {
		if (params) {
			JobHandler.prototype.register.call(this, params);
			// send tracking request if no err diagnose
			this.params && this.sendRequest(this.buildReqOpt("GET", params.$location));
		}
	},
	...
	onSuccess: function(data, response, $url, cbck) {
		...
		if (response.status != 204 && record != undefined) {
			var scrollTop = syra_site.jobsPage.boundFields.$jobDetails[0]._dataValue.scrollTop;
			...
			syra_site.jobsPage.boundFields.$jobDetails[0]._dataValue.scrollTop = scrollTop;
		}
	...
	onClientRelease: function($url, $method, cbck) {
		$url && this.sendRequest(this.buildReqOpt($method, $url), cbck, true);
	}
});