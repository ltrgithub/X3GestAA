...
exports.JobHandler = helpers.defineClass(JobHandler, null, {
	...
	register: function(jobParams, currentState, nextStateSvcId, cancelSvcId, explicitReleaseCallback) {
		...
		self.jobsViewer = document.site.jobsViewer;
		if (!self.jobsViewer) {
			self.jobsViewer = document.site.jobsViewer = new JobsViewer();
	...
	_buildInitRecord: function() {
		...
		if (self.params) {
			...
			return {
				...
				jobDiagnoses: [{
					$index: 0,
					diagMsg: document.site.localize.jobs_no_params
				}]
			};
		}
	},
	...
	onError: function(error, httpquery, cbck) {
		...
		if (error.data.indexOf("$diagnoses") != -1) {
			...
			if ($diagnoses) {
				...
				$diagnoses = [{
					$severity: "error",
					$message: document.site.localize.job_err_without_diag.replace("{status}", error.status)
				}];
			}
		} else {
			$diagnoses = [{
