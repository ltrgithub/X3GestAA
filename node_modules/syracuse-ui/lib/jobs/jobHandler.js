...
exports.JobHandler = helpers.defineClass(JobHandler, null, {
	...
	register: function(jobParams, currentState, nextStateSvcId, cancelSvcId, explicitReleaseCallback) {
		...
		self.jobsViewer = syra_site.jobsViewer;
		if (!self.jobsViewer) {
			self.jobsViewer = syra_site.jobsViewer = new JobsViewer();
			self.jobsViewer.localize = syra_site.localize;
			self.jobsViewer.jobsCount = 0;
			self.jobsViewer.$isEditMode = true;
		}
		// save job handler
		...
		if (!self.jobsViewer.domItem) {
			self.jobsViewer.loadBox();
		}
		self.jobsViewer.toggle(true);
		// append job view (record)
		self.jobsViewer.isPageLoaded = true;
		syra_site.jobsViewer.applyChange(newData, undefined, true);
	},
	_buildInitRecord: function() {
		...
		if (self.params) {
			...
			record.$index = self.jobsViewer.getRecordsList().length || 0;
			...
			self._appendDateTimeData(self.params, record, $isHiddenFalse);
			...
			return {
				...
				jobDiagnoses: [{
					$index: 0,
					diagMsg: syra_site.localize.jobs_no_params
				}]
			};
		}
	},
	...
	_appendDateTimeData: function(data, record, $isHiddenFalse) {
		...
		if (data.startDate) {
			...
			record.$properties["startDate"] = $isHiddenFalse;
			record.$properties["startTime"] = $isHiddenFalse;
		}
	},
	buildRecord: function(data, success) {
		...
		if (self.params) {
			...
			record.$links = {};
			record.$uuid = self.params.uuid;
			record.$index = self.jobsViewer.getRecord(self.params.uuid).getRecordIndex();
			...
			// handling date time data
			self._appendDateTimeData(data, record, $isHiddenFalse);
	...
	onError: function(error, httpquery, cbck) {
		...
		if (error.data.indexOf("$diagnoses") != -1) {
			...
			if ($diagnoses) {
				...
				$diagnoses = [{
					$severity: "error",
					$message: syra_site.localize.job_err_without_diag.replace("{status}", error.status)
				}];
			}
		} else {
			$diagnoses = [{
