...
exports.JobHandlerFusion = helpers.defineClass(JobHandlerFusion, JobHandler, {
	register: function(jobParams, currentState, nextStateSvcId, cancelSvcId, explicitReleaseCallback) {
		...
		if (!jobParams) {
			return {
				$diagnoses: [{
					$severity: 'error',
					$message: syra_site.localize.jobs_err_noparams
				}]
			};
		}
	...
	appendSpecificProperty: function(record) {
		...
		if (self.cancelSvcId) {
			record.$links.$cancel = {
				$title: syra_site.localize.jobs_cancel_label
			};
		}
	},
		...
		if (record) {
			...
			if (success && $url && $url.indexOf("/print/$report") != -1) {
				record.$links.$download = {
					$title: syra_site.localize.jobs_download_label,
					$url: $url
				};
			}
	...
	onClientRelease: function(cbck) {
		...
		if (self.cancelSvcId) {
			...
			var $url = syra_site.expressionMaker.parse(article, self.params.$location, self.params.services[self.cancelSvcId]);
