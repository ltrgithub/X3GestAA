...
exports.JobHandlerFusion = helpers.defineClass(JobHandlerFusion, JobHandler, {
	register: function(jobParams, currentState, nextStateSvcId, cancelSvcId, explicitReleaseCallback) {
		...
		if (!jobParams) {
			return {
				$diagnoses: [{
					$severity: 'error',
					$message: document.site.localize.jobs_err_noparams
				}]
			};
		}
		...
		if (self.params) {
			...
			reqOpt.$location = {
				$url: document.site.expressionMaker.parse(article, self.params.$location, self.params.services[nextStateSvcId])
			};
	...
	appendSpecificProperty: function(record) {
		...
		if (self.cancelSvcId) {
			record.$links.$cancel = {
				$title: document.site.localize.jobs_cancel_label
			};
		}
	},
		...
		if (record) {
			...
			if (success && $url && $url.indexOf("/print/$report") != -1) {
				record.$links.$download = {
					$title: document.site.localize.jobs_download_label,
					$url: $url
				};
			}
	...
	_onOperationComplete: function() {
		try {
			...
			console.log(this.jobsViewer.jobs_no_release_callback);
		}
	},
	onClientRelease: function(cbck) {
		...
		if (self.cancelSvcId) {
			...
			var $url = document.site.expressionMaker.parse(article, self.params.$location, self.params.services[self.cancelSvcId]);
