...
var JobHandler = require('syracuse-ui/lib/jobs/jobHandler').JobHandler;
...
function _clearJob(record) {
	...
	if (page.jobs) {
		...
		_opener.textContent = --page.jobsCount;
	}
...
function _showMessage(page, $bind, dataset, cbck) {
	page.msgBoxId = syra_diagnose.showBox({
		$message: (syra_local['job_' + $bind.slice(1) + '_msg'].replace("{jobTitle}", dataset.jobTitle)).replace("{jobKind}", dataset.jobType),
...
exports.JobsPage = helpers.defineClass(JobsPage, RawPage, {
	...
	loadBox: function() {
		...
		this.scrollview.className = "s-jobs-viewer-body";
		this.jobDetailsField = this.loadNewItem(this.scrollview, {
			...
			renderRecordContent: function(record) {
				//syra_local.jobs_diagStackTrace,
				var page = record.page;
				...
				slot.className = "s-job-view-phase-info";
				record.progressSlide = document.createElement("div");
				record.progressSlide.className = "s-job-view-progress-bar";
				slot.appendChild(record.progressSlide);
				record.progress = document.createElement("div");
				record.progress.className = "s-job-view-progress-data";
				slot.appendChild(record.progress);
				...
				record.jobBtn.syraRecord = record.clearBtn.syraRecord = record.id;
				record.jobBtn.syraItem = record.clearBtn.syraItem = page.id;
				...
				var $diagnoses = record.dataset.$diagnoses;
				if ($diagnoses) {
					record.diagnosesSlot = document.createElement("div");
					record.diagnosesSlot.className = "s-job-diagnoses";
					record.body.appendChild(record.diagnosesSlot);
					record.diagnoses = [];
					for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
						var $diagnose = $diagnoses[ii];
						var severity = $diagnose.$severity || "error";
						var item = syra_menus.addFontIconText($diagnose.$message, "s-job-diagnose s-diag-" + severity, severity == "info" ? severity : "diagnose");
						record.diagnosesSlot.appendChild(item);
						record.diagnoses.push(item);
						if ($diagnose.$stackTrace) {
							record.stackBtn = syra_menus.addIconButton(syra_local.jobs_stack_expand, "s-job-stack-opener", "onStackClick", null, "expand");
							record.stackBtn.$opened = false;
							record.stackBtn.syraDiag = ii;
							record.stackBtn.syraItem = record.id;
							item.insertBefore(record.stackBtn, item.firstChild);
						}
					}
					record.onStackClick = function(event, btn) {
						...
						if (btn.$opened && !this.stackSlot) {
							this.stackSlot = document.createElement("div");
							this.stackSlot.className = "s-job-stack";
							btn.parentNode.appendChild(this.stackSlot);
							this.stackSlot.innerHTML = syra_site.dom.formatHTMLText(record.dataset.$diagnoses[btn.syraDiag].$stackTrace);
						}
						syra_site.dom.display(this.stackSlot, btn.$opened);
					};
				}
			}
		});
		this.domItem.insertBefore(this.scrollview, this.scrollview.nextSibling);
	},
	onClearJob: function(event, target) {
		var record = syra_store.get(target.syraRecord);
		if (_hasJobInProgress([record])) {
			_showMessage(this, "$clearJob", record.dataset, function() {
				_clearJob(record);
			});
		} else {
			_clearJob(record);
	...
	onClearList: function(event, target, records) {
		...
		if (!records) {
			...
			if (_hasJobInProgress(records)) {
				_showMessage(self, "$clearList", self.dataset, function() {
	...
	onJobActionClick: function(event, target) {
		var self = this;
		var record = target.syraBind && syra_store.get(target.syraRecord);
		var $menu = record.$menus[target.syraBind];
		var $url = syra_site.urlMaker.formatMenuUrl(record, $menu);
		switch (target.syraBind) {
			case "$download":
				var job = self.jobs[record.$uuid];
				if (job) {
					if ($menu.$verifyUrl) {
						...
						var $verifyUrl = syra_site.urlMaker.formatMenuUrl(record, $verifyMenu);
						syra_controller.callServer(null, job.buildReqOpt("GET", $verifyUrl, job.params.mime), function() {
				...
				break;
			case "$cancel":
				_showMessage(self, target.syraBind, record.dataset, function() {
					self.jobs[record.$uuid].onClientRelease();
				});
				break;
			case "$abort":
			case "$suspend":
			case "$resume":
				_showMessage(self, {
					$bind: target.syraBind,
					articleParent: record
				}, function() {
					self.jobs[record.$uuid].onClientRelease($url, $menu.$method);
				});
				break;
			default:
	...
	applyChange: function(newData, closeMsgBox, newRecord) {
		...
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			...
			if (record.dataset.$progressValue) {
				record.progressSlide.className = "s-job-view-progress-bar s-job-" + record.dataset.$progressValue;
			}
			record.progress.textContent = record.dataset.progress ? (record.dataset.progress + " %") : "";
			...
			for (var mm = 0, pp = actions.length; mm < pp; mm++) {
				...
				if ($mn && !$mn.$isHidden) {
					visible = true;
					syra_menus.updateButtonIcon(record.jobBtn, $mn.$title, record.jobBtn.syraBind = $mn.$bind);
				}
			}
	...
	incrementCount: function() {
		...
		_opener.textContent = ++this.jobsCount;
	}
});


exports.addOpener = function(slot) {
	_opener = syra_menus.addIconButton("", "s-jobs-viewer-opener", "onJobsOpenerClick");
	_opener.syraItem = syra_site.id;
	syra_site.onJobsOpenerClick = function() {
		exports.page && exports.page.toggle();
	};
	_opener.style.display = "none";
	slot.appendChild(_opener);
};

exports.registerJob = function($location, $state, $title, uuid) {
	var job = new JobHandler();
	job.register({
		$location: $location,
		$state: $state,
		title: $title,
...
exports.addJob = function(job) {
	...
	if (!page) {
		page = exports.page = new JobsPage();
		page.jobsCount = 0;
		page.$isEditMode = true;
	}
	page.jobs = page.jobs || {};
	page.jobs[job.params.uuid] = job;
	page.incrementCount();
	page.updateJobsTypesList(job.params.kind, true);
};

exports.createTest = function() {
	var page = exports.page = new JobsPage();
	page.loadBox();
	page.jobsCount = 0;
	page.jobs = {};
	return page;
	...
	syra_controller.callServer(null, {
		...
		if (data.$resources && data.$resources.length > 0) {
			for (var ii = 0, jj = data.$resources.length; ii < jj; ii++) {
				...
				syra_jobs.registerJob(params.$location, params.$state, params.$title, params.uuid);
			}
		}
	}, function(error, httpquery) {
