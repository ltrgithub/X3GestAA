...
exports.JobView = helpers.defineClass(JobView, null, {
    create: function(job){
        ...
        // body divided into columns. (1st container child)
        self._body = document.createElement("div");
        self._body.className = "s-job-view-body";
        self.$$item.append(self.$$body = self.$$body = $(self._body));
        
        // job type column
        self._jobType = document.createElement('div');
		
		// TO DO
        //self._jobType.className = "s-job-view-type-" + (job.params.kind.toLowerCase() || "print");
		self._jobType.className = "s-job-view-type-" + job.params.kind;
        self.$$body.append(self.$$jobType = $(self._jobType));
        ...
        // result and cancel column
        self._res = document.createElement("div");
        self._res.className = "s-job-view-res";
        // initialize res btn, first btn displayed is *cancel* btn
        self._resBtn = document.createElement("a");
        self._resBtn.className = job.cancelSvcId ? "s-job-view-cancel" : "s-job-view-cancel-disabled";
        self._resBtn.setAttribute("alt", job.cancelSvcId ? self.localize.job_cancel_alt : "");
        self._resBtn.textContent = self.localize.job_cancel;
        self._res.appendChild(self._resBtn);
    ...
    _bindEvents: function(bind){
        ...
        if (self.$$item) {
            if (bind) {
                ...
                }).delegate("a.s-job-view-success", "click", function(){
                    if (self.finalUrl) {
                        window.open(self.finalUrl, "_blank");
                    }
                });
            }
            else {
    ...
    _onJobCancel: function(){
        var self = this;
        if (self.job.cancelSvcId && self._progressValue && self._progressValue.className == "s-job-view-progress-value") {
            var options = {};
            options.$message = (self.localize.job_cancel_msg.replace("{jobTitle}", self._title.textContent)).replace("{jobKind}", self.job.params.kindLabel || self.job.params.kind || "");
            options.$type = "warning";
            options.$buttons = "yesno";
            options.$autoClose = 25000;
            options.$default = "no";
            options.$title = self.localize.job_cancel_title;
            ...
            self.cancelMsgBox = document.site._msgBox;
        }
    },
    _cancelJob: function(){
        var self = this;
        if (self._progressValue && self._progressValue.className == "s-job-view-progress-value") {
            self.job.onClientRelease();
        }
    },
    ...
    _appendInfo: function(values, dom){
        ...
        for (var ii = 0; ii < $bindings.length; ii++) {
            if (values[$bindings[ii]]) {
                ...
                label.textContent = this._capitalize($bindings[ii]) + " : " + values[$bindings[ii]];
    ...
    appendDiagnoses: function($diagnoses){
		...
		document.site.emptyDom(self._diag);
        //self.$$diag.empty();
        if ($diagnoses) {
            for (var ii = 0; ii < $diagnoses.length; ii++) {
                ...
                //self._appendReportUrl(self._diag);
            }
        }
    },
    _appendReportUrl: function(dom){
        if (this.finalUrl) {
            var link = document.createElement("a");
            link.className = "s-job-view-result-report";
            link.textContent = this.localize.job_report_label;
            link.setAttribute("href", this.finalUrl);
            link.setAttribute("target", "_blank");
            dom.appendChild(link)
        }
    },
    setFinalUrl: function(url){
        this.finalUrl = this.finalUrl || url;
    },
    ...
    _onEnd: function(onError){
        ...
        if (self.cancelMsgBox && !self.cancelMsgBox.disposed) {
            document.site._msgBox.close("no");
        }
        
        self._resBtn.className = onError ? "s-job-view-cancel-disabled" : (self.job.params.kind && self.job.params.kind == "Print" && self.finalUrl ? "s-job-view-success" : "s-job-view-success-disabled");
        self._resBtn.textContent = onError ? self.localize.job_cancel : self.localize.job_ok;
        self._resBtn.setAttribute("alt", onError ? '' : (self.job.params.kind && self.job.params.kind == "Print" && self.finalUrl ? self.localize.job_ok_alt : ''));
