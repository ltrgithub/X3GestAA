...
exports.field = {
	...
	findDescription: function(field) {
		return field.querySelector("[data-s-description]");
	},
	getValue: function(field) {
		var value = field.querySelector("[data-s-value]");
		if (value) {
	...
	isMandatory: function(field, checkFlag) {
		var ok = field.getAttribute("data-s-mandatory") == "1";
		if (ok && checkFlag) {
			var title = exports.field.getTitle(field);
			if (title) {
	...
	findAll: function(root, bind) {
		return bind ? root.querySelectorAll("[data-s-field-bind='" + bind + "']") : root.querySelectorAll("[data-s-field-bind]");
	},
	find: function(root, bind) {
		return root.querySelector("[data-s-field-bind='" + bind + "']");
	},
	focus: function() {
	...
	getBind: function(field) {
		return field.getAttribute("data-s-field-bind");
	},
	getType: function(field) {
		return field.getAttribute("data-s-type");
	},
	getTitle: function(field) {
		return field.getAttribute("data-s-title");
	},
	...
	isInputReadOnly: function(field, checkInput) {
		...
		if (ok && checkInput) {
			...
			for (var ii = 0; ii < inputs.length && ok; ii++) {
				ok = !!inputs[0].getAttribute("readonly");
			}
		}
		return ok;
	},
...
exports.list = {
	...
	getRecordSelected: function(list) {
		return list.querySelectorAll(".s-record-selected");
	},
		...
		isSelected: function(records, bind) {
			...
			if (records) {
				for (var ii = 0; ii < records.length; ii++) {
					selected = records[ii].className.indexOf("s-record-selected") >= 0;
				}
			}
			return selected;
		}
		...
		for (var ii = 0; ii < records.length; ii++) {
			if (binds) {
				for (var mm = 0; mm < binds.length; mm++) {
					var item = exports.field.find(records[ii], binds[mm]);
