...
Line.prototype.showMessage = function(show) {
	if (this.field && this.field.isAlertPanelHidden) {
		show = false;
	}
...
Line.prototype.addTopLabel = function() {
	...
	syra_dom.pre("s-diagnose-msg-value", this.$diagnose.$message, this.msgItem.link);

	if (this.$diagnose.$links || this.$diagnose.$actions) {
		this.panel.drawLinks(syra_dom.div("s-diagnose-links " + severity, this.msgItem.link), this.$diagnose, this.page);
	}
	this.$diagnose.$stackTrace = this.$diagnose.$stackTrace || this.$diagnose.stackTrace;
	if (this.$diagnose.$stackTrace) {
		this._trace = syra_dom.div("s-diagnose-stacktrace");
		syra_dom.pre("s-pre", this.$diagnose.$stackTrace, this._trace);
...
function Panel(page) {
	...
	this.domItem = syra_dom.div("s-diagnose");
	this.page.diagnoseSlot.className = this.page.diagnoseSlot.className || "s-diagnose-page-slot";
	this.domItem.syraItem = this.id;
	this.body = syra_dom.div("s-diagnose-msg-list", this.domItem);
...
Panel.prototype.drawLinks = function(slot, message, page) {
	...
	syra_menus.applyChange(page || this.page, message, slot);
	delete message.$diagnosePanelId;
	delete message.$diagnosesSlot;
};

Panel.prototype.hasFieldErrors = function(page) {
	...
	if (box.severitiesCount && box.diagnoseBtn) {
		if (onEnter) {
			if (!box.diagnosePopupSlot) {
				box.diagnosePopupSlot = syra_dom.div("s-diag-field-slot-popup");
	...
	if (!message || (!message.$bind && !message.field && !message.$diagnoses)) {
		...
		if (message.$links || message.$actions) {
			if (!this._diagViewLinks) {
				this._diagViewLinks = syra_dom.div("s-diagnose-links-main", this.domItem);
			}
		...
		if (message.field || message.$bind) {
			...
			if (message.$diagnoses && message.$diagnoses.length > 0) {
				for (var ii = 0, jj = message.$diagnoses.length; ii < jj; ii++) {
					var $diagnose = message.$diagnoses[ii];
					if ($diagnose.$message) {
						var line = this.addLine(page, $diagnose);
						line.article = page;
		...
		if (page && page.isPageLoaded && syra_dom.isVisible(this.domItem)) {
			page.resizeItem();
		}
	}
};
Panel.prototype.addLine = function(page, $diagnose) {
	var line = new Line();
	line.id = this.id + (++this._lineIndex);
	line.panel = this;
	line.page = page;
	line.$diagnose = $diagnose;
	return (this.map[line.id] = line);
};
...
Panel.prototype.refresh = function() {
	...
	if ((self.page == syra_site) || (self.page.isPageLoaded && !self.page._isDataChanging)) {
		var count = self.stack.length;
		var visibleFieldCount = 0;
		for (var ii = 0, jj = self.fieldStack.length; ii < jj; ii++) {
			var line = self.fieldStack[ii];
			if (line.field && !line.field.isAlertPanelHidden) {
				visibleFieldCount++;
			}
		}
		count += visibleFieldCount;
		var showNext = visibleFieldCount > 1;
		if (count) {
