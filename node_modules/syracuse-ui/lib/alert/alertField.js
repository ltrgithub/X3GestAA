...
function _getFieldItems(panel, field) {
	...
	for (var ii = 0, jj = panel.fieldStack.length; ii < jj; ii++) {
		if (panel.fieldStack[ii].field == field) {
			items.push(panel.fieldStack[ii]);
		}
	}
	return items;
}
...
function _onClickGoToField() {
	...
	if (this.fieldArticle) {
		if (this.fieldArticle.rowCardBtn) {
...
function _showBindMessage(panel, message) {
...
exports.show = function(panel, message) {
	if (message.field) {
		...
		_showBindMessage(panel, message);
	}
};
