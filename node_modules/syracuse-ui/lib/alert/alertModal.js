...
function _preformat(txt) {
	return "<pre class='s-alert-md-pre'>" + syra_dom.formatHTMLText(txt, true) + "</pre>";
}
...
function _addDiagnoses(page, $diagnoses) {
	...
	for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
		...
		message.label.className = "s-alert-md-label";
...
function _onMessageClick() {
	...
	if (this.isOpened) {
		if (!this.message.stackTraceView) {
			this.message.stackTraceView = syra_dom.addDiv("s-alert-md-diag-stackTrace", this.message.label);
			this.message.stackTraceView.innerHTML = _preformat(((this.message.appCode != undefined ? this.message.appCode + "\n" : "") + (this.message.stackTrace || this.message.$stackTrace)));
		}
	}
...
exports.AlertModal = helpers.defineClass(AlertModal, DesktopPage, {
	...
	drawPage: function() {
		...
		if (this.$item.$origin) {
			...
			// add to formatted tab content
			$(syra_dom.addDiv("s-alert-md-origin", this._coreFormatted)).html(fHtmlEsc(origin));
			// add to raw tab content
			this._coreRaw.innerHTML += _preformat(this.$item.$origin + "<br/>");
		}
		if (this.$item.$details) {
			_ensureStackTrace(this);
			$(syra_dom.addDiv("s-alert-md-details", this._coreFormatted)).html((fHtmlEsc(this.$item.$details)).replace(/\n/g, '<br/>'));
