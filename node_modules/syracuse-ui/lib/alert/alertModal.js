"use strict";
var helpers = require('@sage/syracuse-core').helpers;
var DesktopPage = require("syracuse-ui/lib/page/desktopPage").DesktopPage;

function _addDiagnoses(page, $diagnoses) {
	_ensureStackTrace(page);
	// add to formatted tab content
	var diagnoses = syra_dom.div("s-alert-md-diagnoses", page._coreFormatted);

	page._groups = {};
	var severities = ["error", "fatal", "warning", "info"];
	for (var ii = 0, jj = severities.length; ii < jj; ii++) {
		var severity = severities[ii];
		page._groups[severity] = {
			groupItem: syra_dom.div('s-alert-md-group-' + severity),
			messages: []
		};
	}
	for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
		var $diagnose = $diagnoses[ii];
		var severity = $diagnose.severity || $diagnose.$severity;
		var group = null;
		if (severity) {
			severity = severity.toLowerCase();
			group = page._groups[severity];
		}
		if (!group) {
			group = page._groups[severity = "error"];
		}
		var _code = $diagnose.appCode != null && $diagnose.appCode != undefined ? $diagnose.appCode : ($diagnose.sdataCode != null && $diagnose.sdataCode != undefined ? $diagnose.sdataCode : null);
		var message = {
			label: syra_dom.div("s-alert-md-label"),
			stackTrace: $diagnose.stackTrace || $diagnose.$stackTrace,
			appCode: _code != null ? (syra_local.msgbox_diag_appCode + _code) : undefined
		};
		group.messages.push(message);
		syra_dom.pre("s-pre", $diagnose.message || $diagnose.$message || syra_local.msgbox_diagnose_default_msg, message.label);

		if ($diagnose.stackTrace || $diagnose.$stackTrace) {
			syra_button.add({
				parent: page,
				slot: message.label,
				text: syra_local.msgbox_diagnose_show_detail,
				iconOnly: true,
				css: "s-alert-md-diag-btn",
				fontIcon: "expand_l",
				message: message,
				click: _onMessageClick
			});
		}
		var flag = document.createElement("em");
		flag.className = "s-alert-md-label-" + severity;
		flag.textContent = ".";
		message.label.insertBefore(flag, message.label.firstChild);
		group.groupItem.appendChild(message.label);
		if ($diagnose.$links || $diagnose.$actions) {
			page.applyChange($diagnose);
		}
		diagnoses.appendChild(group.groupItem);
	}
	syra_dom.pre("s-pre", JSON.stringify($diagnoses, null, 2), page._coreRaw);
}

function _ensureStackTrace(page) {
	if (!page._core) {
		var footer = page.idMap["$footer"];
		page._coreTabsNav = document.createElement("nav");
		page._coreTabsNav.className = "s-alert-md-core-tabs-nav";
		page._coreTabsNav.style.display = "none";
		footer.domItem.parentNode.insertBefore(page._coreTabsNav, footer.domItem);

		page._core = syra_dom.div("s-alert-md-core");
		page._core.style.display = "none";
		footer.domItem.parentNode.insertBefore(page._core, footer.domItem);

		// tabs
		page._openedTab = syra_button.add({
			parent: page,
			slot: page._coreTabsNav,
			text: syra_local.msgbox_nav_title_formatted,
			css: "s-alert-md-core-tab s-open",
			click: _onTabClick
		});
		syra_button.add({
			parent: page,
			slot: page._coreTabsNav,
			text: syra_local.msgbox_nav_title_raw,
			css: "s-alert-md-core-tab",
			click: _onTabClick
		});

		// tabs content
		page._coreFormatted = syra_dom.div("s-alert-md-core-tab-formatted", page._core);

		page._coreRaw = syra_dom.div("s-alert-md-core-tab-raw");
		page._coreRaw.style.display = "none";
		page._core.appendChild(page._coreRaw);

		var slot = syra_dom.div("s-alert-md-btn-slot");
		page._bodyBtn = syra_button.add({
			parent: page,
			slot: slot,
			text: syra_local.msgbox_body_picker_closed,
			css: "s-msg-box-body-btn",
			fontIcon: "expand_s",
			click: function() {
				var msgBox = this.parent;
				syra_dom.toggle(msgBox._coreTabsNav);
				syra_dom.toggle(msgBox._core);
				this.isOpened = !this.isOpened;
				syra_button.setText(this, this.isOpened ? syra_local.msgbox_body_picker_opened : syra_local.msgbox_body_picker_closed, this.isOpened ? "collapse_s" : "expand_s");
				syra_button.hide(msgBox._selectBtn, !this.isOpened);
				msgBox.resizeItem();
			}
		});
		page._message.parentNode.insertBefore(slot, page._message);

		slot = syra_dom.div("s-alert-md-btn-slot");
		page._selectBtn = syra_button.add({
			parent: page,
			slot: slot,
			text: syra_local.msgbox_select_picker,
			css: "s-alert-md-select-btn",
			isHidden: true,
			click: function() {
				this.parent.selectText();
			}
		});
		page._message.parentNode.insertBefore(slot, page._message);
	}
}

function _addAutoClose(page) {
	var count = (typeof(page.$item.$isAutoClose) == "number" ? page.$item.$isAutoClose : 4000) / 1000;
	page.domTitle.textContent = page.$item.$title + "  (" + count + ")";
	page.autoCloseTimeOut = setInterval(function() {
		var countTxt = "(" + (--count) + ")";
		page.domTitle.textContent = page.$item.$title + " " + countTxt;
		if (count == 0) {
			clearInterval(page.autoCloseTimeOut);
			page.close("auto");
		}
	}, 1000);
}

function _onTabClick() {
	var msgBox = this.parent;
	msgBox._openedTab.link.className = "s-alert-md-core-tab";
	(msgBox._openedTab = this).link.className = "s-alert-md-core-tab s-open";
	syra_dom.toggle(msgBox._coreFormatted);
	syra_dom.toggle(msgBox._coreRaw);
	msgBox.resizeItem();
}

function _onMessageClick() {
	this.isOpened = !this.isOpened;
	syra_button.setText(this, this.isOpened ? syra_local.msgbox_diagnose_hide_detail : syra_local.msgbox_diagnose_show_detail, this.isOpened ? "collapse_l" : "expand_l");
	var msg = this.message;
	if (this.isOpened) {
		if (!msg.stackTraceView) {
			msg.stackTraceView = syra_dom.div("s-alert-md-stackTrace", msg.label);
			syra_dom.pre("s-pre", ((msg.appCode != undefined ? msg.appCode + "\n" : "") + (msg.stackTrace || msg.$stackTrace)), msg.stackTraceView);
		}
	}
	syra_dom.hide(this.message.stackTraceView, !this.isOpened);
	this.parent.resizeItem();
}

//$default = isDefault: true severity 
//$isAutoClose pour question 
function AlertModal() {}

exports.AlertModal = helpers.defineClass(AlertModal, DesktopPage, {
	load: function(initData) {
		this.isMenuBarDisabled = true;
		this.$item.$skin = "s-alert-md";
		if (!this.$item.$newButtons) {
			this.$item.$newButtons = [{
				id: "ok"
			}];
		}
		this.isAlertModal = true;
		this.$prototype.$links = {};
		var $itemButtons = [];
		for (var ii = 0, jj = this.$item.$newButtons.length; ii < jj; ii++) {
			var $btn = this.$item.$newButtons[ii];
			this.$prototype.$links[$btn.id] = {
				$title: $btn.title || syra_local["msgbox_" + $btn.id]
			};
			$itemButtons.push({
				$modalBtn: $btn.id,
				$bind: $btn.id
			});
			if ($btn.isDefault || $btn.id == "ok") {
				this._defaultBtn = $btn;
			}
		}
		if (!this._defaultBtn) {
			this._defaultBtn = this.$item.$newButtons[0];
		}

		this.$prototype.$properties = {
			$message: {
				$type: "application/x-string",
				$title: "$isTitleHidden",
			}
		};
		this.$item.$layout = {
			$items: [{
				$bind: "$message",
				$isTitleHidden: true,
				$skin: this.$item.$skin + "-msg"
			}]
		};
		this.$item.$layout.$items.push({
			$skin: "s-alert-md-foot",
			$id: "$footer",
			$category: "menus",
			$layout: {
				$items: [{
					$category: "menus",
					$skin: "s-alert-md-links",
					$isMenusBag: true,
					$isHidden: this.$item.$useLinkAsOk
				}, {
					$skin: "s-alert-md-buttons",
					$layout: {
						$layoutType: "row",
						$items: $itemButtons
					}
				}]
			}
		});

		DesktopPage.prototype.load.call(this, initData);

		if (this.$item.layoutSlot) {
			(this.layoutSlot = this.$item.layoutSlot).appendChild(this.domItem);
			this.domItem.style.display = "inline-block";
		} else {
			this.domItem.style.position = "fixed";
			this.overPanel = syra_over.openModal(syra_site, {
				content: this,
				$zIndex: syra_dom.getTopZIndex() + 5000, //ensure at top
				$isAutoClose: false,
				$autoFocus: true,
				$center: true
			});
		}
		this.keys = {};
		this.$item.$showDetail && this._bodyBtn.link.click();
		this.focusBtnId = this._defaultBtn.id;

		syra_quality.alertModal(this);
	},

	selectText: function() {
		if (this._coreFormatted) {
			var textItem = this._coreFormatted && syra_dom.isDisplay(this._coreFormatted) ? this._coreFormatted : this._coreRaw;
			if (document.body.createTextRange) { //ms
				var range = document.body.createTextRange();
				range.moveToElementText(textItem);
				range.select();
			} else
			if (window.getSelection) { //all others
				var selection = window.getSelection();
				var range = document.createRange();
				range.selectNodeContents(textItem);
				selection.removeAllRanges();
				selection.addRange(range);
			}
		}
	},
	drawPage: function() {
		DesktopPage.prototype.drawPage.call(this);
		if (this.domTitle) {
			this.domTitle.className += " s-alert-md-msg-" + (this.$item.$severity || this.$item.$type);
		}
		this.domItem.syraItem = this.id;
		this.layoutSlot = this.domItem;
		var msgField = this.boundFields.$message[0];
		this._message = msgField.domItem;
		syra_dom.empty(msgField._dataValue);
		syra_dom.pre("s-pre", this.$item.$message || syra_local.msgbox_default_message, msgField._dataValue);

		// links (AFTER MSG ADDITION !)
		if (this.$item.$links || this.$item.$actions) {
			this.$item.noDispDiag = true;
			this.applyChange(this.$item);
		}
		if (this.$item.$origin) {
			_ensureStackTrace(this);
			syra_dom.pre("s-pre", syra_local.msgbox_body_stackTrace_origin + this.$item.$origin, syra_dom.div("s-alert-md-origin", this._coreFormatted));
			syra_dom.pre("s-pre", this.$item.$origin, this._coreRaw);
		}
		if (this.$item.$details) {
			_ensureStackTrace(this);
			syra_dom.pre("s-pre", this.$item.$details, syra_dom.div("s-alert-md-details", this._coreFormatted));
			syra_dom.pre("s-pre", this.$item.$details, this._coreRaw);

		}
		if (this.$item.$diagnoses && this.$item.$diagnoses.length > 0) {
			_addDiagnoses(this, this.$item.$diagnoses);
		}
		this.getMenuItem(this._defaultBtn.id).domItem.className += " s-alert-md-button-default";
		this.$item.$isAutoClose && _addAutoClose(this);
	},
	onMenuClick: function(options) {
		switch (options.menu.$bind) {
			case "yes":
			case "no":
			case "ok":
			case "cancel":
				if (this.$item.$useLinkAsOk && options.menu.$bind == "ok") {
					this.applyChange({
						$links: this.$item.$useLinkAsOk
					});
					var $bind = Object.keys(this.$item.$useLinkAsOk)[0];
					this.menuItems[$bind][0].domItem.click();
				} else {
					this.close(options.menu.$bind, {
						$id: options.menu.$bind
					});
				}
				return false;
		}
		return true;
	},
	resizeItem: function(setMinimumDimensions) {
		if (this._core) {
			this._core.style.height = "";
		}
		this.overPanel && this.overPanel.resize();

		if (this._core) {
			var diff = this.domItem.scrollHeight - this.domItem.clientHeight;
			if (diff) {
				this._core.style.height = Math.max((this._core.getBoundingClientRect().height - diff), 50) + "px";
			}
		}
	},
	applyEscape: function() {
		this.close("esc");
		return true;
	},
	close: function(closedBy, response) {
		if (this.$item) {
			if (response && response.$id) {
				this.$item[response.$id] && this.$item[response.$id]();
			}
			//for convergence compatiblity
			this.$item.callback && this.$item.callback(response, closedBy); //for convergence compatiblity
			if (response && response.$id) {
				var $btns = this.$item.$newButtons;
				for (var ii = 0, jj = $btns.length; ii < jj; ii++) {
					if ($btns[ii].click && $btns[ii].id == response.$id) {
						$btns[ii].click();
						break;
					}
				}
			}
			this.$item.click && this.$item.click(response, closedBy);
			syra_alert.dispose();
		}
	},
	dispose: function() {
		if (this.$item) {
			delete this.$item.callback;
			delete this.$item.click;
		}
		syra_dom.remove(this.domItem);
		if (this._groups) {
			var ids = Object.keys(this._groups);
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				var id = ids[ii];
				this._groups[id].groupItem = null;
			}
		}
		if (this.autoCloseTimeOut) {
			clearInterval(this.autoCloseTimeOut);
		}
		this.overPanel && this.overPanel.dispose();
		DesktopPage.prototype.dispose.call(this);
	}
});