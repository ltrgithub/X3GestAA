...
function _addDiagnoses(page, $diagnoses) {
	...
	var diagnoses = syra_dom.addDiv("s-alert-md-diagnoses", page._coreFormatted);
	...
	for (var ii = 0, jj = severities.length; ii < jj; ii++) {
		...
		page._groups[severity] = {
			groupItem: syra_dom.addDiv('s-alert-md-group-' + severity),
			messages: []
		};
	}
	for (var ii = 0, jj = $diagnoses.length; ii < jj; ii++) {
		...
		var message = {
			label: syra_dom.addDiv("s-alert-md-label"),
			stackTrace: $diagnose.stackTrace || $diagnose.$stackTrace,
			appCode: _code != null ? (syra_local.msgbox_diag_appCode + _code) : undefined
		};
		group.messages.push(message);
		syra_dom.addPreformatText("s-pre", $diagnose.message || $diagnose.$message || syra_local.msgbox_diagnose_default_msg, message.label);
	...
	syra_dom.addPreformatText("s-pre", JSON.stringify($diagnoses, null, 2), page._coreRaw);
}
...
function _ensureStackTrace(page) {
	if (!page._core) {
		...
		page._core.className = "s-alert-md-core";
		...
		// tabs content
		page._coreFormatted = syra_dom.addDiv("s-alert-md-core-tab-formatted", page._core);

		page._coreRaw = syra_dom.addDiv("s-alert-md-core-tab-raw");
		page._coreRaw.style.display = "none";
		page._core.appendChild(page._coreRaw);

		var slot = syra_dom.addDiv("s-alert-md-btn-slot");
		...
		slot = syra_dom.addDiv("s-alert-md-btn-slot");
...
function _onMessageClick() {
	...
	if (this.isOpened) {
		if (!msg.stackTraceView) {
			msg.stackTraceView = syra_dom.addDiv("s-alert-md-stackTrace", msg.label);
			syra_dom.addPreformatText("s-pre", ((msg.appCode != undefined ? msg.appCode + "\n" : "") + (msg.stackTrace || msg.$stackTrace)), msg.stackTraceView);
		}
	}
...
exports.AlertModal = helpers.defineClass(AlertModal, DesktopPage, {
	...
	drawPage: function() {
		...
		syra_dom.addPreformatText("s-pre", this.$item.$message || syra_local.msgbox_default_message, msgField._dataValue);
		...
		if (this.$item.$origin) {
			...
			syra_dom.addPreformatText("s-pre", syra_local.msgbox_body_stackTrace_origin + this.$item.$origin, syra_dom.addDiv("s-alert-md-origin", this._coreFormatted));
			syra_dom.addPreformatText("s-pre", this.$item.$origin, this._coreRaw);
		}
		if (this.$item.$details) {
			_ensureStackTrace(this);
			syra_dom.addPreformatText("s-pre", this.$item.$details, syra_dom.addDiv("s-alert-md-details", this._coreFormatted));
			syra_dom.addPreformatText("s-pre", this.$item.$details, this._coreRaw);
		...
		this.getMenuItem(this._defaultBtnId).domItem.className += " s-alert-md-button-default";
