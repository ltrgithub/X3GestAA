...
exports.openPopup = function(menuItem) {
	if (!_popupSlot) {
		_popupSlot = syra_dom.addDiv("s-mn-popup-body");
	...
	if (!_popup) {
		_popup = syra_dlg.openPopup(syra_site, {
			content: syra_site,
			slot: _popupSlot,
		...
		success: function(data, response, $url) {
			...
			syra_ajax.get({
				...
				success: function(data, response, $url) {
					...
					syra_dlg.openModal(menuItem.page, {
						page: page
					});
				},
				error: function(error) {
					if (error.data.indexOf("$diagnoses") != -1) {
						syra_alert.error(JSON.parse(error.data).$diagnoses);
					} else {
						syra_alert.error(error.data);
					}
				}
			});
