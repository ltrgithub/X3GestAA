...
var _$eulaUrl = "/syracuse-ui/lib/help/SAGE X3 EULA.pdf";
			...
			$layout: {
				$items: [{
					...
					renderRecordContent: function(record) {
						...
						syra_dom.addText("s-legal-item-product", record.dataset.product, head);
						syra_dom.addText("s-legal-item-version", record.dataset.productVersion, head);
						...
						for (var ii = 0, jj = $rows.length; ii < jj; ii++) {
							...
							for (var mm = 0, kk = $cells.length; mm < kk; mm++) {
								...
								var $bind = $cells[mm];
								syra_dom.addText("s-legal-item-field-title", record.$prototype.$properties[$bind].$title, row);
