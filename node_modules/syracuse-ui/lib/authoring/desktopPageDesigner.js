...
var DragDropItems = require("syracuse-ui/lib/authoring/tools/dragDropItems").DragDropItems;
...
exports.DesktopPageDesigner = helpers.defineClass(DesktopPageDesigner, Designer, {
	...
	endTargetPageUpdate: function(selectedItem, isStructureUpdated) {
		...
		if (selectedItem && selectedItem.$designLevel == "field" && selectedItem.boxParent) {
			...
			this.uiSwitchTool.toggleUIDesign(this.awArticle.page, true);
		}
	...
	onClickPicker: function(picker) {
		...
		if (this.viewsTool.onClickPicker(picker)) {
			return false;
		}
	...
	onDragMove: function(target, event) {
		if (document.site.ddManager.requestedDDAuthoringItem) {
			if (document.site.ddManager.requestedDDAuthoringItem.syraSepType == "row") {
				document.site.ddManager.start(this, RowResizer, this.awArticle.page.$$scrollview);
				return;
			}
		}
		if (target.className && target.className.indexOf("s-aw-item") >= 0 || target.className.indexOf("s-aw-add-item") >= 0) {
			var item = this.findItem(target, undefined, event);
			if (item) {
				if (item != this.awArticle.page) {
					if (document.site.ddManager.requestedDDAuthoringItem) {
						document.site.ddManager.start(this, DragDropItems, this.awArticle.page.$$scrollview);
					} else {
						if (event.target.className.indexOf("s-aw-slot-sep-row") >= 0 ||
							event.target.className.indexOf("s-aw-slot-sep-stack") >= 0) {
							item = this.findItem(event.target, undefined, event);
						}
						if (document.site.ddManager.dragDropInstance) {
							document.site.ddManager.dragDropInstance.onDragMouseMove(target, event, item);
						}
					}
				}
	...
	findItem: function(item, findLayout, event) {
		if (item.syraAwTreeNodeId) {
			var node = this.treebar.findNode(event);
	...
	dispose: function() {
		...
		if (this.uiSwitchTool) {
			this.uiSwitchTool.toggleUIDesign(this.awArticle.page, false, true);
		}
