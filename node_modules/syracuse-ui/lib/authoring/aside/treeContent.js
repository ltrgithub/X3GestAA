...
exports.TreeContent = helpers.defineClass(TreeContent, null, {
    load: function(toolsBar){
        var self = this;
        self.toolsBar = toolsBar;
        document.site.$$layoutSlot.delegate(".s-aw-content-tree", "mousedow.authoringTree click.authoringTree", function(event){
    ...
    dispose: function(){
        document.site.$$layoutSlot.undelegate(".authoringTree");
        if (this.block) {
            this.block = this.block.onResize = null;
        }
        this.toggle(false, true);
        this.selectedNode = this.toolsBar = null;
    },
    toggle: function(show, disposing){
        var self = this;
        if (self.toolsBar.authorPage.targetPage) {
            if (show) {
                self._nodes = {};
                if (!self.slot) {
                    self.slot = document.createElement("div");
                    self.slot.className = "s-aw-content-tree";
                    self.toolsBar.idMap.$treeContentBlock.layoutContent._item.appendChild(self.slot);
                    self.block = self.toolsBar.blocks.$treeContentBlock;
                    self.block.onResize = function(height){
                        self.slot.style.height = height + "px";
                    };
                }
                self.slot.style.display = "none";
                document.site.emptyDom(self.slot);
                self._buildTreeNode(self.toolsBar.authorPage.targetPage, {
                    node: self.slot,
                    isRoot: true
                });
    ...
    onNodeEvent: function(event){
        ...
        if (node) {
            if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
                ...
                switch (event.type) {
                    case "mouseenter":
                        this.toolsBar.authorPage.toggleOverItem(node.item, true);
                        this.toolsBar.authorPage.targetPage.scrollToItem(node.item);
                        break;
                    case "mouseleave":
                        this.toolsBar.authorPage.toggleOverItem(node.item, false);
                        break;
                    case "click":
                        if (node.item == this.toolsBar.authorPage.targetPage) {
                            this.toolsBar.authorPage.targetPage.scrollToItem(node.item);
                        }
                        this.selectNode(node.item, true);
                        this._isSelecteDisabled = true;
                        this.toolsBar.authorPage.selectItem(node.item, true);
    ...
    _buildTreeNode: function(item, parentNode){
        ...
        if (!item.isSpaceBox) {
            if (item.isLayout) {
                ...
                if (this.toolsBar.authorPage.awItem == item) {
