...
exports.TreeContent = helpers.defineClass(TreeContent, null, {
    ...
    toggle: function(show, disposing){
        var self = this;
        if (document.site.authorPage.targetPage) {
            if (show) {
                var palette = document.site.authorPage.palette;
                self._nodes = {};
                if (!self.slot) {
                    self.slot = document.createElement("div");
                    self.slot.className = "s-aw-content-tree";
                    palette.idMap.$treeContentBlock.layoutContent._item.appendChild(self.slot);
                    for (var ii = 0, jj = palette.toolsBar.blocks.length; ii < jj; ii++) {
                        if (palette.toolsBar.blocks[ii].$item.$id == "$treeContentBlock") {
                            self.block = palette.toolsBar.blocks[ii];
                            break;
                        }
                    }
                ...
                self._buildTreeNode(document.site.authorPage.targetPage, {
                    node: self.slot,
                    isRoot: true
                });
            ...
            if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
                ...
                switch (event.type) {
                    case "mouseenter":
                        ...
                        authorPage.targetPage.scrollToItem(node.item);
                        break;
                    case "mouseleave":
                        authorPage.toggleOverItem(node.item, false);
                        break;
                    case "click":
                        if (node.item == authorPage.targetPage) {
                            authorPage.targetPage.scrollToItem(node.item);
                        }
