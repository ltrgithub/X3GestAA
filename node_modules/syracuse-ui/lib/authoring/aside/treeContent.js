...
exports.TreeContent = helpers.defineClass(TreeContent, null, {
    ...
    dispose: function(){
        ...
        this.awContext = this.selectedNode = null;
    },
    toggle: function(show, disposing){
        var self = this;
        if (self.awContext.targetPage) {
            if (show) {
                self._nodes = {};
                if (!self.slot) {
                    self.slot = document.createElement("div");
                    self.slot.className = "s-aw-content-tree";
                    self.awContext.toolsBar.idMap.$treeContentBlock.layoutContent._item.appendChild(self.slot);
                    self.block = self.awContext.toolsBar.blocks.$treeContentBlock;
                ...
                self._buildTreeNode(self.awContext.targetPage, {
                    node: self.slot,
                    isRoot: true
                });
            ...
            if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
                ...
                switch (event.type) {
                    ...
                    case "mouseleave":
                        ...
                        break;
                    case "click":
                        if (node.item == this.awContext.targetPage) {
                            this.awContext.targetPage.scrollToItem(node.item);
                        }
