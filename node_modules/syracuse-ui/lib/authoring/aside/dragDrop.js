...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
	start: function(awPalette, $$boundary) {
		var self = this;
		self.awPalette = awPalette;
		self.isDragAuthoring = true;
		self.dropItem = document.site.requestedDDAuthoringItem;
		if (self.dropItem.isLayout) {
			drawHelper.defineLayoutChildAuhtoringLevel(self.dropItem);
		}
			...
			if (self.targetItem) {
				...
				if (targetItem && dropItem) {
					...
					if (enable) {
						...
						var newChild = drawHelper.onDropItem(self.awPalette, targetItem, dropItem, $insertAt, self.$drag);
						self.awPalette.authorPage.endTargetPageUpdate(newChild, true);
					}
				}
			}
			self.stop();
				...
				if (this.dropItem.isLayout) {
					...
					if (this.dropItem.$authoringLevel == "field") {
						...
						if (targetItem.$authoringLevel == "field") {
							...
							if (!sourceBox) {
								text = this.awPalette.authorPage.localize.dd_moveFields;
								isDropEnabled = true;
							}
						} else {
							isDropEnabled = true;
				...
				if (isDropEnabled) {
					if (!text) {
						if (this.dropItem.$awAddNewItem) {
							text = this.awPalette.authorPage.localize.dd_addItem;
						} else {
							text = this.awPalette.authorPage.localize.dd_moveItem;
						}
					}
		...
		if (!this.dropItem.isLayout && this.targetItem) {
			if (this.targetItem.isLayout) {
				if (!this.targetItem.$chilAuthoringLevel) {
					drawHelper.defineLayoutChildAuhtoringLevel(this.targetItem);
				}
	...
	getDropItemTitle: function(dropItem) {
		...
		if (!title) {
			title = this.awPalette.authorPage.localize[dropItem.isLayout ? "aw_rowLayout" : "aw_" + dropItem.$authoringLevel];
		}
		return title;
	},
		...
		if (!isOut && this.isDropEnabled) {
			...
			if (isOut) {
				...
				if (this.dropItem.$awAddNewItem) {
					...
					localText = this.awPalette.authorPage.localize.dd_addItem;
				} else {
					title = this.dropItem.getTitle();
					localText = this.awPalette.authorPage.localize.dd_moveItem;
				}
				if (!title) {
					title = this.awPalette.authorPage.localize["aw_" + this.dropItem.$authoringLevel];
				}
				this._info.textContent = localText.replace("{0}", title);
			}
	...
	toggleDropArea: function(targetItem, dropItem) {
		if (targetItem && targetItem.isSpaceBox) {
			this.awPalette.authorPage.toggleDropArea(targetItem, dropItem);
		}
	},
	dispose: function() {
		...
		this.awPalette = this.dropItem = this.boundary = this._info = this._dropCue = null;
	}
});