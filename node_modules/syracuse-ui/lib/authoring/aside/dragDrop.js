            ...
            if (self.targetItem) {
                ...
                if (targetItem && dropItem) {
                    ...
                    if (enable) {
                        ...
                        var newChild;
                        if (self.$drag.isTab) {
                            if (self.$drag.isTabBody || (self.$drag.$insertAt == "bottom" || self.$drag.$insertAt == "top")) {
                                newChild = authorPage.onDropTabBox(targetItem, dropItem, self.$drag);
                            }
                        }
                        if (!newChild) {
                            if (dropItem.$awAddNewItem) {
                                if (dropItem.$fieldBind) {
                                    authorPage.onAddNewField(targetItem, dropItem);
                                }
                                else {
                                    newChild = authorPage.addNewItem(targetItem, dropItem.$authoringLevel, null, undefined, $insertAt, dropItem.$category);
                                }
                            }
                            else {
                            
                                newChild = authorPage.addNewItem(targetItem, dropItem.$authoringLevel, dropItem.$bind, dropItem, $insertAt);
                            }
                        }
                ...
                if (isDropEnabled) {
                    if (!text) {
                        if (this.dropItem.$awAddNewItem) {
                            text = document.site.authorPage.localize.dd_addItem;
                        }
                        else {
                            text = document.site.authorPage.localize.dd_moveItem;
                        }
                    }
        ...
        if (!isOut && this.isDropEnabled) {
            ...
            if (isOut) {
                ...
                if (this.dropItem.$awAddNewItem) {
                    ...
                    localText = document.site.authorPage.localize.dd_addItem;
                }
                else {
                    title = this.dropItem.getTitle();
                    localText = document.site.authorPage.localize.dd_moveItem;
                }
                if (!title) {
