...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
	start: function(awContext, $$boundary) {
		var self = this;
		...
		if (self.dropItem.isLayout) {
			self.awContext.drawHelper.defineLayoutChildAuhtoringLevel(self.dropItem);
		}
			...
			if (self.targetItem) {
				...
				if (targetItem && dropItem) {
					...
					if (enable) {
						...
						var newChild = self.awContext.drawHelper.onDropItem(self.awContext, targetItem, dropItem, $insertAt, self.$drag);
						self.awContext.authorPage.endTargetPageUpdate(newChild, true);
					}
				}
			}
			self.stop();
				...
				if (this.dropItem.isLayout) {
					...
					if (this.dropItem.$authoringLevel == "field") {
						...
						if (targetItem.$authoringLevel == "field") {
							...
							if (!sourceBox) {
								text = this.awContext.localize.dd_moveFields;
								isDropEnabled = true;
							}
						} else {
							isDropEnabled = true;
				...
				if (isDropEnabled) {
					if (!text) {
						if (this.dropItem.$awAddNewItem) {
							text = this.awContext.localize.dd_addItem;
						} else {
							text = this.awContext.localize.dd_moveItem;
						}
					}
		...
		if (!this.dropItem.isLayout && this.targetItem) {
			if (this.targetItem.isLayout) {
				if (!this.targetItem.$chilAuthoringLevel) {
					this.awContext.drawHelper.defineLayoutChildAuhtoringLevel(this.targetItem);
				}
	...
	getDropItemTitle: function(dropItem) {
		...
		if (!title) {
			title = this.awContext.localize[dropItem.isLayout ? "aw_rowLayout" : "aw_" + dropItem.$authoringLevel];
		}
		return title;
	},
		...
		if (!isOut && this.isDropEnabled) {
			...
			if (isOut) {
				...
				if (this.dropItem.$awAddNewItem) {
					...
					localText = this.awContext.localize.dd_addItem;
				} else {
					title = this.dropItem.getTitle();
					localText = this.awContext.localize.dd_moveItem;
				}
				if (!title) {
					title = this.awContext.localize["aw_" + this.dropItem.$authoringLevel];
				}
	...
	toggleDropArea: function(targetItem, dropItem) {
		if (targetItem && targetItem.isSpaceBox) {
			this.awContext.authorPage.toggleDropArea(targetItem, dropItem);
		}
	},
	dispose: function() {
		...
		this.awContext = this.dropItem = this.boundary = this._info = this._dropCue = null;
	}
});