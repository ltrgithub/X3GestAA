...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
	start: function(awPalette, $$boundary) {
		...
		self.localize = document.site.authorPage.localize;
			...
			if (self.targetItem) {
				...
				if (self.dropItem) {
					...
					if (enable) {
						...
						document.site.authorPage.endTargetPageUpdate(newChild, true);
					}
				}
			}
			self.stop();
						...
						if (this.dropItem.$authoringLevel == "field") {
							...
							if (targetItem.$authoringLevel == "field") {
								if (!this.dropItem.$awAddNewItem) {
									...
									if (!sourceBox) {
										this.infoText = this.localize.dd_moveFields;
										isDropEnabled = true;
									}
								}
							} else {
	...
	getDropItemTitle: function(dropItem) {
		...
		if (!title) {
			title = this.localize[dropItem.isLayout ? "aw_rowLayout" : "aw_" + dropItem.$authoringLevel];
		}
		return title;
	},
	...
	moveDragImage: function(event, targetItem, isOut) {
		...
		this._ensureDragImage();
		if (this.isDropEnabled) {
			this._info.className = "s-aw-drag-info-" + this.dropItem.$authoringLevel;
			var $position = null;
			if (event.target && event.target.className && event.target.className.indexOf) {
				if (this.targetItem.layoutParent || this.targetItem.isLayout) {
					...
					if ($position) {
						...
						var text = this.localize[this.infoText + this.$drag.$insertAt] || "";
			...
			if (isOut) {
				var title, localText = this.localize.dd_nodrop;
				if (this.dropItem.$awAddNewItem) {
				...
				if (!title) {
					title = this.localize["aw_" + this.dropItem.$authoringLevel];
				}
		...
		if (targetItem && targetItem.isSpaceBox) {
			...
			if (dropItem) {
				if (!targetItem.dropArea) {
					...
					targetItem.dropArea.className = "s-aw-drop-area-" + dropItem.$authoringLevel;
					targetItem.dropArea.textContent = this.localize["aw_drop_" + dropItem.$authoringLevel];
