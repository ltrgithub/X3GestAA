...
exports.ToolsBar = helpers.defineClass(ToolsBar, RawPage, {
    loadToolbar: function(palette){
        this.palette = palette;
        this.authorPage = this.palette.authorPage;
        ...
        this.$prototype.$localization = this.palette.$prototype.$localization;
        
        this.preferences = {
            isDocked: true,
            isCollapsed: this.authorPage.targetPage.dialogWrapper
        };
        ...
        this.authorPage.targetPage._item.insertBefore(this.layoutSlot, this.authorPage.targetPage._item.firstChild);
        
        this.treeContent = new TreeContent();
        this.treeContent.load(this);
    ...
    dispose: function(){
        ...
        this.treeContent = this.palette = this.authorPage = this.blocks = this.layoutSlot = this.localize = null;
    },
    ...
    _fillInsertBlock: function(addLinks){
        ...
        var fields = this.authorPage._extractFields(targetPage.layoutContent);
    ...
    drawBox: function(){
        ...
        this.createBlock({
            $category: "section",
            $id: "$treeContentBlock",
            $title: this.localize.aw_viewTypeContent,
            $layout: {
                $items: []
            }
        });
        var isDashboard = this.authorPage.targetPage.isDashBoard;
        this.createBlock({
            $category: "section",
            $id: "$insertBlock",
            $title: this.localize.aw_insert,
            $layout: {
                $items: [{
                    $category: "menus",
                    $itemIcon: {
        ...
        if (isDashboard) {
            this.createBlock({
                $category: "section",
                $id: "$insertLinks",
                $title: this.localize.aw_insert_links,
                $layout: {
                    $items: [{
                        $category: "menus",
                        $itemIcon: {
    ...
    collapseBlock: function(block, show, isFirstTime, onDrawBar){
        ...
        if (show) {
            this.treeContent.toggle(block.$item.$id == "$treeContentBlock");
            this.onWindowResize();
        }
        else {
    ...
    onMenuClick: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            ...
            case "pattern-headerTabs":
                this.palette.onChangeSep(menuItem);
                return false;
            default:
                if (menuItem.$item.$fieldBind) {
                    var newChild = this.authorPage.onAddNewField(awItem, menuItem.$item);
                    this.authorPage.endTargetPageUpdate(newChild, true);
                }
                else {
