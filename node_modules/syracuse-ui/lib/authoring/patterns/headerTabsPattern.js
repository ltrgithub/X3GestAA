...
exports.HeaderTabsPattern = helpers.defineClass(HeaderTabsPattern, null, {
    ...
    build: function($prototype, $sourceArticle){
        ...
        var $rootItems = authorPage.getItemSettings($sourceArticle.$layout);
        ...
        for (var ii = 0, jj = $rootItems.length; ii < jj; ii++) {
            ...
            var $children = authorPage.getItemSettings($parent.$layout);
    ...
    _buildBlock: function(authorPage, $prototype, $parent, $blocks){
        ...
        if ($blocks.length > 1) {
            ...
            for (var ii = 0, jj = $blocks.length; ii < jj; ii++) {
                var $block = $blocks[ii];
                if ($block.$layout) {
                    var $fields = authorPage.getItemSettings($block.$layout);
                    if ($fields.length > 0) {
                        var addFields = true;
                        if ($fields.length == 1) {
                            var $field = $fields[0];
            ...
            if ($blocks.length == 1) {
                var $fields = authorPage.getItemSettings($blocks[0].$layout);
