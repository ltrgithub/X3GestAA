...
exports.PageDesigner = helpers.defineClass(PageDesigner, Designer, {
	...
	appendSideBars: function(append) {
		...
		if (append) {
			...
			document.site.dom.removeChild(this.treebarSlot);
			document.site.dom.removeChild(this.propertiesSlot);
		}
	},
	drawBox: function() {
		...
		help.title = this.localize.aw_help;
		help.syraOnClick = "onDesignerHelpClick";
		help.className = "s-design-history-link-icon";
	...
	onAddRowClick: function(picker) {
		...
		}, (picker.syraAdd == "addRowAfter") ? ++index : index);
		this.endArticleUpdate(newRow, true);
	},
	onExcludeItemClick: function(picker) {
	...
	addExcludeMeLink: function(panelId) {
		if (panelId != "article") {
			...
			if (!this.selectedPanel.datastore.excludeMeLink) {
				...
				link.className = "s-aw-exclude-me";
				link.textContent = link.title = this.localize.aw_excludeMe;
				link.syraOnClick = "onExcludeItemClick";
				var slot = document.createElement("div");
				slot.className = "s-aw-exclude-me-slot";
				this.selectedPanel.datastore.excludeMeLink = slot.appendChild(link);
	...
	showSettingPanel: function() {
		if (this.panels) {
			...
			if (!this.panels[panelId]) {
				...
				this.propertiesBodySlot.syraItem = this.id;
			...
			for (var ii = 0, jj = sections.length; ii < jj; ii++) {
				...
				if (section.$item.$isTitleSection) {
					...
					document.site.dom.toggleClass(titleLocalization.domItem, "s-aw-title-localization", true);
	...
	resizePage: function(resize) {
		if (this.propertiesBodySlot) {
			...
			this.propertiesBodySlot.style.height = document.site.dom.getInnerSize(page.layoutSlot).height + "px";
		}
	...
	dispose: function() {
		...
		document.site.dom.removeChild(this.layoutSlot);
