...
function _addViewBtns(designer, slot) {
	...
	designer.onViewClick = function(event, btn) {
		...
		if (designer.$viewType) {
			syra_site.dom.toggleClass(designer[designer.$viewType + "_btn"].link, "s-selected", false);
		}
	...
	syra_site.dom.toggleClass(designer[(designer.$viewType = "viewTypeLayout") + "_btn"].link, "s-selected", true);

};

function _selectTreesTab(designer, tab, select) {
	if (select && designer._selectedTab && designer._selectedTab.tab != tab) {
		_selectTreesTab(designer, designer._selectedTab.tab, false);
	}
...
function _addTreesTab(designer, title) {
	var body = document.createElement("div");
	body.style.display = "none";
	body.className = "s-aw-content-tree";
	return {
		tab: syra_menus.button.add({
			parent: designer,
			slot: designer.treebarTabs,
			text: title,
			css: "new-s-aw-tlbr-tab",
			btnclick: function() {
				_selectTreesTab(this.parent, this.link, true);
			}
		}).link,
...
function _addTreeBar(designer) {
	designer.treebarBody = document.createElement("div");
	designer.treebarBody.className = "s-aw-treebar-body";

	var bar = designer.treesBar = new BarSplitter();
	bar.options = {
	...
	bar.resizeBar = function() {
		if (BarSplitter.prototype.resizeBar.call(this)) {
			if (designer.contentTree) {
				var height = syra_site.dom.getInnerSize(this.slot).height;
				var bodyHeight = Math.max(height - designer.treebarTabs.getBoundingClientRect().height, 20) + "px";
				designer.contentTree.tabItem.body.style.height = bodyHeight;
				designer.insertTree.tabItem.body.style.height = bodyHeight;
			}
		}
	};
	bar.layoutSlot = designer.propertiesSlot;
	bar.slot = designer.treebarSlot;
	bar.body = designer.treebarBody;
	bar.load(designer.designedArticle.page);
	...
	designer.fillTree = function() {
		if (this.designedArticle) {
			...
			if (this._selectedTab && this._selectedTab.tab) {
				_selectTreesTab(this, this._selectedTab.tab, true);
			}
		}
	};
	...
	(designer.contentTree = new ContentTree()).load(designer, _addTreesTab(designer, syra_local.aw_viewTypeContent));
	(designer.insertTree = new InsertTree()).load(designer, _addTreesTab(designer, syra_local.aw_insert));
	_selectTreesTab(designer, designer.contentTree.tabItem.tab, true);
}

function _togglePageBar(bar, isVisible, isDesigning) {
	if (bar) {
		if (isDesigning) {
			bar.slot.syraAllClickDisabled = true;
		} else {
			delete bar.slot.syraAllClickDisabled;
		}
		bar.slot.style.display = (isVisible && !bar.isHidden && bar.isSlotVisible) ? "" : "none";
		bar.resizeSplitter();
	}
}

function _ensureDesigneBar(designer, isDesigning) {
	isDesigning = isDesigning !== false;
	if (isDesigning) {
		designer.isDesignBarVisible = !(designer.$viewType == "viewTypePreview");
		designer.propertiesSlot.style.display = designer.isDesignBarVisible ? "" : "none";
		designer.treesBar.toggleBar(designer.isDesignBarVisible);
	} else {
		designer.isDesignBarVisible = false;
	}
	designer.asideParent.domItem.style.paddingLeft = designer.isDesignBarVisible ? (designer.treebarSlot.clientWidth + "px") : "";
	designer.asideParent.domItem.style.paddingRight = designer.isDesignBarVisible ? (designer.propertiesSlot.clientWidth + "px") : "";

	_togglePageBar(designer.designedArticle.menuBar, !designer.isDesignBarVisible, isDesigning);
	_togglePageBar(designer.designedArticle.fusionBar, !designer.isDesignBarVisible, isDesigning);
}

function _appendSideBars(designer, append) {
	designer.asideParent = designer.designedArticle.page;
	if (designer.asideParent.isVignettePage) {
		designer.asideParent = designer.asideParent.getMasterPage();
		syra_site.landingPageMaster.bar.toggleBar(!append);
		syra_site.landingPageMaster.bar.resizeSplitter();
	}
	if (append) {
		designer.asideParent.domItem.appendChild(designer.propertiesSlot);
		designer.asideParent.domItem.appendChild(designer.treebarSlot);
	} else {
		syra_site.dom.removeChild(designer.treebarSlot);
		syra_site.dom.removeChild(designer.propertiesSlot);
	}
}
...
exports.Designer = helpers.defineClass(function() {}, Designer, {
	...
	drawBox: function() {
		...
		_appendSideBars(this, true);
		_uiSwitchTool.load(this);
		if (this.designedArticle == this.designedArticle.page) {
			_addViewBtns(this, viewsSlot);
		}
		//var helpbtn = syra_menus.addIconButton(syra_local.aw_help, "s-design-history-btn-icon", "onDesignerHelpClick", null, "help");
		//helpbtn.syraItem = this.id;
		...
		//syra_site.designerHelpLink && this.leftTop.appendChild(syra_site.designerHelpLink.domItem);
	...
	onDesignerHelpClick: function() {
		syra_menus.click.menuId(syra_site, "$designerDocumentation");
	},
	...
	selectItem: function(item, select) {
		if (item && item.isMenusItem && item.referenceField) {
		...
		if (select) {
			...
			_settingPanel.show(this);
			this.awLayout && _uiSwitchTool.toggleItemCss(this.awLayout, "s-aw-designed", true);
			_uiSwitchTool.toggleItemCss(item, "s-aw-designed", true);
		} else {
			item = item || this.designedItem;
			item && _uiSwitchTool.toggleItemCss(item, "s-aw-designed", false);
			this.awLayout && _uiSwitchTool.toggleItemCss(this.awLayout, "s-aw-designed", false);
		}
		if (this.contentTree) {
