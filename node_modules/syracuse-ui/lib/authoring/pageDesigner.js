...
exports.PageDesigner = helpers.defineClass(PageDesigner, Designer, {
	...
	drawBox: function() {
		...
		var help = syra_menus.addIconButton(syra_local.aw_help, "s-design-history-btn-icon", "onDesignerHelpClick");
		help.style.backgroundImage = "url('/syracuse-ui/themes/desktop/sage/images/authoring/s-aw-help.png')";

		this.historySlot = document.createElement("div");
		this.historySlot.className = "s-design-history-slot";
		this.historySlot.appendChild(help);
	...
	ensureDesigneBar: function() {
		this.isDesignBarVisible = !(this.viewsTool && this.viewsTool.$viewType == "viewTypePreview");
		this.propertiesSlot.style.display = this.isDesignBarVisible ? "" : "none";
		this.treesBar.toggleBar(this.isDesignBarVisible);
		this.togglePageBar(this.designedArticle.menuBar, !this.isDesignBarVisible, true);
		this.togglePageBar(this.designedArticle.fusionBar, !this.isDesignBarVisible, true);
	},
	...
	_addExcludeMe: function(panelId) {
		if (panelId != "article") {
			...
			if (!this.selectedPanel.datastore.excludeMeLink) {
				...
				this.selectedPanel.datastore.excludeMeLink = slot.appendChild(btn);
	...
	showSettingPanel: function() {
		if (this.panels) {
			this.startChange();
			var panelId = this.designedItem.$designLevel;
			...
			if (!this.panels[panelId]) {
				...
				this.propertiesSlot.syraItem = this.id;
				this.panels[panelId] = this.loadNewItem(this.propertiesSlot, $item, this);
			}
			(this.selectedPanel = this.panels[panelId]).setTitle(syra_local["aw_" + panelId]);
			this.applyChange({
				$isAdvanced: false,
				$isHidden: false
			});
		...
		if (this.propertiesSlot) {
			...
			if (this.isDesignBarVisible) {
				if (page.scrollview && this.treesBar) {
					...
					this.treebarSlot.parentNode.style.paddingLeft = this.treebarSlot.clientWidth + "px";
				}
				this.propertiesSlot.style.height = syra_site.dom.getInnerSize(page.layoutSlot).height + "px";
				this.propertiesSlot.parentNode.style.paddingRight = this.propertiesSlot.clientWidth + "px";
			} else {
				this.designedArticle.menuBar && this.designedArticle.resizeSplitter(this.designedArticle.menuBar);
				this.designedArticle.fusionBar && this.designedArticle.resizeSplitter(this.designedArticle.fusionBar);
			}
		}
	...
	dispose: function() {
		if (this.propertiesSlot && this.propertiesSlot.parentNode) {
			this.propertiesSlot.parentNode.style.paddingRight = "";
			this.propertiesSlot.parentNode.style.paddingLeft = "";
		}
		if (this.designedArticle) {
			this.togglePageBar(this.designedArticle.menuBar, true);
			this.togglePageBar(this.designedArticle.fusionBar, true);
		}
		...
		this.itemTool = this.pageStorage = this.treebarSlot = this.propertiesSlot = this.historySlot = this.viewsSlot = null;
