...
exports.PageDesigner = helpers.defineClass(PageDesigner, Designer, {
	...
	drawBox: function() {
		...
		//right part
		this.designedArticle.page.domItem.appendChild(this.propertiesSlot);
		...
		this.designedArticle.page.domItem.insertBefore(this.treebarSlot, this.designedArticle.page.domItem.firstChild);
	...
	endArticleUpdate: function(selectedItem, isStructureUpdated) {
		this.designedArticle.page.layoutValidator.validate(this.designedArticle.layoutContent, true);
	...
	saveArticleUpdate: function() {
		if (this.pageStorage) {
			this.pageStorage.saveArticleUpdate();
		}
	},
		...
		if (this.panels) {
			...
			for (var ii = 0, jj = sections.length; ii < jj; ii++) {
				...
				if (section.$item.$isLabelSection) {
					if (this.designedItem.unknowMode || this.designedItem.$field.$type == "application/x-boolean") {
						isHidden = true;
					}
				}
	...
	resizeSideBars: function() {
		if (this.propertiesBodySlot && this.designedArticle.scrollview) {
