...
exports.PageDesigner = helpers.defineClass(PageDesigner, Designer, {
	...
	appendSideBars: function(append) {
		...
		if (this.asideParent.isVignettePage) {
			...
			this.asideParent.landingPageMaster.bar.toggleBar(!append);
		}
		if (append) {
	...
	drawBox: function() {
		...
		if (this.designedArticle == this.designedArticle.page) {
			(this.viewsTool = new ViewsTool()).load(this);
			this.viewTitleSlot = document.createElement("div");
			this.viewTitleSlot.className = "s-aw-view-title-slot";
			this.displayViewTitle(this.designedArticle.$selectedDesignView);
			this.viewsSlot.appendChild(this.viewTitleSlot);
		}
	...
	_addExcludeMe: function(panelId) {
		if (panelId != "article") {
			...
			var enable = !this.designedItem.$isMandatory;
			if (enable && this.designedItem.layoutContent) {
				var fields = this.designedItem.layoutContent.getFields();
				for (var ii = 0, jj = fields.length; enable && ii < jj; ii++) {
					enable = !fields[ii].$isMandatory;
				}
			}
			this.selectedPanel.datastore.excludeMeLink.style.display = enable ? "" : "none";
		}
	},
	...
	dispose: function() {
		...
		this.asideParent = this.itemTool = this.pageStorage = this.treebarSlot = this.propertiesSlot = this.historySlot = this.viewsSlot = null;
		this.leftTop = this.treesBar = this.panels = this.trees = this.selectedPanel = this.transactionLabel = null;
		syra_site.bodyRect = syra_site.layoutRect = null; //important for dialog resize
