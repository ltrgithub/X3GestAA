    ...
    start: function($$boundary){
        ...
        $(document).bind("mouseup.syradragdrop", function(event){
            ...
            if (self.targetItem) {
                ...
                var sourceItem = self.sourceItem;
                var $insertAt = self.$drag.$insertAt;
                if (targetItem && sourceItem) {
                    var sourceLayoutParent = sourceItem.layoutParent || sourceItem.layoutContent;
                    var sourceIndex;
                    if (sourceItem.layoutParent && sourceItem.layoutParent.$layout.$layoutType == "row") {
                        sourceIndex = sourceItem.layoutParent.getItemIndex(sourceItem.$item);
                    }
                    var newChild = authorPage.addNewItem(targetItem, sourceItem.$authoringLevel, sourceItem.$bind, sourceItem, self.$drag.$insertAt);
                    if (sourceIndex !== undefined && newChild.layoutParent != sourceLayoutParent) {
                        sourceLayoutParent.addSpaceBox(sourceIndex);
                    }
                    authorPage.ensureLayoutTree(sourceLayoutParent);
                    authorPage.ensureLayoutTree(sourceItem);
                    authorPage.ensureLayoutTree(newChild);
                    authorPage.autoSizeRow(newChild.layoutParent);
                    authorPage.autoSizeRow(sourceLayoutParent);
                    if (newChild != authorPage.awItem) {
                        authorPage.ensureLayoutTree(authorPage.awItem);
                    }
                    authorPage.selectItem(newChild, true, true);
