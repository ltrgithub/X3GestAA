...
exports.DragDrop = helpers.defineClass(DragDrop, null, {
        ...
        $(document).bind("mouseup.syradragdrop", function(event){
            ...
            if (self.targetItem) {
                ...
                if (targetItem && dropItem) {
                    ...
                    authorPage.validateTargetPageLayout();
        ...
        if (this.dropItem) {
            if (this.dropItem != targetItem) {
                ...
                if ($action) {
                    ...
                    this._info.textContent = text.replace("{0}", this.dropItem.getTitle());
                }
            }
        }
        return $action;
    ...
    moveDragCue: function($position){
        ...
        this._dropCue.className = "s-aw-drop-cue s-aw-drop-cue-" + (this.targetItem ? this.targetItem.$authoringLevel : this.dropItem.$authoringLevel);
        if ($position) {
    ...
    _ensureDragImage: function(){
        if (!this._info) {
            ...
            this._info.textContent = this.dropItem.getTitle();
        }
    },
    ...
    moveDragImage: function(event, targetItem, isOut){
        ...
        if (!isOut && this.$action) {
            ...
            if (isOut) {
                this._info.textContent = document.site.authorPage.localize.dd_moveItem.replace("{0}", this.dropItem.getTitle());
            }
