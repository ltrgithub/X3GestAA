...
function DesignDDAgent() {}

exports.DesignDDAgent = helpers.defineClass(DesignDDAgent, null, {
	start: function(dropItem, designer, boundaryItem) {
		this.designer = designer;
		this.dropItem = dropItem;
		if (this.dropItem.isLayout) {
			this.dropItem.setChildAuhtoringLevel();
		}
		this.toggleDraggedCss(true);
		this.boundary = document.site.getBoundingClientRect(boundaryItem);
		this.boundary.boundaryItem = boundaryItem;
		this.boundary.scrollWidth = boundaryItem.scrollWidth;
		this.boundary.scrollHeight = boundaryItem.scrollHeight;
	},
			...
			if (this.dropItem) {
				...
				if (enable) {
					if (this.$drag.isTab && (this.$drag.isTabBody || (this.$drag.$insertAt == "bottom" || this.$drag.$insertAt == "top"))) {
						if (!targetItem.page.isDashBoard) {
							targetItem = targetItem.layoutParent;
						}
					}
					var newChild = targetItem.articleParent.page.onAddNewItem(targetItem.articleParent, targetItem, this.dropItem, this.$drag.$insertAt);
					if (newChild && newChild.isTabLayout) {
						newChild.openBox(true);
					}
					this.designer.endTargetPageUpdate(newChild, true);
				}
			}
		}
	},
								...
								if (targetItem.page.isDashBoard) {
									if (targetItem.isLayout) {
										isDropEnabled = !(targetItem == targetItem.page.layoutContent ||
											targetItem == this.designer.awItem.layoutContent.tabOpened.layoutContent);
									}
								}
	...
	autoScroll: function(event) {
		...
		if (event.pageY > (this.boundary.bottom - margin)) {
			var scrollTop = this.boundary.boundaryItem.scrollTop;
			if (scrollTop < (this.boundary.scrollHeight - this.boundary.height)) {
				this.boundary.boundaryItem.scrollTop = Math.min(scrollTop + step, this.boundary.scrollHeight - this.boundary.height);
			}
		} else {
			if (event.pageY < (this.boundary.top + margin)) {
				var scrollTop = this.boundary.boundaryItem.scrollTop;
				this.boundary.boundaryItem.scrollTop = Math.max(scrollTop - step, 0);
			}
		}
		if (event.pageX > (this.boundary.right - margin)) {
			var scrollLeft = this.boundary.boundaryItem.scrollLeft;
			if (scrollLeft < (this.boundary.scrollWidth - this.boundary.width)) {
				this.boundary.boundaryItem.scrollLeft = Math.min(scrollLeft + step, this.boundary.scrollWidth - this.boundary.width);
			}
		} else {
			if (event.pageX < (this.boundary.left + margin)) {
				var scrollLeft = this.boundary.boundaryItem.scrollLeft;
				this.boundary.boundaryItem.scrollLeft = Math.max(scrollLeft - step, 0);
			}
		}
	},
	...
	dispose: function() {
		...
		if (this.boundary) {
			this.boundary.boundaryItem = null;
		}
