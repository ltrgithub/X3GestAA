...
function _createLayout(section, $newLayout) {
	syra_layout.clearContent(section.layoutContent);
	var prevLayout = section.layoutContent.domItem;
	section.$item.$layout = $newLayout;
	section.renderLayoutContent();
	syra_layout.removeSpace(section.layoutContent, true);
	return prevLayout;
}

function _splitInColumns(section, children, maxCol) {
	children = children || syra_layout.extractItems(section.layoutContent);
	var prevLayout = _createLayout(section, {
		$layoutType: "row",
		$widths: syra_layout.getDefaultWidths(maxCol),
		$items: []
	});
	var row = 0,
		colIndex = 0;
		...
		if (!children[ii].disposed) {
			...
			switch (row) {
				case 0:
					...
					break;
				case 1:
					layout = syra_layout.wrapIntack(section.layoutContent, [section.layoutContent.items[colIndex]], colIndex);
					break;
				default:
	...
	syra_dom.remove(prevLayout);
}

function _applySectionModel(section, $model) {
	var children = syra_layout.extractItems(section.layoutContent);
	var prevLayout = section.layoutContent.domItem;
	syra_layout.clearContent(section.layoutContent);
	switch ($model) {
		case "model_rows":
			...
			section.renderLayoutContent();
			syra_layout.newLoadChildItems(section.layoutContent, children);
			break;
		case "model_3cols":
	...
	if (children.length > 0) {
		...
		if ($rows.length == 1) {
			...
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				...
				syra_layout.removeSpace(section.layoutContent.items[rowIndex], true);
...
function _applyPageFactoryModel(page) {
	var children = syra_layout.extractItems(page.layoutContent);
	syra_layout.clearContent(page.layoutContent);
	...
	if (!page.$item.$isSimplified && page.isFusionPage) {
		page.$item.$layout = syra_layout.cleanConvergenceSetting(page.$item.$layout);
...
function _convertByColumns($colCount, designedItem, awLayout) {
	...
	if (designedItem.$designLevel == "field") {
		layout = syra_layout.wrapIntack(designedItem.layoutParent, [designedItem], designedItem.layoutParent.items.indexOf(designedItem));
		syra_layout.removeSpace(layout, true);
		awLayout = layout;
	}
	var $widths = ["100", "50,50", "33,33,33", "25,25,25,25", "20,20,20,20,20", "17,17,16,16,17,17"][$colCount - 1];
	syra_layout.convertToNewLayout(layout, {
...
function _applyArticleSmartModel(designer) {
	...
	var children = syra_layout.extractItems(article.layoutContent);
	syra_layout.clearContent(article.layoutContent);
...
var _builder = {
	2: function(designedItem, template, fields) {
		...
		syra_layout.newLoadChildItems(designedItem.layoutContent, fields);
		syra_dom.remove(prevLayout);
	},
	3: function(designedItem, template, fields) {
		...
		switch (template) {
			case "3-1":
				...
				syra_layout.removeSpace(designedItem.layoutContent.items[0], true);
				syra_layout.newLoadChildItems(designedItem.layoutContent.items[0], fields.splice(0, 2));
				syra_layout.newLoadChildItems(designedItem.layoutContent, fields);
				break;
			case "3-2":
				prevLayout = _createLayout(designedItem, {
				...
				syra_layout.newLoadChildItems(designedItem.layoutContent.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
				}), fields.splice(0, 2));
				syra_layout.newLoadChildItems(designedItem.layoutContent, fields);
			...
			case "3-3":
				...
				syra_layout.newLoadChildItems(designedItem.layoutContent, fields.splice(0, 1));
				syra_layout.newLoadChildItems(designedItem.layoutContent.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
				}), fields);
			...
			case "3-4":
				...
				syra_layout.removeSpace(row, true);
				syra_layout.newLoadChildItems(row, fields);
				break;
		}
		syra_dom.remove(prevLayout);
	},
	4: function(designedItem, template, fields) {
		...
		switch (template) {
			case "4-1":
				...
				syra_layout.removeSpace(row, true);
				syra_layout.newLoadChildItems(row, fields.splice(0, 2));
			...
			case "4-3":
				...
				var row = designedItem.layoutContent.items[0];
				syra_layout.removeSpace(row, true);
				syra_layout.newLoadChildItems(row, fields.splice(0, 3));
				syra_layout.newLoadChildItems(designedItem.layoutContent, fields);
				break;
	...
	5: function(designedItem, template, fields) {
		...
		switch (template) {
			case "5-1":
				...
				syra_layout.newLoadChildItems(designedItem.layoutContent.items[0], fields.splice(0, 2));
				syra_layout.newLoadChildItems(designedItem.layoutContent.items[1], fields);
				break;
			case "5-2":
				prevLayout = _createLayout(designedItem, {
					$layoutType: "stack",
	...
	mixt: function(section, template, children) {
		...
		if (children.length) {
			...
			if (children.length > 2) {
				syra_layout.newLoadChildItems(section.layoutContent.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
				}), children, 1);
			} else {
	...
	designer.onTemplateClick = function() {
		...
		if (designer.sectionCards && this.$modelType == "colcount") {
			...
			if (this.$fieldsCount) {
				...
				syra_layout.extractItems(selectItem.layoutContent, fields);
					...
					case "modelTab":
						if (designedItem.layoutParent.$layoutType != this.$layoutType) {
							if (designedItem.layoutParent.items.length == 1) {
								syra_layout.convertToNewLayout(designedItem.layoutParent, {
									$layoutType: this.$layoutType
								});
							} else {
								var layoutRoot = designedItem.layoutParent;
								if (this.$layoutType != "stack") {
									var layoutTab = syra_layout.wrapIntack(layoutRoot, [designedItem], layoutRoot.$layout.$items.indexOf(designedItem.$item));
									syra_layout.convertToNewLayout(layoutTab, {
										$layoutType: "tabs"
									});
								}
							}
