...
exports.TemplatesTool = helpers.defineClass(TemplatesTool, null, {
	...
	_applyArticleSmartModel: function(article) {
		var article = this.designer.getDesignedCardItem ? this.designer.getDesignedCardItem() : this.designer.awArticle;
	...
	applyCardListFactoryModel: function(list) {
		var list = this.designer.awArticle.list || this.designer.awArticle;
		if (list.$item.$format == "grid") {
			var $cardPosition = list.$item.$cardItem.$position;
			list.builder.applyDesignMetaData({
				$cardPosition: "no"
			}, true);
			list.$item.$cardItem = list.defineDefaultCard();
			list.$item.$cardItem.$position = $cardPosition;
			if (list.$item.$cardItem.$position == "row") {
				list.builder.cardDecorator.$rowFieldBinds = list.page.getDefinedFieldBinds(list.$item.$cardItem.$layout.$items);
			}
			list.builder.applyDesignMetaData({
	...
	addLandingPageModels: function(section, awItem) {
	...
	addListModelLinks: function(section, awItem) {
		section.dataStore = section.dataStore || {};
		if (!section.dataStore.modelSlot) {
			section.dataStore.modelSlot = document.createElement("div");
			section.body.appendChild(section.dataStore.modelSlot);
		...
		var slot = document.createElement("div");
		slot.className = "s-aw-template-row";
		this._addModelLink("smart", slot);
		this._addModelLink("factory", slot);
	...
	addPageModelLinks: function(section, awItem, colCount) {
		section.dataStore = section.dataStore || {};
		if (!section.dataStore.modelSlot) {
			section.dataStore.modelSlot = document.createElement("div");
			section.body.appendChild(section.dataStore.modelSlot);
	...
	onTemplateChoiceClick: function(picker, awItem, awLayout) {
		...
		if (!selectItem) {
			if (this.designer.getAwItem) {
				awItem = this.designer.getAwItem(awItem);
			}
			switch (picker.syraPickerType) {
				...
				case "$model_smart":
					...
					return awItem;
				case "$model_factory":
					if (this.designer.awArticle.page == this.designer.awArticle) {
						this._applyPageFactoryModel(this.designer.awArticle);
					} else {
						this.applyCardListFactoryModel();
					}
					return awItem;
				case "$model_3cols":
					var item = this.designer.getDesignedCardItem ? this.designer.getDesignedCardItem() : awItem;
