...
exports.TemplatesTool = helpers.defineClass(TemplatesTool, null, {
	load: function(designer) {
		...
		repository[this.id = (designer.id + "templatestool")] = this;
		this._boxSlots = [];
	},
	dispose: function() {
	...
	_applyArticleSmartModel: function(article) {
		...
		document.site.layoutUpdater.clearContent(article.layoutContent);
	...
	applySectionSmartModel: function(section) {
		...
		document.site.layoutUpdater.clearContent(section.layoutContent);
	...
	_applyPageFactoryModel: function(page) {
		...
		document.site.layoutUpdater.clearContent(page.layoutContent);
	...
	_addModelLink: function($model, slot, isTextIcon) {
		...
		link.syraOnClick = "onTemplateClick";
		link.syraModelType = "$model_" + $model;
	...
	addLandingPageModels: function(section, designedItem) {
		...
		document.site.dom.empty(section.dataStore.modelSlot);
	...
	fillTabChoice: function(section, designedItem) {
		...
		if (!section.dataStore.tabChoice) {
			...
			for (var ii = 0, jj = links.length; ii < jj; ii++) {
				...
				link.syraOnClick = "onTemplateClick";
				link.syraModelType = "$boxTabChoice";
				var img = document.createElement("div");
		...
		for (var ii = 0, jj = links.length; ii < jj; ii++) {
			...
			document.site.dom.disableItem(link, isLinkDisabled);
		}
	},
	onTemplateClick: function(picker) {
		...
		if (picker.syraFieldsCount) {
			...
			document.site.layoutUpdater.extractItems(selectItem.layoutContent, fields);
			this["build_" + funcName](selectItem, picker.syraTemplate, fields);
		}
		if (!selectItem) {
			switch (picker.syraModelType) {
				case "$boxTabChoice":
					...
					break;
				case "$colcount":
					selectItem = this.convertByColumns(picker, designedItem, awLayout);
					break;
				case "$model_smart":
				...
				case "$model_mixt":
					...
					break;
				default:
					if (picker.syraModelType.indexOf("$model_") == 0) {
						var item = this.designer.getDesignedCardItem ? this.designer.getDesignedCardItem() : designedItem;
						this.applySectionModel(item, picker.syraModelType);
						selectItem = designedItem;
					}
					break;
			}
