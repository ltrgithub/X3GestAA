...
function _addModelBtn(designer, $model, slot) {
	return syra_menus.button.add({
		parent: designer,
		slot: slot,
		text: syra_local["aw_model_" + $model] || syra_local.aw_model_default_title,
		css: "s-aw-template",
		iconOnly: true,
		imageName: "authoring/s-template-" + $model + ".png",
...
function _applyCardListFactoryModel(list) {
	if (list.$item.$format == "grid") {
		...
		if (list.$item.$cardItem.$position == "row") {
			list.builder.$rowFieldBinds = syra_page.getDefinedFieldBinds(list.$item.$cardItem.$layout.$items);
		}
		list.applyDesignMetaData({
...
exports.load = function(designer) {
	designer.addLandingPageModels = function(slot, designedItem) {
		...
		//slot.syraarticle = slot.syraTool = this.id;
		var count = designedItem.layoutContent.getFields().length;
		if (count > 1) {
			var title = document.createElement("div");
			title.className = "s-field-title s-field-inline";
			title.textContent = syra_local.ldpTemplate;
	...
	designer.addPageModelLinks = function(box, designedItem, colCount) {
		if (!box.modelSlot) {
			...
			//box.modelSlot.syraarticle = box.modelSlot.syraTool = this.id;
	...
	designer.addColsCountButtons = function(box, designedItem, colCount, max) {
		if (!box.colChoices) {
			...
			for (var ii = 1; ii <= max; ii++) {
				box.colChoices[ii] = syra_menus.button.add({
					parent: designer,
					slot: slot,
					text: ii,
					css: "s-aw-template",
	...
	designer.onTemplateClick = function() {
		...
		if (designer.sectionCards && this.$modelType == "colcount") {
			designer.designedArticle.applyDesignMetaData({
