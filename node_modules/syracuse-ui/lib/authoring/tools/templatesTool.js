...
exports.TemplatesTool = helpers.defineClass(TemplatesTool, null, {
	...
	addLandingPageModels: function(section, designedItem) {
		...
		if (!section.dataStore.modelSlot) {
			...
			section.dataStore.modelSlot.className = "s-aw-template-ldp-models";
				...
				case "$boxTabChoice":
					if (designedItem.layoutParent.$layoutType != picker.$syraValue) {
						if (designedItem.layoutParent.items.length == 1) {
							designedItem.layoutParent.convertToNewLayout({
								$layoutType: picker.$syraValue
							});
						} else {
							var layoutRoot = designedItem.layoutParent;
							if (picker.$syraValue != "stack") {
								var layoutTab = layoutRoot.wrapIntack([designedItem], layoutRoot.$layout.$items.indexOf(designedItem.$item));
	...
	convertByColumns: function(picker, designedItem, awLayout) {
		...
		if (designedItem.$designLevel == "field") {
			layout = designedItem.layoutParent.wrapIntack([designedItem], designedItem.layoutParent.items.indexOf(designedItem));
	...
	build_2: function(designedItem, template, fields) {
		...
		designedItem.layoutContent.newLoadChildItems(fields);
		document.site.removeDomChild(prevLayout);
	},
	build_3: function(designedItem, template, fields) {
		...
		switch (template) {
			case "3-1":
				...
				designedItem.layoutContent.items[0].newLoadChildItems(fields.splice(0, 2));
				designedItem.layoutContent.newLoadChildItems(fields);
				break;
			case "3-2":
				prevLayout = this._createLayout(designedItem, {
				...
				designedItem.layoutContent.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
			...
			case "3-3":
				...
				designedItem.layoutContent.newLoadChildItems(fields.splice(0, 1));
				designedItem.layoutContent.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
	...
	build_5: function(designedItem, template, fields) {
		...
		switch (template) {
			case "5-1":
				...
				designedItem.layoutContent.items[0].newLoadChildItems(fields.splice(0, 2));
				designedItem.layoutContent.items[1].newLoadChildItems(fields);
				break;
			case "5-2":
				prevLayout = this._createLayout(designedItem, {
					$layoutType: "stack",
