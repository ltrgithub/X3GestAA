		...
		onValidate: function(searchPage) {
			...
			if (sels) {
				...
				for (var ii = 0, jj = sels.length; ii < jj; ii++) {
					...
					var newChild = syra_layoutUpdater.onAddNewItem(designedItem.articleParent, designedItem, newItem, layoutParent.isRow ? "right" : "top");
					if (!newSelect) {
						newSelect = newChild;
					}
				}
	...
	if (!box.insertRow) {
		...
		designer.onAddRowClick = function(event, btn) {
			...
			if (!layout.layoutParent) {
				layout = syra_layoutUpdater.ensureHasParent(layout);
			}
					...
					switch (layoutParent.$layout.$layoutType) {
						...
						case "tabs":
							if (layoutParent.items.length == 0) {
								syra_layoutUpdater.convertToNewLayout(layoutParent, {
									$layoutType: "stack",
									$width: "100"
								});
							}
