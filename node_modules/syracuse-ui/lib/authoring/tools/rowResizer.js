...
exports.RowResizer = helpers.defineClass(RowResizer, null, {
    start: function(dropItem, itemTool){
        ...
        this.startX = this.rowSeparator.getBoundingClientRect().left;
        this.index = dropItem.syraSeparatorIndex;
        this.layout = this.designer.findItem(dropItem.parentNode, true);
        this.widths = document.site.layoutUpdater.getWidthValues(this.layout);
        ...
        this.offset = this.layout.domItem.getBoundingClientRect();
        
        this.margin = document.site.dom.getBoundingClientRect(document.site.body);
        
        this.maxWidth = this.layout.domItem.getBoundingClientRect().width;
        this.markerzIndex = document.site.dom.setZIndex();
    ...
    _findStep: function(rowSeparator){
        var rect = document.site.dom.getBoundingClientRect(rowSeparator);
    ...
    onDragMouseUp: function(target, event){
        ...
        document.site.layoutValidator.setRowWidths(this.layout);
    ...
    stop: function(){
        if (this.percentSteps) {
            for (var ii = 0, jj = this.percentSteps.length; ii < jj; ii++) {
                document.site.dom.removeChild(this.percentSteps[ii].marker);
                this.percentSteps[ii].marker = null;
            }
        }
        if (this.smallSteps) {
            for (var ii = 0, jj = this.smallSteps.length; ii < jj; ii++) {
                document.site.dom.removeChild(this.smallSteps[ii].marker);
