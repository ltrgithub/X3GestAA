...
exports.RowResizer = helpers.defineClass(RowResizer, null, {
	start: function(dropItem, itemTool) {
		...
		var nodes = dropItem.parentNode.childNodes;
		this.index = 0;
		for (var ii = 0, jj = nodes.length; ii < jj; ii++) {
			if (!nodes[ii].syraSepType) {
				this.index++;
			} else {
				if (nodes[ii] == dropItem) {
		...
		this.margin = syra_site.getBodySize();
	...
	_drawSmallSteps: function() {
		...
		for (var ii = 0, jj = percents.length; ii < jj; ii++) {
			...
			this.smallSteps.push({
				...
				offsetX: width,
				width: width,
				marker: syra_site.body.appendChild(marker)
			});
			lastX = pageX;
			if (ii > 0) {
	...
	_findStep: function(rowSeparator) {
		var rect = syra_site.dom.getBoundingClientRect(rowSeparator);
	...
	stop: function() {
		if (this.percentSteps) {
			for (var ii = 0, jj = this.percentSteps.length; ii < jj; ii++) {
				syra_site.dom.removeChild(this.percentSteps[ii].marker);
				this.percentSteps[ii].marker = null;
			}
		}
		if (this.smallSteps) {
			for (var ii = 0, jj = this.smallSteps.length; ii < jj; ii++) {
				syra_site.dom.removeChild(this.smallSteps[ii].marker);
