...
function _drawSteps(resizer) {
	...
	resizer.markersSlot = syra_dom.addDiv("s-aw-markers");
...
exports.RowResizer = helpers.defineClass(RowResizer, null, {
	start: function(dropItem, itemTool) {
		...
		this._rowLayout = syra_store.findItem(itemTool.cellToResize.parentNode);
		this.widths = syra_layoutUpdater.getWidthValues(this._rowLayout);

		this.sideProperty = syra_site.isDocumentRTL ? "left" : "right";
	...
	onDragMouseUp: function(target, event) {
		...
		syra_layoutValidator.setRowWidths(this._rowLayout);
	...
	stop: function() {
		this.markersSlot && syra_dom.removeChild(this.markersSlot);
