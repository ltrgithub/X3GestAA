...
exports.RowResizer = helpers.defineClass(RowResizer, null, {
	...
	_newDrawSteps: function() {
		var bigs = [this.maxWidth / 6, this.maxWidth / 5, this.maxWidth / 4, this.maxWidth / 3, 2 * (this.maxWidth / 5), this.maxWidth / 2, 3 * (this.maxWidth / 5), 2 * (this.maxWidth / 3), 3 * (this.maxWidth / 4), 4 * (this.maxWidth / 5), 5 * (this.maxWidth / 6)];
		var smallW = Math.round(this.maxWidth / 30);
		var steps = [];
		var width = 0;
		for (var ii = 0; ii < 29; ii++) {
			steps.push(width += smallW);
		}
		for (var ii = 0, jj = bigs.length; ii < jj; ii++) {
			var big = bigs[ii] = Math.round(bigs[ii]);
			for (var mm = 0, kk = steps.length; mm < kk; mm++) {
				if ((steps[mm] - 2) < big && big < (steps[mm] + 2)) {
					bigs[ii] = steps[mm];
					break;
				}
				if (steps[mm] > big) {
					steps.splice(mm, 0, big);
					break;
				}
			}
			if (mm > kk) {
		...
		for (var ii = 0, jj = steps.length; ii < jj; ii++) {
			...
			marker.style.height = ((bigs.indexOf(steps[ii]) >= 0) ? height : height / 2) + "px";
