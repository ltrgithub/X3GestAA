...
exports.UISwitchTool = helpers.defineClass(UISwitchTool, null, {
	...
	toggleUIDesign: function(item, enable, disposingDesigner) {
		...
		if (item == this.designer.awArticle) {
			var isOnPreviewMode = (this.designer.viewsTool && this.designer.viewsTool.isOnPreviewMode);
		...
		if (domItem) {
			...
			if (item.isSection) {
				if (this.designer.viewsTool.$isLightMode) {
					if (!item.childrenSection || item.childrenSection.length == 0) {
						if (item.layoutContent && item.layoutContent._item) {
							item.$isLayoutContentLightHidden = true;
							item.layoutContent._item.style.display = "none";
						}
					}
				} else {
					if (item.$isLayoutContentLightHidden) {
						item.layoutContent._item.style.display = "";
					}
				}
			}
			if (item.tabTitle) {
				...
				site.toggleClass(item.tabTitle, "s-aw-light", enable && this.designer.viewsTool.$isLightMode);
				site.toggleClass(item.body, "s-aw-light", enable && this.designer.viewsTool.$isLightMode);
			}
			if (enable) {
			...
			if (item.isLayout) {
				if (enable) {
					if (item.$layout.$layoutType == "row") {
						...
						site.ddManager.setDragSpot(item._item, true);
						if (item.layoutSlot) {
				...
				site.toggleClass(domItem, "s-aw-light", enable && this.designer.viewsTool.$isLightMode);
