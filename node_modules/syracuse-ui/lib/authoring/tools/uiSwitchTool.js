	...
	if (bar) {
		...
		if (enable || isOnPreviewMode) {
			if (!bar.designOverlay) {
				...
				document.site.setZIndex(bar.designOverlay);
			...
			if (bar.designOverlay) {
				document.site.removeDomChild(bar.designOverlay);
...
exports.UISwitchTool = helpers.defineClass(UISwitchTool, null, {
	...
	showHiddenItem: function(item) {
		if (item.page != item && !item.$item.$isPopupContent) {
			var display = (!item.isDesigning && (!(item.layoutParent && item.layoutParent.isRow) && (item.$isHidden || item.isItemHidden))) ? "none" : "";
			if (item.layoutSlot) {
				item.layoutSlot.style.display = display;
				if (item.rowSeparator) {
					item.rowSeparator.style.display = display;
				}
				document.site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", item.isDesigning && item.$isHidden);
			}
			if (item.tabTitle) {
