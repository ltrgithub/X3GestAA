...
exports.UISwitchTool = helpers.defineClass(UISwitchTool, null, {
	load: function(designer) {
		...
		repository[this.id = (designer.id + "uiswitchtool")] = this;
	},
		...
		if (item.isSpaceBox) {
			if (enable) {
				if (!item.deleteLink) {
					...
					item.deleteLink.syraOnClick = "onDeleteLayoutClick";
					item.deleteLink.syraSpaceBox = item.id;
					item.deleteLink.syraarticle = this.designer.id;
					item.deleteLink.syraTool = this.id;
					item.deleteLink.title = syra_local.aw_deleteColumn;
	...
	onDeleteLayoutClick: function(picker) {
		var self = this;
		if (!syra_dd.ddAgent) {
			syra_dd.cancel();
			var spaceBox = repository[picker.syraSpaceBox];
			setTimeout(function() {
