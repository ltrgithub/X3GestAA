...
exports.UISwitchTool = helpers.defineClass(UISwitchTool, null, {
	...
	toggleUIDesign: function(item, enable, disposingDesigner) {
		...
		if (item == this.designer.awArticle) {
			if (item.menuBar) {
				item.menuBar.toggleUIDesign(enable || this.designer.viewsTool.isOnPreviewMode, this.designer.viewsTool.isOnPreviewMode);
			}
			if (item.fusionBar) {
				item.fusionBar.toggleUIDesign(enable || this.designer.viewsTool.isOnPreviewMode, this.designer.viewsTool.isOnPreviewMode);
			}
	...
	showHiddenItem: function(item) {
		if (item.page != item && !item.$item.$isPopupContent) {
			...
			if (item.layoutSlot) {
				if (this.designer.viewsTool.$isLightMode && item.$designLevel == "field") {
