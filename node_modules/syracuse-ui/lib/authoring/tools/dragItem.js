...
function _addItemDrag(dropItem, designer) {
	return {
		designer: designer,
		dropItem: dropItem,
...
function _addRowResizer(cellToResize, designer) {
	return ({
		designer: designer,
		_left: cellToResize,
		start: function(context) {
			this._row = this._left.parentNode;
			this._rowLayout = syra_item.findItem(this._row);
...
exports.add = function(designer) {
	return new Draggable({
		...
		mousedown: function(event) {
			if (event.target) {
				var designer, item = syra_item.findItem(event.target);
				if (item && item.isTreeNode) {
					designer = item.designer;
					item = item.item;
				}
				if (item) {
					designer = designer || ((item.articleParent || item).designer);
					if (designer) {
						this._updater = this.cellToResize ? _addRowResizer(this.cellToResize, designer) : _addItemDrag(item, designer);
						this.setDefaultCursor();
						return true;
					}
				}
