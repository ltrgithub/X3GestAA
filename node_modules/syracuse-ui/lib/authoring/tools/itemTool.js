...
exports.ItemTool = helpers.defineClass(ItemTool, null, {
	...
	_resetDDTimer: function() {
		if (this._ddTabEnter) {
			clearTimeout(this._ddTabEnter);
		...
		if (bind) {
			if (!self.eventNamespace) {
				...
				document.site.$$layoutSlot.delegate(".s-aw-item", "click" + self.eventNamespace, function(event) {
					...
					if (document.site.ddManager.ddAgent) {
						var layout = self.designer.findItem(this, true);
						if (layout && layout.$layout.$layoutType == "tabs") {
							if (event.type == "mouseenter") {
								var tabItem = self.designer.findItem(this);
								self._ddTabEnter = setTimeout(function() {
									if (tabItem && tabItem.$item) {
										if (!tabItem.$opened) {
											tabItem.layoutParent.openTab(tabItem);
										}
									}
								}, 500);
							} else {
