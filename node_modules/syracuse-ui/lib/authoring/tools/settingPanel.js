...
function _fillTabChoice(designer, box) {
	...
	if (!box.tabChoice) {
		...
		for (var ii = 0, jj = btnTypes.length; ii < jj; ii++) {
			...
			img.style.backgroundImage = "url('/syracuse-ui/themes/desktop/images/authoring/s-layout-" + btn.$syraValue + ".png')";
			btn.appendChild(img);
...
exports.show = function(designer) {
	if (designer.panels) {
		...
		designer.endChange();
		//section && section.layoutContent && syra_site.layoutValidator.validateLayout(section.layoutContent, true);
