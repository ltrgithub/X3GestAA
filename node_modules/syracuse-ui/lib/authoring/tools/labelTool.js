...
exports.LabelTool = helpers.defineClass(LabelTool, null, {
	...
	setLabelItem: function(item) {
		var alignment = this.designer.designedArticle.page.$isEditMode ? "t" : "ll";
	...
	onDesignerNotifyDataChange: function(designedItem, metaData, field, value) {
		switch (field.$item.$bind) {
			case "$fieldsIsTitleHidden":
				designedItem.applyDesignMetaData({
			...
			case "$labelAlignment":
				...
				if (designedItem.isField) {
					designedItem.applyDesignMetaData(metaData, true);
				} else {
					designedItem.applyDesignMetaData({
	...
	onDesignerApplyChange: function(designedItem, newData) {
