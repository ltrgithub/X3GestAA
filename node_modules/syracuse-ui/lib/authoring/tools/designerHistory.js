...
function DesignerHistory($stepSkin) {
	this._$stepSkin = $stepSkin;
}

exports.DesignerHistory = helpers.defineClass(DesignerHistory, null, {
	...
	drawStepLinks: function() {
		...
		for (var ii = 0, jj = $steps.length; ii < jj; ii++) {
			...
			link.title = this.designer.designerLocalize["aw_" + $step];
			link.setAttribute("data-s-picker", $step);
			if ($step == "undoAllLayout") {
				link.className = this._$stepSkin;
				link.textContent = link.title;
			} else {
				link.className = this._$stepSkin + "-icon";
				link.style.backgroundImage = "url('/syracuse-ui/themes/desktop/sage/images/authoring/s-aw-" + $step + ".png')";
			}
			this[$step] = link;
			this._disableLink($step, true);
			this.designer.toolsSlots.history.appendChild(link);
		}
	},
	refreshStepLinks: function() {
		if (this._steps.length > 1) {
			this.previousLayout.title = this.designer.designerLocalize.aw_previousLayoutOn.replace("{0}", this._currentStep).replace("{1}", this._steps.length);
			this._disableLink("previousLayout", this._currentStep == 0);
			if (this._currentStep == this._steps.length - 1) {
				this.nextLayout.title = this.designer.designerLocalize.aw_nextLayout;
				this._disableLink("nextLayout", true);
			} else {
				this.nextLayout.title = this.designer.designerLocalize.aw_nextLayoutOn.replace("{0}", this._currentStep + 1).replace("{1}", this._steps.length);
		...
		switch ($bind) {
			...
			case "undoAllLayout":
				document.site.showMessage({
					$title: self.designer.designerLocalize.aw_updateMessageTitle,
					$message: self.designer.designerLocalize.aw_confirmUndoAll,
					$type: "question",
					$buttons: "yesno",
