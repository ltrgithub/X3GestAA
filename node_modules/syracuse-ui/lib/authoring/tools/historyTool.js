...
exports.HistoryTool = helpers.defineClass(HistoryTool, null, {
	load: function(designer) {
		...
		syra_map[this.id = (designer.id + "historytool")] = this;
	...
	drawStepLinks: function() {
		...
		for (var ii = 0, jj = _$steps.length; ii < jj; ii++) {
			...
			link.title = syra_local["aw_" + $step];
	...
	refreshStepLinks: function() {
		if (this._links) {
			if (this._steps.length > 1) {
				this._links.previousLayout.title = syra_local.aw_previousLayoutOn.replace("{0}", this._currentStep).replace("{1}", this._steps.length);
				this._links.previousLayout.className = "s-design-history-link-icon" + (this._currentStep == 0 ? " s-disabled" : "");
				if (this._currentStep == this._steps.length - 1) {
					this._links.nextLayout.title = syra_local.aw_nextLayout;
					this._links.nextLayout.className = "s-design-history-link-icon s-disabled";
				} else {
					this._links.nextLayout.title = syra_local.aw_nextLayoutOn.replace("{0}", this._currentStep + 1).replace("{1}", this._steps.length);
