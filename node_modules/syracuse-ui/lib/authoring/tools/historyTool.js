...
function _refreshStepLinks(hist) {
	if (hist) {
		if (hist.steps.length > 1) {
			syra_menus.button.setText(hist.prevBtn, syra_local.aw_previousLayoutOn.replace("{0}", hist.currentStep).replace("{1}", hist.steps.length));
			syra_menus.button.disable(hist.prevBtn, hist.currentStep == 0);
			if (hist.currentStep == hist.steps.length - 1) {
				syra_menus.button.setText(hist.nextBtn, syra_local.aw_nextLayout);
				syra_menus.button.disable(hist.nextBtn, true);
			} else {
				syra_menus.button.setText(hist.nextBtn, syra_local.aw_nextLayoutOn.replace("{0}", hist.currentStep + 1).replace("{1}", hist.steps.length));
