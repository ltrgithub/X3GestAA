...
function _showButtons(tool) {
	...
	for (var ii = 0, jj = _$steps.length; ii < jj; ii++) {
		...
		tool._btns[$step] = btn;
		tool.designer.leftTop && tool.designer.leftTop.appendChild(btn);
	}
}
...
exports.HistoryTool = helpers.defineClass(HistoryTool, null, {
	...
	onAfterStepChange: function($item) {
		try {
			...
			var newList = this.designer.reloadDesignedLayout ? this.designer.reloadDesignedLayout($article) : this.designer.designedArticle.reloadLayout($article);
