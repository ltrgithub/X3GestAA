...
exports.LocalizationTool = helpers.defineClass(LocalizationTool, null, {
	load: function(designer, $stepSkin) {
		...
		this.designer.$prototype.$properties.$titleChoice = {
			...
			$value: {
				...
				$enum: [{
					$value: 1,
					$title: this.designer.localize.aw_titleBlank
				}, {
					$value: 2,
					$title: this.designer.localize.aw_titleHidden
				}]
			}
		};
	...
	fillTitlesChoices: function() {
		var $enum = [];
		var designedPage = this.designer.awArticle.page;
		...
		if ($enum.length) {
			this.designer.$prototype.$properties.$titleChoice.$value.$enum.unshift({
				$value: 0,
				$title: this.designer.localize.aw_titleLocalization
			});
		} else {
			this.designer.$prototype.$properties.$titleChoice.$value.$enum.unshift({
				$value: 0,
				$title: this.designer.localize.aw_titleVisible
			});
	...
	setTitleItem: function(item) {
		...
		if (item.isSection && this.designer.awArticle.page.$isFusionPage && !$titleLocalization.$isDisabled) {
			$titleLocalization.$value = {
