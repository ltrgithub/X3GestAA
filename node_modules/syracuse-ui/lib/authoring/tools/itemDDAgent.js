...
exports.ItemDDAgent = helpers.defineClass(ItemDDAgent, null, {
	...
	toggleDraggedCss: function(show) {
		if (this.dropItem && this.dropItem.$$item) {
			document.site.dom.toggleClass(this.dropItem.$$item[0], "s-dd-dragged", show);
		}
	},
	...
	calculatePanelDropCue: function(event, targetItem) {
		var $position = null;
		this.$drag = document.site.dom.getBoundingClientRect(targetItem.layoutSlot ? targetItem.layoutSlot : targetItem.$$item[0]);
	...
	calculateTabDropCue: function(event, targetItem) {
		...
		if (event.target == targetItem.body) {
			this.$drag = document.site.dom.getBoundingClientRect(targetItem.layoutParent.$$item[0]);
