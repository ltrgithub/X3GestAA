...
exports.ItemDDAgent = helpers.defineClass(ItemDDAgent, null, {
	start: function(dropItem, itemTool, boundaryItem) {
		...
		this.scrollViewRect = document.site.dom.getBoundingClientRect(boundaryItem);
	...
	toggleDraggedCss: function(show) {
		if (this.dropItem && this.dropItem.domItem) {
			document.site.dom.toggleClass(this.dropItem.domItem, "s-dd-dragged", show);
		}
	},
	...
	calculatePanelDropCue: function(event, targetItem) {
		var $position = null;
		this.$drag = document.site.dom.getBoundingClientRect(targetItem.layoutSlot ? targetItem.layoutSlot : targetItem.domItem);
	...
	calculateTabDropCue: function(event, targetItem) {
		...
		if (event.target == targetItem.body) {
			this.$drag = document.site.dom.getBoundingClientRect(targetItem.layoutParent.domItem);
			this.$drag.isTabBody = true;
		} else {
			this.$drag = document.site.dom.getBoundingClientRect(targetItem.tabTitle);
	...
	toggleDropArea: function(targetItem, dropItem) {
		if (targetItem && targetItem.isSpaceBox) {
			...
			if (dropItem) {
				...
				if (targetItem.dropArea) {
					document.site.dom.removeChild(targetItem.dropArea);
	...
	dispose: function() {
		...
		document.site.dom.removeChild(this._info);
		document.site.dom.removeChild(this._dropCue);
