...
exports.ItemDDAgent = helpers.defineClass(ItemDDAgent, null, {
	start: function(dropItem, itemTool) {
		...
		if (dropItem.isTreeNode) {
			this.designer = dropItem.designer;
		} else {
			this.designer = (dropItem.articleParent || dropItem).designer;
		}
		this.designer = this.designer || itemTool.designer;
		this.dropItem = dropItem;
		if (this.dropItem.isLayout) {
			syra_site.layoutUpdater.setChildAuhtoringLevel(this.dropItem);
		}
		this.toggleDraggedCss(true);
		this.boundaryItem = this.designer.designedArticle.page.scrollview;
		this.scrollViewRect = syra_site.dom.getBoundingClientRect(this.boundaryItem);
	...
	toggleDraggedCss: function(show) {
		if (this.dropItem && this.dropItem.domItem) {
			syra_site.dom.toggleClass(this.dropItem.domItem, "s-dd-dragged", show);
		}
	},
	...
	onDragMouseUp: function(target, event) {
		if (this.isDragging && this.targetItem) {
			...
			if (this.dropItem) {
				...
				var newChild = syra_site.layoutUpdater.onAddNewItem(targetItem.articleParent, targetItem, this.dropItem, this.$drag.$insertAt);
				if (newChild && newChild.isTabLayout) {
					newChild.layoutParent.openTab(newChild);
				}
	...
	calculatePanelDropCue: function(event, targetItem) {
		var $position = null;
		this.$drag = syra_site.dom.getBoundingClientRect(targetItem.layoutSlot ? targetItem.layoutSlot : targetItem.domItem);
	...
	calculateTabDropCue: function(event, targetItem) {
		...
		if (event.target == targetItem.body) {
			this.$drag = syra_site.dom.getBoundingClientRect(targetItem.layoutParent.domItem);
			this.$drag.isTabBody = true;
		} else {
			this.$drag = syra_site.dom.getBoundingClientRect(targetItem.tabTitle);
	...
	_ensureDragImage: function() {
		if (!this._info) {
			...
			if (this.dropItem.$field) {
				this._info.appendChild(syra_menus.addFontIconFlag("s-aw-tree-field-type", fieldHelper.getTypeFieldIcon(this.dropItem.$field.$type)));
			}
	...
	moveDragImage: function(event, targetItem, isOut) {
		...
		syra_page.autoScroll(this.scrollViewRect, event);
						...
						if (!this.dropItem.isLayout) {
							if (this.targetItem.isLayout) {
								if (!this.targetItem.$chilAuthoringLevel) {
									syra_site.layoutUpdater.setChildAuhtoringLevel(this.targetItem);
								}
	...
	toggleDropArea: function(targetItem, dropItem) {
		if (targetItem && targetItem.isSpaceBox) {
			if (targetItem.deleteBtn) {
				targetItem.deleteBtn.style.display = dropItem ? "none" : "";
			}
			if (dropItem) {
				...
				if (targetItem.dropArea) {
					syra_site.dom.removeChild(targetItem.dropArea);
