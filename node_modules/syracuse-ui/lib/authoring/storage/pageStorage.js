...
exports.PageStorage = helpers.defineClass(PageStorage, RawPage, {
				...
				if (menuItem.page != self.designer.designedArticle.page) {
					if (self.designer.designedArticle.page.pageViewSelector) {
						...
						syra_site.showMessage({
							...
							$message: syra_local.aw_deleteMessageText.replace("{0}", "'" + selectedView.$title + "'"),
							$type: "question",
							$buttons: "yesno",
							callback: function(response) {
								if (response.$clientId == "yes") {
	...
	saveUpdate: function() {
		syra_menus.clickMenu(this, (this.$menus.$save && !this.$menus.$save.$isDisabled) ? "$save" : "saveAs");
	},
		...
		switch (menuItem.$bind) {
			...
			case "$delete":
				if (this.isActionSuccess(menuItem)) {
					...
					var $diagnose = {
						$message: syra_local.aw_isDeletedMessageText.replace("{0}", "'" + selectedView.$title + "'"),
						$severity: "success"
					};
	...
	dispose: function() {
		...
		this.crudSlot = this.designer = this.diagnosePage = null;
