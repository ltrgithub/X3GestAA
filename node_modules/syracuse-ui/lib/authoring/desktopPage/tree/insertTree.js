...
exports.InsertTree = helpers.defineClass(InsertTree, ItemTree, {
	dispose: function() {
		this.awArticle = this.$sourceItem = this.rootNode = null;
	...
	fill: function() {
		...
		this.showAll = ((this.awArticle.$item.$garbageFields && this.awArticle.$item.$garbageFields.length > 0) ||
			(this.awArticle.garbage._$freeFieldBinds && this.awArticle.garbage._$freeFieldBinds.length > 0));
		...
		if (this.awArticle.page == this.awArticle) {
			...
			if (!this.$sourceItem) {
				...
				for (var ii = 0, jj = binds.length; ii < jj; ii++) {
					var $bind = binds[ii];
					if ($bind.charAt(0) != "$" && !(this.awArticle.page.$isFusionPage && this.awArticle.page.isNavigationListItem($bind))) {
	...
	_bindEvents: function() {
		if (this.tabItem && this.tabItem.body) {
			ItemTree.prototype._bindEvents.call(this);
		}
	},
		...
		if ($item.$category != "space") {
			if ($item.$items && !$item.$bind && !$item.$category) {
				...
				if ($field) {
					...
					var $isFreeField = this.awArticle.garbage._$freeFieldBinds && this.awArticle.garbage._$freeFieldBinds.indexOf($item.$bind) >= 0;
					if ((this.awArticle.garbage._garbageFields && this.awArticle.garbage._garbageFields[$item.$bind]) || $isFreeField) {
