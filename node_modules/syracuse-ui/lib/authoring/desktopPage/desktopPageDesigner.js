...
exports.DesktopPageDesigner = helpers.defineClass(DesktopPageDesigner, Designer, {
    ...
    onBeforeSiteResize: function(site){
        var siteRect = document.site.getBoundingClientRect(site.layoutSlot);
        var rect = document.site.getBoundingClientRect(this.layoutSlot);
        site.body.style.height = (siteRect.height - rect.height) + "px";
    },
    ...
    registerFullScreenDesigner: function(add){
        var site = document.site;
        if (add) {
            if (!site.fullScreenDesigners) {
                site.fullScreenDesigners = [];
            }
            else {
                this._toggleFullScreenDesigner(false);
            }
            site.fullScreenDesigners.push(this);
        }
        else {
            if (site.fullScreenDesigners) {
                var index = site.fullScreenDesigners.indexOf(this);
                if (index >= 0) {
                    site.fullScreenDesigners.splice(index, 1);
                }
                if (site.fullScreenDesigners.length == 0) {
                    delete site.fullScreenDesigners;
                    document.site.header.style.display = "";
                }
                else {
                    this._toggleFullScreenDesigner(true);
                }
            }
        }
    },
    _toggleFullScreenDesigner: function(show){
        var site = document.site;
        if (site.fullScreenDesigners.length > 0) {
            var last = site.fullScreenDesigners[site.fullScreenDesigners.length - 1];
            last.propertiesSlot.style.display = last.layoutSlot.style.display = last.treebarSlot.style.display = show ? "" : "none";
            last.bindEvents(show);
        }
    },
    drawBox: function(){
        this.registerFullScreenDesigner(true);
        ...
        //right part
        this.awArticle.page.domItem.appendChild(this.propertiesSlot);
        ...
        this.awArticle.page.domItem.insertBefore(this.treebarSlot, this.awArticle.page.domItem.firstChild);
    ...
    showTransaction: function(){
        if (this.awArticle.$item.$transaction && this.historySlot) {
            ...
            case "$excludeMe":
                ...
                if (excludedFields) {
                    for (var ii = 0, jj = excludedFields.length; ii < jj; ii++) {
                        this.awArticle.garbage.onExcludeField(excludedFields[ii].$item.$bind, true);
                        this.awArticle.unbind(excludedFields[ii]);
                    }
                }
    ...
    onClickItem: function(item, target, article){
        article = article || document.site.findArticle(target);
        if (article.isDesigned || article.page.isDesigned) {
            ...
            if (document.site.ddManager.ddAgent) {
                ...
                if (article.page.isDesigned && article != this.awArticle) {
                    /*if (!item) {
                ...
                if (article.isDesigned) {
                    ...
                    if (item) {
                        ...
                        if (item && (item != this && item.page != this)) {
                            this.selectItem(this.findTopDesignedITem(item), true);
                        }
                    }
                }
            }
    ...
    onDragMove: function(target, event){
        if (document.site.ddManager.ddAgent && document.site.ddManager.ddAgent.designer != this) {
            return true;
        }
        if (document.site.ddManager.dropableItem) {
            if (document.site.ddManager.dropableItem.syraSepType == "row") {
                var item = this.findItem(target, undefined, event);
                if (item && !item.articleParent || (item.articleParent == this.awArticle)) {
        ...
        if (target.className && target.className.indexOf && (target.className.indexOf("s-aw-item") >= 0 || target.className.indexOf("s-aw-add-item") >= 0)) {
            var item = this.findItem(target, undefined, event);
            if (item) {
                if ((!item.articleParent || (item.articleParent == this.awArticle))) {
    ...
    findItem: function(domItem, findLayout, event){
        if (domItem.syraAwTreeNodeId) {
            ...
            if (article) {
                if (findLayout || domItem.syraAwLayoutId) {
                    while (domItem && !domItem.syraAwLayoutId) {
                        domItem = domItem.parentNode;
                    }
                    if (domItem) {
                        return (article.id == domItem.syraAwLayoutId) ? article : (article.layouts[domItem.syraAwLayoutId] || null);
                    }
                }
                else {
                    while (domItem && !domItem.syraAwItemId) {
                        domItem = domItem.parentNode;
                    }
                    if (domItem) {
                        var item = (article.id == domItem.syraAwItemId) ? article : (article.idMap[domItem.syraAwItemId] || null);
                ...
                if (section.$item.$isFieldAddinSection) {
                    ...
                    if (this.awItem.$authoringType) {
                        ...
                        for (var ii = 0, jj = fieldSections.length; ii < jj; ii++) {
                            var fieldSection = fieldSections[ii], isSectionHidden = true;
                            if (fieldSection.$item.$authoringType == this.awItem.$authoringType) {
                                if (fieldSection.$item.$authoringEditMode !== undefined) {
                                    isHidden = isSectionHidden = !(fieldSection.$item.$authoringEditMode == this.awItem.$isEditMode);
                                }
                                else {
    ...
    dispose: function(){
        ...
        this.registerFullScreenDesigner(false);
        if (this.viewsTool) {
            this.viewsTool.ensureLayoutView();
        }
        this.toggleUIDesign(this.awArticle, false, true);
        ...
        this.leftTop = this.treesBar = this._ddTabEnter = this.panels = this.trees = this.selectedPanel = this.transactionLabel = null;
