...
function _onEndChangeStep(awContext, menuItem, $item){
    ...
    awContext.targetPage.reloadLayout(helpers.object.clone($item || awContext.updatedSteps[awContext.currentUpdtateStep], true));
    _refreshStepLinks(awContext);
    var selectItem;
    if ($bind) {
        var bounds = awContext.targetPage.boundFields[$bind];
        selectItem = bounds ? bounds[0] : null;
    }
    awContext.toolsBar.onEndChangeStep();
    awContext.authorPage.selectItem(selectItem || awContext.targetPage, true);
}
...
exports.notifySteps = function(awContext, resetUpdates){
    if (!resetUpdates && awContext.isUpdated) {
        ...
        awContext.updatedSteps.push(helpers.object.clone(awContext.targetPage.$item, true));
        ...
        default:
            if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                if ((menuItem.$item.$bind != "pattern-headerTabs") && (awContext.awItem.$authoringLevel == "section" || awContext.awItem.$authoringLevel == "block")) {
                    ...
                    var $result = patternFactory.build(menuItem.$item.$bind, awContext.targetPage.$prototype, awContext.awItem.$item);
                    if (!$result && !$result.$layout) {
                        return;
                    }
                    awContext.awItem.$item.$layout = $result.$layout;
                    $item = awContext.targetPage.$item;
                }
                else {
                    $item = patternFactory.build(menuItem.$item.$bind, awContext.targetPage.$prototype, awContext.$sourceItem);
                }
                awContext.authorPage.notifyUpdate(true);
            }
            _onEndChangeStep(awContext, menuItem, $item);
