...
var CardDesigner = require("syracuse-ui/lib/authoring/cardDesigner").CardDesigner;
...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	drawBox: function() {
		...
		if (this.designedArticle.chart && this.designedArticle.chart.designGraphType == "cube$highCharts") {
	...
	designCardItem: function(open, cardItem) {
		if (open) {
			...
			cardItem.designer.onWindowResize();
		} else {
	...
	endArticleUpdate: function() {
		...
		if (record && record.layoutContent) {
			this.designedArticle.page.layoutValidator.validate(record.layoutContent, true);
		}
		this.designedArticle.onWindowResize();
		this.saveArticleUpdate();
	},
	saveArticleUpdate: function() {
		var vignetteField = this.designedArticle.page.vignetteField;
		if (vignetteField) {
			vignetteField.$item.$vignette = helpers.object.clone(this.designedArticle.page.$item, true);
			delete vignetteField.$item.$vignette.$menus;
			vignetteField.page.designer.endArticleUpdate();
		}
	},
	...
	onWindowResize: function() {
		if (this.cardDesigner) {
			this.cardDesigner.onWindowResize();
		}
	},
	dispose: function() {
		...
		this.cardDesigner = this.switchPagerLinks = this.graphAddin = this.modes = null;
