...
function _loadGraphDesigner(designer) {
	...
	if (hasDecorator || hasCube) {
		...
		title.textContent = syra_local.lw_graph_position;
	...
	drawBox: function() {
		...
		for (var ii = 0, jj = modeTypes.length; ii < jj; ii++) {
			...
			if (this.designedArticle.isClientFetch) {
				...
				this.onSwitchPagerClick = function(event, btn) {
			...
			if (newData.$format) {
				...
				for (var ii = 0, jj = this.buttons.length; ii < jj; ii++) {
					...
					if (btn.modeType) {
						...
						syra_site.dom.toggleClass(btn.link, "s-selected", isSelected);
