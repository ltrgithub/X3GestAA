...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	saveArticleUpdate: function() {
		if (this.designedArticle.page.saveArticleUpdate) {
			this.designedArticle.page.saveArticleUpdate();
		}
	},
	...
	appendMeasure: function(bind, delta, body) {
		...
		var title = document.createElement("div");
		title.textContent = $measure.$title ? this.designedArticle.page.getLocalizeText($measure.$title) : "";
		...
		delta["$cube" + bind] = _style($style || $measure.$style || this.graph.defaultStyle);
		delta["$cube-color-" + bind] = $color || $measure.$color || this.graph.defaultColor;
