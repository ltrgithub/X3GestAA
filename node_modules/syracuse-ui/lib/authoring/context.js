...
var ItemPalette = require("syracuse-ui/lib/authoring/palette/itemPalette").ItemPalette;
...
exports.Context = helpers.defineClass(Context, null, {
    create: function(authorPage, awArticle, options){
        ...
        this.$defaultItem = helpers.object.clone(document.site.ensureDefaultArticle($prototype.$article, $prototype), true);
        this.paletteSlot = authorPage.paletteSlot;
        this.topLeftBar = authorPage.topLeftBar;
    ...
    dispose: function(onAuthorPageDispose){
        ...
        if (!onAuthorPageDispose && this.contextStack) {
            for (var ii = 0, jj = this.contextStack.length; ii < jj; ii++) {
                if (this.contextStack[ii] == this) {
                    this.contextStack.splice(ii, 1);
                    break;
                }
            }
        }
        if (this.authorPage && this.authorPage.awContext == this) {
            this.authorPage.awContext = null;
        }
        this.parentContext = this.$prototype = this.drawHelper = this.authorPage = this.localize = null;
        this.awArticle = this.targetPage = this.$sourceItem = this.$defaultItem = this.paletteSlot = this.topLeftBar = null;
        this.scrollview = this.palette = null;
