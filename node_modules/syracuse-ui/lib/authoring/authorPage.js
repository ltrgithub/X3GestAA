...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    resizePalette: function(targetPage){
        this.awContext.palette.onWindowResize();
    },
    notifyUpdate: function(isUpdated){
        this.awContext.isUpdated = isUpdated !== false;
        this.awContext.palette.notifySteps();
    },
    _toggleAuthoringDialog: function(targetPage, begin){
        if (targetPage.dialogWrapper) {
            if (begin) {
                targetPage.dialogWrapper.$$closeBtn[0].style.display = "none";
                targetPage.dialogWrapper.overlay.className = "s-aw-overlay";
            }
            else {
                targetPage.dialogWrapper.$$closeBtn[0].style.display = "";
                targetPage.dialogWrapper.overlay.className = "s-overlay";
            }
        }
    },
    beginAuthoring: function(awPageSlot){
        ...
        if (document.site.mainPage.isFusionPage) {
            targetPage = document.site.fusionGateway.activatedBook.selectedSheet;
        }
        else {
            targetPage = document.site.getTopDialogPage();
            if (targetPage) {
                targetPage = targetPage._content;
            }
        }
        this.diagnosePage = targetPage = targetPage || document.site.mainPage;
        if (targetPage.dialogWrapper) {
            this._toggleAuthoringDialog(targetPage, true);
        }
        targetPage.authoringPage = this;
        targetPage.isOnAuthoring = true;
        this.awContext = {
            isUpdated: false,
            ...
            $sourceItem: helpers.object.clone(targetPage.$item, true),
            $defaultItem: helpers.object.clone(document.site.ensureDefaultArticle(targetPage.$prototype.$article, targetPage.$prototype), true)
        };
        if (this.awContext.targetPage.menuBar) {
            this.awContext.targetPage.menuBar.toggleItemAuthoring(true);
        }
        if (this.awContext.targetPage.fusionBar) {
            this.awContext.targetPage.fusionBar.toggleItemAuthoring(true);
        }
        ...
        document.controller.loadWorkingCopy({
            menu: {
                $url: this.awContext.targetPage.$authorUrl
            },
            article: this,
            callback: function(){
        ...
        switch (menuItem.$item.$bind) {
                    ...
                    document.site.showMessage({
                        ...
                        callback: function(response){
                            if (response.$id == "yes") {
                                ...
                                if (response.$id == "no") {
                                    awContext.targetPage.reloadLayout(helpers.object.clone(awContext.$sourceItem, true));
                                    document.site.openAuthoringPage(false);
                                }
                            }
                        }
            ...
            default:
                if (menuItem.$sourceBind == "$aw_quit") {
                    awContext.targetPage.showDiagnoses(null);
                    document.site.openAuthoringPage(false);
                    return false
                }
                break;
    ...
    drawBox: function(){
        ...
        this.awContext.palette = new ItemPalette();
        this.awContext.palette.awContext = this.awContext;
        this.awContext.palette.loadBox();
        
        document.site.resize();
        var awContext = this.getContext();
        this.toggleItemAuthoring(awContext.targetPage, true);
        this.selectItem(awContext.targetPage, true);
    },
    selectItem: function(item, select, ensureSelect){
        ...
        awContext.treeContent.selectNode(item, select);
    },
    ...
    toggleItemAuthoring: function(item, enable){
        ...
        var awContext = this.getContext();
        if (item.fusionBar) {
            item.fusionBar.toggleItemAuthoring(enable || awContext.isOnPreviewMode, awContext.isOnPreviewMode);
        }
        if (item.menuBar) {
            item.menuBar.toggleItemAuthoring(enable || awContext.isOnPreviewMode, awContext.isOnPreviewMode);
        }
            ...
            if (enable) {
                if (!item.deleteLink) {
                    if (item.boxParent.$authoringLevel == "block") {
                        ...
                        var children = item.boxParent.layoutContent ? drawHelper.getItemsFromLayout(item.boxParent.layoutContent) : [];
        ...
        if (domItem) {
            ...
            if (item.tabTitle) {
                ...
                site.toggleClass(item.tabTitle, "s-aw-light", enable && awContext.$isLightMode);
                site.toggleClass(item._body, "s-aw-light", enable && awContext.$isLightMode);
            }
            if (enable) {
            ...
            if (item.$authoringLevel == "field") {
                item.layoutSlot.style.display = (awContext.$isLightMode || item.$isHidden) ? "none" : "";
            }
            if (item.isLayout) {
                if (enable) {
                    if (item.$layout.$layoutType == "row") {
                        ...
                        if (awContext.$isLightMode) {
                            var children = drawHelper.getItemsFromLayout(item);
                ...
                site.toggleClass(domItem, "s-aw-light", enable && awContext.$isLightMode);
                if (item.$isHidden) {
    ...
    showHiddenItem: function(item){
        ...
        if (item.isAuthingEnabled) {
            ...
            if (item.layoutSlot) {
                item.layoutSlot.style.display = (!item.$isHidden || (!awContext.$isLightMode && forced)) ? "" : "none";
                document.site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", item.$isHidden);
            }
            var domItem = item._item || item._domItem;
            if (domItem) {
                domItem.style.display = (!item.$isHidden || (!awContext.$isLightMode && forced)) ? "" : "none";
    ...
    authorizeEvent: function(item){
        var awContext = this.getContext();
        return (item.page && (item.page.$dialogMode || (item.page == this) || (item.page == awContext.palette) || (item.page == awContext.toolsBar)));
    },
    bindEvents: function(){
        var self = this;
            ...
            if (document.site.dragDropInstance) {
                ...
                if (event.currentTarget.className.indexOf("s-aw-add-item") >= 0) {
                    if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
                        var node = awContext.treeContent.findNode(event);
                        if (node) {
                ...
                if (item) {
                    ...
                    if (item != awContext.targetPage) {
                        if (document.site.requestedDDAuthoringItem) {
                            (document.site.dragDropInstance = new DragDrop()).start(awContext.targetPage.$$scrollview);
                        }
                        else {
                            if (document.site.dragDropInstance) {
                    ...
                    if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
                        ...
                        if (node) {
                            ...
                            if (document.site.requestedDDAuthoringItem) {
                                ...
                                document.site.dragDropInstance.start(awContext.targetPage.$$scrollview);
                            }
                            else {
                                if (document.site.dragDropInstance) {
                                    if (item.layoutParent || item.isLayout) {
    ...
    endTargetPageUpdate: function(selectedItem, isStructureUpdated){
        ...
        if (isStructureUpdated) {
            awContext.treeContent.toggle(true);
        }
    ...
    dispose: function(){
        ...
        if (awContext) {
            ...
            if (awContext.targetPage) {
                if (awContext.targetPage.dialogWrapper) {
                    this._toggleAuthoringDialog(awContext.targetPage, false);
                }
                awContext.targetPage.isOnAuthoring = false;
                awContext.isOnPreviewMode = false;
                this.toggleItemAuthoring(awContext.targetPage, false);
                if (awContext.palette) {
                    document.controller.disposeObject(awContext.palette);
                    awContext.palette = null;
                }
                delete awContext.targetPage.authoringPage;
            }
            awContext.drawHelper = awContext.authorPage = awContext.localize = awContext.targetPage = null;
            awContext.awLayout = awContext.awItem = awContext.$sourceItem = awContext.$defaultItem = null;
            awContext = this.diagnosePage = awContext.targetPage = awContext.awLayout = null;
