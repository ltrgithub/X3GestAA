...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    onEndChangeStep: function(pageContext, menuItem, $item){
        ...
        pageContext.awArticle.reloadLayout(helpers.object.clone($item || this.updatedSteps[this.currentUpdtateStep], true));
    ...
    notifySteps: function(pageContext, resetUpdates){
        ...
        if (!resetUpdates && this.isUpdated) {
            ...
            if (document.site.mobileGateway) {
                document.site.mobileGateway.applyChange(pageContext.$sourceItem);
            }
    ...
    showHiddenItem: function(item){
        if (item.page != item) {
            var display = (!item.isAuthingEnabled && (!(item.layoutParent && item.layoutParent.isRow) && item.$isHidden)) ? "none" : "";
            if (item.layoutSlot) {
