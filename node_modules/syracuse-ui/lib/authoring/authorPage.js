...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    toggleItemAuthoring: function(item, enable){
        ...
        if (item.isLayout) {
            ...
            if (this.preferences.authoring.$showHiddenItems) {
    ...
    _toggleForcedVisibility: function(item, forced){
        ...
        if (item.layoutSlot) {
            ...
            document.site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", forced);
        }
        var domItem = item._item || item._domItem;
        if (domItem) {
            domItem.style.display = (forced || item.isVisible) ? "" : "none";
            document.site.toggleClass(domItem, "s-aw-visibility-forced", forced);
        }
    },
    ...
    applyChange: function(newData){
        if (newData && newData.$actions && newData.$actions.$save) {
            if (newData.$actions.$save.$isDisabled && newData.$actions.$save.$isRequested === false) {
                if (newData.$url) {
                    //   this.targetPage.$authorUrl = newData.$url;
                    this.$prototype.$representationUrl = newData.$url;
                }
    ...
    insertFieldsWrapper: function(targetItem){
        var newChild;
    ...
    addNewItem: function(targetItem, $newAuthoringLevel, $bind, dropItem, $insertAt){
                    ...
                    case "stack":
                        switch ($insertAt) {
                            ...
                            case "right":
                                ...
                                stack.loadChildItem(targetItem);
                                break;
                        }
                        break;
                    case "row":
                        switch ($insertAt) {
                            ...
                            case "left":
                            case "right":
                                var widths = targetItem.layoutParent.$layout.$widths.split(",").map(function(width){
                                    return parseInt(width, 10);
                                });
                                widths[targetItemIndex] = widths[targetItemIndex] / 2;
                                widths.splice(targetItemIndex, 0, widths[targetItemIndex]);
                                targetItem.layoutParent.$layout.$widths = widths.join(",");
                                if ($insertAt == "right") {
                                    targetItemIndex++;
                                }
                                break;
        ...
        switch (targetItem.$authoringLevel) {
            case "layout":
                if (dropItem) {
                    if (targetItem.layoutParent) {
                        var boxParent = targetItem.layoutParent.boxParent;
                        targetItem.layoutParent.loadChildItem(dropItem, null, targetItemIndex);
                        switch (dropItem.$authoringLevel) {
                            case "field":
                                newTopChild = boxParent;
                                if (newTopChild.$authoringLevel != "block") {
                                    newTopChild = targetItem.layoutParent.loadChildItem(null, {
                                        $category: "section",
                                        $applyDefaultTitle: true
                                    }, targetItemIndex);
                                    if (newTopChild.$authoringLevel != "block") {
                                        newTopChild = newTopChild.layoutContent.loadChildItem(null, {
                                            $category: "section",
                                            $applyDefaultTitle: true
                                        });
                                    }
                                    newChild = newTopChild.layoutContent.loadChildItem(dropItem);
                                }
                                else {
    ...
    bindEvents: function(){
        ...
        document.site.$$layoutSlot.delegate(".s-aw-item", "click.author", function(event){
            ...
            $(document).bind("mouseup.syraresize", function(event){
                ...
                var total = self.resizer.layout.$$item.width(); // - (self.resizer.layout.rowSeparators.length * 3) - (self.resizer.layout.items.length * 8);
                var widths = [];
                ...
                self.resizer.leftItem.layoutSlot.style.width = (self.resizer.leftWidth + diff) + "px";
                self.resizer.rightItem.layoutSlot.style.width = (self.resizer.rightWidth + (diff * (-1))) + "px";
    ...
    onClickItem: function(item, target){
        if (document.controller.findArticle($(target)).isOnAuthoring) {
            if (document.site.DDAuthoring) {
                document.site.DDAuthoring.stop(event);
                delete document.site.DDAuthoring;
            }
    ...
    validateTargetPageLayout: function(){
