...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
                ...
                switch (targetItem.layoutParent.$layout.$layoutType) {
                    case "stack":
                        switch ($insertAt) {
                            ...
                            case "left":
                            case "right":
                                var stack = targetItem.layoutParent.createChildItem({
                                    $layoutType: "row",
                                    $items: []
                                }, null, targetItemIndex);
                                stack.loadChildItems([targetItem]);
                                break;
                        }
                        break;
                    case "row":
    ...
    bindEvents: function(){
        ...
        document.site.$$layoutSlot.delegate(".s-aw-item", "click.author", function(event){
            ...
            return false;
        }).delegate(".s-aw-slot-row-sep", "mousedown.author", function(event){
            self.resizer = {
                startX: event.pageX,
                index: this.getAttribute("data-s-index"),
                layout: self._findTargetItemEvent(this, true)
            };
            self.resizer.leftItem = self.resizer.layout.items[self.resizer.index - 1];
            self.resizer.leftWidth = $(self.resizer.leftItem.layoutSlot).width();
            self.resizer.rightItem = self.resizer.layout.items[self.resizer.index];
            self.resizer.rightWidth = $(self.resizer.rightItem.layoutSlot).width();
            self.resizer.leftRightWidth = self.resizer.leftWidth + self.resizer.rightWidth;
            self.resizer.maxWidth = 0;
            
            for (var ii = 0, jj = self.resizer.layout.items.length; ii < jj; ii++) {
                self.resizer.maxWidth += $(self.resizer.layout.items[ii].layoutSlot).width();
            }
            //self.resizer.maxWidth = $(self.resizer.layout._item).width();
            var step = self.resizer.maxWidth / 10;
            /* self.resizer.steps = [];
             for (var ii = 1; ii < 10; ii++) {
             self.resizer.steps.push(ii * step);
             }*/
            self.resizer.steps = [self.resizer.maxWidth / 5, self.resizer.maxWidth / 4, self.resizer.maxWidth / 3, self.resizer.maxWidth / 2, 2 * (self.resizer.maxWidth / 3), 3 * (self.resizer.maxWidth / 4), 4 * (self.resizer.maxWidth / 5)];
            document.site.body.style.cursor = "w-resize";
            $(document).bind("mouseup.syraresize", function(event){
                $(document).unbind(".syraresize");
                document.site.body.style.cursor = "default";
                if (self.resizer) {
                    for (var ii = 0, jj = self.resizer.steps.length; ii < jj; ii++) {
                        if (self.resizer.steps[ii] >= self.resizer.curLeftW || (((self.resizer.steps[ii] + 20) >= self.resizer.curLeftW) && ((self.resizer.steps[ii] - 20) <= self.resizer.curLeftW))) {
                            self.resizer.curLeftW = self.resizer.steps[ii];
                            self.resizer.curRightW = self.resizer.leftRightWidth - self.resizer.curLeftW;
                            break;
                        }
                    }
                    self.resizer.leftItem.layoutSlot.style.width = (self.resizer.curLeftW - 8 - 4) + "px";
                    self.resizer.rightItem.layoutSlot.style.width = (self.resizer.curRightW - 8 - 4) + "px";
                    var total = self.resizer.layout.$$item.width();
                    var widths = [];
                    for (var ii = 0, jj = self.resizer.layout.items.length; ii < jj; ii++) {
                        widths[ii] = Math.floor((($(self.resizer.layout.items[ii].layoutSlot).width() + 8 + 4) * 100) / total);
                        self.resizer.layout.items[ii].layoutSlot.style.width = widths[ii] + "%";
                    }
                    self.resizer.layout.$layout.$widths = widths.join(",");
                    self.resizer = self.resizer.layout = self.resizer.leftItem = self.resizer.rightItem = null;
                }
            }).bind("mousemove.syraresize", function(event){
                if (self.resizer) {
                    var diff = event.pageX - self.resizer.startX;
                    var curLeftW = Math.max(self.resizer.leftWidth + diff);
                    var curRightW = self.resizer.rightWidth + (diff * (-1));
                    if (curLeftW >= self.resizer.steps[0] && curRightW >= self.resizer.steps[0]) {
                        self.resizer.leftItem.layoutSlot.style.width = (self.resizer.curLeftW = curLeftW) + "px";
                        self.resizer.rightItem.layoutSlot.style.width = (self.resizer.curRightW = curRightW) + "px";
                        console.log("LW:" + curLeftW + "  RW:" + curRightW + " STEPs:" + self.resizer.steps.join(",") + " max:" + self.resizer.maxWidth);
                    }
                    else {
                        console.log("LW:" + curLeftW + "  RW:" + curRightW);
                    }
                }
    ...
    _deleteLayoutRowItem: function(layout, deletedIndex){
        var widths = layout.$layout.$widths.split(",").map(function(width){
