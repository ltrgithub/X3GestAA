...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
		...
		switch (menuItem.$item.$bind) {
			...
			default:
				if (menuItem.$sourceBind == "$aw_quit") {
					awPalette.targetPage.showDiagnoses(null);
					document.site.openAuthoringPage(false);
					return false;
				} else {
	...
	drawBox: function() {
		...
		(this.$prototype.$properties = this.$prototype.$properties || {}).$viewType = {
		...
		topLeft.setAttribute("id", "s-aw-page-left");
		...
		topRight.setAttribute("id", "s-aw-page-right");
		...
		if (newData && newData.$actions) {
			...
			if (newData.$actions.saveAs) {
				if (newData.$actions.saveAs && newData.$actions.saveAs.$isDisabled && newData.$actions.saveAs.$isRequested === false) {
					...
					awPalette.targetPage.showDiagnoses({
						$diagnoses: [{
							$message: awPalette.authorPage.localize.aw_saveMessageText,
							$severity: "success"
						}],
						$links: {
							$aw_quit: {
								$title: awPalette.authorPage.localize.aw_quit
							}
						}
					});
				}
			...
			if (newData.$actions.$delete) {
				if (newData.$actions.$delete.$isDisabled && newData.$actions.$delete.$isRequested === false) {
					...
					var $diagnose = {
						$message: awPalette.authorPage.localize.aw_isDeletedMessageText.replace("{0}", "'" + selectedView.$title + "'"),
						$severity: "success"
					};
					document.site.openAuthoringPage(false);
	...
	endTargetPageUpdate: function(selectedItem, isStructureUpdated) {
		...
		this.selectItem(selectedItem || awPalette.awItem, true, true);
		this.notifyUpdate();
