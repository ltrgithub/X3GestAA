...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
	...
	selectItem: function(item, select, ensureSelect) {
		...
		if (awPalette.articleTree) {
			awPalette.articleTree.selectNode(item, select);
		}
	},
	...
	toggleItemAuthoring: function(item, enable) {
		...
		if (awPalette) {
			...
			if (domItem) {
				...
				if (item.tabTitle) {
					if (enable) {
						item.tabTitle.syraAwItem = item.id;
					} else {
						...
						delete item.tabTitle.syraAwItem;
					}
				...
				if (enable) {
					domItem.syraAwItem = item.id;
					if (!item.isLayout) {
						item.layoutSlot.syraAwItem = item.id;
					}
				} else {
					if (domItem) {
						if (item._body) {
						...
						delete domItem.syraAwItem;
						delete item.layoutSlotsyraAwItem;
					}
				}
				if (item.isLayout) {
					if (enable) {
			...
			if (item.rowSeparator) {
				...
				if (item.isLayout) {
					item.rowSeparator.syraLayout = item.id;
				} else {
					item.rowSeparator.syraAwItem = item.id;
				}
			}
			if (item.isLayout) {
				if (item.items) {
	...
	findItem: function(item, findLayout) {
		...
		if (article) {
			if (findLayout || item.syraLayout) {
				while (item && !item.syraLayout) {
					item = item.parentNode;
				}
				return (article.id == item.syraLayout) ? article : (article.layouts[item.syraLayout] || null);
			}
			while (item && !item.syraAwItem) {
				item = item.parentNode;
			}
			return (article.id == item.syraAwItem) ? article : (article.idMap[item.syraAwItem] || null);
		}
		return null;
	},
			...
			if (document.site.dragDropInstance) {
				...
				if (event.currentTarget.className.indexOf("s-aw-add-item") >= 0) {
					if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
						var node = awPalette.articleTree ? awPalette.articleTree.findNode(event) : null;
						if (node) {
