...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    onEndChangeStep: function(pageContext, menuItem, $item){
        ...
        pageContext.awArticle.reloadLayout($article);
        ...
        this.selectItem(selectItem || pageContext.awArticle, true);
    ...
    refreshStepLinks: function(){
        ...
        this.topBar.applyChange(metadata);
    },
    ...
    notifySteps: function(pageContext, resetUpdates){
        ...
        if (!resetUpdates && this.isUpdated) {
            ...
            var $article = helpers.object.clone(pageContext.awArticle.$item, true);
            this.updatedSteps.push($article);
            this.applyChangeToMobile($article);
            this.currentUpdtateStep = this.updatedSteps.length - 1;
        }
        else {
            (this.updatedSteps = []).push(pageContext.$sourceItem);
            //this.applyChangeToMobile(pageContext.$sourceItem);
    ...
    onChangeSep: function(menuItem){
        var self = this;
        var $item;
        var pageContext = self.getPageContext();
        switch (menuItem.$item.$bind) {
            case "$previousLayout":
                self.currentUpdtateStep--;
                self.onEndChangeStep(pageContext, menuItem, $item);
                break;
            case "$nextLayout":
                self.currentUpdtateStep++;
                self.onEndChangeStep(pageContext, menuItem, $item);
            ...
            case "$undoAllLayout":
                document.site.showMessage({
                    ...
                    callback: function(response){
                        if (response.$id == "yes") {
                            ...
                            self.notifySteps(pageContext, true);
                            self.onEndChangeStep(pageContext, menuItem, $item);
                        }
                    }
                });
                break;
            case "$defaultLayout":
                $item = helpers.object.clone(pageContext.targetPage.ensureDefaultArticle(pageContext.$prototype.$article, pageContext.$prototype), true);
                ...
                if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                    if ((menuItem.$item.$bind != "pattern-headerTabs") && (pageContext.awItem.$authoringLevel == "section" || pageContext.awItem.$authoringLevel == "block")) {
                        ...
                        if (pattern.isBoxOnly) {
                            var children = pageContext.awItem.layoutContent.getItems();
    ...
    selectItem: function(item, select, ensureSelect){
        ...
        awContext.toolsBar.articleToolBox.selectNode(item, select);
    },
    ...
    showHiddenItem: function(item){
        if (item.page != item) {
            var isHidden = item.$isHidden || item.isBoxHidden;
            if (false && item.isBoxHidden && !item.isAuthingEnabled && !item.$isHidden) {
                item.hideBox(true);
            }
            else {
                var display = (!item.isAuthingEnabled && (!(item.layoutParent && item.layoutParent.isRow) && isHidden)) ? "none" : "";
                if (item.layoutSlot) {
                    ...
                    document.site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", item.isAuthingEnabled && isHidden);
                }
                var domItem = item._item || item._domItem;
                if (domItem) {
                    if (item.isAuthingEnabled) {
                        if (this.$isLightMode && item.$authoringLevel == "field") {
                            ...
                            // domItem.style.display = isHidden ? "none" : "";
                            domItem.style.display = "";
                        }
                        document.site.toggleClass(domItem, "s-aw-visibility-forced", isHidden);
                    }
                    else {
    ...
    temporalySave: function(awContext, $article){
        var content = awContext.authorPage.ensureSendBag().content = {
            $article: $article || awContext.targetPage.$item
        };
        /*if (document.site.mobileGateway) {
    ...
    notifyActionChange: function(target, value, notifyServer){
        ...
        if (value && value.$save || value.saveAs) {
            awContext.targetPage.garbageFreeItems();
        }
        var $article = helpers.object.clone(awContext.targetPage.$item, true);
        delete $article.$menus;
        if ($article.$generatorVersion) {
            var $prototype = helpers.object.clone(awContext.targetPage.$prototype, true);
            document.site._cvgPageConverterMakePersistent($prototype, $article);
        }
        this.temporalySave(awContext, $article);
            ...
            if (document.site.dragDropInstance) {
                ...
                if (event.currentTarget.className.indexOf("s-aw-add-item") >= 0) {
                    if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
                        var node = awContext.toolsBar.articleToolBox.findNode(event);
                        if (node) {
    ...
    onClickItem: function(item, target){
        ...
        if (article.isOnAuthoring) {
            ...
            if (!article.page.isAuthoringEventEnabled && article.page != this && article.page != awContext.palette) {
    ...
    endTargetPageUpdate: function(selectedItem, isStructureUpdated){
        ...
        if (isStructureUpdated) {
            awContext.toolsBar.articleToolBox.toggle(true);
        }
        this.selectItem(selectedItem || awContext.awItem, true, true);
        this.notifyUpdate();
        this.temporalySave(awContext);
    },
    _toggleBars: function(show){
        if (show) {
            this.paletteSlot.style.display = "";
            if (this._lastToolsBarVisible && this._lastToolsBarVisible.layoutSlot) {
                this._lastToolsBarVisible.layoutSlot.style.display = "";
                delete this._lastToolsBarVisible;
            }
        }
        else {
            this.paletteSlot.style.display = "none";
            this._lastToolsBarVisible = this.awContext.toolsBar;
            if (this._lastToolsBarVisible) {
                this._lastToolsBarVisible.layoutSlot.style.display = "none";
            }
        }
    },
    ...
    dispose: function(){
        ...
        this._lastToolsBarVisible = this._ddTabEnter = this.page = this.paletteSlot = this.topLeftBar = this._topRightBar = this.targetPage = null;
