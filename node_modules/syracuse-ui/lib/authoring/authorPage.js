...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    toggleItemAuthoring: function(item, enable){
        ...
        if (item.isMenuGroup || item.isMenuItem) {
            if (this.targetPage.$pageCategory != "dashboard") {
                return;
            }
        }
    ...
    onAddNewField: function(targetItem, $newItem){
        if ($newItem.$fieldCategory == "link") {
            delete this.targetPage.garbageLinks[$newItem.$fieldBind];
        }
        var newChild = this.addNewItem(targetItem, "field", $newItem.$fieldBind, undefined, undefined, $newItem.$fieldCategory);
        this.palette.onExcludeField($newItem.$fieldBind, true);
            ...
            if (document.site.dragDropInstance) {
                ...
                if (event.currentTarget.className.indexOf("s-aw-add-item") >= 0) {
                    if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
                        var node = self.palette.treeContent.findNode(event);
                        if (node) {
                        ...
                        if (document.site.requestedDDAuthoringItem) {
                            ...
                            switch (menuItem.$item.$bind) {
                                ...
                                default:
                                    ...
                                    $drop.$category = menuItem.$item.$fieldCategory;
                                    break;
                            }
    ...
    onDropTabBox: function(targetItem, dropItem, $drag){
        var newChild;
        ...
        if (!targetItem.layoutParent.layoutParent) {
            ...
            newChild = this.addNewItem(layoutTab, dropItem.$authoringLevel, dropItem.$bind, dropItem, $drag.$insertAt);
            /*newChild = layoutRoot.loadChildItem(dropItem, null, $drag.$insertAt == "top" ? 0 : null);
             if (isDropItemTab && !newChild.loaded && newChild.$item.$opened !== false) {
             newChild.openBox(true);
             }*/
        }
        else {
            newChild = this.addNewItem(layoutRoot, dropItem.$authoringLevel, dropItem.$bind, dropItem, $drag.$insertAt);
            //newChild = layoutRoot.loadChildItem(dropItem, null, $drag.$insertAt == "top" ? 0 : null);
        }
        return newChild;
    },
