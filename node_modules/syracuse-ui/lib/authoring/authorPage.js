...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    toggleItemAuthoring: function(item, enable){
        ...
        if (item.$item && (item.$item.$category == "menus" || item.$item.$category == "link")) {
        ...
        if (item.rowSeparator) {
            ...
            item.rowSeparator.setAttribute("data-s-item", item.id);
        }
        if (item.isLayout) {
            if (item.items) {
