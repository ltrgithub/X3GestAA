...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
	...
	refreshStepLinks: function() {
		if (this.updatedSteps.length > 1) {
			this.links.$authorPreviousLayout.title = this.localize.aw_previousLayoutOn.replace("{0}", this.currentUpdtateStep).replace("{1}", this.updatedSteps.length);
			this.disableLink("$authorPreviousLayout", this.currentUpdtateStep == 0);
			if (this.currentUpdtateStep == this.updatedSteps.length - 1) {
				this.links.$authorNextLayout.title = this.localize.aw_nextLayout;
				this.disableLink("$authorNextLayout", true);
			} else {
				this.links.$authorNextLayout.title = this.localize.aw_nextLayoutOn.replace("{0}", this.currentUpdtateStep + 1).replace("{1}", this.updatedSteps.length);
				this.disableLink("$authorNextLayout", false);
			}
			this.disableLink("$authorUndoAllLayout", false);
		} else {
			this.links.$authorNextLayout.title = this.localize.aw_nextLayout;
			this.disableLink("$authorPreviousLayout", true);
			this.disableLink("$authorNextLayout", true);
			this.disableLink("$authorUndoAllLayout", true);
		}
	},
	...
	createContext: function(awArticle) {
		if (this.awPalette) {
			this.awPalette.toggleContext(false);
		}
	...
	onMenuClick: function(menuItem) {
		...
		switch (menuItem.$item.$bind) {
			case "$delete":
				if (menuItem.page != awPalette.targetPage) {
	...
	addLink: function(parent, $bind, $title, icon, $skin) {
		var link = document.createElement("a");
		link.title = $title;
		link.setAttribute("data-s-picker", $bind);
		$skin = $skin || "s-aw-top-left-link";
		if (icon) {
			link.className = $skin + "-icon";
			link.style.backgroundImage = "url('/syracuse-ui/themes/desktop/sage/images/authoring/s-aw-" + icon + ".png')";
		} else {
			link.className = $skin;
			link.textContent = $title;
		}
		this.links[$bind] = link;
		return parent.appendChild(link);
	},
	disableLink: function($bind, isDisabled) {
		document.site.toggleClass(this.links[$bind], "s-disabled", isDisabled);
	},
	drawBox: function() {
		...
		links.id = "s-aw-top-left-links";
		this.addLink(links, "$authorHelp", this.localize.aw_help, "help");
		this.addLink(links, "$authorPreviousLayout", this.localize.aw_previousLayout, "previousLayout");
		this.addLink(links, "$authorNextLayout", this.localize.aw_nextLayout, "nextLayout");
		this.addLink(links, "$authorUndoAllLayout", this.localize.aw_undoAllLayout);
		topSlot.appendChild(links);
		this.disableLink("$authorPreviousLayout", true);
		this.disableLink("$authorNextLayout", true);
		this.disableLink("$authorUndoAllLayout", true);
		...
		links.appendChild(this.transactionLabel);

		topSlot = document.createElement("div");
		topSlot.id = "s-aw-page-views";
		links = document.createElement("div");
		links.id = "s-aw-viewType";
		this.addLink(links, "$viewTypePreview", this.localize.aw_viewTypePreview, null, "s-aw-viewType-link");
		this.addLink(links, this.$viewType = "$viewTypeLayout", this.localize.aw_viewTypeLayout, null, "s-aw-viewType-link");
		this.addLink(links, "$viewTypeStructure", this.localize.aw_viewTypeStructure, null, "s-aw-viewType-link");
		if (document.site.isMobileSite) {
			this.addLink(links, "$viewTypeJSON", this.localize.aw_viewTypeJSON, null, "s-aw-viewType-link");
		}
		document.site.toggleClass(this.links[this.$viewType], "s-selected", true);
		topSlot.appendChild(links);
		this.layoutSlot.appendChild(topSlot);
	...
	selectItem: function(item, select, ensureSelect) {
		...
		if (awPalette.toolbar.itemTree) {
			awPalette.toolbar.itemTree.selectNode(item, select);
		}
	},
	...
	toggleArrayFieldAuthoring: function(item, enable) {
		if (this.awPalette.awArticle.isList && item == this.awPalette.awArticle) {
			if (item.$item.$format == "grid") {
				if (item.$item.$cards) {
					...
					if (item.$item.$cards.$position == "inline") {
						...
						recordCardView = item.builder.recordCardView;
					}
					if (recordCardView) {
						if (!recordCardView.cardview) {
							recordCardView.builder.toggleCard(recordCardView, true);
						}
			...
			if (document.site.dragDropInstance) {
				...
				if (event.currentTarget.className.indexOf("s-aw-add-item") >= 0) {
					if (event.currentTarget.className.indexOf("s-aw-tree-item") >= 0) {
						var node = awPalette.toolbar.itemTree.findNode(event);
						if (node) {
							document.site.requestedDDAuthoringItem = node.item;
							return false;
						}
					} else {
						document.site.requestedDDAuthoringItem = self.findItem(event.currentTarget);
						return false;
					}
				} else {
				...
				setTimeout(function() {
					...
					if (layout.items.length > 0 && layout.isRow) {
						...
						if (layoutParent) {
							...
							layoutParent.removeItem(layout.boxParent, true);
						}
					}
	...
	endTargetPageUpdate: function(selectedItem, isStructureUpdated) {
		...
		if (isStructureUpdated) {
			awPalette.toolbar.toggleArticleToolBox(false);
		}
	...
	_toggleViewType: function($viewType) {
		if (this.$viewType) {
			document.site.toggleClass(this.links[this.$viewType], "s-selected", false);
		}
		this.$viewType = $viewType;
		var isBarsVisible = true;
		if (this.isOnPreviewMode && $viewType != "$viewTypePreview") {
			this.isOnPreviewMode = false;
			this.toggleItemAuthoring(this.targetPage, true);
		}
		if (this.isJsonMode && $viewType != "$viewTypeJSON") {
	...
	ensureLayoutHasParent: function(layoutContent) {
		var $layoutType = layoutContent.$layout.$layoutType,
			$widths = layoutContent.$layout.$widths,
			$autoSize = layoutContent.$layout.$autoSize;
		var boxParent = layoutContent.boxParent;
		var children = layoutContent.extractAndDispose();
		...
		(layoutContent = boxParent.layoutContent.items[0]).removeSpaceBox(true);
		layoutContent.newLoadChildItems(children);
		return layoutContent;
	},
	insertRow: function(targetRow, $insertAt) {
		var $curRow = {
			$layoutType: targetRow.$layout.$layoutType,
			$widths: targetRow.$layout.$widths,
			$autoSize: targetRow.$layout.$autoSize
		};
		if (!targetRow.layoutParent) {
			targetRow = this.ensureLayoutHasParent(targetRow);
		}
		var index = targetRow.layoutParent.items.indexOf(targetRow);
		var newRow = targetRow.layoutParent.loadChildItem(null, {
			$layoutType: $curRow.$layoutType,
			$widths: $curRow.$widths,
			$autoSize: $curRow.$autoSize,
		}, ($insertAt == "bottom") ? ++index : index);
	...
	dispose: function() {
		...
		if (this.contextStack && this.contextStack.length) {
			...
			if (this.$viewType && this.$viewType != "$viewTypeLayout") {
