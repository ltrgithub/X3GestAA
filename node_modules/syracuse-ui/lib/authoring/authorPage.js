...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    beginAuthoring: function(awPageSlot){
        ...
        if (this.targetPage.menuBar) {
            this.isMenuBarVisible = this.targetPage.menuBar.isSlotVisible;
    ...
    toggleViewType: function($viewType){
        ...
        if ($viewType == "menus") {
            this.targetPage.dataSlot.style.display = "none";
            this.targetPage.menuBar._slot.style.display = "";
            this.targetPage.menuBar._slot.style.width = "auto";
            this.targetPage.menuBar.pickerBar.style.display = "none";
            //this.targetPage.menuBar._header.style.visibility = "hidden";
        }
        else {
            this.targetPage.dataSlot.style.display = "";
            this.targetPage.menuBar._slot.style.display = "none";
            this.targetPage.menuBar._slot.style.width = "";
            this.targetPage.menuBar.pickerBar.style.display = "";
            //this.targetPage.menuBar._header.style.visibility = "";
        }
        this.treeContent.toggle($viewType == "content");
    ...
    drawBox: function(){
        ...
        this.loadNewItem(actions, {
            ...
            $isPopupContent: true,
            $headSkinLink: "s-aw-page-save-link",
            $layout: {
                $items: [{
                    $bind: "$save",
                    $isGroupHeader: true
    ...
    toggleItemAuthoring: function(item, enable){
        ...
        if (item.$isFusionPage) {
            if (item.fusionBar) {
                item.fusionBar.toggleItemAuthoring(enable);
            }
        }
        if (item.menuBar && !enable) {
            item.menuBar.toggleItemAuthoring(enable);
        }
        if (item._menusPager && !enable) {
            this.toggleItemAuthoring(item._menusPager, enable);
        }
        if (item.articleParent && item.articleParent != this.targetPage) {
            return;
        }
        if (enable && item.$item && (item.$item.$category == "menus" || item.$item.$category == "link")) {
            //temp hack for waiting menu implementation
            return;
        }
        if (item.isSpaceBox) {
            if (enable) {
        ...
        if (domItem) {
            ...
            if (item.isLayout) {
                ...
                if (item.$isHidden) { // || (item.$field && item.$field.$type == "application/x-date")) {
                    this._toggleForcedVisibility(item, enable && !this.isOnPreviewMode);
                }
            }
        }
    ...
    _toggleForcedVisibility: function(item, forced){
        if (item.layoutSlot) {
            item.layoutSlot.style.display = (!this.$isLightMode && forced) ? "" : "none";
            document.site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", forced);
        }
        var domItem = item._item || item._domItem;
        if (domItem) {
            domItem.style.display = (!this.$isLightMode && forced) ? "" : "none";
    ...
    applyChange: function(newData){
        if (newData && newData.$actions && (newData.$actions.$save || newData.$actions.saveAs)) {
            if (newData.$actions.saveAs && newData.$actions.saveAs.$isDisabled && newData.$actions.saveAs.$isRequested === false) {
                ...
                if (newData.$actions.saveAs.$diagnoses) {
                    for (var ii = 0, jj = newData.$actions.saveAs.$diagnoses.length; ii < jj; ii++) {
                        if (newData.$actions.saveAs.$diagnoses[ii].severity !== "success") {
                            return false;
                        }
                    }
                }
    ...
    excludeItem: function(removedItem){
        ...
        if (excludedFields) {
            for (var ii = 0, jj = excludedFields.length; ii < jj; ii++) {
                this.palette.onExcludeField(excludedFields[ii].$item.$bind, false);
            }
        }
    ...
    authorizeEvent: function(item){
        return (item.page && (item.page.$dialogMode || (item.page == this) || (item.page == this.palette)));
    },
    bindEvents: function(){
        var self = this;
            ...
            if (!item.isLayout && !isTabLayout) {
                ...
                if ($oldLayoutType == "tabs" && !isTabLayout) {
                    for (var mm = 0, kk = item.items.length; mm < kk; mm++) {
                        ...
                        if (!child.isLayout && child.isSection && !child._tabTitle && !child.loaded && child.$item.$opened !== false) {
                            child.openBox(true);
                        }
                    }
                }
    ...
    dispose: function(){
        ...
        if (this.targetPage) {
            ...
            this.toggleItemAuthoring(this.targetPage, false);
            if (this.targetPage.menuBar) {
                this.targetPage.menuBar.isSlotVisible = this.isMenuBarVisible;
                this.targetPage.menuBar.ensureState();
            }
