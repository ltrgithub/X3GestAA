...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    /*       if (newChild.layoutParent && newChild.layoutParent.layoutParent) {
     
     }*/
    autoSizeRow: function(layoutRow){
        if (layoutRow && layoutRow.$layout) {
            if (layoutRow.$layout.$layoutType == "row") {
                var addLayout;
                for (var ii = 0, jj = layoutRow.items.length; ii < jj; ii++) {
                    var child = layoutRow.items[ii];
                    if (child.isLayout && child.items.length > 1) {
                        addLayout = true;
                        break;
                    }
                }
                if (addLayout) {
                    for (var ii = 0, jj = layoutRow.items.length; ii < jj; ii++) {
                        ...
                        if (!child.isLayout && !child.isSpaceBox) {
                            var stack = layoutRow.createChildItem({
                                $layoutType: "stack",
                                $items: []
                            }, null, ii);
                            stack.loadChildItem(child);
                            stack.ensureEndUpdateLayout();
                        }
                    }
                }
                else {
                    for (var ii = 0, jj = layoutRow.items.length; ii < jj; ii++) {
                        ...
                        if (child.isLayout) {
                            if (child.items.length == 1) {
                                newChild = child.items[0];
                            }
                            if (newChild) {
                                layoutRow.loadChildItem(newChild, null, layoutRow.getItemIndex(child.$item));
                            }
                            layoutRow.removeItem(child, true);
                        }
                    }
                }
            }
            else {
                this.autoSizeRow(layoutRow.layoutParent ? layoutRow.layoutParent : (layoutRow.boxParent ? layoutRow.boxParent.layoutParent : null));
            }
        }
    },
        ...
        switch (targetItem.$authoringLevel) {
            ...
            case "article":
                if ($newAuthoringLevel == "field") {
                    ...
                    var children = this.getItemsFromLayout(targetItem.layoutContent, null, "field");
            ...
            case "block":
                switch ($newAuthoringLevel) {
                    ...
                    case "section":
                        if (dropItem) {
                            var children = this.getItemsFromLayout(dropItem.layoutContent);
    ...
    deleteLayout: function(layout, spaceBox){
        ...
        if (layout.items.length > 0 && layout.$layout.$layoutType == "row") {
            layout.$layout.$widths = null;
            if (layout.items.length == 1 && layout.items[0].isLayout) {
                if (layoutParent) {
                    layoutParent.appendChildItem(layout.items[0], layoutParent.getItemIndex(layout.$item));
                    layoutParent.removeItem(layout);
                }
                else {
        ...
        this.ensureLayoutTree(layout)
        this.ensureLayoutTree(layoutParent);
    },
    newConvert: function(layout, $newLayout){
        ...
        if (layout.$layout.$layoutType == "row") {
            if ($oldLayoutType == "row") {
                ...
                var newWidths = layout.$layout.$widths.split(",");
                if (oldWidths.length > newWidths.length) {
                    ...
                    while (layout.items.length > newWidths.length) {
                ...
                if (layout.items.length > 1) {
                    ...
                    stackCol.ensureEndUpdateLayout();
                }
            }
        }
        else {
        ...
        if (isTabLayout) {
            ...
            for (var ii = 0, jj = toClean.length; ii < jj; ii++) {
                layout.removeItem(toClean[ii], true);
            }
        }
        for (var ii = 0, jj = layout.items.length; ii < jj; ii++) {
            var item = layout.items[ii];
        ...
        layout.ensureEndUpdateLayout();
    },
    ensureLayoutTree: function(item){
        if (item) {
            var layout = item.isLayout ? item : item.layoutContent;
            if (!layout) {
                layout = item.layoutParent;
            }
            while (layout) {
                layout.ensureEndUpdateLayout();
                layout = layout.layoutParent ? layout.layoutParent : layout.boxParent ? layout.boxParent.layoutParent : null;
            }
        }
    },
    dispose: function(){
