...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    autoSizeRow: function(awItem){
        if (awItem.isSection) {
            var layoutParent = awItem.layoutParent;
            if (layoutParent.$layout.$layoutType == "row") {
                var addLayout;
                for (var ii = 0, jj = layoutParent.items.length; ii < jj; ii++) {
                    var child = layoutParent.items[ii];
                ...
                if (addLayout) {
                    ...
                    var stack = layoutParent.createChildItem({
                        $layoutType: "stack",
                        $items: []
                    }, null, layoutParent.getItemIndex(awItem.$item));
                    stack.loadChildItem(awItem);
                    ...
                    for (var ii = 0, jj = layoutParent.items.length; ii < jj; ii++) {
                        ...
                        if (child.isLayout) {
                            ...
                            layoutParent.isSpaceBoxDisabled = true;
                            if (newChild && !newChild.isSpaceBox) {
                                layoutParent.loadChildItem(newChild, null, layoutParent.getItemIndex(child.$item));
                            }
                            layoutParent.removeItem(child, true);
                        }
                    }
                }
