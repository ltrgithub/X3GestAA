...
exports.AuthorPage = helpers.defineClass(AuthorPage, RawPage, {
    ...
    onChangeSep: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            case "$previousLayout":
                this.currentUpdtateStep--;
                this.onEndChangeStep(pageContext, menuItem, $item);
                break;
            case "$nextLayout":
                this.currentUpdtateStep++;
                this.onEndChangeStep(pageContext, menuItem, $item);
            ...
            case "$undoAllLayout":
                document.site.showMessage({
                    $title: this.localize.aw_updateMessageTitle,
                    $message: this.localize.aw_confirmUndoAll,
                    $type: "question",
                    $buttons: "yesno",
                    callback: function(response){
                        if (response.$id == "yes") {
                            this.currentUpdtateStep = 0;
                            this.notifySteps(pageContext, true);
                            this.onEndChangeStep(pageContext, menuItem, $item);
                        }
                    }
                });
