...
exports.Palette = helpers.defineClass(Palette, RawPage, {
	...
	loadTitlesChoices: function() {
		...
		if ($enum.length) {
			this.$prototype.$properties.$titleChoice.$value.$enum.unshift({
				$value: 0,
				$title: this.authorPage.localize.aw_titleLocalization
			});
		} else {
			this.$prototype.$properties.$titleChoice.$value.$enum.unshift({
				$value: 0,
				$title: this.authorPage.localize.aw_titleVisible
			});
	...
	loadBox: function(authorPage, awArticle) {
		var self = this;
		var localize = self.authorPage.localize;
		...
		self.$prototype = {
			$properties: {
				...
				$layoutTabChoice: {
					...
					$value: {
						...
						$enum: [{
							$value: "tabs",
							$title: localize["aw_s-layout-tabs"]
						}, {
							$value: "stack",
							$title: localize["aw_s-layout-panels"]
						}]
					}
				},
				$rowAlign: {
			...
			$links: {
				$addSection: {
					$title: localize.aw_addSection
				},
				$addBlock: {
					$title: localize.aw_addBlock
				},
				$addMenus: {
					$title: localize.aw_addMenus
				},
				$excludeMe: {
					$title: localize.aw_excludeMe
				},
				$addRowBefore: {
					$title: localize.aw_addRowBefore
				},
				$addRowAfter: {
					$title: localize.aw_addRowAfter
				},
				"s-layout-type-1": {
					$title: "1",
					$widths: "100"
		...
		self.authorPage.paletteSlot.appendChild(self.layoutSlot).className = "s-aw-palette-body";
		...
		//  self.$$item = self.$$slot = $(self.toolbarSlot = document.createElement("div"));
		...
		self.ensureState();
		self.authorPage.selectItem(self.awArticle, true);
	...
	onMenuClick: function(menuItem) {
		switch (menuItem.$item.$bind) {
			...
			case "$addBlock":
				...
				return false;
			case "$excludeMe":
				var pageContext = this.authorPage.getPageContext();
				pageContext.authorPage.endTargetPageUpdate(drawHelper.excludeItem(pageContext, this.awItem), true);
				break;
			case "$addRowBefore":
			case "$addRowAfter":
					...
					if (menuItem.$item.$fieldBind) {
						...
						if (menuItem.$item.$bind.indexOf("pattern") == 0) {
							...
							if (menuItem.$item.$bind.indexOf("s-layout-type-") >= 0) {
								...
								this.authorPage.endTargetPageUpdate(layout.$layout.$layoutType == "stack" ? this.awItem : layout, true);
							}
						}
					}
				}
	...
	resizeDialogPage: function() {
		if (this.targetPage.dialogWrapper) {
			...
			if (width) {
				this.getawScrollview().style.width = (width - (this.authorPage._widths.palette + this.toolbarSlot.offsetWidth)) + "px";
			}
		}
	},
	ensureSelectedItemTitle: function() {
		...
		if (title == "" && this.awItem.$authoringLevel == "field") {
			title = this._getBindTitle(this.awItem.$item.$bind, this.awItem.$field);
		}
	...
	_setSelectedPanel: function() {
		...
		this.panels[paletteId].setTitle(this.awItem.isMenuGroup ? localize.aw_menus : localize["aw_" + paletteId]);
	...
	showSettingPanel: function() {
		...
		metadata.$properties = {
			$layoutTabChoice: {
				$isDisabled: (count != 1),
				$title: this.awItem.$authoringLevel == "article" ? this.authorPage.localize.aw_showSectionsAs : this.authorPage.localize.aw_showBlocksAs
			}
		};
			...
			switch (field.$item.$bind) {
				case "$boxTabChoice":
					if (this.awItem.layoutParent.$layoutType != value) {
						drawHelper.switchTabPanelBox(this, value);
					}
					break;
				case "$rowAlign":
					drawHelper.setRowAlign(this, value);
					break;
				...
				case "$labelAlignment":
					...
					if (this.awItem.$authoringLevel == "field") {
						...
						if (metaData.$fieldsIsTopLabelAlignment = (value == "t")) {
							this.authorPage._notifyChildFields(this.awItem, true, "$isTopLabelAlignment");
						} else {
							this.authorPage._notifyChildFields(this.awItem, false, "$isTopLabelAlignment");
							this.authorPage._notifyChildFields(this.awItem, metaData.$fieldsIsRightTextLabelAlignment = (value == "lr"), "$isRightTextLabelAlignment");
						}
						this.authorPage._notifyChildBox(this.awItem, metaData);
					}
					this.authorPage.notifyUpdate();
					break;
				default:
					...
					if (this.awItem.$authoringLevel == "field") {
						...
						if ($bind.indexOf("$fields") == 0) {
							...
							this.authorPage._notifyChildFields(this.awItem, value, "$" + fieldBind);
						}
						this.awItem.applyDesignMetaData(metaData, true);
					}
					this.authorPage.notifyUpdate();
	...
	_getRowSection: function(isField) {
		...
		return {
			...
			$isBoxCollapsable: true,
			$title: this.authorPage.localize.aw_rowLayout,
			$isRow: true,
			$layout: {
				$items: [{
					$category: "menus",
					$title: this.authorPage.localize.aw_addrow,
					$skin: "s-aw-cols",
					$isHidden: true,
					...
					$category: "menus",
					$title: this.authorPage.localize.aw_columns,
					$skin: "s-aw-cols",
					"$itemSkin": "s-aw-cols-link",
					$layout: {
						$layoutType: "row",
	...
	_getLayoutSection: function($level) {
		...
		var $layout = {
			...
			$isBoxCollapsable: true,
			$isStructure: true,
			$title: this.authorPage.localize.aw_layout,
			$layout: {
				$items: [{
					$category: "menus",
					$title: this.authorPage.localize.aw_columns,
	...
	_getModelsSection: function($level) {
		this.applyChange({
			$links: {
				$defaultLayout: {
					$title: this.authorPage.localize.aw_defaultLayout
				},
				"pattern-headerTabs": {
					$title: this.authorPage.localize.aw_autoLayout
				},
				"pattern-1": {
					$title: this.authorPage.localize.aw_layoutComposite
				},
				"pattern-2": {
					$title: this.authorPage.localize.aw_layoutComposite
				},
		...
		return {
			...
			$isBoxCollapsable: true,
			$css: "s-aw-palette-" + $level,
			$title: this.authorPage.localize.aw_models,
			$layout: {
				$items: [{
					$category: "menus",
					$itemIcon: {
	...
	_createInsertLink: function($bind, $field) {
		...
		var bindTitle = this._getBindTitle($bind, $field);
		if ($action.$title == "") {
	...
	_loadInsertLinks: function() {
		...
		if (binds.length > 0) {
			...
			var $fields = this.authorPage.getItemSettings($item.$layout);
			...
			if ($item.$cards) {
				var $fields = this.authorPage.getItemSettings($item.$cards.$layout);
				for (var ii = 0, jj = $fields.length; ii < jj; ii++) {
					if ($fields[ii].$bind) {
						var $action = $actions["$insertAction" + $fields[ii].$bind];
						if ($action) {
	...
	onEndExcludeItem: function() {
		if (!this.isArrayField && this.targetPage.isDashBoard) {
			this.ensureOpenedTab();
		}
		this.toggleArticleToolBox(true);
	},
				...
				if (this.targetPage.isDashBoard) {
					...
					if (isExcluded) {
						if (!this.menuItems[actionId]) {
							...
							var title = $field.$title ? this.targetPage.getLocalizeText($field.$title) : "";
							$actions[actionId] = {
	...
	_createArrayBoxes: function() {
		var $toolbarLayout = {
			...
			$items: [{
				$category: "section",
				$skin: "s-aw-tlbr",
				$title: this.authorPage.localize.aw_listColumns,
				$layout: {
					$items: []
				}
			}, {
				...
				$category: "section",
				$skin: "s-aw-tlbr",
				$title: this.authorPage.localize.aw_listCard,
				$layout: {
					$items: []
				}
			}]
		...
		if (this.awArticle.chart && this.awArticle.chart.$authoringType == "cube$highCharts") {
			...
			if ($fieldCube && $fieldCube.$measures) {
				...
				$toolbarLayout.$items.push({
					$category: "section",
					$skin: "s-aw-tlbr-graph",
					$isContentLoaded: true,
					$title: this.authorPage.localize.aw_graph,
					$layout: {
						$items: $items
					}
				});
		...
		$toolbarLayout.$items.push({
			...
			$skin: "s-aw-tlbr",
			$isContentLoaded: true,
			$title: this.authorPage.localize.aw_insert,
			$layout: {
				$items: [{
					$category: "link",
					$skin: "s-aw-menus-link",
		...
		this.columnsToolBarBox.notifyAddField();
	...
	toggleColumnsToolBox: function(show, disposing) {

		if (this.awArticle) {
			if (show) {
				this.columnsToolBarBox.toolBoxSlot.style.display = "none";
				document.site.emptyDom(this.columnsToolBarBox.toolBoxSlot);
				var items;
				var $items = this.awArticle.$item.$layout.$items;
				this.columnsToolBoxRoot = document.createElement("ul");
				this.columnsToolBoxRoot.className = "s-aw-list-cols";
				this.columnNodes = [];
				for (var ii = 0, jj = $items.length; ii < jj; ii++) {
					var $item = $items[ii];
					var $field = this.awArticle.$fieldProperties[$item.$bind];
					if ($field && !$field.$isHidden && !$field.$isExcluded) {
						...
						if ($item.$isFixed) {
							...
							node.isFixed.title = this.authorPage.localize.aw_listUnFixeColumn;
						} else {
							node.isFixed.title = this.authorPage.localize.aw_listFixeColumn;
						}
						node.$delete.className = "s-aw-list-cols-col-delete";
						node.$delete.title = this.authorPage.localize.aw_listHideColumn;

						node.title.className = "s-aw-list-cols-col-title";
						node.title.textContent = this.awArticle.getLocalizedTitle($field.$title);
						node.item.appendChild(node.isFixed);
						node.item.appendChild(node.$delete);
						node.item.appendChild(node.title);
						this.columnsToolBoxRoot.appendChild(node.item);
						this.columnNodes.push(node);
					}
				}
				this.columnsToolBarBox.toolBoxSlot.appendChild(this.columnsToolBoxRoot);
				this.columnsToolBarBox.toolBoxSlot.style.display = "";
			} else {
				if (this.columnsToolBarBox.toolBoxSlot) {
					this.columnsToolBarBox.toolBoxSlot.style.display = "none";
				}
				if (disposing) {
					document.site.emptyDom(this.columnsToolBarBox.toolBoxSlot);
					delete this.columnsToolBarBox.toolBoxSlot;
				}
				if (disposing) {
	...
	toggleArticleToolBox: function(show, disposing) {
		if (this.awArticle) {
			if (show) {
				this._nodes = {};
				this.articleToolBarBox.toolBoxSlot.style.display = "none";
				document.site.emptyDom(this.articleToolBarBox.toolBoxSlot);
				this._buildArticleTreeNode(this.awArticle, {
					node: this.articleToolBarBox.toolBoxSlot,
					isRoot: true
				});
				this.articleToolBarBox.toolBoxSlot.style.display = "";
			} else {
				if (this.articleToolBarBox.toolBoxSlot) {
					this.articleToolBarBox.toolBoxSlot.style.display = "none";
				}
				if (disposing) {
					document.site.emptyDom(this.articleToolBarBox.toolBoxSlot);
					delete this.articleToolBarBox.toolBoxSlot;
				}
				if (this._nodes) {
	...
	onArticleToolBoxNodeEvent: function(event) {
		...
		if (node) {
			if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
				...
				switch (event.type) {
					case "mouseenter":
						this.authorPage.toggleOverItem(node.item, true);
						this.targetPage.scrollToItem(node.item);
						break;
					case "mouseleave":
						this.authorPage.toggleOverItem(node.item, false);
						break;
					case "click":
						if (node.item == this.awArticle) {
							this.targetPage.scrollToItem(node.item);
						}
						this.selectNode(node.item, true);
						this._isSelecteDisabled = true;
						this.authorPage.selectItem(node.item, true);
	...
	_buildArticleTreeNode: function(item, parentNode) {
		var items;
		if (!item.isSpaceBox) {
			if (item.isLayout) {
				items = item.items;
				...
				if (item.$item && item.$item.$bind) {
					bindTitle = this._getBindTitle(item.$item.$bind, item.$field);
					if (title == "") {
				...
				itemNode.node.setAttribute("title", desc);
			...
			if (items) {
				for (var ii = 0, jj = items.length; ii < jj; ii++) {
					this._buildArticleTreeNode(items[ii], itemNode || parentNode);
				}
			}
		}
	},
	_getBindTitle: function($bind, $field) {
	...
	_createPageBoxes: function() {
		...
		this.toolbarLayout = this.loadNewItem(this.toolbarBody, {
			...
			$items: [{
				...
				$category: "section",
				$skin: "s-aw-tlbr",
				$title: this.authorPage.localize.aw_viewTypeContent,
				$layout: {
					$items: []
				}
			}, {
				...
				$layout: {
					$items: [{
						...
						$category: "menus",
						$title: this.authorPage.localize.aw_insertFields,
						$itemSkin: "s-aw-mn-field-link",
						$titleIcon: {
							$value: "template",
							$path: "authoring/s-aw-"
				...
				$layout: {
					$items: [{
						...
						$items: [{
							...
							$category: "section",
							$skin: "s-aw-tb-fields",
							$title: this.authorPage.localize.aw_insertLinks,
							$layout: {
								$items: []
							}
						}, {
							...
							$category: "section",
							$skin: "s-aw-tb-fields",
							$title: this.authorPage.localize.aw_insertVignettes,
							$layout: {
								$items: []
							}
						}]
	...
	onNotifyDataChange: function(field, value) {
		if (this.isArrayField && field.$item.$measure) {
	...
	onColumnNodeFixeEvent: function(event) {
		delete document.site.requestedDDAuthoringItem;
		event.stopPropagation();
		var foundIndex = this.findColumnNode(event, true);
		var isFixed = !(event.target.className.indexOf("s-field-fixed") >= 0);
		var $items = this.awArticle.$item.$layout.$items;
		if (isFixed) {
			for (var ii = 0, jj = foundIndex; ii <= jj; ii++) {
				var node = this.columnNodes[ii];
				this.awArticle.builder.cols[node.$item.$bind].$isFixed = node.$item.$isFixed = isFixed;
				node.isFixed.className = "s-aw-list-cols-col-fixe s-field-fixed";
				node.isFixed.title = this.authorPage.localize.aw_listUnFixeColumn;
			}
		} else {
			for (var ii = foundIndex, jj = this.columnNodes.length; ii < jj; ii++) {
				var node = this.columnNodes[ii];
				this.awArticle.builder.cols[node.$item.$bind].$isFixed = node.$item.$isFixed = isFixed;
				node.isFixed.className = "s-aw-list-cols-col-fixe";
				node.isFixed.title = this.authorPage.localize.aw_listFixeColumn;
		...
		this.authorPage.endTargetPageUpdate();
	},
	onColumnNodeHideEvent: function(event) {
		delete document.site.requestedDDAuthoringItem;
		event.stopPropagation();
		var node = this.findColumnNode(event);
		if (node) {
			this.awArticle.builder.removeColumn(node.$item.$bind);
			document.site.removeDomChild(node.item);
			this.authorPage.notifyUpdate();
			this.onEndChangeStep();
		}

	},
	onColumnNodeEvent: function(event) {
		delete document.site.requestedDDAuthoringItem;
		event.stopPropagation();
		var node = this.findColumnNode(event);
		if (node) {
			if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
				...
				switch (event.type) {
					case "click":
						var node = this.findColumnNode(event);
						if (node) {
							var col = this.awArticle.builder.cols[node.$item.$bind];
							if (col) {
								this.awArticle.builder.scroller.scrollToTitleItem(col.titleCell);
							}
						}
						break;
				}
	...
	findColumnNode: function(event, getIndex) {
		var $$target = $(event.target);
		...
		if ($bind) {
			for (var ii = 0, jj = this.columnNodes.length; ii < jj; ii++) {
				if (this.columnNodes[ii].$item.$bind == $bind) {
					return getIndex ? ii : this.columnNodes[ii];
				}
			}
		}
		return null;
	...
	_bindEvents: function(bind) {
		...
		if (self.toolbarBody) {
			if (bind) {
				$(self.toolbarBody).delegate(".s-aw-list-cols-col", "mousedow.authoringList click.authoringList", function(event) {
					self.onColumnNodeEvent(event);
					return false;
				}).delegate(".s-aw-list-cols-col-fixe", "click.authoringList", function(event) {
					self.onColumnNodeFixeEvent(event);
					return false;
				}).delegate(".s-aw-list-cols-col-delete", "click.authoringList", function(event) {
					self.onColumnNodeHideEvent(event);
					return false;
					...
					self.onArticleToolBoxNodeEvent(event);
					return false;
	...
	dispose: function(onAuthorPageDispose) {
		...
		if (this.articleToolBarBox) {
			this.toggleArticleToolBox(false, true);
			this.articleToolBarBox = null;
		}
		...
		this.authorPage = this.targetPage = this.$sourceItem = null;
