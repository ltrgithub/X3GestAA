			...
			if (show) {
				...
				for (var ii = 0, jj = $items.length; ii < jj; ii++) {
					...
					if ($field && !$field.$isHidden && !$field.$isExcluded) {
						...
						var bindTitle = "",
							desc = title;
						bindTitle = this.getBindTitle($item.$bind, $field);
						if (title == "") {
		...
		if (self.toolbarBody) {
			if (bind) {
				$(self.toolbarBody).delegate(".s-aw-list-cols-col,.s-aw-list-cols-col-title,.s-aw-list-cols-col-fixe,.s-aw-list-cols-col-delete", "click.authoringList", function(event) {
					...
					if ($bind) {
						for (var ii = 0, jj = self.columnNodes.length; ii < jj; ii++) {
							if (self.columnNodes[ii].$item.$bind == $bind) {
								node = self.columnNodes[nodeIndex = ii];
								break;
							}
						}
					}
					if (node) {
						if (event.currentTarget.className.indexOf("s-aw-list-cols-col-delete") >= 0) {
							self.awArticle.builder.removeColumn(node.$item.$bind);
							...
							if (event.currentTarget.className.indexOf("s-aw-list-cols-col-fixe") >= 0) {
								...
								var $items = self.awArticle.$item.$layout.$items;
								if (isFixed) {
									for (var ii = 0, jj = nodeIndex; ii <= jj; ii++) {
										var colNode = self.columnNodes[ii];
										self.awArticle.builder.cols[colNode.$item.$bind].$isFixed = colNode.$item.$isFixed = isFixed;
									...
									for (var ii = nodeIndex, jj = self.columnNodes.length; ii < jj; ii++) {
										var colNode = self.columnNodes[ii];
										self.awArticle.builder.cols[colNode.$item.$bind].$isFixed = colNode.$item.$isFixed = isFixed;
								...
								if (event.target.className.indexOf("s-aw-tree-item-opener-children") >= 0) {
									...
									switch (event.type) {
										case "click":
											var col = self.awArticle.builder.cols[node.$item.$bind];
											if (col) {
												self.awArticle.builder.scroller.scrollToTitleItem(col.titleCell);
											}
											break;
									}
								}
