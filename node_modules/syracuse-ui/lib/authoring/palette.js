...
exports.Palette = helpers.defineClass(Palette, RawPage, {
	...
	showSettingPanel: function() {
		...
		if (layout && layout.$item) {
			colCount = (layout.$item.$widths || "100").split(",").length;
		}
		var metadata = {
			$links: {}
		};
		...
		metadata.$properties = {
			$layoutTabChoice: {
				...
				$title: this.awItem.$authoringLevel == "article" ? localize.aw_showSectionsAs : localize.aw_showBlocksAs
			}
		};
		...
		for (var ii = 0, jj = sections.length; ii < jj; ii++) {
			...
			if (section.$item.$isColumnSection) {
				paletteSectionProvider.fillColChoices(localize, section, colCount);
			}
		}
	},
