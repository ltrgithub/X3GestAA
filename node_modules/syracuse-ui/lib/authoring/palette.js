			...
			for (var ii = 0, jj = $fieldBinds.length; ii < jj; ii++) {
				...
				if ($bind.charAt(0) != "$") {
					...
					if ($field && $field.$type && !$field.$isExcluded) {
						...
						var title = $field.$title ? this.targetPage.getLocalizeText($field.$title) : "";
						$actions[actionId] = {
							$title: (title == "") ? ($field.$X3Name || $bind) : title
						};
						var $item = {
							$category: "link",
							$skin: "s-aw-mn-field-link",
		...
		if (!item.isSpaceBox) {
			if (item.isLayout) {
				...
				if (item.$item && item.$item.$bind) {
					var $X3Name = item.$field ? item.$field.$X3Name : null;
					if (title == "") {
						title = $X3Name || item.$item.$bind;
					} else {
						bindTitle = " (" + $X3Name || item.$item.$bind + ")";
					}
				}
				itemNode.title.textContent = title;
				itemNode.node.setAttribute("title", title + bindTitle);
