			...
			if (show) {
				...
				for (var ii = 0, jj = $items.length; ii < jj; ii++) {
					...
					if ($field && !$field.$isHidden && !$field.$isExcluded) {
						...
						var title = this.awArticle.getLocalizedTitle($field.$title);
						var bindTitle = "",
							desc = title;
						bindTitle = this._getBindTitle($item.$bind, $field);
						if (title == "") {
		...
		if (!this._isSelecteDisabled && item && !item.disposed) {
			...
			if (node) {
				if (select) {
					...
					this.scrollToItem(this.selectedNode.node, $(this.articleToolBarBox.toolBoxSlot), true);
				} else {
