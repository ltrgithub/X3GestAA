...
var InsertTree = require("syracuse-ui/lib/authoring/insertTree").InsertTree;
...
exports.Palette = helpers.defineClass(Palette, RawPage, {
	...
	extractFields: function(layout, fields, onlyDirectChild) {
		fields = fields || [];
		if (layout) {
			for (var ii = layout.items.length - 1; ii >= 0; ii--) {
				...
				if (item != layout.boxParent) {
					if (item.$authoringLevel === "field") {
						...
						fields = this.extractFields(item.isLayout ? item : item.layoutContent, fields);
					}
				}
			}
		}
	...
	excludeItem: function(removedItem) {
		...
		if (removedItem.$authoringLevel == "field") {
			...
			excludedFields = this.extractFields(removedItem.layoutContent);
		}
		if (excludedFields) {
	...
	onMenuClick: function(menuItem) {
		...
		switch (menuItem.$item.$bind) {
			case "$addMenus":
				var newChild = drawHelper.addNewItem(this, this.awArticle, this.awItem, "block", null, undefined, undefined, "menus");
				authorPage.endTargetPageUpdate(newChild, true);
				return false;
			case "$addSection":
			case "$addBlock":
				var newChild = drawHelper.addNewItem(this, this.awArticle, this.awItem, menuItem.$item.$bind.replace("$add", "").toLowerCase());
				authorPage.endTargetPageUpdate(newChild, true);
				return false;
			case "$excludeMe":
	...
	toggleInsertToolBox: function(show) {
		if (show) {
			if (this.awArticle && this.insertToolBarBox) {
				this.insertTree = new InsertTree();
				this.insertTree.load(this, this.awArticle, this.insertToolBarBox.toolBoxSlot);
			}
		} else {
			if (this.insertTree) {
				this.insertTree.unload();
	...
	_createInsertToolBox: function(box) {
		this.insertToolBarBox = box;
		box.toolBoxSlot = document.createElement("div");
		box.toolBoxSlot.className = "s-aw-content-tree";
		box.layoutContent._item.appendChild(box.toolBoxSlot);
	},
	...
	_createPageBoxes: function() {
		...
		this._createInsertToolBox(this.toolbarLayout.items[1]);
		this._insertMenusBox = this.toolbarLayout.items[2].layoutContent.items[2];
		this.loadAvailableFields();
	},
	_createDashboardBoxes: function() {
		...
		this._createArticleToolBox(this.toolbarLayout.items[0]);
		this._createInsertToolBox(this.toolbarLayout.items[1]);
		var insertTabs = this.toolbarLayout.items[2].layoutContent.items[2];
	...
	dispose: function(onAuthorPageDispose) {
		...
		this.toggleInsertToolBox(false);
