...
exports.Palette = helpers.defineClass(Palette, RawPage, {
	...
	excludeItem: function(removedItem) {
		...
		var index = layoutParent.items.indexOf(removedItem);
	...
	onMenuClick: function(menuItem) {
		switch (menuItem.$item.$bind) {
			case "$excludeMe":
				document.site.authorPage.endTargetPageUpdate(this.excludeItem(this.awItem), true);
				break;
		}
	},
				...
				if ($bind.charAt(0) != "$") {
					if (!(this.targetPage.$isFusionPage && $bind.charAt(0) == "b")) {
						...
						if ($field && $field.$type && !$field.$isExcluded) {
							this.toolbar.onExcludeField($bind, isExcluded, ($field.$type == "application/x-vignette" && $field.$format == "$menu") ? "menus" : "field"); //vignette 
						}
					}
				}
			}
		...
		if ($binds.length > 0) {
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if ($field) { //important test for garbage link
					this.toolbar.onExcludeField($bind, "link");
				}
			}
		}
	},
	...
	onExcludeField: function($bind, isExcluded) {
		...
		//this.toolbar.onExcludeField($bind, isExcluded);
		if (isExcluded) {
