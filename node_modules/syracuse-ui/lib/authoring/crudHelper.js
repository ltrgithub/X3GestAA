...
exports.temporalySave = function(awContext){
    var content = awContext.authorPage.ensureSendBag().content = {
        $article: awContext.targetPage.$item
    };
    if (awContext.targetPage.saveGarbageLinks) {
        awContext.targetPage.saveGarbageLinks(content.$article);
    }
};
    ...
    switch (menuItem.$item.$bind) {
        case "$delete":
            ...
            document.site.showMessage({
                $title: awContext.localize.aw_deleteMessageTitle,
                $message: awContext.localize.aw_deleteMessageText.replace("{0}", "'" + selectedView.$title + "'"),
                $type: "question",
                $buttons: "yesno",
                callback: function(response){
                    if (response.$id == "yes") {
                        awContext.authorPage.deletedView = selectedView;
        ...
        case "$close":
            if (awContext.isUpdated) {
                document.site.showMessage({
                    ...
                    $message: awContext.localize.aw_cancelMessageText,
                    $type: "question",
                    $buttons: "yesnocancel",
                    callback: function(response){
                        if (response.$id == "yes") {
                            awContext.authorPage.menuItems.$save[0].click();
                        }
                        else {
                            if (response.$id == "no") {
