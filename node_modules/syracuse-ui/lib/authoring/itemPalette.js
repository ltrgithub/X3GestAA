...
var fieldLibrary = require("./fieldLibrary");
function ItemPalette(){
}

exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
    _fillModelsMenus: function(){
        var $awModels = this.findItemSetting("$awModels", this.$item.$menus.$layout);
        var $actions = this.$prototype.$actions = this.$prototype.$actions || {};
        var $keys = Object.keys(this.authorPage.layoutAuthoring.getPatterns());
        var $row;
        for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
            ...
            $actions[$key] = {
                $title: this.authorPage.layoutAuthoring.getPattern($key).$title || this.localize.aw_layoutComposite
            };
        }
    },
        ...
        if (this.updatedSteps.length > 1) {
            metadata = {
                $links: {
                    ...
                    $undoAllLayout: {
                        $title: this.localize.aw_undoAllLayoutOn.replace("{0}", this.updatedSteps.length),
                        $isDisabled: false
                    }
                }
            };
    ...
    _onChangeSep: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            ...
            case "$undoAllLayout":
                ...
                break;
            case "$defaultLayout":
                $item = this.authorPage.$sourceItem;
                this.authorPage.notifyUpdate(true);
                break;
            case "$autoLayout":
                $item = this.authorPage.layoutAuthoring.buildAutoLayout(this.authorPage.targetPage.$prototype, helpers.object.clone(this.authorPage.$sourceItem, true));
                this.authorPage.notifyUpdate(true);
                break;
        }
    ...
    loadBox: function(){
        ...
        this.authorPage.paletteSlot.appendChild(this._bar).className = "s-aw-palette-body";
        this.$$container = $(this._bar);
        
        this.isAuthoringDisabled = true;
        ...
        switch (menuItem.$item.$bind) {
            ...
            case "$addBlock":
                this.authorPage.addNewItem({
                    targetItem: this.authorPage.awItem,
                    $category: menuItem.$item.$bind.replace("$add", "").toLowerCase()
                });
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    this.authorPage.addNewItem({
                        targetItem: this.authorPage.awItem,
                        $category: "field",
                        $bind: menuItem.$item.$fieldBind
                    });
                    this.authorPage.notifyUpdate();
                }
                else {
                    if (menuItem.$item.$bind.indexOf("s-layout-type-") >= 0) {
                        var layoutType = menuItem.$item.$bind.replace("s-layout-type-", "");
                        var $layout = (layoutType == "100") ? {
                            $layoutType: "stack"
                        } : {
                            $layoutType: "row",
                            $layoutSubType: layoutType.replace(/-/g, ",")
                        };
                        this.authorPage.layoutAuthoring.newConvert(this.authorPage.awItem.layoutContent, $layout);
                        this.authorPage.selectItem(this.authorPage.awItem, true);
                        this.authorPage.notifyUpdate();
                        break;
                    }
                    if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                        this.authorPage.targetPage.applyDesignMetaData({
                            $layout: {
                                $layoutType: "pattern",
                    ...
                    this.authorPage.awItem.applyDesignMetaData(metaData);
                }
                break;
        }
    },
    ...
    showSettingPanel: function($authoringLevel){
        var selectedPanel;
        var awItem = this.authorPage.awItem;
        for (var ii = 0, jj = this.layoutContent.items.length; ii < jj; ii++) {
            if (this.layoutContent.items[ii].$item.$paletteId == $authoringLevel) {
                selectedPanel = this.layoutContent.items[ii];
            }
        }
        this.applyChange({
            $isHidden: false,
        ...
        if (this.selectedPanel && this.selectedPanel != selectedPanel) {
        ...
        if (awItem.$authoringLevel == "field") {
            if (this._fieldWidget = fieldLibrary.getAuthoringFieldWidget(awItem)) {
                this._fieldWidget.$$container = block.$$body;
                block.setState({
                    $isHidden: false
                });
        ...
        if (fieldArticle.designedField) {
            ...
            switch (field.$item.$bind) {
                case "$layoutTabChoice":
                    this.authorPage.layoutAuthoring.newConvert(awItem.layoutContent, {
                        $layoutType: (value == "tabs") ? "tabs" : "stack"
                    });
                    this.authorPage.selectItem(awItem, true);
                    this.authorPage.notifyUpdate();
                    break;
                case "$viewType":
                    switch (value) {
                        case "content":
                            this.authorPage.treeContent.toggle(true);
                            break;
                        case "structure":
                            this.authorPage.treeContent.toggle(false);
                            if (!this.authorPage.$isLightMode) {
                                this.authorPage.$isLightMode = true;
                                this.authorPage.toggleItemAuthoring(this.authorPage.targetPage, true);
                            }
                            break;
                        case "layout":
                            this.authorPage.treeContent.toggle(false);
                            if (value == "layout" && this.authorPage.$isLightMode) {
                ...
                default:
                    ...
                    if (awItem.$authoringLevel == "field") {
                        ...
                        if (field.$item.$bind.indexOf("$fields") == 0) {
                            ...
                            awItem.applyDesignMetaData(metaData);
                        }
                        else {
                            awItem.applyDesignMetaData(metaData);
                        }
                    }
                    if (this.authorPage.treeContent && field.$item.$bind == "$isHidden") {
    ...
    dispose: function(){
        ...
        this.$item = this.authorPage = this.localize = this.externalAdapter = this.$prototype = this._bar = null;
