...
exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
    ...
    _onChangeSep: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            ...
            case "$undoAllLayout":
                ...
                break;
            case "$defaultLayout":
                $item = this.authorPage.$defaultItem;
                this.authorPage.notifyUpdate(true);
                break;
            default:
                if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                    if (this.authorPage.awItem.$authoringLevel == "section" || this.authorPage.awItem.$authoringLevel == "block") {
                        var pattern = patternFactory.patterns[menuItem.$item.$bind];
                        if (pattern.isBoxOnly) {
                            var children = this.authorPage.getItemsFromLayout(this.authorPage.awItem.layoutContent);
                            if (children.length && children[0].$authoringLevel == "field") {
                                return;
                            }
                        }
                        var $result = patternFactory.build(menuItem.$item.$bind, this.authorPage.targetPage.$prototype, this.authorPage.awItem.$item);
                        if (!$result && !$result.$layout) {
                            return;
                        }
                        this.authorPage.awItem.$item.$layout = $result.$layout;
                        $item = this.authorPage.targetPage.$item;
                    }
                    else {
                        $item = patternFactory.build(menuItem.$item.$bind, this.authorPage.targetPage.$prototype, this.authorPage.$sourceItem);
                    }
                    this.authorPage.notifyUpdate(true);
                }
                break;
    ...
    onMenuClick: function(menuItem){
        switch (menuItem.$item.$bind) {
            case "$addSection":
            case "$addBlock":
                var newChild = this.authorPage.addNewItem(this.authorPage.awItem, menuItem.$item.$bind.replace("$add", "").toLowerCase());
                this.authorPage.ensureLayoutTree(newChild);
                this.authorPage.autoSizeRow(newChild.layoutParent);
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    var newChild = this.authorPage.addNewItem(this.authorPage.awItem, "field", menuItem.$item.$fieldBind);
                    ...
                    if (menuItem.$item.$bind.indexOf("s-layout-type-") >= 0) {
                        ...
                        var $layout = (layoutType == "100") ? {
                            ...
                            $layoutType: "row",
                            $widths: layoutType.replace(/-/g, ",")
                        };
                        this.authorPage.newConvert(this.authorPage.awItem.isLayout ? this.authorPage.awItem : this.authorPage.awItem.layoutContent, $layout);
                        this.authorPage.autoSizeRow(this.authorPage.awItem.layoutParent);
                        this.authorPage.selectItem(this.authorPage.awItem, true, true);
                        this.authorPage.notifyUpdate();
                        break;
                    }
                    if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                    ...
                    this.authorPage.awItem.applyDesignMetaData(metaData, true);
                }
                break;
        }
    },
        ...
        if (fieldArticle.designedField) {
            ...
            switch (field.$item.$bind) {
                case "$layoutTabChoice":
                    ...
                    this.authorPage.newConvert(awItem.isLayout ? awItem : awItem.layoutContent, {
                        $layoutType: (value == "tabs") ? "tabs" : "stack"
                    });
                    this.authorPage.autoSizeRow(awItem.layoutParent);
