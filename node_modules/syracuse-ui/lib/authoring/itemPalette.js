...
exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    ...
                    if (menuItem.$item.$bind.indexOf("s-layout-type-") >= 0) {
                        ...
                        if (menuItem.$item.$bind == "s-layout-type-more") {
                            var list = (layout.$layout.$widths || "100").split(",");
                            var length = ++list.length;
                            menuItem.setDescription("" + length);
                            var width = Math.floor(100 / length);
                            $widths = "";
                            for (var ii = 0; ii < length; ii++) {
                                $widths += width + ",";
                            }
                            $widths = $widths.substr(0, $widths.length - 1);
                        }
    ...
    showSettingPanel: function(){
        ...
        if (selectedPanel) {
            ...
            metadata.$links["s-layout-type-more"] = {
                $style: count > 5 ? "selected" : null,
                $description: count > 5 ? count : "+"
            };
            metadata.$properties = {
                $layoutTabChoice: {
                    $isDisabled: (count != 1)
