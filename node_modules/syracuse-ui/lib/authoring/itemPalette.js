...
exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
    ...
    loadBox: function(){
        ...
        RawPage.prototype.loadBox.call(this, {
            $viewType: "layout",
            $isVisibilityForced: this.authorPage.preferences.authoring.$isVisibilityForced,
            $links: {
    ...
    onMenuClick: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            ...
            case "$addRowAfter":
                ...
                this.authorPage.endTargetPageUpdate(newChild);
                break;
            case "$addSection":
            case "$addBlock":
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    ...
                    if (menuItem.$item.$bind.indexOf("s-layout-type-") >= 0) {
                        ...
                        this.authorPage.endTargetPageUpdate(layout.$layout.$layoutType == "stack" ? awItem : layout);
                        break;
                    }
                    if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                        this._onChangeSep(menuItem);
    ...
    showSettingPanel: function(){
        ...
        if (selectedPanel) {
            ...
            for (var ii = 0, jj = sections.length; ii < jj; ii++) {
                if (ii == 0) {
                    sections[ii].layoutSlot.style.display = awLayout == null ? "none" : "";
                }
                else {
                    if (hideStructure && ii == 1) {
                        sections[ii].layoutSlot.style.display = "none";
                    }
                    else {
                        sections[ii].layoutSlot.style.display = awLayout == null ? "" : "none";
                    }
    ...
    _toggleViewType: function($viewType){
        ...
        if (this.authorPage.isOnPreviewMode && $viewType != "preview") {
            this.authorPage.isOnPreviewMode = false;
            this.authorPage.toggleItemAuthoring(this.authorPage.targetPage, true);
        }
        switch ($viewType) {
            case "preview":
                if (!this.authorPage.isOnPreviewMode) {
                    if (this.authorPage.$isLightMode) {
                        this.authorPage.$isLightMode = false
                        this.authorPage.toggleItemAuthoring(targetPage, true);
                    }
                    this.authorPage.isOnPreviewMode = true;
                    this.authorPage.toggleItemAuthoring(this.authorPage.targetPage, false);
                }
                break;
            case "menus":
    ...
    notifyDataChange: function(field, value){
        ...
        if (fieldArticle.designedField) {
            ...
            switch (field.$item.$bind) {
                case "$isVisibilityForced":
                    this.authorPage.forceVisibility(value);
                    break;
                case "$layoutTabChoice":
                    ...
                    this.authorPage.endTargetPageUpdate(awItem);
                    break;
                case "$viewType":
                ...
                default:
                    ...
                    this.authorPage.notifyUpdate();
                    this.applyChange(metaData);
                    if (field.$item.$bind == "$isExcluded") {
                        this.authorPage.toggleItemAuthoring(awItem, true);
                    }
                    break;
            }
        }
