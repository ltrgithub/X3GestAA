...
exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
                ...
                if (menuItem.$item.$bind.indexOf("pattern") == 0) {
                    if (this.authorPage.awItem.$authoringLevel == "section" || this.authorPage.awItem.$authoringLevel == "block") {
                        ...
                        if (pattern.isBoxOnly) {
                            var children = this.authorPage.extractItemsFromLayout(this.authorPage.awItem.layoutContent);
    ...
    onMenuClick: function(menuItem){
        switch (menuItem.$item.$bind) {
            case "$addSection":
            case "$addBlock":
                this.authorPage.addNewItem(this.authorPage.awItem, menuItem.$item.$bind.replace("$add", "").toLowerCase());
                this.authorPage.autoSizeRow(this.authorPage.awItem);
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    this.authorPage.addNewItem(this.authorPage.awItem, "field", menuItem.$item.$fieldBind);
                    this.authorPage.notifyUpdate();
                }
                else {
        ...
        if (fieldArticle.designedField) {
            ...
            switch (field.$item.$bind) {
                case "$layoutTabChoice":
                    var children = this.authorPage.extractItemsFromLayout(awItem.isLayout ? awItem : awItem.layoutContent);
                    ...
                    this.authorPage.autoSizeRow(awItem);
