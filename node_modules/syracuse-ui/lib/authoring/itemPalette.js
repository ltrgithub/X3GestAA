...
exports.ItemPalette = helpers.defineClass(ItemPalette, RawPage, {
    _fillAddFieldsMenus: function(){
        ...
        for (var ii = 0, jj = binds.length, kk = 0; ii < jj; ii++) {
            ...
            if ($field && $field.$type) {
                ...
                var title = $field.$title ? this.authorPage.targetPage.getLocalizeText($field.$title) : "";
                $actions[actionId] = {
        ...
        var fields = this.authorPage._extractFields(this.authorPage.targetPage.layoutContent);
    ...
    onMenuClick: function(menuItem){
        ...
        switch (menuItem.$item.$bind) {
            case "$excludeMe":
                this.authorPage.endTargetPageUpdate(this.authorPage.excludeItem(awItem));
                break;
            case "$addRowBefore":
            case "$addRowAfter":
            ...
            default:
                if (menuItem.$item.$fieldBind) {
                    ...
                    this.onExcludeField(menuItem.$item.$fieldBind, true);
    ...
    onWindowResize: function(){
        if (this.layoutSlot) {
            if (this.authorPage.targetPage.dialogWrapper) {
                this.layoutSlot.style.height = ($(this.authorPage.paletteSlot).height() - 10) + "px";
            }
            else {
    ...
    dispose: function(){
        ...
        this.diagnosePage = this.$item = this.authorPage = this.localize = this.externalAdapter = this.$prototype = this.layoutSlot = null;
