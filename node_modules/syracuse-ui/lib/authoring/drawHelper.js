			...
			case "field":
				...
				switch ($newAuthoringLevel) {
					...
					case "block":
						if (dropItem) {
							//dragged section							
							var children = _extractFields(dropItem.layoutContent);
							targetItem.layoutParent.loadChildItems(children, targetItemIndex);
							targetItem.layoutParent.removeItem(dropItem, true);
							newChild = targetItem;
						} else {
...
exports.excludeItem = function(awPalette, removedItem) {
	...
	if (removedItem.$authoringLevel == "field") {
		...
		excludedFields = _extractFields(removedItem.layoutContent);
	}
	if (excludedFields) {
...
function _extractFields(layout, fields, onlyDirectChild) {
	fields = fields || [];
	if (layout) {
		for (var ii = layout.items.length - 1; ii >= 0; ii--) {
			...
			if (item != layout.boxParent) {
				if (item.$authoringLevel === "field") {
					...
					fields = _extractFields(item.isLayout ? item : item.layoutContent, fields);
				}
			}
		}
	}
