			...
			switch (targetItem.layoutParent.$layout.$layoutType) {
				case "stack":
					switch ($insertAt) {
                     ...
                     return _addNewItem(awPalette,awArticle,targetItem.layoutParent, $newAuthoringLevel, $bind, dropItem, $insertAt);
                     }*/
		...
		switch (targetItem.$chilAuthoringLevel || targetItem.$authoringLevel) {
			case "article":
				if ($newAuthoringLevel == "field") {
					newChild = _addNewItem(awPalette, awArticle, targetItem, "section");
					newChild = _addNewItem(awPalette, awArticle, newChild, $newAuthoringLevel, $bind, dropItem, undefined, $newCategory);
				} else {
					if (!dropItem) {
						...
						if (dropItem) {
							...
							if (children.length == 0) {
								...
								if (children[0].$authoringLevel == "field") {
									...
									newChild = _addNewItem(awPalette, awArticle, targetItem, children[0].$authoringLevel);
			...
			case "field":
				...
				switch ($newAuthoringLevel) {
					...
					case "block":
						if (dropItem) {
							//dragged section							
							var children = awPalette.extractFields(dropItem.layoutContent);
							targetItem.layoutParent.loadChildItems(children, targetItemIndex);
							targetItem.layoutParent.removeItem(dropItem, true);
							newChild = targetItem;
						} else {
