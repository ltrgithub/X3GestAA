...
function _insertFieldsWrapper(awArticle, targetItem, $insertAt, $newCategory) {
	var newChild;
	var layout = targetItem.isSpaceBox ? targetItem.layoutParent : targetItem.layoutContent;
	var items = layout.items.slice(0);
	if (_hasLevelItems(items, "field")) {
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			layout.extractItem(items[ii]);
		}
		var $prevLayoutType = {
		...
		newChild = _addNewSection(awArticle, layout, null, 0);
		...
		newChild = _addNewSection(awArticle, layout, null, 0, $newCategory);
	}
	return newChild;
}
		...
		switch (targetItem.$chilAuthoringLevel || targetItem.$authoringLevel) {
			case "article":
				if ($newAuthoringLevel == "field") {
					...
					newChild = _addNewItem(awArticle, newChild, $newAuthoringLevel, $bind, dropItem, undefined, $newCategory);
				} else {
					if (!dropItem) {
				...
				switch ($newAuthoringLevel) {
					case "field":
						if (dropItem) {
							...
							var children = targetItem.layoutContent.getItems();
					...
					case "block":
						if (dropItem) {
							...
							if ($newCategory == "menus" && $insertAt) {
								newChild = _addNewSection(awArticle, targetItem.layoutParent, null, targetItemIndex);
								newChild = _addNewSection(awArticle, newChild.layoutContent, null, 0, $newCategory);
							} else {
...
exports.setRowAlign = function(awPalette, valign) {
	if (awPalette.awLayout) {
		awPalette.awLayout.$layout.$rowAlign = valign;
		for (var ii = 0, jj = awPalette.awLayout.items.length; ii < jj; ii++) {
			awPalette.awLayout.items[ii].layoutSlot.style.verticalAlign = valign;
		}
		awPalette.authorPage.endTargetPageUpdate(awPalette.awLayout, true);
	}
};
exports.switchTabPanelBox = function(awPalette, $layoutType) {
	if (awPalette.awItem.layoutParent.items.length == 1) {
		_newConvert(awPalette.awItem.layoutParent, {
			$layoutType: $layoutType
		});
	} else {
		var layoutRoot = awPalette.awItem.layoutParent;
		if ($layoutType == "stack") {
			...
			var layoutTab;
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				if (children[ii] == awPalette.awItem) {
			...
			var layoutTab = layoutRoot.wrapIntack([awPalette.awItem], layoutRoot.getItemIndex(awPalette.awItem.$item));
			_newConvert(layoutTab, {
				$layoutType: "tabs"
			});
		}
	}
	awPalette.authorPage.endTargetPageUpdate(awPalette.awItem, true);
};
		...
		if (dropItem.$awAddNewItem) {
			if (dropItem.$fieldBind) {
				...
				if (dropItem.$vignette) {
					...
					newChild = _addNewItem(awPalette.awArticle, targetItem, dropItem.$authoringLevel, null, newChild, $insertAt, dropItem.$category);
				}
			}
		} else {
			newChild = _addNewItem(awPalette.awArticle, targetItem, dropItem.$authoringLevel, dropItem.$bind, dropItem, $insertAt);
		}
	}
};

exports.excludeItem = function(awPalette, removedItem) {
	var layoutParent = removedItem.layoutParent;
	var excludedFields;
	if (removedItem.$item && removedItem.$item.$vignette) {
		awPalette.onExcludeField(removedItem.$item.$vignette, true, true);
	} else {
		if (awPalette.targetPage.isDashBoard && removedItem.layoutContent) {
			var children = removedItem.layoutContent.getItems();
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				if (children[ii].$item && children[ii].$item.$vignette) {
					awPalette.onExcludeField(children[ii].$item.$vignette, true, true);
				}
			}
		}
	}
	if (removedItem.$authoringLevel == "field") {
		...
		excludedFields = exports.extractFields(removedItem.layoutContent);
	}
	if (excludedFields) {
		for (var ii = 0, jj = excludedFields.length; ii < jj; ii++) {
			awPalette.onExcludeField(excludedFields[ii].$item.$bind, true);
		}
	}
	...
	awPalette.onEndExcludeItem();
	return newItem;
};

exports.onAddNewVignette = function(awPalette, targetItem, $newItem, $insertAt) {
	var newChild = _addNewItem(awPalette.awArticle, targetItem, "block", null, undefined, $insertAt, "menus");
...
exports.onAddNewField = function(awPalette, targetItem, $newItem, $insertAt) {
	...
	newChild = _addNewItem(awPalette.awArticle, targetItem, "field", $newItem.$fieldBind, newChild, $insertAt, $newItem.$fieldCategory);
