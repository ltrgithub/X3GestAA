...
function _addNewSection(article, layoutParent, dropItem, index, $newCategory){
...
function _insertFieldsWrapper(article, targetItem, $insertAt, $newCategory){
    var newChild;
    ...
    if (_hasLevelItems(items, "field")) {
        ...
        newChild = _addNewSection(article, targetItem.layoutContent, null, 0);
        ...
        newChild = _addNewSection(article, targetItem.layoutContent, null, 0, $newCategory);
    }
    return newChild;
}
...
function _addNewItem(article, targetItem, $newAuthoringLevel, $bind, dropItem, $insertAt, $newCategory){
    var newChild;
            ...
            switch (targetItem.layoutParent.$layout.$layoutType) {
                case "stack":
                    switch ($insertAt) {
                     ...
                     return _addNewItem(article,targetItem.layoutParent, $newAuthoringLevel, $bind, dropItem, $insertAt);
                     }*/
        ...
        switch (targetItem.$chilAuthoringLevel || targetItem.$authoringLevel) {
            case "article":
                if ($newAuthoringLevel == "field") {
                    newChild = _addNewItem(article, targetItem, "section");
                    newChild = _addNewItem(article, newChild, $newAuthoringLevel, $bind, dropItem);
                }
                else {
                    if (!dropItem) {
                        newChild = _insertFieldsWrapper(article, targetItem);
                        if ($newAuthoringLevel == "block") {
                            newChild = _insertFieldsWrapper(article, newChild, undefined, $newCategory);
                        }
                    }
                    else {
                        newChild = _addNewSection(article, targetItem.layoutContent, dropItem, 0);
                    }
                }
                break;
            case "section":
                switch ($newAuthoringLevel) {
                    case "field":
                        if (dropItem) {
                            newChild = _addNewSection(article, targetItem.layoutParent, null, targetItemIndex);
                            ...
                            if (children.length == 0) {
                                ...
                                if (children[0].$authoringLevel == "field") {
                                    ...
                                    newChild = _addNewItem(article, targetItem, children[0].$authoringLevel);
                        ...
                        break;
                    case "section":
                        newChild = _addNewSection(article, targetItem.layoutParent, dropItem, targetItemIndex);
                        break;
                    case "block":
                        if (dropItem) {
                            newChild = targetItem.layoutParent.loadChildItem(dropItem, null, targetItemIndex);
                            ...
                            newChild = _insertFieldsWrapper(article, targetItem, undefined, $newCategory);
                        }
                        break;
                }
                break;
                ...
                switch ($newAuthoringLevel) {
                    ...
                    case "section":
                        if (dropItem) {
                            ...
                            newChild = _addNewItem(article, targetItem.boxParent, $newAuthoringLevel);
                        }
                        break;
                    case "block":
                        newChild = _addNewSection(article, targetItem.layoutParent, dropItem, targetItemIndex, $newCategory);
                        break;
                }
                break
            case "field":
                ...
                switch ($newAuthoringLevel) {
                    ...
                    case "block":
                        if (dropItem) {
                            ...
                            newTopChild = newChild = _addNewSection(article, layoutParent, null, targetItemIndex);
                            if (newTopChild.$authoringLevel == "section" && $newAuthoringLevel == "block") {
                                newChild = _addNewSection(article, newTopChild.layoutContent);
                            }
...
exports.switchTabPanel = function(awContext, $layoutType){
    ...
    if (!newChild) {
        if (dropItem.$awAddNewItem) {
            if (dropItem.$fieldBind) {
                ...
                if ($insertAt) {
                    newChild = _addNewSection(awContext.targetPage, awContext.targetPage.layoutContent, null, 0, dropItem.$category);
                    newChild.layoutParent.extractItem(newChild);
                }
                newChild = _addNewItem(awContext.targetPage, targetItem, dropItem.$authoringLevel, null, newChild, $insertAt, dropItem.$category);
            }
        }
        else {
            newChild = _addNewItem(awContext.targetPage, targetItem, dropItem.$authoringLevel, dropItem.$bind, dropItem, $insertAt);
        }
    }
};
...
exports.onAddNewField = function(awContext, targetItem, $newItem, $insertAt){
    ...
    var newChild;
    if ($insertAt) {
        newChild = awContext.targetPage.layoutContent.loadChildItem(null, {
            $bind: $newItem.$fieldBind,
            $category: $newItem.$fieldCategory
        });
        newChild.layoutParent.extractItem(newChild);
    }
    newChild = _addNewItem(awContext.targetPage, targetItem, "field", $newItem.$fieldBind, newChild, $insertAt, $newItem.$fieldCategory);
