...
exports.switchTabPanelChildren = function(awContext, $layoutType) {
	var children = (awContext.awItem.isLayout ? awContext.awItem : awContext.awItem.layoutContent).getItems();
	if (!(children.length > 0 && children[0].$authoringLevel == "field")) {
		...
		var children = awContext.awItem.layoutContent.getItems();
		for (var ii = 0, jj = children.length; ii < jj; ii++) {
			if (!children[ii].isSpaceBox) {
...
exports.excludeItem = function(awContext, removedItem) {
	...
	if (removedItem.$item && removedItem.$item.$vignette) {
		awContext.toolsBar.onExcludeField(removedItem.$item.$vignette, true, true);
	} else {
		if (awContext.targetPage.isDashBoard && removedItem.layoutContent) {
			var children = removedItem.layoutContent.getItems();
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				if (children[ii].$item && children[ii].$item.$vignette) {
					awContext.toolsBar.onExcludeField(children[ii].$item.$vignette, true, true);
				}
			}
		}
	}
	...
	if (excludedFields) {
		for (var ii = 0, jj = excludedFields.length; ii < jj; ii++) {
			awContext.toolsBar.onExcludeField(excludedFields[ii].$item.$bind, true);
		}
	}
	...
	if (awContext.toolsBar) {
		awContext.toolsBar.onEndExcludeItem();
	}
	return newItem;
}
...
exports.onAddNewVignette = function(awContext, targetItem, $newItem, $insertAt) {
	...
	if (awContext.toolsBar) {
		awContext.toolsBar.onExcludeField($newItem.$vignette, false, true);
	}
...
exports.onAddNewField = function(awContext, targetItem, $newItem, $insertAt) {
	...
	if (awContext.toolsBar) {
		awContext.toolsBar.onExcludeField($newItem.$fieldBind, false);
	}
	return newChild;
};
