...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	load: function(designer, title) {
		this.tabItem = syra_menus.button.add({
			parent: designer,
			...
			select: function(select) {
				...
				syra_site.dom.hide(this.body, !select);
			}
		});
		syra_site.dom.hide(this.tabItem.body, true);
	...
	onItemInOut: function(onEnter, event, target) {
		syra_dd.cancel();
		event.stopPropagation();
		var node = syra_store.findItem(target);
		if (node) {
			if (onEnter) {
	...
	_addFieldType: function(itemNode, $field) {
		itemNode.titleLabel.parentNode.insertBefore(syra_menus.button.add({
			isIndicator: true,
			parent: itemNode,
			iconOnly: true,
