...
function _onNodeOpenerClick(event) {
	...
	if (node) {
		...
		syra_menus.button.setText(node.openerBtn, this.opened ? syra_local.aw_tree_collapse : syra_local.aw_tree_expand, this.opened ? "node_expanded" : "node_collapsed");
	}
}
...
function _onNodeClick(event) {
	...
	if (this.itemNode && !this.itemNode.item.$awAddNewItem) {
		if (this.itemNode.item == tree.designer.designedArticle) {
			syra_page.scrollToItem(tree.designer.designedArticle.page, this.itemNode.item);
		}
...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	...
	_createNodeItem: function(id, $designLevel) {
		var itemNode = {};
		itemNode.node = syra_menus.button.add({
			parent: this,
	...
	_addFieldType: function(itemNode, $field) {
		itemNode.titleLabel.parentNode.insertBefore(syra_menus.button.addIndicator({
			parent: itemNode,
			iconOnly: true,
