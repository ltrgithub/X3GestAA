...
function _onNodeOpenerClick(event) {
	...
	if (node) {
		...
		syra_menus.updateButtonIcon(node.openerBtn.link, this.opened ? syra_local.aw_tree_collapse : syra_local.aw_tree_expand, this.opened ? "node_expanded" : "node_collapsed");
	}
}
	...
	_setParentNode: function(parentNode) {
		if (!parentNode.childRoot) {
			...
			if (!parentNode.openerBtn) {
				if (parentNode.title) {
					parentNode.openerBtn = syra_menus.button.add({
						parent: parentNode,
		...
		if (this.nodes) {
			for (var ii = 0, jj = this.nodes.length; ii < jj; ii++) {
				...
				if (node) {
					...
					syra_menus.button.dispose(node.openerBtn);
