...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	load: function(designer, tabItem) {
	...
	openParentNode: function(node, open) {
		...
		syra_menus.updateButtonIcon(node.opener, open ? syra_local.aw_tree_collapse : syra_local.aw_tree_expand, open ? "node_expanded" : "node_collapsed");
		node.opener.syraOpened = open;
	},
	onNodeOpenerClick: function(event) {
		...
		if (node) {
			this.openParentNode(node, !node.opener.syraOpened);
		}
	},
	onNodeClick: function(event) {
		...
		if (node && !node.item.$awAddNewItem) {
			if (node.item == this.designer.designedArticle) {
				syra_page.scrollToItem(this.designer.designedArticle.page, node.item);
			}
			this.selectNode(node.item, true);
			this._isSelecteDisabled = true;
			this.designer.selectItem(node.item, true);
		...
		if (!this._isSelecteDisabled && item && !item.disposed) {
			...
			if (node) {
				if (select) {
					...
					syra_page.scrollToItem(this.designer, this.selectedNode.node, this.tabItem.body, true);
				} else {
	...
	_setParentNode: function(parentNode) {
		if (!parentNode.childRoot) {
			...
			if (!parentNode.opener) {
				if (parentNode.title) {
					parentNode.opener = syra_menus.addIconButton(syra_local.aw_tree_collapse, "s-aw-tree-item-opener", "onNodeOpenerClick", null, "node_expanded");
					parentNode.opener.syraOpened = true;
					parentNode.title.insertBefore(parentNode.opener, parentNode.title.firstChild);
				}
			}
	...
	_addFieldType: function(itemNode, $field) {
		itemNode.type = syra_menus.addFontIconFlag("s-aw-tree-field-type", syra_fields.getTypeFieldIcon($field.$type));
	...
	dispose: function() {
		...
		this._clear();
		this.tabItem = this.nodes = this.selectedNode = this.designer = null;
	}
});