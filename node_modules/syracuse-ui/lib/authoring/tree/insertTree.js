...
exports.InsertTree = helpers.defineClass(InsertTree, ItemTree, {
	...
	_onAddAvailableClick: function() {
		...
		if (newItems) {
			if (newItems[0].isField) {
				...
				if (targetItem.isField) {
					targetItem.layoutParent.loadChildItems(newItems[0].getItems("field"), targetItemIndex);
				} else {
	...
	_addInsertNewSectionBlockNode: function($designLevel) {
		...
		itemNode.titleText.textContent = itemNode.node.title = this.treebar.designer.designerLocalize.aw_addSectionBlock;
		this.nodes.push(itemNode);
		itemNode.item = {
			$title: this.treebar.designer.designerLocalize.aw_addSectionBlock,
			$awAddNewItem: true,
	...
	_addNode: function($item, parentNode) {
		...
		if ($item.$category != "space") {
			if ($item.$items && !$item.$bind && !$item.$category) {
				...
				if (title == "-") {
					title = this.treebar.designer.designerLocalize["aw_" + itemNode.$designLevel];
				}
