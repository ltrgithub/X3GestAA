...
exports.InsertTree = helpers.defineClass(InsertTree, ItemTree, {
	...
	_addInsertNewSectionBlockNode: function($authoringLevel) {
		var itemNode = this._addNodeItem("node-" + this.nodes.length, "section", this.nodes[0]);
		...
		this.nodes[0].children.insertBefore(itemNode.node, this.nodes[0].children.firstChild);
	},
	_addNode: function($item, parentNode) {
		...
		if ($item.$category != "space") {
			if ($item.$items && !$item.$bind && !$item.$category) {
				...
				var itemNode = this._addNodeItem("node-" + this.nodes.length, $authoringLevel, parentNode);
				itemNode.$item = $item;
				this.nodes.push(itemNode);
