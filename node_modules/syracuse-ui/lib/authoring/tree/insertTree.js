...
exports.InsertTree = helpers.defineClass(InsertTree, ItemTree, {
	...
	_onAddAvailableClick: function() {
		var targetItem = this.toolbar.awPalette.awItem;
		...
		document.site.authorPage.endTargetPageUpdate(targetItem, true);
	},
	_showNodes: function() {
		...
		if ($item.$category != "space") {
			if ($item.$items && !$item.$bind && !$item.$category) {
				...
				if ($item.$bind) {
					...
					$authoringLevel = parentNode.$authoringLevel == "section" ? "block" : "section";
				}
				var itemNode = this._createNodeItem(item ? item.id : "node-" + this.nodes.length, $authoringLevel);
