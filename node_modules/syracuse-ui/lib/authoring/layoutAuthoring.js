...
var emptyBox = require('syracuse-ui/lib/article/layouts/emptyBox');

function _extractItemSetting($items, $layout){
    for (var ii = 0, jj = $layout.$items.length; ii < jj; ii++) {
        var $item = $layout.$items[ii];
        if ($item.$layoutType) {
            _extractItemSetting($items, $item);
        }
        else {
            $items.push($item);
        }
...
exports.LayoutAuthoring = helpers.defineClass(LayoutAuthoring, null, {
    ...
    _buildAutoBlock: function($prototype, $parent, $blocks){
        ...
        if ($blocks.length > 1) {
            ...
            for (var ii = 0, jj = $blocks.length; ii < jj; ii++) {
                var $block = $blocks[ii];
                var $children = [];
                _extractItemSetting($children, $block.$layout);
                
                var $fields = [];
                _extractItemSetting($fields, $block.$layout);
                if ($fields.length > 0) {
                    var addFields = true;
                    if ($fields.length == 1) {
                        var $field = $fields[0];
            ...
            if ($blocks.length == 1) {
                var $fields = [];
                _extractItemSetting($fields, $blocks[0].$layout);
                $blocks[0].$layout.$items = $fields;
            }
    },
    buildAutoLayout: function($prototype, $sourceArticle){
        var $newArticle = {
            $layout: {
                $items: []
            }
        ...
        var $rootItems = [];
        _extractItemSetting($rootItems, $sourceArticle.$layout);
        ...
        for (var ii = 0, jj = $rootItems.length; ii < jj; ii++) {
            ...
            var $children = [];
            _extractItemSetting($children, $parent.$layout);
            if ($children.length > 0) {
                if ($children[0].$category == "block") {
                    this._buildAutoBlock($prototype, $parent, $children);
                }
                else {
                ...
                if (dragdrop.sourceItem) {
                    ...
                    switch (event.type) {
                        case "mouseup":
                            ...
                            break;
                        case "mousemove":
                            dragdrop.moveDragImage(event, layout.box);
                            break;
                        case "mouseleave":
                            ...
                            break;
                        case "mouseenter":
                            if (dragdrop.getDropAction(event, layout.box)) {
    ...
    deleteLayout: function(layout, emptyBox){
        debugger;
        if (layout._isColumnMode(layout.$layout) && layout.items.length > 1) {
            var childIndex = _findChildIndex(emptyBox);
            layout.box.getArticle().removeItem(emptyBox, true);
            if (childIndex >= 0) {
                document.site.removeDomChild(layout._childSlots[childIndex]);
                layout.$layout.$items.splice(childIndex, 1);
                layout.items.splice(childIndex, 1);
                layout._childSlots.splice(childIndex, 1);
            }
            layout.colWidths = (layout.$layout.$layoutSubType = layout.items.length == 2 ? "50-50" : "33-33-33").split("-");
            for (var ii = 0, jj = layout.items.length; ii < jj; ii++) {
                layout._childSlots[ii].style.width = layout.colWidths[ii] + "%";
            }
        }
        else {
            if (layout.layoutParent) {
                ...
                var layoutParent = layout.box.layoutParent
                var boxIndex = layoutParent.getItemIndex(layout.box);
                layoutParent.$layout.$items.splice(boxIndex, 1);
                layoutParent.items.splice(boxIndex, 1);
                layout.box.getArticle().removeItem(layout.box, true);
            }
        }
    },
    newConvert: function(layout, $newLayout){
        ...
        var oldChildSlots = layout._childSlots;
        ...
        layout._ensureRoot();
        // if (layout._childSlots.length > 0) {
        if (layout._isColumnMode(layout.$layout)) {
            if (oldChildSlots.length > 0) {
                if (oldColWidths.length > layout.colWidths.length) {
                    //A copier dans la dernierecolonne
                    var lastSlot = layout._childSlots[layout.colWidths.length - 1];
                    var lastItem = layout.items[layout.colWidths.length - 1];
                    for (var ii = layout._childSlots.length, jj = oldColWidths.length; ii < jj; ii++) {
                        layout._childSlots[ii].style.width = layout.colWidths[ii] + "%";
                    }
                }
            }
                ...
                if (layout.colWidths.length > layout.items.length) {
                    ...
                    debugger;
                    var stackCol = layout.box.page.createNewLayout({
                        $layoutType: "stack",
                        $items: []
                    }, layout.box);
            ...
            if (layout.colWidths.length > 0) {
                for (var ii = layout.items.length, jj = layout.colWidths.length; ii < jj; ii++) {
                    // var newEmptyBox = layout.box.page.createNewItem({}, layout.box, new emptyBox.EmptyBox());
    ...
    newMoveItem: function(layout, targetItem, newItem, insertAfter){
        if (!targetItem || (targetItem.$authoringLevel == newItem.$authoringLevel) || (targetItem.isSection == newItem.isSection)) {
            ...
            if (sourceLayout) {
                ...
                var oldIndex = sourceLayout.getItemIndex(newItem);
            ...
            if (insertAfter) {
                ...
                if (!targetItem && !layout._isColumnMode(layout.$layout)) {
                    newItemIndex = 0;
                }
            }
