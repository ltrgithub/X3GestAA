...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	load: function(treebar, tabItem) {
		this.treebar = treebar;
				...
				switch (event.type) {
					case "mouseenter":
						if (!node.item.$awAddNewItem) {
							this.treebar.designer.uiSwitchTool.toggleOverItem(node.item, true);
							this.treebar.designer.awArticle.page.scrollToItem(node.item);
						}
						break;
					case "mouseleave":
						if (!node.item.$awAddNewItem) {
							this.treebar.designer.uiSwitchTool.toggleOverItem(node.item, false);
						}
						break;
					case "click":
						if (!node.item.$awAddNewItem) {
							if (node.item == this.treebar.designer.awArticle) {
								this.treebar.designer.awArticle.page.scrollToItem(node.item);
							}
							this.selectNode(node.item, true);
							this._isSelecteDisabled = true;
							this.treebar.designer.selectItem(node.item, true);
	...
	_bindEvents: function() {
		...
		if (self.tabItem && self.tabItem.body) {
			$(self.tabItem.body).delegate(".s-aw-content-tree", "mousedow.authoringTree click.authoringTree", function(event) {
				...
				return false;
			}).delegate(".s-aw-tree-item-opener", "click.authoringTree", function(event) {
				self.onNodeEvent(event);
				return false;
			...
			if (node) {
				if (select) {
					if (this.selectedNode && this.selectedNode != node && this.selectedNode.item) {
						document.site.toggleClass(this.selectedNode.title, "s-aw-tree-designed-" + this.selectedNode.item.$designLevel, false);
					}
					this.selectedNode = node;
					document.site.toggleClass(node.title, "s-aw-tree-designed-" + node.item.$designLevel, true);
					this.treebar.designer.scrollToItem(this.selectedNode.node, $(this.tabItem.body), true);
				} else {
					if (this.selectedNode == node) {
						this.selectedNode = null;
						document.site.toggleClass(node.title, "s-aw-tree-designed-" + node.item.$designLevel, false);
					}
				}
			}
		}
	...
	_createNodeItem: function(id, $designLevel) {
		...
		document.site.ddManager.setDragSpot(itemNode.node, true);
	...
	_setTitleNode: function(itemNode, $item, $field, title) {
		var bindTitle = "",
			desc = title;
		if ($item.$bind) {
			bindTitle = this.treebar.designer.getBindTitle($item.$bind, $field);
			if (title == "") {
	...
	dispose: function() {
		...
		this.tabItem = this.nodes = this.selectedNode = this.treebar = null;
	}
});