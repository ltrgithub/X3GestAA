...
function applyColumnsModel(item, children, maxCols) {
	...
	item.$item.$layout.$width = widths.join(",");
...
function _applyModel(awItem, $model) {
	...
	var oldLayoutItem = awItem.layoutContent._item;
	awItem.layoutContent.dispose();
	switch ($model) {
		case "$model_rows":
			awItem.$item.$layout = {
				$layoutType: "stack",
...
exports.onModelClick = function($bind, awPalette) {
	...
	document.site.authorPage.notifyUpdate(true);
	document.site.authorPage.onEndChangeStep($item);
};

function _addModelLink($model, slot) {
	...
	for (var ii = 0, jj = sections.length; ii < jj; ii++) {
		if (sections[ii].$item.$isSectionModels) {
			...
			if ($level == "article") {
				...
				_addModelLink("smart", slot);
				_addModelLink("factory", slot);
