...
var Treebar = require("syracuse-ui/lib/authoring/desktopPageDesigner/tree/treebar").Treebar;
var designProperties = require("syracuse-ui/lib/authoring/desktopPageDesigner/designProperties");
...
exports.VignettePageDesigner = helpers.defineClass(VignettePageDesigner, RawPage, {
	...
	endTargetPageUpdate: function(selectedItem, isStructureUpdated) {
		...
		if (selectedItem && selectedItem.$designLevel == "field" && selectedItem.boxParent) {
			...
			this.uiSwitchTool.toggleItemAuthoring(this.awArticle.page, true);
		}
	...
	loadBox: function(designedPage) {
		...
		this.$prototype = {
			...
			$links: {
				$excludeMe: {
					$title: this.localize.aw_excludeMe
				}
			}
		};
		this.$item = {};
	...
	onClickPicker: function(picker) {
		...
		if (this.awArticle.designer.historyTool.onClickPicker($bind) ||
			this.awArticle.designer.viewsTool.onClickPicker($bind)) {
			return false;
		}
	...
	drawBox: function() {
		...
		this.viewsSlot.id = "s-aw-page-views";
		...
		(this.rowResizer = new RowResizer()).load(this);
		...
		this.barSplitter.load(this.awArticle.page, this.propertiesBodySlot, this.treebarSlot, this.treebar.treebarBody);

		this.treebar.load(this);
		this.propertiesBodySlot.style.display = "";
		this.treebarSlot.style.display = "";

		this.uiSwitchTool.toggleItemAuthoring(this.awArticle, true);
		this.bindEvents();
		...
		if (this.awArticle.$item.$transaction) {
			...
			this.historySlot.appendChild(label);
		}
		document.site.resize();
	...
	bindEvents: function() {
		...
		document.site.$$layoutSlot.delegate(".s-aw-item", "click.author", function(event) {
			...
			if (!document.site.dragDropInstance) {
				self.rowResizer.start(this, event);
				return false;
			}
			...
			if (!document.site.dragDropInstance) {
				...
				if (item) {
					...
					self.uiSwitchTool.toggleOverItem(item, true);
				}
				return false;
			}
			return true;
	...
	showSettingPanel: function() {
		...
		for (var ii = 0, jj = sections.length; ii < jj; ii++) {
			...
			if (section.$item.$isSectionModels) {
				this.templatesTool.addModelLinks(section, this.awItem, this.selectedPanel.colCount, this.awLayout);
			}
	...
	dispose: function() {
		...
		if (this.awArticle) {
			...
			if (this.uiSwitchTool) {
				this.uiSwitchTool.toggleItemAuthoring(this.awArticle.page, false);
			}
			delete this.awArticle.designer;
		}
		var ids = ["viewsTool", "uiSwitchTool", "historyTool", "localizationTool", "templatesTool", "labelTool", "fieldAddinsTool", "rowResizer"];
		...
		this.treebarSlot = this.propertiesSlot = this.propertiesBodySlot = this.historySlot = this.viewsSlot = this.workinCopySlot = null;
		this.barSplitter = this._ddTabEnter = this.awItem = this.awLayout = this.awArticle = null;
		this.panels = this.treebar = this.selectedPanel = null;
		this.designerWorkingCopy = this.transactionLabel = this.page = this.diagnosePage = this.helpLink = this.designerWorkingCopy = null;
