...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	...
	openParentNode: function(node, open) {
		...
		document.site.dom.toggleClass(node.opener, "s-open", !open);
	},
	onNodeOpenerClick: function(picker, event) {
	...
	onNodeClick: function(picker, event) {
		document.site.ddManager.cancel();
		event.stopPropagation();
		var node = this.findNode(event);
			...
			if (node) {
				if (select) {
					if (this.selectedNode && this.selectedNode != node && this.selectedNode.item) {
						document.site.dom.toggleClass(this.selectedNode.title, "s-aw-tree-designed-" + this.selectedNode.item.$designLevel, false);
					}
					this.selectedNode = node;
					document.site.dom.toggleClass(node.title, "s-aw-tree-designed-" + node.item.$designLevel, true);
					this.trees.designer.scrollToItem(this.selectedNode.node, this.tabItem.body, true);
				} else {
					if (this.selectedNode == node) {
						this.selectedNode = null;
						document.site.dom.toggleClass(node.title, "s-aw-tree-designed-" + node.item.$designLevel, false);
					}
				}
			}
		}
		...
		if (!parentNode.childRoot) {
			...
			if (!parentNode.opener) {
				if (parentNode.title) {
					...
					parentNode.opener.syraOnClick = "onNodeOpenerClick";
	...
	_clear: function() {
		if (this.tabItem) {
			...
			document.site.dom.empty(this.tabItem.body);
		}
		if (this.nodes) {
			for (var ii = 0, jj = this.nodes.length; ii < jj; ii++) {
