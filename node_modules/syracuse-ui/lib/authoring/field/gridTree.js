...
$(self.treebarBody).delegate(".s-aw-list-cols-col,.s-aw-list-cols-col-title,.s-aw-list-cols-col-fixe,.s-aw-list-cols-col-delete", "click.authoringList", function(event) {
...
exports.GridTree = helpers.defineClass(GridTree, null, {
	load: function(designer, designedArticle, slot) {
		var authorPage = syra_site.authorPage;
		this.designer = designer;
		this.list = designedArticle;
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isHidden && !$field.$isExcluded) {
				...
				node.item.setAttribute("data-s-drag", "1");
				...
				if ($item.$isFixed) {
					...
					node.isFixed.title = syra_site.authorPage.localize.aw_listUnFixeColumn;
				} else {
					node.isFixed.title = syra_site.authorPage.localize.aw_listFixeColumn;
				}
				node.$delete.className = "s-aw-list-cols-col-delete";
				node.$delete.title = syra_site.authorPage.localize.aw_listHideColumn;

				node.title.className = "s-aw-list-cols-col-title";
				var title = this.list.getLocalizedTitle($field.$title);
				var bindTitle = "",
					desc = title;
				bindTitle = syra_site.authorPage.getBindTitle($item.$bind, $field);
				if (title == "") {
	...
	unload: function() {
		...
		syra_site.dom.empty(this.slot);
		this.dispose();
	},
	dispose: function() {
		this._nodes = this.slot = this.designer = this.list = null;
	},
	onNodeEvent: function(event) {
		...
		if (node) {
			if (event.currentTarget.className.indexOf("s-aw-list-cols-col-delete") >= 0) {
				this.list.removeColumn(node.$item.$bind);
				syra_site.dom.removeChild(node.item);
				syra_site.authorPage.notifyUpdate();
				this.designer.onEndChangeStep();
			} else {
				if (event.currentTarget.className.indexOf("s-aw-list-cols-col-fixe") >= 0) {
					...
					if (isFixed) {
						for (var ii = 0, jj = nodeIndex; ii <= jj; ii++) {
							...
							this.list.cols[colNode.$item.$bind].$isFixed = colNode.$item.$isFixed = isFixed;
							colNode.isFixed.className = "s-aw-list-cols-col-fixe s-field-fixed";
							colNode.isFixed.title = syra_site.authorPage.localize.aw_listUnFixeColumn;
						}
					} else {
						for (var ii = nodeIndex, jj = this._nodes.length; ii < jj; ii++) {
							...
							colNode.isFixed.title = syra_site.authorPage.localize.aw_listFixeColumn;
						}
					}
					syra_site.authorPage.endArticleUpdate();
				} else {
					if (event.target.className.indexOf("s-aw-tree-item-opener") >= 0) {
						...
						switch (event.type) {
							case "click":
								var col = this.list.cols[node.$item.$bind];
								if (col) {
									this.list.scroller.scrollToTitleItem(col.titleCell);
								}
								break;
						}
					}
