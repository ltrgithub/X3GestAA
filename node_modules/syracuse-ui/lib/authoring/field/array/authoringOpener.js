...
exports.QuickDesigner = helpers.defineClass(QuickDesigner, null, {
    appendOpener: function(designedField, $$opener){
        ...
        designedField.$$opener.bind("click", function(){
            ...
            if (designedField.$item.$cards) {
                if (designedField.$item.$cards.$position == "popup") {
                    if (designedField.records && designedField.records.length > 0) {
                        designedField.builder.togglePopupCard(designedField.records[0]);
                        return false;
                    }
                }
            }
...
exports.WidgetAuthoring = helpers.defineClass(WidgetAuthoring, Article, {
    loadBox: function(initData){
        if (!this.$$item) {
            ...
            (this.quickDesigner = new QuickDesigner()).appendOpener(this.designedField, this.$$item.appendTo($(this.layoutSlot)));
        }
    },
    dispose: function(){
