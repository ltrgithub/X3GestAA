...
exports.ListAuthoring = helpers.defineClass(ListAuthoring, RawPage, {
    ensureCards: function($cards){
        if (!$cards) {
            $cards = {
                $layout: {
                    $items: Object.keys(self.designedField.$fieldProperties).map(function($bind){
                        return {
                            $bind: $bind
                        };
                    })
    ...
    onNotifyDataChange: function(field, value){
        ...
        if ($bind == "$cardsPosition" || $bind == "$cardsWidth") {
    ...
    applyChange: function(newData){
        if (newData) {
            ...
            if (newData.$cards && newData.$cards.$position) {
                var delta = newData.$properties = newData.$properties || {};
                delta.$cardsWidth = delta.$cardsWidth || {};
                delta.$cardsWidth.$isHidden = (newData.$cards.$position == "no") || (newData.$cards.$position == "inline");
            }
