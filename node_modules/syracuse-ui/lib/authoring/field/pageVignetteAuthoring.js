...
exports.PageVignetteAuthoring = helpers.defineClass(PageVignetteAuthoring, RawPage, {
    create: function(designedField){
        ...
        if (designedField.vignette.$facet == "$query") {
    ...
    onNotifyVignetteContentChange: function($contentItem){
        //this.designedField.$item.$page = $contentItem;
        this.designedField.$item.$page = {};
        var $props = Object.keys($contentItem);
        for (var ii = 0, jj = $props.length; ii < jj; ii++) {
            if ($props[ii] != "$menus") {
                this.designedField.$item[$props[ii]] = $contentItem[$props[ii]];
            }
        }
    },
    showAddin: function(slot){
        if (this.contentAddin) {
            if (this.contentField && this.contentAddin && this.contentAddin.awContext) {
                this.contentAddin.awContext.authorPage.getContext(this.contentField);
            }
            else {
    ...
    ensureDesignContext: function(awContext){
        if (this.contentAddin && this.contentAddin.ensureDesignContext) {
            this.contentAddin.ensureDesignContext(awContext);
        }
    },
    onNotifyDataChange: function(field, value){
    },
    applyChange: function(newData){
        if (this.contentAddin && newData.$page) {
    ...
    loadBox: function(initData){
        ...
        if (this.contentAddin) {
            this.contentAddin.loadBox(initData);
        }
    },
    dispose: function(){
        if (this.contentAddin) {
