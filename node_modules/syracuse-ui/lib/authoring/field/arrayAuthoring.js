...
exports.ArrayAuthoring = helpers.defineClass(ArrayAuthoring, RawPage, {
    ...
    loadBox: function(){
        ...
        if (this.designedField.$item.$cards) {
            delta.$position = this.designedField.$item.$cards.$position || "no";
            delta.$widths = this.designedField.$item.$cards.$widths || "50";
        }
    ...
    refreshToolsBar: function(){
        if (this.awContext && this.awContext.toolsBar) {
            this.awContext.toolsBar.columnsToolBox.toggle(true);
        }
    },
    dispose: function(){
