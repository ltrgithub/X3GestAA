...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	getDesignedCardItem: function() {
		...
		if (this.awArticle.$item.$format == "grid") {
			if (this.awArticle.$item.$cardItem.$position == "inline") {
	...
	onTemplateChoiceClick: function(picker, awItem, awLayout) {
		if (this.awArticle.records && this.awArticle.records.length > 0) {
			switch (picker.syraPickerType) {
				...
				case "$model_factory":
					if (this.awArticle.$item.$format == "grid") {
						var $cardPosition = this.awArticle.$item.$cardItem.$position;
						this.awArticle.builder.applyDesignMetaData({
							$cardPosition: "no"
						}, true);
						this.awArticle.$item.$cardItem = this.awArticle.defineDefaultCard();
						this.awArticle.$item.$cardItem.$position = $cardPosition;
						if (this.awArticle.$item.$cardItem.$position == "inline") {
							this.awArticle.$inlineFieldBinds = this.awArticle.page.getDefinedFieldBinds(this.awArticle.$item.$cardItem.$layout.$items);
						}
						this.awArticle.builder.applyDesignMetaData({
