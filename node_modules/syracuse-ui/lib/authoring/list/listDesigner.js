...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	getDesignedCardItem: function() {
		...
		if (this.awArticle.$item.$format == "grid") {
			if (this.awArticle.$item.$inlineCard || this.awArticle.$item.$cards.$position == "inline") {
	...
	onTemplateChoiceClick: function(picker, awItem, awLayout) {
		if (this.awArticle.records && this.awArticle.records.length > 0) {
			var cardItem = this.getDesignedCardItem();
			switch (picker.syraPickerType) {
				case "$model_smart":
					this.templatesTool._applySectionSmartModel(cardItem);
					return awItem;
				case "$model_3cols":
					this.templatesTool.splitInColumns(cardItem, null, 3);
					return awItem;
				case "$model_4cols":
					this.templatesTool.splitInColumns(cardItem, null, 4);
					return awItem;
				case "$model_mixt":
					this.templatesTool.build_mixt(cardItem);
					return awItem;
				default:
					if (picker.syraPickerType.indexOf("$model_") == 0) {
						this.templatesTool._applySectionModel(cardItem, picker.syraPickerType);
						return awItem;
					}
					break;
			}
