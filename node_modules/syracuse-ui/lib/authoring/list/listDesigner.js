...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	onTemplateChoiceClick: function(picker, awItem, awLayout) {
		if (this.awArticle.records && this.awArticle.records.length > 0) {
			...
			switch (picker.syraPickerType) {
				case "$model_smart":
					this.templatesTool.applySectionSmartModel(record);
					break;
				case "$model_factory":
					this.templatesTool.applyCardListFactoryModel(this.awArticle);
					break;
				case "$model_3cols":
					this.templatesTool.splitInColumns(record, null, 3);
					break;
				case "$model_4cols":
					this.templatesTool.splitInColumns(record, null, 4);
					break;
				case "$model_mixt":
					this.templatesTool.build_mixt(record);
					break;
				default:
					if (picker.syraPickerType.indexOf("$model_") == 0) {
						this.templatesTool.applySectionModel(record, picker.syraPickerType);
					}
					break;
			}
