...
exports.registerDesignViews = function(page, select) {
	...
	if (page.$selectedDesignView) { // && page.$views.length > 1) {
		...
		if (!page.designerViewOpener) {
			page.designerViewOpener = syra_menus.addTextButton(title, "s-designer-view-opener-text", "togglePageOptions");
			page.titleLabel.parentNode.insertBefore(page.designerViewOpener, page.titleLabel.nextSibling);
			if (!page.togglePageOptions) {
				page.togglePageOptions = function() {
					_toggle(this);
				};
			}
		} else {
			syra_menus.setButtonTitle(page.designerViewOpener, title);
		}
	} else {
			...
			onSuccess: function(data, response, requestUrl) {
				if (data.$prototype) {
					setTimeout(function() {
						...
						syra_url.history.load($itemPage.openerUrlSegments.$url, $itemPage);
					}, 100);
				} else {
					if (data.$authorUrl) {
...
function _onDesignViewClick(page, event, target) {
...
function _toggle(page) {
	if (!page.designerViewOpener.syraIsDisabled) {
		if (!page.designViewsPopup) {
			...
			if (!page.onDesignViewClick) {
				page.onDesignViewClick = function(event, target) {
					_onDesignViewClick(this, event, target);
				};
			}
			page.designViewsPopup = syra_site.dialogManager.openPopup(page, {
				content: {},
				slot: slot,
				picker: page.designerViewOpener,
				position: {
					my: "right top",
					at: "right bottom",
