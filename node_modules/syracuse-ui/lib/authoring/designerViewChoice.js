...
exports.registerDesignViews = function(page, select) {
	...
	if (page.$selectedDesignView) { // && page.$views.length > 1) {
		...
		if (!page.designerViewBtn) {
			...
			page.titleLabel.parentNode.insertBefore(page.designerViewBtn.link, page.titleLabel.nextSibling);
		} else {
			syra_menus.setButtonTitle(page.designerViewBtn.link, title);
		}
	} else {
	...
	if (!page.designerViewBtn.link.syraIsDisabled) {
		if (!page.designViewsPopup) {
			...
			for (var ii = 0, jj = page.$views.length; ii < jj; ii++) {
				...
				btn.syraTip = 1;
				$view.$selected && syra_menus.button.setTitle(page.designerViewBtn, btn.text);
			}
