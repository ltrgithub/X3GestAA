...
exports.LandingPageDesigner = helpers.defineClass(LandingPageDesigner, Designer, {
	...
	notifyDataChange: function(field, value) {
		if (this.editTitleField == field) {
			var tabOpened = this.designedArticle.getOpenedTab();
			if (!tabOpened.$item.$title || tabOpened.$item.$title.indexOf("{@") < 0) {
				tabOpened.$item.$title = "{@" + helpers.uuid.generate() + "}";
			}
			this.designedArticle.saveArticleLocalize(this.extractExpressionCode(tabOpened.$item.$title), value);
			tabOpened.setTitle(tabOpened.$item.$title);
		}
	...
	onTabClik: function(tabOpened) {
		if (this.isVisible) {
			...
			this.editTitleField.setDataValue(tabOpened.getTitle());
