...
function _applyHackBlockRankToTitle(source, mapList) {
	if (source && typeof(source) == 'object') {
		var properties = Object.keys(source);
		for (var ii = 0, jj = properties.length; ii < jj; ii++) {
			var property = properties[ii];
			...
			if (typeof(sourceValue) == 'object') {
				if (sourceValue != null) {
					if (Array.isArray(sourceValue)) {
						for (var mm = 0, kk = sourceValue.length; mm < kk; mm++) {
							_applyHackBlockRankToTitle(sourceValue[mm], mapList);
						}
					} else {
						_applyHackBlockRankToTitle(sourceValue, mapList);
					}
				}
			} else {
...
exports.applyHackBlockRank = function($prototype, $article) {
	if ($prototype.$localization && $prototype.$hackBlockRank && !$article.$isBlockRankSet) {
		$article.$isBlockRankSet = true;
		_applyHackBlockRankToTitle($article, $prototype.$hackBlockRank);
	}
};