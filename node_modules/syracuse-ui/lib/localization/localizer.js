...
exports.getLanguage = function() {
	return syra_site.userProfile.getSelectedLocale().code;
};


exports.getDefaultTabTitle = function(boxParent, $id) {
	var defaultTitle = syra_site.localize.defaultTabTitle;
...
function _localizeFieldValue(menuItem) {
	if (!menuItem.contextField.getDataValue() || menuItem.contextField.getDataValue() == '') {
		syra_site.showMessage({
			$title: syra_site.localize.fieldEmptyErrLocalizeTitle,
			$message: syra_site.localize.fieldEmptyErrLocalize,
			$type: "warning",
			$buttons: "ok",
			$default: "ok"
		});
	...
	syra_controller.sendRequest(null, {
		...
		if (!data.$values) {
			syra_site.showDiagnoses({
				$diagnoses: [{
					$message: syra_site.localize.lPageNoValues
				}]
			}, menuItem);
			return false;
		}
		...
		syra_site.dialogManager.openPage(menuItem.page, options);
	}, function(error) {
		var $diagnoses;
		if (error.data.indexOf("$diagnoses") != -1) {
			$diagnoses = JSON.parse(error.data).$diagnoses;
...
function _localizeSectionTitle(menuItem) {
	syra_site.userProfile.getRegionalPreferences(function(preferences) {
		...
		syra_site.dialogManager.openPage(menuItem.page, {
			page: translationPage,
			onValidate: function(page) {
				...
				menuItem.contextField.page.saveDesign(); //page is designer
				return true;
			}
		});
	});
