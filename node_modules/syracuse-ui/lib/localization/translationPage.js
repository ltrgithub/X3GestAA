...
exports.TranslationPage = helpers.defineClass(TranslationPage, DesktopPage, {
	loadBox: function(data) {
					...
					$item: {
						$properties: {
							$localeChoice: {
								...
								$title: syra_site.localize.lPageLocale,
								$value: {
							...
							$localeValue: {
								...
								$title: syra_site.localize.lPageField,
		...
		this.$item = {
			$category: "page",
			$title: syra_site.localize.lPageTitle,
			$layout: {
				$items: [{
					$format: "cards",
					...
					$layout: {
						$items: [{
							...
							$items: [{
								...
								$format: "$combo",
								$isTitleHidden: true,
								$title: syra_site.localize.lPageLocaleTitle,
			...
			if (record.dataset.$localeChoice != null) {
				...
				if (record.dataset.$localeChoice == 0) {
					syra_site.showDiagnoses({
						$diagnoses: [{
							$message: syra_site.localize.lPageLocaleNotSet
						}]
					}, field);
				} else {
