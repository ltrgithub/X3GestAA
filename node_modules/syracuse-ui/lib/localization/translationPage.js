...
exports.TranslationPage = helpers.defineClass(TranslationPage, DesktopPage, {
	load: function(data) {
		...
		this.listField = this.boundFields.$translations[0];
		this.applyChange({
		...
		if (field.$item.$bind == '$localeValue') {
			if (record.dataset.$localeChoice != null) {
				...
				if (record.dataset.$localeChoice == 0) {
					syra_alert.error(syra_local.lPageLocaleNotSet, field);
				} else {
	...
	onMenuClick: function(options) {
		if (options.menu.$bind == '$delete') {
			this._onRemoveLocale(options.menu.articleParent);
			return false;
		}
	},
	_onRemoveLocale: function(record) {
		...
		if (record.dataset.$localeChoice) {
			...
			if ($isListMaxLength && $isLastRecordSet) {
				this._addEmptyRow(list.dataset.length);
			}
		}
	},
	applyChange: function(newData) {
		...
		if (this.listField && this.listField.records) {
			for (var ii = 0, jj = this.listField.records.length; ii < jj; ii++) {
				var record = this.listField.records[ii];
				if (record.dataset && record.dataset.$localeChoice != undefined) {
					...
					if (record.dataset.$localeChoice == 0 || comboItem.$isEditMode) {
						...
						if (keys.length > 0) {
							...
							comboItem.choiceFormat.setEnum(comboItem, $enum);
						}
					}
				}
			}
	...
	applyShortCut: function(shortcuts, focusField) {
		if ((shortcuts.enter || shortcuts.tab) && focusField) {
			if (focusField.getValue() &&
				focusField.$item.$bind == "$localeValue" &&
				focusField.articleParent.getRecordIndex() == (this.listField.dataset.length - 1) &&
				this.listField.dataset.length < Object.keys(this._enumLocale).length) {
				this._addEmptyRow(focusField.articleParent.getRecordIndex() + 1);
			}
		}
	}
});