...
exports.openLookup = function(box, options) {
	options.resizeDialog = function(dlg, isDlgResizing) {
		...
		var style = page.lookupField.bodyTableSlot.style;
		if (!isDlgResizing) {
			style.width = "";
			style.height = "";
		}
		dlg.setScrollPageSize(page, isDlgResizing);
		page.lookupField.resizeScrollView && page.lookupField.resizeScrollView();
		var diff = page.scrollview.scrollHeight - page.scrollview.clientHeight;
		if (diff) {
			diff += 2; // margin for chrome
			style.height = (page.lookupField.bodyTableSlot.getBoundingClientRect().height - diff) + "px";
		}
	};
