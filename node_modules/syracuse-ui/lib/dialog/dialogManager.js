...
exports.openLookup = function(box, options) {
	options.resizeDialog = function(dlg, isDlgResizing) {
		...
		dlg.setScrollPageSize(page, isDlgResizing);
		var diff = page.scrollview.scrollHeight - page.scrollview.clientHeight;
		if (diff) {
			scroller.bodySlot.style.height = (scroller.bodySlot.getBoundingClientRect().height - diff) + "px";
		}
	};
...
exports.closePopups = function(box, event) {
	...
	for (var ii = 0, jj = _stack.length; ii < jj; ii++) {
		...
		if (dlg.options && dlg.options.$isAutoClose) {
			...
			if (!event || !(site.dom.isParentNode(dlg.dialogSlot, event.target) ||
				site.dom.isParentNode(dlg.options.autoCloseBoundary, event.target))) {
				dlg.close();
