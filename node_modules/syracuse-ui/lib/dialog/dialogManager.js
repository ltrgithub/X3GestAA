...
exports.openLookup = function(box, options) {
	options.resizeDialog = function(dlg) {
		...
		var scrollview = dlg._content.scrollview;
		scrollview.style.width = scrollview.style.height = "";

		var scroller = page.lookupField && page.lookupField.scroller;
		var listScrollRect = document.site.getBoundingClientRect(scroller.bodySlot);
		scroller.bodySlot.style.width = scroller.bodySlot.style.height = "";

		dlg.centerDialog();
		var rect = document.site.getInnerSize(dlg.dialogSlot);
		scrollview.style.width = rect.width + "px";
		scrollview.style.height = (rect.height - document.site.getBoundingClientRect(dlg._content.diagnoseSlot).height) + "px";


		var diff = scrollview.scrollHeight - scrollview.clientHeight;
		if (diff) {
			var listScrollRect = document.site.getBoundingClientRect(scroller.bodySlot);
			scroller.bodySlot.style.height = (listScrollRect.height - diff) + "px";
		}

		/*var size = {
         dialog: document.site.getBoundingClientRect(dlg.dialogSlot),
         scrollview: document.site.getBoundingClientRect(dlg._content.scrollview)
         };
         size.dialog.inner = document.site.getInnerSize(dlg.dialogSlot);
         size.dialog.inner.height = size.dialog.inner.height - (dlg._content.diagnoseSlot ? dlg._content.diagnoseSlot.clientHeight : 0);
