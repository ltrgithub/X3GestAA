...
exports.Dialog = helpers.defineClass(Dialog, null, {
	...
	_openContent: function() {
		...
		this._endOpen();
		!this._content.isPageLoaded && this.resizeDialog(); //content is not page => resize
	},
	_endOpen: function() {
		var self = this;
		if (self.isModal) {
		...
		if (self.options.$autoFocus) {
			...
			self.dialogSlot.appendChild(link);
			link.focus();
		}
		self.options.onOpened && self.options.onOpened(self._content);
	...
	_openDialogPage: function($itemPage) {
		...
		if ($itemPage || self.options.page) {
			...
			self._content.domTitle.className = self.$skin + "-page-title";
			...
			if (self.options.onValidate) {
				self.btn_ok = syra_menus.addIconButton(syra_local.dlg_ok, self.$skin + "-page-ok", "onOkClick");
				self.btn_ok.syraTool = this.id;
	...
	appendCloseButton: function(slot) {
		this.btn_close = syra_menus.addIconButton(syra_local.dlg_close, this.$skin + "-page-close", "onCloseClick");
				...
				if (this.options.resizeDialog) {
					...
					if (this.options.position) {
						...
						if (this._content && this._content.page && this._content.page.scrollview) {
							var page = this._content.page;
							if (page.scrollview) {
								var lookupField = page.lookupField;
								if (lookupField && !isDlgResizing) {
								...
								var maxPageWidth;
								if (page.lookupField && page.lookupField.colsWidth) {
								...
								var maxDlgHeight = Math.max(100, this._maxHeight);
								var maxDlgWidth = Math.max(200, this._maxWidth);
								page.barHeight = Math.min(maxPageHeight, maxDlgHeight);
								page.scrollview.style.height = (page.barHeight - this._content.diagnoseSlot.getBoundingClientRect().height) + "px";
								...
								!isDlgResizing && this.centerDialog();
