...
function _setZIndex(dlg) {
	var zIndex;
	if (dlg._content && dlg._content.isAlertDialog) {
...
function _loadPageRepresentation(dlg, onLoaded) {
	if (dlg.options.$method && (dlg.options.$method != "GET"))
		syra_router.postQuery(dlg.options, null, dlg.options.article, function($location, data) {
			...
			syra_router.loadRepresentation({
				...
				success: function($itemPage) {
					if (data && syra_form.isFormUrl(dlg.options.$url)) {
...
function _loadPage(dlg, onLoaded) {
	...
	$(dlg.dialogSlot).draggable({
		containment: "#" + syra_site.layoutSlot.id,
		cancel: "article." + $skin + ",." + dlg.$skin + "-page-close,." + dlg.$skin + "-page-ok",
		stop: function(event, ui) {
			var rect = dlg.dialogSlot.getBoundingClientRect();
			dlg.customPosition = {
				top: ui.position.top,
		...
		minWidth: syra_site.config.size.dlgResizeMinWidth,
		minHeight: syra_site.config.size.dlgResizeMinHeight,
...
exports.Dialog = helpers.defineClass(function() {}, null, {
	...
	activate: function(isActivated) {
		...
		syra_dom.hide(this.dialogSlot, this.isDeactivated);
	},
		...
		if (!this.isDeactivated && this._content && this.isOpened && !this.disposed) {
			...
			if (syra_site.body) {
				...
				if (syra_site.isResizing || !this.isMinMaxInit) {
					...
					var dlgSize = (pageSize && pageSize.dlg) || siteSize.dlg;
					this.dialogSlot.style.maxWidth = dlgSize.maxWidth + "px";
					this.dialogSlot.style.maxHeight = dlgSize.maxHeight + "px";
				}
							...
							if (page && page.scrollview) {
								...
								if (page.$facet == "$lookup" || page.$facet == "$select") {
									if (!this.customPosition) {
										...
										this.dialogSlot.style.top = this.customPosition.top + "px";
										this.dialogSlot.style.left = this.customPosition.left + "px";
									}
								} else {
									var scrollTop = page.scrollview.scrollTop;
									this.dialogSlot.style.minWidth = siteSize.dlg.minWidth + "px";
									var pageSize = page.getPageSize();
									pageSize.maxScrollWidth = pageSize.dlg.maxWidth - pageSize.dlg.marginWidth;
									pageSize.maxScrollHeight = pageSize.dlg.maxHeight - pageSize.dlg.marginHeight;
									...
									if (list && list.onDialogResize) {
										var additionalHeight = list.onDialogResize(pageSize.maxScrollHeight - page.scrollview.scrollHeight);
										if (additionalHeight) {
									...
									if (page.designer && page.designer.isDesignBarVisible) {
										...
										if (page.designer.settingBar) {
											barH = Math.max(page.designer.settingBar.getScrollHeight(), barH);
										}
									} else {
									...
									if (barH) {
										...
										pageSize.barHeight = barH = Math.min(pageSize.maxBarHeight, Math.max(scrollHeight, barH));
	...
	dispose: function() {
		...
		if (this.options) {
			...
			if (this.options.content) {
				...
				this._content && this._content.dispose && this._content.dispose();
				this.dialogSlot && syra_dom.empty(this.dialogSlot);
			}
			this.dialogSlot && syra_dom.remove(this.dialogSlot);
		}
