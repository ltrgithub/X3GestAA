...
function _openPage(dlg, $itemPage) {
	if ($itemPage || dlg.options.page) {
		...
		dlg.appendCloseButton(page.header);
		...
		if (dlg.options.onValidate) {
			...
			page.header.appendChild(dlg.btn_ok);
...
function _resizePage(dlg, page, isDlgResizing) {
	...
	var diffH = (slotRect.bottom - scrollRect.bottom) + (scrollRect.top - slotRect.top);
	var diffW = (slotRect.right - scrollRect.right) + (scrollRect.left - slotRect.left);
	...
	var barH = page.menuBar && page.menuBar.barSlot.clientHeight;
	if (page.fusionBar) {
		barH = Math.max(page.fusionBar.barSlot.clientHeight, barH);
	}
	if (barH) {
		page.barHeight = barH = Math.min(Math.max(scrollRect.height, barH), page.maxBarHeight);
		if (barH != scrollRect.height) {
			page.scrollview.style.height = barH + "px";
	...
	if (page.designer) {
		if (page.designer.treesBar) {
			maxPageHeight = Math.max(page.designer.treesBar.barSlot.scrollHeight, maxPageHeight);
			barWidth += page.designer.treesBar.barSlot.clientWidth;
		}
		maxPageHeight = Math.max(page.designer.propertiesSlot.scrollHeight, maxPageHeight);
		barWidth += page.designer.propertiesSlot.clientWidth;
	}
...
function _resizeLookupPage(dlg, page, isDlgResizing) {
	...
	lookupField.bodyTableSlot.style.width = "0px";
	lookupField.bodyTableSlot.style.height = "0px";
	...
	var slotRect = dlg.dialogSlot.getBoundingClientRect();
	var fieldRect = lookupField.fieldValue.getBoundingClientRect();
	var bodyRect = lookupField.bodyTableSlot.getBoundingClientRect();
	var coreRect = lookupField._core.getBoundingClientRect();
	var diffW = (slotRect.right - fieldRect.right) + (fieldRect.left - slotRect.left);
	lookupField.fieldValue.style.maxWidth = (siteSize.dlgMaxW - diffW) + "px";

	var diffH = (slotRect.bottom - bodyRect.bottom) + (bodyRect.top - slotRect.top);
	var diffW = (slotRect.right - coreRect.right) + (coreRect.left - slotRect.left);
	lookupField.bodyTableSlot.style.maxHeight = (siteSize.dlgMaxH - diffH) + "px";
	lookupField.bodyTableSlot.style.maxWidth = (lookupField.tableMaxWidth = (siteSize.dlgMaxW - diffW)) + "px";

	lookupField.bodyTableSlot.style.width = "";
	lookupField.bodyTableSlot.style.height = "";

	lookupField.resizeScrollView();
