...
exports.Dialog = helpers.defineClass(Dialog, null, {
	...
	_appendAutoClose: function(append) {
		...
		if ((self.options.$autoClose === undefined) ? true : self.options.$autoClose) {
			if (append) {
				...
				document.site.dialogManager.registerAutoClosePopups(self, true);
				...
				if (self._autoCloseNamespace) {
					document.site.dialogManager.registerAutoClosePopups(self, false);
	...
	_openDialogPage: function($itemPage) {
		...
		if ($itemPage || self.options.page) {
			...
			document.site.dialogManager.registerDialogPage(self, true);
		} else {
		...
		if (this._content) {
			...
			if (!this.disposed && document.site.body) { // && document.site.header) {
				...
				if (this.options.position) {
					...
					if (this.options.openerPage && this.options.openerPage.scrollview) {
						var isWithIn = true;
						if (this.options.position.of) {
							isWithIn = document.site.isParentNode(this.options.openerPage.scrollview, this.options.position.of);
						}
						if (isWithIn) {
							$position.collision = "flipfit";
							$position.within = $(this.options.openerPage.scrollview);
						}
					}
	...
	dispose: function() {
		...
		document.site.dialogManager.registerDialogPage(this, false);
		if (this._content) {
			if (this._content.page && this._content.page.registerPositionedDialog) {
		...
		if (this.options) {
			this.options.dragSpot = this.options.page = this.options.openerPage = null;
			if (this.options.boxParent) {
				document.site.dialogManager.closeBoxDialog(this.options.boxParent, this);
			}
		...
		this.okBtn = this.$$dialogSlot = this.dialogSlot = this.overlay = this._content = this.options = null;
	}
});