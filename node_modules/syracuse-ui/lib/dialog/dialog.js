...
function _openPage(dlg, $itemPage) {
	if ($itemPage || dlg.options.page) {
		...
		if (!dlg.options || !dlg.options.syraPlayback) {
			_setZIndex(dlg);
		}
		dlg.dialogSlot.style.top = "0px";
		//dlg.dialogSlot.style.visibility = "hidden";
						...
						if (this.options.position) {
							...
							if (page && page.scrollview) {
								if (page.$facet == "$lookup" || page.$facet == "$select") {
									...
									pageSize.maxScrollWidth = siteSize.dlg.maxWidth - pageSize.dlgMargin.width;
									pageSize.maxScrollHeight = siteSize.dlg.maxHeight - pageSize.dlgMargin.height;
