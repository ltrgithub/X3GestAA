...
exports.Dialog = helpers.defineClass(Dialog, null, {
	...
	resizePositioned: function(siteBodyRect) {
		var slotRect = document.site.getBoundingClientRect(this.dialogSlot);
		if (slotRect.width > siteBodyRect.width) {
			this.dialogSlot.style.width = (slotRect.width = siteBodyRect.width) + "px";
		}
		if (slotRect.height > this._maxHeight) {
			this.dialogSlot.style.height = (slotRect.height = this._maxHeight) + "px";
		}
		var $position = {
		...
		if (page && page.scrollview) {
			...
			if (this.options.position.of) {
				isWithIn = document.site.isParentNode(page.scrollview, this.options.position.of);
			}
			if (isWithIn) {
	...
	resizeFromContent: function(siteBodyRect) {
		if (!this.widgetResizing) {
			this.dialogSlot.style.height = this.dialogSlot.style.width = "";
			var slotRect = document.site.getBoundingClientRect(this.dialogSlot);
			this.dialogSlot.style.width = (slotRect.width = Math.min(slotRect.width, siteBodyRect.width)) + "px";
			this.dialogSlot.style.height = (slotRect.height = Math.min(slotRect.height, siteBodyRect.height)) + "px";
		...
		if (this._content) {
			...
			if (!this.disposed && document.site.body) { // && document.site.header) {
				...
				if (this.options.position) {
					this.resizePositioned(siteBodyRect);
				} else {
					if (this._content) {
						if (this.options.$dialogSize == "content") {
							this.resizeFromContent(siteBodyRect);
						} else {
