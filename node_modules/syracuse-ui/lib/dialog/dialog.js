...
function _resizeOverlay(dlg) {
	...
	if (dlg._content.isFusionPage) {
		...
		dlg.overlay.style.top = bodyRect.top + "px";
		dlg.overlay.style.height = bodyRect.height + "px";
	} else {
		dlg.overlay.style.height = areaRect.height + "px";
	}
	dlg.overlay.style.width = areaRect.width + "px";
...
function _resizePositioned(dlg) {
	var siteBodyRect = dlg.getSiteBodyRect();
	var slot = dlg.dialogSlot;
	var slotRect = document.site.getBoundingClientRect(slot);
	if (slotRect.width > siteBodyRect.width) {
		slot.style.width = (slotRect.width = siteBodyRect.width) + "px";
	}
	if (slotRect.height > dlg._maxHeight) {
		slot.style.height = (slotRect.height = dlg._maxHeight) + "px";
	}
	var $position = {
	...
	if (page && page.scrollview) {
		...
		if (dlg.options.position.of) {
			isWithIn = document.site.isParentNode(page.scrollview, dlg.options.position.of);
		}
		if (isWithIn) {
...
exports.Dialog = helpers.defineClass(Dialog, null, {
	...
	centerDialog: function() {
		...
		var slotRect = document.site.getBoundingClientRect(this.dialogSlot);
		this.dialogSlot.style.width = Math.round(Math.min(slotRect.width, siteBodyRect.width)) + "px";
		this.dialogSlot.style.height = Math.round(Math.min(slotRect.height, siteBodyRect.height)) + "px";
		slotRect = document.site.getBoundingClientRect(this.dialogSlot);
		this.dialogSlot.style.top = (siteBodyRect.bounding.height - slotRect.height) / 2 + "px";
		this.dialogSlot.style.left = (siteBodyRect.bounding.width - slotRect.width) / 2 + "px";
	},
	...
	_openContent: function() {
		...
		self.options.$isAutoClose = self.options.$isAutoClose !== false;
	...
	_openDialogPage: function($itemPage) {
		...
		if ($itemPage || self.options.page) {
			...
			if (self.options.onValidate) {
				self._appendBtn(self._content.header, "ok");
	...
	_appendBtn: function(slot, id) {
		var btn = this["btn_" + id] = document.createElement("a");
		var article = this._content;
		if (!this._content.setArticleId) {
			this._content.dialogWrapper = this;
			btn.syraBoxId = this._content.id;
			article = this._content.articleParent;
		}
		if (article) {
			article.setArticleId(btn);
		}
		btn.setAttribute("data-s-picker", "s-dialog-" + id);
		btn.className = this.$skin + "-page-" + id;
		btn.title = document.site.localize["dlg_" + +id];
		slot.appendChild(btn);
	},
	appendCloseButton: function(slot) {
		this._appendBtn(slot, "close");
	},
			...
			if (!this.disposed && document.site.body) { // && document.site.header) {
				if (this.options.resizeDialog) {
					if (!this.widgetResizing) {
						this.options.resizeDialog(this);
					}
				} else {
					var siteBodyRect = this.getSiteBodyRect();
					...
					if (this.options.position) {
						_resizePositioned(this);
					} else {
						if (this._content && this._content.page && this._content.page.scrollview) {
							var page = this._content;
							var scrollview = this._content.scrollview;
							if (scrollview) {
								scrollview.style.width = scrollview.style.height = "";
								this.centerDialog();
								var rect = document.site.getInnerSize(this.dialogSlot);
								if (page.menuBar && page.menuBar.barSlot) {
									rect.width -= document.site.getBoundingClientRect(page.menuBar.barSlot).width;
								}
								scrollview.style.width = rect.width + "px";
								scrollview.style.height = (rect.height - document.site.getBoundingClientRect(this._content.diagnoseSlot).height) + "px";
								return;
							}
						}
					}
