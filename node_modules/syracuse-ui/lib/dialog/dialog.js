...
function _getSiteBodyRect(){
    var rect = document.site.dom.getBoundingClientRect(document.site.body);
...
function _resizePositioned(dlg){
    ...
    var rect = document.site.dom.getBoundingClientRect(slot);
    ...
    if (page && page.scrollview) {
        ...
        if (dlg.options.position.of) {
            isWithIn = document.site.dom.isParentNode(page.scrollview, dlg.options.position.of[0]);
        }
        if (isWithIn) {
...
exports.Dialog = helpers.defineClass(Dialog, null, {
    ...
    _openContent: function(){
        ...
        self.dialogSlot = self.options.slot || self.options.content.domItem;
    ...
    _endOpen: function(){
        ...
        if (self.isModal) {
            ...
            document.site.dom.setZIndex(self.overlay);
            document.site.layoutSlot.appendChild(self.overlay);
        }
        document.site.dom.setZIndex(self.dialogSlot);
        document.site.layoutSlot.appendChild(self.dialogSlot);
    ...
    setScrollPageSize: function(page, isDlgResizing){
        if (page.scrollview) {
            ...
            var rect = site.dom.getInnerSize(this.dialogSlot);
            if (page.designer) {
            ...
            if (isDlgResizing) {
                page.resizeArticle(isDlgResizing);
            }
        }
    },
    dispose: function(){
        if (this.overlay) {
            document.site.dom.removeChild(this.overlay);
        }
        if (this._widgetResizer) {
        ...
        if (this.options) {
            ...
            if (this.options.content) {
                ...
                if (this.dialogSlot) {
                    document.site.dom.empty(this.dialogSlot);
                }
            }
            if (this.dialogSlot) {
                document.site.dom.removeChild(this.dialogSlot);
            }
