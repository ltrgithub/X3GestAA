...
var _localize = {
    ...
    print: "Print",
    sourceDown: "Show Rich Text",
    sourceUp: "Show source",
    txtmodeDown: "Show Rich Text ",
    txtmodeUp: "Show Raw Text ",
};
...
/*var  $$listTxtMode = $('<select>').attr('mode', function(){
    ...
    $.cleditor.buttons.txtmode.getEnabled = function(data) {
        if (data.editor._isRaw) {
            data.button.title = _localize.txtmodeDown;
            // $(data.button).removeAttr("style") ;
            $(data.button).removeClass("buttonRichText");
            $(data.button).addClass("buttonRawText");
            //.css("background-image", "url(\"/syracuse-ui/themes/desktop/sage/cleditor/images/hello.gif\")" ) ; 
        } else {
            data.button.title = _localize.txtmodeUp;
            // $(data.button).removeAttr("style") ;
            $(data.button).removeClass("buttonRawText");
            $(data.button).addClass("buttonRichText");
            //css("background-image", "url(\"/syracuse-ui/themes/desktop/sage/cleditor/images/hello.png\")" ) ; 
        }
        return true;
    }
    ...
    Object.keys(_localize).forEach(function(key) {
        ...
        if (key == "sourceUp") $.cleditor.buttons.source.title = _localize[key];
        else if (key == "sourceDown");
        else if (key == "txtmodeUp") $.cleditor.buttons.txtmode.title = _localize[key];
        else if (key == "txtmodeDown");
        else {
/*if (!$.cleditor.buttons[key])
    console.log("j'ai une erreur: "+key +" doesn't exists");*/
            $.cleditor.buttons[key].title = _localize[key];
        }
    });
    ...
    $.cleditor.buttons.txtmode.buttonClick = function(e, data) {
        ...
        if (!data.editor._isRaw) {
            ...
            //button.style("background-image", "url(\"/syracuse-ui/themes/desktop/sage/cleditor/images/hello.gif\")" );
            button.title = _localize.txtmodeDown;
            $(button).removeClass("buttonRichText");
            $(button).addClass("buttonRawText");
            //.css("background-image", "url(\"/syracuse-ui/themes/desktop/sage/cleditor/images/hello.gif\")" ) ; //"buttonRichText"
            ...
            button.title = _localize.txtmodeUp;
            $(button).removeClass("buttonRawText");
            $(button).addClass("buttonRichText");
            //$(button).css("background-image", "url(\"/syracuse-ui/themes/desktop/sage/cleditor/images/hello.png\")" ) ; //"buttonRichText"
        }
        data.editor.updateTextArea();
        //setTimeout(function() {refreshButtons(data.editor);}, 100);
        data.editor.disable(data.editor.disabled);
        return true;
    };
    ...
    $.each(formattingbuttons.split(" "), function(idx, buttonName) {
        ...
        $.cleditor.buttons[buttonName].getEnabled = function(data) {
            if (data.editor._isRaw) return false;
            else return true;
        }
    });
    ...
    render: function() {
        ...
        if (this.$isEditMode) {
            ...
            self._editor = area.cleditor({
                ...
                " undo redo | rule image link unlink | cut copy paste  | print source txtmode ",
                /*pastetext*/
...
/*else if (contents && contents.indexOf('<') === 0) {
    ...
    _getEditContents: function() {
        ...
        if (this._isRtf) {
            if (contents != this.currentHtml) this.currentHtml = contents;
...
/* else if (this._isHtml){
            ...
            var outer = $(contents);
            if (outer.is('pre')) contents = outer.html();
