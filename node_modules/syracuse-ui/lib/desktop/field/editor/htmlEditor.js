...
/*var  $$listTxtMode = $('<select>').attr('mode', function(){
    ...
    Object.keys(_localize).forEach(function(key) {
        ...
        else if (key == "rawMode") $.cleditor.buttons.txtmode.title = _localize[key];
        else if (key == "richMode");
        else {
/*if (!$.cleditor.buttons[key])
    console.log("j'ai une erreur: "+key +" doesn't exists");*/
            $.cleditor.buttons[key].title = _localize[key] || key + ":<undefined>";
        }
    });
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
    ...
    render: function() {
        ...
        if (this.$isEditMode) {
            ...
            var area = $('<textarea>').attr('id', "i" + id).attr('name', "i" + id).appendTo(self.$$fieldValue);
