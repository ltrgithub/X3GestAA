    ...
    drawBox: function(){
        ...
        if (list.builder.$$VGridRows) {
            if (list.recordSelector.hasColumn) {
                ...
                list.builder.$$VGridRows[list.builder.$rowSelector][0].appendChild(self._creatCell()).appendChild(div);
            }
            list.parseItems(function($item, $field){
                ...
                if ($field.$capability.filter) {
                    ...
                    var $$container = $(list.builder.$$VGridRows[$item.$bind][0].appendChild(td).appendChild(div));
                    (self._filters[$item.$bind] = new FilterCell()).render(self, $$container, $field, $item.$bind);
                }
                else {
                    list.builder.$$VGridRows[$item.$bind][0].appendChild(self._creatCell());
                }
            });
            ...
            list.builder.tdQuickFilterCapabilities = self._creatCell();
            list.builder.tdQuickFilterCapabilities.style.display = "none";
            item.appendChild(list.builder.tdQuickFilterCapabilities);
