    ...
    drawBox: function(){
        ...
        if (list.builder.vgridRows) {
            if (list.recordSelector.hasColumn) {
                ...
                list.builder.vgridRows[list.builder.$rowSelector].appendChild(self._creatCell()).appendChild(div);
            }
            list.parseItems(function($item, $field){
                //item.appendChild(self._creatCell());
                if ($field.$capability.filter) {
                    var td = self._creatCell();
                    var div = document.createElement("div");
                    div.className = "s-q-filter-cont";
                    //var $$container = $(list.builder.vgridRows[$item.$bind].appendChild(td).appendChild(div));
                    var $$container = $(list.builder.vgridRows[$item.$bind].appendChild(td));
                    (self._filters[$item.$bind] = new FilterCell()).render(self, $$container, $field, $item.$bind);
                }
                else {
                    list.builder.vgridRows[$item.$bind].appendChild(self._creatCell());
                }
            });
            ...
            if (self.$$item) {
                ...
                list.builder._header.appendChild(item);
                self.$$item = $(list.builder._header.appendChild(item));
            }
            
            item.appendChild(self._creatCell()); //append capabilities
            if (list.builder.cardview) {
                if (list.builder.cardview.$cardviewPosition == "inline") {
                    item.appendChild(self._creatCell());
                }
            }
            if (list.recordSelector.hasColumn) {
                ...
                item.appendChild(self._creatCell()).appendChild(div);
            }
            if (list.$item.$isRowIndexVisible) {
                item.appendChild(self._creatCell());
            }
            list.parseItems(function($item, $field){
                var td = item.appendChild(self._creatCell());
                if ($field.$capability.filter) {
                    var div = document.createElement("div");
                    div.className = "s-q-filter-cont";
                    // create filter anchor
                    (self._filters[$item.$bind] = new FilterCell()).render(self, $(td.appendChild(div)), $field, $item.$bind);
                    //(self._filters[$item.$bind] = new FilterCell()).render(self, $(td), $field, $item.$bind);
                }
            });
        }
