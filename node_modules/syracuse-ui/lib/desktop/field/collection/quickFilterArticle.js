    ...
    hookListClick: function(){
        ...
        self.$$list.delegate("a.s-q-filter-item", "click", function(){
            ...
            if (self.operator === "between") {
                ...
                if (highField) {
                    highField[0].$$value.toggle(false);
                }
        ...
        if (list.builder.$$VGridRows) {
            ...
            self.$item.$layout.$items.forEach(function($item){
                ...
                if ($field && !$field.$isHidden) {
                    var $$cell = $(htmlCell).appendTo(self.$$dataRow);
            ...
            if (self.$$dataRow) {
                ...
                self.$$dataRow = $("<tr/>").appendTo(list._$$header);
            }
            if (list.$item.$preView) {
                $(htmlCell).appendTo(self.$$dataRow); //no preview for Vgrid
            }
            if (list.recordSelector.hasColumn) {
                $(htmlCell).appendTo(self.$$dataRow).append("<div class='" + list.$skin + "-quick-flag'/>");
            }
            if (list.$item.$isRowIndexVisible) {
                $(htmlCell).appendTo(self.$$dataRow);
            }
            self.$item.$layout.$items.forEach(function($item){
                var $field = list.$fields[$item.$bind];
                if ($field && !$field.$isHidden) {
