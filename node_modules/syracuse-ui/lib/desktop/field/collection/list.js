...
exports.DesktopList = helpers.defineClass(DesktopList, RawList, {
    ...
    applyDesignMetaData: function(metadata, onAuthoring){
        ...
        if (metadata.$graphMode !== undefined) {
            this.$item.$graphMode = metadata.$graphMode;
            if (this.chart) {
                this.chart.$$container[0].style.display = metadata.$graphMode == "list" ? "none" : "";
            }
            if (metadata.$graphMode != "list") {
                if (!this.chart) {
                    this.appendGraph();
                }
            }
    ...
    appendGraph: function($bind){
        this.$item.$graphMode = this.$item.$graphMode || "both";
        this.$item.$graphPosition = this.$item.$graphPosition || "top";
        var div = document.createElement("div");
        div.className = "s-list-chart";
        this.chart = document.itemFactory.load($(div).prependTo(this.$$coreRight), {
