...
exports.VGridRecord = helpers.defineClass(VGridRecord, HGridRecord, {
    drawBox: function(){
        ...
        if (self.list.recordSelector.appendToRecord) {
            ...
            td.className = css;
            self._selectorCheck = self.list.recordSelector.appendToRecord(self.builder.vgridRows[self.builder.$rowSelector].appendChild(td));
            self._selectorCheck.setAttribute("data-s-record", self.$recordUuid);
        }
        self.list.parseItems(function($item, $field){
            ...
            self.builder.table.cols[$item.$bind].slot.appendChild(cell);
