...
exports.CardRecord = helpers.defineClass(CardRecord, RecordArticle, {
    ...
    drawBox: function(){
        ...
        if (this.list.recordSelector.appendToRecord) {
            this._selectorCheck = this.list.recordSelector.appendToRecord(selectorSlot);
        }
        else {
        ...
        if (this.list.$item.$isRowIndexVisible) {
            ...
            $(toolbar.appendChild(dom)).text((this.list.$startIndex || 0) + this.$recordIndex);
        }
        if (!this.list.$item.$isMenuRecordHidden) {
            var dom = document.createElement("div");
            dom.className = this.list.$skin + "-menus";
            this._loadMenu(toolbar.appendChild(dom));
        }
        
        
        var card = item.appendChild(document.createElement("div"));
        card.className = this.list.$skin + "-record";
        if (this.list.recordSelector.isRowMode) {
