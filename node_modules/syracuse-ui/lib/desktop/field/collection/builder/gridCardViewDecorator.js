...
exports.GridCardViewDecorator = helpers.defineClass(GridCardViewDecorator, null, {
        ...
        if (self.$cardviewPosition != "inline") {
            ...
            if (self.$cardviewPosition == "left" || self.$cardviewPosition == "right") {
                ...
                /*var widths = (self.list.$item.$cardviewColumnWidth || "50,50").split(",");
                 self._cardSlot.style.width = widths[(self.$cardviewPosition == "left") ? 0 : 1] + "%";
                 self.list._tableSlot.style.width = widths[(self.$cardviewPosition == "left") ? 1 : 0] + "%";*/
                self._cardSlot.style.width = (self.list.$item.$cardviewWidth || "50") + "%";
            }
            else {
            ...
            $(self._cardSlot)[(["left", "top"].indexOf(self.$cardviewPosition) >= 0) ? "prependTo" : "appendTo"](self._wrapper);
        }
    },
    release: function(){
        if (this._wrapper) {
    ...
    onMakeTitleRow: function(titleRow){
        if (this.$cardviewPosition == "inline") {
            ...
            titleRow.appendChild(th).appendChild(opener);
    ...
    onRecordHighlightSelection: function(recordArticle, selected){
        if (this.$cardviewPosition == "inline") {
            if (recordArticle.$$cardviewSlot) {
                document.site.toggleClass(recordArticle.$$cardviewSlot[0], "s-list-record-selected", selected);
            }
        }
        else {
            if (this.cardview) {
    ...
    onAfterDataBinding: function(){
        if (this.$cardviewPosition != "inline" && Object.keys(this.list.recordSelector.records).length == 0) {
            if (this.list._records.length > 0) {
                this.list.recordSelector.select(this.list._records[0].$recordUuid, true);
            }
        }
    },
    dispose: function(){
