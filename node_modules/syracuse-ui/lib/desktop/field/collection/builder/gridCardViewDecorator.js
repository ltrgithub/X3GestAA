...
exports.GridCardViewDecorator = helpers.defineClass(GridCardViewDecorator, null, {
    initialize: function(builder){
        ...
        if (self.$cardviewPosition != "inline") {
            ...
            self.list._listSlot.appendChild(self._wrapper);
            $(self._cardSlot)[(["left", "top"].indexOf(self.$cardviewPosition) >= 0) ? "prependTo" : "appendTo"](self._wrapper);
        }
    },
    release: function(){
        if (this._wrapper) {
            $(this.list._listSlot).append($(this.list._tableSlot).children());
            this.list._tableSlot = this.list._listSlot;
    ...
    onAfterDataBinding: function(){
        if (this.$cardviewPosition != "inline" && Object.keys(this.list.selectedRecords).length == 0) {
            if (this.list._records.length > 0) {
                this.list.selectRecord(this.list._records[0].$recordUuid, true);
            }
        }
    },
    dispose: function(){
