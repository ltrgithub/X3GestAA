...
exports.QuickFilterRecord = helpers.defineClass(QuickFilterRecord, Article, {
    ...
    appendFieldCell: function($item, $field){
        var td = this._appendCell(this.boxParent.$item.$format == "vgrid" ? this.boxParent.builder.table.cols[$item.$bind].slot : this.$$item[0]);
        if ($field.$capability.filter) {
            (this._filters[$item.$bind] = new QuickFilterCell()).render(this, $(td), $field, $item.$bind);
        }
    },
    drawBox: function(){
        this._filters = {};
        if (this.boxParent.$item.$format != "vgrid") {
            if (this.$$item) {
                ...
                this.boxParent.builder.table._thead.appendChild(item);
                this.$$item = $(this.boxParent.builder.table._thead.appendChild(item));
            }
        }
    },
