...
exports.Builder = helpers.defineClass(VGridBuilder, HGridBuilder, {
    initialize: function(){
        ...
        this.list.RecordClass = VGridRecord;
    },
    appendRow: function($bind){
        return this.$$VGridRows[$bind] = $("<tr/>").appendTo(this.list._$$body);
    },
    ...
    makeTitleRow: function(){
        ...
        self.list.$item.$items.forEach(function($item){
            var $field = self.list.$fields[$item.$bind];
            if ($field && !$field.$isHidden) {
                var $$title = $field.$capability.sort != false ? $("<a/>").attr("data-s-sort", $item.$bind).addClass(cssLink) : $("<div/>");
                self.appendRow($item.$bind).append($("<th/>").addClass(css).append($$title.text(self.list.getLocalizedTitle($field.$title || ""))));
            }
        });
        self.appendRow(self.$rowMenu = "$grid-row-menu").append($("<th/>").addClass(css));
    },
    ...
    emptyDataBody: function(){
        var self = this;
        self.list._$$body.children().each(function(){
            $(this).children().slice(self.$columnsCount).remove();
        });
    },
    appendNoRecordRow: function(){
        var $$rows = this.list._$$body.children();
