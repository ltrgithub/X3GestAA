...
exports.Table = helpers.defineClass(Table, null, {
    ...
    appendRowIndexCol: function(slot){
		...
		col.group.style.width =(this.builder.list.$capability.reorder)? "2.5em" : "3.5em";
        return col;
    },
    ...
    appendCapabilitiesCol: function(slot){
		var self = this;
        var col = self.cols.$capabilities = self._appendCol();
        col.slot = slot;
        var width = 0;
        col.titleCell.className = self.builder.gridCss.title;
        slot.appendChild(col.titleCell);
        
        if (self.builder.list.$capability) {
            ["reorder", "insert", "delete"].forEach(function($capability){
                if (self.builder.list.$capability[$capability]) {
                    width += 20;
                }
            });
        }
        if (!self.builder.list.$item.$isMenuRecordHidden) {
            width += 20;
        }
        width = 80; //hack temp
        if (width > 0) {
    ...
    makeTitleRow: function(){
        ...
        if (list.$item.$format == "vgrid") {
            ...
            self.orderCols.push(self.appendCapabilitiesCol(self.titleRow));
