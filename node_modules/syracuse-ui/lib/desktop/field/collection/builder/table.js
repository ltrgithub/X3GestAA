...
exports.Table = helpers.defineClass(Table, null, {
    ...
    appendRowIndexCol: function(slot){
        ...
        col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = (this.builder.list.$capability.reorder) ? "2.5em" : "3.5em";
        return col;
    },
    appendCardViewCol: function(slot){
        var col = this.cols.$cards = this._appendCol();
        col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = "1.8em";
        col.slot = slot;
        col.titleCell.className = this.builder.gridCss.title + " s-list-cards-cell";
    ...
    _appendContextMenusCol: function(slot){
        ...
        slot.appendChild(col.titleCell);
        col.$quickActions = {};
        col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = (col.width = 0) + "px";
        return col;
    },
        ...
        if (isRemoved) {
            if (!$quickAction) {
                ...
                if (col.$quickActions[$quickAction]) {
                    ...
                    col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = (col.width -= 20) + 4 + "px";
                }
            }
        }
        else {
            if (!col.$quickActions[$quickAction]) {
                ...
                col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = (col.width += 20) + 4 + "px"; //2padding for grid-cell
            }
        };
            },
    appendFieldCol: function(slot, $item, $field){
        ...
        if (width) {
            ...
            if ($width.$charWidth) {
                ...
                if ($width.$minChar) {
                    minWidth = $width.$charWidth * $width.$minChar;
                }
            }
            else {
            ...
            if (minWidth) {
                col.group.style.minWidth = minWidth + "em";
    ...
    makeTitleRow: function(){
        ...
        if (list.$item.$format == "vgrid") {
            ...
            if (list.$item.$isRowIndexVisible) {
                self.orderCols.push(this.appendRowIndexCol(self.builder._body.appendChild(document.createElement("tr"))));
                if (self.quickFilterRecord) {
                    self.quickFilterRecord.appendFlagCell(self.cols.$rowIndex.slot);
                }
            }
            if (!list.$item.$isMenuRecordHidden) {
                self.orderCols.push(self._appendContextMenusCol(self.builder._body.appendChild(document.createElement("tr"))));
                if (self.quickFilterRecord) {
                    self.quickFilterRecord.appendEmptyCell(self.cols.$contextMenus.slot);
                }
            }
            if (list.recordSelector.appendToTitle) {
                var col = self.appendRecordSelectorCol(self.builder._body.appendChild(document.createElement("tr")));
            ...
            list.parseItems(function($item, $field){
                var row = self.builder._body.appendChild(document.createElement("tr"));
            ...
            $$colgroup.empty();
            var col = self._colgroup.appendChild(document.createElement("col"));
            col.style.width = col.style.minWidth = "10em";
            if (self.quickFilterRecord) {
            ...
            if (self.builder.cardview && self.builder.cardview.$cardsPosition == "inline") {
                if (self.quickFilterRecord) {
                    self.quickFilterRecord.appendEmptyCell();
                }
            ...
            if (!list.$item.$isMenuRecordHidden) {
                ...
                self.orderCols.push(self._appendContextMenusCol(self.titleRow));
            }
