...
exports.HGridRecord = helpers.defineClass(HGridRecord, RecordArticle, {
    ...
    drawBox: function(){
		...
		if (self.list.$item.$isRowIndexVisible) {
            ...
            $(self._dataRow.appendChild(td)).text((self.list.$startIndex || 0) + self.$recordIndex);
        }
        ...
        if (self.list.recordSelector.appendToRecord) {
            var td = document.createElement("td");
            td.className = self.builder.gridCss.rowIndex;
            self._selectorCheck = self.list.recordSelector.appendToRecord(self._dataRow.appendChild(td));
        }
