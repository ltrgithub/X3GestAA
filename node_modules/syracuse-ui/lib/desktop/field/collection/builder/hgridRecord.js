...
exports.HGridRecord = helpers.defineClass(HGridRecord, Article, {
    ...
    remove: function(){
        this.$$item.remove();
        if (this.boxParent.builder.cardview) {
            this.boxParent.builder.cardview.onRecordRemove(this);
        }
        if (this.$$menuRow) {
            this.$$menuRow.remove();
        }
    ...
    drawBox: function(){
        ...
        if (list.$item.$isRowIndexVisible) {
            var td = document.createElement("td");
            td.className = gridCss.rowIndex;
            self.$$item.append($(td).text((list.$startIndex || 0) + self.$recordIndex));
        }
    ...
    highlightSelection: function(selected){
        ...
        if (this.boxParent.builder.cardview) {
            this.boxParent.builder.cardview.onRecordHighlightSelection(this, selected);
        }
