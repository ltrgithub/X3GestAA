...
exports.Builder = helpers.defineClass(HGridBuilder, RawListBuilder, {
    ...
    showAlphaPager: function(show){
        ...
        if (show) {
            if (!self._alphaPager) {
                ...
                self.list._topbar.appendChild(self._alphaPager);
    ...
    makeColGroup: function(){
        ...
        if (self.list.recordSelector.appendToRecord) {
            ...
            self.colgroup.$recordSelector.style.width = self.list.recordSelector.getWidth();
        }
    ...
    makeTitleRow: function(){
        ...
        titleRow.style.display = this.list.$item.$isTitleRowHidden ? "none" : "";
        var th = document.createElement("th");
        th.className = self.gridCss.title;
        self.$columnsCount++;
        titleRow.appendChild(th); //capabilities
        if (self.cardview) {
            self.cardview.onMakeTitleRow(titleRow);
        }
        if (self.list.recordSelector.appendToTitle) {
            self.$columnsCount++;
            var th = document.createElement("th");
            th.className = self.gridCss.title;
            self.list.recordSelector.appendToTitle(titleRow.appendChild(th));
        }
        if (self.list.$item.$isRowIndexVisible) {
            self.$columnsCount++;
            var td = document.createElement("th");
            td.className = self.gridCss.title;
        ...
        self.list.parseItems(function($item, $field){
            ...
            if ($field.$capability.sort) {
                ...
                title.className = self.list.$skin + "-title-label";
            }
            var text = self.list.getLocalizedTitle($field.$title || "");
            $(title).text(text);
        ...
        if (isNew) {
            self._header.appendChild(titleRow);
        }
        
        
        /*   var topBarRow = document.createElement("tr");
         var td = topBarRow.appendChild(document.createElement("td"));
         td.setAttribute("colspan", self.$columnsCount);
         td.appendChild(self.list.$$topbar[0]);
         self._header.insertBefore(topBarRow, titleRow);*/
    },
    ...
    _createTable: function(owner){
        owner._table = document.createElement("table");
        owner._table.style.display = "none";
        owner._table.className = owner.list.$skin;
        owner._table.setAttribute("cellspacing", "0");
        owner._table.setAttribute("cellpadding", "0");
        if (this.list.$item.$fitContainer !== false) {
            owner._table.style.width = "100%";
        }
        owner._table.appendChild(owner._tableColgroup = document.createElement("colgroup"));
        owner._header = owner._table.appendChild(document.createElement("thead"))
    },
    _drawDataList: function(){
        this._createTable(this);
        this.list._$$header = $(this._header);
