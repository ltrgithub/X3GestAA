"use strict";
var d3 = require("syracuse-visual-process/deps/d3");
    ...
    function getHexadecimalColor(str){
        ...
        return hex.toUpperCase();
         ...
         function update(ggg, forme) {
                  ...
                  var curve = ggg.selectAll("path.curve")
                      .data(getCurve);
                  ...
                  curve.attr('stroke', 
                                    function(){
                                            if  (forme.xstrokeProps.xtype==='none' || forme.xstrokeProps.xtype==='nostroke')
                                                return 'none' ;
                                            else
                ...
                curve.attr('fill',function(){if (forme.xfillProps.xtype==='nofill')
                    return 'none';
        ...
        .enter().append('div').attr('class','apishape').attr("style",function(d){return 'position:absolute;width:100%;height:100%'+d.xlevel;}); //.classed('apishape') ;
        var svg=divs.append('svg') ;
        //selectAll("svg").data( formesListe[gn].xpropsArr,function(d){if (d.xshapeType==='apishape' || d.xshapeType==='beziershape') return d ;}).enter().append('svg') ;
        svg.attr("width", 900).attr("height", 900).attr('id',function(d){return d.uniqueID}) ;
        svg.each(function(d,idx){
            appendShape(d,svg[0][idx])
        }) ; 
        //svg.each(appendShape);

        var edge=d3.select($$container[0]).selectAll("svg line, svg rect, svg ellipse, svg line"); //.selectAll();
                            edge.attr('stroke', 
                                    function(d){
                                ...
                                edge.attr('fill',function(d){if (d.xfillProps.xtype==='nofill')
                    return 'none';
        ...
        img.attr("src",function(d){return '/syracuse-ui/themes/desktop/sage/process/images/'+getBibName(d.bibid)+'/'+d.eltid+'.png';}) //
        .attr('alt','float:left');
		}
