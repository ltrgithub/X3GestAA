...
exports.ProcessPlayer = helpers.defineClass(ProcessPlayer, null, {
	...
	show: function() {
		...
		function appendShape(xprop, el, acts) {
			...
			case 'fileshape':
				...
				if (Math.atan2(xprop.xanchors[1].y - xprop.xanchors[0].y, xprop.xanchors[1].x - xprop.xanchors[0].x) != 0) {
					//there is a rotation
					shape.attr('x', xprop.xcenter.x - Math.max(xprop.xsize.xwidth, xprop.xsize.xheight) / 2);
					shape.attr('y', xprop.xcenter.y - Math.max(xprop.xsize.xwidth, xprop.xsize.xheight) / 2);
					shape.attr('width', Math.max(xprop.xsize.xwidth, xprop.xsize.xheight));
					shape.attr('height', Math.max(xprop.xsize.xwidth, xprop.xsize.xheight));
					shape.attr('transform', getRotate(xprop));
				} else {
					//No rotation
					shape.attr('width', function(xprop) {
						var xcoords = [];
						for (var j = 0; j < 4; j++) {
					...
					shape.attr('height', function(xprop) {
						var i;
						var ycoords = [];
						for (i = 0; i < 4; i++) {
					...
					shape.attr('x', function(xprop) {
						var i;
						var xcoords = [];
						for (i = 0; i < 4; i++)
						xcoords.push(xprop.xanchors[i].x);
						xcoords.sort(compare_int_func);
						return (xcoords[0] + 'px');
					});
				...
				links.on("click", function(xprop) {
					document.controller.openPage(self.field.boxParent.getArticle(), constructLinksObj(xprop), null);
				});
				break;
			}

			shape.style('display', (xprop.xactionProps.targ && isHidden(xprop.xactionProps.targ[0], actions)) //
			? 'none' : 'bloc');
		}

		function getRotate(xprop) {
			var anglerad = Math.atan2(xprop.xanchors[1].y - xprop.xanchors[0].y, xprop.xanchors[1].x - xprop.xanchors[0].x);
		...
		function getHRef(xprop) {
			...
			+ getBibFamSubpath(xprop.bibid) //
			+ getBibCatSubpath(xprop.bibid) + getBibName(xprop.bibid) + '/' + xprop.eltid;
			if (xprop.xshapeType == 'clipshape') src += '.png';
			return src;
		}

		function click(d) {
			function isClickTarget(xprop) {
			...
			var typeact = 'none';
			var targets = svgplot.selectAll('g').selectAll('*').filter(isClickTarget);
			var targettexts=d3.select($$container[0]).selectAll('div.text').filter(isClickTarget);
			...
			case 'shoallpopmenu':
				...
				targettexts.style('display', 'block');
				break;
			case 'hid':
				targets.style('display', 'none');
				targettexts.style('display', 'none');
				break;
			case 'shohid':
				...
				targettexts.style('display', function() {
					if (this.style.display == 'none' || this.style.display == "") return 'block';
					else return 'none'
				});
				break;
		...
		function mouseover(d) {
			...
			function isMouseOverTarget(xprop) {
				if (xprop.xactionProps && xprop.xactionProps.targ) {
			...
			var targets = svgplot.selectAll('g').selectAll('*').filter(isMouseOverTarget);
			var targettexts=d3.select($$container[0]).selectAll('div.text').filter(isMouseOverTarget);
		...
		var svgplot = d3.select($$container[0]).append('svg');
		...
		for (var gn = 0, sizeG = layers.length; gn < sizeG; gn++) {
			...
			textparent.each(function(d, idx) {
				appendText(d, $$container[0],self.process.acts.a);
			});
			...
			if (self.process.acts.a) {
				...
				mouseoverActs.on('mouseleave', mouseleave);
			}
...
function appendText(xprop, el,actions) {
	var divtext = d3.select(el).append('div').attr('class','text').attr('id', xprop.uniqueID + 'p');
	divtext.datum(xprop) ;
	...
	divtext.style('word-spacing', xprop.xtextFormat.kerning) ;
	//.style('display', xprop.xtextFormat.display) //
	divtext.style('display', (xprop.xactionProps.targ && isHidden(xprop.xactionProps.targ[0], actions)) //
			? 'none' : 'bloc');
	divtext.style('letter-spacing', xprop.xtextFormat.letterSpacing + 'px') //
