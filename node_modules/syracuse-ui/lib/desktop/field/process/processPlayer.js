       ...
       var  bezier = {} ;
		 	...
		 	var targets=svgplot.selectAll('g').selectAll('*').filter(function(forme){
		 		if (forme.xactionProps && forme.xactionProps.targ ){
		 			for (var j=0;j<forme.xactionProps.targ.length;j++){
		 				if  (forme.xactionProps.targ[j] ==d.xactionProps.trig[0])
		 			return true ;
        }			
								}
			return false ;}) ;
        ...
        document.controller.openPage(self.field.boxParent.getArticle(), constructLinksObj(d), null);
	      ...
	      var clickActs=gforme.filter(function(forme){
	          return ( forme.xactionProps.trig!=null && isClickAct(forme.xactionProps.trig[0],self.process.acts.a));
           }) ; 
	      clickActs.on('click', click) ;
        	
	      var mouseoverActs=gforme.filter(function(forme){
	          return ( forme.xactionProps.trig!=null && isMouseoverAct(forme.xactionProps.trig[0],self.process.acts.a));
	         }) ; 
	      mouseoverActs.on('mouseover', mouseover) ;


	      /*function(d){
	      	var actionId=d.xactionProps.trig ;

	      	var targetId=findActionTarget(actionId) ;
            ...
            if (d.xfillProps && d.xfillProps.xtype==='gradientfill' //
            	|| d.xstrokeProps && (d.xstrokeProps.xstart.xtype!='none' || d.xstrokeProps.xend.xtype!='none') )
                return 1 ;  });
        var defs=gradientsvg.append('defs') ;
        defs.each(function(d,idx){
            appendGradientAndMarkers(d,defs[0][idx]) ;
        });
...
function calculDegrade(alpha) {
 ...
 function isHidden(actId,acts){
  ...
  function isMouseoverAct(actId, acts){
   for (var k=0;k<acts.length;k++){
     if (acts[k].id==actId && (acts[k].type=='shotip' )) 
      return true ;
    }
    return false ;
  }
