	...
	show: function() {
       ...
       var  bezier = {} ;
         ...
         function appendShape(forme,el){
            ...
            switch (forme.xshapeType){
                    ...
                    case 'clipshape' :
	                    ...
	                    /*.attr('onclick', 
	                    	function(d){return 'window.location="'+constructURL(d)+'"'});*/
                break ;
            }
        }    
			...
			function constructLinksObj(forme){
			    ...
			    switch(forme.xlinkProps.xtype){
			                ...
			                obj.$url=  'http://'+forme.xlinkProps.xcode ;  
			                return obj ;
        }
			}
        ...
        function appendText(forme,el) {
			...
			function appendGradientAndMarkers(forme,defs) {
			           ...
			           switch (markertype){
			           	...
			           	case 'disc1' :
			           	var marker=d3.select(defs).append('marker').attr('id','startMarker'+forme.uniqueID) ;
			           	marker.attr('viewBox','0 0 10 10').attr('refX',5).attr('refY',6) ;
			           	marker.append('circle').attr('cx',6).attr('cy',6).attr('r',5) ;
			           	marker.attr('fill',function(){
			           	if (forme.xstrokeProps.xstart.xfilled===true) //
			           		return getHexadecimalColor(forme.xstrokeProps.xcolor) ;
			           	else return 'none' ; }) ; 
                            break ;
			           	case 'none' :
			           ...
			           var markertype=forme.xstrokeProps.xend.xtype ;
			           switch (markertype){
			           	case 'arrow1' :
			           		var marker=d3.select(defs).append('marker').attr('id','endMarker'+forme.uniqueID) ;
			           			marker.append('path').attr('d','M 0 0 L 10 5 L 0 10 z') ;
			           			break ;
			           	case 'arrow2' :
			           	var marker=d3.select(defs).append('marker').attr('id','endMarker'+forme.uniqueID) ;
        ...
        break ;
			           	...
			           	var marker=d3.select(defs).append('marker').attr('id','endMarker'+forme.uniqueID) ;
			           	marker.append('circle').attr('cx',6).attr('cy',6).attr('r',5) //
			           	.attr('fill',function(){if (forme.xstrokeProps.xend.xfilled===true)
			           	return getHexadecimalColor(forme.xstrokeProps.xcolor) ;
			           	else return 'none' ; }) ;   
        }  
    }    
        ...
        document.controller.openPage(self.field.boxParent.getArticle(), constructLinksObj(d), null);
            ...
            if (d.xfillProps!=null && d.xfillProps.xtype==='gradientfill' //
            	|| d.xstrokeProps.xstart.xtype!='none' || d.xstrokeProps.xend.xtype!='none') 
                return 1 ;  });
