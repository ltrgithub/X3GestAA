		...
		if (xprop.xshapeType==='apishape' || xprop.xshapeType==='beziershape'){
			...
			var ycoords =[] ;
		      ...
		      ycoords.sort(compare_float_func);
				...
				shape.attr("x1",xprop.xanchors[0].x).attr("y1",xprop.xanchors[0].y).attr("x2",xprop.xanchors[2].x).attr("y2",xprop.xanchors[2].y) ;
				//}
				...
				case "K_API_RECT":
				shape=$('<rect>') ;
				var apirotation=(Math.atan2(xprop.xanchors[1].y - xprop.xanchors[0].y, //
				...
				.attr("height",Math.abs(xprop.xanchors[3].y - xprop.xanchors[0].y)) ;
					...
					break;
				case "K_API_ELLIPSE":
				var apirotation=(Math.atan2(xprop.xanchors[2].y - xprop.xanchors[0].y, //
...
function createAction(action, index, array){
    ...
    for (var gn = 0, sizeG = layers.length; gn < sizeG; gn++) {
		...
		if (group.xpropsArr[i].xtext!="" //
			...
			var ttext='';
			if (group.xpropsArr[i].xtext!="")
				ttext=createText(group.xpropsArr[i]) ;
			//var svggroup=$('<g>'+shapedescr+ttext +'</g>');

			if (rotation && (group.xpropsArr[i].xshapeType!="apishape" || group.xpropsArr[i].xdrawBehaviorCode!="K_API_LINE"))
			...
			result+=shapedescr ;
			result+=ttext ;
			//svggroup.attr('transform',getRotate(group.xpropsArr[i]));
			//result+=svggroup[0].outerHTML ; //ne marche pas pour l'élément image 
		}
		else
			result+=shapedescr ;
		result+='</g>';
...
function createText(xprop ) {
	...
	.attr('height', xprop.xcaptionSize.xheight/*'100%'*/) ;
