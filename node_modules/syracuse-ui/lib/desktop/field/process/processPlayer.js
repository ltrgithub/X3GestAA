   ...
   case 'fileshape':
    ...
    && (Math.round(forme.xanchors[0].x) != Math.round(forme.xanchors[1].x))) {
     ...
     shape.attr('width', function(d) {
      return d.xsize.xwidth
     });
     shape.attr('height', function(d) {
      return d.xsize.xheight
     });
     shape.attr('transform', function(d) {
      ...
      if (sinx >= -1 && sinx <= 1) angle = (Math.asin(sinx) / Math.PI) * 180;
      else if (cosx >= -1 && cos <= 1) angle = (Math.acos(cosx) / Math.PI) * 180;
...
/*img.attr('preserveAspectRatio', function(d) {
  ...
  for (var gn = 0, sizeG = formesListe.length; gn < sizeG; gn++) {
   ...
   var linesWithMarkers = d3.select($$container[0]).selectAll('svg.shape').filter(function(d, i) {
    return (d.xstrokeProps && (d.xstrokeProps.xstart.xtype != 'none' //
     || d.xstrokeProps.xend.xtype != 'none'));
   });
    linesWithMarkers.each(
      function(d, idx) {
    //appendLineMarker(d, linesWithMarkers[0][idx] );
   });
...
function appendLineMarker(forme, parent ) {
 var markertype=forme.xstrokeProps.xstart.xtype ;
 var filled =forme.xstrokeProps.xstart.xfilled;
 var startend= 'start' ; //TODO
 if (markertype != 'none' && markertype != 'noarrow') {
  ...
  var marker = d3.select(defs).append('marker').attr('id', function() {
   if (startend == 'start') return 'startMarker' + forme.uniqueID;
   else if (startend == 'end') return 'endMarker' + forme.uniqueID;
  });
...
/*marker.attr('viewBox', function() {
   if (markertype == 'arr1' || markertype == 'arr2') {
    if (startend == 'end') return '0 0 ' + sz + ' ' + sz;
    else if (startend == 'start') return ((-sz) + ' ' + (-sz) + ' ' + sz * 2 + ' ' + sz * 2);
   } else if (markertype == 'disc1') {
    if (startend == 'end') return '0 0 ' + sz + ' ' + sz;
    else if (startend == 'start') return (-Math.round(sz / 2) + ' ' + (-Math.round(sz / 2)) + ' ' + sz * 2 + ' ' + sz * 2);

   }
  });
  marker.attr('refX', function() {
   if (markertype == 'arr1' || markertype == 'arr2') {
    if (startend == 'end') return 0;
    else if (startend == 'start') return sz;
   } else if (markertype == 'disc1') {
    if (startend == 'end') return 0;
    else if (startend == 'start') return Math.round(sz / 2);
   }
  ...
  case 'arr1':
   ...
   markershape = marker.append('path').attr('d', function() {
    if (startend == 'end') return 'M 0 0 L ' //
    + sz + ' ' //
    + Math.round(sz / 2) + ' L 0' + ' ' + sz + 'z';
    else if (startend == 'start') return ('M ' + sz + ' 0 L 0 ' + Math.round(sz / 2) + 'L ' + sz + ' ' + sz);
   });
   markershape.attr('stroke', function() {
  ...
  case 'diamond1':
   ...
   markershape.attr('fill', function() {
    if (forme.xstrokeProps.xstart.xfilled) return getHexadecimalColor(forme.xstrokeProps.xcolor);
    else return 'none';
   });
   break;
  ...
  case 'disc1':
   var x1, x2, y1, y2, angle, r, cx, cy, newx2, newy2;
   x1 = forme.xanchors[0].x;
   x2 = forme.xanchors[1].x;
   y1 = forme.xanchors[0].y;
   y2 = forme.xanchors[1].y;
   angle = Math.atan2(y2 - y1, x2 - x1);
   r = forme.xstrokeProps.xthickness + 3;
   cx = x2 - Math.cos(angle) * r;
   cy = y2 - Math.sin(angle) * r;
   newx2 = x2 - Math.cos(angle) * r * 2;
   newy2 = y2 - Math.sin(angle) * r * 2;
   markershape = marker.append('circle').attr('cx', cx).attr('cy', cy).attr('r', r);
   markershape.attr('stroke', function() {
    return getHexadecimalColor(forme.xstrokeProps.xcolor)
   });
   markershape.attr('fill', function() {
...
/*markershape = marker.append('circle').attr('cx', function() {
    return Math.round(sz / 2);
   }).attr('cy', function() {
    return sz;
   }).attr('r', function() {
    return Math.round(sz / 4) * 3;
   });
   markershape.attr('stroke', function() {
    return getHexadecimalColor(forme.xstrokeProps.xcolor)
   }); //markershape.attr('stroke-width',function(){ return forme.xstrokeProps.xthickness} ) ;
