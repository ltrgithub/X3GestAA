                      ...
                      case 'fileshape' :
	                    var g=d3.select(el) ;
	                        ...
	                        return d.uniqueID;
			                    ...
			                    if ((Math.round(forme.xanchors[0].x)!=Math.round(forme.xanchors[3].x)) //
			                    		&& (Math.round(forme.xanchors[0].x)!=Math.round(forme.xanchors[1].x))){

                        //there is a rotation
                       shape.attr('x', function(d){ return d.xanchors[0].x} ) ;
                       shape.attr('y', function(d){ return d.xanchors[0].y}) ;
                        shape.attr('width', function(d){  return d.xsize.xwidth}) ;
                        shape.attr('height' , function(d){  return d.xsize.xheight}) ;
			                    	...
			                    	var sinx=(xcoords[1]-xcoords[0])/d.xsize.xheight ;
			                    	var cosx=(ycoords[1]-ycoords[0])/d.xsize.xwidth ;

			                    	if (sinx>=-1 && sinx<=1)
			                    		angle=(Math.asin(sinx)/Math.PI)*180 ;
			                    	else if (cosx>=-1 && cos<=1)
			                    		angle=(Math.acos(cosx)/Math.PI)*180 ;
			                    	return 'rotate('+ angle+','+d.xcenter.x+',' +d.xcenter.y +')';
			                    });
			                    }else{
                        //No rotation
	                    	...
	                    	ycoords.push(d.xanchors[i].y) ;
			                    	ycoords.sort(compare_int_func);
			                    	return (ycoords[0]/* d.xanchors[0].y*/ +'px') ;
	                    }) ;
	                    	...
	                    	return (xcoords[0] +'px') ;
			                    ...
			                    shape.attr('z-index',function(d){return d.xlevel ;}) ;
			                    shape.style('filter',function(d){return "alpha(opacity="+d.alpha+")" ;}) ;
			                    shape.style('-moz-opacity',function(d){ return d.alpha+"/100" ;}) ;
			                    shape.style('-khtml-opacity',function(d){return d.alpha+"/100" ;}) ;
			                    shape.style('opacity',function(d){return d.alpha+"/100" ;}) ;
                       ...
                       +getBibCatSubpath(d.bibid)+getBibName(d.bibid)+'/'+d.eltid ;
                        if (d.xshapeType=='clipshape')
			                       src +='.png';
                       return  src ; }) ;
			                    var links=shape.filter(function(forme){return ( forme.xlinkProps.xtype!='none');}) ;//
        ...
        document.controller.openPage(self.field.boxParent.getArticle(), constructLinksObj(d), null);
                ...
                break ;
		            ...
		            shape.style('display',function(forme){
		            	if (forme.xactionProps.targ && isHidden(forme.xactionProps.targ[0],actions))
		            		return 'none' ;
		            	else return 'bloc' ;}) ;
			}
 ...
 function click(d) {
              ...
              if (forme.xactionProps && forme.xactionProps.targ ){
               ...
               for (var j=0;j<forme.xactionProps.targ.length;j++){
                 if  (forme.xactionProps.targ[j] == clicktrig[0] )
                   return true ;
                  }   
                }
               return false ;}) ;
      ...
      var mouseovertrig= d.xactionProps.trig.filter( function(el){
          for (var i=0;i<mouseoveractions.length;i++){
           ...
           return false ;
		 		...
		 		for (var j=0;j<forme.xactionProps.targ.length;j++){
                 if  (forme.xactionProps.targ[j] == mouseovertrig[0] )
                   return true ;
                  }   
                }
               return false ;}) ;
   ...
   gforme.attr('id',function(d){return d.uniqueID});
   gforme.attr("style",function(d){return 'position:absolute;width:100%;height:100%;z-index:'+d.xlevel+';';});
   gforme.each(function(d,idx){appendShape(d,gforme[0][idx],self.process.acts.a);}) ; 

		var links=gforme.filter(function(forme){return ( forme.xlinkProps.xtype!='none');}) ; 
  ...
  clickactions=self.process.acts.a.filter(function(el){ return (el.type =='shopopmenu' //
   || el.type =='sho' || el.type =='shohid' || el.type =='hid')}) ;

  mouseoveractions = self.process.acts.a.filter(function(el){ return (el.type =='shotip' )}) ;
  ...
  mouseoverActs.on('mouseover', mouseover) ;
       ...
       return (d.xfillProps && d.xfillProps.xtype==='gradientfill' //
       	|| d.xstrokeProps && (d.xstrokeProps.xstart.xtype!='none' //
        || d.xstrokeProps.xend.xtype!='none') ) ;  });
  var defs=gradientsvg.append('defs') ;
  defs.each(function(d,idx){appendGradientAndMarkers(d,defs[0][idx]) ;});
   ...
   edge.attr('stroke-width',function(d){return d.xstrokeProps.xthickness} ) ;
  ...
  edge.attr('marker-end',function(d){
   if (d.xstrokeProps.xend && d.xstrokeProps.xend.xtype!='none')
   return 'url(#endMarker'+d.uniqueID+')';
  else 
   return '';});
  edge.attr('marker-start',function(d){
    if (d.xstrokeProps.xstart && d.xstrokeProps.xstart.xtype!='none')
   return 'url(#startMarker'+d.uniqueID+')';
  else 
   return '';});

  var textparent = gforme.filter(function(d, i) {  return d.xtext!='';  });
  textparent.each(function(d,idx){appendText(d, $$container[0]) ;});                              
		}
		return this;
	},
	clear: function() {
 ...
 function compare_int_func(a, b) {
  ...
  function constructLinksObj(forme){
         ...
         switch(forme.xlinkProps.xtype){
          ...
          function appendLineMarker(defs,markertype,forme, filled, startend){
          if (markertype!='none'){
            var sz=forme.xstrokeProps.xthickness ;
          var marker = d3.select(defs).append('marker').attr('id', function(){
           if (startend=='start')
           return'startMarker' + forme.uniqueID ;
          else if  (startend=='end')
           return'endMarker' + forme.uniqueID ;
          });
          //marker.attr('viewBox',function(){ '0 0 '+forme.xstrokeProps.xthickness+' '+forme.xstrokeProps.xthickness;}).attr('refX', 0) //
          //.attr('refY', function(){return Math.round(forme.xstrokeProps.xthickness/2);});
          
          marker.attr('viewBox',function(){
           if (markertype=='arr1' || markertype=='arr2'){
           if (startend=='end')
            return '0 0 '+sz+' '+ sz ;
          else if (startend=='start')
            return ((-sz)+' '+(-sz)+' '+sz*2+' '+sz*2) ;
            }else if (markertype=='disc1'){
             if (startend=='end')
            return '0 0 '+sz+' '+ sz ;
          else if (startend=='start')
            return( -Math.round(sz/2)+' '+(-Math.round(sz/2))+' '+sz*2+' '+sz*2) ;
          
          }
          }) ;
          marker.attr('refX', function(){
            if (markertype=='arr1' || markertype=='arr2'){
            if (startend=='end')
             return 0 ;
            else if (startend=='start')
          ...
          case 'arr1':
           ...
           markershape=marker.append('path').attr('d', function(){
           if (startend=='end')
             return 'M 0 0 L ' //
            +sz+' ' //
            +Math.round(sz/2)+' L 0'+' '+sz+'z' ;
            else if (startend=='start')
             return ('M ' + sz+ ' 0 L 0 '+Math.round(sz/2)+'L '+sz+ ' '+sz );
           });
           markershape.attr('stroke',function(){return getHexadecimalColor(forme.xstrokeProps.xcolor)} ) ;
           markershape.attr('fill', function() {
            if (filled) 
            return getHexadecimalColor(forme.xstrokeProps.xcolor);
            else return 'none';
           });
           break;
          case 'arr2':
           markershape=marker.append('path').attr('d', function(){return 'M 0 0 L ' //
            +forme.xstrokeProps.xthickness+' '+Math.round(forme.xstrokeProps.xthickness/2)+' L 0'+' '+forme.xstrokeProps.xthickness});
           markershape.attr('stroke',function(){return getHexadecimalColor(forme.xstrokeProps.xcolor)} ) ;
            //markershape.attr('stroke-width',function(){ return forme.xstrokeProps.xthickness} ) ;
          markershape.attr('fill','none');
           break;
          case 'diamond1':
          markershape.attr('stroke',function(){return getHexadecimalColor(forme.xstrokeProps.xcolor)} ) ;
          
           markershape.attr('fill', function() {
            if (forme.xstrokeProps.xstart.xfilled ) 
            return getHexadecimalColor(forme.xstrokeProps.xcolor);
            else return 'none';
           });
           break;
          ...
          case 'disc1':
           markershape=marker.append('circle').attr('cx', function(){return Math.round(sz/2) ;}).attr('cy', function(){return sz ;}).attr('r', function(){return Math.round(sz/4)*3 ;}) ; 
           markershape.attr('stroke',function(){return getHexadecimalColor(forme.xstrokeProps.xcolor)} ) ;
           /*markershape.attr('stroke-width',function(){ return forme.xstrokeProps.xthickness} ) ;*/
           markershape.attr('fill', function() {
            if (forme.xstrokeProps.xstart.xfilled ) 
     ...
     function appendGradientAndMarkers(forme, defs) {
