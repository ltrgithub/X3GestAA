...
/*
 var importPopup = '<div id="import" class="s-process-fenetreImportExport s-process-hidden"> \
	...
	show: function() {
		...
		function update(shape, xprop) {
			shape.attr('stroke', function() {
				if (xprop.xstrokeProps.xtype === 'none' //
				|| xprop.xstrokeProps.xtype === 'nostroke') //
				return 'none';
				else return getHexadecimalColor(xprop.xstrokeProps.xcolor);
			});
			shape.attr('stroke-opacity', function() {
				return xprop.xstrokeProps.xalpha / 100;
			}) //
			...
			shape.attr('stroke-dasharray', function() {
		...
		function appendShape(xprop, el, acts) {
			...
			case 'apishape':
				...
				case "K_API_RECT":
					...
					shape.attr("width", Math.abs(xprop.xanchors[1].x - xprop.xanchors[0].x)) //
				...
				case "K_API_ELLIPSE":
					...
					shape.attr("cy", xprop.xanchors[0].y) //
...
.attr('y',shape.attr('y')).attr('height', shape.attr('height')).attr('width',shape.attr('width')) ;
  ...
  d3.select(el.parentNode).append('circle').attr('fill', 'green').attr('cx',xprop.xcenter.x) //
  .attr('cy',xprop.xcenter.y).attr('r',2) ;
  */
...
/*appendLineMarker(defs, xprop.xstrokeProps.xstart.xtype, xprop, xprop.xstrokeProps.xstart.xfilled, 'start');
 appendLineMarker(defs, xprop.xstrokeProps.xend.xtype, xprop, xprop.xstrokeProps.xend.xfilled, 'end');
 */
}