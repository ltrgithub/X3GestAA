...
if (Math.atan2(forme.xanchors[1].y-forme.xanchors[0].y,forme.xanchors[1].x-forme.xanchors[0].x)!=0)    
{
    ...
    var xcoords = [];
      for (j = 0; j < 4; j++) {
       xcoords.push(forme.xanchors[j].x);
      }
      xcoords.sort(function(a,b){return a-b});
      var i;
      var ycoords = [];
      for (i = 0; i < 4; i++)
      ycoords.push(forme.xanchors[i].y);
     ...
     shape.attr('transform', function(d) {
      ...
      return 'rotate(' + angle + ',' + forme.xcenter.x + ',' + forme.xcenter.y + ')' ;
     });
...
function appendLineMarker(forme, parent,markertype,filled,startend ) {
  ...
  var x1,x2,y1,y2, angle, anglePoint ;
   ...
   anglePoint= Math.round(Math.PI/2)-angle ;
  switch (markertype) {
  case 'arr1':
  case 'arr2':
  case 'diamond1' :
 ...
 /* }else if (markertype=='arr2'){
      ...
      centerx=x2-Math.cos(angle)*Math.round(sz/2) ;
      centery=y2-Math.sin(angle)*Math.round(sz/2) ;
      point1x=centerx+Math.cos(anglePoint)*Math.round(sz/2) ;
      point1y=centery-Math.sin(anglePoint)*Math.round(sz/2) ;
      point2x=x2-Math.cos(angle)*sz ;
      point2y=y2-Math.sin(angle)*sz ;
      point3x=centerx-Math.cos(anglePoint)*Math.round(sz/2)  ;
      point3y=centery+Math.sin(anglePoint)*Math.round(sz/2)  ;
      point4x=x2 ;
      point4y=y2 ;
      endx=centerx ;
      endy=centery ;
    ...
    if (markertype=='arr1'){
      ...
      endx=x1+(Math.cos(angle)*Math.round(forme.xstrokeProps.xthickness/2 ));
      endy=y1+(Math.sin(angle)*Math.round(forme.xstrokeProps.xthickness/2 )) ;
    }
  }else if (markertype=='diamond1'){
      
      centerx=x1+Math.cos(angle)*Math.round(sz/2) ;
      centery=y1+Math.sin(angle)*Math.round(sz/2) ;
      point1x=centerx-Math.cos(anglePoint)*Math.round(sz/2) ;
      point1y=centery+Math.sin(anglePoint)*Math.round(sz/2) ;
   ...
   marker.attr('stroke-width',function(){ return forme.xstrokeProps.xthickness} ) ;
