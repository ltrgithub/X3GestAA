...
require('syracuse-ui/deps/jquery.svg');
require('syracuse-ui/deps/jquery.svgdom');
	...
	show: function() {
		...
		function createShape(xprop) {
			...
			case 'apishape':
				...
				case "K_API_LINE":
				shape=$('<line>').attr("x1",xprop.xanchors[0].x).attr("y1",xprop.xanchors[0].y).attr("x2",xprop.xanchors[2].x).attr("y2",xprop.xanchors[2].y) ;
				...
				if (xprop.xstrokeProps.xstart){
					...
					//updateLine(xprop,shape,xprop.xstrokeProps.xstart.xtype,"start") ;
				if (xprop.xstrokeProps.xend){
					...
					//updateLine(xprop,shape,xprop.xstrokeProps.xend.xtype,"end") ;
					}
				}
					break;
				case "K_API_RECT":
				shape=$('<rect>').attr("x",xprop.xanchors[0].x).attr("y",xprop.xanchors[0].y)//
				.attr("width",Math.abs(xprop.xanchors[1].x - xprop.xanchors[0].x))//
				.attr("height",Math.abs(xprop.xanchors[3].y - xprop.xanchors[0].y)) ;
				
					break;
				case "K_API_ELLIPSE":
				shape=$('<ellipse>').attr('cx',xprop.xanchors[0].x + Math.abs(xprop.xanchors[1].x - xprop.xanchors[0].x))//
			...
			case 'fileshape':
				...
				shape= $('<image>') ;//parent.append('image');
			...
			/*if (xprop.xtext !== ''){
				...
				var old=(parent.outerHTML)?(parent.outerHTML):'';
				parent.outerHTML=old+appendText(xprop); //(xprop,parent,svgplot) ;
			}*/
			//shape.attr('class', 'shape');
			//shape.attr('style','display:bloc"');
			/*shape.attr('style','display="' //
			+ (xprop.xactionProps.targ && isHidden(xprop.xactionProps.targ[0], actions)) //
			? 'none' : 'bloc' +'"');*/
			/*shape.appendTo(parent) ;

		if (xprop.xtext !== ''){
			parent.appendTo(svgplot) ;
		}*/
...
function createAction(action, index, array){
    ...
    for (var gn = 0, sizeG = layers.length; gn < sizeG; gn++) {
		...
		//appendGradient(group.xpropsArr[i],defs,svgplot) ;
		var shapedescr=createShape(group.xpropsArr[i]) ;
		if (group.xpropsArr[i].xtext!=""){
			var ttext=createText(group.xpropsArr[i]) ;
			var svggroup=$('<g>'+shapedescr+ttext +'</g>');
			result+=svggroup[0].outerHTML ;
		}
		else
			result+=shapedescr ;

		/*if (group.xpropsArr[i].xdrawBehaviorCode=='K_API_LINE'){
			var markertype, filled ;
			if (group.xpropsArr[i].xstrokeProps.xstart) {
				markertype = group.xpropsArr[i].xstrokeProps.xstart.xtype;
				filled = group.xpropsArr[i].xstrokeProps.xstart.xfilled;

				if (markertype != 'none' && markertype != 'noarrow') {
					result+=createLineMarker(group.xpropsArr[i],  markertype, filled, 'start');
				}
			}

		if (group.xpropsArr[i].xstrokeProps.xend) {
			markertype = group.xpropsArr[i].xstrokeProps.xend.xtype;
			filled = group.xpropsArr[i].xstrokeProps.xend.xfilled;

			if (markertype != 'none' && markertype != 'noarrow') {
				result+=createLineMarker(group.xpropsArr[i],  markertype, filled, 'end');
			}
		}
	}*/
...
function createText(xprop ) {
	...
	function replaceParagraph(index,el ){
		...
		if (index==0 || textformat.length==0) 
			$(el).replaceWith('<div style="text-align: '+alignement +';">'+$(el).html()+'</div>');

	}

	var foreignobj= $('<foreignObject>').attr('x',xprop.xcaptionPos.xleft + 'px').attr('y',xprop.xcaptionPos.xtop + 'px')//
	...
	var shadow='';
	if (xprop.xshadowProps.xtype==='global')
		shadow='text-shadow: silver 5px 5px 3px;';

	var stdiv="<div style='width:100%;margin-left:"+xprop.xtextFormat.leftMargin //
		 + "px; margin-right:"+xprop.xtextFormat.rightMargin +"px;" +shadow+ "'>" ;	
