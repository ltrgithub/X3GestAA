    ...
    $combo: {
        ...
        render: function(field){
            ...
            if (field.$isEditMode) {
                ...
                field.$item.$iconMode = "iconText";
                //$iconPath: "", //"charts",
                if (field.$item.$iconMode) {
                    field.$skinItem += " " + field.$skin + "-choice-list-item-icon";
                }
        ...
        fillList: function(field, filter){
            ...
            if (field.$item.$iconMode) {
                $iconHeight = field.$item.$iconHeight;
                $iconWidth = field.$item.$iconWidth;
            }
            field.$enum.forEach(function($enum, index){
                ...
                if (html) {
                    ...
                    link.className = field.$skinItem;
                    if (field.$item.$iconMode) {
                        var image = "/syracuse-ui/themes/desktop/images-general/" + "graphics/s-graph-" + "bar" + ".png";
                        link.style.backgroundImage = "url('" + image + "')";
                        link.style.backgroundRepeat = "no-repeat";
                        link.style.backgroundPosition = "0 top";
                        link.style.backgroundAttachment = "scroll";
                    }
...
exports.DesktopChoiceField = helpers.defineClass(DesktopChoiceField, ChoiceField, {
    applyDesignMetaData: function(metadata, onAuthoring){
        ...
        if (metadata.$format && metadata.$format != this.$item.$format) {
            this.formatAdpater.releaseMode(this);
    ...
    releaseMode: function(onDispose){
        if (this.formatAdpater) {
            this.formatAdpater.releaseMode(this, onDispose);
        }
