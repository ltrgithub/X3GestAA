...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
    ...
    render: function(){
        ...
        if (self.field.$item.$iconMode) {
            self.$iconSize = self.field.$item.$iconSize ||
            {
                $width: "32px",
                $height: "32px"
            };
        ...
        if (self.field.$isEditMode) {
            ...
            if (self.field.$item.$iconMode) {
                ...
                $$box.prepend(self.iconValue);
                self.iconValue.top = "2px";
                self.iconValue.left = "2px";
                var iconHeight = parseInt(self.$iconSize.$height.replace("px", ""));
                btn.style.top = Math.ceil(iconHeight / 3) + "px";
                input.style.paddingLeft = (parseInt(self.$iconSize.$width.replace("px", "")) + 8) + "px";
    ...
    setIcon: function(domImg, $value){
        domImg.style.backgroundImage = $value ? "url('" + this.$iconPath + $value + ".png')" : "";
        //domImg.style.backgroundImage = image ? "url('" + image + "')" : "";
    },
    createIcon: function($value, css){
        var img = document.createElement("div");
        img.className = css;
        img.style.width = this.$iconSize.$width;
        img.style.height = this.$iconSize.$height;
    ...
    renderLayout: function(filter){
        ...
        if (!isRowLayout) {
            $$cols = [];
            var div = document.createElement("div");
            div.className = "s-field-choice-cb-table";
            self._$$table = $(div).appendTo(self._$$list);
            for (var ii = 0, colCount = parseInt(self.field.$item.$choiceLayout); ii < colCount; ii++) {
                div = document.createElement("div");
                div.className = "s-field-choice-cb-col";
                $$cols[ii] = $(div).appendTo(self._$$table);
            }
        }
    ...
    _setMinWidth: function(){
        ((this._$$table) ? this._$$table : this._$$list).css("min-width", this.field.$$input.parent().width());
    },
