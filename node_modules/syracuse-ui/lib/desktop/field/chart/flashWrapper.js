...
/* Class properties */
Wrapper.prototype.fxBootStrap = "xstart_portal.swf";
...
Wrapper.prototype.fxgetctxmenu = function(){
     ...
     * Asynchronous reply			: Menus desc for the line
     */
        ...
        if (this.opts.legacyLinks || !links) {
            ...
            if (links) {
                ...
                _.each(links, function(link, idx){
                    mnuDesc[mnuDesc.length] = {"label": link.title, "linkId": this._getCanonicalLink(line, idx)};
                }, this);
...
Wrapper.prototype.fxOnGetdataDone = function(err, data, meta){
    if (err) 
        ...
        if (this.pendingData) 
            ...
            this._answerFxCommand(this._wrapFxDataInObj(this._processDataIn(data, meta)));
...
Wrapper.prototype.fxOnGetCtxMenuDone = function(err, mnuDesc){
    ...
    if (mnuDesc) {
        ...
        _.each(data, function(mnu){
            if (mnu.label && mnu.label.length > 0 && mnu.linkId) 
                reply += '<MI I="' + mnu.linkId + '"><![CDATA[' + mnu.label + debug + ']]></MI>';
        }, this);
    }
...
Wrapper.prototype.fxOnStdSvcDone = function(err, reply){
    ...
    if (this.opts.xmlNative) {
        ...
        if (reply.result) {
            ...
            if (reply.data) 
                xmlResult = this._processDataIn(data, null);
        if (xmlResult.length > 0) 
            this._answerFxCommand(this._wrapFxDataInObj(xmlResult));
        else 
            this.fxOnError({
...
Wrapper.prototype.setData = function(data, meta){
    ...
    if (data) {
        if (this.firstDataDone) 
            this._populateFxData(this._wrapFxDataInObj(this._processDataIn(data, meta)), this.curCmdCtx._cmd || "fxdataupdated");
        else {
            // Data retention... waiting for "fxgetinitialDataFlash" command
            this.pendingData = {"data": data || null, "meta": null};
        }
    }
...
Wrapper.prototype.setFrameSize = function(width, heigth){
    ...
    if (width || heigth) {
        var objReply = {"pwidth": width || null, "pheight": heigth || null};
...
Wrapper.prototype._initProps = function(){
    ...
    this.props.swf += "/" + this.fxBootStrap;
};
...
Wrapper.prototype._processDataIn = function(data, meta){
    if (this.opts.xmlNative) {
        ...
        return this['_' + this.type + '_DataJson2Xml'](data, meta);
    }
};
...
Wrapper.prototype._getGraphDef = function(data, rawData){
    ...
    if (data.$axes && data.$axes.length > 0 && data.$axes[0].$title && graphDef._metaData.title != data.$axes[0].$title) 
        graphDef._metaData.title += ' / ' + data.$axes[0].$title;
    ...
    if(graphDef._metaData.axis0FieldCode && data.$[graphDef._metaData.axis0FieldCode].$type == "application/x-reference" && data.$[graphDef._metaData.axis0FieldCode].$value) {
        graphDef._metaData.xRefAxis0FieldCode = data.$[graphDef._metaData.axis0FieldCode].$value.substr(1, match.length - 2);
    }
    ...
    if (measures) {
        ...
        _.each(restraintMeasures, function(measure, key){
            ...
            graphDef._metaData.measuresDefinition[code] = {
                "property": code,
                "title": measure.$title || data.$[code].$title || "???",
                "definition": measure,
                "xRefCode": null
            };
            // Reference field?
            if(data.$[code].$type == "application/x-reference" && data.$[code].$value) {
                graphDef._metaData.measuresDefinition[code].xRefCode = data.$[code].$value.substr(1, match.length - 2);
            }
        });
    }
    ...
    if (graphDef._metaData.axis0FieldCode && data.$[graphDef._metaData.axis0FieldCode].$thumb && data.$[graphDef._metaData.axis0FieldCode].$thumb.$links) {
        _.each(data.$[graphDef._metaData.axis0FieldCode].$thumb.$links, function(link, id){
            graphDef._metaData.links.push(this._getSyraLinkDesc("$links", id, link));
        }, this);
    }
    if (graphDef._metaData.xRefAxis0FieldCode && data.$[graphDef._metaData.axis0FieldCode].$links) {
        _.each(data.$[graphDef._metaData.axis0FieldCode].$links, function(link, id){
            graphDef._metaData.links.push(this._getSyraLinkDesc("$links", id, link));
        }, this);
    }    
    ...
    if (!graphDef._metaData.axis0FieldCode || !data.$ || _.isEmpty(graphDef._metaData.measuresDefinition)) 
...
Wrapper.prototype._getLinkObj = function(link){
    ...
    if (!this.opts.legacyLinks && link.linkId && links) {
...
Wrapper.prototype._getSyraLinkDesc = function(type, id, objLink){
    return {"type": type, "id": id, "link": objLink, "title": objLink.$title};
};
...
Wrapper.prototype._graph_DataJson2Xml = function(rawData, meta){
    ...
    if (realRawData) {
        if (realRawData.$item && realRawData.$item.$) {
            this.currentData.$ = realRawData.$item.$;
            hasMetaChanged = true;
        }
        if (realRawData.$cube) {
    ...
    var graphDef = {"_metaData": this.currentData._metaData, "_drillDef": this.currentData._drillDef};
    ...
    if (!graphDef._metaData.isMetaOK) {
        ...
        xmlTitlesTag.push('<ColTitle Num="1" Nam="', graphDef._metaData.axis0FieldCode, '" X3Type="7">', (this.currentData.$[graphDef._metaData.axis0FieldCode].$title || "???"), '</ColTitle>');
    ...
    if (hasMetaChanged) {
        ...
        if (graphDef._drillDef) {
            ...
            if (graphDef._drillDef.currLevel > graphDef._drillDef.minLevel) 
                techTag.push("<PrevDetailTitle>", (this.currentData.$[graphDef._drillDef.levelsProperties[graphDef._drillDef.currLevel - 2]].$title || "???"), "</PrevDetailTitle>");
            if (graphDef._drillDef.currLevel < graphDef._drillDef.maxLevel) 
                techTag.push("<NextDetailTitle>", (this.currentData.$[graphDef._drillDef.levelsProperties[graphDef._drillDef.currLevel]].$title || "???"), "</NextDetailTitle>");
        }
