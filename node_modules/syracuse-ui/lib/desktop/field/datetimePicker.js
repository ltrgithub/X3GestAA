        ...
        for (var weekRow = 0; weekRow < 6; weekRow++) {
            ...
            for (var day = 0; day < 7; day++) {
                ...
                if (this._currentDate.equals(curDate)) {
                    $$cell.addClass("s-calendar-select");
                }
                $$row.append($$cell);
    ...
    function changeYear(isPrev){
        ...
        for (var ii = 1; ii < $$rows.length; ii++) {
            $($$rows[ii]).children("td").slice(2).each(function(){
                ...
                $$cell.toggleClass("s-calendar-select", newVal == _dateValue.year);
                $$link.text(newVal);
            });
        }
    }
    function clickCell($$cell){
        ...
        _$$item.find(".s-calendar-select").each(function(){
            ...
            if (isYear == isSelectedYear) {
                $$selected.removeClass("s-calendar-select");
            }
        });
        $$cell.toggleClass("s-calendar-select", true);
        return false;
    };
    function close($$link){
        if ($$link.index() == 0) { //ok
