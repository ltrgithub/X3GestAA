"use strict";
var helpers = require('syracuse-core/lib/helpers');
var Article = require('syracuse-ui/lib/common/article/article').Article;
var _localize = {
    missing: "missing"
};

function SearchFacets(){
}

exports.SearchFacets = helpers.defineClass(SearchFacets, Article, {
    drawBox: function(){
        this.$$item = $("<aside class='s-search-facets'/>").appendTo(this.$$container);
        this.getArticleParent().bind(this, this.$item.$bind);
    },
    _bindEvents: function(){
        if (!this._searchFacetEventBound) {
            var self = this;
            self._searchFacetEventBound = true;
            self.$$item.delegate("input.s-search-facet-member-check", "change", function(){
                var $$check = $(this);
                var $$member = $$check.closest(".s-search-facet-member");
                var $$facet = $$member.closest(".s-search-facet'");
                var $facet = self.$searchFacets[$$facet.index()];
                var $member = $facet.$members[$$member.index()];
                
                self._updateFilters($facet.$facet, $member.$title, $$check.is(":checked"));
                self.getPage().fetch({
                    jsonParams: {
                        filters: self.filters
                    }
                });
            });
        }
    },
    _updateFilters: function($facet, $title, checked){
        if (checked) {
            (this.filters[$facet] = this.filters[$facet] || []).push($title);
        }
        else {
            var filter = this.filters[$facet];
            if (filter) {
                filter.splice(filter.indexOf($title), 1);
                if (filter.length == 0) {
                    delete this.filters[$facet];
                }
            }
        }
    },
    setDataBind: function($searchFacets, record, metaData){
        var self = this;
        self.$$item.empty();
        self.filters = {};
        if (self.$searchFacets = $searchFacets) {
            $searchFacets.forEach(function($searchFacet){
                var $$facet = $("<div class='s-search-facet'/>");
                var $$body = $("<div class='s-search-facet-body'/>");
                $searchFacet.$members.forEach(function($member){
                    var $$member = $("<div class='s-search-facet-member'/>");
                    var $$checked = $("<input class='s-search-facet-member-check' type='checkbox'/>").appendTo($$member);
                    if ($member.$selected) {
                        $$checked.attr("checked", true);
                        self._updateFilters($searchFacet.$facet, $member.$title, true);
                    }
                    $("<div class='s-search-facet-member-title'/>").text($member.$title).appendTo($$member);
                    $("<div class='s-search-facet-member-count'/>").text("(" + $member.$count + ")").appendTo($$member);
                    if ($member.$links) {
                        var links = document.itemFactory.load($("<div class='s-search-facet-member-links'/>").appendTo($$member), {
                            $category: "links",
                            $skin: document.site.$item.$layoutOptions.searchFacet.menus,
                            $isBindDisabled: true,
                            $links: $member.$links
                        }, self);
                    }
                    $$member.appendTo($$body);
                });
                var $$header = $("<header class='s-search-facet-header'/>");
                $$header.append($("<div class='s-search-facet-header-title'/>").text($searchFacet.$title || $searchFacet.$facet));
                if (true || $searchFacet.$missing) {
                    $$header.append($("<div class='s-search-facet-header-missing'/>").text(_localize.missing + " (" + $searchFacet.$missing + ")"));
                }
                $$facet.append($$header).append($$body).appendTo(self.$$item);
            });
            self._bindEvents();
        }
    },
    onNotifyRecordChange: function(value, binding){
        var data = {
            $actions: {
                $save: {
                    $isRequested: true
                }
            }
        };
        data[binding] = value;
        return data;
    }
});
/*
 exports.create = function(page, $$container){
 var searchFacets = new SearchFacets();
 searchFacets.$$container = $$container;
 searchFacets.$prototype = {};
 document.itemFactory.initializeItem(searchFacets, {
 $bind: "$searchFacets"
 }, page);
 searchFacets.loadBox();
 return searchFacets;
 };*/
