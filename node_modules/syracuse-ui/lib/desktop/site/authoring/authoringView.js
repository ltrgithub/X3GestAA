...
exports.AuthoringView = helpers.defineClass(AuthoringView, Article, {
    ...
    moveContent: function(sourceItem, targetItem, action){
        var count = sourceItem.layoutContent.items.length - 1;
        while (count >= 0) {
            var item = sourceItem.layoutContent.items[count--];
            if (item != targetItem) {
                if (item.authoringNode.$authoringLevel == "field") {
                    (action ? targetItem.layoutParent : targetItem.layoutContent).appendNewItem({
                        newItem: item,
                    ...
                    if (item.layoutContent) {
                        this.moveContent(item, targetItem, action);
                    }
                }
            }
        }
