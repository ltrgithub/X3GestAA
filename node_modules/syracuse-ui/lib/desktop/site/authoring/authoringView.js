...
var palettes = {
    ...
    section: require("./palette/sectionPalette").SectionPalette,
    block: require("./palette/blockPalette").BlockPalette,
...
exports.AuthoringView = helpers.defineClass(AuthoringView, Article, {
    ...
    drawBox: function(){
        ...
        self.$$title = $("<a id='s-author-header-title' href='#'/>").text(_localize.title).appendTo($$header);
        ...
        self.$$switchModeSlot = $("<div id='s-author-header-switch-mode'/>").appendTo($$header);
        self.$$layoutSlot = $("<div id='s-author-header-layouts'/>").appendTo($$header);
        self.$$panelActions = $("<div id='s-author-header-panel-actions'/>").appendTo($$header);
        ...
        self.$$authorBody = $("<div id='s-author-body'/>").appendTo(self.$$container);
        self.appendBreadcrumb(self.$$authorBody);
        self._$$panel = $("<div class='s-author-panel'/>").appendTo(self.$$authorBody);
        self._ensurePanelVisibility();
        document.site.resize();
    ...
    _drawStep: function(node){
        ...
        dom.setAttribute("title", _localize[(node.isPageItem() ? "page" : node.$authoringLevel) + "Step"]);
        node.addNodeId(dom);
    ...
    moveContent: function(options){
        ...
        var doSelectItem = options.doSelectItem === undefined ? true : options.doSelectItem;
        while (count >= 0) {
    ...
    /*moveContent: function(options){
     var count = options.sourceItem.layoutContent.items.length - 1;
     var layout = options.layout || (options.action ? options.targetItem.layoutParent : options.targetItem.layoutContent);
     var doSelectItem = options.doSelectItem === undefined ? true : options.doSelectItem;
     while (count >= 0) {
     var item = options.sourceItem.layoutContent.items[count--];
     if (item != options.targetItem) {
     if (item.authoringNode.$authoringLevel == (options.levelMoved || "field")) {
     layout.appendNewItem({
     newItem: item,
     targetItem: options.targetItem,
     action: options.action,
    ...
    addNewBox: function(options){
        var newBox = document.itemFactory.create({
            ...
            $title: _localize["new" + options.$category.slice(0, 1).toUpperCase() + options.$category.slice(1)],
            $layout: {
                $items: []
            }
        }, options.parentBox);
