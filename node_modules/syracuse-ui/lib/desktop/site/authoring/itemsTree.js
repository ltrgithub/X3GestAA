...
helpers.defineClass(Node, null, {
    ...
    load: function(){
        var self = this;
        self.pound = document.authoringSite.getPound(self.$authoringLevel = (self.item.$authoringLevel || "field"));
    ...
    toggleStepLink: function(show){
        var self = this;
        if (show) {
            self.$$stepLink = $("<a/>").attr("id", "s-author-" + self.item.id).addClass("s-author-path-link s-author-path-" + self.$authoringLevel).appendTo(self.tree.authoringView.$$path);
            self.$$stepLink.bind("click", function(){
                ...
                document.authoringSite.createPalette(self);
            ...
            self.$$opener = $("<a class='s-author-path-opener'/>").bind("click", function(){
                if (!self.popupTree) {
                    ...
                    self.popupTree = document.site.createPopup({
                        $$popup: self._$$treeHost,
                        position: {
                            my: "left top",
                            at: "left bottom",
                            of: self.$$stepLink
                        ...
                        boundarySelector: "#" + self.$$stepLink.attr("id")
                    });
                    self.showOverItem(true);
                    self.tree.showTargetItem(self, true);
                }
    ...
    applyDesignMetaData: function(metaData){
        var self = this;
        if (metaData.$field) {
            if (self.$authoringLevel == "field") {
                metaData = metaData.$field;
            }
            else {
                self.children.forEach(function(child){
                    child.applyDesignMetaData(metaData);
                });
            }
            self.item.applyDesignMetaData(metaData);
        }
        if (metaData.$box) {
            if (self.$authoringLevel != "field") {
                self.item.applyDesignMetaData(metaData.$box);
            }
        }
        if (metaData.$isHidden !== undefined) {
            self.applyHiddenState(metaData);
            self.item.applyDesignMetaData(metaData);
        }
    },
    isPageItem: function(){
