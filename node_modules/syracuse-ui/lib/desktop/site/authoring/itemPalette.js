...
function _formatLinks($protoAddItem){
    return $protoAddItem ? {
        ...
        $items: [{
            ...
            $items: [{
                $bind: "$switchMode",
                $skin: "s-author-designer-switch-full",
                $category: "link"
            }]
        }, {
            $layoutType: "stack",
...
helpers.defineClass(ItemPalette, Article, {
    isFieldItem: function(){
        return this.designedNode.$authoringLevel == "field";
    },
    getPage: function(){
        return this;
    },
    onNotifyRecordChange: function(value, binding){
        ...
        if (self.isFieldItem()) {
            ...
            if (binding.indexOf("$fields") == 0) {
                ...
                _notifyChildrenFields(self.designedNode.children, value, "$" + fieldBind);
    ...
    setSwitchMode: function(switchMenu){
        switchMenu.setTitle(this.designedNode.$isLightMode ? _localize.fullMode : _localize.lightMode);
        switchMenu.$$item[0].className = "s-author-designer-switch-" + (this.designedNode.$isLightMode ? "full" : "light");
    },
    loadBox: function(){
        var self = this;
        self.isAuthoringDisabled = true;
        var $isEditMode = self.designedNode.item.getArticle().$isEditMode;
        if (self.isFieldItem()) {
        ...
        if (self.menuItems.$switchMode) {
            ...
            self.menuItems.$switchMode[0].onMenuClick = function(){
                self.designedNode.toggleLightMode();
                self.setSwitchMode(self.menuItems.$switchMode[0]);
                return false;
            };
                ...
                $items: [{
                    ...
                    $items: [{
                        ...
                        $layout: {
                            ...
                            $items: [{
                                $layoutType: "stack",
                                $items: [_formatDefaultItem("$isHidden"), this._formatFieldWidth(false)]
                            }, {
                                $layoutType: "stack",
                                $items: [_formatDefaultItem("$isTitleHidden"), this._formatFieldsLabelPositionBlock()]
                            }]
                        }
                    }]
                }, {
