...
helpers.defineClass(ItemPalette, Article, {
                    ...
                    case "$isHidden":
                        ...
                        if (self.menuItems) {
                            if (self.menuItems.$switchMode) {
                                ...
                                ["$addBlock", "$addSection", "$addField"].forEach(function(prop){
                                    if (self.menuItems[prop]) {
                                        self.menuItems[prop][0].setMenu($isDisabled);
                                    }
                                })
                            }
                        }
    ...
    onAddFields: function(menuItem){
        ...
        if (!self._popupAdFields) {
            ...
            self.$$popupPanel = $(div).delegate("a.s-author-fields-field-value", "click", function(){
                ...
                setTimeout(function(){
                    ...
                    if (!self.isFieldItem()) {
                        ...
                        if (node.$authoringLevel == "article" || $childLevel == "section" || $childLevel == "block") {
            ...
            Object.keys(article.$prototype.$).forEach(function($name){
                ...
                div.style.backgroundImage = "url('" + document.site.$item.$iconPath + "authoring/s-author-light-" + $field.$type.replace("application/x-", "") + ".png')";
                $$value.append(div);
                
                div = document.createElement("div");
                div.className = "s-author-fields-field-title";
                $$value.append($(div).text($field.$title));
    ...
    _formatLinks: function($addItems){
        var result;
        if ($addItems) {
            result = {
                $switchMode: {
            ...
            (this.$addItems = $addItems).forEach(function(addItem){
                result["$" + addItem] = {
                    $title: _localize[addItem]
                };
            });
        }
        else {
            result = {
                ...
                $items: [{
                    ...
                    $layoutType: "stack",
                    $layoutCss: "s-author-designer-link-stack",
                    $items: this.$addItems.map(function(addItem){
                        return {
                            $bind: "$" + addItem,
                            $skin: "s-author-designer-link s-author-" + addItem.toLowerCase(),
                            $category: "link"
                        }
                    })
                }]
    ...
    _formatBoxLayoutStack: function(){
        return {
            $layoutType: "stack",
            $items: [{
                $layoutType: "columns",
...
exports.PagePalette = helpers.defineClass(PagePalette, ItemPalette, {
    loadBox: function(){
        ...
        self.$prototype = {
            ...
            $links: self._formatLinks(["addSection", "addField"]),
            $boxLayoutType: "100",
...
exports.SectionPalette = helpers.defineClass(SectionPalette, ItemPalette, {
    loadBox: function(){
        ...
        this.$prototype = {
            ...
            $links: this._formatLinks(["addBlock", "addField"]),
            $isSeparatorsVisible: false,
            $boxLayoutType: "100",
            $fieldsWidth: "fit",
            $fieldsIsTopLabelAlignment: false,
...
exports.BlockPalette = helpers.defineClass(BlockPalette, ItemPalette, {
    loadBox: function(){
        ...
        this.$prototype = {
            ...
            $links: this._formatLinks(["addField"]),
            $isSeparatorsVisible: false,
            $boxLayoutType: "100",
            $fieldsWidth: "fit",
