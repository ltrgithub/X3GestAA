    ...
    onNotifyRecordChange: function(value, binding){
        ...
        if (binding.indexOf("$field$") == 0) {
            ...
            if (self.designedNode.$authoringLevel != "field") {
                self.designedNode.children.forEach(function(child){
                    child.item.applyDesignMetaData(metaData);
                });
                metaData["$field" + binding] = value; //restore for applychange
            }
        }
        ...
        //self.designedNode.applyDesignMetaData(metaData);
                ...
                switch ($prop) {
                    case "$isTopLabelAlignment":
                        metaData.$["$field$isRightTextLabelAlignment"] = {
                            $isDisabled: newData.$isTopLabelAlignment
                        };
                        break;
                }
                var bind = $prop;
    ...
    loadBox: function(){
        ...
        if (this.designedNode.item.getAuthoringWidget) {
            if (this.fieldAuthoring = this.designedNode.item.getAuthoringWidget()) {
                ...
                document.itemFactory.initializeItem(this.fieldAuthoring, {}, this.authoringView);
