...
function TreeNode(item, parentNode, tree){
    this.item = item;
...
exports.TreeNode = helpers.defineClass(TreeNode, null, {
    ...
    drawNode: function(draw){
        if (draw) {
            ...
            this.tree.addNodeId(check, this);
            this.applyHiddenState(this);
            
            dom = document.createElement("div");
            dom.className = "s-author-tree-item-title";
            this.tree.addNodeId(dom, this);
    ...
    load: function(){
        ...
        this.pound = {
            ...
            "block": 2,
            "field": 1
        }[this.$authoringLevel = (this.item.$authoringLevel || "field")];
        
        this.item.authoringNode = this;
        
        if (this.item.$$item) {
            this.setDesignableItem(this.item);
            if (this.item.layoutContent) {
                this.item.layoutContent.toggleAuthoring(true);
            }
        }
        this.applyHiddenState(this.item.$item);
        //this.draw();
        this._loadLayout(this.item.layoutContent);
        return this;
    },
    ...
    appendChildItem: function(options){
        ...
        if (!child) {
            child = options.newItem.authoringNode = new TreeNode(options.newItem, this, this.tree);
        }
        else {
        ...
        if (!this.palette) {
            ...
            if (curNode && curNode != this) {
                ...
                while (next = next.nextStepNode) {
                    next.tree.toggleStepLink(next, false);
            ...
            while (curNode) {
                ...
                if (!curNode.$$stepLink) {
                    curNode.tree.toggleStepLink(curNode, true);
                }
