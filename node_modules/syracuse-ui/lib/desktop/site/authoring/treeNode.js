...
exports.TreeNode = helpers.defineClass(TreeNode, null, {
    ...
    setDesignableItem: function(item){
        if (item.$isTabLayout) {
            item.$$header.attr("s-designable", "1").attr("data-s-item", item.id);
            if (item.boxParent) {
                item.$$header.attr("data-s-article", item.boxParent.getArticle().id);
            }
        }
        item.$$item.attr("s-designable", "1").attr("data-s-item", item.id);
        if (item.boxParent) {
            item.$$item.attr("data-s-article", item.boxParent.getArticle().id);
        }
        if (this.authoringView.$isLightMode && this.$authoringLevel != "field") {
            this.toggleBoxLightMode(true);
        }
    },
    selectItem: function(doSelect){
        this.authoringView.selectItem(this, doSelect);
    },
    ...
    load: function(){
        ...
        if (self.authoringView.$isLightMode) {
    ...
    toggleFieldLightMode: function($isLightMode){
        if ($isLightMode) {
            if (!this.item.authoringLight) {
                ...
                this.item.$$item[0].className = "s-author-light-field s-authoring-designable"
                this.item.$$item[0].setAttribute("s-designable", "1");
                
                var div = document.createElement("div");
                div.className = "s-author-light-field-value";
                var $$value = $(div);
                
                var div = document.createElement("div");
                div.className = "s-author-light-field-type";
                div.style.backgroundImage = "url('" + document.site.$item.$iconPath + "authoring/s-author-light-" + this.item.$field.$type.replace("application/x-", "") + ".png')";
                $$value.append(div);
    ...
    _toggleCss: function($$item, oldSkin, newSkin){
        $$item[0].className = $$item[0].className.replace(new RegExp(oldSkin, "g"), newSkin);
    },
    toggleBoxLightMode: function($isLightMode){
        var dom;
        var skinLight = "s-author-light-" + this.$authoringLevel;
        if ($isLightMode) {
            if (!this.fullSkin) {
            ...
            if (this.item.$$header) {
                this._toggleCss(this.item.$$header, this.fullSkin, skinLight);
            }
            if (this.item.$isTabLayout) {
                this.item.$skinTab = this.item.$skin + "-tab";
                this._toggleCss(this.item.$$body, this.fullSkin, skinLight);
                this._toggleCss(this.item.layoutParent._$$tabs, this.fullSkin, skinLight);
            }
            this._toggleCss(this.item.$$item, this.fullSkin, skinLight);
        }
        else {
            this.item.$skin = this.fullSkin;
            if (this.item.$$header) {
                this._toggleCss(this.item.$$header, skinLight, this.item.$skin);
            }
            if (this.item.$isTabLayout) {
                this.item.$skinTab = this.item.$skin + "-tab";
                this._toggleCss(this.item.$$body, skinLight, this.item.$skin);
                this._toggleCss(this.item.layoutParent._$$tabs, skinLight, this.item.$skin);
            }
            this._toggleCss(this.item.$$item, skinLight, this.item.$skin);
        }
    },
    ...
    toggleOverItem: function(isOver){
        ...
        if (isOver && this.authoringView.overNode != this) {
            this.authoringView.showOverPathNodes(this, true);
        }
    },
    toggleItemCss: function(status, show, css){
        ...
        if (this.item && this.item.$$item) {
            ...
            document.site.toggleClass($$item[0], css, show);
            if (this.item.$isTabLayout) {
                document.site.toggleClass(this.item.$$header[0], css, show);
            }
        }
    },
