...
exports.DesignPalette = helpers.defineClass(DesignPalette, Article, {
    ...
    drawBox: function(){
        ...
        if (this.designedNode.$authoringLevel != "field") {
            (this.boxDesigner = new BoxDesigner()).loadDesigner(this, this.designedNode.item.$item);
        }
        else {
            (this.fieldDesigner = new FieldDesigner()).loadDesigner(this, this.designedNode.item.$item);
        }
    ...
    dispose: function(){
        ...
        delete this.boxDesigner;
        delete this.fieldDesigner;
...
exports.BoxDesigner = helpers.defineClass(BoxDesigner, Article, {
    ...
    loadDesigner: function(palette, $boxItem){
        var self = this;
        self.$prototype = {
            "$": {
                $isHidden: {
        ...
        var switchMode = self.menusFacade.items.$switchMode[0];
        ...
        self.menusFacade.items.$addSection[0].onMenuClick = function(){
            self.addNewBox(designedItem, "section");
            return false;
        };
        self.menusFacade.items.$addBlock[0].onMenuClick = function(){
...
exports.FieldDesigner = helpers.defineClass(FieldDesigner, Article, {
    loadDesigner: function(palette, $fieldItem){
        this.$prototype = {
            "$": {
                $isHidden: {
                    $type: "application/x-boolean",
        ...
        this.loadBox($fieldItem);
        if (palette.designedNode.item.getAuthoringSettings) {
            var customSetting = palette.designedNode.item.getAuthoringSettings();
            if (customSetting) {
                (this.customDesign = new CustomFieldDesigner()).loadDesigner($("<div class='s-author-designer-custom-field'/>").prependTo(this.layoutContent.items[2].$$container), palette, customSetting, $fieldItem);
            }
        }
    },
    dispose: function(){
...
helpers.defineClass(CustomFieldDesigner, Article, {
    onNotifyRecordChange: function(value, binding){
        var metaData = {};
        metaData[binding] = value;
        this.palette.designedNode.applyDesignMetaData({
            $field: metaData
        });
        return null; //cancel notify
    },
    loadDesigner: function($$container, palette, $customArticle, $fieldItem){
