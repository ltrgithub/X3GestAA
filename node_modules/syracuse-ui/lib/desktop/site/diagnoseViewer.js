...
exports.DesktopDiagnoses = helpers.defineClass(DesktopDiagnoses, null, {
    ...
    _refreshViewer: function(showAll){
        ...
        self.groups._groups.forEach(function(group){
            ...
            var $$count = $("<a>").attr('id', 's-diag-view-count-' + group.severity).addClass("s-diag-view-count");
            if (count) {
                total += count;
                countSeverity++;
        ...
        if (total) {
            if (countSeverity > 1) {
                self.$$header.children().toggleClass("s-open", false);
                var $$all = $("<a/>").attr("id", "s-diag-view-count-diagnoses").addClass("s-diag-view-count").addClass("s-open").text(total + " Diagnose(s)").appendTo(self.$$header);
    ...
    _initializeCore: function(){
        var self = this;
        self.$$core = $("<div id='s-diag-view-core'/>");
        self.$$item.append(self.$$core).draggable();
        self.$$item.append("<a class='s-diag-view-close'/>");
        self.$$header = $("<header class='s-diag-view-header'/>");
        self.$$body = $("<div class='s-diag-view-msg-list'/>");
        self.groups = new Groups(this)
    ...
    drawLinks: function(message, page){
        if (!this._links) {
            ...
            if (message.$links) {
                this.addMenuItems(message.$links);
            }
            if (message.$actions) {
                this.addMenuItems(message.$actions, true);
            }
        }
    },
