...
function _click(currentTarget, event) {
	try {
		...
		if (target.syraIsDefaultLink) {
			...
			// syra_site.activeMenusPopup && (/^tel:/.test(href) || /^cti:/.test(href)) && syra_site.activeMenusPopup.close();
			// if (/^cti:/.test(href)) {
			if (/^tel:/.test(href) && syra_site.userProfile.$menus.$ctiMakeCall) {
				event.preventDefault();
				event.stopPropagation();
				...
				// syra_site.ctiPhoneCall(syra_site.userProfile.dataset.user.ctiId, href.substring(4));
				return false;
			}
			return;
		}
