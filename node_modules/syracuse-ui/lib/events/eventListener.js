...
function _onDateTimeUTCInout(onEnter, event, target) {
	var field = syra_store.findField(target);
	if (field) {
		if (onEnter) {
			syra_site.siteFunctions.toggleTitleTip(formatApi.utcToLocal(target.textContent), target);
		} else {
...
function _onMouseItemInOut(root, event) {
	if (!syra_site.isInoutDisabled) {
		...
		var target = event.target;
		var isDateTimeUTC = target && target.className && target.className.indexOf && target.className.indexOf("s-dt-utc") >= 0;
		isDateTimeUTC && _onDateTimeUTCInout(onEnter, event, target);
		var related = event.relatedTarget;
		var item;
		while (target && target != root) {
			if (target.syrainout) {
		...
		if (syra_dd.ddAgent) {
			...
			while (target) {
				if (target.syraDragSpot) {
					...
					if (_walkDDListener(syra_dd.barResizer, target, event)) {
						return;
					}
