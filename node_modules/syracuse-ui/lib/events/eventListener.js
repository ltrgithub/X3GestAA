...
exports.addEventListeners = function(root) {
	var listeners = {
		root: root,
		dispose: function() {
	...
	$(root).delegate("textarea,input,select,[contenteditable]", "mousedown change focusin focusout", function(event) {
	...
	root.addEventListener("drop", listeners.drop = function(event) {
		var target = event.target;
	...
	root.addEventListener("focusin", listeners.focusin = function(event) {
		_onFocusInOut(root, event);
	}, false);
	root.addEventListener("focusout", listeners.focusout = function(event) {
		_onFocusInOut(root, event);
	}, false);
	root.addEventListener("click", listeners.click = function(event) {
	...
	root.addEventListener("mouseover", listeners.mouseover = function(event) {
		_mouseover_out(root, event);
	}, false);
	root.addEventListener("mouseout", listeners.mouseout = function(event) {
		_mouseover_out(root, event);
	}, false);
	return listeners;
}
...
exports.load = function() {
	document.addEventListener("keydown", _listeners.keydown = function(event) {
		keyboardHandlers[event.type](event);
	}, false);
	document.addEventListener("keypress", _listeners.keypress = function(event) {
		keyboardHandlers[event.type](event);
	}, false);
	document.addEventListener("keyup", _listeners.keyup = function(event) {
		keyboardHandlers[event.type](event);
	}, false);

	_listeners.root = exports.addEventListeners(syra_site.layoutSlot);

	window.addEventListener("unload", _listeners.unload = function(event) {
...
exports.dispose = function() {
	window.removeEventListener("unload", _listeners.unload);
	window.onerror = window.onbeforeunload = null;
	_listeners.root && _listeners.root.dispose();
	document.removeEventListener("keydown", _listeners.keydown);
	document.removeEventListener("keypress", _listeners.keypress);
	document.removeEventListener("keyup", _listeners.keyup);
