...
function _onClick(currentTarget, event) {
	try {
		...
		syra_site.ensureRecordUniqueSelection(item);
		if (!done) {
...
exports.load = function() {
	...
	window.onbeforeunload = function(event) {
		if (!syra_quality.enableTestRobot && !syra_site.hasDoneLogOut && !syra_site.hasLogonFailed && !syra_site.isOfficeSite) {
	...
	window.onerror = function(errorMsg, url, lineNumber, column, error) {
		alert(errorMsg + '\nurl=' + url + '\nat line ' + lineNumber + '\n' + window.getStack() + (error ? error.stack : ""));
