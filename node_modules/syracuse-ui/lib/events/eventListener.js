	...
	try {
		...
		while (target && target != currentTarget) {
			if (target.syraBtnclick) {
				...
				item = syra_store.findItem(target);
			...
			if (target.syraItem) {
				var observer = item = syra_store.findItem(target);
...
exports.load = function() {
	...
	_handlers.body = {
		contextmenu: document.body.addEventListener("contextmenu", function(event) {
			var target = event.target;
			var item = syra_store.findItem(target);
			if (item && item.page && item.page.isFusionPage) {
				if (item.isMenuItem || (target.tagName == "A" && target.getAttribute("href") == "#")) {
					event.preventDefault();
					return false;
				}
			}
...
exports.dispose = function() {
	_clearWaitingResize();
	_handlers.free(_handlers.body, document.body);
