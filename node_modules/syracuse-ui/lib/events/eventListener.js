...
function _mousedown(event) {
	var target = event.target;
	if (target.syraBtnmouseUpDown) {
		var btn = syra_button.getButton(target.syraBtnmouseUpDown);
		btn && btn.mouseUpDown(event, target);
	}
	if (syra_dd.ddAgent) {
		syra_dd.stop();
	} else {
		if (target.syraNoDDSpot) {
			return;
		}
...
function _mouseup(event) {
	...
	if (syra_dd.ddAgent) {
		syra_dd.ddAgent.onDragMouseUp(target, _emulateEvent(event));
		event.preventDefault();
		event.stopPropagation();
...
function _listenMousEvents() {
	if (!_listeners.mouse) {
		_listeners.mouse = {};
		syra_site.layoutSlot.addEventListener("mousedown", _listeners.mouse.mousedown = function(event) {
			_mousedown(event);
		}, false);
		syra_site.layoutSlot.addEventListener("touchstart", _listeners.mouse.touchstart = function(event) {
			_mousedown(event);
		}, false);
		syra_site.layoutSlot.addEventListener("mousemove", _listeners.mouse.mousemove = function(event) {
			_mousemove(event);
		}, false);
		syra_site.layoutSlot.addEventListener("touchmove", _listeners.mouse.touchmove = function(event) {
			_mousemove(event);
		}, false);
		syra_site.layoutSlot.addEventListener("mouseup", _listeners.mouse.mouseup = function(event) {
			_mouseup(event);
		}, false);
		syra_site.layoutSlot.addEventListener("touchend", _listeners.mouse.touchend = function(event) {
			_mouseup(event);
		}, false);
...
exports.load = function() {
	...
	window.onbeforeunload = function(event) {
		if (!_ignoreUnloadEvent && !syra_context.enableTestRobot && !syra_site.hasDoneLogOut && !syra_site.hasLogonFailed && !syra_site.isOfficeSite) {
