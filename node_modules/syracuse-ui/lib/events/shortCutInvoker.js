	...
	if (shortcuts.esc) {
		...
		if (shortcuts.q) {
			...
			if (list) {
				...
				filterRecord && filterRecord.focus();
			}
			return true;
		}
		...
		if (shortcuts.w) {
			shortcuts.z && syra_pagePreferences.reset(item.page);
			return true;
		}
		if (shortcuts.g) {
		...
		if (item.isField || item.isArrayField || item.isMenuGroupRoot) {
			...
			if (shortcuts.enter) {
				...
				if (syra_site.browser.isMSIE && item.input && syra_site.browser.info.indexOf("10.0") != -1) {
					syra_workingCopy.onInputChange(item, event);
				}
