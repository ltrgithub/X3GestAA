...
function _walkBookmarks(item, shortcuts, event) {
	var button = syra_site.bookmarks.getFirstActiveButton();
	button && button.link && button.link.focus();
	return true;
}
...
function _invokeShortCut(shortcutCbck, event) {
	var ret;
...
exports.apply = function(item, shortcuts, event) {
	...
	if (shortcuts.esc) {
		if (isPage) {
			...
			if ((dynSC = _isRegisteredShortCut(shortcuts, item)) && _invokeShortCut(dynSC.cbck, event)) {
				return true;
			}
		}
		if (shortcuts.f1) {
		...
		if (shortcuts.shift && shortcuts.up) {
			return _walkBookmarks(item, shortcuts, event);
		}
			...
			if (isPage && item.isMessageBox) {
				if (item.$itemButtons.length > 1) {
					...
					if (ii < len) {
						...
						syra_dom.toggleClass(item.getMenuItem(item.focusBtnId).domItem, "s-msgbox-button-default", false);
						item.focusBtnId = item.$itemButtons[ii].$bind;
						syra_dom.toggleClass(item.getMenuItem(item.focusBtnId).domItem, "s-msgbox-button-default", true);
						setTimeout(function() {
		...
		if (item.page && item.page.externalAdapter && item.page.externalAdapter.applyShortCut) {
			...
			if (item.$item.$isFilterMode && item.page.isFusionPage && shortcuts.down && !shortcuts.enter) {
		...
		if (item.isField || item.isArrayField || item.isMenuGroupRoot) {
			...
			if (shortcuts.enter) {
				...
				if (!item.page.isFusionPage && item.$item.$isFilterMode && item.articleParent.list) {
					...
					if ((item.getDataValue() == item.currentValue) || (item.currentValue == undefined && item.getDataValue() == "")) {
						item.articleParent.list.applyShortCut(shortcuts, event);
					} else {
						syra_workingCopy.onInputChange(item);
						this.currentValue = item.getDataValue();
					}
					return true;
				}
			}
