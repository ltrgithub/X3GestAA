...
exports.applyEscape = function(item, shortcuts, event) {
	...
	if (!isApplyed) {
		if (item.popupCard && item.list && item.list.builder && item.list.builder.applyEscape) {
			item.list.builder.applyEscape(item, shortcuts, event);
		} else if (item.articleParent && item.articleParent.popupCard && item.articleParent.list && item.articleParent.list.builder && item.articleParent.list.builder.applyEscape) {
			item.articleParent.list.builder.applyEscape(item, shortcuts, event);
		} else {
			item.page.overPanel && item.page.overPanel.close();
		}
	}
};

exports.applyQuickAccess = function(item, repeatKey, event) {
	var ret = false;
	switch (repeatKey) {
		case "ctrl":
			syra_site.searcher && syra_site.searcher.gotoField(item.page || syra_site);
			ret = true;
			break;
		case "ctrl_shift":
			break;
		...
		case "end":
			break;
		case "end_shift":
			syra_devTools.openByShortCut(item.page || syra_site, item || syra_site);
			ret = true;
			break;
		case "down_shift":
		case "up_shift":
			syra_devTools.scroll((repeatKey.split("_"))[0]);
			ret = true;
			break;
		case "right_shift":
			syra_devTools.validate();
			ret = true;
			break;
		case "left_shift":
			syra_devTools.collapse();
			ret = true;
			break;
		default:
			break;
...
exports.apply = function(item, shortcuts, event) {
	...
	if (shortcuts.esc) {
		...
		if (shortcuts.g) {
			...
			shortcuts.n && syra_nav.openSiteMap();
			return true;
		}
		...
		if (shortcuts.tab || shortcuts.i || shortcuts.d || shortcuts.f2) {
		...
		if (shortcuts.tab || shortcuts.right || shortcuts.left) {
			if (isPage && item.isAlertModal) {
				var $btns = item.$item.$newButtons;
				if ($btns.length > 1) {
					var ii, len;
					for (ii = 0, len = $btns.length; ii < len; ii++) {
						if ($btns[ii].id == item.focusBtnId) {
							break;
						}
					}
					if (ii < len) {
