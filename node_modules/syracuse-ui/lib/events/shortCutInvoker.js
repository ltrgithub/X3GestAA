...
function _walkMenuItem(menuItem, show) {
	var classGroup = menuItem && menuItem.domTitle || menuItem && menuItem.domItem;

	if (menuItem && menuItem.domItem) {
		syra_site.dom.toggleClass(classGroup, "s-enter", show);
	}
...
function _walkLeftList(item, shortcuts, event) {
	...
	var leftList = item.fusionBar && item.fusionBar.openedBlock && item.fusionBar.openedBlock.leftList;
	var filterRecord = leftList.builder && leftList.builder.filterRecord;
...
function _walkSections(item, shortcuts, event) {
	...
	item._lastSection = item._lastSection || "page";
	if (shortcuts.right) {
		if (item._lastSection === "page") {
			return _walkActionPanel(item, shortcuts, event);
		}
		if (item._lastSection === "action") {
			_clearMenuStack(item);
			if (item.isFusionPage && item.fusionBar && !item.fusionBar.$isCollapsed) {
				item._lastSection = "leftList";
				return _walkLeftList(item, shortcuts, event);
			} else {
				item._lastSection = "page";
			}
		}
		if (item._lastSection === "leftList") {
			item._lastSection = "page";
		}
	}
	if (shortcuts.left) {
...
function _walkActionPanel(item, shortcuts, event) {
	...
	if (menuStack.length > 0) {
		if (!item._lastItem) {
			item._lastItem = menuStack[0];
			return _walkMenuItem(item._lastItem, true);
		}
		...
		// new marking
		for (var kk = 0, mm = menuStack.length - 1; kk <= mm; kk++) {
			if (menuStack[kk] === item._lastItem) {
				if (shortcuts.down) {
					delete shortcuts["down"];
					if (kk < mm) {
						item._lastItem = menuStack[kk + 1];
					} else {
						item._lastItem = menuStack[0];
					}
				}
				...
				if (shortcuts.up) {
					...
					if (kk > 0) {
						...
						item._lastItem = menuStack[mm];
					}
				}
				if (shortcuts.left) {
					delete shortcuts["left"];
					if (item._lastItem.$opened === true) {
						item._lastItem.openBox(false);
					}
				}
				if (shortcuts.right) {
					delete shortcuts["right"];
					if (item._lastItem.$opened === false) {
						item._lastItem.openBox(true);
					}
				}
				return _walkMenuItem(item._lastItem, true);
			}
...
exports.registerShortCut = function(item, scDef, id, callback, callbackBind, callbackData) {
	scDef = _helpers.object.clone(scDef, true); //Quick correction to avoid blocking


	if (!scDef.item) {
		scDef.item = item;
	} else if (scDef.item === "page") {
...
exports.unregisterShortCut = function(item, id) {
	var i, len, sc;
...
exports.apply = function(item, shortcuts, event) {
	...
	if (shortcuts.esc) {
		...
		if (shortcuts.q) {
			...
			var list;
			if (isPage && !item.isFusionPage) {
				list = syra_store.findList(item.page);
		...
		if (shortcuts.insert || (shortcuts.n && !shortcuts.g)) {
			...
			if (page.isLandingPage) {
				if (event.shiftKey) {
					page.addVignetteBtn.link.click();
				} else {
					page.addTabBtn.link.click();
				}
				return true;
			}
		...
		if (shortcuts.enter || shortcuts.space) {
			...
			main && syra_menus.click.menuItem(main, event);
			return true;
		}
		if (shortcuts.a) {
		...
		if (shortcuts.g) {
			shortcuts.h && syra_site.homeLink.click();
			shortcuts.n && syra_menus.click.navigation();
			return true;
		}
		...
		if ((shortcuts.up || shortcuts.down || shortcuts.left || shortcuts.right) && (menuStack.length > 0)) {
			return _walkActionPanel(item, shortcuts, event);
		}
		if (shortcuts.enter && item._lastItem) {
			syra_menus.click.menuItem(item._lastItem, event);
			_clearMenuStack(item);
			event.returnValue = false;
			event.preventDefault();
			event.stopPropagation();
		...
		if (shortcuts.tab) {
			...
			if (item.isField && item.$item.$contentEditable) {
				syra_workingCopy.onInputChange(item, event);
				return true;
			}
		}
		...
		if (item.isField || item.isArrayField || item.isMenuGroupRoot) {
			if (item.isMenuGroupRoot && syra_menus.group.applyShortCut(item, shortcuts, event)) {
				return true;
			}
			...
			if (shortcuts.enter) {
				...
				if (item.$item.$bind === "$search" && item.articleParent.isArrayField) {
				...
				if (!item.page.isFusionPage && item.$item.$isFilterMode) {
					syra_workingCopy.onInputChange(item);
					return true;
				}
			}
