...
function _activateKeyNavigation(list, activate) {
	list && syra_site.dom.toggleClass(list.fieldValue, "s-key-activate", activate);
}

function _activateSiblingList(item) {
	_activateKeyNavigation(item.page.activateList, false);
	item.page.activateList = item.isList && item;
	if (!item.page.activateList) {
		//walk up from field
		var list = item.articleParent;
		while (list) {
			if (list.isList) {
				item.page.activateList = list;
				break;
			}
			list = list.articleParent;
		}
	}
	if (!item.page.activateList && item.page.boundFields) {
		//walk up from page
		var binds = Object.keys(item.page.boundFields);
		for (var ii = 0, jj = binds.length; ii < jj; ii++) {
			var list = item.page.boundFields[binds[ii]];
			list = list && list[0];
			if (list && list.isList) {
				item.page.activateList = list;
				break;
	...
	_activateKeyNavigation(item.page.activateList, true);
}
...
exports.apply = function(item, shortcuts, event) {
	...
	if (shortcuts.esc) {
		...
		if (isPage) {
			...
			if (shortcuts.k || shortcuts.j) {
				shortcuts.k && syra_menus.click.next(item, event.shiftKey);
		...
		if (item.isField || item.isList) {
			...
			if (shortcuts.down || shortcuts.up) {
				if (shortcuts.down) {
					/*				var menuGroup = item.menusPopup && item.menusPopup._content;
                     if(menuGroup){
		...
		if (item.page && item.page.activateList) {
			return item.page.activateList.applyShortCut(shortcuts, event);
		}
	}
};