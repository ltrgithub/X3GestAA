...
exports.applyFusionFieldShortcut = function(field, shortcuts, event) {
	if (field.onFieldInputEvent && field.page.isFusionPage) {
		field.onFieldInputEvent(event, shortcuts);
...
exports.apply = function(item, shortcuts, event) {
	...
	if (shortcuts.g && event.shiftKey && event.ctrlKey) {
		var list = syra_store.findList(item);
		list && item.setActivateList(list);
	...
	if (shortcuts.esc) {
		...
		if (shortcuts.r) {
			return syra_grid.columnFitter.applyResizeShortcut(item);
		}
		...
		if (shortcuts.f7 || shortcuts["?"] || shortcuts.s || shortcuts["."]) {
			syra_menus.click.goToSearch(item.page);
			return true;
		}
		...
		if (item.isField || item.isList) {
			...
			if (item.onFieldInputEvent && (shortcuts["delete"] || shortcuts.insert || shortcuts.right || shortcuts.left)) {
				if (item.onFieldInputEvent(event, shortcuts)) {
					return true;
				}
			}
			//LV Convergence
			if (shortcuts.down || shortcuts.up) {
				if (!item.articleParent.popupCard && item.onFieldInputEvent && item.articleParent.isGridRecord) {
					if (item.onFieldInputEvent(event, shortcuts)) {
						return true;
					}
				}
			...
			if (item.applyShortCut) {
				if (item.applyShortCut(shortcuts, event)) return true;
			}
			...
			if (shortcuts.enter) {
				//LV Convergence
				if (exports.applyFusionFieldShortcut(item, shortcuts, event)) {
					return true;
				}
		...
		if (shortcuts.space) {
			if (item.page.isNavigationPage && item.isMenuItem) {
				var record = item.articleParent && item.articleParent;
				record && record.clickBookmark && record.clickBookmark();
				event.preventDefault();
				event.stopPropagation();
				return true;
			}
		}
		if (isPage && item.activateList) {
			return item.activateList.applyShortCut(shortcuts, event);
		}
	}
};