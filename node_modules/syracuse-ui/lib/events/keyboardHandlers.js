...
function _findItem(target) {
	...
	var item = target.syraKeyTarget ? syra_store.get(target.syraKeyTarget) : syra_store.findItem(target);
	...
	return _findItemMenusPopup(item) || item;
}

function _findItemMenusPopup(item) {
	if (item && item.menusBox && item.menusBox.menuGroupRoot)
		return item.menusBox.menuGroupRoot;
	var popup = syra_site.dialogManager.getPopup();
	if (popup && popup.openerBox && popup.openerBox.menusBox)
		return popup.openerBox.menusBox.menuGroupRoot;
	return null;
}
