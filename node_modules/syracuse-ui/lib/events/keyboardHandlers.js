...
exports.keydown = function(event) {
	...
	if (!isShorcutApplyed && targetItem.page && targetItem.page.isFusionPage) {
		if (!isEsc && targetItem.isField && targetItem.onFieldInputEvent) {
			targetItem.onFieldInputEvent(event, _shortcuts);
			event.stopPropagation();
			return;
		}
	}
	//ShorcutApplyed && syra_site.fusionGateway.onKeyDow(event);	
...
exports.keypress = function(event) {
	...
	if (charCode !== undefined && !(_shortcuts && _shortcuts.esc)) {
		var item = _findItem(event.target);
		if (item.isField && !shortCutInvoker.validateKeyPress(item, String.fromCharCode(event.charCode || event.keyCode), _shortcuts, event)) {
			event.returnValue = false;
			event.preventDefault();
			event.stopPropagation();
			return false;
