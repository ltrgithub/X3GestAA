		...
		<div class="s-chat-slot">\
			<header>\
				<div class="s-chat-recipient-label">\
					...
					<a class="s-chat-collect-btn"><i class="fa fa-user"></i></a>\
				</div>\
			</header>\
			<section>\
...
var _buildNotifItem = function(msg) {
	...
	ctx.originLabel = msg.firstName || (msg.from === "business" ? "Business" : (msg.from ? msg.from : "user unidentified"));
	ctx.time = msg.stamp;
	...
	if (msg.from === "business") {
		ctx.typeIcon = "fa fa-brief-case";
	} else {
		ctx.typeIcon = msg.severity && msg.severity === "warning" ? "fa fa-flash" : "fa fa-info-circle";
	}
	// if (msg.severity) {
...
var _bindEvents = function(bind) {
	if (bind) {
		...
		$(document.body).delegate(".s-chat-input-submit", "click", function(evt) {
			// get recipients
			var recipient = $(".s-chat-recipient-label select").val();
			...
			var recipArr = Array.isArray(recipient) ? recipient : recipient.split(",");
			// build message
		...
		$(document.body).delegate(".s-chat-collect-btn", "click", function(evt) {

			var users = syra_site.notifSocket.emit("get users", 30, function(users) {
				console.log("connected users : " + JSON.stringify(users, null, 2));
				var $$select = $(".s-chat-recipient-label select");
				$$select.empty();
				users.forEach(function(user, idx) {
					var status = user.connected ? "connected" : "disconnected";
					var $$content = $("<option value='" + user.login + "'>" + "(" + status + ") " + (user.firstName || user.login) + "</option>");
		...
		// $(document.body).delegate(".s-chat-recipient-label select", "change", function(evt) {
		// 	var users = syra_site.notifSocket.emit("get users", 30, function(users) {
		// 		console.log("connected users : " + JSON.stringify(users, null, 2));
...
var _disconnect = function() {
	syra_site.notifSocket.emit("disconnect", syra_site.userProfile.dataset.user.$value);
};
...
var _appendPicker = function() {
	...
	// retrieve events
	var events = syra_site.notifSocket.emit("get events", login, 30, function(events) {
