...
var _updateEvents = function(events) {
	...
	if (!syra_site.notifArr) {
		...
		syra_site.notifArr.forEach(function(evt, idx) {
...
var _initChat = function(login) {
	var ctx = {};
	ctx.originSrc = "/syracuse-ui/themes/desktop/sage/images/notificationsusers/img004.jpg";
	ctx.login = login;
	ctx.status = _getStatus(login);

	var chatHtml = _getHtml(null, "chatForm", ctx);
...
var _bindEvents = function(bind) {
	if (bind) {
		...
		$(document.body).delegate(".s-chat-input", "change", function(evt) {
			// build message
