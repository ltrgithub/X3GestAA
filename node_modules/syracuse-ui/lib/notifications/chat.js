...
var _buildNotifItem = function(msg) {
	...
	ctx.originLabel = msg.firstName || (msg.from === "business" ? "Business" : "user unidentified");
	ctx.time = msg.stamp;
...
var _bindEvents = function(bind) {
	if (bind) {
		...
		$(document.body).delegate(".s-chat-recipient-label select", "click", function(evt) {
			var users = syra_site.notifSocket.emit("get users", 30, function(users) {
				...
				users.forEach(function(user, idx) {
					var $$content = $("<option value='" + user.login + "'>" + (user.firstName || user.login) + "</option>");
