...
exports.MobileDiagnoseViewer = helpers.defineClass(MobileDiagnoseViewer, MobilePopup, {
    ...
    _appendLinks: function(message){
        ...
        if (!self._menus) {
            ...
            self._menus = document.itemFactory.load(self.$$body, $item, self.page);
            // handle behaviour when links are clicked (clean diagnose, execute click action)            
            self.$$body.delegate("a." + $item.$skin, "click", function(){
                document.site.$diagnoseViewerRemoval = true;
            });
        }
        ["$links", "$actions"].forEach(function($type){
            ...
            if ($menus) {
                ...
                Object.keys($menus).forEach(function($bind){
                    var $menu = $menus[$bind];
                    var bound = self.page.menuItems[$menu.$bind || $bind];
                    if (bound) {
    ...
    close: function(){
        this._emptyBody();
        this.$$item.hide();
        MobilePopup.prototype.close.call(this);
    },
    dispose: function(){
