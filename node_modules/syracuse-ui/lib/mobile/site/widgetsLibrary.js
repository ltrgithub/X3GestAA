...
var _fieldTypes = {
    ...
    "application/x-array": require("../field/collection/list").List,
...
exports.categories = {
    ...
    field: {
        create: function($item, boxParent){
            var $field = $item.$field || boxParent.getArticle().get$Properties()[$item.$bind];
            var field;
            if ($field) {
                ...
                switch ($class) {
                    ...
                    case "application/x-choice":
                        ...
                        if ($item.$inplace || $item.$isFilterMode || $format == undefined && ($field.$value.$enum.length > ($item.$radiosMax || 4))) {
                            $format = "$combo";
                        }
                        $class += ($format || "$radios");
                        break;
