...
exports.MobileMenuItem = helpers.defineClass(MobileMenuItem, MenuItem, {
    ...
    _createMenu: function(article, $menu){
        ...
        if ($icon.$name) {
            this.domItem.setAttribute("data-icon", $icon.$name);
        }
        var iconPos = this.$item.$noText ? "notext" : $icon.$position;
        if (iconPos) {
            this.domItem.setAttribute("data-iconpos", iconPos);
        }
        ...
        switch (this.$item.$format) {
            case "button":
                ...
                if ($button.$inline) {
                    this.domItem.setAttribute("data-inline", true);
                }
                if ($button.$position) {
                    this.domItem.className += (" ui-btn-" + $button.$position);
                }
                this.domItem.setAttribute("data-theme", this.$item.$theme || document.site.$item.$themes.$default.$button);
                this.domItem.setAttribute("data-role", this.$item.$format);
                if (!this.$isjqmButton) {
