...
exports.MobileMenuItem = helpers.defineClass(MobileMenuItem, MenuItem, {
    ...
    _createMenu: function(article, $menu){
        ...
        if ($icon.$name) {
            this.$d$item.setAttribute("data-icon", $icon.$name);
        }
        var iconPos = this.$item.$noText ? "notext" : $icon.$position;
        if (iconPos) {
            this.$d$item.setAttribute("data-iconpos", iconPos);
        }
        ...
        switch (this.$item.$format) {
            case "button":
                ...
                if ($button.$inline) {
                    this.$d$item.setAttribute("data-inline", true);
                }
                if ($button.$position) {
                    this.$d$item.className += (" ui-btn-" + $button.$position);
                }
                this.$d$item.setAttribute("data-theme", this.$item.$theme || document.site.$item.$themes.$default.$button);
                this.$d$item.setAttribute("data-role", this.$item.$format);
                if (!this.$isjqmButton) {
    ...
    disable: function($value){
        $value = $value || false;
        this.$$item.attr("disabled", $value).toggleClass("s-disabled", $value);
