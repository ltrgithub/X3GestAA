...
exports.MobileComboChoice = helpers.defineClass(MobileComboChoice, ChoiceField, {
    ...
    setDataValue: function(value){
        ...
        if (this.$isEditMode) {
            if (this.currentValue && this.currentValue != value) {
                ...
                this.$$dataValue.find("option[value='" + $prevEnum.$index + "']").attr("checked", false);
            }
            this.currentValue = value;
            if (this.$selectedEnum) {
                this.$$dataValue.find("option[value='" + this.$selectedEnum.$index + "']").attr("selected", true);
            }
    ...
    render: function(){
        ...
        if (this.$isEditMode) {
            self.$$dataValue = self.$$input = $("<select data-native-menu='false'/>").appendTo(self.$$item);
            if (self.inputId) {
...
exports.MobileRadioChoice = helpers.defineClass(MobileRadioChoice, ChoiceField, {
    ...
    setDataValue: function(value){
        var self = this;
        self.$selectedEnum = self._findEnum(value);
        var $title = self.$selectedEnum ? self.$selectedEnum.$title : "";
        if (self.$isEditMode) {
            if (self.currentValue && self.currentValue != value) {
                var $prevEnum = self._findEnum(self.currentValue);
                var $$prev = self.$$dataValue.find("input[value='" + $prevEnum.$index + "']").attr("checked", false);
                if (self.$isjqmField) {
            ...
            if (self.$selectedEnum) {
                var $$new = self.$$dataValue.find("input[value='" + self.$selectedEnum.$index + "']").attr("checked", true);
                if (document.site.isjqmPageLoaded()) {
            ...
            if (document.site.isjqmPageLoaded()) {
                ...
                self.$enum.forEach(function(elt){
                    indexes.push(self._findEnum(elt.$value).$index);
                })
                indexes.forEach(function(i){
                    if (self.$selectedEnum && self.$selectedEnum.$index && i != self.$selectedEnum.$index) {
                        var $$checkbox = self.$$dataValue.find("input[value='" + i + "']").attr("checked", false);
                        if (self.$isjqmField) {
    ...
    render: function(){
        ...
        if (self.$isEditMode) {
            ...
            self.$$fieldset = $("<fieldset data-role='controlgroup' data-type='horizontal'/>").appendTo(self.$$dataValue);
