...
exports.MobileComboChoice = helpers.defineClass(MobileComboChoice, ChoiceField, {
    ...
    setDataValue: function(value){
        ...
        if (this.$isEditMode) {
            if (this.currentValue && this.currentValue != value) {
                ...
                this.$$fieldValue.find("option[value='" + $prevEnum.$index + "']").attr("checked", false);
            }
            this.currentValue = value;
            if (this.$selectedEnum) {
                this.$$fieldValue.find("option[value='" + this.$selectedEnum.$index + "']").attr("selected", true);
            }
    ...
    render: function(){
        var self = this;
        self.$enum = self.$field.$value.$enum;
        if (this.$isEditMode) {
            self.$$fieldValue = self.$$input = $("<select data-native-menu='false'/>").appendTo(self.$$item);
            if (self.inputId) {
...
exports.MobileRadioChoice = helpers.defineClass(MobileRadioChoice, ChoiceField, {
    ...
    setDataValue: function(value){
        ...
        if (self.$isEditMode) {
            if (self.currentValue && self.currentValue != value) {
                ...
                var $$prev = self.$$fieldValue.find("input[value='" + $prevEnum.$index + "']").attr("checked", false);
                if (self.$isjqmField) {
            ...
            if (self.$selectedEnum) {
                var $$new = self.$$fieldValue.find("input[value='" + self.$selectedEnum.$index + "']").attr("checked", true);
                if ($isjqmPage) {
            ...
            if ($isjqmPage) {
                ...
                indexes.forEach(function(i){
                    if (self.$selectedEnum && self.$selectedEnum.$index && i != self.$selectedEnum.$index) {
                        var $$checkbox = self.$$fieldValue.find("input[value='" + i + "']").attr("checked", false);
                        if (self.$isjqmField) {
    ...
    render: function(){
        var self = this;
        self.$enum = self.getDataType().$enum;
        if (self.$isEditMode) {
            self.$$fieldValue = self.$$dataValue = self.$$item;
            ...
            self.$$fieldset = $("<fieldset data-role='controlgroup' data-type='horizontal'/>").appendTo(self.$$fieldValue);
