...
exports.ReferenceRecord = helpers.defineClass(ReferenceRecord, Article, {
    ...
    drawBox: function(){
        ...
        self.$$dataRow = $("<div/>").addClass(list.$skin + "-record").attr("data-s-record", self.$recordUuid).addClass(list.$recordCss);
        var $$data = $("<div/>").addClass(list.$skin + "-record-data").appendTo(self.$$dataRow);
        if (this.$isEditMode) {
            // grouped buttons style attribute
            //$$data.attr("data-role", "controlgroup").attr("data-type", "horizontal");
            
            self.$$del = $("<a/>").attr("data-role", "button").attr("data-icon", "delete").attr("data-iconpos", "notext").attr("data-inline", "true").addClass(list.$skin + "-record-del").appendTo($$data).bind("click", function(){
        ...
        self.$$dataRow.appendTo(list._$$body);
    },
    dispose: function(){
        if (this.$$del) {
            this.$$del.unbind();
