...
exports.CardRecord = helpers.defineClass(CardRecord, Article, {
    ...
    drawBox: function(){
        var $$container = this.$$dataRow = $("<li/>").appendTo(this.boxParent._$$body).attr("data-s-record", this.$recordUuid).addClass("s-field-cardRecord");
        var $links = this.$prototype.$links || {};
        var $link = $links.$details || $links.$edit;
        if ($link) {
            this.link = document.itemFactory.load(this.$$dataRow, {
                $bind: "$detail",
                $category: "link",
                $isListWrapper: true,
            }, this);
        ...
        document.itemFactory.load($$container, this.boxParent.$cardView, this);
    },
