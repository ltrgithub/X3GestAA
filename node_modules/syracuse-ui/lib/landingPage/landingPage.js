...
_helpers.defineClass(UserPage, Record, {
	load: function(list, dataset) {
		...
		Record.prototype.load.call(this, list, dataset);
		syra_dom.div("s-ldp-page-reorder", this.slot).syraSortableHandle = list.level;
		this.slot.appendChild(this.title);
		var $links = this.$prototype.$links;
		var actions = syra_dom.div("s-ldp-page-actions", this.slot);
		if ($links.$edit) {
			syra_button.add({
				parent: this,
				slot: actions,
				text: $links.$edit.$title,
				css: "s-ldp-page-action",
				$edit: $links.$edit,
				fontIcon: "edit",
				iconOnly: true,
				click: _editUserPage
