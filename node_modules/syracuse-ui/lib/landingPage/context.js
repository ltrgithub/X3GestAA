...
function Context() {
	this._profile = localStorage && localStorage.getItem("landingpage");
	if (this._profile) {
		this._profile = JSON.parse(this._profile);
	} else {
		//Compatibility with previous version
		this._profile = syra_preference.profile.get("landingpage");
		if (this._profile) {
			_save(this);
			syra_preference.profile.save("landingpage", null);
		}
	}
	...
	this.getRecordContext = function(record) {
		if (!record.context) {
			record.context = {
				vignettes: sessionStorage && sessionStorage.getItem("landingpage_" + record.dataset.$uuid)
			};
	...
	this.saveRecordContext = function(record) {
		sessionStorage && sessionStorage.setItem("landingpage_" + record.dataset.$uuid, JSON.stringify(record.context.vignettes));
	};
	...
	this.setSelectedTab = function(record, index) {
		var self = this;
		self._profile.tabs[record.dataset.$uuid] = index;
	...
	function _save(context) {
		localStorage && localStorage.setItem("landingpage", JSON.stringify(context._profile));
	}
}
