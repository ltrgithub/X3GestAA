...
var UserPreferences = require('syracuse-ui/lib/page/userPreferences').UserPreferences;
var _childPages = [];
...
exports.load = function($itemPage, onMainPagechange) {
	...
	/*item.$breadCrumb && item.$breadCrumb.length && item.$breadCrumb.splice(0, 0, {
		title: "$all"
	});*/
	...
	if (item.$autoFetch) {
		item.fetch({
			...
			if (item && !item.disposed) {
				...
				item.endChange();
				syra_quality.page_onAfterLoad(item);
			}
		});
	} else {
		syra_quality.page_onAfterLoad(item);
