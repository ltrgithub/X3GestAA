...
exports.DashboardPage = helpers.defineClass(DashboardPage, DesktopPage, {
	...
	_addDefaultField: function($bind, $field) {
	...
	ensureDefaultArticle: function($article, $prototype) {
		...
		if (!$article.$layout.$items) {
			...
			for (var ii = 0, jj = binds.length; ii < jj; ii++) {
				...
				if (!$field.$isTOC) {
					$itemFields.push(this._addDefaultField(binds[ii], $prototype.$properties[binds[ii]]));
				}
			}
		...
		if ($article.$importHint == "navigationMenu") {
	...
	loadBox: function(initData, $initDiagnoses) {
		...
		/*if (this.$prototype && this.$prototype.$properties && this.$prototype.$properties["USR_GX3APP_ADMIN_GENE"]) {
         this.$prototype.$properties["USR_GX3APP_ADMIN_GENE"].$isTOC = true;
         }*/
