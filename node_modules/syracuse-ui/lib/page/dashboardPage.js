...
exports.DashboardPage = helpers.defineClass(DashboardPage, DesktopPage, {
	dispose: function() {
		this.$freeFieldBinds = this.registeredVignettes = this.garbageLinks = this.freeLinks = this.loadedVignettesMenus = null;
					...
					if ($newFieldBinds.length > 0) {
						...
						for (var ii = 0, jj = $newFieldBinds.length; ii < jj; ii++) {
							...
							if ($field.$format == "$menu") {
								this.onAddNewItem(this, this.layoutContent, {
									$category: "menus",
									$title: $field.$title,
						...
						for (var ii = 0, jj = $freeLinkBinds.length; ii < jj; ii++) {
							...
							if ($freeLink && $freeLink.$vignettes) {
								...
								for (var mm = 0, kk = $freeLink.$vignettes.length; !vignette && mm < kk; mm++) {
									...
									if (sibling) {
										this.onAddNewItem(this, sibling, {
											$fieldBind: $linkBind,
											$fieldCategory: "link",
											$category: "link"
										}, "top");
	...
	extractSection: function($layout, $sections) {
		...
		if ($layout && $layout.$items) {
			for (var ii = 0, jj = $layout.$items.length; ii < jj; ii++) {
				...
				if ($item.$items) {
					this.extractSection($item, $sections);
				}
			}
		}
		return $sections;
