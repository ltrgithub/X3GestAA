...
function _insertSections(level, insertLibrary, ctx) {
	var result = _helpers.initResult(),
		severity, message, localize = syra_local,
		target, ret, freeSections, i, j, title, len, lenj, $definedBinds = {};
	try {
...
function _insertBlocks(level, insertLibrary, ctx) {
	var target, result = _helpers.initResult(),
		severity, message;
	var i, j, leni, lenj, title, ret, localize = syra_local,
		freeBlocks = [],
		$definedBinds = {};
	try {
...
function _insertFields(level, insertLibrary, ctx) {
	var i, j, lenj, len = this._$freeFieldBinds.length,
		...
		_initialize;
	var oldLibrary, localize = syra_local,
		fldTargets;
	var result = _helpers.initResult(),
		fieldName, self = this;
		...
		if ((ret = lib[lib.rules[i]].apply(this, [context].concat(args || [])))) {
			...
			if (Array.isArray(ret)) {
				for (j = 0, lenj = ret.length; j < lenj; j++) {
					...
					if (!context._alreadyProcessedModes[ret[j].mode]) {
...
var _helpers = {
	...
	getTitle: function(api, code, id) {
		...
		if (api) {
			title = syra_localizer.getLocalizeText(api, code);
		} else {
			title = code;
		}
		return title;
	...
	_findBlockByTitle: function(map, bTitle, sTitle) {
		...
		if (bTitle && (sec = _helpers._findSectionByTitle(map, sTitle))) {
			for (i = 0, len = sec.block.length; i < len; i++) {
				if (_helpers._isSameTitle(sec.block[i].$title, bTitle)) {
					found = sec.block[i];
					break;
				}
			}
	...
	_findBlockById: function(map, $bind, scope, data, page) {
		...
		if (map.section.length >= scope.sectionRank) {
			...
			if (data && data.justSection && secScope.sectionId == scope.sectionId && sec.block.length > 0) {
				data.section = sec;
			}
		}
		return found;
	...
	_getFieldlayoutFromMap: function(layoupMap, $bind, context, byPassById) {
		...
		if (block && block.boundFields.indexOf($bind) >= 0) {
			_helpers._walkCrossBlockLayoutItems(block.items, function(bd, itm) {
				var ret = false;
				if (bd == $bind) {
					ret = true;
					layout = itm;
	...
	_walkCrossBlockLayoutItems: function(items, callback, data) {
	...
	_getGridFieldModes: function(gridArticle, fldBind, context) {
		var _getFieldSrclayout, modes = [],
			proto = syra_fusion.syraUtil.getFieldProtoFromCollObj(gridArticle, fldBind);
		var i, len = _helpers._gridModes.length;
		if (proto && false) {
			// TODO when when the time comes...
			//modes = ["$fldMDGrid", "$fldMDPopup", "$fldMDVcard", "...];
		} else
		if (context._layoutFldR) {
	...
	_getLayoutRGrid: function(article, context, data, mode) {
		var layout, cards;
		layout = context._layoutFldR || _helpers._getFieldlayoutFromMap(context.page._$protoLayoutMap, (article ? article.$item.$bind : data.$gridBind), context, true);
		if (layout) {
			switch (mode) {
				case "$fldMDGrid":
					break;
				case "$fldMDPopup":
					layout = (cards = layout.$cards) && cards.$position == "popup" ? cards : null;
					break;
				default:
					layout = null;
			}
	...
	_getLayoutAGridModes: function(gridArticle) {
		...
		if (gridArticle.$item) {
			ret = {
				$fldMDGrid: gridArticle.$item.$layout ? gridArticle.$item.$layout.$items : null
			};
			/*if (gridArticle.$item.$popupCard) {
             ret.$fldMDPopup = gridArticle.$item.$popupCard.$layout ? gridArticle.$item.$popupCard.$layout.$items : null;
             }*/
		}
		return ret;
	},
	...
	_getBlockLayout: function(block, $bind, where, context, dataEx) {
		...
		function _ensureRowLayout(data, rank) {
			var newLayout, i;
			if (parentItemSlot < parentItem.length - 1 && parentItem[parentItemSlot + 1].$items) {
			...
			if (parentItemSlot < parentItem.length - 1 && !parentItem[parentItemSlot + 1].$items) {
				// Added to fix SAM 99913
				slot = parentItem;
			} else {
				newLayout = syra_site.clone(parentLayout);
		...
		if (!slot) {
			if (context.isFusion) {
				rank = 1;
				if (where.bind != null && where.bind) {
					scope = context.page.getFieldScope(where.bind, false);
					rank = scope.rank;
				}
	...
	_getAuthoredSectionForBind: function(layoutMapA, bind, sectionRefTitle, context, data) {
		var sec = null,
			dataEx = data || {};
		dataEx.justSection = true;
		if (!bind || !context.isFusion || (!(_helpers._findBlockById(layoutMapA, null, context.page.getFieldScope(bind), dataEx, context.page)) && !dataEx.section)) {
			sec = _helpers._findSectionByTitle(layoutMapA, sectionRefTitle);
		} else {
			sec = dataEx.section;
		}
	...
	"cmnlevelField": {
		getFieldSrclayout: function($bind, context, data, mode) {
			var layoutP, layout = null,
				cards;
			if ((layoutP = _helpers._getLayoutRGrid(this && this.article, context, data, mode))) {
				_helpers._walkCrossBlockLayoutItems(layoutP.$layout.$items, function(bd, itm) {
					var ret = false;
					if (bd == $bind) {
						ret = true;
						layout = itm;
			...
			return layout ? syra_site.clone(layout) : ((!data || !data.strict ? {
				"$bind": $bind
			} : null));
		}
	},
		...
		getLastAvailableBlock: function(section, context) {
			...
			function _getBlock(sec) {
				...
				for (j = sec.block.length - 1; j >= 0; j--) {
					if (sec.block[j].boundFields.length > 1 || (sec.block[j].boundFields.length > 0 && !context.library.field._oddFieldTypes[self._$properties[sec.block[j].boundFields[0]].$type])) {
						blck = sec.block[j];
						break;
					}
				}
			...
			targetLayout = _helpers._getBlockLayout(block, context.$bind, {
				"last": true
			}, context);
			return block && targetLayout ? {
				...
				"diagnosis": targetLayout.diag ? this.makeDiagnosis(null, null, targetLayout.diag.mess, targetLayout.diag.sev) : null
			} : null;
		}
	}
};
var _insertFusion = {
	"levelField": {
		"field": {
			...
			$ruleLast: function(context) {
				...
				if (grdFldModes && (len = grdFldModes.length)) {
					...
					for (i = 0; i < len; i++) {
						ret[i] = _helpers._processRuleMulitModes(grdFldModes[i], context, function() {
							...
							if (block.items && block.items.length) {
								_helpers._walkCrossBlockLayoutItems(block.items, function($bind) {
									if (self._$properties[$bind]) {
										// Warning : by this way, deleted fields are discarded
										block.boundFields.push($bind);
									}
									return false;
								});
								if (block.boundFields.length > 0) {
									target = _insertFusion._checkInBlock.call(self, block, context);
									layoutLess = false;
								}
							}
							if (layoutLess) {
	...
	"levelArticle": {
		...
		"field": {
			...
			$ruleDimField: function(context) {
				var ret = null,
					...
					bindDim, targetLayout, dimFldName, dimFldNameBis, dimFldColl = [];
				...
				if ((name = this._$properties[context.$bind].$X3Name) && (persisteP = this.article.$persistPrototype && this.article.$persistPrototype.$properties)) {
					if ((dimFldName = _getDimName(name))) {
						...
						if (!score) {
							...
							dimFldColl.push(dimFldName.id);
							for (i = 0, len = this._$freeFieldBinds.length; i < len; i++) {
								if (this._$freeFieldBinds[i] != context.$bind && (name = this._$properties[this._$freeFieldBinds[i]].$X3Name) && (dimFldNameBis = _getDimName(name)) && dimFldNameBis.n == dimFldName.n) {
									dimFldColl.push(dimFldNameBis.id);
								}
							}
							if ((dimFldColl.sort()).length > 1 && dimFldColl[0] == 1) {
								score = true;
								for (i = 0, len = dimFldColl.length; i < len - 1; i++) {
									if (dimFldColl[i] + 1 != dimFldColl[i + 1]) {
										score = false;
										break;
									}
								}
		...
		"block": {
			...
			$ruleByRank: function(context, freeBlock, srcSection, srcSectionRank) {
				...
				secA = _helpers._getAuthoredSectionForBind(this._$layoutMap, freeBlock.block.boundFields.length ? freeBlock.block.boundFields[0] : null, srcSection.$title, context);
				if (secA) {
					...
					if (i < lenA || (last = freeBlock.rank == (parseInt(temp[lenA - 1], 10) + 1))) {
						...
						if (lenA >= freeBlock.rank) {
							...
							blockA = secA.block[lenA ? lenA - 1 : secA.block.length - 1];
							rank = blockA.parent.length;
						}
						diagnosis = this.makeDiagnosis(null, null, "'" + _helpers.getTitle(this.article, freeBlock.block.$title, "Block '" + freeBlock.rank + "'") + "' : " + syra_local.autoInsertBlockInNearestSlot, "warning");
			...
			$ruleLast: function(context, freeBlock) {
				...
				for (i = 0, len = sections.length; i < len; i++) {
					if (sections[i].block.length) {
						...
						diagnosis = this.makeDiagnosis(null, null, "'" + _helpers.getTitle(this.article, freeBlock.block.$title, "Block '" + freeBlock.rank + "'") + "' : " + syra_local.autoInsertBlockInLastSec, "warning");
			...
			getFreeBlocks: function(sectionRank, context, data) {
				var secR = this.page._$protoLayoutMap.section[sectionRank],
					free = [],
					freeFieldsBind, fieldsBind, i, j, k, block, leni = secR.block.length,
					nbFields, jocker, lenj, lenk, scope, score = 0,
					secA, blockA, excludeBinds, byPassRank = !context.isFusion;
				for (i = 0; i < leni; i++) {
					// Simple detection...
					block = secR.block[i];
					freeFieldsBind = [];
					...
					if (score != nbFields && nbFields && score && (lenj = fieldsBind.length) && (secA = _helpers._getAuthoredSectionForBind(this._$layoutMap, freeFieldsBind[0], secR.$title, context, data))) {
						for (j = 0; j < lenj; j++) {
							if ((blockA = _helpers._searchBlockHoldBind(this._$layoutMap, fieldsBind[j])) && !_helpers._isSameTitle(blockA.$title, block.$title)) {
								jocker = 0;
								...
								if (!jocker || (lenj - 1 > jocker)) {
									...
									score++;
									excludeBinds[fieldsBind[j]] = true;
								}
							}
						}
					}
					// At the end, test garbaged fields (SAM 99604)
					if (score != nbFields && nbFields && score && lenj) {
						for (j = 0; j < lenj; j++) {
							if (this._garbageFields[fieldsBind[j]]) {
								score++;
							}
						}
					}
					...
					if (score == nbFields && nbFields) {
						...
						free.push({
							...
							"excludeBinds": syra_site.clone(excludeBinds)
						});
					} else
					if (!nbFields) {
		...
		"section": {
			...
			$ruleByRank: function(context, freeSection) {
				...
				if (i < lenA || (last = freeSection.rank == (authoSecRank[lenA - 1].rank + 1))) {
					...
					diagnosis = this.makeDiagnosis(null, null, "'" + _helpers.getTitle(this.article, freeSection.$title, "Section '" + freeSection.rank + "'") + "' : " + syra_local.autoInsertSectionInNearestSlot, "warning");
		...
		_processNewSection: function(rankInMapLayoutA, rank, sectionRef, slot, slotRank, slotParentLayout, diag) {
			var i, len, ret, lyTypeA = slotParentLayout.$layoutType,
				secA, lyTypeR = sectionRef.parentLayout.$layoutType;
			var parentLayout = slotParentLayout,
				mess = null,
				tabsLyt, detail, isTab = (lyTypeR == "tabs");
			...
			function _getTabsLyt(layoutMap) {
				var lyt = null;
				for (i = 0, len = layoutMap.section.length; i < len; i++) {
					lyt = layoutMap.section[i].parentLayout;
					if (lyt.$layoutType == "tabs") {
						break;
					}
				}
				return lyt;
			...
			ret = {
				...
				"section": sectionRef,
				"rankOrigin": rank,
				"layoutToInsert": syra_site.clone(sectionRef.parent[sectionRef.parentSlot])
			};
			ret.layout = {
				"slot": slot,
				"rank": slotRank
			...
			if (lyTypeA != lyTypeR) {
				...
				if (!isTab) {
					...
					if (this._$layoutMap.section.length == 1) {
						...
						if (!(tabsLyt = _getTabsLyt(this._$layoutMap))) {
							mess = "autoInsertSectionWarnLayout";
		...
		_processNewBlock: function(rank, blockRef, excludeLyt, slot, slotRank, slotParentLayout, diag) {
			var ret, clonedLyt, i, len, lytKeys, lytToInsert = syra_site.clone(blockRef.parent[blockRef.parentSlot]);
			var excludeMap = [],
			...
			if (slotParentLayout && slotParentLayout.$layoutType == "row") {
				...
				if (slotParentLayout.$items.length == slotRank || slotRank == 0) {
					// Clone cleanup and rebuild parentlayout
					clonedLyt = syra_site.clone(slotParentLayout);
	...
	_checkInBlock: function(block, context) {
		...
		if (fieldRank >= 0 && block.items) {
			targetLayout = _helpers._getBlockLayout(block, context.$bind, where, context, context.dataEx);
			return targetLayout ? {
				"rank": fieldRank,
				"block": block,
				"layout": targetLayout.layout,
