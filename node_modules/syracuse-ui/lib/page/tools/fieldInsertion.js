...
var _helpers = {
	...
	_getGridFieldModes: function(gridArticle, fldBind, context) {
		var _getFieldSrclayout, modes = [],
			proto = syra_fusion.sapUtil.Syra.getFieldProtoFromCollObj(gridArticle, fldBind);
		var i, len = _helpers._gridModes.length;
		if (proto && false) {
	...
	"cmnlevelField": {
		getFieldSrclayout: function($bind, context, data, mode) {
			...
			return layout ? helpers.object.clone(layout, true) : ((!data || !data.strict ? {
				"$bind": $bind
			} : null));
		}
	},
	...
	"levelArticle": {
				...
				for (i = 0; i < leni; i++) {
					...
					if (score == nbFields && nbFields) {
						...
						free.push({
							...
							"excludeBinds": helpers.object.clone(excludeBinds)
						});
					} else
					if (!nbFields) {
		...
		_processNewSection: function(rankInMapLayoutA, rank, sectionRef, slot, slotRank, slotParentLayout, diag) {
			...
			ret = {
				...
				"section": sectionRef,
				"rankOrigin": rank,
				"layoutToInsert": helpers.object.clone(sectionRef.parent[sectionRef.parentSlot], true)
			};
			ret.layout = {
				"slot": slot,
				"rank": slotRank
		...
		_processNewBlock: function(rank, blockRef, excludeLyt, slot, slotRank, slotParentLayout, diag) {
			var ret, clonedLyt, i, len, lytKeys, lytToInsert = helpers.object.clone(blockRef.parent[blockRef.parentSlot], true);
			var excludeMap = [],
