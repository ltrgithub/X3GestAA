...
var _fieldsInsert = require('syracuse-ui/lib/page/tools/fieldInsertion').fieldsInsert;
var _util = require('syracuse-ui/lib/fusion/tools/util');
...
exports.FieldsGarbage = helpers.defineClass(FieldsGarbage, null, {
	...
	_warnAdministrationUpdates: function() {
		this.loadFreeField();
	},
...
exports.FieldsAutoInsert = helpers.defineClass(FieldsAutoInsert, FieldsGarbage, {
	load: function(article, $designLevel) {
		...
		if (!this.page.isAutoInsertFieldDisabled) {
			...
			if (this.loadFreeField(this.page, (this._$layoutMap = {}))) {
				...
				this._result = _fieldsInsert($designLevel, this._isFusionPage, this);
	...
	_getArticleTitle: function() {
		...
		return (code ? ((pref + syra_localizer.getLocalizeText(this.article, code)) || (pref + name)) : (pref + name)) + (viewLabel ? "' - view " + viewLabel : "'");
		...
		if (this._result) {
			...
			if ((result = this._result.field) && result.diagnoses) {
				...
				for (i = 0, len = $diagKeys.length; i < len; i++) {
					...
					if (!_util.isErrDiagnosis(diagField.$severity)) {
			...
			if (statsBlocks || statsFields || statsSections) {
				...
				logger = _util.getLog(_util.getDefaultLogSwitches(document.syraTrans, {}));
				if (failed || (logger && logger.isTraceEnable())) {
					applyChange && this.page.applyChange(change);
					syra_alert.show($diagnoses, this);
				} else {
					logger = _util.getLog({
	...
	insertStats: function(result) {
		...
		for (i = 0, len = $diagKeys.length; i < len; i++) {
			...
			if (_util.isKnownDiagnosisSeverity($diagKeys[i])) {
				if (diag && _util.isErrDiagnosis($diagKeys[i])) {
					ret.ko += diag;
				} else
				if (diag) {
					ret.ok += diag;
	...
	_tracedDiagnoses: function(logger, status, stats, timing, containersDiag, fieldsDiag) {
		...
		function _getLevel(severity) {
			return _util.isErrDiagnosis(severity) ? "error" : (_util.isWarnDiagnosis(severity) ? "warn" : "info");
		}
		logger.trace({
			"severety": "warn"
	...
	makeDiagnosis: function(labelCode, labelCodeArgs, labelString, severity, details, stack) {
		return _util.makeDiagnosisLocale(labelCode, labelCodeArgs, labelString, severity, details, stack, !this._isFusionPage ? "Client framework" : null);
	},
	dispose: function() {
