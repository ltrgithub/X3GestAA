...
helpers.defineClass(BookmarksPage, DesktopPage, {
	...
	notifyChangeToServer: function() {
		_notifyChangeToServer(syra_site.deltaManager.cleanServerIndex(helpers.object.clone(this.dataset, true)));
	},
	...
	_manager.moreBtn = syra_menus.button.add({
		...
		btnclick: function() {
			if (!_manager.morePopupDlg) {
				_manager.morePopupDlg = syra_site.dialogManager.openPopup(syra_site, {
					content: {
				...
				for (var ii = 0, jj = _manager.buttons.length; ii < jj; ii++) {
					var _button = _manager.buttons[ii];
					if (!_button.$isDisabled && _button.isInMore) {
						_button.link.focus();
						break;
					}
				}
...
exports.getFirstActiveButton = function() {
	var _button, _firstButton;

	for (var ii = 0, jj = _manager.buttons.length; ii < jj; ii++) {
		_button = _manager.buttons[ii];
		if (!_button.$isDisabled) {
			_button.isInMore ? _firstButton = _manager.moreBtn : _firstButton = _button;
			break;
		}
	}
...
exports.refresh = function() {
	syra_site.dom.empty(_manager.buttonsSlot);
	syra_site.dom.empty(_manager.morePopupBody);
	syra_menus.button.visibility(_manager.moreBtn, false);
	_resetLinks();
...
function _onManagerClick() {
	...
	_manager.dialog = syra_site.dialogManager.openPage(syra_site, {
