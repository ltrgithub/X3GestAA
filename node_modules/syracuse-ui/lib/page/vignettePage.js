...
exports.VignettePage = helpers.defineClass(VignettePage, DesktopPage, {
		...
		if (this.hasProcessus()) {
			...
			if ($location && $location.$bind) {
				...
				if (menuItems && menuItems.length) {
					syra_site.dom.toggleClass(menuItems[0].domItem, "s-highlight", true);
				}
			}
		}
	},
	...
	onMenuClick: function(menuItem) {
		...
		if (persisLocation) {
			...
			if (masterRecord) {
				syra_site.landingPageMaster.saveLocation(masterRecord, this.vignetteField, menuItem);
			}
		}
		return true;
	},
	onMenusVignetteClick: function(event, picker) {
		syra_menus.showPopup(this, picker);
	},
	ensureMenuBar: function() {
		if (this.$item.$menus) {
			if (!Array.isArray(this.$item.$menus)) {
				this.$item.$menus = syra_page.getDefinedFieldList(this.$item.$menus.$layout.$items);
			}
		}
		...
		if (this.$prototype && this.$prototype.$properties && this.$prototype.$properties.process) {
			...
			if (this.vignetteField) {
				if (!this.vignetteField.btns.items.menus) {
					this.vignetteField.showButton("menus", true, "onMenusVignetteClick");
					this.setArticleId(this.vignetteField.btns.items.menus);
				}
			}
		}
	},
