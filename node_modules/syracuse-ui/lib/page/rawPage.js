...
var notifyHelper = require('./delta/notifyHelper');
...
exports.RawPage = helpers.defineClass(RawPage, Article, {
    scrollToItem: function(item){
        ...
        if (item || item.tabTitle || item.$$item) {
            var target = item.tabTitle || item.$$item[0];
            if (target) {
                if (!this.scroller) {
    ...
    dispose: function(){
        ...
        this._menusPager = this.onAfterApplyPageDataChange = this.authoringPage = null;
    ...
    appendArticleMenus: function(slot){
        ...
        if (!this.$item.$menus) {
            ...
            if (this.$item.$menus.$isMenusBag === undefined) {
                var $layout = this.$item.$menus.$layout;
                if (!($layout && $layout.$items && $layout.$items.length > 1 && $layout.$items[1].$isMenusBag)) {
                    this.$item.$menus.$isMenusBag = true;
                }
            }
        }
        ...
        return this.loadNewItem(slot, this.$item.$menus);
    },
