...
var FieldsAutoInsert = require('syracuse-ui/lib/page/tools/fieldsGarbage').FieldsAutoInsert;
...
exports.RawPage = _helpers.defineClass(RawPage, Article, {
	...
	switchDesigner: function(open) {
		var self = this;
		if (open) {
			var path = self.$designerFileName || "syracuse-ui/lib/authoring/pageDesigner";
	...
	resizeArticle: function() {
		for (var ii = 0, jj = this.resizableList.length; ii < jj; ii++) {
			...
			!item.disposed && (item.displayed !== false) && item.resizeArticle();
		}
	},
	...
	endChange: function(discardRedraw) {
		...
		if (!this.disposed) {
			if (!discardRedraw) {
				...
				syra_site.newEnsureArticleVisibility(this);
			}
		}
