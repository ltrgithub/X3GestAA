...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	scrollToItem: function(item, area, isDomItem) {
		...
		if (item == this) {
			...
			if (target) {
				...
				var areaClientHeight = area.clientHeight;
				targetRect.top -= areaRect.top;
				targetRect.left -= areaRect.left;
				if ((area.scrollHeight - areaClientHeight) > 0) {
					if (targetRect.top < 0) {
						area.scrollTop = Math.max(targetRect.top + (Math.max(area.scrollTop - 20, 0)), 0);
					} else {
						if ((targetRect.top + target.offsetHeight) > areaClientHeight) {
							var newTop = targetRect.top - areaClientHeight;
							var diff = (target.offsetHeight - areaClientHeight);
							if (diff > 0) {
								newTop += (areaClientHeight - 20);
							} else {
	...
	dispose: function() {
		...
		this._menusPager = this.$urlParams = this.openerUrlSegments = this.$views = null;
		Article.prototype.dispose.call(this);
		this.pageMenusBox = this._widgetCategories = this.vignetteField = this.focusField = null;
	...
	applyChange: function(newData) {
		if (!this.disposed && this.dataset) {
			...
			if (this.isPageSpyed) {
				syra_site.spyer.spyDataset(this.dataset, newData);
			}
	...
	onMenuGroupClick: function(menuGroup) {
		if (this.userPreferences && this.menuBar && !this.isVignettePage) {
			var box = menuGroup;
			while (box && box != this.pageMenusBox) {
				box = box.boxParent;
			}
			if (box && !menuGroup.$item.$isPopupContent) {
				this.userPreferences.getForKey("$menuBarGroup", {})[menuGroup.id] = menuGroup.$opened;
	...
	appendPageMenus: function(slot) {
		if (!slot) {
			if (!this.menuBarSlot) {
				slot = this.menuBarSlot = document.createElement("div");
				this.menuBarSlot.className = this.$skin + "-menubar-slot";
				this.menuBarSlot.style.display = "none";
				$(this.menuBarSlot).prependTo(this.domItem);
			}
		}
		this.$mainAction = _$mainActions[this.$facet];
		if (!this.$item.$menus) {
			this.$item.$menus = [{
				$skin: this.$skinMenu || "s-mn-main",
				$category: "menus",
		...
		for (var ii = 0, jj = this.$item.$menus.length; ii < jj; ii++) {
			...
			if ($menus.$skin === undefined) {
				$menus.$skin = this.$skinMenu || "s-mn-main";
			}
		}
	...
	notifyChangeToServer: function(sendBag) {
		...
		if (!self.disposed && !self.isServerNotifyDisabled) {
			if (self.isPageSpyed) {
				syra_site.spyer.spyNotify(sendBag);
			}
	...
	fetch: function(options, callback, error) {
		...
		syra_controller.sendRequest(this, options, callback, error);
	},
	...
	applyShortCuts: function(shortcurts, event, focusField) {
		...
		if (shortcurts.esc) {
			...
			if (shortcurts["delete"]) {
				syra_menus.clickMenu(focusField ? focusField.articleParent : self, "$delete");
				return true;
			}
			if (shortcurts.insert) {
				...
				syra_menus.clickMenu(article || self, "$create");
				return true;
			}
			if (shortcurts.n) {
				syra_menus.clickMenu(self, "$create");
				return true;
			}
			if (shortcurts.p) {
				syra_menus.clickMenu(self, "$print");
				return true;
			}
			if (focusField && ((!shortcurts.h && shortcurts.l) || shortcurts.f12)) {
				focusField.mnPickers.lookup && focusField.mnPickers.lookup.click();
				return true;
			}
			if (shortcurts.r) {
			...
			if (shortcurts.k) {
				syra_menus.clickMenu(self, shortcurts.shift ? "$last" : "$next");
				return true;
			}
			if (shortcurts.j) {
				syra_menus.clickMenu(self, shortcurts.shift ? "$first" : "$previous");
				return true;
			}
			if ((shortcurts[":"] && shortcurts.shift) || shortcurts.s || shortcurts.divide) {
				if (syra_site.searchField) {
					var $searchFields = syra_site.searchField.boundFields.$search;
	...
	openHelp: function(field, $url) {
		...
		if (!$url) {
			if (this.$prototype.$links && this.$prototype.$links.$help) {
				if (field) {
					$url = syra_site.expressionMaker.parse(field.articleParent, this.$prototype.$links.$help.$url) + '#' + field.$item.$bind;
				} else {
					$url = syra_site.expressionMaker.parse(this, this.$prototype.$links.$help.$url);
				}
			}
		}
		if ($url) {
			if (syra_site.browser.isExpected("msie")) {
	...
	getLists: function() {
		var lists = [];
		var $binds = this.boundFields ? Object.keys(this.boundFields) : [];
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
	...
	toggleCssOnEnter: function(domItem, onEnter) {
		if (domItem) {
			syra_site.dom.toggleClass(domItem, "s-enter", onEnter);
		}
	},
