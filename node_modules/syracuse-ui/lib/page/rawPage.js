...
exports.RawPage = helpers.defineClass(RawPage, Article, {
    ...
    initializeNewItem: function(item, $item, boxParent){
        ...
        if (boxParent) {
            ...
            item.articleParent.isContentUpdated = true;
    ...
    ensurePageVisibility: function(place){
        console.log("ensurePageVisibility " + place + ' ' + this.constructor.name + " " + this.isContentUpdated + " loaded " + this.isPageLoaded);
        if (this.isPageLoaded) {
            var isUpdated = this.isContentUpdated;
            if (isUpdated && this.childrenSection) {
            ...
            if (this._articles) {
                for (var ii = 0, jj = this._articles.length; ii < jj; ii++) {
                    var article = this._articles[ii];
                    if (article.isContentUpdated) {
                        if (article.childrenSection) {
                            for (var mm = 0, kk = article.childrenSection.length; mm < kk; mm++) {
                                article.childrenSection[mm].ensureVisibility();
                            }
                        }
                        if (article.layoutContent) {
                            this.layoutValidator.validate(article.layoutContent, true);
                        }
                        isUpdated = true;
                        article.isContentUpdated = false;
                    }
                }
            }
            if (isUpdated) {
    ...
    dispose: function(){
        ...
        if (this.deltaManager) {
            this.deltaManager = this.deltaManager.page = null;
        }
        this.focusedField = this._menusPager = this.$urlParams = this.$urlParts = this.$views = this._$$diagnoseSlot = null;
        Article.prototype.dispose.call(this);
        this.pageMenusBox = this._articles = this.localize = this.layoutValidator = this._widgetCategories = this.vignetteField = this.focusField = null;
    ...
    endChange: function(){
        ...
        this.ensurePageVisibility("applyChange");
        ...
        if (target != this) {
            ...
            if (list.$prototype.$type == "application/x-array") {
                if (value.$delete || value.$create) {
                    ...
                    this.ensurePageVisibility("notifyActionChange");
                    return;
                }
            }
        }
    ...
    showOnEnter: function(domItem, event){
        if (domItem) {
            domItem.style.visibility = (event && event.type == "mouseenter") ? "visible" : "hidden";
        }
    },
