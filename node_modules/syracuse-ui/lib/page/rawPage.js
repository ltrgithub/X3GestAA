...
exports.RawPage = helpers.defineClass(RawPage, Article, {
    ...
    _extractItems: function($items, $menusItems){
        if ($items) {
            for (var ii = 0, jj = $items.length; ii < jj; ii++) {
                ...
                if ($item.$bind) {
                    ...
                    if ($item.$items) {
                        this._extractItems($item.$items, $menusItems);
                    }
                    if ($item.$layout && $item.$layout.$items) {
                        this._extractItems($item.$layout.$items, $menusItems);
                    }
                }
            }
        }
    ...
    appendArticleMenus: function(slot){
        ...
        if (!this.$item.$menus) {
            ...
            if (!Array.isArray(this.$item.$menus)) {
                var $items = [];
                this._extractItems(this.$item.$menus.$layout.$items, $items);
                this.$item.$menus = [{
					$isMenusBag: true,
                    $layout: {
                        $items: $items
        ...
        for (var ii = 0, jj = this.$item.$menus.length; ii < jj; ii++) {
            ...
            if ($menus.$isCollapsable === undefined) {
                $menus.$isCollapsable = true;
            }
            if ($menus.$skin === undefined) {
                $menus.$skin = this.$defaultSkinMenu || "s-mn-bar-main";
            }
        }
        return this.loadNewItem(slot, {
            $category: "section",
            $skin: this.$defaultSkinMenuBox || "s-page-menus-box",
            $layout: {
