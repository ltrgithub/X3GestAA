...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	dispose: function() {
		...
		this.localize = this.layoutValidator = this._widgetCategories = this.vignetteField = null;
	},
	applyChange: function(newData) {
		if (!this.disposed && this.dataset) {
			...
			/*if (newData && this.diagnosesPanel && this.$syraLoaded) {
             this.diagnosesPanel.cleanGlobalDiagnoses();
             } */
	...
	setSectionVisibility: function(section) {
		...
		if ((section.isInvisible !== section.$isHidden) || (!section.isInvisible && !section.$isHidden && section.isItemHidden)) {
			section.hideBox(section.isInvisible);
	...
	applyShortCuts: function(shortcurts) {
		var lastFocusField = document.site.lastFocusField;
		if (shortcurts.esc) {
			if (shortcurts.g) {
				if (shortcurts.h) {
			...
			if (shortcurts.h) {
				if (shortcurts.a || shortcurts.f11 || shortcurts.r) {
					if (this.menuBar) {
						this.menuBar.collapse();
					}
				}
				return true;
			}
			...
			if (shortcurts.left || shortcurts.right) {
				this.openNextTab(shortcurts.right);
				return true;
			}
			if (shortcurts.f1) {
				var focusField = document.site.focusedField;
				if (focusField) {
			...
			if (shortcurts["delete"]) {
				(lastFocusField ? lastFocusField.articleParent : this).clickMenu("$delete");
				return true;
			}
			if (shortcurts.insert) {
				var article;
				if (lastFocusField) {
					article = lastFocusField.articleParent;
					while (!article.disposed && (article != article.page) && !article.menuItems.$create) {
						article = article.articleParent;
					}
				}
				(article || this).clickMenu("$create");
				return true;
			}
			if (shortcurts.n) {
				this.clickMenu("$create");
				return true;
			}
			if (shortcurts.p) {
				this.clickMenu("$print");
				return true;
			}
			if (lastFocusField && (shortcurts.l || shortcurts.f12)) {
				lastFocusField.clickPicker("lookup");
				return true;
			}
			if (shortcurts.r) {
				var list = this.getLastFocusGrid(lastFocusField);
				var lists = list ? [list] : this.getLists();
				if (lists) {
			...
			if (lastFocusField && (shortcurts.m || shortcurts.f4)) {
				if (shortcurts.shift) {
					lastFocusField = lastFocusField.articleParent;
				}
				if (lastFocusField.menusBox && lastFocusField.menusBox.domTitle) {
					$(lastFocusField.menusBox.domTitle).click();
				}
				return true;
			}
			if (shortcurts.k || shortcurts.j) {
				this.clickMenu(shortcurts.k ? (shortcurts.shift ? "$first" : "$previous") : (shortcurts.shift ? "$last" : "$next"));
				return true;
			}
			...
			if (shortcurts.enter || shortcurts.up || shortcurts.down) {
				return this.navRowList(shortcurts.enter, shortcurts.down);
			}
		}
		return false;
	},
	...
	openNextTab: function(isRight) {
		...
		if (!box || box.disposed) {
			...
			for (var ii = 0, jj = this.childrenSection.length; ii < jj; ii++) {
				...
				if (child.tabTitle && child.$opened) {
	...
	navRowList: function(isSelect, isDown, lastFocusField) {
		if (!(lastFocusField && lastFocusField.$$input[0].className === "s-filter-input" && isSelect)) {
			var list = this.getLastFocusGrid(lastFocusField);
			if (list) {
				if (lastFocusField && lastFocusField.$$input[0].className === "s-filter-input") {
					list.$$item.focus();
					document.site.lastFocusField = null;
				}
				var tmp = 0;
				if (isDown) {
					tmp = 1;
