	...
	applyShortCuts: function(shortcurts, event, focusField) {
		...
		if (shortcurts.esc) {
			...
			if (shortcurts.f1) {
				if (focusField) {
					focusField.page.externalAdapter.onFieldClickPicker({
						...
						pickerType: "help",
						doEvent: function() {
							var help = self.$prototype.$links.$help;
							var fragment = focusField.$item.$bind;
							if (help)
								window.open(focusField.articleParent.parseExpression(help.$url) + '#' + fragment, "help");
						}
					});
				} else {
			...
			if (shortcurts["delete"]) {
				(focusField ? focusField.articleParent : this).clickMenu("$delete");
				return true;
			}
			...
			if (shortcurts.enter || shortcurts.up || shortcurts.down) {
				return this.navRowList(shortcurts.enter, shortcurts.down, focusField);
			}
		}
		return false;
	},
