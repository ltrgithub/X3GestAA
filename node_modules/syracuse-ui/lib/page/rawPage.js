...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	scrollToItem: function(item, area, isDomItem) {
		...
		if (item == this) {
			...
			if (target) {
				var targetRect = document.site.dom.getBoundingClientRect(target);
				var areaRect = document.site.dom.getBoundingClientRect(area);
	...
	dispose: function() {
		document.site.siteFunctions.unregisterPage(this);
	...
	applyChange: function(newData) {
		if (!this.disposed && this.dataset) {
			...
			if (this.isPageSpyed) {
				document.site.spyer.spyDataset(this.dataset, newData);
			}
	...
	_initializePage: function() {
		document.site.siteFunctions.registerPage(this);
	...
	savePageDesign: function(saveUserPreferences) {
		...
		if (self.isVignettePage) {
			var $vignette = helpers.object.clone(self.$item, true);
			delete $vignette.$menus;
			if (self.altVignetteKey) {
				(self.vignetteField.$item.$altVignettes = self.vignetteField.$item.$altVignettes || {})[self.altVignetteKey] = $vignette;
			} else {
				self.vignetteField.$item.$vignette = $vignette;
			}
	...
	openHelp: function(field, $url) {
		...
		if ($url) {
			if (document.site.browser.isExpected("msie")) {
	...
	toggleCssOnEnter: function(domItem, onEnter) {
		if (domItem) {
			document.site.dom.toggleClass(domItem, "s-enter", onEnter);
		}
	},
	...
	reloadLayout: function($item, layoutSlot, shouldInsertNewItems) {
		...
		if (prevLayout) {
			document.site.dom.removeChild(prevLayout);
		}
		this.endChange();
		if (autoInsert) {
