...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	applyArticleLocalization: function($article) {
		if ($article.$localization) {
			this.$localization = $article.$localization;
			var langs = Object.keys(this.$localization);
			if (langs.length) {
				var selectedLocale = document.site.userProfile && document.site.userProfile.getSelectedLocale();
				var $currentLocalization;
				if (selectedLocale && selectedLocale.code) {
					$currentLocalization = this.$localization[selectedLocale.code];
				}
				if (!$currentLocalization) {
					$currentLocalization = this.$localization["en-us"] || this.$localization[langs[0]];
				}
				if ($currentLocalization) {
					this.$prototype.$localization = this.$prototype.$localization || {};
					this.ensureDeltaManager().applyObjectDelta(this.$prototype.$localization, $currentLocalization);
				}
			}
		}
	},
	...
	ensurePageVisibility: function() {
		if (this.isPageLoaded) {
			if (this.isContentUpdated && this.childrenSection) {
