...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	onAfterActionMenuExecute: function(menuItem, $menu) {
		switch (menuItem.$bind) {
			case "$save":
				if (this.isActionSuccess(menuItem) && this.dialogWrapper) {
					if (this.dialogWrapper.onSave(menuItem) === false) {
						return false;
					}
				}
				break;
	...
	checkMenuDesignValidaty: function(menuItem) {
		...
		while (articleParent) {
			if (articleParent.designer) {
				return !(articleParent.designer && articleParent.designer.onMenuClick(menuItem) == false);
			}
	...
	initializeNewItem: function(item, $item, boxParent) {
		...
		if (boxParent) {
			...
			if (item.articleParent.registerBoxField) {
				item.articleParent.registerBoxField(item);
			}
		}
		return item;
	},
	registerBoxField: function(item) {
		if (item.boxParent.boxFields && item.$item && item.$item.$bind) {
			item.boxParent.boxFields[item.id] = item;
		}
	},
	...
	dispose: function() {
		...
		this.$urlParams = this.$urlParts = this.$views = this._$$diagnoseSlot = null;
		this.focusedField = this._menusPager = null;
		Article.prototype.dispose.call(this);
		this.localize = this.layoutValidator = this._widgetCategories = this.vignetteField = this.focusField = null;
		this.lastFocusGrid = this.externalAdapter = null;
	},
	...
	ensureSendBag: function(article) {
		...
		if (parent.sendBag && !parent.sendBag.on412 && this.dataset.$etag != parent.sendBag.$etag) {
	...
	attachToPageResizeListener: function(attach, item) {
		if (item && this.resizeListener) {
			if (attach) {
				this.resizeListener.add(item);
			} else {
				this.resizeListener.remove(item);
			}
		}
	},
		...
		if (target != this) {
			...
			if (list.$prototype.$type == "application/x-array") {
				if (value.$delete || value.$create) {
					list.notifyActionChange(target, value, notifyServer);
					return;
				}
			}
		}
	...
	notifyDataChange: function(field, value) {
		...
		if (sessionStorage && this.openerHttpQuery) {
			var key = this.openerHttpQuery.$url;
			var data = sessionStorage.getItem(key) !== "" ? JSON.parse(sessionStorage.getItem(key)) || {} : {};
			data.wcpreloadUrl = data.wcpreloadUrl || this.openerHttpQuery.workingCopyPreloadUrl;
			data.dataset = this.dataset;
			sessionStorage.setItem(key, JSON.stringify(data));
		}
	},
	getDataUrl: function() {
		var $parsedUrl = this.parseExpression(this.dataset.$url || this.$prototype.$url);
	...
	notifyChangeToServer: function(sendBag) {
		...
		if (!self.disposed && !self.isServerNotifyDisabled) {
			...
			if (self.$isAutoSaveEnabled) {
				(sendBag.$actions = sendBag.$actions || {}).$save = {
					$isRequested: true
				};
			}
	...
	setSectionVisibility: function(section) {
		//(!section.isInvisible && !section.$isHidden && section.isItemHidden) test for menugroup et core in working copy
		if ((section.isInvisible !== section.$isHidden) || (!section.isInvisible && !section.$isHidden && section.isItemHidden)) {
			section.hideBox(section.isInvisible || section.$isHidden);
			if (section.isInvisible && section.tabTitle) {
				if (section.layoutParent && section.layoutParent.tabOpened == section) {
					this.layoutValidator.validate(section.layoutParent, true);
				}
			}
			this.validateSectionsVisibility(section.boxParent);
		} else {
			//temp hack mcovergnece bloc section
			if (section.boxParent && section.boxParent.isMenuGroup && !section.isInvisible) {
				this.validateSectionsVisibility(section.boxParent);
			}
		}
	},
	validateSectionsVisibility: function(section) {
		if (section != this && section.childrenSection) {
			section.isInvisible = true;
			for (var ii = 0, jj = section.childrenSection.length; ii < jj; ii++) {
				if (!section.childrenSection[ii].isInvisible) {
					section.isInvisible = false;
					break;
				}
			}
			this.setSectionVisibility(section);
		}
	},
	applyEscape: function() {
		if (this.dialogWrapper && this.dialogWrapper.$$closeBtn) {
			this.dialogWrapper.$$closeBtn.click();
			return true;
		}
		/*if (this.dialogWrapper) {
	...
	applyShortCuts: function(shortcurts, event, focusField) {
		...
		if (shortcurts.esc) {
			...
			if (shortcurts.enter || shortcurts.up || shortcurts.down) {
				if (!(focusField && focusField.$item.$isFilterMode && shortcurts.enter)) {
	...
	openHelp: function(field, $url) {
		...
		if (!$url) {
			if (field) {
				if (this.$prototype.$links.$help) {
					$url = field.articleParent.parseExpression(this.$prototype.$links.$help.$url) + '#' + field.$item.$bind;
				}
			} else {
				$url = this.parseExpression(this.$prototype.$links.$help.$url);
			}
		}
		if ($url) {
	...
	navRowList: function(isSelect, isDown, focusField) {
		if (!(focusField && focusField.$item.$isFilterMode && isSelect)) {
			var list = this.getLastFocusGrid(focusField);
			if (list) {
				if (focusField && focusField.$$input[0].className === "s-filter-input") {
					list.$$item.focus();
				}
				var tmp = 0;
				if (isDown) {
				...
				if (list.records && list.records.length > 0) {
					var index = 0;
					if (list.selector.records && Object.keys(list.selector.records).length > 0) {
						var key = Object.keys(list.selector.records);
						var recordselected = list.selector.records[key];
						index = list.records.indexOf(recordselected) + tmp;
					}
					if (index >= 0 && index < list.records.length) {
						var recordToSelect = list.records[index];
						var key = Object.keys(recordToSelect.idMap)[0];
						/*if (!isSelect) {
                         recordToSelect.list.applyShortCut = true;
                         }*/
