...
exports.RawPage = helpers.defineClass(RawPage, Article, {
	...
	ensurePageVisibility: function() {
		if (this.isPageLoaded && !this._isDataChanging) {
			...
			this.ensureChildListsVisibility();
			if (this.pageMenusBox) {
				syra_site.layoutValidator.validate(this.pageMenusBox.layoutContent, true);
			}
			syra_site.layoutValidator.validate(this.layoutContent, true);
			this.resizePage();
		}
	},
	resizePage: function(resize) {
		this.resizeChildLists(resize);
		this.resizeAutoSizeFields();
	},
	dispose: function() {
		syra_site.pageLoader.disposePage(this);
		this._menusPager = this.$urlParams = this.openerUrlSegments = this.$views = null;
		Article.prototype.dispose.call(this);
		this.pageMenusBox = this.localize = this._widgetCategories = this.vignetteField = this.focusField = null;
	...
	applyShortCuts: function(shortcurts, event, focusField) {
		...
		if (shortcurts.esc) {
			...
			if (focusField && ((!shortcurts.h && shortcurts.l) || shortcurts.f12)) {
				focusField.lookupPicker && focusField.lookupPicker.click();
				return true;
			}
			if (shortcurts.r) {
			...
			if (shortcurts.enter || shortcurts.up || shortcurts.down) {
				if (!(focusField && focusField.$item && focusField.$item.$isFilterMode && shortcurts.enter)) {
					...
					if (list) {
						if (focusField && focusField.input && focusField.input.className === "s-filter-input") {
							list.domItem.focus();
							return true;
						} else
						if (focusField) {
