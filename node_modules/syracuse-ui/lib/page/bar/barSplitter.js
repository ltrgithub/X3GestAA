	...
	load: function(page) {
		...
		if (!this.options.$isResizerDisabled || this.options.$isCollapsable) {
			...
			this.openButton = syra_menus.button.add({
				...
				iconOnly: true,
				shortCutTip: ((this.options.resizeDirection == "left") ? shortCutsTip["hideLeftBar"] : shortCutsTip["hideRightBar"]) + ", " + shortCutsTip["hideFull"],
	...
	ensureState: function(startResize) {
		if (!this.isHidden && this.isVertical) {
			if (startResize && this.$isCollapsed) {
				...
				syra_site.dom.hide(this.body, false);
			} else {
