...
exports.BarSplitter = helpers.defineClass(BarSplitter, null, {
	_appplyPreferences: function() {
		var defaultState;
		var defaultViewMode = this.options.$viewMode || (document.site.isTableDevice() ? "float" : "dock");
		...
		if (this.options.$viewMode || !this.$viewMode) {
	...
	resizeBar: function() {
		if (this._bar && this.barSlot) {
			var height = document.site.getInnerSize(this.page.getPageSlot()).height;
			if (height) {
				this.ensureState();
				...
				if (this.$viewMode == "float") {
					var scrollviewRect = this.page.layoutSlot.getBoundingClientRect();
				...
				if (this.swipeBar) {
					this.swipeBar.style.height = height + "px";
					this.openerPicker.style.display = "";
					this.openerPicker.style.top = ((height - this.openerPicker.clientHeight) / 2) + "px";
				}
				return true;
