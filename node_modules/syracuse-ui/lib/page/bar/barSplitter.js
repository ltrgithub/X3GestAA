	...
	load: function(page) {
		...
		if (!this.options.$isResizerDisabled || this.options.$isCollapsable) {
			...
			this.openButton = syra_menus.button.add({
				...
				iconOnly: true,
				shortCutTip: "ESC H " + ((this.options.resizeDirection == "left") ? "L" : "R") + ", ESC F11",
	...
	ensureState: function(startResize) {
		if (!this.isHidden && this.isVertical) {
			...
			if (this.resizePicker) {
				...
				syra_menus.updateButtonIcon(this.openButton.link, this.$isCollapsed ? syra_local.bar_expand : syra_local.bar_collapse, icon + "_arrow");
			}
		}
	},
