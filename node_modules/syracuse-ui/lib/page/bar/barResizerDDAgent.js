...
function _move(resizer, event) {
	...
	if ((event.pageY >= (resizer.top - 20)) && (event.pageY <= (resizer.bottom + 20))) {
		...
		if (newWidth > 0) {
			...
			resizer.barSplitter.page.setParentSpace(resizer.barSplitter, newWidth);
		}
	}
}
...
exports.BarResizerDDAgent = helpers.defineClass(BarResizerDDAgent, null, {
	start: function(dropItem) {
		...
		syra_site.dom.toggleClass(this.barSplitter.openButton, "s-resizing", true);
	},
	...
	onDragMouseUp: function(target, event) {
		...
		syra_site.dom.toggleClass(this.barSplitter.openButton, "s-resizing", false);
