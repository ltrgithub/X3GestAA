...
exports.showBreadCrumb = function(page) {
	...
	if (page.$breadCrumb && page.$breadCrumb.length > 0) {
		...
		for (var ii = 0, jj = page.$breadCrumb.length - 1; ii <= jj; ii++) {
			var item = addItem(page.$breadCrumb[ii].title, ii == jj);
		...
		var removedRight = true;
		while (page.scrollview.scrollWidth < sizeBC && arr.length > 3) {
			// set ... for the middle one
			if (arr[middle].firstChild.textContent.indexOf("...") === 0) { // if it's already set ... we have to remove the previous one of the next one
				if (middle - 1 > 0 && !removedRight) {
					arr[middle - 1].textContent = "";
					arr.splice(middle - 1, 1);
				} else if (middle + 1 < arr.length - 1 && removedRight) {
					arr[middle + 1].textContent = "";
					arr.splice(middle + 1, 1);

				}
			} else {
				arr[middle].firstChild.textContent = "...";
