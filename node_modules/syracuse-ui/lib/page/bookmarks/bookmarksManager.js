...
var _dataset, _moreBtn, _morePopupBody, _addBtn, _manageBtn, _linksSlot, _links, _morePopupDlg;
...
exports.dispose = function() {
	...
	_dataset = _moreBtn = _morePopupBody = _addBtn = _manageBtn = _linksSlot = _links, _morePopupDlg = _dialog = null;
};
...
function _resetLinks() {
	...
	_fusionMenuItems = {};
	_syracuseMenuItems = {};
	for (var ii = 0, jj = _links.length; ii < jj; ii++) {
		delete _links[ii].menuItem;
	}
	_links = [];
}
		...
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			...
			if (item.menuItem) {
				if (item.menuItem.$url) {
					var link = syra_menus.addTextButton(item.menuItem.title || item.menuItem.description || "???", "s-bookmarks-bar-link", "onBookmarClick");
					if (item.menuItem.$hasSeparator) {
						link.syraHasSeparator = true;
						link.className += " s-separator";
					}
					link.syraIsBarLink = true;
					link.syraItem = syra_site.id;
					link.title = item.menuItem.description;
					var seg = item.menuItem.urlSegment = urlMaker.parse(item.menuItem.$url, true);
					if (seg.host) {
						//set relative url
						seg.isSyracuse = true;
					...
					if (urlMaker.replaceSegmentEndpoint(seg, selectedEndpoint, true)) {
						item.menuItem.$url = urlMaker.build(seg);
						link.syraIsBookmarkEnabled = true;
					} else {
					...
					_links.push(_linksSlot.appendChild(link));
				} else {
...
function _toggleButton(btn, isOn) {
	syra_menus.updateButtonIcon(btn, isOn ? syra_local.bookmarksRemove : syra_local.bookmarksAdd, isOn ? "bookmark_on" : "bookmark_off");
}
...
exports.refresh = function() {
	syra_site.dom.empty(_linksSlot);
	syra_site.dom.empty(_morePopupBody);
	_moreBtn.style.visibility = "hidden";
	_resetLinks();
	...
	if (activePage && !activePage.disposed && activePage != _lastCheckedPage) {
		...
		if (activePage) {
			...
			if (activePage.isFusionPage) {
				...
				urlSegment = syra_site.history.getUrlSegment(activePage.openerUrlSegments.$url);
			}
		...
		for (var ii = 0, jj = _links.length; ii < jj; ii++) {
			var link = _links[ii];
			if (link.menuItem) {
				syra_site.dom.toggleClass(link, "s-on", link.syraBookMarkOn = menuItem && link.menuItem == menuItem);
				syra_site.dom.disableItem(link, !link.syraIsBookmarkEnabled || ((activePage && activePage.isFusionPage) && !link.menuItem.urlSegment.isFusion));
			}
		}
		_manageBtn.style.visibility = _count ? "visible" : "hidden";
		var max = syra_site.siteOptions.$item.$maxBookmarks || 25;
		if (_count >= max) {
			isBookmarkEnabled = false;
		}
		syra_site.dom.disableItem(_addBtn, !isBookmarkEnabled);
...
exports.onResize = function() {
	...
	_linksSlot.style.width = Math.max(0, Math.max(0, maxW - _linksSlot.getBoundingClientRect().left) - _moreBtn.clientWidth) + "px";
	if (_linksSlot.clientWidth <= (_linksSlot.scrollWidth - 2)) {
		for (var ii = _links.length - 1; ii >= 0; ii--) {
			var link = _links[ii];
			if (!link.syraIsInMore) {
				...
				if (link.syraIsDisabled || !link.syraIsBookmarkEnabled) {
					syra_site.dom.disableItem(link, true);
				}
				if (link.syraBookMarkOn) {
					link.className += " s-on";
				}
				_morePopupBody.insertBefore(link, _morePopupBody.firstChild);
			}
			if (_linksSlot.clientWidth >= (_linksSlot.scrollWidth - 2)) {
				break;
			}
	...
	_moreBtn.style.visibility = _morePopupBody.children.length > 0 ? "visible" : "hidden";
};
...
exports.load = function() {
	...
	syra_site.onBookmarClick = function(event, btn) {
		if (btn == _addBtn) {
			_onAddClick();
		}
		if (btn == _manageBtn) {
		...
		if (btn.syraIsBarLink) {
			!btn.syraIsDisabled && exports.onLinkClick(btn.menuItem);
		}
		if (btn.syraIsNavigationItem) {
			_onNavigationMenuItemPickerClick(syra_store.get(btn.syraFieldId));
		}
	};

	_links = [];
	_addBtn = syra_menus.addIconButton(syra_local.bookmarksAdd, "s-bookmarks-bar-btn", "onBookmarClick", null, "bookmark_off");
	_addBtn.syraItem = syra_site.id;

	_manageBtn = syra_menus.addIconButton(syra_local.bookmarksManage, "s-bookmarks-bar-btn", "onBookmarClick", null, "expand");
	_manageBtn.syraItem = syra_site.id;
	_manageBtn.style.visibility = "hidden";

	_linksSlot = document.createElement("div");
	_linksSlot.id = "s-bookmarks-bar-links";

	_moreBtn = syra_menus.addTextButton(syra_local.bookmarksMore, "s-bookmarks-more-btn", "onBookmarClick");
	_moreBtn.syraItem = syra_site.id;
	_moreBtn.title = syra_local.bookmarksMoreTooltip;
	_moreBtn.style.visibility = "hidden";

	syra_site.headerBottom.appendChild(_addBtn);
	syra_site.headerBottom.appendChild(_manageBtn);
	syra_site.headerBottom.appendChild(_linksSlot);
	syra_site.headerBottom.appendChild(_moreBtn);

	_morePopupBody = document.createElement("div");
	_morePopupBody.className = "s-mn-popus-body";
};