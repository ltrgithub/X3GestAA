...
exports.add = function(module, parentNode, dataset) {
	...
	sub.$skin = sub.page.$skin + "-submodule";
	sub.slot = syra_dom.li(sub.$skin);
	sub.slot.syrainout = sub.id;
	sub.title = syra_dom.div(sub.$skin + "-title", sub.slot);
	sub.title.textContent = dataset.title;

	if (sub._useLargeMode(dataset && dataset.items)) {
		sub.mode = "large";
		sub.body = syra_dom.ul(sub.$skin + "-body-large");
		sub.$skin_menu = sub.$skin + "-body-large-link";
	} else {
		sub.mode = "small";
	...
	for (var ii = 0, jj = dataset.items.length; ii < jj; ii++) {
		...
		if (dataItem) {
			var child = item.menuItem ? _menuItem.add(sub) : _subModuleMenuBlock.add(sub);
			child.load(dataItem);
			if (sub.megaDetailPanel) {
				sub.megaDetailPanel.firstColumnSlot().appendChild(child.slot);
			} else {
				sub.body.appendChild(child.slot);
			}
