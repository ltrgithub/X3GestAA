...
SubModule.prototype.onActivated = function() {
	if (this.mode == "large") {
		...
		this.page.popupBody.appendChild(this.body);
		this.megaMenuColumns.adjustColumnContent(this.maxColumns);
...
exports.add = function(module, parentNode, dataset) {
	...
	sub.megaParent = module;

	sub.setVolumeSize(dataset);
	sub.maxColumns = (sub.megaParent.page.isHighRes && sub.mode === "large") ? 5 : 4;
	...
	if (sub.mode == "small") {
		sub.megaScrollPanel = syra_dom.div("simplebar", sub.body);

		sub.megaScrollPanel.id = "contentPanelScrollBar";

		sub.megaMenuColumns = _megaMenuColumns.add(sub.megaScrollPanel, sub.body);
	...
	for (var ii = 0, jj = dataset.items.length; ii < jj; ii++) {
		...
		if (dataItem) {
			...
			if (sub.mode == "large") {
				...
				slot = syra_dom.li("s-nav-mega-link-parent", sub.megaMenuColumns.firstColumnSlot());
			}

			var child;
			if (item.menuItem) {
				child = _menuItem.add();
				child.isSubmoduleLink = sub.mode == "large";
			} else { //item.menuBlock
				child = _subModuleMenuBlock.add();
				child.isHLayout = sub.mode == "large";
			}
			sub.children.push(child);
			child = syra_item.initialize(sub.page, child);
			child.slot = slot;
			child.megaParent = child.submodule = sub;
			child.load(dataItem);
