...
function SubModule(module, parentNode, dataset) {
...
SubModule.prototype.addItems = function(items, parent) {
	...
	for (var ii = 0, jj = items.length; ii < jj; ii++) {
		...
		if (dataItem) {
			var child = item.menuItem ? _menuItem.add(parent) : _subModuleMenuBlock.add(parent);
			child.load(dataItem);
			if (parent.megaDetailPanel) {
				parent.megaDetailPanel.firstColumnSlot().appendChild(child.slot);
			} else {
				parent.body.appendChild(child.slot);
			}
