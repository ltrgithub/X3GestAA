...
exports.SubModuleBuilder = helpers.defineClass(SubModuleBuilder, null, {
	...
	list_onAfterSetDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		if (this.list.records.length && this.list.selector.uuid == undefined && this.list.selector.selectRecord) {
			this.list.selector.selectRecord(this.list.page.preferences.$submodule, true, true);
		}
	},
	...
	record_draw: function(record) {
		...
		record.header.appendChild(record.domTitle).className = record.$skin + "-title";
		...
		if (record.page.isAdminMode) {
			...
			record.btnExpandSubModule = syra_menus.addIconButton(syra_local.nvpCollapseSubModule, "s-nav-submodule-expand", "onExpandSubModule", null, "node_expanded");
			record.btnExpandSubModule.syraItem = record.id;
			record.header.insertBefore(record.btnExpandSubModule, record.domTitle);
		...
		if (record.page.isAdminMode) {
			...
			record.btnExpandAll = record.header.appendChild(syra_menus.addIconButton(syra_local.nvpExpandAll, "s-nav-submodule-expand", "onExpandAll", null, "expand"));
			record.btnExpandAll.syraItem = record.id;
			record.btnCollapseAll = record.header.appendChild(syra_menus.addIconButton(syra_local.nvpCollapseAll, "s-nav-submodule-expand", "onExpandAll", null, "collapse"));
		...
		record.page.isAdminMode && this.list.capabilityMaker.addReorder(record);

		record.cardItem = record.page.loadNewItem(record.domTitle, {
			$bind: "title",
			$category: "field",
