...
function _expandSubModule(subModule, expanded) {
	syra_site.dom.hide(subModule.body, !(subModule.isExpanded = expanded));
	...
	//syra_site.dom.hide(subModule.btnExpandAll, !(subModule.btnExpandAll.hasChildBlock && expanded));
	//syra_site.dom.hide(subModule.btnCollapseAll, !(subModule.btnCollapseAll.hasChildBlock && expanded));
}
...
exports.SubModuleBuilder = helpers.defineClass(SubModuleBuilder, null, {
	initialize: function(list) {
		...
		list.emptyBody = function() {
			...
			if (this.page.isAdminMode) {
				syra_site.dom.empty(this.articleParent.subModulesAdminTitleList);
	...
	record_onBeforeApplyChange: function(record, metaData, serverResponse) {
		serverResponse && syra_delta.applyObjectDelta(record.page, record.dataset, metaData);
	},
	record_onAfterApplyChange: function(record, metaData) {
		...
		syra_menus.button.setText(record.subModuleTitleBtn, title);
	},
	...
	record_onSelect: function(record, selected) {
		if (record.page.isAdminMode) {
			...
			syra_site.dom.hide(record.domItem, !selected);
			if (selected) {
	...
	record_draw: function(record) {
		...
		if (record.page.isAdminMode) {
			record.subModuleTitleBtn = syra_menus.button.add({
				isIndicator: true,
				parent: record,
				slot: record.header,
				text: "",
			...
			syra_site.dom.hide(record.domItem, true);
		...
		if (record.page.isAdminMode) {
			...
			record.btnExpandAll = syra_menus.button.add({
				parent: record,
				slot: slot,
			...
			record.btnCollapseAll = syra_menus.button.add({
				parent: record,
				slot: slot,
