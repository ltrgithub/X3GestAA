	...
	list_onAfterDraw: function() {
		...
		if (!this.list.isBookMarkList) {
			if (this.list.articleParent.isSubModuleRecord) {
				this.list.menusSlot.className = "s-nav-admin-submodule-items-menus";
				this.list.articleParent.body.insertBefore(this.list.menusSlot, this.list.articleParent.body.firstChild);
			} else {
		...
		if (createMn) {
			for (i = 0, len = createMn.length; i < len; i++) {
				syra_menus.setMenuItemTitle(createMn[i], syra_local.nvpMenusCreateBlock);
			}
		}
	},
