...
function _initNavigationList(builder) {
	builder.emptyMessage = syra_local.nvpNoMenuItem;
	builder.list.RecordReorderDDAgentClass = NavigationItemReorderDDAgent;
	builder.list._ensureMenuSlot = function() {
		var list = builder.list;
		if (!list.menusSlot) {
			if (list.articleParent.isSubModuleRecord) {
				list.menusSlot = document.createElement("div");
				list.menusSlot.className = "s-nav-admin-submodule-items-menus";
				list.articleParent.body.insertBefore(list.menusSlot, list.articleParent.body.firstChild);
			} else {
				list.menusSlot = list.articleParent.titleField.domItem;
			}
		}
		return list.menusSlot;
	};
}

function _initNavigationRecord(builder, record) {
	record.onFocusInOut = function(onEnter) {
		record.singleField && _highlightBookmark(record, onEnter);
	};
	record.onAddMenuPicker = function(picker) {
		record.domItem.appendChild(picker);
	};
}

function _initBookmarkList(builder) {
	builder.emptyMessage = syra_local.bookmarksNoMenuItem;
	builder.list._ensureMenuSlot = function() {
		var list = builder.list;
		if (!list.menusSlot) {
			list.menusSlot = document.createElement("div");
			list.menusSlot.className = "s-list-menus-slot";
			var bar = list.ensureTopBar();
			bar.insertBefore(list.menusSlot, bar.firstChild);
		}
		return list.menusSlot;
	};
	builder.list.onAddMenuItem = function($bind, $menu) {
		return null;
	};
}

function _initBookmarkRecord(builder, record) {
	_initNavigationRecord(builder, record);
...
exports.NavigationItemBuilder = helpers.defineClass(NavigationItemBuilder, null, {
	initialize: function(list) {
		this.list = list;
		list.isBookMarkList ? _initBookmarkList(this) : _initNavigationList(this);
	},
	...
	record_onItemInOut: function(record, onEnter, event) {
		for (var ii = 0, jj = record._buttons.length; ii < jj; ii++) {
			syra_site.dom.toggleClass(record._buttons[ii], "s-invisible", !onEnter);
		}
	...
	record_draw: function(record) {
		this.list.isBookMarkList ? _initBookmarkRecord(this, record) : _initNavigationRecord(this, record);
