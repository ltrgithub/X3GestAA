...
exports.NavigationItemBuilder = helpers.defineClass(NavigationItemBuilder, null, {
			...
			switch (menuItem.$sourceBind) {
				...
				case "$delete":
					if (record.singleField.$variantItemKey == "menuItem") {
						...
						if (this.list.isBookMarkList) {
							menuItem.$confirm = syra_expression.parse(menuItem.articleParent.dataset.$singleField.menuItem, syra_local.nvpConfirmDeleteBookmark, menuItem.articleParent.dataset.$singleField.menuItem);
						} else {
				...
				case "$edit":
				case "$canceledit":
					record.page.switchBookmarkEditTitle(record, menuItem.$sourceBind == "$edit");
					return false;
			}
			return true;
		}
	...
	record_draw: function(record) {
		...
		if (this.list.isBookMarkList) {
			record.onFormUpdate = function(field, value) {
