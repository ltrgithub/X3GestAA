...
exports.NavigationItemList = helpers.defineClass(NavigationItemList, SingleList, {
	initializeNavigationList: function() {
		this.isNavigList = true;
		this.RecordClass = NavigationItemRecord;
		this.$item.$isMenusHidden = !this.page.isAdminMode;
		...
		if (!this.articleParent.isSubModuleRecord) {
			this.$itemMenuSkin = "s-mn-ldp-record-link";
			this.$itemMenuIconMode = "icon";
			this.$iconPath = "page/s-nav-";
		} else {
	...
	loadBox: function(initData) {
		...
		this.subModuleRecord && this.subModuleRecord.onLoadNavigationItem(this);
	},
	...
	_ensureMenuSlot: function() {
		if (!this.menusSlot) {
			if (this.articleParent.isSubModuleRecord) {
				...
				this.menusSlot = this.articleParent.titleField.domItem;
			}
		}
		return this.menusSlot;
	},
	dispose: function() {
		this.subModuleRecord = this.cols = null;
	...
	onAppendRecord: function(newRecord) {
		if (this.columnsCount > 1) {
			...
			this.cols[this.currentCol++].slot.appendChild(newRecord.domItem);
			if (this.currentCol == this.columnsCount) {
				this.currentCol = 0;
			}
		} else {
			this.body.appendChild(newRecord.domItem);
		}
	}
});