...
exports.NavigationItemList = helpers.defineClass(NavigationItemList, SingleList, {
	loadBox: function(initData) {
		...
		this.emptyMessage = this.localize.nvpNoMenuItem;
	...
	emptyBody: function(addEmptySlot, onFetch) {
		...
		if (this.emptyDataItem) {
			this.emptyDataItem.setAttribute("data-s-inout", this.id);
			this.emptyDataItem.isDroppable = false;
			this.emptyDataItem.defautTextContent = this.emptyDataItem.textContent;
			this.emptyDataItem.setAttribute("data-s-empty-list", this.id);
		}
	},
	onItemEnterLeave: function(event) {
		if (event && (event.currentTarget == this.emptyDataItem)) {
			var ddAgent = document.site.ddManager.ddAgent;
			var isDroppable = !! (event.type == "mouseenter" && ddAgent && ddAgent.isNavigationItemReorderDDAgent);
			if (this.emptyDataItem.isDroppable != isDroppable) {
				this.emptyDataItem.isDroppable = isDroppable;
				if (isDroppable) {
					this.emptyDataItem.textContent = this.localize.nvpDropOnEmpty;
