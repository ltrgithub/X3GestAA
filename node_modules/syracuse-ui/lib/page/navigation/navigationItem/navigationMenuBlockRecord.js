...
exports.NavigationMenuBlockRecord = helpers.defineClass(NavigationMenuBlockRecord, ChildFieldRecord, {
	drawBox: function() {
		...
		if (!this.itemsField) {
			...
			this.itemsField.menusSlot.className = this.page.isAdminMode ? "s-nav-admin-items-menus-slot" : "s-nav-items-menus-slot";
		} else {
		...
		if (itemRecord.reorderPicker) {
			this.expandPicker.parentNode.insertBefore(itemRecord.reorderPicker, this.expandPicker);
		}
		if (this.page.isAdminMode) {
	...
	onLocalizeField: function(openerMenu, data, response, $url) {
	...
	addExpandPicker: function() {
		if (!this.expandPicker) {
			...
			this.expandPicker.syraItem = this.id;
			this.expandPicker.syraOnClick = "onExpandClick";
			this.expandPicker.className = this.page.isAdminMode ? "s-nav-admin-block-expand" : "s-nav-block-expand";
			this.expandPicker.title = syra_local.box_collapse;
			if (this.$item.$css) {
	...
	ensureExpandedState: function() {
		this.expandPicker.title = this.$expanded ? syra_local.box_collapse : syra_local.box_expand;
		syra_site.dom.toggleClass(this.expandPicker, "s-close", !this.$expanded);
	...
	onItemInOut: function(onEnter) {
		if (this.page.isAdminMode) {
			if (this.itemsField && this.itemsField.menusSlot) {
				this.page.toggleCssOnEnter(this.itemsField.menusSlot, onEnter);
			}
			this.page.toggleCssOnEnter(this.titleField.boxPickers, onEnter);
			this.page.toggleCssOnEnter(this.articleParent.articleParent.menusSlot, onEnter);
			this.page.toggleCssOnEnter(this.articleParent.articleParent.reorderPicker, onEnter);
		}
	},
