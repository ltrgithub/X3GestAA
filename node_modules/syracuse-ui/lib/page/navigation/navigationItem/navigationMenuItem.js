...
exports.NavigationMenuItem = helpers.defineClass(NavigationMenuItem, ReferenceField, {
	...
	onItemEnterLeave: function(onEnter, record) {
		if (record) {
		...
		ReferenceField.prototype.onItemEnterLeave.call(this, onEnter);
	},
				...
				for (var ii = 0, jj = _$executes.length; ii < jj; ii++) {
					...
					if ($execute) {
						...
						if (value.convergenceFunction) {
							$execute.$description = this.page.localize.nvpFunction + ": " + value.convergenceFunction;
						} else {
							if (value.entity) {
								$execute.$description = this.page.localize.nvpEntity + ": " + value.entity;
								if (value.entity != helpers.string.pluralize(value.representation || "")) {
									$execute.$description += " (" + this.page.localize.nvpRepresentation + ": " + value.representation + ")";
								}
							}
						}
						...
						if (this.articleParent.menusSlot) {
							if (this.page.isAdminMode) {
								...
								document.site.dom.removeChild(this.articleParent.menusSlot);
							}
						}
						break;
					}
	...
	addBookmarkPicker: function(item) {
		if (this.codeMenu.href) {
			this.bookmarkPicker = document.createElement("a");
			this.bookmarkPicker.setAttribute("data-s-picker", this.bookmarkPicker.syraPickerType = "s-nav-menu-bookmark");
			this.bookmarkPicker.syraSiteObserver = "bookmarksManager";
			this.bookmarkPicker.syra_field_id = this.id;
			this.bookmarkPicker.className = "s-nav-menu-bookmark";
			this.fieldValue.parentNode.insertBefore(this.bookmarkPicker, this.fieldValue.nextSibling);
			document.site.bookmarksManager.checkNavigationMenuItemBookMark(this);
		}
	}
});