...
exports.NavigationMenuItem = helpers.defineClass(NavigationMenuItem, ReferenceField, {
	loadBox: function() {
		...
		this.$item.$skin = this.page.isAdminMode ? "s-bookmark-field" : "s-nav-menu";
				...
				for (var ii = 0, jj = _$executes.length; ii < jj; ii++) {
					...
					if ($execute) {
						...
						if (this.articleParent.reorderBtn) {
							this._dataValue.parentNode.insertBefore(this.articleParent.reorderBtn, this._dataValue);
						}
						if (this.articleParent.menusSlot) {
							this._dataValue.parentNode.insertBefore(this.articleParent.menusSlot, this._dataValue);
						}
						break;
					}
				}
	...
	addBookmarkButton: function(item) {
		if (this.codeMenu.href) {
			this.bookmarkBtn = syra_menus.button.add({
				parent: this,
				text: syra_local.bookmarksAdd,
				css: this.page.isAdminMode ? "s-nav-admin-menu-bookmark" : "s-nav-menu-bookmark",
				iconOnly: true,
				fontIcon: "bookmark_off",
				btnclick: syra_site.bookmarksManager.onNavigationMenuItemAddClick
			});
			this._dataValue.parentNode.insertBefore(this.bookmarkBtn.link, this._dataValue.nextSibling);
