...
exports.ItemReorderDDAgent = helpers.defineClass(ItemReorderDDAgent, null, {
		...
		if (event.target) {
			if (this.boundaryItem.contains(event.target)) {
				...
				while (target && target != this.boundaryItem) {
					if (target.syraRecord !== undefined && syra_store.findRecord(this.list, target)) {
						if (this.reorderRecord) {
							this.moveDragImage(event, syra_store.get(target.syraRecord));
					...
					if (target.syraIsEmptyItem) {
						if (this.reorderRecord) {
							var record = {
								domItem: target,
								list: syra_store.get(target.syraIsEmptyItem)
							};
	...
	onDragMouseUp: function(target, event) {
		if (this.targetRecord) {
			...
			var target_sendBag = syra_workingCopy.getSendBag(target_list);
			...
			if (target_list != source_list) {
				...
				var source_sendBag = syra_workingCopy.getSendBag(source_list);
