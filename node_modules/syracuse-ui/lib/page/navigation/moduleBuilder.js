...
exports.ModuleBuilder = helpers.defineClass(ModuleBuilder, null, {
	...
	list_onAfterSetDataBind: function() {
		if (this.list.records.length && !this.list.selector.getSelectedRecord()) {
			this.list.selector.selectRecord(this.list.recordsMap[this.list.page.preferences.$module] || this.list.records[0], true);
		}
	},
	...
	record_onSelect: function(record, selected) {
		...
		if (page.isAdminMode) {
			...
			syra_site.dom.hide(record.subModulesHead, !selected);
			syra_site.dom.hide(record.subModulesAdminBodySep, !selected);
			syra_site.dom.hide(record.subModulesBody, !selected);
		} else {
			if (selected) {
				if (!record.subModulesItem) {
			...
			syra_site.dom.hide(record.subModulesSlot, !selected);
		}
