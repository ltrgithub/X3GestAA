...
exports.ModuleBuilder = helpers.defineClass(ModuleBuilder, null, {
	...
	list_onAfterSetDataBind: function() {
		if (this.list.records.length && !this.list.selector.getSelectedRecord()) {
			this.list.selector.selectRecord(this.list.recordsMap[this.list.page.$globalPreferences.$module] || this.list.records[0], true);
		}
	},
	...
	record_dispose: function(record) {
		syra_dom.removeChild(record.subModulesHead);
		syra_dom.removeChild(record.subModulesAdminBodySep);
		syra_dom.removeChild(record.subModulesBody);
		syra_dom.removeChild(record.subModulesSlot);
	},
	...
	record_onSelect: function(record, selected) {
		...
		if (selected && page.isPageLoaded) {
			page.$globalPreferences.$module = record.dataset.$uuid;
			syra_globalPreferences.save("navigationpage", page.$globalPreferences);
