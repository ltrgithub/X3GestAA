...
exports.SubModule = helpers.defineClass(SubModule, null, {
	load: function(module, dataset) {
		...
		if (this.mode == "small") {
			this.megaScrollPanel = syra_dom.div("simplebar", this.body);

			this.megaScrollPanel.id = "contentPanelScrollBar";

			this.megaMenuColumns = new MegaMenuColumns();
			this.megaMenuColumns.load(this.megaScrollPanel, this.body);
			this.body.className += " s-mega-block-detail";
		}
		...
		for (var ii = 0, jj = dataset.items.length; ii < jj; ii++) {
			...
			if (dataItem) {
				var slot;
				if (this.mode == "large") {
					slot = syra_dom.li("s-mega-sub-menu-block-slot", this.body);
				} else {
					slot = syra_dom.li("s-nav-mega-link-parent", this.megaMenuColumns.firstColumnSlot());
				}
	...
	onActivated: function() {
		if (this.mode == "large") {
			...
			this.megaScrollPanel.style.height = _megaMenus.popup.clientHeight + "px";
			_megaMenus.popupBody.appendChild(this.body);

			this.megaMenuColumns.adjustColumnContent(this.megaParent.page.maxColumns);
			this.body.style.width = (this.megaMenuColumns.columnList.cols.length) * 240 + "px";
		}
	},
	activate: function(on) {
		_activate(this, on);
	...
	setVolumeSize: function(dataset) {
		/*
		 * We always show the left-side vertical menu (#1).
		 * We show an adjacent 2nd vertical menu (#2) if either:
		 	1) there are 7 or more entries at the level of menu #2
	...
	hasLargeVolumeDepth: function(items, isLastLevel) {
		var blockCount = 0;
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			if (items[ii].menuBlock) {
				blockCount++;
...
exports.MegaMenuColumns = helpers.defineClass(MegaMenuColumns, null, {
	load: function(megaScrollPanel, scrollBody) {
		...
		this.columnList.cols.push({
			slot: syra_dom.ul("s-mega-scroll-column", megaScrollPanel)
		});
	},
	firstColumnSlot: function() {
		return this.columnList.cols[0].slot;
	},
	adjustColumnContent: function(maxColumnCount) {
		...
		if (this.columnList && !this.columnList.adjusted) {
			...
			if (overflowContent.length) {
				...
				this.addColumn(this.columnList, columnNumber);
			...
			$('#contentPanelScrollBar').simplebar().find("div.simplebar-track").css("right", (maxColumnCount - 2 - this.columnList.cols.length) * 240 + "px");
		}

	},
	addColumn: function(columnList, columnNumber) {
		var slot = document.createElement("ul");
		slot.style.left = columnNumber * 240 + "px";
		slot.className = "s-mega-scroll-column";
