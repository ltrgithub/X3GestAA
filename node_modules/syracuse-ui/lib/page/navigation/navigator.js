...
var SimpleBar = require('syracuse-ui/lib/field/custom/scroller').simplebar;
...
exports.SubModuleMenuBlock = helpers.defineClass(SubModuleMenuBlock, null, {
	load: function(dataset) {
		...
		if (this.isHLayout) {
			this.body.style.width = (this.megaParent.page.maxColumns - 2) * 240 + "px";
			this.scroller = syra_dom.div(css.scroll, this.body);

			this.scroller.id = "contentPanelScrollBar";

			this.submoduleList = {
				cols: []
			};

			this.submoduleList.cols.push({
				slot: syra_dom.div("s-mega-scroll_column", this.scroller)
			});
		}
		...
		for (var ii = 0, jj = dataset.items.length; ii < jj; ii++) {
			...
			if (dataItem) {
				...
				if (item.menuItem) {
					child = new MenuItem();
					child.slot = syra_dom.li(css.titleLink, this.isHLayout ? this.submoduleList.cols[0].slot : this.body);
				} else {
					child = new SubModuleMenuBlock();

					if (this.submoduleList && this.submoduleList.cols[0]) {
						child.slot = syra_dom.li(css.slot, this.submoduleList.cols[0].slot);
					} else {
	...
	onActivated: function() {
		if (this.isHLayout) {
			this.scroller.style.height = _megaMenus.popup.clientHeight + "px";
			_megaMenus.popupBody.appendChild(this.body);

			this._adjustColumnContent(this.submoduleList);
			var emptyColumnCount = this.megaParent.page.maxColumns - 2 - this.submoduleList.cols.length;
			$('#contentPanelScrollBar').simplebar().find("div.simplebar-track").css("right", emptyColumnCount * 240 + "px");
		}
	},
	activate: function(on) {
	...
	_adjustColumnContent: function(subModuleList, maxLinkColumns) {

		if (subModuleList && !subModuleList.adjusted) {
			subModuleList.adjusted = true;
			var firstColumnSlot = subModuleList.cols[0].slot;

			/*
			 * Remove each overflowed root element from the column and place it an array, leave at least one element in the first column
			 */
			var overflowContent = [];
			while (this.body.clientHeight < firstColumnSlot.scrollHeight && firstColumnSlot.childNodes.length > 1) {
				overflowContent.push(firstColumnSlot.removeChild(firstColumnSlot.lastChild));
			}
			...
			if (overflowContent.length) {
				 ...
				 */
				var columnNumber = 1;
				var maxColumns = this.megaParent.page.maxColumns === 5 ? 3 : 2;

				this._addColumn(subModuleList, columnNumber);
				for (var ii = overflowContent.length - 1; ii > -1; ii--) {
					var overflowDomItem = overflowContent[ii];
					subModuleList.cols[columnNumber].slot.appendChild(overflowDomItem);

					if (columnNumber + 1 < maxColumns) {
						if (this.body.clientHeight < subModuleList.cols[columnNumber].slot.scrollHeight && subModuleList.cols[columnNumber].slot.childNodes.length > 1) {
							subModuleList.cols[columnNumber].slot.removeChild(subModuleList.cols[columnNumber].slot.lastChild);

							if (columnNumber === subModuleList.lastColumn - 1)
								break;
	...
	_addColumn: function(subModuleList, columnNumber) {
		var slot = document.createElement("ul");
		slot.style.left = columnNumber * 240 + "px";
		slot.className = "s-mega-scroll_column";
		subModuleList.cols[0].slot.parentNode.appendChild(slot);
		subModuleList.cols.push({
			slot: slot
		});
	},
...
helpers.defineClass(MegaMenus, DesktopPage, {
	load: function() {
		...
		this.body = syra_dom.ul("s-mega-modules", this.slot);
	...
	togglePopup: function(content) {
		...
		if (content) {
			...
			if (!self.popupDlg) {
				...
				self._setMegaMenuSize(self.popup.parentNode.clientHeight * 0.65, self.maxColumns === 5 ? 1200 : 960);
			}
		} else {
			self.popupDlg && self.popupDlg.close();
		}
	},
	_setMegaMenuSize(height, width) {
		this.popupDlg.overSlot.style.height = this.popupDlg.overSlot.style.maxHeight = this.popupImage.style.maxHeight = this.popupBody.style.height = height + "px";
		this.popupImage.style.width = width + "px";
	},
	addMore: function() {
		var self = this;
	...
	resize: function() {
		if (this.body) {
			...
			this.maxColumns = this.body.clientWidth < 1260 ? 4 : 5;
		}
	},
	clear: function() {
...
exports.addMegaMenus = function(page) {
	if (!exports.megaMenus) {
		_megaMenus = exports.megaMenus = new MegaMenus();
		_megaMenus.$prototype = page.$prototype;
		_megaMenus.dataset = page.dataset;
		_isEnabled && _megaMenus.load();
	}
};