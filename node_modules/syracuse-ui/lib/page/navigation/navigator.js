...
exports.SubModuleMenuBlock = helpers.defineClass(SubModuleMenuBlock, null, {
	load: function(dataset) {
		...
		if (this.isHLayout) {
			this.scroller = syra_dom.ul(css.scroll, this.body);
		...
		for (var ii = 0, jj = dataset.items.length; ii < jj; ii++) {
			...
			if (dataItem) {
				...
				if (item.menuItem) {
					child = new MenuItem();
					child.slot = syra_dom.li(css.titleLink, this.isHLayout ? this.scroller : this.body);
				} else {
					child = new SubModuleMenuBlock();
					child.slot = syra_dom.li(css.slot, this.scroller);
				}
	...
	onActivated: function() {
		if (this.isHLayout) {
			...
			$('#contentPanelScrollBar').simplebar();
		}
	},
	activate: function(on) {
	...
	togglePopup: function(content) {
		...
		if (content) {
			...
			if (!self.popupDlg) {
				...
				self.popupDlg.overSlot.style.height =
					self.popupDlg.overSlot.style.maxHeight =
					self.popupImage.style.maxHeight =
					self.popupBody.style.height =
					self.popup.parentNode.clientHeight * 0.65 + "px";
			}
		} else {
