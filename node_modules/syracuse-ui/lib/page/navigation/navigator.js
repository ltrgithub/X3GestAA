...
exports.MegaMenuColumns = helpers.defineClass(MegaMenuColumns, null, {
	load: function(megaScrollPanel, bodySlot) {
		this.body = bodySlot;
		...
		this.columnList.cols.push({
			slot: syra_dom.div("s-mega-scroll_column", megaScrollPanel)
		});
	},
	...
	adjustColumnContent: function(maxColumnCount) {
		...
		if (this.columnList && !this.columnList.adjusted) {
			 ...
			 */
			var overflowContent = [];
			while (this.body.clientHeight < this.firstColumnSlot().scrollHeight && this.firstColumnSlot().childNodes.length > 1) {
			...
			if (overflowContent.length) {
				...
				for (var ii = overflowContent.length - 1; ii > -1; ii--) {
					...
					if (columnNumber + 1 < maxItemColumns) {
						if (this.body.clientHeight < this.columnList.cols[columnNumber].slot.scrollHeight && this.columnList.cols[columnNumber].slot.childNodes.length > 1) {
