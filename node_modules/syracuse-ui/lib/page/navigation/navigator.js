...
function _activate(item, on) {
	if (on) {
		...
		syra_dom.removeChild(item.body);
	}
...
exports.Module = helpers.defineClass(Module, null, {
	load: function(page, dataset) {
		...
		if (dataset.code && this.icon) {
			this.icon.style.backgroundImage = "url('" + syra_site.$iconPath + "modules/s_module_" + dataset.code.toLowerCase() + ".png')";
		}
	},
	click: function() {
		this.activate(true);
	...
	onActivated: function() {
		if (!this.body) {
			...
			if (!this.children.length) {
				syra_dom.addTextInfo("s-mega-submodules-empty-slot", syra_local.nvpNoSubModule, this.body);
			}
		}
...
exports.SubModule = helpers.defineClass(SubModule, null, {
	load: function(module, dataset) {
		...
		this.slot.syrainout = this.id;
		this.title = syra_dom.addDiv("s-mega-submodule-title", this.slot);
...
exports.SubModuleMenuBlock = helpers.defineClass(SubModuleMenuBlock, null, {
	load: function(dataset) {
		...
		this.title = syra_dom.addDiv(css.title, this.domItem);
		this.title.textContent = dataset.title;
...
helpers.defineClass(MegaMenus, DesktopPage, {
	loadBox: function() {
		...
		this.popup = syra_dom.addDiv("s-mega-popup");
		this.popupBody = syra_dom.addDiv("s-mega-popup-body", this.popup);
	...
	togglePopup: function(content) {
		...
		if (content) {
			...
			if (!self.popupDlg) {
				self.popupDlg = syra_dlg.openPopup(self.page, {
					content: {},
					slot: self.popup,
	...
	addMore: function() {
		var self = this;
		self.moreBtn = syra_menus.button.add({
			parent: syra_site,
			...
			btnclick: function() {
				if (!self.morePopupDlg) {
					var slot = syra_dom.addDiv("s-mega-more-popup");
					slot.appendChild(self.morePopupBody);
					self.morePopupDlg = syra_dlg.openPopup(self.page, {
						content: {},
						slot: slot,
	...
	draw: function() {
		...
		if (!this.modules.length) {
			syra_dom.addTextInfo("s-mega-submodules-empty-slot", syra_local.nvpNoModule, this.body);
		}
	},
	resize: function() {
		if (this.body) {
	...
	clear: function() {
		...
		syra_dom.empty(this.body);
		syra_menus.button.remove(this.moreBtn);
