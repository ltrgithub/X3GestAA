...
function _endLoad(page, $representation) {
	if ($representation) {
		...
		syra_nav.megaMenus.$prototype = $representation.$prototype;
		syra_nav.megaMenus.dataset = {};
		var $keys = Object.keys($representation);
		for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
			if ($keys[ii] != "$prototype" && $keys[ii] != "$article") {
				syra_nav.megaMenus.dataset[$keys[ii]] = $representation[$keys[ii]];
			}
		}
		syra_nav.megaMenus.load();
...
exports.page = {
	load: function(page) {
		...
		page.mega_togglePopup = function(module) {
			var self = this;
			...
			if (content) {
				...
				if (!self.mega_popupDlg) {
					...
					module.carousel.adjustContainerHeight(self.mega_popup);

					self.mega_popup.style.width = self.mega_popup.style.minWidth = (self.isHighRes ? 1200 : 960) + "px";
					self.mega_popup.style.left = (self.modulesBar.clientWidth - self.mega_popup.clientWidth) / 2 + "px";

					self.more && self.more.setOnTop(syra_dom.setZIndex(self.mega_popup, true));
				}
			} else {
	...
	resizeItem: function(page) {
		if (page.isModulesLoaded) {
			...
			if (page.mega_popup && page.lastSelected) {
				page.lastSelected.carousel.adjustContainerHeight(page.mega_popup);
			}
