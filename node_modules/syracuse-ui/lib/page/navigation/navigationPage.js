	...
	loadBox: function(initData) {
		...
		if (this.isAdminMode) {
			...
			if (this.isAdminMode) {
				...
				this.btnExpandAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpExpandAll, "s-nav-submodule-expand", "onExpandAll", null, "expand"));
				this.btnExpandAll.syraItem = this.id;
				this.btnCollapseAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpCollapseAll, "s-nav-submodule-expand", "onExpandAll", null, "collapse"));
			...
			this.bar.slot = document.createElement("div");
			this.bar.slot.className = "s-nav-modulebar-slot";
			this.bar.body = document.createElement("div");
			this.bar.body.className = "s-nav-modulebar-body";
			this.domItem.insertBefore(this.bar.slot, this.domItem.firstChild);
			this.bar.load(this);
			this.loadNewItem(this.bar.body, {
				$isTitleHidden: true,
				$bind: "modules"
			});
		}
		...
		if (target.syraRecord !== undefined) {
			...
			if (event.shiftKey) {
				for (var ii = 0, jj = subModule.list.records.length; ii < jj; ii++) {
					_subModuleBuilder.expandMenuBlocks(subModule.list.records[ii], isExpand);
				}
			} else {
				_subModuleBuilder.expandMenuBlocks(subModule, isExpand);
			}
		} else {
			if (this.selectedSubModuleRecord) {
				_subModuleBuilder.expandMenuBlocks(this.selectedSubModuleRecord, target == this.btnExpandAll);
			}
		}
	},
	...
	createField: function($field, $item, boxParent, $class) {
		switch ($item.$bind) {
			case "modules":
				...
				//list.$itemRecordMenuSkin = "s-ldp-record-mn";
				list.createMenuItem = function($bind, $menu) {
					...
					item.domItem.className += " s-mn-list-btn";
					return item;
				};
