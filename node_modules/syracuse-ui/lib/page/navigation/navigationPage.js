...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	...
	loadBox: function(initData, $initDiagnoses) {
		...
		if (this.isAdminMode) {
			...
			this.admin.slot.className = "s-nav-admin-slot";
			this.admin.slot.appendChild(this.admin.modulesSlot).className = "s-nav-admin-modules-slot";
			this.admin.modulesSlot.setAttribute("data-s-inout", this.id);
			this.page.loadNewItem(this.admin.modulesSlot, {
				$isTitleHidden: true,
				$bind: "modules"
			});
			this.page.layoutContent.domItem.appendChild(this.admin.slot);
		} else {
	...
	onItemEnterLeave: function(event) {
		if (this.isAdminMode && event.currentTarget == this.admin.modulesSlot) {
	...
	changeMode: function($url) {
		...
		document.controller.parseUrl($url).sendRequest(null, function($representation, response, $url) {
			...
			$representation.$prototype.$representationUrl = self.$prototype.$representationUrl;
