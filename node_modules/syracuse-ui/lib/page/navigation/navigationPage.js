...
var MegaBar = require("syracuse-ui/lib/page/aside/megaBar").MegaBar;
...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	loadBox: function(initData) {
		...
		if (this.isAdminMode) {
			...
			if (this.isAdminMode) {
				...
				this.btnExpandAll = syra_menus.button.add({
					parent: this,
					slot: slot,
				...
				this.btnCollapseAll = syra_menus.button.add({
					parent: this,
					slot: slot,
			...
			this.megaBar = new MegaBar();
			this.megaBar.width = "100%";
			this.megaBar.options = {
				$isResizerDisabled: true,
				$skin: "s-nav-modulebar"
			};
			this.megaBar.slot = document.createElement("div");
			this.megaBar.slot.className = "s-nav-megabar-slot";
			this.megaBar.body = document.createElement("div");
			this.megaBar.body.className = "s-nav-megabar-body";
			this.domItem.insertBefore(this.megaBar.slot, this.domItem.firstChild);
			this.megaBar.load(this);
			this.megaBar.field = this.loadNewItem(this.megaBar.body, {
				$isTitleHidden: true,
				$bind: "modules"
			});
			...
			if (isOverflowed(this.megaBar.body)) {
	...
	refreshExpandAllLinks: function(subModule) {
		...
		if (article) {
			syra_menus.button.hide(article.btnExpandAll, !(article.btnExpandAll.hasChildBlock = hasChildBlock));
			syra_menus.button.disable(article.btnExpandAll, !hasExpandAll);
			syra_menus.button.hide(article.btnCollapseAll, !(article.btnCollapseAll.hasChildBlock = hasChildBlock));
			syra_menus.button.disable(article.btnCollapseAll, !hasCollapseAll);
		}
	},
		...
		if (!self.disposed && !self.isServerNotifyDisabled) {
			if (self.$prototype.$representationUrl) {
				...
				syra_controller.callServer(self, {
					...
					data: syra_delta.cleanServerIndex(helpers.object.clone(sendBag, true)),
					method: "PUT",
					$etag: sendBag.$etag
				}, function(newData, response, requestUrl) {
					if (!articleSender.disposed) {
						if (articleSender.page != articleSender) {
							syra_delta.applyObjectDelta(articleSender.page, articleSender.dataset, newData);
						}
	...
	notifySubModuleDataChange: function(field, value, notifyServerChange) {
		...
		while (subModule && subModule != this) {
			if (subModule.isSubModuleRecord) {
				var sendBag = syra_workingCopy.getSendBag(subModule, subModule);
	...
	notifyDataChange: function(field, value, notifyServerChange) {
		if (field.isSubModuleList || field.articleParent.isSubModuleRecord) {
			var sendBag = syra_workingCopy.getSendBag(field.articleParent, field.articleParent);
