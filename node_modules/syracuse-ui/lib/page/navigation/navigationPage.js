...
function _expandMenuBlocks(subModuleRecord, expand, itemsField) {
	itemsField = itemsField || subModuleRecord.itemsField;
	for (var ii = 0, jj = itemsField.records.length; ii < jj; ii++) {
		...
		if (record.singleField.variantItem.isMenuBlock) {
			...
			_expandMenuBlocks(subModuleRecord, expand, record.singleField.variantItem.record.itemsField);
		}
	}
}
...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	...
	loadBox: function(initData) {
		...
		if (this.isAdminMode) {
			...
			this.isAdminMode && this.defineExpandAllItems(menusHeader);
	...
	defineExpandAllItems: function(slot, record) {
		var article = record || this;
		article.btnExpandAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpExpandAll, "s-nav-submodule-expand", "onExpandAll", null, "expand"));
		article.btnExpandAll.syraItem = article.id;
		article.btnCollapseAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpCollapseAll, "s-nav-submodule-expand", "onExpandAll", null, "collapse"));
		article.btnCollapseAll.syraItem = article.id;
		if (record) {
			article.btnCollapseAll.syraRecord = article.btnExpandAll.syraRecord = record.id;
		}
	},
	onExpandAll: function(event, target) {
		if (target.syraRecord !== undefined) {
			var subModuleRecord = syra_store.get(target.syraRecord);
			var isExpand = target == subModuleRecord.btnExpandAll;
			if (event.shiftKey) {
				for (var ii = 0, jj = subModuleRecord.list.records.length; ii < jj; ii++) {
					_expandMenuBlocks(subModuleRecord.list.records[ii], isExpand);
				}
			} else {
				_expandMenuBlocks(subModuleRecord, isExpand);
			}
			...
			if (this.selectedSubModuleRecord) {
				...
				this.refreshExpandAllLinks(this.selectedSubModuleRecord);
			}
		}
	},
	refreshExpandAllLinks: function(subModuleRecord) {
		var hasChildBlock, hasExpandAll, hasCollapseAll;
		var itemField = subModuleRecord && subModuleRecord.itemsField;
		...
		var article = this.isAdminMode ? this : subModuleRecord;
		if (article) {
			article.btnExpandAll.style.display = hasChildBlock ? "" : "none";
			article.btnCollapseAll.style.display = hasChildBlock ? "" : "none";
