...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	...
	loadBox: function(initData) {
		...
		if (this.isAdminMode) {
			...
			this.defineExpandAllItems(menusHeader, "s-nav-admin-head-menus");
	...
	defineExpandAllItems: function(slot, $skinMenus, article, $iconPath) {
		article = article || this;
		article.expandLink = this.loadNewItem(slot, {
			$category: "link",
			$skin: $skinMenus + "-link",
			$bind: "$expand-all",
			$icon: {
		...
		article.collapseLink = this.loadNewItem(slot, {
			$category: "link",
			$skin: $skinMenus + "-link",
			$bind: "$collapse-all",
			$icon: {
	...
	refreshExpandAllLinks: function(subModuleRecord) {
		...
		if (subModuleRecord.itemsField) {
			...
			if (subModuleRecord.expandLink) {
				if (this.isAdminMode) {
					subModuleRecord.expandLink.parentNode.style.visibility = hasChildBlock ? "visible" : "hidden";
				} else {
			...
			if (this.isAdminMode && subModuleRecord.expandLink) {
				subModuleRecord.expandLink.parentNode.style.visibility = "hidden";
			}
		}
	},
