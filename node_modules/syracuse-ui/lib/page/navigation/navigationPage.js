...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	...
	defineExpandAllItems: function(slot, article) {
		var css = article ? "s-nav-submodule" : "s-nav-admin-head-btns";
		article = article || this;
		article.btnExpandAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpExpandAll, css + "-expand", "onExpandAll"));
		article.btnExpandAll.syraItem = article.id;
		article.btnCollapseAll = slot.appendChild(syra_menus.addIconButton(syra_local.nvpCollapseAll, css + "-collapse", "onExpandAll"));
		article.btnCollapseAll.syraItem = article.id;
	},
	onExpandAll: function(event, target) {
		if (this.selectedSubModuleRecord) {
			this.selectedSubModuleRecord.expandMenuBlocks(target == this.btnExpandAll);
			this.page.refreshExpandAllLinks(this.selectedSubModuleRecord);
		}
	},
	refreshExpandAllLinks: function(subModuleRecord) {
		...
		var article = this.isAdminMode ? this.page : subModuleRecord;
		if (article) {
	...
	dispose: function() {
		...
		this.btnExpandAll = this.btnCollapseAll = this.selectedModule = this.selectedSubModuleRecord = this.adminSlot = this.adminTable = this.adminTableBody = this.modules = this.bar = null;
