...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	...
	loadBox: function(initData, $initDiagnoses) {
		...
		this._defaultTitle = this.localize.nvpTitle;
		...
		if (this.isAdminMode) {
			...
			title.textContent = this.localize.nvpModules;
			...
			title.textContent = this.localize.nvpSubModules;
			...
			title.textContent = this.localize.nvpMenus;
			title.className = "s-nav-admin-head-menus-title";
			this.defineExpandAllItems("s-nav-admin-head-menus");
		...
		(subModuleRecord || this).applyChange({
			$links: {
				"$expand-all": {
					...
					$title: this.localize.nvpExpandAll
				},
				"$collapse-all": {
					$isHidden: this.isAdminMode && subModuleRecord,
					$title: this.localize.nvpCollapseAll
				}
			}
		});
	},
	defineExpandAllItems: function($skinMenus, article, $iconPath) {
		article = article || this;
		article.expandMenus = document.createElement("div");
		article.expandMenus.className = $skinMenus + "-expand-menus";
		article.expandLink = this.loadNewItem(article.expandMenus, {
			$category: "link",
			$skin: $skinMenus + "-link",
			$bind: "$expand-all",
			$icon: {
		...
		article.collapseLink = this.loadNewItem(article.expandMenus, {
			$category: "link",
			$skin: $skinMenus + "-link",
			$bind: "$collapse-all",
			$icon: {
	...
	getDataUrl: function(article) {
		article = article || this;
		var $parsedUrl = document.site.expressionMaker.parse(article, article.dataset.$url || article.$prototype.$url);
				...
				document.controller.sendRequest(self, {
					...
					if (!articleSender.disposed) {
						if (articleSender.page != articleSender) {
							document.site.deltaManager.applyObjectDelta(articleSender.page, articleSender.dataset, newData);
						}
	...
	resizePage: function(resize) {
		DesktopPage.prototype.resizePage.call(this, resize);
	...
	getPreferences: function() {
		return this.preferences = document.site.getPreferences("navigationpage", {});
	},
	setPreferences: function($module, $submodule) {
		...
		document.site.setPreferences("navigationpage", this.preferences);
	},
	dispose: function() {
