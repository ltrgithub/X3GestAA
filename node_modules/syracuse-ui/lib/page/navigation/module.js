...
Module.prototype.onItemInOut = function(onEnter, event, target) {
	...
	if (onEnter) {
		...
		if (self.page.breadcrumbModules || (self.page.lastSelected && relatedTarget && relatedTarget.parentNode == self.page.modulesBar)) {
			self.activate(true);
		} else {
...
Module.prototype.getChildItem = function(uuid) {
	for (var ii = 0, jj = this.children.length; ii <= jj; ii++) {
		var item = this.children[ii]; //this.dataset.submodules[ii];
