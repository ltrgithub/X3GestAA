...
Module.prototype.onItemInOut = function(onEnter, event, target) {
	...
	if (onEnter) {
		...
		if (self.page.isMapLoaded && self.page.lastSelected && relatedTarget && relatedTarget.parentNode == self.page.modulesBar) {
			self.activate(true);
		} else {
			if (self.page.breadcrumbModules && !self.page.lastSelected) {
				self.activate(true);
			} else {
				...
				}, (self.page.lastSelected && relatedTarget && relatedTarget.parentNode == self.page.modulesBar) ? 200 : (self.page.breadcrumbModules ? 150 : 300));
			}
		}
	} else {
...
Module.prototype.getChildItem = function(uuid) {
