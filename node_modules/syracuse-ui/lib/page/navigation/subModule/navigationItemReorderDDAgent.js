...
exports.NavigationItemReorderDDAgent = helpers.defineClass(NavigationItemReorderDDAgent, null, {
    start: function(dropItem){
        ...
        this.scrollViewRect = this.list.page.scrollview.getBoundingClientRect();
    ...
    bindEvent: function(bind){
        ...
        if (bind) {
            $(self.list.subModuleRecord.domItem).delegate("[data-s-record]", "mousemove" + nameSpace, function(event){
                if (self.reorderRecord) {
                    var record = document.site.itemHelper.findArticle(this);
                    self.moveDragImage(event, record);
                }
                return false;
            }).delegate("[data-s-empty-list]", "mousemove" + nameSpace, function(event){
                if (self.reorderRecord) {
                    var record = {
                        domItem: event.currentTarget,
                        list: document.site.itemHelper.findArticle(event.currentTarget)
                    };
                    self.moveDragImage(event, record);
                }
                return false;
            ...
            $(self.list.subModuleRecord.domItem).undelegate(nameSpace);
        }
    },
        ...
        if (this.targetRecord) {
            ...
            if (target_dataRecords.length) {
                ...
                if (targetRecordIndex > sourceRecordIndex) {
                    sourceRecordIndex = isAfter ? targetRecordIndex : (targetRecordIndex - 2);
                }
                else {
    ...
    moveDragImage: function(event, targetRecord, isOut){
        ...
        if (!isOut && this.$action) {
            document.site.dom.toggleClass(this._info, "s-drag-ok", true);
            ...
            document.site.dom.toggleClass(this._info, "s-drag-ok", false);
