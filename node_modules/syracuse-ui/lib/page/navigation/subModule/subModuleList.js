...
exports.SubModuleList = helpers.defineClass(SubModuleList, MultiList, {
	loadBox: function(initData) {
		...
		this.$confirmDelete = syra_local.nvpConfirmDeleteSubModule;
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if (this.records.length && this.selector.uuid == undefined && this.selector.select) {
			this.selector.select(this.page.preferences.$submodule, true, true);
		}
	},
	...
	onRecordReorderAgentCalculateDragOffset: function(recordReorderDDAgent) {
		recordReorderDDAgent.$drag = syra_site.dom.getBoundingClientRect(recordReorderDDAgent.targetRecord.header);
	},
	drawBox: function() {
	...
	_createMenusBox: function() {
		if (!this.isMenuLoaded) {
			...
			if (this.page.isAdminMode) {
				var $item;
				($item = this.defineNewIconMenu("$create", this.$skinMenus + "-link"));
				this.page.loadNewItem(this.menusSlot, $item, this);
				($item = this.defineNewIconMenu("$select", this.$skinMenus + "-link"));
				this.page.loadNewItem(this.menusSlot, $item, this);
			}
		}
	},
	...
	emptyBody: function(addEmptySlot, onFetch) {
		...
		if (this.page.isAdminMode) {
			...
			if (addEmptySlot) {
				this.articleParent.subModulesAdminTitleList.appendChild(this.emptyDataItem);
			}
		}
	},
	dispose: function() {
