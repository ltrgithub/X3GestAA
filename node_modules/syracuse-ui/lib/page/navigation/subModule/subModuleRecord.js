...
exports.SubModuleRecord = helpers.defineClass(SubModuleRecord, RecordArticle, {
	...
	applyChange: function(newData, serverResponse) {
		if (serverResponse) {
			document.site.deltaManager.applyObjectDelta(this.page, this.dataset, newData);
		}
	...
	onSelectMouseEvent: function(onEnter) {
		document.site.dom.toggleClass(this.header, "s-record-over", onEnter);
	},
	highlightSelection: function(selected) {
		document.site.dom.toggleClass(this.header, "s-list-record-selected", selected);
		this.domItem.style.display = selected ? "" : "none";
		if (selected && this.page.isAdminMode) {
			this.page.setPreferences(null, this.dataset.$uuid);
	...
	drawBox: function() {
		...
		this.reorderPickerParent = this.domTitle.parentNode;
		...
		if (this.page.isAdminMode) {
			...
			this.header.appendChild(this.menusBar);
		}
	...
	onMenuClick: function(menuItem, event) {
		switch (menuItem.$bind) {
			case "$expand-all":
			case "$collapse-all":
				var expand = menuItem.$bind == "$expand-all";
				if (event.shiftKey) {
					for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
						this.list.records[ii].expandMenuBlocks(expand);
					}
				} else {
					this.expandMenuBlocks(expand, event.shiftKey);
				}
				return false;
		}
		return true;
	...
	_createMenusBox: function() {
		if (!this._isMenuLoaded) {
			...
			if (this.page.isAdminMode) {
				this.itemsField.menusSlot.className = "s-nav-admin-items-menus-slot s-enter";
	...
	dispose: function() {
		...
		this.cardItem = this.dataSlot = this.body = this.menusSlot = this.dataRow = null;
