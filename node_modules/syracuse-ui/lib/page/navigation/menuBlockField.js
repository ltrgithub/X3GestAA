...
exports.MenuBlockField = helpers.defineClass(MenuBlockField, ChildField, {
	load: function() {
		...
		ChildField.prototype.load.call(this);
...
helpers.defineClass(MenuBlockFieldRecord, ChildFieldRecord, {
	load: function(initData) {
		...
		this.titleField = this.page.addItem(this.domItem, {
			$bind: "title",
		...
		if (!this.itemsField) {
			this.itemsField = this.page.addItem(this.domItem, {
				$bind: "items",
				$isTitleHidden: true
			}, this);
		...
		syra_item.remove(prevTitleField, true, true);
	...
	ensureExpandedState: function() {
		...
		if (!this.expandBtn) {
			this.expandBtn = syra_button.add({
				parent: this,
				text: text,
			...
			syra_button.setText(this.expandBtn, text, icon);
		}
		syra_dom.hide(this.itemsField.domItem, !this.$expanded);
	}
});