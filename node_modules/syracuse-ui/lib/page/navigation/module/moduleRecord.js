...
exports.ModuleRecord = helpers.defineClass(ModuleRecord, RecordArticle, {
	...
	drawBox: function() {
		...
		this._item.className = "s-nav-module";
		this._item.setAttribute("data-s-record", this.$uuid);
		this.$$item = $(this._item);
		this.slot = document.createElement("div");
		this.slot.className = "s-nav-module-title";
		this._item.appendChild(this.slot);

		this._menusSlot = document.createElement("div");
		this._menusSlot.className = "s-nav-module-links";
		this._item.appendChild(this._menusSlot);
		this.applyReorderCapability(true);



		if (this.list.selector.isRowMode) {
			this.slot.setAttribute("data-s-picker", "list-selector");
			this.slot.className += " s-list-selector-row";
			this.slot.setAttribute("data-s-record", this.$uuid);
		}

		this.cardItem = this.page.loadNewItem(this.slot, {
			$bind: "title",
			$category: "field",
	...
	highlightSelection: function(selected) {
		document.site.toggleClass(this._item, "s-list-record-selected", selected);
		if (selected) {
			if (!this.subModulesItem) {
				...
				this.page.layoutContent._item.appendChild(this.subModulesItemSlot);
