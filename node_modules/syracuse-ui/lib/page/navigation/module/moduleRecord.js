...
exports.ModuleRecord = helpers.defineClass(ModuleRecord, RecordArticle, {
	...
	showRecordSelection: function(selected) {
		RecordArticle.prototype.showRecordSelection.call(this, selected);
		var page = this.page;
		if (page.isAdminMode) {
			if (selected) {
				page.selectedModule = this;
				if (!this.subModulesItem) {
					...
					this.subModulesHead.appendChild(this.subModulesAdminActions).className = "s-nav-admin-submodules-menus-slot";
					...
					this.subModulesItem = page.loadNewItem(this.subModulesBody, {
						$isTitleHidden: true,
						$bind: "submodules"
					}, this);
					page.adminTableBody.appendChild(this.subModulesHead);
					page.adminTableBody.appendChild(this.subModulesAdminBodySep);
					page.adminTableBody.appendChild(this.subModulesBody);
				}
				page.refreshExpandAllLinks(page.selectedSubModuleRecord = this.selectedSubModuleRecord, true);
			} else {
				if (page.selectedModule == this) {
			...
			if (selected) {
				if (!this.subModulesItem) {
					...
					page.layoutContent.domItem.appendChild(this.subModulesSlot);
				}
		...
		if (selected && page.isPageLoaded) {
			page.setPreferences(this.dataset.$uuid);
	...
	dispose: function() {
		...
		this.selectedSubModuleRecor = this.icon = this.cardItem = this.dataSlot = this.body = null;
