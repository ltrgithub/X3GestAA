...
exports.ModuleRecord = helpers.defineClass(ModuleRecord, RecordArticle, {
	...
	reorderItem: function(targetRecord, isAfter) {
		if (targetRecord) {
			...
			if (isAfter) {
				this.builder.body.appendChild(this.domItem);
			} else {
				this.builder.body.insertBefore(this.domItem, this.builder.body.firstChild);
			}
		}
	},
	remove: function() {
	...
	drawBox: function() {
		...
		this._menusSlot.className = this.$skin + "-links";
		this.domItem.appendChild(this._menusSlot);
		...
		this.reorderPicker.style.visibility = "hidden";
	},
	...
	onItemEnterLeave: function(event) {
		if (this._menusSlot) {
			document.site.toggleClass(this._menusSlot, "s-enter", event.type == "mouseenter");
		}
		if (this.reorderPicker) {
			this.reorderPicker.style.visibility = event.type == "mouseenter" ? "" : "hidden";
		}
	...
	highlightSelection: function(selected) {
		...
		if (selected) {
			if (!this.subModulesItem) {
				this.subModulesItemSlot = document.createElement("div");
				this.subModulesItemSlot.className = "s-nav-submodules-slot";
				this.page.layoutContent.domItem.appendChild(this.subModulesItemSlot);
				this.subModulesItem = this.page.loadNewItem(this.subModulesItemSlot, {
					$isTitleHidden: true,
					$bind: "submodules"
				}, this);
			}
			this.subModulesItemSlot.style.display = "";
		} else {
			if (this.subModulesItemSlot) {
				this.subModulesItemSlot.style.display = "none";
			}
		}
	},
	dispose: function() {
		this.subModulesItem = this.subModulesItemSlot = null;
		this.cardItem = this.domItem = this.dataSlot = this.body = this._menusSlot = this.dataRow = null;
