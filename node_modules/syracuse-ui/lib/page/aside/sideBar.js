...
function _appplyPreferences(bar) {
	...
	if (bar.preferenceKey) {
		syra_globalPreferences.get(bar.preferenceKey, state, bar);
	} else {
		if (bar.userPreferenceKey) {
			state = syra_pagePreferences.get(bar.page, bar.userPreferenceKey) || state;
		}
		syra_globalPreferences.applyValues(bar, state);
	}
...
exports.SideBar = helpers.defineClass(SideBar, null, {
	...
	clearPreferences: function() {
		...
		if (key) {
			syra_globalPreferences.save(key);
			delete this.$width;
	...
	savePreferences: function(key) {
		if (this.preferenceKey) {
			syra_globalPreferences.save(this.preferenceKey, this, ["$width", "$isCollapsed"]);
		} else {
			if (this.userPreferenceKey) {
				var preference = syra_pagePreferences.get(this.page, this.userPreferenceKey) || {};
				preference.$width = this.$width;
				preference.$isCollapsed = this.$isCollapsed;
				syra_pagePreferences.save(this.page, this.userPreferenceKey, preference);
			}
		}
	},
	dispose: function() {
