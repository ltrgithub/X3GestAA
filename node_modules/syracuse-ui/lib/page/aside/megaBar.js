...
exports.MegaBar = helpers.defineClass(MegaBar, null, {
	...
	resizeBar: function(resize) {
		...
		if (this.slot) {
			...
			if (this.field.body.scrollWidth > this.slot.clientWidth) {
				// we've overflowed
				this.overflowElements = this.overflowElements || [];
				...
				for (var ii = nodes.length - 1; ii > -1; ii--) {
					if ((nodes[ii].getBoundingClientRect().right + this.moreBtn.link.clientWidth) < maxW) {
						...
						var overflowElement = this.field.body.removeChild(nodes[ii]);
						this.overflowElements.push(overflowElement);
						this.morePopupBody.insertBefore(overflowElement, this.morePopupBody.firstChild);
					}
				}
			} else {
				// re-display hidden buttons
				for (var jj = this.overflowElements.length - 1; jj > -1; jj--) {
					var element = this.overflowElements[jj];
					this.field.body.appendChild(element);
					if ((this.field.body.scrollWidth + this.moreBtn.link.clientWidth) > this.slot.clientWidth) {
						// remove the element then break;
						this.field.body.removeChild(element);
						break;
					} else {
						// we've add the element back into the menu, so remove it from the overflowElements array
						this.overflowElements.splice(-1, 1);
					}
				}
			}
		}
	...
	addMoreBtn: function() {
		...
		if (!this.moreBtn) {
			...
			this.moreBtn = syra_menus.button.add({
				...
				btnclick: function() {
					if (!self.morePopupDlg) {
						self.morePopupDlg = syra_dlg.openPopup(self.field.boxParent, {
							content: self.field,
							slot: self.morePopupBody,
			...
			//syra_menus.button.visibility(self.moreBtn, false);
			self.morePopupBody = document.createElement("div");
			self.morePopupBody.className = "s-mn-popup-body";
		}
	},
