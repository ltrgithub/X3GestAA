...
function _endLoad(page, $itemPage, $representation, onLoaded) {
	var $protoArticle = page.$prototype.$article;
	var $article = page.initializePage ? page.initializePage($itemPage) : $representation.$article;
	if ($article) {
	...
	if (page.$prototype && page.$prototype.$properties) {
		if (page.$prototype.$properties.contract) {
			page.$prototype.$properties.contract.$isHidden = true;
		}
	}

	/*if (item.$prototype && item.$prototype.$links && item.$facet == "$details") {
     item.$prototype.$links.LK_EMB = {
     $target: "embedded",
     $title: "Test embedded",
	...
	syra_site.initializeNewItem(page, $article, $itemPage.boxParent);
	syra_localizer.applyPageLocalization(page);

	page.$autoFetch = $itemPage.$autoFetch;
	...
	page.$views = $representation.$views;
	page.loadBox($itemPage.initData);
	...
	if ($representation.$diagnoses && $representation != $itemPage.initData) {
		...
		page.diagnosesPanel && page.diagnosesPanel.refresh();
	}
...
exports.load = function($itemPage, onLoaded) {
	...
	var page = new($itemPage.$pageCategoryClass || syra_widgets.pageCategories[$pageCategory] || DesktopPage)();
	var $userPreferencesLink = $representation.$links && $representation.$links.$userPreferences;
	if ($userPreferencesLink) {
		(page.userPreferences = new UserPreferences()).load(page, $userPreferencesLink, $representation.userPreferences && $representation.userPreferences.content);
	}
	page.id = syra_site._childPageOffset++;
	page.openerUrlSegments = $itemPage.openerUrlSegments;
	page.$pageCategory = $pageCategory;
	page.dialogWrapper = $itemPage.dialogWrapper;
	page.$pageCollaborationUrl = $representation.$pageCollaborationUrl;
	page.$authorUrl = $representation.$authorUrl;
	page.$facet = $itemPage.$facet || $itemPage.openerUrlSegments.$facet;
	page.$breadCrumb = $representation.$links && $representation.$links.$breadcrumb;

	page.$isEditMode = $itemPage.$isEditMode || page.$facet == "$edit";
	page.$isEditMode && syra_workingCopy.add(page);
	...
	syra_widgets.ensureMediaTypes(page.$prototype, function() {
		_endLoad(page, $itemPage, $representation, onLoaded);
	});
};
...
function _buildQueryArticle(page, $article, $prototype, isCube) {
	var $items = syra_layoutUpdater.getItemSettings(page, $article.$layout.$items, true) || [];
	var $defined = {};
	for (var ii = 0, jj = $items.length; ii < jj; ii++) {
		if ($items[ii].$bind) {
	...
	for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
		...
		if ($field && !$field.$isExcluded) {
			var $item = $defined[$bind] ? _helpers.object.clone($defined[$bind], true) : {
				$bind: $bind
			};
