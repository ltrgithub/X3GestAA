...
exports.DesktopPage = _helpers.defineClass(DesktopPage, Article, {
		...
		if (this.layoutSlot && this.size == undefined) {
			...
			if (this.dialogWrapper) {
				if (this.$facet == "$lookup" || this.$facet == "$select") {
					this.size.lookupMargin = (this.domItem.offsetTop * 2) + this.header.clientHeight;
				} else {
					this.size.dlgMargin = {
						height: this.domItem.offsetTop * 2,
						width: this.domItem.offsetLeft * 2,
					};
					this.size.maxBarHeight = sizeSize.dlg.maxHeight - this.size.dlgMargin.height;
					this.size.barHeight -= this.size.dlgMargin.height;
				}
			}
	...
	_refreshScrollViewHeight: function() {
		if (this.scrollview) {
			this.size.scrollview = {
				height: this.scrollview.clientHeight
			};
		}
	},
			...
			if (syra_dom.isVisible(this.domItem)) {
				...
				if (!this.dialogWrapper) {
					...
					if (this.scrollview && this.size.isHFit) {
						...
						this._refreshScrollViewHeight();
						if (this.footer) {
							this.body.syraMinHeight = (this.size.scrollview.height - this.header.clientHeight - this.footer.clientHeight);
	...
	applyChange: function(newData) {
		if (!this.disposed && this.dataset) {
			...
			syra_delta.applyObjectDelta(this, this.dataset, newData);
	...
	onAfterLoad: function() {
		if (this.mainPageDesignerAccess === "disabled" && syra_site.pageDesignerBtn) {
			syra_menus.button.visibility(syra_site.pageDesignerBtn, false);
			if (this.dialogWrapper && this.dialogWrapper.openerPage) {
				this.openerDialogDesignerAccess = this.dialogWrapper.openerPage.mainPageDesignerAccess;
			}
		}
	...
	dispose: function() {
		if (this.mainPageDesignerAccess === "disabled" && this.openerDialogDesignerAccess !== undefined) {
			syra_site.refreshPageDesignerOpener(this.openerDialogDesignerAccess);
		}
		...
		syra_pageBuilder.disposePage(this.resizableList);
		this._autoInsertFields && this._autoInsertFields.dispose();
