...
var designerViewChoice = require('./utility/designerViewChoice');
...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	onAdvancedClick: function() {
		advancedStateHelper.onAdvancedClick(this);
	},
	...
	onItemInOut: function(onEnter, event, target) {
		if (target.syraViewIndex !== undefined) {
			designerViewChoice.onItemInOut(onEnter, event, target);
		}
	},
	...
	resizeArticle: function(resize, boxParent) {
		if (this.displayed !== false && this.isPageLoaded && !this._isDataChanging) {
			...
			if (this.designer) {
				...
				this.menuBar && this.resizeSplitter(this.menuBar);
				this.fusionBar && this.resizeSplitter(this.fusionBar);
			}
			this.setScrollViewPosition && this.setScrollViewPosition();
			this.bar && this.resizeSplitter(this.bar);
	...
	resizeSplitter: function(barSplitter) {
		if (!barSplitter.designing) {
			if (!barSplitter.options.isAutoModeDisabled) {
				var autoViewMode = syra_site.autoViewMode;
				if (barSplitter.autoViewMode == undefined && barSplitter.$viewMode == autoViewMode) {
					barSplitter.autoViewMode = autoViewMode;
				}
				if (barSplitter.autoViewMode != autoViewMode) {
					barSplitter.$viewMode = barSplitter.autoViewMode = autoViewMode;
					barSplitter.$isCollapsed = barSplitter.$viewMode == "float";
					barSplitter.ensureState();
				}
			}
			this.setParentSpace(barSplitter);
			barSplitter.resizeBar();
		}
	},
	setParentSpace: function(barSplitter, newWidth) {
		var direction = barSplitter.options.resizeDirection;
		if (barSplitter.$isCollapsed || barSplitter.$viewMode != "float") {
			barSplitter.barSlot.parentNode.style["padding" + direction.substr(0, 1).toUpperCase() + direction.substr(1)] = (newWidth || barSplitter.barSlot.getBoundingClientRect().width) + "px";
		}
	},
	...
	togglePageOptions: function() {
		designerViewChoice.toggle(this);
	},
	loadPageViewSelector: function() {
		if (this.$views) {
			this.designerViewOpener = syra_menus.addIconButton("Select view", "s-designer-view-opener", "togglePageOptions");
			this.headerViewSelector = document.createElement("div");
			this.headerViewSelector.className = "s-page-view-old-btn";
			this.titleLabel.parentNode.insertBefore(this.headerViewSelector, this.titleLabel.nextSibling);
			this.titleLabel.parentNode.insertBefore(this.designerViewOpener, this.titleLabel.nextSibling);
	...
	_showBreadCrumb: function() {
		if (this.$breadCrumb && this.$breadCrumb.length > 0) {
			...
			breadLink = syra_menus.addIconButton("", this.$skin + "-breadcrumb-link", "onBreadcrumbClick");
	...
	showDiagnoses: function(message, options) {
		syra_diagnose.ensurePanel(this).showDiagnoses(message, this, options);
	},
	drawBox: function() {
	...
	appendPageMenus: function(slot) {
		!syra_site.mobileGateway && RawPage.prototype.appendPageMenus.call(this, slot);
	...
	reloadLayout: function($item, shouldInsertNewItems) {
		if (this._advancedFieldsSlot) {
			...
			this._advancedFieldsSlot = this._advancedFieldsBtn = null;
		}
	...
	dispose: function() {
		...
		this.designerViewOpener = this.securityViewSlot = this.security = this.openerUrlSegments = this.noProtoFields = null;
		this.garbage = this.$persistPrototype = this.pageViewSelector = null;
		this._advancedFieldsSlot = this._advancedFieldsBtn = this.diagnoseSlot = this.headerViewSelector = null;
		this.scrollHandler = this.scrollview = this.$menus = this.menuBar = this._autoInsertFields = null;
		this.diagnosesPanel = this.domDescription = null;
