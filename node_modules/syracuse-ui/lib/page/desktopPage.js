...
exports.DesktopPage = _helpers.defineClass(DesktopPage, RawPage, {
	...
	loadBox: function(initData) {
		...
		if (this.$facet == "$details") {
			...
			if (!$properties.discussionFeed) {
				$properties.discussionFeed = {
					$title: "Discussion Feed",
					$type: "application/x-discussion-feed"
				};
			}
	...
	sentiment: function() {
		// hackaton
		if (this.openerUrlSegments.fusionParams && ["GESBPC", "GESITM"].indexOf(this.openerUrlSegments.fusionParams.fullCode) != -1) {
			// debugger;
			this.$prototype.$properties["KA2"] = {
				"$type": "application/x-sentiment",
				"$isEditMode": false,
				"$title": "Sentiment analysis",
			};

			var tab = this.layoutContent.items[1] && this.layoutContent.items[1].loadChildItem(null, {
				$category: "section",
				$isEmptyVisible: true,
				$title: "Sentiment analysis",
				$layout: {
