...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	toggleAdvancedFieldsMode: function(enable, onAuthoring) {
		...
		if (!this._advancedFieldsSlot) {
			...
			this._advancedFieldsSlot.setAttribute("data-s-box", this.id);
			this._headerCoreLeft.appendChild(this._advancedFieldsSlot);
			this._advancedFieldsLink = document.createElement("a");
			this._advancedFieldsLink.setAttribute("data-s-box-picker", "advanced");
	...
	updateScrollBtnPosition: function() {
		...
		if (this.scrollBtnHandler) {
			var lists = Object.keys(this.scrollBtnHandler);
			for (var ii = 0, jj = lists.length; ii < jj; ii++) {
				var listBtnHandler = this.scrollBtnHandler[lists[ii]];
				var hscroller = listBtnHandler.listHscroller;
				var headtable = listBtnHandler.listHeadTable;
				var top;
				// computing scroll btn position according whether head table or table hor scroll are visible or not 
				switch (document.site.isScrolledIntoView($(headtable), self.$$scrollview)) {
					// list head table is visible in the page
					case (true):
						// list hor scroller is visible in the page
						if (document.site.isScrolledIntoView($(hscroller), self.$$scrollview)) {
							console.log("1-1");
						...
						// list hor scroller is not visible in the page
							...
							console.log("1-2");
							top = ((self.$$scrollview[0].clientHeight - $(headtable).offset().top) / 2 + $(headtable).offset().top) + "px";
						}
						break;
					...
					case (false):
						...
						// list hor scroller is not visible in the page
							...
							var docViewTop = self.$$scrollview.scrollTop();
							var docViewBottom = self.$$scrollview.height();
				...
				if (!top) {
					listBtnHandler.leftScrollBtn.style.display = listBtnHandler.rightScrollBtn.style.display = "none";
				} else {
					listBtnHandler.leftScrollBtn.style.display = listBtnHandler.rightScrollBtn.style.display = "";
					listBtnHandler.leftScrollBtn.style.top = listBtnHandler.rightScrollBtn.style.top = top;
				}

			}
		}
	},
	addScrollListener: function(listId, listScrollBtnHandler) {
		this.scrollBtnHandler = this.scrollBtnHandler || {};
		this.scrollBtnHandler[listId] = listScrollBtnHandler;
	},
	removeScrollListener: function(listId) {
		if (this.scrollBtnHandler) {
			this.scrollBtnHandler[listId] = null;
		}
	},
	...
	dispose: function() {
		...
		this._advancedFieldsSlot = this._advancedFieldsLink = this._$$ditagnoseSlot = this._$$header = this._headerFoooter = this._headerViewSelector = null;
