...
exports.DesktopPage = _helpers.defineClass(DesktopPage, null, {
	...
	loadNewItem: function(slot, $item, boxParent, initData) {
		var item = syra_widgets.addItem(this, $item, boxParent || this);
		if (item) {
	...
	getScrollviewSize: function() {
		if (!this.scrollviewSize) {
			this.scrollviewSize = syra_dom.getBoundingClientRect(this.scrollview);
		}
	...
	getPageSize: function(updated) {
		if (this.layoutSlot && this.size == undefined) {
			...
			this.size.barHeight = this.size.height;
			this.size.headerHeight = this.header ? this.header.clientHeight : 0;
			this.size.footerHeight = this.footer ? this.footer.clientHeight : 0;

			if (this.dialogWrapper) {
				var dlg = this.size.dlg = {
					maxWidth: sizeSize.dlg.maxWidth,
					maxHeight: sizeSize.dlg.maxHeight
				};
				if (syra_site.isResizing) {
					delete this.dialogWrapper.customPosition;
				}
				if (this.dialogWrapper.customPosition) {
					dlg.maxWidth = this.dialogWrapper.customPosition.width;
					dlg.maxHeight = this.dialogWrapper.customPosition.height;
				}
				if (this.$facet == "$lookup" || this.$facet == "$select") {
					this.size.lookupMargin = (this.domItem.offsetTop * 2) + this.size.headerHeight;
				} else {
					dlg.marginWidth = this.domItem.offsetLeft * 2;
					dlg.marginHeight = this.domItem.offsetTop * 2;
					this.size.maxBarHeight = this.size.dlg.maxHeight - dlg.marginHeight;
					this.size.barHeight -= dlg.marginHeight;
				}
			}
			this._refreshScrollViewHeight();
		}
		return this.size;
	},
	_refreshScrollViewHeight: function() {
		if (this.scrollview) {
			this.size.scrollviewHeight = this.scrollview.clientHeight;
			this.size.bodyHeight = this.size.scrollviewHeight - this.size.headerHeight - this.size.footerHeight;
		}
	},
	delayResizeArticle: function() {
		var self = this;
		clearTimeout(self.resizePageTimeout);
		self.resizePageTimeout = setTimeout(function() {
			console.log("resizeArticle done");
			self.resizeArticle();
		}, 100);
	},
	resizeArticle: function(resize, articleParent) {
		if (this.displayed !== false && this.isPageLoaded && !this._isDataChanging) {
			if (syra_dom.isVisible(this.domItem)) {
				if (this.resizePageTimeout) {
					clearTimeout(this.resizePageTimeout);
					delete this.resizePageTimeout;
				}
				...
				if (!this.dialogWrapper) {
					...
					if (this.scrollview && this.size.isHFit) {
						this.domItem.style.height = this.size.height + "px";
						this.scrollview.syraIsHFit = true;
						this.scrollview.style.height = (this.size.height - this.diagnoseSlot.clientHeight) + "px";
						this._refreshScrollViewHeight();
						if (this.footer) {
							this.body.syraMinHeight = (this.size.scrollviewHeight - this.header.clientHeight - this.footer.clientHeight);
							this.body.style.minHeight = this.body.syraMinHeight + "px";
						}
					}
				}
				syra_dlg.resizePositionedDialogs(this);
				this.designer && this.designer.resizeArticle(resize);
				if (this.scrollPositionSessionKey && !this._scrollPositionSessionDone) {
					var pos = sessionStorage.getItem(this.scrollPositionSessionKey);
					if (pos) {
						pos = JSON.parse(pos);
						this.scrollview.scrollLeft = pos.scrollLeft;
						this.scrollview.scrollTop = pos.scrollTop;
					}
					this._scrollPositionSessionDone = true;
				}
				if (this.bars) {
		...
		if ($description) {
			...
			if (title != $description) {
				if (!this.domDescription) {
					this.domDescription = syra_dom.addDiv(this.$skin + "-description");
					if (this.domTitle) {
	...
	drawPage: function() {
		...
		this.scrollview = syra_dom.addDiv(this.$skin + "-scrollview");
		...
		if (!this.isSecurityViewHidden && !this.isBreadCrumbHidden) {
			this.securityBreadcrumbSlot = syra_dom.addDiv("s-page-head-top");
			syra_dom.hide(this.securityBreadcrumbSlot, true);
		...
		this.domTitle = syra_dom.addDiv(this.$skin + "-title", this.header);
	...
	fetch: function(options, success, error) {
		...
		if (options) {
			...
			self.lastFetchOptions = _helpers.object.clone(options, true);
		} else {
	...
	reloadLayout: function($item, layoutSlot) {
		...
		syra_layoutUpdater.extractItems(this.layoutContent, fields);
		syra_layoutUpdater.clearContent(this.layoutContent);
		this.$item = $item;
		...
		prevLayout && syra_dom.removeChild(prevLayout);
	...
	renderLayoutContent: function() {
		this.layoutContent = syra_widgets.addItem(this.page, this.$item.$layout, this);
	...
	dispose: function() {
		this.siteLegalLink && syra_item.remove(this.siteLegalLink, false, true);
		...
		this.loaded = false;
		if (this.menuBar) {
			this.menuBar.slot && syra_dom.removeChild(this.menuBar.slot);
