...
function _addPagerLink(page, $bind, $title) {
	page.loadNewItem(page._headerNav, {
		$category: "link",
		$skin: "s-page-pager-link",
		$bind: $bind,
		$title: $title,
		$icon: {
			$mode: "icon",
...
function _addPagerLinks(page) {
	syra_site.dom.empty(page._headerNav);
	_addPagerLink(page, "$first", syra_local.dskpFirstRecord);
	_addPagerLink(page, "$previous", syra_local.dskpPreviousRecord);
	_addPagerLink(page, "$next", syra_local.dskpNextRecord);
	_addPagerLink(page, "$last", syra_local.dskpLastRecord);
}
...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	dispose: function() {
		...
		this._advancedFieldsSlot = this._advancedFieldsBtn = this.diagnoseSlot = this.dataSlot = this._headerFoooter = this.headerViewSelector = null;
