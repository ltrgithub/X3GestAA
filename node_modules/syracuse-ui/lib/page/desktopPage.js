...
exports.DesktopPage = _helpers.defineClass(DesktopPage, null, {
			...
			if (title != $description) {
				if (!this.domDescription) {
					...
					if (this.domTitle) {
						...
						this.header && this.header.appendChild(this.domDescription);
					}
				}
				if (this.domDescription) {
	...
	fetch: function(options, success, error) {
		...
		options.success = function(data, response, requestUrl) {
			var res;
			if (success) {
				res = success(data, response, requestUrl);
			} else {
				self.startChange();
		...
		options.error = function(response, ajax, $diagnoses) {
			syra_quality.page_fetch(self, "error");
			return !error || error(response, ajax, $diagnoses);
		}
