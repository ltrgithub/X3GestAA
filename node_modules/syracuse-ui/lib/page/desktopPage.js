...
var _breadcrumb = require('syracuse-ui/lib/links/navigation/breadcrumb/breadcrumb');
...
exports.DesktopPage = _helpers.defineClass(DesktopPage, null, {
	...
	ensureModulesBar: function() {
		var modulesBarBody = syra_site.megaMenus && syra_site.megaMenus.body;
		if (modulesBarBody && !this.isAlertModal) {

			var isMobileOrTabletPage = (document.location.href.search(/syracuse-main\/html\/main-mobile\.html/i) > -1) ||
				(document.location.href.search(/syracuse-main\/html\/main-tablet\.html/i) > -1);
			var hide = (this.$isEditMode && !this.isLandingPage) || isMobileOrTabletPage; //(document.location.href.search(/syracuse-main\/html\/main-mobile\.html/i) > -1);
			if (this.isFusionPage) {
				hide = hide || this.overPanel && this.$facet === "$lookup";
			} else {
				if (this.overPanel && this.overPanel.isModal) {
					return;
				}
			}
			syra_dom.hide(modulesBarBody, hide);
	...
	_refreshScrollViewHeight: function() {
		if (this.scrollview) {
			...
			this.size.bodyHeight = this.size.scrollviewHeight - this.size.headerHeight;
		}
	},
	...
	drawPage: function() {
		...
		this.scrollview = syra_dom.div(this.$skin + "-scrollview");
		this.body = document.createElement("div");

		this.header = syra_dom.header(this.$skin + "-head", this.scrollview);

		// breadcrumb
		!this.isBreadCrumbHidden && _breadcrumb.show(this, this.header);
	...
	onMenuClick: function(options) {
		switch (options.menu.$bind) {
			case "page_breadcrumb":
				syra_nav.showBreadcrumbs(this.$breadCrumb, options);
				return false;
			default:
				break;
		}
	...
	onAfterLoad: function() {
		if (this.mainPageDesignerAccess === "disabled") {
			syra_site.refreshMainPageDesignerSecurity(this);
		}
