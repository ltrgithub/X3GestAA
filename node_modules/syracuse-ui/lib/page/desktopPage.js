...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	_loadAddNewItem: function(article, newAddItem) {
		...
		if (newAddItem.$fieldBind) {
			if (newAddItem.$isFreeField) {
				this.onExcludeField(newAddItem.$fieldBind, false);
			}
			$item = {
			...
			if (newAddItem.sectionNode) {
				for (var ii = 0, jj = newAddItem.sectionNode.garbage.length; ii < jj; ii++) {
					this.onExcludeField(newAddItem.sectionNode.garbage[ii], false);
				}
				for (var ii = 0, jj = newAddItem.sectionNode.free.length; ii < jj; ii++) {
					this.onExcludeField(newAddItem.sectionNode.free[ii], false);
				}
				if (newAddItem.sectionNode.loaded.length) {
					this.reloadingFields = this.reloadingFields || {};
					for (var ii = 0, jj = newAddItem.sectionNode.loaded.length; ii < jj; ii++) {
						var field = newAddItem.sectionNode.loaded[ii];
						if (field && field.$item && field.$item.$bind) {
							this.reloadingFields[field.$item.$bind] = field;
							/*if (field.rowSeparator) {
                             this.layoutValidator.removeRowSeparator(field);
                             }*/
	...
	loadSelectedDesignView: function(reloadLayout) {
		var self = this;
