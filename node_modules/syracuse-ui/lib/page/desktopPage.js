...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
    ...
    _showDefaultAuthoringApplyedDiagnoses: function(){
        ...
        this.showDiagnoses({
            $diagnoses: [{
                $message: localize.p_defaultApplied,
                $severity: "info",
                $links: {
                    "$restorePage": {
                        $title: localize.p_restorePage
                    },
                    "$save_authoring": {
                        $title: localize.p_saveAuthoring
                    },
                    "$saveAs_authoring": {
                        $title: localize.p_saveAsAuthoring
                    },
                    "$open_authoring": {
    ...
    warnAdministrationUpdates: function(showDiagnoses){
        ...
        if (this.$freeFieldBinds.length > 0) {
            if (showDiagnoses !== false) {
                ...
                this.showDiagnoses({
                    $diagnoses: [{
                        $message: localize.p_newField.replace("{0}", this.$freeFieldBinds.length),
                        $severity: "warning",
                        $links: {
                            "$forget_new_content": {
                                $title: localize.p_forgetNewContent
                            },
                            "$apply_default": {
    ...
    loadBox: function(initData, $initDiagnoses){
        ...
        this.appendMobileSlot();
        
    },
    appendMobileSlot: function(){
        if (document.site.$item.$isMobileConfig) {
            var self = this;
            self._item.className += " s-mobile-site";
            self.mobileSlot = document.createElement("div");
            self.mobileSlot.className = self.$skin + "-mobile-slot";
            self.dataSlot.parentNode.insertBefore(self.mobileSlot, self.dataSlot);
            self.mobileFrame = document.createElement("iframe");
            self.mobileFrame.className = self.$skin + "-mobile-frame";
            self.mobileFrame.onload = function(dd, dss, dsd){
                var cw = self.mobileFrame.contentWindow;
                if (cw.$.proto.auth.applyAuth) {
                    //  cw.$.proto.auth.applyAuth($auth.name, value);
                }
            };
            self.mobileFrame.setAttribute("src", "mobile-debug.html");
            self.mobileSlot.appendChild(self.mobileFrame);
    ...
    dispose: function(){
        ...
        this._$sourceArticle = this.authorWorkingCopy = this.garbageFields = this.pageViewSelector = this._$$diagnoseSlot = this._$$header = this._headerFoooter = this._headerViewSelector = null;
