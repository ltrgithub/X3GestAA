...
function _addTestEmbeddedLinks(page) {
	if (page.openerUrlSegments && page.openerUrlSegments.representationRoot == "employee") {
		if (page.openerUrlSegments.$facet == "$edit" || page.openerUrlSegments.$facet == "$details") {
...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	loadBox: function(initData) {
		...
		if (this.openerUrlSegments && this.openerUrlSegments.representationRoot == "employee") {
			_addTestEmbeddedLinks(this);
		}
	...
	_showBreadCrumb: function() {
		if (this.$breadCrumb && this.$breadCrumb.length > 0) {
			...
			var item, breadLink;
			for (var ii = 0, jj = this.$breadCrumb.length - 1; ii <= jj; ii++) {
				item = document.createElement("div");
				item.className = this.$skin + "-breadcrumb-item";
				item.textContent = this.$breadCrumb[ii];
				var child;
				if (ii == jj) {
					child = syra_menus.addIconButton("", this.$skin + "-breadcrumb-close", "onBreadcrumbClick");
				} else {
				...
				this.breadCrumbSlot.appendChild(item);
			}
		}
	},
