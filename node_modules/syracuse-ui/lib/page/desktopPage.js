...
exports.DesktopPage = _helpers.defineClass(DesktopPage, null, {
	...
	getUrlParams: function() {
		return this.urlSeg && this.urlSeg.params;
	},
	...
	addItem: function(slot, $item, boxParent, initData) {
		var item = syra_widgets.add(this, $item, boxParent || this);
		if (item) {
		...
		if (this.layoutSlot && this.size == undefined) {
			...
			if (this.overPanel) {
				var overSize = this.size.overPanel = {
					maxWidth: sizeSize.overPanel.maxWidth,
					maxHeight: sizeSize.overPanel.maxHeight
				};
				if (syra_site.isResizing) {
					delete this.overPanel.customPosition;
				}
				if (this.overPanel.customPosition) {
					overSize.maxWidth = this.overPanel.customPosition.width;
					overSize.maxHeight = this.overPanel.customPosition.height;
				}
				if (this.$facet == "$lookup" || this.$facet == "$select") {
					...
					this.size.maxBarHeight = overSize.maxHeight - overSize.marginHeight;
					this.size.barHeight -= overSize.marginHeight;
				}
			}
	...
	startChange: function() {
		...
		syra_log.page.data.startChange();
	},
	applyChange: function(newData) {
		if (!this.disposed && this.dataset) {
			syra_log.page.data.startApply(newData);
	...
	drawPage: function() {
		...
		if (!this.isFooterDisabled && !this.overPanel && !this.inlinePageHost && !this.isVignettePage) {
			this.footer = document.createElement("footer");
			this.footer.className = "s-site-footer";
			this.siteLegalLink = syra_site.addItem(this.footer, {
				$bind: "$legal",
				$category: "link",
