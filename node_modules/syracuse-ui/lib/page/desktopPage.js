...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	updateListScrollListener: function() {
		var self = this;
		if (this.listScrollListeners) {
			var lists = Object.keys(this.listScrollListeners);
			for (var ii = 0, jj = lists.length; ii < jj; ii++) {
				if (this.listScrollListeners[lists[ii]]) {
					this.listScrollListeners[lists[ii]](this.idMap[lists[ii]]);
				}
			}
		}
	},
	addListScrollListener: function(listId, listScrollBtnUpdateHandler) {
		this.listScrollListeners = this.listScrollListeners || {};
		this.listScrollListeners[listId] = listScrollBtnUpdateHandler;
	},
	removeListScrollListener: function(listId) {
		if (this.listScrollListeners) {
			this.listScrollListeners[listId] = null;
		}
	},
