...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	...
	ensureDesignerOpenerVisibility: function() {
		if (this.mainPageDesignerAccess != "unknow") {
			...
			var enabled = this.mainPageDesignerAccess != "disabled" && !(site.userProfile.getAuthoringLevel() === "none");
			if (site.pageDesignerOpener) {
				site.pageDesignerOpener.style.visibility = enabled ? "" : "hidden";
			}
		}
	},
	tempautoAddCubeFields: function() {
		...
		if ($cubeField && $binds.length) {
			var $items = document.site.layoutUpdater.getItemSettings(this, null, true) || [];
			var definedBinds = {};
	...
	getScrollviewSize: function() {
		if (!this.scrollviewSize) {
			this.scrollviewSize = document.site.dom.getBoundingClientRect(this.scrollview);
		}
	...
	resizePage: function() {
		if (this.isPageLoaded && !this._isDataChanging) {
			...
			document.site.dialogManager.resizePositionedDialogs(this);
			if (this.dialogWrapper && this.dialogWrapper.isOpened) {
				RawPage.prototype.resizePage.call(this);
				this.dialogWrapper.resizeDialog();
			} else {
				if (this.menuBar) {
	...
	loadPageViewSelector: function() {
		document.site.dom.empty(this.headerViewSelector);
	...
	showDiagnoses: function(message, options) {
		if (!this.diagnosesPanel) {
			this.diagnosesPanel = new document.site.diagnoseManager.DiagnosesPanel();
	...
	drawBox: function() {
		...
		this.domItem.setAttribute("data-s-page", this.id);
		...
		document.site.siteFunctions.showPageSecurity(this);
	},
	...
	_bindScrollView: function(bind) {
		var self = this;
		if (bind) {
			self.scrollHandler = self.scrollview.addEventListener("scroll", function(evt) {
				self.onScrollView();
			});
		} else {
			if (self.scrollview) {
	...
	appendPageMenus: function(slot) {
		if (this.$facet == "$details" || this.$facet == "$edit" || this.$facet == "$modal2") {
			...
			document.site.dom.empty(this._headerNav);
	...
	diposeContent: function() {
		...
		if (this.menuBar) {
			if (this.menuBar.barSlot) {
				document.site.dom.removeChild(this.menuBar.barSlot);
			}
			this.menuBar.dispose();
		}
		if (this.diagnosesPanel) {
			this.diagnosesPanel.dispose();
			document.site.dom.empty(this.diagnoseSlot);
		}
	...
	loadSelectedDesignView: function($selectedView) {
		var self = this;
		var query = document.controller.parseUrl(document.site.urlMaker.buildPageCollaborationUrl(self, self.openerUrlSegments));
		query.sendRequest({
			params: {
				select: "$views"
			}
			...
			if (self.pageViewSelector) {
				...
				if (self.openerUrlSegments) {
					...
					document.site.urlMaker.build(self.openerUrlSegments);
					if (!document.site.isMobileSite) {
						document.site.history.update(self, self.openerUrlSegments.$url);
					}
				}
			}
		});
	...
	dispose: function() {
		...
		if (this.memoryArea) {
			document.site.dom.empty(this.memoryArea);
		}
		if (this.breadCrumbSlot) {
			this._breadCrumb = this.breadCrumbSlot = null;
		}
		this.$userPreferencesLink = this._headerNav = this.securityViewSlot = this.security = this.openerUrlSegments = this.noProtoFields = null;
		...
		this.memoryArea = this.diagnosesPanel = this.headerCore = this.headerCoreLeft = this.domDescription = null;
