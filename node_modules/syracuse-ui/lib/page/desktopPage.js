...
var DesktopPageDesigner = require("syracuse-ui/lib/authoring/desktopPageDesigner").DesktopPageDesigner;
...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
	designPage: function(open) {
		if (open) {
			this.designer = new DesktopPageDesigner();
			this.designer.loadBox(this);
		} else {
			if (this.designer) {
				this.showDiagnoses({
					$diagnoses: null
	...
	loadBox: function(initData, $initDiagnoses) {
		this.ensureDesignerOpenerVisibility(!this.isDashBoard && !this.isVignettePage);
		this.loadGarbage();
	...
	onFillLookupList: function(list) {
		if (this.dialogWrapper && this.dialogWrapper._content) {
			var invisible = list.builder.scroller.getInvisibleSize();
	...
	appendHeader: function() {
		...
		this.headerViewSelector.className = this.$skin + "-head-core-right";
		this.headerCore.appendChild(this.headerViewSelector);


		this.header.appendChild(this.headerCore);
		this._$$header = $(this.header);

		this.headerFoooter = document.createElement("div");
		this.headerFoooter.className = this.$skin + "-head-footer";
		this.headerNav = document.createElement("div");
		this.headerNav.className = this.$skin + "-head-footer-nav";
		this.headerFoooter.appendChild(this.headerNav);

		this.loadPageViewSelector();
		this.header.appendChild(this.headerFoooter);
	...
	appendArticleMenus: function(slot) {
		if (this.$facet == "$details" || this.$facet == "$edit") {
			document.site.emptyDom(this.headerNav);
			this._menusPager = this.loadNewItem(this.headerNav, {
	...
	_loadAddNewItem: function(article, $newAddItem) {
		...
		if ($newAddItem.$fieldBind) {
			...
			//    $designLevel = $newAddItem.$designLevel;
			$item = $newAddItem.$item || {
				$category: "section"
			};
		}
		return article.layoutContent.loadChildItem(null, $item);
	},
	onAddNewItem: function(article, targetItem, newItem, $insertAt) {
		...
		if (newItem.$designLevel == "field") {
			this.onExcludeField(newItem.$fieldBind || newItem.$vignette, false);
		}
	...
	onExcludeField: function($bind, isExcluded) {
		if (isExcluded) {
			...
			if (this.$freeFieldBinds.indexOf($bind) >= 0) {
				this.$freeFieldBinds.splice(this.$freeFieldBinds.indexOf($bind), 1);
			}
				...
				if ($targetAuthoringLevel == "block" && $newAuthoringLevel == "section") {
					...
					if (dropItem.layoutContent.$chilAuthoringLevel == "block") {
						...
						if (children.length > 0) {
							newChild = this.layoutContent.ensureSection(children[0], targetItemIndex);
	...
	dispose: function() {
		...
		this.$freeFieldBinds = this.headerNav = this.securityViewSlot = this.security = this.openerHttpQuery = this.noProtoFields = null;
		this.$sourcePrototype = this._$sourceArticle = this.authorWorkingCopy = this.garbageFields = this.pageViewSelector = null;
		this._advancedFieldsSlot = this._advancedFieldsLink = this._$$ditagnoseSlot = this.diagnoseSlot = this.dataSlot = this._$$header = this.headerFoooter = this.headerViewSelector = null;
