...
exports.DesktopPage = helpers.defineClass(DesktopPage, RawPage, {
    initiliazeGarbageLinks: function($garbageLinks){
        if ($garbageLinks) {
            this.garbageLinks = {};
            for (var ii = 0, jj = $garbageLinks.length; ii < jj; ii++) {
                this.garbageLinks[$garbageLinks[ii]] = 1;
            }
        }
    },
    saveGarbageLinks: function($article){
        if (this.$pageCategory == "dashboard") {
    ...
    onWindowResize: function(){
        ...
        if (document.site.authorPage && document.site.authorPage.targetPage == this) {
            document.site.authorPage.palette.onWindowResize();
        }
    ...
    onFillLookupList: function(){
        if (this.dialogWrapper._content) {
            ...
            if (!diff) {
                diff = (this.lookupList.builder._bodySlot.scrollWidth - this.lookupList.builder._$$bodySlot.width()) ? 10 : 0;
            }
            if (diff) {
            ...
            if (value !== this._$selectedVariantView) {
                ...
                query.sendRequest({}, function(data, response, requestUrl){
                    if (data.$prototype) {
                        ...
                        self.$views = data.$views || self.$views
                        self.showVariantSelector(self.$views);
                        /*    self.openerHttpQuery.$urlParts.params.variant = value;
                         self.openerHttpQuery.applyParameters(self.openerHttpQuery.$urlParts.params);
                         document.controller.updateHistory(self.openerHttpQuery.$url);*/
                    }
                });
            }
            return;
    ...
    showVariantSelector: function($views){
        ...
        if ($views && $views.length > 1) {
            ...
            this.loadNewItem(this._headerVariants, {
                ...
                $title: this.pageLocalize.p_viewVariant,
    ...
    appendHeader: function(){
        ...
        if (this.$breadCrumb) {
            this._header.setAttribute("data-s-breadcrumb", true)
    ...
    setBreadCrumb:function(breadCrumb){
    	if(this.domBreadCrumb){
            ...
            breadLink.className = this.$skin + "-head-breadcrumb";
            breadLink.setAttribute("href","#")
            breadLink.textContent = this.getTitle();
    ...
    dispose: function(){
        ...
        this._$$diagnoseSlot = this._$$header = this._$$dataSlot = this.$$scrollview = this.scrollview = this.$menus = this.menuBar = null;
