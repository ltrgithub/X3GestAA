...
exports.NavigationPage = helpers.defineClass(NavigationPage, DesktopPage, {
	designItem: function() {
		if (this.menusBlocks) {
			var ids = Object.keys(this.menusBlocks);
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				this.menusBlocks[ids[ii]].toggleDesignMode();
			}
		}
	},
	ensureDesignedItemVisibility: function(item, event) {
		if (!this.designing || !event) {
			if (item.menusSlot) {
				item.menusSlot.style.visibility = "hidden";
			}
			if (item.reorderPicker) {
				item.reorderPicker.style.visibility = "hidden";
			}
		} else {
			if (event) {
				var visible = event.type == "mouseenter" ? "visible" : "hidden";
				if (item.menusSlot) {
					item.menusSlot.style.visibility = visible;
				}
				if (item.reorderPicker) {
					item.reorderPicker.style.visibility = visible;
				}
			}
		}
	},
	...
	isAdminMode: function() {
		return this.$prototype.$editMode == "admin";
	},
	_ensureMenus: function() {
		this.page.loadNewItem(this.headerCoreRight, {
			...
			$bind: this.isAdminMode() ? "$details" : "$edit",
			$icon: {
				$mode: "icon",
				$path: "page/s-navpage-designer-"
			}
		}, this);
	...
	loadBox: function(initData, $initDiagnoses) {
		...
		if (modules.topbar) {
			modules.topbar.className += " s-nav-modules-bar";
		}
	},
	...
	changeMode: function($url) {
		var self = this;
		var $newEditmode = this.isAdminMode() ? "user" : "admin";
		document.controller.parseUrl($url).sendRequest(null, function($representation, response, $url) {
			$representation.$pageRepresentationURL = $url;
			$representation.$prototype.$editMode = $newEditmode;
			document.site.onMainPageChange({
	...
	onMenuClick: function(menuItem) {
		var self = this;
		if (this.designing && menuItem.isNavigationMenuItem) {
			return false;
		}
		if (!menuItem.$isAction) {
			switch (menuItem.$item.$bind) {
				case "$edit":
					this.changeMode(menuItem.$url);
					return false;
				case "$details":
					this.changeMode(menuItem.$url);
					return false;
	...
	notifySubModuleDataChange: function(field, value) {
	...
	notifyDataChange: function(field, value) {
		if (field.isSubModuleList || field.articleParent.isSubModuleRecord) {
			...
			if (!this.notifySubModuleDataChange(field, value)) {
	...
	onItemEnterLeave: function(event) {
		if (this.moduleBar) {
			if (event.currentTarget == this.moduleBar.barBody) {
				var modules = this.getModuleList();
				if (modules) {
					if (!this.designing) {
						if (modules.topbar) {
					...
					this.page.ensureDesignedItemVisibility(modules, event);
				}
			}
		}
	},
	dispose: function() {
		...
		this.menusBlocks = this.modules = this.moduleBar = null;
