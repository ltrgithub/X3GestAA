...
exports.BarSplitter = helpers.defineClass(BarSplitter, null, {
	...
	toggleBar: function(show) {
		this.isHidden = !show;
		if (this._barSlot) {
			this._barSlot.style.display = this.isHidden ? "none" : "";
		}
	},
		...
		if (!this.isHidden && !this._designing) {
			if (this.preferences.isCollapsed) {
				if (!this.isCollapsed) {
					...
					this.openedWidth = this._barSlot.style.width;
				}
				this._barSlot.style.width = "";
				this._barBody.style.display = "none";
			} else {
				this.isCollapsed = false;
				if (this.openedWidth) {
					this._barSlot.style.width = this.openedWidth;
				}
				if (!this.preferences.isDocked) {
					document.site.toggleClass(this._barSlot, "s-docked", false);
				}
			...
			document.site.toggleClass(this._barSlot, "s-close", this.isCollapsed);
	...
	onWindowResize: function(treebar) {
		if (this._bar && this._barSlot) {
			...
			if (height) {
				...
				this._barBody.style.height = height + "px";
				...
				if (treebar && treebar.onWindowResize) {
	...
	toggleUIDesign: function(enable, isOnPreviewMode) {
		if (enable) {
			...
			this._barSlot.style.display = this.authoringOverlay.style.display = (isOnPreviewMode && (!this.isHidden && this.isSlotVisible)) ? "" : "none";
			this.onWindowResize();
		} else {
			if (this.authoringOverlay) {
	...
	dispose: function() {
		...
		this.$$bar = this.page = this._barSlot = this._barBody = this._bar = null;
