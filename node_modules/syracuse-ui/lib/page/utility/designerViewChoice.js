...
exports.registerDesignViews = function(page, select) {
	...
	if (page.$selectedDesignView && page.$views.length > 0) {
		if (!page.designerViewOpener) {
			page.designerViewOpener = syra_menus.addIconButton(syra_local.aw_pg_choice_opener, "s-designer-view-opener", "togglePageOptions");
...
function _switchView(page, $view) {
...
exports.onDesignViewClick = function(page, event, target) {
	if (!target.syraIsDisabled) {
		...
		(page.$selectedDesignView != $view) && _switchView(page, $view);
	}
};
		...
		if (!page.designViewsPopup) {
			...
			for (var ii = 0, jj = page.$views.length; ii < jj; ii++) {
				...
				if ($view.$selected) {
					...
					view.textContent = view.title = $view.$isFactory ? syra_local.aw_pg_choice_factory : $view.$title;
					view.syraTip = 1;
					...
					if ($view.$isFactory) {
						var view = syra_menus.addTextButton(syra_local.aw_pg_choice_factory, "s-designer-view-factory", "onDesignViewClick");
						...
						var btn = syra_menus.addIconButton(syra_local.aw_pg_choice_edit, "s-designer-view-btn", "onDesignViewClick", null, "$edit");
						syra_site.dom.disableItem(btn, true);
						view.appendChild(btn);
						btn = syra_menus.addIconButton(syra_local.aw_pg_choice_delete, "s-designer-view-btn", "onDesignViewClick", null, "$delete");
						syra_site.dom.disableItem(btn, true);
						view.appendChild(btn);
						if (!more) {
