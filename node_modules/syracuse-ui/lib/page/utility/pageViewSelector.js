...
exports.PageViewSelector = helpers.defineClass(PageViewSelector, RawPage, {
	load: function(page, layoutSlot, $selectedView) {
	...
	selectView: function($view) {
		var selectedId = this._extractKeyFromUrl($view.$url);
		if (selectedId) {
			this.applyChange({
					...
					query.sendRequest({}, function(data, response, requestUrl) {
						if (data.$prototype) {
							setTimeout(function() {
								...
								document.controller.updateHistory(url);
							...
							if (data.$article) {
								...
								if (data.$article.$generatorVersion) {
									document.site._cvgPageConverterMakeVolatile(helpers.object.clone(self.targetPage.$sourcePrototype, true), data.$article);
								}
							...
							if (self.targetPage.openerHttpQuery) {
