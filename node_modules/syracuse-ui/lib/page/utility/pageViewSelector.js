					...
					query.sendRequest({}, function(data, response, requestUrl) {
						if (data.$prototype) {
							setTimeout(function() {
								...
								$itemPage.httpQuery.$url = document.site.urlMaker.applyParameters($itemPage.httpQuery.$url, $itemPage.httpQuery.$urlParts.params);
								var url = decodeURIComponent($itemPage.httpQuery.$url);
								document.site.history.update(url);
								document.controller.changeMainPage(url, $itemPage);
							}, 100);
						} else {
							if (data.$authorUrl) {
							...
							var httpQuery = self.targetPage.openerHttpQuery;
							if (httpQuery) {
								httpQuery.$urlParts.params.pageview = value;
								httpQuery.$url = document.site.urlMaker.applyParameters(httpQuery.$url, httpQuery.$urlParts.params);
								document.site.history.update(decodeURIComponent(httpQuery.$url));
							}
							if (self.targetPage.designer) {
								self.targetPage.designer.toggleUIDesign(self.targetPage, true);
