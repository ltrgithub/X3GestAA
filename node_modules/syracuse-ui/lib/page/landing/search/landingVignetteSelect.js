...
exports.LandingVignetteSelect = helpers.defineClass(LandingVignetteSelect, DesktopPage, {
	...
	onMenuClick: function(menu) {
		...
		if (!menu.$bind && menu.$item && menu.$item.$bind == "$detail") {
			if (this._findSelectedVignette(menu.articleParent.dataset.$key || menu.articleParent.dataset.$uuid)) {
				...
				document.site.dom.toggleClass(menu.articleParent.cardContent, "s-selected");
				...
				document.site.dom.toggleClass(menu.articleParent.cardContent, "s-selected", true);
				// update vignette categories and module info style
	...
	_updatePropertiesStyle: function(record, selected, onClick) {
		if (record.boundFields.properties[0].fieldValue.children.length > 0) {
			...
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				document.site.dom.toggleClass(children[ii], "s-selected", selected);
			}
	...
	onModuleClick: function(picker) {
		...
		if (!this.selectedModId || (this.selectedModId != modId)) {
			document.site.dom.toggleClass(this.modulesLayoutHeader, "s-active", true);
			document.site.dom.toggleClass(this.categoriesLayoutHeader, "s-active");
			document.site.dom.toggleClass(this.vignettesLayoutHeader, "s-active");
			// update selected css
			if (this.selectedModId) {
				document.site.dom.toggleClass(this.selectedModLink, "s-selected");
			}
			document.site.dom.toggleClass(picker, "s-selected", true);
			this.selectedModId = modId;
			this.selectedModLink = picker;
			document.site.dom.empty(this.categoriesLayoutBody);
			document.site.dom.empty(this.vignettesLayoutBody);
	...
	onCategoryClick: function(picker) {
		...
		if (!this.selectedCatId || this.selectedCatId != catId) {
			document.site.dom.toggleClass(this.modulesLayoutHeader, "s-active");
			document.site.dom.toggleClass(this.categoriesLayoutHeader, "s-active", true);
			// update selected css
			if (this.selectedCatId) {
				document.site.dom.toggleClass(this.selectedCatLink, "s-selected");
			}
			document.site.dom.toggleClass(picker, "s-selected", true);
			...
			for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
				...
				if (this._findSelectedVignette(vignettes[ii].$key || vignettes[ii].$uuid)) {
					document.site.dom.toggleClass(vignetteItem, "s-selected", true);
				}
	...
	onVignetteClick: function(picker) {
		document.site.dom.toggleClass(this.modulesLayoutHeader, "s-active");
		document.site.dom.toggleClass(this.categoriesLayoutHeader, "s-active");
		document.site.dom.toggleClass(this.vignettesLayoutHeader, "s-active", true);
		var isSelected = picker.className.indexOf("s-selected") >= 0;
		document.site.dom.toggleClass(picker, "s-selected", !isSelected);
	...
	_applySelectedStyle: function() {
		...
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			...
			if (this._findSelectedVignette(record.dataset.$key)) {
				document.site.dom.toggleClass(record.cardContent, "s-selected", true);
			}
		}
	},
	...
	_updateSelectedEndpoint: function(endpoint) {
		...
		document.site.dom.empty(this.securityViewSlot);
		if (endpoint) {
	...
	_displaySearchFacetsBar: function(page) {
		if (!page.menuBar.page.designer) {
			if (page.boundFields.$searchFacets && page.boundFields.$searchFacets[0].hasFacet) {
				...
				//document.site.dom.toggleClass(page.menuBar.barSlot, "s-sv-search-menubar", true);
	...
	_updateBrowseViewDisplay: function(show) {
		document.site.dom.empty(this.idMap["s-sv-browse-section"].body);
		...
		document.site.dom.toggleClass(this.idMap["s-sv-browse-section"].domTitle, "s-disabled");
	...
	_bindEvents: function(bind) {
		var self = this;
		if (self.domItem) {
			var $$domItem = $(self.domItem);
			if (bind) {
				$$domItem.delegate(".s-sv-search-res-carditem", "click", function() {
					if (!this.getAttribute("data-s-menu")) {
						var $$menu = $(this).find("a[data-s-menu]");
						var menu = document.site.itemHelper.findMenu($$menu[0]);
				...
				$$domItem.delegate(".s-sv-search-res-carditem", "mouseover", function() {
					if (!this.getAttribute("data-s-menu")) {
						var $$menu = $(this).find("a[data-s-menu]");
						var menu = document.itemHelper.findMenu($$menu[0]);
						self._updatePropertiesStyle(menu.articleParent, true);
						return false;
					}
				});
				$$domItem.delegate(".s-sv-search-res-carditem", "mouseout", function() {
					if (!this.getAttribute("data-s-menu")) {
						var $$menu = $(this).find("a[data-s-menu]");
						var menu = document.itemHelper.findMenu($$menu[0]);
