...
exports.LandingVignetteSelect = helpers.defineClass(LandingVignetteSelect, DesktopPage, {
	...
	getPrototype: function() {
		...
		if (this.$prototype && this.$prototype.$properties) {
			this.$prototype.$properties.$searchVignetteField = {
				...
				$title: syra_site.localize.ldpSearch
			};
			...
			this.$prototype.$properties.selectedVignettes = {
				...
				$title: syra_site.localize.ldpSelectedVignettes,
				$item: {
					$type: "application/x-string"
				}
			};
					...
					$properties: {
						$searchResult: {
							...
							$item: {
								...
								$properties: {
									title: {
										$title: syra_site.localize.ldpSearchResultTitle,
										$type: "application/x-string",
										$format: "$html",
										$links: {
											$details: {}
						...
						$layout: {
							...
							$items: [{
								$items: [{
									...
									$noDataText: syra_site.localize.search_no_res,
									$format: "cards",
	...
	onModuleClick: function(picker) {
		...
		if (!this.selectedModId || (this.selectedModId != modId)) {
			...
			syra_site.dom.toggleClass(picker, "s-selected", true);
			...
			catKeys.sort(function(itemA, itemB) {
				if (categories[itemA].description > categories[itemB].description)
					return categories[itemB].description == syra_site.localize.ldpNoCategory ? -1 : 1;
				if (categories[itemA].description < categories[itemB].description)
					return categories[itemA].description == syra_site.localize.ldpNoCategory ? 1 : -1;
				return 0;
			});
			...
			for (var ii = 0, jj = catKeys.length; ii < jj; ii++) {
				...
				categoryItem.syraOnClick = "onCategoryClick";
	...
	onCategoryClick: function(picker) {
		...
		if (!this.selectedCatId || this.selectedCatId != catId) {
			...
			for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
				...
				if (this._findSelectedVignette(vignettes[ii].$key || vignettes[ii].$uuid)) {
					syra_site.dom.toggleClass(vignetteItem, "s-selected", true);
				}
				vignetteItem.syraOnClick = "onVignetteClick";
				vignetteItem.setAttribute("sv-vignette-id", vignettes[ii].$key || vignettes[ii].$uuid);
				vignetteItem.textContent = vignettes[ii].title || syra_site.localize.ldpNoTitle;
		...
		for (var ii = 0, jj = data.$resources.length; ii < jj; ii++) {
			...
			if (!this.vignetteData.modules[moduleKey]) {
				this.vignetteData.modules[moduleKey] = resource.module || {
					title: syra_site.localize.ldpNoModule
				};
			...
			if (!hasCategories) {
				if (!this.vignetteData.modules[moduleKey].categories[noCategoryKey]) {
					this.vignetteData.modules[moduleKey].categories[noCategoryKey] = {
						$uuid: noCategoryKey,
						$vignettes: [],
						description: syra_site.localize.ldpNoCategory
					};
				}
	...
	_buildBrowseView: function() {
		...
		this.modulesLayoutHeader.textContent = syra_site.localize.ldpModulesTitle;
		...
		modKeys.sort(function(itemA, itemB) {
			if (self.vignetteData.modules[itemA].title > self.vignetteData.modules[itemB].title)
				return self.vignetteData.modules[itemB].title == syra_site.localize.ldpNoModule ? -1 : 1;
			if (self.vignetteData.modules[itemA].title < self.vignetteData.modules[itemB].title)
				return self.vignetteData.modules[itemA].title == syra_site.localize.ldpNoModule ? 1 : -1;
			return 0;
		});
		...
		for (var ii = 0, jj = modKeys.length; ii < jj; ii++) {
			...
			modItem.syraOnClick = "onModuleClick";
		...
		this.categoriesLayoutHeader.textContent = syra_site.localize.ldpCategoriesTitle;
		...
		this.vignettesLayoutHeader.textContent = syra_site.localize.ldpVignettesTitle;
	...
	ensureDefaultArticle: function($article, $prototype) {
		...
		if ($article) {
			...
			return {
				$title: syra_site.localize.ldpAddNewVignette,
				$layout: {
					$items: [{
						$category: "section",
						$isTitleHidden: true,
						...
						$layout: {
							$items: [{
								...
								$category: "section",
								$title: syra_site.localize.ldpSelectEndpoint,
								...
								$category: "section",
								$title: syra_site.localize.ldpBrowseCategory,
							...
							$items: [{
								...
								$layout: {
									$items: [{
										...
										$title: syra_site.localize.ldpUseSearch,
										$layout: {
											$items: [{
										...
										$noDataText: syra_site.localize.ldpNoVignetteSelected,
										$isHidden: true
									}]
								}
							}, {
								$category: "section",
								$title: syra_site.localize.ldpNoVignetteFounds,
	...
	dispose: function() {
		this.browseLayout = this.browseView = this.categoriesLayout = this.categoriesLayoutBody = this.categoriesLayoutHeader = this.modulesLayoutHeader = null;
