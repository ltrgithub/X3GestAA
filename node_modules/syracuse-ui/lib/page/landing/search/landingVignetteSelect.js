...
exports.LandingVignetteSelect = helpers.defineClass(LandingVignetteSelect, DesktopPage, {
	...
	onMenuClick: function(menu) {
		...
		if (_gadgetTypes.indexOf(menu.$bind) >= 0) {
			...
			self.sendRequest(menu.$method, menu.$url, function(data) {
				self.selectedCategoryTitle = menu.$title;
				if (!self.backBtn) {
					self.backBtn = syra_menus.addIconButton(syra_local.ldpBack, "s-sv-back-link", "onBackClick");
		...
		if (menu.$item && menu.$item.$bind == "$detail") {
			if (this._findSelectedVignette(menu.articleParent.dataset.$key || menu.articleParent.dataset.$uuid)) {
				...
				syra_site.dom.toggleClass(menu.articleParent.cardContent, "s-selected");
				...
				syra_site.dom.toggleClass(menu.articleParent.cardContent, "s-selected", true);
				// update vignette categories and module info style
	...
	_updatePropertiesStyle: function(record, selected, onClick) {
		if (record.boundFields.properties[0].fieldValue.children.length > 0) {
			...
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				syra_site.dom.toggleClass(children[ii], "s-selected", selected);
			}
			if (onClick) {
	...
	onModuleClick: function(event, btn) {
		...
		if (!this.selectedModId || (this.selectedModId != modId)) {
			...
			if (this.selectedModId) {
				syra_site.dom.toggleClass(this.selectedModLink, "s-selected");
			}
			...
			for (var ii = 0, jj = catKeys.length; ii < jj; ii++) {
				...
				btnSlot.className = "s-sv-browse-category-item-slot";
				this.categoriesLayoutBody.appendChild(btnSlot);

				var btn = syra_menus.addTextButton(this.vignetteData.modules[modId].categories[catKeys[ii]].description, "s-sv-browse-category-item", "onCategoryClick");
	...
	onCategoryClick: function(event, btn) {
		...
		if (!this.selectedCatId || this.selectedCatId != catId) {
			...
			if (this.selectedCatId) {
				syra_site.dom.toggleClass(this.selectedCatLink, "s-selected");
			}
			syra_site.dom.toggleClass(btn, "s-selected", true);
			...
			for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
				...
				slot.className = "s-sv-browse-vignette-item-slot";
				var btn = syra_menus.addTextButton(vignette.title || syra_local.ldpNoTitle, "s-sv-browse-vignette-item", "onVignetteClick");
				if (this._findSelectedVignette(vignette.$key || vignette.$uuid)) {
					syra_site.dom.toggleClass(btn, "s-selected", true);
				}
				btn.setAttribute("sv-vignette-id", vignette.$key || vignette.$uuid);
				btn.title = vignette.code;
				slot.appendChild(btn);
				this.vignettesLayoutBody.appendChild(slot);
			}
		}
		this.endChange();
	},
	onVignetteClick: function(event, btn) {
		...
		var isSelected = btn.className.indexOf("s-selected") >= 0;
		syra_site.dom.toggleClass(btn, "s-selected", !isSelected);
		...
		for (var ii = 0, jj = $dataResources.length; ii < jj; ii++) {
			...
			if (res.categories) {
				for (var mm = 0, pp = res.categories.length; mm < pp; mm++) {
					...
					span.innerHTML = res.categories[mm];
					res.properties += span.outerHTML;
	...
	_applySelectedStyle: function() {
		...
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			...
			if (this._findSelectedVignette(record.dataset.$key)) {
				syra_site.dom.toggleClass(record.cardContent, "s-selected", true);
			}
		}
	},
	...
	_updateSelectedEndpoint: function(endpoint) {
		...
		if (endpoint) {
			...
			item.textContent = item.title = endpoint.$value || endpoint.description;
			this.securityViewSlot.appendChild(item);
		}
	},
	...
	_displaySearchFacetsBar: function(page) {
		if (!page.menuBar.page.designer) {
			if (page.boundFields.$searchFacets && page.boundFields.$searchFacets[0].hasFacet) {
				...
				//syra_site.dom.toggleClass(page.menuBar.barSlot, "s-sv-search-menubar", true);
	...
	_updateBrowseViewDisplay: function(show) {
		syra_site.dom.empty(this.idMap["s-sv-browse-section"].body);
		...
		syra_site.dom.toggleClass(this.idMap["s-sv-browse-section"].domTitle, "s-disabled");
	...
	_buildBrowseView: function() {
		...
		this.modulesLayoutHeader.textContent = syra_local.ldpModulesTitle;
		...
		for (var ii = 0, jj = modKeys.length; ii < jj; ii++) {
			...
			var slot = document.createElement("div");
			slot.className = "s-sv-browse-module-slot";

			var modIcon = document.createElement("div");
			modIcon.className = "s-sv-browse-module-icon";
			modIcon.setAttribute("style", "background-image: url('/syracuse-ui/themes/desktop/sage/images/modules/s_module_" + (this.vignetteData.modules[modKey].code || "no_code").toLowerCase() + ".png')");
			slot.appendChild(modIcon);

			var btn = syra_menus.addTextButton(this.vignetteData.modules[modKey].title, "s-sv-browse-module-item", "onModuleClick");
			btn.setAttribute("sv-module-id", modKey);
			btn.title = this.vignetteData.modules[modKey].code || "";
			slot.appendChild(btn);
		...
		this.categoriesLayoutHeader.textContent = syra_local.ldpCategoriesTitle;
		...
		this.vignettesLayoutHeader.textContent = syra_local.ldpVignettesTitle;
	...
	ensureDefaultArticle: function($article, $prototype) {
		...
		if ($article) {
			...
			var $items = [];
			var masterRecord = syra_site.mainPage.selectedPageRecord;
			var useCurrentEndpoint = masterRecord && masterRecord.dataset && masterRecord.dataset.useCurrentEndpoint;
			$items.push({
				$category: "section",
				$isTitleHidden: true,
				...
				$layout: {
					$items: [{
						...
						$category: "section",
						$title: useCurrentEndpoint ? syra_local.ldpBrowseCategoryNoEndpoint : syra_local.ldpBrowseCategory,
	...
	dispose: function() {
		this.backBtn = this.browseLayout = this.browseView = this.categoriesLayout = this.categoriesLayoutBody = this.categoriesLayoutHeader = this.modulesLayoutHeader = null;
		this.vignettesLayoutBody = this.vignettesLayoutHeader = this.vignettesLayout = null;
