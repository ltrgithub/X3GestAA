...
exports.LandingVignetteSelect = helpers.defineClass(LandingVignetteSelect, DesktopPage, {
										...
										$items: [{
											...
											$layout: {
												...
												$items: [{
													...
													$isTitleHidden: true,
													$noDataText: " ",
													$skin: "s-sv-search-res-mod"
												}, {
													$bind: "menuCategory",
													$isTitleHidden: true,
													$noDataText: " ",
													$skin: "s-sv-search-res-cat"
												}]
											}
										}]
									}
	...
	onMenuClick: function(menu) {
		...
		if (!menu.$bind && menu.$item && menu.$item.$bind == "$detail") {
			var divImg = menu._label.children[0].children[0];
			if (this._findSelectedVignette(menu.articleParent.dataset.$uuid)) {
				document.site.toggleClass(divImg, "s-sv-search-res-title-selected");
				document.site.toggleClass(divImg, "s-sv-search-res-title-unselected", true);
				this._onVignetteRemove(menu.articleParent.dataset.$uuid);
			} else {
				document.site.toggleClass(divImg, "s-sv-search-res-title-selected", true);
				document.site.toggleClass(divImg, "s-sv-search-res-title-unselected");
			...
			case "sv-category-picker":
				...
				if (!this.selectedCatId || this.selectedCatId != catId) {
					...
					for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
						...
						if (this._findSelectedVignette(vignettes[ii].$uuid)) {
	...
	_formatResourcesData: function($dataResources) {
		...
		for (var ii = 0, jj = $dataResources.length; ii < jj; ii++) {
			...
			if (res.module) {
				...
				div.textContent = res.module.$value;
				res.menuModule = [div.outerHTML];
			}
			if (res.categories) {
				res.menuCategory = [];
				for (var mm = 0, pp = res.categories.length; mm < pp; mm++) {
					...
					div.textContent = res.categories[mm].$value;
					res.menuCategory.push(div.outerHTML);
				}
			}

			if (res.title) {
				var div = document.createElement("div");
				div.className = "s-sv-search-res-title-slot";
				var divImg = document.createElement("div");
				divImg.className = this._findSelectedVignette(res.$key || res.$uuid) ? "s-sv-search-res-title-selected" : "s-sv-search-res-title-unselected";
				var divText = document.createElement("div");
				divText.className = "s-sv-search-res-title-text";
				divText.textContent = res.title;
				div.appendChild(divImg);
				div.appendChild(divText);
				res.title = div.outerHTML;
			}
	...
	ensureDefaultArticle: function($article, $prototype) {
		...
		if ($article) {
			...
			return {
				$title: document.site.localize.landingVignetteSelectTitle,
				$layout: {
					$items: [{
						$category: "section",
						$isTitleHidden: true,
						...
						$layout: {
							$items: [{
								...
								$category: "section",
								$title: "1. Please select endpoint",
							...
							$items: [{
								...
								$layout: {
									$items: [{
										...
										$title: "Use the search engine to directly find vignettes",
										$layout: {
											$items: [{
										...
										$noDataText: "No vignette selected or set",
										$isHidden: true
									}]
								}
							}, {
								$category: "section",
								$title: "Vignettes founds",
	...
	_bindEvents: function(bind) {
		...
		if (self.$$item) {
			if (bind) {
				self.$$item.delegate(".s-sv-search-res-carditem-body", "click", function() {
