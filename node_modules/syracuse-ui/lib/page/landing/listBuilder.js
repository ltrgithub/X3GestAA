...
exports.ListBuilder = helpers.defineClass(ListBuilder, null, {
	...
	record_onSelect: function(record, selected) {
		...
		if (selected) {
			...
			if (record.useCurrentEndpoint != record.dataset.useCurrentEndpoint) {
				this.record_clearContent(record);
			}
	...
	record_clearContent: function(record) {
		var page = record.page;
		if (page && record.vignettes) {
			...
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				delete record.vignettes[$binds[ii]];
			}
		}
	},
