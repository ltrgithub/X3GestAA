...
exports.ListBuilder = helpers.defineClass(ListBuilder, null, {
	...
	record_onSelect: function(record, selected) {
		...
		if (selected) {
			...
			var badEndpoint = record.useCurrentEndpoint != record.dataset.useCurrentEndpoint;
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if (vignette) {
					...
					vignette.layoutParent && vignette.layoutParent.extractItem(vignette);
				}
			}
			...
			if (removed.length) {
				for (var ii = 0, jj = removed.length; ii < jj; ii++) {
					page.removeItem(removed[ii], true, true);
				}
			}
	...
	record_dispose: function(record) {
		...
		if (page && record.vignettes) {
			...
			var $binds = Object.keys(record.vignettes);
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				page.removeItem(record.vignettes[$binds[ii]], false, true);
			}
		}
	},
	dispose: function() {
