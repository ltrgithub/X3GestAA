...
function _add_ldpFunctions(record) {
	...
	record.ldp_clearContent = function() {
		if (this.page) {
			syra_dom.removeChild(this.page.body);
			if (this.$vignettes) {
			...
			if (this.page.ldpRecord == this) {
				...
				syra_layoutUpdater.clearContent(this.page.layoutContent);
...
exports.ListBuilder = helpers.defineClass(ListBuilder, null, {
	...
	saveReorder: function(reorderRecord, targetRecord, isAfter) {
		if (this.list.ldp_isUserList) {
			...
			var pages = page.$globalPreferences.pages = [];
	...
	list_onAfterFill: function() {
		if (this.list.ldp_isUserList) {
			var pages = this.list.page.$globalPreferences.pages;
	...
	record_draw: function(record) {
		...
		record.domItem = syra_dom.addDiv(record.$skin = "s-ldpm-record");
		record.domItem.syrainout = record.domItem.syraItem = record.id;
		record.dataSlot = record.body = syra_dom.addDiv(record.$skin + "-title", record.domItem);
		...
		if (this.list.ldp_isUserList) {
			record.menusSlot = syra_dom.addDiv("s-ldpm-record-btns", record.domItem);
		...
		if (selected) {
			...
			if ($binds.length) {
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					...
					if (vignette) {
						if (!record.$vignettes[$bind]) {
							...
							syra_dom.removeChild(vignette.layoutSlot);
							syra_item.remove(vignette, true, true);
							if (record.dataset.stdLayout && record.dataset.stdLayout.content) {
								record.dataset.stdLayout.content = syra_layoutUpdater.cleanBeforeSave(page.$item);
							}
						}
				...
				if (!record.isLayoutDisplay) {
					...
					page.$item.$layout.$tabIndex = page.$globalPreferences.tabs[record.$uuid] || 0;
			...
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if (!(field && field.length)) {
					...
					tab.layoutContent.removeSpaceBox();
			...
			page.$globalPreferences.$selected = record.dataset.$uuid;
			!page.body.parentNode && page.footer.parentNode.insertBefore(page.body, page.footer);
			if (page.isPageLoaded) {
				...
				if (page.designer) {
					...
					syra_site.ensureArticleVisibility(page, true);
				}
			}
		} else {
