...
function _ensureChildPageSelection(masterPage, ignoreLastSelected) {
	var $uuid = (!ignoreLastSelected) ? (masterPage.preferences.$selected || masterPage.dataset.$selected) : undefined;
	if (!$uuid) {
		$uuid = masterPage.userPages && masterPage.userPages.dataset && masterPage.userPages.dataset.length && masterPage.userPages.dataset[0].$uuid;
		if (!$uuid) {
			$uuid = masterPage.rolePages.dataset && masterPage.rolePages.dataset.length && masterPage.rolePages.dataset[0].$uuid;
		}
	}
	if ($uuid) {
		if (masterPage.rolePages.recordsMap[$uuid]) {
			masterPage.userPages && masterPage.userPages.selector.selectRecord(null, false);
			masterPage.rolePages.selector.selectRecord($uuid, true);
		} else {
			if (masterPage.userPages && masterPage.userPages.recordsMap[$uuid]) {
				masterPage.rolePages.selector.selectRecord(null, false);
				masterPage.userPages.selector.selectRecord($uuid, true);
			}
		}
	}
	if (!masterPage.selectedPageRecord && !ignoreLastSelected) {
		_ensureChildPageSelection(masterPage, true);
	}
}

function _deleteVignettes(masterPage, record, $uuids) {
	if ($uuids && $uuids.length) {
		var sendBag = masterPage.ensureSendBag(record);
		...
		masterPage.loadVignettesInChildPage(record, record.dataset.vignettes);
		masterPage.notifyChangeToServer(sendBag, record, true);
	} else {
...
function _addBar(page) {
	page.bar = new BarSplitter();
	page.bar.preferenceKey = "ldpbar";
	page.bar.options = {
		resizeDirection: "left",
		$viewMode: "dock",
		$skin: "s-ldpm-bar"
	};
	page.bar.barSlot = document.createElement("div");
	page.bar.barSlot.className = "s-ldpm-bar-slot";
	page.bar.barBody = document.createElement("div");
	page.bar.barBody.className = "s-ldpm-bar-body";
	if (page.$facet != "$landing_edit") {
		page.domItem.insertBefore(page.bar.barSlot, page.domItem.firstChild);
	}
	page.bar.load(page);

	page.loadNewItem(page.bar.barBody, {
		$isTitleHidden: true,
		$bind: "rolePages"
	});

	if (syra_site.userProfile.hasDesignRight()) {
		var field = page.loadNewItem(page.bar.barBody, {
...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
	onExecuteMenuResponse: function(menuItem, responseData) {
		if (menuItem.$sourceBind == "$delete") {
			...
			if (!this.selectedPageRecord) {
				_ensureChildPageSelection(this);
			}
		}
			...
			syra_diagnose.showBox({
				...
				callback: function(response) {
					if (response && response.$clientId === "yes") {
						...
						_deleteVignettes(self, childPage.masterRecord, uiids);
					}
				}
			});
		}
	...
	saveLocation: function(record, vignetteField) {
	...
	onDeleteVignetteClick: function(vignetteField) {
		...
		syra_diagnose.showBox({
			...
			callback: function(response) {
				if (response && response.$clientId === "yes") {
					_deleteVignettes(self, vignetteField.page.masterRecord, [vignetteField.$field.$vignetteId]);
				}
			}
		});
	},
				...
				case "$edit":
					syra_site.dialogManager.openPage(self, {
						...
						onServerSaved: function(saveItem, dialog) {
							...
							dialog.close(true);
							self.userPages.selector.selectRecord($uuid, true);
