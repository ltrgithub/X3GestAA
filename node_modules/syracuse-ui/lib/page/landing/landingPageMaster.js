        ...
        self.openDialog({
            ...
            onValidate: function(searchPage){
                if (searchPage.selectedVignettes) {
                    self.requestSelectVignettes(Object.keys(searchPage.selectedVignettes), searchPage.selectedEndpoint);
                }
            }
        });
    },
    requestSelectVignettes: function(uuids, selectedEndpoint){
        /*selectedEndpoint = {
         "$description": "",
         "$title": "",
         "$uuid": "6d0cf271-cdf0-45d1-b8e9-f37e292c63fa",
         "$value": "Sprint1",
         "$key": "6d0cf271-cdf0-45d1-b8e9-f37e292c63fa"
         };
         uuids = ["730dfed7-ba6a-4ea6-a77c-7b0142b76728", "c4d4ebc9-cc36-4858-b813-cbf3868f0847"];
         debugger;*/
        var sendBag = this.ensureSendBag(this.selectedPageRecord);
        sendBag.vignettes = sendBag.vignettes || [];
        if (sendBag.vignettes.length == 0 && this.selectedPageRecord.dataset.vignettes) {
            sendBag.vignettes = this.ensureDeltaManager().cleanServerIndex(helpers.object.clone(this.selectedPageRecord.dataset.vignettes, true));
        }
        ...
        //this.notifyDataChange(this.selectedPageRecord.list, sendBag.dataRecords);
        this.notifyChangeToServer(sendBag, this.selectedPageRecord);
    },
    selectChildLanding: function(record, selected){
        if (selected) {
            ...
            if (record.dataset.vignettes) {
                for (var ii = 0, jj = record.dataset.vignettes.length; ii < jj; ii++) {
                    ...
                    if (source.$links && source.$links.$location) {
                        ...
                        $diagnoses.push({
                            $message: this.localize.landingPageVignetteNoLocation.replace("{0}", source.bind),
                            $severity: "warning"
                        })
                    }
                }
            ...
            if (!this.childPageStore[record.$uuid]) {
                var childPage = this.childPageStore[record.$uuid] = document.site.loadNewPage({
                    landingPageMaster: this,
