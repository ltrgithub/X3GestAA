...
exports.Builder = helpers.defineClass(Builder, null, {
	...
	record_draw: function(record) {
		...
		record.ldp_applyChange = function(delta) {
			...
			if (!this.vignettes) {
				...
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					!$vignettes[$binds[ii]] && this.ldp_removeVignette($binds[ii]);
				}
		...
		record.ldp_select = function(selected) {
			syra_site.dom.toggleClass(this.domItem, "s-list-record-selected", selected);
			if (this.vignettes) {
				if (selected) {
					...
					this.page.$item = helpers.object.clone($article, true);
					this.page.preferences.tabs = this.page.preferences.tabs || {};
					this.page.$item.$layout.$tabIndex = this.page.preferences.tabs[this.$uuid] || 0;

					var $binds = Object.keys(this.$vignettes);
					this.page.reloadingFields = {};
					for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
						var $bind = $binds[ii];
						var vignette = this.vignettes[$bind];
						if (vignette) {
							this.page.reloadingFields[$bind] = vignette;
							...
							this.page.addResizable(vignette);
						}
						this.page.$prototype.$properties[$bind] = this.$vignettes[$bind];
					}
					syra_site.localizer.applyPageLocalization(this.page);
					this.ldp_displayed = true;
					this.page.renderLayoutContent();
					delete this.page.reloadingFields;
					for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
						var $bind = $binds[ii];
						var vignette = this.page.boundFields[$bind] && this.page.boundFields[$bind][0];
						if (vignette) {
							delete vignette.onReloading;
							this.vignettes[$bind] = vignette;
						}
					}
					this.isDesignModeEnabled = this.ldp_getSaveLink() != null;
					if (this.isDesignModeEnabled) {
						syra_site.switchItemDesigner(this.page, true);
					}
					this.page.layoutContent.ensureTabsSlot().appendChild(this.page.addTabBtn.link);
					syra_menus.button.hide(this.page.addVignetteBtn, !this.ldp_getAddLink());
					syra_menus.button.hide(this.page.addTabBtn, !this.isDesignModeEnabled);
					syra_menus.button.hide(this.page.designBtn, !this.isDesignModeEnabled);
					syra_menus.button.hide(this.page.addVignetteBtn, !(this.isDesignModeEnabled && this.ldp_getAddLink()));

					this.page.preferences.$selected = this.dataset.$uuid;
					this.page.isPageLoaded && this.page.savePreferences();
					this.page.setTitle(this.dataset.title);
					if (this.page.isPageLoaded) {
						syra_site.ensureArticleVisibility(this.page);
						this.scrollToRecord();
					}
				} else {
					if (this.ldp_displayed) {
						var $binds = Object.keys(this.vignettes);
						var fields = [];
						for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
							var vignette = this.vignettes[$binds[ii]];
							if (vignette) {
								vignette.displayed = false;
								this.page.removeResizable(vignette);
								fields.push(vignette);
							}
						}
						delete this.page.$prototype.$localization;
						delete this.page.$prototype.$allLocalization;
						syra_site.layoutUpdater.extractItems(this.page.layoutContent, fields);
						syra_site.layoutUpdater.clearContent(this.page.layoutContent);
						this.ldp_displayed = false;
					}
				}
			}
		};
	...
	record_onSelect: function(record, selected) {
		if (selected) {
			if (record.page.ldpRecord == record) {
				return;
			}
			record.page.ldpRecord = record;
			record.ldp_applyChange();
		}
		record.ldp_select(selected);
		selected && record.page.designer && record.page.designer.endArticleUpdate(null, null, false);
	},
...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
	...
	onAfterVignetteAdd: function(field) {
		if (_mastePage.isDesignModeEnabled) {
