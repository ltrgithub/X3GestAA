	...
	ensureChildPageSelection: function(ignoreLastSelected) {
		...
		if ($uuid) {
			if (this.rolePages.recordsMap[$uuid]) {
				this.userPages && this.userPages.selector.selectRecord(null, false);
				this.rolePages.selector.selectRecord($uuid, true);
			} else {
				if (this.userPages && this.userPages.recordsMap[$uuid]) {
					this.rolePages.selector.selectRecord(null, false);
					this.userPages.selector.selectRecord($uuid, true);
				}
			}
		}
					...
					syra_site.dialogManager.openPage(self, {
						...
						onServerSaved: function(saveItem, dialog) {
							...
							if (menuItem.$sourceBind == "$create") {
								...
								self.userPages.selector.selectRecord($uuid, false);
								if (record.childPage) {
									record.removeItem(record.childPage, true);
									delete record.childPage;
								}
								self.userPages.selector.selectRecord($uuid, true);
								record.childPage && record.childPage.ensurePageVisibility();
							}
							return false;
						},
