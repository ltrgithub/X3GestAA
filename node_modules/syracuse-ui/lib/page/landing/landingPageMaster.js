...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
	...
	resizeList: function(resize) {
		if (this.selectedPageRecord) {
			this.selectedPageRecord.resizeRecord(resize);
		}
	},
	saveLandingDesign: function(childPage) {
		...
		var sendBag = {
			$etag: 1,
			$url: document.site.urlMaker.formatMenuUrl(childPage.masterRecord, childPage.masterRecord.getSaveDesignLink())
		};
		sendBag.content = document.site.layoutUpdater.cleanBeforeSave(helpers.object.clone(childPage.$item, true));
	...
	onDeleteVignetteClick: function(vignetteField) {
		...
		document.site.showMessage({
			...
			$message: self.localize.ldpConfirmDeleteVignette.replace("{0}", vignetteField.getTitle()),
			$type: "question",
	...
	addVignette: function(childPage) {
		var self = this;
		var $url = document.site.urlMaker.formatMenuUrl(childPage.masterRecord, childPage.getAddVignetteDefinition(), childPage.masterRecord);
	...
	loadBox: function(initData, $initDiagnoses) {
		...
		this.preferences = document.site.getPreferences("landingpage", {});
