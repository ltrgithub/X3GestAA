...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
	...
	saveLandingDesign: function(childPage) {
		...
		var sendBag = {
			$etag: 1,
			$url: syra_url.formatMenuUrl(childPage.masterRecord, this.getSaveDesignLink(childPage.masterRecord))
		};
	...
	addVignette: function(childPage) {
		var self = this;
		var $url = syra_url.formatMenuUrl(childPage.masterRecord, childPage.getAddVignetteDefinition(), childPage.masterRecord);
		syra_site.dialogManager.openPage(self, {
			...
			onValidate: function(searchPage) {
				...
				if (sels) {
					var sendBag = syra_workingCopy.getSendBag(self.selectedPageRecord);
	...
	_deleteVignettes: function(record, $uuids) {
		if ($uuids && $uuids.length) {
			var sendBag = syra_workingCopy.getSendBag(record);
			sendBag.vignettes = sendBag.vignettes || [];
	...
	loadBox: function(initData) {
		...
		this.bar.slot.className = "s-ldpm-bar-slot";
		this.bar.body = document.createElement("div");
		this.bar.body.className = "s-ldpm-bar-body";
		...
		if (syra_site.userProfile.hasDesignRight()) {
			var field = this.loadNewItem(this.bar.body, {
