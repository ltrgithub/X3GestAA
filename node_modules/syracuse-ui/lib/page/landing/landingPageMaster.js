...
exports.Builder = helpers.defineClass(Builder, null, {
	...
	record_draw: function(record) {
		...
		record.ldp_onVignetteAdded = function($bind, field) {
			...
			field.useCurrentEndpoint = this.dataset.useCurrentEndpoint;
		...
		record.ldp_loadContent = function() {
			...
			this.isDesignModeEnabled && syra_site.switchItemDesigner(this.page, true);
			...
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				...
				if (vignette) {
					...
					this.ldp_onVignetteAdded($bind, vignette);
				}
			}
		...
		record.ldp_applyDeltaChange = function(delta) {
			...
			if (delta) {
				...
				for (var ii = 0, jj = delta.length; ii < jj; ii++) {
					...
					if (source.$links && source.$links.$location) {
						var preferences = this.ldp_getVignette(source.bind);
			...
			if (!this.vignettes) {
				...
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					...
					if (vignette) {
						if (!$vignettes[$bind]) {
							...
							if (vignette.useCurrentEndpoint != this.dataset.useCurrentEndpoint) {
								this.page.removeItem(vignette, false, true);
								delete this.vignettes[$bind];
							}
						}
					}
				...
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					...
					if (!$field) {
						var tab = this.page.layoutContent.getOpenedTab() || this.page.createNewTab();
						this.page.$prototype.$properties[$bind] = $vignette;
						this.ldp_onVignetteAdded($bind, tab.layoutContent.createChildItem({
							$bind: $bind
						}));
					}
				}
				...
				case "$edit":
					syra_site.dialogManager.openPage(self, {
						...
						onServerSaved: function(saveItem, dialog) {
							...
							self.userPages.selector.selectRecord(record, false); //important to clean content
