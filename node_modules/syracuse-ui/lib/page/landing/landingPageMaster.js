...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
	...
	saveDesignUpdate: function(childPage) {
		...
		//return;
		var record = childPage.masterContext.record;
		var $save = record.$prototype.$properties.stdLayout.$item.$links.$save;
		var sendBag = {
			$etag: 1,
			$url: record.formatMenuUrl($save),
			content: $article
		};
		...
		document.site.showMessage({
			...
			callback: function(response) {
				if (response && response.$clientId === "yes") {
					self.deleteVignettes(vignetteField.page.masterContext.record, [vignetteField.$field.$vignetteId]);
				}
			}
		});
	},
