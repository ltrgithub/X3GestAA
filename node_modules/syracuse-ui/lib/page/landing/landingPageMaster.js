...
exports.LandingPageMaster = helpers.defineClass(LandingPageMaster, DesktopPage, {
    ...
    onExecuteMenuResponse: function(menuItem, responseData){
        if (menuItem.$bind == "$delete") {
            this.removeChildLanding(menuItem.articleParent);
    ...
    removeChildLanding: function(record){
        if (this.selectedPageRecord == record) {
            this.selectChildLanding(record, false);
        }
        if (this.childPageStore[record.$uuid]) {
            this.removeItem(this.childPageStore[record.$uuid], true);
            delete this.childPageStore[record.$uuid];
        }
    ...
    selectChildLanding: function(record, selected){
        if (selected) {
            ...
            var childPage = this.childPageStore[record.$uuid];
            if (!childPage) {
                childPage = this.childPageStore[record.$uuid] = document.site.loadNewPage({
                ...
                this.childSlot.appendChild(childPage.domItem);
            }
            childPage.onWindowResize();
        }
        else {
            if (record) {
