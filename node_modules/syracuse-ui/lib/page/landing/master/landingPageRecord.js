	...
	loadVignettesInChildPage: function(vignettes) {
		...
		if (vignettes) {
			for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
				...
				if (source.$links && source.$links.$location) {
					...
					$diagnoses.push({
						$message: this.page.localize.landingPageVignetteNoLocation.replace("{0}", source.bind),
						$severity: "warning"
					});
				}
			}
		...
		if (!this.childPage) {
			this.childPage = document.site.loadNewPage({
				landingPageMaster: this.page,
				$pageCategoryClass: LandingPageChild,
				layoutSlot: this.page.childSlot,
				$category: "dashboard",
				$urlParts: this.page.$urlParts,
				$representation: {
					$prototype: {
						$isRolePage: this.list.$item.$bind == "rolePages",
			...
			this.childPage.setMasterContext(this);
		} else {
