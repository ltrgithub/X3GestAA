...
exports.LandingPage = helpers.defineClass(LandingPage, DesktopPage, {
	onExecuteMenuResponse: function($bind, record, responseData) {
		if ($bind == "$delete") {
			...
			var delta = {};
			var index = syra_dom.getIndex(record.domItem);
	...
	saveLandingPreferences: function() {
		syra_globalPreferences.save("landingpage", this.$globalPreferences);
	},
	loadBox: function(initData) {
		...
		this.$globalPreferences = syra_globalPreferences.get("landingpage") || {};
		this.$globalPreferences.tabs = this.$globalPreferences.tabs || {};
		...
		this.addTabBtn = syra_menus.button.add({
			...
			btnclick: function() {
				...
				this.parent.addVignetteBtn.btnclick();
	...
	_ensureRecordSelection: function(ignoreLastSelected) {
		var $uuid = (!ignoreLastSelected) ? (this.$globalPreferences.$selected || this.dataset.$selected) : undefined;
		if (!$uuid) {
	...
	section_onAfterOpentTab: function(section) {
		if (this.ldpRecord) {
			...
			this.$globalPreferences.tabs[this.ldpRecord.$uuid] = this.layoutContent.items.indexOf(section);
		}
	},
	...
	_addVignette: function() {
		...
		syra_dlg.openPage(self, {
			...
			onValidate: function(searchPage) {
				...
				if (sels) {
					var sendBag = syra_workingCopy.getSendBag(self.ldpRecord);
					...
					self.notifyChangeToServer(sendBag, self.ldpRecord, true);
				}
			}
		});
	},
