...
exports.LandingPage = helpers.defineClass(LandingPage, DesktopPage, {
		...
		if ($bind == "$delete") {
			...
			setTimeout(function() {
				if (!self.ldpRecord) {
					...
					if (index >= 0) {
						record = syra_store.findItem(list._core.childNodes[index]);
						if (record) {
	...
	notifyChangeToServer: function(sendBag, article, onUpdateContent,success) {
		var self = this;
					...
					try {
						if (!article.disposed) {
							if (article.page != article) {
								syra_delta.applyObjectDelta(article.page, article.dataset, newData);
							}
			...
			syra_diagnose.box.show({
				...
				callback: function(response) {
					if (response && response.$clientId === "yes") {
						...
						self.ldpRecord.ldp_deleteVignettes(uiids,function(){
							self.saveLayoutContent();
						});
					}
				}
			});
	...
	section_onAfterOpentTab: function(section) {
		if (this.ldpRecord) {
			...
			this.preferences.tabs[this.ldpRecord.$uuid] = this.layoutContent.items.indexOf(section);
		}
	},
