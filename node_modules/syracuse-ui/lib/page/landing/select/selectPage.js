...
var _browse = require('syracuse-ui/lib/page/landing/select/browse');
...
function _addStep1(page) {
	...
	var ldpPage = syra_site.landingPageMaster;
	var isEndpointHidden = ldpPage.ldpRecord && ldpPage.ldpRecord.useCurrentEndpoint;
	syra_site.dom.hide(box.block, isEndpointHidden);
...
function _addStep2(page) {
	...
	slot.className = "s-sv-wiz-results-bar";
	page.searchVignetteBlock.body.appendChild(slot);
	...
	slot.className = "s-sv-wiz-results-data";
	page.searchVignetteBlock.body.appendChild(slot);
	page.searchResultField = page.loadNewItem(slot, {
		$bind: "$searchResult",
		$isEditMode: false,
		...
		renderRecordContent: function(record) {
			...
			title.innerHTML = syra_site.dom.formatHTMLText(record.dataset.title);
			...
			if (record.dataset.module) {
				...
				div.innerHTML = syra_site.dom.formatHTMLText(record.dataset.module);
			...
			if (record.dataset.categories) {
				...
				for (var ii = 0, jj = record.dataset.categories.length; ii < jj; ii++) {
					...
					div.innerHTML = syra_site.dom.formatHTMLText(record.dataset.categories[ii]);
					record.body.appendChild(div);
