...
function _addStep2(page) {
	...
	page.searchResultField = page.addItem(syra_dom.addDiv("s-sv-wiz-results-data", page.searchVignetteBlock.body), {
		$bind: "$searchResult",
		$isEditMode: false,
		...
		renderRecordContent: function(record) {
			var title = syra_dom.addDiv("s-sv-search-res-title", record.body);
			title.innerHTML = syra_dom.escape(record.dataset.title);

			if (record.dataset.module) {
				var div = record.searchModuleItem = syra_dom.addDiv("s-sv-search-res-mod-value", record.body);
				div.innerHTML = syra_dom.escape(record.dataset.module);
			}
			if (record.dataset.categories) {
				record.searchCategoryItems = [];
				for (var ii = 0, jj = record.dataset.categories.length; ii < jj; ii++) {
					var div = syra_dom.addDiv("s-sv-search-res-cat-value", record.body);
					div.innerHTML = syra_dom.escape(record.dataset.categories[ii]);
...
exports.SelectPage = helpers.defineClass(SelectPage, DesktopPage, {
	...
	selectVignette: function(dataVignette, selected) {
		...
		this.overPanel.hideOkButton(this.selectedVignettes.length <= 0);
		return selected;
	},
