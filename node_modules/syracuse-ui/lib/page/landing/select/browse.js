...
function _selectItem(col, btn) {
	col.selectedBtn && syra_site.dom.toggleClass(col.selectedBtn.link, "s-list-record-selected", false);
	syra_site.dom.toggleClass((col.selectedBtn = btn).link, "s-list-record-selected", true);
}
	...
	if (_categories.selectedBtn != this) {
		...
		for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
			...
			var btn = syra_menus.button.add({
				...
				title: vignette.code,
				css: "s-sv-browse-item",
				btnclick: _onVignetteClick,
				code: vignette.$key || vignette.$uuid
			});
			if (page.findSelectedVignette(vignette.$key || vignette.$uuid)) {
				syra_site.dom.toggleClass(btn.link, "s-list-record-selected", true);
			}
		}
	}
	page.endChange();
...
function _onVignetteClick(btn) {
	...
	var isSelected = this.link.className.indexOf("s-list-record-selected") < 0;
	syra_site.dom.toggleClass(this.link, "s-list-record-selected", isSelected);
	if (isSelected) {
