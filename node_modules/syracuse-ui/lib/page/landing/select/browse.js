...
function _selectItem(col, btn) {
	col.selectedBtn && syra_dom.toggleClass(col.selectedBtn.link, "s-list-record-selected", false);
	syra_dom.toggleClass((col.selectedBtn = btn).link, "s-list-record-selected", true);
}
	...
	if (_categories.selectedBtn != this) {
		...
		for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
			...
			if (page.findSelectedVignette(vignette.$key || vignette.$uuid)) {
				syra_dom.toggleClass(btn.link, "s-list-record-selected", true);
			}
		}
	}
	page.endChange();
...
function _onVignetteClick(btn) {
	...
	syra_dom.toggleClass(this.link, "s-list-record-selected", isSelected);
	if (isSelected) {
