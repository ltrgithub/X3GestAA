...
exports.VignettePage = helpers.defineClass(VignettePage, DesktopPage, {
	...
	endChange: function() {
		...
		if (this.hasProcessus()) {
			var $location = this.vignetteField.$field.$location;
	...
	onMenuClick: function(menuItem) {
		if (this.hasProcessus() && sessionStorage) {
			var root = menuItem.boxParent && menuItem.boxParent.menuGroupRoot;
			if (root && root.boxParent == this.pageMenusBox) {
				...
				if (masterRecord && masterRecord.getVignettePreferences) {
					...
					var $location = this.vignetteField.$field.$location;
					$location.$bind = menuItem.$item.$bind;
					$location.$title = menuItem.getTitle();
					$location.$url = menuItem.$url;
					$location.$description = menuItem.$description;
					preferences.$location = helpers.object.clone($location, true);
	...
	registerChildArticle: function() {
		if (this.vignetteField && this.vignetteField.page) {
			this.vignetteField.page.childArticles[this.id] = this;
		} else {
			DesktopPage.prototype.registerChildArticle.call(this);
		}
	},
	unregisterChildArticle: function() {
		if (this.vignetteField && this.vignetteField.page) {
			delete this.vignetteField.page.childArticles[this.id];
		} else {
			DesktopPage.prototype.registerChildArticle.call(this);
		}
	},
