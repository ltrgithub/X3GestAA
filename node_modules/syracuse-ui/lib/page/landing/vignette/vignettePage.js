	...
	_ensureMenus: function() {
		...
		if (this.$prototype && this.$prototype.$properties && this.$prototype.$properties.process) {
			...
			if (this.vignetteField) {
				if (!this.menuBarSlot) {
					if (this.vignetteField.btns && this.vignetteField.btns.items.menus) {
						syra_site.dom.empty(this.vignetteField.btns.items.menus);
					}
				...
				this.page.loadNewItem(this.vignetteField.btns.items.menus, {
					$isMenusBag: true,
					$category: "menus",
					$noText: true,
					$title: syra_local.dskpActions,
					$isTitlePicker: true,
