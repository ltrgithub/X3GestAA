	...
	_loadContentPage: function() {
		...
		if (!self._isContentPageRequested && self.articleParent.isChildItemDisplay(self)) {
			...
			if (!this.$url && this.$field && this.$field.$prototype) {
				...
				if (self.$field.$location && self.$field.$location.$target == "embedded") {
					var matches = self.$field.$location.$url.match(_templatePattern);
					for (var ii = 0, jj = matches.length; ii < jj; ii++) {
						var match = matches[ii];
						matches[ii] = match.substr(1, match.length - 2);
					}
				}
