...
exports.PageVignetteField = helpers.defineClass(PageVignetteField, VignetteField, {
	maximize: function() {
		VignetteField.prototype.maximize.call(this);
		if (this.vignette) {
			this.vignette.ensurePageVisibility();
		}
	},
	minimize: function() {
		VignetteField.prototype.minimize.call(this);
		if (this.vignette) {
			this.vignette.ensurePageVisibility();
		}
	},
	...
	renderLayoutContent: function($url, onVignetteRefresh) {
		...
		if (!self.$url && self.$field && self.$field.$prototype) {
			self.vignette = document.site.loadNewPage({
			...
			document.controller.loadRepresentation(null, self.$url || self.$field.$location.$url, function($itemPage) {
				if (self.body) {
					...
					if (!self.$url && self.$item.$vignette) {
						$itemPage.$representation.$article = self.$item.$vignette;
						delete self.$item.$vignette.$menus;
					}
