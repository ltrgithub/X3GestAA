		...
		if (!this.page.isLandingPage) {
			if (enable) {
				if (!this.btns.items.design) {
					var btn = this.btns.items.design = syra_menus.addIconButton(syra_local.vignetteDesign, this.$skin + "-btn", "onDesignOpenerClick", null, "design");
					btn.syraItem = this.id;
					this.btns.slot.insertBefore(btn, this.btns.slot.firstChild);
				}
			} else {
				if (this.btns.items.design) {
