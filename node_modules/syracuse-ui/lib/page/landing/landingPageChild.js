...
exports.LandingPageChild = helpers.defineClass(LandingPageChild, DesktopPage, {
	...
	dispose: function() {
		this.masterContext = this.addLink = this.refreshLink = this.designPicker = null;
	...
	onClickPicker: function(picker, event) {
		switch (picker.getAttribute("data-s-picker")) {
			case "ldp-addVignette":
				this.masterContext.page.addVignette(this);
				...
				callback: function(response) {
					if (response && response.$clientId === "yes") {
						...
						for (var ii = 0, jj = vignettes.length; ii < jj; ii++) {
							uuids.push(vignettes[ii].$field.$vignetteId);
						}
						self.masterContext.page.deleteVignettes(self.masterContext.record, uuids);
					}
				}
			});
		}
	...
	ensurePageVisibility: function() {
		...
		if (this.isPageLoaded && this.layoutContent) {
