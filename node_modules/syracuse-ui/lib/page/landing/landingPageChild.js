...
exports.LandingPageChild = helpers.defineClass(LandingPageChild, DesktopPage, {
	...
	dispose: function() {
		this.masterPage = this.masterRecord = this.addLink = this.refreshLink = this.designPicker = null;
	...
	appendHeader: function() {
		...
		if (this.isDesignModeEnabled) {
			...
			this.designPicker.title = this.localize.landingPageOpenDesigner;
	...
	onClickPicker: function(picker, event) {
		switch (picker.getAttribute("data-s-picker")) {
			case "ldp-addVignette":
				this.masterPage.addVignette(this);
			...
			case "ldp-openDesigner":
				...
				return false;
			default:
				switch (picker.syraBoxPickerId) {
					case "ldp-edit-tab":
						event.stopPropagation();
						return false;
					case "ldp-delete-tab":
						this.onDeleteTabClick(picker);
						event.stopPropagation();
						return false;
				}
				break;
	...
	onDeleteTabClick: function(picker) {
		var self = this;
		...
		if (box && box.layoutContent) {
			...
			document.site.showMessage({
				...
				$message: self.localize.landingVignetteDeleteTabConfirm.replace("{0}", box.getTitle()),
				$type: "question",
				callback: function(response) {
					if (response && response.$clientId === "yes") {
						...
						self.masterPage.deleteVignettes(self.masterRecord, uiids, box);
					}
				}
			});
		}
		...
		if (this.isDesignModeEnabled && this.isPageLoaded && this.layoutContent) {
			for (var ii = 0, jj = this.layoutContent.items.length; ii < jj; ii++) {
				...
				if (!item.$item.$isAddLandingTab) {
					item.togglePicker("ldp-edit-tab", true);
					item.togglePicker("ldp-delete-tab", true);
				}
			}
		}
	},
	...
	applyVignettesChange: function($vignettes) {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if (!$field) {
				var tab = this.layoutContent.tabOpened;
	...
	loadBox: function(initData, $initDiagnoses) {
		this.masterPage = (this.masterRecord = this.$prototype.masterRecord).page;
