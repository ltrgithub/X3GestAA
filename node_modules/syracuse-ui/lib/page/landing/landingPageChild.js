...
exports.LandingPageChild = helpers.defineClass(LandingPageChild, DesktopPage, {
	...
	getDefaultTabTitle: function($id) {
		...
		var $key = "@" + $id;
		this.saveArticleLocalize($key, defaultTitle + " " + count);
		return "{" + $key + "}";
	},
	saveArticleLocalize: function($key, $value) {
		this.$localization = this.$localization || {};
		var code = document.site.userProfile.getSelectedLocale().code;
		(this.$localization[code] = this.$localization[code] || {})[$key] = $value;
		(this.$prototype.$localization = this.$prototype.$localization || {})[$key] = $value;
	},
	...
	_defineNewTab: function() {
		return {
			$category: "section",
			$title: this.getDefaultTabTitle(helpers.uuid.generate()),
		};
	},
