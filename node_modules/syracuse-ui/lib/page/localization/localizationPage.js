...
exports.LocalizationPage = helpers.defineClass(LocalizationPage, DesktopPage, {
		...
		if (field.$item.$bind == '$localeValue') {
			...
			if (record.dataset.$localeChoice == 0) {
				...
				/*
                 ...
                 var $locale = this._getLocale($enum, record.dataset.$localeChoice);
                 if (!$locale) {
                 this.showErrMsg(document.site.localize.lPageUnexpected);
                 }*/
				...
				this.applyChange({
					$translations: [{
						...
						$links: {
							$clearLocale: {
								$title: document.site.localize.lPageClear
							}
						},
						$properties: {
							$localeChoice: {
				...
				// TODO == add new row if necessary ==
				/*
                 if ((record.list.records.length - 1) == record.$recordIndex) {
                 var newData = {
                 $translations: [{
	...
	onMenuClick: function(menuItem) {
		if (menuItem.$bind == '$save') {
			this._onSave(menuItem);
		}
		if (menuItem.$bind == '$clearLocale') {
			this._onRemoveLocale(menuItem.articleParent);
		}
	},
	_onSave: function(menuItem) {
