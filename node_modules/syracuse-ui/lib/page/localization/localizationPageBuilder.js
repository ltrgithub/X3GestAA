"use strict";
var helpers = require('syracuse-core/lib/helpers');
var LocalizationPage = require('syracuse-ui/lib/page/localization/localizationPage').LocalizationPage;

function LocalizationPageBuilder() {}

exports.LocalizationPageBuilder = helpers.defineClass(LocalizationPageBuilder, null, {
	loadLocalizationPage: function(menu) {
		var self = this;
		if (menu.$url) {
			document.controller.sendRequest(null, {
				method: "GET",
				$location: {
					$url: menu.$url
				}
			}, function(data, response, $url) {
				self._createPage(data, menu);
			}, function(error, httpquery) {
				var $diagnoses;
				if (error.data.indexOf("$diagnoses") != -1) {
					$diagnoses = JSON.parse(error.data).$diagnoses;
				} else {
					$diagnoses = [{
						$severity: "error",
						$message: error.data
					}];
				}
				document.site.showDiagnoses({
					$diagnoses: $diagnoses
				});
			});
		} else {
			self._createPage(data, menu);
		}
	},
	_createPage: function(data, openerMenu) {
		var self = this;
		self.newData = {
			$translations: []
		};
		var $enum = [];
		self.enumLocaleData = {
			enumLocale: {},
			enumLocaleSet: {},
			enumLocaleUnset: {}
		};

		self.isReadOnly = data.$links && data.$links.$save ? false : true;

		// === building locale enum data & newData object ===
		// is payload ok ?
		if (!self._isPayloadOk(data)) {
			return false;
		}

		// payload ok
		self.$values = data.$values;
		for (var ii = 0; ii < self.$values.length; ii++) {

			if (self.$values[ii].$value) {
				// add record data
				self.newData.$translations.push({
					$uuid: helpers.uuid.generate(),
					$index: ii,
					$localeChoice: self.$values[ii].$locale,
					$localeValue: self.$values[ii].$value,
					$properties: {
						$localeChoice: {
							$isReadOnly: true
						}
					}
				});

				self.enumLocaleData.enumLocaleSet[self.$values[ii].$locale] = {
					$title: self.$values[ii].$title
				};

			} else {
				self.enumLocaleData.enumLocaleUnset[self.$values[ii].$locale] = {
					$title: self.$values[ii].$title
				};
			}

			$enum.push({
				$value: self.$values[ii].$locale,
				$title: self.$values[ii].$title
			});

			self.enumLocaleData.enumLocale[self.$values[ii].$locale] = {
				$index: ii,
				$title: self.$values[ii].$title
			};
		}

		// add new empty row only if enumLocaleUnset unempty and if page is not details facet
		if (Object.keys(self.enumLocaleData.enumLocaleUnset).length > 0 && !self.isReadOnly) {
			self.newData.$translations.push({
				$uuid: helpers.uuid.generate(),
				$index: self.newData.$translations.length,
				$localeChoice: "",
				$localeValue: "",
				$links: {
					$delete: {
						$isHidden: true
					}
				},
				$properties: {
					$localeValue: {
						$isEditMode: false
					}
				}
			});
		}

		// === building localization page ===
		var options = {
			$dialogMode: "modal",
			$itemPage: {
				$isEditMode: true,
				$urlParts: {
					$facet: "$localize"
				},
				$pageCategoryClass: LocalizationPage,
				$representation: {
					$prototype: {
						$properties: {
							$translations: {
								$type: "application/x-array",
								$capabilities: self.isReadOnly ? "" : "delete",
								$item: {
									$properties: {
										$localeChoice: {
											$type: "application/x-choice",
											$title: document.site.localize.lPageLocale,
											$value: {
												$type: "application/x-integer",
												$enum: $enum
											}
										},
										$localeValue: {
											$type: "application/x-string",
											$title: document.site.localize.lPageField,
											$isReadOnly: self.isReadOnly ? true : false
										}
									}
								}
							}
						},
						$links: data.$links
					},
					$article: {
						$category: "page",
						$title: document.site.localize.lPageTitle,
						$layout: {
							$items: [{
								$format: "cards",
								$skin: "s-localization-list",
								$isTitleHidden: true,
								$isPagerHidden: true,
								$isQuickDesignerDisabled: true,
								$bind: "$translations",
								$skinCard: "s-localization-list-cards",
								$layout: {
									$items: [{
										$layoutType: "row",
										"$isFixedWidth": true,
										"$widths": "250,*",
										$items: [{
											$bind: "$localeChoice",
											$format: "$combo",
											$isTitleHidden: true,
											$title: document.site.localize.lPageLocaleTitle,
											$isTitlePlaceHolder: true,
											$icon: {
												$mode: "iconText",
												$path: "flags_iso/"
											}
										}, {
											$bind: "$localeValue",
											$isTitleHidden: true
										}]
									}]
								}
							}]
						}
					}
				},
				onAfterLoadPage: function(page) {
					page.openerMenu = openerMenu;
					page.enumLocaleData = self.enumLocaleData; //enumLocale, enumLocaleSet, enumLocaleUnset
					page.$values = self.$values;
					page.valuesCount = page.$values.length;
					page.applyChange(self.newData);
					self.dispose();
				}
			}
		};
		if (!self.isReadOnly) {
			options.onValidate = function(page) {
				page.validateFields();
				page.onSave();
			};
			options.onClose = function(validated, dispose) {
				if (validated) {
					return true;
				}
				return false;
			};
		}
		openerMenu.page.openDialog(options);
	},
	_isPayloadOk: function(data, openerMenu) {
		if (!data.$values) {
			this.showErrMsg(document.site.localize.lPageNoValues, openerMenu);
			return false;
		}
		return true;
	},
	showErrMsg: function(msg, item) {
		document.site.showDiagnoses({
			$diagnoses: [{
				$message: msg
			}]
		}, item);
	},
	dispose: function() {
		this.newData = this.$values = this.enumLocaleData = null;
	}
});