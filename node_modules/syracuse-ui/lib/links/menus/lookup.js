			...
			syra_pageBuilder.load({
				...
				success: function success(page) {
					...
					self.page.fetch(null, function(data, response, requestUrl) {
						...
						var link;
						for (var ii = 0, jj = Math.min(records.length, 15); ii < jj; ii++) {
						...
						self.detailsLink = link = syra_dom.anchor("s-popup-primary-top-sep s-popup-primary-btn-primary", slot);
						link.textContent = syra_local.lookup_details + (data.$totalResults ? (" (" + data.$totalResults + ")") : "");
