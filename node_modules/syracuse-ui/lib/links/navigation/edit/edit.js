...
function _addRecordActions($actions, record, level) {
	var actions = syra_dom.div("s-list-nav-btn-group", record.slot);
	if ($actions.$edit) {
		syra_button.add({
			parent: record,
			slot: actions,
...
exports.module = {
	create: function(record) {
		syra_dom.div("s-nav-record-reorder", record.slot).syraSortableHandle = "modules";
	...
	activate: function(on, module) {
		if (on) {
			if (!module.body) {
				module.body = syra_dom.ul("s-nav-record-body");
				module.addChildren();
			}
...
exports.subModule = {
	create: function(record) {
		syra_dom.div("s-nav-record-reorder", record.slot).syraSortableHandle = "submodules";
		record.title = syra_dom.span(record.$skin + "-title", record.slot);
		record.title.textContent = record.dataset.title;
