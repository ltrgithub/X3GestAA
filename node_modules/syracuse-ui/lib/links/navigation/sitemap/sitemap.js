...
exports.page = {
	draw: function(page) {
		...
		page.$skins = {
			module: "s-nav-record",
			submodule: "s-nav-map-submodule",
			nosubmodules: "s-nav-map-no-submodules",
			block: "s-nav-map-block",
			menu: "s-nav-map-link",
			nomenu: "s-nav-map-no-menu"
		};
		page.layoutSlot = syra_dom.div();
		page.domItem = syra_dom.div("", page.layoutSlot);
		page.body = syra_dom.div("", page.domItem);
		page.mapBody = syra_dom.div("s-nav-map-body", page.body);
		page.modulesBarSlot = syra_dom.div("s-nav-map-modules-slot", page.mapBody);
		page.modulesBar = syra_dom.ul("s-nav-map-modules", page.modulesBarSlot);
		page.modulesContentSlot = syra_dom.div("s-nav-map-modules-content-slot", page.mapBody);
		page.modulesScrollBar = new Scrollbar({
			container: page.modulesBarSlot,
			scrollview: page.modulesBar
		});
		page.modulesContent = syra_dom.ul("s-nav-map-modules-content", page.modulesContentSlot);
		...
		syra_over.openModal(syra_site, {
			...
			resize: function(dlg) {
				if (this.page.isMapLoaded) {
					this.page.modulesBarSlot.style.height = syra_site.size.body.height + "px";
					this.page.modulesContentSlot.style.height = syra_site.size.body.height + "px";

					if (syra_site.$device == "desktop") {
						this.page.body.style.width = this.page.body.clientWidth + 20 + "px"; //Cater for long headings - specific to sitemap for desktop
					}

					var maxContentWidth = this.page.body.clientWidth - this.page.modulesBarSlot.clientWidth - 2;

					if (syra_site.$device == "tablet") {
						var modalHeight = (syra_site.size.body.height - 60);
...
exports.module = {
	...
	_showSubModules: function(module) {
		...
		module.content = syra_dom.li("s-nav-map-module-content");
		module.titleBtn = syra_button.add({
			parent: module,
			slot: module.content,
			text: module.dataset.title,
			title: syra_local.nvpCollapseSubModule,
			css: "s-nav-map-module-content-title",
			fontIcon: "node_expanded",
			click: _clickExpand
		});

		module.body = syra_dom.ul("s-nav-map-submodules", module.content);
		module.addChildren();
