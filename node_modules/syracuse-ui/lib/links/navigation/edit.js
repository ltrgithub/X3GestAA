...
function Column(level, skin) {
	...
	this.resize = function(height) {
		if (this.listView) {
			this.listView.scrollSlot.style.height = height + "px";
			...
			setDraggingText: function(node) {
				...
				switch (moved.level) {
					...
					case "items":
						...
						items = moved.articleParent.dataset.items;
						break;
				}
...
function ItemBuilder() {
	...
	this.createItem = function(item, variant) {
		if (variant == "menuBlock") {
			...
			item.titleField = syra_dom.text("s-sitemap-item-btn-block", item.dataset.title, item.slot);
