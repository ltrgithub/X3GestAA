...
function DetailPanel(navParent) {
	...
	this._body = syra_dom.div("s-nav-mega-detail-body", this._slot);
	this._scrollview = syra_dom.div("s-nav-mega-detail-scrollview", this._body);
	this.cols = [];
	this.addColumn();
}

DetailPanel.prototype.addColumn = function() {
	var col = syra_dom.ul("s-nav-mega-detail-col", this._scrollview);
	this.cols.push(col);
	return col;
};
...
DetailPanel.prototype.activate = function() {
	...
	var viewType = parentMenuItem.viewType || (parentMenuItem.submodule && parentMenuItem.submodule.viewType);
	var colsMax = (viewType == "large" && parentMenuItem.page.isHighRes) ? 3 : 2;
