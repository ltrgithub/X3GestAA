	...
	load: function(page, options) {
		...
		page.resizeItem = function() {
			if (this.areModulesLoaded) {
				if (this.breadcrumb_popup && this.lastSelected) {
					this.lastSelected.carousel.adjustContainerHeight(this.breadcrumb_popup);
				}
...
exports.modules = {
	...
	getModule: function(breadcrumbs, index, modules) {
		...
		for (var ii = 1, jj = index; ii <= jj; ii++) {
			if (item) {
				item = item.getChild(breadcrumbs[ii].$uuid); //this._getItem(breadcrumbs[ii].$uuid, ));
			} else {
