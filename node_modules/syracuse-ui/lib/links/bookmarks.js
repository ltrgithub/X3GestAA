...
View.prototype.fill = function() {
	...
	if (!this.addBtn) {
		this.addBtn = syra_button.add({
			...
			css: "s-popup-primary-bottom-sep s-popup-primary-btn-primary",
			click: function() {
	...
	for (var ii = 0, jj = items.length; ii < jj; ii++) {
		...
		if (item && item.$url) {
			var btn = this.list.addButton({
				...
				dataset: item
			}, item.$hasSeparator && "s-popup-primary-bottom-sep");
...
View.prototype.setSeparator = function() {
	...
	syra_dom.toggleClass(item.slot, "s-popup-primary-bottom-sep", item.dataset.$hasSeparator);
			...
			_view.popup = syra_over.openPopup(syra_site, {
				...
				onresize: function() {
					if (_view.addBtn) {
						...
						_view.list.resize(syra_site.size.headerPopupMaxHeight - (2 * btnHeight));
					}
				},
				close: function() {
					setTimeout(function() {
