...
View.prototype.fill = function() {
	...
	if (!this.addBtn) {
		this.addBtn = syra_button.add({
			...
			css: "s-bookmarks-add s-popup-primary-btn-primary",
			click: function() {
	...
	this.slot.insertBefore(this.list.ulSlot, this.editBtn && this.editBtn.link);
	...
	if (!this.editBtn) {
		this.editBtn = syra_button.add({
			...
			css: "s-bookmarks-edit s-popup-primary-btn-primary",
			click: function() {
...
View.prototype.setSeparator = function() {
	...
	this.list.addSeparator(item.slot, item.dataset.$hasSeparator);
...
View.prototype.setScrollHeight = function() {
	if (this.list.ul) {
		...
		this.list.setScrollheight(syra_site.size.headerPopupMaxHeight - (2 * btnHeight));
	}
};
