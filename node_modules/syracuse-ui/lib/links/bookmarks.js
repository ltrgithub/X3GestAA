...
function View() {
	...
	this.slot.className = "s-over-popup";
...
View.prototype.fill = function() {
	...
	if (!this.addBtn) {
		this.addBtn = syra_button.add({
			...
			css: "s-bookmarks-add s-head-popup-btn-primary",
			click: function() {
	...
	this.ulSlot = syra_dom.div("s-head-popup-ul-slot");
	this.ul = syra_dom.ul("s-head-popup-ul", this.ulSlot);
	...
	for (var ii = 0, jj = items.length; ii < jj; ii++) {
		...
		if (item && item.$url) {
			var btn = syra_button.add({
				parent: syra_site,
				slot: syra_dom.li("s-head-popup-li", this.ul),
				text: item.title || item.description || "???",
				title: item.description,
				css: "s-head-popup-btn-default",
				click: function() {
			...
			if (btn.dataset.$hasSeparator) {
				syra_dom.toggleClass(btn.slot, "s-bookmark-separator", true);
			}
	...
	this.slot.insertBefore(this.ulSlot, this.editBtn && this.editBtn.link);
	...
	if (!this.editBtn) {
		this.editBtn = syra_button.add({
			...
			css: "s-bookmarks-edit s-head-popup-btn-primary",
			click: function() {
...
View.prototype.toggleEdit = function(isEditMode) {
	...
	if (isEditMode) {
		for (var ii = 0, jj = _view.buttons.length; ii < jj; ii++) {
			...
			var slot = syra_dom.div("s-nav-record-btns", btn.slot);
			var reorder = syra_dom.div("s-nav-record-reorder");
			syra_dom.toggleClass(btn.slot, "s-bookmark-btn-edit", true);
			syra_dom.toggleClass(btn.link, "s-bookmark-btn-edit", true);
			btn.slot.insertBefore(reorder, btn.slot.firstchild);
			btn.edit = {
				slot: slot,
				reorder: _view.sortable.setNodeHandle(reorder),
		...
		for (var ii = 0, jj = _view.buttons.length; ii < jj; ii++) {
			...
			if (btn.edit) {
				...
				delete btn.edit;
				syra_dom.toggleClass(btn.link, "s-bookmark-btn-edit", false);
				syra_dom.toggleClass(btn.slot, "s-bookmark-btn-edit", false);
			}
...
View.prototype.setSeparator = function() {
	...
	syra_dom.toggleClass(item.slot, "s-bookmark-separator", item.dataset.$hasSeparator);
...
View.prototype.setScrollHeight = function() {
	if (this.ulSlot) {
		var btnHeight = this.addBtn.link.getBoundingClientRect().height;
		this.ul.style.maxHeight = (syra_site.size.headerPopupMaxHeight - (2 * btnHeight)) + "px";
	}
};
	...
	if (!_view.popup) {
		...
		if (!success) {
			_view.popup = syra_over.openPopup(syra_site, {
				slot: _view.slot,
				picker: syra_site.btns.bookmark.link,
				position: {
					my: "left top",
					at: "left bottom",
					of: syra_site.btns.bookmark.link
				},
				close: function() {
					setTimeout(function() {
						_view.popup = null;
