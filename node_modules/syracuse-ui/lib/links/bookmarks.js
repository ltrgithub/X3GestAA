...
View.prototype.toggle = function(show) {
	var self = this;
...
View.prototype.addMenu = function() {
...
View.prototype.toggleEdit = function(isEditMode) {
	...
	if (isEditMode) {
		for (var ii = 0, jj = _view.buttons.length; ii < jj; ii++) {
			...
			btn.edit = {
				slot: slot,
				reorder: _view.sortable.setNodeHandle(syra_dom.div("s-bookmark-item-edit-reorder", slot)),
...
View.prototype.removeMenu = function() {
	...
	syra_alert.ask({
		...
		yes: function() {
			var index = _view.buttons.indexOf(item);
			_view.dataset.items.splice(_view.dataset.items.indexOf(item.dataset), 1);
			_view.commit(_view.dataset, Math.min(index, _view.buttons.length - 1));
		}
	});
};
