...
helpers.defineClass(Tracker, null, {
	addRow: function(titleText) {
		var slot = syra_dom.div("s-tracker-row");
		syra_dom.hide(slot, true);
		return {
			slot: this.domItem.appendChild(slot),
			title: syra_dom.text("s-tracker-title", titleText, slot),
			value: syra_dom.div("s-tracker-value", slot)
		};
	},
	draw: function() {
		this.domItem = syra_dom.div("s-tracker-item", _page.scrollview);

		var row = syra_dom.div("s-tracker-top");
		this.typeIcon = row.appendChild(document.createElement("div"));

		var slot = syra_dom.div("s-tracker-progress", row);
		this.progressSlide = syra_dom.div("s-tracker-progress-bar", slot);
		this.progress = syra_dom.div("s-tracker-progress-data", slot);

		this.domItem.appendChild(row);

		var cell = syra_dom.div("s-tracker-action-cell");
	...
	fillDiagnoses: function($diagnoses) {
		...
		for (var ii = (this.fusionContext ? 0 : this.diagnoses.items.length), jj = $diagnoses.length; ii < jj; ii++) {
			...
			syra_dom.toggleClass(diagnose.item, "s-record-alt", ii % 2);
...
helpers.defineClass(TrackersPage, DesktopPage, {
	load: function() {
		...
		this.layoutSlot = syra_dom.div($skin, syra_site.layoutSlot);

		DesktopPage.prototype.load.call(this);
		this.header = syra_dom.div("s-trackers-header");
		syra_dom.hide(this.header, true);
		syra_dom.empty(this.domItem);
		this.domItem.appendChild(this.header);
		var div = syra_dom.div("s-trackers-kind-slot");
		...
		this.scrollview = syra_dom.div("s-trackers-body", this.domItem);
	},
