...
helpers.defineClass(Tracker, null, {
	...
	applyChange: function(newData, success, $url) {
		//_stackExchages.push(newData);
		//_stackExchagesSource.push(helpers.object.clone(newData));
...
helpers.defineClass(TrackersPage, DesktopPage, {
	...
	loadBox: function() {
		...
		var btn = syra_menus.addTextButton(syra_local.trackers_clear_all, "s-trackers-clear", "onClear");
	...
	onClearTracker: function(event, target) {
		...
		if (tracker.isProcessing) {
			this._showMessage("clear", tracker.dataset, function() {
	...
	onClear: function(event, target, list) {
		var self = this;
		if (!list) {
			...
			if (isProcessing) {
				self._showMessage("clearList", self.dataset, function() {
					self.onClear(event, target, list);
				});
			}
		}
		if (list) {
	...
	checkKindList: function() {
		...
		for (var ii = 0, jj = _trackers.list.length; ii < jj; ii++) {
			var dataset = _trackers.list[ii].dataset;
		...
		if (!curValue) {
			curValue = $enum.length == 1 ? $enum[0].$value : syra_local.tracker_kind_default;
		}
