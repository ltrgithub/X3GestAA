...
helpers.defineClass(Tracker, null, {
	draw: function() {
		...
		this.actionLink = syra_menus.addTextButton("", "s-tracker-action", "onTrackerActionClick", null, "cancel");
		this.actionLink.style.visibility = "hidden";
		this.clearBtn = syra_menus.addIconButton(syra_local.trackers_clear, "s-tracker-clear", "onClearTracker", null, "delete");
		...
		cell.appendChild(this.actionLink);
		row.appendChild(cell);
		this.actionLink.syraTracker = this.clearBtn.syraTracker = this.dataset.$uuid;
		this.actionLink.syraItem = this.clearBtn.syraItem = _page.id;
		...
		for (var ii = 0, jj = _$binds.$fields.length; ii < jj; ii++) {
			...
			title.textContent = syra_local["trackers_" + $bind];
	...
	applyChange: function(newData, success, $url) {
		...
		if (newData.$links) {
			...
			for (var mm = 0, pp = _$binds.$links.length; mm < pp; mm++) {
				...
				if ($link && !$link.$isHidden) {
					...
					visible = true;
					syra_menus.updateButtonIcon(this.actionLink, title, this.actionLink.syraBind = $bind);
				}
			}
			this.actionLink.style.visibility = visible ? "" : "hidden";
		}
		...
		if (this.isProcessing) {
			...
			if (this.actionLink.syraBind && ["$abort", "$suspend", "$resume", "$cancel"].indexOf(this.actionLink.syraBind) >= 0) {
				this.actionLink.style.visibility = visible ? "" : "hidden";
			}
	...
	fillDiagnoses: function($diagnoses) {
		...
		for (var ii = (this.fusionContext ? 0 : this.diagnoses.items.length), jj = $diagnoses.length; ii < jj; ii++) {
			...
			var diagnose = {
				$diagnose: $diagnose,
				item: syra_menus.addFontIconText($diagnose.$message, "s-tracker-diagnose s-diag-" + $severity, $severity == "info" ? $severity : "diagnose")
			};
			if ($diagnose.$stackTrace) {
				this.diagnoses.hasStack = true;
				var stackBtn = diagnose.stackBtn = syra_menus.addIconButton(syra_local.trackers_stack_expand, "s-tracker-stack-opener", "onStackClick", null, "expand");
				stackBtn.$opened = false;
				stackBtn.syraTracker = this.dataset.$uuid;
			...
			if (!this.diagnoseHeight) {
				this.diagnoses.slot.style.maxHeight = this.diagnoseHeight = (5 * diagnose.item.getBoundingClientRect().height) + "px";
			}
...
helpers.defineClass(TrackersPage, DesktopPage, {
	onStackClick: function(event, btn) {
		...
		syra_menus.updateButtonIcon(btn, syra_local["trackers_stack_" + state], state);
		var diagnose = _trackers.map[btn.syraTracker].diagnoses.items[btn.syraDiag];
	...
	loadBox: function() {
		...
		this.clearAllBtn = syra_menus.addTextButton(syra_local.trackers_clear_all, "s-trackers-clear", "onClear");
		this.clearAllBtn.syraItem = this.id;
		this.header.appendChild(this.clearAllBtn);
	...
	checkHeader: function() {
		...
		if (isClearVisible || !this.kindField.$isHidden) {
			this.clearAllBtn.style.visibility = isClearVisible ? "" : "hidden";
			display = "";
		}
	...
	onClearTracker: function(event, target) {
		_trackers.remove(_trackers.map[target.syraTracker]);
	},
	onClear: function(event, target) {
		var list = [];
		var selectedValue = this.kindField.currentValue;
		var selectedTitle;
		for (var ii = 0, jj = this.kindField.$enum.length; ii < jj; ii++) {
			if (this.kindField.$enum[ii].$value == selectedValue) {
				selectedTitle = this.kindField.$enum[ii].$title;
				break;
			}
		}
		for (var ii = 0, jj = _trackers.list.length; ii < jj; ii++) {
	...
	onTrackerActionClick: function(event, target) {
		var self = this;
		var tracker = _trackers.map[target.syraTracker];
		var $menu = tracker.dataset.$links[target.syraBind];
		var $url = syra_site.urlMaker.formatMenuUrl(self, $menu, tracker.dataset);
		switch (target.syraBind) {
			case "$download":
				if ($menu.$verifyUrl) {
					...
					var $verifyUrl = syra_site.urlMaker.formatMenuUrl(self, $verifyMenu, tracker.dataset);
				...
				break;
			case "$cancel":
				self._showMessage(target.syraBind.slice(1), tracker.dataset, function() {
...
exports.addOpener = function(slot) {
	_opener = syra_menus.addIconButton("", "s-trackers-opener", "onTrackersOpenerClick");
	_opener.syraItem = syra_site.id;
	syra_site.onTrackersOpenerClick = function() {
		_page && _page.toggle();
	};
	_opener.style.display = "none";
