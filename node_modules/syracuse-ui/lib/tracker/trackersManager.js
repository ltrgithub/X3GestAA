...
function _onStackClick() {
	...
	syra_menus.updateButtonIcon(this.link, syra_local["trackers_stack_" + state], state);
	...
	if (this.$opened && !diagnose.stackSlot) {
		...
		this.parentNode.appendChild(diagnose.stackSlot);
	}
	syra_site.dom.display(diagnose.stackSlot, this.$opened);
}
...
helpers.defineClass(Tracker, null, {
	draw: function() {
		...
		this.actionBtn.link.style.visibility = "hidden";
		...
		this.clearBtn.link.style.visibility = "hidden";
	...
	applyChange: function(newData, success, $url) {
		...
		if (newData.$links) {
			...
			for (var mm = 0, pp = _$binds.$links.length; mm < pp; mm++) {
				...
				if ($link && !$link.$isHidden) {
					...
					visible = true;
					syra_menus.updateButtonIcon(this.actionBtn.link, title, this.actionBtn.$bindValue = $bind);
				}
			}
			this.actionBtn.link.style.visibility = visible ? "" : "hidden";
		}
		...
		if (this.isProcessing) {
			...
			this.clearBtn.link.style.visibility = "";
		}
		...
		for (var ii = (this.fusionContext ? 0 : this.diagnoses.items.length), jj = $diagnoses.length; ii < jj; ii++) {
			...
			if ($diagnose.$stackTrace) {
				...
				var btn = syra_menus.button.add({
					...
					iconOnly: true,
					fontIcon: "expand",
					btnclick: onStackClick,
					$opened: false,
					diagnoseIndex: ii,
					trackerId: this.dataset.$uuid
				});
				diagnose.item.insertBefore(btn.link, diagnose.item.firstChild);
			}
...
helpers.defineClass(TrackersPage, DesktopPage, {
	...
	checkHeader: function() {
		...
		if (isClearVisible || !this.kindField.$isHidden) {
			this.clearAllBtn.link.style.visibility = isClearVisible ? "" : "hidden";
			display = "";
		}
