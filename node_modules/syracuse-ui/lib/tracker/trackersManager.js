...
var _trackers = {
	add: function(tracker) {
		...
		syra_button.hide(_opener, false);
		_page.toggle(true);
	},
		...
		case "$download":
			if ($menu.$verifyUrl) {
				...
				syra_ajax.get({
					...
					error: function(error, httpquery) {
...
function _onStackClick() {
	...
	if (this.$opened && !diagnose.stackSlot) {
		diagnose.stackSlot = syra_dom.addDiv("s-tracker-stack");
		diagnose.stackSlot.innerHTML = syra_dom.textToHTML(diagnose.$diagnose.$stackTrace);
...
helpers.defineClass(Tracker, null, {
	...
	sendRequest: function(reqOpt, cbck, onRelease) {
		...
		reqOpt.error = function(error, httpquery) {
	...
	onRequestError: function(error, ignoreOpComplete) {
		...
		var dlg = syra_alert.getDialog();
		dlg && dlg.box.close("no", _page.msgBoxId);
...
helpers.defineClass(TrackersPage, DesktopPage, {
	...
	_showMessage: function($bind, dataset, cbck) {
		this.msgBoxId = syra_alert.dialog({
		...
		if (self.domItem) {
			if (show) {
				...
				if (!_popup) {
					...
					_popup = syra_dlg.openPopup(self, {
						content: self,
						$isAutoClose: false,
	...
	resizeItem: function() {
		if (_popup) {
			...
			height -= (syra_site.header && syra_site.headerTop.clientHeight) || 0;
