...
exports.Calendar = helpers.defineClass(Calendar, null, {
            ...
            if (bind) {
                ...
                self._$$quick.bind("click", function(){
                    if (!self.$$quickList) {
                        ...
                        Object.keys(self._localize.dp_d_qm_interval).forEach(function(link){
                            self.$$quickList.append("<div class='s-calendar-quick'><a class='s-calendar-quick-link'>" + self._localize.dp_d_qm_begin + "</a>/<a class='s-calendar-quick-link'>" + self._localize.dp_d_qm_end + "</a>" + self._localize.dp_d_qm_interval[link] + "</div>");
                        });
    ...
    _appendHead: function(){
        ...
        $(link).addClass("s-calendar-prev-year")
        ...
        link.className = "s-calendar-prev";
        $(link).addClass("s-calendar-prev-month")
        cell.appendChild(link);
        ...
        $(link).addClass("s-calendar-next-month")
        cell.appendChild(link);
        ...
        [1, 2, 3, 4, 5, 6, 0].forEach(function(day){
            var cell = document.createElement("th");
            cell.className = "s-calendar-week-day";
            cell.setAttribute("title", _dateApi.dayName(day));
            $(cell).text(_dateApi.dayName(day, true));
            row.appendChild(cell);
        });
        head.appendChild(row);
        return head;
    ...
    _appendFoot: function(){
        ...
        $(label).text(this._localize.dp_d_week);
        ...
        (this._$$today = $(cell.appendChild(link))).text(this._localize.dp_d_today);
        row.appendChild(cell);
...
function CalendarMonthYear(){
    ...
    self.create = function($$parent, dateValue, close){
        ...
        $$foot.find(".s-calendar-my-btns").append($("<a class='s-calendar-my-btn'/>").text(localize.dp_d_ok)).append($("<a class='s-calendar-my-btn'/>").text(localize.dp_d_cancel));
...
exports.TimeChoice = helpers.defineClass(TimeChoice, null, {
    create: function(field, options){
        ...
        self.curTime = field._input.value ? _timeApi.parse(field._input.value, field.localFormat) : _timeApi.now();
        self.field = field;
    ...
    _createCore: function(){
        ...
        $$core.append("<tr><th class='s-time-choice-th' colspan='" + self.columns + "'>" + localize.dp_t_title + "</th></tr>");
        var cell = 0;
