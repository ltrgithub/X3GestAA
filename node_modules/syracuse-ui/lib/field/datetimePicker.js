...
exports.Calendar = helpers.defineClass(Calendar, null, {
                ...
                self._$$quick.bind("click", function(){
                    if (!self.$$quickList) {
                        ...
                        for (var ii = 0, jj = intervals.length; ii < jj; ii++) {
                            self.$$quickList.append("<div class='s-calendar-quick'><a class='s-calendar-quick-link'>" + self._localize.fdpBegin + "</a>/<a class='s-calendar-quick-link'>" + self._localize.fdpEnd + "</a>" + self._localize[intervals[ii]] + "</div>");
                        }
    ...
    create: function(field, options){
        this._localize = field.getLocalize();
    ...
    _appendFoot: function(){
        ...
        label.textContent = this._localize.fdpWeek;
        ...
        (this._$$today = $(cell.appendChild(link))).text(this._localize.fdpToday);
        row.appendChild(cell);
    ...
    _toggleMonthYear: function(){
        var self = this;
        if (!self._monthYear) {
            self._monthYear = new CalendarMonthYear(self.field);
...
function CalendarMonthYear(field){
    var self = this;
    ...
    self.create = function($$parent, dateValue, close){
        ...
        $$foot.find(".s-calendar-my-btns").append($("<a class='s-calendar-my-btn'/>").text(field.getLocalize().fdpOk)).append($("<a class='s-calendar-my-btn'/>").text(field.getLocalize().fdpCancel));
...
exports.TimeChoice = helpers.defineClass(TimeChoice, null, {
    ...
    _createCore: function(){
        ...
        $$core.append("<tr><th class='s-time-choice-th' colspan='" + this.columns + "'>" + this.field.getLocalize().fdpTitle + "</th></tr>");
        var cell = 0;
        ...
        for (var rr = 0; rr < rowCount; rr++) {
            ...
            for (var col = 0; col < this.columns; col++) {
                ...
                if ($$a && timeHour.hour == this.curTime.hour) 
