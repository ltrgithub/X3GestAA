...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	//this.form
	appendIE9Compatibility: function(btns) {
		...
		this.uploadFrame.setAttribute("id", this.uploadFrameId);
		this.uploadFrame.setAttribute("name", this.uploadFrameId);
		this.uploadFrame.setAttribute("width", "0");
		this.uploadFrame.setAttribute("height", "0");
		this.uploadFrame.setAttribute("border", "0");
		this.uploadFrame.setAttribute("style", "width: 0; height: 0; border: none;");
		this._right.appendChild(this.uploadFrame);
		this._right.appendChild(this.form);

		var self = this;
		self.appendingIE9Compatibility = true;
		self.uploadFrameHandler = self.uploadFrame.addEventListener("load", function() {
			if (self.appendingIE9Compatibility) {
				self.appendingIE9Compatibility = null;
			} else {
				//  self.uploadFrame.removeEventListener("load", self.uploadFrameHandler, false);
				...
				if (self.uploadFrame.contentDocument) {
					...
					if (self.uploadFrame.contentWindow) {
						content = JSON.parse($(self.uploadFrame.contentWindow.body).text());
				...
				if (content) {
					...
					document.site.showDiagnoses({
						$diagnoses: [{
							$severity: "error",
							$message: self.localize.fErrUpload
						}]
					});
				}
	...
	_appendActions: function($skin) {
		...
		if (this.isIe9) {
			this.appendIE9Compatibility(btns);
		} else {
				...
				if (self.$item.$isAutoSize) {
					if (!self._image) {
						...
						self._image.onload = function() {
							...
							self._imageArea.style.height = self.$item.$imageHeight;
							document.site.toggleClass(self._imageArea, self.$skin + "-img-img-wait");
							document.site.toggleClass(self._imageArea, self.$skin + "-img-img", true);
							// display image
							self.onWindowResize();
					...
					self._waitingTimeOut = setTimeout(function() {
						if (self._imageArea) {
							self._imageArea.style.height = "";
							document.site.toggleClass(self._imageArea, self._imageArea.className);
							document.site.toggleClass(self._imageArea, self.$skin + "-img-img-wait", true);
						}
					}, 500);
	...
	releaseMode: function(onDispose) {
		...
		this.uploadFrame && this.uploadFrame.removeEventListener("load", this.uploadFrameHandler, false);
