...
function _setImageFile(field, file) {
	...
	if (file) {
		...
		if (field.anchor) {
			...
			syra_dom.disableItem(field.anchor, true);
...
function _removeImageArea(field) {
	syra_dom.removeChild(field._imageArea);
}

function _addImageArea(field) {
	_removeImageArea(field);
	var img = field._imageArea = syra_dom.addDiv(field.$skin + "-img-img");
	...
	add: function(field) {
		...
		field._waitingTimeOut = setTimeout(function() {
			if (field.anchor && !field._imageWait && !field._isImageLoaded) {
				field._imageWait = syra_dom.addDiv("s-field-img-img-wait", field.anchor);
			}
		}, 500);
		field._imgLoader.src = field.imageUrl;
	},
	clearWait: function(field) {
		syra_dom.removeChild(field._imageWait);
...
function _showImageFile(field) {
	...
	if (field.imageUrl) {
		...
		if (field.anchor) {
			...
			syra_dom.disableItem(field.anchor, false);
		}
	}
}
...
function _setFile(field, file) {
	if (file) {
		...
		if (field.$item.$isEmbedded) {
			...
			field._imgLoader.src = syra_site.$iconPath + "/file/s-file-" + (image || "default") + ".png";
			_showBtn(field, field.removeBtn, true);
...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	disableButtons: function(disable) {
		syra_menus.button.disable(this.selectBtn, disable);
		syra_menus.button.disable(this.removeBtn, disable);
	},
			...
			doEvent: function() {
				for (var ii = 0; ii < files.length; ii++) {
					...
					if (_validateFile(self, file)) {
						...
						self.xhrUploading = syra_controller.upload({
							file: file,
							...
							_releaseXhrUploading(self);
							self.articleParent && self.articleParent.applyChange(data, response);
						});
	...
	_renderEditMode: function() {
		...
		syra_dom.disableItem(this.anchor, true);
		...
		if (this.isImage && this.$field.$acceptedTypes) {
			this.input.setAttribute("accept", this.$field.$acceptedTypes.join(","));
		}
	...
	_onUploadSuccess: function(file, data, response) {
		...
		syra_workingCopy.storeChange(this.page);
	},
