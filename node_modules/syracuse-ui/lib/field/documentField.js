...
helpers.defineClass(Uploaders, null, {
	...
	abortAll: function() {
		if (this.list) {
			for (var ii = 0, jj = this.list.length; ii < jj; ii++) {
				try {
					this.list[ii].xhrUploading && this.list[ii].xhrUploading.abort();
				} catch (error) {}
			}
		}
		_disableAll(false);
	...
	release: function(field, disposed) {
		...
		if (ii >= 0) {
			...
			disposed && field.xhrUploading && field.xhrUploading.abort();
			_disableAll(false);
		}
	}
});
...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	_onEndUpload: function(file) {
		...
		setTimeout(function() {
			if (self._progress) {
				syra_dom.remove(self._progress);
