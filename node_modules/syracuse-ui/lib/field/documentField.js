...
function _setImageFile(field, file) {
	...
	if (file) {
		...
		if (field.removeBtn) {
			field.removeBtn.style.display = "none";
		}
	}
}
...
function _showImageFile(field) {
	...
	if (field.imageUrl) {
		...
		if (field.removeBtn && !field.$isReadOnly) {
			field.removeBtn.style.display = "";
		}
...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	_appendActions: function($skin) {
		...
		this.selectBtn = syra_menus.addTextButton(syra_local.fdocSelectFile, $skin + "-upload", "onSelectClick");
		btns.appendChild(this.selectBtn);

		this.removeBtn = syra_menus.addTextButton(syra_local.fdocRemoveFile, $skin + "-remove", "onRemoveClick");
	...
	resizeField: function() {
		var imgW = this._image && this._imgLoader.width;
	...
	_setFile: function(file) {
		...
		if (file) {
			...
			if (self.$item.$isEmbedded) {
				...
				if (!self._imgLoader) {
					self._imgLoader = document.createElement("img");
					self._imgLoader.style.display = "none";
					self._imgLoader.onload = function() {
						clearTimeout(self._defaultFileTimeOut);
						self._imgLoader.className = self.$skin + "-file-img";
						self._imgLoader.style.display = "";
					};
					self.anchor.appendChild(self._imgLoader);
				}
				...
				self._imgLoader.src = syra_site.$item.$iconPath + "/file/s-file-" + (image || "default") + ".png";
			...
			if (self._imgLoader) {
				delete self._imgLoader.onload;
				delete self._imgLoader;
			}
			if (self.anchor) {
