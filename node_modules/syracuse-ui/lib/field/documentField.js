...
function _setImageFile(field, file) {
	...
	if (file) {
		...
		if (field.anchor) {
			syra_site.dom.empty(field.anchor);
	...
	add: function(field) {
		...
		field._waitingTimeOut = setTimeout(function() {
			if (field.anchor && !field._imageWait && !field._isImageLoaded) {
				field._imageWait = document.createElement("div");
				field._imageWait.className = "s-field-img-img-wait";
				field.anchor.appendChild(field._imageWait);
			}
		}, 500);
		field._imgLoader.src = field.imageUrl;
	},
	clearWait: function(field) {
		if (field._imageWait && field._imageWait.parentNode) {
			field._imageWait.parentNode.removeChild(field._imageWait);
...
function _removeImageArea(field) {
	if (field._imageArea) {
		if (field._imageArea.parentNode) {
			field._imageArea.parentNode.removeChild(field._imageArea);
		}
...
function _addImageArea(field) {
	_removeImageArea(field);
	var img = field._imageArea = document.createElement("div");
	img.className = field.$skin + "-img-img";
...
function _showImageFile(field) {
	...
	if (field.imageUrl) {
		field.imageUrl = syra_site.expressionMaker.parse(field.articleParent, field.imageUrl);
		// key value encoding
		...
		if (!field._imageArea) {
			field._dataValue.style.display = '';
		}
...
function _setFile(field, file) {
	if (file) {
		...
		field.$fileUrl = syra_site.expressionMaker.parse(field.articleParent, field.$fileUrl || field.$dataUrl);
		...
		if (field._imgLoader) {
			delete field._imgLoader.onload;
			delete field._imgLoader;
		}
		if (field.anchor) {
			syra_site.dom.empty(field.anchor);
			syra_site.dom.toggleClass(field.anchor, field.$skin + "-file-default", false);
			field.anchor.setAttribute("href", "#");
			field.anchor.setAttribute("disabled", true);
		}
		_showBtn(field, field.removeBtn, false);
...
function _onBeginUpload(field, file) {
	...
	field._progress.className = "s-file-progress";
	field._info.appendChild(field._progress);
	field.progressValue = document.createElement("div");
	field.progressValue.className = "s-file-progress-value";
	field._progress.appendChild(field.progressValue);
	_disableButtons(field, true);
	field._legend.textContent = syra_local.fdocUpload + ":" + " " + file.name + " 0%";
}

function _onProgressUpload(field, file, progress) {
	field._legend.textContent = syra_local.fdocUpload + ":" + " " + file.name + " " + progress + "%";
	field.progressValue.style.width = progress + "%";
}

function _appendActions(field, $skin) {
	var btns = document.createElement("div");
	btns.className = $skin + "-doc-btns";

	field.inputFile = document.createElement("input");

	syra_site.setSpecificAttributes(field.inputFile);

	field.inputFile.setAttribute("type", "file");
	if (field.isImage && field.$field.$acceptedTypes) {
		field.inputFile.setAttribute("accept", field.$field.$acceptedTypes.join(","));
	}
	if (field.$item.$enableMultiple) {
		field.inputFile.setAttribute("multiple", true);
	}
	//btns.appendChild(field.inputFile);
	field._bindInputFileChange(true);

	btns.appendChild(field.inputFile);
	field.inputFile.className = "s-field-input-uploader";

	var addButton = field.$item.$inplace ? syra_menus.addIconButton : syra_menus.addTextButton;

	field.selectBtn = addButton(syra_local.fdocSelectFile, $skin + "-doc-btn s-field-doc-upload", "onSelectClick", null, "upload");
	btns.appendChild(field.selectBtn);

	field.removeBtn = addButton(syra_local.fdocRemoveFile, $skin + "-doc-btn s-field-doc-remove", "onRemoveClick", null, "cancel");
	_showBtn(field, field.removeBtn, false);
	btns.appendChild(field.removeBtn);

	field._right.appendChild(btns);
}

function _disableButtons(field, isDisabled) {
	field.selectBtn && syra_site.dom.disableItem(field.selectBtn, isDisabled);
	field.removeBtn && syra_site.dom.disableItem(field.removeBtn, isDisabled);
}

function _showBtn(field, btn, show) {
	if (btn) {
		if (field.$item.$inplace) {
			btn.style.visibility = (show && !field.$isReadOnly) ? "visible" : "hidden";
		} else {
			btn.style.display = (show && !field.$isReadOnly) ? "" : "none";
		}
	}
}
...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	dispose: function() {
		...
		this._progress = this._info = this._imageWait = this._legend = this._file = this._imageArea = this.anchor = this.imageUrl = null;
		if (this.uploadFrame) {
