...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	...
	_uploadIE9: function(files) {
		...
		self.page.externalAdapter.onBeforeUploadFiles({
			...
			doEvent: function() {
				...
				self.uploadFrame.setAttribute("id", self.uploadFrameId);
				...
				self.uploadFrameHandler = function() {
					...
					if (self.uploadFrame.contentDocument) {
						content = JSON.parse($(self.uploadFrame.contentDocument.body).text());
						//content = self.uploadFrame.contentDocument.body.innerHTML;
					} else {
						if (self.uploadFrame.contentWindow) {
							content = JSON.parse($(self.uploadFrame.contentWindow.document.body).text());
							//content = self.uploadFrame.contentWindow.document.body.innerHTML;
						} else {
							if (self.uploadFrame.document) {
								content = JSON.parse($(self.uploadFrame.document.body).text());
					...
					if (self.uploadFrame) {
						self.uploadFrame.removeEventListener("load", this.uploadFrameHandler, false);
						document.site.removeDomChild(self.uploadFrame);
						self.uploadFrame = null;
					}
	...
	appendIE9Compatibility: function(btns, $skin) {
		...
		this.form.setAttribute("id", this.id + "uploadform");
		this.form.setAttribute("enctype", "multipart/form-data");
		this.form.setAttribute("encoding", "multipart/form-data");
		this.form.setAttribute("method", "post");
		this.form.setAttribute("target", this.uploadFrameId = this.id + "uploadframe");
		this.form.appendChild(btns);

		var inputFileId = this.id + "inputFile";
		this._inputFile.setAttribute("id", inputFileId);
		this._inputFile.setAttribute("name", "datafile");
		this._inputFile.style.width = "0px";
		this._inputFile.style.height = "0px";
		this._inputFile.style.visibility = "hidden";
		this.selectBtn = document.createElement("label");
		this.selectBtn.className = $skin + "-upload";
		this.selectBtn.setAttribute("for", inputFileId);
		...
		this._right.appendChild(this.form);
	},
			...
			if (self.imageUrl) {
				...
				if (self.$item.$isAutoSize) {
					...
					self._waitingTimeOut = setTimeout(function() {
						if (!self._imageWait && !self._isImageLoaded) {
