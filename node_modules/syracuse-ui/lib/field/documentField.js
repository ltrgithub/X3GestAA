...
function _showImageFile(field) {
	...
	if (field.imageUrl) {
		...
		if (!field._imageArea) {
			field._imageArea = document.createElement("div");
			field._imageArea.className = field.$skin + "-img-img";
			field.anchor.appendChild(field._imageArea);
		...
		if (field.$item.$isAutoSize) {
			...
			field._waitingTimeOut = setTimeout(function() {
				if (field._imageArea && !field._imageWait && !field._isImageLoaded) {
					...
					field._imageArea.appendChild(field._imageWait);
				}
			}, 500);
...
function _setFile(field, file) {
	if (file) {
		...
		if (field.$item.$isEmbedded) {
			...
			field._image.src = syra_site.$item.$iconPath + "/file/s-file-" + (image || "default") + ".png";
			_showBtn(field, field.removeBtn, true);
