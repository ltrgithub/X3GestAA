...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
        ...
        self.page.externalAdapter.onBeforeUploadFiles({
            ...
            doEvent: function(){
                for (var ii = 0; ii < files.length; ii++) {
                    var file = files[ii];
                    if ((self.acceptFilter && (file.type.match(self.acceptFilter) || self.acceptFilter.match(file.type))) || !self.acceptFilter) {
    ...
    _onBeginUpload: function(file){
        ...
        this._legend.textContent = this.getLocalize().fdoc_upload + ":" + " " + file.name + " 0%";
    },
    _onProgressUpload: function(file, progress){
        this._legend.textContent = this.getLocalize().fdoc_upload + ":" + " " + file.name + " " + progress + "%";
        this.progressValue.style.width = progress + "%";
    },
    _onEndUpload: function(file){
        var self = this;
        self._legend.textContent = file.name + " " + this.getLocalize().fdoc_uploaded;
        ...
        setTimeout(function(){
            ...
            self._legend.textContent = self.getLocalize().fdoc_dropSelectFile;
        }, 1500);
    },
    ...
    _appendActions: function($skin){
        ...
        this.selectBtn.setAttribute("title", this.getLocalize().fdoc_selectFile);
        this.selectBtn.textContent = this.getLocalize().fdoc_selectFile;
        ...
        this.removeBtn.setAttribute("title", this.getLocalize().fdoc_removeFile);
        this.removeBtn.textContent = this.getLocalize().fdoc_removeFile;
    ...
    _renderEditMode: function(){
        ...
        this._legend.textContent = this.getLocalize().fdoc_dropSelectFile;
    ...
    _setDocumentFile: function(file){
        if (file) {
            var errors = [];
            if (file.type !== undefined && this.$field.$acceptedTypes) {
                if (this.$field.$acceptedTypes.indexOf(file.type) < 0) {
                    errors.push(this.getLocalize().fdoc_not_supported);
                }
            }
            if (this.$field.$maxByteLength && file.size > this.$field.$maxByteLength) {
                errors.push(self.getLocalize().fdoc_too_large);
            }
            if (errors.length > 0) {
