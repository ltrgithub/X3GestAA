...
exports.UnknowField = helpers.defineClass(UnknowField, Field, {
	...
	loadBox: function() {
		...
		if (this.$item.$inplace) {
			...
			this._core.className = this._$cssField + "-core";
		...
		switch (this.unknowMode) {
			case "nowidget":
				this.fieldValue.textContent = syra_local.fieldUnknowType + ": " + this.$item.$bind + "  type=" + (this.$field.$type || "");
				break;
			case "noproto":
				this.isItemHidden = true;
				this.fieldValue.textContent = syra_local.fieldNoProtoype + ": " + this.$item.$bind;
