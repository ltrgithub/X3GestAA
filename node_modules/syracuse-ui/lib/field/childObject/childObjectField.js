...
exports.ChildObjectField = helpers.defineClass(ChildObjectField, Article, {
    ...
    applyMetaData: function(metaData){
        ...
        if (metaData.$item) {
            record.applyChange(metaData.$item);
        }
    ...
    setDataBind: function(dataRecord, parentDataRecord, metaData){
        var self = this;
        var redraw = false;
        if (self.getPage().externalAdapter.setDataBind(self, dataRecord, parentDataRecord, metaData)) {
            if (metaData) {
                if (redraw = self.applyMetaData(metaData)) {
                    if (dataRecord === undefined) {
                        dataRecord = self.getDataSet();
            ...
            if (dataRecord !== undefined) {
                ...
                self._record = new ChildObjectFieldRecord();
                self._record.$layoutOptions = self.$layoutOptions.$cards;
                self._record.$recordUuid = options.dataRecord.$uuid;
                self._record.$recordIndex = options.$recordIndex;
                self._record.$facet = self.$recordFacet;
                self._record.$isEditMode = self.$isEditMode;
                self._record.list = self;
                self._record.$prototype = options.$prototype || self.$prototype.$item;
                options.$item = {
                    $category: "section",
                    $layout: {
                        $items: self.$item.$layout.$items
                    }
                };
                document.itemFactory.initializeItem(self._record, options.$item || {}, self);
                self._record.loadBox(options.dataRecord);
