...
function lazyInit($) {
	...
	$buttons.size.popupClick = function(e, data) {
		...
		if (ev.startsWith("<p></p>")) ev = ev.substring(7);
		if (ev.endsWith("<p></p>")) ev = ev.substring(0, ev.length - 7);
...
function _getStylesFromNode(node) {
	...
	if (node && node.style) {
		...
		if (node && node.nodeName && node.parentNode) {
			if (node.nodeName === "SPAN") _getStylesFromNode(node.parentNode);
		}
	}
}
...
function _getTagNames(node) {
	...
	while (node && node.parentNode) {
		if (node.tagName) {
			if (node.tagName === "SPAN" || node.tagName === "BODY" || node.tagName === "HTML" || node.tagName === "P" || node.tagName === "FONT") break;
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	render: function() {
		...
		if (this.getTextRows()) {
			$$area.attr("rows", this.getTextRows());
		}
	...
	_getRawEditContents: function() {
		if (this.isEditable() && this._editor && this._editor.doc) {
			if (this._editor.doc.body) {
				return fixHtml(this._editor.doc.body[this._editor.options.noValueOnload ? "outerHTML" : "innerHTML"] || "");
			} else
