...
function _setButtonMode(button, isRaw) {
	...
	syra_site.dom.toggleClass(button, "buttonRichText", !isRaw);
	syra_site.dom.toggleClass(button, "buttonRawText", isRaw);
}
		...
		$buttons.rawMode = {
			...
			buttonClick: function(e, data) {
				...
				if (editor.options._isRaw()) {
					...
					html = syra_site.dom.textToHTML(_fixHtml(editor.$area.val()));
					var $$content = $(html);
					if ($$content.is('pre')) {
						html = $$content.html();
					}
	...
	if (editor && editor.$frame) {
		...
		if (frame) {
			...
			if (childDoc) {
				...
				var height = (childDoc.body.scrollHeight + 35) + "px"; //40 is security margin for iframe
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	_createValueContainer: function() {
		...
		this._dataValue.className = this._$cssField + "-htmleditor-value";
	...
	render: function() {
		...
		syra_site.dom.hide(self._caret, true);
		...
		syra_site.setSpecificAttributes(self.textArea);
