...
function fixHtml(html) {
	return html.replace(/<br\s*[\/]?>$/gi, "").replace(/<p><\/p>/gi, "").replace(/<div><div>/gi, "<div>").replace(/<\/div><\/div>/gi, "<\/div>");
}

// use html sanitizer from https://github.com/google/googlecrisismap/blob/master/aux/html-css-sanitizer.js
function sanitize(html) {
	return rtf.toHtml(rtf.fromHtml(html_sanitize(html)));
}
...
function lazyInit($) {
		...
		$$popup.find(".s-msgbox-buttons-link").unbind("click").bind("click", function(event) {
			...
			if ($(this).attr('data-s-id') === syra_local.msgbox_yes) {
				if (!editor.options._isRaw()) {
					...
					var rawtext = normalizeRawText(editor.doc.body.textContent),
	...
	$buttons.size.buttonClick = function(e, data) {
		...
		// console.log("size click: font-size=" + fs);
	};
	$buttons.size.popupClick = function(e, data) {
		...
		data.value = divS + '<span style="font-size:' + e.target.value + 'pt; ">' + data.editor.selectedHTML().replace(/font-size:[0-9. ]+p(t|x);/g, '').replace(/<span style=""><\/span>/g, '') + '</span>' + divE;
		return true;
	};
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	setDirty: function(dirty) {
		if (!dirty) {
			...
			if (this.isEditable() && !this._isRaw)
				this._setRawEditContents(rtf.toHtml(contents));
	...
	render: function() {
		...
		self._editor = $$area.cleditor({
			...
			// remove source feature to prevent xss attack
	...
	_finalizeCreate: function() {
		...
		if (this.$field.$valueStyle) this._editor.options.bodyStyle = this.$field.$valueStyle;
		this._refreshToolbar();
		this._refreshEditor();
