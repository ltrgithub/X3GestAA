...
function lazyInit($) {
	...
	var popupRawModeContent = document.site.messageBox.$$item[0].outerHTML;
	document.site.messageBox.close();
	...
	var prefix = "he-it",
		popupInsertTableContent = getInsertTableDialog(prefix);
	...
	$buttons.insertTable = {
		...
		popupClass: "cleditorPrompt",
		popupContent: popupInsertTableContent,
	};
	...
	$buttons.insertTable.buttonClick = function(e, data) {
		...
		$$popup.css("padding", "0");
		// Wire up the submit button click event handler
		$$popup.find(".s-page-link").unbind("click").bind("click", function(event) {
			event.preventDefault();
			if ($(this).attr('data-s-id') === "ok") {
				var rows = parseInt($$popup.find("#" + prefix + "-rows-input").val(), 10),
					cols = parseInt($$popup.find("#" + prefix + "-cols-input").val(), 10),
					border = parseInt($$popup.find("#" + prefix + "-border-input").val(), 10),
					header = $$popup.find("#" + prefix + "-header-input").is(':checked'),
					i, j;

				var html = '<br><div><table border="' + (border || 0) + '">';
				if (header) {
					html += '<tr>';
					for (j = 0; j < cols; j++) {
						html += '<th>' + locale.format(_localize.tableHeader, j + 1) + '</th>';
					}
					html += '</tr>';
				}
				for (i = 0; i < rows; i++) {
					html += '<tr>';
					for (j = 0; j < cols; j++) {
						html += '<td>' + locale.format(_localize.tableCell, i + 1, j + 1) + '</td>';
					}
					html += '</tr>';
				}
				html += '</table></div>';
...
function _getSelection(editor) {
	...
	if (!sel && elt.selection) sel = elt.selection.createRange();
	return sel;
}
...
function getInsertTableDialog(prefix) {
	return '<div style="" class="s-modal-page-container s-h2">' + //
	'<div style="" class="s-h2-body">' + //
	'<div class="s-h2-layout s-layout-stack">' + //
	//
	'<div class="s-slot-stack s-h2-layout-parent">' + //
	'<div class="s-h2-layout s-layout-row" style="width: 100%;">' + //
	'<div class="s-field-slot s-slot-row" style="width: 50%;"><div class=" s-field-top" data-s-field="' + prefix + '-rows" data-s-inout="' + prefix + '-rows">' + //
	'<label class="s-field-top-title" for="' + prefix + '-rows-input">Rows</label>' + //
	'<div class="s-field-top-core"><div class="s-field-value-slot" style="max-width: 240px;"><div class="s-field-value">' + //
	'<input type="text" spellcheck="false" autocorrect="off" class="s-field-input" maxlength="3" data-s-field="' + prefix + '-rows" id="' + prefix + '-rows-input"></div></div></div></div></div>' + //
	'<div class="s-slot-sep-row s-h2-sep-row"></div>' + //
	'<div class="s-field-slot s-slot-row" style="width: 50%;"><div class=" s-field-top" data-s-field="' + prefix + '-cols" data-s-inout="' + prefix + '-cols">' + //
	'<label class="s-field-top-title" for="' + prefix + '-cols-input">Columns</label>' + //
	'<div class="s-field-top-core"><div class="s-field-value-slot" style="max-width: 400px;"><div class="s-field-value">' + //
	'<input type="text" spellcheck="false" autocorrect="off" class="s-field-input" maxlength="3" data-s-field="' + prefix + '-cols" id="' + prefix + '-cols-input">' + //
	'<div class="s-field-pickers"><a class="s-field-choice-picker" data-s-picker="choice"></a></div></div></div></div></div></div></div></div>' + //
	'<div class="s-slot-sep-stack s-h2-sep-stack"></div>' + //
	//
	'<div class="s-slot-stack s-h2-layout-parent">' + //
	'<div class="s-h2-layout s-layout-row" style="width: 100%;">' + //
	'<div class="s-field-slot s-slot-row" style="width: 50%;">' + //
	'<div class=" s-field-top" data-s-field="' + prefix + '-border" data-s-inout="' + prefix + '-border">' + //
	'<label class="s-field-top-title" for="' + prefix + '-border-input">Border size</label>' + //
	'<div class="s-field-top-core">' + //
	'<div class="s-field-value-slot" style="max-width: 240px;">' + //
	'<div class="s-field-value">' + //
	'<input type="text" spellcheck="false" autocorrect="off" class="s-field-input" maxlength="3" data-s-field="' + prefix + '-border" id="' + prefix + '-border-input">' + //
	'</div></div></div></div></div>' + //
	'<div class="s-field-slot s-slot-row" style="width: 50%;"><div class=" s-field-top" data-s-field="' + prefix + '-header" data-s-inout="' + prefix + '-header">' + //
	'<label class="s-field-top-title" for="' + prefix + '-header-input">With header</label>' + //
	'<div class="s-field-top-core"><div class="s-field-value-slot" style="max-width: 240px;"><div class="s-field-value s-field-edit-boolean">' + //
	'<input type="checkbox" spellcheck="false" autocorrect="off" class="s-field-input-boolean" data-s-field="' + prefix + '-header" id="' + prefix + '-header-input" checked="checked" data-s-selected="true">' + //
	'<div class="s-slot-sep-row s-h2-sep-row"></div>' + //
	'</div></div>' + //
	'<div class="s-slot-sep-stack s-h2-sep-stack"></div>' + //
	//
	'<div class="s-slot-stack s-h2-layout-parent">' + //
	'<div class="s-h2-layout s-layout-row" style="width: 100%;">' + //
	'<div class="s-page-link-slot s-slot-stack" style="">' + //
	'<a data-s-menu="14-22" data-s-id="ok" style="" class="s-page-link" title="Ok"><div class="s-page-link-label">Ok</div></a>' + //
	'<a data-s-menu="14-22" data-s-id="cancel" style="" class="s-page-link" title="Cancel"><div class="s-page-link-label">Cancel</div></a>' + //
	'</div></div></div>' + //
	'</div></div></div>';
