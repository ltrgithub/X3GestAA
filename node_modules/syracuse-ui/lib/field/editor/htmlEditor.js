...
debug && (function() {
	var WorkBook = require('syracuse-ui/lib/fusion/workBook').WorkBook,
		WinStack = require('syracuse-ui/lib/fusion/core/client/sap/window').WinStack,
		typeMap = {
			"text/rtf": function(p, key) {
				if (p.$maxByteLength) {
					p.$maxLength = p.$maxByteLength;
				} else if (p.$maxLength && p.$maxLength < 12) {
	...
	aspect.before(WorkBook, ["loadSheet"], function(
	context, args, result) {
		var $prototype = args[0] && args[0].$representation && args[0].$representation.$prototype || {},
	...
	aspect.before(WinStack, ["applyChange"], function(
	context, args, result) {
		var winId = args[0] || {},
			change = args[1] || {},
			options = args[2] || {};
...
function lazyInit($) {
	...
	+ '<div class="s-msgbox-header-title">' + _localize.aboutToSwitchToRawTitle + '</div>' //
	...
	+ '</div>' //
	+ '<div class="s-msgbox-body s-msgbox-msg-warning">' + _localize.aboutToSwitchToRawConfirm + '</div>' //
	...
	$buttons.rawMode.buttonClick = function(e, data) {
		...
		$$popup.find(".s-msgbox-button").unbind("click").bind("click", function() {
			if ($(this).attr('data-s-id') === "yes") {
				if (!editor.options._isRaw()) {
					var div = $('<div>').html(normalizeRawText(editor.$area.val())),
						rawtext = div.text();
					div = $('<div>')
					var pre = $('<pre>').appendTo(div).html(rawtext);
					// editor.$area.val(div.html());
					$(editor.doc.body).html(div.html());
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	setDataValue: function(value) {
		debug && console.log('htmlEditor.setDataValue:\n' + value);
	...
	getInputValue: function() {
		debug && console.log('htmlEditor.getInputValue:\n' + this._getPersistentContents(this._editor.$area.val()));
		if (this.$isEditMode && this._editor && this._editor.$area) {
			return this._getPersistentContents(this._editor.$area.val());
		}
		return "";
	},
	/// -------------
	...
	/// 
	isDirty: function() {
		return this._checksumIn && this._checksum !== this._checksumIn;
	},
	_updateChecksum: function(contents) {
		this._checksum = checksum(contents);
		this._checksumIn = this._checksumIn || this._checksum;
	},
	...
	getCaretPosition: function() {
		if (!(this.$isEditMode && this._editor && this._editor.doc.body && this.$$caret)) return 0;

		// TODO: make it work with IE
		if (ie) return 0;
		...
		// 		if (range.commonAncestorContainer.parentNode == root) {
		// 			caretPos = range.endOffset;
		// 		}
		// 	}
		// } else if (doc.createRange) {
		// 	range = doc.createRange();
		// 	range.startContainer = root.firstChild;
		// 	if (range.parentElement() == root) {
		// 		var tempEl = document.createElement("span");
		// 		root.insertBefore(tempEl, root.firstChild);
	...
	render: function() {
		...
		lazyInit(jQuery);
		debug && console.log('htmlEditor.render');
		var self = this;
		...
		if (this.$isEditMode) {
			...
			this.$$caret = $("<div>").attr("id", id + "-caret").attr("style", "display:none;");
			...
			self._editor = area.cleditor({
				...
				updateTextArea: function(html) {
					debug && console.log('htmlEditor.editor.updateTextArea:' + self.getCaretPosition());
					debug && console.log('htmlEditor.editor.updateTextArea:\n' + html);
					// Changes must be propagated manually since the textarea won't trigger a change event.
					var newValue = self._getPersistentContents(html);
					self._updateChecksum(newValue);
					debug && console.log('htmlEditor.editor.updateTextArea notifyChange: isDirty=' + self.isDirty() + "\n" + newValue);
					if (!self.standalone) {
						self.notifyFieldChange(newValue);
					}
					return html;
				}
			})[0];
	...
	_finalizeCreate: function() {
		...
		this._delayedBind && this._delayedBind();
	},
	_refreshToolbar: function() {
		// hack to refresh the toolbar state and disable the toolbar buttons if necessary
		this.$isEditMode && this._editor.disable(this._editor.disabled);
	},

	_bindEditorInput: function(bind) {
		var self = this;
		debug && console.log("htmlEditor._bindEditorInput");
		var doc = this._editor.doc,
			$$doc = $(doc),
			$$input = $(doc.body),
		...
		if (bind) {
			$$input //
			.on("change keydown keypress keyup", function(event) {
	...
	// Event handling for fusion session
	_bindInput: function(bind) {
		debug && console.log("htmlEditor.bindInput");
		var $$input = $(this._editor.doc.body);
		// var $$input = this._editor.$frame;
		var self = this;
		if (bind) {
			this._delayedBind = this._bindEditorInput.bind(this, bind);
		} else {
			this._bindEditorInput(bind);
		}
	},

	_rawAllowed: function() {
		return !!(this.$capabilities && this.$capabilities.raw);
	},
	_setRawEditContents: function(contents) {
		if (this.$isEditMode && this._editor && this._editor.$area) {
			debug && console.log('htmlEditor._setRawEditContents:\n' + contents);
			this._updateChecksum(contents);
	...
	_setContents: function(contents, saveStyles) {
		debug && console.log('htmlEditor._setContents 1: $isEditMode=' + this.$isEditMode + '\n' + contents);

		this._isRaw = false;
		if (!contents || contents.length === 0) {
			// No contents, assume we want to edit Rich Text
		} else if (contents.match(/^\s*{\\rtf/)) {
		...
		if (this.$isEditMode) {
			...
			this.$$fieldValue.empty().append(contents);
		}
	},
	_getPersistentContents: function(contents) {
		...
		if (this._isRaw) {
			...
			debug && console.log('htmlEditor._getPersistentContents fromHtml 1:\n' + contents);
			contents = rtf.fromHtml(contents);
			debug && console.log('htmlEditor._getPersistentContents fromHtml 2:\n' + contents);
		}
		return contents;
	},
