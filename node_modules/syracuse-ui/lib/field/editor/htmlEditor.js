...
function lazyInit($) {
	...
	function _setColorTitle(popup) {
		$(popup).children(":not([title]), [title='']").attr("title", function() {
			...
			var c = /\d+,\s+\d+,\s+\d+/.exec($(this).css("backgroundColor"))[0].split(",");
	...
	_bindInput: function(bind) {
		...
		if (bind) {
			...
			$$input.on("keydown keypress keyup", function(event) {
				debug && console.log("htmlEditor.bindInputEvent $$input: " + event.type + " " + self.id);
				if (keyboardHandlers[event.type]) keyboardHandlers[event.type](event);
				event.stopPropagation();
			});
			...
			$$inputWindow.blur(function(event) {
				...
				if (self.isDirty(syra_site.getBrowserIdentity().isMSIE)) { // When popup is opening $$inputWindow is onblur with IE 
