...
function lazyInit($){
 ...
 +
    '<div class="s-msgbox-header-title">' +
    ...
    '</div>' //
 +
    '<div class="s-msgbox-body s-msgbox-msg-warning">' +
    ...
    $buttons.rawMode.buttonClick = function(e, data){
        ...
        $$popup.find(".s-msgbox-button").unbind("click").bind("click", function(){
            if ($(this).attr('data-s-id') === "yes") {
                if (!editor.options._isRaw()) {
                    var div = $('<div>').html(normalizeRawText(editor.$area.val())), rawtext = div.text();
                    div = $('<div>')
...
function _getRange(editor){
    if (ie) 
        return _getSelection(editor).createRange();
