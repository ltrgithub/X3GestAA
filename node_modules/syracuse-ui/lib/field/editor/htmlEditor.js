...
function lazyInit($) {
	...
	$.each(formattingbuttons.split(" "), function(idx, buttonName) {
		if ($buttons[buttonName]) {
			...
			$buttons[buttonName].buttonClick = (function() {
				var click = $buttons[buttonName].buttonClick;
				return function(e, data) {
					data.editor.options._freezeCacheChange();
					return click ? click.call(this, e, data) : true;
				};
			})();
		}
	});
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	onWindowResize: function() {
		var editor = this._editor,
			width = this.$$item && this.$$item[0].clientWidth;
		debug && console.log("onWindowResize: width=" + (this.$$item && this.$$item[0].clientWidth) + ", layoutSlot.clientWidth=" + this.layoutSlot.clientWidth);
		if (width) {
			if (this.isEditable()) {
				if (!this.isCreated) {
					this.isCreated = true;
