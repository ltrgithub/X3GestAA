	...
	var WorkBook = require('syracuse-ui/lib/fusion/workBook').WorkBook,
		...
		typeMap = {
			"text/rtf": function(p, key) {
				if (p.$maxLength && p.$maxLength < 12) p.$maxLength = 512 * (2 ^ p.$maxLength) - 2;
				p.$isDualMode = true;
			}
		};
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	_finalizeCreate: function() {
		...
		// Intercept paste event to get the opportunity to sanitize the contents
		var $$body = $(this._editor.doc.getElementsByTagName("body")[0]);
		$$body.bind('paste', function(e) {
			...
			if (e && e.clipboardData && e.clipboardData.getData) {
				...
				var el = $(this);
				// Hiding the the underlying element causes some unpredictable behavior
				// var display;
	...
	_rawAllowed: function() {
		return !!(this.$field && this.$field.$isDualMode);
	},
