...
function redactorMode() {
	return syra_site.redactorMode || syra_site.developpementMode;
}

function sanitizeHtml(html) {
	return redactorMode() ? html : html_sanitize(html, uriRewriter);
}
	...
	rawMode: function($buttons) {
		$buttons.rawMode = {
			...
			buttonClick: function(e, data) {
					...
					callback: function(response) {
						if (response.$clientId === "yes") {
							if (!editor.options._isRaw()) {
								...
								var rawtext = _normalizeRawText(_editorHelpers.getEditorInnerText(editor.doc.body, editor.$frame[0].contentWindow)),
				...
				syra_dom.removeChild(data.popup);
			}
		};
	},
...
var _content = {
	set: function(field, contents, saveStyles) {
		...
		!field.$isEditMode && field.setState(field);

	},
	getRawEdit: function(editor) {
		if (editor && editor.doc) {
			if (editor.doc.body) {
				return _fixHtml(editor.doc.body[editor.options.noValueOnload ? "outerHTML" : "innerHTML"] || "");
			} else {
				if (editor.$area) {
	...
	getPersistent: function(field, contents) {
		...
		if (hash === field._cache.checksum) {
			if (!field._isRaw || contents.indexOf('<pre>') < 0)
...
function _disable(field, disabled) {
	var editor = field._editor;
	editor && editor.disable(disabled);
	if (editor && editor.disabled && !field.$isEditMode)
		editor.$toolbar.hide();
}
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
	...
	getInputValue: function() {
		...
		if (this.$isEditMode && this._editor && this._editor.$area) {
			value = _content.getPersistent(this);
	...
	setDirty: function(dirty) {
		if (!dirty) {
			...
			this._cache.dirtyChecksum = this._cache.checksum;
			this.$isEditMode && !this._isRaw && _content.setRawEdit(this._editor, rtf.toHtml(this.$field.$type != "text/html" ? _htmlEscape(contents) : contents));
		}
		this.dirty(dirty);
	},
	dirty: function(set) {
	...
	render: function() {
		...
		if (self.getTextRows()) {
			self.textArea.setAttribute("rows", self.getTextRows());
		}
		...
		self._editor = $(self.textArea).cleditor({
			...
			" | extendSize",
			colors: _colors,
			docCSSFile: "/syracuse-ui/themes/desktop/cleditor/jquery.cleditor-doc.css",
