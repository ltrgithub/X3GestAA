...
function lazyInit($){
    ...
    var popupRawModeContent = document.site.messageBox.domItem.outerHTML;
    ...
    $buttons.rawMode.buttonClick = function(e, data){
        var button = e.target, editor = data.editor, field = ((editor || {}).options || {})._field, $$popup = $(data.popup), html;
    ...
    $buttons.insertTable.buttonClick = function(e, data){
        ...
        document.site.dialogManager.openPopup(document.site.page, {
            $isAutoClose: true,
            $itemPage: {
                $isEditMode: true,
                $facet: "$edit",
            ...
            onValidate: function(page){
                ...
                if (page.dataset && Object.keys(page.dataset).length > 0) {
                    var rows = parseInt(page.dataset.$rows, 10), cols = parseInt(page.dataset.$columns, 10), border = parseInt(page.dataset.$borderSize, 10), header = page.dataset.$withHeader, i, j;
...
exports.HtmlEditor = helpers.defineClass(HtmlEditor, Field, {
    ...
    getLazyUrl: function(value){
        return document.site.expressionMaker.parse(this.articleParent, value && value.$url || this.$field.$url);
    },
    ...
    isDirty: function(){
        ...
        var value = this._getPersistentContents(), dirty = (this._cache.dirtyChecksum != this._cache.checksum);
        if (debug) {
    ...
    setDirty: function(dirty){
        if (!dirty) {
            this._getPersistentContents(); // to compute checksum;
            this._cache.dirtyChecksum = this._cache.checksum;
        }
    ...
    render: function(){
        ...
        var self = this, id = self.id || helpers.uuid.generate();
    ...
    _bindInput: function(bind){
        ...
        if (bind) {
            ...
            $$inputWindow.focus(function(event){
                ...
                document.site.dom.toggleClass(self._dataValue, "cleditorMain-focus", true);
                event.stopPropagation();
            });
            $$inputWindow.blur(function(event){
                ...
                document.site.dom.toggleClass(self._dataValue, "cleditorMain-focus", false);
                if (self.isDirty()) {
    ...
    dispose: function(){
        ...
        this._editor = this.$$caret = this.$$savedStyles = this._cache = this.$capabilities = null;
