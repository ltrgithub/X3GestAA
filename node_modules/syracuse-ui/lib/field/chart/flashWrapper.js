...
function dataHelper($extension, options) {
    var types = {
    ...
    var root = {
        X3PORTAL: {
            ...
            Technical: {
                ...
                DataSrc: {
                    $: {
                        Type: options && options.type || types[$extension.type][0],
                        Code: ""
                    }
                },
                VisCmp: {
                    $: {
                        Type: options && options.vtype || types[$extension.type][1],
                    },
                },
                UsedDataSrcPar: {},
                UserLang: {
    ...
    function _addObject(parent, obj) {
        forEachKey(obj, function(k, v) {
            parent[k] = v;
        });
        return this;
    }

    function _addNode(parent, key, value, attr) {
        var node = parent[key] = {
            $value: value
        };
        forEachKey(attr, function(k, v) {
            node.$[k] = v;
        });
        return this;
    }
    ...
    return {
        ...
        addData: function(obj) {
            return _addObject(root.X3PORTAL.Data, obj);
        },
        addVisCmp: function(key, value, attr) {
            return _addNode(root.X3PORTAL.Technical.VisCmp, key, value, attr);
        }
    };
}
...
function _getComponentType(article) {
    var proto = article.$prototype,
        item = article.$item,
        $extensionType = proto.$extension && proto.$extension.$type;
    if (proto.$cube) return "graph";
    if ($extensionType) {
        var match = /application\/x-(\w+)/.exec($extensionType);
...
function Wrapper(componentType, properties, hosterData, options) {
    ...
    this.type = componentType || _getComponentType(hosterData.instance);
...
Wrapper.prototype.fxsendctxaction = function() {
    /**
     * Scope                        : All components
    ...
    if (this.hosterData.instance.fxexeclink) {
        this.hosterData.instance.fxexeclink(this.fxOnDummy, this.hosterData, this._getLinkObj(link));
    }
};
...
Wrapper.prototype.fxOnStdSvcDone = function(err, reply) {
    ...
    if (this.opts.xmlNative) {
        ...
        } else if (reply.data) xmlResult = this._processDataIn(data, null);
...
Wrapper.prototype.fxOnError = function(diagnoses) {
    var fxErr = new Error(diagnoses, this.id);
...
Wrapper.prototype.setData = function(data, meta) {
    ...
    if (data) {
        if (this.firstDataDone) this._populateFxData(this._wrapFxDataInObj(this._processDataIn(data, meta)), this.curCmdCtx._cmd || "fxdataupdated");
        else {
...
Wrapper.prototype._trace = function(type, mess) {
    if (_log) console.log("FlashWrapper: " + type + " - " + mess)
    if (!_debug) return;
...
Wrapper.prototype._processDataIn = function(data, meta) {
    if (this.opts.xmlNative) {
        ...
        var transformer = this['_' + this.type + '_DataJson2Xml'];
        return transformer && transformer.call(this, data, meta);
    }
};
...
Wrapper.prototype._getGraphDef = function(data, rawData) {
    ...
    if (data.$axes && data.$axes.length > 0 && data.$axes[0].$title && graphDef._metaData.title != data.$axes[0].$title) graphDef._metaData.title += ' / ' + data.$axes[0].$title;
    ...
    if (measures) {
        ...
        _.each(restraintMeasures, function(measure, key) {
            ...
            graphDef._metaData.measuresDefinition[code] = {
                "property": code,
                "title": measure.$title || data.$properties[code].$title || "???",
                "definition": measure,
                "xRefCode": null
            };
            // Reference field?
...
Wrapper.prototype._graph_DataJson2Xml = function(rawData, meta) {
    ...
    // "Messages" Tag
    var messTag = new Error(this.currentData.$diagnoses, this.id);
...
Wrapper.prototype._padtool_DataJson2Xml = function(rawData, meta) {
    // Data mapping
    ...
    // "Technical" Tag
    xmlDocTag.push('<Technical><VisCmp Type="' + (this.type == "padtool" ? "FPADT" : "FDADP") + '"></VisCmp><UserLang>' + this.opts.cdLang + '</UserLang></Technical>');
...
Wrapper.prototype._padplayer_DataJson2Xml = function(rawData, meta) {
    return this._padtool_DataJson2Xml(rawData, meta);
};

Wrapper.prototype._agenda_DataJson2Xml = function(rawData, meta) {
    var proto = null,
        $$container = this.hosterData.instance.$$container;
    ...
    var $resources = this.currentData.$resources,
        ...
        xmlDocTag = [],
        x3map = $extension && $extension.$mapping || {},
        flashInput = {
            X3PORTAL: {
                Data: {
                    Lines: [],
    ...
    console.log(jsxml.stringify(flashInput));
    ...
    function addColumn(i, vi, vv, att) {
        var xml = '<Col Num="' + i + '" X3Type="7"';
        if (att) {
            xml += ' ' + att.name + '="' + att.value + '"';
        }
        xml += '><X3IntVal>' + vi + '</X3IntVal>' + '<X3Value>' + vv + '</X3Value>' + '</Col>';
        xmlDocTag.push(xml);
    };

    function newCol(i, vi, vv, att) {
        var col = {
            Col: {
                $: {
                    Num: "" + i,
    ...
    $resources.forEach(function(r) {
        ...
        cols.forEach(function(k, i) {
            ...
            case "KEYACT":
                att = {
                    name: "X3Link",
                    value: "x3://12074,OBJET=" + actionCode + "&amp;CLES=" + vi
                };
            case "TYPACT":
            ...
            addColumn(i + 1, vi, v, att);
        });
        xmlDocTag.push('</Line>');
    });
    xmlDocTag.push('</Lines>');

    xmlDocTag.push('<Titles>');
    xmlDocTag.push('<Title></Title>');
    xmlDocTag.push('<ColTitle Num="1" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="2" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="3" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="4" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="5" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="6" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="7" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="8" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="9" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="10" X3Type="7"></ColTitle>');
    xmlDocTag.push('<ColTitle Num="11" X3Type="7"></ColTitle>');
    xmlDocTag.push('</Titles>');

    var typact = x3map["TYPACT"],
        actionEnum = this.actionEnum || (typact && this.hosterData.instance.getLocalMenu(typact.$mnu));
    var colors = ["0xFDE6D5", "0xDEF3DD", "0xF9FDD5"];

    xmlDocTag.push('<Categs>');
    actionEnum && actionEnum.forEach(function(m, i) {
        xmlDocTag.push('<Categ Id="' + (i + 1) + '" Color="' + (colors[i] || "") + '">' + m + '</Categ>');
    });
    xmlDocTag.push('</Categs>');
    ...
    // HtcProvType value is 'CLOB' if the flash component is bound to a clob otherwise it's 'TABLEBLOC'
    xmlDocTag.push('<HtcProvType>TABLEBLOC</HtcProvType>');

    xmlDocTag.push('<DataSrc Type="AGD" Code=""/>');
    xmlDocTag.push('<VisCmp Type="FAG">');
    // The next 3 values are not used by the agenda but only by the calendar
    xmlDocTag.push('<FCCTYPE>XWEEK</FCCTYPE>');
    xmlDocTag.push('<FCSELC>2</FCSELC>');

    xmlDocTag.push('<FCDATEDEB>3</FCDATEDEB>');
    xmlDocTag.push('<FCDATEFIN>5</FCDATEFIN>');
    xmlDocTag.push('<FCHEURDEB>4</FCHEURDEB>');
    xmlDocTag.push('<FCHEURFIN>6</FCHEURFIN>');
    xmlDocTag.push('<FCCHRONO>2</FCCHRONO>');
    xmlDocTag.push('<FCTEXTE>10</FCTEXTE>');
    xmlDocTag.push('<FCTYPE>1</FCTYPE>');
    xmlDocTag.push('<FCCATEG>9</FCCATEG>');
    xmlDocTag.push('<FCTIERS>7</FCTIERS>');
    xmlDocTag.push('<FCINTERLOC>8</FCINTERLOC>');
    xmlDocTag.push('<FCSTATUS>11</FCSTATUS>');
    xmlDocTag.push('</VisCmp>');
    xmlDocTag.push('<Action>INIT</Action>');
    xmlDocTag.push('<UsedDataSrcPar>');
    xmlDocTag.push('<DAYBEF>10</DAYBEF>');
    xmlDocTag.push('<DAYAFT>10</DAYAFT>');
    xmlDocTag.push('</UsedDataSrcPar>');
    xmlDocTag.push('<UserLang>BRI</UserLang>');
    xmlDocTag.push('</Technical>');
...
<X3PORTAL> \
   <Data> \
      <items title="" start="201208010000" end="201209021200"> \
         <item Num="1" tlabel="Utilisateur 1"> \
            <subi typ="2" ccount="5" sdate="201208311200" dur="2.00" elapse="2.00" /> \
         </item> \
         <item Num="3" tlabel="Utilisateur 2"> \
            <subi typ="2" ccount="5" sdate="201208010000" dur="14.00" elapse="14.00" /> \
         </item> \
         <item Num="5" tlabel="Utilisateur 3"> \
            <subi typ="3" ccount="5" sdate="201208070000" dur="12.00" elapse="12.00" /> \
         </item> \
         <item Num="7" tlabel="Utilisateur 4"> \
            <subi typ="2" ccount="5" sdate="201208140000" dur="15.00" elapse="15.00" /> \
         </item> \
         <item Num="9" tlabel="Utilisateur 5" /> \
         <item Num="10" tlabel="Utilisateur 6"> \
            <subi typ="2" ccount="5" sdate="201208101200" dur="16.00" elapse="16.00" /> \
         </item> \
         <item Num="12" tlabel="Utilisateur 7"> \
            <subi typ="2" ccount="5" sdate="201208140000" dur="17.00" elapse="17.00" /> \
         </item> \
         <item Num="14" tlabel="Utilisateur 8"> \
            <subi typ="2" ccount="5" sdate="201208140000" dur="17.00" elapse="17.00" /> \
         </item> \
         <item Num="16" tlabel="Utilisateur 9"> \
            <subi typ="2" ccount="5" sdate="201208010000" dur="4.00" elapse="4.00" /> \
            <subi typ="2" ccount="5" sdate="201208140000" dur="1.00" elapse="1.00" /> \
         </item> \
         <item Num="19" tlabel="Utilisateur 10"> \
            <subi typ="2" ccount="5" sdate="201208010000" dur="4.00" elapse="4.00" /> \
            <subi typ="2" ccount="5" sdate="201208090000" dur="1.00" elapse="1.00" /> \
            <subi typ="2" ccount="5" sdate="201208140000" dur="13.00" elapse="13.00" /> \
         </item> \
      </items> \
      <itemproperties> \
         <iprop id="tlabel" label="Nom" type="7" /> \
      </itemproperties> \
      <subitypes> \
         <sityp id="1" label="Travaillé" type="4" color="" /> \
         <sityp id="2" label="Congés payés" type="4" color="" /> \
         <sityp id="3" label="RTT" type="4" color="" /> \
         <sityp id="4" label="Maladie" type="4" color="" /> \
         <sityp id="5" label="Formation" type="4" color="" /> \
      </subitypes> \
   </Data> \
   <Technical> \
      <DataSrc Type="GAT" Code="B" /> \
      <HtcProvType>TABLEBLOC</HtcProvType> \
      <VisCmp Type="FGA" Width="972"> \
         <FTLISTCOLS>tlabel</FTLISTCOLS> \
         <FTLOOK>SAGE1</FTLOOK> \
      </VisCmp> \
      <UsedDataSrcPar> \
         <DAYBEF>100</DAYBEF> \
    ...
    // Todo: change to addItem and sub item
    ...
    // TODO: get the local menu number from the prototype definition for itemproperties and subitypes
    var localMenu = this.localMenu || this.hosterData.instance.getLocalMenu("2948");
    var colors = ["0xFDE6D5", "0xDEF3DD", "0xF9FDD5"];

    // review this to adapt it for itemproperties
    xmlDocTag.push('<itemproperties>');
    localMenu && localMenu.forEach(function(m, i) {
        xmlDocTag.push('<iprop id="' + (i + 1) + '" Color="' + (colors[i] || "") + '">' + m + '</iprop>');
    });
    xmlDocTag.push('</itemproperties>');
    ...
    xmlDocTag.push('<DataSrc Type="AGD" Code=""/>');
    xmlDocTag.push('<VisCmp Type="FAG">');
    // These 3 values are not used by the agenda but only by the calendar
    xmlDocTag.push('<FCCTYPE>XWEEK</FCCTYPE>');
    xmlDocTag.push('<FCSELC>2</FCSELC>');
    xmlDocTag.push('<FCSELC>1</FCSELC>');
...
function Error(diagnoses, callerId) {
    /**
...
Error.prototype.getXML = function() {
    ...
    if (this.diagnoses) {
        _.each(this.diagnoses, function(diagnos) {
            ...
            xmlDiagnoses.push("<Mes><![CDATA[" + diagnosEx.message + (_fixture ? " from : " + this.callerId : "") + "]]></Mes></" + severity + ">");
        }, this);
    }
