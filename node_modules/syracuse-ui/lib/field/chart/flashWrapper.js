...
Wrapper.prototype._processDataIn = function(data, meta) {
	if (this.opts.xmlNative) {
		return this.currentData = data, data;
	} else {
...
Wrapper.prototype._gantt_DataJson2Xml = function(rawData, meta) {
	...
	$resources.forEach(function(r) {
				...
				case "ELAP":
					...
					if (key) {
						if (typ > 1) {
							...
							header = '<item Num="' + row + '" tlabel="' + label + '">';
						}
					}
					break;
			}
		...
		if (header != "" && pos != "") {
			if (xmlDocTag[xmlDocTag.length - 1] != "</item>") {
				xmlDocTag.push('</item>');
			}
			pos = "";
		}
