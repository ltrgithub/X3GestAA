...
var int = require('syracuse-core').types.integer;
...
// wmode="transparent" \
function _getTemplateFlashTag(props) {
	var isMSIE = syra_site.getBrowserIdentity().isMSIE;
...
<param name="allowScriptAccess" value="' + (isMSIE ? 'always' : 'sameDomain') + '"/>\
...
<param name="bgcolor" value="<%= bgcolor %>"/>' + //
		...
		(isMSIE ? '' : ('<embed src="<%= swf %>" name="<%= name %>" quality="high" \
...
function Wrapper(componentType, properties, hosterData, options) {
	...
	this.$$elt = $(_getTemplateFlashTag(this.props));
	this.elt = isMSIE ? this.$$elt.get(0) : this.$$elt.find("embed").get(0);
	...
	// Fix size
	if (this.opts._height && this.hosterData.instance && this.hosterData.instance.layoutSlot) {
		this.hosterData.instance.layoutSlot.style.height = this.opts._height;
	}
}
...
Wrapper.prototype._getGraphStyle = function(cubeDef, measuresDef) {
	// Init
	var graphStyle = {}, trick = {}, mStyle = "XPIE",
		mStyleEX = "",
		mStyleCode = "",
		mStyleEXCode = "",
		mDefault = null;
...
Wrapper.prototype._agenda_DataJson2Xml = function(rawData, meta) {
	...
	this.hosterData.instance.layoutSlot.style.height = "450px";

	var $resources = this.currentData.$resources,
		$properties = this.currentData.$properties,
		$decorator = this.$decorator,
		x3map = $decorator && $decorator.$bindings || {}, userLang = this.opts && this.opts.userLang || "BRI",
		xb = flash.xmlBuilder($decorator.$stereotype, {
			uersLang: userLang
		});
	...
	function newCol(i, vi, vv, attrs) {
		...
		if (attrs) {
			helpers.object.forEachKey(attrs, function(k, v) {
				node.Col.$[k] = v;
			});
		}
		return node;
