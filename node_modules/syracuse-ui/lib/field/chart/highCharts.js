...
var _syraUtil = require('syracuse-ui/lib/fusion/core/client/sap/util').Syra;

var forEachKey = _helpers.object.forEachKey;
			...
			article.dataset.reduce(function(res, current) {
				...
				if (categoryName != null) {
					...
					forEachKey(article._meta.series, function(key, s) {
						...
						if (!serie) {
							serie = _helpers.object.clone(s.options);
							serie.data = [];
	...
	if (!label) {
		...
		switch (item.$type) {
			...
			case "application/x-date":
				...
				label = _datetime.parse(measure[code]).toString(item.localeFormat);
				break;
...
exports.TabularChart = _helpers.defineClass(TabularChart, ChartArticle, {
	...
	getEnumeration: function(code) {
		var $fusionController = _syraUtil.getFusionController(this),
	...
	_createChart: function(settings) {
		...
		if (settings) {
			...
			forEachKey(settings.series, function(key, s) {
				var serie = _helpers.object.clone(s),
			...
			forEachKey(self._meta.series, function(key, s) {
				var serie = _helpers.object.clone(s.options);
				if (options.chart.polar && serie && serie.type) {
					delete serie.type;
				}
				serie.data = [0];
	...
	_getMenuItem: function(link) {
		var links = {};
		var $link = links[link.id] = _helpers.object.clone(link.link);
		return link && {
			$item: {
				$links: links
			}
	...
	_drill: function(serie, up) {
		...
		function pushProto() {
			stack.unshift(_helpers.object.clone(proto, true));
			return stack[0];
		}
