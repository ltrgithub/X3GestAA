...
// draggable-legend is not part of the official delivery but provided under MIT License by Torstein Hønsi the creator of highcharts
...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
	...
	_mergeMetaData: function(metadata) {
		...
		if (metadata && metadata.$item && metadata.$item.$properties && this.currentMetaData && this.currentMetaData.$cube && this.currentMetaData.$cube.$measures) {
			forEachKey(this.currentMetaData.$cube.$measures, function(key, m) {
		...
		if (settings) {
			if (!this._chart) {
				...
				if (series && series.length == 1 && series[0].name) {
					self._chart.xAxis && self._chart.xAxis[0].userOptions && (self._chart.xAxis[0].userOptions.title = series[0].name);
				}
	...
	_settingsFromValue: function() {
		...
		if ($cube.$style === "spiderweb") {
			...
			self.dataset &&
				self.dataset.reduce(function(res, current) {
					var $bindSelector = self.currentMetaData.$cube.$displaySelector;
	...
	_redraw: function() {
		if (!this.layoutSlot || !this._chart || this.layoutSlot.style.display === "none") {
			debug && console.log("Highcharts._redraw: chart not visible. Skip redraw!");
			return;
		}
		this._chart.redraw();
		this._displayLegend(this.displayLegend);
	},
		...
		if (measures) {
			forEachKey(measures, function(key, m) {
				...
				if (init) {
					...
					serie = self._meta.series[code] = {
						options: {
							id: '#s-' + code,
							// legendIndex: "",
							name: self.getLocalizeText(measure.$title) || String.fromCharCode(160),
				...
				if (serie) {
					...
					serie.options.name = self.getLocalizeText(measure.$title) || String.fromCharCode(160);
	...
	_getLinkDef: function(type, id, link) {
		return {
			...
			"id": id,
			"link": link,
			"title": this.getLocalizeText(link.$title)
		};
	},
	_getPref: function() {
	...
	_getMeasureLabel: function(measure, code) {
		...
		var label = this.getLocalizeText(measure.$title);
		if (label)
			return label;
		var self = this;
			...
			case "application/x-choice":
				...
				item.$value.$enum.some(function(element, i, array) {
					if (element.$value === label) {
						label = this.getLocalizeText(element.$title);
						return true;
					}
					return false;
				});
	...
	_execLink: function(link, recordIndex) {
		...
		syra_menus.click.menuId(this.record, Object.keys(link.$item["$links" || "$actions"])[0]);
	},
	...
	dispose: function() {
		...
		this._chart = this._meta = this.currentMetaData = null;
