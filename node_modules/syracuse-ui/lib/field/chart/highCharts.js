...
function mergeChanges(update, chartSerie, serie){
    ["name", "type", "visible"].map(function(key){
...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
    ...
    __applyDesignMetaData: function(metadata){
        ...
        var self = this, meta = self._meta, chart = self._chart, designMeta = this.currentMetaData, needRefresh = false;
            ...
            if (!this._chart) {
                ...
                if (!series || series.length === 0) {
                    ...
                    for (var ii = 0, len = series.length; ii < len; ii++) {
                        var chartSerie = series[ii], key = chartSerie.options.id.slice(3), serie = settings.series[key], update = {};
    ...
    _settingsFromValue: function(){
        var self = this, $cube = this.currentMetaData.$cube, settings;
    ...
    _createChart: function(settings){
        ...
        if (settings) {
            ...
            forEachKey(settings.series, function(key, s){
                var serie = objectClone(s), serieMeta = self._meta.series[key];
    ...
    _getOptions: function(proto){
        var self = this, $cube = this.currentMetaData.$cube, defOptions = Highcharts.getOptions(), buttons = defOptions.exporting.buttons, measuresTitle = this._getFieldEvalTitle(), axe = proto.$axes && proto.$axes[0] || {}, axeTitle = axe.$hierarchies && measuresTitle[axe.$hierarchies[0][0]] || axe.$title;
    ...
    _processMeasures: function(metaData, init){
        ...
        var self = this, items = this.$prototype.$properties;
        ...
        if (measures) {
            forEachKey(measures, function(key, m){
                ...
                if (!init && serie) {
                    ...
                    serie.options.visible = measure.$title !== "";
                }
    ...
    _getRestraintMeasures: function(){
        var proto = this.currentMetaData, $cube = proto.$cube, measures = $cube && $cube.$measures, restraintMeasures = measures;
        if (measures) {
    ...
    _drill: function(serie, up){
        ...
        var hierName = hierarchies[i][0], cubeHier = cube.$hierarchies[hierName];
        ...
        var rawData = this.dataset && this.dataset[serie.x], member = rawData && (rawData[hierName] + "");
