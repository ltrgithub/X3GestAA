...
require('syracuse-ui/deps/highcharts/highcharts-more');
...
exports.TabularChart = helpers.defineClass(TabularChart, Article, {
    ...
    applyDesignMetaData: function(metadata, onAuthoring) {
        ...
        if (metadata.$cube) {
            // self.cube.update(metadata.$cube);
            if (metadata.$cube.$measures) {
                forEachKey(metadata.$cube.$measures, function(k, m) {
                    // self.cube.update(m);
                    console.log("TODO: applyDesignMetaData on measure '" + k + "'");
                });
            }
        }
    },
    ...
    _settingsFromValue: function() {
        ...
        if ($cube.$style === "spiderweb") {
            ...
            self.dataset && self.dataset.reduce(function(res, current) {
                ...
                if (serie.name && serie.name.length > 0) {
            ...
            self.dataset && self.dataset.reduce(function(res, current) {
                ...
                forEachKey(self._meta.series, function(key, s) {
                    ...
                    serie.data.push([categoryName, parseFloat(current[key])]);
                });
                return res;
            }, settings);
        }
...
/*              align: 'center',
            ...
            tooltip: {
                formatter: function() {
                    ...
                    label = (label ? label + "<br>" : "") + (this.x || this.key) + ': <b> ' + (measure ? measure.formatApi.format(this.y, measure.localeFormat) : this.y) + '</b>';
    ...
    _processLinks: function(proto) {
        ...
        if (reprUrl) {
            proto.$links.$pptslide = {
                ...
                "$url": reprUrl + "&pptMode=newSlide&%24bind=" + this.$item.$bind,
