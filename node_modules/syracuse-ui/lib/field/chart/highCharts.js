...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
	...
	_selectChartMeasure: function() {
		...
		comboDiv.style.left = (rect.left) + "px";
		self.layoutSlot.appendChild(comboDiv);

		var $enums = [];
		self._chart.series.map(function(seriesItem, index) {
			$enums.push({
				$value: index,
				$title: seriesItem.name
			});
			seriesItem.setVisible(index === 0, true);
		});
		...
		self.choice.setDatasetValue = function(selectedIndex) {
			self._chart.series.map(function(seriesItem, index) {
				seriesItem.setVisible(index == selectedIndex, true);
			});
		};
	},
