...
exports.TabularChart = helpers.defineClass(TabularChart, Article, {
    onWindowResize: function(display){
        ...
        if (this._chart && !this._chart.isResizing) {
            ...
            this.resizeTimer = setTimeout(function(){
                self._chart && self._chart.setSize(self.$$container[0].clientWidth, self._chart.chartHeight || self.$$container[0].clientHeight);
            }, 100);
        }
    ...
    _createChoices: function(){
        var $choices = ["area", "areaspline", "bar", "column", "line", "scatter", "spline"];
        var $enums = [];
    ...
    setDataBind: function(dataRecordSet, parentDataRecord, metaData){
        // debug && console.log("HighCharts.setDataBind " + this.id + ": dataRecordSet=" + typeof(dataRecordSet);
        //this.dataRecordSet = dataRecordSet;
        debugger;
        if (metaData) {
            this._processMeta(metaData);
        }
        this.dataset = parentDataRecord || {};
        this.currentValue = dataRecordSet;
        
        if (dataRecordSet && this._chart) {
            var self = this;
            var settings = self._settingsFromValue(dataRecordSet);
            ...
            if (!series || series.length === 0) {
                ...
                for (var i = 0; i < series.length; i++) {
                    var key = series[i].options.id.slice(3), serie = settings.series[key];
    ...
    _settingsFromValue: function(value){
        var self = this, value = value || this.currentValue;
        return value &&
        value.reduce(function(res, current){
    ...
    _getOptions: function(proto){
        var self = this, $cube = this.$prototype.$cube, defOptions = Highcharts.getOptions(), buttons = defOptions.exporting.buttons, measuresTitle = this._getFieldEvalTitle(), axe = proto.$axes && proto.$axes[0] || {}, axeTitle = axe.$hierarchies && measuresTitle[axe.$hierarchies[0][0]] || axe.$title;
        ...
        var options = {
            ...
            tooltip: {
                formatter: function(){
                    var seriesMeta = self._meta.series, measure = seriesMeta[this.series.options.id.slice(3)].measure, label = '' + this.x + ': <b> ' + (measure ? measure.formatApi.format(this.y, measure.localeFormat) : this.y) + '</b>', data = this.series.processedYData;
    ...
    _getMenuItem: function(link){
        var links = {}, $link = links[link.id] = objectClone(link.link);
    ...
    _executeRecordMenu: function(context){
        ...
        if (!this.recordArticle) {
            ...
            this.recordArticle.ensureDataSet();
    ...
    _onMeasureClick: function(context){
        ...
        if (!this.attachedField.$isDisabled) {
            if (this.page.externalAdapter.fusionSite) {
                ...
                if (!event.shiftKey) 
                    this._drill(data, event.shiftKey);
            }
        }
