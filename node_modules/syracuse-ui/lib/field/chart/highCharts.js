...
var forEachKey = helpers.object.forEachKey, objectCopy = helpers.object.copy, objectClone = helpers.object.clone, stringify = helpers.object.stringify;
...
exports.TabularChart = helpers.defineClass(TabularChart, Article, {
    onWindowResize: function(display){
        ...
        if (this._chart) {
            ...
            this.resizeTimer = setTimeout(function(){
                ...
                if (self._chart && !self._chart.isResizing) {
                    debug && self._chart && console.log("HighCharts.resizeTimer " + self.id + ": Resize");
                    self._chart.setSize(self.layoutSlot.clientWidth, self._chart.chartHeight || self.layoutSlot.style.clientHeight);
                    self._redraw();
                }
            }, 300);
        }
    ...
    applyDesignMetaData: function(metadata){
        var self = this, meta = self._meta, chart = self._chart;
        if (metadata.$style) {
            if (meta) {
                this._setStyle(metadata.$style);
            }
        }
        if (meta && chart && metadata.$cube) {
            if (metadata.$cube.$measures) {
                forEachKey(metadata.$cube.$measures, function(k, m){
                    var s = meta.series[k], serie = chart.get(s.options.id);
            ...
            if (this._chart && settings) {
                ...
                if (!series || series.length === 0) {
                    ...
                    for (var ii = 0; ii < series.length; ii++) {
                        var key = series[ii].options.id.slice(3), serie = settings.series[key];
    ...
    _settingsFromValue: function(){
        var self = this, $cube = this.$prototype.$cube, settings;
        ...
        if ($cube.$style === "spiderweb") {
            ...
            settings.series = {};
            self.dataset &&
            self.dataset.reduce(function(res, current){
                var serie = {
                ...
                if (serie.name) {
                    ...
                    forEachKey(self._meta.series, function(key, s){
                        serie.data.push([key, parseFloat(current[key])]);
                    });
            ...
            self.dataset.reduce(function(res, current){
                ...
                if (categoryName) {
                    ...
                    forEachKey(self._meta.series, function(key, s){
                        ...
                        serie.data.push([categoryName, parseFloat(current[key]) || 0]);
                    });
                }
                return res;
            }, settings);
    ...
    _createChart: function(settings){
        var self = this, options = _merge(self._getPref(), self._getOptions(self.$prototype));
    ...
    _getOptions: function(proto){
        var self = this, $cube = proto.$cube, defOptions = Highcharts.getOptions(), buttons = defOptions.exporting.buttons, measuresTitle = this._getFieldEvalTitle(), axe = proto.$axes && proto.$axes[0] || {}, axeTitle = axe.$hierarchies && measuresTitle[axe.$hierarchies[0][0]] || axe.$title;
        ...
        var options = {
            ...
            tooltip: {
                formatter: function(){
                    var seriesMeta = self._meta.series, serie = this.key && seriesMeta[this.key] || seriesMeta[this.series.options.id.slice(3)], measure = serie && serie.measure, data = this.series.processedYData, y = this.y;
                    
                    if (measure) {
                        self._ensureFormatter(measure);
                    }
                    var label = this.series.name || measure && measure.$title, mformat = measure && measure.formatApi, mlocale = measure && measure.localeFormat;
                    ...
                    var proto = self.$prototype, $cube = proto.$cube, hCount = $cube.$hierarchies && Object.keys($cube.$hierarchies).length || 0;
        ...
        if (Highcharts.theme.version[0] === 3) {
            ...
            if (Highcharts.theme.version[0] === 2) {
    ...
    _processMeasures: function(proto){
        var self = this, $cube = this.$prototype.$cube, items = this.$prototype.$properties;
    ...
    _processLinks: function(proto){
        var meta = this._meta, xAxis = meta.xAxis && meta.xAxis[0], reprUrl = this.page.$prototype && this.page.$prototype.$representationUrl;
    ...
    _processAxes: function(proto){
        ...
        var self = this, currAxisHier = 0, axis0 = proto.$axes && proto.$axes[0];
    ...
    _drill: function(serie, up){
        var self = this, cube = this.$prototype.$cube, meta = self._meta, i = meta.drill.currAxisHier, stack = self._meta.drill.stack;
        ...
        var proto = stack[0], axis0 = proto.$axes && proto.$axes[0];
    ...
    _onMeasureClick: function(context){
        var self = this;
        var event = context.event || {}, data = context.data || {}, line = data.x + 1;
    ...
    _setStyle: function(type){
        var self = this, count = Object.keys(self._meta.series).length, size = 80, slice = Math.round(size / count);
        forEachKey(self._meta.series, function(key, s){
            s.type = type;
            if (self._chart) {
    ...
    _setType: function(serie, type, size, innerSize){
        var self = this, chart = serie.chart;
        var s = {
            id: serie.options.id,
            type: type,
            name: serie.name,
