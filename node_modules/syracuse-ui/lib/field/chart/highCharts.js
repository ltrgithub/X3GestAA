...
Highcharts.theme = require('syracuse-ui/themes/desktop/highcharts/theme').theme;
...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
	...
	_resizeChart: function() {
		var height = Math.max(this._chart.chartHeight || this.layoutSlot.clientHeight, 250);
		this._chart.setSize(this.layoutSlot.clientWidth, height);
		this._redraw();
		if (this.currentMetaData.$cube.$displaysOneMeasure) {
			this._selectChartMeasure();
		}
	},
	...
	_selectChartMeasure: function() {
		...
		self.choice.onFieldInputEvent = function(event) {
			if (event.type == "change") {
				var newValue = this.getDataValue(event);
				if (newValue != this.currentValue) {
					for (var ii = 0, jj = self._chart.series.length; ii < jj; ii++) {
						self._chart.series[ii].setVisible(ii === newValue, true);
					}
					self.resizeChart();
					this.currentValue = newValue;
					self.saveWidgetChoice(self, newValue);
				}
			}
		};
		self.choice.chart = self._chart;
		self.choice.setDataValue(self.getWidgetChoice(self));
		self.choice.setDatasetValue = function(selectedIndex) {
			for (var ii = 0, jj = self._chart.series.length; ii < jj; ii++) {
				self._chart.series[ii].setVisible(ii == selectedIndex, true);
			}
		};
		self.choice.setDatasetValue(self.getWidgetChoice(self));
	},
			...
			chart: {
				...
				events: {
					afterPrint: function(event) {
						syra_site && syra_site.resizeArticle();
					}
				},
				// marginLeft: 30,
				// marginRight: 150,
				...
				buttons: {
					contextButton: { // docs
						...
						menuItems: [{
							...
							//         document.body.innerHTML = '<pre>' + syra_dom.escape(svg, true) + '</pre>';
							//     }
							// }
						]
					}
	...
	getWidgetChoice: function(widget) {
		return !sessionStorage.getItem("landingpage_" + widget.page.openerUrlSegments.uri) ?
			0 : parseInt(sessionStorage.getItem("landingpage_" + widget.page.openerUrlSegments.uri));
	},

	saveWidgetChoice: function(widget, val) {
		sessionStorage.setItem("landingpage_" + widget.page.openerUrlSegments.uri, val);
	},
