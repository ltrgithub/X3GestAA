...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
	...
	_getOptions: function(proto) {
		...
		var options = {
			...
			legend: {
				...
				x: -10,
				title: {
					text: ':::: ' + syra_local.highCharts_seriesTitle,
				},
				floating: true,
				draggable: true,
				zIndex: 20
	...
	_getMeasureLabel: function(measure, code) {
		...
		var self = this;
		var label = self.getLocalizeText(measure.$title);
		if (label)
			return label;
			...
			case "application/x-choice":
				...
				item.$value.$enum.some(function(element, i, array) {
					if (element.$value === label) {
						label = self.getLocalizeText(element.$title);
						return true;
					}
					return false;
				});
	...
	_execLink: function(context, recordIndex) {
		if (typeof context === "string") {
			...
			context = links && this._getMenuItem(links[context]);
		}
		...
		if ($links) {
			...
			syra_menus.click.menuId(this.record, Object.keys($links)[0]);
		}
	},
