...
exports.TabularChart = helpers.defineClass(TabularChart, Article, {
	...
	createChoices: function() {
		var $choices = ["area", "areaspline", /*"bar",*/
		"column", "line", "scatter", "spline", "pie"];
		var $enums = [];
	...
	_setDataSet: function(dataRecordSet) {
		if (this.page.externalAdapter.fusionSite) {
			var last = dataRecordSet && dataRecordSet.length > 0 && dataRecordSet[dataRecordSet.length - 1] || {};
			if (this._isEmptyRecord(last)) {
				dataRecordSet = dataRecordSet.slice(0, -1);
			}
		}
		this.dataset = dataRecordSet;
	},

	_isEmptyRecord: function(dataRecord) {
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		debug && console.log(Date.now() + " - HighCharts.setDataBind " + this.id + ": dataset=" + (this.dataset && this.dataset.length) + "\n" + stringify(this.dataset));
