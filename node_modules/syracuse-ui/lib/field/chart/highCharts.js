...
exports.TabularChart = helpers.defineClass(TabularChart, ChartArticle, {
	...
	_getOptions: function(proto) {
		...
		var options = {
			navigation: {
				buttonOptions: {
					align: syra_site.isDocumentRTL ? "left" : "right"
				}
			},
			chart: {
			...
			legend: {
				useHTML: syra_site.isDocumentRTL,
				layout: 'vertical',
				align: syra_site.isDocumentRTL ? "left" : "right",
				rtl: syra_site.isDocumentRTL,
				verticalAlign: 'top',
				y: 60,
				x: -10,
				/*title: {
			...
			xAxis: [{
				reversed: syra_site.isDocumentRTL,
				categories: [],
				title: {
			...
			yAxis: [{
				opposite: syra_site.isDocumentRTL,
				title: {
					text: '',
					useHTML: syra_site.isDocumentRTL
				},
	...
	_execLink: function(link, recordIndex) {
		...
		syra_menus.click.menuId(this.recordArticle, Object.keys(link.$item["$links" || "$actions"])[0]);
	},
