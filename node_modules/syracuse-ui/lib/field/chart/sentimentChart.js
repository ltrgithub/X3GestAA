...
exports.SentimentChart = helpers.defineClass(SentimentChart, Field, {
	...
	createChart: function(value) {
		...
		function buildSerie(data, type) {
			return data.$resources.filter(function(r) {
				return r.sentiment.type === type;
			}).map(function(r) {
				var url = /href="([^"]*)"/.exec(r.tweet.source)[1];
				url = "https://twitter.com/" + r.tweet.user.name + "/status/" + r.tweet.id_str;
				return {
					x: r.tweet.follower_count,
					y: parseFloat(r.sentiment.score) || 0,
					z: r.tweet.retweet_count,
					url: url
				};
			});
		}
		...
		syra_controller.sendRequest(null, {
			$location: {
				$url: "/twitter/sentiment?entity=article&key=car&keywords=" + value,
			...
			var link1 = [],
				link2 = [];
			serie1 = buildSerie(data, "positive");
			serie2 = buildSerie(data, "negative");
