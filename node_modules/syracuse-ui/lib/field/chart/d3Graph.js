	...
	setDataValue: function(value) {
		...
		if (value) {
			if (value.$url) {
				self.page.fetch({
					$location: {
						$url: syra_expression.parse(self.articleParent, value.$url)
					}
				...
				if (value.$nodes || value.$links) {
					if (value.$properties) {
						if (value.$properties.$linkCategories) {
							self.$field.$linkCategories = value.$properties.$linkCategories;
						}
						if (value.$properties.$nodeCategories) {
							self.$field.$nodeCategories = value.$properties.$nodeCategories;
						}
					}
					self.$data = value;
					self.render();
