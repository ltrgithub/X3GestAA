...
function _makeData(field) {
	...
	var nodes = (field.$data.$nodes || []).map(function(node, i) {
		...
		var value = document.site.expressionMaker.parse(field.articleParent, proto.$prototype.$properties.$value, node);
...
function _updateGraph(field) {
	...
	newNodes.append("circle").attr("class", "d3-graph-node").attr("r", function(d) {
		...
		d.$field.$prototype && d.$field.$prototype.$links && d.$field.$prototype.$links.$default && document.controller.openPage(field.articleParent, d.$field.$prototype.$links.$default, d.$item);
	});
...
exports.nodeChart = helpers.defineClass(NodeChart, Field, {
	setDataValue: function(value) {
		...
		if (value && value.$url) {
			self.articleParent.fetch({
				$location: {
					$url: document.site.expressionMaker.parse(self.articleParent, value.$url)
				}
	...
	loadBox: function() {
		...
		this.forceHeight = this.height - 100;
	...
	render: function() {
		...
		var $$d3 = $("<div/>").addClass("d3-graph-field").appendTo(this._dataValue);
		...
		/*this.$$svg = $("<div/>").attr("width", width).attr("height", height).appendTo(svgDiv);
         return;*/
		this.$$svg = svgDiv.append("svg").attr("width", self.width).attr("height", self.height);
		// compute sizes
		this.$categories = {};
		// svg legend
		// HORZ LEGEND
