...
exports.OrgChart = helpers.defineClass(OrgChart, ChartArticle, {
	...
	_isDelta: function() {
		if (!this.dataset || !this.dataset.length) return false;
		var a = 1,
			b = 0,
	...
	_createChart: function(settings) {
		if (!this._isDelta()) return //In some cases receiving data identical, so do nothing to save time;
		this._clearTree(); // generally when X3 server resend unexpected datas 
				...
				chart: {
					...
					events: {
						...
						click: function(event) {
							syra_site.dialogManager.closePopups(null, event);
						}
					},
					spacingTop: 15,
					marginTop: 0
