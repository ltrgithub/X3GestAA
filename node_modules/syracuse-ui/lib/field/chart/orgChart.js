...
exports.OrgChart = helpers.defineClass(OrgChart, Article, {
	onWindowResize: function() {
		debug && console.log("OrgChart.onWindowResize " + this.id + " : displayed=" + this.displayed + " , size=(" + this.layoutSlot.clientWidth + ", " + this.layoutSlot.clientHeight + "), css-size=(" + this.layoutSlot.style.clientWidth + ", " + this.layoutSlot.style.clientHeight + ")");
	...
	_setDataSet: function(record) {
		var dataSet = this.$item.$bind && record && record[this.$item.$bind] || [];
		this.dataset = arrayHelper.fixDataSet(this, dataSet);
	},

	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		debug && console.log("OrgChart.setDataBind " + this.id + ": dataRecordSet.length=" + (dataRecordSet && dataRecordSet.length) + " , size=(" + this.layoutSlot.clientWidth + ", " + this.layoutSlot.clientHeight + ")");
		debug && console.log("OrgChart.setDataBind " + this.id + ": parentDataRecord.length=" + (parentDataRecord && parentDataRecord.length));
		var self = this;
		if (metaData) {
		...
		//Hack LV
		// Le pb est que setDataBinding est appelé en cas de delta partiel, en, cas de meta et en cas de delta total.
		// En cas de delta total pas de pb
		// en cas de meta, on passe dans _setDataSet alors que dans parentDataRecord il n'y a pas de record, le dataset est donc vidé
		// En cas de delta partiel meme type de pb (le cas est plus rare)
		if (!metaData) this._setDataSet(parentDataRecord);
		// debug && console.log("OrgChart.setDataBind " + this.id + ": dataset=" + (this.dataset && this.dataset.length) + "\n" + stringify(this.dataset));
		debug && console.log("OrgChart.setDataBind " + this.id + ": dataset=" + (this.dataset && this.dataset.length));

		var settings;
		if (this.dataset /*&& this.dataset.length > 0*/ ) { // LV : if no data : display empty chart
			if (this._chart) {
				// this.destroy();
	...
	externalCall: function(proxy, values, opt) {
		...
		function _resultCallback(res) {
			...
			err && document.site.showDiagnoses({
				$diagnoses: err
			});
		...
		dataset.forEach(function(e, i, a) {
			...
			if (id) {
				var parentId = e[$bindings.$parent],
					...
					node = _map[id],
					data = $bindings.$data && $bindings.$data.map(

					function($bind) {
						return e[$bind];
					}) || [];

				debug && console.log("OrgChart._buildTree: [" + i + "] id=" + id + ", parentId=" + parentId + ", desc=" + ($bindings.$title ? e[$bindings.$title] : ""));

				if (!parent && parentId && parentId.length !== 0) {
					parent = {
						children: [],
						id: parentId
					};
				...
				console.log("OrgChart._buildTree: [" + i + "] id=<null>");
			}
	...
	_createChart: function(settings) {
		...
		if (this.orgWidget) delete this.orgWidget;
...
OrgChartWidget.prototype = {
	...
	draw: function(root) {
		...
		var bbox = null,
			...
			worker = self._nodeSlot("", -999, -999, IMAGE_PLACEHOLER).add(tree),
			// create the label
		...
		function drawNode(node) {
			...
			addEvent(nodeSlot.element, 'mouseenter', function() {
			...
			addEvent(nodeSlot.element, 'mouseleave', function() {
		...
		function move(e) {
			...
			if (drag && drag.node) {
				...
				if (drag.target) {
					...
					if (!drag.target.parent) {
						dropHint = "bottom";
					} else if (pos.y > rect.y + rect.height / 2) {
						dropHint = "bottom";
			...
			} else if (chart.hoverNode) {
				parentOffset = $container.offset();
				pos = {
					x: e.pageX - parentOffset.left + chartSpacingLeft,
					y: e.pageY - parentOffset.top
				};
				chart.hoverNode.tooltipDesc && tooltip.attr({
		...
		function drawTree() {
			...
			self.chart.redraw();
			self.article.page.onWindowResize();
		}
		...
		function interact(args) {
			return self.article.page.externalCall(interactDesc, args);
		}
		...
		if ($allowMove) {
			addEvent(container, START, startDrag);
	...
	_nodeElement: function(str, x, y, img) {
		var renderer = this.chart.renderer,
			wrapper = renderer.g("node"),
			// text = renderer.text('', 0, 0, true).attr({
