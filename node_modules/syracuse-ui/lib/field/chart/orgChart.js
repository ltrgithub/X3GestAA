...
OrgChartWidget.prototype = {
    ...
    draw: function(root) {
        var self = this,
            ...
            minWidth = 50;
        var bbox = null,
            leftDropArea = renderer.rect(-999, -999, 10, 10).attr({
                fill: 'red',
                'fill-opacity': 0.1
            }).add(),
            rightDropArea = renderer.rect(-999, -999, 10, 10).attr({
                fill: 'red',
                'fill-opacity': 0.1
            }).add(),
            bottomDropArea = renderer.rect(-999, -999, 10, 10).attr({
                fill: 'red',
                'fill-opacity': 0.1
            }).add(),
        ...
        function showDropAreas(node) {
            leftDropArea.toFront().attr({
                x: node.rect.x,
                y: node.rect.y,
                width: node.rect.width/2,
                height: node.rect.height/2
            }).show();
            rightDropArea.toFront().attr({
                x: node.rect.x + node.rect.width/2,
                y: node.rect.y,
                width: node.rect.width/2,
                height: node.rect.height/2
            }).show();
            bottomDropArea.toFront().attr({
                x: node.rect.x,
            ...
            //     height: 0
            // });
            // leftDropArea.show().animate({
            //     x: node.rect.x - 15,
            //     width: 15,
            // });
            // rightDropArea.show().animate({
            //     x: node.rect.x + node.rect.width,
            //     width: 15
            // });
            // bottomDropArea.show().animate({
            //     height: 15
            // });
        }
