...
exports.OrgChart = helpers.defineClass(OrgChart, Article, {
        ...
        if (this._chart && this._chart.container) {
            ...
            this.resizeTimer = setTimeout(function() {
                ...
                if (self._chart && self._chart.container && !self._chart.isResizing) {
                    debug && self._chart && console.log("OrgChart.resizeTimer " + self.id + ": Resize");
                    if (self.layoutSlot.clientWidth) {
                        self._chart.setSize(self.layoutSlot.clientWidth, self._chart.chartHeight || self.layoutSlot.style.clientHeight || self.layoutSlot.clientHeight);
                        self._redraw();
                    }
                }
            }, 200);
    ...
    setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
        debug && console.log(Date.now() + " - OrgChart.setDataBind " + this.id + ": dataRecordSet.length=" + (dataRecordSet && dataRecordSet.length) + " , size=(" + this.layoutSlot.clientWidth + ", " + this.layoutSlot.clientHeight + ")");
        var self = this;
        if (metaData) {
        ...
        dataset.forEach(function(e, i, a) {
            ...
            if (id) {
                var parentId = e[$bindings.$parent],
                    ...
                    node = _map[id],
                    data = $bindings.$data && e[$bindings.$data] || [];
                ...
                if (!node.parent) {
                    node.parent = parent;
                    node.desc = e[$bindings.$title] + data.join(" ");
...
OrgChartWidget.prototype = {
    ...
    draw: function(root) {
        var self = this,
            ...
            $allowDoubleClick = ($prototype.$allowDoubleClick !== false),
            $allowMove = ($prototype.$allowMove !== false),
            $boxSize = $prototype.$boxSize,
        ...
        var bbox = null,
            ...
            tooltip = renderer.label('', 0, 0, null, null, null, true, null, 'tooltip').attr({
                ...
                zIndex: 8
            }).css(orgOptions.tooltip.style).hide().add();
        ...
        interactDesc.spgm = ($prototype.$interact || "AMAJHIER") + ":MAJHIER";
        ...
        function move(e) {
            ...
            if (drag && drag.node) {
                ...
                parentOffset = $container.offset();
                pos = {
                    x: e.pageX - parentOffset.left + 15,
                    y: e.pageY - parentOffset.top
                };
