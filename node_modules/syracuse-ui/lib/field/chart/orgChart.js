...
var merge = Highcharts.merge, extend = Highcharts.extend, each = Highcharts.each, addEvent = Highcharts.addEvent, removeEvent = Highcharts.removeEvent, HCElement = Highcharts.SVGElement, mathRound = Math.round, STROKE_WIDTH = 'stroke-width', IMAGE_PLACEHOLER = "$placeholder", UNDEFINED;
...
exports.OrgChart = helpers.defineClass(OrgChart, ChartArticle, {
    ...
    _buildTree: function(dataset){
        var $fusionController = syraUtil.getFusionController(this), imageBaseUrl = null, _map = this._map, $bindings = this.$prototype.$decorator.$bindings, binds = Object.keys($bindings).map(function(k){
            return $bindings[k];
        });
        if ($fusionController) {
        ...
        dataset.forEach(function(e, i, a){
            ...
            if (id) {
                var parentId = e[$bindings.$parent],                // key = parentId + "." + id,
                parent = _map[parentId], node = _map[id], data = $bindings.$data &&
...
// styles => $style
var options = Highcharts.getOptions(), colors = options.colors, orgOptions = options.orgChart, reparenter = {
...
var minHeight = 50, minWidth = 50, chartSpacingLeft = 45, chartSpacingTop = 45;
...
OrgChartWidget.prototype = {
    ...
    draw: function(root){
        var self = this, chart = this.chart, renderer = chart.renderer, options = chart.options, $prototype = self.article.$prototype, $decorator = $prototype.$decorator, $allowDoubleClick = ($decorator.$allowDoubleClick !== false), $allowMove = ($decorator.$allowMove !== false), $boxSize = $decorator.$boxSize, $style = $decorator.$style;
        ...
        function drawNode(node){
            var rect = node.rect, parent = node.parent, pRect = parent ? parent.rect : rect, p1 = {
        ...
        var container = chart.container, $container = $(container), drag;
        ...
        function drawTree(){
            ...
            self.chart.redraw();
            //self.article.page.resizeArticle();
        }
    ...
    _nodeElement: function(str, x, y, img){
        ...
        function updateBoxSize(){
            var boxX, boxY, style = text.element.style, textBBox = (width === undefined || height === undefined || wrapper.styles.textAlign) && text.getBBox();
