...
exports.OrgChart = helpers.defineClass(OrgChart, Article, {
			...
			this.resizeTimer = setTimeout(function() {
				...
				if (self._chart && self._chart.container && !self._chart.isResizing) {
					if (self.layoutSlot.clientWidth) {
						var size = {
							width: Math.max(self.orgWidget.size.width, self.layoutSlot.clientWidth),
	...
	_setDataSet: function(record) {
		...
		if (this.page.externalAdapter.fusionSite) {
			var last = dataSet && dataSet.length > 0 && dataSet[dataSet.length - 1] || {};
			if (arrayHelper.isEmptyRecord(this.$prototype.$properties, last)) {
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		this._setDataSet(parentDataRecord);
...
OrgChartWidget.prototype = {
	...
	draw: function(root) {
		...
		function drawTree() {
			...
			self.chart.setSize(Math.max(self.chart.chartWidth, self.size.width), self.size.height);
			self.chart.redraw();
