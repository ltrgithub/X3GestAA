...
helpers.defineClass(ChartRecord, Article, {
	showMenus: function(chartArticle, index, $links, event) {
		var from = (!chartArticle.getElement) ? event.target : chartArticle.getElement(null, index);
		from = (from && chartArticle.getElement) ? from.lastChild : from;
		if (from) {
			...
			syra_menus.pickers.menus.showPopup(this, from, true);
		}
	},
	drawBox: function() {},
...
exports.ChartArticle = helpers.defineClass(ChartArticle, FieldArticle, {
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if ((this.$item.$category && this.$item.$category == "tabularChart") || (this.$prototype.$type == "application/x-array")) {
			...
			if (this.page.externalAdapter.fusionSite) {
				...
				if (_sapUtil.isEmptyGridDataRecordEx(this.$prototype.$properties, last)) {
