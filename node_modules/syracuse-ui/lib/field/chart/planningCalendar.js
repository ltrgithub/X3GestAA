...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
	...
	_fetchDataRange: function(view) {
		...
		var cellsPerMonth = view.rowCnt * view.colCnt;

		var lastMonthFirstDate = view.incrementDate(view.intervalStart, -1);
		range.start = lastMonthFirstDate.subtract(lastMonthFirstDate.weekday(), 'days');

		var nextMonthFirstDate = view.incrementDate(view.intervalStart, 1);
		range.end = nextMonthFirstDate.add(cellsPerMonth - nextMonthFirstDate.weekday() - 1, 'days');
	...
	_fetchCalendarEvents: function(representationUrl, view) {
		...
		if (self.startDateFetchedEvents && self.endDateFetchedEvents) {
			...
			if (range.start.isBefore(self.startDateFetchedEvents)) {
				where = "&where=((" + filterStartProp + " GE @" + range.start.format('YYYY-MM-DD') + "@) and (" + filterStartProp + " LT @" + self.startDateFetchedEvents.format('YYYY-MM-DD') + "@))";
				self.startDateFetchedEvents = range.start;
			}

			if (range.end.isAfter(self.endDateFetchedEvents)) {
				where = "&where=((" + filterStartProp + " GT @" + self.endDateFetchedEvents.format('YYYY-MM-DD') + "@) and (" + filterStartProp + " LE @" + range.end.format('YYYY-MM-DD') + "@))";
				self.endDateFetchedEvents = range.end;
			}

		} else {
			where = "&where=((" + filterStartProp + " GE @" + range.start.format('YYYY-MM-DD') + "@) and (" + filterStartProp + " LE @" + range.end.format('YYYY-MM-DD') + "@))";
			self.startDateFetchedEvents = range.start;
			self.endDateFetchedEvents = range.end;
			append = false;
		}

		var url = representationUrl + where;
		var httpQuery = syra_controller.parseUrl(url);
		httpQuery.sendNewRequest({
			method: "GET",
			onSuccess: function(data, response) {
				self._setLandingPageCalendarDataBind(self.$prototype.$item.$properties, data, data, null, append);
			},
	...
	_setLandingPageCalendarDataBind: function($bindings, dataRecordSet, parentDataRecord, metaData, append) {
		var self = this;
