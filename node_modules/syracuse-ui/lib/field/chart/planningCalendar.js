...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
		...
		var calendar = self._widget.fullCalendar({
			...
			eventMouseover: function(event, jsEvent, view) {
				this.showTimeOut = setTimeout(function() {
					...
					if (self.dataset.$calendars[0] && self.dataset.$calendars[0].$categoryLegend) {
						var toolTip = self.dataset.$calendars[0].$categoryLegend.legend[event.category];
	...
	_setLandingPageCalendarDataBind: function($bindings, dataRecordSet, parentDataRecord, metaData) {
		...
		if (parentDataRecord && parentDataRecord.agenda) {
			...
			if (parentDataRecord.agenda.$calendars[0] && parentDataRecord.agenda.$calendars[0].$categoryLegend) {
				self._addCalendarLegend(parentDataRecord.agenda.$calendars[0].$categoryLegend.legend);
			}
			...
			$events.forEach(function(event) {
				...
				fcEvent.id = event.id;
				fcEvent.category = event.category;
