...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
		...
		if (this.calendar) {
			...
			this.resizeTimer = setTimeout(function() {
				if (self.calendar) {
					...
					{
						debug && self.calendar && console.log("PlanningCalendar.resizeTimer " + self.id + ": setSize(" + size.width + ")");
	...
	__setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		this.planningSlotsPerCell = $bindings.SLOT.length;
		...
		if (dataRecordSet) {
			...
			this.calendarYear = this._events[5].start;
		}

		var view = this.$prototype.$decorator.$style;
		if (view == 'year') {
			this.defaultView = 'planningYear';
		}
		else {
			this.defaultView = 'planningMonth';
		}

		this.planningDayTypes = [];
		this.$prototype.$decorator.$events && 
			this.$prototype.$decorator.$events.forEach(function(dayType) {
