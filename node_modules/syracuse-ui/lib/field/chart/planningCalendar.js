	...
	_createLandingPageCalendarWidget: function(calendarType) {
		...
		var calendar = self._widget.fullCalendar({
			...
			viewRender: function() {
				...
				$('.fc-button.fc-agendaWeek-button.fc-state-default', $('.fc-right')).prepend('<span class="fc-tab-icon s-fonticon-btn">' + syra_menus.fontIcons.time + '</span>');

				$('.fc-button.fc-agendaDay-button.fc-state-default', $('.fc-right')).text(syra_local.plcal_day);
				$('.fc-button.fc-agendaDay-button.fc-state-default', $('.fc-right')).prepend('<span class="fc-tab-icon s-fonticon-btn">' + syra_menus.fontIcons.time + '</span>');
		...
		if (parentDataRecord && parentDataRecord.agenda) {
			...
			$events.forEach(function(event) {
				...
				if (!(event.start.datetime && event.start.datetime.length)) {
					...
					fcEvent.actualEnd = moment(fcEvent.start);
				...
				if (!(event.end.datetime && event.end.datetime.length)) {
					...
					if (startDate > endDate) {
						...
						if (startHour === 0 && startMinutes === 0 && endHour === 0 && endMinutes === 0) {
							...
							fcEvent.actualEnd = moment(fcEvent.end);
							fcEvent.allDay = true;
						} else {
							fcEvent.start = event.start.datetime;
							fcEvent.actualEnd = moment(event.end.datetime);
