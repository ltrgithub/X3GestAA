...
function _toggleEventTip(calEvent, jsEvent) {
	if (!calEvent) {
		...
		["start", "end"].forEach(function(key) {
			...
			val.textContent = datetime.parse(dt._i, dt.f).toString(syra_site.userProfile.getDatetimeFormat());
...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
	...
	_resize: function(width) {
		if (this._widget) {
			...
			this.calendar && (this._widget.planningFullCalendar('option', 'aspectRatio', aspectRatio) || this._widget.fullCalendar('option', 'aspectRatio', aspectRatio));
		}
	},
	...
	_setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if ($bindings) {
			...
			$bindings = self.$prototype.$item.$properties;
			self._setLandingPageCalendarDataBind($bindings, dataRecordSet, parentDataRecord, metaData);
		}
