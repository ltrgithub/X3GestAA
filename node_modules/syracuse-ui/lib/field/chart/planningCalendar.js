...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
	...
	_eventPreceedsView: function(event, view) {
		if (view.name == "month") {
			/*
			 * Is the event startdate is before the first date of the visible view?
			 */
	...
	_eventExceedsView: function(event, view) {
		if (view.name == "month") {
			/*
			 * Is the event end date beyond the last date of the visible view?
			 */
	...
	_addPreviousArrow: function(event, element) {
		var previousArrowDiv = document.createElement("span");
		var iconSpan = document.createElement("span");
		iconSpan.className = "fc-tab-icon s-fonticon-btn";
		iconSpan.innerHTML = syra_menus.fontIcons.left_arrow;
		previousArrowDiv.appendChild(iconSpan);

		var endEvent = event.end ? event.end : event.actualEnd;
		var previousArrowText = datetime.parse(endEvent._i, endEvent.f).toString(formatApi.getLocalFormat("application/x-date")); // + "Z").slice(0, -1);
		var arrowTextSpan = document.createTextNode(syra_local["plcal_start"] + " " + previousArrowText);
		previousArrowDiv.appendChild(arrowTextSpan);

		previousArrowDiv.className = "s-previous-arrow";
		element[0].firstChild.insertBefore(previousArrowDiv, element[0].firstChild.firstChild);
	},

	_addNextArrow: function(event, element) {
		var endEvent = event.end ? event.end : event.actualEnd;
		var nextArrowText = datetime.parse(endEvent._i, endEvent.f).toString(formatApi.getLocalFormat("application/x-date")); // + "Z").slice(0, -1);
		var arrowTextSpan = document.createTextNode(syra_local["plcal_end"] + " " + nextArrowText);
		var nextArrowDiv = document.createElement("span");
		nextArrowDiv.appendChild(arrowTextSpan);

		var iconSpan = document.createElement("span");
		iconSpan.className = "fc-tab-icon s-fonticon-btn";
		iconSpan.innerHTML = syra_menus.fontIcons.right_arrow;
		nextArrowDiv.appendChild(iconSpan);

		nextArrowDiv.className = "s-next-arrow";
		element[0].appendChild(nextArrowDiv);
	},

	_addFontIconText: function(text, css, fontIcon) {
		var item = document.createElement("div");
		item.className = css;
	...
	_setLandingPageCalendarDataBind: function($bindings, dataRecordSet, parentDataRecord, metaData) {
		...
		if (parentDataRecord && parentDataRecord.agenda) {
			...
			// if (fcTestEvent) {
			// 	fcTestEvent.start = "2015-04-21T00:00:00.000Z";
			// 	fcTestEvent.end = "2015-05-15T00:00:00.000Z"
			// }
