...
function _toggleEventTip(calEvent, jsEvent) {
	if (!calEvent) {
		...
		var css = jsEvent.target.parentNode.className;
		if (css.indexOf("fc-time") >= 0 || css.indexOf("fc-title") >= 0) {
			_tip.target = _tip.target.parentNode;
		}
		...
		if (calEvent.description) {
			...
			div.appendChild(_getPropertyDescription(calEvent));
			_tip.slot.appendChild(div);
		}
...
function _getPropertyDescription(calEvent) {
	var descriptionKeys = Object.keys(calEvent.description);
	...
	for (var ii = 0, jj = descriptionKeys.length; ii < jj; ii++) {
		...
		var item;
		var hyperlink = calEvent.description[descriptionKey] && calEvent.$bindings[descriptionKey].$links && calEvent.$bindings[descriptionKey].$links.$details && calEvent.$bindings[descriptionKey].$links.$details;
		if (hyperlink) {
			item = syra_menus.button.add({
				parent: calEvent.calendar,
				text: calEvent.description[descriptionKey],
			...
			syra_url.history.setHref({
				$url: syra_expression.parse(calEvent.calendar, hyperlink.$url, calEvent.description),
				domItem: item
			});
			...
			var itemField = calEvent.calendar.page.loadNewItem(item, {
				$bind: descriptionKey,
				$inplace: true
			}, calEvent.calendar);
	...
	calEvent.calendar.applyChange(calEvent.description);
	return slot;
}

function onPlanningEventHover(event, jsEvent) {
	// var self = this;
	...
	if (!event) {
		...
		_tip.slot.className = "pfc-planning-tip";

		var date = datetime.parse(event.start._i).toString(formatApi.getLocalFormat("application/x-date"));
		var dateDiv = document.createElement("div");
		dateDiv.className = "pfc-planning-tip-date";
		dateDiv.textContent = date;
		_tip.slot.appendChild(dateDiv);

		_moment.lang(formatApi.getLocale().currentLangCode);
		var weekNum = _moment(event.start._i).week();

		var weekDiv = document.createElement("div");
		weekDiv.className = "pfc-planning-tip-week";
		weekDiv.textContent = syra_local["plcal_week"] + " " + weekNum;
		_tip.slot.appendChild(weekDiv);
...
exports.PlanningCalendar = helpers.defineClass(PlanningCalendar, Article, {
			...
			self.resizeTimer = setTimeout(function() {
				if (self.calendar) {
					if (self.layoutSlot.clientWidth) {
						...
						var calWidth = self.calendar.width();
	...
	_createLandingPageCalendarWidget: function(calendarType) {
		...
		var calendar = self._widget.fullCalendar({
			...
			eventAfterRender: function(event, element, view) {
				...
				var childNode = element[0].firstChild;
				...
				if (self._eventPreceedsView(event, view)) {
					view.segEach(function(seg) {
						if ((seg.row == 0) && seg.el == element) {
							var previousArrowDiv = self._addPreviousArrow(event, element, view);
							previousArrowDiv.appendChild(infoDiv);
						}
					}, event);
				}
	...
	_createPlanningCalendarWidget: function(calendarType) {
		...
		var calendar = self._widget.planningFullCalendar({
			...
			eventMouseover: function(event, jsEvent, view) {
				onPlanningEventHover(event, jsEvent);
				return false;
			},
	...
	_preSetDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		return true; //!(parentDataRecord && parentDataRecord.$isPartialDelta);
	},
	_setLandingPageCalendarDataBind: function($bindings, dataRecordSet, parentDataRecord, metaData, append, isOverlapping) {
		...
		if (parentDataRecord && parentDataRecord.agenda) {
			...
			$events.forEach(function(event) {
				...
				fcEvent.$bindings = $bindings;
				fcEvent.calendar = self;
				fcEvent.id = event.id;
