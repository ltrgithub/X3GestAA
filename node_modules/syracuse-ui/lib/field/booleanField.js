...
exports.BooleanField = helpers.defineClass(BooleanField, Field, {
    ...
    notifyFieldChange: function(newValue){
        ...
        if (this.$isEditMode && this._input) {
            var boolValue = (newValue || this.statusValues.off) == this.statusValues.on;
            this._input.setAttribute("data-s-selected", newValue);
        }
    },
    ...
    setDataValue: function(value){
        ...
        if (this.$isEditMode) {
            $(this._input).attr("checked", boolValue);
            this._input.setAttribute("data-s-selected", boolValue);
        }
        else {
            document.site.dom.toggleClass(this.fieldValue, "s-on", boolValue);
    ...
    _onSetReadOnly: function($isReadOnly){
        if (this._input) {
            if (this.$isReadOnly) {
                ...
                if (this.domTitle) {
                    document.site.dom.toggleClass(this.domTitle, "s-disabled", true);
                }
            }
            else {
                if (!this.$isDisabled) {
                    ...
                    if (this.domTitle) {
                        document.site.dom.toggleClass(this.domTitle, "s-disabled");
                    }
                }
            }
        }
