		...
		field.setValue = function(value, metaData) {
			if (value !== undefined) {
				...
				if (this.$isEditMode) {
					...
					this.displayText = value;
					if (value) {
						var isCtiEnabled = false; // temporary until we get the CTI status in the user profile.
						this._dataValue.textContent = value;
						if (!this._iconFlag) {
						...
						if (isCtiEnabled) {
							this._dataValue.syraOnClick = "onPhoneClick";
						} else {
		...
		field.onPhoneClick = function() {
			this.$menusPopup = {
				...
				$cti: {
					...
					customHref: "cti:" + this.currentValue
				}
			};
			syra_menus.open({
				scope: this,
