...
exports.ChildField = helpers.defineClass(ChildField, Article, {
	...
	getLocalizedTitle: function($title) {
		if ($title && $title.length > 0 && $title[1] == "@") {
			return syra_site.expressionMaker.parse(this.boxParent, $title);
		}
		return $title;
	},
	...
	setMenus: function($menus) {
		if ($menus) {
			this.record.applyActionLinkChange({
				$links: $menus.$links,
	...
	applyActionLinkChange: function(resources) {
		this.record.applyActionLinkChange(resources);
	},
	...
	dispose: function() {
		...
		this.record = this.ChildRecordClass = null;
