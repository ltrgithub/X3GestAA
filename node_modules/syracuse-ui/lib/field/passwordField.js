...
exports.PasswordField = helpers.defineClass(PasswordField, Field, {
	createHtmlInput: function() {
		...
		input.setAttribute("placeholder", this.getLocalize().pw_newPassword);
		return input;
	},
	...
	validateType: function(errors, value) {
		if (this.confirmInput && (this.getInputValue() != this.confirmInput.value)) {
			if (this.confirmInput.value) errors.push(this.getLocalize().pw_confirmPasswordError);
			else errors.push(this.getLocalize().pw_enterConfirmationError);
		} else {
			// format value
			...
			if (salt) {
				...
				if (r) {
					errors.push(locale.format(self.getLocalize().pw_Illegal, r[0]));
				}
				salt = salt.replace(/\{(\w+)\}/g, function(d0, d1) {
					try {
						return self.articleParent.boundFields[d1][0].getDataValue();
					} catch (e) {
						errors.push(locale.format(self.getLocalize().pw_replFieldNotFound, d1));
					}
				});
			}
			if (errors.length === 0) {
