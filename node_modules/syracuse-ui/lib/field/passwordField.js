...
exports.PasswordField = helpers.defineClass(PasswordField, Field, {
	...
	validateType: function(errors, value) {
		if (this.confirmInput && (this.getInputValue() != this.confirmInput.value)) {
			if (this.confirmInput.value) errors.push(this.localize.fpwConfirmPasswordError);
			else errors.push(this.localize.fpwEnterConfirmationError);
		} else {
			// format value
			...
			if (salt) {
				...
				salt = salt.replace(/\{(\w+)\}/g, function(d0, d1) {
					try {
						return self.articleParent.boundFields[d1][0].getDataValue().toLowerCase();
					} catch (e) {
