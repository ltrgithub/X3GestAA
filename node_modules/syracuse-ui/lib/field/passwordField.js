...
exports.PasswordField = helpers.defineClass(PasswordField, Field, {
	...
	render: function() {
		...
		if (this.$isEditMode) {
			...
			if (!this.$item.$isConfirmDisabled && this.$field.$capabilities && (this.$field.$capabilities.indexOf("confirm") >= 0)) {
				var div = syra_dom.div(this._$cssField + "-value-edit s-field-pwd-conf");
				this.confirmInput = syra_dom.input(this.$skinInput, div, this.$inputType);
				this.confirmInput.syraOnClick = "onFieldInputEvent";
	...
	setState: function(state) {
		...
		if (this.confirmInput && state.$isDisabled !== undefined) {
			syra_dom.disable(this.confirmInput, state.$isDisabled);
		}
	},
		...
		if (value !== undefined) {
			...
			if (salt) {
				salt = salt.replace(/\{(\w+)\}/g, function(d0, d1) {
					try {
						return self.articleParent.boundFields[d1][0].getValue();
					} catch (e) {
