...
exports.PasswordField = helpers.defineClass(PasswordField, Field, {
	createHtmlInput: function() {
		...
		input.setAttribute("placeholder", this.getLocalize().fpwNewPassword);
		return input;
	},
	...
	render: function() {
		...
		if (this.$isEditMode) {
			if (!this.$item.$isConfirmDisabled && this.$field.$capabilities && (this.$field.$capabilities.indexOf("confirm") >= 0)) {
				...
				this.confirmInput.setAttribute("placeholder", this.getLocalize().fpwConfirmPassword);
	...
	onInputValidate: function(event) {
		...
		if (this.confirmInput && (this.getInputValue() != this.confirmInput.value)) {
	...
	validateType: function(errors, value) {
		if (this.confirmInput && (this.getInputValue() != this.confirmInput.value)) {
			if (this.confirmInput.value) errors.push(this.getLocalize().fpwConfirmPasswordError);
			else errors.push(this.getLocalize().fpwEnterConfirmationError);
		} else {
			// format value
			...
			if (salt) {
				var r = /[^ -\x7E\xA0-\xFF]/.exec(this.getInputValue()); // when you change regexp, also change it in newPassword.html
				if (r) {
					errors.push(locale.format(self.getLocalize().fpwIllegal, r[0]));
				}
				salt = salt.replace(/\{(\w+)\}/g, function(d0, d1) {
					try {
						return self.articleParent.boundFields[d1][0].getDataValue();
					} catch (e) {
						errors.push(locale.format(self.getLocalize().fpwReplFieldNotFound, d1));
					}
				});
			}
			if (errors.length === 0) {
				var md5result = salt ? md5(salt + ":" + this.getInputValue()) : this.getInputValue();
				this.confirmValue = this.currentValue = md5result;
			}
		}
	}
});