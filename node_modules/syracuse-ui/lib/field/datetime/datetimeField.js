...
var _formatter = {
	"application/x-date": function(field, val) {
		...
		var twoDigitYearMax = syra_culture.format.getTwoDigitYearMax().toString();
...
function _onPickerClick(options) {
	...
	if (!field.popupPicker) {
		...
		field.popupPicker = syra_over.openPopup(field.boxParent, {
			...
			position: field.$item.$popupRight ? {
				my: "left top",
				at: "right top",
				of: field.domItem
			} : {
...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
	...
	initialize: function() {
		...
		this.formatApi = syra_culture.api[this.$field.$type];
		this.localFormat = syra_culture.format.get(this.$field.$type);
		this._formatInput = _formatter[this.$field.$type];
		this.localFormatDisplay = this.$field.$localFormatDisplay || syra_culture.format.get(this.$field.$type, this.$field.$format);
		this.$field.$maxLength = Math.max(this.$field.$maxLength || 0, syra_culture.format.getLength(this.localFormatDisplay));
		switch (this.$field.$type) {
			case "application/x-time":
				this.isTimeField = true;
				break;
			case "application/x-date":
				this.formatObj = syra_culture.processFormat(this.localFormat);
				break;
		}
	...
	parseMask: function(val) {
		...
		if (dayOffset != null) {
			...
			if (!this.page.externalAdapter.getFormatedValFromHelperVal) {
				newVal = dateObj.toString(this.localFormat || syra_culture.format.get(this.$field.$type));
			} else {
					...
					if (val) {
						...
						if (val) {
							try {
								...
								if (this.$field.$type == "application/x-datetime") {
									val = this.formatApi.parse(val, syra_culture.format.date_short).toString();
								} else {
									throw error;
								}
							}
