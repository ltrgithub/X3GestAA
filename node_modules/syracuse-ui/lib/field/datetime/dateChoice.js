...
function _onQuickClick() {
	...
	if (!item.quickList) {
		...
		for (var ii = 0, jj = intervals.length; ii < jj; ii++) {
			var row = syra_dom.addTextInfo("s-calendar-quick", syra_local["fdpIntervalOf" + intervals[ii]]);
...
exports.DateChoice = helpers.defineClass(DateChoice, null, {
	...
	create: function(field, options) {
		...
		if (options && (options.internalValue || options.internalValue === "")) {
			...
			try {
				var value = field.getDataValue(); //catch exception if format error
				if (value) {
					this._selectedDate = _dateApi.parse(field.getInputValue(), field.localFormat);
				}
			} catch (error) {
				field.showErrors([error.message]);
			}
			this._selectedDate = this._selectedDate || _dateApi.today();
			this._currentDate = _dateApi.fromInternalValue(this._selectedDate._value);
		}
		this.field = field;
	...
	_addHead: function() {
		...
		for (var ii = 0; ii < 7; ii++) {
			...
			cell.className = "s-calendar-week-day";
			cell.title = _dateApi.dayName(dayOfWeek);
			cell.textContent = _dateApi.dayName(dayOfWeek, true);
			row.appendChild(cell);
		}
		head.appendChild(row);
		return head;
