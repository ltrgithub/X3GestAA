...
function _onQuickClick() {
	...
	if (!item.quickList) {
		...
		for (var ii = 0, jj = intervals.length; ii < jj; ii++) {
			var row = syra_dom.addText("s-calendar-quick", syra_local["fdpIntervalOf" + intervals[ii]]);
...
function _onDayClick(event, btn) {
	this.parent.field.setButtonValue(syra_culture.date.fromInternalValue(this.value).toString(this.parent.field.localFormat), this.value);
}
...
exports.DateChoice = helpers.defineClass(DateChoice, null, {
	...
	create: function(field, options) {
		...
		if (options && (options.internalValue || options.internalValue === "")) {
			this._selectedDate = isNaN(options.internalValue) || options.internalValue === "" ? syra_culture.date.today() : syra_culture.date.fromInternalValue(options.internalValue);
			this._currentDate = syra_culture.date.fromInternalValue(this._selectedDate._value);
		} else {
			try {
	...
	_addHead: function() {
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "first_step",
			shortCutTip: syra_shortCuts.tip.movePrevYear,
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "prev_step",
			shortCutTip: syra_shortCuts.tip.movePrevMonth,
			click: _onChangePeriode,
			period: "month",
			isPrev: true
		});
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "next_step",
			shortCutTip: syra_shortCuts.tip.moveNextMonth,
			click: _onChangePeriode,
			period: "month"
		});
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "last_step",
			shortCutTip: syra_shortCuts.tip.moveNextYear,
			click: _onChangePeriode,
			period: "year"
		});
	...
	_appendFoot: function() {
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "prev_step",
			shortCutTip: syra_shortCuts.tip.movePrevWeek,
			click: _onChangePeriode,
			period: "week",
			isPrev: true
		});
		...
		syra_button.add({
			...
			iconOnly: true,
			fontIcon: "next_step",
			shortCutTip: syra_shortCuts.tip.moveNextWeek,
		...
		syra_button.add({
			...
			shortCutTip: syra_shortCuts.tip.moveToday,
			click: function() {
				var selector = this.parent;
				selector._currentDate = syra_culture.date.today();
				selector.field.setButtonValue(syra_culture.date.fromInternalValue(selector._currentDate._value).toString(selector.field.localFormat), selector._currentDate._value);
			}
		});
	...
	_drawBody: function() {
		...
		var month = syra_culture.date.monthName(curMonth);

		syra_button.setText(this._monthBtn, month);
		...
		for (var weekRow = 0; weekRow < 6; weekRow++) {
			...
			cell.textContent = weekDay.weekNumber(firstDayOfWeek);
