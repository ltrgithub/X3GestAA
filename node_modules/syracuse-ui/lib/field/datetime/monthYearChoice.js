...
function _onClick() {
	...
	if (this.isMonth) {
		var month = item._selectedValue ? item._selectedValue : item._dateValue.month;
		item._dateValue = _dateApi.fromJsDate(new Date(item._dateValue.year, month - 1, item._dateValue.day));
	} else {
		var year = item._selectedValue ? item._selectedValue : item._dateValue.year;
		item._dateValue = _dateApi.fromJsDate(new Date(year, item._dateValue.month, item._dateValue.day));
	}
	item._close(item._dateValue);
}
