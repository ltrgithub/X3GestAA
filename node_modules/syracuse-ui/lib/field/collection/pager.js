...
exports.Pager = helpers.defineClass(Pager, null, {
    create: function(list, container){
        ...
        self._$$item = $(container.appendChild(div));
    ...
    draw: function(){
        ...
        if (this.list.$totalResults) {
            ...
            if (pageTotal <= 1) {
                $$range.text(this.list.$totalResults + " " + localize.pager_totalRecords);
            }
            else {
                var lastRecordIndex = Math.min((this.list.$startIndex + this.list.$itemsPerPage) - 1, this.list.$totalResults);
                $$range.text(this.list.$startIndex + " - " + lastRecordIndex + " " + localize.pager_itemOf + " " + this.list.$totalResults);
                ...
                this._appendServerStep({
                    $bind: "$previous",
                    $title: "<" + localize.pager_prev
                });
                this._appendServerStep({
                    $bind: "$next",
                    $title: localize.pager_next + ">"
                });
            }
        }
        else {
