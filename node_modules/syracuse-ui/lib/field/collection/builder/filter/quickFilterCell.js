...
exports.QuickFilterCell = helpers.defineClass(QuickFilterCell, null, {
    addOperator: function(field){
        var localizeOperators = locale.resources(module)();
        var self = this;
        if (field.$$input) {
            field.$$input[0].className = field.$$input[0].className.replace("s-inplace-input", "s-filter-input");
        }
        ...
        $(field.$$dataValue[0].appendChild(self._operator)).bind("click", function(){
            if (!self.listPopup) {
                self.$$list.empty();
                var operators;
                ...
                operators.forEach(function(op){
                    ...
                    $(link).appendTo(self.$$list).text(localizeOperators["filter_" + op]);
                });
        ...
        if (!self.$$list) {
            ...
            self.$$list = $(field.$$dataValue[0].appendChild(list)).delegate("a.s-list-filter-item", "click", function(){
    ...
    loadField: function($field, $bind){
        this.addOperator(document.itemFactory.load(this.$$container, {
            $bind: $bind || this.$bind,
