...
exports.IconField = helpers.defineClass(IconField, Field, {
	...
	setState: function(state) {
		...
		if (state.$isDisabled !== undefined) {
			document.site.dom.disableItem(this.iconLink, this.$isDisabled);
		}
	},
	focus: function() {
		//Hack lv
	...
	_renderIcon: function(metaData) {
		...
		if (this.$links = metaData.$links) {
			document.site.dom.toggleClass(this.fieldValue, "s-icon-link", this.iconLink != null);
			this.iconLink.className = this.$skin + "-value-icon s-icon-link";
		} else {
			document.site.dom.toggleClass(this.fieldValue, this.$skin + "-icon", true);
