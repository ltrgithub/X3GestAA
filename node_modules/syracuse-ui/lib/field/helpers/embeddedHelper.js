...
exports.hasMasterChildRelation = function(field) {
	...
	if ($location && $location.$masterBinds) {
		field.masterKeys = $location.$masterBinds;
...
function _refreshChildRelation(article, vignetteField) {
	var refresh;
	var keys = Object.keys(vignetteField.masterKeys);
	for (var mm = 0, kk = keys.length; mm < kk; mm++) {
		var key = keys[mm];
		if (refresh || (article.dataset[key] != vignetteField.masterKeys[key])) {
			vignetteField.masterKeys[key] = article.dataset[key];
			refresh = true;
		}
	}
	if (refresh) {
