...
exports.checkbox = {
	render: function(item, id, css, isRadio) {
		...
		var slot = syra_dom.addDiv(css + " s-checkbox-input-slot");
...
exports.scrollToField = function(field) {
	if (!field._isScollToTieldDisabled) {
		if (field.articleParent.isRecordArticle && field.articleParent.scrollToRecordField) {
			field.articleParent.scrollToRecordField(field);
		} else {
...
exports.getDatasetValue = function(field) {
	if (field.variantItem) {
		field = field.variantItem;
	}
	if (field.getDatasetValue) {
		return field.getDatasetValue(value);
	}
...
exports.setDatasetValue = function(field, value) {
	if (field.variantItem) {
		field = field.variantItem;
	}
	...
	var value = field.currentValue = (value === undefined ? field.getDataValue() : value);
...
exports.setDescription = function(field, $description) {
	if ($description !== undefined) {
		...
		if (field.descriptionText.length > 0) {
			...
			if (field.descriptionItem) {
				syra_dom.removeChild(field.descriptionItem);
...
exports.addValueSlot = function(field) {
	...
	if (field.$item.$inplace) {
		...
		field._core = syra_dom.addDiv(field._$cssField + "-core" + _getCsBeside(field));
...
exports.setState = function(field, state) {
	...
	if (!field.$item.$isFilterMode) {
				...
				if (field.domTitle && field.getTitle().length > 0) {
					...
					if (state.$isMandatory) {
						if (!field._mandatoryFlag) {
							field._mandatoryFlag = syra_menus.button.add({
								isIndicator: true,
								parent: field,
								slot: field.domTitle,
								iconOnly: true,
						...
						syra_menus.button.remove(field._mandatoryFlag);
		...
		if (state.$isDisabled !== undefined) {
			...
			if (state.$isDisabled) {
				field.domItem.setAttribute("disabled", true);
			} else {
				field.domItem.removeAttribute("disabled");
			}
			if (field.input) {
				syra_dom.disableItem(field.input, state.$isDisabled);
				if (field.$item.$contentEditable) {
					_contentEditable.setState(field, state);
				}
			}
			field.domTitle && syra_dom.disableItem(field.domTitle, state.$isDisabled);
			field.descriptionItem && syra_dom.disableItem(field.descriptionItem, state.$isDisabled);
			syra_dom.disableItem(field._dataValue, state.$isDisabled);
		}
		...
		if (field._mandatoryFlag) {
			syra_menus.button.disable(field._mandatoryFlag, field.$isDisabled || field.$isReadOnly);
		}
...
exports.applyMetaData = function(field, metaData, options) {
	if (field._dataValue) { //important for reloading and initiliaze method called before render
		if (metaData) {
			...
			syra_menus.menus.applyChange(field, metaData);
...
exports.applyDesignMetaData = function(field, metaData, designing) {
	if (field.variantItem) {
		field = field.variantItem;
	}
...
exports.setTitle = function(field, $title) {
	...
	if (field.domTitle) {
		if (field._mandatoryFlag) {
			syra_dom.removeChild(field._mandatoryFlag.link);
