...
exports.setValueLength = function(field, length) {
	field.valueLength = length;
	if (!field.articleParent.mustBeResized) {
...
exports.getFieldWidth = function($field, useSmartMode, $titleLength, isSortable) {
	...
	if (useSmartMode) {
		...
		if (!widthValue) {
			$maxLength = Math.min(_smartWidth.medium, $maxLength || _smartWidth.medium);
			widthValue = (_smartWidth.charWidth * $maxLength);
		}
	...
	if (field.variantItem) {
		...
		if (newStyle !== undefined) {
			...
			if (field.page.isFusionPage) {
				...
				if (field.$valueStyle) {
					document.site.dom.toggleClass(target, field.$valueStyle, false);
				}
				document.site.dom.toggleClass(target, (field.$valueStyle = (newStyle || "").replace("cst_sty_", "s-cst-sty-")), true);
			}
		}
		...
		if (field.domTitle && newStyle !== undefined) {
			if (field.page.isFusionPage) {
				...
				if (field.$titleStyle) {
					document.site.dom.toggleClass(field.domTitle, field.$titleStyle, false);
				}
				document.site.dom.toggleClass(field.domTitle, (field.$titleStyle = (newStyle || "").replace("cst_sty_", "s-cst-sty-")), true);
			}
		}
	}
};
