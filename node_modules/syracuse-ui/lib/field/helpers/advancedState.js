...
function _ensureFieldVisibility(field) {
	if (field.variantItem) {
		...
		field.showItem(!field.$isHidden && (!field.boxParent.hasAdvancedFields || field.boxParent.advancedFieldsVisibled));
	}
}
...
function _showFields(box, show) {
	...
	if (box.isDesktopPage) {
		if (!box.isVignettePage) {
			...
			syra_menus.button.setText(box.advancedFieldsBtn, title, icon);
		}
	} else {
		if (box.vignette) {
	...
	box.isDesktopPage && syra_site.ensureArticleVisibility(box);
}
	...
	if (page.isDesktopPage) {
		...
		if (page.isVignettePage) {
			...
			if (!page.advancedFieldsBtn) {
				page.advancedFieldsBtn = syra_menus.button.add({
					parent: page,
					slot: page.header,
					text: title,
				...
				syra_menus.button.setText(page.advancedFieldsBtn, title);
			}
			syra_menus.button.setText(page.advancedFieldsBtn, title, icon);
			syra_menus.button.hide(page.advancedFieldsBtn, !page.hasAdvancedFields);
		}
	} else {
		if (!show) {
...
exports.clearPageButton = function(page) {
	if (page.advancedFieldsBtn) {
		...
		syra_menus.button.remove(page.advancedFieldsBtn);
