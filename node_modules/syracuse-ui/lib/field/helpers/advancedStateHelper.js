...
function _showFields(box, show) {
	...
	if (box.isDesktopPage) {
		...
		var picker = box.btns && box.btns.items && box.btns.items.advanced;
		if (picker) {
			syra_menus.setButtonTitle(picker, (box._areAdvancedFieldsVisible) ? syra_local.box_advanced_hide : syra_local.box_advanced);
	...
	if (box.isDesktopPage) {
		...
		if (box.isVignettePage) {
			...
			if (!box._advancedFieldsSlot) {
				...
				box._advancedFieldsBtn = syra_menus.addTextButton(title, box.$skin + "-head-advanced-fields-link", "advanced");
				box._advancedFieldsSlot.appendChild(box._advancedFieldsBtn);
			} else {
...
exports.showButton = function(box, show, designing) {
	_showButton(box, show, designing);
};
