...
function _showFields(box, show) {
	...
	if (box.isDesktopPage) {
		if (!box.isVignettePage) {
			...
			syra_menus.updateButtonIcon(box._advancedFieldsBtn, title, icon);
		}
	} else {
		if (box.vignette) {
	...
	if (box.isDesktopPage) {
		...
		if (box.isVignettePage) {
			...
			if (!box._advancedFieldsBtn) {
				box._advancedFieldsBtn = syra_menus.addTextButton(title, "s-page-advanced-fields-link", "onAdvancedClick", null, icon);
				box.header.appendChild(box._advancedFieldsBtn);
			} else {
