...
function _showFields(box, show) {
	...
	if (box.isDesktopPage) {
		if (!box.isVignettePage) {
			...
			document.site.toggleClass(box._advancedFieldsLink, "s-hide-advanced", box._areAdvancedFieldsVisible);
		}
	} else {
		if (box.vignette) {
		...
		if (picker) {
			...
			document.site.toggleClass(picker, "s-hide-advanced", box._areAdvancedFieldsVisible);
		}
	}
	...
	if (box.isDesktopPage) {
		...
		if (box.isVignettePage) {
			...
			if (!box._advancedFieldsSlot) {
				...
				box._advancedFieldsLink.setAttribute("data-s-picker", "box-picker");
				box._advancedFieldsLink.syraBoxPickerId = "advanced";
				box._advancedFieldsLink.syraBoxId = box.id;
...
exports.onClickPicker = function(box) {
...
exports.setAdvancedState = function(field, $isAdvanced, designing) {
	if (field.variantItem) {
		...
		if (designing) {
			if (field.page.designer.uiSwitchTool) {
				field.page.designer.uiSwitchTool.showAdvancedItem(field, $isAdvanced);
			}
		}
	}
};
