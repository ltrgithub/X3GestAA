...
helpers.defineClass(TitleDiagnose, null, {
    addSeverityCount: function(severity){
        var self = this;
        // add severity count
        if (!self.severities[severity].count) {
            self.severities[severity].count = 1;
        }
        else {
            self.severities[severity].count++;
        }
    },
    ...
    removeDiagnoseIcon: function(){
        // only if count = 0
    },
    dispose: function(){
        delete this.severities;
    }
});
