...
exports.drawDiagnose = function(field, $diagnose, severityGroup){
    ...
    if (field.isDiagnoseInPlace) {
        if (!field._diagnoseFlag) {
            ...
            field._addButtonToValue(field._diagnoseFlag);
        }
    ...
    link.setAttribute("data-s-article", field.boxParent.getArticle().id);
...
exports.ensureDiagnoseSlot = function(field){
    if (!field.domDiagnose) {
        ...
        if (field.$item.$inplace && article.ensureFieldDiagnoseSlot) {
            field.isDiagnoseInPlace = true;
            article.ensureFieldDiagnoseSlot(field);
        }
        else {
...
exports.toggleDiagnose = function(field, css, show){
    if (field.$$input) {
        document.site.toggleClass(field.$$input[0], css, show);
    }
    document.site.toggleClass(field.$$dataValue[0], css, show);
};
