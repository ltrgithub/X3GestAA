...
exports.applyFieldStyle = function(field, metaData){
    ...
    if (metaData.$fieldStyle !== undefined || metaData.$valueStyle !== undefined) {
        ...
        if (field.page.$isFusionPage) {
            if (field.$valueStyle) {
                var styles = field.$valueStyle.split(";");
                for (var ii = 0, jj = styles.length; ii < jj; ii++) {
                    var style = styles[ii];
                    if (style) {
                        ...
                        if (parts.length > 0) {
                            ...
                            for (var mm = 1, kk = subKeys.length; mm < kk; mm++) {
                                subKeys[mm] = subKeys[mm].charAt(0).toUpperCase() + subKeys[mm].substr(1);
                            }
                            target.style[subKeys.join("")] = "";
                        }
                    }
                }                
            }
