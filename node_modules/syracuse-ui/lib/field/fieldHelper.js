...
exports.drawDiagnose = function(field, $diagnose, severityGroup){
    ...
    if (field.isDiagnoseInPlace) {
        ...
        if (!severityGroup.$$group.children().children("a[data-s-menu='" + field.id + "']")[0]) {
...
function TitleDiagnose($$titleDom){
...
helpers.defineClass(TitleDiagnose, null, {
    ...
    appendDiagnoseIcon: function(severity, $isTab){
        ...
        // icon not yet appended.
            ...
            self.$hasIcon = true;
            self.prioritySeverity = severity;
        }
        
        // add severity count first
        // two possibilites 
    ...
    removeDiagnoseIcon: function(severity, $isTab){
        ...
        //        var prioritySeverity = self.getPrioritySeverity();
        //        if ((self.prioritySeverity != prioritySeverity) && (self.severitiesCount[prioritySeverity] > 0)) {
        //            self.$$titleDom.toggleClass(diagClassNameRoot + self.prioritySeverity).toggleClass(diagClassNameRoot + prioritySeverity);
        //            self.prioritySeverity = prioritySeverity;
        //        }
        //        else {
        //            if (self.severitiesCount) {
        //            
        //            }
        //        }
...
function drawBoxParentDiagnose(field, $diagnose){
    ...
    while (f.boxParent) {
        ...
        if (f.boxParent.$item.$isBoxCollapsable) {
            (f.boxParent._header.titleDiagnose = f.boxParent._header.titleDiagnose || new TitleDiagnose($(f.boxParent._header))).appendDiagnoseIcon($diagnose.severity);
        }
            ...
            if (f.boxParent._tabTitle) {
                //$(f.boxParent._tabTitle).toggleClass("s-tab-header-error");
                (f.boxParent._tabTitle.titleDiagnose = f.boxParent._tabTitle.titleDiagnose || new TitleDiagnose($(f.boxParent._tabTitle))).appendDiagnoseIcon($diagnose.severity, true);
            }
        f = f.boxParent;
    }
};
...
function emptyBoxParentDiagnose(field){
    ...
    while (f.boxParent) {
        ...
        //f.boxParent.$opened
        if (f.boxParent.$item.$isBoxCollapsable) {
            f.boxParent._header.titleDiagnose.removeDiagnoseIcon($diagnose.severity);
        }
            ...
            //f.boxParent.$isTabLayout
            if (f.boxParent._tabTitle) {
                f.boxParent._tabTitle.titleDiagnose.removeDiagnoseIcon($diagnose.severity, true);
            }
        f = f.boxParent;
    }
...
exports.emptyDiagnoseSlot = function(field){
    ...
    //TODO call emptyBoxParentDiagnose();
    // in the function, decrease count. if 0, toggleClass
