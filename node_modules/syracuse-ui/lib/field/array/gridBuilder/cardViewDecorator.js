...
exports.CardViewDecorator = helpers.defineClass(CardViewDecorator, null, {
    initialize: function(builder){
        ...
        $item.$cards = (typeof($item.$cards) === "object") ? $item.$cards : {
            $fieldsWidth: "largest",
            $layout: {
                $items: Object.keys(self.list.$fieldProperties).map(function($bind){
                    return {
                        $bind: $bind
                    };
                })
    ...
    onRecordDrawBox: function(recordArticle){
        if (this.$cardsPosition == "inline") {
            ...
            th.className = this.builder.gridCss.cell + " s-list-cards-cell";
    ...
    _bindOpener: function(){
        var self = this;
        self.list.$$item.delegate("a." + self.list.$skin + "-cardview-opener", "click.cardviewdecorator", function(){
            var $$opener = $(this);
            var show = $$opener.hasClass("s-close");
            var $uuid = $$opener.closest("[data-s-record]").attr("data-s-record");
            var $uuidList = ($uuid) ? [$uuid] : Object.keys(self.list._store._map);
            $uuidList.forEach(function($uuid){
                self._toggleRecordCardview(self.list._store._map[$uuid], show);
            });
            document.site.toggleClass(this, "s-close", !show);
            /*setTimeout(function(){
             debugger;
             self.list.builder.onWindowResize(true);
             }, 5);*/
        });
    },
    _toggleRecordCardview: function(recordArticle, show){
        ...
        if (!recordArticle.$$cardsSlot) {
            ...
            td.className = recordArticle.boxParent.$skin + "-cards-cell";
            td.setAttribute("colspan", recordArticle.boxParent.builder._orderCols.length);
            var tr = document.createElement("tr");
            tr.setAttribute("data-s-record", recordArticle.$uuid);
            var $$cell = $(td).appendTo(recordArticle.$$cardsSlot = $(tr).insertAfter(recordArticle.$$item));
    ...
    _createCardView: function($$container, recordArticle){
        ...
        recordArticle.bindFieldEvents($$container);
    },
    ...
    dispose: function(){
        if (this.list && this.list.$$item) {
            this.list.$$item.undelegate(".cardviewdecorator");
        }
        delete this.list;
        delete this.cardview;
        delete this.builder;
