...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	removeInlineCard: function() {
		...
		if (col) {
			...
			for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
				...
				if (record.inlineCardItem) {
					record.removeItem(record.inlineCardItem, true, true);
				}
				record.inlineCardOpener = record.inlineCardCell = record.inlineCardRow = record.inlineCardItem = null;
				document.site.removeDomChild(record.dataRow.childNodes[index]);
			}
			delete this.cols.$inlineCard;
			this.$inlineCard = this.list.$item.$inlineCard = this.$inlineFieldBinds = null;
		}
	},
	appendInlineCard: function(designing) {
		...
		if (this.$inlineCard) {
			this.$inlineFieldBinds = this.list.page.getDefinedFieldBinds(this.$inlineCard.$layout ? this.$inlineCard.$layout.$items : this.$inlineCard.$items);
		}
		if (designing) {
			...
			if (metaData.$cardPosition !== undefined) {
				if (metaData.$cardPosition == "no" || metaData.$cardPosition == "inline") {
					...
					if (metaData.$cardPosition == "inline") {
						...
						this.list.$item.$inlineCard = $cards;
