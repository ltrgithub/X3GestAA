...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    _setBodyHeight: function(){
        if (this._bodySlot) {
            var fixedHeight = parseInt(this.list.$item.$fixedHeight, 10);
            if (fixedHeight) {
                this._bodySlot.style.overflowY = "auto";
                this._bodySlot.style.height = Math.max(20, fixedHeight) + "px";
            }
            else {
    ...
    applyDesignMetaData: function(metaData, onAuthoring){
        if (metaData.$fixedHeight !== undefined) {
            if (!metaData.$fixedHeight) {
                delete this.list.$item.$fixedHeight;
            }
            this.list.$item.$fixedHeight = metaData.$fixedHeight;
            this._setBodyHeight();
            if (onAuthoring) {
                this.onWindowResize();
            }
    ...
    _toggleDataList: function(show){
        if (this._tableBody) {
            ...
            this._orderCols = [];
            this.cols = {};
            this._$$headSlot = $(this._headSlot = document.createElement("div"));
            ...
            //this._$$trace = $(document.createElement("div"));
            this._$$hscroller = $(this._hscroller = document.createElement("div"));
            ...
            // this.list._core.appendChild(this._$$trace[0]);
            
            
            this.list._store.setSortCapability(true);
            
            this._body = this._bodyTable.appendChild(document.createElement("tbody"));
    ...
    _initializeSize: function(initialize){
        var self = this;
        // if (initialize) {
        delete self._maxCol;
        if (self.list && self.list.$item.$format == "grid") {
            if (!self._maxCol && self._bodySlot.clientWidth) {
                ...
                if (self.widthTotal < (self._bodySlot.clientWidth)) {
                    //self._maxCol.groupHead.style.width = self._maxCol.groupBody.style.width = ((self._bodySlot.clientWidth - self.widthTotal) + self._maxCol.widthValue) + "px";
    ...
    onWindowResize: function(initialize){
        ...
        setTimeout(function(){
            ...
            self._initializeSize(initialize);
            self._ensureScrollBar();
        }, 2);
        
    },
    _ensureScrollBar: function(){
        /*var txt = "SH:" + this._bodySlot.scrollHeight + " CH:" + this._bodySlot.clientHeight + " " + $(this._bodySlot).height();
         txt += " SW:" + this._bodySlot.scrollWidth + " CW:" + this._bodySlot.clientWidth + " " + $(this._bodySlot).width();
         this._$$trace.text(txt);*/
        if (this._bodySlot.scrollHeight > (this._bodySlot.clientHeight + 1)) {
            this._headSlot.style.marginRight = "17px";
        }
        else {
            this._headSlot.style.marginRight = "0px"
        }
    ...
    _bindScrollEvent: function(bind){
        ...
        if (bind) {
            self._$$hscroller.bind("scroll", function(event){
                var left = self._$$hscroller.scrollLeft();
                self._$$headSlot.scrollLeft(left);
                self._$$bodySlot.scrollLeft(left);
            });
        }
        else {
            if (self._$$hscroller) {
                self._$$hscroller.unbind("scroll");
            }
        }
    },
    ...
    _makeTitleRow: function(){
        ...
        if (self.list.$item.$format == "vgrid") {
            var $$colgroup = $(self._bodyColgroup).detach();
            ...
            $$colgroup.empty();
            var col = self._bodyColgroup.appendChild(document.createElement("col"));
            ...
            $$colgroup.prependTo(self._bodyTable);
        }
        else {
            if (self.titleRow) {
                $(self.titleRow).empty();
    ...
    dispose: function(){
        if (this.list && this.list._store && this.list._store.filterCapability) {
            this.list._store.filterCapability.removeQuickFilter();
        }
