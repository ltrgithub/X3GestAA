...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    emptyBody: function(addEmptySlot){
        ...
        if (addEmptySlot) {
            var cell = this.list.helper.createEmptyDataItem(this.list, "td");
    ...
    _bindScrollEvent: function(bind){
        ...
        if (bind) {
            ...
            self._$$bodySlot.bind("scroll", function(event){
                ...
                if (!self.lastRecord) {
                    self.firstRecord = self.list.records[0].$$item;
                    self.lastRecord = self.list.records[self.list.records.length - 1].$$item;
                    
                }
                if (!self.middleRecord && self.list.records[28]) {
                    self.middleRecord = self.list.records[28].$$item;
                }
                var offset = self._$$bodySlot.offset();
                console.log("F " + (self.firstRecord.offset().top - offset.top) + " --L " + (self.lastRecord.offset().top - offset.top) + " --M " + (self.middleRecord ? (self.middleRecord.offset().top - offset.top) : 0) + " top" + self._$$bodySlot[0].scrollTop + "   scrollHeight " + self._$$bodySlot[0].scrollHeight + "   clientHeight " + self._$$bodySlot[0].clientHeight);
            });
