...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ensureSendBag: function(){
        var page = this.list.getPage();
        if (this.sendBag && page.dataset.$etag != this.sendBag.$etag) {
            delete this.sendBag;
        }
        if (!this.sendBag) {
            this.sendBag = {
                $etag: page.dataset.$etag,
                recordDataset: []
            };
        }
        return this.sendBag;
    ...
    onNotifyRecordChange: function(value, binding){
        ...
        if (page.$isNewCollectionEdit) {
            if (binding == "$actions" && (value.$create || value.$delete)) {
                if (value.$create) {
                    self.ensureSendBag();
                    var newRecord = self.list._store.appendRecord({
                        dataRecord: {
                    ...
                    self.sendBag.recordDataset.push(newRecord.dataset);
                    document.controller.notifyChange(self.list.getArticleParent(), self.list.$item.$bind, self.sendBag.recordDataset);
                    return null;
                }
                if (value.$delete) {
                    self.ensureSendBag();
                    self.sendBag.recordDataset = [];
                    document.controller.notifyChange(self.list.getArticleParent(), self.list.$item.$bind, self.sendBag.recordDataset);
                }
                return null;
            }
    ...
    _toggleDataList: function(show){
        if (this._table) {
            ...
            (this.list._tableSlot || this.list._core).appendChild(this._table);
    ...
    appendFieldCol: function(slot, $item, $field){
        ...
        if (width) {
            ...
            var minWidth;
            if ($width.$charWidth) {
                widthValue = ($width.$charWidth * ($field.$size || $width.$size));
                if ($width.$minChar) {
    ...
    _makeTitleRow: function(){
        ...
        if (self.list.$item.$format == "vgrid") {
            ...
            document.site.emptyDom(self._colgroup);
    ...
    _appendRowIndexCol: function(slot){
        ...
        col.titleCell.className = this.gridCss.title;
        slot.appendChild(col.titleCell);
        col.group.style.maxWidth = col.group.style.minWidth = col.group.style.width = (this.list.$capability.reorder) ? "3.5em" : "2.5em";
        return col;
    },
    ...
    _appendRecordSelectorCol: function(slot){
        ...
        col.group.style.width = this.list._store.selector.getWidth();
        return col;
    },
        ...
        if (this.cardview) {
            if (this.cardview.$cardsPosition != "inline" && Object.keys(this.list_store.selector.records).length == 0) {
                if (this.list._store._records.length > 0) {
                    this.list_store.selector.select(this.list._store._records[0].$recordUuid, true);
                }
            }
        }
    },
