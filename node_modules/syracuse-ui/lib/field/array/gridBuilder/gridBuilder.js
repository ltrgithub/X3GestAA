...
function _addRowIndexCol(list) {
	if (list.$item.$isRowIndexVisible || list.$capability.reorder || list.$item.$selectByRowIndex) {
		list.filterRecord && list.filterRecord.appendCell();
...
function _addFieldCol(list, $item, $field) {
	...
	list.orderCols.push(col);
	list.filterRecord && list.filterRecord.appendFieldCell(col, $field);
	return col;
}
...
exports.addTitleRow = function(list) {
	...
	if (list.selector.useColumnSelector) {
		list.filterRecord && list.filterRecord.appendFlagCell();
	...
	list.filterRecord && list.filterRecord.checkVisibility();
...
exports.loadInlinePage = function(record, $itemPage, options) {
	...
	if (!record.quickEdit) {
		list.cardDecorator && list.cardDecorator.toggleRowCard(record, false);
		var cell;
		record.quickEdit = {
		...
		cell.className = record.boxParent.$skin + "-cards-cell";
		cell.setAttribute("colspan", list.scrollTable.cols.length);
		record.quickEdit.row.appendChild(cell).appendChild(record.quickEdit.contentSlot);
		record.dataRow.parentNode.insertBefore(record.quickEdit.row, record.dataRow.nextSibling);
		$itemPage.layoutSlot = record.quickEdit.contentSlot;
		syra_site.pageLoader.load($itemPage);
	}

};
exports.toggleQuickEdit = function(record) {
	if (!record.quickEdit) {
		var $quickEdit = record.$menus.$quickEdit = helpers.object.clone(record.$menus.$edit, true);
		$quickEdit.$target = "inline";
		$quickEdit.$bind = $quickEdit.$sourceBind = "$quickEdit";
		syra_menus.click.menuId(record, "$quickEdit");
	}
};
