...
exports.Builder = helpers.defineClass(Builder, null, {
	...
	_hasFreezeField: function($items) {
		if (!syra_site.enableTestRobot && !this.list.page.lookupField && !(this.list.$item.$cardItem && this.list.$item.$cardItem.$position == "row")) {
	...
	_addTitleRow: function() {
		...
		if (this.list.$item.$isQuickFilter) {
			if (this.filterRecord) {
				...
				this.filterRecord = _filterBuilder.addRecord(this.list);
			}
		}
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isExcluded) {
				...
				($isFixed ? freezeTable : this.scrollTable).addFieldCol($item, $field);
			}
		}
	...
	resizeScrollView: function() {
		if (!this.list.isTableSlotHidden) {
			var container = this.list.tableSlot;
			if (!this.list.tableRow) {
				container = this.list.tableSlot.parentNode; //if row tableSlott is cell so maxWidth
			}
