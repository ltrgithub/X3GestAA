...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	removeInlineCard: function() {
		...
		if (col) {
			...
			for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
				...
				record.inlineCardOpener = record.inlineCardCell = record.inlineCardRow = record.cardItem = null;
				document.site.removeDomChild(record.dataRow.childNodes[index]);
			}
			delete this.cols.inlineCard;
			this.$cardItem = this.list.$item.$cardItem = this.$inlineFieldBinds = null;
		}
	},
	appendInlineCard: function(designing) {
		this.$cardItem = this.list.$item.$cardItem;
		if (!this.$cardItem && this.$complexBinds.length) {
			this.$cardItem = {
				$position: "inline",
				$layout: {
					$layoutType: "tabs",
			...
			for (var ii = 0, jj = this.$complexBinds.length; ii < jj; ii++) {
				...
				this.$cardItem.$layout.$items.push({
					$category: "section",
		...
		if (this.$cardItem) {
			this.$inlineFieldBinds = this.list.page.getDefinedFieldBinds(this.$cardItem.$layout ? this.$cardItem.$layout.$items : this.$cardItem.$items);
		}
		if (designing) {
			...
			if (metaData.$cardPosition !== undefined) {
				if (metaData.$cardPosition == "no" || metaData.$cardPosition == "inline") {
					...
					if (metaData.$cardPosition == "inline") {
						...
						this.list.$item.$cardItem = $cards;
						this.appendInlineCard(designing);
					} else {
						this.removeInlineCard();
					}
				} else {
					this.removeInlineCard();
	...
	_makeTitleRow: function() {
		...
		if (this.$cardItem && this.$cardItem.$position == "inline") {
			if (this.list.filterRecord) {
				this.list.filterRecord.appendCell();
			}
