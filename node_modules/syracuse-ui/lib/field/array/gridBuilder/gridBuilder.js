...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	reorderColumns: function(colSource, colTarget, dropAtEnd) {
		...
		if (this.list.awPalette) {
			this.list.awPalette.toggleColumnsToolBox();
		}
		this.onWindowResize();
		this.notifyAuthoring();
	},
	...
	applyDesignMetaData: function(metaData, onAuthoring) {
		if ((metaData.$fixedBodyHeight !== undefined) || (metaData.$fitContainer !== undefined) || (metaData.$dockPageHeight !== undefined)) {
			...
			if (metaData.$dockPageHeight !== undefined) {
				this.list.$item.$dockPageHeight = metaData.$dockPageHeight;
			}
			if (this.scroller) {
	...
	_createComplexFieldsCard: function() {
		...
		if ($arrayBinds.length > 0) {
			this.list.$item.$layout.$items = this.list.$item.$layout.$items.filter(function($item) {
				return $arrayBinds.indexOf($item.$bind) < 0;
			});
			var $cardItems = [];
			for (var ii = 0, jj = $arrayBinds.length; ii < jj; ii++) {
				var $bind = $arrayBinds[ii];
