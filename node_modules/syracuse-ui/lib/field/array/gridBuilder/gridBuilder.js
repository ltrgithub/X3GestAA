...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	initialize: function() {
		...
		this.appendInlineCard();
	},
	convertOldCardItem: function() {
		if (this.list.$item.$inlineCard) {
			this.list.$item.$cardItem = this.list.$item.$inlineCard;
			this.list.$item.$cardItem.$position = "inline";
	...
	removeRecordsCardItem: function() {
		var col = this.cols.inlineCard;
		var inlineColIndex;
		if (col) {
			inlineColIndex = this.findColIndex(col);
			this.orderCols.splice(inlineColIndex, 1);
			...
			delete this.cols.inlineCard;
			this.$inlineFieldBinds = null;
		}
		for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
			var record = this.list.records[ii];
			if (record.inlineCardRow) {
				document.site.removeDomChild(record.inlineCardRow);
				delete record.inlineCardOpener;
				delete record.inlineCardCell;
				delete record.inlineCardRow;

			}
			if (record.cardItem) {
				record.removeItem(record.cardItem, true, true);
	...
	appendInlineCard: function(designing) {
		if (!this.list.$item.$cardItem) {
			if (this.$complexBinds.length) {
		...
		if (this.list.$item.$cardItem) {
			this.$inlineFieldBinds = this.list.page.getDefinedFieldBinds(this.list.$item.$cardItem.$layout ? this.list.$item.$cardItem.$layout.$items : this.list.$item.$cardItem.$items);
		}
		if (designing) {
			this.list.$item.$cardItem.$position = "inline"; //ensure position
			var record = this.list.filterRecord;
			if (record) {
				record.dataRow.insertBefore(record.createCell(), record.dataRow.firstChild.nextSibling);
			}
			var col = this._appendInlineCardCol(this.titleRow, this.titleRow.firstChild.nextSibling);
			this.orderCols.splice(1, 0, col);
			...
			for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
				...
				record.dataRow.insertBefore(record.appendInlineCardOpener(), record.dataRow.firstChild.nextSibling);
				if (ii == 0) {
	...
	onColumnResized: function(column) {
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			if ($items[ii].$bind == column.$bind) {
				...
				this.list.notifyDesignerUpdate();
				break;
			}
		}
	},
	...
	toggleFixRecordCell: function(cellIndex, isFixed) {
		for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
			document.site.toggleClass(this.list.records[ii].dataRow.childNodes[cellIndex], "s-grid-col-fixed", isFixed);
		}
	},
		...
		if (designing) {
			...
			if (metaData.$cardPosition !== undefined) {
				if (metaData.$cardPosition == "no" || metaData.$cardPosition == "inline") {
					var $cardItem = this.list.$item.$cardItem;
					this.removeRecordsCardItem();
					delete this.list.$item.$cardItem;
					this.list.appendCardGraphSlots();
					if (metaData.$cardPosition == "inline") {
						this.list.$item.$cardItem = $cardItem;
						this.appendInlineCard(designing);
					}
				} else {
	...
	_makeTitleRow: function() {
		...
		if (this.list.treeDecorator) {
			...
			if (this.list.filterRecord) {
				this.list.filterRecord.appendCell();
			}
	...
	_appendInlineCardCol: function(slot, beforeCell) {
	...
	drawBuilder: function() {
		...
		this.list.fieldValue.style.overflow = "";
	...
	dispose: function() {
		...
		this.$inlineFieldBinds = this._headColgroup = this.cols = this.gridCss = this.orderCols = this.titleRow = this.prevCols = null;
		this.list = this.emptyData = this.scroller = this.$colBinds = this._$$body = null;
	}
});