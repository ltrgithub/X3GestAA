...
function _addRowIndexCol(list) {
	...
	_showRowIndexCol(list);
}

function _showRowIndexCol(list) {
	if (list.cols && list.cols.$rowIndex) {
		if (list.$item.$isRowIndexVisible || list.$item.$selectByRowIndex) {
			list.cols.$rowIndex.colWidth = (list.$capability && list.$capability.reorder) ? 35 : 25;
		} else {
			list.cols.$rowIndex.colWidth = 0;
		}
	}
}
...
exports.isDraggable = function(list, target, event) {
	if (target.syraResizerColId == list.id) {
		var col = _findEventCol(list, event);
		colResizer.isDraggable(list, col, event);
