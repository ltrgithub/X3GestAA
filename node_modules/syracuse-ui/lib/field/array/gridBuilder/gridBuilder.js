...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    emptyBody: function(addEmptySlot){
        ...
        if (self.list.$item.$format == "vgrid") {
            vgridHelper.emptyBody(self, addEmptySlot);
        }
        else {
            $(self._body).empty();
            if (addEmptySlot) {
    ...
    _setBodyHeight: function(){
        ...
        if (!bodyHeight) {
            var $$container = this.list._$$fitContainer || this.list.$$container;
            this._containerHeight = $$container[0].offsetHeight - ((this.list.$$item.outerHeight() - this.list.$$item.height()));
            bodyHeight = this._containerHeight - (this._$$bodySlot.offset().top - $$container.offset().top) - this._hscroller.offsetHeight;
        }
    ...
    onWindowResize: function(initialize){
        if (!this.disposed && this._bodySlot) {
            ...
            if (self.list.$item.$format == "grid") {
                var oldMaxCol = self._maxCol;
                delete self._maxCol;
                if (self._bodySlot.clientWidth) {
                    self.widthTotal = 0;
                    self._maxCol = self._orderCols[0];
                    
                    for (var ii = 0, jj = self._orderCols.length; ii < jj; ii++) {
                        var col = self._orderCols[ii];
                    ...
                    if (oldMaxCol && oldMaxCol != self._maxCol) {
                        oldMaxCol.groupHead.style.width = oldMaxCol.groupBody.style.width = oldMaxCol.widthValue + "px";
                    }
                }
    ...
    appendFieldCol: function(slot, $item, $field){
        ...
        if (width) {
            ...
            var $maxLength = $field.$displayLength || $field.$maxLength || $width.$maxLength;
            if ($maxLength) {
                col.widthValue = (10 * $maxLength);
            }
        }
    ...
    _makeTitleRow: function(){
        ...
        if (this.list.$item.$format == "vgrid") {
            ...
            if (!this.list.$item.$isMenuRecordHidden) {
                //contextmenu
                this.list.filterCapability.appendEmptyCell();
                this._orderCols.push(this._appendContextMenusCol(this.titleRow));
            }
    ...
    dispose: function(){
        ...
        if (this.scrollLinks) {
            ...
            delete this.scrollLinks;
        }
        delete this._$$hscroller;
        delete this._$$bodySlot;
        delete this._$$headSlot;
