...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if (designing) {
			if (metaData.$graphPosition !== undefined) {
				this._toggleDataList(metaData.$graphPosition != "front");
			}
			if (metaData.$cards !== undefined) {
				...
				if (this._cardSlot) {
					...
					this.list._core.appendChild(scroller.bodySlot);
	...
	_toggleDataList: function(show) {
		if (this._tableBody) {
			...
			this.scroller.drawSlots(core, this._headTable, this.bodyTable);
	...
	wrapListForOutCard: function($outCards) {
		if ($outCards) {
			$outCards.$category = $outCards.$category || "section";
			if (this.list.$item.$selectMode === undefined) {
				this.list.$item.$selectMode = "row";
			}
			var horzCss, cardWidth;
			if ($outCards.$position == "left" || $outCards.$position == "right") {
				horzCss = "-horz";
				cardWidth = ($outCards.$widths || "50") + "%";
			} else {
				cardWidth = horzCss = "";
			}
			this.list._core.className = "s-list-core s-list-gridcard-slot" + horzCss;
			(this.tableSlot = document.createElement("div")).className = "s-list-grid-slot" + horzCss;
			(this._separator = document.createElement("div")).className = "s-list-card-sep" + horzCss;;
			(this.outCardSlot = document.createElement("div")).className = "s-list-card-slot" + horzCss;
			this.tableSlot.style.width = "";
			this.outCardSlot.style.width = cardWidth;
			if ($outCards.$position == "top" || $outCards.$position == "left") {
				this.list._core.appendChild(this.outCardSlot);
	...
	drawBuilder: function() {
		...
		this.wrapListForOutCard(this.list.$item.$cards);
	...
	dispose: function() {
		...
		this.tableSlot = this._separator = this.outCardSlot = null;
	}
});