...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    _setBodyHeight: function(){
        if (this._bodySlot) {
            ...
            if (fixedHeight) {
                ...
                this._bodySlot.style.height = (Math.max(40, fixedHeight) + 17) + "px";
            }
            else {
    ...
    _toggleDataList: function(show){
        if (this._tableBody) {
            ...
            this._bodyTable.className = this.list.$skin + "-table-head";
            ...
            this._$$bodySlot = $(this._bodySlot);
            this._bodySlot.style.overflowX = "auto";
            this._setBodyHeight();
        }
    },
    _initializeSize: function(initialize){
        ...
        if (self.list.$item.$format == "grid") {
            if (!self._maxCol && self._bodySlot.clientWidth) {
                ...
                self._orderCols.forEach(function(col){
                    widthTotal += col.widthValue;
                    if (col.widthValue > self._maxCol.widthValue) {
                        self._maxCol = col;
                    }
                });
                if (widthTotal < (self._bodySlot.clientWidth - 1)) {
                    self._maxCol.groupHead.style.width = self._maxCol.groupBody.style.width = ((self._bodySlot.clientWidth - 1 - widthTotal) + self._maxCol.widthValue) + "px";
                }
            }
        }
    },
    onWindowResize: function(initialize){
        this._initializeSize(initialize);
        if (!this.list.$item.$fixedHeight) {
            this._$$bodySlot.height((this._bodyTable.scrollHeight + 17) + "px");
        }
        this._ensureScrollBar();
    },
    _ensureScrollBar: function(){
        if (this._hasScrolling = (this._bodySlot.scrollHeight != this._bodySlot.clientHeight)) {
    ...
    _bindScrollEvent: function(bind){
        ...
        if (bind) {
            self._$$bodySlot.bind("scroll", function(event){
                self._$$headSlot.scrollLeft(self._$$bodySlot.scrollLeft());
            });
        }
        else {
            if (self._$$bodySlot) {
                self._$$bodySlot.unbind("scroll");
            }
        }
    },
