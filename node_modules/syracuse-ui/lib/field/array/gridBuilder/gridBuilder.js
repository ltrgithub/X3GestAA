...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    emptyBody: function(addEmptySlot){
        ...
        if (addEmptySlot) {
            ...
            cell.setAttribute("colspan", this._orderCols.length);
    ...
    _makeTitleRow: function(){
        ...
        if (this.list.treeDecorator) {
            this._orderCols.push(this.list.treeDecorator.appendTreeviewCol(this.titleRow));
        }
        else {
        ...
        if (self.list.$item.$cards) {
            ...
            if (self.list.$item.$cards.$position != "inline") {
                ...
                if (self.list.$item.$cards.$position == "left" || self.list.$item.$cards.$position == "right") {
                    ...
                    self._cardSlot.style.width = (self.list.$item.$cards.$widths || "50") + "%";
                }
                else {
                ...
                if ((self.list.$item.$cards.$position == "top") ||
                (self.list.$item.$cards.$position == "left")) {
    ...
    dispose: function(){
        ...
        this.recordCardView = this._$$hscroller = this._$$bodySlot = this._$$headSlot = this.list = this.emptyData = null;
    }
});
