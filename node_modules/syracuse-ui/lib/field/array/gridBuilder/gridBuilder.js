...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
    ...
    _initializeSize: function(){
        var self = this;
        if (!this._isInitSize) {
            this._isInitSize = true;
            if (self.list.$item.$format == "grid") {
                var widthTotal = 0, maxCol = self._orderCols[0];
                self._orderCols.forEach(function(col){
                    widthTotal += col.widthValue;
                    if (col.widthValue > maxCol.widthValue) {
                        maxCol = col;
                    }
                });
                if (widthTotal < (this._bodySlot.clientWidth -1)) {
                    maxCol.groupHead.style.width = maxCol.groupBody.style.width = ((this._bodySlot.clientWidth -1 - widthTotal) + maxCol.widthValue) + "px";
                }
            }
        }
    },
