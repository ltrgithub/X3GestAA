...
exports.Scroller = helpers.defineClass(Scroller, null, {
	...
	getInvisibleSize: function() {
		return {
			height: this.bodySlot.scrollHeight - this._$$bodySlot.height(),
			width: this.bodySlot.scrollWidth - this._$$bodySlot.width()
		};
	},
	...
	getScrollBoundary: function() {
		...
		var boundary = $(this.bodySlot).offset();
	...
	_bindEvents: function(bind) {
		...
		if (bind) {
			(self._$$hscroller = $(self._hscroller)).bind("scroll", function(event) {
				document.site.closePopups(self.list, event);
	...
	_scrollCols: function(left) {
		...
		self._scrollColsTimeout = setTimeout(function() {
			var cols = self.builder.orderCols;
			var colToScroll;
	...
	isScrolledIntoView: function($$elem, $$view) {
		return document.site.isScrolledIntoView($$elem, $$view);
	},
	_appendButtons: function() {
		if (this.list.isScrollButtonEnabled !== false) {
			if (this.list.pagging.getItemsPerPage() > 20 && this.list.dataset.length > 20) {
				if (!this.leftBtn) {
					var page = this._getPage();
					var $$scrollview = page.$$scrollview;
					if ((!this.isScrolledIntoView(this._$$hscroller, $$scrollview) && this.isScrolledIntoView($(this.builder._headTable), $$scrollview)) || (this.isScrolledIntoView(this._$$hscroller, $$scrollview) && !this.isScrolledIntoView($(this.builder._headTable), $$scrollview)) || (!this.isScrolledIntoView(this._$$hscroller, $$scrollview) && !this.isScrolledIntoView($(this.builder._headTable), $$scrollview))) {
						...
						if (page && page.addScrollViewListener) {
							this._getPage().addScrollViewListener(this.list.id, this._updateButtonsPosition);
						}
					}
				}
				if (this.leftBtn) {
					...
					//this.leftBtn.style.display = this.rightBtn.style.display = this.isScrolledIntoView($(this.list.builder.scroller._hscroller), page.$$scrollview) ? "none" : "";	
				}
			} else {
	...
	_updateButtonsPosition: function(scroller) {
		if (scroller._areButtonsVisible) {
			var $$scrollview = scroller._getPage().$$scrollview;
			...
			switch (scroller.isScrolledIntoView($$headtable, $$scrollview)) {
				// list head table is visible in the page
				case (true):
					// list hor scroller is visible in the page
					if (scroller.isScrolledIntoView(scroller._$$hscroller, $$scrollview)) {
	...
	dispose: function() {
		if (this.$$leftBtn) {
			var page = this._getPage();
			if (page && page.removeScrollViewListener) {
				page.removeScrollViewListener(this.list.id);
			}
			this.$$leftBtn.unbind();
			this.$$rightBtn.unbind();
			this.$$rightBtn = this.$$leftBtn = this.leftBtn = this.rightBtn = null;
		}
		this.builder = this.list = this._headSlot = this.bodySlot = this._hscroller = this._$$headSlot = this._$$hscroller = this._$$bodySlot = this._hscrollBody = this._scrollColsTimeout = null;
	}
});