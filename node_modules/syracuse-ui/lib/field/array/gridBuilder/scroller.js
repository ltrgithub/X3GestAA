...
exports.Scroller = helpers.defineClass(Scroller, null, {
	...
	scrollToTitleItem: function(domItem, $$area) {
		this.list.page.scrollToItem(domItem, $(this._headSlot), true);
	},
	scrollToItem: function(domItem, $$area) {
	...
	_appendButtons: function() {
		if (this.list.$item.$itemsPerPage > 20) {
			if (!this.leftBtn) {
	...
	_updateButtonsPosition: function() {
		if (this._areButtonsVisible) {
			var $$scrollview = document.site.mainPage.$$scrollview;
			var $$headtable = $(this.builder._headTable);
			var top;
			...
			switch (document.site.isScrolledIntoView($$headtable, $$scrollview)) {
				...
				case (true):
					...
					if (document.site.isScrolledIntoView(this._$$hscroller, $$scrollview)) {
						top = ((this._$$hscroller.offset().top - $$headtable.offset().top) / 2 + $$headtable.offset().top) + "px";
					}
				...
				case (false):
					// list hor scroller is visible in the page
					if (document.site.isScrolledIntoView(this._$$hscroller, $$scrollview)) {
						top = (this._$$hscroller.offset().top / 2) + "px";
					}
					...
					// list hor scroller is not visible in the page
						...
						var hscrollerBottom = this._$$hscroller.position().top + this._$$hscroller.height();
			...
			if (!top) {
				...
				this.leftBtn.style.display = this.rightBtn.style.display = "";
				this.leftBtn.style.top = this.rightBtn.style.top = top;
			}
		}
	},
	dispose: function() {
