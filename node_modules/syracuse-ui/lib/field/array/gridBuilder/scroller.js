...
exports.Scroller = helpers.defineClass(Scroller, null, {
		...
		if (this.bodySlot && this.bodySlot.clientWidth) {
			...
			if (this.$fixedBodyHeight || this.list.$item.$fitContainer || this.list.$isDockedHeight) {
				...
				if (this.list.$isDockedHeight) {
					var scrollRect = this.list.page.scrollview.getBoundingClientRect();
					var bodyRect = this.bodySlot.getBoundingClientRect();
					var fieldRect = this.list.$$fieldValue[0].getBoundingClientRect();
	...
	isScrolledIntoView: function($$elem, scrollTop, scrollHeight) {
		var docViewBottom = scrollTop + scrollHeight;
			...
			if (this.list.records.length > 20) {
				if (!this.leftBtn) {
					...
					if (scrollview) {
						...
						var scrollHeight = scrollview.getBoundingClientRect();
						if ((!this.isScrolledIntoView(this._$$hscroller, scrollTop, scrollHeight) && this.isScrolledIntoView($(this.list._headTable), scrollTop, scrollHeight)) || (this.isScrolledIntoView(this._$$hscroller, scrollTop, scrollHeight) && !this.isScrolledIntoView($(this.list._headTable), scrollTop, scrollHeight)) || (!this.isScrolledIntoView(this._$$hscroller, scrollTop, scrollHeight) && !this.isScrolledIntoView($(this.list._headTable), scrollTop, scrollHeight))) {
	...
	_updateButtonsPosition: function(scroller) {
		if (scroller._areButtonsVisible) {
			...
			if (scroller.isScrolledIntoView($$headtable, scrollTop, scrollHeight)) {
				// list head table is visible in the page
				// list hor scroller is visible in the page
				if (scroller.isScrolledIntoView(scroller._$$hscroller, scrollTop, scrollHeight)) {
				...
				// list hor scroller is not visible in the page
					...
					top = ((scrollview.clientHeight - $$headtable.offset().top) / 2 + $$headtable.offset().top) + "px";
				}
			} else {
				...
				// list hor scroller is not visible in the page
					...
					if ((hscrollerBottom < scrollTop) || (headtableTop > docViewBottom)) {
						top = top;
					}
