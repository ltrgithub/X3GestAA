...
exports.Scroller = helpers.defineClass(Scroller, null, {
    ...
    validateFixedColumns: function(){
        ...
        for (var ii = 0, jj = this.builder.orderCols.length; ii < jj; ii++) {
            ...
            if (!col.$isFixed && !this.fixed) {
                this.fixed = {
                    firstColIndex: ii
                };
            }
        }
        if (!this.builder.list.page.isFixeEnabled) {
            this.fixed = null;
        }
    ...
    onWindowResize: function(){
        if (this._bodySlot && this._bodySlot.clientWidth) {
            ...
            if (clientWidth) {
                this._widthTotal = 0;
                var lastVisibleColIndex = null;
                for (var ii = 0, jj = this.builder.orderCols.length; ii < jj; ii++) {
                    var col = this.builder.orderCols[ii];
                    if (this.fixed) {
                        col.scrollWidth = col.widthValue;
                        if (!lastVisibleColIndex && (this._widthTotal + col.widthValue) >= clientWidth) {
                            col.scrollWidth = clientWidth - this._widthTotal;
                            lastVisibleColIndex = ii;
                        }
                    }
                ...
                if (this._widthTotal < clientWidth) {
                    ...
                    if (this.fixed) {
                        this.fixed.minX = 0;
                        this.fixed.mawX = this.builder.orderCols[this.fixed.firstColIndex].scrollWidth;
                        this.fixed.nextColIndex = this.fixed.firstColIndex;
                    }
                }
    ...
    scrollFixedColumns: function(fixed){
        if (this._hscroller.scrollLeft >= fixed.mawX) {
            var col = this.builder.orderCols[fixed.nextColIndex];
            col.groupBody.style.display = "none";
            col.groupHead.style.display = "none";
            col.titleCell.style.display = "none";
            this._displayRecords(fixed.nextColIndex, "none");
            fixed.minX += this.builder.orderCols[fixed.nextColIndex].scrollWidth;
            fixed.nextColIndex = Math.min(++fixed.nextColIndex, this.builder.orderCols.length - 1);
            fixed.mawX += this.builder.orderCols[fixed.nextColIndex].scrollWidth;
        }
        
        if (this._hscroller.scrollLeft < fixed.minX) {
            fixed.minX -= this.builder.orderCols[fixed.nextColIndex].scrollWidth;
            fixed.minX = Math.max(0, fixed.minX);
            fixed.mawX -= this.builder.orderCols[fixed.nextColIndex].scrollWidth;
            var col = this.builder.orderCols[--fixed.nextColIndex];
            this._displayRecords(fixed.nextColIndex, "");
            col.groupBody.style.display = "";
            col.groupHead.style.display = "";
            col.titleCell.style.display = "";
        }
    },
        ...
        if (bind) {
            (self._$$hscroller = $(self._hscroller)).bind("scroll", function(event){
                ...
                if (!self.fixed) {
                    ...
                    self.scrollFixedColumns(self.fixed);
                }
                return false;
            });
