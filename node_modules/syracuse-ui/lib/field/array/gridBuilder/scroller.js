...
exports.Scroller = helpers.defineClass(Scroller, null, {
	...
	getVisibleSize: function() {
		return document.site.getBoundingClientRect(this.bodySlot);
	},
	...
	scrollToItem: function(domItem) {
		this.list.page.scrollToItem(domItem, this._bodySlot, true);
	},
		...
		if (this.bodySlot && this.bodySlot.clientWidth) {
			...
			if (this.$fixedBodyHeight || this.list.$item.$fitContainer || this.list.$isDockedHeight) {
				...
				if (this.list.$isDockedHeight) {
					var scrollRectTop = document.site.getBoundingClientRect(this.list.page.scrollview);
					...
					if (this.$fixedBodyHeight || this.list.$item.$fitContainer) {
						...
						if (!bodyHeight) {
							var slotRect = document.site.getBoundingClientRect(this.list.layoutSlot);
		...
		if (this.list.isScrollButtonEnabled !== false && !this.list.treeDecorator) {
			if (this.list.records.length > 20) {
				if (!this.leftBtn) {
					...
					if ((!this.isScrolledIntoView(this._$$hscroller, $$scrollview) && this.isScrolledIntoView($(this.list._headTable), $$scrollview)) || (this.isScrolledIntoView(this._$$hscroller, $$scrollview) && !this.isScrolledIntoView($(this.list._headTable), $$scrollview)) || (!this.isScrolledIntoView(this._$$hscroller, $$scrollview) && !this.isScrolledIntoView($(this.list._headTable), $$scrollview))) {
				...
				if (this.leftBtn) {
					...
					//this.leftBtn.style.display = this.rightBtn.style.display = this.isScrolledIntoView($(this.list.scroller._hscroller), this.list.page.$$scrollview) ? "none" : "";	
				}
			} else {
	...
	dispose: function() {
		...
		this.list = this._headSlot = this.bodySlot = this._hscroller = this._$$headSlot = this._$$hscroller = this._$$bodySlot = this._hscrollBody = this._scrollColsTimeout = null;
	}
});