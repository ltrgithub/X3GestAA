...
function _addScrollHandlers(tb) {
	...
	tb.headSlot_onScroll = tb.headSlot.addEventListener("scroll", function(event) {
		var page = tb.builder.list.page;
		if (!syra_site.isInoutDisabled && page.isPageLoaded && !page._isDataChanging) {
			if (tb.bodySlot.scrollLeft != this.scrollLeft) {
...
function _removeScrollHandlers(tb) {
	...
	tb.headSlot_onScroll && tb.headSlot.removeEventListener("scroll", tb.headSlot_onScroll);
}
...
exports.GridTable = helpers.defineClass(GridTable, null, {
	...
	_addColumn: function($key, before) {
		...
		this.builder.allColumns[method](col);
		this.columns[method](col);
		return col;
	},
	...
	addRowCardCol: function(before) {
		var col = this._addColumn("rowCard", before);
		col.useContentWidth = true;
		...
		if (before) {
			col.titleCell.parentNode.insertBefore(col.titleCell, col.titleCell.parentNode.firstChild.nextSibling);
		}
		this.builder.filterRecord && this._addFilterCell(col);
		return col;
	},
	...
	reorderRow: function(target, source, isAfter) {
		...
		if (sourceRow) {
			if (target) {
				...
				if (isAfter) {
					sourceRow.parentNode.appendChild(sourceRow);
				} else {
					sourceRow.parentNode.insertBefore(sourceRow, sourceRow.parentNode.firstChild);
				}
			}
		}
	},
	setColWidths: function() {
		...
		//$prefWidths = {};
		var row = records && records[0] && records[0][this.$rowKey];
		var cells = row && row.cells;
		for (var ii = 0, jj = this.columns.length; ii < jj; ii++) {
			!this.columns[ii].$isHidden && _columnSizer.setColumnWidth(this.columns[ii], records, $prefWidths, cells && cells[ii]);
		}
	...
	dispose: function() {
		...
		this.columns = this.headSlot = this.headTable = this.titleRow = this.bodySlot = this.bodyTable = null;
		this.bodySlot_onScroll = this.headSlot_onScroll = null;
	}
});