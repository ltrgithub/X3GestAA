...
exports.ColResizer = helpers.defineClass(ColResizer, null, {
	...
	setResizable: function(builder) {
		...
		self.$$row.delegate(".s-grid-title-resizer", "mousedown", function(event) {
			...
			if (!document.site.ddManager.ddAgent && self.startX !== null && !self.drag) {
				self.resizedCol = col;
				self._startResize();
		...
		}).delegate("th", "mousedown", function(event) {
	...
	_startResize: function() {
		...
		document.site.ddManager.setddAgent(self);
		...
		$(document).bind("mouseup." + self.id, function(event) {
			...
			document.site.ddManager.setddAgent();
			event.preventDefault();
			return false;
		}).bind("mousemove." + self.id, function(event) {
			self.onResize(event);
