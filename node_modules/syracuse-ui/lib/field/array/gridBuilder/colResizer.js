...
exports.ColResizer = helpers.defineClass(ColResizer, null, {
    ...
    setResizable: function(builder){
        ...
        self.$$row.delegate(".s-grid-title-resizer", "mousedown", function(event){
            ...
            if (self.isEnabled && self.startX !== null && !self.drag) {
                self.resizedCol = col;
                self._startResize();
    ...
    _setScrollBoundary: function($$boundary, $$vscroller, $$hscroller){
        this.scrollBoundary = $$boundary.offset();
        this.scrollBoundary.$$vscroller = $$vscroller;
        this.scrollBoundary.$$hscroller = $$hscroller;
        this.scrollBoundary.$$boundary = $$boundary;
        this.scrollBoundary.height = $$boundary.height();
        this.scrollBoundary.width = $$boundary.width();
        this.scrollBoundary.scrollWidth = $$hscroller[0].scrollWidth;
        this.scrollBoundary.scrollHeight = $$vscroller[0].scrollHeight;
        this.scrollBoundary.right = this.scrollBoundary.left + this.scrollBoundary.width;
        this.scrollBoundary.bottom = this.scrollBoundary.top + this.scrollBoundary.height;
    },
    ...
    _startDrag: function(event){
        ...
        self._setScrollBoundary($(self.builder.list._core), self.builder._$$bodySlot, $(self.builder._hscroller));
        ...
        $(document).bind("mouseup." + self.id, function(event){
            ...
            if (self.drag && self.drag.target) {
                self.builder.reorderColumns(self.drag.source, self.drag.target, self.drag.dropAtEnd);
            }
            delete self.drag;
            event.preventDefault();
            return false;
    ...
    _startResize: function(){
        ...
        if (self.builder.isProportionalWidth) {
            var resizedColIndex;
