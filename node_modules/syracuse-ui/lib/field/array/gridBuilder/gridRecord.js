...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	applyChange: function(newData) {
		...
		if (newData && !this.list.$isEditMode) {
			this.list.fitToWidFields = this.list.fitToWidFields || {};
			for (var ii = 0, jj = this.list.orderCols.length; ii < jj; ii++) {
				var col = this.list.orderCols[ii];
				if (col.$bind) {
					var field = this.boundFields[col.$bind];
					field = field && field[0];
					if (field && field.valueLength) {
						if (!this.list.fitToWidFields[col.$bind]) {
							this.list.fitToWidFields[col.$bind] = {
								valueLength: 0
							};
						}
						if (this.list.fitToWidFields[col.$bind].valueLength < field.valueLength) {
							this.list.fitToWidFields[col.$bind].valueLength = field.valueLength;
							this.list.fitToWidFields[col.$bind].field = field;
						}
					}
				}
			}
	...
	remove: function() {
		...
		if (this.rowCardRow) {
			document.site.dom.removeChild(this.rowCardRow);
		}
	},
	applyReorderCapability: function(reorder) {
		if (!this.isReorderDisabled) {
			...
			if (reorder) {
				...
				document.site.dom.toggleClass(this.rowIndexPicker, "s-grid-record-reorder", true);
				...
				if (this.reorderPicker) {
					...
					document.site.dom.toggleClass(this.rowIndexPicker, "s-grid-record-reorder", false);
	...
	appendRowIndexCell: function(slot) {
		...
		if (this.list.$item.$selectByRowIndex) {
			...
			this.rowIndexPicker.syraOnClick = "onSelectorClick";
		}
	...
	renderAltStyle: function(showAlt) {
		document.site.dom.toggleClass(this.dataRow, "s-list-row-alt", showAlt);
	},
	drawBox: function() {
	...
	toggleFocus: function(focused) {
		...
		document.site.dom.toggleClass(this.dataRow, "s-record-focused", focused);
	},
	highlightSelection: function(selected) {
		document.site.dom.toggleClass(this.dataRow, "s-list-record-selected", selected);
