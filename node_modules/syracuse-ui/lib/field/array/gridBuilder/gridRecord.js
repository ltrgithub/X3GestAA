...
function _drawVgridRecord(record){
    ...
    if (record.list._store.selector.useColumnSelector) {
    ...
    for (var ii = 0, jj = $items.length; ii < jj; ii++) {
        ...
        if ($field && !$field.$isHidden && !$item.$isExcluded) {
...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
    ...
    appendTreeviewCell: function(slot){
    	console.log("appendTreeviewCell");
        var cell = document.createElement("td");
        cell.className = this.builder.gridCss.cell + " s-list-tree-cell";
        slot.appendChild(cell);
        this.list.appendTreeNode(this, cell);
        return cell;
    },
    ...
    drawBox: function(){
        if (this.list.$item.$format == "vgrid") {
            ...
            if (this.isInsert) {
                ...
                if (this.builder.list._store._records[this.$recordIndex]) {
                    this.$$item.insertBefore(this.builder.list._store._records[this.$recordIndex].$$item);
                }
                else {
            ...
            if (this.list.$prototype.$treeview) {
                this.appendTreeviewCell(this._dataRow);
            }
            ...
            if (this.list._store.selector.useColumnSelector) {
            ...
            if (this.list._store.selector.isRowMode) {
