...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	...
	drawBox: function() {
		...
		if (!this.list.treeDecorator) {
			if (this.isInsert) {
				if (this.list.records[this.$recordIndex]) {
					...
					this.builder._body.appendChild(this.dataRow);
				}
		...
		if (this.list.treeDecorator) {
			this.list.treeDecorator.appendTreeviewCell(this, this.dataRow);
		} else {
		...
		if (this.list.$item.$cards && this.list.$item.$cards.$position == "inline") {
			...
			this.cardsOpener.setAttribute("data-s-btn", "list-card-opener");
		...
		if (this.list.selector.isRowMode) {
			this.dataRow.setAttribute("data-s-btn", "list-selector");
