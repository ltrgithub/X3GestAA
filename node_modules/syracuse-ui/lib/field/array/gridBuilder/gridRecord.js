...
function CellDiagnose() {}

exports.CellDiagnose = helpers.defineClass(CellDiagnose, null, {
	loadBox: function() {
		var self = this;
		// $$item
		self._item = document.createElement("div");
		self._item.className = "s-cell-diag";
		self.$$item = $(self._item);
		//  $$header (to append close button)
		self._header = document.createElement("div");
		self._header.className = "s-cell-diag-header";
		self.$$header = $(self._header);
		self._item.appendChild(self._header);
		// $$body
		self._body = document.createElement("div");
		self._body.className = "s-cell-diag-body";
		self.$$body = $(self._body);
		self._item.appendChild(self._body);
	},
	appendDomDiagnose: function(domDiagnose) {
		this.$$body[0].appendChild(domDiagnose);
	},
	dispose: function() {
		this.$$item = null;
	}
...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	dispose: function() {
		this.cellDiagnose && this.cellDiagnose.dispose();
		this.cardview = this.dataRow = this._cardsRow = this._cardsSlot = this.cardsOpener = this.cellDiagnose = this._diagPopups = null;
		...
		if (self.$$item) {
			if (bind) {
				self.$$item.bind("mouseenter", function() {
					...
					var id = this.getAttribute("data-s-celldiag-popup");
	...
	ensureFieldDiagnoseSlot: function(field) {
		if (!this.cellDiagnose) {
			this.cellDiagnose = new CellDiagnose();
			this.cellDiagnose.loadBox();
		}
		this.cellDiagnose.appendDomDiagnose(field.domDiagnose);
		/*
         if (!this.cellDiagnose) {
	...
	drawBox: function() {
		...
		if (this.isInsert) {
			...
			if (this.list.records[this.$recordIndex]) {
				...
				this.$$item = $(this.builder._body.appendChild(this.dataRow));
			}
		...
		if (this.list.treeList) {
			this.list.treeList.appendTreeviewCell(this, this.dataRow);
		} else {
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isHidden && !$field.$isExcluded) {
				this.appendFieldCell(this.dataRow, $item);
			}
		}
	...
	highlightSelection: function(selected) {
		...
		if (this.list.treeList) {
			this.list.treeList.highlightSelection(true);
		}
		if (selected) {
