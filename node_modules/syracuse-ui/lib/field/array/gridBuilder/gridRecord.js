...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	...
	drawBox: function() {
		...
		if (this.builder.$inlineCard) {
			...
			this.inlineCardOpener.setAttribute("data-s-picker", "list-card-opener");
			this.inlineCardOpener.className = "s-cardview-opener s-close";
	...
	highlightSelection: function(selected) {
		...
		if (selected) {
			...
			if (this.builder.outCardSlot) {
				document.site.emptyDom(this.builder.outCardSlot);
				this.builder.outCardSlot.setAttribute("data-s-record", this.$uuid);
				this.builder.outCardSlot.setAttribute("data-s-article", this.id);
				if (!this.outCardItem) {
					this.outCardItem = this.list.page.loadNewItem(this.builder.outCardSlot, this.list.$item.$cards, this);
				} else {
					this.builder.outCardSlot.appendChild(this.outCardItem.$$item[0]);
				}
			}
	...
	toggleInlineCard: function(show) {
		...
		if (!this.inlineCardRow) {
			...
			this.inlineCardItem._item.className = "s-grid-record-card-slot";
