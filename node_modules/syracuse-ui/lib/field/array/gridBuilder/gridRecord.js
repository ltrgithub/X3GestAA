...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	...
	alertDiagInCard: function() {
		if (!this.isGridCardDiagAlertSet) {
			document.site.showDiagnoses({
				$diagnoses: [{
					$message: "Click to go to grid and open card"
				}]
			}, this, {
				gridCardData: {
					gridRecordId: this.id,
					gridArticleId: this.articleParent.id,
					linkText: "Error(s) in grid " + this.articleParent.titleText + " (line " + this.$serverIndex + "):"
				}
			});
	...
	dispose: function() {
		this.treeNode = this.outCardItem = this.inlineCardItem = this.popupCardItem = this.dataRow = this.inlineCardRow = this.inlineCardCell = this.inlineCardOpener = null;
	...
	highlightSelection: function(selected) {
		...
		if (selected) {
			...
			if (this.list.outCardSlot) {
				document.site.emptyDom(this.list.outCardSlot);
				this.list.outCardSlot.setAttribute("data-s-record", this.$uuid);
				this.list.outCardSlot.setAttribute("data-s-article", this.id);
				if (!this.outCardItem) {
					this.list.$item.$cards.$category = this.list.$item.$cards.$category || "section";
					this.outCardItem = this.list.page.loadNewItem(this.list.outCardSlot, this.list.$item.$cards, this);
				} else {
					this.list.outCardSlot.appendChild(this.outCardItem.$$item[0]);
				}
			}
	...
	removeOutCard: function() {
		if (this.outCardItem) {
			this.removeItem(this.outCardItem, true, true);
			delete this.outCardItem;
		}
	},
	toggleInlineCard: function(show) {
		...
		if (!this.inlineCardRow) {
			...
			this.inlineCardItem = this.list.page.loadNewItem(this.inlineCardCell, this.builder.$inlineCard, this);
			this.builder._hasCardview = true;
			this.inlineCardItem._item.className = "s-grid-inlinecard-slot";
	...
	resizeCardItems: function() {
		if (this.outCardItem || this.inlineCardItem || this.popupCardItem) {
			if (this.builder.scroller.bodySlot.clientWidth) {
				if (this.outCardItem) {
					this.outCardItem._item.style.width = this.builder.scroller.bodySlot.clientWidth + "px";
				}
				if (this.inlineCardItem) {
					this.inlineCardItem._item.style.width = this.builder.scroller.bodySlot.clientWidth + "px";
				}
