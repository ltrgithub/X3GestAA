...
function _drawVgridRecord(record){
    ...
    record.builder.parseItems(function($item, $field){
        ...
        if (record.$isEditMode) {
            record.boundFields[$item.$bind][0].$$dataValue[0].style.minWidth = cols[$item.$bind].groupBody.style.width;
        }
        if (record.list._store.selector.isRowMode) {
            td.className += " s-list-selector-row";
        }
...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
    ...
    ensureFieldDiagnoseSlot: function(field){
        if (!this.cellDiagnose) {
            ...
            this.builder.bindDiagnoseFieldEvent(true);
        }
    ...
    toggleDeletedStatus: function($isDeleted){
        if (this.list.$item.$format == "vgrid") {
            this.orderCols.forEach(function(cell){
                document.site.toggleClass(cell, "s-grid-record-deleted", $isDeleted);
            });
        }
        else {
            document.site.toggleClass(this.$$item[0], "s-grid-record-deleted", $isDeleted);
        }
    },
    remove: function(){
