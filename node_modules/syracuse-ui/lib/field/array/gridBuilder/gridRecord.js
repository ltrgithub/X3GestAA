...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
    dispose: function(){
        delete this.$isVerticalDirection;
    ...
    appendRowIndexCell: function(slot){
        ...
        if (this.list.$capability.reorder) {
            this._rowIndexCell.className += " s-list-record-reorder";
        }
        this.renderRowIndex();
        return this._rowIndexCell;
    },
    appendFieldCell: function(slot, $item){
        //console.log(this.$recordIndex + " " + this.id + " " + $item.$bind);
    ...
    drawBox: function(){
        if (this.list.$item.$format == "vgrid") {
            ...
            if (!this.list.$item.$isMenuRecordHidden) {
                this._contextMenusSlot = document.createElement("td");
                this._contextMenusSlot.className = this.builder.gridCss.cell + " " + this.list.$skin + "-actions-cell";
                this._dataRow.appendChild(this._contextMenusSlot);
                this.list.applyCapabilities(this);
            }
            
            if (this.list._store.selector.appendToRecord) {
