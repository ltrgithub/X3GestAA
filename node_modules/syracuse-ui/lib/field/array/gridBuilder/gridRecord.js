...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
    dispose: function(){
        this.cardview = this.dataRow = this._cardsRow = this._cardsSlot = this.cardsOpener = null;
    ...
    ensureFieldDiagnoseSlot: function(field){
        ...
        // --- handling grid field diagnose slot ---
        if (!field.cellDiagnose) {
            field.cellDiagnose = new CellDiagnose();
            field.cellDiagnose.loadBox();
        }
        field.cellDiagnose.appendDomDiagnose(field.domDiagnose);
        
        // --- handling list diag alert info ---
        self.list.onGridFieldDiagnose(true);
    },
    emptyFieldDiagnoseSlot: function(field){
        field.cellDiagnose = null;
        this.list.onGridFieldDiagnose();
    },
    ...
    onMenuClick: function(menuItem){
        ...
        switch (menuItem.$bind) {
            case "$first":
                ...
                break;
            case "$previous":
                $serverIndex = (self.list.paging.$startIndex - 1) + (self.$recordIndex - 1);
                break;
            case "$next":
                $serverIndex = (self.list.paging.$startIndex - 1) + (self.$recordIndex + 1);
                break;
        ...
        $startIndex = (Math.floor($serverIndex / self.list.paging.$itemsPerPage) * self.list.paging.$itemsPerPage) + 1;
        setTimeout(function(){
            var list = self.list;
            if ($startIndex != list.paging.$startIndex) {
