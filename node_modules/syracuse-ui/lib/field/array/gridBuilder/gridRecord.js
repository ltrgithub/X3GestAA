...
exports.GridRecord = helpers.defineClass(GridRecord, RecordArticle, {
	applyChange: function(newData) {
		...
		newData && syra_grid.columnFitter.calculateMaxFieldLength(this);
	},
	...
	dispose: function() {
		this.disposeCard();
		this.fieldCells = this.treeNode = this.dataRow = this.popupCard = this._selectorCheck = this.titleSlot = null;
	...
	drawBox: function() {
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isExcluded) {
				...
				this.fieldCells[$item.$bind] = this.page.loadNewItem(this.dataRow.appendChild(td), {
					$bind: $item.$bind,
					$isCellChild: true,
					$inplace: true
				}, this);
	...
	onMenuClick: function(menuItem) {
		...
		switch (menuItem.$sourceBind) {
			...
			case "$last":
				...
				break;
			case "$recordCard":
				syra_grid.popupCardBuilder.togglePopup(this);
				return false;
			default:
