			...
			doEvent: function() {
				if (!cell.listPopup) {
					...
					for (var ii = 0, jj = operators.length; ii < jj; ii++) {
						...
						link.className = "s-filter-popup-link";
						...
						cell.list.appendChild(link).textContent = syra_local["flFilter_" + op];
					}
...
function _onFilterCellClick(event, picker) {
...
function _loadField(cell, $field, $bind) {
	...
	cell._operator.style.backgroundImage = "url('" + syra_site.$item.$iconPath + "list/s-filter-none.png')";
