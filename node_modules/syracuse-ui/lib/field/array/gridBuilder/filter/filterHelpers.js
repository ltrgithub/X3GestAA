...
exports.getQuickSelValues = function(list) {
	...
	if (list.filterRecord) {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			var field = list.filterRecord.fieldCells[$binds[ii]];
			if (field.$field) {
...
exports.addFieldCell = function(list, col, $field) {
	var td = document.createElement("td");
	td.className = "s-list-filter-cell";
	list.filterRecord.dataRow.appendChild(td);
	if (col.$isHidden) {
	...
	if (list.hasFieldFilterCapability($field)) {
		var field = _loadField(list.filterRecord, td, $field, col.$item.$bind);
		list.filterRecord.fieldCells[col.$item.$bind] = field;
		// update filter cell if where params in url (issue #3022)
		if (list.$urlParams.where) {
			var $filterBind = col.$item.$bind;
			var exp = syra_site.filterMaker.parseWhere(list.$urlParams.where, col.$item.$bind);
			...
			if (exp && $filterBind == exp.$bind) {
				...
				if ($isRefField) {
					list.filterRecord.dataset[col.$item.$bind] = {};
					list.filterRecord.dataset[col.$item.$bind][$filterBind] = exp.$value;
				} else {
					list.filterRecord.dataset[col.$item.$bind] = exp.$value;
				}
