...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	validateDisplay: function() {
		if (this.cols.$menusBox) {
			var record = this.records && this.records.length && this.records[0];
			if (record && record.mnPickers) {
				var width = 0;
				if (record.mnPickers.edit) {
					width += 18;
				}
				if (record.mnPickers.menus) {
					width += 18;
				}
				if (width != this.cols.$menusBox.colWidth) {
					this.cols.$menusBox.colWidth = this.cols.$menusBox.width = width;
					delete this.cols.$menusBox.scrollWidthValue;
				}
			}
		}
