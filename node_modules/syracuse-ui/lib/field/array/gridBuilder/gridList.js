...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	resizeList: function() {
		...
		MultiList.prototype.resizeList.call(this);
	},
	drawBox: function() {
	...
	onItemInOut: function(onEnter, event, currentTarget) {
		if (this.isColReordering && currentTarget.syraCell) {
			var ddAgent = document.site.ddManager && document.site.ddManager.ddAgent;
			if (ddAgent && ddAgent.onColumnHeaderEnterLeave) {
				ddAgent.onColumnHeaderEnterLeave(currentTarget.syraCell, onEnter);
			}
		}
	},
	removeColumn: function($bind, designing) {
		...
		if (index >= 0) {
			...
			document.site.dom.removeChild(col.groupBody);
			document.site.dom.removeChild(col.groupHead);
			document.site.dom.removeChild(col.titleCell);
			if (this.filterRecord) {
				document.site.dom.removeChild(this.filterRecord.dataRow.childNodes[index]);
			}
			for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
				var record = this.records[ii];
				document.site.dom.removeChild(record.dataRow.childNodes[index]);
			}
			this.resizeList();
		}
	},
	...
	toggleFixRecordCell: function(cellIndex, isFixed) {
		for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
			document.site.dom.toggleClass(this.records[ii].dataRow.childNodes[cellIndex], "s-grid-col-fixed", isFixed);
		}
	},
			...
			if (this.lastColFixed) {
				...
				for (var ii = 0, jj = lastColFixedIndex; ii < jj; ii++) {
					...
					if (!col.$isFixed) {
						...
						document.site.dom.toggleClass(col.titleCell, "s-grid-col-fixed", true);
				...
				for (var ii = jj + 1, jj = this.orderCols.length; ii < jj; ii++) {
					...
					if (col.$isFixed) {
						...
						document.site.dom.toggleClass(col.titleCell, "s-grid-col-fixed", false);
	...
	_makeTitleRow: function() {
		...
		if (this.titleRow) {
			document.site.dom.empty(this.titleRow);
		} else {
			this.titleRow = this._thead.appendChild(document.createElement("tr"));
		}
		document.site.dom.empty(this.bodyColgroup);
		document.site.dom.empty(this._headColgroup);
	...
	validateDisplayfdfd: function() {
		if (this.fitToWidFields) {
			if (!this.sizeBody) {
				this.sizeBody = document.createElement("table");
				this.sizeBody.className = this.$skin + "-size-body";
				this.sizeBody.setAttribute("cellspacing", "0");
				this.sizeBody.setAttribute("cellpadding", "0");
				this.sizerow = document.createElement("tr");
				this.sizeBody.appendChild(this.sizerow);
				this.tableSlot.appendChild(this.sizeBody);
			}
			document.site.dom.empty(this.sizerow);
			for (var ii = 0, jj = this.orderCols.length; ii < jj; ii++) {
				var col = this.orderCols[ii];
				if (col.$bind) {
					var field = this.fitToWidFields[col.$bind];
					if (field && field.field.domItem) {
						col.widthValue = this.sizerow.appendChild(field.field.domItem.cloneNode(true)).scrollWidth + 16;
						col.groupHead.style.width = col.widthValue + "px";
						col.groupBody.style.width = col.widthValue + "px";
					}
				}
			}
			if (this.scroller && this.scroller._hscroller && this.scroller._hscroller.scrollLeft) {
				this.scroller.scrollFixedColumns();
			}
		}
	},
	...
	dispose: function() {
		...
		this.sizerow = this.sizeBody = this.body = this.bodyColgroup = this.bodyTable = this._headTable = this._thead = null;
