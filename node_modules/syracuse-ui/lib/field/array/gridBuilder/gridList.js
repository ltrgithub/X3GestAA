...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	initializeList: function() {
		...
		this.$popupCard = this.$item.$popupCard;
	...
	reorderColumns: function(colSource, colTarget, dropAtEnd) {
		...
		this.saveArticleUpdate(true, this.saveUserPreferences("$cols"));
	},
	saveUserPreferences: function(key) {
		...
		switch (key) {
			case "$cols":
				this.$userPreferences.$cols = this.$userPreferences.$cols || [];
				var $items = this.$item.$layout.$items;
				for (var ii = 0, jj = $items.length; ii < jj; ii++) {
					var $col = {
						$bind: $items[ii].$bind
					};
					if ($items[ii].$width !== undefined) {
						$col.$width = $items[ii].$width;
					}
					this.$userPreferences.$cols.push($col);
				}
				break;
		}
