...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	resizeList: function(resize) {
		if (!this.disposed) {
			...
			if (mustBeResized) {
				mustBeResized = this.isTableSlotHidden || (this.scroller && this.scroller.onListResize());
				if (mustBeResized) {
	...
	appendFieldCol: function(slot, $item, $field) {
		...
		if (width) {
			...
			col.userPreferenceWidth = this.$userPreferences.$colWidths && this.$userPreferences.$colWidths[$item.$bind];
		} else {
	...
	_makeTitleRow: function() {
		...
		syra_site.dom.empty(this._headColgroup);
	...
	dispose: function() {
		...
		if (this.filterRecord) {
			this.removeRecord(this.filterRecord);
