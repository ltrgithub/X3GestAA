...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	resizeList: function(resize) {
		if (!this.disposed) {
			...
			if (mustBeResized) {
				mustBeResized = this.scroller && this.scroller.onListResize();
				if (mustBeResized) {
	...
	appendFieldCol: function(slot, $item, $field) {
		...
		if (width) {
			...
			col.widthValue = fieldHelper.getCellFieldWidth($field, titleText.length, isSortable);
			col.widthValue += 8; //for padding
			if (isSortable) {
				col.widthAdded -= 16; //space added for sort
			}
			var hasFilter;
			...
			if (this.$isEditMode || hasFilter) {
				if ($field.$type == "application/x-choice" || $field.$type == "application/x-date" || $field.$type == "application/x-time" || $field.$type == "application/x-reference") {
					col.widthAdded += 18;
				}
			}
			if (this.$isEditMode) {
	...
	_makeTitleRow: function() {
		...
		for (var ii = 0, jj = $items.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isExcluded) {
				...
				if (!this.maxCol || col.widthValue > this.maxCol.widthValue) {
					this.maxCol = col;
				}
			}
		}
