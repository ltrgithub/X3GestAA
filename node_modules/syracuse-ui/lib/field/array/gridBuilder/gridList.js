...
var FilterRecord = require("syracuse-ui/lib/field/filter/filterRecord").FilterRecord;
...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	resizeScrollView: function(newWidth) {
		var newWidth = newWidth || this.bodyTableSlot.clientWidth;
		if (newWidth) {
			if (!this.isTableSlotHidden) {
				if (this.scroller.$fixedBodyHeight) {
					if (this.bodyTableSlot.style.overflowY != "auto") {
						this.bodyTableSlot.style.overflowY = "auto";
					}
					var height = this.bodyTableSlot.clientHeight;
					this.bodyTableSlot.style.height = ((Math.abs(this.scroller.$fixedBodyHeight - height) < 2) ? height : this.scroller.$fixedBodyHeight) + "px";
				} else {
					if (this.$fitContainer) {
						if (this.bodyTableSlot.style.overflowY != "auto") {
							this.bodyTableSlot.style.overflowY = "auto";
						}
						var savScrollTop = this.bodyTableSlot.scrollTop;
						this.bodyTableSlot.style.height = "";
						var bodyHeight = this.bodyTableSlot.clientHeight;
						var diff = this.layoutSlot.scrollHeight - this.layoutSlot.clientHeight;
						if (diff && !this.tableSlotWidth) { //never init
							diff += 17;
						}
						bodyHeight -= diff;
						this.bodyTableSlot.style.height = bodyHeight + "px";
						this.bodyTableSlot.scrollTop = savScrollTop;
					} else {
						if (this.$isDockedHeight) {
							...
							if (this.bodyTableSlot.style.overflowY == "auto") {
								this.bodyTableSlot.style.overflowY = "";
								this.bodyTableSlot.style.height = "";
							}
						}
					}
				}
				if (!this.tableSlotWidth && (this.bodyTableSlot.scrollHeight > (this.bodyTableSlot.clientHeight + 1))) {
					newWidth -= 17;
				}
				this._setTableSlotWidth(newWidth);
				if (syra_grid.columnFitter.fit(this)) {
					this.scroller.ensureColumnsHScoll();
				}
				...
				this.scroller.scrollAreaWidth = this.colsWidth - this.tableSlotWidth;
				this.hscrollBody.style.width = this.colsWidth + "px";
				var isHScroll;
				this.hscroller.style.display = (isHScroll = this.scroller.scrollAreaWidth > 0) ? "" : "none";

				if (this.bodyTableSlot.scrollHeight > (this.bodyTableSlot.clientHeight + 1)) {
					this.headTableSlot.style.marginRight = "17px";
					this.hscrollBody.style.width = (this.colsWidth + 17) + "px";
					this._showHeaderCorner(true);
				} else {
					this.headTableSlot.style.marginRight = "";
					this._showHeaderCorner(false);
				}!this.page.dialogWrapper && this.scroller._appendButtons(isHScroll);
			} else {
				this._setTableSlotWidth(newWidth);
			}
		}
	},
	_setTableSlotWidth: function(width) {
		if (width != this.tableSlotWidth) {
			this.tableSlotWidth = width;
			/*this.topbar.style.width = width + "px"
             this.headTable.style.width = width + "px"
             this.bodyTable.style.width = width + "px";*/
			this.headTable.style.tableLayout = "fixed";
			this.bodyTable.style.tableLayout = "fixed";
		}
	},
	_showHeaderCorner: function(show) {
		if (show) {
			if (!this._headerCorner) {
				this._headerCorner = document.createElement("div");
				this._headerCorner.className = this.$skin + "-head-corner";
				this.headTableSlot.parentNode.appendChild(this._headerCorner);
			}
			this._headerCorner.style.height = this.headTableSlot.clientHeight + "px";
	...
	drawBox: function() {
		...
		syra_grid.columnFitter.reset(this);
	...
	onReloading: function($item) {
		...
		this.cols = {};
		this._makeTitleRow();
		return MultiList.prototype.onReloading.call(this, $item);
	},
		...
		if (this.cols.$menus) {
			...
			if (record && record.mnPickers) {
				...
				if (width != this.cols.$menus.colWidth) {
					...
					delete this.cols.$menus.scrollWidthValue;
				}
			}
		}
	...
	emptyBody: function(addEmptySlot, onFetch) {
		...
		if (addEmptySlot) {
			if (!this.emptyDataItem) {
				...
				this.bodyTableSlot.appendChild(this.emptyDataItem);
			}
	...
	onItemInOut: function(onEnter, event, target) {
		if (target.syraCell) {
			...
			if (onEnter) {
				if (col.title.title) {
					col.title.syraTipTitle = col.title.title;
					col.title.title = "";
					if (col.title.clientWidth != col.title.scrollWidth) {
						syra_site.siteFunctions.toggleTitleTip(col.title.syraTipTitle, col.title);
					}
				}
			} else {
				if (col.title.syraTipTitle) {
					col.title.title = col.title.syraTipTitle;
				}
	...
	resetRecordReorder: function(record) {
		...
		for (var ii = 0, jj = this.orderCols.length; ii < jj; ii++) {
			var $bind = this.orderCols[ii].$bind;
	...
	resetReorder: function() {
		if (this.$origItem.$columns) {
			...
			var $items = this.$item.$layout.$items = [];
			for (var ii = 0, jj = this.$origItem.$columns.length; ii < jj; ii++) {
				var $bind = this.$origItem.$columns[ii];
				var $item = this.bindMap[$bind];
				if ($item) {
			...
			//find first bind Col
			for (var ii = 0, jj = this.orderCols.length; ii < jj; ii++) {
				if (this.orderCols[ii].$bind) {
					if (ii > 0) {
						this.orderCols.splice(ii);
					} else {
						this.orderCols = [];
					}
					break;
			...
			for (var ii = 0, jj = this.$origItem.$columns.length; ii < jj; ii++) {
				var $bind = this.$origItem.$columns[ii];
				this.orderCols.push(this.cols[$bind]);
			}
			for (var ii = 0, jj = this.orderCols.length; ii < jj; ii++) {
				var col = this.orderCols[ii];
			...
			if (this.filterRecord) {
				this.resetRecordReorder(this.filterRecord);
			}
			for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
				this.resetRecordReorder(this.records[ii]);
			}
		}
	...
	reorderColumns: function(colSource, colTarget, dropAtEnd) {
		...
		if (sourceIndex >= 0) {
			...
			var isNotFixed;
			var prevLastColFixed = this.lastColFixed;
			this.lastColFixed = null;
			for (var ii = 0, jj = this.orderCols.length; ii < jj; ii++) {
				var col = this.orderCols[ii];
				if (col.$isFixed) {
					if (isNotFixed) {
						this.setFixedCol(col, ii, false);
					} else {
			...
			if (prevLastColFixed != this.lastColFixed) {
				prevLastColFixed && this.setLastFixedCol(prevLastColFixed, false);
				this.lastColFixed && this.setLastFixedCol(this.lastColFixed, true);
			}
		}
		this.savePreferences(true, "$columns");
		this.resizeScrollView();
	},
	setLastFixedCol: function(col, isLastFixed) {
		var index = this.orderCols.indexOf(col);
		syra_site.dom.toggleClass(col.titleCell, "s-grid-col-fixed-last", isLastFixed);
		for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
			syra_site.dom.toggleClass(this.records[ii].dataRow.childNodes[index], "s-grid-col-fixed-last", isLastFixed);
		}
	},
	setFixedCol: function(col, index, isFixed) {
		if (isFixed) {
			col.$isFixed = col.$item.$isFixed = isFixed;
		} else {
			delete col.$isFixed;
			delete col.$item.$isFixed;
		}
		syra_site.dom.toggleClass(col.titleCell, "s-grid-col-fixed", isFixed);
		this.toggleFixRecordCell(index, isFixed);
	},
	_setColumnTitle: function(col, $title) {
		var text = this.getLocalizedTitle($title || "");
		if (text.length > 1) {
			text = text.substr(0, 1).toUpperCase() + text.substr(1);
		}
		syra_menus.setButtonTitle(col.title, text);
		return text;
	},
	appendFieldCol: function(slot, $item, $field) {
		...
		col.slot = slot;
		//avoid sort for tree
		if (!this.treeDecorator && $field.$capabilities && ($field.$capabilities.indexOf("sort") >= 0)) {
			col.isSortable = true;
			col.title = this.capabilityMaker.sorter.addPicker(this, col, $item);
		} else {
		...
		col.$field = $field;
		col.$item = $item;
		syra_grid.columnFitter.setFieldWidth(this, col, $field);
	...
	_makeTitleRow: function() {
		...
		if (!this.$item.$isMenuRecordHidden) {
			...
			this.orderCols.push(this._addMenusCol(this.titleRow));
		}
		...
		this.filterRecord && this.filterRecord.loadFilter();
	...
	_appendRecord: function(options) {
		...
		syra_grid.popupCardBuilder.addOpenLink(record);
		return record;
	},
	...
	_addMenusCol: function(slot) {
		var col = this.cols.$menus = this.appendCol();
		col.slot = slot;
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if ((metaData.$fixedBodyHeight !== undefined) ||
			...
			this.scroller && this.scroller.setFixedBodyHeight(this.$item.$fixedBodyHeight);
			resize = true;
		}
		if (designing) {
	...
	setFocus: function(select, name, $serverIndex) {
		if (syra_grid.popupCardBuilder.isOpened(this) && this.selector.uuid != this.clientDataset[$serverIndex].$uuid) {
	...
	dispose: function() {
		...
		this.bindMap = this._headerCorner = this.headTableSlot = this.bodyTableSlot = this.hscroller = this.hscrollBody = null;
		...
		this.cardDecorator = this.emptyData = this.filterRecord = this.scroller = this.$colBinds = null;
