...
var popupCardBuilder = require('./card/popupCardBuilder');
...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	initializeList: function() {
		...
		this.popupCardBuilder = popupCardBuilder;
		this.$field.$isContentFusionList && popupCardBuilder.makeLayout(this);

		(this.cardDecorator = new CardDecorator()).load(this);
	},
	resizeScrollView: function() {
		var newWidth = this.bodyTableSlot.clientWidth;
		if (newWidth) {
			if (!this.isTableSlotHidden) {
				...
				if (this.columnFitter.fit()) {
					this.scroller.ensureColumnsHScoll();
				}
				if (this.page.lookupField == this) {
					if (this.colsWidth < this.tableMaxWidth) {
						var oldWidth = this.bodyTable.style.width;
						this.headTable.style.width = "";
						this.bodyTable.style.width = "";
						if ((this.tableSlot.clientWidth - this.bodyTable.clientWidth) > 17) {
							this.headTable.style.width = oldWidth;
							this.bodyTable.style.width = oldWidth;
						}
					...
					if (this.maxWidthCol) {
						var width;
						if (this.colsWidth < this.tableSlotWidth) {
							width = this.maxWidthCol.proportionalWidth = this.maxWidthCol.colWidth + (this.tableSlotWidth - this.colsWidth);
						} else {
							if (this.maxWidthCol.proportionalWidth) {
								width = this.maxWidthCol.colWidth;
								delete this.maxWidthCol.proportionalWidth;
							}
						}
						if (width !== undefined) {
							this.setColWidth(this.maxWidthCol, width + "px");
						}
					}
				}
				this.scroller.scrollAreaWidth = this.colsWidth - this.tableSlotWidth;
				this.hscrollBody.style.width = this.colsWidth + "px";
				this.hscroller.style.display = this.scroller.scrollAreaWidth > 0 ? "" : "none";
	...
	_appendRecord: function(options) {
		...
		this.$item.$popupCard && popupCardBuilder.addOpenLink(record);
		return record;
	},
	...
	setFocus: function(select, name, $serverIndex) {
		if (this.popupCardBuilder && this.popupCardBuilder.isOpened(this) && this.selector.uuid != this.clientDataset[$serverIndex].$uuid) {
	...
	dispose: function() {
		...
		this.popupCardBuilder && this.popupCardBuilder.dispose(this);
		...
		this._headerCorner = this.headTableSlot = this.bodyTableSlot = this.hscroller = this.hscrollBody = null;
