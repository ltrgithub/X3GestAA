...
exports.GridList = helpers.defineClass(GridList, MultiList, {
	...
	onItemEnterLeave: function(onEnter, event, currentTarget) {
	...
	removeColumn: function($bind, designing) {
		...
		if (index >= 0) {
			...
			if (col.titleCell) {
				col.titleCell.syrainout = col.titleCell.syraCell = null;
			}
	...
	fitToWidth: function(autoFit) {
		if (this.records && this.records.length) {
			...
			var s = first.menusBags && first.menusBags.length > 0 ? first.menusBags[0].domItem.clientWidth : 0;
			var currentSize = s;
			if (first.menuItems) {
				var item = Object.keys(first.menuItems);
				for (var j = 0; j < item.length; j++) {
					s = first.menuItems && first.menuItems.length > 0 ? first.menuItems[item[j]][0].domItem.clientWidth : 0;
					currentSize += s;
				}
			}
			var colSize = {};
					...
					if (item.$field) {
						if (item.fieldValue) {
							if (colSize[item.$item.$bind]) {
								// get the max
								colSize[item.$item.$bind] = Math.max(item.fieldValue.scrollWidth + 8, colSize[item.$item.$bind]);
							} else
								colSize[item.$item.$bind] = item.fieldValue.scrollWidth + 8;
						}
					} else {
						if (!colSize[item.$item.$bind])
