...
exports.emptyBody = function(builder, addEmptySlot, redraw){
...
exports.makeTitleRow = function(builder){
    ...
    builder.list._store.filterCapability.ensureQuickFilter();
    ...
    if (builder.list.$item.$isRowIndexVisible) {
        ...
        builder.list._store.filterCapability.appendFlagCell(builder.cols.$rowIndex.slot);
    }
    if (!builder.list.$item.$isMenuRecordHidden) {
        builder._orderCols.push(builder._appendContextMenusCol(builder._body.appendChild(document.createElement("tr"))));
        builder.list._store.filterCapability.appendEmptyCell(builder.cols.$contextMenus.slot);
    }
    if (builder.list._store.selector.appendToTitle) {
        ...
        builder.list._store.filterCapability.appendEmptyCell(builder.cols.$recordSelector.slot);
    }
    builder.parseItems(function($item, $field){
        var row = builder._body.appendChild(document.createElement("tr"));
        builder._orderCols.push(builder.appendFieldCol(row, $item, $field));
        builder.list._store.filterCapability.appendFieldCell($item, $field);
    });
    ...
    if (builder.list._store.filterCapability.filterRecord) {
