			...
			$$row.delegate("th", "mouseenter" + nameSpace + " mouseleave" + nameSpace, function(event) {
				if (this.syraFieldBind) {
					if (event.type == "mouseenter") {
						self.colTarget = self.list.findEventCol(event);
						self.targetX = self.colTarget.titleCell.getBoundingClientRect().left - 4; //4 cue width /2
						if (self.list.orderCols[self.list.orderCols.length - 1] == self.colTarget) {
						...
						if (self.colTarget == self.list.findEventCol(event)) {
							self.colTarget = null;
