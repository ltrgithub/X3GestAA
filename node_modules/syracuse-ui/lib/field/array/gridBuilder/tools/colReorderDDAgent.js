...
exports.ColReorderDDAgent = helpers.defineClass(ColReorderDDAgent, null, {
	start: function(dropItem) {
		...
		this.boundary = this.builder.titleRow.getBoundingClientRect();
		this.info = document.createElement("div");
		this.cueTop = document.createElement("div");
		this.cueBottom = document.createElement("div");
		this.scrollBoundary = this.builder.scroller.getScrollBoundary();
	...
	bindEvent: function(bind) {
		var self = this;
		var nameSpace = ".reorder" + this.builder.list.id;
		var $$row = $(this.builder.titleRow);
		if (bind) {
			$$row.delegate("th", "mouseenter" + nameSpace + " mouseleave" + nameSpace, function(event) {
				if (this.syraFieldBind) {
					if (event.type == "mouseenter") {
						self.colTarget = self.builder.findEventCol(event);
						self.targetX = self.colTarget.titleCell.getBoundingClientRect().left - 4; //4 cue width /2
						if (self.builder.orderCols[self.builder.orderCols.length - 1] == self.colTarget) {
							self.lastColBoundaryX = self.targetX + self.colTarget.realWidth / 2;
						}
						self.cueTop.style.left = self.cueBottom.style.left = self.targetX + "px";
						self.cueTop.style.display = self.cueBottom.style.display = "";
					} else {
						if (self.colTarget == self.builder.findEventCol(event)) {
							self.colTarget = null;
							delete self.lastColBoundaryX;
							self.cueTop.style.display = self.cueBottom.style.display = "none";
						}
					}
					document.site.toggleClass(self.info, "s-drag-ok", self.colTarget != null);
	...
	onDragMouseUp: function(target, event) {
		...
		if (this.colTarget) {
			this.builder.reorderColumns(this.reordeCol, this.colTarget, this.dropAtEnd);
		}
	...
	dispose: function() {
		...
		this.colTarget = this.boundary = this.autoScrolTimeout = this.reordeCol = this.builder = this.info = this.cueTop = this.cueBottom = this.scrollBoundary = null;
	}
});