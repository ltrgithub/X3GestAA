...
exports.ColReorderDDAgent = helpers.defineClass(ColReorderDDAgent, null, {
	start: function(dropItem) {
		...
		this.scrollBoundary = syra_site.dom.getBoundingClientRect(this.list.bodyTableSlot);
		this.scrollBoundary.scrollWidth = this.list.hscroller.scrollWidth;
		this.scrollBoundary.scrollHeight = this.list.bodyTableSlot.scrollHeight;
		...
		self.autoScrolTimeout = setTimeout(function() {
			if (self.scrollBoundary) {
				if (event.pageY > (self.scrollBoundary.bottom - margin)) {
					var scrollTop = self.list.bodyTableSlot.scrollTop;
					if (scrollTop < (self.scrollBoundary.scrollHeight - self.scrollBoundary.height)) {
						self.list.bodyTableSlot.scrollTop = Math.min(scrollTop + step, self.scrollBoundary.scrollHeight - self.scrollBoundary.height);
					}
				} else {
					if (event.pageY < (self.scrollBoundary.top + margin)) {
						var scrollTop = self.list.bodyTableSlot.scrollTop;
						self.list.bodyTableSlot.scrollTop = Math.max(scrollTop - step, 0);
					}
				}
				if (event.pageX > (self.scrollBoundary.right - margin)) {
					var scrollLeft = self.list.hscroller.scrollLeft;
					if (scrollLeft < (self.scrollBoundary.scrollWidth - self.scrollBoundary.width)) {
						self.list.hscroller.scrollLeft = Math.min(scrollLeft + step, self.scrollBoundary.scrollWidth - self.scrollBoundary.width);
					}
				} else {
					if (event.pageX < (self.scrollBoundary.left + margin)) {
						self.list.hscroller.scrollLeft = Math.max(self.list.hscroller.scrollLeft - step, 0);
					}
				}
			}
		}, 10);
