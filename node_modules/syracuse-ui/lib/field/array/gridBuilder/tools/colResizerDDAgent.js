	...
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (this.top - 20)) && (event.pageY <= (this.bottom + 20))) {
			...
			if (newWidth > 50) {
				...
				this.list.selColWidth(this.resizedCol, newWidth + "px");
				cursor = "e-resize";
				if (this.list.maxWidthCol.proportionalWidth) {
					if (this.list.maxWidthCol != this.resizedCol) {
						...
						this.list.selColWidth(this.list.maxWidthCol, this.list.maxWidthCol.newProportionalWidth + "px");
					} else {
						this.rightCol.newWidth = this.rightCol.colWidth - (event.pageX - this.startX);
						this.list.selColWidth(this.rightCol, this.rightCol.newWidth + "px");
					}
				}
			}
			var widthTotal = 0;
