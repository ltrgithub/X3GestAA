...
exports.ColResizerDDAgent = helpers.defineClass(ColResizerDDAgent, null, {
	...
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (this.top - 20)) && (event.pageY <= (this.bottom + 20))) {
			...
			if (newWidth > 50) {
				...
				this.resizedCol.groupHead.style.width = newWidth + "px";
				this.resizedCol.groupBody.style.width = newWidth + "px";
				cursor = "e-resize";
				if (this.list.maxWidthCol.proportionalWidth && this.list.maxWidthCol != this.resizedCol) {
					...
					this.list.maxWidthCol.groupHead.style.width = this.list.maxWidthCol.newProportionalWidth + "px";
					this.list.maxWidthCol.groupBody.style.width = this.list.maxWidthCol.newProportionalWidth + "px";
				}
			}
			var widthTotal = 0;
			for (var ii = 0, jj = this.list.scrolledCols.length; ii < jj; ii++) {
				widthTotal += (this.list.scrolledCols[ii].newWidth || this.list.maxWidthCol.newProportionalWidth || this.list.maxWidthCol.proportionalWidth || this.list.scrolledCols[ii].colWidth);
			}
	...
	onDragMouseUp: function(target, event) {
		...
		if (this.resizedCol.newWidth !== undefined) {
			...
			this.list.saveListDesign(true, preferencesHelper.save(this.list, "$columnWidths"));
