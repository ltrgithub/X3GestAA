...
exports.ColResizerDDAgent = helpers.defineClass(ColResizerDDAgent, null, {
	start: function(dropItem) {
		...
		this.startX = this.resizedCol.resizer.getBoundingClientRect()[syra_site.isDocumentRTL ? "right" : "left"];
	...
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (this.top - 20)) && (event.pageY <= (this.bottom + 20))) {
			var newWidth = this.resizedCol.colWidth + (syra_site.isDocumentRTL ? (this.startX - event.pageX) : (event.pageX - this.startX));
			if (newWidth > 50) {
			...
			for (var ii = 0, jj = this.list.scrolledCols.length; ii < jj; ii++) {
				widthTotal += (this.list.scrolledCols[ii].newWidth || this.list.scrolledCols[ii].colWidth);
			}
