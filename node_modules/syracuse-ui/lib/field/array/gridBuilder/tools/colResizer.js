...
var ColResizerDDAgent = require("./colResizerDDAgent").ColResizerDDAgent;
...
exports.ColResizer = helpers.defineClass(ColResizer, null, {
	_findCol: function(event) {
		...
		return this.builder.cols[target.syraFieldBind];
	},
	isDraggable: function(target, event) {
		if (target.syraListId == this.builder.list.id) {
			return {
				resizer: this,
				resizedCol: this._findCol(event)
			};
		}
		return null;
	},
	onDragMove: function(target, event) {
		var dropableItem = document.site.ddManager.dropableItem;
		if (dropableItem && dropableItem.resizer && dropableItem.resizer == this) {
			document.site.ddManager.start(this, ColResizerDDAgent, $(this.builder.list.item));
			return;
		}
		return true;
	},
	setResizable: function(builder) {
		...
		return;
		self.$$row.delegate("th", "mousedown", function(event) {
		...
		}).delegate("th", "mouseenter mouseleave", function(event) {
			if (this.syraFieldBind) {
				if (self.drag) {
	...
	dispose: function() {
		...
		this.builder = this.$$row = this.drag = null;
	}
});