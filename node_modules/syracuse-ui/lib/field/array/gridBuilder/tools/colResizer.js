...
helpers.defineClass(ColResizerDDAgent, null, {
	start: function(dropItem) {
		...
		this.maxContainerWidth = _list.tableSlot.parentNode.clientWidth;
		this.maxTableSlotWidth = _list.tableSlot.getBoundingClientRect().width;
	},
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (_colReszing.rect.top - 20)) && (event.pageY <= (_colReszing.rect.bottom + 20))) {
			...
			if (this.isLastCol) {
				...
				_list.tableSlot.style.maxWidth = Math.min((this.maxTableSlotWidth + diff), this.maxContainerWidth) + "px";
			}
		}
