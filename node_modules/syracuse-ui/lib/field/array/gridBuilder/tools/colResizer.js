...
var _colReszing, _list, _startX, _startWidth, _dataCell, _maxContainerWidth, _freezeWidth, _maxTableSlotWidth;
...
helpers.defineClass(ColResizerDDAgent, null, {
	start: function(dropItem) {
		...
		if (_colReszing.table.isFreeze) {
			_freezeWidth = _list.builder.freezeTableSlot.getBoundingClientRect().width;
		}
		_maxContainerWidth = _list.tableSlot.parentNode.clientWidth;
		_maxTableSlotWidth = _list.tableSlot.getBoundingClientRect().width;
	},
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (_colReszing.rect.top - 20)) && (event.pageY <= (_colReszing.rect.bottom + 20))) {
			...
			if (this.isLastCol) {
				_list.tableSlot.style.maxWidth = Math.min((_maxTableSlotWidth + diff), _maxContainerWidth) + "px";
			}
			if (_colReszing.table.isFreeze) {
				_list.builder.freezeTableSlot.style.width = _freezeWidth + diff + "px";
			}
		}
