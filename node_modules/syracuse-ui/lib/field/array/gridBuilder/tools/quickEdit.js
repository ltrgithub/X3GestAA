...
helpers.defineClass(QuickEdit, null, {
	load: function(record, $itemPage) {
		...
		if (this.list) {
			this.record = record;
			var cardDecorator = this.list.cardDecorator;
			cardDecorator && cardDecorator.isRowCardMode && cardDecorator.toggleRowCard(record, false);
			this.card = this.list.fomatter.builder.createCardRow(record);
			this.card.cell.className = record.boxParent.$skin + "-inline-page-cell";
	...
	confirmClose: function(onConfirmed) {
		var self = this;
		if (self.page.isWorkingCopyUpdated()) {
...
exports.toggle = function(record, confirmed) {
	if (record.list.quickEdit) {
		if (!confirmed && !record.list.quickEdit.confirmClose()) {
			return;
		}
