...
function _findFieldEditColWidth(builder, records, col, firstCell) {
	var firstField = records[0] && records[0].fieldCellsMap[col.$bind];
	var pickerWidth = 0;
	if (firstField && firstField.mnPickers) {
		var pickers = Object.keys(firstField.mnPickers);
		pickerWidth = pickers.length && (pickers.length * firstField.mnPickers[pickers[0]].clientWidth);
	}
	if (builder.isAdjustedToContent) {
		col.colWidth = 0;
	} else {
	...
	if (width && syra_site.isTabletDevice) {
		width += 10; //add padding for specific buttons, mailny for number
	}
...
exports.setColumnWidth = function(col, records, $prefWidths, firstCell) {
	...
	if (col.colWidth) {
			...
			if (col.useContentWidth && firstCell) {
				...
				if (col.$field) {
					if (builder.list.$isEditMode) {
						col.colWidth = _findFieldEditColWidth(builder, records, col, firstCell);
					} else {
		...
		if (!col.titleWidth) {
			if (col.titleLabel) {
				col.titleWidth = col.titleLabel.scrollWidth + (col.sortLink ? _widths.titleCell.sort : _widths.titleCell.noSort);
			} else {
