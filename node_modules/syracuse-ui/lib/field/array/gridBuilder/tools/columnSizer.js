...
function _findMaxWidth(maxField) {
	...
	if (maxField) {
		if (maxField.input) {
			width = maxField.input.scrollWidth + 12;
		} else {
		...
		switch (col.$field.$type) {
			...
			case "application/x-datetime":
				if (firstField && firstField.localFormat) {
					width = firstField.localFormat.length * 8;
				}
				break;
			case "application/x-quantity":
			case "application/x-reference":
...
exports.setColumnWidth = function(col, records, $prefWidths, firstCell) {
	...
	col.colWidth = $prefWidths[col.$bind] || (col.$item && (col.$item.$fixWidth || col.$item.$width)) || (col.$field && col.$field.$width);
