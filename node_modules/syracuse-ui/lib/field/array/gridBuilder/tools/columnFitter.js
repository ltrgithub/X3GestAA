...
exports.setFieldWidth = function(list, col, $field) {
	var width = col.$item.$fixWidth || col.$item.$width || $field.$width;
	if (width == "NaNpx") {
		width = null;
	}
	col.widthAdded = 0;
	if (width) {
		...
		var hasFilter;
		if (list.hasFieldFilterCapability($field)) {
	...
	if (lists.length) {
		for (var ii = 0, jj = lists.length; ii < jj; ii++) {
			...
			if (list.isGridList) {
				hasFit = true;
				if (list.$isEditMode) {
					if (!list.resizedByShortCut) {
						list.resizedByShortCut = true;
						list.adjusted = {
							fields: {}
						};
						...
						_clearColWidths(list, list.adjusted.fields);
						_clearColWidths(list, list.$userPreferences && list.$userPreferences.$columnWidths);
						delete list.adjusted;
						delete list.resizedByShortCut;
					}
				} else {
					exports.reset(list);
					for (var mm = 0, kk = list.records.length; mm < kk; mm++) {
	...
	if (true || !list.colFitter.isFitted) {
		...
		for (var ii = 0, jj = list.orderCols.length; ii < jj; ii++) {
			...
			if (col.$isHidden) {
				...
				if (col.$bind) {
					if ($columnWidths[col.$item.$bind]) {
						...
						if (list.$isEditMode) {
							...
							if (field && field.field.domItem && !col.$item.$fixWidth) {
				...
				if (col.$isFixed) {
					...
					list.scrollTable.cols.push(col);
					scrolledWidth += col.colWidth;
				}
		...
		if (list.fixedCols.length && (list.tableSlotWidth < list.colsWidth)) {
			...
			while (list.fixedCols.length) {
				var space = list.tableSlotWidth - fixedWidth;
				if (space < 100 && scrolledWidth > (space + 100)) {
					var col = list.fixedCols.pop();
					list.scrollTable.cols.unshift(col);
					scrolledWidth += col.colWidth;
					fixedWidth -= col.colWidth;
				} else {
					break;
		...
		var width = 0;
		for (var ii = 0, jj = list.scrollTable.cols.length; ii < jj; ii++) {
			var col = list.scrollTable.cols[ii];
			col.minX = width;
			width += col.colWidth;
			col.maxX = width;
		}
...
exports.dispose = function(list) {
	if (list) {
		list.colFitterRow = list.colFitter = list.fixedCols = null;
	}
};