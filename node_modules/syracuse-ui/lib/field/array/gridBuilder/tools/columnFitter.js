...
exports.setFieldWidth = function(list, col, $field) {
	...
	if (!syra_site.isTabletDevice) {
		width = col.$item.$fixWidth || col.$item.$width || $field.$width;
	}
	...
	if (width) {
		...
		if (list.$isEditMode) {
			...
			if (syra_site.isTabletDevice) {
				col.widthAdded += 10; //add padding for specific buttons, mailny for number
			}
		}
		if (col.widthAdded) {
			...
			if (col.$isHidden) {
				...
				if (col.$bind) {
					if ($columnWidths[col.$item.$bind]) {
						col.scrollWidthValue = col.colWidth = parseInt(($columnWidths[col.$item.$bind] + "").replace("px", ""), 10);
					} else {
						if (list.$isEditMode) {
							if (list.adjusted) {
								var field = list.adjusted.fields[col.$bind];
