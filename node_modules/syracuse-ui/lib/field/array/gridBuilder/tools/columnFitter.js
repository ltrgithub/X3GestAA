...
exports.applyResizeShortcutOld = function(item) {
	var lists = [];
	if (item.page == item) {
		lists = _getLists();
	}
			...
			if (list.isGridList) {
				...
				if (list.$isEditMode) {
					...
					for (var ii = 0, jj = list.records.length; ii < jj; ii++) {
						exports.calculateMaxFieldLength(list.records[ii], list.adjusted);
					}
					for (var ii = 0, jj = list.orderCols.length; ii < jj; ii++) {
						var col = list.orderCols[ii];
						if (col.$bind) {
							var field = list.adjusted.fields[col.$bind];
							if (!field && col.titleText) {
								list.adjusted.fields[col.$bind] = {
									valueLength: col.titleText.length
								};
							}
						}
					}
					...
					exports.reset(list);
					for (var ii = 0, jj = list.records.length; ii < jj; ii++) {
						exports.calculateMaxFieldLength(list.records[ii]);
					}
				}
				list.resizeArticle(true);
			}
