...
exports.Scroller = helpers.defineClass(Scroller, null, {
	...
	_scrollCols: function(left) {
		...
		self._scrollColsTimeout = setTimeout(function() {
			...
			var hScrollLeft = self.getScrollLeft();
			hScrollLeft = left ? hScrollLeft - offset : hScrollLeft + offset;
			if (syra_site.isDocumentRTL) {
				hScrollLeft += self.maxScroll;
			}
			self.list.hscroller.scrollLeft = hScrollLeft;
		}, 20);
	},
	...
	_appendButtons: function() {
		var show;
		if (this.list.page.scrollview && this.list.isScrollButtonEnabled !== false && !this.list.treeDecorator) {
			if (this.list.records.length > 20) {
				if (!this.leftBtn) {
					this.leftBtn = syra_menus.addIconButton("", "s-list-btn-scroll-prev", "onButonClick");
					this.rightBtn = syra_menus.addIconButton("", "s-list-btn-scroll-next", "onButonClick");
	...
	_showButtons: function(show) {
		if (show != this.leftBtn.syraIsDisplay) {
			show = this.list.hscroller.scrollWidth != this.list.hscroller.clientWidth;
	...
	onPageScrollViewChange: function() {
		if (this.leftBtn) {
			...
			if (!(buttonsRect.top > scrollRect.top && buttonsRect.bottom < scrollRect.bottom)) {
				...
				this.leftBtnStyle.left = "-32px";
				this.rightBtnStyle.top = top + "px";
				this.rightBtnStyle.left = Math.min(this.list.bodyTable.clientWidth, this.list.bodyTableSlot.clientWidth) + "px";
				show = true;
			}
