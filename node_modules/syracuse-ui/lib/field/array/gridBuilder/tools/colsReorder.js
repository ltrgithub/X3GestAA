...
exports.moveCols = function(list, colSource, colTarget, dropAtEnd) {
	...
	if (sourceIndex >= 0) {
		...
		var $sourceItem = list.$item.$layout.$items.splice(list.findColItem(colSource.$bind), 1)[0];
		var $newSourceItem = list.findColItem(colTarget.$bind);
		if (dropAtEnd) {
			$newSourceItem++;
		}
		list.$item.$layout.$items.splice($newSourceItem, 0, $sourceItem);
