...
function _removeRecordCards(cardDecorator) {
	...
	var inlineColIndex;
	if (col) {
		inlineColIndex = list.builder.allColumns.indexOf(col);
		list.builder.allColumns.splice(inlineColIndex, 1);
		if (list.builder.filterRecord) {
			syra_site.dom.removeChild(list.builder.filterRecord.dataRow.childNodes[inlineColIndex]);
		}
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") {
			...
			if (metaData.$cardPosition == "row") {
				...
				this.appendRowCard();
				this._ensureMode();
				if (this.list.builder.filterRecord) {
					var td = document.createElement("td");
					td.className = "s-list-filter-cell";
					this.list.builder.filterRecord.dataRow.insertBefore(td, record.dataRow.firstChild.nextSibling);
				}
				var col = this.list.builder.freezeTable.addRowCardCol(true);
