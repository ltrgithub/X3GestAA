...
exports.CardDecorator = helpers.defineClass(CardDecorator, null, {
	...
	toggleRowCard: function(record, show) {
		document.site.dom.toggleClass(record.rowCardOpener, "s-close", !show);
	...
	onRowCardOpenerClick: function(opener) {
		...
		var $uuids = opener.syraOpenAll ? Object.keys(this.list.recordsMap) : [document.site.itemHelper.findArticle(opener).$uuid];
		for (var ii = 0, jj = $uuids.length; ii < jj; ii++) {
			this.toggleRowCard(this.list.recordsMap[$uuids[ii]], show);
		}
		document.site.dom.toggleClass(opener, "s-close", !show);
	},
	...
	_removeRecordCards: function() {
		...
		if (col) {
			...
			if (this.list.filterRecord) {
				document.site.dom.removeChild(this.list.filterRecord.dataRow.childNodes[inlineColIndex]);
			}
		...
		for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
			...
			if (record.layoutContent) {
				document.site.layoutUpdater.clearContent(record.layoutContent);
				record.layoutContent.dispose();
				if (record.rowCardRow) {
					document.site.dom.removeChild(record.rowCardRow);
				}
				record.disposeCard();
				delete record.cardItem;
			}
			if (inlineColIndex !== undefined) {
				document.site.dom.removeChild(record.dataRow.childNodes[inlineColIndex]);
			}
		}
	},
	...
	onRecordApplyChange: function(record) {
		if (!this.list.$isEditMode && record.dataset && this.list.$complexBinds.length && record.rowCardOpener) {
			var hasData;
			for (var ii = 0, jj = this.list.$complexBinds.length; ii < jj; ii++) {
	...
	onSelectRecord: function(record, selected) {
		if (this.isRowCardMode && record.rowCardRow) {
			document.site.dom.toggleClass(record.rowCardRow, "s-list-record-selected", selected);
		}
		if (selected) {
			...
			if (this.isOutCardMode) {
				document.site.dom.empty(this.outCardSlot);
	...
	removeOutCardSlot: function() {
		document.site.dom.removeChild(this.outCardSeparator);
		document.site.dom.removeChild(this.outCardSlot);
