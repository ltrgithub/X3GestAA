...
exports.CardDecorator = helpers.defineClass(CardDecorator, null, {
	...
	resizeCardItem: function(record) {
		if (record.cardItem || record.popupCardItem) {
			if (this.builder.scroller.bodySlot.clientWidth) {
				if (this.isRowCardMode && record.body) {
					record.layoutSlot.style.width = this.builder.scroller.bodySlot.clientWidth + "px";
				}
				if (record.popupCardItem) {
					record.popupCardItem._item.style.width = this.builder.scroller.bodySlot.clientWidth + "px";
				}
				record.resizeResizableChildren();
			}
		}
	},
	toggleRowCard: function(record, show) {
		...
		if (!record.rowCardRow) {
			...
			record.rowCardCell.setAttribute("colspan", record.builder.orderCols.length);
	...
	appendRowCardOpener: function(record) {
		...
		th.className = this.builder.gridCss.cell + " s-grid-rowcard-cell-picker";
	...
	appendRowCardCol: function(slot, beforeCell) {
		var col = this.builder.cols.rowCard = this.builder.appendCol();
		col.groupHead.style.width = col.groupBody.style.width = (col.widthValue = 25) + "px";
		col.slot = slot;
		col.titleCell.className = this.builder.gridCss.title + " s-grid-rowcard-cell-picker";
		var opener = document.createElement("a");
	...
	_renderRecordCard: function(record, slot) {
		...
		slot.setAttribute("data-s-article", record.id);
		if (!record.cardItem) {
			record.$item = record.list.$item.$cardItem;
			record.$skin = "s-record-card";
			record.layoutSlot = slot;
			record.dataSlot = record._item = record.body = document.createElement("div");
			record.body.className = "s-carditem";
			record.$$body = $(record.body);
			record.layoutSlot.appendChild(record._item);
	...
	onWindowResize: function() {
		if (this.hasRowCard && this.list.records) {
			for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
				this.resizeCardItem(this.list.records[ii]);
			}
		}
	},
	_removeRecordCards: function() {
		var col = this.builder.cols.rowCard;
		var inlineColIndex;
		if (col) {
			inlineColIndex = this.builder.findColIndex(col);
			this.builder.orderCols.splice(inlineColIndex, 1);
			...
			delete this.builder.cols.rowCard;
	...
	appendRowCard: function(designing) {
		if (!this.list.$item.$cardItem) {
			if (this.builder.$complexBinds.length) {
				...
				for (var ii = 0, jj = this.builder.$complexBinds.length; ii < jj; ii++) {
					var $bind = this.builder.$complexBinds[ii];
	...
	onRecordApplyChange: function(record) {
		if (this.list.$item.$isRowCardAutoHide) {
			if (record.dataset && this.builder.$complexBinds.length && record.rowCard) {
				var hasData;
				for (var ii = 0, jj = this.builder.$complexBinds.length; ii < jj; ii++) {
					var data = record.dataset[this.builder.$complexBinds[ii]];
					if (data !== null && data !== undefined) {
						hasData = true;
						break;
					}
				}
				record.rowCard.opener.style.display = hasData ? "" : "none";
			}
		}
	},
	onSelectRecord: function(record, selected) {
		...
		if (selected) {
			if (this.builder.popupCardBuilder) {
				this.builder.popupCardBuilder.toggleCard(record, true);
			}
			if (this.isOutCardMode) {
				document.site.emptyDom(this._outCardSlot);
				this._renderRecordCard(record, this._outCardSlot);
			}
		}
	},
	applyDesignMetaData: function(metaData, designing) {
		var $cardItem = helpers.object.clone(this.list.$item.$cardItem, true);
		if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") {
			...
			if (metaData.$cardPosition == "row") {
				...
				var col = this.appendRowCardCol(this.builder.titleRow, this.builder.titleRow.firstChild.nextSibling);
				this.builder.orderCols.splice(1, 0, col);
	...
	_alertDiagInCard: function(record) {
		if (!record.isGridCardDiagAlertSet) {
			document.site.showDiagnoses({
				...
				gridCardData: {
					gridRecordId: record.id,
					gridArticleId: record.articleParent.id,
					linkText: record.list.localize.diag_in_card_link.replace("{gridTitle}", record.articleParent.titleText).replace("{gridIndex}", record.$serverIndex + 1)
				}
			});
	...
	appendOutCardSlot: function() {
		if (!this._outCardSeparator) {
			this._outCardSeparator = document.createElement("div");
		}
		if (!this._outCardSlot) {
			this._outCardSlot = document.createElement("div");
		}
		var cssCell = "";
		var tableSlot = this.list.tableSlot;
		switch (this.list.$item.$cardItem.$position) {
			case "left":
				tableSlot.parentNode.insertBefore(this._outCardSeparator, tableSlot);
				tableSlot.parentNode.insertBefore(this._outCardSlot, this._outCardSeparator);
				cssCell = "-cell";
				break;
			case "right":
				tableSlot.parentNode.insertBefore(this._outCardSeparator, tableSlot.nextSibling);
				tableSlot.parentNode.appendChild(this._outCardSlot, this._outCardSeparator.nextSibling);
				cssCell = "-cell";
				break;
			case "top":
				var sibling = this.list.tableRow || tableSlot;
				sibling.parentNode.insertBefore(this._outCardSeparator, tableSlot);
				sibling.parentNode.insertBefore(this._outCardSlot, this._outCardSeparator);
				this._outCardSlot.style.width = "";
				break;
			case "bottom":
				var sibling = this.list.tableRow || tableSlot;
				sibling.parentNode.appendChild(this._outCardSeparator);
				sibling.parentNode.appendChild(this._outCardSlot);
				this._outCardSlot.style.width = "";
				break;
		}
		this._outCardSeparator.className = "s-list-card-sep" + cssCell;
		this._outCardSlot.className = "s-list-card-slot" + cssCell;
		if (this.list.$item.$cardItem.$widths) {
			this._outCardSlot.style.width = $cardItem.$widths + "%";
		}
		tableSlot.style.display = "";
	},
	removeOutCardSlot: function() {
		document.site.removeDomChild(this._outCardSeparator);
		document.site.removeDomChild(this._outCardSlot);
		this._outCardSeparator = this._outCardSlot = null;
	},
	dispose: function() {
		this.builder = this.list = this.$rowFieldBinds = this._outCardSeparator = this._outCardSlot = null;
	}
});