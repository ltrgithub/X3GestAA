...
function _removeRecordCards(list) {
	var col = list.cols.rowCard;
	var inlineColIndex;
	if (col) {
		inlineColIndex = list.orderCols.indexOf(col);
		list.orderCols.splice(inlineColIndex, 1);
		if (list.filterRecord) {
			syra_site.dom.removeChild(list.filterRecord.dataRow.childNodes[inlineColIndex]);
		}
		syra_site.dom.removeChild(col.groupBody);
		syra_site.dom.removeChild(col.groupHead);
		syra_site.dom.removeChild(col.titleCell);
		delete list.cols.rowCard;
		list.cardDecorator.$rowFieldBinds = null;
	}
...
exports.CardDecorator = helpers.defineClass(CardDecorator, null, {
	...
	onRecordResize: function(record) {
		if (this.list.scrollTable.bodySlot.clientWidth) {
			if (this.isRowCardMode && record.body) {
				record.layoutSlot.style.width = this.list.scrollTable.bodySlot.clientWidth + "px";
			}
		}
	},
	onShowRecordSelection: function(record, selected) {
		...
		if (selected) {
			var popupCardBuilder = this.list.fomatter.popupCardBuilder;
	...
	toggleRowCard: function(record, show) {
		...
		if (!record.rowCard) {
			record.rowCard = record.list.fomatter.builder.createCardRow(record);
		...
		if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") {
			...
			if (metaData.$cardPosition == "row") {
				...
				if (this.list.filterRecord) {
					...
					this.list.filterRecord.dataRow.insertBefore(td, record.dataRow.firstChild.nextSibling);
				}
				var col = syra_list.grid.builder.addRowCardCol(this.list, true);
			...
			this.isRowCardMode && _removeRecordCards(this.list);
