...
exports._makeObsoleteLayout = function(list) {
	list.$item.$popupCard = {
		$layout: {
			$items: []
		}
	...
	for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
		if (!$fields[$binds[ii]].$isExcluded) {
			if ($row && $row.$items.length == 3) {
				$row = null;
			}
			if (!$row) {
				list.$item.$popupCard.$layout.$items.push($row = {
					$layoutType: "row",
					$items: []
				});
			}
...
exports.makeLayout = function(list) {
	var $fields = list.$field.$item.$properties;
	var $binds = Object.keys($fields);
	var hasViewScope;
...
exports.addOpenLink = function(record) {
...
exports.findField = function(name, record) {
...
exports.onSelectRecord = function(record) {
	if (!record._isOnApplyingChange) {
		...
		if (list == _activeList) {
			...
			if (!record.popupCard && record.list.isClientFetch) {
				var $popupCard = list.$item.$popupCard;
			...
			_slotBody.syraRecord = record.$uuid;
			_slotBody.syraItem = record.id;
			_slotBody.setAttribute("data-s-record", record.$uuid);
			record.setArticleId(_slotBody);
			var card = record.popupCard;
			if (!card) {
				record.popupCard = card = {
					content: list.page.loadNewItem(_slotBody, $popupCard || ($popupCard || list.$item.$popupCard), record),
					pagerSlot: document.createElement("div")
				};
				card.pagerSlot.className = "s-popupcard-pager-slot";
				syra_menus.addPagerStepLinks(record, card.pagerSlot);
			...
			if (record.$menusPopup && !card.mnPicker) {
				var picker = card.mnPicker = document.createElement("a");
				picker = document.createElement("a");
				picker.className = "s-popupcard-picker s-picker-menus";
				picker.syraOnClick = "onPickerClick";
				picker.syrainout = picker.syraIsSyraMenuHandler = true;
				picker.syraPickerType = "menus";
				picker.title = picker.syraTitle = syra_local.flActions;
				card.pagerSlot.insertBefore(picker, card.pagerSlot.firstChild);
			}
			...
			syra_menus.setMenus(record, {
				$actions: {
					$first: {
						...
						$isDisabled: list.pagging.$startIndex == 1 && recordIndex == 0
					},
					$previous: {
						$isHidden: list.dataset.length <= 2,
						$isDisabled: list.pagging.$startIndex == 1 && recordIndex == 0
					...
					$next: {
						$isHidden: list.dataset.length <= 2,
						$isDisabled: (list.pagging.$startIndex + recordIndex) == list.dataset.length
					},
					$last: {
						$isHidden: list.dataset.length < 2,
						$isDisabled: (list.pagging.$startIndex + recordIndex) == list.dataset.length
			...
			syra_site.layoutValidator.validate(card.content.layoutContent, true);
...
exports.togglePopup = function(record) {
	if (record) {
		...
		if (!(_activeList == list && _activeRecord == record)) {
			if (_activeList == list) {
				list.selector.select(record.$uuid, true); //ensure selection
			} else {
				_activeList = list;
				list.page.externalAdapter.onFieldClickPicker({
					field: record,
					...
					doEvent: function() {
						...
						_dialog.appendCloseButton(_slotHeader);
					}
				});
			}
			return;
