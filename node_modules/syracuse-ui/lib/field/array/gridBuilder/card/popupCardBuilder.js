...
function _makeObsoleteLayout(list) {
	...
	var $meta = list.metaHelper.getMetaData(list);
...
exports.onShowRecordSelection = function(record) {
...
exports.togglePopup = function(record) {
	if (record) {
		...
		if (!(_activeList == list && _activeRecord == record)) {
			if (_activeList == list) {
				list.selector.selectRecord(record.$uuid, true); //ensure selection
			} else {
				_activeList = list;
				list.page.externalAdapter.onFieldClickPicker({
					field: record,
