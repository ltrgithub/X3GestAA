...
exports.RecordArticle = helpers.defineClass(RecordArticle, Article, {
	...
	onMenuClick: function(menuItem) {
		...
		if (this.list.builder.record_onMenuClick) {
			return this.list.builder.record_onMenuClick(this, menuItem);
		}
		return true;
	},
	applyChange: function(metaData, serverResponse) {
		if (metaData) {
			...
			syra_menus.applyArticleState(this);
		}
	},
	...
	onItemInOut: function(onEnter, event, target) {
		if (!this.list.$item.$isHoverDisabled) {
			...
			for (var ii = 0, jj = this._buttons.length; ii < jj; ii++) {
				var btn = this._buttons[ii];
				syra_site.dom.toggleClass(btn, "s-enter", onEnter && !btn.syraIsDisabled);
			}
		}
	},
	showRecordSelection: function(selected) {
		if (this.list.builder.record_onShowSelection) {
			...
			selected && this.setFocusRecord();
			this.checkBox && syra_fields.checkbox.setValue(this, selected);
			if (selected && !this.list.selector.isMulti) {
				this.scrollToRecord();
			}
		}
	},
	setFocusRecord: function() {
		this.list.filler.setFocusRecord(this.list, this);
	},
	...
	scrollToRecordField: function(field) {
		this.list.scrollToField();
		if (this.list.builder.list_scrollToRecordField) {
			this.list.builder.list_scrollToRecordField(field);
		} else {
			this.scrollToRecord();
		}
	}
