...
exports.RecordArticle = helpers.defineClass(RecordArticle, Article, {
		...
		if (!this.isReorderDisabled) {
			if (this.hasReorderCapability = reorder) {
				if (!this.reorderPicker) {
					...
					this.reorderPicker.syraDragSpot = this.id;
					...
					if (this.list.$isDisabled) {
						syra_site.dom.disableItem(this.reorderPicker, this.list.$isDisabled);
					}
				}
			} else {
				if (this.reorderPicker) {
					syra_site.dom.removeChild(this.reorderPicker);
	...
	onTreeNodeClick: function(picker, event) {
		this.list.treeDecorator.onNodeEvent(this, picker, event);
	},
	onRowCardClick: function(picker, event) {
		this.list.cardDecorator.onRowCardOpenerClick(picker, this);
	},
	onSelectorClick: function(picker, event) {
		this.list.onSelectorClick(picker, event);
	},
	...
	onItemInOut: function(onEnter) {
		if (this.menusSlot) {
			syra_site.dom.toggleClass(this.menusSlot, "s-enter", onEnter);
		}
		if (this.reorderPicker) {
			syra_site.dom.toggleClass(this.reorderPicker, "s-enter", onEnter);
		}
	...
	onSelectMouseEvent: function(onEnter) {
		syra_site.dom.toggleClass(this.domItem, "s-record-over", onEnter);
	},
	...
	ensureRecordVisibility: function() {
	...
	resizeRecord: function(resize) {
		...
		if (this.list.cardDecorator && (this.cardItem || this.popupCardItem)) {
