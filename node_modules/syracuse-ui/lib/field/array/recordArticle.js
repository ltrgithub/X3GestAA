...
exports.RecordArticle = helpers.defineClass(RecordArticle, Article, {
	loadBox: function(initData, isCreateAction) {
		if (this.list.builder && this.list.builder.onBeforeLoadRecord) {
			this.list.builder.onBeforeLoadRecord(this, initData, isCreateAction);
		}
		this._buttons = [];
	...
	reorderItem: function(targetRecord, isAfter) {
		if (this.list.builder) {
			this.list.builder.reorderItem(this, targetRecord, isAfter);
		} else {
	...
	dispose: function() {
		if (this.list) {
			...
			this.list.builder && this.list.builder.disposeRecord(this);
		}
	...
	resizeArticle: function(doResize, boxParent) {
		...
		if (this.list.builder && this.list.builder.onResizeRecord) {
			this.list.builder.onResizeRecord(this, doResize, boxParent);
		}
	},
	ensureLayoutMode: function() {
		Article.prototype.ensureLayoutMode.call(this);
		if (this.list.builder && this.list.builder.onRecordEnsureLayoutMode) {
			this.list.builder.onRecordEnsureLayoutMode(this);
		}
	},
	...
	remove: function() {
		if (this.list.builder) {
			this.list.builder.removeRecord(this);
		} else {
	...
	onMenuClick: function(menuItem) {
		...
		if (this.list.builder) {
			return this.list.builder.onRecordMenuClick(this, menuItem);
		}
		return true;
	},
	applyChange: function(metaData) {
		if (metaData) {
			...
			if (this.list.selector && metaData.$isSelected !== undefined) {
				this.list.selector.onRecordApplyChange(this, metaData.$isSelected);
			}
			Article.prototype.applyChange.call(this, metaData);

			this.list.builder && this.list.builder.onRecordApplyChange(this, metaData);
		}
	},
	...
	onItemInOut: function(onEnter, event, target) {
		if (!this.list.$item.$isHoverDisabled) {
			if (this.list.builder) {
				this.list.builder.onItemInOutRecord(this, onEnter);
			} else {
