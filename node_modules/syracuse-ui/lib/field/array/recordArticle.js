...
exports.RecordArticle = helpers.defineClass(RecordArticle, Article, {
	...
	onBoxToggled: function(box, isHidden) {
		if (box.id == (this.id + "-menu-picker")) {
			box.$isPickerVisible = !isHidden;
			this.setMenusBoxCol("menu", box.$isPickerVisible, 22);
		}
	},
	...
	onItemEnterLeave: function(event) {
		if (event && this.list.selector && this.list.selector.isRowMode) {
			this.onSelectMouseEvent(event);
	...
	dispose: function() {
		this.garbage = this.localize = this.menusBox = this.list = this.menusSlot = this.rowIndexPicker = this.reorderPicker = null;
