...
exports.appendScrollButtons = function(list) {
	console.log("appendscrollbuttons");
	if (list.builder.scroller) {
		if (list.$item.$itemsPerPage > 20) {
			if (!list.leftScrollBtn) {
				if ((!document.site.isScrolledIntoView($(list.builder.scroller._hscroller), document.site.mainPage.$$scrollview) && document.site.isScrolledIntoView($(list.builder._headTable), document.site.mainPage.$$scrollview)) || (document.site.isScrolledIntoView($(list.builder.scroller._hscroller), document.site.mainPage.$$scrollview) && !document.site.isScrolledIntoView($(list.builder._headTable), document.site.mainPage.$$scrollview)) || (!document.site.isScrolledIntoView($(list.builder.scroller._hscroller), document.site.mainPage.$$scrollview) && !document.site.isScrolledIntoView($(list.builder._headTable), document.site.mainPage.$$scrollview))) {
					var page = document.site.mainPage;
					var scrollview = page.$$scrollview[0];

					var containLeft = document.createElement("div");
					containLeft.className = "s-list-btn-scroll-contain-left";
					var containRight = document.createElement("div");
					containRight.className = "s-list-btn-scroll-contain-right";

					list.leftScrollBtn = document.createElement("a");
					list.leftScrollBtn.className = "s-list-btn-scroll-left";
					//list.leftScrollBtn.style.display = "none";
					list.rightScrollBtn = document.createElement("a");
					list.rightScrollBtn.className = "s-list-btn-scroll-right";

					containLeft.appendChild(list.leftScrollBtn);
					containRight.appendChild(list.rightScrollBtn);

					list._core.appendChild(containLeft);
					list._core.appendChild(containRight);
					exports.bindScrollButtonsEvents(list, true);

					// handle scroll btn display
					this.$hasScrollListener = list.addScrollListener();

					/*
                     if (document.site.mainPage && document.site.mainPage.addScrollListener && document.site.mainPage.updateScrollBtnPosition) {
                     var listScrollBtnHandler = {
                     leftScrollBtn: list.leftScrollBtn,
                     rightScrollBtn: list.rightScrollBtn,
                     listHscroller: list.builder.scroller._hscroller,
                     listHeadTable: list.builder._headTable
                     };
                     document.site.mainPage.addScrollListener(list.id, listScrollBtnHandler);
                     document.site.mainPage.updateScrollBtnPosition();
                     }*/
				}
			} else {
				if (list.$hasScrollListener) {
					list.updateScrollBtnPosition(this);
				}
				//list.leftScrollBtn.style.display = list.rightScrollBtn.style.display = document.site.isScrolledIntoView($(list.builder.scroller._hscroller), document.site.mainPage.$$scrollview) ? "none" : "";
			}
		} else {
			if (list.leftScrollBtn) {
				list.leftScrollBtn.parentNode.removeChild(list.leftScrollBtn);
				list.rightScrollBtn.parentNode.removeChild(list.rightScrollBtn);
				list.leftScrollBtn = list.rightScrollBtn = null;
				if (list.$hasScrollListener) {
					list.removeScrollListener();
				}
			}
		}
	}
...
exports.bindScrollButtonsEvents = function(list, bind) {
	if (list.leftScrollBtn) {
		if (bind) {
			_bind(list.$$leftScrollBtn = $(list.leftScrollBtn), list, true);
			_bind(list.$$rightScrollBtn = $(list.rightScrollBtn), list);
		} else {
			list.$$leftScrollBtn.unbind();
			list.$$rightScrollBtn.unbind();
		}
	}
};


function _bind($$btn, list, left) {
	$$btn.bind("click", function() {
		list.builder.scroller._hscroller.scrollLeft = left ? list.builder.scroller._hscroller.scrollLeft - 25 : list.builder.scroller._hscroller.scrollLeft + 25;
	}).bind("mousedown", function() {
		if (left) {
			list.scrollLeftTimeout = setInterval(function() {
				list.builder.scroller._hscroller.scrollLeft -= 25;
			}, 100);
		} else {
			list.scrollRightTimeout = setInterval(function() {
				list.builder.scroller._hscroller.scrollLeft += 25;
			}, 100);
		}
	}).bind("mouseup mouseout", function() {
		clearInterval(left ? list.scrollLeftTimeout : list.scrollRightTimeout);
	});
}
...
exports.fillList = function(list, dataRecordSet, parentDataRecord, isDelta) {
	...
	if (isDelta) {
		...
		if (list.page.$isPartialDelta || (dataRecordSet.length == 1 && dataRecordSet[0].$index !== undefined)) {
			...
			list.isClientFetchResult = list.onClientFetch;
	...
	for (var $serverIndex = 0, jj = dataRecordSet.length; $serverIndex < jj; $serverIndex++) {
		...
		if (found) {
			if (dataSourceRecord.$isDeleted) {
				...
				if (dataSourceRecord.$index !== undefined && found.$serverIndex != dataSourceRecord.$index) {
					isReorder = list.isClientFetchResult;
					//TODO if list.isClientFetchResult trgger client resort  
				...
				if (isReorder) {
					...
					found.reorderItem(nextRecord, nextRecord ? "insertBefore" : "append");
				}
				/*var recordIndex = foundIndex;
