...
exports.createEmptyDataItem = function(list, tag){
    list.emptyDataItem = document.createElement(tag || "div");
    list.emptyDataItem.className = list.$skin + "-empty-slot";
    list.emptyDataItem.textContent = list.$item.$noDataText || list._localize.l_noData;
    return list.emptyDataItem;
};
...
/************************************* Capabilities **********************************************/
exports.parseCapabilities = function(list, $capabilities){
    if (!$capabilities) {
        $capabilities = $capabilities || list.$prototype.$capabilities;
    }
    list.$capability = {};
    if ($capabilities !== undefined) {
        var parts = $capabilities.split(",");
        for (var ii = 0, jj = parts.length; ii < jj; ii++) {
            list.$capability[parts[ii]] = true;
        }
        if (list.$capability.reorder !== undefined) {
            if (list.$capability.reorder) {
                list.$item.$isRowIndexVisible = true;
                if (!list.reorderCapability) {
                    (list.reorderCapability = new ReorderCapability(list)).setDraggable();
                }
            }
            else {
                if (list.reorderCapability) {
                    document.controller.disposeObject(list.reorderCapability);
                    delete list.reorderCapability;
                }
            }
            list.builder.setRowIndexVisibility();
        }
    }
};
/************************************* Client Fetching **********************************************/
...
exports.fillList = function(list, dataRecordSet, parentDataRecord, isDelta){
    ...
    if (list.$item.$cards && list.$item.$format != "cards") {
        if (list.$item.$cards.$position != "inline" && Object.keys(list.selector.records).length == 0) {
            if (list.records.length > 0) {
                //list.selector.select(list.records[0].$uuid, true);
            }
        }
    }
...
exports.applyMetaData = function(list, metaData){
    ...
    if (metaData && metaData.$capabilities !== undefined) {
        ...
        var $actions;
        exports.parseCapabilities(list, metaData.$capabilities);
