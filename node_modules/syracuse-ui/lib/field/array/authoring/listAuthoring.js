...
exports.ListAuthoring = helpers.defineClass(ListAuthoring, Article, {
    ...
    _onOpenGrapContainer: function(){
        ...
        if (containerBox && containerBox.$$body) {
            if (!this.chartAuthoring && this.designedField.chart && this.designedField.chart.getAuthoringWidget) {
                ...
                dom.className = "s-aw-designer-custom-field";
    ...
    applyChange: function(newData){
        if (newData) {
            if (newData.$format) {
                ...
                delta.$fitContainer = delta.$fitContainer || {};
                delta.$fitContainer.$isHidden = isCards;
    ...
    loadBox: function(opener){
        ...
        self.$prototype = {
            "$properties": {
                ...
                $fitContainer: {
                    ...
                    $title: self.localize.lw_fitContainer
                },
                $isQuickFilter: {
                    $type: "application/x-boolean",
        ...
        self.$layoutOptions = document.site.$item.$layoutOptions.list.authoring;
    ...
    appendOkCancel: function(){
        var self = this;
        var btn;
        if (self.opener.isQuickDesigner && !document.site.authorPage) {
            btn = document.createElement("a");
            btn.className = "s-list-designer-ok";
            btn.setAttribute("title", self.localize.lw_linkOk);
            self.$$item.append(btn);
        }
        btn = document.createElement("a");
        btn.className = "s-list-designer-cancel";
        btn.setAttribute("title", self.localize.lw_linkCancel);
        self.$$item.append(btn);
        
        self.$$item.delegate(".s-list-designer-cancel,.s-list-designer-ok", "click", function(){
            if (self.isUpdated && !document.site.authorPage) {
                if (this.className.indexOf("s-list-designer-cancel") >= 0) {
                    ...
                    document.site.showMessage({
                        $title: self.localize.lw_updateMessageTitle,
                        $message: self.localize.lw_saveMessageText,
                        $type: "question",
                        $buttons: "yesno",
                        callback: function(response){
                            if (response.$id != "no") {
