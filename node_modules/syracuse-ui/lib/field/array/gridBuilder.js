...
function _addFilterRecord(builder) {
	if (builder.list.$item.$isQuickFilter) {
		if (builder.filterRecord) {
			...
			builder.filterRecord = _gridFilter.addRecord(builder.list);
		}
	}
}
...
function _addTitleRow(builder) {
	...
	if (list.treeDecorator) {
		...
		if (hasFixedCol && !syra_context.enableTestRobot && list.isFixedColEnabled) {
		...
		if (syra_dom.isVisible(this.tableSlot)) {
			...
			if (scrollview) {
				...
				if (this.emptyDataItem.parentNode && (this.list.treeDecorator || this.list.$item.$isTitleRowHidden)) {
					...
					this.scrollTable.headSlot.style[syra_context.isRTL ? "marginLeft" : "marginRight"] = hasScollHeight ? "17px" : "";
					scrollBodySlot.style[syra_context.isRTL ? "paddingLeft" : "paddingRight"] = hasScollHeight ? "17px" : "";
