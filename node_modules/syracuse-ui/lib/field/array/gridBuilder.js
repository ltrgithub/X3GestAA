	...
	addResizer: function() {
		this.resizer = new Resizer({
			...
			start: function(resizing) {
				resizing.body = syra_dom.getBoundingClientRect(this.builder.scrollTable.bodySlot);
				if (!this.recordHeight) {
					var records = this.builder.list.records;
			...
			drag: function(resizing) {
				this.builder.scrollTable.bodySlot.style.maxHeight = "";
				if (resizing.deltaX != undefined) {
					this.builder.scrollTable.bodySlot.style.width = (resizing.newWidth = resizing.body.width + resizing.deltaX) + "px";
				}
				if (resizing.deltaY != undefined) {
					resizing.newHeight = resizing.body.height + resizing.deltaY;
					resizing.newHeight = Math.max(resizing.newHeight, 2 * this.recordHeight);
					this.builder.scrollTable.bodySlot.style.height = resizing.newHeight + "px";
					this.builder.setFixedTableHeight();
				}
			},
			stop: function(resizing) {
				resizing.newHeight = Math.max(Math.round(resizing.newHeight / this.recordHeight) * this.recordHeight, 2 * this.recordHeight);
				this.builder.scrollTable.bodySlot.style.height = resizing.newHeight + "px";
				this.builder.onResizeArray();
			}
		});
	},
