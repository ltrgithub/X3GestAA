...
var FilterRecord = require("./gridBuilder/filter/filterRecord").FilterRecord;
var CardDecorator = require('./gridBuilder/card/cardDecorator').CardDecorator;
var _filterBuilder = require('./gridBuilder/filter/filterBuilder');
var _colResizer = require("./gridBuilder/tools/colResizer");
var _colsReorderer = require('./gridBuilder/tools/colsReorderer');
var _quickEditBuilder = require("./gridBuilder/tools/quickEditBuilder");
var _popupCardBuilder = require('./gridBuilder/card/popupCardBuilder');
var _preferencesHelper = require("syracuse-ui/lib/field/array/tools/preferencesHelper");
var _sideButtons = require('./gridBuilder/sideButtons');
var GridTable = require('./gridBuilder/gridTable').GridTable;
...
function _hasFreezeField(list, $items) {
	if (!syra_site.enableTestRobot && !list.page.lookupField) {
...
function _addTitleRow(builder) {
	...
	list.metaHelper.ensureGlobalMetaRecord(list, true);
	if (list.$item.$isQuickFilter) {
		if (builder.filterRecord) {
			builder.filterRecord.drawBox();
		} else {
			(builder.filterRecord = new FilterRecord()).loadRecord(list);
		}
	}
	...
	builder.cardDecorator.isRowCardMode && freezeTable.addRowCardCol();
	...
	for (var ii = 0, jj = $items.length; ii < jj; ii++) {
		...
		if ($field && !$field.$isExcluded) {
			...
			builder.filterRecord && _filterBuilder.addFieldCell(col, $field);
		}
	}
...
function _checkHeaderVisibility(builder) {
	...
	if (filter && filter.isFilterVisible !== isFilterVisible) {
		...
		syra_site.dom.display(filter.freezeRow, isFilterVisible);
		syra_site.dom.display(filter.dataRow, isFilterVisible);
	}
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	list_scrollToRecordField: function(field) {
		...
		if (col && !col.table.isFreeze) {
			syra_page.scrollToItem(this.list.page, field.domItem, col.table.bodySlot, true);
		}
	},
	list_scrollToRecord: function(record) {
		record && syra_page.scrollToItem(this.list.page, record.dataRow, record.page.scrollview, true);
	},
	...
	onAfterLoadRecord: function(record, initData, isCreateAction) {
		if (isCreateAction) {
			this.cardDecorator.isRowCardMode && cardDecorator.toggleRowCard(record, true);
		}
	},
	record_draw: function(record) {
		...
		record.domItem = record.dataRow = _addRow(record);
		if (this.allColumns[0].table.isFreeze) {
			record.freezeRow = _addRow(record);
		}
	...
	list_onAfterSetDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		this.list.graphDecorator && this.list.graphDecorator.list_onAfterSetDataBind(dataRecordSet, parentDataRecord, metaData);
	},
	list_onAfterDraw: function() {
		...
		this.tableSlot.appendChild(slotTable);
		...
		this.list.endDrawBox(this.list);
	},
	...
	record_onShowSelection: function(record, selected) {
		syra_site.dom.toggleClass(this.list.treeDecorator ? (record.treeNode.desc || record.treeNode.item) : record.domItem, "s-list-record-selected", selected);
		selected && record.setFocusRecord(record);
		record.freezeRow && syra_site.dom.toggleClass(record.freezeRow, "s-list-record-selected", selected);
		this.cardDecorator.onShowRecordSelection(record, selected);
		record.checkBox && syra_fields.setCheckBockValue(record, selected);
		if (selected && !this.list.selector.isMulti) {
			record.scrollToRecord();
		}
	},
	dispose: function(removeItem) {
		removeItem && syra_site.dom.removeChild(this.tableSlot);
		syra_dd.addToColResizers(this.list, false);
		this.scrollTable && _sideButtons.dispose(this.scrollTable);
		syra_site.disposeObject(this.cardDecorator);
	...
	resizeScrollView: function() {
		if (!this.list.isTableSlotHidden) {
			...
			if (this.list.$item.$fixedBodyHeight) {
				...
				if (this.list.adjustToScrollPage) {
					var bodySlot = this.scrollTable.bodySlot;
					bodySlot.style.maxHeight = "";
					var bodyRect = this.scrollTable.bodySlot.getBoundingClientRect();
					var scrollH = this.list.page.scrollview.scrollHeight;
					var diff = scrollH - this.list.page.scrollview.clientHeight;
					if (diff > 0) {
						bodySlot.style.maxHeight = Math.max((bodyRect.height - diff) - 2, 50) + "px";
					}
				}
				if (this.list.$fitContainer) {
					if (this.emptyDataItem.syraIsDisplay) {
						this.scrollTable.bodySlot.style.height = "0px";
					} else {
						var top = this.scrollTable.bodySlot.scrollTop;
						var bodyRect = this.scrollTable.bodySlot.getBoundingClientRect();
						var slotRect = this.list.layoutSlot.getBoundingClientRect();
						var height = Math.max(slotRect.height - (bodyRect.top - slotRect.top), 0);
						this.scrollTable.bodySlot.style.height = height + "px";
						this.scrollTable.bodySlot.scrollTop = top;
					}
				}
			}
			if (this.emptyDataItem.syraIsDisplay && this.list.$item.$isTitleRowHidden) {
				...
				var fitWidth;
				if (this.list.fitWidth) {
					fitWidth = maxSlotWidth - freezeWidth;
				}
				var scrollLeft = this.scrollTable.bodySlot.scrollLeft;
				if (!scrollLeft) {
				...
				this.scrollTable.bodySlot.scrollLeft = scrollLeft;
	...
	record_onAfterResize: function(record, doResize, boxParent) {
		var cardDecorator = this.cardDecorator;
		if (this.cardDecorator && (record.cardItem || record.popupCard)) {
			if (this.cardDecorator.isRowCardMode && record.body) {
				record.layoutSlot.style.maxWidth = this.scrollTable.bodySlot.clientWidth + "px";
			}
		}
	},
	record_onAfterApplyChange: function(record, metaData) {
		this.cardDecorator.onAfterApplyChange(record);
	},
	record_onMenuClick: function(record, menuItem) {
		if (!record.disposed) {
			...
			switch (menuItem.$sourceBind) {
				case "$recordCard":
					_popupCardBuilder.togglePopup(record);
					return false;
				case "$first":
					$startIndex = 1;
					$newIndex = 0;
					break;
				case "$previous":
					$newIndex = (this.list.pagging.$startIndex - 1) + (record.getRecordIndex() - 1);
					break;
				case "$next":
					$newIndex = (this.list.pagging.$startIndex - 1) + (record.getRecordIndex() + 1);
					break;
			...
			var $startIndex = (Math.floor($newIndex / this.list.pagging.getItemsPerPage()) * this.list.pagging.getItemsPerPage()) + 1;
			if ($startIndex != this.list.pagging.$startIndex) {
			...
			this.list.selector.selectRecord(this.list.clientDataset[$newIndex].$uuid, true);
			return false;
		}
	},
	...
	onBeforeListSetFocus: function(select, name, $serverIndex) {
		if (_popupCardBuilder.isOpened(this.list) && this.list.selector.uuid != this.list.clientDataset[$serverIndex].$uuid) {
			this.list.selector.unSelectAll();
			this.list.selector.selectRecord(this.list.clientDataset[$serverIndex].$uuid, true);
		}
	},
	onAfterAddRecord: function(record) {
		_popupCardBuilder.addOpenLink(record);
	}
});