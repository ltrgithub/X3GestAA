...
var _filterGridRow = require('syracuse-ui/lib/filter/view/gridRow');
...
function _addTitleRow(builder) {
	...
	for (var ii = 0, jj = $items.length; ii < jj; ii++) {
		...
		if ($field && !$field.$isExcluded) {
			...
			list.filter_row && list.filter_row.addCriteria(col);
		}
	}
	list.filter_row && list.filter_row.validateDisplay();
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	findEventCol: function(event) {
		...
		while ((!target.syra_field_bind) && (target != this.slotTable)) {
