...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	list_onItemInOut: function(onEnter, event, target) {
		if (target.syra_field_bind) {
			...
			if (onEnter) {
				...
				col.titleLabel.title && syra_site.dialogManager.titleTip.show(col.titleLabel.title, col.titleLabel, true);
			}
		}
	...
	onAfterLoadRecord: function(record, initData, isCreateAction) {
		if (isCreateAction) {
			this.cardDecorator.isRowCardMode && this.cardDecorator.toggleRowCard(record, true);
		}
	},
		...
		if (!this.list.isTableSlotHidden) {
			...
			if (scrollview) {
				...
				if (this.emptyDataItem.syraIsDisplay && this.list.$item.$isTitleRowHidden) {
					...
					if (this.list.fitWidth || this.list.isMaximized || this.list.$field.$isFusionNavigationList) {
