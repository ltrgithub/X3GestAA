...
function _addTitleRow(builder) {
	...
	if (list.treeDecorator) {
		...
		var hasFixedCol;
		list.hasRowIndexCol = list.$item.$isRowIndexVisible || list.$capability.reorder || list.$item.$selectByRowIndex;
		if (!syra_quality.enableTestRobot && list.isFixedColEnabled) {
			hasFixedCol = list.hasRowIndexCol;
			for (var ii = 0, jj = $items.length; !hasFixedCol && ii < jj; ii++) {
				var $item = $items[ii];
				var $field = list.$fields[$item.$bind];
				if ($field && !$field.$isExcluded) {
					hasFixedCol = ($item.$isFixed !== undefined) ? $item.$isFixed : $field.$isFixed;
				}
			}
		}
		if (hasFixedCol) {
...
function _checkHeaderVisibility(builder) {
	...
	if (filter && filter.isFilterVisible !== isFilterVisible) {
		...
		syra_site.dom.hide(filter.fixedRow, !isFilterVisible);
	...
	if (builder.isTitleRowVisible !== isTitleRowVisible) {
		...
		builder.fixedTable && syra_site.dom.hide(builder.fixedTable.headSlot, !isTitleRowVisible);
		syra_site.dom.hide(builder.scrollTable.headSlot, !isTitleRowVisible);
	}
}
...
function _addHScroller(builder) {
	if (builder.list.$item.$isDockedHeight || builder.list.$fitContainer) {
		builder.scrollTable.bodySlot.style.overflowY = "auto";
	}
	builder.bodySlot_onScroll = builder.scrollTable.bodySlot.addEventListener("scroll", function(event) {
		var page = builder.list.page;
		if (page && !syra_site.isInoutDisabled && page.isPageLoaded && !page._isDataChanging) {
			if (builder.scrollTable.headSlot.scrollLeft != this.scrollLeft) {
				builder.scrollTable.headSlot.scrollLeft = this.scrollLeft;
			}
			var fixedBody = builder.fixedTable && builder.fixedTable.bodySlot;
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	initialize: function(list) {
		...
		list.showEmptyItem = function(show) {
			if (show) {
				...
				syra_site.dom.removeChild(this.builder.emptyDataItem);
			}
		};
	...
	record_disposeCard: function(record, isRecordRemoved) {
		if (record.rowCard) {
			syra_site.dom.removeChild(record.rowCard.row);
	...
	record_resizeInlineCard: function(card) {
		card.contentSlot.style.maxWidth = (this.scrollTable.bodySlot.clientWidth - 16) + "px";
	},
	...
	record_highlightOnEnter: function(record, onEnter, event) {
		if (this.list.selector) {
			if (record.treeNode) {
				...
				record.fixedRow && syra_site.dom.toggleClass(record.fixedRow, "s-record-enter", onEnter);
	...
	record_onSelect: function(record, selected) {
		...
		record.fixedRow && syra_site.dom.toggleClass(record.fixedRow, "s-list-record-selected", selected);
	...
	onResizeArray: function() {
		if (syra_site.dom.isVisible(this.tableSlot)) {
			...
			if (scrollview) {
				...
				if (this.emptyDataItem.parentNode && this.list.$item.$isTitleRowHidden) {
					...
					if (this.fixedTable) {
						...
						fixedWidth = table.getBoundingClientRect().width;
						...
						if (hasMoved) {
							if (this.fixedTable.columns.length) {
								...
								fixedWidth = 0;
								syra_site.dom.removeChild(this.fixedTable.slot);
								syra_site.disposeObject(this.fixedTable);
					...
					var hasScollHeight = this.scrollTable.bodySlot.scrollHeight > (this.scrollTable.bodySlot.clientHeight + 1);
					if (fitWidth || this.list.fitWidth || this.list.isMaximized || (this.list.$field.$isFusionNavigationList && !this.list.isLookUpList)) {
					...
					syra_site.dom.hide(this.scrollTable._headerCorner, !hasScollHeight);
					...
					if (fixedBody) {
						fixedBody.style.maxHeight = (maxHeight === undefined) ? "" : (bodySlot.clientHeight + "px");
					}
			...
			if (this.list.inlineCards) {
				for (var ii = 0, jj = this.list.inlineCards.length; ii < jj; ii++) {
					this.record_resizeInlineCard(this.list.inlineCards[ii]);
				}
			}
		}
	},
