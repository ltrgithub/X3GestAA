...
function _addFilterRecord(builder) {
	if (builder.list.$item.$isQuickFilter) {
		if (builder.filterRecord) {
			builder.filterRecord.drawRecord();
		} else {
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	endInitiallize: function() {
		...
		if (this.list.$userPreferences.$columns) {
	...
	dispose: function(removeItem) {
		...
		if (this.list) {
			syra_item.remove(this.filterRecord);
			...
			if (scrollview) {
				...
				if (this.list.$item.$fixedBodyHeight) {
					...
					if (adjustToScrollview) {
						...
						if (diff > 1) {
							bodySlot.style.maxHeight = maxHeight = Math.max((bodySlot.clientHeight - diff) - 2, 50) + "px";
						}
					} else {
						if (this.list.$fitContainer) {
							...
							if (scrollview) {
								...
								bodySlot.style.maxHeight = maxHeight = Math.max(maxH, 100) + "px";
							}
						}
					}
				}
				if (this.emptyDataItem.parentNode && (this.list.treeDecorator || this.list.$item.$isTitleRowHidden)) {
					...
					this.scrollTable.headSlot.style[syra_site.isRTL ? "marginLeft" : "marginRight"] = hasScollHeight ? "17px" : "";
					bodySlot.style[syra_site.isRTL ? "paddingLeft" : "paddingRight"] = hasScollHeight ? "17px" : "";
					...
					if (fixedBody) {
						fixedBody.style.maxHeight = bodySlot.clientHeight + "px";
					}
					if (this.emptyDataItem.parentNode) {
