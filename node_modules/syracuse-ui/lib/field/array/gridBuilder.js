...
function _addTitleRow(builder) {
	...
	for (var ii = 0, jj = $items.length; ii < jj; ii++) {
		...
		if ($field && !$field.$isExcluded) {
			...
			var hasFilter = builder.filterRecord && _filterBuilder.addFieldCell(col, $field);
	...
	if (builder.filterRecord) {
		syra_site.dom.display(builder.filterRecord.freezeRow, isFilterVisible);
		syra_site.dom.display(builder.filterRecord.dataRow, isFilterVisible);
	}
	builder.scrollTable.headSlot.style.display = isRowVisible ? "" : "none";
}
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	endInitiallize: function() {
		...
		/*if (this.list.page.$prototype && this.list.page.$prototype.$pluralType == "employees") {
	...
	resizeScrollView: function() {
		if (!this.list.isTableSlotHidden) {
			...
			if (this.emptyDataItem.syraIsDisplay && this.list.$item.$isTitleRowHidden) {
				...
				var fitWidth;
				if (this.list.page.isVignettePage || this.list.$item.$fitWidth) {
