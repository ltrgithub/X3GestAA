...
var _filterBuilder = require('syracuse-ui/lib/field/filter/gridFilterBuilder');
...
function _addFixedTable(builder) {
	var slot = document.createElement("div");
	builder.fixedTable = new GridTable();
	builder.fixedTable.load(builder, true, slot);
	builder.scrollTable.slot.parentNode.insertBefore(slot, builder.scrollTable.slot);
	return builder.fixedTable;
}

function _addFilterRecord(builder) {
	if (builder.list.$item.$isQuickFilter) {
		if (builder.filterRecord) {
			builder.filterRecord.drawBox();
		} else {
			builder.filterRecord = _filterBuilder.addRecord(builder.list);
		}
	}
}
...
function _addTitleRow(builder) {
	...
	if (list.treeDecorator) {
		...
		if (hasFixedCol && !syra_quality.enableTestRobot && list.isFixedColEnabled) {
			fixedTable = _addFixedTable(builder);
		}
	}
	_addFilterRecord(builder);
	...
	builder.filterRecord && builder.filterRecord.loadFilter();
...
function _checkHeaderVisibility(builder) {
	...
	if (filter && filter.isFilterVisible !== isFilterVisible) {
		...
		syra_dom.hide(filter.fixedRow, !isFilterVisible);
		syra_dom.hide(filter.dataRow, !isFilterVisible);
	}
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	initialize: function(list) {
		...
		this.columnsMap = {};
		this.allColumns = [];
		list.onDialogResize = function(additionalHeight) {
			var bodySlot = this.builder.scrollTable && this.builder.scrollTable.bodySlot;
			if (bodySlot) {
				var diffH = bodySlot.scrollHeight - (bodySlot.clientHeight + 1);
				return diffH > 0 ? Math.min(diffH, additionalHeight) : 0;
			}
		};
		...
		list.showEmptyItem = function(show) {
			if (show) {
				...
				syra_dom.removeChild(this.builder.emptyDataItem);
			}
		};
	...
	record_disposeCard: function(record, isRecordRemoved) {
		if (record.rowCard) {
			syra_dom.removeChild(record.rowCard.row);
	...
	list_onAfterDraw: function() {
		...
		this.tableSlot = syra_dom.addDiv("s-grid-slot", this.list._core);
		...
		this.slotTable = syra_dom.addDiv("s-grid-slot-table");

		this.scrollTable = new GridTable();
		this.scrollTable.load(this, false, document.createElement("div"));
		...
		this.emptyDataItem = syra_dom.addDiv(this.list.$skin + "-empty-slot");
	...
	appendCardGraphSlots: function() {
		...
		if ($cardItem || this.list.$item.$graphPosition) {
			...
			if (hasRow) {
				if (!this.list.tableRow) {
					this.list.tableRow = syra_dom.addDiv("s-grid-table-row");
	...
	record_onSelect: function(record, selected) {
		if (this.list.treeDecorator) {
			syra_dom.toggleClass(record.treeNode.desc || record.treeNode.item, "s-list-record-selected", selected);
			for (var ii = 0, jj = this.allColumns.length; ii < jj; ii++) {
				var col = this.allColumns[ii];
				if (!col.$isHidden && !col.isTreeview && col.key) {
					syra_dom.toggleClass(record.cellsMap[col.key], "s-list-record-selected", selected);
				}
			}
		} else {
			syra_dom.toggleClass(record.dataRow, "s-list-record-selected", selected);
		}
		record.fixedRow && syra_dom.toggleClass(record.fixedRow, "s-list-record-selected", selected);
	...
	dispose: function(removeItem) {
		if (removeItem) {
			syra_dom.removeChild(this.tableSlot);
	...
	adjustChartSize: function(maxHeight) {
		if (this.list.chart) {
			switch (this.list.$item.$graphPosition) {
				...
				case "top":
					maxHeight /= 2;
					this.list.chart.layoutSlot.style.height = maxHeight + "px";
					break;
			}
		}
		return maxHeight;
	...
	onResizeArray: function() {
		if (syra_dom.isVisible(this.tableSlot)) {
			...
			if (adjustToScrollview && this.list.chart) {
			...
			if (scrollview) {
				...
				var maxSlotWidth = container.clientWidth; // container.getBoundingClientRect().width;
				var maxHeight;
				this.tableSlot.style.maxWidth = "";
				if (this.list.$item.$fixedBodyHeight) {
					var height = scrollBodySlot.clientHeight;
					scrollBodySlot.style.height = ((Math.abs(parseInt(this.list.$item.$fixedBodyHeight, 10) - height) < 2) ? height : this.list.$fixedBodyHeight) + "px";
				} else {
						...
						if (this.list.$fitContainer) {
							...
							if (scrollview) {
								...
								if (this.list.page.isLookUpPage) {
									maxH = syra_site.getSize().dlg.maxHeight;
									maxH -= (this.scrollTable.headSlot.clientHeight + this.list._core.offsetTop + pageSize.lookupMargin);
								} else {
									maxH = pageSize.maxScrollHeight || scrollview.clientHeight;
									maxH -= (this.scrollTable.headSlot.clientHeight + 100);
								}
				...
				if (this.emptyDataItem.parentNode && (this.list.treeDecorator || this.list.$item.$isTitleRowHidden)) {
					...
					if (this.fixedTable) {
						...
						var hasMoved;
						while (this.fixedTable.columns.length && Math.max(maxSlotWidth - fixedWidth, 0) <= 300) {
							hasMoved = true;
						...
						if (hasMoved) {
							if (this.fixedTable.columns.length) {
								...
								fixedWidth = 0;
								syra_dom.removeChild(this.fixedTable.slot);
								syra_site.disposeObject(this.fixedTable);
								delete this.fixedTable;
							}
						}
					}
					var hasScollHeight = scrollBodySlot.scrollHeight > (scrollBodySlot.clientHeight + 1);
					var fitWidth;
					if (this.list.$isEditMode) {
						var definedWidth;
					...
					scrollBodySlot.style[syra_site.isDocumentRTL ? "paddingLeft" : "paddingRight"] = hasScollHeight ? "17px" : "";
					...
					if (scrollWidth && !this._slotTableWidh) {
						this.slotTable.style.width = this._slotTableWidh = "100%";
					}
					var fixedBody = this.fixedTable && this.fixedTable.bodySlot;
					if (fixedBody) {
						var height = scrollBodySlot.clientHeight;
						if (!height && this.list.records.length) {
							fixedBody.style.maxHeight = ""; //manage case where there is no scroll column visible but only fixed column
						} else {
							fixedBody.style.maxHeight = height + "px";
						}
					}
					this.scrollTable.slot.style.maxWidth = this.tableSlot.clientWidth - (fixedWidth || 0) + "px"; //adjust maxwidth to size of container for table
					if (this.emptyDataItem.parentNode) {
			...
			if (this.list.inlineCards) {
				var width = scrollBodySlot.clientWidth;
				if (scrollBodySlot.scrollWidth != width) {
					width -= 17;
				}
				width = (width - (16 + 4)) + "px";
	...
	adjustColToContent: function() {
		...
		for (var ii = 0, jj = this.allColumns.length; ii < jj; ii++) {
			delete this.allColumns[ii].dynWidth;
		}
		this.onResizeArray();
	},
