...
var FilterRecord = require("./gridMode/filter/filterRecord").FilterRecord;
var GridCardBuilder = require('./gridMode/gridCardBuilder').GridCardBuilder;
var _filterBuilder = require('./gridMode/filter/filterBuilder');
...
function _addTitleRow(builder) {
	...
	list.gridCardBuilder.isRowCardMode && freezeTable.addRowCardCol();
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	endInitiallize: function() {
		...
		this.list.gridCardBuilder = new GridCardBuilder();
		this.list.gridCardBuilder.load(this.list);
	},
	...
	list_onAfterApplyDesignMetaData: function(metaData, designing) {
		if (designing && metaData.$cardPosition !== undefined) {
			this.list.gridCardBuilder.applyDesignMetaData(metaData, designing);
		}
	...
	onAfterLoadRecord: function(record, initData, isCreateAction) {
		if (isCreateAction) {
			this.list.gridCardBuilder.isRowCardMode && this.list.gridCardBuilder.toggleRowCard(record, true);
		}
	},
	...
	record_draw: function(record) {
		...
		for (var ii = 0, jj = this.allColumns.length; ii < jj; ii++) {
			...
			if (col == this.columnsMap.rowCard) {
				record[col.table.$rowKey].appendChild(this.list.gridCardBuilder.appendRowCardOpener(record, col));
				continue;
			}
	...
	record_onSelect: function(record, selected) {
		...
		this.list.gridCardBuilder.onShowRecordSelection(record, selected);
	...
	dispose: function(removeItem) {
		if (removeItem) {
			...
			this.list.gridCardBuilder && this.list.gridCardBuilder.removeOutCardSlot();
		}
		syra_dd.addToColResizers(this.list, false);
		syra_site.disposeObject(this.list.gridCardBuilder);
	...
	resizeRecord: function(record) {
		if (this.list.gridCardBuilder && (record.cardItem || record.popupCard)) {
			if (this.list.gridCardBuilder.isRowCardMode && record.body) {
	...
	record_onAfterApplyChange: function(record, metaData) {
		this.list.gridCardBuilder.onAfterApplyChange(record);
	},
	record_onMenuClick: function(record, menuItem) {
		if (!record.disposed) {
			...
			this.list.selector.selectRecord(this.list.recordsMap[this.list.clientDataset[$newIndex].$uuid], true);
			return false;
		}
	},
	...
	onBeforeListSetFocus: function(select, name, $serverIndex) {
		if (_popupCardBuilder.isOpened(this.list)) {
			var record = this.list.selector.getSelectedRecord();
			var focusRecord = this.list.recordsMap[this.list.clientDataset[$serverIndex].$uuid];
			if (focusRecord && (!record || (record.$uuid != focusRecord.$uuid))) {
				this.list.selector.unSelectAll();
				this.list.selector.selectRecord(focusRecord, true);
			}
		}
	},
