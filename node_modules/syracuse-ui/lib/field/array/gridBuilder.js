...
function _addHScroller(builder) {
	...
	scrollTable.bodySlot_onScroll = scrollTable.bodySlot.addEventListener("scroll", function(event) {
...
exports.GridBuilder = helpers.defineClass(GridBuilder, null, {
	...
	resizeScrollView: function() {
		if (!this.list.isTableSlotHidden) {
			...
			var scrollview = this.list.page.scrollview;
			var disableAdjust;
			...
			if (this.list.$item.$fixedBodyHeight) {
				...
				if (this.list.adjustToScrollPage && !disableAdjust) {
					...
					if (this.list.$fitContainer) {
						...
						if (scrollview && this.list.page.lookupField != this.list) {
							var maxH = scrollview.clientHeight;
							maxH -= this.scrollTable.headSlot.clientHeight;
							bodySlot.style.maxHeight = maxHeight = Math.max(maxH - 50, 100) + "px";
						}
					}
				}
			...
			if (this.emptyDataItem.syraIsDisplay && this.list.$item.$isTitleRowHidden) {
				...
				if (freezeBody && maxHeight !== undefined) {
					freezeBody.style.maxHeight = bodySlot.clientHeight + "px";
				}
