...
exports.Builder = helpers.defineClass(Builder, null, {
	...
	list_scrollToField: function(field) {
		this.list_scrollToRecord(field.articleParent);
	...
	list_scrollToRecord: function(record) {
		record && syra_page.scrollToItem(this.list.page, record.dataRow, this.scrollTable.bodySlot, true);
	},
	...
	list_onAfterDraw: function() {
		...
		this.list._endDrawBox();
	},
	_hasFreezeField: function($items) {
		if (!syra_site.enableTestRobot && !this.list.page.lookupField) {
	...
	record_dispose: function(record) {
		...
		record.rowIndexBtn = record.cellsMap = record.fieldCellsMap = record.treeNode = record.dataRow = record.freezeRow = record.popupCard = record.titleSlot = null;
	},
	dispose: function() {
		...
		this.tableSlot = this.freezeTable = this.scrollTable = this.list = this.columnsMap = this.allColumns = this.popupCardBuilder = this.filterRecord = this.cardDecorator = this.emptyDataItem = null;
	},
	...
	record_reorder: function(record, target, isAfter) {
		...
		if (record.list.quickEdit && record.list.quickEdit.record == record) {
			record.dataRow.parentNode.insertBefore(record.list.quickEdit.card.row, record.dataRow.nextSibling);
		}
	},
	...
	record_onMenuClick: function(record, menuItem) {
		if (!record.disposed) {
			...
			if ($startIndex != this.list.pagging.$startIndex) {
				...
				this.list.fetcher.fetchClientRecordSet(this.list, options);
			}
