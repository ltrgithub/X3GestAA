...
GridTable.prototype._addFilterCell = function(col) {
...
GridTable.prototype.addFieldCol = function($item, $field) {
	...
	if (!this.list.treeDecorator && $field.$capabilities && ($field.$capabilities.indexOf("sort") >= 0)) {
		col.isSortable = true;
		col.sortBtn = syra_button.add({
			parent: col,
			css: "s-sort " + col.css,
			link: col.titleCell,
			click: this.list.sorter.onSortClick
		});
	}
...
GridTable.prototype.setColWidths = function(fitWidth) {
	...
	var total = 0;
	var fieldsCount = 0;
	var $columnWidths = (this.list.$pagePreferences.$columnWidths) || {};
