...
var _colUpdater = require('./tools/colUpdater');
	...
	if (on) {
		if (!self.headSlot_onScroll) {
			self.headSlot.addEventListener("scroll", self.headSlot_onScroll = function(event) {
				...
				if (page && page.isPageLoaded && !page._isDataChanging) {
	...
	if (on) {
		if (!self.bodySlot_onScroll) {
			self.bodySlot.addEventListener("scroll", self.bodySlot_onScroll = function(event) {
				if (self.scrollRequested) {
					delete self.scrollRequested;
				} else {
					var page = self.list.page;
					if (page && page.isPageLoaded && !page._isDataChanging) {
						...
						if (!self.isFixedCols) {
							if (self.builder.fixedTable) {
								var body = self.builder.fixedTable.bodySlot;
								if (body && body.scrollTop != this.scrollTop) {
									self.builder.fixedTable.scrollRequested = true;
									body.scrollTop = this.scrollTop;
								}
							}
						} else {
							if (self.builder.scrollTable) {
								var body = self.builder.scrollTable.bodySlot;
								if (body && body.scrollTop != this.scrollTop) {
									self.builder.scrollTable.scrollRequested = true;
									body.scrollTop = this.scrollTop;
								}
							}
						}
...
GridTable.prototype.addFieldCol = function($item, $field) {
	...
	switch ($field.$type) {
		...
		case "application/x-datetime":
			$field.$localFormatDisplay = syra_culture.format.get($field.$type, $field.$format);
			$field.$maxLength = syra_culture.format.getLength($field.$localFormatDisplay);
			break;
	}
	...
	if (!this.colUpdater) {
		this.colUpdater = _colUpdater.add(this);
	}
	return col;
};
...
GridTable.prototype.addSelectorColumn = function() {
	...
	this.list.filter_row && this.list.filter_row.addEmptyCell(col);
	return col;
};
	...
	if (sourceRow) {
		if (target) {
			...
			if (targetRow) {
				...
				targetRow.parentNode && targetRow.parentNode.insertBefore(sourceRow, isAfter ? targetRow.nextSibling : targetRow);
...
GridTable.prototype.dispose = function() {
	this.colUpdater && this.colUpdater.dispose();
