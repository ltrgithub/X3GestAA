...
function _setColumnTitle(col, $title) {
	...
	if ($title && $title.length > 0 && $title[1] == "@") {
		$title = syra_site.expressionMaker.parse(col.list, $title);
	}
	col.titleText = $title || "";
	if (col.titleText.length > 1) {
		col.titleText = col.titleText.substr(0, 1).toUpperCase() + col.titleText.substr(1);
	}
	syra_site.dom.toggleClass(col.titleLabel.link, "s-grid-title-empty", col.titleText.length == 0);
...
exports.GridTable = _helpers.defineClass(GridTable, null, {
	load: function(builder, isFixedCols, slot) {
		...
		if (this.list.$item.$isTitleRowHidden) {
			syra_site.dom.hide(this.headTable, true);
		}
	...
	showBody: function(show) {
		if (show) {
			...
			if (!this.bodyInsert) {
				this.bodyInsert = {
					parentNode: this.bodySlot
				};
				syra_site.dom.removeChild(this.bodyTable);
			}
		}
	},
	...
	addFieldCol: function($item, $field) {
		...
		syra_site.dom.hide(col.titleCell, col.$isHidden);
		col.$bind = $item.$bind;
		col.$item = _helpers.object.clone($item, true);
		col.$item.$isCellChild = true;
	...
	applyColumnMetaData: function(col, $metaField) {
		...
		if ($metaField.$isHidden !== undefined) {
			...
			for (var mm = 0, kk = this.list.records.length; mm < kk; mm++) {
				syra_site.dom.hide(this.list.records[mm].cellsMap[col.$bind], col.$isHidden);
			}
			if (this.builder.filterRecord) {
				syra_site.dom.hide(this.builder.filterRecord.cellsMap[col.$bind], col.$isHidden);
			}
		}
	},
	...
	reorderRow: function(target, source, isAfter) {
		...
		if (sourceRow) {
			if (target) {
				...
				if (targetRow) {
					var widthUpdated;
					var sourceIndex = syra_site.dom.getIndex(sourceRow);
					var targetIndex = syra_site.dom.getIndex(targetRow);
		...
		for (var ii = 0, jj = this.columns.length; ii < jj; ii++) {
			...
			if (!col.$isHidden) {
				...
				if (col.hasTitleEmptyCss != isEmpty) {
					...
					syra_site.dom.toggleClass(col.titleCell, "s-grid-title-cell-empty", col.hasTitleEmptyCss);
				}
