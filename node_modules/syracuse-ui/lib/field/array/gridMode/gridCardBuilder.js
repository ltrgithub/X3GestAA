...
exports.GridCardBuilder = helpers.defineClass(GridCardBuilder, null, {
	...
	appendRowCardOpener: function(record, col) {
	...
	onRowCardOpenerClick: function(opener, record) {
		...
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			var item = records[ii];
			this.toggleRowCard(item, show);
			if (show) {
				syra_site.ensureArticleVisibility(item, true);
			}
		}
		var title;
		if (record) {
		...
		if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") { // || metaData.$cardPosition == undefined) {
			...
			if (metaData.$cardPosition == "row") {
				...
				for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
					...
					var row = record[col.table.$rowKey];
					row.insertBefore(this.appendRowCardOpener(record, col), record.cellsMap.$rowIndex ? record.cellsMap.$rowIndex.nextSibling : row.firstChild);
					if (ii == 0) {
