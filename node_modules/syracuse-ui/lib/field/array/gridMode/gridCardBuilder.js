...
exports.removeRecordCards = function(builder) {
	...
	if (col) {
		...
		syra_dom.removeChild(col.titleCell);
		var record = builder.filterRecord;
		if (record) {
			syra_dom.removeChild(record.cellsMap[col.key]);
...
function _setRowCardOpenerIcon(opener, expanded) {
	syra_menus.button.setText(opener, undefined, (opener.expanded = expanded) ? "collapse_s" : "expand_s");
}
...
function _createRowCard(record) {
	...
	card.cell.className = "s-grid-cards-cell";
...
function _onRowCardClick() {
	...
	for (var ii = 0, jj = records.length; ii < jj; ii++) {
		...
		show && syra_site.validateLayoutContent(item);
	}
	syra_menus.button.setText(this, title);
...
exports.addRowCardCol = function(builder) {
	if (builder.isRowCardMode) {
		...
		_setRowCardOpenerIcon(syra_menus.button.add({
			parent: builder.list,
			slot: col.titleCell,
...
exports.addRowCardOpener = function(record, col) {
	...
	record.rowCardBtn = syra_menus.button.add({
		parent: record,
		slot: td,
...
exports.removeOutSlot = function(builder) {
	syra_dom.removeChild(builder.outCardSeparator);
	syra_dom.removeChild(builder.outCardSlot);
...
function _ensureCard(record, slot) {
	...
	if (!record.cardItem) {
		...
		syra_site.ensureArticleVisibility(record);
...
exports.load = function(builder) {
	...
	builder.record_onAfterApplyChange = function(record) {
		if (!record.list.$isEditMode && record.dataset && record.rowCardBtn) {
			...
			syra_menus.button.visibility(record.rowCardBtn, hasData);
		}
	};
