...
function _removeRecordCards(gridCardBuilder) {
	var list = gridCardBuilder.list;
	...
	if (col) {
		...
		gridCardBuilder.$rowFieldBinds = null;
	}
...
function _onRowCardOpenerClick(list, btn) {
	...
	if (record == list) {
		...
		list.gridCardBuilder.expandAll = show;
	} else {
		records = [record];
	...
	for (var ii = 0, jj = records.length; ii < jj; ii++) {
		var item = records[ii];
		list.gridCardBuilder.toggleRowCard(item, show);
		if (show) {
...
exports.GridCardBuilder = helpers.defineClass(GridCardBuilder, null, {
	load: function(list) {
		...
		if (this.list.$item.$inlineCard || this.list.$item.$cards) {
			_convertOldCardItem(this.list);
		}
		this.appendRowCard();
		this._ensureMode();
		this.list.onRowCardClick = function(event, btn) {
	...
	addRowCardOpener: function(record, col) {
		var td = document.createElement("td");
		td.className = this.list.cssCell + " s-grid-rowcard-cell-picker";
		...
		if (this.expandAll) {
			record.expandRowCard = this.expandAll;
		}
	...
	onAfterApplyChange: function(record) {
		...
		if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") { // || metaData.$cardPosition == undefined) {
			...
			if (metaData.$cardPosition == "row") {
				...
				for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
					...
					var row = record[col.table.$rowKey];
					row.insertBefore(this.addRowCardOpener(record, col), record.cellsMap.$rowIndex ? record.cellsMap.$rowIndex.nextSibling : row.firstChild);
					if (ii == 0) {
						record.expandRowCard = true;
					}
				}
				this.list.resizeArticle();
			} else {
				this._ensureMode();
			}
		} else {
			...
			if (this.list.records.length > 0) {
				this.list.selector.selectRecord(this.list.selector.getSelectedRecord() || this.list.records[0], true);
			}
		}
	},
	addRowCardCol: function() {
		if (this.isRowCardMode) {
			var table = this.list.builder.freezeTable;
			if (!table || !table.columns.length) {
				table = this.list.builder.scrollTable;
			}
			...
			_setRowCardOpenerIcon(opener, false);
			col.titleCell.appendChild(opener);
			this.list.builder.filterRecord && table._addFilterCell(col);
			return col;
		}
	},
	appendOutCardSlot: function() {
		...
		var cssCell = "";
		var tableSlot = this.list.builder.tableSlot;
