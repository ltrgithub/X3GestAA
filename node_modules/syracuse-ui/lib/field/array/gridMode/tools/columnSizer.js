	...
	switch (col.$field.$type) {
		...
		case "application/x-datetime":
			if (records.length) {
				...
				col.colWidth = (col.$field.$maxLength * 6) + _widths.input.padding;
			}
			break;
...
function _findFieldEditColWidth(records, col, firstCell) {
		...
		switch (col.$field.$type) {
			case "application/x-choice":
				...
				/*case "application/x-date":
				 ...
				 col.colWidth = (firstField.$field.$maxLength || (firstField.localFormat.length * 8)) + _widths.input.padding;
				 }
				 break;*/
			case "application/x-quantity":
			case "application/x-reference":
	...
	if (syra_site.isTabletDevice) {
		col.colWidth += 10; //add padding for specific buttons, mailny for number
	}
	if (firstField) {
...
exports.setColumnWidth = function(col, records, $prefWidths, firstCell) {
	...
	if (!col.colWidth && !syra_site.isTabletDevice) {
		col.colWidth = $prefWidths[col.key];
		if (!col.colWidth) {
			if (col.isTreeview) {
				col.colWidth = builder.list.$item.$treeWidth;
			}
			if (col.$field) {
				col.colWidth = col.$item.$width || col.$field.$width;
			}
		}
	}
	...
	if (col.colWidth) {
		...
		if (col == builder.columnsMap.$treeview) {
			...
			if (col.useContentWidth && firstCell) {
				...
				if (col.$field) {
					...
					if (col.$field.$filterable) {
						col.colWidth += 36;
						if (col.filter_cell && col.filter_cell.field && col.filter_cell.field.highField) {
							col.colWidth = Math.min(300, col.colWidth * 2);
						}
					}
					if (col.dynWidth) {
						col.dynWidth = col.colWidth;
		...
		if (col.dynWidth !== undefined && col.dynWidth <= 2) {
			col.colWidth = col.dynWidth = 0;
		}
		if (col.colWidth > 1) {
			//>1 cause scrollwidth error
			col.colWidth += 4; //firstCell border 	
		} else {
			col.colWidth = 0;
		}
		if (col.isTitleEmpty) {
