...
function _findFieldEditColWidth(records, col, firstCell) {
	var firstField = records[0] && records[0].fieldCellsMap[col.$bind];
	var pickerWidth = 0;
	if (firstField && firstField.buttons) {
		var count = firstField.buttons.length;
		pickerWidth = count && (count * 22);
	}
	col.colWidth = col.$field.$displayLength || col.$field.$maxLength || 0;
	if (col.colWidth) {
		col.colWidth = col.dynWidth = (col.colWidth * 8) + _widths.input.padding;
	}
	if (!col.colWidth) {
		switch (col.$field.$type) {
			case "application/x-choice":
				...
				if ($enum) {
					...
					col.dynWidth = col.colWidth = (col.colWidth * 8) + _widths.input.padding;
				}
				break;
			case "application/x-date":
			case "application/x-time":
			case "application/x-datetime":
				if (firstField && firstField.localFormat) {
					col.dynWidth = col.colWidth = (firstField.localFormat.length * 8) + _widths.input.padding;
				}
				break;
			case "application/x-quantity":
			case "application/x-reference":
			...
			case "application/x-boolean":
				if (firstCell) {
					col.dynWidth = col.colWidth = firstCell.scrollWidth;
				}
				break;
			case "application/x-icon":
				col.dynWidth = col.colWidth = 82; //32 for padding
	...
	col.colWidth = (col.colWidth || 0) + pickerWidth;
}

exports.setColumnWidth = function(col, records, $prefWidths, firstCell) {
	var builder = col.table.builder;
	col.colWidth = col.dynWidth || $prefWidths[col.$bind] || (col.$item && col.$item.$width) || (col.$field && col.$field.$width);
