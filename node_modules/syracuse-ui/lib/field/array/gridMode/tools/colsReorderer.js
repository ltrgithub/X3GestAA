...
function _moveCols(colSource, colTarget, dropAtEnd) {
	...
	syra_site.dom.toggleClass(colSource.titleCell, "s-grid-fixed", colSource.table.isFixedCols);
	_fillItemsList(list);
	var $columns = [];
	for (var ii = 0, jj = list.$item.$layout.$items.length; ii < jj; ii++) {
		$columns.push(list.$item.$layout.$items[ii].$bind);
	}
	(list.$userPreferences = list.$userPreferences || {}).$columns = $columns;
	list.saveListDesign(true, list.$userPreferences);
	list.resizeArticle();
}
...
exports.moveToSrollTable = function(col) {
	...
	syra_site.dom.toggleClass(col.titleCell, "s-grid-fixed", false);
	var record = list.builder.filterRecord;
	if (record) {
...
exports.resetReorder = function(list) {
	...
	if ($binds) {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			col.table = newTable;
			syra_site.dom.toggleClass(col.titleCell, "s-grid-fixed", col.table.isFixedCols);
		}
		_fillItemsList(list);
	}
};
...
helpers.defineClass(ColsReorderDDAgent, null, {
	start: function(dropItem) {
		...
		syra_site.dom.hide(this.cueTop, true);
		syra_site.dom.hide(this.cueBottom, true);
	...
	onColumnHeaderEnterLeave: function(col, onEnter) {
		if (onEnter) {
			...
			syra_site.dom.hide(this.cueTop, false);
			syra_site.dom.hide(this.cueBottom, false);
