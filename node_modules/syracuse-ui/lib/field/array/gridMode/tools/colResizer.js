...
function _getStartX(col) {
	return col.rect[syra_context.isRTL ? "left" : "right"];
}
...
helpers.defineClass(ColResizerDDAgent, null, {
	...
	onDragMouseMove: function(target, event) {
		...
		if ((event.pageY >= (_colReszing.rect.top - 20)) && (event.pageY <= (_colReszing.rect.bottom + 20))) {
			...
			if (this.isDragging) {
				var newWidth = _startWidth + (syra_context.isRTL ? (_startX - event.pageX) : (event.pageX - _startX));
				if (newWidth > 50) {
	...
	onDragMouseUp: function(target, event) {
		if (this.isDragging) {
			_list.$pagePreferences.$columnWidths = _list.$pagePreferences.$columnWidths || {};
			delete _list.$pagePreferences.$colWidths;
			_list.$pagePreferences.$columnWidths[_colReszing.$item.$bind] = _colReszing.$item.$width = Math.round(_colReszing.colWidth) + "px";
			delete _colReszing.dynWidth;
			_list.saveListDesign();
			syra_preference.page.saveField(_list, _list.$pagePreferences);
			delete _colReszing.newWidth;
			syra_site.layoutSlot.style.cursor = "default";
			_list.resizeItem(true);
		}
	},
	dispose: function() {
