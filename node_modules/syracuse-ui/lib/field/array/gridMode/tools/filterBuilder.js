...
function _addFieldCell(record, col, $field) {
	...
	if ($field.$hasFilterCapability) {
		...
		if (params && params.where) {
			...
			var exp = _filterMaker.parseWhere(params.where, keys);
			if (exp && keys == keys) {
				if (exp.$operator === undefined) {
					exp.$operator = _operators.getOperator(field, exp.$value);
				}
				_operators.setOperator(field, exp.$operator);
				if (field.isReferenceField) {
					var obj = {};
