...
function _getDefaultOperator(field) {
	return _ensureOperators(field)[1];
}

function _ensureOperators(field) {
	if (!field.operators) {
...
function _onFilterOperatorClick(event) {
...
function _onFilterItemClick(event) {
	var field = this.parent;
	_setOperator(field, this.operator);
	_popup.close();
...
function _setOperator(field, operator) {
	...
	syra_menus.updateButtonIcon(field.filterOperatorBtn.link, "", "filter_" + (field.$filterOperator = operator));
}
...
function _loadField(record, slot, $field, $bind) {
	...
	field.filterOperatorBtn = syra_menus.button.add({
		...
		btnclick: _onFilterOperatorClick
	});
...
function _onNotifyDataChange(record, field, value) {
	...
	if (value == null || value == "") {
		...
		if (field.$filterOperator === "none" && field.$field) {
			_setOperator(field, _getDefaultOperator(field));
		}
	}
	_buildFilter(record);
	return false;
	...
	if (record) {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if (field.$field) {
				...
				var defaultFilter = (value !== "" && field.$filterOperator === "none") ? _getDefaultOperator(field) : (value === "" && field.$filterOperator !== "none") ? "none" : undefined;
...
exports.addFieldCell = function(col, $field) {
	...
	if ($field.$hasFilterCapability) {
		...
		if (params && params.where) {
			...
			var exp = syra_site.filterMaker.parseWhere(params.where, $filterBind);
...
exports.addRecord = function(list) {
	...
	record.onNotifyDataChange = function(field, value) {
		return _onNotifyDataChange(this, field, value);
	};
