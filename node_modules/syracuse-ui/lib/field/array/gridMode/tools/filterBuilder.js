...
var _operators = {
	...
	setOperator: function(field, operator) {
		syra_site.dom.toggleClass(field.filterCell, "s-list-filter-cell-active", operator != "none");
	...
	if ($field.$hasFilterCapability) {
		...
		if (params && params.where) {
			...
			if (exp && field.$filterBind == exp.$bind) {
				_setDataValue(record, field, exp.$value, exp.$operator);
			}
		}
		return true;
	}
...
exports.addRecord = function(list) {
	...
	record.drawBox = function() {
		...
		this.list.builder.freezeTable && this.list.builder.freezeTable.headTable.appendChild(this.freezeRow = document.createElement("tr"));
	...
	record.getValues = function() {
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			var value = _getCellValue(this.boundFields[$binds[ii]][0]);
			value && values.push(value);
		}
		return values;
	};
	record.validateDisplay = function() {
		...
		if (show && !this.list.toggleFilterBtn) {
			this.list.toggleFilterBtn = syra_menus.button.add({
				...
				iconOnly: true,
				css: "s-mn-list-btn " + this.list.$mnPickersCss,
				fontIcon: "clear_filter",
				btnclick: function() {
					var fields = this.parent.boundFields;
					var $binds = Object.keys(fields);
					for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
						_setDataValue(this.parent, fields[$binds[ii]][0], null);
					}
					_triggerFilter(this.parent);
				}
			});
			this.list.topbar.insertBefore(this.list.toggleFilterBtn.link, this.list.searcherSlot);
			this.list.warn = document.createElement("div");
			this.list.warn.className = "s-list-filter-warn";
			this.list.warn.textContent = syra_local.flfilter_warn;
			this.list.topbar.insertBefore(this.list.warn, this.list.searcherSlot);

		}
		syra_site.dom.display(this.list.warn, show);
		syra_menus.button.display(this.list.toggleFilterBtn, show);
	};
