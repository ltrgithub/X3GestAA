...
function _onPickerClick() {
	...
	if (record) {
		...
		var $mn = record.$menus.$quickEdit = syra_site.clone(record.$menus.$edit);
		$mn.$target = "inline";
		syra_menus.click.fire({
			scope: record,
			$bind: $mn.$bind = $mn.$sourceBind = "$quickEdit"
		});
	}
}
...
helpers.defineClass(QuickEdit, null, {
	load: function(record, $itemPage) {
		...
		if (self.list) {
			...
			self.card = builder.createQuickEditCard(self.record = record);
			$itemPage.layoutSlot = self.card.contentSlot;
			$itemPage.inlinePageHost = self;
			self.resize();
	...
	dispose: function() {
		...
		this.card && syra_dom.remove(this.card.row);
		if (this.list) {
