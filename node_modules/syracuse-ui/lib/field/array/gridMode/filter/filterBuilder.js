		...
		doEvent: function() {
			if (_popup && _popupItem && _popupItem.syraItem == field.id) {
				...
				for (var ii = 0, jj = field.operators.length; ii < jj; ii++) {
					...
					var btn = syra_menus.addTextButton(syra_local["flFilter_" + op], "s-mn-link", "onFilterItemClick", null, "filter_" + op);
					btn.syraOperator = op;
...
function _setOperator(field, operator) {
	syra_menus.updateButtonIcon(field.filterOperatorBtn, "", "filter_" + (field.$filterOperator = operator));
}
...
function _loadField(record, slot, $field, $bind) {
	...
	field.filterOperatorBtn = syra_menus.addIconButton(syra_local.flfilter_choiceOperator, "s-list-filter-cell-picker", "onFilterOperatorClick", null, "filter_none");
