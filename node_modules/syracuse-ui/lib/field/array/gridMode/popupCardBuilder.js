...
function _activatePopupCard(record, bind) {
	if (!record._isOnApplyingChange) {
		...
		if (list == list.cardActiveList) {
			...
			if (bind && record.menuItems && record.menuItems) {
				var externalAdapter = record.page && record.page.externalAdapter;
				var recordLinks = {};
				if (externalAdapter.getGridRecordLinks(record, bind, recordLinks)) {
					var cap = list && list.$capability;
					if (cap && (cap.insert || cap["delete"]) && record.$capability) {
						var canCreate = record.$capability.insert && recordLinks.$create && recordLinks.$create.$isHidden !== true;
						var canDelete = record.$capability["delete"] && recordLinks.$delete && recordLinks.$delete.$isHidden !== true;
