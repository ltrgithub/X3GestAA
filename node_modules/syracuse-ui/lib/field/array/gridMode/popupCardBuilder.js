"use strict";
var _activeList, _activeRecord, _slotCard, _slotHeader, _slotBody, _modal;
...
function _hightlightCardRecord(record, show) {
	record.dataRow && syra_dom.toggleClass(record.dataRow, "s-list-popupcard-record", show);
	record.fixedRow && syra_dom.toggleClass(record.fixedRow, "s-list-popupcard-record", show);
}

function _findPopupCardField(name, record) {
	if (record.popupCard && _activeRecord == record) {
		var list = record.list;
		if (exports.isOpened(list) && _modal && !_modal.disposed) {
			var found = null;
		...
		if (list == _activeList) {
			if (_activeRecord) {
				...
				if (_activeRecord.popupCard) {
					syra_dom.remove(_activeRecord.popupCard.content.domItem);
				}
			}
			_hightlightCardRecord(_activeRecord = record, true);
			syra_dom.empty(_slotBody);
			...
			if (!card) {
				record.popupCard = card = {
					content: list.page.addItem(_slotBody, $popupCard || list.$popupCard, record),
			...
			if (_modal) {
				_modal._content = _modal.options.content = card.content;
			}
			...
			if (record.$menusPopup && !card.picker_menus) {
				card.picker_menus = syra_picker.menus.add(record);
			...
			syra_layout.ensureArticleVisibility(record, true, card.content);
			_modal && _modal.resize();
		}
	}
};
		...
		if (!(_activeList == list && _activeRecord == record)) {
			if (_activeList == list) {
				...
				list.page.externalAdapter.onFieldClickPicker({
					...
					pickerType: "openCard",
					doEvent: function() {
						_slotCard = syra_dom.addDiv("s-popupcard-slot");
						_slotHeader = syra_dom.addDiv("s-popupcard-slot-header", _slotCard);
						_slotBody = syra_dom.addDiv("s-popupcard-slot-body", _slotCard);
						!record.popupCard && _activatePopupCard(record);
						_modal = syra_over.openModal(record.page, {
							$isAutoClose: false,
							...
							resize: function(pn, isDlgResizing) {
								...
								var rect = syra_dom.getInnerSize(_slotCard);
						...
						_modal.addCloseButton();
						syra_dom.toggleClass(_modal.btn_close.link, "s-popupcard-close", true);
					}
				});
			}
			return;
...
exports.dispose = function(list) {
	if (_activeList == list) {
		_activeList = _activeRecord = _slotCard = _slotHeader = _slotBody = _modal = null;
	}
};
