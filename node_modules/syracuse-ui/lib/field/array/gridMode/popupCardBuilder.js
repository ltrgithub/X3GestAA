...
function _onClose() {
	if (_activeList) {
		...
		list.page.externalAdapter.onFieldClickPicker({
			...
			doEvent: function() {
				syra_dom.removeChild(_slotCard);
				_dialog.dispose();
				...
				for (var ii = 0, jj = list.records.length; ii < jj; ii++) {
					...
					if (record.popupCard) {
						...
						for (var mm = 0, kk = fields.length; mm < kk; mm++) {
							syra_article.removeItem(fields[mm], false, true);
						}
						syra_article.removeItem(record.popupCard.content, true, true);
		...
		if (list == _activeList) {
			if (_activeRecord) {
				...
				if (_activeRecord.popupCard) {
					syra_dom.removeChild(_activeRecord.popupCard.content.domItem);
				}
			}
			...
			if (_activeList == list) {
				...
				list.page.externalAdapter.onFieldClickPicker({
					...
					doEvent: function() {
						...
						_dialog = syra_dlg.openModal(record.page, {
							$isAutoClose: false,
						...
						syra_dom.toggleClass(_dialog.btn_close.link, "s-popupcard-close", true);
					}
				});
			}
			return;
