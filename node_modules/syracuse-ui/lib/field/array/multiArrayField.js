		...
		if (this.hasGraph(list)) {
			if (!list.toggleGraph) {
				...
				list.builder.list_onAfterSetDataBind = function(dataRecordSet, parentDataRecord, metaData) {
					list.chart && _setChartDataValue(this.list, parentDataRecord, metaData);
				};
	...
	_ensureGraph: function(list, load) {
		if (!list.chart && (load || list.$item.$graphPosition != "behind")) {
			...
			if (list.chart && list.records && list.records.length) {
				_setChartDataValue(list, dataset, dataset.$properties && dataset.$properties[list.$item.$bind]);
			}
		}
	}
};

function _setChartDataValue(list, parentDataRecord, metaData) {
	var dataRecords = [];
	var count = list.records.length;
	if (list.page.isFusionPage && count > 0) {
		if (syra_fusion.sapUtil.Fusion.isEmptyGridDataRecordEx(list.$fields, list.records[count - 1].dataset)) {
			count = count - 1;
		}
	}
