...
var _graph = {
	hasGraph: function(list) {
		return !!(list.$prototype.$cube || list.$prototype.$decorator);
	},
	ensureGraph: function(list, load) {
		if (this.hasGraph(list)) {
			if (!list.toggleGraph) {
				...
				list.builder.list_onAfterSetDataBind = function(dataRecordSet, parentDataRecord, metaData) {
					if (list.chart) {
						...
						_setChartDataValue(this.list, graphRecords, parentDataRecord, metaData);
					}
				};
				list.applyGrapDesignMetaData = function(metaData, designing) {
					...
					this.ensureGraphVisibility();
					this.chart && this.chart.applyDesignMetaData && this.chart.applyDesignMetaData(metaData);
				};
				list.ensureGraphVisibility = function() {
					if (!this.$field.$isListHidden) {
						if (this.$item.$graphPosition == "front" || this.$item.$graphPosition == "behind") {
							var icon, title;
							if (this.$item.$graphPosition == "front") {
								icon = "field_array";
							...
							if (!this.toggleGraphBtn) {
								this.toggleGraphBtn = syra_menus.button.add({
									parent: this,
									text: title,
								...
								this.topbar.insertBefore(this.toggleGraphBtn.link, this.searcherSlot);
							} else {
								syra_menus.button.setText(this.toggleGraphBtn, title, icon);
								syra_menus.button.hide(this.toggleGraphPicker, false);
							}
						} else {
							syra_menus.button.hide(this.toggleGraphBtn, true);
						}
					}
					_graph._ensureGraph(this);
					if (!this.page.isFusionPage && !(this.$field.$decorator && this.$field.$decorator.$stereotype == "calendar")) {
						syra_menus.menus.applyChange(this, {
							$links: {
								$pptslide: {
									$title: syra_local.highCharts_powerpointSlide,
									$url: this.page.$prototype.$representationUrl + "&count={recordsPerPage}" + "&pptMode=newSlide&$bind=" + this.$item.$bind + "&{linkstartP}",
					...
					syra_dom.hide(this.builder.tableSlot, this.$field.$isListHidden || (this.$item.$graphPosition == "front" && !this.builder.isOutCardMode));
					syra_dom.hide(this.graphSlot, this.$item.$graphPosition == "behind");
				};
			}
			list.$item.$graphPosition = list.$item.$graphPosition || "bottom";
			load && this._ensureGraph(list, true);
		}
	},
	_ensureGraph: function(list, load) {
		if (!list.chart && (load || list.$item.$graphPosition != "behind")) {
			var $graphItem = syra_dataset.applyDelta(list.page, {}, list.$item);
			$graphItem.$category = "tabularChart";
			$graphItem.attachedField = list;
			list.chart = list.page.loadNewItem(list.graphSlot, $graphItem, list.articleParent);
			if (list.chart && list.records && list.records.length) {
				var dataset = list.articleParent.dataset;
				if (dataset) {
					_setChartDataValue(list, dataset[list.$item.$bind], dataset, dataset.$properties && dataset.$properties[list.$item.$bind]);
				}
			}
		}
	}
...
function _setChartDataValue(list, dataRecordSet, parentDataRecord, metaData) {
	if (list.page.isFusionPage) {
		var fixedDataSet = dataRecordSet;
		var last = fixedDataSet.length > 0 && fixedDataSet[fixedDataSet.length - 1] || {};
		if (_sapUtil.Fusion.isEmptyGridDataRecordEx(list.$fields, last)) {
			dataRecordSet = fixedDataSet.slice(0, -1);
		}
	}
	list.chart.setDataBind(dataRecordSet, parentDataRecord, metaData);
}
...
exports.MultiArrayField = helpers.defineClass(MultiArrayField, ArrayField, {
	...
	resizeArticle: function() {
		if (!this.disposed && syra_dom.isVisible(this.domItem)) {
			...
			this.chart && this.chart.resizeArticle && this.chart.resizeArticle();
	...
	notifyActionChange: function(menuItem, target, value, notifyServer) {
		...
		if (value.$create) {
			var dataRecords = syra_dataset.ensure(this);
	...
	endDrawBox: function() {
		...
		if (!this.$isQuickDesignerDisabled) {
			syra_menus.menus.applyChange(this, {
				$links: {
					$design: {
						$title: _graph.hasGraph(this) ? syra_local.flDesignGraph : syra_local.flDesign
					}
				}
			});
		}
