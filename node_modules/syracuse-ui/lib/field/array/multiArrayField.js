...
function _applyFacetSetting(list) {
	switch (list.articleParent.$facet) {
		...
		case "$lookup":
			...
			list.page.lookupField = list;
			list.pagging.pagerTopOnly = true;
			break;
		case "$select":
...
exports.MultiArrayField = helpers.defineClass(MultiArrayField, ArrayField, {
	loadBox: function(initData) {
		if (!this.$field.$builderClass) {
			if (this.$item.$format != "cards") {
				this.$item.$format = "grid";
				this.$field.$builderClass = _gridBuilder;
			} else {
				this.$field.$builderClass = _cardsBuilder;
			}
		}
