...
function _endSwitchDesigner(list, open) {
	...
	if (list.page.isVignettePage && list.articleParent == list.page) {
		var vignette = list.page.vignetteField;
		if (vignette.btns.items) {
			var ids = Object.keys(vignette.btns.items);
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				if (ids[ii] != "maximize") {
					syra_site.dom.disableItem(vignette.btns.items[ids[ii]], open);
				}
			}
		}
	}
...
exports.MultiArrayField = helpers.defineClass(MultiArrayField, ArrayField, {
		...
		this.switchDesigner = function(open) {
			...
			if (open && !self.designer) {
				require.async('syracuse-ui/lib/authoring/listDesigner', function(err, module) {
					self.graphDecorator && self.graphDecorator.ensureGraph(true);
					self.ensureGrapDecorator();
		...
		this.toggleUIDesign = function(enable, disposingDesigner) {
			if (enable) {
				if (!this.designBtn) {
					...
					this.topbar.insertBefore(slot, this.topbar.firstChild);
				}
			} else {
	...
	initializeList: function() {
		...
		this.fitWidth = this.page.isVignettePage || this.$item.$fitWidth;
		switch (this.articleParent.$facet) {
			case "$query":
				...
				this.$addMaximize = !this.page.vignetteField;
				break;
			case "$bulk":
				this.adjustToScrollPage = true;
				this.fitWidth = true;
			...
			default:
				...
				if (this.$addMaximize === undefined) {
					this.$addMaximize = !this.$item.$selectorCard;
				}
	...
	clearPreferences: function() {
		this.pagging && this.pagging.clearPreferences();
	...
	resizeArticle: function() {
		if (!this.disposed && this.page.isItemVisible(this)) {
			this.builder.onResizeArray && this.builder.onResizeArray();
			if (this.records & this.builder.resizeRecord) {
				for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
					this.builder.resizeRecord(this.records[ii]);
				}
			}
	...
	onNotifyDataChange: function(field, value) {
		if (field.$item.$bind == "$itemsPerPage") {
			...
			(this.$userPreferences = this.$userPreferences || {}).$itemsPerPage = this.pagging.getItemsPerPage();
