...
exports.SingleRecord = helpers.defineClass(SingleRecord, RecordArticle, {
	...
	onItemEnterLeave: function(event) {
		if (this.menusSlot) {
			document.site.dom.toggleClass(this.menusSlot, "s-enter", (event.type == "mouseenter"));
		}
		if (this.reorderPicker) {
			document.site.dom.toggleClass(this.reorderPicker, "s-enter", (event.type == "mouseenter"));
		}
	...
	_createMenusBox: function() {
		...
		this.page.loadNewItem(this.menusSlot, {
			$layoutType: "row",
			$autoSize: true,
			$items: [this.list.defineNewIconMenu("$delete", this.list.$skinMenus + "-link", true), {
				$isTestMenu: true,
				$isMenusBag: true,
				$category: "menus",
				$noText: true,
	...
	dispose: function() {
		this.recordValue = this.singleField = this.toolbar = this.$$value = this.domItem = null;
