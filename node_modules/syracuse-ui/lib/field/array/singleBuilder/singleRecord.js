...
exports.SingleRecord = helpers.defineClass(SingleRecord, RecordArticle, {
    onNotifyRecordChange: function(value, binding){
        ...
        if (binding == "$actions") {
            if (!self.$isDisabled && !self.$isDeleted) {
                if (value.$delete || value.$create) {
                    ...
                    if (value.$create) {
                        ...
                        builder.list._store.appendRecord({
                            $recordIndex: index,
                            isInsert: true
                        });
                    }
                    builder.setRecordsIndex();
            ...
            var dataset = this.builder.list.getDataSet();
    ...
    notifyReorder: function(targetRecord, $insert){
        ...
        var $resources = this.builder.list._store._records.map(function(record){
            return record._singleField.getDataValue();
        });
        ...
        if (self.$isEditMode) {
            ...
            if (self.list.$capability.insert) {
                metaData.$actions.$create = {
                    $title: self.list._localize.slist_insert
                };
            }
            if (self.list.$capability["delete"]) {
                metaData.$actions.$delete = {
                    $title: self.list._localize.slist_delete
                };
            }
