...
exports.SingleRecord = helpers.defineClass(SingleRecord, RecordArticle, {
	loadBox: function(initData) {
		...
		// append diagnoses if any for reference type single records
		if (this._singleField && this._singleField.$field && this._singleField.$field.$type == 'application/x-reference') {
	...
	drawBox: function() {
		...
		self._menusSlot = document.createElement("div");
		self._menusSlot.className = self.list.$skin + "-actions-cell";
		toolbar.appendChild(self._menusSlot);
		...
		self._singleField = self.page.loadNewItem(dataSlot, {
		...
		if (self.isInsert) {
			self.$$item.insertBefore(self.list.records[self.$recordIndex].$$item);
	...
	onFieldValidate: function(field, value, errors) {
		...
		if (!self.list.$prototype.$isMultiValue && (self.list.$prototype.$item.$type != "application/x-boolean")) {
			...
			self.list.records.forEach(function(record) {
				if (record != self) {
