...
exports.SingleList = helpers.defineClass(SingleList, DesktopList, {
	...
	drawBox: function() {
		...
		this.menusbar.appendChild(bar);
		this._core.appendChild(this.body);
		this.menusbar.style.display = bar.style.display = "";
	...
	_appendMenusBar: function() {
		if (this.$isMenuBarAtTop) {
			if (this.bodyInsert) {
				this.bodyInsert.parentNode.insertBefore(this.menusbar, this.bodyInsert.parentNode.firstChild);
			} else {
				this.body.parentNode.insertBefore(this.menusbar, this.body);
			}
		} else {
	...
	validatetemp: function(newValue, errors) {
		if (!this.$prototype.$isMultiValue && (this.$prototype.$item.$type != "application/x-boolean")) {
			var values = [];
