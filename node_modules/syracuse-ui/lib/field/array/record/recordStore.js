...
exports.RecordStore = helpers.defineClass(RecordStore, null, {
    ...
    onReorder: function(sourceRecord, targetRecord, $insert){
        var sendBag = this.list.getPage().ensureSendBag(this.list);
        var listDataSet = this.list.ensureDataSet();
        ...
        this.setRecordsIndex();
        if (this.list.builder.isSingleBuilder) {
            sendBag.recordDataset = listDataSet;
        }
        else {
    ...
    notifyDeleteRecord: function(record){
        ...
        var listDataSet = this.list.ensureDataSet();
        if (record) {
            listDataSet.splice(record.$recordIndex, 1);
            this._records.splice(record.$recordIndex, 1);
            if (this.list.builder.isSingleBuilder) {// && !this.list.builder.isReferenceArray) {
                this.list.removeItem(record, true);
                sendBag.recordDataset = listDataSet;
            }
            else {
                record.toggleDeletedStatus(record.$isDeleted = true);
                sendBag.deleteRecord(record);
            }
        }
        else {
            listDataSet.splice(0, listDataSet.length);
            sendBag.recordDataset = [];
            if (this.list.builder.isSingleBuilder) {
                this.removeRecords();
            }
            else {
    ...
    notifySelectRecords: function(selectedRecords){
        if (selectedRecords) {
            var self = this;
            var sendBag = self.list.getPage().ensureSendBag(self.list);
            var listDataSet = self.list.ensureDataSet();
            if (self.list.builder.isSingleBuilder) {
                ...
                Object.keys(selectedRecords).forEach(function($uuid){
                    ...
                    listDataSet.push(options.dataRecord);
                    self.appendRecord(options);
                });
            }
            self.setRecordsIndex();
            self.list._notifyChangeToParent(sendBag.recordDataset);
        }
    },
    notifyCreateRecord: function(record){
        ...
        if (this.list.builder.isSingleBuilder) {
            (sendBag.recordDataset = listDataSet).splice(options.$recordIndex, 0, null);
            ...
            listDataSet.splice(options.$recordIndex, 0, options.dataRecord);
    ...
    notifyRecordChange: function(record, $bind, value){
        if (!record.$isDisabled && !record.$isDeleted) {
            var sendBag = record.getPage().ensureSendBag(this.list);
            ...
            if ($bind == "$actions" && (value.$delete || value.$create)) {
                ...
                if (this.list.builder.isSingleBuilder) {
                    (sendBag.recordDataset = listDataSet)[record.$recordIndex] = value;
                }
                else {
                    sendBag.saveRecordChange(record, $bind, value);
                    listDataSet[record.$recordIndex][$bind] = value;
                }
                this.list._notifyChangeToParent(sendBag.recordDataset);
            }
        }
