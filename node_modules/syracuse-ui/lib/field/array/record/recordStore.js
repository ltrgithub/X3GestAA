...
exports.RecordStore = helpers.defineClass(RecordStore, null, {
        ...
        if (selectedRecords) {
            ...
            if (self.list.builder.isSingleBuilder) {
                ...
                Object.keys(selectedRecords).forEach(function($uuid){
                    ...
                    switch (self.list.$prototype.$item.$type) {
    ...
    doSelectAction: function(){
        ...
        if (self.list.$prototype.$item.$type == "application/x-choice") {
            ...
            self.list.boxParent.openDialog({
                article: self.list,
                $url: self.list.menuItems.$select[0].$sourceUrl,
                onValidate: function(){
    ...
    disableAdd: function(isAddDisabled){
        ...
        if (this.list.menuItems) {
            if (this.list.menuItems.$create) {
                this.list.menuItems.$create[0].disable(this._isAddDisabled);
            }
            if (this.list.menuItems.$select) {
                this.list.menuItems.$select[0].disable(this._isAddDisabled);
            }
        }
    ...
    checkMaxItems: function(){
        if (this.list.$prototype.$maxItems) {
            if (this._isAddDisabled !== (this.list.$prototype.$maxItems <= this.list.dataset.length)) {
