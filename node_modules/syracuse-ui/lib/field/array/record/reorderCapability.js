...
exports.ReorderCapability = helpers.defineClass(ReorderCapability, null, {
    setDraggable: function(){
        ...
        self.list.$$item.delegate(".s-list-record-reorder", "mousedown" + self._eventNamespace, function(event){
            self.start(self.list._store.findRecord($(this)));
            return false;
        }).delegate("[data-s-record]", "mousemove" + self._eventNamespace, function(event){
            if (self.sourceRecord) {
                self.moveDragImage(event, self.list._store.findRecord($(this)));
            }
            return false;
        });
    },
    ...
    start: function(record, $$boundary){
        ...
        self.$$boundary = $$boundary || $(self.list.builder._body);
        ...
        $(document).bind("mouseup" + self._eventNamespace, function(){
            if (self.targetRecord) {
                debugger;
                self.list.getPage().notifyReorderRecords(self.list, self.sourceRecord, self.targetRecord, self.$drag.$insert);
            }
            self.stop();
    ...
    getDropAction: function(event, targetRecord){
        ...
        if (this.sourceRecord && this.sourceRecord != targetRecord) {
            ...
            $(this._info).text(this.list._localize.rr_moveItem);
        }
        return $action;
    },
    _calculateDropCuePosition: function(event){
        var isVertical = false;
        var isVgrid = this.targetRecord.list.$item.$format == "vgrid";
        var $$target = (isVgrid) ? $(event.currentTarget) : this.targetRecord.$$item;
        var offset = $$target.offset();
        this.$drag = {
            left: offset.left,
            top: offset.top,
            width: $$target.outerWidth(),
            height: $(isVgrid ? this.targetRecord.list.builder._body : $$target).outerHeight()
        };
    ...
    moveDragImage: function(event, targetRecord, isOut){
        ...
        if (!isOut && this.$action) {
            ...
            if (isOut) {
                $(this._info).text(this.list._localize.rr_moveItem.replace("{0}", "move record"));
            }
