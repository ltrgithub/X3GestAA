	...
	if (nodes) {
		...
		for (var ii = 0, jj = nodes.length; ii < jj; ii++) {
			...
			if (record) {
				...
				if (tree.list.selector.useColumnSelector) {
				...
				if ($bindings.$icon) {
					...
					if (name) {
						var curNode = record.treeNode;
						if (!curNode.icon) {
							curNode.icon = document.createElement("div");
							curNode.icon.className = tree.cssNodeIcon;
							curNode.item.insertBefore(curNode.icon, curNode.desc);
						}
						if (curNode.iconName != name) {
							curNode.iconName = name;
							curNode.icon.style.backgroundImage = "url('" + syra_site.$item.$iconPath + "tree/" + name + "')";
						}
					} else {
						if (curNode.icon) {
							syra_site.dom.removeChild(curNode.icon);
							delete curNode.icon;
							delete curNode.iconName;
						}
					}
				}
...
function _appendNode(tree, record, slot) {
	var node = record.treeNode = {
		slot: slot,
		item: document.createElement("div")
	};
	var $bindings = tree.list.$prototype.$treeview.$bindings;
	node.item.className = tree.cssNode;
	node.picker = syra_menus.addIconButton("", tree.cssNodePicker + (tree.list.$item.$isNodeLazyLoad ? " s-close" : ""), "onTreeNodeClick");
	node.picker.style.display = tree.list.$item.$isNodeLazyLoad ? "" : "none";
	node.item.appendChild(node.picker);
	if (tree.list.selector.useColumnSelector) {
		node.selectorSlot = document.createElement("div");
		node.selectorSlot.style.display = "none";
		node.selectorSlot.className = tree.cssNodeSelector;
		node.item.appendChild(node.selectorSlot);
	}
	node.desc = document.createElement("div");
	node.desc.className = tree.cssNodeDesc;
	...
	node.descriptionField._dataValue.className += " " + tree.cssNodeDescValue;
...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
	...
	onNodeSelected: function(uuid, selected) {
		...
		if (parent && this.list.$item && this.list.$item.$selectMode === "multi") {
			...
			if (parent.$uuid !== undefined)
				this.list.selector.selectRecord(parent.$uuid, selectParent);
		}
	},
	load: function(list) {
		this.list = list;
	...
	getPrevNextRecord: function(recordIndex, next) {
		var index;
