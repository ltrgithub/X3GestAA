...
function _openNode(tree, record, open) {
	...
	syra_menus.button.setText(record.treeNode.expandBtn, "", node.isOpened ? "node_expanded" : "node_collapsed");
...
function _displayChildren(list, parentNode, isParentOpened, uuids) {
	if (parentNode.childRoot) {
		syra_site.dom.hide(parentNode.childRoot, !isParentOpened);
	}
	for (var ii = 0, jj = uuids.length; ii < jj; ii++) {
		var record = list.recordsMap[uuids[ii]];
		syra_site.dom.hide(record.dataRow, !(record.treeNode.isVisible = isParentOpened));
...
function _drawRecord(tree, parentNode, record, level) {
	...
	if (tree.isCardFormat) {
		if (hasChildren) {
			if (!node.childRoot) {
				...
				syra_site.dom.hide(node.childRoot, true);
			}
		}
	...
	if (tree.list.selector.appendToRecord) {
		if (tree.list.selector.isMulti) {
			if (!record.checkBox) {
				...
				syra_site.dom.hide(record.treeNode.selectorSlot, false);
			}
		} else {
			if (hasChildren) {
				if (record.checkBox) {
					syra_site.dom.removeChild(record.checkBox.slot);
					delete record.checkBox;
					syra_site.dom.hide(record.treeNode.selectorSlot, true);
				}
			} else {
				if (!record.checkBox) {
					tree.list.selector.appendToRecord(record, record.treeNode.selectorSlot);
	...
	if (hasChildren) {
		syra_menus.button.setText(record.treeNode.expandBtn, "", node.isOpened ? "node_expanded" : "node_collapsed");
		if (node.childRoot) {
			syra_site.dom.hide(node.childRoot, !node.isOpened);
		}
	}
	syra_menus.button.hide(record.treeNode.expandBtn, !hasChildren);
	...
	syra_site.dom.hide(record.dataRow, !node.isVisible);
	record.treeNode.slot.style[syra_site.isDocumentRTL ? "paddingRight" : "paddingLeft"] = (record.treeNode.level * 22) + padding + "px";
	if (skinLevel) {
...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
	buildTreeMap: function() {
		...
		if (max && this.list.page.isFusionPage && this.list.page.externalAdapter.isListRecordEmpty(this.list.$prototype.$item.$properties, this.list.records[max - 1].dataset)) {
	...
	addCardNode: function(record, slot) {
		var node = record.treeNode = {
			slot: slot,
		...
		node.expandBtn = syra_menus.button.add({
			parent: record,
			slot: node.item,
		...
		if (this.list.selector.appendToRecord) {
			...
			syra_site.dom.hide(node.selectorSlot, true);
