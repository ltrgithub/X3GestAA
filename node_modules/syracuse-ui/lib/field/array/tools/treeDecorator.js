...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
	...
	onNodeSelected: function(uuid, selected) {
		var parent = this._nodes[this._uuidNodes[uuid].parentId],
			selectParent = selected,
			childCount;
		if (!selectParent) {
			if (parent.children && (childCount = parent.children.length)) {
				for (var ii = 0; ii < childCount; ii++) {
					if (this.list.dataset[parent.children[ii].$uuid].$isSelected) {
						selectParent = true;
						break;
					}
				}
		...
		if (parent.$uuid !== undefined) this.list.selector.select(parent.$uuid, selectParent);
	},
	load: function(list) {
		this.list = list;
