...
function _openNode(tree, record, open) {
	...
	syra_menus.updateButtonIcon(record.treeNode.picker, "", node.isOpened ? "node_expanded" : "node_collapsed");
	if (node.children.length && node.isOpened && !node.childrenLoaded && $bindings.$lazy) {
		for (var ii = 0, jj = node.children.length; ii < jj; ii++) {
			var child = tree.list.recordsMap[node.children[ii]];
			child && _drawRecord(tree, node, child, node.level + 1);
		}
...
function _drawRecord(tree, parentNode, record, level) {
	...
	if (hasChildren && (node.isOpened || !$bindings.$lazy)) {
		for (var ii = 0, jj = node.children.length; ii < jj; ii++) {
			var child = tree.list.recordsMap[node.children[ii]];
...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
	...
	appendCardNode: function(record, slot) {
		...
		node.picker = syra_menus.addIconButton("", this.cssNodePicker, "onTreeNodeClick", null, this.list.$item.$isNodeLazyLoad ? "node_expanded" : "node_collapsed");
