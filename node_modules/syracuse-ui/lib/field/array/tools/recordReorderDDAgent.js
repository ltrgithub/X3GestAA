...
exports.RecordReorderDDAgent = helpers.defineClass(RecordReorderDDAgent, null, {
	start: function(dropItem) {
		...
		this.list.onRecordReorderStart && this.list.onRecordReorderStart(this);
	...
	onDragMouseUp: function(target, event) {
		if (this.targetRecord) {
			...
			if (!(this.list.builder.saveReorder && this.list.builder.saveReorder(this.reorderRecord, this.targetRecord, isAfter))) {
				var dataRecords = syra_dataset.ensure(this.list);
				...
				this.list.builder.record_reorder(this.reorderRecord, this.targetRecord, isAfter);
				this.list.filler.validateDisplay(this.list);
				var sendBag = syra_form.getSendBag(this.list);
				...
				syra_form.updateDelta(this.list, sendBag.dataRecords);
			}
		}
		syra_dom.removeChild(this._info);
		syra_dom.removeChild(this._dropCue);
	},
	...
	moveDragCue: function(event) {
		...
		if (!this._dropCue) {
			this._dropCue = syra_dom.addDiv("s-list-drop-cue", syra_site.layoutSlot);
		}
	...
	_calculateDropCuePosition: function(event) {
		if (this.list.onRecordReorderCalculatePosition) {
			this.list.onRecordReorderCalculatePosition(this);
		} else {
			this.$drag = syra_dom.getBoundingClientRect(this.targetRecord.dataRow || this.targetRecord.domItem);
			if (this.targetRecord.list && this.targetRecord.list.scroller) {
				this.$drag.width = this.targetRecord.list.builder.scrollTable.bodySlot.clientWidth;
			}
		}
	...
	_ensureDragImage: function() {
		if (!this._info) {
			this._info = syra_dom.addTextInfo("s-list-drag-image", "move record", syra_site.layoutSlot);
