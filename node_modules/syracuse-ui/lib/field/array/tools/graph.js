			...
			syra_menus.applyChange(list, {
				$links: {
					$pptslide: {
						...
						$officeAddinSetup: _officeAddin.exe + "?&format=application/x-msi"
					}
				}
			});
		}
...
exports.draw = function(list, load) {
	if (exports.isDefined(list)) {
		if (!list.toggleGraph) {
			...
			list.setGraphValue = function() {
				if (this.chartField && this.records) {
					var fullDataset = this.articleParent.dataset[this.$item.$bind];
					var dataRecords = [];
					if (this.chartField.$useFullData) {
						dataRecords = this.dataset.slice(0, this.dataset.length);
					} else {
						for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
							dataRecords.push(this.records[ii].dataset);
						}
					}
					if (this.page.isFusionPage && dataRecords.length > 0) {
						if (syra_fusion.sapUtil.isEmptyGridDataRecordEx(this.$fields, dataRecords)) {
							dataRecords.splice(dataRecords.length - 1);
						}
					}
					this.chartField.setValue(dataRecords, this.getMetaData(), fullDataset);
				}
			};
