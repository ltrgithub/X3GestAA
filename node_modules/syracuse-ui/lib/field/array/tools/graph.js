...
function _load(list, load) {
	if (!list.chart && (load || list.$item.$graphPosition != "behind")) {
		...
		//_setDataBind(list, dataset, dataset.$properties && dataset.$properties[list.$item.$bind]);
	}
}

function _setDataBind(list, parentDataRecord, metaData) {
	if (list.chart && list.records) {
		var dataRecords = [];
...
function _draw(list) {
	if (list.$field.$decoratorType != "application/x-array-chart-calendar") {
	...
	if (exports.isDefined(list)) {
		if (!list.toggleGraph) {
			...
			list.builder.list_onAfterSetDataBind = function(dataRecordSet, parentDataRecord, metaData) {
				_setDataBind(this.list, parentDataRecord, metaData);
			};
