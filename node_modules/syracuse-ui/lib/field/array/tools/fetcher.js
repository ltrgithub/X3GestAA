...
exports.fetch = function(list, options, $location) {
	...
	if (options) {
		...
		if (options.orderBy !== undefined) {
			params.startIndex = 1;
			list.sortCapability.setOrderBy(options.orderBy);
		}
	...
	params.orderBy = list.sortCapability.stringify();
	var fetchOptions = {
		params: params
	};
		...
		if (options.params.orderBy !== undefined) {
			if (options.params.orderBy === null) {
				...
				if (options.params.startLetter) {
					list.sortCapability.filterByStartLetter({
						binding: $orderBy[0].binding,
						startLetter: options.params.startLetter
					});
				}
			}
			list.sortCapability.sort($orderBy);
		}
	}
};
