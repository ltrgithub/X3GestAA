...
function _drawPageLink(slot, list, value, currentPageIndex, text) {
	var btn = syra_menus.button.add({
		parent: list,
		slot: slot,
		text: text || value || "",
		css: "s-pager-link",
		btnclick: _onPageClick,
		page: value
	});
...
function _drawItemsPerPage(pager, list) {
	if (list.paging.getItemsPerPage()) {
		pager.size = syra_dom.addDiv("s-pager-size");
		...
		pager.itemsPerPageField = list.page.loadNewItem(syra_dom.addDiv("s-pager-size-value", pager.size), {
			$bind: "$itemsPerPage",
			$category: "field",
			$isEditMode: true,
			$inplace: true,
		...
		pager.itemsPerPageField.setDataValue(list.paging.getItemsPerPage());
	}
}
...
exports.addPagerLinksToPage = function(page, $bind) {
	if (!page.pageSteps && _pagerPageFacets.indexOf(page.$facet) >= 0) {
		var slot = syra_dom.addDiv("s-page-pager-slot");
...
function _addPager(list, slot) {
	...
	list.pagerSlot.appendChild(range);
	...
	if (!list.isClientPagingPerPageDisable && list.paging.$totalResults !== undefined) {
		...
		if (pageTotal > 1) {
			var group = syra_dom.addDiv("s-pager-nav");
			var page = 0;
			var lastStepPage = pageTotal - 1;
			_drawPageLink(group, list, ++page, currentPageIndex);
			...
			if (page < lastStepPage) {
				_drawPageLink(group, list, page + 1, currentPageIndex, "...");
			}
			_drawPageLink(group, list, pageTotal, currentPageIndex);
	...
	if (pager) {
		if (clear) {
			if (pager.itemsPerPageField) {
				syra_article.removeItem(pager.itemsPerPageField);
				delete pager.itemsPerPageField;
			}
			if (pager.pageSteps) {
				var $binds = Object.keys(pager.pageSteps);
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					syra_article.removeItem(pager.pageSteps[$binds[ii]], true, true);
				}
...
exports.Paging = helpers.defineClass(Paging, null, {
	getParseIndexes: function() {
		var start = this.$startIndex || 1;
		var last = start + (this.getItemsPerPage() - 1);
		return {
			start: start - 1,
			last: Math.min(last, this.list.filler.ensureClientDataset(this.list).length) - 1
		};
	},
