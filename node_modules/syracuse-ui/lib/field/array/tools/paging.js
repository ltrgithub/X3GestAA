...
var _pagers = {
	$steps: [{
		...
		$shortCut: shortCutsTip["firstRecord"],
		$shortCutList: "SHIFT PAGEUP"
	}, {
		$bind: "$previous",
		$title: "dskpPreviousRecord",
		$shortCut: shortCutsTip["prevRecord"],
		$shortCutList: "PAGEUP"
	}, {
		$bind: "$next",
		$title: "dskpNextRecord",
		$shortCut: shortCutsTip["nextRecord"],
		$shortCutList: "PAGEDOWN"
	}, {
		$bind: "$last",
		$title: "dskpLastRecord",
		$shortCut: shortCutsTip["lastRecord"],
...
function _addPager(list, slot) {
	...
	if (!list.isClientPagingPerPageDisable && list.paging.$totalResults !== undefined) {
		...
		range.style.display = pager.size.style.display = (list.designer || (list.paging.$totalResults > _pageSizes[0])) ? "" : "none";
...
function _disposePager(list, clear) {
	...
	if (pager) {
		if (clear) {
			...
			syra_site.dom.empty(list.pagerSlot);
		}
...
exports.Paging = helpers.defineClass(Paging, null, {
	...
	showPagers: function(designing) {
		...
		if (pager) {
			...
			// this.list.pagerSlot.style.display = (show !== false) ? "" : "none";
