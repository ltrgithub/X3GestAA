	...
	if (list.sortCriterions) {
		for (var ii = 0, jj = list.sortCriterions.length; ii < jj; ii++) {
			...
			if (col && col.sortBtn) {
				...
				syra_site.dom.toggleClass(col.sortBtn.link, " s-sort-desc", false);
				syra_site.dom.toggleClass(col.sortBtn.link, " s-sort-asc", false);
			}
		}
...
exports.onSortClick = function(event, target) {
	...
	col.list.page.externalAdapter.onSortClickEvent({
		...
		event: event,
		doEvent: function() {
			var $field = col.list.$fields[col.$bind];
...
exports.render = function(list) {
		...
		for (var ii = 0, jj = list.$orderBy.length; ii < jj; ii++) {
			...
			if (cols) {
				...
				if (col && col.sortBtn) {
					...
					syra_site.dom.toggleClass(col.sortBtn.link, criterion.descending ? " s-sort-desc" : " s-sort-asc", true);
	...
	if (isSorted && !list.cancelSortBtn) {
		list.cancelSortBtn = syra_menus.button.add({
			parent: list,
			text: syra_local.flUnsort,
			iconOnly: true,
			css: "s-mn-list-btn " + list.$mnPickersCss,
			fontIcon: "unsort",
			btnclick: function() {
		...
		syra_menus.button.display(list.cancelSortBtn, isSorted);
	}
};