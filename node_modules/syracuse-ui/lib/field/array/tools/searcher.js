...
function _onSearchClick(event) {
	this.parent && this.parent.onClick(this.btn.key, event);
}

function Searcher() {}

exports.Searcher = helpers.defineClass(Searcher, null, {
	unload: function() {
		if (this._itemSearchField) {
			this.list.removeItem(this._itemSearchField);
			delete this._itemSearchField;
		}
		syra_site.dom.display(this.list.searcherSlot, false);
	...
	load: function(list) {
		...
		if (syra_local.flSearch) {
			...
			this.list.searcherSlot.appendChild(label).textContent = syra_local.flSearch + ":";
		}

		syra_menus.button.add({
			parent: syra_site,
			slot: this.list.searcherSlot,
			text: "",
			iconOnly: true,
			css: "s-list-search-picker",
			fontIcon: "search",
			btnclick: _onSearchClick,
			key: "$searchCollapse"
		});

		this._fieldSlot = document.createElement("div");
		this._fieldSlot.className = "s-list-search-value";
		this._fieldSlot.style.display = "none";

		this._itemSearchField = this.list.page.loadNewItem(this.list.searcherSlot.appendChild(this._fieldSlot), {
			$bind: "$search",
		...
		this.list.searchNext = syra_menus.button.add({
			parent: syra_site,
			slot: this._itemSearchField._dataValue,
			text: "Next",
			iconOnly: true,
			css: "s-list-search-goto",
			fontIcon: "expand_row",
			btnclick: _onSearchClick,
			key: "$searchNext"
		});
		...
		syra_menus.button.add({
			...
			iconOnly: true,
			css: "s-list-search-goto",
			fontIcon: "collapse_row",
			btnclick: _onSearchClick,
			key: "$searchPrevious"
		});
		syra_site.dom.display(this.list.searcherSlot, this.list.$item.$graphPosition != "front");
	},
	onClick: function($bind, event) {
		switch ($bind) {
			case "$searchCollapse":
				if (this._fieldSlot) {
					if (this._fieldSlot.style.display == "none") {
						this._fieldSlot.style.display = "";
			...
			case "$searchNext":
			case "$searchPrevious":
				var found, value = this._itemSearchField.getInputValue();
				if (value === "") {
					return false;
				}
				this._ensureOrderCols();
				if (found = _run(this, value, ($bind == "$searchNext"))) {
					this.list.page.externalAdapter.onSearchClickEvent({
						field: this.list,
						found: found,
						event: event,
						doEvent: function() {
	...
	incrementalSearch: function(event, incrementalSearchValue) {
	...
	setStart: function($uuid, $bind, $serverIndex) {
		this.start = {
			$uuid: $uuid,
			$bind: $bind,
