	...
	if (!list.disableDisplayValidation) {
		...
		if (list.builder.emptyDataItem) {
			...
			if (list.showEmptyItem) {
				...
				syra_site.dom.hide(list.builder.emptyDataItem, count);
			}
		}
		...
		for (var ii = 0; ii < count; ii++) {
			...
			if (list.$item.$alternateStyle) {
				...
				if (record.showAlt != showAlt) {
					...
					syra_site.dom.toggleClass(record.domItem, "s-list-alt", showAlt);
					record.fixedRow && syra_site.dom.toggleClass(record.fixedRow, "s-list-alt", showAlt);
				}
			}
