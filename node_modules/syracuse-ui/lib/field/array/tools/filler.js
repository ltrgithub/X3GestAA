...
exports.applyClientFetchOptions = function(list, options, pagingChange) {
	...
	if (options && options.params && !pagingChange) {
		if (options.params.where) {
			...
			list.clientDataset = [];
			var compare = syra_dataset.compare;
			for (var ii = 0, jj = currentDataset.length; ii < jj; ii++) {
				...
				if (ff != null) {
					...
					if (typeof(ff) == "object") {
						...
						ok = compare.string(ff.$title, exp.children[1].value, exp.value.code);
					} else {
						if (typeof(exp.children[1].value) == "string") {
							ok = compare.string(ff, exp.children[1].value, exp.value.code);
						} else {
							// filter date
							if (typeof(exp.children[1].value) == "object" && list.$fields[exp.children[0].value] && list.$fields[exp.children[0].value].$type == "application/x-date") {
								ok = compare.date(ff, exp.children[1].value, exp.value.code);
							} else {
								// case of integer (application/x-choice)
								if (typeof(ff) == "number") {
									ok = compare.number(ff, exp.children[1].value, exp.value.code);
								} else {
									ok = false;
								}
							}
