...
var _clientFilter = require('syracuse-ui/lib/field/filter/clientFilter');
...
exports.notifyClientSave = function(list, savedDelta) {
	if (savedDelta && savedDelta.$uuid) {
		...
		if (dataRecord) {
			syra_delta.applyObjectDelta(list.page, dataRecord, delta);
			...
			delta = [delta];
			syra_delta.applyPageArrayDelta(list.page, dataset, delta);
...
exports.trigger = function(list, options, $location) {
	if (!list.disposed && !list.isReloading) {
		...
		if (options.orderBy !== undefined) {
			params.startIndex = 1;
			list.sorter.setOrderBy(list, options.orderBy);
		}
		...
		if (list.isClientFetch || list.isClientSortEnable) {
			...
			list.page.fetch(fetchOptions, function(data, response, requestUrl) {
				if (list.page.isMainPage) {
					syra_url.history.update(list.page, requestUrl);
				} else {
					list.page.urlSeg = syra_url.parse(requestUrl);
				}
...
exports.applyClientFetchOptions = function(list, options, pagingChange) {
	...
	if (options && options.params && !pagingChange) {
		...
		if (options.params.orderBy !== undefined) {
			list.sorter.applyClientFetchOptions(list, options.params);
		}
	}
};
	...
	if (!list.disableDisplayValidation) {
		...
		for (var ii = 0; ii < count; ii++) {
			...
			if (list.$item.$alternateStyle) {
				showAlt = !showAlt;
				if (record.showAlt != showAlt) {
					record.showAlt = showAlt;
					syra_dom.toggleClass(record.domItem, "s-list-alt", showAlt);
					record.fixedRow && syra_dom.toggleClass(record.fixedRow, "s-list-alt", showAlt);
				}
			}
...
exports.addRecord = function(list, options) {
	...
	record.list = list;
	record.$prototype = options.$prototype || _helpers.object.clone(list.$prototype.$item, true);
...
function _addEmptyItem(list, onFetch) {
	var empty = list.builder.emptyDataItem = syra_dom.addDiv(list.$skin + "-empty-slot");
...
function _notifySelectRecords(list, menuItem) {
	if (list.currentSelectRecords) {
		var sendBag = syra_workingCopy.getSendBag(list);
...
exports.doSelectAction = function(list, menuItem) {
	...
	syra_dlg.openModal(list.boxParent, {
		article: list,
...
exports.getField = function(list, name, $serverIndex, select) {
	var fields, record = syra_store.findRecordByServerIndex(list, $serverIndex);
...
exports.setFocus = function(list, select, name, $serverIndex) {
	if (list.treeDecorator) {
		list.selector.selectRecord(syra_store.findRecordByServerIndex(list, $serverIndex), true);
		return true;
	}
	if (list.searcher) {
	...
	if (!field) {
		if (list.isClientFetch && !syra_store.findRecordByServerIndex(list, $serverIndex)) {
