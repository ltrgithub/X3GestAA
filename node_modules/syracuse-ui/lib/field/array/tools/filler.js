...
exports.applyClientFetchOptions = function(list, options, pagingChange) {
	...
	if (options && options.params && !pagingChange) {
		...
		if (options.params.orderBy !== undefined) {
			list.capabilityMaker.sorter.applyClientFetchOptions(list, options.params);
		}
	}
};
...
exports.fill = function(list, dataRecordSet, parentDataRecord, isDelta) {
	if (isDelta) {
		...
		if (list.clientFetchOptions) {
			...
			_applyFullDelta(list, list.clientDataset, list.pagging.getParseIndexes(), updatedMap);
		} else {
			list.clientDataset = list.ensureDataSet();
			if (partialDelta) {
				_applyPartialDelta(list, dataRecordSet, list.pagging.getParseIndexes());
			} else {
				_applyFullDelta(list, dataRecordSet, list.pagging.getParseIndexes());
			}
		}
	} else {
		...
		if (list.clientDataset) {
			...
			if (list.isClientFetch) {
				...
				range = list.pagging.getParseIndexes();
			} else {
				range = {
					start: 0,
