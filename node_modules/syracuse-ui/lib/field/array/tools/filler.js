	...
	if (!list.disableDisplayValidation) {
		...
		for (var ii = 0, jj = list.records.length; ii < jj; ii++) {
			...
			if (record.expandRowCard) {
				list.builder.cardDecorator.toggleRowCard(record, true);
			}
		}
		//_checkMaxItems
		if (list.$isEditMode) {
...
exports.addRecord = function(list, options) {
	...
	record.applyChange(list.metaHelper.ensureGlobalMetaRecord(list));
...
exports.setFocus = function(list, select, name, $serverIndex) {
	...
	if (list.treeDecorator) {
		var record = exports.findRecordByServerIndex(list, $serverIndex);
		record && list.selector.selectRecord(record.$uuid, true);
		return true;
	}
...
exports.setFocusRecord = function(list, record) {
	list.page.setShortcutSpyerList(list);
	if (list.focusedRecord && list.focusedRecord != record) {
		syra_site.dom.toggleClass(list.focusedRecord.domItem, "s-record-focused", false);
		list.focusedRecord.freezeRow && syra_site.dom.toggleClass(list.focusedRecord.freezeRow, "s-record-focused", false);
	}
	list.focusedRecord = record;
