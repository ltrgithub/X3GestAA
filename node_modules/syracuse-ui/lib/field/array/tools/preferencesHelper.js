...
exports.onInitialize = function(list, bindMap) {
	...
	if (list.$userPreferences) {
		...
		if (list.$userPreferences.$colWidths) {
			var $items = list.$item.$layout.$items;
			for (var ii = 0, jj = $items.length; ii < jj; ii++) {
				var $width = list.$userPreferences.$colWidths[$items[ii].$bind];
				if ($width) {
...
exports.save = function(list, key) {
	...
	switch (key) {
		case "$itemsPerPage":
			...
			break;
		case "$colWidths":
			list.$userPreferences.$colWidths = {};
			var $items = list.$item.$layout.$items;
			for (var ii = 0, jj = $items.length; ii < jj; ii++) {
				var $item = $items[ii];
				if ($item.$width !== undefined) {
					list.$userPreferences.$colWidths[$item.$bind] = $item.$width;
				}
			}
			break;
		case "$columns":
