...
function _ensureGraphVisibility(list) {
	if (!list.$field.$isListHidden) {
		if (list.$item.$graphPosition == "front" || list.$item.$graphPosition == "behind") {
			...
			if (!list.toggleGraphPicker) {
				list.toggleGraphPicker = syra_menus.addIconButton(title, list.$mnPickersCss, "onToggleGraphClick", null, icon);
				list.toggleGraphPicker.className += " s-mn-list-btn";
				list.topbar.insertBefore(list.toggleGraphPicker, list.searcherSlot);
				list.onToggleGraphClick = function() {
					this.toggleGraph();
				};
			} else {
				syra_menus.updateButtonIcon(list.toggleGraphPicker, title, icon);
				list.toggleGraphPicker.className += " s-mn-list-btn";
				syra_site.dom.display(list.toggleGraphPicker, true);
			}
		} else {
			syra_site.dom.display(list.toggleGraphPicker, false);
		}
	}
	...
	list.builder.tableSlot.style.display = list.isTableSlotHidden ? "none" : "";
	list.graphSlot.style.display = list.$item.$graphPosition == "behind" ? "none" : "";
}
