	...
	if (!list.$field.$isListHidden) {
		if (list.$item.$graphPosition == "front" || list.$item.$graphPosition == "behind") {
			...
			if (!list.toggleGraphBtn) {
				...
				list.toggleGraphBtn.className += " s-mn-list-btn";
				list.topbar.insertBefore(list.toggleGraphBtn.link, list.searcherSlot);
			} else {
				syra_menus.updateButtonIcon(list.toggleGraphBtn.link, title, icon);
				list.toggleGraphBtn.link.className += " s-mn-list-btn";
				syra_menus.button.display(list.toggleGraphPicker, true);
			}
		} else {
			syra_menus.button.display(list.toggleGraphBtn, false);
		}
	}
