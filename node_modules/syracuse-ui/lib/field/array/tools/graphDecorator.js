...
function _addPowerPointLink(list, $links) {
	if (!list.page.isFusionPage) {
		if (!(list.$field.$decorator && list.$field.$decorator.$stereotype == "calendar")) {
...
exports.GraphDecorator = helpers.defineClass(GraphDecorator, null, {
	...
	ensureGraphVisibility: function() {
		var $links;
		if (!this.list.$field.$isListHidden) {
			if (this.list.$item.$graphPosition == "front" || this.list.$item.$graphPosition == "behind") {
				...
				$links = {
					$toggleGraph: {
						$isHidden: false,
						$title: showGraph ? syra_local.flSwitchToList : syra_local.flSwitchToGraph,
						$iconValue: showGraph ? "field_array" : "field_graph"
					}
				};
			} else {
				$links = {
		...
		this.ensureGraph();
		$links = _addPowerPointLink(this.list, $links);
		if ($links) {
			syra_menus.setMenus(this.list, {
				$links: $links
			});
		}
		this.list.isTableSlotHidden = this.list.$field.$isListHidden || (this.list.$item.$graphPosition == "front" && !(this.list.gridCardBuilder && this.list.gridCardBuilder.isOutCardMode));
		this.list.builder.tableSlot.style.display = this.list.isTableSlotHidden ? "none" : "";
	...
	list_onAfterSetDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		this.setDataBind(graphRecords, parentDataRecord, metaData);
	},
	applyDesignMetaData: function(metaData, designing) {
		...
		designing && this.list.appendCardGraphSlots();

		this.ensureGraphVisibility();
		this.list.chart && this.list.chart.applyDesignMetaData(metaData);
	...
	appendSlot: function() {
		...
		switch (this.list.$item.$graphPosition) {
			case "left":
				...
				if (this.outCardSlot && this.list.$item.$graphWidth) {
			...
			case "top":
				...
				delete this.separator.syraDragSpot;
