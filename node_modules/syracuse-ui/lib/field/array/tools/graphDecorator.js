		...
		syra_menus.menus.applyChange(list, {
			$links: {
				$pptslide: {
					...
					$officeAddinSetup: officeHelper.officeAddinSetupInstallation + "?&format=application/x-msi"
				}
			}
		});
	}
...
function _ensureGraphVisibility(list) {
	...
	syra_dom.hide(list.builder.tableSlot, list.$field.$isListHidden || (list.$item.$graphPosition == "front" && !list.builder.isOutCardMode));
...
exports.ensureGraph = function(list, load) {
	if (!list.chart && (load || list.$item.$graphPosition != "behind")) {
		...
		if (list.chart.isFlashField) {
			syra_site.registerFlashWidget(list);
		}
