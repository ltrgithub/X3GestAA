...
function _applyMetaDataToRecord(record, metaData) {
	...
	if (metaData.$links || metaData.$actions) {
		...
		syra_menus.menus.applyChange(record, metaData);
...
exports.SingleArrayBuilder = helpers.defineClass(SingleArrayBuilder, null, {
	...
	record_draw: function(record) {
		...
		record.domItem = syra_dom.addDiv(record.$skin = this.list.$skin + "-item");
		record.domItem.syrainout = record.domItem.syraItem = record.id;

		record.reorderBtnParent = record.recordValue = syra_dom.addDiv(this.list.$skin + "-item-value", record.domItem);

		var dataSlot = syra_dom.addDiv(this.list.$skin + "-item-data", record.recordValue);
