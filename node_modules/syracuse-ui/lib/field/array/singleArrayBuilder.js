...
function _applyMetaDataToRecord(record, metaData) {
	...
	if (metaData.$links || metaData.$actions) {
		...
		syra_menus.setMenus(record, metaData);
		delete metaData.$links;
		delete metaData.$actions;
	}
	record.singleField.applyMetaData(metaData);
	if (metaData.$diagnoses && record.singleField.diagnoseFieldSlot) {
		record.singleField.diagnoseFieldSlot.className += (record.list.$skin + "-item-diagnoses");
		record.domItem.appendChild(record.singleField.diagnoseFieldSlot);
	}
...
exports.SingleArrayBuilder = helpers.defineClass(SingleArrayBuilder, null, {
	...
	record_draw: function(record) {
		...
		record.reorderBtnParent = record.recordValue = document.createElement("div");
		...
		record.recordValue.appendChild(dataSlot);
		...
		record.singleField = record.page.loadNewItem(dataSlot, {
