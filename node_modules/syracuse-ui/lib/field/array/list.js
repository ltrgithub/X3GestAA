...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	fetch: function(options, $location) {
		...
		if (this.isClientFetch || this.isClientSortEnable) {
			this.fetchClientRecordSet(fetchOptions);
		} else {
	...
	fetchClientRecordSet: function(options) {
		if (options && options.params) {
			...
			this.fillList(resource.dataRecordSet, this.articleParent.dataset);
	...
	reloadBuilder: function(fillData) {
		...
		this.builder.drawBuilder();
		if (fillData) {
			this.fillList(this.ensureDataSet(), this.articleParent.dataset);
		}
	},
	...
	applyPartialDelta: function(dataRecordSet, firstIndex, lastIndex) {
		...
		for (var $serverIndex = 0, jj = dataRecordSet.length; $serverIndex < jj; $serverIndex++) {
			...
			found = null;
			foundIndex = 0;
			for (var mm = this.records.length; foundIndex < mm; foundIndex++) {
				found = this.records[foundIndex];
			...
			if (found) {
				if (dataSourceRecord.$isDeleted) {
					...
					delete this.recordsMap[found.$uuid];
					...
					if (dataSourceRecord.$index !== undefined && found.$serverIndex != dataSourceRecord.$index) {
						isReorder = this.isClientFetchResult;
						//TODO if this.isClientFetchResult trgger client resort  
					...
					if (isReorder) {
						this.records.splice(foundIndex, 1);
						this.records.splice(recordIndex, 0, found);
						var nextRecord;
				...
				if (!dataSourceRecord.$isDeleted) {
					...
					if (index != -1) {
						this._appendRecord({
							dataRecord: this.dataset[dataSourceRecord.$index],
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if (dataRecordSet !== undefined) {
			if (this.page.$isPartialDelta || (this.page.$isEditMode && this.hasRecords() && !this.builder.isSingleBuilder)) {
				...
				this.fillList(dataRecordSet, parentDataRecord);
			}
		}
		if (this.graphDecorator) {
			var graphRecords = [];
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		if (this.isClientFetch && this.pagging) {
			...
			delete this.pagging.clientTotalRecords;
		}
		...
		if (isDelta) {
			...
			if (this.page.$isPartialDelta || (dataRecordSet.length == 1 && dataRecordSet[0].$index !== undefined)) {
				...
				this.isClientFetchResult = this.onClientFetch;
			...
			if (lastIndex) {
				...
				if (this.isClientFetch && (this.dataset.length <= firstIndex || (this.records.length < this.pagging.$itemsPerPage && this.records.length < (this.dataset.length - firstIndex)))) {
					this.pagging.setPage(0);
				}
			}
		} else {
			if (this.treeDecorator) {
			...
			this.removeRecords(!(dataRecordSet && dataRecordSet.length > 0));
			var dataSource = this.ensureDataSet();
			this.isClientFetchResult = this.onClientFetch;
			if (dataRecordSet) {
				var $startIndex = 1;
				var $itemsPerPage = this.pagging.$itemsPerPage || dataRecordSet.length;
				...
				if (this.isClientFetch && this.pagging) {
					...
					if (this.onClientFetch) {
						this.pagging.clientTotalRecords = dataRecordSet.length;
						$startIndex = this.pagging.$startIndex;
						$itemsPerPage = this.pagging.$itemsPerPage;
					} else {
						if (dataRecordSet.length > this.pagging.$itemsPerPage) {
	...
	dispose: function() {
		...
		this.awAddin = this.emptyDataItem = this.reorderCapability = null;
		this.topbar = this.bottombar = null;
		this._menus = this.page = this.treeDecorator = null;
