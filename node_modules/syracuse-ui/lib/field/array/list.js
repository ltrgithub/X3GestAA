...
//http://demos.devexpress.com/aspxgridviewdemos/Accessibility/KeyboardSupport.aspx
//http://demos.devexpress.com/ASPxTreeListDemos/Accessibility/KeyboardSupport.aspx
...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	applyShortCut: function(shortcuts, event) {
		return shortCutInvoker.apply(this, shortcuts, event);
	},
	...
	drawBox: function() {
		...
		this.fieldValue.className = this.$item.$skin ? (this.$item.$skin + "-list s-list") : "s-list";
	...
	dispose: function() {
		...
		this.capabilityMaker = this.fetcher = this.tableSlot = this.tableRow = this._core = null;
	...
	highlightSelection: function(record, selected) {
		...
		var lastFocus = this.getFocusedRecord(false);
		lastFocus && lastFocus.toggleFocus(false);
		...
		if (cardDecorator) {
			...
			if (selected) {
				this.popupCardBuilder && this.popupCardBuilder.onSelectRecord(record, true);
	...
	selectHTMLContent: function() {
		...
		this.headTable && range.selectNode(this.headTable);
