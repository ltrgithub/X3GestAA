...
var PopupCardBuilder = require('./tools/popupCardBuilder').PopupCardBuilder;
...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	onClickPicker: function(picker, event) {
		switch (picker.getAttribute("data-s-picker")) {
			...
			case "list-alphatab":
				...
				break;
			case "grid-inlinecard":
				this.builder.onInlineCardOpenerClick(picker);
				break;
			case "list-page":
	...
	appendCardGraphSlots: function() {
		...
		if (!$cardItem) {
			document.site.removeDomChild(this.cardItemSlotSeparator);
			document.site.removeDomChild(this.cardItemSlot);
			this.cardItemSlotSeparator = this.cardItemSlot = null;
		}
		if (!this.$item.$graphPosition) {
			document.site.removeDomChild(this.graphSeparator);
			document.site.removeDomChild(this.graphSlot);
		...
		if ($cardItem || this.$item.$graphPosition) {
			var hasRow;
			if ($cardItem) {
				if (!this.cardItemSlotSeparator) {
					this.cardItemSlotSeparator = document.createElement("div");
				}
				if (!this.cardItemSlot) {
					this.cardItemSlot = document.createElement("div");
				}
				hasRow = $cardItem.$position == "left" || $cardItem.$position == "right";
			}
			if (this.$item.$graphPosition) {
				if (!this.graphSeparator) {
					this.graphSeparator = document.createElement("div");
				}
				if (!this.graphSlot) {
					this.graphSlot = document.createElement("div");
				}
				if (!hasRow) {
					hasRow = this.$item.$graphPosition == "left" || this.$item.$graphPosition == "right";
				}
			}
			if (hasRow) {
				if (!this.tableRow) {
			...
			if ($cardItem) {
				var cssCell = "";
				switch ($cardItem.$position) {
					case "left":
						this.tableSlot.parentNode.insertBefore(this.cardItemSlotSeparator, this.tableSlot);
						this.tableSlot.parentNode.insertBefore(this.cardItemSlot, this.cardItemSlotSeparator);
						cssCell = "-cell";
						break;
					case "right":
						this.tableSlot.parentNode.insertBefore(this.cardItemSlotSeparator, this.tableSlot.nextSibling);
						this.tableSlot.parentNode.appendChild(this.cardItemSlot, this.cardItemSlotSeparator.nextSibling);
						cssCell = "-cell";
						break;
					case "top":
						var sibling = this.tableRow || this.tableSlot;
						sibling.parentNode.insertBefore(this.cardItemSlotSeparator, this.tableSlot);
						sibling.parentNode.insertBefore(this.cardItemSlot, this.cardItemSlotSeparator);
						this.cardItemSlot.style.width = "";
						break;
					case "bottom":
						var sibling = this.tableRow || this.tableSlot;
						sibling.parentNode.appendChild(this.cardItemSlotSeparator);
						sibling.parentNode.appendChild(this.cardItemSlot);
						this.cardItemSlot.style.width = "";
						break;
				}
				this.cardItemSlotSeparator.className = "s-list-card-sep" + cssCell;
				this.cardItemSlot.className = "s-list-card-slot" + cssCell;
				if ($cardItem.$widths) {
					this.cardItemSlot.style.width = $cardItem.$widths + "%";
				}
				this.tableSlot.style.display = "";
			}
			switch (this.$item.$graphPosition) {
				case "left":
					this.tableSlot.parentNode.insertBefore(this.graphSeparator, this.tableSlot);
					this.tableSlot.parentNode.insertBefore(this.graphSlot, this.graphSeparator);
					this.graphSeparator.className = "s-list-graph-sep-cell";
					this.graphSlot.className = "s-list-graph-slot-cell";
					break;
				case "right":
					this.tableSlot.parentNode.insertBefore(this.graphSeparator, this.tableSlot.nextSibling);
					this.tableSlot.parentNode.appendChild(this.graphSlot, this.graphSeparator.nextSibling);
					this.graphSeparator.className = "s-list-graph-sep-cell";
					this.graphSlot.className = "s-list-graph-slot-cell";
					break;
				case "top":
					var sibling = this.tableRow || this.tableSlot;
					sibling.parentNode.insertBefore(this.graphSeparator, sibling);
					sibling.parentNode.insertBefore(this.graphSlot, this.graphSeparator);
					this.graphSeparator.className = "s-list-graph-sep";
				...
				case "bottom":
					var sibling = this.tableRow || this.tableSlot;
					sibling.parentNode.appendChild(this.graphSeparator);
	...
	reloadLayout: function($item) {
		...
		this.tableSlot = this.cardItemSlotSeparator = this.cardItemSlot = this.graphSeparator = this.graphSlot = this.tableRow = null;
		this.createBuilder();
		this.builder.drawBuilder();
	...
	_appendRecord: function(options) {
		...
		record.loadBox();
		if (options.isCreate && this.$item.$cardItem && this.$item.$cardItem.$position == "inline") {
			record.toggleInlineCard(true);
		}
	...
	applyFullDelta: function(dataRecordSet) {
		...
		//var dataIndex = this.pagging.getParseIndexes(dataRecordSet);
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		...
		if (isDelta) {
			...
			if (this.clientFetchOptions) {
				...
				this.applyFullDelta(this.clientDataset);
			} else {
				this.clientDataset = this.ensureDataSet();
		...
		this._validateDisplay();
		if (this.$item.$cardItem && this.$item.$cardItem.$position != "inline") {
	...
	dispose: function() {
		...
		this.$designing = this.popupCardBuilder = this.emptyDataItem = this.reorderCapability = null;
