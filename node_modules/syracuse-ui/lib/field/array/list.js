...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    fitToWidth: function(){
        ...
        var s = first.menuBoxes[0].$$item[0].clientWidth;
        var currentSize = s;
        var item = Object.keys(first.menuItems);
        for (var j = 0; j < item.length; j++) {
            s = first.menuItems[item[j]][0].$$item[0].clientWidth;
            currentSize += s;
        }
		...
		for(var i=0;i<this.builder.orderCols.length;i++){
			var col = this.builder.orderCols[i];
			if( col.$bind && this.builder.cols[col.$bind] ){
				col.widthValue = this.builder.cols[col.$bind].widthValue
			}
