...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
                ...
                if (this.page.$prototype.$filters) {
                    if (this.page.menuBar) {
                        this._filtersMenus = this.page.loadNewItem(this.page.menuBar._body, {
                            ...
                            $title: this._localize.l_filters
                        }, this);
                        this._filtersMenus._ensurePrivateMenus(this.page.$prototype, "$filters");
                    }
                }
                break;
            case "$bulk":
    ...
    _createMenusBox: function(){
        this.menusBox = this.page.loadNewItem(this._menusSlot, {
            ...
            $category: "menus",
			$noText: true,
            $title: this._localize.l_actions,
            $isTitlePicker: true,
	...
	function getWidthOfInput(inputEl) {
		...
		var value = typeof inputEl.currentValue === 'object' ? inputEl.currentValue.$value+" "+inputEl.currentValue.$title : (typeof inputEl.currentValue === 'string' ? inputEl.currentValue: "");
		tmp.innerHTML = value.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
		document.body.appendChild(tmp);
		var theWidth = tmp.scrollWidth+20;
		document.body.removeChild(tmp);
		return (theWidth < 20 ? 20: theWidth)+(inputEl.boxPickers ? inputEl.boxPickers.clientWidth:0)+(inputEl.menusBox ? inputEl.menusBox.$$item[0].clientWidth:0);
	}
	var colSize = {};
	...
	for(var i=0;i<colKey.length;i++){
		if( this.builder.cols[colKey[i]] && colSize[colKey[i]]!==-1 ) {
			this.builder.cols[colKey[i]].groupHead.style.width = colSize[colKey[i]]+"px";
			this.builder.cols[colKey[i]].groupBody.style.width = colSize[colKey[i]]+"px";
			this.builder.cols[colKey[i]].widthValue = colSize[colKey[i]];
		}
	}
    },
    ...
    dispose: function(){
        ...
        this.clientDataset = this.emptyDataItem = this.helper = this.reorderCapability = null;
