...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	applyFullDelta: function(dataRecordSet, firstIndex, lastIndex) {
		...
		for (var ii = firstIndex; ii <= lastIndex; ii++) {
			...
			var sourceDataset = this.dataset[dataRecord.$serverIndex || dataRecord.$index || ii];
			...
			if (record) {
				...
				if (!(firstIndex !== undefined && (ii < firstIndex || (lastIndex != undefined && ii >= lastIndex)))) {
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		...
		if (isDelta) {
			...
			if (this.clientFetchOptions) {
				...
				if (firstIndex !== undefined) {
					if (this.clientDataset.length <= firstIndex) {
						var options = this.pagging.goToClientPage();
						lastIndex = firstIndex + this.pagging.$itemsPerPage;
						this.clientFetchOptions.params.startIndex = firstIndex;
					}
				}
				if (this.treeDecorator) {
				...
				if (lastIndex !== undefined) {
					lastIndex = Math.min(lastIndex, (dataRecordSet.length - 1) - firstIndex);
				} else {
					lastIndex = dataRecordSet.length - 1;
				}
				this.applyFullDelta(this.clientDataset, firstIndex, lastIndex);
			} else {
				if (partialDelta) {
					if (this.treeDecorator) {
