...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	notifyClientSave: function(savedDelta) {
		if (savedDelta && savedDelta.$uuid) {
			...
			if (dataRecord) {
				this.page.ensureDeltaManager().applyObjectDelta(dataRecord, delta);
				...
				delta = [delta];
				this.page.ensureDeltaManager().applyPageArrayDelta(dataset, delta);
	...
	designItem: function(open) {
		...
		if (!open) {
			if (this.$designing) {
				this.$designing = this.$designing.cards = this.$designing.grid = null;
			}
		}
	...
	toggleUIDesign: function(enable, disposingDesigner) {
		if (enable) {
			...
			if (this.designPicker && this.designer) {
	...
	onClickPicker: function(picker, event) {
		switch (picker.getAttribute("data-s-picker")) {
			...
			case "list-alphatab":
				...
				break;
			case "grid-rowcard":
				this.builder.cardDecorator.onRowCardOpenerClick(picker);
				break;
			case "list-page":
	...
	loadBox: function(initData) {
		...
		if (document.site.enableTestRobot) {
			Field.prototype.appendQualityAtt.call(this, this.domItem);
		}
	...
	appendCardGraphSlots: function() {
		...
		if (!$cardItem && this.builder.cardDecorator) {
			this.builder.cardDecorator.removeOutCardSlot();
		}
		...
		if ($cardItem || this.$item.$graphPosition) {
			...
			if ($cardItem) {
				this.builder.cardDecorator.appendOutCardSlot();
			}
			if (this.$item.$graphPosition && this.graphDecorator) {
	...
	appendMenusslot: function() {
		...
		this.menusSlot.className = "s-list-menus-cell";
	...
	fetch: function(options, $location) {
		...
		if (options) {
			...
			params.count = this.pagging.getItemsPerPage(options.$itemsPerPage);
		...
		if (this.isClientFetch || this.isClientSortEnable) {
			...
			if (params.where !== undefined) {
				this.page.$prototype.$where = encodeURIComponent(params.where);
			}
			if (params.filter !== undefined) {
				this.page.$prototype.$filter = params.filter;
			}
		...
		if (operator == 'like' && type == 'string') {
			...
			// "starts with" operation
				...
				if (/(([^%]|\%)*)%$/g.test(val2)) {
					...
					throw new Error(this.localize.flFilter_err_wrong_value.replace('{operator}', operator).replace('{value}', val2));
				}
			}
		} else {
			if (operator == 'like' && type !== 'string') {
				throw new Error(this.localize.flFilter_err_wrong_type.replace('{operator}', operator).replace('{type}', type));
			} else {
				switch (operator) {
					case ">":
	...
	reloadLayout: function($item) {
		var $prevItemsPerPage;
		if ($item != undefined) {
			$prevItemsPerPage = this.$item.$itemsPerPage;
			this.$item = $item;
		}
		...
		this.tableSlot = this.tableRow = null;
		this.createBuilder();
		this.builder.drawBuilder();
		if (($prevItemsPerPage !== undefined) && $prevItemsPerPage != this.$item.$itemsPerPage) {
			this.pagging.applyMetadata(this.$item, true);
		} else {
			this.fillList(this.clientDataset, this.articleParent.dataset);
		}
	},
	...
	onFieldDiagnoseChange: function(add, record) {
		...
		if (add) { // list diag count
			...
			if (!this.diagAlert) {
				this.diagAlert = document.createElement("span");
				this.diagAlert.className = "s-grid-diag-count";
				if (this.domTitle) {
					this.domTitle.appendChild(this.diagAlert);
				}
			}
	...
	notifyActionChange: function(menuItem, target, value, notifyServer) {
		...
		if (value.$create) {
			...
			if (this.builder.isSingleBuilder) {
				...
				(sendBag.dataRecords = dataRecords).splice(options.$recordIndex, 0, options.dataRecord);
				this._appendRecord(options);
				this.validateDisplay();
	...
	emptyBody: function(addEmptySlot, onFetch) {
		if (this.builder.onEmptyBody) {
			if (this.builder.onEmptyBody(addEmptySlot, onFetch) == false) {
				return;
			}
		}
		document.site.emptyDom(this.builder.body);
		if (addEmptySlot) {
			var empty = document.createElement("div");
			empty.className = this.$skin + "-empty-slot";
			empty.textContent = (!this.isClientFetch && onFetch) ? this.localize.flWaitingData : (this.emptyMessage || this.$item.$noDataText || this.localize.flNoData);
			this.builder.body.appendChild(this.emptyDataItem = empty);
		}
	},
	...
	parseCapabilities: function($capabilities, $variantItemKey) {
		if ($capabilities) {
			var parts = $capabilities.split(",");
			for (var ii = 0, jj = parts.length; ii < jj; ii++) {
				var part = parts[ii];
				if (part) {
					this.$capability[part] = this.$capability[part] || {};
					if ($variantItemKey) {
						(this.$capability[part].$variantItemKeys = this.$capability[part].$variantItemKeys || []).push($variantItemKey);
					}
				}
			}
		}
	...
	addCapabilityAction: function($actions, name) {
		...
		var $newAction = $actions[action.$bind] = {
			$title: this.localize[action.$title],
			$confirm: this.localize[action.$confirm],
			$isHidden: false
		};
	...
	applyCapabilities: function($metaCapablities) {
		...
		if ($metaCapablities) {
			...
			this.parseCapabilities(this.$prototype.$capabilities);
			if (this.$field.$item && this.$field.$item.$variants) {
				var $keys = Object.keys(this.$field.$item.$variants);
				for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
					this.parseCapabilities(this.$field.$item.$variants[$keys[ii]].$capabilities, $keys[ii]);
				}
		...
		//this.$capability.reorder = true; //to virer 
	...
	applyDesignMetaData: function(metaData, designing) {
		if (designing) {
			if (metaData.$format !== undefined || metaData.$cardPosition !== undefined) {
				this.$designing = this.$designing || {};
				if (this.$item.$format == "grid") {
					this.$designing.grid = this.$designing.grid || {};
					this.$designing.grid.$layout = helpers.object.clone(this.$item.$layout, true);
					if (this.$item.$cardItem) {
						this.$designing.grid.$cardItem = helpers.object.clone(this.$item.$cardItem, true);
					}
				} else {
					this.$designing.cards = {
						$layout: helpers.object.clone(this.$item.$layout, true)
					};
				}
				if (metaData.$format !== undefined && metaData.$format != this.$item.$format) {
					delete this.$item.$cardItem;
					delete this.$item.$layout;
	...
	_appendRecord: function(options) {
		...
		record.loadBox();
		if (options.isCreate && this.builder.cardDecorator) {
			if (this.builder.cardDecorator.isRowCardMode) {
				this.builder.cardDecorator.toggleRowCard(record, true);
			}
		}
	...
	applyFullDelta: function(dataRecordSet, dataIndex) {
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if (dataRecordSet !== undefined) {
			this.fillList(dataRecordSet, parentDataRecord, this.page.$isPartialDelta || (this.page.$isEditMode && this.hasRecords() && !this.builder.isSingleBuilder));
		}
		if (this.graphDecorator) {
			var graphRecords = [];
			for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
		...
		if (this.selectFirstRecordByDefault) {
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		...
		if (isDelta) {
			...
			if (this.clientFetchOptions) {
				...
				this.applyFullDelta(this.clientDataset, dataIndex);
			} else {
				this.clientDataset = this.ensureDataSet();
				var dataIndex = this.pagging.getParseIndexes();
				if (partialDelta) {
			...
			this.clientDataset = this.builder.isSingleBuilder ? this.ensureDataSet() : dataRecordSet;
			if (this.treeDecorator) {
				this.treeDecorator.buildTreeMap(this.clientDataset);
			}
		...
		this.validateDisplay();
		if (this.builder.cardDecorator && this.builder.cardDecorator.isOutCardMode) {
	...
	releaseMode: function(onDispose) {
		...
		this.tableSlot = this.tableRow = this._core = this.menusBox = this.serverFilters = this.builder = this.domTitle = this._mandatoryFlag = this._description = this._helpFlag = null;
	},
	dispose: function() {
		if (this.articleParent.resizableChildren) {
			delete this.articleParent.resizableChildren[this.id];
		}
		if (this.page) {
		...
		this.$designing = this.emptyDataItem = null;
		this.designPickerSlot = this.designPicker = this.topbar = this.bottombar = this.clientDataset = null;
		this.BuilderClass = this._menus = this.page = this.treeDecorator = this.selectorCardRecord = null;
		this.paramsFilter = this.$fields = this.$field = this.$capability = this.menusSlot = null;
		this._dataValue = this.domItem = this.domValueSlot = this.fieldValue = this.globalMetaRecords = null;
		this.BuilderClass = this.recordsMap = this.sortCapability = this.records = this.pagging = this.selector = null;
	...
	setFocus: function(select, name, $serverIndex) {
		...
		if (this.builder.popupCardBuilder && this.builder.popupCardBuilder.isPopupCardOpened()) {
	...
	isReorderRecordDraggable: function(target, event) {
		if (target.syraReorderRecordId == this.id) {
			...
			document.site.ddManager.start(this.builder, RecordReorderDDAgent, $(this.builder.body));
	...
	getClientDataSetServerIndex: function($recordIndex) {
		this.ensureClientDataset();
		if (this.clientDataset[parseInt($recordIndex, 10)]) {
			return this.clientDataset[parseInt($recordIndex, 10)].$serverIndex;
		}
		return 0;
	},
	...
	getField: function(name, $serverIndex, select) {
		...
		if (record && !record.disposed) {
			if ((fields = record.boundFields[name]) && fields.length > 0) {
				return (this.builder.popupCardBuilder ? this.builder.popupCardBuilder.findPopupField(name, record) : null) || fields[0];
			}
		}
		return null;
	},
	...
	getFocusedRecord: function() {
		var record;
		for (var ii = 0, jj = this.dataset.length; ii < jj; ii++) {
			var dataRecord = this.dataset[ii];
			if (record = this.recordsMap[this.dataset[ii].$uuid]) {
				if (record.isFocused) return record;
			}
		}
		return null;
	},
	...
	addSlotResizer: function(target, event) {
		...
		if (resizeCard || resizeGraph) {
			...
			document.site.ddManager.start(this.builder, SlotResizerDDAgent, $(this._core));
			event.syraRetValue = false;
			return true;
		}
	}
