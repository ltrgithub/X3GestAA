...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    fetchClientRecordSet: function(options){
        if (options && options.params) {
            ...
            if (options.params.count || options.params.startIndex) {
                this.pagingCapability.goToPage(resource, {
            ...
            var fields = this.articleParent.boundFields[this.$item.$bind];
            for (var ii = 0, jj = fields.length; ii < jj; ii++) {
                var field = fields[ii];
                field.onClientFetch = true;
                field.setDataBind(dataRecordSet, this.articleParent.dataset, resource);
        ...
        if (metaData.$item) {
            if (metaData.$item.$properties) {
                ...
                for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
                    ...
                    var $updated;
                    if (onAuthoring) {
                        this.$item.$layout.$items.some(function($item){
                            if ($item.$bind == $bind) {
                                $updated = $item;
                                return true;
                            }
                    ...
                    if ($updated) {
                        ...
                        this.page.deltaManager.applyObjectDelta($updated, $metaField, true);
                    }
                }
            }
            if (metaData.$item.$links) {
                var $menus = this.$fieldProperties.$links = this.$prototype.$item.$links || {};
                this.page.deltaManager.applyObjectDelta($menus, metaData.$item.$links, true);
            }
            if (metaData.$item.$actions) {
                var $menus = this.$fieldProperties.$actions = this.$prototype.$item.$actions || {};
                this.page.deltaManager.applyObjectDelta($menus, metaData.$item.$actions, true);
            }
        }
        if (redraw) {
            this.reloadBuilder();
    ...
    _applyPartialDelta: function(dataRecordSet, firstIndex, lastIndex){
        ...
        for (var $serverIndex = 0, jj = dataRecordSet.length; $serverIndex < jj; $serverIndex++) {
            ...
            this._store._records.some(function(record, index){
    ...
    _applyRecordListDelta: function(dataRecordSet, firstIndex, lastIndex){
        ...
        for (var ii = 0, jj = uiids.length; ii < jj; ii++) {
            this.removeRecord(oldMap[uiids[ii]]);
        }
        if (orderChanged) {
            this._store.ensureRecordsIndex();
        }
    ...
    _fillList: function(dataRecordSet, parentDataRecord, isDelta){
        ...
        if (isDelta) {
            ...
            if (self.page.$isPartialDelta || (dataRecordSet.length == 1 && dataRecordSet[0].$index !== undefined)) {
                self._applyPartialDelta(dataRecordSet, firstIndex, lastIndex);
            }
            else {
    ...
    setDataBind: function(dataRecordSet, parentDataRecord, metaData){
        this.ensureDataSet();
        var redraw = false;
        if (this.page.externalAdapter.setDataBindArray(this, dataRecordSet, parentDataRecord, metaData)) {
            if (metaData) {
            ...
            if (dataRecordSet !== undefined) {
                if (this.page.$isPartialDelta || (!this.onClientFetch && this.page.isDeltaMode && this._store.hasRecords() && !this.builder.isSingleBuilder)) {
    ...
    setFocus: function(name, nl){
        var record = this._store._records[nl - 1], field, ret = null;
        if (record && !record.disposed) {
            field = record.boundFields[name];
            if (field) {
                field[0].focus();
                ret = (field[0].$isHidden !== undefined && field[0].$isHidden)|| (field[0].$isDisabled !== undefined && field[0].$isDisabled) || (field[0].$isEditMode !== undefined && !field[0].$isEditMode) ? {"noFocusable": true} : ret;
            }
            else {
                ret = {"noFocusable": true};
            }
        }
        return ret;
    },
    getDataValue: function(name, nl){
        var record = this._store._records[nl - 1], field;
        if (record && !record.disposed) {
            field = record.boundFields[name];
