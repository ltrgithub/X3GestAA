...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    drawBox: function(){
        ...
        this._store.filterCapability.validateIsQuickFilter();
        var articleParent = this.getArticleParent();
        this.$recordFacet = articleParent.$item.$recordFacet || this.$facet || "$details";
        ...
        this._store.applySettings(this.$prototype);
    ...
    fetch: function(options, $location){
        ...
        if (self.isClientFetch) {
            ...
            fetchOptions.$updateHitory = self.getPage().isMainPage;
            fetchOptions.$location = $location;
            articleParent.fetch(fetchOptions);
        }
        if (articleParent.onFetchHandler) {
            articleParent.onFetchHandler(fetchOptions);
        }
    },
    ...
    notifyChange: function($bind, updatedRecord){
        if ($bind == "$itemsPerPage") {
            this._store.pagingCapability.fetchItemPerPage(updatedRecord);
        }
        else {
            if (this.builder.isSingleBuilder || this.getPage().$isNewCollectionEdit) {
                if ($bind == "$actions" && (updatedRecord.$select || updatedRecord.$create || updatedRecord.$delete)) {
                    if (updatedRecord.$create) {
                        this._store.notifyCreateRecord();
                    }
                    if (updatedRecord.$delete) {
                        this._store.notifyDeleteRecord();
                    }
                    if (updatedRecord.$select) {
    ...
    parseCapabilities: function($capabilities){
        ...
        if ($capabilities !== undefined) {
            ...
            switch (self.$facet) {
                case "$edit":
                case "$details":
                    if (self.getPage().$isNewCollectionEdit) {
                        delete self.$capability.add;
                        self.$capability.append = true;
                        self.$capability.insert = true;
                        self.$capability["delete"] = true;
                    }
                    break;
            }
    ...
    applyCapabilities: function(record){
        ...
        if (record) {
            ...
            if (this.$capability.select) {
                $actions.$select = {
                    $title: this._localize.slist_select
                };
            }
    ...
    applyMetaData: function(metaData){
        ...
        if (metaData && metaData.$capabilities !== undefined) {
            ...
            //return self.$isEditMode && (self._store._records.length > 0);
            return (self._store._records.length > 0);
        }
    },
        ...
        if (self.getPage().externalAdapter.setDataBind(self, dataRecordSet, parentDataRecord, metaData)) {
            ...
            if (dataRecordSet !== undefined) {
                ...
                if (parentDataRecord.$isDelta) {
                    _setDataBindOldDelta(self, dataRecordSet);
                }
                else {
                    if (dataRecordSet && dataRecordSet.length > 0 && dataRecordSet[0].$index) {
...
function _setDataBindOldDelta(list, dataRecordSet){
    if (list.$prototype.$item.$type == "application/x-reference") {
        //temp modif for Cristi for single field plus de delta
        if (list.builder.onAfterDataBinding) {
            list.builder.onAfterDataBinding();
        }
        list._store.pagingCapability.draw();
        list._store.sortCapability.renderOrderBy();
        list.builder._body.style.display = "";
    }
    else {
        /* if (list._store.pagingCapability._$totalResults == 0) {
         list.builder.emptyBody();
         }*/
        dataRecordSet.forEach(function(dataRecord, $recordIndex){
            var record = list._store._map[dataRecord.$uuid];
            if (dataRecord.$isDeleted) {
                if (record) {
                    list.removeItem(record, true);
                    delete list._store._map[dataRecord.$uuid];
                    list._store._records[$recordIndex] = {
                        $recordUuid: dataRecord.$uuid,
                        $isDeleted: true
                    };
                }
