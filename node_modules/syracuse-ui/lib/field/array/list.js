...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    reloadBuilder: function(){
        ...
        setTimeout(function(){
            ...
            $(self._core).empty();
            self.articleParent.unbind(self);
            self.createBuilder();
    ...
    /*_applyFullDelta: function(dataRecordSet, firstIndex, lastIndex){
     var self = this;
     var orderChanged;
     dataRecordSet.forEach(function(dataRecord){
     var record = self._store._map[dataRecord.$uuid];
     if (dataRecord.$isDeleted) {
     if (record) {
     self._store._records.splice(record.$recordIndex, 1);
     ...
     if (firstIndex === undefined || (dataRecord.$index >= firstIndex && dataRecord.$index < lastIndex)) {
     self._store.appendRecord({
     dataRecord: dataRecord,
     $serverIndex: dataRecord.$index,
    ...
    _applyFullDelta: function(dataRecordSet, firstIndex, lastIndex){
        ...
        dataRecordSet.forEach(function(dataSourceRecord){
            ...
            if (found) {
                ...
                if (firstIndex === undefined || (dataSourceRecord.$index >= firstIndex && dataSourceRecord.$index < lastIndex)) {
                    self._store.appendRecord({
    ...
    _applyRecordListDelta: function(dataRecordSet, firstIndex, lastIndex){
        ...
        dataRecordSet.forEach(function(dataRecord, $serverIndex){
            ...
            if (record) {
                ...
                if (record.$serverIndex != $serverIndex) {
                    if (lastRecord) {
                        record.$$item.insertAfter(lastRecord.$$item);
                    }
                    else {
                        $(self.builder._body).prepend(record.$$item);
                    }
        ...
        Object.keys(oldMap).forEach(function(uuid){
            self.removeItem(oldMap[uuid], true);
        });
        if (orderChanged) {
            self._store.ensureRecordsIndex();
        }
    },
    _fillList: function(dataRecordSet, redraw){
        var self = this;
        ...
        if (self.page.externalAdapter.setDataBind(self, dataRecordSet, parentDataRecord, metaData)) {
            ...
            if (dataRecordSet !== undefined) {
                ...
                if (self.page.$isFullDelta || (!self.onClientFetch && self.page.isDeltaMode && self._store.hasRecords() && !self.builder.isSingleBuilder)) {
                    ...
                    self._fillList(dataRecordSet, redraw);
                }
