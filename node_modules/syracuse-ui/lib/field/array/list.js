...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	scrollToRecord: function(record) {
		record && syra_page.scrollToItem(this.page, record);
	},
	...
	_applyCommonStatus: function(obj, metaData) {
		if (metaData) {
			obj = obj || {};
	...
	ensureGlobalMetaRecord: function(reset) {
		...
		if (reset || !this.globalMetaRecords) {
			this.globalMetaRecords = this._applyCommonStatus(null, this.$field);
			var $meta = this.getMetaData();
			if ($meta) {
				this.globalMetaRecords = {};
				this._applyCommonStatus(this.globalMetaRecords, $meta);
	...
	findRecord: function(target) {
		var record = null;
		while (!record && target && target != document) {
			if (target.syraRecord !== undefined) {
				record = this.recordsMap[target.syraRecord];
				if (!record) {
					record = syra_store.get(target.syraItem);
					if (record && record.list != this) {
						record = null;
					}
				}
			}
