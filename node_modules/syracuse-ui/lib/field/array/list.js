...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    drawBox: function(){
        ...
        this.paging = new PagingCapability(this);
    ...
    applyCapabilitiesSettings: function(newData){
        this.paging.setOptions(newData);
    ...
    fetch: function(options, $location){
        var params = {
            where: this.filterCapability.$where,
            select: this.$select
        };
        if (options) {
            if (options.$filters) {
            ...
            this.paging.setOptions({
                $itemsPerPage: options.$itemsPerPage
            });
            params.count = this.paging.$itemsPerPage;
            if (options.orderBy) {
                params.startIndex = 1;
            ...
            if (options.filter != null) {
                params.startIndex = 1;
                params.where = this.filterCapability.$where = options.filter != "" ? options.filter : null;
            }
    ...
    updateGridDiagAlert: function(add, record){
        // --- handling grid diag alert info ---
        
        if (record) {
            var ids = record._diagnosesFields ? Object.keys(record._diagnosesFields) : ids;
            if (ids && ids.length != 0) {
                for (var ii = 0, jj = ids.length; ii < jj; ii++) {
                    this.updateGridDiagAlert();
                }
            }
        }
    ...
    onNotifyDataChange: function(field, value){
        if (field.$item.$bind == "$itemsPerPage") {
            this.paging.fetchItemPerPage(value);
        }
        return false;
    },
    ...
    _showField: function(show){
        Field.prototype._showField.call(this, show);
    },
    ...
    applyDesignMetaData: function(metaData, onAuthoring){
        ...
        this.paging.applyMetadata(metaData, onAuthoring);
    },
    ...
    applyMetaData: function(metaData){
        ...
        this.paging.applyMetadata(metaData);
    },
        ...
        if ((field = this.getField(name, $serverIndex))) {
            ...
            if (this.isClientFetch) {
                if (!listHelper.findRecordByServerIndex(this, $serverIndex)) {
                    this.paging.setPage($serverIndex);
                    if ((field = this.getField(name, $serverIndex))) {
                        return field.focus(select);
                    }
                }
