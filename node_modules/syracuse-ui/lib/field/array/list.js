...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    drawBox: function(){
        ...
        if (this.$prototype.$treeview) {
            this.treeMap = {};
            this.treeOrphans = {};
            var $bindings = Object.keys(this.$prototype.$treeview.$bindings);
            for (var ii = 0, jj = $bindings.length; ii < jj; ii++) {
                var $field = this.$fieldProperties[this.$prototype.$treeview.$bindings[$bindings[ii]]];
                if ($field) {
                    $field.$isHidden = true;
                }
            }
    ...
    linkToParentTreeNode: function(record, parentRecord){
        var parentNode = record.treeNode.parentRecord.treeNode;
        parentNode.children.push(record);
        record.treeNode.nodeLevel = parentNode.nodeLevel + 1;
        record.treeNode.node.style.paddingLeft = (record.treeNode.nodeLevel * 18) + "px";
        if (!parentNode.isOpened || !parentNode.isRecordVisible) {
            record.treeNode.isRecordVisible = false;
            record.$$item[0].style.display = "none";
        }
    },
    displayChilTreeNodes: function(treeNode, isRecordVisible){
        for (var ii = 0, jj = treeNode.children.length; ii < jj; ii++) {
            var chidRecord = treeNode.children[ii];
            chidRecord.treeNode.isRecordVisible = isRecordVisible;
            chidRecord.$$item[0].style.display = isRecordVisible ? "" : "none";
            if (chidRecord.treeNode.children.length) {
                this.displayChilTreeNodes(chidRecord.treeNode, chidRecord.treeNode.isOpened && isRecordVisible);
            }
        }
    },
    onTreeNodeEvent: function(event){
        var record = this._store.findRecord($(event.target));
        record.treeNode.isOpened = !record.treeNode.isOpened;
        record.treeNode.nodePicker.className = "s-list-tree-node-picker " + (record.treeNode.isOpened ? "s-open" : "s-close");
        this.displayChilTreeNodes(record.treeNode, record.treeNode.isOpened);
    },
    getLastNodeRecord: function(record){
        if (record.treeNode.children.length > 0) {
            return this.getLastNodeRecord(record.treeNode.children[record.treeNode.children.length - 1]);
        }
        return record;
    },
    appendTreeNode: function(record, slot){
        var $bindings = this.$prototype.$treeview.$bindings;
        var treeNode = {
            children: [],
            parentId: record.dataset[$bindings.$parent],
            id: record.dataset[$bindings.$id],
            slot: slot,
            node: document.createElement("div"),
            nodeDesc: document.createElement("div"),
            nodePicker: document.createElement("a"),
            isOpened: $bindings.$open ? (record.dataset[$bindings.$open] === true || record.dataset[$bindings.$open] === 2) : true
        };
        record.treeNode = treeNode;
        if (treeNode.id != "") {
            this.treeMap[treeNode.id] = record;
            treeNode.parentRecord = this.treeMap[treeNode.parentId];
            if (treeNode.parentRecord) {
                this.linkToParentTreeNode(record);
                var orphans = this.treeOrphans[treeNode.parentId];
                if (orphans) {
                    for (var ii = 0, jj = orphans.length; ii < jj; ii++) {
                        var orphan = orphans[ii];
                        orphan.treeNode.parentRecord = record;
                        this.linkToParentTreeNode(orphan);
                        orphan.$isTreeRecordItemMoved = true;
                        orphan.$$item.insertAfter(record.$$item);
                    }
                    delete this.treeOrphans[treeNode.parentId];
                }
                var parentNode = treeNode.parentRecord.treeNode;
                if (parentNode.children.length == 1) {
                    parentNode.nodePicker.className = "s-list-tree-node-picker " + (parentNode.isOpened ? "s-open" : "s-close");
                    parentNode.nodePicker.style.visibility = "";
                    if (treeNode.parentRecord.$isTreeRecordItemMoved || ((treeNode.parentRecord.$recordIndex - 1) != record.$recordIndex)) {
                        record.$isTreeRecordItemMoved = true;
                        record.$$item.insertAfter(treeNode.parentRecord.$$item);
                    }
                }
                else {
                    ...
                    if (prevChild.$isTreeRecordItemMoved || ((prevChild.$recordIndex - 1) != record.$recordIndex)) {
                        record.$isTreeRecordItemMoved = true;
                        record.$$item.insertAfter(this.getLastNodeRecord(prevChild).$$item);
                    }
                    // }
                ...
                if (treeNode.parentId != "") {
                    (this.treeOrphans[treeNode.parentId] = this.treeOrphans[treeNode.parentId] || []).push(record);
                }
                treeNode.nodeLevel = 0;
            }
            treeNode.node.className = "s-list-tree-node";
            
            treeNode.nodePicker.setAttribute("href", "#");
            treeNode.nodePicker.className = "s-list-tree-node-picker";
            treeNode.nodePicker.style.visibility = "hidden";
            treeNode.node.appendChild(treeNode.nodePicker);
            
            if ($bindings.$icon) {
                var iconName = record.dataset[$bindings.$icon];
                treeNode.nodeIcon = document.createElement("div");
                treeNode.nodeIcon.className = "s-list-tree-node-icon";
                treeNode.nodeIcon.style.backgroundImage = "url('" + document.site.$item.$iconPath + "tree/" + iconName + "')";
                treeNode.node.appendChild(treeNode.nodeIcon);
            }
            treeNode.nodeDesc.className = "s-list-tree-node-desc";
            slot.appendChild(treeNode.node);
            treeNode.descriptionField = record.page.loadNewItem(treeNode.node.appendChild(treeNode.nodeDesc), {
        ...
        if (metaData.$item) {
            if (metaData.$item.$properties) {
                ...
                for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
                    ...
                    if ($updated) {
                        if (!redraw) {
                            redraw = ($metaField.$isHidden !== undefined) || ($metaField.$isExcluded !== undefined) || ($metaField.$isReadOnly !== undefined) ||
    ...
    unregiterTreeNode: function(record){
        if (record && record.treeNode) {
            ...
            delete this.treeMap[record.treeNode.id];
    ...
    removeRecord: function(record, removeDom){
        if (record.list) {
            ...
            if (this.treeMap) {
                this.unregiterTreeNode(record);
            }
            this.removeItem(record, removeDom);
        }
    },
    traceData: function(lib, bind){
        //LVtrace
        if (this.$item.$bind == bind) {
            console.log("-------------------" + lib + " " + this.$item.$bind + " this dataset --------------------------")
            for (var i = 0; i < this.dataset.length; i++) {
                console.log("uuid:" + this.dataset[i].$uuid + " serverIndex:" + this.dataset[i].$serverIndex + " index:" + this.dataset[i].$index + " isSelected:" + this.dataset[i].$isSelected)
            }
            console.log("------------------- end trace --------------------------")
        }
    },
    _fillList: function(dataRecordSet, parentDataRecord, isDelta){
        this.traceData("Start _fillList", "BA")
        ...
        this.traceData("End _fillList", "BA")
    },
    ...
    getDataValue: function(name, $serverIndex){
        ...
        if (field && field.$isEditMode && $serverIndex) {
    ...
    unSelectLines: function(serverIndex){
        ...
        for (var ii = 0; ii < this.dataset.length; ii++) {
            if (recordData = this._store._map[this.dataset[ii].$uuid]) {
