...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	toggleUIDesign: function(enable, disposingDesigner) {
		if (enable) {
			...
			if (this.designPicker) {
				document.site.dom.removeChild(this.designPickerSlot);
	...
	_ensureDesignMenuSlotVisibility: function() {
		...
		if (this.designPicker) {
			document.site.dom.disableItem(this.designPicker, this.designer);
		}
	},
	...
	_appendRecord: function(options) {
		if (this.emptyDataItem) {
			document.site.dom.removeChild(this.emptyDataItem);
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		if (isDelta) {
			...
			this.mustBeResized = true;
			delete this.fitToWidFields;
			delete this.isFittedToWidth;
	...
	ensureListVisibility: function() {
		...
		if (this.records) {
			...
			for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
				...
				this.records[ii].ensureRecordVisibility();
			}
		}
	},
			...
			if (this.records) {
				...
				for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
					...
					if (resize || record.mustBeResized) {
						record.resizeRecord(resize);
						record.mustBeResized = false;
					}
				}
			}
			if (this.designer) {
				this.designer.resizePage(resize);
			}
