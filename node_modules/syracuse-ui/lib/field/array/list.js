...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	drawBox: function() {
		...
		switch (this.articleParent.$facet) {
			...
			case "$lookup":
				...
				document.site.lastFocusGrid = this;
				break;
			case "$select":
	...
	onWindowResize: function() {
		...
		// scroll buttons
		listHelper.appendScrollButtons(this);
	},
	...
	dispose: function() {
		...
		this.awAddin = this.emptyDataItem = this.helper = this.reorderCapability = null;
		this.topbar = this._core = this.bottombar = null;
		this._menus = this.page = this.treeDecorator = this.menusBox = null;
		this.paramsFilter = null;

		// unbind scroll buttons if any
		listHelper.bindScrollButtonsEvents(this, false);


		// remove listeners
		if (document.site.mainPage && document.site.mainPage.removeScrollListener) {
			document.site.mainPage.removeScrollListener(this.id);
		}

		// delete scroll buttons
		this.leftScrollBtn = this.rightScrollBtn = null;
	...
	addScrollListener: function() {
		if (document.site.mainPage && document.site.mainPage.addListScrollListener && document.site.mainPage.updateListScrollListener) {
			this.listScrollBtnHandler = {
				leftScrollBtn: this.leftScrollBtn,
				rightScrollBtn: this.rightScrollBtn,
				listHscroller: this.builder.scroller._hscroller,
				listHeadTable: this.builder._headTable
			};
			document.site.mainPage.addListScrollListener(this.id, this.updateScrollBtnPosition);
			this.updateScrollBtnPosition(this);
			return true;
		}
		return false;
	},
	removeScrollListener: function() {
		document.site.mainPage.removeListScrollListener(this.id);
	},
	updateScrollBtnPosition: function(list) {
		var btnHdlr = list.listScrollBtnHandler;
		var hscroller = btnHdlr.listHscroller;
		var headtable = btnHdlr.listHeadTable;
		var top;
		// computing scroll btn position according whether head table or table hor scroll are visible or not 
		switch (document.site.isScrolledIntoView($(headtable), document.site.mainPage.$$scrollview)) {
			// list head table is visible in the page
			case (true):
				// list hor scroller is visible in the page
				if (document.site.isScrolledIntoView($(hscroller), document.site.mainPage.$$scrollview)) {
					top = (($(hscroller).offset().top - $(headtable).offset().top) / 2 + $(headtable).offset().top) + "px";
				}

				// list hor scroller is not visible in the page
				else {
					top = ((document.site.mainPage.$$scrollview[0].clientHeight - $(headtable).offset().top) / 2 + $(headtable).offset().top) + "px";
				}
				break;
			...
			case (false):
				// list hor scroller is visible in the page
				if (document.site.isScrolledIntoView($(hscroller), document.site.mainPage.$$scrollview)) {
					top = ($(hscroller).offset().top / 2) + "px";
				}

				// list hor scroller is not visible in the page
				else {
					var docViewTop = document.site.mainPage.$$scrollview.scrollTop();
					var docViewBottom = docViewTop + document.site.mainPage.$$scrollview.height();
					var hscrollerBottom = $(hscroller).position().top + $(hscroller).height();
		...
		if (!top) {
			btnHdlr.leftScrollBtn.style.display = btnHdlr.rightScrollBtn.style.display = "none";
		} else {
			btnHdlr.leftScrollBtn.style.display = btnHdlr.rightScrollBtn.style.display = "";
			btnHdlr.leftScrollBtn.style.top = btnHdlr.rightScrollBtn.style.top = top;
		}
	}
});