...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    ensureDataRecords: function(){
        var dataRecords = this.getDataSet();
        if (!dataRecords) {
            dataRecords = this.getArticleParent().dataset[this.$item.$bind] = [];
        }
        return dataRecords;
    },
    findDataRecord: function($uuid){
        var found;
        this.ensureDataRecords().some(function(dataRecord, $index){
    ...
    _notifyChangeToParent: function(data, isNotifyOnly){
        this.builder.onNotifyChangeToParent(data, isNotifyOnly);
        Article.prototype._notifyChangeToParent.call(this, data, isNotifyOnly);
    },
    notifyChange: function($bind, updatedRecord, isNotifyOnly){
        if ($bind == "$itemsPerPage") {
            ...
            if ($bind == "$actions" && (updatedRecord.$create || updatedRecord.$delete)) {
                ...
                if (updatedRecord.$select) {
                    ...
                    var $prop = {};
                    $prop[this.$item.$bind] = {
                        $actions: updatedRecord
                    }
                    //hack temp a voir avec Christi
                    this.getArticleParent()._notifyChangeToParent({
                        $properties: $prop
                    }, true);
                }
            }
    ...
    setDataBind: function(dataRecordSet, parentDataRecord, metaData){
        ...
        if (self.getPage().externalAdapter.setDataBind(self, dataRecordSet, parentDataRecord, metaData)) {
            ...
            if (dataRecordSet !== undefined) {
                ...
                if (!self.onClientFetch && self._isDeltaMode() && self._store.hasRecords() && !self.builder.isSingleBuilder) {
                    dataRecordSet.forEach(function(dataRecord, $dataRecordIndex){
                        ...
                        if (dataRecord.$isDeleted) {
                            ...
                            if (record) {
                                ...
                                self._store.appendRecord({
                                    dataRecord: dataRecord,
                                    $recordIndex: dataRecord.$index || $dataRecordIndex,
                                    isInsert: true
                                });
                            }
                        }
