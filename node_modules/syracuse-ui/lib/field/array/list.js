...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	appendCardGraphSlots: function() {
		...
		if (!this.$item.$cards && !this.$item.$graphPosition) {
			if (this.tableRow) {
		...
		if (this.$item.$cards || this.$item.$graphPosition) {
			...
			if (this.$item.$cards) {
				...
				hasRow = this.$item.$cards.$position == "left" || this.$item.$cards.$position == "right";
			}
			if (this.$item.$graphPosition) {
			...
			if (this.$item.$cards) {
				switch (this.$item.$cards.$position) {
					case "left":
				...
				if (this.$item.$cards.$widths) {
					this.outCardSlot.style.width = this.$item.$cards.$widths + "%";
				}
		...
		if (this.$prototype.$cube) {
			if (this.$item.$graphMode != undefined) {
				if (this.$item.$graphMode = "list") {
					this.$item.$graphPosition = "behind";
				} else {
