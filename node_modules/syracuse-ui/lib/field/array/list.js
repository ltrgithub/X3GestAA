...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	notifyClientSave: function(savedDelta) {
		if (savedDelta && savedDelta.$uuid) {
			...
			if (dataRecord) {
				document.site.deltaManager.applyObjectDelta(this.page, dataRecord, delta);
				...
				delta = [delta];
				document.site.deltaManager.applyPageArrayDelta(this.page, dataset, delta);
	...
	saveArticleUpdate: function(resize, $userPreferences) {
		var $fieldUserPreferences;
		if ($userPreferences) {
			$fieldUserPreferences = {};
			$fieldUserPreferences[this.$item.$bind] = $userPreferences;
		}
		...
		if (designer) {
			...
			if (this.page.saveArticleUpdate) {
				this.page.saveArticleUpdate($fieldUserPreferences);
			}
		}
	},
	...
	_ensureDesignMenuSlotVisibility: function() {
		...
		if (this.designPicker) {
			document.site.disableItem(this.designPicker, this.designer);
		}
	},
	...
	loadBox: function(initData) {
		...
		this.$skinRecordMenus = this.$skinMenus + "-record-menus";
		this.$skinRecordMenusLink = this.$skinRecordMenus + "-link";
	...
	appendMenusSlot: function() {
		...
		this.menusSlot.className = this.$skinMenus + "-cell";
	...
	defineNewIconMenu: function($bind, $skin, iconOnly) {
		return {
			$category: "link",
			$skin: $skin,
			$bind: $bind,
			$icon: {
				$mode: iconOnly ? "icon" : "iconText",
				$path: this.$iconPath
			}
		};
	},
	defineMenuItems: function($menuBag) {
	...
	_createMenusBox: function() {
		var $menuBag = {
			...
			$skin: this.$skinMenus,
			$itemSkin: this.$skinRecordMenusLink,
			$layout: {
				$items: [this.defineNewIconMenu("$delete", this.$skinMenus + "-link")]
			}
		};
		this.defineMenuItems($menuBag);
		this.page.loadNewItem(this.menusSlot, {
			$layoutType: "row",
			$autoSize: true,
			$items: [this.defineNewIconMenu("$create", this.$skinMenus + "-link"), this.defineNewIconMenu("$select", this.$skinMenus + "-link"), $menuBag]
	...
	doSelectAction: function(menuItem) {
		...
		document.site.dialogManager.openBoxDialog(self.boxParent, {
			article: self,
	...
	applyMetaData: function(metaData) {
		...
		if (metaData.$item) {
			if (metaData.$item.$links) {
				...
				document.site.deltaManager.applyObjectDelta(this.page, $menus, metaData.$item.$links, true);
			}
			if (metaData.$item.$actions) {
				//var $menus = this.$fields.$actions = this.$prototype.$item.$actions || {};
				var $menus = this.$prototype.$item.$actions || {};
				document.site.deltaManager.applyObjectDelta(this.page, $menus, metaData.$item.$actions, true);
			}
		}
	...
	setDataBind: function(dataRecordSet, parentDataRecord, metaData) {
		...
		if (dataRecordSet !== undefined) {
			this.fillList(dataRecordSet, parentDataRecord, this.page.$isPartialDelta || (this.page.$isEditMode && this.hasRecords() && !this.isSingList));
		} else {
			if (metaData) {
	...
	validateDisplay: function() {
		var showAlt = true;
		var $quickActionsEdit;
		var $quickActionsMenu;
		for (var ii = 0, jj = this.records.length; ii < jj; ii++) {
			var record = this.records[ii];
			...
			if (record.$quickActionsEdit) {
				$quickActionsEdit = true;
			}
			if (record.$quickActionsMenu) {
				$quickActionsMenu = true;
			}
		}
		if (this.validateQuickActionsDisplay) {
			this.validateQuickActionsDisplay($quickActionsEdit, $quickActionsMenu);
		}
		//_checkMaxItems
		if (this.$isEditMode) {
	...
	dispose: function() {
		...
		this.tableSlot = this.tableRow = this._core = this.menusBox = this.serverFilters = null;
		...
		this._menus = this.paramsFilter = this.$fields = this.$field = this.$capability = this.menusSlot = null;
	...
	isReorderRecordDraggable: function(target, event) {
		if (target.syraReorderRecordId == this.id) {
			...
			document.site.ddManager.start(this, this.RecordReorderDDAgentClass || RecordReorderDDAgent, this.body);
