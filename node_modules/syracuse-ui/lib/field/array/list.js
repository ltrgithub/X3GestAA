...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    loadBox: function(initData){
        ...
        this.page = this.articleParent.page;
    ...
    _applyConvergenceStyle: function($convergenceStyle){
        Field.prototype._applyConvergenceStyle.call(this, $convergenceStyle);
    },
    ...
    applyFieldsMetaData: function(metaData, onAuthoring){
        ...
        if (metaData.$orderBy !== undefined) {
            self._store.sortCapability.setOrderBy(metaData.$orderBy, metaData.$startLetter);
        }
        self._store.pagingCapability.applyMetadata(metaData, onAuthoring);
        if (metaData.$item) {
            if (metaData.$item.$properties) {
                Object.keys(metaData.$item.$properties).forEach(function($bind){
                    ...
                    if ($updated) {
                        self.page.deltaManager.applyObjectDelta($updated, $metaField, true, function(target, sourceValue, property){
    ...
    onNotifyDataChange: function(field, value){
        if (field.$item.$bind == "$itemsPerPage") {
            this._store.pagingCapability.fetchItemPerPage(value);
        }
        return false;
    },
    ...
    _fillList: function(dataRecordSet){
        var self = this;
    ...
    setDataBind: function(dataRecordSet, parentDataRecord, metaData){
        ...
        self.ensureDataSet();
        var redraw = false;
        if (self.page.externalAdapter.setDataBind(self, dataRecordSet, parentDataRecord, metaData)) {
            if (metaData) {
                if (redraw = self.applyMetaData(metaData)) {
                    if (dataRecordSet === undefined) {
                        dataRecordSet = self.ensureDataSet();
                        parentDataRecord = self.articleParent.dataset;
                    }
                }
            }
            if (dataRecordSet !== undefined) {
                self._store.applySettings(parentDataRecord);
                if (self.page.$isFullDelta || (!self.onClientFetch && self.page.isDeltaMode && self._store.hasRecords() && !self.builder.isSingleBuilder)) {
                    var firstIndex, lastIndex;
                    if (self._store.pagingCapability) {
                        firstIndex = self._store.pagingCapability._$startIndex;
                        if (firstIndex !== undefined) {
                            --firstIndex;
                            if (self._store.pagingCapability._$itemsPerPage !== undefined) {
                                lastIndex = firstIndex + self._store.pagingCapability._$itemsPerPage;
                            }
                        }
                    }
                    ...
                    if (lastIndex && self._store._records.length > self._store.pagingCapability._$itemsPerPage) {
                        while (self._store._records.length > self._store.pagingCapability._$itemsPerPage) {
