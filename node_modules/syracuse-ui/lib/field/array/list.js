...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	designArticle: function(open) {
		document.site.designArticle(open, this, ListDesigner);
		if (!open) {
			if (this.$designing) {
	...
	_createMenusBox: function() {
		...
		if (this.$prototype.$cube || this.$prototype.$decorator) {
			$menuBag.$layout.$items.push(this.defineNewIconMenu("$toggleGraph"));
		}
		if (this.$isQuickDesignerEnabled !== false) {
			$menuBag.$layout.$items.push(this.defineNewIconMenu("$design"));
		}
		$menuBag.$layout.$items.push(this.defineNewIconMenu("$cancelSort"));
		...
		for (var ii = dataIndex.start; ii <= dataIndex.last; ii++) {
			...
			if (sourceDataset) {
				...
				if (record) {
					...
					if (record.getRecordIndex() != recordIndex) {
						if (lastRecord) {
	...
	dispose: function() {
		...
		this.RecordClass = this.body = null;
