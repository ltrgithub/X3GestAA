...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
    ...
    onGridFieldDiagnose: function(append){
        ...
        if (append) {
            // list diag count
            self.diagCount = self.diagCount || 0;
            // list diag info message
            if (!self.diagAlert) {
                var msg = document.createElement("div");
                msg.className = "s-grid-diag-alert";
                $(self._menusSlot).after($(msg));
                self.diagAlert = msg;
            }
            self.diagAlert.textContent = ++self.diagCount + self.localize.flDiagAlert;
        }
        ...
        // if diagnose is removed from a cell. clean
            ...
            self.diagAlert.textContent = --self.diagCount + self.localize.flDiagAlert;
            if (self.diagCount == 0) {
                document.site.emptyDom(self.diagAlert);
                self.diagAlert = null;
            }
        }
    },
    ...
    applyMetaData: function(metaData){
        ...
        if (metaData.$type === undefined) {
            ...
            if ((metaData.$item && metaData.$item.$properties) || (metaData.$isReadOnly !== undefined || metaData.$isDisabled !== undefined || metaData.$fieldStyle !== undefined || metaData.$valueStyle !== undefined || metaData.$titleStyle !== undefined)) {
                var $metaRecord = {};
                var statusList = ["$isReadOnly", "$isDisabled", "$fieldStyle", "$valueStyle", "$titleStyle"];
                var $newMetaProperties = metaData.$item ? metaData.$item.$properties : null;
                var $binds = Object.keys(this.$fieldProperties);
                var $deltListProperties = this.articleParent.dataset.$properties ? this.articleParent.dataset.$properties[this.$item.$bind] : null;
                var $deltaFieldProperties = $deltListProperties ? ($deltListProperties.$item ? $deltListProperties.$item.$properties : null) : null;
                for (var ii = 0, jj = statusList.length; ii < jj; ii++) {
                    var status = statusList[ii];
                    if (metaData[status] !== undefined) {
                        if ($deltaFieldProperties) {
                            for (var mm = 0, kk = $binds.length; mm < kk; mm++) {
                                var $bind = $binds[mm];
                                if ($deltaFieldProperties[$bind]) {
                                    if (!($newMetaProperties && $newMetaProperties[$bind] && $newMetaProperties[$bind][status] !== undefined)) {
                                        $deltaFieldProperties[$bind][status] = metaData[status];
                                    }
                                }
                            }
                        }
                        $metaRecord[status] = metaData[status];
                    }
                }
                if (metaData.$item) {
        ...
        if (metaData && metaData.$capabilities !== undefined) {
            ...
            var $actions;
            exports.parseCapabilities(this, metaData.$capabilities);
        ...
        this.pagingCapability.applyMetadata(metaData);
    },
