...
var ListDesigner = require("syracuse-ui/lib/authoring/listDesigner").ListDesigner;
...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
	...
	toggleUIDesign: function(enable, disposingDesigner) {
		if (enable) {
			...
			if (this.designPicker) {
				document.site.removeDomChild(this.designPicker);
	...
	appendCardGraphSlots: function() {
		var $cards = this.$item.$format != "cards" && this.$item.$cards;
		if (!$cards) {
		...
		if (!$cards && !this.$item.$graphPosition) {
			if (this.tableRow) {
		...
		if ($cards || this.$item.$graphPosition) {
			...
			if ($cards) {
				...
				hasRow = $cards.$position == "left" || $cards.$position == "right";
			}
			if (this.$item.$graphPosition) {
			...
			if ($cards) {
				switch ($cards.$position) {
					case "left":
						this.tableSlot.parentNode.insertBefore(this.outcardSeparator, this.tableSlot);
						this.tableSlot.parentNode.insertBefore(this.outCardSlot, this.outcardSeparator);
						this.outcardSeparator.className = "s-list-outcard-sep-cell";
						this.outCardSlot.className = "s-list-outcard-slot-cell";
						break;
					case "right":
						this.tableSlot.parentNode.insertBefore(this.outcardSeparator, this.tableSlot.nextSibling);
						this.tableSlot.parentNode.appendChild(this.outCardSlot, this.outcardSeparator.nextSibling);
						this.outcardSeparator.className = "s-list-outcard-sep-cell";
						this.outCardSlot.className = "s-list-outcard-slot-cell";
						break;
					case "top":
						var sibling = this.tableRow || this.tableSlot;
						sibling.parentNode.insertBefore(this.outcardSeparator, this.tableSlot);
						sibling.parentNode.insertBefore(this.outCardSlot, this.outcardSeparator);
						this.outcardSeparator.className = "s-list-outcard-sep";
						this.outCardSlot.className = "s-list-outcard-slot";
				...
				if ($cards.$widths) {
					this.outCardSlot.style.width = $cards.$widths + "%";
				}
		...
		if (operator == 'like' && type == 'string') {
			...
			// "starts with" operation
				...
				if (/(([^%]|\%)*)%$/g.test(val2)) {
					...
					throw new Error(document.site.localize.flFilter_err_wrong_value.replace('{operator}', operator).replace('{value}', val2));
				}
			}
		} else {
			if (operator == 'like' && type !== 'string') {
				throw new Error(document.site.localize.flFilter_err_wrong_type.replace('{operator}', operator).replace('{type}', type));
			} else {
				switch (operator) {
					case ">":
	...
	reloadLayout: function($item) {
		...
		this.tableSlot = this.outcardSeparator = this.outCardSlot = this.graphSeparator = this.graphSlot = this.tableRow = null;
		this.createBuilder();
		this.builder.drawBuilder();
	...
	_appendRecord: function(options) {
		...
		record.loadBox();
		if (options.isCreate && record.builder.$inlineCard) {
	...
	applyFullDelta: function(dataRecordSet) {
		...
		for (var ii = dataIndex.start; ii <= dataIndex.last; ii++) {
			...
			var sourceDataset = this.dataset[dataRecord.$serverIndex || dataRecord.$index || (ii - 1)];
	...
	applyPartialDelta: function(deltaSet, firstIndex) {
		var startIndex = this.pagging.$startIndex || 1;
		var dataIndex = this.pagging.getParseIndexes(this.clientDataset || deltaSet);
		...
		if (isDelta) {
			...
			if (this.clientFetchOptions) {
				...
				if (this.pagging.$startIndex !== undefined && this.clientDataset.length < this.pagging.$startIndex) {
					...
					this.clientFetchOptions.params.startIndex = options.$startIndex;
				}
				if (this.treeDecorator) {
			...
			if (this.clientDataset) {
				...
				if (this.isClientFetch) {
					...
					var dataIndex = this.pagging.getParseIndexes(this.clientDataset);
