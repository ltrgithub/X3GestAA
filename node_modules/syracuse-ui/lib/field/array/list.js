...
exports.DesktopList = helpers.defineClass(DesktopList, Article, {
			...
			if (metaData.$format !== undefined && metaData.$format != this.$item.$format) {
				...
				if (this.$item.$format == "grid") {
					this.$designing.grid = {
						...
						$cardItem: this.$item.$cardItem ? helpers.object.clone(this.$item.$cardItem, true) : undefined
					};
	...
	_appendRecord: function(options) {
		...
		record.loadBox();
		if (options.isCreate && this.list.$item.$cardItem && this.list.$item.$cardItem.$position == "inline") {
