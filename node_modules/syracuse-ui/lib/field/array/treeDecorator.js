...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
    ...
    expendCollapseAll: function(expend){
        var records = this.list.records, node, self = this;
    ...
    getPreviousNextServerIndexVisible: function(recordIndex, next){
        ...
        for (var ii = 0, jj = this._orderedNodes.length; ii < jj; ii++) {
            if (this._orderedNodes[ii].$recordIndex == recordIndex) {
                index = ii;
                break;
            }
        }
        for (var ii = (next ? Math.min(index + 1, this._orderedNodes.length - 1) : Math.max(index - 1, 0)), jj = (next ? this._orderedNodes.length : -1); (next && ii < jj) || (!next && ii > jj); (next ? ii++ : ii--)) {
