...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
	...
	expendCollapseAll: function(expend) {
		...
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			...
			if (node) {
				if (expend === node.isOpened) continue;
		...
		setTimeout(function() {
			self.list.onWindowResize();
		}, 100);
	},
	getPreviousNextServerIndexVisible: function($recordIndex, next) {
		var list = this.list,
			index = parseInt($recordIndex),
			found;
		list.ensureClientDataset();
		for (var ii = (next ? Math.min(index + 1, list.clientDataset.length - 1) : Math.max(index - 1, 0)), jj = (next ? list.clientDataset.length : -1);
			(next && ii < jj) || (!next && ii > jj);
			(next ? ii++ : ii--)) {
			if (this._uuidNodes[list.clientDataset[ii].$uuid].isVisible) {
				return ii;
			}
		}
		return index;
	},
	isNodeOpened: function($uuid) {
		return this._uuidNodes[$uuid].isOpened;
	},
	...
	onWindowResize: function() {
		if (this.hasFixWith && this.list.records) {
			var maxDiff = 0;
			for (var ii = 0, jj = this.list.records.length; ii < jj; ii++) {
				var record = this.list.records[ii];
				if (record.treeNode && record.treeNode.item) {
					var diff = record.treeNode.item.scrollWidth - record.treeNode.item.clientWidth;
					if (diff > maxDiff) {
						maxDiff = diff;
					}
				}
			...
			if (maxDiff) {
				...
				col.groupHead.style.width = col.groupBody.style.width = (col.widthValue += maxDiff) + "px";
			}
		}
	},
