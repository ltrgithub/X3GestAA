...
exports.TreeDecorator = helpers.defineClass(TreeDecorator, null, {
				...
				if (record) {
					if (isCard) {
						...
						if (parentNode && parentNode.childRoot) {
							...
							this.list.builder.body.appendChild(record.dataRow);
						}
					} else {
						this.list.builder.body.appendChild(record.dataRow);
					}
	...
	appendTreeviewCol: function(slot) {
		var col = this.list.builder.cols.$treeview = this.list.builder.appendCol();
		col.slot = slot;
	...
	onWindowResize: function() {
		if (this.hasFixWith && this.list.records) {
			...
			if (maxDiff) {
				var col = this.list.builder.cols.$treeview;
	...
	appendCellNode: function(record, slot) {
		...
		cell.className = record.builder.gridCss.cell + " " + this.cssNodeCell;
