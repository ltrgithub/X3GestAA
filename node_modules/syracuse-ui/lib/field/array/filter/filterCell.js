...
exports.FilterCell = helpers.defineClass(FilterCell, null, {
				...
				doEvent: function() {
					if (!self.listPopup) {
						...
						for (var ii = 0, jj = operators.length; ii < jj; ii++) {
							...
							self.$$list[0].appendChild(link).textContent = self.record.boxParent._localize["filter_" + op];
						}
	...
	ensureDefault: function() {
		...
		if (this.$field) {
			...
			if (defaultFilter) this.setOperator(defaultFilter);
		}
	},
	dispose: function() {
		if (this._operator) {
		...
		this.$filterField = this.record = this.field = null;
	},
	// FDB LV
