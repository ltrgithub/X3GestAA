...
exports.FilterCell = helpers.defineClass(FilterCell, null, {
	...
	render: function(record, layoutSlot, $field, $bind) {
		...
		if (this.record.list.$urlParams.where) {
			//var exp = parser.Parser.parse(this.record.list.$urlParams.where.replace(/%/g, ""));
			var exp = parser.Parser.parse(this.record.list.$urlParams.where);
			...
			if ($filterBind == exp.children[0].value) {
				...
				var value = this.unFormat(exp.value.text, exp.children[1].value);
	...
	unFormat: function(operator, value) {
		switch (operator) {
			case "like":
