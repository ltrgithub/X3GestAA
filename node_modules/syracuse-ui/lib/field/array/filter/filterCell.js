...
exports.FilterCell = helpers.defineClass(FilterCell, null, {
	...
	render: function(record, layoutSlot, $field, $bind) {
		...
		this.loadField(this.$field, this.$bind); // fk
		//previous version : this.loadField(self.$field, self.$bind)
	},
		...
		if (this.operator != "none" && value != null) {
			...
			switch (op) {
				case "like":
					...
					right = "'%" + this._format(field, this.$field.$type, value, true).replace(/%/g, '\%') + "%'";
					break;
				case "like_s":
					// like is for strings only so direct access to store
					right = "'" + this._format(field, this.$field.$type, value, true).replace(/%/g, '\%') + "%'";
				...
				case "between":
					...
					if (value && !highValue) {
						right = this._format(field, this.$field.$type, value).replace(/%/g, '\%');
						op = "ge";
					} else {
						if (!value && highValue) {
							right = this._format(field, this.$field.$type, highValue).replace(/%/g, '\%');
							op = "le";
						} else {
							if (value && highValue) {
								right = this._format(field, this.$field.$type, value).replace(/%/g, '\%') + " and " + this._format(field, this.$field.$type, highValue).replace(/%/g, '\%');
							} else {
								return "";
							}
						}
