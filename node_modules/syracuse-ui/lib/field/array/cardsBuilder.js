...
function _addDefaultCell(builder, row) {
	var cell = syra_dom.div(builder.list.$skin + "-cell");
...
exports.CardsBuilder = helpers.defineClass(CardsBuilder, null, {
			...
			if (metaData.$cardsByRowCount !== undefined) {
				...
				if (this.list.records.length) {
					...
					for (var ii = 0, jj = this.rows.length; ii < jj; ii++) {
						syra_dom.remove(this.rows[ii].slot);
					}
					this.rows = newRows;
				}
			}
	...
	record_draw: function(record) {
		...
		if (this.list.treeDecorator) {
			...
			if (this.list.selector.isRowMode) {
				...
				record.treeNode.item.className += " s-record-selector-row";
	...
	record_onSelect: function(record, selected) {
		syra_dom.toggleClass(this.list.treeDecorator ? (record.treeNode.desc || record.treeNode.item) : record.domItem, "s-record-selected", selected);
	...
	record_onAfterEnsureLayoutMode: function(record) {
		...
		if (record.rowIndexBtn) {
			...
			if (this.list.$item.$selectByRowIndex) {
				record.rowIndexBtn.className += " s-record-row-index";
				record.rowIndexBtn.syraOnClick = "onSelectorClick";
			}
		}
		if (this.list.selector.isRowMode) {
			record.body.className += " s-record-selector-row";
		}
		if (record.menusSlot) {
	...
	addResizer: function() {
		this.resizer = new Resizer({
			...
			start: function(resizing) {
				resizing.body = syra_dom.getBoundingClientRect(this.builder.tableSlot);
			},
			drag: function(resizing) {
				this.builder.tableSlot.style.maxHeight = "";
				if (resizing.deltaX != undefined) {
					this.builder.tableSlot.style.width = (resizing.newWidth = resizing.body.width + resizing.deltaX) + "px";
				}
				if (resizing.deltaY != undefined) {
					resizing.newHeight = Math.max(resizing.body.height + resizing.deltaY, 25);
					this.builder.tableSlot.style.height = resizing.newHeight + "px";
			...
			if (this.list.$item.$fixedBodyHeight) {
				...
				if (adjustToScrollview) {
					...
					if (this.list.$fitContainer) {
						...
						if (scrollview) {
							var maxH = Math.floor(scrollview.clientHeight / 2);
	...
	dispose: function(removeItem) {
		removeItem && syra_dom.remove(this.tableSlot);
