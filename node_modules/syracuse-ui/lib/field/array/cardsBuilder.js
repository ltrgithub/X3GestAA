...
function _addRow(builder) {
	return {
		slot: syra_dom.div(builder.list.$skin + "-carditem-slot"),
		index: builder.rows.length,
		count: 0,
		cells: [],
		cellWidth: Math.ceil(100 / builder.$cardsByRowCount) + "%",
	...
	onResizeArray: function() {
		if (syra_dom.isVisible(this.tableSlot)) {
			...
			if (this.list.$item.$height) {
				...
				this.tableSlot.style.height = ((Math.abs(parseInt(this.list.$item.$height, 10) - height) < 2) ? height : this.list.$height) + "px";
			} else {
				if (adjustToScrollview) {
					...
					if (this.list.$fitContainer) {
						...
						if (scrollview) {
							...
							if (this.list.page.searchResultField) {
								this.tableSlot.style.maxHeight = pageSize.overPanel.maxHeight / 2 + "px"; //  Math.max(maxH - 75, 100) + "px";
							} else {
