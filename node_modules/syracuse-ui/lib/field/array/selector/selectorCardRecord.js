...
exports.SelectorCardRecord = helpers.defineClass(SelectorCardRecord, Article, {
	dispose: function() {
		...
		this._item = this.slot = this._menusSlot = this._popup = this._popupSlot = null;
	...
	initialize: function(list) {
		...
		this.$facet = this.list.$recordFacet;
	},
	loadBox: function() {
		//this.$uuid = options.dataRecord.$uuid;
	...
	drawBox: function() {
		...
		this._item.className = this.list.$skin + "-record-selector";
		this.$$item = $(this._item);
		var card = this._item.appendChild(document.createElement("div"));
		card.className = this.list.$skin + "-record";
		this.page.loadNewItem(card, this.$item, this);
		this.list.layoutSlot.appendChild(this._item);
	...
	onFieldClickPicker: function(field, pickerType, btn) {
		if ((field.$item.$bind == "$selectorField") && pickerType == "lookup") {
			...
			self._popupSlot.appendChild(self.list._domItem);
			self._popup = self.openDialog({
				content: field,
				isPageBoundary: true,
	...
	onWindowResize: function() {
		if (this._popup) {
			this._popup.onWindowResize(true);
		}
	}
});