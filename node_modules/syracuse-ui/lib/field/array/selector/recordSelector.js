...
function _addCheckBox(slot, isCheck) {
...
var _types = {
	...
	single: {
		...
		appendToRecord: function(selector, slot) {
			return _addCheckBox(slot, false);
		}
	},
	button: {
		load: function(selector) {
			...
			selector._widthSelector = (11 * selector.text.length);
		},
		appendToRecord: function(selector, slot) {
...
function _ensureContiguousSelection(selector, targetRecord) {
	...
	if (range) {
		var records = selector.list.records;
		if (range.targetIndex < (range.firstIndex - 2) || range.targetIndex > (range.lastIndex + 2)) {
...
function _onSelectOn(selector, targetRecord, isSelected) {
	selector.isMulti && selector.$useContiguousSelection && _ensureContiguousSelection(selector, targetRecord);
...
exports.RecordSelector = helpers.defineClass(RecordSelector, null, {
	...
	appendToGridHeader: function(row) {
		var col = this.list.cols.$recordSelector = this.list.appendCol();
		col.slot = row;
		col.titleCell.className = this.list.gridCss.title;
		var th = row.appendChild(col.titleCell);
		if (this.isMulti) {
			_addCheckBox(th, true).syraSelectAll = true;
		}
		col.colWidth = this._widthSelector || 25;
		return col;
	},
	clear: function() {
		this.records = {};
