...
exports.RecordSelector = helpers.defineClass(RecordSelector, null, {
	...
	select: function(uuid, selected, selectFirstByDefault) {
		if (uuid === undefined) {
			...
			var recordArticle = this.list.recordsMap[uuid];
			if (recordArticle) {
			...
			if (selectFirstByDefault && !recordArticle) {
				this.select(undefined, true);
				return;
			}
			if (this.list.selectorCardRecord) {
				this.list.selectorCardRecord.onSelect(uuid, selected);
			}
			if (this.list.treeDecorator) {
				this.list.treeDecorator.onNodeSelected(uuid, selected);
			}
