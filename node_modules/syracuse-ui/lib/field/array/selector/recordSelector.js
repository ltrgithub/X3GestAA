...
var _modes = {
	...
	button: function(selector) {
		...
		selector.appendToRecord = function(record, slot) {
			...
			slot.appendChild(syra_menus.addTextButton(this.text, "s-list-selector-btn", "onSelectorClick"));
		};
	},
	...
	selectRecord: function(record, selected) {
		if (record) {
			if (!this.isMulti && selected) {
				if (this.selectedRecord && this.selectedRecord.$uuid != record.$uuid) {
			...
			if (this.list) {
				if (selected && !this.list.$field.$isFusionNavigationList) {
					if (this.list.page && !this.list.page.isNavigationPage && (this.list.page != syra_site.landingPageMaster)) {
