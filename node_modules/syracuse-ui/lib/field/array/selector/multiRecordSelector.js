...
exports.MultiRecordSelector = helpers.defineClass(MultiRecordSelector, RecordSelector, {
    ...
    onclick: function(options){
        if (this.list.$item.$selectByRowIndex) {
            ...
            options.isSelected = options.selectorTarget.checked;
            options.uuidTarget = options.selectorTarget.className.indexOf("s-list-selector-all") >= 0 ? null : this.list.findRecordUiid(options.selectorTarget);
        }
    },
    onSelect: function(options){
        if (options.selectorTarget && options.selectorTarget.className.indexOf("s-list-selector-all") >= 0) {
            this.records = {};
            var uuids = Object.keys(this.list.recordsMap);
            for (var ii = 0, jj = uuids.length; ii < jj; ii++) {
                if (!options.unselectable ||
                !options.unselectable(this.list, {
                    "uuid": uuids[ii]
                })) {
