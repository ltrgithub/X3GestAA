...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	initializeList: function() {
		...
		// Auto insert fields in authored list... just for Fusion for now
		if (this.page.isFusionPage) {
			if (!this.discardFieldsGarbage)
				(this._autoInsertFields = new FieldsAutoInsert()).load(this, "field");
		}
		...
		if (this.page.$autoFetch) {
			var params = this.page.openerUrlSegments.params;
			if (params) {
				if (params.where) {
	...
	_endDrawBox: function() {
		...
		if (!this.$isQuickDesignerDisabled) {
			this.applyActionLinkChange({
				$links: {
					$design: {
						$title: this.hasGraph() ? this.localize.flDesignGraph : this.localize.flDesign
					}
				}
			});
		}
