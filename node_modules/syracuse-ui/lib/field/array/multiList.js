...
var recordSelector = require('./selector/recordSelector');
...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	onItemInOut: function(onEnter, event, target) {
		this.builder && this.builder.onItemInOut(onEnter, event, target);
		this.selector && this.selector.highlightOnEnter(this, onEnter, event, target);
	},
	setFocusRecord: function(record) {
		this.page.setActivateList(this);
		if (this.focusedRecord && this.focusedRecord != record) {
			syra_site.dom.toggleClass(this.focusedRecord.domItem, "s-record-focused", false);
			this.focusedRecord.freezeRow && syra_site.dom.toggleClass(this.focusedRecord.freezeRow, "s-record-focused", false);
		}
		this.focusedRecord = record;
		syra_site.dom.toggleClass(record.domItem, "s-record-focused", true);
		record.freezeRow && syra_site.dom.toggleClass(record.freezeRow, "s-record-focused", true);
	},
	getFocusRecord: function(selected) {
		if (this.focusedRecord && !this.focusedRecord.disposed) {
	...
	initializeList: function() {
		...
		this.builder && this.builder.endInitiallize();
	},
	showRecordSelection: function(record, selected) {
		syra_site.dom.toggleClass(this.treeDecorator ? (record.treeNode.desc || record.treeNode.item) : record.domItem, "s-list-record-selected", selected);
		record.freezeRow && syra_site.dom.toggleClass(record.freezeRow, "s-list-record-selected", selected);
		selected && this.setFocusRecord(record);

		var cardDecorator = this.builder && this.builder.cardDecorator;
		cardDecorator && cardDecorator.onShowRecordSelection(record, selected);

		record.checkBox && syra_fields.setCheckBockValue(record, selected);

		if (selected && !this.selector.isMulti) {
			syra_page.scrollToItem(this.page, record);
		}
	},
	resizeArticle: function(resize) {
		if (!this.disposed) {
			...
			if (mustBeResized && this.articleParent.isChildItemDisplay(this)) {
				this.builder && this.builder.resizeScrollView && this.builder.resizeScrollView();
	...
	loadSelector: function() {
		...
		this.selector = new recordSelector.RecordSelector();
	...
	drawBox: function() {
		...
		this.builder && this.builder.onAfterDrawList();
	},
	...
	ensureBottomBar: function() {
		if (!this.bottombar) {
			...
			this._dataValue.appendChild(this.bottombar);
		}
	...
	onSelectorClick: function(event, btn) {
		recordSelector.onSelectorClick(this, event);
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		this.builder && this.builder.applyDesignMetaData(metaData, designing);
	},
	notifyActionChange: function(menuItem, target, value, notifyServer) {
		...
		if (value.$create) {
			...
			if ((this.pagging.$startIndex + (this.pagging.$itemsPerPage - 1)) > options.$serverIndex) {
				var newRecord = this.addRecord(options);
			}
			//sendbag begin
	...
	setFocus: function(select, name, $serverIndex) {
		if (this.builder && this.builder.onBeforeListSetFocus) {
			this.builder.onBeforeListSetFocus(select, name, $serverIndex);
		}
		...
		if (field) {
			...
			if (this.isClientFetch && !this.findRecordByServerIndex($serverIndex)) {
				...
				if (field) {
					var record = this.findRecordByServerIndex($serverIndex);
					this.selector.selectRecord(record.$uuid, true);
					return field.focus(select);
				}
			}
		}
	...
	getField: function(name, $serverIndex, select) {
		...
		if (record && !record.disposed) {
			if ((fields = record.boundFields[name]) && fields.length > 0) {
				return (this.builder && this.builder.popupCardBuilder && this.builder.popupCardBuilder.findField(name, record)) || fields[0];
			}
		}
		return null;
	},
	...
	dispose: function() {
		this.builder && this.builder.dispose(this);
		if (this.page) {
		...
		this.builder = this.body = this.focusedRecord = this.capabilityMaker = this.garbage = this.selector = this.treeDecorator = this.graphDecorator = this.selectorCardRecord = this._autoInsertFields = null;
