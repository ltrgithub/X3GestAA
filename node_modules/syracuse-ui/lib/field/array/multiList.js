...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	initializeList: function() {
		this.$userPreferences = this.page.getFieldUserPreferences(this.$item.$bind);
	...
	saveUserPreferences: function(key) {
		...
		switch (key) {
			case "$itemsPerPage":
				this.$userPreferences.$itemsPerPage = this.pagging.getItemsPerPage();
				break;
		}
	...
	selectServerFilter: function(filterId) {
		if (this.serverFilters) {
			if (this.serverFilters.selected) {
				document.site.dom.toggleClass(this.serverFilters.selected.domItem, "s-selected", false);
			...
			document.site.dom.toggleClass(this.serverFilters.selected.domItem, "s-selected", true);
		}
	},
	...
	appendCardGraphSlots: function() {
		...
		if (!$cardItem && !this.$item.$graphPosition) {
			if (this.tableRow) {
				document.site.dom.moveChildNodes(this.tableRow, this._core);
		...
		if ($cardItem || this.$item.$graphPosition) {
			...
			if (hasRow) {
				if (!this.tableRow) {
					...
					document.site.dom.moveChildNodes(this._core, this.tableRow);
	...
	onNotifyDataChange: function(field, value) {
		if (field.$item.$bind == "$itemsPerPage") {
			...
			this.saveListDesign(false, this.saveUserPreferences(field.$item.$bind));
			this.triggerFetch();
		}
		return false;
	},
