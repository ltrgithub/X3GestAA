...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	resizeList: function(resize) {
		...
		if (mustBeResized && this.page.isChildItemDisplay(this)) {
			DesktopList.prototype.resizeList.call(this, resize);
				...
				if (this._hasVisibleFilters()) {
					if (this.page.menuBar) {
						...
						for (var ii = 0, jj = filtersIds.length; ii < jj; ii++) {
							var $filter = this.page.$prototype.$filters[filtersIds[ii]];
							if (!$filter.$isHidden && !$filter.$isMandatory) {
								allowed[filtersIds[ii]] = $filter;
							}
						}
						this.serverFilters.menus._ensurePrivateMenus({
							$filters: allowed
	...
	selectServerFilter: function(filterId) {
		if (this.serverFilters) {
			if (this.serverFilters.selected) {
				syra_site.dom.toggleClass(this.serverFilters.selected.domItem, "s-selected", false);
			...
			syra_site.dom.toggleClass(this.serverFilters.selected.domItem, "s-selected", true);
		}
	},
	...
	_hasVisibleFilters: function() {
		var self = this;
		var filtersIds = Object.keys(this.page.$prototype.$filters);
		if (filtersIds.length === 0) { // Display "No filters" if no filter is available but $filters section is in proto
			return true;
		}
		var allowed = false;
		// Return only true if there are filters and at least one is visible
		return filtersIds.some(function(filterId) {
			var $filter = self.page.$prototype.$filters[filterId];
			return (!$filter.$isHidden && !$filter.$isMandatory);
		});
	},
	_endDrawBox: function() {
		...
		if (!this.$isQuickDesignerDisabled) {
			this.applyActionLinkChange({
				$links: {
					$design: {
						$title: this.hasGraphDesigner() ? this.localize.flDesignGraph : this.localize.flDesign
					}
				}
			});
		}
	...
	onPagerClick: function(picker) {
		this.pagging.onPageClick(picker);
	},
	onSearchClick: function(picker, event) {
		this.searchCapability.onSearchClick(picker.syraValue, event);
	},
	onSortClick: function(picker, event) {
		this.sortCapability.onSortClick(picker.syraValue, event);
	},
	onSortAlphaTab: function(picker, event) {
		this.sortCapability.onSortAlphaTab(picker);
	},
	onRowCardClick: function(picker, event) {
		this.cardDecorator.onRowCardOpenerClick(picker);
	},
	onSelectorClick: function(picker, event) {
	...
	applyShortCuts: function(shortcuts, event, focusField) {
		if (shortcuts.pagedown || shortcuts.pageup) {
			...
			if (this.pagging && this.pagging.onShortKeyEvent) {
		...
		if (shortcuts.enter) {
			if (focusField && focusField.$item.$bind === "$search" && this.searchCapability) {
				this.searchCapability.onSearchClick("$searchNext", event);
				return true;
			}
		}
		return false;
	...
	onNotifyDataChange: function(field, value) {
		if (field.$item.$bind == "$itemsPerPage") {
			...
			this.saveListDesign(false, preferencesHelper.save(this, field.$item.$bind));
			this.triggerFetch();
		}
		return false;
	},
	dispose: function() {
		...
		this.sortCapability && this.sortCapability.dispose();
		this.pagging && this.pagging.dispose();
		...
		this.garbage = this.selector = this.pagging = this.sortCapability = this.treeDecorator = this.graphDecorator = this.selectorCardRecord = this._autoInsertFields = null;
