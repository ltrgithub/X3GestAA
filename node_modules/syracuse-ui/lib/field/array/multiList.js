...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	highlightSelection: function(record, selected) {
		syra_site.dom.toggleClass(this.treeDecorator ? record.treeNode.item : record.domItem, "s-list-record-selected", selected);
		selected && this.setFocusRecord(record);

		var cardDecorator = this.cardDecorator;
		if (cardDecorator) {
			if (cardDecorator.isRowCardMode && record.rowCardRow) {
				syra_site.dom.toggleClass(record.rowCardRow, "s-list-record-selected", selected);
			}
			if (selected) {
				this.isGridList && syra_grid.popupCardBuilder.onSelectRecord(record, true);
				if (cardDecorator.isOutCardMode) {
					syra_site.dom.empty(cardDecorator.outCardSlot);
					cardDecorator._renderRecordCard(record, cardDecorator.outCardSlot);
				}
			}
		}
		...
		if (selected && !this.selector.isMulti) {
			this.scroller && this.scroller.scrollToItem(record.domItem);
		}
	},
	...
	setFocus: function(select, name, $serverIndex) {
		if (this.treeDecorator) {
			...
			record && this.selector.select && this.selector.select(record.$uuid, true);
			return true;
		}
		...
		if (field) {
			...
			if (this.isClientFetch && !this.findRecordByServerIndex($serverIndex)) {
				...
				if (field) {
					var record = this.findRecordByServerIndex($serverIndex);
					record && this.selector.select(record.$uuid, true);
					return field.focus(select);
				}
			}
		}
	...
	getField: function(name, $serverIndex, select) {
		...
		if (record && !record.disposed) {
			if ((fields = record.boundFields[name]) && fields.length > 0) {
				return syra_grid.popupCardBuilder.findField(name, record) || fields[0];
			}
		}
		return null;
	},
