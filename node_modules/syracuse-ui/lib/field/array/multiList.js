...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	showRecordSelection: function(record, selected) {
		...
		if (record._selectorCheck) {
			record._selectorCheck.checked = selected;
		}
	...
	_endDrawBox: function() {
		...
		if (!this.$isQuickDesignerDisabled) {
			syra_menus.setMenus(this, {
				$links: {
					$design: {
						$title: this.hasGraph() ? syra_local.flDesignGraph : syra_local.flDesign
					}
				}
			});
		}
	...
	fillList: function(dataRecordSet, parentDataRecord, isDelta) {
		...
		this.treeDecorator && this.treeDecorator.endFill();
		this.pagging.draw();
	...
	setFocus: function(select, name, $serverIndex) {
		if (this.builder && this.builder.popupCardBuilder.isOpened(this) && this.selector.uuid != this.clientDataset[$serverIndex].$uuid) {
	...
	dispose: function() {
		...
		this.bodyInsert = this.focusedRecord = this.capabilityMaker = this.garbage = this.selector = this.treeDecorator = this.graphDecorator = this.selectorCardRecord = this._autoInsertFields = null;
