...
exports.MultiList = helpers.defineClass(MultiList, DesktopList, {
	...
	ensureArticleId: function() {
		DesktopList.prototype.ensureArticleId.call(this);
		if (this.bottombar) {
	...
	applyShortCuts: function(shortcuts, event, focusField) {
		if (shortcuts.pagedown || shortcuts.pageup) {
		...
		} else if (shortcuts.left || shortcuts.right) {
			if (this.treeDecorator) {
				var record;
			...
			return false;
		} else if (shortcuts.up || shortcuts.down) {
			if (this.treeDecorator) {
				...
				var articleParent;
				if ((articleParent = focusField.articleParent) && articleParent.popupCardItem && articleParent.menuItems) {
					articleParent.menuItems[(shortcuts.up ? "$previous" : "$next")][0].click();
					return true;
				}
			}
		} else
