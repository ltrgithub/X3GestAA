...
exports.TreeList = helpers.defineClass(TreeList, null, {
	buildTreeMap: function(dataset) {
		...
		for (var ii = 0, jj = dataset.length; ii < jj; ii++) {
			...
			var desc = dataRecord[$bindings.$description] || "";
			...
			if (node) {
				...
				parentNode.children.push(this._nodes[id] = node = {
					id: id,
					desc: desc,
					children: []
				});
			...
			node.isOpened = $bindings.$open ? (dataRecord[$bindings.$open] === true || dataRecord[$bindings.$open] === 2) : true;
	...
	dispose: function() {
		this.list = this.nodes = this.$visibleFields = null;
	}
});