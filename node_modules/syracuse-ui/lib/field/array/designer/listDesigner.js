...
exports.ListDesigner = helpers.defineClass(ListDesigner, Designer, {
	...
	endArticleUpdate: function() {
		...
		if (this.designedArticle.page.designer) {
			...
			this.designedArticle.resizeList(true);
			this.saveDesign();
		}
	},
	saveDesign: function() {
		...
		for (var ii = 0, jj = modeTypes.length; ii < jj; ii++) {
			...
			switch (modeType) {
				...
				case "cards":
					...
					link.title = link.textContent = this.localize.lw_designCard;
				...
				case "detailgrid":
					...
					var link = document.createElement("a");
					link.syraOnClick = "onDesignCardClick";
					link.title = link.textContent = this.localize.lw_designDetailView;
					link.className = "s-aw-list-design-card-picker";
					body.appendChild(link);
					this.addSwitchPagerLink(body);
					break;
		...
		if (newData) {
			...
			if (newData.$format) {
				...
				for (var ii = 0, jj = types.length; ii < jj; ii++) {
					...
					document.site.dom.toggleClass(mode.link, "s-selected", isSelected);
