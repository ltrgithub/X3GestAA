...
function _ensureDesignMenuSlotVisibility(list) {
	if (list.menusSlot) {
		list.menusSlot.style.display = (list.designBtn || list.designer) ? "none" : "";
	}
	list.designBtn && syra_site.dom.disableItem(list.designBtn, list.designer);
}

exports.toggleUIDesign = function(list, enable, disposingDesigner) {
	if (enable) {
		if (!list.designBtn) {
			var slot = document.createElement("div");
			slot.className = "s-list-designer-btn-slot";
			list.designBtn = syra_menus.addIconButton(syra_local.flDesign, "s-list-designer-btn", "onDesignOpenerClick");
			list.designBtn.syraItem = list.id;
			slot.appendChild(list.designBtn);
			var bar = list.ensureTopBar();
			bar.insertBefore(slot, bar.firstChild);
		}
	} else {
		if (disposingDesigner) {
			list.designArticle(false);
		}
		if (list.designBtn) {
			syra_site.dom.removeChild(list.designBtn.parentNode);
			list.designBtn = null;
		}
	}
	_ensureDesignMenuSlotVisibility(list);
	list.filler.toggleFakeRecord(list, enable);
};

exports.design = function(list, open) {
	if (open) {
		if (list.graphDecorator) {
			list.graphDecorator.ensureGraph(true);
		}
		list.ensureGrapDecorator();
		list.designer = new ListDesigner();
	...
	if (list.page.isVignettePage && list.articleParent == list.page) {
		list.page.vignetteField.onDesignVignette(open);
	}
};

exports.reloadLayout = function(list, $item) {
	$item = $item || list.$item;
	var newField = list.layoutParent.loadChildItem(null, $item, list.layoutParent.items.indexOf(list));
	var isDesigner = list.designer;
	newField.$designing = list.$designing;
	list.articleParent.unbind(list);
	list.layoutParent.removeItem(list, true);
	...
	isDesigner && newField.designArticle(true);
	return newField;
};
...
helpers.defineClass(ListDesigner, Designer, {
			...
			$properties: {
				$cardPosition: {
					...
					$value: {
						...
						$enum: [{
							$value: "row",
							$title: syra_local.lw_cardPositionRow
						}, {
							$value: "top",
							$title: syra_local.lw_cardPositionTop
						}, {
							$value: "left",
							$title: syra_local.lw_cardPositionLeft
						}, {
							$value: "right",
							$title: syra_local.lw_cardPositionRight
						}, {
							$value: "bottom",
							$title: syra_local.lw_cardPositionBottom
						}]
					}
				},
				$graphPosition: {
					...
					$title: syra_local.lw_graphPosition,
					$value: {
						$type: "application/x-string",

						$enum: [{
							$value: "top",
							$title: syra_local.lw_graphPositionTop
						}, {
							$value: "left",
							$title: syra_local.lw_graphPositionLeft
						}, {
							$value: "right",
							$title: syra_local.lw_graphPositionRight
						}, {
							$value: "bottom",
							$title: syra_local.lw_graphPositionBottom
						}, {
							$value: "front",
							$title: syra_local.lw_graphPositionFront
						}, {
							$value: "behind",
							$title: syra_local.lw_graphPositionBehind
						}]
					}
				}
			}
	...
	drawBox: function() {
		...
		if (this.designedArticle.hasGraphDesigner()) {
	...
	onSwitchPagerClick: function(event, btn) {
		this.designedArticle.pagging.onPageClick(event, btn);
		this._setSwitchPagerBtnsTitle();
	},
	onDesignCardClick: function() {
		this.designCardItem(true, this.getDesignedCardItem());
	},
	onDesignGridCardClick: function() {
		this.designCardItem(true, this.getDesignedCardItem());
	},
	onModeClick: function(event, btn) {
		var designer = this;
		var metaData = {};
		switch (btn.syraMode) {
			case "cards":
		...
		if (metaData.$format !== undefined && metaData.$format != list.$item.$format) {
			...
			var newList = exports.reloadLayout(list);
	...
	onTemplateClick: function(event, btn) {
		if (this.templatesTool) {
			if (btn.syraModelType == "colcount") {
				this.designedArticle.applyDesignMetaData({
					$cardsByRowCount: btn.$syraColCount
				}, true);
				this.templatesTool.selectColChoice(this.sectionCards, this.designedArticle, btn.$syraColCount);
				this.endArticleUpdate();
			} else {
				Designer.prototype.onTemplateClick.call(this, event, btn);
			}
	...
	getDesignedCardItem: function() {
		...
		if (this.designedArticle.$item.$format == "grid") {
			if (this.designedArticle.$item.$cardItem) {
				if (this.designedArticle.builder.cardDecorator.isRowCardMode) {
					this.designedArticle.builder.cardDecorator.toggleRowCard(record, true);
					cardItem = record;
				} else {
	...
	_addSwitchPagerBtn: function(slot) {
		if (this.designedArticle.isClientFetch) {
			var btn = syra_menus.addTextButton("", "s-aw-list-pager-btn", "onSwitchPagerClick");
			btn.syraIsDesign = true;
			slot.appendChild(btn);
			(this._switchPagerBtns = this._switchPagerBtns || []).push(btn);
		}
	},
	_setSwitchPagerBtnsTitle: function() {
		if (this._switchPagerBtns) {
			...
			for (var ii = 0, jj = this._switchPagerBtns.length; ii < jj; ii++) {
				syra_menus.setButtonTitle(this._switchPagerBtns[ii], title);
			}
	...
	appendGroupModes: function() {
		...
		title.textContent = syra_local.lw_presentation;
		var slot = document.createElement("div");
		...
		for (var ii = 0, jj = modeTypes.length; ii < jj; ii++) {
			...
			var mode = {
				modeType: modeType,
				link: syra_menus.addIconButton(syra_local["lw_mode_" + modeType], "s-aw-list-mode-" + modeType, "onModeClick"),
				box: document.createElement("div")
			};
			mode.box.className = "s-aw-list-mode";
			var title = document.createElement("div");
			title.className = "s-aw-list-title";
			title.textContent = syra_local["lw_mode_" + modeType + "_title"];
			...
			switch (modeType) {
				...
				case "cards":
					...
					this.templatesTool.addColsCountButtons(this.sectionCards, this.designedItem, this.designedArticle.builder.$cardsByRowCount || 1, 6);
					body.appendChild(syra_menus.addTextButton(syra_local.lw_designCard, "s-aw-list-design-card-picker", "onDesignCardClick"));
					this._addSwitchPagerBtn(body);
					break;
				case "detailgrid":
					var slot = document.createElement("div");
					...
					body.appendChild(syra_menus.addTextButton(syra_local.lw_designDetailView, "s-aw-list-design-card-picker", "onDesignCardClick"));
					this._addSwitchPagerBtn(body);
					break;
			}
		}
		...
		if (newData) {
			...
			if (newData.$format) {
				...
				for (var ii = 0, jj = types.length; ii < jj; ii++) {
					...
					syra_site.dom.toggleClass(mode.link, "s-selected", isSelected);
	...
	loadGraphDesigner: function() {
		...
		title.textContent = syra_local.lw_graphPosition;
		slot.appendChild(title);
