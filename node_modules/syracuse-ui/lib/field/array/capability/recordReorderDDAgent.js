...
exports.RecordReorderDDAgent = helpers.defineClass(RecordReorderDDAgent, null, {
	...
	bindEvent: function(bind) {
		...
		if (bind) {
			$(self.boundaryItem).delegate("[data-s-record]", "mousemove" + nameSpace, function(event) {
			...
			$(self.boundaryItem).undelegate(nameSpace);
		}
	},
	...
	_calculateDropCuePosition: function(event) {
		if (this.list.onRecordReorderAgentCalculateDragOffset) {
			...
			this.$drag = document.site.dom.getBoundingClientRect(this.targetRecord.dataRow || this.targetRecord.domItem);
