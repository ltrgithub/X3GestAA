...
exports.RecordReorderDDAgent = helpers.defineClass(RecordReorderDDAgent, null, {
	start: function(dropItem) {
		...
		this.list = this.builder.list;
		this.boundary = dropItem.builder.body.getBoundingClientRect();
		this.bindEvent(true);
	},
	bindEvent: function(bind) {
		...
		if (bind) {
			self.list.$$item.delegate("[data-s-record]", "mousemove" + nameSpace, function(event) {
			...
			self.list.$$item.undelegate(nameSpace);
		}
	},
	...
	onDragMouseUp: function(target, event) {
		if (this.targetRecord) {
			...
			this.list.records.splice(this.reorderRecord.$recordIndex, 1);
			...
			this.list.records.splice(this.reorderRecord.$recordIndex, 0, this.reorderRecord);
	...
	_calculateDropCuePosition: function(event) {
		...
		if (this.targetRecord.builder.scroller) {
			this.$drag.width = this.targetRecord.builder.scroller.getVisibleSize().width;
		}
	...
	dispose: function() {
		if (this.list) {
			...
			this.list = this.builder = this.reorderRecord = this.boundary = this._info = this._dropCue = this.$drag = null;
		}
	}
});