...
function _applyDefaultReorder(record) {
	if (record.hasReorderCapability) {
		if (!record.reorderBtn) {
			...
			if (record.list.$isDisabled) {
				syra_site.dom.disableItem(record.reorderBtn, record.list.$isDisabled);
			}
		}
	} else {
		if (record.reorderBtn) {
			syra_site.dom.removeChild(record.reorderBtn);
			delete record.reorderBtn;
		}
	}
}

function _applyGridReorder(record) {
	if (record.hasReorderCapability) {
		record.reorderBtn = record.rowIndexBtn;
		record.reorderBtn.syraReorderRecordId = record.list.id;
		syra_site.dom.toggleClass(record.rowIndexBtn, "s-grid-record-reorder", true);
		record.rowIndexBtn.syraDragSpot = record.id;
	} else {
		if (record.reorderBtn) {
			delete record.reorderBtn.syraReorderRecordId;
			delete record.reorderBtn;
			syra_site.dom.toggleClass(record.rowIndexBtn, "s-grid-record-reorder", false);
			delete record.rowIndexBtn.syraDragSpot;
		}
	}
}
...
exports.addReorder = function(record, reorder) {
	if (!record.isReorderDisabled) {
		record.hasReorderCapability = reorder;
		if (record.isGridRecord) {
			_applyGridReorder(record, reorder);
		} else {
			_applyDefaultReorder(record, reorder);
		}
	}
};
...
function _addAction(list, $actions, name) {
	...
	var $keys = list.$capability[name].$variantItemKeys;
	if ($keys) {
		$newAction.$variants = {};
...
exports.applyToRecord = function(record, onListMetadata) {
	...
	if (onListMetadata) {
		//on metadata
		if (record.hasReorderCapability != record.$capability.reorder) {
			exports.addReorder(record, record.$capability.reorder);
		}
	}
};
...
exports.apply = function(list, $meta) {
	...
	if ($prev) {
		...
		if ($prev.reorder && !list.$capability.reorder) {
			if (list.showRowIndexCol) {
				list.showRowIndexCol();
			}
		}
	}
	...
	if (list.$capability.reorder) {
		list.$item.$isRowIndexVisible = true;
		list.showRowIndexCol && list.showRowIndexCol();
	}
