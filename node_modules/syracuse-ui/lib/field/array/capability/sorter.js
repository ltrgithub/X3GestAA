...
function _resetSort(list) {
	if (list.builder.sortedCols) {
		for (var ii = 0, jj = list.builder.sortedCols.length; ii < jj; ii++) {
			var col = list.builder.columnsMap[list.builder.sortedCols[ii]];
			if (col && col.sortLink) {
				delete col.sortLink.syraTip;
				syra_site.dom.toggleClass(col.sortLink, " s-sort-desc", false);
				syra_site.dom.toggleClass(col.sortLink, " s-sort-asc", false);
			}
		}
		delete list.builder.sortedCols;
	}
}
...
exports.setFetchParams = function(list, params, options) {
	...
	params.orderBy = list._$orderBy ? _helpers.url.formatOrderBy(list._$orderBy) : null;
};
...
exports.dispose = function(list) {
	list.builder.sortedCols = list._$orderBy = null;
};
...
exports.onSortClick = function(list, $bind, event) {
	list.page.externalAdapter.onSortClickEvent({
		...
		doEvent: function() {
			...
			list.filler.trigger(list, $newOrderBy ? {
				orderBy: $newOrderBy
			} : null);
		}
	});
	...
	if (list._$orderBy) {
		...
		for (var ii = 0, jj = list._$orderBy.length; ii < jj; ii++) {
			...
			if (cols) {
				...
				if (col && col.sortLink) {
					col.sortLink.syraTip = 2;
					syra_site.dom.toggleClass(col.sortLink, criterion.descending ? " s-sort-desc" : " s-sort-asc", true);
					list.builder.sortedCols.push(criterion.binding);
				}
			}
		}
	}
	var isSortedCols = list.builder.sortedCols && (list.builder.sortedCols.length > 0);
