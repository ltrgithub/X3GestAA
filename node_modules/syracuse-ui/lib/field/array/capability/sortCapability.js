		...
		var options = {
			...
			doEvent: function() {
				...
				if ($field.$type == "application/x-reference") {
					if ($field.$item.$value) {
						var refBind = document.site.expressionParser.extractCode($field.$item.$value);
						if (refBind && $field.$item[refBind]) {
							refBind = document.site.expressionParser.extractCode($field.$item[refBind]);
	...
	findSortableReference: function($bind) {
		...
		for (var ii = 0, jj = $fields.length; ii < jj; ii++) {
			...
			if ($field.$type == "application/x-reference" && $field.$item && $field.$item.$value) {
				var refBind = expressionParser.extractCode($field.$item.$value);
				if (refBind && $field.$item[refBind]) {
					refBind = expressionParser.extractCode($field.$item[refBind]);
