...
exports.SortCapability = helpers.defineClass(SortCapability, null, {
    ...
    resetSort: function(){
        if (this._sortedCols) {
            for (var ii = 0, jj = this._sortedCols.length; ii < jj; ii++) {
                ...
                if (col && col.sortLink) {
                    col.sortLink.className = this.list.$skin + "-title-link";
                }
            }
    ...
    renderOrderBy: function($bind){
        ...
        if (this._$orderBy) {
            this._sortedCols = [];
			var css = this.list.$skin + "-title-link";
    ...
    syracuseSort: function(resource, $orderBy, property){
        this.sort(resource, (resource.$orderBy = ($orderBy || [{binding:"$serverIndex",descending:false,$type:"application/x-integer"}])).map(function(criterion){
            return {
