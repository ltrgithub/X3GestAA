...
exports.SortCapability = helpers.defineClass(SortCapability, null, {
	...
	appendSortLink: function(col, $item) {
		...
		title.setAttribute("data-s-picker", "list-sort");
		...
		var options = {
			...
			doEvent: function() {
				...
				if ($field.$type == "application/x-reference") {
					if ($field.$item.$value) {
						var refBind = document.site.expressionMaker.extractCode($field.$item.$value);
						if (refBind && $field.$item[refBind]) {
							refBind = document.site.expressionMaker.extractCode($field.$item[refBind]);
	...
	resetSort: function() {
		if (this._sortedCols) {
			for (var ii = 0, jj = this._sortedCols.length; ii < jj; ii++) {
				...
				if (col && col.sortLink) {
					document.site.dom.toggleClass(col.sortLink, " s-grid-sort-desc", false);
					document.site.dom.toggleClass(col.sortLink, " s-grid-sort-asc", false);
				}
			}
		...
		if (this._$orderBy) {
			...
			for (var ii = 0, jj = this._$orderBy.length; ii < jj; ii++) {
				...
				if (this.list.cols) {
					...
					if (col && col.sortLink) {
						document.site.dom.toggleClass(col.sortLink, criterion.descending ? " s-grid-sort-desc" : " s-grid-sort-asc", true);
		...
		if (show) {
			if (!self._alphaTab) {
				...
				for (var ii = "A".charCodeAt(0), end = "Z".charCodeAt(0); ii <= end; ii++) {
					...
					btn.setAttribute("data-s-picker", "list-alphatab");
