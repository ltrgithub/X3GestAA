...
exports.Pager = helpers.defineClass(Pager, null, {
	designItem: function(open) {
		if (this.list.isClientFetch) {
			if (open) {
				this._item.className = "s-pager s-aw-pager";
				this._setDesignHideLink();
			} else {
				this._item.className = "s-pager";
				if (this.designHideLink) {
					document.site.removeDomChild(this.designHideLink);
					delete this.designHideLink;
				}
			}
		}
	},
	_setDesignHideLink: function() {
		if (this.list.isClientFetch) {
			if (this.list.designer) {
				if (!this.designHideLink) {
					this.designHideLinkSlot = document.createElement("div");
					this.designHideLink = document.createElement("a");
					this.designHideLink.setAttribute("data-s-picker", "list-page");
					this.designHideLink.syraIsDesignEnable = this.designHideLink.syraIsDesign = true;
					this.designHideLinkSlot.appendChild(this.designHideLink);
				}
				if (this.list.$item.$isPagerHidden) {
					this.designHideLink.textContent = this.designHideLink.title = "show pager";
					this.designHideLink.className = "s-aw-pager-picker-show";
					this.designHideLinkSlot.className = "s-aw-pager-picker-show-slot";
					this._item.parentNode.insertBefore(this.designHideLinkSlot, this._item);
				} else {
					this._item.appendChild(this.designHideLinkSlot);
					this.designHideLink.textContent = "";
					this.designHideLink.title = "hide pager";
					this.designHideLinkSlot.className = "s-aw-pager-picker-hide-slot";
					this.designHideLink.className = "s-aw-pager-picker-hide";
				}
			}
		}
	},
	load: function(list, slot) {
		...
		if (!this.list.isClientPagingPerPageDisable && this.list.pagging.$totalResults !== undefined) {
			...
			range.style.display = this.size.style.display = this.list.pagging.$totalResults > _pageSizes[0] ? "" : "none";
	...
	_drawItemsPerPage: function() {
		if (this.list.pagging.getItemsPerPage()) {
			...
			this._itemsPerPageField = this.list.page.loadNewItem(this.size.appendChild(div), {
				$bind: "$itemsPerPage",
				$category: "field",
				$isEditMode: true,
			...
			this._item.appendChild(this.size);
			this._itemsPerPageField.setDataValue(this.list.pagging.getItemsPerPage());
		}
	},
	...
	_drawServerStep: function($step) {
		...
		var title = $step.$title.toLocaleLowerCase();
		...
		var step = self.list.page.loadNewItem(div, {
			...
			$bind: $step.$bind,
			$title: $step.$title,
			$skin: "s-direction-" + title + " s-pager-link"
	...
	dispose: function() {
		if (this.designHideLinkSlot) {
			document.site.removeDomChild(this.designHideLinkSlot);
		}
		this.size = this.designHideLinkSlot = this.designHideLink = this.list = this._serverSteps = this._item = this._itemsPerPageField = this.pages = null;
	}
});