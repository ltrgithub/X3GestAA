...
exports.Pager = helpers.defineClass(Pager, null, {
	load: function(list, slot) {
		...
		if (this.list.isClientFetch) {
			this.list.paging.$totalResults = this.clientTotalRecords || this.list.dataset.length;
		}
		if (!this.list.isClientPagingPerPageDisable && this.list.paging.$totalResults) {
			this.list.paging.ensureItemsPerPage();
			this._drawItemsPerPage();
			var pageTotal = Math.ceil(this.list.paging.$totalResults / this.list.paging.$itemsPerPage);
			var currentPageIndex = Math.ceil(this.list.paging.$startIndex / this.list.paging.$itemsPerPage);
			if (pageTotal <= 1) {
				range.textContent = this.list.paging.$totalResults + " " + localize.flPagerTotalRecords;
			} else {
				range.textContent = this.list.paging.$totalResults + " " + localize.flPagerTotalRecords;
	...
	_drawItemsPerPage: function() {
		if (this.list.paging.$itemsPerPage) {
			...
			this._itemsPerPageField.setDataValue(this.list.paging.$itemsPerPage);
		}
	},
