...
exports.Pager = helpers.defineClass(Pager, null, {
	...
	_setDesignHideLink: function() {
		if (this.isDesigned) {
			if (!this.designHideLink) {
				...
				this.designHideLink.syraIsDesign = true;
			}
			if (this.list.$item.$isPagerHidden) {
				...
				this._item.parentNode.insertBefore(this.designHideLink, this._item);
			} else {
				this._item.appendChild(this.designHideLink);
	...
	load: function(list, slot) {
		...
		var show;
		if (this.list.clientFetchOptions) {
			this.list.pagging.$totalResults = this.list.clientDataset.length;
		}
		if (!this.list.isClientPagingPerPageDisable && this.list.pagging.$totalResults !== undefined) {
			...
			this._drawItemsPerPage();
			if (this.list.isServerStepPager !== false) {
				this._drawServerStep({
					$bind: "$first",
	...
	dispose: function() {
		this.designHideLink = this.list = this._serverSteps = this._item = this._itemsPerPageField = this.pages = null;
	}
});