...
exports.Pager = helpers.defineClass(Pager, null, {
			...
			if (pageTotal > 1) {
				...
				if (!this.list.isClientFetch) {
					this._drawServerStep({
						$bind: "$first",
						$title: syra_local.flPagerFirst
					}, group);
					this._drawServerStep({
						$bind: "$previous",
						$title: syra_local.flPagerPrev
					}, group);
					this._drawServerStep({
						$bind: "$next",
						$title: syra_local.flPagerNext
					}, group);
					this._drawServerStep({
						$bind: "$last",
						$title: syra_local.flPagerLast
					}, group);
				}
			}
		} else {
	...
	_drawServerStep: function($step, group) {
		var self = this;
		var step = self.list.page.loadNewItem(group || self.domItem, {
			$icon: {
				$mode: "icon",
				$path: "page/s-list-"
			},
		...
		step.onMenuClick = function() {
			var $url = this.$url;
			if (self.list) {
				self.list.fetcher.trigger(self.list, null, {
					$url: $url
				});
			}
			return false;
	...
	clear: function() {
		...
		if (this._serverSteps) {
			for (var ii = 0, jj = this._serverSteps.length; ii < jj; ii++) {
				this.list.page.removeItem(this._serverSteps[ii], true, true);
			}
		}
