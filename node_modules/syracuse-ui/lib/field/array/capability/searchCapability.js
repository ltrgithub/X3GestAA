...
function _loadItem(list, slot) {
	...
	list.ensureTopBar().appendChild(searcher.domItem);
	...
	var btn = syra_menus.addIconButton("", "s-list-search-expend-btn", "onSearchClick");
	...
	var btn = syra_menus.addIconButton("Next", "s-list-search-next-btn", "onSearchClick");
	btn.syraValue = "$searchNext";
	searcher._itemSearchField.fieldValue.appendChild(btn);
	btn = syra_menus.addIconButton("Previous", "s-list-search-previous-btn", "onSearchClick");
...
function _onCollapseClick(searcher, picker) {
	if (searcher._fieldSlot) {
		if (searcher._fieldSlot.style.display == "none") {
			...
			searcher._fieldSlot.style.display = "none";
			picker.className = "s-list-search-expend-btn";
		}
	}
}
...
function _compare(value, datasetValue, colProperties) {
...
function _search(list, value, next, notFound) {
	var searcher = list.searcher;
	...
	for (var ii = startIndexes.row;
		...
		for (var jj = startIndexes.col;
			...
			(next ? jj++ : jj--)) {
			if (list.clientDataset[ii][searcher.orderCols[jj].$bind] && _compare(value, list.clientDataset[ii][searcher.orderCols[jj].$bind], searcher.orderCols[jj])) {
				exports.start(list, list.clientDataset[ii].$uuid, searcher.orderCols[jj].$bind, list.clientDataset[ii].$serverIndex);
	...
	if (!notFound) {
		...
		return _search(list, value, next, true);
	}
	searcher.start = {};
	return null;
}
...
function _ensureOrderCols(list) {
	...
	for (var ii = 0; ii < list.orderCols.length; ii++) {
		if (!list.orderCols[ii].$isHidden && list.orderCols[ii].$bind) {
			colProperties = {};
			colProperties.$bind = list.orderCols[ii].$bind;
			colProperties.$type = list.$fields[list.orderCols[ii].$bind].$type;
			if (list.$fields[list.orderCols[ii].$bind].$value && list.$fields[list.orderCols[ii].$bind].$value.$enum) {
				colProperties.$enum = list.$fields[list.orderCols[ii].$bind].$value.$enum;
			}
...
function _disposeItem(list, clear) {
	if (clear) {
		if (list.searcher._itemSearchField) {
			list.searcher.list.removeItem(list.searcher._itemSearchField);
