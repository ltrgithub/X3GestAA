...
exports.FilterCapability = helpers.defineClass(FilterCapability, null, {
    applyFilter: function(resource, where){
        ...
        resource.dataRecordSet = resource.dataRecordSet.filter(function(record){
            ...
            if (typeof(ff) == "object") {
                return ff.$title.search(new RegExp(exp.children[1].value.replace("%", "", "g"), "i")) >= 0;
            }
            else {
                if (typeof(exp.children[1].value) == "string") {
                    return ff.search(new RegExp(exp.children[1].value.replace("%", "", "g"), "i")) >= 0;
                }
                else {
                    // case of integer (application/x-choice)
                    return typeof(ff) == "number" && typeof(exp.children[1].value) && ff == exp.children[1].value;
                }
            }
    ...
    validateIsQuickFilter: function(){
        ...
        var isFilterCapability = Object.keys(self.list.$fieldProperties).some(function($bind){
            ...
            return (!$field.$isHidden && !$field.$isExcluded && $field.$capabilities && ($field.$capabilities.indexOf("filter") >= 0));
        });
    ...
    removeQuickFilter: function(){
        if (this.filterRecord) {
            if (this.list.helper) {
                this.list.helper.removeRecord(this.filterRecord, true);
            }
