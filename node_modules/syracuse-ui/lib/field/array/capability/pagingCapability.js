...
exports.PagingCapability = helpers.defineClass(PagingCapability, null, {
    ...
    showPagers: function(){
        if (this.pagers) {
            var show = !this.list.$item.$isPagerHidden;
            if (show) {
                show = !((this.list.$item.$graphMode == "both" && this.list.$item.$graphPosition == "front") ||
                (this.list.$item.$graphMode == "graph"));
            }
    ...
    onShortKeyEvent: function(isPageDown, isShift){
        if (this.pagers.length) {
            ...
            if (pager.pages.length > 0) {
                ...
                if (this.list.isClientFetchResult && this.list.records && this.list.records.length) {
                    /* aurelien
