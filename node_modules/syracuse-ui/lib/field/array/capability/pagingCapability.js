...
function _ensureBottomBar(list) {
	if (!list.bottombar) {
		list.bottombar = document.createElement("div");
		list.bottombar.className = "s-list-bottombar";
		list.setArticleId(list.bottombar);
		list._dataValue.appendChild(list.bottombar);
	}
	return list.bottombar;
}
	...
	draw: function(designing) {
		...
		if (this.list.isPagingDisabled !== true && this.list.$item.$isPagerHidden !== true) {
			...
			if (!this.pagerTopOnly && this.list.records && this.list.records.length > 20) {
				...
				this.pagerBottom.load(this.list, _ensureBottomBar(this.list));
			}
			this.showPagers(designing);
			this._last = {
