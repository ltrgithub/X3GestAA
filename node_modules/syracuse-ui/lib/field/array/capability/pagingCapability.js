...
exports.PagingCapability = helpers.defineClass(PagingCapability, null, {
	getParseIndexes: function(dataset) {
		var start = this.$startIndex || 1;
		var last = this.$itemsPerPage !== undefined ? (start + (this.$itemsPerPage - 1)) : dataset.length;
		return {
			start: start,
			last: Math.min(last, dataset.length)
		};
	},
	goToClientPage: function(options) {
		...
		var options = {
			...
			$totalResults: this.list.ensureClientDataset().length
		};
	...
	applyMetadata: function(metaData, designing) {
		...
		if (metaData.$itemsPerPage !== undefined) {
			this.list.$item.$itemsPerPage = this.$itemsPerPage = metaData.$itemsPerPage;
			if (designing) {
	...
	setPage: function($serverIndex) {
		var index = this.list.getClientDataSetIndex($serverIndex);
		var newPage = Math.ceil((index + 1) / this.$itemsPerPage);
		this.list.fetch({
			startIndex: ((newPage - 1) * this.$itemsPerPage) + 1
		});
	},
	draw: function() {
		...
		if (this.list.isPagingDisabled !== true && (this.list.designer || this.list.$item.$isPagerHidden !== true)) {
			this.pagers = [];
			var pager = new Pager();
