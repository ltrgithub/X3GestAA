...
exports.PagingCapability = helpers.defineClass(PagingCapability, null, {
	...
	draw: function(designing) {
		...
		if (this.list.isPagingDisabled !== true && this.list.$item.$isPagerHidden !== true) {
			...
			this.pagerTop.load(this.list, this.list.topbar);
	...
	onShortKeyEvent: function(isPageDown, isShift) {
		if (this.pagerTop) {
			if (this.pagerTop.pages.length > 0) {
				// looking for the next page
				var currentIndex = Math.ceil(this.$startIndex / this.getItemsPerPage());
				var goToPage;
				if (isPageDown) {
					goToPage = isShift ? (this.pagerTop.pages.length - 1) : (currentIndex + 1);
				} else {
					goToPage = isShift ? 1 : (currentIndex - 1);
				}
				if (goToPage !== undefined) {
					goToPage = Math.min(Math.max(1, goToPage), this.pagerTop.pages.length - 1);
					this.pagerTop.pages[goToPage].click();
				}
				...
				for (var ii = 0, jj = this.pagerTop._serverSteps.length; ii < jj; ii++) {
					if (this.pagerTop._serverSteps[ii].$bind === bind) {
						this.pagerTop._serverSteps[ii].click();
						break;
					}
				}
			}
	...
	_disposePagers: function(clear) {
		this.pagerTop && this.pagerTop.dispose(clear);
		this.pagerBottom && this.pagerBottom.dispose(clear);
		this.pagerTop = this.pagerBottom = null;
	},
	dispose: function() {
		this._disposePagers();
