...
exports.PagingCapability = helpers.defineClass(PagingCapability, null, {
    ...
    applyMetadata: function(metaData, onAuthoring){
        if (metaData.$isPagerHidden !== undefined) {
            var show = !(this.list.$item.$isPagerHidden = metaData.$isPagerHidden);
            if (this.pagers) {
    ...
    onPageClick: function(link){
        var page = parseInt(link.getAttribute("data-s-page"), 10);
    ...
    onShortKeyEvent: function(context){
        if (this.pagers.length) {
            ...
            if (context.isFusion) {
                ...
                if (context.isPageDown) {
                    goToPage = context.isShift ? (pager._pages.length - 1) : (currentIndex + 1);
                }
                else {
                    goToPage = context.isShift ? 1 : (currentIndex - 1)
                }
                if (goToPage !== undefined) {
                    goToPage = Math.min(Math.max(1, goToPage), pager._pages.length - 1);
                    pager._pages[goToPage].click();
                }
                ...
                var bind = context.isPageDown ? (context.isShift ? '$last' : '$next') : (context.isShift ? '$first' : '$previous');
    ...
    _disposePagers: function(clear){
        if (this.pagers) {
            for (var ii = 0, jj = this.pagers.length; ii < jj; ii++) {
                ...
                document.controller.disposeObject(this.pagers[ii]);
            }
            this.pagers = null;
        }
    },
    dispose: function(){
        this._disposePagers();
        this.list = this._item = this.pagers = null;
    }
});
