...
exports.Builder = helpers.defineClass(Builder, null, {
	...
	emptyBody: function(addEmptySlot, onFetch) {
		this.rows = [];
		syra_site.dom.empty(this.list.body);
		if (addEmptySlot) {
			this.list.emptyDataItem = document.createElement("div");
			this.list.emptyDataItem.className = this.list.$skin + "-empty-slot";
			this.list.emptyDataItem.textContent = (!this.list.isClientFetch && onFetch) ? syra_local.flWaitingData : (this.list.emptyMessage || this.list.$item.$noDataText || syra_local.flNoData);
			this.list.body.appendChild(this.list.emptyDataItem);
		}
	},
	onBeforeLoadRecord: function(record, initData, isCreateAction) {
		var rowIndex;
		...
		if (!record.row || (record.row.count == this.$cardsByRowCount)) {
			...
			if (this.list.treeDecorator) {
				...
				if (record.articleParent.$item && record.articleParent.$item.$isDrawOrderReversed) {
					this.list.body.insertBefore(record.row.slot, this.list.body.firstChild);
				} else {
	...
	drawRecord: function(record) {
		...
		if (this.list.treeDecorator) {
			...
			this.list.treeDecorator.appendCardNode(record, record.cardContent);
			if (this.list.selector.isRowMode) {
				...
				record.treeNode.item.setAttribute("data-s-record", record.$uuid);
			}
			...
			if (this.list.selector.useColumnSelector || this.list.$item.$isRowIndexVisible || (this.list.$capability && this.list.$capability.reorder) || this.list.$item.$selectByRowIndex || !this.list.$item.$isMenuRecordHidden) {
				this.list.$capability.reorder && this.list.capabilityMaker.addReorder(record);
			...
			if (this.list.selector.isRowMode) {
				...
				record.body.setAttribute("data-s-record", record.$uuid);
			}
			record.ensureLayoutMode();
	...
	onRecordEnsureLayoutMode: function(record) {
		if (record.toolbar) {
	...
	onRecordApplyChange: function() {

	},
	onItemInOutRecord: function(record, onEnter) {
		if (this.list.selector) {
			syra_site.dom.toggleClass(this.list.treeDecorator ? (record.treeNode.desc || record.treeNode.item) : record.domItem, "s-record-enter", onEnter);
		}

	},
	clearEmpty: function() {
		if (this.list.emptyDataItem) {
			syra_site.dom.removeChild(this.list.emptyDataItem);
			this.list.emptyDataItem = null;
		}
	},
	disposeRecord: function(record) {
		record.rowIndexBtn = record.toolbar = record.cardItem = record.treeNode = record.row = record.cardContent = record.menusSlot = record.rowIndexBtn = record.dataRow = null;
	},
	dispose: function() {
		this.rows = this.tableSlot = null;
	}
});