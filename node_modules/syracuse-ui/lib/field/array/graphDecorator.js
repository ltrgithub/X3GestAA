...
exports.GraphDecorator = helpers.defineClass(GraphDecorator, null, {
	...
	applyGraphSettings: function(list, metaData) {
		...
		if (list.$item.$graphPosition == "behind") {
			if (this._graphSlot) {
			...
			if (list.$item.$graphPosition == "front") {
				...
				this._toggleSwitchMenu(false);
				if (list.$item.$graphPosition == "left" || list.$item.$graphPosition == "right") {
					...
					this._graphSlot.style.width = widths[(list.$item.$graphPosition == "left") ? 0 : 1] + "%";
					list._core.style.width = widths[(list.$item.$graphPosition == "left") ? 1 : 0] + "%";
				} else {
					this._wrapper.style.display = "";
			...
			$(this._graphSlot)[(["left", "top"].indexOf(list.$item.$graphPosition) >= 0) ? "prependTo" : "appendTo"]($(this._wrapper));
		}
		if (this._chart) {
