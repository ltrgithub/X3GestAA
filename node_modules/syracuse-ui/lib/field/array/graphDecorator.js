...
exports.GraphDecorator = helpers.defineClass(GraphDecorator, null, {
	...
	ensureGraphVisibility: function() {
		...
		if (!this.list.page.isFusionPage) {
			($links = $links || {}).$pptslide = {
				...
				$url: this.list.page.$prototype.$representationUrl + "&count={recordsPerPage}" + "&pptMode=newSlide&$bind=" + this.list.$item.$bind,
		...
		this.list.tableSlot.style.display = (this.list.$item.$graphPosition == "front" && !(this.list.cardDecorator && this.list.cardDecorator.isOutCardMode)) ? "none" : "";
	...
	onListResize: function() {
		this.list.chart && this.list.chart.resizeArticle && this.list.chart.resizeArticle();
	},
	...
	appendSlot: function() {
		...
		if (!this.separator) {
			...
			document.site.ddManager.setDragSpot(this.separator, true);
		}
		if (!this.graphSlot) {
	...
	removeSlot: function() {
		document.site.removeDomChild(this.separator);
