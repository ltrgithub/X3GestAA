...
exports.GraphDecorator = helpers.defineClass(GraphDecorator, null, {
    _selectTab: function(){
        ...
        this._listTab.className = showGraph ? "s-list-graph-tab" : "s-list-graph-tab-open";
        this._graphTab.className = showGraph ? "s-list-graph-tab-open" : "s-list-graph-tab";
        
    },
    _appendGraphModeTabs: function(){
        var self = this;
        $(self._listTab = document.createElement("a")).text(self.list._localize.gd_graphTabDataList);
        self._listTab.setAttribute("data-s-tab", "list");
        
        $(self._graphTab = document.createElement("a")).text(self.list._localize.gd_graphTabDataGraph);
    ...
    applyGraphSettings: function(list, metaData){
        ...
        if (list.$item.$graphMode != "list") {
            if (!this._graphSlot) {
                ...
                list.chart = list.page.loadNewItem($(this._graphSlot), {
        ...
        if (list.chart) {
            list.chart.applyDesignMetaData(metaData);
        }
    },
    onWindowResize: function(initialize){
        this.list.chart && this.list.chart.onWindowResize && this.list.chart.onWindowResize(initialize);
    },
    dispose: function(){
        if(this.list.chart) {
            delete this.list.chart.attachedField;
            delete this.list.chart;
        }
        delete this.list;
        if (this._graphTabs) {
