...
exports.CardRecord = helpers.defineClass(CardRecord, RecordArticle, {
	...
	drawBox: function() {
		...
		this.reorderBtnParent = this.cardContent = document.createElement("div");
		...
		if (this.list.treeDecorator) {
			...
			if (this.list.selector.isRowMode) {
				this.treeNode.item.syraOnClick = "onSelectorClick";
				this.treeNode.item.className += " s-list-selector-row";
				this.treeNode.item.syrainout = this.id;
			...
			if (this.list.selector.useColumnSelector || this.list.$item.$isRowIndexVisible || this.list.$item.$selectByRowIndex || !this.list.$item.$isMenuRecordHidden) {
				...
				this.rowIndexBtn = document.createElement("div");
				this.toolbar.appendChild(this.rowIndexBtn);
				this.renderRowIndex();
			}
	...
	ensureLayoutMode: function() {
		...
		if (this.rowIndexBtn) {
			this.rowIndexBtn.className = this.$skin + "-row-index";
	...
	renderAltStyle: function(showAlt) {
		syra_site.dom.toggleClass(this.domItem, "s-list-row-alt", showAlt);
	},
	onSelectMouseEvent: function(onEnter) {
		syra_site.dom.toggleClass(this.list.treeDecorator ? this.treeNode.item : this.domItem, "s-record-over", onEnter);
	},
	highlightSelection: function(selected) {
		syra_site.dom.toggleClass(this.list.treeDecorator ? this.treeNode.item : this.domItem, "s-list-record-selected", selected);
		if (this._selectorCheck) {
			this._selectorCheck.checked = selected;
		}
	},
	dispose: function() {
		this.toolbar = this.cardItem = this.treeNode = this.row = this.cardContent = this._selectorCheck = this.menusSlot = this.rowIndexBtn = this.dataRow = null;
