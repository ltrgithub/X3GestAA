...
exports.CardRecord = helpers.defineClass(CardRecord, RecordArticle, {
    ...
    toggleDeletedStatus: function($isDeleted){
        document.site.toggleClass(this.$$item[0], "s-cards-record-deleted", $isDeleted);
    },
    notifyReorder: function(){
        ...
        var rowOffset = 0;
        self.builder.list._store._records.forEach(function(record, index){
    ...
    drawBox: function(){
        ...
        if (this.list._store.selector.appendToRecord) {
            this._selectorCheck = this.list._store.selector.appendToRecord(selectorSlot);
        }
        else {
        ...
        if (this.list.$item.$isRowIndexVisible) {
            ...
            if (this.list.$capability.reorder) {
                dom.className += " s-list-record-reorder";
            }
            $(toolbar.appendChild(dom)).text((this.list._store.pagingCapability._$startIndex || 0) + this.$recordIndex);
        }
