...
exports.CardRecord = helpers.defineClass(CardRecord, RecordArticle, {
	loadBox: function() {
		...
		if (!this.row || (this.row.count == this.builder.$cardsByRowCount)) {
			this.row = this.builder.createRow();
			if (this.list.treeDecorator) {
				...
				for (var ii = 0; ii < this.row.max; ii++) {
					this.builder.appendDefaultCell(this.row);
				}
			}
	...
	applyReorderCapability: function(reorder) {
		if (this.hasReorderCapability = reorder) {
			if (!this.reorderPicker) {
				...
				this.cardContent.insertBefore(this.reorderPicker, this.cardContent.firstChild);
			}
		} else {
			if (this.reorderPicker) {
	...
	ensureLayoutMode: function() {
		...
		if (this._menusSlot) {
			this._menusSlot.className = this.$skin + "-actions-cell s-record-actions";
		}
		this.cardContent.className = this.$skin + "-content";
	},
	renderAltStyle: function() {
		if (this.list.$item.$alternateStyle) {
			document.site.toggleClass(this.domItem, "s-list-row-alt", this.$recordIndex % 2);
		}
	},
	onSelectMouseEvent: function(event) {
		document.site.toggleClass(this.list.treeDecorator ? this.treeNode.item : this.domItem, "s-record-over", event.type == "mouseenter");
	},
	highlightSelection: function(selected) {
		document.site.toggleClass(this.list.treeDecorator ? this.treeNode.item : this.domItem, "s-list-record-selected", selected);
		if (this._selectorCheck) {
			this._selectorCheck.checked = selected;
		}
	},
	dispose: function() {
		this.toolbar = this.cardItem = this.domItem = this.treeNode = this.row = this.cardContent = this._selectorCheck = this._menusSlot = this.rowIndexPicker = this.dataRow = null;
