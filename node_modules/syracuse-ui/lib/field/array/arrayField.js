...
exports.ArrayField = helpers.defineClass(ArrayField, null, {
	loadBox: function(initData) {
		...
		if (this.$addRefresh) {
			this.btnRefresh = syra_button.add({
				parent: this,
	...
	addReorder: function(record) {
		if (!record.reorderBtn) {
			...
			record.list.$isDisabled && syra_dom.disableItem(record.reorderBtn, record.list.$isDisabled);
		}
	},
		...
		if (this.builder.showBody) {
			...
			if (show) {
				...
				if (!this.bodyInsert) {
					...
					syra_dom.removeChild(this.body);
				}
			}
		}
	},
	...
	onMenuClick: function(menuItem) {
		if (!menuItem.$isAction) {
			switch (menuItem.$sourceBind) {
				...
				case "$next":
				case "$last":
					_filler.trigger(this, null, syra_url.formatMenuUrl(menuItem.articleParent, menuItem));
					return false;
				case "$select":
