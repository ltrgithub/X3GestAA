...
function _applyListMeta(list, metaData) {
	...
	if (metaData.$item) {
		if (metaData.$item.$links) {
			...
			syra_delta.applyObjectDelta(list.page, $menus, metaData.$item.$links, true);
		}
		if (metaData.$item.$actions) {
			var $menus = list.$prototype.$item.$actions || {};
			syra_delta.applyObjectDelta(list.page, $menus, metaData.$item.$actions, true);
		}
	}
...
exports.ArrayField = helpers.defineClass(ArrayField, FieldArticle, {
	loadBox: function(initData) {
		...
		FieldArticle.prototype.loadBox.call(this, initData);
	...
	isReorderRecordDraggable: function(target, event) {
		if (target.syraReorderRecordId == this.id) {
			syra_dd.dropableItem = {
				list: this,
				reorderRecord: syra_store.findRecord(this, event.target)
			};
	...
	notifyClientSave: function(savedDelta) {
		if (savedDelta && savedDelta.$uuid) {
			var dataRecord = _filler.findDataRecord(this, savedDelta.$uuid);
			var delta = {};
			...
			if (dataRecord) {
				syra_delta.applyObjectDelta(this.page, dataRecord, delta);
				...
				delta = [delta];
				syra_delta.applyPageArrayDelta(this.page, dataset, delta);
	...
	dispose: function() {
		...
		FieldArticle.prototype.dispose.call(this);
	},
