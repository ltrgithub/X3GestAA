...
function _toggleUIDesign(list, enable, disposingDesigner) {
	if (enable) {
		if (!list.designBtn) {
		...
		disposingDesigner && syra_site.designStore.design(list, false);
		if (list.designBtn) {
...
exports.ArrayField = helpers.defineClass(ArrayField, Article, {
	...
	toggleUIDesign: function(enable, disposingDesigner) {
		_toggleUIDesign(this, enable, disposingDesigner);
	},
	onDesignOpenerClick: function() {
		syra_site.designStore.design(this, !this.designer);
	},
		...
		if (!menuItem.$isAction) {
			...
			switch (menuItem.$sourceBind) {
				...
				case "$select":
					...
					return false;
				case "$design":
					syra_site.designStore.design(this, true);
					return false;
			}
		}
		return true;
