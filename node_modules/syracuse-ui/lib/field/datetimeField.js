...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
		...
		if (shortcurts.down) {
			...
			if (nodes) {
				for (var ii = 0, jj = nodes.length; ii < jj; ii++) {
					var node = nodes[ii];
					if ((node.syraSelector == "date") || (node.syraSelector == "time")) {
						node.click();
						break;
					}
				}
	...
	onDateTimePickerClick: function(btn, options) {
		var self = this;
		if (!self.$isDisabled && !self.$isReadOnly) {
			self.page.externalAdapter.onFieldClickPicker({
				field: self,
				pickerType: btn.syraSelector,
				doEvent: function(options) {
					self.focus();
					if (!self.popupPicker) {
						self.domItem.id = self.id;
						self.picker = new selectorType[btn.syraSelector]();
	...
	render: function() {
		if (this.$isEditMode) {
			if (this.$field.$type != "application/x-datetime") {
				...
				var selector = (this.$field.$type == "application/x-time") ? "time" : "date";
				this.addPicker("onDateTimePickerClick", selector).syraSelector = selector;
	...
	_setNotNullDataValue: function(value, dateObj) {
		...
		if (this.$isEditMode) {
			...
			this.fieldHelper.setValueLength(this, formated.length);
