...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
	...
	applyShortCuts: function(shortcurts, event) {
		if (shortcurts.down) {
			for (var ii = 0, jj = this.pickers.length; ii < jj; ii++) {
				var picker = this.pickers[ii];
				if ((picker.syraSelector == "date") || (picker.syraSelector == "time")) {
					picker.click();
					break;
				}
			}
	...
	render: function() {
		if (this.$isEditMode) {
			if (this.$field.$type != "application/x-datetime") {
				...
				this.pickersHelper.addDateTime(this);
