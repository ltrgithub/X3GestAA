...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
	...
	initialize: function() {
		...
		this.$field.$maxLength = Math.max(this.$field.$maxLength || 0, formatApi.getFormatLength(this.localFormatDisplay));
	...
	getDataValue: function(entity, index, event) {
		if (this.input) {
					...
					if (val) {
						...
						if (val) {
							try {
								...
								if (this.$field.$type == "application/x-datetime") {
									val = this.formatApi.parse(val, formatApi.getLocale().format.date_short).toString();
								} else {
									throw error;
								}
							}
