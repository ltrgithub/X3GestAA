...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
	...
	applyShortCut: function(shortcuts, event) {
		...
		if (event.shiftKey) {
			if (shortcuts.down && !this.popupPicker) {
				(this.mnPickers.date || this.mnPickers.time).click();
				return true;
			}
		}
	...
	render: function() {
		if (this.$isEditMode) {
			if (this.$field.$type != "application/x-datetime") {
				...
				syra_menus.addFieldPicker(this, this.isTimeField ? "time" : "date", this.isTimeField ? syra_local.dfSelectTime : syra_local.dfSelectDate, "ESC F12");
			} else {
				this.input = document.createElement("input");
				this.input.setAttribute("type", "text");
				syra_site.setSpecificAttributes(this.input);
	...
	processInputHelper: function(val) {
		...
		if (val === "0") {
			dayOffset = 0;
		} else if (val.length > 1 && (val.startsWith("+") || val.startsWith("-"))) {
	...
	onInputChange: function(input, event) {
		try {
			var inputHelper = this.processInputHelper(input.value);
			if (inputHelper) {
				input.value = inputHelper;
			}
			var errors = []; //, val = input.value, valFormatted = val;
			// for application/x-date and application/x-time types only for now
			if (input.value != '') {
				// applying api validation rules
				var rawVal = this._formatInput(this, input.value);
				var constraints = {
					$isMandatory: this.$isMandatory,
					$isNullable: this.$isNullable
				};
				this.formatApi.validate(rawVal, constraints, errors);

				// formatted value to send to server
				input.value = rawVal.toString(this.localFormat);
				//input.value = valFormatted = this._formatInput(this, val);
			}

			this.validate(input.value, errors) && this.onInputValidate(event);
		} catch (error) {
			this.showErrors([error.message]);
		}
	},
