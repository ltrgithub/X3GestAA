...
var _formatter = {
    "application/x-date": {
        formatInput: function(field, val){
            ...
            switch (splittedLen) {
                                ...
                                if (f.dir == "y") {
                                    ...
                                    switch (len) {
                                        case 2:
                                            ...
                                            break;
                                        default:
                                            throw new Error(field.localize.dfInvalidDate + " : " + val + ". " + field.localize.dfFormat + " : " + field.localFormat);
                                    }
                                }
                ...
                case 3:
                    // day, month and year
                    var i = 0;
                    for (var mm = 0, jj = splitted.length - 1; mm < jj; mm++) {
                        var spl = splitted[mm];
    ...
    "application/x-time": {
        formatInput: function(field, val){
                ...
                case 1:
                    ...
                    switch (time.length) {
                        ...
                        case 6:
                            ...
                            break;
                        default:
                            throw new Error(field.localize.dfInvalidTime + " : " + val);
                    }
                    break;
                case 2:
                    // hour and minute
            ...
            return timeObj;
            //return timeObj.toString(field.localFormat);
        }
    },
...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
    ...
    applyShortCuts: function(shortcurts, event){
        if (shortcurts.down) {
            var nodes = this.boxPickers ? this.boxPickers.childNodes : null;
            if (nodes) {
                for (var ii = 0, jj = nodes.length; ii < jj; ii++) {
                    var node = nodes[ii];
                    if ((node.syraPickerType == "date") || (node.syraPickerType == "time")) {
                        node.click();
                        break;
                    }
                }
    ...
    doClickPicker: function(btn, options){
        ...
        if (!self.$isDisabled && !self.$isReadOnly) {
            self.focus();
            if (!self.popupPicker) {
                self.domItem.setAttribute("id", self.id);
                self.picker = new picker[btn.className.indexOf("-time-picker") > 0 ? "TimeChoice" : "Calendar"]();
                self.picker.create(self, options);
                self.popupPicker = document.site.dialogManager.openPopup(self.boxParent, {
                    content: self,
                    slot: self.picker.domItem,
                    position: {
                        my: "right top",
                        at: "right bottom",
                        of: $(self._input)
    ...
    setPickerValue: function(formatedValue, internalValue){
        ...
        this.popupPicker.close();
        if (!this.$isReadOnly) {
            this.page.externalAdapter.setPickerValue({
                target: this,
                value: formatedValue,
    ...
    render: function(){
        if (this.$isEditMode) {
            ...
            if (this.$field.$type != "application/x-datetime") {
                ...
                this.appendToPickerBox(this.createPicker(this.$field.$type == "application/x-time" ? "time" : "date"));
            ...
            // application/x-datetime
                ...
                document.site.setSpecificAttributes(this._input);
                this._input.className = this.$skinInput;
                this.fieldValue.appendChild(this._input).className = this.$skinInput;
            }
        }
        else {
    ...
    initialize: function(){
        ...
        this._formatInput = _formatter[this.$field.$type].formatInput;
        ...
        if (value !== undefined && value !== null) {
            try {
                ...
                if (this.$field.$type == "application/x-time") {
                    self._setNotNullDataValue(value, dateObj);
                }
                else {
                    if (!dateObj.isNull()) {
                        self._setNotNullDataValue(value, dateObj);
    ...
    _setNotNullDataValue: function(value, dateObj){
        ...
        if (this.$isEditMode) {
            this.setInputValue(dateObj.toString(this.localFormat));
            this._input.setAttribute("placeholder", this.localFormat);
        }
        else {
            document.site.dom.empty(this.fieldValue);
            var formated = dateObj.toString(this.localFormatDisplay);
            if (formated && !this.$isDetailLinkDisabled && this.$rootLinks.$links && this.$rootLinks.$links.$details) {
                this.appendDetailLink(formated, this.$rootLinks.$links.$details);
            }
            else {
    ...
    getDataValue: function(){
        return this.getInputValue() ? this.formatApi.parse(document.site.trimString(this.getInputValue()), this.localFormat).toString() : null;
    },
    ...
    onInputChange: function(input, event){
        try {
            var self = this, errors = []; //, val = input.value, valFormatted = val;
    ...
    dispose: function(){
        ...
        delete this.localFormatDisplay;
        delete this.localFormat;
        delete this._formatInput;
        delete this.formatObj;
