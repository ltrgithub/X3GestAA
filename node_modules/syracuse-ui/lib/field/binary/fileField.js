...
exports.FileField = helpers.defineClass(FileField, BinaryField, {
    ...
    _setFile: function(file){
        ...
        if (file) {
            ...
            self._image.src = document.site.$item.$iconPath + "/file/s-file-" + image + ".png";
            if (self.removeBtn) {
                self.removeBtn.style.display = "";
            }
            self.$$anchor.attr("target", "_blank").attr("href", self.articleParent.parseExpression(self.$dataUrl)).attr("disabled", false);
        }
        else {
            if (self._image) {
