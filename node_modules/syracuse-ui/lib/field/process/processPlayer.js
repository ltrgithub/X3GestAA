...
function createPattern(xprop) {
	...
	var img=$('<img>') ;
	 	img.attr('src',"/syracuse-ui/themes/desktop/sage/process/images/patterns/" + xprop.xfillProps.xpatLinkId+".gif" /*".gif"*/);
		img.attr('id','tmpimage') ;
	$('body').append(img) ;
	...
	tmpimage.remove() ;
	 ...
	 var img=$('<image>') ;
	 	img.attr('xlink:href',"/syracuse-ui/themes/desktop/sage/process/images/patterns/" + xprop.xfillProps.xpatLinkId+".jpg" /*".gif"*/);
	img.attr('x','0')//
					.attr('y','0')//
					.attr('width', imgwidth ) //
					.attr('height',imgheight ) ;
	var pattern=$('<pattern>').attr('id','pattern' + xprop.uniqueID)//
	.attr('patternUnits',"userSpaceOnUse") ;
					...
					res=res.replace(/<\/pattern>/, img[0].outerHTML+"</pattern>");
	return res ;
	}
...
function createGradient(xprop) {
	function stops(){
		...
		for (var j=0;j<xprop.xfillProps.xgcolors.length;j++){
			var d ;
			/*if (coords)
				d=Math.sqrt((coords.x2-coords.x1)*(coords.x2-coords.x1)+(coords.y2-coords.y1)*(coords.y2-coords.y1)) ;
			else d=1 ;*/
			//(xprop.xfillProps.xgratios[j])*d/255
			//var newratio=xprop.xfillProps.xgratios[j]*d/255 ;
