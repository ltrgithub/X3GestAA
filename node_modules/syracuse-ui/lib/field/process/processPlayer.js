...
exports.ProcessPlayer = helpers.defineClass(ProcessPlayer, null, {
    ...
    show: function () {
        ...
        function createShape(xprop) {
            ...
            switch (xprop.xshapeType) {
                ...
                case 'fileshape':
                    ...
                    /*shape=appendSvg('svg:image',{x: (xprop.xcenter.x - width / 2)+ 'px', //
                     y: (xprop.xcenter.y - height / 2 )+'px', //
                     width: width + 'px',//
                    height: height + 'px', //
                     preserveAspectRatio: 'none',//
                    id: xprop.uniqueID ,//
                    'z-index' : xprop.xlevel, //
                    'xlink:href' : getHRef(xprop) ,//
                    ...
                    /*shape = $('<image>');
                    shape.attr('xmlns','http://www.w3.org/2000/svg>');*/
                    //var image=document.createElementNS("http://www.w3.org/2000/svg","svg:image") ;
                    //shape = $(image) ;

                    /*shape.attr('x', xprop.xcenter.x - width / 2 + 'px');
                    shape.attr('y', xprop.xcenter.y - height / 2 + 'px');
                    shape.attr('width', width + 'px');
                    shape.attr('height', height + 'px');
				...
				shape.style('opacity', xprop.alpha + "/100");*/
                   ...
                   /* shape.attr("xlink:href", getHRef(xprop));
                    shape.attr("externalResourcesRequired", "true");*/

                    if (xprop.xlinkProps.xtype != 'none') {
                imagedescr+='link="'+ JSON.stringify(xprop.xlinkProps)+'"';
                //shape.attr('link', JSON.stringify(xprop.xlinkProps)) ;
            }
            imagedescr+='/>'
            return  imagedescr ;
        ...
        function createAction(action, index, array) {
            function mouseleaveHdl(e) {
                        //console.log("mouseover - "+e.target.tagName) ;
                        //var x=svgplot.find('.target' + action.id);
                        //x.toggle();
                       //x.show() ;
                       $(e.currentTarget).attr('display', 'none' ) ;
                       //.hide() ;
                    }
           function clickHdl() {
                           var  targ=svgplot.find('.target' + action.id)
                            //svgplot.find('.target' + action.id) ;
                            //targ.show();
                            targ.attr('display', 'block') ;
                    }
            ...
            switch (action.type) {
                case 'shotip':
                    ...
                    .mouseleave(function (e) {
                      //  console.log("mouseleave - "+ e.target.tagName + " - "+e.currentTarget.tagName+'.'+$(e.currentTarget).attr('class'));
                       // var hide=(e.currentTarget.firstChild==e.target && $(e.currentTarget).is('.trigger'+action.id));
                    var hide= e.currentTarget.firstChild==e.target ;//&& $(e.currentTarget).is('.trigger'+action.id) ;
                        //(e.currentTarget.firstChild==e.target) ;
                       // if(hide)
                            svgplot.find('.target' + action.id).hide();
                            //(e.currentTarget.firstChild!=e.target || !($(e.currentTarget).is('.trigger'+action.id)))
                        /*else
                ...
                case 'sho':
                    ...
                    array.forEach(function(a,index,array){
                        if (a.type==='hid' && target.is('.target' + a.id))
                            isHideTarget=true ;
                    }) ;
                    if (!isHideTarget)
                        target.hide();
                    ...
                    case 'shopopmenu':
                        ...
                        target.attr('display', 'none' ) ;
                         //.hide() ;
                        target.bind( 'mouseleave',mouseleaveHdl) ;
                       // $(target).bind() ;
                 ...
                 trigger.click(function () {
                    array.forEach(function(a,index,array){
                        if (a.type==="shopopmenu" ){
                            var targets=svgplot.find('.target' + a.id ) ; //+':hidden') ;
                            var trig=svgplot.find('.trigger' + a.id ) ;
                            
                            targets.each(function(index,el){
                               // $(el).show() ;
                                 var hidden = ($(el).attr('display') == 'none');
                            if (hidden){
                                $(el).unbind('mouseleave',mouseleaveHdl) ;
                                trig.unbind('click',clickHdl);
                                 $(el).attr('display', 'block');
                             }
                            else {
                                $(el).bind('onmouseleave',mouseleaveHdl) ;
...
function getGradientTransVector(alpha) {
    ...
    //students's algorythm
    /*else if (alpha < Math.PI / 4) {
		x2 = 50;
		y2 = x2 * Math.tan(alpha);
	} else if (alpha < Math.PI / 2 - margeArrondi) {
		y2 = 50;
		x2 = y2 * (Math.tan(alpha - Math.PI / 4));
		...
		//x2 = 50 * (Math.cotan(alpha - Math.PI )) * -1;

		x2 = 50 * (1/Math.tan(alpha - Math.PI )) * -1; 
		//test 
		y2 = -50;
		//x2 = y2 * (1/Math.tan(alpha )) ;

		
	} else if (alpha < 7 * Math.PI / 4) {
		//x2 = 50 * (Math.tan(alpha - 3 * Math.PI / 2)) * -1;
		y2 = -50;
		x2 = 50 * (1/Math.tan(alpha))* -1;
		
	} else { //alpha>315
		x2 = 50;
		y2=x2*Math.tan(alpha) ;
		//y2 = (50 * Math.tan(alpha - 7 * Math.PI / 4)) * -1;
		//y2 = (50 * Math.tan(2* Math.PI-alpha ));
	}*/
...
function createText(xprop) {
    ...
    //- xprop.xtextFormat.rightMargin - xprop.xtextFormat.leftMargin
