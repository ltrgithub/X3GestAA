...
function _getGradientTransVector(alpha) {
	...
	if (_isEqual(alpha, Math.PI / 2)) {
		x2 = 0;
		y2 = 50;
	} else
	if (_isEqual(alpha, Math.PI)) {
		x2 = -50;
		y2 = 0;
	} else
	if (_isEqual(alpha, 3 * Math.PI / 2)) {
		x2 = 0;
		y2 = -50;
	} else {
...
function _createShadow(xprop) {
	return "<filter id='shadow" + xprop.uniqueID + "'>" +
		"<feoffset result='offOut' dy='3' dx='3' in='SourceAlpha'></feoffset><fegaussianblur result='blurred' stddeviation='3' in='offOut'></fegaussianblur><feblend mode='normal' in2='blurred' in='SourceGraphic'></feblend>" +
		"</filter>";
}
		...
		case 'diamond1':
			...
			marker.attr('fill', function() {
				if (markertype == 'diamond1' || markertype == 'arr1') {
					if (filled)
						return _getHexadecimalColor(xprop.xstrokeProps.xcolor);
					else
						return '#FFFFFF';
				} else
					return 'none';
...
exports.createShape = function(xprop, csize, bibMapping) {
	var defs;
	if (xprop.xshapeType === 'apishape' || xprop.xshapeType === 'beziershape') {
		...
		if (xprop.xfillProps.xtype === 'patternfill') {
	...
	switch (xprop.xshapeType) {
		case 'apishape':
			...
			switch (xprop.xdrawBehaviorCode) {
				...
				case "K_API_RECT":
					...
					height = Math.sqrt((xprop.xanchors[3].x - xprop.xanchors[0].x) * (xprop.xanchors[3].x - xprop.xanchors[0].x) +
		...
		case 'fileshape':
			width = Math.sqrt((xprop.xanchors[1].x - xprop.xanchors[0].x) * (xprop.xanchors[1].x - xprop.xanchors[0].x) + (xprop.xanchors[1].y - xprop.xanchors[0].y) * (xprop.xanchors[1].y - xprop.xanchors[0].y));
			height = Math.sqrt((xprop.xanchors[2].x - xprop.xanchors[1].x) * (xprop.xanchors[2].x - xprop.xanchors[1].x) +
			...
			var href = '/syracuse-ui/themes/desktop/sage/process/images/' + bib.famSubpath + bib.catSubpath + bib.name + '/' + xprop.eltid;
			if (xprop.xshapeType == 'clipshape') {
				href += '.png';
			}
			var imagedescr = '<image x="' + (xprop.xcenter.x - width / 2) + 'px" y="' + (xprop.xcenter.y - height / 2) + 'px" width="' + width + 'px" height="' +
				height +
				'px" preserveAspectRatio="none" id="' +
				xprop.uniqueID +
				'" z-index="' +
				xprop.xlevel +
				'" xlink:href="' +
			...
			if (xprop.xshadowProps.xtype == 'global') {
				imagedescr += "filter='url(#shadow" + xprop.uniqueID + ")'";
			}
			if (xprop.xlinkProps.xtype != 'none') {
				imagedescr += ' link="' + _encodeLink(xprop.xlinkProps) + '"';
	...
	if (xprop.xshapeType === 'beziershape') {
		...
		if (xprop.xstrokeProps.xtype === 'solidstroke') {
...
exports.appendText = function(forme, container) {
	var text = "";
	var shadow = '';
	if (forme.xshadowProps.xtype === 'global')
		shadow = 'text-shadow: silver 5px 5px 3px;';

	var div = document.createElement("div");
	div.setAttribute('id', forme.uniqueID + 'p');
	div.setAttribute('style', 'position:absolute; left:' + forme.xcaptionPos.xleft + 'px; top:' + top + 'px;width: ' + (forme.xcaptionSize.xwidth) + 'px; height: ' + (forme.xcaptionSize.xheight + 10) +
		'px; ' +
		shadow +
		"vertical-align:" +
		forme.xcaptionProps.xvertAlignMode +
		'; z-index:' +
		forme.xlevel);
	var $$div = $(div);
	...
	if (forme.xactionProps.targ || forme.xactionProps.trig) {
		...
		if (forme.xactionProps.targ) {
			for (var ii = 0, jj = forme.xactionProps.targ.length; ii < jj; ii++) {
				css += " target" + forme.xactionProps.targ[ii];
			}
		}
		if (forme.xactionProps.trig) {
			for (var ii = 0, jj = forme.xactionProps.trig.length; ii < jj; ii++) {
...
exports.createText = function(xprop) {
