...
function loadLibInfo(cb) {
	...
	syra_controller.callServer(null, {
		$location: {
			$url: "/syracuse-ui/addins/sageFlash/swf/FLASH/FlowCharting/xtec/sage_libraries/sage_basic/sgx_libsinfo.txt"
		},
		method: "GET"
...
exports.VisualProcess = helpers.defineClass(VisualProcess, Field, {
	...
	setDataValue: function(value) {
		try {
			if (this.currentValue != value) {
				...
				// var parentSlot = this.$$playerSlot.context.parentNode;
				// if (parentSlot) {
				// 	parentSlot.style.overflow = "auto";
				// 	parentSlot.style.width = "100%";
				// }
						...
						for (var ii = 0, jj = li.length; ii < jj; ii++) {
							...
							if (lib.biblist) {
								for (var mm = 0, kk = lib.biblist.length; mm < kk; mm++) {
									self.bibMapping[lib.biblist[mm].bibid] = lib.biblist[mm];
								}
							}
						}
					}
	...
	_showPlayer: function() {
		if (!this.process || this.isDisplayed || !this.playerSlot || !this.bibMapping) {
			return;
		}
		...
		self._core.style.maxWidth = csize.x + 10 + "px";
		self._core.style.width = "100%";
		self._core.style.overflow = "auto";
		...
		for (var gn = 0, sizeG = layers.length; gn < sizeG; gn++) {
			...
			for (var i = 0; i < group.xpropsArr.length; i++) {
				var shapeInfo = tools.createShape(group.xpropsArr[i], csize, self.bibMapping, this.symbolManager);
				defs += (shapeInfo.defs || "");
				var rotation = false;
