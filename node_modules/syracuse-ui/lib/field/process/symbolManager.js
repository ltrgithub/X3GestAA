...
function SvgNode(renderer, symbol) {
	var groups = [],
		root;
	...
	this.element = root = renderer.g(symbol.id).attr({
		id: symbol.id
	}).add();
	create(symbol);
	this.box = this.element.getBBox();
	this.box.cx = this.box.cx + this.box.width / 2;
	this.box.yx = this.box.yx + this.box.height / 2;
}
...
function SymbolManager(slot) {
	initSymbols();
	this.slot = slot;
	this.renderer = new Highcharts.Renderer(this.slot, 800, 200);
}
...
exports.SymbolManager = helpers.defineClass(SymbolManager, null, {
	...
	get: function(id) {
		var s = symbols[id];
		if (s && !s.node) {
			s.node = new SvgNode(this.renderer, s);
		}
		return s;
	},
