...
var pickersHelper = require('syracuse-ui/lib/field/helpers/pickers');
...
exports.Field = helpers.defineClass(Field, null, {
	...
	_createValueContainer: function() {
		if (this.$item.$inplace) {
			...
			this.domValueSlot.className = this._$cssField + (this.$isEditMode ? "-value-edit" : "-value-detail");
			this._core = this._dataValue = this.domValueSlot;
		...
		if (this.$isEditMode) {
			...
			if (this.$cssFieldType) {
				this.domItem.className += " " + this.$cssFieldType;
			}
			this._createFieldValueRead();
		}
	},
	_createFieldValueRead: function() {
		...
		this._dataValue.appendChild(this.fieldValue);
	},
	_initCss: function() {
	...
	loadBox: function() {
		...
		this.pickersHelper = pickersHelper;
		this.articleParent = this.boxParent.getArticle();
		this.ensureEditMode();
		...
		if (this.input) {
			...
			this.setInputFieldId(this.input);
	...
	setFieldId: function(dom) {
		dom.setAttribute("data-s-field", this.id);
	},
	setInputFieldId: function(dom) {
		dom.setAttribute("data-s-field", this.id);
		dom.syraOnClick = "onInputFieldClick";
	},
	setFieldWidth: function() {
		if (this.$isEditMode && !this.hasFreeWidth && !this.$item.$rows && !this.$item.$inplace && this.domValueSlot && !this.$item.$isAutoSizeDisabled) {
			...
			if (this.input) {
				this.input.style.maxWidth = this.widthValue + "px";
			}
		}
	},
	...
	createHtmlInput: function() {
		...
		if (!this.$item.$isFilterMode) {
			input.setAttribute("type", this.$inputType || "text");
		}
		syra_site.setSpecificAttributes(input);
		return input;
	},
	_appendLinksPicker: function() {
		if (this.pickersHelper && this.$rootLinks.$links) {
			if (this.$rootLinks.$links.$lookup) {
				this.pickersHelper.addLookup(this);
			}
			if (!this.$item.$inplace && this.$rootLinks.$links.$tunnel) {
				this.pickersHelper.addTunnel(this);
			}
			if (this.$rootLinks.$links.$localize && this.$item.$useLocalizePicker) {
				this.pickersHelper.addLocalize(this);
			}
		}
	},
	...
	onItemInOut: function(on) {
		if (this.$isEditMode) {
			syra_site.dom.toggleClass(this.domValueSlot, "s-enter", on);
		}
		this.pickersHelper && this.pickersHelper.highlight(this, on);
	...
	onFieldInputEvent: function(event, shortcuts) {
		...
		if (event.type == "focusin") {
			syra_site.dom.toggleClass(self._dataValue, "s-focus", true);
			self.pickersHelper && self.pickersHelper.highlight(self, true);
			self.page.focusField = self;
			if (self.$item && self.$item.$isCellChild) {
				syra_site.scrollToField(self);
			}
		} else {
			if (event.type == "focusout") {
				syra_site.dom.toggleClass(this._dataValue, "s-focus", false);
				if (self.page.focusField == self) {
					self.page.focusField = null;
				}
				self.pickersHelper && self.pickersHelper.highlight(self, false);
			}
		}
	...
	onLocalizeClick: function(picker) {
		var self = this;
	...
	onLookupClick: function(picker) {
		var self = this;
		if (!self.authoringNode && !self.$isDisabled) {
			if (self.articleParent.onLookupClick ? self.articleParent.onLookupClick(self, picker) : true) {
				self.page.externalAdapter.onFieldClickPicker({
					field: self,
					pickerType: "lookup",
					doEvent: function(options) {
						...
						syra_site.dialogManager.openLookup(self.boxParent, {
	...
	onTunnelClick: function(picker) {
		if (!this.authoringNode) {
			this.page.externalAdapter.onFieldClickPicker({
				field: this,
				pickerType: "tunnel",
				doEvent: function(options) {}
			});
	...
	onMenuPickerClick: function(picker) {
		this.menusHelper.onMenuPickerClick(this, picker);
	},
		...
		if ($help != undefined) {
			...
			if (this.domTitle && this.getTitle().length > 0) {
				if (this.$help) {
					...
					if (this._helpFlag) {
						syra_site.dom.removeChild(this._helpFlag);
	...
	setDescription: function($description) {
		if ($description !== undefined) {
			...
			if (text.length > 0) {
				if (text[1] == "@") {
					text = syra_site.expressionMaker.render(this.boxParent, text);
				}
				if (!this.$item.$inplace) {
	...
	setState: function(state) {
		if (!this.$item.$isFilterMode) {
			...
			if (state.$isDisabled !== undefined) {
				...
				if (this.domTitle) {
					syra_site.dom.disableItem(this.domTitle, state.$isDisabled);
				}
				if (this._description) {
					syra_site.dom.disableItem(this._description, state.$isDisabled);
				}
			...
			this.pickersHelper && this.pickersHelper.applyState(this);

			if (this.menusBox && this.$isReadOnly != undefined) {
				//§§§§§§ important findmenu n'existe plus modifier direct le proto  
	...
	setDataValue: function(value, $bind) {
		...
		if (this.$isEditMode) {
			...
			if (value && this.$rootLinks.$links && this.$rootLinks.$links.$details && !this.$item.$isDetailLinkDisabled) {
				...
				if (this.$item.$rows) {
					this.fieldValue.innerHTML = syra_site.formatHTMLText(value);
				} else {
					this.fieldValue.textContent = value;
				}
			}
			this.pickersHelper.showMenu(this);
		}
	},
	...
	focus: function(select) {
		syra_site.scrollToField(this);
		if (this.input && !this.$isHidden) {
			this.input.focus();
	...
	validate: function(newValue, errors) {
		...
		if (dataType.$pattern && value) {
			...
			if (!match) { //(match && match.index == 0 && match[0].length == value.length)) {
				errors.push(syra_local.fieldInvalidValue + " : " + value);
			}
		}
		this.validateType && this.validateType(errors, value);
		...
		if (errors.length > 0 || this.$displayedDiagnoses) {
			...
			if (saveLinks && saveLinks.length > 0) {
				if (!saveLinks[0].$isDisabled) {
					if (syra_site.diagnoseManager.hasErrors()) {
	...
	setMenus: function(metaData) {
		...
		if (!this.$item.$isMenusHidden && (!this.$item.$isUnitField || this.$item.$isEditMode)) {
			var $menus = metaData.$item || metaData;
			if ($menus.$links !== undefined || $menus.$actions !== undefined) {
				if ($menus.$links === null) {
					if (this.menusBox) {
						this.menusBox.clearMenus(true);
						this.pickersHelper.showMenu(this, false);
						this.menusBox.unload();
						delete this.menusBox;
					}
				} else {
					$menus.$links && this._applyMenusChange($menus.$links);
					$menus.$actions && this._applyMenusChange($menus.$actions, true);
					if (this.$menus && Object.keys(this.$menus).length) {
						!this.menusPicker && this.pickersHelper.addMenu(this);
					}
				}
			}
		}
	...
	onMenuClick: function(menuItem) {
		if (!this.$isDisabled && !menuItem.$isDisabled) {
			if (menuItem.$sourceBind == "$localize") {
				syra_site.localizer.onLocalizeFieldClick(menuItem);
				return false;
			}
		}
		return true;
	...
	onInputFieldClick: function(input, event) {
		this.onFieldInputEvent(event);
		this.page.registerLastFocusGrid(this);
		event.syraRetValue = true;
		event.stopPropagation();
	},
	dispose: function() {
		this.pickersHelper && this.pickersHelper.dispose(this);
		...
		this.menusBox && this.menusBox.dispose();
		if (this.articleParent && this.$item && this.$item.$isAutoSize) {
			this.articleParent.registerAutoSizeFields(this, false);
		}
		this.fieldHelper = this.$rootLinks = this.rowSeparator = null;
		this.domValueSlot = this.diagSeverities = this.authoringNode = null;
		...
		this.pickersHelper = this.layoutParent = this.boxParent = this.articleParent = this.page = this.parentVariantField = null;
	}
});