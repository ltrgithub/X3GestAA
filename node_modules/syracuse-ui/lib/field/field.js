...
exports.Field = helpers.defineClass(Field, null, {
	...
	setFieldWidth: function() {
		if (this.$isEditMode && !this.hasFreeWidth && !this.$item.$rows && !this.$item.$inplace && this.domValueSlot && !this.$item.$isAutoSizeDisabled) {
			this.setInputWidth(this.widthValue = fieldHelper.getCardFieldWidth(this.$field));
		}
	},
	...
	_appendLinksPicker: function() {
		if (this.$rootLinks.$links) {
			if (this.$rootLinks.$links.$lookup) {
				this.lookupPicker = this.addPicker("onLookupClick", "lookup");
				this.lookupPicker.title = this.localize.fieldLookup;
			}
			...
			if (this.$rootLinks.$links.$localize && this.$item.$useLocalizePicker) {
				this.localizePicker = this.addPicker("onLocalizeClick", "localize");
				this.localizePicker.title = this.$rootLinks.$links.$localize.$title;
			}
	...
	appendTunnelPicker: function($tunnel) {
		if (!this.$item.$inplace) {
			this.tunnelPicker = this.addPicker("onTunnelClick", "tunnel");
	...
	removeFromPickerBox: function(picker) {
		...
		if (picker) {
			syra_site.dom.removeChild(picker);
			if (this.boxPickersPadding == 0 && this.boxPickers) {
				syra_site.dom.removeChild(this.boxPickers);
	...
	addPicker: function(pickerId, cssId, picker) {
		if (!this.boxPickers) {
		...
		if (!picker) {
			...
			var css = this.$skin + "-" + (cssId || pickerId);
			if (this.$item.$css) {
				css = this.$item.$css + " " + css;
			}
			picker.className = css + "-picker";
			picker.syraOnClick = pickerId;
			picker.setAttribute("data-s-picker2", pickerId);
			picker.setAttribute("data-s-field2", this.id);
		}
	...
	highlightPickers: function(hightLight) {
		if (this.menusBox) {
			syra_site.dom.toggleClass(this.menusBox.domItem, "s-enter", hightLight);
		}
		if (this.boxPickers) {
			syra_site.dom.toggleClass(this.boxPickers, "s-enter", hightLight);
		}
	},
	onItemInOut: function(onEnter) {
		this.highlightPickers(onEnter);
		if (this.isDiagnosePopup && this.page.diagnosesPanel) {
			this.page.diagnosesPanel.onFieldItemInOut(this, onEnter);
		}
	},
	...
	setState: function(state) {
		if (!this.$item.$isFilterMode) {
			...
			if (state.$isDisabled !== undefined) {
				...
				this._description && syra_site.dom.disableItem(this._description, state.$isDisabled);
	...
	setDataValue: function(value, $bind) {
		...
		if (this.$isEditMode) {
			...
			if (value && this.$rootLinks.$links && this.$rootLinks.$links.$details && !this.$item.$isDetailLinkDisabled) {
				...
				if (this.$item.$rows) {
					this.fieldValue.innerHTML = syra_site.formatHTMLText(value, true);
				} else {
	...
	dispose: function() {
		...
		this.fieldHelper = this.localize = this.$rootLinks = this.rowSeparator = null;
		this.menusBox = this.diagSeverities = this.authoringNode = null;
		this.boxPickers = this.domValueSlot = null;
		this.lookupPicker = this.localizePicker = null;
