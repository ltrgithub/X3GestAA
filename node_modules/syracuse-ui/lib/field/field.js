...
exports.Field = helpers.defineClass(Field, null, {
	...
	appendQualityAtt: function(dom) {
		...
		dom.setAttribute("data-s-q-title", this.getTitle());
		...
		if (this.$isEditMode) {
			if (this.$item.$rows) {
				...
				if (this.$item.$contentEditable) {
					...
					this._input.className = this.$skinInput + " s-contenteditable";
				} else {
	...
	highlightPickers: function(hightLight) {
		if (this.menusBox) {
			document.site.toggleClass(this.menusBox._item, "s-mouseenter", hightLight);
		}
		if (this.boxPickers) {
			document.site.toggleClass(this.boxPickers, "s-mouseenter", hightLight);
		}
	},
		...
		if (!this.$item.$isFilterMode) {
			...
			if (state.$isDisabled !== undefined) {
				...
				if (this._input) {
					...
					if (this.$item.$contentEditable) {
						if (state.$isDisabled) {
							this._input.removeAttribute("contenteditable");
						} else {
							this._input.setAttribute("contenteditable", "true");
						}
					}
			...
			if (this.menusBox && this.$isReadOnly != undefined) {
				var menusIds = Object.keys(this.menusBox.boxFields);
				for (var ii = 0, jj = menusIds.length; ii < jj; ii++) {
					var menuItem = this.menusBox.boxFields[menusIds[ii]];
	...
	getInputValue: function() {
		if (this._input) {
			return document.site.trimString(this.$item.$contentEditable ? this._input.textContent : this._input.value);
		} else {
	...
	setInputValue: function(val) {
		if (this.$item.$contentEditable) {
			this._input.textContent = document.site.trimString(val);
		} else {
	...
	focus: function(select) {
		if (this.articleParent.builder && this.articleParent.builder.scroller) {
			this.articleParent.builder.scroller.ensureFieldVisibility(this);
		} else {
	...
	showDiagnoses: function(diagnoses, options) {
		...
		var parent = this.articleParent || this.boxParent;
		if (this.arrayLevel && this.arrayLevel == 'cell' && parent.isRecordArticle && parent.builder && parent.builder.$inlineFieldBinds && !parent.inlineCardRow) {
			if (parent.builder.$inlineFieldBinds.indexOf(this.$item.$bind) >= 0) {
				parent.alertDiagInCard();
			}
		}
	},
	...
	ensureLayoutMode: function() {
		...
		if (this.page.designer) {
			this.page.designer.toggleUIDesign(this, true);
		}
	},
