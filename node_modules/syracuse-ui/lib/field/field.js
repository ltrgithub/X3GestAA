...
exports.Field = helpers.defineClass(Field, null, {
    ...
    appendDetailLink: function(value, $detailsLink){
        if (!this.$isDetailLinkIgnore) {
            ...
            document.itemFactory.load($(this.fieldValue), {
                $bind: "$detail",
    ...
    _appendLinksPicker: function(){
        if (this.$field.$links) {
            ...
            if (this.$field.$links.$tunnel) { //&& !this.$item.$inplace) {
                this.appendToPickerBox(this.createPicker("tunnel"));
            }
            if (this.$field.$links.$lookup && !this.$item.$inplace) {
                this.appendToPickerBox(this.createPicker("lookup"));
            }
        }
    },
    ...
    onFieldMouseEvent: function(event){
        if (event.type == "mouseenter") {
            if (this.boxPickers) {
                this.boxPickers.className += " s-mouseenter";
            }
        }
        else {
            if (!this.hasFocus && this.boxPickers) {
                this.boxPickers.className = this.$skin + "-pickers";
            }
        }
    },
            ...
            self.page.externalAdapter.onFieldClickPicker({
                ...
                doEvent: function(options){
                    switch (btn.getAttribute("data-s-picker")) {
                        case "lookup":
                            self.focus();
                            var $lookup = self.$lookup || self.$field.$links.$lookup;
    ...
    setTitle: function($title){
        ...
        if (this.domTitle) {
            ...
            if (this._input && this.$item.$isPlaceHolder) {
                this._input.setAttribute("placeholder", this.titleText);
            }
        }
    },
    ...
    _removeButtonToValue: function(btn){
        if (btn) {
            ...
            if (this.$item.$inplace && this._input) {
                this._input.style.paddingRight = ((this.valueBtnsChildren * 17) + (this._boxPickerPaddingRight += 20)) + "px";
            }
            else {
                this._dataValue.style.paddingRight = (this.valueBtnsChildren * 17) + "px";
            }
            $(btn).remove();
        }
    },
    setHelp: function($help){
        ...
        if ($help != undefined) {
            this.$help = $help;
            if (this.domTitle && this.titleText.length > 0) {
                if (this.$help) {
                    if (!this._helpFlag) {
    ...
    setDataValue: function(value, record){
        ...
        if (this.$isEditMode) {
            ...
            if (value && this.$field.$links && this.$field.$links.$details && !this.$item.$isMenusDisabled) {
                this.appendDetailLink(value, this.$field.$links.$details);
            }
            else {
    ...
    setMenus: function(metaData){
        if (!this.$item.$isFilterMode && !this.$item.$isExpressionChild) {
            if ((metaData.$links !== undefined) || (metaData.$actions !== undefined)) {
                ...
                this.menusController.setMenus(metaData);
            }
        }
    },
    applyMetaData: function(metaData, options){
        if (metaData) {
            ...
            if (metaData.$convergenceStyle !== undefined) {
                this._applyConvergenceStyle(metaData.$convergenceStyle);
                
            }
        }
        this._ensureMenuPickerVisible();
    },
    _applyConvergenceStyle: function($convergenceStyle){
        var target = this._input ? this._input : this._dataValue;
        if (this.$convergenceStyle) {
            var curStyle = this._dataValue.getAttribute("style");
            target.setAttribute("style", curStyle.replace(this.$convergenceStyle, ""));
        }
        this.$convergenceStyle = $convergenceStyle;
        target.setAttribute("style", this.$convergenceStyle);
    },
    _applyLookUpLinkChange: function($newLookup){
        var self = this;
        if (!self.$lookup) {
            if (self.$field.$links && self.$field.$links.$lookup) {
                self.$lookup = helpers.object.clone(self.$field.$links.$lookup, true);
            }
            else {
    ...
    dispose: function(){
        ...
        delete this.$skin;
        delete this.$skinField;
        delete this.arrayLevel;
        delete this._$cssField;
        delete this._$cssTopField;
