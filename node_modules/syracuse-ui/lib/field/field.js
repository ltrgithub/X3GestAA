...
exports.Field = helpers.defineClass(Field, null, {
	...
	_createValueContainer: function() {
		...
		if (this.$item.$inplace) {
			if (this.articleParent.isGridRecord && !this.$isEditMode) {
				this._core = this.domItem;
				if (this._dataValue) {
					this.domItem.appendChild(this._dataValue);
				} else {
					this._dataValue = this.domItem;
				}
	...
	setState: function(state) {
		...
		/*if (this.articleParent.isGridRecord && this.$numeric) {
         state.$isHidden = true;
         }*/
	...
	showItem: function(show) {
		...
		if (this.$item.$inplace) {
			if (this.articleParent.isGridRecord && !this.$isEditMode) {
				this.domItem.style.visibility = show ? "" : "hidden";
			} else {
	...
	setDataValue: function(value, $bind) {
		...
		if (this.$isEditMode) {
			...
			if (value && this.$menus && this.$menus.$details && !this.$item.$isDetailLinkDisabled) {
				...
				var htmlValue = fieldHelper.emphasisUTC(value);
