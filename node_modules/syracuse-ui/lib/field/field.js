...
exports.Field = helpers.defineClass(Field, null, {
	...
	_appendLinksPicker: function() {
		if (this.$rootLinks.$links) {
			if (this.$rootLinks.$links.$lookup) {
				var picker = this.addPicker("lookup");
			...
			if (this.$rootLinks.$links.$localize && this.$item.$useLocalizePicker) {
				var picker = this.addPicker("localize");
	...
	appendTunnelPicker: function($tunnel) {
		if (!this.$item.$inplace) {
			this.tunnelPicker = this.addPicker("tunnel");
	...
	onClickPicker: function(picker) {
		...
		if (!self.authoringNode && !self.$isDisabled) {
			var doClick = self.articleParent.onFieldClickPicker ? self.articleParent.onFieldClickPicker(self, picker.syraOnClick, picker) : true;
			if (doClick) {
				self.page.externalAdapter.onFieldClickPicker({
					field: self,
					pickerType: picker.syraOnClick,
					doEvent: function(options) {
						switch (picker.syraOnClick) {
							case "lookup":
								self.focus();
							...
							case "menus":
								break;
							default:
								self[picker.syraOnClick] && self[picker.syraOnClick](picker, options);
								break;
						}
					}
				});
