...
function _ensureSectionVisibility(field) {
	if (!field.$item.$inplace && field.page.isPageLoaded) {
		...
		if (field.isInvisible) {
			while (section && !section.isInvisible) {
				syra_site.ensureSectionVisibility(section);
				if (section.isInvisible && section.isTabSection && section.$opened) {
					var items = section.layoutParent.items;
					for (var ii = 0, jj = items.length; ii < jj; ii++) {
						var tab = items[ii];
						if (!tab.isInvisible && tab.isTabSection) {
							tab.openTab();
							break;
						}
					}
...
exports.Field = helpers.defineClass(Field, null, {
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if (designing !== false) {
			if (metaData.$isTopLabelAlignment !== undefined) {
				...
				if (this.domTitle) {
					syra_dom.toggleClass(this.domTitle, beside, !this.$item.$isTopLabelAlignment);
				}
				syra_dom.toggleClass(this._core, beside, !this.$item.$isTopLabelAlignment);
				syra_dom.toggleClass(this.domItem, beside, !this.$item.$isTopLabelAlignment);
			}
			if (metaData.$isAdvanced !== undefined) {
				syra_fields.advancedState.setState(this, metaData.$isAdvanced, designing);
			}
		}
	...
	hideTitle: function($isTitleHidden) {
		syra_dom.hide(this.domTitle, $isTitleHidden);
	},
	_createValueContainer: function() {
		...
		if (this.$item.$inplace) {
			...
			if (this.$isEditMode && this.$item.$isGridCell) {
			...
			this._dataValue = this._dataValue || document.createElement("div");
			this._dataValue.className = css;
			this._core = syra_dom.addDiv(this._$cssField + "-core" + _getCsBeside(this));
	...
	getTextRows: function() {
		...
		return this.$item && this.$item.$rows;
	},
	setTextRows: function($rows) {
		...
		if (this.$isEditMode && !this.hasFreeWidth && !this.getTextRows() && !this.$item.$inplace && this._dataValue && !this.$item.$isAutoSizeDisabled) {
			...
			if (this.input) {
				if (this.buttons && this.buttons.length) {
					this.widthValue += this.buttons.length * 20; // syra_button.btnWidth;
				}
	...
	render: function() {
		...
		if (this.$isEditMode) {
			if ($textRows) {
				...
				this.input.setAttribute("rows", $textRows);
			...
			if ($textRows) {
				syra_dom.toggleClass(this._dataValue, "s-field-multi-row", true);
				this._dataValue.style.maxHeight = (syra_config.size.textHeight * $textRows) + "px";
			}
		}
	},
	...
	getTitle: function() {
		if (this.titleText == null && this.$titleValue) {
			...
			if (this.titleText.length > 0 && this.titleText[1] == "@") {
				this.titleText = syra_expression.render(this.boxParent, this.titleText);
			}
		}
	...
	setTitle: function($title) {
		...
		if (this.domTitle) {
			if (this._mandatoryFlag) {
				syra_dom.removeChild(this._mandatoryFlag.link);
				this.domTitle.textContent = this.$item.$isTitleEmpty ? "" : this.getTitle();
				this.domTitle.appendChild(this._mandatoryFlag.link);
			} else {
				this.domTitle.textContent = this.$item.$isTitleEmpty ? "" : this.getTitle();
			}
		} else {
		...
		if ($description !== undefined) {
			...
			if (this.descriptionText.length > 0) {
				if (this.descriptionText[1] == "@") {
					this.descriptionText = syra_expression.render(this.boxParent, this.descriptionText);
				}
				if (!this.$item.$inplace) {
				...
				if (this.descriptionItem) {
					syra_dom.removeChild(this.descriptionItem);
	...
	setState: function(state) {
		if (!this.$item.$isFilterMode) {
			if (state.$isMandatory !== undefined) {
				...
				if (this.$isEditMode) {
					if (this.domTitle && this.getTitle().length > 0) {
						syra_dom.toggleClass(this.domTitle, "s-mandatory", state.$isMandatory);
						if (state.$isMandatory) {
							if (!this._mandatoryFlag) {
								this._mandatoryFlag = syra_menus.button.add({
									isIndicator: true,
									parent: this,
									slot: this.domTitle,
									iconOnly: true,
							...
							syra_menus.button.remove(this._mandatoryFlag);
							this._mandatoryFlag = null;
						}
					}
					syra_dom.toggleClass(this._dataValue, "s-mandatory", state.$isMandatory);
				}
			}
			if (state.$isDisabled !== undefined) {
				this.disable(state.$isDisabled);
			}
			...
			if (this._mandatoryFlag) {
				syra_menus.button.disable(this._mandatoryFlag, this.$isDisabled || this.$isReadOnly);
			}
	...
	disable: function($isDisabled) {
		...
		if (this.input) {
			syra_dom.disableItem(this.input, $isDisabled);
			if (this.$item.$contentEditable) {
				_contentEditable.setState(this, state);
			}
		}
		this.domTitle && syra_dom.disableItem(this.domTitle, $isDisabled);
		this.descriptionItem && syra_dom.disableItem(this.descriptionItem, $isDisabled);
		syra_dom.disableItem(this._dataValue, $isDisabled);
	},
	showItem: function(show) {
		this.isInvisible = !show;
		if (this.$item.$inplace) {
			syra_dom.hide(this._dataValue, !show);
		} else {
		...
		if (this.$isEditMode) {
			...
			if (value && this.$menus && this.$menus.$details && !this.$item.$isDetailLinkDisabled) {
				...
				if (htmlValue) {
					...
					if (value && this.getTextRows()) {
						if (this._escapedRawValue != value) {
							this._escapedValue = value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/\r\n|\n/g, "<br/>").replace(/\s/g, "&nbsp;");
							this._escapedValue = syra_dom.escape(this._escapedValue);
						}
						this._dataValue.innerHTML = this._escapedValue;
					} else {
	...
	getCaretPosition: function() {
		if (this.input) {
			return this.getTextRows() && (this.input.selectionDirection === "forward" ? this.input.selectionEnd : this.input.selectionStart);
		}
		return -1;
	},
	applyMetaData: function(metaData, options) {
		if (this._dataValue) { //important for reloading and initiliaze method called before render
			if (metaData) {
				...
				syra_menus.menus.applyChange(this, metaData);
				if (!this.$isEditMode && this.currentValue && !this.$isDetailLinkDisabled) {
					if (metaData.$links && metaData.$links.$details) {
						syra_fields.addDetailLink(this, this.currentValue, metaData.$links.$details);
					}
				}
	...
	scrollToField: function() {
		if (!this._isScollToTieldDisabled) {
			var list = this.articleParent.isRecordArticle && this.articleParent.list;
			if (list && list.scrollToRecordField) {
				list.scrollToRecordField(this);
			} else {
