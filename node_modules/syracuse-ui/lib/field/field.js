...
exports.Field = helpers.defineClass(Field, null, {
	...
	toggleAutoCompleteList: function(show) {
		...
		if (self.getDataValue() !== "") {
			self._domItem.setAttribute("id", self.id);
			self.focus();
	...
	onFieldMouseEvent: function(event) {
		...
		if (this.cellDiagnose) {
			switch (event.type) {
				case "mouseenter":
					if (this.cellDiagnose && this.cellDiagnose.$$item) {
						var diagPopup = this.page.openDialog({
							content: this.cellDiagnose,
							$dialogMode: "popup",
							$$dialog: this.cellDiagnose.$$item,
							position: {
								my: "left top",
								at: "left bottom",
								of: this.$$item
						...
						this._domItem.setAttribute("data-s-celldiag-popup", diagPopup.id);
						this._diagPopups = this.diagPopups || {};
						this._diagPopups[diagPopup.id] = diagPopup;
					}
					break;
				case "mouseleave":
					var id = this._domItem.getAttribute("data-s-celldiag-popup");
					// close on mouse leave, and if mouse is not over popup
					if (id) {
						if (this._diagPopups && this._diagPopups[id]) {
							this._diagPopups[id].close();
							this._diagPopups[id] = null;
						}
					}
					break;
			}
	...
	setState: function(state) {
		if (!this.$item.$isFilterMode) {
			...
			if (state.$isDisabled !== undefined) {
				...
				this._domItem.setAttribute("disabled", state.$isDisabled);
				if (this._input) {
		...
		if (state.$isHidden !== undefined) {
			this._showField(!(this.$isHidden = state.$isHidden));
		}
	},
	...
	showAdvancedState: function(show) {
		if (show) {
			if (!this.$isHidden) {
				this._showField(this.boxParent.$isAdvancedFieldsVisible);
			}
		} else {
			this._showField(!this.$isHidden);
		}
	},
	_showField: function(show) {
