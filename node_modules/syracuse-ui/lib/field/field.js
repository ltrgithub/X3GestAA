...
exports.Field = helpers.defineClass(Field, null, {
    ...
    onItemEnterLeave: function(event){
        this.highlightPickers(event.type == "mouseenter");
        if (this.isDiagnosePopup) {
            document.site.diagnoseManager.onFieldItemEnterLeave(this, event);
        }
    },
    ...
    setState: function(state){
        if (!this.$item.$isFilterMode) {
            ...
            if (state.$isDisabled !== undefined) {
                ...
                if (this._input) {
                    document.site.dom.disableItem(this._input, state.$isDisabled);
                    if (this.$item.$contentEditable) {
    ...
    _onSetReadOnly: function($isReadOnly){
        if (this._input) {
            ...
            document.site.dom.toggleClass(this._input, "s-readonly", this.$isReadOnly);
        }
    },
    ...
    getCaretPosition: function(){
        return this.$item && this.$item.$rows && this._input && (this.$$input.prop("selectionDirection") === "forward" ? this.$$input.prop("selectionEnd") : this.$$input.prop("selectionStart"));
    },
            ...
            if (shortcuts.down || shortcuts.up) {
                ...
                if (shortcuts.tab && this.$item.$contentEditable) {
                    ...
                    if (shortcuts.enter) {
                        ...
                        if (this._input && this.getDataValue() !== this.currentValue) {
    ...
    dispose: function(){
        ...
        this._core = this._input = this.currentValue = this.menusBox = null;
