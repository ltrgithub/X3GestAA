...
exports.Field = helpers.defineClass(Field, null, {
    ...
    appendDetailLink: function($$dataValue, value, $detailsLink){
        if (!this.$isDetailLinkIgnore) {
            ...
            if (this.boxPickers) {
                $$dataValue[0].appendChild(this.boxPickers);
            }
        }
    },
...
>>>>>>> 8703da232ab2b7eb009714216e7a3531fc46aebf
        }
    },
    _initCss: function(){
    ...
    appendToPickerBox: function(picker){
        if (!this.boxPickers) {
            ...
            this.$$fieldValue[0].appendChild(this.boxPickers).className = this.$skin + "-pickers";
        }
        this.boxPickers.appendChild(picker);
        (this.$$input ? this.$$input : this.$$fieldValue)[0].style.paddingRight = (this.boxPickersPadding += 18) + "px";
    },
    ...
    validate: function(newValue, errors){
        ...
        if (article.onFieldValidate) {
            article.onFieldValidate(this, value, errors);
        }
    ...
    notifyFieldChange: function(newValue, validated){
        ...
        if (validated || self.validate()) {
            self.getArticle().dataset[self.$item.$bind] = self.currentValue;
            self.getPage().externalAdapter.onFieldNotifyChange({
                field: self,
                doEvent: function(){
                    var article = self.getArticle();
                    if (article.onNotifyDataChange ? article.onNotifyDataChange(self, self.currentValue) : true) {
                        self.getPage().notifyDataChange(self, self.currentValue);
                    }
                }
            });
        }
