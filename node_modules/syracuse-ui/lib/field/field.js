...
var _contentEditable = {
	...
	onInputFocusin: function(field, input, event) {
	...
	onInputFocusout: function(field, input, event) {
		var val = field.input.textContent;
		if (val == "") {
			field.setInputValue("");
		} else {
	...
	setFieldWidth: function() {
		if (this.$isEditMode && !this.hasFreeWidth && !this.$item.$rows && !this.$item.$inplace && this._dataValue && !this.$item.$isAutoSizeDisabled) {
			...
			if (this.input) {
				...
				this.widthValue += (this.pickerCount * syra_button.btnWidth) + _minCardFieldWidth; //for border + padding
	...
	render: function() {
		if (this.$isEditMode) {
			...
			if (this.$item.$rows) {
				...
				this.input = syra_dom.addTextArea(css, this._dataValue, this.$item.$rows);
				this.input.style.height = "auto";
			} else {
				if (this.$item.$contentEditable) {
					this.input = syra_dom.addEditable(css, this._dataValue);
				} else {
					this.input = syra_dom.addInput(css, this._dataValue, !this.$item.$isFilterMode ? this.$inputType : undefined);
				}
			}
		}
	},
		...
		if (value !== undefined) {
			...
			if (this.$isEditMode) {
				...
				if (this.currentValue && typeof(this.currentValue) == 'object' && this.$item && this.$item.$bind) {
					...
					//this.currentValue.$value = this.getValue();
				...
				if (value && this.$menus && this.$menus.$details && !this.$item.$isDetailLinkDisabled) {
					...
					if (htmlValue) {
						this._dataValue.innerHTML = syra_dom.escape(this._dataValue, htmlValue);
					} else {
