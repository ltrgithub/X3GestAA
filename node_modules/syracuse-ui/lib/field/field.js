...
exports.Field = helpers.defineClass(Field, null, {
	...
	addEscapePicker: function() {
		if (this.isTabletDevice && !this.isEscapePickerDisabled && this.input && !this.$isReadOnly && this.page.isFusionPage && !this.$item.$inplace) {
			syra_menus.addFieldPicker(this, "escape", syra_local.fieldEscape, "ESC");
		}
	},
	...
	onFieldInputEvent: function(event, shortcuts) {
		...
		switch (event.type) {
			case "focusin":
			case "focusout":
				isIn = event.type == "focusin";
				var target = (self.isTabletDevice && self.input) ? self.input : self._dataValue;
				syra_site.dom.toggleClass(target, "s-focus", isIn);
				self.articleParent && self.articleParent.onChildFieldFocus && self.articleParent.onChildFieldFocus(self, isIn);
				syra_menus.highlight(self, isIn);
				if (isIn) {
					self.page.focusField = self;
					syra_site.checkUniqueSelection(self);
				} else {
			...
			if (text.length > 0) {
				...
				if (!this.$item.$inplace) {
					if (!this._description) {
						...
						this._dataValue.parentNode.insertBefore(this._description, this._dataValue.nextSibling);
					}
	...
	forceVisibility: function() {
		if (this.isInvisible && this.$item && this.$item.$isAdvanced) {
			this.page && this.page.onAdvancedClick && this.page.onAdvancedClick();
		}
	},
	scrollToField: function() {
		if (!this._isScollToTieldDisabled) {
			if (this.articleParent.isRecordArticle && this.articleParent.list && this.articleParent.list.scrollToRecordField) {
				this.articleParent.list.scrollToRecordField(this);
			} else {
	...
	dispose: function() {
		...
		this.domTitle = this._description = this._helpFlag = null;
