...
exports.Field = helpers.defineClass(Field, null, {
	...
	onInputFocusin: function(input, event) {
		...
		if (this.diagnoseFieldSlot) {
			this.page.diagnosesPanel.onNavigationClick(event, 0, this);
		}
	},
	...
	appendDetailLink: function(value, $detailsLink) {
		if (!this.$isDetailLinkDisabled) {
			if (this.builder) {
				value = (this.builder.$selectedEnum && this.builder.$selectedEnum.$title) || value;
			}
			...
			menus && this.fieldValue.appendChild(menus);
		}
	},
	...
	_createValueContainer: function() {
		this._dataValue = document.createElement(this.$valueTag || "div");
		this._dataValue.className = this._$cssField + (this.$isEditMode ? "-value-edit" : "-value-read");
	...
	onAddMenuPicker: function(picker) {
		this.fieldValue.appendChild(picker);
	...
	setFieldWidth: function() {
		if (this.$isEditMode && !this.hasFreeWidth && !this.getTextRows() && !this.$item.$inplace && this._dataValue && !this.$item.$isAutoSizeDisabled) {
			...
			if (this.input) {
				if (this.mnPickers) {
					this.widthValue += Object.keys(this.mnPickers).length * syra_menus.btnWidth;
				}
	...
	onItemInOut: function(on) {
		if (this.$isEditMode) {
			syra_site.dom.toggleClass(this._dataValue, "s-enter", on);
		}
	...
	onFieldInputEvent: function(event, shortcuts) {
		...
		if (event.type == "focusin") {
			...
			syra_menus.highlight(self, true);
			...
			if (event.type == "focusout") {
				syra_site.dom.toggleClass(self._dataValue, "s-focus", false);
				if (self.page.focusField == self) {
					self.page.focusField = null;
				}
				syra_menus.highlight(self, false);
			}
		}
			...
			if (text.length > 0) {
				...
				if (!this.$item.$inplace) {
					if (!this._description) {
						...
						this._description.className = css;
						$(this._dataValue).after(this._description);
					}
	...
	disable: function($isDisabled) {
		...
		if (this.input) {
			syra_site.dom.disableItem(this.input, $isDisabled);
			if (this.$item.$contentEditable) {
				contentEditableHelper.setState(this, state);
			}
		}
		this.domTitle && syra_site.dom.disableItem(this.domTitle, $isDisabled);
		this._description && syra_site.dom.disableItem(this._description, $isDisabled);
		syra_site.dom.disableItem(this._dataValue, $isDisabled);
	},
	...
	_onSetReadOnly: function($isReadOnly) {
		if (this.input) {
			...
			syra_site.dom.toggleClass(this.fieldValue, "s-readonly", this.$isReadOnly);
		}
	},
	setDataBind: function(value, record, metaData, $bind) {
		if (this.page && this.page.externalAdapter.setDataBind(this, value, record, metaData)) {
			if (value !== undefined) {
				this.setDirty(false);
	...
	setDataValue: function(value, $bind) {
		...
		if (this.$isEditMode) {
			...
			if (value && this.$menus && this.$menus.$details && !this.$item.$isDetailLinkDisabled) {
				...
				if (!htmlValue && this.getTextRows()) {
					htmlValue = value;
				}
				if (htmlValue) {
					this._dataValue.innerHTML = syra_site.formatHTMLText(htmlValue);
				} else {
	...
	getCaretPosition: function() {
		return this.$item && this.getTextRows() && this.input && (this.input.selectionDirection === "forward" ? this.input.selectionEnd : this.input.selectionStart);
	},
	...
	focus: function(select) {
		...
		//this.page.setActivateList(this);
	...
	validateMandatoryValue: function(errors, value) {
		if (this.$isMandatory && (value == '' || (value === null)) && this.page.externalAdapter.notifyFldMandatoryErr()) {
	...
	validate: function(newValue, errors) {
		...
		if (dataType.$pattern && value) {
			...
			if (!match) { //(match && match.index == 0 && match[0].length == value.length)) {
				errors.push(dataType.$patternMessage || syra_local.fieldInvalidValue + " : " + value);
			}
		}
	...
	dispose: function() {
		...
		this.fieldHelper = this._dataValue = this.diagSeverities = this.authoringNode = null;
		this.$field = this.$item = this.boxParent = this._mandatoryFlag = null;
		this.layoutSlot = this.domItem = null;
