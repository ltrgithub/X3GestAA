	...
	validate: function(newValue, errors) {
		...
		if (errors.length > 0 || this.$displayedDiagnoses) {
			...
			if (saveLinks && saveLinks.length > 0) {
				var hasErrors = this.page.diagnosesPanel && this.page.diagnosesPanel.hasErrors();
	...
	onInputValidate: function(event, notify) {
		try {
			...
			if (notify || (newValue != this.currentValue)) {
				if (this.$field.$x3Format) {
					newValue = formatter.format(newValue, this.$field.$x3Format, this.$field.$type, this.page.$prototype.$x3FormatExt);
				}
