...
exports.Field = helpers.defineClass(Field, null, {
	...
	_appendLinksPicker: function() {
		if (this.$rootLinks.$links) {
			...
			if (this.$rootLinks.$links.$lookup) {
				...
				picker.setAttribute("title", this.getLocalize().fieldLookup);
			}
			if (this.$rootLinks.$links.$tunnel) {
				this.tunnelPicker = this._appendSpecialPicker("tunnel");
				this.tunnelPicker.setAttribute("title", this.getLocalize().fieldTunnel);
			}
		}
	},
	...
	onFieldMouseEvent: function(event) {
		...
		if (this.cellDiagnose) {
			switch (event.type) {
				case "mouseenter":
					if (self.cellDiagnose && self.cellDiagnose.$$item) {
						var diagPopup = self.page.openDialog({
							content: self.cellDiagnose,
							$dialogMode: "popup",
							$$dialog: self.cellDiagnose.$$item,
							position: {
								my: "left top",
								at: "left bottom",
								of: self.$$item
						...
						self.$$item[0].setAttribute("data-s-celldiag-popup", diagPopup.id);
						self._diagPopups = self.diagPopups || {};
						self._diagPopups[diagPopup.id] = diagPopup;
					}
					break;
				case "mouseleave":
					var id = self.$$item[0].getAttribute("data-s-celldiag-popup");
					// close on mouse leave, and if mouse is not over popup
					if (id) {
						if (self._diagPopups && self._diagPopups[id]) {
							self._diagPopups[id].close();
							self._diagPopups[id] = null;
						}
					}
					break;
			}
	...
	getDefaultTitle: function() {
		return this.getLocalize().fieldDefaultTitle;
	},
	getTitle: function() {
	...
	setState: function(state) {
		...
		if (state.$isHidden !== undefined) {
			...
			if (this.$item.$inplace) {
				this.$$dataValue[0].style.display = this.$isHidden ? "none" : "";
			} else {
				if (this.layoutParent) {
					this.layoutParent.ensureItemVisibility(this);
				}
				this.$$item[0].style.display = this.$isHidden ? "none" : "";
				...
				if (metaData.$links) {
					if (this.$isEditMode) {
						if (metaData.$links.$lookup) {
							//to improve not good
							if (!this.$lookup) {
								if (this.$rootLinks.$links && this.$rootLinks.$links.$lookup) {
									this.$lookup = helpers.object.clone(this.$rootLinks.$links.$lookup, true);
								} else {
									this.$lookup = metaData.$links.$lookup;
								}
							} else {
								this.page.deltaManager.applyObjectDelta(this.$lookup, metaData.$links.$lookup);
							}
						}
					} else {
	...
	validate: function(newValue, errors) {
		...
		if (this.$isMandatory && value == '' && this.page.externalAdapter.notifyFldMandatoryErr()) {
			errors.push(this.getLocalize().fieldIsMandatory);
		}
		...
		if (dataType.$pattern && value) {
			...
			if (!match) { //(match && match.index == 0 && match[0].length == value.length)) {
				errors.push(this.getLocalize().fieldInvalidValue + " : " + value);
			}
		}
		...
		if (errors.length > 0 || this.$displayedDiagnoses) {
			...
			if (saveLinks && saveLinks.length > 0) {
				if (!saveLinks[0].$isDisabled) {
					...
					if (this.$isServerSaveEnabled && errors.length === 0) {
	...
	_createMenusBox: function() {
		this.menusBox = this.page.loadNewItem(this._input ? this.fieldValue : this._dataValue, {
			$isOwner: true,
			$noText: true,
			$title: this.getLocalize().fieldActions,
	...
	dispose: function() {
		...
		this._core = this.$$input = this._input = this.currentValue = this.menusBox = this.cellDiagnose = null;
		this._autoCompleteList = null;
