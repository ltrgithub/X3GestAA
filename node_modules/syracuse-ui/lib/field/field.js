...
exports.Field = helpers.defineClass(Field, null, {
    ...
    loadBox: function($$item){
        ...
        if (this.$item.$bind == "exportedObjects") {
            debugger;
        }
    },
    ...
    _appendSpecialPicker: function(pickerType){
        ...
        if (this.$item.$inplace && !this.$item.$isFilterMode) {
            this._ensureContextMenu().appendPicker(picker);
        }
        else {
    ...
    _ensureMenuPickerVisible: function(){
        if (this.contextMenu && this.contextMenu.menuPicker) {
            this.contextMenu.menuPicker.style.display = (this.$isEditMode || (this.currentValue != null)) ? "" : "none";
        }
    },
    ...
    setState: function(state){
        ...
        if (state.$isHidden !== undefined) {
            ...
            if (this.$item.$inplace) {
                ...
                this.$$item[0].style.display = state.$isHidden ? "none" : "";
            }
        }
    },
    ...
    showErrors: function(errors, options){
        fieldDiagnoseUtils.showErrors(this, errors, options);
    },
    showDiagnoses: function(diagnoses, options){
        fieldDiagnoseUtils.showDiagnoses(this, diagnoses, options);
    },
        ...
        if (errors.length > 0 || this.$displayedDiagnoses) {
            ...
            if (saveLinks && saveLinks.length > 0) {
                ...
                if (this.$isServerSaveEnabled !== undefined) {
                    this.articleParent.applyActionLinkChange({
                        $actions: {
                            $save: {
    ...
    setMenus: function(metaData){
        if (!this.$item.$isFilterMode && !this.$item.$isExpressionChild) {
            ...
            if ($menus.$links !== undefined) {
                this._applyMenuChange(metaData.$links);
            }
            if ($menus.$actions !== undefined) {
            ...
            if ($menus) {
                ...
                if (count == 0) {
                    ...
                    for (var ii = 0; ii < count; ii++) {
                        ...
                        switch ($bind) {
                            case "$details":
                                if (this && this.$isDetailLinkIgnore) {
    ...
    _addToContextMenu: function($menu, $isAction, $bind, article){
        $menu.$bind = ($menu.$sourceBind = $bind) + "-" + this.id;
        this._ensureContextMenu();
