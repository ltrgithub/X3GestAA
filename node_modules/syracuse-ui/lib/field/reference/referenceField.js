...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
		...
		if (value !== undefined) {
			...
			if (value) {
				...
				if (this.uuidEnabled) {
					...
					if (this.$item.$value) {
						this.currentValue.$value = syra_expression.parse(this.articleParent, this.$item.$value, value);
					} else {
	...
	onItemInOut: function(on, event, target) {
		...
		if (this.$item.$openBy == "over" && this.picker_$lookup) {
			syra_button.delayClick(this.picker_$lookup, on);
		}
	},
