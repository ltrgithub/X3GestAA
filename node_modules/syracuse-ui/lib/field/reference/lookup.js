					...
					if (self.field) {
						...
						self.page.fetch(null, function(data, response, requestUrl) {
							if (self.field) {
								...
								var link;
								var $valueBind = self.field.uuidEnabled ? "$value" : self.field.$reference.$value.$itemProp;
								for (var ii = 0, jj = records.length; ii < jj; ii++) {
									var record = records[ii];
									link = self.list.addAnchor();
									link.textContent = record[$valueBind];
									link.syraRecord = ii;
								...
								self._popup = syra_over.openPopup(syra_site, {
									...
									position: position,
									onresize: function(popup, maxHeight) {
										if (self.list) {
											self.list.resize(maxHeight - self.detailsLink.clientHeight);
										}
									},
									close: function() {
