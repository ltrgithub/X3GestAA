...
exports.XMLViewer = helpers.defineClass(XMLViewer, null, {
			...
			if (type == 'object') {
				...
				if (value.constructor == Array) {
					...
					if (!(fieldCount == 1 && value.$) || (fieldCount > 1)) {
						...
						collapsableSlot = syra_dom.div("s-xml-collapsible", parent);
	...
	addExpandDiv: function(slot, element) {
		var self = this;
		var expandedDiv = syra_dom.div("s-xml-expanded", slot);
		//Add line
		var line = syra_dom.div("s-xml-line", expandedDiv);
		//Collapse button
		...
		var content = syra_dom.div("s-xml-collapsible-content", expandedDiv);
		this.addEndTag(element, syra_dom.div("s-xml-line", expandedDiv));
		return content;
	},
	addCollapsedDiv: function(slot, element) {
		var self = this;
		//Add the collapsed DIV
		var collapsedDiv = syra_dom.div("s-xml-collapsed s-xml-hidden", slot);
		var line = syra_dom.div("s-xml-line");
		//Expand button
		var btn = syra_dom.span("s-xml-btn", line);
		btn.innerHTML = "&#43;"
		btn.onclick = function() {
	...
	addXmlTag: function(parent, collapsableSlot, element) {
		...
		if (element.isCollapsable) {
			...
			var line = syra_dom.div("s-xml-line", parent)
			this.addBeginTag(element, line);
