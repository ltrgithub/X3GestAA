...
exports.RichDataVignetteField = helpers.defineClass(RichDataVignetteField, VignetteField, {
	...
	setDataBind: function(value, record, metaData, $bind) {
		if (metaData && metaData.$title !== undefined) {
	...
	resizeItem: function(doResize) {
		if (this.$item.$height) {
			this._dataValue.style.height = this.$item.$height + "px";
		} else {
			this._dataValue.style.height = this.page.size.height * 0.3 + "px";
		}
	},
	loadBox: function() {
		VignetteField.prototype.loadBox.call(this);
		this._dataValue = syra_dom.addDiv("s-richdata-viewer", this.body);
	},
	renderData: function() {
		if (this.currentValue) {
			this._dataValue.style.height = this.page.scrollview.clientHeight * 0.3 + "px";
			this.discoverViewer();
		} else {
