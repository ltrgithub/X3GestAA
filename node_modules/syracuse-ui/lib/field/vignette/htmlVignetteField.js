...
var HtmlVignetteAuthoring = require("syracuse-ui/lib/authoring/field/htmlVignetteAuthoring").HtmlVignetteAuthoring;
...
exports.HTMLVignetteField = helpers.defineClass(HTMLVignetteField, VignetteField, {
	getAuthoringWidget: function(designedField) {
		if (!this.awAddin) {
			this.awAddin = new HtmlVignetteAuthoring();
	...
	setDataBind: function(value, record, metaData, $bind) {
		if (this.page.externalAdapter.setDataBind(this, value, record, metaData)) {
			if (value !== undefined) {
				this.$item.$url = value;
				if (this._iframe) this._iframe.setAttribute("src", value);
			}
	...
	applyDesignMetaData: function(metaData, onAuthoring) {
		...
		if (metaData.$url) {
			...
			if (this.domTitle && this.$item.$url) {
				this.domTitle.setAttribute("href", this.$item.$url);
			}
		}
		if (metaData.$width) {
	...
	onWindowResize: function() {
		...
		this._iframe.style.height = (maxHeight - ($$scrollview[0].scrollHeight - maxHeight)) + "px";
	},
	...
	renderLayoutContent: function() {
		...
		this._iframe.setAttribute("target", "_top");
		this._iframe.style.width = this.$item.$width || "100%";
		this._iframe.style.height = this.$item.$height || document.body.clientHeight * 0.7 + "px";
