...
exports.HTMLVignetteField = helpers.defineClass(HTMLVignetteField, VignetteField, {
	...
	applyDesignMeta: function(metaData, designing) {
		VignetteField.prototype.applyDesignMeta.call(this, metaData, designing);
		...
		if (syra_dom.isVisible(this.domItem)) {
			...
			if (this._iframe) {
				if (this.isMaximized) {
					...
					var slot = this.page.layoutSlot;
					var rectHeader = this.header.getBoundingClientRect();
					var diff = rectHeader.height + (2 * (rectHeader.top - slot.getBoundingClientRect().top));
					var rectFrame = this._iframe.getBoundingClientRect();
					var maxHeight = slot.getBoundingClientRect().height - diff - 3 - (rectFrame.top - rectHeader.bottom);
					this._iframe.style.height = maxHeight + "px";
				} else {
					if (this.$item.$height) {
						this._setDefinedHeight();
					} else {
						this._iframe.style.height = this.page.size.bodyHeight + "px";
						var diff = this.page.scrollview.scrollHeight - this.page.scrollview.clientHeight;
						if (diff) {
							this._iframe.style.height = Math.max(Math.floor(this.page.size.bodyHeight / 2), this.page.size.bodyHeight - diff) + "px";
						}
					}
	...
	_showUrl: function($url) {
		...
		if (this.$isLoaded && syra_dom.isVisible(this.domItem)) {
			if (this.$contentUrl) {
				...
				this.externalBtn && syra_button.remove(this.externalBtn);
				this.externalBtn = syra_button.add({
					parent: this,
					text: this.titleText,
