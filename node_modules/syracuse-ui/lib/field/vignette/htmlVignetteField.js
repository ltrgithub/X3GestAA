			...
			if (this._iframe) {
				if (this.isMaximized) {
					...
					if (this.$item.$height) {
						...
						this.body.style.height = this.page.size.bodyHeight + "px";
						var diff = this.page.scrollview.scrollHeight - this.page.scrollview.clientHeight;
						if (diff) {
							this.body.style.height = Math.max(Math.floor(this.page.size.bodyHeight / 2), this.page.size.bodyHeight - diff) + "px";
						}
					}
				}
			}
