...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
	...
	_fillSelectedEnum: function($selectedEnum) {
		...
		if (this.field.$isEditMode) {
			...
			if ($title && this.field.$rootLinks.$links && this.field.$rootLinks.$links.$details && !this.field.$item.$isMenusDisabled) {
	...
	renderLayout: function(filter) {
		...
		if (this._listBody) {
			...
			var maxByCol;
			if (this.field.$enum.length > 6) {
				$choiceLayout = Math.min(Math.ceil(this.field.$enum.length / 6), 4);
			...
			for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
				...
				if (html) {
					...
					choice.link.setAttribute("data-s-btn", "field-choice");
					if ($icon) {
		...
		switch (event.keyCode) {
			...
			case keyHelper.keyCode.UP:
				if (!self.field.$isReadOnly) {
					...
					this._selectItem(self._choices[self._$selectedListIndex].$index, true);
				}
				break;
			...
			case keyHelper.keyCode.F4:
			case keyHelper.keyCode.F5:
			case keyHelper.keyCode.F6:
			case keyHelper.keyCode.F7:
			case keyHelper.keyCode.F9:
			case keyHelper.keyCode.F11:
			case keyHelper.keyCode.F12:
			case keyHelper.keyCode.CAPSLOCK:
