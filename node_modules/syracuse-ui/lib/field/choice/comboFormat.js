...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
	...
	render: function() {
		...
		if (this.field.$isEditMode) {
			...
			document.site.setSpecificAttributes(this.field.input);
			...
			this.choiceBtn = this.field.addPicker("onChoiceClick", "onChoiceClick");
	...
	_selectChoice: function(selectedIndex) {
		if (this._choices) {
			for (var ii = 0, jj = this._choices.length; ii < jj; ii++) {
				...
				if (choice.link) {
					document.site.dom.toggleClass(choice.link, "s-selected", choice.$index == selectedIndex);
				}
			}
		}
	},
	...
	renderLayout: function(filter) {
		...
		this._choices = [];
		if (this._listBody) {
			document.site.dom.empty(this._listBody);
	...
	toggleList: function(show, filter) {
		...
		if (!self._popup && show !== false) {
			...
			self._popup = document.site.dialogManager.openPopup(self.field.boxParent, {
				content: self.field,
				...
				onClose: function() {
					setTimeout(function() {
						document.site.dom.removeChild(self._list);
