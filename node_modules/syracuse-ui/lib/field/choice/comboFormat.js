...
function _addChoiceItem(field, index, filter, cols) {
	...
	if (html != null) {
		...
		choice.button = syra_menus.button.add({
			parent: field,
		...
		if (field.$item.$icon) {
			choice.button.link.className += " s-choice-icon-item";
			_setIcon(field, choice.button.link, $enum.$value);
			syra_menus.button.setText(choice.button, html);
		} else {
			if (field.$item.$colorChoice) {
				...
				choice.button.link.appendChild(div);

			} else {
				choice.button.link.innerHTML = html;
			}
		}
...
exports.render = function(field) {
	...
	if (field.$isEditMode) {
		...
		if (field.$item.$icon) {
			...
			field.iconValue = document.createElement("div");
			field.iconValue.className = "s-choice-icon";
			_setIcon(field, field.iconValue, field.currentValue);

			$(field._dataValue).prepend(field.iconValue);
			field.iconValue.appendChild(field.input);
