...
function _addChoiceItem(field, index, filter, cols) {
	...
	if (html != null) {
		...
		if (field.$item.$icon) {
			choice.link.className += " s-field-choice-icon-item";
			_setIcon(field, choice.link, $enum.$value);
			syra_menus.setButtonTitle(choice.link, html);
		} else {
			if (field.$item.$colorChoice) {
				choice.link.className += " s-field-choice-color-item";
				var div = document.createElement("div");
				div.className = "s-field-choice-color-value";
...
exports.render = function(field) {
	...
	if (field.$isEditMode) {
		...
		if (field.$item.$icon) {
			...
			field.iconValue.className = field.$skin + "-choice-icon";
			...
			field.input.className = "s-field-choice-icon-input";
		}
		if (field.$item.$colorChoice) {
			field.mnPickers.choice.className += " s-icon";
			field.cssValue = document.createElement("div");
			field.cssValue.className = "s-field-choice-color-value";
			field.cssValue.style.backgroundColor = field.currentValue;
			$(field._dataValue).prepend(field.cssValue);
