...
function _setItemText(field, $enum, filter) {
	if (filter) {
		/*var exp = (field.$field.$isAutoComplete === false) ? ("^" + filter) : "(" + filter + ")";
         if ((new RegExp(exp, "ig")).test($enum.$title)) {
         var className = field.currentValue && $enum.$value == field.currentValue ? 's-auto-complete-filter selected' : 's-auto-complete-filter';
         return $enum.$title.replace(new RegExp(exp, "ig"), "<strong class='" + className + "'>$1</strong>");
         }	*/
		var exp = (field.$field.$isAutoComplete === false) ? ("^" + filter) : "(" + filter + ")";
...
function _onChoiceItemClick(combo, field, btn) {
	...
	if (index !== undefined && (!field.$isDisabled && !field.$isReadOnly)) {
	...
	if (shortcuts.enter) {
		...
		if (!shortcuts.esc) {
			if (field.$field.$isAutoComplete === false) {
				var choices = combo._choices;
				if (choices && choices.length && combo._$selectedListIndex !== undefined && combo._$selectedListIndex >= 0 && _isPopupOpened(combo)) {
					_onChoiceItemClick(combo, field, choices[combo._$selectedListIndex]);
					return true;
				}
			} else {
