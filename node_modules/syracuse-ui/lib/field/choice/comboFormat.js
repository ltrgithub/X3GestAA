...
var _modes = {
	string: {
		setValue: function(field, $selectedEnum) {
			field.setInputValue($selectedEnum ? $selectedEnum.$title : "");
		},
	...
	icon: {
		setValue: function(field, $selectedEnum) {
			field.setInputValue($selectedEnum ? $selectedEnum.$title : "");
			syra_button.setText(field.iconValue, $selectedEnum ? $selectedEnum.$title : "", this.setImageName(field, $selectedEnum && $selectedEnum.$value));
	...
	colorChoice: {
		setValue: function(field, $selectedEnum) {
			field.setInputValue($selectedEnum ? $selectedEnum.$title : "");
			field.cssValue.style.backgroundColor = $selectedEnum ? $selectedEnum.$value : null;
		},
		addItem: function(field, $btn, $enum) {
			$btn.text = "";
			var btn = syra_button.add($btn);
			btn.link.className += " s-choice-color-item-link";
			var div = document.createElement("div");
			div.className = "s-choice-color-item";
			div.style.backgroundColor = $enum.$value;
			btn.link.appendChild(div);
			return btn;
		},
		addInput: function(field) {
			field.cssValue = document.createElement("div");
			field.cssValue.className = "s-choice-color-item s-choice-color-value";
			field.cssValue.style.backgroundColor = field.currentValue;
...
function _toggleList(field, show, filter) {
	if (!field.popupPicker && show !== false) {
		...
		field._list = syra_dom.div("s-mn-popup-body " + (field.$item.$css ? field.$item.$css : ""));
...
exports.render = function(field) {
	...
	field.fillSelectedEnum = function($selectedEnum) {
		if (this.$isEditMode) {
			...
			if ($title && !this.$isDetailLinkDisabled && this.$menus && this.$menus.$details) {
				syra_fields.addDetailLink(this, $selectedEnum.$value, this.$menus.$details);
			} else {
				this._dataValue.textContent = this.displayText = $title;
			}
		}
	};
	...
	if (field.$item.$colorChoice) {
		field.comboMode = _modes.colorChoice;
	}
	...
	if (field.$isEditMode) {
		...
		field.comboMode.addInput(field);
