	...
	icon: {
		setValue: function(field, $enum) {
			...
			if (field.$isEditMode) {
				...
				syra_button.setText(field.iconValue, $title, this.setImageName(field, $enum && $enum.$value));
	...
	indicator: {
		setValue: function(field, $enum) {
			...
			if (field.isColorChoice) {
				field.cssValue.style.backgroundColor = value;
			} else {
				field.cssValue.className = field.$cssValue + " s-" + field.$item.$indicator + "-" + value;
			}
			if (field.$isEditMode) {
				field.setInputValue($title);
			} else {
		...
		addValue: function(field) {
			if (field.$isEditMode) {
				if (field.isColorChoice) {
					field.cssValue = syra_dom.div("s-choice-color-item s-choice-indic-value");
					field.cssValue.appendChild(field.input);
					field.input.className = "s-choice-icon-input";
				} else {
					field.cssValue = syra_dom.div(field.$cssValue = "s-choice-indic-value");
				}
			} else {
				field.cssValue = syra_dom.div(field.$cssValue = "s-choice-indic-item");
			}
			field._dataValue.insertBefore(field.cssValue, field._dataValue.firstChild);
		},
		setWidth: function(field) {
...
function _addChoiceItem(field, index, filter, cols) {
	...
	if (text != null) {
		...
		if (isHTML) {
			choice.button.link.innerHTML = syra_dom.escape(text);
		}
...
exports.renderLayout = function(field, filter) {
	...
	if (field._list) {
		...
		if ($choiceLayout != "row") {
			...
			cols = [];
			field._table = syra_dom.div("s-choice-list-table", field._list);
			for (var ii = 0, colCount = $choiceLayout; ii < colCount; ii++) {
				cols[ii] = syra_dom.div("s-choice-list-col", field._table);
			}
		}
...
function _filterInput(field, charCode, isSelectDisabled, event) {
	if (!field.$isDisabled && !field.$isReadOnly) {
		...
		field._autoCompleteTimer = event && event.syraPlayback ? _insideFilter() : setTimeout(_insideFilter, 100);
	}
}
...
function _toggleList(field, show, filter, event) {
	if (!field.popupPicker && show !== false) {
		if (field.page.focusField != field) {
			field.focus();
		}
		field.domItem.id = field.id;
		field._list = syra_dom.div(field.$item.$css ? field.$item.$css : "");
		...
		var position = field.$item.$popupRight ? {
			my: "left top",
			at: "right top"
		} : {
			my: "left top",
		...
		field.popupPicker = syra_over.openPopup(field.boxParent, {
			...
			onresize: function(popup) {
				if (field.input) {
					...
					var rect = popup.overSlot.getBoundingClientRect();
					field._list.style.minWidth = minWidth;
					var diff = field._list.scrollHeight - field._list.clientHeight;
					if (diff) {
						popup.overSlot.style.height = Math.min(syra_site.getSize().overPanel.maxHeight, rect.height + diff) + "px";
					}
					if (field._table) {
...
exports.dispose = function(field) {
	if (field) {
		...
		field.comboMode = field.getValue = field.applyEscape = field.applyShortCut = field.validateKeyPress = null;
	}
};
...
exports.render = function(field) {
	...
	field.validateKeyPress = function(charCode, event) {
		if (event.keyCode != 13) {
			_filterInput(this, charCode, undefined, event);
			event.preventDefault();
			event.stopPropagation();
		}
		return true;
	...
	field.$skinListItem = field.$item.$skinListItem || "s-popup-btn-choice";
	...
	if (field.$isEditMode) {
		...
		if (field.$item.$contentEditable) {
			field.input = syra_dom.addEditable(css, field._dataValue);
		} else {
			field.input = syra_dom.input(css, field._dataValue);
		}
