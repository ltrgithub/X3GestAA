...
var _modes = {
	...
	icon: {
		setValue: function(field, $selectedEnum) {
			field.setInputValue($selectedEnum ? $selectedEnum.$title : "");
			syra_menus.button.setText(field.iconValue, $selectedEnum ? $selectedEnum.$title : "", this.setImageName(field, $selectedEnum && $selectedEnum.$value));
...
function _addChoiceItem(field, index, filter, cols) {
	...
	if (text != null) {
		if (text == "") {
			text = syra_local.fieldChoiceEmpty;
		}
...
function _filterInput(field, event, charCode, isSelectDisabled) {
	if (!field.$isDisabled && !field.$isReadOnly) {
		...
		field._autoCompleteTimer = setTimeout(function() {
			...
			if (!isSelectDisabled && field._choices.length == 1) {
				...
				_toggleList(field, true, filter, event);
			}
		...
		if (shortcuts.enter || shortcuts.tab) {
			...
			if (field._choices.length > 0) {
				...
				if (field._choices.length == 1) {
					ret = _toggleList(field, false, undefined, event);
...
exports.render = function(field) {
	...
	field.applyEscape = function(shortcuts, event) {
		...
		var closed = _toggleList(this, false, undefined, event);
	...
	field.validateKeyPress = function(charCode, event) {
		if (event.keyCode != 13) {
			_filterInput(this, event, charCode);
			event.preventDefault();
			event.stopPropagation();
		}
		return true;
