...
function _setIcon(combo, dom, $value) {
	var $path = syra_site.$iconPath + (combo.$icon.$path || "");
...
function _addChoiceItem(combo, field, index, filter, cols) {
	...
	if (html != null) {
		...
		if (combo.$selectedEnum && index == combo.$selectedEnum.$index) {
		...
		if (syra_checker.enableTestRobot) {
			choice.link.setAttribute("data-s-picker2", "field-choice");
		}
...
function _endInputFilter(combo, field) {
	clearTimeout(combo._autoCompleteTimer);
	var closed = _toggleList(combo, field, false);
	delete combo.autoCompleteFilter;
	return closed;
}
...
function _onChoiceItemClick(combo, field, btn) {
	...
	if (index !== undefined && (!field.$isDisabled && (!field.$isReadOnly || field.$field.$isAutoComplete === false))) {
		...
		field.setInputValue(combo.$selectedEnum.$title);
		$(field.input).change();
		if (combo.iconValue) {
			_setIcon(combo, combo.iconValue, combo.$selectedEnum ? combo.$selectedEnum.$value : null);
		}
		if (combo.cssValue) {
			_setCssValue(field, combo.cssValue, combo.$selectedEnum ? combo.$selectedEnum.$value : null);
		}
...
function _filterInput(combo, field, charCode, isSelectDisabled) {
	clearTimeout(combo._autoCompleteTimer);
	if (field.$field.$isAutoComplete === false) {
		combo.autoCompleteFilter = (combo.autoCompleteFilter || "") + charCode;
	}
	combo._autoCompleteTimer = setTimeout(function() {
		var filter;
		if (combo.autoCompleteFilter !== undefined) {
			filter = combo.autoCompleteFilter;
		} else {
			filter = field.input.value;
		}
		_renderLayout(combo, field, filter);
		if (!isSelectDisabled && combo._choices.length == 1) {
			_onChoiceItemClick(combo, field, combo._choices[0]);
			_toggleList(combo, field, false);
		} else {
			_toggleList(combo, field, true, filter);
		}

	}, 100);
}

function _applyShortCut(combo, field, shortcuts, event) {
	if (shortcuts.backspace) {
		if (combo.autoCompleteFilter) {
			combo.autoCompleteFilter = combo.autoCompleteFilter.slice(0, combo.autoCompleteFilter.length - 1);
		}
		_filterInput(combo, field, "", true);
		if (field.$field.$isAutoComplete === false) {
	...
	if (shortcuts.down || shortcuts.up) {
		...
		if (isOpened) {
			...
			if (shortcuts.down) {
				current++;
				if (current >= combo._choices.length) {
					current = 0;
				}
			} else {
				current--;
				if (current < 0) {
					current = combo._choices.length - 1;
				}
			}
		}
		if (!combo._choices.length)
			return;
		combo._$selectedListIndex = Math.max(current, 0);
	...
	if (shortcuts.enter) {
		...
		if (combo._choices.length > 0) {
			...
			if (combo._choices.length == 1) {
				return _toggleList(combo, field, false);
			}
		}
		return false;
	}
...
function _toggleList(combo, field, show, filter) {
	if (!combo.popupPicker && show !== false) {
		...
		_renderLayout(combo, field, filter);
		if (combo.$selectedEnum) {
			combo._selectItem(combo.$selectedEnum.$index, true);
		}
		combo.popupPicker = syra_site.dialogManager.openPopup(field.boxParent, {
			...
			onClose: function() {
				setTimeout(function() {
					if (field && field.input) { //need test on field input for list pagging combo 
						field.input.value = combo.$selectedEnum ? combo.$selectedEnum.$title : "";
...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
	setDataValue: function(value) {
		...
		_fillSelectedEnum(this, this.field, this.$selectedEnum = this.field.findEnum(value));
	},
	getDataValue: function() {
		if (this.field.$isEditMode && this.field.$field.$isAutoComplete !== false) {
	...
	onInputFocusout: function() {
		//_endInputFilter(this, this.field);
	},
	applyEscape: function() {
		return _endInputFilter(this, this.field);
	},
	validateKeyPress: function(charCode, event) {
		if (event.keyCode != 13) {
			_filterInput(this, this.field, charCode);
		}
		return true;
	},
	applyShortCut: function(shortcuts, event) {
		return _applyShortCut(this, this.field, shortcuts, event);
	},
	render: function() {
		//this.field.$field.$isAutoComplete = false;
		...
		if (this.field.$isEditMode) {
			...
			this.field._dataValue.className += " s-combo";
			...
			this.field._dataValue.appendChild(this.field.input);
			if (!this.field.mnPickers.choice) {
				syra_menus.addFieldPicker(this.field, "choice", syra_local.fieldOpenChoiceList, "SHIFT DOWN");
			}
			if (this.field.$item.$icon) {
				...
				$(this.field._dataValue).prepend(this.iconValue);
			...
			if (this.field.$item.$cssMode) {
				...
				this.cssValue.syraCss = this.field.$skin + "-choice-cssmode " + this.field.$item.$cssMode.$css;
				this.field._dataValue.insertBefore(this.cssValue, this.field._dataValue.firstChild);
	...
	_selectItem: function($index, select) {
		if (this.$selectedEnum) {
			delete this.$selectedEnum;
		}
		if (select) {
			this.$selectedEnum = this.field.$enum[$index];
			if (this.$selectedEnum) {
				this._selectChoice(this.$selectedEnum.$index = $index);
			}
		}
	},
