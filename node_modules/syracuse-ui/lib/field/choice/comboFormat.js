...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
    ...
    render: function(){
        ...
        if (this.field.$isEditMode) {
            (this.field.$$input = $("<input type='text'/>"))[0].className = this.field.$skinInput;
            this.field._appendLinksPicker();
            this.choiceBtn = this.field._appendPicker("choice");
            var div = document.createElement("div");
            div.className = this.field.$skin + "-choice-list";
            this._$$list = $(div).hide();
            this.field.$$fieldValue.append(this._$$list);
            var input = this.field.$$input[0];
            if (this.field.$item.$icon) {
                this.iconValue = this.createIcon(this.field.currentValue, this.field.$skin + "-choice-icon");
                $(this.field._boxPicker).prepend(this.iconValue);
                this.iconValue.top = "2px";
                this.iconValue.left = "2px";
                var iconHeight = parseInt(this.$icon.$height.replace("px", ""));
                this.choiceBtn.style.top = Math.ceil(iconHeight / 4) + "px";
                input.style.paddingLeft = (parseInt(this.$icon.$width.replace("px", "")) + 8) + "px";
                input.style.paddingRight = (this.field._boxPickerPaddingRight - 20) + "px";
                input.style.height = (iconHeight + 4) + "px";
                if (this.field.$item.$icon.$inputMode == "icon") {
                    input.style.width = "0px";
                    input.style.visibility = "hidden";
                    this.field._boxPicker.className += " s-icon-mode";
                }
            }
    ...
    setEnum: function($enum){
		if (this._$selectedEnum && ($enum[this._$selectedEnum.$index]==null || this.field.currentValue != $enum[this._$selectedEnum.$index].$value)) {
			this._selectItem(this._$selectedEnum.$index, false);
			this.setDataValue("");
		}
        ...
        self.field.$enum.forEach(function($enum, index){
            ...
            if (html) {
                ...
                if (self.field.currentValue == $enum.$value) {
					console.log("selected-->" + self.field.currentValue );
