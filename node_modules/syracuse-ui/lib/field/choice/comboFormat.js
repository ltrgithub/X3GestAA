	...
	validateKeyPress: function(charCode, event) {
		...
		if (event.keyCode != 13) {
			...
			if (self.field.$field.$isAutoComplete === false) {
				self.autoCompleteFilter = (self.autoCompleteFilter || "") + charCode;
			}
			self._autoCompleteTimer = setTimeout(function() {
				var filter;
				if (self.autoCompleteFilter !== undefined) {
					filter = self.autoCompleteFilter;
				} else {
					filter = self.field.input.value;
				}
				_renderLayout(self, self.field, filter);
				if (self._choices.length == 1) {
					_onChoiceItemClick(self, self.field, self._choices[0]);
					_toggleList(self, self.field, false);
				} else {
					_toggleList(self, self.field, true, filter);
				}
