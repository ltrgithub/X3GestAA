...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
	...
	toggleList: function(show, filter) {
		var self = this;
		if (!self.popupPicker && show !== false) {
			...
			self.popupPicker = syra_site.dialogManager.openPopup(self.field.boxParent, {
				content: self.field,
				...
				onClose: function() {
					setTimeout(function() {
						if (self.field) {
							self.field.input.value = self.field.builder._$selectedEnum ? self.field.builder._$selectedEnum.$title : "";
							syra_site.dom.removeChild(self._list);
						}
						self.popupPicker = self._list = self._listBody = null;
					}, 200);
				}
			});
		} else {
