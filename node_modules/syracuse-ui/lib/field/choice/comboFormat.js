...
function _selectChoice(field, selectedIndex) {
	if (field._choices) {
		for (var ii = 0, jj = field._choices.length; ii < jj; ii++) {
			...
			if (choice.button) {
				syra_site.dom.toggleClass(choice.button.link, "s-selected", choice.$index == selectedIndex);
			}
		}
	}
}
...
function _onItemClick() {
	_onChoiceItemClick(this.parent, this.parent._choices[this.$index]);
}
...
function _addChoiceItem(field, index, filter, cols) {
	...
	if (text != null) {
		...
		if (isHTML) {
			choice.button.link.innerHTML = syra_site.dom.formatHTMLText(text);
		}
...
function _fillSelectedEnum(field, $selectedEnum) {
	if (field.$isEditMode) {
		field.comboMode.setValue(field, $selectedEnum);
	} else {
...
exports.dispose = function(field) {
	if (field) {
		...
		field.comboMode = field.getDataValue = field.onInputValueChanged = field.applyEscape = field.applyShortCut = field.validateKeyPress = null;
	}
};
