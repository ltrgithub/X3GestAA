...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
    ...
    render: function(){
        ...
        if (this.field.$isEditMode) {
            var input = (this.field.$$input = $("<input type='text'/>"))[0];
            input.className = this.field.$skinInput;
            this.field.$$fieldValue[0].appendChild(input);
            ...
            if (this.field.$item.$icon) {
				...
				input.style.paddingLeft = (parseInt(this.$icon.$width.replace("px", "")) + 2) + "px";
                ...
                if (this.field.$item.$icon.$inputMode == "icon") {
                    ...
                    this.field.$$fieldValue[0].className += " s-icon-mode";
                }
            }
