...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
	...
	_fillSelectedEnum: function($selectedEnum) {
		...
		if (this.field.$isEditMode) {
			...
			if ($title && !this.field.$isDetailLinkDisabled && this.field.$menus && this.field.$menus.$details) {
				this.field.appendDetailLink($title, this.field.$menus.$details);
			} else {
	...
	render: function() {
		...
		if (this.field.$isEditMode) {
			...
			if (!this.field.mnPickers.choice) {
				syra_menus.addPicker(this.field, "choice");
			}
			if (this.field.$item.$icon) {
				...
				this.field.input.className += " " + this.field.$skin + "-choice-icon-input";
	...
	onChoiceClick: function(event, picker) {
		if (picker.syraOnChoiceItemClick) {
			this.onChoiceItemClick(picker);
		} else {
			this.toggleList();
		}
	},
	onChoiceItemClick: function(picker) {
		var index = picker.$index || picker.syraChoice;
		if (index !== undefined && (!this.field.$isDisabled && !this.field.$isReadOnly)) {
			...
			if (this.field.page.focusField != this.field) {
				this.field.focus();
			}
		}
	},
	...
	renderLayout: function(filter) {
		...
		this._choices = [];
		if (this._listBody) {
			syra_site.dom.empty(this._listBody);
			...
			if (!isRowLayout) {
				...
				this._table.className = "s-choice-list-table";
			...
			for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
				...
				if (html) {
					...
					choice.link.syraOnClick = "onChoiceClick";
	...
	toggleList: function(show, filter) {
		...
		if (!self._popup && show !== false) {
			...
			self._list.className = "s-choice-list-popup";
			if (self.field.$item.$css) {
				self._list.className += " " + self.field.$item.$css;
			}
			self._list.syraItem = self.field.id;
			self.field.articleParent.setArticleId(self._list);
			self._list.style.display = "none";
			self._listBody = document.createElement("div");
			self._listBody.className = "s-choice-list-body";
			...
			self._popup = syra_site.dialogManager.openPopup(self.field.boxParent, {
				content: self.field,
				...
				position: {
					my: "left top",
					at: "left bottom",
					of: $(self.field.domValueSlot)
				},
				onresize: function(dialog) {
					if (self.field.input) {
						var parentRect = self.field.input.parentNode.getBoundingClientRect();
						...
						if (diff) {
							...
							self._listBody.style.height = dialog.dialogSlot.getBoundingClientRect().height + "px";
						}
						if (self._table) {
				...
				onClose: function() {
					setTimeout(function() {
						syra_site.dom.removeChild(self._list);
	...
	validateKeyPress: function(charCode, isEnter) {
		...
		if (self.field.$field.$isAutoComplete !== false) {
			self._autoCompleteTimer = setTimeout(function() {
				var filter = charCode || self.field.input.value || "";
				self.renderLayout(filter);
				if (self._choices.length > 0) {
					if (isEnter) {
						self.onChoiceItemClick(self._choices[0]);
						return false;
					}
				}
	...
	applyEscape: function() {
		this.field.input.value = this._$selectedEnum ? this._$selectedEnum.$title : "";
	...
	applyShortCuts: function(shortcurts, event) {
		...
		if (shortcurts.down || shortcurts.up) {
			if (!this.field.$isReadOnly) {
				...
				this.field.input.value = this.field.$enum[this._choices[this._$selectedListIndex].$index].$title;
			...
			if (!shortcurts.esc && shortcurts.enter) {
				if (this.field.$field.$isAutoComplete === false) {
					...
					if (choices && choices.length && this._$selectedListIndex !== undefined && this._$selectedListIndex >= 0 && this._popup && this._popup.isOpened) {
						this.onChoiceItemClick(choices[this._$selectedListIndex]);
						return true;
					}
				} else {
	...
	dispose: function(onApplyMetadata) {
		this._choices = this._table = this._list = this._listBody = this._$selectedEnum = null;
