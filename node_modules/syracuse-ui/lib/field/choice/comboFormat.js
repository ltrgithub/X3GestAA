...
exports.ComboFormat = helpers.defineClass(ComboFormat, null, {
    ...
    render: function(){
        ...
        if (this.field.$isEditMode) {
            ...
            this._$$list = $(this.field.fieldValue.appendChild(this._list));
    ...
    renderLayout: function(filter){
        ...
        var $skinItem = this.field.$skin + "-choice-list-item";
        ...
        for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
            ...
            if (html) {
                ...
                link.setAttribute("data-s-cb-index", ii);
                var $$item = $(link)
                if ($icon) {
                    $$item.append(this.createIcon($enum.$value, $skinItem + "-icon"));
                    if ($icon.$mode != "icon") {
                        $$item.append(html);
                    }
                    else {
        ...
        if (filter && this._choices.length == 0) {
            this._list.textContent = (locale.resources(module)()).cf_choiceFieldNoMatch;
        }
        this._setMinWidth();
    },
    ...
    onInputKeyup: function(input, event){
        ...
        switch (event.keyCode) {
            case keyHelper.keyCode.ESCAPE:
                input.value = this._$selectedEnum ? this._$selectedEnum.$title : "";
                this.toggleList(false);
                break;
            ...
            case keyHelper.keyCode.UP:
                ...
                input.value = (this._$listSelection = field.$enum[newIndex]).$title;
                break;
            ...
            default:
                ...
                if (this._choices.length > 0) {
                    ...
                    if (event.keyCode != keyHelper.keyCode.BACKSPACE) {
                        ...
                        input.value = this._$listSelection.$title;
                        this.selectText(input, filter.length, this._$listSelection.$title.length);
                    }
    ...
    releaseMode: function(){
        this._choices = this._table = this._$$list = this._list = this._$selectedEnum = this.choiceBtn = this._$listSelection = this._choices = null;
    },
