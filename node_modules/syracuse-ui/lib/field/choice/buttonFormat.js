...
exports.ButtonFormat = helpers.defineClass(ButtonFormat, null, {
	...
	setDataValue: function(value) {
		...
		var oldSelected = this.$selectedEnum;
		...
		if (this.field.$isEditMode) {
			...
			if (this.$selectedEnum) {
				syra_site.dom.toggleClass(this._choices[this.$selectedEnum.$index], "s-selected", true);
			}
		} else {
			var $title = this.$selectedEnum ? this.$selectedEnum.$title : "";
			this.field._dataValue.textContent = this.field.displayText = $title;
		}
	},
	...
	renderLayout: function() {
		...
		var enableTestRobot = syra_checker.enableTestRobot;
		for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
			...
			btn.syraChoice = ii;
			enableTestRobot && btn.setAttribute("data-s-picker2", "onChoiceClick");
			...
			if (this.field.$item.$icon) {
				...
				enableTestRobot && btn.setAttribute("data-s-picker2", "choice");
				btn.syraChoice = ii;
				btn.appendChild(img);
