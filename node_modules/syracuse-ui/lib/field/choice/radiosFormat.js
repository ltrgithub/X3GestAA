...
function _getChecked(format) {
	var index = -1;
	for (var ii = 0, jj = format._choices.length; ii < jj; ii++) {
		if (format._choices[ii].checkBox.input.checked) {
			index = ii;
			break;
		}
	}
...
exports.RadiosFormat = helpers.defineClass(RadiosFormat, null, {
	focus: function() {
		if (this.field.$isEditMode && !this.field.$isHidden && !this.field.$isDisabled) {
			this._choices[_getChecked(this)].checkBox.input.focus();
			return true;
		}
		return false;
	},
	...
	getDataValue: function(event) {
		var index = event.target ? event.target.syraEnumIndex : -1;
		if (index < 0) {
			index = _getChecked(this);
		}
	...
	dispose: function() {
		this._choices = this._currendSelected = this._choices = this.field = this.$selectedEnum = this._fieldset = null;
	}
});