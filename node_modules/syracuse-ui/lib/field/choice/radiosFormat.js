...
exports.renderLayout = function(field) {
	if (field.$isEditMode) {
		...
		if (!field._fieldset) {
			...
			field._fieldset = syra_dom.addDiv($skin, field._dataValue);
		} else {
		...
		if (isColLayout) {
			...
			for (var ii = 0, max = Math.ceil(field.$enum.length / colCount); ii < max; ii++) {
				rows[ii] = syra_dom.addDiv("s-field-choice-rd-row", field._fieldset);
			}
		}
		...
		if (isColLayout && curCol > 0) {
			while (curCol < colCount) {
				syra_dom.addDiv($skinItem, rows[curRow]);
				curCol++;
			}
		}
		rows = null;
