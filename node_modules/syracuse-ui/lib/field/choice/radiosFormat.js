...
exports.RadiosFormat = helpers.defineClass(RadiosFormat, null, {
	...
	setDataValue: function(value) {
		...
		if (this.field.$isEditMode) {
			if (this.$selectedEnum) {
				this._choices[this.$selectedEnum.$index].checked = true;
			} else
			if (value == null && $currSelectedEnum) {
				this._choices[$currSelectedEnum.$index].checked = false;
			}
			this._setSelectedState(this._choices[this.$selectedEnum ? this.$selectedEnum.$index : null]);
		} else {
	...
	renderLayout: function() {
		if (this.field.$isEditMode) {
			...
			if (!this._fieldset) {
				...
				this.field.fieldValue.className += " s-radios";
			...
			for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
				...
				this.field.setInputFieldId(input);
	...
	dispose: function() {
		...
		this._currendSelected = this._choices = this.choicesItems = this.field = this.$selectedEnum = this._fieldset = null;
	}
});