...
function _selectRadioBtn(format, index, selected) {
	if (selected) {
		if (format.selectedIndex >= 0 && format.selectedIndex != index) {
			syra_fields.checkbox.setValue(format._choices[format.selectedIndex], false);
		}
		format.selectedIndex = index;
	}
	if (index !== null) {
		syra_fields.checkbox.setValue(format._choices[index], selected);
	} else {
		format.selectedIndex >= 0 && syra_fields.checkbox.setValue(format._choices[format.selectedIndex], false);
...
exports.RadiosFormat = helpers.defineClass(RadiosFormat, null, {
	...
	onNotifyFieldChange: function(newValue) {
		if (this._choices) {
			...
			selected && _selectRadioBtn(this, selected.$index, true);
		}
	},
	setDataValue: function(value) {
		...
		if (this.field.$isEditMode) {
			if (this.$selectedEnum) {
				_selectRadioBtn(this, this.$selectedEnum.$index, true);
			} else {
				(value == null) && _selectRadioBtn(this, null, false);
			}
		} else {
			var $title = this.displayText = this.$selectedEnum ? this.$selectedEnum.$title : "";
	...
	onInputValidate: function(event) {
		if (event && event.target) {
			_selectRadioBtn(this, event.target.syraEnumIndex, true);
		}
	},
	...
	renderLayout: function() {
		if (this.field.$isEditMode) {
			...
			for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
				...
				syra_fields.checkbox.render(rdItem, id, this.$skin + "-radio", true);
				...
				if (this.field.$item.$icon) {
					var $path = syra_site.$iconPath + (this.$icon.$path || "");
