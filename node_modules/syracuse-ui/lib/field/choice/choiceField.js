...
exports.ChoiceField = helpers.defineClass(ChoiceField, Field, {
	...
	onChoiceClick: function(btn) {
		this.builder && this.builder.onChoiceClick && this.builder.onChoiceClick(btn);
	},
		...
		if (metaData && metaData.$value && metaData.$value.$enum !== undefined) {
			if (this.builder) {
				...
				if (this.currentValue != null) {
					this.setDataValue(this.currentValue, true);
				}
			}
		}
	...
	setDataValue: function(value, outOfDate) {
		this.builder.setDataValue(value, outOfDate);
	...
	applyEscape: function(shortcuts, event) {
		var ret = this.builder.applyEscape ? this.builder.applyEscape() : false;
		if (!ret) {
			ret = Field.prototype.applyEscape.call(this, shortcuts, event);
		}
		return ret;
	},
	applyShortCuts: function(shortcuts, event) {
		var applyed = this.builder.applyShortCuts ? this.builder.applyShortCuts(shortcuts, event) : false;
		if (!applyed) {
			applyed = Field.prototype.applyShortCuts.call(this, shortcuts, event);
		}
		return applyed;
	},
