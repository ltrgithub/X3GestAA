...
exports.ChoiceField = helpers.defineClass(ChoiceField, Field, {
	validateKeyPress: function(charCode, event) {
		if (this.builder.validateKeyPress) {
			return this.builder.validateKeyPress(charCode, event);
		}
		return true;
	},
	applyEscape: function(shortcuts, event) {
		return this.builder.applyEscape && this.builder.applyEscape(shortcuts, event);
	},
	applyShortCut: function(shortcuts, event) {
		return this.builder.applyShortCut && this.builder.applyShortCut(shortcuts, event);
	},
	onInputFocusout: function(input, event) {
		Field.prototype.onInputFocusout.call(this, input, event);
		this.builder.onInputFocusout && this.builder.onInputFocusout(input, event);
	},
	...
	onChoiceClick: function(event, btn) {
		this.builder && this.builder.onChoiceClick && this.builder.onChoiceClick(event, btn);
	},
	...
	getDataValue: function(event) {
		return this.builder.getDataValue(event);
	},
	getDisplayText: function() {
		return (this.builder.$selectedEnum && this.builder.$selectedEnum.$title) || "";
	},
	render: function() {
	...
	highlightOnEntrer: function(on, event, target) {
		if (this.builder.highlightOnEntrer) {
			this.builder.highlightOnEntrer(on, event, target);
		} else {
			Field.prototype.highlightOnEntrer.call(this, on, event, target);
		}
	},
	highlightOnFocus: function(on, event) {
		if (this.builder.highlightOnFocus) {
			this.builder.highlightOnFocus(on, event);
		} else {
	...
	dispose: function() {
		this.builder && this.builder.dispose();
