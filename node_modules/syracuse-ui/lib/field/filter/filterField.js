...
function _getOperators($type) {
	switch ($type) {
		...
		case "application/x-reference":
			return ["none", "gt", "ge", "lt", "le", "eq", "ne", "like_s", "like", "like_e", "between"];
			break;
		default:
			return ["none", "eq", "ne"];
			break;
...
exports.FilterField = helpers.defineClass(FilterField, null, {
	...
	addMenu: function() {
		...
		self._operator.style.backgroundImage = "url('" + syra_site.$item.$iconPath + "page/s-filter-none.png')";
		...
		$(self.labelAndMenu.appendChild(self._operator)).bind("click", function() {
			if (!self.listPopup) {
				...
				operators.forEach(function(op) {
					...
					link.style.backgroundImage = "url('" + syra_site.$item.$iconPath + "page/s-filter-" + op + ".png')";
					link.textContent = syra_local["filter_" + op];
					self.$$list[0].appendChild(link);
				});
				self.listPopup = syra_site.dialogManager.openPopup(self.filterBlock.articleParent, {
	...
	setOperator: function(operator) {
		...
		self._operator.style.backgroundImage = "url('" + syra_site.$item.$iconPath + "page/s-filter-" + (self.operator = operator) + ".png')";
	...
	getFieldValue: function(field) {
		...
		if (field.$isEditMode === true) {
			switch (self.$item.$type) {
				...
				case "application/x-datetime":
				case "application/x-time":
					value = field.getInputValue() === "" ? null : field.getDataValue();
					break;
				case "application/x-reference":
	...
	setFilterValues: function(operator, value, highValue) {
		...
		if (value != undefined) {
			...
			if (value.length > 1 && ((operator === "like_s" && value.charAt(value.length - 1) === "%") || (operator === "like_e" && value.charAt(0) === "%"))) {
				value = value.replace(/%/g, "");
			}
		}
			...
			if (value) {
				switch (this.$item.$type) {
					...
					case "application/x-time":
						...
						break;
					case "application/x-boolean":
						value = (value === "true") || (value === true) ? field.statusValues.on : field.statusValues.off;
						break;
					case "application/x-reference":
						// temporary fix for reference field in filter block widget - FK
	...
	dispose: function() {
		...
		this.layoutSlot = this.label = this.labelAndMenu = this.valuesSlot = null;
		this.fieldHigh = this.fieldLow = this.$$fieldHigh = this.$$fieldLow = null;
		this.$item = this._operator = this.filterBlock = this.$$list = null;
	},
});