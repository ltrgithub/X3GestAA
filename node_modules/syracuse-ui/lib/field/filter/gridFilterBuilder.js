...
var _operators = {
		...
		if (!field.operators) {
			field.operators = _filterMaker.getFieldOperators(field.$field.$type, field.$filterField && field.$filterField.$type, field.page.isFusionPage);
		}
	...
	setOperator: function(field, operator) {
		...
		syra_menus.button.setText(field.filterOperatorBtn, "", "filter_" + (field.$filterOperator = operator));
	},
	...
	field.page.externalAdapter.onFilterEvent({
		...
		doEvent: function() {
			...
			if (field.$filterOperator != "none") {
				...
				if (value != null && value != "") {
					field.setDataValue(null);
					syra_workingCopy.notifyFieldChange(field, null);
				} else {
					_triggerFilter(record);
				}
			}
...
function _setDataValue(record, field, value, operator) {
...
function _triggerFilter(record) {
	var filter = record.getValues().join(" and ");
...
function _loadField(record, slot, $field, $bind) {
	...
	field.filterOperatorBtn = syra_menus.button.add({
		parent: field,
...
function _setCriteria(field, criteria) {
	if (criteria.$operator === undefined) {
		criteria.$operator = _operators.getOperator(field, criteria.$value);
	}
	_operators.setOperator(field, criteria.$operator);
	if (field.isReferenceField) {
		var obj = {};
		obj[field.$filterBind] = criteria.$value;
		criteria.$value = obj;
	}
	field.setDataValue(criteria.$value);
};

function _fetchReferenceItem(where, field, keys) {
	keys.pop();
	var criteria = _filterMaker.parseWhere(where, keys);
	if (criteria) {
		if (criteria.$value.length == 0) {
			_setCriteria(field, criteria);
			...
			if (field.$menus && field.$menus.$lookup) {
				...
				syra_controller.callServer(field.page, {
					$location: {
						$url: $url,
					},
					params: {
...
function _setWhere(where, field) {
	...
	if (field.isReferenceField) {
		...
		criteria = _filterMaker.parseWhere(where, keys);
		if (!criteria && keys.length > 1) {
			_fetchReferenceItem(where, field, keys);
			return;
		}
		$isRefField = true;
	} else {
...
exports.addRecord = function(list) {
	...
	record.loadFilter = function() {
		...
		for (var ii = 0, jj = columns.length; ii < jj; ii++) {
			var col = columns[ii];
			if (col && col.$bind) {
				var field = this.addFieldCell(col);
				if (field) {
					if (this.where) {
						this.where && _setWhere(this.where, field);
						if (field.$filterOperator != "none") {
							if (this.fixed_where) {
								field.disable(true);
								syra_menus.button.disable(field.filterOperatorBtn, true);
							}
						}
					}
				}
	...
	record.addFieldCell = function(col) {
		var field;
		var td = this.cellsMap[col.$item.$bind] = document.createElement("td");
		td.className = "s-list-filter-cell";
		this[col.table.$rowKey].appendChild(td);
		col.$isHidden && syra_dom.hide(td, true);
		col.cssFilter = null;
		if (col.$field.$hasFilterCapability) {
			field = _loadField(this, td, col.$field, col.$item.$bind);
			this.fieldCellsMap[col.$item.$bind] = field;
	...
	record.validateDisplay = function(event) {
		...
		if (show && !this.list.toggleFilterBtn) {
			this.list.toggleFilterBtn = syra_menus.button.add({
				parent: this,
				...
				btnclick: function() {
					...
					for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
						var field = fields[$binds[ii]][0];
						if (!field.$isDisabled) {
							_setDataValue(record, field, null);
						}
					}
					record.page.externalAdapter.onFilterEvent({
						list: record.list,
		...
		syra_menus.button.hide(this.list.toggleFilterBtn, !show);
	};
	record.onNotifyDataChange = function(field, value) {
		_setDataValue(this, field, value);
		_triggerFilter(this);
		return false;
	};
	list.page.initializeNewItem(record, {
		$layout: {
...
exports.fusion = {
	...
	_getUIOrdereddBinds: function(list, filterRcd) {
		var ret;
		return ((ret = list.$userPreferences) && (ret = ret.$columns) && ret.length > 0) ? ret : this._getSortedFilterRcdFields(filterRcd);
	},
	...
	getNextQuickSelField: function(list, bind, nextSel) {
		...
		if (record.$item && record.$item.$layout && record.$item.$layout.$items) {
			if (nextSel === undefined) {
				...
				var cells = this._getUIOrdereddBinds(list, record);
