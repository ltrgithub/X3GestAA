...
exports.FilterBlock = helpers.defineClass(FilterBlock, Article, {
	...
	setSDataWhere: function(sdataWhere) {
		...
		function expIdentifierAndLiteral(exp) {
			if (!(exp.children.length === 2 && exp.children[0].type === "identifier" && exp.children[1].type === "literal")) return;
		...
		function expBetween(exp) {
			if (!(exp.children.length === 3 && exp.children[0].type === "identifier" && exp.children[1].type === "literal" && exp.children[2].type === "literal")) return;
			var field = exp.children[0].value;
		...
		Object.keys(self.filterFields).forEach(function(key) {
			if (matchedFields.indexOf("," + key + ",") < 0) self.filterFields[key].setFilterValues("none");
		});
	},
