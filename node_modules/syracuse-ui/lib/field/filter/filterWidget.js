...
exports.FilterWidget = helpers.defineClass(FilterWidget, Field, {
	loadBox: function($$item){
		...
		self.$skinField = self.$skin = self.$item.$skin || (self.$item.$inplace ? "s-inplace" : "s-field");
		
		self.$$item = $$item || $(self.layoutSlot);
        ...
        self.setTitle(self.$item.$title || self.$field.$title);
//		self.setTitle(self.getTitle());
    ...
    applyMetaData: function(metaData, options){
    	...
    	if (metaData)
    	{
    		...
    		//
			if(self.$isEditMode) {
				var p_url = self.articleParent.parseExpression((metaData.$links && metaData.$links.$prototype && metaData.$links.$prototype.$url) || 
						(self.$field.$links && self.$field.$links.$prototype && self.$field.$links.$prototype.$url));
    ...
    dispose: function() {
        ...
        if (self.diagSeverities) {
            self.diagSeverities = null;
        }
        self.layoutParent = self.authoringNode = self.boxParent = self.articleParent = null;
        self.page = self.boxPickers = self.domValueSlot = null;
        self.$field = self.$item = self.boxParent = null;
        self.layoutSlot = self.$$item = self._domItem = self.$$dataValue = self._dataValue = null;
        self._core = self.$$input = self._input = self.currentValue = null;
        
        if (self.filterBlock) {
        	self.filterBlock.dispose();
        }
    }
});
