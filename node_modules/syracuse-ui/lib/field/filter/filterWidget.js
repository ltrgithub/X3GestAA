...
// this.domItem  = topmost DOM element of this widget
// 
exports.FilterWidget = helpers.defineClass(FilterWidget, Field, {
	loadBox: function($$item) {
		...
		(self.domItem = self.$$item[0]).className += " " + self._$cssTopField;

		self._appendTitle();
		self._createValueContainer();
		self.domItem.syra_field_id = this.id;
		self.domItem.setAttribute("data-s-field", this.id);
		self.domItem.setAttribute("data-s-inout", this.id);
		self.domItem.syraFieldBind = this.$item.$bind || "";
	...
	applyMetaData: function(metaData, options) {
		...
		if (metaData) {
			...
			//
			if (self.$isEditMode) {
				var p_url = self.articleParent.parseExpression((metaData.$links && metaData.$links.$prototype && metaData.$links.$prototype.$url) || (self.$field.$links && self.$field.$links.$prototype && self.$field.$links.$prototype.$url));
	...
	setSDataWhere: function(sdataWhere) {
		...
		if (self.$isEditMode) {
			...
			if (sdataWhere && (self.sdataWhere !== sdataWhere)) {
