...
exports.FilterWidget = helpers.defineClass(FilterWidget, Field, {
    ...
    applyMetaData: function(metaData, options){
    	...
    	if (metaData)
    	{
    		...
    		//
    		var p_url = (metaData.$links && metaData.$links.$prototype && metaData.$links.$prototype.$url) || 
    		...
    		if (p_url) {
    			document.controller.sendRequest(self.articleParent, {
    				$location: {
    					$url: self.articleParent.parseExpression(p_url)
    				}
    				...
    				Object.keys(dps || {}).forEach(function(p) {
    					...
    					if((dp.$capabilities || "").indexOf("filter") >= 0) {
    						// localization
    						dp.$title = self.articleParent.parseExpression(dp.$title, data);
    						pp[p] = dp;
    					}
    				});
