	...
	addPrevNextCell: function(row, isPrev) {
		...
		syra_menus.button.add({
			...
			btnclick: function(event, btn) {
				...
				var row = this.parent.rows;
				for (var ii = 1, jj = row.length; ii < jj; ii++) {
					var nodes = row[ii].childNodes;
					this.parent._refreshYearCell(nodes[0], step);
					this.parent._refreshYearCell(nodes[1], step);
				}
			},
			isPrev: isPrev
		});
