...
exports.YearChoice = helpers.defineClass(YearChoice, null, {
	...
	addPrevNextCell: function(row, isPrev) {
		...
		var btn = syra_menus.addIconButton(isPrev ? syra_local.fdpPrevYear : syra_local.fdpNextYear, "s-calendar-btn", "onChangeYear", null, isPrev ? "prev_step" : "next_step");
		btn.syraIsPrev = isPrev;
		td.appendChild(btn);
		row.appendChild(td);
	},
	...
	onChangeYear: function(event, btn) {
		var step = btn.syraIsPrev ? (-10) : 10;
	...
	onSelectYear: function(event, btn) {
		if (this.selectedCell) {
			this.selectedCell.className = "s-calendar-my-item";
		}
		(this.selectedCell = btn).className = "s-calendar-my-item s-calendar-select";
		this._selectedYear = btn.syraValue;
	...
	dispose: function() {
		...
		this.rows = this.parent = this.domItem = this._dateValue = this._close = this.selectedCell = null;
	}
