...
var shortCutsTip = require("syracuse-ui/lib/events/shortCrutInvoker").shortCutsTip;
...
exports.DateSelector = helpers.defineClass(DateSelector, null, {
	...
	onQuickClick: function() {
		if (!this.quickList) {
			...
			for (var ii = 0, jj = intervals.length; ii < jj; ii++) {
				...
				row.innerHTML = syra_site.dom.formatHTMLText("<a class='s-calendar-quick-link'>" + //
					syra_local.fdpBegin +
					"</a>/<a class='s-calendar-quick-link'>" + //
					syra_local.fdpEnd +
					"</a>" + //
					syra_local["fdpIntervalOf" + intervals[ii]], false);
	...
	_appendHead: function() {
		...
		var btn = syra_menus.addIconButton(syra_local.fdpPrevYear, "s-calendar-btn", "onChangePeriode", shortCutsTip["movePrevYear"], "first_step");
		btn.syraIsPrev = true;
		btn.syraPeriod = "year";
		cell.appendChild(btn);

		// prev month btn
		btn = syra_menus.addIconButton(syra_local.fdpPrevMonth, "s-calendar-btn", "onChangePeriode", shortCutsTip["movePrevMonth"], "prev_step");
		btn.syraPeriod = "month";
		btn.syraIsPrev = true;
		cell.appendChild(btn);

		this._monthBtn = syra_menus.addIconButton(syra_local.fdpChoiceMonth, "s-calendar-month-link", "onMonthClick");
		cell.appendChild(this._monthBtn);

		this._yearBtn = syra_menus.addIconButton(syra_local.fdpChoiceYear, "s-calendar-year-link", "onYearClick");
		cell.appendChild(this._yearBtn);

		btn = syra_menus.addIconButton(syra_local.fdpNextMonth, "s-calendar-btn", "onChangePeriode", shortCutsTip["moveNextMonth"], "next_step");
		btn.syraPeriod = "month";
		cell.appendChild(btn);

		btn = syra_menus.addIconButton(syra_local.fdpNextYear, "s-calendar-btn s-calendar-next-year", "onChangePeriode", shortCutsTip["moveNextYear"], "last_step");
		btn.syraPeriod = "year";
		cell.appendChild(btn);
	...
	_appendFoot: function() {
		...
		var btn = syra_menus.addIconButton(syra_local.fdpPrevWeek, "s-calendar-btn", "onChangePeriode", shortCutsTip["movePrevWeek"], "prev_step");
		btn.syraIsPrev = true;
		btn.syraPeriod = "week";
		cell.appendChild(btn);
		...
		btn = syra_menus.addIconButton(syra_local.fdpNextWeek, "s-calendar-btn", "onChangePeriode", shortCutsTip["moveNextWeek"], "next_step");
		btn.syraPeriod = "week";
		cell.appendChild(btn);
		row.appendChild(cell);
		...
		btn = syra_menus.addTextButton(syra_local.fdpToday, "s-calendar-today-link", "onTodayClick", shortCutsTip["moveToday"]);
		cell.appendChild(btn);
		row.appendChild(cell);
		...
		this._quick = syra_menus.addIconButton(syra_local.fdpQuickSelection, "s-calendar-btn", "onQuickClick", null, "next_step");
