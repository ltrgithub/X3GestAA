...
//		}
		Timeline.OriginalEventPainter.prototype._callX3Event=function(A){
			...
			//that.GET_INFO.call(OppNum,OppLin,WipTyp,VcrNum,VcrLin,VcrSeq);
		};
...
//		this.MOVE_ORDER.build();
	...
	afterBodyResize:	function() {
		try{
			 ...
			 if(this.winWidth !== winNewWidth || this.winHeight !== winNewHeight){
				window.clearTimeout();
				var resizeTimeout = window.setTimeout(function(){this.reCreate();}, 10);
			 }
		}catch(e){}
	},
	reCreate: function() {
	...
	GOBJET:{
		call: function(obj,cle1,cle2,cle3){
			var p={};
			p.no=99;
			p.callBackError = this.error;
			...
			if (cle2!=="") strCle+=("^"+cle2);
			if (cle3!=="") strCle+=("^"+cle3);
			p.params.CLES=strCle;
			PUBLIC.spgmSend(p);
		},
		error: function(err){
			PUBLIC.alert("error: "+err.description);
		},
		reply: function(rep){
		}
	},
	...
	GET_INFO:{
		build: function(){
			var desc = new spgmDescription(100,"CNSX1MCSPV:GET_INFO");
			desc.addParam("Char",20,1,"OPPNUM") ;
			desc.addParam("Integer",0,1,"OPPLIN") ;
			desc.addParam("Integer",0,1,"WIPTYP") ;
			desc.addParam("Char",20,1,"VCRNUM") ;
			desc.addParam("Integer",0,1,"VCRLIN") ;
			desc.addParam("Integer",0,1,"VCRSEQ") ;
			desc.addParam("Clbfile",6,1,"INN") ;
			PUBLIC.addSpgmDesc(desc) ;
		},
		call: function(oppnum,opplin,wiptyp,vcrnum,vcrlin,vcrseq){
			var p={};
			p.no=100;
			p.callBackError = this.error;
			p.callBack = this.reply;
			p.params = {};
			p.params["OPPNUM"]=oppnum;
			p.params["OPPLIN"]=opplin;
			p.params["WIPTYP"]=wiptyp;
			p.params["VCRNUM"]=vcrnum;
			p.params["VCRLIN"]=vcrlin;
			p.params["VCRSEQ"]=vcrseq;
			
			PUBLIC.spgmSend(p);
		},
		error: function(err){
			PUBLIC.alert("error in GET_INFO");
		},
		reply: function(rep){
			//PUBLIC.alert("<xmp>"+rep.xml+"</xmp>")
			this.eventDescr=PUBLIC.spgmGetParamValue("INN");
			this.oTL._bandInfos[0].eventPainter._showBubble(this.xEvent,this.yEvent,this.event);
		}
	},
	// fin de GET_INFO
	MOVE_ORDER:{
		build: function(){
			var desc= new spgmDescription(101,"CNSX1MCSPV:MOVE_ORDER") ;
			desc.addParam("Integer",0,1,"TLIN") ;
			desc.addParam("Char",10,1,"DS") ;
			desc.addParam("Char",10,1,"DE") ;
			desc.addParam("Clbfile",6,1,"INN") ;
			PUBLIC.addSpgmDesc(desc) ;
		},
		call: function(tlin,ds,de){
			var p={};
			p.no=101;
			p.callBackError=this.error;
			p.callBack=this.reply;
		...
		error: function(err){
			PUBLIC.alert("CNSX1MCSPV:error in MOVE_ORDER");
		},
		reply: function(rep){
			//PUBLIC.alert("<xmp>"+rep.xml+"</xmp>")
			this.eventDescr=PUBLIC.spgmGetParamValue("INN");
	...
	BUBBLE:{
		...
		zoom:function(Objet,Cle){
			that.article.externalCall("LINKVUE", [Cle]);
			//this.GOBJET.call(Objet,Cle,"","");
		}
	},
	// fin de BUBBLE
	DetectTZ:{
