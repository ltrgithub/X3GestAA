"use strict";
//JSLint options:
/*global Timeline, SimileAjax, document, window, $, console */
...
//script.text = "Timeline_ajax_url='/syracuse-ui/deps/Timeline/timeline_ajax/simile-ajax-api.js';";
//script.text += "Timeline_urlPrefix='/syracuse-ui/deps/Timeline/timeline_js/';";
//script.text += "Timeline_parameters='bundle=true';";
//
//document.getElementsByTagName("head")[0].appendChild(script); 

window.Timeline_ajax_url = '/syracuse-ui/deps/Timeline/script/timeline_ajax/simile-ajax-api.js';
window.Timeline_urlPrefix = '/syracuse-ui/deps/Timeline/script/timeline_js/';
window.Timeline_parameters = 'bundle=false';

var script = document.createElement("script");
script.type = "text/javascript";
script.language = "JavaScript";
script.src = "/syracuse-ui/deps/Timeline/script/timeline_js/timeline-api.js?bundle=true";
document.getElementsByTagName("head")[0].appendChild(script);

//var link = document.createElement("link");
//link.rel = "stylesheet";
//link.href = "/syracuse-ui/themes/desktop/timeline/timeline.css";
//	
...
function _createWrapper(article) {
	var bindings = article.$prototype.$decorator.$bindings;
	article._wrapper = new TimelineWrapper(article);
	article._wrapper.afterBodyLoad();

}

function _externalCall(article, proxy, values, opt) {
	...
	if (article.page && article.page.externalAdapter && article.page.externalAdapter.onBlockExRpc) {
		article.page.externalAdapter.onBlockExRpc({
			field: article.attachedField,
			call: {
				proxy: proxy,
				values: values,
		...
		if (this.dataset && this.dataset.length > 0) {
			...
			if (this.displayed) {
				if (!this._wrapper) {
					...
					this._wrapper.afterDataChange(dataRecordSet);
				}
			}
		}
	},
...
var TimelineWrapper = helpers.defineClass(function TimelineWrapper(article) {
	...
	afterBodyLoad: function() {
		...
		Timeline.OriginalEventPainter.prototype._callX3Event = function(A) {
			...
			var OppNum = cell[that.bindings.PJT];
			...
			_externalCall(that.article, "GET_INFO", [OppNum, OppLin, WipTyp, VcrNum, VcrLin, VcrSeq], {
		...
		Timeline.OriginalEventPainter.prototype._moveX3Event = function(S, E, evt) {
			...
			//				}
			//			}
			//			_externalCall(that.article,"MOVE_ORDER", [I,DS,DE]);
		};
		...
		img.src = "/syracuse-ui/themes/desktop/timeline/icons/ajax-loader.gif";
	...
	populateJSON: function() {
		...
		for (var j = this.firstRow; j < nbRows; j++) {
			...
			this.timelineJSON.events[i] = {
				...
				'icon': '/syracuse-ui/themes/desktop/timeline/icons/ico_' + wClassName + '.png',
	...
	setupFilterHighlightControls: function(div, timeline, bandIndices, theme) {
		...
		td = tr.insertCell(0);
		var input = syra_dom.addInput();
		input.className = "s-timeline-flt";
	...
	afterDataChange: function(dataRecordSet) {
	...
	BUBBLE: {
		...
		zoom: function(Objet, Cle) {
			_externalCall(Timeline.WRAPPER.article, "LINKVUE", ["OBJET", Objet, Cle]);
			SimileAjax.WindowManager.cancelPopups();
		}
	},
	// fin de BUBBLE
