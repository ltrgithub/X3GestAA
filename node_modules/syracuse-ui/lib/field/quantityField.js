...
exports.QuantityField = helpers.defineClass(QuantityField, Field, {
	...
	render: function() {
		...
		if (!self.$field.$unit) {
			self.showErrors([this.localize && this.localize.qfNoUnitProperty || document.site.localize.qfNoUnitProperty]);
			return;
		}
		...
		if ($unitField && $unitField.$type) {
			...
			if ($unitField.$type == 'application/x-reference' || $unitField.$type == 'application/x-choice') {
				...
				self.showErrors([this.localize && this.localize.qfWrongUnitType || document.site.localize.qfWrongUnitType]);
				return;
			}

		} else {
			self.showErrors([this.localize && this.localize.qfNoUnitType || document.site.localize.qfNoUnitType]);
			return;
		}
		...
		self.input = self.numField && self.numField.input;
	},
	getDataValue: function() {
		if (this.numField.$field.$type == "application/x-decimal") {
			...
			if (this.numField.$field.$type == "application/x-integer") {
				...
				this.showErrors([this.localize && this.localize.qfWrongValueType || document.site.localize.qfWrongValueType]);
				return;
			}
		}
	},
