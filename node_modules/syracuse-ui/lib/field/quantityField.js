...
exports.QuantityField = helpers.defineClass(QuantityField, Field, {
	...
	render: function() {
		...
		if (!self.$field.$unit) {
			self.showErrors([syra_local.qfNoUnitProperty || syra_local.qfNoUnitProperty]);
			return;
		}
		...
		if ($unitField && $unitField.$type) {
			...
			if ($unitField.$type == 'application/x-reference' || $unitField.$type == 'application/x-choice') {
				...
				self.showErrors([syra_local.qfWrongUnitType || syra_local.qfWrongUnitType]);
				return;
			}

		} else {
			self.showErrors([syra_local.qfNoUnitType || syra_local.qfNoUnitType]);
			return;
		}
