...
exports.QuantityField = helpers.defineClass(QuantityField, Field, {
	...
	render: function() {
		...
		if (!self.$field.$unit) {
			self.showErrors([this.localize && this.localize.qfNoUnitProperty || syra_site.localize.qfNoUnitProperty]);
			return;
		}
		...
		if ($unitField && $unitField.$type) {
			...
			if ($unitField.$type == 'application/x-reference' || $unitField.$type == 'application/x-choice') {
				...
				self.showErrors([this.localize && this.localize.qfWrongUnitType || syra_site.localize.qfWrongUnitType]);
				return;
			}

		} else {
			self.showErrors([this.localize && this.localize.qfNoUnitType || syra_site.localize.qfNoUnitType]);
			return;
		}
	...
	getDataValue: function() {
		if (this.numField.$field.$type == "application/x-decimal") {
			...
			if (this.numField.$field.$type == "application/x-integer") {
				...
				this.showErrors([this.localize && this.localize.qfWrongValueType || syra_site.localize.qfWrongValueType]);
				return;
			}
		}
	},
