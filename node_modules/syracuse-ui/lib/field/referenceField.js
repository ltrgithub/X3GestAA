...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
	...
	setState: function(state) {
		...
		if (state.$isHidden !== undefined && this.articleParent.isSingleRecord) {
			syra_site.dom.hide(this.articleParent.domItem, this.$isHidden);
		}
	},
	...
	onInputValueChanged: function() {
		...
		if (value == "") {
			if (this.refDescriptionItem) {
				if (!this._titleField) {
					this.refDescriptionItem.textContent = "";
					syra_site.dom.hide(this.refDescriptionItem, true);
				}
			} else {
				this.input.title = "";
			}
			syra_workingCopy.notifyFieldChange(this, null);
		} else {
			if ((this.currentValue && this.currentValue.$value != value) || !this.currentValue) {
				var postValue = {};
				postValue[this.$reference.$value.$itemProp] = value;
				syra_workingCopy.notifyFieldChange(this, postValue);
			}
		}
	},
	render: function() {
		if (this.$isEditMode) {
			...
			if (!this.$item.$inplace && this.$item.$isReferenceTitleVisible !== false) {
				this.refDescriptionItem = document.createElement("div");
				this.refDescriptionItem.syraTip = 1;
				this.refDescriptionItem.className = $skinInput + "-ref-desc";
				if (this._isTitleField) {
					this._titleField = this.page.loadNewItem(this.refDescriptionItem, {
	...
	notifyFieldChange: function(newValue, validated) {
		...
		if (validated || syra_workingCopy.validateInput(self, newValue)) {
			self.page.externalAdapter.onFieldNotifyChange({
				...
				doEvent: function() {
					...
					if (self.articleParent.onNotifyDataChange ? self.articleParent.onNotifyDataChange(self, self.currentValue) : true) {
						...
						var sendBag = syra_workingCopy.getSendBag(self.page);
						if (self.currentValue !== null) {
	...
	_renderCurrentValue: function() {
		...
		if (this.$isEditMode) {
			if (this.input) {
				...
				this.referenceDescriptionText = this.currentValue.$title || "";
				if ((!this.referenceDescriptionText || !this.$item.$inplace) && this.currentValue.$description) {
					this.referenceDescriptionText = this.currentValue.$description;
				}
				if (this.refDescriptionItem) {
					if (!this._titleField) {
						this.refDescriptionItem.textContent = this.referenceDescriptionText;
						this.refDescriptionItem.style.display = this.referenceDescriptionText ? "" : "none";
					}
				} else {
	...
	showDiagnosSlot: function() {
		if (this.diagsSlot && this.refDescriptionItem && this.$skinInput) {
			syra_site.dom.toggleClass(this.diagsSlot, this.$skinInput + "-ref-diag", true);
		}
	}
});