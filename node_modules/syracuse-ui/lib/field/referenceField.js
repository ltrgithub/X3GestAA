...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
		...
		if (value == "") {
			if (this._refTitle) {
				if (!this._titleField) {
					...
					syra_site.dom.hide(this._refTitle, true);
				}
			} else {
				this.input.title = "";
			}
	...
	_renderCurrentValue: function() {
		...
		if (this.$isEditMode) {
			if (this.input) {
				...
				if (this._refTitle) {
					if (!this._titleField) {
						this._refTitle.textContent = this.referenceDescriptionText;
						this._refTitle.style.display = this.referenceDescriptionText ? "" : "none";
					}
				} else {
	...
	showDiagnosSlot: function() {
		if (this.diagsSlot && this._refTitle && this.$skinInput) {
