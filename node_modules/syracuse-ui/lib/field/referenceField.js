            ...
            doEvent: function(){
                ...
                if (self.articleParent.onNotifyDataChange ? self.articleParent.onNotifyDataChange(self, self.currentValue) : true) {
                    ...
                    if (self.currentValue !== null) {
                        ...
                        /*var $keys = Object.keys(self.$reference.$keys);
                         for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
                         var $key = $keys[ii];
                         var parentKey = self.$reference.$keys[$key];
                         var $prop = $properties[$key];
                         if ($prop && !$prop.$isReadOnly) {
                         if (self.articleParent.$prototype.$properties[parentKey]) {
                         self.articleParent.ensureDataSet()[parentKey] = null;
                         sendBag.saveDataChange(self.page, self.articleParent, parentKey, null);
                         }
                         }
                         }*/
