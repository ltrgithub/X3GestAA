...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
	...
	_parseSettings: function($property, $default) {
		var setting = this.$reference[$property] = {
			$itemProp: this.$field.$item[$property] ? document.site.expressionParser.extractCode(this.$field.$item[$property]) : $property
		};
		setting.$prop = this.$field.$item[setting.$itemProp];
		if (setting.$prop) {
			setting.$prop = document.site.expressionParser.extractCode(setting.$prop);
	...
	initialize: function() {
		...
		var expressionParser = document.site.expressionParser;
		for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
			var $key = $keys[ii];
			var $itemProp = expressionParser.extractCode($key);
			var prop = this.$field.$item[$itemProp];
			this.$reference.$keys[$itemProp] = prop ? expressionParser.extractCode(prop) : $itemProp;
		}
			...
			switch (event.type) {
				case "mouseenter":
					if (this.popupTitleBody.textContent != "") {
						this.popupTitleInfo = document.site.dialogManager.openBoxDialog(this.page, {
							content: {},
							$dialogMode: "popup",
