...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
	...
	_parseSettings: function($property, $default) {
		var setting = this.$reference[$property] = {
			$itemProp: this.$field.$item[$property] ? this.articleParent.extractExpressionCode(this.$field.$item[$property]) : $property
		};
		setting.$prop = this.$field.$item[setting.$itemProp];
		if (setting.$prop) {
			setting.$prop = this.articleParent.extractExpressionCode(setting.$prop);
	...
	initialize: function() {
		...
		for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
			var $key = $keys[ii];
			var $itemProp = this.articleParent.extractExpressionCode($key);
			var prop = this.$field.$item[$itemProp];
			this.$reference.$keys[$itemProp] = prop ? this.articleParent.extractExpressionCode(prop) : $itemProp;
		}
	...
	render: function() {
		if (this.$isEditMode) {
			...
			var picker = this.createPicker("lookup");
			picker.setAttribute("title", this.localize.fieldLookup);
	...
	_isShowMenuPicker: function() {
		var show = (this.$isEditMode || (this.currentValue && this.currentValue.$value && this.currentValue.$value != ""));
						...
						if (self.currentValue !== null) {
							...
							if ($prop && !$prop.$isReadOnly) {
								if (self.articleParent.$prototype.$properties[self.$reference.$value.$prop]) {
									...
									sendBag.saveDataChange(self.page, self.articleParent, self.$reference.$value.$prop, null);
								}
							}
							postValue = null;
						}
	...
	setDataValue: function(value, $bind) {
		...
		if (value) {
			...
			if (this._hasUiid) {
				...
				this.currentValue.$value = value.$value || this.articleParent.parseExpression($field.$value, value) ||
					"";
				this.currentValue.$key = value.$key || this.articleParent.parseExpression($field.$key, value) ||
					"";
			} else {
				if (typeof(value) == 'object') {
					...
					if (this.currentValue.$title == null && this.$reference.$title.$field) {
						this.currentValue.$title = this.articleParent.parseExpression("{" + this.$reference.$title.$prop + "}");
					}
					...
					if (this.currentValue.$value == null && this.$reference.$value.$field) {
						this.currentValue.$value = this.articleParent.parseExpression("{" + this.$reference.$value.$prop + "}");
					}
	...
	_renderCurrentValue: function() {
		if (this.$isEditMode) {
			if (this._input) {
				...
				if (!this.$item.$inplace && this.currentValue.$description) {
	...
	onFieldMouseEvent: function(event) {
		Field.prototype.onFieldMouseEvent.call(this, event);
		if (this.popupTitle) {
			switch (event.type) {
				case "mouseenter":
					if (this.popupTitle) {
						this.popupTitleInfo = this.page.openDialog({
							content: {},
							$dialogMode: "popup",
							$$dialog: $(this.popupTitle),
							position: {
								my: "center bottom",
								at: "center center",
								of: this.$$item
	...
	dispose: function() {
		...
		this._unitItem = this.popupTitle = this.popupTitleInfo = null;
	}
});