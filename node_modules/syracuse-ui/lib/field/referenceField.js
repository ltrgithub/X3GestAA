...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
	...
	_parseSettings: function($property, $default) {
		var setting = this.$reference[$property] = {
			$itemProp: this.$field.$item[$property] ? syra_site.expressionMaker.extractCode(this.$field.$item[$property]) : $property
		};
		setting.$prop = this.$field.$item[setting.$itemProp];
		if (setting.$prop) {
			setting.$prop = syra_site.expressionMaker.extractCode(setting.$prop);
	...
	initialize: function() {
		//this.$menus = this.$field.$item && this.$field.$item.$links;
		this.pickerCount = 1;
		this.$reference = {};
	...
	render: function() {
		if (this.$isEditMode) {
			...
			if (this._refTitle) {
				...
				this._dataValue.parentNode.appendChild(this._refTitle);
			}
		} else {
	...
	_renderCurrentValue: function() {
		...
		if (this.$isEditMode) {
			if (this.input) {
				var value = this.currentValue.$value || "";
				this.setInputValue(value);
				this.descriptionText = this.currentValue.$title || "";
				if ((!this.descriptionText || !this.$item.$inplace) && this.currentValue.$description) {
					this.descriptionText = this.currentValue.$description;
				}
				if (this._refTitle) {
					if (!this._titleField) {
						this._refTitle.textContent = this.descriptionText;
						this._refTitle.style.display = this.descriptionText ? "" : "none";
					}
				} else {
					this.input.title = this.descriptionText;
					// title for singlerecord reference field 
					if (this.boxParent.titleSlot) {
						this.boxParent.titleSlot.textContent = this.descriptionText;
					}
				}
			...
			if (this.codeMenu) {
				...
				if (this.$item.$isUnitField) {
					if (this._dataValue) {
						this._dataValue.textContent = this.displayText = this.currentValue.$symbol;
						if (this.$item.$isQuantityEditable && !this.articleParent.isGridRecord) {
							this.domItem.style.width = this.domItem.scrollWidth + "px";
						}
					}
				}
			}
	...
	_showDescription: function() {
		if (!this.$item.$inplace) {
			...
			if ($description) {
				this.setDescription(syra_site.expressionMaker.parse(this.articleParent, $description, this.articleParent.dataset[this.$item.$bind]));
			}
		}
	},
	onItemInOut: function(onEnter, event, target) {
		Field.prototype.onItemInOut.call(this, onEnter, event, target);
		if (this.descriptionText) {
			if (onEnter) {
				if (this.$item.$isCellChild || (this._refTitle && this._refTitle.scrollWidth != this._refTitle.clientWidth)) {
					syra_site.siteFunctions.toggleTitleTip(this.descriptionText, this.domItem);
				}
			} else {
	...
	showDiagnosSlot: function() {
		if (this.diagnoseFieldSlot && this._refTitle && this.$skinInput) {
			syra_site.dom.toggleClass(this.diagnoseFieldSlot, this.$skinInput + "-ref-diag", true);
		}
	},
	dispose: function() {
		this.currentValue = this.codeMenu = this._titleField = null;
		Field.prototype.dispose.call(this);
		this._unitItem = this.$reference = this._refTitle = null;
	}
});