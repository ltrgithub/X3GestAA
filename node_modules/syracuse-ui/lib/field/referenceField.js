...
exports.ReferenceField = helpers.defineClass(ReferenceField, Field, {
    ...
    _parseSettings: function($property, $default){
        var setting = this.$reference[$property] = {
            $itemProp: this.$field.$item[$property] ? this._extractCode(this.$field.$item[$property]) : $property
        };
        setting.$prop = this.$field.$item[setting.$itemProp];
        if (setting.$prop) {
            setting.$prop = this._extractCode(setting.$prop);
    ...
    initialize: function(){
        ...
        for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
            ...
            this.$reference.$keys[$itemProp] = prop ? this._extractCode(prop) : $itemProp;
        }
    ...
    _renderCurrentValue: function(){
        if (this.$isEditMode) {
            if (this._input) {
                ...
                var title = this.$item.$inplace ? (this.currentValue.$title || "") : ((this.currentValue.$description || ""));
                if (this._refTitle) {
                    if (!this._titleField) {
            ...
            if (this.codeMenu) {
                ...
                $details.$description = this.$item.$inplace ? (this.currentValue.$title || "") : ((this.currentValue.$description || ""));
                this.codeMenu.setMenu($details, this.currentValue, this.currentValue);
            }
            else {
                if (this._unitItem) {
                    this._unitItem.textContent = this.currentValue.$symbol || this.currentValue.$value || this.currentValue.$title || this.currentValue.$description;
                }
            }
