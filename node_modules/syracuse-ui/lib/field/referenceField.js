    ...
    initialize: function(){
        ...
        if (this.$field.$item.$key === "{$uuid}") {
            ...
            for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
                var $key = $keys[ii];
                var part = {
                    $itemProp: this._extractCode($key)
                };
                part.prop = this.$field.$item[part.$itemProp];
                if (part.prop) {
                    part.prop = this._extractCode(part.prop);
                }
                else {
                    part.prop = part.$itemProp;
                }
                this.$refKey.parts.push(part);
            }
        }
    },
                ...
                self.page.externalAdapter.onFieldNotifyChange({
                    ...
                    doEvent: function(){
                        if (self.articleParent.onNotifyDataChange ? self.articleParent.onNotifyDataChange(self, self.currentValue) : true) {
                            var $props = Object.keys(self.currentValue);
                            var postValue = {};
                            for (var ii = 0, jj = $props.length; ii < jj; ii++) {
                                var $prop = $props[ii];
                                if ($prop != "$value" && $prop != "$title" && $prop != "$title") {
                                    postValue[$prop] = self.currentValue[$prop];
                                }
                            }
        ...
        if (value) {
            ...
            if (this._hasUiid) {
                ...
                if (typeof(value) == 'object') {
                    ...
                    for (var ii = 0, jj = this.$refKey.parts.length; ii < jj; ii++) {
                        var $prop = this.$refKey.parts[ii].prop;
                        this.currentValue[$prop] = value[$prop];
                    }
                }
                else {
