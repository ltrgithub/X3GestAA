...
function _ensureUnitField(field) {
	...
	if ($unit && $unit.$type) {
		...
		if (unitArticle != field.articleParent) {
			field.unitField.setDataValue(unitArticle.dataset[field.unitField.$item.$bind], unitArticle.dataset, null, field.unitField.$item.$bind);
		}
...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	render: function() {
		var renderDone;
		if (this.$isEditMode) {
			this.input = syra_dom.addInput(syra_site.isTabletDevice ? "number" : undefined);
