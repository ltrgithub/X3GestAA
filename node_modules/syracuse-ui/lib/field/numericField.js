...
function _formatInput(field, val) {
	...
	if (type == "application/x-decimal" || type == "application/x-real") {
		...
		//(val.match(new RegExp(ds,"g"))||[]).length

		if (gs == "." || gs == ",") {
			// ds value defined in locale has to be used by the user as decimal separator
			// we can't have more than one occurence of ds
			if ((val.match(new RegExp(ds, "g")) || []).length > 1) {
				throw new Error(field.localize.nfInvalidNumber + " : " + val);
			}

			chars_ok += ds;
			val = val.replace(new RegExp("[^" + chars_ok + "]", "g"), '');

			if (gs == '.') {
				val = val.replace(new RegExp(ds, "g"), '.');
			}
		} else {
			// "." and "," are both accepted as decimal separator. thus there can't be more than one occurence of any of them
...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	applyShortCuts: function(shortcurts, event) {
		if (shortcurts.esc) {
			var numObj = this.formatApi.parse(this._input.value || "");
			this._input.value = this.formatApi.format(numObj, this.localFormat);
			event.preventDefault();
		}
	},
	validateKeyPress: function(charCode) {
		var validated = ((charCode >= "0" && charCode <= "9") || charCode == "-" || charCode == "+" || charCode == this.numFormatObj.groupSeparator || charCode == (this.$field.$type == "application/x-integer" ? '' : ".") || charCode == (this.$field.$type == "application/x-integer" ? '' : ","));
		document.site.toggleClass(this._input, 's-field-input-error', !validated);
		return validated;
	},
