...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	dispose: function() {
		var formatEx;
		this.formatApi = this.numFormatObj = this.localFormat = this._formatInput = this._checkSep = null;
	...
	render: function() {
		...
		if (this.$isEditMode) {
			...
			this.input.setAttribute("type", (this.isInteger && syra_site.isTabletDevice) ? "number" : "text");
			this.input.className = this.$skinInput + " " + this.$skinInput + "-num";
			this.fieldValue.appendChild(this.input);
			renderDone = true;
		} else {
			if (this.isProgressMode) {
				this._core.className = "s-field-progress";
				this.fieldValue.className = "s-field-progress-value";
			}
		}
	...
	setDataValue: function(value) {
		if (this.isProgressMode) {
			this.fieldValue.style.width = ((this._core.clientWidth * value) / 100) + "px";
		} else {
			var formattedNum = '';
			...
			if (this.$isEditMode) {
				...
				value && this.unitFieldSlot && this.fieldValue.appendChild(this.unitFieldSlot);
