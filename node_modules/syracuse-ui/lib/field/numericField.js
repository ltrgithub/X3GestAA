...
function decimalParser(decimalSep, groupSep, groupSize) {
	...
	return function(str, field) {
		...
		if (!match)
			throw new Error(field.localize.nfInvalidNumber + " : " + str);
...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	onInputFocusin: function(input, event) {
		document.site.dom.toggleClass(input, 's-field-input-error', false);
	...
	applyShortCuts: function(shortcurts, event) {
		if (shortcurts.esc) {
			var numObj = this.formatApi.parse(this.input.value || "");
			this.input.value = this.formatApi.format(numObj, this.localFormat);
			event.preventDefault();
		}
		return Field.prototype.applyShortCuts.call(this, shortcurts, event);
	},
	validateKeyPress: function(charCode, shortCutOpts) {
		var validated = ((charCode >= "0" && charCode <= "9") || (shortCutOpts && shortCutOpts.ctrl && charCode >= "A" && charCode <= "z") || charCode == "-" || charCode == "+" || charCode == this.numFormatObj.groupSeparator || charCode == (this.$field.$type == "application/x-integer" ? '' : ".") || charCode == (this.$field.$type == "application/x-integer" ? '' : ","));
		document.site.dom.toggleClass(this.input, 's-field-input-error', !validated);
		return validated;
	},
	render: function() {
		if (this.$isEditMode) {
	...
	setDataValue: function(value) {
		try {
			if (this.progressValue) {
				...
				this.fieldHelper.setValueLength(this, formattedNum.length);
				if (this.$isEditMode) {
					this.setInputValue(formattedNum);
				} else {
					if (formattedNum && !this.$isDetailLinkDisabled && this.$rootLinks.$links && this.$rootLinks.$links.$details) {
