...
function _ensureUnitField(field) {
	...
	if ($unit && $unit.$type) {
		field.unitFieldSlot = syra_dom.label(field.$isEditMode ? "s-quantity-unit-edit" : "s-quantity-unit");
		field.unitField = field.page.addItem(field.unitFieldSlot, {
			$bind: $unitBind,
			$field: $unit,
			$inplace: true,
			$isUnitField: true,
		...
		if (unitArticle != field.articleParent) {
			field.unitField.setValue(unitArticle.dataset[field.unitField.$item.$bind], null, unitArticle.dataset);
		}
...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	initialize: function() {
		...
		if (!this.formatApi) {
			this.formatApi = syra_culture.api[this.$numeric.$type];
			this.numFormatObj = syra_culture.format.getNumberFormatObj(this.$numeric.$type);
	...
	onInputFocus: function(input, focusin) {
		if (focusin) {
			...
			if (this.formatApi.parse(editVal, ensureUnitFieldFormat(this)).numberValue() == 0) {
				editVal = 0;
			}
		...
		Field.prototype.onInputFocus.call(this, input, focusin);
	},
	render: function() {
		var renderDone;
		if (this.$isEditMode) {
			this.input = syra_dom.input(this.$skinInput + " " + this.$skinInput + "-num", this._dataValue, (this.isInteger && syra_site.isTabletDevice) ? "number" : undefined);
			renderDone = true;
		} else {
