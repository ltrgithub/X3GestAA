...
exports.NumericField = helpers.defineClass(NumericField, Field, {
    ...
    onInputKeypress: function(input, event){
        ...
        if (!regex.test(c) && !this.$isReadOnly) {
            // error color
            document.site.toggleClass(input, 's-field-input-error', true);
            event.preventDefault();
            return false;
        }
        else {
    ...
    onInputKeydown: function(input, event){
        ...
        /*
         ...
         var self = this;
         if (key == '27') {
         var numObj = self.formatApi.parse(input.value || ""));
         input.value = self.formatApi.format(numObj, self.localFormat);
         event.preventDefault();
         return false;
         }
         */
    ...
    setDataValue: function(value, record){
        try {
            ...
            if (this.$isEditMode) {
                this._input.value = formattedNum;
            }
            else {
