...
function decimalParser(decimalSep, groupSep, groupSize) {
	...
	if (groupSep !== '.' && groupSep !== ',' && (decimalSep === '.' || decimalSep === ',')) decimalSep = '[.,]';
	else decimalSep = "\\" + decimalSep;
	var re = new RegExp("^\\s*([+-]?\\d*(?:\\" + groupSep + "\\d{" + groupSize + "})*)" + decimalSep + "?(\\d*)\\s*$");
	return function(str, field) {
		var match = re.exec(str);
		if (!match) throw new Error(field.localize.nfInvalidNumber + " : " + str);
...
exports.NumericField = helpers.defineClass(NumericField, Field, {
	...
	applyShortCuts: function(shortcurts, event) {
		...
		Field.prototype.applyShortCuts.call(this, shortcurts, event);
	},
