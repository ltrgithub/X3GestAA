...
exports.openPopup = function(openerBox, options) {
	...
	if (!syra_context.enableTestRobot && over.options.$isAutoClose && !over.options.$isAutoCloseTimeoutDisabled) {
...
exports.checkPopupDisplay = function(target, onEnter) {
	if (onEnter) {
		...
		if (_autoClosePopup && _autoClosePopup.overSlot) {
			...
			if (!inPopup && _stack.length > 1 && _stack[0].options.isParentPopup) {
				inPopup = (target == _stack[0].overSlot) || _stack[0].overSlot.contains(target);
			}
			...
			if (inPopup) {
				...
				if (!_autoCloseTimeout) {
					_autoCloseTimeout = setTimeout(function() {
						...
						if (_stack && _stack.length > 0 && _stack[0].options.isParentPopup) {
							_stack[0].close();
