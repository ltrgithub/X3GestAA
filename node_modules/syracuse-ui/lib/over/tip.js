...
function _show(tip, text, target) {
	if (!tip._item) {
		tip._item = syra_dom.div(tip.$css);
	}
	if (text.indexOf("<em") >= 0) {
		if (!tip.em) {
			tip.em = "<em class='" + tip.$css + "-em'>";
		}
		tip._item.innerHTML = syra_dom.escape(text.replace(/<em>/g, tip.em));
	} else {
		tip._item.textContent = exports.utcTip.replaceUTC(text) || target.syraShortCutTitle || text;
	}
	tip._item.style.zIndex = syra_dom.getTopZIndex();
	if (target.syraShortCutTip) {
		syra_dom.text("s-title-shortcut", "(" + target.syraShortCutTip + ")", tip._item);
	} else if (target.syraShortCutTitle) {
		syra_dom.text("s-title-shortcut", text, tip._item);
	}
	var posX = syra_context.isRTL ? "right" : "left";
...
function _requestShow(tip, text, target, check) {
	...
	if (text) {
		...
		target.title = "";
		if (!check || (target.syraShortCutTip || target.syraShortCutTitle || target.syraTip == 2 || ((target.scrollWidth - target.clientWidth) > 1)) || exports.utcTip.hasUTC(text)) {
