...
function _show(tip, text, target) {
	...
	if (text.indexOf("<em") >= 0) {
		...
		tip._item.textContent = exports.utcTip.replaceUTC(text) || text;
	}
...
function _requestShow(tip, text, target, check) {
	...
	if (text) {
		...
		target.title = "";
		if (!check || (target.syraShortCutTip || target.syraTip == 2 || ((target.scrollWidth - target.clientWidth) > 1)) || exports.utcTip.hasUTC(text)) {
