...
helpers.defineClass(Tip, null, {
	_show: function(config) {
		...
		if (config.node) {
			...
			if (config.text.indexOf("<em") >= 0) {
				if (!this.em) {
					this.em = "<em class='s-tip-em'>";
				}
				this._item.innerHTML = syra_dom.escape(config.text.replace(/<em>/g, this.em));
			} else {
				this._item.textContent = exports.utcTip.replaceUTC(config.text) || config.target.syraShortCutTitle || config.text;
			}
			if (config.target.syraShortCutTitle) {
				syra_dom.text("s-title-shortcut", config.text, this._item);
			}
		...
		syra_site.siteApp.appendChild(this._item);
		$(this._item).position({
			my: posX + " top",
			at: posX + " bottom+15",
			of: $(config.target)
