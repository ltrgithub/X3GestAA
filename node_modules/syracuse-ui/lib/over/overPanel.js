...
function _setZIndex(pn) {
	...
	if (pn.isModal) {
		pn.overlay = syra_dom.addDiv("s-overlay");
...
function _onOkClick() {
	...
	if ((pn.options.onValidate && pn.options.onValidate(pn._content, pn) !== false) || (pn.options && pn.options.onServerSaved)) {
		if (pn.options.onServerSaved) {
			syra_menus.menus.applyChange(pn._content, {
				$actions: {
					$save: {
						$isDisabled: false
					}
...
function _loadPage(pn, page, onLoaded) {
	...
	$(pn.overSlot).draggable({
		containment: "#" + syra_site.layoutSlot.id,
		cancel: "article." + $skin + ",." + pn.$skin + "-page-close,." + pn.$skin + "-page-ok",
		stop: function(event, ui) {
			var rect = pn.overSlot.getBoundingClientRect();
			pn.customPosition = {
				top: ui.position.top,
		...
		minWidth: syra_site.config.size.overResizeMinWidth,
		minHeight: syra_site.config.size.overResizeMinHeight,
...
Panel.prototype.open = function(options) {
	...
	if (options.$itemPage || options.$url || options.page) {
		...
		!this._content.isPageLoaded && this.resizeOver(); //content is not page => resize
	}
};
...
Panel.prototype.resizeOver = function(resize, onScroll, isResizing) {
	if (!this.isDeactivated && this._content && this.isOpened && !this.disposed) {
		...
		if (syra_site.body) {
			...
			if (this.options.resizeOver) {
				this.options.resizeOver(this, isResizing);
			} else {
						...
						if (page && page.scrollview) {
							...
							if (page.$facet == "$lookup" || page.$facet == "$select") {
								if (!this.customPosition) {
									...
									this.overSlot.style.top = this.customPosition.top + "px";
									this.overSlot.style.left = this.customPosition.left + "px";
								}
							} else {
								...
								if (list && list.onOverResize) {
									var additionalHeight = list.onOverResize(pageSize.maxScrollHeight - page.scrollview.scrollHeight);
									if (additionalHeight) {
