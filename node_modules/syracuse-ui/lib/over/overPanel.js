...
function _setZIndex(pn) {
	...
	if (pn.isModal) {
		...
		syra_site.siteApp.appendChild(pn.overlay);
	}
...
function _loadPage(pn, page, onLoaded) {
	if (!page.layoutSlot) {
		pn.setOverSlot(page.layoutSlot = document.createElement("div"));
	...
	syra_site.siteApp.appendChild(pn.overSlot);
	pn.addDraggable();
	if (page && page.$facet != "$lookup" && page.$facet != "$select") {
		pn.addResizer();
	}
	pn.isOpened = true;
	...
	syra_quality.overLoaded(pn);
}

function _openPage(pn, $itemPage, onLoaded) {
	if ($itemPage || pn.options.page) {
		if (pn.options.page) {
			pn.setOverSlot(pn.options.page.layoutSlot || document.createElement("div"));
			_loadPage(pn, pn.options.page, onLoaded);
		} else {
			pn.setOverSlot($itemPage.layoutSlot = document.createElement("div"));
...
Panel.prototype.addDraggable = function() {
	this.draggable = new Draggable({
		...
		stop: function() {
			var onEndDrag = this.pn.options ? this.pn.options.onEndDrag : null;
			onEndDrag && onEndDrag(this.pn);
		}
	});
};
...
Panel.prototype.resizePage = function(resizer) {
	var siteSize = syra_site.getSize();
...
Panel.prototype.setOverSlot = function(slot) {
	this.slotListener = syra_site.addEventListeners(this.overSlot = slot);
};
...
Panel.prototype.open = function(options) {
	...
	if (options.$itemPage || options.$url || options.page) {
		...
		if (options.page) {
			options.page.resizeItem(true); //Resize as page already resize on preload
			syra_quality.page_endChange(options.page);
		}
	} else {
		if (!options.content) {
			options.content = {};
		...
		this.setOverSlot(options.slot || options.content.domItem);
		...
		if (!options || !options.syraPlayback) {
			_setZIndex(this);
		}
		this.overSlot.style.top = "0px";
		this.overSlot.style.display = '';
		options.$isAutoClose = options.$isAutoClose !== false;
		syra_site.siteApp.appendChild(this.overSlot);
		...
		syra_quality.overLoaded(this);
	}
};

Panel.prototype.close = function(validated, dispose) {
	if (!this.isCloseDisabled && !syra_over.no && !this.disposed && this.options && (this.options.close ? (this.options.close(validated !== true, dispose, this) !== false) : true)) {
		this.dispose();
	}
};
				...
				if (this.options.$center) {
					...
					if (this.options.position) {
						...
						if (this.options.onresize) {
							var maxHeight = document.getElementById("s_site_body").clientHeight;
							slot.style.maxHeight = maxHeight + "px";
							this.options.onresize(maxHeight, this);
						}
					} else {
...
Panel.prototype.setPosition = function(within) {
	...
	$position.within = $position.within || syra_site.layoutSlot;
