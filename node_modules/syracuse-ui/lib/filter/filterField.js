"use strict";
var _filterMaker = require('syracuse-ui/lib/filter/filterMaker');
...
function _onOperatorClick() {
	...
	if (_popup && _popupItem && _popupItem.syraItem == field.id) {
		...
		if (!_popupItem) {
			_popupItem = syra_dom.div("s-over-popup");
		}
...
exports.load = function($property, $item, filterBlock, index) {
	var field = new FilterField();
	field.filterBlock = filterBlock;
	field.id = filterBlock.id + "-" + index;
	syra_item.register(field);
	field.$field = field.$item = $item;
	field.$bind = $property;
	field.$skin = filterBlock.$skin;

	field.layoutSlot = syra_dom.div(field.$skin + "-field", field.filterBlock.filterBlockContainer);
	field.layoutSlot.syraItem = field.id;
	field.labelAndMenu = syra_dom.div(field.$skin + "-field-header", field.layoutSlot);
	field.valuesSlot = syra_dom.div(field.$skin + "-field-values", field.layoutSlot);

	field.label = document.createElement("label");
	field.label.className = field.$skin + "-field-title";
	field.label.textContent = syra_expression.parse(field.filterBlock, field.$item.$title);
	field.labelAndMenu.appendChild(field.label);

	field.addValueFields();

	field._operatorBtn = syra_button.add({
		parent: field,
		slot: field.labelAndMenu,
		text: syra_local.flfilter_choiceOperator,
		css: field.$skin + "-field-menu",
		iconOnly: true,
