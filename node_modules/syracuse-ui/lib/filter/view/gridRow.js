		...
		if (!col.$field) {
			...
			if (col.$field.$filterable) {
				...
				if (params && params.where) {
					if (!row.sdata_where) {
						row.sdata_where = syra_filter.parse(params.where);
					}
					syra_filter.setField(row.sdata_where, this.field);
				}
			}
		}
		this.append();
...
_helpers.defineClass(GridRow, null, {
	...
	fusion_getCriterion: function() {
		var criterion = syra_filter.getCriterion(this, this.fusion_getSortedBinds(), true);
		this.validateDisplay(criterion);
		return criterion;
	},
