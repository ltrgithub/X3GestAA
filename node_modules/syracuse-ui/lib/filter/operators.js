			...
			if (field.page.filter_popup) {
				...
				for (var ii = 0, jj = field.filter_operators.length; ii < jj; ii++) {
					...
					var btn = syra_button.add({
						parent: field,
						slot: syra_dom.li("s-over-popup-li", ul),
						text: syra_local["flFilter_" + op],
						css: "s-over-popup-btn-defaut",
						fontIcon: "filter_" + op,
	...
	if (field.page.isFusionPage) {
		...
		switch ((field.$filterField && field.$filterField.$type) || field.$field.$type) {
			...
			case "application/x-datetime":
				...
				return operators;
			case "application/x-string":
				return ["none", "like_s", "like", "gt", "ge", "lt", "le", "eq", "ne", "empty", "notempty", "between"];
	...
	if (operator == "between") {
		if (!field.highField) {
			/*var $bind = field.$item.$bind;
			 ...
			 $item.$bind = $bind + "$high";
			 field.highField = field.page.addItem(field.filter_slot, $item, field.articleParent);
			 syra_dom.toggleClass(field._dataValue, "s-filter-between-left", true);
			 syra_dom.toggleClass(field.highField._dataValue, "s-filter-between-right", true);*/
			var $bind = field.$item.$bind;
			field.articleParent.$prototype.$properties[$bind + "$high"] = _helpers.object.clone(field.articleParent.$prototype.$properties[$bind], true);
			var $item = _helpers.object.clone(field.$item, true);
			$item.$bind = $bind + "$high";
		...
		if (field.highField) {
			//syra_dom.remove(field.highField._dataValue);
