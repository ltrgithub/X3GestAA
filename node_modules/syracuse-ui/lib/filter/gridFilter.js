			...
			doEvent: function() {
				if (_popup && _popupItem && _popupItem.syraItem == field.id) {
					...
					if (!_popupItem) {
						...
						_popupItem.className = "s-over-popup";
					}
					...
					_popup = syra_over.openPopup(record.boxParent, {
						content: record,
						slot: _popupItem,
...
function _loadField(record, slot, $field, $bind) {
	...
	var field = record.page.addItem(filterCell, {
		$bind: $bind,
...
function _addFieldCell(record, col, $field) {
	var td = record.cellsMap[col.$item.$bind] = syra_dom.td("s-list-filter-cell", record[col.table.$rowKey]);
	...
	if ($field.$filterable) {
		...
		var params = record.page.urlSeg && record.page.urlSeg.params;
