...
function _validateDisplay() {
	...
	for (var ii = _buttons.length - 1; ii >= 0; ii--) {
		...
		if (btn.menuItem) {
			...
			syra_button.disable(btn, !btn.isBookmarkEnabled || ((page && page.isFusionPage) && !btn.menuItem.urlSeg.isFusion));
			if (btn.bookMarkOn) {
				btnOn = btn;
			}
		}
	...
	_addBtn.disableAddBookmarkText = !btnOn && _disableAddBookmarkText(page);
...
function _disableAddBookmarkText(page) {
	var disableAddBookmarkText;
	if (_buttons.length >= syra_config.bookmarksMax) {
		disableAddBookmarkText = syra_local.bookmarks_max_items;
	} else if (page.isBookmarkDisable) {
		disableAddBookmarkText = syra_local.bookmarks_page_disabled;
	} else if (page.$facet == "$edit" && !page.isFusionPage) {
		disableAddBookmarkText = syra_local.bookmarks_facet_disabled;
	}
	return disableAddBookmarkText;
}
...
exports.onSiteMapMenuItemAddClick = function(menuItem) {

	if (menuItem.urlSegment) {
		var bookmarkMenuItem = exports.find(menuItem.urlSegment);
		if (bookmarkMenuItem) {
			menuItem.isBookmarkEnabled = false;
			exports.deleteMenu(bookmarkMenuItem);
		} else {
			...
			if (menuItem.description) {
				bookmarkMenuItem.description = menuItem.description;
			}
			_dataset.items.push({
...
exports.deleteMenu = function(deleted) {
	...
	for (var ii = 0, jj = items.length; ii < jj; ii++) {
		var item = items[ii];
		if (item === null || item.menuItem == deleted) {
			items.splice(ii, 1);
			return true;
		}
	}
...
exports.fetchBookmarks = function(callback) {
	if (!_dataset) {
...
exports.findMenuItem = function(urlSegment) {
	var menuItem;
	if (_dataset.items && _dataset.items.length > 0) {
		if (urlSegment.fusionParams) {
			menuItem = _fusion.map[urlSegment.fusionParams.fullCode];
		} else {
			var menuItems = _syracuse.map[urlSegment.uri];
			if (menuItems) {
...
function _fill() {
	...
	if (!_addBtn) {
		_addBtn = syra_button.add({
			...
			css: "s-bookmarks-action s-bookmarks-separator",
			click: function() {
	...
	if (items) {
		...
		for (var ii = 0, jj = items.length; ii < jj; ii++) {
			var item = items[ii];
			if (item && item.menuItem && item.menuItem.$url) {
		...
		_aside.insertBefore(_ul, _editBtn && _editBtn.link);
	}
