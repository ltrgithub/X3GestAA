...
exports.BookmarksBar = helpers.defineClass(BookmarksBar, null, {
        ...
        switch ($bind) {
            case "s-bookmarks-bar-link":
                if (picker.syraLink && picker.syraLink !== "") {
                    this.navigate(this._findMenuItem(picker.syraLink));
                }
                return false;
    ...
    checkNavigationMenuItemBookMark: function(navigationMenuItem){
        if (this._findMenuItem(navigationMenuItem.codeMenu.href)) {
    ...
    onNavigationMenuItemPickerClick: function(navigationMenuItem){
        ...
        navigationMenuItem.isBookmark = !navigationMenuItem.isBookmark;
        if (navigationMenuItem.isBookmark) {
            if (this.dataset.items.length < this.maxBookmarks) {
                navigationMenuItem.isBookmark = this.save(navigationMenuItem.codeMenu.href, navigationMenuItem.codeMenu.$title, navigationMenuItem.codeMenu.$description);
            }
            else {
            ...
            var url = navigationMenuItem.codeMenu.href;
            var start = url.indexOf("?");
            if (start) {
                url = url.slice(start);
            }
            for (var ii = 0; ii < this.dataset.items.length; ii++) {
                if (this.dataset.items[ii].menuItem && this.dataset.items[ii].menuItem.$links.$execute_$details.$url === url) {
    ...
    _findMenuItem: function(url, items){
        ...
        for (var ii = 0, jj = items.length; !menuItem && ii < jj; ii++) {
            var item = items[ii];
            if (item.menuItem) {
                if (item.menuItem.$links.$execute_$details.$url === url) {
        ...
        if (items) {
            for (var ii = 0, jj = items.length; ii < jj; ii++) {
                ...
                if (item.menuItem) {
                    ...
                    if (item.menuBlock && !this.addLineLinks(item.menuBlock.items)) {
                        return false;
                    }
                }
            }
    ...
    refresh: function(){
        ...
        this.addLineLinks(this.dataset.items);
        this.morePicker.style.visibility = this.dataset.items.length ? "visible" : "hidden";
    },
                    ...
                    $prototype: {
                        ...
                        $properties: {
                            items: {
                                ...
                                $links: {
                                    $select: this._defineMenuItemLinks()
                                },
                                $item: {
