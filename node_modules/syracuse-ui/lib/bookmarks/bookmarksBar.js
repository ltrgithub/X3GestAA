...
exports.BookmarksBar = helpers.defineClass(BookmarksBar, null, {
	...
	onLinkLinkClick: function(menuItem, picker) {
		var httpQuery = document.controller.parseUrl(decodeURIComponent(picker ? picker.syraEncodedUrl : menuItem.href)); // (menuItem.$url);
		if (httpQuery.$urlParts && httpQuery.$urlParts.isFusion) {
			httpQuery = document.controller.parseUrl(httpQuery.replaceEndpoint(document.site.userProfile.dataset.selectedEndpoint));
			httpQuery.$url = "?url=" + this.extractUrlFromLocationHref(httpQuery.$url, encodeURIComponent);
		} else {
			httpQuery = document.controller.parseUrl(picker ? picker.syraEncodedUrl : menuItem.href);
		}
	...
	_params: function(item) {
		...
		if (urlParsed.query && urlParsed.query.f) {
			...
			params.f = fctr[0];
			if (fctr.length) params.trans = fctr[1];
			return params;
		}
		return null;
	},
		...
		if (items) {
			for (var ii = 0, jj = items.length; ii < jj; ii++) {
				...
				if (item.menuItem && !item.menuItem.$isSeparator) {
					...
					var link = this.addPicker(css, item.menuItem.title || item.menuItem.description || "???", item.menuItem.description);
				...
				} else if (item.menuItem && item.menuItem.$isSeparator) {
