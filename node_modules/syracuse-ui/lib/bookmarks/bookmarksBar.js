...
exports.BookmarksBar = helpers.defineClass(BookmarksBar, null, {
	...
	checkMainPage: function() {
		document.site.toggleClass(this.addPicker, "s-bookmark-on", this._findMenuItem(document.site.history.getCurrentSdataUrl()));
		this.onResize();
	},
	...
	_getPageInfo: function() {
		...
		if (document.site.fusionGateway && document.site.fusionGateway.activatedBook && (sheet = document.site.fusionGateway.activatedBook.selectedSheet)) {
			var fct = sheet.$fusionPageMeta.winModel.getFctName(),
				...
				openerUrlSegments = sheet.$fusionPageMeta.winModel.syraModel.openerUrlSegments;
			if (fct.name && (sheet.$fusionPageMeta.winModel._mdata.type == 1 || sheet.$fusionPageMeta.winModel._mdata.type == 2)) {
				...
				var af = openerUrlSegments.params.f.split("/");
				af[0] = pageInfo.params.f + (pageInfo.params.transaction ? ("~" + pageInfo.params.transaction) : "");
				pageInfo.url = openerUrlSegments.uri + "?f=" + af.join("/");
			}
		} else {
	...
	onNavigationMenuItemPickerClick: function(navigationMenuItem) {
		if (!navigationMenuItem.isBookmark) {
			...
			var fusionParams = this.getURLFusionParameters(menuItem.$url || menuItem.href);
			navigationMenuItem.isBookmark = this.save(menuItem.$url, menuItem.$title, menuItem.$description, fusionParams);
		} else {
	...
	getURLFusionParameters: function(url) {
		url = document.site.history.getSdataUrl(url);
		var segments = document.site.urlMaker.parse(url);
		return segments && segments.fusionParams;
	},
		...
		if (workBook) {
			...
			if (fusionParams) {
				...
				document.site.mainPage.externalAdapter.onBlockExRpc({
					...
					call: {
						proxy: "EXEFNC1",
						values: [fusionParams.functionCode, fusionParams.transaction],
						callback: function() {
	...
	getCurrentEndpointSdataUrl: function(url) {
		...
		var url = document.site.history.getSdataUrl(url);
		...
		if (items) {
			for (var ii = 0, jj = items.length; ii < jj; ii++) {
				...
				if (item.menuItem && !item.menuItem.$isSeparator) {
					...
					if (this.linksSlot.clientWidth != this.linksSlot.scrollWidth || this.moreItemsSlot.children.length > 0) {
	...
	dispose: function() {
		...
		this._links = this.dialog = this.addPicker = this.morePicker = this.linksSlot = this.dataset = null;
	}
});