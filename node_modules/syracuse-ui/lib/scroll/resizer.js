...
function Resizer(config) {
	...
	for (var ii = 0, jj = config.handles.length; ii < jj; ii++) {
		self.handles[ii] = syra_dom.div((config.css || "") + " s-resize-handle s-resize-" + config.handles[ii], config.slot);
	}
...
Resizer.prototype._start = function(event) {
	...
	syra_dom.toggleClass(self.resizing.handle, "s-resize-move", true);
	syra_site.layoutSlot.style.cursor = self.resizing.dir + "-resize";
	if (self.config.body) {
		self.config.body.style.pointerEvents = "none";
		syra_dom.getBoundingClientRect(self.config.body, self.resizing.start);
	}
	if (self.config.start) {
		self.config.start(self.resizing);
	} else {
		self.config.$width = self.resizing.start.width;
		self.config.$height = self.resizing.start.height;
	}
	syra_site.layoutSlot.addEventListener("mousemove", self.on_mousemove = function(event) {
		...
		switch (self.resizing.dir) {
			...
			case "sw":
			case "nw":
				self.resizing.deltaY = event.pageY - self.resizing.start.pageY;
				break;
		}
		switch (self.resizing.dir) {
			case "ne":
			...
			case "w":
			case "nw":
				self.resizing.deltaX = event.pageX - self.resizing.start.pageX;
				break;
		}
		if (self.config.body) {
			if (self.resizing.deltaX != undefined) {
				self.config.$width = self.resizing.start.width + self.resizing.deltaX;
				self.config.body.style.width = self.config.$width + "px";
			}
			if (self.resizing.deltaY != undefined) {
				self.config.$height = self.resizing.start.height + self.resizing.deltaY;
				self.config.body.style.height = self.config.$height + "px";
			}
		}
		if (self.config.drag) {
	...
	syra_site.layoutSlot.addEventListener("mouseup", self.on_mouseup = function(event) {
		self._unbindMove();
		if (self.config.body) {
			self.config.body.style.pointerEvents = "auto";
		}
		self.config.stop(self.resizing);
		syra_dom.toggleClass(self.resizing.handle, "s-resize-move", false);
		syra_site.layoutSlot.style.cursor = "";
