...
exports.DataModel = helpers.defineClass(DataModel, null, {
	...
	getArticle: function(reprName, facet) {
		...
		if (r && r[facet] && r[facet].$layout) return {
	...
	getPrototype: function(entity, facet, representationName) {
		...
		entity.$properties && Object.keys(entity.$properties).forEach(function(propName) {
			...
			if (prop.$isArray) {
				...
				if (prop.$select) {
					...
					if (prop.$select.$filter) l.$url += "&where=" + prop.$select.$filter;
					if (prop.$select.$parameters) l.$url += "&" + prop.$select.$parameters;
				}
			}
			r.$title = prop.$title;
		...
		entity.$relations && Object.keys(entity.$relations).forEach(function(relName) {
			...
			if (rel.$isPlural) {
				...
				if (rel.$select) {
					if (rel.$select.$filter) l.$url += "&where=" + rel.$select.$filter;
					if (rel.$select.$parameters) l.$url += "&" + rel.$select.$parameters;
				}
			} else {
				...
				if (rel.$lookup) {
					if (rel.$lookup.$filter) l.$url += "&where=" + rel.$lookup.$filter;
					if (rel.$lookup.$parameters) l.$url += "&" + rel.$lookup.$parameters;
				}
			}
		});
		//
		...
		if (entity.$actions) {
			...
			Object.keys(entity.$actions).forEach(function(lName) {
				...
				if (!a.$facets || (a.$facets && a.$facets.indexOf(facet) >= 0)) var p = proto.$actions[lName] = proto.$actions[lName] || {};
	...
	applyMeta: function(entity, resource) {
		function _setState(prop, item, pName, state, cb) {
			if (prop[state] && typeof prop[state] === "function") {
				...
				if (cb) cb(ip, prop[state](item));
				else ip[state] = prop[state](item);
			}
		}
		...
		res.forEach(function(item) {
			...
			Object.keys(entity.$properties || {}).forEach(function(pName) {
				...
				_setState(p, item, pName, "$isDefined", function(p, s) {
					p.$isHidden = !s;
				});
				_setState(p, item, pName, "$isDisabled");
				if (p.$applyMeta) p.$applyMeta(item);
			});
	...
	createWorkingCopy: function(entity, reprName, id) {
		...
		if (id) data = entity.$fetch(id);
			...
			Object.keys(entity.$properties || {}).forEach(function(pName) {
				var p = entity.$properties[pName];
				if (p.hasOwnProperty("$default")) data[pName] = p.$default;
			});
			Object.keys(entity.$relations || {}).forEach(function(pName) {
				var p = entity.$relations[pName];
				if (p.hasOwnProperty("$default")) data[pName] = p.$default;
