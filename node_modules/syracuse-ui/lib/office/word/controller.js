    ...
    notifyChange: function(article, binding, value){
        ...
        if (binding === "$actions") {
            ...
            if (repr === "msoMailMergeTemplate" || repr == "msoReportTemplate" || repr === "msoWordDocument") {
                document.site.wordInterface.publishDocument(article.parseExpression(article.$prototype.$url + "?representation={$representation}.$details"), article.dataset.description, article.parseExpression(article.get$Properties().content.$url), function(data, resp){
