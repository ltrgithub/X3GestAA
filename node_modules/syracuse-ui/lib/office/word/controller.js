...
exports.Controller = helpers.defineClass(Controller, CommonController, {
	executeMenu: function(menu, article, confirmed) {
		...
		if (menu && menu.$url && menu.$url.indexOf("$workingCopies") > -1) {
			...
			menu.$url += "&templateClass=" + syra_site.wordInterface.getDocumentRepresentation();
			if (menu.$sourceUrl) {
				menu.$sourceUrl += "&templateClass=" + syra_site.wordInterface.getDocumentRepresentation();
			}
			var locale = syra_site.wordInterface.getDocumentLocale();
			if (locale) {
	...
	_setLinkingProperties: function(menu) {
		var resUrl = syra_site.wordInterface.getResourceUrl();
		if (!resUrl) return;
		resUrl += "&";
