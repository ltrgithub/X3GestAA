...
exports.OfficePage = helpers.defineClass(OfficePage, DesktopPage, {
	...
	notifyActionChange: function(menuItem, target, value, notifyServer) {
		...
		if (reprn && reprn[1])
			repr = reprn[0];
		var documentUrl = syra_site.expressionMaker.parse(this, "{$baseUrl}/{$pluralType}('{$key}')?representation={$representation}.$details");
		var publisher;
		var saveUrl = (this.$prototype.$properties.content || {}).$url;
		if (saveUrl) {
			saveUrl = syra_site.expressionMaker.parse(this, saveUrl);
			switch (repr) {
				case "msoMailMergeTemplate":
				case "msoReportTemplate":
				case "msoWordDocument":
					publisher = syra_site.wordInterface;
					break;
				case "msoPptDocument":
					// save the document to the working copy
					publisher = syra_site.pptInterface;
					break;
				case "msoExcelReportTemplate":
				case "msoExcelDocument":
					publisher = syra_site.excelInterface;
					break;
				case "documentExcel":
				case "documentExcelSI":
					// save the document to the working copy
					publisher = syra_site.excelDocument;
					break;
			}
		}
