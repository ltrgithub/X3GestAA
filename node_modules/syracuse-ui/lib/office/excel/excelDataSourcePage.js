...
function _loadPage(articleParent, $excelView, dataset) {
	...
	page.unloadRelated = function(save) {
		...
		setTimeout(function() {
			...
			related.dispose();
			syra_site.dom.display(record.page.domItem, true);
		}, 10);
	};
...
exports.ExcelDatasourcePage = helpers.defineClass(ExcelDatasourcePage, DesktopPage, {
	initializeArticle: function($itemPage) {
		var tiit = 0;
		var $excelView = $itemPage.$excelView || $itemPage.$representation.$excelView;
		var $items = [];
		this.$prototype = {
			$properties: {
				description: {
		...
		if (this.$facet == "$query") {
			...
			switch ($excelView) {
				case "browseMenu":
					...
					$binds = ["endpointRef", "menuItem"];
					break;
				case "browseEntity":
					this.$prototype.$title = "Data model browse";
					$binds = ["endpointRef", "representationRef"];
					break;
			}
			...
			$binds = ($binds || []).concat(["description", "title", "filter", "fetchAll", "fetchLimit", "orderBys"]);
			for (var ii = 0, jj = $binds.length; ii < jj; ii) {
				var $item = {
					$bind: $binds[ii]
				};
