			...
			return items.map(function(item) {
				...
				switch (item._type) {
					...
					case "application/x-choice":
						var recordProps = record && record.$properties && record.$properties[item._orgName];
						if (!recordProps || recordProps.$isHidden !== true) {
							var ref = propProto.$value || propProto.$key;
							if (ref && ref.$enum) {
								for (var ii = 0, jj = ref.$enum.length; ii < jj; ii++) {
									if (ref.$enum[ii] && ref.$enum[ii].$value && ref.$enum[ii].$value === prop) {
