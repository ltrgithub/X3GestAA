...
exports.ExcelPage = helpers.defineClass(ExcelPage, RawPage, {
	...
	_makeExcelTablePrototype: function($prototype, $item) {
		...
		if (self.$prototype.fields) {
			...
			items = (_item.$layout && _item.$layout.$items) || _item.$items || Object.keys(proto).filter(function(prop) {
	...
	_makeExcelTableData: function(data, $prototype, $excelTablePrototype) {
		...
		return data.$resources.map(function(res) {
			return items.map(function(item) {
				...
				//
				if (record && propProto.$links && propProto.$links.$details) return [result, document.location.protocol + "//" + document.location.host + document.site.$syracuseMainPageUrl +
					"?url=" + encodeURIComponent(self.$grid.$record.parseExpression(propProto.$links.$details.$url, record))];
				else return result;
			});
		});
	},
