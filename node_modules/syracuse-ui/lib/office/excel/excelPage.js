...
exports.ExcelPage = helpers.defineClass(ExcelPage, RawPage, {
			...
			return items.map(function(item) {
				...
				if (record && propProto.$links && propProto.$links.$details)
					return [result, document.location.protocol + "//" + document.location.host + syra_site.$syracuseMainPageUrl +
						"?url=" +
						encodeURIComponent(syra_site.expressionMaker.parse(self.$grid.$record, propProto.$links.$details.$url, res, undefined, item._orgName))];
				else
					return result;
			});
		});
	...
	createExcelTableShell: function(linesCount) {
		return (syra_site.isExcelLoaded && external.ResizeTable(this.$prototype.$datasourceName, JSON.stringify(this.$excelTablePrototype), linesCount, this.$prototype.cellAddress || "")) || !_isExcelLoaded;
	},
