	...
	_makeExcelTablePrototype: function($prototype, $item) {
		...
		return items.map(function(item) {
			...
			var sp = {
				...
				_title: document.site.expressionMaker.parse(self, protoItem.$title),
			...
			return items.map(function(item) {
				...
				switch (item._type) {
					...
					case "application/x-reference":
						...
						result = (record && valueTemplate && (document.site.expressionMaker.parse(self.$grid.$record, valueTemplate, record) || document.site.expressionMaker.parse(self.$grid.$record, valueTemplate, res))) ||
							"";
						break;
					default:
						result = "";
				...
				if (record && propProto.$links && propProto.$links.$details)
					return [result, document.location.protocol + "//" + document.location.host + document.site.$syracuseMainPageUrl +
						"?url=" +
						encodeURIComponent(document.site.expressionMaker.parse(self.$grid.$record, propProto.$links.$details.$url, record))];
				else
					return result;
			});
		});
