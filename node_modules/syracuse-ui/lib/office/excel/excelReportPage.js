...
exports.ExcelReportPage = helpers.defineClass(ExcelReportPage, RawPage, {
	drawBox: function() {
		var self = this;
		self.pageData = null;
	},
	createExcelTemplate: function(excelInterface, refreshOnly) {
		...
		self.createLayout(self.$item, self.$prototype.$properties, null, self.excelLayoutBoxes, null, self.$prototype);
		excelInterface.createExcelTemplate(self.excelLayoutBoxes, self.createSimpleData(refreshOnly), refreshOnly);
	},
	createBasicLayout: function() {
		var self = this;
		self.excelLayoutBoxes = new Array();
	...
	populateExcelTemplate: function(excelInterface) {
		...
		self.createBasicLayout();
		self.createLayout(self.$item, self.$prototype.$properties, null, self.excelLayoutBoxes, null, self.$prototype);
		excelInterface.populateExcelTemplate(self.excelLayoutBoxes, self.createSimpleData());
	},
	refreshExcelDocument: function(excelInterface) {
		var self = this;
		self.createBasicLayout();
		self.createLayout(self.$item, self.$prototype.$properties, null, self.excelLayoutBoxes, null, self.$prototype);
		excelInterface.refreshExcelDocument(self.excelLayoutBoxes, self.createSimpleData());
	},
				...
				if (prop.$type === "application/x-array") {
					...
					if (p.$item && p.$item.$value) {
						...
						if (value && typeof value === "string") {
							...
							value = format(value, dataIn[property]);
						}
						var result = {};
						result.$value = value;
