...
var _prototypeMap = {
	GET: function(context, data, onSuccess, onError) {
		...
		// compat
			...
			var m = syra_site.excelDocument.getDataModel();
			var rep = m.getRepresentation(r);
			var eName = (rep && rep.$entityName) || r;
			var e = m.nameToEntity(eName);
			if (!e)
				return onError(locale.format(module, "entityNotFound", r));
			...
			if (p)
				...
				onError && onError(locale.format(module, "representationNotFound", r));
		}
	}
};
...
var _wcMap = {
	GET: function(context, data, onSuccess, onError) {
		var wc = syra_site.excelDocument.getDataModel().getWorkingCopy(context.resourceId);
		onSuccess(wc);
	},
	DELETE: function(context, data, onSuccess, onError) {
		onSuccess({}, {});
...
helpers.defineClass(ExcelQuery, null, {
	...
	sendRequest: function(options, onSuccess, onError) {
		...
		_dispatch(context, _handlers, options && options.data, onSuccess, onError);
	}
});
