...
exports.Controller = helpers.defineClass(Controller, CommonController, {
	...
	executeMenu: function(menu, article, confirmed) {
		...
		if (menu && menu.$url && menu.$url.indexOf("$workingCopies") > -1 && syra_site.excelInterface) {
			repr = self._getRepresentation(menu.$url);
			menu.$url += "&templateClass=" + syra_site.excelInterface.getDocumentRepresentation();
			if (menu.$sourceUrl) {
				menu.$sourceUrl += "&templateClass=" + syra_site.excelInterface.getDocumentRepresentation();
			}
			var locale = syra_site.excelInterface.getDocumentLocale();
			if (locale) {
	...
	_setLinkingProperties: function(menu) {
		var resUrl = syra_site.excelInterface.getResourceUrl();
		if (!resUrl) return;
		resUrl += "&";
