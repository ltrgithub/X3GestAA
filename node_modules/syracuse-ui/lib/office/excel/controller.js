...
exports.Controller = helpers.defineClass(Controller, CommonController, {
	...
	executeMenu: function(menu, record, article, confirmed) {
		...
		if (menu && menu.$url && menu.$url.indexOf("$workingCopies") > -1 && document.site.excelInterface) {
			repr = self._getRepresentation(menu.$url);
			menu.$url += "&templateClass=" + document.site.excelInterface.getDocumentRepresentation();
			if (menu.$sourceUrl) {
				menu.$sourceUrl += "&templateClass=" + document.site.excelInterface.getDocumentRepresentation();
			}
			var locale = document.site.excelInterface.getDocumentLocale();
			if (locale) {
	...
	_setLinkingProperties: function(menu) {
		var resUrl = document.site.excelInterface.getResourceUrl();
		if (!resUrl) return;
		resUrl += "&";
