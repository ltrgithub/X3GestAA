...
//fusion importat getPopupCtxFromField !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
exports.MenuGroup = helpers.defineClass(MenuGroup, Box, {
    loadBox: function(){
        ...
        this.$item.$isHidden = true; //!this.$item.$isPopupContent;
        this._ensureSkin(this.$authoringLevel);
        this._$registeredMenus = {};
        Box.prototype.loadBox.call(this);
        if (this.boxParent.isMenuGroup && !this.boxParent.isMenuGroupRoot) {
    ...
    _registerMenuItem: function($bind, $isAction){
    ...
    _loadNewMenuItem: function($bind, $isAction){
        var newItem;
        var $menu = this._registerMenuItem($bind, $isAction);
        if (this.layoutContent) {
            //no layoutContent if popup
            newItem = this.page.loadNewItem(this.layoutContent.$$container, $menu, this)
        }
        else {
    ...
    _ensurePrivateMenus: function($menus, $type, $isAction){
        var $links = $menus[$type];
        if ($links) {
            ...
            for (var ii = 0, jj = ids.length; ii < jj; ii++) {
                ...
                if (this.contextField) {
                    isValidate = this.contextField.filterMenu(id);
                }
                if (isValidate) {
                    if (this.$item.$isOwner) {
                        prefixId = this.id + "-" + prefixId;
                    }
                    var $menu = $linksToApply[prefixId] = $links[id];
    ...
    _renderLayoutContent: function(){
        ...
        Box.prototype._renderLayoutContent.call(this);
    },
    addMenuItem: function($menu, record){
        this._loadNewMenuItem($menu.$bind, $menu.$isAction);
    },
    unload: function(){
        if (this.$item.$isOwner) {
