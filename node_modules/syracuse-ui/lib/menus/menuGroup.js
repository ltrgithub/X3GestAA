...
exports.MenuGroup = helpers.defineClass(MenuGroup, SectionBlock, {
	...
	getDefaultTitle: function() {
		return this.localize.box_menusTitle;
	},
	...
	hideBox: function(isHidden) {
		SectionBlock.prototype.hideBox.call(this, isHidden);
		if (this.contextField && this.contextField.showContextMenus) {
			this.contextField.showContextMenus(!isHidden);
		}
	},
	loadBox: function() {
		...
		this.drawBox();
		this.page.applyMenuGroupPreferences(this);
		if (this.$item.$isMenusBag) {
			this.articleParent.menusBags.push(this);
		}
		if (this.$item.$masterFriendMenuId) {
			this.articleParent.idMap[this.$item.$masterFriendMenuId].addFriendMenuGroup(this);
		}
	...
	setRootGroupCss: function() {
		document.site.toggleClass(this.body, "s-menu-group-root", true);
		document.site.toggleClass(this.header, "s-menu-group-root", true);
		document.site.toggleClass(this.domItem, "s-menu-group-root", true);
	},
	addFriendMenuGroup: function(friend) {
		if (!this._friends) {
			this._friends = [];
		}
		friend.setState({
	...
	doOpenPicker: function($$ofPosition) {
		if (this.$item) {
			if (this.$item.$masterFriendMenuId) {
				var master = this.articleParent.idMap[this.$item.$masterFriendMenuId];
				if (master) {
					master.doOpenPicker($(this.header));
				}
			} else {
				this.page.onMenuGroupClick(this);
				SectionBlock.prototype.doOpenPicker.call(this, $$ofPosition);
			}
		}
	},
	onBeforeAutoClose: function(box) {
		if (this._friends) {
			for (var ii = 0, jj = this._friends.length; ii < jj; ii++) {
				if (this._friends[ii] == box) {
					return false;
				}
			}
		}
	...
	clearMenus: function(removeDom) {
		for (var ii = 0, jj = this.$item.$layout.$items.length; ii < jj; ii++) {
			var $item = this.$item.$layout.$items[ii];
			this.articleParent.removeMenuItem(this.$item.$layout.$items[ii].$bind, removeDom);
		}
	},
		...
		if ($links) {
			...
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				...
				if (this.menuGroupRoot.contextField) {
					isValidate = this.menuGroupRoot.contextField.filterMenu(id, $links[id]);
				}
				if (isValidate) {
					prefixId = this.id + "-" + prefixId;
					var $menu = $links[id];
					if ($menu === null) {
						this.articleParent.removeMenuItem(prefixId, true);
					} else {
	...
	dispose: function() {
		this._friends = this.menuGroupRoot = this.contextField = this.$childMenus = null;
