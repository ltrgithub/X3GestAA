...
exports.onItemInOut = function(menuItem, onEnter) {
	if (onEnter) {
		if (menuItem.outOfDate) {
			menuItem.setMenu(null, menuItem.articleParent.dataset);
			menuItem.outOfDate = false;
		}
		if (menuItem.titleText && menuItem._label) {
			menuItem.domItem.title = "";
			if (menuItem.$item.$noText || menuItem._label.clientWidth != menuItem._label.scrollWidth) {
				syra_site.siteFunctions.showTip(menuItem, true, menuItem.titleText);
			}
		}
	} else {
		if (menuItem.domItem && menuItem.titleText) {
			menuItem.domItem.title = menuItem.titleText;
		}
		// close on mouse leave, and if mouse is not over popup
		syra_site.siteFunctions.showTip(menuItem, false);
	}
};
