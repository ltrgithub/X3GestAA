...
function _addEmbeddedFields(field, $bind, $menu) {
	if (field.isDesktopPage) {
		...
		if ($properties) {
			...
			if ($menu.$url) {
				$menu.$url = syra_site.expressionMaker.parseUrl(field, $menu.$url);
				var seg = syra_site.urlMaker.parse($menu.$url, true);
				if (!seg.isSyracuse) {
					if ($menu.$url.indexOf("http://www.google.fr") >= 0) {
						$menu.$url = $menu.$url.replace("http://www.google.fr", "http://www.lemonde.fr/");
					}
					$menu.$type = "html";
				}
			}
...
exports.onPickerClick = function(event, picker, field) {
	if (!field.authoringNode && !field.$isDisabled) {
		if (syra_site.currentQuickEdit && !field.page.isMessageBox && syra_site.currentQuickEdit.page != field.page) {
			syra_site.currentQuickEdit.confirmClose(function() {
		...
		field.page.externalAdapter.onFieldClickPicker({
			...
			doEvent: function(options) {
				switch (picker.syraPickerType) {
					case "$quickEdit":
						field.list.fomatter.quickEdit.toggle(field);
						break;
					case "escape":
