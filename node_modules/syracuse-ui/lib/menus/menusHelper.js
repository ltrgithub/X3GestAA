...
exports.onItemInOut = function(onEnter, event, target) {
	if (onEnter) {
		...
		if (target.syraTitle) {
			target.title = "";
			show && syra_site.siteFunctions.toggleTitleTip(target.syraTitle, target);
		}
...
function _isPopupMenuItem(field, $bind, $link) {
	...
	switch ($bind) {
		...
		case "$select":
			if (field.isList) {
				field.page.loadNewItem(field.topbar, {
					$bind: $bind,
					$category: "link",
	...
	if (!field.$item.$isMenusHidden && (!field.$item.$isUnitField || field.$isEditMode)) {
		...
		if ($menus.$links !== undefined || $menus.$actions !== undefined) {
			if ($menus.$links === null) {
				...
				if (!field.mnPickers.menus && !field.$item.$isPickerMenuHidden && field.$menusPopup && Object.keys(field.$menusPopup).length) {
...
exports.dispose = function(field) {
	field.menusPopup && field.menusPopup.close();
	field.mnPickers = field.menusPopup = null;
};