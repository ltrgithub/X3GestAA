					...
					if (field.isPopupMenuItem && !field.isPopupMenuItem($bind, $menu)) {
						...
						switch ($bind) {
							...
							case "$edit":
								...
								break;
							case "$delete":
								if (field.onAddMenuItem && field.$item.$useDeletePicker) {
							...
							case "$tunnel":
								...
								if (!$menu.$isPopup && !field.mnPickers.$tunnel && !field.$item.$inplace) {
									_addPicker(field, "$tunnel", syra_local.fieldTunnel, "tunnel");
							...
							case "$search":
								...
								break;
							case "$lookup":
								field.$isEditMode && !field.mnPickers.$lookup && _addPicker(field, $bind, syra_local.fieldLookup, "lookup");
								$menu.$isPopup = false;
								break;
							default:
								$menu.$isPopup = true;
...
function _addPicker(field, type, title, css) {
