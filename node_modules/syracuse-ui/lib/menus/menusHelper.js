...
exports.onItemInOut = function(onEnter, event, target) {
	if (onEnter) {
		...
		if (target.title) {
			target.syraTipTitle = target.title;
			target.title = "";
			(show || target.syraShortCutTip) && syra_site.siteFunctions.toggleTitleTip(target.syraTipTitle, target);
		}
	} else {
	...
	if (!field.menusPopup) {
		...
		field.menusPopup = syra_site.dialogManager.openPopup(field, {
			...
			onClose: function() {
				...
				var $items = field.menusBox.$item.$layout.$items;
				for (var ii = 0, jj = $items.length; ii < jj; ii++) {
					syra_menus.removeMenu(field, $items[ii].$bind);
				}
				var article = field.menusBox && field.menusBox.articleParent;
				article && article.removeItem(field.menusBox, true, true);
...
exports.applyArticleState = function(field) {
	...
	if (pickers) {
		if ( !! field.isPickersDisabled != (field.$isReadOnly || field.$isDisabled)) {
			field.isPickersDisabled = field.$isReadOnly || field.$isDisabled;
		...
		if ( !! field.isPickersMenuDisabled != field.$isDisabled) {
			...
			doEvent: function(options) {
				switch (picker.syraPickerType) {
					...
					case "escape":
						...
						break;
					case "menus":
						syra_site.closePopups(field.menusBox, event);
