	...
	highlightFieldPickers: function(field, on) {
		if (field && field.buttons) {
			for (var ii = 0, jj = field.buttons.length; ii < jj; ii++) {
				...
				btn.pickerId && syra_site.dom.toggleClass(btn.link, "s-field-enter", on);
			}
		}
	},
	add: function(btn) {
		...
		if (btn.iconOnly) {
			...
			if (btn.titleImage) {
				btn.titleImageItem = document.createElement("div");
				btn.titleImageItem.className = btn.css + "-title-icon";
				btn.titleImageItem.style.backgroundImage = "url('" + syra_site.$iconPath + btn.titleImage + "')";
				link.appendChild(btn.titleImageItem);
			} else {
				if (btn.isTextHTML) {
					link.innerHTML = syra_site.dom.textToHTML(btn.text);
				} else {
	...
	setText: function(btn, text, icon) {
		...
		if (text != undefined) {
			...
			if (!btn.iconOnly && !btn.titleImage) {
				if (btn.isTextHTML) {
					btn.link.innerHTML = syra_site.dom.textToHTML(btn.text);
				} else {
	...
	hide: function(btn, hidden) {
		if (btn) {
			if (btn.link_isHidden != hidden) {
				...
				syra_site.dom.hide(btn.link, hidden);
			}
		}
	...
	menus: {
		add: function(field) {
			return syra_menus.button.addFieldPicker({
				...
				shortCutTip: (field.isField && !field.isArrayField) ? shortCutsTip["contextMenu"] : null,
		...
		add: function(field, $menu) {
			...
			if (!$menu.$isPopup && !field.picker_$tunnel && !field.$item.$inplace) {
				syra_menus.button.addFieldPicker({
					...
					shortCutTip: shortCutsTip["openTunnel"],
					btnclick: this.onClick
				});
		...
		add: function(field, $menu) {
			if (field.$isEditMode && !field.picker_$lookup) {
				syra_menus.button.addFieldPicker({
					...
					shortCutTip: shortCutsTip["openLookup"],
					btnclick: this.onClick
				});
			}
			$menu.$isPopup = false;
		...
		if (group.$item) {
			...
			if (group.$item.$isPopupContent) {
				if (!group.menusPopup) {
					...
					group.menusPopup = syra_site.dialogManager.openPopup(group, {
						content: group,
						slot: group.body,
						position: {
							my: "right top",
						...
						onClose: function() {
							group.domTitle && syra_site.dom.toggleClass(group.domTitle, "s-close", true);
							group.$opened = false;
