		...
		showPopup: function(options) {
			if (options.field) {
				if (!options.field.menusPopup) {
					...
					options.field.menusBox = options.field.page.loadNewItem(null, $item, article);
					...
					syra_site.activePopupMenu = options.field.menusPopup = syra_dlg.openPopup(options.field, {
					...
					onSelectRecord: function(selectedRecords) {
						if (!field.$isDisabled && !field.$isReadOnly) {
							if (field.isChildFieldRecord) {
								...
								field.articleParent.setDataValue(record.dataset);
		...
		if (group.$item) {
			...
			if (page.menuBar && !page.isVignettePage) {
				if (page.isInMenuBar(group) && !group.$item.$isPopupContent) {
					var pref = syra_pagePreferences.get(page, "$menuBarGroup") || {};
					pref[group.id] = group.$opened;
					syra_pagePreferences.save(page, "$menuBarGroup", pref);
				}
			}
