...
function _setTitleIconValue(menu, $title, $iconValue) {
	...
	if (menu.$item.$icon && !menu.iconValue) {
		menu.cssTitle = null;
		var cssBtn = " s-mn-fonticon s-btn-" + ((name[0] == "$") ? name.substr(1) : name);
...
exports.addIconButton = function(text, css, onClick, shortCutTip, fontIcon) {
	...
	if (fontIcon) {
		...
		if (icon) {
			css += " s-fonticon-btn s-btn-" + ((fontIcon[0] == "$") ? fontIcon.substr(1) : fontIcon);
			btn.innerHTML = icon;
		}
	}
	btn.className = css;
...
exports.onTipItemInOut = function(onEnter, event, target) {
	if (onEnter) {
		...
		var show = target.title != undefined && target.textContent != target.title;
		show = show || target.syraShortCutTip || target.syraTip == 2 || target.clientWidth != target.scrollWidth;
		target.title = "";
		if (show) {
...
exports.showPopup = function(field, picker) {
	if (!field.menusPopup) {
		...
		picker && _showPicketOpen(picker, true);
		field.menusPopup = syra_site.dialogManager.openPopup(field, {
			...
			onClose: function() {
				picker && _showPicketOpen(picker, false);
...
exports.highlight = function(field, on) {
	...
	if (pickers) {
		...
		pickers.$tunnel && syra_site.dom.toggleClass(pickers.$tunnel, "s-enter", on);
			...
			doEvent: function(options) {
				switch (picker.syraPickerType) {
					...
					case "$lookup":
						...
						break;
					case "choice":
						field.onChoiceClick && field.onChoiceClick(null, field.mnPickers.choice);
						break;
					case "$localize":
