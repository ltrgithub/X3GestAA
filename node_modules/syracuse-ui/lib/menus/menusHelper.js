...
function _isPopupMenuItem(field, $bind, $link) {
	...
	switch ($bind) {
		case "$edit":
			_addPicker(field, $bind.slice(1)).syraBind = $bind;
			return false;
		case "$delete":
			if (field.isRecord) {
				_addPicker(field, $bind.slice(1)).syraBind = $bind;
			}
			return !field.isRecord;
		case "$create":
		case "$select":
			if (field.isList) {
				_addPicker(field, $bind.slice(1)).syraBind = $bind;
		...
		if ($menus.$links !== undefined || $menus.$actions !== undefined) {
			if ($menus.$links === null) {
					...
					if (field.$isEditMode) {
						...
						if ($menus.$links.$lookup && !field.mnPickers.lookup) {
							if (!field.mnPickers.lookup) {
								_addPicker(field, "lookup").title = syra_local.fieldLookup;
							}
						}
						if ($menus.$links.$tunnel && !field.mnPickers.tunnel && !field.$item.$inplace) {
							if (!field.mnPickers.tunnel) {
								_addPicker(field, "tunnel").title = syra_local.fieldTunnel;
						...
						if ($menus.$links.$localize && !field.mnPickers.localize && field.$item.$useLocalizePicker) {
							if (!field.mnPickers.localize) {
								_addPicker(field, "localize").title = field.$menus.$localize.$title;
							}
						}
					}
				}
				...
				if (!field.mnPickers.menus && !field.$item.$isPickerMenuHidden && field.$popupMenus && Object.keys(field.$popupMenus).length) {
					_addPicker(field, "menus").title = syra_local.flActions;
				}
			}
		}
	}
...
function _addPicker(field, type) {
	...
	picker.syraIsSyraMenuHandler = true;
...
exports.onPickerClick = function(field, picker) {
			...
			doEvent: function(options) {
				switch (picker.syraPickerType) {
					...
					case "tunnel":
						...
						break;
					case "choice":
						field.onChoiceClick(field, field.mnPickers.choice);
						break;
					case "localize":
