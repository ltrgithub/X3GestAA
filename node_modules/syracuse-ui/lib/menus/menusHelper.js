...
exports.addTextButton = function(text, css, onClick, shortCutTip) {
...
exports.addIconButton = function(text, css, onClick, shortCutTip) {
	var btn = document.createElement("a");
	btn.className = css;
	//btn.setAttribute("href", "#"); //oneed to improve cancel action otherwiese add # to url 
	btn.syraOnClick = onClick;
						...
						switch ($bind) {
							...
							case "$suggest":
								...
								if (!field.mnPickers.clear) {
									_addPicker(field, "clear", syra_local.siteSearchClear).style.display = "none";
								}
								break;
							case "$details":
							...
							case "$localize":
								...
								if (field.$isEditMode && !field.mnPickers.localize && field.$item.$useLocalizePicker) {
									_addPicker(field, $bind, field.$menus.$localize.$title, "localize");
								}
								$menu.$isPopup = true;
								break;
							case "$tunnel":
								$menu.$title = syra_local.fieldTunnel;
								$menu.$isPopup = !field.$isEditMode && field.$item.$inplace && !field.$item.$isFilterMode;
								if (!$menu.$isPopup && !field.mnPickers.$tunnel && !field.$item.$inplace) {
									_addPicker(field, "$tunnel", syra_local.fieldTunnel, "tunnel", "ESC + F9");
								...
								break;
							case "$search":
								!field.mnPickers.$search && _addPicker(field, $bind, syra_local.siteSearchLink, "search");
								$menu.$isPopup = false;
								break;
							case "$lookup":
								field.$isEditMode && !field.mnPickers.$lookup && _addPicker(field, $bind, syra_local.fieldLookup, "lookup", "ESC + L, ESC + F12");
								$menu.$isPopup = false;
								break;
							default:
								$menu.$isPopup = true;
			...
			if ($menus.$links !== undefined || $menus.$actions !== undefined) {
				if ($menus.$links === null) {
					...
					if (field.mnPickers && !field.mnPickers.menus && !field.$item.$isFilterMode && !field.$item.$isPickerMenuHidden && field.$menusPopup && Object.keys(field.$menusPopup).length) {
						_addPicker(field, "menus", syra_local.flActions, null, "ESC + M");
					}
				}
			}
		}
...
exports.applyState = function(field) {
	...
	if (pickers) {
		...
		pickers.date && _togglePicker(pickers.date, syraDisplay);
		pickers.time && _togglePicker(pickers.time, syraDisplay);
		pickers.choice && _togglePicker(pickers.choice, syraDisplay);
		pickers.$lookup && _togglePicker(pickers.$lookup, syraDisplay);
		pickers.localize && _togglePicker(pickers.localize, syraDisplay);
		pickers.menus && _togglePicker(pickers.menus, syraDisplay);
		if (pickers.$tunnel) {
			if (field.$isReadOnly) {
				syraDisplay = "";
			}
			_togglePicker(pickers.$tunnel, syraDisplay);
		}
	}
};
...
function _addPicker(field, type, title, css, shortCutTip) {
	var picker = field.mnPickers[type] = document.createElement("a");
	var css = " s-picker-" + (css || type);
	if (field.$item.$css) {
		css += " " + field.$item.$css;
	}
...
exports.clickContextualMenu = function(field) {
...
exports.clickSelectPicker = function(field) {
	var pickers = field.mnPickers;
	if (pickers) {
		var picker = pickers.$lookup || pickers.choice || pickers.date || pickers.time;
		picker &&
			setTimeout(function() {
				picker.click();
			}, 10);
...
exports.clickCreate = function(field) {
	if (field == field.page) {
		exports.clickMenu(field, field.isFusionPage ? "1115" : "$create");
	} else {
		exports.clickMenu(field, "$create");
	}
...
exports.addPagerStepLinks = function(parent, slot, article) {
	...
	for (var ii = 0, jj = _pagers.$steps.length; ii < jj; ii++) {
		...
		steps[$step.$bind] = parent.page.loadNewItem(slot, {
			...
			$icon: {
				$mode: "icon",
				$path: parent.isList ? "page/s-list-" : "page/s-record-"
			}
		}, article || parent);
	}
	return steps;
...
exports.gotoPrev = function(parent, toFirst) {
	var bind = toFirst ? "$first" : "$previous";
	if (parent.menuItems[bind]) {
		exports.clickMenu(parent, bind);
		return true;
	}
};

exports.gotoNext = function(parent, toLast) {
	var bind = toLast ? "$last" : "$next";
	if (parent.menuItems[bind]) {
		exports.clickMenu(parent, bind);
		return true;
	}
...
exports.gotoNavigationPage = function() {
	exports.clickMenu(syra_site, "$navigation");
};
exports.gotoHome = function() {
	exports.clickMenu(syra_site, "$home");
};
