...
var _pagingCapability = require("syracuse-ui/lib/field/array/capability/pagingCapability");
...
function _setTitleIconValue(menu, $title, $iconValue) {
	...
	if (menu.$item.$icon && !menu.iconValue) {
		menu.cssTitle = null;
		var cssBtn = _cssFontIconMn + "s-btn-" + name;
	...
	if (fontIcon) {
		...
		var $path = syra_site.$item.$iconPath + (menu.$item.$icon.$path || "");
...
exports.addTextButton = function(text, css, onClick, shortCutTip, fontIcon) {
	...
	if (fontIcon) {
		...
		btn.insertBefore(exports.addFontIconFlag(css + "-flag", fontIcon), btn.firstChild);
	}
	return btn;
};
...
var _listMenuBinds = ["$edit", "$create", "$delete", "$details", "$toggleGraph", "$design", "$cancelSort"];
			...
			if ($defined) {
				...
				if (field.isPopupMenuItem && !field.isPopupMenuItem($bind, $menu)) {
					...
					switch ($bind) {
						...
						case "$next":
						case "$last":
							field.isDesktopPage && _pagingCapability.addPagerLinksToPage(field, $bind);
							break;
						case "$edit":
						...
						case "$tunnel":
							...
							if (!$menu.$isPopup && !field.mnPickers.$tunnel && !field.$item.$inplace) {
								_addFieldPicker(field, "$tunnel", syra_local.fieldTunnel, shortCutsTip["tunnel"]);
							...
							break;
						case "$lookup":
							field.$isEditMode && !field.mnPickers.$lookup && _addFieldPicker(field, $bind, syra_local.fieldLookup, shortCutsTip["openLookup"]);
							$menu.$isPopup = false;
							break;
						default:
							$menu.$isPopup = true;
			...
			if ($menus.$links !== undefined || $menus.$actions !== undefined) {
				if ($menus.$links === null) {
					...
					if (field.mnPickers && !field.mnPickers.menus && !field.$item.$isFilterMode && !field.$item.$isPickerMenuHidden && field.$menusPopup && Object.keys(field.$menusPopup).length) {
						_addFieldPicker(field, "menus", syra_local.flActions, (field.isField && !field.isList) ? shortCutsTip["contextMenu"] : null);
						if (!field.$isDisabled && field.$isReadOnly) {
