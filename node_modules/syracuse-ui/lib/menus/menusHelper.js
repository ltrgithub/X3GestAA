...
function _setTitleIconValue(menu, $title, $iconValue) {
	...
	if (menu.$item.$icon && !menu.iconValue) {
		menu.cssTitle = null;
		var cssBtn = exports.cssFontIconBtn + "s-btn-" + name;
...
exports.addTextButton = function(text, css, onClick, shortCutTip, fontIcon) {
	...
	btn.syraTip = 1;
	if (fontIcon) {
		btn.syraHasIcon = true;
		btn.insertBefore(exports.addFontIconFlag(css + "-flag s-mn-icontext", fontIcon), btn.firstChild);
	}
	return btn;
};
...
exports.button = {
	...
	add: function(opt) {
		opt.id = ++_index;
		opt.btnclick = opt.btnclick;
		var btn = opt.link = document.createElement("a");
		_buttons[btn.syraBtn = opt.id] = opt;

		btn.setAttribute("href", "#");
		btn.title = opt.text;
		if (opt.shortCutTip) {
			btn.syraShortCutTip = opt.shortCutTip;
		}
		if (opt.iconOnly) {
			btn.syraTip = 2;
			btn.syraIconCss = opt.css;
			if (!(opt.fontIcon && _setFontIconButton(btn, opt.fontIcon))) {
				btn.className = opt.css;
				if (opt.imageName) {
					btn.style.backgroundImage = "url('" + syra_site.$iconPath + opt.imageName + "')";
				}
			}
		} else {
			btn.className = opt.css;
			...
			if (opt.fontIcon) {
				btn.syraHasIcon = true;
				btn.insertBefore(exports.addFontIconFlag(opt.css + "-flag s-mn-icontext", opt.fontIcon), btn.firstChild);
			}
		}
		opt.slot && opt.slot.appendChild(btn);
		(opt.parent.buttons = opt.parent.buttons || []).push(opt);
		return opt;
	},
	setTitle: function(btn, text) {
		if (btn.syraTipTitle) {
	...
	display: function(btn, show) {
		btn && syra_site.dom.display(btn.link, show);
	}
};
...
function _setFontIconButton(btn, fontIcon) {
	...
	if (icon) {
		btn.innerHTML = icon;
		btn.className = btn.syraIconCss + exports.cssFontIconBtn + "s-btn-" + fontIcon;
		return true;
	}
}
...
exports.addFontIconText = function(text, css, fontIcon) {
	...
	item.insertBefore(exports.addFontIconFlag(css + "-flag s-mn-icontext", fontIcon), item.firstChild);
	return item;
};
...
exports.updateButtonIcon = function(btn, text, fontIcon) {
	var icon;
	if (btn.syraHasIcon) {
		btn.removeChild(icon = btn.firstChild);
	}
	exports.setButtonTitle(btn, text);
	if (icon) {
		btn.insertBefore(icon, btn.firstChild);
	}
	fontIcon && _setFontIconButton(icon || btn, fontIcon);
};
...
function _clearMenus(item) {
	...
	item.mnPickers.menus && _togglePicker(item.mnPickers.menus, "none");
}
						...
						case "$edit":
							if (field.onAddMenuItem) {
								if (field.articleParent.$capability && field.articleParent.$capability.quickedit) {
									_addFieldPicker(field, "$quickEdit", syra_local.fieldQuickEdit);
						...
						case "$tunnel":
							...
							if (!$menu.$isPopup && !field.mnPickers.$tunnel && !field.$item.$inplace) {
								...
								if (!field.$isDisabled && field.$isReadOnly) {
									field.mnPickers.$tunnel.style.display = field.mnPickers.$tunnel.syraDisplay = "";
								}
							}
							break;
						case "$lookup":
			...
			if ($menus.$links !== undefined || $menus.$actions !== undefined) {
				if ($menus.$links === null) {
					...
					if (field.mnPickers && !field.mnPickers.menus && !field.$item.$isFilterMode && !field.$item.$isPickerMenuHidden && field.$menusPopup && Object.keys(field.$menusPopup).length) {
						_addFieldPicker(field, "menus", syra_local.flActions, (field.isField && !field.isArrayField) ? "ESC F4,ESC M" : null);
						if (!field.$isDisabled && field.$isReadOnly) {
...
exports.showPopup = function(field, picker) {
	if (!field.menusPopup) {
		...
		picker && _showPickerOpen(picker, true);
		syra_site.activePopupMenu = field.menusPopup = syra_site.dialogManager.openPopup(field, {
			...
			onClose: function() {
				...
				picker && _showPickerOpen(picker, false);
...
exports.applyArticleState = function(field) {
	...
	if (field.isPickersDisabled != disabled) {
		if (pickers) {
			...
			pickers.$quickEdit && syra_site.dom.disableItem(pickers.$quickEdit, field.isPickersDisabled);
			pickers.$edit && syra_site.dom.disableItem(pickers.$edit, field.isPickersDisabled);
			if (!field.page.isFusionPage) {
				pickers.$delete && syra_site.dom.disableItem(pickers.$delete, field.isPickersDisabled);
				pickers.$create && syra_site.dom.disableItem(pickers.$create, field.isPickersDisabled);
			}
		}
	...
	if (pickers && (field.isPickersMenuDisabled != field.$isDisabled)) {
		field.isPickersMenuDisabled = field.$isDisabled;
		pickers.menus && syra_site.dom.disableItem(pickers.menus, field.isPickersMenuDisabled);
	}
};
...
exports.showMenuPicker = function(field) {
	var picker = field.mnPickers.menus;
	if (picker) {
		var show = field.$isEditMode || field.hasValue();
		_togglePicker(picker, show ? "" : "none");
		if (!picker.parentNode && field.onAddMenuPicker) {
			field.onAddMenuPicker(picker);
		}
	}
};

function _showPickerOpen(picker, open) {
...
function _addPicker(type, title, css, shortCutTip) {
	...
	var icon = (type[0] == "$") ? type.substr(1) : type;
	css += " s-btn-" + icon;
	var fontIcon = _fontIcons[icon];
	if (fontIcon) {
	...
	if (!field.authoringNode && !field.$isDisabled) {
		if (syra_site.currentQuickEdit &&
			syra_site.currentQuickEdit.needCloseConfirm(field.page, function() {
				exports.onPickerClick(event, picker, field);
			})) {
			return;
		}
		field.page.externalAdapter.onFieldClickPicker({
			...
			doEvent: function(options) {
				switch (picker.syraPickerType) {
					case "$quickEdit":
						field.list.builder.toggleQuickEdit(field);
						break;
					case "escape":
