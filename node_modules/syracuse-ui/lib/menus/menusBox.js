...
exports.MenusBox = helpers.defineClass(MenusBox, Box, {
    ...
    addMenuItem: function($menu, record, subMenuParent){
        if (!this.$item.$excludeBind || this.$item.$excludeBind.indexOf($menu.$bind) < 0) {
            ...
            if (!menuItem) {
                ...
                (menuItem.$$container = $(menuItem.liParent))[action]($$root);
            }
            ...
            if (this.$item.onAddMenuItem) {
                this.$item.onAddMenuItem(menuItem)
            }
            return menuItem;
        }
    },
    ...
    _renderLayoutContent: function(){
        ...
        if (!self.$item.$columnLimit) {
            self.$moreLimit = self.$item.$moreLimit || 200;
        }
        if (self.$item.$menus) {
    ...
    dispose: function(){
        ...
        this.$item.onAddMenuItem = this.onMenuClick = null;
