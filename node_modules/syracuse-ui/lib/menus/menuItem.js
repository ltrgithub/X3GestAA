...
var scInvoker = require('syracuse-ui/lib/events/shortCutInvoker');
...
function _load(menu, record) {
	...
	if (menu.$item.$shortCutTip) {
		...
		if (menu.$item.$shortCutDyn) {
			scInvoker.registerShortCut(menu, menu.$item.$shortCutDyn, menu.$item.$shortCutTip, _processShortCut, null, menu);
		}
	}
...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	disable: function($value) {
		...
		if (!syra_site.siteOptions.isOffice) {
			if (this.$isDisabled) {
	...
	dispose: function() {
		this.onItemInOut();
		if (this.$item.$shortCutDyn) {
			scInvoker.unregisterShortCut(this.$item.$shortCutDyn.item, this.$item.$shortCutTip);
		}
