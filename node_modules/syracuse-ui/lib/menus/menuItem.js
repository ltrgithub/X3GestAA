...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	loadBox: function(record) {
		...
		this.mn.setAttribute("data-s-inout", this.id);
	...
	onItemEnterLeave: function(event) {
		switch (event.type) {
			case "mouseenter":
				if (!this.mn.title && this.mn.children.length > 0 && document.site.isEllipsisActive(this.mn.children[0])) {
					this.mn.title = this.getTitle();
				}
				break;
		}
	},
		...
		if ($menu) {
			...
			if ($menu.$isSeparator || $menu.$title == "-") {
				...
				if ((this.$sourceBind || this.$item.$bind) == "$lazyload") {
					if (this.mn.parentNode) {
						this.mn.parentNode.removeChild(this.mn);
					}
				}
