...
function _initMenu(menu, $menu) {
	...
	if (menu.$isHidden) {
		...
		syra_site.dom.hide(menu.domItem, true);
	} else {
		if (menu.$item.$showIfSet) { //used by job
			menu.isInvisible = false;
			syra_site.dom.hide(menu.domItem, false);
		}
	}
...
function _setMenu(menu, $menu, record, outOfDate) {
	if ($menu) {
		...
		if ($menu.$isRequested === false) {
			if (!syra_workingCopy.onActionExecuted(menu, $menu)) {
				return;
			}
		}
		if ($menu.$variants) {
	...
	if ($menu || outOfDate) {
		...
		if (!menu.isRepresentationUrl && ($menu.$type || $menu.$title) && (menu.$sourceType.indexOf("html") < 0)) {
			...
			menu.value = syra_menus.getDocumentIcon(menu.$sourceType); //  menu.$sourceType.replace("application/", "").replace("/", "-");
			if (!menu.$item.$icon) {
				menu.$item.$icon = {
					$path: "file/s-link-"
				};
...
function _load(menu, record) {
	...
	if (menu.$item.$shortCutTip) {
		...
		if (menu.$item.$shortCut) {
			shortCutInvoker.registerShortCut(menu, menu.$item.$shortCut, menu.$item.$shortCutTip, _processShortCut, null, menu);
		}
	}
...
function _processShortCut(event, menu) {
	return syra_menus.click.menuItem(menu, event);
}
...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	dispose: function() {
		...
		if (this.$item && this.$item.$shortCut) {
			shortCutInvoker.unregisterShortCut(this.$item.$shortCut.item, this.$item.$shortCutTip);
		}
