...
function _onActionExecuted(menu, $menu) {
	if (menu.$item.onServerRequest ? menu.$item.onServerRequest(menu) : true) {
		...
		/*if ($menu.$clientAgent) {
         var agent = syra_site.agents && syra_site.agents[$menu.$clientAgent.$clientId];
         if (agent) {
		...
		if ($menu.$location && menu.$invocationMode == "async") { //$menu.$state
