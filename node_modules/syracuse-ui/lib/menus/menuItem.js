...
var Job = require('syracuse-ui/lib/jobs/jobHandlerSyra').JobHandlerSyra;
...
function _onMenuItemClick(menuItem) {
	...
	if (doClick) {
		if (menuItem.articleParent != menuItem.page) {
			doClick = menuItem.page.onMenuClick ? menuItem.page.onMenuClick(menuItem) : true;
		}
...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	loadBox: function(record) {
		...
		//this.mn.setAttribute("data-s-menu-bind", this.$item.$bind);
		...
		if (this.$skin == "s-page-menus-link") {
			...
			if (!this.$skin) {
				this.$skin = this.page.isDashBoard ? "s-mn-h2-link" : "s-link";
			}
		}
		this.layoutSlot.appendChild(this.mn);
		this.isInvisible = true; //invisible if not setMenu
		this._hide(true);

		this.mn.className = this.$skin;
		if (this.$item.$css) {
			this.mn.className += " " + this.$item.$css;
		}
		this.$$item = $(this.mn);
		...
		if (this.boxParent.isMenuGroup) {
			if (this.boxParent.$item.$style) {
				...
				document.site.toggleClass(this._label, $style, true);
				document.site.toggleClass(this.mn, $style, true);
			}
		}
	...
	setState: function(state) {
		if (state.$isHidden !== undefined) {
			this._hide(this.$isHidden = state.$isHidden);
		}
	},
	_hide: function(isHidden) {
		this.mn.style.display = isHidden ? "none" : "";
		if (this.layoutParent) {
			this.layoutParent.ensureItemVisibility(this, isHidden);
		}
	},
	...
	click: function() {
		...
		if (!self.$isDisabled) {
			if (self.page) {
				...
				if (self.page instanceof Site && self.page.mainPage && self.page.mainPage.externalAdapter) {
					options.isSiteMenu = true;
					page = self.page.mainPage;
				} else {
					page = self.page;
				}
	...
	_onMenuItemClick: function() {
		...
		var self = this;
		if (document.site.authorPage) {
			doClick = document.site.authorPage.onMenuClick(self);
		}
		if (doClick && self.boxParent) {
			doClick = self.boxParent.onMenuClick ? self.boxParent.onMenuClick(self) : true;
			if (doClick) {
				doClick = self.articleParent.onMenuClick ? self.articleParent.onMenuClick(self) : true;
			}
		}

		if (doClick) {
			if (self.articleParent != self.page) {
				doClick = self.page.onMenuClick ? self.page.onMenuClick(self) : true;
			}
			if (self.onMenuClick ? self.onMenuClick() : true) {
	...
	setMenu: function($menu, record) {
		...
		if ($menu) {
			...
			if (!this._isInit) {
				this.articleParent.onMenuDataFilled(this);
				this.mn.style.display = "";
				this.isInvisible = false;
				if (this.layoutParent) {
					this.layoutParent.ensureItemVisibility(this);
				}
				if (this.$item.$bind == "$lazyload") {
			...
			if (this.$isDisabled && $menu.$isRequested === false) {
				if (this.$item.onServerRequest ? this.$item.onServerRequest(this) : true) {
					...
					if ($menu.$location && $menu.$state) {
						var job = new Job();
						var jobParams = {};
						jobParams.$location = $menu.$location;
						jobParams.$state = $menu.$state;
						jobParams.title = this.$title;
						jobParams.uuid = $menu.$location.split(/'/)[1];
						jobParams.kind = "operation";
		...
		if (!this.$sourceUrl || ($menu && $menu.$url)) {
			...
			if (this.$type && (this.$type.indexOf('json') == -1) && (this.$type != "html")) {
				if (this.$sourceUrl && (this.$sourceUrl.indexOf("format=") < 0)) { // TODO: improve this hack
					this.$sourceUrl += (this.$sourceUrl.indexOf("?") >= 0 ? "&" : "?") + "format=" + this.articleParent.formatMenuUrl({
						$url: this.$type,
		...
		if (this.$sourceType && (this.$sourceType.indexOf('json') == -1) && (this.$sourceType != "html")) {
			...
			this.value = this.$sourceType.replace("application/", "");
			if (!this.$item.$icon) {
				this.$item.$icon = {
					$path: "file/s-link-"
				};
