...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	loadBox: function(record) {
		...
		if (this.$skin == "s-page-menus-link") {
			...
			if (!this.$skin) {
				this.$skin = this.page.isDashBoard ? "s-nav-menu-link" : "s-mn-bar-extra-link";
			}
		}
		...
		if (this.$item.$style) {
			css += " " + (this.$style = "s-cst-sty-" + this.$item.$style);
		}
		...
		if (this.boxParent.isMenuGroup && this.boxParent.$item.$style) {
			syra_site.dom.toggleClass(this.domItem, "s-menus-sty-" + this.boxParent.$item.$style, true);
		}
