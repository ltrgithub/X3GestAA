...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
	...
	setCssValue: function($value) {
		if (this.$cssValue != $value) {
			if (this.$cssValue) {
				var css = this.$item.$prefixValue + this.$cssValue;
				syra_site.dom.toggleClass(this._label, css, false);
				syra_site.dom.toggleClass(this.domItem, css, false);
			}
		}
		this.$cssValue = $value;
		if (this.$cssValue) {
			var css = this.$item.$prefixValue + this.$cssValue;
			syra_site.dom.toggleClass(this._label, css, true);
			syra_site.dom.toggleClass(this.domItem, css, true);
		}
	},
	setStyle: function($style) {
		if ($style === null) {
			if (this.$style) {
				syra_site.dom.toggleClass(this._label, this.$style, false);
				syra_site.dom.toggleClass(this.domItem, this.$style, false);
			}
			this.$style = null;
		} else {
			this.$style = "s-cst-sty-" + $style;
			syra_site.dom.toggleClass(this._label, this.$style, true);
			syra_site.dom.toggleClass(this.domItem, this.$style, true);
		}
	},
	...
	onItemInOut: function(onEnter) {
		if (onEnter) {
			if (this.outOfDate) {
				this.setMenu(null, this.articleParent.dataset);
				this.outOfDate = false;
			}
			if (this.titleText && this._label && !this.$item.$isTitleVisible) {
				this.domItem.title = "";
				if (this.$item.$noText || this._label.clientWidth != this._label.scrollWidth) {
					syra_site.siteFunctions.showTip(this, true, this.titleText);
				}
			}
		} else {
			if (this.domItem && this.titleText) {
				this.domItem.title = this.titleText;
			}
			// close on mouse leave, and if mouse is not over popup
			if (this.domItem && this.titleText) {
				this.domItem.title = this.titleText;
			}
			syra_site.siteFunctions.showTip(this, false);
		}
	},
	getArticle: function() {
	...
	setTitle: function($title) {
		...
		if (this._label) {
			if (this.$item.$format == '$html') {
				this._label.innerHTML = syra_site.formatHTMLText(title, false);
			} else {
		...
		if (!self.$isDisabled) {
			if (self.page) {
				...
				if (self.page.checkMenuDesignValidaty(self)) {
					...
					if (self.$bind == "$save" || self.$bind == "$saveAs") {
						isValidated = self.page.validateFields();
					}
					if (isValidated) {
						var options = {
							isSiteMenu: (self.page == syra_site),
							menuItem: self,
							doEvent: function() {
	...
	_onMenuItemClick: function(event) {
		...
		if (doClick) {
			...
			if (doClick && (self.onMenuClick ? self.onMenuClick(event) : true)) {
				syra_controller.executeMenu(self);
			}
		}
		setTimeout(function() {
			if (self) {
	...
	setMenu: function($menu, record) {
		if ($menu) {
			...
			if ($menu.$isSeparator || $menu.$title == "-") {
				...
				if ($menu.$value !== undefined && this.$item.$prefixValue) {
					this.setCssValue($menu.$value);
				}
				...
				if ($menu.$isDisabled && $menu.$isRequested === false) {
					if (this.$item.onServerRequest ? this.$item.onServerRequest(this) : true) {
						...
						if ($menu.$location && $menu.$state) {
							syra_site.registerJobSyra($menu.$location, $menu.$state, this.$title, $menu.$location.split(/'/)[1]);
						}
			...
			if (this.$type && (this.$type.indexOf('json') == -1) && (this.$type != "html")) {
				if (this.$sourceUrl && (this.$sourceUrl.indexOf("format=") < 0)) { // TODO: improve this hack
					this.$sourceUrl += (this.$sourceUrl.indexOf("?") >= 0 ? "&" : "?") + "format=" +
						syra_site.urlMaker.formatMenuUrl(this.articleParent, {
							$url: this.$type,
		...
		if (this.$sourceType && (this.$sourceType.indexOf('json') == -1) && (this.$sourceType != "html")) {
			if (this.isHrefEnabled()) {
				syra_site.history.setHref(this, record, true);
			}
			...
			if (this.domItem && this.$url && this.isHrefEnabled()) {
				syra_site.history.setHref(this, record);
			}
		}
	...
	dispose: function() {
		...
		this.popupTitle = this.titleTip = null;
	}
});