...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
    ...
    _createMenu: function(article, $menu){
        ...
        //this.$$item = $(this.$$container[0].appendChild(this.mn)); // previous version
		// little trick for the anchor to be prepended (in case of menuIcons div)
        this.$$item = $(this.$$container[0].insertBefore(this.mn, this.$$container[0].firstChild));	// new version
    ...
    setMenu: function($menu, record){
        ...
        if (self.$type && self.$type.indexOf('json') == -1) {
            ...
            self.value = self.$type.replace("application/", "");
            if (!self.$item.$icon) {
                self.$item.$icon = {
                    $path: "file/s-link-"
                };
            ...
            if (self.mn) {
                if (self.$url) {
                    ...
                    if (self.$url.indexOf("http") >= 0 && self.$url.indexOf(window.location.host) < 0) {
                        self.mn.setAttribute("target", self.$target = "blank");
                    }
                }
                else {
        ...
        if (self.$target == "blank") {
            self.mn.className += " s-external-link";
        }
    },
    dispose: function(){
        ...
        delete this.$item;
        delete this.id;
        delete this.liParent;
        delete this.$$container;
        delete this.$skin;
        delete this.$isSetMenuOn;
        delete this.mn;
        delete this.$$item;
        delete this.$$label;
        delete this.$title;
