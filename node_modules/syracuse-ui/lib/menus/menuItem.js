...
exports.MenuItem = helpers.defineClass(MenuItem, null, {
			...
			if (self.page) {
				if (self.page.checkMenuDesignValidaty(self)) {
					...
					var page = self.page.mainPage,
						dialog = document.site.getTopDialogPage();
	...
	_onMenuItemClick: function() {
		...
		if (doClick && self.contextField) {
			doClick = self.contextField.onMenuClick ? self.contextField.onMenuClick(self) : true;
		}
		...
		if (doClick) {
			...
			if (doClick && (self.onMenuClick ? self.onMenuClick() : true)) {
	...
	setVariants: function($variants) {
		...
		for (var ii = 0, jj = $keys.length; ii < jj; ii++) {
			if (!this.childVariants[$keys[ii]]) {
				...
				this.childVariants[$keys[ii]] = this.layoutParent.createChildItem($newItem, null, ++$startIndex);
			}
		}
		this._hide(true);
	},
	setMenu: function($menu, record) {
		...
		if (this.$sourceType && (this.$sourceType.indexOf('json') == -1) && (this.$sourceType != "html")) {
			if (this.isHrefEnabled()) {
				...
				this.mn.setAttribute("href", this.href = this.$url = this.articleParent.formatMenuUrl(this, record));
			}
			...
			if (this.mn && this.$url) {
				if (this.isHrefEnabled()) {
					this.$url = this.articleParent.formatMenuUrl(this, record);
					this.mn.setAttribute("href", this.href = "?url=" + encodeURIComponent(this.$url));
	...
	dispose: function() {
		...
		this.$url = this.mn = this.$$item = this._label = this.$title = this.$icon = this.iconValue = null;
