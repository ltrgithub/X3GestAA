...
function _clearDynamicMenu(parent) {
	if (_dynamicMenu) {
		...
		_dynamicMenu.articleParent && _dynamicMenu.articleParent.removeItem(_dynamicMenu, true, true);
		_dynamicMenu = null;
	}
}
...
exports.tunnel = function(field, event) {
	...
	} else if (field.picker_$tunnel || (field.$menusPopup && field.$menusPopup.$tunnel)) {
		field.page.externalAdapter.onFieldClickPicker({
			field: field,
			pickerType: "$tunnel"
		});
...
exports.menuId = function(field, $bind) {
	...
	if (!menuItem || !menuItem[0]) {
		var $menu = field.$menus && field.$menus[$bind];
		if ($menu) {
	...
	if (menuItem) {
		menuItem.$url = syra_url.formatMenuUrl(menuItem.articleParent, menuItem);
	...
	if (!menu.$isDisabled) {
		if (menu.page) {
			...
			if (menu.$bind == "$save" || menu.$bind == "$saveAs") {
				isValidated = syra_workingCopy.validateAllInputs(menu.page);
				if (isValidated && menu.page.uploaders && menu.page.uploaders.getCount()) {
