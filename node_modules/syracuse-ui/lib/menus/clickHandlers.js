...
function _onClick(menu, event) {
	...
	if (doClick && menu.$item.$ownerId) {
		var owner = syra_item.get(menu.$item.$ownerId);
...
exports.menuId = function(field, $bind) {
	...
	if (!menuItem || !menuItem[0]) {
		...
		if ($menu) {
			...
			_clearDynamicMenu();
			menuItem = _dynamicMenu = field.page.addItem(null, {
				$category: "link",
				$bind: $menu.$bind,
	...
	if (!menu.$isDisabled) {
		if (menu.page) {
			...
			if (menu.$bind == "$save" || menu.$bind == "$saveAs") {
				isValidated = syra_form.validate(menu.page);
			}
			if (isValidated) {
				var field;
