...
function _show(menu, show) {
	if (menu.isInvisible == show) {
		if (show == false && menu.$siNavigTestList) {
			//    debugger;
		}

		menu.domItem.style.display = show ? "" : "none";
		menu.isInvisible = !show;
		menu.layoutParent && menu.layoutParent.ensureItemVisibility(menu, !show);
	}
}
exports.show = _show;
...
function _onNotRequested(menu, $menu) {
	if (menu.$item.onServerRequest ? menu.$item.onServerRequest(menu) : true) {
		...
		if ($menu.$location && $menu.$state) {
			syra_site.registerJobSyra($menu.$location, $menu.$state, menu.$title, $menu.$location.split(/'/)[1]);
		}
	}
...
function _setMenu(menu, $menu, record) {
	if ($menu) {
		if ($menu.$type) {
		...
		if ($menu.$isSeparator || $menu.$title == "-") {
			menu.domItem.className = menu.$skin + "-separator";
		} else {
			...
			if ($menu.$title !== undefined || $menu.$iconValue !== undefined) {
				_setTitle(menu, $menu.$title, $menu.$iconValue);
			}
			if ($menu.$description !== undefined) {
				...
				if (menu._description) {
					...
					menu.domItem.title = $menu.$description;
				}
			}
			_setState(menu, $menu);
	...
	if (menu.$sourceType && (menu.$sourceType.indexOf('json') == -1) && (menu.$sourceType != "html")) {
		if (_isHrefEnabled(menu)) {
			syra_site.history.setHref(menu, record, true);
		}
		menu.value = menu.$sourceType.replace("application/", "").replace("/", "-");
		if (!menu.$item.$icon) {
			menu.$item.$icon = {
				$path: "file/s-link-"
			};
		...
		if (menu.domItem && menu.$url && _isHrefEnabled(menu)) {
			syra_site.history.setHref(menu, record);
		}
	}
	$menu && _ensureCss(menu, $menu.$style, $menu.$value);
};

function _setState(menu, state) {
	if (state.$isHidden !== undefined) {
		_show(menu, !(menu.$isHidden = state.$isHidden));
	}
}

exports.setState = _setState;
exports.show = _show;
...
exports.load = function(menu, record) {
	...
	syra_map[menu.domItem.syraItem = menu.domItem.syrainout = menu.id] = menu;
