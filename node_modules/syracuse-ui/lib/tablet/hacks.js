...
function events(site) {
	...
	site.$$layoutSlot.delegate("[data-m-picker]", "mousedown click", function(event) {
		...
		if (attr == "escapefield") {
			...
			if (input.length > 0) {
				var field = document.controller.findField(input[0]);
				if (field) {
					// Simulate escape
...
function menuItem() {
    ...
    function isMenuGroupHidden($item) {
        return ($item && $item.$clientId === "322") ? true: false; // 322 = FILE_EX (no need to load cvg class only for this test here, so just copied literal clientID)
    }
...
function adjustHighcharts()
{
    ...
    highCharts.TabularChart.prototype._printSupported = function() { return false; };
    highCharts.TabularChart.prototype._exportSupported = function() { return false; };
}
