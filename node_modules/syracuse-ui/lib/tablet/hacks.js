...
function field() {
	...
	Field.prototype.loadBox = function($$item) {
		...
		if (this._input) {
			// Add reset button at the end of icons list
			if (!this._tabletResetPicker) {
				this._tabletResetPicker = document.createElement("div");
				this.fieldValue.appendChild(this._tabletResetPicker).className = this.$skin + "-reset";
				this._tabletResetPicker.appendChild(this.createPicker("reset"));
			}
	...
	Field.prototype.setState = function(state) {
		...
		var a = [];
		a.push(this.boxPickers);
		a.push(this.menusBox ? this.menusBox._item : null);
		a.push(this._input);
	...
	Field.prototype.onFieldInputEvent = function(event) {
		...
		if (this._tabletResetPicker) {
			var s = this._tabletResetPicker.style;
			var hasReset = this.$isEditMode && !this.$isReadOnly;
			if (event.type == "focusin" && hasReset) {
				s.display = "table-cell";
				s.opacity = "1";
	...
	// Send shiftTAB event on "reset button" click
	var onClickPicker = Field.prototype.onClickPicker;
	Field.prototype.onClickPicker = function(btn) {
		var self = this;
		if (self._tabletResetPicker && self.page && self.page.externalAdapter) {
