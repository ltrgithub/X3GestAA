...
function events() {
	...
	document.site.$$layoutSlot.delegate("[data-m-picker]", "mousedown click", function(event) {
	...
	eventListeners.forEach(function(x) {
		if (x[0] == "wdgt.cell.keydown") {
			...
			x[1] = function(event) {
				...
				// Hack VK_ESCAPE
				if (event.target && !sapUtil.isSpecialKey(keycode) && keycode == keys["VK_ESCAPE"]) {
...
// Remove Error icon and Error message on grid field - Keep just the red border
...
function field() {
	...
	Field.prototype.loadBox = function($$item) {
		...
		if (self._input) {
			...
			if (self.page.$isFusionPage && !self._tabletEscapePicker && (_escapeFields.indexOf(this.$field.$type) >= 0)) {
				...
				btn.setAttribute("data-m-picker", "escape");
			...
			parent.children().each(function(idx, elmt) {
				var e = $(elmt);
				if (e.hasClass("s-field-context-menus") || e.hasClass("s-field-pickers") || e.hasClass("xxxxs-field-escape")) {
					e.css({
						"position": "relative",
						"display": "table-cell",
						"padding-right": "0px",
	...
	Field.prototype.showDiagnoses = function($diagnoses, options) {
		...
		if (self._tabletEscapePicker) {
			var ctxMenu = self.menusBox ? self.menusBox._item : null;
	...
	Field.prototype.mobileEscape = function() {
		var self = this;
		if (self._tabletEscapePicker) {
	...
	var showField = Field.prototype.showField;
	Field.prototype.showField = function(show) {
		var self = this;
		showField.call(self, show);
