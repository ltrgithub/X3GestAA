...
exports.MobileEventListener = helpers.defineClass(MobileEventListener, null /*EventListener*/ , {
	...
	_bindSwipeEvents: function() {
		...
		if ($(event.target).is("article.s-list *") ){
		...
		}else if  ($(event.target).is(".s-fusion-bar") ){
			event.preventDefault() ;
			var panel = $(".s-fusion-bar-slot");
			var article= document.site.findArticle(event.target) ;
			var barobj = article.fusionBar;
			barobj.collapse() ;
			//panel.css("width", "20px");
			
		}else if (".s-page-menubar-slot"){
			event.preventDefault() ;
		...
		//document.site.$$layoutSlot.delegate("article.s-list *", "swiperight", function(event, data) {
			...
			if (article.list && article.list.pagging) {
				...
				event.preventDefault() ;
				var panel = $(".s-fusion-bar-slot");
				var bar= $(".s-fusion-bar");
				var article= document.site.findArticle(event.target) ;
				var barobj = article.fusionBar;
				barobj.expand() ;
				//var field=document.site.findField($(event.target)) ;
				
				//panel.css("width", +(3*($(document).width())/4)+"px");
		...
		/*$(document).on("swiperightleftborder", function(event, data) {
			...
			var leftpanel = $(".s-fusion-bar-slot");
			//leftpanel.css("display", "table-cell");
			leftpanel.css("width", (3*($(document).width())/4)+"px");
		...
		document.site.$$layoutSlot.delegate(".s-page-menubar", "swiperight", function(event, data) {
			/*event.preventDefault() ;
			if  ($(event.target).is(".s-fusion-bar") ){
				//var panel = $(".s-fusion-bar-slot");
				//var bar= $(".s-fusion-bar");
				var article= document.site.findArticle(event.target) ;
				var barobj = article.fusionBar;
		...
		document.site.$$layoutSlot.delegate(".s-page-menubar-slot", "swipeleft", function(event, data) {
			/*event.preventDefault() ;
			if  ($(event.target).is(".s-fusion-bar") ){
				var article= document.site.findArticle(event.target) ;
				var barobj = article.fusionBar;
		...
		document.site.$$layoutSlot.delegate(".s-fusion-bar", "swiperight", function(event, data) {
			event.preventDefault();
			if ($(event.target).is(".s-fusion-bar")) {
				//var panel = $(".s-fusion-bar-slot");
				//var bar= $(".s-fusion-bar");
		...
		document.site.$$layoutSlot.delegate(".s-fusion-bar", "swipeleft", function(event, data) {
			event.preventDefault();
			if ($(event.target).is(".s-fusion-bar")) {
				var article = document.site.findArticle(event.target);
				var barobj = article.fusionBar;
		...
		$(document).on("taphold", function(event, data) { //
			...
			if (title) {
				...
				var p = tg.parents('#s-site'); // ('#s-site-body');
				var position = tg.offset();
				var tooltip = $('<div>');
				tooltip.html(title);
				tooltip.css("position", "absolute");
				tooltip.css("top", (position.top + 20) + "px");
				tooltip.css("left", (position.left + 20) + "px");
		...
		/*$(document).on("mousedown", function(event, data) {
			
			alert("touchstart/mousedown") ;
		});*/
		...
		/*document.site.$$layoutSlot.delegate(".d3-graph-node", "mousedown", function(event, data) {
		//$('svg').addEventListener("taphold",
