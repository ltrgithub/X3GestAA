...
function DesktopPage() {
	...
	DesktopPage.prototype.drawBox = function() {
		...
		/*    document.site.setPreferences("menuBar", {
         ...
         "$viewMode": "dock",
         }, ["$isResizerDisabled", "$swiper", "$isCollapsed", "$viewMode"]);
         document.site.setPreferences("menubar", {
         "$isResizerDisabled": true,
         "$swiper": true,
         "$isCollapsed": collapsed
         }, ["$isResizerDisabled", "$swiper", "$isCollapsed"]);
         document.site.setPreferences("dash-menubar", {
         "$isResizerDisabled": true,
         "$swiper": true,
         "$isCollapsed": false,
         "$viewMode": "dock",
         }, ["$isResizerDisabled", "$swiper", "$isCollapsed", "$viewMode"]);
         document.site.setPreferences("convergenceBar", {
         "$viewMode": "float",
         "$isResizerDisabled": true,
         "$swiper": true,
         "$isCollapsed": true,
         }, ["$viewMode", "$isResizerDisabled", "$swiper", "$isCollapsed"]);
         document.site.setPreferences("vignette-menubar", {
         "$isResizerDisabled": true,
         "$swiper": true,
         "$isCollapsed": collapsed,
         "$viewMode": "dock"
		...
		evtListener.load();
		evtListener._bindSwipePanel($(".s-dash-menubar-resizer"));
		evtListener._bindSwipePanel($(".s-page-menubar-resizer"));


	};


	DesktopPage.prototype.resizeSideBars = function() {
		if (document.site.body.clientWidth) {
			...
			isAutoFloat = false; //forces  the left panel  to open on the left for the Demo, as I do not yet replied if it is aligned 
			//on the right on purpose for portrait mode. If so  swipe  handlers should take  onto account this particular case 
			// and the opener picker image should be changed to show the right direction

			//isAutoFloat = true; makes the menubar right-aligned
