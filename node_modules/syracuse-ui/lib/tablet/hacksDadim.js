...
function touchover(site) {
	...
	}).delegate(".s-pager-link, a.s-page-pager-link", "touchend", function(event) {
...
function WorkBook() {
	...
	WorkBook.prototype.onFieldEvent = function(options, redirect) {
		...
		if (options && options.event && options.event.type === "focusin" && options.event.relatedTarget && options.event.relatedTarget.id != "s-site-body" && !$(options.event.relatedTarget).is(".s-fusion-bar-block-title")) {
			...
			var srcField = syra_store.findField(options.event.relatedTarget);
			...
			if (options.field.arrayLevel == "cell") {
				if (options.field.articleParent.$serverIndex)
					targetnl = options.field.articleParent.$serverIndex + 1;
			}
			...
			if (curentFusionField.xid != targetId || (insideGrid && curentFusionField.nl != targetnl)) {
				...
				if (sapUtil.isListScreen(syraUtil.getMetaFromObject(options.field).$bind)) {
					...
					var e = jQuery.extend(new jQuery.Event(), {
						type: "keydown",
						target: $(curDomElmt.domItem).get(0),
						keyCode: keys["VK_TAB"],
						isSimulated: true,
						shiftKey: (backward) ? true : false,
						originalEvent: {},
