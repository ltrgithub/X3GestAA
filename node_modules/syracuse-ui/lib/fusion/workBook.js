...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	onMenuItemClick: function(options) {
		var doEvt = true;
		var article = options.menuItem && options.menuItem.articleParent;
		...
		try {
			if (this.fusionSite && this.fusionSite.controller && this.fusionSite.controller._sapController) {
				...
				if (bProcess) {
					...
					err = [utilEx.makeDiagnosis(this.fusionSite._session.getlabel("error", "27", syra_site.fusionGateway.getMessage("16")), 4, stack, "27", utilEx.cltFusionNice, options.errorMsg || "")];
					...
					options.errorMsg = this.fusionSite._session.getlabel("error", "28", syra_site.fusionGateway.getMessage("17"));
					options.doEvent();
				}
			} else {
				options.doEvent();
