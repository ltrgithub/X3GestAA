...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	_togglePlainSheet: function(sheet, show) {
		var cssPage = "s-page",
			siteBody;
		document.site.dialogManager.activateBoxDialogs(sheet, show);
		...
		if (dialogs.length > 0) {
			for (ii = 0, len = dialogs.length; ii < len; ii++) {
				if (dialogs[ii]._content) {
					document.site.dialogManager.activateBoxDialogs(dialogs[ii]._content, show);
				}
				dialogs[ii].activate(show);
			}
			if (show) {
