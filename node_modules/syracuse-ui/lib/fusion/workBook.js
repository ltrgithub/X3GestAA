...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    setDataBind: function(field, $resources, record, metaData){
        if (metaData) {
            ...
            if (metaData.sty != undefined && field._applyConvergenceStyle) {
                field._applyConvergenceStyle(metaData.sty);
            }
        }
        return true;
    },
    ...
    applyDesignMetaExt: function(field, $field, $item){
        var controller, style;
        if($field.$convergenceStyle && (controller = syraUtil.getFusionController(field))) {
            style = {"sty": $field.$convergenceStyle};
            if (sapUtil.cureStyle(style, $item.$bind, controller.getSession())) {
                field._applyConvergenceStyle(style.sty);
            }
        }
        return true;
    },
    ...
    onFieldEvent: function(options,redirect){
        var doEvt = true, listner,currIstEdit;
    ...
    dispose: function(){
        ...
        delete self.httpQuery;
        delete self.openerHttpQuery;
        delete self.$$slot;
