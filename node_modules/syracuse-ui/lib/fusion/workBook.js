...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    setDataBind: function(field, $resources, record, metaData){
        if (metaData) {
            ...
            if (metaData.sty) {
                // TODO : 
                /*
                 - metaData.sty peut être vide : indique de supprimer le style personnalisé sur le field
                 - metaData.sty peut contenir une chaîne correspondant à des attributs css : appliquer le style personnalisé sur le field, mais pas son intitulé
                 - etc...
                 */
            }
        }
    ...
    selectSheet: function(sheet){
        var sheetMeta;
        if (sheet == undefined) {
            sheet = this._lastSelectedSheet;
        }
        if (this.selectedSheet && this.selectedSheet != sheet) {
            this.unselectSheet();
        }
        else {
            if (!this.selectedSheet && document.site.$$body) {
                document.site.$$body[0].style.display = "none";
                (this._lastSelectedSheet = this.selectedSheet = sheet).$$item.appendTo(sheet.$$container = document.site.$$body);
    ...
    loadSheet: function($itemPage, $facet, opts){
        ...
        (this._lastSelectedSheet = this.selectedSheet).selectSheet(true);
        this.fusionGateway.activatedBook = this;
    ...
    _disposeSheet: function(sheet){
        ...
        if (sheet && !sheet.disposed) {
            ...
            if (this._dialogs) {
                dialog = _.find(this._dialogs, function(dialog){
                    return dialog._content == sheet;
                });
            }
            if (dialog) {
    ...
    onMenuItemClick: function(options){
        var doEvt = true, article = options.menuItem && options.menuItem.getArticle ? options.menuItem.getArticle() : null;
    ...
    onFieldEvent: function(options){
        ...
        switch (options.event.type) {
            case "click":
                doEvt = this.fusionSite.controller.triggerAdx("wdgt." + (options.field && options.field.arrayLevel && !sapUtil.isListScreen(syraUtil.getMetaFromObject(options.field).$bind) ? options.field.arrayLevel : "field") + "." + options.event.type, {
            ...
            case "keydown":
            case "change":
                doEvt = this.fusionSite.controller.triggerAdx("wdgt." + (options.field && options.field.arrayLevel && !sapUtil.isListScreen(syraUtil.getMetaFromObject(options.field).$bind) && syraUtil.getFieldType(options.field) == syraUtil.dataTypes.booleanType ? options.field.arrayLevel : "field") + "." + options.event.type, {
                    "target": options.field,
                    "type": options.event.type,
                    "data": {
                        "nativeEvt": options.event
	...
	onGlobalClick: function(event){
        ...
        doEvt = !fusionController ? true : fusionController.triggerAdx("wdgt.win." + event.type, {"target": currWindow, "type": event.type, "data": {"nativeEvt": event}});
    ...
    dispose: function(){
        ...
        delete self._lastSelectedSheet;
