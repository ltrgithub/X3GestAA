...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    _togglePlainSheet: function(sheet, show){
        ...
        document.site.dialogManager.activateAll(sheet, show);
    ...
    _toggleDialogSheets: function(show, sheet2Show){
        document.site.dialogManager.activateAll(this, show);
        if (show) {
            var ensMP = false;
    ...
    setDataBind: function(field, $resources, record, metaData){
        if (metaData) {
            ...
            if (metaData.sty != undefined) {
                fieldHelper.applyCustomStyle(field, {
    ...
    _getDashBoard: function(){
        if (!this.dash) {
            this.dash = document.site.itemHelper.getDashBoard();
        }
    },
    loadSheet: function($itemPage, $facet, opts){
        ...
        if (syraUtil.isDialogFacet($facet)) {
            ...
            if (this.sheets.length == 0) {
                ...
                if (this.sheets.length === 1 && this.sheets[0].dialogWrapper) {
                    this.selectedSheet = _site.onMainPageChange($itemPage);
                }
                else {
    ...
    _loadDialog: function($itemPage, $facet, opts){
        ...
        if ($facet == "$lookup" || $facet == "$select") {
            dialog = document.site.dialogManager.openLookup(this, options);
        }
        else {
            dialog = document.site.dialogManager.openModal(this, options);
        }
    ...
    isListRecordEmpty: function(properties, dataRecord){
        return sapUtil.isEmptyGridDataRecordEx(properties, dataRecord);
    },
    ...
    dispose: function(){
        document.site.dialogManager.closeAll(this, true);
        if (this.fusionSite) {
        ...
        this.openerUrlSegments = this.selectedSheet = this.fusionGateway = null;
    }
});
