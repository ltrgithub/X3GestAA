...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	_togglePlainSheet: function(sheet, show) {
		var cssPage = "s-page",
			siteBody;
		syra_site.dialogManager.activateAll(sheet, show);
		if (show && sheet.domItem && sheet._displayded !== true) {
			sheet._displayded = show;
			siteBody = syra_site.body;
			siteBody.style.display = "none";
			syra_site.mainPage = sheet;
		...
		if (!show && sheet._displayded !== false) {
			sheet._displayded = show;
			syra_site.dom.removeChild(sheet.domItem);
		}
	},
	...
	_toggleDialogSheets: function(show, sheet2Show) {
		syra_site.dialogManager.activateAll(this, show);
		if (show) {
			var ensMP = false;
	...
	_getDashBoard: function() {
		if (!this.dash) {
			...
			var pages = syra_site.pageLoader.getLoaded();
		...
		if (syraUtil.isDialogFacet($facet)) {
			...
			if (this.sheets.length == 0) {
				...
				if (this.sheets.length === 1 && this.sheets[0].dialogWrapper) {
					...
					this.selectedSheet = _site.pageLoader.load($itemPage, true);
				}
			}
	...
	_loadDialog: function($itemPage, $facet, opts) {
		...
		if ($facet == "$lookup" || $facet == "$select") {
			dialog = syra_site.dialogManager.openLookup(this, options);
		} else {
			dialog = syra_site.dialogManager.openPage(this, options);
		}
		...
		try {
			if (this.fusionSite && this.fusionSite.controller && this.fusionSite.controller._sapController) {
				...
				if (bProcess) {
					...
					err = [utilEx.makeDiagnosis(this.fusionSite._session.getlabel("error", "27", syra_site.fusionGateway.getMessage(this.fusionSite._session.cdIsoLang, "16")), 4, stack, "27", utilEx.cltFusionNice, options.errorMsg || "")];
					...
					delete this.fusionSite.uncaughtErrorObj;
					syra_site.messageBox.dispose();
					options.errorMsg = this.fusionSite._session.getlabel("error", "28", syra_site.fusionGateway.getMessage(this.fusionSite._session.cdIsoLang, "17"));
					options.doEvent();
				}
			} else {
				options.doEvent();
	...
	dispose: function() {
		syra_site.dialogManager.closeAll(this, true);
		if (this.fusionSite) {
