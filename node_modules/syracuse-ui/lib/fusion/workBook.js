...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	_ensureSheetIsMainPage: function(sheet) {
		...
		if (sheet.$$item && sheet.$$item.closest(document.site.$$body).length == 0) {
			...
			if (sheet.boundFields) {
				...
				for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
					...
					for (var mm = 0, kk = fields.length; mm < kk; mm++) {
						var field = fields[mm];
						if (!field.disposed && field.isList && field.builder && field.builder.scroller) {
							field.builder.scroller.validateScrollVisibility();
						}
					}
				}
			}
	...
	selectSheet: function(sheet) {
		if (this.fusionGateway.activatedBook && this.fusionGateway.activatedBook != this) {
			...
			if (sheet) {
				...
				if (this.selectedSheet.$facet !== "$dashboard") {
	...
	loadSheet: function($itemPage, $facet, opts) {
		...
		var props = this.selectedSheet.$prototype.$properties; // ($itemPage.$representation && $itemPage.$representation.$prototype.$properties) ||  {};
