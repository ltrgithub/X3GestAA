...
var devTools = require('syracuse-ui/lib/site/aside/devToolsPanel');
...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	loadSheet: function($itemPage, $facet, opts, onLoaded) {
		function onLoadedEx(page) {
			onLoaded(page);
			devTools.restoreLinks(page);
		}
		var self = this;
		if (syra_fusion.syraUtil.isDialogFacet($facet)) {
			...
			self._loadDialog($itemPage, $facet, opts, function(dlg) {
				self.sheets.push(self.selectedSheet = dlg.page);
				onLoadedEx(self.selectedSheet);
			});
			...
			if (self.sheets.length == 0) {
				...
				if (self.sheets.length === 1 && self.sheets[0].overPanel) {
					syra_site.onMainPageChange($itemPage, function(page) {
						page.displayed = true;
						self.sheets.push(self.selectedSheet = page);
						onLoadedEx(page);
	...
	_loadDialog: function($itemPage, $facet, opts, onLoaded) {
		...
		syra_over.openModal(this, options);
	},
	onActivate: function(status) {
		...
		this._toggleOverSheets(status !== false);
