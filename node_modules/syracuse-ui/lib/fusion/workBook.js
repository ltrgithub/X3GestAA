...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	releaseMainPage: function(options) {
		...
		this._toggleDialogSheet(false);
	},
	_ensureSheetIsMainPage: function(sheet) {
		if (!sheet._displayded) {
	...
	_toggleDialogSheet: function(show, sheet2Show) {
		var dialogs = [],
			ensMP = false,
			ii, len, display = show ? "" : "none",
			sheet;

		function _toggleDialog(diag, disp) {
		...
		if (dialogs.length > 0) {
			for (ii = 0, len = dialogs.length; ii < len; ii++) {
				_toggleDialog(dialogs[ii], display);
			}
			if (show) {
				for (ii = this.sheets.length - 1; ii >= 0; ii--) {
					...
					if (!sheet.$dialogMode && !ensMP) {
						...
						_toggleDialog(sheet.dialogWrapper, "none");
					}
				}
				if (!ensMP) {
					// So dashboard!
	...
	selectSheet: function(sheet) {
		if ((sheet = sheet || this.sheets[this.sheets.length - 1])) {
			...
			if (sheet.$dialogMode) {
				this._toggleDialogSheet(true, sheet);
			} else
				this._ensureSheetIsMainPage(sheet);
	...
	onActivate: function(status) {
		...
		this._toggleDialogSheet(status !== false);
