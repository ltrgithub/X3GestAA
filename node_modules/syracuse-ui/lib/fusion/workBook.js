...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	loadSheet: function($itemPage, $facet, opts) {
		...
		if (syraUtil.isDialogFacet($facet)) {
			...
			if (this.sheets.length == 0 && $itemPage.$isEditMode) {
				syra_site.siteFunctions.refreshMainPageSecurity($itemPage);
			}
			...
			this.selectedSheet.displayed = true;
		}
	...
	applyEscape: function(field) {
		...
		if ((fieldCxt = sapController.getCurrInst()) && !fieldCxt.nl) {
			$fieldMeta = syraUtil.getMetaFromObject(field);
			$fusionPageMeta = syraUtil.getFusionPageMeta(field);
			currValue = $fusionPageMeta.winModel.getDataInputValue(fieldCxt.xid, 0);
			if (currValue && currValue.v != undefined && sapUtil.needToControlField(syraUtil.getFieldType(field))) {
				oldValue = $fusionPageMeta.winModel.getDataStoreValue($fieldMeta.$bind, 0, false, true);
				if (fieldCxt.fmt) {
					result = sapController.fmtWrapper.displayToEdit(fieldCxt.fmt, oldValue);
