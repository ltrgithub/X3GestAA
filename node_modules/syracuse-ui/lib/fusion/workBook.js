...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    setDataBind: function(field, $resources, record, metaData){
        if (metaData) {
            ...
            if (metaData.stt) {
                var syraMetaData = sapUtil.getState(metaData.stt);
                field.applyHiddenState(syraMetaData);
                field.applyDisableState(syraMetaData);
                field.applyMandatoryState(syraMetaData);
    ...
    onActivate: function(satus){
        this.fusionSite.activate(satus);
    },
    ...
    onMenuItemClick: function(options){
        ...
        if (this.fusionSite.controller) {
            doEvt = this.fusionSite.controller.trigger("wdgt.link.click", {
                "target": options.menuItem, "type": "click"});
        }
        if (doEvt) {
            options.doEvent();
        }
        ...
        if (this.fusionSite.controller && (boxMeta = syraUtil.getMetaFromObject(options.box))) {
            ...
            if(targetType && !options.onDrawParent && !this.fusionSite.controller.isFixtureMode()) {
				doEvt = this.fusionSite.controller.trigger("wdgt." + targetType + ".toggle", {
					"target": options.box, "type": "toggle", "data": options}, xid);
            }
            if(doEvt) {
                options.doEvent();
                if(targetType) {
                    this.fusionSite.controller.trigger("wdgt." + targetType + ".toggled", {
                        "target": options.box, "type": "toggled", "data": options}, xid);
                }
            }
        }
    },
        ...
        if (this.fusionSite.controller && (boxMeta = syraUtil.getMetaFromObject(options.box))) {
            ...
            if(targetType && !this.fusionSite.controller.isFixtureMode()) {
                doEvt = this.fusionSite.controller.trigger("wdgt." + targetType + ".click", {
                    "target": options.box, "type": "click", "data": options}, xid);
            }
    ...
    onListSelectLine: function(options){       
        var doEvt = true;
        if (this.fusionSite.controller && !options.forceDoEvt) {
			doEvt = this.fusionSite.controller.trigger("wdgt.list.selline", {
				"target": options.list,
				"type": "selline",
				"data": options
			});
    ...
    onFieldEvent: function(options){
        /*
         field : instance du field
         event: event jquery
         doEvent: : effectue l'evenement par d�fault
         */
        ...
        options.doEvent();
        //return false pour ignorer le comportement par d�faut de syracuse 
    },
