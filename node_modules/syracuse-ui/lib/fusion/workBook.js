...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    _togglePlainSheet: function(sheet, show){
        var cssPage = "s-page", siteBody;
        this._toggleDialogsInSheet(sheet, show);
        if (show && sheet.$$item && sheet._displayded !== true) {
            sheet._displayded = show;
            siteBody = document.site.body;
            siteBody.style.display = "none";
            document.site.mainPage = sheet;
            sheet.$$item.appendTo(sheet.layoutSlot = siteBody);
            //sheet.$$item.addClass(cssPage);
            //sheet.$$item.show();
            siteBody.style.display = "";
        }
        else 
            if (!show && sheet._displayded !== false) {
                sheet._displayded = show;
                if (!sheet.$$fusionMemHost) {
                    sheet.$$fusionMemHost = $(document.createElement("div"));
                }
                sheet.$$fusionMemHost.append(sheet.$$item);
            //sheet.$$item.hide();
            //sheet.$$item.removeClass(cssPage);
            }
    },
    ...
    _toggleDialog: function(diag, show){
        var disp = show ? "" : "none";
        diag.isDeactivated = !show;
        if (diag.overlay) {
            diag.overlay.style.display = disp;
        }
        diag.dialogSlot.style.display = disp;
    },
    _toggleDialogsInSheet: function(sheet, show){
        var i, len, diag;
        if (sheet._dialogs && (len = sheet._dialogs.length) > 0) {
            for (i = 0; i < len; i++) {
                diag = sheet._dialogs[i];
                if (!diag.disposed) {
                    this._toggleDialog(diag, show);
                }
            }
        }
    },
    _toggleDialogSheets: function(show, sheet2Show){
        var dialogs = [], ensMP = false, ii, len, sheet;
        if (this._dialogs) {
            for (ii = 0, len = this._dialogs.length; ii < len; ii++) {
                if (!this._dialogs[ii].disposed) {
                    dialogs.push(this._dialogs[ii]);
                }
            }
        }
        if (dialogs.length > 0) {
            for (ii = 0, len = dialogs.length; ii < len; ii++) {
                if (dialogs[ii]._content) {
                    this._toggleDialogsInSheet(dialogs[ii]._content, show);
                }
                this._toggleDialog(dialogs[ii], show);
            }
            if (show) {
                for (ii = this.sheets.length - 1; ii >= 0; ii--) {
                    sheet = this.sheets[ii];
                    if (!sheet.$dialogMode && !ensMP) {
                        this._togglePlainSheet(sheet, true);
                        ensMP = true;
                    }
                    else 
                        if (ensMP && sheet.$dialogMode) {
                            this._toggleDialog(sheet.dialogWrapper, false);
                        }
                }
                if (!ensMP && this.dash) {
                    // So dashboard!
                    this._togglePlainSheet(this.dash, true);
                }
            }
        }
    },
    ...
    getPagerTotalRcdLabel: function(pagerBoundList, theoreticNb, dfltLabel){
        var listMeta, realNb = null, cvgWin = syraUtil.getFusionPageMeta(pagerBoundList);
    ...
    _unselectSheet: function(sheet, onOpenNewOne, nextOneSheet){
        ...
        if (sheet && sheet.$$item) {
            ...
            if (!realSheet.$dialogMode && (nextOneSheet == null || (nextOneSheet != realSheet && !onOpenNewOne))) {
                ret = true;
                this._togglePlainSheet(realSheet, false);
            }
            return ret;
        }
    },
    ...
    _getDashBoard: function(){
        var i, len, keys, found = null;
        if (!this.dash) {
            keys = Object.keys(document.site.pagesMap);
            for (i = 0, len = keys.length; i < len && found === null; i++) {
                if (document.site.pagesMap[keys[i]].$facet === '$dashboard') {
                    found = document.site.pagesMap[keys[i]];
                }
            }
            if (found) {
                this.dash = found;
    ...
    loadSheet: function($itemPage, $facet, opts){
        ...
        if (syraUtil.isDialogFacet($facet)) {
            ...
            if (this.sheets.length == 0) {
                this.selectedSheet = _site.onMainPageChange($itemPage);
            }
            else {
                this._toggleDialogSheets(false);
                this._unselectSheet(this.selectedSheet, true, null);
                _site.body.style.display = "none";
                $itemPage.boxParent = null;
                $itemPage.layoutSlot = _site.body;
                if (this.sheets.length === 1 && this.sheets[0].$dialogMode) {
                    this.selectedSheet = _site.onMainPageChange($itemPage);
                }
                else {
                    this.selectedSheet = _site.loadNewPage($itemPage);
                }
                _site.body.style.display = "";
            }
            this.selectedSheet._displayded = true;
        }
        _site.resize();
        this.sheets.push(this.selectedSheet);
        if (_site.getPageSortedFields) {
            _site.getPageSortedFields(this.selectedSheet);
        }
    ...
    _loadDialog: function($itemPage, $facet, opts){
        ...
        if (opts && opts.validate) {
            options.onValidate = function(sheet){
    ...
    _disposeSheet: function(sheet){
        ...
        if (sheet && !sheet.disposed) {
            if (sheet.$$fusionMemHost) {
                ...
                sheet.$$fusionMemHost = null;
            }
            delete sheet.externalAdapter;
            if (this._dialogs && (diag = this.findDialog(sheet))) {
                this.closeDialog(diag);
            }
            document.controller.disposeObject(diag || sheet);
        }
    },
    ...
    onFieldEvent: function(options, redirect){
        var doEvt = true, logger;
        var listner, aListner = ["wdgt", "", options.event.type], currIstEdit;
        if (!this.isPageDesigned()) {
            ...
            if (list && this.fusionSite.controller && (listMeta = syraUtil.getMetaFromObject(options.field))) {
                ...
                if (sapUtil.isListScreen(syraUtil.getMetaFromObject(options.field).$bind)) {
                    var doEvt = this.fusionSite.controller.triggerAdx("wdgt.list.selline", {
                        ...
                        "data": {
                            "nativeEvt": options.event,
                            "line": (options.uuidTarget ? list.findDataRecord(options.uuidTarget).dataRecord.$serverIndex + 1 : undefined),
    ...
    onSearchClickEvent: function(options){
        var list = options ? options.field : null, doEvt = true;
    ...
    onFilterEvent: function(options){
        var doEvt = true, doEvt = this.fusionSite.controller.triggerAdx("wdgt.list." + options.type, {
            "target": options.list,
            "data": options
        });
        if (doEvt && options.doEvent) {
    ...
    onRemoveUploadedFile: function(options){
        if (this.fusionSite.controller.triggerAdx("wdgt.file.uploadRemove", {
            "target": options.field,
            "type": "uploadRemove",
            "data": options
        })) {
    ...
    getFieldEvalTitle: function(field){
        var ret = null, fieldMeta = syraUtil.getMetaFromObject(field), pageMeta;
        ...
        try {
            if (this.fusionSite && this.fusionSite.controller && this.fusionSite.controller._sapController) {
                ...
                if (bProcess) {
                    ...
                    delete this.fusionSite.uncaughtErrorObj;
                    document.site.messageBox.dispose();
