...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	_ensureSheetIsMainPage: function(sheet) {
		...
		if (sheet.boundFields) {
			var $binds = Object.keys(sheet.boundFields);
			for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
				var fields = sheet.boundFields[$binds[ii]];
				for (var mm = 0, kk = fields.length; mm < kk; mm++) {
					var field = fields[mm];
					if (!field.disposed && field.isList && field.scroller) {
						field.scroller.validateScrollVisibility();
					}
				}
			}
		}
			...
			if (show) {
				for (ii = this.sheets.length - 1; ii >= 0; ii--) {
					...
					if (!sheet.$dialogMode && !ensMP) {
						...
						this._toggleDialog(sheet.dialogWrapper, "none");
					}
				}
				if (!ensMP) {
					// So dashboard!
					this._ensureSheetIsMainPage(this.dash);
				}
			}
		}
	},
	...
	loadSheet: function($itemPage, $facet, opts) {
		...
		if (document.site.isTablet) {
			var props = this.selectedSheet.$prototype.$properties;
			this.selectedSheet.fieldsOrder = {};
			_sortFields(props, this.selectedSheet.fieldsOrder);
		}
	...
	_disposeSheet: function(sheet) {
		...
		if (sheet && !sheet.disposed) {
			if (this._detachPlainSheet && sheet.$$fusionMemHost) {
