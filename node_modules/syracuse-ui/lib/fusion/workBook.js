...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	setDataBind: function(field, $resources, record, metaData) {
		if (metaData) {
			...
			if (metaData.stt != undefined && field.setState) {
				field.setState(syra_fusion.fusionUtil.getState(metaData.stt));
			}
	...
	applyDesignStyle: function(field, $field, $item) {
		...
		if (propStyle && (controller = syra_fusion.syraUtil.getFusionController(field))) {
			...
			if (syra_fusion.fusionUtil.cureStyle(style, $item.$bind, controller.getSession())) {
	...
	setButtonValue: function(options) {
		...
		if (this.fusionSite.controller) {
			var widget = (options.target && options.target.arrayLevel) ? ((syra_fusion.fusionUtil.isListScreen(syra_fusion.syraUtil.getMetaFromObject(options.target).$bind)) ? "list" : options.target.arrayLevel) : "field";
	...
	onMenuItemClick: function(options) {
		...
		var isLeftListItem = articleMeta && articleMeta.$bind && syra_fusion.fusionUtil.isListTab(articleMeta.$bind) || null;
	...
	onBoxToggle: function(options) {
		if (this.isPageDesigned()) {
			...
			if (this.fusionSite.controller && (boxMeta = syra_fusion.syraUtil.getMetaFromObject(options.box))) {
				targetType = (xid = syra_fusion.fusionUtil.isFolderTab(boxMeta[syra_fusion.fusionUtil.metaNameMap.xid])) ? "tabs" : null;
				targetType = xid ? targetType : ((xid = syra_fusion.fusionUtil.isListTab(boxMeta[syra_fusion.fusionUtil.metaNameMap.xid])) ? "list" : null);
			...
			switch (options.event.type) {
				case "focusin":
					if (syra_site.isTabletDevice) {
						...
						var fieldCxt = syra_fusion.fusionUtil.makeIst($fusionPageMeta.winModel.getWinId(), $fieldMeta.$bind, syra_fusion.syraUtil.getFieldNL(options.field) || 0);
						if (!syra_fusion.fusionUtil.cmpIst(currentCxt, fieldCxt, true)) {
				...
				case "mousedown":
					...
					if (!redirect && options.field.arrayLevel) {
						if (!syra_fusion.fusionUtil.isListScreen(syra_fusion.syraUtil.getMetaFromObject(options.field).$bind)) {
					...
					break;
				case "change":
					listner = "wdgt." + (options.field && options.field.arrayLevel && !syra_fusion.fusionUtil.isListScreen(syra_fusion.syraUtil.getMetaFromObject(options.field).$bind) && syra_fusion.syraUtil.getFieldType(options.field) == syra_fusion.syraUtil.dataTypes.booleanType ? options.field.arrayLevel : "field") + "." + options.event.type;
	...
	onSelectRecordEvent: function(options) {
		...
		if (doEvt && options.doEvent) {
			options.doEvent(syra_fusion.fusionUtil.isEmptyGridDataRecord);
		}
	},
	onSortClickEvent: function(options) {
		...
		var isLeftListItem = (fieldMeta && fieldMeta.$bind && syra_fusion.fusionUtil.isListScreen(fieldMeta.$bind) || null);
		var doEvt = true;
