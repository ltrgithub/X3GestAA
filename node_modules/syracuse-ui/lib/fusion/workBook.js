...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
    ...
    _ensureSheetIsMainPage: function(sheet){
        ...
        if (sheet.$$item.closest(document.site.$$body).length == 0) {
            ...
            sheet.$$item.appendTo(sheet.layoutSlot = document.site.$$body[0]);
            sheet.layoutSlot.style.display = "";
        }
    },
    _toggleSheetsTree: function(show){
        if (this._dialogs) {
            var display = show ? "" : "none";
            for (var ii = 0, jj = this._dialogs.length; ii < jj; ii++) {
                var dialog = this._dialogs[ii];
                dialog.isDeactivated = !show;
                dialog.$$overlay[0].style.display = display;
                dialog.layoutSlot.style.display = display;
            }
            if (show) {
    ...
    sortArray: function(list, $resource){
        return true;
        var cvgWin = syraUtil.getFusionPageMeta(list), ret = true;
        if((cvgWin = cvgWin && cvgWin.winModel)) {
            $resource.dataRecordSet = cvgWin.getDataStoreValue(list.$item.$bind, null, false, true, {"rawVal": true, "sort": {"$orderBy": $resource.$orderBy}});
            ret = false;
        }        
        return ret;
    },
    ...
    loadSheet: function($itemPage, $facet, opts){
        ...
        if (syraUtil.isDialogFacet($facet)) {
            ...
            if (this.sheets.length == 0) {
                ...
                $itemPage.layoutSlot = document.site.$$body[0];
    ...
    _loadDialog: function($itemPage, $facet, opts){
        ...
        if (this.sheets.length > 0) {
            this._unselectSheet(this.selectedSheet, document.site.mainPage != this.selectedSheet);
            delete this.selectedSheet;
        }
    ...
    onActivate: function(status){
        ...
        this._toggleSheetsTree(status !== false);
            ...
            if (this._dialogs) {
                for (var ii = 0, jj = this._dialogs.length; ii < jj; ii++) {
                    if (this._dialogs[ii]._content == sheet) {
                        this.closeDialog(dialog = this._dialogs[ii]);
                        break;
                    }
                }
            }
    ...
    onFieldEvent: function(options, redirect){
        ...
        if (!document.site.authorPage) {
            ...
            if (logger) {
                logger.trace({}, "**onFieldEvent : [" + options.event.type + "] on field [" + (syraUtil.getMetaFromObject(options.field)).$bind + " " + (options.field && options.field.arrayLevel ? options.field.arrayLevel : "field") + "]");
            }
            switch (options.event.type) {
                case "click":
                    ...
                    this.fusionSite._session.logger.trace({}, " -----> Listner : [" + aListner.join(".")  + "] field type : [" + syraUtil.getFieldType(options.field) + "] Edit :[" + this.fusionSite.controller._sapController.getCurrInst().edit + "]");
    ...
    onSortClickEvent: function(options){
        ...
        this.fusionSite._session.logger.trace({}, "**onSortClickEvent : [" + options.event.type + "] on field [" + (syraUtil.getMetaFromObject(options.field)).$bind + " " + (options.field && options.field.arrayLevel ? options.field.arrayLevel : "field") + "]");
    ...
    onBlockExRpc: function(options){
        var doEvt = document.site.authorPagetrue ? false : this.fusionSite.controller.triggerAdx("wdgt.xblock.rpc", {
            "target": options.field,
            "type": "rpc",
            "data": options
        });
    ...
    onGraphClickPicker: function(options){
        var doEvt = document.site.authorPagetrue ? true : this.fusionSite.controller.triggerAdx("wdgt.graph.picker", {
            "target": options.field,
            "type": "picker",
            "data": options
        });
