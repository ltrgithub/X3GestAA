...
exports.WorkBook = helpers.defineClass(WorkBook, null, {
	...
	closeSheet: function(event, fusionDispose) {
		...
		function isForceBackUrl(url) {
			return url && syra_site.firstUrl && url !== syra_site.firstUrl && url.indexOf("/auth/") === -1;
		}

		if (this.sheets.length === 1 && isForceBackUrl(syra_site.lastHref)) {
			history.forceBack(this.sheets[0].historyback); // force history back to don't keep the fusion url
		}
	...
	onCloseAllSheets: function(callback) {
		this.sheets.length && this.fusionSite.controller.triggerAdx("wdgt.win.closeall", {
			"target": this,
			"type": "close",
			"callback": callback
		});
	...
	applyShortCut: function(field, shortcuts, event) {
		...
		if ((fieldCxt = sapController.getCurrInst())) {
			...
			} else if (!(field.arrayLevel && field.arrayLevel === "cell")) {
