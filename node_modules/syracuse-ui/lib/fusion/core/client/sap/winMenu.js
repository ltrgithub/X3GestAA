...
var _menuFlat = false, _menuWithMagneto = true, _protoMnuInsertRank = 9000, _helpMnuInsertRank = _protoMnuInsertRank + 100;
var _menuIDs = {	
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    ...
    _addInternalCoreMenu: function() {
    	...
    	_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.DISPLAY_EX, this._idMnuCpt++, null, "26", 1, 52, null, "SRV_FICHIER_LIAISON"));
		...
		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 2, 50, null, null));
  		...
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "10", 1, 21, null, "SRV_ADXEXECUTE", true));
  		...
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "10", 1, 21, null, "SRV_ADXEXECUTE", true));
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
 		var i, j, len, linksArray, layout, timer = utilBis.getTimer(true), bar = this._getWinMenuBar(winId, winFacet, winMenusDef, winProto, _menuFlat);
		if(!_menuFlat) {
			// Set links
			if(winProto.$links) {
			...
			if(layout.$items.length === 0) {
				...
				i = 0;
				while(bar.$menus.$items[i].rnk <= _protoMnuInsertRank) {
					layout.$items.splice(i, 0, bar.$menus.$items[i++]);
				}
	...
	_getWinMenuBar: function(winId, winFacet, winMenusDef, winProto, flat){
		...
		(this.winMenuColl[winId] = new WinMenu()).initialize(this, winMenusDef, this._coreMenus, winProto, winFacet, flat, this._mnuWithMagneto, this._session);
    ...
    dispose: function(){
   	...
   	_getCoreMenuIDs: function() {
		...
		ids[_menuIDs.SUBMENU_PRINT_CMN] = {"tag":"SUBMENU_PRINT_CMN", "txt": this._session.getlabel("text", 3, "Print"), "typ": 0, "opts": {"collapsed": true, "ico": "6", "icoVp": "x3ico", "layout": "row"}};
		...
		ids[_menuIDs.STATUS_CMN] = {"tag":"STATUS_CMN", "txt": "", "typ": 7};
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winMenus, coreMenus, winProto, winFacet, flat, showMagneto, session){
  	...
  	_addWinProtoAction: function(actions, localization){
   	...
   	buildMenus: function(){
   		...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.STATUS_CMN, "rnk": 3}, {"id": _menuIDs.FILE_EX, "rnk": _helpMnuInsertRank - 10}, {"id": _menuIDs.DISPLAY_EX, "rnk": _helpMnuInsertRank - 5}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": 15}, {"id": _menuIDs.WINDOW_GIRL, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _helpMnuInsertRank}];
    	}
   		else if (this._winFacet !== syraUtil.pageFacet.lookup){
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": 3}, {"id": _menuIDs.DISPLAY_EX, "rnk": _helpMnuInsertRank - 10}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": 15}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": menuIDs.HELP_EX, "rnk": _helpMnuInsertRank}];
   		}
   		...
   		// ... add "actions" menus...
   		if(this._hasActWin) {
   			_addMenu(this._menus, null, this._menuManager._getMenuItem(_menuIDs.BTN_WIN, this._menuManager._idMnuCpt++, null, "14", 1, 2, null, "SRV_FICHIER_NOUVEAU", false, {"sty": "main"}));
   	...
   	buildTreeMenusObj: function(){
		...
		function walk(srcItems, trgtItems) {
    		var len, srcItemsArray, i, act, style, mnuItem, layout;
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
	    		if (!srcItems[srcItemsArray[i]].inactive && !_kMnuHelper.isItemSep(srcItems[srcItemsArray[i]].typ)) {
	    			...
	    			if((_kMnuTypes[srcItems[srcItemsArray[i]].typ] & _kMnuHelper.cat.popup) && ((srcItems[srcItemsArray[i]].items && (Object.keys(srcItems[srcItemsArray[i]].items)).length > 0) || _kMnuHelper.isPopupNoFacet(srcItems[srcItemsArray[i]].typ)))  {
						...
						if((layout = srcItems[srcItemsArray[i]].opts)) {
							if((layout = layout.layout)) {
	    						mnuItem.$layout.$layoutType = layout;
								//mnuItem.$skin= "s-test-menus-icon";
							}
							if(srcItems[srcItemsArray[i]].opts.collapsed) {
								mnuItem.$opened = false;
							}
						}
  	...
  	buildFlatMenusObj: function(){
   		...
   		function walk(items) {
	    	_.each(items, function(menu) {
	    		if (!menu.inactive && !_kMnuHelper.isItemSep(typ)) {
	    			if(_kMnuTypes[menu.typ] & _kMnuHelper.cat.popup) {
	    				walk(menu.items);
	    			}
	    			else {
