...
var _kVirtualPath = {"file": "file/", "x3ico": "x3icons/"}, _menuWithMagneto = true, _kWinOwnMagneto = false;
var _kMnuHelper = {
...
var _kStatusWithMenu = {"260": {"mnu": {"act": "SRV_FICHIER_COMMENTAIRE", "ico": _kMnuHelper.stdIco.comment}, "status": {"ico": "305", "acts": [3355]}},
						"238": {"mnu": {"act": "SRV_FICHIER_PIECE_JOINT", "ico": _kMnuHelper.stdIco.pj}, "status": {"ico": "304", "acts": [3356]}}
	};
var _kMnuTypes = {
...
var _insertRank = {"status": 3, "protoMnus": 9000, "help": 100};
var _menuIDs = {	
...
function _cureMenu (menuItem){
	...
	if(menuItem.id == 18000 && menuItem.rnk == 58 && menuItem.fid == _menuIDs["FILE_EX"] && menuItem.typ == 0) {
		menuItem["opts"] = {"layout": "row", "collapsed" : true};
	}
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    ...
    _addInternalCoreMenu: function() {
    	...
    	_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.FILE_EX, this._idMnuCpt++, null, "26", 1, 22, null, "SRV_FICHIER_LIAISON"));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "16", 4, 98, null, "SRV_FICHIER_PIECE_JOINT", false, {"ico": _kMnuHelper.stdIco.pj, "icoVp": "x3ico"}));		   		
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "17", 4, 99, null, "SRV_FICHIER_COMMENTAIRE", false, {"ico": _kMnuHelper.stdIco.comment, "icoVp": "x3ico"}));		   		
  		// Items of HELP_EX
		...
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 2, 50, null, null));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "10", 1, 21, null, "SRV_ADXEXECUTE", false));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "10", 1, 21, null, "SRV_ADXEXECUTE", false));
	...
	_addCoreSubMenuToRoot: function(fid, id, rank){
		if(!this.coreMenus[fid].items) {
			this.coreMenus[fid].items = {};
		}
 		this.coreMenus[fid].items[id] = this.coreMenus[id];
 		this.coreMenus[fid].items[id].rnk = rank;	   		
 		this.coreMenus[fid].items[id].typ = this.coreMenus[id].typ || 0;
	},
	...
	_getWinMenuBar: function(winId, winFacet, winMenusDef, winProto, setMainGrp){
		...
		(this.winMenuColl[winId] = new WinMenu()).initialize(this, winMenusDef, this.coreMenus, winProto, winFacet, setMainGrp, this._mnuWithMagneto, this._session);
	...
	addStatusIco: function(winId, syraUISheet, icoId, act, showFlag){
		var ret = true, winMenu, isStatusMnu = _kStatusWithMenu[icoId] && _kStatusWithMenu[icoId].status.acts.indexOf(act) >= 0, newItem = {"$links": {}};
		if((winMenu = this.winMenuColl[winId]) && syraUISheet) {
			if(isStatusMnu && winMenu.menuBarTree.items[_menuIDs.STATUS_CMN]) {
				...
				this._updateMnuItem(syraUISheet, requestActions.getSrvAction(_kStatusWithMenu[icoId].mnu.act), true);
			}
			else {
				isStatusMnu = false;
			}
			if(showFlag) {
				// Menu ico already exist, just show it
				this._updateMnuItem(syraUISheet, act, false);
			}
			else {
				// Add ico
				newItem.$links[act] = {"$act": act};
				_kMnuHelper.appendIco({"typ": "4", "opts": {"icoVp": "x3ico", "ico": !isStatusMnu ? icoId : _kStatusWithMenu[icoId].status.ico}}, newItem.$links[act]);
				syraUISheet.addMenuItem(newItem, _menuIDs.STATUS_CMN);
			}
		}
		return ret;
	},
	...
	removeStatusIco: function(winId, syraUISheet, icoId, act){
		var ret = true, statusMnuAct, winMenu, isStatusMnu = _kStatusWithMenu[icoId] && _kStatusWithMenu[icoId].status.acts.indexOf(act) >= 0;
		if((winMenu = this.winMenuColl[winId]) && syraUISheet) {
			if(isStatusMnu && winMenu.menuBarTree.items[_menuIDs.STATUS_CMN]) {
				// Show standard status menu ico
				statusMnuAct = requestActions.getSrvAction(_kStatusWithMenu[icoId].mnu.act);
				this._updateMnuItem(syraUISheet, statusMnuAct, false, !this._hoster.isActionEnabled(winId, statusMnuAct));
			}
			this._updateMnuItem(syraUISheet, act, true);
		}
		return ret;
	},

	_updateMnuItem: function(syraUISheet, id, isHidden, isDisabled){
    ...
    dispose: function(){
   	...
   	_getCoreMenuIDs: function() {
		...
		ids[_menuIDs.SUBMENU_PRINT_CMN] = {"tag":"SUBMENU_PRINT_CMN", "txt": this._session.getlabel("text", 3, "Print"), "typ": 0, "opts": {"collapsed": false, "ico": "6", "icoVp": "x3ico", "layout": "row"}};
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winMenus, coreMenus, winProto, winFacet, setMainGrp, showMagneto, session){
  	...
  	_addWinProtoAction: function(actions){
   	...
   	buildMenus: function(){
   		...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status}, {"id": _menuIDs.FILE_EX, "rnk": _insertRank.help - 10}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": 15}, {"id": _menuIDs.WINDOW_GIRL, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help}];
    	}
   		else if (this._winFacet === syraUtil.pageFacet.modal2){
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status}, {"id": _menuIDs.FILE_EX, "rnk": _insertRank.help - 10}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": 15}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help}];
   		}
