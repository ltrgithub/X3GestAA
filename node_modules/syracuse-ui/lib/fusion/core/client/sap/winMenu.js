...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	_statusWithMenu: {
		"260": {"mnu": {"act": "SRV_FICHIER_COMMENTAIRE", "ico": _kMnuHelper.stdIco.comment}, "status": {"ico": "305", "acts": [3355], "rnk": _insertRank.cmntInStatus, "txt": "17"}},
		"238": {"mnu": {"act": "SRV_FICHIER_PIECE_JOINT", "ico": _kMnuHelper.stdIco.pj}, "status": {"ico": "304", "acts": [3356], "rnk": _insertRank.pjInStatus, "txt": "16"}}
	},
		...
		_.each(this._statusWithMenu, function(item, key){
			_.each(item.status.acts, function(act, idx){
				// By default, add and hide alternative icon "status" menu bound with menu item
		  		_addMenu(self.coreMenus, self._tabMenusFather, self._getMenuItem(_menuIDs.STATUS_CMN, self._idMnuCpt++, null, this.status.txt, 4,  this.status.rnk + idx + 1, act, null, false, {"ico": this.status.ico, "icoVp": "x3ico", "hidden": true}));		   		
			}, item);
		});
  		// Items of HELP_EX
