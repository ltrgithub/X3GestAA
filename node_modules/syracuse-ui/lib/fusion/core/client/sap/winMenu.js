...
function _accelDecode(accel) {
	...
	if (accel) {
		// Modifier...
		if (accel > _accelMap.altgr.code) {
			// Not yet supported
			accel -= _accelMap.altgr.code;
		}
		if (accel > _accelMap.alt.code) {
			accel -= _accelMap.alt.code;
			modifiers.push(_accelMap.alt.key);
		}
		if (accel > _accelMap.ctrl.code) {
			accel -= _accelMap.ctrl.code;
			modifiers.push(_accelMap.ctrl.key);
		}
		if (accel > _accelMap.shift.code) {
			accel -= _accelMap.shift.code;
			modifiers.push(_accelMap.shift.key);
		}
		sc = keyboardHandlers.buildShortCutDef(modifiers, accel);
	}
	return sc;
}
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
				...
				if (!item.inactive && !_kMnuHelper.isItemSep(item.typ)) {
					...
					if ((act = item.act)) {
						...
						if ((shortCut = _accelDecode(item.acl))) {
							shortCut.def.item = "page";
						}
						mnuItem = {
