	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
		...
		if(layout.$items.length > 0 && winFacet != syraUtil.pageFacet.modal3) {
			...
			if(layout.$items[i].$layout && layout.$items[i].$layout.$items) {
				...
				//layout.$items[i].$layout.$items.splice(0, 0, {"$bind": mainBtnId, "$style": _kMnuHelper.mainSty});
				if(this.coreMenus._isRefreshRegistered) {
					refreshId = requestActions.specialSrvActId.refresh;
					layout.$items[i].$layout.$items.splice(0, 0, {"$layout": {"$layoutType": "row", "$autoSize": true, "$items": [{"$bind": mainBtnId, "$style": _kMnuHelper.mainSty}, {"$bind": refreshId, "$icon": {"$mode": "icon", "$path": "x3iconsmenu/", "$value": "306"}}]}});
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winId, winMenus, winProto, winFacet, setMainGrp, showMagneto, session){
    	...
    	if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit || this._winFacet === syraUtil.pageFacet.modal2) {
    		// Tempory ack (F7)
    		_addMenu(this._menus, this._tabMenusFather, this._menuManager._getMenuItem(_menuIDs.ROOT_SUPERVISOR, this._idMnuCpt++, null, "10", 1, _insertRank.status + 1, null, "SRV_ADXEXECUTE", false, {"ico": "274", "icoVp": "x3icomnu"}));
    	}
    	if(winProto.$actions) {
  	...
  	_addWinProtoAction: function(actions){
   	...
   	buildMenus: function(){
   		var map = null, appMemus, coreMenus = this._menuManager.coreMenus;
   		// Map standards menus by group...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": _insertRank.help - 3}, {"id": _menuIDs.WINDOW_GIRL, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help}];
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				...
   				map.push({"id": _menuIDs.FILE_EX, "rnk": _insertRank.grpUtilities});
   			}
    	}
   		else if (this._winFacet === syraUtil.pageFacet.modal2){
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": _insertRank.help - 3}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help}];
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				// Deal with more popup...
   		...
   		if(this._hasActWin) {
			this._menus[_menuIDs.BTN_WIN].opts.sty = this._setMainGrp ? _kMnuHelper.mainSty : null;
			this._menus[_menuIDs.BTN_WIN].rnk = this._setMainGrp ? 1 : _insertRank.status - 1;
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
	    		if (!srcItems[srcItemsArray[i]].inactive && !_kMnuHelper.isItemSep(srcItems[srcItemsArray[i]].typ)) {
	    			...
	    			if((_kMnuTypes[srcItems[srcItemsArray[i]].typ] & _kMnuHelper.cat.popup) && ((srcItems[srcItemsArray[i]].items && (Object.keys(srcItems[srcItemsArray[i]].items)).length > 0) || _kMnuHelper.isPopupNoFacet(srcItems[srcItemsArray[i]].typ)))  {
	    				...
	    				_kMnuHelper.appendIco(srcItems[srcItemsArray[i]], mnuItem);
	    				if((opts = srcItems[srcItemsArray[i]].opts) && opts.hidden) {
