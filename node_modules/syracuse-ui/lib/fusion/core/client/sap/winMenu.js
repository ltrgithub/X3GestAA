...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	_addInternalCoreMenu: function() {
		...
		// Items of HELP_EX
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "29", 1, 11, requestActions.svcHelp, "AIDE_SUR_CHAMP", true, null, "$fieldHelp"));
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "28", 1, 12, requestActions.svcHelp, "AIDE_SUR_FONCTION", false, null, "$fonctionHelp"));
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto) {
		var i, j, k, l, len, linksArray, working, timer = utilBis.getTimer(true),
			bar, nbLayoutItems, mainGrpInProto, mainBtnId, refreshId, splitup, mnuItem, $mnus = [],
			$mnusEx;
		// Init
		...
		// Get win bar def
		if (!(bar = this._getWinMenuBar(winId, winFacet, winMenusDef, winProto, !mainGrpInProto))) {
			return;
		}
			...
			if (i < $mnus.length) {
				...
				if (this.coreMenus._isRefreshRegistered) {
					...
					$mnus[i].$layout.$items.splice(0, 0, {
						...
						"$items": [{
							"$bind": mainBtnId,
							"$style": _kMnuHelper.mainSty
						}, {
							"$bind": refreshId,
							"$layoutCss": "s-mn-bar-main-icon-slot",
							"$icon": {
								"$mode": "icon",
								"$path": "x3iconsmenu/",
								"$value": "306"
					...
					bar.$links[refreshId] = {
						"$title": this.coreMenus._refreshDesc.$title,
						"$act": refreshId
					};
				} else {
					$mnus[i].$layout.$items.splice(0, 0, {
	...
	_getWinMenuBar: function(winId, winFacet, winMenusDef, winProto, setMainGrp) {
		if(!this.winMenuColl) {
			if(this._session && this._session.logger) {
				this._session.logger.trace({"severety": "warn", "toJson": false}, "Session settings menus definition aren't been set!");
			}
			return null;
		}
