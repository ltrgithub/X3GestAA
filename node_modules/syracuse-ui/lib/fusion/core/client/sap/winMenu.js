...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
 		var i, j, len, linksArray, layout, timer = utilBis.getTimer(true), bar, nbLayoutItems, mainGrpInProto;
 		// Init
		...
		if(layout.$items.length > 0 && winFacet != syraUtil.pageFacet.modal3) {
			...
			if(layout.$items[i].$layout && layout.$items[i].$layout.$items) {
				layout.$items[i].$layout.$items.splice(0, 0, {"$bind": this._idMnuCpt, "$style": _kMnuHelper.mainSty});
				bar.$links[this._idMnuCpt++] = {"$title": this._session.getlabel("text", "14", "14"), "$act": requestActions.getSrvAction("SRV_FICHIER_NOUVEAU")};
			}
		}
		// Set links
		if(winProto.$links) {
  	...
  	_addWinProtoAction: function(actions){
   	...
   	buildMenus: function(){
   		...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
 			...
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 10}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": 15}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help}];
   		}
