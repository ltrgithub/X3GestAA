...
var _kVirtualPath = {"file": "file/", "x3ico": "x3icons/"}, _menuWithMagneto = true, _kWinOwnMagneto = false, _menuWithMainMore = false;
var _kMnuHelper = {
	...
	"appendIco": function (mnuItem, syraMnuItem) {
		if(_kMnuTypes[mnuItem.typ] & _kMnuHelper.facet.ico) {
			if(_kMnuTypes[mnuItem.typ] & this.cat.item) {
				syraMnuItem.$icon = {"$mode": _kMnuTypes[mnuItem.typ] & _kMnuHelper.facet.txt ? "iconText" : "icon", "$path": _kVirtualPath[mnuItem.opts.icoVp || "x3ico"], "$value": mnuItem.opts.ico};
			}
			else {
				syraMnuItem.$titleIcon = {"$path": _kVirtualPath[mnuItem.opts.icoVp || "x3ico"], "$value": mnuItem.opts.ico};					
			}
		}
	}
};
...
var _insertRank = {"status": 3, "protoMnus": 9000, "help": 100, "pjInStatus": 80, "cmntInStatus": 90};
var _menuIDs = {	
...
function _cureMenu (menuItem){
	...
	if(menuItem.id == 18000 && menuItem.rnk == 58 && menuItem.fid == _menuIDs["FILE_EX"] && menuItem.typ == 0) {
		menuItem["opts"] = {"layout": "row", "collapsed" : true, "popup": true};
		menuItem.rnk = 67;
	}
	...
	if(menuItem.fid == _menuIDs.DISPLAY_EX){
		...
		if (menuItem.act == requestActions.specialSrvActId.refresh) {
			...
			menuItem.rnk = 1;
			menuItem["opts"] = {"ico": "s-vignette-refresh", "icoVp": "x3ico"};
			menuItem.typ = 4;
		}
	...
	if(menuItem.fid == _menuIDs.ROOT_SUPERVISOR && menuItem.id >= 50500 && menuItem.rnk == 80 && menuItem.act == 1024) {
		// Change "Select" rank
		menuItem.rnk = _insertRank.help - 15;
	}
}
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    ...
    _addInternalCoreMenu: function() {
    	...
    	// Items of "SUBMENU_DIAGNOS_CMN"
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_DIAGNOS_CMN, this._idMnuCpt++, null, "11", 6, 11, null, "SRV_CALCULATRICE", false, {"ico": "42", "icoVp": "x3ico"}));
 		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_DIAGNOS_CMN, this._idMnuCpt++, null, "12", 6, 12, null, "SRV_DEBUGGER", false, {"ico": "282", "icoVp": "x3ico"}));
   		// Items of "FILE_EX"
    	...
    	_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.FILE_EX, this._idMnuCpt++, null, "26", 1, 22, null, "SRV_FICHIER_LIAISON"));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.FILE_EX, this._idMnuCpt++, null, "20", 1, 51, null, "SRV_FICHIER_WORKFLOW", false, {"ico": "237", "icoVp": "x3ico"}));		   		
  		// Items for "STATUS_CMN"
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "16", 4, _insertRank.pjInStatus, null, "SRV_FICHIER_PIECE_JOINT", false, {"ico": _kMnuHelper.stdIco.pj, "icoVp": "x3ico"}));		   		
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "17", 4, _insertRank.cmntInStatus, null, "SRV_FICHIER_COMMENTAIRE", false, {"ico": _kMnuHelper.stdIco.comment, "icoVp": "x3ico"}));		   		
		...
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 2, 50, null, null));
 		...
 		this.addCoreSubMenuToRoot(_menuIDs.FILE_EX, _menuIDs.SUBMENU_PRINT_CMN, 65);
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
 		var i, j, len, linksArray, layout, timer = utilBis.getTimer(true), bar, nbLayoutItems, mainGrpInProto, mainBtnId;
 		// Init
	...
	addStatusIco: function(winId, syraUISheet, icoId, act, showFlag){
		...
		if((winMenu = this.winMenuColl[winId]) && syraUISheet) {
			...
			ret = showFlag ? this.updateMnuItem(syraUISheet, act, false) : this.addIcoItemToMnu(syraUISheet, _menuIDs.STATUS_CMN, act, !isStatusMnu ? icoId : this._statusWithMenu[icoId].status.ico, "x3ico");
		}
		return ret;
	},
    ...
    dispose: function(){
   	...
   	_getCoreMenuIDs: function() {
		...
		ids[_menuIDs.SUBMENU_PRINT_CMN] = {"tag":"SUBMENU_PRINT_CMN", "txt": this._session.getlabel("text", 3, "Print"), "typ": 0, "opts": {"collapsed": true, "popup": true, "ico": "6", "icoVp": "x3ico", "layout": "row"}};
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winId, winMenus, winProto, winFacet, setMainGrp, showMagneto, session){
    	...
    	if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit || this._winFacet === syraUtil.pageFacet.modal2) {
    		// Tempory ack (F7)
    		_addMenu(this._menus, this._tabMenusFather, this._menuManager._getMenuItem(_menuIDs.ROOT_SUPERVISOR, this._idMnuCpt++, null, "10", 1, _insertRank.status - 1, null, "SRV_ADXEXECUTE", false));
    	}
    	if(winProto.$actions) {
   	...
   	buildMenus: function(){
   		...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			...
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				...
   				map.push({"id": _menuIDs.FILE_EX, "rnk": _insertRank.help - 10});
   			}
    	}
	    			...
	    			if((_kMnuTypes[srcItems[srcItemsArray[i]].typ] & _kMnuHelper.cat.popup) && ((srcItems[srcItemsArray[i]].items && (Object.keys(srcItems[srcItemsArray[i]].items)).length > 0) || _kMnuHelper.isPopupNoFacet(srcItems[srcItemsArray[i]].typ)))  {
						...
						if((opts = srcItems[srcItemsArray[i]].opts)) {
							...
							if(opts.popup) {
								...
								mnuItem.$skin = "s-test-menus-icon";
							}
						}
