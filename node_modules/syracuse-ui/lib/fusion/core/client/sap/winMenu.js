...
var _kMnuHelper = {
		...
		"isItemSep": function (mnuItem) {
			return (mnuItem & this.cat.item) && (mnuItem & this.facet.sep);
		},
...
var _menuFlat = false, _menuWithMagneto = true, _protoMnuInsertRank = 9000, _helpMnuInsertRank = _protoMnuInsertRank + 1;
var _menuIDs = {	
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    ...
    dispose: function(){
   	...
   	_getCoreMenuIDs: function() {
		...
		ids[_menuIDs.SUBMENU_PRINT_CMN] = {"tag":"SUBMENU_PRINT_CMN", "txt": this._session.getlabel("text", 3, "Print"), "typ": 5, "opts": {"collapsed": true, "ico": "6", "icoVp": "x3ico", "layout": "row"}};
  	...
  	_addWinProtoAction: function(actions, localization){
   	...
   	buildMenus: function(){
   		...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.FILE_EX, "rnk": 5}, {"id": _menuIDs.DISPLAY_EX, "rnk": 10}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": 15}, {"id": _menuIDs.WINDOW_GIRL, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": _helpMnuInsertRank}];
    	}
   		else if (this._winFacet !== syraUtil.pageFacet.lookup){
 			map = [{"id": _menuIDs.FILE_EX, "rnk": 5}, {"id": _menuIDs.DISPLAY_EX, "rnk": 10}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": 15}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": menuIDs.HELP_EX, "rnk": _helpMnuInsertRank}];
   		}
   	...
   	buildTreeMenusObj: function(){
		...
		function walk(srcItems, trgtItems) {
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
	    		if (!srcItems[srcItemsArray[i]].inactive && !_kMnuHelper.isItemSep(_kMnuTypes[srcItems[srcItemsArray[i]].typ])) {
	   				style = srcItems[srcItemsArray[i]].opts ? srcItems[srcItemsArray[i]].opts.sty : undefined;
	    			if((_kMnuTypes[srcItems[srcItemsArray[i]].typ] & _kMnuHelper.cat.popup) && srcItems[srcItemsArray[i]].items && (Object.keys(srcItems[srcItemsArray[i]].items)).length > 0) {
  	...
  	buildFlatMenusObj: function(){
   		...
   		function walk(items) {
	    	_.each(items, function(menu) {
	    		if (!menu.inactive && !_kMnuHelper.isItemSep(_kMnuTypes[typ])) {
	    			if(_kMnuTypes[menu.typ] & _kMnuHelper.cat.popup) {
	    				walk(menu.items);
	    			}
	    			else {
