...
var _kVirtualPath = {"file": "file/", "x3ico": "x3icons/", "x3icomnu": "x3iconsmenu/"}, _menuWithMagneto = true, _kWinOwnMagneto = false, _menuWithMainMore = false;
var _kMnuHelper = {
	"stdIco": {"pj": "310", "comment": "311"},
	"mainSty": "main", "moreSty": "more",
	"group": {"grpExtra": "extra", "grpLinks": "links", "grpCrud": "crud", "grpKpass": "compass", "grpTools": "toolbar", "grpAct": "actions"},
	"cat": {"popup": 0x010000, "item": 0x020000},
	"facet": {"sep": 0x000001, "txt": 0x000002, "ico": 0x000004},
	...
	"appendIco": function (mnuItem, syraMnuItem) {
		if(_kMnuTypes[mnuItem.typ] & _kMnuHelper.facet.ico) {
			syraMnuItem[_kMnuTypes[mnuItem.typ] & this.cat.item ? "$icon" : "$titleIcon"]= {"$mode": _kMnuTypes[mnuItem.typ] & _kMnuHelper.facet.txt ? "iconText" : "icon", "$path": _kVirtualPath[mnuItem.opts.icoVp || "x3icomnu"], "$value": mnuItem.opts.ico};
		}
	}
};
var _kMnuTypes = {
...
var _insertRank = {"status": 5, "protoMnus": 9000, "help": 100, "pjInStatus": 80, "cmntInStatus": 90};
var _menuIDs = {	
...
function _cureMenu (menusObj, menuItem){
	...
	if(menuItem.id == 18000 && menuItem.rnk == 58 && menuItem.fid == _menuIDs["FILE_EX"] && menuItem.typ == 0) {
		menuItem["opts"] = {"collapsed" : true, "popup": false};
		menuItem.rnk = 67;
	}
	if(menuItem.fid == 18000 && menuItem.typ == 1) {
		if (menuItem.act == 1174) {
			menuItem["opts"] = {"ico": "s-link-syracuse-word-mailmerge", "icoVp": "file"};
			menuItem.typ = 6;
		}
		else if (menuItem.act == 1185) {
			menuItem["opts"] = {"ico": "s-link-syracuse-excel-worksheet", "icoVp": "file"};
			menuItem.typ = 6;
		}
	}
	if(menuItem.fid == _menuIDs.DISPLAY_EX){
		...
		if (menuItem.act == requestActions.specialSrvActId.refresh) {
			...
			menuItem.rnk = 1;
			menuItem["opts"] = {"ico": "306", "icoVp": "x3icomnu"};
			menuItem.typ = 4;
			// Since refresh is with "new" main button... we just disabled it!
			menusObj._isRefreshRegistered = true;
			menusObj._refreshDesc = {"$title": menuItem.txt, "acl": menuItem.acl};
			menuItem.inactive= true;
		}
	...
	if(menuItem.fid == _menuIDs.ROOT_SUPERVISOR && menuItem.id >= 50500 && menuItem.rnk == 80 && menuItem.act == 1024) {
		...
		menuItem["opts"] = {"collapsed": true, "grp": _kMnuHelper.group.grpExtra};
	}
}
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    initialize: function(hoster, session, commonMenus){
    	var timer = utilBis.getTimer(true), i, len;
    	this._hoster = hoster;
    	...
    	_addMenus(this.coreMenus, this._tabMenusFather, commonMenus, {"logger": this._session.logger});
   		this._addInternalCoreMenu();
        //this._session.logger.timer(null, "Load core menus : total [" + timer.elapsedMs() + "]");
    },

	_statusWithMenu: {
		"260": {"mnu": {"act": "SRV_FICHIER_COMMENTAIRE", "ico": _kMnuHelper.stdIco.comment}, "status": {"ico": "260", "acts": [3355], "rnk": _insertRank.cmntInStatus, "txt": "17"}},
		"238": {"mnu": {"act": "SRV_FICHIER_PIECE_JOINT", "ico": _kMnuHelper.stdIco.pj}, "status": {"ico": "238", "acts": [3356], "rnk": _insertRank.pjInStatus, "txt": "16"}}
	},
    ...
    _addInternalCoreMenu: function() {
    	...
    	// Items of "SUBMENU_DIAGNOS_CMN"
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_DIAGNOS_CMN, this._idMnuCpt++, null, "11", 6, 11, null, "SRV_CALCULATRICE", false, {"ico": "42", "icoVp": "x3icomnu"}));
 		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_DIAGNOS_CMN, this._idMnuCpt++, null, "12", 6, 12, null, "SRV_DEBUGGER", false, {"ico": "282", "icoVp": "x3icomnu"}));
   		// Items of "FILE_EX"
    	...
    	_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.FILE_EX, this._idMnuCpt++, null, "26", 1, 22, null, "SRV_FICHIER_LIAISON"));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.FILE_EX, this._idMnuCpt++, null, "20", 1, 51, null, "SRV_FICHIER_WORKFLOW", false, {"ico": "237", "icoVp": "x3icomnu"}));		   		
  		// Items for "STATUS_CMN"
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "16", 4, _insertRank.pjInStatus, null, "SRV_FICHIER_PIECE_JOINT", false, {"ico": _kMnuHelper.stdIco.pj, "icoVp": "x3icomnu"}));		   		
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.STATUS_CMN, this._idMnuCpt++, null, "17", 4, _insertRank.cmntInStatus, null, "SRV_FICHIER_COMMENTAIRE", false, {"ico": _kMnuHelper.stdIco.comment, "icoVp": "x3icomnu"}));		   		
		_.each(this._statusWithMenu, function(item, key){
			_.each(item.status.acts, function(act, idx){
				// By default, add and hide alternative icon "status" menu bound with menu item
		  		_addMenu(self.coreMenus, self._tabMenusFather, self._getMenuItem(_menuIDs.STATUS_CMN, self._idMnuCpt++, null, this.status.txt, 4,  this.status.rnk + idx + 1, act, null, false, {"ico": this.status.ico, "icoVp": "x3icomnu", "hidden": true}));		   		
			}, item);
		});
  		// Items of HELP_EX
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 1, 11, null, "AIDE_SUR_CHAMP", true));
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 1, 12, null, "AIDE_SUR_FONCTION", true));
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 2, 50, null, null));
  		...
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_PRINT_CMN, this._idMnuCpt++, null, "18", 1, 10, null, "SRV_FICHIER_PRINT"));
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_PRINT_CMN, this._idMnuCpt++, null, "19", 1, 20, null, "SRV_FICHIER_PRINT_LISTE"));		   		
   		// Items of "NAVIGATION_OWN"
   		mnuId = !this._mnuWithMagneto || !_kWinOwnMagneto ? _menuIDs.NAVIGATION_OWN : _menuIDs.MAGNETO_OWN;
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(mnuId, this._idMnuCpt++, null, "22", 1, 11, null, "SRV_FICHIER_PREMIER", true, null, "$first"));
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(mnuId, this._idMnuCpt++, null, "23", 1, 12, null, "SRV_FICHIER_PRECEDENT", true, null, "$previous"));		   		
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(mnuId, this._idMnuCpt++, null, "24", 1, 13, null, "SRV_FICHIER_SUIVANT", true, null, "$next"));		   		
  		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(mnuId, this._idMnuCpt++, null, "25", 1, 15, null, "SRV_FICHIER_DERNIER", true, null, "$last"));		   		
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
 		var i, j, len, linksArray, layout, timer = utilBis.getTimer(true), bar, nbLayoutItems, mainGrpInProto, mainBtnId, refreshId;
 		// Init
		syraUtil.appendProto(winProto, {"$article": {"$menus": {"$layout": {"$items" : []}}}});
		layout = winProto.$article.$menus.$layout;
		nbLayoutItems = layout.$items.length;
		mainGrpInProto = !!(nbLayoutItems > 0 && layout.$items[0].$style && layout.$items[0].$style === _kMnuHelper.mainSty);
		if(mainGrpInProto) {
			layout.$items[0].rnk = 1;
			layout.$items[0]._cloudId = _kMnuHelper.group.grpCrud;
		}
 		// Get win bar def
 		bar = this._getWinMenuBar(winId, winFacet, winMenusDef, winProto, !mainGrpInProto);
		// Set menu layout, consider that items set in proto are members of "ROOT_SUPERVISOR", so the last group in the bar
		if(nbLayoutItems === 0) {
			layout.$items = bar.$menus.$items;
		}
		else if (bar.$menus.$items.length > 0){
			j = mainGrpInProto ? 1 : 0;
			for(i = j; i < nbLayoutItems; i++) {
				layout.$items[i]._cloudId = _kMnuHelper.group.grpLinks;
			}
			i = 0;
			while(bar.$menus.$items[i].rnk <= _insertRank.protoMnus) {
				layout.$items.splice(i + j, 0, bar.$menus.$items[i++]);
			}
			for(j = i, len = bar.$menus.$items.length; j < len; j++) {
				layout.$items.push(bar.$menus.$items[j]);
			}
		}
		// Add "main" Button... i.e "New" at moment
		if(layout.$items.length > 0 && winFacet != syraUtil.pageFacet.modal3) {
			i = 0;
			while(layout.$items[i].$style && layout.$items[i].$style !== _kMnuHelper.mainSty) {
				i++;
			}
			i = i < layout.$items.length ? i : 0;
			if(layout.$items[i].$layout && layout.$items[i].$layout.$items) {
				mainBtnId = requestActions.getSrvAction("SRV_FICHIER_NOUVEAU");
				if(this.coreMenus._isRefreshRegistered) {
					refreshId = requestActions.specialSrvActId.refresh;
					layout.$items[i].$layout.$items.splice(0, 0, {"$layoutType": "row", "$isFixedWidth": true, "$widths": "*,*", "$items": [{"$bind": mainBtnId, "$style": _kMnuHelper.mainSty}, {"$bind": refreshId, "$layoutCss": "s-page-menus-icon-slot", "$icon": {"$mode": "icon", "$path": "x3iconsmenu/", "$value": "306"}}]});
					bar.$links[refreshId] = {"$title": this.coreMenus._refreshDesc.$title, "$act": refreshId};
				}
				bar.$links[mainBtnId] = {"$title": this._session.getlabel("text", "14", "14"), "$act": mainBtnId};
			}
		}
		// Set links
		if(winProto.$links) {
	...
	_getWinMenuBar: function(winId, winFacet, winMenusDef, winProto, setMainGrp){
		if(this.winMenuColl[winId]) {
			this.releaseWinMenu(winId);
		}
		(this.winMenuColl[winId] = new WinMenu()).initialize(this, winId, winMenusDef, winProto, winFacet, setMainGrp, this._mnuWithMagneto, this._session);
	...
	addStatusIco: function(winId, syraUISheet, icoId, act, showFlag){
		var ret = true, winMenu, isStatusMnu = this._statusWithMenu[icoId] && this._statusWithMenu[icoId].status.acts.indexOf(act) >= 0;
		if((winMenu = this.winMenuColl[winId]) && syraUISheet) {
			...
			ret = showFlag ? this.updateMnuItem(syraUISheet, act, false) : this.addIcoItemToMnu(syraUISheet, _menuIDs.STATUS_CMN, act, !isStatusMnu ? icoId : this._statusWithMenu[icoId].status.ico, "x3icomnu");
		}
		return ret;
	},

	removeStatusIco: function(winId, syraUISheet, icoId, act){
		var ret = true, statusMnuAct, winMenu, isStatusMnu = this._statusWithMenu[icoId] && this._statusWithMenu[icoId].status.acts.indexOf(act) >= 0;
		if((winMenu = this.winMenuColl[winId]) && syraUISheet) {
			if(isStatusMnu && winMenu.menuBarTree.items[_menuIDs.STATUS_CMN]) {
	...
	addIcoItemToMnu: function(syraUISheet, mnuGrpId, act, icoId, icoPath, isHidden){
		var newItem = {"$links": {}}, ret = true;
		newItem.$links[act] = {"$act": act};
		_kMnuHelper.appendIco({"typ": "4", "opts": {"icoVp": icoPath, "ico": icoId}}, newItem.$links[act]);
		if(isHidden) {
			newItem.$links[act].$isHidden = isHidden;
		}
		syraUISheet.addMenuItem(newItem, mnuGrpId);
		return true;
	},

	updateMnuItem: function(syraUISheet, id, isHidden, isDisabled){
		var ret = true, updateItem = {"$links": {}};
		updateItem.$links[id] = {};
    ...
    dispose: function(){
   	...
   	_getCoreMenuIDs: function() {
		...
		ids[_menuIDs.FILE_EX] = {"tag":"FILE_EX", "txt": this._session.getlabel("text", 2, "File"), "typ": 0, "opts": {"collapsed": true}, "items": {}};
		ids[_menuIDs.SUBMENU_PRINT_CMN] = {"tag":"SUBMENU_PRINT_CMN", "txt": this._session.getlabel("text", 3, "Print"), "typ": "3", "opts": {"collapsed": true, "popup": true, "ico": "6", "icoVp": "x3icomnu", "layout": "row"}};
		ids[_menuIDs.HELP_EX] = {"tag":"HELP_EX", "txt": this._session.getlabel("text", 4, "Help"), "typ": 0, "opts": {"collapsed": true}};
		ids[_menuIDs.SUBMENU_DIAGNOS_CMN] = {"tag":"SUBMENU_DIAGNOS_CMN", "txt": this._session.getlabel("text", 5, "Diagnosis"), "typ": 0};
		ids[_menuIDs.DISPLAY_EX] = {"tag":"DISPLAY_EX", "txt": this._session.getlabel("text", 6, "Display"), "typ": 0, "opts": {"collapsed": true}};
		ids[_menuIDs.STATUS_CMN] = {"tag":"STATUS_CMN", "txt": "", "typ": 7, "opts": {"layout": "row"}};
		...
		ids[_menuIDs.SUBMENU_MORE_GIRL] = {"tag":"SUBMENU_MORE_GIRL", "txt": this._session.getlabel("text", 27, "More"), "typ": 7, "opts":{"sty": _kMnuHelper.moreSty}};
		...
		ids[_menuIDs.SUBMENU_MORE_OWN] = {"tag":"SUBMENU_MORE_OWN", "txt": this._session.getlabel("text", 27, "More"), "typ": 7, "opts":{"sty": _kMnuHelper.moreSty}};
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winId, winMenus, winProto, winFacet, setMainGrp, showMagneto, session){
    	...
    	this._menuBarObject = null;
 		...
 		if(winMenus && winMenus.length > 0) {
    		_addMenus(this._menus, this._tabMenusFather, winMenus, {"logger": this._session.logger, "grp": _kMnuHelper.group.grpLinks});
    	}
    	if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit || this._winFacet === syraUtil.pageFacet.modal2) {
    		// Tempory ack (F7)
    		_addMenu(this._menus, this._tabMenusFather, this._menuManager._getMenuItem(_menuIDs.ROOT_SUPERVISOR, this._idMnuCpt++, null, "10", 1, _insertRank.status + 1, null, "SRV_ADXEXECUTE", false, {"ico": "274", "icoVp": "x3icomnu", "grp": _kMnuHelper.group.grpKpass}));
    	}
    	if(winProto.$actions) {
  	...
  	_addWinProtoAction: function(actions){
        ...
        for (i = 0, len = actsdArray.length; i < len; i++) {
        	...
        	_addMenu(this._menus, null, this._menuManager._getMenuItem(_menuIDs.BTN_WIN, this._idMnuCpt++, null, actItm.$title, 1, i + 50, actItm.$act));
        }
        this._hasActWin = len > 0 ? true : false;
  	},
  
   	buildMenus: function(){
   		var map = null, appMemus, coreMenus = this._menuManager.coreMenus, extraGrp = _kMnuHelper.group.grpExtra;
   		// Map standards menus by group...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status, "grp": _kMnuHelper.group.grpTools}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5, "grp": extraGrp}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": _insertRank.help - 3, "grp": extraGrp}, {"id": _menuIDs.WINDOW_GIRL, "rnk": _insertRank.help - 2, "grp": extraGrp}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help, "grp": extraGrp}];
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				...
   				map.push({"id": _menuIDs.FILE_EX, "rnk": _insertRank.grpUtilities, "grp": extraGrp});
   			}
    	}
   		else if (this._winFacet === syraUtil.pageFacet.modal2){
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status, "grp": _kMnuHelper.group.grpTools}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5, "grp": extraGrp}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": _insertRank.help - 3, "grp": extraGrp}, {"id": _menuIDs.WINDOW_OWN, "rnk": _insertRank.help - 2, "grp": extraGrp}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help, "grp": extraGrp}];
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				// Deal with more popup...
   		...
   		if(this._hasActWin) {
   			if(this._setMainGrp) {
   				this._menus[_menuIDs.BTN_WIN].opts.sty = _kMnuHelper.mainSty;
   				this._menus[_menuIDs.BTN_WIN].rnk = 1;
				this._menus[_menuIDs.BTN_WIN].opts.grp = _kMnuHelper.group.grpCrud;
   			}
   			else {
   				this._menus[_menuIDs.BTN_WIN].opts.sty = null;
   				this._menus[_menuIDs.BTN_WIN].rnk = _insertRank.status - 1;
				this._menus[_menuIDs.BTN_WIN].opts.grp = _kMnuHelper.group.grpAct;
   			}
   	...
   	buildTreeMenusObj: function(){
   		var $menus = {"$items": []}, $links = {}, magnetoMnuId, i, len, magnetoArray, magetoItem, coreMenus = this._menuManager.coreMenus;
		function walk(srcItems, trgtItems, ctx) {
    		var len, srcItemsArray, i, act, style, mnuItem, layout, opts;
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
	    		if (!srcItems[srcItemsArray[i]].inactive && !_kMnuHelper.isItemSep(srcItems[srcItemsArray[i]].typ)) {
	    			...
	    			if((_kMnuTypes[srcItems[srcItemsArray[i]].typ] & _kMnuHelper.cat.popup) && ((srcItems[srcItemsArray[i]].items && (Object.keys(srcItems[srcItemsArray[i]].items)).length > 0) || _kMnuHelper.isPopupNoFacet(srcItems[srcItemsArray[i]].typ)))  {
    					mnuItem = {"$title": srcItems[srcItemsArray[i]].txt, "$style": style, "$layout": {"$items": []}, "rnk": srcItems[srcItemsArray[i]].rnk};
						mnuItem.$opened = ctx.level > 0 ? false : mnuItem.$opened;
						mnuItem.$id = srcItemsArray[i];
	    				...
	    				mnuItem = {"$bind": act, "$style": style, "rnk": srcItems[srcItemsArray[i]].rnk};
	    				_kMnuHelper.appendIco(srcItems[srcItemsArray[i]], mnuItem);
	    				if((opts = srcItems[srcItemsArray[i]].opts)) {
	    					if(opts.hidden) {
   		...
   		walk(this.menuBarTree.items, $menus.$items, {"level": 0});
   	...
   	dispose: function(){
	 	if(this.winProto) {
 	 	...
 	 	_.each(this._tabMenusFather, function(ptr, key){delete this[key];}, this._tabMenusFather);
	 	_.each(this.menuBarTree.items, function(ptr, key){delete this[key];}, this.menuBarTree.items);
  	...
  	_getSpecialMenuIDs: function() {
		...
		ids[_menuIDs.BTN_WIN] = {"tag": "BTN_WIN", "typ": 0, "rnk": 1, "opts": {}};
		return ids;
	}
});
