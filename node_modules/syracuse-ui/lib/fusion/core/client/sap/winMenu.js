...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto, flat){
 		var timer = utilBis.getTimer(true), bar = this.getWinMenuBar(winId, winFacet, winMenusDef, winProto, flat);
		if(!flat) {
			winProto.$links = bar.$links;
			if(!winProto.$article) {
				winProto.$article = {};
			}
			winProto.$article.$menus = bar.$menus;
		}
		else {
	...
	getWinMenuBar: function(winId, winFacet, winMenusDef, winProto, flat){
		if(this.winMenuColl[winId]) {
			this.releaseWinMenu(winId);
		}
		(this.winMenuColl[winId] = new WinMenu()).initialize(winMenusDef, winProto, this._session);
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(winMenus, winProto, session){
  		this._session = session;
    	this._tabMenusFather = {};
    	...
    	if(winProto.$links) {
    		// TODO!!
    		//this._addWinLink(winProto.$links, winProto.$localization);
   	...
   	getMenuBar: function(flat) {
   		if(!this._menuBarObject.bar) {
			...
			this._menuBarObject.bar = flat ? this.buildFlatMenusObj() : this.buildTreeMenusObj();
		}
		return this._menuBarObject.bar;
   	}, 

   	buildTreeMenusObj: function(){
   		var $menus = {"$layout": {"$items": []}}, $links = {}, $mnuItmsPtr = $menus.$layout.$items;
		function walk(srcItems, trgtItems) {
    		var len, srcItemsArray, i, act;
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
	    		if (!srcItems[srcItemsArray[i]].inactive && srcItems[srcItemsArray[i]].typ != 2) {
	    			if(srcItems[srcItemsArray[i]].typ === 0) {
	    				trgtItems.push({"$title": srcItems[srcItemsArray[i]].txt, "$layout": {"$items": []}});
	    				...
	    				trgtItems.push({"$bind": act, "rnk": srcItems[srcItemsArray[i]].rnk});
	    			}
	    		}
			}
   		}
   		walk(this._menuBarTree.items, $mnuItmsPtr);
   	...
   	dispose: function(){
	 	...
	 	if(this._menuBarObject.bar) {
		 	if (this._menuBarObject.flat){
		 		...
		 		delete this._menuBarObject.bar.$menus;
		 		delete this._menuBarObject.bar.$links;
		 	}
	 	}
    	if(this._session) {
    		delete this._session;
  	...
  	_getCustomMenuIDs: function() {
		...
		ids[_menuIDs.BTN_WIN] = {"tag":"BTN_WIN", "typ": 0, "rnk": 1};
		return ids;
	}
});