...
var _kMnuHelper = {
	...
	"group": {"grpExtra": {"cld": "grpExtra", "skin": "s-mn-bar-extra"}, "grpLinks": {"cld": "grpLinks", "skin": "s-mn-bar-business"}, "grpCrud": {"cld": "grpCrud", "skin": "s-mn-bar-main"}, "grpKpass": {"cld": "grpKpass", "skin": ""}, "grpTools": {"cld": "grpTools", "skin": "s-mn-bar-toolbar"}, "grpAct": {"cld": "grpAct", "skin": "s-mn-bar-preferred"}},
...
function _cureMenu (menusObj, menuItem){
	...
	if(menuItem.fid == _menuIDs.ROOT_SUPERVISOR && menuItem.id >= 50500 && menuItem.rnk == 80 && menuItem.act == 1024) {
		...
		menuItem["opts"] = {"collapsed": true, "grp": _kMnuHelper.group.grpExtra.cld};
	}
}
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto){
 		var i, j, k, l, len, linksArray, working, timer = utilBis.getTimer(true), bar, nbLayoutItems, mainGrpInProto, mainBtnId, refreshId, splitup, mnuItem, $mnus = [], $mnusEx;
 		// Init
 		if((working = winProto.$menus || winProto.$article.$menus)) {
 			// Comaptibility mode...
		...
		nbLayoutItems = $mnus.length;
 		// Check "main" group
		mainGrpInProto = !!(nbLayoutItems > 0 && $mnus[0].$style && $mnus[0].$style === _kMnuHelper.mainSty);
		if(mainGrpInProto) {
			$mnus[0].rnk = 1;
			$mnus[0]._cloudId = _kMnuHelper.group.grpCrud.cld;
		}
 		// Get win bar def
		...
		else if ((len = bar.$menus.$items.length) > 0){
		...
		if($mnus.length > 0 && winFacet != syraUtil.pageFacet.modal3) {
			i = 0;
			while($mnus[i].$style && $mnus[i].$style !== _kMnuHelper.mainSty) {
				i++;
			}
			if(i < $mnus.length) {
				...
				if(this.coreMenus._isRefreshRegistered) {
					...
					$mnus[i].$layout.$items.splice(0, 0, {"$layoutType": "row", "$isFixedWidth": true, "$widths": "*,*", "$items": [{"$bind": mainBtnId, "$style": _kMnuHelper.mainSty}, {"$bind": refreshId, "$layoutCss": "s-mn-bar-main-icon-slot", "$icon": {"$mode": "icon", "$path": "x3iconsmenu/", "$value": "306"}}]});
					bar.$links[refreshId] = {"$title": this.coreMenus._refreshDesc.$title, "$act": refreshId};
				}
				else {
					$mnus[i].$layout.$items.splice(0, 0, {"$bind": mainBtnId, "$style": _kMnuHelper.mainSty});
				}
		...
		for(i = 0, len = $mnus.length; i < len; i++) {
			...
			if(mnuItem._cloudId !== splitup) {
				...
				$mnusEx[++j] = {"$skin": _kMnuHelper.group[splitup].skin, "$layout": {"$items": []}};
			}
			if(!mnuItem.$title) {
				if(mnuItem.$layout) {
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(menuManager, winId, winMenus, winProto, winFacet, setMainGrp, showMagneto, session){
    	...
    	this._menuBarObject = null;
 		...
 		if(winMenus && winMenus.length > 0) {
    		_addMenus(this._menus, this._tabMenusFather, winMenus, {"logger": this._session.logger, "grp": _kMnuHelper.group.grpLinks.cld});
    	}
    	if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit || this._winFacet === syraUtil.pageFacet.modal2) {
    		// Tempory ack (F7)
    		_addMenu(this._menus, this._tabMenusFather, this._menuManager._getMenuItem(_menuIDs.ROOT_SUPERVISOR, this._idMnuCpt++, null, "10", 6, _insertRank.grpUtilities - 2, null, "SRV_ADXEXECUTE", false, {"ico": "226", "icoVp": "x3icomnu", "grp": _kMnuHelper.group.grpExtra.cld}));
    	}
    	if(winProto.$actions) {
  	...
  	_addWinProtoAction: function(actions){
   	...
   	buildMenus: function(){
   		var map = null, appMemus, coreMenus = this._menuManager.coreMenus, extraGrp = _kMnuHelper.group.grpExtra.cld;
   		// Map standards menus by group...
   		if(!this._winFacet || this._winFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status, "grp": _kMnuHelper.group.grpTools.cld}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5, "grp": extraGrp}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": _insertRank.help - 3, "grp": extraGrp}, {"id": _menuIDs.WINDOW_GIRL, "rnk": _insertRank.help - 2, "grp": extraGrp}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help, "grp": extraGrp}];
 			...
 			map = [{"id": _menuIDs.STATUS_CMN, "rnk": _insertRank.status, "grp": _kMnuHelper.group.grpTools.cld}, {"id": _menuIDs.DISPLAY_EX, "rnk": _insertRank.help - 5, "grp": extraGrp}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": _insertRank.help - 3, "grp": extraGrp}, {"id": _menuIDs.WINDOW_OWN, "rnk": _insertRank.help - 2, "grp": extraGrp}, {"id": _menuIDs.HELP_EX, "rnk": _insertRank.help, "grp": extraGrp}];
   			if(_menuWithMainMore && this._hasActWin && this._setMainGrp) {
   				// Deal with more popup...
