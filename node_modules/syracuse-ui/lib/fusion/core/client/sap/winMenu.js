...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto) {
		...
		/*if(closeItemMngt.rankInColl >= 0) {
			if(winFacet && winFacet == syraUtil.pageFacet.modal3 ) {
				closeItemMngt.coll.splice(closeItemMngt.rankInColl, 1);
			}
			else {
				mainSlotItem.button = {
					"$bind": closeItemMngt.coll.rankInColl.$bind,
					"$act": winProto.$links[closeItemMngt.coll.rankInColl.$bind].$act,
					"$title": winProto.$links[closeItemMngt.coll.rankInColl.$bind].$title,
					"ico": {"$path": "x3iconsmenu", "$value": "306"}
				};
			}
		}*/
		// Add "main" link and button... i.e "New"/"refresh" at moment
