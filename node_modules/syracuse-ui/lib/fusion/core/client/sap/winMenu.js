...
function _cureMenu (menuItem){
	...
	// SubMenu Export ans these items... TODO : should be better to set this properties Superviseur Side!
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	addStatusIco: function(winId, syraUISheet, icoId, act){
		var ret = false, isMnuStatus = _kMnuWithStatus.act[act];
		if(this.winMenuColl[winId] && syraUISheet) {
			//syraUISheet.addMenuItem({"$links": {"testMenu": {"$bind": act, "$ico": {"$mode": "icon", "$path": "x3ico", "$value": icoId}}}}, _menuIDs.STATUS_CMN);
		}
		return ret;
	},

	removeStatusIco: function(winId, syraUISheet, icoId){
		var ret = false;
		if(this.winMenuColl[winId]) {
		}
	},
