...
function _addMenu(menusObj, tabMenusFather, menu, options) {
	var father = tabMenusFather[menu.fid] ? tabMenusFather[menu.fid] : menusObj[menu.fid];
	if(!father) {
		if(options && options.logger) {
			options.logger.trace(null, "'winMenu._addMenu' : attemp to add an item menu to an unknown menu id (" + menu.fid +  ")");
		}
		return;
	}
	if(!father.items) {
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
    initialize: function(session, commonMenus){
        ...
        this._session.logger.timer(null, "Load Menus : total [" + timer.elapsedMs() + "]");
     },
    ...
    _addInternalCoreMenu: function() {
		...
		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.HELP_EX, this._idMnuCpt++, null, "0", 2, 50, null, null));
  		...
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "22", 1, 11, null, "SRV_FICHIER_PREMIER"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "23", 1, 12, null, "SRV_FICHIER_PRECEDENT"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "24", 1, 13, null, "SRV_FICHIER_SUIVANT"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "25", 1, 15, null, "SRV_FICHIER_DERNIER"));		   		
 		...
 		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_OWN, this._idMnuCpt++, null, "26", 1, 55, null, "SRV_FICHIER_LIAISON"));		   		
  		...
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "22", 1, 11, null, "SRV_FICHIER_PREMIER"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "23", 1, 12, null, "SRV_FICHIER_PRECEDENT"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "24", 1, 13, null, "SRV_FICHIER_SUIVANT"));		   		
  		_addMenu(this._coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.NAVIGATION_GIRL, this._idMnuCpt++, null, "25", 1, 15, null, "SRV_FICHIER_DERNIER"));		   		
	...
	_getMenuItem: function (fid, id, txt, txtNum, typ, rnk, act, actTxt, inactive){
    	return {"fid": fid, "id": id.toString(), "txt": txt || this._session.getlabel("text", txtNum, txtNum), "typ": typ, "rnk": rnk, "act": act || requestActions.getSrvAction(actTxt), "inactive": inactive};
    },

	getWinMenuBar: function(winId, winFacet, winMenusDef, flat){
		(this.winMenuColl[winId] = new WinMenu()).initialize(winMenusDef, this._session);
		this.winMenuColl[winId].buildMenus(winFacet, this._coreMenus);
       	this._session.logger.trace(null, "Menu Bar JSON : ", this.winMenuColl[winId]._menuBarTree);
		return this.winMenuColl[winId].getMenuBar(flat);
	},
	
	releaseWinMenu: function(winId){
		this.winMenuColl[winId].dispose();
...
WinMenu = helpers.defineClass(WinMenu, null, {
  	initialize: function(winMenus, session){
  		this._session = session;
    	this._tabMenusFather = {};
    	this._menuBarTree = {"items": {}};
    	this._menuBarObject = {"flat": false, "bar": null};
 		this._menus = this._getCustomMenuIDs();
    	_addMenus(this._menus, this._tabMenusFather, winMenus, {"logger": this._session.logger});
   	},

   	buildMenus: function(windowFacet, coreMenus){
   		var map, appMemus;
   		// Map standards menus by group...
   		if(!windowFacet || windowFacet === syraUtil.pageFacet.edit) {
   			map = [{"id": _menuIDs.FILE_EX, "rnk": 1}, {"id": _menuIDs.DISPLAY_EX, "rnk": 2}, {"id": _menuIDs.NAVIGATION_GIRL, "rnk": 4}, {"id": _menuIDs.WINDOW_GIRL, "rnk": 97}, {"id": _menuIDs.HELP_EX, "rnk": 98}];
    	}
   		else {
 			map = [{"id": _menuIDs.FILE_EX, "rnk": 1}, {"id": _menuIDs.DISPLAY_EX, "rnk": 2}, {"id": _menuIDs.NAVIGATION_OWN, "rnk": 4}, {"id": _menuIDs.WINDOW_OWN, "rnk": 97}, {"id": menuIDs.HELP_EX, "rnk": 98}];
   		}
   	...
   	getMenuBar: function(flat) {
   		if(!this._menuBarObject.bar) {
   			this._menuBarObject.flat = flat;
			this._menuBarObject.bar = flat ? this.buildFlatMenusObj() : this.buildSyraMenusObj();
		}
		return this._menuBarObject.bar;
   	}, 

   	buildSyraMenusObj: function(){
   		var menus = null;
   		// TODO : don't forget to sort by rank!
   		return menus;
   	},
  	...
  	buildFlatMenusObj: function(){
   	...
   	dispose: function(){
	 	...
	 	if(this._menuBarObject.bar) {
		 	if (this._menuBarObject.flat){
			 	_.each(this._menuBarObject.bar, function(ptr, idx){delete this[idx]}, this._menuBarObject.bar);
		 	}
		 	else {
		 		// TODO
		 	}
