...
exports.PluginManager = helpers.defineClass(function PluginManager(hoster) {
	...
	dispatchCommand: function(pluginParams, hosterAck) {
		...
		try {
			...
			if (hosterAck) {
				hosterAck(result.action, result.data);
			}
		}
	},
	_getRank: function() {
	...
	_getPluginClassError: function(constructor, command, params, error) {
		...
		result = dummy.onUnexpectedErr(command, params, params.id, error);
...
var Plugin = helpers.defineClass(function Plugin() {}, null, {
	checkError: function(error) {
		if (error.errt != null && error.errt == _KerrDefCode.errt.internal && error.errn != null && this._pluginManager.hoster.getMessLabel) {
			...
			// TODO : change 54 to the right error offset
		}
	},
	_serializeResult: function(data, err, method) {
		var reply = {
			"method": method
		};
		if (!err) {
	...
	serializeReply: function(action, method, data, error) {
		return {
			"action": srvAct.getProxyAction(action),
			"data": this._serializeResult(data, error, method)
		};
	},
...
var _AsyncPlugin = helpers.defineClass(function() {}, Plugin, {
	launch: function(args) {
		var err = null;
		var winRef = window.open("SafeV2ILOGWrapper:" + (args ? helpers.string.htmlEscape(args) : ""));
...
var _IlogPlugin = helpers.defineClass(function() {}, _AsyncPlugin, {
	...
	create: function(params) {
		...
		args.push(["s", syra_site.getSessionId()]);
		args.push(["a", appType]);
		...
		error = this.launch(args.join(argSep));
		return this._serializeReply(params.methodID, params.method, error ? 0 : this._id, error);
	},
	destroy: function(params, force) {
		// Nothing special to do at now...
		return this._serializeReply(params.methodID, params.method, this._id);
	},
	onUnexpectedErr: function(method, params, id, err) {
		return this._serializeReply(params && params.MethodID, method, method != "new" ? id : 0, err);
	},
	_serializeReply: function(methodId, method, instanceId, error) {
		var data = {}, errCode, errMess;
		if (!error) {
			data.name = (methodId - 48).toString();
		...
		return this.serializeReply("PLUGIN_ILOG_ACK", method, {
			"property": data
		}, error);
...
var _ComPlugin = helpers.defineClass(function() {}, Plugin, {
	...
	destroy: function(params, force) {},
	onUnexpectedErr: function(method, params, id, err) {},
	_serializeReply: function(instanceId, error) {}
});
