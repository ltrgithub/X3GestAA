...
var _AsyncPlugin = helpers.defineClass(function() {}, Plugin, {
	launch: function(args, isDebugOn) {
		...
		if (winRef && winRef.close) {
			...
			mess = this._pluginManager.hoster.getMessLabel("76", "error") + "\n" + this._pluginManager.hoster.getMessLabel("77", "error");
			syraUtil.showDiagnoses([util.makeDiagnosis(mess, "warning")]);
		}
		return err;
	}
});
...
var _CrystalReportPlugin = helpers.defineClass(function() {}, _AsyncPlugin, {
	create: function(params) {
		...
		switch (this._subClass) {
			...
			default:
				...
				args.push(["t", params.ReportID]);
				args.push(["p", params.PrintServer]);
				rptCan = [params.Report || " ", params.Lang || " ", params.Solution || " ", params.Folder || " "];
				args.push(["k", rptCan.join(";")]);
	...
	onUnexpectedErr: function(method, params, id, err) {
		return this._serializeReply("PLUGIN_CRDEV_ACK", method, id, err);
	},
	_serializeReply: function(actionId, method, instanceId, error) {
		var data = {}, errCode, options = null;
		switch (this._subClass) {
			case "developement":
			default:
				data.name = "ReportDev";
				...
				options = {
					"autoRelease": true,
					"autoReleaseParams": util.duplicateObj(this._data)
				};
				options.autoReleaseParams.id = this._id;
				break;
		}
		if (error) {
			this.checkError(error);
