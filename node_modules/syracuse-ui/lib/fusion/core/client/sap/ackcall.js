...
var _stdCallDef = {
    "LINKVUE": {"spgm": "SUBAPSMON:LINKVUE", "params": [{"dim": 1, "typ": "STRU", "siz": 10}, {"dim": 1, "typ": "STRU", "siz": 3}, {"dim": 1, "typ": "STRU", "siz": 100}]},
    "AINTERFL": {"spgm": "AINTERFL:AINTERFL", "params": [{"dim": 1, "typ": "CLOB", "siz": 7}, {"dim": 20, "typ": "STRU", "siz": 100}, {"dim": 20, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "INT", "siz": 0}]},
    "MAJHIER": {"spgm": "AMAJHIER:MAJHIER", "params": [{"dim": 1, "typ": "STRU", "siz": 1}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "STRU", "siz": 100}, {"dim": 1, "typ": "INT", "siz": 0}]},
    "GET_INFO": {"spgm": "CNSX1MCSPV:GET_INFO", "params": [{"dim": 1, "typ": "STRU", "siz": 1}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "STRU", "siz": 20}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "CLOB", "siz": 6}]},
    "MOVE_ORDER": {"spgm": "CNSX1MCSPV:MOVE_ORDER", "params": [{"dim": 1, "typ": "INT", "siz": 0}, {"dim": 1, "typ": "STRU", "siz": 10}, {"dim": 1, "typ": "STRU", "siz": 10}, {"dim": 1, "typ": "CLOB", "siz": 6}]}
};
var _testCallDef = {
    "TEST_ADD": {"spgm": "QLFWSADD:ADDDECIMAL", "params": [{"dim": 1, "typ": "DCB", "siz": 0}, {"dim": 1, "typ": "DCB", "siz": 0}, {"dim": 1, "typ": "DCB", "siz": 0}]},
    "TEST_CMI": {"spgm": "ZZCMIACKCALL:TEST3", "params": [{"dim": 8, "typ": "STRU", "siz": 250}, {"dim": 8, "typ": "STRU", "siz": 0}, {"dim": 1, "typ": "STRU", "siz": 250}, {"dim": 6, "typ": "INT", "siz": 0}]},
    "TEST_FUNC_CMI": {"spgm": "ZZCMIACKCALL:TEST_FUNC_CMI", "ret": {"typ": "STRU"}, "params": [{"dim": 8, "typ": "STRU", "siz": 250}, {"dim": 8, "typ": "STRU", "siz": 0}, {"dim": 1, "typ": "STRU", "siz": 250}, {"dim": 6, "typ": "INT", "siz": 0}]}
};
var _KcallFmt = "xml";
...
exports.AckCallStack = helpers.defineClass(AckCallStack, null, {
    ...
    buildRequest: function(request, target, callParams, isFunc) {
        var ackCallRqst = null, ret = false;
        try {
            ackCallRqst = this.stackRqst(callParams, isFunc);
            request.result = {"$act": requestActions.getSrvAction(isFunc ? "SRV_FUNC" : "SRV_CALL"), "params": {"target": target, "call": {}}};
            request.result.params.call[_KcallFmt] = ackCallRqst.serializeRqst();
            request.result.post = {"onSvcPendingCallback": function () {ackCallRqst.remove.call(ackCallRqst);}};
            ret = true;
        }
        catch (ex){
            try {
    ...
    removeRqst: function(idx) {
        var len = this._stack.length, rqst = len > idx ? this._stack.splice(idx, 1) : null;
        if(rqst && rqst[0]) {
            setTimeout(function(){rqst[0].dispose();}, 5);
        }
    },

    dispatchReply: function(reply) {
        // Last call stacked must be dispatched first (lifo)!
        var len = this._stack.length, rqst = len > 0 ? this._stack.splice(this._stack.length - 1, 1) : null, result = null, ret = null;
        if(rqst && rqst[0]) {
            if (rqst[0].replyCallBack.callback) {
                ...
                this.log("trace", {"severety": "info", "toJson": true}, "Dispatch NDRESULT : ", result);
                rqst[0].replyCallBack.callback.apply(rqst[0], rqst[0].replyCallBack.args ? ([result].concat(rqst[0].replyCallBack.args)) : [result]);
            }
            rqst[0].dispose();
        }
        else {
            ret = {"ok": false, "diagnoses": util.makeDiagnosis(this.getErrMessLabel("33"), 3, "dispatchReply()@AckCallStack", "33")};
            this.log("trace", {"severety": "info", "toJson": true}, "Dispatch NDRESULT failed : ", ret);
        }
        return ret;
    },
    ...
    diagnosisReply: function(message, stack, appCode, detail, diagnosis) {
        ...
        return {"failed": true, "$diagnoses": [diagnosis || util.makeDiagnosis(messEx, 3, stack, appCode, null, detail)]};
    },
...
AckCallRequest = helpers.defineClass(AckCallRequest, null, {
    initialize: function(request, isFunc, hoster, id, dbgFlags){
        ...
        this._reply = null;
        this._func = isFunc;
        this._funcRet = this._desc.ret ? this._desc.ret.typ || "CLOB" : "CLOB";
        this.spgm = (this._desc && this._desc.spgm) || "";
    	this.replyCallBack = {"callback": request.callback, "args": request.callbackArgs};
    ...
    serializeRqst: function() {
        ...
        this._hoster.log("trace", {"severety": "info", "toJson": true}, "Ack_call serialized '", ret);
        return ret;
    ...
    _serializeRqst2XML: function() {
    	var i, j, lenArgs, lenArgsDesc = this._argsDesc.length, xheader = '<spgm id="' + this.spgm + '">', xfooter = '</spgm>', xparams = [], xparam, nb;
    ...
    _serializeRqst2JSON: function() {
        var i, j, lenArgs, lenArgsDesc = this._argsDesc.length, rqstJ = {"spgm": this.spgm, "params": []}, nb;
        if(this._args && lenArgsDesc > 0) {
            for (i = 0; i < lenArgsDesc; i++) {
                nb = 0;
                if(this._args[i] !== null && this._args[i] !== undefined) {
                ...
                rqstJ.params[i] = {"dim": this._argsDesc[i].dim, "nb": nb, "size": this._argsDesc[i].siz, "typ": this._argsDesc[i].typ, "resu": []};
                if(nb > 0) {
                    if(this._argsDesc[i].dim == 1) {
    ...
    _replyJSON2JSON: function(replyJSON) {
        ...
        if(!replyJSON.result || replyJSON.result.errn === undefined || replyJSON.result.errn !== 0) {
            ...
            if(this._func) {
                this._reply.return = replyJSON.return ? replyJSON.return.resu : undefined;
            }
    ...
    _replyXML2JSON: function(replyXML) {
        ...
        if((replyDOM = jsXML.parse(replyXML))) {
            ...
            if(!replyDOM["spgm"] || !replyDOM["spgm"].$ || replyDOM["spgm"].$["errn"] === undefined || replyDOM["spgm"].$["errn"] !== "0") {
                ...
                if(this._func) {
                    this._reply.return = replyDOM["spgm"]["resu"] ? this._getArgValFromStrg(this._func.typ, _.isObject(replyDOM["spgm"]["resu"]) ? replyDOM["spgm"]["resu"].$cdata : replyDOM["spgm"]["resu"]) : undefined;
                }
            }
        }
        else {
    ...
    _getSpgmDiagnosis: function(spgmErr) {
        return util.makeDiagnosis(this.spgm + " : " + this._hoster.getErrMessLabel("34"), 3, (spgmErr["errm"] || "...") + "\nat : " + (spgmErr["errp"] || "unknown") + "\nline : " + (spgmErr["errl"] || "-1"), spgmErr["errn"] || "-1", util.backOfficeNice);
    },
