...
exports.AckCallStack = helpers.defineClass(AckCallStack, null, {
	initialize: function(hoster, jsonCallFmt) {
		...
		this._hoster = hoster;
		this.srvExAct = {
			"default": ["SUP_FIELD_SRVFUNC", "SUP_FIELD_SRVCALL"]
		};
		this.srvExAct[syraUtil.dataTypes.collectionType] = ["SUP_CELL_FOCUS_SRVFUNC", "SUP_CELL_FOCUS_SRVCALL"];
	...
	buildRequest: function(request, target, callParams, isFunc) {
		var win, entityDef, ackCallRqst = null,
			ret = false,
			hasFocus, act, srvActM, callP = {}, params = {
				"target": target
			};
		try {
			...
			if (hasFocus) {
				...
				if ((win = this._hoster.getWindow(target)) && (entityDef = win.getEntityEx(target))) {
					act = this.srvExAct[entityDef.type] || this.srvExAct.
					default;
				} else {
					act = this.srvExAct[syraUtil.dataTypes.collectionType];
				}
				act = isFunc ? act[0] : act[1];;
				srvActM = "getSrvActionEX";
				params.sudo = [null, [callP]];
			}
			request.result = {
				"$act": requestActions[srvActM](act),
				"params": params
			};
			request.result.post = {
