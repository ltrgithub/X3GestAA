...
var _srvActionEx = {
	...
	"SUP_LIST_ABANDON":39 // abandon tableau (abandon ligne + tab)
};
var _proxyAction = {
	"PRINTER_SEL_OPTS": 1,
...
exports.srvActionStart = 768;

exports.svcFormat = 500;
exports.svcPrint = 501;

exports.getSrvAction = function (action){
	return (_srvAction[action] + 1024);
};
exports.getSrvActionEX = function (action){
	return (_srvActionEx[action] + exports.srvActionStart);
};
...
exports.isActFocusSelected = function(actionNum){
	var ret = false, acts = ["SRV_CHAMP_SUIVANT", "SRV_CHAMP_INACTIF", "SRV_CHAMP_PRECEDENT", "SRV_FICHIER_NOUVEAU"], i, len;
	for (i = 0, len = acts.length; i < len; i++) {
		if(exports.getSrvAction(acts[i]) == actionNum) {
			ret = true;
			break;
		}
	}
...
exports.isActFldFocus = function(actionNum){
	var ret = false, acts = ["SRV_GETFOCUS", "SRV_COMMANDGRID", "SRV_KIB"], i, len;
	for (i = 0, len = acts.length; i < len; i++) {
		if(exports.getSrvAction(acts[i]) == actionNum) {
			ret = true;
			break;
