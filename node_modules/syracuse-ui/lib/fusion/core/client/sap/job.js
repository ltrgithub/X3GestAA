...
var hosterJobHandler = require('syracuse-ui/lib/jobs/jobHandlerFusion').JobHandlerFusion;
...
exports.AsyncJobsManagement = helpers.defineClass(AsyncJobsManagement, null, {
    initialize: function(siteController, hosterController) {
        ...
        this._jobTypesClass = {"report": {"cla": exports.ReportJob, "labelId": "101"}};
        this._jobList = {};
        this._jobsPending = 0;
        this._jobsCount = 0;
    },
    ...
    register: function(jobType, params){
        var id, job = this._jobTypesClass[jobType] ? new this._jobTypesClass[jobType].cla() : null, result, $diag = null, self = this, j, lenj;
        if(job) {
            job._typeId = jobType;
            ...
            try {
                ...
                result = job._hosterJobHandler.register(job.getProperties(), (job._phase = "started"), job.getProgressScvId(), job.getDeleteScvId(), function() {
                    try {
                        self._releaseAsyncJob(id, {"noHosterRelease": true});
                    }
                    catch (e) {}
                });
                ...
                var errNum = "66", mess = [this._getErrMessLabel(errNum, "An unexpected javascript error has been detected while registering an asynchronous job")];
        ...
        if($diag) {
            for (j = 0, lenj = $diag.length; j < lenj; j++) {
                if(util.isErrDiagnosis($diag[j].severity)) {
                    try {
                        this._releaseAsyncJob(id, {"noHosterRelease": true});
                    }
                    catch (e) {}
                    finally{
                        break;
    ...
    releaseAllAsyncJobs: function(opts, doDispose){
        var status = true, st, i, len, jobsArray = Object.keys(this._jobList), hosterJH, job;
    ...
    dispose: function(){
        this.releaseAllAsyncJobs({"noHosterRelease": true}, false)
