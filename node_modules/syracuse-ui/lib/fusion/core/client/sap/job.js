...
exports.AsyncJobsManagement = helpers.defineClass(AsyncJobsManagement, null, {
    ...
    processStatements: function(jobType, statements) {
        var status = null, i, j, leni, lenj, jobStatements = Object.keys(statements), jobStatement;
        for (i = 0, leni = jobStatements.length; i < leni; i++) {
            ...
            if(_KStatementsStub[jobStatement]) {
                for (j = 0, lenj = statements[jobStatement].length; j < lenj; j++) {
                    this[_KStatementsStub[jobStatement]].call(this, jobType, statements[jobStatement][j]);
                }
            }
        }
        return status;
    ...
    register: function(jobType, params){
        var id, job = this._jobTypesClass[jobType] ? new this._jobTypesClass[jobType]() : null, ret = false;
        if(job) {
            job._typeId = jobType;
    ...
    releaseAllAsyncJob: function(opts, doDispose){
        var status = false, st, i, len, jobsArray = Object.keys(this._jobList);
    ...
    dispose: function(){
        this.releaseAllAsyncJob(null, true)
