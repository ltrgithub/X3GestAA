...
var _KdateOrderFlags = {"DMY": 1, "YMD": 2, "MDY": 0};
var _KtypeAcceptFmt = {};
...
_KtypeAcceptFmt[syraUtil.dataTypes.passwdType] = true;
...
function FormatterWrapper(){
	this.locales = {"datePivot": 1963, "dateSep": "/", "thousandSep": " ", "decimalSep": ".", "timeSep": ":", "dateOrder": _KdateOrderFlags.MDY};
	this.locales.vOn = true;
	this.locales.zOn = false;
	this.settings = {"localCtrl": true, "localFormat": false, "ctrlOnfly": false};
}
...
exports.FormatterWrapper = helpers.defineClass(FormatterWrapper, null, {
	...
	initialize: function (extendFmt, locales, hoster, fmtType, opts) {
		...
		this._hoster = hoster;
		// Set SvcMapping
		this.svcMapping[svcId.toRaw] = {"method": "_svcFormatRaw", "raw": undefined};
		this.svcMapping[svcId.toDisplay] = {"method": "_svcFormatDisp", "raw": false};
		this.svcMapping[svcId.toEdit] = {"method": "_svcFormatEdit", "raw": false};
		this.svcMapping[svcId.rawToEdit] = {"method": "_svcFormatEdit", "raw": true};
		this.svcMapping[svcId.rawToDisp] = {"method": "_svcFormatDisp", "raw": true};
		// Set locales
		if (locales){
	...
	rawToEdit: function(raw, fmt) {
		var output = {"succeeded": true, "edit": raw};
		if (this._isThickFormat(fmt) && raw) {
			try {
				output.edit = this.formatter.formatEdit({"format": fmt}, raw, true);
			}
			catch(e) {
				output.succeeded = false;
				output.$diagnoses = [this.makeDiagnosis("63", "Error while deformatting value", e)];
			}
		}
		return output;
	},
	...
	editToDisplay: function(fmt, inputValue, rawValue) {
		var output = {"succeeded": true, "show": inputValue};
		if (this._isThickFormat(fmt)) {
			try {
				output.show = this.formatter.formatEditToDisplay({"format": fmt}, inputValue, rawValue);
			}
			catch(e) {
				output.succeeded = false;
	...
	checkOnFocusOut: function(inputValue, type, fmt, fmtKind, lastRawValue) {
		var output = {"raw": inputValue, "show": undefined, "$diagnoses": null, "succeeded": true}, format;
	...
	unformat: function(inputValue, type, fmt, check) {
		var output = {"succeeded": true, "raw": inputValue}, format;
		if (_KtypeAcceptFmt[type] && this._isThickFormat(fmt) && inputValue !== null && inputValue.length > 0) {
			format = {"format": fmt};
			try {
...
exports.LegacyCSFieldFormatter = helpers.defineClass(LegacyCSFieldFormatter, LegacyCSFormat.LegacyCSFormatter, {
	getSyraTypeFromFormat: function (format) {
		var fType = this.getFmtDataType(format || this.szFormat), sType = syraUtil.dataTypes.defaultType, type = fType.t;
		switch(type) {
