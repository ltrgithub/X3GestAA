...
exports.FormatterWrapper = helpers.defineClass(FormatterWrapper, null, {
	...
	_svcFormatEdit: function(value, format, type, raw) {
		var fmtOk = format ? this.isThickFormat(format.format) : false,
			process = true;
		var oFormatter = fmtOk && (value == undefined || value.length == 0) ? new LegacyCSFieldFormatter() : null;
		if (!fmtOk || (oFormatter && (type = oFormatter.getSyraTypeFromFormat(format.format)) && _KtypeIgnoreFmt2DispIfEmpty[type])) {
			process = false;
		}

		return process ? this.formatter.formatEdit(format, value, raw) : value;
	}
});
