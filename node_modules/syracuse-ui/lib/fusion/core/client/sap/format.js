...
exports.dateOrderK = LegacyCSFormat.dateOrderK;
...
function FormatterWrapper() {
	this.locales = {
		...
		"decimalSep": ".",
		"timeSep": ":",
		"dateOrder": LegacyCSFormat.dateOrderK.MDY
	};
...
exports.FormatterWrapper = helpers.defineClass(FormatterWrapper, null, {
	...
	checkOnFocusOut: function(inputValue, type, fmt, fmtKind, lastRawValue) {
		...
		if (_KtypeAcceptFmt[type] && this.isThickFormat(fmt) && inputValue !== null && inputValue.length > 0) {
			...
			if (fmtKind == syra_fusion.sapUtil.fieldModeType.edit) {
				try {
			...
			if (!output.$diagnoses) {
				try {
					...
					if (type === syra_fusion.syraUtil.dateType && output.raw == _kDateZero) {
						output.raw = "";
					}
				} catch (e) {
					output.succeeded = false;
	...
	formatSvc: function(svcId, value, fmt, type, options) {
		...
		try {
			...
			output.result = null;
			output.succeeded = false;
			output.$diagnoses = [this.makeDiagnosis(this.svcMapping[svcId].errorCode, this.svcMapping[svcId].erroDefLabel, e)];
		}
		return output;
	},

	_svcFormatRaw: function(value, format, type) {
		var checkFlag = type == syra_fusion.syraUtil.dataTypes.dateType ? true : false;
		try {
...
exports.LegacyCSFieldFormatter = helpers.defineClass(LegacyCSFieldFormatter, LegacyCSFormat.LegacyCSFormatter, {
	getSyraTypeFromFormat: function(format) {
		var fType = this.getFmtDataType(format || this.szFormat),
			sType = syra_fusion.syraUtil.dataTypes.defaultType,
			type = fType.t;
		switch (type) {
			case this.pTypesC.S_STRG:
				sType = syra_fusion.syraUtil.dataTypes.stringType;
				break;
			case this.pTypesC.S_NUM:
				if (fType.st == this.pSubTypesC[type].X_FLOAT) {
					sType = syra_fusion.syraUtil.dataTypes.realType;
				} else if (fType.st == this.pSubTypesC[type].X_DCB) {
					sType = syra_fusion.syraUtil.dataTypes.decType;
				} else {
					sType = syra_fusion.syraUtil.dataTypes.integerType;
				}
				break;
			case this.pTypesC.S_DATE:
				sType = syra_fusion.syraUtil.dataTypes.dateType;
				break;
			case this.pTypesC.S_MNL:
				sType = syra_fusion.syraUtil.dataTypes.choiceType;
				break;
		}
		return sType;
	},
	getMaxLengthFromFormat: function(fmt, syraType) {
		...
		try {
			switch (sType) {
				case syra_fusion.syraUtil.dataTypes.decType:
					// "N3:9.2#"
					...
					break;
				case syra_fusion.syraUtil.dataTypes.integerType:
				case syra_fusion.syraUtil.dataTypes.realType:
					// "N3:4#" - "N:10F"
					...
					maxLength += _ts(cut.left);
					if (syra_fusion.syraUtil.dataTypes.realType == sType) {
						maxLength += 6;
					}
					break;
				case syra_fusion.syraUtil.dataTypes.stringType:
				...
				case syra_fusion.syraUtil.dataTypes.dateType:
					...
					maxLength = 11;
					break;
				case syra_fusion.syraUtil.dataTypes.choiceType:
					// Arbitrary...
					maxLength = 30;
					break;
				default:
