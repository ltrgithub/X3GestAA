...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    setFocus: function(style, entity){
        var self = this, $entityEx, $bounds = this._fieldsColl.bound, args, setFocusProxy, setFocusStyleProxy, $screenTab = null, screenTab, evtData;
        var retOk = null, proxyFocusRet = {"noFocusable": true}, ret = _getNextFieldDescObj(sapUtil.getObjFieldValue(null, sapUtil.fieldModeType.show, null, true), "SRV_CHAMP_INACTIF"); 
        if (!entity || entity.length === 0) {
            ...
            if (($entityEx = this._getEntityEx(entity)) && $entityEx.entity) {
                ...
                if ($bounds[$entityEx.entity] && $bounds[$entityEx.entity].length > 0) {
                    ...
                    if (setFocusProxy && $bounds[$entityEx.entity][0][setFocusProxy]) {
                        proxyFocusRet = $bounds[$entityEx.entity][0][setFocusProxy].apply($bounds[$entityEx.entity][0], args);
                    }
                    if(!proxyFocusRet || !proxyFocusRet.noFocusable) {
    ...
    getCollRcdUUID: function (collName, nl){
        return this._dataset && this._dataset[collName] ? this._dataset[collName].getCollRcdUUID(nl - 1) : null;
    },
