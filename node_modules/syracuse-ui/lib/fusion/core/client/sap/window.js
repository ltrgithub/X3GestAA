...
WinModel = helpers.defineClass(WinModel, null, {
        ...
        _.each(entities, function(data, entityName){
            ...
            if ($entities[entityName]) {
                ...
                if (!this._dataset[entityName]) {
                    ...
                    features = store.initialize($type, _instVtbl[$type] ? _instVtbl[$type].isDatasetColl : _instVtbl["default"].isDatasetColl, entityName, $entities[entityName], this._isLocalFmt ? this._fmtWrapper : null, {"isNodeWS": this._isNodeWS, "locales": this._fmtWrapper.locales});
    ...
    _setDataField: function(entityName, entityBound, winProto, recordSet, data, options, refProps){
        // Adjust "style" meta
        var newRawValue = false, newFmt = false, dataPtr = this._dataset[entityName].data, fmt, refTitle, refValue = {}, entities = {}, rawValueEx, rawValue, yetFormatted;
        sapUtil.cureStyle(data, entityName, this._getSession());
        // Check Format
        if(this._isLocalFmt && dataPtr.fmt !== undefined) {
            ...
            if(data.v !== undefined && data.v !== null && fmt) {
                if(!yetFormatted) {
                    ...
                    rawValueEx = this._fmtWrapper.formatter.unformatField(fmt, this._dataset[entityName].realType, data.v, true);
        ...
        if (_storeCommit(options)) {
            ...
            if(this._isLocalFmt && newRawValue) {
    ...
    _setDataColl: function(entityName, entityBound, winProto, recordSet, data, options){
        ...
        syraElapseEx = this._dataset[entityName].applyDataCollEx(data, commit, options, function(update, updateOpts){
            ...
            tick = setUI ? self._bindDataColl(entityName, entityBound, change, updateOpts, logger) : 0;
    ...
    _getDataColl: function(entityName, line, isExternalWrap, onlyValue, opts){
        var data, isLinesColl = sapUtil.isBlockInst(entityName), dateSet, options, getRawVal, rawVal, i, len, parentEntity;
        if(!this._dataset[entityName] && isLinesColl) {
            return {};
        }
        dateSet = isLinesColl ? this._dataset[entityName].data : this._dataset[(parentEntity = sapUtil.getBlockInst(entityName))].data;
        if (line === undefined || line === null) {
            ...
            else if((len = dateSet.length)) {
                // Full row...
                getRawVal = !!(opts && opts.rawVal && this._dataset[parentEntity].metaFmt[entityName]);
                data = [];
                for (i = 0; i < len; i++) {
                    data[i] = getRawVal ? dataSet[i].$rawV[entityName] : dataSet[i][entityName];
                }
                if(isExternalWrap) {
                    data = {"val": data};
                }
            ...
            if (isLinesColl) {
                data = isExternalWrap ? {"val": dateSet[line - 1]} : dateSet[line - 1];
            }
            else {
                getRawVal = !!(opts && opts.rawVal && (rawVal = dataSet[line - 1].$rawV[entityName]));
                data = isExternalWrap ? {"val": getRawVal ? rawVal.v : dateSet[line - 1][entityName]} : (getRawVal ? rawVal.v : dateSet[line - 1][entityName]);
            }
        }
    ...
    _getFmtColl: function(entityName, parentEntity, line, opts){
        ...
        return dataset ? dataset.getMetaFmtColl(entityName, line - 1) : null;
    },
