...
_instVtbl[syraUtil.dataTypes.treeType] = {
	"_setDataStore": "_setDataTree",
	"_getDataStore": "_getDataTree",
	"_getDataFmt": "_getFmtTree",
	"_getState": "_getStateTree"
};
...
exports.WinStack = helpers.defineClass(WinStack, null, {
	...
	postBuildInit: function(winId) {
		var win = this.getWindow(winId),
			pageFacet, syraModel, sortTabs, barLists, i, len, metaBar, metaObj = {}, listId;
...
WinModel = helpers.defineClass(WinModel, null, {
	...
	onWinEvent: function(event, controller) {
		...
		if (!isGrid) {
			$cont = ($$target && (id = event.target.syraFieldId)) ? this.syraModel.idMap[id] : null;
	...
	setFocus: function(highlight, style, entity, nl) {
		...
		if (!entity || sapUtil.getScreenRank(entity) == 0) {
			...
			if (($entityEx = this.getEntityEx(entity)) && $entityEx.entity) {
				...
				// So, screen is online
				// BO integration - maybe need to be review more properly
				if ($entityEx && $entityEx.type === "application/x-vignette") {
					return retOk;
				}
				if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
					...
					if (proxyFocusable || proxyFocusable === undefined) {
						...
						ret = retOk;
					} else if (proxyFocusable == null && $entityEx.type == syraUtil.dataTypes.collectionType && syraUtil.isEntityMandatory(entity, this.getProto() === true)) {
	...
	_setDataColl: function(entityName, entityBound, winProto, recordSet, data, options) {
			...
			if (entityBound) {
				_.each(entityBound, function(item) {
					if (item.sortCapability && item.sortCapability._$orderBy) {
						item.helper.fetchClientRecordSet(item, {
							params: {
								orderBy: null
							}
						});
		...
		if (data.$meta) {
			...
			if (setUI && entityBound) {
				...
				syraElapseEx.meta = this._bindDataColl(entityName, entityBound, change, {
					"$isPartialDelta": true
				}, options, logger);
			}
		}
	...
	_bindDataColl: function(entityName, entityBound, $resources, bindOpts, setDataOpts, logger) {
		var elapse = (new Date()).getTime(),
			i, len, item,
			discardUnsort = setDataOpts && setDataOpts.discardUnsort;
	...
	_getStateTree: function(boundTree, entityName, idx, opts) {
		return {};
	},
...
var _PrivateEvtHandlingWin = helpers.defineClassEx(function() {}, WinModel, {
	...
	trigger: function(event) {
		...
		} else if (this.isInternalEvtManagerCat(event)) {
			processed = true;
		}
...
var _formulaWin = helpers.defineClass(function() {
	...
	setFocus: function() {
		...
		if (this._formulafld) {
			WinModel.prototype.setFocus.call(this, true, null, this._formulafld, 0);
		}
		return null;
	}
});
...
_privateEvtHandlingWinT = helpers.defineClass(_privateEvtHandlingWinT, _PrivateEvtHandlingWin, {
	registerEvtHandler: function() {
		var _evtHandlingModules = { //"selimp": require('../eventhandling/selimp').SelimpEventHandling
		};
		// Init private event manager
