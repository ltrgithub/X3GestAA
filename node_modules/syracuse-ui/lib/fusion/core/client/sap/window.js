...
exports.WinStack = helpers.defineClass(WinStack, null, {
		...
		if (this._windows[idEx.id] && nbChanges > 0) {
			if (fromSrv) {
				if (this._sapController.isReplyFromTabulationAction(srvReply)) {
					applyChangeOpts = {
						"tabulation": true,
						"field": this._windows[idEx.id].getBoundField(options.currSrvTarget.xid, sapUtil.isGridInst(options.currSrvTarget) ? {
	...
	postBuildInit: function(winId) {
		var win = this.getWindow(winId),
			...
			lenj, $XID = sapUtil.metaNameMap.xid;
		...
		if (win && (syraModel = win.getSyraPage())) {
				...
				if ((barLists = syraModel.childrenSection)) {
					...
					for (i = 0, len = barLists.length; i < len; i++) {
						if (barLists[i].isSection) {
							if ((sectionMeta = syraUtil.getMetaFromObject(barLists[i])) && (xid = sectionMeta[$XID])) {
					...
					if ((len = win.$folderTabs.length) && syraModel.$persistPrototype) {
						...
						secMap = syraUtil.getTopSectionsMap(syraModel.$persistPrototype.$article);
			...
			if (win.getWinName() === sapUtil.specialWindow.selbox.id) {
				...
				for (ii = 0, jj = keysProto.length; ii < jj; ii++) {
					...
					if (propProto.$type == syraUtil.dataTypes.collectionType) {
			...
			if ((logger = this._sapController.getlogger())) {
				...
				if (this._sapController.isActionEnabled(winId, actId) !== null) {
					...
					updateItem.$links[actId] = {
						$title: (syra_fusion.getMessage(isTraceEnable ? "menuTraceLocalOff" : "menuTraceLocalOn")),
						$icon: {
...
WinModel = helpers.defineClass(WinModel, null, {
	...
	getBoundField: function(entityName, opts) {
		...
		if (!boundFld && sapUtil.isSpecialScrnId(entityName)) {
	...
	setDataStoreValue: function(entityName, line, value, options) {
		var data = {};
		sapUtil.appendEntitiesValue(data, null, entityName, line, value);
			...
			if (($entityEx = this.getEntityEx(entity)) && $entityEx.entity) {
				if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
					...
					if (setFocusProxy && $boundField[setFocusProxy]) {
						if ($entityEx.type == syraUtil.dataTypes.collectionType && $boundField.$orderBy && !$boundField.$field.$isFusionNavigationList) {
					...
					if (proxyFocusable == null && $entityEx.type == syraUtil.dataTypes.collectionType && syraUtil.isEntityPopupCard(entity, this.getProto())) {
						// Perhaps mandatory field just set in cardview... or focus for second time
						if (!this._hackCardViewFocus) {
							this._hackCardViewFocus = {};
						}
						if ((syraUtil.isEntityMandatory(entity, this.getProto()) || this._hackCardViewFocus[entity]) && sapUtil.isEmptyData(syraUtil.getEntityType(entity, this.getProto()), this.getDataStoreValue(entity)[index])) {
					...
					} else if (proxyFocusable === false && $entityEx.type == syraUtil.dataTypes.collectionType && sapUtil.isFieldInst(entity)) {
	...
	setEntitiesValue: function(entities, options, fromSrv, entitiesKey) {
		...
		for (i = 0, len = arrEntities.length; i < len; i++) {
			...
			if ($entities[entityName]) {
				...
				if (!this._setDataContainer(entityName, data, null) && (screen = sapUtil.getScreenId(entityName)) && this._pendingMeta[screen]) {
	...
	_setDataContainer: function(containerName, data, options) {
		...
		if ((obj = this._getContainerObj(containerName, options))) {
			...
			if ((sty = data.sty) != null) {
				...
				if (sapUtil.isScreenInst(containerName)) {
					proto = this.getProto();
					arrP = Object.keys(proto);
					sc1 = sapUtil.getScreenId;
					sc2 = syraUtil.dataTypes.collectionType;
	...
	_getContainerObj: function(containerName, opts) {
		var tabIdx, fusionController, firstField = sapUtil.getFirstChild(containerName, this._fieldsColl.bound),
			$screenTab = null,
			contObj = null,
			button, menus;
		if (firstField) {
		...
		if (sapUtil.isScreenInst(containerName) && this.$folderTabs) {
			...
			if (!$screenTab && sapUtil.isTabScreen(containerName)) {
		...
		} else if (sapUtil.isButtonScreen(containerName)) {
		...
		} else if (opts && opts.isGridInst && this._fieldsColl.bound[containerName]) {
...
var _formulaWin = helpers.defineClass(function() {
	...
	setFocus: function(highlight, style, entity, nl, isEdit, params, opts) {
		...
		if (fusionController || (fusionController = syraUtil.getFusionController(this.syraModel)) && fusionController._sapController) {
