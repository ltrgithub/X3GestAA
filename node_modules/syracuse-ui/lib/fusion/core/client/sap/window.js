...
var _instVtbl = {"default": {"_getDataInput": "getDataValue", "setFocus": "focus", "_setDataStore": "_setDataField", "_getDataStore": "_getDataField", "getCtxProps": "_getFieldCtxProps", "isDatasetColl": false, "isDirty": "dirty"}};
    _instVtbl[syraUtil.dataTypes.treeType] = {"_setDataStore": "_setDataTree", "_getDataStore": "_getDataTree"};
    _instVtbl[syraUtil.dataTypes.collectionType] = {"_getDataInput": "_getValueCel","setFocus": "setFocus", "_setDataStore": "_setDataColl", "_getDataStore": "_getDataColl", "isDatasetColl": true, "removeFocus": "resetFocus", "setFocusStyle": "setFocusStyle"};
...
exports.WinStack = helpers.defineClass(WinStack, null, {
    ...
    setGlobalMenu: function(session, cmnMenus) {
        this.winMenus.initialize(session, cmnMenus);
    },
    ...
    shouldDiscardCloseOpen: function(closeId, openWins){
        ...
        var ret = false, idExClose, closeWin;
        if (openWins && (idExClose = this._getWinIdEx(closeId))) {
            if(openWins[idExClose.id]) {
                ...
                else if ((closeWin = this.getWindow(idExClose.id)) && closeWin.isSameWin(openWins[idExClose.id]) && sapUtil.isSpecialWin(closeWin.getWinName())) {
                    // Avoid to close, then reopen Selbox, formula or choose!
                    ret = true;
                }
            }
        }
    ...
    stack: function(id, winDef, syraPageModel){
        ...
        (this._windows[idEx.id] = new WinModel()).initialize(idEx, winDef, syraPageModel, this._sapController.fmtWrapper && this._sapController.fmtWrapper.settings.localFormat, this._sapController.fmtWrapper);
        ...
        if (this._windows[idEx.id]) {
            _.each(change, function(data, type){
                if (_winK.stubWins[type] && this._windows[idEx.id][_winK.stubWins[type]]) {
                    retEx[type] = this._windows[idEx.id][_winK.stubWins[type]](data, options, this);
                }
            }, this);
        }
        else {
    ...
    postBuildInit: function(winId){
        var win = this.getWindow(winId), pageFacet, syraModel, sortTabs;
        if (win) {
...
WinModel = helpers.defineClass(WinModel, null, {
    initialize: function(idEx, winDef, syraModel, localFmtData, fmtWrapper){
    ...
    _getValueFieldRef:function($boundField, entity){
        ...
        return refProps ? value[refProps.property] : value;
    },

    setFocus: function(style, entity, nl){
        ...
        if (!entity || entity.length === 0) {
            ...
            if (($entityEx = this.getEntityEx(entity)) && $entityEx.entity) {
                ...
                if ($screenTab && $screenTab._tabTitle) {
                ...
                if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
                    ...
                    index = sapUtil.getIdxFromNL(nl);
                    args = arguments.length > 3 ? [entity, index].concat(Array.prototype.slice.call(arguments, 3)) : [entity, index];
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity, idx, isEdit, params){
        var sess, boundCell;
    ...
    setEntitiesValue: function(entities, options, winStack){
        ...
        var $entities = this.getProto(), $bounds = this._fieldsColl.bound, screen;
        _.each(entities, function(data, entityName){
            ...
            if ($entities[entityName]) {
                ...
                if (!this._dataset[entityName]) {
                    (this._dataset[entityName] = new EntityStore()).initialize($type, _instVtbl[$type] ? _instVtbl[$type].isDatasetColl : _instVtbl["default"].isDatasetColl, entityName, $entities[entityName], this._isLocalFmt ? this._fmtWrapper.formatter : null, {"isNodeWS": this._isNodeWS, "locales": this._fmtWrapper.locales});
                }
                // Set data
    ...
    _setStatusBar: function(status, options, winStack){
        var len, i, winMns, ret, item;
        // TODO : append this._statusBar;?
        if(status.ico && (winMns = winStack.winMenus)) {
            for (i = 0, len = status.ico.length; i < len; i++) {
                item =  status.ico[i];
                ret = item[0] > 0 ? winMns.addStatusIco(this.getWinId(),item[1], item[0]) : winMns.removeStatusIco(this.getWinId(), item[1]);
            }
        }
        if(status.t) {
            // TODO : Notify Hoster?
    ...
    _setTitle: function(title, options, winStack){
        this.syraModel.setTitle(this._title = title);
        return null;
    },
    
    _setDataField: function(entityName, entityBound, winProto, entitiesData, data, options, refProps){
        // Adjust "style" meta
        var newValue = false, newFmt = false, dataPtr = this._dataset[entityName].data, fmt, refValue = {};
        sapUtil.cureStyle(data, entityName, this._getSession());
        // Check Format
        if(this._isLocalFmt) {
            fmt = dataPtr.fmt;
            // If entity accept format "fmt" is not undefined... then check if there is a new format in meta
            if(fmt !== undefined && data.fmt) {
                fmt = data.fmt;
                newFmt = true;
            }
            ...
            if(fmt && (newFmt || newValue)) {
                try {
                    data.v = this._fmtWrapper.formatter.formatField({"format": fmt}, dataPtr.$rawV, true, true);
                }
                catch (e){
                    data.v = dataPtr.$rawV;
                }
                ...
                if(!refProps) {
                    ...
                    if(data.v !== dataPtr.v) {
                        ...
                        if(refProps.propTitle) {
                            if (entitiesData && entitiesData[refProps.propTitle] !== undefined && entitiesData[refProps.propTitle].v != undefined) {
                                // TODO : potentially, we have to format ref title...
                                refValue[refProps.propTitle] = entitiesData[refProps.propTitle].v;
                            }
                            else {
    ...
    _setDataFieldRef: function(entityName, entityBounds, winProto, entitiesData, data, options){
        // We never have reference field in collection (x-array)
        var $entity = winProto[entityName], refProps = syraUtil.getRefPropertyEntity($entity), type = refProps ? refProps.proto.$type : "";
        this[_instVtbl[type] && _instVtbl[type]._setDataStore ? _instVtbl[type]._setDataStore : _instVtbl["default"]._setDataStore](entityName, entityBounds, winProto, entitiesData, data, options, refProps);
    },
    
    _setDataIconField: function(entityName, entityBound, winProto, entitiesData, data, options, refProps){
        data = sapUtil.adjustIcoFieldData(data);
        return this._setDataField(entityName, entityBound, winProto, entitiesData, data, options, refProps);
     },

    _setDataImageField: function(entityName, entityBound, winProto, entitiesData, data, options, refProps){
    ...
    _setDataColl: function(entityName, entityBound, winProto, entitiesData, data, options){
        ...
        if (entityBound) {
            _.each(entityBound, function(item){
                if(item.sortCapability && item.sortCapability._$orderBy){
                    item.fetchClientRecordSet({params:{orderBy:null}})
                }
            });
        }
    ...
    _setDataTree: function(entityName, entityBound, winProto, entitiesData, data, options){
        // TODO : improve??
        if (entityBound && (!options || !options.noUI)) {
            _.each(entityBound, function(item){
    ...
    _getDataFieldRef: function(entityName, line, isExternalFmt, onlyValue){
        // We never have reference field in collection (x-array)
