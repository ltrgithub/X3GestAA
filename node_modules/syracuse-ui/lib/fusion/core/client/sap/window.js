...
WinModel = helpers.defineClass(WinModel, null, {
	...
	getFctName: function(onlyName) {
		var winName = this.getWinName(),
			fctName, prefFct = "GES",
			ret = null;
		// Way to compute function name has changed -> Cf. #116664 - suggested by core team
		fctName = this._mdata ? (this._mdata.func || "") : "";
		fctName = winName && winName.charAt(0) === "O" && fctName.substr(0, 3) === prefFct ? (prefFct + winName.substr(1)) : fctName;
		if (fctName) {
			ret = onlyName ? fctName : {
				"title": this.getTitle(),
				"name": fctName
			};
		}
