        ...
        if (this._windows[idEx.id]) {
            _.each(change, function(data, type){
                if (_winK.stubWins[type] && this._windows[idEx.id][_winK.stubWins[type]]) {
                    retEx[type] = this._windows[idEx.id][_winK.stubWins[type]](data, options, this._sapController);
                }
            }, this);
        }
        else {
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    setEntities: function(entities, options, sapController){
    ...
    _setStatusBar: function(status, options, sapController){
       var len, i, winMenus, ret;
        if(status.ico && (winMenus = sapController.winMenus)) {
            for (i = 0, len = status.ico.length; i < len; i++) {
                ret = status.ico[0] > 0 ? winMenus.addStatusIco(this.getWinId(), status.ico[1], status.ico[0]) : winMenus.removeStatusIco(this.getWinId(), status.ico[1]);
            }
        }
        if(status.t) {
            // TODO : Notify Hoster?
    ...
    _setTitle: function(title, options, sapController){
