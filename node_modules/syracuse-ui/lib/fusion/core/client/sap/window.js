...
var _winK = {
 ...
 var _instVtbl = {"default": {"getValue": "getValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "isDatasetColl": false}};
     _instVtbl[syraUtil.treeType] = {"getValue": "getValue", "setFocus": "focus", "setData": "_setDataTree", "getData": "_getDataTree", "isDatasetColl": false};
     _instVtbl[syraUtil.collectionType] = {"getValue": "getValue", "setFocus": "setFocus", "setData": "_setDataGrid", "getData": "_getDataGrid", "isDatasetColl": true, "removeFocus": "resetFocus"};
        ...
        if(this._windows[idEx.id]) {
            if(sapUtil.isSpecialScrnId(entity)) {
                if(sapUtil.isButtonScreen(entity) || sapUtil.isListScreen(entity)) {
                    ...
                    ret = this._sapController.getCurrTab();
                }
            }
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _bindEvents: function(){
        var self = this;
        this._$$syraPageItem.on("click keydown", "." + _winK.fusionPageK + " *", null, function(event){
                var $$target, $$cont, id, $cont, $contMeta, $fusionPageMeta, doEvt, isGrid = false, isField = false, caught = false;
                console.log("**onPageEvent : " + "'" + self.syraModel.getTitle() + "'" + " [" + event.type + "]");
                // Is it a fusion grid or one of these children?
                $fusionPageMeta = syraUtil.getFusionPageMeta(self.syraModel);
                $$cont = ($$target = $(event.target)) ? $$target.closest("." + syraUtil.gridFusionK.cont) : null;
                $cont = ($$cont && (id = $$cont.attr("id"))) ? self.syraModel.idMap[id] : null;
                isGrid = ($cont && ($contMeta = syraUtil.getMetaFromObject($cont))) ? (self.getProto())[$contMeta.$bind] : null;
                // So, notify grid...
                if(isGrid && event.type == "click"){
                    caught = true;
                    doEvt = $cont.onClick($fusionPageMeta.controller, event);
                }
                // Is it a fusion field?
                if(!isGrid)
                {
                    $cont = ($$target && (id = $$target.attr("data-s-field"))) ? self.syraModel.idMap[id] : null;
                    isField = ($cont && ($contMeta = syraUtil.getMetaFromObject($cont))) ? (self.getProto())[$contMeta.$bind] : null;
                }
                // Field fusion events are caught by dedicated handler...
                if(!caught && !isField && event.type != "click") {
                    // General case
                    doEvt = $fusionPageMeta.controller.trigger("wdgt.win." + event.type, {"target": self, "type": event.type, "data": {"nativeEvt": event}});
                }
                // So...
    ...
    getValue: function (entity) {
        ...
        if($entityEx.entity) {
            if($bounds[$entityEx.entity] && $bounds[$entityEx.entity].length > 0) {
                ...
                ret = $bounds[$entityEx.entity][0][_instVtbl[$entityEx.type] ? _instVtbl[$entityEx.type].getValue : _instVtbl["default"].getValue].apply($bounds[$entityEx.entity][0], arguments);
            }
        }
        return ret;
    },
    ...
    setFocus: function (entity) {
        var $entityEx, $bounds = this._fieldsColl.bound, ret = null;
        if(sapUtil.isSpecialScrnId(entity)) {
            // TODO : focus on tab, left list or buttons block ==> Next Field
            if(sapUtil.isButtonScreen(entity)) {
                ret = {"ok": true, "srvAct": true, "act": "SRV_CHAMP_SUIVANT", "v": null};
            }
        }
        else if (!entity || entity.length === 0) {
    ...
    setEntities: function(entities){
        /*var test = document.syraXTestX.cmi.reply.OAOC_1.entities;
