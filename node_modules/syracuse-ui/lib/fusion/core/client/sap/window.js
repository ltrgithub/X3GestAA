...
WinModel = helpers.defineClass(WinModel, null, {
   ...
   _getValueChoiceField:function($boundField, entity, index){
    ...
    _setStatusBar: function(status, options, winStack){
        var len, i, winMns, ret, item, page = syraUtil.getPage(this.syraModel);
        // TODO : append this._statusBar;?
        if(status.ico && (winMns = winStack.winMenus)) {
            for (i = 0, len = status.ico.length; i < len; i++) {
                item =  status.ico[i];
                ret = item[0] > 0 ? winMns.addStatusIco(this.getWinId(), page, item[1], item[0]) : winMns.removeStatusIco(this.getWinId(), page, item[1]);
            }
        }
        if(status.t) {
            // TODO : Notify Hoster?
