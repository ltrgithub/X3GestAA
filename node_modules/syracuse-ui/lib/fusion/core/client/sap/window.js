...
var _instVtbl = {"default": {"getValue": "getDataValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "isDatasetColl": false}};
     ...
     _instVtbl[syraUtil.dataTypes.iconType]["setData"] = "_setDataIcodField";
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity) {
    ...
    _setDataIcodField: function(entityName, entity, entityBound, data, options){
        var state, num = !data.v || data.v.length == 0 ? -1 : parseInt(data.v), disable = null;
        // Deal with disable & readonly (disable overrides, readonly is rerouted)
        state = data.stt ? sapUtil.getState(data.stt) : {};
        if (state.$isReadOnly != undefined) {
            disable = state.$isDisabled != undefined ? state.$isDisabled : state.$isReadOnly;
            data.stt = sapUtil.setState({"$isDisabled": disable}, data.stt || 0);
            data.stt = sapUtil.removeStateMask({"$isReadOnly": ""}, data.stt);
        }
        // Hidden?
        if (num < 0) {
            data.v = null;
        }
        data.stt = sapUtil.setState({"$isHidden": num < 0 ? true : false}, data.stt || 0);
        // Call default API?
