...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity, nl, isEdit, params){
        ...
        if (entityExType == syraUtil.dataTypes.collectionType && $field._store._records.length >= nl) {
            $field =($field._store._records[nl - 1]?$field._store._records[nl - 1].boundFields[entity][0]:null);
        }
    ...
    _setDataColl: function(entityName, entityBound, data, options){
        var mvtCode, delta, rcd = {}, commit = _storeCommit(options), self = this, elapse = (new Date()).getTime(), syraElapseEx = {
            "data": 0,
            "mvt": 0,
            "meta": 0
        }, syraElapse = 0, styObj, styEx, setUI = (!options || !options.noUI);
        var logger, fusionController = syraUtil.getFusionController(this.syraModel), leftListColl = sapUtil.isListScreen(entityName);
        ...
        syraElapseEx = this._dataset[entityName].applyDataCollEx(entityName, data, commit, options, function(update, options){
            return setUI ? self._bindDataColl(entityName, entityBound, update, {}, null, options, logger) : 0;
        });
        ...
        // Binding...
        if (data.$meta && setUI) {
            syraElapseEx.meta = this._bindDataColl(entityName, entityBound, undefined, null, data.$meta, options, logger);
        }
        if (logger) {
            syraElapse += (syraElapseEx.data + syraElapseEx.mvt + (syraElapseEx.meta || 0));
            logger.trace({
                "severety": "info"
            }, "Binding Duration for '" + entityName + "' ==> Fusion : " + ((new Date()).getTime() - elapse - syraElapse) + " - Syra : " + syraElapse + " (with apply Mvt : " + syraElapseEx.mvt + ", update data : " + syraElapseEx.data + ", and meta : " + (syraElapseEx.meta || 0) + ")");
        }
        return null;
    },
    
    _bindDataColl: function(entityName, entityBound, $resources, record, metaData, options, logger){
        var temp, elapse = (new Date()).getTime(), update = {
            "$properties": {},
            "$isFullDelta": options && options.$isFullDelta ? true : false
        };
        if (logger && entityName) {
            temp = " / with " + (metaData ? "meta" : "") + ($resources ? (metaData ? "and data" : "data") : "");
            //logger.trace({"severety": "warn", "toJson": true}, "==> update for " + entityName + " (full delta : " + (update.$isFullDelta ? "true" : "false") + temp + "): ", $resources, metaData);
        }
        if (entityBound) {
            _.each(entityBound, function(item){
                if (metaData) {
    ...
    _getDataField: function(entityName, line, isExternalFmt, onlyValue){
        var dataSet = this._dataset[entityName].data, data = isExternalFmt ? {
            "val": dataSet.v,
            "rcd": null,
            "meta": onlyValue ? null : dataSet
        } : (onlyValue ? dataSet.v : dataSet);
    ...
    _getDataColl: function(entityName, line, isExternalFmt, onlyValue){
        ...
        var dateSet = isLinesColl ? this._dataset[entityName].data : this._dataset[sapUtil.getBlockInst(entityName)].data;
        if (line === undefined || line === null) {
            ...
            if (isLinesColl) {
                ...
                data = isExternalFmt ? {
                    "val": dateSet[line - 1][entityName]
                } : dateSet[line - 1][entityName];
