...
var _instVtbl = {"default": {"getValue": "getDataValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "isDatasetColl": false}};
     ...
     _instVtbl[syraUtil.dataTypes.rtfType] = {"getValue": "_getEditContents", "removeFocus": "resetFocus"};
     _instVtbl[syraUtil.dataTypes.iconType] = {"setData": "_setDataIconField"};
 
/* Misc utilities */
function storeCommit(rule) {
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _bindEvents: function(){
        var self = this, $fusionPageMeta = syraUtil.getFusionPageMeta(self.syraModel);
        ...
        $fusionPageMeta.controller.addGlobalListeners(function (event){
    ...
    _getSession: function() {
        var $fusionPageMeta = syraUtil.getFusionPageMeta(this.syraModel);
        return $fusionPageMeta && $fusionPageMeta.controller ? $fusionPageMeta.controller.getSession() : null;
    },
    ...
    _cureStyle: function(meta, fieldName) {
        ...
        if(sty === undefined || sty === null || sty.length == 0) {
            ...
            if(meta.sty === null) {
                sess.logger.trace(null, "Style '" + sty + "' is not defined. Can't apply style for field '" + fieldName + "'");
                delete meta.sty;
            }
            else {
                ret = true;
    ...
    closeCurrCtxPopup: function(field){
        var popup = this._currCtxPopupField  ? this._currCtxPopupField._popupMenus : null;
        if(popup) {
    ...
    getDatasetValue: function(entityName){
        ...
        if($entity) {
            ...
            return this[_instVtbl[$type] && _instVtbl[$type].getData ? _instVtbl[$type].getData : _instVtbl["default"].getData](entityName);
        }
        else {
            return null;
        }
        ...
        if(style && $field && $field.$$input) {
            ...
            if(sapUtil.focusStyle[style] && sapUtil.focusStyle[style].css) {
                ...
                if(style == sapUtil.focusStyle.fmtErr.code && (sess = this._getSession())) {
                    this._showFieldError($field, [sess.getlabel("error", "12", "Incorrect value. Value does not respect field format or field type")]);
                }
            }
        }
    },
    ...
    setEntities: function(entities, options){
        ...
        _.each(entities, function(data, entityName) {
            ...
            if($entities[entityName]) {
                ...
                // Set data               
                this[_instVtbl[$type] && _instVtbl[$type].setData ? _instVtbl[$type].setData : _instVtbl["default"].setData](entityName, $entities[entityName], $bounds[entityName], data, options);
    ...
    _setDatadField: function(entityName, entity, entityBound, data, options){
        // Adjust "style" meta
        ...
        if(storeCommit(options)) {
            _.extend(this._dataset[entityName].data, data);
        }
        return null;
    },

    _setDataIconField: function(entityName, entity, entityBound, data, options){
        data = sapUtil.adjustIcoFieldData(data);
        this._setDatadField(entityName, entity, entityBound, data, options);
        return null;
    },

    _setDataColl: function(entityName, entity, entityBound, data, options){
        var mvtCode, delta, rcd = {}, commit = storeCommit(options), t1 = (new Date()).getTime(), t2 = 0, styObj, styEx;
        ...
        // Binding...
        console.log("Local Binding Duration: " + ((new Date()).getTime() - t1 - t2));
        if(data.$rcd) {
        ...
        console.log("Total Binding Duration: " + ((new Date()).getTime() - t1));
        return null;
    },
    ...
    _setDataTree: function(entityName, entity, entityBound, data, options){
        // TODO : improve??
        if(entityBound){
            _.each(entityBound, function(item){
    ...
    _getDataColl: function(entityName){
    ...
    dispose: function(){
        ...
        if(this.syraModel) {
            this._unBindEvents();
            if(($fusionPageMeta = syraUtil.getFusionPageMeta(this.syraModel))) {
                $fusionPageMeta.controller.rmGlobalListeners(this);
            }
