...
exports.WinStack = helpers.defineClass(WinStack, null, {
    ...
    getTopWin: function(id, rank, model){
        var idEx = this._getWinIdEx(this.length()), ret = null;
    ...
    unStack: function(id, rank, dispose){
        var idEx = this._getWinIdEx(id), win = this._windows[idEx.id], syraPage = null, i;
        if (win) {
    ...
    applyChange: function(winId, change, options, fromSrv, srvReply){
        var page, ret = null, retEx = {}, idEx = this._getWinIdEx(winId);
        if (!options) {
            options = {};
        }
    ...
    postBuildInit: function(winId){
        var win = this.getWindow(winId), pageFacet, syraModel, sectionMeta, xid, barLists, i, len, metaBar, metaObj = {}, listId, j = 0;
    ...
    setFocus: function(winId, highlight, style, entity){
        var ret = null, page, idEx = this._getWinIdEx(winId), $tab;
        if (this._windows[idEx.id]) {
    ...
    delayedDataBinding: function(options){
        var win, i, syraPage, logger = this._sapController.getlogger(), sess = this._sapController._session;
        if (this._stack) {
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    getEntityEx: function(target){
        var $entityEx, entityEx = null, $entities = this.getProto(), entity = typeof target == 'object' ? target.xid : target, $type = null;
        if ($entities) {
    ...
    delayedDataBinding: function(logger, session, options){
        ...
        var elapse = (new Date()).getTime(), syraElapseEx = {
            "data": 0,
            "meta": 0
        };
    ...
    setDirty: function(entityName, line, dirty){
        var $type, $entityEx = this.getEntityEx(entityName), setDirtyProxy, $bounds = this._fieldsColl.bound, i, len;
    ...
    getDataInputValue: function(entity, nl){
        ...
        var $boundField = null, len, index, getValueProxy, getStateProxy;
    ...
    setFocus: function(highlight, style, entity, nl){
        var fusionController, self = this, $entityEx, $bounds = this._fieldsColl.bound, args, setFocusProxy, setFocusStyleProxy, $screenTab = null, screenTab, evtData, $boundField, len, index, sess;
    ...
    _focusFieldCardView: function(entity, fieldName, index, list){
        ...
        var ret = false, mnus = list.recordsMap[uuid].menuItems;
    ...
    setEntitiesValue: function(entities, options, fromSrv){
        var $entities = this.getProto(), $bounds = this._fieldsColl.bound, screen, page, notifyChange = false, discardRedraw = false;
        if (options) {
    ...
    _setStatusBar: function(status, options, fromSrv){
        var len, i, winMns, ret, item, zone, page = syraUtil.getPage(this.syraModel), autoHide;
    ...
    _setDataColl: function(entityName, entityBound, winProto, recordSet, data, options){
        var mvtCode, delta, rcd = {}, commit = _storeCommit(options), self = this, elapseUnsort = 0, elapse = (new Date()).getTime(), syraElapseEx = {
            "data": 0,
            "mvt": 0,
            "meta": 0
    ...
    _bindDataColl: function(entityName, entityBound, $resources, bindOpts, setDataOpts, logger){
        var delayedBinding, elapse = (new Date()).getTime(), i, len, item, discardUnsort = setDataOpts && setDataOpts.discardUnsort;
    ...
    _getDataColl: function(entityName, line, isExternalWrap, onlyValue, opts){
        var data, isLinesColl = sapUtil.isBlockInst(entityName), dataSet, options, getRawVal, rawVal, i, len, parentEntity, dt;
    ...
    _getStateColl: function(boundColl, entityName, idx, opts){
        var cell, status = null, type, getStateProxy;
    ...
    setFieldError: function(field, errors, raz){
        ...
        var fusionController = syraUtil.getFusionController(this.syraModel), fieldMeta;
        if (field.showErrors) {
...
var _PrivateEvtHandlingWin = helpers.defineClassEx(function(){
}, WinModel, {
    initialize: function(){
        var $fusionPageMeta;
    ...
    trigger: function(event){
        var fusionController, processed = undefined, triggerProxy = EventHandlingCore.EventManagerMembers.trigger, stdHandler = true;
