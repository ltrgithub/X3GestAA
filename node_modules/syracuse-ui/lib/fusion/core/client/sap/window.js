...
WinModel = helpers.defineClass(WinModel, null, {
	...
	getEntityEx: function(target) {
		var $entityEx, entityEx = null,
			...
			$type = null;
		if ($entities) {
			$type = (($entityEx = $entities[(entityEx = sapUtil.getBlockInst(entity))]) && $entityEx && $entityEx.$type == syra_fusion.syraUtil.dataTypes.collectionType) ? $entityEx.$type : ($entities[entity] ? $entities[entity].$type : null);
	...
	getDataInputValue: function(entity, nl, event) {
		...
		if ($entityEx.entity) {
			if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
				...
				//modeType = entityType && (entityType == syra_fusion.syraUtil.dataTypes.stringType || entityType == syra_fusion.syraUtil.dataTypes.dateType) ? sapUtil.fieldModeType.edit : sapUtil.fieldModeType.show;
			...
			if (($entityEx = this.getEntityEx(entity)) && $entityEx.entity) {
				if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
					...
					if (proxyFocusable == null && $entityEx.type == syra_fusion.syraUtil.dataTypes.collectionType && syra_fusion.syraUtil.isEntityPopupCard(entity, this.getProto())) {
						...
						if ((syra_fusion.syraUtil.isEntityMandatory(entity, this.getProto()) || this._hackCardViewFocus[entity]) && sapUtil.isEmptyData(syra_fusion.syraUtil.getEntityType(entity, this.getProto()), this.getDataStoreValue(entity)[index])) {
	...
	setFieldError: function(field, errors, raz) {
		...
		if (fusionController && fusionController._sapController.isCurrIstInputValidate() != !err) {
			if ((fieldMeta = syra_fusion.syraUtil.getMetaFromObject(field)) && sapUtil.cmpIstEx(sapUtil.makeIst(this._idEx.id, fieldMeta.$bind), fusionController._sapController.getCurrInst(), true)) {
