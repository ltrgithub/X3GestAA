...
var _focusStyle = {
    "fmtErr": {"css": "s-field-input-error"},
    "std": {"css": null}
};
var _winK = {
...
var _instVtbl = {"default": {"getValue": "getDataValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "isDatasetColl": false}};
     ...
     _instVtbl[syraUtil.dataTypes.collectionType] = {"getValue": "getDataValue", "setFocus": "setFocus", "setData": "_setDataColl", "getData": "_getDataColl", "isDatasetColl": true, "removeFocus": "resetFocus"};
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _setFocusStyle: function($field, entityEx, entityExType, style, entity) {
        if(style && $field) {
            // Field in list bloc ?
            if ($field.$$input) {
                // Raz
                _.each(_focusStyle, function (stl, name) {
                    if(name != style && stl.css) {
                        $field.$$input.toggleClass(stl.css, false);
                    }
                });
                // Set
                if(_focusStyle[style] && _focusStyle[style].css) {
                    $field.$$input.toggleClass(_focusStyle[style].css, true);
                }
            }
            else {
                // Collection, other... TODO
            }
        }
    },
    ...
    setFocus: function (style, entity) {
        var $entityEx, $bounds = this._fieldsColl.bound, ret = null, args;
        if (!entity || entity.length === 0) {
            ...
            if($entityEx.entity) {
                if($bounds[$entityEx.entity] && $bounds[$entityEx.entity].length > 0) {
                    ...
                    this._setFocusStyle.apply(this, [$bounds[$entityEx.entity][0], $entityEx.entity, $entityEx.type, style].concat(args));
                }
                else
                {
