...
function _getArrayBoundCell($field, entity, nl){
    var boundCell = undefined;
    if (nl > 0 && $field._store._records.length >= nl) {
        boundCell = ($field._store._records[nl - 1].boundFields && $field._store._records[nl - 1].boundFields[entity] ? $field._store._records[nl - 1].boundFields[entity][0] : null);
    }
    return boundCell;
}
...
WinModel = helpers.defineClass(WinModel, null, {
        ...
        if ($entityEx.entity) {
            if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
                ...
                if ((isDirtyProxy = _instVtbl[$entityEx.type] ? (_instVtbl[$entityEx.type].isDirty || _instVtbl["default"].isDirty) : _instVtbl["default"].isDirty) && $boundField[isDirtyProxy] && !$boundField[isDirtyProxy]()) {
                    ...
                    value = $boundField[_instVtbl[$entityEx.type] && _instVtbl[$entityEx.type].getValue ? _instVtbl[$entityEx.type].getValue : _instVtbl["default"].getValue].apply($boundField, arguments);
                }
                ...
                if ((ctxProxy = _instVtbl[entityType] && _instVtbl[entityType].getCtxProps || _instVtbl["default"].getCtxProps)) {
                    ctxProps = this[ctxProxy]($boundField);
                }
            }
        }
    ...
    setFocus: function(style, entity){
        var self = this, $entityEx, $bounds = this._fieldsColl.bound, args, setFocusProxy, setFocusStyleProxy, $screenTab = null, screenTab, evtData, $boundField, len;
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity, serverIndex, isEdit, params){
        var sess, boundCell;
        if (entityExType == syraUtil.dataTypes.collectionType) {
            boundCell = _getArrayBoundCell($field, entity, sapUtil.getNlFromServerIndex(serverIndex));
    ...
    removeFocus: function(entity){
        var $entityEx, $bounds, proxy, $boundField, len;
        if (entity && entity.length > 0) {
            $entityEx = this._getEntityEx(entity);
            ...
            if ($entityEx.entity && $bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
                ...
                if ((proxy = _instVtbl[$entityEx.type] ? _instVtbl[$entityEx.type].removeFocus : _instVtbl["default"].removeFocus) && $boundField[proxy]) {
                    $boundField[proxy].apply($boundField, arguments);
                }
                else {
                    // ... call Fusion default stub...
                    this._removeFocusDef.apply(this, [$boundField, $entityEx.entity, $entityEx.type, entity].concat(Array.prototype.slice.call(arguments, 1)));
                }
            }
        }
    },
    
    _removeFocusDef: function($field, entityEx, entityExType, entity, serverIndex){
        var boundCell;
        if (entityExType == syraUtil.dataTypes.collectionType) {
            boundCell = _getArrayBoundCell($field, entity, sapUtil.getNlFromServerIndex(serverIndex));
            $field = boundCell !== undefined ? boundCell : $field;
    ...
    _getFieldCtxProps: function(boundField){
        var ctx = {};
        if (boundField.getCaretPosition) {
            ctx["crsr"] = boundField.getCaretPosition() + 1;
        }
        // Others properies???
        return ctx;
    },
    
    _getRtfFieldCtxProps: function(boundField){
        return this._getFieldCtxProps(boundField);
    },
