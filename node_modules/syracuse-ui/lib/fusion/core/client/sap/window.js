...
var _winK = {
 ...
 var _instVtbl = {"default": {"setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "isDatasetColl": false}};
     _instVtbl[syraUtil.treeType] = {"setFocus": "focus", "setData": "_setDataTree", "getData": "_getDataTree", "isDatasetColl": false};
     _instVtbl[syraUtil.collectionType] = {"setFocus": "setFocus", "setData": "_setDataGrid", "getData": "_getDataGrid", "isDatasetColl": true};
...
WinModel = helpers.defineClass(WinModel, null, {
    initialize: function(idEx, winDef, syraModel){
        ...
        if(this.syraModel) {
            ...
            this._$$syraPageItem.addClass("s-page-fusion");
            this._bindEvents();
       }
    },
    ...
    _bindEvents: function(){
        var self = this;
        this._$$syraPageItem.on("click keydown dblclick", ".s-page-fusion *", null, function(event){
                console.log("**onPageEvent : " + "'" + self.syraModel.getTitle() + "'" + " [" + event.type + "]");
                var $fusionPageMeta = syraUtil.getFusionPageMeta(self.syraModel), doEvt = $fusionPageMeta.controller.trigger("wdgt.win." + event.type, {
                        "target": self, "type": "event.type", "data": {"nativeEvt": event}}
                    );
    ...
    setFocus: function (entity) {
        var $entityEx, entityEx, $entities = this.getProto(), $bounds = this._fieldsColl.bound, $type, ret = null;
        if(sapUtil.isSpecialScrnId(entity)) {
            // TODO : focus on tab, left list or buttons block ==> Next Field
            if(sapUtil.isButtonBlock(entity)) {
            ...
            $type = (($entityEx = $entities[(entityEx = sapUtil.getBlockInst(entity))]) && $entityEx && $entityEx.$type == syraUtil.collectionType) ? $entityEx.$type : $entities[entity].$type;
            $entityEx = $entityEx || $entities[(entityEx = entity)];
            if($entityEx) {
                if($bounds[entityEx] && $bounds[entityEx].length > 0) {
                    // TODO deal with XBlocks... $bounds[entity][0] or $bounds[entity][n]
                    // TODO : deal with disabled fields!
                    $bounds[entityEx][0][_instVtbl[$type] ? _instVtbl[$type].setFocus : _instVtbl["default"].setFocus].apply($bounds[entityEx][0], arguments);
                }
                else
                {
    ...
    _setDataGrid: function(entityName, entity, entityBound, data){
        ...
        // Finally, apply meta-datas
        if(data.$meta){
            this._dataset[entityName].storeMetaColl(data.$meta);
        }
        console.log("Local Binding Duration: " + ((new Date()).getTime() - todo));
        if(data.$rcd || data.$meta) {
            this._bindDataGrid(entityBound, data.$rcd ? data.$rcd : undefined, rcd, data.$meta);
        }
