    ...
    _setStatusBar: function(status, options, winStack){
        ...
        if(status.ico && (winMns = winStack.winMenus)) {
            for (i = 0, len = status.ico.length; i < len; i++) {
                item =  status.ico[i];
                if(item[0] > 0 && !this._statusBar.ico[item[1]]) {
                    this._statusBar.ico[item[1]] = item[0];
                    ret = winMns.addStatusIco(this.getWinId(), page, item[1], item[0]);
                }
                else if(this._statusBar.ico[item[1]]){
                    ret = winMns.showStatusIco(this.getWinId(), page, this._statusBar.ico[item[1]], !!(item[0] > 0));
                }
            }
        }
        if(status.t) {
