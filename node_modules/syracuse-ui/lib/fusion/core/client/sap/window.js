...
var _instVtbl = {"default": {"getValue": "getDataValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "getCtxProps": "_getFieldCtxProps", "isDatasetColl": false}};
     ...
     _instVtbl[syraUtil.dataTypes.rtfType] = {"getValue": "getInputValue", "removeFocus": "resetFocus", "getCtxProps": "_getRtfFieldCtxProps"};
...
exports.WinStack = helpers.defineClass(WinStack, null, {
    ...
    stack: function(id, winDef, syraPageModel){
        ...
        this._sapController.getSiteController().triggerAdx("sap.win.stacked", {"target": this._windows[idEx.id],
            "type": "opened", "data": {}});
    },
    ...
    unStack: function(id, rank, dispose){
        ...
        if(win){
            ...
            this._sapController.getSiteController().triggerAdx("sap.win.unstack", {"target": this._windows[idEx.id],
                "type": "close",
                "data": {}
            }, dispose);
            ...
            this._sapController.getSiteController().triggerAdx("sap.win.unstacked", {"target": null,
                "type": "closed",
                "data": {"sheet": syraPage}
            }, dispose);
            // TODO : check error?
    ...
    showWinTab: function (winId, tabNum) {
        ...
        if($tab && $tab.$$header) {
            $tab.$$header.trigger("click", {"internal": true});
        }
        ...
        if(this._windows[idEx.id]) {
            if(sapUtil.isSpecialScrnId(entity)) {
                if(sapUtil.isButtonScreen(entity)) {
                    ret = {"ok": true, "srvAct": true, "act": "SRV_CHAMP_SUIVANT", "params": sapUtil.getButtonValue()};
                }
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _bindEvents: function(){
        ...
        // Catch click event on Transition page
        this._$$syraPageItem.on("click", "." + _winK.fusionPageK + " *", null, function(event){
            ...
            if((logger = fusionController ? fusionController.getLogger() : null)) {
                logger.trace({},"**onPageEvent : " + "'" + self.syraModel.getTitle() + "'" + " [" + event.type + "]");
            }
            doEvt = self.onWinEvent(event, fusionController);
    ...
    onWinEvent: function(event, controller){
        ...
        if(!caught && !isField) {
            // General case
            doEvt = controller.triggerAdx("wdgt.win." + event.type, {"target": this, "type": event.type, "data": {"nativeEvt": event}});
        }
        return doEvt;
    },
    ...
    _cureStyle: function(meta, fieldName) {
        var sty = meta.sty, sess, ret = false;
        if(sty === undefined || sty === null || sty.length == 0) {
            ret = true;
        }
        else if((sess = this._getSession())) {
            meta.sty = sess.getStyleClass(meta.sty);
            if(meta.sty === null) {
                sess.logger.trace({"severety": "warn"}, "Style '" + sty + "' is not defined. Can't apply style for field '" + fieldName + "'");
                delete meta.sty;
            }
            else {
                ret = true;
    ...
    _getExtendProps: function(){
        ...
        if(this._mdata && this._mdata.extend) {
            if ((tab = this._mdata.extend.split(";")) && tab.length > 0) {
                props = {"title": tab[0], "width": (tab.length > 1 ? tab[1] : null), "height": (tab.length > 2 ? tab[2] : null)};
            }
        }
        return props;
    },
    ...
    closeCurrCtxPopup: function(obj){
        ...
        if (this._currCtxPopupField)
        {
            if ((popup = syraUtil.getPopupDateFromField(this._currCtxPopupField))){
                ...
                delete this._currCtxPopupField;
                field.$$item.find("[data-s-picker]").trigger("click");
            }
            else if((popup = syraUtil.getPopupCtxFromField(this._currCtxPopupField))) {
                // Contextual popup
                delete this._currCtxPopupField;
    ...
    setDataValue: function(entityName, line, value, options){
        ...
        if (line !== null && line !== undefined && line > 0) {
            ...
            data[collId] = {"$rcd": [{"$uuid": line - 1}]};
            data[collId].$rcd[0][entityName] = value;
        }
        else {
            data[entityName] = {"v": value};
        }
        this.setEntities(data, options);
    },

    getDatasetValue: function(entityName, line, isExternalFmt){
        var $type, $entity = this._dataset[entityName];        
        if($entity) {
            $type = $entity.$type || "default";
            return this[_instVtbl[$type] && _instVtbl[$type].getData ? _instVtbl[$type].getData : _instVtbl["default"].getData](entityName, line, isExternalFmt);
        }
        else {
            return null;
        }
    ...
    getValue: function (entity) {
        var $entityEx, $bounds = this._fieldsColl.bound, ret = null, value = null, modeType = sapUtil.fieldModeType.show, entityType = null, ctxProxy, ctxProps = null;
        ...
        if(value !== null && value !== undefined) {
            ret = sapUtil.getObjFieldValue(value, modeType, ctxProps);
        }
        return ret;
    },
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity, nl, isEdit, params) {
        var sess;
        if(style && $field && $field.$$input) {
            // Raz
            syraUtil.razStyleOnInput($field.$$input, sapUtil.focusStyle, style);
            this._showFieldError($field, null, true);
            // Set
            if(sapUtil.focusStyle[style] && sapUtil.focusStyle[style].css) {
                $field.$$input.toggleClass(sapUtil.focusStyle[style].css, true);
    ...
    _removeFocusDef: function ($field, entityEx, entityType, entity) {
        this._showFieldError($field, null, true);
        if ($field.$$input) {
            syraUtil.razStyleOnInput($field.$$input, sapUtil.focusStyle, "");
        }
    },
    ...
    setData: function(entityName, entityType, entityBounds, data, options){
        ...
        if (!this._dataset[entityName]) {
            (this._dataset[entityName] = new EntityStore()).initialize(entityType, _instVtbl[entityType] ? _instVtbl[entityType].isDatasetColl : _instVtbl["default"].isDatasetColl);
        }
        // Set data
    ...
    _setDatadField: function(entityName, entityBound, data, options){
         // Adjust "style" meta
        this._cureStyle(data, entityName);
        // Bind data & md
    ...
    _setDataColl: function(entityName, entityBound, data, options){
        ...
        var logger, fusionController = syraUtil.getFusionController(this.syraModel), oldFashion = false;
        logger = fusionController ? fusionController.getLogger() : null;
        // Optimization : no data store update for Left list collection!
        commit = sapUtil.isListScreen(entityName) ? false : commit;
        // First, apply delete/insert : TODO ==> remove old fashion!
        if(!oldFashion) {
            syraElapseEx = this._dataset[entityName].applyDataColl(entityName, data, commit, options, function (update) {
                return setUI ? self._bindDataColl(entityName, entityBound, update, {}, null, oldFashion, logger) : 0;
            });
        }
        else {
        if(data.$mvt) {
            _.each(data.$mvt, function(mvt){
                ...
                if(delta && (delta.mvtData.length > 0 || delta.delAll)) {
                    syraElapse += this._bindDataColl(entityName, entityBound, delta.mvtData, {"$isDelta": delta.isDelta, "$range": delta.mvtMeta}, null, oldFashion);                   
                }
            }, this);            
        }
        // Update data in dataset
        if(data.$rcd) {
            rcd["$isDelta"] = this._dataset[entityName].storeDataColl(data.$rcd, commit);
        }
        }
        // Update meta-data in dataset
        if(data.$meta && commit){
            // Warning : deal with style...
            if(data.$meta.sty) {
                for (var i = data.$meta.sty.length - 1; i >= 0; i--) {
                    styEx = data.$meta.sty[i];
                    styObj = {"sty" : styEx[2]};
                    // TODO : enlever le commentaite ou activer!
                    /*if(!this._cureStyle(styObj, styEx[0] + "-" + styEx[1])){
                        data.$meta.sty.splice(i, 1);
                    }
                    else {
                        styEx[2] = styObj.sty;
                    }*/
        ...
        // Binding...
        if(data.$rcd && setUI && oldFashion) {
            syraElapse += this._bindDataColl(entityName, entityBound, data.$rcd, rcd, null, oldFashion);
        }
        if(data.$meta && setUI) {
            syraElapseEx.meta = this._bindDataColl(entityName, entityBound, undefined, null, data.$meta, oldFashion);
        }
        if(logger) {
    ...
    _bindDataColl: function(entityName, entityBound, $resources, record, metaData, oldFashion, logger){
        var elapse = (new Date()).getTime(), update = {"$properties": {}};
        /*if(logger && entityName == "BA") {
            logger.trace({"severety": "warn", "toJson": true}, "==> update : ", $resources);
        }*/
        if(entityBound){
            _.each(entityBound, function(item){
                if(oldFashion) {
                    ...
                    if(metaData) {
                        update.$properties[entityName] = metaData;
                    }
                    if($resources) {
                        update[entityName] = $resources;
                    }
                    item.getPage().applyChange(update);
                    delete update[entityName];
                }
            });
        }
    ...
    _setDataContainer: function(containerName, data){
        ...
        this._cureStyle(data, containerName);
        if(obj) {
    ...
    _getContainerObj: function(containerName){
        ...
        else if (sapUtil.isScreenInst(containerName) && this.$folderTabs){
            $screenTab = _.find(this.$folderTabs, function(tab) {
                var $tabMeta = syraUtil.getMetaFromObject(tab);
    ...
    _getDataField: function(entityName, line, isExternalFmt){
        var dataSet = this._dataset[entityName].data, data = isExternalFmt ? {"val": dataSet.v, "rcd": null, "meta": null} : dataSet;
    ...
    _getDataColl: function(entityName, line, isExternalFmt){
        var data = null, isLinesColl = sapUtil.isBlockInst(entityName);
        var dateSet = isLinesColl ? this._dataset[entityName].data : this._dataset[sapUtil.getBlockInst(entity)].data;
        if (line === undefined || line === null) {
            if(isLinesColl) {
                // Full collection
                data = isExternalFmt ? {"val": dateSet} : dateSet;
                if (this._pendingMeta[sapUtil.getScreenId(entityName)].fields[entityName]) {
    ...
    _getDataTree: function(entityName, line, isExternalFmt){
        ...
        var data = isExternalFmt ? {"val": dataSet.$rcd, "rcd": null, "meta": null} : dataSet;
        if (this._pendingMeta[sapUtil.getScreenId(entityName)].fields[entityName]) {
    ...
    _getFieldCtxProps: function(entityName, entityBound){
        ...
        if(entityBound[entityName][0].getCaretPosition){
            ctx["crsr"] = entityBound[entityName][0].getCaretPosition();
        }
        // Others properies???
        return ctx;
    },
    ...
    _showFieldError: function(field, errors, raz){
        ...
        if (field.showErrors) {
            if((raz || (len = errors.length) == 0) && field.$displayedDiagnoses) {
                ...
                field.showErrors(errors, {"noViewer": true});
            }
        }
    },
    ...
    dispose: function(){
        ...
        if(this.syraModel) {
            ...
            _.each(this.$folderTabs, function ($tabs, idx) {delete this[idx];}, this.$folderTabs);
        ...
        delete this._currCtxPopupField;
        _.each(this._dataset, function (entityStore) {entityStore.dispose();});
