...
exports.WinStack = helpers.defineClass(WinStack, null, {
    ...
    getEntityType: function(winId, xid, opts){
        ...
        return proto ? sapUtil.getEntityType(xid, proto) : null;
    },
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _initDataEx: function(opts){
        var i, datasetArray = this._dataset ? Object.keys(this._dataset) : null, len = datasetArray ? datasetArray.length : 0, proto;
        if (opts && opts.keepDataset && len > 0) {
            proto = this.getProto();
            for (i = 0; i < len; i++) {
                this._dataset[datasetArray[i]].initStore(proto ? proto[datasetArray[i]] : null, this._fmtWrapper.locales);
            }
        }
        else {
            this._dataset = {};
        ...
        _.each(entities, function(data, entityName){
            ...
            if ($entities[entityName]) {
                ...
                if (!this._dataset[entityName]) {
                    (this._dataset[entityName] = new EntityStore()).initialize($type, _instVtbl[$type] ? _instVtbl[$type].isDatasetColl : _instVtbl["default"].isDatasetColl, entityName, $entities[entityName], {"isNodeWS": this._isNodeWS, "locales": this._fmtWrapper.locales});
                }
                // Set data
                entityBounds = $bounds[entityName];
                this[_instVtbl[$type] && _instVtbl[$type]._setDataStore ? _instVtbl[$type]._setDataStore : _instVtbl["default"]._setDataStore](entityName, entityBounds, data, options);
                if (!entityBounds) {
    ...
    _setDataField: function(entityName, entityBound, data, options){
        // Adjust "style" meta
        var newValue = false, newFmt = false, dataPtr = this._dataset[entityName].data, fmt;
        ...
        if(this._isLocalFmt) {
            ...
            if(data.v !== undefined && data.v !== null && fmt) {
                dataPtr._rawV = data.v;
                newValue = true;
            }
            // Formatting data if necessary...
            if(newFmt || newValue) {
                try {
                    data.v = this._fmtWrapper.formatter.formatField({"format": fmt}, dataPtr._rawV, true, true);
                }
                catch (e){
                    data.v = dataPtr._rawV;
                }
            }
        }
        // Bind data & md
    ...
    _setDataIconField: function(entityName, entityBound, data, options){
        ...
        return this._setDataField(entityName, entityBound, data, options);
     },

    _setDataImageField: function(entityName, entityBound, data, options){
