...
var _instVtbl = {"default": {"getValue": "getDataValue", "setFocus": "focus", "setData": "_setDatadField", "getData": "_getDataField", "getCtxProps": "_getFieldCtxProps", "isDatasetColl": false, "isDirty": "dirty"}};
    ...
    _instVtbl[syraUtil.dataTypes.collectionType] = {"getValue": "_getValueCel","setFocus": "setFocus", "setData": "_setDataColl", "getData": "_getDataColl", "isDatasetColl": true, "removeFocus": "resetFocus", "setFocusStyle": "setFocusStyle"};
	...
	_instVtbl[syraUtil.dataTypes.imageType] = {"setData": "_setDataImageField"};
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    _initDataEx: function(opts){
        ...
        if (opts && opts.keepDataset && len > 0) {
            for (i = 0; i < len; i++) {
                this._dataset[datasetArray[i]].initStore();
            }
        }
        else {
            this._dataset = {};
    ...
    getBoundField: function(entityName, opts){
        var boundFld = null, len, $entityEx = this._getEntityEx(entityName), $bounds = this._fieldsColl.bound;
        ...
        if ($entityEx.entity) {
            if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
                ...
                if(isNotModified || !$boundField.$isEditMode) {
                	...
                	getValueProxy=_instVtbl[$entityEx.type] && _instVtbl[$entityEx.type].getValue ? _instVtbl[$entityEx.type].getValue : _instVtbl["default"].getValue;
    ...
    _getValueCel:function($boundField, entity, index){
    	var value=$boundField[_instVtbl["default"].getValue].call($boundField, entity, index), fusionController = syraUtil.getFusionController(this.syraModel), nbDataSet;
    ...
    setEntities: function(entities, options, winStack){
    ...
    setData: function(entityName, entityType, entityBounds, data, options){
        ...
        if (!this._dataset[entityName]) {
            (this._dataset[entityName] = new EntityStore()).initialize(entityType, _instVtbl[entityType] ? _instVtbl[entityType].isDatasetColl : _instVtbl["default"].isDatasetColl, entityName, {"isNodeWS": this._isNodeWS});
        }
        // Set data
    ...
    _setDataIconField: function(entityName, entityBound, data, options){
        ...
        return this._setDatadField(entityName, entityBound, data, options);
     },

    _setDataImageField: function(entityName, entityBound, data, options){
    	var fusionController = syraUtil.getFusionController(this.syraModel),datad={};
    	if(data.stt){datad=sapUtil.getState(data.stt)}
    	if(data.v && data.v!=""){
    		datad.v={'$url': fusionController.getImageUrl(data.v)}
    	}
        return this._setDatadField(entityName, entityBound, datad, options);
    },
