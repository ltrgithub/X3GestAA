...
exports.WinStack = helpers.defineClass(WinStack, null, {
    ...
    showWinTab: function (winId, tabNum) {
        var ret = null, $articleMeta = null, idEx = this._getWinIdEx(winId), tabIst, $tab;
        tabIst = sapUtil.specialScrnId.gfolder + sapUtil.getWinId(tabNum + 1);  // TODO : improve!!!! le calcul de l'instance du tab est approximatif...
        $tab = _.find(this._windows[idEx.id].syraModel.idMap, function(article) {
            return ($articleMeta = syraUtil.getMetaFromObject(article)) && tabIst === $articleMeta[sapUtil.metaNameMap.xid];
        });
        if($tab) {
            ...
            if(sapUtil.isSpecialScrnId(entity)) {
                if(sapUtil.isButtonScreen(entity)) {
                    ...
                    var $articleMeta = null, $tab = _.find(this._windows[idEx.id].syraModel.idMap, function(article) {
                        return ($articleMeta = syraUtil.getMetaFromObject(article)) && sapUtil.isTabScreen($articleMeta[sapUtil.metaNameMap.xid]) && $articleMeta.$opened;
                    });
                    if($tab) {
...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    setCurrCtxPopup: function(popup){
        this._currCtxPopup = popup;
    },

    closeCurrCtxPopup: function(){
        if(this._currCtxPopup) {
            $(document).trigger("click." + this._currCtxPopup._autoCloseNamespace);
        }
    },
    ...
    setFocus: function (style, entity) {
        var self = this, $entityEx, $bounds = this._fieldsColl.bound, ret = null, args, setFocusProxy, setFocusStyleProxy, $entityMeta, $screenTab = null, screenTab, evtData;
        if (!entity || entity.length === 0) {
            ...
            if($entityEx.entity) {
                ...
                screenTab = sapUtil.getScreenId($entityEx.entity);
                $screenTab = _.find(this.syraModel.idMap, function(article) {
                     return ($entityMeta = syraUtil.getMetaFromObject(article)) && screenTab == sapUtil.isFolderTab($entityMeta[sapUtil.metaNameMap.xid]);
                });
                if($screenTab && !$entityMeta.$opened) {
    ...
    _setFocusStyleDef: function($field, entityEx, entityExType, style, entity, nl, isEdit, params) {
        ...
        if(style && $field && $field.$$input) {
            ...
            if(sapUtil.focusStyle[style] && sapUtil.focusStyle[style].css) {
                ...
                if(style == sapUtil.focusStyle.fmtErr.code) {
                    if (($fusionPageMeta = syraUtil.getFusionPageMeta(this.syraModel)) && (sess = $fusionPageMeta.controller.getSession())) {
    ...
    _setDataIconField: function(entityName, entity, entityBound, data, options){
        data = sapUtil._adjustIcoFieldData(data);
    ...
    _setDataColl: function(entityName, entity, entityBound, data, options){
        var mvtCode, delta, rcd = {}, commit = storeCommit(options), t1 = (new Date()).getTime(), t2 = 0;
