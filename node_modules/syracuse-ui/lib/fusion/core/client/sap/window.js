   ...
   _getValueChoiceField:function($boundField, entity, index){
    ...
    _setDataColl: function(entityName, entityBound, winProto, recordSet, data, options){
        ...
        if (logger) {
            ...
            logger.trace({"severety": "info"}, "Binding Duration for '" + entityName + "' ==> Fusion : " + ((new Date()).getTime() - elapse - syraElapse) + " (including Format : " + (this._dataset[entityName]._tgFmt || "unavailable") + ") - Syra : " + syraElapse + " (including apply Mvt : " + syraElapseEx.mvt + ", update data : " + syraElapseEx.data + ", and meta : " + (syraElapseEx.meta || 0) + ")");
        }
        return null;
    },
    ...
    _bindDataColl: function(entityName, entityBound, $resources, options, logger){
        ...
        if (entityBound) {
            _.each(entityBound, function(item){
                if(options.unsortBefore && item.sortCapability && item.sortCapability._$orderBy){
                    // Cancel sort
                    item.helper.fetchClientRecordSet({params:{orderBy:null}})
                }
                item.page.applyChange($resources);
                if(options.unsortAfter && item.sortCapability && item.sortCapability._$orderBy){
                    // Cancel sort...
                    item.helper.fetchClientRecordSet({params:{orderBy:null}})
                }
            });
