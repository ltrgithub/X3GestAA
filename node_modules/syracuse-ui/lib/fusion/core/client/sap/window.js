...
WinModel = helpers.defineClass(WinModel, null, {
    ...
    setDataValue: function(entityName, line, value, options){
        var data = {}, collId, record;
        if (line !== null && line !== undefined && line > 0) {
            // Warning : record must exist! 
            collId = sapUtil.getBlockInst(entityName);
            record = this.getDatasetValue(collId, line, false);
            data[collId] = {"$rcd": [{"$uuid": record.$uuid, "$index": line - 1}]};
    ...
    _setDataColl: function(entityName, entityBound, data, options){
        var mvtCode, delta, rcd = {}, commit = _storeCommit(options), t1 = (new Date()).getTime(), t2 = 0, styObj, styEx,  setUI = (!options || !options.noUI);
        var logger, fusionController;
        ...
        if(data.$mvt) {
            _.each(data.$mvt, function(mvt){
                ...
                if(delta && (delta.mvtData.length > 0 || delta.delAll)) {
                    t2 = (new Date()).getTime();
                    // TODO : remove logger
                    //logger.trace({"severety": "info", "toJson": true}, "Apply 'mvt' on array  " + entityName + ": ", delta.mvtData, {"$isDelta": delta.isDelta, "$range": delta.mvtMeta});
                    this._bindDataColl(entityBound, delta.mvtData, {"$isDelta": delta.isDelta, "$range": delta.mvtMeta}, null);                   
        ...
        if(data.$meta && commit){
            ...
            if(data.$meta.sty) {
                for (var i = data.$meta.sty.length - 1; i >= 0; i--) {
                    ...
                    styObj = {"sty" : styEx[2]};
                    // TODO : enlver le commentaite ou activer!
        ...
        // Binding...
        if(logger) {
            logger.trace({}, "Local Binding Duration: " + ((new Date()).getTime() - t1 - t2));
        }
        if(data.$rcd && setUI) {
            // TODO : remove logger
            //logger.trace({"severety": "info", "toJson": true}, "Apply 'data' on array  " + entityName + ": ", data.$rcd, rcd);
            this._bindDataColl(entityBound, data.$rcd, rcd);
        }
        if(data.$meta && setUI) {
            // TODO : remove logger
            //logger.trace({"severety": "info", "toJson": true}, "Apply 'meta' on array  " + entityName + ": ", data.$meta);
            this._bindDataColl(entityBound, undefined, null, data.$meta);
        }
        if(logger) {
            logger.trace({}, "Total Binding Duration: " + ((new Date()).getTime() - t1));
        }
        return null;
    },

    _bindDataColl: function(entityBound, $resources, record, metaData){
        if(entityBound){
            _.each(entityBound, function(item){
