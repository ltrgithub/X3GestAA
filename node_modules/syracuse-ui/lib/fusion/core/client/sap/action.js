...
exports.ActionsList = helpers.defineClass(ActionsList, null, {
    ...
    setState: function(actions, categorie){
    	var acts = (!Array.isArray(actions) ? [actions] : actions), refresh, ret = null, i, len, action;
    	...
    	for(i = 0, len = acts.length; i < len; i++) {
			...
			if(!shortCut[action.id]){
				...
				this._refreshItems(shortCut[action.id], categorie, true);
			}
    	}
	...
	_refreshItems: function(action, categorie, pendingInst) {
