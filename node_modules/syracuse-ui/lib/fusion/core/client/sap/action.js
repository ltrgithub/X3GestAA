...
exports.ActionsList = helpers.defineClass(ActionsList, null, {
    ...
    getState: function(action, winId, categorie){
    	var actionMap, state = null, actionDesc;
    	if((actionMap = this._actions[categorie || _actionK.srvCatg]) && (actionMap = actionMap[action])) {
    ...
    setState: function(actions, currWinId, categorie, notify, notifyFor){
    	var acts = (!Array.isArray(actions) ? [actions] : actions), ret = null, i, len, action;
    	...
    	for(i = 0, len = acts.length; i < len; i++) {
			...
			if(!shortCut[action.id]){
				...
				shortCut[action.id] = this._initActProps(action.st);
				shortCut[action.id].orphan[currWinId] = {"type": "unknown"};
			}
			else if (shortCut[action.id].st != action.st) {
				shortCut[action.id].st = action.st;
				this._refreshItems(shortCut[action.id], action.id, categorie, false, notify, notifyFor);
			}
			else if(shortCut[action.id]._pendingInst) {
				this._refreshItems(shortCut[action.id],  action.id, categorie, true, notify, notifyFor);
			}
    	}
		...
		_.each([_actionK.localCatg, _actionK.srvCatg], function(catg){
			_.each(this._actions[catg], function(act, actId){
				...
				if(act.orphan && act.orphan[idEx.id] ){
					delete this._actions[catg][actId].orphan[idEx.id];
				}
			}, this);
		}, this);
	},
	...
	_initActProps: function(status) {
		return {"st": status, "insts": {}, "orphan": {}};
	},

	_register: function(inst, winId, actId, type, status, opts, categorie) {
		var shortCut = this._actions[categorie || _actionK.srvCatg], idEx = this._winsStack._getWinIdEx(winId), alreadyRegistered = true;
		if (!shortCut[actId]) {
	...
	_refreshItems: function(action, actId, categorie, pendingInst, notify, notifyFor) {
		var catg = categorie || _actionK.srvCatg, boundActItems = null, syraModel;
