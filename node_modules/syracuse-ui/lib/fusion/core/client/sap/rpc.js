    ...
    _getModifiedArgs: function(){
        ...
        if(this._nbArgs > 0 &&  this._callArgs.length > 0) {
            ...
            for (i = 0; i < this._nbArgs; i++) {
                // TODO : test + improve...
                if(this._inputArgs[i].dim > 0) {
                    for (j = 0, len = this._callArgs.length; j < len; j++) {
                        // TODO : check modif.... +++
                        ouputParams.push({"num": i, "poste": j, "typ": this._inputArgs[i].typ, "resu": "todo"});
                    }
                }
                else {
                    if((!this._inputArgs[i].resu || this._inputArgs[i].resu.length == 0 && this._callArgs[i]) || this._callArgs[i] !== this._inputArgs[i].resu[0]){
