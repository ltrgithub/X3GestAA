            ...
            "stub": function(agent, keys, values) {
                ...
                try {
                    ...
                    var host = {
                        ...
                        url = "http://" + host.name + ":" + host.port + "/sdata/safex3/studio/-/$service/openInEditor?",
                        key, i, m,
                        params = [];
                    if (keys && values) {
...
exports.RemoteCall = helpers.defineClass(RemoteCall, null, {
        ...
        try {
            if(this._internalMethod) {
                ...
                if(temp.length < 1 || !(internalComponent = _stdCallDef[temp[0]])) {
                    ...
                    try {
                        ret = this._execObjMethod(this, internalMethod.stub, this._callArgs);
                    }
                    catch (e) {
                        this._dbgInf.exceptiondetail = e.stack;
                        reply = this.getErrReply({"code": e.errn || 2, "line": 130, "message": e.message, "errt": e.errt || _KerrDefCode.errt.component});
                    }
                }
            }
            else {
                if(!this._object) {
                    ...
                    try {
                        ret = this._execObjMethod(this._object, this._object[this._method], this._callArgs);
                    }
                    catch (e) {
                        this._dbgInf.exceptiondetail = e.stack;
                        reply = this.getErrReply({"code": e.errn || 2, "line": 147, "message": e.message, "errt": e.errt || _KerrDefCode.errt.component});
                    }
                }
            }
    ...
    _execObjMethod: function(object, method, args) {
           ...
           /* for (i = 0; i < this._nbArgs; i++) {
                ...
                if(this._inputArgs[i].dim > 1) {
                    for (j = 0, len = this._callArgs.length; j < len; j++) {
                        ...
                        ouputParams.push({"num": i, "poste": j, "typ": this._inputArgs[i].typ, "resu": this._inputArgs[i].typ == "STRU" ? ("todo_" + i + "-" + j) : 999});
                    }
                }
                else {
