...
exports.Controller = helpers.defineClass(SapController, null, {
	...
	forceValue: function(params, noStoreCommmit, hardForce) {
		var num, data = {}, collId, istType, winModel, record;
		if(params.xid && params.hasOwnProperty("v") && params.v != null) {
			...
			// Set value
			winModel = this._wndwsStack.getWindow(params.win);
			if(!winModel) {
				return null;
			}
			if (params.hasOwnProperty("nl") && params.nl != null && params.nl > 0) {
				collId = sapUtil.getBlockInst(params.xid);
				record = winModel.getDatasetValue(collId, params.nl, false);
				// Error : "$uuid": should be retrieve from entity store, using "params.nl - 1" for index + add $index!
				data[collId] = {"$rcd": [{"$uuid": record.$uuid, "$index": params.nl - 1}]};
				data[collId].$rcd[0][params.xid] = params.v;
			}
			else {
				data[params.xid] = {"v": params.v};
			}
			winModel.setEntities(data, {"noStoreCommmit": noStoreCommmit});
		}
		return null;
 	},
