...
exports.Controller = helpers.defineClass(SapController, null, {
	...
	_initNewCurrIst: function(params) {
		...
		if (!params.ctrlFailed) {
			this._wndwsStack.setDirty(currIst.win, currIst.xid, currIst.nl, false);
		}
	...
	getBoundField: function(currIst) {
		function _getArrayBoundCell($array, entity, idx) {
			return idx !== undefined && idx >= 0 ? $array.getField(entity, idx) : undefined;
		}
		var boundField = this._wndwsStack.getBoundField(currIst.win, currIst.xid);
		var boundCell;

		if (boundField.$field.$type == syraUtil.dataTypes.collectionType) {
			var idx = sapUtil.getIdxFromNL(currIst.nl);

			boundCell = _getArrayBoundCell(boundField, currIst.xid, idx);
			//$field = boundCell !== undefined ? boundCell : $field;
			return boundCell;
		} else return boundField;
	...
	_stubSapMisc_help: function(params) {
		...
		(this.getSiteController()).postHelpRqst("", help[0] || sapUtil.helpHandler.obj.cat, help[1], topWin && topWin.getSyraPage());
		return null;
	},
	...
	_stubSapMisc_openf: function(params) {
		...
		function _openf(protocol, uri, delay) {
			setTimeout(function() {
				var win = window.open(protocol + "://" + uri, "_blank");
				if (!win && !warn) {
		...
		for (i = 0, len = target.length; i < len; i++) {
			_openf(target[i][0], target[i][1], !i ? 1 : (i * 400 + 500));
		}
		return null;
	},
	...
	_stubSapTarget_rpc: function(params, replyCompleteCallback) {
		var self = this,
			errReply, isEvilIst = params.ist ? this._initNewCurrIst(params.ist) : false,
			obj = null;
	...
	_stubSapTarget_selpr: function(params, replyCompleteCallback, dummy, reply) {
		...
		} else if (params && params.$facet == "morePrinter" && this._printerAccess) {
			setTimeout(function() {
	...
	_sapException: function(reply, diagnoses, recovery, localContext, messCallback, replyCompleteCallback) {
		var self = this,
			...
			siteCtrl;
		var err7 = localize.getMessage(this._session ? this._session.cdIsoLang : null, "9"),
			err8 = localize.getMessage(this._session ? this._session.cdIsoLang : null, "10");
		if (this._session) {
