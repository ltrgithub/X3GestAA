...
var _sapAuto = [
    {"root": _sapK.pNodes.sap, "catg": "$4glDiagnoses", "id": null, "subid": null, "exec": "_stub4glDiagnoses", "end": false, "next": true},
    ...
    {"root": _sapK.pNodes.sap, "catg": "jobs", "id": null, "subid": null, "exec": "_stubSapJobs", "end": false, "next": true},
...
var _sessSetRequiredK =  ["ADXLangIso", "mnu", "datePivot", "cdLang", "DefaultDateFormat", "fmtExt", "ADXStyleFile", "dateOrder",
                        "dateSep", "thousandSep", "decimalSep"];
...
exports.isSessStatementsOk = function(reply){
    var settings = exports.getSessSetsFromReply(reply), i = 0, l = _sessSetRequiredK.length, ok = false, srvOpts;
    if(settings) {
        while(i < l && settings[_sessSetRequiredK[i]] !== undefined) {
            i++;
        }
    }
    if(i === l) {
...
function _shouldHighlightTarget(reply, target){
    var srvops = _getSrvopRqst(reply), highlight = true;
    if(srvops) {
...
exports.Controller = helpers.defineClass(SapController, null, {
    ...
    getRequestData: function(inst, action, params) {
        // Init
        var valueEx = null, data = {"act": action, "fld": {"ist": sapUtil.makeIst("", "", 0)}, "param": params, "tech": {}, ctrlResult: {"succeeded": true}}, extraProps;
        ...
        if(!this._siteController._localOpts.fixture || !this._siteController._localOpts.fixture.on) {
            ...
            extraProps.type = this._wndwsStack.getEntityType(this._currCtx.ist.win, this._currCtx.ist.xid);
    ...
    _checkCurrFieldData: function(fieldData, params) {
        var extraProps, ctrlResult = {"succeeded": true}, ist;
        if(fieldData && (ist = fieldData.ist)) {
            ...
            if(ctrlResult.succeeded && !fieldData.notModified && sapUtil.isFieldInst(ist.xid)) {
                this._wndwsStack.setDataStoreValue(ist.win, ist.xid, ist.nl, fieldData.v, {"noUI": true});
            }
        }
        return ctrlResult;
    },
    ...
    _initNewCurrIst: function(params) {
        var isSameIst, editVal, currIst, oldShow, oldFmt, oldIst,field;
        ...
        oldIst = util.duplicateObj(currIst);
        isSameIst = sapUtil.cmpIst(params, currIst, true);
        if(sapUtil.isFieldInst(currIst.xid) && currIst.edit && (!isSameIst || (!sapUtil.isGridInst(currIst) && !params.ctrlFailed))) {
            // Set "show" value in prev field, if ctrl OK (little trick to avoid erase field if same ist in  cell array...)
            oldFmt = this._wndwsStack.getDataFmt(currIst.win, currIst.xid, currIst.nl);
            if(oldFmt) {
                oldShow = this._wndwsStack.getDataStoreValue(currIst.win, currIst.xid, currIst.nl, false, true);
                this._wndwsStack.setDataStoreValue(currIst.win, currIst.xid, currIst.nl, oldShow, {"noStoreCommmit": true, "discardUnsort": true});
            }
        }
        // Init new context
        ...
        if(sapUtil.isFieldInst(currIst.xid)) {
            ...
            currIst.raw = this._wndwsStack.getDataStoreValue(params.win, params.xid, params.nl, false, true, {"rawVal": true});
            if(!isSameIst && currIst.fmt && currIst.raw) {
                // Put edit value in field
                editVal = this.fmtWrapper.rawToEdit(currIst.raw, currIst.fmt);
                if(editVal.succeeded) {
                    this._wndwsStack.setDataStoreValue(currIst.win, currIst.xid, currIst.nl, editVal.edit, {"formatted": true, "noStoreCommmit": true, "discardUnsort": true});
                }
            }
        }
        // Misc controls...
    ...
    forceValue: function(params, noStoreCommmit, hardForce) {
        ...
        if(params.xid && params.hasOwnProperty("v") && params.v != null) {
            ...
            // Set value
            this._wndwsStack.setDataStoreValue(params.win, params.xid, params.nl, params.v, {"noStoreCommmit": noStoreCommmit, "discardUnsort": true});
        }
        return null;
    },

    _getLoadRepParams: function(winParams) {
        var params = {}, specialWin = sapUtil.isSpecialWin(winParams.name), winType = winParams.type, extProps = sapUtil.getWinExtProps(winParams.extend);
        if(specialWin) {
    ...
    processReply: function(reply, workingCtx) {
        try {
            var callNotify, ret = (workingCtx && workingCtx.failed) ? null : this._parseReply(reply, workingCtx || (workingCtx = this.initReplyParseCtx()), _sapAuto), st = true;
            if(ret && !ret.ok){
                 st = false;
            }
            ...
            ret = {"ok": false};
            ret.diagnoses = util.makeDiagnosis(this._session.getlabel("error", "18", localize.getMessage(this._session.cdIsoLang, "13")), 4, ex.message + "\n" + ex.stack, "18", util.cltFusionNice);
            st = false;
        }
        finally {
            if(ret && !ret.ok){
                ...
                if(!workingCtx.throwError) {
                    this._sapException(reply, ret.diagnoses, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_FIN_APPLI"), "param": {"ist": null, "v": ""}}, null, null, workingCtx && workingCtx._techReply && workingCtx._techReply.replyComplete);
                }
                else {
    ...
    _replyAbortBoxEnd: function(params, ctx){
        var status = null, defMess, srvMess;
        if(params.type == 4) {
            ctx.abort = true;
    ...
    _replyAbortClose: function(params, ctx, categorie, reply, reboot){
        ...
        if(ctx.diagnoses.length === 0){
            ...
            //ctx.diagnoses = ctx.diagnoses.unshift(util.makeDiagnosis(this._session.getlabel("error", "4", localize.getMessage(this._session.cdIsoLang, "11")), 3, "sap.Controller._replyAbortClose()", "4", util.backOfficeNice, ctx.reason || ""));
            ctx.diagnoses[0].$details = ctx.reason;
        }
        return null;
    },
    ...
    _stubWinClose: function(params, parseCtx, func, reply) {
        var self = this, closeArray = params.slice(0), closeArraySorted;
        if(params.length == 0) {
            return null;
        }
        // Sort "close" clause in order to process the top stack windows first
        closeArraySorted = closeArray.sort(function(a, b) {return (b.charCodeAt(0) - a.charCodeAt(0));});
    ...
    _stubWinOpen: function(params, parseCtx, func, reply) {
        ...
        function _onWinOpenErr(dgns, errCtx) {
            if(!parseCtx.pollingMode) {
                self._sapException(reply, dgns, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_ABANDON"), "param": {"ist": null, "notModified": true}}, errCtx, null, parseCtx && parseCtx._techReply ? parseCtx._techReply.replyComplete : null);
            }
            else {
            ...
            this._siteController.loadRepresentation(loadParams, null, 
                ...
                function (error){
                    ...
                    if (err && (diagnoses = [util.makeDiagnosesFromOurAjaxErrEx(err)])){
                        if (diagnoses[0].$diagnoses) {
                             diagnoses[0].stackTrace = diagnoses[0].$diagnoses[0].message || "";
                ...
                function (representation){
                    ...
                    if(!specialWin || specialWin.id == sapUtil.specialWindow.formula.id) {
                        self._wndwsStack.setWinMenu(loadParams.id, loadParams.$facet || syraUtil.mapWinTypeToFacet(representation.$prototype.$x3Facet), winId["mnu"], (syraUtil.getProtoFromPage(representation) || representation));
                    }
                },
    ...
    _stubSetWins: function(params, parseCtx, dummy, reply) {
        var ret = null, retEx = {}, oldFmtStgBFmt, fmtStg = this.fmtWrapper.formatter.settings;
        if(fmtStg) {
        ...
        _.each(params, function(data, winId) {
            retEx[winId] = this._wndwsStack.applyChange(winId, data, null, true, reply);
        }, this);
    ...
    _stubSapTarget: function(params, parseCtx, dummy, reply) {
        ...
        if(params.type && _sapK.stubTarget[params.type] && this[_sapK.stubTarget[params.type]]) {
            status = this[_sapK.stubTarget[params.type]](params[params.type], callNotify, reply);
    ...
    _stubSapMisc_openf: function(params) {
        window.open(params.target[0] + "://" + params.target[1], "_blank");
        return null;
    }, 

    stubSapTarget_ist: function(params, replyCompleteCallback, reply) {
    	var ret = null, self = this, dispfmtErrMess = false, isEvilInst, field, highlight;
        if(sapUtil.isGridInst(this._currCtx.ist) && !sapUtil.cmpIstGrid(this._currCtx.ist, params, true)){
       		if((field = this._wndwsStack.getBoundField(this._currCtx.ist.win, sapUtil.getBlockInst(this._currCtx.ist.xid)))){
       			field.unSelectLines();
        	}
        }
    ...
    _stubSapTarget_rpc: function(params, replyCompleteCallback) {
        ...
        function _ackRpc(result) {
            ...
            (self.getSiteController()).postSapRqst(requestActions.getSrvAction("SRV_RESI"), {"notModified": true, "reply": result});
        }
        ...
        obj = sapUtil.isNullIst(params.ist) ? undefined : this._wndwsStack.getBoundField(params.ist.win, params.ist.xid);
    ...
    _stubSapTarget_selpr: function(params, replyCompleteCallback, dummy, reply) {
        var self = this, printerAccess = new Printer.PrinterAccess();
        var winId = sapUtil.getWinId(this._wndwsStack.length() + 1);
        (this.getSiteController()).forceUnlockUI();
        function ackSelpr(action, result) {
            if(replyCompleteCallback) {
                replyCompleteCallback();
            }
            (self.getSiteController()).postSapRqst(requestActions.getProxyAction(action), {"notModified": true, "selpr": result});
        }
        // TODO : just for test!!!!
        if(params && params.$facet == "configPrinter") {
            setTimeout(function(){printerAccess.initialize(params, ackSelpr, self, winId)}, 80);
        }
        else {
            ackSelpr("PRINTER_SEL_CANCEL", {});
        }
        return {"notifyCalled": true};
    },
    ...
    _stubSapJobs: function(params, parseCtx) {
        var result, i, j, k, ii, len, lenj, lenk, lenii, jobsArray = Object.keys(params), jobType, $diag, diagArray, $diagnoses = [], status = null;
                        ...
                        for (ii = 0, lenii = $diag.length; ii < lenii; ii++) {
                            if($diag[ii] && $diag[ii].length > 0) {
                                for (k = 0, lenk = $diag[ii].length; k < lenk; k++) {
                                    if (util.isErrDiagnosis($diag[ii][k].severity, true)) {
    ...
    _sapException: function(reply, diagnoses, recovery, localContext, messCallback, replyCompleteCallback){
        var self = this, boxOpts, boxParams, logger = this.getlogger(), siteCtrl;
