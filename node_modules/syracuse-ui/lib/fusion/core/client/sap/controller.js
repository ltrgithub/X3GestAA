...
exports.Controller = helpers.defineClass(SapController, null, {
    ...
    _checkCurrFieldData: function(fieldData, params) {
        var extraProps, ctrlResult = {"succeeded": true}, localCtrl = this.fmtWrapper.settings.localCtrl, ist;
        if(fieldData && (ist = fieldData.ist)) {
            ...
            // Update sent data # Warning : no update for field with format... because the value could be wrong, and anyway... web server set the value in reply...
            if(((!localCtrl && !fieldData.extra.fmt) || (localCtrl && ctrlResult.succeeded)) && !fieldData.notModified && sapUtil.isFieldInst(ist.xid)) {
                this._wndwsStack.setDataStoreValue(ist.win, ist.xid, ist.nl, localCtrl && ctrlResult.show !== undefined && !this.fmtWrapper.settings.localFormat ? ctrlResult.show : fieldData.v, {"noUI": true});
            }
        }
        return ctrlResult;
    },
    ...
    _initNewCurrIst: function(params) {
        ...
        if(this.fmtWrapper.settings.localFormat && sapUtil.isFieldInst(currIst.xid) && currIst.edit && (!isSameIst || (!sapUtil.isGridInst(currIst) && !params.ctrlFailed))) {
        ...
        currIst = this._currCtx.ist;
        if(this.fmtWrapper.settings.localFormat && sapUtil.isFieldInst(currIst.xid)) {
    ...
    _getLoadRepParams: function(winParams) {
		...
		if( winType === 2 ) params.$facet = syraUtil.pageFacet.modal2;// ack window type 2 
		if( winType === 3 ) params.$facet = syraUtil.pageFacet.modal3;// ack window type 2 
        ...
        if (parseCtx.win2Process) {
            ...
            this._siteController.loadRepresentation(loadParams, null, 
                ...
                function (representation){
                    ...
                    if(!specialWin) {
                        if(winId.name === "CALCULETTE" || winId.name === "EURO") {
                            // Hack for calculette. TODO : to remove after modal implementation!!!
                            representation.$prototype.$x3facet = "$lookup";
                        }
    ...
    _stubSetWins: function(params) {
        var ret = null, retEx = {}, oldFmtStgBFmt, fmtStg = null;
        if(this.fmtWrapper && this.fmtWrapper.settings.localFormat) {
            // Must turn off "skipBadFormat" formatter opt while receiving server datas (Cf.#1110)
            fmtStg = this.fmtWrapper.formatter.settings
