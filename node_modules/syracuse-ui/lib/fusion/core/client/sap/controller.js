...
exports.Controller = helpers.defineClass(SapController, null, {
    ...
    getRequestData: function(inst, action, params) {
        ...
        if(!this._siteController._localOpts.fixture || !this._siteController._localOpts.fixture.on) {
            ...
            // Check data value...
            data.ctrlResult = this.checkCurrFieldData(data.fld, params);
    ...
    checkCurrFieldData: function(fieldData, params) {
        var extraProps, ctrlResult = {"succeeded": true}, localCtrl = this.fmtWrapper.settings.localCtrl;
        if(fieldData && fieldData.ist) {
            // Local control & cie...
            if(localCtrl) {
            ...
            if(!localCtrl || (localCtrl && ctrlResult.succeeded)) {
                this.updateSentData(fieldData, ctrlResult.show !== undefined ? ctrlResult.show : fieldData.v, localCtrl);
            }
        }
        return ctrlResult;
    },

    updateSentData: function(fieldData, v, force) {
        var ist = fieldData.ist;
        if((force || !fieldData.extra.fmt) && sapUtil.isFieldInst(ist.xid)) {
            // Warning : no update for field with format... because the value could be wrong, and anyway... web server set the value in reply...
            this._wndwsStack.setDataStoreValue(ist.win, ist.xid, ist.nl, v, {"noUI": true});
        }
    },
    ...
    _initNewCurrIst: function(params) {
        ...
        if(this.fmtWrapper.settings.localFormat) {
            this._currCtx.ist.fmt = this._wndwsStack.getDataFmt(params.win, params.xid, params.nl);
        }
        return false;
    },    
