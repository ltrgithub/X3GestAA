...
exports.Controller = helpers.defineClass(SapController, null, {
	...
	_stubSapTarget_selpr: function(params, replyCompleteCallback, dummy, reply) {
		var self = this, printerAccess, completeCalled = false, winId = this._wndwsStack.getTopWin(false, true);
		var ret = {"notifyCalled": true};
		(this.getSiteController()).forceUnlockUI();
		...
		if (params && params.$facet == "configPrinter") {
			...
			if(!this._printerAccess) {
				...
				setTimeout(function() {
					self._printerAccess.initialize(params, ackSelpr, self, ++winId);}, 80
				);
			}
			else {
			...
			setTimeout(function() {
				self._printerAccess.openProperties(params, ackSelpr);} , 80
			);
		}
		else {
			ret = null;
	...
	registerLoadedWindow: function(winId, winDef, syraPageModel, initialData) {
		...
		this._wndwsStack.postBuildInit(winId);
		this._wndwsStack.applyChange(winId, {"entities": initialData}, null, true);
	},
