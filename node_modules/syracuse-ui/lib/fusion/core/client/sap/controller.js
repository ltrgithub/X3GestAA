...
exports.Controller = helpers.defineClass(SapController, null, {
    initialize: function(siteController, srvOpts){
    	this._srvOpts = srvOpts && srvOpts.sessinfo ? util.duplicateObj(srvOpts.sessinfo): {};
    	this._siteController = siteController;
    	this._session = null;
   
    	this._winMenus = new MenuManagement();
   		...
   		if(ret) {
   			...
   			this._session.waitSharedRes(100, 30, function(diagnoses) {
   				if(diagnoses && diagnoses.length > 0) {
			   		...
			   		self._winMenus.initialize(self._session, self._session ? self._session.getWinSessMnus() : null);
	...
	forceValue: function(params, noStoreCommmit, hardForce) {
		var num, data = {}, collId, istType = params.win ? sapUtil.getEntityType(params.xid, this._wndwsStack.getWindowProto(params.win)) : null;
		...
		if(params.xid && params.hasOwnProperty("v") && params.v != null) {
			if (params.hasOwnProperty("nl") && params.nl != null && params.nl > 0) {
				...
				// Error : "$uuid": should be retrieve from entity store, using "params.nl - 1" for index + add $index!
				data[_sapK.pSubNodes.entities][collId] = {"$rcd": [{"$uuid": params.nl - 1}]};
				data[_sapK.pSubNodes.entities][collId].$rcd[0][params.xid] = params.v;
			}
			else {
				data[_sapK.pSubNodes.entities][params.xid] = {"v": params.v};
			}
    ...
    _replyAbortClose: function(params, ctx){
    	...
    	if(ctx.diagnoses.length === 0 && (!params.reason || params.reason.length === 0)){
    		...
    		ctx.diagnoses.push(util.makeDiagnosis(this._session.getlabel("error", "3", defMess), 3, "sap.Controller._replyAbortClose()", "3"));
    	}
    	else {
    		defMess = localize.getMessage(this._session.cdIsoLang, "11");
    		ctx.diagnoses = ctx.diagnoses.unshift(util.makeDiagnosis(this._session.getlabel("error", "4", defMess) + " :\n " + params.reason || "", 3, "sap.Controller._replyAbortClose()", "4"));
    	}
		return null;
	},
    ...
    _parseReply: function(reply, workingCtx, rules){
    	...
    	if(i >= len) {
			...
			if(!next){
				...
				status.ok = false;
				status.diagnoses.push(util.makeDiagnosis(this._session.getlabel("error", "5", "Last message sent by server does not contain\"end\" rule."), 3, "sap.Controller._parseReply()", "5"));
	    	}
    	}
    	return status;
	},
	...
	_stubWinClose: function(params, parseCtx, func) {
		...
		_.each(params, function(winId){
			if(!this._wndwsStack.shouldDiscardClose(winId, func.open)) {
				...
				// So, remove "open" clause in reply!!
				delete func.open[(self._wndwsStack._getWinIdEx(winId)).id];
			}
		}, this);	
		return null;
 	}, 
	...
	_stubWinOpen: function(params, parseCtx, func, reply) {
		...
		if (parseCtx.win2Process) {
	    	...
	    	this._siteController.loadRepresentation(loadParams, null, 
			    ...
			    function (error){
				    var diagnoses = util.getDiagnosesFromAjax(error, self._session);
				    util.prependDiagnosisMess (diagnoses, self._session.getlabel("error", "10", "Unexpected error retrieving window description on web server : ") + " [" + winId.name + "] " + "\n\n" + self._session.getlabel("error", "11", "Error detail : "));
			    ...
			    function (representation){
			    	...
			    	var specialWin = sapUtil.isSpecialWin(winId.name), winMenusBar;
					if(!specialWin) {				    	
				    	winMenusBar = self._winMenus.getWinMenuBar(loadParams.id, loadParams.$facet, winId["mnu"], true);
						sapUtil.appendMnus2Syra(representation, winMenusBar, true);
				    	self._winMenus.releaseWinMenu(loadParams.id);
			    	}
			    	else if(specialWin.id == sapUtil.specialWindow.choose.id && winId.appendModel) {				    		
				    	syraUtil.appendProto(representation, winId.appendModel, function (obj, key, value ) {
				    		self._appendChooseModel(obj, key, value);
				    	});
			    	}
			    }
			);
    ...
    _stubSapTarget_ist: function(params, replyCompleteCallback) {
    	...
    	this._currCtx = {"ist": {}};
	 	...
	 	if (params.ctrlFailed && dispfmtErrMess) {
	 		// Display message
	 		params.$diagnoses = params.$diagnoses || [util.makeDiagnosis(this._session.getlabel("error", "12", "Incorrect value, it does not respect field format or field type"), 3, "_stubSapTarget_ist()", 12)];
	    	...
	    	boxOpts.callback = function(resp, closureMode) {
				self._wndwsStack.setFocus(params.win, sapUtil.focusStyle.fmtErr.code, params.xid, params.nl || 1, (params.edit === null || params.edit === undefined ? true : params.edit), params);
   ...
   _sapException: function(reply, diagnoses, recovery, localContext, messCallback, replyCompleteCallback){
		...
		boxParams = util.diagnosesToOurErr(diagnoses);
		msgBox.setMsgBoxDefaultProp(boxParams, 4, 50, err8, err7);
     	boxOpts = msgBox.getMsgBoxOpts(boxParams);
    	boxOpts.callback = function(resp, closureMode) {
    		if (replyCompleteCallback) {
    			replyCompleteCallback();
    	...
    	if(this._session) {
    		this._session.logger.debug({"severety": "error"}, "Error notification from 'sapController._sapException()'. Stack and diagnoses below :");
    		this._session.logger.debug({"toJson": true},  diagnoses);
    	}
    	setTimeout(function(){syraUtil.msgBox(boxOpts);}, 50);
    },
    ...
    _appendChooseModel: function (obj, key, value) {
    	var fmt = null;
        if (key == "tit") {
            obj.$title = value;
        }
    ...
    dispose: function(){
    	...
    	if(this._winMenus) {
    		this._winMenus.dispose();
    		delete this._winMenus;
    	}
    	if(this._session){
