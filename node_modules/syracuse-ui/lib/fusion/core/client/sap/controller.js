...
exports.Controller = helpers.defineClass(SapController, null, {
	...
	initSurvivalMode: function(cdLang) {
		var self = this;
		this._session = {
			"cdIsoLang": cdLang || syra_site.fusionGateway.defLang
		};
		this._session.getlabel = function(catg, code, dflt) {
			// Just one category supported : "error"
			var messId = parseInt(code, 10) + 1000;
			return syra_site.fusionGateway.getMessage(self._session.cdIsoLang, messId) || dflt;
		};
	},
	...
	processReply: function(reply, workingCtx) {
		try {
			...
			ret.diagnoses = util.makeDiagnosis(this._session.getlabel("error", "18", syra_site.fusionGateway.getMessage(this._session.cdIsoLang, "13")), 4, ex.message + "\n" + ex.stack, "18", util.cltFusionNice);
			st = false;
		} finally {
			if (ret && !ret.ok) {
	...
	_replyAbortBoxEnd: function(params, ctx) {
		...
		if (params.type == 4) {
			...
			defMess = syra_site.fusionGateway.getMessage(this._session.cdIsoLang, "12");
	...
	_sapException: function(reply, diagnoses, recovery, localContext, messCallback, replyCompleteCallback) {
		var self = this,
			...
			siteCtrl;
		var err7 = syra_site.fusionGateway.getMessage(this._session ? this._session.cdIsoLang : null, "9"),
			err8 = syra_site.fusionGateway.getMessage(this._session ? this._session.cdIsoLang : null, "10");
		if (this._session) {
