...
var _sapAuto = [
	...
	{"root": _sapK.pNodes.sap, "catg": "prev", "id": null, "subid": null, "exec": "_stubSetPrevValue", "end": false, "next": true},
...
exports.Controller = helpers.defineClass(SapController, null, {
    initialize: function(siteController, srvOpts){
		...
		this._ackcallStack = [];
	   	this._currCtx = {"ist": sapUtil.makeIst("A", "", 0), "ctxMenu":[], "v": null, "tabIdx": null};
	...
	getRequestData: function(inst, action, params) {
		// TODO : le test sur "params.v" n'est pas terrible (utilisé en réponse à une msgbox)... voir si on peut faire mieux
		var data = {"act": action, "fld": {"ist": sapUtil.makeIst("", "", 0), "v": params ? params.v : null, "ctx": {"cusr": null}}, "param": params, "tech": {}};

		// Hack
		if(!this._siteController._localOpts.fixture || !this._siteController._localOpts.fixture.on) {
			data.fld.ist = sapUtil.makeIst(this._currCtx.ist.win, this._currCtx.ist.xid, this._currCtx.ist.nl);
				if(!sapUtil.isListTab(this._currCtx.ist.xid) && this._wndwsStack.getWindow(this._currCtx.ist.win) && ((this._wndwsStack.getWindow(this._currCtx.ist.win)).getDatasetValue(this._currCtx.ist.xid))) {
					data.fld.v = ((this._wndwsStack.getWindow(this._currCtx.ist.win)).getDatasetValue(this._currCtx.ist.xid)).val;
			}
		}
		// Fin HACK
	...
	getCurrInst: function() {
    	return this._currCtx.ist;
    },

	setCurrTab: function(tabIdx) {
		this._currCtx.tabIdx = tabIdx;
    },
		...
		if (parseCtx.win2Process) {
			...
			if(this._wndwsStack.shouldDiscardOpen(parseCtx.wndwsId[parseCtx.win2Process.id], func.close)) {
		    	...
		    	this._siteController.loadRepresentation({"id": parseCtx.wndwsId[parseCtx.wndwPending.id], "name": winId.name, "mainPage": !!(this._wndwsStack.length() === 0)}, null, 
				    ...
				    function (error){
				    	// TODO : decorate error with window name, etc... and package error in diagnosis schema!
				    ...
				    function (proto){
				    	...
				    	//TODO : hack!!
				    	winId.mnu = [{"typ": 0,"act": requestActions.getSrvAction("SRV_FIN_APPLI"),"acl": null,"txt": "Close Appli","rnk": "10","id": "21212511111","fid": "51584"},
				    		{"typ": 0,"act": 145,"acl": null,"txt": "Exporter...","rnk": "10","id": "11332","fid": "5154"},
	...
	_stubSetPrevValue: function(params) {
		// Previous value : http://syracuse/w/index.php/Request_Reply_Fusion#sap.prev
		var data = {}, collId;
		if(params.xid && params.v) {
			if (params.nl && params.nl > 0) {
				collId = sapUtil.getBlockInst(params.xid);
				data[collId] = {"$rcd": [{"$i": params.nl}]};
				data[collId].$rcd[0][params.xid] = params.v;
			}
			else {
				data[params.xid] = {"v": params.v};
			}
			this._wndwsStack.applyChange(params.win, data)
		}
		return null;
 	},
	...
	_stubSapTarget: function(params) {
		...
		if(params.type && _sapK.stubTarget[params.type] && this[_sapK.stubTarget[params.type]]) {
			...
			status = {"ok": false, "diagnoses" : [util.makeDiagnosis(this._session.getlabel("error", "6", "Unable to set the focus because the target instance is not defined in the message server response."), 3, "sap.Controller._stubSapTarget()", "6")]};
		}
		return status;
 	}, 
    ...
    _stubSapTarget_ist: function(params) {
    	...
    	this._currCtx.ctxMenu = params.acts && params.acts.length > 0 ? _.map(params.acts, function(act, idx){
	 	...
	 	delete params.acts;
	 	this._currCtx.v = params.v || null;
	 	delete params.v;
	 	_.each(params, function(value, name){this[name] = value;}, this._currCtx.ist);
 		//this._stubSetPrevValue(params);	// TODO
 		ret = this._wndwsStack.setFocus(params.win, params.xid, params.nl || 1, (params.edit === null || params.edit === undefined ? true : params.edit), params);
 		if(ret && ret.ok && ret.srvAct) {
	    	setTimeout( function(){
				(self.getSiteController()).sendSapRqst(requestActions.getSrvAction(ret.act), {"v": ret.v});
		    }, 50);
		    return null;
		}
		else if(ret && !ret.ok) {
    ...
    _stubSapTarget_box: function(params) {
    	...
    	boxOpts.callback = function(resp, closureMode) {
    		(self.getSiteController()).sendSapRqst(requestActions.getSrvAction("SRV_CHAMP_SUIVANT"), {"v": msgBox.getMsgBoxRet(resp, closureMode, params)});
    	};
   ...
   _sapException: function(reply, diagnoses, recovery, localContext){
   	var todoAvirer = recovery;
     	if(!recovery || todoAvirer) {
			...
			var boxParams = util.diagnosesToOurErr(diagnoses), boxOpts, self = this;
    		...
    		this["_sapRecovery" + recovery.typ](reply, recovery);
    	}
    },
