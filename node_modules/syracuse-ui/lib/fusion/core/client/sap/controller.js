...
exports.Controller = helpers.defineClass(SapController, null, {
    ...
    processReply: function(reply, workingCtx) {
    	...
    	if(ret && !ret.ok){
    		...
    		this._sapException(reply, ret.diagnoses, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_FIN_APPLI"), "param": {"ist": null, "v": ""}});
    		st = false;
    	}
	...
	_stubSapTarget: function(params, parseCtx) {
		...
		if(params.type && _sapK.stubTarget[params.type] && this[_sapK.stubTarget[params.type]]) {
			...
			// Process
			status = this[_sapK.stubTarget[params.type]](params[params.type], function () {
				// TODO : delegate pending + mettre aussi sur le "processReply()" sur cas d'erreur, + autre stubs de fin (reboot et close)
				if(parseCtx._techReply) {
					parseCtx._techReply.pending = false;
				}
			});
		}
		else {
