    ...
    _parseReply: function(reply, workingCtx, rules){
    	...
    	if(i >= len) {
			...
			if(!next){
				...
				status.ok = false;
				status.diagnoses.push(util.makeDiagnosis(this._session.getlabel("error", "5", "Last message sent by server does not contain\"end\" rule."), 3, "sap.Controller._parseReply()", "5", util.cltFusionNice));
	    	}
    	}
    	return status;
	},
	...
	_stubWinOpen: function(params, parseCtx, func, reply) {
		...
		if (parseCtx.win2Process) {
	    	...
	    	this._siteController.loadRepresentation(loadParams, null, 
			    ...
			    function (error){
				    var diagnoses = util.makeDiagnosis(self._session.getlabel("error", "10", "Unexpected error retrieving window description on web server : ") + " [" + winId.name + "].", 3, null, "10", util.cltFusionNice, null, null, util.getDiagnosesFromAjax(error, self._session));
