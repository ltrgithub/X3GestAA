...
exports.Controller = helpers.defineClass(SapController, null, {
	...
	getRequestData: function(inst, action, params) {
		// Init
		var data = {"act": action, "fld": {"ist": sapUtil.makeIst("", "", 0), "v": null, "ctx": {"cusr": null}}, "param": params, "tech": {}};
		...
		if(!this._siteController._localOpts.fixture || !this._siteController._localOpts.fixture.on) {
			...
			// Fin HACK
	 		data.fld.v = params && params.hasOwnProperty("v") ? params.v : this._wndwsStack.getValue(this._currCtx.ist.win, this._currCtx.ist.xid, this._currCtx.ist.nl || 1);
	...
	getRequestTech: function(tech) {
		if (document[sapUtil._fNS] && document[sapUtil._fNS].hasOwnProperty("srvTrc")) {
    		tech.trace = tech.trace || {};
    		tech.trace.on =  document[sapUtil._fNS]["srvTrc"];
    ...
    _stubSapTarget_ist: function(params) {
    	...
    	this._currCtx.ctxMenu = params.acts && params.acts.length > 0 ? _.map(params.acts, function(act, idx){
 		...
 		if(ret && ret.ok && ret.srvAct) {
	    	setTimeout( function(){
				(self.getSiteController()).postSapRqst(requestActions.getSrvAction(ret.act), {"v": ret.v});
		    }, 50);
		    return null;
		}
		else if(ret && !ret.ok) {
    ...
    _stubSapTarget_box: function(params) {
    	...
    	boxOpts.callback = function(resp, closureMode) {
    		(self.getSiteController()).postSapRqst(requestActions.getSrvAction("SRV_CHAMP_SUIVANT"), {"ist": null, "v": msgBox.getMsgBoxRet(resp, closureMode, params)});
    	};
