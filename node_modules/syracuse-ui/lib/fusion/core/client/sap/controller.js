...
exports.Controller = helpers.defineClass(SapController, null, {
    ...
    _getLoadRepParams: function(winParams) {
        ...
        if(extProps && extProps.transaction) {
            params.$variant = "trans," + extProps.transaction;
        }
        return params;
    },
    ...
    processReply: function(reply, workingCtx) {
        try {
            ...
            var ret = (workingCtx && workingCtx.failed) ? null : this._parseReply(reply, workingCtx || (workingCtx = this.initReplyParseCtx()), _sapAuto), st = true, diag;
            if(ret && !ret.ok){
            ...
            diag = util.makeDiagnosis(this._session.getlabel("error", "18", localize.getMessage(this._session.cdIsoLang, "13")), 4, ex.message + "\n" + ex.stack, "18", util.cltFusionNice);
            if (!workingCtx.throwError) {
                this._sapException(null, diag, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_FIN_APPLI"), "param": {"ist": null, "v": ""}}, null, null, callNotify);
            }
            else {
                workingCtx.failed = true;
                workingCtx.diagnoses.push(diag);
            }
            st = false;
        }
        finally {
    ...
    _stubWinOpen: function(params, parseCtx, func, reply) {
        ...
        function _onWinOpenErr(dgns, errCtx) {
            if(!parseCtx.pollingMode) {
                self._sapException(reply, dgns, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_ABANDON"), "param": {"ist": null, "v": ""}}, errCtx, null, parseCtx && parseCtx._techReply ? parseCtx._techReply.replyComplete : null);
            }
            else {
    ...
    _stub4glDiagnoses: function(params) {
        ...
        if(params && ourSite && ourSite.showDiagnoses) {
            ourSite.showDiagnoses({"$diagnoses": params});            
        }
        return null;
    },
