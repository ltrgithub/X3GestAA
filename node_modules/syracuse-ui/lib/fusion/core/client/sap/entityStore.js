...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
            ...
            if(this._formatter && this._proto.$item && (itemsProps = this._proto.$item.$properties)) {
                ...
                for (i = 0, len = itemsPropsArray.length; i < len; i++) {
                    ...
                    if(KisTypeAcceptFmt[entityType]) {
                        ...
                        if(protoFmt || defFmt) {
                            this.metaFmt[itemsPropsArray[i]] = {"fmt": protoFmt || defFmt, "cellsSet": false, "_fmtEmpty": !!KisTypeAcceptFmtED[entityType]};
                        }
                    }
                }
            }
    ...
    _storeDataColl: function(newData, commit, options){
        var isDelta = false, jd, lend, len, rcdArray, i, j, dataFmt, niemeDataPtr, skip;
        if (!commit) {
                ...
                if (commit || this._formatter) {
                    ...
                    for (j = 0, len = rcdArray.length; j < len; j++) {
                        if(!this._formatter || !(dataFmt = this.getMetaFmtColl(rcdArray[j], i))) {
                            ...
                            try {
                                if(commit) {
                                    niemeDataPtr[rcdArray[j]] = skip ? newData[jd][rcdArray[j]] : this._formatter.formatField({"format": dataFmt}, newData[jd][rcdArray[j]], true, false);
