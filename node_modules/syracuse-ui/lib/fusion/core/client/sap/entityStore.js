...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
	...
	isSupportPartialChange: function() {
		...
		if (this._isColl) {
			...
			if (this._proto.$decorator && this._proto.$decorator.$stereotype) {
				ret = false;
			}
		}
		return ret;
		...
		if ((state.$isDisabled != undefined || state.$isReadOnly != undefined) && itemsPropsArray) {
			...
			for (i = 0, len = itemsPropsArray.length; i < len; i++) {
				if ((newS = syraUtil.purgeStateMeta(itemsProps[itemsPropsArray[i]], metaState)) !== null) {
					if (isHiddenState) {
						newS = sapUtil.removeStateMask({
							"$isHidden": state.$isHidden
						}, newS);
						if (!state.$isHidden) {
							newS -= sapUtil.statusFlag.show;
						}
					}
					if (newS > 0) {
			...
			if (isHiddenState) {
				newMetaState.push([this.entityName, 0, sapUtil.setState({
					"$isHidden": state.$isHidden
				}, 0)]);
			}
		}
			...
			for (i = 0, lenI = metaArray.length; i < lenI; i++) {
				...
				if (doublon[dblKey] != undefined) {
					...
					if (type == _datasetK.metaType.state) {
						...
						purged[purged.length - 1].val = sapUtil.completeState(metaArray[doublon[dblKey]][2], metaArray[i][2]);
					}
				}
				doublon[dblKey] = i;
				line = false;
	...
	getFmtColl: function(property, idx) {
		...
		if (this.data.length == 0 && metaFmtProp) {
	...
	sortDataColl: function(orderBy, raw, opts) {
		...
		function getSortType(binding) {
			...
			if (!rawEx) {
				...
				if (collProps[binding].$type == syraUtil.dataTypes.decType || collProps[binding].$type == syraUtil.dataTypes.realType) {
					intg = false;
					flt = true;
				}
				return {
