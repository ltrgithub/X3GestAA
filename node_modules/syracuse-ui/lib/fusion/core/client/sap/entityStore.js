...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    ...
    initStore: function(regOpts){
        ...
        var defFmt, KdefFmt = ":", KdefDateFmt = "D:" + ((regOpts ? regOpts.dateFormat : null) || "MM[/]DD[/]YY"), ignoreFmt = false;
        if (this._isColl) {
            this.data = [];
            this.meta = {};
            this.metaFmt = {};
    ...
    _fillTemplateColl: function(options){
        ...
        for (i = 0, ii = this.data.length; i < ii; i++) {
            coll[i] = {"$uuid": this.data[i].$uuid, "$serverIndex": i};
        }
        return coll;
    },
    ...
    applyDataCollEx: function(data, commit, options, callbackBind, callbackUnsort){
        ...
        if (nbInsrt > 0) {
            ...
            if (!(isEmpty = this._$empty) && _updateCollOneByOne(this.data.length, nbInsrt)) {
                ...
                syraElapse.data += callbackBind(collMvt, {"$isPartialDelta": partialUpdate});
        ...
        if (data.$rcd && !updated) {
            if (this._storeDataColl(data.$rcd, commit, options) && !leftListColl && data.$rcd.length !== this.data.length) {
                // By delta
                syraElapse.data += callbackBind(data.$rcd, {"$isPartialDelta": data.$rcd.length > 1 ? true : false, "unsortBefore": false});
            }
            else {
                // Full
                syraElapse.data += callbackBind(leftListColl ? data.$rcd : this.data, {"$isPartialDelta": false, "unsortBefore": false});
            }
            _razIndexPropr(this.data);
        }
        return syraElapse;
    ...
    applyCollMvt: function(mvt, oneByOne, options, callbackBind, callbackUnsort){
        ...
        if (mvtCode === sapUtil.dataSetMvt.delAll || (lenCollAfter === 0 && mvtCode === sapUtil.dataSetMvt.del)) {
            syraElapse += callbackBind([], {"$isPartialDelta": false, "unsortAfter": true});
        }
        else if (lenCollAfter == 1 && mvtCode == sapUtil.dataSetMvt.del) {
                // Optimisation in Syracuse protocole...
                delete this.data[0].$index;
                syraElapse += callbackBind([this.data[0]], {"$isPartialDelta": false});
        }
        else {
            if (oneByOne) {
                _.each(delta.mvtData, function(rcd){
                    syraElapse += callbackBind([rcd], {"$isPartialDelta": false});
                });
            }
            else {
                ...
                syraElapse += callbackBind(collMvt, {"$isPartialDelta": partialDelta});
                if (!partialDelta) {
                    _razIndexPropr(this.data);
                }
            }
    ...
    applyMvt: function(mvt, start, count){
        ...
        if (mvt == sapUtil.dataSetMvt.delAll) {
            ...
            for (i = start - 1, j = 0; i < start - 1 + count; i++) {
                ...
                this.data.splice(i, 0, {"$uuid": _uuid, "$serverIndex": i});
                renumber = bAtEnd ? 0 : 1;
                if(this._fmtWrapper) {
    ...
    _storeDataColl: function(newData, commit, options){
        var isDelta = false, jd, lend, len, rcdArray, i, j, dataFmt, niemeDataPtr, skip, yetFormatted, rawValueEx, _tgInter;
        if (!commit) {
        ...
        _.each(newMeta, function(newMetaColl, type){
            if (this.meta[type].length > 0) {
                _.each(newMetaColl, function(newMeta){
                    ...
                    // Bonus pack: on "state" meta, set global "isDisabled" flag... like in cs client...
                    if(type === _datasetK.metaType.state) {
                        state = sapUtil.getState(newMeta[2]);
                        this.meta._fullyDisabled = gridInst && newMeta[1] == 0 && (state.$isDisabled || state.$isReadOnly) ? true : false;
                    }
                }, this);
            }
            else {
    ...
    getMetaColl: function (update, meta, session, options){
        ...
        var arrayMeta, colMeta, rowMeta, propMember = "$properties", rowMetaMap = {}, ptr, hackDch = true;
        // Helper
    ...
    getColumnsDataColl: function(property, raw, sort){
        var i, len, dataCol = [], rawEx = !!(raw && this.metaFmt[property] && this._fmtWrapper.isThickFormat(this.metaFmt[property].fmt));
        if(!sort) {
            if(rawEx) {
            ...
            dataCol = this.sortDataColl([{"binding": property, "asc": sort.asc}], raw, sort);
        }
        return dataCol;
    },

    getLineDataColl: function(idx, raw){
        var itemsProps = this._proto.$item.$properties, itemsPropsArray, i, len, record = null, rawEx, property;
    ...
    sortDataColl: function(orderBy, raw, opts){
        var sortCol, desc, self = this, sortProps, lenk = orderBy.length, dLength = this.data.length, slice = null, collProps = this._proto.$item.$properties;
        var zero = "0", i, k, nieme, len, dataCol = [], lastRcd = null, lastRcdArray;
        ...
        if(sapUtil.isEmptyGridDataRecordEx(collProps, lastRcd || nieme)) {
            dLength--;
            slice = -1;
        }
        function getSortType(binding) {
            var metaFmt = self.metaFmt[binding], intg = false, flt = false, rawEx = !!(raw && metaFmt &&  self._fmtWrapper.isThickFormat(metaFmt.fmt));
            if(binding == "$serverIndex") {
                return {"intg": true, "flt": false, "rawEx": false};
            }
            else if(!rawEx) {
                ...
                return {"intg": intg, "flt": false, "rawEx": false};
            }
            else {
                ...
                return {"intg": intg, "flt": flt, "rawEx": true};
            }
        }
       ...
       if(opts && opts.onlyColumn) {
            ...
            if(opts.onlyData) {
                ...
                if(sortProps.rawEx) {
                    for (i = 0; i < dLength; i++) {
                        nieme = this.data[i];
                        dataCol[i] = {"v": nieme.$rawV[sortCol].v, "$value": nieme[sortCol], "$serverIndex": nieme.$serverIndex, "$uuid": nieme.$uuid};
                    }
                }
                else {
                    for (i = 0; i < dLength; i++) {
                        nieme = this.data[i];
                        dataCol[i] = {"v": nieme[sortCol], "$value": nieme[sortCol], "$serverIndex": nieme.$serverIndex, "$uuid": nieme.$uuid};
                    }                    
                }
                dataCol.sort(function(a, b) {
            ...
            if(slice != null) {
                ...
                dataCol.push(opts.onlyData ? nieme[sortCol] : {"v": nieme[sortCol], "$value": nieme[sortCol], "$serverIndex": dLength, "$uuid": nieme.$uuid});
            }
        }
