...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
	...
	initStore: function(regOpts) {
		var KfmtMeta = _datasetK.metaType.format, occur, ro,
	...
	_cureMetaStateOnArrayScope: function(metaState) {
		...
		if(state.$isDisabled != undefined || state.$isReadOnly != undefined && itemsPropsArray) {
			...
			if (roCount > 0){
				// Ok, should set global "isDisabled" flag (like in cs client)?
				if(state.$isDisabled === true || state.$isReadOnly === true) {
		...
		_.each(newMeta, function(newMetaColl, type) {
			...
			if (type === _datasetK.metaType.state) {
				...
				for (i = len - 1; i >= 0; i--) {
					...
					if(!gridInst && occur[1] == 0 && (occur[2] = syraUtil.purgeStateMeta(this._proto.$item.$properties[occur[0]], occur[2])) == null) {
						discard = true;
					}
