...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
	...
	sortDataColl: function($syraOrderBy, raw, opts) {
		var orderBy = [];
		if ($syraOrderBy && $syraOrderBy.length > 0) {
			for (var ii = 0, jj = $syraOrderBy.length; ii < jj; ii++) {
				orderBy.push({
					binding: $syraOrderBy[ii].binding,
					asc: $syraOrderBy[ii].descending !== true
				});
			}
		} else {
			orderBy.push({
