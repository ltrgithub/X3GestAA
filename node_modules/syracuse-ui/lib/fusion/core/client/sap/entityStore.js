...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    ...
    applyDataCollEx: function(entityName, data, commit, options, callback){
        ...
        var leftListColl = options && options.lf, collMvt = null, fullUpdate, syraElapse = {
            "data": 0,
            "mvt": 0
        };
        ...
        if (data.$rcd && !updated) {
            ...
            if (this.storeDataColl(data.$rcd, commit) && !leftListColl && data.$rcd.length !== this.data.length) {
                ...
                // Full
                syraElapse.data += callback(leftListColl ? data.$rcd : this.data, {
    ...
    applyCollMvt: function(entityName, mvt, oneByOne, options, callback){
        ...
        if (mvtCode === sapUtil.dataSetMvt.delAll || (lenCollAfter === 0 && mvtCode === sapUtil.dataSetMvt.del)) {
            ...
            if (lenCollAfter == 1 && mvtCode == sapUtil.dataSetMvt.del) {
                ...
                syraElapse += callback([this.data[0]], {
                    "$isPartialDelta": false
                });
            }
            else {
...
// Get meta-data object accordong to Syracuse protocol
function getMetaColl(arrayName, update, meta, session, options){
    ...
    if (!update[propMember][arrayName]) {
        update[propMember][arrayName] = {};
    }
    if (!update[propMember][arrayName].$item) {
        update[propMember][arrayName].$item = {};
    }
    if (!update[propMember][arrayName].$item[propMember]) {
        update[propMember][arrayName].$item[propMember] = {};
    }
    if (!update[arrayName]) {
        update[arrayName] = [];
    }
    arrayMeta = update[propMember][arrayName];
    colMeta = update[propMember][arrayName].$item[propMember];
    rowMeta = update[arrayName];
    // Set meta Title     
    ...
    _.each(meta[_datasetK.metaType.state], function(stt){
        var line = stt[1] - 1, st = sapUtil.getState(stt), ptr = null;
    ...
    _.each(meta[_datasetK.metaType.style], function(sty){
        var line = sty[1] - 1, styEx = {}, metaName = "$style", ptr = null, styPropr = _datasetK.metaType.style;
        styEx[styPropr] = sty[2];
        if (sapUtil.cureStyle(styEx, arrayName, session)) {
...
exports.getSyraMetaColl = getMetaColl;
