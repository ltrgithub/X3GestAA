...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    initialize: function(internalType, isCollection, name, proto, formatter, opts){
        ...
        this._proto = proto;
        this.initStore(opts ? opts.locales : null);
    },

    initStore: function(regOpts){
        var KfmtMeta = _datasetK.metaType.format, itemsProps, i, len, itemsPropsArray, entityType, protoFmt;
        ...
        if (this._isColl) {
            ...
            if(this._formatter && this._proto.$item && (itemsProps = this._proto.$item.$properties)) {
                ...
                for (i = 0, len = itemsPropsArray.length; i < len; i++) {
                    ...
                    if(KisTypeAcceptFmt[entityType]) {
                        ...
                        if(protoFmt || defFmt) {
                            this.metaFmt[itemsPropsArray[i]] = {"fmt": protoFmt || defFmt, "cellsSet": false};
                        }
                    }
                }
            }
            ...
            this.data = {};
            if(this._formatter && KisTypeAcceptFmt[this.$type]) {
                defFmt = !this._proto.$X3Fmt && this.$type == syraUtil.dataTypes.dateType ? KdefDateFmt : defFmt;
                this.data[KfmtMeta] = this._proto.$X3Fmt || defFmt;
            }
        }
    },
    ...
    applyDataCollEx: function(data, commit, options, callback){
        ...
        var meta = data.$meta, metaFmtObj = this._formatter && meta && meta[_datasetK.metaType.format] ? meta[_datasetK.metaType.format] : null;
    ...
    applyMvt: function(mvt, start, count){
        ...
        var ii, lenii, itemsPropsArray, itemsProps, _$raw, itemProp;
        // Update data
        if (mvt == sapUtil.dataSetMvt.delAll) {
            ...
            for (i = start - 1, j = 0; i < start - 1 + count; i++) {
                ...
                if(this._formatter && this._proto.$item && (itemsProps = this._proto.$item.$properties)) {
                    ...
                    _$raw = this.data[i]._$rawV = {};
                    itemsPropsArray = Object.keys(itemsProps);
                    for (ii = 0, lenii = itemsPropsArray.length; ii < lenii; ii++) {
                        itemProp = itemsPropsArray[ii];
                        if(KisTypeAcceptFmt[itemsProps[itemProp].$type]) {
                            _$raw[itemProp] = {};
                        }
                    }
                }
    ...
    storeDataColl: function(newData, commit){
        var isDelta = false, jd, lend, len, rcdArray, i, j, dataFmt, niemeDataPtr;
        if (!commit) {
            isDelta = !this._$empty;
        }
        if (this._$empty && !this._formatter) {
            if (commit) {
            ...
            for (jd = 0, lend = newData.length; jd < lend; jd++) {
                ...
                if (commit || this._formatter) {
                    rcdArray = Object.keys(newData[jd]);
                    for (j = 0, len = rcdArray.length; j < len; j++) {
                        if(!this._formatter || !(dataFmt = this.getMetaFmtColl(rcdArray[j], i))) {
                            if(commit) {
                            ...
                            try {
                                if(commit) {
                                    niemeDataPtr[rcdArray[j]] = this._formatter.formatField({"format": dataFmt}, newData[jd][rcdArray[j]], true, false);
                                    niemeDataPtr._$rawV[rcdArray[j]].v = newData[jd][rcdArray[j]];
                                    newData[jd][rcdArray[j]] = niemeDataPtr[rcdArray[j]];
                                }
                                else {
                                    newData[jd][rcdArray[j]] = this._formatter.formatField({"format": dataFmt}, newData[jd][rcdArray[j]], true, false);
                                }
                            }
                            catch (e){
                                if(commit) {
    ...
    storeMetaFmtColl: function(metaFmtArray){
        ...
        for (i = 0; i < len; i++) {
            ...
            if(meta[1] == 0 && metaFmtProp) {
                ...
                if(metaFmtProp.cellsSet) {
                    for (j = 0, lenj = this.data.length; j < lenj; j++) {
                        if((_$raw =  this.data[j]._$rawV[[meta[0]]])) {
                            _$raw.fmt = null;
                        }
                    }
                }
            ...
            else if (metaFmtProp && this.data.length > meta[1] - 1 && (_$raw = this.data[meta[1] - 1]._$rawV[meta[0]])){
                // Cell scope
                _$raw.fmt = meta[2];
    ...
    getMetaFmtColl: function(property, line){
        var _$raw, fmt = null, metaFmtProp;
        if(this.data.length > line && (metaFmtProp = this.metaFmt[property])) {
            // Warning : like others meta, "format" could be set at "property" level (default) or "property/line" level.
            if(metaFmtProp.cellsSet && (_$raw = this.data[line]._$rawV[property]) && _$raw.fmt) {
                fmt = _$raw.fmt;
            }
            else {
                fmt = metaFmtProp.fmt;
