...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
            ...
            for (jd = 0, lend = newData.length; jd < lend; jd++) {
                ...
                if (commit || this._fmtWrapper) {
                    ...
                    for (j = 0, len = rcdArray.length; j < len; j++) {
                        if(!this._fmtWrapper || !(dataFmt = this.getMetaFmtColl(rcdArray[j], i, this.metaFmt[rcdArray[j]]))) {
                            if(commit) {
    ...
    getMetaFmtColl: function(property, idx, propCurrFmt){
        ...
        if(this.data.length > idx && (metaFmtProp = this.metaFmt[property])) {
            ...
            // Warning 2 : propCurrFmt is a stuff to manage current col format while processing dataset update traversing in server sequence order....
            if(metaFmtProp.cellsSet && (_$raw = this.data[idx].$rawV[property]) && _$raw.fmt) {
                ...
                fmt = propCurrFmt && propCurrFmt.fmt || metaFmtProp.fmt;
            }
        }
        return fmt;
    },
    ...
    getColumnsDataColl: function(property, raw, sort){
        var i, len, dataCol = [], rawEx = !!(raw && this.metaFmt[property]);
        if(!sort) {
            if(rawEx) {
            ...
            dataCol = this.sortDataColl([{"binding": property, "descending": sort.descending}], raw, sort);
        }
        return dataCol;
    },

    sortDataColl: function($orderBy, raw, opts){
        var sortCol, descending, self = this, sortProps, lenk = $orderBy.length;
        var zero = "0", i, k, nieme, len, dataColSorted, dataCol = [];
        // It's a quite clumsy... but decidedly better for perf.
        function getSortType(binding) {
            var metaFmt = self.metaFmt[binding], intg = false, flt = false, rawEx = !!(raw && metaFmt);
            if(!rawEx) {
                return {"intg": intg, "flt": flt, "rawEx": false};
            }
            else {
        ...
        if(opts && opts.onlyColumn) {
            ...
            if(opts.onlyData) {
                ...
                dataColSorted = dataCol.sort(function(a, b) {
                ...
                if(sortProps.rawEx) {
                    for (i = 0, len = this.data.length; i < len; i++) {
                        nieme = this.data[i];
                        dataCol[i] = {"v": nieme.$rawV[sortCol].v, "$value": nieme[sortCol], "$idx": i, "serverIndex": nieme.$serverIndex, "$uiid": nieme.$uuid};
                    }
                }
                else {
                    for (i = 0, len = this.data.length; i < len; i++) {
                        nieme = this.data[i];
                        dataCol[i] = {"v": nieme[sortCol], "$value": nieme[sortCol], "$idx": i, "serverIndex": nieme.$serverIndex, "$uiid": nieme.$uuid};
                    }                    
                }
                dataColSorted = dataCol.sort(function(a, b) {
            ...
            if (descending) {
                dataColSorted.reverse();
            }
        }
        else {
            dataCol = this.data.slice(0);
            sortProps = [];
            for (k = 0; k < lenk; k++) {
                sortProps[k] = getSortType($orderBy[k].binding);
                sortProps[k].descending = $orderBy[k].descending ? -1 : 1;
            }
            if(lenk > 1) {
                dataColSorted = dataCol.sort(function(a, b) {
                    var cmp, l, sortCol;
                    for (l = 0; l < lenk; l++) {
                        ...
                        if(!sortProps[l].rawEx) {
                            cmp = $orderBy[l].intg ? parseInt(a[sortCol] || zero, 10) - parseInt(b[sortCol] || zero, 10) : ($orderBy[l].flt ? parseFloat(a[sortCol] || zero) - parseFloat(b[sortCol] || zero) : a[sortCol].localeCompare(b[sortCol]));
                        }
                        else {
                            cmp = $orderBy[l].intg ? parseInt(a.$rawV[sortCol].v || zero, 10) - parseInt(b.$rawV[sortCol].v || zero, 10) : ($orderBy[l].flt ? parseFloat(a.$rawV[sortCol].v || zero) - parseFloat(b.$rawV[sortCol].v || zero) : a.$rawV[sortCol].v.localeCompare(b.$rawV[sortCol].v));
                        }
                        if(cmp != 0) {
                            cmp = cmp*sortProps[l].descending;
                            break;
                        }
                    }
                    return cmp;
               ...
               if(!sortProps[0].rawEx) {
                    dataColSorted = dataCol.sort(function(a, b) {
                        ...
                        return $orderBy[0].intg ? parseInt(a[sortCol] || zero, 10) - parseInt(b[sortCol] || zero, 10) : ($orderBy[0].flt ? parseFloat(a[sortCol] || zero) - parseFloat(b[sortCol] || zero) : a[sortCol].localeCompare(b[sortCol]))
                    });
                }
                else {
                    dataColSorted = dataCol.sort(function(a, b) {
                        var sortCol = $orderBy[0].binding;
                        return $orderBy[0].intg ? parseInt(a.$rawV[sortCol].v || zero, 10) - parseInt(b.$rawV[sortCol].v || zero, 10) : ($orderBy[0].flt ? parseFloat(a.$rawV[sortCol].v || zero) - parseFloat(b.$rawV[sortCol].v || zero) : a.$rawV[sortCol].v.localeCompare(b.$rawV[sortCol].v));
                    });
                }
