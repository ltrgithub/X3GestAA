    ...
    applyDataCollEx: function(data, commit, options, callbackBind, callbackUnsort){
        ...
        if (data.$rcd && !updated) {
            if (this._storeDataColl(data.$rcd, commit, options) && !leftListColl && data.$rcd.length !== this.data.length) {
                // By delta
                syraElapse.data += callbackBind(data.$rcd, {"$isPartialDelta": data.$rcd.length > 1 ? true : false, "unsortBefore": true});
            }
            else {
                // Full
                syraElapse.data += callbackBind(leftListColl ? data.$rcd : this.data, {"$isPartialDelta": false, "unsortBefore": true});
            }
            _razIndexPropr(this.data);
        }
        return syraElapse;
        ...
        if(nieme.$rawV) {
            ...
            for(i = 0, len = lastRcdArray.length; i < len; i++) {
                ...
                if(!_.isObject(nieme[desc])) {
                    if (nieme.$rawV[desc]) {
                        lastRcd[desc] = nieme.$rawV[desc].v;
                    }
                    else {
