...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    initialize: function(internalType, isCollection, name, proto, opts){
        this._isColl = isCollection;
        this.$type = internalType;
        this.entityName = name;
        this._isNodeWS = opts && opts.isNodeWS;
        this.initStore(proto, opts ? opts.locales : null);
    },

    initStore: function(proto, regOpts){
        var fmtMeta = _datasetK.metaType.format, itemsProps, i, len, itemsPropsArray, entityType, protoFmt, fmtDate = regOpts ? regOpts.dateFormat : null;
        var defFmt = ":", defDateFmt = "D:" + fmtDate || "MM[/]DD[/]YY";
        if (this._isColl) {
            this.data = [];
            this.meta = {};
            ...
            if(proto.$item && (itemsProps = proto.$item.$properties)) {
                ...
                for (i = 0, len = itemsPropsArray.length; i < len; i++) {
                    ...
                    if(KisTypeAcceptFmt[entityType]) {
                        defFmt = !protoFmt && entityType == syraUtil.dataTypes.dateType ? defDateFmt : defFmt;
                        this.meta[fmtMeta].push([itemsPropsArray[i], 0, protoFmt || defFmt]);
                    }
                }
            }
        }
            ...
            this.data = {};
            if(KisTypeAcceptFmt[this.$type]) {
                defFmt = !proto.$X3Fmt && this.$type == syraUtil.dataTypes.dateType ? defDateFmt : defFmt;
                this.data[fmtMeta] = proto.$X3Fmt || defFmt;
            }
        }
    },
    ...
    storeDataColl: function(newData, commit){
        var isDelta = false, jd, lend, len, rcdArray, i, j;
        if (!commit) {
            ...
            for (jd = 0, lend = newData.length; jd < lend; jd++) {
                ...
                if (commit) {
                    ...
                    for (j = 0, len = rcdArray.length; j < len; j++) {
                        this.data[i][rcdArray[j]] = newData[jd][rcdArray[j]];
                    }
                }
                newData[jd].$index = i;
                newData[jd].$uuid = this.data[i].$uuid;
            }
            }
            catch (ex) {
