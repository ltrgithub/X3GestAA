...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    ...
    applyDataCollEx: function(entityName, data, commit, options, callback){
        var isEmpty, ruptInsrt = null, nbMvt = data.$mvt ? data.$mvt.length : 0, i = nbMvt - 1, j, nbInsrt = 0, isDelta = false, delta, updated = false, oneByOne;
        var syraElapse = {"data": 0, "mvt": 0};
        while (i >= 0 && data.$mvt && data.$mvt[i][0] == sapUtil.dataSetMvt.insrt) {
            ruptInsrt = i;
            nbInsrt += data.$mvt[i][2];
            i--;
        }
        i = ruptInsrt !== null ? ruptInsrt : nbMvt;
        if(i > 0) {
        ...
        if(nbInsrt > 0) {
            ...
            if (!(isEmpty = this._$empty) &&_updateCollOneByOne(this.data.length, nbInsrt)){
                ...
                for (j = ruptInsrt; j < nbMvt; j++){
                    syraElapse.mvt += this.applyCollMvt(entityName, data.$mvt[j], true, options, callback);
                }
            }
            else {
                // Full update : insert movements + data
                for (j = ruptInsrt; j < nbMvt; j++){
                    delta = this.applyMvt(_getMvtCollCode(data.$mvt[j]), data.$mvt[j][1], data.$mvt[j][2]);
                }
                this.storeDataColl(data.$rcd, commit);
                syraElapse.data += callback(isEmpty ? data.$rcd : this.data, {"$isFullDelta": false});
                updated = true;
            }
        }
        if (data.$rcd && !updated)  {
            // Finally updata data if necessary
            if(this.storeDataColl(data.$rcd, commit)) {
                // By delta
                syraElapse.data += callback(data.$rcd, {"$isFullDelta": data.$rcd.lenght > 1 ? true : false});
            }
            else {
                // Full
                syraElapse.data += callback(this.data, {"$isFullDelta": false});
            }
        }
        return syraElapse;
    },
    ...
    applyCollMvt: function(entityName, mvt, oneByOne, options, callback){
        var mvtCode = _getMvtCollCode(mvt), delta, lenCollBefore = this.data.length, lenCollAfter, collMvt, isDelta, syraElapse = 0;
        // Init
        if(mvtCode == sapUtil.dataSetMvt.del && !oneByOne && lenCollBefore -  mvt[2] != 1) {
            //collMvt = this._fillTemplateColl(); TODO remved for isFulldelat=  true
        }
        // Apply dataset mvt
        ...
        if(mvtCode == sapUtil.dataSetMvt.delAll) {
            syraElapse += callback(delta.mvtData, {"$isFullDelta": false});
        }
            ...
            if(oneByOne) {
                ...
                syraElapse += callback(collMvt, {"$isFullDelta": mvtCode == sapUtil.dataSetMvt.insrt ? false : true});
                collMvt = [];
           }
        }
        return syraElapse; 
