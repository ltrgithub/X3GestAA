...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    ...
    initStore: function(regOpts){
        var KfmtMeta = _datasetK.metaType.format, occur, ro, itemsProps, i, len, itemsPropsArray, entityType, protoFmt, refProp, ret = null;
    ...
    applyDataCollEx: function(data, commit, options, callbackBind, callbackUnsort){
        ...
        var leftListColl = options && options.lf, collMvt = null, partialUpdate, syraElapse = {
            "data": 0,
            "mvt": 0
        };
    ...
    applyMvt: function(mvt, start, count){
        var result = {
            ...
            "isEmpty": false,
            "uuidInserted": {}
        }, i, j, bAtEnd, len = this.data.length, _uuid;
    ...
    _storeDataColl: function(newData, commit, options){
        var isDelta = false, nbRcds = this.data.length - 1, jd, lend, len, rcdArray, i, j, dataFmt, niemeDataPtr, skip, yetFormatted, rawValueEx, _tgInter;
        if (!commit) {
    ...
    _cureMetaStateOnArrayScope: function(metaState){
        var newMetaState = null, len, i, itemsProps = this._proto.$item.$properties, newS, state, roCount = 0;
    ...
    getMetaCollEx: function(updateEx, meta, session, options){
        function _purge(metaArray){
            var i, j, k, lenI, scope, toPurge = {}, purged = [], splited = [], idx, line = false, grid = false, cell = false, col = false;
            var doublon = {}, dblKey, nbCells = 0, nbLines = 0;
        ...
        var splitedStatus = _purge(meta[_datasetK.metaType.state]), lenStatus = splitedStatus.length;
        var splitedStyle = _purge(meta[_datasetK.metaType.style]), lenStyle = splitedStyle.length;
        var metaEx = [{}], len, i;
    ...
    getMetaColl: function(update, meta, session, options){
        ...
        var arrayMeta, colMeta, rowMeta, propMember = "$properties", styDescObj = options && options.styDescObj, sttDescObj = options && options.sttDescObj, rowMetaMap = {}, ptr, hackDch = true;
        // Helper
        ...
        _.each(meta[_datasetK.metaType.style], function(sty){
            var line, id, val, styEx = {}, metaName = "$valueStyle", ptr = null, styPropr = _datasetK.metaType.style;
            if (styDescObj) {
                line = sty.idx;
                id = sty.id;
                val = sty.val;
