...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
    ...
    applyDataCollEx: function(data, commit, options, callback){
        ...
        if (nbInsrt > 0) {
            ...
            if (!(isEmpty = this._$empty) && _updateCollOneByOne(this.data.length, nbInsrt)) {
                ...
                syraElapse.data += callback(collMvt, {"$isPartialDelta": partialUpdate, "unsortBefore": true});
        ...
        if (data.$rcd && !updated) {
            if (this._storeDataColl(data.$rcd, commit, options) && !leftListColl && data.$rcd.length !== this.data.length) {
                // By delta
                syraElapse.data += callback(data.$rcd, {"$isPartialDelta": data.$rcd.length > 1 ? true : false, "unsortBefore": true});
            }
            else {
                // Full
                syraElapse.data += callback(leftListColl ? data.$rcd : this.data, {"$isPartialDelta": false, "unsortBefore": true});
            }
            _razIndexPropr(this.data);
        }
        return syraElapse;
    ...
    applyCollMvt: function(mvt, oneByOne, options, callback){
        ...
        if (mvtCode === sapUtil.dataSetMvt.delAll || (lenCollAfter === 0 && mvtCode === sapUtil.dataSetMvt.del)) {
            syraElapse += callback([], {"$isPartialDelta": false, "unsortAfter": true});
        }
            ...
            if (oneByOne) {
                _.each(delta.mvtData, function(rcd){
                    syraElapse += callback([rcd], {"$isPartialDelta": false, "unsortBefore": mvtCode != sapUtil.dataSetMvt.del});
                });
            }
            else {
                ...
                syraElapse += callback(collMvt, {"$isPartialDelta": partialDelta, "unsortBefore": mvtCode != sapUtil.dataSetMvt.del});
                if (!partialDelta) {
                    _razIndexPropr(this.data);
                }
            }
    ...
    sortDataColl: function(orderBy, raw, opts){
       ...
       if(opts && opts.onlyColumn) {
            ...
            if(slice != null) {
                ...
                dataCol.push(opts.onlyData ? nieme[sortCol] : {"v": nieme[sortCol], "$value": nieme[sortCol], "$idx": dLength, "serverIndex": nieme.$serverIndex, "$uiid": nieme.$uuid});
            }
        }
        else {
