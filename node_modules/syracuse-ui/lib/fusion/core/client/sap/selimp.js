...
exports.PrinterAccess = helpers.defineClass(PrinterAccess, EventManager, {
	initialize: function(params, onInitComplete, sapController, winRank) {
		var winId = sapUtil.getWinId(winRank), stackParams;
		stackParams = {"id": winId, "name": "printerSetup", "mainPage": false, "$itemPage": printerSetup.getSetupPage(), "_resumeAct": "PRINTER_SEL_CANCEL"};
		this._dialogStack = [];
	...
	_getDialogInfo: function(dialogId){
		...
		return ret >= 0 ? {"rank": ret, "dlg": this._dialogStack[ret]}: null;
	},

	openProperties: function(params, onInitComplete) {
		var winId = sapUtil.getWinId(this._winRank + 1), stackParams;
		stackParams = {"id": winId, "name": "printerProperties", "mainPage": false, "$itemPage": printerSetup.getPropertiesPage(), "_resumeAct": "PRINTER_OPTS_CANCEL"};
		this.stackDialog(winId, params.$data, {}, stackParams, onInitComplete);
	},
	...
	displaySetup: function(params) {
		var dlgId = sapUtil.getWinId(this._winRank), dlgInfo = this._getDialogInfo(dlgId);
		this.trigger("sap.extdlg.activate", {"target": dlgInfo ? dlgInfo.dlg.page : null, "type": "activate", "data": params.$data}, dlgId, true);
	},
	...
	stackDialog: function(winId, initialData, dlgDefinition, loadParams, onInitComplete) {
		...
		function(page) {
			var $fusionPageMeta = syraUtil.getFusionPageMeta(page), isMainDlg = !self.isOnGoing();
			if($fusionPageMeta) {
				$fusionPageMeta._privateEventManager = self;
			}
			self._dialogStack.push({"id": winId, "page": page});
			self._sapController.registerLoadedWindow(winId, dlgDefinition, page, initialData);
			self.trigger("sap.extdlg.loaded", {"target": page, "type": "loaded", "data": {}}, winId, isMainDlg);
			onInitComplete();
			self.trigger("sap.extdlg.activate", {"target": page, "type": "activate", "data": null}, winId, isMainDlg);
		},
		function(error) {
	...
	unstackDialog: function() {
		var occur = this._dialogStack.length - 1, dlgId = this._dialogStack[occur].id;
	...
	dispose: function() {
		var result = true;
		if(this.isOnGoing()) {
			// TODO : release dialog if necessary (this._dialogStack not empty)...
			// result = self.trigger("sap.extdlg.dispose", {"target": null, "type": "dispose", "data": {}}, false);
		}
