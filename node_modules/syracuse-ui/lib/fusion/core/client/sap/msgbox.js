...
exports.releaseBoxParams = function(boxParams) {
	...
	if(boxParams.diagnoses) {
		_.each(boxParams.diagnoses, function (value, idx) {delete boxParams.diagnoses[idx];});
...
exports.releaseMsgBoxParams = function(msgBoxParams) {
	...
	if(msgBoxParams.$diagnoses) {
		_.each(msgBoxParams.$diagnoses, function (value, idx) {delete msgBoxParams.$diagnoses[idx];});
...
exports.setMsgBoxDefaultProp = function(boxProps, type, timeOut, title, message, showDetail) {
	_.defaults(boxProps, {"type": type || 4, "to": timeOut || 0, "tit": title, "li": message || "????", "showDetail": showDetail});
	return boxProps;
};