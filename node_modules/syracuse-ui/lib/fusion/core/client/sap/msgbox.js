...
function _checkBoxType(type) {
	return type == null || type < 0 || type >= _msgBoxDesc.length ? 3 : type;
}

exports.isErrBox = function(boxType) {
	return !!(boxType != undefined && boxType >= 3);
};

exports.getMsgBoxOpts = function(boxParams, opts) {
	var boxOpts = {},
		finalType = _checkBoxType(boxParams.type);
	var htmlEsc = helpers.string.htmlEscape;
	...
	if (finalType == 1 || finalType == 2) {
		// Warnbox (1) and Qstbox (2)
		var $title = (boxParams.btnsL && util.duplicateObj(boxParams.btnsL)) || {};
		var ids = (finalType == 1) ? ["ok", "cancel"] : ["yes", "no"];
		var $default = !boxParams.def || boxParams.def <= 1 ? (finalType == 1 ? "ok" : "yes") : exports._getMsgBoxDefaultRet(finalType, boxParams.def, true);
		boxOpts.$newButtons = [];
		for (var ii = 0; ii < jj; ii++) {
			var id = ids[ii];
			boxOpts.$newButtons.push({
				id: id,
				title: $title[id],
	...
	if (opts && opts.escape == true) {
		boxOpts.$message = htmlEsc(boxOpts.$message);
	}
	return boxOpts;
};
...
exports._getMsgBoxDefaultRet = function(type, defButton, txtRetFmt) {
	...
	if (txtRetFmt) {
		switch (defaultRet) {
			case _msgBoxStatus.cancel:
				defaultRet = "cancel";
				break;
			case _msgBoxStatus.no:
				defaultRet = "no";
				break;
			case __msgBoxStatus.yes:
				defaultRet = "yes";
				break;
			case _msgBoxStatus.ok:
			default:
				defaultRet = "ok";
				break;
		}
