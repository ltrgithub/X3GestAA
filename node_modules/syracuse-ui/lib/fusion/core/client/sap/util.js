...
exports.Fusion = {
    ...
    adjustIcoFieldData: function(data){
        ...
        if (state.$isReadOnly != undefined) {
            ...
            data.stt = this.setState({"$isDisabled": disable}, data.stt || 0);
            data.stt = this.removeStateMask({"$isReadOnly": ""}, data.stt);
        }
        // Hidden?
        if (num < 0) {
            data.v = null;
        ...
        if (state.$isHidden === undefined && data.v !== undefined) {
            data.stt = this.setState({"$isHidden": num < 0 ? true : false}, data.stt || 0);
        }
        return data;
    },
	...
	isEmptyGridDataRecord: function (list,data){
    	var dataSetRecord,empty=false,value;
    	if(data.uuid){
        ...
        for (var key in dataRecord){
            if(listFldProps[key]){
                ...
                switch (listFldProps[key].$type){
                    case exports.Syra.dataTypes.stringType  : empty =(value==="");break;
                    case exports.Syra.dataTypes.booleanType : empty=(value===-1);break;
                    case exports.Syra.dataTypes.dateType    : empty=(value==="");break;
                    case exports.Syra.dataTypes.choiceType  : empty=(value===-1);break;
                    case exports.Syra.dataTypes.integerType : empty=(value==="");break;
                    case exports.Syra.dataTypes.iconType    : empty=(value===-1 || value==="" || value==="-1");break;
                    case exports.Syra.dataTypes.decType     : empty=(value==="");break;
                    case exports.Syra.dataTypes.realType    : empty=(value==="");break;
                    case exports.Syra.dataTypes.timeType    : empty=(value==="");break;
                    default                                 : empty = true;
                }
                if(!empty) {
                    return false;
   ...
   checkPopupCard:function(oldIst,newIst,grid){
		...
		if (!this.cmpIstBlock(oldIst,newIst)) {
			if(grid.builder.recordCardView){
				if (grid.builder.popupCard && grid.builder.popupCard.dialog) {
                	grid.builder.togglePopupCard();
            	}
			}
		}
	},
	setFieldMod:function(field, ist){
		if(this.isGridInst(ist)){
			field=field.getField(ist.xid,ist.nl-1)
		}
		if(field && field._input){
			field._input.setAttribute("data-s-q-x3mod",ist.edit?"edit":"command");
		}
	}
   };
...
exports.Syra = {
    ...
    getEntityType: function(entity, syraProto, keepRef){
        var coll, $entity = null, type = null, refProperty;
        if (entity && syraProto) {
            if (!($entity = syraProto[entity])) {
                ...
                if (syraProto[coll] && syraProto[coll].$type == this.dataTypes.collectionType) {
                    $entity = (this.getCollFieldsFromProto(syraProto, coll))[entity];
                }
            }
    ...
    getRefPropertyEntity: function(entityProto){
        var refPropItem, matches, props, refProp = null, propName;
        if((refPropItem = entityProto.$item) && refPropItem.$value && (props = refPropItem.$properties)) {
            if((matches = refPropItem.$value.match(/(\{.*?\})/g)) && matches.length > 0) {
                ...
                refProp = {"property": propName, "proto": props[propName]};
    ...
    setHeader:function(on){
    	var stat=on?"visible":"hidden";
    	 $(".s-search-rq").css("visibility",stat)
    	 $("#s-site-home").css("visibility",stat)
    	 $("#s-site-top-pn-opener-link").css("visibility",stat)
    	 //$("#s-fusion-sessions-opener").css("visibility","hidden")
    },
};
