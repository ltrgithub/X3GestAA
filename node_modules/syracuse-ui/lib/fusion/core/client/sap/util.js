...
exports.Fusion = {
    ...
    getObjFieldValue: function(value, modeType, ctxProps){
        return {
            "v": value,
            "xFmt": modeType,
            "ctx": ctxProps
    ...
    getFieldInfo: function(field,nl){
        ...
        if (field && (fieldInfo.$fusionPageMeta = exports.Syra.getFusionPageMeta(field)) && (fieldInfo.$fieldMeta = exports.Syra.getMetaFromObject(field)) && ((fieldInfo.$fusionPageMeta.winModel.getProto())[fieldInfo.$fieldMeta.$bind] || field.arrayLevel==="cell")) {
            fieldInfo.fieldIst = this.makeIst(fieldInfo.$fusionPageMeta.winModel.getWinId(), fieldInfo.$fieldMeta.$bind,nl||0);
            fieldInfo.readOnlyByModel = exports.Syra.isFieldModelReadOnly(fieldInfo.$fusionPageMeta.winModel.getProto(), null, fieldInfo.$fieldMeta.$bind);
            fieldInfo.winModel = fieldInfo.$fusionPageMeta.winModel;
            fieldInfo.type = exports.Syra.getFieldType(field);
            if ((fieldInfo.$field = field.$field)) {
                fieldInfo.isCheck = fieldInfo.$field.$type == exports.Syra.dataTypes.booleanType ? true : false;
                if (!fieldInfo.isCheck && fieldInfo.$field.$type == exports.Syra.dataTypes.choiceType) {
                    fieldInfo.isRadio = fieldInfo.$field.$format == exports.Syra.wdgtTypes.radio ? true : false;
                    fieldInfo.isCombo = !fieldInfo.isRadio;
                }
                else {
                    fieldInfo.isIco = fieldInfo.$field.$type == exports.Syra.dataTypes.iconType ? true : false;
					fieldInfo.isIco = fieldInfo.$field.$type == exports.Syra.dataTypes.iconType ? true : false;
                }
            }
        }
...
exports.Syra = {
    ...
    getPopupCtxFromField: function(field){
        return field && field.menusController && field.menusController.contextMenu && field.menusController.contextMenu._popupMenus;
    },
    ...
    razStyleOnInput: function($$input, styleColl, excludeStyle){
        _.each(styleColl, function(stl, name){
            if (name != excludeStyle && stl.css) {
                $$input.toggleClass(stl.css, false);
            }
        });
    },
    appendProto: function(representation, append, appendCallback){
        var proto = this.get$FromPage(representation);
