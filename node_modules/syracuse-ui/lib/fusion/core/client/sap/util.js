...
exports.Fusion = {
	...
	checkPopupCard: function(oldIst, newIst, grid) {
		...
		if (!this.cmpIstBlock(oldIst, newIst)) {
			grid.popupCardBuilder && grid.popupCardBuilder.togglePopup();
		}
	},
	setFieldMod: function(field, ist) {
		...
		if (field && field.$isEditMode) {
			syra_site.dom.toggleClass(field.domValueSlot, "s-x3mode-command", !ist.edit);
		}
	},
...
exports.Syra = {
	...
	isEntityInGridCard: function(entityName, gridField) {
		...
		if (gridMeta && gridMeta.$popupCard && (layout = gridMeta.$popupCard.$layout)) {
