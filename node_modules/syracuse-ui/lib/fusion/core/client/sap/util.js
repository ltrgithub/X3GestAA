...
exports.Syra = {
		...
		return _.map(mapIn, function(mapFrom, idx) {
			...
			if (mapFrom[action] && mapFrom[action] > 0) {
				...
				if (mapFrom.checkOfficeInstalled) {
					...
					map2[mapFrom[id]].$officeAddinSetup = officeHelper.officeAddinSetupInstallation;
				}
	...
	showDiagnoses: function(diagnoses, item, option, autoHide) {
		var api = syra_diagnose,
			hide = autoHide && autoHide >= 0 ? autoHide : null;
		if (api && api.showDiagnoses) {
			api.showDiagnoses({
				$diagnoses: diagnoses,
				autoHide: hide
			}, item, option);
...
exports.Syra._mapNullValue[exports.Syra.dataTypes.choiceType] = -1; //0
