...
exports.Fusion = {
	...
	goToHelp: function(url, $prototype, target, tokens) {
		var propertie = "$baseUrl",
			baseHelpUrl = ($prototype[propertie] || "http://uranus2:8080/AdxDoc_DOCV7X3/FRA/");
		url = url.replace("{" + propertie + "}", baseHelpUrl);
		if (tokens) {
			url = url.replace("{$help}", tokens[0]);
		}
...
exports.Syra = {
	...
	purgeStateMeta: function(fldModel, stateMeta) {
		...
		if (fldModel.$isReadOnly) {
			...
			stateMeta = exports.Fusion.removeStateMask({"$isDisabled": "", "$isReadOnly": ""}, stateMeta);
