...
_evtProcGrid.sap = function() {
	this.eventListeners = function() {
		return [["wdgt.cell.click", clickCellTab, true, true], ["wdgt.cell.mousedown", clickCellTab, true, true], ["wdgt.cell.change", changeCellTab, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.record.action", actionLine, true, true], ["wdgt.array.action", actionArray, true, true], ["wdgt.cell.picker", clickPicker, true, true

		], ["wdgt.array.picker", clickArrayPicker, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.cell.calendarclosed", onDatePickerClose, true, true], ["wdgt.cell.escape", applyEscape, true, true], ["wdgt.cell.applyShortCut", applyShortCut, true, true], ["wdgt.cell.keydown", keyDownCellTab, true, true], ["wdgt.cell.keyup", keyUpCellTab, true, true], ["wdgt.grid.sortLocal", sortLocal, true, true], ["wdgt.grid.searchLocal", searchLocal, true, true]];
	};
	...
	function _getNlFromMagneto(list, current, isSorted, next, last) {
		...
		if (!isSorted) {
			...
			if (next !== null) {
				currRcdIdx = syra_list.grid.popupCardBuilder.getRecord(list).getRecordIndex();
	...
	function actionLine(event) {
		...
		if (event.target) {
			...
			if (!discard) {
				...
				if (event.data.doEvent && ("$first $previous $next $last".search("\\" + $bind) > -1)) {
					event.result.post = {
						"exposeReply": true,
	...
	function doLinesCopied(grid) {
		...
		if (grid.selector) grid.selector.unSelectAll();
	}
	...
	function applyEnter(opt) {
		...
		if (fieldInfo.isEdit) {
			...
			if (syra_list.grid.popupCardBuilder.isOpened(articleParent)) {
	...
	function applyShortCut(opt) {
		// syraUtil.log("grid applyShortCut ",opt);
		var processed = undefined,
			fieldInfo = _getFieldInfo(opt),
		...
		if (opt.data.shortcuts.down || opt.data.shortcuts.up) {
			if (syra_list.grid.popupCardBuilder.isOpened(array)) return false
		...
		if (opt.data.shortcuts.delete || opt.data.shortcuts.insert) {
			if (fieldInfo.isEdit) return true
			return false
		} else
		if (opt.data.shortcuts.left || opt.data.shortcuts.right) {
			if (fieldInfo.isEdit) {
				...
				fieldIst.xid = record.list.getNextPreviousField(opt.data.shortcuts.right, fieldIst.xid);
				acts = requestActions.getSrvAction("SRV_COMMANDGRID");
				target = true;
				processed = true;
			}
	...
	function getLocalActionLine(fieldInfo, action) {
		...
		if (!cap || !cap[(action == 'SRV_DELETELINEGRID') ? "delete" : "insert"]) return null;
		var bind = fieldInfo.$fieldMeta.$bind,
			collName = bind ? sapUtil.getBlockInst(bind) : null;
		if (fieldInfo.winModel.getEntityMetaByCriteria(collName, "_fullyDisabled", collName, -1)) return null; //grid disable
		if (fieldInfo.winModel.isCollFull(collName)) return null; // gird full
		if (fieldInfo.array.dataset.length === fieldInfo.nl) return null; //Last line
