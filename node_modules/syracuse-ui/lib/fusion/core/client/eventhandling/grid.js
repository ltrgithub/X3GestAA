...
_evtProcGrid.sap = function() {
	this.eventListeners = function() {
		return [["wdgt.cell.click", clickCellTab, true, true], ["wdgt.cell.change", changeCellTab, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.record.action", actionLine, true, true], ["wdgt.array.action", actionArray, true, true], ["wdgt.cell.picker", clickPicker, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.cell.calendarclosed", onDatePickerClose, true, true], ["wdgt.cell.keydown", keyDownCellTab, true, true], ["wdgt.grid.sortLocal", sortLocal, true, true]];
	};

	function sortLocal(event) {
		console.log("sortLocald")
		var self = this,
	...
	function actionLine(event) {
		var processed = undefined,
			fieldInfo = _getFieldInfo(event),
			acts = {}, srvActEx, srvAct, srvActNum;
		if (event.target) {
	...
	function keyDownCellTab(event) {
		...
		if (event.target && !sapUtil.isSpecialKey(keycode)) {
			switch (keycode) {
				...
				case keys["VK_ESCAPE"]:
					if (fieldInfo.isEdit) {
						syraUtil.getFusionController(event.target)._sapController._wndwsStack.setDataValue(fieldInfo.currIst.win, fieldInfo.currIst.xid, fieldInfo.currIst.nl, "", {
							"noStoreCommmit": true
						})
	...
	function _getNoLine(event) {
		if (event.data) {
			...
			if (event.data.uuid) return event.target._store._map[event.data.uuid].$serverIndex + 1
		}
