...
_evtProcGrid.sap = function(){
    ...
    function selectLine(event){
        var processed = undefined, fieldInfo = _getFieldInfo(event), acts = {}, l, recordIndex;
        if (event.target) {
    ...
    function menuCtxPickerClick(event, process, fieldInfo){
        var popupCtx = syraUtil.getPopupCtxFromField(event.target), exclude = {},target=false;
    ...
    function _getFieldInfo(event){
        var nl = (event.data && event.data.line) ? event.data.line : ((event.data && event.data.uuid) ?event.target._store._map[event.data.uuid].$recordIndex+1:event.target.getArticle().$recordIndex + 1), fieldInfo = evtTools.getFieldInfo(_getXElement(event.target), nl), $fusionPageMeta = syraUtil.getFusionPageMeta(event.target);
