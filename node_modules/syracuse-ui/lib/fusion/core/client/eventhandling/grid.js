...
_evtProcGrid.sap = function () {
	...
	function focusCell(event) {
        ...
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target))) {
			...
			if(event.data.isReadOnlyStatic){
					srvAct = requestActions.getSrvAction("SRV_COMMANDGRID");    // ask for focus
					send=true;
			}else if(event.target.boxParent.builder._hasFocus()){ // focus on this table
	...
	function focusCellCheckBox(event) {
            ...
            // Misc init
				...
				if(!event.target.boxParent.builder._hasFocus()){ // focus not on this table 
					  srvAct = requestActions.getSrvActionEX("SUP_CELL_EDIT"); //(SRV_COMMANDGRID+SRV_GETFOCUSCELL)
	...
	// SUP_CELL_ICOCLICK       775 : SRV_KIB + SRV_COMMANDGRID
	function focusCellIco(event) {
			 var processed = undefined, fieldIst, currIst ,$fusionPageMeta, $fieldMeta, $field, val, srvAct, srvActChaine=null;
			if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target))) {
            // Misc init
			 	currIst = this.sapController.getCurrInst();
				fieldIst = sapUtil.makeIst($fusionPageMeta.winModel.getWinId(), event.data.xid, event.data.line);
				focus = sapUtil.cmpIst(currIst, fieldIst);
				val=event.target.boxParent.builder.rows[event.data.line-1].getDataSrcValue(event.data.xid);
				if(!event.target.boxParent.builder._hasFocus()){ // focus not on this table 
					  srvAct = requestActions.getSrvActionEX("SUP_CELL_EDIT");
					  srvActChaine=requestActions.getSrvActionEX("SUP_CELL_ICOCLICK"); //775
					  
				}else { // focus on this table 
					if (!event.target.boxParent.builder._isTableEditing()){ //command mod
						if (!focus){
							srvAct = requestActions.getSrvActionEX("SUP_CELL_EDIT"); 
							srvActChaine=requestActions.getSrvActionEX("SUP_CELL_ICOCLICK"); 
						}else{
							srvAct = requestActions.getSrvAction("SRV_GETFOCUSCELL");
							srvActChaine=requestActions.getSrvActionEX("SUP_CELL_ICOCLICK"); 
						}
					}else{ 	 //edit mod
						if (!focus){ // focus not on this ico
							srvAct = requestActions.getSrvAction("SRV_GETFOCUSCELL");
							srvActChaine=requestActions.getSrvAction("SRV_KIB"); 
						}else{	// focus on this ico
							srvAct = requestActions.getSrvAction("SRV_KIB");//1202
						}
					}
				}	
				fieldIst.nl = event.data.line;
				...
				event.result.params = {"target": fieldIst};
				if(srvActChaine){
					event.result.post = {"exposeReply": true, "callback": toggleFieldAfterClick, "args": [this, event.target, fieldIst, val,srvActChaine]};
				}
				processed = true;	
			}else{
			}
