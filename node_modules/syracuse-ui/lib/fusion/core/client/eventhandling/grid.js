...
_evtProcGrid.sap = function() {
	...
	function _getNlFromMagneto(list, current, isSorted, next, last) {
		...
		if (!isSorted) {
			...
			if (next !== null) {
				currRcdIdx = syra_grid.popupCardBuilder.getRecord(list).getRecordIndex();
	...
	function applyEnter(opt) {
		...
		if (fieldInfo.isEdit) {
			...
			if (syra_grid.popupCardBuilder.isOpened(articleParent)) {
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.down || opt.data.shortcuts.up) {
			if (syra_grid.popupCardBuilder.isOpened(array)) return false
		...
		if (opt.data.shortcuts.left || opt.data.shortcuts.right) {
			if (fieldInfo.isEdit) {
				...
				fieldIst.xid = record.getNextPreviousField(opt.data.shortcuts.right, fieldIst.xid);
				acts = requestActions.getSrvAction("SRV_COMMANDGRID");
				target = true;
				processed = true;
			}
