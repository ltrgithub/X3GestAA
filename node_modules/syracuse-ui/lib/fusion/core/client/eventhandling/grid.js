...
_evtProcGrid.sap = function() {
	this.eventListeners = function() {
		return [["wdgt.cell.click", clickCellTab, true, true], ["wdgt.cell.mousedown", clickCellTab, true, true], ["wdgt.cell.change", changeCellTab, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.record.action", actionLine, true, true], ["wdgt.array.action", actionArray, true, true], ["wdgt.cell.picker", clickPicker, true, true], ["wdgt.array.picker", clickArrayPicker, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.cell.calendarclosed", onDatePickerClose, true, true], ["wdgt.cell.keydown", keyDownCellTab, true, true], ["wdgt.grid.sortLocal", sortLocal, true, true]];
	};
	...
	function _unSort(event) {
		...
		fieldInfo.array.fetchClientRecordSet({
			params: {
				orderBy: null
			}
		});
	...
	function actionLine(event) {
		...
		if (event.target) {
			...
			switch (event.target.$bind) {
				...
				case "$previous":
					srvAct = "SRV_COMMANDGRID";
					fieldInfo.fieldIst.nl = Math.max(1, fieldInfo.fieldIst.nl - 1);
					processed = true;
					break;
				case "$next":
					srvAct = "SRV_COMMANDGRID";
					fieldInfo.fieldIst.nl++;
					processed = true;
					break;
				case "$last":
					srvAct = "SRV_COMMANDGRID";
					fieldInfo.fieldIst.nl = fieldInfo.array.dataset.length;
					processed = true;
					break;
				case "$recordCard":
					processed = false;
	...
	function _getFieldInfo(event) {
		...
		fieldInfo.isCardView = (fieldInfo.target.getArticle && fieldInfo.target.getArticle().cardview != undefined);
		return fieldInfo;
	}
