...
_evtProcGrid.sap = function(){
    ...
    function changeCellTab(event){
        ...
        if (fieldInfo.targetType === syraUtil.dataTypes.booleanType) {
    ...
    function _getFieldInfo(event){
        ...
        fieldInfo.nl = (event.data && event.data.line) ? event.data.line : event.target.getArticle().$recordIndex + 1;
        var xTarget = (event.target.arrayLevel ? event.target : (event.target.getArticle().arrayLevel == "record" ? event.target.getArticle().getArticleParent() : event.target.getArticle()))
        fieldInfo.xid = syraUtil.getMetaFromObject(xTarget).$bind;
