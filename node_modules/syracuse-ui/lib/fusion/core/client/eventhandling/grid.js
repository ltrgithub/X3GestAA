...
_evtProcGrid.sap = function() {
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.left || opt.data.shortcuts.right) {
			if (fieldInfo.isEdit) {
				...
				while (syraUtil.isFieldModelReadOnly(array.$prototype.$item.$properties, null, fieldIst.xid)) {
		...
		if (!fieldInfo.isEdit && (fieldInfo.type != syraUtil.dataTypes.booleanType || (fieldInfo.type == syraUtil.dataTypes.booleanType && (fieldInfo.isDisabled || fieldInfo.isReadOnly)))) {
			opt.event.preventDefault();
			opt.event.stopPropagation();
		}
	...
	function _getNoLine(event) {
		...
		return syraUtil.getFieldNL(event.target);
	}
};