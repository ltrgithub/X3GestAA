...
_evtProcGrid.sap = function() {
	...
	function keyUpCellTab(event) {
		var processed = undefined,
			...
			acts = undefined,
			data, target, fieldIst = fieldInfo.fieldIst;
	...
	function keyDownCellTab(event) {
		...
		if (event.data.nativeOpt.shortcuts) {
			...
			if (event.data.nativeOpt.shortcuts.down || event.data.nativeOpt.shortcuts.up) {
				...
				if (syra_grid.popupCardBuilder.isOpened(array)) return true
				...
				if (!fieldInfo.isEdit) {
					acts = getLocalActionLine.call(this, fieldInfo, event.data.nativeOpt.shortcuts.delete ? "SRV_DELETELINEGRID" : "SRV_INSERTLINEGRID");
					if (acts) {
						target = true;
						processed = true;
					} else {
	...
	function clickCellDefault(fieldInfo) {
		...
		if (!fieldInfo.hasFocus) { // focus change
			/*if (fieldInfo.target.$isReadOnly) { //103752
				acts.srvAct = requestActions.getSrvAction("SRV_COMMANDGRID"); // ask for focus
				acts.send = true;
			} else {*/
			...
			if (!fieldInfo.isEdit /*&& !fieldInfo.target.$isReadOnly*/ ) { // focus already on this table but fiend not edit mode : 103752 but can becomes editing
				acts.srvAct = requestActions.getSrvAction("SRV_GETFOCUSCELL"); // ask for editing
				acts.send = true;
			}
		}
