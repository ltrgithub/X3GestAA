...
_evtProcGrid.sap = function(){
    ...
    function selectLine(event){
        ...
        if (event.target) {
           ...
           if(event.data.nativeEvt.shiftKey && l.length >0){
           		 serverIndex = fieldInfo.nl - 1;
           		 for (var ii=0;ii<fieldInfo.array.dataset.length;ii++){
           		 	if(recordData=fieldInfo.array._store._map[fieldInfo.array.dataset[ii].$uuid]){
       		 			if(recordData.$serverIndex>=l[0].start && recordData.$serverIndex <= serverIndex){
       		 				fieldInfo.array._store.selector.select(recordData.$uuid,true)
       		 			}else{
       		 				fieldInfo.array._store.selector.select(recordData.$uuid,false)
       		 			} 
       		 		}else{
            ...
            if (fieldInfo.changeLine || fieldInfo.firstFocus) { // focus not already on this table or check and change line
    ...
    function _unSelectLines(fieldInfo){
    	var recordData,serverIndex = fieldInfo.nl - 1;
		for (var ii=0;ii<fieldInfo.array.dataset.length;ii++){
   		 	if(recordData=fieldInfo.array._store._map[fieldInfo.array.dataset[ii].$uuid]){
	 			if(recordData.$serverIndex != serverIndex){
	 				fieldInfo.array._store.selector.select(recordData.$uuid,false)
	 			}
	 		}else{
	 			if(fieldInfo.array.dataset[ii].$serverIndex !=serverIndex){
	 				fieldInfo.array.dataset[ii].$isSelected=false
	 			}
	 		}
   		 }   	
    ...
    function keyDownCellTab(event){
	    ...
	    if (event.target && !sapUtil.isSpecialKey(keycode)) {
            switch (keycode) {
				...
				case keys["VK_PGDOWN"]:
						//Test re sort grid
                    	var grid=event.target.articleParent.getArticleParent();
                    	evtTools.cancelSortArray(grid,evtTools.getFieldInfo(_getXElement(grid)))
                    	break;
				case keys["VK_F1"]:
				case keys["VK_F2"]:
				case keys["VK_F3"]:
    ...
    function unSortBeforeEdit(fieldInfo){
    	if(fieldInfo.isSortedArray){
           evtTools.cancelSortArray(fieldInfo.array,fieldInfo)
    	}
    }
