...
_evtProcGrid.sap = function() {
	this.eventListeners = function() {
		return [["wdgt.cell.click", clickCellTab, true, true], ["wdgt.cell.mousedown", clickCellTab, true, true], ["wdgt.cell.change", changeCellTab, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.record.action", actionLine, true, true], ["wdgt.array.action", actionArray, true, true], ["wdgt.cell.picker", clickPicker, true, true], ["wdgt.array.picker", clickArrayPicker, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.cell.calendarclosed", onDatePickerClose, true, true], ["wdgt.cell.keydown", keyDownCellTab, true, true], ["wdgt.grid.sortLocal", sortLocal, true, true], ["wdgt.grid.searchLocal", searchLocal, true, true]];
	};
	...
	function _getNlFromMagneto(list, current, isSorted, next, last) {
		...
		if (!isSorted) {
			...
			if (next !== null) {
				currRcdIdx = list.builder.popupCardBuilder._recordPopupCard.$recordIndex;
				nl = next ? currRcdIdx + 1 : Math.max(0, currRcdIdx - 1);
			} else if (last !== null) {
				nl = last ? list.clientDataset.length - 1 : 0;
			}
			nl = list.clientDataset[nl].$serverIndex + 1;
		}
		return nl;
	}
		...
		if (event.target) {
			...
			switch (event.target.$bind) {
				...
				case "$last":
					...
					fieldInfo.fieldIst.nl = _getNlFromMagneto(fieldInfo.array, fieldInfo.fieldIst.nl, fieldInfo.isSortedArray, next, last);
					processed = true;
					break;
				case "$recordCard":
					processed = false;
	...
	function selectLine(event) {
		...
		if (event.target) {
			...
			if ((fieldInfo.changeLine || fieldInfo.firstFocus) && (!event.data.nativeEvt.shiftKey || indexList.length == 0)) { // focus not already on this table or check and change line
	...
	function clickArrayPicker(event) {
		...
		if (fieldInfo.firstFocus || fieldInfo.currIst.edit) {
			...
			var self = this;
			setTimeout(function() {
				evtTools.openFieldCtx(null, self, event, event.target, fieldInfo);
			}, 10);
			return true;
		}
		return false;
	...
	function keyDownCellTab(event) {
		...
		if (event.data.nativeOpt.shortcuts) {
			...
			if (event.data.nativeOpt.shortcuts.down || event.data.nativeOpt.shortcuts.up) {
				...
				serverIndex = fieldInfo.array.getClientDataSetServerIndex(recordIndex);
				...
				case keys["VK_ENTER"]:
					if (fieldInfo.isEdit) {
						...
						if (articleParent.popupCardItem) {
							...
							articleParent.builder.popupCardBuilder._popupCard.dialog.$$closeBtn.click();
							return false;
						} else {
							event.target.page.externalAdapter.onFieldEvent(event.data.nativeOpt, true);
							return false
