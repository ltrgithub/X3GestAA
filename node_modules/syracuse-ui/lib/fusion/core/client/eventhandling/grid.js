...
_evtProcGrid.sap = function() {
	...
	function actionArray(event) {
		...
		if (event.target) {
			switch (event.target.$sourceBind || event.target.$bind) {
				case "$delete":
					processed = deleteAllLines.call(this, event);
					break;
			    case "$deleteSelected":
                    processed = deleteSelectedLines.call(this, event);
                    break;
                 case "$copy":
    ...
    function deleteAllLines(event){
        var process;
        if (event.target) {
          var fieldInfo = _getFieldInfo(event);
            process=deleteLines.call(this,event,0,Math.max(fieldInfo.array.dataset.length, 1));
        }
        return process;
    };
    function deleteSelectedLines(event){        
