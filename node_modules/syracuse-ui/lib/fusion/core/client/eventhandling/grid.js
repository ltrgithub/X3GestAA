...
_evtProcGrid.sap = function(){
    this.eventListeners = function(){
        return [["wdgt.cell.click", clickCellTab, true, true], ["wdgt.cell.change", changeCellTab, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.record.action", actionLine, true, true], ["wdgt.array.action", actionArray, true, true], ["wdgt.cell.picker", clickPicker, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.cell.calendarclosed", onDatePickerClose, true, true], ["wdgt.cell.keydown", keyDownCellTab, true, true]];
    };
            ...
            if (sIndex.length > 0) {
                ...
                if (fieldInfo.firstFocus || fieldInfo.changeLine || fieldInfo.isEdit) { // focus not already on this table or line change or same ligne but edit field
                    event.result = {
                        ...
                        "params": {
                            "target": fieldInfo.fieldIst,
                            "sudo": [null, [event.data.line,sIndex[0].count]]
                        }
                    }
                }
                else {
    ...
    function selectLine(event){
        var processed = undefined, fieldInfo = _getFieldInfo(event), acts = {}, l, serverIndex;
        if (event.target) {
            l = event.target.getSelectedRecordIndexes();
            if (l.length > 0 && fieldInfo.nl) {
                serverIndex = fieldInfo.nl - 1;
                if (event.data.nativeEvt.currentTarget.checked && serverIndex != l[0].start + l[0].count && serverIndex != l[0].start - 1) {
                    event.data.nativeEvt.currentTarget.checked = false;
                    return true
                }
                if (!event.data.nativeEvt.currentTarget.checked && serverIndex != l[0].start + l[0].count - 1 && serverIndex != l[0].start) {
                    event.data.nativeEvt.currentTarget.checked = true;
                    return true
                }
            }
            if ((fieldInfo.changeLine && event.data.nativeEvt.currentTarget.checked) || fieldInfo.firstFocus) { // focus not already on this table or check and change line
    ...
    function menuCtxPickerClick(event, process, fieldInfo){
    ...
    function datePickerClick(event, process, fieldInfo){
        if (process && !syraUtil.getPopupDateFromField(event.target)) {
            ...
            if (!syraUtil.getPopupDateFromField(event.target)) {
                ...
                var currValue = syraUtil.getFusionPageMeta(event.target).winModel.getValue(syraUtil.getMetaFromObject(event.target).$bind, event.target.getArticle().$serverIndex + 1);
                if (currValue.v) {
                    event.result = {
    ...
    function keyDownCellTab(event){
        var processed = undefined, fieldInfo = _getFieldInfo(event), doEvt, keycode = event.data.nativeEvt.keyCode, acts = undefined,target;
      	if(console) console.log("keyDownCellTab :"+keycode)
	    if (event.target && !sapUtil.isSpecialKey(keycode)) {
            switch (keycode) {
                ...
                case keys["VK_ESCAPE"]:
                    if (fieldInfo.isEdit) {
						syraUtil.getFusionController(event.target)._sapController._wndwsStack.setDataValue(fieldInfo.currIst.win,fieldInfo.currIst.xid,fieldInfo.currIst.nl, "", {"noStoreCommmit": true})
    ...
    function clickCellDefault(fieldInfo){
        ...
        if (!fieldInfo.hasFocus) { // focus change
            if (fieldInfo.target.$isReadOnly) {
                ...
                if (!fieldInfo.isEdit) { // focus already on this table but fiend not edit mode
                    acts.srvAct = requestActions.getSrvAction("SRV_GETFOCUSCELL"); // ask for editing
                    acts.send = true;
                }			
		}
    ...
    function clickCellBoolean(fieldInfo, newIst){
        ...
        acts.post = {
            ...
            "args": [this, fieldInfo.target, newIst, newVal, true, actionNext]
        };
        return acts;
    };
	...
	function _getNoLine(event){
		if (event.data) {
			...
			if (event.data.uuid) 
				return event.target._store._map[event.data.uuid].$serverIndex + 1
		}
		if(event.target.arrayLevel=="record"){
			return event.target.getArticle().$serverIndex + 1
		}
		if (event.target.$uuid){ 
			return event.target._store._map[event.target.$uuid].$serverIndex + 1
		}
		return event.target.getArticle().$serverIndex + 1
	}
