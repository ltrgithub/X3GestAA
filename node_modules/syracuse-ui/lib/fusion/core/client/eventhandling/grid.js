...
_evtProcGrid.sap = function(){
    ...
    function actionLine(event){
        var processed = undefined, $fusionPageMeta , fieldInfo=_getFieldInfo.call(this,event), xid,  newIst,  acts = {},  srvActEx, srvAct;
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target))) {
			if(fieldInfo.firstFocus){
				...
				*  
				*/ 
				newIst = sapUtil.makeIst($fusionPageMeta.winModel.getWinId(), xid, fieldInfo.nl);
			}else {
				newIst = sapUtil.makeIst($fusionPageMeta.winModel.getWinId(), fieldInfo.currIst.xid, fieldInfo.nl);
			}
            ...
            if (fieldInfo.firstFocus || fieldInfo.changeLine || fieldInfo.isEdit) { // focus not already on this table or line change same ligne but edit field
		...
		if (process && !syraUtil.getPopupDateFromField(event.target)) {
			event.result.post = {
                ...
                if (currValue.v)
                {
                    ...
                    event.result.post = {"callback": _openDatePicker, "args": [this, event, event.target]};
                    process = true;
                }
                else {
					var self=this
	...
	function onDatePickerClose(event){
        ...
        if (event.target && (fieldInfo = _getFieldInfo.call(this,event)) && fieldInfo.focus) {
            ...
            event.result.post = {"callback": function (succeeded, reply, evtHandlingObj, initialEvent) {
                    if(fieldInfo.focus) {
