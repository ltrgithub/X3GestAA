...
_evtProcGrid.sap = function(){
    this.eventListeners = function(){
        return [["wdgt.cell.click", clickCell, true, true], ["wdgt.grid.selline", selectLine, true, true], ["wdgt.grid.insertline", insertLine, true, true], ["wdgt.grid.deleteline", deleteLine, true, true], ];
    };
    function deleteLine(event){
        ...
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target))) {
            ...
            filedInfo.changeLine = (!fieldInfo.firstFocussap && Ctrl.getCurrInst().nl != fieldInfo.nl)
            if (fieldInfo.firstFocus || filedInfo.changeLine) { // focus not already on this table or line change
                //TODO ... SRV_COMMANDGRID +SRV_DELETELINEGRID
            }
            else {
                acts.srvAct = requestActions.getSrvAction("SRV_DELETELINEGRID");
                acts.send = true;
            }
            if (acts.send) {
                event.result = {
    ...
    function insertLine(event){
        ...
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target))) {
            ...
            filedInfo.changeLine = (!fieldInfo.firstFocussap && Ctrl.getCurrInst().nl != fieldInfo.nl)
            if (fieldInfo.firstFocus || filedInfo.changeLine) { // focus not already on this table or line change
                //TODO ... SRV_COMMANDGRID +SRV_INSERTLINEGRID
            }
            else {
                acts.srvAct = requestActions.getSrvAction("SRV_INSERTLINEGRID");
                acts.send = true;
            }
            if (acts.send) {
                event.result = {
