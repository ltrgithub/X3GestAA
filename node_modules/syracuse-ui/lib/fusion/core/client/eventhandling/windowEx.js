			...
			if (!fieldInfo.readOnlyByModel && !this.sapController.hasFocus(fieldInfo.fieldIst)) {
				...
				if (fieldInfo.isRadio) {
					event.result.post = {
						...
						"args": [this, event.target, fieldInfo.fieldIst, event.target.getDataValue(nativeEvt), true, requestActions.getSrvAction("SRV_GETFOCUS")]
					};
					if ($.browser.msie) {
			...
			if (!fieldInfo.readOnlyByModel) {
				if (fieldInfo.isRadio) {
					event.result = evtTools.getliveNotifyValueParams(fieldInfo.fieldIst, {
						"exposeReply": true,
						"callback": evtTools.toggleFieldAfterClick,
						"args": [this, event.target, fieldInfo.fieldIst, event.target.getDataValue(nativeEvt), false]
