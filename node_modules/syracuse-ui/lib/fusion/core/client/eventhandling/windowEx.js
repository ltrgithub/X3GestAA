...
_evtProcWindow.sap = function() {
	...
	function onKeyDown(event) {
		...
		if (shortCuts) {
			if (shortCuts.down || shortCuts.up) {
				...
				if (!fieldInfo.isCombo && (fieldInfo.type != syraUtil.dataTypes.dateType || !fieldInfo.popupPicker)) {
					return true;
				} else {
					return false;
				}
		...
		switch (keyCode) {
			...
			case 100:
				if (event.data.nativeEvt.altKey) {
					if (this.sapController.getSiteController()) {
						(this.sapController.getSiteController()).toggleLog(event.data.nativeEvt.shiftKey && event.data.nativeEvt.ctrlKey);
					}
				}
				break;
			case 73:
			...
			case keys["VK_TAB"]:
				if (!($$cont = $$field.closest("." + syraUtil.pageSectionK.bar)) || $$cont.length == 0 || event.data.nativeEvt.target.tagName.toLowerCase() != "input") {
					...
					if (!event.noControl && event.data.nativeEvt.shiftKey) {
	...
	function escapeOnField(field, sapController) {
		var fieldCxt, currValue, oldValue, result, $fieldMeta, $fusionPageMeta;
	...
	function onFieldKeyUp(event) {
		...
		var $itemMeta = syraUtil.getMetaFromObject(event.target),
			...
			$$cont, oldValue;
		syraUtil.log("onFieldKeyUp", keyCode, event);
		if ($itemMeta && sapUtil.isListScreen($itemMeta.$bind) && event.target.boxParent.boxParent) {
			...
			switch (keyCode) {
				...
				case keys["VK_ESCAPE"]:
					if (!($$cont = $$field.closest("." + syraUtil.pageSectionK.bar)) || $$cont.length == 0 || event.data.nativeEvt.target.tagName.toLowerCase() != "input") {
						escapeOnField(event.target, this.sapController);
						processed = true;
					}
