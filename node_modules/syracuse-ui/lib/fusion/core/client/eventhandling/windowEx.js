...
_evtProcWindow.sap = function(){
    this.eventListeners = function(){
        return [["wdgt.win.keydown", onKeyDown, true, true], ["wdgt.win.dblclick", onDblClick, true, true], ["wdgt.win.click", onClick, true, true], ["wdgt.win.close", onClose, true, true], ["wdgt.field.keydown", onFieldKeyDown, true, true], ["wdgt.field.click", onFieldClick, true, true], ["wdgt.field.picker", onFieldClickPicker, true, true], ["wdgt.field.change", onFieldChange, true, true]];
    };
    ...
    function onFieldClick(event){
        var processed = undefined, newVal, fieldInfo;
        if (event.target && (fieldInfo = _getFieldInfo(event.target))) {
            ...
            if (!fieldInfo.readOnlyByModel && !this.sapController.hasFocus(fieldInfo.fieldIst)) {
                ...
                if (fieldInfo.isIco) {
                    event.result.params.sudoV = [null, event.target.getDataValue()];
                }
                if (event.data.doEvent) {
                    ...
                    if (fieldInfo.isCheck || fieldInfo.isRadio) {
                        newVal = fieldInfo.isRadio ? event.target.getInputValue() : (event.target.currentValue == sapUtil.check.off ? sapUtil.check.on : sapUtil.check.off);
                        event.result.post = {
                            "exposeReply": true,
                            "callback": _toggleFieldAfterClick,
                            "args": [this, event.target, fieldInfo.fieldIst, newVal]
                        };
                    }
                processed = true;
            }
                ...
                if (!fieldInfo.readOnlyByModel) {
                    if (fieldInfo.isCheck || fieldInfo.isRadio) {
                        event.result = getliveNotifyValueParams(fieldInfo.fieldIst, null);
                        processed = true;
                    }
                    else 
                        if (fieldInfo.isIco) {
    ...
    function onFieldClickPicker(event){
        var processed = undefined, pickerOk = false, pickerType, fieldInfo = null, isCtx = false, params = null, act, actEx;
        if (event.data) {
            // Init
            if ((pickerType = event.data.pickerType)) {
                ...
                switch (pickerType) {
                    ...
                    case "tunnel":
                        ...
                        break;
                    case "lookup":
                        act = "SRV_CHAMP_SELECTION";
                        actEx = "SUP_FIELD_SELECT";
                        break;
                    default:
                        // Field own picker, like  "choice", "numeric-up", "numeric-down", etc...
            ...
            if (pickerOk && fieldInfo && !fieldInfo.readOnlyByModel) {
                ...
                if (event.data.doEvent && isCtx) {
                    event.result.post = {
                        "exposeReply": isCtx,
                        "callback": isCtx ? _openFieldCtx : event.data.doEvent,
                        "args": isCtx ? [this, event, event.target, fieldInfo.fieldIst] : []
                    };
                }
                processed = true;
            }
    ...
    function _openFieldCtx(reply, evtHandlingObj, initialEvent, field, fieldIst){
        var fixture = [["1168", true, "Property"], ["1160", true, "Advanced Selection"]];
        var ctx = fixture;//this.sapController.getTargetCtxFromReply(reply);
        if (ctx) {
            // Quid?
            field.applyMetaData({
                $links: {
                    link1: {
                        $title: "link1 " + field.getDataValue()
                    },
                    link2: {
                        $title: "link2 " + field.getDataValue()
                    }
                }
            });
    ...
    function onFieldChange(event){
        var processed = undefined, fieldIst, $fusionPageMeta, $fieldMeta, $field, isCombo = false;
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target)) && ($fieldMeta = syraUtil.getMetaFromObject(event.target)) && ($fusionPageMeta.winModel.getProto())[$fieldMeta.$bind]) {
            ...
            if (($field = event.target.$field)) {
                isCombo = $field.$type == syraUtil.dataTypes.choiceType && $field.$format == syraUtil.wdgtTypes.combo ? true : false;
            }
            // What should we do with this event?
            if (isCombo && this.sapController.hasFocus(fieldIst)) {
