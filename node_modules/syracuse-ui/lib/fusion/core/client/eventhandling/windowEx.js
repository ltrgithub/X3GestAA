...
_evtProcWindow.sap = function(){
                                        ...
                                        event.result.post = {
                                            "callback": function(succeeded, result, evtHandlingObj, initialEvt, callerFieldInfo){
                                                ...
                                                if (!succeeded) {
                                                    ...
                                                    callerFieldInfo.winModel.setData(callerFieldInfo.$fieldMeta.$bind, callerFieldInfo.type, null, data, {
                                                        "noStoreCommmit": true
                                                    });
                                                }
    ...
    function _getFieldInfo(field){
        return evtTools.getFieldInfo(field);
    }
        ...
        if (event.target && (fieldInfo = _getFieldInfo(event.target)) && !fieldInfo.isCheck) {
            ...
            if (!fieldInfo.readOnlyByModel && !this.sapController.hasFocus(fieldInfo.fieldIst)) {
                ...
                if (event.data.doEvent) {
                    ...
                    if (fieldInfo.isRadio) {
                        event.result.post = {
                            ...
                            "args": [this, event.target, fieldInfo.fieldIst, event.target.getDataValue(), true]
                        };
                    }
                processed = true;
            }
                ...
                if (!fieldInfo.readOnlyByModel) {
                    if (fieldInfo.isRadio) {
                        event.result = _getliveNotifyValueParams(fieldInfo.fieldIst, {
                            "exposeReply": true,
                            "callback": _toggleFieldAfterClick,
