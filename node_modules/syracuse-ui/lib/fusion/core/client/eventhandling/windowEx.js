...
_evtProcWindow.sap = function() {
	...
	function _onClickExcludeForceFocus($$nativeEvtTarget) {
		...
		if ($$nativeEvtTarget.hasClass(syra_fusion.syraUtil.gridFusionK.scroll_right) || $$nativeEvtTarget.hasClass(syra_fusion.syraUtil.gridFusionK.scroll_left)) {
			// Scroll on grid
			exclude = true;
		}
		return exclude;
	...
	function onInput(event) {
		...
		if (fieldCxt && (fieldInfo = _getFieldInfo(event.target, fieldCxt.nl)) && syra_fusion.syraUtil.isPlainTextField($itemMeta, fieldInfo.type)) {
			event.target.setDirty && event.target.setDirty(true);
		}
		return processed;
	}
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.enter) {
			...
			if (opt.target.isTextArea) {
				...
				fieldInfo = _getFieldInfo(opt.target), $itemMeta = syra_fusion.syraUtil.getMetaFromObject(opt.target);
				if (!syra_alert.getModal()) {
					if ((fieldInfo && _getEnterExcludeType()[fieldInfo.type]) || (fieldInfo && fieldInfo.type == syra_fusion.syraUtil.dataTypes.stringType && $itemMeta.$rows)) {
						processed = undefined;
					} else {
						opt.result = {
	...
	function onFieldClick(event, force) {
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target)) && (force || !evtTools.discardFieldEvent(event.type, fieldInfo))) {
			...
			if (!fieldInfo.readOnlyByModel && !this.sapController.hasFocus(fieldInfo.fieldIst)) {
				...
				if (fieldInfo.type == syra_fusion.syraUtil.dataTypes.flash) {
					event.result.post = {
						"exposeReply": false,
