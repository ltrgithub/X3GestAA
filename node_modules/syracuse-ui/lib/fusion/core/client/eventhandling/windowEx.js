...
_evtProcWindow.sap = function() {
	...
	function onInput(event) {
		if (this.sapController.getCurrInst()) {
			...
			if (field.dirty && field.getTextRows && field.getTextRows() && field.$field.$type == "application/x-string") {
				field.setDirty(true);
			}
		}
	}
		...
		switch (keyCode) {
			case keys["VK_ENTER"]:
				if (!syra_diagnose.getOpenedBox()) {
					...
					if (field && (_vkEnterExcludeType[field.$field.$type] || ($rows && field.$field.$type == "application/x-string"))) {
						processed = undefined;
					} else {
						event.result = {
			...
			switch (keyCode) {
				...
				case keys["VK_ESCAPE"]:
					if (!($$cont = $$field.closest("." + syraUtil.pageSectionK.bar)) || $$cont.length == 0 || event.data.nativeEvt.target.tagName.toLowerCase() != "input") {
						if ((fieldCxt = this.sapController.getCurrInst()) && (fieldInfo = _getFieldInfo(event.target, fieldCxt.nl))) {
							currValue = fieldInfo.winModel.getDataInputValue(fieldInfo.fieldIst.xid, fieldInfo.fieldIst.nl);
							if (currValue && currValue.v != undefined && sapUtil.needToControlField(fieldInfo.type)) {
								oldValue = fieldInfo.winModel.getDataStoreValue(fieldInfo.$fieldMeta.$bind, fieldInfo.fieldIst.nl, false, true);
								if (fieldCxt.fmt) {
									event.result = {
										"$act": requestActions.svcFormat
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target))) {
			...
			if (fieldInfo.isCheck) {
				...
				event.result = evtTools.getliveNotifyValueParams(fieldInfo.fieldIst, {
					...
					"args": [this, event.target, fieldInfo.fieldIst, event.target.getDataValue(), !hasFocus, (!hasFocus) ? requestActions.getSrvAction("SRV_GETFOCUS") : null, oldValue]
				}, requestActions.getSrvAction("SRV_GETFOCUS"));
				processed = true;
			}
		} else {
