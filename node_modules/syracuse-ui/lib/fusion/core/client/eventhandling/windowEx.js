                            ...
                            if ((fieldInfo = _getFieldInfo(event.target)) && (fieldCxt = this.sapController.getCurrInst())) {
                                ...
                                if (currValue.v && sapUtil.needToControlField(fieldInfo.type)) {
                                    ...
                                    if(keyCode == keys["VK_TAB"] && fieldCxt.fmt) {
                                        ...
                                        oldValue = fieldInfo.winModel.getDatasetValue(fieldInfo.$fieldMeta.$bind, null, false, true);
                                        if(fieldCxt.fmt) {
                                            event.result = {"$act": requestActions.svcFormat};
                                            ...
                                            event.result.post = {"callback": function (succeeded, result, evtHandlingObj, initialEvt, callerFieldInfo) {
                                                ...
                                                callerFieldInfo.winModel.setDataValue(callerFieldInfo.$fieldMeta.$bind, callerFieldInfo.fieldIst.nl, succeeded ? svcFmt.getReplyResult(result) : oldValue, {"noStoreCommmit": true});
                                            }, "args": [this, event, fieldInfo]};
                                        }
                                        else {
                                            fieldInfo.winModel.removeFocus(fieldInfo.$fieldMeta.$bind);
                                            setTimeout(function() {
                                                fieldInfo.winModel.setDataValue(fieldInfo.$fieldMeta.$bind, fieldInfo.fieldIst.nl, oldValue, {"noStoreCommmit": true});
                                            }, 10);
                                        }
                                    }
                                }
