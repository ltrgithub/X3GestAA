...
_evtProcWindow.sap = function(){
    ...
    function _showSessInfo(sapController, currField, filedInf, curWin){
        var diagnoses = [], info = sapController._session.getSessInfo(), boxParams, boxOpts, lastWin, fmt, fn = "", fnEx;
        ...
        if(filedInf.$field && (fnEx = filedInf.$field.$X3Name)) {
            fnEx = fnEx.split("_");
            fn = fnEx.length > 1 ? (fnEx[0] + " - screen : " + fnEx[1]) : fnEx[0];
        }
        ...
        if ($itemMeta && sapUtil.isListScreen($itemMeta.$bind) && event.target.boxParent.boxParent) {
            ...
            switch (keyCode) {
                ...
                if((fieldInfo = fieldCxt ? _getFieldInfo(event.target, fieldCxt.nl) : null)) {
                    currValue = KisTypeAcceptFmt[fieldInfo.type] ? fieldInfo.winModel.getDataInputValue(fieldInfo.fieldIst.xid, fieldInfo.fieldIst.nl): null;
                    control = currValue && currValue.v ? formatW.editToDisplay(fieldCxt.fmt, currValue.v) : {"succeeded": true};
    ...
    function onClose(event){
      ...
      function onCloseAll(event){
        var processed = undefined, wins=[], currist=event.target.fusionSite.controller._sapController.getCurrInst();
...
                    event.target.setFocus(true, null, currIst.xid, (currIst.nl || 0), (currIst.edit === null ? true : currIst.edit));
                    processed = true;
                }
            }
        }
...
    }
    ...
    function onFieldClick(event, force){
        ...
        if (event.target && (fieldInfo = _getFieldInfo(event.target)) && (force || !evtTools.discardFieldEvent(event.type, fieldInfo))) {
			...
			if( fieldInfo.$fusionPageMeta.winModel._mdata.name === "_FORMULA" ){
				return false;
			}
