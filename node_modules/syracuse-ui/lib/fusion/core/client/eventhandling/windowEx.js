...
_vkEnterExcludeType[syraUtil.dataTypes.choiceType] = true;
...
_evtProcWindow.sap = function(){
    this.eventListeners = function(){
        return [
            ...
            ["wdgt.field.keyup", onFieldKeyUp, false, false],
    ...
    function _showSessInfo(sapController, currField, filedInf, curWin){
        //TODO : REMOVE
        //var cmiWin = sapController._wndwsStack.getWindow(sapController._wndwsStack.length() + 1);
        //var res = cmiWin.getDataStoreValue("bA5", null, false, true, {"rawVal": true, "sort": {"descending": "true", "onlyData": false}});
        //var res = cmiWin.getDataStoreValue("bA", null, false, true, {"rawVal": true, "sort": {"$orderBy": [{"binding": "bA5", "descending": true}, {"binding": "bA2", "descending": false}]}});
        //var res = cmiWin.getDataStoreValue("bA", null, false, true, {"rawVal": true, "sort": {"$orderBy": [{"binding": "bA5", "descending": true}]}});

        var diagnoses = [], info = sapController._session.getSessInfo(), boxParams, boxOpts, lastWin, fmt;
        ...
        diagnoses.push(util.makeDiagnosis("Current field is : " + (currField.xid ? (currField.xid + " (format = "+ fmt + ")") : "???"), "info", null, 0, null, "", null, []));
            ...
            switch (keyCode) {
                    ...
                    if (event.data.nativeEvt.ctrlKey && event.data.nativeEvt.altKey) {
                        setTimeout(function(){
                            ...
                            event.target.page.externalAdapter.onBlockExRpc({
                                ...
                                "call": {
                                    "proxy": "TEST_CMI",
                                    "values": [["string1 1 origin ùﭸ", "string1 2 origin", "string1 3 origin", "string1 4 origin", "string1 5 origin"], ["string2 1 origin", "string2 2 origin", "string2 3 origin", "string2 4 origin", "string2 5 origin"], "STRING1", [1, 2, 3, 4, 5, 6]],
                                            ...
                                            "callback": function(succeeded, result, evtHandlingObj, initialEvt, callerFieldInfo){
                                                ...
                                                if (!succeeded) {
                                                    ...
                                                    callerFieldInfo.winModel.setDataStoreValue(callerFieldInfo.$fieldMeta.$bind, callerFieldInfo.fieldIst.nl, data, {
                                                        "noStoreCommmit": true, "formatted": true
                                                    });
                                                }
                ...
                case keys["VK_ENTER"]:
                    ...
                    if (($itemMeta = syraUtil.getMetaFromObject(event.target)) && sapUtil.isListScreen($itemMeta.$bind) && (grid = event.target.boxParent.boxParent)) {
                        ...
                        event.result = {
                            "$act": hasFocus ? requestActions.getSrvAction("SRV_BRWRAPIDESEL") : requestActions.getSrvActionEX("SUP_LIST_FILTER"),
                            "params": {
                                "target": listIst
                            }
                        };
    ...
    function onFieldKeyUp(event){
        var processed = undefined, fieldCxt, currValue, fieldInfo, control, formatW = this.sapController.fmtWrapper;
        var $itemMeta = syraUtil.getMetaFromObject(event.target), keyCode = event.data.nativeEvt.keyCode;
        if ($itemMeta && sapUtil.isListScreen($itemMeta.$bind) && event.target.boxParent.boxParent) {
            // Nothing special in this case (ie quicke filter in array)
        }
        else if (formatW && formatW.settings.ctrlOnfly) {
            switch (keyCode) {
                case keys["VK_ESCAPE"]:
                case keys["VK_TAB"]:
                case keys["VK_ENTER"]:
                ...
                if((fieldInfo = fieldCxt ? _getFieldInfo(event.target, fieldCxt.nl) : null)) {
                    currValue = fieldInfo.winModel.getDataInputValue(fieldInfo.fieldIst.xid, fieldInfo.fieldIst.nl);
                    control = currValue.v ? formatW.editToDisplay(fieldCxt.fmt, currValue.v) : {"succeeded": true};
                    if(!control.succeeded) {
                        fieldInfo.winModel.showFieldError(event.target, [" "]);
                    }
                    else {
                        fieldInfo.winModel.showFieldError(event.target, null, true);
                    }
                }
            }
        }
...
        var processed = undefined, $$field, currIst, $$cont, $$fieldDocLink;
...
                // Warning... have to check if target parent is not a "special" attached file link. To be improve (TODO - ).
                $$fieldDocLink = $$field.parent();
                if(!$$fieldDocLink || $$fieldDocLink.length <= 0 || !$$fieldDocLink.hasClass("s-field-file-link")) {
...
    }
    ...
    function onFileUpload(event){
        var processed = undefined, fieldInfo, isFileOnError;
    ...
    function _uploadCtrlImage(reply, evtHandlingObj, event){
        var error = false, session, err, lerr
        if (event.data.files[0]) {
            ...
            if (err) {
                ...
                error = true;
                syraUtil.getFusionPageMeta(event.target).winModel.showFieldError(event.target, [session.getlabel("error", err, lerr)])
            }
            else {
