...
_evtProcWindow.sap = function(){
    ...
    function _showSessInfo(sapController){
        var diagnoses = [], info = sapController._session.getSessInfo(), boxParams, boxOpts, fieldCxt = sapController.getCurrInst();
        ...
        msgBox.setMsgBoxDefaultProp(boxParams, 0, 180, null, null, true);
        boxOpts = msgBox.getMsgBoxOpts.call(sapController, boxParams);
        boxOpts.callback = function () {
            sapController._wndwsStack.setFocus(fieldCxt.win, sapUtil.focusStyle.std.code, fieldCxt.xid, fieldCxt.nl || 1, true);                           
        }
            ...
            switch (keyCode) {
               case 73:
                    if(event.data.nativeEvt.ctrlKey && event.data.nativeEvt.altKey) {
                        _showSessInfo(this.sapController);
                    }
                    break;
                case keys["VK_TAB"]:
                case keys["VK_ESCAPE"]:
