...
_evtProcWindow.sap = function(){
    ...
    function toggleCheckFieldAfterClick(reply, evtHandlingObj, $field, fieldIst, lastSrvValue){
        var checkValue = evtHandlingObj.sapController.getEntityValueInReply(reply, fieldIst), value, chain = null;
        if(checkValue == null && checkValue == undefined) {
            // Ok, check has not been updated by the server after focus/click, we can process post local update...
            value = util.duplicateObj(fieldIst);
            value.v = lastSrvValue == sapUtil.check.off ? sapUtil.check.on : sapUtil.check.off;
            evtHandlingObj.sapController.forceValue(value);
    ...
    function onFieldClick(event){
        var processed = undefined, fieldIst, $fusionPageMeta, $fieldMeta, readOnlyByModel = false, isFieldCheck = false, fieldType;
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target)) && ($fieldMeta = syraUtil.getMetaFromObject(event.target)) && ($fusionPageMeta.winModel.getProto())[$fieldMeta.$bind]) {
            ...
            if(event.target.$field && (fieldType = event.target.$field.$type)) {
                isFieldCheck = fieldType == syraUtil.dataTypes.booleanType ? true : false;
             }
            ...
            if (!this.sapController.hasFocus(fieldIst) && !readOnlyByModel) {
                ...
                if(event.data.doEvent) {
                    ...
                    event.result.post = {"exposeReply": true, "callback": toggleCheckFieldAfterClick, "args": [this, event.target, fieldIst, event.target.currentValue]};
                }
                processed = true;
            }
            else if(!readOnlyByModel && isFieldCheck) {
