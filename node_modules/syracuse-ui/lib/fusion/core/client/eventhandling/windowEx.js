...
_evtProcWindow.sap = function(){
    ...
    function _openFieldCtx(reply, evtHandlingObj, initialEvent, field, fieldInfo){
        ...
        if (ctx && ctx.length > 0) {
            ...
            fieldInfo.$fusionPageMeta.winModel.setCurrCtxPopup(field);
        }
            ...
            if ((pickerType = event.data.pickerType)) {
                switch (pickerType) {
                    case "contextMenu":
                        ...
                        isCtx = true;
                        if(fieldInfo && !event.target._popupMenus) {
                            // Ctx popup not already displayed
                            act = "SRV_OPEN_CTXMENU";
                            actEx = "SUP_FIELD_CTX";
                        }
                        else if (event.target._popupMenus && event.data.doEvent) {
