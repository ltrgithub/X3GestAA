...
_evtProcWindow.sap = function(){
    ...
    function _getFieldInfo(field){
        var fieldInfo = {"readOnlyByModel": false, "isRadio": false, "isCheck": false, "isIco": false};
    ...
    function onFieldClick(event){
        var processed = undefined, fieldIst, $fusionPageMeta, $fieldMeta, readOnlyByModel = false, isRadio = false, isCheck = false, isIco = false, $field, newVal;
        if (event.target && ($fusionPageMeta = syraUtil.getFusionPageMeta(event.target)) && ($fieldMeta = syraUtil.getMetaFromObject(event.target)) && ($fusionPageMeta.winModel.getProto())[$fieldMeta.$bind]) {
            ...
            if(($field = event.target.$field)) {
                ...
                if(!isCheck && $field.$type == syraUtil.dataTypes.choiceType) {
                    ...
                    isIco = $field.$type == syraUtil.dataTypes.iconType ? true : false;
                }
            }
            // What should we do with this event?
            if (!readOnlyByModel && !this.sapController.hasFocus(fieldIst)) {
                event.result = {"$act": !isIco ? requestActions.getSrvAction("SRV_GETFOCUS") : requestActions.getSrvActionEX("SUP_FIELD_ICOCLICK")};
                ...
                if(event.data.doEvent) {
                    ...
                    event.result.post = {"exposeReply": true, "callback": _toggleFieldAfterClick, "args": [this, event.target, fieldIst, newVal]};
                }
                processed = true;
            }
            else if(!readOnlyByModel) {
                if (isCheck || isRadio) {
    ...
    function onFieldClickPicker(event){
        var processed = undefined, fieldInfo = _getFieldInfo(event.target), hasFocus;
        if (fieldInfo) {
            hasFocus = this.sapController.hasFocus(fieldInfo.fieldIst);
            if (!fieldInfo.readOnlyByModel) {
                event.result = {"$act": hasFocus ? requestActions.getSrvAction("SRV_OPEN_CTXMENU") : requestActions.getSrvActionEX("SUP_FIELD_CTX")};
                event.result.params = {"target": fieldInfo.fieldIst};
                if(!hasFocus) {
                    event.result.params.sudoV = [null, fieldInfo.field.getDataValue()];
                }
                if(event.data.doEvent) {
                    event.result.post = {"exposeReply": true, "callback":_openFieldCtx, "args": [this, event, event.target, fieldInfo.fieldIst]};
                }
                processed = true;
            }
        }
