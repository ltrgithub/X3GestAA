...
_evtProcWindow.sap = function() {
	this.eventListeners = function() {
		return [
		["wdgt.win.keydown", onKeyDown, true, true], ["wdgt.win.dblclick", onDblClick, true, true], ["wdgt.win.click", onClick, true, true], ["wdgt.win.close", onClose, true, true], ["wdgt.win.closeall", onCloseAll, true, true], ["wdgt.field.keydown", onFieldKeyDown, true, true], ["wdgt.field.click", onFieldClick, true, true], ["wdgt.field.mousedown", onFieldClick, true, true], ["wdgt.field.picker", onFieldClickPicker, true, true], ["wdgt.field.change", onFieldChange, true, true], ["wdgt.field.calendarclosed", onDatePickerClose, true, true], ["wdgt.field.keyup", onFieldKeyUp, false, false], ["wdgt.cell.keyup", onFieldKeyUp, false, false], ["wdgt.file.upload", onFileUpload, true, true]];
	};
	...
	function onClick(event) {        
		var processed = undefined,
			$$field, currIst, $$cont, $$fieldDocLink;        
		if (event.target && ($$field = $(event.data.nativeEvt.target))) {            $$cont = $$field.closest("." + syraUtil.pageSectionK.body);            
			if (($$cont && $$cont.length > 0) || _.find(syraUtil.pageSectionK, function(cssK) {
				...
				if (!$$fieldDocLink || $$fieldDocLink.length <= 0 || !$$fieldDocLink.hasClass("s-field-file-link")) {                    currIst = this.sapController.getCurrInst(true);                    event.target.setFocus(true, null, currIst.xid, (currIst.nl || 0), (currIst.edit === null ? true : currIst.edit));                    processed = true;                
				}            
			}        
		}        
		return processed;    
			...
			if ((pickerType = event.data.pickerType)) {
				switch (pickerType) {
					...
					case "date":
						...
						break;
					case "help":
					   evtTools.goToHelpField( _getFieldInfo(event.target));
					   return true;
					   break;
					default:
