...
_evtProcBlockEx.sap = function() {
	...
	function _ensureIstFromTarget(target) {
		var xid, winId, ist = null,
			objMeta = target ? syraUtil.getMetaFromObject(target) : null;
		if (objMeta) {
			xid = objMeta[sapUtil.metaNameMap.xid];
			winId = syraUtil.getFusionPageMeta(target);
			if (winId && (winId = winId.winModel) && (winId = winId.getWinId()) && xid) {
				// Try to retrieve instance  for special entities... to complete if necessary
				if (sapUtil.isTabScreen(xid)) {
					ist = sapUtil.makeIst(winId, sapUtil.specialScrnId.gfolder, null, true);
				}
			} else if (winId && objMeta.$bind && sapUtil.isButtonScreen(objMeta.$bind)) {
				ist = sapUtil.makeIst(winId, objMeta.$bind, null, true);
			}
		}
		return ist;
	}
