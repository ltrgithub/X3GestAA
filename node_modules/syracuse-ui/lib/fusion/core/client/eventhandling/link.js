...
_evtProcLink.sap = function() {
	...
	function onClick(event, ist) {
		var processed = undefined,
			act = null,
			$fusionPageMeta, $itemMeta, itemIst, actEx, $menu;
		if (event.target && !event.target.$isDisabled) {
			...
			if ((act = event.target.$act) || (act = $menu && $menu.$act)) {
				...
				// Special link
				$fusionPageMeta = event.target.boxParent ? syra_fusion.syraUtil.getFusionPageMeta(event.target.boxParent) : null;
				itemIst = $fusionPageMeta && ist ? syra_fusion.sapUtil.makeIst($fusionPageMeta.winModel.getWinId(), ist, null, true) : null;
				if (itemIst) {
					// Item is valid, we retrieve action code for this link. Warning, following SAM #111357, and according legacy client behaviour, we always notify server with super action (Focus+pahe up/down)
					...
					if (act) {
						event.result = {
							"$act": requestActions.getSrvActionEX(act),
							"params": {
								"target": itemIst
							}
						};
