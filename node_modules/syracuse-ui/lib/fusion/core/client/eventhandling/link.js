    ...
    function onClick(event){
        ...
        if (event.target && !event.target.$isDisabled) {
            if (event.target.$act && event.target.$act > 0) {
                ...
                if (event.target.$url) {
                    // Special link
                    $itemMeta = event.target.boxParent ? syraUtil.getMetaFromObject(event.target.boxParent) : null;
                    $fusionPageMeta = $itemMeta ? syraUtil.getFusionPageMeta(event.target.boxParent) : null;
                    itemIst = $fusionPageMeta && $itemMeta && $itemMeta.$bind ? sapUtil.makeIst($fusionPageMeta.winModel.getWinId(), $itemMeta.$bind) : null;
                    if (itemIst) {
                        // Item is valid, we retrieve action code for this link
                        focus = sapUtil.cmpIst(this.sapController.getCurrInst(), itemIst)
