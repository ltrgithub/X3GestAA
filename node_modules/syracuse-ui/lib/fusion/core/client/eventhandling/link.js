...
_evtProcLink.sap = function() {
	...
	function onClick(event, ist) {
		var processed = undefined, act = null, $fusionPageMeta, $itemMeta, itemIst, focus, actEx;
		if (event.target && !event.target.$isDisabled) {
			if ((act = event.target.$act)) {
				...
				event.opts = {"$link": event.target.$bind};
				if (event.target.page.$fusionPageMeta && event.target.page.$fusionPageMeta.winModel && event.target.page.$fusionPageMeta.winModel._mdata.name === "_FORMULA") {
