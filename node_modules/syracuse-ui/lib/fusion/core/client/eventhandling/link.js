	...
	function onClick(event, ist) {
		...
		if (event.target && !event.target.$isDisabled) {
			...
			if ((act = event.target.$act) || (act = $menu && $menu.$act)) {
				...
				if (event.target.page && event.target.page.$fusionPageMeta && event.target.page.$fusionPageMeta.winModel && event.target.page.$fusionPageMeta.winModel._mdata.name === "_FORMULA") {
