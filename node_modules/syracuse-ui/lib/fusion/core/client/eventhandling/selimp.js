...
_evtProcSelimp.sap = function() {
	this.eventListeners = function() {
		return [
		["sap.extdlg.loaded", onDlgLoaded, false, false], ["sap.extdlg.activate", onDlgActivate, false, false], ["sap.extdlg.abort", onAbort, false, false], ["wdgt.link.click", onClick, true, true], ["wdgt.win.close", onClose, true, true], ["wdgt.record.action", onRecordAction, true, true], ["wdgt.record.picker", onRecordSel, true, true], ["wdgt.grid.selline", onRecordSel, true, true], ["wdgt.cell.click", onRecordSel, true, true], ["wdgt.field.change", onFieldchange, true, true]];
	};
	...
	function _getActionRqst(linkId, rqst) {
		...
		switch (linkId) {
			case "$save":
				boundsFields = syraUtil.getBoundFldsFromPage(this._ctx.currDlg);
				if (!this._ctx.isMainDlg) {
					printerHelpers.checkAdvancedSettings(boundsFields);
	...
	function onFieldchange(event) {
		...
		if (event.target && this._ctx.isMainDlg) {
			if ((bind = syraUtil.getMetaFromObject(event.target)) && printerHelpers.shouldCheckOnchange(bind.$bind)) {
				printerHelpers.checkSettings(syraUtil.getBoundFldsFromPage(this._ctx.currDlg), true, bind.$bind);
			}
		}
		return processed;
	}
	...
	function onDlgLoaded(event, dialogId, isMainDlg) {
		...
		if (page) {
			...
			if (isMainDlg) {
				...
				printerHelpers.checkAdvancedSettings(boundFields);
			}
	...
	function onRecordAction(event) {
		var processed = undefined,
			ret, linkId;
		if ((linkId = event.target.$bind) && linkId !== "$recordCard" && this._ctx.currPrinterRcd) {
			event.result = {};
