...
function _getArrayMetaLink(fieldInfo, line, rcdLevel, arrayLevel, links, sapController, raz) {
	var change = {
		"$isPartialDelta": true
	...
	var list = fieldInfo.array,
		...
		iStatus, dStatus, isGridFull, lastLine;
	if (arrayLevel && uuid) {
	...
	openFieldCtx: function(reply, evtHandlingObj, initialEvent, field, fieldInfo, exclude) {
		if (field && !field.disposed) {
			...
			if (arrayLevel || rcdLevel !== null) {
				// Issue 2330 - meta = _getArrayMetaLink(fieldInfo, fieldInfo.nl, rcdLevel, arrayLevel, null,null, true);
				// Issue 2330 - field.page.applyChange(meta);
				...
				// Issue 2330 - field.applyMetaData({"$links": null});
