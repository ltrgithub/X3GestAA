...
_processOnClickForTypes[syraUtil.dataTypes.choiceType] = false;
_processOnClickForTypes[syraUtil.dataTypes.flash] = false;
...
function _getArrayMetaLink(field, fieldInfo, line, rcdLevel, arrayLevel, links, sapController, raz) {
	...
	if (uuid !== null) {
		...
		if (!raz && cap) {
			...
			} else if (arrayLevel && (cap["deleteAll"] || cap["copy"])) {
...
exports.CommonEventHandlingTools = {
	getGridRecordLinks: function(grid, capabilities, bind, fieldInfo, sapController, links) {
		...
		if (capabilities.insert) {
			...
			links.$create = {
				"$isHidden": isGridFull || lastLine || isGridDisabled || grid._$orderBy ? true : (status === null ? false : !status)
			};
			defined = true;
		}
		if (capabilities["delete"]) {
			...
			links.$delete = {
				"$isHidden": isGridDisabled || (lastLine && !isGridFull) || grid._$orderBy ? true : (status === null ? false : !status)
			};
			defined = true;
		}
		return defined;
	...
	getGridLinks: function(grid, capabilities, bind, fieldInfo, sapController, links) {
		...
		if (capabilities.deleteAll) {
			...
			links.$delete = {
				"$isHidden": (isGridDisabled || grid._$orderBy) ? true : !status,
			...
			if (capabilities.copy) {
				links.$cut = {
					"$isHidden": (isGridDisabled || grid._$orderBy) ? true : ((status && recordsSelected.length && exports.CommonEventHandlingTools._isArraySelectionContiguous(recordsSelected)) ? false : true)
				};
			}
			defined = true;
		}
		if (capabilities.copy) {
			links.$copy = {
				"$isHidden": (isGridDisabled || grid._$orderBy) ? true : (recordsSelected.length && exports.CommonEventHandlingTools._isArraySelectionContiguous(recordsSelected)) ? false : true
			};
			links.$paste = {
				"$isHidden": (isGridDisabled || grid._$orderBy) ? true : !grid.x3LinesCopied
			};
		}
	...
	discardFieldEvent: function(evtName, fldInfo) {
		...
		switch (evtName) {
			case "click":
				discard = fldInfo.isCheck || fldInfo.isRadio || !_processOnClickForTypes[fldInfo.type];
				break;
			case "mousedown":
				discard = fldInfo.isCheck || fldInfo.isRadio || _processOnClickForTypes[fldInfo.type];
				break;
	...
	openDatePicker: function(succeeded, result, evtHandlingObj, initialEvent, field, callerFieldInfo) {
		...
		if (field && !field.disposed && evtHandlingObj.sapController.hasFocus(callerFieldInfo.fieldIst) && !syraUtil.getPopupDateFromField(field)) {
			if (!succeeded) {
				...
				value = result ? parseInt(svcFmt.getReplyResult(result), 10) : (result === "" ? result : null);
			}
	...
	_isArraySelectionContiguous: function(recordsSelected) {
		if (recordsSelected.length < 1) return false;
		var rec = recordsSelected[0];
		for (var i = 1, l = recordsSelected.length; i < l; i++) {
			if ((recordsSelected[i].$serverIndex - rec.$serverIndex) > 1) return false;
	...
	toggleFieldAfterClick: function(reply, evtHandlingObj, $field, fieldIst, newValue, notifySrv, act, oldValue, fromGrid) {
		...
		if (fieldValue && fromGrid) {
			...
			if (oldValue != fieldValue) {
				newValue = (fieldValue == sapUtil.check.off ? sapUtil.check.on : sapUtil.check.off);
			}
			oldValue = fieldValue;
		}
		if (fieldValue == null || fieldValue === undefined || fieldValue === oldValue) {
			// Ok, field value has not been updated by the server after focus/click, we can process post local update... and finally we inform the server by this change
			if (notifySrv) {
				chain = {
					"$act": act,
					"params": {
	...
	applyKeyI: function(opt) {
		if (opt.data.shortcuts.esc && opt.data.shortcuts.alt) {
			...
			var sapController = syraUtil.getFusionController(opt.target)._sapController;
