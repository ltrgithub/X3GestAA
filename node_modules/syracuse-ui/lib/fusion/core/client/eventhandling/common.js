...
function _getArrayMetaLink(fieldInfo, line, rcdLevel, arrayLevel, links, sapController) {
	...
	var list = fieldInfo.array,
		...
		iStatus, dStatus, isGridFull, lastLine, collLength;
	if (arrayLevel && uuid) {
	...
	if (uuid !== null) {
		...
		if (cap) {
			...
			if (rcdLevel && (cap.insert || cap["delete"])) {
				lastLine = ((collLength = list.dataset.length) > 0) ? fieldInfo.winModel.getDataStoreValue(collName, list.dataset.length, false, false, {
					"rawVal": true
				}) : null;
...
exports.CommonEventHandlingTools = {
	...
	goToHelpField: function(fieldInfo) {
		var url, baseHelpUrl, HelpUrl = "$baseUrl";
		if (fieldInfo.$field && fieldInfo.$field.$links && fieldInfo.$field.$links.$help && (url = fieldInfo.$field.$links.$help.$url)) {
			baseHelpUrl = (fieldInfo.winModel.syraModel.$prototype[HelpUrl] || "http://uranus2:8080/AdxDoc_DOCV7X3/FRA/");
			url = url.replace("{" + HelpUrl + "}", baseHelpUrl);
			window.open(url, (fieldInfo.$field.$links.$help.$target || "_blank"));
		}
	}
};