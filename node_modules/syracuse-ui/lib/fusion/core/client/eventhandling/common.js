...
exports.CommonEventHandlingTools = {
	getGridRecordLinks: function(grid, capabilities, bind, fieldInfo, sapController, links) {
		...
		if (capabilities.insert) {
			...
			links.$create = {
				"$isHidden": isGridFull || lastLine || isGridDisabled ? true : (status === null ? false : !status)
			};
			defined = true;
		}
		if (capabilities["delete"]) {
			...
			links.$delete = {
				"$isHidden": isGridDisabled || (lastLine && !isGridFull) ? true : (status === null ? false : !status)
			};
			defined = true;
		}
		return defined;
	...
	getGridLinks: function(grid, capabilities, bind, fieldInfo, sapController, links) {
		...
		if (capabilities.deleteAll) {
			...
			links.$delete = {
				"$isHidden": isGridDisabled ? true : !status,
			...
			if (capabilities.copy) {
				links.$cut = {
					"$isHidden": isGridDisabled ? true : ((status && recordsSelected.length && exports.CommonEventHandlingTools._isArraySelectionContiguous(recordsSelected)) ? false : true)
				};
			}
			defined = true;
		}
		if (capabilities.copy) {
			links.$copy = {
				"$isHidden": isGridDisabled ? true : (recordsSelected.length && exports.CommonEventHandlingTools._isArraySelectionContiguous(recordsSelected)) ? false : true
			};
			links.$paste = {
				"$isHidden": isGridDisabled ? true : !grid.x3LinesCopied
			};
		}
