...
function _getArrayMetaLink(fieldInfo, line, rcdLevel, arrayLevel, links, sapController) {
    var change = {"$isPartialDelta": true}, bind = fieldInfo.$fieldMeta.$bind, collName = bind ? sapUtil.getBlockInst(bind) : null, filled = false;
    var root, uuid = bind && fieldInfo && fieldInfo.winModel, isListDisabled;
    var list = fieldInfo.array, cap = list && list.$capability, iStatus, dStatus, isGridFull, lastLine, collLength;
    if(arrayLevel && uuid) {
        root = change.$properties = {};
        root = root[collName] = {};
    }
    else if ((uuid = uuid ? fieldInfo.winModel.getCollRcdUUID(collName, fieldInfo.nl) : null) !== null) {
    ...
    if(uuid !== null) {
        ...
        if(cap) {
            ...
            if(rcdLevel && (cap.insert || cap["delete"])) {
                lastLine = ((collLength = list.dataset.length) > 0) ? fieldInfo.winModel.getDataStoreValue(collName, list.dataset.length, false, false, {"rawVal": true}) : null;
                isGridFull = lastLine ? !sapUtil.isEmptyGridDataRecordEx(syraUtil.getCollFieldsFromCollObj(list), lastLine) : false;
                lastLine = collLength === fieldInfo.nl;
                if(!root.$links) {
                    root.$links = {};
                }
                if(cap.insert) {
                    iStatus = sapController.isActionEnabled(fieldInfo.winModel.getWinId(), requestActions.getSrvAction("SRV_INSERTLINEGRID"));
                    root.$links.$create = {"$isHidden": isGridFull || lastLine || isListDisabled ? true : (iStatus === null ? false : !iStatus)};
                }
                if(cap["delete"]) {
                    dStatus = sapController.isActionEnabled(fieldInfo.winModel.getWinId(), requestActions.getSrvAction("SRV_DELETELINEGRID"));
                    root.$links.$delete = {"$isHidden": isListDisabled || (lastLine && !isGridFull) ? true : (dStatus === null ? false : !dStatus)};
                }
                filled = true;
            }
            else if(arrayLevel && cap["delete"]) {
                dStatus = sapController.isActionEnabled(fieldInfo.winModel.getWinId(), requestActions.getSrvAction("SRV_DELETELINESGRID"));
                root.$links.$delete = {"$isHidden": isListDisabled ? true : (dStatus === null ? false : !dStatus)};
                filled = true;
            }
        }
    }
...
exports.CommonEventHandlingTools = {
    ...
    openFieldCtx:function(reply, evtHandlingObj, initialEvent, field, fieldInfo, exclude){
    	if(field && !field.disposed){
	        var ctxEx = [], lkEx = {}, ctx = evtHandlingObj.sapController.getTargetCtxMenuFromReply(reply), meta, arrayLevel, rcdLevel = null, openCtx = false;
	        if (ctx) {
				// Get items
			...
			if (field.arrayLevel && field.arrayLevel === "array") {
				...
				if(fieldInfo.isSortedArray){
					ctxEx.push({
						"txt" : evtHandlingObj.controller.getSession().getlabel("text", 201, "Cancel sort"),
						"act" : "1",
						"id" : "$cancelSort"
					});
				}
            ...
            if(arrayLevel || rcdLevel !== null) {
                if ((meta = _getArrayMetaLink(fieldInfo, fieldInfo.nl, rcdLevel, arrayLevel, openCtx ? lkEx : null, evtHandlingObj.sapController))) {
