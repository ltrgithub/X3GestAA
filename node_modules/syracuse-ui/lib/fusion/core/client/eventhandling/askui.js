...
_evtProcAskui.sap = function() {
	...
	function _initUploadDlg(self, dialog, dialogName) {
		...
		if ((boundFields = syraUtil.getBoundFldsFromPage(dialog)) && (statementParams = self._ctx.statementParams)) {
			...
			self._ctx.upLoadStatus.field._inputFile.click();
		}
	}
	...
	function onFileUpload(event, data) {
		...
		if (_isOurUploadFld(this, event.target) && event.data && (files = event.data.files) && files.length > 0) {
			syraUtil.showDiagnoses(null, this._ctx.currDlg);
			...
			if (this._ctx.statementParams.UIMaxLength && files[0].size > maxSize) {
				...
				syraUtil.showDiagnoses([util.makeDiagnosis(messErr, "error")], this._ctx.currDlg);
				sizeOk = false;
				processed = true;
			}
			if (sizeOk) {
	...
	function onFileUploadSucceeded(event) {
		...
		if (_isOurUploadFld(this, (docField = event.target))) {
			...
			setTimeout(function() {
				if (docField && docField.localize) {
					docField.setLegend(event.file.name + " " + docField.localize.fdocUploaded);
				}
	...
	function onFileUploadRemove(event) {
		...
		if (_isOurUploadFld(this, docField)) {
			...
			if (docField.localize) {
				docField.setLegend(docField.localize.fdocDropSelectFile);
			}
			document.controller.deleteFile(docField.$dataUrl, docField.articleParent, function(data, response) {
				docField.setDataValue(null);
