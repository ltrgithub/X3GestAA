...
_evtProcList.sap = function(){
    this.eventListeners = function(){
        return [["wdgt.list.toggle", beforeToggle, true, true], 
                ...
                ["wdgt.list.picker",clickPicker,true,true],
                ["wdgt.list.calendarclosed", onDatePickerClose, true, true]
                ];
    };
        ...
        var session;
         if (!syraUtil.getPopupDateFromField(field)) {
        	if(succeeded){
                ...
                callerFieldInfo.winModel.showFieldError(field, [session.getlabel("error", "22", "Date entered is invalid; calendar can not be opened")],true);
            }
	    }
    ...
    function selectLine(event){
    	var fieldInfo=_getFieldInfo(event);
    	return (fieldInfo.list.isPick)?_selectLinePicking(event,fieldInfo):(fieldInfo.list.isHerarchy?_selectLineHierarchy(event,fieldInfo):(fieldInfo.list.isFormula?_selectLineFormula(event,fieldInfo):_selectLineList(event,fieldInfo)));  	
    }
    ...
    function _selectLineFormula(event,fieldInfo){
		if( fieldInfo.isSpecialNode ){
			event.result = {$act : null ,params : {target:{} }}
			// send a 1071 action 
			event.result.$act=requestActions.getSrvAction("SRV_ELEMENT_FORMULE");
			event.result.params.std=[fieldInfo.$key];
			return true;
		}else{
			if( !fieldInfo.winModel.$itemForm ){
				...
				if( found ){
					fieldInfo.winModel.$itemForm = found;
				}
			}
			var itemForm = fieldInfo.winModel.$itemForm;
			...
			itemForm.setInputValue(itemForm.$formulaFull.replace('%',''));
    ...
    function _getFieldInfo(event){
        ...
        fieldInfo.list.isFormula=$fusionPageMeta && $fusionPageMeta.winModel._mdata.name === "_FORMULA"?true:false;
        fieldInfo.list.isHerarchy=(event.target.$item && event.target.$item.$isNavigationList && fieldInfo.array && fieldInfo.array.treeList)?true:false;
        ...
        if(fieldInfo.list.isRecursive){
        	...
        	fieldInfo.isLeaf=(event.data.field && event.data.field.treeNode && event.data.field.treeNode.children && event.data.field.treeNode.children.length>0)?false:true;
        }else if(fieldInfo.list.isHerarchy ){
		...
		}else if( fieldInfo.list.isFormula ){
			if( fieldInfo.data.recTarget ){
				var srvIdx = fieldInfo.data.recTarget.dataRecord.$serverIndex
				if( (srvIdx === 0 || srvIdx === 1) && fieldInfo.data.recTarget.dataRecord.bA0){
					fieldInfo.isSpecialNode = true;
					fieldInfo.$key = fieldInfo.data.recTarget.dataRecord.bA0;
				}else{ 
