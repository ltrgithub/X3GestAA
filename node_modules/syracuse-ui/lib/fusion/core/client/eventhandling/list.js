...
_evtProcList.sap = function(){
    ...
    //TODO : 1062 depliage hierarchie recursive
    function expendLine(event){
		...
		if (fieldInfo.list.isRecursive && fieldInfo.isLeaf) {
			...
			actionsMap.push(requestActions.getSrvActionEX("SUP_LIST_TREEEXPANDLINE"));
			//1044 + 1063
			if (fieldInfo.firstFocus) {
				args.push([])
    ...
    function _selectLinePicking(event,fieldInfo){
    	...
    	if(fieldInfo.isLeaf){
    		...
    		//TODO : Select/unselect all descending leafs and mark ascending branch as selected/unselected or partial selected
    		var leafs=[],branchs=[],keys=[]
    		_selectLinesPick(leafs,branchs,keys,fieldInfo.array._store.findRecordByServerIndex(event.data.recTarget.dataRecord.$serverIndex),checked)
    		if(leafs.length>0){
           		event.target._applyPartialDelta(leafs)
           	}
           	if(branchs.length>0){
           		event.target._applyPartialDelta(branchs)
           	}
    ...
    function _getFieldInfo(event){
        ...
        fieldInfo.list.isHerarchy=(event.target.$item && event.target.$item.$isNavigationList && fieldInfo.array && fieldInfo.array.treeMap)?true:false;
        fieldInfo.list.isPick=(event.target.$item && event.target.$item.$isNavigationList  && event.target.$item.$selectMode === "multi")?true:false ;
        fieldInfo.list.isRecursive=(fieldInfo.list.isHerarchy && false);
        ...
        if(fieldInfo.list.isHerarchy && event.data && event.data.recTarget && event.data.recTarget.dataRecord){
        	record = fieldInfo.array._store.findRecordByServerIndex(event.data.recTarget.dataRecord.$serverIndex);
        	...
        	record = fieldInfo.array._store.findRecordByServerIndex(nl-1);
