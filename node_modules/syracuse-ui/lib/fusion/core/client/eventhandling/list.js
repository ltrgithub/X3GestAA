...
_evtProcList.sap = function(){
	...
	function operatorChange(event) {
		var sap = this.sapController, listXid = _listIst(event, sap, event.target.getXid()), hasFocus = _hasFocus(event, sap, event.target.getXid());
    ...
    function selectLine(event){
        var processed = undefined, $fusionPageMeta, sap = this.sapController,srvAct, args=[],cuist;
				...
				case keys["VK_TAB"]:
					if (!event.data.nativeEvt.shiftKey) {
						...
						event.result.post = {
							...
							"args" : [this, event, event.target, fieldInfo, true]
						};
						processed = true;
					}	
					break;
    ...
    function nextQuickSel(reply, evtHandlingObj, initialEvent, field, fieldInfo,next){
    	var exec=sapUtil.cmpIstBlock(fieldInfo.blockIst,evtHandlingObj.sapController.getTargetIstFromReply(reply)),field
    	if(exec){
    		if(field = fieldInfo.array.getNextQuickSelField(fieldInfo.$fieldMeta.$bind,next)){
    			field[0].focus()
    		}
