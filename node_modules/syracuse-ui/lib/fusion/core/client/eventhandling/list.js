...
var filterBuilder = require('syracuse-ui/lib/field/array/gridMode/tools/filterBuilder');
var _evtProcList = {};
...
_evtProcList.sap = function() {
	...
	function operatorChange(event) {
		...
		if ((event.data.operator === "none" && (value == undefined || value == null || value == "")) || ((value !== undefined && value !== null && value !== ""))) {
			var filters = filterBuilder.fusion.getQuickSelValues(event.target);
			event.result = {
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.enter) {
			filters = filterBuilder.fusion.getQuickSelValues(fieldInfo.array);
			opt.result = {
				"$act": requestActions.getSrvActionEX("SUP_LIST_FILTER")
			};
			opt.result.params = {
	...
	function nextQuickSel(reply, evtHandlingObj, fieldInfo, $bind, next) {
		...
		if (exec) {
			if (field = filterBuilder.fusion.getNextQuickSelField(fieldInfo.array, $bind ? $bind : fieldInfo.$fieldMeta.$bind, next)) {
				setTimeout(function() {
					field[0].focus();
				}, 10);
			}
