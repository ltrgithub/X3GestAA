...
var _filterBuilder = require('syracuse-ui/lib/field/array/gridMode/tools/filterBuilder');
var _evtProcList = {};
...
_evtProcList.sap = function() {
	...
	this._triggerFilter = function(opt, $bind, next) {
		...
		opt.result = {
			...
			params: {
				target: fieldInfo.blockIst,
				sudo: [_filterBuilder.fusion.getQuickSelValues(fieldInfo.array), null]
			}
		};
	...
	function _selectLinePicking(event, fieldInfo) {
		...
		if (fieldInfo.isLeaf) {
			...
			_selectLinesPick(leafs, branchs, keys, syra_store.findRecordByServerIndex(fieldInfo.array, event.data.recTarget.dataRecord.$serverIndex), checked, fieldInfo);
	...
	function nextQuickSel(reply, evtHandlingObj, fieldInfo, $bind, next) {
		...
		if (exec && !fieldInfo.array.disposed) {
			if (field = _filterBuilder.fusion.getNextQuickSelField(fieldInfo.array, $bind ? $bind : fieldInfo.$fieldMeta.$bind, next)) {
				setTimeout(function() {
					field[0].focus();
				}, 10);
			}
	...
	function _getFieldInfo(event) {
		...
		if (fieldInfo.list.isHerarchy) {
			record = syra_store.findRecordByServerIndex(fieldInfo.array, event.data.recTarget.dataRecord.$serverIndex);
			...
			record = syra_store.findRecordByServerIndex(fieldInfo.array, nl - 1);
			if (record)
