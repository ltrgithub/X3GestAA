...
_evtProcList.sap = function() {
	...
	function beforeToggle(event, listXid) {
		...
		var listIst = sapUtil.makeIst(syra_fusion.syraUtil.getFusionPageMeta(event.target).winModel.getWinId(), listXid, this.sapController.getCurrInst().nl);
	...
	function checkDate(evtHandlingObj, event, fieldInfo, value) {
		...
		event.result.params = svcFmt.buildRqst(svcFmt.fmtSvc.toRaw, value, fieldInfo.fieldIst.fmt, syra_fusion.syraUtil.getFieldType(event.target), null);
		event.result.post = {
	...
	function openDatePicker(succeeded, result, evtHandlingObj, initialEvent, field, callerFieldInfo) {
		...
		if (!syra_fusion.syraUtil.getPopupDateFromField(field)) {
			if (succeeded) {
