...
function _displayError(sap, error,callBack, notifyComplete){
	var diagnoses,ourAjaxErr;
	if ((ourAjaxErr = util.isOurAjaxErr(error))) {
		diagnoses = util.makeDiagnosesFromOurAjaxErr(ourAjaxErr, sap._session);
	}
	else {
		diagnoses = util.getDiagnosesFromAjax(error, sap._session);
	}
	sap._sapException(null, diagnoses, null, error,callBack, true===notifyComplete);
}

/* "Server page input" event handling class for "Left List" widget */
_evtProcList.sap = function(){
    this.eventListeners = function(){
        return [["wdgt.list.toggle", beforeToggle, true, true], ["wdgt.list.selline", changeLine, true, true], ["wdgt.list.toggled", afterToggle], ["wdgt.list.sortLocal", sortLocal, true, true]];
    };
