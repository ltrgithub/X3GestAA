...
_evtProcList.sap = function() {
	...
	function _selectLinePicking(event, fieldInfo) {
		var checked = event.data.nativeEvt.currentTarget.checked,
			actionsMap = [],
			srvAct, args = [];
		if (fieldInfo.isLeaf) {
	...
	function _selectLinesPick(leafs, branchs, keys, record, checked) {
		...
		if (record.treeNode) {
			for (var i = 0; i < record.treeNode.children.length; i++) {
				_selectLinesPick(leafs, branchs, keys, record.treeNode.children[i].record, checked);
			}
		}
	}
	...
	function _selectLineList(event, fieldInfo) {
		...
		if ((fieldInfo.list.isSelbox || fieldInfo.list.isChoose) && !event.data.nativeEvt.applyShortCut) {
