...
_evtProcList.sap = function(){
    	...
    	switch (event.data.pickerType) {
		    ...
		    case "date" :
		    	if(fieldInfo.firstFocus){
					...
					event.result.post = {
						"callback" : datePickerClick,
						"args" : [this,event,true,fieldInfo]
					};
					return true
		    	}else{
		    		datePickerClick(this,event, false, fieldInfo)
					return true
		    	}
		    	break;
		    default :
    ...
    function datePickerClick(evtHandling,event, process, fieldInfo){
        if (process && !syraUtil.getPopupDateFromField(event.target)) {
            event.result.post = {
                ...
                "args": [evtHandling, event, event.target, fieldInfo]
            };
        }
        else 
            if (!syraUtil.getPopupDateFromField(event.target)) {
                var fieldContext = evtHandling.sapController.getCurrInst();
    ...
    function _getFieldInfo(event){
        ...
        if(!fieldInfo.currIst.fmt){
        	fieldInfo.currIst.fmt= fieldInfo.winModel._getFmtColl(fieldInfo.xid,fieldInfo.blockIst.xid,(fieldInfo.nl?fieldInfo.nl:1))
        }
