...
_evtProcList.sap = function() {
	...
	this._triggerFilter = function(opt, $bind, next) {
		...
		opt.result = {
			...
			params: {
				target: fieldInfo.blockIst,
				sudo: [(fieldInfo.array.filter_row && fieldInfo.array.filter_row.fusion_getQuickSelValues()) || [], null]
			}
		};
	...
	function nextQuickSel(reply, evtHandlingObj, fieldInfo, $bind, next) {
		...
		if (exec && !fieldInfo.array.disposed) {
			if (field = fieldInfo.array.filter_row.fusion_getNextQuickSelField($bind ? $bind : fieldInfo.$fieldMeta.$bind, next)) {
				setTimeout(function() {
					field[0].focus();
				}, 10);
			}
