...
var filterBuilder = require('syracuse-ui/lib/field/array/gridBuilder/filter/filterBuilder');
var _evtProcList = {};
...
_evtProcList.sap = function() {
	this.eventListeners = function() {
		return [["wdgt.list.toggle", beforeToggle, true, true], ["wdgt.list.selline", selectLine, true, true], ["wdgt.list.toggled", afterToggle], ["wdgt.list.sortLocal", sortLocal, true, true], ["wdgt.list.operatorchange", operatorChange, true, true], ["wdgt.list.operatorclick", operatorClick, true, true], ["wdgt.list.keydown", keyDown, true, true], ["wdgt.list.applyShortCut", applyShortCut, true, true], ["wdgt.list.click", clickQuickSelField, true, true], ["wdgt.list.picker", clickPicker, true, true], ["wdgt.list.calendarclosed", onDatePickerClose, true, true]];
	};
	...
	function operatorChange(event) {
		...
		if ((event.data.operator === "none" && (value == undefined || value == null || value == "")) || ((value !== undefined && value !== null && value !== ""))) {
			var filters = filterBuilder.getQuickSelValues(event.target);
			event.result = {
			...
			event.result.post = {
				...
				"args": [this, fieldInfo, field.$item.$bind]
			};
	...
	function selectLine(event) {
		var fieldInfo = _getFieldInfo(event);
		return (fieldInfo.list.isPick) ? _selectLinePicking(event, fieldInfo) : (fieldInfo.list.isHerarchy ? (fieldInfo.list.isRecursive ? false : _selectLineHierarchy(event, fieldInfo)) : _selectLineList(event, fieldInfo));
	};

	function expendLine(fieldInfo, event) {
		var actionsMap = [],
			args = [];
		var treeNode = fieldInfo.target.treeNode;
		event.result = {
			$act: null,
			params: {
				target: null
		...
		if (fieldInfo.isLeaf && fieldInfo.$key && !treeNode.isOpened) {
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.enter) {
			filters = filterBuilder.getQuickSelValues(fieldInfo.array);
			opt.result = {
				"$act": requestActions.getSrvActionEX("SUP_LIST_FILTER")
			};
			opt.result.params = {
			...
			opt.result.params.sudo = [filters, null];
			processed = true;
		} else
		if (opt.data.shortcuts.tab) {
			...
			opt.result.post = {
				"exposeReply": true,
				"callback": nextQuickSel,
				"args": [this, fieldInfo, null, !opt.data.shortcuts.shift]
			};
			processed = true;
		}
		return processed;
	...
	function nextQuickSel(reply, evtHandlingObj, fieldInfo, $bind, next) {
		...
		if (exec && !fieldInfo.array.disposed) {
			if (field = filterBuilder.getNextQuickSelField(fieldInfo.array, $bind ? $bind : fieldInfo.$fieldMeta.$bind, next)) {
				setTimeout(function() {
					field[0].focus();
				}, 10);
			}
