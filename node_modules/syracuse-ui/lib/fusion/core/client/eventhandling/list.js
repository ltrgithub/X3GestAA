	...
	function _selectLinePicking(event, fieldInfo) {
		...
		if (fieldInfo.isLeaf) {
			...
			var leafs = [],
				branchs = [],
				keys = [];
			_selectLinesPick(leafs, branchs, keys, fieldInfo.array.helper.findRecordByServerIndex(fieldInfo.array, event.data.recTarget.dataRecord.$serverIndex), checked, fieldInfo);
			if (leafs.length > 0) {
	...
	function _selectLinesPick(leafs, branchs, keys, record, checked, fieldInfo) {
		...
		if (treeNode) {
			for (var i = 0; i < treeNode.children.length; i++) {
				_selectLinesPick(leafs, branchs, keys, fieldInfo.array.helper.findRecordByServerIndex(fieldInfo.array, treeNode.children[i].$uuid), checked, fieldInfo);
			}
		}
	}
