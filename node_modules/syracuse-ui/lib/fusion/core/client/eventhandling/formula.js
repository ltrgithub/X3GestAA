...
_evtProcFormula.sap = function(){
    ...
    function onLinkClick(event){
        ...
        var $menu = event.target.$item, act;
    ...
    function keyDown(event){
        var processed = undefined, fieldInfo = _getFieldInfo(event), keycode = event.data.nativeEvt.keyCode;
    ...
    function selectLine(event){
        var fieldInfo = _getFieldInfo(event), found;
    ...
    function _getFieldInfo(event){
        var nl = (event.data && event.data.line) ? event.data.line : 0, fieldInfo = evtTools.getFieldInfo(_getXElement(event.target), nl), $fusionPageMeta = syraUtil.getFusionPageMeta(event.target), record, treeNode = null;
    ...
    function _getActionId(actionExpr){
        var actStrg, actTab, ret = null, matches = actionExpr.match(/(\{.*?\})/g);
