...
_evtProcFormula.sap = function() {
	this.eventListeners = function() {
		return [
			["wdgt.list.selline", selectLine, true, true], ["wdgt.field.keydown", keyDown, true, true], ["wdgt.link.click", onLinkClick, true, true], ["wdgt.win.close", onClose, true, true], ["wdgt.win.closeall", onCloseAll, true, true], ["wdgt.win.validate", onValidate, true, true], ["wdgt.field.change", onFieldChange, true, true]
		];
	};
	...
	function onFieldChange(event) {
		...
		if (!fieldInfo.isCheck) return false;
		return helpUrl(event, event.target, fieldInfo);
	}

	function helpUrl(event, showHelp, fieldInfo) {
		var found = _findItem(fieldInfo, "help"),
			isChecked = showHelp ? (showHelp.getDataValue() == 2) : false;
		if (found) {
			if (isChecked) {
	...
	function selectLine(event) {
		...
		if (fieldInfo.isSpecialNode) {
			...
			if (fieldInfo.helpKey && (found = _findItem(fieldInfo, "help"))) {
				...
				helpUrl(event, _findItem(fieldInfo, "showHelp"), fieldInfo);
			}
			return true;
		}
	};
	...
	function _setItemFormula(fieldInfo) {
		if (!fieldInfo.target.page.itemFormula) {
			var found = _findItem(fieldInfo, "AA1");
			if (found) {
	...
	function _getFieldInfo(event) {
		...
		if (fieldInfo.data.recTarget) {
			...
			if ((!fieldInfo.isLeaf || treeNode.parentId == "") && fieldInfo.data.recTarget.dataRecord[fieldInfo.blockIst.xid + "0"]) {
				...
				fieldInfo.helpKey = fieldInfo.data.recTarget.dataRecord.extends4;
			}
		}
		return fieldInfo;
	};
