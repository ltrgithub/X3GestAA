...
_evtProcFormula.sap = function() {
	this.eventListeners = function() {
		return [
			["wdgt.grid.selline", selectLine, true, true], ["wdgt.field.keydown", keyDown, true, true], ["wdgt.field.keyup", keyUp, true, true], ["wdgt.field.click", click, true, true], ["wdgt.link.click", onLinkClick, true, true], ["wdgt.record.picker", clickRecordPicker, true, true], ["wdgt.win.close", onClose, true, true], ["wdgt.win.closeall", onCloseAll, true, true], ["wdgt.win.validate", onValidate, true, true]
		];
	};
	...
	function click(event) {
		...
		event.target.selectionEnd = event.target.$$input.prop("selectionEnd");
		event.target.selectionStart = event.target.$$input.prop("selectionStart");
		return true;
	}
		...
		if (fieldInfo.isSpecialNode) {
			...
			if (itemForm.boxParent.$opened) {
				if (!itemForm.formulaFull) {
					...
					else itemForm.formulaFull = itemForm.formulaFull.substr(0, Math.min(itemForm.caretPosition, itemForm.selectionStart)) + fieldInfo.formula + itemForm.formulaFull.substr(Math.max(itemForm.caretPosition, (itemForm.selectionEnd ? itemForm.selectionEnd : 0)));
				}
	...
	function _setItemFormula(fieldInfo) {
		if (!fieldInfo.target.page.itemFormula) {
			var found = _findItem(fieldInfo, "formula");
			if (found) {
