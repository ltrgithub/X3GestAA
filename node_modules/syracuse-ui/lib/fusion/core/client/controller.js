...
var _rqstSvc = {
    "sapPost": {"name": "Native protocol", "proxy": "sapHttpSvc", "method": "PUT", "subUrl": "requestSvc", "contentType": "application/json"},
    "sapDel": {"name": "Shutdown session", "proxy": "sapHttpSess", "method": "DELETE", "subUrl": null, "contentType": "application/json"},
    "format": {"name": "Format service", "proxy": "fmtHttpSvc", "method": "PUT", "subUrl": "formatSvc", "contentType": "application/json"},
    "print": {"name": "Print services", "proxy": "printHttpSvc", "method": "PUT", "subUrl": "print", "contentType": "application/json"}
};
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    postSapRqst: function(action, params, post){
        ...
        this._sendRequest(_rqstSvc.sapPost, data, "act=" + action, 
            ...
            function(error, notifyComplete) {
                var ourAjaxErr;
                if((ourAjaxErr = util.isOurAjaxErr(error))) {
                    self._ourSapRqstError(ourAjaxErr, notifyComplete);
                }
                else {
                    self._sapController._sapException(null, util.getDiagnosesFromAjax(error, self._session), null, error, null, notifyComplete);
                }
            }, true);
    },

    _ourSapRqstError: function(ourAjaxErr, notifyComplete, messCallback) {
        var diagnoses = util.makeDiagnosesFromOurAjaxErr(ourAjaxErr, this._session), recovery = null;
        var messErr = this._session.getlabel("error", "19", "Web server encountered following error during last request processing.");
        switch (ourAjaxErr.errCode) {
            case "KILL" : 
            ...
            function(error, notifyComplete) {
               // Force close...
                ...
                if(!self.delSessRetry) {
                    ...
                    if ((ourAjaxErr = util.isOurAjaxErr(error))) {
                        diagnoses = util.makeDiagnosesFromOurAjaxErr(ourAjaxErr, self._session);
                    }
                    else {
    ...
    formatRqst: function(action, params, post){
        var self = this;
        this._sendRequest(_rqstSvc.format, data, "act=" + action, 
            function(data, response, notifyComplete) {
    ...
    onStart: function(httpQuery, rqstLocation, session, statements, notifyCallBack){
    	...
    	if(!statements) {
            err = [util.makeDiagnosis(localize.getMessage(this._site.cdIsoLang, "3"), 4, "client.Controller.onStart()", "03")];
            try {
            ...
            notifyCallBack([util.makeDiagnosis(localize.getMessage(this._site.cdIsoLang, "4") + "\n" + localize.getMessage(this._site.cdIsoLang, "2") + "\nStack : client.Controller.onStart()", 4, "client.Controller.onStart()", "04")]);
    	}
    },
    ...
    loadRepresentation: function(representation, article, success, error, customProto){
        ...
        }, error, customProto, this.httpQuery.noDisplayErr ? {"noDisplayErr" : true} : null);
    },
