...
function Controller() {
	...
	this._evtBuffering = !syra_site.userProfile.dataset.disableKeyboardBuffering && syra_quality.enableEvtBuffering !== false ? new _keyboardBuffering(null, null, this) : null;
	this._uiLocking = new _uiLockDefault(false, null, this, this._evtBuffering);
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	_initSvcs: function(svcsDesc, svcHttpLoc) {
		_.each(svcsDesc, function(value, key) {
			this[value.proxy] = {
				...
				$url: svcHttpLoc.$url + (value.subUrl && value.subUrl.length > 0 ? ("/" + value.subUrl) : ""),
	...
	_callServer: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest) {
		...
		if (!pollingManifest) {
			...
			rqstPlg = {
				"noDisplayErr": proxy.noDisplayErr,
		...
		options.success = function(dataResp, response) {
	...
	postSapRqst: function(action, params, post, options) {
		...
		this._callServer(_rqstSvc.sapPost, rqstData, ("act=" + action + this._debugMode.qs), function(data, response, notifyComplete) {
			...
			if (!(ourAjaxErr = util.isOurAjaxErrEx(error))) {
				...
				ourAjaxErr = util.makeOurAjaxErrEx("KILL", util.getDiagnosesFromAjax(error, self._session), 23, syra_fusion.getMessage("14"));
			}
	...
	_ourSapRqstError: function(ourAjaxErr, setDefMess, notifyComplete) {
		...
		if (this._sapController) {
			...
			boxOpts.$delay = 10;
			boxOpts.click = function(resp, closureMode) {
				self.dispose();
			};
	...
	loadRepresentation: function(representation, article, success, error, customProto, stamp, onErr) {
		function _onProtoComplete($itemPage) {
			...
			try {
				hosterInstance.loadSheet($itemPage, $facet, syra_fusion.sapUtil.optDialog(representation.name), function(page) {
					success(page);
		...
		var diag, self = this,
			...
			_srvDuration;
		this.urlSeg.params.representation = representation.name + ".$fusion";
		if (representation.$variant) {
	...
	getSapPostBaseUrl: function(xid) {
		var qs, url = this.sapHttpSvc.$url;
		if (url) {
			qs = url.indexOf("?");
			url = qs >= 0 ? url.substr(0, qs) : url;
			url = syra_url.addHost(url);
		}
		return url;
	},
	...
	shouldHideRqstSurveyOnLongProcess: function() {
		return this.isPluginsPending();
	},
		...
		try {
			...
			if (evtDomain == "lcl" || evtDomain == "wdgt") {
				...
				if ((evtProps[1] == "field" || evtProps[1] == "cell") && targetMeta && targetMeta.$bind && targetMeta.$bind[0] === "$") {
					...
					if (evtProps[2] == "keydown" && nativeEvt && nativeEvt.keyCode == KeyboardHandlers.keyCodeNameMap["tab"]) {
						succeeded = true;
					} else {
						return true;
					}
						...
						if (object.result.$act > requestActions.srvActionStart) {
							if (this[_rqstSvc.sapPost.proxy]._tech.pending || this._site.checkCnxStatus(syra_fusion.sapUtil.cnxStatus.established, true)) {
								...
								if (object.target.$confirm && object.target.$officeAddinSetup && article) {
									succeeded = true;
									if (_officeAddin.installed.call(this, this, object.target, article, null, this.postSapRqst, [object.result.$act, object.result.params, object.result.post, {
		...
		if (curWin && (curField = curWin.getBoundField(curIst.xid, curIst.nl ? {
			...
			for (i = 0, len = evtBuffering.buffLength(); i < len; i++) {
				if (this[svcId] && this[svcId]._tech.pending && !force) {
					...
					break;
				} else if (force && (msgBox = syra_alert.getModal()) && !msgBox.disposed) {
					// Box is displayed. It could be an important message, so we prefer to stop/cancel dispatching
					evtBuffering.reset();
					KeyboardHandlers.release();
					break;
				} else {
					evt = evtBuffering.pop();
					//window.document.title = "" + ++_recoveredKeyboardEvt;
					if (evt.type == "keypress") {
						if (evt.nativeEvt.charCode == KeyboardHandlers.keyCodeNameMap.space && syra_fusion.syraUtil.getFieldType(curField, true) == syra_fusion.syraUtil.dataTypes.booleanType) {
					...
					} else if (evt.type == "keydown" && evt.nativeEvt.keyCode == KeyboardHandlers.keyCodeNameMap["backspace"]) {
					...
					ret = KeyboardHandlers[evt.type](evt.nativeEvt, curField, true);
					//console.error("Evt" + i + " : type=" + evt.type + " # keyCode=" + evt.nativeEvt.keyCode + " # keyMap=" + KeyboardHandlers.keyCodeMap[evt.nativeEvt.keyCode] + " # Return=" + ret);
					// What else?
...
// Lock UI class
var _uiLockDefault = helpers.defineClass(function(dummy, opts, hoster, evtBuffering) {
	this._locked = false;
	this._longProcessing = false;
	this._lockerId = null;
	this._hoster = hoster || null;
	this._evtBuff = evtBuffering || null;
	this._requestSurveyHidden = false;
	this._dummy = !!(dummy);
}, null, {
	lock: function(lockerId, longProcess) {
		...
		if (!this._locked) {
			...
			this._locked = true;
			this._longProcessing = false;
			syra_locker.lock(0, false, true, this._evtBuff);
			//document[syra_fusion.sapUtil._fNS].log.trace({"severety": "warn"}, "**** UI locked by '" + (this._lockerId || "") + "' ***");
		} else
		if (longProcess != this._longProcessing && (!this._lockerId || lockerId === this._lockerId)) {
			this._longProcessing = longProcess;
			syra_locker.toggleOverlay();
			if (longProcess && this._hoster && this._hoster.shouldHideRqstSurveyOnLongProcess) {
				if ((this._requestSurveyHidden = this._hoster.shouldHideRqstSurveyOnLongProcess())) {
					syra_ajax.survey.disable(true);
				}
			}
			//document[syra_fusion.sapUtil._fNS].log.trace({"severety": "warn"}, "**** UI locked 'long' by '" + (this._lockerId || "") + "' ***");
		}
	},
	unLock: function(lockerId, force, panic) {
		...
		if (this._locked && (force || !this._lockerId || lockerId === this._lockerId)) {
			...
			this._longProcessing = false;
			syra_locker.unlock();
			if (this._requestSurveyHidden) {
				syra_ajax.survey.disable(false);
			}
			//document[syra_fusion.sapUtil._fNS].log.trace({"severety": "warn"}, "**** UI UNLocked by '" + (this._lockerId || "") + (force ? "' in forced mode " : "'") + " ***");
			this._lockerId = null;
			if (this._evtBuff) {
				this._evtBuff[panic ? "reset" : "dispatch"](lockerId, force);
			}
		}
	},
	dispose: function() {
