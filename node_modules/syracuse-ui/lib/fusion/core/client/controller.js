		...
		this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action, function(data, response, notifyComplete) {
			...
			try {
				if (ret && post && post.callback) {
					...
					if (options.forceChain || (!ctrlFailed && (sapUtil.cmpIst(params.target, ist) || ((sapUtil.isBlockInst(params.target.xid) || sapUtil.isLineGridInst(params.target)) && sapUtil.cmpIstGrid(params.target, ist)) || sapUtil.cmpIstList(params.target, ist, true)))) {
