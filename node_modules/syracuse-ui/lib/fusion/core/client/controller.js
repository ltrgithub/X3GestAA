...
var _enableKeyboardBuffering = true;
...
function Controller() {
	...
	this._evtBuffering = _enableKeyboardBuffering ? new _keyboardBuffering(null, null, this) : null;
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	_callServer: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest) {
		...
		options.success = function(dataResp, response) {
			...
			if (!delegatePending) {
				...
				}, dataResp && dataResp.srvop && dataResp.srvop.request && dataResp.srvop.request.v == 1025 ? 1800 : 0);
			}
	...
	dispatchEvtBuffered: function(svcId, evtBuffering) {
		var ret, i, len, evt, currIst = this._sapController ? this._sapController.getCurrInst(true) : null;
		var currWin, value, currField = currIst ? this._sapController.getBoundField(currIst) : null;
		if (currField) {
			currWin = this._sapController.getWindow(currIst);
			for (i = 0, len = evtBuffering.isEventPending(); i < len; i++) {
				if (this[svcId] && this[svcId]._tech.pending) {
					break;
				} else {
					evt = evtBuffering.pop();
					if (evt.type == "keypress" && currWin) {
						evtBuffering.charData += String.fromCharCode(evt.nativeEvt.charCode || evt.nativeEvt.keyCode);
						currWin.setDataStoreValue(currIst.xid, currIst.nl, evtBuffering.charData, {
							"noStoreCommmit": true
						});
						// Deal with use case of space on check field
						...
						currWin.setDataStoreValue(currIst.xid, currIst.nl, value, {
							"noStoreCommmit": true
						});
					}
					ret = KeyboardHandlers[evt.type](evt.nativeEvt, currField, true);
					//console.error("Evt" + i + " : type=" +  evt.type + " # keyCode=" + evt.nativeEvt.keyCode + " # keyMap=" + evt.nativeEvt.syraKeyMap + " # Return=" + ret);
					if (evt.type == "keypress" && ret === false) {
						// Error on keypress validation
						evtBuffering.reset();
						break;
					} else if (evt.type == "keydown" && ret === false) {
						// Means that a request has been sent to the server, and so need to clear the current data
						evtBuffering.charData = "";
					}
				}
			}
			if (!evtBuffering.isEventPending()) {
				evtBuffering.charData = "";
			}
		} else {
			evtBuffering.reset();
...
var _uiLockDefault = helpers.defineClass(function(dummy, opts, hoster, evtBuffering) {
	...
	unLock: function(lockerId, force, panic) {
		...
		if (this._locked && (force || !this._lockerId || lockerId === this._lockerId)) {
			...
			this._lockerId = null;
			if (this._evtBuff) {
				this._evtBuff[panic ? "reset" : "dispatch"](lockerId);
			}
		}
	},
	dispose: function() {
...
var _keyboardBuffering = helpers.defineClass(function(bufferSize, timer, controller) {
	...
	push: function(evt) {
		// Functional rule
		if (this.len == 0 && (evt && evt.type == "keyup")) {
			// Remove parasite keyup event
			return false;
		}
		// store
	...
	dispatch: function(svcId) {
		var ret = null,
			ctrlDispatcher = this._controller && this._controller.dispatchEvtBuffered;
		if (ctrlDispatcher) {
			ret = ctrlDispatcher.call(this._controller, svcId, this);
		} else {
			return null;
		}
		return ret;
