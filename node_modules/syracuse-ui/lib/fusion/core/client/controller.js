...
exports.Controller = helpers.defineClass(Controller, EventManager, {
		...
		this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action, function(data, response, notifyComplete) {
			...
			try {
				if (ret && post && post.callback) {
					...
					if (options.forceChain || (!ctrlFailed && ist && (sapUtil.cmpIst(params.target, ist) || ((sapUtil.isBlockInst(params.target.xid) || sapUtil.isLineGridInst(params.target)) && sapUtil.cmpIstGrid(params.target, ist)) || sapUtil.cmpIstList(params.target, ist, true)))) {
		...
		if ((page = syraUtil.getPage(currentTarget))) {
			...
			if (data[helpSC.cat]) {
				...
				if (post && post.callback) {
					var url = document.site.expressionMaker.parse(page, "{" + helpSC.bUrl + "}", null, data);
					post.callback.apply(self, (post.args ? ([url].concat(post.args)) : [url]));
				} else {
					page.openHelp(null, document.site.expressionMaker.parse(page, "{" + helpSC.bUrl + "}", null, data));
				}
			} else {
				// Other help topic...
			}
	...
	loadRepresentation: function(representation, article, success, error, customProto, stamp, onErr) {
		...
		if (!representation.$itemPage) {
			this._hosterController.loadRepresentation(article, this.openerUrlSegments, _onProtoComplete, error, customProto, this.protoQueryOpts);
		} else {
			_onProtoComplete({
				openerUrlSegments: {},
				$representation: {
