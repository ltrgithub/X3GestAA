...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    onStart: function(statements, sharedResErr, notifyError){
        var self = this, isAbortReply = null, _elapse, recovery;
        var sttmntsErr = !statements || !SapModule.isSapReply(statements) ? [util.makeDiagnosis(localize.getMessage(self._site.cdIsoLang, "4") + "\n" + localize.getMessage(self._site.cdIsoLang, "2"), 4, "client.Controller.onStart()", "04", util.cltFusionNice)] : null;
        if (sharedResErr) {
        ...
        if(isAbortReply) {
            ...
            if(sharedResErr || sttmntsErr){
                setTimeout(function(){
                    try {
                        self._sapController._sapException(null, sttmntsErr || sharedResErr, recovery);
                    }
                    catch (ex) {
                        notifyError(sttmntsErr || sharedResErr);
                    }
                }, 10);
            }
            else {
