...
var dialogHelper = require('syracuse-ui/lib/site/tools/dialogHelper');
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	_callServer: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest) {
		...
		if (!pollingManifest) {
			...
			self[svc.proxy]._tech.elapseLocalOnPolling = 0;
			// Check options...
			if (options) {
		...
		// Send request
		syra_controller.callServer(null, rqstPlg || this[svc.proxy], function(dataResp, response) {
	...
	delSapSessExPanic: function(additionalSess, notifyCallback) {
		...
		this[proxy].syncRqst = !syra_site.getBrowserIdentity().isMSIE;
		// Go!
		this.forceUnlockUI();
		...
		if ((page = syraUtil.getPage(currentTarget))) {
			...
			if (data[helpSC.cat]) {
				...
				if (post && post.callback) {
					...
					syra_site.helpCenter.open(page, null, syra_expression.parse(page, "{" + helpSC.bUrl + "}", null, data));
				}
			} else {
				// Other help topic...
			}
	...
	getSapPostBaseUrl: function(xid) {
		var qs, url = this.sapHttpSvc.$location;
		if (url) {
			qs = url.$url.indexOf("?");
			url = qs >= 0 ? url.$url.substr(0, qs) : url.$url;
					...
					if (object.result && object.result.$act) {
						if (object.result.$act > requestActions.srvActionStart) {
							if (this[_rqstSvc.sapPost.proxy]._tech.pending || this._site.checkCnxStatus(sapUtil.cnxStatus.established, true)) {
								...
								var article = syraUtil.getPage(object.target);
								if (object.target.$confirm && object.target.$officeAddinSetup && article) {
									succeeded = true;
									if (dialogHelper.dialogConfirm.call(this, this, object.target, article, null, this.postSapRqst, [object.result.$act, object.result.params, object.result.post, {
	...
	dispatchEvtBuffered: function(svcId, force, evtBuffering) {
		...
		if (curWin &&
			(curField = curWin.getBoundField(curIst.xid, curIst.nl ? {
				"arrayCell": curIst.nl
			} : null))) {
			//try {
			for (i = 0, len = evtBuffering.buffLength(); i < len; i++) {
				...
				if (force && (msgBox = syra_alert.getModal()) && !msgBox.disposed) {
					...
					if (evt.type == "keydown" && evt.nativeEvt.keyCode == KeyboardHandlers.keyCodeNameMap["backspace"]) {
...
var _uiLockDefault = helpers.defineClass(function(dummy, opts, hoster, evtBuffering) {
	...
	lock: function(lockerId, longProcess) {
		...
		if (!this._locked) {
			...
			this._locked = true;
			this._longProcessing = false;
			syra_uiLocker.lock(0, false, true, this._evtBuff);
		...
		if (longProcess != this._longProcessing && (!this._lockerId || lockerId === this._lockerId)) {
			...
			if (longProcess && this._hoster && this._hoster.shouldHideRqstSurveyOnLongProcess) {
				if ((this._requestSurveyHidden = this._hoster.shouldHideRqstSurveyOnLongProcess())) {
					syra_site.disableRequestSurvey(true);
				}
			}
	...
	unLock: function(lockerId, force, panic) {
		...
		if (this._locked && (force || !this._lockerId || lockerId === this._lockerId)) {
			this._locked = false;
			this._longProcessing = false;
			syra_uiLocker.unlock();
			if (this._requestSurveyHidden) {
				syra_site.disableRequestSurvey(false);
			}
