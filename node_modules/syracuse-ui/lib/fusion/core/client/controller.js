...
var _rqstSvc = {
    "sapPost": {"name": "Native protocol", "act": null, "proxy": "sapHttpSvc", "method": "PUT", "subUrl": "requestSvc", "contentType": "application/json", "tracking": false},
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    _sendRequest: function(svc, data, queryString, onSuccess, onError, delegatePending, options){
        var qs = null, qsEx = null, self = this;
        // Fixture mode
        ...
        if(queryString && queryString.length > 0) {
            qsEx = queryString + (this[svc.proxy]._tracking ? ("&trackingId=" + this[svc.proxy]._tech.count.s) : "");
        }
        else if (this[svc.proxy]._tracking) {
            qsEx = "trackingId=" + this[svc.proxy]._tech.count.s;
        }
    ...
    postSapRqst: function(action, params, post, options){
        var self = this, rqstData, currIst;
        // Init
        if(!options) {
            options = {};
        }
        ...
        else if (rqstData.ctrlResult.succeeded && rqstData.ctrlResult.show !== undefined && !this._formatAdx.wrapper.settings.localFormat) {
            // Deal with "previous" value
            options.previous = util.duplicateObj(this._sapController.getCurrInst());
            options.previous.v = rqstData.ctrlResult.show;
        }
		...
		// add aurel for test asynchronous request
        //this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action+"&trackngId="+helpers.uuid.generate('-'), 
        this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action,
            function(data, response, notifyComplete) {
                var ret, chain = null, ist, ctrlFailed, delPrev = false;
                if(options.previous && data.sap) {
                    data.sap.previous = options.previous;
                    delPrev = true;
                }
                ret = self._sapController.processReply(data, {"_techReply": {"replyComplete": notifyComplete}});
                if(delPrev) {
                    delete data.sap.previous;
                }
                try {
    ...
    delSapSessExPanic: function(additionalSess){
        ...
        this._sendRequest(_rqstSvc.sapDel, null, qr, function() {if(self._site){self._site.updtCnxStatus(sapUtil.cnxStatus.deleted)};return;}, function() {if(self._site){self._site.updtCnxStatus(sapUtil.cnxStatus.deletedFailed)}; return;}, false);
    ...
    _printRqst: function(rqstSvc, params, post, options){
    ...
    _addProtoToCache: function(name, stamp, proto, meta) {
        if(this._protoCache) {
            this._protoCache[name + "_" + this._session.cdIsoLang + "_" + stamp] = proto;
        }
    },

    _getProtoFromCache: function(name, stamp, meta) {
        return this._protoCache && this._protoCache[name + "_" + this._session.cdIsoLang + "_" + stamp];
    },
    ...
    onStart: function(httpQuery, rqstLocation, session, statements, notifyCallBack){
        ...
        if(session && session.format.wrapper) {
            ...
            _rqstSvc["format"].act[1] = session.format.wrapper.settings.localCtrl ? "_localFormatSvcRqst" : "_basicSvcRqst";
        }
        if (rqstLocation) {
        ...
        if(!statements) {
            ...
            this._protoCache = this._session.initProtoCache(this._session.endpointParts.join(".")); // TODO : Warning when app and trt server are differents!
    ...
    triggerAdx: function(event, object){
        try {
            ...
            var winEvt, evtTimelapse = (new Date()).getTime(), succeeded = true, intercepted = false, evtDomain, evtProps = (this.getEvtName(event)).split("."), actId, mapping;
            evtDomain = evtProps[0]; 
            // Standard control : only for "local" and "widget" event domain.
            if(evtDomain == "lcl" || evtDomain == "wdgt") {
                winEvt = object ? syraUtil.getFusionPageMeta(object.target) : null;
                if(winEvt && ( winEvt = winEvt.winModel) && winEvt.isPreviewWin() && evtProps[1] != "link") {
