...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    _delSapSess: function(reuse, panic, additionalSess){
        var self = this, data = additionalSess ? {"cluster": additionalSess} : undefined;
        ...
        if(panic) {
            this._sendRequest(_rqstSvc.sapDel, null, null, function() {if(self._site){self._site.updtCnxStatus(sapUtil.cnxStatus.deleted)};return;}, function() {if(self._site){self._site.updtCnxStatus(sapUtil.cnxStatus.deletedFailed)}; return;}, false);
            setTimeout(function () {onComplete("closed");}, 1500);
            return;
        }
        this._sendRequest(_rqstSvc.sapDel, data, reuse ? "reuse=true" : null, 
            function(data, response, notifyComplete) {
