...
var _rqstSvc = {
    "sapPost": {"name": "Native protocol", "act": null, "proxy": "sapHttpSvc", "method": "PUT", "subUrl": "requestSvc", "contentType": "application/json", "tracking": true, "lockUI": "common"},
...
var _cnxSvc = {
    "sapCnx": {"name": "Session connection", "act": null, "proxy": "sapHttpCnx", "method": "POST", "subUrl": "", "contentType": "application/json", "tracking": true, "lockUI": null}
};
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    _initSvcs: function(svcsDesc, svcHttpLoc){
        _.each(svcsDesc, function(value, key){
            ...
            this[value.proxy]._uiLocking = value.lockUI ? (value.lockUI == "common" ? this.uiLocking : new (_uiLockerClasses[value.lockUI])()) : new _uiLockDefault(true);
    ...
    _sendRequest: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest){
        ...
        // Send request
        this[svc.proxy]._uiLocking.lock();
        this._hosterController.sendRequest(null, rqstPlg || this[svc.proxy],
            function (dataResp, response) {
                ...
                if(response.status == 202 && self[svc.proxy]._tracking) {
                    // Polling!
                    self[svc.proxy]._uiLocking.lock(true);
                    plgType = dataResp && dataResp.type || "default";
                    headers = response.headers;
                    plgManifest = {"$location": headers.location, "method": "GET", "type": plgType};
                    if(!onPolling) {
                ...
                if(!delegatePending) {
                    ...
                    self[svc.proxy]._uiLocking.unLock();
    ...
    _appendCnxReply: function(partialReply, consolidatedReply) {
        ...
        return SapModule.isSessStatementsOk(consolidatedReply);
    },

    postCnxpRqst: function(cnxData, onSuccess, onError, options){
        var proxy = this[_cnxSvc.sapCnx.proxy], qs = proxy.$location.$url.split("?");
        var self = this, replyParseCtx = null, pngBoot = null, pollingErr = null, sharedResErr = null;
        ...
        this._sendRequest(_cnxSvc.sapCnx, cnxData, qs[1], 
            function(data, response) {
                var headers = response.headers, errRaised = null, $location = {"$url": headers.location, "type": headers["content-type"]};
                try {
                    if(!pngBoot) {
            ...
            function(pngType, pngData, pngCbk, pngHeaders) {
                ...
                try {
                    ...
                    if(pngData && pngData.reply && !pollingErr && !sharedResErr) {
                        if(!pngBoot) {
                            pngBoot = {"isSttmntsPending": true, "statements": {}, "done": false};
                        }
                        if (!(pngBoot.isSttmntsPending = !self._appendCnxReply(pngData.reply, pngBoot.statements))) {
                            ...
                            if(!pngBoot.done) {
                                ...
                                onSuccess({"$url": realLoc[0], "type": pngHeaders["content-type"]}, pngBoot.statements, function(isSharedResErr, fatalErr){
    ...
    postSapRqst: function(action, params, post, options){
        var self = this, rqstData, currIst, pollingErr = null, replyParseCtx = this._sapController.initReplyParseCtx();
        // Init
        if(!options) {
            options = {};
        }
        ...
        this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action,
            function(data, response, notifyComplete) {
                ...
                if(pollingErr) {
                    ...
                    self._sapController._sapException(null, replyParseCtx.diagnoses, {"typ": "DelSess"});
                    return;
                }
                replyParseCtx._techReply = {"replyComplete": notifyComplete};
                ...
                try {
                    ...
                    if(post) {
                        ...
                        if(ret && chain) {
                            ...
                            setTimeout(function(){self.postSapRqst(chain.$act, chain.params, chain.post);}, 2);
                        }
                    }
                    // TODO : check error...
                }
    ...
    delSapSessExPanic: function(additionalSess){
        ...
        this[_rqstSvc.sapDel.proxy].syncRqst = !util.isExpectedBrowser("msie");
        // Go!
    ...
    initialize: function(site, cnxHttpLoc){
        ...
        this.uiLocking = new _uiLockDefault();
    ...
    completeInit: function(rqstLocation, httpQuery, session, localOpts){
        ...
        this.protoQueryOpts = {"headers": {"accept-language": this._session ? this._session.cdIsoLang : localize.defLang}, "noDisplayErr": this.httpQuery.noDisplayErr ? true : undefined};
        // Formatter policies
    ...
    onStart: function(statements, sharedResErr, notifyError){
        var self = this, isAbortReply = null, _elapse, recovery, err;
        ...
        if (sharedResErr) {
            recovery = {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_FIN_APPLI"), "param": {"ist": null, "v": ""}};
        }
        if (!sttmntsErr) {
            SapModule.cleanInitialReply(statements);
            isAbortReply = this._sapController.onConnected(statements);
        }
        else {
            recovery = {"typ": "DelSess", "panic": true};
        }
        if(!isAbortReply) {
    ...
    loadRepresentation: function(representation, article, success, error, customProto, stamp, onErr){
        var diag, self = this, hosterSite = this._site.getHosterSite(), hosterInstance = this._site.getHosterInstance(), _elapse = utilBis.getTimer(true), _srvDuration;
        this.httpQuery.$urlParts.params.representation = representation.name + ".$fusion";
        if(representation.$variant) {
            this.protoQueryOpts.params = {"modelRepresentation": representation.$variant};
        }
        else {
...
var _uiLockDefault = helpers.defineClass(
    ...
    lock: function(longProcess) {
        if(this._dummy) {
            return;
        }
        if(!this._locked) {
            siteUILocking.lock(3000);
            this._locked = true;
            //document[sapUtil._fNS].log.trace({"severety": "warn"}, "****UI locked***");
        }
        else if(longProcess != this._longProcessing) {
            this._longProcessing = longProcess;
            siteUILocking.unlock();
            siteUILocking.lock(0, true);
            //document[sapUtil._fNS].log.trace({"severety": "warn"}, "****UI locked long***");
        }
    },
    unLock: function() {
        if(this._dummy) {
        ...
        if(this._locked) {
            //document[sapUtil._fNS].log.trace({"severety": "warn"}, "****UI UNLocked long***");
            siteUILocking.unlock();
            this._locked = false;
        }
    }
});
var _uiLockerClasses = {"cvg": _uiLockDefault};
