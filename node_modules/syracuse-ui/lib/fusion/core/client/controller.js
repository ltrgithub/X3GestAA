...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    _sendRequest: function(svc, data, queryString, onSuccess, onError){
        ...
        if(this[svc.proxy]._tech.pending) {
            ...
            alert("Request '" + svc.name + "'' is pending... try again!!");
            return false;
        }
        this[svc.proxy]._tech.pending = true;
        delete this[svc.proxy]._tech.elapse;
        this[svc.proxy]._tech.elapse = utilBis.getTimer(true);
        // Buil request
        ...
        this._hosterController.sendRequest(null, this[svc.proxy],
            function (data, response) {
                ...
                console.log("Request duration for '" + svc.name +"' : total [" + self[svc.proxy]._tech.elapse.elapsedMs() + "], with ajax [" + self[svc.proxy]._tech.elapseAjax + "] and local [" + (self[svc.proxy]._tech.elapse.elapsedMs() - self[svc.proxy]._tech.elapseAjax) + "]");
            },
            function (error) {
                self[svc.proxy]._tech.pending = false;
                onError(error);
        ...
        this._sendRequest(_rqstSvc.sapDel, null, reuse ? "reuse=true" : null, 
            ...
            function(error) {
               ...
               if(!self.delSessRetry) {
                    ...
                    self._sapController._sapException(null, util.getDiagnosesFromAjax(error), null, error, function() {
    ...
    loadRepresentation: function(representation, article, success, error, customProto){
        ...
        this._hosterController.loadRepresentation(article, this.httpQuery, function($itemPage){
            ...
            console.log("Load activity duration : total [" + _elapse.elapsedMs() + "], with ajax [" + _srvDuration + "] and local [" + (_elapse.elapsedMs() - _srvDuration) + "]");
        }, error, customProto);
    },
