...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    postSapRqst: function(action, params, post, options){
        ...
        if(this._site.checkCnxStatus(sapUtil.cnxStatus.closing, null, true)) {
            setTimeout(function(){
                self.postSapRqst(requestActions.getSrvAction("SRV_FIN_APPLI"), {"ist": null, "v": ""});
            }, 2);
            return false;
        }
    ...
    postInit: function(httpQuery, session, srvOpts, localOpts){
        // More init...
    ...
    onStart: function(rqstLocation, statements, notifyError){
        var sttmntsErr, self = this;
        if (rqstLocation) {
        ...
        if(!statements || !SapModule.isSapReply(statements)) {
            // Who is talking to me?
            sttmntsErr = [util.makeDiagnosis(localize.getMessage(this._site.cdIsoLang, "4") + "\n" + localize.getMessage(this._site.cdIsoLang, "2"), 4, "client.Controller.onStart()", "04", util.cltFusionNice)];
            try {
                // TODO : delete or not delete session?
                this._sapController._sapException(null, sttmntsErr, {"typ": "DelSess", "panic": true});
            }
            catch (ex) {
                notifyError(sttmntsErr);
            }
        }
        else {
            // Check shared resources before to continue
            this._session.waitSharedRes(100, 30, function(diagnoses) {
                var isSharedResErr = !!(diagnoses && diagnoses.length > 0), isAbortReply, _elapse;
                if (!isSharedResErr) {
                    // Post-init shared resources and check if all it's ok
                    self._session.postInitSharedRes();
                    isAbortReply = self._sapController.onConnected(statements);
                }
                if(isAbortReply) {
                    // Sorry....
                    setTimeout(function() {notifyError(isAbortReply);}, 10);
                 }
                else if (isSharedResErr) {
                    self._site.updtCnxStatus(sapUtil.cnxStatus.established);
                    setTimeout(function(){
                        self._sapController._sapException(null, diagnoses, {"typ": "RqstSrv", "act": requestActions.getSrvAction("SRV_FIN_APPLI"), "param": {"ist": null, "v": ""}});
                    }, 10);
                }
                else {
