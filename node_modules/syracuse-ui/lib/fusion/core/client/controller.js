...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	postCnxpRqst: function(cnxData, onSuccess, onError, options) {
		...
		function _getDiagnosis(stdErrNum, defErrNum, messEx, stack) {
			var errMess, defErrMess = syra_site.fusionGateway.getMessage(defErrNum);
					...
					if (!pngBoot.isSttmntsPending) {
						...
						if ((pngBoot.isSttmntsPending = !self._appendCnxReply(data, pngBoot.statements, extraErr))) {
							...
							if (tempSapCtrl.getTargetBoxFromReply(pngBoot.statements) && SapModule.isSessStatementsOk(pngBoot.statements, false, extraErr)) {
								...
								// Real sap error!
								pollingErr = [_getDiagnosis("2", "19", null, extraErr ? (syra_site.fusionGateway.getMessage("missCnxStmt") + " : '" + extraErr.missingStatement + "'") : null)];
								errRaised = {
									"typ": "DelSess"
								};
							}
	...
	postSapRqst: function(action, params, post, options) {
		...
		this._callServer(_rqstSvc.sapPost, rqstData, "act=" + action, function(data, response, notifyComplete) {
			...
			try {
				...
				var ourAjaxErr = util.makeOurAjaxErrEx("CLOSESESS", [util.makeDiagnosis("stack trace...", 4, ex.stack, 18)], 18, syra_site.fusionGateway.getMessage("13"), "Script error catched in 'controller.postSapRqst/Onsuccess() : " + ex.message);
			...
			try {
				...
				pollingErr = [util.makeDiagnosis(self._session.getlabel("error", "18", syra_site.fusionGateway.getMessage("13")), 4, ex.message + "\n" + ex.stack, "18", util.cltFusionNice)];
			} finally {
				if (ack) {
					pngCbk(true);
				}
	...
	completeInit: function(rqstLocation, openerUrlSegments, session, localOpts) {
		...
		this.protoQueryOpts = {
			"headers": {
				"accept-language": this._session ? this._session.cdIsoLang : syra_site.fusionGateway.defLang
			},
			"noDisplayErr": true
		};
		// Formatter policies
	...
	onStart: function(statements, sharedResErr, notifyComplete) {
		...
		var sttmntsErr = !statements || !SapModule.isSapReply(statements) ? [util.makeDiagnosis(syra_site.fusionGateway.getMessage("4") + "\n" + syra_site.fusionGateway.getMessage("2"), 4, "client.Controller.onStart()", "04", util.cltFusionNice)] : null;
		if (sharedResErr) {
			recovery = {
				"typ": "RqstSrv",
				"act": requestActions.getSrvAction("SRV_FIN_APPLI"),
