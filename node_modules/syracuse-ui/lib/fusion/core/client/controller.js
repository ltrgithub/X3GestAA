...
function _getTimingInfo(svcTech, reply) {
	...
	return "total [" + info.total + "], with ajax [" + info.ajax + " (rts : " + info.rts + " - ws : " + info.webServer + ")] and local [" + info.localRqst + "]" + ". Extra local pre-process duration [" + info.localRpl + "]";
}
	...
	_callServer: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest) {
		...
		options.success = function(dataResp, response) {
			...
			if (self._log && self._debugMode.timingOn) {
				self._log.timer(null, "Request duration for '" + svc.name + (data && data.act ? (' - act ' + data.act) : '') + "' : " + _getTimingInfo(self[svc.proxy]._tech, response));
			}
		};
