...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    postSapRqst: function(action, params, post, options){
        ...
        this._sendRequest(_rqstSvc.sapPost, data, "act=" + action, 
            function(data, response, notifyComplete) {
                var ret = self._sapController.processReply(data, {"_techReply": {"replyComplete": notifyComplete}}), chain = null, ist, ctrlFailed;
                try {
                    if(ret && post && post.callback) {
                        ...
                        if((options && options.forceChain) || (!ctrlFailed && (sapUtil.cmpIst(params.target, ist) || ((sapUtil.isBlockInst(params.target.xid) || sapUtil.isLineGridInst(params.target)) && sapUtil.cmpIstGrid(params.target, ist))))){
    ...
    onStart: function(httpQuery, rqstLocation, session, statements, notifyCallBack){
        ...
        // Formatter policies
        if(session && session.wrapperFormatter) {
            _rqstSvc["format"].act[1] = session.wrapperFormatter.settings.localCtrl ? "_localFormatSvcRqst" : "_basicSvcRqst";
        }
        if (rqstLocation) {
    ...
    triggerAdx: function(event, object){
        try {
            ...
            var evtTimelapse = (new Date()).getTime(), succeded = true, intercepted = false, evtDomain = this.getEvtName(event), actId, mapping;
            ...
            // Call handler
            if(succeded) {
                // If event object impelment "target', we notify target that an event occured... and give it the opportunity to interrupt event process
                ...
                if(intercepted !== undefined)
                {
                    if(object.result && object.result.diagnoses)
                    {
                        ...
                        if (object.result.$act > requestActions.srvActionStart) {
                            this.postSapRqst(object.result.$act, object.result.params, object.result.post, {"preProcElapse": evtTimelapse});
                        }
    ...
    _localFormatSvcRqst: function(rqstSvc, data, post, options){
        var succeded = false, result = {};
        // Call formatter : result = bidule(data); succeded = result ? true : false;
        if(post) {
            post.callback.apply(this, post.args ? [succeded, result].concat(post.args) : [succeded, result]);
        }
    },
