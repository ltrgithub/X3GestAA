...
function _getTimingInfo(svcTech, reply) {
	...
	return "total [" + info.total + "], with ajax [" + info.ajax + " (rts : " + info.rts + " - ws : " + info.webServer + " - other : " + (info.other || "unvailable") + ")] and local [" + info.localRqst + "]" + ". Extra local pre-process duration [" + info.localRpl + "]";
}
...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	postCnxpRqst: function(cnxData, onSuccess, onError, options) {
		...
		this._sendRequest(_cnxSvc.sapCnx, cnxData, qs[1] + this._debugMode.qs, function(data, response, notifyComplete) {
	...
	postSapRqst: function(action, params, post, options) {
		...
		// Send request
		this._sendRequest(_rqstSvc.sapPost, rqstData, ("act=" + action + this._debugMode.qs), function(data, response, notifyComplete) {
			var ret, chain = null,
