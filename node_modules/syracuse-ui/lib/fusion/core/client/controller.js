    ...
    _sendRequest: function(svc, data, queryString, onSuccess, onPolling, onError, delegatePending, options, pollingManifest){
        ...
        this._hosterController.sendRequest(null, rqstPlg || this[svc.proxy],
            function (dataResp, response) {
                var plgManifest, plgType, location;
                ...
                if(response.status == 202 && self[svc.proxy]._tracking) {
                    ...
                    plgManifest = {"$location": location, "method": "GET", "type": plgType};
                    if(!onPolling) {
    ...
    postCnxpRqst: function(cnxData, onSuccess, onError, options){
        ...
        this._sendRequest(_cnxSvc.sapCnx, cnxData, qs[1], 
            function(data, response) {
                var headers = response.headers, errRaised = null;
                try {
                    if(!pngBoot) {
                        onSuccess({"$url": headers.location, "type": headers["content-type"]}, data);
                    }
                    else {
                       if(pollingErr || (replyParseCtx && replyParseCtx.failed)) {
                            ...
                            if(pngBoot.done) {
                                if((pollingErr = self.controller.onStart(data, sharedResErr))){
                                    errRaised = {};
                                }
                            }
                            else {
                                self._appendCnxReply(data, pngBoot.statements);
                                onSuccess({"$url": headers.location, "type": headers["content-type"]}, pngBoot.statements);
                            }
                        }
                        else {
            ...
            function(pngType, pngData, pngCbk, location) {
                var ack = true, self = this, realLoc;
                ...
                try {
                    this._site.updtCnxStatus(sapUtil.cnxStatus.polling);
                    if(pngData && pngData.reply && !pollingErr && !sharedResErr) {
                        ...
                        if (!(pngBoot.isSttmntsPending = self._appendCnxReply(pngData.reply, pngBoot.statements))) {
                            ...
                            if(!pngBoot.done) {
                                pngBoot.done = true;
                                realLoc = location.split("?");
                                onSuccess({"$url": realLoc[0], "type": headers["content-type"]}, data, function(isSharedResErr, fatalErr){
