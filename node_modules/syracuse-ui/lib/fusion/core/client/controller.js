...
exports.Controller = helpers.defineClass(Controller, EventManager, {
		...
		if ((page = syra_fusion.syraUtil.getPage(currentTarget))) {
			...
			if (data[helpSC.cat]) {
				...
				if (!data[helpSC.kw]) {
					...
					if (data[helpSC.cat] == sapUtil.helpHandler.fct.cat && (currField = syra_fusion.syraUtil.getFusionPageMeta(page))) {
	...
	loadRepresentation: function(representation, article, success, error, customProto, stamp, onErr) {
		function _onProtoComplete($itemPage) {
			...
			try {
				hosterInstance.loadSheet($itemPage, $facet, sapUtil.optDialog(representation.name), function(page) {
					success(page);
