...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	postCnxpRqst: function(cnxData, onSuccess, onError, options) {
		...
		function _getDiagnosis(stdErrNum, defErrNum, messEx, stack) {
			var errMess, defErrMess = syra_site.fusionGateway.getMessage(self._site.cdIsoLang, defErrNum);
					...
					if (!pngBoot.isSttmntsPending) {
						...
						if ((pngBoot.isSttmntsPending = !self._appendCnxReply(data, pngBoot.statements, extraErr))) {
							...
							if (tempSapCtrl.getTargetBoxFromReply(pngBoot.statements) && SapModule.isSessStatementsOk(pngBoot.statements, false, extraErr)) {
								...
								// Real sap error!
								pollingErr = [_getDiagnosis("2", "19", null, extraErr ? (syra_site.fusionGateway.getMessage(self._site.cdIsoLang, "missCnxStmt") + " : '" + extraErr.missingStatement + "'") : null)];
								errRaised = {
									"typ": "DelSess"
								};
							}
	...
	postSapRqst: function(action, params, post, options) {
		...
		this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action, function(data, response, notifyComplete) {
			...
			try {
				...
				var ourAjaxErr = util.makeOurAjaxErrEx("CLOSESESS", [util.makeDiagnosis("stack trace...", 4, ex.stack, 18)], 18, syra_site.fusionGateway.getMessage(self._site.cdIsoLang, "13"), "Script error catched in 'controller.postSapRqst/Onsuccess() : " + ex.message);
			...
			try {
				...
				pollingErr = [util.makeDiagnosis(self._session.getlabel("error", "18", syra_site.fusionGateway.getMessage(self._session.cdIsoLang, "13")), 4, ex.message + "\n" + ex.stack, "18", util.cltFusionNice)];
			} finally {
				if (ack) {
					pngCbk(true);
				}
			...
			if (!(ourAjaxErr = util.isOurAjaxErrEx(error))) {
				...
				ourAjaxErr = util.makeOurAjaxErrEx("KILL", util.getDiagnosesFromAjax(error, self._session), 23, syra_site.fusionGateway.getMessage(self._site.cdIsoLang, "14"));
			}
	...
	onStart: function(statements, sharedResErr, notifyComplete) {
		...
		var sttmntsErr = !statements || !SapModule.isSapReply(statements) ? [util.makeDiagnosis(syra_site.fusionGateway.getMessage(this._site.cdIsoLang, "4") + "\n" + syra_site.fusionGateway.getMessage(this._site.cdIsoLang, "2"), 4, "client.Controller.onStart()", "04", util.cltFusionNice)] : null;
		if (sharedResErr) {
			recovery = {
				"typ": "RqstSrv",
				"act": requestActions.getSrvAction("SRV_FIN_APPLI"),
