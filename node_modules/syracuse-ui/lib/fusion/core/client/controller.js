...
exports.Controller = helpers.defineClass(Controller, EventManager, {
	...
	postHelpRqst: function(category, keyword, currentTarget, params) {
		this[_rqstSvc.onLineHelp.act[1]](_rqstSvc.onLineHelp, params, null, {
			"$category": category,
			"$keyword": keyword
		}, currentTarget);
	},
	...
	_oneLineHelpRqst: function(rqstSvc, params, post, options, currentTarget) {
		var hCategory = options.$category,
			hKeyWord, currField, notFound = "NONE",
			proto, props = {}, page;
		if ((page = syraUtil.getPage(currentTarget))) {
			if (options.$link) {
				hCategory = _.find(sapUtil.helpHandler, function(value, key) {
					return value.linkId == options.$link;
				});
				hCategory = hCategory && hCategory.cat;
			}
			if (hCategory) {
				// Legacy help
				if (!(hKeyWord = options.$keyword)) {
					hKeyWord = notFound;
					if (hCategory == sapUtil.helpHandler.fld.cat && (currField = this._sapController.getCurrInst(true))) {
						proto = syraUtil.getEntityProto(currField.xid, syraUtil.get$FromPage(page));
						hKeyWord = proto && proto.$keyword;
					} else if (hCategory == sapUtil.helpHandler.fct.cat && (currField = syraUtil.getFusionPageMeta(page))) {
						hKeyWord = currField.winModel.getFctName();
					}
				}
				if (hKeyWord) {
					props[syraUtil.helpToken.cat] = hCategory;
					props[syraUtil.helpToken.kw] = hKeyWord;
					//window.open(page.parseExpression("{" + syraUtil.helpToken.bUrl + "}", null, props), "TODO help");
				}
			} else {
				// Other help topic...
			}
