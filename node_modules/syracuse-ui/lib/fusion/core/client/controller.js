...
exports.Controller = helpers.defineClass(Controller, EventManager, {
    ...
    postCnxpRqst: function(cnxData, onSuccess, onError, options){
        ...
        this._sendRequest(_cnxSvc.sapCnx, cnxData, qs[1], function(data, response, notifyComplete){
            var tempSapCtrl, headers = response.headers, errRaised = null, $location = {
                        ...
                        if (!pngBoot.isSttmntsPending) {
                            ...
                            if ((pngBoot.isSttmntsPending = !self._appendCnxReply(data, pngBoot.statements))) {
                                ...
                                if (pngBoot.statements && (pollingErr = tempSapCtrl.isAbortReply(pngBoot.statements))) {
                                    ...
                                    if (tempSapCtrl.getTargetBoxFromReply(pngBoot.statements) && SapModule.isSessStatementsOk(pngBoot.statements)) {
                                        // Box target...
                                        onSuccess($location, pngBoot.statements, null, false, notifyComplete);
                                    }
                                    else {
    ...
    postSapRqst: function(action, params, post, options){
        var self = this, rqstData, currIst, pollingErr = null, blockingNode = null, replyParseCtx = this._sapController.initReplyParseCtx();
        // Init
        if (!options) {
            options = {};
        }
        ...
        this._sendRequest(_rqstSvc.sapPost, rqstData, "act=" + action, function(data, response, notifyComplete){
            ...
            replyParseCtx._techReply = {
                "replyComplete": post && post.callback ? null : notifyComplete
            };
    ...
    _ourSapRqstError: function(ourAjaxErr, setDefMess, notifyComplete){
        var diagnoses = util.makeDiagnosesFromOurAjaxErrEx(ourAjaxErr, this._session), recovery = null, dftMainMess = "Web server encountered following error during last request processing";
    ...
    delSapSessExPanic: function(additionalSess){
        var self = this, proxy = _rqstSvc.sapDel.proxy, qr = _kPanicQr + (additionalSess ? additionalSess : "");
        // Check viability
        if (!this[proxy]) {
            return;
        }
    ...
    _delSapSess: function(reuse, panic, additionalSess){
        ...
        this._sendRequest(_rqstSvc.sapDel, null, reuse ? "reuse=true" : null, function(data, response, notifyComplete){
            if (self._site) {
                self._site.updtCnxStatus(sapUtil.cnxStatus.deleted);
            }
    ...
    _oneLineHelpRqst: function(rqstSvc, params, post, data, currentTarget){
        var hCategory, fctId, hKeyWord, currField, noHelp = "ANONE", proto, page, helpSC = syraUtil.helpToken, self = this;
        if ((page = syraUtil.getPage(currentTarget))) {
            ...
            if (data[helpSC.cat]) {
                ...
                if (post && post.callback) {
                    var url = page.parseExpression("{" + helpSC.bUrl + "}", null, data);
    ...
    onStart: function(statements, sharedResErr, notifyComplete){
        var self = this, isAbortReply = null, _elapse, recovery, err, replyParseCtx = null;
    ...
    displayBusinessHelp: function(helpTokens){
        this._sapController.stubSapMisc_help(helpTokens);
    },
    ...
    triggerAdx: function(event, object){
        try {
            ...
            var winEvt = null, privateEvtManager = null, targetMeta = null, nativeEvt, evtTimelapse = (new Date()).getTime(), succeeded = true, intercepted = false, evtDomain, evtProps = (this.getEvtName(event)).split("."), actId, mapping;
    ...
    lock: function(lockerId, longProcess){
        if (this._dummy) {
            ...
            if (!this._locked) {
                ...
                this._locked = true;
                this._longProcessing = false;
                document.site.lockUI(0, false, true);
            //document[sapUtil._fNS].log.trace({"severety": "warn"}, "**** UI locked by '" + (this._lockerId || "") + "' ***");
                ...
                if (longProcess != this._longProcessing && (!this._lockerId || lockerId === this._lockerId)) {
                    document.site.lockUI(1, (this._longProcessing = longProcess), true);
