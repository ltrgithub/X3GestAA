...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
	...
	connect: function(httpQuery, onError) {
		...
		function _onError(diagnoses, notifyComplete) {
			onError(diagnoses ? {
				"$diagnoses": diagnoses
			} : undefined);
			if (notifyComplete) {
		...
		try {
			...
			self._unLockUI();
			onError([util.makeDiagnosis(localize.getMessage(this.cdIsoLang, "1") + " 'client.FusionInPlaceSite.connect()' : " + ex.message, 4, ex.stack, "01", util.cltFusionNice)]);
		}
	},
	...
	_ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete) {
		...
		boxOpts.callback = function(resp, closureMode) {
			...
			hosterOnError(null, notifyComplete);
		};
		setTimeout(function() {
