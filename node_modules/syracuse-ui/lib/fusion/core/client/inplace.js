...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
	...
	_ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete) {
		...
		var boxParams, diagnoses = util.makeDiagnosesFromOurAjaxErrEx(ourAjaxErr),
	...
	free: function(reason) {
		...
		if (syra_site && syra_fusion && hosterInst) {
			syra_fusion.closeBook(hosterInst, this._noNavOnClose, this.controller && this.controller.sessCloseOnTimeOutPending);
		} else {
			this.dispose();
		}
		...
		if (urlSeg.params && urlSeg.params.d == "on" && urlSeg.params.r) {
			syra_ajax.get({
				...
				success: function(reply) {
					...
					self._bootStrap(urlSeg, null, sap.getSessSetsFromReply(reply.r0), reply.r0, sap.getsrvOptsFromReply(reply.r0), {
						"fixture": {
							"on": true,
							"reply": reply,
							"next": 1,
