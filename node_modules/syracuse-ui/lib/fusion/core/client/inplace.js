...
var formatWrapper = require('syracuse-ui/lib/fusion/core/client/sap/format').FormatterWrapper; // TODO : remove when local format will be completed
...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
    ...
    connect: function(httpQuery, onError){
        var self = this, _elapse = utilBis.getTimer(true), _srvDuration, ourAjaxErr, data = {"settings": {}};
        var hackFmt = new formatWrapper();
        if(hackFmt.settings.localCtrl || hackFmt.settings.localFormat) {
            // TODO : remove when local format will be completed (and remove 'delete' on sendData...)
             httpQuery.sendData = {"settings": {"format": util.duplicateObj(hackFmt.settings)}};
        }
        try {
