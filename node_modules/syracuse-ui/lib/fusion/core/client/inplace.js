...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
	create: function(uuid, hoster, opts) {
		...
		this.cdIsoLang = (syraUserLocalPref = syraUtil.getUserLocalPref()) && syraUserLocalPref.code || syra_site.fusionGateway.defLang;
	...
	connect: function(openerUrlSegments, onError) {
		...
		try {
			...
			self._unLockUI();
			_onError([util.makeDiagnosis(syra_site.fusionGateway.getMessage(this.cdIsoLang, "1") + " 'client.FusionInPlaceSite.connect()' : " + ex.message, 4, ex.stack, "01", util.cltFusionNice)]);
		}
	},
	...
	_ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete) {
		...
		diagnoses.message = diagMess ? diagMess + "\n" + syra_site.fusionGateway.getMessage(null, "23") : syra_site.fusionGateway.getMessage(null, "12");
	...
	_bootStrap: function(openerUrlSegments, rqstLocation, sessSettings, statements, srvOpts, localOpts, onError, acknowledge, notifyComplete) {
		try {
			...
			err = [util.makeDiagnosis(syra_site.fusionGateway.getMessage(this.cdIsoLang, "1") + " 'client.CvgInPlaceSite._bootStrap()' : " + ex.message + ".\n" + syra_site.fusionGateway.getMessage(this.cdIsoLang, "2"), 4, ex.stack, "01", util.cltFusionNice)];
			if (!acknowledge) {
				onError(err, notifyComplete);
			} else {
				acknowledge(isSharedResErr ? diagnoses : null, err);
	...
	free: function(reason) {
		var site = this.getHosterSite(),
			hosterInst = this.getHosterInstance(),
	...
	_fixtureMode: function(openerUrlSegments, onError) {
		...
		if (openerUrlSegments.params && openerUrlSegments.params.d == "on" && openerUrlSegments.params.r) {
			(this._hoster.controller.parseUrl("/sdata/x3stb/erp/stubs/" + openerUrlSegments.params.r)).sendRequest(null, function(reply) {
