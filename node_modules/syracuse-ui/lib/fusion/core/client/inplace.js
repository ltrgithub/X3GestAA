...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
	create: function(uuid, hoster, opts) {
		...
		this.cdIsoLang = (syraUserLocalPref = syra_site.userProfile.getSelectedLocale()) && syraUserLocalPref.code || syra_site.fusionGateway.defLang;
	...
	connect: function(openerUrlSegments, onError) {
		...
		try {
			...
			self._unLockUI();
			_onError([util.makeDiagnosis(syra_site.fusionGateway.getMessage("1") + " 'client.FusionInPlaceSite.connect()' : " + ex.message, 4, ex.stack, "01", util.cltFusionNice)]);
		}
	},
	...
	_ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete) {
		...
		diagnoses.message = diagMess ? diagMess + "\n" + syra_site.fusionGateway.getMessage("23") : syra_site.fusionGateway.getMessage("12");
	...
	_bootStrap: function(openerUrlSegments, rqstLocation, sessSettings, statements, srvOpts, localOpts, onError, acknowledge, notifyComplete) {
		try {
			...
			err = [util.makeDiagnosis(syra_site.fusionGateway.getMessage("1") + " 'client.CvgInPlaceSite._bootStrap()' : " + ex.message + ".\n" + syra_site.fusionGateway.getMessage("2"), 4, ex.stack, "01", util.cltFusionNice)];
			if (!acknowledge) {
				onError(err, notifyComplete);
			} else {
				acknowledge(isSharedResErr ? diagnoses : null, err);
	...
	free: function(reason) {
		...
		if (syra_site && syra_site.fusionGateway && hosterInst) {
			syra_site.fusionGateway.closeBook(hosterInst, this._noNavOnClose);
		} else {
			this.dispose();
		}
		logger.trace({
	...
	_fixtureMode: function(openerUrlSegments, onError) {
		...
		if (!document.syraXTestX) {
			(syra_controller.parseUrl("/sdata/x3stb/erp/stubs/fusion-testdata")).sendRequest(null, function(data) {
		...
		if (openerUrlSegments.params && openerUrlSegments.params.d == "on" && openerUrlSegments.params.r) {
			(syra_controller.parseUrl("/sdata/x3stb/erp/stubs/" + openerUrlSegments.params.r)).sendRequest(null, function(reply) {
