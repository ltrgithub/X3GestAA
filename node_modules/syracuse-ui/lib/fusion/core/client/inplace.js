...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
    create: function(uuid, $$container, hoster, opts){
        ...
        document[sapUtil._fNS].log = document[sapUtil._fNS].log || util.getLog();
    },
    ...
    connect: function(httpQuery, onError){
        ...
        try {
            ...
            if (!this._fixtureMode(httpQuery, onError)) {
                this._hoster.controller._postQuery(httpQuery, null, this._hoster.site,
                    function($location, reply){
                        ...
                        document[sapUtil._fNS].log.timer(null, "Connection duration : total [" + _elapse.elapsedMs() + "], with ajax [" + _srvDuration + "] and local [" + (_elapse.elapsedMs() - _srvDuration) + "]");
                    },
                    function(err) {
                        if((ourAjaxErr = util.isOurAjaxErr(err))) {
                            self._ourConnectError(ourAjaxErr, onError);
                        }
                        else {
                            onError({"$diagnoses": util.getDiagnosesFromAjax(err, self.cdIsoLang)});
                        }
                    }
                );
            }
            ...
            onError({"$diagnoses": [util.makeDiagnosis(localize.getMessage(this.cdIsoLang, "1") + " 'client.FusionInPlaceSite.connect()' : " + ex.message + ".\n\nStack :\n" + ex.stack || null, 4, ex.stack, "01")]});
        }
    },
    ...
    _ourConnectError: function(ourAjaxErr, hosterOnError) {
        ...
        boxParams = util.diagnosesToOurErr(util.makeDiagnosesFromOurAjaxErr(ourAjaxErr));
        msgBox.setMsgBoxDefaultProp(boxParams, 4, 30);
        boxOpts = msgBox.getMsgBoxOpts(boxParams);
        boxOpts.callback = function(resp, closureMode) {
            delete ourAjaxErr.diagnoses;
            hosterOnError();
        };
    ...
    _bootStrap: function(httpQuery, rqstLocation, sessSettings, statements, srvOpts, localOpts, onError){
        try {
            ...
            (this._session = new Session()).initialize(sessSettings, httpQuery, localOpts);
