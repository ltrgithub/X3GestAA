...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
    ...
    connect: function(httpQuery, onError){
        ...
        try {
            ...
            self._unLockUI();
            _onError([util.makeDiagnosis(localize.getMessage(this.cdIsoLang, "1") + " 'client.FusionInPlaceSite.connect()' : " + ex.message, 4, ex.stack, "01", util.cltFusionNice)]);
        }
    },
    ...
    _ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete){
        ...
        var boxOpts, boxParams, diagnoses = util.makeDiagnosesFromOurAjaxErrEx(ourAjaxErr), diagMess = diagnoses.message || diagnoses.$message;
        if (diagMess) {
            if ([".", "?", "!", ":", ";"].indexOf(diagMess.charAt(diagMess.length - 1)) < 0) {
                diagMess += ".";
            }
        }
        diagnoses.message = diagMess ? diagMess + "\n" + localize.getMessage(null, "23") : localize.getMessage(null, "12");
    ...
    _bootStrap: function(httpQuery, rqstLocation, sessSettings, statements, srvOpts, localOpts, onError, acknowledge, notifyComplete){
        try {
