...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
    ...
    connect: function(httpQuery, onError){
		var self = this;
 		try {
	    	...
	    	onError({"$diagnoses": [util.makeDiagnosis("Unexpected error : " + ex.message + "\n" + ex.stack || null, 4, ex.stack)]});
	    }
    },
	...
	_fixtureMode: function(requester, onError) {
       	...
       	if(!document.syraXTestX) {
			(this._hoster.controller.parseUrl("/sdata/x3stb/erp/stubs/fusion-testdata")).sendRequest(null,
	            function (data) {document.syraXTestX = data;},
	            function (err){void (0);}
	        );
	    }
        // Fixture mode
		if(requester.$urlParts.params && requester.$urlParts.params.d == "on" && requester.$urlParts.params.r) {
	        (this._hoster.controller.parseUrl("/sdata/x3stb/erp/stubs/" + requester.$urlParts.params.r)).sendRequest(null,
	            function (reply) {
			        requester.$urlParts.params = requester.$urlParts.params || {};
			        self._bootStrap(requester, null, sap.getSessSetsFromReply(reply.r0), reply.r0, sap.getsrvOptsFromReply(reply.r0), {"fixture": {"on": true, "reply": reply, "next": 1, "hash": "r"}}, onError);
		        },
	            function (err){
		            onError({"$diagnoses": util.getDiagnosesFromAjax(err)}, err);
		        });
	        fixture = true;
	    }
	    return fixture;
	...
	dispose: function() {
		...
		if(this._hoster) {
			_.each(this._hoster, function(prop) {delete this[prop];}, this._hoster);	
			delete this._hoster;
		}
		if(this._$$container){
