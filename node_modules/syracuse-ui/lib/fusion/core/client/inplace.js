...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
    ...
    _initDisconnectRules: function() {
        ...
        this._disconnectCtrlMap = {
            "connecting": {"rnk": 1, "mess": "49", "messForce": "50", "check": this.checkCnxStatus, "checkArgs": [sapUtil.cnxStatus.establishing, null, true], "forceClose": this._emergencyCltClose},
            ...
            "closePending": {"rnk": 3, "mess": "37", "messForce": "46", "check": this.checkCnxStatus, "checkArgs": [sapUtil.cnxStatus.closing, null, true], "forceClose": this._emergencyCltClose},
            "jobPending": {"rnk": 4, "mess": "38", "messForce": "45", "check": this._isJobPending, "release": this._releaseJobPending, "forceClose": this._emergencyCltClose}
        };
    ...
    disconnect: function(callback, timeout, force, noNavOnClose){
        ...
        function callBackEx(diag) {
            if(callback) {
                ...
                if(diag.severity === _kSvrtSuccess) {
                    self._noNavOnClose = noNavOnClose;
                }
                else {
                    self._noNavOnClose = false;
                }
                callback(diag);
            }
        }
        if(this._disposed || !this.controller) {
            ...
            for (i = 0, len = this._disconnectCtrlOrdered.length; i < len; i++) {
                ...
                if(rule.check.apply(this, rule.checkArgs || []) && (!force || (force && rule.release && !rule.release()))) {
        ...
        if(diagnosis) {
            ...
            post = {
                ...
                "callback": function (reply) {
                    var diagnosis = mkDiag(_kSvrtSuccess, "39", "closeDone");
                    if(!self.controller._sapController.isEndSessReply(reply)) {
                        diagnosis = mkDiag(_kSvrtError, "40", "closeRefused");
                        if(force) {
                            diagnosis = mkDiag(_kSvrtSuccess, "41", _KforceCloseTag);
                            setTimeout(function(){self._emergencyCltClose();}, 50);
                        }
                    }
                    setTimeout(function(){callBackEx(diagnosis);}, 200);
                },
                "onErrCallBack": function(reply) {
                    var diagnosis = mkDiag(_kSvrtError, "42", "closeRefused");
                    if(force) {
                        diagnosis = mkDiag(_kSvrtSuccess, "41", _KforceCloseTag);
                        setTimeout(function(){self._emergencyCltClose();}, 50);
                    }
                    setTimeout(function(){callBackEx(diagnosis);}, 20);
                },
                "onSvcPendingCallback": function() {
                    var diagnosis = mkDiag(_kSvrtError, "43", "rqstPending");
                    if(force) {
                        diagnosis = mkDiag(_kSvrtSuccess, "44", _KforceCloseTag);
                        setTimeout(function(){self._emergencyCltClose();}, 50);
                    }
                    setTimeout(function(){callBackEx(diagnosis);}, 20);
                }
        ...
        try {
            ...
            this.controller.onStart(httpQuery, rqstLocation, this._session, statements, function(err, data){
                if (err) {
                    ...
                    // Success!
                    self.updtCnxStatus(sapUtil.cnxStatus.established);
                }
            }, localOpts);
        } 
        catch (ex) {
    ...
    _isJobPending: function(){
        // TODO
        return false;
    },

    _releaseJobPending: function(){
        // TODO
        return true;
    },

    _emergencyCltClose: function(){
        if(this.controller) {
    ...
    free: function(reason){
        var hosterCtrl = this.getHosterController(), hosterInst = this.getHosterInstance(), logger = this._getLogger();
        ...
        if(hosterCtrl && hosterCtrl.fusionGateway && hosterInst) {
            hosterCtrl.fusionGateway.closeBook(hosterInst, self._noNavOnClose);
        }
        else {
            this.dispose();
        }
