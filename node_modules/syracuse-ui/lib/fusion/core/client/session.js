...
exports.Session = helpers.defineClass(Session, null, {
	initialize: function(settings, openerUrlSegments, localOpts, srvOpts) {
		...
		this.cdIsoLang = settings && settings.ADXLangIso ? settings.ADXLangIso : (localOpts && localOpts.cdIsoLang ? localOpts.cdIsoLang : syra_site.fusionGateway.defLang);
	...
	waitSharedRes: function(wait, repeat, callback) {
		...
		if (!stop && repeat > 0) {
			...
			detail = syra_site.fusionGateway.getMessage(this._sharedRes.$resOnErr ? "7" : "22");
			delete this._sharedRes.$resOnErr;
			code = syra_site.fusionGateway.getMessage("8") + (member ? ("\n" + syra_site.fusionGateway.getMessage("15") + "'" + member + "'") : "");
