...
var _staticResConf = {
    "menloc": {"cache": true, "common": false, "name": "{cdLang}", "srvName": "menloc", "srvExt": ".js", "srvSubPath": ["GEN", "{cdLang}", "MENL"].join("/")},
    "localize": {"cache": true, "common": true, "name": "{cdLang}", "srvName": "localize", "srvExt": ".js", "srvSubPath": ["GEN", "{cdLang}", "LIB"].join("/")},
    "styleSheet": {"cache": true, "common": false, "name": "{styleSheet}", "srvName": "{styleSheet}", "srvExt": ".js", "srvSubPath": ["GEN", "ALL", "STY"].join("/")},
    "screen": {"cache": false, "common": false, "name": null, "srvName": null, "srvExt": ".js", "srvSubPath": ["GEN", "ALL", "SCRN"].join("/")}
};
var _staticURLTemplate = "/trans/{endpointEx}/{typres}/{res}", _fusioNS = "syraFusion";
...
exports.Session = helpers.defineClass(Session, null, {
    initialize: function(settings, httpQuery){
        var endpointObj = httpQuery.getEndpoint();
        this.endpointParts = endpointObj ? endpointObj.endpoint.split(endpointObj.sep) : ["x3", "erp", "X3"];
    	this.cdLang = settings ? (settings.cdLang || "ENG") : "ENG";
    	this.styleSheet = settings ? (settings.styles || "STX_DEFAULT") : "STX_DEFAULT";
        this.cnxDate = settings ? (settings.cnxDate || "") : "";
    	this._initSharedRes();
    ...
    _loadSettings: function(settings){
        ...
        _.defaults(this.env || (this.env = {}), {"history": settings.ADXMaskAppName, "sol": settings.AdxSolution}, {"history": null, "sol": ""});
        _.defaults(this.format || (this.format = {}), {"datePivot": settings.DefaultDatePivot, "dateFormat": settings.DefaultDateFormat, "VOn": (settings.VOn && settings.VOn === "1" ? true : false), "ZOn": (settings.ZOn && settings.ZOn === "1" ? true : false)}, {"datePivot": "1940", "dateFormat": "DD[/]MM[/]YY", "VOn": true, "ZOn": true});
        this._mnu = (settings._mnu ? util.duplicateObj(settings._mnu) : []);
        this._months = settings._months ? settings._months.split(";") : ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        this._setUserInf({"code": settings.user, "profile": settings.profil}, false);
    ...
    _setSrvVersions: function(settings){
        ...
        this.version.runtime.protocol = settings.technoProtocol;
        this.version.runtime.vers = settings.technoVersion;
    ...
    _setUserInf: function(userInf, update){
        _.defaults(this.userInf || (this.userInf = {}), (userInf || {}), {"code": "", "profile": ""});
        if(update) {      
    ...
    updateSettings: function(settings){
        ...
        if(settings.styles) {
            this._changeStyleSheet(settings.styles);
        }
    ...
    getWinStaticMnus: function(){
        // TODO : nécessaire ou pas?? dans tous les cas sera stocké au format JSON sur le srv d'appi, 
        // selon la spec http://syracuse/w/index.php/Request_Reply_Fusion#sap.func.open.cdWin.mnu
        return null;
    },
    ...
    getStyleClass: function(code){
        var id = "styleSheet";
	   	return (this._checkSharedRes(id, this.styleSheet) ? this._sharedRes[id][this.styleSheet]["styles"][code] : null);
    },
    ...
    dispose: function(){
    	if (this.endpointParts && document[_fusioNS]._sharedRes[this.endpointParts.join(".")]._cRef > 0){
            // TODO : delete if == 0 ??
            delete this._sharedRes;
            document[_fusioNS]._sharedRes[this.endpointParts.join(".")]._cRef--;
        }
        if(this.version){
            delete this.version;
        }
    ...
    _initSharedRes: function() {
        var endpointId = this.endpointParts.join(".");
 		document[_fusioNS] = document[_fusioNS] || {};
		document[_fusioNS]._sharedRes = document[_fusioNS]._sharedRes || {};
		if(!document[_fusioNS]._sharedRes[endpointId]){
			document[_fusioNS]._sharedRes[endpointId] = {
				"_cRef": 0,
				"_pending": {}
			};
		...
		this._sharedRes = document[_fusioNS]._sharedRes[endpointId];
		this._verifySharedRes();
    ...
    _requestRes: function(member, code, callback){
       if(!_staticResConf[member].cache || !this._sharedRes._pending.hasOwnProperty(member + code)) {
            var self = this, path = [this._parseResExp(_staticResConf[member].srvSubPath), _staticResConf[member].srvName ? this._parseResExp(_staticResConf[member].srvName) : code].join("/") + (_staticResConf[member].srvExt || "");
            if(_staticResConf[member].cache){
                self._sharedRes._pending[member + code] = true;
            }
            (this._getStaticHttpQuery(member, path)).sendRequest(null, !_staticResConf[member].cache ? callback : function(data) {
                if(self._sharedRes) {
    ...
    _getStaticHttpQuery: function(type, res){
        var httpStaticQuery, params = {"endpointEx": this.endpointParts.join("/"), "typres": _staticResConf[type].common ? "$shared" : "$files", "res": res};
        httpStaticQuery = httpController.parseUrl(this._parseResExp.call(params, _staticURLTemplate));
        httpStaticQuery.appendHost();
        return httpStaticQuery;
    }
});