...
var _staticResConf = {
    ...
    "styleSheet": {"wait": true,"cache": true, "common": false, "name": "{styleSheet}", "srvName": "{styleSheet}", "srvExt": ".js", "srvSubPath": ["GEN", "ALL", "STY"].join("/")},
...
exports.Session = helpers.defineClass(Session, null, {
    ...
    _setRegionalOptions: function(settings){
        ...
        _.defaults(this.regionalOptions.date, {"datePivot": settings["date.pivot"], "dateFormat": settings["date.format.server"]}, {"datePivot": "1940", "dateFormat": "DD[/]MM[/]YY"});
        this.regionalOptions.date.months = settings.months ? settings.months.split(";") : ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        _.defaults(this.regionalOptions.numeric, {"decSep": settings["UI_SEPDECIMAL"], "thsndSep": settings["UI_STHOUSAND"]}, {"decSep": ".", "thsndSep": " "});
        _.defaults(this.regionalOptions.format, {"dateShort": settings["UI_SHORTDATE"], "VOn": (settings["formater.von"] && settings["formater.von"] === "on" ? true : false), "ZOn": (settings.ZOn && settings.ZOn === "1" ? true : false)}, {"dateShort": "D:MM[/]DD[/]YYYY", "VOn": true, "ZOn": true});
        _.defaults(this.regionalOptions.time, {"sep": settings["hour.symbol.minute"]}, {"sep": ":"});
    },
    ...
    waitSharedRes: function(wait, repeat, callback) {
        var self = this, stop = true;
        _.each(_staticResConf, function(value, key){
            if(stop && value.cache && value.wait) {
                var name = value.name ? this._parseResExp(value.name) : "";
                stop = this._checkSharedRes(key, name);
            }
        }, this);
        if (!stop && repeat > 0) {
            setTimeout(function(){
                self.logger.trace(null, "Waiting shared resources...");
                self.waitSharedRes(wait, --repeat, callback);
            }, wait);
        }
        else {
            callback(!stop ? [util.makeDiagnosis(this.getlabel("error", "17", "Can't retrieve shared resources. Close session..."), 3, "client.Controller.waitSharedRes()", "17")] : null);
        }       
    },
