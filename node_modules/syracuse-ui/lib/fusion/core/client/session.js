...
var _staticResConf = {
    ...
    "localize": {"wait": true, "cache": true, "common": true, "name": "{cdLang}", "srvName": "localize", "srvExt": ".json", "srvSubPath": ["GEN", "{cdLang}", "LIB"].join("/")},
...
exports.Session = helpers.defineClass(Session, null, {
    initialize: function(settings, httpQuery, localOpts, srvOpts){
    	...
    	this._initSharedRes();
        this._mnu = null;
        this.format = {"wrapper": null, "formatter": null};
    ...
    _setRegionalOptions: function(settings){
        ...
        _.defaults(regOpts.date, {"dateSep": settings.dateSep, "dateOrder": settings.dateOrder, "datePivot": settings.datePivot, "dateFormat": settings.DefaultDateFormat}, {"dateSep": "/", "dateOrder": dateOrderK.MDY, "datePivot": 1940, "dateFormat": "MM[/]DD[/]YY"});
        ...
        _.defaults(regOpts.numeric, {"decimalSep": settings.decimalSep, "thousandSep": settings.thousandSep}, {"decimalSep": ".", "thousandSep": " "});
        _.defaults(regOpts.format, {"vOn": (settings["formater.von"] && settings["formater.von"] === "on" ? true : false), "zOn": (settings.ZOn && settings.ZOn === "1" ? true : false)}, {"vOn": true, "zOn": false});
        _.defaults(regOpts.time, {"timeSep": settings.timeSep}, {"timeSep": ":"});
        ...
        this.format.formatter = this.format.wrapper.initialize(settings.fmtExt, fmtLocales, this, "cvg", {"tracer": function(data) {
            self.logger.trace.apply(self.logger, [{"severety": "warn"}, "Message from Formatter module :"].concat(Array.prototype.slice.call(arguments)));}}
        );
    },
    ...
    getSessInfo: function() {
        var sessInfo = {"login": {}, "client": {}, "version": {}, "node": {}, "runtime": {}};
        // User
    ...
    postInitSharedRes: function() {
        ...
        if(!errMessObj.postInitDone) {
            ...
            _.each(["2", "3", "7", "17", "18", "23", "26", "27", "29", "67"], function(value){
    ...
    waitSharedRes: function(wait, repeat, callback) {
        ...
        if (!stop && repeat > 0) {
            ...
            callback(!stop ? [util.makeDiagnosis(code, 3, "client.Controller.waitSharedRes()", "08", util.cltFusionNice)] : null);
        }       
    },
    ...
    _requestRes: function(member, code, onSuccess, onError){
       if(!_staticResConf[member].cache || !this._sharedRes._pending.hasOwnProperty(member + code)) {
            ...
            (this._getStaticHttpQuery(_staticURLTemplate, {"typres": _staticResConf[member].common ? "$shared" : "$files", "res": path})).sendRequest({"noDisplayErr" : true}, !_staticResConf[member].cache ? onSuccess : function(data) {
                if(self._sharedRes) {
            ...
            !_staticResConf[member].cache ? onError : function (error, httpQuery) {
                if (_staticResConf[member].cache && self._sharedRes) {
                    ...
                    self._sharedRes.$resOnErr = httpQuery.$url;
    ...
    initProtoCache: function (endpointId) {
        if(this._useProtoCache) {
            if(!document[_fusionNS]._sharedRes[endpointId]) {
                document[_fusionNS]._sharedRes[endpointId] = {};
            }
            if(!document[_fusionNS]._sharedRes[endpointId]._protoCache) {
                document[_fusionNS]._sharedRes[endpointId]._protoCache = {};
            }
            return document[_fusionNS]._sharedRes[endpointId]._protoCache;
        }
        else {
            return null;
        }
    ...
    getWorkFlowUrl: function(wrkflwCtx, options){
        var httpQuery, url = ["/", syraUtil.mainPage, "?url=", "/trans/", this.endpointParts.join("/"), "/$sessions", "?f=", wrkflwCtx].join("");
        httpQuery = this._getHttpQuery(url);
   ...
   _parseResExp: function(expr){
    ...
    _getStaticHttpQuery: function(templateUrl, params, options){
        ...
        return this._getHttpQuery(this._parseResExp.call(params, url));
    },

    _getHttpQuery: function(url){
        var httpQuery = httpController.parseUrl(url);
