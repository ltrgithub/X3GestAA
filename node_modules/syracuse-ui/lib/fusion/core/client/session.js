...
exports.Session = helpers.defineClass(Session, null, {
	...
	getHelpBaseUrl: function() {
		...
		if (!url) {
			...
			if ((url = this._getHttpQuery("help/" + this.cdIsoLang, urlPart))) {
				url = url.$url;
			}
		}
		return url || "";
	...
	_requestRes: function(member, code, onSuccess, onError) {
		...
		if (!_staticResConf[member].cache || !this._sharedRes._pending.hasOwnProperty(member + code)) {
			...
			if (_staticResConf[member].cache) {
				...
				options.onError = function(error, httpQuery) {
			...
			(this._getStaticResHttpQuery(member, code)).sendNewRequest(options);
		}
	},

	getWorkFlowUrl: function(wrkflwCtx, options) {
		var httpQuery = this._getHttpQuery([syra_fusion.syraUtil.mainPage, "?url=", "/trans"].join(""), ["$sessions", "?f=", wrkflwCtx].join(""));
		return httpQuery.$url;
	},

	getAppServerHttpQuery: function(shared, subPath, options) {
		var httpQuery = this._getStaticResHttpQuery("genericApp" + (shared ? "Shared" : ""), subPath);
		return options && options.url ? httpQuery.$url : httpQuery;
	},
	...
	_getHttpQuery: function(prefUrl, templateUrl, params, options) {
		...
		var httpQuery = syra_controller.parseUrl(params ? this._parseResExp.call(params, url) : url);
		httpQuery.$url = syra_url.addHost(httpQuery.$url);
		return httpQuery;
	}
});