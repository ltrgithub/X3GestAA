...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
	...
	openBook: function(openerUrlSegments) {
		...
		if (document.site.mainPage) {
			book.openerUrlSegments = document.site.mainPage.openerUrlSegments;
		}
	...
	showSiteFusionHeader: function(show) {
		...
		if (site.homeLink) {
			site.homeLink.mn.style.display = show ? "none" : "";
		}
		if (site.searchField) {
	...
	closeBook: function(book, noNavigation) {
		...
		var openerUrlSegments = book.openerUrlSegments;
		...
		if (!noNavigation) {
			if (openerUrlSegments) {
				document.site.history.load(openerUrlSegments.url = openerUrlSegments.fullUrl);
			} else {
