...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
	...
	showSiteFusionHeader: function(show) {
		if (syra_site.searcher) {
			...
			syra_site.dom.hide(syra_site.searcher.domItem, show);
		}
		if (show) {
			if (!syra_site.fusionGotoFunctionLink) {
				...
				syra_site.fusionGotoFunctionLink = syra_site.loadNewItem(syra_site.headerTop, {
					...
					$act: cvgAct,
					$shortCutTip: shortCutsTip['searchBox']
				});
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			syra_site.dom.toggleClass(link.domItem, "s-fusion-link", show);
		...
		if (!noNavigation) {
			if (openeropenerUrlSegments) {
				syra_url.history.load({
					$url: openeropenerUrlSegments.$url
				});
			} else {
