...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
    _bindEventsEx: function(httpQuery){
        ...
        $(document).delegate("body", "keydown", function(event){
            ...
            if ($fusionPageMeta && !syraUtil.isMsgBoxOpened() && winId && self.activatedBook.selectedSheet.$$item && self.activatedBook.selectedSheet.$$item.is(":visible")) {
    ...
    detachAllBooks: function(notifyHoster, timeout, force, noNavigateOnClose) {
        var i, len, tabClt = [], curr = 0, max = 0, svrtSuccess = "success", diagOk = util.makeDiagnosis("", svrtSuccess, null, "39");
        for (i = 0, len = this._books.length; i < len; i++) {
            tabClt[i] = {"cltRef": this._books[i]};
        }
        max = tabClt.length;
        function release () {
            for (i = 0; i < max; i++) {
        ...
        function walkDetach(diagnosis, hardDetach) {
            var currErr, forceEx = hardDetach !== null && hardDetach !== undefined ? hardDetach : force, noNavOnClose;
            if(diagnosis.severity == svrtSuccess) {
                if (curr == max) {
                    release();
                    notifyHoster(diagnosis);
                }
                else {
                    noNavOnClose = !!(noNavigateOnClose && curr == max - 1);
                    tabClt[curr++].cltRef.fusionSite.disconnect(walkDetach, timeout, forceEx, noNavOnClose);
                }
            }
            else {
                currErr = curr - 1;
