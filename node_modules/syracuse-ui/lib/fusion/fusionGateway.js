...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
	...
	onWindowBeforeUnload: function(event) {
		if (this._books && this._books.length > 0) {
			return syra_site.localize.disconnect_convergence_unload.replace("{booksNb}", this._books.length);
		}
	},
	_promptOnUserProfileChange: function(reason, cbckContinue, cbckCancel) {
		if (this._books && this._books.length) {
			...
			if (reason == "logout") {
				mess = syra_site.localize[prefLocal + reason];
				options.$title = syra_site.localize[prefLocal + "title" + reason];
			} else {
				if (!reason || !(mess = syra_site.localize[prefLocal + reason])) {
					mess = syra_site.localize[prefLocal + "defaultmess"];
				}
				mess += ("\n" + syra_site.localize[prefLocal + "mark"]);
				options.$title = syra_site.localize[prefLocal + "title"];
			}
			mess += ("\n" + String.fromCharCode(1) + "\n" + syra_site.localize[prefLocal + "confirm"]);
			...
			self.serverTimeOutHandler = setTimeout(function() {
				if (self.canDetachBooks()) {
					syra_site.showMessage({
						$message: syra_site.localize.disconnect_timeout_msg,
						$type: "warning",
						$buttons: "yesno",
						$isAutoClose: 25000,
						$default: "no",
						$title: syra_site.localize.disconnect_timeout_title,
	...
	onKeyDow: function(event) {
		...
		if ($fusionPageMeta && !syraUtil.isMsgBoxOpened() && winId && this.activatedBook.selectedSheet.domItem && $(this.activatedBook.selectedSheet.domItem).is(":visible")) {
	...
	showSiteFusionHeader: function(show) {
		...
		if (site.fusionHeaderHomeLink) {
			site.fusionHeaderFunctionLink.domItem.style.display = site.fusionHeaderHomeLink.style.display = show ? "" : "none";
		}
