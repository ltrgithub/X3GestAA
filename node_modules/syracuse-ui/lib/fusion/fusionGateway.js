...
exports.gateway = {
	...
	onWindowBeforeUnload: function(event) {
		...
		if (this._books && this._books.length > 0) {
			if (this.activatedBook && (discardUnload = syra_url.getUrlParam(this.activatedBook.openerUrlSegments, "discardUnloadEvent")) && discardUnload == "true") {
				ret = "$$discard$$";
			} else {
	...
	showSiteFusionHeader: function(show) {
		...
		if (show) {
			if (!syra_site.fusionGotoFunctionLink) {
				...
				syra_site.fusionGotoFunctionLink = syra_site.loadNewItem(syra_site.headerTop, {
					$bind: cvgAct,
					$category: "link",
					$noText: true,
		...
		syra_site.fusionGotoFunctionLink && syra_site.fusionGotoFunctionLink.showItem(show);
	...
	closeBook: function(book, noNavigation, timeout) {
		...
		if (!syra_fusion.isClosingAll && !noNavigation) {
			syra_url.history.load({
				replaceState: syra_url.history.state.editByFusion && (openeropenerUrlSegments && openeropenerUrlSegments.$facet == "$details"),
