...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
    ...
    openMainPage: function(httpQuery){
        ...
        if (book) {
            ...
            this._books[book.id = helpers.uuid.generate()] = book;
    ...
    activateBook: function(book, newSelectedSheet){
        var sheet;
        if (this.$$books) {
            this.$$books.find(".s-fusion-sheet-activated").toggleClass("s-fusion-sheet-activated", false);
        }
        if (this.activatedBook) {
            if (this.activatedBook != book) {
                this.activatedBook.selectedSheet.$$sheetLink.toggleClass("s-fusion-book-link-selected", false);
                this.activatedBook.unselectSheet();
    ...
    _bindEvents: function(bind){
        ...
        if (bind) {
            self.$$books.delegate(".s-fusion-sheets-opener-children", "click", function(){
                ...
                var $$link = $(this);
                var book = self._findBook($$link), newSelectedSheet;
                if (!$$link.hasClass("s-fusion-book-link")) {
                    newSelectedSheet = book.sheets[$$link.attr("data-s-sheet")];
                }
                self.activateBook(book, newSelectedSheet);
                setTimeout(function(){
                    self.closeMenus();
                }, 100);
                return false;
