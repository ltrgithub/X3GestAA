...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
    _bindEventsEx: function(httpQuery) {
        ...
        $(document).delegate("body", "keydown", function(event){
            var $fusionPageMeta, doEvt, isGrid = false, winId;
            ...
            if($fusionPageMeta && winId && self.activatedBook.selectedSheet.$$item && self.activatedBook.selectedSheet.$$item.is(":visible")) {
                console.log("**onGatewayEvent : [" + event.type + "] for " + $fusionPageMeta.winModel.syraModel.getTitle());
    ...
    openMainPage: function(httpQuery){
        ...
        var book = new WorkBook(), self = this;
        book.fusionGateway = this;
        book.$syraOpenPage = document.site.mainPage.$memItemPage;
    ...
    _releaseBook: function(book, noNavigation){
        ...
        var $syraOpenPage = book.$syraOpenPage;
        delete this._books[book.id];
        document.controller.disposeObject(book);
        this._refreshCount();
        if (!noNavigation) {
            document.site.onMainPageChange($syraOpenPage);
        }
    },
    ...
    _bindEvents: function(bind){
        ...
        if (bind) {
            self.$$books.delegate(".s-fusion-sheets-opener-children", "click", function(){
                ...
                if (self.activatedBook && self.activatedBook != book) {
                ...
                if (document.site.mainPage != (sheet = book.sheets[0])) {
                    var body = document.site.$$body[0];
                    body.style.display = "none";
                    document.site._releaseMainPage();
                    book.selectSheet(document.site.mainPage = book.sheets[0]);
                    body.style.display = "";
                }
    ...
    _createMenus: function(){
        ...
        (self.$$menusOpener = $("#s-fusion-sessions-opener")).bind("click", function(){
            if (!self.popupMenus) {
                self._createTree();
                self.$$menusPanel[0].style.zIndex = 3200;
                self.popupMenus = document.site.createPopup({
                    $$popup: self.$$menusPanel,
                    position: {
                        my: "right top",
                        at: "right bottom",
                        of: self.$$menusOpener
                    },
                    boundarySelector: "#s-fusion-sessions-opener",
                    closeHandler: function(){
                        setTimeout(function(){
                            self.$$menusPanel.empty();
