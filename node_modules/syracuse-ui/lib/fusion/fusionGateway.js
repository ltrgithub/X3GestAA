...
exports.FusionGateway = helpers.defineClass(FusionGateway, null, {
    ...
    detachAllBooksPanic: function(){
        var i, len = this._books.length, tabSid = [], nieme, masterBookCtrl, strgSid = null;
        if (len > 0) {
            for (i = 0; i < len; i++) {
    ...
    detachAllBooks: function(notifyHoster, timeout, force, noNavigateOnClose, opts){
        var i, len, tabSid = [], tabClt = [], curr = 0, max = 0, svrtSuccess = "success", diagOk = util.makeDiagnosis("", svrtSuccess, null, "39");
        for (i = 0, len = this._books.length; i < len; i++) {
            tabClt[i] = {
