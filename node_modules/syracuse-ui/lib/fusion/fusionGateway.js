	...
	showSiteFusionHeader: function(show) {
		...
		if (show) {
			if (!btns.fusion_goto) {
				...
				btns.fusion_goto = syra_site.addItem(syra_site.headerTop, {
					$bind: cvgAct,
					$noText: true,
					$category: "link",
					$skin: "s-site-btn-goto",
					$act: cvgAct,
				...
				btns.home.domItem.parentNode.insertBefore(btns.fusion_goto.domItem, btns.home.domItem);
			}
		}
		...
		syra_dom.toggleClass(btns.navigation.domItem, "s-fusion-link", show);
		btns.navigation.isExternalHidden = true;
		btns.navigation.setMenu({
			$title: show ? syra_local.site_btn_navigation_fusion : syra_local.site_btn_navigation
		});
		btns.navigation.$target = show ? "blank" : "";
	...
	closeBook: function(book, noNavigation) {
		...
		if (!syra_fusion.isClosingAll && !noNavigation) {
			syra_url.history.load({
				$url: openerUrlSeg ? openerUrlSeg.$url : syra_site.menuItems.$navigation[0].$url
			});
		}
	},
