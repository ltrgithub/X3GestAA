...
exports.gateway = {
	...
	onWindowBeforeUnload: function(event) {
		...
		if (this._books && this._books.length > 0) {
			if (this.activatedBook && (discardUnload = syra_url.getUrlParam(this.activatedBook.urlSeg, "discardUnloadEvent")) && discardUnload == "true") {
				ret = "$$discard$$";
			} else {
				ret = syra_local.disconnect_convergence_unload.replace("{booksNb}", this._books.length);
			}
		}
		return ret;
	},
		...
		if (this._books && this._books.length) {
			...
			syra_alert.modal({
				...
				click: function(response, closedBy) {
					...
					if (!close) {
						syra_site.userProfile.toggle(false);
						if (cbckCancel) {
							cbckCancel();
						}
					} else {
	...
	openMainPage: function(urlSeg) {
		urlSeg.$url = syra_url.addHost(urlSeg.$url);
		this.openBook(urlSeg);
	},
	...
	openBook: function(urlSeg) {
		var self = this;
		var book = new WorkBook();
		this.activateBook(book);
		book.urlSeg = urlSeg;
		if (syra_site.mainPage) {
			book.openerUrlSeg = syra_site.mainPage.urlSeg;
		}
	...
	showSiteFusionHeader: function(show) {
		var btns = syra_site.btns;
		if (show) {
			syra_site.searcher.applyEscape();
		}
		syra_button.hide(btns.search, show);
		if (show) {
			if (!btns.fusion_goto) {
				var cvgAct = _requestActions.getSrvAction("SRV_ADXEXECUTE");
				btns.fusion_goto = syra_site.addItem(syra_site.headerBar, {
					$bind: cvgAct,
					$noText: true,
					$category: "link",
					$skin: "s-header-link s-header-search",
					$act: cvgAct,
					$shortCutTip: syra_shortCuts.tip.searchBox
				});
				btns.fusion_goto.setMenu({
					$title: syra_local.site_btn_goto
				});
				syra_site.headerBar.insertBefore(btns.fusion_goto.domItem, btns.search.link);
			}
		}
		syra_dom.toggleClass(btns.home.domItem, "s-fusion-link", show);
		btns.home.isExternalHidden = true;
		btns.home.setMenu({
			$title: show ? syra_local.site_btn_home_fusion : syra_local.site_btn_home
		});
		btns.home.$target = show ? "blank" : "";
		btns.fusion_goto && btns.fusion_goto.showMenuItem(show);
	},
	closeBook: function(book, noNavigation, timeout) {
		if (this.activatedBook == book) {
			this.activatedBook = null;
		}
		var openerUrlSeg = book.openerUrlSeg;
		...
		if (!syra_fusion.isClosingAll && !noNavigation) {
			syra_url.history.load({
				replaceState: syra_url.history.state.editByFusion && (openerUrlSeg && openerUrlSeg.$facet == "$details"),
				$url: openerUrlSeg ? openerUrlSeg.$url : syra_site.menuItems.$home[0].$url,
				timeout: timeout
			});
		}
	},
	...
	executeFusionFunc: function(menuItem, workBook) {
		var executed = false;
		if (workBook) {
			...
			if (menuItem.urlSeg.fusionParams) {
				...
				externalAdapter = externalAdapter && externalAdapter.onBlockExRpc ? externalAdapter : workBook.selectedSheet && workBook.selectedSheet.externalAdapter;
				if (sapController && externalAdapter.onBlockExRpc) {
