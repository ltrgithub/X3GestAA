...
exports.log = function() {
    if (window.console) {
        var args = arguments;
        var str = exports.formatInt(_logTimer.elapsedMs(), 5);
        ...
        else window.console.log(Array.prototype.slice.call(args));
    }
}
exports.logObj = function(o, name, level) {
    var level = level != null ? level : 0;
    var pref = ""
    for (var i = 0; i < level; i++) 
        pref += "  ";
    if (o == null || typeof(o) == "undefined") {
        exports.log(pref + name + "=" + o);
    } else if (_.isArray(o) || _.isObject(o)) {
        exports.log(pref + name + "\n");
        _.each(o, function(a, b) {
            exports.logObj(a, b.toString(), level + 1);
        });
    } else {
        exports.log(pref + name + "=" + o.toString());
    }
}
...
exports.formatInt = function(num, leading0) {
    if (leading0 == null) return num.toString();
    num = Math.round(num);
...
exports.err2Str = function(err, log) {
    ...
    var res = e.toString();
    if (log && console) console.log(res);
    return res;
}
// Statics
...
function _jsErr2Str(e) {
    ...
    var res = [];
    if (e.message != null) res.push("message: " + e.message);
    if (e.fileName != null) res.push("fileName: " + _parseFName(e.fileName));
    if (e.lineNumber != null) res.push("lineNumber: " + e.lineNumber);
    if (e.stack != null) res.push("stack:\n" + _parseFName(e.stack, "   "));
    if (e.source != null) res.push("source:\n  " + e.source);
    for (var p in e) {
...
var _classTimer = helpers.defineClass(function Timer(start) {
    ...
    stop: function() {
        if (this._stop != null) this._stop = new Date().getTime();
        return this._stop;
    },
    elapsedMs: function() {
...
var _logTimer = new _classTimer(true);
...
exports.callHandlerLater = function() {
    ...
    if (jQuery.browser.mozilla) handler.useTimeout = true;
    ...
    if (ele.classList) {
        try {
            if (newClass == null) {
                ...
                if (oldClass) ele.classList.remove(oldClass);
                if (newClass) ele.classList.add(newClass);
            }
            return true;
        } catch (e) {
        }
...
exports.removeClass = function(e, klass) {
    ...
    for (var i = 0; i < klass.length; i++) {
        ...
        else e.className = _replaceClass(e.className, c, "");
    }
};
...
// elmt = tag li ou treeBody - force=true -> force add klass - force=false --> force remove false - undefiend --> Toggle
...
exports.fixtureMode = function() {
    return sapUtil.Syra.getFusionController(self).isFixtureMode()
}
