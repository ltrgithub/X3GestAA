			...
			if (errAjax.data && errAjax.data.length > 0) {
				...
				if (errAjax.status && errAjax.status == 404) {
					if (!messObj) {
						...
						errMess = _.isString(messObj) ? syra_site.fusionGateway.getMessage("5") + "\n" + syra_site.fusionGateway.getMessage("6") : messObj.getlabel("error", "14", "Can't get resource on web server") + "\n" + messObj.getlabel("error", "15", "HTTP status");
					}
					...
					if (!messObj) {
						...
						errMess = _.isString(messObj) ? syra_site.fusionGateway.getMessage("7") + "\n" + syra_site.fusionGateway.getMessage("6") : messObj.getlabel("error", "16", "HTTP error") + "\n" + messObj.getlabel("error", "15", "HTTP status");
					}
...
exports.diagnosesToOurErrEx = function(diagnoses) {
	...
	if ((!diag.$diagnoses || diag.$diagnoses.length == 0) && ((diagStack = diag.stackTrace || diag.$stackTrace))) {
		diag.$diagnoses = [];
		diag.$diagnoses.push(exports.makeDiagnosis(syra_site.fusionGateway.getMessage("stackT"), 3, diagStack));
	...
	if ((diagStack = diag.stackTrace || diag.$stackTrace)) {
		...
		diag.$diagnoses.splice(0, 0, exports.makeDiagnosis(diagDetail || syra_site.fusionGateway.getMessage("stackT"), 3, diagStack));
	}
	...
	_.each(ourErr.diagnoses, function(diagnosis, idx) {
		if ((diagnosis.$stackTrace || diagnosis.stackTrace) && !diagnosis.$message && !diagnosis.message) {
			diagnosis.$message = syra_site.fusionGateway.getMessage("stackT");
		}
	});
	return ourErr;
};
