...
exports.diagnosesToOurErrEx = function (diagnoses){
    ...
    ourErr.li[0] = (diag.message || "") +  (diag.appCode || diag.sdataCode  ? (" [" + (diag.appCode || diag.sdataCode) + "]") : "");
    ...
    ourErr.details = diag.$details;
    ourErr.diagnoses = diag.$diagnoses;
    ourErr.links = diag.$links;
    if(compatibiltyMode) {
        // TODO remove!
        if(ourErr.details) {
            ourErr.li[ourErr.li.length] = ourErr.details;
        }
        _.each(ourErr.diagnoses, function(err, idx){
            ourErr.li[ourErr.li.length] = (err.message || "") +  (err.appCode || err.sdataCode  ? (" [" + (err.appCode || err.sdataCode) + "]") : "") + (err.stackTrace ? ("\n\nStack :\n " + err.stackTrace) : "");
        });
    }
    return ourErr;
};
