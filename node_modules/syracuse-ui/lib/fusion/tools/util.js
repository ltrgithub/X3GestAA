			...
			if (errAjax.data && errAjax.data.length > 0) {
				...
				if (errAjax.status && errAjax.status == 404) {
					if (!messObj) {
						...
						errMess = _.isString(messObj) ? syra_site.fusionGateway.getMessage(messObj, "5") + "\n" + syra_site.fusionGateway.getMessage(messObj, "6") : messObj.getlabel("error", "14", "Can't get resource on web server") + "\n" + messObj.getlabel("error", "15", "HTTP status");
					}
					...
					if (!messObj) {
						...
						errMess = _.isString(messObj) ? syra_site.fusionGateway.getMessage(messObj, "7") + "\n" + syra_site.fusionGateway.getMessage(messObj, "6") : messObj.getlabel("error", "16", "HTTP error") + "\n" + messObj.getlabel("error", "15", "HTTP status");
					}
...
exports.diagnosesToOurErrEx = function(diagnoses) {
	...
	if ((!diag.$diagnoses || diag.$diagnoses.length == 0) && ((diagStack = diag.stackTrace || diag.$stackTrace))) {
		diag.$diagnoses = [];
		diag.$diagnoses.push(exports.makeDiagnosis(syra_site.fusionGateway.getMessage(null, "stackT"), 3, diagStack));
	...
	if ((diagStack = diag.stackTrace || diag.$stackTrace)) {
		...
		diag.$diagnoses.splice(0, 0, exports.makeDiagnosis(diagDetail || syra_site.fusionGateway.getMessage(null, "stackT"), 3, diagStack));
	}
	...
	_.each(ourErr.diagnoses, function(diagnosis, idx) {
		if ((diagnosis.$stackTrace || diagnosis.stackTrace) && !diagnosis.$message && !diagnosis.message) {
			diagnosis.$message = syra_site.fusionGateway.getMessage(null, "stackT");
		}
	});
	return ourErr;
};
...
var _classLog = helpers.defineClass(function Log(options) {
	...
	timer: function(options) {
		if (this._ptmrOn) {
			this._report.push(exports.makeDiagnosis("Timming", "warning", utilBis.logEx.call(this, {
				"severety": "info"
	...
	_dispReport: function(options) {
		...
		var self = this;
		diag.push(exports.makeDiagnosis("Classic client : timing and trace report", "info", null, 0, null, "", null, this._report));
		boxParams = exports.diagnosesToOurErrEx(diag);
