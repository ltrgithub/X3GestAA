...
exports.diagnosesToOurErrEx = function (diagnoses){
    var diag = diagnoses, ourErr = {"li": []}, mainDiagnosis = 0, topSevevity = 0, diagSeverity, diagStack;
    ...
    if(!diag.$diagnoses || diag.$diagnoses.length == 0 && ((diagStack = diag.stackTrace || diag.$stackTrace))) {
        diag.$diagnoses = [];
        diag.$diagnoses.push(exports.makeDiagnosis ("Stack trace", 3, diagStack));
        delete diag.stackTrace;
        delete diag.$stackTrace;
    }
    else if((diagStack = diag.stackTrace || diag.$stackTrace)) {
        ourErr.li[1] = diagStack;
    }
...
exports.makeDiagnosis = function (label, severity, stackTrace, appCode, origin, details, links, childDiagnoses){
    var severityEx = _diagnosSeverity[3], num;
...
exports.isExpectedBrowser = function (expected) {
    var ret = null, b = document && document.site ? document.site.getBrowserName().toLowerCase() : null;
    if(b && b == expected) {
        ret = true;
    }
    else if(b) {
...
var _classLog = helpers.defineClass(function Log(options) {
    ...
    toggleLog: function () {
        var flags = this.getLogSwitch(), turnOn = true, j, len, array;
