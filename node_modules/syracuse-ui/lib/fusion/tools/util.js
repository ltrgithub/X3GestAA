...
exports.diagnosesToOurErrEx = function(diagnoses) {
	...
	if (_.isArray(diagnoses)) {
		...
		_.each(diagnoses, function(diagnosis, idx) {
			...
			if (topSevevity !== null && diagnosis.$diagnoses) {
				mainDiagnosis = idx;
				topSevevity = null;
			} else if (topSevevity !== null && diagSeverityInner) {
	...
	if ((!diag.$diagnoses || diag.$diagnoses.length == 0) && ((diagStack = diag.stackTrace || diag.$stackTrace))) {
		diag.$diagnoses = [];
		diag.$diagnoses.push(exports.makeDiagnosis(localize.getMessage(null, "stackT"), 3, diagStack));
		delete diag.stackTrace;
		delete diag.$stackTrace;
	} else if ((diagStack = diag.stackTrace || diag.$stackTrace)) {
		if ((diagDetail = diag.$details || diag.details)) {
			diag.$details = null;
			diag.details = null;
		}
		diag.$diagnoses.splice(0, 0, exports.makeDiagnosis(diagDetail || localize.getMessage(null, "stackT"), 3, diagStack));
	}
	...
	_.each(ourErr.diagnoses, function(diagnosis, idx) {
		if ((diagnosis.$stackTrace || diagnosis.stackTrace) && !diagnosis.$message && !diagnosis.message) {
			diagnosis.$message = localize.getMessage(null, "stackT");
		}
	});
	return ourErr;
};
...
exports.makeDiagnosisLocale = function(localeObj, labelCode, labelCodeArgs, labelString, severity, details, stack, origin, links, appCode) {
	var mess = labelString;
	if (labelCode) {
		mess = localeObj[labelCode];
