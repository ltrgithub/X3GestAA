	...
	collapseBlock: function(block, show, isFirstTime, onDrawBar) {
		...
		self.page.externalAdapter.onBoxToggle({
			...
			doEvent: function() {
				...
				document.site.toggleClass(block.header, "s-open", show);
				document.site.toggleClass(block.domItem, "s-open", show);
				if (show) {
					if (!block.islistLoaded) {
						...
						document.site.emptyDom(block.body);
			...
			if (this.openedBlock) {
				...
				for (var ii = 0, jj = this.blocks.length; ii < jj; ii++) {
					...
					if (block != this.openedBlock) {
						blockHeight += document.site.getBoundingClientRect(block.domItem).height;
					}
				}
				this.openedBlock.domItem.style.height = (document.site.getInnerSize(this.barBody).height - blockHeight) + "px";
				var inner = document.site.getInnerSize(this.openedBlock.domItem);
				this.openedBlock.body.style.height = (inner.height - document.site.getBoundingClientRect(this.openedBlock.header).height) + "px";
				if (this.openedBlock.leftList) {
					this.openedBlock.leftList.resize(document.site.getInnerSize(this.openedBlock.body).height);
				}
			}
		}
	},
