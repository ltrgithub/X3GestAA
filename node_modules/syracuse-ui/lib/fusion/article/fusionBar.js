...
exports.FusionBar = helpers.defineClass(FusionBar, BarSplitter, {
	load: function(page, $fusionBar) {
		...
		if (document.site.isTabletDevice) {
	...
	onClickPicker: function(picker, event) {
		switch (picker.getAttribute("data-s-picker")) {
			case "s-bar-title":
				this.collapseBlock(event.target.syraBlock, true);
				break;
			default:
	...
	collapseBlock: function(block, show, onDrawBar) {
		...
		self.page.externalAdapter.onBoxToggle({
			...
			doEvent: function() {
				...
				document.site.dom.toggleClass(block.header, "s-open", show);
				document.site.dom.toggleClass(block.domItem, "s-open", show);
				if (show) {
					if (!block.islistLoaded) {
						...
						document.site.dom.empty(block.body);
					...
					if (!onDrawBar) {
						...
						self.resizeBar();
						if (block.leftList) {
							block.leftList.ensureArticleVisibility();
							block.leftList.resizeArticle();
						}
					}
				} else {
					if (block.leftList) {
