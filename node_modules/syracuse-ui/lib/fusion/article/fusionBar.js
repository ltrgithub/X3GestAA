...
exports.FusionBar = helpers.defineClass(FusionBar, BarSplitter, {
	...
	onBlockTitleClick: function(event, btn) {
		for (var ii = 0, jj = this.blocks.length; ii < jj; ii++) {
			if (this.blocks[ii].title == btn) {
	...
	createBlock: function($item, index, count) {
		...
		block.title = syra_menus.addTextButton($title, "s-fusion-bar-block-title", "onBlockTitleClick");
				...
				if (show) {
					if (!block.leftList) {
						...
						if (block.leftList) {
							...
							block.leftList.filler.validateDisplay(block.leftList.filler);
						}
					}
	...
	resizeBar: function() {
		if (BarSplitter.prototype.resizeBar.call(this) || this.resizeOnExpand) {
