...
exports.SheetPage = helpers.defineClass(SheetPage, DesktopPage, {
    ...
    _setHeader:function(){
 		/*var $headerRight=$("#s-site-header-top-right-fusion")
    	if($headerRight && !$("#s-site-func")[0]){
    		//Build 
    		var div = document.createElement("div");
	        div.setAttribute("id", "s-site-func");
	        this.loadNewItem($headerRight[0].appendChild(div), {
	            $bind: "1186",
	            $category: "link",
	            $noText: true,
        ...
        if ($item.$isNavigationList || ($class === "application/x-array" && sapUtil.isListTab($item.$bind))) {
        	...
        	if (!$item.$cards) {
				...
				for (var key in $field.$item.$properties) {
					if (count % 3 === 0) {
						ii=cards.$layout.$items.push({"$layoutType": "row","$items":[]});
