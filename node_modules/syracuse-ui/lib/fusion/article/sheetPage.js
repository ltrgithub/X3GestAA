...
exports.SheetPage = helpers.defineClass(SheetPage, DesktopPage, {
	...
	appendArticleMenus: function(slot) {
		...
		DesktopPage.prototype.appendArticleMenus.call(this, slot);
	},
	drawBox: function() {
	...
	_setHeader: function() {
		...
		var $$newTab, fctItem, div, id = syraUtil.pageHeaderK.linkGotoId,
			cvgAct = requestActions.getSrvAction("SRV_ADXEXECUTE");
	...
	createField: function($field, $item, boxParent, $class) {
		...
		if ($item.$isNavigationList || ($class === "application/x-array" && sapUtil.isListTab($item.$bind))) {
			...
			$item.$format = "grid";
		} else if ($class === "application/x-array") {
			...
			if (!$item.$cards) {
				...
				for (var key in $field.$item.$properties) {
					if (count % 3 === 0) {
						ii = cards.$layout.$items.push({
							"$layoutType": "row",
							"$items": []
						});
						items = cards.$layout.$items[ii - 1].$items;
					}
					items.push({
						"$bind": key
					});
	...
	applyShortCuts: function(shortcurts) {
		...
		if (shortcurts.esc) {
			...
			if (shortcurts.f9) {
				if (lastFocusField && lastFocusField.tunnelPicker) {
					lastFocusField.tunnelPicker.click();
				}
				return true;
			}
			if (shortcurts.n) {
			...
			if (shortcurts.tab) {
				//nextFieldConvGrid
				if (lastFocusField && lastFocusField.arrayLevel === "cell") {
					lastFocusField.page.externalAdapter.onFieldClickPicker({
						"field": lastFocusField,
						"pickerType": "cancelEdit"
					});
				}
				return true;
			}
			if (shortcurts.h && (shortcurts.a || shortcurts.f11 || shortcurts.l)) {
				if (this.fusionBar) {
		...
		return DesktopPage.prototype.applyShortCuts.call(this, shortcurts);
	},
	dispose: function() {
