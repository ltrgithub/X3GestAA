...
exports.SheetPage = helpers.defineClass(SheetPage, DesktopPage, {
    onBarEvent: function(picker, event){
        if (picker.className.indexOf("s-fusion-bar") >= 0) {
            this.fusionBar.onClickPicker(picker, event);
        }
        else {
    ...
    loadBox: function(initData, $initDiagnoses){
        ...
        if (!this.$prototype.$fusionBar) {
            ...
            if (!this.$item.$isSimplified) {
                ...
                this.$item.$layout = document.site.layoutUpdater.cleanConvergenceSetting(this.$item.$layout);
        ...
        $(this.domItem).delegate("." + this.$skin + "-breadcrumb-link", "click", function(event){
        ...
        document.site.fusionGateway.showSiteFusionHeader(true);
    },
    ...
    ensureSiteFusionHeader: function(){
        ...
        if (!site.fusionHeaderHomeLink) {
            ...
            site.fusionHeaderFunctionLink = document.site.loadNewItem(site.headerTop, {
                $bind: cvgAct,
                $category: "link",
                $noText: true,
            ...
            site.fusionHeaderFunctionLink.domItem.style.display = "none";
            site.fusionHeaderFunctionLink.domItem.title = document.site.localize.fusionHeaderFunctionLink;
            
            site.fusionHeaderHomeLink = document.createElement("a");
            site.fusionHeaderHomeLink.className = "s-site-fusion-home";
            site.fusionHeaderHomeLink.style.display = "none";
            site.fusionHeaderHomeLink.setAttribute("href", "?representation=home.$landing");
            site.fusionHeaderHomeLink.setAttribute("target", "_blank");
            site.fusionHeaderHomeLink.title = document.site.localize.fusionHeaderHomeLink;
            site.headerTop.appendChild(site.fusionHeaderHomeLink);
        }
    },
