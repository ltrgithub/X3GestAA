...
exports.SheetPage = helpers.defineClass(SheetPage, DesktopPage, {
	...
	ensureSiteFusionHeader: function() {
		if (!syra_site.fusionHeaderHomeLink) {
			...
			syra_site.fusionHeaderFunctionLink.setMenu({
				$title: syra_site.localizer.fusionHeaderFunctionLink
			});
			...
			syra_site.fusionHeaderHomeLink.setMenu({
				$title: syra_site.localizer.userProfile_siteTabLink
			});
	...
	applyShortCuts: function(shortcurts, event, focusField) {
		...
		if (shortcurts.esc) {
			...
			if (shortcurts.f9) {
				if (this.focusField && this.focusField.pickersHelper) {
					this.focusField.pickersHelper.clickTunnel(this.focusField);
				}
				return true;
			}
			if (shortcurts.n) {
