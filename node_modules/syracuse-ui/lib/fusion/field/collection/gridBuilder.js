...
exports.Builder = helpers.defineClass(GridBuilder, RawListBuilder, {
    ...
    drawBuilder: function(){
        ...
        var $$list = $("<div/>").addClass(self.makeSkinFusion(list.$skin)).hide();
    ...
    onClick: function(ctrl,ev){
		...
		var self=this;
        if (ctrl) {
			var p=this.getOnClickParams(ev)
            doEvt = self.ctrl.trigger(p.event,p.opt); 
        }
		return doEvt;
    },
	getOnClickParams:function(ev){
		...
		var p={"event":"",opt:{}};
		if(lineClicked) {
			p.event="wdgt.grid.command";
			p.opt={"target": this, "type": "click","data":{"line":(this.$recordIndex+1),"xid":this.boxParent._getGridXid()}};
		}else{
			var css = this.boxParent.$skin + "-cell";
			p.event="wdgt.grid.focuscell";
			p.opt={"target": this, "type": "click","data":{"line":(this.$recordIndex+1),"xid":(($(ev.target).hasClass(css))?$(ev.target).attr('id'):$(ev.target).parents("."+css).attr('id'))}}
...
*/           
    ...
    resizeWidth: function(wh){
        ...
        //this._$$innerhScroller.width(l + 18);
	this._$$innerhScroller.width(this.sizesline.scroll.minLength+this.sizesline.frozen.minLength + 18);
