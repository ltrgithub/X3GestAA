...
exports.Builder = helpers.defineClass(GridBuilder, null, {
    initialize: function(){
        this.isFusionTemp = true;
    ...
    drawBuilder: function(){
        ...
        if (isNew) {
            list._$$body = $(this._body = document.createElement("div")).appendTo($$list).addClass(self.makeSkinFusion(list.$skin, "-content")); //container
    ...
    endBinding: function($isDelta){
        ...
        if (this.list.callBack) {
            this.list.callBack($isDelta, null, this.list._store._map);
        }
    ...
    reSynch: function(record){
        ...
        Object.keys(list._store._map).forEach(function(val){
            if (list._store._map[val].$recordIndex >= start) {
                if (del) {
                    list._store._map[val]._refresh(list._store._map[val].$recordIndex - count, insrt, del);
                }
                else {
                    list._store._map[val]._refresh(list._store._map[val].$recordIndex + count, insrt, del);
                }
                ...
                if (list._store._map[val].$recordIndex < 0) {
                    list._store._map[val]._refresh(Math.abs(list._store._map[val].$recordIndex) - 1 + start, insrt, del);
                }
            }
        });
        if (del) {
    ...
    _makeTitleRow: function(){
        var self = this;
        if (!self.list.$item.$isTitleRowHidden) {
            var list = self.list;
            var o = {};
            ...
            if (!self.$fieldnl.$isHidden) {
                ...
                list.$fieldProperties[self.getGridXid() + "_NL"] = self.$fieldnl;
            }
    ...
    isFieldFrozen: function($field){
        return ($field.$capabilities && ($field.$capabilities.indexOf("frozen") >= 0));
    },
    ...
    _computeMinWidth: function($field, $item){
        ...
        if ($field && !$field.$isHidden) {
            ...
            $field.$maxLength = ($field.$maxLength >= 3) ? $field.$maxLength : 3;
            $field.$showLength = ($field.$showLength) ? $field.$showLength : $field.$maxLength;
            $field.$cellMinLength = Math.max($field.$showLength, 3) * this.size("charWidth");
            var tl = line.frozen;
            if (!this.isFieldFrozen($field)) {
                tl = line.scroll;
            }
    ...
    _cellVisible: function(xid){
        var list = this.list, $field = list.$fieldProperties[xid], pos = 0;
        if (this.isFieldFrozen($field)) 
            return null;
    ...
    sort: function(){
        ...
        this.sortCriter.forEach(function(crit){
            crit.binding = crit.p;
            crit.t = crit.$type = self.list.$field.$item.$properties[crit.p].$type;
        })
        self.list._store.sort({
            dataRecordSet: self.rowsOrder
        }, self.sortCriter, "dataset")
        self.redrawRows()
    },
    ...
    resetSort: function(){
        this.sortCriter = []
        this.list._store.sort({
            dataRecordSet: this.rowsOrder
        }, [{
            "binding": "$recordIndex",
            "asc": true,
    ...
    //DC:bug metaData n'est pas pass�
    endGuiMods: function(){
    ...
    setGuimodsByTyp: function($guimod, $typ){
        ...
        $guimod.forEach(function(item){
            if (list.$fieldProperties[item[0]]) {
                if (!list.$fieldProperties[item[0]].$isHidden) {
                    if (item[1] === 0) { //col
                        if ($typ === "tit" || $typ === "stt") {
                        ...
                        if ($typ !== "tit") {
                            ...
                            Object.keys(list._store._map).forEach(function(val){
                                if (!list._store._map[val].guimodCel[item[0]]) {
                                    list._store._map[val].guimodCel[item[0]] = new X3GuiMod();
                                }
                                list._store._map[val].guimodCel[item[0]][$typ](item[2], list._store._map[val]._isBuilded());
                            });
                        }
                    }
                    else { //cel
        ...
        if (self.guimodCol) {
            for (var key in self.guimodCol) {
                ...
                if (self.guimodCol[key]._status && self.guimodCol[key]._status.$isHidden) {
                    w += self.list.$fieldProperties[key].$cellLength;
                };
                            };
                    };
    ...
    lineCount: function(v){
        ...
        this.list._store.pagingCapability.setOptions({
            $itemsPerPage: this.$lineCount
        });
    },
    ...
    resizingCell: function(ev, target, pos, sizeI){
        ...
        this.list.$fieldProperties[$(target.parentElement).attr("s-bind")].$cellLength = w
    ...
    stopResizeCell: function(ev, target){
        ...
        w = this.list.$fieldProperties[$(target.parentElement).attr("s-bind")].$cellLength;
        return;
    ...
    getStringXmlData: function(){
        var xAr = [], list = this.list, ordered = this.rowsOrder.slice(0);
        var Prfx = this.getGridXid().substr(this.getGridXid().length - 1);
        list._store.sort({
            dataRecordSet: ordered
        }, [{
            "binding": "$recordIndex",
            "asc": true,
            "$type": "application/x-integer"
    ...
    getStringXmlColumnUsage: function($Caller){
        ...
        Object.keys(list.$fieldProperties).forEach(function(key){
            if (list.$fieldProperties[key].$X3) {
                xAr.push('<CPH XD1="' + key.substr(1) + '" ');
                xAr.push('NAM="' + list.$fieldProperties[key].$X3.$name + '" ');
                xAr.push('TYP="' + list.$fieldProperties[key].$X3.$type + '" ');
                xAr.push('FMT="' + list.$fieldProperties[key].$X3.$format + '" ');
                xAr.push('TDO="' + list.$fieldProperties[key].$X3.$dataType + '" ');
                xAr.push('TFL="' + list.$fieldProperties[key].$X3.$usages[$Caller] + '">');
                xAr.push('<![CDATA[' + list.$fieldProperties[key].$title + ']]>');
                xAr.push('</CPH>');
            }
        });
        return xAr.join('')
