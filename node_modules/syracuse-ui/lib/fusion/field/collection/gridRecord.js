...
exports.GridRecord = helpers.defineClass(GridRecord, Article, {
    ...
    getOnClickParams: function(ev){
        var lineClicked = this._isLineClicked(ev),$field;
        var p = {
            "event": "",
            "opt": {}
        };
        if (lineClicked) {
            ...
            var css = this.boxParent.builder._css.cellFusion, isCheck=false;

			var element=($(ev.target).hasClass(css)) ? $(ev.target) : $(ev.target).parents("." + css);
			var field = this.getField(element.attr('id'));
