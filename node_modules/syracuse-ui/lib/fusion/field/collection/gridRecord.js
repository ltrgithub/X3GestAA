...
exports.GridRecord = helpers.defineClass(GridRecord, Article, {
    ...
    _removeLineRow: function(){
        if (!this._isBuilded()) 
            return this.$$dataRowFrozen.unbind("click");
        this.$$dataRowScroll.unbind("click");
    ...
    _refresh: function(recordIndex, insert){
        ...
        this.$recordIndex = recordIndex
        if (this._isBuilded()) {
            top = (this.$recordIndex * listBuilder.size("lineHight")) + "px";
            this.$$noline[0].firstChild.textContent = (this.$recordIndex + 1);
            this.$$dataRowFrozen.attr('style', 'top:' + top);
            this.$$dataRowFrozen.attr("data-s-record", this.$recordIndex);
            this.$$dataRowScroll.attr('style', 'top:' + top);
    ...
    drawBox: function(position){
        ...
        self.$$dataRowFrozen.addClass(listBuilder.makeSkinFusion(list.$skin, '-line'))
        self.$$dataRowFrozen.bind("click", function(event){
            self.onClick(event);
        });
        self.$$dataRowScroll = $("<div/>").attr("data-s-record", self.$recordIndex);
        self.$$dataRowScroll.addClass(listBuilder.makeSkinFusion(list.$skin, '-line'))
        self.$$dataRowScroll.bind("click", function(event){
            self.onClick(event);
        });
        ...
        var css = list.$skin + "-cell";
        css = listBuilder.appendSkinFusion(css, null);
        /*
         if (list.$item.$preView) {
         self.$$dataRow.append($("<th/>").addClass(css).addClass(list.$skin + "-preview-cell").append(self.$$previewOpener = $("<a class='" + list.$skin + "-preview-opener s-close'/>")));
         }
         ...
         */
		var c=self._buildCel(listBuilder.$fieldnl, list.$itemnl, self._cssNoline());
        ...
        self.$$noline = $(c);
        self.$$noline.html('<div class="s-inplace-value s-inplace-value-fusion">' + (this.$recordIndex + 1) + '</div>');
	   ...
	   self.$item.$layout.$items.forEach(function($item){
		...
		if(self.current) {self._setInterfaceCurrentCell()};
    ...
    _cssNoline: function(){
        var list = this.boxParent, css = list.$skin + "-cell ";
        css += this._cssNolineFusion();
        return css;
    },
    _cssNolineFusion: function(){
        var list = this.boxParent;
        return list.builder.makeSkinFusion(list.$skin, "-cell") + "-noline";
    },
	...
	_buildCel:function($field,$item,css){
		...
		c.style.width = ($field.$cellLength) + "px";
		return c;
	},
    ...
    addHtmlRow: function(position){
        var listBuilder = this.boxParent.builder, top = (position * listBuilder.size("lineHight")) + "px";
        this.$$dataRowFrozen.attr('style', 'top:' + top)
        this.$$dataRowScroll.attr('style', 'top:' + top)
    ...
    highlightSelection: function(selected){
        ...
        if (this.$$previewRow) {
            this.$$previewRow.toggleClass("s-list-record-selected", selected);
        }
    ...
    onClick: function(ev){
        var doEvt = true;
        var self = this;
        if (self.getSiteController()) {
            var p = self.getOnClickParams(ev)
            doEvt = self.getSiteController().trigger(p.event, p.opt);
        }
        if (doEvt) {
            self.doClick((p)?p.opt:null);
        }
				//TEST
    ...
    getOnClickParams: function(ev){
        ...
        if (lineClicked) {
            ...
            var css = this.boxParent.$skin + "-cell";
    ...
    _isLineClicked: function(ev){
        return (($(ev.target).data("s-record") != undefined) || $(ev.target).hasClass(this._cssNoline()) || $(ev.target).parents("." + this._cssNolineFusion()).length !== 0);
    },
    doClick: function(p){
        console.log("doClick");
    },
    ...
    setFocus: function(xid, edit, opt){
        console.log(" setFocus xid:" + xid + " recordIndex:" + this.$recordIndex + " edit:" + edit);
		var wChangeEdit=((this.current && this.current.xid==xid)?(this.current.edit!=edit):edit)
		if(this.current && this.current.xid!=xid){this.resetFocus(this.current.xid)}
		this.current={"xid":xid,"edit":edit,"opt":opt};
		if(edit && wChangeEdit){
			var field=this.getField(xid);
			field.$item.$isEditMode=edit;
			field.$isEditMode=edit;
			field=this._drawInnerCel(field.$$item[0],field.$item,null,edit)
			field.focus()
		}
		this._setInterfaceCurrentCell()
    },
	resetFocus:function(xid){
	    console.log(" resetFocus xid:" + xid + " recordIndex:" + this.$recordIndex + " edit:" + false)
        var field = this.getField(xid);
		this.current=null
		field.$$item.toggleClass( "select", false )
		var wChangeEdit=((this.current && this.current.xid==xid)?(this.current.edit):true)
		if(wChangeEdit){
			var field=this.getField(xid);
			field.$item.$isEditMode=false;
			this._drawInnerCel(field.$$item[0],field.$item,null,false)
		}
	},
