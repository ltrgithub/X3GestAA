...
exports.GridRecord = helpers.defineClass(GridRecord, Article, {
    ...
    _buildCel: function($field, $item, css){
		...
		if($field.$isReadOnly){
			...
			this.guimodCel[$item.$bind]._status.$isReadOnly=true;
    ...
    getOnClickParams: function(ev){
        ...
        if (lineClicked) {
			...
			var element=($(ev.target).hasClass(css)) ? $(ev.target) : $(ev.target).parents("." + css);
            ...
            p.opt = {
                ...
                "data": {
                    "line": (this.$recordIndex + 1),
                    "xid": (element.attr('id')),
		"isReadOnly":(element.hasClass(this.boxParent.builder.guimod.css.readOnly)),
		"isDisabled":(element.hasClass(this.boxParent.builder.guimod.css.disabled))
                }
            };
        }
        return p;
    ...
    setFocus: function(xid, edit, opt){
        ...
        if (field && edit && !field.$isEditMode) {
			...
			setTimeout(function(){
				...
				if (field && field.$item) {
					console.log('gridRecord.setFocus xid:'+xid + " recordIndex:" + self.$recordIndex +" isEditMode:" + field.$item.$isEditMode)
		...
		if(!edit) field.$$item.toggleClass("s-grid-fusion-select", true);
    },
        ...
        if (field) {
			setTimeout(function(){					
				...
				if (field && field.$item) {
					...
					console.log('gridRecord.resetFocus xid:'+xid + " recordIndex:" + self.$recordIndex +" isEditMode:" + field.$item.$isEditMode) 
					if(field.$item.$isEditMode) {
						document.fieldController.toggleEditMode(field);
					}
				}
