...
exports.NavigationList = helpers.defineClass(NavigationList, GridList, {
	...
	applyShortCuts: function(shortcuts, event, focusField) {
		...
		} else if ((shortcuts.left || shortcuts.right) && this.treeDecorator) {
			var record, recordIndex, serverIndex, newSelect;
			record = this.getFocusedRecord();
			if (record) {
				if ((this.treeDecorator.isNodeOpened(record.$uuid) && shortcuts.left) || (!this.treeDecorator.isNodeOpened(record.$uuid) && shortcuts.right)) {
