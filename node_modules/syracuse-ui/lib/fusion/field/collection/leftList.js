...
exports.NavigationList = helpers.defineClass(NavigationList, DesktopList, {
	...
	applyShortCuts: function(shortcuts, event, focusField) {
		if (this.page.$facet != "$lookup") return false //TODO : navigation list;
		console.log("LeftList applyShortCuts " + JSON.stringify(shortcuts));
		if (shortcuts.down || shortcuts.up) {
			var record, recordIndex, serverIndex, newSelect;
			if (this.selector.records && this.selector.uuid != undefined) {
			...
			if (newSelect) {
				...
				newData[this.$item.$bind] = [{
					$uuid: newSelect.$uuid,
					$isSelected: true,
					$index: serverIndex
				}];
				this.page.applyChange(newData);
				if (focusField && focusField.$$input) {
					focusField.$$input[0].blur() // leave  quick sel input;
				}
				return true;
			}
		} else if (shortcuts.esc && event.target.syraFieldId) {
			if (focusField && focusField.$$input) {
				focusField.$$input[0].blur() //leave  quick sel input;
				return true;
			}
		} else if (shortcuts.enter && !event.target.syraFieldId) {
			if (this.selector.records && this.selector.uuid != undefined && this.selector.records[this.selector.uuid].$$item) {
				this.selector.records[this.selector.uuid].$$item.click();
