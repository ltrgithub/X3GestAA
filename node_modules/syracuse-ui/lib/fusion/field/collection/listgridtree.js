...
helpers.defineClass(LGridTree, Article, {
	...
	loadBox: function(initData) {
		...
		this._cacheNodes = {
			lvl: $("<ul class='s-fusion-ltree-lvl'>"),
			lvlTtl: $("<li  class='" + _CLOSE + "'><div class='s-fusion-ltree-lvlTtlContent'><a href='#' class='s-fusion-ltree-lvlSel'></a><a href='#' class='s-fusion-ltree-lvlDisp'></a><div class='s-fusion-ltree-lvlTtl'></div></div></li>"),
			itmTtl: $("<li  class='" + _CLOSE + "'><div class='s-fusion-ltree-itmContent'><div class='s-fusion-ltree-itmSel'></div><div class='s-fusion-ltree-itmTtl'></div></div></li>")
	...
	_appendNode: function(root, node, id) {
		...
		if (id && id.length > 0) {
			...
			if (this._cacheItems) this._nodes[id] = node;
		}
		root.appendChild(n);
		return node;
	},
	_reset: function() {
		...
		if (this._root) this._root.empty();
		this._nodes = {}
		this._resetCpt();
		this._depth = 0;
	},
	...
	_addLvl: function(root, text, id) {
		...
		if (depth == null) root.data(_DEPTH, depth = -1);
		this._cpt.levels++;
	...
	_insertNode: function(root, r) {
		...
		} else if (!root.is(".s-fusion-ltree-body")) {
			logErr("Bad root - .s-fusion-ltree-body expected");
		} else {
	...
	// elmt = tag li ou s-fusion-ltree-body - force=true forceClose - force=false forceOpen
	_lvlOpenClose: function(e, force) {
		e = util.closest(e, "li", this._root[0]);
		if (e) util.toggleClass(e, _CLOSE, force);
	},
	...
	_guiCheckLevel: function(node) {
		...
		jQuery.each(util.children(node, "li"), function(j, child) {
			...
			nbChilds++;
			if (util.hasClass(child, _SEL)) nbSel++;
			if (util.hasClass(child, _HSEL)) nbHalfSel++;
		});
		node = node.parent();
		util.removeClass(node, _SEL + " " + _HSEL);
		if (nbChilds == 0) {
			node.css('display', 'none');
		} else if (nbSel == nbChilds) {
			util.addClass(node, _SEL);
		} else if (nbSel != 0 || nbHalfSel != 0) {
			util.addClass(node, _HSEL);
		}
	},
	// root s-fusion-ltree-body ou tag li du level
	_guiProcessTree: function(root) {
		if (!root) root = this._root;
		var lvl = util.children(root, "ul", true)
		if (lvl) this._guiCheckLevel(lvl);
	},
	_doGui: function(data) {
		...
		jQuery.each(data, function(i, v) {
			...
			var id = v[0].toString(),
				gui = sapUtil.Fusion.getState(v[1]),
				itm = self._nodes[id];
			// tag li de l'item
			if (!itm) itm = self._root.find('li[' + _ID + '=' + id + "]");
			if (itm && gui) {
				util.removeClass(itm, _SEL);
				if (gui.$isSelected) util.addClass(itm, _SEL);
				var parent = itm[0].parentNode.parentNode;
				if (jQuery.inArray(parent, parents) == -1) parents.push(parent)
			}
		});
			...
			jQuery.each(parents, function(i, parent) {
				while (parent != null) {
					...
					if (parent.nodeName == "LI") {
						if (jQuery.inArray(parent, allParents) == -1) allParents.push(parent);
					} else parent = null;
				}
			});
	...
	_doInsert: function(data) {
		var op = "set";
		if (this._type.rec && op == "set") op = "updt";
		switch (op) {
			case "set":
				this._doSet(data);
	...
	_update: function(rcd, mvt, meta) {
		...
		if (!done) tr = " - Bad data - No mvt/stt found";
	...
	_init: function(layoutSlot, data) {
		var self = this;
		self.isInit = true;
		self._reset();
		self._root = $("<div class='s-fusion-ltree-body " + self._type.id + "'>").appendTo($(layoutSlot));
		self._root.on("click", ".s-fusion-ltree-lvlSel", function(event) {
			var x3ids = [],
				stt = [],
				level = $(this).closest('li');
		...
		self._root.on("click", ".s-fusion-ltree-lvlDisp", function(event) {
			if (self._type.rec) {
				var me = $(this),
					done = false,
					level = me.closest('li');
				if (level.is('.close') && !level.data('done')) {
					...
					if (self._testId == null) self._testId = 1000000;
					self.testSendReply({
						$mvt: [
							[1, 1]
						],
		...
		self._root.on("click", ".s-fusion-ltree-itmContent", function(event) {
			var itm = $(this).parent(),
				x3id = itm.attr(_ID);
			if (self._type.std) self._root.find('li.select').toggleClass(_SEL);
			self.testSendReply({
				"$mvt": [],
				"$rcd": [],
				"$meta": {
					"stt": [
						[x3id, sapUtil.Fusion.getNumStatus({
		...
		self._root.on("resize", ".s-fusion-ltree-itmContent", function(event) {
			var itm = $(this).parent(),
				x3id = itm.attr(_ID);
			if (self._type.std) self._root.find('li.select').toggleClass(_SEL);
			self.testSendReply({
		...
		if (data) self.testSendReply(data);
		return self;
	},
