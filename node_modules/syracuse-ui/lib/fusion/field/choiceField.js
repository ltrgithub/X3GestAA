...
function _getMenloc(choiceField){
    var $fusionController = syraUtil.getFusionController(choiceField), fusionSess = $fusionController ? $fusionController.getSession() : null;
...
function _updateEnum(choiceField, forceRender, rmItems){
    var menloc = _getMenloc(choiceField), rmItemsEx = null, useIn = false, i, len;
    if(menloc) {
        if (rmItems != undefined && rmItems != null && rmItems.length > 0) {
            if(_.isString(rmItems)) {
                rmItemsEx = rmItems.split(",");
                for (i = 0, len = rmItemsEx.length; i < len; i++) {
                    rmItemsEx[i] = parseInt(rmItemsEx[i], 10) ;
                }       
            }
            else {
                useIn = true;
        ...
        choiceField.applyMetaData({$enum: _getEnum(choiceField.$enum, menloc, useIn ? rmItems : rmItemsEx)});
        if(forceRender) {
...
exports.FusionChoiceField = helpers.defineClassEx(FusionChoiceField, desktopField.ChoiceField, {
    initialize: function(){
        ...
        if (!this.$field.$value) {
            this.$field.$value = {"$type": syraUtil.dataTypes.integerType, "$enum": []};
        }
        if(menloc) {
    ...
    removeListItems: function(items){
        _updateEnum(this, true, items);
    },
	forceRender: function(){		
		var build = this.builder, dummyVal = " ", i, len;
		if (build && build._$selectedEnum){
			var newIdx = -1;
            for (i = 0, len = this.$enum.length; i < len; i++) {
                if (this.$enum[i].$value == build._$selectedEnum.$value){
                    newIdx = i;
                    break;
                }
            }
			if (newIdx >= 0){
                ...
                this.$enum.unshift({"$value": 0, "$title": dummyVal})
                this.applyMetaData({"$enum": this.$enum});
				build.setDataValue(dummyVal);
			}
		}
    }
});
