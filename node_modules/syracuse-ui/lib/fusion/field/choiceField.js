...
function _getMenloc(choiceField) {
	var $fusionController = syraUtil.getFusionController(choiceField),
...
exports.FusionChoiceField = helpers.defineClassEx(FusionChoiceField, desktopField.ChoiceField, {
	...
	forceEnum: function($enum) {
		var currVal = this.getDataValue();
		this.applyMetaData({"$value": {"$enum": $enum}});
		if(currVal != null) {
			this.forceRender();
		}
		else if($enum.length > 0){
			this.setDataValue($enum[0].$value);
		}
	},
	forceRender: function() {
		var dummyVal = " ",	i, len, newVal = null, currVal = this.getDataValue();
		if (currVal != null) {
			...
			this.setDataValue(newVal || dummyVal);
		}
	}
});