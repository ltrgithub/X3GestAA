...
function _updateEnum(choiceField, forceRender, rmItems){
    var menloc = _getMenloc(choiceField), rmItemsEx = [], rmItemsTemp, i, len, exclude = false;
    if(menloc) {
        if (rmItems != undefined && rmItems != null && rmItems.length > 0) {
            ...
            for (i = 0, len = rmItemsTemp.length; i < len; i++) {
                if(rmItemsTemp[i].length == 0) {
                    // Raz
                    choiceField.applyMetaData({"$value": {$enum: _getEnum(choiceField.$enum, menloc)}});
                }
                else {
                    exclude = true;
        ...
        choiceField.applyMetaData({"$value": {$enum: _getEnum(choiceField.$enum, menloc, exclude ? rmItemsEx : null)}});
        if(forceRender) {
	...
	forceRender: function(){		
		...
		if (build && build._$selectedEnum){
			...
			if (newIdx >= 0){
                ...
                this.applyMetaData({"$value": {"$enum": this.$enum}});
