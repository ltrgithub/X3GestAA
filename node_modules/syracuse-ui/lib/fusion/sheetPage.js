...
var _fieldTypes = {
	...
	"application/x-string$text": require('syracuse-ui/lib/field/field').Field,
	...
	loadBox: function(initData) {
		...
		if (!this.$prototype.$fusionBar) {
			...
			if (!this.$item.$isSimplified) {
				this.$item.$layout = syra_layoutUpdater.cleanConvergenceSetting(this.$item.$layout);
		...
		if (fieldClass) {
			...
			switch ($class) {
				...
				case "application/x-datetime":
					...
					/*field.formatApi = formatApi.getApi($class);
                 field.localFormat = formatApi.getLocalFormat($class);
                 field.localFormatDisplay = formatApi.getLocalFormat($class, $field.$format);
                 field.formatObj = formatApi.processFormat(formatApi.getLocalFormat($class));
				...
				case "application/x-real":
					...
					field.formatApi = formatApi.getApi($class);
					field.numFormatObj = formatApi.getLocale().getNumberFormatObj($class, numSettings.decimalSep || ".", numSettings.thousandSep || "", 3);
					field._checkSep = false;
					field.localFormat = field.numFormatObj.numFormat;
					field._formatInput = field.formatApi.formatInput;
					field.formatApi.regionalOptions = field.numFormatObj;
					break;
				case "application/x-string$text":
