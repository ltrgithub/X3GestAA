        ...
        onValidate: function(dialogPage){
            if (dialogPage.validateFields()) {
                Object.keys(dialogPage.dataset).forEach(function($prop){
                    if ($prop.slice(0, 1) !== "$") {
                        if (dialogPage.$prototype && dialogPage.$prototype.$properties && dialogPage.$prototype.$properties[$prop] &&
                        dialogPage.boundFields &&
                        dialogPage.boundFields[$prop] &&
                        dialogPage.boundFields[$prop][0] &&
                        dialogPage.$prototype.$properties[$prop].$type === "application/x-reference" &&
                        dialogPage.boundFields[$prop][0].currentValue &&
                        dialogPage.boundFields[$prop][0].currentValue.$uuid) {
...
exports.formatCallBackMenuUrl = function(article, $menu, record, callback){
    ...
    exports.parseParameters(article, $menu.$parameters, record, function(values, isCanceled){
        if (isCanceled) {
            ...
            if ($menu.$type && $menu.$type.indexOf("{") == 0) {
                $menu.$type = article.parseExpression($menu.$type, record, values);
