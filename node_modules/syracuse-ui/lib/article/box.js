...
exports.Box = helpers.defineClass(Box, null, {
	...
	onOpenerClick: function(picker, event, data) {
		var self = this;
		document.site.onBeforeClick(self, event);
				...
				if (isTabSelected) {
					...
					if (!document.site.ddManager.ddAgent) {
						...
						self.page.externalAdapter.onBoxToggle({
							...
							doEvent: function() {
								if (!isTabSelected) {
									self.layoutParent.openTab(document.site.boxFocus = self);
								}
							}
						});
						return false;
		...
		if (self.$item.$isPopupContent) {
			if (!self._popupMenus) {
				self._popupMenus = document.site.dialogManager.openPopup(self, {
					...
					onClose: function() {
						if (self.domTitle) {
							document.site.dom.toggleClass(self.domTitle, "s-close", true);
						}
						self.$opened = false;
	...
	expandBody: function($opened, event) {
		...
		if (self.expandPicker) {
			...
			document.site.dom.toggleClass(self.expandPicker, "s-close", !self.$opened);
		}
	...
	openBox: function(open, load, drawing) {
		...
		if (this.tabTitle) {
			...
			document.site.dom.toggleClass(this.tabTitle, "s-tab-open", open);
		}
		if (this.$item.$isBoxCollapsable) {
			if (this.expandPicker) {
				document.site.dom.toggleClass(this.expandPicker, "s-close", !open);
				this.expandPicker.syraOnClick = "onOpenerClick";
			}
			this.domTitle.syraOnClick = "onOpenerClick";
			document.site.dom.toggleClass(this.domTitle, "s-close", !open);
		}
		this.showBody(open);
	...
	ensureLayoutMode: function() {
		...
		if (this.domItem) {
			...
			if (this.isTabLayout) {
				if (!this.tabTitle || this.isTabTitleRemoved) {
					...
					if (this.pickers) {
						if (this.pickers.slot) {
							document.site.dom.removeChild(this.pickers.slot);
						}
						this.pickers = null;
				...
				document.site.dom.toggleClass(this.tabTitle, "s-tab-open", this.$opened);
				...
				if (this.tabTitle) {
					document.site.dom.removeChild(this.tabTitle);
