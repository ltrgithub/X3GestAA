...
function _onMaximizeBoxClick(box) {
	syra_site.siteFunctions.maximizeItem(box, box.btns.items.maximize);
	if (box.isMaximized) {
		if (box.tabTitle) {
			var slot = box.domItem.parentNode;
...
function _showExpandButton(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		if (!box.expandBtn) {
			...
			box.expandBtn = syra_menus.addIconButton(syra_local.box_collapse, css, "onOpenerClick", null, "node_expanded");
...
function _setExpandButtonState(box) {
	var state = box.$opened ? "node_collapsed" : "node_expanded"; //    "collapse" : "expand";
...
function _onCloseClick(box) {
	box.isMaximized && box.onMaximizeBoxClick();
