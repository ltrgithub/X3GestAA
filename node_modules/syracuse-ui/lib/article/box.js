...
exports.Box = helpers.defineClass(Box, null, {
	...
	showAdvancedFields: function(show) {
		...
		if (this.onShowAdvancedFields) {
			...
			if (this._pickers && this._pickers.items) {
				var advancedPicker = this._pickers.items.advanced;
				if (advancedPicker) {
	...
	toggleAdvancedFieldsMode: function(enable, onAuthoring) {
		...
		boxHelper.toggleAdvancedFieldPicker(this, this.$isAdvancedModeEnabled = enable);
		if (this.boxParent) {
			this.boxParent.toggleAdvancedFieldsMode(enable, onAuthoring);
		}
	},
	getArticle: function() {
	...
	drawBox: function() {
		...
		if (this.$item.$expression) {
			...
			if (this.page.isDashBoard) {
				this.openBox(!this.tabTitle && this.$item.$opened !== false, this.$item.$isContentLoaded);
			} else {
				this.openBox(!this.tabTitle && this.$item.$opened !== false, true);
			}
		}
	},
	...
	doPicker: function(pickerId) {
		boxHelper.doPicker(this, pickerId);
	},
			...
			if (!self._popupMenus) {
				self._popupMenus = self.openDialog({
					...
					position: {
						my: "right top",
						at: "right bottom",
						of: $$ofPosition || $(self.titleMenu ? self._header : self.domTitle)
					},
					onClose: function() {
						if (self.domTitle) {
	...
	_expandBody: function($opened, event) {
		boxHelper.expandBody(this, $opened, event);
	},
	renderLayoutContent: function() {
		this.isLayoutContentRendering = true;
		this.layoutContent = this.page.loadNewItem(this.$$body[0], this.$item.$layout, this);
		this.isLayoutContentRendering = false;
	},
	...
	ensureLayoutMode: function() {
		if (this._item) {
			...
			if (this.isTabLayout) {
				...
				if (this.$item.$isPopupContent) {
					this._header.className += " s-popup";
					this._body.className += " s-popup";
					if (this.domTitle) {
						this.domTitle.className += " s-popup";
					}
					if (this._expandPicker) {
						this._expandPicker.className += " s-popup";
					}
				}
				if (this.$item.$titleIcon) {
					if (this.$item.$titleIcon.$mode === "icon") {
						this.domTitle.className += " s-icon";
						this._header.className += " s-icon";
					}
				}
				if ($style) {
					this._item.className += $style;
	...
	setState: function(state) {
		if (state.$isHidden !== undefined) {
			...
			if (this.$isHidden !== state.$isHidden) {
				...
				if (updated && document.site.authorPage) {
					if (this.page && this.page.isOnAuthoring) {
						document.site.authorPage.showHiddenItem(this, this.$isHidden);
					}
				}
			}
		}
	...
	closeDialogs: function(dispose) {
		if (this._dialogs) {
			for (var ii = 0, jj = this._dialogs.length; ii < jj; ii++) {
				if (!this._dialogs[ii].$hasCallback) {
	...
	dispose: function() {
		...
		this._expandPicker = this._pickers = this.$item = this.layoutSlot = this.layoutContent = null;
		...
		this.rowSeparator = this.severitiesCount = this.diagnosesFields = null;
		this.page = this.childrenSection = null;
	}
});