...
exports.Box = helpers.defineClass(Box, null, {
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		boxHelper.applyButtonMetadata(this, metaData, designing);
		if (designing !== false) {
	...
	showButton: function(id, show, syraOnClick, fontIcon) {
		boxHelper.showButton(this, id, show, syraOnClick, fontIcon);
	},
	ensureButtonsVisibility: function() {
		if (this.btns && this.btns.items) {
			var display = this.$opened ? "" : "none";
			if (this.btns.items.maximizebox) {
				this.btns.items.maximizebox.style.display = display;
			}
	...
	expandBody: function($opened, event) {
		...
		if (self.expandBtn) {
			syra_menus.setButtonTitle(self.expandBtn, self.$opened ? syra_local.box_collapse : syra_local.box_expand);
			syra_site.dom.toggleClass(self.expandBtn, "s-close", !self.$opened);
		}
	...
	openBox: function(open, load, drawing) {
		...
		if (this.$item.$isBoxCollapsable) {
			if (this.expandBtn) {
				syra_site.dom.toggleClass(this.expandBtn, "s-close", !open);
	...
	showBody: function(show) {
		...
		if (this.hasFlashChildField) {
			this.body.parentNode && syra_site.dom.hideFlashContainer(this.body, !this.isBodyShow);
		} else {
			syra_site.dom.display(this.body, this.isBodyShow);
		}
		//  }
		this.isBodyShow = show;
	},
			...
			if (this.isTabLayout) {
				if (!this.tabTitle || this.isTabTitleRemoved) {
					...
					if (this.btns) {
						...
						this.btns = null;
						boxHelper.applyButtonMetadata(this, this.$item);
					}
				...
				if (this.$item.$isPopupContent) {
					...
					domTitleCss += " s-popup";
					if (this.expandBtn) {
						this.expandBtn.className += " s-popup";
					}
				}
				if (this.$item.$titleIcon) {
					if (this.$item.$titleIcon.$mode === "icon") {
	...
	dispose: function() {
		...
		this.expandBtn = this.btns = this.$item = this.layoutSlot = this.layoutContent = null;
		this.domItem = this.body = this.header = null;
