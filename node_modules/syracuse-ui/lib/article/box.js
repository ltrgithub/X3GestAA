...
exports.Box = helpers.defineClass(Box, null, {
	...
	applyPickerMetadata: function(metaData, designing) {
		...
		if (metaData.$isMaximizable !== undefined) {
			this.setMaximizePicker(this.$item.$isMaximizable = metaData.$isMaximizable);
		}
	...
	setRefreshPicker: function(show) {
		this.togglePicker("refresh", show);
	},
	setMaximizePicker: function(show) {
		this.togglePicker("maximize", show);
	},
	...
	getTitle: function(applyDefault) {
		var title = (this.textTitle && this.textTitle.textContent) || this.$item.$title || "";
		...
		if (this.domTitle) {
			if (title != null && title != "") {
				...
				if (displayTitle.indexOf("{") < 0) {
					...
					var parsedText = this.renderExpression(displayTitle, this.textTitle, this.$skin + "-title-field", false);
	...
	_setTitleIcon: function() {
		...
		this.textTitle.className = this.$skin + "-title-text s-icon";
		this.domTitle.appendChild(this.textTitle);
	},
	...
	doOpenPicker: function($$ofPosition) {
		...
		if (self.$item.$isPopupContent) {
			if (!self._popupMenus) {
				self._popupMenus = document.site.dialogManager.openBoxDialog(self, {
					$dialogMode: "popup",
					content: self,
					$$dialog: $(self.body),
					position: {
	...
	dispose: function() {
		document.site.dialogManager.closeBoxDialogs(this);
