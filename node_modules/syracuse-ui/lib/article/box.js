...
exports.Box = helpers.defineClass(Box, null, {
	...
	_createBoxTitle: function() {
		...
		this.$item.$titleIcon && boxHelper.setTitleIcon(this);
	...
	expandBody: function($opened, event) {
		...
		self.expandBtn && boxHelper.setExpandButtonState(self);
	...
	openBox: function(open, load, drawing) {
		...
		if (this.$item.$isBoxCollapsable) {
			if (this.expandBtn) {
				this.expandBtn && boxHelper.setExpandButtonState(this);
	...
	showBody: function(show) {
		...
		if (this.hasFlashChildField) {
			this.body.parentNode && syra_dom.hideFlashContainer(this.body, !this.isBodyShow);
		} else {
	...
	dispose: function() {
		...
		this.tabTitleHead = this.tabTitleText = this.tabTitle = this.domTitle = this.domDescription = this.titleLabel = null;
