...
function _showExpandButton(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		if (!box.expandBtn) {
			...
			box.expandBtn = syra_menus.button.add({
				parent: box,
			...
			box.header.insertBefore(box.expandBtn.link, box.header.firstChild);
		}
		box.$opened = box.$item.$opened !== false;
		_setExpandButtonState(box);
	} else {
		syra_menus.button.remove(box.expandBtn);
	}
	if (designing) {
...
function _setExpandButtonState(box) {
	...
	syra_menus.updateButtonIcon(box.expandBtn.link, syra_local["box_" + state], state);
}
...
function _applyButtonMetadata(box, metaData, designing) {
	...
	if (metaData.$isCloseable !== undefined) {
		box.toggleCloseButton(box.$item.$isCloseable = metaData.$isCloseable);
	}
	if (metaData.$isMaximizable !== undefined) {
		box.toggleMaximizeButton(box.$item.$isMaximizable = metaData.$isMaximizable);
	}
...
exports.Box = helpers.defineClass(Box, null, {
	ensureButtonsSlot: function() {
		if (!this.buttonsSlot) {
			...
			(this.tabTitleHead ? this.tabTitleHead : this.header).appendChild(this.buttonsSlot);
		}
	...
	ensureButtonsVisibility: function() {
		this.maximizeBtn && syra_menus.button.display(this.maximizeBtn, this.$opened);
	},
	getArticle: function() {
	...
	expandBody: function($opened, event) {
		...
		self.page.externalAdapter.onBoxToggle({
			...
			doEvent: function() {
				if (self.onOpenButtonClick) {
					self.onOpenButtonClick();
				} else {
					self.openBox(self.$opened);
					syra_site.ensureArticleVisibility(self.articleParent, true, self);
				}
			}
		});
