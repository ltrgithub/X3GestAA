...
exports.Layout = helpers.defineClass(Layout, null, {
    ...
    appendNewItem: function($item, newItem){
        newItem = this.boxParent.page.createNewItem($item, this.boxParent, newItem);
    ...
    toggleAuthoring: function(isOn){
        if (!this.boxParent.page.isAuthoringDisabled) {
    ...
    wrapInSlot: function(item, itemIndex){
        ...
        if (this.$layout.$layoutType == "row") {
            if (this._slots.length <= itemIndex) {
                var slot = document.createElement("div");
                slot.className = this.boxParent.$skin + "-layout-cell s-layout-cell";
                if (this.colWidths) {
    ...
    renderChildItem: function(item, itemIndex){
        ...
        if (item.$layout) {
            item.render();
            if (item.boxParent.page.authoringPage) {
    ...
    _ensureRoot: function(){
        ...
        if (!this._root) {
            ...
            this._root.className = this.boxParent.$skin + "-layout s-layout-" + this.$layout.$layoutType;
    ...
    render: function(){
        ...
        for (var ii = 0, jj = this.$layout.$items.length; ii < jj; ii++) {
            ...
            if ($item.$layoutType) {
                item = this.boxParent.page.createNewLayout($item, this.boxParent);
            }
            else {
                item = this.boxParent.page.createNewItem($item, this.boxParent);
            }
            if (item) {
        ...
        if (this.boxParent.page.authoringPage) {
    ...
    dispose: function(){
        this._tabs = this.items = this.layoutParent = this.boxParent = this.$$container = this.$layout;
    }
});
