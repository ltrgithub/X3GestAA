...
exports.Layout = helpers.defineClass(Layout, null, {
    ...
    appendChildItem: function(item, index, load){
        ...
        if (!this.boxParent.hasField) {
            if (item && item.$item.$bind) {
    ...
    ensureItemVisibility: function(item, $isHidden){
        var display = (!item.isAuthingEnabled && (!this.isRow && ($isHidden || item.$isHidden || item.isInvisible || item.isBoxHidden))) ? "none" : "";
        if (item.layoutSlot) {
