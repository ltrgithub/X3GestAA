...
exports.Layout = helpers.defineClass(Layout, null, {
	...
	extractItem: function(item, addSpaceBox) {
		...
		if (item.isTabLayout && item.tabTitle) {
			document.site.dom.removeChild(item.tabTitle);
	...
	removeItem: function(item, removeSlot, addSpaceBox) {
		...
		if (removeSlot && item.layoutSlot) {
			document.site.dom.removeChild(item.layoutSlot);
		}
		if (item.rowSeparator) {
			document.site.layoutValidator.removeRowSeparator(item);
		}
	...
	ensureLayoutMode: function() {
		...
		if (!this.domItem) {
			...
			this.domItem.syraDesignedLayoutId = this.id;
	...
	ensureItemVisibility: function(item, $isHidden) {
		...
		if (item.isDesigning) {
			...
			if (item.layoutSlot) {
				site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", item.$isHidden);
				site.toggleClass(item.layoutSlot, "s-aw-advanced-visibility-forced", item.$item.$isAdvanced);
			}
			if (item.domItem) {
				site.toggleClass(item.domItem, "s-aw-visibility-forced", item.$isHidden);
				site.toggleClass(item.domItem, "s-aw-advanced-visibility-forced", item.$item.$isAdvanced);
			}
			...
			if (item.isDesigningVisibilityForced) {
				var site = document.site;
				if (item.layoutSlot) {
					site.toggleClass(item.layoutSlot, "s-aw-visibility-forced", false);
					site.toggleClass(item.layoutSlot, "s-aw-advanced-visibility-forced", false);
				}
				if (item.domItem) {
					site.toggleClass(item.domItem, "s-aw-visibility-forced", false);
					site.toggleClass(item.domItem, "s-aw-advanced-visibility-forced", false);
				}
			}
		}
	},
	...
	dispose: function() {
		this.rowSeparators = this.$$item = this.domItem = this.rowSeparator = this._tabs = this.items = this.layoutSlot = this.$item = this.$layout = null;
