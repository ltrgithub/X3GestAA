...
exports.Layout = helpers.defineClass(Layout, null, {
	...
	getItems: function(children, $authoringLevel) {
		if (!children) {
			children = [];
		}
		for (var ii = 0, jj = this.items.length; ii < jj; ii++) {
			...
			if (item != this.boxParent) {
				if (item.isLayout) {
					children = item.getItems(children, $authoringLevel);
				} else {
	...
	dispose: function() {
		...
		this.rowSeparators = this.$$item = this._item = this.rowSeparator = this._tabs = this.items = this.layoutParent = this.articleParent = this.boxParent = this.page = this.layoutSlot = this.$item = this.$layout = this.tabOpened = null;
	}
});