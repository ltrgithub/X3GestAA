...
exports.Layout = helpers.defineClass(Layout, null, {
                ...
                if (child.$layout) {
                    if ($layoutType != "tabs") {
                        ...
                        if ($layoutType == "columns") {
                            document.site.toggleClass(this._domTable, "s-aw-layout-table-on", isOn);
                        }
    ...
    toggleSeparator: function(show, propagate){
        if (propagate) {
            for (var ii = 0, jj = this.items.length; ii < jj; ii++) {
                ...
                if (item.toggleSeparator) {
                    item.toggleSeparator(show, propagate);
                }
            }
        }
    },
        ...
        switch (this.$layout.$layoutType) {
            ...
            case "row":
                if (!this.sideRow) {
                    ...
                    this.sideRow.className = "s-layout-table-row";
                    this.$$container[0].appendChild(this.sideRow);
                    if (this.$layout.$fitToContent) {
                        this.sideRow.style.width = "auto";
                    }
                }
    ...
    render: function(refresh){
        var isLargest = this.box.$item.$fieldsWidth == "largest";
        if (isLargest) {
    ...
    dispose: function(){
        ...
        this.items = this.layoutParent = this.box = this.$$container = this.$layout;
    }
});
