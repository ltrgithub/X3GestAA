...
exports.Layout = helpers.defineClass(Layout, null, {
	...
	extractItem: function(item) {
		...
		if (item.boxParent && item.boxParent.boxFields) {
			delete item.boxParent.boxFields[item.id];
		}
		if (item.isSection) {
	...
	ensureLayoutMode: function() {
		...
		if (!this._item) {
			...
			this._item.syraAwLayoutId = this.id;
	...
	getItems: function($authoringLevel, children) {
		if (!children) {
			children = [];
		}
		for (var ii = 0, jj = this.items.length; ii < jj; ii++) {
			var item = this.items[ii];
			if (item != this.boxParent) {
				if (item.isLayout) {
					children = item.getItems($authoringLevel, children);
				} else {
	...
	convertToNewLayout: function($newLayout) {
		...
		/*if (this.isRow) {
         for (var ii = 0, jj = this.items.length; ii < jj; ii++) {
         var col = this.items[ii];
         if (col.isLayout) {
         children = children.concat(col.getChildren());
         }
         else {
         children.push(col);
         }
		...
		//}
		this.extractItems(children);
		if (this._tabs) {
			document.site.removeDomChild(this._tabs);
			delete this._tabs;
		}
		this.removeItems(); //Clear space old layout        
		children = this.clearChildren(children);
		...
		if (this.isRow) {
			...
			if (children.length <= widths.length) {
				...
				this.loadChildItem(null, {
					$layoutType: "stack",
					$items: []
				}).newLoadChildItems(this.clearRootStackChildren(children)); //fill first row
			}
		} else {
			this.newLoadChildItems(this.clearRootStackChildren(children));
		}
	},
	clearRootStackChildren: function(children) {
		var newChildren = [];
		if (children) {
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				var child = children[ii];
				if (child && child.isLayout && child.$item.$layoutType == "stack") {
					newChildren = newChildren.concat(child.extractAndDispose());
				} else {
	...
	clearChildren: function(children) {
		var newChildren = [];
		if (children) {
			for (var ii = 0, jj = children.length; ii < jj; ii++) {
				var child = children[ii];
	...
	setChildAuhtoringLevel: function() {
		if (this.boxParent.$authoringLevel == "block") {
			this.$chilAuthoringLevel = "field";
		} else {
			var children = this.getItems();
			if (children.length > 0) {
				this.$chilAuthoringLevel = children[0].$authoringLevel;
			} else {
				this.$chilAuthoringLevel = this.boxParent.$authoringLevel == "section" ? "block" : "section";
			}
		}
	},
	dispose: function(extractChildren) {
		var children;
		if (this.id && this.articleParent && this.articleParent.layouts) {
			delete this.articleParent.layouts[this.id];
		}
		this.rowSeparators = this.$$item = this._item = this.rowSeparator = this._tabs = this.items = this.layoutSlot = this.$item = this.$layout = this.tabOpened = null;
