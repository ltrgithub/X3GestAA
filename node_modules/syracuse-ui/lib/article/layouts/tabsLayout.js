...
exports.TabsLayout = helpers.defineClass(TabsLayout, Layout, {
    ...
    _ensureTabs: function(){
        if (!this._tabs) {
			this.$$container.prepend(this._tabs = document.createElement("nav"));
        }
    },
    _initializeTabs: function(box){
        this._isTabsInitialized = true;
        this._ensureTabs();
        this.$tabSkin = box.$skin + "-tab";
    ...
    appendChildTab: function(box){
        ...
        box.domTitle.setAttribute("data-s-box", box.id);
        box.$$header = $(box._header = box.domTitle);
        if (!this._isTabsInitialized) {
            this._initializeTabs(box);
        }
		this._tabs.appendChild(box._header);
    ...
    _setOpenSkin: function(item, open){
        item._header.className = item._header.className.replace(item.$skinTab + (open ? "" : "-open"), item.$skinTab + (open ? "-open" : ""));
    },
