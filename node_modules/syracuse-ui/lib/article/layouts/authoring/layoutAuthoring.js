...
exports.LayoutAuthoring = helpers.defineClass(LayoutAuthoring, null, {
    onClickItem: function(item, target){
        if (!document.controller.findArticle($(target)).$item.$isAuthoringDisabled) {
    ...
    convertLayout: function(newLayout, oldLayout){
        var self = this;
        if (newLayout.$layout.$layoutType == "pattern") {
            var $newLayout = newLayout.box.$item.$layout = helpers.object.clone(layoutSettings.getComposite(newLayout.$layout.$layoutSubType || "pattern-3-1"), true);
