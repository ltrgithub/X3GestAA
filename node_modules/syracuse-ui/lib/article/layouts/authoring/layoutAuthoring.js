...
function _fillLayout(items, layout, isRoot){
    var lastLayout;
    layout.items.forEach(function(newLayout){
        if (newLayout.$layout && items.length > 0) {
            if (newLayout.items.length == 0) {
                var item = items.shift();
                _moveItem(newLayout, item.layoutParent, item);
                ...
                lastLayout = _fillLayout(items, newLayout);
            }
        }
    });
    if (isRoot && items.length > 0) {
        items.forEach(function(item){
            _moveItem(lastLayout, item.layoutParent, item);
        });
    }
    return lastLayout;
}
...
exports.convertLayout = function(newLayout, oldLayout){
    if (newLayout.$layout.$layoutType == "composite") {
        ...
        var rootLayout = newLayout.box.layoutContent = document.site.createLayout(newLayout.box, $newLayout);
        rootLayout.$$container = newLayout.$$container || oldLayout.$$container;
        rootLayout.render();
        rootLayout.toggleAuthoring(true);
        var items = [];
        _extractItem(items, oldLayout);
        _fillLayout(items, rootLayout, true);
    ...
    if (oldLayout.items) {
        oldLayout.items.forEach(function(converted){
            if (converted.$layout) {
                ...
                _moveItem(newLayout, oldLayout, converted);
            }
        });
    }
...
function _moveItem(newLayout, oldLayout, converted){
...
exports.onItemMovedOut = function(layout, item, onAppend){
    switch (layout.$layout.$layoutType) {
        case "tabs":
            ...
            delete item.domTitle;
            delete item.$isTabLayout;
            item.$$header.remove();
            delete item._header;
            delete item.$$header;
            item.drawBox();
            item.authoringNode.setDesignableItem(item, true);
            break;
        case "stack":
            if (item.$layout) {
...
exports.LayoutAuthoring = helpers.defineClass(LayoutAuthoring, null, {
    begin: function(layout, $$container, selector){
        ...
        if (layout.$layout.$layoutType == "tabs") {
            self.$$container = $(layout._tabs);
            self.selector = ".s-aw-item" || layout.$tabSkin;
        ...
        self.$$container.delegate(".s-aw-empty", "mouseup.author mousemove.author mouseleave.author mouseenter.author", function(event){
            if (document.site.DDAuthoring) {
            ...
            if (child.$layout) {
                if ($layoutType != "tabs") {
                    ...
                    if ($layoutType == "columns") {
                        document.site.toggleClass(self.layout._domTable, "s-aw-layout-table-on", isOn);
                    }
                ...
                if (columns.$layout.$layoutType == "columns") {
                    setTimeout(function(){
                        if (columns.items.length == 2) {
                            ...
                            var newLayout = document.site.createLayout(columns.box, {
                                $layoutType: "stack",
                                $items: []
                            });
                            var childIndex;
