...
exports.extractItems = function(layout, children, addSpaceBox) {
	...
	for (var ii = 0, jj = children.length; ii < jj; ii++) {
		...
		if (child.layoutParent && !child.isSpaceBox) {
			child.layoutParent.extractItem(child, addSpaceBox);
		}
	}
	return children;
};
...
function _addNewItem(designedArticle, targetItem, $newAuthoringLevel, $bind, dropItem, $insertAt) {
	...
	if (dropItem && !dropItem.layoutParent && dropItem.articleParent) {
		syra_article.removeItem(dropItem, true);
	}
	return newChild;
}
...
function _loadAddNewItem(article, newAddItem) {
	...
	var newItem = syra_widgets.addItem(article.page, $item, article.page);
...
exports.ensureHasParent = function(layout) {
	...
	syra_dom.removeChild(layout.domItem);
	layout.dispose();
...
exports.clearContent = function(layout) {
	if (layout._tabs) {
		syra_dom.removeChild(layout._tabs);
		delete layout._tabs;
	}
...
function _cleanBeforeSave($item) {
	if ($item) {
		delete $item.$tabIndex;
		if ($item.$layout) {
			_cleanBeforeSave($item.$layout);
		}
		if ($item.$items) {
			for (var ii = 0, jj = $item.$items.length; ii < jj; ii++) {
				_cleanBeforeSave($item.$items[ii]);
			}
		}
	}
	return $item;
...
exports.cleanBeforeSave = function($item) {
	return _cleanBeforeSave(_helpers.object.clone($item, true));
};