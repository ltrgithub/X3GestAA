...
Layout.prototype.appendChildItem = function(item, index, load) {
	...
	if (item.isSection) {
		syra_item.registerSectionBlock(item);
	}
...
Layout.prototype.createChildItem = function($item, item, index) {
	item = syra_widgets.addItem(this.page, $item, this.boxParent, item);
	if (item) {
		this.appendChildItem(item, index, true);
	}
	return item;
};
Layout.prototype.extractItem = function(item, addSpaceBox) {
	if (this.items && this.$layout && this.$layout.$items) {
		...
		if (item.isSection) {
			syra_item.unregisterSectionBlock(item);
		}
	} else {
		item.layoutParent = null;
		if (this.isRow && item.layoutSlot) {
...
Layout.prototype.removeSpaceBox = function(all) {
	var spaces = [];
...
Layout.prototype.ensureTabsSlot = function() {
	if (!this._tabs) {
		this._tabs = syra_dom.addDiv("s-layout-tabs-nav");
...
Layout.prototype.load = function() {
	...
	syra_log.widgets.layout.load(this);
