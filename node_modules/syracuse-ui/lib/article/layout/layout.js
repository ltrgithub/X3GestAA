...
exports.Layout = helpers.defineClass(Layout, null, {
	...
	ensureTabsSlot: function() {
		if (!this._tabs) {
			...
			this._tabs.className = "s-layout-tabs-nav";
	...
	ensureItemVisibility: function(item, $isHidden) {
		...
		if (item.layoutSlot) {
			syra_site.dom.hide(item.layoutSlot, isHidden);
		}
		if (item.syraSeparator && item.layoutSlot) {
			item.layoutSlot.syraSeparatorType = item.syraSeparatorType;
			syra_site.dom.toggleClass(item.layoutSlot, item.syraSeparator, !isHidden);
		}
		if (item.isDesigning) {
			var site = syra_site;
