...
exports.toggleSeparator = function(item, sepType, skinSep) {
	if (sepType) {
		if (item.syraSeparator && item.syraSeparator != skinSep) {
			syra_site.dom.toggleClass(item.layoutSlot, item.syraSeparator, false);
		}
		item.syraSeparator = skinSep;
		item.layoutSlot.syraSeparatorType = item.syraSeparatorType = sepType;
		syra_site.dom.toggleClass(item.layoutSlot, item.syraSeparator, true);
	} else {
...
exports.validateLayout = function(layout, parseTree) {
	if (layout) {
		...
		if (parseTree) {
			for (var ii = 0, jj = layout.items.length; ii < jj; ii++) {
				var item = layout.items[ii];
				exports.validateLayout(item.isLayout ? item : item.layoutContent, parseTree);
			}
		}
		layout.isDesigned = false;
	}
