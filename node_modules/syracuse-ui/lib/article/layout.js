...
exports.Layout = helpers.defineClass(Layout, null, {
	openTab: function(box) {
		this.$layout.$tabIndex = this.items.indexOf(box);
		syra_site.layoutValidator.validateLayout(box.layoutParent, true);
		syra_site.ensureArticleVisibility(box.articleParent, true, box);
	},
	...
	appendChildItem: function(item, index, load) {
		...
		if (item.isSection) {
			this.page.registerSectionBlock(item);
		}
		...
		if (item.isTabLayout) {
			if (sibling && sibling.isTabSection && item.isTabSection) {
				...
				if (append) {
					this.ensureTabsSlot().insertBefore(item.header, this.tabStop || null);
				}
			}
		}
		return item;
	...
	extractItem: function(item, addSpaceBox) {
		if (this.items && this.$layout && this.$layout.$items) {
			...
			if (item.isTabLayout && item.isTabSection) {
				syra_site.dom.removeChild(item.header);
	...
	ensureLayoutMode: function() {
		...
		this.isRow = this.$layout.$layoutType == "row";
		this.isTabLayout = this.$layout.$layoutType == "tabs";
		this.isTabLayout && this.ensureTabsSlot();
	},
	...
	ensureItemVisibility: function(item, $isHidden) {
		...
		if (item.syraSeparator && item.layoutSlot) {
			...
			syra_site.dom.toggleClass(item.layoutSlot, item.syraSeparator, display == "");
		}
		if (item.isDesigning) {
			var site = syra_site;
	...
	dispose: function() {
		...
		this._tabs = this.items = this.layoutSlot = this.$item = this.$layout = null;
