...
var helpers = require('syracuse-core/lib/helpers'), uiHelper = require('syracuse-ui/lib/field/process/helpers');
require('syracuse-ui/lib/field/process/jquery.contextMenu');
require('syracuse-ui/deps/jqModal/jqModal');


var imagesRoot = "/syracuse-ui/themes/desktop/sage/process/images", libInfoRoot = "/syracuse-ui/lib/field/process";
...
exports.ProcessPlayer = helpers.defineClass(ProcessPlayer, null, {
    init: function($$div, /* options,*/ fld){
        ...
        /*  $("body").contextMenu({
         ...
         //affichage du cadre EchangeDeDonn�es et mise � jour du champ
        ...
        var messagePopup = '<div id="msgEtTraces" class="s-process-hidden">   \
            ...
            <div>   \
                ...
                <div id="actions" class="s-process-left"> \
                    ...
                    <div class="s-process-divAction" id="btnDetail"> D�tail </div>    \
        ...
        /*******************************************************
         * Fermeture de la fen�tre message et trace avec un click sur l'image de fermeture
         *******************************************************/
        ...
        $("#menuprincipal").click(function(event){
            ...
            if ($("#menuprincipal").hasClass('orangebackground')) {
                ...
                $('#body').bind('click', function(){
                    $("#menuprincipal").removeClass('orangebackground');
                    $("#divSousMenu").toggle();
                    //suppression du listener une fois celui ci utilis�
                    $('#body').unbind('click');
                });
            }
        });
        ...
        /*******************************************************
         * Gestion de l'import quand on entre le json � la main
         *******************************************************/
        $('#btnValiderImport').click(function(){
            ...
            if (jsonTxt != "") {
                ...
                // on verifie ensuite que le json est import�, pour v�rifier qu'il n'avait pas d'erreur.
                // donc getJson() doit renvoy� le m�me contenu que l'import
                self.options.process && self.options.process.loadJson(jsonTxt);
                if (self.options.process && (self.options.process.getJson() == jsonTxt)) {
                    ...
                    // si �a ne correspond pas, on affiche un petit popup
                    alert("Le JSON import� n'est pas valide");
                }
            }
        });
        
        /*******************************************************
         * Fenetre echange de donn�es
         *******************************************************/
        //Cacher la fenetre echange de donn�e avec un clic sur la bordure
    ...
    loadProcess: function(process){
        ...
        this.$$layoutSlot.css('height', process.contentSize.xheight + 'px').css('width', process.contentSize.xwidth + 'px').css('top', process.docDims.xtop).css('left', process.docDims.xtop);
    },
...
.attr('stroke-width', function(){
            ...
            curve.attr('stroke-dasharray', function(){
                if (forme.xstrokeProps.xtype === 'dottedstroke') 
                    ...
                    if (forme.xstrokeProps.xtype === 'dashedstroke') 
            ...
            curve.attr('fill', function(){
                if (forme.xfillProps.xtype === 'nofill') 
                    ...
                    if (forme.xfillProps.xtype === 'solidfill') 
...
.attr("ry", Math.abs(forme.xanchors[1].y - forme.xanchors[3].y) / 2);
                ...
                case 'fileshape':
                    var g = d3.select(el);
                    shape = g.append('image'); // d3.select($$layoutSlot[0]).selectAll("img") ;
...
.attr('cy', forme.xcenter.y).attr('r', 2);
 ...
 +
        ...
        var self = this, $$layoutSlot = this.$$layoutSlot, formesListe = self.process && self.process.layersArr;
        svgplot = d3.select($$layoutSlot[0]).append('svg');
...
.data(formesListe[gn].xpropsArr).enter().append('g');
 ...
 ||
            ...
            var gradientsvg = d3.select($$layoutSlot[0]).selectAll('svg.shape').filter(function(d, i){
            ...
            var linesWithMarkers = d3.select($$layoutSlot[0]).selectAll('line' /*'svg.shape'*/).filter(function(d, i){
                return (d.xstrokeProps &&
                (d.xstrokeProps.xstart.xtype != 'none' //
 ||
                d.xstrokeProps.xend.xtype != 'none'));
            });
            linesWithMarkers.each(function(d, idx){
            
                appendLineMarkers(d, (linesWithMarkers[0][idx]).parentNode);
            });
            var edge = d3.select($$layoutSlot[0]).selectAll("svg line, svg rect, svg ellipse svg.path");
            ...
            edge.attr('stroke-dasharray', function(d){
                if (d.xstrokeProps.xtype === 'dottedstroke') 
                    ...
                    if (d.xstrokeProps.xtype === 'dashedstroke') 
            ...
            edge.attr('fill', function(d){
                if (d.xfillProps.xtype === 'nofill') 
                    ...
                    if (d.xfillProps.xtype === 'solidfill') 
                        return getHexadecimalColor(d.xfillProps.xcolor);
                    else 
                        if (d.xfillProps.xtype === 'gradientfill') {
...
var txtMsg = "b";
//variable stockant le ou les �change de donn�es
var traceEchData = "c";
...
function traceAlert(amsg){
    //cette fonction ne comprend pas le test relatif � xtraceType la variable n'�tant pas impl�menter dans notre prototype
    //amsg est aussi trac�
    laisseTrace(amsg)
...
//R�cup�re le code hexadecimal de la couleur � partir du code d�cimal
...
function parseLien(chaine){
    var chaineParse = ""
    if (chaine.indexOf("erpfunc", 0) > -1) {
    } // lien vers une fonction erp //non g�r�
...
function calculDegrade(alpha){
    // l'angle de rotation est calcul� par rapport au centre de la forme, on consid�re donc ce point comme centre de notre graphe
    // le rep�re est orthonorm�e de longueur max 50 (la fenetre graphique �Math.tant de longueur et largeur 100)
    var x1, x2, y1, y2;
    
    //traitement cas sp�ciaux
            ...
            if (alpha == Math.PI) {
                ...
                //cas normaux
                    ...
                    if (alpha < Math.PI / 4) {
                        ...
                        if (alpha < Math.PI / 2 - margeArrondi) {
    ...
    //calcul de x1 y1 en fonction de x2, y2. x1 et y1 sont oppos� par rapport � l'origine
    x1 = -x2;
    y1 = -y2;
    // calcul pour basculer en coordonn�es grphique (origine en haut a gauche)
    x1 += 50;
    y1 += 50;
    x2 += 50;
    y2 += 50;
    ...
    switch (markertype) {
        ...
        case 'diamond1':
            ...
            marker = d3.select(parent).append('path').attr('d', function(){
                ...
                if (markertype == 'diamond1') 
                    pathdescr += ' L ' + point4x + ' ' + point4y;
                if (markertype == 'arr1' || markertype == 'diamond1') 
                    pathdescr += ' z';
