...
exports.DeltaManager = helpers.defineClass(DeltaManager, null, {
    applyObjectArrayDelta: function(targetList, sourceList, callback){
        if (sourceList) {
            var self = this;
            if (targetList.length == sourceList.length) {
                sourceList.forEach(function(sourceRecord,index){
                    self.applyObjectDelta(targetList[index], sourceRecord, callback);
                });
            }
            else {
    ...
    applyPageArrayDelta: function(targetList, sourceList, callback){
        if (sourceList) {
            var self = this;
            var targetMap = {};
            if (self.page.$isFullDelta || (sourceList.length == 1 && sourceList[0].$index !== undefined)) {
                sourceList.forEach(function(sourceRecord){
                    ...
                    if (found) {
                        if (sourceRecord.$isDeleted) {
                            ...
                            self.applyObjectDelta(found, sourceRecord, callback);
                ...
                targetList = sourceList.filter(function(record){
                    ...
                    if ((record.$uuid !== undefined) && targetMap[record.$uuid]) {
                        return self.applyObjectDelta(targetMap[record.$uuid], record, callback);
                    }
                    return record;
                });
            }
    ...
    applyObjectDelta: function(target, source, isObjectDelta, callback){
        if (source) {
            var self = this;
                ...
                if (typeof(sourceValue) == 'object') {
                    if (!(targetValue == null || sourceValue === null)) {
                        if (Array.isArray(sourceValue)) {
                            if (isObjectDelta) {
                                sourceValue = self.applyObjectArrayDelta(targetValue, sourceValue, callback);
                            }
                            else {
                                if (self.page.isDeltaMode || self.page.$isFullDelta) {
                                    sourceValue = self.applyPageArrayDelta(targetValue, sourceValue, callback);
                                }
                            }
                        }
                        else {
                            sourceValue = self.applyObjectDelta(targetValue, sourceValue, isObjectDelta, callback);
                        }
                    }
                }
                target[property] = sourceValue;
                if (callback) {
                    callback(target, sourceValue, property);
                }
            });
        }
        return target;
