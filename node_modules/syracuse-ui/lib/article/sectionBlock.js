...
exports.SectionBlock = helpers.defineClass(SectionBlock, Box, {
	...
	onItemInOut: function(onEnter, event, target) {
		...
		if (this.pickers && this.pickers.items.diagnoses) {
			syra_site.diagnoseManager.onBoxItemEnterLeave(this, onEnter);
		}
		if (onEnter) {
			if (this.titleText && this.titleLabel && this.titleLabel.clientWidth != this.titleLabel.scrollWidth) {
				syra_site.siteFunctions.showTip(this, true, this.titleText, this.titleLabel);
			}
	...
	disable: function($isDisabled) {
		...
		if (this.header) {
			syra_site.dom.toggleClass(this.header, "s-disabled", $isDisabled);
		}
		syra_site.dom.toggleClass(this.body, "s-disabled", $isDisabled);
		if (this.domTitle) {
			syra_site.dom.toggleClass(this.domTitle, "s-disabled", $isDisabled);
		}
		if (this.expandPicker) {
			syra_site.dom.toggleClass(this.expandPicker, "s-disabled", $isDisabled);
		}
	},
	...
	ensureVisibility: function() {
		...
		if (this.$item && this.$item.$isEmptyVisible) {
			...
			if (!isVisible && this.boxChildItems.length) {
				var $isDisabled = true;
				for (var ii = 0, jj = this.boxChildItems.length; ii < jj; ii++) {
					var child = this.boxChildItems[ii];
