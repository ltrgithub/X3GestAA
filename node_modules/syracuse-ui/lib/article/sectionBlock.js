...
exports.SectionBlock = helpers.defineClass(SectionBlock, Box, {
	...
	onItemInOut: function(onEnter, event, target) {
		...
		if (onEnter && this.titleText) {
			var show = this.isTitleIconOnly || (this.titleLabel && this.titleLabel.clientWidth != this.titleLabel.scrollWidth);
			show && syra_site.dialogManager.titleTip.show(this.titleText, this.isTitleIconOnly ? this.domTitle : this.titleLabel);
		}
	},
	...
	ensureVisibility: function() {
		...
		if (this.$item && this.$item.$isEmptyVisible) {
			...
			if (!isVisible && this.boxChildItems.length) {
				for (var ii = 0, jj = this.boxChildItems.length; !isVisible && ii < jj; ii++) {
					isVisible = !this.boxChildItems[ii].isInvisible;
				}
			}
		}
