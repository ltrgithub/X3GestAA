...
function _expandBody(section, $opened, event) {
	if (!section.$isDisabled && event) {
		...
		if (section.$item.$isBoxCollapsable && !section.$item.$isPopupContent) {
...
function _ensureDisplay(section) {
	...
	if (!section.isItemHidden && section.isBodyShow != section.$opened) {
		...
		if (syra_quality.isActivated) { //#7168 temp waiting automation improvment
			!section.body.parentNode && section.domItem.appendChild(section.body);
		}
		if (section.hasFlashChildField && section.body.parentNode) {
			/*if (section.body.syraIsHidden) {
             syra_dom.hide(section.body, !section.isBodyShow);
             } else {*/
			...
			if (syra_quality.isActivated) {
				//#7168 temp waiting automation improvment
				syra_dom.hide(section.body, !section.isBodyShow);
			} else {
				if (section.isBodyShow) {
					!section.body.parentNode && section.domItem.appendChild(section.body);
				} else {
					syra_dom.removeChild(section.body);
				}
				//			syra_dom.hide(section.body, !section.isBodyShow);	
			}
...
exports.SectionBlock = helpers.defineClass(SectionBlock, null, {
	...
	loadBox: function() {
		...
		syra_item.registerSectionBlock(this);
		this.boxChildItems = [];
	...
	dispose: function() {
		_resetDDTimer();
		syra_item.unregisterSectionBlock(this);
