...
function _ensureVisibility(section) {
	syra_site.dom.hide(section.domItem, section.isItemHidden);
	syra_site.dom.hide(section.header, section.isItemHidden || section.isTitleHidden);
	if (!section.isItemHidden && section.isBodyShow != section.$opened) {
		section.isBodyShow = section.$opened;
		if (section.hasFlashChildField) {
			if (section.isBodyShow && !section.body.parentNode) {
				section.domItem.appendChild(section.body);
			} else {
			...
			if (section.isBodyShow) {
				...
				syra_site.dom.removeChild(section.body);
			}
			syra_site.dom.hide(section.body, !section.isBodyShow);
		}
	}
}
...
exports.SectionBlock = helpers.defineClass(SectionBlock, null, {
	...
	toggleMaximizeButton: function(show) {
		syra_site.maximizer.toggleButton(this, show, this.ensureButtonsSlot(), this.$skin + "-btn");
	},
	loadBox: function() {
		this.isSection = true;
		if (!this.$item.$layout) {
		...
		if (this.$item.$expression) {
			...
			syra_site.dom.hide(this.body, true); //!!important to avoid unnecesary loading
	...
	ensureLayoutMode: function() {
		if (this.layoutParent) {
			...
			if (this.titleBtn && this.articleParent && this.articleParent.isDesigned) {
				...
				if (updateTitle || (this.isTabLayout != this.isTabSection)) {
...
exports.SpaceBox = helpers.defineClass(SpaceBox, null, {
	loadBox: function() {
		...
		this.layoutSlot.appendChild(this.domItem);
		this.domItem.className = this.$skin = "s-aw-empty";
		this.loaded = true;
	},
	getTitle: function() {
		return "";
