...
function _expandBody(section, $opened, event) {
	if (!section.$isDisabled && event) {
		...
		if (section.$item.$isBoxCollapsable && !section.$item.$isPopupContent) {
			...
			//syra_menus.button.setText(section.titleBtn, syra_local["box_" + state], state);
...
exports.SectionBlock = helpers.defineClass(SectionBlock, null, {
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if (designing !== false) {
			...
			if (updateTitle) {
				...
				//this.openBox(!this.isTabSection || this.$opened);
			}
	...
	openTab: function() {
		...
		syra_site.ensureArticleVisibility(this.articleParent, true);
	},
	...
	loadBox: function() {
		...
		if (this.$item.$expression) {
			...
			this.openBox(!this.isTabSection && this.$item.$opened !== false, true);
		}
	},
	...
	openBox: function(open, drawing) {
		if ((open || drawing) && !this.loaded) {
