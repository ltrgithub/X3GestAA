		...
		if (!section.closeBtn) {
			section.closeBtn = syra_menus.button.add({
				...
				btnclick: function() {
					...
					syra_site.ensureArticleVisibility(articleParent, true);
				}
			});
		}
	} else {
		...
		section.page.externalAdapter.onBoxToggle({
			...
			doEvent: function() {
				if (section.isMenuGroup) {
					syra_menus.group.onBoxToggle(section);
				} else {
					section.openBox(section.$opened);
					syra_site.ensureArticleVisibility(section.articleParent, true);
				}
			}
		});
		if (!section.$item.$isPopupContent) {
			_setExpandIcon(section);
		}
	}
}

function _setExpandIcon(section) {
	if (section.$item.$isBoxCollapsable) {
		var state = section.$opened ? "node_expanded" : "node_collapsed";
		syra_menus.button.setText(section.titleBtn, undefined, state);
	}
}
...
exports.SectionBlock = helpers.defineClass(SectionBlock, null, {
	_ensureDisplay: function() {
		syra_dom.hide(this.domItem, this.isItemHidden);
		syra_dom.hide(this.header, this.isItemHidden || this.isTitleHidden);
		if (!this.isItemHidden && this.isBodyShow != this.$opened) {
			this.isBodyShow = this.$opened;
			if (this.hasFlashChildField && this.body.parentNode) {
				syra_dom.hideFlashContainer(this.body, !this.isBodyShow);
			} else {
				if (this.isBodyShow) {
					!this.body.parentNode && this.domItem.appendChild(this.body);
				} else {
	...
	hideHeader: function(hide) {
		syra_dom.hide(this.header, hide);
	},
	getArticle: function() {
	...
	applyDesignMetaData: function(metaData, designing) {
		...
		if (metaData.$isMaximizable !== undefined) {
			this.$item.$isMaximizable = metaData.$isMaximizable;
			if (this.$item.$isMaximizable) {
				...
				syra_menus.button.remove(this.maximizeBtn);
	...
	openTab: function() {
		...
		syra_site.ensureSectionVisibility(this);
		syra_layoutValidator.validate(this.layoutParent);
		this.page.resizeArticle(true, this.articleParent != this.page ? this.articleParent : null);
	...
	ensureButtonsSlot: function() {
		if (!this.buttonsSlot) {
			this.buttonsSlot = syra_dom.addDiv("s-section-btns", this.header);
		}
	...
	setDescription: function($description) {
		if ($description != null && this.header) {
			if (!this.domDescription) {
				this.domDescription = syra_dom.addDiv(this.$skin + "-description", this.header);
			}
		...
		if (this.titleBtn) {
			...
			if (title != null && title != "") {
				...
				if (isDynamic && !this.isItemHidden && this.loaded) {
					this.hideHeader(this.isTitleHidden);
				}
...
exports.MenuGroup = helpers.defineClass(MenuGroup, SectionBlock, {
	loadBox: function() {
		...
		if (!this.$item.$isPopupContent) {
			...
			if (preferences && preferences.$menuBarGroup) {
				var $opened = preferences.$menuBarGroup[this.id];
	...
	renderLayoutContent: function($layout, slot) {
		syra_menus.group.renderLayoutContent(this, $layout, slot);
	},
...
exports.SpaceBox = helpers.defineClass(SpaceBox, null, {
	loadBox: function() {
		this.isSpaceBox = true;
		this.$field = {};
		this.$designLevel = "field";
		this.domItem = this.header = this.body = syra_dom.addDiv(this.$skin = "s-aw-empty", this.layoutSlot);
		this.loaded = true;
	},
	getTitle: function() {
		return "";
