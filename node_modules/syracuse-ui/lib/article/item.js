...
exports.bind = function(item, $bind) {
	if (item && $bind) {
		...
		item.setDataValue(article.$prototype[$bind], undefined, article.$prototype);
		if (article.isArticleLoaded) {
			var dataset = article.dataset;
			item.setDataValue(dataset[$bind], dataset.$properties && dataset.$properties[$bind], dataset);
		}
	}
};
...
exports.registerSectionBlock = function(section) {
...
exports.unregisterSectionBlock = function(section) {
	if (section.boxParent.childrenSection) {
		var index = section.boxParent.childrenSection.indexOf(section);
		if (index >= 0) {
