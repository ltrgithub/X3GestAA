...
exports.Article = helpers.defineClass(Article, Box, {
    ...
    matchProperty: function(property, record, properties, resultObject){
        var value;
        if (properties) {
        ...
        if (value === undefined) {
            if ((value = record[property]) === undefined) {
                if ((value = this.$prototype[property]) === undefined) {
                    ...
                    if (articleParent) {
                        return articleParent.matchProperty(property, articleParent.dataset, properties, resultObject);
                    }
                }
            }
        }
        ...
        if (result && result.indexOf("{") >= 0) {
            if (result[1] == "@") {
                ...
                matches.forEach(function(match){
                    ...
                    var value = self.matchProperty(property, record, properties, resultObject);
                    if (value != null) {
                        if (value.indexOf && value.indexOf("{") >= 0) {
                            value = self.parseExpression(value, record, properties, resultObject);
                        }
                        else {
