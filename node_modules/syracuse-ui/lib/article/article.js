...
exports.Article = helpers.defineClass(Article, Box, {
	...
	_applyMenusChange: function($deltaMenu, $isAction) {
		...
		for (var bindIndex = 0, bindCount = binds.length; bindIndex < bindCount; bindIndex++) {
			...
			if ($menu === null) {
				...
				if (this.$menus[$bind]) {
					syra_site.deltaManager.applyObjectDelta(this.page, this.$menus[$bind], $menu);
				} else {
		...
		if (item) {
			if (removeDom) {
				if (item.remove) {
					...
					if (item.domItem) {
						syra_site.dom.removeChild(item.domItem);
					}
			...
			if (item.dispose) {
				item.dispose();
				//syra_controller.disposeObject(item);
			}
		}
	},
	...
	ensureChildListsVisibility: function() {
		if (this.childLists) {
			for (var ii = 0, jj = this.childLists.length; ii < jj; ii++) {
				this.childLists[ii].ensureListVisibility();
			}
		}
	},
