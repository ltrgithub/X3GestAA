...
exports.Article = helpers.defineClass(Article, Box, {
    ...
    bindMenuBox: function(box){
        ...
        if (this.$menus) {
            ...
            for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
                var $bind = $binds[ii];
                var item = this.menuItems[$bind];
                if (!item || (item.length == 0)) {
                    box.addMenuItem(this.$menus[$bind], this.dataset);
                }
            }
        }
    },
        ...
        if (resources) {
            if (resources.$links === null) {
                ...
                for (var ii = 0, jj = this.menuBoxes.length; ii < jj; ii++) {
                    this.menuBoxes[ii].clearMenuItems();
                }
            }
            else {
                if (resources.$links) {
                    this._applyMenuChange(resources.$links);
                }
                if (resources.$actions) {
                    this._applyMenuChange(resources.$actions, true);
                }
            }
        }
    },
    _applyMenuChange: function($deltaMenu, $isAction){
        ...
        for (var bindIndex = 0, bindCount = binds.length; bindIndex < bindCount; bindIndex++) {
            ...
            if (this.$menus[$bind]) {
                this.page.deltaManager.applyObjectDelta(this.$menus[$bind], $menu);
            }
            else {
