...
exports.beforeDrawBox = function(article) {
	...
	article.boundFields = {};
	article.idMap = {};
	syra_menus.menus.initialize(article);
};
exports.endDrawBox = function(article, initData) {
	syra_menus.menus.applyChange(article, article.$prototype);
		...
		if (article.isArrayField) {
			if (savedDelta.$uuid) {
				...
				if (dataRecord) {
					...
					article.setDataValue(delta, undefined, syra_dataset.ensure(article.articleParent));
				}
			}
		} else {
			var delta = {};
		...
		for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
			...
			if (newFieldValue !== undefined || newFieldMetaData !== undefined) {
				...
				for (var kk = 0, mm = fields.length; kk < mm; kk++) {
					fields[kk].setDataValue(newFieldValue, newFieldMetaData, newData);
