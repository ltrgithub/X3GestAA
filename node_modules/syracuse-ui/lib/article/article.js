			...
			if (Object.keys($properties).length > 0) {
				...
				for (var mm = 0, pp = $binds.length; mm < pp; mm++) {
					if (!bindsInProperties[$binds[mm]] && $properties[$binds[mm]].$diagnoses && $properties[$binds[mm]].$diagnoses.length > 0) {
						...
						if (this.isRecordArticle && this.builder.$inlineFieldBinds && !this.inlineCardRow) {
							if (this.builder.$inlineFieldBinds.indexOf($binds[mm]) >= 0) {
								hasHiddenCardDiagnose = true;
								continue;
							}
						}
						...
						for (var aa = 0, bb = $diagnoses.length; aa < bb; aa++) {
							$diagnoses[aa].$message = $binds[mm].toUpperCase() + " : " + ($diagnoses[aa].$message || $diagnoses[aa].message);
							$diagnoses[aa].$severity = $diagnoses[aa].$severity || $diagnoses[aa].severity;
						}
			...
			if (result[1] == "@") {
				...
				for (var ii = 0, jj = matches.length; ii < jj; ii++) {
					...
					if (ignoreCurrent && ignoreCurrent[match]) {
						if (this.articleParent) {
							value = this.articleParent.matchProperty(match, property, this.articleParent.dataset, properties, $subRecordKey);
						}
					} else {
						value = this.matchProperty(match, property, record, properties, $subRecordKey);
					}
					if (value != null) {
						if (value.indexOf && value.indexOf("{") >= 0) {
							...
							value = this.parseExpression(value, record, properties, $subRecordKey, ignoreCurrent);
						} else {
