...
exports.Article = helpers.defineClass(Article, Box, {
    _isDeltaMode: function(){
        return this.getPage().$facet == "$edit";
    },
    ...
    loadBox: function(initData){
        ...
        if (articleParent) {
            ...
            this.$prototype = this.$prototype || articleParent.$prototype.$properties[this.$item.$bind];
        }
        this.dataset = {};
        this.boundFields = {};
        this.idMap = {};
    ...
    bindFieldEvents: function($$secondary){
        ...
        if ($$fieldsParent) {
            ...
            $$fieldsParent.delegate("textarea[data-s-field],input[data-s-field],select[data-s-field]", "click change keydown keypress keyup focusin focusout", function(event){
                ...
                var field = self.idMap[$(event.target).closest("[data-s-field]").attr("data-s-field")];
                if (field) {
                    field.onFieldInputEvent(event);
                    event.stopPropagation();
                }
                ...
                if (event.target.getAttribute("data-s-picker") == "menus") {
                    var fieldId = event.target.getAttribute("data-s-field");
                    (fieldId ? self.idMap[fieldId].menusController : self).contextMenu.onFieldClickPicker();
                }
                else {
                    var field = self.idMap[$(event.target).closest("[data-s-field]").attr("data-s-field")];
                    if (field) {
    ...
    _applyArrayDelta: function(targetList, sourceList){
        if (sourceList) {
            var self = this;
            var targetMap = {};
            if (sourceList.length == 1) {
                var sourceRecord = sourceList[0];
        ...
        if (newData) {
            ...
            Object.keys(newData).forEach(function($bind){
                ...
                if (typeof(newValue) == 'object') {
                    ...
                    if (!(recordValue == null || newValue === null)) {
                        if (Array.isArray(newValue)) {
                            if (self._isDeltaMode()) {
                                newValue = self._applyArrayDelta(recordValue, newValue);
                            }
                        }
                        else {
                            newValue = self._applyObjectDelta(recordValue, newValue);
                        }
                    }
                }
    ...
    bind: function(item, $bind){
        ...
        if ($bind == "xarray") {
            if (this.boundFields[$bind].length > 3) {
                debugger;
            }
        }
