...
exports.Article = helpers.defineClass(Article, Box, {
	...
	loadBox: function(initData) {
		document.site.registerArticle(this);
		this.autosizeFields = [];
		this.childArticles = {};
		this.registerChildArticle();
	...
	formatMenuUrl: function($menu, record, callback) {
		if (callback) {
			if ($menu.$parameters) {
				...
				callback(this.parseExpression($menu.$sourceUrl || $menu.$url, record, undefined, $menu.$subRecordKey, undefined, true));
			}
			return;
		} else {
			var properties;
			...
			return this.parseExpression($menu.$sourceUrl || $menu.$url, record, properties, $menu.$subRecordKey, undefined, true);
		}
	},
	...
	parseExpression: function(expression, record, properties, $subRecordKey, ignoreCurrent, isUrl) {
		var result = expression;
		record = record || this.dataset;
		if (result && result.indexOf("{") >= 0) {
			if (result[1] == "@") {
				var text = result.slice(1, result.length - 1);
				var $localization = this.$prototype.$localization || this.page.$prototype.$localization;
				if ($localization) {
	...
	diposeContent: function() {
		...
		this.autosizeFields = this.childArticles = this.layouts = this.idMap = this.boundFields = this.layoutContent = null;
	},
	...
	registerChildArticle: function() {
		if (this.articleParent) {
			this.articleParent.childArticles[this.id] = this;
		}
	},
	unregisterChildArticle: function() {
		if (this.articleParent && this.articleParent.childArticles) {
			delete this.articleParent.childArticles[this.id];
		}
	},
	resizeArticle: function() {
		if (this.childArticles) {
			var ids = Object.keys(this.childArticles);
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				this.childArticles[ids[ii]].resizeArticle();
			}
		}
		if (this.autosizeFields.length) {
	...
	ensureArticleVisibility: function() {
		if (this.isContentUpdated) {
			...
			if (this.layoutContent) {
				this.page.layoutValidator.validate(this.layoutContent, true);
			}
			this.page.isContentUpdated = true;
			this.isContentUpdated = false;
		}
	},
	dispose: function() {
		this.diposeContent();
		this.unregisterChildArticle();
		document.site.unregisterArticle(this);
