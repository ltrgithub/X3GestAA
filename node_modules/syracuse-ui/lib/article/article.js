...
exports.Article = helpers.defineClass(Article, Box, {
	...
	getPrototype: function() {
		if (!this.$prototype) {
			...
			this.$prototype = syra_page.addPrototypeToRepository(this.page, this.$prototype);
		}
					...
					if (field.validate && field.$isEditMode) {
						...
						if (!field.validate(newValue)) {
							...
							if (newValue != undefined) {
								if (this.page.isDialogParameter || (this.page.isPageValidating && newValue != field.currentValue)) {
	...
	applyChange: function(newData) {
		if (newData) {
			...
			this.applyChangeToDetailRelations();
			this.$syraLoaded = true;
		}
	},
	applyChangeToDetailRelations: function() {
		if (this.masterChildRelations && this.masterChildRelations.length) {
			for (var ii = 0, jj = this.masterChildRelations.length; ii < jj; ii++) {
				var relation = this.masterChildRelations[ii];
				var refresh = false;
				var keys = Object.keys(relation.masterKeys);
				for (var mm = 0, kk = keys.length; mm < kk; mm++) {
					var key = keys[mm];
					if (this.dataset[key] != relation.masterKeys[key]) {
						relation.masterKeys[key] = this.dataset[key];
						refresh = true;
					}
				}
				refresh && relation.refreshChildRelation();
			}
		}
	},
		...
		if (this.autosizeFields && this.autosizeFields.length) {
			for (var ii = 0, jj = this.autosizeFields.length; ii < jj; ii++) {
				if (!boxParent || boxParent.isChild(this.autosizeFields[ii])) {
					this.autosizeFields[ii].resizeField && this.autosizeFields[ii].resizeField(doResize);
				}
			}
		}
	},
	dispose: function() {
		...
		this.diposeContent();
		this.masterChildRelations = this.parentVariantField = this.$prototype = this.dataset = this.layouts = this.$urlParams = this.designer = null;
