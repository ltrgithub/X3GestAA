...
exports.FieldsAutoInsert = helpers.defineClass(FieldsAutoInsert, FieldsGarbage, {
	...
	showDiagnoses: function() {
		var i, len, $diagKeys, diagField, $diagnoses = [], failed = 0, succeed = 0;
		if(this._result && this._result.diagnoses) {
			// Deal with fields diagnosis
			$diagKeys = Object.keys(this._result.diagnoses);
			for(i = 0, len = $diagKeys.length; i < len; i++) {
				$diagnoses.push(this._result.diagnoses[$diagKeys[i]]);
			}
			// Deal with general diagnosis
			$diagKeys = Object.keys(this._result);
			for(i = 0, len = $diagKeys.length; i < len; i++) {
				diagField = this._result[$diagKeys[i]];
			...
			document.site.showDiagnoses({"$diagnoses": $diagnoses}, this);
		}
	},
	makeDiagnosis: function(labelCode, labelCodeArgs, labelString, severity, details, stack) {
		return util.makeDiagnosis(labelCode ? (this.page.localize[labelCode] || labelCode) : labelString, severity, stack, !this._isFusionPage ? "Client framework" : null, null, details);
	},
	dispose: function() {
