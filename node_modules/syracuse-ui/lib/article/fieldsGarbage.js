...
exports.FieldsGarbage = helpers.defineClass(FieldsGarbage, null, {
    ...
    _getDefinedFieldsObject: function($article, $object, layoutMap){
        var secCount = -1, blockCount, cat, currSec = null, currBlock = null, deepMax = 9999, deep = 0, deepS = deepMax, deepB = deepMax;
...
exports.FieldsAutoInsert = helpers.defineClass(FieldsAutoInsert, FieldsGarbage, {
    load: function(article, $designLevel){
        var $definedBinds = {};
        this._elapse = {"elapseF": (new Date()).getTime(), "elapseI": 0};
    ...
    showDiagnoses: function() {
        var result, i, len, $diagKeys, diagField, $diagnoses = [],
            statsBlocks = null, statsFields = null, statsSections = null, failed, applyChange = false,
            change = {"$properties": {}};
        if (this._result) {
            ...
            if((result = this._result.block) && result.diagnoses) {
                // Deal with blocks diagnoses
                $diagKeys = Object.keys(result.diagnoses);
                for (i = 0, len = $diagKeys.length; i < len; i++) {
                    $diagnoses.push(result.diagnoses[$diagKeys[i]]);
                }
                statsBlocks = this.insertStats(result);
            }
            if((result = this._result.section) && result.diagnoses) {
                // TODO
                statsSections = this.insertStats(result);
            }
            // General diagnoses
    ...
    insertStats: function(result) {
        var i, len, $diagKeys = Object.keys(result), diag, ret = {"ok": 0, "ko": 0};
        for (i = 0, len = $diagKeys.length; i < len; i++) {
