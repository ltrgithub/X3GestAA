...
exports.FieldsAutoInsert = helpers.defineClass(FieldsAutoInsert, FieldsGarbage, {
	...
	showDiagnoses: function() {
		var i, len, $diagKeys, diagField, $diagnoses = [],
			failed = 0,
			succeed = 0,
			applyChange = false,
			change = {"$properties": {}};
		if (this._result && this._result.diagnoses) {
			...
			for (i = 0, len = $diagKeys.length; i < len; i++) {
				diagField = this._result.diagnoses[$diagKeys[i]];
				if(!util.isErrDiagnosis(diagField.$severity)) {
					change.$properties[$diagKeys[i]] = {"$diagnoses": [this._result.diagnoses[$diagKeys[i]]]}
					applyChange = true;
				}
				else {
