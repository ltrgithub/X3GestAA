				...
				if (fields) {
					for (var mm = 0, kk = fields.length; mm < kk; mm++) {
						...
						if (field.domItem) {
							...
							field.domItem.appendChild(flag);
							document.site.dom.setZIndex(flag);
							if (field.$isHidden && field.layoutSlot) {
								field.layoutSlot.style.display = "";
								if (field.rowSeparator) {
									field.rowSeparator.style.display = "";
								}
								field.domItem.style.display = "";
								document.site.dom.toggleClass(field.layoutSlot, "s-aw-visibility-forced", true);
							}
						}
					}
				}
