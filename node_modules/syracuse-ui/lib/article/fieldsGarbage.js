...
exports.FieldsGarbage = helpers.defineClass(FieldsGarbage, null, {
	load: function(article) {
		...
		if ($garbageFields) {
			...
			for (var ii = 0, jj = $garbageFields.length; ii < jj; ii++) {
				this._garbageFields[$garbageFields[ii]] = $properties[$garbageFields[ii]];
			}
		}
	...
	onAddField: function($bind) {
		if (this._$freeFieldBinds.indexOf($bind) >= 0) {
	...
	loadFreeField: function() {
		this._$freeFieldBinds = [];
		var $properties = this.getProperties();
		var $fieldsBinds = Object.keys($properties);
		var $definedBinds = {};
		this._getDefinedFieldsObject(this.article.$item.$layout.$items, $definedBinds);
		for (var ii = 0, jj = $fieldsBinds.length; ii < jj; ii++) {
			...
			if ($field && !$field.$isExcluded) {
				if (!$definedBinds[$bind] && ($bind.charAt(0) != "$")) {
					if (!(this.article.page.$isFusionPage && $bind.charAt(0) == "b")) {
	...
	_getDefinedFieldsObject: function($items, $object) {
		if ($items) {
			for (var ii = 0, jj = $items.length; ii < jj; ii++) {
				...
				if ($item.$bind) {
					...
					if ($item.$items) {
						this._getDefinedFieldsObject($item.$items, $object);
					}
					if ($item.$layout && $item.$layout.$items) {
						this._getDefinedFieldsObject($item.$layout.$items, $object);
					}
				}
			}
		}
