...
exports.SendBag = helpers.defineClass(SendBag, null, {
    _findDataRecord: function($uuid){
        var found;
        if (this.dataRecords) {
            this.dataRecords.some(function(dataRecord){
    ...
    _ensureFullDataSet: function(list){
        ...
        this.dataRecords = list.ensureDataRecords().map(function(dataRecord){
            var saved;
    ...
    _ensureRecord: function(record){
        ...
        var found = this._findDataRecord(recorDataset.$uuid);
        if (!found) {
            if (this.dataRecords.length > 0) {
                this._ensureFullDataSet(record.list);
                found = this._findDataRecord(recorDataset.$uuid);
                if (!found) {
                    this.dataRecords.unshift(found = {
                        $uuid: dataRecord.$uuid
                    });
                }
            }
    ...
    createRecord: function(record, index){
        ...
        if (this.dataRecords.length > 0) {
            this._ensureFullDataSet(record.list);
            var found = this._findDataRecord(saved.$uuid);
            if (!found) {
