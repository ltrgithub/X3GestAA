...
function _toggleExpandPicker(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		...
		if (!(box.$opened = box.$item.$opened !== false)) {
			box.expandPicker.title = box.localize.box_expand;
			document.site.toggleClass(box.expandPicker, "s-close", true);
		}
	} else {
		if (box.expandPicker) {
...
exports.onBoxLayoutChange = function(box) {
	if (box.pickers) {
		document.site.removeDomChild(box.pickers);
		box.pickers = null;
...
exports.toggleAdvancedFieldPicker = function(box, $isAdvancedMode) {
	...
	if (box.pickers && box.pickers.items) {
		var advancedPicker = box.pickers.items.advanced;
		if (advancedPicker) {
...
exports.doPicker = function(box, pickerId) {
	switch (pickerId) {
		case "maximize":
			...
			box.page.body.appendChild(box._item);
			box._$isMaximized = true;
			box.pickers.items.maximize.title = box.localize.box_maximize;
			box.pickers.items.maximize.className = box.$skin + "-maximize s-minimize";
			box.pickers.items.maximize.setAttribute("data-s-picker", "box-picker");
			box.pickers.items.maximize.syraBoxId = box.id;
			box.pickers.items.maximize.syraBoxPickerId = "minimize";
			break;
		case "minimize":
			if (box.tabTitle) {
			...
			box.pickers.items.maximize.title = box.localize.box_minimize;
			box.pickers.items.maximize.className = box.$skin + "-maximize";
			box.pickers.items.maximize.setAttribute("data-s-picker", "box-picker");
			box.pickers.items.maximize.syraBoxId = box.id;
			box.pickers.items.maximize.syraBoxPickerId = "maximize";
			break;
		case "advanced":
