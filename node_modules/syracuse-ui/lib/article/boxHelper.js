...
function _setMaximizeBtn(box) {
	var btn = box.btns.items.maximizebox;
	btn.className = box.$skin + (box.isMaximized ? "-maximizebox" : "-minimizebox");
	syra_menus.setButtonTitle(btn, box.isMaximized ? syra_local.box_minimizebox : syra_local.box_maximizebox);
}
...
exports.applyButtonMetadata = function(box, metaData, designing) {
	...
	if (metaData.$isMaximizable !== undefined) {
		exports.showButton(box, "maximizebox", box.$item.$isMaximizable = metaData.$isMaximizable);
	}
	if (metaData.$hasMenus !== undefined) {
		exports.showButton(box, "menus", metaData.$hasMenus);
	}
...
function _showExpandButton(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		if (!box.expandBtn) {
			...
			box.expandBtn = syra_menus.addIconButton(syra_local.box_collapse, css, "expand");
			box.expandBtn.syraItem = box.id;
			box.header.insertBefore(box.expandBtn, box.header.firstChild);
		}
		box.$opened = box.$item.$opened !== false;
		syra_menus.setButtonTitle(box.expandBtn, box.$opened ? syra_local.box_collapse : syra_local.box_expand);
		syra_site.dom.toggleClass(box.expandBtn, "s-close", !box.$opened);
	} else {
		if (box.expandBtn) {
			syra_site.dom.removeChild(box.expandBtn);
		}
	}
	if (designing) {
...
exports.showButton = function(box, btnId, show, syraOnClick) {
	var slotParent = box.tabTitleHead ? box.tabTitleHead : box.header;
	var btns = box.btns;
	if (show) {
		if (!btns) {
			box.btnWidth = box.btnWidth || 18;
			btns = box.btns = {
				items: {},
		...
		if (!btns.items[btnId]) {
			var btn = syra_menus.addIconButton(syra_local["box_" + btnId], box.$skin + "-" + btnId, _onClickHandlers[btnId] || syraOnClick);
