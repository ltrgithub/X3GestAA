...
function _showExpandButton(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		if (!box.expandBtn) {
			...
			box.expandBtn = syra_menus.addIconButton(syra_local.box_collapse, css, "onOpenerClick");
...
exports.showButton = function(box, btnId, show, syraOnClick, fontIcon) {
	...
	if (show) {
		...
		if (!btns.items[btnId]) {
			var btn = syra_menus.addIconButton(syra_local["box_" + btnId], box.$skin + "-" + btnId, _onClickHandlers[btnId] || syraOnClick, null, fontIcon || btnId);
			btns.items[btnId] = btn;
			btns.width += box.btnWidth;
			if (!box.isButtonsAutoSize) {
