...
function _setMaximizePicker(box) {
	...
	picker.title = syra_local["box_" + key];
}
...
exports.toggleExpandPicker = function(box, designing) {
	if (box.$item.$isBoxCollapsable && !box.$item.$isTitlePicker) {
		if (!box.expandPicker) {
			...
			box.expandPicker.title = syra_local.box_collapse;
			if (box.$item.$css) {
		...
		box.expandPicker.title = box.$opened ? syra_local.box_collapse : syra_local.box_expand;
...
exports.togglePicker = function(box, pickerId, show, syraOnClick) {
	...
	if (show) {
		...
		if (!box.pickers.items[pickerId]) {
			...
			picker.title = syra_local["box_" + pickerId];
