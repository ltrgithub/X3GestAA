...
exports.fieldsInsert = function (designLevel, isFusion, garbageObj) {
	...
	var insertLibrary = (isFusion && _insertFusion[level]) || _insertStandard[level], oldLibrary;
	var i, len = garbageObj._$freeFieldBinds.length, targets = {}, $bind, ret, _getSrclayout, _getFreeBlock, srcLayout, freeBlock;
	var ctx = {"library": insertLibrary, "page": garbageObj.article.page}, targetsKeys, target, localize = ctx.page.localize;
	var result = {"diagnoses": {}, "warning": 0, "success": 0, "error": 0, "fatal": 0}, severity, message, detail;
	...
	try {
		...
		for(i = 0, len = targetsKeys.length; i < len; i++) {
			...
			else if(!target.block && target.section) {
				if (!(freeBlock =_getFreeBlock.call(garbageObj, target.section, ctx))) {
					message = "can't find free block slot - todo"; //localize.autoInsertNoFreeBlock
				}
				else {
					ret = _insertLayout.call(garbageObj, srcLayout, freeBlock, freeBlock.field.length);
					if(ret.success) {
						message = "insert in last block - todo"; //localize.autoInsertInLastBlock
						severity = "warning";
					}
				}
			}
			else {
				if (!(freeBlock = _getFreeBlock.call(garbageObj, null, ctx))) {
					message = "can't find free block slot - todo"; //localize.autoInsertNoFreeBlock
				}
				else {
					ret = _insertLayout.call(garbageObj, srcLayout, freeBlock, freeBlock.field.length);
					if(ret.success) {
						message = "insert in first section/last block - todo"; //localize.autoInsertInLastSecBlock
						severity = "warning";
					}
				}
			}
			// Set diagnosis
			result.diagnoses[targetsKeys[i]] = [{"$severity": severity, "$message": message, "$details": ret ? ret.details : null}];
			result[severity]++;
		}
	}
	catch (e) {
		...
		if(garbageObj && garbageObj._$freeFieldBinds) {
			for(ii = 0, lenii = garbageObj._$freeFieldBinds.length; ii < lenii; ii++) {
				if(!result.diagnoses[garbageObj._$freeFieldBinds[ii]]) {
					result.diagnoses[garbageObj._$freeFieldBinds[ii]] = [{"$severity": "fatal", "$message": "todo - localize.autoInsertUnexpected", "$detail": e.message, "$stack": e.stack}];
					result.fatal++;
				}
			}
		}
...
var _insertFusion = {
	...
	"levelArticle": {
		...
		"_oddFieldTypes" : {"application/x-array": true, "text/rtf": true, "text/html": true, "image": true, "application/wid": true, "application/x-shockwave-flash": true},
		...
		ruleByTitle: function(context) {
			...
			return ret || {"section": data.section};
		},
		getSrcLayout: function($bind, context) {
			var block, slot = null, layout = null;
			...
			return layout || {"$bind": $bind};
		},
		getlastFreeBlock: function(section, context) {
			var i, block = null, self = this;
		...
		_checkInBlock: function(block, context) {
			var i, len, scope, score = 0, fieldSlot = null;
			...
			return fieldSlot !== null ? {"block": block, "slot": fieldSlot} : null;
		},
		...
		_searchBlock: function(map, $bind, data) {
			var found = null, sec, i, j, leni = map.section.length, lenj;
