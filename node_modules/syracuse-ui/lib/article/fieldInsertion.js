	...
	"levelArticle": {
		...
		"field": {
			...
			$ruleDimField: function(context) {
				var ret = null, block, name, i, len, persisteP, score = 0, bindDim, targetLayout, dimFldName;
				function _getDimName(fldName) {
					var dimIdx, dimMap = {"n": null, "id": null}, ret = false;
				...
				if((name = this._$properties[context.$bind].$X3Name) && (persisteP = this.article.$persistPrototype && this.article.$persistPrototype.$properties)) {
					if ((dimFldName = _getDimName(name))) {
						...
						if (score && score == dimFldName.id - 1 && (bindDim = persisteP[dimFldName.n + "_" + (dimFldName.id - 1)].$xid)) {
							...
							if(block && (targetLayout = context.library._getBlockLayout.call(this, block, context.$bind, {"bind": bindDim, "after": true}, context, {"strictRow": true}))) {
								ret = {"rank": block.boundFields.indexOf(bindDim) + 1, "block": block, "layout": targetLayout.layout, "diagnosis": targetLayout.diagnosis};
							}
						}
						else if(!score) {
			...
			getFieldSrclayout: function($bind, context, data) {
				var block, layout = null, byPassFindById = !!(data && data.byPassById);
			...
			$ruleLast: function(context, secRank) {
				...
				for(i = 0; i < lenR; i++) {
					...
					for(j = 0, len = blockR.boundFields.length; j < len; j++) {
						if(this._$freeFieldBinds.indexOf(blockR.boundFields[j]) >= 0) {
							score++;
						}
					}
				...
				if(newBlocks.length) {
					...
					// Process...
					// TODO: modif = "trier sur rank croissant et insérer de décroissant à croissant" + "tester sur fin"
		...
		_getBlockLayout: function(block, $bind, where, context, dataEx) {
			...
			var parentItem = block.items, parentItemSlot = 0, deep = 0, dummy;
	...
	"levelArticle": {
		...
		"field": {
			...
			getFieldSrclayout: function($bind, context) {
				return _insertFusion[context.level].field.call(this, context, {"byPassById": true});
			},
			getLastFreeBlock: function(section, context) {
