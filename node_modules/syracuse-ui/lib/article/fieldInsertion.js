...
var _insertFusion = {
	...
	"levelArticle": {
			...
			$ruleByRank: function(context, freeBlock, srcSection, srcSectionRank) {
				...
				if (secA) {
					...
					if (i < lenA || (last = freeBlock.rank == (parseInt(temp[lenA - 1], 10) + 1))) {
						...
						ret = context.library._processNewBlock.call(this, freeBlock.rank, freeBlock.block, blockA.parent, !last ? blockA.parentSlot : blockA.parent.length, blockA.parentLayout);
					} else {
						...
						ret = context.library._processNewBlock.call(this, freeBlock.rank, freeBlock.block, blockA.parent, rank, blockA.parentLayout, diagnosis);
					}
				}
				return ret;
			},
			$ruleLast: function(context, freeBlock) {
				...
				for (i = 0, len = sections.length; i < len; i++) {
					if (sections[i].block.length) {
						...
						ret = context.library._processNewBlock.call(this, freeBlock.rank, freeBlock.block, blockA.parent, blockA.parent.length, blockA.parentLayout, diagnosis);
						break;
					}
				}
				return ret;
		...
		_processNewBlock: function(rank, blockRef, slot, slotRank, slotParentLayout, diag) {
			var ret, clonedLyt, i, len, lytKeys;
			ret = {
				"diagnosis": diag,
				"block": blockRef,
				"rankOrigin": rank,
