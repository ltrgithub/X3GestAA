...
function _insertFields (level, insertLibrary, ctx) {
	...
	var result = _helpers.initResult(), severity, message, fieldName, self = this;
	...
	try {
		...
		for (i = 0; i < len; i++) {
			...
			targets[ctx.$bind] = _processEntity.call(this, ctx, "field");
		}
		...
		for (i = 0, len = targetsKeys.length; i < len; i++) {
			...
			// Set diagnosis
			fieldName = ctx.isFusion && _getFusionFldName(targetsKeys[i]);
			result.diagnoses[targetsKeys[i]] = target.diagnosis || this.makeDiagnosis(null, null, (fieldName || targetsKeys[i]) + " : " + localize[message], severity, ret ? ret.details : null);
			result[target.diagnosis && target.diagnosis.$severity || severity]++;
		}
	} catch (e) {
		var ii, lenii;
	...
	"levelArticle": {
		...
		"field": {
			...
			$ruleById: function(context) {
				...
				if ((block = context.library._findBlockById(this._$layoutMap, null, context.fieldScope, null, context.page))) {
			...
			getLastFreeBlock: function(section, context) {
				...
				targetLayout = context.library._getBlockLayout.call(this, block, context.$bind, block.boundFields.length, context);
				return block && targetLayout ? {"block": block, "layout": targetLayout.layout, "diagnosis": targetLayout.diagnosis} : null;
			}
		},
		"block": {
			"rules": ["$ruleLast"],
			$ruleLast: function(context, secRank) {
				var ret = {"inserted": [], "defects": []}, i, len, secA = this._$layoutMap.section[secRank], secR = this._$protoLayoutMap.section[secRank];
				var blockR, blockA, lenR = secR.block.length, lenA = secA.block.length, nbToInsert, rank, scope, score = 0, diagnosis;
				var newBlocks = [], authoBlocks = {}, temp;
				var j, k, len, self = this;
				...
				if(lenR > lenA) {
					...
					if (nbToInsert) {
						...
						if(nbToInsert) {
							// Oups, can't get reference to new block... 
							diagnosis = this.makeDiagnosis("autoInsertBlockCantGet", [nbToInsert], null, "error");
							ret.defects.push ({"diagnosis": diagnosis});
						}
					}
				}
				return ret;
		...
		_checkInBlock: function(block, context) {
			...
			if(fieldRank >= 0) {
				targetLayout = context.library._getBlockLayout.call(this, block, context.$bind, fieldRank, context);
				return targetLayout ? {"block": block, "layout": targetLayout.layout, "diagnosis": targetLayout.diagnosis} : null;
			}
			else {
				return null;
			}
