...
function _insertBlocks(level, insertLibrary, ctx) {
	var targetsKeys, target, result = _helpers.initResult(),
		severity, message;
...
function _insertFields(level, insertLibrary, ctx) {
	var i, len = this._$freeFieldBinds.length,
		_getFieldSrclayout, _getFreeBlock;
		...
		"block": {
			...
			$ruleLast: function(context, secRank) {
				var ret = {
					"inserted": [],
					"defects": []
				}, i, len, secA = this._$layoutMap.section[secRank],
					secR = this._$protoLayoutMap.section[secRank];
				var blockR, blockA, lenR = secR.block.length,
					lenA = secA.block.length,
					nbToInsert, rank, scope, score = 0,
					diagnosis;
				var newBlocks = [],
					authoBlocks = {}, temp;
				...
				function _processNewBlock(idx, blockRef, slot, slotRank, diag) {
					...
					ret.inserted.push({
						"diagnosis": diag,
						"block": blockRef,
						"rankOrigin": newBlocks[idx].rank,
						"layoutToInsert": blockRef.parent[blockR.parentSlot],
						"layout": {
							"slot": slot,
							"rank": slotRank
						}
