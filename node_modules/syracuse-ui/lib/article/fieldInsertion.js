...
function _insertFields(level, insertLibrary, ctx) {
	...
	function _getMethod(name, deft) {
		return (ctx.isFusion && _insertFusion[level] && _insertFusion[level].field && _insertFusion[level].field[name]) || (deft ? _insertStandard[level].field[name] : null);
	}
	...
	function _insert(bind, target) {
		...
		if (target && target.notBound) {
			...
			if (!target || !target.block) {
				...
				if (!_getAvailableBlock || !(target = _getAvailableBlock.call(self, jocker, ctx))) {
		...
		result.diagnoses[bind] = (target && target.diagnosis) || self.makeDiagnosis(null, null, (fieldName || bind) + " : " + localize[message], severity, ret ? ret.details : null);
		result[target && target.diagnosis && target.diagnosis.$severity || severity]++;
	}
	try {
		// Init
		_getFieldSrclayout = _getMethod("getFieldSrclayout", true);
		_getAvailableBlock = _getMethod("getLastAvailableBlock", true);
		if ((_initialize = _getMethod("initialize"))) {
...
var _insertFusion = {
	"levelField": {
		"field": {
			...
			$ruleLast: function(context) {
				// For the time being, we don't care abour card mode...
				var ret = null,
					articleLytA = this.article.$item,
					block = {
						"boundFields": []
					};
				context._layoutLytR = _helpers._getFieldlayoutFromMap(this.page._$protoLayoutMap, this.article.$item.$bind, context, true);
				if (context._layoutLytR) {
					context.fieldScope = context.page.getFieldScope(context.$bind, false);
					block.items = articleLytA.$items || (articleLytA.$layout ? articleLytA.$layout.$items : []);
			...
			getFieldSrclayout: function($bind, context, data) {
				...
				layoutP = context._layoutLytR || _helpers._getFieldlayoutFromMap(this.page._$protoLayoutMap, this.article.$item.$bind, context, true);
				if (layoutP) {
	...
	"levelArticle": {
		...
		"field": {
			...
			getFieldSrclayout: function($bind, context, data) {
				return _helpers._getFieldlayoutFromMap(this.page._$protoLayoutMap, $bind, context, !! (data && data.byPassById));
			},
		...
		"section": {
			...
			getFreeSections: function(context, data) {
				var free = [],
					...
					scope, score = 0;
				var byPassRank = !! (data && sectionRank.byPassRank),
		...
		_getAuthoredSectionForBind: function(bind, sectionRefTitle, context, data) {
			...
			if (!bind || (dataEx && dataEx.byPassById) || (!(_helpers._findBlockById(this._$layoutMap, null, context.page.getFieldScope(bind), dataEx, context.page)) && !dataEx.section)) {
...
var _insertStandard = {
	...
	"levelArticle": {
		...
		"field": {
			...
			getFieldSrclayout: function($bind, context) {
				return _helpers._getFieldlayoutFromMap(this.page._$protoLayoutMap, $bind, context, true);
			},
			getLastAvailableBlock: function(section, context) {
				// TODO : close cvg => factorize!
			}
