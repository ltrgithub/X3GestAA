		...
		for (i = freeSections.length - 1; i >= 0; i--) {
			...
			if (target) {
				...
				if (ret.success) {
					...
					// Have to remap authored layoud according section insertion
	...
	"levelArticle": {
		...
		"block": {
			...
			$ruleByRank: function(context, freeBlock, srcSection, srcSectionRank) {
				...
				// Resolve authored section according source section description. TODO : be carefully with block.boundFields[0] if empty...
			...
			getFreeBlocks: function(sectionRank, context, data) {
				...
				for (i = 0; i < leni; i++) {
					...
					if (score == lenj) {
						// TODO : be carrefullif block without fields
		...
		"section": {
			...
			$ruleByRank: function(context, freeSection) {
				...
				if (i < lenA || (last = freeSection.rank == (authoSecRank[lenA - 1].rank + 1))) {
					secA = this._$layoutMap.section[!last ? authoSecRank[i].rank : authoSecRank[lenA - 1].rank];
					ret = context.library._processNewSection.call(this, freeBlock.rank, freeBlock.block, blockA.parent, !last ? secA.paranetSlot : secA.parent.length);
				} else if (lenA >= freeSection.rank) {
					// By default, insertion in the nearest slot
					secA = this._$layoutMap.section[freeSection.rank - 1];
					...
					ret = context.library._processNewSection.call(this, freeSection.rank, freeSection.block, secA, secA.paranetSlot, diagnosis);
				}
				return ret;
			},
			$ruleLast: function(context, freeSection) {
				var i, len, diagnosis, ret = null;
				// Tab and layout tab => last tab
				// Tab and no layout tab ==> last stack
				// no layout tab and tab ==> last tab
				// no layout tab and no layout tab ==> last stack

				return ret;
			},
			getFreeSections: function(context, data) {
				var free = [],
					i, leni = this._$protoLayoutMap.section.length,
					scope, score = 0;
				var byPassRank = !! (data && sectionRank.byPassRank),
					section;
				context._freeBlocksBySec = [];
				for (i = 0; i < leni; i++) {
					section = this._$protoLayoutMap.section[i];
					context._freeBlocksBySec[i] = context.library.block.getFreeBlocks.call(this, i, context, data);
					if (context._freeBlocksBySec[i] == section.block.length) {
		...
		_processNewSection: function(rank, sectionRef, slot, slotRank, diag) {
			var i, len, ret;
			ret = {
				"diagnosis": diag,
				"section": sectionRef,
			...
			for (i = 0, len = sectionRef.block.length; i < len; i++) {
				_helpers._deleteFreeBinds(sectionRef.block[i], this._$freeFieldBinds);
			}
			return ret;
		},
		_processNewBlock: function(rank, blockRef, slot, slotRank, diag) {
			...
			_helpers._deleteFreeBinds(blockRef, this._$freeFieldBinds);
			return ret;
		},
