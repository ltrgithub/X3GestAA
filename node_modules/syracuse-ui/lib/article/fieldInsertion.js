			...
			for (i = freeBlocks[j].length - 1; i >= 0; i--) {
				...
				if (target) {
					...
					if (ret.success) {
						...
						// Have to remap authored layout according blocks insertion
						this._getDefinedFieldsObject(this.article.$item, $definedBinds, this._$layoutMap);
					}
				}
				// Set diagnosis
	...
	"levelArticle": {
			...
			getFreeBlocks: function(sectionRank, context, data) {
				...
				for (i = 0; i < leni; i++) {
					...
					if (score == lenj && lenj) {
						...
						// Empty block... never mind, we ca discard it, even if it's a new one!
					}
				}
		...
		"section": {
			...
			$ruleByRank: function(context, freeSection) {
				...
				if (i < lenA || (last = freeSection.rank == (authoSecRank[lenA - 1].rank + 1))) {
					insertRank = !last ? authoSecRank[i].idx : authoSecRank[lenA - 1].idx;
					secA = this._$layoutMap.section[insertRank];
		...
		_processNewSection: function(rankInMapLayoutA, rank, sectionRef, slot, slotRank, slotParentLayout, diag) {
			...
			if (lyTypeA != lyTypeR) {
				if (isTab) {
					if (this._$layoutMap.section.length == 1) {
						...
						parentLayout = slotParentLayout.$items[slotParentLayout.$items.length - 1];
						ret.layout = {
			...
			for (i = rankInMapLayoutA, len = this._$layoutMap.section.length; i < len; i++) {
				sec = this._$layoutMap.section[i];
				sec.parent[sec.parentSlot].$XID = this.page.getScreenInstFromRank(i + 1, (sec.parentLayout.$layoutType == "tabs"));
			}
			return ret;
		},
