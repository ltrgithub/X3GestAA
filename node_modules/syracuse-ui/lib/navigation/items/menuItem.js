...
function MenuItem(navParent, variant_dataset) {
	...
	this.load();
	this.mode = this.page.mode.menuItem;
	this.mode && this.mode.create(this);
...
MenuItem.prototype.load = function() {
	(this.domItem = this.navParent.listView.addAnchor(this.navParent.listView.css + "-btn-menu")).textContent = this.dataset.title;
	this.slot = this.domItem.parentNode;
		...
		for (var ii = 0, jj = _$executes.length; ii < jj; ii++) {
			...
			if ($execute) {
				...
				if (this.dataset.convergenceFunction) {
					...
					this.domItem.setAttribute("data-s-sitemap-link", this.dataset.convergenceFunction);
				} else {
					if (this.dataset.entity) {
						$execute.$description = syra_local.nvpEntity + ": " + this.dataset.entity;
						this.domItem.setAttribute("data-s-sitemap-link", this.dataset.entity);
...
MenuItem.prototype.activate = function(on) {
	this.mode && this.mode.activate && this.mode.activate(on, this);
};
MenuItem.prototype.click = function(event) {
	if (!this.$disabled) {
		...
		this.page.onNavMenuClick && this.page.onNavMenuClick();
	}
};