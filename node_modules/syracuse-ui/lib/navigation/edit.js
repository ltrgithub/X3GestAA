...
exports.EditSitemap = _helpers.defineClass(EditSitemap, NavigatorPage, {
	...
	draw: function() {
		_page = this;
		this.mode = _mode;
		this.activateOnOverDisabled = true;
		this.isMenuBarDisabled = true;
		this.mainPageDesignerAccess = "disabled";
		this.isAutoInsertFieldDisabled = true;

		this.$skin = "s-nav-edit";
		this.domItem = syra_dom.article("s-page");
		this.domItem.syraPage = this.domItem.syraItem = this.id;
		this.diagnoseSlot = syra_dom.div("", this.domItem);

		this.body = syra_dom.div("s-page-body", this.domItem);
		_cols = [];
		_groupBuilder = new GroupBuilder();
		var modules = this.$prototype.$properties.modules;
		...
		_cols.modules.tieListView(this.listView, true);
		...
		syra_localizer.onMenuClick({
			scope: {
				...
				onLocalizeField: function(newData, response, $url) {
					syra_dataset.applyDelta(_page, block.submodule.dataset.items, newData && newData.items);
					if (newData.$diagnoses) {
		...
		syra_alert.ask({
			...
			yes: function() {
				for (var ii = 0, jj = navParent.dataset.items.length; ii < jj; ii++) {
					...
					if (navParent.dataset.items[ii].menuBlock) {
