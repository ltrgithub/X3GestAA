...
exports.load = function(page) {
	...
	function _onTrackerClick() {
		...
		if (btn.parent.boundFields.mode[0].getDataValue() == "auto") {
			btn.step++;
			...
			syra_menus.button.setTitle(btn, "Stop " + test.$title + " ( " + (btn.step + 1) + "/" + test.trackers.length + ")");
			if (btn.step == 0) {
				btn.step++;
				return;
			}
			...
			if (btn.step == test.trackers.length) {
				...
				syra_menus.button.setTitle(btn, test.$title + " ( " + test.trackers.length + ")");
			}
		}
	};
