...
function _addUnitTests(page, scenarios) {
	...
	page.refreshData && page.rightCol.appendChild(syra_menus.addTextButton("Refresh Data", menuCss, "refreshData"));
	page.rightCol.appendChild(syra_menus.addTextButton("Run Unit Tests", menuCss, "runUnits"));
		...
		names.forEach(function($name) {
			...
			scenario.forEach(function(unit) {
				...
				unit.assert = function(success, message) {
					...
					this._diagnosesSlot.appendChild(syra_menus.addFontIconText(this.assertResult.message, "s-diag-field s-diag-" + $severity, $severity == "info" ? $severity : "diagnose"));
				};
...
exports.TestPage = helpers.defineClass(TestPage, DesktopPage, {
	...
	setJsonValue: function(json) {
		this.jsonField && this.jsonField.setDataValue(JSON.stringify(json, null, 2));
	},
	addJsonField: function(slot) {
		...
		this.jsonField = this.loadNewItem(box.body, {
			$bind: "$fieldJsonDelta",
			$isTitleHidden: true,
			$isEditMode: true,
			$rows: 40,
