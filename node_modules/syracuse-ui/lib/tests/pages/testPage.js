...
function _addMenus(page, $menus, list) {
	for (var ii = 0, jj = $menus.length; ii < jj; ii++) {
		var $menu = $menus[ii];
		if ($menu.$menus) {
			var box = page.addBox($menu.$title);
			list.addLine().appendChild(box.slot);
			$menu.$menus &&
				_addMenus(page, $menu.$menus, new List({
					parentNode: box.body,
					css: "s-sitemap-submodule-item"
				}));
		} else {
			if ($menu.$url) {
				var btn = list.addButton({
					parent: page,
					text: $menu.$title,
...
exports.TestPage = helpers.defineClass(TestPage, DesktopPage, {
	...
	load: function(initData) {
		...
		_addMenus(self, _map, new List({
			parentNode: self.menuBar.body,
			css: "s-sitemap-submodule-item"
		}));
