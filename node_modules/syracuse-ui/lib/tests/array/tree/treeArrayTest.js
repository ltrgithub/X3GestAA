...
exports.TreeArrayTest = helpers.defineClass(TreeArrayTest, FieldsTest, {
    ...
    applyEdit: function($actionId, delta){
        ...
        switch ($actionId) {
            case "reorder":
                if (delta.$isPartialDelta) {
                    var count = self.dataset.treexsl.length - 1;
                    delta.treexsl = self.dataset.treexsl.map(function(record, index){
                        return {
                            $uuid: record.$uuid,
                            $index: count - index
                        };
                    ...
                    delta.treexsl = self.dataset.treexsl.map(function(record){
                        return {
                            $uuid: record.$uuid
                        };
                    }).reverse();
    ...
    _appendAuthoring: function(){
        ...
        this.authoring.designedTestFields = this.boundFields["treexsl"].filter(function(item){
            return item.builder != null;
        });
        this.idMap.authoringTest.$$body.empty().append(this.authoring.layoutContent.layoutSlot)[0].style.backgroundColor = "#222222";
        this.authoring.layoutContent.layoutSlot.setAttribute("data-s-article", this.authoring.id);
    },
