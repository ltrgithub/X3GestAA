...
exports.MultiArrayTest = helpers.defineClass(MultiArrayTest, FieldsTest, {
	fillDeltaMenuGroup: function($layout) {
		for (var ii = 0, jj = $layout.$items.length; ii < jj; ii++) {
			...
			if ($item.$clientId == "$defaultDeltaModeLinks") {
				$item.$category = "menus";
				$item.$layout = deltaTests.$menus.$layout;
			} else {
				if ($item.$clientId == "$partialDeltaLinks") {
					$item.$category = "menus";
	...
	_initiliazeTestPage: function() {
		...
		document.site.deltaManager.applyObjectDelta(this, this.$prototype, this._makeArrayPrototype(), true);
	},
	loadBox: function() {
		var self = this;
	...
	_makeArrayPrototype: function() {
		...
		return {
			"$properties": {
				...
				"xarray": {
					"$title": "xarray",
					"$capabilities": "sort,filter,insert,append,delete,search",
