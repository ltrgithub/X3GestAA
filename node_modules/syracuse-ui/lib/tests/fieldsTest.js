...
exports.FieldsTest = helpers.defineClass(FieldsTest, Article, {
    ...
    onNotifyRecordChange: function(value, binding){
        ...
        switch (binding) {
            ...
            case "$field$description":
                self.applyFieldMetaData(binding, value ? "description dqdfsdhfdlsk fdqsmf,qsdmfmqsdj fmsdfmqsdf fmsdmdlsjgq" : null);
                break;
            case "$field$isDiagnosesPanelHidden":
                if (self.isDiagnosesPanelHidden = value) {
                    if (document.site._diagnosesPanel && document.site._diagnosesPanel.$$item) {
                        document.site._diagnosesPanel.$$item.hide();
                    }
                }
                break;
            ...
            default:
                if (binding.indexOf("$field") == 0) {
                    if (binding.indexOf("$field$capability") >= 0) {
                        var $capabilities = "";
                        ...
                        self.applyFieldMetaData("$capabilities", $capabilities, self.get$Properties()[binding])
                    }
                    else {
                        self.applyFieldMetaData(binding, value);
                    }
    ...
    _appendServerBar: function($prototype){
        if ($prototype) {
    ...
    _appendCapabilities: function($prototype){
        if ($prototype) {
            var $capabilities = {
            ...
            Object.keys($capabilities).forEach(function($prop){
                $prototype.$properties[$prop] = $capabilities[$prop];
            });
            ...
            Object.keys($actions).forEach(function($prop){
                $prototype.$actions[$prop] = $actions[$prop];
            });
        }
        else {
            return {
