...
exports.DiagnosesTest = helpers.defineClass(DiagnosesTest, TestPage, {
    _initiliazeTestPage: function(){
        ...
        self.$item = {
            $layout: {
                $items: ["error", "fatal", "info", "warning", "multitest"].map(function($type){
                    return {
                        $category: "section",
    ...
    loadBox: function(){
        var self = this;
        TestPage.prototype.loadBox.call(self);
		/*
        Object.keys(self.diagnoseTypes).forEach(function($type){
    ...
    _showDiagnosesInSection: function($type){
        ...
        var diagnosesPanel = document.site.appendDiagnosesPanel(document.site, true);
        ...
        if ($type == "multitest") {
            section.domTitle.textContent = "Multi diagnoses";
        }
    },
    onMenuClick: function(menu){
        var diagnoseType = this.diagnoseTypes[menu.$bind];
        if (diagnoseType) {
            document.site.showDiagnoses(this.buildDiagnose(diagnoseType));
            return false;
        }
        return true;
    },
