...
exports.MenusTreeTest = helpers.defineClass(MenusTreeTest, TestPage, {
    _initiliazeTestPage: function(){
        this.ensureDeltaManager().applyObjectDelta(this.$prototype, menuTreeSettings.$prototype, true);
    ...
    notifyActionChange: function(target, value){
        ...
        if (action && action.$bind.indexOf("menutest") == 0) {
            ...
            var newMenus = this.createNewItem(action.testcase, this);
            var layoutContent = this.idMap["$result"].layoutContent;
            if (layoutContent.items.length > 0) {
                layoutContent.newRemoveItem(layoutContent.items[0]);
            }
            layoutContent.appendNewItem({
                load: true,
                newItem: newMenus
            });
            return;
