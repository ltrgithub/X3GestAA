...
var moment = require('syracuse-ui/deps/fullcalendar/moment');
...
var pfc = $.planningFullCalendar = { version: "2.0.1" };
var pfcViews = pfc.views = {};


$.fn.planningFullCalendar = function(options) {
...
pfc.langs = langOptionHash; // expose


// Initialize jQuery UI Datepicker translations while using some of the translations
// for our own purposes. Will set this as the default language for datepicker.
// Called from a translation file.
pfc.datepickerLang = function(langCode, datepickerLangCode, options) {
...
// Called from a translation file.
pfc.lang = function(langCode, options) {
	var langOptions;
...
function Calendar(element, instanceOptions) {
	...
	t.moment = function() {
		...
		if (options.timezone === 'local') {
			mom = pfc.moment.apply(null, arguments);
			...
			mom = pfc.moment.utc.apply(null, arguments); // process as UTC
		}
		else {
			mom = pfc.moment.parseZone.apply(null, arguments); // let the input decide the zone
		}
	...
	function initialRender() {
		tm = options.theme ? 'ui' : 'pfc';
		element.addClass('pfc');
		if (options.isRTL) {
			element.addClass('pfc-rtl');
		}
		else {
			element.addClass('pfc-ltr');
		}
		if (options.theme) {
			element.addClass('ui-widget');
		}

		content = $("<div class='pfc-content' />")
			.prependTo(element);
	...
	function destroy() {
		...
		header.destroy();
		content.remove();
		element.removeClass('pfc pfc-ltr pfc-rtl ui-widget');
	}
	...
	function _changeView(newViewName) {
		...
		currentView = new pfcViews[newViewName](
			$("<div class='pfc-view pfc-view-" + newViewName + "' />")
				.appendTo(content),
	...
	function droppableDragStart(ev, ui) {
		var _e = ev.target;
		var e = $(_e);
		if (!e.parents('.pfc').length) { // not already inside a calendar
...
function PlanningHeader(calendar, options) {
	var t = this;
	...
	function render() {
		...
		if (sections) {
			var html = "<div class='pfc-planning-header'>";

			options.planningDayTypes.forEach(function(planningDayType, i) {
				html += "<span>";
				html += "<span class='pfc-planning-header-day-type-" + (i + 1) + "'/>";
				html += "<span class='pfc-planning-header-day-type'>" + planningDayType + "</span>";
				html += "</span>";
			});
...
function Header(calendar, options) {
	...
	function render() {
		...
		var sections = options.header;
		if (sections) {
			element = $("<table class='pfc-header' style='width:100%'/>")
				.append(
					$("<tr/>")
	...
	function renderSection(position) {
		var e = $("<td class='pfc-header-" + position + "'/>");
		var buttonStr = options.header[position];
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				if (i > 0) {
					e.append("<span class='pfc-header-space'/>");
				}
				var prevButton;
				$.each(this.split(','), function(j, buttonName) {
					if (buttonName == 'title') {
						e.append("<span class='pfc-header-title'><h2>&nbsp;</h2></span>");
						if (prevButton) {
						...
						if (buttonClick) {
							...
							if (customText) {
								...
								html = "<span class='pfc-icon pfc-icon-" + normalIcon + "'></span>";
							}
							else {
							...
							var button = $(
								"<span class='pfc-button pfc-button-" + buttonName + " " + tm + "-state-default'>" +
									html +
								"</span>"
								)
								.click(function() {
	...
	function activateButton(buttonName) {
		element.find('span.pfc-button-' + buttonName)
...
function EventManager(options) { // assumed to be a calendar
	...
	function _fetchEventSource(source, callback) {
		var i;
		var fetchers = pfc.sourceFetchers;
		var res;
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		var normalizers = pfc.sourceNormalizers;
		var source;
		var i;
...
function markFirstLast(e) { // TODO: use CSS selectors instead
	e.children()
		.removeClass('pfc-first pfc-last')
		.filter(':first-child')
			.addClass('pfc-first')
		.end()
		.filter(':last-child')
...
function setDayID(cell, date) {
	cell.each(function(i, _cell) {
		_cell.className = _cell.className.replace(/^pfc-\w*/, 'pfc-' + dayIDs[date.day()]);
...
function commonlyAmbiguate(inputs) {
	...
	for (i=0; i<inputs.length; i++) {
		outputs.push(pfc.moment(inputs[i]));
...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	date1 = pfc.moment.parseZone(date1);
	date2 = pfc.moment.parseZone(date2);
...
pfc.formatRange = formatRange; // expose
...
pfcViews.planningMonth = PlanningMonthView;

function PlanningMonthView(element, calendar) {
	var t = this;
	...
	// imports
    View.call(t, element, calendar, 'planningMonth');
	...
	function incrementDate(date, delta) {
		...
		if ((month < 11 && delta > 0)  || (month > 0 && delta < 0 )) {
			return date.clone().stripTime().add('months', delta).startOf('month');
		}
		return date;
	}
	...
	function render(date) {
    	...
    	var firstTime = $('div.pfc-planning-header').length === 0;
        if (firstTime) {
	    	var planningHeader = new PlanningHeader(t, calendar.options);
	    	if (planningHeader) {
	    		var planningHeaderElement = planningHeader.render();
	    		var headerElement = $('table.pfc-header');
	    		planningHeaderElement.insertBefore(headerElement);
	    	}
	    }
		...
		renderBasic(rowCnt, t.getCellsPerWeek(), true);
	...
	function disableButton(buttonName) {
		var headerElement = $('table.pfc-header');
		headerElement.find('span.pfc-button-' + buttonName)
			.addClass(tm + '-state-disabled')
			.removeClass(tm + '-state-hidden');
	}
	...
	disableTextSelection(element.addClass('pfc-grid'));
	...
	function buildEventContainer() {
		daySegmentContainer =
			$("<div class='pfc-event-container' style='position:absolute;z-index:8;top:0;left:0'/>")
				.appendTo(element);
	}
	...
	function buildTable() {
		...
		headCells = head.find('.pfc-day-header');
		body = table.find('tbody');
		bodyRows = body.find('tr');
		bodyCells = body.find('.pfc-day');
		bodyFirstCells = bodyRows.find('td:first-child');

		firstRowCellInners = bodyRows.eq(0).find('.pfc-day > div');

    	firstRowCellContentInners = bodyRows.eq(0).find('div.pfc-planning-slot-container');
    	firstRowSlotTopInners = bodyRows.eq(0).find('div.pfc-planning-slot');
		
		markFirstLast(head.add(head.find('tr'))); // marks first+last tr/th's
		markFirstLast(bodyRows); // marks first+last td's
		bodyRows.eq(0).addClass('pfc-first');
		bodyRows.filter(':last').addClass('pfc-last');
	...
	function buildTableHTML() {
		var html =
			"<table class='pfc-border-separate' style='width:100%' cellspacing='0'>" +
			buildHeadHTML() +
			buildBodyHTML() +
			"</table>";
	...
	function buildHeadHTML() {
		...
		if (showWeekNumbers) {
			html +=
				"<th class='pfc-week-number " + headerClass + "'>" +
		...
		for (col=0; col<colCnt; col++) {
			...
			html +=
				"<th class='pfc-day-header pfc-" + dayIDs[date.day()] + " " + headerClass + "'>" +
	...
	function buildBodyHTML() {
		...
		for (row=0; row<rowCnt; row++) {
			...
			html += "<tr class='pfc-week'>";

			if (showWeekNumbers) {
				date = cellToDate(row, 0);
				html +=
					"<td class='pfc-week-number " + contentClass + "'>" +
					"<div>" +
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		if (date.month() != month) {
			classNames.push('pfc-other-month');
		}
		if (date.isSame(today, 'day')) {
			...
			classNames.push('pfc-past');
		}
		else {
			classNames.push('pfc-future');
		}

        var planningSlotsPerCell = opt('planningSlotsPerCell');
		var dayContent = "<div class='pfc-planning-slot-container'>";
		var offset = 0;
		var slotWidth = 100 / planningSlotsPerCell;

    	for ( i = 0; i < planningSlotsPerCell; i++){
    		dayContent += 
	       			"<div class='pfc-planning-slot' style='left: " + offset + "%; width: " + slotWidth + "%;'>&nbsp;</div>";
    		offset += slotWidth;
    	}
		dayContent += "</div>";
		...
		if (showNumbers) {
			html += "<div class='pfc-day-number'>" + date.date() + "</div>";
		}

		html +=
			"<div class='pfc-day-content'>" + dayContent + 
				"</div>" +
				"</div>" +
				"</td>";
	...
	function setWidth(width) {
		...
		slotContentPositions.clear();

		weekNumberWidth = 0;
		if (showWeekNumbers) {
			weekNumberWidth = head.find('th.pfc-week-number').outerWidth();
		}
	...
	function renderDayOverlay(overlayStart, overlayEnd, refreshCoordinateGrid) { // overlayEnd is exclusive
		...
		for (var i=0, jj=segments.length; i<jj; i++) {
    ...
    slotContentPositions = new HorizontalPositionCache(function(col) {
        return firstRowSlotTopInners.eq(col);
    });

    function colContentLeft(col, slot) {
    	return colContentPositions.left(col) + (slot ? slotContentPositions.left(slot) : 0);
    }
    
    
    function colContentRight(col, slot) {
    	if (slot) {
			return colContentLeft(col, slot) + slotContentPositions.right(slot);
		}
	...
	function rangeToSegments(start, end, slot) {

		var rowCnt = t.getRowCnt();
		var colCnt = t.getColCnt();
		var segments = []; // array of segmentts to return

		// day offset for given date range
		//var rangeDayOffsetStart = dateToDayOffset(start);
		var rangeDayOffsetStart = dayOfYear (start.toDate());

		//var rangeDayOffsetEnd = dateToDayOffset(end); // an exclusive value
		var rangeDayOffsetEnd = dayOfYear (end.toDate());
    ...
    // the number of days from the start of the year (base 1)
    function dayOfYear(nativeDate) {
		var timeDiff = Math.abs(t.start.toDate().getTime() - nativeDate.getTime());
		return Math.ceil(timeDiff / (1000 * 3600 * 24));
    }
}

pfcViews.planningYear = PlanningYearView;
function PlanningYearView( element, calendar )
{
    var t = this;
    ...
    // imports
    View.call(t, element, calendar, 'planningYear');
    OverlayManager.call(t);
    SelectionManager.call(t);
    ...
    var bodyRows;
    var bodyCells;
    var bodyRowHeadCells;
    var bodyFirstCells;
    var bodyCellTopInners;
    var bodySlotTopInners;
    var daySegmentContainer;
    var firstRowCellInners;
	...
	function daySelectionMousedown(ev) { // not really a generic manager method, oh well
		...
		var _hoverListener = t.getHoverListener();
		var reportDayClick = t.reportDayClick; // this is hacky and sort of weird

		if (ev.which == 1 && opt('selectable')) { // which==1 means left mouse button
			t.unselect(ev);
			var dates;
			_hoverListener.start(function(cell, origCell) { // TODO: maybe put cellToDate/getIsCellAllDay info in cell
				clearSelection();
	...
	function cellToDate() {
		...
		var date = t.dayOffsetToDate(dayOffset - 1);
		return date;
	}
	...
	function cellToCellOffset(row, col) {
		...
		// month is 0 - 11
		// date of month is 1 - 31
		var cellOffset = t.visStart.clone().month(row).date(col+1).dayOfYear();
		return cellOffset;
	}
    ...
    function render( date, delta )
    {
        ...
        var end = start.clone().add('years', 1);
        ...
        t.yearMoment = visStart.clone(); // used a temporary moment to avoid the need for performance-sapping moment clones.
        thisYear = t.yearMoment.year();
    ...
    function buildSkeleton( maxRowCnt, showNumbers )
    {
		...
		dayContent.push("<div class='pfc-planning-slot-container'>");
    	for ( i = 0; i < planningSlotsPerCell; i++){
    		dayContent.push("<div class='pfc-planning-slot' style='left: ");
    		dayContent.push(offset);
    		dayContent.push("%; width: ");
    		dayContent.push(slotWidth);
    		dayContent.push("%;'>&nbsp;</div>");
    		offset += slotWidth;
    	}
		dayContent.push("</div>");
		dayContent = dayContent.join("");
        
        s.push("<table class='pfc-border-separate pfc-skeleton pfc-skeleton-year-vertical' style='width:100%' cellspacing='0'>");
        
        for (i = 0; i < colCnt; i++) {
            s.push("<colgroup class='col");
            s.push(i);
            s.push("'></colgroup>");
        }

        s.push("<thead><tr><th></th>");
        ...
        for (i = 0; i < maxRowCnt; i++) {
            s.push("<tr class='pfc-date");
            s.push(i + 1);
            s.push("'>");
            s.push('<th class="pfc-month-header"></th>');
            ...
            for (j = 0; j < colCnt; j++)
            {
                ...
                if (showNumbers)
                	s.push("<div class='pfc-day-number'/>");

				s.push("<div class='pfc-day-content'>");
				s.push(dayContent);
				s.push("</div></div></td>");
            }
            s.push("</tr>");
    	...
    	bodyCellTopInners = bodyRows.eq(0).find('div.pfc-planning-slot-container');
    	bodySlotTopInners = bodyRows.eq(0).find('div.pfc-planning-slot');

        firstRowCellInners = bodyRows.eq(0).find('.pfc-day > div');
        
        markFirstLast(head.add(head.find('tr'))); // marks first+last tr/th's
        markFirstLast(bodyRows); // marks first+last td's
        bodyRows.eq(0).addClass('pfc-first'); // pfc-last is done in updateCells
        bodyCells.each(function(i, _cell) {
            var index = parseInt( _cell.className.match( /fc\-num(\d+)/ )[ 1 ], 10);
    ...
    function updateCells( firstTime, showNumbers )
    {
        ...
        if (dowDirty)
        {
            headCells.each(function(i, _cell)
            {
                if (i > 0) {
                    ...
                    date = t.visStart.stripTime().date(i);
        ...
        bodyRowHeadCells.each( function( i, _cell ) {
            cell = $( _cell );
        	date = t.visStart.stripTime().month(i).date(1);
        ...
        bodyCells.each( function( i, _cell )
        {
            ...
            if ( !date )
            {
                cell.find( 'div.pfc-day-number' ).text( '' );
                cell.addClass( 'pfc-empty' );
            }
            else
            {
                if ( +date == +today ) {
                    cell.addClass(tm + '-state-highlight pfc-today');
                }else{
                    cell.removeClass(tm + '-state-highlight pfc-today');
                }
                if (showNumbers) {
                    cell.find( 'div.pfc-day-number' ).text( date.getDate());
                }
        ...
        bodyRows.each(function(i, _row)
        {
            ...
            if (i < rowCnt) {
                row.show();
                if (i == rowCnt-1) {
                    row.addClass('pfc-last');
                }else{
                    row.removeClass('pfc-last');
                }
            }else{
                row.hide();
            }
    ...
    function indexDate( index )
    {
        ...
        //      @see    daysInMonth()
        var nb_days = daysInMonth( current_row, t.visStart.get('year'));//getFullYear());
        
        //      Not a valid cell (e.g. rows > 28 or 29 in february)
        if ( current_col >= nb_days )
            return false;
        else {
            return t.visStart.month(current_row).date(current_col + 1);
        }
    }
    ...
    function dayClick( ev )
    {
        if ( !opt( 'selectable' ))
        {
            var index = parseInt( this.className.match( /pfc\-num(\d+)/ )[ 1 ], 10);
            var date = indexDate( index - 1);
            if (date)
    ...
    function daysInMonth( month, year ) {
        ...
        return t.visStart.clone().year(year).month(month).endOf('month').date();
    }
    ...
    function renderSelection(startDate, endDate, allDay) {
    	renderDayOverlay(startDate, startDate, true);
    }
    ...
    slotContentPositions = new HorizontalPositionCache(function(col) {
        return bodySlotTopInners.eq(col);
    });
    ...
    function dateCell(date) {
        return {
            row: Math.floor(date.from(t.visStart) / 7),
            col: date.day() //  dayOfWeekCol(date.getDay())
        };
    }
    ...
    // - isEnd
    //
    function rangeToSegments(startDate, endDate, slot) {
        var _rowCnt = t.getRowCnt();
        //var colCnt = t.getColCnt();
        var segments = []; // array of segments to return

        // The number of days from the beginning of the year.
        // We're only looking at single day events here, so no need to do anything with multi-day events.
        if (t.visStart.year() != startDate.year())
        	return segments;

		var eventDayOfYear = dayOfYear (startDate.toDate());

         // loop through all the rows in the view
        for (var row=0; row<_rowCnt; row++) {

            // get the day offset for the 1st day of this month (row) from the 1st of January.
            t.yearMoment.toDate().setMonth(row, 1);
            var firstDayOfYearForMonth = dayOfYear(t.yearMoment.toDate());
			
            // get the day offset for the last day of this month (row) from the 1st of January.
			t.yearMoment.toDate().setMonth(row+1, 0);
            var lastDayOfYearForMonth = dayOfYear(t.yearMoment.toDate());

        	if (firstDayOfYearForMonth <= eventDayOfYear && eventDayOfYear <= lastDayOfYearForMonth) {

                var segmentCellFirst = cellOffsetToCell(eventDayOfYear); 

                var cols = [ segmentCellFirst.col, segmentCellFirst.col ].sort();
    ...
    function cellOffsetToCell(cellOffset) {
		...
		for (var i = 0; i < 12; i +=1) {
		    var _firstDay = (Math.ceil((new Date(thisYear, i, 1) - new Date(thisYear, 0, 1)) / 86400000));
		    if (_firstDay - 1  < cellOffset) {
		        adjustment += (31 - new Date(thisYear, i, 0).getDate());
		    }
		}
        
	    cellOffset += adjustment;

	    var _colCnt = t.getColCnt();

	    // rtl variables. wish we could pre-populate these. but where?
	    var _dis = isRTL ? -1 : 1;
	    var _dit = isRTL ? colCnt - 1 : 0;

	    var row = Math.floor(cellOffset / _colCnt);
	    var col = ((cellOffset % _colCnt + _colCnt) % _colCnt) * _dis + _dit; // column, adjusted for RTL (dis & dit)
...
} // end of PlanningYearView


pfcViews.basicWeek = BasicWeekView;
...
pfcViews.basicDay = BasicDayView;
...
function BasicView(element, calendar, viewName) {
	...
	function buildTable() {
		...
		firstRowCellInners = bodyRows.eq(0).find('.pfc-day > div');
		firstRowCellContentInners = bodyRows.eq(0).find('.pfc-day-content > div');
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		var dayContent;
        if (opt('planningCalendar')) {
	        var planningSlotsPerCell = opt('planningSlotsPerCell');
    		var offset = 0;
    		var slotWidth = 100 / planningSlotsPerCell;

			dayContent = "<div class='pfc-planning-slot-container'>";

        	for (var i = 0; i < planningSlotsPerCell; i++){
        		dayContent += 
 	       			"<div class='pfc-planning-slot' style='left: " + offset + "%; width: " + slotWidth + "%;'>&nbsp;</div>";
        		offset += slotWidth;
        	}
		...
		html +=
			"<div class='pfc-day-content'>" +

            (opt('planningCalendar') ?
                	dayContent :
...
pfcViews.agendaWeek = AgendaWeekView;
...
pfcViews.agendaDay = AgendaDayView;
...
function AgendaView(element, calendar, viewName) {
	...
	disableTextSelection(element.addClass('pfc-agenda'));
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			s =
				"<table style='width:100%' class='pfc-agenda-allday' cellspacing='0'>" +
				"<tr>" +
				"<th class='" + headerClass + " pfc-agenda-axis'>" +
				(
					opt('allDayHTML') ||
				...
				"</th>" +
				"<td>" +
				"<div class='pfc-day-content'><div style='position:relative'/></div>" +
				"</td>" +
				"<th class='" + headerClass + " pfc-agenda-gutter'>&nbsp;</th>" +
				"</tr>" +
				"</table>";
			...
			slotLayer.append(
				"<div class='pfc-agenda-divider " + headerClass + "'>" +
				"<div class='pfc-agenda-divider-inner'/>" +
				"</div>"
			);
		...
		s =
			"<table class='pfc-agenda-slots' style='width:100%' cellspacing='0'>" +
			"<tbody>";
		...
		while (slotTime < maxTime) {
			...
			s +=
				"<tr class='pfc-slot" + slotCnt + ' ' + (!minutes ? '' : 'pfc-minor') + "'>" +
				"<th class='pfc-agenda-axis " + headerClass + "'>" +
				((!slotNormal || !minutes) ?
	...
	function buildDayTable() {
		...
		dayBodyCellContentInners = dayBodyCells.find('.pfc-day-content > div');
	...
	function buildDayTableHTML() {
		var html =
			"<table style='width:100%' class='pfc-agenda-days pfc-border-separate' cellspacing='0'>" +
	...
	function buildDayTableHeadHTML() {
		...
		if (opt('weekNumbers')) {
			...
			html +=
				"<th class='pfc-agenda-axis pfc-week-number " + headerClass + "'>" +
				htmlEscape(weekText) +
				"</th>";
		}
		else {
			html += "<th class='pfc-agenda-axis " + headerClass + "'>&nbsp;</th>";
		}

		for (col=0; col<colCnt; col++) {
			date = cellToDate(0, col);
			html +=
				"<th class='pfc-" + dayIDs[date.day()] + " pfc-col" + col + ' ' + headerClass + "'>" +
				htmlEscape(formatDate(date, colFormat)) +
				"</th>";
		}

		html +=
			"<th class='pfc-agenda-gutter " + headerClass + "'>&nbsp;</th>" +
			"</tr>" +
			"</thead>";
	...
	function buildDayTableBodyHTML() {
		...
		html +=
			"<tbody>" +
			"<tr>" +
			"<th class='pfc-agenda-axis " + headerClass + "'>&nbsp;</th>";
		...
		html += cellsHTML;
		html +=
			"<td class='pfc-agenda-gutter " + contentClass + "'>&nbsp;</td>" +
			"</tr>" +
			"</tbody>";
	...
	function setWidth(width) {
		...
		var gutterCells = dayTable.find('.pfc-agenda-gutter');
		if (allDayTable) {
			gutterCells = gutterCells.add(allDayTable.find('th.pfc-agenda-gutter'));
		}
	...
	function slotClick(ev) {
		if (!opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
			...
			var match = this.parentNode.className.match(/pfc-slot(\d+)/); // TODO: maybe use data
			if (match) {
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if ((eventElement = seg.element)) {
				...
				titleElement = eventElement.find('.pfc-event-title');
		...
		for (i=0; i<segCnt; i++) {
			...
			if ((eventElement = seg.element)) {
				...
				if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
					...
					eventElement.find('div.pfc-event-time')
						.text(
							formatDate(event.start, opt('timeFormat')) + ' - ' + event.title
						);
					eventElement.find('div.pfc-event-title')
						.remove();
				}
	...
	function slotSegHtml(event, seg) {
		...
		var classes = ['pfc-event', 'pfc-event-vert'];
		if (isEventDraggable(event)) {
			classes.push('pfc-event-draggable');
		}
		if (seg.isStart) {
			classes.push('pfc-event-start');
		}
		if (seg.isEnd) {
			classes.push('pfc-event-end');
		}
		...
		html +=
			...
			" style=" +
				...
				"'" +
			">" +
			"<div class='pfc-event-inner'>" +
			"<div class='pfc-event-time'>" +
			htmlEscape(t.getEventTimeText(event)) +
			"</div>" +
			"<div class='pfc-event-title'>" +
			htmlEscape(event.title || '') +
			"</div>" +
			"</div>" +
			"<div class='pfc-event-bg'></div>";
	...
	function bindSlotSeg(event, eventElement, seg) {
		var timeElement = eventElement.find('div.pfc-event-time');
		if (isEventDraggable(event)) {
...
function DayEventRenderer() {
	...
	function _renderDayEvents(events, doAppend, doRowHeights) {
		...
		setVerticals(segments, opt('planningCalendar') ? 0 : doRowHeights);
	...
	function buildSegmentsForEvent(event) {
		var segments = rangeToSegments(event.start, getEventEnd(event), event.slot);
		for (var i=0; i<segments.length; i++) {
	...
	function calculateHorizontals(segments) {
		...
		for (var i=0; i<segments.length; i++) {
			...
			var right = rightFunc(segment.rightCol/*, segment.slot*/);

			if (opt('planningCalendar') && planningSlotsPerCell > 1) {
				var slotWidth = (right - left) / planningSlotsPerCell;
				left += slotWidth * segment.slot;
				right = left + slotWidth; 
			}

			segment.left = left;
			segment.outerWidth = Math.round(right - left); 
		}
	}
	...
	function buildHTMLForSegment(segment) {
		var html = '';
		var isRTL = opt('isRTL');
		var planningCalendar = opt('planningCalendar');

		var event = segment.event;
		var url = event.url;

		// generate the list of CSS classNames
		var classNames = [ 'pfc-event', 'pfc-event-hori' ];
		if (isEventDraggable(event)) {
			classNames.push('pfc-event-draggable');
		}
		if (segment.isStart) {
			classNames.push('pfc-event-start');
		}
		if (segment.isEnd) {
			classNames.push('pfc-event-end');
		}
		...
		if (planningCalendar) {
			html += "<div class='pfc-planning-event-slot-type-" + event.dayType /*+ " pfc-event-calendar-type-" + t.name*/ + "'>";
			html += "<div class='pfc-planning-event-inner-" + event.slotType + "'>"; 
			html += "</div>";
		}else{
			html += "<div class='pfc-event-inner'>";
		}


		if (!event.allDay && segment.isStart) {
			html +=
				"<span class='pfc-event-time'>" +
				htmlEscape((planningCalendar ? '' : t.getEventTimeText(event))) +
				"</span>";
		}

		html +=
				"<span class='pfc-event-title'>" +
				htmlEscape(planningCalendar ? '' : (event.title || '')) +
				"</span>" +
				"</div>";
	...
	function setVerticals(segments, doRowHeights) {
		...
		segmentElementEach(segments, function(segment, element) {
			element.css(
				'top',
				rowContentTops[segment.row] + (opt('planningCalendar') ? -1 : segment.top) // ...now, relative to views's origin
			);
		});
	}
...
function OverlayManager() {
	...
	function renderOverlay(rect, parent) {
		...
		if (!e) {
			e = $("<div class='pfc-cell-overlay' style='position:absolute;z-index:3'/>");
		}
