...
function PlanningHeader(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			var html = "<div class='fc-planning-header'>";

			options.planningDayTypes.forEach(function(planningDayType, i) {
				html += "<span class='fc-planning-header-day-type'>" + planningDayType;
				html += "<span class='fc-planning-header-day-type-" + (i + 1) + "'></span>";
				html += "</span>";
			});
...
function setDayID(cell, date) {
	cell.each(function(i, _cell) {
		_cell.className = _cell.className.replace(/^fc-\w*/, 'fc-' + dayIDs[date.day()]);
...
fcViews.planningMonth = PlanningMonthView;
...
function PlanningMonthView(element, calendar) {
	...
	function disableButton(buttonName) {
		var headerElement = $('table.fc-header');
		headerElement.find('span.fc-button-' + buttonName)
			.addClass(tm + '-state-disabled')
	...
	function buildTable() {
    	...
    	firstRowCellContentInners = bodyRows.eq(0).find('div.fc-planning-slot-container');
    	firstRowSlotTopInners = bodyRows.eq(0).find('div.fc-planning-slot');
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		var dayContent = "<div class='fc-planning-slot-container'>";
		var offset = 0;
		var slotWidth = 100 / planningSlotsPerCell;

    	for ( i = 0; i < planningSlotsPerCell; i++){
    		dayContent += 
	       			"<div class='fc-planning-slot' style='left: " + offset + "%; width: " + slotWidth + "%;'>&nbsp;</div>";
    		offset += slotWidth;
    	}
		dayContent += "</div>";
		...
		html +=
			"<div class='fc-day-content'>" + dayContent + 
				"</div>" +
				"</div>" +
				"</td>";
...
fcViews.planningYear = PlanningYearView;
function PlanningYearView( element, calendar )
{
    ...
    function buildSkeleton( maxRowCnt, showNumbers )
    {
		...
		dayContent.push("<div class='fc-planning-slot-container'>");
    	for ( i = 0; i < planningSlotsPerCell; i++){
    		dayContent.push("<div class='fc-planning-slot' style='left: ");
        ...
        s.push("<table class='fc-border-separate fc-skeleton fc-skeleton-year-vertical' style='width:100%' cellspacing='0'>");
        ...
        for (i = 0; i < maxRowCnt; i++) {
            s.push("<tr class='fc-date");
            s.push(i + 1);
            s.push("'>");
            s.push('<th class="fc-month-header"></th>');
            ...
            for (j = 0; j < colCnt; j++)
            {
                ...
                if (showNumbers)
                	s.push("<div class='fc-day-number'/>");

				s.push("<div class='fc-day-content'>");
				s.push(dayContent);
				s.push("</div></div></td>");
            }
            s.push("</tr>");
    	...
    	bodyCellTopInners = bodyRows.eq(0).find('div.fc-planning-slot-container');
    	bodySlotTopInners = bodyRows.eq(0).find('div.fc-planning-slot');
    ...
    function dayClick( ev )
    {
        if ( !opt( 'selectable' ))
        {
            var index = parseInt( this.className.match( /fc\-num(\d+)/ )[ 1 ], 10);
            var date = indexDate( index - 1);
            if (date)
...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) { // date assumed to have stripped time
        ...
        if (opt('planningCalendar')) {
			...
			dayContent = "<div class='fc-planning-slot-container'>";
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		if (planningCalendar) {
			html += "<div class='fc-planning-event-slot-type-" + event.dayType /*+ " fc-event-calendar-type-" + t.name*/ + "'>";
			html += "<div class='fc-planning-event-inner-" + event.slotType + "'>"; 
			html += "</div>";
		}else{
