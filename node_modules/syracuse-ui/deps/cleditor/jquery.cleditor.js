...
(function($) {
	...
	$.cleditor = {
		...
		defaultOptions: {
			...
			"Arial,Arial Black,Arial Unicode MS,Comic Sans MS,Courier New,Narrow,Garamond," + "Georgia,Impact,MS Sans Serif,Sans Serif,Serif,Tahoma,Trebuchet MS,Verdana",
	...
	//==================
	var // Misc constants
		BACKGROUND_COLOR = "backgroundColor", BUTTON = "button", BUTTON_NAME = "buttonName", CHANGE = "change", CLEDITOR = "cleditor", CLICK = "click", DISABLED = "disabled", DIV_TAG = "<div>", TRANSPARENT = "transparent", UNSELECTABLE = "unselectable", // Class name constants
		...
		// Browser detection
		ua = navigator.userAgent.toLowerCase(), ie = /msie/.test(ua), ieVer = parseFloat(ie && ua.match(/msie\s([0-9\.]+)/)[1] || 6.0), ie6 = /msie\s6/.test(ua), webkit = /webkit/.test(ua), // Test for iPhone/iTouch/iPad
		iOS = /iphone|ipad|ipod/i.test(ua), // Popups are created once as needed and shared by all editor instances
		popups = {}, // Used to prevent the document click event from being bound more than once
	...
	// Public Methods
	//===============
	var fn = cleditor.prototype, // Expose the following private functions as methods on the cleditor object.
		// The closure compiler will rename the private functions. However, the
		// exposed method names on the cleditor object will remain fixed.
		methods = [["clear", clear], ["disable", disable], ["execCommand", execCommand], ["focus", focus], ["hidePopups", hidePopups], ["sourceMode", sourceMode, true], ["refresh", refresh], ["select", select], ["selectedHTML", selectedHTML, true], ["selectedText", selectedText, true], ["showMessage", showMessage], ["updateFrame", updateFrame], ["updateTextArea", updateTextArea]];
			...
			if (!sourceMode(editor)) {
				...
				// Print
					...
					// All other buttons
						...
						if (!execCommand(editor, data.command, data.value, data.useCSS, buttonDiv))
							return false;
	...
	function execCommand(editor, command, value, useCSS, button) {
		...
		if (ie && command.toLowerCase() == "inserthtml") {
			if (ieVer < 9.0) {
				...
				var sel = getSelection(editor), doc = editor.doc, range;
				if (sel.getRangeAt) {
					if (sel.rangeCount) {
	...
	function getSelection(editor) {
		if (ie && ieVer < 9.0)
			return editor.doc && editor.doc.selection;
		var elt = editor.sourceMode() ? document : editor.$frame[0].contentWindow, sel;
		if (elt && elt.getSelection) {
			sel = elt.getSelection();
		}
		if (!sel && elt && elt.selection)
	...
	function refresh(editor) {
		...
		var doc = editor.doc = contentWindow.document, $doc = $(doc);
		...
		$.each(editor.$toolbar.find("." + BUTTON_CLASS), function(idx, elem) {
			...
			if (editor.disabled)
				...
				if (button.getEnabled) {
					...
					if (((inSourceMode || iOS) && button.name != "source") || (ie && (command == "undo" || command == "redo")))
						enabled = false;
					else
	...
	function showPopup(editor, popup, button) {
		...
		if (syra_quality.enableTestRobot) {
			return;
		}
	...
	function updateTextArea(editor, checkForChange) {
		...
		if (html === '<p>&nbsp;</p>' || html === '<pre>&nbsp;</pre>' || html.match(/^<p><span style="font-size: \d{1,3}\pt;"><\/span><\/p>$/g) ||
			html.match(/^<p><span style="font-size: \d{1,3}\pt;">&nbsp;<\/span><\/p>$/g) ||
			html.match(/^<p><span style="font-size: \d{1,3}\pt;"><\/span>&nbsp;<\/p>$/g)) {
			html = "";
