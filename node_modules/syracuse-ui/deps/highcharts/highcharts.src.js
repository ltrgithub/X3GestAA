...
(function(root, factory) {
		...
		initPath: function(elem, fromD, toD) {
			...
			if (start.length) {
				...
				while (start.length < endLength) {
					...
					[].splice.apply(
						start, [(start.length / positionFactor), 0].concat(slice)
					);
	...
	function animObject(animation) {
		return isObject(animation) ? merge(animation) : {
	...
	Chart.prototype = {
		...
		layOutTitles: function(redraw) {
			...
			if (subtitle) {
				subtitle
					.css({
						width: (subtitleOptions.width || spacingBox.width + subtitleOptions.widthAdjust) + PX
					})
					.align(extend({
