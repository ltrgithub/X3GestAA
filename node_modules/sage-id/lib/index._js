...
var __tracer = function() {
	[].unshift.call(arguments, (new Date()).toISOString());
...
exports.create = function(options) {
	return new function() {
		...
		var signOnStart = function(request, response, session, _) {
			...
			__tracer("SageID signOnStart: ", request.url, request.headers);
		...
		var signOnSuccess = function(request, response, session, _) {
			...
			__tracer("SageID signOnSuccess: ", request.url, request.headers);
...
function client(_, options, action, postData) {
	...
	__tracer("SageID request: options", trc, action, postData);
	...
	if (resp.statusCode !== 200) {
		var body2 = resp.readAll(_);
		// console.error((new Date()).toISOString(), "Bad response from SageId", clientOptions, body2);
		throw new Error("Authentication failed: " + action + " - " + resp.statusCode + " from " + options.baseUrl);
	}

	// Read all data from Sage ID response
	var body = resp.readAll(_);

	__tracer("SageID response: ", body);
...
function insertSessionNotification(_, host, sessionId) {
	function _f(_, host, sessionId) {
		...
		try {
			...
			__tracer("SageID insert notification error:", e.message);
		} finally {
			var elapsed = t0 - Date.now();
			__tracer("SageID insert notification elapsed:", elapsed, "ms");
		}
	}
	_f(_, host, sessionId);
}
		...
		try {
			...
			if (doc) {
				...
				if (doc.hosts.length === 1)
					db.collection(mongoConfig.sageIdCollection, _).remove({
						_id: sessionId
					}, null, _);
				else {
					...
					db.collection(mongoConfig.sageIdCollection, _).update({
						_id: sessionId
					}, {
						$set: {
							hosts: doc.hosts
			...
			__tracer("SageID delete notification error:", e.message);
		} finally {
			var elapsed = t0 - Date.now();
			__tracer("SageID delete notification elapsed:", elapsed, "ms");
		}
	}
	_f(_, host, sessionId);
}
...
function initializeMongoDB(mongoHost, mongoPort, mongoDatabase) {
	...
	var opt = (config.mongoNotify || {}).options || {
		db: {
			w: 1
		}
	};
	if (hostArray.length > 1) {
		...
		var replSet = new mongodb.ReplSet(servers, opt.replSet);
