...
exports.create = function(options) {
	return new function() {
		...
		var signOnSuccess = function(request, response, session, _) {
			...
			var sessionExpire = json.EndSignOnAttemptResponse.SuccessResult.SessionExpiry;

			var tokenData = {};
			tokenData.access_token = 'sageid' + accessToken;
			tokenData.expirationDate = new Date(sessionExpire);
			tokenData.retrievalUrl = (config.hosting.https ? "https://" : "http://") + request.headers.host + config.sage_id.oauth.retrieveTokenPath + '/' + sessionId;
			storeAccessToken(_, sessionId, tokenData);

			// Store response in session, for future use
			session.EndSignOnAttemptResponse = json.EndSignOnAttemptResponse.SuccessResult;
			// For testing session expiration
			/*var fs = require('fs');
			fs.writeFile("C:/Users/gsparedes/Desktop/Sage_Notify/test.txt", sessionId, function(err) {
			    if(err) {
			        console.log(err);
			    } else {
			...
			insertSessionNotification(_, request.headers.host, sessionId);
		...
		this.logout = function(request, response, session, _) {
			...
			deleteSessionNotification(_, request.headers.host, sessionId);
		...
		var notificationRoute = function(request, response, session, _) {
			// Read request body and decode from Base64 format
			// Data is decoded to XML
			var reqBody = request.readAll(_);
			var buf = new Buffer(reqBody, 'base64');
			...
			if (!notification) {
				...
				// Initialize mongoDB and route notification to respected host
				var db = initializeMongoDB();
				// Find customer host by sessionId extract from Sage ID notification
				var doc = db.open(_).collection('Notifications', _).find({ _id: sessionId }, _).toArray(_)[0];
				db.close(_);
				var hosts = doc.hosts;

				for (var i = 0; i < hosts.length; i++) {
					var host = hosts[i];
				// Encode xml in base64 to route notification data
				var encodeXML = new Buffer(xml).toString('base64');
				var clientOptions = {
					method: "POST",
					host: host,
					port: "80",
					path: "/auth/sage-id/notifyRouted",
					headers: {
						'Content-Type': 'text/xml',
						'Content-Length': Buffer.byteLength(encodeXML)
					}
				};
				var httpClient = options.httpClient || require("ez-streams").devices.http.client;
				// Route request to respected customer with notification data
				var req = options.httpClient ? httpClient(_, clientOptions) : httpClient(clientOptions);
				req.end(new Buffer(encodeXML));
				};
			} else {
		...
		var routes = {
			...
			// Redirect for notification routing of Sage ID
...
function insertSessionNotification(_, host, sessionId) {
	...
	db.open(_).collection('Notifications', _).update({ _id: sessionId }, { $push: { hosts: host } }, { upsert: true }, _);
	db.close(_);
}
...
function deleteSessionNotification(_, host, sessionId) {
	...
	if (doc.hosts.length === 1)
		db.open(_).collection('Notifications', _).remove({ _id: sessionId }, null, _);
	else {
		// Delete respected host object
		var index = doc.hosts.indexOf(host);
		doc.hosts.splice(index, 1);
		// Update doc after removing host value
		db.open(_).collection('Notifications', _).update({ _id: sessionId }, { $set: { hosts: doc.hosts } }, _);
	}
	db.close(_);
}
...
function tokenValidation(_, request, response, accessToken) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	var filter = {
	...
	if (!token)
		throw authHelper.error(401, "Invalid access token");
	db.close(_);
	...
	if (valid)
		...
		throw authHelper.error(401, "Access token no longer valid, please re-authenticate");
}
