...
var mongoConfig = config.mongoNotify || {
	host: config.collaboration.hostname,
	port: config.collaboration.port,
	database: config.collaboration.databaseName
};
	...
	return new function() {
		...
		var notificationRoute = function(request, response, session, _) {
			...
			if (!notification) {
				...
				var doc = db.open(_).collection('Notifications', _).find({
					_id: sessionId
				}, _).toArray(_)[0];
				db.close(_);
				var hosts = doc.hosts;
...
function insertSessionNotification(_, host, sessionId) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	db.open(_).collection('Notifications', _).update({
		_id: sessionId
	}, {
		$push: {
			hosts: host
...
function deleteSessionNotification(_, host, sessionId) {
	...
	if (doc.hosts.length === 1)
		db.open(_).collection('Notifications', _).remove({
			_id: sessionId
		}, null, _);
	else {
...
function initializeMongoDB(mongoHost, mongoPort, mongoDatabase) {
	...
	return new mongodb.Db(mongoDatabase, server, opt.db);
}
