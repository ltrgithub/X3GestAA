...
function initializeMongoDB(mongoHost, mongoPort, mongoDatabase) {
	var server = new mongodb.Server(mongoHost || 'localhost', mongoPort || 27017, config.mongodb.options.server);
	var db = new mongodb.Db(mongoDatabase, server, config.mongodb.options.db);
	return db;
}