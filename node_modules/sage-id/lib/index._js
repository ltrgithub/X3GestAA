...
exports.create = function(options) {
	return new function() {
		...
		this.authenticate = function(request, response, session, _) {
			...
			var signOnOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebStartSignOnAttempt",
			...
			if (options.httpClient)
				req = httpClient(_,signOnOptions);
			else
				req = httpClient(signOnOptions)
			// Write post data to request
			req.end(new Buffer(postData));

			// Initialize response from Sage ID
			var resp = req.response(_);
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from StartSignOnAttempt: " + resp.statusCode);

			// Read all data from Sage ID response
			var body = resp.readAll(_);
			// Convert xml response to JSON
			var json = jsxml.parse(body);
		...
		this.dispatch = function(request, response, session, _) {
			var fs = request.url.substring(options.prefix.length).split('/')[0];
		...
		this.closeAuthenticate = function(request, response, session, _) {
			...
			var postData = "<EndSignOnAttemptRequest>"
								+"<ResultId xmlns=\"http://sso.sage.com\">"
									+resultId
								+"</ResultId>"
							+"</EndSignOnAttemptRequest>";

			// Initialize end sign on options
			var endSignOnOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebEndSignOnAttempt",
				pfx: options.pfx_object,
				passphrase: options.pass,
				headers: {
			      'Content-Type': 'text/xml',
			...
			var req = "";
			if (options.httpClient)
				req = httpClient(_, endSignOnOptions);
			else
				req = httpClient(endSignOnOptions)
			// Write post data to request
			req.end(new Buffer(postData));
			...
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from EndSignOnAttempt: " + resp.statusCode);
			...
			console.log("End sign on Successful")
		...
		this.signup = function(request, response, session, _) {
			...
			var postData = "<StartNewUserRegistrationAttemptRequest>"
								...
								+"<State xmlns=\"http://sso.sage.com\">"+options.StartNewUserRegistrationAttemptRequest.State+"</State>"
								+"<SessionLengthMinutes xmlns=\"http://sso.sage.com\">"+options.StartNewUserRegistrationAttemptRequest.SessionLengthMinutes+"</SessionLengthMinutes>"
			...
			var signUpOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebStartNewUserRegistrationAttempt",
				pfx: options.pfx_object,
				passphrase: options.pass,
				headers: {
			      'Content-Type': 'text/xml',
			...
			var req = "";
			if (options.httpClient)
				req = httpClient(_, signUpOptions);
			else
				req = httpClient(signUpOptions)
			// Write post data to request
			req.end(new Buffer(postData));
			...
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from StartNewUserRegistrationAttempt: " + resp.statusCode);
		...
		this.closeSignUp = function(request, response, session, _) {
			...
			var endSignUpOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebEndNewUserRegistrationAttempt",
				pfx: options.pfx_object,
				passphrase: options.pass,
				headers: {
			      'Content-Type': 'text/xml',
			...
			var req = "";
			if (options.httpClient)
				req = httpClient(_, endSignUpOptions);
			else
				req = httpClient(endSignUpOptions)
			// Write post data to request
			req.end(new Buffer(postData));
			...
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from EndRegistrationAttempt: " + resp.statusCode);
			...
			console.log("End sign up Successful")
			// Create json containing email and unique identityId
			var sageIdData = {
				email: email,
				identityId: identityId,
		...
		this.logout = function(request, response, session, _) {
			...
			var signOutOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebSessionSignOff",
				pfx: options.pfx_object,
				passphrase: options.pass,
				headers: {
			      'Content-Type': 'text/xml',
			...
			var req = "";
			if (options.httpClient)
				req = httpClient(_, signOutOptions);
			else
				req = httpClient(signOutOptions)
			// Write post data to request
			req.end(new Buffer(postData));
			...
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from SessionSignOffAttempt: " + resp.statusCode);
		...
		this.sessionExtend = function(request, response, session, _) {
			...
			var extendOptions = {
				method: "POST",
				url: options.sageIdBase + "/WebSSOService/web/WebSessionExtend",
				pfx: options.pfx_object,
				passphrase: options.pass,
				headers: {
			      'Content-Type': 'text/xml',
			...
			var req = "";
			if (options.httpClient)
				req = httpClient(_, extendOptions);
			else
				req = httpClient(extendOptions)
			// Write post data to request
			req.end(new Buffer(postData));
			...
			console.log(resp.statusCode)
			// Check to ensure response comes back OK
			if (resp.statusCode !== 200) throw new Error("bad response from SessionExtendAttempt: " + resp.statusCode);
		...
		this.sessionEnd = function(request, response, sessionId, _) {
...
// Simple function to pad single number with 0 if between 1 and 9
function pad(number) {
  var r = String(number);
  if ( r.length === 1 ) {
    r = '0' + r;
...
// Simple function to turn date into ISO date string
