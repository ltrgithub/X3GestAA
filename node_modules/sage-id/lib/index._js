	...
	return new function() {
		...
		var notificationRoute = function(request, response, session, _) {
			...
			if (!notification) {
				...
				var doc = db.open(_).collection('Notifications', _).find(filter, _).toArray(_)[0];
				var host = doc.host;
...
exports.deleteSessionNotification = function(_, sessionId) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	db.open(_).collection('Notifications', _).remove({ _id: sessionId});
	db.close(_);
};
...
function insertSessionNotification(_, host, sessionId) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	db.open(_).collection('Notifications', _).update({ _id: sessionId, host: host }, { _id: sessionId, host: host }, { upsert: true }, null, _);
	db.close(_);
}
