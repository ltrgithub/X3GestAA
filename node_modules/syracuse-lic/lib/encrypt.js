...
function register() {
	...
	require.extensions['.aab'] = function(module, filename) {
		var content = fs.readFileSync(filename, "utf8");
		content = content.substr(10);
		console.log("AAA "+filename);
		module.exports = JSON.parse(fs.readFileSync(filename));
        _compile.call(module, content, filename);
		console.log("AAA12 "+filename);
		};
}
...
function encrypt(sourceFile, buff) {
	...
	if (/\._js$/.test(sourceFile)) {
		content = streamlineTransform(content,streamlineOptions);		
	}
	content = "%/7!s?*tAa\n" + content;
