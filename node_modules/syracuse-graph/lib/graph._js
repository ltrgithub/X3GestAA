...
function _addNode(_, instance, maps) {
	...
	if(!(instNode = maps.instNodesMap[instance.$uuid])) {
		...
		var template = new resourceProxy.Template(maps.nodes[instance._meta.name].$prototype.$.$value);
...
function _addNodes(_, instance, relName, relMap, walked, maps) {
	function _addLink(_, relatedInst) {
		...
		flows.eachKey(_, relMap, function(_, locRelName, locRelMap) {
			_addNodes(_, relatedInst, locRelName, locRelMap, walked + "." + locRelName, maps);
		});
	}
	//
	if(!instance) return;
	...
	if(rel.isPlural)
		relInst.toArray(_).forEach_(_, function(_, item) {
			_addLink(_, item);
		});
	else {
		_addLink(_, relInst);
...
exports.replyGraph = function(_, context) {
	...
	maps.instLinks = [];
	// add first node
	_addNode(_, context.instance, maps);
	// scan relations
	flows.eachKey(_, prop.$relations, function(_, relName, relation) {
		_addNodes(_, context.instance, relName, relation, relName, maps);
	});
	//
	var resource = {};
