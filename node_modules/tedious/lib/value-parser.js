  ...
  switch (type.name) {
    ...
    case 'DecimalN':
      if (dataLength === 0) {
        ...
        console.log("size :"+(dataLength - 1));
       	switch (dataLength - 1) {
          case 4:
          	if (options.decimalsAsString) {
          	  value = buffer.readAsStringUInt32LE( ) ;
            } else {
          ...
          case 8:
          	if (options.decimalsAsString) {
	          value = buffer.readAsStringUInt64LE( ) ;
	        } else {
          ...
          case 12:
          	if (options.decimalsAsString) {
          	  value = buffer.readAsStringUInt96LE( ) ;
          	} else {
          ...
          case 16:
          	if (options.decimalsAsString) {
          	  value = buffer.readAsStringUInt128LE( ) ;
          	} else {
          ...
          default:
       	...
       	if (options.decimalsAsString) {       		
   		  value = insertDecimalSeparator(value, metaData.scale) ;
...
insertDecimalSeparator = function(val, scale) {
	if (scale) {
		var l = val.length-scale ;
		if (l <= 0) {
			for (var i = l; i <= 0; i++ ) val = "0"+val ;
			l = val.length-scale ;
		}
		return val.slice(0, l)+"."+val.slice(l) ;		
	} else return val ;	
};
