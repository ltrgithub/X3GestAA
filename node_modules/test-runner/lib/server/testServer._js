...
function _runFile(_, file, verbose, results) {
	...
	if (typeof Proxy !== "undefined") nodeOpts.push("--harmony-proxies");
	var harmony = false;
	try {
		eval("(function*(){})");
	...
	var logpath = pathUtil.isAbsolute(file) 
	? pathUtil.join(pathUtil.dirname(file), pathUtil.basename(file) + '.testlog')
	: pathUtil.join(__dirname, "../../..", file + '.testlog');
	...
	try {
		...
		console.log(nodeOpts.concat([__dirname + "/../../deps/node-qunit/lib/bootstrap.js", JSON.stringify(opts)]))
