...
// no need here as it should be done in loader.js and it breaks code coverage report
...
process.nextTick(function() {
	...
	var srv = require('syracuse-main/lib/syracuse').server;
	if (srv && srv.setTimeout) srv.setTimeout(0);
});
...
function doDir(_, root, path, rPath, depth, result, tier) {
	function _dirOk(file) {
		switch (depth) {
			...
			case 1:
				return file == "test" || rPath.toString() == '/^test$/';
			case 2:
	...
	files.forEach_(_, function(_, file, i) {
		var fpath = path + "/" + file;
		// hack to avoid problem with inaccessible mongodb.log
		if (file.substr(file.length - 4) === ".log") {
			console.log("Ignore log file " + file);
			return;
		}
		// temporary hack for etna tests
		/*
		 if (/\/etna\/test\/(engine|supervisor|api)$/.test(fpath)) {
		 fpath += '/server';
		 file = 'server';
		 }*/
		try {
			...
			if (e.code === "ENOENT") {
				console.log("Ignore missing path " + root + fpath)
				return; // ignore missing test directory
			}
			throw e;
		}
		if (stats.isDirectory()) {
			...
			var match;
			if (depth == 3 && (match = /(^[^.].*)\.(_?js|src|api|ts)$/.exec(file))) {
				if (match[2] === 'src' && (!config.etna || config.etna && file.indexOf("QLFAR_SUB_") >= 0)) return;
				if (match[2] === 'api' && !config.etna) return;
...
function _getFiles(_, tier) {
	...
	if (tier === 'server' && config.etna && config.etna && config.etna.localRoot && typeof Proxy !== 'undefined') {
		const root = pathUtil.join(config.etna.localRoot, "X3DVLP/folders/GX3APP/x3-erp"); // quick hack
		fs.readdir(root, _).forEach_(_, (_, sub) => {
			const testDir = pathUtil.join(root, sub, 'test');
			if (fs.exists(testDir, _)) {
				fs.readdir(testDir, _).forEach_(_, (_, name) => {
					if (name.endsWith('.x3js')) result.push(pathUtil.join(testDir, pathUtil.basename(name)));
				});
			}
		});
	}
	...
	if (config.unit_test && config.unit_test.rootDirs) {
		...
		doDir(_, pathUtil.join(__dirname, "../../.."), "/@sage/syracuse-lib", /^test$/, 0, result, tier);
		doDir(_, pathUtil.join(__dirname, "../../.."), "/@sage/etna", /^test$/, 0, result, tier);
...
function _runFile(_, file, verbose, results) {
	if (/^\.\./.test(file)) file = pathUtil.join(__dirname, '../../..', file); // fix relative path
	var opts = {
		code: file,
		tests: [file],
		coverage: false,
	...
	if (typeof Proxy !== "undefined" && /^v4\./.test(process.version)) nodeOpts.push("--harmony-proxies");
	var harmony = false;
	try {
		eval("(function*(){})");
	...
	var logpath = pathUtil.isAbsolute(file) ? pathUtil.join(pathUtil.dirname(file), pathUtil.basename(file) + '.testlog') : pathUtil.join(__dirname, "../../..", file + '.testlog');
...
function runIt(_) {
	...
	program
		...
		.option('-n, --fileName <fileName>', 'FileName of test to execute')
		.parse(process.argv);
	...
	var files = _getFiles(_, "server").filter(function(f) {
		...
		var folderOk = !folder || ~f.indexOf(folder);
		var fileNameOk = !fileName || ~f.indexOf(fileName);
		//console.log("file : " +f + " -> " + (folderOk && fileNameOk ?"yes":"no"));
		return folderOk && fileNameOk;
	});
	...
	if (program.httpOut || program.mongoServer) {
		...
		rapport.status = rapport.tests.some(function(it) {
			return it.status === "error";
		}) ? "error" : "success";
