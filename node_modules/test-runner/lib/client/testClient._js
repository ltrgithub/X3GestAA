...
function runServer(files, _) {
	...
	files.forEach_(_, function(_, file) {
		...
		results.forEach(function(result) {
			...
			var message = result.message + (result.errorStack ? "<br/>" + result.errorStack.replace(/\n/g, "<br/>") : "");
...
exports.main = function(_) {
	try {
		...
		if (!qs.rc && !qs.rs) {
			...
			var x3files = files.server.filter(function(val) {
				return val.substring(0, 11) == "syracuse-x3";
			});
