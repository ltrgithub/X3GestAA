...
function summary(type, files) {
	var cType = type[0];

	var href = function(files) {
		return "?r" + cType + "=" + files.join(',');
	}
	var link = function(title, files) {
		return '<a href="' + href(files) + '">' + title + '</a>';
	}

	_li("ALL " + type.toUpperCase() + " TESTS", href(files[type]));

	var root = files[type].reduce(function(r, file) {
		// hack to separate etna engine and supervisor tests
		var ffile = file.replace(/@sage\//, '');
		ffile = ffile.replace(/etna\/test\/(supervisor|engine|api)\//, 'etna-$1/test/');
		console.log(ffile);
		ffile = ffile.replace(/^.*\/x3-erp\/([^\/]*)\/test\//, 'x3-$1/test/');
		ffile = ffile.replace(/^\.\.\/test\/([^\/]*)/, 'syracuse-$1/test');
		var subModule = ffile.split('/')[0];
		var module = subModule.split('-')[0];
		var node = r[module] = r[module] || {};
		node[subModule] = node[subModule] || [];
		node[subModule].push(file);
		return r;
	}, {});

	$.each(Object.keys(root), function(i, module) {
		var nodeModule = root[module];

		var modFiles = [];
		$.each(Object.keys(nodeModule), function(i, subModule) {
			modFiles = modFiles.concat(nodeModule[subModule])
		});

		var modLinks = link(module.toUpperCase(), modFiles);
		var smodLinks = [];
		$.each(Object.keys(nodeModule), function(i, subModule) {
			smodLinks.push(link(subModule, nodeModule[subModule]));
		});

		var $files = $("#qunit-tests");
		$files.append('<li class="pass"><strong>' + modLinks + "</strong> : " + smodLinks.join(', ') + '</li>');
	});
}
