...
function runServer(files, _) {
	...
	function finish() {
		...
		var $b = $('<strong>' + mod + ": " + name + counts + '</strong>');
		$ol.hide();
	...
	files.forEach_(_, function(_, file) {
		...
		results.forEach( function(result) {
			...
			var message = result.message + (result.errorStack 
				? "<br/>" + result.errorStack.replace(/\n/g, "<br/>") : "");
...
exports.main = function(_) {
	try {
      ...
      var x3files = files.server.filter(function(val){return val.substring(0,11) == "syracuse-x3";});
      _li("ALL X3 TESTS", "?rs=" + x3files.join(','));
		}
		if (qs.rc) {
