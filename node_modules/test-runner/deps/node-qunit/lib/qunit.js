...
function run() {
    ...
    // run the test
    if ( !pause ) {
    	function logError(err, sync) {
	        if (err) {
	        	log({
	              message: "" + (err.message || ("ERROR: " + sys.inspect(err))),
	           	  errorStack: "" + (err.stack || "<unkown stack>")
	         	});
	         	__start();
	         }
	    }
    	try {
            console.error("test: ", test.name);
       		test.fn.call( QUnit.current_testEnvironment = extend( true, test.env || {}, module.env ), logError);
       	}
       	catch (err) {
       		logError(err);
       	}
...
function testDone() {
    ...
    if ( test.expected !== undefined && test.expected !== test.done ) {
        log({
            ...
            message: "check assertion count",
            errorStack: "Expected " + test.expected + " assertions, but " + test.done + " were run"  
        });
    }  
...
function log( obj ) {
    ...
    obj.xqz = "t@)$"; // marker to reduce confusion with accidental log messages in JSON format
    ...
    for ( ; i < args.length; ++i ) {
        typeof args[i] === "object" && Object.keys( args[i] ).forEach(function( key ) {
            ...
            if (deep === true && target[key]) {
				...
				var getter = args[i].__lookupGetter__(key);
				var setter = args[i].__lookupSetter__(key);	
				if (getter || setter) {
					if (getter) 
						target.__defineGetter__(key, getter);
					if (setter) 
						target.__defineGetter__(key, setter);
				}
				else {
 ...
 */
var __test = QUnit.api.test = function( name, expect, fn, async ) {
    ...
    console.log(JSON.stringify({
        ...
        test: name,
        expect: expect,
        xqz: "t@)$" // marker to reduce confusion with accidental log messages in JSON format
    }));
...
QUnit.api.asyncTest = function( name, expect, fn ) {
    __test.call(QUnit, name, expect, fn, true );
    return this;
};
 ...
 */
var __start = QUnit.api.start = function() {
    pause = false;
    testDone();
    run();
    return this;
...
function trimStack(err) {
    if (!err || typeof err !== "object") return typeof err + ": " + err;
