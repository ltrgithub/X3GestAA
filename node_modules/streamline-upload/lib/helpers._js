...
exports.sanitizeReader = function(_, reader, contentType) {
	...
	if (rejectors.some_(_, function (_, a) {
		return a(_, contentType, detected, buf);
	})) {
		 throw new MediaTypeError(_, reader, detected);
	}
	...
	} else if (/svg/i.test(contentType) || /svg/i.test(detected)) {
		reader = reader.map(function(_, buf) {
			return buf.toString('binary');
		}).transform(htmlTransforms.escaper({
