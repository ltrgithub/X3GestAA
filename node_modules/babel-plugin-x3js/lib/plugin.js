...
function isDollarMember(t, node) {
	...
	return {
		visitor: {
			...
			CallExpression: function(path, state) {
				...
				if (isArrayBuiltinMember(t, node.callee) && t.isFunctionExpression(fArg)) {
					var replaced = node.callee.property.name.replace(/\$$/, '_');
					fArg =  t.functionExpression(fArg.id, [t.identifier('_')].concat(fArg.parameters), fArg.body);
					path.replaceWith(t.callExpression(
						t.memberExpression(node.callee.left.object, t.identifier(replaced)),
				...
				node.body = [
					t.variableDeclaration('var', [
						t.variableDeclarator(
							...
							t.callExpression(t.identifier('require'), [t.identifier('etna/lib/engine/x3js')]))
					])
				].concat(node.body);
			},
		},
