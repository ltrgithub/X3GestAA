	...
	function constructList(_, map, el, dd, opt) {
		...
		while (stack.length) {
			...
			if (item.minOccurs && item.minOccurs !== '0' ||  item.maxOccurs) {
				card = "[" + (item.minOccurs ||  "0") + (item.maxOccurs ? ".." + item.maxOccurs  : "") + "]";
				...
				if (elem && elem.$leafAttribute) {
					...
					if (list[father] && list[father].elems && list[father].elems.length) {
						list[father].elems.forEach_(_, function(_, item) {
							...
							}, new Buffer(JSON.stringify(element) + "\n", "utf8"));
						});
					}
					...
					//console.log("list father " + JSON.stringify(list[father]));
...
/// ## var result = getXmlPath(_,opt) :
/// This function return a page of xmlPath in order to let the applitive screen displayed it define the setup for xml edi.
/// result : the object that all xml path with id and type description
///
/// result = {
///    "itemPerPage": 'number of item per page",
///    "totalResult" : "total number of items",
///    feed : { [
///         {
///              id :"0",
///              path : "",
...
/// ```javascript
/// var option =
///    ediEntityUuid : "uuid1", //list of uuid of xsd file store in storage area
