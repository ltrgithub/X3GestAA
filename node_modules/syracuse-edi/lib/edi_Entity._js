...
function _executeParseXsd(_, opt, jsXsds) {
	...
	function constructList(_, map, el, dd, opt) {
		var stack = [{
			map: map,
			elem: el,
		}];
		...
		while (stack.length) {
			...
			if (elem) {
				if (elem && elem.$leafAttribute) { // we stop on the element that describe the attribut
					...
					if (list[father] && list[father].elems && list[father].elems.length) {
						list[father].elems.forEach_(_, function(_, itm) {
							var att = {
								...
								restriction: itm.restriction = mapType[elem.$type] && mapType[elem.$type].$restriction
							};
				...
				} else if (elem && (elem.$type && (elem.$type.substring(0, 3) === "xs:" || (mapType[elem.$type] && mapType[elem.$type].$type && mapType[elem.$type].$type.substring(0, 3) === "xs:")))) { // its  a simple type or a link to a simple type
					...
					console.log("father key " + fatherKey + " current " + elem.$name);
					var nb = 0;
					Object.keys(elem) && Object.keys(elem).forEach(function(itm) {
						if (["$complex", "$name", "$type", "$maxOccurs", "$minOccurs"].indexOf(itm) === -1) {
					...
					console.log("finish father key " + fatherKey + " current " + elem.$name);
					...
					//console.log("next " + JSON.stringify(elem));
					if (elem.$type) {
						stack.push({
							father: father,
							map: mapType,
	...
	mapType.root && mapType.root.forEach_(_, function(_, root) {
		constructList(_, mapType, mapType[root], ddXml, opt);
	});
