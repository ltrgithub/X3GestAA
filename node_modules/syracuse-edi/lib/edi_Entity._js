...
var ediEntityTracer = require('syracuse-core').getTracer("edi.entity");
...
function _createEdiEntity(_, opt) {
	...
	if (klass.getInstance(_, filder) !== null) {
		throw new Error(locale.format(module, "cantCreate", klass.entityName, opt.id, opt.type));
	} else {
...
function _createEdiCacheEntity(_, opt) {
	...
	try {
		...
		// log exception
		ediEntityTracer.error && ediEntityTracer.error("createEdiCacheEntity message " + " stack " + e.stack);
...
function _getEdiCacheEntity(_, opt) {
	...
	try {
		...
		// log exception
		ediEntityTracer.error && ediEntityTracer.error("_getEdiCacheEntity message " + e + " stack " + e.stack);
...
exports.getEdiCacheJson = function(_, opt) {
	...
	try {
		...
		// log exception
		ediEntityTracer.error && ediEntityTracer.error("getEdiCacheJson message " + e + " stack " + e.stack);
...
function _updateEdiCacheEntity(_, opt) {
	...
	try {
		...
		// log exception
		ediEntityTracer.error && ediEntityTracer.error("updateEdiCacheEntity message " + e + " stack " + e.stack);
...
exports.removeEdiCacheEntity = function(_, opt) {
	...
	try {
		...
		// log exception
		ediEntityTracer.error && ediEntityTracer.error("removeEdiCacheEntity message " + e + " stack " + e.stack);
...
function _executeParseXsd(_, opt, jsXsds) {
	...
	function _parseComplexType(name, elemComplexType, map, prefix, listNameSpace) {
		...
		var complexContent = base && base[prefix + "element"] && base[prefix + "element"][prefix + "complexType"] && base[prefix + "element"][prefix + "complexType"][prefix + "complexContent"];
		if (complexContent) {
			// manage xtension only today
			if (complexContent[prefix + "extension"]) {
				_addElem({
					$: {
						name: base[prefix + "element"].$.name,
						maxOccurs: base[prefix + "element"].$.maxOccurs,
						minOccurs: base[prefix + "element"].$.minOccurs,
					...
					if (list[father] && list[father].elems && list[father].elems.length) {
						list[father].elems.forEach_(_, function(_, itm) {
							var att = {
								...
								restriction: elem.$restriction || mapType[elem.$type] && mapType[elem.$type].$restriction,
								documentation: itm.$documentation
							};
							//console.log("att ", att.$type,att);

							// remove namespace for the path stored that will be send to the 4GL
							if (att.type.indexOf("http://") !== -1) {
