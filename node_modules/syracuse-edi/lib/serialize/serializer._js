...
exports.serialize = function(_, opt) {
	...
	var sContext;
	var encoding = "utf8";
	var entity = opt.process.endPoint(_).getModel(_).getEntity(_, opt.process.x3RepName(_), "$details");
	var prototype = entity && entity.getPrototype(_, entity.name, "$details");
	...
	if (opt.action === "edi") {
		...
		sContext = {
			...
			sepRecord: messageMapping.SEPREC,
			delimField: messageMapping.FLDLIM,
			getPropertyName: function(item) {
				return item.SASVAR;
			},
			fileName: "import" + opt.process.x3RepName(_) + ".csv"
		};
	}
	if (serializer) {
 ...
 */
function _getJsonElem(json, key, prototype, prop) {

	var properties = prop || prototype && prototype.$properties;
	if (key.indexOf(".") === -1) {
		if (properties && properties[key] || helpersEdi.getSubProperties(properties) && helpersEdi.getSubProperties(properties)[key]) {
			return json[key];
		} else {
		...
		for (var i = 0; i < keys.length && !stop; i++) {
			...
			properties = properties && properties[item] || helpersEdi.getSubProperties(properties) && helpersEdi.getSubProperties(properties)[item] || null;
		}
		if (Array.isArray(js)) { // add all element of each line
			var keyInArray = keys.slice(i).join(".");
			for (var k = 0; k < js.length; k++) {
				if (properties) {
					var l = _getJsonElem(js[k], keyInArray, prototype, properties);
					l && result.push(l);
				}
			}
		} else if (stop) {
	...
	delimited: function(json, elems, prototype, sContext) {
		...
		elems.forEach(function(elem) {
			...
			if (propertyName) {
				...
				var items = _getJsonElem(json, propertyName, prototype);
