...
var sdataClient = require('./client'),
	sa = require('@sage/syracuse-lib/src/orm/storageArea'),
	fs = require('streamline-fs'),
	ez = require("ez-streams"),
	pdfReader = require('@sage/syracuse-lib/src/pdf/reader'),
	pdfWriter = require('@sage/syracuse-lib/src/pdf/writer'),
	jsxml = require('js-xml'),
		...
		if (options != null) {
			...
			if (options.sdata != null && options.sdata.format !== "json") {
				if (options.sdata.format === "xml") {
					try {
						if (options.sdata.payload != null) options.sdata.payload = jsxml.parse(options.sdata.payload);
					} catch (e) {
			...
			if (options != null && options.binaries != null) {
				if (options.binaries.pdf == null) throw new Error("The document storage uuid for PDF is required in 'options' parameter.");
				if (options.binaries.ubl == null) throw new Error("The document storage uuid for UBL is required in 'options' parameter.");
			...
			if (options.method === "GET" && options.url.indexOf('eInvoice') !== -1 && resp.statusCode === 200) {
				...
				if (format === 'xml') resp.body = jsxml.parse(resp.body);
				...
				if (format === 'xml') resp.body = jsxml.stringify(resp.body);
			}
...
function checkOptions(options) {
	if (options.host == null) throw new Error("The host server name or IP adress is required in 'options' parameter.");
	if (options.url == null) throw new Error("The http url is required in 'options' parameter.");
	if (options.method == null) {
		throw new Error("The http method is required in 'options' parameter.");
	} else {
		if (options.sdata == null && (options.method === 'POST' || options.method === 'PUT')) throw new Error("SData information are required in 'options' parameter for 'POST' and 'PUT' requests.");
	}
	if (options.auth == null) throw new Error("Authentication information are required in 'options' parameter.");
...
function tempDocument(_, uuid) {
	...
	if (!dd) throw new Error("cannot open temp document for " + uuid);

	var fname = tempDir + 'tmp-' + uuid;
	var astream = ez.devices.node.writer(fs.createWriteStream(fname));
