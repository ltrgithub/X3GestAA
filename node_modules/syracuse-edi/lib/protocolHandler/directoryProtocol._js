...
function _getSadFsq(_, endpoint, opt) {
	return new Sadfsq(_, endpoint, null, null, null, false, (opt.sadfsq && opt.sadfsq.recOptions) || {});
	...
	try {
		...
		out && Object.keys(out).forEach_(_, function(_, filename) {
			...
			if ((/\w|\d/.test(out[filename].content)) || out[filename].mandatory) {
				console.log("PFAD1 ", opt.path, itemsIssue && itemsIssue[0][protoMap.listDirectories.path], filename)
				sadFsq.writeFile(_, {
				...
				if (!opt.test) {
					...
					console.log("PFAD2", opt.path, itemsArch && itemsArch[0][protoMap.listDirectories.path], filename, opt.idProcess)
					sadFsq.writeFile(_, {
...
exports.getReceiptFiles = function(_, protocol, seqFile, sqMap, ep, opt) {
