...
exports.parse = function(_, opt) {
	helpersEdi.log("INFO", "parse ediProcess " + opt.process.idProcess(_));
	...
	helpersEdi.log("FINE", "parser used " + SEQFILE_LIB[seqFile.TYPFIL]);
...
var _mapParse = {
	delimited: function(input, elems, prototype, json, sContext) {
		helpersEdi.log("INFO", "parse delimited");

		var mapJson = json || {};
		var jsonInstance;

		helpersEdi.log("FINE", "json " + JSON.stringify(json));
		Object.keys(input).forEach(function(filename) {
			//nb of readed character
			helpersEdi.log("FINE", "file parsed " + filename);
			...
			if (records) {
				...
				records.forEach(function(record) {
					...
					helpersEdi.log("FINE", "records fields " + JSON.stringify(recValues));
					elems.filter(function(prev, cur) {
						...
						if (idx < recValues.length) {
							helpersEdi.log("FINE", "process item " + item.EXPRESSION);
	...
	fixedLength: function(input, elems, prototype, json, sContext) {
		helpersEdi.log("INFO", "parse fixed length");
...
function _addToJsonObject(key, jsonRoot, prototype, item, value) {
	...
	if (jsonKey && jsonKey.length > 1) {
		for (var i = 0; i < jsonKey.length - 1 && !stop; i++) {
			...
			if (properties) { // we get only the object level and not the property
				...
				stop = true;
				helpersEdi.log("WARN", "property " + key + " doesn't exists for the entity " + prototype.$representation);
			}
		}
	...
	if (properties) {
		if (properties.$type === "application/x-array") {
			helpersEdi.log("FINE", "add in array  value " + _formatType(properties, value));
			json.push(_formatType(properties.$item, value));
		} else {
			helpersEdi.log("FINE", "add " + jsonKey[jsonKey.length - 1] + " value " + _formatType(properties, value));
