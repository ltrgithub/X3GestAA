	...
	Object.keys(filesBuff).forEach(function(filename) {
		...
		lines && lines.forEach(line) {
			seqFile.ESFESFD.filter(function(prev, cur) {
				return cur.FILNAM === filename) {

			}).forEach(function(item) {
					if (item.LOC >= line)
					// retrieve the property name in the json
						var propertyName = item.EXPRESSION.substring(item.EXPRESSION.indexOf(".") + 1);
					// extract the value
					var val = filesBuff[filename].substring(offsetLineStart + item.LOC - 1, (offset + item.LOC - 1) + item.LNG);
				...
				val = _removeSpace(val.replace("/\n|\r/g", "")); _addInJson(propertyName, prototype, jsonToGen, {
					value: val,
					endRec: item.ENDREG
				});
