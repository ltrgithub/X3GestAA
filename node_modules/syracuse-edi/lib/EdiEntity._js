...
exports.createEdiCacheEntity = function(_, opt) {
	ediEntityTracer.debug && ediEntityTracer.debug("launch createEdiCacheEntity opt " + JSON.stringify(opt, null, 2));
	if (Array.isArray(opt.json)) {
		...
		ediEntityTracer.debug && ediEntityTracer.debug("result createEdiCacheEntity uuid " + JSON.stringify(uuids, null, 2));

		return uuids.length === 0 ? null : uuids;
	} else {
		var uuids = _createEdiEntity(_, opt).$uuid;
		ediEntityTracer.debug && ediEntityTracer.debug("result createEdiCacheEntity uuid " + JSON.stringify(uuids, null, 2));
		return uuids;
	}
};
