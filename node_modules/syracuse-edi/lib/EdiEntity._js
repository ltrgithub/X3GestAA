...
exports.createEdiCacheEntity = function(_, opt) {
	...
	try {
		...
		/*objs.forEach(_, function(_, itemToSave) {
            itemToSave && itemToSave.save(_);
		});*/
	}
};
	...
	try {
		if (opt.elems) {
			...
			if (!instances || instances.length !== Object.keys(changes).length) {
				...
				throw new Error("can't find instance associate to uuid in parameter uuids =" + JSON.stringify(Object.keys(changes)));
			}
