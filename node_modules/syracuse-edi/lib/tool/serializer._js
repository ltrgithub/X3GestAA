...
var _mapSerialize = {
	xml: function(_, opt) {
		...
		if (input) {
			...
			console.log("elem ", JSON.stringify(input));

			Object.keys(configSerializer.elems).forEach_(_, function(_, filename) {
				var elem = configSerializer.elems[filename];
				var jsonxml = serializeHelp.convert(input, serializeHelp.preprocessMapping(_, elem, true));
				console.log("jsxml ", jsonxml);
