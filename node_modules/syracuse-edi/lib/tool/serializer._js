			...
			function _serialize(items, index) {
				items && items.forEach(function(val, idx, array) {
					...
					if (elem.isEnd && isFinishedBySepRecord(fileBuf[filename][idLine], configSerializer.sepRecord)) {
						// add record separator
						fileBuff[filename][idLine] += configSerializer.sepRecord;
					}
				});
			}
	...
	delimited: function(json, configSerializer, prototype) {
		...
		configSerializer.elems.forEach(function(elem) {
			...
			if (propertyName) {
				...
				itemOfItems[elem.flag][elem.level] = itemOfItems[elem.flag][elem.level] || {};

				itemOfItems[elem.flag][elem.level].nbLine = itemOfItems[elem.flag][elem.level].nbLine < items.length ? items.length : (itemOfItems[elem.flag].nbLine || 1);
		...
		itemOfItems && itemOfItems.items.forEach(function(item, idx) {
			for (var ii = 0; ii < itemOfItems[item.elem.flag][item.elem.level].nbLine; ii++) {
				...
				if (item.elem.isEnd && isFinishedBySepRecord(fileBuf[filename][idLine], configSerializer.sepRecord)) {
					// add record separator
	...
	fixedLength: function(json, configSerializer, prototype) {
		...
		configSerializer.elems.forEach(function(elem) {
			...
			if (propertyName) {
				...
				itemOfItems[elem.fileName][elem.level].nbLine = itemOfItems[elem.fileName][elem.level].nbLine < items.length ? items.length : (itemOfItems[elem.fileName].nbLine || 1);
		...
		itemOfItems && itemOfItems.items.forEach(function(item, idx) {
			for (var ii = 0; ii < itemOfItems[item.elem.fileName][item.elem.level].nbLine; ii++) {
				...
				if (item.elem.isEnd && isFinishedBySepRecord(fileBuf[filename][ii], configSerializer.sepRecord)) { // add the endrecord
		...
		Object.keys(fileBuf).forEach(function(filename) {
			...
			Object.keys(listLine).forEach(function(lineKey, idx, array) {
				...
				// last record we put the sepRecord even if it's not define in the model (should never be true because we should have receive the last one
				/*if (idx < array.length - 1 && isFinishedBySepRecord(fileBuf[filename], configSerializer.sepRecord)) {
					fileBuf[filename] += configSerializer.sepRecord;
				}*/
