...
exports.serialize = function(_, opt) {
	...
	if (opt.action === "edi") {
		...
		if (seqFile) {
			log.debug && log.debug("seqFile type  " + SEQFIL_LIB[seqFile[sqMap.fileType]]);

			serializer = SEQFIL_LIB[seqFile[sqMap.fileType]];
			elems = seqFile[sqMap.elem];
			sContext = {
		...
		if (messageMapping) {
			...
			elems = messageMapping[mmMap.elem];

			serializer = IMPORTFILE_LIB[messageMapping[mmMap.fileType]];
			sContext = {
				sepDecimal: messageMapping[mmMap.deimalSep],
				sepField: messageMapping[mmMap.fieldSep],
				sepRecord: "\r\n",
				delimField: messageMapping[mmMap.fieldDelimiter],
				getPropertyName: function(item) {
					return item[mmMap.elems.expression];
				},
				fileName: "import" + opt.process.x3RepName(_)
			};
		} else {
			throw new Error(locale.format(module, "noMessMapp", opt.process.idMessageMapping(_)));
		}
	}
...
var _mapSerialize = {
		...
		elems.forEach(function(elem) {
			...
			function _serialize(items, index) {
				items && items.forEach(function(val, idx, array) {
					...
					var filename = sContext.fileName || elem[sqMap.elems.filedId];
					var idLine = elem[sqMap.elems.flag] + (index || idx); // serialize
					...
					if (!fileBuf[filename][idLine]) { // if empty string so start of the line
						fileBuf[filename][idLine] += sContext.delimField + elem[sqMap.elems.flag] + sContext.delimField;
					}
	...
	fixedLength: function(json, elems, prototype, sContext) {
		...
		elems.forEach(function(elem) {
			var realLoc = elem[sqMap.elems.offset] - 1;
			var propertyName = sContext.getPropertyName(elem);
			propertyName = propertyName.substring(propertyName.indexOf(".") + 1);
			if (propertyName) {
				...
				items && items.forEach(function(val, idx) {
					var filename = sContext.fileName || elem[sqMap.elems.fileId];
					fileBuf[filename] = fileBuf[filename] || {};
					fileBuf[filename][idx] = fileBuf[filename][idx] || "";
					fileBuf[filename][idx] += _getFixedLengthValue(val, elem[sqMap.elems.length]);
				});
			}
		});
		Object.keys(fileBuf).forEach(function(filename) {
