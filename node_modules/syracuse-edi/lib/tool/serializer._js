...
var log = require('syracuse-core').getTracer("edi.serializer");
...
var _mapSerialize = {
	xml: function(_, opt) {
		...
		if (input) {
			...
			console.log("filename ", configSerializer.elems);
	...
	sepRecord: function(_, opt) {
		...
		configSerializer.elems.forEach(function(elem) {
			...
			function _serialize(items, level, index) {
				items && items.forEach(function(val, idx, array) {
					...
					if (elem.isEnd && !helpersEdi.endsWith(fileBuf[filename][idLine], configSerializer.sepRecord)) {
		...
		Object.keys(fileBuf).forEach(function(f) {
			...
			Object.keys(listLine).forEach(function(lineKey, idx, array) {
				...
				if (idx <= array.length - 1 && !helpersEdi.endsWith(fileBuf[filename], configSerializer.sepRecord)) {
	...
	fixedLength: function(_, opt) {
		...
		configSerializer.elems.forEach(function(elem) {
			...
			if (propertyName) {
				log.debug && log.debug("serialize filename " + fileName);
				...
				itemOfItems[fileName] = itemOfItems[fileName] || {};
				itemOfItems[fileName][level] = itemOfItems[fileName][level] || {
					nbLine: 1
				};
				var nbLine = itemOfItems[fileName][level].nbLine;
				itemOfItems[fileName][level].nbLine = nbLine < items.length ? items.length : (nbLine || 1);
			}
		});
		var lastline;
		var removeLineSep = {};
		...
		itemOfItems && itemOfItems.items.forEach(function(item, idx) {
			//removeLineSep = false;
			var nbLine = itemOfItems[item.elem.fileName][item.elem.level].nbLine;
			for (var ii = 0; ii < nbLine; ii++) {
				...
				if (item.elem.isEnd) {
					lineCounter[filename] = lineCounter[filename] == undefined ? 1 : lineCounter[filename] + 1;
					if (!helpersEdi.endsWith(lastline, configSerializer.sepRecord)) { // add the endrecord
						fileBuf[filename][ii] += configSerializer.sepRecord;
					}
					if (lineCounter[filename] === nbLine) {
						removeLineSep[filename] = true;
					}
				}
			}
		});
		Object.keys(fileBuf).forEach(function(filename) {
			var listLine = fileBuf[filename];
			fileBuf[filename] = "";
			var lineKeys = Object.keys(listLine);
			lineKeys.forEach(function(lineKey, idx, array) {
			...
			var line = fileBuf[filename];
			if (removeLineSep[filename] === true && helpersEdi.endsWith(line, configSerializer.sepRecord)) {
				// 122645 No sepRecord at the end of the record. sepRecord is added by the main process when buffer is saved in the file
				// Avoid double sepRecord between two records 
				fileBuf[filename] = line.substr(0, line.length - configSerializer.sepRecord.length);
			}
		});
