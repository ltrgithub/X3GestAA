	...
	fixedLength: function(json, configSerializer, prototype) {
		...
		itemOfItems && itemOfItems.items.forEach(function(item, idx) {
			for (var ii = 0; ii < itemOfItems[item.elem.fileName][item.elem.level].nbLine; ii++) {
				...
				temp[filename][ii] = temp[filename][ii] || "";
				// get the string correspond to the last line
				lastline = temp[filename][ii];
				...
				temp[filename][ii] += val;
				if (/\w/.test(temp[filename][ii]) && item.elem.isEnd && !isFinishedBySepRecord(temp[filename][ii], configSerializer.sepRecord)) { // add the endrecord
					temp[filename][ii] += configSerializer.sepRecord;
					fileBuf[filename] = fileBuf[filename] ||  {};
					fileBuf[filename][ii] = temp[filename][ii];
				}
			}
		});
		Object.keys(fileBuf).forEach(function(filename) {
