...
function preprocessMapping(_, mappings, toXML, diags, prototype) {
	...
	mappings.forEach(function(mapping) {
		...
		if (mapping.condition && toXML) {
			...
			// - path of condition variable (may contain dots: then from repetition group level there will be object path to reach variable
			// - comparison operator, bitwise combination of negation, equality, greater, numerical
			// - comparison constant (with which the condition variable will be compared)
			...
			var cond = [];
			var bracketStart = [0]; // Index where current bracketing level starts
			var andStart = [-2]; // Index where group of AND operands start
			var bracketLevel = 0; // current bracket level
			var current = -1; // current index of condition (excluding)
			var startIndex = -1;
			var linkBefore = true; // beginning or there has been AND/OR operator or '(' before
			while (condition) {
