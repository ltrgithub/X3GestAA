...
function preprocessMapping(_, mappings, toXML, diags, prototype) {
	...
	var rXML = /(.*?)(\[[^\]]+\])\.?/g;
	var rJSON = /(.*?)\./g;
	var rCOND = /^\s*((?:\w+\.)*)(\w+)\s*([!><=]{1,2}|[gtlneq]{2,3})\s*(["'])?((?:[^"']|\\["'])*)\4\s*$/;
	...
	mappings.forEach(function(mapping) {
		...
		mXML = mXML.replace(/\[0\.\.1\]/g, ""); // ignore optional elements
		...
		LOOP: while (sBase = rBase.exec(mBase)) {
				log.debug && log.debug("part of start mapping: " + JSON.stringify(sBase))
				var pBase = sBase[1];
				if (protopath && toXML && pBase) {
					do {
						protopath = _walkpath(protopath, sBase[1]);
						if (!protopath) _throwError("Mapping with JSON element " + mJSON + ": part " + sJSON + " does not exist in prototype");
						if (!protopath.$type) _throwError("Mapping with JSON element " + mJSON + ": type for part " + sJSON + " does not exist in prototype");
						if (protopath.$type === "application/x-reference") {
							sBase = rBase.exec(mBase);
							if (!sBase) { // reached last part
								break LOOP;
							}
							pBase += "." + sBase[1];
						} else {
				...
				if (protopath && !toXML && pDest) {
					do {
						protopath = _walkpath(protopath, sDest[1]);
						if (!protopath) _throwError("Mapping with JSON element " + mJSON + ": part " + sJSON + " does not exist in prototype");
						if (!protopath.$type) _throwError("Mapping with JSON element " + mJSON + ": type for part " + sJSON + " does not exist in prototype");
						if (protopath.$type === "application/x-reference") {
							console.log("ZZZZZZZZZZxxZ " + mDest + " " + JSON.stringify(sDest))
							sDest = rDest.exec(mDest);
							if (!sDest) { // reached last part
								break LOOP;
							}
							pDest += "." + sDest[1];
						} else {
		...
		if (!ok && rDest.exec(mDest)) _throwError("Mapping from " + mBase + " to " + mDest + ": destination path contains more repetition groups", "DESTMOREGROUPS");
		...
		if (mapping.condition) {
			var r = rCOND.exec(mapping.condition);
			if (!r)
				_throwError("Mapping from " + mBase + " to " + mDest + ": condition has wrong format: " + mapping.condition, "WRONGCONDFORMAT");
			...
			switch (r[3]) { // operator
				...
				case 'leq':
					...
					break; // not strictly bigger
				default:
					_throwError("Mapping from " + mBase + " to " + mDest + ": wrong operator in condition variable " + r[3], "WRONGCONDOP");
			}
			// examine constant
			var cnst = r[5];
			if (r[4]) // string
				cnst = cnst.replace(/\\(["'])/g, "$1");
			else if (!/^\-?\d+(?:\.\d+)?$/.test(cnst))
				_throwError("Mapping from " + mBase + " to " + mDest + ": wrong constant in condition: " + r[5], "WRONGCONDCONST");
			...
			if (mapped[0] !== pDest) {
				if (!toXML) {
					...
					if (!cond || mapped.length % 4) // there must be a condition on all entries - then the array consists of blocks of length 4
						_throwError("Mapping from " + mBase + " to " + mDest + ": Final path " + mBase + " was mapped to " + beginningDest + pDest + " and is now mapped to different path " + beginningDest + mapped, "FULLCHANGE");
		...
		while (--i >= 0) {
			...
			if (mapped) {
				...
				if (Array.isArray(mapped)) {
					for (var index = 0; index < mapped.length; index += 4) {
						if (mapped[index + 3]) { // condition
							var cond = mapped[index + 3];
							// get corresponding value
							var val = (cmp & 0x08 ? (+element[cond[0]] || 0) : "" + element[cond[0]]);
							var cmp = cond[1];

							if (((cmp & 0x01) && (val === cond[2]) || (cmp & 0x02) && (val > cond[2])) ^ !(cmp & 0x04)) {
