...
function preprocessMapping(_, mappings, toXML, diags) {
	...
	mappings.forEach(function(mapping) {
		...
		if (mapping.condition) {
			...
			var cnst;
			if (r[4]) // string
				cnst = r[5].replace(/\\(["'])/g, "$1");
			else if (r[5].toLowerCase() == "true") cnst = true;
			else if (r[5].toLowerCase() == "false") cnst = false;
			else if (/^\-?\d+(?:\.\d+)?$/.test(r[5])) cnst = Number(r[5]);
			else _throwError("Mapping from " + mBase + " to " + mDest + ": wrong constant in condition: " + r[5], "WRONGCONDCONST");
			cond = [r[2], cmp, cnst];
		}
				...
				if (Array.isArray(mapped)) {
					for (var index = 0; index < mapped.length; index += 4) {
						if (mapped[index + 3]) { // condition
							...
							var val = element[cond[0]] || "";
							var cmp = cond[1];
							if (((cmp & 0x01) && ((cmp & 0x08) ? +val === +cond[2] : "" + val === "" + cond[2]) || (cmp & 0x02) && ((cmp & 0x08) ? +val > +cond[2] : "" + val > "" + cond[2])) ^ !(cmp & 0x04)) {
