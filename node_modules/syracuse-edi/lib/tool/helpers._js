...
// T1 before the tag T2. So the mapping to XML will contain some initial strings of fixed length before the tag names. 
// The ordering of these strings corresponds to the ordering of the mappings. The fixed length is mentioned in the description:
...
function preprocessMapping(_, mappings, toXML, diags, prototype) {
	// splits the part of the XML path into parts (according ".") and assigns the number of the first rule found for this
	function withNumbers(x) {
	...
	var numbertranslation = {}; // helps to find numbers of rules for the parts of XML. Each XML level (tag) will be assigned to
	// an object which contains the names of the subtags and the name with full prefix of rule number as value for the "" key.
	// In above example, it will be
	// { "A":{"":"0A", "L":{"":"0L"}, "B":{"":"1B", "C":{"":"1C", "D":{"":"1D"}, "E":{"":"2E"}}},"F":{"":"3F","G":{"":"3G","H":{"":"3H"}},"I":{"":"4I","J":{"":"4J"}}}}
	var trans; // will be numbertranslation in the beginning, is used for walking through object structure of numbertranslation 
	...
	var prefix = ""; // for leading zeros
	if (toXML) {
		while (jump < mappings.length) {
			jump *= 10;
			prefix += "0";
		}
		result[""].toXML = prefix.length + 1;
		jump = 10;
	} else
	...
	mappings.forEach(function(mapping) {
		if (toXML) {
			if (++index0 >= jump) { // number will take one more digit: prefix should be smaller
				jump *= 10;
				prefix = prefix.substr(1);
			}
			fullprefix = prefix + index0; // with correct number of leading zeros to have fixed length
			if (fullprefix.length != result[""].toXML) throw new Error("Inconsistent prefix length " + fullprefix + " " + result.toXML)
		}
		...
		//
		trans = numbertranslation; // 'trans' is variable for loop
		...
		if (mapping.condition && toXML) {
			...
			while (condition) {
				...
				if (r[5] || r[6]) { // position of AND/OR operator
					...
					if (linkBefore) _throwError("Mapping from " + mBase + " to " + mDest + ": condition has operator " + operator + " at wrong place (position " + errorIndex + "): " + mapping.condition, "WRONGCONDOP");
					linkBefore = true;
				}
				if (r[5]) { // AND operator
...
function convert(input, mapping) {
	...
	function _stripNumbers(obj) {
		// console.error("IN STR "+JSON.stringify(obj))
		if (Array.isArray(obj)) {
			for (var i = obj.length - 1; i >= 0; i--) {
				obj[i] = _stripNumbers(obj[i]);
			}
			return obj;
		} else
		if ((typeof obj) === 'object') {
			...
			keys.forEach(function(x) {
				result[x.substr(toXML)] = _stripNumbers(obj[x])
			});
			return result;
		} else
			return obj;
