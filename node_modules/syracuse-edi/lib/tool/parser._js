...
function getPrimaryKey(prefixExp, jsonInstance, item, config) {
	...
	console.log("jsIsnt " + JSON.stringify(jsIsnt));
	...
	fixedLength: function(input, configParser, prototype, json) {
		...
		Object.keys(input).forEach(function(filename) {
			...
			lines && lines.forEach(function(line) {
				...
				if (lastItem) { // we've finish to creat the JSON associate to the line- we have to compute the key and foreign key in order to push it in the final json structure
