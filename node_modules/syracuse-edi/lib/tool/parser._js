...
exports.parse = function(_, opt) {
	...
	Object.keys(opt.configParser.linkKey).forEach(function(filenamePattern) { // TODO manage generation without order consideration
		...
		for (var i = 0; i < Object.keys(input).length; i++) {
			found = regexp.test(Object.keys(input)[i]) ? Object.keys(input)[i] : null; // add found data
			if (found) {
...
var _mapParse = {
	sepRecord: function(json, configParser, prototype) {
		log.info && log.info("------------------");
					...
					elems.filter(function(prev, cur) {
						...
						if (regexp && regexp.test(filename)) {
							...
							if (idElem && Object.keys(idElem).length) { // if idelem exists and idLine correspond we have t take it else false
								return lines.substring(idElem.offset, idElem.offset + idElem.length) === idElem.idLine;
...
function _addToJsonObject(key, jsonRoot, prototype, item, value, config) {
	...
	if (properties) {
		if (properties.$type === "application/x-array") {
			var valFmt = _formatType(properties.$item, value, config);
			log.debug && log.debug("add in array  value " + valFmt);
			json.push(valFmt);
		} else {
			var valFmt = _formatType(properties, value, config);
...
function _formatType(propItem, str, config) {
