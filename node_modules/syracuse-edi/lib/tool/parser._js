...
exports.parse = function(_, opt) {
	...
	if (_mapParse[opt.configParser.parser]) {
		return _mapParse[opt.configParser.parser](_, opt);
	} else {
...
var _mapParse = {
	...
	xml: function(_, opt) {
		...
		var input = opt.input;
		var configParser = opt.configParser;
		log.debug && log.debug("input " + JSON.stringify(input));

		if (input) {
			var mapJson = opt.json || {};
			Object.keys(input).forEach_(_, function(_, fileName) {
				var keys = Object.keys(configParser.elems);
				Object.keys(configParser.elems) && Object.keys(configParser.elems).forEach_(_, function(_, f) {
					var regexp = new RegExp(f)
					if (regexp && regexp.test(fileName)) {
						var xmlIn = jsxml.parse(input[fileName]);
						var diags = [];
						var map = parseHelp.preprocessMapping(_, configParser.elems[f], false, diags, opt.prototype, {
							ignoreNS: true
						});
						mapJson[helpers.uuid.generate()] = parseHelp.convert(xmlIn, map);
					}
				});
			});
			log.debug && log.debug("json " + JSON.stringify(mapJson));
		}
		return mapJson;
	},
	...
	fixedLength: function(_, opt) {
		...
		log.debug && log.debug("mapJson " + JSON.stringify(mapJson) + " - NbFiles: " + files.length);
		var lastItem;
