...
exports.parse = function(_, opt) {
	...
	var sortInput = {};
	Object.keys(opt.configParser.linkKey).forEach(function(filenamePattern) { // TODO sorted link key to have th right order
	...
	if (_mapParse[opt.configParser.parser]) {
		return _mapParse[opt.configParser.parser](sortInput, opt.configParser, opt.prototype, json);
	} else {
				...
				records.forEach(function(record) {
					...
					elems.filter(function(prev, cur) {
						if (configParser.elems[cur]) {
							...
							return regexp && regexp.test(filename);
						}
						return false;
