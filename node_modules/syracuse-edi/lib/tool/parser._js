...
exports.parse = function(_, opt) {
	...
	if (typeof opt.prototype === "string") {
		prototype = EdiEntity.getEdiCacheJson(_, {
			uuid: opt.prototype,
			db: opt.db
		});
	} else {
	...
	log.debug && log.debug("parser used " + SEQFILE_LIB[seqFile[sqMap.fileType]]);
	...
	var sContext = {
		...
		getPropertyName: function(item) {
			return item[sqMap.elems.expression];
		}
	};
	return _mapParse[SEQFILE_LIB[seqFile[sqMap.fileType]]](input, seqFile[sqMap.elem], prototype, sContext, json);

};

var _mapParse = {
	delimited: function(input, elems, prototype, sContext, json) {
			...
			if (records) {
				...
				records.forEach(function(record) {
					...
					elems.filter(function(prev, cur) {
						return elems[cur] && elems[cur][sqMap.elems.fileId] === filename && elems[cur][sqMap.elems.flag] === recFlag;
					}).forEach(function(item, idx, arr) {
						if (idx < recValues.length) {
							log.debug && log.debug("process item " + item[sqMap.elems.expression]);

							var propertyName = item[sqMap.elem.expression].substring(item[sqMap.elem.expression].indexOf(".") + 1);
	...
	fixedLength: function(input, elems, prototype, sContext, json) {
		...
		Object.keys(input).forEach(function(filename) {
			...
			lines && lines.forEach(function(line) {
				elems.filter(function(prev, cur) {
					...
					return elems[cur] && elems[cur][sqMap.elems.fileId] === filename;
				}).forEach(function(item) {
					log.debug && log.debug("process item " + item[sqMap.elems.expression]);

					// TODO need to identify the key - hack we use the elem of loc =1

					var realLoc = item[sqMap.elems.offset] - 1;
					var propertyName = item[sqMap.elems.expression].substring(item[sqMap.elems.expression].indexOf(".") + 1);

					if (realLoc < line.length) {

						// extract the value
						var val = parseHelp.removeSpace(line.substring(realLoc, realLoc + item[sqMap.elems.length]).replace("/\n|\r/g", ""));
