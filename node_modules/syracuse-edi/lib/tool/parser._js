...
exports.parse = function(_, opt) {
	...
	if (_mapParse[opt.configParser.parser]) {
		return _mapParse[opt.configParser.parser](opt);
	} else {
...
var _mapParse = {
	...
	xml: function(opt) {
		...
		var input = opt.input;
		if (input) {
			if (!Array.isArray(opt.input)) {
				input = [input];
			}
			var mapJson = json || {};

			Object.keys(input).forEach(function(fileName) {
				var xmlIn = input[fileName];
				mapJson[helpers.uuid.generate()] = _convert(_preprocessMapping(xmlIn, opt.elem[fileName]), true);
			});
...
function _preprocessMapping(_, mappings, toXML) {
	var result = {
		"": {
			toXML: !!toXML,
			depth: 1
...
exports.preprocessMapping = _preprocessMapping;
...
function _convert(input, mapping) {
	var result = {};
	...
	function _conv(element, map, res, path, depth) {
		...
		console.log("CONV " + keys.join(",") + " " + JSON.stringify(res) + " " + path + " " + depth);
		var i = keys.length;
		while (--i >= 0) {
			var key = keys[i];
			var path2 = path + key
			...
			if (mapped) {
				...
				if (Array.isArray(mapped)) {
					...
					console.log("VAL " + value + " " + path2 + " " + mapped.join(","))
					if (toXML) {
						if (mapped[2]) {
							switch (mapped[2]) {
								case 'date':
