...
function getPrimaryKey(prefixExp, jsonInstance, item, config) {
	...
	var primarykey = "";
	config.linkKey && config.linkKey[item.fileName] && config.linkKey[item.fileName].primary && config.linkKey[item.fileName].primary.split("+").forEach(function(i) {
		if (jsIsnt[i])
			primarykey += jsIsnt[i];
	});
	return primarykey;
...
function getForeignKey(prefixExp, jsonInstance, item, config) {
	// process primary key
	var jsIsnt = jsonInstance;
	if (prefixExp && prefixExp.indexOf(".") !== -1) {
		prefixExp.split(".").forEach(function(key) {
	...
	var foreignkey = "";
	config.linkKey && config.linkKey[item.fileName] && config.linkKey[item.fileName].foreign && config.linkKey[item.fileName].foreign.split("+").forEach(function(i) {
		if (jsIsnt[i])
			foreignkey += jsIsnt[i];
	});
	return foreignkey;
...
function pushJson(configParser, lastItem, res, mapJson, jsonInstance) {
	console.log("jsIsnt " + JSON.stringify(jsonInstance));
	...
	var primarykey = getPrimaryKey(prefixExp, jsonInstance, lastItem, configParser);
	...
	if (configParser.linkKey[lastItem.fileName].fatherFileId) {
		var foreignkey = configParser.linkKey[lastItem.fileName].fatherFileId + "_" + getForeignKey(prefixExp, jsonInstance, lastItem, configParser);
...
var _mapParse = {
				...
				records.forEach(function(record) {
					...
					elems.filter(function(prev, cur) {
						...
						if (idx < recValues.length) {
							...
							if (!idx) { // TODO first name  don't determine the key of the JSON. we have to first process the foreign key to find the rigth JSON
							...
							if (idx === arr.length) { // we've finish to creat the JSON associate to the line- we have to compute the key and foreign key in order to push it in the final json structure
			...
			if (records) {
				...
				records.forEach(function(record) {
					...
					elems.filter(function(prev, cur) {
						...
						} else if (idx == arr.length - 1) { // last element treated
	...
	fixedLength: function(input, configParser, prototype, json) {
			...
			lines && lines.forEach(function(line) {
				configParser.elems.filter(function(prev, cur) {
					...
					if (realLoc < line.length) {
						...
						if (!realLoc) { // reset lastItem process and jsonInstnace process
							jsonInstance = {};
						}
					...
					if (idx === arr.length - 1) { // we've finish to creat the JSON associate to the line- we have to compute the key and foreign key in order to push it in the final json structure
		...
		console.log("mapJson " + JSON.stringify(res, null, 2));
