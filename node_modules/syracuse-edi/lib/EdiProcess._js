...
function _createEdiProcess(_, opt) {
	ediProcessTracer.debug && ediProcessTracer.debug(" _createEdiProcess opt " + JSON.stringify(opt));
	try {
...
function _getEdiProcess(_, opt) {
	ediProcessTracer.debug && ediProcessTracer.debug(" _getEdiProcess opt " + JSON.stringify(opt));
	try {
...
function _removeEdiProcess(_, opt) {
	ediProcessTracer.debug && ediProcessTracer.debug(" _removeEdiProcess opt " + JSON.stringify(opt));
	try {
...
function _writeIssueFiles(_, protocol, ep, files, opt) {
	ediProcessTracer.debug && ediProcessTracer.debug(" _writeIssueFiles protocol " + protocol && protocol. + " endpoint " + ep.description(_) + " files "++" opt " + JSON.stringify(opt));
	try {
...
function _generateFiles(_, idProcess, opt) {
	...
	if (!document || !document.length) {
		...
		opt.uuid.forEach_(_, function(_, uuid) {
			// read files
			var option = opt;
			option.uuid = uuid;
			option.process = cachedEdi;
			...
			fileNames.forEach(function(filename) {
				if (output && output[filename]) { //concat the content of filename to this existing one with new line
					output[filename] += "\n" + res[filename];
				} else { // new file
		...
		var fileNames = Object.keys(output);

		// store file content in store area temporary
		for (var i = 0; i < fileNames.length; i++) {
			var filename = fileNames[i];
			document.push(_createSADocument(_, idProcess, filename + "." + encodingContext.EXT, new Buffer(output[filename], opt.encoding)));
		}
		...
		document.forEach_(_, function(_, doc) {
			output[doc.fileName(_)] = sa.readAll(_, {
				jsonWhere: {
					$uuid: doc.$uuid
				}
