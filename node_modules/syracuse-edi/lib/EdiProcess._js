...
function _getEndpoint(_, protocol, ep, opt) {
	...
	if (endpoint === null) {
		throw new Error('not Endpoint define in order to read or write file to perform action');
	}
	return endpoint;
}
...
function _getReceiptFiles(_, protocol, seqFile, ep, opt) {
	...
	function _checkPatternFileName(name, sf) {
		var exists = false;
		for (var i = 0; sf && sf.ESFESFD && i < sf.ESFESFD.length && !exists; i++) { // se base sur le tableau qui decrit la liste des fichiers //TODO check pattern
