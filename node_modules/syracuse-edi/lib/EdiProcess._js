...
function _generateFiles(_, idProcess, opt) {
	...
	opt.sadfsq.password = opt.sadfsq.password || (config.edi && config.edi.password);
	// store in the cache the context (env) received
	// TODO check if the document is already generate
	...
	// saved temporary files in storage if the write in a remove folder via sadfsq works or not to prevent to reserialize the file
	/*
    TODO test if it store properly in document
    ...
    for(var i=0;i<fileNames.length;i++){
        ...
        sa.writeAll(_, {
            ...
            content: {
                ...
                fileName: fileNames+encodingContext.EXT,
            }
        }, new Buffer(res[filename], opt.encoding), null, "EDI");
    }*/
	//if (protocol && protocol.PTCTYP === PROTOCOL.directory) {
