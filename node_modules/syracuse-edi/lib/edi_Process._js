...
var sqMap = helpersEdi.seqentialFilePropertyMap;
var xmlMap = helpersEdi.xmlFilePropertyMap;

var mmMap = helpersEdi.messageMappingPropertyMap;
var encodingMenulocalMap = helpersEdi.encodingMenuLocalMap;
var helpersTool = require('./tool/helpers');

var protoMap = helpersEdi.protocolPropertyMap;
...
var ediClass = helpersEdi.EdiClass;
...
function _generateFiles(_, idProcess, opt) {
	...
	try {
					...
					if (res) {
						Object.keys(res).forEach_(_, function(_, filename) {
							if (document && document[filename]) { //concat the content of filename to this existing one with new line
								document[filename].output += _EOL + res[filename];
							} else { // new file
								document[filename] = {
		...
		Object.keys(document).forEach_(_, function(_, filename) {
			...
			if (isSequentialFile && !helpersEdi.endsWith(out[filename].content, _EOL)) {
				// 122645 We add EOL at the end of the file for compatibility with recorded unit tests
				out[filename].content += _EOL;
			}
			var match = false;
