...
function _validateXml(callBack, input, xsdFilePath) {
	xsdValidator.parseFile(xsdFilePath, function(err, schema) {
		if (err) {
			callBack(new Error("xsd validation failed \n" + err));
		}
		console.log("input ", input);
		input && Object.keys(input).forEach(function(key) {
			schema && schema.validate(input[key], function(err, validationErrors) {
				if (validationErrors)
					callBack(new Error("xsd validation failed \n" + validationErrors));
				else
					callBack(null, null);
...
function _createContextXmlFile(_, opt) {
	...
	xmlFile[xmlMap.elem].forEach(function(item, idx, arr) {
				...
				if (lines && lines.length) {
					var idLine = item[xmlMap.elems.idXmlPath];
					if (lines[idLine])
						throw new Error(locale.format(module, "xmlpathNotFound", idLine); lines[idLine] = typeof lines[idLine] === "string" ? JSON.parse(lines[idLine]) : lines[idLine]; fullxmlpath = lines[idLine].path;
							...
							}; elems[fileName] = elems[fileName] || []; elems[fileName].push(element);
						});
