...
//var xsdValidator = require("libxml-xsd").libxmljs;
...
exports.parseXsd = function(_, opt) {
	...
	try {
		...
		console.log("uuud-------------->" + JSON.stringify(opt.xsdUuid))
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		//}
		if (xsdFiles) { // validation of xml
			var schema = xsdValidator.parse(xsd) // parse xsdFiles
			var validationErrors = schema.validate(option.input);
		}
...
function _createContextXmlFile(_, opt) {
	...
	// get cache uuid
	var lines = ediCacheXsdPath && _getXmlPathFile(_, ediCacheXsdPath).lines;
