...
function _executeParseXsd(jsXsds) {
	...
	mapType.root && mapType.root.forEach(function(root) {
		console.log("root " + JSON.stringify(root.$type()));
...
exports.createContextParserXmlFile = function(_, xmlFile, option) {
	...
	option.configParser = {
		parser: "xml",
		elems: _createContextXmlFile(_, xmlFile, mapFilesCode),
	};
	return option;
}

function _createContextParserRawFile(seqFile, option) {
...
exports.validateMapping = function(_, opt) {
		...
		if (opt && typeof opt.xmlFile === "string") {
			xmlFile = getFromCache(_, opt.xmlFile)
		} else {
			xmlFile = opt.xmlFile;
		}

		console.log("xml ", xmlFile);
		// construct context for the prepareMapping
		var option = exports.createContextParserXmlFile(_, xmlFile, opt);
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		if (seqFile) {
			sFile = seqFile;
			sMap = sqMap;
			option = _createContextParserRawFile(seqFile, option);

		} else {
			sFile = xmlFile;
			sMap = xmlMap; //TODO map for xml
			option = exports.createContextParserXmlFile(_, xmlFile, option);
...
function _createContextXmlFile(_, xmlFile, mapFilesCode) {
	var elems = [];
	...
	xmlFile[xmlMap.elem].forEach(function(item, idx, arr) {
		...
		var fullxmlpath = lines[idLine].path;
		console.log("fullxmlpath1 ", lines[idLine]);

		console.log("fullxmlpath2 ", fullxmlpath);
...
exports.createContextSerializerXmlFile = function(_, xmlFile, option) {

	option.configSerializer = {
		serializer: "xml",
		elems: _createContextXmlFile(_, xmlFile, option.fileNames),
	};
...
function _generateFiles(_, idProcess, opt) {
	...
	function _createContextSerializerRawFile(encodingContext, mapFilesCode, opt) {
	...
	try {
		...
		if (uuids.length > 0) {
			...
			if (!xmlFile) {
				...
				option = _createContextSerializerRawFile(encodingContext, opt.fileNames, opt);
			} else {
				encodingContext = xmlFile;
				mapField = xmlMap;
				option = exports.createContextSerializerXmlFile(_, xmlFile, opt);
...
function _createOrderedSchemaList(listXsd, jsonXsd, elem) {
	if (elem) { // add schema dependency
		...
		for (var i = 0; i < nameSpace.length; i++) {
			...
			if (listXsd[nmmspace] && !listXsd[nmmspace].treated) {
				listXsd[nmmspace].treated = true;
				_createOrderedSchemaList(listXsd, jsonXsd, listXsd[nmmspace]);
			}
		}
		jsonXsd.push(elem.schema);
...
exports.parseXsd = function(_, opt) {
	...
	try {
		...
		docs && docs.forEach(function(doc) {
			...
			console.log("list name space ", listNameSpace);
			console.log("target name space ", schema.$["targetNamespace"]);
		...
		console.log("list XSd ", listXsd, 'jsonXsd ', jsonXsd);
		_createOrderedSchemaList(listXsd, jsonXsd);
...
function _getXmlPathFile(_, opt) {
	var edicache = EdiEntity.getEdiCacheEntity(_, opt);
...
exports.getXmlPath = function(_, opt) {
	var lines = _getXmlPathFile(_, opt)
	if (lines) {
