...
function _createContextParserXmlFile(xmlFile, option) {
	var elems = [];
	var prefixExp = {};
	var mapFilesCode = {};
	var keyByFileCode = {};
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		if (seqFile) {
			...
			option = _createContextParserXmlFile(xmlFile, option);
...
function _generateFiles(_, idProcess, opt) {
	...
	function _createContextSerializerXmlFile(_, opt) {
		var elems = [];
		var prefixExp = {};
		var mapFilesCode = {};
		var keyByFileCode = {};
		...
		xmlFile[xmlMap.elem].forEach(function(item, idx, arr) {
			...
			var fileName = mapFilesCode[item[sqMap.elems.fileId]];
			prefixExp[fileName] = prefixExp[fileName] == null ? "" : retrieveCommonPrefix(prefixExp[fileName], exp); // get common path for the file id
			var element = {
				json: item[xmlMap.elems.expression],
				fileName: fileName,
				xml: item[xmlMap.elems.xmlPath],
	...
	function _createContextSerializerRawFile(encodingContext, opt) {
	...
	try {
		...
		if (uuids.length > 0) {
			...
			if (!xmlFile) {
				...
				option = _createContextSerializerRawFile(encodingContext, opt);
			} else {
				encodingContext = xmlFile;
				mapField = xmlMap;
				option = _createContextSerializerXmlFile(xmlFile, opt);
