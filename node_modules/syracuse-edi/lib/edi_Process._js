...
var ediProcessTracer = require('syracuse-core').getTracer("edi.process");
...
function _executeParseXsd(jsXsds) {
	...
	function getType(elem) {
		var ref = elem.$.ref && elem.$.ref.indexOf(":") !== -1 ? {
			type: elem.$.ref.substring(elem.$.ref.indexOf(":") + 1),
			xmlns: elem.$.ref.substring(0, elem.$.ref.indexOf(":"))
		} : {
			type: elem.$.ref,
			xmlns: "target"
		};
		var type = ref ||  (elem.$.type && elem.$.type.indexOf(":") !== -1 ? {
			type: elem.$.type.substring(elem.$.type.indexOf(":") + 1),
			xmlns: elem.$.type.substring(0, elem.$.type.indexOf(":"))
		} : {
			type: elem.$.type,
			xmlns: "target"
		});
	...
	function parseSimpleType(elems, mapSimpleType, prefix, listNameSpaceByPrefix) {
		...
		elems && elems.forEach(function(elem) {
			...
			simpleTypes && simpleTypes.forEach(function(simpleType) {
				...
				var enumeration = [];
				var en = restriction && (restriction[prefix + "enumeration"]);
				if (en && !Array.isArray(en)) {
					en = [en];
				}
				en && en.forEach(function(enume) {
	...
	function parseComplexType(name, elemComplexType, map, prefix, listNameSpaceByPrefix) {
		var atts = elemComplexType[prefix + "attribute"];
		if (atts && !Array.isArray(atts)) {
			atts = [atts];
		}
		atts && atts.forEach(function(elem) {
			map[listNameSpaceByPrefix.target] = map[listNameSpaceByPrefix.target] ||  {};
			map[listNameSpaceByPrefix.target][name] = map[listNameSpaceByPrefix.target][name] || {};
			var type = getType(elem);

			map[listNameSpaceByPrefix.target][name][elem.$.name] = {
				$name: elem.$.name,
				$leafAttribute: true,
				$type: function() {
					console.log("type " + JSON.stringify(type));
					return map[listNameSpaceByPrefix[type.xmlns]][type.type];
				}
			};
			map[listNameSpaceByPrefix.target][name].$complex = true;
			map[listNameSpaceByPrefix.target][name].$name = name;
			//console.log("elem complex 1 "+JSON.stringify(map[listNameSpaceByPrefix.target][name]));

		});
		var seq = elemComplexType[prefix + "sequence"];
		var elems = seq && (seq[prefix + "element"]);
		if (!elems) {
			var all = elemComplexType[prefix + "all"];
			elems = all && (all[prefix + "element"]);
		...
		elems && elems.forEach(function(elem) {
			...
			var elemName = elem.$.name ||  type && type.type;
			map[listNameSpaceByPrefix.target] = map[listNameSpaceByPrefix.target] ||  {};
			map[listNameSpaceByPrefix.target][name] = map[listNameSpaceByPrefix.target][name] || {};
			map[listNameSpaceByPrefix.target][name][elemName] = {
				$name: elemName,
				$type: function() {
					return map[listNameSpaceByPrefix[type.xmlns]][type.type];
				},
				$leafElement: function() {
					return mapType[listNameSpaceByPrefix[type.xmlns]][type.type] && mapType[type.type].$simple;
				}
			};
			map[listNameSpaceByPrefix.target][name].$name = name;
			map[listNameSpaceByPrefix.target][name].$complex = true;
			...
			parseComplexTypeInElem(elem, map, prefix, listNameSpaceByPrefix);

		});

	}
	// return if we have treated a complextype
	function parseComplexTypeInElem(jsXsd, mapComplexType, prefix, listNameSpaceByPrefix) {
		var elements = jsXsd;
		...
		elements && elements.forEach(function(element) {
			...
			if (firstComplexType) { // treat the first complex type and manage all sons
				parseComplexType(element.$.name, firstComplexType, mapComplexType, prefix, listNameSpaceByPrefix);
			}
		});
	...
	function parseElementsRoot(elems, mapComplexeType, prefix, listNameSpaceByPrefix) {
		if (elems && !Array.isArray(elems)) {
			elems = [elems];
		}
		elems && elems.forEach(function(elem) {
			var type = getType(elem);
			mapComplexeType[listNameSpaceByPrefix.target] = mapComplexeType[listNameSpaceByPrefix.target] ||  {};
			mapComplexeType[listNameSpaceByPrefix.target][elem.$.name] = mapComplexeType[listNameSpaceByPrefix.target][elem.$.name] || {
				$name: elem.$.name,
				$type: function() {
					return mapComplexeType[listNameSpaceByPrefix[type.xmlns]][elem.$.name];
				}
			};
			//console.log("elem root "+JSON.stringify(mapComplexeType[listNameSpaceByPrefix.target][elem.$.name]));
			mapComplexeType["root"] = mapComplexeType["root"] ||  [];
			mapComplexeType["root"].push({
				$name: elem.$.name,
				$type: function() {
					return mapComplexeType[listNameSpaceByPrefix[type.xmlns]][elem.$.name];
				}
	...
	function constructList(map, el) {
		...
		while (stack.length) {
			...
			var name = item.name;
			console.log("current " + JSON.stringify(elem, null, 2));
			...
			if (elem && treated.indexOf(elem.$name) === -1) {
				...
				} else if (elem && (elem.$leafElement && elem.$leafElement() || (elem.$type && elem.$type().substring(0, 3) === "xs:"))) { // its  a simple type
	...
	jsXsds && jsXsds.forEach(function(jsXsd) {
		// determine prefix for xsd
		var prefix = _getPrefix(jsXsd);
		//console.log("prefix "+prefix);
		var schema = jsXsd[prefix + "schema"];
		var listNameSpaceByPrefix = {
			target: "target"
		};
		schema.$ && Object.keys(schema.$).forEach(function(attr) {
			if (attr.indexOf("xmlns:") !== -1) {
				listNameSpaceByPrefix[attr.split(':')[1]] = schema.$[attr];
			} else if (attr === "targetNameSpace" || attr === 'xmlns') {
				listNameSpaceByPrefix.target = schema.$[attr];
			}
		});

		var listComplexType = schema[prefix + "complexType"];
		if (listComplexType) { // case of complexType describe in the root of the schema
			...
			listComplexType.forEach(function(complexType) {
				parseComplexType(complexType.$.name, complexType, mapType, prefix, listNameSpaceByPrefix);
			});
		}
		parseComplexTypeInElem(schema[prefix + "element"], mapType, prefix, listNameSpaceByPrefix);

		parseSimpleType(schema, mapType, prefix);

		parseElementsRoot(schema[prefix + "element"], mapType, prefix, listNameSpaceByPrefix);
	...
	mapType.root && mapType.root.forEach(function(root) {
		var l = constructList(mapType, root.$type());
...
function _createOrderedSchemaList(listXsd, jsonXsd, elem) {
	console.log("createOrderedSchemaList")
	if (elem) { // add schema dependency
		for (var i = 0; elem.listNameSpace && i < elem.listNameSpace.length; i++) {
			...
			if (listXsd[nmmspace]) {
				...
				var pref = _getPrefix(mergeXsd);
				// get only the schema content
				var usefulContent = listXsd[nmmspace].rawContent;
				var idx = usefulContent.indexOf("<" + pref + "element")
				var idx = idx < usefulContent.indexOf("<" + pref + "complexType") ? idx : usefulContent.indexOf("<" + pref + "complexType");
				var idx = idx < usefulContent.indexOf("<" + pref + "simpleType") ? idx : usefulContent.indexOf("<" + pref + "simpleType");

				usefulContent = usefulContent.substring(idx - 1, usefulContent.indexOf("</" + pref + "schema"));
				console.log("useful content ", usefulContent);
				mergeXsd = mergeXsd.replace("/<" + pref + "import.*namespace=" + nmmspace + ".*>/", usefulContent);
...
exports.parseXsd = function(_, opt) {
	...
	try {
		...
		opt.xsdUuid && Array.isArray(opt.xsdUuid) && opt.xsdUuid.forEach_(_, function(_, uuid) {
			...
			var rawContent = sa.readAll(_, {
				jsonWhere: {
					$uuid: uuid
				}
			}).toString('utf8');
			doc.content = jsxml.parse(rawContent);
		...
		_createOrderedSchemaList(listXsd, jsonXsd, mergeXsd);
		...
		entity.xsdFiles(_, JSON.stringify(opt.xsdUuid));
...
exports.createContextParserXmlFile = function(_, option) {
	...
	xmlFile[xmlMap.filesDescription] && xmlFile[xmlMap.filesDescription].forEach(function(item) {
		mapFilesCode[item[xmlMap.filesDescriptions.fileId]] = item[xmlMap.filesDescriptions.fileName];
	});
...
exports.validateMapping = function(_, opt) {
	...
	if (opt && typeof opt.xmlFile === "string") {
		...
		opt.xmlFile = opt.xmlFile;
	}


	var option = exports.createContextParserXmlFile(_, opt);
	Object.keys(option.configParser.elems).forEach_(_, function(_, fileName) {
		helpersTool.preprocessMapping(_, option.configParser.elems[fileName], true);
		helpersTool.preprocessMapping(_, option.configParser.elems[fileName], false);
	})
}

function _validateXml(input, xsd) {
	var validationErrors
	var schema = xsdValidator.parse(xsd) // parse xsdFiles
	input && Object.keys(input).forEach(function(key) {
		console.log("xml ", input[key]);
		var validationErrors = schema.validate(input[key]);
		if (validationErrors) {
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		if (seqFile) {
			...
			xsdFile = opt.edicacheXsdPath.getXsdFile(_);
		...
		//}
		if (xsdFile) { // validation of xml
			_validateXml(option.input, xsdFile);
...
function _createContextXmlFile(_, opt) {
	...
	xmlFile[xmlMap.elem].forEach(function(item, idx, arr) {
		...
		if (lines && lines.length) {
			...
			fullxmlpath = lines[idLine].path;

		}
		var fileName = mapFilesCode[item[xmlMap.elems.fileId]];
		var element = {
...
function _generateFiles(_, idProcess, opt) {
	...
	var excep;
	try {
		if (!opt.uuid || !opt.saquentialFile && !opt.messageMapping && !opt.xmlFile || !opt.folder || !opt.protocol) {
...
function _createOrderedSchemaList(listXsd, jsonXsd, mergeXsd, elem) {
	if (elem) { // add schema dependency
		...
		for (var i = 0; i < nameSpace.length; i++) {
			...
			if (item && !item.treated) {
				...
				item.treated = true;
				mergeXsd = _createOrderedSchemaList(listXsd, jsonXsd, mergeXsd, item);
				var usefulContent = item.rawContent;
				var idx = usefulContent.indexOf("<" + item.pref + "element");
				idx = idx < usefulContent.indexOf("<" + item.pref + "complexType") ? idx : usefulContent.indexOf("<" + item.pref + "complexType");
				idx = idx < usefulContent.indexOf("<" + item.pref + "simpleType") ? idx : usefulContent.indexOf("<" + item.pref + "simpleType");
				usefulContent = usefulContent.substring(idx, usefulContent.indexOf("</" + item.pref + "schema>"));
				// convert "/" to \/
				var namespaceRg = nmmspace.replace(new RegExp("/", "g"), "\\/");
				mergeXsd = mergeXsd.replace(new RegExp("<" + elem.pref + "import.*namespace=\"" + namespaceRg + "\".*\\/>", "g"), usefulContent);
			}
		}
		jsonXsd.push(elem.schema);
		return mergeXsd;
		...
		for (var i = 0; listXsd && i < Object.keys(listXsd).length; i++) {
			var current = listXsd[Object.keys(listXsd)[i]];
			if (current && jsonXsd.indexOf(current.schema) === -1 && !current.treated) {
				current.treated = true;
				mergeXsd = current.rawContent;
				mergeXsd = _createOrderedSchemaList(listXsd, jsonXsd, mergeXsd, current);
			}
		}
		return mergeXsd;
	}
...
exports.parseXsd = function(_, opt) {
	...
	function saveXsd(_, opt, xsd) {
		var ddXsd = sa.open(_, null, {
			volume: 'EDI'
		});
		sa.write(_, ddXsd, {
			description: opt.id,
			content: {
				...
				fileName: "xsd_" + opt.id,
			}
		}, xsd, "utf8", true);

		return sa.close(_, ddXsd, true).$uuid;
	}
	try {
		...
		var mergedXsd = _createOrderedSchemaList(listXsd, jsonXsd, mergedXsd);
		// create Document that store the xsd merged files
		var xsduuid = saveXsd(_, opt, mergedXsd)
...
exports.getXmlPath = function(_, opt) {
	...
	if (lines) {
		...
		var k = 0;
		var list = [];
		for (var i = startIndex; lines && i < lines.length && i < count + startIndex; i++) {
