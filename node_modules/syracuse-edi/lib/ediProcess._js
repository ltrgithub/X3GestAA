...
function _executeParseXsd(jsXsds) {
		...
		elems && elems.forEach(function(elem) {
			...
			map[name][elem.$.name] = {
				...
				$leafElement: function() {
					return mapType[elem.$.type] && mapType[elem.$.type].$simple || (elem.$.type && elem.$.type.substring(0, 3) === "xs:");
				}
			};
			map[name].$name = name;
			map[name].$complex = true;
	...
	function constructList(mapType, elem) {
		var stack = [];
		...
		if (elem && elem.$leafAttribute) {
		...
		} else if (elem && (elem.$leafElement && elem.$leafElement())) { // its  a simple type
			...
			//console.log("complex "+JSON.stringify(elem));
			...
			Object.keys(elem) && Object.keys(elem).forEach(function(item) {
				if (["$complex", "$name", "$type"].indexOf(item) === -1) {
					...
					constructList(mapType, elem[item]).forEach(function(elemSub) {
						list.push({
							path: elem.$name + "." + elemSub.path,
							type: elemSub.type,
							restriction: elemSub.restriction
						});
	...
	mapType.root && mapType.root.forEach(function(root) {
		...
		list.concat(constructList(mapType, mapType[root]));
	});
...
exports.parseXSD = function(_, opt) {
	var ediCacheXSD = EdiEntity.getEdiCacheEntity(_, opt);
	var jsonXsd = ediCacheXSD.json(_);
	_parseXsd(jsonXsd);
};
