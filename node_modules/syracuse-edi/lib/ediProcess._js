...
function _executeParseXsd(jsXsds) {
	...
	function constructList(map, el) {
		...
		while (stack.length) {
			...
			console.log("type " + (elem.$type && elem.$type.substring(0, 3)));

			if (elem && elem.$leafAttribute) {
				console.log("$leafAttribute " + JSON.stringify(elem));
				if (father && list[father].length) {
					list[father] && list[father].forEach(function(item) {
						list["final"] = list["final"] ||  [];

						list["final"].push({
							path: item.path + ".$." + elem.$name,
				...
				Object.keys(elem) && Object.keys(elem).forEach(function(item) {
					if (["$complex", "$name", "$type"].indexOf(item) === -1) {
						...
						if (father && list[father].length) {
							list[father] && list[father].forEach(function(item) {
								console.log("itemFather " + JSON.stringify(item));
				...
				stack.push({
					father: father,
					map: mapType,
					elem: mapType[elem.$type],
					list: list
				});
				//return elem && elem.$type && constructList(mapType, mapType[elem.$type]);
			}
		}
		console.log("final " + JSON.stringify(list["final"]));
