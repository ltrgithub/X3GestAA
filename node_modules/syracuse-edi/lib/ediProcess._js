...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		var mapFilesCode = {};
		seqFile.filesDescription && seqFile.filesDescription.forEach(function(item) {
			mapFilesCode[item[seqFile.filesDescriptions.fileId]] = item[seqFile.filesDescriptions.fileName];
		});
		var elems = [];
		seqFile[sqMap.elem].forEach(function(item, idx, arr) {
			elems.push({
				...
				fileName: mapFilesCode[item[sqMap.elems.fileId]]
			});
		});
		option.configParser = {
...
function _generateFiles(_, idProcess, opt) {
	...
	try {
		...
		if (uuids.length > 0) {
			...
			var mapFilesCode = {};
			encodingContext[mapField.filesDescription] && encodingContext[mapField.filesDescription].forEach(function(item) {
			...
			encodingContext && encodingContext[mapField.elem] && encodingContext[mapField.elem].forEach(function(item, idx, arr) {
				elems.push({
					...
					expression: item[mapField.elems.expression],
			...
			option.configSerializer = {
				...
				elems: elems,
				fileName: opt.action === "import" ? "import" + opt.repName : null
			};
		...
		Object.keys(document).forEach_(_, function(_, filename) {
			var filenameext = filename + "." + encodingContext.EXT; // TODO manahe extension for writing file in

			out[filenameext] = document[filename].output;

		});
		_writeIssueFiles(_, protocol, cachedEdi.endPoint(_), out, opt);
