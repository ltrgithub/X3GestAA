...
function _executeParseXsd(_, opt, jsXsds) {
	...
	function constructList(_, opt, map, el, docUuid) {
		var stack = [{
			map: map,
			elem: el,
		}];
		...
		while (stack.length) {
			...
			if (elem && treated.indexOf(elem.$name) === -1) {
				if (elem && elem.$leafAttribute) {
					...
					}, new Buffer(sep + JSON.stringify(obj), "utf8"));
					writeFirstChild = false;
	...
	mapType.root && mapType.root.forEach_(_, function(_, root, idx, arr) {
		constructList(_, opt, mapType, mapType[root], docUuid);
		if (idx < arr.length - 1) {
			sa.write(_, docUuid, {
				description: opt.id,
				content: {
