...
function _writeIssueFiles(_, protocol, seqFile, ep, files, opt, map) {
	...
	try {
		...
		files && Object.keys(files).forEach_(_, function(_, filename) {
			...
			if ((/\w|\d/.test(files[filename].content)) || files[filename].mandatory) {
				sadFsq.writeFile(_, {
...
exports.mvArchiveFiles = _mvArchiveFiles;
/// !doc
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		option.configParser = {
			...
			dateFormat: seqFile[sqMap.dateFormat] && seqFile[sqMap.dateFormat].replace(/A/g, "y").replace(/J/g, "d")
		};
	...
	try {
		...
		if (uuids.length > 0) {
			...
			option.configSerializer = {
				...
				dateFormat: encodingContext[mapField.dateFormat] && encodingContext[mapField.dateFormat].replace(/A/g, "y").replace(/J/g, "d")
