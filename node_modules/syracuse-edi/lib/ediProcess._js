...
function _generateFiles(_, idProcess, opt) {
	...
	try {
			...
			uuids.forEach_(_, function(_, uuid) {
				try {
					...
					if (res) {
						...
						var filenameext = [];
						Object.keys(document).forEach(function(previousValue, currentValue, index, array) {
							filenameext.push(index + "." + encodingContext.EXT);
						});
		...
		Object.keys(document).forEach_(_, function(_, filename) {
			...
			var filenameext = filename + "." + encodingContext.EXT; // TODO manage extension for writing file in
