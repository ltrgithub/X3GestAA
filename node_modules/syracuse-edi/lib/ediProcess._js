	...
	try {
		...
		seqFile[sqMap.elem].forEach(function(item, idx, arr) {
			...
			var element = {
				...
				idLineElem: currentIdLine
			};
			if (item[sqMap.elems.idLine]) { // set to the currentIdLine item to all element to the line
				currentIdLine.offset = element.offset;
				currentIdLine.length = element.length;
				currentIdLine.expression = element.expression;
				currentIdLine.flag = element.flag;
				currentIdLine.isEnd = element.isEnd;
				currentIdLine.fileName = element.fileName;
				currentIdLine.fileId = element.fileId;
				currentIdLine.level = element.level;
				currentIdLine.isMandatory = element.isMandatory;
				currentIdLine.idLine = item[sqMap.elems.idLine];
			}
			if (element.isEnd) { // end of the line, currentIdLine must be new one for the next line
				currentIdLine = {};
			}
			elems.push(element);
		});
