...
function _getReceiptFiles(_, protocol, seqFile, ep, opt) {
	...
	function _checkPatternFileName(name, sf) {
		var fileid = null;
		for (var i = 0; sf && sf[sqMap.elem] && i < sf[sqMap.elem].length && !fileid; i++) { // se base sur le tableau qui decrit la liste des fichiers //TODO check pattern
			var item = sf[sqMap.elem][i][sqMap.elems.fileId];
				...
				try {
					...
					encodingContext[mapField.elem].forEach(function(item, idx, arr) {
						elems.push({
							...
							fileId: item[mapField.elems.fileId]
						});
					});
					...
					option.configSerializer = {
						...
						elems: elems,
						fileName: opt.action === "import" ? "import" + opt.process.x3RepName(_) : null
					};
