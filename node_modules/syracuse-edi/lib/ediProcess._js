...
function _getSadFsq(_, endpoint, opt) {
	...
	return new Sadfsq(_, endpoint.x3server(_).serverHost(_), endpoint.x3server(_).serverPort(_), null, null, null, (opt.sadfsq && opt.sadfsq.recOptions) || {});

}

function _writeIssueFiles(_, protocol, seqFile, ep, files, opt, map) {
	...
	try {
		...
		files && Object.keys(files).forEach_(_, function(_, filename) {
			...
			console.log("filename " + filename + " content '" + files[filename].content + "'  mandatory " + files[filename].mandatory);
			if (files[filename].content || files[filename].mandatory) {
				sadFsq.writeFile(_, {
		...
		for (var i = 0; i < listFile.length; i++) {
			...
			if (fileId) {
				try {
					...
					ediProcessTracer.error && ediProcessTracer.error("can't read file " + listFile[i] + " maybe it's a folder : message " + e.message + " stack " + e.stack);
				}
			}
		}
		return fileBuff;
...
function _mvArchiveFiles(_, protocol, listFile, ep, opt) {
	ediProcessTracer.debug && ediProcessTracer.debug(" _getReceiptFiles protocol = " + (protocol && protocol.PTCCOD) + ", endpoint = " + (ep && ep.dataset(_)));
...
exports.decodeEdiFiles = function(_, idProcess, opt) {
	...
	try {
		...
		if (opt.moveEnd) {
			_mvArchiveFiles(_, protocol, Object.keys(input), cachedEdi.endPoint(_), opt);
		}
		return result;
	} catch (e) {
		// log exception
...
function _generateFiles(_, idProcess, opt) {
	...
	try {
		...
		if (uuids.length > 0) {
			...
			encodingContext && encodingContext[mapField.elem] && encodingContext[mapField.elem].forEach(function(item, idx, arr) {
				...
				elems.push({
					...
					fileName: mapFilesCode && mapFilesCode[item[mapField.elems.fileId]],
		...
		Object.keys(document).forEach_(_, function(_, filename) {
			...
			var match = false;
			for (var i = 0; i < Object.keys(mandatoryFile) && !match; i++) {
				var patternFile = Object.keys(mandatoryFile)[i];
				match = new RegExp(patternFile).test(filename);
				if (match) {
					out[filename].mandatory = mandatoryFile[filename] === 2;
				}
			}
