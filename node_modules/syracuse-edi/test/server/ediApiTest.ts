...
var protocolDirectory = require('../../lib/protocolHandler/directoryProtocol');
...
describe(module.id, () => {
	...
	function getNewSadFsq(_, endpoint, recOptions) {
		return new SadFsqClient(_, endpoint, null, null, false, {}, recOptions);
	}
	...
	it('start test environment ', function(_) {
	...
	// t
	it('test retrieve cache from a process ', function(_) {
		_initPrototypeCache(_, db);
	...
	it('cache process id context identifier', function(_) {
		_initPrototypeCache(_, db);
	...
	it('read receipt file', function(_) {
		_initPrototypeCache(_, db);
		...
		strictEqual(Object.keys(buffList).length === 6, true, " list file to read (nbreaded=[" + JSON.stringify(Object.keys(buffList)) + "] ok");
	...
	it('write issue file', function(_) {
		_initPrototypeCache(_, db);
	...
	it('decode file API', function(_) {
		...
		uuidList.forEach_(_, function(_, item) {
			...
			strictEqual(JSON.stringify(entityGen.json(_)), contents[cnt++], "json generate from files ok");
		...
		uuidList.forEach_(_, function(_, item) {
			...
			strictEqual(JSON.stringify(entityGen.json(_)), contents[cnt++], "json generate without cache from files ok");
		...
		var uuidList = EdiProcess.decodeEdiFiles(_, "idProcessNoCustomFolder", option);
		strictEqual(uuidList != null, true, "decoded file without cache ok and return a uuid = " + JSON.stringify(uuidList));
		// check if the json generate exists
		cnt=0;
		uuidList.forEach_(_, function(_, item) {
	...
	it('generate EDI file API', function(_) {
		_initPrototypeCache(_, db);
	...
	it('generate import file API', function(_) {
		_initPrototypeCache(_, db);
	...
	it('generate import file API with failure during pushed files', function(_) {
		_initPrototypeCache(_, db);
	...
	it('test update edi entity ', function(_) {
		// clean
		EdiEntity.dropAllEdiCacheEntity(_, {
			db: db
		});
	...
	it('test parsing xsd file with storage area', function(_) {
		...
		console.log(JSON.stringify(resStored))
		strictEqual(JSON.stringify(resStored), '{"startIndex":1,"count":20,"elems":[{"id":1,"path":"shiporder.item[0..n].price","type":"decimal","restriction":{"base":"xs:decimal"}},{"id":2,"path":"shiporder.item[0..n].quantity","type":"positiveInteger","restriction":{"base":"xs:positiveInteger"}},{"id":3,"path":"shiporder.item[0..n].note","type":"string","restriction":{"base":"xs:string"}},{"id":4,"path":"shiporder.item[0..n].title","type":"string","restriction":{"base":"xs:string"}},{"id":5,"path":"shiporder.shipto.country","type":"string","restriction":{"base":"xs:string"}},{"id":6,"path":"shiporder.shipto.city","type":"string","restriction":{"base":"xs:string"}},{"id":7,"path":"shiporder.shipto.address","type":"string","restriction":{"base":"xs:string"}},{"id":8,"path":"shiporder.shipto.name","type":"string","restriction":{"base":"xs:string"}},{"id":9,"path":"shiporder.orderperson","type":"string","restriction":{"base":"xs:string"}},{"id":10,"path":"shiporder.$.orderid","type":"string","restriction":{"base":"xs:string","pattern":"[0-9]{6}"}}],"totalResult":10}', "result ok");
	...
	it('decode/Generate Edi File Xml File', function(_) {
        ...
        let caString = fs.readFileSync(require('path').join(__dirname, 'data/testsig.crt'), "utf8");
        ...
        let keyString = fs.readFileSync(require('path').join(__dirname, 'data/testsig.key'), "utf8");
		...
		var option = {
				...
				db: db,
				unittest: true,
				fileNames: "FACTURAE_S.xml",
				uuid: [inst.$uuid],
				keepCache: true,
					//moveEnd : true
				signature: {
					certificate: "testsig_test",
					path: [""],
					includeKeyInfo: true
				}
			};
			recOptions.fileName = "generateFileXmlrSig21.json";
			var uuidList = EdiProcess.generateEdiFiles(_, "idProcessXml2", option);
			var endpoint = endpointTest;
			recOptions.fileName = "checkGenerateXmlFileSig.json";
			var sadfs = getNewSadFsq(_, endpointTest, recOptions);
			var read = sadfs.readFile(_, {
				path: "/home1/adonix/GX3APP/TMP/TESTAP/ISSUE/FACTURAE_S.xml"
			}, {
				flag: "r",
				encoding: "utf-8"
			});
			strictEqual(read.indexOf("SignatureValue>nQBvpqqIRTKde/ZFmGuHe1oJiyPi") > 0, true, "Correct signature");
	...
	it('test Facturae - ', function(_) {
		...
		var prototype = JSON.parse(readFile("context/facturaeTestProto.json", 'utf-8', _));
		...
		/* work in online mode but not offline. check recorder or player
			...
			strictEqual(inst != null, true, "update 2 entity : instance exists in mongo (updated ok)");
			strictEqual(inst.$uuid, uuidList[0].uuid, "update 2 entity : uuid  " + inst.$uuid);
			strictEqual(inst.type(_), "idProcessXmlFacturae", "update 2 entity : type  " + inst.type(_));
			strictEqual(inst.id(_), uuidList[0].id, "update 2 entity : id  " + inst.id(_));
			strictEqual(inst.json(_) != null, true, " json generate ok " + JSON.stringify(inst.json(_)));
		*/
