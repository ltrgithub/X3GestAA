...
asyncTest("parser fixed size ", function(_) {
	...
	elems = [];
	mmFile[mmMap.elem].forEach(function(item, idx, arr) {
		var classInstname = seqFile[mmMap.classInstance] || null;
		var exp = item[mmMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
		elems.push({
			...
			fileName: item[mmMap.elems.fileId] === "C" ? "CABFAC" : "LINFAC"
		});
	});
	configParser = {
		parser: IMPORTFILE_LIB[mmFile[mmMap.fileType]],
		sepDecimal: mmFile[mmMap.decimalSep],
		sepField: mmFile[mmMap.fieldSep],
		sepRecord: "\r\n",
		delimField: mmFile[mmMap.fieldDelimiter],
		elems: elems
	};
...
asyncTest("serialize import file ", function(_) {
	...
	messageMapping[mmMap.elem].forEach(function(item, idx, arr) {
		var classInstname = messageMapping[sqMap.classInstance] || null;
		var exp = item[mmMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
...
asyncTest("parser", function(_) {
	...
	seqFileJson[sqMap.elem].forEach(function(item, idx, arr) {
		var classInstname = seqFileJson[sqMap.classInstance] || null;
		var exp = item[sqMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
	...
	seqFileJsonReal[sqMap.elem].forEach(function(item, idx, arr) {
		var classInstname = seqFileJsonReal[sqMap.classInstance] || null;
		var exp = item[sqMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
	...
	seqFileJsonDelim[sqMap.elem].forEach(function(item, idx, arr) {
		var classInstname = seqFileJsonDelim[sqMap.classInstance] || null;
		var exp = item[sqMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
...
asyncTest(" serialize import files", function(_) {
	...
	messageMappingJson[mmMap.elem].forEach(function(item, idx, arr) {
		var classInstname = messageMappingJson[mmMap.classInstance] || null;
		var exp = item[mmMap.elems.expression];
		if (classInstname && exp.indexOf(classInstname) === 0) { // first elem is the class inst
			exp = exp.substring(exp.indexOf(".") + 1);
		}
