...
asyncTest("parser fixed size ", function(_) {
	...
	var res = parser.mapParse["fixedLength"](fileBuff, seqFile.ESFESFD, prototype, sContext);
	...
	res = parser.mapParse["fixedLength"](fileBuff, seqFile.ESFESFD, prototype, sContext);
	...
	res = parser.mapParse["fixedLength"](fileBuff, seqFile.ESFESFD, prototype, sContext, res);
...
asyncTest("parser", function(_) {
	...
	uuid[0] = EdiEntity.createEdiCacheEntity(_, {
		json: seqFileJsonReal,
		type: EdiType.SEQFILE,
		id: "testReal",
		etag: "etagBidon",
		db: db
...
asyncTest(" serialize edi file", function(_) {
	...
	try {
		...
		ok(false, "ERROR CASE : serialize on a non exists seqential file");

	} catch (e) {
		ok(true, "ERROR CASE : serialize on a non exists seqential file mess:" + e.message);
	}
...
asyncTest(" serialize import files", function(_) {
	...
	try {
		...
		ok(false, "ERROR CASE : serialize on a non exists message mapping");

	} catch (e) {
		ok(true, "ERROR CASE : serialize on a non exists message mapping mess:" + e.message);
	}
