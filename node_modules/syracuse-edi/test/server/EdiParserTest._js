...
function _testError(_, data, code, toXML) {
	console.log("------------------ TEST " + code)
	try {
		helpersTool.preprocessMapping(_, data, toXML)
...
asyncTest("test with prototype", 10, function(_) {
	var prototype = {
		$properties: {
			val: {
...
asyncTest("different mappings test", 18, function(_) {
	var mappings = [{
		xml: "A.B",
		json: "u"
	}, {
	...
	strictEqual(JSON.stringify(map), JSON.stringify({
		...
		"u": ["A.B", "10", null, ["a", 6, "ab'cd"], "A.C", "0", null, ["a", 2, 'ab"cd'], "A.D", "0", null, ["a", 3, "abcd"]]
	}), "several");
	var map = helpersTool.preprocessMapping(_, [{
		xml: "A.B",
		json: "u",
...
asyncTest("parse xml file", function(_) {
	...
	var option = EdiProcess.createContextParserXmlFile(_, xmlFile, {})
	...
	var option = EdiProcess.createContextSerializerXmlFile(_, xmlFile, {
		fileNames: {
