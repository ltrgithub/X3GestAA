...
function createContextParserXmlFile(xmlFile) {
	var elems = [];
	var prefixExp = {};
	var mapFilesCode = {};
	var keyByFileCode = {};
...
asyncTest("converter test", 15, function(_) {
	var mappings = [{
		xml: "A.L",
		json: "u",
...
asyncTest("serialized fixed size ", function(_) {
	...
	var res = serializer.mapSerialize["fixedLength"]({
		json: json,
...
asyncTest("serialize import file ", function(_) {
	...
	var res = serializer.mapSerialize["delimited"]({
		json: json,
		configSerializer: configParser,
		prototype: prototype
	});
	...
	res = serializer.mapSerialize["sepField"]({
		json: json,
		configSerializer: configParser,
		prototype: prototype
	});
	...
	res = serializer.mapSerialize["sepRecord"]({
		json: json,
		configSerializer: configParser,
		prototype: prototype
	});
...
asyncTest("parse xml file", function(_) {
	...
	// store in mongodb all needed to perform the test
	var xmlFile = JSON.parse(fs.readFile(_defDataDir + "context/xmlFileTest.json", 'utf-8', _));
	var input = {
		"FACTURAE.xml": jsxml.parse(fs.readFile(_defDataDir + "ediFiles/xmltest.xml", 'utf-8', _))
	};

	var configParser = createContextParserXmlFile(xmlFile);
	var protocolJson = JSON.parse(fs.readFile(_defDataDir + "context/protocol.json", 'utf-8', _));
