...
function createContextSerializer(mm, mapField, isImport) {
	// create map between file name and file code
	var mapFilesCode = {};
	mm[mapField.filesDescription] && mm[mapField.filesDescription].forEach(function(item) {
		mapFilesCode[item[mapField.filesDescriptions.fileId]] = item[mapField.filesDescriptions.fileName];
	});
...
asyncTest("parser fixed size ", function(_) {
	...
	fileBuff["CABFAC"] = fs.readFile(_defDataDir + "ediFiles/CABFAC1", 'utf-8', _);
	...
	configParser = createContextParser(mmFile, mmMap);
...
asyncTest("serialized fixed size ", function(_) {
	...
	var configParser = createContextParser(seqFile, mmMap);
	...
	var configParser = createContextParser(mmFile, mmMap);
	var res = serializer.mapSerialize["fixedLength"](json, configParser, prototype);
...
asyncTest("serialize import file ", function(_) {
	...
	result = "E,ASN,CDE-2010-0491,LS,21/09/2010,ASN,EUR,L,CD100,Camion semi remorque rouge,Un,300000,L,ARTICLE2,Désignation article 2,Un,L,ARTICLE2,Désignation article 2,Un,L,ARTICLE4,ARTICLE4,Un";
	...
	result = "E,ASN,CDE-2010-0491,LS,21/09/2010,ASN,EUR\nL,CD100,Camion semi remorque rouge,Un,300000\nL,ARTICLE2,Désignation article 2,Un\nL,ARTICLE2,Désignation article 2,Un\nL,ARTICLE4,ARTICLE4,Un";
...
asyncTest("parser", function(_) {
	...
	var configParser = createContextSerializer(seqFileJson, sqMap, false);
	var fileBuff = {};
	fileBuff["CABFAC"] = fs.readFile(_defDataDir + "ediFiles/CABFAC.txt", 'utf-8', _);
	fileBuff["LINFAC"] = fs.readFile(_defDataDir + "ediFiles/LINFAC.txt", 'utf-8', _);
	var jsonSIH = {
	...
	var configParser = createContextSerializer(seqFileJsonReal, sqMap, false);
...
asyncTest(" serialize import files", function(_) {
	...
	var configParser = createContextSerializer(messageMappingJson, mmMap, true);
	var protocolJson = JSON.parse(fs.readFile(_defDataDir + "context/protocol.json", 'utf-8', _));
