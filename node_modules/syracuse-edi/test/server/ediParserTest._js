...
asyncTest("parser fixed size ", function(_) {
	var fileBuff = {};
	fileBuff["CABFAC"] = fs.readFile(_defDataDir + "CABFAC.txt", 'utf-8', _);
	fileBuff["LINFAC"] = fs.readFile(_defDataDir + "LINFAC.txt", 'utf-8', _);
	fileBuff["OBSFAC"] = fs.readFile(_defDataDir + "OBSFAC.txt", 'utf-8', _);

	var opt = {};
	var seqFile = fs.readFile(_defDataDir + "seqFile.json", 'utf-8', _);

	opt.seqFile = JSON.parse(seqFile);
	opt.isSizeFixed = true;
	opt.json = {};
