...
asyncTest("test parsing xsd file with storage area", function(_) {
	...
	strictEqual(JSON.stringify(resStored), '{"startIndex":1,"count":20,"elems":[{"id":1,"path":"shiporder.item[0..n].price","type":"xs:decimal","restriction":{"base":"xs:decimal"}},{"id":2,"path":"shiporder.item[0..n].quantity","type":"xs:positiveInteger","restriction":{"base":"xs:positiveInteger"}},{"id":3,"path":"shiporder.item[0..n].note","type":"xs:string","restriction":{"base":"xs:string"}},{"id":4,"path":"shiporder.item[0..n].title","type":"xs:string","restriction":{"base":"xs:string"}},{"id":5,"path":"shiporder.shipto.country","type":"xs:string","restriction":{"base":"xs:string"}},{"id":6,"path":"shiporder.shipto.city","type":"xs:string","restriction":{"base":"xs:string"}},{"id":7,"path":"shiporder.shipto.address","type":"xs:string","restriction":{"base":"xs:string"}},{"id":8,"path":"shiporder.shipto.name","type":"xs:string","restriction":{"base":"xs:string"}},{"id":9,"path":"shiporder.orderperson","type":"xs:string","restriction":{"base":"xs:string"}},{"id":10,"path":"shiporder.$.orderid","type":"xs:string","restriction":{"base":"xs:string","pattern":"[0-9]{6}"}}],"totalResult":10}', "result ok");
	...
	strictEqual(resStored.totalResult, 443, " size of xml path ok");
	strictEqual(resStored.count, 20, " size of xml path ok");


	var jsXsd = readFile("context/facturaeSignature.xsd", 'utf-8', _);
	...
	sa.write(_, doc, {
		...
		content: {
			contentType: "application/xml",
			fileName: "facturaeSignature.xsd",
		}
	}, new Buffer(jsXsd, "utf8"));
	...
	console.log("parse xsd uuid ", uuid);
	strictEqual(resStored.totalResult, 460, " size of xml path ok");
	strictEqual(resStored.count, 20, " size of xml path ok");
...
asyncTest("decode Edi File Xml File", function(_) {
	...
	// store in mongodb all needed to perform the test
	var xmlFile = JSON.parse(readFile("context/xmlFileTest2.json", 'utf-8', _));
	// var messageMappingJson = readFile("context/messageMapping.json", 'utf-8', _);
	var protocolJson = JSON.parse(readFile("context/protocol.json", 'utf-8', _));
	var prototype = JSON.parse(readFile("context/EDISIHFA_$details.json", 'utf-8', _));
