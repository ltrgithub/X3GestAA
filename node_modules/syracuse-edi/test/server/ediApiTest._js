...
asyncTest("decode file API", function(_) {
	...
	strictEqual(entityGen != null, true, "json generate without cache exists in syracuse mongodb database identify by uuid = " + uuid);
	...
	var uuid = EdiProcess.decodeEdiFiles(_, "idProcess", option);
	strictEqual(uuid != null, true, "decoded file with non exists file ok and return a uuid = " + uuid);
	var entityGen = EdiEntity.getEdiCacheEntity(_, {
		uuid: uuid,
		db: db
	});
