...
asyncTest("decode file API", function(_) {
	...
	var uuid = EdiProcess.decodeFiles(_, "idProcess", option);
	...
	option.className = "NOTEXISTCLASS";
	var uuid = EdiProcess.decodeFiles(_, "idProcess", option);
...
asyncTest("decode file API with init cache ", function(_) {
	var recOptions = {
		recMode: ediSadUnitConfig.online ? "REC" : "PLAY",
		fileName: "decodeFile2.json",
		path: "node_modules/syracuse-edi/test/server/data/sadrec"
	};


	var option = {
		messageMapping: "INVOIC~10",
		sequentialFile: "INVOIC~10~300",
		protocol: "PDIRECTORY",
		sadfsq: {
			recOptions: recOptions,
			user: "apisu",
