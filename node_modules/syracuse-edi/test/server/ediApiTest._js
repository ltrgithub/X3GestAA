...
asyncTest("API store json", function(_) {
	...
	EdiEntity.removeEdiCacheEntity(_, uuids2);
	strictEqual(EdiEntity.getEdiCacheJson(_, {
		uuid: uuids2
	}) === null, true, "remove EdiCacheEntity json from a uuid=[" + JSON.stringify(uuids2) + "] ok ");

	EdiEntity.removeEdiCacheEntity(_, uuids);
	strictEqual(EdiEntity.getEdiCacheJson(_, {
		uuid: uuids
	...
	EdiEntity.removeEdiCacheEntity(_, "test uuid bidon");
	...
	EdiEntity.removeEdiCacheEntity(_, uuidsSeqFile);
...
asyncTest("test retrieve cache from a process ", function(_) {
	...
	var ediProcess = EdiProcess.createEdiProcess(_, "idProcessTest", {
		messageMapping: "test",
		sequentialFile: "test",
		protocol: "test",
		ediFlow: "test",
	...
	//delete all entity
	EdiProcess.removeEdiProcess(_, "idProcessTest");
	EdiEntity.removeEdiCacheEntity(_, uuid);
...
asyncTest("read receipt file", function(_) {
	...
	var buffList = EdiProcess.getReceiptFiles(_, protocolJson, seqFileJson, {}); // for unit test
	strictEqual(buffList.length === 3, true, " list file to read (nbreaded=[" + buffList.length + "] ok");
	start();
});
