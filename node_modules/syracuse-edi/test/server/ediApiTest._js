...
asyncTest("test parsing xsd file", function(_) {
	...
	var list = EdiProcess.executeParseXsd(jsXsd);
	...
	var list = EdiProcess.executeParseXsd([jsXsd1, jsXsd2]);
...
asyncTest("test parsing xsd file with storage area", function(_) {
	...
	// ensure the file are deleted after test
	doc1 && doc1.deleteSelf(_);

	doc2 && doc2.deleteSelf(_);

	var list = fs.readdir(_defDataDir + "context/ubl/", 'utf-8', _);
	var docs = [];
	list && list.filter(function(item) {
		return item.indexOf("xsd") !== -1;
	}).forEach_(_, function(_, item) {
		var jsXsd = fs.readFile(_defDataDir + "context/ubl/" + item, 'utf-8', _);
	...
	// ensure the file are deleted after test
	docs.forEach_(_, function(_, uuid) {
		var doc = sa.getDocumentInstance(_, {
			jsonWhere: {
				$uuid: uuid
			}
