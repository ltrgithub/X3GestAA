...
asyncTest("API store json", function(_) {
	...
	try {
		uuids = EdiEntity.createEdiCacheEntity(_, {
			json: ["test with bad data", "test2", "test3"],
			type: "sas",
			id: "testbidonArray",
			db: db
		});
...
asyncTest("cache process id context identifier", function(_) {
	...
	var protocolJson = JSON.parse(fs.readFile(_defDataDir + "context/protocolWithFolder.json", 'utf-8', _));
...
asyncTest("write issue file", function(_) {
	...
	var endpoint = adminHelper.getEndpoint(_, {
		dataset: protocolJson.folder
	});

	EdiProcess.writeIssueFiles(_, protocolJson, null, output, opt); // for unit test
...
asyncTest("decode file API", function(_) {
		...
		uuid.push(EdiEntity.createEdiCacheEntity(_, {
			...
			var uuid = EdiProcess.decodeEdiFiles(_, "idProcessWithoutCache", option);

			strictEqual(uuid != null, true, "decoded file without cache ok and return a uuid = " + uuid);
			// check if the json generate exists
			var entityGen = EdiEntity.getEdiCacheEntity(_, {
				uuid: uuid,
				db: db
			}); strictEqual(entityGen != null, true, "json generate without cache exists in syracuse mongodb database identify by uuid = " + uuid); strictEqual(JSON.stringify(entityGen.json(_)), '{"NUM":"FCC11014VEN00000014","BPR":"ESP0001","CPY":"110","FCY":"C110","BPAPAY":"A01","CUR":"EUR","PTE":"CHEQ100COMPTANT","AMTATI":75880,"AMTNOT":66000,"SIVTYP":"FAC","ACCDAT":"2014-02-24","ESIH1SID":[{"NUM":"FCC11014VEN00000014","SIDLIN":1000,"ITMREF":"CDROM","ITMDES":"CD-ROM 16X","SAU":"Un","QTY":3,"GROPRI":1000000,"NETPRI":1000000,"AMTLIN":30000},{"NUM":"FCC11014VEN00000012","SIDLIN":2000,"ITMREF":"CDROM1","ITMDES":"CD-ROM1 16X","SAU":"Un","QTY":2,"GROPRI":1000000,"NETPRI":1000000,"AMTLIN":20000}]}', "json generate without cache from files ok");
			...
			var uuid = EdiProcess.decodeEdiFiles(_, "idProcess", option); strictEqual(uuid != null, true, "decoded file with non exists file ok and return a uuid = " + uuid);
			var entityGen = EdiEntity.getEdiCacheEntity(_, {
				uuid: uuid,
				db: db
			}); strictEqual(entityGen == null, true, "json generate exists in syracuse mongodb database identify by uuid = " + uuid);
	...
	asyncTest("genrate EDI file API", function(_) {
		_initPrototypeCache(_, db);
		...
		EdiProcess.generateEdiFiles(_, "idProcess", option);
		...
		EdiProcess.generateEdiFiles(_, "idProcessWithoutCache", option);
		// check if the file generate exists and is ok
		var endpoint = endpointTest;
		...
		start();
	}); asyncTest("genrate import file API", function(_) {
		_initPrototypeCache(_, db);
		...
		var fileName = EdiProcess.generateImportFiles(_, "idProcess", option);
		// check if the file generate exists and is ok
		var endpoint = endpointTest;
		recOptions.fileName = "checkGenerateImport.json";
		var sadfs = new SadFsqClient(_, endpoint.x3server(_).serverHost(_), endpoint.x3server(_).serverPort(_), "apisu", "apisu", false, recOptions);
		var read = sadfs.readFile(_, {
			path: "/home1/adonix/GX3APP/TMP/TESTAP/import/" + fileName[0]
		}, {
			flag: "r",
			encoding: "utf-8"
		});
		...
		sadfs.unlink(_, {
			path: "/home1/adonix/GX3APP/TMP/TESTAP/import/" + fileName
		});
		...
		var fileName = EdiProcess.generateImportFiles(_, "idProcessWithoutCache", option);
		// check if the file generate exists and is ok
		var endpoint = endpointTest;
	...
	asyncTest("genrate import file API with failure during pushed files", function(_) {
		_initPrototypeCache(_, db);
		...
		strictEqual(doc.length != 0, true, "document saved in storage area");
		strictEqual(sa.readAll(_, {
			jsonWhere: {
				$uuid: doc.$uuid
			}
		}).toString('utf-8'), fileBuff, "document saved content ok");
		...
		var fileName = EdiProcess.generateImportFiles(_, "idProcessFailed", option);
		strictEqual(fileName != null, true, "recall generate to write file : " + fileName + " ");
		var doc = sa.getDocumentInstance(_, {
			sdataWhere: "description eq idProcessFailed"
		}, "EDI");
