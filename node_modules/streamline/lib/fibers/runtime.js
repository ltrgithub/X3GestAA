...
function invoke(that, fn, args, options) {
	...
	if (typeof fn !== 'function') {
		if (typeof that === 'function' && that.fstreamlineFunction && fn === 'call') {
			return applyFast(that.fstreamlineFunction, args[0], args.slice(1));
		}
		fn = that[fn];
	}
	if (fn.fstreamlineFunction) {
		return applyFast(fn.fstreamlineFunction, that, args);
	}
	return invokeSlow(that, fn, args, options, idx);
}

function invokeSlow(that, fn, args, options, idx) {
...
Function.prototype.apply_.fstreamlineFunction = function(dummy, thisObj, args, index) {
	if (this.fstreamlineFunction) {
		args = Array.prototype.slice.call(args, 0);
		args.splice(index != null && index >= 0 ? index : args.length, 0, dummy);
		return this.fstreamlineFunction.apply(thisObj, args);
	} else {
