"use strict";
var fs = require('fs');
var os = require('os');
var util = require("util");
var t = require('streamline/deps/narcissus').definitions.tokenIds;
var Narcissus = require('streamline/deps/narcissus');
var project = "node_modules/syracuse-collaboration";

var local = JSON.parse(fs.readFileSync(project + "/lib/resources/strings-en.json", "utf8"));
// console.log("ANZ "+Object.keys(local).length)
var target = {};
var EOL = os.EOL;

function toJSON(object, indent) {
	var result = "{" + EOL;
	var indentInner = (indent || "") + "\t";
	var init = true;
	Object.keys(object).sort().forEach(function(key) {
		if (init) init = false;
		else result += "," + EOL;
		result += indentInner + '"' + key + '": ';
		var value = object[key];
		if (value instanceof Object) {
			result += toJSON(value, indentInner);
		} else {
			result += '"' + value.toString().replace(/(["\\])/g, "\\$1") + '"';
		}
	});
	// final part
	result += (init ? "" : "" + EOL) + (indent || "") + "}";
	return result;
}


function file(filepath) {
	var r = /([^\\\/]+\.)_?js$/.exec(filepath);
	if (!r) throw new Error("Wrong path " + filepath);
	var relevant = {};
	var entname = r[1];
	var source = fs.readFileSync(filepath, "utf8");
	var index = 0;
	if (source.indexOf("\n") >= 0) {
		source = source.replace(/\r/g, "");
	}
	r = /^(\s*)exports\.entity\s*\=\s*/m.exec(source);
	if (!r) {
		// console.log("No entity "+filepath);
		return;
	}
	console.log("Entity " + filepath);
	// console.log("ANF "+entname)
	Object.keys(local).forEach(function(item) {
		if (item.indexOf(entname) === 0 && item.length > entname.length) {
			target[item] = relevant[item] = local[item];
		}
	});
	entname = entname.substr(0, entname.length - 1);
	// console.log("REL "+util.format(relevant))
	index = r.index + r[1].length;
	var result = source.substring(0, index);
	// source = source.replace(/^"use strict";/g, "")
	source = source.substr(index);
	// source = "(function(exports){\n"+source+"})()";
	index = 0;
	// console.error("ANFANG "+util.format(new Buffer(source.substring(0, 300))))
	var parsed = Narcissus.parser.parse(source, filepath);
	if (parsed.type !== t.SCRIPT) throw new Error("Wrong type");
	parsed = parsed.children[0];
	if (parsed.type !== t.SEMICOLON) throw new Error("Wrong type2");
	/*
	parsed = parsed.expression;
	if (parsed.type !== t.CALL) throw new Error("Wrong type21");
	parsed = parsed.children[0];
	if (parsed.type !== t.FUNCTION) throw new Error("Wrong type22");
	parsed = parsed.body; 
	if (parsed.type !== t.SCRIPT) throw new Error("Wrong type23");
	parsed = parsed.children[0]; 
	if (parsed.type !== t.SEMICOLON) throw new Error("Wrong type24");
	*/
	parsed = parsed.expression;
	if (parsed.type !== t.ASSIGN) throw new Error("Wrong type3");
	parsed = parsed.children[1];


	function findTitle(parsed, path, next_start0, mode) {
		if (parsed.type !== t.OBJECT_INIT) throw new Error("Wrong type4");

		for (var i = 0; i < parsed.children.length; i++) {
			var next_start = (i + 1 < parsed.children.length ? parsed.children[i + 1].start : next_start0);
			var child = parsed.children[i];
			var val;
			if (child.type === t.PROPERTY_INIT) {
				var prop = child.children[0].value;
				switch (child.children[1].type) {
					case t.OBJECT_INIT:
						if (prop === '$properties' || prop === '$relations') {
							findTitle(child.children[1], path, next_start);
						} else {
							findTitle(child.children[1], path + "." + prop, next_start);
						}
						break;
					case t.ARRAY_INIT:
						if (prop === '$enum') {
							var arrChildren = child.children[1].children;
							for (var j = 0; j < arrChildren.length; j++) {
								var chld = arrChildren[j];
								var next_start1 = (j + 1 < arrChildren.length ? arrChildren[j + 1].start : next_start);
								// console.log("ZZZ "+next_start1+" "+next_start+" "+u[chld.type]);

								var p = findTitle(chld, path, next_start1, 1);
								findTitle(chld, path + "." + p, next_start1, 2);
								// console.log("XXXy "+path+"."+p+"="+q+" ");
								// console.log(chld.end+" "+next_start1+"DANACH <<"+chld.tokenizer.source.substring(chld.end, next_start1)+">>")

							}
						}
						break;
					case t.UNARY_MINUS:
						try {
							val = "-" + child.children[1].children[0].value;
							if (mode === 1 && prop === '$value') {
								return val;
							}
						} catch (e) {
							console.error("XXX " + util.format(child));
							throw e;
						}
						break;
					case t.NUMBER:
						val = child.children[1].value;
						if (mode === 1 && prop === '$value') {
							return val;
						}
						break;
					case t.STRING:
						val = child.children[1].value;
						if (mode === 1 && prop === '$value' || prop === '$default') {
							// if (mode === 1 && prop === '$value') {
							return val;
						}
						if (mode !== 1 && /^\$\w*(default|description|[Tt]itle|Template)$/.test(prop)) {
							// test whether there is translatable content
							var val1 = val;
							var temp;
							var endpos, inbetween;
							// remove all placeholders
							while ((temp = val1.replace(/\{\w+\}/g, "")) !== val1) {
								val1 = temp;
							}
							// console.log("1. "+val+" - "+val1);
							// when only whitespace remains, do not put it into strings-en.json
							if (!/^\s*$/.test(val1)) {
								var key;
								if (mode === 2) {
									key = path;
								} else {
									key = path + '.' + prop;
								}
								// console.log("XXX "+key+"="+child.children[1].value);
								if (key in relevant) {
									// target[key] = relevant[key];
									if (child.children[1].value !== relevant[key]) {
										console.log("  " + key + " in entity: <<" + child.children[1].value + ">>, in strings-en.json: <<" + relevant[key] + ">>");
									}
								} else {
									target[key] = child.children[1].value;
								}
								val = target[key].replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
								endpos = child.end;
								inbetween = child.tokenizer.source.substring(endpos, next_start);
								if ((r = /^\s*,/.exec(inbetween))) {
									// result += "JJ";
									endpos += r[0].length;
									inbetween = inbetween.substr(r[0].length);
								}
								// result += "<<<<"+val+"|"+inbetween+">>>>"
								if ((r = /^[ \t]*\/\/[ \t]*LOC\:[ \t]*\"((?:\\.|[^"])*)\"[ \t]*/.exec(inbetween))) {
									// result += "$<<"+r[1]+"|"+val+">>$";
									if ((r[1] !== val)) {
										result += child.tokenizer.source.substring(index, endpos);
										result += ' // LOC: "' + val + '" ';
										result += inbetween.substr(r[0].length);
										index = next_start;
									}
								} else if (/^[ \t]*\/\//.test(inbetween) || /^[ \t]*[\r\n]/.test(inbetween)) {
									result += child.tokenizer.source.substring(index, endpos);
									result += ' // LOC: "' + val + '" ';
									result += inbetween;
									index = next_start;
								} else { // insert line feed
									var lastSpaces = /([\r\n]\s*)[^\s]*$/.exec(child.tokenizer.source.substring(index, endpos));
									result += child.tokenizer.source.substring(index, endpos);
									result += ' // LOC: "' + val + '" ';
									result += lastSpaces ? lastSpaces[1] : EOL;
									result += inbetween;
									index = next_start;
								}
								// console.log(child.end+" "+next_start+"DANACH <<"+child.tokenizer.source.substring(child.end, next_start)+">>")
							} else
							// entfernen
								endpos = child.end;
							inbetween = child.tokenizer.source.substring(endpos, next_start);
							var rem = inbetween.replace(/^(\s*,)?[ \t]*\/\/[ \t]*LOC\:[ \t]*\"(?:\\.|[^"])*\"[ \t]*/, "$1");
							if (rem !== inbetween) {
								result += child.tokenizer.source.substring(index, endpos);
								result += rem;
								index = next_start;
							}
							// console.log("Kein Inhalt "+path+'.'+prop+"="+child.children[1].value);
						}
						break;
				}
			}
		}
	}

	findTitle(parsed, entname, source.length);
	result += parsed.tokenizer.source.substring(index);
	result = result.replace(/\n/g, EOL);
	fs.writeFileSync(filepath, result, "utf8");

	// console.log("LLLLL "+result)
	// console.log("LLLLL "+util.format(target))
}

function scan(f) {
	if (f.indexOf('/test/') >= 0 || //
		f.indexOf('/dotnet/') >= 0 || //
		f.indexOf('node_modules/tools/') >= 0 || //
		f.indexOf('/junk/') >= 0) return;
	var stat = fs.lstatSync(f);
	if (stat.isDirectory()) {
		var files = fs.readdirSync(f);
		files.forEach(function(n) {
			if (n !== '.git' && n !== '.svn') scan(f + "/" + n);
		});
	} else if (!stat.isSymbolicLink()) {
		var ext = f.substring(f.lastIndexOf('.')).toLowerCase();
		if (ext === '.js' || ext === '._js') {
			try {
				file(f);
			} catch (e) {
				console.error("FEHLER" + e.stack);
			}
		}
	}
}

scan(project);
// write strings-en.json
// console.log("ENDE "+util.format(target))

fs.writeFileSync(project + "/lib/resources/strings-en.json", toJSON(target, ""), "utf8");
// var local = JSON.parse(fs.readFileSync(project+"/lib/resources/strings-en.json", "utf8"));
