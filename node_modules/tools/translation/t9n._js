...
var commandMap = {
	...
	"--get": {
		...
		sample: "--get http://aws-syrindambas:8124/ -a erbou:ERBOU -d X3INDAMBAS --chp 10031..10032 --lan FRA --tstamp 2010-01-01T00:00:00Z --out translation-indexes/messages_@@.txt",
		exec: function get(_, opts) {
			...
			if (opts["--fix"]) {
				...
				set.load(data);
				var fix = new MessageSet();
				set.autoFix(function(entry, lan, tu, proposal) {
					var id = entry.$origin.split("/");
					if (!tu[lan]) {
						if (!fix.get(id[0], id[1], "XXX")) {
							fix.init(id[0], id[1], "XXX", entry.$key, getStamp(), "");
						}
						fix.init(id[0], id[1], lan, proposal, getStamp(), "");
					}
				});
				if (out && out.indexOf("@@")) {
