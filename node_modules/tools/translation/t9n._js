...
var walk = translationHelpers.walk;
...
var commandMap = {
	...
	"--get": {
		...
		exec: function get(_, opts) {
			...
			if (opts["--dic"]) {
				var outputDir = fsp.dirname(out);
				_genDictionary(_, outputDir, endpoint);
			}
		}
	},
	"--gen-dic": {
		...
		exec: function genDic(_, opts) {
			var input = opts["--input"];
			var inputFile = fsp.join(cwd, input);
			var outputDir = opts["--out"] || fsp.dirname(inputFile);
			...
			Object.keys(data).forEach_(_, function(_, l) {
				var name = fsp.join(outputDir, "-" + l + ".txt");
	...
	"--master-index": {
		exec: function test(_, opts) {
			// @sage/etna/lib/engine/runtime/resources/format
			// ==> etna/engine/runtime/format
			// @sage/syracuse-lib/src/collaboration/resources/helpers
			// ==> s-lib/collaboration/helpers
			// syracuse-hrm/lib/resources/hrmDispatch
			// ==> s-hrm/hrmDispatch
			// syracuse-ui/lib/field/helpers/resources/format
			// ==> s-ui/field/helpers/format
			// ui-visual-process/public/resources/player
			// ==> ui-visual-process/player
			...
			_updateMasterIndex(_, null, opts["--type"], opts)
		}
	},
	"--test": {
		exec: function test(_, opts) {
			...
			opts["--dir"] && console.error("resolved dir=" + fsp.resolve(opts["--dir"]));
			var dir = opts["--dir"] ? fsp.resolve(opts["--dir"]) : fsp.resolve(__dirname, "../../");
			...
			walk(_, dir, visitor = {
				...
				visitFile: function(_, file, path, level) {
					...
					var g, gn, u;
					if (m) {
						gn = _normalizeGroup(m[1].substring(this._rootDir.length + 1));
	
						u = this.$uniqueKeys[gn] || (this.$uniqueKeys[gn] = { path:m[1] });
						if (u.path !== m[1])
							console.error("duplicate keys:\n  " + path + "\n  " + u.path);
						// if (this.$groups[gn] == null) console.error(new Array(level+1).join(" ") + gn + " " + m[2]);
						// if (this.$groups[gn] == null) console.error(new Array(level+1).join(" ") + gn);
			...
			console.error("groups: " + Object.keys(visitor.$groups).join("\n"));
	...
	rtype.forEach_(_, function(_, t) {
		...
		translationHelpers.traverseIndex(index, function(file, unit, txt) {
			...
			if (!txt) {
				...
				console.error("             : entry=" + entry.$key + ", " + entry.$origin);
				if (!entry) {
...
function _genDictionary(_, outputDir, endpoint) {
	var dic = readJson(_, fsp.join(outputDir, "dictionary-en-us.json"));
	...
	dictionary.traverse(function(k, v) {
		Object.keys(v.text).forEach(function(l) {
			var iso = isolangs[l]
			u = data[iso] = data[iso] || (data[iso] = {
			});
			if (v.text[l].length > 0) {
				var d = u[k] = u[k] || {};
				d[iso] = d[iso] || [];
				v.text[l].forEach(function(e) {
					console.log("e:", e);
...
var _normalizeGroup = translationHelpers.normalizeGroup;
var _getMapping = translationHelpers.getMapping;

function _updateMasterIndex(_, set, type, opts) {
	// 1. Traverse the index
	// 2. For every entries
	//    a. add a mapping for the sha1 key to the actual index key
	opts = opts || {};
	var out = opts.out;
	// var set = new MessageSet();
	var rtype = (type || "syracuse,dotnet,admin").split(",");
	...
	var indexDir = fsp.join(dir, "translation-indexes");
	var masterIndexFile = fsp.join(opts["--out"] || indexDir, "masterIndex.json");
	var masterIndex = readJson(_, masterIndexFile);
	var mkeys = masterIndex.keys || (masterIndex.keys = {});
	var mapping = masterIndex.mapping || {};

	console.error("_updateMasterIndex: file:", masterIndexFile, ", keys:" + !!mkeys);
	rtype.forEach_(_, function(_, t) {
		var indexFile = fsp.join(indexDir, "index" + t + ".json");
		var index = readJson(_, indexFile);
		var current = {}, entry, h0, h1;
		translationHelpers.traverseIndex(index, function(file, unit, txt) {
			// console.error("==> file=" + (file && file[0]) + ", unit=" + (unit && unit[0]) + ", txt=" + (txt && txt[0]));
			if (txt || !file || file[0][0] === "$") return;
			var key, id, m;

			// group level
			if (!unit) {
				key = file[1].$key;
				id = _getMapping(mapping, _normalizeGroup(file[0]));
				entry = {
					gid: id,
					// paths: [[file[0]]] 
				};
				m = mkeys[key];
				current.group = m;
				h1 = _hash(file[0], true);
				if (!m) {
					m = current.group = mkeys[key] = entry;
				} else 	if (m.gid != entry.gid) {
					console.error("[ERROR] group collision, bad mapping! skip new. \nkey:", key, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
					return;						
				}

				// path is an array of pair [path, hash] where hash not defined for the original (first) element
				// if (m && m.paths[0][0] != file[0]) {
				// 	if (m.gid != entry.gid) {
				// 		console.error("[ERROR] group collision, bad mapping! skip new. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
				// 		return;						
				// 	}
				// 	var fi = m.paths.findIndex(function (p) {
				// 		return p[0] == file[0];
				// 	});
				// 	current.group = entry = m;
				// 	if (fi < 0) {
				// 		// not found
				// 		h0 = _hash(m.paths[0][0], true);
				// 		h1 = _hash(file[0], true);
				// 		entry.paths.push([file[0], h1]);
				// 		console.error("[INFO] group collision, add new. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
				// 	} else {
				// 		// found: compare existing hash
				// 		h0 = m.paths[fi][1];
				// 		h1 = _hash(file[0], true);
				// 		if (h0 != h1)
				// 			console.error("[ERROR] group collision bad existing hash. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
				// 	}
				// }
				// if (!m) {
				// 	h1 = _hash(file[0], true);
				// 	if (h1 != key) {
				// 		console.error("[WARNING] group replace hash. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
				// 		entry.paths[0][1] = h1;
				// 	}
				// 	current.group = mkeys[key] = entry;
				// }
				return;			
			}
			...
			if (unit[0][0] === "$") return;
			key = unit[1].$key;
			id = _getMapping(mapping, current.group.gid + "/" + unit[0]);
			var path = file[0] + "/" + unit[0];
			entry = {
				uid: id,
				// paths: [[path]]
			};
			m = mkeys[key];
			h1 = _hash(path);
			var txt = utils.cleanString(unit[1].text["en-us"]);
			if (!m) {
				// entry.paths.push([file[0], h1]);
				entry.text = txt;
				m = mkeys[key] = entry;
			} else if (m.uid != entry.uid) {
				if (m.text === txt) {
					console.error("[WARNING] unit collision, bad mapping but same text, auto add mapping. \nkey:", key, "\nh1 :", h1, "\nnew:", entry, "\nold:", m);
					mapping[m.uid] = entry.uid;
					m.uid = entry.uid
				}
				else {
					console.error("[ERROR] unit collision, bad mapping! skip new. \nkey:", key, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ntext:'", txt + "'");	
					console.error("==> old:", m.text && Buffer.from(m.text).toString("hex"));
					console.error("==> new:", txt && Buffer.from(txt).toString("hex"));
				}
			}

			// path is an array of pair [path, hash] where hash not defined for the original (first) element
			// if (m && m.paths[0][0] != path) {
			// 	if (m.uid != entry.uid) {
			// 		console.error("[ERROR] unit collision, bad mapping! skip new. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
			// 		return;						
			// 	}
			...
			// 	if (fi < 0) {
			// 		// not found
			// 		h0 = _hash(m.paths[0][0]);
			// 		h1 = _hash(path);
			// 		entry.paths.push([path, h1]);
			// 		console.error("[INFO] unit collision, add new. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
			// 	} else {
			// 		// found: compare existing hash
			// 		h0 = m.paths[fi][1];
			// 		h1 = _hash(path);
			// 		if (h0 != h1)
			// 			console.error("[ERROR] unit collision bad existing hash. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
			// 	}
			// }
			// if (!m) {
			// 	h1 = _hash(path);
			// 	if (h1 != key) {
			// 		console.error("[WARNING] unit replace hash. \nkey:", key, "\nh0 :", h0, "\nh1 :", h1, "\nnew:", entry, "\nold:", m, "\ncurrent:", current);
			// 		entry.paths[0][1] = h1;
			// 	}
			// 	mkeys[key] = entry;
			// }
	...
	mkeys = masterIndex.keys;
	masterIndex.keys = {};
	var list = Object.keys(mkeys).map(function (k) {
		return [k, mkeys[k]];
	}).sort(function (a, b) {
		return a[1].path === b[1].path ? 0 : (a[1].path > b[1].path ? 1 : -1);
	}).forEach(function (v) {
		masterIndex.keys[v[0]] = v[1];
	});

	fs.writeFile(masterIndexFile, JSON.stringify(masterIndex, null, "\t"), "utf8", _);
}

function _hash(path, isGroup) {
	// if (key==null)
	// 	return sha1(group);
	// return sha1(group + "/" + key.split("/").slice(-1));
	if (isGroup)
		return sha1(path);
	var parts = path.split("/");
	return sha1(sha1(parts.slice(0,-1)) + "/" + parts.slice(-1));
}
