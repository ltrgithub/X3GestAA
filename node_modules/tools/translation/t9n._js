...
var TranslationProcess = require('syracuse-translation/lib/translation').TranslationProcess;
...
var commandMap = {
	...
	"--trans": {
		...
		exec: function trans(_, opts) {
			var auth = _auth(opts);
			var endpoint = opts["--endpoint"] || "X3INDAMBAS";
			if (!auth.user || !auth.password)
				usage("user/password are mandatory");
			if (!endpoint)
				usage("endpoint is mandatory");
			...
			var topt = {
				$tracking: tracking,
				// $addDiagnosis:null
				rootpath: (opts["--dir"] || ".") && fsp.resolve(opts["--dir"]) || "d:/workspaces/syracuse",
				resources: {},
				endpointRead: ep,
				endpointWrite: ep,
				noDBFlush: true,
			...
			ops.forEach_(_, function(_, op) {
				...
				console.error(tracking.map(function(t) {
					return "[" + t.$severity.toUpperCase() + "] " + t.$message;
				}).join("\n"));
			});
		}
	},
	...
	"--transform": {
		syntax: "--transform -- <message file>",
		sample: "--transform -- translation-indexes/output/messages.txt",
		exec: function transform(_, opts) {
			var input = opts["--input"];
			var cwd = process.cwd();
	...
	"--help": {
		exec: function help() {
			...
			console.log("  " + syntax("--get"));
			console.log("  " + syntax("--trans"));
			console.log("  --verify");
			console.log("  --tmx");
			console.log("  --help");
			process.exit(0);
