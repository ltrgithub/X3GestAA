...
var commandMap = {
	...
	"--get": {
		...
		exec: function get(_, opts) {
			...
			if (fixopt) {
				...
				set.traverse(function(context, chp, id, lan, text, meta) {
					...
					if (loc[deflang] == null) {
						...
						return;
					} else if (!messageStore.isEqualPlaceHolder(text)) {
					...
					} else if (!meta.lanori && text != null && (text === loc[deflang] || messageStore.isEqualPlaceHolder(text))) {
			...
			if (file) {
				lines = [];
				var re = /(\w+)\t(\d+)\t(\d+)\t(\w*)\t([\d\-T:Z]*)\t([\s\S]*?)[\n\r]+/g;
				utils.parsePlainContents(data, re, function(chp, id, lan, text, stamp, lanori) {
					lines.push(utils.formatLine(chp, id, lan, text, lanori));
				});
				fs.writeFile(file, lines.join("\n"), "utf8", _);
			} else {
