...
function World(context) {
	...
	this.getDeliveryFiles = function(_) {
		var session = context.session;
		var resp = session.get(_, "/sdata/syracuse/collaboration/syracuse/resourcePacks(code eq DELIVERY)?count=200");
		var files = (resp && resp.body && resp.body.items || []).map(function (e) {
			return e.fileName;
		});

		console.error("getDeliveryFiles:\n\t" + files.join('\n\t'));
	};
