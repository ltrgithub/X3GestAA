...
function toJSON(object, indent) {
	...
	Object.keys(object).sort().forEach(function(key) {
		...
		if (value instanceof Object) {
			...
			result += '"' + value.toString().replace(/(["\\])/g, "\\$1").replace(/\r?\n/g, "\\n") + '"';
		}
	});
	// final part
	result += (init ? "" : "\n") + (indent || "") + "}";
...
function treatFile(content, filepath, changed) {
	...
	if (RESOURCE_CHANGED in tmpResourceContents) { // check whether locale is defined
		...
		if (!/locale\s*\=\s*require\s*\(\s*([\"\'])((?:@sage\/)?syracuse-core\1\)\.locale|streamline-locale\1\))/.test(content)) {
			var r;
			var index = 0;
			...
			content = content.substr(0, index) + 'var locale = require("streamline-locale");\n' + content.substr(index);
		}
	}
	...
	for (var i = 0; i < lines.length; i++) {
		...
		if (line.search(/^\s*((var|const)\s+?)(\w+)\s*\=\s*require\s*\(\s*['"]((?:@sage\/)?syracuse-core['"]\)\.locale|streamline-locale['"]\))/) >= 0 ||
			/import \* as locale from 'streamline-locale'/.test(line)) {
			decl = true;
		}
		var r;
...
exports.checkLocalization = function _checkLocalization(content, filepath, changed) {
	...
	function culture(l) {
		return l == "en" ? "en-us" : (l && l.length === 2 ? l + "-" + l : l);
	}

	changed.messages = changed.messages || [];

	var match = /-(\w\w(-\w\w)?)\.json$/.exec(filepath);
	if (!match)
		return;
	var loc = culture(match[1].toLowerCase());
	var dirname = path.dirname(filepath);
	var groupName = filepath.replace(/-\w\w(-\w\w)?\.json$/, ""); // without locale and extension

	var indexContent;
	var checkChange = false;
	var pos = dirname.indexOf("/node_modules/");
	var indexFile = "translation-indexes/indexSyracuse.json";
	try {
		indexContent = indexContent || JSON.parse(fs.readFileSync(indexFile, "utf8"));
		checkChange = true;
	} catch (e) {
		mess(null, "indexFile " + indexFile + " does not contain valid JSON");
	}
	if (!checkChange) return;

	var group = indexContent[groupName];
	if (group == null) return;
	var rejected = [];
	Object.keys(content).forEach(function(key) {
		var value = content[key];
		var text = group[key] && group[key].text && group[key].text[loc]; 
		// mess(null, "diff=" + (text && text != value) + " on " + key + '="' + value + '" / "' + text + '"');
		if (text && text != value) {
			rejected.push({key:key,change:value,current: text});
		}
	});
	if (rejected.length > 0){
		mess(null, "The following Localized text" + (rejected.length > 1 ? "s are" : " is") + " under translation control and need to be changed with the translation tools:");
		rejected.forEach(function (r) {
			mess(null, "[" + r.key + '] "' + r.change + '" <<>> "' + r.current + '"');
		})
	}
};
