...
function treatFile(content, filepath, changed) {
	...
	if (RESOURCE_CHANGED in tmpResourceContents) { // check whether locale is defined
		...
		if (!/locale\s*\=\s*require\s*\(\s*([\"\'])(syracuse-core\1\)\.locale|streamline-locale\1\))/.test(content)) {
			var r;
			var index = 0;
	...
	for (var i = 0; i < lines.length; i++) {
		...
		if (line.search(/^\s*((var|const)\s+?)(\w+)\s*\=\s*require\s*\(\s*['"](syracuse-core['"]\)\.locale|streamline-locale['"]\))/) >= 0) {
			decl = true;
		}
		var r;
