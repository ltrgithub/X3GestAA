...
function checkFile(filepath, buf) {
	...
	if ("-m" in options) { // convert messages
		...
		if (RESOURCE_CHANGED in tmpResourceContents) { // check whether locale is defined
			...
			if (!/locale\s*\=\s*require\s*\(\s*([\"\'])(syracuse-core\/lib\/|streamline-)locale\1/.test(content)) {
				var index = 0;
	...
	for (var i = 0; i < lines.length; i++) {
		...
		if (line.search(/^\s*((var|const)\s+?)(\w+)\s*\=\s*require\s*\(\s*['"](syracuse-core\/lib\/|streamline-)locale['"]\s*\)/) >= 0) {
			decl = true;
		}
