...
exports.run = function(_, exporter, entity) {
	...
	var names = executeSql(_, "select " + mainKey + "_0 from " + entity.tableName + " order by " + mainKey + '_0').map(function(elt) {
		return elt[mainKey];
	}).filter(function(s) {
	...
	names.forEach_(_, exporter.config.parallel || 4, function(_, name, i) {
		...
		var data = executeSql(_, "select " + mainColumns + " from " + entity.tableName + " where " + mainKey + "_0=:1", //
			[name || ''])[0];
