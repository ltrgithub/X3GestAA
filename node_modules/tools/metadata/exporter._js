...
exports.newExporter = function(config) {
	...
	function buildExecute(_, select, tableName, orderBy, elt, where) {
		if (!Array.isArray(select) && !~select.indexOf('_0')) select = buildKey(select);
		if (!where && elt) where = Object.keys(elt).join(',');
		return executeSql(_, "select " + select + " from " + tableName //
		  + (where ? " where " + buildKey(where,1) : '') + (orderBy ? " order by " + buildKey(orderBy) : ''), //
		  (elt ? Object.keys(elt).map(function(c) {
				return elt[c] || '';
			}) : []));
	}
	...
	return {
		...
		init: function(_) {
			...
			var folderType = parseInt(buildExecute(_, "VALEUR", "ADOVAL", null, {"PARAM": "TYPDOS"})[0].VALEUR_0, 10);
		...
		getTableColumns: function(_, tableName) {
			...
			return buildExecute(_, "CODZONE,DIME,CODTYP", "ATABZON", "CODZONE", {'CODFIC': tableName}).map(function(elt) {
				var arr = [];
		...
		objVals: function(elt, objectTypes) {
			return Object.keys(elt).map(function(k) {
