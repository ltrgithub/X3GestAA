...
exports.newExporter = function(config) {
	...
	return {
		...
		buildKey: buildKey,
		buildExecute: function(_, elt, select, tableName, keys, orderBy) {
			return executeSql(_, "select distinct " + select + " from " + tableName //
			  + (keys ? " where " + buildWhere(keys) : '') + (orderBy ? " order by " + buildKey(orderBy) : ''), //
			  (keys ? keys.map(function(c) {
					return elt[c] || '';
				}) : []));
		},
