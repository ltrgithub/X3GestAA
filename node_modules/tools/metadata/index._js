"use strict";

var config = require('../../../nodelocal').config;

if (!config.metadata) throw new Error("config.metadata missing");

function run(_) {

	var exporter = require("tools/metadata/exporter").newExporter(config.metadata);
	exporter.clean(_);
	exporter.init(_);
	require("tools/metadata/export-tables").run(_, exporter);
	require("tools/metadata/export-masks").run(_, exporter);
	require("tools/metadata/export-windows").run(_, exporter);
}

run(_);