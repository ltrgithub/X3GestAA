...
function getResource(_, path) {
	if (program.test) {
		return fs.exists(__dirname + path + program.test + "._js", _) ? [require("." + path + program.test)] : [];
	} else {
...
if (program.export || !program.argv.slice(2).length) runExporter(_);
if (program.import) runImporter(_);

var gitR = program.git ? "nodegit" : program.jsgit ? "jsgit" : program.nodegit ? "nodegit" : null;
if (gitR) {
	var git = require("tools/metadata/" + gitR).newGit(config.metadata);
	git.gitify(_);
}
