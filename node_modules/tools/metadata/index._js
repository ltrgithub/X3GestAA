...
function runExporter(_) {
	...
	if (!Object.keys(tester).length || tester.clean) exporter.clean(_);
	exporter.init(_);
...
function runGit(_) {
	var git = require("tools/metadata/jsgit").newGit(config.metadata);
	git.gitify(_);
}
...
program
  ...
  .option('-t, --test <test>', 'test single export entity or text')
  .option('-g, --git', 'git repo commit')
  .option('-i, --import', 'import into store')
  .parse(process.argv);

tester.clean = program.clean;
tester.resource = program.test;

program.git ? runGit(_) : program.import ? runImporter(_) : runExporter(_);
