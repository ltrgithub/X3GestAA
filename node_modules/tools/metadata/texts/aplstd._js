...
exports.run = function(_, exporter) {
	...
	var TEXT_ELTS = executeSql(_, "select LANCHP_0 from APLSTD order by LANCHP_0");
	...
	TEXT_ELTS.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		...
		var data = executeSql(_, "select " + APLSTD_COLUMNS + " from APLSTD where LANCHP_0=:1 order by LAN_0,LANNUM_0", //
		 	[elt.LANCHP_0 || 0]);
