...
exports.run = function(_, exporter) {
	...
	TEXT_ELTS.forEach_(_, exporter.config.parallel || 4, function(_, elts, i) {
		trace && trace((i + 1) + "/" + TEXT_ELTS.length + ": exporting text definition " + elts.NUMERO_0 + " " //
		 + elts.LAN_0 + " " + objectTypes[elts.TYPOBJ_0] + " " + elts.NOMOBJ_0);

		var data = executeSql(_, "select " + ATEXTE_COLUMNS + " from ATEXTE where NUMERO_0='" //
				+ elts.NUMERO_0 + "' and LAN_0='" + elts.LAN_0 + "'")[0];
		if (entities[elts.TYPOBJ_0] && fs.exists("../entities/" + entities[elts.TYPOBJ_0] + "._js")) {
			var entity = require("../entities/" + entities[elts.TYPOBJ_0]).entity;
			mergeInto(data, executeSql(_,"select " + // merge objects
				(entities[elts.TYPOBJ_0] === "parameters" ? "CHAPITRE_0,CODACT_0" : "MODULE_0,CODACT_0") //
				+ " from " + entity.tableName + " where " + entity.primaryKey + "='" + elts.NOMOBJ_0 + "'")[0]);
		}
		
		exporter.writeResource(!_, {
			path: (exporter.moduleNames[data.MODULE_0] || data.CHAPITRE_0 ? "GLOBAL/" + data.CHAPITRE_0 : "GLOBAL") //
				+ "/" + (data.CODACT_0 || "STD") + "/" + objectTypes[data.TYPOBJ_0] + "/" + elts.LAN_0,
			name: elts.NOMOBJ_0,
			data: data
		});
	});
