...
exports.run = function(_, exporter) {
	...
	TEXT_ELTS.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		trace && trace((i + 1) + "/" + TEXT_ELTS.length + ": exporting text definition " + objectTypes[elt.TYPOBJ_0] + " " + elt.NOMOBJ_0);
		...
		var data = executeSql(_, "select " + ATEXTE_COLUMNS + " from ATEXTE where TYPOBJ_0=:1 " //
				+ " and NOMOBJ_0=:2 order by LAN_0", [elt.TYPOBJ_0 || 0, elt.NOMOBJ_0 || '']);
		...
		Object.keys(bylan).forEach_(_, function(_, lan) {
			exporter.writeResource(!_, {
				path: (meta ? (exporter.moduleNames[meta.MODULE_0] || (meta.CHAPITRE_0 ? ("GLOBAL/" //
				 ...
				 + "/" + (elt.TYPOBJ_0 ? objectTypes[elt.TYPOBJ_0] : "MISC") + "/" + lan,
				name: (elt.NOMOBJ_0 ? elt.NOMOBJ_0 : "_ALL_"),
				data: bylan[lan]
			});
		})
