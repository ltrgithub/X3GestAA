...
exports.run = function(_, exporter, text) {
	...
	elts.forEach_(_, exporter.config.parallel || 4, function(_, elt, i) {
		// trace && trace((i + 1) + "/" + elts.length + ": exporting " + text.title + ": " + exporter.objVals(elt, text.objectTypes));
		...
		case 'ATEXTRA':
			var meta = buildExecute(_, 'MODULE,CODACT', 'ATABLE', null, elt, 'CODFIC');
			if (!meta) return;
			res = {
		...
		case 'ATEXTE':
			...
			if (entities[elt.TYPOBJ] && fs.exists("./entities/" + entities[elt.TYPOBJ] + "._js")) {
				...
				var meta = buildExecute(_, mainKey, entity.tableName, null, elt, entity.primaryKey)[0];
			}
			res = {
