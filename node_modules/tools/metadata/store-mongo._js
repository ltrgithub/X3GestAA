...
exports.newStore = function(config, files, readFile) {
	...
	function cleanupData(data) {
		...
		return Object.keys(data).reduce(function(r, k) {
			...
			if (v && typeof v === "object")
				r[k] = Array.isArray(v) ? v.map(cleanupData) : cleanupData(v);
			else if (i > 0) {
	...
	return {
		...
		fillTexts: function(_) {
			...
			var names = Object.keys(files.TEXTS || {});
			var coln = db.collection("ATEXTE", _);
			names.forEach_(_, function(_, name) {
				var data = readFile(_, files.TEXTS[name]);
				data = cleanupData(data);
				coln.insert([data], _);
			});
