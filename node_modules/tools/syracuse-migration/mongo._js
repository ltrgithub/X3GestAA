...
exports.Client = helpers.defineClass(function(options) {
	...
	if (options.ssl) {
		...
		if (options.sslCAFile) this.connectOptions.server.sslCA = fs.readFileSync(options.sslCAFile);
		if (options.sslCAFile) this.connectOptions.server.sslPass = options.sslPEMKeyPassword;
		if (options.sslPEMKeyFile) {
			var key = fs.readFileSync(options.sslPEMKeyFile);
			this.connectOptions.server.sslKey = key;
			this.connectOptions.server.sslCert = key;
		}
	}
}, null, {
	connect: function(_) {
