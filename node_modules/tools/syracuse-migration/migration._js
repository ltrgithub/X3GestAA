...
function updateEndpoints(_) {
	utils.log("*************** Endpoints transformations ***************", "cyan");
	config.endpoints && config.endpoints.forEach_(_, function(_, e) {
		...
		if (old && old.length) {
			...
			utils.log("INFO: Endpoint '" + oldEpName + "' updated with new configuration '" + e.dataset + "'...", "green");
		} else {
...
function updateHost(_) {
	...
	if (h && h.hostnameToModify && h.hostname && h.tcpHostName) {
		...
		if (old && old.length) {
			...
			utils.log("INFO: Host '" + h.hostnameToModify + "' updated with new configuration '" + h.hostname + "'...", "green");
			//
		...
		utils.log("ERROR: 'config.host' section have to be defined... 'hostnameToModify', 'hostname' and 'tcpHostName' properties are mandatory to change Host instance settings", "magenta");
	}
}
