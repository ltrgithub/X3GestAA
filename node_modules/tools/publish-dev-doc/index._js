...
function publish(_) {
	...
	var dest = __dirname + "/../../../../online-doc/en-US/V7DEV";
	var pubdir = __dirname + "/../../../../online-doc/public";
	...
	function doIt(_, s, f) {
		...
		if (stat.isDirectory()) {
			...
			if (pub) pubs.push(f);
			var transformed = transform(f, source);
			fs.writeFile(dest + '/' + f.replace(/\.md$/, '.html'), transformed, 'utf8', _);
			if (pub) fs.writeFile(pubdir + '/' + f.replace(/\.md$/, '.html'), transformed, 'utf8', _);
