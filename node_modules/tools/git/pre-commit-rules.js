...
var asiCount = 0;
var errCount = 0;
var checkLocaleSync = require('../check/checkLocaleSync');
var changedFiles = [];
var checkVariableSync = require('../check/checkVariableSync');
...
function acceptFile(f) {
	...
	if (ext !== '.json' && ext !== '.js' && ext !== '._js' && ext !== '.ts' && !isTextFileExtension(ext)) return false;
	if (f.indexOf('/cvgRecords/') >= 0) return false;
	if (f.indexOf('node_modules/tools/') >= 0) return false;
	if (f.indexOf('node_modules/bundles/') < 0 && f.indexOf('syracuse-') < 0 && 
		f.indexOf('etna-') < 0 && 
		f.indexOf('node_modules/jsx509/') < 0 && 
		f.indexOf('._js') < 0 && 
		f.indexOf("node_modules/license-tool") < 0) return false;
...
exports.processFileSync = function(file) {
	...
	if (/require\s*\([^\)]+\._?js["']\s*\)/.test(source)) error(file+ ': no require with extension .js or ._js');
	...
	if (errCount !== errCountOld || exports.do_not_change)
		return;
	// beautify
	var beautified = source;
	if (!/\.ts$/.test(file)) beautified = beautify(source, {
		indent_char: '\t',
		indent_size: 1,
	});
	...
	if (false && !JSHINT(source, jshintrcConf)) {
