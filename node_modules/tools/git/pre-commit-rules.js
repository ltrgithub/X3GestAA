...
exports.processFileSync = function(file) {
	...
	if (!/^\uFEFF?["']use strict["'];/.test(contents.substring(0, contents.indexOf('\n')))) error(file + ': "use strict"; missing at top of file (must be first line)');
	if (/^[^\/\"\']*debugger/m.test(contents)) error(file + ': contains active "debugger" statement');
	if (/^(<<<<<<<|=======|>>>>>>>)/m.test(contents)) error(file + ': unresolved merge conflict (search for <<<<, ==== , >>>>)');
	if (/\bthrow\s+\(*\s*[\'\"]/m.test(contents)) error(file + ': throw "..." detected - change it to throw new Error("...")');
	// FOR NOW RETURN HERE - WILL ENABLE BEAUTIFY LATER
	return;
