...
function acceptFile(f) {
	...
	if (f.indexOf('syracuse-') < 0 && f.indexOf('/node_modules/tools/') < 0 && f.indexOf('node_modules/streamline-auth/') < 0) return false;
...
exports.processFileSync = function(file, do_not_change) {
	...
	// variable check
	/* commented out until clear criteria for client side JavaScript file paths have been found
	try {
	...
	*/	
	...
	try {
		var changed = { allowChange: ((errCount === errCountOld) && !do_not_change)};
	...
	// */
	if (errCount !== errCountOld || do_not_change)
		return;
	// beautify
...
function execute(error, stdout, stderr) {
	...
	if (changedFiles.length) {
		...
		child_process.exec('git add '+files, execute);
	} else {
		process.exit(errCount > 0 || beautifyCount > 0 || asiCount > 0);		
	}	
}
