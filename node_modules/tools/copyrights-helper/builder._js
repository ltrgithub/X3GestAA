...
function processPackages(_) {
	...
	fs.writeFile(path.join(__dirname, "output-packages.json"), JSON.stringify(output,null,2), "utf8", _);
...
function processDeps(_, packs) {
	try {
		packs = packs || JSON.parse(fs.readFile(path.join(__dirname, "output-packages.json"), "utf8", _));
	} catch (e) {
	...
	fs.writeFile(path.join(__dirname, "output-deps.json"), JSON.stringify(output,null,2), "utf8", _);
