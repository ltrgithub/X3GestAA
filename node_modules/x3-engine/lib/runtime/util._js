...
exports.sourceLoc = { line: 0 };


exports.badOperand = function(x) {
	return new X3Error(10, "bad operand: " + x + " (type " + typeof(x) + ")");
}

exports.badArgumentType = function(x) {
	return new X3Error(10, "bad argument: " + x + " (type " + typeof(x) + ")");
}

exports.checkNumber = function(x) {
	if (typeof x !== "number") throw exports.badArgumentType(x);
}

exports.checkInteger = function(x) {
	if (typeof x !== "number" || x % 1 !== 0) throw exports.badArgumentType(x);
}
...
exports.checkNotZero = function(x) {
	if (x == 0) throw new X3Error(53);
}

exports.instructions = {
	C: function(x) {
		var fn = function c$do(_, frame) {
			//console.log(x);
			return x;
		};
		fn.value = x;
	...
	A: function(items, forget) {
		return function a$do(_, frame) {
			var results = !forget && [];
			for (var i = 0; i < items.length; i++) {
				var item = items[i];
				var v = item.value !== undefined ? item.value : item(_, frame);
	...
	NIY: function(node) {
		return function niy$do(_, frame) {
