...
exports.functions = {
	ERRL: {
		...
		fn: function() {
			var cx = this.context;
			return cx.err && cx.err.frames[0] ? cx.err.frames[0].line : 0;
		}
	},
	ERRN: {
		count: 0,
	...
	ERRP: {
		...
		fn: function() {
			var cx = this.context;
			return cx.err && cx.err.frames[0] ? cx.err.frames[0].file : "";
		}
	},
	ERRM: {
		count: 0,
...
exports.instructions = {
	...
	B: function(f1, f2) {
		...
		return function b$do(_, frame) {
			...
			//console.error("ENTERING BLOCK IN " + sub.name+ " at " + loc.line)
			for (var i = 0; i < fns.length && !sub.done && !frame.done && frame.loopLevel >= level; i++) {
				try {
					//console.error("BEFORE STMT IN B:  " + sub.name + " " + i + " at " + loc.line)
					fns[i](_, frame);
					//console.error("AFTER  STMT IN B:  " + sub.name + " " + i + " at " + loc.line)
				} catch (e) {
					//console.error("BEFORE ERROR IN B: " + sub.name + " " + i + " at " + loc.line)
					_handleError(_, frame, e);
					//console.error("AFTER  ERROR IN B: " + sub.name + " " + i + " at " + loc.line)
				}
			}
		}
	},
	...
	CALL: function(ns, name, args) {
		...
		return function call$do(_, frame) {
			...
			var nameVal = name.value !== undefined ? name.value : name(_, frame);
			trace && trace("calling " + nameVal);
