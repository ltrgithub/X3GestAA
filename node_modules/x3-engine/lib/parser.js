...
exports.Parser = function(source, fname) {
	...
	function error(message) {
		return new X3Error(200, message + " near " + near(), fname, line, scopeName);
	}
	...
	function parseWord(mandatory) {
		...
		if (saveBeg === pos) {
			...
			var beg = pos;
			var ch;
			if (!isLetter(source.charCodeAt(pos))) {
				w = null;
			} else {
				pos++;
	...
	function parseString(end) {
		...
		while ((ch = source.charCodeAt(pos)) !== end) {
			if (ch === LF) throw error("unterminated string literal: " + source.substring(beg - 1, pos));
			pos++;
		}
	...
	function parseNumber(mandatory) {
		...
		node.value = decimal ? parseFloat(s) : parseInt(s, 10);
	...
	var termKeywords = {
		...
		FMET: parseResultNode,
		FUNCIU: parseFuncIu,
		NEWSTC: parseNewStc,
		NEWINSTANCE: parseNewStc,
		NOT: function(node) {
			node.tag = 'OP';
			node.op = 'NOT';
	...
	function parseSimpleTerm(mandatory) {
		...
		default:
			if (w = parseWord()) {
				...
				if (keywords[kw] || declKeywords[kw]) {
					unparseWord(w);
					if (mandatory) {
	...
	function parseTerm(mandatory) {
		...
		while (peekChar(DOT) && source.charCodeAt(pos + 1) != DOT) {
			...
			var dot = new Node('OP');
			dot.op = 'DOT';
			dot.args = [node, parseSimpleTerm(true)];
			node = dot;
		}
		if (peekChar(PAR_OPEN)) {
			var call = new Node('INDEX');
			call.target = node;
			call.args = parseParExps();
			node = call;
		}
		return node;
	}
	...
	function parseExp(mandatory, precedence) {
		...
		while (true) {
			...
			if (op.code === 'RANGE' && (peekChar(PAR_CLOSE) || peekChar(COMMA))) {
				...
				var arg = parseExp(true, op.precedence - 1);
				nexp.args = [exp, arg];
			}
			exp = nexp;
	...
	function parseColumn(mandatory) {
		...
		node.field = parseIdent(mandatory);
		node.tag = 'COLUMN';
		return node.field ? node : null;
	}
	...
	function parent(chainType, mandatory) {
	...
	function startLoop(node) {
		node.chainType = 'LOOP';
		node.parentLoop = parent('LOOP');
		blocks.push(node);
	}

	function insideLoop(node) {
		node.parentLoop = parent('LOOP', true);
	}
	...
	function parseStruct(node) {
		...
		parseKeyword('USING', true);
		node.using = parseDynName();
	}
	...
	function parseScope(node) {
		...
		var fn = declKeywords[type];
		if (fn) fn(node);
		else throw error('invalid type ' + type);
	}

	function parseNewStc(node) {
		if (!peekKeyword('WITH')) node.clas = parseDyn(parseTerm);
		if (parseChar(COMMA)) node.args = parseExps();
	...
	function parseHint() {
		if (!parseKeyword('HINT')) {
			return parseKeyword('NOHINT'); // bad ret - fix later
		}
		var node = new Node('HINT');
		parseKeyword('KEY', true);
		parseChar(EQ); // optional!
	...
	var keywords = {
		...
		SEEK: function parseSeek(node) {
			...
			if (node.direction !== 0) node.exp = parseExp(true);
			parseOption(node, 'USING', parseClass);
		},
		// Misc
		...
		CALL: function parseCall(node) {
			node.name = parseDynName();
			parseOption(node, 'WITH', parseExps);
			if (!node.args) node.args = parseParExps();
			parseOption(node, 'FROM', parseDynName);
		},
		FUNC: null,
		OPADXD: parseCallIu,
		...
		FOR: function parseFor(node) {
			...
			function parseForVar(key) {
				if (key.letter || key.abbrev || key.indices) throw error("invalid for syntax, expected identifier alone");
			...
			function parseForSql(node) {
				// undocumented
				node.vars = parseNodes(COMMA, 'VAR', function(n) {
