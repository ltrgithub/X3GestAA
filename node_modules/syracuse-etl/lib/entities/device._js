...
var handlers = {
	file: {
		reader: function(_, dev) {
			...
			var rd = ez.devices.file.binary.reader(filename);
			if (compress) rd = rd.nodeTransform(zlib.createGunzip());
			if (/^(text\/|application\/json)/.test(mediaType)) rd = rd.map(ez.mappers.convert.stringify());
			if (/^application\/json/.test(mediaType)) rd = rd.transform(ez.transforms.json.parser(jsonOptions));
			return rd;
		},
		writer: function(_, dev) {
			...
			var mediaType = dev.mediaType(_);
			if (compress && !/\.gz$/.test(filename)) filename += '.gz';
			var wr = ez.devices.file.binary.writer(filename);
			if (compress) wr = wr.pre.nodeTransform(zlib.createGzip());
			if (/^(text\/|application\/json)/.test(mediaType)) wr = wr.pre.map(ez.mappers.convert.bufferify());
			if (/^application\/json/.test(mediaType)) wr = wr.pre.transform(ez.transforms.json.formatter(jsonOptions));
			return wr;
		},
	},
	mongoCollection: {
		reader: function(_, dev) {
			...
			return ezmongo.reader(collection);
		...
		writer: function(_, dev) {
			...
			var collection = db.collection(dev.mongoCollection(_), _);
			return ezmongo.writer(collection);
		},
	},
	console: {
		reader: function(_, dev) {
