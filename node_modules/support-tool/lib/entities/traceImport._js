		...
		analyse: {
			...
			$execute: function(_, context, instance) {
				...
				if (first && first.entity) {
					...
					entityName = peekableReader.read(_).entity;
					ent = require('support-tool/lib/entities/'+entityName+'._js').entity;
					// Customize list title
					ent.$listTitle = "Imported traces : " + instance.name(_);
					// Add link on query representation
					ent.$queryLinks = {
						import: {
							"$title": "Import detail",
							"$url": "{$baseUrl}/traceImports('"+instance.$uuid+"')?representation=traceImport.$details",
							"$method": "GET",
							"$type": "application/json; vnd-sage=syracuse"
						}
					};
					// store entity to GridFS
					entityWriter.write(_, helpers.object.clone(ent));
					entityWriter.write(_);
					logInfo(_, locale.format(module, "entityStored"));
				} else {
				...
				t.$links.$details = {
					"$title": "Traces view",
					"$url": context.baseUrl + "/support_"+instance.$uuid+"_traces?representation=support_"+instance.$uuid+"_trace.$query",
					"$method": "GET",
					"$type": "application/json; vnd-sage=syracuse"
				};
