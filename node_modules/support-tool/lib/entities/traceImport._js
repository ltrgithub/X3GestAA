...
exports.entity = {
	...
	$functions: {
		...
		registerEntity: function(_) {
			...
			var self = this;
			var _entity = require('support-tool/lib/entities/'+this.entityName(_)).entity;
		...
		analyseImports: function(_, t) {
			...
			function log(severity, msg, stack) {
				...
				if (t) t.$diagnoses.push(d);
				else self.$diagnoses.push(d);
			}
			...
			function handleZipFile(_, _r, name) {
				...
				try {
					var buf = _r.readAll(_);
				var unzipper = new unzip.Unzip(buf, function(filename, filecontent, headers, _) {
					var _subR = ez.devices.buffer.reader(filecontent);
					handleGzFile(_, _subR, name + "_" + filename.split('.')[0]);
				});			
				unzipper.unzip(_);
				} catch (e) {
					log("error", "Cannot process zip file", e.stack);
				}
				return sReaders;
			}
			function handleGzFile(_, _r, name) {
				...
				var s;
				if (validGz) writeToDatabase(_, _r);
			}
			
			// preliminary version - without buffer 
			function noStreamGz(_, buffer, name) {				
				if (!writer) {
					var dbHandle = supportHelpers.getOrm(_);
					var collection = dbHandle.db.collection("Support_" + self.$uuid + "_trace", _);
					writer = ezmongo.writer(collection, {upsert: true}); //
					writer = writer.pre.map(tracker());
				}
				buffer = zlib.gunzip(buffer, _);
				...
				for (var i = 2; i<buffer.length; i++) {
					writer.write(_, buffer[i]);
					}
					
				}
			
			function writeToDatabase(_, reader) {
				if (!writer) {
					var dbHandle = supportHelpers.getOrm(_);
					var collection = dbHandle.db.collection("Support_" + self.$uuid + "_trace", _);
					writer = ezmongo.writer(collection, {upsert: true}); //
			...
			//
			//
			if (t) t.phase = locale.format(module, "analyseFiles");
			var self = this;
			...
			var writer; // mongodb writer
			files.forEach_(_, function(_, content) {
				var _reader = content.import(_).createReadableStream(_);
				try {
					
				// start preliminary code	
				var buffer = _reader.readAll(_);
				if (buffer[0] === 0x1f && buffer[1] === 0x8b){
					noStreamGz(_, buffer, content.name(_));
				} else {
					var unzipper = new unzip.Unzip(buffer, function(filename, filecontent, headers, _) {
						noStreamGz(_, filecontent, content.name(_) + "_" + filename.split('.')[0]);
					});			
					unzipper.unzip(_);
				}
...
//						handleGzFile(_, _reader, content.name(_));
//					} else handleZipFile(_, _reader, content.name(_));
					i++;
			...
			if (t) t.phaseDetail = locale.format(module, "tracesProcessed", i);
	...
	$services: {
		analyse: {
			...
			$execute: function(_, context, instance) {
				...
				else instance.$diagnoses = instance.$diagnoses || [];
