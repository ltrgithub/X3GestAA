...
function resolveLocale(_, name) {
	if (name.indexOf("-") >= 0) return name;
	if (name === "en") return "en-US";
	var files = fs.readdir(fspath.join(__dirname, "../../syracuse-core/lib/types/resources"), _);
	if (files.indexOf("date-" + name + "-" + name.toUpperCase() + ".json") >= 0) return name + "-" + name.toUpperCase();
	files.some(function(f) {
		if (f.substr(0, 7) === "date-" + name) {
			name = f.substring(5, f.indexOf('.'));
		}
	});
	return name;
}

function _loadLocale(_, name) {
	return JSON.parse(fs.readFile(fspath.join(__dirname, "../../syracuse-core/lib/types/resources/date-" + resolveLocale(_, name) + ".json"), "utf8", _));
};

var actionRoutes = {
	'user-profile': function(_, request, response) {
		var pref = locale.extractLocaleCode(request.headers && request.headers["accept-language"] || "en-US");
		try {
			var s = _loadLocale(_, pref);
			var sl = {
				code: s.name,
				description: s.nativeName,
				enabled: true,
				firstDayOfWeek: s.firstDayOfWeek,
				firstWeekOfYear: s.firstWeekOfYear,
				longDate: s.formatPatterns.longDate,
				longDatetime: s.formatPatterns.fullDateTime,
				longTime: s.formatPatterns.longTime,
				numberDecimalSeparator: ".",
				numberGroupSeparator: " ",
				numberGroupSize: 3,
				shortDate: s.formatPatterns.shortDate,
				shortDatetime: s.formatPatterns.shortDate + " " + s.formatPatterns.shortTime,
				shortTime: s.formatPatterns.shortTime,
				twoDigitYearMax: s.twoDigitYearMax,
			};
		} catch (e) {
			if (e.code === 'ENOENT') {
				console.error("Cannot read file for locale " + pref + " use US defaults");
				var sl = {
					code: "en-US",
				...
				throw new Error("Cannot read locale " + pref + ": " + e);
			}
		}
...
function updateWorkingCopy(_, entity, wc, data) {
	...
	if (refUpdate || changed && !result.$uuid && !result.$diagnoses) {
