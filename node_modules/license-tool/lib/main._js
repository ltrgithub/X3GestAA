...
function jsonReply(_, response, status, headers, data) {
		...
		if (status < 300 && data.$diagnoses && data.$diagnoses.some(function(diag) { return diag.$severity === "error"})) {
			status = 400;	
		}		
	...
	Object.keys(obj).forEach(function (key) {
		...
		if (key.substr(-1) === "_") { // localized properties with standard locales
			helper.cfg.locales.forEach(function(loc) {
				...
				if (val.$title) val.$title = val.$title+" "+(loc.title || loc.code);
				if (loc.code === "en-us") val.$isMandatory = true;
				result[key+loc.code] = val;
			})
...
function _handleArrays(val, capabilities) {
	if (val.$type === "application/x-array") {
		...
		for (var b in val.$item) {
			_handleArrays(val.$item[b], capabilities);
		}
		val.$item = {
				$properties: val.$item,
...
var workingCopyActions = {
	$save: function(_, entity, wc, result) {
		...
		if (!result.$diagnoses || !result.$diagnoses.some(function(item) { return item.severity==="error"})) {
...
function _transfer(dest, src, item) {
	var data = src[item];
	if (Array.isArray(data)) {
		...
		data.forEach(function (part) {
			...
			if (index < 0) {
				if (!part.$isDeleted) destData.push(part);
			} else {
				if (part.$isDeleted) destData.splice(index, 1);
				else {
					Object.keys(part).forEach(function(p) {
						_transfer(dest1, part, p);
					})
				}
			}
		});
...
function updateWorkingCopy(_, entity, wc, data) {
	...
	Object.keys(data).forEach_(_, function(_, key) {
		if (key !== "$actions" && key !== "$url") {
			if (props[key] && !props[key].$isReadOnly) {
				if (props[key].$type === "application/x-reference") {
					result = ent.$referenceUpdate(_, wc, data[key]);
				}
				_transfer(wc, data, key);				
			}
		}
	});
	result = result || {};
	...
	try {
		if (key === '$workingCopies') {
			...
			if (request.method === "DELETE") {
				...
				if (!diags.length) {
					diags.push({"$severity":"info","$message":"Ressource has been deleted"})
				}
				jsonReply(_, response, 200, null, 
					{$diagnoses:diags}
				);				
				...
				if (!ent.$extra) {
					jsonReply(_, response, 200, null, 
							{$diagnoses:[{ severity: "error", message: "No extra action for "+entity}]}
						);				
				} else {
		...
		if (ex.errno === 'ENOENT') return notFound(_, request, response);
			...
			jsonReply(_, response, 400, null, {
				$diagnoses: [{ severity: "error", message: ""+ex}]
			}); 
		}
	}
	return true;
...
function errorReply(_, response, e) {
	...
	jsonReply(_, response, 400, null, 
			{$diagnoses:[{ severity: "error", message: ""+e}]}
		);										
}
...
function downloadDispatcher(_, request, response) {
	...
	var header = { "content-type": "application/binary", "x-file-name": "test"};
	var data;
	switch(prop) {
	case "publicKey":
	...
	case "license":
		...
		var license = fs.readFile(helper.LICENSES+"/"+parts[1]+"/"+parts.slice(2).join("-")+".json");
	...
	if (!users.some(function(userdata) {
		if (userdata.login === user) {
			if (!helper.verifyPassword(userdata, pass)) return _unauthenticated(response);
				...
				globals.context = { login: userdata.login, pwd: pass, salt: userdata.salt, name: (userdata.name || userdata.login), admin: (userdata.admin || false), pass: userdata.pass};
				return true;
			}
		}
...
ez.devices.http.server(function(request, response, _) {
	if (request.url === '/') {
		response.writeHead(301, {location: '/license-tool/html/main.html?url=' + encodeURIComponent("/home?representation=home.$page")});
		response.end();
		return;
	}
