...
function jsonReply(_, response, status, headers, data) {
	...
	if (data.$diagnoses && data.$diagnoses.some(function(item) {
		return item.$severity === "error";
	})) {
		data.$actions = data.$actions || {};
		data.$actions.$save = data.$actions.$save || {};
		data.$actions.$save.$isRequested = false;
		data.$actions.$save.$isDisabled = true; // avoid double saving
	}
...
var workingCopyActions = {
	$save: function(_, entity, wc, result) {
		...
		if (!result.$diagnoses || !result.$diagnoses.some(function(item) {
			...
			result.$actions.$save.$isRequested = false;
			result.$actions.$save.$isDisabled = true; // avoid double saving
			result.$diagnoses.push({
				$severity: "info",
				$message: locale.format(module, "saved"),
				$links: {
					$details: {
						$title: locale.format(module, "details"),
			...
			result.$actions.$save = result.$actions.$save || {};
			result.$actions.$save.$isRequested = false;
			result.$actions.$save.$isDisabled = false;
		}
	},
