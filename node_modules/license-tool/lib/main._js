...
var actionRoutes = {
	'user-profile': function(_, request, response) {
		...
		if (pref.indexOf("-") < 0) {
			if (pref === "en") pref = "en-US";
			else {
				var files = fs.readdir(__dirname + "/../../syracuse-core/lib/types/resources", _);
				if (files.indexOf("date-" + pref + "-" + pref.toUpperCase() + ".json") >= 0) pref += "-" + pref.toUpperCase();
				else {
					files.some(function(f) {
						if (f.substr(0, 7) === "date-" + pref) {
							pref = f.substring(5, f.indexOf('.'));
						}
					});
				}
			}
		...
		try {
			var s = JSON.parse(fs.readFile(__dirname + "/../../syracuse-core/lib/types/resources/date-" + pref + ".json", "utf8", _));
			var sl = {
				code: pref,
