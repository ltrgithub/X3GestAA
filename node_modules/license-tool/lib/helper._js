...
exports.setPartnerFile = function(content) {
	...
	if (exports.prefix) {
		if (!/^(?:[A-Z]+\,)*[A-Z]+$/.test(exports.prefix)) throw new Error(locale.format(module, "wrongPrefix"));
		exports.prefix += ",Y";
	} else exports.prefix = "Y";
	//adjust pattern for badge code
	try {
		require('../entities/policies').$properties.badges.$item.code.$pattern = "^(?:" + exports.prefix.replace(/,/g, "|") + ")\\w*$";
		...
		if (exports.cfg.locales) {
			...
			if (!noFile) {
				try {
					...
					if (diagnoses) helper.addError(locale.format(module, "internalError", e), diagnoses);
				}
				if (diagnoses) diagnoses.push({
					$severity: "info",
...
exports.addError = function(message, diagnoses) {
	var diagnose = {
		$severity: "error",
		$message: message
	};
	...
	if (!allLocales) {
		...
		list.forEach(function(item) {
			if (item.substr(0, 5) === "date-") {
				...
				allLocales[_adjust(data.name)] = data.nativeName;
			}
		});
	}
	return allLocales;
