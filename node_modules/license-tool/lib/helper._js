...
function init(port, user, passwd, pubkey, privkey, passphrase) {
	...
	try {
		...
		var config = {locales:[{code: "en-us", title: "English US"}]};					
	}
...
exports.updateLocales = function(localeArray, diagnoses) {
	var newLocales = [];
	var tempLoc = exports.localeObject || {};
	exports.localeObject = {};
	var changed = ((exports.cfg.locales ? exports.cfg.locales.length : 0) !== localeArray.length);
	localeArray.forEach(function(item) {
		if (!item.code) {
			if (diagnoses) diagnoses.push({severity: "error", message: "No locale code"});
		} else {
			item.code = item.code.toLowerCase();
			var oldValue = tempLoc[item.code];
			if (oldValue === null) {
				// duplicate code
				if (diagnoses) diagnoses.push({severity: "warning", message: "Ignore duplicate locale "+item.code});
			} else {
				if (!(item.code in tempLoc) || oldValue !== item.title) changed = true;
				tempLoc[item.code] = null;
				newLocales.push({code: item.code, title: item.title});
				exports.localeObject[item.code] = item.title;
			}
		}
	});
	if (changed) {
		if (exports.cfg.locales) {
			...
			try {
				...
				console.error(e);
				if (diagnoses) diagnoses.push({severity: "error", message: "Error when updating configuration: "+e});
			}
			if (diagnoses) diagnoses.push({severity: "info", message: "Configuration updated"})						
...
function signLicense(jsonLic, ctx) {
	if (!privateKey) {
		...
		console.log("DECRYPT KEY"+passphrase+" "+privateKeyEnc);
