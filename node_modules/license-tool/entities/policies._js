"use strict";
var helper = require('license-tool/lib/helper');
var fs = require('streamline-fs')
var util = require('util')
var globals = require('streamline/lib/globals')
var tracer;

module.exports = { 
	$title: "Policies",
	$modes: "duplicate!,query,details,delete,edit!,create!",
	$properties: {
		productCode: {
			$title: "Product code",
			$type: "application/x-string",
			$pattern: "^\\w+$",
			$links: {
				$details: {
					$url: "/data/policies/{$uuid}?representation=policies.$details",
				},
			},
			$isMandatory: true,
		},
		productTitle_: {
			$title: "Product title",
			$type: "application/x-string",
			$noQuery: true
		},
		productLocales: {
			$title: "More locales for product",
			$type: "application/x-array",
			$item: {
					code: {
						$title: "Code",
						$pattern: "^[a-z]{2}-[a-z]{2}$",
						$displayLength: 5,
						$type: "application/x-string", 
						$isMandatory: true,
					},
					title: {
						$title: "Title",
						$isMandatory: true,
						$displayLength: 10,
						$type: "application/x-string"
					}
			}			
		},
		productVersion: {
			$title: "Product version",
			$isMandatory: true,
			$pattern: "^[\\w\\.]+$",
			$type: "application/x-string"
		},
		policyCode: {
			$title: "Policy code",
			$isMandatory: true,
			$pattern: "^\\w+$",
			$type: "application/x-string",
		},
		policyTitle_: {
			$title: "Policy title",
			$type: "application/x-string",
				$noQuery: true
		},
		policyLocales: {
			$title: "More locales for policy",
			$type: "application/x-array",
			$item: {
					code: {
						$title: "Code",
						$pattern: "^[a-z]{2}-[a-z]{2}$",
						$displayLength: 5,
						$type: "application/x-string", 
						$isMandatory: true,
					},
					title: {
						$title: "Title",
						$displayLength: 10,
						$isMandatory: true,
						$type: "application/x-string"
					}
			}			
		},
		policyVersion: {
			$title: "Policy version",
			$isMandatory: true,
			$pattern: "^[\\w\\.]+$",
			$type: "application/x-string"
		},
		baseProduct: {
			$title: "Base product",
			$type: "application/x-string",
			$isMandatory: true,
			$noQuery: true
		},
		activityCodes: {
			$title: "Activity codes",
			$type: "application/x-array",
			$item: {
				code: {
					$title: "Code",
					$pattern: "^[XY]\\w+$",
					$type: "application/x-string",
					$displayLength: 4,
					$isMandatory: true,
				},
				itemTitle_: {
					$title: "Title",
					$displayLength: 8,
					$type: "application/x-string"
				},
				itemMore: {
					$title: "More locales",
					$type: "application/x-boolean",
					$displayLength: 2,
					$isReadOnly: true
				},
				itemLocales: {
					$title: "More locales",
					$type: "application/x-array",
					$item: { 
						code: {
							$title: "Locale",
							$type: "application/x-string", 
						},
						title: {
							$title: "Text", 
							$type: "application/x-string", 
						}
					}
				},
				condition: {
					$title: "Condition",
					$type:"application/x-choice",
					$displayLength: 4,
					$isMandatory: true,
					$value: {
						$type: "application/x-string",
						$enum: [{$value: "never", $title: "Never"},  {$value: "license", $title: "License"}, {$value: "always", $title: "Always"}],
					},
					$default: "never"
				}
			}			
		},
		badges: {
			$title: "Badges",
			$type: "application/x-array",
			$item: {
				code: {
					$title: "Code",
					$type: "application/x-string", 
					$displayLength: 4,
					$pattern: "^[XY]\\w+$",
					$isMandatory: true,
				},
				itemTitle_: {
					$title: "Title",
					$displayLength: 8,
					$type: "application/x-string"
				},
				itemMore: {
					$title: "More locales",
					$type: "application/x-boolean",
					$displayLength: 2,
					$isReadOnly: true
				},
				itemLocales: {
					$title: "More locales",
					$type: "application/x-array",
					$item: { 
						code: {
							$title: "Locale",
							$type: "application/x-string", 
						},
						title: {
							$title: "Text", 
							$type: "application/x-string", 
						}
					}
				},
				functions: {
					$title: "Functions",
					$type:"application/x-string",
					$pattern: "^(?:[X-Y][A-Z0-9_]+[ ,;]+)*[X-Y][A-Z0-9_]+$",
					$displayLength: 20,
					$isMandatory: true,
				}
			}			
		},
		parameters: {
			$title: "Parameters",
			$type: "application/x-array",
			$item: {
				code: {
					$title: "Code",
					$pattern: "^[XY]\\w+$",
					$type: "application/x-string", 
					$displayLength: 4,
					$isMandatory: true,
				},
				itemTitle_: {
					$title: "Title",
					$displayLength: 8,
					$type: "application/x-string"
				},
				itemLocales: {
					$title: "More locales",
					$type: "application/x-array",
					$item: { 
						code: {
							$title: "Locale",
							$type: "application/x-string", 
						},
						title: {
							$title: "Text", 
							$type: "application/x-string", 
						}
					}
				},
				itemMore: {
					$title: "More locales",
					$type: "application/x-boolean",
					$displayLength: 2,
					$isReadOnly: true
				},
			}			
		},
		languages: {
			$title: "Languages",
			$type: "application/x-array",
			$item: {
				code: {
					$title: "Code",
					$pattern: "^Y\\w+$",
					$displayLength: 4,
					$type: "application/x-string", 
					$isMandatory: true,
				},
				itemTitle_: {
					$title: "Title",
					$displayLength: 8,
					$type: "application/x-string"
				},
				itemMore: {
					$title: "More locales",
					$type: "application/x-boolean",
					$displayLength: 2,
					$isReadOnly: true
				},
				itemLocales: {
					$title: "More locales",
					$type: "application/x-array",
					$item: { 
						code: {
							$title: "Locale",
							$type: "application/x-string", 
						},
						title: {
							$title: "Text", 
							$type: "application/x-string", 
						}
					}
				},
				condition: {
					$title: "Condition",
					$type:"application/x-choice",
					$displayLength: 4,
					$isMandatory: true,
					$value: {
						$type: "application/x-string",
						$enum: [{$value: "never", $title: "Never"},  {$value: "license", $title: "License"}, {$value: "always", $title: "Always"}],
					},
					$default: "never"
				}
			}			
		},
		legislations: {
			$title: "Legislations",
			$type: "application/x-array",
			$item: {
				code: {
					$title: "Code",
					$pattern: "^Y\\w+$",
					$displayLength: 4,
					$type: "application/x-string", 
					$isMandatory: true,
				},
				itemTitle_: {
					$title: "Title",
					$displayLength: 8,
					$type: "application/x-string"
				},
				itemMore: {
					$title: "More locales",
					$type: "application/x-boolean",
					$displayLength: 2,
					$isReadOnly: true
				},
				itemLocales: {
					$title: "More locales",
					$type: "application/x-array",
					$item: { 
						code: {
							$title: "Locale",
							$type: "application/x-string", 
						},
						title: {
							$title: "Text", 
							$type: "application/x-string", 
						}
					}
				},
				condition: {
					$title: "Condition",
					$type:"application/x-choice",
					$displayLength: 4,
					$isMandatory: true,
					$value: {
						$type: "application/x-string",
						$enum: [{$value: "never", $title: "Never"},  {$value: "license", $title: "License"}, {$value: "always", $title: "Always"}],
					},
					$default: "never"
				}
			}			
		},
	},
	$list: function(_, query) {
			var startIndex = +query.startIndex || 1;
			var endIndex = startIndex+(+query.count || 50)-1;
			var counter = 0;
			var pol = fs.readdir(helper.POLICIES, _);
			var resources = [];
			pol.some(function(p) {
				if (p.indexOf(".json") === p.length-5) {
					if (++counter < startIndex) return;
					else if (counter > endIndex) {
						resources.truncated = true;
						return true;
					}
					p = p.substr(0, p.length-5);
					var parts = p.split("-");
					resources.push({ $uuid: p,
						productCode: parts[0],
						productVersion: parts[1],
						policyCode: parts[2],
						policyVersion: parts[3]});
				}
			});
			return resources;
	},
	$load: function(_, key, query, edit) {
		if (edit && !query.duplicate) {
			var cnt = helper.usedPolicies(_, key);
			if (cnt) throw new Error("Policy cannot be edited because "+cnt+" license(s) are based on it");
		}
		var pol = helper.fileTest(_, helper.POLICIES+"/"+key+".json", "Policy");
		var result = {
				productCode: pol.product.code,
				productVersion: pol.product.version,
				policyCode: pol.policy.code,
				policyVersion: pol.policy.version,
				baseProduct: pol.baseProduct			
		};
		_distributeLocales(result, "product", pol.product.title);
		_distributeLocales(result, "policy", pol.policy.title);
		if (!query.duplicate) { // do not change key of existing policy
			result.$properties = {
				productCode: { $isReadOnly: true },
				productVersion: { $isReadOnly: true },
				policyCode: { $isReadOnly: true },
				policyVersion: { $isReadOnly: true },
			}
		}
		if (pol.activityCodes) {
			result.activityCodes = pol.activityCodes.map(function(ac) {
				return _distributeLocales({ $uuid: ac.code, code: ac.code, condition: ac.condition }, "item", ac.title)
			});
		}
		if (pol.legislations) {
			result.legislations = pol.legislations.map(function(ac) {
				return _distributeLocales({ $uuid: ac.code, code: ac.code, condition: ac.condition }, "item", ac.title)
			});
		}
		if (pol.languages) {
			result.languages = pol.languages.map(function(ac) {
				return _distributeLocales({ $uuid: ac.code, code: ac.code, condition: ac.condition }, "item", ac.title)
			});
		}
		if (pol.badges) {
			result.badges = pol.badges.map(function(item) {
				var result = { $uuid: item.code, code: item.code, functions: item.functions.join(" ") };
				return _distributeLocales(result, "item", item.title);
			});			
		}
		if (pol.parameters) {
			result.parameters = pol.parameters.map(function(item) {
				return _distributeLocales({ $uuid: item.code, code: item.code}, "item", item.title);
			});			
		}
		return result;
	},
	$save: function(_, wc, diagnoses) {
		if (!helper.partnerId) {
			diagnoses.push({
				$severity: "error",
				$message: "No partner file",
			});
			return key;			
		}
		var key = wc.productCode+"-"+wc.productVersion+"-"+wc.policyCode+"-"+wc.policyVersion;
		if (key !== wc.$uuid) {
			if (fs.exists(helper.POLICIES+"/"+key+".json", _)) {
				diagnoses.push({
					$severity: "error",
					$message: "Policy already exists",
				});
				return key;
			}
		};
		wc.activityCodes = wc.activityCodes || [];
		wc.languages = wc.languages || [];
		wc.legislations = wc.legislations || [];
		wc.badges = wc.badges || [];
		wc.parameters = wc.parameters || [];
		var result = {
				fileType: "Policy",
				generationStamp: new Date(),
				partnerId: helper.partnerId,
				product: {
					code: wc.productCode,
					title: _collectLocales(wc, "product"),
					version: wc.productVersion
				},
				policy: {
					code: wc.policyCode,
					title: _collectLocales(wc, "policy"),
					version: wc.policyVersion
				},
				baseProduct: wc.baseProduct,
				activityCodes: wc.activityCodes.map(function(ac) {
					return { code: ac.code, title: _collectLocales(ac, "item"), condition: ac.condition }}),
				languages: wc.languages.map(function(ac) {
						return { code: ac.code, title: _collectLocales(ac, "item"), condition: ac.condition }}),
				legislations: wc.legislations.map(function(ac) {
							return { code: ac.code, title: _collectLocales(ac, "item"), condition: ac.condition }}),
				badges:wc.badges.map(function(item) {
					return { code: item.code, title: _collectLocales(item, "item"), functions: item.functions.split(/[^\w]+/) };
				}),
				parameters: wc.parameters.map(function(item) {
					return { code: item.code, title: _collectLocales(item, "item"), type: "integer" };
				}),
				signature: helper.partnerId,
				signatureText: helper.partnerId
		};
		var signed = helper.signLicense(result, globals.context);
		// console.log(signed);
		fs.writeFile(helper.POLICIES+"/"+key+".json", signed, _);
		return key;
	}, 
	$delete: function(_, key){
		var cnt = helper.usedPolicies(_, key);
		if (cnt) throw new Error("Policy cannot be deleted because "+cnt+" license(s) are based on it");
		fs.unlink(helper.POLICIES+"/"+key+".json", _);
	}

}

// INITIALIZATION!
//adjust pattern for badge code
if (helper.prefix) {
	module.exports.$properties.badges.$item.code.$pattern = "^(?:"+helper.prefix.replace(/,/g, "|")+")\\w*$";
	tracer && tracer("Pattern "+module.exports.$properties.badges.$item.code.$pattern)
}




function _upper(loc) {
	var index = loc.indexOf("-");
	return loc.substr(0, index+1)+loc.substr(index+1).toUpperCase();
}

function _collectLocales(source, name) {
	var result = {};
	var en_us = false; // en-us locale
	for (var key in helper.localeObject) {
		var title = source[name+"Title_"+key];
		if (title) {
			if (key === "en-us") en_us = true;
			result[_upper(key)] = title;
		}
	}
	var extra = source[name+"Locales"];
	if (extra) extra.forEach(function(item) {
		if (item.title)	{
			var key = item.code.toLowerCase();
			result[_upper(key)] = item.title;
			if (key === "en-us") en_us = true;
		}
	});
	if (!en_us) throw new Error("Locale en-us missing");
	return result;	
}

function _distributeLocales(result, name, source) {
	result[name+"Locales"] = [];
	Object.keys(source).forEach(function(key) {
		var lkey = key.toLowerCase();
		if (lkey in helper.localeObject) {
			// standard locale
			result[name+"Title_"+lkey] = source[key];
		} else { // extra locale
			result[name+"Locales"].push({
				$uuid: lkey, code: lkey, title: source[key]
			});
			if (name === "item") result[name+"More"] = true;
		}
	});
	return result;
}

