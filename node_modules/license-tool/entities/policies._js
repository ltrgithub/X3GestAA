...
module.exports = {
	...
	$list: function(_, query) {
		...
		try {
			...
			if (e.code === 'ENOENT') {
				try {
					fs.mkdir(helper.POLICIES, _);
					var pol = [];
				} catch (e) {
					throw new Error(locale.format(module, "createPoliciesDir", e));
				}
			} else throw new Error(locale.format(module, "readPoliciesDir", e));
		}
		// dummy query
	...
	$load: function(_, key, query, edit) {
		...
		if (edit && !query.duplicate && cnt) {
			return {
				$diagnoses: helper.addError(locale.format(module, "notEditPolicy", cnt), null, "policies", key)
			};
		}
		try {
	...
	$save: function(_, wc, diagnoses) {
		...
		try {
			...
			helper.addError(locale.format(module, "saveError", e), diagnoses);
			return key;
		}
	...
	$delete: function(_, key, diags) {
		...
		try {
			...
			helper.addError(locale.format(module, "deleteError", key, e), diags);
		}
	}
