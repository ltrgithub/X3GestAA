...
module.exports = { 
	$title: "Licenses",
	$modes: "duplicate!,query,details,delete,create!,extra!",
	$links: {
		$extra: {
			...
			try {
				...
				if (lics.some(function(p) {
					if (p.indexOf(".json") === p.length-5) {
						...
						var parts = p.split("-");
						var date = parts[4]+"00";
						resources.push({ $uuid: customer+"-"+p,
							productCode: parts[0],
	...
	$save: function(_, wc, diagnoses) {
		...
		var result = {
				...
				activityCodes: wc.activityCodes.map(function(item) {
					...
					return { $uuid: item.code, code: item.code, validity: validity }}),
					...
					badges:wc.badges.map(function(item) {
						var validity;
						return { $uuid: item.code, code: item.code, max: item.max};
					}),
					parameters: wc.parameters.map(function(item) {
						var validity;
						return { $uuid: item.code, code: item.code, value: item.value };
					}),
...
function _fillData(_, result, uuid, policy) {
	...
	if (uuid) {
		...
		var filename = parts[0]+"/"+parts.slice(1).join("-")+".json";
		var license = helper.fileTest(_, helper.LICENSES+"/"+filename, "License");
		if (!policy) {
	...
	if (!policy) {
		return { $diagnoses: [{ severity: "error", message: "No policy available"}]};
	}
	console.log("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
	result.policyFile = {$uuid: policy};
	...
	console.log("ERGEBNIS "+util.format(result))
	return result;
}
