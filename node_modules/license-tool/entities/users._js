...
module.exports = { 
	...
	$save: function(_, wc, diagnoses) {
		// console.log("wcUUID "+wc.$uuid+" "+wc.passwd)
		if (!globals.context.admin && wc.$uuid !== globals.context.login) {
			diagnoses.push({
				$severity: "error",
				$message: "Non admin user cannot change other users"
			});
			return;
		};
	    ...
	    if (wc.$uuid !== wc.login) {
	       if(users.some(function(user) {
	    	// console.log("VGL "+user.login+ " "+wc.login)
	    ...
	    if (globals.context.admin && !wc.admin && wc.$uuid === globals.context.login) {
			diagnoses.push({severity: "error", message: "User cannot remove own administrator rights"});
			return;
		}
		...
		if (wc.passwd && !globals.context.admin && wc.oldPasswd !== globals.context.passwd) {
			diagnoses.push({
				$severity: "error",
				$message: (wc.oldPasswd ? "Wrong old password" : "No old password given"),
			})										
			return false;
		}
		if (users.some(function(user) {
		...
		try {
			// console.log("SCHREIBE "+util.format(users));
	...
	$load: function(_, key, query) {
		...
		users.forEach(function(user) {
			if (user.login === key) {
				wc = { login: user.login, name: user.name, admin: (user.admin || false) };
			}
		});
		return wc;
	},
	...
	$list: function(_, query) {
		...
		if (globals.context.admin) {
			...
			console.log("UPP "+upperBound+" "+endIndex)
			if (upperBound > endIndex) {
				resources.truncated = true;
				upperBound = endIndex;
			}
			for (var i = startIndex-1; i < upperBound; i++) {
				console.error(i+" "+util.format(resources0))
				var resource = resources0[i];
				resources.push({ login: resource.login, name: resource.name, "$uuid": resource.login, admin: (resource.admin || false) });
			}
		} else {
			resources0.some(function(user) {
				if (user.login === globals.context.login) {
					resources[0] = { "$uuid": user.login, login: user.login, name: user.name, admin: (user.admin || false) };
					return true;
				}
			});	
		}
	...
	$delete: function(_, key, diagnoses) {		
		...
		if (key === globals.context.login) {
			diagnoses.push({severity: "error", message: "User cannot delete itself"});
			return;
		}
