...
exports.generate = function(node, fname) {
	...
	var generators = {
		...
		FOR: function(node) {
			...
			if (node.key) {
				used.FORF = true;
				return insert(node, "FORF(" + gen(node.key) + "," + ((node.hint) ? gen(node.hint) : "null") + "," + ((node.lock) ? "true" : "false") + "," + generators.WHERE(node.where) + ",p," + id(node.branch) + ",B(", "", "))");
			} else if (node.from) {
				used.FORV = true;
		...
		ORDER: function(node) {
			...
			return "ORDER(" + name + ",[" + (node.keyBy || []).map(genVariant("SQL")) + "]," + genDefNull(node.keyWith) + ")";
		},
		...
		COLUMNS: function(node) {
			...
			return insert(node, "COLUMNS('" + (node.clas.abbrev || '') + "'" + (node.columns ? "[" + node.columns.map(gen0) + "]" : '') + ")");
		},
		LINK: function(node) {
			used.LINK = true;
