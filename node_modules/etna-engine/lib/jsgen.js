	...
	var generators = {
		...
		FCALL: function(node, variant) {
			...
			if(node.name === 'PARSE' && node.args.length == 1) { 
				...
				node.args.push({tag:"INTEGER",value:0,line:node.args[0].line,col:node.args[0].col});
			}
		...
		FMET: function(node) {
			...
			if(node.name === 'AGETATTR') {
				used.AGETATTR = true;
				return "AGETATTR(" + gen(node.target.target||node.target) + "," + (node.target.target ? "'"+node.target.name+"'": "null")  + "," + gen(node.args[0]) + ")";
			} 
		...
		CALLMET: function(node) {
			...
			if(node.name === 'ASETATTR') {
				used.ASETATTR = true;
				return insert(node, "ASETATTR(" + gen(node.target.target||node.target) + "," + (node.target.target ? "'"+node.target.name+"'" : "null") + "," + gen(node.args[0]) + ","+gen(node.args[1])+")");
			}
