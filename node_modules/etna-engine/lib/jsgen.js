...
exports.generate = function(node, fname) {
	...
	var generators = {
		...
		PUTSEQ: function(node) {
			...
			else return insert(node, "PUTSEQ('" + abbrev + "'," + gen(node.count) + ",[" + node.exps.map(gen0).join(',') + "]," + gen(node.sized) + ")");
		},
		GETSEQ: function(node) {
			used.GETSEQ = true;
			var abbrev = node.using ? node.using.abbrev : '';
			return insert(node, "GETSEQ('" + abbrev + "'," + gen(node.count) + ",[" + node.exps.map(gen0).join(',') + "]" + ")");
		},
		SEEK: function(node) {
			used.SEEK = true;
			var abbrev = node.using ? node.using.abbrev : '';
