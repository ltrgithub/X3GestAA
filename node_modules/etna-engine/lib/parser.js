...
exports.Parser = function(source, fname) {
	...
	function parseNumber(mandatory) {
		...
		node.value = decimal ? bcd.fromString(s) : parseInt(s, 10);
		...
		switch (source.charCodeAt(pos)) {
			...
			default:
				if (w = parseWord()) {
					...
					if(! /^[a-zA-Z_]{1}[a-zA-Z_0-9]*$/.test(w)) throw error("invalid character in "+w);
					node = new Node('IDENT');
