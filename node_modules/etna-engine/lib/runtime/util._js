...
exports.badArgumentType = function(x) {
	console.log( "bad badArgumentType"  ) ;
	return new X3Error(10, "bad argument: " + x + " (type " + x.constructor.name + ")");
};

exports.checkNumber = function(x) {
	console.log( "checkNumber" )
	if (!x.isNumeric || !x.isNumeric( )) throw exports.badArgumentType(x);
};

exports.checkInteger = function(x) {
	console.log( "checkInteger" ) ;
	if (!tinteger.isInteger(x)) throw exports.badArgumentType(x);
};

exports.isInteger = function(x) {
	return tinteger.isInteger(x);
};

exports.checkString = function(x) {
	if (!tstring.isString( )) throw exports.badArgumentType(x);
};
...
exports.checkNotZero = function(x) {
	if (!x.isZero || !x.isZero( )) throw new X3Error(53);
};
