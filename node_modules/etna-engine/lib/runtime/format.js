...
fmt = "";
for(var i=0;i<fmtS[1].length;i++) {
	var fmtC = fmtS[1][i];
	var p = values.ADXTUL[2].indexOf(fmtC);
	if(p >= 0) fmtC = values.ADXTUT[2][p];
	fmt += fmtC;
}
fmt = [fmtS[0],fmt].join(':');
*/
...
exports.format = function($fmt, $val) {
	...
	var len = fmt.length,
		...
		tail = "",
		cur, n3 = 0;
	var special = glob.context.x3frame.context.sys.values.ADXSCA;

	var isDigit;

	var adxtul = glob.context.x3frame.context.sys.values.ADXTUL[2];
	var adxtuc = glob.context.x3frame.context.sys.values.ADXTUC[2];
	var adxtut = glob.context.x3frame.context.sys.values.ADXTUT[2];
		...
		K: function() {
			...
			for (; rep > 0; rep--) {
				...
				if (fk) vch = fk.call(this, vch);
					...
					} else throw new X3Error(5, "bad format character: " + ch);
				}
	...
	//dbg:console("val:"+$val+" fmt:"+$fmt+ " result:'"+result + "' valid:"+valid);
