...
exports.instructions = {
	...
	PROP: function(target, name) {
		return function dot$do(_, assign, val, silent) {
			...
			try {
				if (assign) {
					if (typeof t.setProperty === "function") return x3Val(t.setProperty(_, name, val));
					else return t[name] = val;
				} else {
					if (typeof t.getProperty === "function") return x3Val(t.getProperty(_, name, glob.context.debug));
					else {
						var v = t[name];
						if (v === undefined) {
