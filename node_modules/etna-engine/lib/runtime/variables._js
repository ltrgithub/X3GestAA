...
exports.instructions = {
	DIM: function(arr, dim) {
		return function dim$do(_, frame) {
			var a = arr(_, frame, false, undefined, true);
			if (a === undefined) return -1;
			var d = dim !== undefined ? dim(_, frame) : 1;
	...
	CLALEV: function(arg) {
		return function clalev$do(_, frame) {
			var a = arg(_, frame, false, undefined, true);
			if (!a) return 0;
			if (typeof a === "number") {
	...
	CV: function(letter, abbrev, field, silent) {
		return function cv$do(_, frame, assign, val, silent) {
			...
			if (_tracer) {
				if (assign) _tracer(frame.sub.loc.line + ": SET " + field + " <-  " + val);
				else _tracer(frame.sub.loc.line + ": GET " + field + " ->  " + dict.values[field]);
			}
	...
	PROP: function(target, name) {
		return function dot$do(_, frame, assign, val, silent) {
			var t = target(_, frame);
	...
	XMET: function(target, name, args) { // fmet and callmet
		...
		return function dot$do(_, frame) {
			var t = target(_, frame);
			...
			try {
				return meth.apply_(_, t, [frame].concat(vals), 0);
			} catch (ex) {
				throw new X3Error(26, ex.message, ex.stack);
			}
		};
	...
	NEW: function(clas) {
		return function new$do(_, frame) {
			var name = clas(_, frame);
	...
	FREE: function(allocGroup, isGroup) {
		return function free$do(_, frame) {};
	},
};