...
exports.initStack = function(superv) {
	...
	sysVar('INDICE', 'I', 0);
	sysVar('adxmda', 'S', 0);
	sysVar('adxdcs', 'I', 1940);
	sysVar('messname', 'S', superv.LAN) ;
	sysVar('adxmbm', 'I', 0) ;
	sysVar('datesyst', 'D', date.today(false)) ;
...
var assignInteger = function($type) {
	return function(field,type,val) {
		if ( val.x3ToInteger) val = val.x3ToInteger() ;
		else throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
...
var assigners = {
	...
	'J':assignInteger('J'),
	'V':assignInteger('V'),
	'N':function(field,type,val) {
		if (!val.x3ToBCD()) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
		return val.x3ToBCD() ;				
	},
	'D':function(field,type,val) {
		if (!val.x3ToDate) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
		return val.x3ToDate();
	},
	'B':function(field,type,val) {
		if (!util.x3IsBlbfile(val)) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
		// nothing else
		return val;
	},
	'S':function(field,type,val) {
...
exports.instructions = {
			...
			for (var i = 0, len = dicts.length; i < len; i++) {
				...
				if (found) {
					if (assign) {
						...
						else if (!Array.isArray(val) && assigners[dict.types[key][1]]) {
							val = assigners[dict.types[key][1]](field,dict.types[key],val);
						}
						dict.values[key]  = val;
						return;
					} else { 
	...
	CV: function(letter, abbrev, field, silent) {
		return function cv$do(_, assign, val, silent) {
			...
			if (assign) {
				...
				if (!Array.isArray(val) && assigners[dict.types[field][1]]) {
					val = assigners[dict.types[field][1]](field,dict.types[field],val);
				}
	...
	DECL: function(key, field, using, lens0, ranges0) {
		return function decl$do(_) {
			...
			var ifield = (frame.prog.params && frame.prog.params.indexOf(field));
			if (ifield>=0 && frame.prog.pardef[ifield]===0 ) {
		    	frame.prog.pardef[ifield] = 1;
		    }
