...
exports.instructions = {
			...
			for (var i = 0, len = dicts.length; i < len; i++) {
				...
				if (found) {
					if (assign) {
						...
						if(dict.types[key][1] == "I" && typeof val == 'number') {
	...
	ASSIGN: function(lhs, rhs, silent) {
		return function assign$do(_) {
			...
			if(args) {
				var instructions = require('./arrays').instructions;
				var fINDEX = instructions.INDEX(fV,args);
				fINDEX(_,true, rhs.value !== undefined ? rhs.value : rhs(_));
			} else {
				fV(_, true, rhs.value !== undefined ? rhs.value : rhs(_), silent);
			}
		};
	},
	RAZ: function(exps) {
