...
exports.instructions = {
	...
	V: function(field) {
		return function v$do(_, assign, val, silent,strict) {
			...
			// not found, create in local dict if assign and strict = false 
			if (assign && strict !== true) {
	...
	KILL: function(letter, field) {
		return function kill$do(_) {
			var frame = glob.context.x3frame;
			var dict;
			...
			if(dict) {
				if(dict.values[field]) {
					delete dict.values[field];
					delete dict.types[field];
				} else {
					throw new X3Error(6,"Non existent variable : " + field);
				}
			}
		}
	},
