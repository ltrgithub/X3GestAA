...
exports.instructions = {
					...
					if (assign) {
						...
						if (dict.types[key][1] === "I") {
							...
							if (bcd.isBCD(val)) {
								...
								val = bcd.toInteger(val) ;								
							}
							else if (typeof val === "number") {
								
								val = integer.fromDouble(val) ;
							}
							else if (!integer.isInteger(val)) 								
								throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
							
							
							if(Math.abs(integer.toDouble(val)) >= Math.pow(2,31)) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
						} else if (dict.types[key][1] ==="N") { 
							if (typeof val === "number" ) val = bcd.fromDouble( val ) ;
							else if (integer.isInteger(val)) val = bcd.fromInteger( val ) ;
							else if (!bcd.isBCD(val)) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
						} else if (dict.types[key][1] === "S" &&  dict.types[key].length > 2 ) {
							val = val.substring(0,parseInt(dict.types[key].substring(2)));
						}
	...
	DECL: function(key, field, using, lens0, ranges0) {
		return function decl$do(_) {
			...
			//console.log("RANGE=" + JSON.stringify(ranges)+typeof ranges+" dim="+dim+" values"+JSON.stringify(values[field]));
			if ((ranges===undefined || ranges===null) && Array.isArray(arr))
			{
				//console.log("RANGE modif....");
				ranges = [0, null];
			...
			//console.log("RANGE suite=" + JSON.stringify(rangesVals)+" dim="+dim+" values"+JSON.stringify(values[field]));
			...
			if (ranges) {
				...
				if (arr === undefined) {
					...
					for (var dim = 0; dim < ndims; dim++) {
						...
						//console.log("RANGE fin=" + JSON.stringify(range)+" dim="+dim+" values"+JSON.stringify(values[field]));
					}
				}
			} else {
