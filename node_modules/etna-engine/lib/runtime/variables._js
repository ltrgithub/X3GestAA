...
exports.initStack = function(superv) {
	...
	declVar(cx.sys,'ADXFMT','CS', [1,exports.types["S"].def,
		...
		'9.2',
		'X',
		'DD[/]MM[/]YYYYYY[-]MM[-]DD[T]hh[:]mm[:]ss[Z]'],0,7]);	
	...
	declVar(cx.sys,'ADXIFS','CS','');//Field separator
	declVar(cx.sys,'ADXIRS','CS2','\n');//Record separator
	declVar(cx.sys,'ADXIUM','CI',0);//Character set
	...
	declVar(cx.sys,'ADXUSR','CS',process.env.USER || "user");
	...
	ASETATTR: function(target, property, attribut,val) {
		if(property)
			return function asetattr$do(_,silent) {
				...
				if (!t || !(t.class || t.$exported)) throw new X3Error(26, "invalid attribute target: " + t);
				try {
					if (typeof t.setPropertyAttribute === "function") return x3Val(t.setPropertyAttribute(_,property,attribut(_),val(_)));
				} catch (ex) {
					handleError(ex);
				}			
			};
			...
			return function asetattr$do(_,silent) {
				...
				if (!t || !(t.class || t.$exported)) throw new X3Error(26, "invalid attribute target: " + t);
				try {
					if (typeof t.setAttribute === "function") return x3Val(t.setAttribute(attribut(_),val(_)));
				} catch (ex) {
					handleError(ex);
				}			
			};
