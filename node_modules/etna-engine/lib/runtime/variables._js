...
exports.vOperations = {
	...
	SET: {
		fn: function(frame, dict, field, val, silent, strict) {
			//console.log("SET "+field+" type:"+dict.types[field]+" val:"+val);
...
exports.instructions = {
	...
	V: function(field) {
		...
		return function v$do(_, operation, val, silent, strict, full) {
			var frame = glob.context.x3frame;
			...
			for (var i = 0, len = dicts.length; i < len; i++) {
				...
				if (dict === frame.context.tables) {
					...
					return fdo.fn(frame, dict, field, val, silent, strict, full);
				}
			}
