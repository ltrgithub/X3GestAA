...
//console.log("dict.types[key][1] " + dict.types[key][1]+ "   "+dict.types[key]);
						if (dict.types[key]===undefined) {
							...
							//use on Callprog/finally 
						}
						else if ("IJV".indexOf(dict.types[key][1]) !== -1) {
							if(!val.x3IsNumeric) console.log(val.constructor.name+" has no method isNumeric");
							if (!val.x3IsNumeric( ))
								throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");

							val = val.x3ToInteger( ) ;
							if (!val.x3AssertOverflow( dict.types[key][1]) )
								throw new X3Error(13, "invalid value '" + val + "' assigned to '" + field + "'");
							...
							if (!util.x3IsString(val))
								throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");

							val =  val.substring(0,parseInt(dict.types[key].substring(2)));
						} else if (dict.types[key][1] === "D") {
							val = val.x3ToDate( ) ;
						} else if (dict.types[key][1] === "B") {
							if (!util.x3IsBlbfile(val))
								throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
							// nothing else
						}
	...
	DECL: function(key, field, using, lens0, ranges0) {
		return function decl$do(_) {
			...
			if (ranges && arr && Array.isArray(arr) && (key[0]==='C' || key[0]==='B')) {
			    var newtab = [] ;
