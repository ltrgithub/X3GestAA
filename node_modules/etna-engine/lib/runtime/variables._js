...
exports.initStack = function(superv) {
	...
	declVar(cx.globals, 'AVOID', 'VY', {
		$exported: true,
		ADATE: exports.clone(exports.types['D'].default),
		ADATTIM: exports.clone(exports.types['E'].default),
		ASHORT: exports.clone(exports.types['J'].default),
		AINT: exports.clone(exports.types['I'].default),
		ACHAR: exports.clone(exports.types['S'].default),
		ADEC: exports.clone(exports.types['N'].default),
		ACLOB: exports.clone(exports.types['T'].default),
		ABLOB: exports.clone(exports.types['B'].default),
		AUUID: exports.clone(exports.types['Z'].default),
	});
	...
	declVar(cx.sys, 'ADXFMT2', 'CS', [1, exports.types["S"].def, ['D:DD[/]MM[/]YYYY',
	...
	declVar(cx.sys, 'ADXTUL', 'CS', [1, exports.types["S"].def, [], 0, 30]);
	declVar(cx.sys, 'ADXTUT', 'CS', [1, exports.types["S"].def, [], 0, 30]);
	declVar(cx.sys, 'ADXTUC', 'CS', [1, exports.types["S"].def, [], 0, 30]);
...
exports.types = {
	...
	E: {
		name: 'DATETIME',
		value: 526,
		default: new datetime.Datetime(0),
...
var setACTX = function(_, frame) {
	...
	return frame.context.globals.values.GACTX = glob.context.x3session.actx;
};
...
exports.instructions = {
	...
	DECL: function(key, field, using, lens0, ranges0) {
		...
		return function decl$do(_) {
			...
			/*dbg*/
			tracerJs.debug && tracerJs.debug("decl " + field + " type:" + key);
			...
			/*dbg*/
			tracerJs.debug && tracerJs.debug("ranges :" + !! ranges);
		...
		return function dot$do(_, assign, val, silent) {
			...
			try {
				if (assign) {
					...
					else if (typeof t.setProperty === "function") return x3Val(t.setProperty(_, name, val));
					else return t[name] = val;
				} else {
					if (typeof t.get === "function") return x3Val(t.get(_, name, glob.context.debug));
					else if (typeof t.getProperty === "function") return x3Val(t.getProperty(_, name, glob.context.debug));
					else {
						var v = t[name];
						if (v === undefined) {
		...
		return function dot$do(_) {
			...
			try {
				...
				if (typeof meth === "function") {
					return x3Val(meth.apply_(_, t, vals, 0));
				} else {
					if (meth.params && meth.body && typeof meth.body === 'function') {
						var frame = glob.context.x3frame;
						return x3Val(flowControl.callProg(_, meth, args, vals, frame.sub.loc));
					} else throw new X3Error(26, "missing method in built-in object: " + name);
