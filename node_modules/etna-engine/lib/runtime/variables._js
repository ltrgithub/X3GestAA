			...
			for (var i = 0, len = dicts.length; i < len; i++) {
				...
				if (found) {
					if (assign) {
						...
						if(dict.types[key][1] === "I" && bcd.isBCD(val)) {
							//Do no set a decimal value into an integer variable:
							val = bcd.toInt32(val) ;
							if(Math.abs(val) >= Math.pow(2,31)) throw new X3Error(50, "invalid value '" + val + "' assigned to '" + field + "'");
						} else if (dict.types[key][1] ==="N" && typeof val === "number" ) {
							val = bcd.fromInt32( val ) ;
