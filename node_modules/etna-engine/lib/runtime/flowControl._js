...
function callProg(_, prog, args, argVals, loc) {
	...
	for (var i = 0; i < params.length; i++) {
		// arrays passed by reference : make a save copy
		if (Array.isArray( argVals[i])) {
	     var newtab = [] ;
	     for (var j = 0; j <argVals[i].length; j++) {
			// copy arrays of object passed
			if (Array.isArray( argVals[i][j])) {
		     var newtab2 = [] ;
		     for (var k = 0; k <argVals[i][j].length; k++) {
					newtab2[k] =  argVals[i][j][k];
		     }
		     newtab[j] = newtab2;
			}
			else {
		...
		// raz Pardef
		if (Array.isArray(frame.prog.pardef)){
			frame.prog.pardef[i]=0;
		}
	}
	try {
		prog.body(_);
		...
		// pop frame
		var savframe = glob.context.x3frame;
		var sav=savframe;
		...
		for (var i = 0; i < params.length; i++) {
			...
			if (key && key[0] === 'A' && arg) {
				if (Array.isArray(val)) {
				 // arrays passed by reference & variable: get values, length into save copy
  				 argVals[i][2] =  val[2];
 				 if (argVals[i][4]!==null){
					argVals[i][4] =  argVals[i][2].length+argVals[i][3]-1;
				 }

				 // arrays passed by reference : get the changed copy into reference
			     for (var j = 0; j <argVals[i].length; j++) {
					 val[j] =  argVals[i][j];
				 } 
				 val.length=argVals[i].length;
				 argVals[i]=val;
				}
				try {
			    arg(_, true, frame.values[param]);
				}
				catch (ex) {
					//exception for INDEX(arrays)
					if (!(ex.errn && ex.errn===8)) {
						_handleError(_, frame, ex);
					}
				}
			}
			else if (Array.isArray(val)) {
			//a new copy has been created in variables/DECL or V
			//	// arrays passed by reference : get the save copy
			//     for (var j = 0; j <argVals[i].length; j++) {
			//		 val[j] =  argVals[i][j];
			//	 } 
			//	 val.length=argVals[i].length;
...
function getSetter(field,val) {
	...
	for (var i = 0, len = dicts.length; i < len; i++) {
		if(dicts[i].values[field]!== undefined) {
			...
			setter = variables.setters[key[1]];
			return function(val) {
	...
	if(val !== undefined) {
		...
		if(typeof val !== "number") {
			...
			else if(date.isDate(val)) key = 'LD';
		} 
	}
...
exports.functions = {
	...
	MESS: {
		...
		fn: function(_, id, chapter, swtch) {
			...
			// --------------------------------------------------------------------------
			if (frame.context.sys.values["messname"] !== frame.context.superv.LAN)
...
exports.instructions = {
	...
	FORV: function(varName, from, to, step, p, end, block) {
		return function forv$do(_) {
			...
			var fromI = from(_);
			var toI = to(_);
			var stepI = step ? step(_).x3ToDouble() : 1 ;
			var condition = (stepI >= 0)? 
				function(){return i <= toI && frame.loopLevel === level;}:
				function(){return i >= toI && frame.loopLevel === level;};
			
			var setIndex = (function(field) {
				var setValue = getSetter(field);
					return function(val) {
						setValue(new tdouble.Double(val));
					}
				})(varName);
			
			var level = ++frame.loopLevel;
			for (var i = fromI; condition(); i += stepI) {
				setIndex(i);
				block(_);
			}
