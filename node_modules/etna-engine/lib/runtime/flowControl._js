...
function callProg(_, frame, prog, args, argVals, loc) {
	...
	cx.traceCall && cx.traceCall(_, frame, "enter", argVals);
	...
	cx.traceCall && cx.traceCall(_, frame, "exit", frame.result);
	return frame.result;
}
...
exports.instructions = {
	...
	F: function(name, args) {
		...
		return function f$do(_, frame) {
			...
			try {
				traceFunc && traceFunc(_, frame, "enter", vals);
				var result;
				if (fn.async) result = fn.fn.apply_(_, frame, vals, 0);
				else result = fn.fn.apply(frame, vals);
				traceFunc && traceFunc(_, frame, "exit", result);
				return result;
			} finally {
				frame.fcall = fcall;
			}
