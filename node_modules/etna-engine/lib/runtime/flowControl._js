...
exports.functions = {
	...
	ERRN: {
		...
		fn: function() {
			var cx = this.context;
			return new tinteger.Integer(cx.err ? cx.err.errn || 26 : 0);
		}
	},
	ERRP: {
		count: 0,
...
exports.instructions = {
	...
	F: function(name, args) {
		...
		return function f$do(_) {
			...
			try {
				...
				var result;
				if (fn.async) result = fn.fn.apply_(_, vals, 0);
	...
	FORV: function(varName, from, to, step, p, end, block) {
		return function forv$do(_) {
			...
			var fromI = from(_);
			var toI = to(_);
			var stepI = step ? step(_) : new tinteger.Integer(1);
			var values = frame.values;
			var level = ++frame.loopLevel;
			for (var i = fromI.toInteger().valueOf(); i <= toI.toInteger().valueOf() && frame.loopLevel === level; i += stepI.toInteger().valueOf()) {
				values[varName] = new tinteger.Integer(i);
				block(_);
			}
