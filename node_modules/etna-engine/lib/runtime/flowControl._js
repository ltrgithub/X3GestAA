	...
	for (var i = 0; i < params.length; i++) {
		...
		if (Array.isArray( argVals[i])) {
	     ...
	     for (var j = 0; j <argVals[i].length; j++) {
			...
			if (Array.isArray( newtab[j])) {
		     var newtab2 = [] ;
		     for (var k = 0; k <newtab[j].length; k++) {
					newtab2[k] =  newtab[j][k]
		     }
		     newtab[j] = newtab2
			} 
	     }
		...
		for (var i = 0; i < params.length; i++) {
			...
			if (key && key[0] === 'A' && arg) {
				...
				arg(_, true, frame.values[param]);
				 ...
				 // copy arrays of object passed by variable
 				 argVals[i][2] =  val[2]
 				 if (argVals[i][4]!==null){
 				  argVals[i][4] =  val[2].length+argVals[i][3]-1
 				 }
				}
			}
		}
...
exports.functions = {
	...
	MESS: {
		...
		fn: function(_, id, chapter, swtch) {
			...
			return frame.context.superv.loadMess(_, (swtch !== 0)?chapter:chapter+9000, id ) ;
		}
	},	
};
