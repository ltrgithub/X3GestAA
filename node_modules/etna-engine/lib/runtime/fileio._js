...
exports.instructions = {
	...
	RDSEQ: function(abbrev, vars) {
		return function rdseq$do(_) {
			...
			// loop on all values
			// for (var i = 0, var index = 0; i < vals.lenght; i++)
			var f = vars[0];
			var s = f(_);
			...
			if (Array.isArray(s)) {
				...
				for (var j = s[3]; j < s[4]; j++) {
					...
					fidx(_, true, "tata"); // add parameter conversion function x3FromString
				}
			} else {
				//console.log("value");
				f(_, variables.vOperations.SET, "toto", 0); // add parameter conversion function x3FromString
			}
		};
	}
};