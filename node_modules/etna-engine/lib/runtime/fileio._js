...
exports.instructions = {
		...
		return function rdseq$do(_) {
			...
			while (vars[i]) {
				...
				if (Array.isArray(s)) {
					...
					//var _inf = f(_, variables.vOperations.GETINFO ) ;
					//console.log("info var:"+_inf.vtype+" "+_inf.vlength+" "+_inf.vstring) ;
	...
	PUTSEQ: function(abbrev, nbr, vars, sized) {
		return function putseq$do(_) {
			...
			while (vars[i]) {
				...
				if (inf.vlength) lens.push(inf.vlength);
				else lens.push(0);
				if (inf.vsize) size.push(inf.vsize);
				else size.push(0);
			}
			console.log(vals + " " + vnbr);
			console.log(lens);
			console.log(size);

			return file.x3put(_, vals, lens);
		};
	},
	GETSEQ: function(abbrev, nbr, vars) {
		return function putseq$do(_) {
			var frame = glob.context.x3frame;
			var file = findFileR(frame, abbrev);
		};
