...
exports.functions = {
	ASCII: {
		count: 1,
		fn: function(s) {
			if (typeof expression!==='string') throw util.badOperand(s) ;
	...
	PAT: {
		...
		fn: function(s1, s2) {
			//util.checkString(s1);
			//util.checkString(s2);
			var regex = new RegExp("^"+s2.x3ToString().replace(/\*/g, ".*").replace(/#/g,"\\d{1}").replace(/!/g,"[a-zA-Z]{1}").replace(/\?/g,".{1}")+"$");
			return regex.test(s1.x3ToString())?1:0;
		}
	},
	VAL: {
		count: 1,
		fn: function(s) {
			if(util.x3IsDate(s)) return s.x3ToString();

			if(util.x3IsString(s)) {
				var m = RE_NUMBER.exec(s.x3ToString());
				if (m) {
					if(m[2]) return new tbcd.BCD(parseFloat(s.x3ToString()));
					return new tinteger.Integer(parseFloat(s.x3ToString()));
				}
			}
			else throw util.badOperand(s) ;
		}
	},
	LEFT$: {
		count: 2,
		fn: function(s, i) {
			return s.x3ToString().substring(0, i);
		}
	},
	MID$: {
		count: 3,
		fn: function(s, i, l) {
			return s.x3ToString().substring(i - 1, i - 1 + l);
		}
	},
	RIGHT$: {
		count: 2,
		fn: function(s, i) {
			return s.x3ToString().substring(i - 1);
		}
	},
	SEG$: {
		count: 3,
		fn: function(s, i, j) {
			return s.x3ToString().substring(i - 1, j);
		}
	},
	CTRANS: {
		count: -1,
