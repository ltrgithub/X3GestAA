...
exports.functions = {
	...
	VAL: {
		...
		fn: function(s) {
			...
			else if (util.x3IsString(s)) {
				var r = bcd.fromString( s ) ;
				if (bcd.isZero( r ) && (RE_NUMBER.exec( s ) === null))
					throw util.badOperand(s) ;					
					
				return (bcd.isInt32(r, 1)) ? bcd.toInt32(r) : r ; 
	...
	FORMAT$: {
		...
		fn: function(fmt, val) {
			...
			var settings = {
				"ADXSCA":this.context.sys.values.ADXSCA,
				vKDefault:-1
			};
			return format.format(settings,fmt,val);
		}
	},
	NUM$: {
		count: 1,
