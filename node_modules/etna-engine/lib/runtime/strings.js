...
exports.functions = {
	ASCII: {
		count: 1,
		fn: function(s) {
			if (!(s instanceof tstring.TString)) throw util.badOperand(s) ;

			return new tinteger.Integer( s.valueOf( ).charCodeAt(0) ) ;
		}
	},
	LEN: {
		count: 1,
		fn: function(s) {
			...
			return new tinteger.Integer( s.valueOf( ).length ) ;
		}
	},
	PARSE: {
		count: 1,
		fn: function(expression) {
			try {
				if (!(expression instanceof tstring.TString)) throw util.badOperand(expression
					) ;
				var node = new Parser(expression.valueOf( ), "eval").parseExp(true);
			}
			catch(e) {
				return new tinteger.Integer( 5 ) ;
			}
			return new tinteger.Integer( 0 ) ;
		}
	},
	PAT: {
		count: 2,
	...
	RIGHT$: {
		count: 2,
		fn: function(s, i) {
			return new tstring.TString(s.toString().substring(i - 1));
		}
	},
	SEG$: {
		count: 3,
		fn: function(s, i, j) {
			return new tstring.TString(s.toString().substring(i - 1, j));
		}
	},
	CTRANS: {
		count: -1,
