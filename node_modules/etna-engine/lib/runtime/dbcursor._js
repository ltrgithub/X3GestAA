...
function _dispatch(tables, row, columns) {
	if (columns) {
		Object.keys(columns).forEach(function(abv) {
...
var DBCursor = helpers.defineClass(function(_, reader, dbreader, tables, maptrim, pwhere, key, lvl, typ, columns) {
	var self = this;
	...
	x3GetRecord: function(_, sv) {
		...
		// hack for sqlsever //
		//////////////////////////////////////////////////////////////////////
		//if (self.typecursor === "read") {
		//	while (self.reader.read(_));
		//}
		//////////////////////////////////////////////////////////////////////

		if ((!wok) && (!rec)) _dispatch(tables, vsave, undefined);
	...
	x3CloseDBReader: function(_) {
		...
		self.reader.stop();
		tracer.debug && tracer.debug(" x3DiscardCurrent...");
		if (self.dbreader) self.dbreader.discard();
	},
