...
function _buildQuery(_, clause, self, key, wkey, fwhere, lock) {
	...
	var orderBySql = "";
	if (((!wkey) || (!wkey.rowid)) && key && key.sql)
...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, lock, hint, tcnx) {
		...
		if (key.reverted) rkey = _transformKey(key, {
			direction: -1,
			length: 0
		});
		else rkey = key;

		var qry = _buildQuery(_, 'read', self, rkey, wkey, undefined, lock);
	...
	x3Update: function(_, fwhere, asssql, consts, tcnx) {
		...
		if ((dwhere && dwhere.postEvaluation) ||
			...
			// need to build a query ------------------------------
			var nrecord = 0;
			var qry = _buildQuery(_, "for", self, undefined, undefined, fwhere, true);
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry.sql, qry.constants);
			var cf = new DBCursor(rdr, undefined, qry.tables, qry.maptrim, qry.pwhere, undefined, 0);
			...
			var whereSqls = [];
			var wheres = [self.where, self.filter.where, dwhere];
	...
	x3Rowcount: function(_, tcnx) {
		...
		var master = (self.master || self);
		var loop = false;
		if ((self.where && self.where.postEvaluation) ||
			(self.filter.where && self.filter.where.postEvaluation))
			loop = true;
		if ((!loop) && self.master && (
			(self.master.where && self.master.where.postEvaluation) ||
			(self.master.filter.where && self.master.filter.where.postEvaluation)))
			loop = true;

		if (loop) {
			var qry = _buildQuery(_, "for", self, undefined, undefined, undefined, false);
			...
			var result = self.superv.executeSql(_, qry.sql, qry.constants, qry.typesmap, true);
