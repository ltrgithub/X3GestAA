...
function _buildQuery(_, clause, self, key, wkey, fwhere, fwfrom, fwto, lock, hint) {
	...
	if (clause === "rowcount") {
		...
		typesmap = [0];
		columnsSql = (self.columns ? Object.keys(self.columns).reduce(function(result, a) {
			//debugger ;
			var cdef = self.columns[a];
			if (cdef !== null) {
				for (var cnum = 0; cnum < cdef.length; cnum++) {
					typesmap.push(cdef[cnum].field.type.data.TYPTYP);
					result.push(a + "." + cdef[cnum].field.data.CODZONE + '_' + cdef[cnum].index + " As " + a + "$" + cdef[cnum].field.data.CODZONE + '_' + cdef[cnum].index);
				}
			} else {
				var tab = tables[a];
...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, wfrom, wto, tcnx) {
		...
		if (tcnx) {
			...
			master.currentFor = new DBCursor(_, rdr, dbrd.reader, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl, "for", self.columns);
		} else {
			self.driver.withConnection(_, function(_, cnx) {
