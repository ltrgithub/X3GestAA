...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		self.driver.withConnection(_, function(_, cnx) {
			for (var idx = 0; idx < tqry.length; idx++) {
				...
				var cf = new DBCursor(_, rdr, q.tables, q.maptrim, q.pwhere, undefined, undefined, "read", q.columns);
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, wfrom, wto, tcnx) {
		...
		self.driver.withConnection(_, function(_, cnx) {
			var rdr = self.driver.reader(_, cnx, q.sql, q.constants, q.sqlOpts);
			master.currentFor = new DBCursor(_, rdr, q.tables, q.maptrim, q.pwhere, key, lvl, "for", q.columns);
		}, tcnx);
	...
	x3Delete: function(_, key, wkey, fwhere, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, q.sql, q.constants, q.sqlOpts);
			var cf = new DBCursor(_, rdr, q.tables, q.maptrim, q.pwhere, key, key.items.length, "for", q.columns);
	...
	x3Update: function(_, fwhere, asssql, consts, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, q.sql, q.constants, q.sqlOpts);
			var cf = new DBCursor(_, rdr, q.tables, q.maptrim, q.pwhere, undefined, 0, "for", q.columns);
	...
	x3Rowcount: function(_, tcnx) {
		...
		if (loop) {
			...
			var rdr;
			self.driver.withConnection(_, function(_, cnx) {
				rdr = self.driver.reader(_, cnx, q.sql, q.constants, q.sqlOpts);
			}, tcnx);
			var cf = new DBCursor(_, rdr, q.tables, q.maptrim, q.pwhere, undefined, 0, "for", undefined);
