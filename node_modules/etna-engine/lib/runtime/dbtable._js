...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		for (var idx = 0; idx < tqry.length; idx++) {
			...
			var cf = new DBCursor(_, rdr, tqry[idx].tables, tqry[idx].maptrim, tqry[idx].pwhere, undefined, undefined, "read", tqry[idx].columns);
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, wfrom, wto, tcnx) {
		...
		if (tcnx) {
			...
			master.currentFor = new DBCursor(_, rdr, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl, "for", qry[0].columns);
		} else {
			self.driver.withConnection(_, function(_, cnx) {
	...
	x3Delete: function(_, key, wkey, fwhere, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			var cf = new DBCursor(_, rdr, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, key.items.length, "for", qry[0].columns);
	...
	x3Update: function(_, fwhere, asssql, consts, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			var cf = new DBCursor(_, rdr, qry[0].tables, qry[0].maptrim, qry[0].pwhere, undefined, 0, "for", qry[0].columns);
	...
	x3Rowcount: function(_, tcnx) {
		...
		if (loop) {
			...
			var cf = new DBCursor(_, rdr, qry[0].tables, qry[0].maptrim, qry[0].pwhere, undefined, 0, "for", undefined);
