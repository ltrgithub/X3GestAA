...
function _buildQuery(_, clause, self, key, wkey, fwhere, fwfrom, fwto, lock, hint) {
	...
	if (wkey && wkey[0].rowid) {
		...
		if (clause === "for") columns = self.columns;
	} else {
		var wheres;

		// we gonna modify the columns list, so we clone it
		if (clause === "for") {
			columns = self.columns && (function(origin) {
				var cpy = {};
	...
	if (clause === "rowcount") {
		...
		typesmap = [0];
		columnsSql = (columns ? Object.keys(columns).reduce(function(result, a) {
			//debugger ;
			var cdef = columns[a];
			if (cdef !== null) {
				for (var cnum = 0; cnum < cdef.length; cnum++) {
					typesmap.push(cdef[cnum].field.type.data.TYPTYP);
...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		for (var idx = 0; idx < tqry.length; idx++) {
			...
			var cf = new DBCursor(_, rdr, ord.reader, tqry[idx].tables, tqry[idx].maptrim, tqry[idx].pwhere, undefined, undefined, "read", undefined);
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, wfrom, wto, tcnx) {
		...
		if (tcnx) {
			...
			master.currentFor = new DBCursor(_, rdr, dbrd.reader, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl, "for", qry[0].columns);
		} else {
			self.driver.withConnection(_, function(_, cnx) {
