...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		for (var idx = 0; idx < tqry.length; idx++) {
			...
			if (tcnx) {
				rdr = self.driver.reader(_, tcnx, tqry[idx].sql, tqry[idx].constants, tqry[idx].sqlOpts);
				tracer.debug && tracer.debug("test read...");
				tracer.debug && tracer.debug("test read ok");
			} else {
				self.driver.withConnection(_, function(_, cnx) {
					rdr = self.driver.reader(_, cnx, tqry[idx].sql, tqry[idx].constants, tqry[idx].sqlOpts);
				});
			}
			tracer.debug && tracer.debug("Cursor declaration....");
			var cf = new DBCursor(_, rdr, tqry[idx].reader, tqry[idx].tables, tqry[idx].maptrim, tqry[idx].pwhere, undefined, undefined, "read", tqry[idx].columns);
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, wfrom, wto, tcnx) {
		...
		// execution ---------------------------------------
		if (tcnx) {
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			master.currentFor = new DBCursor(_, rdr, qry[0].sqlOpts.reader, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl, "for", qry[0].columns);
		} else {
			self.driver.withConnection(_, function(_, cnx) {
				var rdr = self.driver.reader(_, cnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
				master.currentFor = new DBCursor(_, rdr, qry[0].sqlOpts.reader, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl, "for", qry[0].columns);
			});
		}
	...
	x3Delete: function(_, key, wkey, fwhere, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			var cf = new DBCursor(_, rdr, undefined, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, key.items.length, "for", qry[0].columns);
	...
	x3Update: function(_, fwhere, asssql, consts, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			var cf = new DBCursor(_, rdr, undefined, qry[0].tables, qry[0].maptrim, qry[0].pwhere, undefined, 0, "for", qry[0].columns);
	...
	x3Rowcount: function(_, tcnx) {
		...
		if (loop) {
			...
			var rdr;
			if (tcnx) {
				rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
			} else {
				self.driver.withConnection(_, function(_, cnx) {
					rdr = self.driver.reader(_, cnx, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
				});
			}
			var cf = new DBCursor(_, rdr, undefined, qry[0].tables, qry[0].maptrim, qry[0].pwhere, undefined, 0, "for", undefined);
			...
			qry[0].sqlOpts.typesmap = qry[0].typesmap;
			qry[0].sqlOpts.dispatch = true;
			var result = self.superv.executeSql(_, qry[0].sql, qry[0].constants, qry[0].sqlOpts);
