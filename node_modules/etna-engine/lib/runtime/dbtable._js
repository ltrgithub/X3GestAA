...
// -----------------------------------------------------------------------
function _buildQuery(_, clause, self, key, wkey, fwhere, lock, hint) {
...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		var tqry = _buildQuery(_, 'read', self, rkey, wkey, undefined, lock, hint);
		var qry,
			rdr,
			ord;
		...
		for (var idx = 0; idx < tqry.length; idx++) {
			...
			var cf = new DBCursor(rdr, ord.reader, tqry[idx].tables, tqry[idx].maptrim, tqry[idx].pwhere);
	...
	x3OpenFor: function(_, key, lvl, hint, lock, where, tcnx) {
		...
		var qry = _buildQuery(_, 'for', self, key, undefined, where, lock, hint);
		...
		if (tcnx) {
			...
			master.currentFor = new DBCursor(rdr, dbrd.reader, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, lvl);
		} else {
			self.driver.withConnection(_, function(_, cnx) {
	...
	x3Delete: function(_, key, wkey, fwhere, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			var nrecord = 0;
			var qry = _buildQuery(_, "for", self, key, wkey, fwhere, true, null);
			...
			var cf = new DBCursor(rdr, undefined, qry[0].tables, qry[0].maptrim, qry[0].pwhere, key, key.items.length);
	...
	x3Update: function(_, fwhere, asssql, consts, tcnx) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) ||
			...
			// need to build a query ------------------------------
			var nrecord = 0;
			var qry = _buildQuery(_, "for", self, undefined, undefined, fwhere, true, null);
			...
			// execution ---------------------------------------			
			var rdr = self.driver.reader(_, tcnx, qry[0].sql, qry[0].constants);
			var cf = new DBCursor(rdr, undefined, qry[0].tables, qry[0].maptrim, qry[0].pwhere, undefined, 0);
	...
	x3Rowcount: function(_, tcnx) {
		...
		if (loop) {
			var qry = _buildQuery(_, "for", self, undefined, undefined, undefined, false, null);
