...
var DBTable = helpers.defineClass(function(driver, abbrev, superv, desc, keys, loc) {
	...
	x3Read: function(_, key, lvl, wkey, krevert, lock, hint, tcnx) {
		...
		for (var idx = 0; idx < tqry.length; idx++) {
			...
			var cf = new DBCursor(_, rdr, ord.reader, tqry[idx].tables, tqry[idx].maptrim, tqry[idx].pwhere, undefined, undefined, "read", tqry[idx].columns);
	...
	x3Rowcount: function(_, tcnx) {
		...
		if (loop) {
			...
			var result = self.superv.executeSql(_, qry[0].sql, qry[0].constants, {
				typesmap: qry[0].typesmap,
				dispatch: true,
			});
