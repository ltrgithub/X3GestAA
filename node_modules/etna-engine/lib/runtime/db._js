...
function _closeFile(frame, abbrev) {
	(frame.context.tables[abbrev].sisters || []).forEach(function(absister) {
		if (frame.context.tables[absister]) {
			frame.context.tableSlots.freeSlot(frame.context.tables[absister].slot);
...
exports.instructions = {
	...
	FILESQL: function(abbrev, name, where, order) {
		return function file$do(_) {
			...
			var ctx = {};
			//if (frame.context.tables[abb]) throw new X3Error(26, "Table open twice "+abb)
			...
			if (frame.context.tables[abb]) {
				...
				table = frame.context.tables[abb];
			} else {
				table = frame.context.tables[abb] = new DBTable(driver, abb, frame.context.superv, desc, tkeys, frame.stackLevel).init(_);
