...
*/
	...
	LOGICCLOSE: function(scope, abbrevs) {
		return function logicclose$do(_) {
			closeFile(abbrevs, glob.context.x3frame.stackLevel);
		};
	},
	...
	LOOK: function(key, op, values, lock, hint) {
		...
		return function look$do(_) {
			...
			var fst = ka.table.x3Read(_, ka.nkey, ka.kln, ka.wkey, lock, hint, session.dbConnection);
				...
				for (var i = 0; i < npk; i++) {
					...
					for (var j = 0; j < kp; j++) {
						...
						if (j < kp - 1) {
							...
							sql += "(" + key.items[j] + " " + sqlCmpOps[op].op + " " + s.sql + ")";
