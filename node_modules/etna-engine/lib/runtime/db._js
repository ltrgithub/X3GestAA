...
exports.instructions = {
	...
	READ: function(key, op, values) {
		...
		return function read$do(_) {
			...
			var vals = avalues(_);
			var table = findTable(frame, k.abbrev);
			var param = table.superv.sqlDriver.param;
			...
			var where = '(' + indexExps[0] + cmpOp.op + param(0) +')';
			var lexi = '(' + indexExps[0] + '=:1)';;
			for (var i = 1; i < vals.length; i++) {
				var clause = indexExps[i] + cmpOp.op + param(i);
				if (cmpOp.combine === 'lexi') {
					where = where + ' or (' + lexi + ' and (' + clause + '))';
					lexi = '(' + lexi + ' and (' + indexExps[i] + '=' + param(i) + ')';
				} else {
	...
	REWRITE: function(key, op, values) {
		...
		return function rewrite$do(_) {
			...
			var cmpOp = sqlCmpOps[op || 'EQ'];
			var where = '(' + indexExps[0] + cmpOp.op + param(descs.length) + ')';
			var lexi = '(' + indexExps[0] + '=' + param(descs.length) + ')';
			for (var i = 1; i < vals.length; i++) {
				var clause = indexExps[i] + cmpOp.op + param(descs.length + i);
				if (cmpOp.combine === 'lexi') {
					where = where + ' or (' + lexi + ' and (' + clause + '))';
					lexi = '(' + lexi + ' and (' + indexExps[i] + '=' + param(descs.length + i) + ')';
				} else {
					where = where + ' ' + cmpOp.combine + ' (' + clause + ')'
				}
			}
			var sql = "update " + table.desc.name + " set " + descs.map(function(desc) {
				return desc.name + "_" + (desc.index || 0) + '=' + param(desc.offset);
			}).join(',') + " where " + where;
			//console.log(vals);
