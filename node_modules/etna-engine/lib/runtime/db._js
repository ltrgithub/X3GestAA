...
function describe(_, frame, name) {
	...
	return cx.superv.load(_, 'Table', name);
/*
	var cx = frame.context;
	...
	var rows = cx.superv.executeSql(_, "select * from information_schema.columns where table_name=:1", [name]);
	if (rows.length === 0) {
	...
	rows = cx.superv.executeSql(_, "select * from information_schema.statistics where table_name=:1 order by table_name, index_name, seq_in_index", [name]);
	...
	desc.abbrev = desc.abbrev || cx.superv.executeSql(_, "select ABRFIC_0 from ATABLE where CODFIC_0=:1", [name])[0].ABRFIC;
...
exports.instructions = {
	...
	CLOSEFILE: function(scope, abbrevs) {
		var loc = util.sourceLoc;
		return function closefile$do(_, frame) {
			frame.sub.loc = loc;
	...
	READ: function(key, op, values) {
		...
		return function read$do(_, frame) {
			frame.sub.loc = loc;
			var k = key(_, frame);
			if (k.indices) throw new X3Error(26, "NIY: READ with indices");
			var vals = avalues(_, frame);
			var table = findTable(frame, k.abbrev);
			...
			var where = '(' + indexExps[0] + cmpOp.op + ':1)';
			var lexi = '(' + indexExps[0] + '=:1)';;
			for (var i = 1; i < vals.length; i++) {
				var clause = indexExps[i] + cmpOp.op + ':' + (i + 1);
				if (cmpOp.combine === 'lexi') {
					where = where + ' or (' + lexi + ' and (' + clause + '))';
					lexi = '(' + lexi + ' and (' + indexExps[i] + '=:' + (i + 1) + ')';
				} else {
			...
			var sql = "select " + Object.keys(table.desc.columns).map_(_, function(_, name) {
				...
				return column.sqlNames(_);
			}).join(',') + " from " + table.desc.name + " where " + where;
			var row = frame.context.superv.executeSql(_, sql, vals)[0];
			//console.log(row);
			if (row) {
	...
	REWRITE: function(key, op, values) {
		var loc = util.sourceLoc;
		var avalues = util.instructions.A(values);
		return function rewrite$do(_, frame) {
			frame.sub.loc = loc;
			var k = key(_, frame);
			if (k.indices) throw new X3Error(26, "NIY: READ with indices");
			var vals = avalues(_, frame);
			...
			var cmpOp = sqlCmpOps[op || 'EQ'];
			var where = '(' + indexExps[0] + cmpOp.op + ':' + (descs.length + 1) + ')';
			var lexi = '(' + indexExps[0] + '=:' + (descs.length + 1) + ')';
			for (var i = 1; i < vals.length; i++) {
				var clause = indexExps[i] + cmpOp.op + ':' + (descs.length + i + 1);
				if (cmpOp.combine === 'lexi') {
					where = where + ' or (' + lexi + ' and (' + clause + '))';
					lexi = '(' + lexi + ' and (' + indexExps[i] + '=:' + (descs.length + i + 1) + ')';
				} else {
					where = where + ' ' + cmpOp.combine + ' (' + clause + ')'
				}
			}
			var sql = "update " + table.desc.name + " set " + descs.map(function(desc) {
				return desc.name + "_" + (desc.index || 0) + '=:' + (desc.offset + 1);
			}).join(',') + " where " + where;
			//console.log(vals);
	...
	DELETE: function(abbrev, where, op, values) {
		var loc = util.sourceLoc;
		return function write$delete(_, frame) {
			frame.sub.loc = loc;
			var table = findTable(frame, abbrev);
			var view = findView(frame, abbrev);
			...
			var result = frame.context.superv.executeSql(_, "delete from " + escape(view.table.desc.name) + " " + abbrev + " " + whereSql);
			if (!result || result.updateCount === undefined) throw new X3Error(26, "bad SQL result: " + result);
	...
	TRBEGIN: function(abbrevs) {
		...
		return function commit$trbegin(_, frame) {
			frame.sub.loc = loc;
			var name = "TX"; //aabrevs(_, frame).join('_');
			frame.context.superv.executeSql(_, "set transaction name '" + name + "'");
		};
	},
	COMMIT: function() {
		var loc = util.sourceLoc;
		return function commit$do(_, frame) {
			frame.sub.loc = loc;
	...
	ROLLBACK: function() {
		var loc = util.sourceLoc;
		return function rollback$do(_, frame) {
			frame.sub.loc = loc;
	...
	FORF: function(key, where, p, end, block) {
		...
		return function forf$do(_, frame) {
			...
			return table.doFor(_, frame, where, p, end, block);
