...
exports.instructions = {
	...
	FILTER: function(abbrev, where, order) {
		return function filter$do(_) {
			...
			// delete old filter
			if (table.filter) {
				if (table.filter.key) delete table.keys[table.filter.key];
	...
	FORF: function(key, where, p, end, block) {
		return function forf$do(_) {
			...
			if (cf && cf.klvl === undefined) throw X3Error(26, "Bad nested loop");

			if (!cf) {
				cf = table.x3OpenFor(_, ky, klvl, where);
				...
				if (ky.name !== cf.key.name) throw X3Error(26, "Inconsitent nested loop key");
