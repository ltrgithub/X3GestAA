...
*/
	...
	SQLKEY: function(kyname, op, np, args) {
		//
		return function sqlsql$do(_, ctx) {
			// find key
			var frame = glob.context.x3frame;
			...
			var npk = np && np(_);
			npk = (npk) ? npk[0](_) : kln;
			npk = (npk > args.length) ? arg.length : npk;
			...
			if (op === 'EQ' || npk === 1) {
				...
				for (var i = 0; i < npk; i++) {
					...
					for (var j = 0; j < kp; j++) {
						var f;
						var s = genSql(_, args[j], ctx);
						if (j < kp - 1) f = exports.instructions.SQLEQ(key.func[j], s.func);
						else f = exports.instructions['SQL' + op](key.func[j], s.func);
