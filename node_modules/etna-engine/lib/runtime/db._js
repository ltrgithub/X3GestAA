	...
	FILESQL: function(abbrev, name, where, order) {
		return function file$do(_) {
			...
			if (newkey) {
				...
				if (newkey.sql) {
					newkey.kslot = tkeys.push(newkey) - 1;
					table.currkey = newkey.kslot;
					table.currlen = newkey.items.length;
				} else {
					var mk = table.findkey(newkey.keyName);
					...
					if (newkey.modified) {						
						...
						var mk = table.findkey(newkey.keyName);
						table.currkey = mk.kslot;
						table.currlen = mk.items.length;
	...
	FILTER: function(abbrev, where, order) {
		return function filter$do(_) {
			...
			if (newkey) {
				...
				newkey.kslot = table.keys.push(newkey) - 1;
				table.currkey = newkey.kslot;
				table.currlen = newkey.items.length;
				*/
				/*
	...
	LINK: function(abbrev, as, joins, where, order) {
		return function link$do(_) {
			...
			if (newkey) {
				...
				newkey.kslot = table.keys.push(newkey) - 1;
				lktable.currkey = newkey.kslot;
				lktable.currlen = newkey.items.length;
				*/
