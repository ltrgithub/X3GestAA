...
exports.instructions = {
	...
	FILTER: function(abbrev, where, order) {
		return function filter$do(_) {
			...
			// table in views
			var ctx = {};
			ctx.tables = [view.table.abbrev];
			if (table.joins) {}
			table.joins.forEach(function(join) {
				ctx.tables.push(join.abbrev);
			});
	...
	FORF: function(key, where, p, end, block) {
		return function forf$do(_) {
			...
			var tablesSql = master.desc.name + " " + master.abbrev + (table.joins || []).map(function(join) {
