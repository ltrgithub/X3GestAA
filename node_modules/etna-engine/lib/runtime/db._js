...
exports.instructions = {
	...
	FILESQL: function(abbrev, name, where, order) {
		return function file$do(_) {
			...
			var tkeys = Object.keys(desc.indexes).map(function(name) {
				...
				var sql = "";
				var funcs = [];
				sql += Object.keys(idx.columns).map(function(colname) {
					funcs.push(exports.instructions.SQLCV('F', abb, colname));
					return idx.columns[colname].sqlNames(abb + ".");
				});
				return {
					name: name,
					constants: [],
			...
			// where
			var table = frame.context.tables[abb] = new DBTable(driver, abbrev, frame.context.superv, desc, tkeys);
			table.where = where && where(_, ctx);

			// order by
			var newkey = order && order(_, ctx);
			if (newkey) {
				var slt = tkeys.length;
				tkeys[slt] = order;
				table.currkey = slt;
			}
	...
	FILTER: function(abbrev, where, order) {
		return function filter$do(_) {
			...
			if ((!(where)) && (!(order))) {
				// suppress filter
				if (table.filter) {
					delete table.keys[filter.key];
					if (table.currkey === filter.key) table.currkey = 0;
					table.filter = {};
				}
				return 0;
			}
			...
			var newkey = order && order(_, ctx);
			if (newkey) {
				var slt = table.keys.length;
				table.keys[slt] = newkey;
				table.currkey = slt;
			}
	...
	LINKITEM: function(key, outer, values) {
		return function linkitem$do(_, ctx) {
			...
			if (table.master) throw X3Error(26, "Alread linked");
			...
			+ " ON " + values.map_(_, function(_, val, i) {
				...
				if (val.tooComplex) throw X3Error(26, "Unable to build the link because of comlexity");
	...
	LINK: function(abbrev, as, joins, where, order) {
		return function link$do(_) {
			...
			var ctx = {};
			ctx.tables = [table.abbrev];
			if (table.master) throw X3Error(26, "Link on a Link");

			table.sisters = (table.sisters || []).push(as);
			var lktable = frame.context.tables[as] = new DBTable(table, table.abbrev);
			...
			*/
			...
			lktable.order = order && order(_, ctx);
		};
	},
	READ: function(key, op, values) {
		...
		return function read$do(_) {
			...
			frame.context.sys.values.FSTAT = table.x3Read(_, index, op, vals);
		};
	},
	...
	DELETE: function(abbrev, where, op, values) {
		return function delete$do(_) {
			...
			var sql = "delete from " + driver.escape(table.desc.name) + " " + abbrev + " " + whereSql;
			console.log(sql);
			var result = frame.context.superv.executeSql(_, "delete from " + driver.escape(table.desc.name) + " " + abbrev + " " + whereSql, constants);
			if (!result || result.updateCount === undefined) throw new X3Error(26, "bad SQL result: " + result);
		...
		return function sqlv$do(_, ctx) {
			...
			for (var i = 0; i < abbrevs.length; i++) {
				...
				if (table.values && table.values[field] !== undefined) return {
					...
					func: exports.instructions.SQLV(field),
					tooComplex: false,
				};
			}
