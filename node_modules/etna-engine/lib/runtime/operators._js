...
exports.binaryOps = {
	ADD: function add(a, b) {
		...
		if (date.isDate(a) && tb === "string") return a.toString("yyyyMMdd")+b ;

		if (date.isDate(b) && ta === "number") return b.addDays(a);
		if (date.isDate(b) && bcd.isBCD(a)) return b.addDays(bcd.toInt32(a));
		if (date.isDate(b) && ta === "string") return a+b.toString("yyyyMMdd") ;		
	...
	SUB: function sub(a, b) {
		...
		if (date.isDate(a) && tb === "number") return a.addDays(-b);
		if (date.isDate(a) && bcd.isBCD(b)) return a.addDays(bcd.toInt32(-b));
		if (date.isDate(a) && tb === "string") return trimFuncs[1](a.toString("yyyyMMdd"))+ " " + trimFuncs[0](b) ;

		if (date.isDate(b) && ta === "string") return trimFuncs[1](a) + " " + trimFuncs[0](b.toString("yyyyMMdd")) ;
	...
	MUL: function mul(a, b) {
		...
		if (bcd.isBCD(a) || bcd.isBCD(b)) {		
			if (ta === "number") return bcd.multiply( b, bcd.fromInt32(a)) ;
			else if (tb === "number") return bcd.multiply( a, bcd.fromInt32(b)) ;
			else  return bcd.multiply( a, b ) ;
		}
	...
	DIV: function div(a, b) {
		...
		if (bcd.isBCD(a) || bcd.isBCD(b)) {
			...
			else if (tb === "number") return bcd.divide( a, bcd.fromInt32(b) ) ;
			else return bcd.divide( a, b ) ;
		}
