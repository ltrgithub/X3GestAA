...
var _unaryOps = {
	USUB: function minus(a) {
		if (integer.isInteger(a)) return integer.minus(a) ;
		else if (bcd.isBCD(a)) return bcd.minus(a) ;
		else if( typeof a === "number") return -a ;
		else throw util.badOperand(a) ;
	},

	UADD: function plus(a) {
		if (integer.isInteger(a)) return a ;
		else if (bcd.isBCD(a)) return bcd ;
		else if( typeof a === "number") return a ;
		else throw util.badOperand(a) ;
	},
	...
	NOT: function not(a) {
		...
		else if( typeof a === "number") return a?0:1 ;
		else throw util.badOperand(a) ;
	}
};
...
function compare( a, b ) {
	...
	//console.log("compare:"+a.constructor.name+" with "+b.constructor.name);
...
exports.binaryOps = {
	ADD: function add(a, b) {
		console.log(a.constructor.name+":"+a+" add"+ b.constructor.name+":"+b);
		var ta = typeof a,
			tb = typeof b;		

		if(a.add) return a.add(b);
		else if (integer.isInteger(a) && integer.isInteger(b)) return integer.add( a, b ) ;
		else if ((ta === "string") || (tb === "string"))
		{
			if (ta !== "string")
			{
				if (date.isDate(a)) return a.toString()+b ;
				else throw util.badOperand(a);
			}
			return a + b ;
		} else if ((ta === "number") || (tb === "number")) {
	...
	SUB: function sub(a, b) {
		...
		else if ((ta === "string") || (tb === "string"))
		{
			if (tb !== "string")
			{
				if (date.isDate(b)) return trimFuncs[1](a) + " " + trimFuncs[0](b.toString()) ;
				else throw util.badOperand(b);	
			}
	...
	MOD: function mod(a, b) {
		...
		if(a.mod) return a.mod(b);
		
		if (integer.isInteger(a) && integer.isInteger(b)) {
			return integer.toDouble(a)%integer.toDouble(b)
		}
		else if (ta === "number" || tb === "number") {
			if (ta !== "number") {
				if (bcd.isBCD(a)) return  bcd.toDouble(a) % b ;
			...
			var dv = bcd.divide( bcda, bcdb) ;
			var fx = bcd.toInt32( dv ) ;
			var ml = bcd.multiply( bcd.fromInt32(fx), bcdb ) ;
