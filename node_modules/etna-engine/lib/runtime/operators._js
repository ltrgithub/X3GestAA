...
function op1(f) {
	return function(f1) {
		return function op1$do(_, frame) {
			return f(f1.value !== undefined ? f1.value : f1(_, frame));
		};
	};
}
...
function op2(f) {
	return function(f1, f2) {
		return function op2$do(_, frame) {
			return f(f1.value !== undefined ? f1.value : f1(_, frame), f2.value !== undefined ? f2.value : f2(_, frame));
		};
	};
}
...
exports.instructions = {
	AND: function(f1, f2) {
		return function and$do(_, frame) {
			return f1(_, frame) && f2(_, frame);
		};
	},
	OR: function(f1, f2) {
		return function or$do(_, frame) {
			return f1(_, frame) || f2(_, frame);
		};
	},
