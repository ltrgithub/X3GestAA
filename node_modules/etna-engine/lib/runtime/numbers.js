...
exports.functions = {
	...
	SQR: {
		count: 1,
		fn: function(v) { 
			if (bcd.isBCD(v)) v = bcd.toDouble( v ) ;
			else if (typeof v != "number") throw util.badOperand(b) ;

			return bcd.fromDouble( Math.sqrt(v) ) ;
		}
	},
	AR2: {
		count: 1,
	...
	ARR: {
		...
		fn: function(x, y) {
			...
			val = bcd.fromInt32(bcd.toInt32(val)) ;
	...
	COS: {
		count: 1,
		fn: function(angle) {
			if (bcd.isBCD(angle)) angle = bcd.toDouble( angle ) ;
			else if (typeof angle != "number") throw util.badOperand(b) ;

			
			return bcd.fromDouble( Math.cos(toRadians(angle)) ) ;
		}
	},
	SIN: {
		count: 1,
		fn: function(angle) {
			...
			return bcd.fromDouble(  Math.sin(toRadians(angle)) ) ;
		}
	},
	TAN: {
		count: 1,
