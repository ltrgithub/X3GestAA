	...
	INDEX: function(target, args) {
		...
		return function index$do(_, assign, val) {
			...
			if (Array.isArray(targetVal)) {
				...
				if (!Array.isArray(data)) return targetVal[argVals[0] - 1]; //:It's a table's indexed property

				var offset = _offset(targetVal, argVals, frame.orderCount > 0);
				//dbg:console.log("index offset:" + offset);
				if (assign) {
				...
				throw new X3Error(8, "Index incorrect:" + JSON.stringify(targetVal));
			}
		};
	},
