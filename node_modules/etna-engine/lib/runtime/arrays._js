...
function _evalOrder(order) {
	return function evalOrder$do(_, frame) {
		var values = [];
		var cursor = frame.L.INDICE = {
			type: 'LI',
			value: 0
		};
		while (cursor.value < frame.orderCount) {
			values.push(order(_, frame));
			cursor.value++;
		}
...
exports.instructions = {
	RANGE: function(lo, hi) {
		...
		return function range$do(_, frame) {
			return [lo(_, frame), hi(_, frame)];
		};
	},
	INDEX: function(target, args) {
		var aargs = util.instructions.A(args);
		return function index$do(_, frame, assign, val) {
			var targetVal = target(_, frame);
	...
	SORTA: function(rep, vars, orders) {
		...
		return function sorta$do(_, frame) {
			var repVal = rep(_, frame);
