				 ...
				 //console.log("DELA: apres=" + JSON.stringify(arr));
  				 ...
  				 //console.log("INSA: lo=" +lo+" hi="+hi+" num="+num+" base="+base);
					...
					if (arr[4]!==null) {
						...
						if (base===0 & hi<0) i++; // special case for add first element in array(0..)
						while (i+num >= lo - base) {
							data[i+num] = def;
							i--;
						}
...
exports.functions = {
	MAXTAB: {
		...
		fn: function(arr) {
			...
			if (arr[4]===null) {
				//return arr[2].length===0 ? 0 : arr[2].length-arr[3]-(arr[3]===0?1:-arr[3])
				if (arr[2].length===0) return 0;
				if (arr[0]===1) return arr[2].length-arr[3]-(arr[3]===0?1:-arr[3])
				var size, sliceSize = 1;
				for (var i = arr[0] - 1; i > 0; i--) {
					size = (arr[4 + 2 * i] != null)? arr[4 + 2 * i]-arr[3 + 2 * i]+1 : null;
					sliceSize *= size;
				}
				//return ((arr[2].length+arr[3])/sliceSize)-arr[3]-(arr[3]===0?1:-arr[3])
				return ((arr[2].length+arr[3])/sliceSize)-1
			}
			else {
