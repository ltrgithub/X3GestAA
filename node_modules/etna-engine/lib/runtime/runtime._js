...
exports.requireScript = function(_, name) {
	...
	if(exports.requireScript.serverScript[name]) {
		var path = exports.requireScript.serverScript[name].replace(/\\/g, '/');
		var cpath = __dirname + "/../../../etna-engine/test/tmp/"+name+".src" ;
		//var cpath = register.cachedPath(path,{"verbose":true,"force":false,"extension":"src"});
		register.mkdirsSync(cpath);
		var source = fs.readFile(path, "utf8", _);
		fs.writeFileSync(cpath, source, "utf8");
		exports.requireScript.serverScript[name] =cpath ;
		exports.requireScript.scripts[name] = require(exports.requireScript.serverScript[name]);
		return exports.requireScript.scripts[name];
	}
