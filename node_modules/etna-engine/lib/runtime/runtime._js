...
var scriptDebugger = require("etna-engine/lib/debugger/server");
...
var trueInst = ["SORTA", "INSA", "DELA", "DECL", "DEFAULT", "ASSIGN", "RAZ", "KILL", "GOSUB", "GOTO", "ONERRGO",
	"RET", "RESUME", "PROG", "END", "CALL", "FUNC", "XMET", "IF", "CASE", "FORF", "FORV", "FORIN", "FORSQL", "WHILE",
	"BREAK", "EXECSQL", "ANASQL", "FILEVAL", "FILESYS", "FILESQL", "FILTER", "COLUMNS", "LINK", "LOOK", "READ",
	"WRITE", "REWRITE", "DELETE", "UPDATE", "TRBEGIN", "COMMIT", "ROLLBACK", "LOGICCLOSE", "OPEN", "CLOSE", "IOMODE",
	"WRSEQ", "RDSEQ", "PUTSEQ", "GETSEQ", "SEEK", "CALLJS", "BOX", "DBGETNA", "SLEEP", "FREE",
];
...
exports.requireScript = function(_, name) {
	//dbg:console.log("requireScript:"+name);
	...
	if (exports.requireScript.builtins[name]) {
		//dbg:console.log("require builtin:"+name);
	...
	if (exports.requireScript.nodeScript[name]) {
		//dbg:console.log("require nodeScript:"+name);
...
if (config.debug && config.debug.enabled) _initDebugger(_);
...
exports.executeProg = function(_, mod, prog, args) {
	...
	try {
		...
		args && Object.keys(args).forEach(function(name) {
			if (args[name].type && args[name].type.charAt(0) === 'A') {
...
function _initDebugger(_) {
	...
	Object.keys(exports.instructions).filter(function(k) {
		...
		exports.instructions[k] = function debugWrapper() {
			...
			console.error("wrap debug: " + k + " in " + module + ':' + sys.inspect(loc));
			// console.error("wrap debug: " + k + " in " + module + ':' + loc.line + ' - ' + sys.inspect(arguments, {
			// 	depth: 3
			// }));
			return function(_) {
				var frame = glob.context.x3frame,
				...
				// console.error("call debug: active=" + debugContext.isActive + ", status=" + debugContext.status + ", hit " + k + " in " + module + ':' + loc.line);
				if (debugContext.isActive) {
					if (!glob.context.debug) {
						...
						debugContext.hit(_, frame, request);
						glob.context.debug = false;
					}
					// console.error("call debug: " + k + " in " + module + ':' + loc.line + '\nargs=' + sys.inspect(arguments, {
					// 	depth: 3
