...
exports.requireScript = function(_, name) {
	var path = __dirname + "/../../../etna-supervisor/lib/builtins/" + name + "._js";
	if (fs.exists(path, _)) return require(path);
	return require(name);
}
...
function initTraces() {
	...
	function traceCall(_, action, arg) {
		...
		console.log(indent + loc.module + ':' + loc.line + ': ' + action + ' ' + frame.prog.name + ": " + arg);
