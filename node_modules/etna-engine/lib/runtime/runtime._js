...
var trueInst = ["SORTA", "INSA", "DELA", "DECL", "DEFAULT", "ASSIGN", "SET", "RAZ", "KILL", "GOSUB", "GOTO", "ONERRGO",
...
function _initDebugger(_) {
	...
	Object.keys(exports.instructions).filter(function(k) {
		...
		exports.instructions[k] = function debugWrapper() {
			...
			tracer.debug && tracer.debug("wrap debug: " + k + " in " + module + ':' + loc.line + ", loc=" + sys.inspect(loc));
			// tracer.debug && tracer.debug("wrap debug: " + k + " in " + module + ':' + loc.line + ' - ' + sys.inspect(arguments, {
			// 	depth: 3
			// }));
			return function() {
				var frame = glob.context.x3frame,
