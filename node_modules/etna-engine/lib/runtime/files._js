...
var File = helpers.defineClass(function(ctx) {
	...
	x3write: function(_, vals, nl) {
		...
		// write it
		var len = fs.write(self.fd, str + ((nl) ? irs : ifs), self.offset + self.bom, ium || "utf8", _);
	...
	x3put: function(_, nbr, vals, lens, size) {
		var self = this;
		var i = 0,
			nbv = 0;
		var vlist = [];
		...
		while (vals[i]) {
			...
			if (Array.isArray(v)) {
				var t = v[2];
				for (var j = v[3]; j <= v[4]; j++) {
					if (t[j - v[3]]) {
						var val = t[j - v[3]].x3ToBlbfile(lens[i] || size[i]).valueOf();
						tsize += val.length;
						vlist.push(val);
						if (++nbv >= nbr) break;
					} else break;
				...
				var val = v.x3ToBlbfile(lens[i] || size[i]).valueOf();
				tsize += val.length;
				vlist.push(val);
				if (++nbv >= nbr) break;
			}
