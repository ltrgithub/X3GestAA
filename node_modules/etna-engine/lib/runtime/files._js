...
var File = helpers.defineClass(function(ctx) {
	...
	x3write: function(_, vals, nl) {
		...
		// write it
		//TODO : _ or _/*~_*/ : doesn's work in fast mode 
		var len = fs.write(self.fd, str + ((nl) ? irs : ifs), self.offset + self.bom, ium || "utf8", _ /*~_*/ );
	...
	x3read: function(_) {
		...
		if (self.mda === 0) {
			...
			if (ium === "utf8") {
				...
				var len = fs.read(self.fd, bom, 0, bom.length, 0, _ /*~_*/ );
				self.bom = ((bom[0] === 0xef) && (bom[1] === 0xbb) && (bom[2] === 0xbf)) ? 3 : 0;
			}
		}

		// algo 
		var len = fs.read(self.fd, cache, 0, cache.length, curr + self.bom, _ /*~_*/ );
		if (len === 0) return [];
		...
		while (len > 0) {
			...
			if (lines.length > 1)
				...
				curr += (len - bck);
				len = fs.read(self.fd, cache, 0, cache.length, curr + self.bom, _ /*~_*/ );
			}
		}
		// end
	...
	x3get: function(_, size) {
		...
		var len = fs.read(self.fd, bf, 0, bf.length, curr, _ /*~_*/ );
	...
	x3put: function(_, nbr, vals, size) {
		...
		while (vals[i]) {
			...
			if (Array.isArray(v)) {
				...
				for (var j = min; j <= max; j++) {
					if (t[j]) {
						var val = t[j].x3ToBlbfile(size[i]).valueOf();
						tsize += val.length;
						vlist.push(val);
						if (++nbv >= nbr) break;
					} else break;
		...
		fs.write(self.fd, bf, 0, bf.length, curr, _);
	},
