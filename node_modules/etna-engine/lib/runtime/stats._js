...
exports.functions = {
	...
	MAXCOL: {
		...
		fn: function(_, target, property, range) {
			for (var i = range[0][0], maxVal = 0; i <= range[0][1]; i++) {
				maxVal = Math.max(maxVal, target.index(_, i).get(_, property));
			}
			//dbg:console.error("MAXCOL "+property+" ["+range[0][0]+".."+range[0][1] +"]="+maxVal);
			return maxVal;
		}
	},
	MIN: {
