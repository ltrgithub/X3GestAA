...
Datetime.prototype.type = function() {
  return new Integer(526) ;
} ;
...
Datetime.prototype.toTString = function( ) {
...
Datetime.prototype.compare = function(v) {
    if(exports.isDatetime(v)) return base.Datetime.prototype.compare.call(this,v);
...
exports.parse = function(str) {
  if(!tstring.isString(str)) throw util.badOperand();

  if(/^\d{4}-(?:1[0-2]|0\d{1})-\d{2}T\d{2}\:\d{2}\:\d{2}Z$/.test(str.toString())){
    return new Datetime(base.parse(str.toString(),"yyyy-MM-ddTHH:mm:ssZ"));
  } else if(/^\d{4}-\d{2}-\d{2}Z$/.test(str.toString())){
    return new Datetime(base.parse(str.toString(),"yyyy-MM-ddZ"));
  } else if(/^\d{2}:\d{2}:\d{2}Z$/.test(str.toString())){
    return new Datetime(base.parse(str.toString(),"HH:mm:ssZ"));
  }
  return new Datetime();
}
