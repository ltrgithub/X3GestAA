...
function DebugContext(socket) {
	...
	this.readCommand = function(_) {
		...
		log.debug("readCommand cmd= " + cmd);
		return cmd;
	};

	this.write = function(_, data, opts) {
		opts = opts || {};
		if (opts.json) {
...
exports.DebugContext = helpers.defineClass(DebugContext, null, {
	...
	hit: function(_, frame, request) {
		...
		log.debug((new Array(~~frame.stackLevel + 1).join("+")) + "hit: state=" + this.state + "/" + this.status + " - " + debug.instruction + " at " + mod + ":" + debug.location.line + ", stepMark=" + this.stepMark + ", debugMark=" + sub.debugStepMark + ", stackLevel=" + frame.stackLevel);
		if (this.stepMark > 0 && debugStepMark >= this.stepMark) {
			this.suspend();
		}
		this.frame = frame;
		this.request = request;
		// log.debug("hit: state=" + this.state + ", request=" + sys.inspect(request));
		// log.debug("hit: state=" + this.state);
		try {
			if (this.state === "init") {
				this.state = "attached";
			...
			if (this.status === "running" && this.breakOn(frame)) {
				log.debug("hit breakOn");
				this.suspend();
			}
			if (this.status === "break") {
				var cmd;
				// log.debug("hit: " + debug.instruction + " in " + mod + ":" + debug.location.line + ", status=" + this.status);
				this.leave = false;
	...
	interpret: function(_, commandLine) {
		...
		log.debug("interpret: '" + commandLine + "' parts=" + parts + " => command=" + typeof(command) + ", resp=" + JSON.stringify(resp));
		if (command) {
			try {
	...
	breakOn: function(frame) {
		var debug = frame.sub.debug,
			superv = frame.context.superv,
			mod = [superv.folderName, _getModule(debug)].join("/");
		if (!debug)
			return;
		// log.debug("breakOn: " + debug.instruction + " in " + mod + ":" + debug.location.line + ", status=" + this.status + ", bp=" + sys.inspect(this.breakpointsByKey));
		// line breakpoints
		var bpkey;
		if (mod) {
	...
	suspend: function(argument) {
		this.status = "break";
	},

	markFrame: function(prev) {
...
var _commandsMap = {
	...
	attach: function(_, resp, options) {
		...
		log.debug("attach: id=" + id + ", session=" + sys.inspect(session, {
			depth: 1
		}));
		if (session) {
			this.suspend();
		...
		// log.debug("attach: id=" + id + ", session=" + typeof(session));
		resp.$.session = id || "";
		this.setStatus(resp);
	},
	...
	stack_get: function(_, resp, options) {
		...
		log.debug("stack_get: dataset=" + dataset);
		if (sub) {
			var loc, stackinfo, debug, level = 0;
			while (sub) {
				...
				stackinfo = {
					...
					where: _getModuleName(debug), // + (debug.instruction ? || ""),
	...
	context_get: function(_, resp, options) {
		...
		resp.$.context = ctxid;
		// log.debug("context_get: frame=" + sys.inspect(this.frame));
		// log.debug("context_get: values=" + sys.inspect(values));
		switch (ctxid) {
			// Local
			case 0:
	...
	property_get: function(_, resp, options) {
		...
		log.debug("property_get name='" + longName + "', value=" + sys.inspect(value));
	...
	source: function(_, resp, options) {
		log.debug("source: options=" + sys.inspect(options));
		...
		log.debug("source: fullpath=" + fullpath + ", uri=" + sys.inspect(uri));
	...
	// stderr
	// -c	[0|1|2] 0 - disable, 1 - copy data, 2 - redirection
	stderr: function(_, resp, options) {
		resp.$.success = 1;
	},
	...
	eval: function(_, resp, options) {
		...
		var value = evalue(_, expr);
		var parents = [];
		log.debug("eval expr='" + expr + "', value=" + sys.inspect(value));
...
function _previousSub(sub) {
	return sub && sub.prevSub || (sub.prev && sub.prev.sub);
}

function _getModule(debug) {
	return debug && typeof(debug.module) === "string" ? debug.module : "";
}

function _getModuleName(debug) {
	var mod = _getModule(debug);
	return mod.substring(0, mod.lastIndexOf("."));
}

function _getProperty(_, parents, name, value, context) {
	parents = parents || [];
	...
	// if (parent == null)
	// 	log.debug("_getProperty: property=" + sys.inspect(value, {
	// 		depth: 3
	// 	}));
	// log.debug("_getProperty: parent=" + parent + ", name=" + name + ", value=" + value + ", type=" + type);
	...
	if (type === "Array") {
		...
		prop.property = value.map_(_, function(_, e, i) {
			return _getProperty(_, descendents, i, e, context);
		});
		...
		prop.property = keys.map_(_, function(_, k) {
			return _getProperty(_, descendents, k, x3Obj ? value.get(_, k, true) : value[k], context);
		});
		if (parent == null) {
