...
asyncTest("TC_AERROR", function(_) {
	...
	setTimeout(_, 100);
	instance.ASETERROR(_, "CHAPITRE", "Neil Young new EP", constants.CST_OK);
	setTimeout(_, 100);
	instance.ASETERROR(_, "", "Liam Gallagher drunk", constants.CST_INFO);
	setTimeout(_, 100);
	instance.ASETERROR(_, "CHAPITRE", "Pixies new bass player", constants.CST_INFO);
	setTimeout(_, 100);

	var err = instance.getDiagnoses();
	...
	setTimeout(_, 100);
	instance_c1.ASETERROR(_, "", "Elvis Alive", constants.CST_AFATAL);
	setTimeout(_, 100);
	...
	err = instance_c1.getDiagnoses();

	strictEqual(err[0].message, "Celine DION sings AC/DC");
	strictEqual(err[1].message, "Elvis Alive");
	strictEqual(err[2].message, "The Cure remix");
	strictEqual(err[3].message, "Neil Young new EP");
	strictEqual(err[4].message, "Liam Gallagher drunk");
	strictEqual(err[5].message, "BB King dead");
	strictEqual(err[6].message, "Pixies new bass player");
