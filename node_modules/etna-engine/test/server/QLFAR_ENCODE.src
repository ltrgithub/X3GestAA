...
Call OUVRE_TRACE("QLFAR_ENCODE") From LECFIC
Call TESTSUITE()

Call FERME_TRACE From LECFIC
GTRACE = "QLFAR_ENCODE_"+GACTX.USER
Call LEC_TRACE From LECFIC
End

Funprog TESTSUITE()
Call ADD_TESTCASE("TEST_STRENCODE", "Encoding to UTF-8 blob", 15) From AXUNIT
Call ADD_TESTCASE("TEST_STRDECODE", "Decoding from UTF-8 blob", 15) From AXUNIT
Call ADD_TESTCASE("TEST_B64ENCODE_STRING", "Encoding to base64 string", 10) From AXUNIT
Call ADD_TESTCASE("TEST_B64DECODE_STRING", "Decoding from base64 string", 5) From AXUNIT

Local Clbfile  SUITE_RESULT
SUITE_RESULT = func AXUNIT.RUN_TESTSUITE("QLFAR_ENCODE", "QLFAR_ENCODE")
End SUITE_RESULT
...
Subprog CHECK_B64ENCODE_STRING(STRIN, EXPECT)
  ...
  BLB = toBlbfile(STRIN,CST_ASCII)
  GOT = toBase64(BLB)
  Call CHECK_EQUAL(GOT, EXPECT) From AXUNIT

  #Direct encoding : 
  Call CHECK_EQUAL(toBase64(STRIN), EXPECT) From AXUNIT
End
...
Subprog CHECK_B64DECODE_STRING(STRIN, EXPECT)
  ...
  BLB = fromBase64(STRIN)
  GOT = toClbfile(BLB, CST_ASCII)
  Call CHECK_EQUAL(GOT, EXPECT) From AXUNIT
End
...
Subprog CHECK_ENCODE_UTF8(STRIN, EXPECT)
  ...
  BLB = toBlbfile(STRIN, CST_UTF8) : # utf-8 bytes
  Call CHECK_EQUAL(len(BLB), len(EXPECT)) From AXUNIT

  #reverse :
  CLB = toClbfile(BLB, CST_ASCII) : # utf-8 string
  Call CHECK_EQUAL(CLB,      EXPECT) From AXUNIT
  Call CHECK_EQUAL(len(CLB), len(EXPECT)) From AXUNIT

  CLB = toClbfile(BLB, CST_UTF8) : # iso-8859-1 string
  Call CHECK_EQUAL(len(CLB), len(STRIN)) From AXUNIT
...
Subprog CHECK_DECODE_UTF8(STRIN, EXPECT)
  ...
  BLB = toBlbfile(STRIN, CST_ASCII) : # iso-8859-1 bytes
  Call CHECK_EQUAL(len(BLB), len(STRIN)) From AXUNIT

  CLB = toClbfile(BLB, CST_ASCII) : # iso-8859-1 string
  Call CHECK_EQUAL(len(CLB), len(STRIN)) From AXUNIT
  Call CHECK_EQUAL(CLB, STRIN) From AXUNIT

  CLB = toClbfile(BLB, CST_UTF8) : # utf-8 string
  Call CHECK_EQUAL(len(CLB), len(EXPECT)) From AXUNIT
  Call CHECK_EQUAL(CLB, EXPECT) From AXUNIT
End
...
Funprog ENCODE(STRIN)
  ...
  BLB = toBlbfile(STRIN, CST_UTF8) : # utf-8 bytes
  CLB = toClbfile(BLB,  CST_ASCII) : # iso-8859-1
