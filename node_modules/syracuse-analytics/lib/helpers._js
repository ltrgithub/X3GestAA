...
function getAdminId(_) {
	if (!_ADMIN_ID) {
		db = db || adminHelper.getCollaborationEndpoint(_).getOrm(_);
...
function getClientParams(_, context) {
	...
	return {
		...
		//uip: session && (session.getData("requestInfo") || {}).peerAddress, // disabled for now to not send IP address
...
function hideHostname(_, hostname) {
	db = db || adminHelper.getCollaborationEndpoint(_).getOrm(_);
	if (!hostname) return;
	var parts = hostname.split(":");
	var host = parts && parts[0];
	var port = parts && parts[1];
	if (host && port) {
		syracuse = syracuse || require('syracuse-main/lib/syracuse');
		if (!(syracuse.server instanceof mock.MockStreamServer)) {
			return "INDEX:" + port;
		}
		var hosts = db.fetchInstances(_, db.getEntity(_, "host"));
		var _idx;
		hosts.forEach_(_, function(_, h, idx) {
			if (host === h.hostname(_)) _idx = idx;
		});
		if (_idx != null) return "NANNY_" + _idx + ":" + port;
	}
}
...
exports.hitPage = function(_, context, pageName, options) {
	...
	// hack on AppName
	if (appName === "X3 ERP") appName = "SAGE X3";

	// hack for lookup on landing page
	if (options.facet === "$lookup" && appName === "Syracuse Collaboration" && options.origin === "$landing") {
		options.module = "Administration";
	}
...
exports.hitEvent = function(_, category, action, label, value, options) {
	...
	ga.collect.event(evtParams, {
		...
		AppInternal: helpers.uuid.generate('-'),
		AppVersion: getVersion(_),
		AppHost: hideHostname(_, options.host),
	}).store();
};
