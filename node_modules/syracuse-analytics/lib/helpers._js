...
var _TRACKING_ID = "UA-63873235-1"; // default is following ID of property pepx3-test
var _CUSTOMER_ID;
var _SYRA_VERSION;
var _ADMIN_ID;
var _COUNTRY_CODE;
...
function getClientParams(_, context) {
	...
	return {
		...
		cid: getAdminId(_), // Anonymous Customer ID.
		uid: session && session.getData("userID"), // Anonymous Client ID.
		ul: locale.current.toLowerCase(),
		ua: context.request.headers['user-agent'],
		uip: session && (session.getData("requestInfo") || {}).peerAddress,
		geo: getGeoId(_), // Geographical Override
	};
}
