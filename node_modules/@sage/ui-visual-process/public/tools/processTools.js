"use strict";
var _imagesPath = "/@sage/ui-visual-process/public/images/";
var _imagesPatternPath = _imagesPath + "patterns/";
...
function _createPattern(xprop){
    ...
    tmpmage.src = _imagesPatternPath + xprop.xfillProps.xpatLinkId + ".gif";
    ...
    res += "<image xlink:href='" + _imagesPatternPath + xprop.xfillProps.xpatLinkId + ".gif'";
...
exports.createShape = function(xprop, csize, bibMapping, symbolManager){
    ...
    switch (xprop.xshapeType) {
        ...
        case 'fileshape':
            ...
            var collection = bib.famSubpath + bib.catSubpath + bib.name, href = _imagesPath + collection + '/' + xprop.eltid;
...
function _replaceFont(el){
    var $el = $(el), size = $el.attr('size') ? 'font-size:' + $el.attr('size') + 'px;' : '', face = $el.attr('face') ? 'font-family:' + $el.attr('face') + ';' : '', color = $el.attr('color') ? 'color:' + $el.attr('color') + ';' : '';
...
function _replaceParagraph(index, el, xprop){
    var $el = $(el), alignement = $el.attr('align');
...
exports.appendText = function(forme, container){
    ...
    if (forme.xshadowProps.xtype === 'global') 
        style += 'text-shadow: silver 5px 5px 3px;';
    
    var div = document.createElement("div"), // ugly hack to slightly shift the vertical position
...
exports.createText_ie = function(xprop, arge2){
    ...
    var text = $("<text>" + xprop.xtext + "</text>"), svg = '<g transform="translate( ' + xprop.xcaptionPos.xleft + " " + xprop.xcaptionPos.xtop + ')">';
...
function _transform(node, options){
    options = options || {};
    if (node.nodeType === node.TEXT_NODE) {
        ...
        if (node.nodeType === node.ELEMENT_NODE) {
            ...
            var res = "<" + tag, closeTag = "</" + tag + ">", styles = [];
            if (options.newline && tag === "tspan") {
                res += ' x="0" dy="1.2em"';
            ...
            switch (node.localName) {
                case "a":
                    res = '<tspan style="text-decoration: underline;">' + res + _transformChilds(node, options) + closeTag + "</tspan>";
                    break;
                default:
                    res += _transformChilds(node, options) + closeTag;
                    break;
            }
            
            options.newline = options.newline || node.localName === "p";
            return res;
        }
    return "";
}

function _transformChilds(node, options){
    var res = "", child;
    for (var i = 0, len = node.childNodes.length; i < len; ++i) {
        child = node.childNodes[i];
        res += _transform(child, options);
    }
    return res;
}

function _transformAttribute(list, styles){
    var attrs = [], str = "", name, a, att;
...
exports.createText = function(xprop, arge2){
    ...
    var liList = textobj.find("li");
	if (liList) {
		for (var ii = 0; ii < liList.length; ii++) {
			liList[ii].style.listStyleType = "disc";
		}
	}
