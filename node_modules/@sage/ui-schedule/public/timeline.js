...
window.Timeline_ajax_url = '/@sage/ui-schedule/public/deps/timeline/script/timeline_ajax/simile-ajax-api.js';
window.Timeline_urlPrefix = '/@sage/ui-schedule/public/deps/timeline/script/timeline_js/';
window.Timeline_parameters = 'bundle=false';

require('@sage/ui-schedule/public/deps/timeline/script/timeline_ajax/simile-ajax-api');
require('@sage/ui-schedule/public/deps/timeline/script/timeline_js/timeline-api');

var _ressourcePath = "/@sage/ui-schedule/public/deps/timeline/";
exports.stylesheets = [_ressourcePath + "timeline.css"];
...
var Widget = function(context){
    ...
    context.prototype.$properties = context.prototype.$item.$properties;
};


Widget.prototype.resize = function(){
    if (!this._isTimeLineLoaded && this.dataset && this.dataset.length > 0) {
        this.loadTimeLine();
    }
};

Widget.prototype.setValue = function(dataset){
    this.dataset = dataset;
    if (this.dataset && this.dataset.length > 0) {
        this.displayed = this.context.div.clientWidth !== 0;
        if (this.displayed) {
            if (!this._isTimeLineLoaded) {
                // defer chart creation when we are ready to display
                this.resize();
            }
            else {
                $("#s-timeline-loader").toggle();
...
Widget.prototype.loadTimeLine = function(){
    var self = this;
    self._isTimeLineLoaded = true;
    self.bindings = self.context.prototype.$decorator.$bindings;
    self.timelineJSON = {};
    self.wedmax = new Date();
    self.$url = '.'; // The base url for image, icon and background image references in the data
    self.theme = [];
    self.eventDescr = "";
    self.tapeHeight = 8;
    self.xEvent = 0;
    self.yEvent = 0;
    self.event = null;
    self.firstRow = 5; // les données commencent après les lignes réservées aux traductions
    self.eventSource1 = {};
    self.loader = 0;
    if (typeof Timeline === "undefined") {
        console.error("Timeline not loaded");
        return;
    }
    if (typeof SimileAjax === "undefined") {
        console.error("SimileAjax not loaded");
        return;
    }
    if (typeof Timeline.OriginalEventPainter === "undefined") {
        console.error("OriginalEventPainter not loaded");
        return;
    }
    Timeline.OriginalEventPainter.prototype._callX3Event = function(A){
        $("#s-timeline-loader").show();
        var cell = self.dataset[parseInt(A.getID(), 10) + self.firstRow - 1];
        var OppNum = cell[self.bindings.PJT];
        var OppLin = parseInt(cell[self.bindings.OPPLIN], 10);
        var WipTyp = parseInt(cell[self.bindings.WIPTYP], 10);
        var VcrNum = cell[self.bindings.VCRNUM];
        var VcrLin = parseInt(cell[self.bindings.VCRLIN], 10);
        var VcrSeq = parseInt(cell[self.bindings.VCRSEQ], 10);
        var this_TL = this;
        
        _ext.classic.onBlockExRpc(self.context, "GET_INFO", [OppNum, OppLin, WipTyp, VcrNum, VcrLin, VcrSeq], {
            callback: function(res){
                this_TL.eventDescr = res.argsModified[6];
                self.oTL._bandInfos[0].eventPainter._showBubble(this_TL.xEvent, this_TL.yEvent, this_TL.event);
            }
        });
    };
    ...
    Timeline.OriginalEventPainter.prototype._moveX3Event = function(S, E, evt){
        //			var I=parseInt(evt._id)+self.firstRow-1;
        ...
        //					self.oTL._bandInfos[0].eventPainter._showBubble(this_TL.xEvent,this_TL.yEvent,this_TL.event);
        //			}
        //			self.context.onBlockExRpc("MOVE_ORDER", [I,DS,DE]);
    };
        ...
        if (TB._index === 0) {
            ...
            BE.draggable({
                ...
                stop: function(evt, ui){
                    ...
                    B.width = D3;
                    var D4 = parseInt(self.left(D3, String(D3).length - 2), 10);
            ...
            BE.resizable({
                ...
                stop: function(evt, ui){
                    $("#validBtn").button("option", "disabled", false);
                    var D1 = B.elmt.style.left;
                    var D2 = parseInt(self.left(D1, String(D1).length - 2), 10);
                    B.left = D2;
                    var DS = TB.pixelOffsetToDate(D2);
                    DS = SimileAjax.DateTime.removeTimeZoneOffset(DS, TB._timeZone);
                    var D3 = B.elmt.style.width;
    ...
    Timeline.OriginalEventPainter.prototype.paintImpreciseInstantEvent = function(P, U, Z, W){
        ...
        var M2 = function(d, c, e){
            return Q._onMouseOverEvent(C.elmt, c, P);
        };
        ...
        var M3 = function(d, c, e){
            return Q._onMouseOutEvent(C.elmt, c, P);
        };
    ...
    Timeline._Band.prototype._moveEther = function(A, B, E){
        ...
        this._onChanging();
        if ((this._index === 0 && E === "mousemove") || E === "mousewheel") {
            this._viewOffset2 += B;
        }
        if (Timeline._mainBand != null) {
            Timeline._Repos(Timeline._mainBand);
        }
    };
    ...
    Timeline._Band.prototype._onMouseScroll = function(G, H, B){
        ...
        if (!this._lastScrollTime || ((A - this._lastScrollTime) > 50)) {
            ...
            if (F === "scroll" && H.ctrlKey) {
                ...
                if (F === "scroll" && H.shiftKey) {
                    if (this._index === 0) {
                        C = 30 * (I < 0 ? -1 : 1);
                        this._autoScroll(C, function(){
                        }, 400, true);
                    }
                }
                else 
    ...
    Timeline._Band.prototype._autoScroll = function(D, C, dur, Y){
        ...
        if (!Y) {
            B = SimileAjax.Graphics.createAnimation(function(E, F){
                A._moveEther(F, 0, "mousewheel");
            }, 0, D, dur, C);
        }
        else {
            B = SimileAjax.Graphics.createAnimation(function(E, F){
                A._moveEther(0, F, "mousewheel");
            }, 0, D, dur, C);
        }
        B.run();
    };
    ...
    Timeline.WRAPPER = self;
    Timeline.DateTime = SimileAjax.DateTime;
    self.theme = Timeline.ClassicTheme.create();
    self.eventSource1 = new Timeline.DefaultEventSource();
    self.bandInit();
    self.populateJSON();
    self.eventSource1.loadJSON(self.timelineJSON, self.$url); // The data was stored into the timelineJSON variable.
    // Generate HTML
    ...
    loader.id = "s-timeline-loader";
    var img = document.createElement("img");
    img.src = _ressourcePath + "icons/ajax-loader.gif";
    loader.appendChild(img);
    
    zoomElement.appendChild(contTL);
    zoomElement.appendChild(loader);
    
    self.context.div.appendChild(zoomElement);
...
Widget.prototype.bandInit = function(){
    ...
    this.theme.event.bubble.width = 520;
    ...
    this.bandInfos = [Timeline.createBandInfo({
        ...
        zoomSteps: [{
            pixelsPerInterval: 26,
            pixelsPerDay: (26 * 24),
            unit: Timeline.DateTime.HOUR
        }, {
            pixelsPerInterval: 412,
            pixelsPerDay: 412,
            ...
            pixelsPerInterval: 183,
            pixelsPerDay: 183,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 122,
            pixelsPerDay: 122,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 81,
            pixelsPerDay: 84,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 54,
            pixelsPerDay: 54,
            unit: Timeline.DateTime.DAY
        }, // DEFAULT zoomIndex
        {
            pixelsPerInterval: 263,
            pixelsPerDay: (263 / 7),
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 169,
            pixelsPerDay: (169 / 7),
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 113,
            pixelsPerDay: (113 / 7),
            unit: Timeline.DateTime.WEEK
        }, {
            ...
            pixelsPerInterval: 144,
            pixelsPerDay: (144 / (365.25 / 12)),
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 96,
            pixelsPerDay: (96 / (365.25 / 12)),
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 64,
            pixelsPerDay: (64 / (365.25 / 12)),
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 43,
            pixelsPerDay: (43 / (365.25 / 12)),
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 342,
            pixelsPerDay: (342 / 365.25),
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 228,
            pixelsPerDay: (228 / 365.25),
            unit: Timeline.DateTime.YEAR
        }]
        ...
        timeZone: this.DetectTZ.get_timezone_info().utc_offset,
        zoomSteps: new Array({
            pixelsPerInterval: 124,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 82,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 55,
            unit: Timeline.DateTime.DAY
        }, {
            pixelsPerInterval: 256,
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 171,
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 114,
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 76,
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 51,
            unit: Timeline.DateTime.WEEK
        }, {
            pixelsPerInterval: 146,
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 98,
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 65,
            unit: Timeline.DateTime.MONTH
        }, {
            pixelsPerInterval: 43,
            unit: Timeline.DateTime.MONTH
        ...
        {
            pixelsPerInterval: 347,
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 231,
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 154,
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 103,
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 68,
            unit: Timeline.DateTime.YEAR
        }, {
            pixelsPerInterval: 46,
            unit: Timeline.DateTime.YEAR
        })
    })];
    ...
    for (var i = 0; i < this.bandInfos.length; i++) {
        this.bandInfos[i].decorators = [new Timeline.SpanHighlightDecorator({
            endDate: new Date(),
            startDate: new Date(),
            inFront: true,
            cssClass: "s-timeline-today",
...
Widget.prototype.populateJSON = function(){
    ...
    for (var j = this.firstRow; j < nbRows; j++) {
        ...
        this.timelineJSON.events[i] = {
            ...
            'icon': _ressourcePath + "icons/ico_" + wClassName + ".png",
            ...
            //'image':,url to an image self will be displayed in the bubble
            ...
            //'eventID': a cookie attribute self is stored, not used by the Timeline library. If you write a custom labeller or event bubble filler, the attribute can be obtained using the getEventID() method on the event object.
        };
    }
};

Widget.prototype.buildDateISO8601 = function(t, s){
    var wtxt = [];
    wtxt.push("20");
    wtxt.push(this.right(t, 2)); // année
    wtxt.push(s);
    wtxt.push(this.mid(t, 3, 2)); // mois
    wtxt.push(s);
    wtxt.push(this.left(t, 2)); // jour
    wtxt.push("T12:00");
    return wtxt.join("");
};

Widget.prototype.reCreate = function(){
    ...
    if ((this.resizeTimerID == null) && (divTL != null)) {
        ...
        var currDate;
        if (self.oTL) 
            currDate = self.oTL._bands[0].getCenterVisibleDate();
...
Widget.prototype.getCell = function(x, y){
...
Widget.prototype.setupFilterHighlightControls = function(div, timeline, bandIndices, theme){
    ...
    td = tr.insertCell(0);
    SimileAjax.DOM.registerEvent(_ext.dom.input("s-timeline-flt", td), "keypress", handler);
    ...
    function flthov(el){
        el.className = 's-timeline-flthov';
    }
    
    function addHighlight(theme, handler){
        var _tr = self._$("rowFilter");
        var nbHL = 0;
        var trcN = _tr.childNodes;
        for (var i = 0; i < trcN.length; i++) {
            if (trcN[i].firstChild.className === "s-timeline-flt") 
                nbHL++;
        }
        ...
        divColor.className = "s-timeline-surlign";
        divColor.style.background = theme.event.highlightColors[nbHL - 1];
        td.appendChild(divColor);
        if (nbHL >= theme.event.highlightColors.length) 
            self._$("addBtn").disabled = true;
    }
    
    function addButton(btn, par1, par2, par3){
        var td = tr.insertCell(tr.cells.length);
        var defBtn = "<input type='button' class='s-timeline-btn' value='";
        //var defBtn="<input type='button' value='";
        var styBtn = "' onmouseover='this.className=\"s-timeline-btn s-timeline-btnhov\"' onmouseout='this.className=\"s-timeline-btn\"'/>";
        //var styBtn="'/>";
        switch (btn) {
            case "addBtn":
                td.innerHTML += defBtn + "+' id='" + btn + styBtn;
                SimileAjax.DOM.registerEvent(eval(btn), "click", function(){
                    addHighlight(par1, par2);
                });
            ...
            case "todayBtn":
                ...
                SimileAjax.DOM.registerEvent(eval(btn), "click", function(){
                    self.centerSimileAjax(new Date());
                });
...
Widget.prototype.centerSimileAjax = function(date){
    this.oTL.getBand(0).setCenterVisibleDate(date);
};

Widget.prototype.left = function(str, n){
    if (n <= 0) 
        return "";
    else 
...
Widget.prototype.right = function(str, n){
    if (n <= 0) 
        return "";
    else 
        if (n > String(str).length) 
...
Widget.prototype.mid = function(str, start, len){
    if (start < 0 || len < 0) 
        return "";
...
Widget.prototype.BUBBLE = {
    ...
    zoom: function(Objet, Cle){
		_ext.classic.onBlockExRpc(Timeline.WRAPPER.context,"LINKVUE", ["OBJET", Objet, Cle]);
        SimileAjax.WindowManager.cancelPopups();
    }
};
Widget.prototype.DetectTZ = {
    currYear: function(){
        var d = new Date();
