...
var _flashHelpers = require("./flashHelpers");
...
var _initFlash2Js = function(elt, id, callBackFct){
    if (_ext.context.browser.isMSIE) {
		if(elt.attachEvent){
			elt.attachEvent('fscommand', callBackFct);	
		}
        else{
			elt.addEventListener('fscommand', callBackFct,false);
		}
    }
    else {
...
function _getTemplateTechTag(props){
    var template = _.template('<HtcProvType><%= provType %></HtcProvType> \
                    <VisCmp Type="<%= type %>"> \
...
function _getComponentType(article){
    var proto = article.context.prototype, $item = article.context.design, $decoratorType = proto.$decorator && proto.$decorator.$stereotype;
    if (proto.$cube) 
        return "graph";
    if ($decoratorType) {
...
function Wrapper(properties, hosterData, options){
    // Instance properties initialization
    this.typeInfo = _flashHelpers.typeInfo(_getComponentType(hosterData.instance));
    ...
    // Fix size
    if (this.opts._height && this.hosterData.instance && this.hosterData.instance.context.div) {
        this.hosterData.instance.context.div.style.height =  this.props.height = this.opts._height;
    }
    this.$$elt = $(this._getTemplateFlashTag(this.props));
    this.elt = _ext.context.browser.isMSIE ? this.$$elt.get(0) : this.$$elt.find("embed").get(0);
    
    // Perform Flash->Javascript communication initialization
    _initFlash2Js(this.elt, this.id, this._dispatchFxCommand);
    
    // Finally, append Flash element to the context
    this.$$elt.appendTo(this.$$cont);
    
}

Wrapper.prototype._getTemplateFlashTag = function(props){
    var template = _.template('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" align="<%= align %>" \
height="<%= height %>" width="<%= width %>" id="<%= id %>" name="<%= name %>">\
<param name="allowScriptAccess" value="' + (_ext.context.browser.isMSIE ? 'always' : 'sameDomain') + '"/>\
...
<param name="bgcolor" value="<%= bgcolor %>"/>' + //
    ...
    (_ext.context.browser.isMSIE ? '' : ('<embed src="<%= swf %>" name="<%= name %>" quality="high" \
scale="noscale" salign="tl" bgcolor="<%= bgcolor %>" base="<%= base %>" \
...
Wrapper.prototype._populateFxData = function(dataObj, command, preserveNL){
    var res = "xflashreqcode==" + command, excpt;
...
Wrapper.prototype.fxgetctxmenu = function(){
     ...
     */
    var links, $$xmlArgs = $($.parseXML(this.curCmdCtx._args)), $$mmrNode, line, uuid;
    if ($$xmlArgs) {
...
Wrapper.prototype.fxspro = function(){
    ...
    if (this.opts.legacyRequest) {
        ...
        if ($$fxDom) {
            var $$svcId = $$fxDom.find("TYPINTER"), $$fxType = $$fxDom.find("TYPFL");
            request = {
                "svcId": $$svcId.text(),
                "fxType": $$fxType.text(),
                "svcData": {
...
Wrapper.prototype._fxprocessdrill = function(fxcmd, zoomWhat){
    ...
    var hier = this.currentData.$axes[0].$hierarchies, idxHier;
...
Wrapper.prototype._getfxDrillParams = function(fxcmd){
    var $$fxDom = $.parseXML(fxcmd), drillParams = null, critMax = 0, kCrit = "criterion", test;
    if ($$fxDom) {
        if (!this.opts.legacyRequest) {
            drillParams = {
...
Wrapper.prototype.fxnamedsvccall = function(){
     ...
     *                                "iData" is the datasource name (metadata of the component)
     */
    var $$fxDom = $.parseXML(this.curCmdCtx._args), request = null;
    if ($$fxDom) {
    }
...
Wrapper.prototype.fxOnGetCtxMenuDone = function(err, mnuDesc){
    ...
    var reply = "<MN>", debug = (_fixture ? " from : " + this.id : "");
    if (mnuDesc) {
        var data = _.isArray(mnuDesc) ? mnuDesc : [mnuDesc];
        _.each(data, function(mnu){
            if (mnu.$title && mnu.$title.length > 0 && mnu.linkId) 
                reply += '<MI I="' + mnu.linkId + '"><![CDATA[' + mnu.$title + debug + ']]></MI>';
        }, this);
    }
...
Wrapper.prototype.fxOnStdSvcDone = function(err, reply){
     ...
     */
    var preserveNL = false, xmlResult = [], xmlMess = [], i, len, diagnoses, severity, dataRes = false, arrayRes = true, ptlTag = "X3PORTAL>", messTag = "Messages>";
    if (err) {
        this.fxOnError(err);
    }
    else {
...
Wrapper.prototype.cleanUp = function(){
    if (_.isFunction(this.elt.ERPClear)) 
        this.elt.ERPClear();
};
...
Wrapper.prototype.setData = function(data, meta){
    ...
    if (data) {
        ...
        if (this.firstDataDone) 
            this._populateFxData(this._wrapFxDataInObj(this._processDataIn(data, meta)), "fxdataupdated");
        else 
            if (!this.fxInitialDataPending) {
...
Wrapper.prototype._initProps = function(){
    // DHTML Flash context Object propertie
    ...
    if (this.props.swf.charAt(len - 1) === "/" || this.props.swf.charAt(len - 1) === "\\") 
        this.props.swf = this.props.swf.substr(0, len - 1);
...
Wrapper.prototype._getLinkObjFromCanonical = function(canonicalLink){
    var link = {}, tabLink = canonicalLink.split(",");
    if (tabLink.length < 2) {
...
Wrapper.prototype._trace = function(type, mess){
    var logger, $fusionController = _ext.classic.getController(this.hosterData.instance.context);
    if ($fusionController && (logger = $fusionController.getLogger())) {
        ...
        if (!_debug) 
            return;
        if (type != "inf") 
            alert("Object \"" + this.id + "\" : \n" + mess);
        if (this.opts.messCont) 
            $("#" + this.opts.messCont).text("Object \"" + this.id + "\" : \n" + mess);
    }
};
...
Wrapper.prototype._processDataIn = function(data, meta){
    if (this.opts.xmlNative) {
        ...
        var transformer = this['_' + this.typeInfo.name + '_DataJson2Xml'], xml = transformer && transformer.call(this, data, meta);
        _log && console.log("FlashWrapper._processDataIn for '" + this.hosterData.instance.context.design.$bind + "' : xml=" + xml);
        return xml;
    }
};
...
Wrapper.prototype._getGraphDef = function(data, rawData){
    ...
    // Misc
    var axeTitle = null, measuresTitle = this.hosterData.instance.getFieldEvalTitle();
    ...
    if (axeTitle && graphDef._metaData.title != axeTitle) 
        graphDef._metaData.title += ' / ' + axeTitle;
    ...
    if (isOK) {
        ...
        if (!mainHLevelsTab && data.$axes[0].$hierarchies[0].length > 0) {
            ...
            if (mainHLevelsTab && mainHLevelsTab.length > 0) 
                graphDef._metaData.axis0FieldCode = mainHLevelsTab[0];
        }
    }
    ...
    if (!graphDef._metaData.axis0FieldCode || !data.$properties || _.isEmpty(graphDef._metaData.measuresDefinition)) 
        graphDef._metaData.isMetaOK = false;
...
Wrapper.prototype._getGraphStyle = function(cubeDef, measuresDef){
    // Init
    var graphStyle = {}, trick = {}, mStyle = "XPIE", mStyleEX = "", mStyleCode = "", mStyleEXCode = "", mDefault = null;
    var msCode = new Array(measuresDef.length), msCodeStyle = new Array(measuresDef.length), i = 0;
    ...
    if (cubeDef.$style == "pie") {
        ...
        _.each(measuresDef, function(measure){
            ...
            if (measure.definition.$isStacked) {
                ...
                if (measure.definition.$isNormalized) {
                    mStyleEX = "CO";
                    mStyleEXCode = "co";
                }
            if (measure.definition.$isDefault) 
    ...
    if (_.size(trick) <= 1) {
        ...
        if (cubeDef.$displaysOneMeasure) 
            graphStyle.measureStyle = mStyle;
        else 
            if (mStyleEX.length === 0) 
                graphStyle.measureStyle = mStyle + "M";
            else 
                graphStyle.measureStyle = mStyle + mStyleEX;
...
Wrapper.prototype._getLink = function(type, id){
    var links = this._getLinksFromCurrData(), i = 0;
...
Wrapper.prototype._getLinksFromCurrData = function(){
    if (this.currentData._metaData) 
        return this.currentData._metaData.links;
    else 
        return null;
};
...
Wrapper.prototype._graph_DataJson2Xml = function(rawData, meta){
    // Data mapping
    var hasMetaChanged = false, realRawData = null;
    ...
    if (realRawData) {
        ...
        if (realRawData[this.opts.dataBinding]) 
            this.currentData.$resources = realRawData[this.opts.dataBinding];
    }
    
    // Get graph meta-data
    if (hasMetaChanged) 
        _.extend(this.currentData, this._getGraphDef(this.currentData, realRawData));
    var graphDef = {
    ...
    var xmlTitlesTag = ["<Titles><Title>", graphDef._metaData.title, "</Title>"], nbMeasures = 0;
    ...
    var xmlLinesTag = new Array((this.currentData.$resources ? (this.currentData.$resources.length + 2) : 2)), xmlLineTag, colNum, value, detailLinkIdx = null, detailLink;
    xmlLinesTag[0] = "<Lines>";
    if (this.currentData.$resources && graphDef._metaData.isMetaOK) {
        ...
        _.each(this.currentData.$resources, function(occur, idx){
            ...
            if (graphDef._metaData.provType == "TABLEBLOC") 
                xmlLineTag[0] = '<line Num="' + idx + '" Uuid="' + occur['$uuid'] + '"><col><v>' + value + '</v></col>';
            else {
    ...
    if (hasMetaChanged) {
        var graphStyle = {};
        if (this.currentData.$cube && graphDef._metaData.measuresDefinition) 
            graphStyle = this._getGraphStyle(this.currentData.$cube, graphDef._metaData.measuresDefinition);
        if (!graphDef._metaData.isMetaOK) {
            // Fixture
        ...
        // Technical template...
        techTag[0] = "<Technical>";
        techTag.push(_getTemplateTechTag(graphStyle));
        ...
        if (graphDef._drillDef) {
            ...
            techTag.push("</Criteria>");
            if (graphDef._drillDef.currLevel > graphDef._drillDef.minLevel) 
                techTag.push("<PrevDetailTitle>", (this.currentData.$properties[graphDef._drillDef.levelsProperties[graphDef._drillDef.currLevel - 2]].$title || "???"), "</PrevDetailTitle>");
            if (graphDef._drillDef.currLevel < graphDef._drillDef.maxLevel) 
                techTag.push("<NextDetailTitle>", (this.currentData.$properties[graphDef._drillDef.levelsProperties[graphDef._drillDef.currLevel]].$title || "???"), "</NextDetailTitle>");
        }
...
Wrapper.prototype._padDesigner_DataJson2Xml = function(rawData, meta){
    ...
    if (this.currentData.menuNavig) 
        xmlDocTag.push('<MainNavig><![CDATA[', this.currentData.menuNavig, ']]></MainNavig>');
    xmlDocTag.push('</Data>');
...
Wrapper.prototype._agenda_DataJson2Xml = function(rawData, meta){
    var proto = null, hasMetaChanged = false;
    ...
    this.hosterData.instance.context.div.style.height = "450px";
    
    var $resources = this.currentData.$resources, $properties = this.currentData.$properties, $decorator = this.$decorator, x3map = $decorator && $decorator.$bindings || {}, userLang = this.opts && this.opts.userLang || "BRI", xb = _flashHelpers.xmlBuilder($decorator.$stereotype, {
        userLang: userLang
    });
    ...
    function value(k, r){
        var key = x3map[k], p = key && $properties[key], val = key && r[key];
        return p && val && ("" + val);
    }
    ...
    $resources.filter(function(r, i){
        ...
        var actionCode = 0, line = xb.addNode("Data.Lines.Line", null, {
            Num: i + 1
        });
        cols.forEach(function(k, i){
            var vi = value(k, r), v = "", att;
    ...
    var typact = $properties[x3map["TYPACT"]], actionEnum = this.actionEnum || (typact && this.hosterData.instance.getEnumeration(typact.$mnu));
    var colors = ["0xFDE6D5", "0xDEF3DD", "0xF9FDD5"];
    
    // xb.addNode("Data.Categs");
    actionEnum &&
    actionEnum.forEach(function(m, i){
...
Wrapper.prototype._gantt_DataJson2Xml = function(rawData, meta){
    ...
    var $resources = this.currentData.$resources, $properties = this.currentData.$properties, $decorator = this.$decorator, xmlDocTag = [], x3map = $decorator && ($decorator.$bindings || $decorator.$mapping) || {};
    
    function value(k, r, checkRaw, mustRaw){
        var val, rawV, key = x3map[k], p = key && $properties[key];
    ...
    function newEnd(start, duri){
        ...
        var d = _ext.culture.datetime.make(year, month, day, hour, minut);
    ...
    xmlDocTag.push($resources.length ? '<items title="" start="' + start + '" end="' + end + '">' : '<items/>');
    
    var row = 0, header = "", pos = "", header_once = false;
    
    var typact = $properties[x3map["TYPDAY"]], actionEnum = this.actionEnum || (typact && this.hosterData.instance.getEnumeration(typact.$mnu));
    _log && console.log(actionEnum);
    
    $resources.forEach(function(r){
        row++;
        var label = "", key, typ = "", sdate = "", dur = "", elapse = "";
        
        cols.forEach(function(k, i){
            var vi = value(k, r, k == "KEY" || k == "LIB", k == "DURI" || k == "ELAP"), v = "", att;
        ...
        if (parseInt(typ, 10) > actionEnum.length) 
            typ = 0;
        dur = (dur != undefined) ? dur : ""; //DURI is not mandatory  
        _log && console.log(" key:" + key + " label:" + label + " sdate:" + sdate + " dur:" + dur + " typ:" + typ + " elapse:" + elapse);
        if (key) {
            if (sdate != "") {
                if (dur.substring(0, 1) != "-") {
    ...
    if (hasMetaChanged) {
        ...
        var lang = this.hosterData.instance.context.field.page.$fusionPageMeta.controller._session.cdLang;
        lang = lang || "ENG";
...
FlashError.prototype.getXML = function(){
    var xmlDiagnoses = ["<Messages>"], diagnosEx, severity;
    if (this.diagnoses) {
...
exports.load = function(properties, hosterData, options){
    return new Wrapper(properties, hosterData, options);
};
