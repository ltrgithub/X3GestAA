...
exports.addFieldPicker = function(btn) {
	...
	btn.isHidden = btn.isHidden || btn.parent.$isDisabled || btn.parent.$isReadOnly;
	exports.add(btn);
	...
	if (!self.parent.authoringNode && !self.parent.$isDisabled) {
		self.parent.page.externalAdapter.onFieldClickPicker({
			...
			doEvent: function(options) {
				self.pickerclick(options, event);
			}
		});
	}
}
...
exports.add = function(btn) {
	...
	if (btn.click) {
		...
		if (btn.targetClick) {
			btn.targetClick.syraBtnclick = btn.id;
		}
	}
	...
	if (btn.iconOnly) {
		...
		if (!((btn.fontIcon && exports.setFontIcon(link, btn.fontIcon)) || (btn.newFontIcon && exports.setNewFontIcon(link, btn.newFontIcon)))) {
			link.className = btn.css;
			if (btn.imageName) {
				link.className += " s-mn-icon";
				link.style.backgroundImage = "url('" + syra_config.$iconPath + btn.imageName + "')";
			}
		}
	} else {
		var css = btn.css;
		...
		if (btn.titleImage) {
			btn.titleImageItem = syra_dom.div(btn.css + "-title-icon");
			btn.titleImageItem.style.backgroundImage = "url('" + syra_config.$iconPath + btn.titleImage + "')";
		...
		if (btn.fontIcon || btn.imageName) {
			...
			if (btn.fontIcon) {
				...
				btn.iconflag.style.backgroundImage = "url('" + syra_config.$iconPath + btn.imageName + "')";
			}
...
exports.setText = function(btn, text, icon, title) {
	if (btn) {
		...
		if (icon) {
			if (btn.imageName) {
				btn.imageName = icon;
				(btn.iconflag || btn.link).style.backgroundImage = "url('" + syra_config.$iconPath + btn.imageName + "')";
			} else {
...
exports.setNewFontIcon = function(btn, fontIcon) {
	var icon = document.createElement("i");
	icon.className = "s_btn_i s_sagearmonyeicon";
	icon.textContent = fontIcon;
	btn.className = btn.syraIconCss;
...
exports.delayClick = function(btn, on) {
	if (on) {
		if (!btn.delayTimeout) {
			btn.delayTimeout = setTimeout(function() {
				if (btn && btn.link) {
					btn.link.click();
				}
			}, 200);
		...
		if (btn.delayTimeout) {
			clearTimeout(btn.delayTimeout);
			btn.delayTimeout = null;
		}
	}
};