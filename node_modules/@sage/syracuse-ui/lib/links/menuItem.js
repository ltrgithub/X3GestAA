...
function _setTitle(menu, $title, $iconValue) {
	menu.$title = (menu.$item.$title || $title || "") + ""; //ensure title is string
...
function _setMenu(menu, $menu, record, outOfDate) {
	if ($menu) {
		...
		if ($menu.$isRequested === false) {
			if (!syra_router.onActionExecuted(menu, $menu)) {
				return;
			}
		}
		if ($menu.$variants) {
	...
	if ($menu || outOfDate) {
		...
		if (!menu.isRepresentationUrl && ($menu.$type || $menu.$title) && (menu.$sourceType.indexOf("html") < 0)) {
			if ($menu.$url && _isHrefEnabled(menu)) {
				syra_url.setHref(menu, record, true);
			} else {
...
function _load(menu, record) {
	...
	if (menu.$item.$shortCutTip) {
		...
		if (menu.$item.$shortCut) {
			if (!Array.isArray(menu.$item.$shortCut)) {
				menu.$item.$shortCut = [menu.$item.$shortCut];
			}
			for (var i = 0, len = menu.$item.$shortCut.length; i < len; i++) {
				syra_shortcut.dynamic.register(menu, menu.$item.$shortCut[i], menu.$item.$shortCutTip, _processShortCut, null, menu);
			}
		}
	}
	...
	menu.layoutParent && menu.layoutParent.ensureLayoutSlot(menu);
};
...
MenuItem.prototype.dispose = function() {
	...
	if (this.$item && this.$item.$shortCut) {
		for (var i = 0, len = this.$item.$shortCut.length; i < len; i++) {
			syra_shortcut.dynamic.unregister(this.$item.$shortCut[i].item, this.$item.$shortCutTip);
		}
	}
