...
exports.contextualMenu = function(field, event) {
	if (field.picker_menus) {
		if (!field.page.isFusionPage || !event || !event.syraPlayback) {
			setTimeout(function() {
			...
			field.page.externalAdapter.onFieldClickPicker({
				field: field,
				pickerType: "menus",
				doEvent: function doEvent(options) {
					field.picker_menus.pickerclick(options);
				}
			});
		}
		return true;
...
exports.selector = function(field, dateTimeOnly, event) {
	if (!field.$isDisabled && !field.$isReadOnly) {
		...
		if (picker) {
			if (picker != field.picker_$lookup || !field.page.isFusionPage || !event || !event.syraPlayback) {
				setTimeout(function() {
					picker.link.click();
				}, 10);
			} else {
...
exports.tunnel = function(field, event) {
	if (field.picker_$tunnel && (!field.page.isFusionPage || !event || !event.syraPlayback)) {
		setTimeout(function() {
			field.picker_$tunnel.link.click();
		}, 10);
	} else
	if (field.picker_$tunnel || (field.$menusPopup && field.$menusPopup.$tunnel)) {
		field.page.externalAdapter.onFieldClickPicker({
			field: field,
			pickerType: "$tunnel"
		});
...
exports.fire = function(options) {
	...
	if (options.menu && !options.menu.$isDisabled) {
		var page = options.menu.page;
		if (page) {
			if (options.menu.isNoSyracuseUrl) {
				syra_url.setHref(options.menu);
				if (options.menu.domItem) {
					if (options.event) {
					...
					var target = options.menu.$target
					if (target) {
						if (target[0] != "_") {
							target = "_" + target;
						}
						if (target == "_self" && !/exe/i.test(options.menu.$url)) {
							target = null
						}
					}
					window.open(options.menu.$url, target || "_blank");
				}
				return;
			}
			...
			if (options.menu.$bind == "$save" || options.menu.$bind == "$saveAs") {
				...
				if (isValidated && page.uploaders && page.uploaders.getCount()) {
					syra_diagnose.showDiagnoses({
						$diagnoses: [{
							$message: syra_local.fdocInProgress
						}]
					}, page);
					isValidated = false;
				}
			...
			if (isValidated) {
				...
				if (page && !page.disposed) {
					if (page.externalAdapter && page.externalAdapter.onMenuItemClick) {
...
function _runFire(options) {
	...
	if (doClick && options.menu.$item && options.menu.$item.$ownerId) {
