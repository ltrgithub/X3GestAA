...
function SuggestBox(field, $source, slot) {
	...
	this.slot = syra_dom.div("s-suggest-box", slot);

	this.header = syra_dom.text("s-suggest-box-title", this.$title = syra_local["search_" + ($source == "data" ? "documents" : $source)], this.slot);

	this.body = syra_dom.ul("s-suggest-box-items", this.slot);
			...
			options: {
					...
					if (!self.disposed) {
						if (response.ok) {
							...
							self._setEmptyMessage(syra_local.search_no_res);
						}
					}
				}
			}
	...
	this.fill = function(data) {
		...
		if (this.$totalResults) {
			...
			for (var ii = 0, jj = this.$records.length; ii < jj; ii++) {
				...
				var li = syra_dom.li("s-suggest-box-item");
				var row = syra_dom.a("s-suggest-box-item-link", li);
				this.rows.push(row);
				...
				row.appendChild(this.field.drawRecordText("s-suggest-box-item-title", $record.$searchItemTitle, $record));
				isData && row.appendChild(this.field.drawRecordText("s-suggest-box-item-type", $record.$resultTypeTitle, $record));
	...
	this._setEmptyMessage = function(text) {
		syra_dom.empty(this.body);
		syra_dom.text("s-suggest-box-empty", text, this.body);
	};
}
...
helpers.defineClass(SearchField, Field, {
	getUrl: function($seach) {
		return syra_config.$searchUrl + ("&q=" + $seach) + (this.isFuzzyMode ? "&qt=fuzzy" : "") + ("&profile={$userprofile}");
	},
	drawRecordText: function(css, data, dataset) {
		var div = syra_dom.div(css);
		var text = syra_expression.parse(this.page, data, dataset);
		div.innerHTML = syra_core.escape(text.replace(/<em>/g, "<em class='" + css + "-em'>"));
		return div;
	},
	load: function() {
		var self = this;
		self.results = {};
		Field.prototype.load.call(self);

		self.clearBtn = syra_button.add({
			parent: self,
			slot: self._dataValue,
			text: syra_local.siteSearchClear,
			css: "s_sagearmonyeicon s-search-field-clear",
			iconOnly: true,
			click: function() {
				self.setValue("");
				self.applyEscape();
		...
		self.clearBtn.link.textContent = "close"; // hack for sagearmoyeicon
		setTimeout(function() {
			self.focus();
		}, 100);
	},
		...
		if (value !== "") {
			...
			if (self.searchValue !== value) {
				...
				if (!self.suggestPopup) {
					var slot = syra_dom.div("s-suggest");
					self.suggestBox = {
					...
					syra_site.siteApp.appendChild(slot);

					self.suggestPopup = syra_popup.open({
						isAutoClose: false,
						content: self,
	...
	applyEscape: function() {
		clearTimeout(this._searchTimer);
		this.suggestPopup && this.closeSuggest();
		this.onHidden(this.getValue());
	},
				...
				["functions", "data"].forEach(function($source) {
					self.fetchSearch({
						...
						options: {
								...
								if (!self.disposed) {
									if (response.ok) {
										self.results[$source] = response.data;
...
exports.load = function(slot, value, onHidden) {
	syra_site.$prototype.$properties.$search = {
		$type: "application/x-string",
		$title: syra_local.searchFieldTitle
	};
	var field = syra_site.searcher = syra_site.addItem(slot, {
		$bind: "$search",
		$isEditMode: true,
		$class: SearchField,
		$skin: "s-search-field",
