...
var PageBuilder = require("@sage/syracuse-ui/lib/page/pageBuilder");
var base64 = require("msoffice/lib/Base64").Base64;

function WordReportPage() {}

exports.WordReportPage = helpers.defineClass(WordReportPage, DesktopPage, {
	load: function() {
	...
	createWordTemplate: function(wordInterface, refreshOnly) {
		...
		this.createLayout(this.$item, this.$prototype.$properties, null, this.wordLayoutBoxes, null, this.$prototype);
		if (this.embedded) {
			this.createEmbeddedLayout(wordInterface.createWordTemplate, refreshOnly);
		} else {
			wordInterface.createWordTemplate(this.wordLayoutBoxes, this.createSimpleData(refreshOnly), refreshOnly);
		}
	},
	...
	populateWordTemplate: function(wordInterface) {
		...
		this.createLayout(this.$item, this.$prototype.$properties, null, this.wordLayoutBoxes, null, this.$prototype);
		if (this.embedded) {
			this.createEmbeddedLayout(wordInterface.populateWordTemplate);
		} else {
			wordInterface.populateWordTemplate(this.wordLayoutBoxes, this.createSimpleData());
		}
	},
	refreshWordDocument: function(wordInterface) {
		...
		this.createLayout(this.$item, this.$prototype.$properties, null, this.wordLayoutBoxes, null, this.$prototype);
		if (this.embedded) {
			this.createEmbeddedLayout(wordInterface.refreshWordDocument);
		} else {
			wordInterface.refreshWordDocument(this.wordLayoutBoxes, this.createSimpleData());
		}
	},
		...
		for (var ii = 0, jj = self.embedded.length; ii < jj; ii++) {
			...
			syra_router.loadRepresentation({
				...
				success: function(response) {
					...
					$article = PageBuilder.ensureDefaultArticle({
						$facet: response.urlSeg.$facet
					}, $article, response.$representation.$prototype);
					...
					syra_ajax.send({
						url: response.urlSeg.url,
						page: self,
						end: function(response) {
							if (response.ok) {
								self.applyChangeOffice(response.data, response, response.url);
								callback && callback(self.wordLayoutBoxes, self.createSimpleData(refreshOnly), refreshOnly);
							} else {
	...
	_parseMenu: function(article, $menu) {
		...
		if (!syra_url.parse($menu.$url, true).isSyracuse) {
		...
		function addProperties(proto, dataOut, dataIn, embeddedProto) {
			...
			for (property in proto) {
				...
				if (prop.$type === "application/x-document" || prop.$type === "text/rtf" || prop.$type === "text/html" || prop.$type === "text/plain") {
					...
					if (!contentType) {
						contentType = proto[property].$acceptedTypes;
					}
					if (lazyUrl && contentType) {
						...
						syra_ajax.get({
							url: lazyUrl,
							syncRqst: true,
							accept: "html", // Set html since it may be HTML, plain text or rtf ("html" forces jQuery not to parse the response)
							end: function(response) {
								if (response.ok) {
									prop.$value = {
										$value: response.data
					...
					if (p.$links && p.$links.$details && p.$links.$details.$url) {
						prop.$link = format(p.$links.$details.$url, dataIn, null, embeddedProto);
