...
function _loadPage(articleParent, $excelView, dataset) {
	syra_pageBuilder.load({
		$itemPage: {
			layoutSlot: articleParent.page.layoutSlot,
		...
		success: function(page) {
			page.unloadRelated = function(save) {
				...
				setTimeout(function() {
					...
					openerPage.fetch();
					syra_dom.remove(related.domItem);
...
exports.ExcelDatasourcePage = helpers.defineClass(ExcelDatasourcePage, DesktopPage, {
	initializePage: function($itemPage) {
		...
		this.$prototype = {
			$properties: {
				server: {
					$title: syra_local.excel_datasource_server,
					$type: "application/x-string"
				},
				description: {
					$title: syra_local.excel_datasource_description,
					$type: "application/x-string"
				},
				dsName: {
					$title: syra_local.excel_datasource_name,
					$type: "application/x-string"
				},
				title: {
					$title: syra_local.excel_datasource_title,
					$type: "application/x-string"
				},
				endpoint: {
					$title: syra_local.excel_datasource_endpoint,
					$type: "application/x-string"
				},
				endpointRef: {
					$isMandatory: true,
				...
				menuItem: {
					$title: syra_local.excel_datasource_menuItem,
					$type: "application/x-reference",
					$item: {
						$value: "{description}",
						$key: "{description}",
				...
				orderBys: {
					$title: syra_local.excel_datasource_orderBys,
					...
					$links: (this.$facet == "$edit") ? {
						$select: {
							$title: syra_local.excel_datasource_add,
				...
				entity: {
					$title: syra_local.excel_datasource_entity,
					$type: "application/x-string"
				},
				representation: {
					$title: syra_local.excel_datasource_representation,
					$type: "application/x-string"
				},
				representationRef: {
					$title: syra_local.excel_datasource_representation,
				...
				fetchAll: {
					$title: syra_local.excel_datasource_fetchAll,
					$type: "application/x-boolean"
				},
				fetchLimit: {
					$title: syra_local.excel_datasource_fetchLimit,
					$type: "application/x-integer"
				},
				filter: {
					$title: syra_local.excel_datasource_filter,
		...
		if (this.$facet == "$query") {
			this.$prototype = {
				$title: syra_local.excel_datasource_workbook,
				$properties: {
					$resources: {
						$type: "application/x-array",
						$item: this.$prototype
			...
			$items = [{
				...
				renderRecordContent: function(record) {
					...
					var $binds = ["dsName", "server", "description", "filter", "orderBys", "fetchAll", "fetchLimit"];
			...
			switch ($excelView) {
				case "browseMenu":
					this.$prototype.$title = syra_local.excel_datasource_predefined;
					...
					break;
				case "browseEntity":
					this.$prototype.$title = syra_local.excel_datasource_browse;
			...
			if (this.$facet == "$details") {
				$binds = ["entity", "server", "description", "title", "filter", "fetchAll", "fetchLimit", "orderBys"];
			}
			if (!$binds) {
				$binds = ["server", "description", "title", "filter", "fetchAll", "fetchLimit", "orderBys"];
			}
			for (var ii = 0, jj = $binds.length; ii < jj; ii) {
				var $item = {
					$bind: $binds[ii]
			...
			if (this.$facet == "$edit") {
				this.$prototype.$actions = {
					$save: {
						$title: syra_local.excel_datasource_apply
					},
					$abort: {
						$title: syra_local.excel_datasource_cancel
					}
				};
			}
			$items = [{
		...
		switch (this.$facet) {
			case "$query":
				this.layoutContent.domItem.insertBefore(_addMenuBar(this, [{
					$bind: "browseMenu",
					$title: syra_local.excel_datasource_predefined
				}, {
					$bind: "browseEntity",
					$title: syra_local.excel_datasource_browse
				}, {
					$bind: "refreshAll",
					$title: syra_local.excel_datasource_refresh_all,
	...
	updateWorkingCopy: function(sendBag) {
		...
		if (sendBag.$isDeleted) {
			...
			if (sendBag.server !== undefined) {
				resData.server = sendBag.server;
			}
		...
		if ($action && $action.$isRequested) {
			...
			($action.$diagnoses = $action.$diagnoses || []).push({
				severity: "success",
				message: syra_local.excel_datasource_modificationsApplied
			});
...
exports.executeOperation = function(name, id, method, data, onSuccess, onError) {
	...
	if (syra_site.excelDocument.documentUrl)
		syra_site.excelDocument.publishDocument("", null, null, function(data, response) {
			...
			syra_alert.info(syra_local.excel_office_document_saved);
		});
	else {
