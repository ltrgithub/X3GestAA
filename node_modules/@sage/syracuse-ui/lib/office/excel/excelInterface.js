...
var ExcelInterfaceImpl = require("@sage/syracuse-ui/lib/office/excel/excelInterfaceImpl").ExcelInterfaceImpl;
var ExcelInterfaceDummy = require("@sage/syracuse-ui/lib/office/excel/excelInterfaceDummy").ExcelInterfaceDummy;
...
exports.ExcelInterface = helpers.defineClass(ExcelInterface, null, {
	...
	onLogin: function() {
		...
		var role = self.excelInterfaceImpl.getSyracuseRole() || syra_site.userProfile.selectedRoleUiid;
		var loc = self.excelInterfaceImpl.getSyracuseLocale();
		var endpoint = self.excelInterfaceImpl.getSyracuseEndpoint() || syra_site.userProfile.selectedEndpointUiid;
			...
			url: "/sdata/syracuse/collaboration/syracuse/moduleVersions('officeAddin')?representation=moduleVersion.$details",
				...
				if (response.ok) {
					if (response.data && response.data.version) {
						self.excelInterfaceImpl.expectedVersion(response.data.version);
					}
				}
			}
		});
	...
	fetchData: function(page, maxLines, data, onLoaded) {
		...
		page.fetch(options, function(fetchData, response, requestUrl) {
			...
			if (fetchData && !fetchData.$resources) {
				page.applyChangeOffice(fetchData, response, requestUrl, self.excelInterfaceImpl);
				onLoaded && onLoaded();
				return;
			}
