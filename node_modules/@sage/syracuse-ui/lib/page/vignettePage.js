...
exports.VignettePage = helpers.defineClass(VignettePage, DesktopPage, {
	load: function(initData) {
		...
		this.scrollview.className = "s-vignette-scrollview"
		this.body.className = "s-vignette-body";

	},
	endChange: function() {
		DesktopPage.prototype.endChange.call(this);
		if (this.hasProcessus()) {
			var $field = this.vignetteField && this.vignetteField.$field;
			if ($field) {
				var $location = $field.$altLocation || $field.$location;
	...
	onMenuClick: function(options) {
		...
		if (this.hasProcessus()) {
			persisLocation = this.isInMenuBar(options.menu);
			if (!options.menu.$target) {
				options.menu.$target = "_self";
			}
		}
		if (!persisLocation) {
			if (!(options.menu.$sourceBind && options.menu.$sourceBind.indexOf("$drill") >= 0)) {
				if (!options.menu.$target) {
					options.menu.$target = "main";
				}
			}
			persisLocation = !((options.menu.$target && options.menu.$target != "_self") ||
				(options.menu.$method && options.menu.$method != "GET"));
		}
		if (persisLocation) {
			var ldpRecord = this.vignetteField.page.ldpRecord;
			ldpRecord && ldpRecord.saveVignetteLocation(this.vignetteField, options.menu);
		}
		return true;
	},
