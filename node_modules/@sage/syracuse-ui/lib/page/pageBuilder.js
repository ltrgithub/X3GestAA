"use strict";
var _localizationConverter = require('@sage/syracuse-ui/lib/localization/localizationConverter');
...
var vignettePage = require('@sage/syracuse-ui/lib/page/vignettePage').VignettePage;
exports.types = {
	request: require('@sage/syracuse-ui/lib/page/specific/soapPage').SoapPage,
	test: require('@sage/syracuse-ui/lib/page/testPage').TestPage,
	landing: require('@sage/syracuse-ui/lib/landingPage/landingPage').LandingPage,
	portlet: vignettePage,
	vignette: vignettePage,
	fusion: require('@sage/syracuse-ui/lib/fusion/sheetPage').SheetPage,
	search: require('@sage/syracuse-ui/lib/search/searchPage').SearchPage,
	landingVignetteSelect: require('@sage/syracuse-ui/lib/landingPage/select/selectPage').SelectPage
};
...
exports.resize = function() {
	for (var ii = 0, jj = _childPages.length; ii < jj; ii++) {
		var page = _childPages[ii];
		page && page.resizeItem && page.resizeItem(true);
	}
};
...
function _endLoad(context) {
	try {
		...
		/*if (page.$prototype) {
		 ...
		 "dailyBackups": {
		 "$title": "Daily Backups",
		 "$where": "(description like 'DailyBackup%')"
		 },
		 "manualBackups": {
		 "$title": "Manual Backups",
		 "$where": "(not (description like 'DailyBackup%'))"
		 }
		 };
		 }*/
		...
		if (page.$autoFetch === undefined) {
			if (page.$facet == "$edit" && $itemPage.initData) {
				...
				//autofetch will be removed when all page will be moved to react, for moment only query, detail,bulk and lookup are in react if main page
				switch (page.$facet) {
					case "$lookup":
					case "$select":
					case "$edit":
					...
					case "$cube":
					case "$details":
						if ($itemPage.inModal || (page.$pageCategory == "vignette") || syra_site.isOfficeSite) {
							page.$autoFetch = $itemPage.inModal || (page.$pageCategory == "vignette") || syra_site.isOfficeSite;
						} else {
							page.$reactAutoFetch = true;
						}
						break;
				}
			}
		...
		page.isMenuBarDisabled = $itemPage.isMenuBarDisabled;
		page.authoringDisabled = $itemPage.authoringDisabled;
		...
		if (context.$representation.$diagnoses && context.$representation != $itemPage.initData) {
			syra_alert.show({
				$diagnoses: context.$representation.$diagnoses
			}, page);
		} else {
			page.alertPanel && page.alertPanel.refresh();
		}
		$itemPage.$page = $itemPage.boxParent = $itemPage.layoutSlot = null;
		...
		if (!page) {
			if ($itemPage.urlSeg.representationRoot == "landingVignetteSelect") {
				...
				switch ($itemPage.urlSeg.$facet) {
					case '$request':
						page = new exports.types.request();
						break;
					case "$test":
						page = new exports.types.test();
						break;
					case "$search":
