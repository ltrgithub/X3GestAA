...
exports.LandingPage = _helpers.defineClass(LandingPage, DesktopPage, {
	...
	load: function(initData) {
		...
		this.body.className = "s-ldp-body";
		this.scrollview.className = "s-ldp-scrollview";
		...
		if (syra_site.userProfile.authoringAllowed) {
			(this.userList = new UserList()).load(this);
		}

		this.contentActions = syra_dom.div("s-ldp-head-btns", this.headerBottom);
	...
	applyShortCut: function(shortcuts, event) {
		if (shortcuts.esc) {
			var record = syra_ldp.ldpRecord;
			if (shortcuts.f5) {
				record.refreshBtn.link.click();
				return true;
			}
			if (shortcuts.insert || (shortcuts.n && !shortcuts.g)) {
				if (event.shiftKey) {
					record.addVignetteBtn.link.click();
				} else {
					record.addTabBtn && record.addTabBtn.link.click();
				}
				return true;
			}
		}
