...
var ListView = require('@sage/syracuse-ui/lib/scroll/list').List;
...
exports.Record = _helpers.defineClass(Record, null, {
	load: function(list, dataset) {
		...
		if (stdLayout && stdLayout.$item && stdLayout.$item.$links) {
	...
	onVignetteAdded: function($bind, vignette) {
		...
		if (!vignette.endpointFlag) {
			...
			var nature = vignette.$field.$vignetteNature;
			if (nature && vignette.$field.$vignetteEndpoint && !this.useCurrentEndpoint) {
				var natureDiv = syra_dom.div("s-vignette-nature s_endpoint_nature_" + nature);
				vignette.buttonsSlot.parentNode.insertBefore(natureDiv, vignette.endpointFlag.link);
			}
		}
						...
						syra_alert.ask({
							...
							onClose: function(closeBy) {
								if (closeBy == "yes") {
									...
									record.deleteVignettes([vignette], function() {
	...
	activate: function(on) {
		if (on) {
			...
			if (!syra_ldp.contentActions.children.length) {
					...
					if (vignette) {
						if (!this.$vignettes[$bind]) {
							...
							if (this.dataset.stdLayout && this.dataset.stdLayout.content) {
								this.dataset.stdLayout.content = syra_layout.cleanEmptyTabs(syra_ldp.$item);
							}
						}
	...
	addRefresh: function() {
		this.refreshBtn = syra_button.add({
			parent: this,
			slot: syra_ldp.contentActions,
			text: syra_local.ldpRefresh,
			css: "s-ldp-btn",
			iconOnly: true,
			fontIcon: "refresh",
			shortCutTip: syra_shortcut.tip.refresh,
			click: function() {
	...
	addContentActions: function() {
		if (this.isDesignModeEnabled) {
			this.addVignetteBtn = syra_button.add({
				...
				iconOnly: true,
				fontIcon: "create",
				shortCutTip: syra_shortcut.tip.create,
				click: function() {
					...
					syra_over.openSelectModal({
						...
						multiSelect: true,
						url: syra_expression.parse(syra_ldp, $url),
		...
		if (this.isDesignModeEnabled) {
			...
			this.addTabBtn = syra_button.add({
				...
				css: "s-ldp-add-tab",
				shortCutTip: syra_shortcut.tip.newTab,
				click: function() {
	...
	save: function(success) {
		...
		var content = syra_layout.cleanEmptyTabs(syra_ldp.$item);
		...
		syra_ajax.put({
			...
			data: sending,
				...
				if (response.ok) {
					if (!self.disposed && response.data.content) {
						(self.dataset.stdLayout = self.dataset.stdLayout || {}).content = response.data.content;
					}
					success && success();
				}
			}
	...
	clear: function() {
		if (syra_ldp) {
			...
			syra_dom.empty(syra_ldp.contentActions);
		}
	...
	removeTab: function(tab) {
		var self = this;
		var fields = tab.layoutContent.getFields();
		if (fields.length) {
			self.deleteVignettes(fields, function() {
				self.removeTabLayout(tab);
			});
		} else {
			self.removeTabLayout(tab);
		}
	...
	syra_ajax.put({
		...
		data: sending,
			...
			if (!syra_ldp.disposed) {
				if (response.ok) {
					syra_dataset.applyDelta(syra_ldp, record.dataset, response.data);
					if (response.data.$diagnoses) {
						syra_alert.show(response.data.$diagnoses, record);
					}
					success();
				} else {
