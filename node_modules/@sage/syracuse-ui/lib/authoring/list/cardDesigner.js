...
var PageDesigner = require("@sage/syracuse-ui/lib/authoring/pageDesigner").Designer;
var _gridCardBuilder = require('@sage/syracuse-ui/lib/field/array/gridMode/gridCardBuilder');
...
exports.CardDesigner = helpers.defineClass(function() {}, PageDesigner, {
	drawPage: function() {
		...
		syra_button.add({
			parent: this,
			slot: syra_dom.div("s-aw-page-wc", this.layoutSlot),
		...
		this.cardTitle = syra_dom.text("s-aw-carddesigner-title", title);
	...
	reloadDesignedLayout: function($item) {
		...
		syra_layout.extractItems(record.layoutContent, fields);
		...
		syra_menus.applyChange(record, record.$prototype);
		syra_menus.applyChange(record, record.dataset);
		if (prevLayout) {
	...
	_togleOverLaySlots: function(show) {
		...
		if (show) {
			...
			for (var ii = 0, jj = ids.length; ii < jj; ii++) {
				var overlay = this.overlays[ids[ii]] = syra_dom.div("s-aw-carddesigner-overlay", this.designedArticle.page.scrollview);
	...
	endArticleUpdate: function(selectedItem, isStructureUpdated) {
		...
		this.designedArticle && syra_layout.validateLayoutContent(this.designedArticle);
		this.resizeItem();
	},
