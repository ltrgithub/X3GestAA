...
var CardDesigner = require("@sage/syracuse-ui/lib/authoring/list/cardDesigner").CardDesigner;
...
function _loadGraphDesigner(designer) {
	...
	if (hasDecorator || hasCube) {
		...
		var table = syra_dom.div("s-aw-graph");
		var slot = syra_dom.div("s-aw-graph-pos");
		syra_dom.text("s-aw-graph-title", syra_local.lw_graph_position, slot);
		designer.addItem(slot, {
			$bind: "$graphPosition",
			$inplace: true,
		...
		if (hasCube) {
			var chart = designer.designedItem.chartField.extendWidget;
			var $cube = chart.currentMetaData && chart.currentMetaData.$cube;
			if (!$cube || $cube.$style != "spiderweb") {
				slot = syra_dom.div("s-aw-graph-measures", table);
				var measures = syra_dom.div("s-aw-graph-measures-list");
				var delta = {};
...
function _onModeClick(event, btn) {
	...
	if (this.modeType) {
		...
		if (list.$item.$format == "grid") {
			...
			list.$designing.grid.$layout = syra_site.clone(list.$item.$layout);
			if (list.$item.$cardItem) {
				list.$designing.grid.$cardItem = syra_site.clone(list.$item.$cardItem);
			}
		} else {
			list.$designing.cards = {
				$layout: syra_site.clone(list.$item.$layout)
			};
		}
	...
	designer.designedArticle.applyDesignMeta(metaData, true);
...
exports.Designer = helpers.defineClass(function() {}, Designer, {
	...
	drawPage: function() {
		...
		this.settingsSlot = syra_dom.div("s-aw-list-body", this.layoutSlot);
		...
		for (var ii = 0, jj = modeTypes.length; ii < jj; ii++) {
			...
			if (modeType != "grid") {
				var slot = syra_dom.div("s-aw-list-card-position", box);
				if (modeType == "cards") {
					this.sectionCards = {
						body: slot
					};
			...
			if (this.designedArticle.isClientFetch) {
				(this._switchPagerBtns = this._switchPagerBtns || []).push(syra_button.add({
					parent: this,
					slot: box,
					text: syra_local["lw_mode_" + modeType],
					css: "s-aw-list-pager-btn",
					click: _onSwitchPagerClick
				}));
			}
	...
	endArticleUpdate: function() {
		...
		if (this.designedArticle.page.designer) {
			...
			this.designedArticle.resizeItem(true);
			this.saveDesign();
		}
	},
	saveDesign: function() {
	...
	_setSwitchPagerBtnsTitle: function() {
		if (this._switchPagerBtns) {
			...
			for (var ii = 0, jj = this._switchPagerBtns.length; ii < jj; ii++) {
				syra_button.setText(this._switchPagerBtns[ii], title);
			}
	...
	onFormUpdateDelta: function(field, value) {
		...
		if (field.$item.$measure) {
			...
			this.designedArticle.applyDesignMeta({
				$cube: $cube
			}, true);
		} else {
			this.designedArticle.applyDesignMeta(metaData, true);
			var record = this.getDesignedCardItem();
			record && record.designer && record.designer.toggleUIDesign(record, true);
		}
	...
	resizeItem: function() {
		this.cardDesigner && this.cardDesigner.resizeItem();
	},

	appendMeasure: function(bind, delta, body) {
		var chart = this.designedArticle.chartField.extendWidget;
		...
		title.textContent = serie.measure.$title ? syra_localizer.getLocalizeText(this.designedArticle.page, serie.measure.$title) : "";
		...
		this.$prototype.$properties["$cube-color-" + bind] = {
			...
			$value: {
				$type: "application/x-string",
				$enum: syra_site.clone(this.graph.colors)
			}
		};

		var cell = syra_dom.div("s-aw-graph-meassure-item", slot);
		this.graph.styleFields["$cube" + bind] = this.addItem(cell, {
			$category: "field",
			$inplace: true,
			$bind: "$cube" + bind,
			$measure: bind,
		...
		this.graph.colorFields["$cube" + bind] = this.addItem(syra_dom.div("s-aw-graph-meassure-item", slot), {
			$category: "field",
			$inplace: true,
