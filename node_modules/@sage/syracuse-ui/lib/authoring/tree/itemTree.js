...
exports.ItemTree = helpers.defineClass(ItemTree, null, {
	...
	_setParentNode: function(parentNode) {
		if (!parentNode.childRoot) {
			parentNode.children = [];
			parentNode.childRoot = syra_dom.ul("s-aw-tree-level", parentNode.node);
	...
	_createNodeItem: function($designLevel) {
		...
		itemNode.data = syra_dom.div("s-aw-tree-item-data", itemNode.node);

		itemNode.title = syra_dom.div("s-aw-tree-item-title s-aw-tree-" + (itemNode.$designLevel = $designLevel), itemNode.data);
		itemNode.title.syrainout = this.id;
		itemNode.titleLabel = syra_dom.div("s-aw-tree-item-text", itemNode.title);
		return itemNode;
	},
