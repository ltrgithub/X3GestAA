...
var ItemTree = require("@sage/syracuse-ui/lib/authoring/tree/itemTree").ItemTree;

function InsertTree() {}

exports.InsertTree = helpers.defineClass(InsertTree, ItemTree, {
	load: function(designer) {
		ItemTree.prototype.load.call(this, designer, syra_local.aw_insert);
	},
	dispose: function() {
	...
	fill: function() {
		...
		this._addInsertNewSectionNode();
		this._addInsertNewVignette();
		...
		this.$sourceItem = syra_site.clone(this.$sourceItem);
	...
	_addInsertNewSectionNode: function() {
		var itemNode = this._createNodeItem("section");
		this.tabItem.body.appendChild(itemNode.node);
		itemNode.titleLabel.textContent = itemNode.node.title = syra_local.aw_drop_new_section;
		this.nodes.push(itemNode);
		itemNode.item = {
			$title: syra_local.aw_drop_new_section,
			$awAddNewItem: true,
	...
	_addInsertNewVignette: function() {
		var itemNode = this._createNodeItem("field");
		this.tabItem.body.appendChild(itemNode.node);
		itemNode.titleLabel.textContent = itemNode.node.title = syra_local.aw_drop_new_vignette;
		this.nodes.push(itemNode);
		itemNode.item = {
			$title: syra_local.aw_drop_new_vignette,
			$awAddNewItem: true,
			$designLevel: itemNode.$designLevel,
			$field: {
	...
	_addNode: function($item, parentNode) {
		...
		if ($item.$category != "space") {
			if ($item.$items && !$item.$bind && !$item.$category) {
				...
				var itemNode = this._createNodeItem($designLevel);
				...
				if (title == "-" || title == "") {
					title = syra_local["aw_" + itemNode.$designLevel];
				}
	...
	checkNodes: function() {
		var loadedNodes = this.designer.contentTree.sectionsNodes;
