"use strict";
var _addins = [require("syracuse-ui/lib/authoring/fieldAddins/choiceFieldAddin"), require("syracuse-ui/lib/authoring/fieldAddins/documentFieldAddin"), require("@sage/syracuse-ui/lib/authoring/fieldAddins/htmlFielddAddin")];

function _addPanel($title, $skin) {
	$skin = " s-aw-type-" + $skin;
	var slot = syra_dom.div("s-aw-panel" + $skin);
	var head = syra_dom.text("s-aw-panel-head" + $skin, $title, slot);
	return {
		slot: slot,
		head: head,
		itemTitle: syra_dom.div("s-aw-panel-head-title" + $skin, head),
		body: syra_dom.div("s-aw-panel-body" + $skin, slot)
	};
}

function _addBox(container, $title, $skin) {
	$skin = " s-aw-type-" + $skin;
	var slot = syra_dom.div("s-aw-box" + $skin, container);
	syra_dom.text("s-aw-box-head" + $skin, $title, slot);
	return {
		slot: slot,
		body: syra_dom.div("s-aw-box-body", slot)
	};
}

function _addTitleBox(designer, container, $level) {
	var box = _addBox(container, syra_local.aw_sectionAppearance, $level);
	box.titleField = designer.addItem(box.body, {
		$bind: "$title",
		$isEditMode: true,
		$isLeftSpaceHidden: true
	}, designer);
	box.titleChoiceField = designer.addItem(box.body, {
		$bind: "$titleChoice",
		$isTitleHidden: true,
		$isEditMode: true,
		$choiceLayout: "1"
	}, designer);
	box.titleLocalizationField = designer.addItem(box.body, {
...
function _addFieldsLabelBox(designer, container, $level) {
	...
	designer.addItem(box.body, {
		$bind: "$fieldsTitleChoice",
		$isTitleHidden: true,
		$isEditMode: true,
		$choiceLayout: "1"
	}, designer);
	box.labelAlignmentField = designer.addItem(box.body, {
		$bind: "$labelAlignment",
		$isEditMode: true,
		$choiceLayout: "1"
	}, designer);
...
function _addBehaviorBox(designer, container, $level) {
	...
	box.isBoxCollapsableField = designer.addItem(box.body, {
		$bind: "$isBoxCollapsable",
		$isEditMode: true,
		$isLeftSpaceHidden: true
	}, designer);

	box.isMaximizableField = designer.addItem(box.body, {
		$bind: "$isMaximizable",
		$isEditMode: true,
		$isLeftSpaceHidden: true
	}, designer);
...
function _addVignette(designer) {
	...
	syra_over.openSelectModal({
		...
		multiSelect: true,
		url: syra_expression.parse(designer, syra_config.$landingUrl),
		onLoadRepresentation: function(diagnoses) {
			diagnoses && syra_alert.show(diagnoses);
		},
		onSelectRecord: function(selectedVignettes) {
			return selectedVignettes;
		},
		onValidateSelection: function(selectedVignettes) {
			if (selectedVignettes) {
				var newSelect;
				for (var ii = 0, jj = selectedVignettes.length; ii < jj; ii++) {
					var sel = selectedVignettes[ii];
					var newItem = {
						...
						$field: {
							...
							$title: sel.vignette.title,
							$location: syra_site.clone(sel.vignette.$links[Object.keys(sel.vignette.$links)[0]])
						}
					};
					var newChild = syra_layout.onAddNewItem(designedItem.articleParent, designedItem, newItem, layoutParent.isRow ? "right" : "top");
					if (!newSelect) {
						newSelect = newChild;
					}
				}
...
function _fillRowSection(designer, box) {
	if (!box.insertRow) {
		var row = syra_dom.div("s-aw-insert-row");
		syra_dom.text("s-aw-insert-row-label", syra_local.aw_addrow, row);
		var cell = syra_dom.div("s-aw-insert-row-btns", row);
		designer.onAddRowClick = function(event, btn) {
			...
			if (!layout.layoutParent) {
				layout = syra_layout.ensureHasParent(layout);
			}
...
function _addArticle(designer) {
	...
	panel.advancedSection = designer.addItem(panel.body, {
		$bind: "$isAdvanced",
		$isEditMode: true,
		$isLeftSpaceHidden: true
	}, designer);
...
function _addField(designer) {
	...
	box.isTitleHiddenField = designer.addItem(box.body, {
		$bind: "$isTitleHidden",
	...
	panel.fieldAddinSection.properties = [];

	for (var ii = 0, jj = _addins.length; ii < jj; ii++) {
		_addins[ii].load(designer);
		_addins[ii].addToSection(designer, panel.fieldAddinSection);
	}
	return panel;
}
...
function _fillTabChoice(designer, box) {
	...
	if (!box.tabChoice) {
		...
		for (var ii = 0, jj = $layoutTypes.length; ii < jj; ii++) {
			...
			box.tabChoice[$layoutType] = syra_button.add({
				parent: designer,
				slot: box.body,
				...
				click: designer.onTemplateClick,
				$modelType: "modelTab",
	...
	for (var ii = 0, jj = $layoutTypes.length; ii < jj; ii++) {
		...
		syra_dom.disable(link, isLinkDisabled);
	}
}
...
function SettingBar(designer, slot) {
	...
	slot.appendChild(this.slot);
}

SettingBar.prototype.show = function() {
	this.designer.startChange();
	var designedItem = this.designer.designedItem;
	this.selectedPanel && syra_dom.remove(this.selectedPanel.slot);
	...
	if (designedItem.isField) {
		for (var ii = 0, jj = _addins.length; ii < jj; ii++) {
			_addins[ii].onShow(this.designer);
		}
	}
	...
	if (panel.rowSection) {
		...
		syra_dom.hide(panel.rowSection.slot, sectionHidden);
	}
	if (panel.apperanceSection) {
		_fillTabChoice(this.designer, panel.apperanceSection);
		syra_dom.hide(panel.apperanceSection.slot, isHidden);
	}
	if (panel.layoutSection) {
		...
		syra_dom.hide(panel.layoutSection.slot, sectionHidden);
	}
	if (panel.titleSection) {
		syra_dom.toggleClass(panel.titleSection.titleLocalizationField.domItem, "s-aw-title-localization", true);
		...
		syra_dom.hide(panel.titleSection.slot, isHidden);
	}
	panel.behaviorSection && syra_dom.hide(panel.behaviorSection.slot, isHidden);
	panel.fieldsLabelSection && syra_dom.hide(panel.fieldsLabelSection.slot, isHidden);

	if (panel.fieldAddinSection) {
		isHidden = true;
		if (designedItem.$authoringType) {
			var properties = panel.fieldAddinSection.properties;
			for (var ii = 0, jj = properties.length; ii < jj; ii++) {
				var field = properties[ii],
		...
		syra_dom.hide(panel.fieldAddinSection.slot, isHidden);
	}
	this.designer.endChange();
	if (designedItem.$field && designedItem.$field.$type == "application/x-vignette") {
		!designedItem.$field.$location && !designedItem.$contentUrl && _addVignette(this.designer);
	}
};

SettingBar.prototype._addExcludeMe = function(panelId) {
	if (panelId != "article") {
		if (!this.selectedPanel.excludeMeBtn) {
			var slot = syra_dom.div("s-aw-exclude-me-slot");
			this.selectedPanel.body.insertBefore(slot, this.selectedPanel.body.firstChild);
			this.selectedPanel.excludeMeBtn = syra_button.add({
					...
					switch (layoutParent.$layout.$layoutType) {
						...
						case "tabs":
							if (layoutParent.items.length == 0) {
								syra_layout.convertToNewLayout(layoutParent, {
									$layoutType: "stack",
									$width: "100"
								});
							}
		...
		syra_button.hide(this.selectedPanel.excludeMeBtn, !enable);
	}
};

SettingBar.prototype.dispose = function() {
