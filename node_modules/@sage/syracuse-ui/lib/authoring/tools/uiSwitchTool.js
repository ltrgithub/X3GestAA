...
function _onDeleteLayoutClick() {
	...
	setTimeout(function() {
		...
		if (layout.items.length > 0 && layout.isRow) {
			syra_layout.deleteRowColumn(layout, deletedIndex);
			designer.endArticleUpdate(layout, true);
		} else {
			if (layoutParent) {
				layoutParent.removeItem(layout, true);
				layoutParent.isRow && syra_layout.deleteRowColumn(layoutParent, deletedIndex);
			} else {
...
exports.load = function(designer) {
	designer.toggleUIDesign = function(item, enable, disposingDesigner) {
		...
		if (this.designedArticle.isRecordArticle) {
			if (item.articleParent &&
				!((item.articleParent == this.designedArticle.list) ||
		...
		if (item.isSpaceBox) {
			if (enable) {
				if (!item.deleteBtn) {
					...
					item.deleteBtn = syra_button.add({
						parent: this,
						slot: item.body,
						...
						fontIcon: "delete",
						spaceBox: item,
						click: _onDeleteLayoutClick
					});

				}
			} else {
				if (item.deleteBtn) {
					syra_button.remove(item.deleteBtn);
					item.deleteBtn = null;
				}
				syra_dom.toggleClass(item.layoutSlot, "s-aw-designed", false);
				syra_dom.toggleClass(item.layoutSlot, "s-aw-over", false);
			}
			syra_dom.toggleClass(item.layoutSlot, "s-aw-item", enable);
		}
		if (item.isTabLayout) {
		...
		if (domItem) {
			...
			if (item.isTabSection) {
				if (!enable) {
					syra_dom.toggleClass(item.header, "s-aw-designed", false);
					syra_dom.toggleClass(item.header, "s-aw-over", false);
					syra_dom.toggleClass(item.body, "s-aw-designed", false);
					syra_dom.toggleClass(item.body, "s-aw-over", false);
				}
				syra_dom.toggleClass(item.header, "s-aw-item", enable);
				syra_dom.toggleClass(item.body, "s-aw-item", enable);
				syra_dom.toggleClass(item.header, "s-aw-light", enable && isLightMode);
				syra_dom.toggleClass(item.body, "s-aw-light", enable && isLightMode);
			}
			if (domItem && !enable) {
				if (item.body) {
					syra_dom.toggleClass(item.body, "s-aw-over", false);
				...
				syra_dom.toggleClass(item.layoutSlot, "s-aw-designed", false);
				syra_dom.toggleClass(item.layoutSlot, "s-aw-over", false);
				syra_dom.toggleClass(domItem, "s-aw-designed", false);
				syra_dom.toggleClass(domItem, "s-aw-over", false);
			}
			if (item.isLayout) {
				if (item.$layout.$layoutType == "row") {
					item.domItem.syraItem = item.id;
					var children = item.domItem.children;
					item.page.isLandingPage && syra_dom.toggleClass(item.domItem, "s-aw-ldp-row", enable);
					syra_dom.toggleClass(item.domItem, "s-aw-item", enable);
					if (item.layoutSlot) {
						syra_dom.toggleClass(item.layoutSlot, "s-aw-item", enable);
					}
				} else {
					if (item.layoutParent && item.layoutParent.isRow) {
						item.page.isLandingPage && syra_dom.toggleClass(item.layoutSlot, "s-aw-ldp-row-col", enable);
						syra_dom.toggleClass(item.layoutSlot, "s-aw-item", enable);
					}
				}
				if (!enable) {
				...
				if (item.layoutSlot) {
					if (item.page.isLandingPage) {
						if (item.layoutParent && item.layoutParent.isRow) {
							syra_dom.toggleClass(item.layoutSlot, "s-aw-ldp-row-col", enable);
						}
					}
					syra_dom.toggleClass(item.layoutSlot, "s-aw-item", enable);
					item.layoutSlot.syraItem = item.id;
				}
				syra_dom.toggleClass(domItem, "s-aw-item", enable);
				syra_dom.toggleClass(domItem, "s-aw-light", enable && isLightMode);
				if (item.page != item) {
					var show = enable || !(item.$isHidden || item.isItemHidden || item.isInvisible);
					if (item.showSection) {
						item.showSection(show);
					} else {
						item.showField && item.showField(show);
					}
				}
	...
	designer.toggleItemCss = function(item, css, show) {
		if (item.domItem) {
			if (!item.isTabSection) {
				if (item.dataSlot) {
					syra_dom.toggleClass(item.dataSlot, css, show); //page
				} else {
					item.layoutSlot && syra_dom.toggleClass(item.layoutSlot, css, show);
				}
				item.isLayout && syra_dom.toggleClass(item.domItem, css, show);
			}
			item.domItem && syra_dom.toggleClass(item.domItem, css, show);
			item.header && syra_dom.toggleClass(item.header, css, show);
			item.body && syra_dom.toggleClass(item.body, css, show);
		}
	};
};