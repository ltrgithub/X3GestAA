...
var utilEx = require('@sage/syracuse-ui/lib/fusion/tools/util');
var DesktopPage = require('@sage/syracuse-ui/lib/page/desktopPage').DesktopPage;
var _culture = require('@sage/syracuse-ui/lib/fusion/field/culture');
var FusionBar = require('@sage/syracuse-ui/lib/fusion/fusionBar').FusionBar;
var requestActions = require('./core/client/sap/srvactions');
var cvgPageConverter = require('@sage/syracuse-lib/src/collaboration/entities/page/classicPageConverter');
var _fieldTypes = {
	"application/x-choice": require('@sage/syracuse-ui/lib/fusion/field/choiceField').FusionChoiceField,
	"application/x-boolean": require('@sage/syracuse-ui/lib/field/booleanField').BooleanField,
	"application/x-date": require('@sage/syracuse-ui/lib/field/datetime/datetimeField').DateTimeField,
	"application/x-integer": require('@sage/syracuse-ui/lib/field/numericField').NumericField,
	"image": require('@sage/syracuse-ui/lib/fusion/field/documentField').FusionImageField,
	"application/x-exportxls": require('@sage/syracuse-ui/lib/fusion/field/exportXlsField').ExportXlsField,
	"application/x-binary": require('@sage/syracuse-ui/lib/fusion/field/documentField').CvgDocumentField,
	"application/x-string": require('@sage/syracuse-ui/lib/field/stringField').StringField,
	"application/x-reference": require('@sage/syracuse-ui/lib/field/reference/referenceField').ReferenceField,
	"application/x-password": require('@sage/syracuse-ui/lib/field/passwordField').PasswordField
};
...
var vignetteFormats = {
	"$dhtml": require('@sage/syracuse-ui/lib/fusion/field/htmlVignetteField').DHTMLVignetteField,
};
...
exports.SheetPage = helpers.defineClass(SheetPage, DesktopPage, {
	load: function(initData) {
		...
		this.isLargeClassicPage = Object.keys(this.boundFields).length > 5;
		if (this.$prototype.$fusionBar) {
			if (!this.fusionBar) {
			...
			case "application/x-array":
				if ($item.$isNavigationList || this.isNavigationListItem($item.$bind, true)) {
					...
					if (!$field.$treeview) {
						...
						if ($item.$selectMode == "row" && !$field.$decorator && !$field.$cube) {
		...
		if (fieldClass) {
			...
			switch ($class) {
				...
				case "application/x-datetime":
					field.formatEx = {
						formatApi: _culture.getFormatApi($class),
						localFormat: {},
						localFormatDisplay: {},
						formatObj: {}
					};
				...
				case "application/x-real":
					...
					field.formatApi = _culture.getFormatApi($class);
					field.numFormatObj = {
						numFormat: "",
						decimalSeparator: numSettings.decimalSep || ".",
						groupSeparator: numSettings.thousandSep || " ",
						groupSize: 3
					};
	...
	onUncaughtError: function(options) {
		var err, stack, bProcess = true,
			unCaughtObj;
		try {
			var fusionSite = syra_fusion && syra_fusion.book && syra_fusion.book.fusionSite;
			if (fusionSite && fusionSite.controller && fusionSite.controller._sapController) {
				...
				if (bProcess) {
					...
					if (options.error && (options.error.stack || options.error.stackTrace)) {
						stack += "\n" + (options.error.stack || options.error.stackTrace);
					}
