...
exports.getDiagnosesFromAjax = function(errAjax, messObj) {
	...
	if (_.isString(errAjax.data)) {
		...
		diagnoses = errAjax.data && (errAjax.data.$diagnoses || errAjax.data.diagnoses);
	}
...
exports.getLoadPageErrFromAjax = function(response, pageName, messObj) {
	...
	var diagnosis = exports.makeDiagnosis(mess1 + " '" + (pageName || "????") + "'.\n" + mess2, 3, null, "10", exports.cltFusionNice, null, null);
...
var _classLog = helpers.defineClass(function Log(options) {
	...
	_dispReport: function(options) {
		var self = this;
		var boxParams = exports.diagnosesToOurErrEx([exports.makeDiagnosis("Classic client : timing and trace report", "info", null, 0, null, "", null, this._report)]);
...
exports.versionCheck = {
	_minSupvCsvVers: "R090.002",
	_formatSupvVers: function(vers) {
		let formattedVers = 0;
		if (vers && vers[0] === 'R') {
			try {
				let parts = vers.substr(1).split('.');
				if (parts.length > 1) {
					let major = parseInt(parts[0], 10);
					let minor = parseInt(parts[1], 10);
					formattedVers = (major << 24) + minor;
				}
			} catch (ex) {}
		}
		return formattedVers;
	},
	_compareVers: function(vers1, vers2) {
		if (vers1 > vers2)
			return 1;
		else if (vers1 === vers2)
			return 0;
	...
	isSupCsvEnabled: function(vers) {
		return this._compareVers(this._formatSupvVers(vers), this._formatSupvVers(this._minSupvCsvVers)) !== -1;
	}
};