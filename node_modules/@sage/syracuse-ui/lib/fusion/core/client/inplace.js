...
var Controller = require('@sage/syracuse-ui/lib/fusion/core/client/controller').Controller;
var Session = require('@sage/syracuse-ui/lib/fusion/core/client/session').Session;
var util = require('@sage/syracuse-ui/lib/fusion/tools/util');
var utilBis = require('@sage/syracuse-ui/lib/fusion/tools/utilFdb');
var sap = require('@sage/syracuse-ui/lib/fusion/core/client/sap/controller');
var msgBox = require('@sage/syracuse-ui/lib/fusion/core/client/sap/msgbox');
var requestActions = require('@sage/syracuse-ui/lib/fusion/core/client/sap/srvactions');
...
exports.FusionSite = helpers.defineClass(FusionInPlaceSite, null, {
	create: function(uuid, hoster, opts) {
		...
		this.cnxStatus = {
			"current": syra_fusion.sapUtil.cnxStatus.unestablished,
			"history": syra_fusion.sapUtil.cnxStatus.unestablished
		};
		var locale = syra_site.userProfile.selectedLocale;
		this.cdIsoLang = locale && locale.code || syra_fusion.defLang;
		var fns = document[syra_fusion.sapUtil._fNS] = document[syra_fusion.sapUtil._fNS] || {};
		fns.log = fns.log || util.getLog(util.getDefaultLogSwitches(null, null, true));
	},
	...
	_initDisconnectRules: function() {
		...
		this._disconnectCtrlMap = {
			"connecting": {
				...
				"checkArgs": [syra_fusion.sapUtil.cnxStatus.established, true],
			...
			"closed": {
				...
				"messForce": "36",
				"check": this.checkCnxStatus,
				"checkArgs": [syra_fusion.sapUtil.cnxStatus.closedByClt, false],
			...
			"closePending": {
				...
				"messForce": "46",
				"check": this.checkCnxStatus,
				"checkArgs": [syra_fusion.sapUtil.cnxStatus.closing, null, true],
				"forceClose": this.emergencyCltClose
			},
			"jobPending": {
				"rnk": 4,
	...
	_getCnxStatusHistory: function() {
		var strg = ["initialized"],
			stK = syra_fusion.sapUtil.cnxStatus,
	...
	connect: function(urlSeg, onError) {
		var self = this;
		...
		try {
			(this.controller = new Controller()).initialize(this, urlSeg);
			this.updtCnxStatus(syra_fusion.sapUtil.cnxStatus.establishing);
			if (this._fixtureMode(urlSeg, onError)) {
				return;
			}
			this.controller.postCnxpRqst({
				...
				self._bootStrap(urlSeg, $location, sap.getSessSetsFromReply(reply), reply, sap.getsrvOptsFromReply(reply), {
					"cdIsoLang": self.cdIsoLang,
					"standBy": standBy
				}, _onError, acknowledge, notifyComplete);
			}, function(response, isInternalErr, notifyComplete) {
				var ourAjaxErr;
				if (!isInternalErr && (ourAjaxErr = util.isOurAjaxErrEx(response.error))) {
					self._ourConnectError(ourAjaxErr, _onError, notifyComplete);
				} else {
					_onError(isInternalErr ? response.error : util.getDiagnosesFromAjax(response.error, self.cdIsoLang), notifyComplete);
				}
			});
		} catch (ex) {
			self._unLockUI();
	...
	_ourConnectError: function(ourAjaxErr, hosterOnError, notifyComplete) {
		...
		var boxParams;
		var diagnoses = util.makeDiagnosesFromOurAjaxErrEx(ourAjaxErr);
		var diagMess = diagnoses.message || diagnoses.$message;
		if (diagMess) {
	...
	disconnect: function(callback, timeout, force, noNavOnClose) {
		...
		function onSapErrorClosing(diagError, forceClose, messForce) {
			...
			if (forceClose) {
				...
				setTimeout(function() {
					self.controller.sessCloseOnTimeOutPending = false;
		...
		if (diagnosis) {
			...
			post = {
				...
				"callback": function(reply) {
					var _diagnosis;
					if (!syra_fusion.sapUtil.isEndSessReply(reply)) {
			...
			this.controller.sessCloseOnTimeOutPending = timeout;
	...
	_getLogger: function() {
		...
		return logger || document[syra_fusion.sapUtil._fNS].log;
	},
	...
	_bootStrap: function(urlSeg, rqstLocation, sessSettings, statements, srvOpts, localOpts, onError, acknowledge, notifyComplete) {
		try {
			...
			(this._session = new Session()).initialize(sessSettings, urlSeg, localOpts, srvOpts);
			this.controller.completeInit(rqstLocation, urlSeg, this._session, localOpts);
	...
	free: function(reason) {
		...
		this.updtCnxStatus(syra_fusion.sapUtil.cnxStatus.released);
		if (syra_fusion && syra_fusion.book && hosterInst) {
			syra_fusion.book.closeBook(this._noNavOnClose, this.controller && this.controller.sessCloseOnTimeOutPending);
		} else {
			this.dispose();
		}
		if (this.controller && this.controller.sessCloseOnTimeOutPending) {
			this.controller.sessCloseOnTimeOutPending = null;
		}
		logger.trace({
			"severety": "info"
	...
	_fixtureMode: function(urlSeg, onError) {
		// Retrieve test data
		var fixture = false,
			self = this;
		...
		if (!document.syraXTestX) {
			syra_ajax.get({
				url: "/sdata/x3stb/erp/stubs/fusion-testdata",
				end: function(response) {
					if (response.ok) {
						document.syraXTestX = response.data;
					} else {
						void(0);
					}
				}
		...
		// Fixture mode
		if (urlSeg.params && urlSeg.params.d == "on" && urlSeg.params.r) {
			syra_ajax.get({
				url: "/sdata/x3stb/erp/stubs/" + urlSeg.params.r,
				end: function(response) {
					if (response.ok) {
						var data = response.data;
						urlSeg.params = urlSeg.params || {};
						self._bootStrap(urlSeg, null, sap.getSessSetsFromReply(data.r0), data.r0, sap.getsrvOptsFromReply(data.r0), {
							fixture: {
								on: true,
								reply: data,
								next: 1,
						...
						onError(util.getDiagnosesFromAjax(response, self._session));
					}
				}
			});
			fixture = true;
