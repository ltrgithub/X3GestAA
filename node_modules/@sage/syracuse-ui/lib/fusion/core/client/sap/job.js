			...
			try {
				...
				if (!params) {
					...
					syra_trackers.addTracker(params, {
						onLoaded: function(jobItem) {
							job._hosterJobHandler = jobItem;
							jobItem.fusionProps.cancelSvcId = job.getDeleteScvId();
							var nextStateSvcId = job.getProgressScvId();
							var hasError;
							var $diagnoses = jobItem.store.dataset.$diagnoses;
							if ($diagnoses) {
							...
							if (!$diagnoses || ($diagnoses && !hasError)) {
								jobItem.sendRequest({
									method: "POST",
									url: syra_expression.parse(syra_site, jobItem.store.dataset.$location, jobItem.store.dataset.services[nextStateSvcId])
								});
							}
							if ($diagnoses) {
