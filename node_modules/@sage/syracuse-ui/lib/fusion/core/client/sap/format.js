...
FormatterWrapper.prototype.constructor = FormatterWrapper;
var _FormatterWrapperMenbers = {
	initialize: function(extendFmt, locales, hoster, fmtType, opts) {
		...
		if (opts) {
			...
			if (opts.blankStrict) {
				this.formatter.settings.blankStrict = opts.blankStrict;
			}
	...
	editToDisplay: function(fmt, inputValue, rawValue) {
		...
		if (this.isThickFormat(fmt)) {
			try {
				...
				output.$diagnoses = [this._makeDiagnosis("64", "Value does not respect format controls")];
			}
		}
		return output;
	},
...
for (var name in _FormatterWrapperMenbers) {
	FormatterWrapper.prototype[name] = _FormatterWrapperMenbers[name];
}
