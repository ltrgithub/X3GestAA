...
var sapUtil = require('@sage/syracuse-ui/lib/fusion/core/client/sap/util').Fusion;
...
exports.WinStack = helpers.defineClass(WinStack, null, {
	...
	postBuildInit: function(winId) {
		var win = this.getWindow(winId),
			secMap, pageFacet, syraModel, sectionMeta, xid, barLists, i, ii, jj, mm, nn, keysProto, propProto, fields, field, len, metaBar, metaObj = {},
			listId, j = 0,
			lenj, $XID = syra_fusion.sapUtil.metaNameMap.xid;
		var logger, updateItem, actId, isTraceEnable;
		if (win && (syraModel = win.getSyraPage())) {
			...
			// Other...
			if ((logger = this._sapController.getlogger())) {
				actId = requestActions.getLocalAction("LCL_TOGGLE_LOG");
				if (this._sapController.isActionEnabled(winId, actId) !== null) {
					updateItem = {
						"$links": {}
					};
					isTraceEnable = logger.isTraceEnable();
					updateItem.$links[actId] = {
						$title: (syra_fusion.getMessage(logger.isTraceEnable() ? "menuTraceLocalOff" : "menuTraceLocalOn")),
						$icon: {
							$value: isTraceEnable ? "144" : "145"
						}
					};
					syraModel.applyChange(updateItem);
				}
			}
		}
	},
...
WinModel = helpers.defineClass(WinModel, null, {
	...
	getMenuManager: function(onlyName) {
	...
	getBoundField: function(entityName, opts) {
		...
		if (!boundFld && syra_fusion.sapUtil.isSpecialScrnId(entityName)) {
			boundFld = this._getContainerObj(entityName, opts);
		}
		return boundFld;
	},
	...
	setFocus: function(highlight, style, entity, nl, editMode, replydIst, previousIst, requestedIst) {
		var fusionController, self = this,
			entityProto, $entityEx, $bounds = this._fieldsColl.bound,
			args, setFocusStyleProxy, $screenTab = null,
			screenTab, evtData, $boundField, len, index, sess, boundField;
		var retOk = null,
			proxyFocusable = true,
		...
		if (!entity || syra_fusion.sapUtil.getScreenRank(entity) == 0) {
			...
			if (($entityEx = this.getEntityEx(entity)) && $entityEx.entity) {
				if ($bounds[$entityEx.entity] && ((len = $bounds[$entityEx.entity].length) > 0)) {
					...
					if ($boundField.focus) {
						if ($entityEx.type == syra_fusion.syraUtil.dataTypes.collectionType && $boundField.$orderBy && !$boundField.$field.$isFusionNavigationList) {
							if (args.length > 3 && args[3])
								$boundField.sorter.cancel();
						}
						proxyFocusable = $boundField.focus.apply($boundField, args);
					}
					...
					if (proxyFocusable == null && $entityEx.type == syra_fusion.syraUtil.dataTypes.collectionType && syra_fusion.syraUtil.isEntityPopupCard(entity, (entityProto = this.getProto()))) {
						// Perhaps mandatory field just set in cardview... or focus for second time (even if field is already filled or/and not mandatory)
						if (!this._hackCardViewFocus) {
							this._hackCardViewFocus = {};
						}
						if ((syra_fusion.syraUtil.isEntityMandatory(entity, entityProto) || (this._hackCardViewFocus[entity] && this._hackCardViewFocus[entity] == 1)) && syra_fusion.sapUtil.isEmptyData(syra_fusion.syraUtil.getEntityType(entity, entityProto), this.getDataStoreValue(entity)[index])) {
							if (this._focusFieldCardView($entityEx.entity, entity, index, $boundField, arguments, retOk)) {
								ret = retOk;
							}
						} else
						if (this._hackCardViewFocus[entity] && this._hackCardViewFocus[entity] > 1) {
							if (this._focusFieldCardView($entityEx.entity, entity, index, $boundField, arguments, retOk)) {
								ret = retOk;
							}
						} else {
							if (this._hackCardViewFocus[entity] && this._hackCardViewFocus[entity] == 1) {
								this._hackCardViewFocus[entity] += 1;
							} else {
								this._hackCardViewFocus = {};
								this._hackCardViewFocus[entity] = 1;
							}
					...
					if (proxyFocusable === false && $entityEx.type == syra_fusion.syraUtil.dataTypes.collectionType && syra_fusion.sapUtil.isFieldInst(entity)) {
						boundField = this.getBoundField(entity, {
							"arrayCell": nl
						});
						if (boundField && boundField.$isDisabled && !boundField.$isHidden && !editMode && requestedIst && ((requestedIst.v == requestActions.getSrvAction("SRV_OPEN_CTXMENU") && requestedIst.rqtdist.xid == ($entityEx.entity + "1") && requestedIst.rqtdist && requestedIst.rqtdist.nl == nl) || requestedIst.v == requestActions.getSrvAction("SRV_CHAMP_SUIVANT") || requestedIst.v == requestActions.getSrvAction("SRV_CHAMP_PRECEDENT"))) {
							// Symptom issue SAM 117912 (github #9050) : workaround to be able to display properly the "action" popup menu on a record disabled. In this situation (cf. conditions), instance is really relevant and we can accept the focus
							ret = retOk;
						}
					}
				}
		...
		if (ret === retOk && this._hackCardViewFocus) {
			this._hackCardViewFocus = null;
		}
		return ret;
	},
	_focusFieldCardView: function(entity, fieldName, index, list, setFocusArgs, retFocusCheck) {
		var uuid = this.getCollRcdUUID(entity, index, true);
		var ret = false;
		if (uuid != null) {
			syra_menus.click.recordCard(list.recordsMap[uuid]);
			ret = (this.setFocus.apply(this, setFocusArgs) === retFocusCheck) ? true : false;
		}
		return ret;
	},
	...
	_setStatusBar: function(status, options, fromSrv) {
		...
		if (status.notify) {
			...
			syra_alert.show(status.notify, page, "error", false, null, true);
		}
		return null;
	},
	...
	_setDataField: function(entityName, entityBound, winProto, recordSet, data, options, refProps) {
		...
		if (dataPtr.fmt !== undefined) {
			...
			if (!yetFormatted && fmt && (newFmt || newRawValue)) {
				try {
					data.v = (!rawValue && !dataPtr._fmtEmpty) ? rawValue : this._fmtWrapper.format(fmt, rawValue, true, false);
				} catch (e) {
					data.v = rawValue;
				}
			}
	...
	_setDataColl: function(entityName, entityBound, winProto, recordSet, data, options) {
		var mvtCode, delta, rcd = {},
			commit = _storeCommit(options),
			self = this,
			elapseUnsort = 0,
		...
		if (data.$meta) {
			...
			if (setUI && entityBound) {
				var changeEx = [],
					iEx, lenEx, collTitle;
				if ((collTitle = this._dataset[entityName].purgeCollTitleMeta(data.$meta)) !== null) {
					this._setDataContainer(entityName, {
						"tit": collTitle
					}, {
						"isGridInst": true
					});
	...
	_setDataContainer: function(containerName, data, options) {
		...
		if ((obj = this._getContainerObj(containerName, options))) {
			if (data.tit != null) {
	...
	_getContainerObj: function(containerName, opts) {
		...
		if (syra_fusion.sapUtil.isScreenInst(containerName) && this.$folderTabs) {
			...
			if (!$screenTab && syra_fusion.sapUtil.isTabScreen(containerName)) {
				if ((fusionController = syra_fusion.syraUtil.getFusionController(this.syraModel)) && fusionController._sapController) {
					...
					if ($screenTab === null && !sapUtil.isFolderTab(containerName) && opts && opts.noStrict) {
						// Folder group XID, we take the fisrt tab - SAM 113642
						$screenTab = this.$folderTabs[0];
					}
				}
			}
		} else
		if (syra_fusion.sapUtil.isButtonScreen(containerName)) {
			if ((button = this.syraModel.menuItems)) {
				if (!button[containerName]) {
					menus = Object.keys(button);
					contObj = menus.length > 0 ? button[menus[0]][0] : null;
				} else {
					contObj = button[containerName][0];
				}
			}
		} else
		if (opts && opts.isGridInst && this._fieldsColl.bound[containerName]) {
			contObj = this._fieldsColl.bound[containerName][0].boxParent || null;
		}
	...
	_getDataColl: function(entityName, line, isExternalWrap, onlyValue, opts) {
		var data, isLinesColl = syra_fusion.sapUtil.isBlockInst(entityName),
			dataSet, options, getRawVal, rawVal, i, len, parentEntity, dt, pendingM;
			...
			if (isLinesColl) {
				...
				if (opts && opts.sort) {
					...
					if (isExternalWrap) {
						...
						pendingM = this._pendingMeta[sapUtil.getScreenId(entityName)].fields;
						if (!onlyValue || pendingM[entityName]) {
							data["meta"] = {
								"$isPartialDelta": true
							};
							this._dataset[entityName].getMetaColl(data["meta"], this._dataset[entityName].meta, this._getSession(), options);
							if (pendingM[entityName]) {
								delete pendingM[entityName];
							}
						}
					} else {
						data = dataSet;
	...
	setFieldError: function(field, errors, raz) {
		...
		var fusionController = syra_fusion.syraUtil.getFusionController(this.syraModel),
			fieldMeta;
		if (!err && field.isInvalid) {
			syra_form.invalidateField(field);
		} else {
			err && syra_form.invalidateField(field, errors, true);
		}
