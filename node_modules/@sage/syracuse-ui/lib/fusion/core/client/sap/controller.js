...
var FormulaRes = require('@sage/syracuse-ui/lib/site/aside/formula');
...
var _debugProto = {
	"debugMode": false,
	"trace": false,
	"$itemPage": require('@sage/syracuse-ui/lib/fusion/tools/debugProto').proto,
	"$facet": "fullscreen",
	"ist": {
		"xid": "AA1",
		"win": "B",
...
function _dispFmtErrBox(params, ctrlObj, replyCompleteCallback) {
	...
	var boxParams = util.diagnosesToOurErrEx(params.$diagnoses);
	msgBox.setMsgBoxDefaultProp(boxParams, 4, 60, ctrlObj._session.getlabel("error", "13", "Input Error"));

	var boxOpts = msgBox.getMsgBoxOpts.call(ctrlObj, boxParams);
	boxOpts.$delay = 10;
	boxOpts.onClose = function() {
		ctrlObj._wndwsStack.setFocus(params.win, true, syra_fusion.sapUtil.focusStyle.fmtErr.code, params.xid, params.nl || 1, (params.edit === null || params.edit === undefined ? true : params.edit), params);
...
function _shouldHighlightTarget(reply, target) {
	var srvops = _getSrvopRqst(reply),
		highlight = true;
	if (srvops) {
		highlight = srvops.v && (requestActions.isActFocusSelected(srvops.v) || (requestActions.isActFldFocus(srvops.v) && !syra_fusion.sapUtil.cmpIst(srvops.rqtdist, target, true)));
	}
	return highlight;
}
...
exports.Controller = helpers.defineClass(SapController, null, {
	initialize: function(siteController) {
		...
		this._currCtx = {
			"ist": syra_fusion.sapUtil.makeIst("A", "", 0),
			"localIst": null,
			"ctxMenu": [],
			"v": null,
			"screenTab": {}
	...
	getRequestData: function(inst, action, params) {
		...
		var valueEx = null,
			data = {
				"act": action,
				"fld": {
					"ist": syra_fusion.sapUtil.makeIst("", "", 0)
				},
				"param": params,
				"tech": {},
				ctrlResult: {
		...
		if (!this._siteController._localOpts.fixture || !this._siteController._localOpts.fixture.on) {
			data.fld.ist = params && params.hasOwnProperty("ist") ? params.ist : syra_fusion.sapUtil.makeIst(this._currCtx.ist.win, this._currCtx.ist.xid, this._currCtx.ist.nl);
			valueEx = this._wndwsStack.getDataInputValue(this._currCtx.ist.win, this._currCtx.ist.xid, this._currCtx.ist.nl || 1);
			data.fld.fmtKind = params && params.hasOwnProperty("xFmt") ? params.xFmt : (valueEx ? valueEx.xFmt : syra_fusion.sapUtil.fieldModeType.show);
			data.fld.ctx = params && params.hasOwnProperty("ctx") ? params.ctx : (valueEx ? valueEx.ctx || {} : {});
	...
	_checkCurrFieldData: function(fieldData, params) {
		...
		if (fieldData && (ist = fieldData.ist)) {
			...
			// Update sent data
			if (ctrlResult.succeeded && !fieldData.notModified && syra_fusion.sapUtil.isFieldInst(ist.xid)) {
	...
	getRequestTech: function(tech) {
		...
		var fns = document[syra_fusion.sapUtil._fNS];
		if (fns && fns.hasOwnProperty(trcSwitch)) {
			tech.trace = tech.trace || {};
			tech.trace.on = fns[trcSwitch];
			tech.trace.server = fns["srvTrcSrv"] || null;
			tech.trace.port = fns["srvTrcPort"] || null;
			tech.trace.level = fns["srvTrcLevel"] || null;
			delete fns[trcSwitch];
		}
	},
	...
	getEntityValueInReply: function(reply, entityIst) {
		...
		if ((ref = reply[_sapK.pNodes.sap]) && (ref = ref[_sapK.pSubNodes.wins]) && (ref = ref[entityIst.win]) && (ref = ref[_sapK.pSubNodes.entities])) {
			...
			if (!entity && entityIst.nl) {
				coll = syra_fusion.sapUtil.getBlockInst(entityIst.xid);
	...
	_initNewCurrIst: function(params) {
		var isSameIst, editVal, currIst, oldRaw, oldFmt, oldIst, field, showVal, oldShow, newEdit, updateData = {},
			updateOpts, win;
		var isGrisOldIst;
		...
		isSameIst = syra_fusion.sapUtil.cmpIst(params, oldIst, true);
		isGrisOldIst = syra_fusion.sapUtil.isGridInst(oldIst);
		if (!params.ctrlFailed) {
			this._wndwsStack.setDirty(oldIst.win, oldIst.xid, oldIst.nl, false);
		}
		if (syra_fusion.sapUtil.isFieldInst(oldIst.xid) && oldIst.edit && (!isSameIst || (!isGrisOldIst && !params.ctrlFailed))) {
			...
			if (oldFmt || isGrisOldIst) {
				oldRaw = this._wndwsStack.getDataStoreValue(oldIst.win, oldIst.xid, oldIst.nl, false, true, {
					"rawVal": oldFmt ? true : false
				});
				if (oldFmt) {
					showVal = this.fmtWrapper.rawToDisplay(oldRaw, oldFmt, true);
				} else if (oldRaw != null) {
					// Need to force the old value in grid, even is the format doesn't exist, in order to bind the grid when the popupcard is currently opened
					showVal = {
						"succeeded": true,
						"show": oldRaw
					};
		...
		if (syra_fusion.sapUtil.isFieldInst(currIst.xid)) {
		...
		updateOpts = {
			...
			"strictFmtManagement": true
		};
		if (showVal && showVal.succeeded) {
			syra_fusion.sapUtil.appendEntitiesValue(updateData, oldIst.win, oldIst.xid, oldIst.nl, showVal.show);
		}
		if (editVal && editVal.succeeded) {
			syra_fusion.sapUtil.appendEntitiesValue(updateData, currIst.win, currIst.xid, currIst.nl, editVal.edit);
		}
		...
		// Misc controls...
		syra_fusion.sapUtil.setFieldMod(this._wndwsStack.getBoundField(currIst.win, currIst.xid), currIst);
		if (!isSameIst) {
			if (isGrisOldIst) {
				// Check grid cardview status...
				syra_fusion.sapUtil.checkPopupCard(oldIst, params, this._wndwsStack.getBoundField(oldIst.win, syra_fusion.sapUtil.getBlockInst(oldIst.xid)));
			}
		}
		return false;
	},
	...
	getGridRecordLinks: function(grid, capabilities, bind, winModel, nl, links) {
		var status, defined = false;
		var isGridDisabled = winModel.getEntityMetaByCriteria(bind, "_fullyDisabledAtRuntime", bind, -1);
		var isGridFull = winModel.isCollFull(bind);
		var lastLine = grid.dataset.length === nl;
		if (capabilities.insert) {
			status = this.isActionEnabled(winModel.getWinId(), requestActions.getSrvAction("SRV_INSERTLINEGRID"));
			links.$create = {
		...
		if (capabilities["delete"]) {
			status = this.isActionEnabled(winModel.getWinId(), requestActions.getSrvAction("SRV_DELETELINEGRID"));
			links.$delete = {
	...
	hasFocus: function(field, winId, nl, extra, discardLine) {
		...
		if (field.hasOwnProperty("xid")) {
			return syra_fusion.sapUtil[method](this.getCurrInst(), field, discardLine);
		} else {
			meta = syra_fusion.syraUtil.getMetaFromObject(field);
			return (meta = syra_fusion.syraUtil.getMetaFromObject(field)) ? syra_fusion.sapUtil[method](this.getCurrInst(), syra_fusion.sapUtil.makeIst(winId, meta.$bind, nl), discardLine) : false;
		}
	},
	...
	forceValue: function(params, noStoreCommmit, hardForce) {
		...
		if (params.xid && params.hasOwnProperty("v") && params.v != null) {
			...
			if (!hardForce && (!istType || istType == syra_fusion.syraUtil.dataTypes.rtfType || syra_fusion.sapUtil.isSpecialScrnId(params.xid))) {
				return null;
			} else
			if (!hardForce && istType == syra_fusion.syraUtil.dataTypes.choiceType && !_.isNumber(params.v)) {
	...
	_getLoadRepParams: function(winParams) {
		var params = {},
			specialWin = syra_fusion.sapUtil.isSpecialWin(winParams.name),
			winType = winParams.type,
			extProps = syra_fusion.sapUtil.getWinExtProps(winParams.extend);
		if (specialWin) {
			params = {
				"name": specialWin.name
			};
			if (specialWin.id == syra_fusion.sapUtil.specialWindow.selbox.id || specialWin.id == syra_fusion.sapUtil.specialWindow.choose.id) {
				params.$facet = syra_fusion.syraUtil.pageFacet.lookup;
			} else
			if (specialWin.id == syra_fusion.sapUtil.specialWindow.formula.id) {
				params.$itemPage = FormulaRes.getPage();
			}
		}
	...
	_shoulDispWinMagneto: function(winFacet, reply) {
		var i, len, ret = false,
			facetCd = syra_fusion.syraUtil.pageFacet,
			actsList, act;
		// Cf. SAM 98635.
	...
	_checkEvilInstance: function(instance) {
		if (syra_fusion.sapUtil.isButtonScreen(instance.xid) && syra_fusion.sapUtil.cmpIst(instance, this.getCurrInst())) {
	...
	_stubWinOpen: function(params, parseCtx, func, reply) {
		...
		if (parseCtx.win2Process) {
			...
			this._siteController.loadRepresentation(loadParams, null, function(page) {
				...
				_onWinOpenErr(util.getLoadPageErrFromAjax(response, winId.name, self._session), response);
			}, function(representation) {
				// Extend prototype with dynamic model and layout
				var dispMagneto, specialWin = syra_fusion.sapUtil.isSpecialWin(winId.name),
					winFacet, menuOpts = null;
				if (specialWin && specialWin.id == syra_fusion.sapUtil.specialWindow.choose.id && winId.appendModel) {
				...
				if (winId.appendModel) {
					syra_fusion.syraUtil.appendProto(syra_fusion.syraUtil.get$FromPage(representation), winId.appendModel, null, true, function(obj, objKey, key, value) {
						if (key === "$X3Fmt" && syra_fusion.sapUtil.isListScreen(objKey) && syra_fusion.sapUtil.isFieldInst(objKey)) {
							obj[key] = value;
						}
					});
				}
				if (!specialWin || specialWin.id == syra_fusion.sapUtil.specialWindow.formula.id) {
					winFacet = loadParams.$facet || syra_fusion.syraUtil.mapWinTypeToFacet(representation.$prototype.$x3Facet);
					dispMagneto = self._shoulDispWinMagneto(winFacet, reply);
					if (!dispMagneto) {
						menuOpts = {
							"hideMagneto": true
						};
					}
					if (dispMagneto === null && parseCtx.pollingMode) {
						// SAM 113666
						if (!parseCtx.checkMagneto) {
							parseCtx.checkMagneto = [];
						}
						parseCtx.checkMagneto.push({
							"$facet": winFacet,
							"winId": loadParams.id
						});
					}
					self._wndwsStack.setWinMenu(loadParams.id, winFacet, winId["mnu"], (syra_fusion.syraUtil.getProtoFromPage(representation) || representation), menuOpts);
				}
			}, _onWinOpenErr);
			ret = {
				"brk": true,
	...
	_stubActs: function(actions, parseCtx, dummy, reply) {
		// Action activation : http://syracuse/w/index.php/Request_Reply_Fusion#sap.acts
		var i, len, links, winModel, menuManager, syraPage;
		if (parseCtx.checkMagneto && (len = parseCtx.checkMagneto.length) > 0) {
			for (i = len - 1; i >= 0; i--) {
				if (this._shoulDispWinMagneto(parseCtx.checkMagneto[i].$facet, reply)) {
					links = {};
					winModel = this._wndwsStack.getWindow(parseCtx.checkMagneto[i].winId);
					menuManager = winModel ? winModel.getMenuManager() : null;
					syraPage = winModel ? winModel.getSyraPage() : null;
					if (syraPage && menuManager) {
						menuManager.getMagnetoLinks(parseCtx.checkMagneto[i].$facet, links);
						syraPage.applyChange({
							"$links": links
						});
						parseCtx.checkMagneto.splice(i, 1);
					}
				}
			}
		}
	...
	_stubSetWins: function(params, parseCtx, dummy, reply) {
		var ret = null,
			retEx = {},
			oldFmtStgBFmt, fmtStg = this.fmtWrapper.getFormatterSettings();
		if (fmtStg) {
	...
	stubSapMisc_help: function(params, callback) {
		...
		(this.getSiteController()).postHelpRqst("", help[0] || syra_fusion.sapUtil.helpHandler.obj.cat, help[1], topWin && topWin.getSyraPage(), null, callback);
		return null;
	},
	...
	_stubSapMisc_openf: function(params) {
		...
		function _openf(protocol, uri, appSrv, delay, options) {
			setTimeout(function() {
				var win, fullUrl;
				if (appSrv !== null) {
					fullUrl = self._session.getAppServerHttpUrl(appSrv, uri);
				} else if (protocol === "classic_page") {
					fullUrl = self._session.getWorkFlowUrl(uri);
				} else {
					if (protocol !== "app") {
				...
				win = window.open(fullUrl, ((options && options.windowName) || "_blank"), (options && options.windowFeatures) || undefined);
				if (!win && !warn) {
					warn = self._session.getlabel("error", "70") + "\n" + self._session.getlabel("error", "71");
					syra_alert.warn(warn);
				} else
				if (options && options.windowTimeOut && win && win.close) {
					setTimeout(function() {
						win.close();
	...
	stubSapTarget_ist: function(params, replyCompleteCallback, reply) {
		...
		if (syra_fusion.sapUtil.isGridInst(this._currCtx.ist) && !syra_fusion.sapUtil.cmpIstGrid(this._currCtx.ist, params, true)) {
			unSelRcd = !syra_fusion.sapUtil.isListScreen(this._currCtx.ist.xid) || syra_fusion.sapUtil.isListScreen(params.xid);
		}
		...
		if (params.ctrlFailed && dispfmtErrMess) {
			...
			ret = this._wndwsStack.setFocus(params.win, highlight, params.ctrlFailed ? syra_fusion.sapUtil.focusStyle.fmtErr.code : syra_fusion.sapUtil.focusStyle.std.code, params.xid, params.nl || 1, (params.edit === null || params.edit === undefined ? true : params.edit), params, {
				"prevIst": oldIst
			}, _getSrvopRqst(reply));
	...
	_stubSapTarget_box: function(params, replyCompleteCallback) {
		...
		syra_fusion.confirmCloseClassicOpened = true;
		var boxOpts = msgBox.getMsgBoxOpts(params, {
				"escape": false
			}),
			self = this;
		boxOpts.$delay = 0;
		boxOpts.onClassicClose = function(resp, closureMode) {
			delete syra_fusion.confirmCloseClassicOpened;
			if (resp.$id != "yes") {
				delete syra_fusion.confirmCloseClassicNext
			}
			var data = syra_fusion.sapUtil.getObjFieldValue(msgBox.getMsgBoxRet(resp, closureMode, params), syra_fusion.sapUtil.fieldModeType.show);
			data.ist = null;
	...
	_stubSapTarget_rpc: function(params, replyCompleteCallback) {
		var self = this,
			nullIst = syra_fusion.sapUtil.isNullIst(params.ist),
	...
	_sapException: function(reply, diagnoses, recovery, localContext, messCallback, replyCompleteCallback) {
		var self = this,
			boxParams, logger = this.getlogger(),
			siteCtrl;
	...
	_appendChooseModel: function(winProto, chooseModel) {
		var j, i = 0,
			...
			listPropsSorted2, listPropsSortedEx = {},
			self = this;
		if ((listProps = syra_fusion.syraUtil.geChooseLstPropFromProperties(chooseModel))) {
			...
			listPropsSorted2 = listPropsSorted.sort(function(left, right) {
				var colIdL = parseInt(syra_fusion.sapUtil.getFieldId(left), 10),
					colIdR = parseInt(syra_fusion.sapUtil.getFieldId(right), 10);
				return colIdL - colIdR;
			});
			for (j = 0; j < i; j++) {
				listProps[listPropsSorted2[j]] = listPropsSortedEx[listPropsSorted2[j]];
			}
			syra_fusion.syraUtil.appendProto(syra_fusion.syraUtil.get$FromPage(winProto), chooseModel, function(obj, key, value) {
	...
	_addChooseProperty: function(obj, key, value) {
		...
		if ((key == "fmt" || key == "$X3Fmt") && this.fmtWrapper) {
			obj.$type = this.fmtWrapper.getSyraTypeFromFormat(value);
			obj.$maxLength = this.fmtWrapper.getMaxLengthFromFormat(value, obj.$type);
	...
	_officeSETLINK: function(id, definition, status, slot) {
		...
		if (winId = this._wndwsStack.getTopWin(true)) {
			if (page = syra_fusion.syraUtil.getPage(this._wndwsStack.getSyraModel(winId))) {
				if (definition) {
					change.$links[idEx] = JSON.parse(definition);
					change.$links[idEx].$confirm = syra_local.installOfficeAddin;
					change.$links[idEx].$officeAddinSetup = syra_officeAddin.exe;
