...
var formatter = require('@sage/syracuse-ui/lib/fusion/core/client/sap/format');
...
function _getMvtCollCode(mvt) {
	return mvt[0] == syra_fusion.sapUtil.dataSetMvt.del ? (mvt.length == 1 ? syra_fusion.sapUtil.dataSetMvt.delAll : syra_fusion.sapUtil.dataSetMvt.del) : syra_fusion.sapUtil.dataSetMvt.insrt;
}
...
exports.EntityStore = helpers.defineClass(EntityStore, null, {
		...
		if (this._isColl) {
			...
			if (this._fmtWrapper && this._proto.$item && (itemsProps = this._proto.$item.$properties)) {
				...
				for (i = 0, len = itemsPropsArray.length; i < len; i++) {
					...
					if (entityType == syra_fusion.syraUtil.dataTypes.reference && (refProp = syra_fusion.syraUtil.getRefPropertyEntity(occur))) {
						entityType = refProp.proto.$type;
						protoFmt = refProp.proto.$X3Fmt;
					}
					if (this._fmtWrapper.isTypeAcceptFmt(entityType)) {
						defFmt = !protoFmt && entityType == syra_fusion.syraUtil.dataTypes.dateType ? KdefDateFmt : KdefFmt;
						if (protoFmt || defFmt) {
							this.metaFmt[itemsPropsArray[i]] = {
								"fmt": protoFmt || defFmt,
								"cellsSet": false,
								"_fmtEmpty": !!this._fmtWrapper.isTypeAcceptEmptyData(entityType),
								"realType": entityType
							};
						}
					}
			...
			if (entityType == syra_fusion.syraUtil.dataTypes.reference && (refProp = syra_fusion.syraUtil.getRefPropertyEntity(this._proto))) {
				// Reference, then...
				entityType = refProp.proto.$type;
			...
			ignoreFmt = !this._fmtWrapper || !this._fmtWrapper.isTypeAcceptFmt(entityType) || !!(protoFmt && protoFmt.substring(0, 2) === "KR"); // Ignores format for RTF or raw text (ie. "KR:..."
			if (this._fmtWrapper && !ignoreFmt) {
				defFmt = !protoFmt && entityType == syra_fusion.syraUtil.dataTypes.dateType ? KdefDateFmt : KdefFmt;
				this.data[KfmtMeta] = protoFmt || defFmt;
				this.data._fmtEmpty = !!this._fmtWrapper.isTypeAcceptEmptyData(entityType);
			}
		}
		return ret;
	},
	...
	applyDataCollEx: function(data, commit, options, callbackBind, callbackUnsort) {
		...
		this._tgFmt = 0;
		if (i >= 0 && data.$mvt[i][0] == syra_fusion.sapUtil.dataSetMvt.insrt) {
			ruptInsrt = i;
	...
	applyCollMvt: function(mvt, oneByOne, options, callbackBind, callbackUnsort) {
		var mvtCode = _getMvtCollCode(mvt),
			...
			partialDelta = true;
		// Apply dataset mvt
		if (mvtCode === syra_fusion.sapUtil.dataSetMvt.insrt) {
			callbackUnsort();
		}
		delta = this.applyMvt(mvtCode, mvt[1], mvt[2]);
		lenCollAfter = this.data.length;
		if (lenCollAfter > 0) {
			this._isLastLineEmpty = this._isLineEmpty(lenCollAfter - 1);
		}
		// Bind
		if (mvtCode === syra_fusion.sapUtil.dataSetMvt.delAll || (lenCollAfter === 0 && mvtCode === syra_fusion.sapUtil.dataSetMvt.del)) {
			syraElapse += callbackBind([], {
				"$isPartialDelta": false,
				"unsortAfter": true
			});
		} else
		if (lenCollAfter == 1 && mvtCode == syra_fusion.sapUtil.dataSetMvt.del) {
	...
	applyMvt: function(mvt, start, count) {
		...
		// Update data
		if (mvt == syra_fusion.sapUtil.dataSetMvt.delAll) {
			result.mvtData = [];
		...
		if (mvt == syra_fusion.sapUtil.dataSetMvt.del) {
				...
				for (jd = 0, lend = newData.length; jd < lend; jd++) {
					...
					if (commit || this._fmtWrapper) {
						...
						for (j = 0, len = rcdArray.length; j < len; j++) {
							if (!this._fmtWrapper || !(dataFmt = this._getMetaFmtColl(rcdArray[j], i, this.metaFmt[rcdArray[j]], options && options.fromSrv && !options.strictFmtManagement))) {
								if (commit) {
								...
								try {
									if (commit) {
										...
										niemeDataPtr[rcdArray[j]] = skip || yetFormatted ? newData[jd][rcdArray[j]] : this._fmtWrapper.format(dataFmt, newData[jd][rcdArray[j]], true, false);
										...
										if (!skip && !yetFormatted) {
											newData[jd][rcdArray[j]] = this._fmtWrapper.format(dataFmt, newData[jd][rcdArray[j]], true, false);
										}
									}
								} catch (e) {
	...
	_cureMetaStateOnArrayScope: function(metaState) {
		var newMetaState = null,
			...
			isHiddenState, newS, state, roCount = 0;
		...
		state = syra_fusion.sapUtil.getState(metaState);
		if ((state.$isDisabled != undefined || state.$isReadOnly != undefined) && itemsPropsArray) {
			isHiddenState = state.$isHidden != undefined;
			newMetaState = [];
			for (i = 0, len = itemsPropsArray.length; i < len; i++) {
				if ((newS = syra_fusion.syraUtil.purgeStateMeta(itemsProps[itemsPropsArray[i]], metaState)) !== null) {
					if (isHiddenState) {
						newS = syra_fusion.sapUtil.removeStateMask({
							"$isHidden": state.$isHidden
						}, newS);
						if (!state.$isHidden) {
							newS -= syra_fusion.sapUtil.statusFlag.show;
						}
					}
					if (newS > 0) {
			...
			if (roCount == itemsPropsArray.length) {
				...
				if (roCount == 0) {
					newMetaState = isHiddenState ? [] : null;
				}
			}
			if (isHiddenState) {
				newMetaState.push([this.entityName, 0, syra_fusion.sapUtil.setState({
					"$isHidden": state.$isHidden
				}, 0)]);
			}
		}
		...
		_.each(newMeta, function(newMetaColl, type) {
			...
			if (type === _datasetK.metaType.state) {
				...
				for (i = len - 1; i >= 0; i--) {
					...
					splited = null;
					occur = newMetaColl[i];
					gridInst = syra_fusion.sapUtil.isBlockInst(occur[0]);
					if (!gridInst && occur[1] == 0) {
						...
						if (!(entityProto = this._proto.$item.$properties[occur[0]])) {
							...
							occur[2] = syra_fusion.syraUtil.purgeStateMeta(entityProto, occur[2]);
							if (occur[2] == null) {
								discard = true;
							} else
							if (syra_fusion.sapUtil.isStateStatedBis(occur[2])) {
					...
					if (gridInst && occur[1] == 0) {
						...
						if ((occur[2] = syra_fusion.syraUtil.purgeStateMeta(this, occur[2])) == null) {
							discard = true;
						} else {
					...
					if (occur[1] > 0 && syra_fusion.sapUtil.isStateStatedBis(occur[2])) {
						// Same rule as CS : cancel "_fullyDisabled" flag when line or cell scope...
						this.meta._fullyDisabled = false;
					}
					if (discard || splited) {
			...
			if (this.meta[type].length > 0) {
				_.each(newMetaColl, function(newMeta) {
					// Clean-up old metas
					gridInst = syra_fusion.sapUtil.isBlockInst(newMeta[0]);
	...
	_cleanupMetaColl: function(mvt, start, count, insertAtEnd) {
		...
		_.each(this.meta, function(metaColl, type) {
			if ((len = metaColl.length) > 0) {
				for (i = len - 1; i >= 0; i--) {
					if (mvt == syra_fusion.sapUtil.dataSetMvt.delAll && metaColl[i][1] > 0) {
						// Remove all none "grid" or "column" scope meta on raz
						metaColl.splice(i, 1);
					} else
					if (mvt == syra_fusion.sapUtil.dataSetMvt.del && metaColl[i][1] > 0) {
					...
					if (mvt == syra_fusion.sapUtil.dataSetMvt.insrt && metaColl[i][1] > start && !insertAtEnd) {
	...
	getMetaByCriteria: function(type, target, level) {
		...
		if (this._isColl) {
			if (type == "_fullyDisabledAtRuntime") {
				ret = !!(this.meta._fullyDisabled);
			} else if (type == "_fullyDisabled") {
				ret = !!(this.meta._fullyDisabled) || this.$isReadOnly;
			} else
			if (this.meta[type] && this.meta[type].length > 0) {
				// Target?
				isRow = target && !syra_fusion.sapUtil.isBlockInst(target);
				if (isRow) {
				...
				for (i = 0, len = this.meta[type].length; i < len; i++) {
					...
					isFieldMeta = !syra_fusion.sapUtil.isBlockInst(meta[0]);
	...
	getMetaCollEx: function(updateEx, meta, session, options) {
		function _purge(metaArray, type) {
			var i, j, k, lenI, scope, toPurge = {},
				purged = [],
				splited = [],
				idx, line = false,
				grid = false,
			...
			for (i = 0, lenI = metaArray.length; i < lenI; i++) {
				// Scan for purge
				idx = metaArray[i][1];
				if (syra_fusion.sapUtil.isBlockInst(metaArray[i][0])) {
					if (idx > 0) {
						line = true;
						nbLines++;
						scope = "ln";
				...
				if (doublon[dblKey] != undefined) {
					...
					if (type == _datasetK.metaType.state) {
						// Warning, for meta "state", need to consolidate!
						purged[purged.length - 1].val = syra_fusion.sapUtil.completeState(metaArray[doublon[dblKey]][2], metaArray[i][2]);
					}
				}
				doublon[dblKey] = i;
				line = false;
		...
		var splitedStatus = _purge(meta[_datasetK.metaType.state], _datasetK.metaType.state),
			lenStatus = splitedStatus.length;
		var splitedStyle = _purge(meta[_datasetK.metaType.style], _datasetK.metaType.style),
	...
	getMetaColl: function(update, meta, session, options) {
		...
		_.each(meta[_datasetK.metaType.title], function(tit) {
			// Array or col level?
			if (syra_fusion.sapUtil.isBlockInst(tit[0])) {
				if (setArray) {
		...
		_.each(meta[_datasetK.metaType.state], function(stt) {
			...
			if (sttDescObj) {
				line = stt.idx;
				id = stt.id;
				st = syra_fusion.sapUtil.getState(stt.val);
			} else {
				line = stt[1] - 1;
				id = stt[0];
				st = syra_fusion.sapUtil.getState(stt[2]);
			}
			if (syra_fusion.sapUtil.isBlockInst(id)) {
				if (line >= 0) {
					if (setLine) {
		...
		_.each(meta[_datasetK.metaType.style], function(sty) {
			...
			styEx[styPropr] = val;
			if (syra_fusion.sapUtil.cureStyle(styEx, this.entityName, session)) {
				if (syra_fusion.sapUtil.isBlockInst(id)) {
					if (line >= 0) {
						if (setLine) {
		...
		_.each(meta[_datasetK.metaType.dchoise], function(dch) {
			...
			var line = dch[1] - 1;
			if (!syra_fusion.sapUtil.isBlockInst(dch[0]) && line >= 0 && setCell) {
	...
	_getMetaFmtColl: function(property, idx, propCurrFmt, srvFmtManagement) {
		var _$raw, fmt = null,
			metaFmtProp;
		if (!srvFmtManagement) {
			fmt = this.getFmtColl(property, idx);
		} else if (this.data.length > idx && (metaFmtProp = this.metaFmt[property])) {
	...
	getFmtColl: function(property, idx) {
		var _$raw, fmt = null,
			metaFmtProp;
		if ((metaFmtProp = this.metaFmt[property]) && this.data.length > idx) {
			fmt = metaFmtProp.cellsSet && (_$raw = this.data[idx].$rawV[property]) && _$raw.fmt || metaFmtProp.fmt;
			return this._fmtWrapper.isThickFormat(fmt) ? fmt : null;
		} else if (this.data.length == 0 && metaFmtProp) {
			return this._fmtWrapper.isThickFormat(metaFmtProp.fmt) ? metaFmtProp.fmt : null;
		} else {
			return null;
		}
	},
	...
	sortDataColl: function(orderBy, raw, opts) {
		...
		function getSortType(binding) {
			...
			if (!rawEx) {
				if (collProps[binding] && collProps[binding].$type == syra_fusion.syraUtil.dataTypes.booleanType || collProps[binding].$type == syra_fusion.syraUtil.dataTypes.choiceType || collProps[binding].$type == syra_fusion.syraUtil.dataTypes.integerType) {
					intg = true;
					flt = false;
				} else if (collProps[binding].$type == syra_fusion.syraUtil.dataTypes.decType || collProps[binding].$type == syra_fusion.syraUtil.dataTypes.realType) {
					intg = false;
					flt = true;
				}
				return {
				...
				intg = !(metaFmt.realType == syra_fusion.syraUtil.dataTypes.stringType || metaFmt.realType == syra_fusion.syraUtil.dataTypes.passwdType);
				if (intg && (metaFmt.realType == syra_fusion.syraUtil.dataTypes.realType || metaFmt.realType == syra_fusion.syraUtil.dataTypes.decType)) {
					intg = false;
					flt = true;
				}
				return {
	...
	_isLineEmpty: function(lineIdx) {
		return syra_fusion.sapUtil.isEmptyGridDataRecordEx(this._proto.$item.$properties, this.getLineDataColl(lineIdx, true));
	},

	purgeCollTitleMeta: function($meta) {
		var i, len, titMeta, title = null;
		if ($meta && (titMeta = $meta[_datasetK.metaType.title]) && (len = titMeta.length) > 0) {
			// Title meta on the grid itself has to be managed as parent layout title; so we remove it from metadata
			for (i = len - 1; i >= 0; i--) {
				if (titMeta[i][0] == this.entityName && titMeta[i][1] == 0) {
					if (title === null) {
						title = titMeta[i][2];
					}
					titMeta.splice(i, 1);
