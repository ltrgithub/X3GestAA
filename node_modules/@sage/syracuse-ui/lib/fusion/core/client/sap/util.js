...
var speckeys = require('@sage/syracuse-ui/lib/fusion/tools/constant').keybordSpecialKey;
...
exports.Fusion = {
	...
	completeState: function(sttSrc, sttTarget) {
		var sc = this.statusMask;
		var sf = this.statusFlag;
		if (sttSrc & sc.show && !(sttTarget & sc.show)) {
			sttTarget += sc.show;
			if (sttSrc & sf.show) {
				sttTarget += sf.show;
			}
		}
		if (sttSrc & sc.state && !(sttTarget & sc.state)) {
			sttTarget += sc.state;
			if (sttSrc & sf.state) {
				sttTarget += sf.state;
			}
		}
		if (sttSrc & sc.readOnly && !(sttTarget & sc.readOnly)) {
			sttTarget += sc.readOnly;
			if (sttSrc & sf.readOnly) {
				sttTarget += sf.readOnly;
			}
		}
		if (sttSrc & sc.selected && !(sttTarget & sc.selected)) {
			sttTarget += sc.selected;
			if (sttSrc & sf.selected) {
				sttTarget += sf.selected;
			}
		}
		return sttTarget;
	},
	...
	adjustIcoFieldData: function(data) {
		var state, num = data.v === undefined ? undefined : (data.v === null || data.v.length == 0 ? -1 : parseInt(data.v, 10)),
			disable = null;
		...
		// Note : Negative value, empy value or NULL value mean "hidden"
		if (state.$isHidden === undefined && data.v !== undefined) {
			data.stt = this.setState({
				"$isHidden": num < 0 ? true : false
			}, data.stt || 0);
		} else
		if (state.$isHidden !== undefined && !state.$isHidden && num < 0) {
			data.stt = this.setState({
				"$isHidden": true
			}, data.stt || 0);
		}
	...
	getBlockRank: function(xid) {
		return _getRank(xid, 1);
	},
	getleftListRank: function(xid) {
		return _getRank(xid, 1);
	},
	...
	getScreenInst: function(rank, isTab, isLeftList, isButton) {
		var spScrId = exports.Fusion.specialScrnId;
		var pref = isTab ? spScrId.gfolder : (isLeftList ? spScrId.leftZone : (isButton ? spScrId.buttons : ""));
		return pref + (!isButton ? exports.Fusion.getWinId(rank) : (rank + 1));
	},
	...
	cmpIstList: function(a, b, discardLine) {
		var helpers = exports.Fusion;
		return helpers.isListScreen(a.xid) && helpers.isListScreen(b.xid) && helpers.cmpIstGrid(a, b, discardLine);
	},
	...
	isEndSessReply: function(reply) {
		var ret = false;
		if (reply) {
			if ((ret = reply.sap) && (ret = ret.target) && (ret = ret.type)) {
				ret = ret == "portal" ? ret : false;
			} else
			if (reply.session && (reply.session.close || reply.session.reboot)) {
				ret = reply.session.close ? "close" : "reboot";
			}
		}
		return ret;
	},
	...
	isEmptyGridDataRecord: function(list, data) {
		var dataSetRecord, empty = false,
			value;
		if (data.uuid) {
			data.dataRecord = (list.findDataRecord(data.uuid).dataRecord || null);
		}
		if (!data.dataRecord || data.dataRecord.$serverIndex < (list.dataset.length - 1)) {
			return false;
		} else {
			return exports.Fusion.isEmptyGridDataRecordEx(list.$fields, data.dataRecord);
		}
	},
	isEmptyGridDataRecordEx: function(listFldProps, dataRecord) {
		...
		for (var key in dataRecord) {
			if (listFldProps[key]) {
				if ((value = dataRecord[key]) === null || value === undefined) {
					empty = true;
				} else {
					empty = exports.Fusion.isEmptyData(listFldProps[key].$type, value);
					if (!empty) {
						return false;
					}
				}
	...
	isEmptyData: function($type, value) {
		switch ($type) {
	...
	setFieldMod: function(field, ist) {
		...
		if (field && field.$isEditMode) {
			field.input && syra_dom.toggleClass(field.input, "s-x3mode-command-input", !ist.edit);
			syra_dom.toggleClass(field._dataValue, "s-x3mode-command", !ist.edit);
		}
	},
	optDialog: function(pageName) {
		var opt;
		if (pageName === "AAFORMULA") {
			opt = {};
			opt.validate = true;
		}
		return opt;
	},
	appendEntitiesValue: function(objEntities, winId, entityName, line, value) {
		var ptr, grid, gridRcd;
		if (winId) {
			if (!objEntities[winId]) {
				objEntities[winId] = {};
			}
			ptr = objEntities[winId];
		} else {
			ptr = objEntities;
		}
		if (line !== null && line !== undefined && line > 0) {
			// Warning : record must exist!
			grid = exports.Fusion.getBlockInst(entityName);
			if (!ptr[grid]) {
				ptr[grid] = {
					"$rcd": []
				};
			}
			gridRcd = ptr[grid].$rcd;
			gridRcd.push({
				"$uuid": line - 1
			});
			gridRcd[gridRcd.length - 1][entityName] = value;
		} else {
			ptr[entityName] = {
				"v": value
			};
...
exports.Syra = {
	...
	dataTypes: {
		...
		boWebiDoc: "application/wid",
		htmlVignette: "application/x-vignette",
		sqlDesigner: "application/x-sql-designer"
	},
	wdgtTypes: {
		radio: "$radios",
		combo: "$combo"
	...
	getUserName: function() {
		var user = syra_site;
		return (user && (user = syra_site.userProfile) && (user = user.dataset) && (user = user.user) && (user = user.login)) ? user : null;
	},
	...
	getFieldProtoFromCollObj: function(collObj, fldBind) {
		var proto = exports.Syra.getProtoFromPage(collObj);
		return (proto && (proto = proto.$item) && (proto = proto.$properties) ? proto[fldBind] : null);
	},
	...
	getPage: function(syraObject) {
		var obj = syraObject,
			page = syraObject;
		while ((!page || !page.$fusionPageMeta) && obj && obj.boxParent) {
			page = obj.boxParent.articleParent;
			obj = obj.boxParent;
		}
	...
	isEntityPopupCard: function(entity, syraProto) { //field only in popupcard
		var $entity = exports.Syra.getEntityProto(entity, syraProto);
		return $entity ? $entity.$viewScope == "popup" : false;
	},
	...
	getFieldNL: function(field) {
		var srvIdx = null;
		if (field.arrayLevel == "record") {
			srvIdx = field.$serverIndex;
		} else
		if (field.$uuid) {
			srvIdx = field.recordsMap[field.$uuid].$serverIndex;
		} else
		if (field.articleParent.$serverIndex != null) {
			srvIdx = field.articleParent.$serverIndex;
		}
		return srvIdx != null ? (srvIdx + 1) : 0;
	},
	...
	mapMnus2Links: function(mapIn, title, action, id, type, rank, father) {
		...
		return _.map(mapIn, function(mapFrom, idx) {
			...
			if (mapFrom[action] && mapFrom[action] > 0) {
				...
				if (mapFrom.icon) {
					map2[mapFrom[id]].$icon = util.duplicateObj(mapFrom.icon);
				}
				if (mapFrom.checkOfficeInstalled) {
					map2[mapFrom[id]].$confirm = syra_local.installOfficeAddin;
					map2[mapFrom[id]].$officeAddinSetup = syra_officeAddin.exe;
				}
				if (mapFrom.disable != null) {
					map2[mapFrom[id]].$isDisabled = mapFrom.disable;
				}
				return map2;
			}
		});
	...
	setTabFocus: function(tab) {
		if (tab.titleBtn && tab.titleBtn.link) {
			tab.titleBtn.link.focus();
		}
	},
	showTab: function(tab, data) {
		if (tab && tab.isTabSection) {
			tab.syraOnEventData = data;
	...
	razStyleOnInput: function(input, styleColl, excludeStyle) {
		_.each(styleColl, function(stl, name) {
			if (name != excludeStyle && stl.css) {
				syra_dom.toggleClass(input, stl.css, false);
			}
		});
	},
	appendProto: function(proto, append, appendObjCallback, processProperty, processPropertyCallback) {
		function appendInner(from, to, toKeyName) {
			_.each(from, function(node, name) {
				if (Array.isArray(node) || _.isObject(node)) {
					if (this.hasOwnProperty(name)) {
						appendInner(node, this[name], name);
					} else {
						this[name] = util.duplicateObj(node, appendObjCallback);
					}
				} else
				if (processProperty) {
					if (processPropertyCallback) {
						processPropertyCallback(this, toKeyName, name, node);
					} else {
						this[name] = node;
					}
				}
	...
	getTopSectionsMap: function($article) {
		var secMap = [];
	...
	isEntityInGridCard: function(entityName, gridField) {
		...
		if (gridField.$popupCard && (layout = gridField.$layout)) {
...
exports.Syra._mapNullValue[exports.Syra.dataTypes.choiceType] = -1;


exports.Syra.getXElement = function(field) {
	field = field.contextField || field;
	while (field && !(field.isArrayField || field.arrayLevel === "cell")) {
		field = field.articleParent;
	}
	return field;
};

exports.Syra.getXElementGrid = function(field) {
	field = field.contextField || field;
	while (field && !field.isArrayField) {
		field = field.articleParent;
	}
	return field;
};