...
function _accelDecode(accel, item) {
	var sc = null,
		modifiers;
	var i, len, accelEx, sci, scEx = null;

	// Decoding
	if (accel) {
		sc = [];
		accelEx = Array.isArray(accel) ? accel : [accel];
		len = accelEx.length
		for (i = 0; i < len; i++) {
			// Modifier...
			modifiers = [];
			if (accelEx[i] > _accelMap.altgr.code) {
				// Not yet supported
				accelEx[i] -= _accelMap.altgr.code;
			}
			if (accelEx[i] > _accelMap.alt.code) {
				accelEx[i] -= _accelMap.alt.code;
				modifiers.push(_accelMap.alt.key);
			}
			if (accelEx[i] > _accelMap.ctrl.code) {
				accelEx[i] -= _accelMap.ctrl.code;
				modifiers.push(_accelMap.ctrl.key);
			}
			if (accelEx[i] > _accelMap.shift.code) {
				accelEx[i] -= _accelMap.shift.code;
				modifiers.push(_accelMap.shift.key);
			}
			sci = syra_shortcut.quick.buildShortCutDef(modifiers, accelEx[i]);
			if (sci) {
				sci.def.item = item;
				sc.push(sci);
			}
	...
	if (sc && (len = sc.length) > 1) {
		...
		for (i = 0; i < len; i++) {
			scEx.def[i] = sc[i].def;
			scEx.label += ((scEx.label ? " || " : "") + sc[i].label);
		}
	} else if (sc) {
		scEx = sc[0];
	}
...
exports.MenuManagement = helpers.defineClass(MenuManagement, null, {
	...
	_addInternalCoreMenu: function() {
		var mnuId, self = this,
			helpHandler = syra_fusion.sapUtil.helpHandler;
		var logger = this._hoster._sapController.getlogger(),
			traceEnable;
		...
		_addMenu(this.coreMenus, this._tabMenusFather, this._getMenuItem(_menuIDs.SUBMENU_DIAGNOS_CMN, this._idMnuCpt++, syra_fusion.getMessage((traceEnable = logger && logger.isTraceEnable()) ? "menuTraceLocalOff" : "menuTraceLocalOn"), null, 6, 13, requestActions.getLocalAction("LCL_TOGGLE_LOG"), null, false, {
			"acl": 2116,
			"ico": (traceEnable ? "145" : "144"),
			"icoVp": "x3icomnu"
		}));
		// Items of "FILE_EX"
	...
	addMnuBarToProto: function(winId, winFacet, winMenusDef, winProto, options) {
		...
		if ((len = bar.$menus.$items.length) > 0) {
			...
			for (i = j; i < nbLayoutItems; i++) {
				...
				if ((title = $mnus[i].$title)) {
					...
					len = title.length - 1;
					$mnus[i].$id = (title[0] == "{" && title[len] == "}" ? title.substring(1, len) : title) + winMenuId;
		...
		if (closeItemMngt.rankInColl >= 0) {
			if (!winFacet || !syra_fusion.syraUtil.isDialogFacet(winFacet)) {
				mainSlotItem.button = {
		...
		if ($mnus.length > 0 && winFacet != syra_fusion.syraUtil.pageFacet.modal3) {
			i = 0;
	...
	addIcoItemToMnu: function(article, groupId, $bind, icoId, icoPath, isHidden) {
		var $link = {
			$act: $bind
		};
		...
		$menus.$links[$bind] = $link;
		var group = groupId && article.idMap[groupId];
		if (group) {
			group.addMenuItem({
				$bind: $bind,
				$icon: $link.$icon
			});
			syra_menus.applyChange(article, $menus);
		}
		return true;
	},
	...
	getMagnetoLinks: function(facet, $links) {
		var coreMenus = this.coreMenus;
		var magetoItem, i, len, magnetoMnuId = !facet || facet === syra_fusion.syraUtil.pageFacet.edit ? _menuIDs.MAGNETO_GIRL : _menuIDs.MAGNETO_OWN;
		var magnetoArray = coreMenus[magnetoMnuId] && coreMenus[magnetoMnuId].items ? Object.keys(coreMenus[magnetoMnuId].items) : [];
...
WinMenu = helpers.defineClass(WinMenu, null, {
	initialize: function(menuManager, winId, winMenus, winProto, winFacet, setMainGrp, showMagneto, session) {
		...
		this._idMnuCpt = 99999000 + syra_fusion.sapUtil.getWinRank(winId) * 100;
		...
		if (!this._winFacet || syra_fusion.syraUtil.isFacetWithGotoFct(this._winFacet)) {
	...
	buildMenus: function() {
		...
		if (!this._winFacet || this._winFacet === syra_fusion.syraUtil.pageFacet.edit) {
			map = [{
		...
		if (this._winFacet === syra_fusion.syraUtil.pageFacet.modal2) {
			map = [{
				"id": _menuIDs.STATUS_CMN,
				"rnk": _insertRank.status,
				"grp": _kMnuHelper.group.grpTools.cld
	...
	buildTreeMenusObj: function() {
		...
		function walk(srcItems, trgtItems, ctx) {
			var len, srcItemsArray, i, act, style, mnuItem, layout, opts, linkId, shortCut, item;
			...
			for (i = 0, len = srcItemsArray.length; i < len; i++) {
				item = srcItems[srcItemsArray[i]];
				if (!item.inactive && !_kMnuHelper.isItemSep(item.typ)) {
					style = item.opts ? item.opts.sty : undefined;
					if ((_kMnuTypes[item.typ] & _kMnuHelper.cat.popup) && ((item.items && (Object.keys(item.items)).length > 0) || _kMnuHelper.isPopupNoFacet(item.typ))) {
						mnuItem = {
							"$title": item.txt,
							"$style": style,
							"$layout": {
						...
						mnuItem.$id = srcItemsArray[i] + winId;
						if ((opts = item.opts)) {
						...
						_kMnuHelper.appendIco(item, mnuItem);
						trgtItems.push(mnuItem);
						if (item.items) {
							ctx.level += 1;
							walk(item.items, trgtItems[trgtItems.length - 1].$layout.$items, ctx);
							ctx.level -= 1;
						}
					} else
					if ((act = item.act)) {
						linkId = item.linkId || act;
						$links[linkId] = {
							"$act": act,
							"$title": item.txt
						};
						shortCut = _accelDecode(item.acl, "page")
						mnuItem = {
							"$shortCutTip": shortCut ? shortCut.label : null,
							"$shortCut": shortCut ? shortCut.def : null,
							"$bind": linkId,
							"$style": style,
							"rnk": item.rnk
						};
		...
		if (this._showMagneto && (this._winFacet === syra_fusion.syraUtil.pageFacet.edit || (this._winFacet === syra_fusion.syraUtil.pageFacet.modal2 && _kWinOwnMagneto))) {
			this._menuManager.getMagnetoLinks(this._winFacet, $links);
		}
		return {
			"$menus": $menus,
			"$links": $links
