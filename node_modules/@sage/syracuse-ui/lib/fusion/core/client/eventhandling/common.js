...
function _showSessInfo(sapController, currField, filedInf, curWin) {
	...
	var boxParams = util.diagnosesToOurErrEx(diagnoses);
	msgBox.setMsgBoxDefaultProp(boxParams, 0, 0, null, null, true);
	var boxOpts = msgBox.getMsgBoxOpts.call(sapController, boxParams);
	boxOpts.$delay = 50;
...
exports.CommonEventHandlingTools = {
	getGridRecordLinks: function(grid, capabilities, bind, fieldInfo, sapController, links) {
		return sapController.getGridRecordLinks(grid, capabilities, bind, fieldInfo.winModel, fieldInfo.nl, links);
	},
	...
	openDatePicker: function(succeeded, result, evtHandlingObj, initialEvent, field, callerFieldInfo, force) {
		...
		if (field && !field.disposed && (evtHandlingObj.sapController.hasFocus(callerFieldInfo.fieldIst) || force) && !syra_fusion.syraUtil.getPopupDateFromField(field)) {
			if (!succeeded) {
				...
				value = result ? parseInt(result.raw, 10) : (result === "" ? result : null);
			}
	...
	closeDatePicker: function(evtHandlingObj, event, fieldInfo, fieldFormat) {
		...
		var fmtOuput = fmtW.rawToEdit(event.data.internalValue, fieldFormat);
		event.data.doStatements({
			"value": fmtOuput.edit
		});
		delete event.data.doStatements;
	},
	checkDate: function(evtHandlingObj, event, fieldInfo, value, format, force) {
		var fmtW = evtHandlingObj.sapController.fmtWrapper;
		var fmtOuput = fmtW.unformat(value, fieldInfo.type, format, true);
		exports.CommonEventHandlingTools.openDatePicker(fmtOuput.succeeded, fmtOuput, evtHandlingObj, event, event.target, fieldInfo, force);
	},
		...
		if (field && !field.disposed) {
			...
			if (field.arrayLevel && field.arrayLevel === "array") {
				var me = (readOnly ? {} : {
					...
					"act": "2",
					"id": "$tabularExport",
					"checkOfficeInstalled": true,
					"disable": evtHandlingObj.sapController.isActionEnabled(fieldInfo.winModel.getWinId(), requestActions.getSrvAction("SRV_EXPXLSGRID")) === false,
					"icon": {
						"$mode": "iconText",
						"$path": "file/",
						"$value": "s-link-excel"
				...
				var versions = evtHandlingObj.controller.getSession().version;
				var supVers = versions && versions.supervisor && versions.supervisor.vers;
				if (util.versionCheck.isSupCsvEnabled(supVers)) {
					var meCsv = (readOnly ? {} : {
						"txt": syra_local.export_csv,
						"act": "2",
						"id": "$tabularCsvExport",
						"checkOfficeInstalled": true,
						"disable": evtHandlingObj.sapController.isActionEnabled(fieldInfo.winModel.getWinId(), requestActions.getSrvAction("SRV_EXPCSVGRID")) === false,
						"icon": {
							"$mode": "iconText",
							"$path": "file/",
							"$value": "s-link-csv"
					...
					if (exports.CommonEventHandlingTools._checkCtxMenuEntry(meCsv)) {
						ctxEx.push(meCsv);
					}
				}
	...
	_checkCtxMenuEntry: function(me) {
		if (me && me.id && (me.id === "$tabularExport" || me.id === "$tabularCsvExport") && syra_site.isTabletDevice) {
			return false;
		}
	...
	getFieldInfo: function(field, nl) {
		var proto, bind, isCell, fieldInfo = {
		...
		if (field && (fieldInfo.$fusionPageMeta = syraUtil.getFusionPageMeta(field)) && (fieldInfo.$fieldMeta = syraUtil.getMetaFromObject(field)) && (((proto = fieldInfo.$fusionPageMeta.winModel.getProto()))[(bind = fieldInfo.$fieldMeta.$bind)] || (isCell = (field.arrayLevel === "cell")))) {
			fieldInfo.winModel = fieldInfo.$fusionPageMeta.winModel;
			fieldInfo.fieldIst = syra_fusion.sapUtil.makeIst(fieldInfo.winModel.getWinId(), bind, nl || 0);
			fieldInfo.readOnlyByModel = syraUtil.isFieldModelReadOnly(isCell ? proto[syra_fusion.sapUtil.getBlockInst(bind)].$item.$properties : proto, null, bind);
			...
			if ((fieldInfo.$field = field.$field)) {
				...
				if (!fieldInfo.isCheck && fieldInfo.$field.$type == syraUtil.dataTypes.choiceType) {
					if (fieldInfo.$fieldMeta && fieldInfo.$fieldMeta.$format) {
						fieldInfo.isRadio = fieldInfo.$fieldMeta.$format == syraUtil.wdgtTypes.radio ? true : false;
					} else {
	...
	isFieldInputMode: function(fieldInfo) {
		return !fieldInfo.isReadOnly && !fieldInfo.isDisabled && !fieldInfo.readOnlyByModel;
	},
	...
	restoreFieldAfterClick: function(isCrlFailed, reply, evtHandlingObj, $field, fieldIst, newValue, notifySrv, act, oldValue, fromGrid) {
		...
		if (redirWin || (redirBox && !msgBox.isErrBox(redirBox.type))) {
			// SAM #112376 & #113555... not really clean, but difficult to manage properly this kind of situation client side...
