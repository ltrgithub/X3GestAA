...
_evtProcAskui.sap = function() {
	this.eventListeners = function() {
		return [
			...
			["sap.extdlg.abort", onAbort, false, false],
			...
			["wdgt.file.uploadDone", onFileUploadDone, true, true],
	...
	function onFileUpload(event, data) {
		...
		if (_isOurUploadFld(this, event.target) && event.data && (files = event.data.files) && files.length > 0) {
			syra_alert.clear(this._ctx.currDlg);
			...
			if (this._ctx.statementParams.UIMaxLength && files[0].size > maxSize) {
				syra_alert.error(this.sapController.getMessLabel("72", "error", "File size is greater than maximun size allowed") + " (" + maxSize + " octets).", this._ctx.currDlg);
				sizeOk = false;
				processed = true;
			}
			if (sizeOk) {
			...
			syra_ajax.del({
				...
				headers: {
					...
					if (response.ok) {
						...
						docField.articleParent.applyChange(response.data, response);
						_updateLinksStatus(self._ctx.currDlg, [{
							"$bind": "$ok",
							"$isDisabled": true
						}]);
					} else {
						if (response.error.diagnoses) {
							docField.articleParent.applyChange({
