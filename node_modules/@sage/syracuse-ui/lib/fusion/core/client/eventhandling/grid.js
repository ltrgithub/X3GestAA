...
var keys = require('@sage/syracuse-ui/lib/fusion/tools/constant').keybordKey;
var _evtProcGrid = {};
...
_evtProcGrid.sap = function() {
	this.eventListeners = function() {
		return [
			["wdgt.cell.click", clickCellTab, true, true],
			["wdgt.cell.mousedown", clickCellTab, true, true],
			["wdgt.cell.change", changeCellTab, true, true],
			["wdgt.grid.escape", escapeOnLine, true, true],
			...
			["wdgt.array.picker", clickArrayPicker, true, true],
			["wdgt.record.picker", clickRecordPicker, true, true],
			["wdgt.cell.calendarclosed", onDatePickerClose, true, true],
			["wdgt.cell.escape", applyEscape, true, true],
			["wdgt.cell.applyShortCut", applyShortCut, true, true],
			["wdgt.cell.keydown", keyDownCellTab, true, true],
			["wdgt.cell.keyup", keyUpCellTab, true, true],
			["wdgt.grid.sortLocal", sortLocal, true, true],
			["wdgt.grid.searchLocal", searchLocal, true, true]
		];
	};
	...
	function sortLocal(event) {
		...
		if (fieldInfo.firstFocus || fieldInfo.currIst.edit) {
			ist = syra_fusion.sapUtil.makeIst(fieldInfo.fieldIst.win, fieldInfo.fieldIst.xid, 1);
			event.result = {
				...
				"post": {
					...
					"args": [syra_fusion.sapUtil.isEmptyGridDataRecord]
				}
			};
			return true;
		}
	...
	function searchLocal(event) {
		...
		if (event.data.found) {
			var sap = this.sapController;
			ist = syra_fusion.sapUtil.makeIst(fieldInfo.fieldIst.win, event.data.found.$bind, event.data.found.$serverIndex + 1);
			event.result = {
				"$act": requestActions.getSrvAction("SRV_COMMANDGRID"),
				"params": {
					"target": ist
	...
	function _getNlFromMagneto(list, current, isSorted, next, last) {
		...
		if (!isSorted) {
			...
			if (next !== null) {
				currRcdIdx = syra_recordCardModal.getRecord(list).getRecordIndex();
		...
		if (syra_fusion.sapUtil.isEmptyGridDataRecord(list, {
	...
	function actionLine(event) {
		var processed = undefined,
			...
			acts = {},
			srvActEx, srvAct, srvActNum, indexes, grid = fieldInfo.array;
		var crudActs = {
			"$delete": {
		...
		if (event.target) {
			switch (event.target.$sourceBind || event.target.$bind) {
				...
				case "$cut":
					...
					break;
				case "$tabularExport":
					processed = _exportOffice(event, "SUP_EXPXLSGRID", "SRV_EXPXLSGRID");
					break;
				case "$tabularCsvExport":
					processed = _exportOffice(event, "SUP_EXPCSVGRID", "SRV_EXPCSVGRID");
					break;
			}
		}
		return processed;
	...
	function _exportOffice(event, supAction, srvAction) {
		var fieldInfo = _getFieldInfo(event);
		if (fieldInfo.firstFocus || fieldInfo.isEdit) { // focus not already on this table or line change or same ligne but edit field
			...
			event.result = {
				"$act": requestActions.getSrvActionEX(supAction),
				"params": {
					"target": fieldInfo.fieldIst
				}
			};
			...
			event.result = {
				"$act": requestActions.getSrvAction(srvAction),
				"params": {
	...
	function escapeOnLine(event) {
		return selectLine(event, true);
	};

	function selectLine(event, escapeOnLine) {
		var processed = undefined,
			fieldInfo = _getFieldInfo(event),
			acts = {};
		if (event.data.uuid != undefined) {
			if (syra_fusion.sapUtil.isEmptyGridDataRecord(event.target, {
		...
		if (event.target) {
			var indexList = fieldInfo.array.selector.getSelectedList();
			if ((fieldInfo.changeLine || fieldInfo.firstFocus || fieldInfo.isEdit) && (!event.data.nativeEvt.shiftKey || indexList.length == 0)) { // focus not already on this table or check and change line
				acts.srvAct = requestActions.getSrvAction((escapeOnLine ? "SRV_ABANDONLINE" : "SRV_COMMANDGRID")); // ask for focus
				acts.send = true;
			...
			if (pickerType = event.data.pickerType) {
				switch (pickerType) {
					...
					case "date":
						...
						break;
					case "cancelEdit":
						var ist = syra_fusion.sapUtil.makeIst(fieldInfo.fieldIst.win, fieldInfo.fieldIst.xid, fieldInfo.nl);
						if (fieldInfo.isEdit) {
							event.result = {
				...
				if (fieldInfo.array.$orderBy && event.result) {
					if (event.result.$act == requestActions.getSrvActionEX("SUP_CELL_EDIT") || event.result.$act == requestActions.getSrvAction("SRV_GETFOCUSCELL")) {
	...
	function clickArrayPicker(event) {
		...
		var readOnlyByModel = false;
		if (!menusPopup) {
			readOnlyByModel = fieldInfo.array.$isReadOnly && (!fieldInfo.array.$menus || !fieldInfo.array.$menus.$lazyload);
			if ((fieldInfo.firstFocus || fieldInfo.isEdit) && !readOnlyByModel) {
				ist = syra_fusion.sapUtil.makeIst(fieldInfo.fieldIst.win, fieldInfo.fieldIst.xid, fieldInfo.firstFocus ? 1 : fieldInfo.currIst.nl);
				event.result = {
					"$act": requestActions.getSrvAction("SRV_COMMANDGRID"),
					"params": {
						"target": ist
				...
				var self = this;
				setTimeout(function() {
					evtTools.openFieldCtx({}, self, event, event.target, fieldInfo, null, readOnlyByModel);
				}, 10);
				return true;
			}
		}
		...
		if (pickerType = event.data.pickerType) {
			switch (pickerType) {
				case "menus":
					...
					break;
				case "closeCard":
					if (syra_fusion.sapUtil.cmpIstBlock(fieldInfo.currIst, fieldInfo.fieldIst)) {
	...
	function datePickerClick(event, process, fieldInfo) {
		if (process && !syra_fusion.syraUtil.getPopupDateFromField(event.target)) {
			event.result.post = {
				"callback": function(evtHandlingObj, initialEvent, callerField) {
					...
					evtTools.openDatePicker(true, fieldContext.raw ? {
						"raw": fieldContext.raw
					} : null, evtHandlingObj, initialEvent, callerField, fieldInfo);
				},
				"args": [this, event, event.target, fieldInfo]
			};
		} else
		if (!syra_fusion.syraUtil.getPopupDateFromField(event.target)) {
			...
			var currValue = syra_fusion.syraUtil.getFusionPageMeta(event.target).winModel.getDataInputValue(syra_fusion.syraUtil.getMetaFromObject(event.target).$bind, event.target.articleParent.$serverIndex + 1);
			if (currValue && currValue.v) {
				evtTools.checkDate(this, event, fieldInfo, currValue.v, fieldContext.fmt);
				process = true;
			} else {
				var self = this;
				setTimeout(function() {
	...
	function applyTab(opt, fldInfo) {
		var fieldInfo = fldInfo || _getFieldInfo(opt),
		...
		if (opt.data.shortcuts.esc) {
			...
			if (!fieldInfo.isEdit /*&& !fieldInfo.target.$isReadOnly*/ ) {
				if (!opt.data.shortcuts.shift) {
					isGridNotEditable = fieldInfo.winModel.getEntityMetaByCriteria(fieldInfo.blockIst.xid, "_fullyDisabled", null, -1);
					acts = isGridNotEditable ? requestActions.getSrvAction("SRV_CHAMP_SUIVANT") : requestActions.getSrvActionEX("SUP_CELL_EDIT_TAB");
				} else {
	...
	function applyEnter(opt, fldInfo) {
		var fieldInfo = fldInfo || _getFieldInfo(opt),
			fieldIst = fieldInfo.fieldIst,
			acts, target, processed = undefined;
		...
		if (fieldInfo.isEdit) {
			...
			if (syra_recordCardModal.isOpened(list)) {
				//TODO : Best way to close popup dialog ??
				syra_recordCardModal.togglePopup();
				return false;
			} else {
	...
	function applyShortCut(opt) {
		...
		var record = fieldInfo.target.articleParent,
			...
			serverIndex = -1;
		var tempIst, array = fieldInfo.array;
		if (opt.data.shortcuts.down || opt.data.shortcuts.up) {
			if (syra_recordCardModal.isOpened(array))
				return false;
			...
			fieldIst = syra_fusion.sapUtil.makeIst(fieldInfo.fieldIst.win, fieldInfo.xid, parseInt(serverIndex, 10) + 1);
			if (!fieldInfo.isEdit) {
				acts = requestActions.getSrvAction("SRV_COMMANDGRID");
				target = true;
				processed = true;
			} else {
				if (fieldInfo.type != syra_fusion.syraUtil.dataTypes.choiceType && (fieldInfo.type != syra_fusion.syraUtil.dataTypes.dateType || !fieldInfo.popupPicker)) {
		...
		if (opt.data.shortcuts.left || opt.data.shortcuts.right) {
			if (fieldInfo.isEdit) {
				...
				while (syra_fusion.syraUtil.isFieldModelReadOnly(array.$prototype.$item.$properties, null, fieldIst.xid)) {
					tempIst = fieldIst.xid;
					fieldIst.xid = record.list.builder[opt.data.shortcuts.right ? "getNextVisibleField" : "getPreviousVisibleField"](fieldIst.xid);
					if (fieldIst.xid == tempIst) {
						break;
					}
				}
				acts = requestActions.getSrvAction("SRV_COMMANDGRID");
		...
		if (opt.data.shortcuts.enter) {
			return applyEnter(opt, fieldInfo);
		} else
		if (opt.data.shortcuts.tab) {
			return applyTab(opt, fieldInfo);
		} else
		if (opt.data.shortcuts.i) {
			return evtTools.applyKeyI(opt);
		} else if (opt.data.shortcuts.f2) {
			if (!fieldInfo.isCardView && !fieldInfo.isEdit && evtTools.isFieldInputMode(fieldInfo)) {
				acts = requestActions.getSrvAction("SRV_GETFOCUSCELL");
				target = true;
				processed = true;
			}
		...
		if (!fieldInfo.isEdit && (fieldInfo.type != syra_fusion.syraUtil.dataTypes.booleanType || (fieldInfo.type == syra_fusion.syraUtil.dataTypes.booleanType && (fieldInfo.isDisabled || fieldInfo.isReadOnly)))) {
			opt.event.preventDefault();
			opt.event.stopPropagation();
		}
	...
	function getLocalActionLine(fieldInfo, action) {
		...
		if (fieldInfo.winModel.getEntityMetaByCriteria(collName, "_fullyDisabledAtRuntime", collName, -1))
	...
	function clickCell(event, fieldInfo) {
		...
		if (event.target) {
			switch (fieldInfo.type) {
				case syra_fusion.syraUtil.dataTypes.booleanType:
					acts = clickCellBoolean.call(this, fieldInfo, fieldInfo.fieldIst);
					break;
				case syra_fusion.syraUtil.dataTypes.iconType:
					acts = clickCellIcon.call(this, fieldInfo);
					break;
				case syra_fusion.syraUtil.dataTypes.dateType:
	...
	function clickCellBoolean(fieldInfo, newIst) {
		...
		if (!fieldInfo.hasFocus || !fieldInfo.isEdit) { //Only if field does not have already focus or command mode
			var newVal = (fieldInfo.target.currentValue == syra_fusion.sapUtil.check.off ? syra_fusion.sapUtil.check.on : syra_fusion.sapUtil.check.off);
	...
	function onDatePickerClose(event) {
		...
		if (event.target && (fieldInfo = _getFieldInfo(event)) && fieldInfo.hasFocus) {
			evtTools.closeDatePicker(this, event, fieldInfo, fieldInfo.currIst.fmt);
			processed = true;
		}
		return processed;
	}
	...
	function _getFieldInfo(event) {
		var syraUtil = syra_fusion.syraUtil,
			sapUtil = syra_fusion.sapUtil;
		var nl = _getNoLine(event),
			fieldEx = syraUtil.getXElement(event.target),
			fieldInfo = evtTools.getFieldInfo(fieldEx, nl);
		fieldInfo.data = event.data;
		fieldInfo.nl = nl
		fieldInfo.xid = syraUtil.getMetaFromObject(fieldEx).$bind;
		...
		fieldInfo.isSortedArray = (fieldInfo.array && fieldInfo.array.$orderBy) ? true : false
	...
	function _getNoLine(event) {
		...
		return syra_fusion.syraUtil.getFieldNL(event.target);
	}
};