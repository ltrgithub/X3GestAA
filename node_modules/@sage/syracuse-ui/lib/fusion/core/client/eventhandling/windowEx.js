...
function _getEnterExcludeType() {
	if (!_vkEnterExcludeType) {
		var type = _vkEnterExcludeType = {};
		...
		type[syra_fusion.syraUtil.dataTypes.flash] = true;
	}
	return _vkEnterExcludeType;
}
...
_evtProcWindow.sap = function() {
	...
	this.eventListeners = function() {
		return [
			...
			["wdgt.win.closeall", onCloseAll, true, true],
			["wdgt.field.applyShortCut", applyShortCut, true, true],
			["wdgt.field.escape", applyEscape, true, true],
			["wdgt.field.input", onInput, true, true],
			["wdgt.field.keydown", onFieldKeyDown, true, true],
			["wdgt.field.click", onFieldClick, true, true],
			["wdgt.field.mousedown", onFieldClick, true, true],
			...
			["wdgt.field.keyup", onFieldKeyUp, true, true],
			["wdgt.cell.keyup", onFieldKeyUp, false, false],
			["wdgt.file.upload", onFileUpload, true, true],
			["wdgt.file.uploadSucceeded", onFileUploadSucceeded, true, true],
			["wdgt.file.uploadRemove", onFileUploadRemove, true, true],
			["wdgt.file.uploadRemoveSucceeded", onFileUploadRemoveSucceeded, true, true]
		];
	};
	...
	function onInput(event) {
		if (this.sapController.getCurrInst()) {
			var field = event.target;
			if (field.setDirty && field.getTextRows && field.getTextRows() && field.$field.$type == syra_fusion.syraUtil.dataTypes.stringType) {
				field.setDirty(true);
			}
		}
	}
	...
	function applyShortCut(opt) {
		...
		if (opt.data.shortcuts.enter) {
			...
			if (opt.target.isTextArea) {
				...
				var field = opt.target;
				if (field) {
					fieldInfo = _getFieldInfo(field);
					if (!(syra_alert.getModal && syra_alert.getModal())) {
						var field = opt.target;
						var $rows = field && field.getTextRows && field.getTextRows();
						if (field && (_getEnterExcludeType()[field.$field.$type] || ($rows && field.$field.$type == "application/x-string"))) {
							processed = undefined;
						} else {
							opt.result = {
								"$act": requestActions.getSrvAction(fieldInfo.isIco && !fieldInfo.readOnlyByModel && !fieldInfo.isReadOnly && !fieldInfo.isDisabled ? "SRV_KIB" : "SRV_VALIDE_ECRAN")
							};
							processed = true;
						}
					} else {
		...
		if (opt.data.shortcuts.up || opt.data.shortcuts.down) {
			return !!opt.target.isTextArea;
		} else
	...
	function applyEscape(opt) {
		var applied = false;
		var $fieldMeta = syra_fusion.syraUtil.getMetaFromObject(opt.target),
			$fusionPageMeta = syra_fusion.syraUtil.getFusionPageMeta(opt.target),
			fieldCxt = this.sapController.getCurrInst(),
			currValue = $fusionPageMeta.winModel.getDataInputValue(fieldCxt.xid, 0),
			oldValue, result;
		if (currValue && currValue.v != undefined && syra_fusion.sapUtil.needToControlField(syra_fusion.syraUtil.getFieldType(opt.target))) {
	...
	function onFieldKeyUp(event) {
		...
		var $itemMeta = syra_fusion.syraUtil.getMetaFromObject(event.target),
			keyCode = event.data.nativeEvt.keyCode,
			$$cont, oldValue;
		//syra_fusion.syraUtil.log("windowEx onFieldKeyUp", keyCode, event);
		if ($itemMeta && syra_fusion.sapUtil.isListScreen($itemMeta.$bind) && event.target.boxParent.boxParent) {
				...
				default:
					if (formatW) {
						...
						if ((fieldInfo = fieldCxt ? _getFieldInfo(event.target, fieldCxt.nl) : null)) {
							if (fieldInfo && !fieldInfo.isReadOnly) {
								currValue = formatW.isTypeAcceptFmt(fieldInfo.type) && fieldCxt.fmt ? fieldInfo.winModel.getDataInputValue(fieldInfo.fieldIst.xid, fieldInfo.fieldIst.nl, event) : null;
								control = currValue && currValue.v ? formatW.editToDisplay(fieldCxt.fmt, currValue.v) : {
									"succeeded": true
								};
								var msg = null;
								if (!control.succeeded) {
									var $diagnose = control.$diagnoses && control.$diagnoses[0];
									msg = [($diagnose && $diagnose.$message) || ""];
								}
								fieldInfo.winModel.setFieldError(event.target, msg, control.succeeded);
							}
						}
					}
			}
		...
		event.result = {
			...
			post: {
				"callback": function(reply) {
					event.callback(!!(syra_fusion.sapUtil.isEndSessReply(reply)));
				},
	...
	function onClick(event) {
		...
		if (event.target && ($$field = $(event.data.nativeEvt.target))) {
			$$cont = $$field.closest("." + syra_fusion.syraUtil.pageSectionK.body);
			if (($$cont && $$cont.length > 0) ||
				_.find(syra_fusion.syraUtil.pageSectionK, function(cssK) {
	...
	function onFileUpload(event) {
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target)) && !fieldInfo.readOnlyByModel && !fieldInfo.isReadOnly && !fieldInfo.isDisabled) {
			if (fieldInfo.type === syra_fusion.syraUtil.dataTypes.imageType) {
	...
	function onFileUploadRemoveSucceeded(event) {
		var processed = undefined,
			fieldInfo;
		if (event.target && (fieldInfo = _getFieldInfo(event.target)) && this.sapController.hasFocus(fieldInfo.fieldIst)) {
			event.result = evtTools.getliveNotifyValueParams(fieldInfo.fieldIst, null, requestActions.getSrvAction("SRV_GETFOCUS"));
	...
	function _uploadCtrlImage(reply, evtHandlingObj, event) {
		...
		if (event.data.files[0]) {
			...
			syra_fusion.syraUtil.getFusionPageMeta(event.target).winModel.setFieldError(event.target, messErr, !error);
			if (reply) {
				if (error) {
					return undefined;
				} else {
	...
	function onFieldClick(event, force) {
		// syra_fusion.syraUtil.log("onFieldClick",event,force)
		var processed = undefined,
			fieldInfo;
		if (event.target && (fieldInfo = _getFieldInfo(event.target)) && (force || !evtTools.discardFieldEvent(event.type, fieldInfo))) {
			...
			if (!fieldInfo.readOnlyByModel && !this.sapController.hasFocus(fieldInfo.fieldIst)) {
				...
				if (fieldInfo.type == syra_fusion.syraUtil.dataTypes.flash || fieldInfo.type == syra_fusion.syraUtil.dataTypes.sqlDesigner) {
					event.result.post = {
						"exposeReply": false,
	...
	function onDatePickerClick(event) {
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target))) {
			...
			if (!fieldInfo.readOnlyByModel && !syra_fusion.syraUtil.getPopupDateFromField(event.target)) {
				// Open picker
				fieldCxt = this.sapController.getCurrInst();
				currValue = fieldInfo.winModel.getDataInputValue(fieldInfo.fieldIst.xid, fieldInfo.fieldIst.nl);
				if (currValue && currValue.v) {
					evtTools.checkDate(this, event, fieldInfo, currValue.v, fieldCxt.fmt);
					processed = true;
				} else {
					setTimeout(function() {
	...
	function onDatePickerClose(event) {
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target)) && this.sapController.hasFocus(fieldInfo.fieldIst)) {
			fieldCxt = this.sapController.getCurrInst();
			evtTools.closeDatePicker(this, event, fieldInfo, fieldCxt.fmt);
			processed = true;
		}
		return processed;
	}
	...
	function onFieldChange(event) {
		// syra_fusion.syraUtil.log("onFieldChange",event)
		var nativeEvt = event && event.data && event.data.nativeEvt;
		var processed = undefined,
		...
		if (event.target && (fieldInfo = _getFieldInfo(event.target))) {
			...
			if (fieldInfo.isCheck || fieldInfo.isRadio) {
				...
				if (fieldInfo.isCheck) {
					oldValue = oldValue !== syra_fusion.sapUtil.check.off && oldValue !== syra_fusion.sapUtil.check.on ? syra_fusion.sapUtil.check.off : oldValue;
				}
				event.result = evtTools.getliveNotifyValueParams(fieldInfo.fieldIst, {
					...
					"args": [this, event.target, fieldInfo.fieldIst, event.target.getValue(nativeEvt), !hasFocus, (!hasFocus) ? requestActions.getSrvAction("SRV_GETFOCUS") : null, oldValue]
