...
var FormatWrapper = require('@sage/syracuse-ui/lib/fusion/core/client/sap/format').FormatterWrapper;
...
var _defLang = "ENG";
var _pColorSty = [";color:#000000", ";color:#FF8240", ";color:#41A940", ";color:#009FDA"];


function Session() {}

exports.Session = helpers.defineClass(Session, null, {
	makeDiagnosis: util.makeDiagnosis,
	_getHttUrl: function(prefUrl, templateUrl, params, options) {
		var url = ["/" + (prefUrl || "trans"), this.endpointParts.join("/"), templateUrl].join("/");
		return syra_url.addHost(syra_url.parse(params ? this._parseResExp.call(params, url) : url).url);
	},
	initialize: function(settings, urlSeg, localOpts, srvOpts) {
		settings && settings.thousandSep && (settings.thousandSep = (syra_site.isTabletDevice) ? "" : settings.thousandSep);
		this.endpointParts = urlSeg.endpoint ? [urlSeg.endpoint.application, urlSeg.endpoint.contract, urlSeg.endpoint.dataset] : ["x3", "erp", "X3"];
		...
		this.cdIsoLang = settings && settings.ADXLangIso ? settings.ADXLangIso : (localOpts && localOpts.cdIsoLang ? localOpts.cdIsoLang : syra_fusion.defLang);
		this.logger = util.getLog(util.getDefaultLogSwitches(document[syra_fusion.sapUtil._fNS], settings));
		this._initSharedRes();
		this._mnu = null;
		this.formatWrapper = null;
	...
	_loadSettings: function(settings) {
		...
		_.defaults(this.help || (this.help = {}), {
			...
			"baseUrl": settings[syra_fusion.syraUtil.helpToken.bUrl]
		}, {
			"type": 0,
			"location": "",
			"name": ""
	...
	_setRegionalOptions: function(settings) {
		...
		this.formatWrapper = new FormatWrapper();
		this.formatWrapper.initialize(settings.fmtExt, fmtLocales, this, "cvg", {
			...
			"fieldModeTypesK": syra_fusion.sapUtil.fieldModeType,
			"blankStrict": true
		});
	},
	...
	_setSrvVersions: function(settings) {
		...
		this.version.supervisor.vers = settings.ADXSupVersEx || "";
	...
	getHelpBaseUrl: function() {
		...
		if (!url) {
			urlPart = "{" + syra_fusion.syraUtil.helpToken.cat + "}/" + "{" + syra_fusion.syraUtil.helpToken.kw + "}";
			if (this.help && this.help.location) {
				url = this.help.location + "/" + this.cdLang + "/" + urlPart + ".htm";
			} else {
				url = this._getHttUrl("help/" + this.cdIsoLang, urlPart);
			}
		}
		return url || "";
	},
	...
	getFlashHelpUrl: function(code) {
		return this._getStaticResHttpUrl("flashHelp", code);
	},
	getSessInfo: function() {
		...
		// User
		sessInfo.login.user = syra_fusion.syraUtil.getUserName();
	...
	dispose: function() {
		if (this.endpointParts && document[syra_fusion.sapUtil._fNS]._sharedRes[this.endpointParts.join(".")]._cRef > 0) {
			// TODO : delete if == 0 ??
			delete this._sharedRes;
			document[syra_fusion.sapUtil._fNS]._sharedRes[this.endpointParts.join(".")]._cRef--;
		}
		if (this.version) {
			delete this.version;
		}
		...
		if (this.formatWrapper) {
			this.formatWrapper.dispose();
			this.formatWrapper = null;
		}
		this.makeDiagnosis = null;
		this.fieldModeTypesK = null;
	},
	...
	_initSharedRes: function() {
		...
		fns._sharedRes = fns._sharedRes || {};
		if (!fns._sharedRes[endpointId]) {
			fns._sharedRes[endpointId] = {
				"_cRef": 0,
				"_pending": {}
			};
		...
		this._sharedRes = fns._sharedRes[endpointId];
		this._verifySharedRes();
	...
	waitSharedRes: function(wait, repeat, callback) {
		...
		if (!stop && repeat > 0) {
			...
			detail = syra_fusion.getMessage(this._sharedRes.$resOnErr ? "7" : "22");
			delete this._sharedRes.$resOnErr;
			code = syra_fusion.getMessage("8") + (member ? ("\n" + syra_fusion.getMessage("15") + "'" + member + "'") : "");
			callback(!stop ? [self.makeDiagnosis(code, 3, "client.Controller.waitSharedRes()", "08", util.cltFusionNice, detail)] : null);
		}
	},
	...
	_getResPath: function(member, code) {
		var path = [this._parseResExp(_staticResConf[member].srvSubPath), _staticResConf[member].srvName ? this._parseResExp(_staticResConf[member].srvName) : code];
		if (!path[0]) {
			path.splice(0, 1);
		}
		return path.join("/") + (_staticResConf[member].srvExt || "");
	},

	_requestRes: function(member, code) {
		var header = null;
		if (!_staticResConf[member].cache || !this._sharedRes._pending.hasOwnProperty(member + code)) {
			...
			var options = {
				url: self._getStaticResHttpUrl(member, code),
				headers: header
			};
			if (_staticResConf[member].cache) {
				options.end = function(response) {
					if (response.ok) {
						if (self._sharedRes) {
							...
							self._sharedRes[member][code] = response.data;
						...
						if (_staticResConf[member].cache && self._sharedRes) {
							...
							self._sharedRes.$resOnErr = response.url || "";
	...
	getWorkFlowUrl: function(wrkflwCtx, options) {
		return this._getHttUrl([syra_fusion.syraUtil.mainPage, "?url=", "/trans"].join(""), ["$sessions", "?f=", wrkflwCtx].join(""));
	},
	getAppServerHttpUrl: function(shared, subPath) {
		return this._getStaticResHttpUrl("genericApp" + (shared ? "Shared" : ""), subPath);
	},
	...
	_getStaticResHttpUrl: function(member, code) {
		return this._getHttUrl(null, _staticURLTemplate, {
			"typres": _staticResConf[member].common ? "$shared" : "$files",
			"res": this._getResPath(member, code)
		});
	}
