...
exports.setRowWidths = function(layout) {
	var useInline = layout.page.inModal && !layout.page.isLargeClassicPage;
	if (!useInline && !layout.$layout.$widths) {
		...
		if (layout.items.length > 0) {
			...
			for (var ii = count - 1; ii >= 0; ii--) {
				...
				if ((item.$opened && (ii != layout.$layout.$tabIndex)) || (item.isInvisible && !layout.isDesigned)) {
					item.openBox && item.openBox(false);
				}
			...
			if (layout.$layout.$tabIndex !== undefined && !layout.items[layout.$layout.$tabIndex].$opened) {
				var item = layout.items[layout.$layout.$tabIndex];
				item.openBox && item.openBox(true);
			} else {
				if (layout.$layout.$tabIndex === undefined) {
					delete layout.$layout.$tabIndex;
				}
...
exports.ensureArticleVisibility = function(article, resizeIfNeed) {
	...
	exports.validateLayoutContent(article);
	resizeIfNeed && article.resizeItem && article.resizeItem(true);
};

exports.validateLayoutContent = function(article) {
	...
	article.layoutContent && exports.validate(article.layoutContent, true);
};

exports.cleanEmptyTabs = function($item, inLoop) {
	if ($item) {
		if (!inLoop) {
		...
		if ($item.$layout) {
			exports.cleanEmptyTabs($item.$layout, true);
		}
		if ($item.$items) {
			for (var ii = 0, jj = $item.$items.length; ii < jj; ii++) {
				...
				exports.cleanEmptyTabs($child, true);
			}
		}
	}
	return $item;
