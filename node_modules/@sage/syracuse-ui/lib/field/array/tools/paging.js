...
function _addPagerLink(slot, list, value, currentPageIndex, text) {
	...
	btn.link.setAttribute("data-s-list-pager", value);
...
function _getSteps() {
	if (!_$steps) {
		_$steps = [{
			...
			$shortCut: syra_shortcut.tip.firstRecord,
			$shortCutList: "SHIFT PAGEUP"
		}, {
			$bind: "$previous",
			$title: "dskpPreviousRecord",
			$shortCut: syra_shortcut.tip.prevRecord,
			$shortCutList: "PAGEUP"
		}, {
			$bind: "$next",
			$title: "dskpNextRecord",
			$shortCut: syra_shortcut.tip.nextRecord,
			$shortCutList: "PAGEDOWN"
		}, {
			$bind: "$last",
			$title: "dskpLastRecord",
			$shortCut: syra_shortcut.tip.lastRecord,
...
exports.addPagerLinksToPage = function(page, $bind) {
	if (!page.pageSteps && _pagerPageFacets.indexOf(page.$facet) >= 0) {
		var slot = syra_dom.div("s_page_pager_slot");
...
function _addPagerStepLinks(parent, slot, article) {
	...
	for (var ii = 0, jj = $steps.length; ii < jj; ii++) {
		var $step = $steps[ii];
		var btn = steps[$step.$bind] = parent.page.addItem(slot, {
			$category: "link",
		...
		btn.domItem.setAttribute("data-s-list-pager", $step.$bind);
	}
	return steps;
}
