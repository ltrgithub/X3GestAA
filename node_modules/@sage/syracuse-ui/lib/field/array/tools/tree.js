...
function _addExpandCollapseBtn(tree) {
	var icon, title;
	if (tree.expand) {
		icon = "collapse_l";
	...
	if (!tree.list.toggleExpandBtn) {
		tree.list.toggleExpandBtn = syra_button.add({
			parent: tree.list,
			text: title,
			css: tree.list.$mnPickersCss,
			fontIcon: icon,
			click: function() {
				tree.expand = !tree.expand;
				tree.list.treeDecorator.openAll(tree.expand);
				_addExpandCollapseBtn(tree);
			}
		});
		tree.list.topbar.insertBefore(tree.list.toggleExpandBtn.link, tree.list.searcherSlot);
	} else {
		syra_button.setText(tree.list.toggleExpandBtn, title, icon);
	}
}
