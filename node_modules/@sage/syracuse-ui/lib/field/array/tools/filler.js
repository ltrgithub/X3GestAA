...
var RecordArticle = require("@sage/syracuse-ui/lib/field/array/recordArticle").RecordArticle;
var _clientFilter = require('@sage/syracuse-ui/lib/filter/clientFilter');
	...
	if (!list.disposed && !list.isReloading) {
		...
		if (options) {
			if (options.$filters) {
				...
				syra_filter.query.onFetch(list, list.paramsFilter = params.filter);
			} else {
		...
		if (list.isClientFetch || list.isClientSortEnable) {
			...
			list.page.fetch(fetchOptions, function(data, response, requestUrl) {
				if (syra_site.mainPage == list.page) {
					syra_history.replaceState(requestUrl);
				}
...
exports.fetchClientRecordSet = function(list, options, pagingChange) {
	if (options && options.params) {
		...
		list.setGraphValue && list.setGraphValue();
...
exports.validateDisplay = function(list) {
	if (!list.disableDisplayValidation) {
		...
		if (list.builder.validateDisplay) {
			list.builder.validateDisplay();
		}
...
exports.doSelectAction = function(list, menuItem) {
	syra_over.openSelectModal({
		...
		url: syra_expression.parse(list, menuItem.$sourceUrl),
		...
		onValidateSelection: function(selected) {
			...
			if (list.isSingList) {
				...
				list.records[list.records.length - 1].scrollToRecord();
			} else {
				var $newRecords = syra_dataset.delta.array.add(list.dataset, selected);
...
exports.findDataRecord = function(list, $uuid) {
	var item = syra_dataset.array.find(list.dataset, $uuid);
	if (item) {
		return {
			dataRecord: item,
			dataRecordIndex: list.dataset.indexOf(item)
		};
	}
};
