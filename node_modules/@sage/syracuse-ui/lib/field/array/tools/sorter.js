	...
	list.page.externalAdapter.onSortClickEvent({
		...
		doEvent: function() {
			...
			if ($field.$type == "application/x-reference") {
				if ($field.$item.$value) {
					refBind = syra_expression.extractCode($field.$item.$value);
					if (refBind && $field.$item[refBind]) {
						refBind = syra_expression.extractCode($field.$item[refBind]);
			...
			if ($orderBy) {
				for (var ii = 0, jj = $orderBy.length; ii < jj; ii++) {
					...
					if ($orderItem.binding == $bind) {
						...
						var binding = $orderItem.refBinding ? $orderItem.refBinding : $orderItem.binding;
						// if (event.shiftKey || list.sorter._visibleCriterions.indexOf($orderItem.binding) < 0) {
						if (event.shiftKey || list.sorter._visibleCriterions.indexOf(binding) < 0) {
							$newOrderBy.push($orderItem);
						}
					}
				}
			...
			if (!found) {
				var $newOrder = {
					binding: $bind, //This is binding to data
					descending: false
				};

				if (refBind) {
					$newOrder.refBinding = col.$bind; //Binding to displayed column
				}

				$newOrderBy.push($newOrder);
...
Sorter.prototype.render = function() {
	if (this.list.articleParent == this.list.page) {
		var params = this.list.page.urlSeg && this.list.page.urlSeg.params;
		...
		for (var ii = 0, jj = this.list.$orderBy.length; ii < jj; ii++) {
			...
			if (cols) {
				...
				if (col && col.sortBtn) {
					...
					this._visibleCriterions.push(criterion.refBinding ? criterion.refBinding : criterion.binding);
					// this._visibleCriterions.push(col.$bind);
				}
			}
		}
	}
...
Sorter.prototype.toggleCancelButton = function() {
	...
	if (isSorted && !self.cancelBtn) {
		...
		syra_button.hide(self.cancelBtn, !isSorted);
	}
};
...
Sorter.prototype.addButton = function(options) {
	...
	return syra_button.add(options);
};
Sorter.prototype.cancel = function() {
	...
	if ($orderBy) {
		this._visibleCriterions.forEach(function(binding) {
			for (var ii = 0, jj = $orderBy.length; ii < jj; ii++) {
				var $orderBinding = $orderBy[ii].refBinding ? $orderBy[ii].refBinding : $orderBy[ii].binding;
