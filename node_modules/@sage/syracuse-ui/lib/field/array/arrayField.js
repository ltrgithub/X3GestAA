...
var _filler = require("@sage/syracuse-ui/lib/field/array/tools/filler");
var _shortcutInvoker = require("@sage/syracuse-ui/lib/field/array/tools/shortCutInvoker");
var _sortable = require("./tools/sortable");
var _paging = require("./tools/paging");
var _searcher = require("./tools/searcher");
...
function _applyCommonStatus(obj, metaData) {
	if (metaData) {
		...
		if (metaData.$fieldStyle !== undefined) {
			obj.$fieldStyle = metaData.$fieldStyle;
		}
...
function _applyListMeta(list, metaData) {
	...
	if (metaData.$item && metaData.$item.$properties) {
		if (Object.keys(metaData.$item.$properties) == 0) {
			delete metaData.$item.$properties;
		} else {
	...
	list.sorter.applyMetadata(metaData);
	if (metaData.$item) {
		if (metaData.$item.$links) {
			var $menus = list.$prototype.$item.$links || {};
			syra_dataset.applyDelta(list.page, $menus, metaData.$item.$links, true);
		}
		if (metaData.$item.$actions) {
			var $menus = list.$prototype.$item.$actions || {};
			syra_dataset.applyDelta(list.page, $menus, metaData.$item.$actions, true);
		}
	}
...
exports.ArrayField = _helpers.defineClass(ArrayField, null, {
	load: function(initData) {
		this.filler = _filler;
		this.sorter = _sorter.load(this);
		_paging.add(this);
		this.$mnPickersCss = "s-mn-list-btn s-mn-link";
		...
		this.domItem = syra_dom.div(this._$cssField);
		this.domItem.syraItem = this.id;
		syra_fields.addTitle(this);

		this._core = this._dataValue = syra_dom.article(this._$cssField + "-value", this.domItem);
		...
		this._dataValue.className = this.$item.$skin ? (this.$item.$skin + "-list s-array") : "s-array";


		this._core = syra_dom.div("s-array-core");
		if (!this.topbar) {
			this.topbar = syra_dom.div("s-array-topbar");
		}
		this.menusSlot = this.topbar;

		this.searcherSlot = document.createElement("div");
		syra_dom.hide(this.searcherSlot, true);
		this.searcherSlot.className = "s-array-search";
		this.topbar.appendChild(this.searcherSlot);

		this.topbarRightSlot = syra_dom.div("s-array-topbar-right", this.topbar);
		this.pagerSlot = syra_dom.div("s-pager", this.topbarRightSlot);
		...
		if (this.records.length == 0) {
			this.onDataFetch = this.page.$autoFetch || this.page.$reactAutoFetch;
			_filler.validateDisplay(this);
		}
	},
	...
	addReorder: function(record) {
		if (!record.reorderBtn) {
			record.reorderBtn = syra_dom.div(record.$skin + "-reorder");
			record.reorderBtn.syraSortableHandle = record.list.id;
			...
			record.list.$isDisabled && syra_dom.disable(record.reorderBtn, record.list.$isDisabled);
		}
	},
	showBody: function(show) {
		if (this.builder.showBody) {
			this.builder.showBody(show);
		} else {
			if (show) {
				if (this.bodyInsert) {
					this.bodyInsert.parentNode.insertBefore(this.body, this.bodyInsert.next);
					delete this.bodyInsert;
				}
			} else {
				if (!this.bodyInsert) {
					this.bodyInsert = {
						next: this.body.nextSibling,
						parentNode: this.body.parentNode
					};
					syra_dom.remove(this.body);
				}
			}
		}
	},
	setValue: function(value, metaData, parentDataRecord) {
		syra_dataset.ensure(this);
		...
		if (value !== undefined) {
			...
			this.fillList(value, parentDataRecord, isDelta);
		} else {
			metaData && _filler.validateDisplay(this);
		}
		this.builder.list_onAfterSetDataValue && this.builder.list_onAfterSetDataValue(value, parentDataRecord, metaData);
	},
	...
	onAddMenuPicker: function(btn) {
		this.searcherSlot.parentNode.insertBefore(btn.link, this.searcherSlot);
	},
	onMenuClick: function(options) {
		if (!options.menu.$isAction) {
			switch (options.menu.$sourceBind) {
				...
				case "$next":
				case "$last":
					_filler.trigger(this, null, syra_url.formatMenuUrl(options.menu.articleParent, options.menu));
					return false;
				case "$select":
					_filler.doSelectAction(this, options.menu);
					return false;
				case "$design":
	...
	onItemInOut: function(on, event, target) {
		this.builder.list_onItemInOut && this.builder.list_onItemInOut(on, event, target);
	},
	...
	getValue: function(name, $serverIndex) {
		return _filler.getValue(this, name, $serverIndex);
	},
	focus: function(select, name, $serverIndex) {
		return _filler.focus(this, select, name, $serverIndex);
	},
	...
	ensureGlobalMetaRecord: function(reset) {
		...
		if (reset || !this.globalMetaRecords) {
			this.globalMetaRecords = _applyCommonStatus(null, this.$field);
	...
	setSortable: function() {
		if (this.$capability.reorder) {
			if (!this.sortable) {
				this.sortable = _sortable.add(this, this._core);
			}
		} else {
	...
	dispose: function() {
		...
		this.sorter && this.sorter.dispose();
		syra_site.disposeObject(this.searcher);
		this.paging && this.paging.dispose();
		this.builder && this.builder.dispose && this.builder.dispose(this);
	...
	scrollToRecordField: function(field) {
		...
		field.articleParent.scrollToRecord();
	}
});