...
exports.SingleArrayBuilder = helpers.defineClass(SingleArrayBuilder, null, {
	...
	record_onAfterApplyChange: function(record, metaData) {
		var singleField = record.singleField;
		...
		var state = singleField.getState();
		if (state) {
			record.$isReadOnly = state.$isReadOnly;
			record.$isDisabled = state.$isDisabled;
		}
	},
	record_highlightOnEnter: function(record, onEnter, event) {
		if (record.singleField && record.singleField.onItemInOut) {
			record.singleField.onItemInOut(onEnter, event);
		}
	}
});