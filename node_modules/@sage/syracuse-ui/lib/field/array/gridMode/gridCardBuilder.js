"use strict";
var _slotResizer = require("syracuse-ui/lib/field/array/tools/slotResizer");

exports.removeRecordCards = function(builder) {
	...
	if (col) {
		...
		syra_dom.remove(col.titleCell);

		col.filter_cell && col.filter_cell.remove();

		var records = builder.list.records;
		for (var ii = 0, jj = records.length; ii < jj; ii++) {
			var record = records[ii];
			if (record.layoutContent) {
				syra_layout.clearContent(record.layoutContent);
				record.layoutContent.dispose();
				builder.record_disposeCard(record);
			}
			syra_dom.remove(record.cellsMap[col.key]);
...
function _setRowCardOpenerIcon(opener, expanded) {
	syra_button.setText(opener, undefined, (opener.expanded = expanded) ? "collapse_s" : "expand_s");
}
...
function _addRowCard(builder) {
	...
	if (list.$item.$cardItem) {
		builder.$rowFieldBinds = syra_pageBuilder.getDefinedFieldBinds(list.$item.$cardItem.$layout ? list.$item.$cardItem.$layout.$items : list.$item.$cardItem.$items);
	}
	_ensureMode(builder);
}
...
function _createRowCard(record) {
	var card = {
		cell: syra_dom.td("s-grid-cards-cell"),
		row: document.createElement("tr"),
		contentSlot: document.createElement("div")
	};
	card.contentSlot.className = "s-grid-cards-cell-content";
	...
	(record.list.inlineCards = record.list.inlineCards || []).push(card);
	return card;
}
...
function _onRowCardClick() {
	var title, record, list, records;
	var show = !this.expanded;
	if (this.parent.isRecordArticle) {
		record = this.parent;
		list = record.list;
		records = [record];
	...
	for (var ii = 0, jj = records.length; ii < jj; ii++) {
		...
		show && syra_layout.validateLayoutContent(item);
	}
	syra_button.setText(this, title);
	_setRowCardOpenerIcon(this, show);
	show && list.page.resizeItem();
}

exports.addRowCardCol = function(builder) {
	if (builder.isRowCardMode) {
		var table = builder.fixedTable;
		...
		_setRowCardOpenerIcon(syra_button.add({
			parent: builder.list,
			slot: col.titleCell,
			text: syra_local.flShowAllCards,
			iconOnly: true,
			fontIcon: "expand_s",
			css: "s-grid-rowcard-btn-all s-fonticon-btn",
			click: _onRowCardClick
		}), false);
		builder.list.filter_row && builder.list.filter_row.addEmptyCell(col);
		return col;
	}
};
...
exports.addRowCardOpener = function(record, col) {
	...
	record.rowCardBtn = syra_button.add({
		parent: record,
		slot: td,
		text: syra_local.flShowCard,
		iconOnly: true,
		fontIcon: "expand_s",
		css: "s-grid-rowcard-btn s-fonticon-btn",
		click: _onRowCardClick
	});
	_setRowCardOpenerIcon(record.rowCardBtn, false);
...
exports.applyDesignMeta = function(builder, metaData, designing) {
	var list = builder.list;
	var $cardItem = list.$item.$cardItem && syra_site.clone(list.$item.$cardItem);
	if (metaData.$cardPosition == "no" || metaData.$cardPosition == "row") { // || metaData.$cardPosition == undefined) {
		exports.removeRecordCards(builder);
		...
		if (metaData.$cardPosition == "row") {
			...
			col.filter_cell && col.filter_cell.insertBefore(builder.columnsMap.$rowIndex);
		...
		builder.isRowCardMode && exports.removeRecordCards(builder);
...
exports.addOutSlot = function(builder) {
	var list = builder.list;
	if (!builder.outCardResizer) {
		builder.outCardResizer = _slotResizer.addCardResizer(builder.list);
	}
	var separator = builder.outCardResizer.context.handle;
	...
	separator.className = "s-array-outcard-sep" + cssCell;
	slot.className = "s-array-outcard" + cssCell;
	syra_dom.hide(tableSlot, false);
};

exports.removeOutSlot = function(builder) {
	if (builder.outCardResizer) {
		syra_dom.remove(builder.outCardResizer.context.handle);
		builder.outCardResizer.dispose();
	}
	syra_dom.remove(builder.outCardSlot);
	builder.outCardResizer = builder.outCardSlot = null;
};
...
function _ensureCard(record, slot) {
	if (!record.cardItem) {
		...
		syra_layout.ensureArticleVisibility(record);
...
function _hightlightOutCardRecord(record, show) {
	record.dataRow && syra_dom.toggleClass(record.dataRow, "s-array-outcard-record", show);
	record.fixedRow && syra_dom.toggleClass(record.fixedRow, "s-array-outcard-record", show);
}
...
exports.load = function(builder) {
	...
	builder.record_onAfterApplyChange = function(record) {
		if (!record.list.$isEditMode && record.dataset && record.rowCardBtn) {
			var hasData;
			...
			syra_button.visibility(record.rowCardBtn, hasData);
		}
	};
	...
	builder.loadRowCard = function(record, show) {
		if (!record.rowCard) {
			record.rowCard = _createRowCard(record);
			_ensureCard(record, record.rowCard.contentSlot);
		}
		syra_dom.hide(record.rowCard.row, !show);
	};
	builder.toggleRowCard = function(record, show) {
		if (this.isRowCardMode && record) {
			_setRowCardOpenerIcon(record.rowCardBtn, show);
			this.loadRowCard(record, show);
	...
	builder.showOutCard = function(record) {
		if (this.isOutCardMode) {
			...
			syra_dom.empty(this.outCardSlot);
