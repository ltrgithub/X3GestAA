...
function _addEscapePicker(field) {
	if (!field.isEscapePickerDisabled && field.input && !field.$isReadOnly && field.page.isFusionPage && !field.$item.$inplace) {
		syra_button.addFieldPicker({
			...
			shortCutTip: syra_shortcut.tip.oldescape,
			click: function(event) {
				syra_shortcut.applyEscape(this.parent, null, event);
				this.parent.focus();
			}
		});
	}
...
exports.Field = helpers.defineClass(Field, null, {
	...
	onInputFocus: function(input, focusin) {
		...
		!focusin && this.onItemInOut(false);
	},
	...
	load: function() {
		...
		syra_site.isTabletDevice && _addEscapePicker(this);
	...
	setFieldWidth: function() {
		if (this.$isEditMode && !this.hasFreeWidth && !this.getTextRows() && !this.$item.$inplace && this._dataValue && !this.$item.$isAutoSizeDisabled) {
			...
			if (this.input) {
				...
				/*if(this.page && this.page.inModal){
                    this._core.style.width = this.widthValue + "px";
                }
                else{*/
	...
	render: function() {
		...
		if (this.$isEditMode) {
			...
			if ($textRows) {
				...
				this.input = syra_dom.textarea(css, this._dataValue, $textRows);
		...
		if (value !== undefined) {
			...
			if (this.$isEditMode) {
				...
				if (value && this.$menus && this.$menus.$details && !this.$item.$isDetailLinkDisabled) {
					...
					if (htmlValue) {
						this._dataValue.innerHTML = syra_core.escape(htmlValue);
					} else {
						if (value && this.getTextRows()) {
							if (this._escapedRawValue != value) {
								...
								this._escapedValue = syra_core.escape(this._escapedValue);
							}
