...
var _format = {
			...
			if (value !== undefined) {
				...
				if (this.$isEditMode) {
					...
					if (value && !this.$isDetailLinkDisabled && this.$menus && this.$menus.$details) {
						...
						if (this.getTextRows()) {
							this._dataValue.innerHTML = syra_core.escape(value);
						} else {
			...
			if (value !== undefined) {
				...
				if (this.$isEditMode) {
					...
					if (value) {
						...
						if (syra_site.userProfile.ctiMakeCall) {
							this.ctiCall = true;

							this._iconSchemePicker = syra_button.add({
								isIndicator: true,
								parent: this,
								iconOnly: true,
								css: " s-fonticon-btn s-scheme-picker",
								fontIcon: "expand_m",
								click: function() {
									self.$menusPopup = {
										$tel: {
											$title: syra_local.telephone,
											customHref: "tel:" + self.currentValue
										},
										$cti: {
											$title: syra_local.telephone_cti,
											customHref: "tel:" + self.currentValue,
											ctiCall: true
										}
									};
									syra_menus.open({
										scope: self,
										picker: self._iconSchemePicker,
										$itemSkin: "s-popup-telephone"
									});
								}
							});
							!this._iconSchemePicker.link.parentNode && this._dataValue.appendChild(this._iconSchemePicker.link);
	...
	$url: function(field) {
		...
		this.onUrlClick = function() {
			syra_history.openNewWindow(this.currentValue, false);
		};
	}
};
