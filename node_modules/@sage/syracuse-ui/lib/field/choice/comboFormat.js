	...
	if (field.$isEditMode) {
		...
		if (!field.picker_choice) {
			syra_button.addFieldPicker({
				...
				shortCutTip: syra_shortcut.tip.openPicker,
...
Combo.prototype.applyEscape = function(event) {
	...
	if (this.list.filter != filter) {
		...
		for (var ii = 0, jj = this.list.allItems.length; ii < jj; ii++) {
			...
			if (hasFilter) {
				...
				item.link.innerHTML = syra_core.escape(html);
			} else {
...
Combo.prototype.fill = function() {
	...
	for (var ii = 0, jj = this.field.$enum.length; ii < jj; ii++) {
		...
		if (btn.text != null) {
			if (btn.text == "") {
				btn.text = syra_local.fieldChoiceEmpty;
			}
...
exports.render = function(field) {
	...
	field.applyEscape = function(event) {
		return this.combo.applyEscape(event);
	};
