...
exports.setValue = function(field, value) {
	...
	if (field.$isEditMode) {
		...
		if (value && !field.$isDetailLinkDisabled && field.$menus && field.$menus.$details) {
			syra_fields.addDetailLink(field, value, field.$menus.$details);
		} else {
...
exports.setState = function(field, state) {
	if (state.$isDisabled !== undefined || state.$isReadOnly !== undefined) {
		...
		for (var ii = 0, jj = field._choices.length; ii < jj; ii++) {
			...
			var input = choice.checkBox.input;
			syra_dom.disable(input, isDisabled, true);
			if (state.$isReadOnly !== undefined) {
				if (field.$isReadOnly) {
					input.syraReadOnly = true;
					input.setAttribute("readonly", "readonly");
				} else {
					if (input.syraReadOnly) {
						input.syraReadOnly = false;
...
exports.dispose = function(field) {
	if (field) {
		field.getValue = field.selectedIndex = field.selectRadioBtn = field._fieldset = null;
	}
};

exports.render = function(field) {
	field._choices = [];
	field.$item.$choiceLayout = field.$item.$choiceLayout || "row";
	field.getValue = function(event) {
...
function _draw(field) {
	if (field.$isEditMode) {
		...
		if (!field._fieldset) {
			...
			field._fieldset = syra_dom.div($skin, field._dataValue);
		} else {
		...
		if (isColLayout) {
			...
			for (var ii = 0, max = Math.ceil(field.$enum.length / colCount); ii < max; ii++) {
				rows[ii] = syra_dom.div("s-field-choice-rd-row", field._fieldset);
			}
		}
		...
		for (var ii = 0, jj = field.$enum.length; ii < jj; ii++) {
			...
			rdItem.checkBox.input.syraOnClick = "onFieldInputEvent";
			...
			if (field.$item.$icon) {
				var $path = syra_config.$iconPath + (field.$item.$icon.$path || "");
		...
		if (isColLayout && curCol > 0) {
			while (curCol < colCount) {
				syra_dom.div($skinItem, rows[curRow]);
				curCol++;
			}
		}
		rows = null;
