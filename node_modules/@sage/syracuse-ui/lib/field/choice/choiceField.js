...
exports.ChoiceField = helpers.defineClass(ChoiceField, Field, {
	...
	applyDesignMeta: function(metaData, designing) {
		...
		if (metaData.$format && metaData.$format != this.$item.$format) {
			...
			syra_dom.remove(this._dataValue);
			this._core = this._dataValue = null;
			syra_fields.addValueSlot(this);
			this.render();
			updated = true;
		}
		if (metaData.$choiceLayout !== undefined && metaData.$choiceLayout != this.$item.$choiceLayout) {
			this.choiceFormat.applyLayoutChange(this, metaData.$choiceLayout);
			updated = true;
		}
		if (updated) {
	...
	setValue: function(value, metaData) {
		if (value !== undefined) {
			this.choiceFormat.setValue(this, value);
	...
	localizeEnum: function() {
		for (var ii = 0, jj = this.$enum.length; ii < jj; ii++) {
			...
			if ($title.length > 0 && $title[1] == "@") {
				this.$enum[ii].$title = syra_expression.render(this.boxParent, $title);
			}
		}
	},
	...
	setFieldWidth: function() {
		if (!this.choiceFormat.setFieldWidth || this.choiceFormat.setFieldWidth(this)) {
	...
	applyShortCut: function(shortcut, event) {
		return this.choiceFormat.applyShortCut && this.choiceFormat.applyShortCut(this, shortcut, event) || false;
