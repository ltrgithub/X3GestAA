...
var _ALL_IMAGES = "image/" + "*";


function uploadFile(field, file) {
	if (_validateFile(field, file)) {
		field._progress = document.createElement("div");
		field._progress.className = "s-progress";
		field.progressValue = syra_dom.div("s-progress-value", field._progress);
		field.selectBtn.link.parentNode.insertBefore(field._progress, field.selectBtn.link);
		field.disableButtons(true);
		field.setLegend(syra_local.fdocUpload + ":" + " " + file.name + " 0%");
		field.ajaxUploader = syra_ajax.createUploader(syra_expression.parse(field.articleParent, field.$field.$url), file, field.$contentType || field.$field.$contentType || file.type, function(progress, isLoaded) {
			if (field.articleParent) {
				if (!isLoaded) {
					field.setLegend(syra_local.fdocUpload + ":" + " " + file.name + " " + progress + "%");
					field.progressValue.style.width = progress + "%";
				} else {
					field.setLegend(file.name + " " + syra_local.fdocUploaded);
					field.disableButtons(false);
					field.setLegend(syra_local.fdocUpload + ":" + " " + file.name + " 100%");
					field.progressValue.style.width = "100%";
					setTimeout(function() {
						if (field._progress) {
							syra_dom.remove(field._progress);
							delete field._progress;
						}
						delete field.progressValue;
						field.setLegend(syra_local.fdocDropSelectFile);
					}, 1500);
				}
			}
		});
		field.ajaxUploader.upload(function(response) {
			_releaseXhrUploading(field);
			if (field.articleParent) {
				if (response.error) {
					field.articleParent.applyChange(response.error);
				} else {
					if (!response.success) {
						field._setDocumentFile(file);
					} else {
						field.page.applyChange(response.success);
					}
					field.page.externalAdapter.onUploadSucceeded({
						"field": field
					});
				}
			}
		});
		field.page.uploaders.add(field);
	}
}
...
function _setImageFile(field, file) {
	...
	if (file) {
		...
		if (field._imgLoader) {
			field._imgLoader.onload = field._imgLoader.onerror = field._imgLoader = null;
		}
		if (field.anchor) {
			syra_dom.empty(field.anchor);
			field.anchor.setAttribute("href", "#");
			syra_dom.disable(field.anchor, true);
...
function _saltUrl(url) {
	...
	return url + sep + "salt=" + Math.round(Math.random() * 1000) + (syra_culture.languageCode ? "&language=" + syra_culture.languageCode : "");
}

function _removeImageArea(field) {
	syra_dom.remove(field._imageArea);
}

function _addImageArea(field) {
	_removeImageArea(field);
	if (field.$item.imageArea) {
		return field._imageArea = field.$item.imageArea;
	} else {
		var img = field._imageArea = syra_dom.div(field.$skin + "-img-img");
		img.style.width = field.$item.$imageWidth;
		img.style.height = field.$item.$imageHeight;
		img.style.backgroundSize = "contain";
		img.style.backgroundRepeat = "no-repeat";
		field.anchor.appendChild(img);
	}
}
...
var _imageLoader = {
	add: function(field) {
		...
		_imageLoader.clear(field);
		field._imgLoader = document.createElement("img");
		field._imgLoader.onload = function() {
			if (!this.syraIsClear) {
				// handling image display waiting icon
				clearTimeout(field._waitingTimeOut);
				if (field.anchor && field._imgLoader) {
					field._isImageLoaded = true;
					_imageLoader.clearWait(field);
					_addImageArea(field);
					field.resizeItem();
		...
		field._imgLoader.onerror = function() {
			if (!this.syraIsClear) {
				clearTimeout(field._waitingTimeOut);
				field._isImageLoaded = true;
				if (field.anchor && field._imgLoader) {
					_imageLoader.clearWait(field);
					if (field._imageArea) {
						field._imageArea.style.backgroundImage = "none";
					}
				}
			}
		};
		...
		field._waitingTimeOut = setTimeout(function() {
			if (field.anchor && !field._imageWait && !field._isImageLoaded) {
				field._imageWait = syra_dom.div("s-field-img-img-wait", field.anchor);
			}
		}, 500);
		field._imgLoader.src = field.imageUrl;
	},
	clearWait: function(field) {
		syra_dom.remove(field._imageWait);
		delete field._imageWait;
	},
	clear: function(field) {
		_imageLoader.clearWait(field);
		if (field._imgLoader) {
			field._imgLoader.syraIsClear = true;
			field._imgLoader.onload = field._imgLoader.onerror = field._imgLoader = null;
		}
	}
};
...
function _showImageFile(field) {
	...
	if (field.imageUrl) {
		...
		if (field.anchor) {
			...
			syra_dom.disable(field.anchor, false);
		}
	}
}
...
function _setFile(field, file) {
	if (file) {
		...
		if (field.$item.$isEmbedded) {
			...
			if (!field._imgLoader) {
				field._imgLoader = document.createElement("img");
				syra_dom.hide(field._imgLoader, true);
				field._imgLoader.onload = function() {
					clearTimeout(field._defaultFileTimeOut);
					if (field._imgLoader) {
						field._imgLoader.className = field.$skin + "-file-img";
						syra_dom.hide(field._imgLoader, false);
					}
				};
				field.anchor.appendChild(field._imgLoader);
			}
			...
			var image = syra_icon.getDocument($type);
			field._imgLoader.src = syra_config.$iconPath + "/file/s-file-" + (image || "default") + ".png";
			_showBtn(field, field.removeBtn, true);
...
function _validateFile(field, file) {
	var errors = [];
	if (file.type !== undefined && field.$field.$acceptedTypes) {
		if (field.$field.$acceptedTypes.indexOf(file.type) < 0) {
			if (field.isImage) {
				if (!(file.type.indexOf("image") == 0 && field.$field.$acceptedTypes.indexOf(_ALL_IMAGES) >= 0)) {
	...
	if (errors.length > 0) {
		syra_form.invalidateField(field, errors);
		return false;
	}
	return true;
}
...
function _showBtn(field, btn, show) {
	if (btn) {
		if (field.$item.$inplace) {
			syra_button.visibility(btn, show && !field.$isReadOnly);
		} else {
			syra_button.hide(btn, !show || field.$isReadOnly);
		}
	}
}
...
function _onRemoveClick(btn) {
	...
	field.page.externalAdapter.onRemoveUploadedFile({
		...
		doEvent: function() {
			syra_ajax.del({
				url: syra_expression.parse(field.articleParent, field.$dataUrl),
				end: function(response) {
					if (response.ok) {
						field.setValue(null);
						field.selectBtn.link.focus();
						field.articleParent.applyChange(response.data, response);
						field.page.externalAdapter.onRemoveUploadedFileSucceeded({
							field: field
						});
					} else {
						syra_alert.show(response.error.diagnoses, field);
					}
				}
			});
		}
...
function _addUploadInput(field) {
	syra_dom.remove(field.input);
	field.input = syra_dom.input("s-field-input-uploader", field._uploadSlot, "file");
	if (field.isImage && field.$field.$acceptedTypes) {
		field.input.setAttribute("accept", field.$field.$acceptedTypes.join(","));
	}
	if (field.$item.$enableMultiple) {
...
helpers.defineClass(Uploaders, null, {
	add: function(field) {
		if (this.list.indexOf(field) < 0) {
	...
	abortAll: function() {
		if (this.list) {
			for (var ii = 0, jj = this.list.length; ii < jj; ii++) {
				try {
					this.list[ii].ajaxUploader && this.list[ii].ajaxUploader.abort();
				} catch (error) {}
			}
		}
		_disableAll(false);
	...
	release: function(field, disposed) {
		var ii = this.list.indexOf(field);
		if (ii >= 0) {
			syra_locker.unlock();
			this.list.splice(ii, 1);
			disposed && field.ajaxUploader && field.ajaxUploader.abort();
			_disableAll(false);
		}
	}
});
...
function _releaseXhrUploading(field, disposed) {
	if (field.page && field.page.uploaders) {
		field.page.uploaders.release(field, disposed);
	}
	field.ajaxUploader && field.ajaxUploader.dispose();
	field.ajaxUploader = null;
	if (disposed) {
		var ii = _docFields.indexOf(field);
		if (ii >= 0) {
			_docFields.splice(ii, 1);
		}
	}
}
...
function _disableAll(disable) {
	for (var ii = 0, jj = _docFields.length; ii < jj; ii++) {
		_docFields[ii].disableButtons(disable);
	}
}
...
exports.DocumentField = helpers.defineClass(DocumentField, Field, {
	load: function() {
		if (!this.page.uploaders) {
			this.page.uploaders = new Uploaders();
		}
	...
	initialize: function() {
		...
		if (this.isImage) {
			this.$field.$acceptedTypes = this.$field.$acceptedTypes || [_ALL_IMAGES];
			if (this.$field.$acceptedTypes[0] == "image") {
				this.$field.$acceptedTypes[0] = _ALL_IMAGES;
			}
	...
	setState: function(state) {
		...
		if (state.$isDisabled !== undefined) {
			this.disableButtons(state.$isDisabled);
		}
	...
	disableButtons: function(disable) {
		syra_button.disable(this.selectBtn, disable);
		syra_button.disable(this.removeBtn, disable);
	},
	...
	onFieldInputEvent: function(event, shortcuts) {
		switch (event.type) {
			case "change":
				...
				break;
			case "click":
				return Field.prototype.onFieldInputEvent.call(this, event, shortcuts);
		}
	},
	...
	_renderEditMode: function() {
		...
		syra_dom.disable(this.anchor, true);
		this.anchor.syraOnClick = "onFieldInputEvent";
		...
		this._uploadSlot = document.createElement("div");
		this._uploadSlot.className = this.$skin + "-doc-upload";
		if (!this.$item.$isGridCell) {
			this._legend = document.createElement("div");
			this._legend.className = $skin + "-legend";
			this._legend.textContent = syra_local.fdocDropSelectFile;
			this._uploadSlot.appendChild(this._legend);
		}
		_addUploadInput(this);

		this.selectBtn = syra_button.add({
			parent: this,
			slot: this._uploadSlot,
			text: syra_local.fdocSelectFile,
			css: this.$skin + "-doc-btn s-field-doc-upload",
			fontIcon: "upload",
			iconOnly: this.$item.$inplace,
			click: _onSelectClick
		});

		this.removeBtn = syra_button.add({
			parent: this,
			slot: this._uploadSlot,
		...
		this._right.appendChild(this._uploadSlot);
		this.isImage && _setImageAnchorDefaultSize(this);
	},
	_renderDetailMode: function() {
		var $skin = this.$skin + (this.isImage ? "-img" : "-file");
		if (this.anchor) {
			this._imgLoader = this._imageArea = null;
			syra_dom.remove(this.anchor);
		}
		this.anchor = document.createElement("a");
		this.anchor.className = $skin + "-link";
		this.anchor.syraOnClick = "onFieldInputEvent";
	...
	setValue: function(value, metaData) {
		if (!this.ajaxUploader) {
			if (value !== undefined) {
				this.currentValue = value;
				if (!this.$isEditMode) {
					this.currentValue && this._renderDetailMode();
				} else {
	...
	resizeItem: function() {
		var imgW = this._imgLoader && this._imgLoader.width;
		if (this._imageArea && imgW) {
			var linW = this.anchor.clientWidth;
			var percent = (imgW >= linW) ? (linW / imgW) : 1;
			this._imageArea.style.height = (this._imgLoader.height * percent) + "px";
	...
	dispose: function() {
		_releaseXhrUploading(this, true);
		if (this.$isEditMode) {
			syra_button.dispose(this.selectBtn);
			syra_button.dispose(this.removeBtn);
			this._bindDropArea(false);
		}
		if (this._imgLoader) {
			this._imgLoader.onload = this._imgLoader.onerror = this._imgLoader = null;
		}
		if (this.uploadFrame) {
