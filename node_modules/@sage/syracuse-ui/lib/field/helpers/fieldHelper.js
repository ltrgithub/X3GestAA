...
var _format = require('@sage/syracuse-ui/lib/field/helpers/format');
//syra_fields.embedded
exports.advancedState = require('@sage/syracuse-ui/lib/field/helpers/advancedState');
exports.embedded = require('@sage/syracuse-ui/lib/field/helpers/embedded');
...
exports.checkbox = {
	render: function(item, id, css, isRadio) {
		var box = {
			slot: syra_dom.div(css + " s-checkbox-slot"),
			icon: syra_dom.label(css + " s-checkbox-icon s-fonticon-btn"),
...
exports.onFieldFocusChange = function(field, isIn, event) {
	if (isIn) {
		...
		syra_site.checkUniqueSelection(field);
		syra_popup.closeAll();
	} else {
...
exports.addDetailLink = function(field, value, $detailsLink) {
	if (!field.$isDetailLinkDisabled) {
		...
		if (value && field.$item && field.$item.$isUnitField) {
			value = value.$symbol || value.$value || value;
		}
...
exports.setDescription = function(field, $description) {
	if ($description !== undefined) {
		...
		if (field.descriptionText.length > 0) {
			...
			if (!field.$item.$inplace && !field.$item.$isDescriptionHidden) {
...
exports.getState = function(field) {
	return field; //state is set diorectly on field instance
}

exports.setState = function(field, state) {
	if (!field.$item.$isFilterMode) {
		if (state.$isMandatory !== undefined) {
			exports.mandatory(field, state.$isMandatory);
		}
		if (state.$isDisabled !== undefined) {
			exports.disable(field, state.$isDisabled);
		}
	...
	if (state.$isHidden !== undefined) {
		field.showField(!(field.$isHidden = state.$isHidden));
	}
};

exports.mandatory = function(field, $isMandatory) {
	field.$isMandatory = $isMandatory;
	if (field.$isEditMode) {
		if (field.domTitle && field.getTitle().length > 0) {
			syra_dom.toggleClass(field.domTitle, "s-mandatory", $isMandatory);
			if ($isMandatory) {
				if (!field._mandatoryFlag) {
		...
		syra_dom.toggleClass(field._dataValue, "s-mandatory", $isMandatory);
	}
};
exports.disable = function(field, $isDisabled) {
	field.$isDisabled = $isDisabled;
	syra_dom.disable(field.domItem, $isDisabled, true);
	if (field.input) {
		syra_dom.disable(field.input, $isDisabled);
		if (field.$item.$contentEditable) {
			_contentEditable.setState(field, state);
		}
	}
	field.domTitle && syra_dom.disable(field.domTitle, $isDisabled);
	field.descriptionItem && syra_dom.disable(field.descriptionItem, $isDisabled);
	syra_dom.disable(field._dataValue, $isDisabled);
};

exports.applyMetaData = function(field, metaData, isRefItem) {
		...
		if (metaData) {
			if (!isRefItem) {
				...
				if (metaData.$description !== undefined) {
					field.setDescription(metaData.$description);
				}
			}
			field.setState(metaData);
			if (metaData.$diagnoses !== undefined) {
				syra_alert.show(metaData.$diagnoses, field);
			}
			...
			if (metaData.$fieldStyle || metaData.$valueStyle != undefined || metaData.$titleStyle) {
				field.applyCustomStyle(metaData);
			}
...
exports.addResizer = function(field) {
	field.resizer = new Resizer({
		item: field,
		body: field._dataValue,
		slot: field.domItem,
		handles: ["s"],
...
exports.invalidateField = function(field, errors, noViewer) {
	let diagnoses = [];
	errors = errors || [];
	...
	syra_alert.show(diagnoses, field, "error", noViewer);
