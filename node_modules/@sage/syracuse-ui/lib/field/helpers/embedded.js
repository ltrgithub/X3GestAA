...
exports.checkListFilterCapabilities = function(list) {
	...
	for (var ii = 0, jj = $binds.length; ii < jj; ii++) {
		...
		if (!$field.$isExcluded) {
			$field.$filterable = $field.$capabilities && ($field.$capabilities.indexOf("filter") >= 0);
			if ($field.$filterable) {
...
exports.addEmbeddedLink = function(article, $bind, $menu) {
	if (article.isDesktopPage && article.$prototype.$properties) {
		...
		if (!$vignette) {
			...
			syra_dataset.applyDelta(article.page, $vignette.$location, $menu);
		}
		...
		if (unknowField) {
			...
			syra_item.remove(unknowField, true);
			layoutParent.loadChildItem(null, layoutParent.$layout.$items[index], index);
			layoutParent.isItemRegisterDisable = false;
		} else {
			if (article.layoutContent && (!article.boundFields[$bind] || !article.boundFields[$bind][0])) {
...
function _refreshChildRelation(article, vignetteField) {
	...
	if (refresh) {
		vignetteField.$url = syra_expression.parseUrl(vignetteField.articleParent, vignetteField.$field.$location.$url, vignetteField.articleParent.dataset, vignetteField.$masterBinds);
