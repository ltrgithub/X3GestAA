...
function _onClick() {
	...
	item._close(item._dateValue = syra_culture.date.fromJsDate(new Date(year, month - 1, item._dateValue.day)));
}
...
exports.MonthChoice = helpers.defineClass(MonthChoice, null, {
	create: function(field, parent, dateValue, close) {
		...
		for (var ii = 0; ii < 6; ii++) {
			...
			this.addBtn(row, ii + 1, syra_culture.date.monthName(ii + 1, true), this._dateValue.month);
			this.addBtn(row, ii + 7, syra_culture.date.monthName(ii + 7, true), this._dateValue.month);
...
exports.YearChoice = helpers.defineClass(YearChoice, null, {
	...
	addPrevNextCell: function(row, isPrev) {
		syra_button.add({
			parent: this,
			slot: syra_dom.td("s-calendar-my-link-cell", row),
			...
			click: function(event, btn) {
				...
				for (var ii = 0, jj = yearBtns.length; ii < jj; ii++) {
					var btn = yearBtns[ii];
					btn.value += step;
					syra_button.setText(btn, btn.value);
