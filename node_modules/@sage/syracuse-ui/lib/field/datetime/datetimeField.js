...
var _choiceType = {
	time: require('@sage/syracuse-ui/lib/field/datetime/timeChoice').TimeChoice,
	date: require('@sage/syracuse-ui/lib/field/datetime/dateChoice').DateChoice
};
...
var _formatter = {
	"application/x-date": function(field, val) {
		...
		var twoDigitYearMax = syra_format.getTwoDigitYearMax().toString();
...
function _onPickerClick(options) {
	...
	if (!field.popupPicker) {
		...
		field.popupPicker = syra_popup.open({
...
exports.DateTimeField = helpers.defineClass(DateTimeField, Field, {
	...
	setButtonValue: function(date) {
		var self = this;
		this.popupPicker.close();
		if (!this.$isReadOnly) {
			if (this.$field.$type == "application/x-datetime") {
				date = syra_culture.datetime.make(date.year, date.month, date.day);
			}
			this.page.externalAdapter.setButtonValue({
				target: this,
				value: date.toString(this.localFormat),
				internalValue: date._value,
	...
	render: function() {
		if (this.$isEditMode) {
			this.input = syra_dom.input(this.$skinInput, this._dataValue);
			syra_button.addFieldPicker({
				...
				shortCutTip: syra_shortcut.tip.openDatePick,
	...
	initialize: function() {
		...
		this.formatApi = syra_culture.getFormatApi(this.$field.$type);
		this.localFormat = syra_format.getPattern(this.$field.$type);
		this._formatInput = _formatter[this.$field.$type];
		this.localFormatDisplay = this.$field.$localFormatDisplay || syra_format.getPattern(this.$field.$type, this.$field.$format);
		this.$field.$maxLength = Math.max(this.$field.$maxLength || 0, syra_format.getLength(this.localFormatDisplay));
		switch (this.$field.$type) {
			case "application/x-time":
				this.isTimeField = true;
				break;
			case "application/x-date":
				this.formatObj = syra_format.processFormat(this.localFormat);
				break;
		}
	...
	setValue: function(value, metaData) {
		if (value !== undefined) {
			...
			try {
				...
				syra_form.invalidateField(this, error.message);
			}
	...
	parseMask: function(val) {
		...
		if (dayOffset != null) {
			dateObj = syra_culture.datetime.now().addDays(dayOffset);
			if (!this.page.externalAdapter.getFormatedValFromHelperVal) {
				newVal = dateObj.toString(this.localFormat || syra_format.getPattern(this.$field.$type));
			} else {
			...
			if (!event) {
				...
				if (inputHelper) {
					...
					if (val) {
						...
						if (val) {
							try {
								val = this.formatApi.parse(val, this.localFormat).toString();
							} catch (error) {
								if (this.$field.$type == "application/x-datetime") {
									val = this.formatApi.parse(val, syra_culture.date_short).toString();
								} else {
									throw error;
								}
							}
