"use strict";
exports.query = require('syracuse-ui/lib/filter/view/query');
	...
	switch (operator) {
		...
		default:
			if (value != null && value != "") {
				...
				if (type === "application/x-choice" && field.$field.$value)
					type = field.$field.$value.$type;
				switch (type) {
					case "application/x-reference":
						rawValue = rawValue.$value.replace(/\*/g, "%");
						value = "'" + rawValue + "'";
						break;
...
function _fetchReferenceItem(sdata_where, field, keys) {
	keys.pop();
	var criteria = sdata_where && _getCriteria(sdata_where, keys);
	if (criteria) {
		if (criteria.value.length == 0) {
			...
			if (field.$menus && field.$menus.$lookup) {
				...
				syra_ajax.get({
					...
					params: {
						...
						if (response.ok) {
							var list = response.data && response.data.$resources;
							...
							syra_alert.show(response.error.diagnoses, field.page);
						}
					}
				});
			}
...
exports.setField = function(sdata_where, field) {
	...
	if (field.isReferenceField) {
		...
		criteria = sdata_where && _getCriteria(sdata_where, keys);
