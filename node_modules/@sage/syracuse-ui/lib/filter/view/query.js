...
function _selectLink(list, $filterId, show) {
	...
	if (show && !menu && $filterId != "$noFilters") {
		...
		menu && syra_dom.toggleClass(menu.domItem, "s-selected", show);
	}
}
...
exports.addLinks = function(list) {
	var page = list.page;
	var $filters = page.$prototype.$filters;
	if ($filters && page.menuBar) {
		...
		if ($items.length) {
			...
			page.addItem(page.menuBar.body, {
				$category: "menus",
				$skin: "s-mn-filters",
			...
			}, page).onMenuClick = function(options) {
				list.filler.trigger(list, {
					$filters: options.menu.$item.$sourceBind
				});
				return false;
			};
			syra_menus.applyChange(page, {
				$links: $links
			});
