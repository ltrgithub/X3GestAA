	...
	record.page.externalAdapter.onFilterEvent({
		...
		doEvent: function() {
			if (field.page.filter_popup) {
				...
				field.page.filter_popup = syra_popup.open({
					slot: field.filter_list.scrollSlot,
					of: field.layoutSlot,
					onResize: function(maxHeight) {
						if (field.filter_list) {
							field.filter_list.resize(maxHeight);
						}
					},
					close: function() {
						field.filter_list && field.filter_list.dispose();
						delete field.filter_list;
						if (field.page) {
							field.page.filter_popup = null;
						}
					}
				});
				syra_quality.listFilter(field, btns);
			}
		}
	});
}
	...
	if (field.page.isFusionPage) {
		switch ($type) {
			...
			case "application/x-datetime":
				return ["none", "ge", "gt", "le", "lt", "eq", "ne"];
			case "application/x-string":
				return ["none", "like", "like_s", "like_e", "ge", "gt", "le", "lt", "eq", "ne"];
			case "application/x-choice":
			case "application/x-real":
			case "application/x-decimal":
			case "application/x-integer":
			case "application/x-quantity":
				return ["none", "eq", "ge", "gt", "le", "lt", "ne"];
			default:
				return ["none", "eq", "ne"];
		}
	} else {
		switch ($type) {
			...
			case "application/x-quantity":
				if (field.$field.$type != "application/x-reference") {
					return ["none", "eq", "ge", "gt", "le", "lt", "between", "ne"];
				}
				return ["none", "eq", "ne"];
		}
		return ["none", "eq", "ne"];
...
exports.addPicker = function(field, css) {
	...
	field.filter_picker.link.setAttribute("data-s-list-filter-op", field.$item.$bind);
