...
exports.ExtendField = _helpers.defineClass(ExtendField, Field, {
	load: function(initData) {
		...
		//temp hack on x-array-gantt as instead of highchart, it has to be bound to datasource.Has to be improved 
		//if (this.$field.$type.indexOf("x-array") >= 0) {
		if (this.$field.$type != "application/x-array-gantt" && this.$field.$type.indexOf("x-array") >= 0) {
	...
	addValueSlot: function() {
		...
		this._dataValue = syra_dom.div(css, this.domItem);
	},
	render: function(slot) {
		...
		self.extendWidget = self.api.create({
			...
			invalidate: function(error) {
				syra_form.invalidateField(self, error);
			},
			setDirty: function(dirty, sendChanges) {
				...
				if (self.$isEditMode) {
					...
					if (sendChanges) syra_form.onInputChange(self, "change");
				}
			},
		});
	...
	getValue: function() {
		if (this.extendWidget && this.extendWidget.getValue) {
			return this.extendWidget.getValue();
		} else {
