"use strict";
exports.culture = {};
exports.icon = require("@sage/syracuse-ui/lib/utility/icon");
exports.Scrollbar = require('@sage/syracuse-ui/lib/scroll/scrollbar').Scrollbar;
exports.Draggable = {
	add: function(props) {
		return syra_drag.add(props);
	}
};
...
exports.registerFlashWidget = function(context) {
	...
	context.field.page.hasFlashChildField = true;
	while (boxParent) {
...
exports.data = {
	applyDelta: function(context, target, source) {
		syra_dataset.applyDelta(context.field.page, target, source);
	},
	clone: function(obj) {
...
exports.menu = {
	fire: function(context, options) {
		...
		options.scope = options.scope || context.field.articleParent;
...
exports.page = {
	getBodySize: function(context) {
		var page = context.field.page;
		return {
			height: page.scrollview.clientHeight - page.getPageSize().headerHeight,
			width: page.scrollview.clientWidth
		};
	},
...
exports.popup = {
	open: function(option) {
		if (option.position) {
			option.my = option.position.my;
			option.at = option.position.at;
			option.of = option.position.of;
		}
		return syra_popup.open(option);
	},
	closeAll: function() {
...
exports.keyboard = {
	getKeyName: function(code) {
		return syra_keyboard.keyCodeMap[code];
	},
	keydown: function(event, targetItem, playback) {
		return syra_keyboard.keydown(event, targetItem, playback);
	},
	keypress: function(event, targetItem, playback) {
		return syra_keyboard.keypress(event, targetItem, playback);
	},
	keyup: function(event, targetItem, playback) {
		return syra_keyboard.keyup(event, targetItem, playback);
	}
}