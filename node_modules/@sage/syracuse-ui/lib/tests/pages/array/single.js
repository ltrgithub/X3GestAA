...
exports.scenarios = require("@sage/syracuse-ui/lib/tests/pages/field/scenarios").scenarios;
...
exports.load = function(page, finish) {
	...
	page.form_postAction = function(menuItem, target, value) {
					...
					self.boundFields[$bind].forEach(function(field, index) {
						if (index == 0) {
							...
							data[$bind] = field.records.map(function(row) {
								return row.boundFields.$singleField[0].getValue();
							});
							if (value.$deleteItem) {
								data[$bind].shift();
							}
	...
	page.onFormUpdateDelta = function(field, value) {
		var self = this;
	...
	page.fillDiagnoses = function(value, $bind) {
		if ($bind.indexOf("array") == 0) {
			...
			syra_dataset.ensure(field).forEach(function(dataValue) {
				metaData.$items.push({
					$diagnoses: $diagnoseItem
				});
			});
