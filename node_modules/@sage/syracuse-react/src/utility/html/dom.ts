...
let _browserFlagsInitialized = false;
let _browserFlags = {
	isIE: false
}
...
export function scrollTo(nodeId: string, axis?: "v"|"h") {
	let node = document.getElementById(nodeId);
	if (!node) return;
	// Custom scroll implementation for IE because there the native scrollbar could show up since IE does a horizontal scroll on scrollIntoView sometimes
	if (isIE() && axis === "v" && scrollToScrollbar(node)) return;
	node.scrollIntoView(true);
}

export function scrollToScrollbar(node: HTMLElement): boolean {
	let parent = node.offsetParent as HTMLElement;
	let offsetTop = node.offsetTop;
	while (parent) {
		if (parent.classList && parent.classList.contains("s_scrollbar_slot")) break;
	...
	parent.children[0].scrollTop = offsetTop;
	return true;
}

export function isIE() {
	if (!_browserFlagsInitialized) {
		initBrowserFlags();
	}
	return _browserFlags.isIE;
}

function initBrowserFlags() {
	_browserFlagsInitialized = true;
	let ua = window.navigator.userAgent;
	_browserFlags.isIE = ua.indexOf('Trident/') !== -1;
}