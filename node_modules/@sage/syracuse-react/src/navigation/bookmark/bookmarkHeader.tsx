...
export class BookmarkHeader extends React.Component<IBookmarkHeaderProps, IBookmarkHeaderState> {
    ...
    private bookmarkHeaderList: HTMLElement;

    private getLast(width: number, items: number[]): number {
    ...
    private mapWidth(items: any): number[] {
        const arr = [];
    ...
    constructor(props: IBookmarkHeaderProps) {
        super(props);
        setBookmarkHeaderUpdateHandler(() => {
            this.forceUpdate(() => {
                this.setState({
                    width: this.bookmarkHeaderList.clientWidth,
                    lastShown: this.getLast(this.bookmarkHeaderList.clientWidth, this.mapWidth(this.bookmarkHeaderList.children))
                });
            });
        });
    ...
    public render() {
        return (
            <div className={'s_profile_bar_bookmark_header' + (this.props.show ? '' : ' hidden')}>
                <ul ref={n => (this.bookmarkHeaderList = n)} className="s_profile_bar_bookmark_header_list">
                    {this.props.bookmarks.menus.map((menu: BookmarkMenu, index: number) =>
                        <BookmarkHeaderListItem
                            key={`BookmarkHeaderListItem_${index}`}
                            menu={menu}
                ...
                <BookmarkHeaderIcons
                    show={this.state.lastShown && this.state.lastShown !== this.bookmarkHeaderList.children.length - 1}
...
export const BookmarkHeaderIcons = (props: IBookmarkHeaderIconsProps) =>
    <div className={'s_profile_bar_bookmark_header_icon' + (props.show ? '' : ' invisible')}>
