...
export default class SitemapPage extends React.Component<ISitemapPageProps, {}>{
	...
	componentDidMount() {
		setTimeout(() => {
			this.pageNode && this.pageNode.classList.add('s_sitemap_page_moving_in');
		});
	}
	...
	rollUp(): Promise<void> {
		return new Promise((res, rej) => {
			const callback = () => {
				this.pageNode.removeEventListener('transitionend', callback);
				this.searchBar.clear();
				res();
			};
			this.pageNode.addEventListener('transitionend', callback);
			this.pageNode ? this.pageNode.classList.remove('s_sitemap_page_moving_in') : res();
		});
	}
	...
	rollDown(): Promise<void> {
		return new Promise((res, rej) => {
			...
			this.pageNode.addEventListener('transitionend', callback);
			this.pageNode ? this.pageNode.classList.add('s_sitemap_page_moving_in') : res();
		});
	}
