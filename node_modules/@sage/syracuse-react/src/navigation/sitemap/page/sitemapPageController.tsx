...
export class SitemapPageController extends React.Component<IProps, {}>{
            ...
            if (!this.modulePositions) {
                ...
                for (let module of modules) {
                    ...
                    this.modulePositions.push({
                        module: module,
                        node: document.getElementById(module.$uuid)
                    })
                }
            }
            for (let position of this.modulePositions) {
                position.offsetTop = position.node.offsetTop - 5
            }
        }
    }
    searchPosition(x: number, low: number, high: number): SitemapModule {
        ...
        if (x > this.modulePositions[mid].offsetTop) {
            let next = mid + 1;
            if (next >= this.modulePositions.length) {
                return this.modulePositions[this.modulePositions.length - 1].module;
            }
    ...
    onModuleBodyScroll = () => {
        if (this.modulesBody) {
            ...
            let mod = this.searchPosition(this.modulesBody.scrollTop + (this.modulesBody.clientHeight * 0.33), 0, this.modulePositions.length - 1);
    ...
    componentWillUnmount() {
        clearTimeout(this.timeout);
        this.modulePositions = this.modulesBody = this._selectedModule = this.search = this.searchResults = null;
    }
    ...
    onSearch = (text: string) => {
        ...
        if (this.searchResults.length > 0) {
            ...
            Dom.scrollTo(this.search.searchCurrentItem.uuid);
        }
        this.setState({});
    }
    ...
    rollUp(): Promise<void> {
        return this.page.rollUp().then(() => {
            this.onClearSearch();
            Dom.scrollTo(this.selectedModule.$uuid);
        });
    }

    rollDown(): Promise<void> {
        return this.page.rollDown();
    }
