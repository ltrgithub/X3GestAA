...
export class SitemapPageController extends React.Component<IProps, {}>{
    ...
    sitemapSearch: ISitemapSearch;
    _sitemapSearchResults: ISitemapSearchResult[];
    ...
    componentDidUpdate() {
        this.calculatePosition();
        (this._sitemapSearchResults.length > 0) && this.hightlightCurrentSearchResult(true);
    }
    calculatePosition() {
    ...
    selectModule(mod: SitemapModule, scrollTo?: boolean) {
        ...
        if (scrollTo) {
            clearTimeout(this.timeout);
            this.timeout = window.setTimeout(() => Dom.scrollTo(this.selectedModule.$uuid), 100);
        }
    }
    ...
    traverseModules(sitemapObject: any, objType: string) {
        for (let obj of sitemapObject) {
            ...
            let matches = obj.title.match(new RegExp(this.sitemapSearch.searchFilter, "gi"));
            if (matches && matches.length > 0) {
                ...
                for (let i in matches) {
                    ...
                    objType === 'modules' && this._sitemapSearchResults.push({
                        uuid: obj.$uuid + "_bar",
                        occurence: parseInt(i) + 1
                    });

                    this._sitemapSearchResults.push({
                        uuid: obj.$uuid,
                        occurence: parseInt(i) + 1
                    });
                }
    ...
    expandParentsUpTree(obj: any) {
        if (obj.navParent) {
            let parent = obj.navParent;
            parent.collapsed && (parent.collapsed = false);
            this.expandParentsUpTree(parent);
        }
    }

    traverseModules2(sitemapObject: any, objType: string) {
        for (let obj of sitemapObject) {
            //This regexp will find all occurences of string in the title
            let matches = obj.title.match(new RegExp(this.sitemapSearch.searchFilter, "gi"));
            ...
            obj.submodules && this.traverseModules2(obj.submodules, 'submodules');
            obj.children && this.traverseModules2(obj.children, 'children');
        }
    }


    clearSearchResults(text?: string) {
        this._sitemapSearchResults = [];
        this.sitemapSearch = {
    ...
    onSearch = (text: string) => {
        ...
        if (this._sitemapSearchResults.length > 0) {
            this.sitemapSearch.searchCurrentIndex = 0;
            this.sitemapSearch.searchCurrentItem = this._sitemapSearchResults[this.sitemapSearch.searchCurrentIndex];
            this.sitemapSearch.searchTotalResults = this._sitemapSearchResults.length;
            Dom.scrollTo(this.sitemapSearch.searchCurrentItem.uuid);
        }
        this.setState({});
    }
    ...
    hightlightCurrentSearchResult(isFocused: boolean) {
        let node = this._sitemapSearchResults[this.sitemapSearch.searchCurrentIndex];
        let nodes = document.getElementById(node.uuid).getElementsByTagName("span");
    ...
    onSearchResultIterate = (direction: string): number => {
        if (this._sitemapSearchResults.length > 0) {
            ...
            if (direction === 'next') {
                if (++this.sitemapSearch.searchCurrentIndex >= this._sitemapSearchResults.length)
                    this.sitemapSearch.searchCurrentIndex = 0; //loop to first element
            } else {
                if (--this.sitemapSearch.searchCurrentIndex < 0)
                    this.sitemapSearch.searchCurrentIndex = this._sitemapSearchResults.length - 1; //loop to last element
            }
            this.sitemapSearch.searchCurrentItem = this._sitemapSearchResults[this.sitemapSearch.searchCurrentIndex];
            Dom.scrollTo(this.sitemapSearch.searchCurrentItem.uuid);
            this.hightlightCurrentSearchResult(true);
        }
        return this.sitemapSearch.searchCurrentIndex;
    }
    ...
    render() {
        return <SitemapPage
            ...
            searchResult={this.sitemapSearch}
        />;
    }
}