...
export class SitemapPageController extends React.Component<IProps, {}>{
    ...
    sitemapSearch: ISitemapSearchResult;
    ...
    traverseModules(sitemapObject: any) {
        for (let obj of sitemapObject) {
            let matches = obj.title.match(new RegExp(this.sitemapSearch.searchFilter, "gi"));

            if( matches && matches.length > 0) {
            // if(obj.title.search(new RegExp(this.sitemapSearch.searchFilter, "i")) >= 0) {
                for(let i in matches){
                    this.sitemapSearch.foundItems.push({
                        uuid: obj.$uuid,
                        occurence: i+1});
                }
                // this.sitemapSearch.foundItems.push(obj.$uuid);
            }
            obj.submodules && this.traverseModules(obj.submodules);
            obj.children && this.traverseModules(obj.children);
        }
    }
    ...
    onSearch = (text: string) => {
        ...
        this.traverseModules(this.props.profile.sitemap.modules);
        console.log("Finished searching:" + this.sitemapSearch.foundItems.length + " found");
        console.log(JSON.stringify(this.sitemapSearch.foundItems));
        this.setState({});
    }
    
    onSearchResultIterate = (direction: string) => {
        if(this.sitemapSearch &&this.sitemapSearch.foundItems){
            if(direction === 'next'){
                // this.sitemapSearch.currentHighlightedItem++;
            }
            // this.sitemapSearch.currentHighlighteditem = (direction)
        }
    }
