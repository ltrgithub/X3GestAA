...
class SyraPageModal implements SyraModal.ISyraModal {
    ...
    constructor(readonly page: any, public props: IPageOptions, public openerBox: any) {
        this.root = overHelper.createOverRoot("s_modal_root");
        page.inModal = true;
        page.overPanel = this;
    ...
    onDidMount = (modalHost: PageModal) => {
        (this.page.layoutSlot = modalHost.body).appendChild(this.page.domItem);
        this.page.modalHost = modalHost;
    ...
    onResize = (modal: PageModal) => {
        if (this.page) {
            let height = modal.maxBodyHeight + "px";
            if (modal.resized) {
                this.page.domItem.style.height = height;
                this.page.contentSlot.style.height = height;
                this.page.domItem.style.maxHeight = "";
                this.page.contentSlot.style.maxHeight = "";
            }
            else {
                this.page.domItem.style.maxHeight = height;
                if (browser.isMSIE) {
                    this.page.domItem.style.height = (this.page.domItem.clientHeight > modal.maxBodyHeight) ? height : "";
                }
                this.page.contentSlot.style.maxHeight = height;
            }
            if (this.page.isLargeClassicPage) {
                let css = modal.dialog.className;
                if (css.indexOf("s_modal_dialog_classic_XXL") < 0) {
                    modal.dialog.className = css + " s_modal_dialog_classic_XXL";
                }
            }
        }
    }
    onResizeStop = () => {
        this.page && this.page.resizeItem(true);
    }
    onOk = () => {
        if ((this.props.onValidate && this.props.onValidate(this.page, this) !== false)
            || this.props.onServerSaved) {
            if (this.props.onServerSaved && this.props.closeOnOk !== true) {
    ...
    dispose() {
        ...
        if (this.props) {
            if (this.page) {
                this.page.modalHost = null;
                delete this.page.overPanel;
                this.page.dispose();
            }
        }
        this.root = overHelper.removeOverRoot(this.root);
        this.props && this.props.onDisposed && this.props.onDisposed();
        this.props = null;
    }
} 