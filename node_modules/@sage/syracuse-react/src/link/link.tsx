...
export interface ILinkProp {
	...
	highlightTextOccurence?: number,
	highlightTextFocused?: boolean,
...
export class Link extends React.Component<ILinkProp, ILinkState> {
	...
	renderTitle2() {
		...
		if (this.props.highlightText) {
			...
			let occurences = 0;
			if (parts) {
				let className = "s_link_highlight_text";
				let field = [];
				for (let i in parts){
					if(parts[i].toLowerCase() === this.props.highlightText.toLowerCase()){
						occurences++;
						if(this.props.highlightTextFocused && this.props.highlightTextOccurence == occurences) {
							className +=" focus";
						}
						field.push(<span key={this.props.id + i} className={className}>{parts[i]}</span>);
					} else {
						field.push(parts[i]);
					}
				}
	...
	renderTitle() {
		...
		if (this.props.highlightText) {
			...
			if (parts) {
				...
				for (let i in parts){
					if(parts[i].toLowerCase() === this.props.highlightText.toLowerCase()){
						let className = this.props.highlightClassName || "s_link_highlight_text";
						field.push(<span key={this.props.id + i} className={className}>{parts[i]}</span>);
					} else {
						field.push(parts[i]);
					}
