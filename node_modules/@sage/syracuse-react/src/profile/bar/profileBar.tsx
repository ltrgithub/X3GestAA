...
export class ProfileBar extends React.Component<IProfileBarProps, {
	...
	componentDidMount() {
		if (!this.keySpot) {
			...
			this.keySpot.bind(keyCommands.goToSitemap, () => { this.onSitemapLinkClick(); return true; });
	...
	renderSearch() {
		if (this.props.searchFieldVisible) {
			return <SearchField onHidden={this.onSearchFieldHidden} value={this.state && this.state.searchValue} />
		}
		else {
			return <Link
	...
	onSitemapLinkClick = () => {
		this.props.onToggleSitemap(this.onSitemapLinkClick);
		this.setState({
	...
	render() {
		...
		return <header className="s_profile_bar" onClick={this.props.onHeaderClick}>
			<ul className="s_profile_bar_bar">
				...
				<SitemapLink opener={true} profile={this.props.profile} opened={this.state.sitemapOpened} onClick={this.onSitemapLinkClick} />
				<li>{this.renderSearch()}</li>
				{this.state.sitemapOpened && <SitemapLink profile={this.props.profile} opened={this.state.sitemapOpened} onClick={this.onSitemapLinkClick} />}
			</ul>
		</header>;
	}
}