...
import { BookmarkHeader } from '../../navigation/bookmark/bookmarkHeader';
...
export class ProfileBar extends React.Component<IProfileBarProps, {
	...
	constructor(props: IProfileBarProps, context: any) {
		super(props, context);
		const showBookmarkHeader = (Storage.local.get("show_bookmark_header")!==undefined)
			? this.toBoolean(Storage.local.get("show_bookmark_header")) : false;
		this.state = {showBookmarkHeader};
        this.toggleBookmarkHeader = this.toggleBookmarkHeader.bind(this);
	}
	...
	toggleBookmarkHeader() {
		this.setState({showBookmarkHeader: !this.state.showBookmarkHeader}, ()=>{
            Storage.local.set("show_bookmark_header", this.state.showBookmarkHeader.toString());
        })
	}
	renderSearch() {
	...
	render() {
		...
		return <header className="s_profile_bar" onClick={this.props.onHeaderClick}>
			<BookmarkHeader show={this.state.showBookmarkHeader} bookmarks={profile.bookmarks} onBookmarkClick={this.props.onBookmarkClick} />
			<ul className="s_profile_bar_bar">
				...
				<li><BookmarkBar
					...
					toggleBookmarkHeader={this.toggleBookmarkHeader}
					showBookmarkHeader={this.state.showBookmarkHeader}
				/></li>
				...
				{this.props.sitemapOpened && <SitemapLink profile={this.props.profile} opened={this.props.sitemapOpened} onClick={this.props.onToggleSitemap} />}
			</ul>
		</header>;
	}
}