...
export function getRoles(profile: Profile, onLoaded: (roles: IRole[]) => void) {
	ajax && ajax.dispose();
	ajax = new Ajax();
	ajax.get({
		lock: true,
		url: profile.roleLookupUrl + "&count=500",
			...
			if (response.ok) {
				onLoaded(response.data.$resources);
			}
			else {
...
export function getEndpoints(profile: Profile, onLoaded: (endpopints: IEndpoint[]) => void) {
	ajax && ajax.dispose();
	ajax = new Ajax();
	ajax.get({
		lock: true,
