...
export function show(message: any, target?: any, severity?: string, noViewer?: boolean, $links?: any, autoHide?: boolean) {
    if (message !== undefined) {
        ...
        if (page && page.alertPanel && !page.disposed && page != syra_site && page != syra_site.userProfile) {
            message.noViewer = noViewer; //set by field.invalidateField => fusion/core/client/sap/window setFieldError
            if($links){
            ...
            message.autoHide = autoHide; //set by field.invalidateField => fusion/core/client/sap/window setStatusBar             
            page.alertPanel.show(message);
        } else
            message.$diagnoses && modal({
                $diagnoses: message.$diagnoses,
                onClose: _reloadPage(message.$diagnoses) ? ((evt: any) => { 
                    location.reload() ;
                }) : undefined
            });
    }
...
export function dispose() {
    //appeller sur onUncaughtError de la sheetpage pour libérer la messageBox
}

/**
 * Check if alert is raised because of disconnect and demands a relogin
 */
function _reloadPage($diagnoses: any) {
	let $links = $diagnoses && $diagnoses[0] && $diagnoses[0].$links; 
