...
const _parameters = {
    openModal: function (article: any, values: any, $parameters: any, record: any, callback: any) {
        ...
        if (!$parameters.$url) {
            ...
            // https://jira-sage.valiantyscloud.net/browse/X3-49107
            // $parameters in link used to render a query/detail facet
            // $parameters": {
            //   "$url": "{refUrl}/ASYRREQ?representation=ASYRREQ.$query&orderBy=NUMREQ desc",
            //   "$method": "GET",
            //   "$properties": {}
            // }
            // We can never save or validate in read only (GET) facets, so remove properties here
            if (props.method === "GET") {
                delete props.onValidate;
                delete props.onServerSaved;
            }
...
function _formatMenuUrlWithCallback(article: any, $menu: any, record: any, callback: any) {
    if ($menu.$parameters) {
        ...
        callback(parseUrl(article, $menu, record));
    }
}
...
export function formatMenuUrl(article: any, $menu: any, record?: any, callback?: any) {
    if (callback) {
        ...
        return parseUrl(article, $menu, record, $menu.$parameters && formatParameters(article, $menu.$parameters, record));
    }
}

function parseUrl(article: any, $menu: any, record: any, properties?: any) {
    if ($menu.$subRecordKey && $menu.$item && $menu.$item.$variantItemKey) {
        record = record || article.dataset;
        if (record) {
            record = record[$menu.$subRecordKey];
            record = record && record[$menu.$item.$variantItemKey];
        }
    }
