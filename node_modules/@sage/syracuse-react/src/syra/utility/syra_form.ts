...
class SyraForm {
    ...
    private _validateField(field: any, newValue: any, errors?: any) {
        ...
        if (errors.length > 0 || field.isInvalid) {
            this.invalidateField(field, errors);
                    ...
                    if (!field.$isHidden && field.$isEditMode) {
                        ...
                        if (!field.unknowMode) {
                            ...
                            try {
                                ...
                                isValidated = false;
                                this.invalidateField(field, error.message);
                            }
                        }
                    }
                }
    ...
    invalidateField(field: any, errors: any, noViewer?: boolean) {
        if (field.variantItem) {
            this.invalidateField(field.variantItem, errors, noViewer);
        } else {
            let diagnoses = [];
            errors = errors || [];
            ...
            SyraAlert.show(diagnoses, field, "error", noViewer);
