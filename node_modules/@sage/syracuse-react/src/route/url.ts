...
class EtnaProtocol {
    public id = "etna";
    parse(urlObj: Url): boolean {
        let m = /\/etna\/([^\/]*)\/([^\/]*)\/([^\/]*)\/?([^\/]*)\/?([^\/]*)/.exec(urlObj.url.split('?')[0]);
        if (m) {
            // functional module becomes contract
            //urlObj.endpoint =  'etna.' + m[2] + '.' + m[1];
            urlObj.endpoint = {
                application: this.id,
                contract: m[2],
                dataset: m[1]
            ...
            if (m[2][0] !== '$') {
                ...
                if (!m[4]) {
                    facet = '$query';
                } else if (/^\$(workingCopies)$/.test(m[4])) {
                    // working copy without id
                    facet = '$create';
                } else if (!m[5]) {
                    facet = '$details';
                } else if (/^\$(edit|create|lookup|summary)$/.test(m[5])) {
                    facet = m[5];
                } else if (/^\$(workingCopies)$/.test(m[5])) {
                    // working copy with id
                    facet = '$edit';
                }
                if (facet) urlObj.params.representation = m[3] + '.' + facet;
            }
            return true;
        }
    }
...
export class Url {
    ...
    constructor(url: string) {
        ...
        if (!protocol.sdata.parse(this)) {
            if(!protocol.classic.parse(this)){
                protocol.etna.parse(this);
            }
        }
