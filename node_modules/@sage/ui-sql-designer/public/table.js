...
let Scrollbar = require('@sage/syracuse-ui/lib/scroll/scrollbar').Scrollbar;
...
class Table {
    ...
    addField(f, data) {
        ...
        if (data.Indexes) {
            ...
            this.makeTableRowDblClickable(tr);
    ...
    makeTableRowDblClickable(tr) {
        let that = this;
        tr.addEventListener('dblclick', (ev) => {
            this.onDblClick(ev, that);
        }, false);
    }
    onDblClick(ev, that) {
        let req;
        let dfnItem = ev.currentTarget.getAttribute('id');
        let dflItem = $(ev.currentTarget).children('td').eq(2).text();
        let level = parseInt(that.syra_parent_class.getCurrentMaxLevel(), 10)
        let params = {
            dfnItem: dfnItem,
            dflItem: dflItem,
            level: level,
            sibling: null,
            index: that.syra_parent_class.dropfieldlist.length
        }
        req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.ADDFIELD}</AT><SK>${that.syra_parent_class.dropfieldlist.length + 1}</SK><P1>${dfnItem}</P1><P2>${level}</P2></ACT></LINES></INTERFL>`
    ...
    makeTableRowDroppable(tr) {
        ...
        $(tr).droppable({
            ...
            drop: function(event, ui) {
                if (that.isElementVisible(event.target).visible, that.isElementAboveSplitter(event.target)) {
    ...
    onClick(ev, that) {
        ...
        switch (ev.target.getAttribute('id')) {
            case 'icon-toolbar-close':
                ...
                if (that.syra_parent_class.getCountPerTable(this) === 0) {
                    req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.DELETETABLE}</AT><SK>${that.syra_id}</SK></ACT></LINES></INTERFL>`
                    that.syra_parent_class.makeAjaxCall(_base.requestTypes.DELETETABLE, null, {
                        'params': params
                    }, req);
                }
                break;
    ...
    isElementAboveSplitter(el) {
        var splitter = this.syra_parent_class.sqlsplitter;
        if (splitter !== null) {
            let elRect = el.getBoundingClientRect();
            let parRect = splitter.getBoundingClientRect();
            return elRect.top < parRect.top;
        }
        return false;
    }
    isElementVisible(el) {
