    ...
    makeGroupDroppable(sqlgroup) {
        ...
        $(sqlgroup).droppable({
            ...
            drop: function() {
                ...
                req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.MOVEFIELD}</AT><SK>${dfsIndex + 1}</SK><TK>${dftIndex + 1}</TK></ACT> `
        ...
        $(this.droppanelleft).droppable({
            ...
            drop: function(event, ui) {
                if (ui.draggable[0].tagName.toUpperCase() === 'TR') {
                    ...
                    req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.ADDFIELD}</AT><SK>${that.syra_parent_class.getDropFieldIndex(that.syra_id) + 1}</SK><P1>${dfnItem}</P1><P2>${that.level}</P2></ACT></LINES></INTERFL>`;
                    ...
                    if (dfs && dft && dfs.level !== dft.level) { //&& !that.onlyDropFieldOnLevel(dfs)) {
                        ...
                        that.syra_parent_class.createNewLevels(_base.changefieldType.MOVE, dfs, dfsIndex, dftIndex, dft.level);
                        req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.MOVEFIELD}</AT><SK>${dfsIndex + 1}</SK><TK>${dftIndex + 1}</TK></ACT> </LINES></INTERFL>`
                    }
                    that.syra_parent_class.makeAjaxCall(_base.requestTypes.MOVEFIELD, null, {
                        'params': params
                    }, req);
        ...
        $(this.droppanelright).droppable({
            ...
            drop: function(event, ui) {
                if (ui.draggable[0].tagName.toUpperCase() === 'TR') {
                    ...
                    req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.ADDFIELD}</AT><SK>${that.syra_parent_class.getDropFieldIndex(that.syra_id) + 2}</SK><P1>${dfnItem}</P1><P2>${that.level}</P2></ACT></LINES></INTERFL>`
                    that.syra_parent_class.makeAjaxCall(_base.requestTypes.ADDFIELD, null, {
                        'params': params
                    }, req);
                } else {
    ...
    onClick(ev, that) {
        ...
        switch (ev.target.getAttribute('id')) {
            ...
            case 'dropfieldboundary':
                ...
                req = `<INTERFL><TYPFL>fqd</TYPFL><TYPINTER>updateQuery</TYPINTER><LINES><ACT><AT>${_base.requestTypes.CHANGEFIELDPROPERTY}</AT><SK>${that.syra_parent_class.dropfieldlist.indexOf(that) + 1}</SK><P1>${_base.changeFieldProperty.BOUNDS}</P1><P2>${this.boundary === 'Y' ? 'N' : 'Y'}</P2></ACT></LINES></INTERFL>`
