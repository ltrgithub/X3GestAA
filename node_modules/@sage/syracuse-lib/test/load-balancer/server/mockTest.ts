...
import * as mocks from '../../../src/load-balancer/mock';
...
describe(module.id, () => {
	...
	it('mock server test with encoding and writeHead', function(_) {
		...
		var server = new mocks.Mock(toClient, toServer, function(request, response, _) {
			...
			try {
				response.writeHead(202);
				strictEqual(true, false, "No error after calling writeHead twice");
			} catch (e) {
				strictEqual(true, true, "error after calling writeHead twice");				
			}
			request.setEncoding('ucs2');
			request.on('data', function(chunk) {
				tracer && tracer("DATA" + chunk);
