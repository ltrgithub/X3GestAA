...
asyncTest("list snapshot test", 5, function(_) {
	...
	var userEntity = db.model.getEntity(_, "userTest");
...
asyncTest("safe call test", 46, function(_) {
	...
	var res = fut1(_);
	strictEqual(res, "field1_val", "Correct result for first short invocation")
	var res = fut2(_);
	strictEqual(res, "field1_val", "Correct result for second short invocation");
	var res = fut3(_);
	strictEqual(res, "field1_val2", "Correct result for third short invocation");
	var res = fut4(_);
	strictEqual(res, "field1_val", "Correct result for forth short invocation");
	strictEqual(instance.errors.length, 0, "No error in short invocations")

	var futures = [];
	for (var i = 0; i < 13; i++) {
		futures.push(_safeCall(!_, instance, "propName", fn_field1, "field", "val" + i));
	}
	var errorcase = "";
	for (var i = 0; i < 13; i++) {
		strictEqual(futures[i](_), "field1_val" + i, "Correct first result for first multiple execution");
	}
	strictEqual(typeof instance._safeCalls, "undefined", "temporary structures released");
