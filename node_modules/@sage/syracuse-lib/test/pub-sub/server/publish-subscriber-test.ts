...
describe(module.id, () => {
	...
	it('Test subscribe/unsubscribe', function(_) {	 		  	  
	  ...
	  c2.unSubscribe();
	  ok(broker.channels["discovery"] === undefined, "Zero clients in channel discovery, channel deleted");  	  
	  ok(broker.channels["bbc"].clients.length == 1, "One client in channel bbc");  	  
	  c3.unSubscribe();
	  ok(broker.channels["bbc"] === undefined, "Zero clients in channel bbc, channel deleted");
	});
});