...
config.security.signIn.rememberMe.lifetime = 4 * 7; // 4 weeks
...
describe(module.id, () => {
	function _browserCookie(ckHeader) {
		if (Array.isArray(ckHeader)) return ckHeader.map(ck => ck.split(";")[0]).join("; ");
		return ckHeader;
	}

	function extractCookie(ckHeader, name) {
		ckHeader = _browserCookie(ckHeader);
	...
	it('sessionManager - Authenticate with form', function(_) {
		...
		resp = response.readAll(_);
		var browserCookie = _browserCookie(response.headers["set-cookie"]);
		var loginCk = extractCookie(browserCookie, loginCookieKey);
		...
		var response = new ez.devices.http.client({
			...
			resp = response.readAll(_);
	var browserCookie2 = _browserCookie(response.headers["set-cookie"]);
	strictEqual(response.statusCode, 200, "user authenticated login cookie");
	var loginCk = extractCookie(browserCookie2, loginCookieKey);
	...
	config.security.signIn.rememberMe.lifetime = 0; // disable remember me

	// make a request with right token, should get 401, remember lifetime to 0
	var response = new ez.devices.http.client({
		url: baseUrl + "/sdata",
		headers: {
			cookie: browserCookie,
	...
	resp = response.readAll(_);
	var browserCookie2 = _browserCookie(response.headers["set-cookie"]);
	strictEqual(response.statusCode, 401, "token reuse fails because of remember me lifetime to 0 ok");

	config.security.signIn.rememberMe.lifetime = 1; // 1 day

	// make a request with right token, should get 401, because token has been deleted
	var response = new ez.devices.http.client({
		url: baseUrl + "/sdata",
		headers: {
			cookie: browserCookie,
	...
	resp = response.readAll(_);
	var browserCookie2 = _browserCookie(response.headers["set-cookie"]);
	strictEqual(response.statusCode, 401, "token reuse fails because token has been deleted ok");
	...
	// make a request with right token, should get 200
	var response = new ez.devices.http.client({
		url: baseUrl + "/sdata",
		headers: {
			cookie: browserCookie,
