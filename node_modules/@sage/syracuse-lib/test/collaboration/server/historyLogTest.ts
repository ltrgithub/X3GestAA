...
//------------------------
it("Init history log setting", function(_) {
...
//------------------------
/*
it("Delta serializer", function(_) {
...
//------------------------
it("User history inv relation ", function(_) {
	// Enable user traceability
	_enableTraceability(_, "users", true);
	var group = db.fetchInstance(_, db.model.getEntity(_, "group"), {jsonWhere:{"description":"Guests"}});
	ok(group, "Group guest found");
	var user = db.fetchInstance(_, db.model.getEntity(_, "user"), {jsonWhere:{"login":"import"}});
	ok(user, "User import found");
	group.users(_).set(_,user);
	group.save(_);	
