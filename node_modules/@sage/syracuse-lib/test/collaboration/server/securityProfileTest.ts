...
describe(module.id, () => {
	...
	function get(_, cookie, url, statusCode, fullResponse) {
		return testAdmin.get(_, cookie, url.indexOf("http") == 0 ? url : baseUrl + "/sdata/syracuse/collaboration/unit_test/" + url, statusCode, fullResponse);
	}

	function post(_, cookie, url, data, statusCode, fullResponse) {
		return testAdmin.post(_, cookie, url.indexOf("http") == 0 ? url : baseUrl + "/sdata/syracuse/collaboration/unit_test/" + url, data, statusCode, fullResponse);
	}

	function put(_, cookie, url, data, statusCode, fullResponse) {
		return testAdmin.put(_, cookie, url.indexOf("http") == 0 ? url : baseUrl + "/sdata/syracuse/collaboration/unit_test/" + url, data, statusCode, fullResponse);
	}

	function del(_, cookie, url, statusCode, fullResponse) {
		return testAdmin.del(_, cookie, url.indexOf("http") == 0 ? url : baseUrl + "/sdata/syracuse/collaboration/unit_test/" + url, statusCode, fullResponse);
	}
	...
	it("update restrictions tests", function(_) {
		...
		body = get(_, cookie, "users(login eq 'import')?representation=user.$details", 403);
		...
		// check group added
		body = get(_, cookie, "users(login eq 'import')?representation=user.$edit", 403);
		ok(!body.groups || body.groups.every(function(g) {
			return g.description !== "French CFO";
		}), "Group NOT added ok (2)");
		// I can modify my own firstName
	body = post(_, cookie, "users(login eq 'guest')/$workingCopies?representation=user.$edit&trackingId=1234567890", {}, 201);
	body = put(_, cookie, "$workingCopies('1234567890')?representation=user.$edit", {
		firstName: "New Guest Name",
	}, 200);
	body = get(_, cookie, "$workingCopies('1234567890')?representation=user.$edit", 200);
	strictEqual(body.firstName, "New Guest Name", "Changed own firstName Ok");
	// cannot set ctiId to a non-empty value 
	body = put(_, cookie, "$workingCopies('1234567890')?representation=user.$edit", {
		ctiId: "abc",
	}, 200);
	strictEqual(body.$properties.ctiId && body.$properties.ctiId.$diagnoses && body.$properties.ctiId.$diagnoses.length, 1, "One diagnose");
	strictEqual(body.$properties.ctiId.$diagnoses[0].$severity, "error", "Error diagnose");
	body = put(_, cookie, "$workingCopies('1234567890')?representation=user.$edit", {
		ctiId: "",
	}, 200);
	strictEqual(!!body.$properties.ctiId, false, "No ctiId in response, because change was ignored");
