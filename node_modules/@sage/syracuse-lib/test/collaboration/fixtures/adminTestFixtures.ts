...
export function createTestAdminEndpoint(_, datasetName, fctModel, testData) {
	...
	registry.register([adminEndpoint], true);
...
export function initializeTestEnvironnement(_, datasetName, port, noCompress) {
	...
	globals.context.session.getData = globals.context.session.getData || function(code) { return null; };
	globals.context.session.getSecurityProfile = globals.context.session.getSecurityProfile || function(_) { return null; };
	globals.context.session.getUserLogin = globals.context.session.getUserLogin || function(_) { return "admin"; };
	globals.context.session.getUserProfile = globals.context.session.getUserProfile || function(_) { return null; };
	//force basic auth
	config.session = config.session || {};
	config.session.auth = "basic";
	config.session.serviceAuth = null;
 	authHelpers.setup(config.session);
	config.session.ignoreStoreSession = false;
	if (!port) {
	...
	// make sure we restro apis
	apis.register('syracuse-x3', () => ({
		pool: require('syracuse-x3/lib/pool'),
	...
	//
	// avoid scheduled events
	require('syracuse-search/lib/elasticIndex').registerModelEvents = function(_, config) {
		// do nothing
	}
...
export function initializeTestContract(_, testData, dsName, keepDb) {
...
//	config.sdata.endpoints.push(endPoint);
	...
	return dataModel.getOrm(_, dataModel.make(endPoint.contract, "test"), endPoint.datasets[dsName]);
};