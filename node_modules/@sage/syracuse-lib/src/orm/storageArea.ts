...
/// 
export function getDocumentInstance(_, filter, volume, entityName) {
...
/// Returns a Document instance  
/// 
export function getDocumentInstanceByDD(_, dd) {
...
/// 
export function readAll(_, filter, entityName) {
...
/// Returns a Document entity.  
///  
export function writeAll(_, properties, buf, filter, volume, entityName) {
...
/// ``` javascript
/// var document = sa.writeStream(_, properties, writeFunction, filter, volume);  
/// ```
/// Update document in storage area if filter is valid, else Create it. 
/// 
/// * The `properties` parameter represents the document properties in JSON format :  
...
/// }
/// ```
/// * The `writeFunction` parameter represents a function(_, writableStream).  
/// * The `filter` parameter represents filter selector. It's used only for update.  
/// 
/// ``` javascript
/// var filter = {
...
/// Returns a Document entity.  
///  
export function writeStream(_, properties, filter, volume, entityName, writeFunction) {
	return new Helper(_, entityName).writeStream(_, properties, filter, volume, writeFunction)
};
/// -------------
/// ## remove function :
/// ``` javascript
...
///  
export function remove(_, filter, entityName) {
...
///  
export function open(_, filter, options, entityName) {
...
///  
export function write(_, ddUuid, properties, stream, forceSave) {
...
///  
export function read(_, ddUuid, chunkSize) {
...
/// Returns a Document entity.  
///  
export function close(_, ddUuid, withSave) {
...
///  
export function listDocuments(_, filter, serialize, expectedProperties, entityName) {
...
var Helper = helpers.defineClass(function(_, entityName) {
	...
	getDocumentInstance(_, filter, volume) {
		var document;
	...
	writeAll(_, props, buf, filter, volume) {
	...
	writeStream(_, props, filter, volume, writeFunction) {
		var document = this.getDocumentInstance(_, filter, volume);
		// Manage volume only for 'document' entity
		if (this.entityName === "document") {
			var docVolume = document.volume(_);
		...
		props = props && props.content != null ? props.content : {};
		var stream = document.content(_).createWritableStream(_, props); 

		if (stream) {
			writeFunction(_,stream);
		}
		document.save(_);
		checkExpirationDocument(_, document);
		ckeckDiagnoses(document);
	...
	write(_, ddUuid, props, stream, forceSave) {
	...
	listDocuments(_, filter, serialize, expected) {
