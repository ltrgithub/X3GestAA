...
import * as config from 'config'; // must be first
...
import { Factory } from '../../orm/factory';
...
import * as entityClasses from '../../orm/entities';
...
helpers.defineClass(X3Entity, null, {
	...
	getPrototype(_, reprName, facetName, device) {
...
helpers.defineClass(X3Model, null, {
	getRepresentation(_, entityName, facet) {
	...
	getEntity(_, entityName, facet, device) {
	...
	registerEvent(_, eventName, eventId, handler, entityName) {
...
helpers.defineClass(X3DbHandle, null, {
	...
	getClient(_) {
		var x3Api = apis.get('syracuse-x3');
		var factory = this._endpoint.useEtna(_) ? require('etna/lib/supervisor/client') : x3Api.pool;
	...
	postAction(_, operation, entity, facet, params, representation, options) {
	...
	fetchInstanceEtag(_, entity, params, etag) {
	...
	/// ```
	///
	createCursor(_, entity, params, bulkFacet) {
	...
	fetchInstances(_, entity, params) {
		var self = this;
	...
	getUserRights(_, actualRights, currentX3Login) {
	...
	batch(_, entity, resources, options) {
	...
	_service(_, verb, service, parameters, body) {
	...
	deleteService(_, service, parameters) {
		...
		// postService(_, service, parameters) {
