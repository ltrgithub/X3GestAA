...
function MongoDbHandle(model, dataset) {
	...
	self.connect = function (_) {
		...
		try {
			...
			syncTracer && syncTracer("Indexes updated in " + ((new Date()).getTime() - startTime) + " ms");
			//		
					...
					if (automaticImports.length > 0) {
						...
						while (automaticImports.length > 0) {
							...
							if (!importHandler.jsonImport(_, self, fname, importOptions)) {
								...
								syncTracer && syncTracer("Initialization file: " + fname + " imported in " + ((new Date()).getTime() - startTime) + " ms " + (globals.context.tenantId || ""));
							}
						}
			...
			var res = coll.update( query,
					{
					  $set: {
						sessionId: ssid,
						lockDate: new Date(),
						_expire: new Date((new Date()).getTime() + lockExpireDelai),
						lockUser: userLogin
					  }
				   },
