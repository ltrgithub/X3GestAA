...
function MongoDbHandle(model, dataset) {
	...
	self.connect = function(_) {
		...
		syncTracer && syncTracer(new Date().toISOString(), "Indexes updated in " + ((new Date()).getTime() - startTime) + " ms");
		//		
					...
					if (automaticImports.length > 0) {
						...
						while (automaticImports.length > 0) {
							...
							if (!importHandler.jsonImport(_, self, fname, importOptions)) {
								...
								syncTracer && syncTracer(new Date().toISOString(), "Initialization file: " + fname + " imported in " + ((new Date()).getTime() - startTime) + " ms " + (globals.context.tenantId || ""));
							}
						}
