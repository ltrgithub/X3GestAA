...
var lockWaitTimeout = (config.collaboration && config.collaboration.lockWaitTimeout) || 60000;
...
function MongoDbHandle(model, dataset) {
	...
	// add/remove expire index
	// assumes now that collection exists in MongoDB 
	// nocheck: only one query for listCollection is done outside (for performance reasons)
	self.ensureExpireIndex = function(_, entity, nocheck) {
		var db = self.db;
		if (!nocheck) { // expire index only on existing collection
	...
	function _expireEntities(_, model, db) {
		...
		if (entities) {
			// call listCollections only once to save time
			var listCollections = db.listCollections({}).toArray(_);
			var collectionNames = {};
			listCollections.forEach(function(c) {
				collectionNames[c.name] = null;
			})
			for (var name in entities) {
				if (entities[name].className in collectionNames) self.ensureExpireIndex(_, entities[name], true);
			}
		}
	}
	...
	self.lockDatabaseRetry = function(_) {
		var start = new Date();
		syncTracer && syncTracer("locking database...");
		var lock = self.lockDatabase(_);
		while ((lock.status !== "success") && (((new Date()) - start) < dbLockWaitTimeout)) {
			syncTracer && syncTracer("waiting for db unlock:", dbLockWaitTimeout - (((new Date()) - start));
			// wait some time, 50ms min
			setTimeout(_, Math.floor(Math.random() * 10000) + 50);
			//
		...
		syncTracer && syncTracer("database lock acquired after", ((new Date()) - start), "ms");
		return lock;
	};
