...
function MongoDbHandle(model, dataset) {
	...
	self.lockDatabase = function(_) {
		...
		try {
			...
			if (res.result.ok) {
				self._lockKeepAliveTimer = setInterval(function() {
					mongoTracer.debug && mongoTracer.debug("Updating database lock");
			...
			if (ex.code == 11000) { // MongoError: duplicate key violation
				...
				if (locks && locks[0]) {
					// dangerous, the lock will be removed even if the locking host still alive
					// we rely on expire index on lock collection now
					/*if (i < 1) { // maybe the lock can be removed 
						// lock expiration
	...
	self.unlockAll = function(_) {
		...
		if (this._lockKeepAliveTimer) clearInterval(this._lockKeepAliveTimer);
		this.db.collection("dbLocks", _).remove({}, {
			safe: true
		}, _);
		this._lockId = null;
