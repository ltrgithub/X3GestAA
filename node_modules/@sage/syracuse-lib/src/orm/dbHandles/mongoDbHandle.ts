...
import * as mongoHelper from '../../mongo-util/mongoHelper';
var base64;
import * as importHandler from '../../import/jsonImport';
...
var lockWaitTimeout = (config.collaboration && config.collaboration.lockWaitTimeout) || 20 * 60 * 1000;
var dbLockWaitTimeout = (config.collaboration && config.collaboration.dbLockWaitTimeout) || lockWaitTimeout;
var _funnels = {};

function _syncTracer(message?: any, ...optionalParams: any[]) {
	console.log(new Date().toISOString(), message, optionalParams.join(" "));
};
...
function getSelectFields(entity) {
	var fields = [];
...
function MongoDbHandle(model, dataset) {
	...
	function _ensureCollectionExpire(_, collection, action) {
		if (!collection) return;
		var indexes = collection.indexInformation({
			full: true
		}, _);
		var found = false;
	...
	self.ensureExpireIndex = function (_, entity) {
		...
		if (entity.$expire) {
			...
			if (listCollections.length > 0) {
				_ensureCollectionExpire(_, db.collection(entity.className, _), "create");
			}
		} else {
			// expire index only on existing collection
			var listCollections = db.listCollections({
			...
			if (listCollections.length > 0) {
				_ensureCollectionExpire(_, db.collection(entity.className, _), "drop");
			}
		}
	};
	...
	function _expireEntities(_, model, db) {
		...
		// particular case : dbLocks
		_ensureCollectionExpire(_, db.collection("dbLocks", _), "create");
	}
	...
	self.connect = function (_) {
		...
		mongoTracer.debug && mongoTracer.debug("Mongo connect to " + key);
		if (connectionPool && connectionPool[key]) {
			...
			*/
			mongoTracer.debug && mongoTracer.debug("Mongo got connection in pool for " + key);
			return;
		}
		// create a connection
		// mongoTracer.debug && mongoTracer.debug("Mongodb connect : " + host + ":" + port);
		var db = self.db = mongoHelper.connect(_, dataset, config.mongodb, globals.context.tenantId);
		...
		// crnit: not required anymore but ensure old locks are systematically removed
/*		if (process.argv[2] === "--dbUnlock" || process.argv[process.argv.length - 1] === "--dbUnlock") {
		...
		self.db.collection("dbLocks", _).remove({ _expire: null }, { safe: true }, _);
		//

		// lock database before detect if synchronisation is required. If the winner process crashes during sync then the 
		// next one can take over the sync
		...
		try {
			...
			//Create dbMessage collection for publish-subscriber
            db.createCollection("dbMessage", { "capped": true, "size": 100000, "max": 1000 }, _);

			// always update indexes
			_ensureIndexes(_, model, db);
			_expireEntities(_, model, db);
			syncTracer = _syncTracer;
			syncTracer && syncTracer("Indexes updated in " + (Date.now() - startTime) + " ms");
			//		
			...
			if (sync) {
				var sa; // load update scripts before database locking
				if (pm && pm.dbVersions) {
					sa = _getUpdateScripts(model);
					sa.forEach(function (us) {
						require(us.script);
					})
				}
				...
				// pm = _getModelParams(_); // don't read anymore, lock is moved before initial read
				if (pm && pm.dbVersions) {
					mongoTracer.debug && mongoTracer.debug("mongodbDbHandle.connect ModelName: " + model.name + "; found dbParam: " + sys.inspect(pm));
					sa = sa || _getUpdateScripts(model);
				...
				if (!(config.system && config.system.protectSettings)) {
					syncTracer && syncTracer("!!! EXECUTION OF THIS SCRIPT MAY TAKE UP TO 15 MINUTES, DON'T STOP YOUR SERVER !!!");
					...
					if (automaticImports.length > 0) {
						...
						while (automaticImports.length > 0) {
							...
							if (!importHandler.jsonImport(_, self, fname, importOptions)) {
								...
								syncTracer && syncTracer("Initialization file: " + fname + " imported in " + (Date.now() - startTime) + " ms " + (globals.context.tenantId || ""));
							}
						}
						...
						// transform automaticImport in plain text list
						var autoList = model.dbMeta.automaticImport.map(function (it) {
							return typeof it === "object" && it.import ? it.import : it;
						});
						Object.keys(pm.automaticImportEtags).forEach(function (item) {
							if (autoList.indexOf(item + ".json") < 0) {
			...
			// "always sync"" scripts
			var alwaysUpdate = model.dbMeta.alwaysSyncScript || [];
			alwaysUpdate.forEach_(_, function(_, scr) {
				console.log("Executing sync script", scr.script);
				require(scr.script).execute(_, self);
			});
		} finally {
			console.log("Database update done, unlocking database");
			self.unlockDatabase(_);
		}
	...
	function _ensureIndexes(_, model, db) {
		// create indexes
		var entities = model.getEntities();
		if (entities) //
		...
		// special collections
		db.collection("dbScheduledOperations", _).createIndex({
			"_uuid": 1
		}, {
				unique: false
			}, _);
		...
		if (parameters.orderBy && parameters.orderBy.length) {
			...
			if (entity.defaultOrder && entity.defaultOrder.length) {
				entity.defaultOrder.forEach(function (order) {
					var propName = order[0];
					if (propName[0] == "$") propName = "_" + propName.substring(1);
					if (entity.$properties && entity.$properties[propName] && entity.$properties[propName].$isLocalized) {
						sort.push([propName + "." + locale.current.toLowerCase(), (order[1] ? "ascending" : "descending")]);
						sort.push([propName + ".default", (order[1] ? "ascending" : "descending")]);
					} else
						sort.push([propName, (order[1] ? "ascending" : "descending")]);
				});
			} else {
	...
	self.createCursor = function (_, entity, params, shallow) {
		...
		options.fields = getSelectFields(entity);
		//
		var filter = _paramsToFilter(_, parameters, entity);
		//
	...
	self.hasLock = function (_, lockId) {
		try {
	...
	self.isSessionExpired = function (_, sessionId) {		
		if (sessionId.indexOf("-") > 0 && sessionId.length == 36) {
			// test whether sessionId still exists
			var collSession = this.db.collection("SessionInfo", _);
			if (collSession) {
				var existingSessions = collSession.count({
					sid: sessionId
				    }, {safe: true}, _);				
	...
	self.lock = function (_, lockId, lockExpireDelai, checkSession) {
		mongoTracer.debug && mongoTracer.debug("Mongodb locking " + lockId);		
		var session = globals.context.session;
		var userLogin = (session && session.getUserLogin(_)) || "internal";
		var ssid;
		var query = {_id:lockId};
		if (checkSession) 
		   ssid = (session && session.id) || processId;
		else 		   
		   ssid = processId;		  		
		 query.sessionId = ssid
		// try un upsert with _id=instance.$uuid and currect sessionId. If allready locked, returns count of 0

		var coll = this.db.collection("dbLocks", _);
		try {
			var res = coll.update( query,
					{
					  $set: {
						sessionId: ssid,
						lockDate: new Date(),
						_expire: new Date(Date.now() + lockExpireDelai),
						lockUser: userLogin
					  }
				   },
				   {safe: true,	upsert: true},_);

			if (res.result.ok) {
				self._lockKeepAliveTimer = self._lockKeepAliveTimer || [];
				self._lockKeepAliveTimer[lockId] = setInterval(function () {
					mongoTracer.debug && mongoTracer.debug("Updating lock " + lockId);
					// update lock expire date
					var res = coll.update(query, {
							$set: {
								_expire: new Date(Date.now() + lockExpireDelai)
							}
						 ...
						 function (err) {
							if (err) console.error("Error updating database lock", err);
						});
				}, 10 * 1000);				
				return {status: "success", id: lockId, sessionId:ssid};
			}
		} catch (ex) {				
			if (ex.code == 11000) { // MongoError: duplicate key violation
				...
				if (locks && locks[0]) {
					if (checkSession && self.isSessionExpired(_, locks[0].sessionId || "")) {
						self.unlock(_, lockId);
						return self.lock(_, lockId, lockExpireDelai, checkSession);
					}
	...
	self.unlock = function (_, lockId) {
		mongoTracer.debug && mongoTracer.debug("Unlocking " + lockId);		
		if (this._lockKeepAliveTimer && this._lockKeepAliveTimer[lockId]) clearInterval(this._lockKeepAliveTimer[lockId]);
		this.db.collection("dbLocks", _).remove({_id: lockId}, {safe: true}, _);
	};

	self.unlockAll = function (_) {
		syncTracer && syncTracer("Unlocking all mongoDB database and instance locks");
		mongoTracer.debug && mongoTracer.debug("Mongodb unlocking all database and instance locks");
		this._lockKeepAliveTimer && this._lockKeepAliveTimer.forEach_(_, timer => clearInterval(timer));
		this.db.collection("dbLocks", _).remove({}, {safe: true}, _);
	};

	// atomically create a database lock
	self.lockDatabase = function (_) {
		return self.lock(_, "database", 2 * 60 * 1000, false)
	};

	self.unlockDatabase = function (_) {
		syncTracer && syncTracer("Unlocking mongoDB database");
		return self.unlock(_, "database");
	};

	self.hasDatabaseLock = function (_) {
		return self.hasLock(_, "database");
	};
	...
	self.lockInstance = function (_, instance) {		
		if (instance && instance.$uuid)
			return self.lock(_, instance.$uuid, lockLifeTime, true);
		else
			return null;
	};

	self.unlockInstance = function (_, instance) {
		return self.unlock(_, instance.$uuid);
	};
	...
	self.lockInstanceRetry = function (_, instance) {
		...
		while ((lock.status !== "success") && (((new Date()) - start) < lockWaitTimeout)) {
	...
	self.lockDatabaseRetry = function (_) {
		...
		while ((lock.status !== "success") && (((new Date()) - start) < dbLockWaitTimeout)) {
			mongoTracer.info && mongoTracer.info("waiting for db unlock:", dbLockWaitTimeout - ((new Date()) - start));
			// wait some time, 1000ms min
			setTimeout(_, Math.floor(Math.random() * 10000) + 1000);
			//
...
export function create(_, model, dataset) {
...
//
export function setup(mongodbConfig, mongoConnPool) {
	// Do not take tracer from nodelocal config anymore
	//tracer = mongodbConfig && mongodbConfig.tracer;
	connectionPool = mongoConnPool;
	//
	if (mongodbConfig && mongodbConfig.logger) {
		if (mongodbConfig.logger.level) mongodb.Logger.setLevel(mongodbConfig.logger.level);
		if (mongodbConfig.logger.filter && typeof mongodbConfig.logger.filter === "object") {
			Object.keys(mongodbConfig.logger.filter).forEach(function(key) {
				mongodb.Logger.filter(key, mongodbConfig.logger.filter[key]);
			});
		}
	}
};
