...
export class EtnaOrmClient {
    ...
    constructor(config: EtnaConfig, dataset: string, userName: string) {
        this.config = config;
    ...
    etnaUrl(url: string) {
        return 'http://localhost:' + this.etnaPort + url;
    }
...
export function getClient(_: _, session: X3Session, endpoint: Endpoint) {
    ...
    const client = clients[dataset] || (clients[dataset] = new EtnaOrmClient(config, dataset, config.session.userName));
    client.start(_);
