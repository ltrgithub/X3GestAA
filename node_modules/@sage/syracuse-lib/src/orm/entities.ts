...
function _normalizeCapabilities(prop, facetName, isChild) {
	...
	if (prop.$capabilities == null) {		
		if (!prop.$compute && !(prop.relType == "reference" && !prop._$isChild) && (cap.indexOf("sort") < 0)) cap.push("sort");		
		if (facetName === "$edit") {
...
var BasePropClass = helpers.defineClass(BaseProperty, null, {
	getPropertyPrototype(_, stringRes, facetName, isChild, childPrefix, visited, options) {
		var self = this;
...
export const Property = helpers.defineClass(function(entity, meta) {
	var prop = meta;
...
export const Relation = helpers.defineClass(function(entity, meta) {
	var rel = meta;
	...
	getLink(facetName, variantName, params, title) {
		var self = this;
		switch (facetName) {
			case "$select":
...
function _checkDefaultOrder(entity, orders) {
	orders.forEach(function(order) {
		...
		var meta = entity.$showMeta ? entity.$showMeta.split(',') : [];	
		if (!entity.$properties[order[0]] && meta.indexOf(order[0])==-1) throw new Error(entity.name + ": invalid order property: " + order[0]);
...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix, visited, options) {
	...
	flows.eachKey(_, entity.$properties, function(_, name, prop) {
		...
		if (prop.$isRedactorFeature && !(((globals.context.config || {}).system || {}).enableRedactorFeatures)) return;
		//
...
export const Entity = helpers.defineClass(function(model, name, meta) {
	var self = this;
	var entity = meta;
	self._model = model;
	//
	...
	fillLinksResource(reprType, resource) {
		var contract = this._model.contract;
		var entity = this;
		// add standard links resources for the entity
		var stringRes = contract.resources && contract.resources();
	...
	getPrototype(_, reprName, facetName, isChild, childPrefix, visited, options) {
	...
	tick(_, db) { // atomically increases the tick and sets the local endpoint for the digest of this entity
		var entity = this;
	...
	/// }
	///
	getCounterValue(_, db, code, options) {
