...
import * as sdataAsync from '../sdata/sdataAsync';
import * as checksum from './checksum';
import * as perfmon from '../../src/perfmon/record';
...
function _securityError(request, code) {
	var method = request.method + (request.overriddenMethod ? " (override " + request.overriddenMethod + ")" : "");
	console.error("[SECURITY]", "[" + code + "]", method, request.url, request.headers);
	return new Error("Forbidden [" + code + "]");
}

function _safeParseRequestBody(_, request, payload) {
	if (payload) {
		// payload as a query parameter creates a security issue
		throw _securityError(request, "x-da531cc304d9");
	}
	var checkedHeader = request.overriddenMethod === "get" ? "accept" : "content-type";
	if (!(request && request.headers && /\bapplication\/json\b/.test(request.headers[checkedHeader]))) {
		throw _securityError(request, "x-03928c8394a4");
	}
	// if (payload) {
	// 	return jsurl.parse(payload);
	// }	
	var body = request.readAll(_);
	return request.overriddenMethod === "get" ? null : JSON.parse(body);
}


function _addRights(_, ent, params) {
	var tracer = config.factory.tracer; // || console.log;
...
function _getPropertiesResourceMeta(_, resource, instance) {
	...
	// security
	if (sp && !config.factory.showRights) {
...
var _propTypeFormaterMap = {
	graph(property, instance) {
		return {
			$url: _makeLazyUrl(instance.$url, property.name, "$graphs"),
			$type: property.$type
		};
	},
	"tag-cloud"(property, instance) {
		return {
...
function _internalSerialize(_, instance, shallow, sync, relation, options) {
	...
	while (--j >= 0) {
		...
		if (property.$type === "text/rtf") {
			resource[key] = rtfconvert2unicodertf.parse(resource[key]);
		}
	};
...
export function fetchFromUrl(_, url) {
	return _resolveDetailUrl(_, url);
};

// sets and replaces global UUID. Ensures that the global UUID is really unique.
export function setSyncUuid(_, context, replaceUuid) {
	// var data = _safeParseRequestBody(_, context.request);
...
function _getLocalizedProp(_, instance, name, property, localeCode) {
	...
	//
	var locCode = (localeCode || globals.context.sessionLocale || locale.current || 'en-US').toLowerCase();
...
function _applyDelta(_, context, instance, delta) {
	...
	// find out whether value has changed 
	function hasDifference(oldVal, newVal, key) {
		if (oldVal === newVal) return false;
		if (oldVal == null && newVal === "") return false;
		factoryTracer.debug && factoryTracer.debug("Difference found for " + key + ": old value " + oldVal + " new value " + newVal);
		return true;
	}
		...
		if (sp && !sp.canUpdateProperty(_, instance._meta.name, key)) {
			...
			if (r) {
				if (r.isPlural) { // plural relation
					...
					if (!value.some_(_, function(_, newElt, idx) {
							if (!newElt) return false; // nothing new
							if (newElt.$isDeleted) {
								return true; // deletion: always change
							}
							var index = newElt.$index; // index explicitly given?
							if (index === undefined) {
								// check whether instances have been deleted
								if (value.length != oldElts.length) {
									return true;
								}
								index = idx;
							}
							var oldElt = oldElts[index];
							if (!oldElt) {
								return true; // obviously new element has been added
							}
							// compare contents
							return Object.keys(newElt).some_(_, function(_, key2) {
								if (key2 === "$index" || key2 === "$serverIndex") return false; // $index is extra property
								return hasDifference(key2[0] === '$' ? oldElt[key2] : oldElt[key2](_), newElt[key2], key2);
							});
						})) return; // no change
				} else { // singular relation
					var relInst = instance[key](_);
					if (relInst == null && value == null) return; // no change
					if (relInst && value) {
						if (!Object.keys(value).some_(_, function(_, key2) {
								return hasDifference(relInst[key2](_), value[key2], key2);
							})) return; // no change						
					}
				}
			} else {
				// just a value
				if (!hasDifference(instance[key](_), value, key)) {
					return; // no change
				}
			}
...
function _safeCall(_, instance, propName, fn, field, val) {
	if (typeof(fn) != "function") {
		instance.$addError((instance._meta && instance._meta.name || "<undefined>") + "." + propName + " must be a function for " + field, propName);
		return;
	}
	...
	return flows.withContext(_ => propRef.funnel(_, (_) => {
		...
		try {
			...
			factoryTracer.error && factoryTracer.error("Error executing " + (fn && fn.name) + " for " + (instance._meta && instance._meta.name) + "." + propName, e);
		} finally {
...
export const unitTestSafeCall = _safeCall; // for unit tests
...
function _computeCollection(_, collection) {
	const relation = collection && collection._relMeta;
	if (relation && relation.$compute) {
		var coll = _safeCall(_, collection._parent, relation.name, relation.$compute, "_computing", collection) || collection;
		if (!(coll instanceof InstanceCollection)) throw new Error("Compute of '" + relation.name + "' must return a collection instance");
		collection.loaded = true;
	}
	else if (collection._parent && collection._parent.$uuid) {
		var filter = {};
...
_collProto.set = function(_, value, isInverse, variantType) {
	...
	// Set dirty map	
	this._parent.$dirtyProps[this._relName] = true;
	...
	if (!isInverse && invRelation) {
		...
		if (invRelation.isPlural) {
			...
			if (!invRelation.$isComputed || !this._parent.$importing)
			{
				...
				//Set snapshot on inverse !!!! should be the same
				current.$snapshot = current[this._relMeta.$inv](_)._parent.$snapshot; 								
			}
...
streamerProto.createWritableStream = function(_, options) {
	...
	// detect upload done
	if (self.property.$uploadDone || opt.uploadDone) {
		...
		stream.write = function(_, buf, enc) {
			...
			if (buf == undefined) {
				...
				self.property.$uploadDone && self.property.$uploadDone(_, self.instance);
				opt.uploadDone && opt.uploadDone(_, self.instance);
			}
			return res;
		};
	}
...
// Factory class
export const Factory = helpers.defineClass(function(entityMeta) {
	...
	forEachKey(entityMeta.$relations, function(name, relation) {
		if (relation.$compute) {
			if (typeof(relation.$compute) != "function") {
				throw new Error("$compute must be a function: " + entityMeta.name + "." + relation.name);	
			}
			_proto[name] = function(_, val, isInverse, variantName) {
				if (arguments.length === 1) {
					...
					if (relation.isPlural) {
						this._data[name] = this._data[name] || new InstanceCollection(this, name, relation);
						var coll = _safeCall(_, this, relation.name, relation.$compute, "_computing", this._data[name]) || this._data[name];
						if (!(coll instanceof InstanceCollection)) throw new Error("Compute of '" + relation.name + "' must return a collection instance");
						return coll;
					}
	...
	_proto.save = function(_, parameters, options) {
		...
		if (!hasErrors) try {
				...
				// cleanup snapshot
				// Do not delete snapshoot before fire afterSave event - used in traceability
				//this.$snapshot = null;
		...
		// fire after save
		// Delete snapshoot after fire the event - used in traceability

		if (!hasErrors) {
			_fireAfterSave(_, this, parameters);
			self.$created = false;
			if (this._snapshotEnabled && this.$snapshot) this.$snapshot = null;
		...
		if (expression.type === "operator") {
			...
			switch (expression.value.code) {
				...
				case "like":
					...
					var reg = new RegExp("^" + expression.children[1].value.replace(/%/g, ".*") + "$", "i");
					return (_evaluateChild(_, expression.children[0], instance) || "").search(reg) >= 0;
				case "between":
	...
	_proto.deleteSelf = function(_, options) {
		...
		if (!opt.ignoreRestrictions && self.$factory && !_canModifyFactoryInstance(_, self)) {
			config.factory.tracer && config.factory.tracer("factory.deleteFactoryInstance forbidden will reply 403");
		...
		try {
         ...
         self.$deleted = true;
		// Fire model event $afterDelete
		_runEvents(_, self._db.model && self._db.model.$events && self._db.model.$events.$afterDelete, self);
	...
	/// -------------
	/// ## Factory idDirty function :
	/// ``` javascript
	/// if(instance.isDirty(_, "code") doSomething();
	/// ```
	/// Returns true if the property is modified 
	///
	_proto.isDirty = function(propName) {
		if (!propName)
			return (Object.keys(this.$dirtyProps).length > 0);
		else if (this.$dirtyProps.hasOwnProperty(propName))
			return this.$dirtyProps[propName];
		else
			return false;
	};
	...
	/// Returns the created new element
	///
	factorySelf.createInstance = function(_, data, connection, context, initialInstanceId, importing) {
		var inst = new Instance(connection);
		if (importing) inst.$importing = importing;
...
export function createTemplate(_, context, factory) {
...
export function createWorkingCopy(_, context, factory) {
	var delta = _safeParseRequestBody(_, context.request, context.query.$payload);
...
export function getWorkingCopy(_, context, instance) {
	context.reply(_, 200, _serialize(_, instance));
};

export function updateWorkingCopyLocalizedProp(_, context, wcInstance, updInstance, propertyName, delta) {
	var newDelta = {};
...
export function updateWorkingCopy(_, context, instance, delta, opt) {
...
export function replyLocalizedProperty(_, context, instance, propertyName, readOnly) {
...
///
export function replyInstances(_, context, entity, parameters) {
...
///
export function fetchInstance(_, context) {
...
/// The context must provide the database abstraction, an entity and the instanceId
///
export function replyInstance(_, context, notFoundCallback) {
...
function _createInstance(_, context, factory, batchRequest) {
	factoryTracer.debug && factoryTracer.debug("factory.create instance " + factory._entityMeta.name);
	// rights management
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	if (sp) {
		factoryTracer.debug && factoryTracer.debug("filters._createWorkingCopy found security profile: " + sp.code(_));
	...
	//
	var delta = batchRequest ? batchRequest : _safeParseRequestBody(_, context.request, context.query.$payload);
	//
...
export function createInstance(_, context, factory) {
...
function _updateInstance(_, context, data, instance, opt) {
	...
	instance._sysSnapshotEnabled = true;
	instance.$updUser = (context && context.getUser(_) && context.getUser(_).login(_));
	var delta = data ? data : _safeParseRequestBody(_, context.request);
	var deltaApplied = _applyDelta(_, context, options.updInstance || instance, delta);
	// save
	try {
...
export function updateLocalizedProperty(_, context, instance, relInstance, propertyName, delta) {
	var newDelta = {};
	newDelta[propertyName] = ((delta && delta.$values) || []).reduce(function(prev, locVal) {
		prev[locVal.$locale.toLowerCase()] = locVal.$value;
		return prev;
...
export function updateInstance(_, context) {
...
export const deleteInstance = _deleteInstance;

// setup
export function setup(factoryConfig) {
	config.factory = factoryConfig || {};
};

export function batch(_, context) {
	try {
		var batchResult = {
			$resources: []
		};
		var brequest = _safeParseRequestBody(_, context.request);
