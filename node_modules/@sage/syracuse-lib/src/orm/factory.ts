...
function _computeCollection(_, collection) {
	...
	if (relation && relation.$compute) {
		_safeCall(_, collection._parent, relation.name, relation.$compute, "_computing", collection);
			...
			_proto[name] = function(_, val, isInverse, variantName) {
				if (arguments.length === 1) {
					...
					if (relation.isPlural) {
						...
						return _safeCall(_, this, relation.name, relation.$compute, "_computing", this._data[name]) || this._data[name];
					}
