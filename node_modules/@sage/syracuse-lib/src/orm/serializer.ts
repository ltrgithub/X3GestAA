...
export function getFactoryOwner(_, sp) {
...
helpers.defineClass(Serializer, null, {
	canSerializeProperty(_, instance, property) {
		return true;
	},
	serializeProperty(_, instance, property, resource) {

	},
	canSerializeRelation(_, instance, relation) {
		return true;
	},
	serializePlural(_, instance, relation, resource) {

	},
	serializeReference(_, instance, relation, resource) {

	},
	onSerialize(_, instance, resource) {

	},
	serialize(_, instance, options, withMeta) {
...
function _addTemplateValueFields(_, e, r) {
	if (e._meta.$valueTemplate && e._meta.$valueTemplate.matches) {
		e._meta.$valueTemplate.matches.forEach_(_, function(_, match) {
			...
			if (obj && obj[seg](_)) res[seg] = res[seg] || obj._data[seg];
		});
	}
}
...
function _addTypeFields(_,e,r,relation) {
...
export const DeltaSerializer = helpers.defineClass(function() {}, Serializer, {
...
// TODO: delta
export const SaveSerializer = helpers.defineClass(function(withDelta) {}, Serializer, {
	serializeProperty(_, instance, property, resource) {
		// TODO: delta
		if (property.$compute) return;
		var key = property.name;
	...
	serializePlural(_, instance, relation, resource) {
		...
		resource[key] = instance[key](_).toArray(_, true).map_(_, function(_, e) {
			...
			if (relation.$isDynamicType && !relation.getIsChild()) {
				...
				r = (relation.getIsChild(e.$variantType) || relation.$inlineStore) ? self.serialize(_, e) : {
					$uuid: e.$uuid
				};
				if (r && (relation.$isDynamicType || relation.$variants)) r.$type = e.getEntity(_).name;
				if (e.$variantType) r.$variantType = e.$variantType;
...
export const SignSerializer = helpers.defineClass(function() {}, SaveSerializer, {
	canSerializeProperty(_, instance, property) {
	...
	canSerializeResourceProperty(resource, entity, property) {
		if (property.isExternalStorage() || property.$compute) return false;
		var ent = entity;
	...
	canSerializeResourceRelation(resource, entity, relation) {
		if (relation.isComputed) return false;
		var ent = entity;
	...
	serializePluralResource(instance, relation, resource) {
		// plural relations are always fully stored
		if (relation.isComputed) return;
		//
		var key = relation.name;
	...
	serializeReferenceResource(instance, relation, resource) {
		// TODO: delta
		var self = this;
		var key = relation.name;
		resource[key] = null;
	...
	serializeResource(entity, instance) {
		if (!instance) return null;
...
export function serialize(_, instance, serializer, options) {
