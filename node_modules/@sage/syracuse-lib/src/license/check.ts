...
function getAdminHelper() {
	return require('../collaboration/helpers').AdminHelper;
}

export function updateFromDB(_) {
	try {
...
export function getParsedLicense(_) {
...
export function getFullLicense(_) {
	var data = _manageLicenseData(_);
	if (data) return data;
	return null;
}

export function deleteLicense(partnerId, productCode, productVersion, policyCode, policyVersion, diagnoses, _) {
...
export function getRawLicense(_) {
	var data = _manageLicenseData(_);
...
function _manageLicenseData(_, newLicenses, doNotStore, diagnoses, checkNamed, deleteProfile) {
	...
	if (!doNotStore && (firstTimeLicenseFile || result !== data)) { // change in licenses: update contents in database
		...
		var db;
		try {
			db = getAdminHelper().getCollaborationOrm(_);
		} catch (e) {
...
function _storeDB(licenses, _) {
	...
	try {
		//		db = db.open(_);
		var db = mongoHelper.connect(_, config.collaboration, config.mongodb, globals.context.tenantId);
...
export function _preParseLicenses(contents) {
	var policies = {};
	var licenses = {};
	var partnerContents = {};
	var licenseContents = {};
...
export const _ci = _checkISODate; // for unit tests
...
export const _p = _parseLicenses;
...
export function _getX3LicenseInfoInt(productCode, productVersion, badges, parsedData, diagnoses) {
...
export function getX3LicenseInfo(_, productCode, productVersion, session, diagnoses) {
...
export function getLicenseParameters(_, productCode) {
	// read license content
	var data = getParsedLicense(_);
...
function _checkNamedInt(_, parsed, inst, diagnoses) {
	...
	var db = getAdminHelper().getCollaborationOrm(_);
...
export function checkNamed(_, inst, messages) {
	var data = _manageLicenseData(_);
...
export function findUsedBadges(_, device, instance) {
	// read license content
	var data = getParsedLicense(_);
...
export function checkConcurrent(_, session, role, userName, device, diagnoses, newSession) {
...
export function propagateChange(_) {
...
export function _evaluateBadges(availableBadges, parsedData, badgeInfo, productInfo, concurrent, device, diagnoses, otherBadges) {
...
export const pS = _putServer; // !!!! TODO
...
export function licenseChange(content, diagnoses, _, extra) {
...
export function updateLicense(license, _) {
	...
	try {
		...
		require('../scheduler/scheduler').scheduleAll(_);
		return true;
	} catch (e) {
		return false;
	}
...
function _WSstatus(_) {
	...
	var orm = getAdminHelper().getCollaborationOrm(_);
...
export function step(_, length, poolMetadata) {
...
function _updateOldInfo(_, data, pd, timeString, key, productCode, test) {
	...
	for (var k in data.object) {
		...
		if (timeString.indexOf(stamp) !== 0) {
			...
			db = db || getAdminHelper().getCollaborationOrm(_);
...
//for unit tests!
export const _st = _step;
export const _nl = _clearWebserviceLicense;
