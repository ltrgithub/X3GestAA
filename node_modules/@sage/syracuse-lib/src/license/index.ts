...
if (!(config.system && config.system.enableDevelopmentFeatures)) {
	...
	var LIC_FILE = fsp.join(config.system.root, 'devLic/license.json');
	if (!fs.existsSync(LICENSE_FILE)) {
		config.noDevLic = true;
	} else console.log("Developer license");
}
if (config.noDevLic) {
	var LIC_FILE = (process.argv[2] === "PATCH" ? fsp.join(config.system.root, 'license.json') : fsp.join(config.system.root, 'temp/license.json'));
}
