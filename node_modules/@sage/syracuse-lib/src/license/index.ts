...
if (!(config.system && config.system.enableDevelopmentFeatures)) {
	...
	LIC_FILE = fsp.join(config.system.root, 'devLic/license.json');
...
/*export function getDatabase(tenantId) {
	...
	return new mongodb.Db((tenantId ? tenantId + "-" : "") + (config.collaboration.databaseName || "syracuse"), 
                         new mongodb.Server(config.collaboration.hostname || "localhost", config.collaboration.port || 27017, mongoOptions.server || {}), 
                         mongoOptions.db || { w: 1 });
};
*/

export function connectDatabase(callback, tenantId) {
	config.collaboration = config.collaboration || {};
	config.collaboration.driver = config.collaboration.driver || "mongodb";
	...
	var mongoUser = "";
	var mechanism = "";
	if (config.collaboration.mongoX509Cert) {
		var certdata = config.collaboration.mongoX509Cert;
		...
		var res = certtools.getData(name, cas);
	...
	var dbUrl = "mongodb://" + mongoUser+(config.collaboration.connectionString || (config.collaboration.hostname || "localhost") + ":" + (config.collaboration.port || 27017)) + "/" + ((tenantId ? tenantId + "-" : "") + (config.collaboration.databaseName || "syracuse"))+mechanism;
	mongodb.MongoClient.connect(dbUrl, opt, callback);
};
