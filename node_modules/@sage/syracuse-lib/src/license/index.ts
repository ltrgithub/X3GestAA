...
if (!(config.system && config.system.enableDevelopmentFeatures)) {
	...
	LIC_FILE = fs.readdirSync(fsp.join(config.system.root, 'devLic')).map(function(fileName) {
		return fsp.join(config.system.root, 'devLic', fileName);
...
if (config.noDevLic) {
	LIC_FILE = [(process.argv[2] === "PATCH" ? fsp.join(config.system.root, 'license.json') : fsp.join(config.system.root, 'temp/license.json'))].filter(function(path) {
		return fs.existsSync(path);
	});
}
export const LICENSE_FILE = LIC_FILE;
...
export function register(callback, options) {
	var l = exports.load("license");
	if (l) {
		if (LICENSE_FILE && LICENSE_FILE.length) { // read license from file
			var licenseData = LICENSE_FILE.map(function(file) {
				console.log("Reading license from file", file);
			...
			return exports.readLicenses(function(error, data) {
				...
				if (data && data.length > 0) {
					console.log("Reading license from database");
...
export function readLicenses(callback, tenantId) {
		...
		db.createCollection('license', function(err, collection) {
			...
			collection.find().toArray(function(err, docs) {
				...
				var result = docs.map(function(doc) {
					if (doc.text && doc.text.indexOf('"fileType": "License"') > 0) {
						if (config.licensetest) try {
							console.log("License from DB: " + doc.text.substring(doc.text.indexOf("partnerId") - 1, doc.text.indexOf("licensedTo") - 3).replace(/[\r\n\t]/g, ""));
						} catch (e) {
							console.log("Error reading license " + doc.text);
						}
					}
					return doc.text;
				});
	...
	}, config.collaboration, config.mongodb, tenantId);
};