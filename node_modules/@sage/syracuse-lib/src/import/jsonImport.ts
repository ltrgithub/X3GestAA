...
var tracer = require('@sage/syracuse-core').getTracer("imports");
...
// key: value of $key of entity prototype taken from $prototypes in JSON import data
// value: either value of instance (object) taken from $items in JSON import data or just the value corresponding to the key
// entity: metadata of entity taken from model
// instProto: entity prototype taken from $prototypes in JSON import data
// localization: value of $localization in JSON import data
// 	relProto: entity prototype taken from $prototypes in JSON import data - of related instance
// Remark: instProto is prototype of the object proprietary of the instance, not relation's proto
function _getFilter(_, db, key, value, entity, instProto, localization, relProto) {
	...
	if (Array.isArray(key)) {
		...
		key.forEach_(_, function(_, item) {
			if (Array.isArray(value[item])) {
				filter.jsonWhere[item] = value[item].map_(_, function(_, mapItem) {
					...
					if (instProto && instProto[item] && instProto[item].$key && entity && entity.$relations && entity.$relations[item] && entity.$relations[item].isPlural) {
						var mapFilter = {
							jsonWhere: {}
						};
						mapFilter.jsonWhere[instProto[item].$key] = mapItem;
		...
		if (entity.$properties && entity.$properties[simpleKey] && entity.$properties[simpleKey].$isLocalized && //
			typeof value[simpleKey] == "string" && instProto.$localized && (instProto.$localized.indexOf(simpleKey) >= 0)) {
		...
		parts = key.split(".");
		// console.log("yyyyyyyyyyyyyyyyyyyyyyyyPARTS", parts, instProto, relProto)
		if (relProto && parts.length > 1 && typeof value == "string" && relProto.$localized && (relProto.$localized.indexOf(parts[0]) >= 0)) {
...
//
// import plural relation
// inst: instance obtained by fetchInstance
// key: one of the keys of the instance taken from $items in JSON import data (this is name of relation in entity prototype from model)
// relMeta: metadata of entity for that relation, obtained from model
// relProto: part of prototype taken from $prototypes in JSON import data (which describes access to that related entity)
// value: value of the key of the instance (taken from $items in JSON import data)
// localization: value of $localization in JSON import data
// _pointerTypes: see description of _importInstance
// itemProto: entity prototype taken from $prototypes in JSON import data (entity of 'inst', not of related instance)
//proto: value of $prototypes in JSON import data
function _importCollection(_, inst, relName, relMeta, relProto, value, localization, _pointerTypes, options, itemProto, proto) {
		...
		if (relMeta.getIsChild(varType)) {
			...
			relKeys.forEach(function(relKeyItem) {
				...
				if (rp[relKeyItem]) { //relProto[parts[0]]
					//TODO relProto[relKeyItem].$key peut être aussi un tableau
			...
			// import
			_importInstance(_, relInst, relItemValue, rp, localization, _pointerTypes, options, proto);
			//
			...
			//references
			relInst = relItemValue ? db.fetchInstance(_, te, _getFilter(_, db, relKey, relItemValue, te, itemProto, localization, proto[te.name])) : null;
...
// import singular relation
// inst: instance obtained by fetchInstance
// key: one of the keys of the instance taken from $items in JSON import data (this is name of relation in entity prototype from model)
// varType: variant type for that key (obtained from metadata in model)
// relMeta: metadata of entity for that relation, obtained from model
// instProto: entity prototype taken from $prototypes in JSON import data (entity of 'inst', not of related instance)
// value: value of the key of the instance (taken from $items in JSON import data)
// localization: value of $localization in JSON import data
// _pointerTypes: see description of _importInstance
//proto: value of $prototypes in JSON import data
function _importReference(_, inst, relName, variantName, relMeta, instProto, value, localization, _pointerTypes, options, proto) {
	var key = relName;
	var db = inst._db;
	...
	if (relMeta.getIsChild(variantName) || relMeta.$inlineStore) {
		...
		if (!relInst) {
			...
			_importInstance(_, relInst, value, rp, localization, _pointerTypes, options, proto);
			inst[key](_, relInst);
		} else _importInstance(_, relInst, value, rp, localization, _pointerTypes, options, proto);
	} else {
		relInst = null;
		var te = relMeta.getTargetEntity(variantName);
		if (!te) {
			inst.$addDiagnose("error", locale.format(module, "cannotFindType", variantName));
		} else {
			if (value) {
				var filter = _getFilter(_, db, relKey, value, te, instProto, localization, proto[te.name]);
...
// imports a single instance (with related instances)
//
// instance: instance obtained by fetchInstance
// data: value of instance taken from $items in JSON import data
// itemProto: entity prototype taken from $prototypes in JSON import data
// localization: value of $localization in JSON import data
// _pointerTypes: object which maps the $id value of entity prototype (from $prototypes in JSON import data) 
//     to the corresponding entity prototype (from $prototypes in JSON import data)			
// proto: value of $prototypes of JSON import data
function _importInstance(_, instance, data, itemProto, localization, _pointerTypes, options, proto) {
	...
	try {
		flows.eachKey(_, data, function(_, key, value) {
			...
			if (entity.$relations && (relMeta = entity.$relations[key])) {
				...
				if (relMeta.isPlural) {
					_importCollection(_, inst, key, relMeta, relProto, value, localization, _pointerTypes, options, itemProto, proto);
				} else {
					var varType = relMeta.$variants && value && (value.$variantType || value.$type);
					_importReference(_, inst, key, varType, relMeta, itemProto, value, localization, _pointerTypes, options, proto);
				}
			} else //
...
// Options: importMode: insert, update instances
//          initialImport: mark initial import during server startup (e. g. for saving salt of user entity)
//          etc.
function _import(_, db, input, options) {
	...
	try {
		...
		data.some_(_, function(_, item) {
			...
			if (!inst) {
				// create
				inst = entity.factory.createInstance(_, null, db, null, null, true);
				created = true;
			}
			// update data
			_importInstance(_, inst, item, itemProto, localization, _pointerTypes, options, proto);
			...
			inst.$importing = true;
			if (options.initialImport) inst.$initialImporting = true;
			try {
				inst.save(_, null, {
				...
				inst.$importing = false;
				if (inst.$initialImporting) inst.$initialImporting = false;				
			}
			//
			var hasErrors = false;
			var diags = [];
...
export function importTest(_, filename, ifNoneMatch) {
...
export function jsonImport(_, db, filename, options) {
	//
	options = options || {};
	var fname;
	try {
...
export function streamImport(_, db, stream, options) {
	//
	options = options || {};
...
export function jsonImportFromJson(_, user, input, options) {
...
export function simplifyItems(_, user, input, options) {
	options = options || {};
	input = typeof input === "object" ? input : JSON.parse(input);
	var db = adminHelper.getCollaborationOrm(_);
	var items;
