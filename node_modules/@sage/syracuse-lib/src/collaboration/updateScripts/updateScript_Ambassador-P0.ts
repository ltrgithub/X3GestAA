...
import * as helperAdmin from '../../collaboration/helpers';
var _scripts = [];
...
_scripts[7] = function(_, db) {
	exports.tracer && exports.tracer("Executing update script to version: 7; dummy function because real function has been removed");
	/*	
	 * This has been inserted in Github #7134, but removed again in commit aed38e9 in Github #814
	 * I add it as dummy function so that database version is not higher than script version and therefore causes 
	 * always update.
	 * 
		exports.tracer && exports.tracer("Executing update script to version: 7; Fix security profile name and right");
		//
		...
		if (elts && elts.length > 0) {
			...
			elts.forEach_(_, function(_, e) {
				...
				e.profileItems && e.profileItems.forEach(function(item, idx, arr) {
					if (item.code === "maintenanceAndStatus") {
						arr[idx].code = "statusAndUsage";
					}
