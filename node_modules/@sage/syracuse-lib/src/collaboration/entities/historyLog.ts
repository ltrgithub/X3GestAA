...
function _propertyToText(_, instance, obj, path) {
	...
	if (_isCollection(obj)) {
		...
		for (var key in obj) {
			...
			if (prop && (prop instanceof entities.Property) && (obj[key].$value || obj[key].$prev)) {
				realName = prop.getTitle();
				v =  _getLocalizeValue(prop,obj[key].$value);
				p =  _getLocalizeValue(prop,obj[key].$prev);
...
function _getValueFromTemplate(_, instance, propName, obj) {
	...
	template.matches.forEach_(_, function(_, match) {
        ...
        val = _getLocalizeValue(prop,val); 
	...
	for (var key in delta) {					
		...
		if (prop && prop instanceof entities.Property && (delta[key].$value || delta[key].$prev)) {							
			result.push({
				...
				oldValue:  _getLocalizeValue(prop,delta[key].$prev),
				newValue:  _getLocalizeValue(prop,delta[key].$value),						
			});
