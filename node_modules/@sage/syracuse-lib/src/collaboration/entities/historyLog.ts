...
function _propertyToText(_, instance, obj, path) {
	...
	if (_isCollection(obj)) {
		for (var item in obj) {	
			var prop = _getProperty(_, instance.getLoggedEntity(_), path);
			if (!prop) return;
		...
		for (var key in obj) {
			var v, p;			
			var prop = _getProperty(_, instance.getLoggedEntity(_), path + '.' + key);
			if (prop && (prop instanceof entities.Property) && obj[key] && (obj[key].$value || obj[key].$prev)) {
				realName = prop.getTitle();
				v =  _getLocalizeValue(_,prop,obj[key].$value);
				p =  _getLocalizeValue(_,prop,obj[key].$prev);
				...
				result = Object.assign(result,_propertyToText(_, instance, obj[key], path + '.' + key));
			}
		}
...
function _getProperty(_, entity, propName, type) {	
	var oProp = propName;
	var nProp;    
	if (!entity) return;
...
function _getLocalizeValue(_,prop,data) {
	if (prop) {	
      ...
      if (prop.$isLocalized) {
	    ...
	    if (locCode && data && data.hasOwnProperty(locCode)) return data[locCode];
...
function _getValueFromTemplate(_, instance, propName, obj) {
	...
	var template;
	if ((prop) && (prop.$variants)) {	
		// TO DO !!!  use the type to have the right template	
		template =  prop.$variants[Object.keys(prop.$variants)[0]].targetEntity.$valueTemplate
        prop.$type = prop.$variants[Object.keys(prop.$variants)[0]].$type; 		
	}
	else if ((!prop) || (!prop.targetEntity)) { 
		var d = JSON.stringify(obj), null, 5);
		d = d.replace(/[\[\]\{\}\"]+/g, '').replace(/,\n|,$/g, '\n').replace(/^\s*\n/gm, '');
	    return d
	}
	 
	template = template || prop.targetEntity.$valueTemplate;
	...
	template.matches.forEach_(_, function(_, match) {
		...
		prop = _getProperty(_, instance.getLoggedEntity(_), propName + '.' + pName);		
		if (val && val.$value) 
		  val = _getLocalizeValue(_,prop,val.$value)		
		else if (val && val.$prev) 
		  val = _getLocalizeValue(_,prop,val.$prev)
        else 
		  val = _getLocalizeValue(_,prop,val); 
	...
	for (var key in delta) {					
		...
		if (prop && prop instanceof entities.Property && (delta[key].$value || delta[key].$prev)) {							
			result.push({
				...
				oldValue:  _getLocalizeValue(_,prop,delta[key].$prev),
				newValue:  _getLocalizeValue(_,prop,delta[key].$value),						
			});
