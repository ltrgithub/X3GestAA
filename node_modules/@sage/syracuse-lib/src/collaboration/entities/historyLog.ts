...
function _propertyToText(_, instance, obj, path) {
	...
	if (_isCollection(obj)) {
		for (var item in obj) {
			realName = _getProperty(_, instance.getLoggedEntity(_), path).getTitle();
		...
		for (var key in obj) {
			...
			if (prop && (prop instanceof entities.Property) && (obj[key].$value || obj[key].$prev)) {
				...
				result[key] = _propertyToText(_, instance, obj[key], key);
			}
		}
...
function _getValueFromTemplate(_, instance, propName, obj) {
	...
	template.matches.forEach_(_, function(_, match) {
		...
		if (obj[pName]) res = res.replace(match, _getLocalizeValue(prop,obj[pName]));
...
function _logHistory(_, instance, param) {
    ...
    console.log("log history",instance._meta.name);
