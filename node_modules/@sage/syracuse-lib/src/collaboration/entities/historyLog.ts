...
/**
 * Can return null during database initialization #122269 (needs endPoint collection)
 * Returns ORM according to end-point protocol (collaboration)
 */
function _getOrm(_){
	var ep = adminHelper.getCollaborationEndpoint(_);
	return ep ? ep.getOrm(_) : null;
}
...
function _allowLogHistory(_, instance) {
	...
	if (!entitiesToLog) {
		...
		var setting = db.fetchInstance(_, db.model.getEntity(_, "setting"), {});
		setting && setting.historyLogSettings(_).toArray(_)
