...
function _eraseLoginToken(_, instance) {
	var db = instance._db;
	var loginToken = db.fetchInstance(_, db.getEntity(_, "loginToken"), {
		jsonWhere: {
			login: instance.login(_)
		}
	});
	loginToken && loginToken.deleteSelf(_);
}

//finds out whether current user has the ability to change user data

export function noAdmin(_) {
...
export const entity = {
	...
	$services: {
		...
		*/
		...
		changeLogin: {
			...
			$isMethod: true,
			$facets: ["$details"],
			$isDefined(_, instance) { // factory users cannot change login
	...
	$functions: {
		$serialize(_, sync, shallow, relation, options) {
		...
		getSecurityLevel(_) {
			...
			grps.forEach_(_, function(_, grp) {
				...
				var spLevel = secProfile && secProfile.securityLevel(_);
					if (spLevel != null && spLevel < secLevel) secLevel = spLevel;
				}
			});
			return secLevel;
		},
		setSecurityLevel(_) {
			this.securityLevel(_, this.getSecurityLevel(_));
		},
allowRead(_) {			var sp = globals.context.session && globals.context.session.getSecurityProfile && globals.context.session.getSecurityProfile(_);
		...
		getUserEndpointsList(_, roleUuid) {
			var eps = [];
			var grps = this.groups(_).toArray(_);
			grps.forEach_(_, function(_, grp) {
		...
		getEndpointLogin(_, endpointUuid) {
		...
		getPersLocaleByCode(_, localeCode) {
		...
		getUserLocaleByCode(_, localeCode) {
		...
		getOAuth2Username(_, oauth2ServerId) {
		...
		$canSave: [
			...
			function(_, instance) {
				...
				if (!config.session || !config.session.authAlwaysLogin) {
					var auth1 = instance.authentication(_);
					var auth2 = authHelper.getStandardSetting(_).source || "";
					var auth = auth1 || auth2;
					var authCondition = 'active eq true and (authentication eq "' + auth + '"' + (auth === auth2 ? ' or authentication eq "")' : ')');
					if (instance.active(_) && (auth === "oauth2" || auth === "saml2" || auth === "sage-id")) {
						var useremail = instance.email(_) || instance.login(_);
						if (useremail.indexOf('@') >= 0) {
							var condition = authCondition + ' and (email like "' + useremail + '" or ((email eq null or email eq "") and (login eq "' + useremail + '"))) and $uuid ne "' + instance.$uuid + '"';
							var conflict = instance._db.fetchInstance(_, instance.getEntity(_), {
								sdataWhere: condition
							});
							if (conflict) instance.$addError(locale.format(module, "doubleEmail", conflict.login(_), useremail));
							return false;
						}
					} else if (instance.active(_) && instance.email(_)) { // for basic authentication there may be conflict when a user has an email address as login name
						var login = instance.login(_);
						var email = instance.email(_);
						var conflict = undefined; // conflicting user
						if (login !== email) { // is there a user whose login name is this email address?
							var condition = authCondition + ' and (login eq "' + email + '") and $uuid ne "' + instance.$uuid + '"';
							conflict = instance._db.fetchInstance(_, instance.getEntity(_), {
								sdataWhere: condition
							});
							if (conflict && conflict.email(_) === email) { // user with this email address as login name found and email occurs for 2 users (current instance and that user) 
								if (conflict) instance.$addError(locale.format(module, "userBlocked", email));
								return false;
							}
							// when user is just found but has different email address, there may be a third user with this email address: search below
						}
						if (login === email || conflict) { // search for other user with this email address: will block this user
							var condition = authCondition + ' and $uuid ne "' + instance.$uuid + '" and email eq "' + email + '"';
							conflict = instance._db.fetchInstance(_, instance.getEntity(_), {
								sdataWhere: condition
							});
							if (conflict) instance.$addError(locale.format(module, "doubleEmail", conflict.login(_), email));
							return false;
						}
					}
				}
	...
	$searchIndex: {
		$fields: ["login", "firstName", "lastName", "email", "groups", "adminTeams", "authorTeams", "memberTeams", "securityLevel"]
	},
	$defaultOrder: [
		["login", true]
	]
