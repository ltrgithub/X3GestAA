		...
		$canSave: [
			...
			function(_, instance) {
				...
				if (!config.session || !config.session.authAlwaysLogin) {
					...
					var authCondition = '(authentication eq "' + auth + '"' + (auth === auth2 ? ' or authentication eq "")' : ')');
					if (auth === "oauth2" || auth === "saml2" || auth === "sage-id") {
					...
					} else if (instance.email(_)) { // for basic authentication there may be conflict when a user has an email address as login name
