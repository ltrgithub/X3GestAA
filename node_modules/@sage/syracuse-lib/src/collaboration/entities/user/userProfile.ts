...
import * as depend from 'streamline-require/lib/server/depend';
...
export const entity = {
	...
	$properties: {
		...
		redactorMode: {
			$title: "Redactor mode active",
			$type: "boolean",
			$isRedactorFeature: true,
			$compute: function(_, instance) {
				return ((globals.context.config || {}).system || {}).enableRedactorFeatures;
    },
    clientSettings: {
			$title: "Specific Client Settings",
			$type: "json",
			$compute(_, instance) {
				return (globals.context.config || {}).clientSettings;
			}
		},
		mobileClientConfig: {
			$isHidden: true,
		...
		applicationConnectionData: {
			...
			$compute(_, instance) {
				tracer && tracer("userProfile.applicationConnectionData.enter");
				...
				if (userPrefs) {
					...
					if (selEp && appConn) {
						...
						tracer && tracer("userProfile.applicationConnectionData.exit1");
						return data;
					}
				}
				tracer && tracer("userProfile.applicationConnectionData.exit2");
				return;
			}
		}
	},
	...
	$functions: {
		...
		loadUserProfile(_, user, defaultLocale, paramUserProfile) {
			tracer && tracer("userProfile.loadUserProfile.enter");
			var self = this;
			var selRole;
			...
			tracer && tracer("userProfile.loadUserProfile.exit");
		},
		...
		getRepresentationPrefs(_, reprName, facetName) {
			var db = this._db;
		...
		createRepresentationPrefs(_, reprName, facetName) {
			var db = this._db;
		...
		$setParameters(_, context) {
			tracer && tracer("userProfile.$setParameters.enter");
			var self = this;
			// loads the current user profile
			this._initialize(_, context);
			tracer && tracer("userProfile.$setParameters.afterInitialize");
			// affect current user
			var user = context.getUser(_);
			tracer && tracer("userProfile.$setParameters.after context.getUser");
			if (!user) return;
			//
			...
			tracer && tracer("userProfile.$setParameters.exit");
		},
		$save(_, saveRes) {
			var self = this;
			...
			if (!oldRole || !newRole || (oldRole.$uuid !== newRole.$uuid && _badgeSet(_, oldRole) !== _badgeSet(_, newRole)) || (lastEndpoint && (!selEndpoint || lastEndpoint.dataset(_) !== selEndpoint.dataset(_)))) {
				if (selEndpoint) {
					...
					// check license rights for new endpoint
					if (selEndpoint.protocol(_) !== "syracuse") {
						var dataset = selEndpoint.dataset(_);
						this.$diagnoses = this.$diagnoses || [];
						if (adminHelper.hasProductDataByEndpoint(dataset) && !check.getX3LicenseInfo(_, adminHelper.getProductCodeByEndpoint(dataset), adminHelper.getProductVersionByEndpoint(dataset), globals.context.session, this.$diagnoses)) {
							if (userPrefs) {
								this.selectedRole(_, oldRole); // restore old role;
								if (lastEndpoint && (!selEndpoint || lastEndpoint.dataset(_) !== selEndpoint.dataset(_))) this.selectedEndpoint(_, lastEndpoint); // restore endpoint
							}
							return;
						}
					}
				...
				if (globals.context.request.session) {
					globals.context.request.session.closeReuse(_, "Endpoint/role change");
				}
			}
		...
		getAccessRightAuthorizations(_, endpoint) {
			var up = this;
		...
		$resetConnection(_, instance) {
			...
			if (selEp) {
				...
				if (globals.context.session.appConnection && globals.context.session.appConnection[dataset] && globals.context.session.appConnection[dataset].$actions && globals.context.session.appConnection[dataset].$actions.$resetConnection) {
					globals.context.session.closeReuse(_, "Application connection properties changed");
