...
import { ldapAuthentication } from '../../../auth/ldap';
...
export function getLdapAttributes(instance, _) {
...
export function updateUsers(_, instance, users, del, translateTable, _track) {
	...
	if (standardLdapServer && standardLdapServer.$uuid === instance.$uuid) {
		var db = adminHelper.getCollaborationOrm(_);
	...
	for (i = 0; i < instanceCount; i++) {
		...
		if (!newValues) { // LDAP entry does not exist any more
			if (del) {
				...
				user.active(_, false); // include X3 users
				user.save(_);
				...
				user.endpoints(_)
					...
					.forEach_(_, function(_, ue) {						
						if (_isX3EndPoint(_, ue.endpoint(_))) {
...
export const entity = {
	$properties: {
		...
		userAuthentication: {
			...
			$description: "Default user authentication for new users",
			$enum: [{
				$title: "LDAP",
				$value: "ldap"
			}, {
