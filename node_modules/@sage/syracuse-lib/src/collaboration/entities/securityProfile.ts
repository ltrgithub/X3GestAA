...
import { getFactoryOwner } from '../../../src/orm/serializer';
...
export const entity = {
	...
	$properties: {
		...
		allowOfficeDocumentUpload: {
			$title: "Allow Office document upload",
			$description: "Allow Office document upload. This option allow to protect against malicious Office documents.",
			$type: "boolean",
			$default: false
		},
		sageOwner: {
	...
	$events: {
		...
		//propagate security level to user (role -> group -> user)
		$afterSave: [

			function(_, instance) {
				if (instance.isDirty("securityLevel")) instance.setUserSecurityLevel(_);
			},
		]
	},
	$functions: {
		setUserSecurityLevel(_) {
			this.roles(_).toArray(_).forEach_(_, function(_, role) {
				role.setUserSecurityLevel(_);
			});
		},
		...
		canCreateClass(_, entityName, checkFilters) {
			var self = this;
		...
		canReadProperty(_, entityName, propName) {
		...
		canUpdateProperty(_, entityName, propName) {
			// ignore technical properties. We should check if propName is one of the properties or relations but we'll have 
			// a performance issue. So for now we just check for $
			if (propName && (propName[0] === "$")) return true;
			//
		...
		canExecuteService(_, entityName, operationName) {
			var self = this;
			if (!self._authByClass) self._authByClass = _buildAuthByClass(_, self);
		...
		canUpdateFactoryInstance(_, instance, ignoreInstance) {
		...
		canSaveSecLevel(_, instance) {
			...
			if (sp && sp.securityLevel(_) > secLevel) {
				var msg = locale.format(module, 'secLevel', sp && sp.securityLevel(_), secLevel);
