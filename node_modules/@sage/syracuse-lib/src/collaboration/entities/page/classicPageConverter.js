	...
	return function(proto, $article) {
		...
		function fixProperties(elt) {
			elt.$properties = Object.keys(elt.$properties).reduce(function(result, key) {
				...
				//New sql designer
				if (prop.$type == "application/x-shockwave-flash" && !(prop.$keyword)) {
					prop.$type = "application/x-sql-designer";
				}
...
export const makePersistent = fixPrototype('$X3Name', '$xid');
/// converts a prototype to the volatile form that we use in the client
export const makeVolatile = fixPrototype('$xid', '$X3Name');
...
export function convertLocalization($prototype, $article, options) {
	var opt = options || {};
...
export function getLocalizationNotFoundArray($article, $protoLocalization) {
