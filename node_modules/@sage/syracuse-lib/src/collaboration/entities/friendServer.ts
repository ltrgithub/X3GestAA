	...
	$functions: {
		createWritableStream: function(_, options) {
			...
			if (cert) {
				...
				var opt = {
					agent: false,
					key: fs.readFile(fsp.join(certPath, certName + ".key"), _),
					cert: fs.readFile(fsp.join(certPath, certName + ".crt"), _),
					// use our server ca, should be the same
					ca: fs.readFile(fsp.join(__dirname, "../../../syracuse-main/lib/ssl", "ca.crt"), _),
					//
					url: this.endpointUrl(_) + "/" + service,
					method: options.method || "POST"
				};
