...
import * as jsonExport from '../../../import/jsonExport';
...
export const entity = {
	...
	$functions: {
		...
		generateContent(_, options) {
			return this.exportPersonalizations(_, options);
		},
		exportPersonalizations(_, options) {
			var opt = options;
			var instance = this;
			//
			...
			if (instance.navPagesExport(_)) {
				...
				if (instance.navPageCleanupScript(_) && instance.navPageFilter(_)) {
					dItem._scripts = {
						...
						"$scriptsBefore": [{
							"module": "@sage/syracuse-lib/src/collaboration/advancedScripts/pre-import-sitemap",
							"options": {
						...
						"$scriptsAfter": [{
							"module": "@sage/syracuse-lib/src/collaboration/advancedScripts/post-import-sitemap",
							"options": {
								"homepagesFilter": instance.navPageFilter(_),
								"modulesFilter": instance.navPageModulesFilter(_),
								"submodulesFilter": instance.navPageSubmodulesFilter(_),
			...
			if (instance.homepagesExport(_)) {
                ...
                if (instance.homepageVignetteFilter(_)) {
					//add prefix for all words(fields) outside the quotes
					stdProfileItem.$relations.vignettes.$filter = instance.homepageVignetteFilter(_).replace(/\((?=(?:[^']*'[^']*')*[^']*$)/g,"(vignette.");
				}
				stdProfileItem.$relations.vignettes.$exportParentIfEmpty = false;
