...
export const entity = {
	...
	$functions: {
        ...
        getIndexName(_, localeCode, overrideValue?: string, noPrefix?: boolean) {
            let baseIndexName;

            if (localeCode) {
                localeCode = typeof(localeCode) === "string" ? localeCode.trim() : null;
            }
            ...
            if (!overrideValue) {
                ...
                baseIndexName = (parts.join(INDEX_NAME_SEPARATOR)).toLowerCase().replace(" ", "_");
            } else {
                baseIndexName = (overrideValue + (localeCode ? INDEX_NAME_SEPARATOR + localeCode : "")).toLowerCase().replace(" ", "_");
            }

            if (globals.context.tenantId && !noPrefix) {
                return globals.context.tenantId + INDEX_NAME_SEPARATOR + baseIndexName;
            }
            if (config.searchEngine && config.searchEngine.indexPrefix && !noPrefix) {
                return config.searchEngine.indexPrefix + INDEX_NAME_SEPARATOR + baseIndexName;
            }
