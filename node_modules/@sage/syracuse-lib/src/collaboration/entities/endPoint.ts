...
export const entity = {
	...
	$functions: {
		...
		importSyrDefFolder(_,tracker) {
		   ...
		   for (var i = 0; i < dir.length; i++) {
			   ...
			   if (tracker) { 			   
			   		tracker.phase = "Import file " + options.name;
			   		tracker.$diagnoses = (tracker.$diagnoses || []);
			   		tracker.$diagnoses.push({$severity: "info",$message: "Start importing file " + options.name});			   				
			   }
			   ...
			   if (tracker) {			   
			   		...
			   		tracker.progress = 100*(i+1)/dir.length;			   	
			   }
		   }
		}
	},
	$services: {		
		...
		importSyrDefFolder: {
			...
			$description: "Import default homepages, menus from X3 folder",
			$isMethod: true,
			$method: "POST",
			$invocationMode: "async",
			$execute(_, context, instance, parameters) {				
				 instance.importSyrDefFolder(_,context && context.tracker);				
			}
		},
	},
	$events: {
