...
export const entity = {
	...
	$functions: {
		...
		isSame(_, application, contract, dataset, host, port) {
			// TODO: host and port
		...
		getWebServerBaseUrl(_, withFolder, secure) {
			var x3solution = this.x3solution(_);
			if (x3solution)
		...
		getFusionPrototypeUrl(_, options) {
			var opt = options || {};
			var x3solution = this.x3solution(_);
			if (!x3solution)
				return null;
		...
		getFusionDataServerBaseUrl(_, secure) {
			var x3solution = this.x3solution(_);
		...
		getAuthorizedAccessRight(_, user, role) {
			var p = this.protocol(_);
			return p && _authRightSolverMap[p] && _authRightSolverMap[p](_, this, user, role);
		},
		getHelpBaseUrl(_, secure) {
			var hs = this.helpServer(_);
			var hsp = this.helpServerPort(_);
			return (secure ? "https://" : "http://") + hs + ":" + hsp;
		},
		getEtnaConfig(_, session) {
			const solution = this.x3solution(_);
			if (!solution) throw new Error('X3 solution not configured');
			const sqlService = solution.sqlService(_);
			if (!sqlService) throw new Error('SQL service not configured in X3 solution');

			const sqlUser = sqlService.getKind() === 'oracle' ? this.x3ServerFolder(_) : sqlService.user(_);
			const sqlPassword = sqlService.getKind() === 'oracle' 
				? this.etnaDatabasePassword(_) || sqlService.password(_)
				: sqlService.password(_);

			const mongoService = solution.mongoService(_);
			const mongoUrl = mongoService && mongoService.url(_);

			var adminEp = adminHelper.getCollaborationEndpoint(_);
			var etnaConfig = {
				serviceUrl: this.etnaServiceUrl(_),
				...
				solutionPath: solution.rootDir(_) || '/x3-solution',
				// TODO: remove etnaMongo properties
				mongo: {
					url: mongoUrl || `mongodb://${adminEp.databaseHost(_) || 'localhost'}:${adminEp.databasePort(_) || 27017}`,
					database: `ETNA-${this.dataset(_)}`,
				},
				sql: {
					driver: sqlService.getKind(),
					user: sqlUser,
					password: sqlPassword,
				}
			};
			if (config.scm)
				etnaConfig.scmDefinitionFilesFolder = config.scm.scmDefinitionFilesFolder;
			etnaConfig.scmDefinitionFilesFolder = etnaConfig.scmDefinitionFilesFolder || this.gitFolder(_) + '/definitionFiles';

			switch (etnaConfig.sql.driver) {
				case "oracle":
					etnaConfig.sql.database = sqlService.sid(_);
					etnaConfig.sql.port = sqlService.port(_);
					etnaConfig.sql.hostname = sqlService.host(_);
					break;
				case "sqlServer":
					etnaConfig.sql.database = sqlService.database(_);
					etnaConfig.sql.hostname = sqlService.instance(_);
					break;
				default:
					throw new Error("bad driver: " + etnaConfig.sql.driver);
			}

			//etnaConfig.mongo.port = etnaConfig.mongo.port || 27017;
			//etnaConfig.mongo.database = etnaConfig.solutionName + '-' + etnaConfig.folderName;
			if (etnaConfig.tenantId) // add tenant prefix to mongo config
				etnaConfig.mongo.database = etnaConfig.tenantId + "-" + etnaConfig.mongo.database;
			...
			if (session) {
				...
				etnaConfig.session = {
					...
					userName: user.getEndpointLogin(_, this.$uuid).toUpperCase(),
		...
		$onSerializeReference(_, result) {
			result.nature = this.nature(_);
		},
		importSyrDefFolder(_, tracker, progressParts, force) {

			tracker && (tracker.$diagnoses = (tracker.$diagnoses || []));
			var diagnoses = tracker ? tracker.$diagnoses : [];

			if (this.initialized(_) && !force) {
				diagnoses.push({
					$severity: "error",
					$message: locale.format(module, "errorInitializedEndPoint")
				});
				return diagnoses;
			}

			try {
				var client = new SadFsqClient(_, this, null, null, false);
				var options = { folder: this.x3ServerFolder(_), path: "SYRDEF" }
				var dir = client.readdir(_, options);
				var diag, mtime, content;

				if ((dir.length == 0) || (dir.join().indexOf("Error 20") != -1)) {
					diagnoses.push({
						$severity: "info",
						$message: locale.format(module, "noImportData")
					});
					return diagnoses;
				}
				...
				for (var i = 0; i < dir.length; i++) {
					var item = dir[i];
					options.name = path.basename(item);
					diagnoses.push({
						$severity: "info",
						$message: locale.format(module, "importFile", options.name)
					});
					if (tracker) {
						tracker.phase = locale.format(module, "importFile", options.name);

					}
					//mtime = client.stat(_, options,["mtime"]).mtime;
					content = client.readFile(_, options, { encoding: "utf-8" });

					globals.withContext(_ => { jsonImport.jsonImportFromJson(_, null, content, importOptions); },
						{ tenantId: globals.context.tenantId })(_);

					diagnoses.push.apply(diagnoses,importOptions.$diagnoses);
					if (tracker) {
						tracker.progress = tracker.progress + 100 / (dir.length * progressParts);
					}

				}
				if (!importOptions.$diagnoses.some(d => d.$severity == "error")) {
	...
	$services: {
		...
		importSyrDefFolder: {
			$title: "Personalizations and Menus initialization",
			$description: "Personalizations and Menus initialization from X3 folder",
			$isMethod: true,
			$method: "POST",
			$confirm: "This operation will reset the menus and personalisations with original factory values",
			$invocationMode: "async",
			$isDisabled(_, instance) {
				return (instance.protocol(_) !== "x3" || instance.initialized(_));
			},
			$execute(_, context, instance, parameters) {
				if (!parameters) parameters = context.parameters;
				var force = parameters && parameters.force;
				var diagnoses = instance.importSyrDefFolder(_, context && context.tracker, 1, force);
				if (!context || !context.tracker) instance.$diagnoses = diagnoses;
			}
		},
	},
	$events: {
