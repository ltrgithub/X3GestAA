...
function _getUrlSearchEngine() {
	// Would be better if this used getElasticBaseUrl() from syracuse-search/lib/helpers
	// but don't want to contaminate ts code here
	var searchConf = config.searchEngine || {};
	var baseUrl = (searchConf.secure ? "https://":"http://") + (searchConf.hostname || "localhost");
	if (!searchConf.excludePort) {
		baseUrl = baseUrl + ":" + (searchConf.port || 9200);
	}
	return baseUrl;
}
