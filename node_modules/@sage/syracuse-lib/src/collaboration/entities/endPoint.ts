...
export const entity = {
	...
	$functions: {
		...
		getEtnaConfig(_, session) {
			...
			var etnaConfig = {
				...
				mongo: {
					host: mongoService ? mongoService.host(_) : adminEp.databaseHost(_),
					port: mongoService ? mongoService.port(_) : adminEp.databasePort(_)
				},
				sql: {
		...
		importSyrDefFolder(_,tracker,progressParts) {
           ...
           if ((dir.length == 0) || (dir.join().indexOf("Error 20") != -1)) {
               tracker.$diagnoses.push({$severity: "info",
		   ...
		   for (var i = 0; i < dir.length; i++) {
			   ...
			   if (tracker) {			   
               		tracker.$diagnoses = tracker.$diagnoses.concat(importOptions.$diagnoses);  
	...
	$services: {		
		...
		importSyrDefFolder: {
			...
			$execute(_, context, instance, parameters) {			
			   instance.importSyrDefFolder(_,context && context.tracker,1);				
			}
		},
	},
	$events: {
