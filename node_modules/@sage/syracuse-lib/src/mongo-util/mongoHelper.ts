...
export function connect(callback, collaboration, mongoOptions, tenantId) {
	...
	// manage replicaset name and transfert old and now unsupported opt
	if (opt.rs_name) {
		opt.replicaSet = opt.rs_name;
		delete opt.rs_name;
	}
	
	if (opt.replicaSet) {
		params.push("replicaSet=" + encodeURIComponent(opt.replicaSet));
		delete opt.replicaSet;
	...
	if (collaboration.mongoX509Cert) {
		...
		if (typeof collaboration.mongoX509Cert === "boolean") {
			...
			if (!("sslValidate" in opt)) opt.sslValidate = true;
			opt.sslKey = res.key;
			opt.sslCert = res.cert;
			opt.sslCA = res.ca;
			dn = res.dn;
		}
