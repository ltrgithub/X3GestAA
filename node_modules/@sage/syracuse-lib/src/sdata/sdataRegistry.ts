...
import { registry } from '@sage/syracuse-core';
import * as adminHelper from '../collaboration/helpers';
...
export function register(endpoints, withReset) {
	endpoints.forEach(function(endpoint) {
		...
		if (withReset) {
			delete applications[endpoint.contract.application];
		}
...
export function getContract(application, contract, failIfNull) {
...
export function getDataset(_, contract, datasetName) {
...
export function loadAllEndpoints(_) {
	try {
	var app = adminHelper.AdminHelper.getCollaborationApplication(_);
	} catch (e) {
		console.log("Strange error", Object.keys(adminHelper));
	}
	if (!app) return;
...
function extend(contractRef, contractSec) {
	...
	function copy(src, dst) {
		Object.keys(src).forEach(function(name) {
			if (src[name].$extends) {
				...
				if (dst[name]) { // special handling of $beforeSave, $canSave, $afterSave events: append the arrays
					if (name === "$beforeSave" || name === "$canSave" || name === "$afterSave")
						src[name].forEach(ev => dst[name].push(ev))
					else
						throw new Error("cannot extend contract, entity already exists: " + name);
				} else
					dst[name] = src[name];
			}
		});
	...
	if (contractSec.dbMeta) {
		...
		if (contractSec.onContractLoad) contractSec.onContractLoad(contractRef);
	}
}
...
export function extendContract(contract) {
	function copy(src, dst) {
		Object.keys(src).forEach(function(name) {
			dst[name] = src[name];
		});
