...
import * as sdataRegistry from '../sdata/sdataRegistry';
import * as dataModel from '../../src/orm/dataModel';
import * as AdminHelpers from '../collaboration/helpers';
var adminUtil = AdminHelpers.adminHelper;
//import * as adminUtil from '../collaboration/util';
...
export const tracer = null; // console.log;
...
export const Context = class Context {
	constructor(request, response, options) {
		...
		if ((request.method && request.method.toLowerCase()) !== this.method) {
			request.overriddenMethod = request.method.toLowerCase();
	...
	reply(_, statusCode, result, headers) {
		...
		try {
			...
			console.error("Error during context reply "+e, e.stack, "Message "+result)
		}
	...
	getSerializeOptions() {
		...
		return {
			...
			getPluralDbHandles(_, dbHandle, relation) {
	...
	filterInstancesArray(_, instances, parameters) {
		...
		//
		var insts =  instances.filter_(_, function(_, instance) {
