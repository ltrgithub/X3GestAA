...
import * as registry from './sdataRegistry';
import { Context } from './sdataContext';
import * as dataModel from '../../src/orm/dataModel';
var find = helpers.object.find;
import * as factory from '../../src/orm/factory';
import * as searchEngine from 'syracuse-search/lib/elasticSearch';
...
import * as graph from 'syracuse-graph/lib/graph';
import * as tagCloud from 'syracuse-graph/lib/tagCloud';
import { http as httpHelpers } from '@sage/syracuse-core';
import * as sdataAsync from '../sdata/sdataAsync';
import * as sys from 'util';
import * as ez from 'ez-streams';
import * as uploadHelpers from '../../src/upload/helpers';
import * as htmlTransforms from '@sage/html-sanitizer/src/transforms';
import * as etnaServer from 'etna/lib/supervisor/server';
import * as sdataStateless from 'syracuse-tablet/lib/sdataStatelessHack';
import * as sdataStatelessWC from 'syracuse-tablet/lib/sdataStatelessWC';
import * as httpClient from '../http-client/httpClient';
import * as fstreams from 'f-streams';
import * as fs from 'fs';

var tracer; // = console.log;

uploadHelpers.registerRejector(function(_, contentType, detectedType, buf) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	var re = globals.context.config && globals.context.config.security.profile.officeDocumentBlackList;
	if (re && (re.test(contentType) || re.test(detectedType))) {
		return !sp.allowOfficeDocumentUpload(_);
	}
});
...
function startEtnaServer() {
	if (etnaHttpServer) return;
	// for now run etna server in-process
	const config = require('config');
	const range = (config.etna && config.etna.portRange && config.etna.portRange.slice()) || [8250, 8299];
	for (var port = range[0]; port <= range[1]; port++) {
		var etnaServer = require("@sage/etna/lib/supervisor/server");
		etnaHttpServer = new fstreams.devices.http.HttpServer(etnaServer.httpDispatch, {});
		try {
			etnaHttpServer.listen(port);
			etnaPort = port;
			console.log(`ETNA service started on port ${port}`)
			return;
		} catch (ex) {
...
function etnaForward(_, context, ep) {
	try {
		startEtnaServer();
		const etnaConfig = ep.getEtnaConfig(_, context.request.session);
		const headers = Object.assign({
			'x-etna-config': JSON.stringify(etnaConfig), // could pass it as cookie?
		}, context.request.headers, {
			cookie: context.request.session.id,
		});
		const req = httpClient.httpRequest(_, {
			url: 'http://localhost:' + etnaPort + context.request.url,
			method: context.request.method,
			headers: headers,
		});
		context.request.reader.pipe(_, req.writer);
		const resp = req.response(_);
		context.response.writeHead(resp.statusCode, resp.headers);
		resp.reader.pipe(_, context.response.writer);
	} catch (err) {
...
var _x3ContractMap = {
	walk(_, context, name, id) {
		...
		if (context.applicationRef) { // TEST to allow stubs
			...
			if (ep.useEtna(_)) return etnaForward(_, context, ep);
...
var _syracuseDatasetMap = {
	...
	$canExecute(_, context, operation) {
		// security
...
var _x3Hacks = {
	...
	batchAction(_, context, representation, id) {
		...
		try {
			...
			var reply = x3Api().pool.jsonSend(_, context.request.session, context.endpoint, {
				head: {
			...
			ts = new Date().getTime();
			reply = x3Api().pool.jsonSend(_, context.request.session, context.endpoint, {
				head: {
					"accept": "application/json;vnd.sage=syracuse",
					"content-type": "application/json; charset=UTF-8",
					"method": "PUT",
	...
	calendarQuery(_, context) { // format query response that map x-calendar to calendar response expect by the client
		...
		var resp = x3Api().pool.jsonSend(_, context.request.session, context.endpoint, {
			head: {
				"accept": "application/json;vnd.sage=syracuse",
				"content-type": "application/json; charset=UTF-8",
				"method": "GET",
...
var _x3DatasetMap = {
	walk(_, context, name, id) {
		...
		if (context.parameters.search) {
			...
			if (context.path && context.path.indexOf('/$batchGet') > 0) {
				...
				return x3Api().pool.send(_, context);
			}
		}
	},
	prototypes(_, context, name, id) {
		if (context.request && context.request.$stateless) {
	...
	workingCopies(_, context, name, id) {
...
var _x3ServiceMap = {
	walk(_, context, name, id) {
		...
		return method ? method.call(op, _, context) : x3Api().pool.send(_, context);
	}
};
...
var _x3ServiceOperationMap = {
	upload: {
		...
		get(_, context) {
			...
			var stream = x3Api().fs.createDownloadStream(_, context.endpoint, fileName, {
		...
		put(_, context) {
			...
			var stream = x3Api().fs.createUploadStream(_, context.endpoint, this._getFileName(context), {
				recOptions: this._createRecOptions(context)
			});
		...
		"delete"(_, context) {
			...
			x3Api().fs.deleteFile(_, context.endpoint, this._getFileName(context), {
				recOptions: this._createRecOptions(context)
			});
			context.reply(_, 204);
		},
...
var _linkedMap = {
	get(_, context) { // fetch instance(s) by global UUID
		if (context.syncUuid) {
	...
	"delete"(_, context) { // delete global UUID of an instance (do not delete instance itself)
		if (context.syncUuid) {
			var instance = factory.fetchInstance(_, context);
			if (instance) {
...
var _importTools = {
	"application/json": require("@sage/syracuse-lib/src/import/jsonImport").streamImport
};
