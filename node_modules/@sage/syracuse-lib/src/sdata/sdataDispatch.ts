...
import { etnaForward } from '../orm/dbHandles/etna-client';
...
var _sdataMap = {
	walk(_, context, name, id) {
		if (name === 'etna') return etnaRewrite(_, context);
...
export function etnaForwarder(_, request, response) {
	var context = new Context(request, response, {});
	const endpointName = request.url.split('/')[2];
	const ep = adminHelper.getEndpoint(_, {
		application: 'x3',
		contract: 'erp',
		dataset: endpointName,
	});
	if (!ep) throw context.notFound(locale.format(module, "endpointNotFound", 'x3', 'erp', endpointName));
	etnaForward(_, context, ep);
}

// handles prototype links that have been generated by the client
function etnaRewrite(_, context) {
	const m = /\/sdata\/etna\/(.*)\/(.*)\/\$prototypes\('(.*)\.(.*)'\)$/.exec(context.request.url);
	if (!m) throw context.notFound(`cannot rewrite  ${context.request.ur}`);
	context.request.url = `/etna/${m[2]}/${m[1]}/$prototypes/${m[3]}/${m[4]}`;
	return etnaForwarder(_, context.request, context.response);
}
