...
import * as checkLicense from '../license/check';
var flows = require('streamline-runtime').flows;
var funnel = require('streamline-runtime').flows.funnel(1);

var serviceAuthMethods, authMethods, dbMethod;

export function setup(options) {
	var opt = options || config.session || {};
	serviceAuthMethods = opt.serviceAuth || [];
	if (!Array.isArray(serviceAuthMethods)) serviceAuthMethods = [serviceAuthMethods];

	authMethods = opt.auth || ["basic"];
	if (!Array.isArray(authMethods)) authMethods = [authMethods];

	var dbMethods = authMethods.concat(serviceAuthMethods).filter(function(method, i, arr) {
		// remove duplicates and keep only basic and digest
		return arr.indexOf(method) === i && /^(basic|digest)$/.test(method);
	});
	if (dbMethods.length > 1) throw new Error("configuration error: session.auth and session.serviceAuth cannot contain both basic and digest");
	dbMethod = dbMethods[0];
}
exports.setup();

export function isAllowed(method, interactiveOnly) {
	if (interactiveOnly) return authMethods.indexOf(method) >= 0;
	else return authMethods.indexOf(method) >= 0 || serviceAuthMethods.indexOf(method) >= 0;
};

export function getDbMethod() {
...
export function getAuthModule(name) {
	if (authMethods.indexOf(name) < 0 && unofficialMods.indexOf(name) < 0 && serviceAuthMethods.indexOf(name) < 0) return null;
...
function _makeSetting(sc) {
	...
	if ("saml2" in sc) result.saml2 = sc.saml2;
	return result;
...
export function getStandardSetting(_) {
...
export function error(status, message, headers, location?: string) {
	var err = new Error(message);
	err.$httpStatus = status;
	err.$httpHeaders = headers;
	err.$httpLocation = location;
	return err;
};


export function unauthorized(challenge) {
	_errorLogging(!_);
...
export function forceErrorResponse(friendlyMessage, statusCode, challenge, parsedUrl, response) {
	_errorLogging(!_);
		console.error(friendlyMessage);
			response.writeHead(statusCode, {
		"content-type": "text/plain",
		});
		response.end(friendlyMessage);

		return exports.error(statusCode, friendlyMessage,
 	 { 'www-authenticate': challenge }, 
    parsedUrl.href);
}

export function accessDenied(status, login) {
...
export function genPage(_, response, path, params) {
...
export function redirect(_, request, response, location, authenticated, status) {
...
export function dispatcher(level, routes) {
