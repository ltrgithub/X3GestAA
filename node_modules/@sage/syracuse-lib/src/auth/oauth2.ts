...
import * as nodelocal from 'config';
	...
	return new function() {
		...
		this.loginStart = function(_, request, response, session) {
			...
			if (nodelocal.hosting && nodelocal.hosting.multiTenant)
				insertSessionNotification(request.headers['x-forwarded-host'] || request.headers.host, request.session.id);
...
export function getServerList(_) {
...
export function getUserInfo(_, accessToken, dataRequestURL) {
...
export function dispatch(_, request, response) {
	...
	if (name !== 'sageid') {
		...
		var redirectUri = (("authorized" in request.connection || nodelocal.hosting && nodelocal.hosting.https) ? "https://" : "http://") + (request.headers['x-forwarded-host'] || request.headers.host) + server.redirectPath(_);
		if (nodelocal.hosting && nodelocal.hosting.multiTenant)
			redirectUri = (nodelocal.webApiUrl ? nodelocal.webApiUrl : "https://devapi.dev-sageerpx3online.com") + server.apiPath(_);
...
function insertSessionNotification(host, sessionId) {
	(function(_) {
...
function initializeMongoDB(mongoHost, mongoPort, mongoDatabase) {
	...
	var opt = (nodelocal.mongoNotify || {}).options || {
		db: {
			w: 1
		}
	};
...
export function decryptToken(key, iv, encryptedToken, request) {
...
export function sageIdTokenValidation(_, request, response, accessToken) {
	var sageIdOAuth = (nodelocal.sage_id && nodelocal.sage_id.oauth) ? nodelocal.sage_id.oauth : {};
