...
export function authenticate(_, request, response, session) {
	...
	// To be more explicit, 
	if (!parsedUrl.query || !parsedUrl.query.oauth2)  {
		throw authHelper.forceErrorResponse( 
		'Oauth2 Bearer authentication - Missing parameter or value \'oauth2\' in Http request: ' + parsedUrl.href, 
		401, 'Basic realm=' + config.session.realm, parsedUrl, response );
	}
	...
	if (!server) {
		throw authHelper.forceErrorResponse( 
		"internal error: no OAuth2 server with batch authentication found", 
		404, 'Basic realm=' + config.session.realm, parsedUrl, response ); 
		//  authHelper.error(404, "internal error: no OAuth2 server with batch authentication found");
	}
	if (!server.active(_)) {
			throw authHelper.forceErrorResponse( 
		"internal error: oauth2 server not active: " + server.name(_), 
		403, 'Basic realm=' + config.session.realm, parsedUrl, response );
		// throw authHelper.error(403, "internal error: oauth2 server not active: " + server.name(_));
	}
