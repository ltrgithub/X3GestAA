...
export const BALANCER_HEADER = "syracusebalancerheader"; // special Http header for passing load balancing information (not used in this file 
export const TENANT_HEADER = "syracusetenantheader"; // special Http header for passing the tenant in multitenant mode
export const RESTRICT_HEADER = "x-syracuse-restrict"; // special Http header for entering restricted mode (does not accept new connections any more, not used in this file)
...
//header for transmitting a timeout change
var CHANGETIMEOUT = SPECIAL + 10;
...
export function getLocalPort(request) {
...
export function extractDataFromRequest(source, target) {
	target = target || {};
...
export function attachEngineIO(engine, client, findClient, transferSocket) {
...
export function Mock(outputStream, inputStream, requestHandler, clientOptions, wrapped) {
...
Mock.prototype.registerEngine = function(socket) {
	...
	socket.on('close', function(reason) {
		...
		// writing IO_CLOSE event may break the stream
		// var b = new Buffer(reason || "");
		// self._write([_frameHeader(IO_CLOSE, number, b.length), b]);
		console.log("Engine.io close event " + reason);
...
Mock.prototype.setTimeout = function(sec, noNotification) {
	tracer && tracer("SET TIMEOUT MOCK ", sec, noNotification);
	this._clientOptions = this._clientOptions || {};
	this._clientOptions.timeout = 1000 * sec;
	if (!noNotification) { // inform mock client on other side about timeout change
		var self = this;
		if (self._globalError) {
			console.error("Cannot propagate timeout because of global error " + self._globalError);
			return;
		}
		var currentRequest = self._step();
		self._registry[currentRequest] = [null, null, null, null];
		var sec0 = new Buffer("" + sec);
		self._write([_frameHeader(CHANGETIMEOUT, currentRequest, sec0.length), sec0]);
	}
};
	...
	if (headerType & SPECIAL) {
		switch (headerType) {
			...
			case CHANGETIMEOUT:
				if (responseHeader) {
					if (headerNumber in self._registry) {
						tracer && tracer("Delete CHANGETIMEOUT " + headerNumber);
						delete self._registry[headerNumber];
					}
				} else {
					tracer && tracer("In CHANGETIMEOUT " + headerNumber + " " + data);
					try {
						self.setTimeout(+data.toString(), true);
					} catch (e) {
						console.log("ERROR CHANGETIMEOUT ", e, e.stack);
					}
					self._write([_frameHeader(CHANGETIMEOUT + RESPONSE, headerNumber, 0)]);
				}
				break;
			case CLOSED:
...
ReadableMockStream.prototype._processData = function() {
	...
	process.nextTick(function() {
		if (!self._paused && self.readable) {
			if (self._endEmitted) {
				console.error("Ignore data after end: chunk " + util.format(self._chunk) + " remaining " + util.format(self._remainingBuffer) + " Headers " + util.format(self.headers));
				return;
			}
...
WritableMockStream.prototype.writeHead = function(statusCode) {
	...
	var self = this;
	if (self._hs) {
		   throw new Error('Cannot render headers after they are sent to the client.');
	}
...
WritableMockStream.prototype.setHeader = function(name, value) {
	this._options = this._options || {}; // avoid null pointer exceptions
	if (this._hs) throw new Error("writeHead already called");
	try {
...
export function bufferEncoding(buffer, enc) {
...
export function _frameHeader(type, number, length) {
...
export function MockStreamServer(disp, outputStream, inputStream, clientOptions, options) {
	events.EventEmitter.call(this);
	var self = this;
	...
	self.listen = function() {
		...
		try {
			...
			if (options.extraIn) {
				var extraInputStream = fs.createReadStream(null, {
					fd: options.extraIn
				});
				self._xx = new Mock(new DescriptorWriteStream(options.extraOut), extraInputStream, dispatcher, clientOptions, true);
				extraInputStream.resume();
			}
			if (self._mockEngine) {
...
export const simpleRequest = function(client, options, content, callback) {
	var result = "";
	var answered = false;
...
// for unit tests
export const _WritableMockStream = WritableMockStream;

// very simple write stream which 

export function DescriptorWriteStream(fd) {
	var self = this;
	self.writable = true;
	var ended = false;
