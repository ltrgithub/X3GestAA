...
export function sign(_, cert, algorithm, data, options, tenantId) {
	var key;
	if (cert && cert.key) {
		key = cert.key;
	} else {
...
/// ```
/// 
/// Obtain certificate data without querying MongoDB
/// 
/// * The `name` parameter gives the name of the certificate in the certificate entity.
/// * The optional `cas` parameter contains an array of names of CA certificates
/// * An optional `tenantId` which will be used to find the certificates for the tenant
/// * Result: an object with the keys `cert` for certificate, `dn` for its subject DN in RFC2253 format, 
/// * `key` for private key, `ca` for array of CA certificates
///
export function getData(name, cas, tenantId) {
	// get contents of key file
	var directory = _getDirectory(undefined, tenantId);
	var passPhrases = _readPassphrasesSync(directory);
	var keyfile = fs.readFileSync(directory + name + ".key", "utf8");
	var certfile = fs.readFileSync(directory + name + ".crt", "utf8");
	var cert = new jsx509.Certificate(certfile);
	var result = {
			dn: cert.subjectDnRFC2253,
			cert: certfile,
			key: jsx509.stripEncryption(keyfile, passPhrases[name])
	};
	if (cas) {
		result.ca = cas.map(function(ca) {
			return fs.readFileSync(directory+ca+".cacrt", "utf8");
		})
	}
	return result;
}
...
/// ```
///
/// Extract DN from cert raw data
///
/// * The `certData` parameter contains the cert raw data
/// * Result: a string representing the subject DN in RFC2253 format
///
export function getDNFromCert(certData) {
	return new jsx509.Certificate(certData).subjectDnRFC2253;
}
...
export function checkPassphrase(_, name, passphrase, tenantId) {
	var directory = _getDirectory(undefined, tenantId);
	try {
...
//  
export function setPassphrase(_, name, passphrase, tenantId) {
...
export function verify(_, cert, algorithm, data, signature, options, tenantId) {
	var certificate;
	if (cert && cert.cert) {
...
export function getPEMCertificate(_, name, ca, tenantId) {
	var certificate;
	// get contents of key file
	var directory = _getDirectory(undefined, tenantId);
	if (!name) {
...
export function streamHttpRequest(_, name, options, cas, tenantId) {
	// copy old options
	var opt = {};
...
export function parseRequestCert(_, data, certificates, caCertificates, directory, host, changed) {
	tracer && tracer("parse request cert DATA " + util.format(data));
	var response = [];
	var updatePassphrases;
	var passphraseTenantId = undefined;
	var passphrases;
	if (!fs.exists(directory, _, _)) fs.mkdir(directory, _);
	...
	data.forEach_(_, function(_, part) {
		...
		if (part.del) {
			...
			if (part.cert || part.key || part.cas) {
				...
				if (!fs.exists(fullDir, _, _)) fs.mkdir(fullDir, _);
				// update file system
				try {
					if (part.cert) {
...
export function readPassphrases(directory, _, exc) {
	try {
...
// special function to read passphrases without callbacks
// used for MongoDB with X509 authentication
function _readPassphrasesSync(directory, exc) {
	try {
		var crypted = fs.readFileSync(directory + lcHostName + ".pwd", "utf8");
		var cr = _getCryptoModule();
		return JSON.parse(cr.decrypt(new Buffer(crypted, "base64"), require));
	} catch (e) {
		if (e.code === 'ENOENT') {
...
export function writePassphrases(directory, phrases, _) {
...
export function pushMissing(host, data) {
