...
import * as jsx509 from 'jsx509';
import * as crypto from 'crypto';
import * as certTools from './certTools';
...
function start(conf, _) {
	switch (process.argv[2]) {
		...
		case "--help":
			...
			console.log("node nanny check <Timeout(in seconds)>\n  run self-check and stop");
	...
	try {
		...
		if (process.argv[2] === "install") {
			...
			try {
				...
				try {
					server.listen(port, config.system.bindIP || 'localhost', _);
		...
		switch (process.argv[2]) {
			case "check":
				if (localHost) localHost.selfCheck = true;
				//
				var timeout = process.argv[3] || 300;
				if (!/^\d+$/.test(timeout)) throw new Error(locale.format(module, "invalidPort", timeout));
				// if didn't started after timeout then exit 1
				setTimeout(function() {
					console.error("Self check failure", localHost);
					process.exit(1);
				}, +timeout * 1000);
				// let continue with run
			case "run":
			case undefined:
			case "":
				// read version file
			...
			case "patch":
				...
				if (process.argv[3] === "--unlock") {
					...
					if (!fs.exists(process.argv[3], _, _)) {
...
export function startCb(conf, cb) {
...
function startChildProcesses(force, _) {
	if (localHost.status > STATUS_INIT || force) {
		startChildrenFunnel(_, function(_) {
			...
			var cnt0 = 0;
			// start B0 process when current server is alphabetically first process which is not deactivated
			while (hosts[cnt0] && hosts[cnt0].deactivated) {
				cnt0++; // skip deactivated processes
			}
			//console.log("Index for B0 process " + cnt0 + " " + hosts.length + " " + (hosts[cnt0] && hosts[cnt0].hostname))
			...
			/* No batch process will be created
...
function startedChildProcesses(futures, _) {
	...
	if (ok) {
		...
		// if check -> exit
		if (localHost.selfCheck) process.exit(0);
...
// for unit tests
export const _cc = _compareConnection;

// exported for unit tests
export function _updateHosts(oldHosts, hosts, name) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		switch (command[0]) {
			...
			case 'notifyB':
				...
				if (command[0] === 'notifyW' && wsChildren.length === 0) {
					var path  = command.join('/');
					var i = path.lastIndexOf("?");
					if (i > 0){
						path = path.substr(0, i)
					}
					throw new Error(locale.format(module, "wsErrorNoProcess", path));
				}
...
var doRequest = function(streamRecorder, response, hostPort, callback, sessionId, cookieValue, client) {
	...
	if (hostPort[0].hostname === localHost.hostname) {
		...
		if (!client) {
			...
			//else 
			child0 = (hostPort[1][0] === "W" ? wsChildren : children)[hostPort[1].substr(1)];
