...
function _internalSecureCookie(secure, value) {
	const att = value && value.split(/\s*;\s*/) || [];
	if (secure) att.push("Secure");
	att.push("HttpOnly");

	// avoid duplicates
	return att.filter((e, i, arr) => arr.indexOf(e) === i).join("; ");
}
 ...
 */
export const sessionManager = new function() {
	...
	this.secureCookie = _internalSecureCookie;
	...
	function _getSession(_, request, response, useSessionPool, ignoreCookie) {
		...
		if (cookie) {
			...
			if (session) {
				if (request.url && request.url.indexOf("timeout=true") !== -1) {
					session.touch();
				}
	...
	function _setSessionCookie(_, request, response) {
		...
		function _secureCookie(value) {
			return _internalSecureCookie(secure, value);
		}
...
export function gentlyTerminate(_, origin, withReply) {
	_mustTerminate = true;
