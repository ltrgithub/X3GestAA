...
import * as replSet from 'mongodb/lib/replset';
	...
	return new function() {
		...
		var signOnSuccess = function(request, response, session, _) {
			...
			if (config.hosting && config.hosting.multiTenant)
				insertSessionNotification(request.headers['x-forwarded-host'] || request.headers.host, sessionId);
		...
		this.logout = function(request, response, session, _) {
			...
			if (config.hosting && config.hosting.multiTenant)
				deleteSessionNotification(request.headers['x-forwarded-host'] || request.headers.host, sessionId);
...
function deleteSessionNotification(host, sessionId) {
	(function(_) {
		var mongoConfig = config.mongoNotify || {};
		var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
