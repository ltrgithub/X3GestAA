...
class ChannelDB extends Channel {
  protected stream;
  constructor(broker, topic, startNumber) {
    super(broker, topic);
    this.clients = [];
    var condition = {
      channel: topic,
      msgNumber: { $gt: startNumber}
    };
  ...
  public start(_): Channel {
    ...
    this.stream.on('data', function (data) {
      tracer.debug && tracer.debug("Received on " + channel.name + JSON.stringify(data.message) + ", broker=" + channel.broker.fullName);
      (_ => {

        for (var i = 0; i < channel.clients.length; i++) {
          var client = channel.clients[i];
          client.callback(_, JSON.parse(data.message));
          tracer.debug && tracer.debug("Client " + client.id.toString() + " received " + JSON.stringify(data.message) + ", broker=" + channel.broker.fullName);
        }
      })(flows.check);
    });
    this.stream.on('close', function () { tracer.debug && tracer.debug("Close cursor on " + channel.name + ", broker=" + channel.broker.fullName) });
...
export class BrokerDB extends Broker {
  ...
  public initDatabase(_, db) {
    ...
    this.orm = db;
    this.collection = this.collection || this.orm.db.createCollection("dbMessage", { "capped": true, "size": 100000, "max": 1000 }, _);
  }
  ...
  public subscribe(_, topic, callback): Client {
    ...
    if (!channel) {
      let startNumber = this.orm.getCounterValue(_,"pubsub","batch",{increment:1}).value;
      channel = new ChannelDB(this, topic, startNumber);
  ...
  public publish(_, topic, message) {
    let number = this.orm.getCounterValue(_,"pubsub","batch",{increment:1}).value;
    this.collection.insertOne({ "channel": topic,"msgNumber":number, "message": JSON.stringify(message), "time": new Date().toISOString() }, _);
