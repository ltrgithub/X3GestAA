"use strict";

import { helpers } from '@sage/syracuse-core';
import { _, flows } from 'streamline-runtime';
import { adminHelper } from '../collaboration/helpers';
const locale = require('streamline-locale');
var tracer = require('@sage/syracuse-core').getTracer("pubsub");
var os = require("os");

function debug(message) {tracer.debug && tracer.debug(message);}

class Broker {
  public name;
  public channels = {};  
  public clients  = {}:  
  constructor() {
    debug("New broker : " +this.name);              
  }
  public connect(_,namespace) {
  public subscribe(_,topic) {}
  public unSubscribe(_,topic) {}
  public publish(_,topic,message) { 
     debug("Publish on " + topic + " " +JSON.stringify(message));              
  )}
  public removeChannel(channel) {
    debug("Remove channel " + channel.name);
    delete this.channels[channel.name];
  }

  public publishAndWaitOn(_,pubTopic,message,subTopic) {
    var _subCallback;
    var broker = this;
			
		function _publish(cb, topic, pmessage) {
				_subCallback = cb;
				broker.publish(function(err) {
					if (err) {
						ok(false, "Publish got error: " + err);
					}
				}, topic, pmessage);

			}
    var client = this.subscribe(_, subTopic || pubTopic , function(_, smessage) {
				_subCallback && _subCallback(null, smessage);
			});

    var data = _publish(_, pubTopic, message);
    client.unSubscribe();      
    return data;
  }
}

class Channel {
  public name:string;
  public clients;  
  private broker:Broker
  constructor(broker,topic) {
      debug("Create channel : " + topic);  
      this.clients = [];
      this.broker = broker;
      this.name = topic;
  }
  protected addClient(client:Client) {
    this.clients.push(client);
    client.channel = this;
    debug("Client " + client.id.toString() + " subscribe to channel " + this.name);  
  }
  protected removeClient(client:Client) {
    var removeIndex = this.clients.map(function(obj) { return obj.id.toString(); })
                       .indexOf(client.id.toString());
    ~removeIndex && this.clients.splice(removeIndex, 1);
    debug("Remove client " + client.id + " from channel " + this.name)
    if (this.clients.length == 0) broker.removeChannel(this);
  }

}

class Client {
  public hostname;    
  public id;  
  protected channel;
  protected callback;
  protected broker;
  public unSubscribe() {
    this.channel.removeClient(this);
  };  
}


var broker;
export function getBroker(_) {
    if (broker) return broker;    
    broker = new BrokerDB();    
    var db = adminHelper.getCollaborationOrm(_);    
    broker.initDatabase(_,db);    
    return broker;
}

//------------------------------------------------
//             DB broker implementation
//------------------------------------------------

class ClientDB extends Client {
    constructor(broker,callback) {
      this.callback = callback;     
      this.broker = broker;      
    }

    public start(_) {
      var res = this.broker.getCollection().insertOne({"channel": "$sys", "message":{"type":"subscribe", "data": {host:os.hostname(), "pid": process.pid}}, "time": Date.now()},_);
      this.id = res.insertedId;      
      debug("Start client " + this.id.toString());
      return this;
    }
}


class ChannelDB extends Channel {
   protected stream; 
   constructor(broker,topic,startId) {
      super(broker,topic);
      this.clients = [];
      var condition = {
          channel:topic,
          _id:{$gt:startId)}
       };       
       this.stream = broker.getCollection().find(condition, {tailable:true, numberOfRetries: Number.MAX_VALUE, tailableRetryInterval:200, noCursorTimeout: true, awaitData:true}).stream();      
       });

   public start(_) {
      var channel = this;
      debug("Start channel " + this.name +"(" + this.clients.length + ")")
      this.stream.on('data',function(data){
        debug("Received on " + channel.name + JSON.stringify(data.message));            
        (_ => {
            
            for (var i = 0; i < channel.clients.length; i++) {
                var client= channel.clients[i];
                client.callback(_,JSON.parse(data.message));
                debug("Client " + client.id.toString() + " received " + JSON.stringify(data.message))     
            }           
		})(flows.check);
     }
     this.stream.on('close',function(){console.log("Close cursor",channel.name)});
     this.stream.on('error',function(err){console.log("Error onr",channel.name,err)});
     return this;
   } 
}

export class BrokerDB extends Broker {
  protected db;
  protected stream;
  static collection;
  constructor() {
    this.name= "Mongo DB broker"; 
    super();
  }

  public initDatabase(_,db) {
    //console.log("Using database",db.db);
    this.db = db.db;  
    BrokerDB.collection = BrokerDB.collection || this.db.createCollection("messages", { "capped": true, "size": 100000, "max": 1000},_);    
  }
  public getCollection() {
      return BrokerDB.collection;
  }
  
 
  public subscribe(_,topic,callback) {        
    var client = new ClientDB(this,callback); 
    client.start(_);   
    var channel = this.channels[topic];  
    if (!channel) {           
      channel = new ChannelDB(this,topic,client.id);   
      this.channels[topic] = channel.start(_);
    }
    channel.addClient(client);    
    return client;    
  }

  public publish(_,topic,message) {    
    BrokerDB.collection.insertOne({"channel": topic, "message":JSON.stringify(message), "time": Date.now()},_);  
    super.publish(_,topic,message);
    debug(this.channels[topic].clients.length + " listen on " + topic);            
  }

}

