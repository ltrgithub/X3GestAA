...
class Broker {
  ...
  public clients = {};
  constructor() {
    tracer.debug && tracer.debug("New broker : " + this.name);
  }
  public connect(_, namespace) { }
  public subscribe(_, topic, callback) { }
  public unSubscribe(_, topic) { }
  public publish(_, topic, message) {
    tracer.debug && tracer.debug("Publish on " + topic + " " + JSON.stringify(message));
  }
  public removeChannel(channel) {
    tracer.debug && tracer.debug("Remove channel " + channel.name);
...
class Channel {
  ...
  constructor(broker, topic) {
    tracer.debug && tracer.debug("Create channel : " + topic);
    this.clients = [];
    this.broker = broker;
    this.name = topic;
  }
  protected addClient(client: Client) {
    ...
    client.channel = this;
    tracer.debug && tracer.debug("Client " + client.id.toString() + " subscribe to channel " + this.name);
  }
  protected removeClient(client: Client) {
    ...
    tracer.debug && tracer.debug("Remove client " + client.id + " from channel " + this.name)
    if (this.clients.length == 0) broker.removeChannel(this);
  }
...
class ClientDB extends Client {
  ...
  public start(_) {
    ...
    tracer.debug && tracer.debug("Start client " + this.id.toString());
    return this;
  }
}
...
class ChannelDB extends Channel {
  ...
  public start(_) {
    var channel = this;
    tracer.debug && tracer.debug("Start channel " + this.name + "(" + this.clients.length + ")")
    this.stream.on('data', function (data) {
      tracer.debug && tracer.debug("Received on " + channel.name + JSON.stringify(data.message));
      (_ => {

        for (var i = 0; i < channel.clients.length; i++) {
          var client = channel.clients[i];
          client.callback(_, JSON.parse(data.message));
          tracer.debug && tracer.debug("Client " + client.id.toString() + " received " + JSON.stringify(data.message))
        }
      })(flows.check);
    });
    this.stream.on('close', function () { tracer.debug && tracer.debug("Close cursor on " + channel.name) });
    this.stream.on('error', function (err) { console.log("Error onr", channel.name, err) });
    return this;
  }
}
...
export class BrokerDB extends Broker {
  ...
  public initDatabase(_, db) {
    ...
    this.db = db.db;
    BrokerDB.collection = BrokerDB.collection || this.db.createCollection("dbMessage", { "capped": true, "size": 100000, "max": 1000 }, _);
  }

  public getCollection() {
    return BrokerDB.collection;
  }

  public subscribe(_, topic, callback) {
    var client = new ClientDB(this, callback);
    client.start(_);
  ...
  public publish(_, topic, message) {
    BrokerDB.collection.insertOne({ "channel": topic, "message": JSON.stringify(message), "time": new Date().toISOString() }, _);
    super.publish(_, topic, message);
    tracer.debug && tracer.debug(this.channels[topic].clients.length + " listen on " + topic);
  }
