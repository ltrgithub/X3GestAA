...
<head>
	...
	<script type="text/javascript">
		...
		function init() {
			var pics = [
				'/syracuse-auth/html/images/login/1.jpg',
				'/syracuse-auth/html/images/login/2.jpg',
				'/syracuse-auth/html/images/login/3.jpg',
				'/syracuse-auth/html/images/login/4.jpg',
				'/syracuse-auth/html/images/login/5.jpg',
				'/syracuse-auth/html/images/login/6.jpg',
				'/syracuse-auth/html/images/login/7.jpg',
				'/syracuse-auth/html/images/login/8.jpg'
			];
		...
		function authenticate(type) {
			if (type === undefined) {
				type = ['basic', 'digest', 'ldap'].filter(function(t) {
					return document.getElementById('go-' + t).style.visibility == "visible";
				})[0];
			}
	...
	<div class="container">
		<div class="s-text-containt">
			<div class="s-text" style="float:left;">
				...
				<form id="loginForm" >
				<div class="s-visibility-{{passwordVisibility}}" style="float:left;width:100%;">
					<div class="z-field" style="float:left;width:100%;">
						<div style="float:left;width:30%;">{{loginLabel}}</div>
						<input id="username" type="text" style="float:left;" autocomplete="on">
					</div>
					<div class="z-field" style="float:left; width:100%;">
						<div style="float:left;;width:30%;">{{passwordLabel}}</div>
						<input id="password" type="password" style="float:left;" autocomplete="on">
					</div>
					<div class="z-action" style="float:left; width:100%;">
						<a id="go-basic" style="visibility:{{basicVisibility}};" href="javascript:authenticate('basic')">{{dbGo}}</a>
						<a id="go-digest" style="visibility:{{digestVisibility}};" href="javascript:authenticate('digest')">{{dbGo}}</a>
						<a id="go-ldap" style="visibility:{{ldapVisibility}};" href="javascript:authenticate('ldap')">{{ldapGo}}</a>
					</div>
    				<input type="submit" style="display:none;" onclick="javascript:authenticate(); return false;"/><!-- to capture enter key -->
    			</div>
				</form>
			</div>
			<div class="s-text s-visibility-{{oauth2Visibility}}" style="float:left;width:89%;padding-top:0;">
				<div class="s-product" style="font-size:0.5em;">{{oauth2Title}}</div>
				<div id="oauth2-0" class="z-action" style="float:left;width:100%;">
					<a href=""></a>
				</div>
			</div>
			<div class="s-text s-visibility-{{sage-idVisibility}}" style="float:left;width:89%;padding-top:0;">
