...
<meta http-equiv="content-type" content="text/html; charset=UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=10">
	...
	<script type="text/javascript">
		...
		function init() {
			...
			document.getElementById('password').value = "bidon"; // doesn't matter we use the login cookie
		...
		<div class="s-text-containt">
			<div class="s-text">
				...
				<div class="s-visibility-{{passwordVisibility}}">
					...
					<div class="s-visibility-{{passwordVisibility}}">
						<div class="z-field">
							<input id="username"  autocomplete="on" onclick="javascript:askResign()" placeholder="{{loginLabel}}" type="text">
						</div>
						<div class="z-field">
							<input id="password"  autocomplete="on" onclick="javascript:askResign()" placeholder="{{passwordLabel}}" type="password">
						</div>
						<div class="z-field" id="check">
							{{keepConnectedLabel}}<input id="keepConnected"  autocomplete="on" type="checkbox">
						</div>
						<div class="z-action s-visibility-{{basicVisibility}}">
							<a id="go-basic" href="javascript:authenticateAndForget('basic')">{{dbGo}}</a>
						</div>
						<div class="z-action s-visibility-{{digestVisibility}}">
							<a id="go-digest"href="javascript:authenticateAndForget('digest')">{{dbGo}}</a>
						</div>
						<div class="z-action s-visibility-{{ldapVisibility}}">
							<a id="go-ldap" href="javascript:authenticateAndForget('ldap')">{{ldapGo}}</a>
						</div>
