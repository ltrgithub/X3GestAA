...
exports.getStandardSetting = function(_) {
	...
	if (authMethods.indexOf(method === "ldap" ? "basic" : method) < 0) throw new Error("configuration error: default authentication " + method + " not enabled. Fix your nodelocal.js config file!");

	var lpref = (setting && setting.localePref(_));
	var result = {
		method: method,
		source: method,
		twoDigitYearMax: setting.twoDigitYearMax(_),
...
exports.redirect = function(_, request, response, location, authenticated, status) {
	...
	if (acceptHtml) {
		...
		if (authTargetUrl) {
			if (location.indexOf("?url") >= 0 || location.indexOf("/syracuse-tablet/") >= 0) {
