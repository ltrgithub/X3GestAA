...
exports.getStandardSetting = function(_) {
	var diff = Date.now() - cacheAccess;
	if (diff < 1000) {
		var result = {
			method: standardCache.method,
			source: standardCache.source,
		};
		if ("ldap" in standardCache) result.ldap = standardCache.ldap;
		if ("oauth2" in standardCache) result.oauth2 = standardCache.oauth2;
		return result;
	}
	cacheAccess += diff;
