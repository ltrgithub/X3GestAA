...
exports.getProductName = function() {
	...
	var productName = ' ERP X3';
	var currentLocale = ((globals.context && globals.context.locale) || 'en-US').toLowerCase();;
	if (listLicense.length === 1) {
		productName = listLicense[0].productTitle[currentLocale] ? listLicense[0].productTitle[currentLocale].replace(/^Sage/g, '') : '';
	} else if (listLicense.length > 0) {
		// check if al license describe the same productTitle
		var stop = false;
		productName = listLicense[0].productTitle[currentLocale] ? listLicense[0].productTitle[currentLocale].replace(/^Sage/g, '') : '';
		for (var i = 1; i < listLicense.length && !stop; i++) {
			stop = (productName !== listLicense[0].productTitle[currentLocale] && listLicense[0].productTitle[currentLocale] ? listLicense[0].productTitle[currentLocale].replace(/^Sage/g, '') : '');
		}
		if (stop) {
			productName = " ERP X3";
		}
