...
exports.getStandardSetting = function(_) {
	...
	console.log("GETSTANDARDSETTING ENTER");
	var result;
	var now = Date.now();
	if (now - cacheAccess < 1000) {
		console.log("GETSTANDARDSETTING TRYING CACHE");
		if (multiTenant) {
			var tenantId = globals.context.tenantId;
			var sc = standardCache[tenantId];
			if (sc && now - sc._time < 1000) return console.log("GETSTANDARDSETTING CACHE OK"), _makeSetting(sc);
		} else {
			if (standardCache) return console.log("GETSTANDARDSETTING CACHE OK"), _makeSetting(standardCache);
		}
	} else {
		if (multiTenant) standardCache = {};
	}
	// introduce funnel to avoid race conditions for standard cache updates
	console.log("GETSTANDARDSETTING BEFORE FUNNEL");
	funnel(_, function(_) { // check again whether database operation is really necessary:
		...
		if (now - cacheAccess < 1000) {
			if (multiTenant) {
				...
				if (sc && now - sc._time < 1000) {
					...
					console.log("GETSTANDARDSETTING FUNNEL CACHE OK");
					return result;
				}
			} else {
				if (standardCache) {
		...
		console.log("GETSTANDARDSETTING MISSED CACHE IN FUNNEL");
		...
		if (authMethods.indexOf(method === "ldap" ? "basic" : method) < 0) {
			...
			// This hack should be reviewed
			// update of mongo db should be done in a proper db upgrade script 
			// For now we do it asynchronously to avoid deadlock in funnel re-entry
			var saveAuthChanges = function(_) {
				console.log("GETSTANDARDSETTING SAVING AUTHENTICATION");
			...
			saveAuthChanges(flows.check);
		}
		console.log("GETSTANDARDSETTING BEFORE LOCALEPREF");
	...
	console.log("GETSTANDARDSETTING RETURNING RESULT");
	return result;
...
function _errorLogging(_) {
	...
	try {
		...
		console.log(c + "Local sessions " + require('syracuse-session/lib/sessionManager').localSessions());
...
exports.redirect = function(_, request, response, location, authenticated, status) {
	...
	if (acceptHtml) {
		...
		console.log(new Date().toISOString(), "before redirect writeHead", request.url);
		...
		console.log(new Date().toISOString(), "after redirect end", request.url);
	} else {
