...
function initializeMongoDB(mongoHost, mongoPort, mongoDatabase) {
	var server = new mongodb.Server(mongoHost || 'localhost', mongoPort || 27017, nodelocal.mongodb.options.server);
	var db = new mongodb.Db(mongoDatabase, server, nodelocal.mongodb.options.db);
	return db;
}
...
function sageIdTokenValidation(_, request, response, accessToken) {
	var sageIdOAuth = (nodelocal.sage_id && nodelocal.sage_id.oauth) ? nodelocal.sage_id.oauth : {};
	var decryptedXML = decryptToken(sageIdOAuth.key, sageIdOAuth.iv, accessToken);
	var decryptedJSON = jsxml.parse(decryptedXML);
