...
function create(config, redirectUri) {
	return new function() {
		...
		this.sageIdRedirect = function(_, request, response) {
			...
			var doc = db.open(_).collection('OAuth', _).find(filter, _).toArray(_)[0];
			var host = doc.host;
...
function insertSessionNotification(_, host, sessionId) {
	...
	db.open(_).collection('OAuth', _).update({ _id: sessionId, host: host }, { _id: sessionId, host: host }, { upsert: true }, _);
	db.close(_);
}

function deleteSessionNotification(_, sessionId) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	db.open(_).collection('OAuth', _).remove({ _id: sessionId}, null, _);
	db.close(_);
}
