...
function create(config, redirectUri) {
	return new function() {
		...
		this.sageIdCallback = function(_, request, response) {
			...
			authHelper.genPage(_, response, __dirname + "/../html/token.html", {sageIdTokenId: ''});
		};
		...
		this.sageIdDownloadToken = function(_, request, response) {
			...
			var tokenData = db.open(_).collection('Tokens', _).find({_id: id}).toArray(_)[0];
			db.close(_);
		...
		this.sageIdTokenRetrieval = function(_, request, response) {
			...
			// Find access token
			var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
			var token = db.open(_).collection('Tokens', _).find({_id: id}).toArray(_)[0];
			if (!token)
...
function storeAccessToken(_, sessionId, data) {
	var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
	db.open(_).collection('Tokens', _).update({ _id: sessionId}, { _id: sessionId, tokenData: data }, { upsert: true }, _);
	db.close(_);
}
...
function sageIdTokenValidation(_, request, response, accessToken) {
	...
	if (valid)
		...
		var db = initializeMongoDB(mongoConfig.host, mongoConfig.port, mongoConfig.database);
		db.open(_).collection('Tokens', _).update({"tokenData.refresh_token": refreshToken}, { tokenData: newTokenData }, { upsert: true }, _);
		db.close(_);
