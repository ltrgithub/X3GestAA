...
exports.dispatch = function(_, request, response) {
	...
	var sageIdAuth = require("sage-id").create(getOptions(_, request));
	...
	if (authData.auth) {
		...
		console.log(new Date().toISOString(), "before validate user", request.url);
		validateUser(_, request, response, session, authData);
		console.log(new Date().toISOString(), "after validate user", request.url);
		updateSessionWithID(request, response, session, session.id, authData, _);
		//checkForAuthenticatedUser(request, response, session, _);
		console.log(new Date().toISOString(), "before redirect", request.url);
		authHelper.redirect(_, request, response, session.authTargetUrl || '/', true);
		console.log(new Date().toISOString(), "after redirect", request.url);
		...
		if (authData.notificationType == "Ended" && session) {
			require("sage-id").deleteSessionNotification(_, request.headers['x-forwarded-host'] || request.headers.host, authData.sessionId);
			session.authData = null;
		}
	}
	var elapsed = Date.now() - t0;
	console.log(new Date().toISOString(), "SageID dispatch:", elapsed, "ms for", request.url);
};
