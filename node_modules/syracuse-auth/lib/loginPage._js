...
function genPage(_, request, response) {
	var params = {};
	['loginLabel', 'passwordLabel', 'keepConnectedLabel', 'dbGo', 'ldapGo', 'oauth2Title', 'sageIdTitle', 'sageIdSignOn', 'sageIdRegister', 'sageIdRegisterExist'].forEach(function(label) {
...
function submit(_, request, response) {
	...
	if (httpHelper.parseHeaders(request.headers || {})["content-type"] === "application/json") params = JSON.parse(params);
