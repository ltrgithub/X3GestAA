...
exports.authenticate = function(_, request, response, session) {
	...
	var prefix = credentials[1].substr(0,6);
	if (prefix !== 'sageid') {
		...
		if (server.name(_) !== 'sageid') {
			...
			var user = checkUser.fromLoginPage(_, request, 'oauth2', login, null, null, server.name(_));
		} else {
			var sageIdOAuth = (config.sage_id && config.sage_id.oauth) ? config.sage_id.oauth : {};
			oauth2.sageIdTokenValidation(_, request, response, credentials[1]);
			var decryptedXML = oauth2.decryptToken(sageIdOAuth.key, sageIdOAuth.iv, credentials[1]);
			var decryptedJSON = jsxml.parse(decryptedXML);
			var login = decryptedJSON["AuthenticationToken"]["Subject"]["UserPrincipal"]["EmailAddress"];
			var user = checkUser.fromLoginPage(_, request, 'sage-id', login, null);
		}
	} else {
		sageid.tokenValidation(_, request, response, credentials[1]);
		var token = credentials[1].substr(6, credentials[1].length);
		var decodedXML = new Buffer(token, 'base64').toString();
		var decryptedJSON = jsxml.parse(decodedXML);
		var login = decryptedJSON["AuthenticationToken"]["Subject"]["UserPrincipal"]["EmailAddress"];
		var user = checkUser.fromLoginPage(_, request, 'sage-id', login, null);
	}
