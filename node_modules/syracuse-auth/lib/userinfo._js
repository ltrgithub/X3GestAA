...
function verifyToken(_, token) {
	...
	if (!jti) throw authHelper.error(401, "Error 1: Invalid token");

	// check if iat is in the future
	if (!decoded.iat || (Date.now() - (decoded.iat * 1000) < 0)) throw authHelper.error(401, "Error 2: Invalid token");
	...
	if (!tokenInfo) throw authHelper.error(401, "Error 3: Invalid token");

	let appConn = tokenInfo.app(_);
	if (!appConn || !appConn.active(_)) throw authHelper.error(401, "Error 4: Invalid token");

	try {
		// check signature and expiration
		jwt.verify(token, appConn.secret(_));
	} catch (e) {
		let err = authHelper.error(401, `Error 5: Invalid token: ${e.name}: ${e.message}`);
		err.expiredAt = e.expiredAt;
		throw err;
	}

	// check if login is provided
	if (!decoded.sub) throw authHelper.error(401, "Error 6: Invalid token");
	...
	if (!user || !user.active(_)) throw authHelper.error(401, "Error 7: Invalid token");
