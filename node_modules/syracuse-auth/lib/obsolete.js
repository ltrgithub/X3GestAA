		...
		if (!session.isAuthenticated()) {
			...
			if (options.forceAuth) {
				...
				if (options.authType == "sage-id") {
					...
					if (session.expireDue) {
						...
						require("syracuse-auth/lib/sage-id").verifyUser(request, response, authData, _);
					} else {
						...
						if (authData.sessionNotify) {
							...
							var extend = require("syracuse-auth/lib/sage-id").extendSession(request, response, session, authData, _);
			...
			if (authRes) {
				...
				if (options.authType == "sage-id") {
					...
					session = require("syracuse-auth/lib/sage-id").updateSessionWithID(request, response, _sessions, cookie, authData, _);
				}
...
exports.getAuthModuleObsolete = function(request, redirect, _) {
	...
	if (request.connection.authorized) return require("syracuse-auth/lib/certificate").create(checkUser.fromCertificate);
	// certificate auth <<<
	var result;
	...
	if (regexResult && regexResult[1].indexOf("oauth2") < 0 && regexResult[3]) { // login with token
		var user = require("syracuse-auth/lib/changePassword").getTempLogin(regexResult[3]);
		if (user) {
			return {
	...
	if (authmethod === "basic") {
		return require("syracuse-auth/lib/basic").create(checkUser.fromLoginPage);
	} else if (authmethod === "digest") {
		return require("syracuse-auth/lib/digest").create(checkUser.fromLoginPage);
	} else if (authmethod === "sageerpx3") {
		return require("syracuse-auth/lib/sageerpx3").create(checkUser.fromLoginPage);
		...
		return require("syracuse-auth/lib/oauth2").create(checkUser.fromLoginPage, authserver);
	}
};
...
function dispatcherObsolete(config) {
	return function(_, request, response) {
		...
		if (request.url.indexOf("/auth/pwd--") === 0) {
			...
			var newUrl = "/";
			var changePasswordModule = require('syracuse-auth/lib/changePassword');
