...
exports.fromLoginPage = function(_, request, method, login, password, challenge, serverName) {
	console.log("FROMLOGINPAGE ENTER", request.url);
	var session = request.session;
	var standardSetting = authHelper.getStandardSetting(_);
	console.log("FROMLOGINPAGE AFTER GETSTANDARDSETTING", request.url);
	var user = null;
	...
	var db = adminHelpers.AdminHelper.getCollaborationOrm(_);
	console.log("FROMLOGINPAGE AFTER GETCOLLABORATIONORM", request.url);
	var condition = {};
	var users = [];
	var server;
	...
	if (isBearer) {
		console.log("FROMLOGINPAGE ISBEARER BEGIN", request.url);
		if (server == null) {
		...
		console.log("FROMLOGINPAGE ISBEARER END", request.url);
	} else {
		console.log("FROMLOGINPAGE NOTBEARER BEGIN", request.url);
		...
		console.log("FROMLOGINPAGE NOTBEARER END", request.url);
	}
	console.log("FROMLOGINPAGE USERS LENGTH", users && users.length, request.url);
	...
	console.log("FROMLOGINPAGE BEFORE USER ACTIVE TEST", request.url);
	...
	console.log("FROMLOGINPAGE AFTER USER ACTIVE TEST", request.url);
	...
	console.log("FROMLOGINPAGE BEFORE LOCAL CONFIG TESTS", request.url);
	...
	// check role
	console.log("FROMLOGINPAGE BEFORE CHECKROLEANDLICENSE", request.url);
	var diagnoses = _checkRoleAndLicense(_, user, request, session);
	console.log("FROMLOGINPAGE AFTER CHECKROLEANDLICENSE", request.url);
	...
	console.log("FROMLOGINPAGE BEFORE GETUSERPROFILE", request.url);
	...
	console.log("FROMLOGINPAGE RETURN", !!user, request.url);
	return user;
};
