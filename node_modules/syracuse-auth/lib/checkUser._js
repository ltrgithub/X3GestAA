...
exports.fromLoginPage = function(_, request, method, login, password, challenge, serverName) {
	...
	var condition = {};
	if (login.indexOf('@') > 0) {
		condition.sdataWhere = 'active eq true and (email eq "' + login + '" or ((email eq null or email eq "") and (login eq "' + login + '")))';
	} else {
		condition.jsonWhere = {
			login: login,
			active: true
	...
	if (method !== "basic") filter.authentication = method;
	...
	if (users.length == 0) {
		...
		} else throw authHelper.unauthorized(challenge);
	}
