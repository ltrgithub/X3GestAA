...
exports.create = function(validatePassword, standardConfig) {
	return new function() {
		function _unAuthenticated(request, response, _) {
			...
			response.end(basic.textUnauth(request, _));
			return false;
		}
		...
		this.authenticate = function(request, response, _) {
			...
			if (!getNonce(auth.nonce)) return _unAuthenticated(request, response, _);

			// client nonce count must be 1
			if (!auth.cnonce || auth.nc * 1 !== 1) return _unAuthenticated(request, response, _);

			var status = validatePassword(request.session, auth.username, null, function(hashPasswd) {
			...
			switch (status) {
				case 1:
					return _unAuthenticated(request, response, _);
				case 2:
		...
		this.setAuthRequestHeader = function(headers) {
			headers['WWW-Authenticate'] = 'Digest realm="' + config.session.realm + '", qop="auth", algorithm="MD5", nonce="' + nonce + '", opaque="00"';
		}; 
	};
