...
exports.create = function(validatePassword, standardConfig) {
	return new function() {
		...
		this.authenticate = function(request, response, session, _) {
			...
			}, "digest", standardConfig, _, request);
			...
			switch (status) {
				...
				case 5:
				case 6:
					return basic.noAccess(request, response, _, status, auth.username);
				case 3:
					...
					return _unAuthenticated(_, request, response, locale.format(module, "userInactive", auth.username));
					*/
				case 0:
					break;
				default:
