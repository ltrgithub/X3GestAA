...
function extendSession(request, response, session, data, _) {
	if (data.notificationType === 'ExpiryDue') {
		...
		var newExpirationDate = new Date(lastActivity.getTime() + 180);
...
function updateSessionWithID(request, response, session, cookie, authData, _) {
	var _sessions = require('syracuse-session/lib/sessionManager').getGlobals(_);
	delete _sessions[cookie];
	session.id = authData.sessionId;
	_sessions[authData.sessionId] = session;
};
...
exports.dispatch = function(_, request, response) {
	...
	if (authData.auth) {
		...
		if (authData.notificationType == "Ended") {
