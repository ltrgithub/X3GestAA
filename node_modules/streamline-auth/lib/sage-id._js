...
exports.dispatch = function(_, request, response) {
	...
	if (authData.auth) {
		...
		var status = checkUser.fromLoginPage(_, request.session, authData.email, null, null, 'sage-id', request);
		if (status !== 'ok') throw new Error("user check failed: " + status); // see later
		request.session && request.session.afterAuthentication({
