...
function getOptions(request) {
	return {
		...
		cancelAllowed: true,
		sessionLengthMinutes: configOpts.sessionLengthMinutes || 10,
...
function updateSessionWithID(request, response, session, cookie, authData, _) {
	var sessions = require('syracuse-session/lib/sessionManager').sessionManager.getTenantSessions();
...
exports.dispatch = function(_, request, response) {
	...
	if (authData.auth) {
		...
		authHelper.redirect(_, request, response, session.authTargetUrl || '/');
...
function checkForAuthenticatedUser(request, response, session, _) {
	var listOfSessions = require('syracuse-session/lib/sessionManager').sessionManager.getTenantSessions();
	var sessions = Object.keys(listOfSessions).map(function(k) { return listOfSessions[k] });
	for (var i = 0; i < sessions.length; i++) {
		...
		if (searchSession.EndSignOnAttemptResponse || searchSession.EndRegistrationAttemptResponse) {
			...
			if (searchIdentity === currentIdentity && searchSessionId != currentSessionId) {
				...
				return require('syracuse-session/lib/sessionManager').sessionManager.logout(_, request, response, session);
			}
		}
	};
}