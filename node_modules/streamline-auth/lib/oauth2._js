...
exports.create = function(validateUser, config, standardConfig) {
  return new function() {
    ...
    auth.setAccessTokenName(""); // Bearer authentication
    auth.setUrlEncode(false); // only minimum URL encoding
    var self = this;

    
    // wrapper for streamline (auth.get)
    this._wrapGet = function(dataRequestURL, access_token, cb) {       
    ...
    this._wrapGetToken = function(code, params, cb) {
        auth.getOAuthAccessToken(code, params, function(error, access_token, refresh_token, result) {
          ...
          if (error) {
            ...
            cb(null, {"access_token": access_token, "refresh_token": refresh_token, "result": result});
          }
        });
      };
...
/// second step of authentication: obtain access token and user name
    this.authenticate2 = function(request, response, _) {    
      ...
      try
      {
        ...
        var ok = validateUser(request.session, user, "", method, standardConfig, _);
        if (!ok) { 
