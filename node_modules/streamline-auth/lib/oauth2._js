...
exports.create = function(validateUser, config) {
	return new function() {
		...
		this.authenticate2 = function(request, response, _) {
			...
			try {
				...
				var status = validateUser(_, request.session, user, "", null, method);
				...
				switch (status) {
					...
					case "noRole":
					case "wrongOAuth2":
						return basic.noAccess(_, request, response, status, user);
					case "ok":
						break;
					default:
						// non-fatal error
