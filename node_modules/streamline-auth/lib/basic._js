...
function getRedirectPageContent(reason, _) {
	...
	content += '<html><head>';
	content += '<META HTTP-EQUIV="refresh" charset="utf-8" content="' + delay + ';URL=' + url + '">';
	content += '<style>';
...
exports.noAccess = function(request, response, _, status, user) {
	...
	if (request.headers && request.headers.accept && request.headers.accept.indexOf("application/json") >= 0) {
		...
		response.end(getRedirectPageContent(message, _));
	}
...
exports.create = function(validatePassword, standardConfig) {
	return new function() {
		...
		this.authenticate = function(request, response, session, _) {
			...
			var status = validatePassword(request.session, user, pass, null, "basic", standardConfig, _, request);
			switch (status) {
				case 1:
