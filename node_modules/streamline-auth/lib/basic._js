...
exports.textUnauth = function(request, _) {
	...
	var t1 = locale.format(module, "unauth");
	var t2 = locale.format(module, "click");
	return "<!DOCTYPE html><html><head></head><body>" + t1 + "<br><a href='/index.html'>" + t2 + "</a></body></html>";
};

exports.textUnlicensed = function(request, _) {
	setLocaleFromRequest(request, _);
	var t1 = locale.format(module, "nolicense");
	var t2 = locale.format(module, "click");
	return "<!DOCTYPE html><html><head></head><body>" + t1 + "<br><a href='/index.html'>" + t2 + "</a></body></html>";
};
...
exports.create = function(validatePassword, standardConfig) {
	return new function() {
		function _unAuthenticated(request, response, _) {
			...
			var text = exports.textUnauth(request, _);
			response.end(text);
			return false;
		}
		...
		this.authenticate = function(request, response, _) {
			...
			if (!(credentials && credentials[1])) return _unAuthenticated(request, response, _);

			var usrpwd = new Buffer(credentials[1], "base64").toString("binary");
			var index = usrpwd.indexOf(':');
			if (index < 0) return _unAuthenticated(request, response, _);

			var user = usrpwd.substr(0, index);
			var pass = usrpwd.substr(index + 1);

			if (!(user && pass)) return _unAuthenticated(request, response, _);
