...
*/
// TEMPORARY - remove if redir hack is not needed any more
function _redir(login, status, userAuthentication, request) {
	return status;
}
...
exports.fromLoginPage = function(_, session, login, password, passwordCheck, method, request) {
	...
	if (!user.active(_)) return _redir(login, "inactiveUser", userAuthentication, request);
	...
	// for external auth providers we also need to check that the 
	if (localConfig.method !== method) {
		return _redir(login, "wrongAuth", userAuthentication, request);
	}
	...
	switch (localConfig.source) {
		case "db":
			if (passwordCheck) {
				if (!passwordCheck(user.password(_))) return "unAuthenticated";
			} else {
				...
				if (hash.digest("hex") !== user.password(_)) return "unAuthenticated";
			}
			break;
		case "ldap":
			...
			break;
		default:
			// if external provider we are fine; otherwise we bail out.
			if (!/^(oauth2-|sage-id)/.test(localConfig.method)) throw new Error("internal error: bad auth source: " + localConfig.source);
			break;
	}
	...
	if (!/^(sage-id|oauth2|ldap)/.test(method) && user.changePassword(_)) {
