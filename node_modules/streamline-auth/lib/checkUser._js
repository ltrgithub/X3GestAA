...
exports.fromLoginPage = function(_, request, method, login, password, challenge, serverName) {
	...
	switch (userAuthentication) {
		...
		case "ldap":
			...
			break;
		case "oauth2":
			localConfig.oauth2 = user.oauth2(_).data;
			break;
		case "sage-id":
			break;
		default:
	...
	if (localConfig.method !== method) {
		throw authHelper.accessDenied("wrongAuth", user);
	}
	...
	switch (localConfig.source) {
		...
		case "ldap":
			...
			break;
		case "oauth2":
			if (serverName !== localConfig.oauth2.name) throw authHelper.accessDenied("wrongAuth", user);
			break;
		case "sage-id":
