...
exports.ensureAuthenticated = function(_, request, response) {
	...
	try {
		...
		httpStatus = 307; // redirect
		location = "/auth/login/page";
		session.loginError = "";
	} catch (ex) {
		httpStatus = ex.$httpStatus || 500;
	...
	// TODO: localize
	if (acceptHtml) {
		authHelper.redirect(_, response, location);
	} else {
		...
		// add fake www-authenticate header to be HTTP friendly.
		if (httpStatus === 401) headers['www-authenticate'] = "LoginPage";

		response.writeHead(httpStatus, headers);
		response.end(JSON.stringify({
			$diagnoses: [{
				$message: session.loginError || "authentication required",
				$severity: "error",
				$links: {
					continue: {
						$title: "login page",
