...
function getAuthModule(tok, standardSetting) {
	...
	switch (tok) {
		...
		case "sage-id":
			return require("streamline-auth/lib/sage-id").create(_userCheck, standardSetting);
	}
	return null;
}
...
exports.ensureAuthenticated = function(_, request, response) {
	...
	if (request.headers.authorization) {
		...
		var authModule = getAuthModule(tok, standardSetting);
...
function genLoginPage(_, request, response) {
	...
	html = html.replace(/<!--\s*auth-if\s+([\w-,]+)\s*-->([\s\S]*?)<!--\s*auth-endif\s*-->/g, function(all, toks, body) {
		return toks.split(',').some(function(tok) {
			return authMethods.indexOf(tok) >= 0;
		}) ? body : '';
	});
