...
exports.create = function(div, mode) {
	...
	return {
		setValue: function(val) {
			...
			root = refactor(val, "tree");

			function refactor(val, name) {
				var propertyNames = Object.keys(val);
				var data = {};
				data.name = name;
				if (propertyNames.length > 0) {
					data.children = propertyNames.map(function(elt, i) {
						return refactor(val[elt], elt);
					});
				} else if (typeof val !== "object") { //leaf case
					/*data.children.push({
                        name: val
                    });*/
					data.name += ": " + val;
			...
			var update = require("./indentedTree").updater({
				root: root,
				margin: margin,
				barHeight: barHeight,
				barWidth: barWidth,
