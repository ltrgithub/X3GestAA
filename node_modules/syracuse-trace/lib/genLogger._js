...
exports.genLogger = helpers.defineClass(function(logType, config) {
	...
	this.level = this.methods.indexOf((traceConfig && traceConfig.level) || this.defaultLevel);
	this.enabled = (traceConfig && traceConfig.enabled) || false;
	this.expiry = (traceConfig && traceConfig.expiry) || 120;
	this.transport = null;
	switch ((traceConfig && traceConfig.transport) || "console") {
		case "storagearea":
			this.transport = new saLogger(logType, this.expiry);
			break;
		case "mongo":
	...
	trace: function(_, type, len) {
		...
		return this.logMain(_, type, format, needstack, args);
	},
	debug: function(_) {
		var msg = arguments.length > 1 ? arguments[1] : null;
		return this.trace(_, 'debug', arguments.length, msg);
	},
	info: function(_) {
		var msg = arguments.length > 1 ? arguments[1] : null;
		return this.trace(_, 'info', arguments.length, msg);
	},
	warn: function(_) {
		var msg = arguments.length > 1 ? arguments[1] : null;
		return this.trace(_, 'warn', arguments.length, msg);
	},
	error: function(_) {
		var msg = arguments.length > 1 ? arguments[1] : null;
		return this.trace(_, 'error', arguments.length, msg);
	},
	close: function(_) {
