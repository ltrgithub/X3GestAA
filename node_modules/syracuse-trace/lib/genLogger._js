...
exports.genLogger = helpers.defineClass(function(config) {
	...
	_isTraceEnabled: function(severity, level) {
		if (this.name === 'test.1' || this.name === 'test.2') {
			console.log(this.name + " - Current: " + severity + "(" + traceHelper.methods.indexOf(severity) + ") - level setting : " + level + " - res: " + res);
		}
	...
	_trace: function(severity, msg, obj) {
		...
		Object.keys(self.records).forEach(function(recId) {
			...
			if (rec.level != null && self._isTraceEnabled(severity, rec.level)) {
				data = data || self._createLine(severity, msg, obj);
				if (self.name === 'test.1' || self.name === 'test.2') {
