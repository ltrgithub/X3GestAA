...
var genLogger = exports.genLogger = helpers.defineClass(function(config) {
	...
	isTraceEnabled: function(method) {
		...
		//this.enabled && console.log("Level: " + this.level + "/" + method + "(" + myLevel + ") Enabled: " + this.enabled + "Result: " + res);
		return res;
	},
	...
	_trace: function(_, severity, data) {
		...
		if (this.isTraceEnabled(severity) && data) {
			return writeFunnel(_, function(_) {
				...
				if (self._mustTrace) {
					...
					traceHelper.writer && traceHelper.writer.write(_, data);
				}
	...
	_traceAll: function(_, severity, msg, obj) {
		...
		if (sid && self.sessions[sid]) {
			...
			if (self.sessions[sid]._trace(_, severity, data)) return;
		}
		// global traces
