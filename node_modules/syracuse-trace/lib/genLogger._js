...
exports.GenLogger = helpers.defineClass(function(config) {
	...
	this.pausedRecords = {}; // data of paused records. Keys are uuids, values are the objects
	...
	// pause logging for this record
	pauseRecord: function(uuid) {
		var paused = deleteRecord(uuid);
		if (paused) this.pausedRecords[uuid] = paused;
	},
	// resume logging for this record
	resumeRecord: function(uuid) {
		var paused = this.pausedRecords[uuid];
		if (paused) addRecord0(paused);
	},
	deleteRecord: function(uuid) {
		var res = deleteRecord0(uuid);
		if (res) return res.queue;
		else return undefined;
	},
	// delete record with this uuid and maybe remove certain log levels. Returns the result.
	deleteRecord0: function(uuid) {
		var self = this;
	...
	// internal addRecord function
	addRecord0: function(element) {
		var uuid = element.uuid;
		var sid = element.sid;
		var ilevel = methods.indexOf(element.level);
		if (ilevel < 0) throw new Error("Wrong log level " + element.level);
		if (!element.queue) throw new Error("No queue in record " + uuid);
