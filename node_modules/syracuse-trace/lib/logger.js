	...
	_config.methods.forEach(function(title, i) {
		...
		_self[title] = function() {
			if (arguments.length === 0) {
				...
				var traceConfig = x3helper.config(_config.logType, baseConfig);
				var levelIndex = _config.methods.indexOf(traceConfig.level);

				// Can't use this to trace itself, that would be silly
				// console.log("Function name: <" + title +"> Enabled: <" + traceConfig.enabled + "> Level: <" + traceConfig.level + "><" + levelIndex + "> (i)<" + i + ">");

				return traceConfig.enabled && levelIndex <= i;
			}
