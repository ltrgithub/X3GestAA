...
var genLogger = exports.genLogger = helpers.defineClass(function(config) {
	...
	this.stackIndex = (config && config.stackIndex) || 2; // get the specified index of stack as file information. It is useful for development package.
	...
	_createLine: function(type, format, needstack, args) {
		var data = {
		...
		this.transport && this.transport.preprocess(data);
	...
	isTraceEnabled: function(method) {
		...
		var res = this.enabled && this.level && myLevel >= this.level;
		...
		Object.defineProperty(this, type, {
			get: function() {
				...
				res = Object.keys(self.sessions).some(function(sessId) {
					var _res = self.sessions[sessId].isTraceEnabled(type);
					_res && delete self.sessions[sessId][type];
					return _res;
				}) || res;
				// If at least one tracer is enabled, the property is removed
				res && delete self[type];
				return res;
			},
			configurable: true
		});
	},
	trace: function(type, data) {
		if (this.isTraceEnabled(type) && data) {
			// transport the final result
			this.transport && this.transport.trace(data);
		}
		this._defineProperty(type);
	},
	traceAll: function(type, msg) {
		var format = this._config.format;
		var needstack = /\{\{(method|path|line|pos|file)\}\}/i.test(format);
		var data = this._createLine(type, format, needstack, [msg]);
		var self = this;
		try {
			Object.keys(this.sessions).forEach(function(sessId) {
				self.sessions[sessId].trace(type, data);
			});
		} finally {
			this.trace(type, data);
		}

	},
	debug: function(msg) {
		this.trace('debug', msg);
	},
	info: function(msg) {
		this.traceAll('info', msg);
	},
	warn: function(msg) {
		this.trace('warn', msg);
	},
...
exports.sessLogger = helpers.defineClass(function(config) {
	...
	this.stackIndex = (config && config.stackIndex) || 4; // get the specified index of stack as file information. It is useful for development package.
	this.sid = config.sid;
}, genLogger, {
	_loadConfig: function(config, mod) {
