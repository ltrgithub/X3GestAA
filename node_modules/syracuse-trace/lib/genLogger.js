...
var genLogger = exports.genLogger = helpers.defineClass(function(config) {
	...
	traceAll: function(type, len, msg) {
		var self = this;
		var mustTrace = false;
		try {
			Object.keys(this.sessions).forEach(function(sessId) {
				if (self.sessions[sessId].trace(type, len, msg)) mustTrace = true;
			});
		} finally {
			if (this.trace(type, len, msg)) mustTrace = true;
			return mustTrace;
		}

	},
	debug: function(msg) {
		return this.traceAll('debug', arguments.length, msg);
	},
	info: function(msg) {
		return this.traceAll('info', arguments.length, msg);
	},
	warn: function(msg) {
		return this.traceAll('warn', arguments.length, msg);
	},
	error: function(msg) {
		return this.traceAll('error', arguments.length, msg);
	},
	toJSON: function() {
		return {
			name: this.name,
