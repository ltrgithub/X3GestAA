...
exports.saLogger = helpers.defineClass(function(name, expiry) {
	...
	_getLogFile: function() {
		// Work in Progress - not sure if I want to do it this way or not yet
		var self = this;
		var today = new Date();
		var fName = "Trace_" + this.name + "_" + today.getDayOfYear();
		if (!self.log) {
			self.log = {};
			var filter = {
				jsonWhere: {
			...
			if (self.fileName !== fName) {
				// File name has changed - new day, close the old log and open a new one
			}
		}
	},
	initialise: function() {
		...
		if (!self.log) {
			var today = new Date();
			var fName = "Trace_" + this.name + "_" + today.getDay();
			var filter = {
				jsonWhere: {
					fileName: fName
				}
			...
			self.log.properties.content.fileName = fname;
			self.log.dd = sa.open(cb, filter, null, "serverLog");
			if (!self.log.dd) {
				console.log("Failed to open the serverLog file by fileName");
				self.log.dd = sa.open(cb, null, null, "serverLog");
			} else {
				console.log("Opened - " + self.log.dd);
			}
			...
			if (!self.log.dd) {
				console.log("Failed to re-open the serverLog file");
			} else {
				sa.write(cb, self.log.dd, self.log.properties, "=== " + self.name + " Trace Initialised ===\n", true);
			}
	...
	trace: function(data) {
		try {
			...
			if (t) {
				// Never write null to the stream or it will be closed automatically
				sa.write(cb, t.dd, t.properties, (data.output || " ") + "\n", true);
			}
		} catch (e) {
