...
function enableConsole(_, name) {
	...
	var tracesConfig = config && config.traces && config.traces;
	var defLevel = "error";
	...
	if (name.indexOf('.') !== -1) {
		...
		tracerName = parts[0];
		moduleName = parts[1];
		level = tracesConfig.console ? (tracesConfig.levels && tracesConfig.levels[tracerName] && tracesConfig.levels[tracerName][moduleName]) || defLevel : defLevel;
	} else {
		tracerName = name;
		level = tracesConfig.console ? (tracesConfig.levels && tracesConfig.levels[tracerName]) || defLevel : defLevel;
	}
