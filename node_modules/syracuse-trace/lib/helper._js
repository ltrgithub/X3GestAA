...
function enableConsole(_, name) {
	...
	if (name.indexOf('.') !== -1) {
		...
		tracerName = parts[0];
		moduleName = parts[1];
		level = (config && config.traces && config.traces.levels && config.traces.levels[tracerName] && config.traces.levels[tracerName][moduleName]) || "error";
	} else {
		tracerName = name;
		level = (config && config.traces && config.traces.levels && config.traces.levels[tracerName]) || "error";
	}
	tracers[name].addRecord(helpers.uuid.generate(), {
		level: level,
		queue: q
	});
...
exports.getTracer = function(name) {
	if (!tracers[name]) {
		...
		if (config && config.traces && config.traces.console) {
