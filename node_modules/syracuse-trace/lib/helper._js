...
var genLogger = require('./genLogger');
...
var startCache;
if (!(config.traces && config.traces.console)) startCache = [];


var consoleQueue0 = {
	put: function(n) {
		var tenantId = globals.context.tenantId;
		console.error(n.stamp + " | " + n.severity + " | " + n.category + (tenantId ? " | " + tenantId : "") + " | " + n.message + (n.stack ? " | " + n.stack : ""));
...
require('@sage/syracuse-core').getTracer = exports.getTracer;
...
function _getCollaboration(_) {
	if (!adminHelper) adminHelper = require('@sage/syracuse-lib/src/collaboration/helpers').AdminHelper;
...
var manageRunningRecords = function(_, start) {
	var db = _getCollaboration(_);
	var names = {};
	// only search for finished records on local host
...
exports.computeSize = function(size) {
	...
	return 10240000; // 10 Mo by default
};
