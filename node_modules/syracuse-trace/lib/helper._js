...
var consoleQueue0 = {
	put: function(n) {
		console.error(n.stamp + " | " + n.severity + " | " + n.message + (n.stack ? " | " + n.stack : ""));
		return true;
	}
};
...
var manageRunningRecords = function(_, start) {
	...
	while (record = cursor.next(_)) {
		...
		if (status === "running" || status === "paused") {
			...
			if (mustStop) {
				record.unlockInstance(_); // maybe a lock has remained because of server crash...
...
function _launchAutoTraceRecord(_) {
	...
	if (!devMode || !consoleMode) {
		...
		autoRecord.startRecording(_, false, true, !consoleMode); // do not save, unset console
		autoRecord.save(_);
		return;
	}
};
...
var checkNodelocal = function() {
	if (config && config.traces && config.traces.maxSize) {
		...
		if (exports.computeSize(config.traces.maxSize) < 10000000) {
			config.traces.maxSize = 10000000;
			console.error("Traces maxSize is too small - will be changed to 10M");
		}
	}
};
