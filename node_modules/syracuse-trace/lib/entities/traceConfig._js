			...
			function(_, instance) {
				if (instance.modules(_).getLength() > 0) {
					instance.modules(_).toArray(_).forEach_(_, function(_, mod) {
						loadTracerConfig(_, instance, instance.name(_), mod.name(_));
					});
				} else {
					loadTracerConfig(_, instance, instance.name(_));
				}
			}
		]
	}
