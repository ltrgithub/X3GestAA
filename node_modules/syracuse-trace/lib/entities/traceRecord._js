...
exports.entity = {
	...
	$functions: {
		...
		concatContents: function(_, t) {
			...
			t.progress = 100;
			var result = Buffer.concat(outstream._data);
			/* unzip example. Several usual .gz files are now in one zip archive
			var unzip = require('streamline-zip/lib/unzip/unzip');
			var unzipper = new unzip.Unzip(result, function(filename, filecontent, headers, _) {
				console.log("Contained file: "+filename+" content length: "+filecontent.length);
			});			
			unzipper.unzip(_);
			*/
			return result;
