...
exports.entity = {
	...
	$properties: {
		...
		status: {
			...
			$enum: [{
				...
				$title: "halted", // special status for instances which have been halted because the process has finished
				$value: "halted",
			}, {
				$title: "paused",
				$value: "paused",
	...
	$functions: {
		...
		// create a new instance from this instance (only for manual records)
	...
	$services: {
		...
		testAll: {
			...
			$execute: function(_, context, instance) {
				...
				if (!instance.onlySid(_)) {
					tracer && tracer("test rec: " + _propagate(_, "POST", "test?seq=" + instance.sequence(_)));
				}
			},
		},
		start: {
...
function stopInternal(_, sequence, instance, noUnset, halt) {
	...
	if (status !== "error" && status !== "complete") {
		...
		try {
			...
			instance.status(_, halt ? "halted" : "complete");
			instance.log(_, locale.format(module, "recordCompleted"));
		} catch (e) {
			instance.status(_, halt ? "halted" : "error");
