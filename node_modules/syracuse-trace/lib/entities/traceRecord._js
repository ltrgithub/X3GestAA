...
function setRecord(_, instance) {
	...
	instance.configs(_).toArray(_).forEach_(_, function(_, inst) {
		...
		var day = instance.auto(_) ? instance.stamp(_).day : null;
		var sid = instance.onlySid(_) ? globals.context.session && globals.context.session.id : null;
		if (conf.modules) {
			conf.modules.forEach(function(mod) {
				traceHelper.getTracer(name + "." + mod.name).addRecord(uuid, mod.level, tasks[uuid].q, sid);
			});
		} else {
			traceHelper.getTracer(name).addRecord(uuid, conf.level, tasks[uuid].q, sid);
		}
	});
}
...
function unsetRecord(_, instance) {
	instance.configs(_).toArray(_).forEach_(_, function(_, inst) {
		...
		if (conf.modules) {
			conf.modules.forEach(function(mod) {
				traceHelper.getTracer(name + "." + mod.name).deleteRecord(instance.$uuid);
			});
		} else {
			traceHelper.getTracer(name).deleteRecord(instance.$uuid);
		}
	});
}
