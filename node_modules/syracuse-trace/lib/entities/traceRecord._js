...
exports.entity = {
	...
	$properties: {
		...
		onlySid: {
			...
			$description: "Traces information corresponding to creator's session only",
			$type: "boolean",
			$default: true,
			$isReadOnly: function(_, instance) {
		...
		isSessionConcerned: {
			$title: "Is current session concerned",
			$type: "boolean",
			$isReadOnly: true,
			$compute: function(_, instance) {
	...
	performChange: function(_, newDate, oldInstance) {
		...
		try {
			...
			if (oldInstance) {
				...
				console.error("Previous running record [" + self.uuid + "] instance not available : " + self.sequence);
			}
