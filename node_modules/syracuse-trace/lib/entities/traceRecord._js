...
exports.entity = {
	...
	$properties: {
		...
		isSessionConcerned: {
			...
			$compute: function(_, instance) {
				return instance.running(_) && (!instance.onlySid(_) || (instance.sessionId(_) === (globals.context.session && globals.context.session.id)));
			}
		},
		sessionId: {
	...
	$services: {
		start: {
			...
			$isDefined: function(_, instance) {
				return !instance.auto(_) && ["complete", "error"].indexOf(instance.status(_)) !== -1;
			},
		...
		flush: {
			...
			$isDefined: function(_, instance) {
				return ["complete", "error"].indexOf(instance.status(_)) !== -1;
			},
			$execute: function(_, context, instance) {
	...
	$events: {
		$beforeSave: [
			...
			function(_, instance) {
				...
				} else if (status === "created" && !instance.auto(_) && instance.pid(_) === process.pid) {
					var pref = "Record_";
					if (globals.context && globals.context.session && globals.context.session.sessionInfo) {
						pref = pref + globals.context.session.sessionInfo.userName(_) + "_";
					} else {
						pref = pref + "UNKNOW_";
					}
