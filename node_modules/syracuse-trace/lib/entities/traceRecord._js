...
function setRecord(_, instance) {
	function _createRecord(_instance, _mod) {
		return {
			queue: queues[_instance.$uuid],
			config: {
				enabled: _mod.enabled,
				level: traceHelper.methods.indexOf(_mod.level)
			}
		};
	}
	//
	instance.configs(_).toArray(_).forEach_(_, function(_, inst) {
		...
		if (conf.modules) {
			conf.modules.forEach_(_, function(_, mod) {
				//if (mod.enabled) {
				traceHelper.getTracer(name + "." + mod.name).records[instance.$uuid] = _createRecord(instance, conf);
				//}
			});
		} else {
			traceHelper.getTracer(name).records[instance.$uuid] = _createRecord(instance, conf);
		}
		//}
	});
}
...
exports.entity = {
	...
	$properties: {
		...
		auto: {
			...
			$propagate: function(_, instance, val) {
				if (val) instance.startRecording(_);
			}
		},
		pid: {
			$title: "Process ID",
	...
	$functions: {
		...
		abortRecording: function(_) {
			// TODO: This implementation doesn't works !!! 
			// If the server crash the queue disappear and nothing has been added to the record.
			// Closing the GridFS is not sufficient...
			this.recording(_).close(_);
			this.status(_, "complete");
			this.log(_, "recording aborted");
			this.save(_);
		},
