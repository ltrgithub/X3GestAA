...
exports.entity = {
	...
	$functions: {
		...
		startNewContent: function(_) {

			function renameOldAutoRecord(_) {
				var recs = self.contents(_).toArray(_).sort_(_, function(_, a, b) {
					if (a.start(_) > b.start(_))
						return 1;
					if (a.start(_) < b.start(_))
						return -1;
					return 0;
				...
				if (recs.length >= maxFiles) {
					self.contents(_).deleteInstance(_, recs.shift().$uuid);
				}
			...
			var contentEntity = db.model.getEntity(_, "traceContent");

			renameOldAutoRecord(_);
			var content = contentEntity.createInstance(_, db, null);
			content.name(_, this.name(_));
			content.status(_, "running");
			this.contents(_).set(_, content);




			// create output stream
			var writer = content.recording(_).createWritableStream(_, {
				"contentType": "application/x-trace",
				"contentEncoding": "gzip",
				"fileName": "Auto_record_content_" + content.start(_) + "_PID-" + process.pid + ".gz",
				"append": "w+"
			});
			...
			var taskUuid = content.$uuid;
			console.log("CREATE NEW SUB RECORD : " + taskUuid);
			this.sequence(_, taskUuid);
			var day = (new Date(content.start(_))).getUTCDate();
			tasks[taskUuid] = {
				len: 0,
				ended: false,
				q: q,
				changeNeeded: function(data) {
					if (data && data.stamp && (new Date(data.stamp)).getUTCDate() !== day) {
						return 1;
					}
					return 0;
				},
				performChange: function(_, data, changeStatus) {
					// exchange trace record with new record
					var inst = tasks[taskUuid];
					if (self.auto(_)) {
						// If day changed, the current auto record must be set as completed, and a new one must be created
						if (changeStatus === 1) {
							console.log("PERFORM DAY CHANGE");
							self.auto(_, false);
							self.finishRecording(_);
							inst = tasks[traceHelper.launchAutoTraceRecord(_)];
						}
			...
			if (isAuto) {
				// Apply special mapper for auto records to be able to rotate on maxSize
				q.reader = q.reader.map(function(_, r) {
					if (!tasks[taskUuid]) {

						// @ERMUE : I have a problem here !!! I don't why I don't retrieve the task...
						// I will have a look later
						var msg = "Task not retrieved !!!!";
						console.error(msg);
						throw new Error(msg);
					}

					tasks[taskUuid].len += r.length;
					console.log("LEN: " + tasks[taskUuid].len);
					if (tasks[taskUuid].len > maxSize) {
						if (!tasks[taskUuid].ended) {
							tasks[taskUuid].ended = true;
							self.stopCurrentContent(_);
							self.startNewContent(_);
						}
					}
					return r;
				});
		...
		stopCurrentContent: function(_) {

			// Write EOF
			var uuid = this.sequence(_);
			if (tasks[uuid] && tasks[uuid].q) {
				tasks[uuid].q.write(_);
				this.contents(_).get(_, this.sequence(_)).status(_, "complete");
				this.save(_);
			} else {
		...
		startRecording: function(_) {
			...
			// @emueller: I commented this lines to resolve conflict. I let you do it as you think.
			//			var self = this;
			...
			var taskUuid = this.startNewContent(_);
		...
		finishRecording: function(_, err) { // EM: new implementation for auto records: should set a flag so that changeNeeded is true and put a message. Implementation must be synchronous
			...
			if (!err) {
				this.stopCurrentContent(_);
				if (this.auto(_)) {
					// Launch another auto record

					this.startNewContent(_);
				} else {
...
function _makeAutoQueue(task) {
	...
	return {
		put: function(data) {
			...
			var changeStatus = changeNeeded(data);
			if (changeStatus) {
				temp = [data];
					...
					performChange(_ >> function(err, inst) {
						if (err) { // change does not work: continue logging to console.error
							...
							if (temp1.length) {
								...
								if (!queue.put(item)) { // put first item directly into queue to avoid 'changeNeeded'
									console.error(JSON.stringify(item));
								}
