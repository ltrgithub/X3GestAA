...
exports.entity = {
	...
	$functions: {
		...
		concatContents: function(_) {
			var reader = ez.devices.generic.empty.reader,
				writer = ez.devices.buffer.writer();
			var i = 0;
			this.contents(_).toArray(_).forEach_(_, function(_, content) {
				if (content.recording(_).fileExists(_)) {
					reader = reader.concat(content.recording(_).createReadableStream(_) //
			...
			reader.transform(ez.transforms.json.formatter()) //
			.map(ez.mappers.convert.bufferify()) //
	...
	$services: {
		...
		download: {
			...
			$execute: function(_, context, instance) {
				...
				var recs = instance.concatContents(_);
...
function _startRecording(_, instance, pause) {
	...
	performChange: function(_, newDate, oldInstance) {
		...
		try {
			...
			if (newDate) {
				...
				var writer = newInstance.startNewContent(_, self.index, self.maxFiles);
				...
				var writer = oldInstance.startNewContent(_, self.index, self.maxFiles);
			...
			adminHelper.getCollaborationOrm(_).unlockInstance(_, oldInstance);
...
function startInternal(_, uuid) {
	...
	_startRecording(_, copy, false, true);
...
function _refreshInstance(_, instance, statusError) {
	...
	var currentTranscript = currentInstance.transcript(_);
...
function changeInternal(_, sequence, instance) {
	...
	traceHelper.lockInstanceRetry(copy);
	try {
