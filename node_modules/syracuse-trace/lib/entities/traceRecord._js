...
exports.entity = {
	...
	$functions: {
		...
		haltRecording: function(_) {
			stopInternal(_, this.sequence(_), this, false, true);
		},
		// create a new instance from this instance (only for manual records without 'onlySid')
		duplicateManual: function(_) {
			var newInstance = this.getEntity(_).createInstance(_, this._db, null);
			...
			configs.reset(_);
			_copyConfig(_, newInstance.configs(_), this.configs(_));
			return newInstance;
		},
		// return a writer
		startNewContent: function(_, index, maxFiles) {
			...
			var contentPrefix = this.auto(_) ? "Auto_record_content_" : child.name(_) + "_";
			// create output stream
			var writer = child.recording(_).createWritableStream(_, {
				"contentType": "x-trace",
				"contentEncoding": "gzip",
				"fileName": contentPrefix + timestamp + "_" + hostname + "-" + process.pid + ".gz",
				"append": "w+"
			});
			this.save(_);
			var diags = [];
		...
		startRecording: function(_, pause) {
			_startRecording(_, this, "mockServer" in config, pause);
		},
		errorStopped: function(_, err) {
			this.status(_, "error");
			this.log(_, err);
			this._noPropagate = true;
			this.save(_);
		},
	},
	$services: {
		...
		flush: {
			...
			$execute: function(_, context, instance) {
				...
				if (!instance.onlySid(_)) {
					tracer && tracer("test rec: " + _propagate(_, "POST", "flush?seq=" + instance.sequence(_)));
				}
			},
		},
		testAll: {
			...
			$execute: function(_, context, instance) {
				...
				if (!instance.onlySid(_)) {
					tracer && tracer("test rec: " + _propagate(_, "POST", "test"));
				}
			},
		},
		start: {
			...
			$execute: function(_, context, instance) {
				...
				if (!instance.onlySid(_)) {
					tracer && tracer("start rec: " + _propagate(_, "POST", "start?uuid=" + instance.$uuid));
				}
			},
		},
		stop: {
			...
			$execute: function(_, context, instance) {
				// propagate changes only for session independent loggers
				if (!instance.onlySid(_)) {
					tracer && tracer("stop rec: " + _propagate(_, "POST", "stop?seq=" + instance.sequence(_))); // stop other instances first
				}
		...
		pause: {
			...
			$execute: function(_, context, instance) {
				// propagate changes only for session independent loggers
				if (!instance.onlySid(_)) {
					tracer && tracer("pause rec: " + _propagate(_, "POST", "pause?seq=" + instance.sequence(_))); // pause other instances first
				}
		...
		resume: {
			...
			$execute: function(_, context, instance) {
				...
				// propagate changes only for session independent loggers
				if (!instance.onlySid(_)) {
					tracer && tracer("resume rec: " + _propagate(_, "POST", "resume?seq=" + instance.sequence(_)));
				}
			},
		},
	},
				...
				if (status === "running" || status === "paused") {
					...
					if (!_updateQueue(_, queue, instance)) instance._noPropagate = true;
						...
						if (!instance._noPropagate && instance.auto(_)) {
							...
							if (confs) {
								confs.reset(_);
								_copyConfig(_, confs, instance.configs(_));
								settings.save(_);
		...
		$afterSave: [
			...
			function(_, instance) {
				...
				if (!instance._noPropagate && (status === "running" || status === "paused") && !instance.onlySid(_)) {
					tracer && tracer("change rec: " + _propagate(_, "POST", "change?seq=" + instance.sequence(_) + "&uuid=" + instance.$uuid));
				}
			}
		]
...
function initConfig(_, instance) {
	...
	var settings = db.fetchInstances(_, db.model.getEntity(_, "traceSetting"))[0];
	var confs = settings && settings.configs(_);
	if (confs) _copyConfig(_, instance.configs(_), confs);
}

// make and register a queue, notify the loggers 
function _startRecording(_, instance, cluster, pause) {
	...
	queue = new AutoQueue(queue, serialized, currentDay);
...
var AutoQueue = coreHelpers.defineClass(function(queue, serialized, currentDay) {
...
function _copyConfig(_, copyConfig, origConfig) {
	copyConfig.reset(_);
	origConfig.toArray(_).forEach_(_, function(_, c) {
		copyConfig.set(_, c);
	});
}
...
function startInternal(_, uuid) {
	...
	_copyConfig(_, copy.configs(_), instance.configs(_));
...
exports.pauseExternal = function(_, sequence) {
	pauseInternal(_, sequence, _getInstanceFromSequence(_, sequence));
};
...
exports.resumeExternal = function(_, sequence) {
	resumeInternal(_, sequence, _getInstanceFromSequence(_, sequence));
};
...
function changeInternal(_, sequence, instance) {
	...
	console.error("CI " + sequence + " " + instance.$uuid + " " + queue.uuid);
...
exports.changeExternal = function(_, sequence, uuid) {
	changeInternal(_, sequence, _fetchRecord(_, uuid));
};
...
exports.flushExternal = function(_, sequence) {
	flushInternal(_, sequence, _getInstanceFromSequence(_, sequence));
};
...
exports.stopExternal = function(_, sequence) {
	stopInternal(_, sequence, _getInstanceFromSequence(_, sequence));
};
