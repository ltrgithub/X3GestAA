...
function checkConcurrent(session, role, userName, _, device, diagnoses, newSession) {
	...
	if (!_evaluateBadges(badges, data, badgeInfo, productInfo, true, device, diagnoses, otherBadges)) {
		...
		session.sessionInfo.lastAccess(_, datetime.fromJsDate(session.lastAccess));
...
function _usedLicenses(_, db, totalBadges, totalProducts, otherBadges, session, userName, serverNames, badgeCounts, productCounts) {
	...
	function addSession(sessionKey, item, counts) {
	...
	function addSessionBadges(sessionKey, badges) {
		var currentProducts = {};
		badges.forEach(function(badge) {
			addSession(sessionKey, badge, badgeCounts);
			var data = totalBadges[badge];
			var product = (data ? data[1] : otherBadges[badge]);
		...
		Object.keys(currentProducts).forEach(function(product) {
			addSession(sessionKey, product, productCounts);
		})
	}
	...
	sessionInfos.forEach_(_, function(_, sessionInfo) {
		...
		addSessionBadges(sessionKey, badges);
	});
