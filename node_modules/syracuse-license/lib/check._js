...
function _step(_, length, poolMetadata, data, key, licenseData, timeString) {
	...
	if (!pd.licenseStart) {
		if (poolMetadata && poolMetadata.product) {
			...
			pd.size = (params.WS_SIZE_LIMIT || 10) * (1 << 20);
		...
		if (pd.counter) {
			...
			pd.counter = +length;
		pd.other = pd.other || 0;
		pd.status = 0; // 0: OK, 1: exceeds grace limit, 2: exceeds total limit
	...
	if (timeString.indexOf(pd.timeString) !== 0) {
		if (timeString.indexOf(pd.timeString.substr(0, pd.length)) !== 0) {
			...
			if (pd.status < 2) pd.counter += +length;
		}
	...
	//console.log("FFg " + (pd.counter + pd.total));
	if ((pd.counter + pd.total) > pd.size) {
		if ((pd.counter + pd.total) > pd.graceLimit) {
			//console.log("FF " + (pd.counter + pd.total));
			return -1; // over grace limit: forbid use
		} else
		// slow down
		if (pd.status < 1 && !licenseData) {
			_fireEvent(_, true, pd, productCode);
			pd.status = 1;
		}
...
function _fireEvent(_, graceLimit, data, productCode) {
	var whereClause = {
		code: graceLimit ? "license_web_grace" : "license_web_max"
	};
	adminHelper = adminHelper || require('syracuse-collaboration/lib/helpers').AdminHelper;
