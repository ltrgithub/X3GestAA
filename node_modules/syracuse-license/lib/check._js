...
//		var badge = badges[j].code(_); 
	//	if (badgeCodes.indexOf(badge) < 0) badgeCodes.push(badge);
	}
...
/// userName: default value for user name (if not found in session)
...
function _remainingLicenses(_, db, parsedData, badgeCodes, session, userName, serverNames) {
	...
	// })
	//console.log(userName+"Total "+util.format(badgeCount))
	// count already used badges
	console.log("BADGECODES "+util.format(badgeCodes))
	...
	while (--i >= 0) {
		...
		badges = (sessionInfos[i].badge(_) || "").split(',');
		clientId = sessionInfos[i].clientId(_)+"_"+sessionInfos[i].userName(_);
		console.log(sid+" "+util.format(badges)+" "+sessionInfos[i].serverName(_))
		// var clientId = sessionInfos[i].clientId(_)+"_"+sessionInfos[i].userName(_);
		console.log("Client ID "+clientId)
		var j = badges.length;
		while (--j >= 0) {
			var badge = badges[j];
			console.log("BADDDDDGGGGGGGG "+badge)
			if (!badge) continue;
			...
			console.log("ADD1 "+badge+" "+clientId)
		}
	}
	
	// add data of current session
	var clientId = session.clientId+"_"+(userName || session.getData("userId"))
	console.log("Client ID2 "+clientId)
	for (var badge in badgeCodes) {
		...
		if (!(clientId in sessionBadges[badge]))
			...
			sessionBadges[badge][clientId]++;
		console.log("ADD12 "+badge+" "+clientId)
	}
