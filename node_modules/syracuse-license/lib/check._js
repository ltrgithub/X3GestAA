	...
	contents.forEach(function(content) {
		...
		switch (parsed.fileType) {
			case "Partner":
				...
				if (parsed.partnerId && (parsed.partners.length !== 1 || parsed.partners[0].partnerId !== parsed.partnerId))
					throw new Error("Partner file of partner " + parsed.partnerId + " defines too many or foreign public keys");
...
function _getX3LicenseInfoInt(productCode, productVersion, badges, parsedData, diagnoses) {
	...
	for (var code in parsedData.products) {
		...
		if (!correctProduct) { // no badge for correct product
			if (diagnoses) diagnoses.push({
				$severity: "error",
				$message: locale.format(module, "noBadgeAvail", productCode)
			});
			return undefined;
		}
		return result;
...
function checkConcurrent(session, role, userName, _, device, diagnoses, newSession) {
	...
	var serverNames = {};
	var sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, newSession ? undefined : unlicensedBadges, !newSession);
	if (sessionInfos != null) { // maybe old sessions have been counted
		...
		sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, unlicensedBadges, false);
		if (sessionInfos != null) { // second attempt: try to delete oldest session of same user with same client ID
			...
			sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, unlicensedBadges, true);
		}
	}
...
function _remainingLicenses(_, db, totalBadges, totalProducts, session, userName, serverNames, unlicensedBadges, allowNoBadges) {
