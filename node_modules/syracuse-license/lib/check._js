...
function updateFromDB(_) {
	try {
		...
		if (e instanceof Error) // do not log message that there is no database configuration
			 { console.log("TTTT"+e); console.log(util.inspect(e, false, 10)) }
	}
}
...
function _manageLicenseData(_, newLicenses, doNotStore, diagnoses, checkNamed) {
	...
	if (result === data && diagnoses)
		diagnoses.push({severity: "warning", message: locale.format(module, "NoNewLicense")})
	// store new content
	try {
		l.license(result);
	} catch (e) {
...
function _parseLicenses(data, newLicenses, diagnoses) {
	...
	if ((!newLicenses || newLicenses.length === 0) && data[1] && (data[1].nextCheck >= currentDateString && data[1].previousCheck <= currentDateString))
		return data;
	var licenseData = {};
	var policyData = {};
	var grouped = {};
	...
	if (!newLic && data[1] && (data[1].nextCheck >= currentDate && data[1].previousCheck <= currentDate))
		return data;
	...
	for (var i = 0; i<partners.length; i++) {
		...
		parsed.modules.forEach(function (module0) {
			if (module0.code in licensedModules) {
				if (diagnoses) diagnoses.push({severity: "warning", message: locale.format(module, "ignoredM", module0.code, key)})
				return;
			}
			...
			if (module0.validity[1]) {
				...
				if (valid) expires.push({module: module0.code, expire: module0.validity[1], partner: parsed.partnerId}); 
			}
			// module is licensed
		...
		parsedPolicy.modules.forEach(function (module0) {
			if (module0.code in policyModules) {
				if (diagnoses) diagnoses.push({severity: "warning", message: locale.format(module, "ignoredM2", module0.code, key)})
				return;
			}
		...
		parsed.badges.forEach(function (badge) {
			if (badge.code in infos.badges) {
				if (diagnoses) diagnoses.push({severity: "warning", message: locale.format(module, "ignoredB", badge.code, key)})
				return;
			}
			badgesInLicense[badge.code] = "";
			infos.badges[badge.code] = {max: badge.max, func:[], title: badge.title };
		});
...
function _checkNamedInt(_, parsed, inst, diagnoses) {
	...
	var cursor = db.createCursor(_, db.model.getEntity("user"), null);
	var user;
	var usedBadges = {};
	...
	Object.keys(usedBadges).forEach(function(badge) {
		if (badge in parsed.badges) {
			...
			if (diff > 0) {
				if (diagnoses) {
					diagnoses.push({severity: "error", message: locale.format(module, "namedTooMuch", badge, diff)})
				}
				ok = false;
...
function checkConcurrent(session, role, userName, _) { 
	...
	if (sessionInfos != null && !("mockServer" in config)) { // maybe old sessions have been counted
		var ok = false;
...
function _remainingLicenses(_, db, parsedData, badgeCodes, session, userName, serverNames) {
	...
	while (--i >= 0) {
		...
		if (sid === session.id || !sessionInfos[i].badge(_))
			continue;
		if (serverNames)
		...
		while (--j >= 0) {
			...
			if (!sessionBadges[badge])
				sessionBadges[badge] = {};
			if (!(clientId in sessionBadges[badge]))
				sessionBadges[badge][clientId] = 1;
			else
				sessionBadges[badge][clientId]++;
		}
	}
...
function _putServer(callback, name, method, path, content) {
	if (!callback) return fut.future(_putServer, arguments, 0);
	var parts = name.split(':');
	var options = {hostname: parts[0], port: parts[1] || 80, path: path, method: method}
	...
	req.on("error", function(e,d) { console.log("REQUESTERROR"+e+" "+name+" "+path+" "+content); return callback(null, false)});
...
function _propagate(_, method, path, content) {
	if ("mockServer" in config) {
		var options = { path: "/nannyCommand/notifyAll"+path, method: method, hostname: "", port: 0, headers: {}};
		options.headers[mock.BALANCER_HEADER] = config.port
		console.log("11oooooooooooooooooMock client"+util.format(options))
		try {
...
exports._s = function(content) {
	...
	var parts = content.split(/\}[^\{\},]*[\n\r][^\{\},]*\{/).map(function(part){ return "{"+part+"}"; });
	return parts;
}
...
exports.licenseChange = function(content, session, diagnoses, _) {
	...
	console.log("MANAGE");
	if (!_manageLicenseData(_, exports._s(content), false, diagnoses))
