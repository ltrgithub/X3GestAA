...
function getFunctionsAndLicenses(session, _) {
	...
	if (badgeString)
		badgeString.split(",").forEach(function(badge) {
...
function checkNamed(_, inst, messages) {
	...
	Object.keys(usedBadges).forEach(function(badge) {
		if (badge in data.badges) {
			if (usedBadges[badge] > data.badges[badge].max) {
				if (messages) {
					messages.put(locale.format(module, "namedTooMuch", badge, usedBadges[badge] - data.badges[badge].max))
				}
				ok = false;
				console.log(badge+" more "+usedBadges[badge]+" "+data.badges[badge].max)
			} else
				console.log(badge+" OK "+usedBadges[badge]+" "+data.badges[badge].max)
		} else {
			if (messages) {
				messages.put(locale.format(module, "unlicensedBadge", badge))				
			}
			ok = false;
...
exports.licenseChange = function(content, session, _) {
