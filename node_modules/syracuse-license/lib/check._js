...
function checkConcurrent(session, role, userName, _, device, diagnoses, newSession) {
	...
	var serverNames = {};
	var sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, newSession ? undefined : unlicensedBadges, !newSession, otherBadges, data.badges);
	if (sessionInfos != null) { // maybe old sessions have been counted
		...
		sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, unlicensedBadges, false, otherBadges, data.badges);
		if (sessionInfos != null) { // second attempt: try to delete oldest session of same user with same client ID
			...
			sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, unlicensedBadges, true, otherBadges, data.badges);
		}
	}
...
function _remainingLicenses(_, db, totalBadges, totalProducts, session, userName, serverNames, unlicensedBadges, allowNoBadges, otherBadges, licBadgeInfo) {
	...
	if (unlicensedBadges && unlicensedBadges.length) {
		// maybe the functions of some unlicensed badges are included in other badges - then these badges need not be unlicensed
		// any more
		var allowedFunctions = ",";
		allowedBadges.forEach(function(badge) {
			allowedFunctions += licBadgeInfo[badge].func.join(",") + ",";
		});
		for (var i = unlicensedBadges.length - 1; i >= 0; i--) {
			if (!licBadgeInfo[unlicensedBadges[i]].func.some(function(item) {
				return allowedFunctions.indexOf("," + item + ",") < 0
			})) {
				tracer && tracer("Removed unlicensed badge " + unlicensedBadges[i]);
				unlicensedBadges.splice(i, 1);
			}
		}
		if (!unlicensedBadges.length) allLicensed = true;
	}
