...
function _preParseLicenses(contents) {
	...
	contents.forEach(function(content) {
		...
		switch (parsed.fileType) {
			case "Partner":
				if (parsed.partnerId) throw new Error(locale.format(module, "partnerFile", parsed.partnerId));
				result.partnerContents = content;
				break;
			case "Policy":
				if (parsed.partnerId && !parsed.baseProduct) throw new Error(locale.format(module, "noBaseProduct", parsed.product.code, parsed.partnerId));
				if (parsed.sessionTypes) {
					var devices = {};
					parsed.sessionTypes.forEach(function(type) {
						type.devices.forEach(function(device) {
							if (device in devices) {
								throw new Error(locale.format(module, "duplicateDevice", device, parsed.product.code));
							}
						});
					});
				}
...
function _parseLicenses(data, newLicenses, diagnoses, deleteProfile, firstTime) {
	...
	// Partner file
	if (newData.partnerContents && oldData.partnerContents !== newData.partnerContents) {
		tracer && tracer("Change partner file OLD " + oldData.partnerContents + " NEW " + newData.partnerContents);
		oldData.partnerContents = newData.partnerContents;
		newLic = true;
	}
	// Policy files
	...
	for (var i = 0; i < 2; i++) { // merge Sage licenses first, then partner licenses
		for (var key in objOld) {
			...
			if (lic.partnerId) { // check whether base product is OK for partner licenses
				...
				if (!("\0" + pol.baseProduct in prodLicenses)) {
					// no base product
		...
		for (var key in objNew) {
			...
			if (lic.partnerId) { // check whether base product is OK
				...
				if (!("\0" + pol.baseProduct in prodLicenses)) {
					...
					if (diagnoses) diagnoses.push({
						$severity: "warning",
						$message: locale.format(module, "noCorrespondingBaseProduct", lic.partnerId, lic.product.code, lic.product.version, lic.policy.code, lic.policy.version, pol.baseProduct)
					});
	...
	if (newLic || deleteProfile || !data[1]) {
		...
		if (oldData.partnerContents) {
			k.push(oldData.partnerContents);
		}
		...
		["parameters", "modules", "activityCodes", "languages", "legislations", "parameterKits"].forEach(function(item) {
			...
			parsedPolicy[item].forEach(function(item0) {
				...
				if (code in policyItems0) { // does this item occur in another policy file?
					if (diagnoses) diagnoses.push({
						$severity: "warning",
						$message: locale.format(module, "ignored" + item, code, parsedPolicy.productId, parsedPolicy.partnerId)
					});
					return;
				} else {
...
function _getX3LicenseInfoInt(productCode, productVersion, badges, parsedData, diagnoses) {
	...
	for (var code in parsedData.products) {
		...
		if (helpers.relNumberCmp(productVersion, product.productVersion) > 0) {
			...
			if (diagnoses) diagnoses.push({
				$severity: "error",
				$message: locale.format(module, "lowVersion", productCode, product.productVersion, productVersion)
			});
			return undefined;
		}
...
function _checkNamedInt(_, parsed, inst, diagnoses) {
	...
	var badgeInfo = {};
	var productInfo = {};
	if (!_evaluateBadges(Object.keys(usedBadges), parsed, badgeInfo, productInfo, false, null, diagnoses)) {
		return true;
	}
...
function checkConcurrent(session, role, userName, _, device, diagnoses) {
	// read license content
	var data = getParsedLicense(_);
	...
	if (!_evaluateBadges(badges, data, badgeInfo, productInfo, true, device, diagnoses)) {
	...
	var serverNames = {};
	var sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, undefined);
	if (sessionInfos != null) { // maybe old sessions have been counted
		if (config.mockServer) {
			...
			if (!data) serverNames = {};
				...
				data.forEach(function(host) {
					console.error("HOST " + host.hostname + " " + host.status + " " + host.children);
					var hostName = host.hostname;
					if (+host.status === 3) {
						for (var i = host.children - 1; i >= 0; i--) {
							delete serverNames[hostName + ":N" + i];
							console.error("Delete " + hostName + ":N" + i);
						}
					}
				});
			}
		...
		sessionInfos = _remainingLicenses(_, db, badgeInfo, productInfo, session, userName, serverNames, unlicensedBadges);
...
function _evaluateBadges(availableBadges, parsedData, badgeInfo, productInfo, concurrent, device, diagnoses) {
	...
	availableBadges.forEach(function(code) {
		...
		if (productCode in productInfo) {
			var max = (parsedData.products[productCode].concurrent === concurrent ? databadge.max : -1);
			badgeInfo[code] = [max, productCode];
		} else {
...
function _remainingLicenses(_, db, totalBadges, totalProducts, session, userName, serverNames, unlicensedBadges) {
	...
	Object.keys(totalBadges).forEach(function(badge) {
		...
		// product of badge is allowed
		if (totalProducts[product] !== 0 && (!(product in productCounts) || productCounts[product] > 0)) {
			// badge itself is allowed
			tracer && tracer("BADGE local " + badge + " product " + product);
	...
	if (allLicensed || (unlicensedBadges && allowedBadges.length > 0)) {
