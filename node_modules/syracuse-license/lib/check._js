...
function _WSstatus() {
	...
	for (var key in d) {
		if (key !== "object") {
			result[key] = d[key].counter + "/" + d[key].total + "/" + d[key].size;
		}
	}
	return result;
}
...
function _step(_, length, poolMetadata, data, key, licenseData, timeString) {
	...
	if (!pd.licenseStart) {
		if (poolMetadata && poolMetadata.product) {
			...
			pd.size = (params.WS_SIZE_LIMIT || 1) * (1 << 30);
	...
	if ((pd.counter + pd.total) > pd.size) {
		if ((pd.counter + pd.total) > pd.graceLimit) {
			if (pd.status < 2 && !licenseData) {
				_fireEvent(_, false, pd, productCode);
				pd.status = 2;
			}
