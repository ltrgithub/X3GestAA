...
function _putServerInt(callback, name, method, path, content) {
	...
	console.log("PUT SERVER INT" +("mockClient" in syracuse.server))
	if ("mockClient" in syracuse.server) {
		options = { path: "/nannyCommand/notifyAll"+path, method: method, hostname: "", port: 0, syracuseBalancerHeader: config.port};
		client = syracuse.server.mockClient;
		console.log("oooooooooooooooooMock client"+util.format(options))
	} else {
		options = {hostname: parts[0], port: parts[1] || 80, path: path, method: method}
		client = http;
	}	
