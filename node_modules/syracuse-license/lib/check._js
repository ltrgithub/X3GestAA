...
var tracer = require('syracuse-trace/lib/helper').getTracer('license'); //= console.log;
...
function _getX3LicenseInfoInt(productCode, productVersion, badges, parsedData, diagnoses) {
	...
	if (!("1" in parsedData.products)) {
		console.log("###No product 1 " + globals.context.tenantId + " " + util.format(parsedData))
	}
...
function findUsedBadges(_, device) {
	// read license content
	var data = getParsedLicense(_);
...
function _usedLicenses(_, db, totalBadges, totalProducts, otherBadges, session, userName, serverNames, badgeCounts, productCounts, details) {
	...
	if (session) {
		...
		var sessionKey = (req.connection ? (req.connection.remoteAddress || "") : "-") + "_" + userName;
...
function _clearWebserviceLicense(data) {
	var d = data;
	if (multiTenant) {
		var tenantId = globals.context.tenantId;
		if (data[tenantId]) d = d[tenantId]
	}
	if (d) {
		for (var key in d) {
			if (key !== "object") d[key].licenseStart = undefined;
		}
...
function _WSstatus() {
	...
	for (var key in d) {
		if (key !== "object" && key != "percentage") {
			result[key] = d[key].counter + "/" + d[key].other + "/" + d[key].size;
		}
	}
	return result;
}
...
exports.step = function(_, length, poolMetadata) {
	if (devMode) return 0; // allow always in development mode
	return _step(_, length, poolMetadata, data, localhost + ":" + config.port);
};

function _sendData(_, counter, hostname, productCode, period) {
	console.log("Data for period: counter " + counter + " hostname " + hostname + " product code " + productCode + " period " + period)
}



function _step(_, length, poolMetadata, data, key, licenseData, timeString) {
	var d = data;
	if (multiTenant) {
		var tenantId = globals.context.tenantId;
	...
	var productCode = poolMetadata.product && poolMetadata.product.code;
	...
	if (!pd.licenseStart) {
		if (poolMetadata && poolMetadata.product) {
			...
			pd.size = (params.WS_SIZE_LIMIT || 10) * (1 << 30);
			pd.graceLimit = pd.size * (1 + (params.WS_GRACE_LIMIT || 10) / 100);
			pd.graceSlowdown = params.WS_GRACE_SLOWDOWN || 5;
			// WS_SIZE_LIMIT (in GB)
		...
		switch (params.WS_PERIOD || "DAY") {
			...
			case 'YEAR':
				...
				break;
			default:
				throw new Error("Illegal period: " + params.WS_PERIOD);
		}
		...
		if (!d.object) checkObject(_, d, productCode, timeString, key, licenseData)
		if (pd.counter) {
			pd.counter += +length;
		} else
			pd.counter = +length;
	...
	if (timeString.indexOf(pd.timeString) !== 0) {
		if (timeString.indexOf(pd.timeString.substr(0, pd.length)) !== 0) {
			...
			// new period
			_sendData(_, pd.counter, localhost + ":" + config.port, productCode, pd.timeString.substr(0, pd.length));
			pd.counter = +length;
			pd.status = 0;
		} else {
			if (pd.status < 3) pd.counter += +length;
		}
	...
	if ((pd.counter + pd.other) > pd.size) {
		...
		if (pd.status === 0) {
			if (pd.counter * 100 > d.percentage * pd.size) {
...
function _fireEvent(_, limitType, data, productCode, percentage) {
	...
	if (event) {
		...
		event.schedule(_, productCode, Date.now(), {
			...
			totalPercentage: 100 + ((product && product.parameters && product.parameters.WS_GRACE_LIMIT) ? product.parameters.WS_GRACE_LIMIT : 10)
		}, "");
	}
}

//updates content in database and compares data of other servers
function checkObject(_, data, productCode, timestamp, key, test) {
	var hash = "---"; // TODO
	var obj = data.object;
	var pd = data[productCode];
	...
	if (obj) {
		options.data[key0] = pd.counter + ";" + timestamp + ";" + hash;
	};
	if (test) {
		...
		if (!obj) {
			...
			data.percentage = setting.webServiceWarnThreshold(_);
		}
	}
	pd.other = 0;
	if (!cnt.data) {
	...
	for (var k in cnt.data) {
		...
		// format for the parts: value;timestamp;hash, e. g. 25;2015-04-22T12:36:17.926Z;a0b1c2a0
		// consistency check
		if (period === parts[1].substr(0, pd.length)) { // timestring belongs to correct period
			if (k === key0) {
				if (!obj) {
					pd.counter = +parts[0]; // get data only when there is no local object. Reason: 
				}
			} else {
				if (obj) {
					var value_old = obj[k];
					if (value_old) {
						...
						if (+parts_old[0] > +parts[0] && oldStamp >= pd.licenseStart && oldStamp.indexOf(timeString.substr(0, pd.length)) === 0)
