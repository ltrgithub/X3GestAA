...
function _storeDB(licenses, _) {
	...
	try {
		...
		if (licenses.length > 0) {
			db = lic.connectDatabase(_, globals.context.tenantId);
			if (config.logLicChange)
				console.log("Store DB", globals.context.tenantId, new Error().stack);
			...
			if (config.logLicChange)
				console.log("Store DB - removed", globals.context.tenantId);
			...
			if (config.logLicChange)
				console.log("Store DB - insert", globals.context.tenantId, licenses.map(function(item) {
					return (item ? item.substr(0, 300).replace(/[\r\n\t]/g, "") : "-");
				}));
			collection.insert(ins, {
				safe: true
			}, _);
			if (config.logLicChange)
				console.log("Store DB - inserted", globals.context.tenantId);
		}
	} finally {
		if (db) db.close();
	}
