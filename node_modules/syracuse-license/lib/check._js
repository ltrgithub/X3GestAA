...
function _sendToGA(_, counter, hostname, productCode, period) {
	var data = getParsedLicense(_);
	var product = (data && data.products[productCode]);
	if (product) {
		var cat = "License " + product.productTitle["en-us"];
		ga.hitEvent(_, cat, "Classic SOAP WS", period, counter, {
			host: hostname
		});
	}
}
...
function _step(_, length, poolMetadata, data, key, licenseData, timeString) {
	...
	if (timeString.indexOf(pd.timeString) !== 0) {
		if (timeString.indexOf(pd.timeString.substr(0, pd.length)) !== 0) {
			// new period
			_sendToGA(_, pd.counter, localhost + ":" + config.port, productCode, pd.timeString.substr(0, pd.length));
			pd.counter = +length;
			pd.status = 0;
		} else {
