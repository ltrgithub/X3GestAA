...
function _getX3LicenseInfoInt(productCode, productVersion, badges, parsedData, diagnoses) {
	...
	for (var code in parsedData.products) {
		...
		if (helpers.relNumberCmp(productVersion, product.productVersion) > 0) {
			...
			else console.error("License error1: " + productCode + " " + productVersion + " " + util.format(badges) + " " + util.format(parsedData) + " " + locale.format(module, "lowVersion", title, product.productVersion, productVersion));
			return undefined;
		}
		...
		if (!correctProduct) { // no badge for correct product
			...
			else console.error("License error2: " + productCode + " " + productVersion + " " + util.format(badges) + " " + util.format(parsedData) + " " + locale.format(module, "noBadgeAvail", title));
			return undefined;
		}
		return result;
	}
	console.error("License error 3: " + productCode + " " + productVersion + " " + util.format(badges) + " " + util.format(parsedData))
	return undefined;
}
...
function _remainingLicenses(_, db, totalBadges, totalProducts, session, userName, serverNames, unlicensedBadges, allowNoBadges, otherBadges) {
	...
	// user login as keys and number of sessions as values
	tracer2 && tracer2("Remaining licenses " + util.format(totalProducts) + util.format(totalBadges))
	...
	tracer2 && tracer2("Used licenses " + util.format(productCounts) + util.format(badgeCounts))
	// check remaining licenses for current session
	var allowedBadges = [];
	var allLicensed = true;
	Object.keys(totalBadges).forEach(function(badge) {
		...
		tracer2 && tracer2("product comparison " + productCounts[product] + " <= " + totalProducts[product]);
		if (totalProducts[product] && (!(product in productCounts) || productCounts[product] <= totalProducts[product])) {
			...
			tracer2 && tracer2("badge comparison " + badgeCounts[badge] + " <= " + (data && data[0]));
			if (!data || data[0] < 0 || !(badge in badgeCounts) || badgeCounts[badge] <= data[0]) {
				allowedBadges.push(badge);
				tracer && tracer("badge allowed " + badge);
				return;
			}
		}
		allLicensed = false;
...
function _usedLicenses(_, db, totalBadges, totalProducts, otherBadges, session, userName, serverNames, badgeCounts, productCounts, details) {
	...
	function addSession(sessionKey, item, counts, sid, product) {
		...
		tracer2 && tracer2("ADD " + item + " " + util.format(counts));
	}
	...
	sessionInfos.forEach_(_, function(_, sessionInfo) {
		...
		tracer2 && tracer2("Session " + sid + " Badges " + sessionInfo.badge(_));
