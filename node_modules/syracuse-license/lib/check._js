...
function _step(_, length, productCode, data, key, licenseData, timeString, noupdate) {
	...
	if (!pd.licenseStart) {
		if (productCode) {
			...
			pd.size = (params.WSSIZELIMIT || params.MAXWEBSERVICES || 1) * (1 << 30);
			pd.graceLimit = pd.size * (1 + (params.WSGRACELIMIT || 100) / 100);
...
function _fireEvent(_, limitType, data, productCode, percentage) {
	...
	if (event) {
		...
		event.schedule(_, productCode, Date.now(), {
			...
			totalPercentage: 100 + ((product && product.parameters && product.parameters.WSGRACELIMIT) ? product.parameters.WSGRACELIMIT : 10)
		}, "");
	}
}
...
function checkObject(_, data, productCode, timestamp, key, test, noupdate) {
	...
	for (var k in cnt.data) {
		...
		parts[3] = parts[3] || 4; // length of period string cannot be less than 4
