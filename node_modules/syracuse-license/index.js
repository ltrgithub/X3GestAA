...
if ((!config.system || !config.system.enableDevelopmentFeatures) && !require('syracuse-core/lib/helpers').enableDeveloppementFeatures) {
	config.noDevLic = true;
} else if (!config.noDevLic) {
	var LICENSE_FILE = __dirname + "/../../devLic/license.json";
	if (!fs.existsSync(LICENSE_FILE)) {
		config.noDevLic = true;
	} else console.log("Developer license");
}
if (config.noDevLic) {
	var LICENSE_FILE = (process.argv[2] === "PATCH" ? __dirname + "/../../license.json" : __dirname + "/../../temp/license.json");
}
...
exports.getDatabase = function(tenantId) {
	...
	return new mongodb.Db((tenantId ? tenantId + "-" : "") + (config.collaboration.dataset || "syracuse"), new mongodb.Server(config.collaboration.hostname || "localhost", config.collaboration.port || 27017, {}), {
		w: "majority"
	});
};
