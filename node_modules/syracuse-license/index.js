...
if (!fs.existsSync(__dirname + '/bin/' + arch)) {
...
exports.load = function(name) {
	...
	var filename = __dirname + '/bin/' + arch + '/' + v8 + '/' + name + '.node';
	if (fs.existsSync(filename)) return require(filename);
	else {
		filename = __dirname + '/bin/' + arch + '/default/' + name + '.node';
		if (fs.existsSync(filename)) return require(filename);
	}
...
exports.connectDatabase = function(callback, tenantId) {
	...
	var dbUrl = "mongodb://" + (config.collaboration.hostname || "localhost") + ":" + (config.collaboration.port || 27017) + "/" + ((tenantId ? tenantId + "-" : "") + (config.collaboration.dataset || "syracuse"));
