...
//function _step(_, length, productCode, data, licenseData, timeString)
var productCode = "x";
...
asyncTest('step test', 29, function(_) {
	try {
		...
		var data = {};
		// _step(_, length, productCode, data, key, licenseData, timeString)
		var result = step(_, 1 << 30, productCode, data, 'A', licenseData, '2015-05-05T09:00:00.000Z');
		strictEqual(result, 0, "Load 1 GB of 2");
		strictEqual(data.x.counter, 1 << 30, "Correct counter 1");
		var result = step(_, 1 << 30, productCode, data, 'A', licenseData, '2015-05-05T09:01:00.000Z');
		strictEqual(result, 0, "Load 2 GB of 2");
		strictEqual(data.x.counter, 2147483648, "Correct counter 2");
		var result = step(_, 1, productCode, data, 'A', licenseData, '2015-05-05T09:02:00.000Z');
		strictEqual(result, 5, "Load 2 GB of 2 plus 1");
		var result = step(_, (1 << 29) - 1, productCode, data, 'A', licenseData, '2015-05-05T09:03:00.000Z');
		strictEqual(result, 5, "Load 2 GB of 2 plus 25%");
		strictEqual(data.x.counter, 2684354560, "Correct counter 3");
		var result = step(_, 1, productCode, data, 'A', licenseData, '2015-05-05T09:04:00.000Z');
		strictEqual(result, -1, "Load 2 GB of 2 plus 25% plus 1");
		strictEqual(data.x.counter, 2684354561, "Correct counter 3");
		// new period
		var result = step(_, 7, productCode, data, 'A', licenseData, '2015-05-06T00:04:00.000Z');
		strictEqual(result, 0, "New day");
		strictEqual(data.x.counter, 7, "Correct counter 3");
		newLicense(data);
		var result = step(_, 1 << 30, productCode, data, 'A', licenseData2, '2015-05-06T09:00:00.000Z');
		strictEqual(result, 0, "Load 1 GB of 2 plus 7");
		strictEqual(data.x.counter, 1073741831, "Correct counter 4");
		var result = step(_, (1 << 30) - 7, productCode, data, 'A', licenseData2, '2015-05-17T09:00:00.000Z');
		strictEqual(result, 0, "Load 2 GB of 2");
		strictEqual(data.x.counter, 2147483648, "Correct counter 5");
		var result = step(_, 1, productCode, data, 'A', licenseData2, '2015-05-27T09:00:00.000Z');
		strictEqual(result, 6, "Load 2 GB of 2 plus 1");
		strictEqual(data.x.counter, 2147483649, "Correct counter 6");
		var result = step(_, 1, productCode, data, 'A', licenseData2, '2015-06-01T09:00:00.000Z');
		strictEqual(result, 0, "Load 1");
		strictEqual(data.x.counter, 1, "Correct counter 7");
		newLicense(data);
		var result = step(_, 6, productCode, data, 'A', licenseData3, '2015-06-06T09:00:00.000Z');
		strictEqual(result, 0, "Load 6");
		strictEqual(data.x.counter, 7, "Correct counter 8");
		var result = step(_, (1 << 30) - 7, productCode, data, 'A', licenseData3, '2015-11-17T09:00:00.000Z');
		strictEqual(result, 0, "Load 1 GB of 1");
		strictEqual(data.x.counter, 1073741824, "Correct counter 9");
		var result = step(_, 1 << 28, productCode, data, 'A', licenseData3, '2015-12-27T09:00:00.000Z');
		strictEqual(result, 8, "Load 1.25 GB of 1");
		strictEqual(data.x.counter, 1342177280, "Correct counter 10");
		var result = step(_, 1, productCode, data, 'A', licenseData3, '2015-12-31T09:00:00.000Z');
		strictEqual(result, -1, "Load 1.25 GB of 1 plus 1");
		strictEqual(data.x.counter, 1342177281, "Correct counter 11");
		var result = step(_, 1, productCode, data, 'A', licenseData3, '2016-12-31T09:00:00.000Z');
		strictEqual(result, 0, "Load 1");
...
asyncTest('server restart test', 4, function(_) {
	try {
		...
		var data = {};
		// _step(_, length, productCode, data, key, licenseData, timeString)
		var result = step(_, 12, productCode, data, 'A', licenseData, '2015-05-05T09:00:00.000Z');
		// "$" attribute contains database content (in this mock situation for tests)
		var dataB = {
			"$": data.$
		};
		var result = step(_, 15, productCode, dataB, 'B', licenseData, '2015-05-05T09:00:00.000Z');
		// simulate restart: clear all non-persistent data
		data = {
			"$": dataB.$
		};
		console.log(">> " + JSON.stringify(data));
		var result = step(_, 13, productCode, data, 'A', licenseData, '2015-05-05T10:00:00.000Z');
		...
		var result = step(_, 15, productCode, data, 'B', licenseData, '2015-05-05T09:00:00.000Z');
		// simulate restart: clear all non-persistent data
		data = {
			"$": dataB.$
		};
		var result = step(_, 13, productCode, data, 'A', licenseData, '2015-05-06T10:00:00.000Z');
...
asyncTest('step test with different servers', 27, function(_) {
	...
	try {
		...
		dataB.$ = dataA.$;
		var result = step(_, 2, productCode, dataB, 'B', licenseData, '2015-05-05T09:05:00.000Z');
		strictEqual(result, 0, "Load 2 B (without contact)");
		strictEqual(dataB.x.counter, 3, "Correct counter 2");
		strictEqual(dataB.x.other, 1, "Correct other 1");
		dataA.$ = dataB.$;
		var result = step(_, 1 << 30, productCode, dataA, 'A', licenseData, '2015-05-05T09:15:00.000Z');
		...
		dataB.$ = dataA.$;
		var result = step(_, 1 << 30, productCode, dataB, 'B', licenseData, '2015-05-05T09:15:00.000Z');
		...
		dataA.$ = dataB.$;
		var result = step(_, 3, productCode, dataA, 'A', licenseData, '2015-05-05T09:25:00.000Z');
		...
		dataC.$ = dataA.$;
		var result = step(_, 4, productCode, dataC, 'C', licenseData, '2015-05-05T09:25:00.000Z');
		...
		var result = step(_, 1, productCode, dataC, 'C', licenseData, '2015-05-06T09:25:00.000Z');
...
asyncTest('error conditions', 5, function(_) {
	try {
		...
		// _step(_, length, productCode, data, key, licenseData, timeString)
		var data = {};
		var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T08:00:00.000Z');
		var dataB = {
			"$": data.$
		};
		var result = step(_, 19, productCode, dataB, 'B', licenseData2, '2015-05-05T08:01:00.000Z');
		data.$ = dataB.$;
		// again contact with data
		var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T08:10:00.000Z');
		// make counter smaller
		data.$ = JSON.parse(JSON.stringify(data.$)); // duplicate object
		data.$.data["B/x"] = data.$.data["B/x"].substr(1);
		var error = false;
		try {
			var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T09:10:00.000Z');
			error = false;
		} catch (e) {
			error = true;
		}
		...
		// _step(_, length, productCode, data, key, licenseData, timeString)
		var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T09:00:00.000Z');
		// make counter smaller
		data.$ = JSON.parse(JSON.stringify(data.$)); // duplicate object
		data.$.data["B/x"] = '19;2015-05-05T08:44:44.000Z;---';

		try {
			var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-06T09:10:00.000Z');
			error = false;
		} catch (e) {
			error = true;
		}
		...
		var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T10:00:00.000Z');
		var dataB = {
			"$": data.$
		};
		var result = step(_, 19, productCode, data, 'B', licenseData2, '2015-05-05T11:00:00.000Z');
		data.$ = dataB.$;
		var error = false;
		try {
			var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-05-05T11:10:00.000Z');
			error = false;
		} catch (e) {
			error = true;
		}
		...
		var result = step(_, 19, productCode, data, 'B', licenseData2, '2015-06-05T11:00:00.000Z');
		data.$ = dataB.$;
		var error = false;
		try {
			var result = step(_, 19, productCode, data, 'A', licenseData2, '2015-06-05T11:10:00.000Z');
			error = false;
		} catch (e) {
			error = true;
		}
