...
test('Only new files, 1 licensed module', 15, function() {
	...
	strictEqual(callException([
		[], null], [license]), "Error: No policy file", "No policy file");
	var res = check._p([
		[], null], [license, policy]);
...
// partner licenses
test('Partner licenses', 16, function() {
	var policyR = {
		"modules": [{
			"code": "A",
			"keyFunctions": ["A1", "A2", "A3"]
	...
	strictEqual(callException([
		[], null], [license, policy, licensePartner]), "Error: No policy file for partner P", "Missing policy file for partner");
	...
	strictEqual(diagnoses.length, 6, "Number of diagnoses")
});
