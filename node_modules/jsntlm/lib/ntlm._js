...
function buildType1Message(ntDomain, workstation) {
	...
	// host string (ASCII)
    var host = workstation.indexOf('.') !== -1 ? workstation.split('.')[0] : workstation;
    // domain string (ASCII)
    var domain = ntDomain.indexOf('.') !== -1 ? ntDomain.split('.')[0] : ntDomain;
	...
	header.writeUInt16LE(domain.length, 9);
	header.writeUInt16LE(domain.length, 11);
	header.writeUInt16LE(32 + host.length, 13);
	header.writeUInt8(0, 15);
	header.writeUInt8(0, 16);
	header.writeUInt16LE(host.length, 17);
	header.writeUInt16LE(host.length, 19);
	header.writeUInt16LE(32, 21);
	header.writeUInt8(0, 23);
...
function buildType3Message(username, ntDomain, workstation, lmResp, ntResp) {
	...
	var lmBuffer = new Buffer(lmResp,'hex');
	var ntBuffer = new Buffer(ntResp,'hex');
	
	var buffers = [];
	buffers.push(new Buffer('NTLMSSP', 'ascii')); // 'N', 'T', 'L', 'M', 'S', 'S', 'P'
	
	var header = new Buffer(56);
	...
	header.writeUInt16LE(300, 49);// message length TODO
	...
	buffers.push(hostBuffer);
	buffers.push(lmBuffer);
	buffers.push(ntBuffer);
...
function handleType2Message(msg, passw) {
	...
	function padHash(srcHex){
	    var buf = new Buffer(srcHex, 'hex');
	...
	var lm_hash_passw = helpers.lmHash(passw);
	var nt_hash_passw = helpers.ntlmHash(passw);

	return {
		lmResp: helpers.calc_resp(padHash(lm_hash_passw), $nonce),
		ntResp: helpers.calc_resp(padHash(nt_hash_passw), $nonce)
	};
}

exports.authenticate = function(_, proxy){

	function sendMsg(_,msg, cookie){
		var opt = {
			protocol: "http:",
			method: "GET",
			host: proxy.host,
			...
			headers: {
				...
				"Proxy-Connection": "Persist",//"keep-alive"
		...
		tracer("Request Headers: "+JSON.stringify(opt.headers,null,2));
	...
	var response1 = sendMsg(_,msg1);
	tracer("Response 1 Headers: "+JSON.stringify(response1.headers,null,2));
	tracer("Response 1 status code: "+response1.statusCode);
	...
	msg3 = buildType3Message(proxy.user, proxy.domain, proxy.workstation, resps.lmResp, resps.ntResp);
	
	var response2 = sendMsg(_,msg3, cookie);
	tracer("Response 2 Headers: "+JSON.stringify(response2.headers,null,2));
	tracer("Response 2 status code: "+response2.statusCode);
