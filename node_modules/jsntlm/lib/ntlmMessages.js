 ...
 */
var des_encrypt = function(ks, str, encoding){
	/**
	...
	var tokencrypt = crypto.createCipheriv('des-cbc', new Buffer(ks, 'hex'), iv);
	var token = tokencrypt.update(str, encoding, 'hex');
 ...
 * @api public
 */
var crc32Checksum = function(str, hex) {
	var crc32tab = [
...
//			var part1 = new Buffer(des_encrypt(keyBytes.slice(0,7), truncatedResponse, 'binary'), 'hex');
//			var part2 = new Buffer(des_encrypt(keyBytes.slice(7), truncatedResponse, 'binary'), 'hex');
...
//			
    ...
    mac: function(negotiateFlags, seqNum, signingKey, sealingKey, randomPad, message, input_encoding) {
        if ((negotiateFlags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY_FLAG) !== 0) {
			...
			*/
			...
			var checkSum = crc32Checksum(message.toString('ucs2'), true);
...
exports.Type1Message = helpers.defineClass(function(session, domain, workstation) {
...
exports.Type2Message = helpers.defineClass(function(message) {
...
exports.Type3Message = helpers.defineClass(function(session, user, password, host, domain) {
    ...
    getResponse: function() {
        ...
        var domainOffset = 72;// TODO: allocate space (+16) for MIC
        var userOffset = domainOffset + this.domainBytes.length;
        var hostOffset = userOffset + this.userBytes.length;
        var lmRespOffset = hostOffset + this.hostBytes.length;  
        var ntRespOffset = lmRespOffset + this.lmResp.length;
        var sessionKeyOffset = ntRespOffset + this.ntResp.length;
        var finalLength = sessionKeyOffset + (this.sessionKey != null ? this.sessionKey.length : 0);
