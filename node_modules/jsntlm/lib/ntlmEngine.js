...
var hmac_md5_encrypt = function(str, key){
    var iv = new Buffer(16);
    for (var i = 0 ; i < 16 ; i++) iv[i] = 0x00;
	var tokencrypt = crypto.createCipheriv('md5', new Buffer(key, 'hex'), iv);
	var token = tokencrypt.update(str, 'ascii', 'hex');
	tokencrypt.final('hex');
	return token;
};
...
var CipherGen = exports.CipherGen = helpers.defineClass(function(domain, user, password, challenge, target, targetInformation, clientChallenge, clientChallenge2, secondaryKey, timestamp) {
    ...
    getNTLMv2Blob: function(forcedTime) {
		if (this.ntlmv2Blob == null)
			this.ntlmv2Blob = createBlob(this.getClientChallenge2(), this.targetInformation, forcedTime);
    ...
    getNTLMv2ResponseBIS: function(forcedTime) {
	   var temp = Buffer.concat([new Buffer([1,1,0,0,0,0,0,0]), forcedTime, this.getClientChallenge(), new Buffer([0,0,0,0]), this.targetInformation, new Buffer([0,0,0,0])]);

	    //return Buffer.concat([blobSignature, reserved, timestamp, clientNonce, unknown1, targetInformation, unknown2], blobLen);

	   console.log("KEY: "+this.getNTLMv2Hash());	   
console.log("CHALLENGE: "+this.challenge);
console.log("TEMP: "+temp);
	    var ntProof =  crypto.createHmac('md5', this.getNTLMv2Hash()).update(Buffer.concat([this.challenge, temp])).digest('hex');
console.log("NTPROOF: "+ntProof.toString('hex'));
	    return Buffer.concat([new Buffer(ntProof,'hex'), temp]);
    },
...
var getType1Message = function(host, domain) {
    return new Type1Message(domain, host);
};
...
//exports.negociate = function(_, options){
//	
...
//	
//	
//	var msg1 = generateType1Msg(proxy.domain, proxy.workstation);
//	
...
//		//new Type3Message(domain, host, user, password, nonce, type2Flags, target, targetInformation).getResponse();
//		
//		var request = streams.httpRequest(options);
//		var response1 = request.end().response(_);
//			
//			
...
//			var msg3 = generateType3Msg(proxy.user, proxy.password, proxy.domain, proxy.workstation, msg2);
...
//			msg3.getResponse();
//		}else{
//			throw new Error ("NTLM Type-2 message not received.");
//		}
//	});
//
//	msg1.getResponse();
...
//	tracer("Message 1: "+"NTLM "+msg1);
//	
//	var agent = new http.Agent();
//	agent.maxSockets = 1;
//	console.log("Agent: "+require('util').inspect(agent));
//	
//	var opt = {
//		method: "GET",
...
////				
////				
////				tracer("NEGOCIATE RETURN: "+"NTLM "+msg3);
////				return "NTLM "+msg3;
////				//sendMsg(msg3, cookie, cb2);
////			}else{
////				throw new Error ("Type-2 message not received.");
