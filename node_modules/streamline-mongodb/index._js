"use strict";
(function() { // avoid return at top level
	...
	function MongoClient() {
		this.obj = mongodb.MongoClient;
	}
	var mongoClientProto = MongoClient.prototype;

	mongoClientProto.connect = function(url, options, _) {
		var db = new exports.Db();
		db.obj = this.obj.connect(url, options, ~_);
		return db;
	}
	...
	exports.Db = function Db(databaseName, serverConfig, options) {
		...
		// can be invoked without database name to support wrap of MongoClient
		if (databaseName) this.obj = new mongodb.Db(databaseName, serverConfig, options);
	};
	var dbProto = exports.Db.prototype;

	// warning: admin API is not handled by wrapper. You must pass ~_ to admin calls in fast mode.
	dbProto.admin = function() {
	...
	dbProto.collectionNames = function(_) {
		...
		var lstCollection = new CommandCursor(this.obj.listCollections({}));
		return lstCollection.toArray(_).map(function(o) {
			return o.name;
		});
	};
	dbProto.collections = function(_) {
		return this.obj.collections(~_);
	};

	dbProto.listCollections = function(filter, options) {
		return new CommandCursor(this.obj.listCollections(filter, options));
	};
	...
	colProto.indexes = function(_) {
		return this.obj.indexes(~_);
	};
	...
	function CommandCursor(obj) {
		this.obj = obj;
	}
	var commandCursorProto = CommandCursor.prototype;

	commandCursorProto.toArray = function(_) {
		return this.obj.toArray(~_);
	};
