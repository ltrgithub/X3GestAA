...
function _normalizeMapUri(uri) {
	...
	if (uri.indexOf("http") == 0) return "/" + uri.split("/").slice(3).join("/");
	else return uri;
}
...
helpers.defineClass(ProxyRequestManager, null, {
	...
	updateProxyMap: function(proxyMapEntry) {
		if (proxyMapEntry && !this.getMap(proxyMapEntry.uri)) this.map[_normalizeMapUri(proxyMapEntry.uri)] = proxyMapEntry;
