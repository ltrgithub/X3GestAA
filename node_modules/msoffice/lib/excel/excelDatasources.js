...
//		if(resource.orderBys)
//			params.push("orderBys="+resource.orderBys.join(","));
...
var _actions = {
	...
	$save: function(resource, onSuccess, onError) {
		resource.serviceUrl = _computeServiceUrl(resource);
		document.site.excelDocument.addDatasource(resource);
	}
}
...
function _applyDelta(resource, target) {
	...
	var datasource = target || document.site.excelDocument.getDatasource(resource.$uuid) || {};
	if(resource.$actions)
		forEachKey(resource.$actions, function(key, action) {
			if(!action.$isRequested)
				return;
			result = _actions[key] && _actions[key](target);
	...
	if(resource.$isDeleted) {
		...
		if(resource.hasOwnProperty("endpoint")) {
			target.endpoint = resource.endpoint;
			entity.$relations["endpoint"].$propagate(target, resource.endpoint);
		}
...
/*	GET: function(excelQuery, id, data, onSuccess, onError) {
		if(id) {
//			var result = document.site.excelDocument.getDatasourcesResource(document.site.$currentDatasource);
			var result = document.site.excelDocument.getDatasourcesResource(id);
			onSuccess(result);
			// currently not used, since filter is x-string (changing to x-filter will require additional metadata
			// _addFilterProperties(excelQuery, id, data, result, onSuccess);
		} else {
