...
var dataModel = syra_site.excelDocument.getDataModel();
...
function _computeServiceUrl(resource) {
	if (resource.dataBaseUrl && resource.representation && resource.entity) {
		...
		if (!resource.fetchAll && (facetName === "$query" || facetName === "$cube")) params.push("count=" + limit);
...
var _operationMap = {
	...
	refresh: {
		POST: function(id, data, onSuccess, onError) {
			syra_site.excelDocument.refreshDatasource(id);
		}
	}
};
...
exports.workingCopies = {
	...
	DELETE: function(excelQuery, id, data, onSuccess, onError) {
		syra_site.excelDocument.getDataModel().deleteWorkingCopy(id);
	}
};