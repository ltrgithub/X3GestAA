...
function _fetchQueryResource(_, parsedUrl, nextUrl, resources) {
	...
	var handle = endpoint.getOrm(_);
	var client = handle.getClient(_);
	var url = nextUrl || (handle._baseUrl + "/" + parsedUrl.syraClass + "?" + parsedUrl.queryParams);
...
function _makeTableHeader($tablePrototype, $localization) {
	...
	for (var ii = 0, jj = $tablePrototype.length; ii < jj; ii++) {
		...
		var title = headerColumn._title || headerColumn._name;
...
function _parseRequestUrl(_, requestUrl) {
	...
	for (var ii = 0, jj = keys.length; ii < jj; ii++) {
		var key = keys[ii];
		if (key !== "format") {
			var nvp = ((ii > 0) ? "&" : "") + key + "=" + parsed.query[key];
