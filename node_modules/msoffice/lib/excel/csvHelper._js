...
function _generateCsv(_, ctx, options) {
	...
	_buildQueryArticle($article, $protoItem);
	...
	var data = _makeTableData(newData, $protoItem.$properties, items, $prototype.$localization);
...
function _fetchResource(_, parsedUrl) {
	...
	if (parsedUrl.contract === "collaboration") {
		...
		entity = db.getEntity(_, parsedUrl.representationName, "$bulk");
		db.fetchInstances(_, entity).forEach_(_, function(_, record) {
			resources.push(record._data);
		});
	}
	return resources;
}
...
function _makeTableHeader($tablePrototype, $localization) {
	...
	for (var ii = 0, jj = $tablePrototype.length; ii < jj; ii++) {
		...
		let title = headerColumn._title;
		if ($localization && title.startsWith('{')) {
			title = $localization[title.slice(1, -1)] || title;
		}
					...
					if (ref && ref.$enum) {
						for (var ii = 0, jj = ref.$enum.length; ii < jj; ii++) {
							if (ref.$enum[ii] && ref.$enum[ii].$value !== undefined && ref.$enum[ii].$value === prop) {
								var titleRef = ref.$enum[ii].$title;
								if ($localization && titleRef && titleRef.startsWith('{')) {
									titleRef = titleRef.slice(1, -1);
									result = '"' + $localization[titleRef] + '"';
								} else {
									result = titleRef;
								}
								break;
				...
				case "application/x-reference":
					...
					if (record && valueTemplate && valueTemplate.startsWith('{')) {
						...
						if (record[valueTemplate]) {
							result = '"' + record[valueTemplate] + '"'; // escape the output...
						} else {
							var key = propProto[valueTemplate];
							result = key && res[key.slice(1, -1)];
						}
					} else {
						result = "";
					}
