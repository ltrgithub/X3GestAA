...
function _csvDirectExport(_, data, stream) {
	...
	let os = (stream.pre || stream.emitter) ? stream : ez.devices.node.writer(stream, options);

	let reader = ez.devices.array.reader(data, {
		sync: true
	});
	...
	reader.transform(ez.transforms.csv.formatter()).forEach(_, function(_, line, index) {
		if (index > 0) {
			var tempFileBuffer = Buffer.from(line, 'utf-8');
			bufferLength += tempFileBuffer.length;
			fileBuffer = Buffer.concat([fileBuffer, tempFileBuffer], bufferLength);
		}
	});
