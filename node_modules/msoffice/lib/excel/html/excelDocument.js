...
function _loadDatasources(){
    return  syra_site.isExcelLoaded ? 
        JSON.parse(_getCustomData(_defines.datasourcesAddress) || "{}") :
        _stub.dataset;
}
...
exports.ExcelDocument = _helpers.defineClass(ExcelDocument, null, {
    ...
    getLocalDatasources: function(){
        ...
        for (var ii = 0, jj = keys.length; ii < jj; ii++) {
            ...
            if (!server || this.isLocalDatasource(server)) { 
                datasources[key] = allDatasources[key];
            }
        }
        return datasources;
    },
    isLocalDatasource(server) {
        return server && server.length && server === this.getBaseURI();
    },
    ...
    getDatasourcesResource: function(uuid, applyMeta){
        function getDsResource(datasource){
            ...
            dsItem.$etag = 1;
            dsItem.description = _locale.format(syra_office_local.document_representation, dsItem.title, dsItem.representation, dsItem.entity, dsItem.endpoint);
    ...
    workingCopyToDatasource: function(workingCopy){
        ...
        var ds = {
            server: record.server || this.getBaseURI(),
            $uuid: record.$uuid,
    ...
    loadTable: function(params, onLoaded, onError){
        ...
        syra_ajax.get({
            ...
            error: function(response, ajax, $diagnoses){
                if (onError) {
                    var $diagnose = $diagnoses && $diagnoses[0];
