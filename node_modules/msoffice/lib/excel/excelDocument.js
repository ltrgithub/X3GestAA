...
function ExcelDocument() {
	...
	self.enableSageIntelligence = false;
...
exports.ExcelDocument = helpers.defineClass(ExcelDocument, null, {
	...
	customProperty: function(propName, value) {
		if (arguments.length == 1) {
			...
			else this.workbook.CustomDocumentProperties.Add(propName, false, external.msoString, value);
		}
	},
	...
	getDataModel: function() {
		if (!this._dataModel) this._dataModel = new DataModel(require("msoffice/lib/excel/contract/contract").contract);
	...
	deleteWorkingCopy: function(wcId) {
		if (this.$workingCopies && this.$workingCopies[wcId]) this.$workingCopies[wcId] = null;
	},
	...
	workingCopyToDatasource: function(workingCopy) {
		...
		if (record.orderBys) ds.orderBys = record.orderBys.map(function(orderBy) {
			return {
				$index: orderBy.$index,
				name: orderBy.name,
				title: orderBy.title,
	...
	addDatasource: function(record, onLoaded) {
		if (record.serviceUrl) {
			...
			if (ds) this.loadPage(ds, ds.dsName, null, onLoaded);
		}
	},
	...
	fetchSlice: function(page, maxLines, data, onLoaded) {
		...
		if (data) {
			if (data.$links && data.$links.$next) {
				...
				if (url.indexOf("&count=") < 0) {
					if (url.indexOf("?") < 0) url = url + "?count=" + excelHelpers.defines.defaultFetchCount;
					else url = url + "&count=" + excelHelpers.defines.defaultFetchCount;
				}
				options = {
					$location: {
						$url: url,
		...
		page.fetch(options, function(fetchData, response, requestUrl) {
			...
			if (!data) {
				...
				var res;
				if (maxLines < 0) res = page.createExcelTableShell(fetchCount);
				else res = page.createExcelTableShell(Math.min(fetchCount, maxLines));
	...
	loadTable: function(params, onLoaded, onError) {
		...
		var epUrl = isDetail ? self.syracuseBaseUrl + "/endPoints('" + params.endpointId + "')?representation=endPoint.$details" : self.syracuseBaseUrl + "/endPoints?representation=endPoint.$query&where=(dataset eq \"" + params.endpointName + "\")";
		// fetch the endpoint
		//var facetName = "$query";
		var facetName = "$bulk";
	...
	selectionChanged: function() {
		...
		if (activeCell && activeCell.ListObject) {
			...
			if (tableId && (document.site.$currentDatasource !== tableId)) {
				...
				document.site.mainPage && document.site.mainPage.idMap && Object.keys(document.site.mainPage.idMap).forEach(function(p) {
					var vignette = document.site.mainPage.idMap[p];
					if (vignette.vignette && vignette.vignette.$urlParts && (vignette.vignette.$urlParts.representationRoot == "excelActionPanelDs")) vignette.doPicker("refresh");
				});
			}
		}
	}
