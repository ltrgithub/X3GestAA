...
exports.ExcelDocument = helpers.defineClass(ExcelDocument, null, {
	...
	getDatasourcesResource: function(uuid,facet) {
		function getDsResource(datasource) {
			...
			dsItem.$etag = 1;
//			dsItem.name = dsName;
			dsItem.description = locale.format("\"{0}\" is a representation \"{1}\" of \"{2}\" on \"{3}\"",
				dsItem.title, dsItem.representation, dsItem.entity, dsItem.endpoint);
			dsItem.orderBys = datasource.orderBys && datasource.orderBys.map(function(orderBy) {
		...
		if(uuid) {
			...
			ds.$resources = Object.keys(datasources || {}).map(function(dsName) {
	...
	refreshDatasource: function(dsUuid, onLoaded) {
		var ds = this.getDatasource(dsUuid);
		ds && this.loadPage(ds, ds.dsName, onLoaded);
	},
	...
	fetchSlice: function(page, maxLines, data, onLoaded) {
		...
		if(data) {
			if(data.$links && data.$links.$next) {
				var url = page.parseExpression(data.$links.$next.$url, data);
				...
				options = { $location: { $url: url, $type: data.$links.$next.$type }};
			}
			else
				return;
		}
		page.fetch(options, function(fetchData, response, requestUrl) {
			...
			if(!data) {
				...
				if(maxLines < 0)
					page.createExcelTableShell(fetchCount);
				else
					page.createExcelTableShell(Math.min(fetchCount, maxLines));
			}
			//
			...
			// apply data
            page.applyChange(fetchData, response, requestUrl);
            //
	...
	loadPage: function(datasource, name, onLoaded) {
		var self = this;
		name = name || self.makeDatasourceName(datasource);
		//
		excelLoaded && external.StartUpdateTable();
		document.controller.loadRepresentation(null, datasource.$url, function(response) {
...
//			alert("selectionChanged tableid:" + tableId);
			//
			if(tableId && (document.site.$currentDatasource !== tableId)) {
//				alert("selectionChanged tableid(1):" + tableId);
				...
				document.site.mainPage && document.site.mainPage.idMap && Object.keys(document.site.mainPage.idMap).forEach(function(p) {
					var portlet = document.site.mainPage.idMap[p];
//					alert("portlet: "+(portlet.portlet && portlet.portlet.$urlParts && portlet.portlet.$urlParts.representationRoot));
					if(portlet.portlet && portlet.portlet.$urlParts && (portlet.portlet.$urlParts.representationRoot == "excelDatasource"))
						portlet.refresh();
				});
			}
		}
