...
exports.ExcelDocument = helpers.defineClass(ExcelDocument, null, {
	...
	loadTable: function(params, onLoaded, onError) {
		var self = this;
		// fetch the endpoint by name
		var epUrl = "/sdata/syracuse/collaboration/syracuse/endPoints?representation=endPoint.$query&where=" +
			"(dataset eq \"" + params.endpointName + "\")";
		var query = httpController.parseUrl(epUrl).sendRequest(null, function(data) {
			if(data && data.$resources && data.$resources.length) {
				var ep = data.$resources[0];
				var ds = {
					$url: ["/sdata", ep.application, ep.contract, ep.dataset, params.className].join("/") + 
						"?representation=" + params.representationName + ".$query" + (params.parameters ? "&" + params.parameters : ""),
					$title: params.dsName,
					title: params.dsName,
				...
				if(params.cellAddress) {
					var r = Application.Range(params.cellAddress);
					r && r.Activate();
				}
				//
				self.loadPage(ds, params.dsName, onLoaded);
			} else
				// TODO: localize
	...
	loadTables: function(params, onLoaded, onError) {
		...
		function asyncLoad() {
			var t = p.shift();
			(t && self.loadTable(t, asyncLoad, onError)) || onLoaded;
		}
        ...
        if (activeCell && activeCell.ListObject) {
            ...
            if (tableId && (document.site.$currentDatasource !== tableId)) {
                ...
                Object.keys(document.site.mainPage.idMap).forEach(function(p){
                    ...
                    //					alert("vignette: "+(vignette.vignette && vignette.vignette.$urlParts && vignette.vignette.$urlParts.representationRoot));
