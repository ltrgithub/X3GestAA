...
exports.render = function(_, ctx, result, prototype, headers, statusCode) {
	...
	if (reportMode === "new_csv") {
		return csv.render(_, ctx, result, prototype, headers, statusCode);
	}
...
function _xlsx_report(_, ctx, result) {
	var doc_uuid = (ctx.parameters && ctx.parameters.doc_uuid) || "";
...
function _xlsx_serve_empty_template(_, ctx, customDataObj) {
	...
	var parts = (ctx.url.split("?")[0] || "").split("/");
	var connectUrl = parts.slice(0, 3).join('/');

	xlsxHelper.serve_xlsx_dir(_, ctx.response, tplPath, customDataXml, connectUrl, module);
}
...
function _xlsx_build_custom_data(_, ctx, result, mode, refresh) {
	...
	var userRole, userLocale, userEndpoint;
	if (up) {
		userRole = up.selectedRole(_).$uuid;
		userLocale = up.selectedLocale(_).$uuid;
		userEndpoint = up.selectedEndpoint(_).$uuid;
	}
	...
	var datasources = {};
	if (mode === "rpt_build_tpl" || mode === "rpt_fill_tpl" || mode === "rpt_is_tpl") {
		var baseURI = urlRep.match(/^http[s]{0,1}:\/\/\w*(?::\d+)/)[0];
		var repr = ctx ? ctx.parameters.representation : "";
		if (repr.indexOf(".$query") > 0) {
			repr = repr.split(".$query")[0];
		}
	...
	return {
		...
		"syracuseLocale": userLocale,
		"syracuseEndpoint": userEndpoint
	};
}
...
var xlsx_direct_export = exports.xlsx_direct_export = function(_, ctx, options) {
	...
	var baseURI = url.match(/^http[s]{0,1}:\/\/\w*(?::\d+)/);
	...
	var cvgData;
	if (options.data) {
		cvgData = "<SyracuseOfficeCustomData>" + JSON.stringify(options.data).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</SyracuseOfficeCustomData>";
		...
		transform: function(_, contents, entry) {
			switch (entry.name) {
				case "xl/sharedStrings.xml":
					//
					return new Buffer(contents.toString('utf8').replace("___dictionnary___", JSON.stringify({
						...
						syracuseLocale: excelDefines.syracuseLocale,
						syracuseEndpoint: excelDefines.syracuseEndpoint
					})).replace("___serverUrl___", connectUrl).replace("___documentUrl___", "").replace("___documentTitle___", "").replace("___datasources___", JSON.stringify(datasources)).replace("___addInDownloadLabel___", locale.format(module, "addInDownloadLabel")).replace("___addSupportedLocales___", ctx ? JSON.stringify(xlsxHelper.get_supported_locales(_)) : []).replace("___addSyracuseRole___", userRole).replace("___addSyracuseLocale___", userLocale).replace("___addSyracuseEndpoint___", userEndpoint));
				case "xl/worksheets/sheet2.xml":
					return new Buffer(contents.toString('utf8').replace("___addInDownloadTip___", locale.format(module, "addInDownloadTip")));
				case "xl/worksheets/_rels/sheet2.xml.rels":
					return new Buffer(contents.toString('utf8').replace("___addInDownloadAddress___", connectUrl + "/msoffice/lib/general/addIn/SyracuseOfficeAddinsSetup.EXE"));
...
exports.handleV6Document = function(_, document, documentUrl) {
	...
	if (isZipFile == false) {
		...
		xlsxHelper.serve_xlsx_dir(_, newDoc, "/templates/new_doc", customDataXml, connectUrl, module);
	} else {
	...
	return {
		type: httpHelpers.mediaTypes.xlsx,
