...
exports.render = function(_, ctx, result, proto, headers, statusCode) {
	...
	if (reportMode === "no_tpl") return _xlsx_direct_export(_, ctx);

	_xlsx_set_headers(_, ctx, result, proto, headers, statusCode);

	if (reportMode === "new_tpl") return _xlsx_report_tpl(_, ctx, result, proto);
	if (reportMode === "mod_tpl") return _xlsx_report_mod_tpl(_, ctx, result, proto);
};
...
function _xlsx_serve_empty_template(_, ctx, customDataObj, test_template) {
...
function _xlsx_set_headers(_, ctx, result, proto, headers, statusCode) {
 ...
 function _xlsx_direct_export(_, ctx) {
	...
	//require('msoffice/lib/excel/xlsx').render(_, ctx.response, {"url": xlsxUrl});
...
exports.render2 = function(_, response, options) {
var url = options.url || "";
	var parts = (url.split("?")[0] || "").split("/");
	var queryParts = helpers.url.parseQueryString(url.split("?")[1] || "");
	var repr = url.match(/\?.*representation=(.*)\.(\$|%24)/);
