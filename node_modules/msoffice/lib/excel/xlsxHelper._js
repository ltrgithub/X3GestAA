...
require('streamline-runtime').globals;

exports.XlsxHelper = {

	/**
	 *  extract the xlsx buffer in document
	 * extract custom data if available -> call onCustomDataRead
	 * call onCustomDataWrite
	 * Write new document to response
	 * 
	 */
	serve_xlsx_buffer: function(_, response, document, onCustomDataRead, onCustomDataWrite) {
		var excel_rels_document_xml_rels_name = "xl/_rels/workbook.xml.rels";
		var content_typesName = "[Content_Types].xml";
		var xlsx_rels_document_xml_rels;
		var content_types;
		...
		function(filename, filecontent, headers, _) {
			...
			} else if (filename === excel_rels_document_xml_rels_name) {
				// Maybe this file has to be modified later, so don't add it to archive yet
				xlsx_rels_document_xml_rels = filecontent;
		...
		if (customXmlFile) { // There is already a custom XML-File, just change the content
			...
			var newFileNum = 0;
			var rid = 0;
			var rels = xlsx_rels_document_xml_rels.toString();
			...
			xlsx_rels_document_xml_rels = new Buffer(rels.replace("</Relationships>", newRel + "</Relationships>"));
		...
		if (xlsx_rels_document_xml_rels) archive.add(_, {
			name: excel_rels_document_xml_rels_name,
			data: xlsx_rels_document_xml_rels
		});
	...
	serve_xlsx_dir: function(_, response, tplPath, customDataXml, connectUrl, mod) {
		new zip.Zip(response, {
			...
			transform: function(_, contents, entry) {
				...
				if (entry.name === 'xl/sharedStrings.xml' && mod)
					return new Buffer(contents.toString('utf8').replace("___addInDownloadLabel___", locale.format(mod, "addInDownloadLabel")));
				if (entry.name === 'xl/worksheets/sheet2.xml' && mod)
					return new Buffer(contents.toString('utf8').replace("___addInDownloadTip___", locale.format(mod, "addInDownloadTip")));
				if (entry.name === 'xl/worksheets/_rels/sheet2.xml.rels' && connectUrl)
	...
	get_template: function(_, doc_uuid) {
		...
		var docEntity = db.model.getEntity(_, "msoExcelTemplateDocument");
	...
	is_xlsx_format: function(_, buffer) {
...
exports.XlsxHelper.MemoryOutStream = helpers.defineClass(function() {
	this.writable = true;
	this.chunks = [];
	this.len = 0;
	this.emitter = {
