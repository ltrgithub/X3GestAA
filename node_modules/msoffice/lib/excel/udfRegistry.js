...
function _registerUdf(x3Module, udfName, udfPrototype) {
	...
	// declaration
	"Public Function " + udfName + "(" + argNames.map(function(argName) {
	...
	}).join("") + udfName + " = managedObject.InvokeFunction(\"" + udfName + "\", args)\nEnd Function");
	// register help
...
exports.registerUdfs = function(workbook) {
	try {
		...
		for (var i = 1; i <= workbook.VBProject.VBComponents.Count; i++) {
			...
			if ((vbComponent.Type == external.vbext_ct_StdModule) && (vbComponent.Name == x3ModuleName)) x3Module = vbComponent;
		}
		if (!x3Module) {
