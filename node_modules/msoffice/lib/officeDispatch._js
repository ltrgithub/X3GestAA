...
var _dispatchMap = {
	"_xlsx": function(_, context) {
		...
		require('./excel/xlsx').render(_, context.response, {
			"url": xlsxUrl
		});
	...
	"xlsx": function(_, context) {
		...
		require('./excel/xlsx').render(_, context, result, proto, {}, 200);
		context.response.end();
		return;
	},
	"csv": function(_, context) {
		var label = context.parameters.representation ? context.parameters.representation.split(".")[0] : "unknown";
		var proto = {
			$title: label,
			$description: label
		...
		require('./excel/csv').render(_, context, result, proto, {}, 200);
		context.response.end();
		return;
	},
	"docx": function(_, context) {
		...
		require('./word/docx').render(_, context, result, proto, {}, 200);
		context.response.end();
		return;
	},
	"pptx": function(_, context) {
		...
		require('./ppt/pptx').render(_, context, result, proto, {}, 200);
		context.response.end();
		return;
	}
};
