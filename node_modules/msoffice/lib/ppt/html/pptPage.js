...
var highChartsTheme = require('syracuse-ui/themes/desktop/highcharts/theme').theme;
...
exports.PptPage = helpers.defineClass(PptPage, DesktopPage, {
	...
	drawBox: function() {
		this.$boundProp = this.openerUrlSegments && this.openerUrlSegments.params && this.openerUrlSegments.params.$bind || "$resources";
		...
		var $items = syra_layoutUpdater.getItemSettings(this, null, true);
		var $listItem;
		...
		this.$grid = new PptDataGrid(this, this.$proto);
		this.$grid.$record = new PptDataGridRecord(this.$grid, this.$proto.$item);
	},
			...
			return items.map(function(item) {
				...
				switch (item._type) {
					...
					case "application/x-reference":
						record = prop;
						result.value = (record && propProto.$value && syra_site.expressionMaker.parse(self.$grid.$record, propProto.$value, record)) || "";
						break;
					default:
						result.value = "";
				}
