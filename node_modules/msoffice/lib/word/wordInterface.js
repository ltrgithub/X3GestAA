...
exports.WordInterface = helpers.defineClass(WordInterface, null, {
    ...
    publishDocumentX3: function(docUrl) {
    	...
    	function displayDiagnoses(error) {
    		self.wordInterfaceImpl.signalError(true, error && error.data);
    		/* To display in the browser use this, but it's better to show a Popup message and close the dialog
		...
		try {
			...
			var imagePUTUrl = baseUrl + "/IMAGE('" + x3Class + "~" + x3Prop + "~" + wc.$uuid + "~" + x3Key + "')";
		...
		var xhr = new XMLHttpRequest();
		xhr.open('POST', refererUrl, false);
