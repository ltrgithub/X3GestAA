...
exports.render = function(_, ctx, result,  proto) {
	...
	if (format.indexOf(httpHelper.mediaTypes.word_mailmerge) > -1) {
		return _docx_mailmerge(_, response, url, urlRep, proto);
		...
		return _docx_report(_, response, url, urlRep, proto);
	}
	
};

function _docx_mailmerge(_, response, url, urlRep, proto) {
...
function _docx_report_tpl(_, ctx, data, prototype) {
	var urlRep = ctx.url + "?representation=" + ctx.parameters.representation;
...
function _docx_report(_, response, url, urlRep, prototype) {
}
