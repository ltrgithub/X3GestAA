...
function _docx_serve_empty_template(_, ctx, customDataObj, test_template) {
	...
	}).add(_, { path: __dirname + tplPath}).finish(_);
}

function _docx_get_storage_area_template(_, doc_uuid) {
	var doc = {};
	var	filter = {	jsonWhere : {	$uuid : doc_uuid	}	};
...
function _docx_serve_template(_, ctx, doc_uuid, customDataObj) {
	...
	doc = _docx_get_storage_area_template(_, doc_uuid);
