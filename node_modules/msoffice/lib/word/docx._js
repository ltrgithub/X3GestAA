...
function _docx_mailmerge(_, ctx, result, proto) {
	...
	{
		_docx_serve_storage_template(_, ctx, doc_uuid, customData);
	}
}
...
function _docx_report_tpl(_, ctx, result, prototype) {
	_docx_serve_empty_template(_, ctx, _docx_build_custom_data(ctx, result, "rpt_build_tpl", "1"));
}

function _docx_report_mod_tpl(_, ctx, result, prototype) {
	var doc_uuid 	= (ctx.parameters && ctx.parameters.doc_uuid) || "";
	_docx_serve_storage_template(_, ctx, doc_uuid, _docx_build_custom_data(ctx, result, "rpt_is_tpl", "1"));
}

function _docx_report(_, ctx, result, prototype) {
	var doc_uuid 	= (ctx.parameters && ctx.parameters.doc_uuid) || "";
	_docx_serve_storage_template(_, ctx, doc_uuid, _docx_build_custom_data(ctx, result, "rpt_fill_tpl", "1"));
}
...
function _docx_serve_storage_template(_, ctx, doc_uuid, customDataObj) {
	var customDataXml = _docx_build_custom_data_xml(customDataObj);
