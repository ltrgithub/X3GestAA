...
function _docx_report_mod_tpl(_, ctx, result, prototype) {
	var doc_uuid 	= (ctx.parameters && ctx.parameters.doc_uuid) || "";
	_docx_serve_template(_, ctx, doc_uuid, _docx_build_custom_data(ctx, result, "rpt_is_tpl", "1"));
}

function _docx_report(_, ctx, result, prototype) {
	var doc_uuid 	= (ctx.parameters && ctx.parameters.doc_uuid) || "";
	_docx_serve_template(_, ctx, doc_uuid, _docx_build_custom_data(ctx, result, "rpt_fill_tpl", "1"));
}
