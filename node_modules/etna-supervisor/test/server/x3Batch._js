...
var x3Helper = require("etnaetna-supervisor/test/fixtures/x3Helper");
...
if (!config.unit_test || !config.unit_test.x3endpoint || !config.unit_test.run || !config.unit_test.run.x3Batch) {
	return test('X3 BATCH UNIT TESTS DISABLED FOR NOW', 1, function() {
	...
	(function test(_) {
		...
		var Batch = function(_class, _repName) {
			...
			this.update = function(_, _resource, _key) {
				...
				_resource.$baseUrl = "/sdata/x3/erp/" + config.unit_test.QLF.dataset;
