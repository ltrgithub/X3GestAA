...
function randomDCB(integer, decimals) {
	return new BCD(bcd.fromDouble(((Math.random() > 0.5) ? 1 : -1) * Math.floor((Math.pow(10, integer + decimals) - 1) * Math.random()) / Math.pow(10, decimals)));
}
			...
			try {
				...
				[0, maxRows - 1].forEach(function(i) {
					...
					Object.keys(values[i]).forEach(function(key) {
						...
						strictEqual(cmp, 0, "Value of [" + i + "]." + key + "=" + values[i][key].x3ToString());
					});
					console.log("done");
				});
			} catch (e) {
