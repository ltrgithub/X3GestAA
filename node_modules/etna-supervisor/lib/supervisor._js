...
function mongoConnect(_, config) {
	...
	var server = new mongodb.Server(config.mongo.host || "localhost", config.mongo.port || 27017, {});
...
function loadErrorMessages(_, mongo, lan) {	
	...
	var mess = mongo.collection('APLSTD', _).find({				
		LAN: lan,
		LANCHP: 9014
	}).sort({LANCHP:1, LANNUM:1}).toArray(_).filter( function(m) {
...
exports.create = function(_, config) {
	...
	function sqlReader(_, cnx, sql, args) {
		//trace && trace("SQL: " + sql + ", args=" + args);
		//if (!/^select /.test(sql) && !conf.etnaSqlStore.writable) throw new Error("cannot modify: SQL store is read-only");
		return sqlDriver.reader(cnx, sql, args || []).map(function(_, record) {
			return trim(record);
		});
		//return Array.isArray(result) ? result.map(trim) : result;
	}
	...
	var superv = {
		...
		loadContextChapter: function(_,chap) {
			return mongoStore.collection('ACONTEXT', _).find({CHAPTER:chap}, _).toArray(_)[0];
		},
		...
		LAN : function(_,lan) {
			if(arguments.length > 1) return superv.LAN.value = lan;
		...
		LANDEF : function(_) {
			...
			return sqlReader(_,
				cnx,"select LANDEF_0 from ADOSSIER where (DOSSIER_0=:1)",
				[config.folderName]).limit(1).toArray(_);
			});
		...
		LANISO : function(_,laniso) {
			...
			var rTablan = sqlDriver.withConnection(_, function(_, cnx) {
				return sqlReader(_,
						cnx,"select LAN_0 from TABLAN where (LOWER(LANISO_0)=:1)",
						[superv.LANISO.value.toLowerCase()]).limit(1).toArray(_);
