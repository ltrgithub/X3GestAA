...
exports.create = function(_, config) {
	...
	var superv = {
		...
		load: function(_, className, keyValue) {
			...
			if (instance !== undefined && (!data || !data._etag || !instance._etag || (data._etag === instance._etag))) return instance;
			if (data) {
		...
		currentContext: function(_) {
			...
			tracerJs.debug && tracerJs.debug("load rights " + superv.context.builtinProperties.USER().getValue(_));
			superv.context._x3Rights.getJsonRigths(_, superv.context.builtinProperties.USER().getValue(_));
		...
		LAN: function(_, lan) {
			...
			//VVV
			superv.LAN.value = superv.LAN.value || "FRA" || superv.LANDEF(_);
