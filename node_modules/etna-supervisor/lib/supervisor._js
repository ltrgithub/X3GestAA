...
exports.create = function(_, config) {
	...
	function loadRights(_, user) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			...
			params = [user, 2];
			res.$accessCodes = sqlDriver.execute(_, cnx, "select A.CODACC_0 from ACCES A Where A.USR_0=" + param(0) +
			...
			if (allfct === 2 && config.folderName !== "SUPERV") {
				res.$authorizedFunctions.push("*").join(",");
			} else {
				if (config.folderName === dosx3) {
					...
					res.$authorizedFunctions = sqlDriver.execute(_, cnx, "select A.CODINT_0 from AFONCTION A " +
						"Where " + crit + " order by A.CODINT_0").map(trim).reduce(function(wafc, afc) {
						wafc.push(afc.CODINT);
						return wafc;
					}, []);
				} else if (allfct === 2) {
					res.$authorizedFunctions = sqlDriver.execute(_, cnx, "select A.CODINT_0 from AFONCTION A order by A.CODINT_0").map(trim).reduce(function(wafc, afc) {
						wafc.push(afc.CODINT);
						return wafc;
					}, []);
				} else {
					params = [userPrfCod];
					res.$authorizedFunctions = sqlDriver.execute(_, cnx,
						"select AFC.CODINT_0 " +
			...
			res.$disabledFunctions = sqlDriver.execute(_, cnx,
				"select AFC.CODINT_0 " +
			...
			var rsAsw = sqlDriver.execute(_, cnx, "select ASW.CODREP_0,ASW.AFCRIGHT_0,ASW.CODCLA_0 from ASHW ASW where " + criteria + " and (ASW.CODACT_0=' ' or " +
				...
				var indexFunction = res.$authorizedFunctions.indexOf(asw.AFCRIGHT);
			...
			Object.keys(rsAsw).forEach_(_, function(_, key) {
				...
				if (rsAsw[key].afcright === "" || allfct === 2 || rsAsw[key].indexFunction >= 0) {
					if (rsAsw[key].codrep !== rsAsw[key].codcla) {
						repAuth[key].$entity = rsAsw[key].codcla;
					}
				}
				if (rsAsw[key].indexFunction >= 0 && rsSites[rsAsw[key].afcright] !== undefined) {
					repAuth[key].$sites = rsSites[rsAsw[key].afcright].join(",");
				}
					...
					if (objX3 !== "") {
						...
						if (table !== "") {
							if (rsArl[objX3]) {
								...
								if (keys !== "") {
									repAuth[key].$conditions = [];
									var condition = {
										$type: "authorization",
										$where: ""
									};
									condition.$where = "(" + rsArl[objX3].fld;
									if (keys.length > 1) {
										condition.$where += " in ('" + keys.join("','") + "'))";
									} else {
										condition.$where += " eq '" + keys.join("','") + "')";
									}
									repAuth[key].$conditions.push(condition);
								}
							}
			...
			res.$authorizedRepresentations = repAuth;
		});


		res.$accessCodes = res.$accessCodes.join(",");
		res.$disabledFunctions = res.$disabledFunctions.join(",");
		res.$authorizedFunctions = res.$authorizedFunctions.join(",");
		//res.$authorizedRepresentations = res.$authorizedRepresentations.join(",");
		return res;
	...
	var superv = {
		...
		currentContext: function(_) {
			...
			return (superv.context = (new(require('etna-supervisor/lib/builtins/ACTX').constructor)(superv)).init(_));
		...
		LAN: function(_, lan) {
			...
			superv.LAN.value = superv.LAN.value || superv.LANDEF(_);
