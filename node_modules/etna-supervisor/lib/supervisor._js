...
exports.create = function(_, config) {
	...
	function executeSql(_, sql, args, opts) {
		...
		if (glob.context.x3session.dbConnection) {
			...
			var result = sqlDriver.withConnection(_, function(_, cnx) {
				return sqlDriver.execute(_, cnx, sql, sqlargs, opts);
			});
		}
	...
	/*
	sqlReader convenience supports 2 types of call :
		sqlReader(_, cnx, sql, args, typesmap, dispatch)
		sqlReader(_, sql, args, typesmap, dispatch)
	*/
	//function sqlReader(_, cnx, sql, args, typesmap, dispatch) {
	function sqlReader(_, a, b, c, d, e) {
		var cnx = (typeof a === "string") ? undefined : a;
		var sql = cnx ? b : a;
		var args = cnx ? c : b;
		var typesmap = cnx ? d : c;
		var dispatch = cnx ? e : d;
		var withConnection = cnx ? function(_, f) {
				return f(_, cnx);
			} : sqlDriver.withConnection;

		tracerJs.debug && tracerJs.debug("SQL: " + sql + ", args=" + args);
		//if (!/^select /.test(sql) && !conf.etnaSqlStore.writable) throw new Error("cannot modify: SQL store is read-only");

		var maptrim = typesmap ? utilDriver.maptrim(sqlDriver, typesmap, dispatch) : trim;

		return withConnection(_, function(_, cnx) {
			return sqlDriver.reader(_, cnx, sql, args || []).map(function(_, record) {
				return typesmap ? maptrim(_, record) : trim(record);
			});
		});
	}
