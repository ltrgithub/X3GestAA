...
exports.create = function(_, config) {
	...
	function executeSql(_, sql, args, opts) {
		...
		tracerJs.debug && tracerJs.debug("executeSql: " + sql + ", args=" + args);
	...
	function sqlReader(_, cnx, sql, args, opts) {
		if (typeof cnx === "string") throw new Error("bad cnx parameter: " + cnx);
		opts = opts || {};
		...
		tracerJs.debug && tracerJs.debug("sqlReader: " + sql + ", args=" + args);
		//if (!/^select /.test(sql) && !conf.etnaSqlStore.writable) throw new Error("cannot modify: SQL store is read-only");
		...
		return withConnection(_, function(_, cnx) {
			return sqlDriver.reader(_, cnx, sql, args || [], opts).map(function(_, record) {
