...
exports.create = function(_, name) {
	...
	var conf = config[name];
	var sqlPool = connectionPool.create(conf.etnaSqlStore);
	var cache = {
		ATEXTE: {},
		APLSTD: {},
		scripts: {},
	...
	var sqlDriver = require('etna-supervisor/lib/drivers/' + conf.etnaSqlStore.driver)(conf.etnaSqlStore);

	var superv = {
		LAN: "FRA",
		// for now
		baseUrl: "/sdata/etna/erp/" + name,
		...
		loadScript: function(_, name) {
			...
			if (!script) {
				...
				if (!fs.exists(path, _)) path = conf.scriptsRoot + "/" + name + ".src";
