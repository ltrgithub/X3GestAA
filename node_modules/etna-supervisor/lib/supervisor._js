...
exports.create = function(_, name) {
	...
	function executeSql(_, sql, args) {
		...
		var result = sqlPool.withConnection(_, function(_, cnx) {
	...
	function sqlReader(_, cnx, sql, args) {
		//trace && trace("SQL: " + sql + ", args=" + args);
		if (!/^select /.test(sql) && !conf.etnaSqlStore.writable) throw new Error("cannot modify: SQL store is read-only");
		var reader = cnx.reader(sql, args || []);
		return ez.devices.oracle.reader(reader).map(function(_, record) {
			return trim(record);
		});
		//return Array.isArray(result) ? result.map(trim) : result;
	}
	...
	var superv = {
		...
		sqlPool: sqlPool,
		sqlDriver: require('etna-supervisor/lib/drivers/' + conf.etnaSqlStore.driver),
