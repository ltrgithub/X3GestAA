...
var X3Rights = util.defineClass(function(superv) {
	...
	loadRightsFromBase: function loadRights(_, login, wetag) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			var params = [login];
			var userPrfCod = "";
			var wuser = sqlDriver.execute(_, cnx, "select U.USR_0,U.PRFFCT_0,U.CODMET_0 from AUTILIS U Where U.LOGIN_0=" + param(0), params).map(trim);
			...
			tracerJs.debug && tracerJs.debug("userPrfCod : " + userPrfCod);
			...
			if (allfct === 2 && self.supervisor.folderName !== "SUPERV") {
				...
				if (self.supervisor.folderName === dosx3) {
					...
					tracerJs.debug && tracerJs.debug("adonix " + adonix + " " + crit);
