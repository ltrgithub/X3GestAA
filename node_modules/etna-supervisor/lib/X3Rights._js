...
var X3Rights = util.defineClass(function(superv) {
	...
	loadRightsFromBase: function loadRights(_, login, wetag) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			...
			var usrPrfFct = "";
			var usrPrfMen = "";
			var wuser = sqlDriver.execute(_, cnx, "select U.USR_0,U.PRFFCT_0,U.CODMET_0,U.PRFMEN_0 from AUTILIS U Where U.LOGIN_0=" + param(0), params).map(trim);
			...
			if (usrPrfMen === "" && wuser[0]) {
				...
				var profilMetier = sqlDriver.execute(_, cnx, "select M.PRFFCT_0,M.PRFMEN_0 from AMETUTI M Where M.CODMET_0=" + param(0), params).map(trim);
			...
			params = [usrPrfFct];
			var wafctfct = sqlDriver.execute(_, cnx, "select U.ALLFCT_0, U.PRFCOD_0, U.ALLACS_0 from AFCTFCT U Where U.PRFCOD_0=" + param(0), params).map(trim);
			...
			if (allfct === 2 && self.supervisor.folderName !== "SUPERV") {
				...
				if (self.supervisor.folderName === dosx3) {
					if (allfct === 2) {
						params = [usrPrfFct];
						res._authorizedFunctions = sqlDriver.execute(_, cnx, "select AFC.CODINT_0 from AFONCTION AFC ,AMENUSER AMU where " +
							" AMU.CODPRF_0=" + param(0) + " and AMU.FONCTION_0=AFC.CODINT_0 " +
							"order by AFC.CODINT_0", params).map(trim).reduce(function(wafc, afc) {
					...
					res._authorizedFunctions = sqlDriver.execute(_, cnx,
						...
						"and AFP.PRFCOD_0=" + param(0) + " and AFP.FNC_0=AFC.CODINT_0 and AFP.ACS_0=2) order by AFC.CODINT_0", params).map(trim).reduce(function(wafc, afc) {
						wafc.push(afc.CODINT);
						return wafc;
					}, []);
				}
