...
var X3Rights = util.defineClass(function(_, superv) {
	...
	getRigths: function(_, login) {
		var self = this;

		tracerJs.info && tracerJs.info("getRigths login = " + login);
		...
		tracerJs.debug && tracerJs.debug("loadRigthsFromMongo " + login + "  " + self._etag);
		var data = {};
		data = self.loadRigthsFromMongo(_, login, self._etag);
		if (data) return self._rigths = data;

		tracerJs.debug && tracerJs.debug("loadRigthsFromBase " + login + "  " + self._etag);
		data = self.loadRigthsFromBase(_, login, self._etag);
		if (data) return self._rigths = data;
	},
	getJsonRigths: function(_, login) {
		var self = this;
		var res = self.getRigths(_, login);
	...
	loadRigthsFromMongo: function(_, login, wetag) {
		var self = this;
	...
	loadRigthsFromBase: function loadRights(_, login, wetag) {
