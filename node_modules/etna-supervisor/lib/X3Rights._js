...
var X3Rights = util.defineClass(function(_, superv) {
	...
	loadRigthsFromMongo: function(_, user, wetag) {
		...
		var filter = {};
		filter["endpoint"] = self.supervisor.endpointName;
		filter["user"] = user;
		var data = self.supervisor.loadRightsCache(_, filter);
	...
	loadRigthsFromBase: function loadRights(_, user, wetag) {
		...
		var update = self.supervisor.updateRightsCache(_, user, wetag, res);
		return res;
	}
});
