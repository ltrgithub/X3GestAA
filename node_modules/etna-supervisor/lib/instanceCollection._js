...
Object.defineProperty(InstanceCollection.prototype, "crud", {
	value: function(_, f) {
		...
		try {
			...
			console.error(e);
			var status = this.$parent.setStatus(_, "", this.supervisor.constants.CST_AERROR, e.message);
...
Object.defineProperty(InstanceCollection.prototype, "add", {
	value: function(_, position) {
		...
		self.crud(_, function ADDLINE(_, col) {
			...
			line.$astalin = col.supervisor.constants.CST_ANEW;
			col.$astalin = col.supervisor.constants.CST_AUPD;
...
Object.defineProperty(InstanceCollection.prototype, "del", {
	value: function(_, position) {
		...
		self.crud(_, function ADELLINE(_, col) {
			// Memorize the line to be deleted in the database:
			col.linesToDelete = col.linesToDelete || [];

			deletedAt = (position === col.supervisor.constants.CST_ALASTPOS) ? col.lines.length - 1 : position;

			col.linesToDelete.push(col.lines[deletedAt]);
			// Remove the line  
			col.lines.splice(deletedAt, 1);

			return col.linesToDelete[col.linesToDelete.length - 1];
		});
		return deletedAt;
	},
	enumerable: false
...
Object.defineProperty(InstanceCollection.prototype, "applyOperationToEach", {
	value: function(_, f) {
		var status;
		// straigth loop on lines;
		for (var i = 0, len = this.lines.length; i < len; i++) {
			status = f(_, this.lines[i]);
			if (status === this.supervisor.constants.CST_AERROR) return status;
		}
		return status;
	},
...
Object.defineProperty(InstanceCollection.prototype, "applyToEach", {
	value: function(_, f) {
		var status = this.supervisor.constants.CST_AERROR;

		// 1. remove lines to delete
		if (this.linesToDelete) {
			for (var i = 0, len = this.linesToDelete.length; i < len; i++) {
				status = f(_, this.linesToDelete[i], "delete");
				if (status === this.supervisor.constants.CST_AERROR) return status;
			}
		}
		// 2. update lines which were modified:
		var linesToInsert = [];
		for (var i = 0, len = this.lines.length; i < len; i++) {
			var line = this.lines[i];

			if (line.$astalin === this.supervisor.constants.CST_AUPD) {
				status = f(_, line, "update");
				if (status === this.supervisor.constants.CST_AERROR) return status;
			} else if (line.$astalin === this.supervisor.constants.CST_ANEW) {
				// To be handled in the last loop:
				linesToInsert.push(line);
			}
		}
		// 3. insert new lines:
		for (var i = 0, len = linesToInsert.length; i < len; i++) {
			status = f(_, linesToInsert[i], "insert");
			if (status === this.supervisor.constants.CST_AERROR) return status;
		}
		return status;
	},
...
Object.defineProperty(InstanceCollection.prototype, "onPropagate", {
	value: function(_, property) {
		return this.applyOperationToEach(_, function(_, line) {
