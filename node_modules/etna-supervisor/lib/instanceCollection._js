...
Object.defineProperty(InstanceCollection.prototype, "del", {
	value: function(_, position) {
		...
		self.crud(_, function ADELLINE(_, col) {
			...
			col.lines[deletedAt].$astalin = (col.lines[deletedAt].$astalin === col.supervisor.constants.CST_ANEW) ?
				col.supervisor.constants.CST_ANEWDEL :
				col.supervisor.constants.CST_ADEL;
...
Object.defineProperty(InstanceCollection.prototype, "applyToEach", {
	value: function(_, f) {
		...
		var lines = this.lines.filter(function(line) {
			return line.$astalin > self.supervisor.constants.CST_ANEWDEL;
		...
		for (var i = 0, len = lines.length; i < len; i++) {
			var line = lines[i];
			var operation = operations[line.$astalin];
			if (operation) {
				status = f(_, line, operation);
				if (status === this.supervisor.constants.CST_AERROR) return status;
			}
		}
		return status;
