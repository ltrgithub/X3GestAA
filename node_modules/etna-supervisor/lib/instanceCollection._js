...
var InstanceCollection = helpers.defineClass(function(parent, collection) {
	...
	newline: function(_) {
		var line = this.$parent.supervisor.new(_, 'Instance', this.class, this.$parent).afterCreate(_);
		line.bindAttributes(this);
		line.AINIT(_);
	...
	getDiagnoses: function(_) {
		...
		this.applyToEach(_, function(_, line, operation) {
			...
			res = res.concat(lerr.map(function(d) {
				d.PRO = self.name + ((d.PRO !== "") ? '.' + d.PRO : "");
				return d;
			}));
		}, "getDiagnoses");
		return res;
		/*
		var self = this;
		return this.lines.reduce(function(r, d) {
			return r.concat(d.getDiagnoses().map(function(d) {
				d.PRO = self.name + ((d.PRO !== "") ? '.' + d.PRO : "");
				return d;
			}));
		}, []);
