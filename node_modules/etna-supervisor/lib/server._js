...
var routes = [{
	...
	handler: function(_, context, codcla, key) {
		...
		if ((facet !== '$query' && facet !== '$lookup' && facet !== '$bulk') && !key) throw new HttpError(400, "key value missing in " + facet + " facet");
		...
		else return repr.query(_, context, facet, qs); //res = repr.query(_, facet, qs);
	...
	handler: function(_, context, user) {
		var wetag = context.supervisor.executeSql(_, "Select A.VALEUR_0 From ADOVAL A where A.CMP_0=' ' and A.FCY_0=' ' and A.PARAM_0='RIGHTSETAG'");
		var rightsEtag;
		var currentEtag = new Date().toJSON();
		if (wetag[0] && wetag[0].VALEUR > "") {
			rightsEtag = wetag[0].VALEUR;
		} else {
			var updEtag = context.supervisor.executeSql(_, "update ADOVAL A SET A.VALEUR_0 = '" + currentEtag +
				"' WHERE A.CMP_0=' ' and A.FCY_0=' ' and A.PARAM_0='RIGHTSETAG'");
			trace && trace("Update in base, etag : " + JSON.stringify(updEtag));
		}
		var headers = {
			'content-type': 'application/json',
			'etag': rightsEtag || currentEtag
		};
		trace && trace(' ETna rightsEtag base :' + rightsEtag);
		trace && trace(JSON.stringify(headers));
		trace && trace('context.request._request.headers["if-none-match"] ' + context.request._request.headers["if-none-match"]);
		context.supervisor._x3Rights = context.supervisor.loadRights(_, user, (rightsEtag || currentEtag));
		if (rightsEtag === context.request._request.headers["if-none-match"]) {
			trace && trace("Value in Base : " + wetag[0].VALEUR);
			context.jsonReply(_, 304, headers);
		} else {
			var res = context.supervisor._x3Rights;
			res._accessCodes = res._accessCodes.join(",");
			res._disabledFunctions = res._disabledFunctions.join(",");
			res._authorizedFunctions = res._authorizedFunctions.join(",");
	...
	handler: function(_, context, codcla, key, service) {
		...
		var repr = context.supervisor.load(_, 'Representation', codrep);
		if (!repr) throw new HttpError(404, "representation not found: " + codrep);
		if (facet !== '$query') throw new HttpError(400, "bad facet: " + facet);
		if (repr.data.CODCLA !== codcla) throw new HttpError(400, "class/representation mismatch: " + repr.data.CODCLA + " <> " + codcla);

		var res = repr.service(_, key, service);
		context.jsonReply(_, 200, res);
	},
}];
