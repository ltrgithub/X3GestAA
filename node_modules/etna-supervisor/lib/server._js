...
var HttpContext = util.defineClass(function(request, response, superv, qs) {
	...
	checkMethod: function(method) {
		if (this.request.method !== method) throw new HttpError(505, "bad method " + this.request.method);
	},
	private: {
		get: function() {
...
var routes = [{
	...
	handler: function(_, context, superv) {
		var lis = context.superv.executeSql(_, "select R.MODULE_0, R.CODREP_0, R.CODCLA_0, R.INTREP_0, C.TYPCLA_0 " + //
		"from ASHW R, ACLASSE C where R.CODCLA_0 = C.CODCLA_0 and C.TYPCLA_0 = 2 order by CODREP_0").map_(_, function(_, rep) {
			return '<tr><td><a href="/syracuse-main/html/main.html?url=/sdata/etna/erp/SUPERV/' + //
			rep.CODCLA + '?representation=' + rep.CODREP + '.$query">' + rep.CODREP + '</td><td>' + //
			context.superv.loadText(_, parseInt(rep.INTREP, 10)) + '</a></td></tr>';
		}).join('\n');
	...
	handler: function(_, context, codrep, facet) {
		...
		var repr = context.superv.load(_, 'Representation', codrep);
	...
	handler: function(_, context, codcla, key) {
		context.checkMethod("GET");
		key = parseKey(key);
		var clas = context.superv.load(_, 'Class', codcla);
	...
	handler: function(_, context, codcla, key) {
		...
		var wc = workingCopies[qs.trackingId] || (workingCopies[qs.trackingId] = context.superv.new(_, 'WorkingCopy', res, qs.trackingId));
...
exports.httpDispatch = function(_, request, response) {
	try {
		...
		if (request.url === '/sdata/etna') request.url = '/sdata/etna/erp/' + Object.keys(config)[0] + '/';
		var m = /^\/sdata\/(?:etna|x3)\/erp\/(\w+)\/([^?]*)\??(.*$)/.exec(request.url);
		if (!m) throw new HttpError(404, "resource not found: " + request.url);
		var dataset = m[1],
			path = m[2],
			qs = parseQueryString(m[3]);
		var superv = supervisors[dataset] || (supervisors[dataset] = supervisor.create(_, dataset));
