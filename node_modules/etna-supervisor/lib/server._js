...
var routes = [{
	...
	handler: function(_, context, keyword, keyvalue, facet) {
		...
		if (keyword.indexOf("QUERY~") === 0 && facet === "$query") {
			...
			var proto = module("requester").proto(_, context.supervisor, script, keyvalue, facet);
			if (!proto) throw new HttpError(404, "query not found: " + script);
			context.reply(_, 200, proto);
		} else if (keyword === "STATS" && facet === "$cube") {
			var proto = module("statistics").proto(_, context.supervisor, keyvalue, facet);
	...
	handler: function(_, context, codcla, key) {
		...
		key = parseKey(key);
		if (codcla === 'QUERY') {
			var res = module("requester").query(_, context, key, context.codrep);
		...
		if (facet !== "$search") {
			...
			var repr = context.supervisor.load(_, 'Representation', codrep);
			if (!repr.class.isSearchable) throw new HttpError(406, "search not available for class: " + codcla);
...
exports.httpDispatch = function(_, config, request, response) {
	try {
		...
		tracerJs.debug && tracerJs.debug("DBGURL=" + request.url);
