...
var requester = require("etna-supervisor/lib/requester");
var statistics = require("etna-supervisor/lib/statistics");
...
var routes = [{
	...
	handler: function(_, context, keyword, keyvalue, facet) {
		...
		if (keyword.indexOf("QUERY~") === 0 && facet === "$query") {
			...
			var proto = requester.proto(_, context.supervisor, script, keyvalue, facet);
			if (!proto) throw new HttpError(404, "query not found: " + script);
			context.reply(_, 200, proto);
		} else if (keyword === "STATS" && facet === "$cube") {
			var proto = statistics.proto(_, context.supervisor, keyvalue, facet);
	...
	handler: function(_, context, codcla, key) {
		...
		key = parseKey(key);
		if (codcla === 'QUERY') {
			var res = requester.query(_, context, key, context.codrep);
			return context.jsonReply(_, 200, res);
		} else if (codcla === 'STATS') {
			var res = statistics.query(_, context, key, context.codrep);
			return context.jsonReply(_, 200, res);
		}
	...
	handler: function(_, context, user) {
		...
		trace && trace("headers :" + JSON.stringify(headers));
...
exports.httpDispatch = function(_, config, request, response) {
	try {
		...
		//console.log("DBGSESSION=" + JSON.stringify(config.session));
		...
		//console.log(JSON.stringify(config.session));
