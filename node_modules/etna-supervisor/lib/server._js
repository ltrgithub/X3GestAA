...
var routes = [{
	...
	handler: function(_, context, keyword, process, facet) {
		context.checkMethod("GET");
		//VVV manage STAT, PROCESS.....
		if (!(keyword === "PROCESS" && process === "UNKNOWN" && facet === "$details"))
			throw new HttpError(404, keyword + " not yet managed: " + process + ' facet=' + facet);
	...
	handler: function(_, context, user) {
		...
		var res = context.supervisor.currentContext(_)._x3Rights.getJsonRigths(_, user);
		var rightsEtag = context.supervisor.currentContext(_)._x3Rights._etag;
		var headers = {
