...
var routes = [{
	...
	// sotkage en base si vide a faire
	// creer var repsonse herder pour etag
	...
	handler: function(_, context, user) {
		...
		if (wetag[0] && wetag[0].VALEUR > "") {
			...
			// TODO ecrire en base
			var d = new Date();
			var updEtag = context.supervisor.executeSql(_, "update ADOVAL A SET A.VALEUR_0 = '" + d.toJSON() +
		...
		trace && trace('rightsEtag base :' + rightsEtag);
		...
		if (rightsEtag === context.request._request.headers["if-none-match"]) {
			...
			var res = context.supervisor.loadRights(_, user);
			context.jsonReply(_, 200, res, headers);
		}
	},
}, {
