...
exports.constructor = util.defineClass(function(superv, anchor, data) {
	...
	name: {
		get: function() {
			...
			} else return this.data.CODLNK;
		}
	},
	prepareKeyparams: function(repr) {
		...
		switch (this.data.TYPLNK) {
			case 1:
				...
				switch (this.data.CMPLNK) {
					...
					case 9:
						// lookup
						node.$url = this.url(_, "$lookup", false, false) + this.paramsFilterExpression(_);
						break;
					case 10:
						// summary
			...
			case 3:
				...
				node.$method = "POST";
				node.$parameters = {};
				node.$url = "{$baseUrl}/" + this.data.CLALNK + "('" + this.keyExpression(_) + "')/" + //
	...
	paramsFilterExpression: function(_) {
		...
		this.params.filter(function(paramData) {
			return paramData.PARFIL === 2 && (paramData.alias || paramData.VALEUR);
	...
	params4Expression: function(_) {
		function isAfcObjGES3(_, link) {
			...
			var self = link;
			var recordset = link.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return link.supervisor.sqlReader(_, cnx, sql, params).toArray(_);
			});
			if (recordset) {
				if (recordset[0].ACTION === "GOBJET" && recordset[0].CODPAR === "OBJET" && recordset[0].VALPAR !== "") {
					...
					var recordset2 = link.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
						return link.supervisor.sqlReader(_, cnx, sql2, params2).toArray(_);
					});
					if (recordset2) {
	...
	url: function(_, facet, withKey, withWorkingCopy, withTemplate) {
