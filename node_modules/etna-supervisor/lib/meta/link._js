...
exports.constructor = util.defineClass(function(superv, anchor, data) {
	...
	prototypeNode: function(_) {
		var node = {
			$title: this.superv.loadText(_, this.data.INTLNK),
		};
		...
		case 3:
			...
			node.$type = "application/json;vnd.sage=syracuse";
			node.$method = "POST";
			node.$url = "{$baseUrl}/" + clas.name + "('" + clas.instanceFactory.keyExpression(_) + "')/" + //
		...
		case 4:
			// convergence
			node.$method = "POST";
			node.$url = this.superv.transUrl + "/$sessions?f=" + this.data.CODFNCLNK;
			//node.$debug = this.data;
			break;
		case 5:
			switch (this.data.CMPLNK) {
