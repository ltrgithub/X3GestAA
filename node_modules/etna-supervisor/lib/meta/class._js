	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			if (this.data.TYPCLA === 2) { // 2 for persistent class
				...
				try {
					var jsInterface = require("etna-supervisor/lib/builtins/" + this.data.CODCLA);
					if (jsInterface) {
		...
		queryInstances: function(_, select, max) {
			return this.reader(_, select, max, null);
		},
		sqlReader: function(_, select, max, $recordMapper) {
			...
			Object.keys(properties).forEach_(_, function(_, name) {
				//dbg:tracerJs.debug && tracerJs.debug("getSql:'" + name +"'");
		...
		jsReader: function(_, select, max, $recordMapper) {
		...
		searchResourceFactory: function(_) {
			...
			var self = this;
			if (this.data.ACCSTR) {
				factory.$access = function(property) {
			...
			factory.properties = Object.keys(this.properties)
				.filter(function(property) {
					return ((self.properties[property].isSearchable && !/.*_REF$/.test(property)) || self.properties[property].collection);
				})
				.reduce_(_, function(_, r, property) {
					r[property] = {
						resourceFactory: factory,
						supervisor: self.supervisor,
						type: self.properties[property].type,
						tag: property,
						prototypeNode: self.properties[property].prototypeNode,
						data: util.clone(self.properties[property].data)
					};
					if (self.properties[property].collection) {
						var targetResourceFactory = self.properties[property].collection.class.searchResourceFactory(_);
						if (targetResourceFactory) {
							r[property].collection = self.properties[property].collection;
			...
			if (!Object.keys(factory.properties).length) return;

			factory.class = this;
			factory.fillTypeNode = function(_, node) {
			...
			factory.searchItemProto = function(_, facet) {
				var proto = {
					$title: this.class.supervisor.loadText(_, this.class.data.INTCLA) + " {$key}",
				};
				if (facet) {
		...
		searchProto: function(_) {
			...
			return {
				...
				$classTitle: this.supervisor.loadText(_, this.data.INTCLA),
				$url: "{$baseUrl}/" + this.name + "?representation=" + this.name + ".$search",
				$prototype: "{$baseUrl}/$prototypes('{$representation}.$thumb')",
				$type: "{$baseType}." + this.name + ".$search",
				$properties: {
					$resources: {
						$type: "application/x-array",
						$item: factory.searchItemProto(_, "$searchItem"),
					}
				},
				$links: {
					$search$details: {
						$title: "Detail",
						$default: true,
						$url: "{$baseUrl}/" + this.name + "('{$key}')?representation=" + this.name + ".$details",
						$devices: "desktop"
					}
				}
			};
		...
		search: function(_, httpContext, qs) {
			...
			this.reader(_, select, Infinity).map(fromInstanceToResource).pipe(_, dst);
		}
	}),
};