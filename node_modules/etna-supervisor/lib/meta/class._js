...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		if (data.PROPERTIES.NUMFLD && typeof data.PROPERTIES.NUMFLD === 'string') {
			if (!isNaN(data.PROPERTIES.NUMFLD) && bcd.toDouble(bcd.fromString(data.PROPERTIES.NUMFLD)) > 0) data.PROPERTIES.NUMFLD = bcd.toDouble(bcd.fromString(data.PROPERTIES.NUMFLD));
			else delete data.PROPERTIES.NUMFLD;
		}
		if (data.PROPERTIES.LONG && typeof data.PROPERTIES.LONG === 'string') {
			if (!isNaN(data.PROPERTIES.LONG) && bcd.toDouble(bcd.fromString(data.PROPERTIES.LONG)) > 0) data.PROPERTIES.LONG = bcd.toDouble(bcd.fromString(data.PROPERTIES.LONG));
			else delete data.PROPERTIES.LONG;
		}
		this.data = data;
		this.collections = {};
		this.methods = {};
		...
		init: function(_) {
			...
			if (this.data.dataChildren) {
				...
				this.data.CODCLA = this.data.dataChildren.CODCLA;
			}
			tracerJs.debug && tracerJs.debug("Init de  " + this.data.CODCLA);
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if (!p) {
					...
					coln.class = this.supervisor.load(_, 'Class', this.data.CODCLA + "." + targetName, {
						ownMapping: coln.mappingData,
						destMapping: mappingDest,
						CODCLA: this.data.CODCLA + "." + targetName
					});
