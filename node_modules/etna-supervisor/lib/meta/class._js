...
var stdMethods = {
	ASETERROR: function(_, path, msg, severity) {
		...
		this.diagnoses.push(this.superv.new(_, 'Diagnosis', severity, path, msg));
		return 0;
	},
	AINIT: function(_) {
		util.checkInstance(this);
	...
	constructor: util.defineClass(function(superv, data) {
		...
		init: function(_) {
			...
			if (this.data.TYPCLA !== 5) {
				this.table = this.table || this.superv.load(_, 'Table', tableCode);
				if (!this.table) throw new Error("table not found: " + tableCode);
			}
			this.collections = {};
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				this.collections[data.CODCOL] = this.superv.new(_, 'Collection', this, data);
			}, this);
			this.instanceFactory = this.superv.new(_, 'InstanceFactory', this);
			this.scripts = this.data.CODTRT.map_(_, function(_, codtrt) {
				return this.superv.loadScript(_, codtrt);
			}, this);
			return this;
		},
		name: {
		...
		invokePropertyScripts: function(_, instance, action, name) {
			...
			this.scripts.forEach_(_, function(_, script) {
				...
				// TODO: review
				var frame = glob.context.x3frame = this.superv.newEngineFrame();
