...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			tracerJs.debug && tracerJs.debug("Init de  C_" + this.data.CODCLA + " path = '" + this.curpath + "'");
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if (!p) {
					...
					coln.class = this.supervisor.load(_, 'Class', this.data.CODCLA + "." + targetName, {
						...
						curpath: (this.curpath) ? this.curpath + "." + name : name
					});
		...
		sqlReader: function(_, select, max, $recordMapper) {
			...
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						var instance = _this.supervisor.new(_, 'Instance', _this, null, _this.addSysProperties());
						var astatus = _this.invokeScripts(_, instance, "$AREAD_BEFORE", "", _this.curpath);
						instance.fromRecord(_, record);
						var astatus = _this.invokeScripts(_, instance, "$AREAD_AFTER", "", _this.curpath);
						return instance;
