	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if (!mapping || mapping.TYPLNK !== 3) { // special treatment reserved to collection where TYPLNK=1x1 :
		...
		queryExpToSelect: function(_, exp, params, tableNames) {
			...
			return Object.keys(exp).reduce_(_, function(_, r, prop) {
				var column = _this.properties[prop].column.sqlName();
				...
				r.push(Object.keys(operators).reduce(function(r, operator) {
					if (map[operator]) {
						...
						if (_this.properties[prop].etnaType === 'D' && (typeof value === 'string')) {
							var date = tdate.parse(value, "yyyy-MM-dd");
							value = new Date(date.year, date.month - 1, date.day);
						} else if (_this.properties[prop].etnaType === 'E') {
						...
						params.push(value);
						r.push(column + ' ' + sqlOperator + ' ' + param(params.length - 1));
					}
					return r;
				}, []).join(" and "));
				return r;
		...
		sqlReader: function(_, select, max, $recordMapper, instanceMapper) {
			...
			// clear the texts cache
			var texts = {};

			Object.keys(properties).forEach_(_, function(_, name) {
				properties[name].getSql(_, columnNames, tableNames, wheres, params, descs, texts);
			});
			this.texts = texts;
			var mapTexts;
			if (_this.texts.properties) {
				mapTexts = function(texts) {
					return function(_, record) {
						record.$id = texts.getKeyRecord(record);
						texts.ids.push(record.$id);
						return record;
					};
				}(_this.texts);
			}
			...
			return this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				...
				if (mapTexts) reader = reader.map(mapTexts);
		...
		loadInstances: function(_, $index, vals, max, select) {
			...
			var select = index.reduce(function(o, i) {
				o[index[i]] = vals[i];
				return o;
			}, {});
			return this.reader(_, select, 1).toArray(_);
		},
		index: {
			get: function() {
