...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		queryExpToSelect: function(exp, params) {
			...
			return Object.keys(exp).reduce(function(r, prop) {
				...
				r.push(Object.keys(operators).reduce(function(r, operator) {
					console.log("queryExpToSelect operator:" + operator);
					if (map[operator]) {
		...
		queryToSelect: function(query, params) {
			...
			if (operator) {
				var _this = this;
				return query.$or.reduce(function(r, exp) {
		...
		queryInstances: function(_, select, max) {
			...
			return this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return _this.supervisor.sqlReader(_, cnx, sql, params).limit(max).map(mapper).toArray(_);
			});
		},
