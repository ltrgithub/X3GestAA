...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		queryInstances: function(_, select, max, instanceMapper) {
			console.log("class.queryInstances(" + max + "):" + JSON.stringify(select));
			...
			if (select && select.$query) {
				wheres.push(this.queryToSelect(select.$query, params));
			...
			return this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				if (instanceMapper) return _this.supervisor.sqlReader(_, cnx, sql, params).limit(max).map(mapper).map(instanceMapper);
				return _this.supervisor.sqlReader(_, cnx, sql, params).limit(max).map(mapper);
			});
		},
