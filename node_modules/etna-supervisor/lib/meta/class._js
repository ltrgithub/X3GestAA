...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		dataReader: function(_, cnx, index, vals, where, raw) {
		...
		reader: function(_, cnx, index, vals, where, raw) {
			return this.dataReader(_, cnx, index, vals, where, raw).map(function(_, data) {
				return this.supervisor.new(_, 'Instance', this, null).fromRecord(_, data);
			}, this);
		},
		read :function(_, key, instance) {
			...
			this.invokeScripts(_, instance, "AREAD_BEFORE","","",param);

			//var record = table.loadRecords(_, index, key, 1)[0];
			var _this = this;
			var record = this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return _this.dataReader(_, cnx, index, key).limit(1).toArray(_);
			});
			/*dbg*///console.log("\nrecord:"+JSON.stringify(record));

			if (!record || !record.length) return instance.supervisor.constants.CST_AERROR;
			instance.fromRecord(_,record[0]);

			return this.invokeScripts(_, instance, "AREAD_AFTER");
		}	
	}),
};