...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			if (this.data.TYPCLA === 2) { // 2 for persistent class
				...
				try {
					...
					if (jsInterface && jsInterface.QUERY) {
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if (!p) {
					...
					coln.nbrPro = coln.class.data.PROPERTIES.length;
					//Calculate count
					if (data.ACTGRP & data.ACTGRP !== "") {
						...
						if (activ.data.DIMFIL > 0 && activ.data.DIME > 0 && activ.data.DIMMAX > 0) {
							coln.count = Math.min(Math.max(activ.data.DIMFIL, activ.data.DIME), activ.data.DIMMAX);
						} else if (activ.data.DIMFIL > 0 && activ.data.DIME > 0) {
							coln.count = Math.max(activ.data.DIMFIL, activ.data.DIME);
						} else if (activ.data.DIMFIL > 0) {
							coln.count = activ.data.DIMFIL;
						}
						...
						coln.class.data.PROPERTIES.forEach_(_, function(_, propData) {
							...
							if (col) {
								...
								if (col.CODACT & col.CODACT !== "") {
									...
									if (activcol.data.DIMFIL > 0 && activcol.data.DIME > 0 && activcol.data.DIMMAX > 0) {
										dime = Math.min(Math.max(activcol.data.DIMFIL, activcol.data.DIME), activcol.data.DIMMAX);
									} else if (activcol.data.DIMFIL > 0 && activcol.data.DIME > 0) {
										dime = Math.max(activcol.data.DIMFIL, activcol.data.DIME);
									} else if (activcol.data.DIMFIL > 0) {
										dime = activcol.data.DIMFIL;
									}
								}
		...
		invokeScripts: function(_, instance, action, $curpro, $curpth, values) {
			var astatus = util.invokeScripts(_, this, instance, action, $curpro, $curpth, values);
			if (this.parent) {
				var pth = ($curpth) ? this.curpath + "." + $curpth : this.curpath;
				astatus += this.parent.invokeScripts(_, instance, action, $curpro, pth, values);
			}
			return astatus;
		},
		...
		sqlReader: function(_, parent, select, max, $recordMapper) {
			...
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						...
						if (parent !== null)
							astatus = _this.invokeScripts(_, instance, "$AREAD_BEFORE", "");
						instance.fromRecord(_, record);
						if (parent !== null)
							astatus = _this.invokeScripts(_, instance, "$AREAD_AFTER", "");
						return instance;
					};
				}();
