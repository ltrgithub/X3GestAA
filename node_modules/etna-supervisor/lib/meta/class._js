	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			this.stdMethods = this.data.STD_METHODS ? this.data.STD_METHODS.reduce_(_, function(_, r, method) {
				if (method.ENAMETSTD === 2 && (!method.ACTMETSTD || activ.getActiv(_, this.supervisor, method.ACTMETSTD) > 0)) {
					switch (method.CODMETSTD) {
						case 'C': // Create
							r.AINSERT = function(_) {
								util.checkInstance(this);
								this.insert(_);
								return 0;
							};
