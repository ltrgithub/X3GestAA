	...
	constructor: baseClass.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			this.properties = [{
				...
				.reduce_(_, function(_, r, propData) {
					if (!((self.group || "") !== ((!propData.LNKCLA && propData.FLDGRP) || ""))) {
						...
						r[name].new = newInstanceProperty(r[name]);
						...
						if (r[name].isReference(_)) {
							...
							name += '_REF';
							r[name] = self.supervisor.new(_, 'Property', self, propData);
							r[name].new = newInstanceReference(r[name]);
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if (!p) {
					// data.CODCOL is a collection :
					self.relations[data.CODCOL] = coln;
					p = self.properties[name] = self.supervisor.new(_, 'Property', self, {
						FLDCLA: name
					});
					p.new = newInstanceCollection(p);
					...
					if (coln.mappingData && coln.mappingData.TYPLNK === 3) {
						...
						tracerJs.debug && tracerJs.debug(self.codclaPath + "." + data.CODCOL + " is a class");
						var name = data.CODCOL;
						p = self.properties[name] = self.supervisor.new(_, 'Property', clas, {
							FLDCLA: name
						});
						p.new = newInstance(p);
					} else {
						// relation = class <-> collection
						tracerJs.debug && tracerJs.debug(self.codclaPath + "." + data.CODCOL + " is a collection");
						p.new = newInstanceCollection(p);
					}
					coln.class = clas;
					p.collection = coln;
		...
		sqlReader: function(_, parent, select, max, $recordMapper) {
			...
			return this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				var reader = self.supervisor.sqlReader(_, cnx, sql, params, sqlOpts).limit(max);
		...
		read: function(_, instance, key) {
			...
			//TODO : instance.properties.AUUID._set(_,tuuid.generate(),true);
