...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		method: function(name) {
			return this.methods[name] || stdMethods[name] || cacheMethods[name];
		},
		...
		sqlReader: function(_, parent, select, max, $recordMapper) {
			...
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						...
						if (parent !== null)
							var astatus = _this.invokeScripts(_, "$AREAD_AFTER", instance, "");
						return instance;
					};
				}();
		...
		fromRecordMapper: function(_, instance, params) {
			if (this.scripts(_).length) {
				return function(_, record) {
					...
					instance.fromRecord(_, record);
					var astatus = this.invokeScripts(_, "AREAD_AFTER", instance, "", "", params);
					// TODO: check astatus
					return instance;
				}.bind(this);
			} else {
