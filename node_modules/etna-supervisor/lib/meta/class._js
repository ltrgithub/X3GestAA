...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			//!console.log("class this.data.TYPCLA:"+this.data.TYPCLA);
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				if ((mapping === null) || mapping.TYPLNK !== 3) { // special treatment reserved to collection where TYPLNK=1x1 :
					var p = this.properties[name];
					if (!p) {
						//dbg:console.log("collection indexed properties:"+name);
						...
						//dbg:console.log("collection is a collection:"+name);
						p.collection = coln;
					...
					//dbg:console.log("collection type 1x1:"+name);

					this.clalnk = this.supervisor.load(_, 'Class', mapping.CLALNK).init(_);
					// add cla properties to this.properties:
					this.clalnk.table.abrev = mapping.ABRLNK;
					this.explnk = mapping.EXPLNK;
					if (this.clalnk.properties) {
						Object.keys(this.clalnk.properties).forEach_(_, function(_, property) {
							this.properties[property] = this.clalnk.properties[property];
						}, this);
					}
				}
		...
		invokeScripts: function(_, instance, action, $curpro, $curpth, values) {
			...
			////!console.log("invokeScripts:"+action);
			var curpro = $curpro || "";
			var curpth = $curpth || "";
			...
			this.scripts.forEach_(_, function(_, script) {
				...
				try {
					...
					try {
						...
						//!console.log("exception in invokeScripts\n" + e.message);
						e.frames && e.frames.forEach(function(frame) {
							//!console.log('\tat ' + frame.name + " (" + frame.file + ')');
						});
					}
		...
		sqlReader: function(_, select, max, $recordMapper, instanceMapper) {
			//!console.log("class.sqlReader(" + max + "):" + JSON.stringify(select));
			...
			if (this.clalnk) {
				var lnkIndex = this.clalnk.table.indexes[0];
				var explnk = this.explnk.split(';');
				tableNames[0] += ' inner join ' + this.clalnk.table.name + ' ' + this.clalnk.table.abrev + ' on ';
				tableNames[0] += Object.keys(lnkIndex.columns).reduce_(_, function(_, r, c) {
					var exp = explnk[c];
					if (exp.indexOf(']') >= 0) {
						r.push(lnkIndex.columns[c].sqlName() + ' = T.' + exp.substring(exp.indexOf(']') + 1) + '_0');
					} else if (exp.indexOf('GACTX.') >= 0) {
						var acxtProperty = exp.substring(exp.indexOf('.') + 1);
						if (_this.supervisor[acxtProperty]) {
							var acxtPropertyValue = _this.supervisor[acxtProperty](_);
							params.push(acxtPropertyValue);
							wheres.push(lnkIndex.columns[c].sqlName() + ' = ' + param(params.length - 1));
						}
					}
					return r;
				}, []).join(',');
			...
			Object.keys(properties).forEach_(_, function(_, name) {
				properties[name].getSql(_, columnNames, tableNames, wheres, params, descs, cache);
			});
		...
		jsReader: function(_, select, max, $recordMapper, instanceMapper) {
			//!console.log("class.jsReader(" + max + "):" + JSON.stringify(select));
			var _this = this;
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						return _this.supervisor.new(_, 'Instance', _this, null).fromRecord(_, record);
		...
		rsReader: function(_, recordset, select) {
			var mapOperators = {
				...
				$regex: function(property, value) {
					var re = new RegExp(value);
					return function(_, record) {
						return re.test(record[property]);
					};
				},
			};
			var logOperators = {
				$or: function(args) {
					return function(_, record) {
						for (var i = 0; i < args.length; i++) {
							if (args[i](_, record)) return true;
						}
						return false;
					};
				},
			...
			var buildFilterExp = function(exp) {
				return logOperators.$and(Object.keys(exp).reduce(function(args, prop) {
					var propExp = exp[prop];
					var hasExp = (typeof propExp === 'object') && Object.keys(propExp).some(function(key) {
						return (key.charAt(0) === '$');
					}) || false;

					var _exp = hasExp ? propExp : {
						$eq: propExp
					};
					Object.keys(_exp).forEach(function(operator) {
						if (mapOperators[operator]) {
							args.push(mapOperators[operator](prop, _exp[operator]));
						}
					});
					return args;
				}, []));
			...
			var buildFilter = function(query) {
				...
				if (operator) {
					var _this = this;
					return logOperators[operator](query[operator].reduce(function(args, exp) {
						args.push(buildFilter(exp));
						return args;
					}, []));
				}
				return buildFilterExp(query);
			};
			...
			var filter;
			if (select.$query || (!select.$query && !select.$orderBy)) {
				filter = buildFilter(select.$query || select);
			}
		...
		fromRecordMapper: function(instance, params) {
			return function(_, record) {
				...
				//dbg:console.log("record:"+JSON.stringify(record));
