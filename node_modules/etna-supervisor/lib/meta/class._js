...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		queryExpToSelect: function(exp, params) {
			var _this = this;
			var map = {
				'$eq': '=',
				'$ne': '!=',
			...
			return Object.keys(exp).reduce(function(r, prop) {
				var column = "T." + _this.properties[prop].column.name + "_0";
		...
		queryToSelect: function(query, params) {
			var operator = (query.$or) ? "or" : (query.$and ? "and" : undefined);
			if (operator) {
				var _this = this;
				return query['$' + operator].reduce(function(r, exp) {
					r.push('(' + _this.queryToSelect(exp, params) + ')');
					return r;
				}, []).join(' ' + operator + ' ');
			}
			return this.queryExpToSelect(query, params);
		},
		queryInstances: function(_, select, max, instanceMapper) {
			return reader(_, select, max, null, instanceMapper);
		},
		reader: function(_, select, max, $recordMapper, instanceMapper) {
			return this.sqlReader(_, select, max, $recordMapper, instanceMapper);
		},
		sqlReader: function(_, select, max, $recordMapper, instanceMapper) {
			...
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						//console.log("map:"+JSON.stringify(record));
			...
			var params = this.supervisor.sqlDriver.param;
			...
			Object.keys(properties).forEach_(_, function(_, name) {
				properties[name].getSql(_, columnNames, tableNames, wheres, params, null);
			});

			if (select && select.$orderBy) {
				orderBy = [];
				Object.keys(select.$orderBy).forEach_(_, function(_, name) {
					var column = "T." + properties[name].column.name + "_0";
					orderBy.push(column + " " + ((select.$orderBy[name] === 1) ? "asc" : "desc"));
				});
			}
			if (select) {
				wheres.push(this.queryToSelect(select.$query || select, params));
			...
			return this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				if (instanceMapper) return _this.supervisor.sqlReader(_, cnx, sql, params).limit(max).map(recordMapper).map(instanceMapper);
				return _this.supervisor.sqlReader(_, cnx, sql, params).limit(max).map(recordMapper);
			});
		},
