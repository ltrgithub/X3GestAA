...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			trace && trace("class this.data.TYPCLA:" + this.data.TYPCLA);
			...
			this.data.COLLECTIONS.forEach_(_, function(_, data) {
				...
				trace && trace("Class collection:" + data.CODCOL);
				...
				if ((mapping === null) || mapping.TYPLNK !== 3) { // special treatment reserved to collection where TYPLNK=1x1 :
					...
					if (!p) {
						trace && trace("collection indexed properties:" + name);
						...
						trace && trace("collection is a collection:" + name);
						p.collection = coln;
					...
					trace && trace("collection type 1x1:" + name);
		...
		invokeScripts: function(_, instance, action, $curpro, $curpth, values) {
			...
			//trace && trace("invokeScripts:"+action);
			var curpro = $curpro || "";
			var curpth = $curpth || "";
			...
			this.scripts.forEach_(_, function(_, script) {
				...
				try {
					...
					try {
						...
						trace && trace("exception in invokeScripts\n" + e.message);
						e.frames && e.frames.forEach(function(frame) {
							trace && trace(('\tat ' + frame.name + " (" + frame.file + ')'));
						});
					}
		...
		checkFacet: function(_, facet) {
			//VVV
			if (this.data.CODACT) {
				if (activ.isActiv(_, this.supervisor, this.data.CODACT) <= 0)
					throw new HttpError(406, this.supervisor.loadMess(_, 140, 31) + this.data.CODCLA + " (" + this.data.CODACT + ")");
			}
			switch (facet) {
				case "$search":
					break;
				...
				case "$summary":
					break;
				case "$edit":
					if (this.data.CODCLA === "AQCPROC10") throw new HttpError(406, this.supervisor.loadMess(_, 149, 32) + this.data.CODCLA);
					break;
			}
		},
		...
		sqlReader: function(_, select, max, $recordMapper, instanceMapper) {
			trace && trace("class.sqlReader(" + max + "):" + JSON.stringify(select));
			var _this = this;
			...
			if (this.data.ACCSTR) {
				trace && trace("add ACCSTR " + this.data.ACCSTR);
		...
		jsReader: function(_, select, max, $recordMapper, instanceMapper) {
			trace && trace("class.jsReader(" + max + "):" + JSON.stringify(select));
			var _this = this;
			var recordMapper = $recordMapper || function() {
					return function(_, record) {
						return _this.supervisor.new(_, 'Instance', _this, null).fromRecord(_, record);
		...
		fromRecordMapper: function(instance, params) {
			return function(_, record) {
				...
				trace && trace("record:" + JSON.stringify(record));
