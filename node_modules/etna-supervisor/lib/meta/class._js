				...
				r[method.CODMET] = self.data.PARAM_DEFS ? self.data.PARAM_DEFS
					...
					.reduce_(_, function(_, r, param) {
						...
						if (param.MODPAR === 1) { ///1:variable arg 2:value arg 3:constant
							r[param.CODPAR].set = function(param) {
								return function(_, args, value) {
									var arg = args[param.NOPAR - 1];
		...
		*/
		addMethods: function(instance) {
			...
			Object.keys(this.methods).forEach(function(method) {
				...
				instance.methods["xmet" + method] = function(clas, method) {
					return function(_) {
						var args = Array.prototype.slice.call(arguments).slice(1)[0];

						var meth = self.methods[method];
						var xmetName = "xmet" + method;
						if (instance[xmetName]) {
							return instance[xmetName](_, args);
						}
						...
						Object.keys(meth).forEach_(_, function(_, arg) {
							meth[arg].set && meth[arg].set(_, args, methArgs[arg].value);
						});
						return status;
					};
				}(self, method);
