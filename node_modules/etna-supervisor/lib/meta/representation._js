...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
						...
						if (match) {
							...
							for (var i = 0; i < match.length; i++) {
								...
								switch (name.charAt(0)) {
									case '+':
										name = name.substring(1) + "%20asc";
										break;
									case '-':
										name = name.substring(1) + "%20desc";
										break;
								}
								this._index[i] = {
									name: name
		...
		filter: function(name) {
			if (this.filter.filters === undefined || this.filter.filters[name] === undefined) {
				var filter;
				var option;
				var defaut;
				this.filter.filters = {};
				...
				if (option) {
					console.log("filter:" + option.OPTCND);
					this.filter.filters[name] = Jmql.x3ToJmql(option.OPTCND);
				}
			}
			return this.filter.filters[name] && this.filter.filters[name]();
		},
	}),
	FACETS: FACETS,
};