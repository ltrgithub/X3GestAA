...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		init: function(_) {
			...
			this.properties = this.data.PROPERTIES.reduce_(_, function(_, r, prop) {
			...
			this.exportedProperties.filter(function(expprop) {
				...
				if (this.properties[path[0]]) {
					...
					if (path[0] === this.data.ABRCLA) {
						...
						var cm = util.find(this.data.COLLECTIONS, "ALIASCOL", path[0]);
						if (cm) {
							console.log("Representation collection:" + path[0]);
							this.collections = this.collections || {};
							this.collections[path[0]] = this.supervisor.new(_, 'Collection', this, cm);
							// TODO :
						}
					}
		...
		service: function(_, keys, service) {
			...
			var status = this.class.invokeScripts(_, "$" + (pos > 0 ? service.substring(0, pos) : service));
			return reply;
		}
	}),
	FACETS: FACETS,
