...
module.exports = {
	...
	constructor: baseClass.defineClass(function(superv, data) {
		...
		x3Action: function(_, action) {
			var astatus = this._x3Action(_, action);
			if (astatus !== this.supervisor.constants.CST_AERROR) {
				astatus = this.class._x3Action(_, action);
			}
			return astatus;
		},
		AQUERY_OPEN: function(_) {
			return this.x3Action(_, "AQUERY_OPEN");
		},
		AQUERY_CLOSE: function(_) {
			return this.x3Action(_, "AQUERY_CLOSE");
		},
		service: function(_, keys, $service) {
			...
			var service = pos > 0 ? $service.substring(0, pos) : $service;
			tracerJs.debug && tracerJs.debug("$service:" + service);
			this.diagnoses = [];
			var status = this.x3Action(_, service);
			if (status) {
				reply.$actions[service].$diagnoses = this.diagnoses;
			}
			return reply;
		}
	}),
