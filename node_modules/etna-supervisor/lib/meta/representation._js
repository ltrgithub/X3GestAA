...
module.exports = {
	collectionName: 'ASHW',
	keyName: 'CODREP',
	constructor: base.defineClass(function(superv, data) {
		this.curpath = "";
	}, null, {
		init: function(_) {
			...
			this.properties = this.data.PROPERTIES
				...
				.reduce_(_, function(_, r, prop) {
					r[prop.CODFLD] = this.supervisor.new(_, 'Property', this, prop);
					return r;
				}, {}, this);

			this.collections = this.data.COLLECTIONS.reduce_(_, function(_, r, col) {
		...
		prepareLinks: function(_) {
			...
			self.data.LINKS.filter(function(link) {
				...
				return link.ENALNK === 2 && linksRemRef[[link.AFFLNK, link.ANCLNK, link.CODLNK].join("/")] === undefined;
				...
				if (link.TYPLNK === 1) {
					...
					if (!repr || activ.getActiv(_, repr.supervisor, repr.data.CODACT) <= 0) return;
					if (repr.class) {
						if (activ.getActiv(_, repr.supervisor, repr.class.data.CODACT) <= 0) return;

						if (enumStdLinks[link.CMPLNK]) {
							if (enumStdLinks[link.CMPLNK].toFacet && !repr.isFacetEnabled(enumStdLinks[link.CMPLNK].toFacet)) return;
							var behaviors = enumStdLinks[link.CMPLNK].requiredBehaviors;
							for (var i = 0; behaviors && i < behaviors.length; i++) {
								if (!repr.isBehaviourEnabled(behaviors[i])) return;
							};
						}
					}
				}
				...
				var codLnk = [link.AFFLNK, link.ANCLNK, link.CODLNK].join("/");
				linksOk[codLnk] = link;
				if (link.AFFLNK === 5 && (link.DETLNK === 2 || link.EDILNK === 2 || link.SUMLNK === 2)) {
					link = util.clone(linksOk[codLnk]);
					linksOk[codLnk].DETLNK = 1;
					linksOk[codLnk].EDILNK = 1;
					linksOk[codLnk].SUMLNK = 1;
					link.AFFLNK = 4;
					link.QRYLNK = 1;
					link.LOKLNK = 1;
					...
					linksOk[[link.AFFLNK, link.ANCLNK, link.CODLNK].join("/")] = link;
				}
			});
			self.data.LINKS = Object.keys(linksOk).reduce(function(r, lk1) {
				r.push(linksOk[lk1]);
				return r;
			}, []);
		...
		invokeScripts: function(_, action, instance, $curpro, $curpth, values) {
			return util.invokeScripts(_, this, action, instance, $curpro, $curpth, values);
		},
		resourceFactory: function(_, facet) {
			...
			this.exportedProperties.filter(function(expprop) {
				...
				if (this.collections && this.collections[path[0]]) {
					if (!(prop = factory.properties[path[0]])) {
						...
						prop.targetResourceFactory = this.supervisor.new(_,
							...
							facet,
							path[0],
							this.collections[path[0]].class);
						factory.properties[path[0]] = prop;
					}
					f = prop.targetResourceFactory;
					prop = this.collections[path[0]].class.properties[path[1]];
					prop = prop.clone(prop);
					prop.resourceFactory = factory;
				...
				if (prop.isExtra === true && prop.isReference(_)) {
					...
					prop2.resourceFactory = factory;
					prop2.isExtraRef = true;
					prop2.isReference(_, true);
					prop2.getSql = undefined;
					f.properties[path[i] + '_REF'] = prop2;
					prop.getReferenceKey = function(_, $this, superv) {
						var self = $this;
						var name = self.name;
						var refKeys = self.class.data.PARAMS.filter(function(paramData) {
							return paramData.TYPPAR === 1 && paramData.CODFLD === name && paramData.TYPKEY === 2;
						}, prop).reduce_(_, function(_, r, paramData) {
					...
					}(_, prop, prop.supervisor);

					f.cache = f.cache || {};
					f.cache.references = f.cache.references || {};
					f.cache.references[prop.name] = {
						$name: prop.name,
						$ids: {},
						...
						}(_, prop.name, prop.getReferenceKey)
					};
				}
			}, this);
		...
		addSysProperties: function(prop) {
			...
			return !prop ? properties : Object.keys(prop).reduce(function(r, name) {
				r[name] = r[name] || prop[name];
				return r;
			}, properties);
		},
		...
		service: function(_, keys, service) {
			...
			var pos = service.lastIndexOf('_');
			var instance = this.supervisor.new(_, 'Instance', this, this.addSysProperties());
			// Add instance in order to create this for operations :
			var status = this.class.invokeScripts(_, "$" + (pos > 0 ? service.substring(0, pos) : service), instance);
