	...
	constructor: util.defineClass(function(superv, data) {
		...
		resourceFactory: function(_, facet) {
			...
			this.data.LINKS.filter(function(link) {
				return link.ENALNK === 2 && link[facetFlag] === 2;
		...
		filters: function(_, resourceFactory) {
			...
			var tab = list || {};
			tab["GACTX.USER"] = '"' + context.builtinProperties.USER().getValue(_) + '"';
			tab["[V]CST_ANO"] = "false";
			tab["[V]CST_AYES"] = "true";
						...
						return {
							...
							$layout: {
								$items: this.data.BLOCKS.filter(function(block) {
									...
									function FindCol(targetResourceFactory, facet) {
										...
										Object.keys(targetResourceFactory.properties).forEach(function(prop) {
											if (targetResourceFactory.properties[prop].exportedProperty) {
												CondCol = CondCol || FindProp(targetResourceFactory.properties[prop], facet);
											} else if (targetResourceFactory.properties[prop].targetResourceFactory) {
												CondCol = CondCol || FindCol(targetResourceFactory.properties[prop].targetResourceFactory, facet);
											}
									...
									var items = Object.keys(resourceFactory.properties).filter(function(prop) {
										//layout for $edit, $details and $summary only
										var condBind = false;
										if (resourceFactory.properties[prop].exportedProperty) {
											condBind = FindProp(resourceFactory.properties[prop], facet);
										} else if (resourceFactory.properties[prop].targetResourceFactory) {
											condBind = FindCol(resourceFactory.properties[prop].targetResourceFactory, facet);
										}
										return (condBind === true);
									}).reduce(function(r, prop) {
										r[resourceFactory.properties[prop].tag] = true;
										return r;
									}, {}, facet);
		...
		proto: function(_, facet) {
			...
			if (/^\$(details|edit|summary)$/.test(facet)) proto.$article = this.article(_, facet);
			else proto.$article = {};
			proto.$article.$menus = {
				$layout: {
					$items: []
