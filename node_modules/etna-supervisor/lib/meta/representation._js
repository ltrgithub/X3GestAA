	...
	constructor: util.defineClass(function(superv, data) {
		...
		prepareLinks: function(_) {
			...
			var LINKS_REM_REF = self.data.LINKS.filter(function(link) {
				...
				if (tablnk && tablnk[0]) r[lk1.AFFLNK + "/" + lk1.ANCLNK + "/" + lk1.REMSTDLNK] = tablnk[0];
				return r;
			}, {});
			var LINKS_OK = {};
		...
		service: function(_, keys, service) {
			...
			service = service.substring(0, service.lastIndexOf('_'));
			console.log("invokeScripts $" + service);
			var status = this.class.invokeScripts(_, "$" + service);
			return reply;
		}
	}),
	FACETS: FACETS,
