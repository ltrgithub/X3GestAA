...
module.exports = {
	...
	constructor: baseClass.defineClass(function(superv, data) {
		...
		$x3Methods: function(_, action) {
			var astatus = this._$x3Methods(_, action);
			if (astatus !== this.supervisor.constants.CST_AERROR) {
				astatus = this.class._$x3Methods(_, action);
			}
			return astatus;
		},
		$AQUERY_OPEN: function(_) {
			return this.$x3Methods(_, "AQUERY_OPEN");
		},
		$AQUERY_CLOSE: function(_) {
			return this.$x3Methods(_, "AQUERY_CLOSE");
		},
		service: function(_, keys, $service) {
			...
			this.diagnoses = [];
			var status = this.$x3Methods(_, service);
			if (status) {
