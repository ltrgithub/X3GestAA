...
module.exports = {
	...
	constructor: base.defineClass(function(superv, data) {
		...
		deleteResource: function(_, key, qs) {
			var reply = {
				$diagnoses: []
			};

			try {
				tracerJs.debug && tracerJs.debug("deleteResource " + key + " qs:" + JSON.stringify(qs));
				var tableLinks = this.class.table.links(_);

				// Control blocking links :
				tableLinks.control(_, key);
				...
				tracerJs.error && tracerJs.error(e);
				reply.$diagnoses.push(this.supervisor.new(_, 'Diagnosis', "error", null, e.message).dataNode());
			}
			return reply;
		},
