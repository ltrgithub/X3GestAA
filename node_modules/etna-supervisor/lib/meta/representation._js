...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		resourceFactory: function(_, facet) {
			...
			if (!this._preparedLinks) this.prepareLinks(_);
			...
			function prepareKeyparams(repr, link) {
				//prepare keys+params
				if (repr.data.PARAMS) {
					var keyParams = repr.data.PARAMS.filter(function(paramData) {
						return paramData.TYPPAR === 3 && paramData.TYPKEY === 2 && paramData.CODLNK === link.data.oCODLNK && paramData.AFFLNK === link.data.oAFFLNK && paramData.CODFLD === link.data.ANCLNK;
						...
						if (propData) paramData.alias = propData.ALIAS;
							...
							paramData.value = fvalkey(paramData.VALEUR, paramData.TYPINT);
						}
						r.push(paramData);
						return r;
					}, []);
					link.keys = keyParams;
					var params = repr.data.PARAMS.filter(function(paramData) {
						return paramData.TYPPAR === 3 && paramData.TYPKEY === 1 && paramData.CODLNK === link.data.oCODLNK && paramData.AFFLNK === link.data.oAFFLNK && paramData.CODFLD === link.data.ANCLNK;
					}).reduce(function(r, paramData) {
						if (/^\=num.*$/.test(paramData.VALEUR)) paramData.VALEUR = paramData.VALEUR.substring(6, paramData.VALEUR.length - 1);
						else if (/^\=val.*$/.test(paramData.VALEUR)) paramData.VALEUR = paramData.VALEUR.substring(5, paramData.VALEUR.length - 1);
						var propData = util.find(repr.data.EXPORTED_PROPERTIES, 'CODPRO', paramData.VALEUR);
						if (propData) paramData.alias = propData.ALIAS;
							...
							paramData.value = fvalpar(paramData.VALEUR, paramData.TYPINT);
						}
						r.push(paramData);
						return r;
					}, []);
			...
			this.data.LINKS.filter(function(link) {
				...
				//prepare links for actions save and abort
		...
		filters: function(_, resourceFactory) {
								...
								$items: this.data.BLOCKS.filter(function(block) {
									...
									function FindProp(prop, facet) {
										...
										if (prop.exportedProperty.data.BLCPRO === block.CODBLC) {
											switch (facet) {
												case "$details":
													condProp = (prop.exportedProperty.data.DETPRO === 2 && prop.exportedProperty.data.STADETPRO !== 3);
													break;
												case "$edit":
													condProp = (prop.exportedProperty.data.EDIPRO === 2 && prop.exportedProperty.data.STAEDIPRO !== 3);
													break;
												case "$summary":
													condProp = (prop.exportedProperty.data.SUMPRO === 2 && prop.exportedProperty.data.STASUMPRO !== 3);
													break;
											}
											return {
												cond: condProp,
		...
		article: function(_, facet, f) {
			...
			//Menu
			function fillMenu(tabLinks, curMenu, items) {
				var hasElt = false;
				if (tabLinks) {
			...
			function fillMenuMen(_, self, tabMenus, curMenu, links1, links2, items) {
				var hasElt = false;
				var selfItems = items;
				if (tabMenus) {
					tabMenus.filter_(_, function(_, menu) {
						...
						hasBind1 = fillMenu(links1, menu.CODMENU, items);
						hasBind2 = fillMenu(links2, menu.CODMENU, items);
						...
						hasBind = fillMenuMen(_, self, tabMenus, menu.CODMENU, links1, links2, items);
			...
			if (facet !== '$bulk') {
				...
				if (facet !== '$query') fillMenu(f.links, "", items);
				fillMenu(f.page && f.page.links, "", items);
				items = items.sort(function(item1, item2) {
					return item1.$order - item2.$order;
				});
				items.forEach(function(item) {
				...
				fillMenuMen(_, this, this.data.MENUS, "", (facet !== '$query' ? f.links : undefined), (f.page && f.page.links), items);
			}
		...
		proto: function(_, facet) {
			...
			if (/^\$(query|bulk|lookup)$/.test(facet)) proto.$filters = this.filters(_, f);
