			...
			this.properties = this.data.PROPERTIES
				...
				.reduce_(_, function(_, r, prop) {
					if (prop.NUMFLD && typeof prop.NUMFLD === 'string') {
						prop.NUMFLD = (!isNaN(prop.NUMFLD) && bcd.toDouble(bcd.fromString(prop.NUMFLD)) > 0) ? bcd.toDouble(bcd.fromString(prop.NUMFLD)) : undefined;
					}
					if (prop.LONG && typeof prop.LONG === 'string') {
						prop.LONG = (!isNaN(prop.LONG) && bcd.toDouble(bcd.fromString(prop.LONG)) > 0) ? prop.LONG = bcd.toDouble(bcd.fromString(prop.LONG)) : undefined;
					}
