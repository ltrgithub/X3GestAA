...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	incrementUpdtick: function() {
		this.$updtick = (this.$updtick === 999999999) ? 1 : this.$updtick + 1;
	},
		...
		try {
			...
			if (key === undefined) {
				...
				if (status === constants.CST_AERROR) {
					throw new Error("abort"); // to handle abort actions carefully
				}
			}
	...
	insert: function(_, options) {
		...
		var AINSERT = function(_, instance) {
			...
			if (status === constants.CST_AOK) instance.$updtick = 1;
			return status;
		};
	...
	update: function(_, options) {
		...
		var AUPDATE = function(_, instance) {
			...
			if (status === constants.CST_AOK) instance.incrementUpdtick();
			return status;
		};
