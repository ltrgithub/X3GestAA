...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	crud: function(_, fname, f, key) {
		...
		try {
			...
			if (key) this.key = key;
			status = this.action(_, fname + "_BEFORE", this.key);
			...
			if (key === undefined) {
				...
				if (status === instance.supervisor.constants.CST_AERROR) {
					return Math.max(status, this.action(_, fname + "_ABORT"));
				}
			}
			this.transaction && this.transaction.commit(_);

			return this.action(_, fname + "_AFTER", this.key);
		} catch (e) {
			console.error(fname + "_ABORT exception:" + e.stack);
			this.transaction && this.transaction.rollback(_);
			var status = this.addDiagnose(_, constants.CST_AERROR, e.message);
			return Math.max(status, this.action(_, fname + "_ABORT", this.key));
		}
	},
	read: function(_, key) {
