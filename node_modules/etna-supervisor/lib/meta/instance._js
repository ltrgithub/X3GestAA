...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	afterCreate: function(_) { // for $create action, not $edit
		...
		this.class.mandatoryProperties && this.class.mandatoryProperties.forEach_(_, function(_, property) {
			self.slot(_, property).$isMandatory = true;
		});
		return this;
	},
	clone: function() {
	...
	crud: function(_, f) {
		...
		try {
			...
			return Math.max(status, this.action(_, "A" + f.name + "_ABORT", this.key));
		}
	},
	read: function(_, key) {
		this.key = key;
	...
	controlBeforeInsert: tmp = function(_) {
		try {
			...
			Object.keys(this.slots).forEach_(_, function(_, property) {
				var slot = self.slot(_, property);

				// Control mandatory fields
				if (slot.$isMandatory && slot.isUndefined) {
					//  Mandatory property :
					status = Math.max(status,
						self.setStatus(_, property, cst_aerror, self.supervisor.loadMess(_, 123, 10)));
				}
				// Control the format
				if (slot.isFormatInvalid && slot.isFormatInvalid(_)) {
					// invalid format
					status = Math.max(status,
						self.setStatus(_, property, cst_aerror, self.supervisor.loadMess(_, 100, 149)));
				}
				// Other controls
				if (slot.areControlsInvalid && slot.areControlsInvalid(_)) {
					// Other controls invalid
					status = Math.max(status,
						self.setStatus(_, property, cst_aerror, self.supervisor.loadMess(_, 123, 83)));
				}
			});
			...
			status = this.propertiesEvent(_, "CONTROL");
			if (status === cst_aerror) return status;
