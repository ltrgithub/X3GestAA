...
var Instance = exports.constructor = util.defineClass(function(superv, clas, parent, properties) {
	...
	afterCreate: function(_) { // for $create action, not $edit
		...
		Object.keys(properties).forEach_(_, function(_, name){
			this.properties[name] = properties[name].createInstance(this);
		}, this);
	...
	updateProperty : function(from,to) {
		var _this = this;
		Object.keys(_this.properties).some(function(property) {
		    if(_this.properties[property] == from) {
		    	_this.properties[property] = to;
		    	return true;	
		    }
		    return false;
		});
	...
	revertTo : function() {
		...
		Object.keys(from.properties)
			...
			var fromChild = from.properties[property];
	...
	fromRecord : function(_,record,colnIndex) {
		...
		Object.keys(this.properties).forEach_(_,function(_,p) {
			if(this.properties[p] && this.properties[p].fromRecord) this.properties[p].fromRecord(_,record,colnIndex);
		}, this);
		return this;
	},
	read: function(_, key) {
		return this.class.read(_,key,this);
	},
	toRecord : function() {
		var _this = this;
		return Object.keys(this.properties).reduce(function(record, name) {
	...
	setColumnAttribute:function(_,collection,column,attribut,value) {
		var p = this.properties[collection];
		if(p && p.setColumnAttribute ) return p.setColumnAttribute(column,attribut,value);
	},
	getColumnAttribute:function(_,collection,column,attribut) {
		var p = this.properties[collection];
		if(p && p.getColAttribute) return p.getColAttribute(column,attribut);
	},
