...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
		...
		try {
			...
			if (key === undefined) {
				...
				Object.keys(this.class.relations).forEach_(_, function(_, name) {
					console.error("AAAAAAAAA", name, "FCT", fct, " --> ", instance.get(_, name)[fct]);
	...
	read: function(_, key) {
		return this.crud(_, 'AREAD', function AREAD(_, instance) {
			return instance.class.read(_, instance, instance.key);
		}, key);
	},
	insert: function(_) {
		this.getProKeys(_);
		return this.crud(_, 'AINSERT', function AINSERT(_, instance) {
	...
	update: function(_) {
		this.getProKeys(_);
		return this.crud(_, 'AUPDATE', function AUPDATE(_, instance) {
			var actx = glob.context.x3session.actx;
	...
	delete: function(_) {
		...
		return this.crud(_, 'ADELETE', function ADELETE(_, instance) {
