...
var Instance = exports.constructor = util.defineClass(function(superv, clas, data, colnIndex) {
	...
	get: function(_, name) {
		...
		if (val && (typeof val === "Object") && this.$isSnapshot)
		{
			if (!val.snapshots) val.snapshots = [null, null] ;
			if (this.$isClaSnaphot) {
				val = val.snapshots[0] = val.clone();
				val.$isClaSnapshot = true;
			}
			if (this.$isSysSnaphot) {
				val = val.snapshots[1] = val.clone();
				val.$isSysSnapshot =true;
			}
		}
	...
	set: function(_, name, value, raw) {
		...
		if (this.$isSnapshot) throw new X3Error(155, "") ;

		if (this.$snapshotEnabled) {
			if (!this.snapshots) this.snapshots = [null, null] ;
			if (this.snapshots[0] === null) {
				this.snapshots[0] = this.clone();
				this.snapshots[0].$isClaSnapshot = true;
			}

		}		
		if (this.$sysSnapshotEnabled) {
			if (!this.snapshots) this.snapshots = [null, null] ;	
			if (this.snapshots[1] === null) {
				this.snapshots[1] = this.clone();
				this.snapshots[1].$isSysSnapshot = true;
			}
		}
	...
	getSnapshot : function() {
		if (!this.$snapshotEnabled) return null ;
		else {
			if (!this.snapshots) this.snapshots = [null, null] ;
			
			if (this.snapshots[0]) 	return this.snapshots[0] ;
	...
	setsnapshotEnabled: function(value) {
		var snapshotEnabled = !!value;
		if(!snapshotEnabled) {
