...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
		...
		if (this.parent && dataChildren && dataChildren.ownMapping) {
			...
			dataChildren.ownMapping.MAPS
				...
				.forEach_(_, function(_, map) {
					instance.setProperty(_, map.KEYMAP, instance.parent.getProperty(_, map.PROMAP), true);
				});
		}
	},
	getKey: function(_) {
		...
		return this.class.data.KEYINT ? this.class.data.KEYINT.match(/(\w+)/g).map_(_, function(_, p) {
			return self.getProperty(_, p, true);
		}).join('~') : undefined;
	},
		...
		try {
			...
			if (key === undefined) {
				...
				Object.keys(this.class.relations).forEach_(_, function(_, name) {
					status = Math.max(status, instance.getProperty(_, name)[fct](_));
				});
	...
	insert: function(_) {
		...
		return this.crud(_, function AINSERT(_, instance) {
			...
			instance.setProperty(_, "AUUID", tuuid.generate(), true);
			instance.setProperty(_, "CREUSR", actx.$USER(_));

			var expnum = instance.slot(_, "EXPNUM");
			if (expnum) expnum.set(_, 8); // TODO EXPNUM

			instance.setProperty(_, "UPDUSR", "");
			if (instance.getProperty(_, "CREDATTIM").x3IsZero()) instance.setProperty(_, "CREDATTIM", tdatetime.now());
			instance.setProperty(_, "UPDDATTIM", new tdatetime.Datetime());
	...
	update: function(_) {
		...
		return this.crud(_, function AUPDATE(_, instance) {
			var actx = glob.context.x3session.actx;
			instance.setProperty(_, "UPDUSR", actx.$USER(_));

			var expnum = instance.slot(_, "EXPNUM");
			if (expnum) expnum.set(_, 8); // TODO EXPNUM

			instance.setProperty(_, "UPDDATTIM", tdatetime.now());
	...
	propagateControl: function(_, type, operation) {
		...
		var status = constants.CST_AOK;
		Object.keys(this.class.relations).forEach_(_, function(_, name) {
			status = Math.max(status, instance.getProperty(_, name).onControl(_, type, operation));
		});
		return status;
	},
	...
	propagateControlExecution: function(_, type) {
		...
		var instance = this;
		Object.keys(this.class.relations).forEach_(_, function(_, name) {
			instance.getProperty(_, name).executeControl(_, type);
		});
		return status;
	},
...
exports.newInstance = function(property) {
	return function(_, parent) {
		var instance = new this.supervisor.new(_, 'Instance', property.class, parent).afterCreate(_);
