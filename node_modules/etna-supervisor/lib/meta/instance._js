...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	read: function(_, key) {
		var status;
		status = this.actionRead(_, "AREAD_BEFORE", key);
		if (status === this.supervisor.constants.CST_AERROR) return status;
		status = this.class.read(_, this, key);
		if (status === this.supervisor.constants.CST_AERROR) return status;
		return this.actionRead(_, "AREAD_AFTER", key);
	},
	toRecord: function() {
		var _this = this;
	...
	actionRead: function(_, action, key) {
		tracerJs.debug && tracerJs.debug("instance.actionRead " + action + " key:" + key);
		...
		var dargs = delayedArgs(_, this, this.getActionArgs(_, action, key));
	...
	controlBefore: function(_, operation) {
		...
		if (!status) {
			...
			Object.keys(this.class.collections).forEach_(_, function(_, collection) {
				...
				// if(self.class.collections[collection].data.FLGA === 2)
				// 	console.log("controlBefore on collection " + collection);
			});
		}
		return status;
	},
