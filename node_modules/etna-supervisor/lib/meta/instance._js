...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
		...
		try {
			...
			if (key === undefined) {
				...
				if (status === instance.supervisor.constants.CST_AERROR) {
					console.log(f.name + "_ABORT status:" + status);
			...
			console.log(f.name + "_ABORT exception:" + e.message);
			this.transaction && this.transaction.rollback(_);
			var status = this.addDiagnose(_, constants.CST_AERROR, e.message);
	...
	action: function(_, action, key, crupro, curpth) {
		...
		if (status === constants.CST_AERROR || !this.parent || !this.parent.resourceFactory) return status;
	...
	executeControl: function(_, type) {
		...
		// 1.Control the instance
		return (this[control] && this[control](_, type)) || constants.CST_AOK;
	},
	...
	executeControlMandatory: function(_, type) {
		...
		Object.keys(this.slots).forEach_(_, function(_, property) {
			...
			slot.deleteDiagnoses && slot.deleteDiagnoses();
			if (slot.$isMandatory) {
		...
		return (status === constants.CST_AERROR) ? status : this.executeControlChildren(_, type);
	},
