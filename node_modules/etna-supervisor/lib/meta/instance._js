...
var stateFlag = {
	snapshotEnabled:    1,
	sysSnapshotEnabled: 2,
	setAccessorEnabled: 4,
	getAccessorEnabled: 8,
	isClaSnapshot:16,
	isSysSnapshot:32,
	isSnapshot:48,
	isReadonly:64
...
var Instance = exports.constructor = util.defineClass(function(superv, clas, data, colnIndex) {
	...
	afterCreate: function(_) { // for $create action, not $edit
		...
		Object.keys(properties).forEach_(_, function(_, name){
			...
			if(prop.collection){
				this.children[name] = new InstanceCollection(prop.collection,this);
			}
			else
	...
	get: function(_, name) {
		...
		if (!prop) {
			...
			return (typeof valattr === 'boolean')? (valattr ? variables.constants.CST_ATRUE : variables.constants.CST_AFALSE): val;;
		}
		if (prop.collection) {
			if(!this.children[name].isLoaded) this.children[name].load(_,this.loadChildren(_, prop.collection)); 
		...
		if (val && (typeof val === "Object") && this.getStateFlag("isSnaphot"))
		{
			if (!val.snapshots) val.snapshots = [null, null] ;
			if (this.getStateFlag("isClaSnaphot")) val = val.snapshots[0] = val.clone( ).setStateFlag("isClaSnapshot") ;
			if (this.getStateFlag("isSysSnaphot")) val = val.snapshots[1] = val.clone( ).setStateFlag("isSysSnapshot") ;
		}
	...
	set: function(_, name, value, raw) {
		...
		if (this.getStateFlag("isSnapshot")) throw new X3Error(155, "") ;

		if (this.getStateFlag("snapshotEnabled")) {
			if (!this.snapshots) this.snapshots = [null, null] ;
			if (this.snapshots[0] === null) this.snapshots[0] = this.clone( ).setStateFlag("isClaSnapshot") ;

		}		
		if (this.getStateFlag("sysSnapshotEnabled")) {
			if (!this.snapshots) this.snapshots = [null, null] ;	
			if (this.snapshots[1] === null) this.snpshots[1] = this.clone( ).setStateFlag("isSysSnapshot") ;
		}
	...
	setStateFlag : function( flag ) {
		if (stateFlag[flag] === undefined) throw new Error( "Set unkwnon instance flag "+flag) ;
		if (this._state === undefined) this._state = stateFlag[flag];
		else this._state |= stateFlag[flag] ;

		// if flag != "isSnaphot" do it in sublcass (flag &= ~stateFlag["isSnaphot"])
		return this ;
	},
	unsetStateFlag : function( flag ) {
		if (stateFlag[flag] === undefined) throw new Error("Unset unkwnon instance flag "+flag) ;
		if (this._state === undefined) this._state = 0 ;
		else this._state &= ~stateFlag[flag] ;

		// if flag != "isSnaphot" do it in sublcass
		return this ;
	},	
	getStateFlag : function( flag ) {
		if (stateFlag[flag] === undefined) throw new Error("Get unkwnon instance flag "+flag) ;
		if (this._state === undefined) return false
		else return (this._state & stateFlag[flag])? true : false ;
	},
	displayFlag : function( ) {
		var _This = this ;
		console.log( "------Instance flag------") ;
		Object.keys(stateFlag).forEach( function( key ) { 
			console.log( key+":"+_This.getStateFlag(key)) ;
		}) ;
		console.log( "-------------------------") ;
	},
	dummyclone : function( ) {
		// this is a dummy clone, just to make it work
		var _this = this ;
		return Object.keys(this.data).reduce(function(r, name) {
			var v = _this.data[name] ;
			r[name] = v;
			return r;
		}, {});	
	...
	clone: function() {
		var instance = new Instance(this.supervisor, this.class, this.dummyclone(this.data, true), this.colnIndex);
	...
	read: function(_, key) {
		...
		this.data.AUUID = tuuid.generate() ;
	...
	setPropertyAttribute:function(_,property,attribut,value) {
		if(this.data[property]) this.data[property].setAttribute(attribut,value);
		else if(this.children[property]) this.children[property].setAttribute(attribut,value);
	},
	...
	setColumnAttribute:function(_,collection,column,attribut,value) {
		if(this.children[collection]) this.children[collection].setColAttribute(column,attribut,value);
	},
