...
exports.constructor = util.defineClass(function(superv, clas, group) {
	...
	init: function(_) {
		...
		this.class.data.PROPERTIES.forEach_(_, function(_, propData) {
			...
			this.properties[name] = this.superv.new(_, 'Property', this, propData);
		}, this);
		if (!this.group) this.class.data.COLLECTIONS.forEach_(_, function(_, colnData) {
			...
			if (!p) {
				p = this.properties[name] = this.superv.new(_, 'Property', this, name);
				p.collection = coln;
				coln.instanceFactory = this.superv.new(_, 'InstanceFactory', this.class, name);
			} else {
				p.collection = coln;
				...
				var cla = this.superv.load(_, 'Class', targetName);
				if (!cla) throw new Error("target class missing for collection " + name);
				coln.instanceFactory = this.superv.new(_, 'InstanceFactory', cla);
			}
	...
	createInstance: function(_, qs) {
		...
		var data = {
			...
			UPDDATTIM: now,
			CREDATTIM: now,
			ACTX: this.superv.currentContext(_),
			AERROR: util.emptyArray(),
		}
		return this.superv.new(_, 'Instance', this, data).afterCreate(_);
	},
	...
	loadInstances: function(_, index, vals, max) {
		...
		return table.loadRecords(_, index, vals, max).map_(_, function(_, data) {
			return this.superv.new(_, 'Instance', this, data);
		}, this);
	},
