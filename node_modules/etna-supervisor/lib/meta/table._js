...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		insert: function(_, instance) {
			...
			if (instance.class.collection && !! instance.class.collection.mappingData) {
				tracerDbms.debug && tracerDbms.debug("skip insert for class " + instance.class.name);
				return constants.CST_AOK;
			}
			this.computeColumnNames(_);
			var values = this.sql.columnDescs.reduce_(_, function(_, r, desc) {
				r.push(instance.getField(_, desc));
				return r;
			}, [], this);
		...
		update: function(_, instance) {
			...
			if (instance.class.collection && !! instance.class.collection.mappingData) {
				tracerDbms.debug && tracerDbms.debug("skip update for class " + instance.class.name);
				return constants.CST_AOK;
			}
			...
			this.sql.columnDescs.forEach_(_, function(_, desc) {
				...
				dest.push(instance.getField(_, desc));
			});
			// + updtick
