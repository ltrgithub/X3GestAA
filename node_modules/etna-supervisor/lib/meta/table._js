...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		if (data.COLUMNS.LONG && typeof data.COLUMNS.LONG === 'string') {
			if (!isNaN(data.COLUMNS.LONG) && bcd.toDouble(bcd.fromString(data.COLUMNS.LONG)) > 0) data.COLUMNS.LONG = bcd.toDouble(bcd.fromString(data.COLUMNS.LONG));
			else delete data.COLUMNS.LONG;
		}
		this.data = data;
		this.sql = {};
		this.abrev = 'T';
		...
		computeColumnNames: function(_) {
			if (!this.sql.columnNames) {
				...
				Object.keys(this.columns).forEach_(_, function(_, name) {
					...
					column.getSql(_, names, null, null, null, null, descs, true);
				}, this);
		...
		insert: function(_, data) {
			...
			var values = this.sql.columnDescs.map(function(name, i) {
				...
				var v = desc.index == null ? data[desc.name].value : data[desc.name][desc.index].value;
