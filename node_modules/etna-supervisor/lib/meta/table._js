...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		reader: function(_, cnx, index, vals, where, raw, options) {
			...
			return this.supervisor.sqlReader(_, cnx, sql, params, sqlOpts);
		},
		...
		insert: function(_, instance) {
			...
			var values = this.sql.columnDescs.reduce_(_, function(_, r, desc) {
				var slot = instance.slot(_, desc.name);
				var value;
				if (slot) {
					value = slot._get(_, true);
					if (slot.collection) {
						value = value.get(_, desc.name, true);
					}
				} else {
					// Set a default value :
					value = desc.type.defaultValue;
				}
				r.push(value);
				return r;
			}, [], this);
			...
			try {
				tracerJs.debug && tracerJs.debug(sql);
				return this.supervisor.executeSql(_, sql, values);
			} catch (e) {
				console.error("Exception in table.insert:" + e.stack);
		...
		update: function(_, instance) {
			...
			this.sql.columnDescs.forEach_(_, function(_, desc) {
				...
				if (slot) {
					...
					if (slot.collection) value = value.get(_, desc.name, true); // For 1x1 collections
				} else {
					// Set a default value :
					value = desc.type.defaultValue;
				}
