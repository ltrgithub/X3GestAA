	...
	constructor: util.defineClass(function(superv, data) {
		...
		deleteIndexes: function(_, ifExists) {
			this.indexes.forEach_(_, function(_, index) {
				try {
					var sql = this.superv.sqlDriver.dropIndexSql(this.name + "_" + index.name, this.name);
					this.superv.executeSql(_, sql);
				} catch (ex) {
					// do not fail, just report error
					if (ifExists && this.superv.sqlDriver.isIndexNotFound(ex)) return; // console.error(index.name + ": " + ex.message);
					else throw ex;
				}
			}, this);
		},
		...
		loadRecords: function(_, index, vals, max, where, raw) {
			...
			var self = this;
			return this.superv.sqlDriver.withConnection(_, function(_, cnx) {
		...
		writer: function(_, cnx) {
			...
			var sql = "insert into " + this.name + " (" + this.sql.columnNames.join(',') + //
			") values (" + this.sql.insertVars + ")";
			var stmt = this.superv.sqlDriver.prepare(cnx, sql);
			var self = this;
			var count = 0;
		...
		drop: function(_, ifExists) {
			try {
				...
				if (ifExists && this.superv.sqlDriver.isTableNotFound(ex)) return;
				else throw ex;
			}
		},
