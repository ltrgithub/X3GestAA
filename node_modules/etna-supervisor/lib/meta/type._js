...
module.exports = {
	...
	constructor: util.defineClass(function(superv, data) {
		...
		jsType: {
			get: function() {
				switch (parseInt(this.data.TYPTYP)) {
				case 1:
					// blob type
					return "string";
				case 2:
		...
		sqlType: function(colData) {
			var driver = this.superv.sqlDriver;
			switch (parseInt(this.data.TYPTYP)) { // temporary parseInt hack
			case 1:
		...
		fromSql: function(val) {
			...
			case 8:
				// date
				return val ? date.fromJsDate(val) : null;
			case 12:
				// datetime
				return val ? datetime.fromJsDate(val) : null;
			default:
				return val;
			}
		},
		...
		fillNode: function(_, node) {
			...
			case 'ATYPE1':
				...
				node.$value = {
					...
					$enum: this.superv.loadEnum(_, this.data.NOLIB),
				}
				break;
			case 'ATYPE2':
				// some integer
		...
		targetClass: function(_) {
			if (this.data.CODCLA) return this.superv.load(_, 'Class', this.data.CODCLA).init(_);
		},
		maxLength: function() {
