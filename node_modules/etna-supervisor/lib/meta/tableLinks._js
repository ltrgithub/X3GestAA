...
var linkOperations = {
	control: {
		type: 1,
		execute: function(_, instance, superv, cnx, sql) {
	...
	reset: {
		...
		execute: function(_, instance, superv, cnx, sql) {
			...
			superv.executeSql(_, sql, sql.params);
			return constants.CST_AOK;
				...
				Object.keys(self.links[operation.type]).forEach_(_, function(_, groupKey) {
					...
					if (group.links) {
						...
						if (sql.wheres) {
							...
							status = Math.max(status, operation.execute(_, instance, self.supervisor, cnx, sql));
						}
					}
				});
			});
