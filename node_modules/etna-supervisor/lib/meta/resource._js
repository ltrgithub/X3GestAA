...
var Resource = exports.constructor = util.defineClass(function(superv, resourceFactory, instance) {
	...
	get: function(_, name) {
		if (name === this.resourceFactory.representation.data.ABRCLA) {
	...
	dataNode: function(_) {
		...
		if (this.instance.class.group == null) {
			...
			result.$uuid = this.instance.class.properties['AUUID'] ?
				this.instance.get(_, 'AUUID') :
				tuuid.generate();
		}
		var self = this;
		Object.keys(this.resourceFactory.properties).forEach_(_, function(_, name) {
			...
			if (prop.collection) {
				...
				if (prop.collection.mappingData.TYPLNK === 3) {
				...
				result[prop.tag] = this.properties[prop.data.CODFLD];
			} else {
