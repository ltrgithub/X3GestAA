...
var Resource = exports.constructor = baseInstance.defineClass(function(superv, resourceFactory, instance, slots) {
	...
	applyDelta: function(_, delta) {
		...
		/*dbg*/
		console.log("applyDelta to:");
		var self = this;
		Object.keys(this.properties).forEach(function(p) {
			console.error([self.properties[p].tag, self.properties[p].name]);
		});
		...
		Object.keys(delta).forEach_(_, function(_, tag) {
			...
			var prop = util.find(self.properties, 'tag', tag);
			/*dbg*/
			console.log("applyDelta prop:" + JSON.stringify(prop));
			...
			if (prop.collection) {
				...
				/*dbg*/
				console.error(JSON.stringify(prop.data));
				this.instance.set(_, prop.name, v);
			}
		}, this);
		var newDelta = this.getDelta(_, before);
