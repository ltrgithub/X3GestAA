...
var Resource = exports.constructor = util.defineClass(function(superv, resourceFactory, instance) {
	...
	dataNode: function(_) {
		var result = this.instance.instanceFactory.group == null ? {
		...
		Object.keys(this.resourceFactory.properties).forEach_(_, function(_, name) {
			...
			if (prop.collection) {
				result[prop.tag] = this.instance.get(_, name).map_(_, function(_, child) {
					return this.superv.new(_, 'Resource', prop.targetResourceFactory, child).dataNode(_);
				}, this);
	...
	clone: function() {
		return new Resource(this.superv, this.resourceFactory, this.instance.clone());
	},
