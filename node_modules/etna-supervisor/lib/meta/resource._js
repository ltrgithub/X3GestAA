...
var Resource = exports.constructor = baseInstance.defineClass(function(superv, resourceFactory, instance, slots) {
	...
	x3ActionRead: function(_, action, key) {
		tracerJs.debug && tracerJs.debug("resource.x3ActionRead " + action + " key:" + key);
		...
		var dargs = delayedArgs(_, this.instance, this.getX3ActionArgs(_, action, key));
	...
	afterQuery: function(_) {
		return this.x3ActionRead(_, "AQUERY_TRANS_AFTER");
	},
	read: function(_, key) {
		this.key = key;
		...
		var status = this.x3ActionRead(_, "AREAD_BEFORE", key);
		if (status !== this.supervisor.constants.CST_AOK) return status;
		status = this.instance.read(_, key);
		if (status === this.supervisor.constants.CST_AERROR) {
			this.setStatus(_, null,
				this.supervisor.constants.CST_AERROR,
				this.supervisor.constants.CST_AHTTP_NOT_FOUND,
				key + ":" + this.supervisor.loadMess(_, 100, 23));
		} else {
			status = this.x3ActionRead(_, "AREAD_AFTER");
		}
		return status;
	},
	isDeleteAllowed: function(_) {
		if (0) {
			// TODO AGETWRIGHTD
			return this.setStatus(_,
				null,
				this.supervisor.constants.CST_AERROR,
				this.supervisor.constants.CST_AHTTP_FORBIDDEN,
				this.supervisor.loadMess(_, 139, 327) + " " + this.supervisor.loadMess(_, 139, 328));
		}
	...
	delete: function(_) {
		...
		var status = this.isDeleteAllowed(_);
		...
		return this.setStatus(_, null,
			...
			this.supervisor.constants.CST_AHTTP_OK,
