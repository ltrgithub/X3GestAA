...
var Resource = exports.constructor = baseInstance.defineClass(function(superv, resourceFactory, instance, slots) {
	...
	dataNode: function(_) {
		...
		if (this.instance.class.group == null) {
			if (this.instance.class.properties['UPDDATTIM']) {
				this.instance.getProperty(_, 'UPDDATTIM') ? this.instance.getProperty(_, 'UPDDATTIM').toString() : "0000-00-00T00:00:00Z";
			}
		}
		...
		Object.keys(this.properties).filter(function(name) {
			...
			if (prop.collection) {
				if (prop.collection.mappingData && prop.collection.mappingData.TYPLNK === 3) {
					...
					var child = instance.getProperty(_, name);
					...
					var rChild = instance.getProperty(_, name).lines.map_(_, function(_, child) {
				...
				result[prop.tag] = prop.type.toJson(instance.getProperty(_, name), prop.data);
				if (prop._isReference) {
					result[prop.tag + '_REF'] = instance.getProperty(_, name + '_REF');
	...
	getDiagnoses: function(reply) {
		var httpStatus = constants.CST_AHTTP_OK;
	...
	getDelta: function(_, old) {
		...
		Object.keys(this.properties).forEach_(_, function(_, name) {
			...
			if (prop.collection) {
				...
				var oldValue = old.instance.getProperty(_, name);
				var newValue = this.instance.getProperty(_, name);
	...
	applyDelta: function(_, delta) {
		...
		Object.keys(delta).forEach_(_, function(_, name) {
			...
			if (prop.collection) {
				...
				this.instance.setProperty(_, name, v);
			}
		}, this);
	...
	read: function(_, key) {
		...
		return this.instance.read(_, key);
	},
	delete: function(_) {
