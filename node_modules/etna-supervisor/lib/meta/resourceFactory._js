...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	loadResource: function(_, key, qs) {
		...
		r.instance = this.class.loadInstance(_, key, qs);
	...
	query: function(_, httpContext, qs) {
		...
		if (/^\$(query|lookup)$/.test(this.facet)) {
			var resources = this.class.queryInstances(_, select, itemsPerPage + 1).map(fromInstanceToResource).toArray(_);
			...
			this.class.queryInstances(_, select, qs.count || Infinity).map(fromInstanceToResource).pipe(_, dst);
		}
	},
