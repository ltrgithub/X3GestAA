...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	query: function(_, httpContext, qs) {
		/*dbg*/
		trace && trace(this.facet + " " + JSON.stringify(qs));
		...
		Object.keys(index).forEach(function(idx) {
			/*dbg*/
			trace && trace("index[" + idx + "]=" + index[idx].name);
		...
		if (rights._authorizedRepresentations[this.representation.data.CODREP]._conditions) {
			if (rights._authorizedRepresentations[this.representation.data.CODREP]._conditions._type = "authorization") {
				trace && trace("add rights where" + JSON.stringify(rights._authorizedRepresentations[this.representation.data.CODREP]._conditions[0]._where));
		...
		//dbg:trace && trace("wheres.length:" + wheres.length);
		//dbg:trace && trace("wheres:" + JSON.stringify(wheres));
