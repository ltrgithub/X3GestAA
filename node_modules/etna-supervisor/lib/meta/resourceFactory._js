...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	itemProto: function(_, proto, facet) {
		...
		Object.keys(self.properties).filter(function(name) {
			...
			if (prop.collection && prop.collection.mappingData && prop.collection.mappingData.TYPLNK === 3) {
				...
				hasToBeSorted = true;
			} else if (prop.collection && !prop.collection.mappingData && prop.collection.nbrPro === 1) {
				...
				delete properties[properties.length - 1].$title;
				delete properties[properties.length - 1].$shortTitle;
				...
				if (p && prop._isReference) {
					...
					proto.$properties[(prop.tag || name) + "_REF"] = prop.prototypeRefNode(_, p);
				}
			}
	...
	pageProto: function(_) {
		...
		if (this.facet !== '$bulk') {
			...
			Object.keys(this.page.links).filter_(_, function(_, name) {
				var isLink = (~[1, 3, 4, 5].indexOf(this.page.links[name].data.TYPLNK)) && this.page.links[name].data.CMPLNK < 17;
				return isLink === false && this.page.links[name].data.CMPLNK !== 19;
			}, this).forEach_(_, function(_, name) {
	...
	loadResource: function(_, key, qs) {
		var r = this.supervisor.new(_, 'Resource', this, null, this.representation.addSysProperties());
		var astatus = this.representation.invokeScripts(_, r, "$AREAD_BEFORE");
		r.instance = this.class.loadInstance(_, r, key, qs);
		var astatus = this.representation.invokeScripts(_, r, "$AREAD_AFTER");
		return r;
	...
	tag2PropertyTags: {
		get: function() {
			if (this._tag2PropertyTags === undefined) {
				var self = this;
				this._tag2PropertyTags = Object.keys(self.properties).reduce(function(r, property) {
	...
	tag2Property: function(tag) {
		...
		return this.tag2PropertyTags[tag] || tag;
	},
	...
	filterRep: function(_, name) {
		if (this.representation._filters === undefined || this.representation._filters[name] === undefined) {
			...
			this.representation.data.OPTIONS.forEach(function(elt) {
	...
	query: function(_, httpContext, qs) {
		...
		select.$params = Object.keys(qs).reduce(function(r, tag) {
			var property = self.tag2PropertyTags[tag] || (self.properties[tag] && self.properties[tag].tag);
			if (property) {
				r[property] = qs[tag];
			}
			return r;
		...
		var fromInstanceToResource = function(_, reader, writer) {
			var instance;
			var resource;
			var astatus;
			var CST_AOK = this.supervisor.constants.CST_AOK;
			while (instance = reader.read(_)) {
				resource = this.supervisor.new(_, 'Resource', this, instance, this.representation.addSysProperties());
				instance.parent = resource;
				astatus = this.representation.invokeScripts(_, resource, "$AQUERY_TRANS_AFTER");
				if (astatus === CST_AOK) {
		...
		if (/^\$(query|lookup)$/.test(this.facet)) {
			//var resources = this.class.queryInstances(_, null /*, this*/ , select, itemsPerPage + 1).transform(fromInstanceToResource).toArray(_);
			var resources = this.class.queryInstances(_, null /*, this*/ , select).transform(fromInstanceToResource).limit(itemsPerPage + 1).toArray(_);
			...
			this.class.queryInstances(_, null, select, qs.count || Infinity).transform(fromInstanceToResource).pipe(_, dst);
		}
	},
