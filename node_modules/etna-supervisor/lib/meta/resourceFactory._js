...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	loadResource: function(_, key, qs) {
		...
		var astatus = this.representation.invokeScripts(_, "$AREAD_BEFORE", r);
		r.instance = this.class.loadInstance(_, r, key, qs);
		var astatus = this.representation.invokeScripts(_, "$AREAD_AFTER", r);
		return r;
	...
	query: function(_, httpContext, qs) {
		...
		var fromInstanceToResource = function(_, reader, writer) {
			...
			while (instance = reader.read(_)) {
				...
				astatus = this.representation.invokeScripts(_, "$AQUERY_TRANS_AFTER", resource);
				if (astatus === CST_AOK) {
		...
		if (/^\$(query|lookup)$/.test(this.facet)) {
			this.representation.invokeScripts(_, "$AQUERY_OPEN");
			var resources = this.class.queryInstances(_, null, select).transform(fromInstanceToResource).limit(itemsPerPage + 1).toArray(_);
			this.representation.invokeScripts(_, "$AQUERY_CLOSE");
