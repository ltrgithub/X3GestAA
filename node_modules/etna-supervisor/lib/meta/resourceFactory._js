...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	itemProto: function(_, facet) {
		...
		Object.keys(self.properties).forEach_(_, function(_, name) {
			...
			if (prop.collection && prop.collection.mappingData && prop.collection.mappingData.TYPLNK === 3) {
				...
				if (p && prop.type && prop.type.data.CODCLA && prop.isReference) {
	...
	tag2Property: function(tag) {
		if (this.tag2PropertyTags === undefined) {
			var self = this;
			this.tag2PropertyTags = Object.keys(self.properties).reduce(function(r, property) {
	...
	query: function(_, httpContext, qs) {
		...
		var actx = glob.context.x3session.actx;
		var rights = actx._x3Rights.getRigths(_, actx.$LOGIN(_));
