...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	deleteResource: function(_, httpContext, $key, qs) {
		...
		var resource = this.supervisor.new(_, 'Resource', this, null);
		var status = resource.delete(_, key);

		if (status !== this.supervisor.constants.CST_AOK) {
			httpStatus = 404;
			reply.$diagnoses = resource.diagnoses.reduce(function(r, diag) {
				r.push(diag.dataNode());
				return r;
			}, []);
		} else {
			reply.$diagnoses.push(this.supervisor.new(_, 'Diagnosis', 'success', null,
				this.supervisor.loadMessParams(_, 149, 61, $key)).dataNode());
		}
