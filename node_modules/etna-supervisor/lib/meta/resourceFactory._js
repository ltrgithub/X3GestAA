...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	query: function(_, httpContext, qs) {
		...
		//dbg:tracerJs.debug && tracerJs.debug("wheres.length:" + wheres.length);
		//dbg:tracerJs.debug && tracerJs.debug("wheres:" + JSON.stringify(wheres));
		...
		var fromInstanceToResource = function(_, instance) {
			// if (instance._resource) {
			// 	var resource = instance._resource;
			// 	instance._resource = null;
			// 	delete instance._resource;
			// } else
			var resource = this.supervisor.new(_, 'Resource', this, instance, this.representation.addSysProperties());
			instance.parent = resource;
			var astatus = this.representation.invokeScripts(_, resource, "$AQUERY_TRANS_AFTER");
			return resource;
		}.bind(this);

		// (require itemsPerPage+1 items to manage next link 
		if (/^\$(query|lookup)$/.test(this.facet)) {
			var resources = this.class.queryInstances(_, null /*, this*/ , select, itemsPerPage + 1).map(fromInstanceToResource).toArray(_);
			...
			this.class.queryInstances(_, null /*, this*/ , select, qs.count || Infinity).map(fromInstanceToResource).pipe(_, dst);
		}
	},
