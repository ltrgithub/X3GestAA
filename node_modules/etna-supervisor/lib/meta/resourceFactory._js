...
var activ = require("etna-supervisor/lib/meta/activ._js"); // GH 6167
...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	pageProto: function(_) {
		...
		pageProto.$actxLegCur = actx.getProperty(_, "AFOLD").getProperty(_, "ALEGCUR");
		pageProto.$actxNbLeg = "" + actx.getProperty(_, "AFOLD").getProperty(_, "ANBLEG");
		...
		if (this.facet === "$edit") {
			pageProto.$x3FormatExt = {};
			for (var i = 1; i <= 30; i++) {
				var wparamTUL;
				try {
					wparamTUL = actx.$APARAM(_).AGETVALCHAR(_, 1, pageProto.$actxFolder, "TUL" + ("00" + i).slice(-2));
				} catch (e) {
					wparamTUL = "";
				}
				if (wparamTUL !== undefined && wparamTUL !== "") {
					var tableau = wparamTUL.split(":");
					if (tableau !== undefined && tableau.length === 2) {
						var wTul = tableau[0][0];
						pageProto.$x3FormatExt[wTul] = {
							"stdType": [],
							"chars": []
						};
						for (var j = 0; j < tableau[1].length; j++) {
							pageProto.$x3FormatExt[wTul].stdType.push(tableau[1][j]);
						}
						for (var j = 1; j < tableau[0].length; j++) {
							pageProto.$x3FormatExt[wTul].chars.push(tableau[0][j]);
						}
					}
				}
			}
	...
	deleteResource: function(_, httpContext, $key, qs) {
		...
		status = resource.$ADELETE(_);

		var reply = {};
		var httpStatus = resource.getDiagnoses(reply);
		return httpContext.jsonReply(_, httpStatus, reply);
	},
	tag2PropertyTags: {
		get: function() {
		...
		var fromInstanceToResource = function(self) {
			return function(_, reader, writer) {
				...
				while (instance = reader.read(_)) {
					resource = self.supervisor.new(_, 'Resource', self, instance);
			...
			if (resources.length) {
				...
				var linkOrderBy = orderBy.reduce_(_, function(_, r, property) {
					if (!firstPage) {
						...
						links[names[0]].push(resources[0].instance.getProperty(_, property.property));
					}
					if (resources.length > itemsPerPage) {
						links[names[1]] = links[names[1]] || [keyNext];
						links[names[1]].push(resources[resources.length - 2].instance.getProperty(_, property.property));
					}
