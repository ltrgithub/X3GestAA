...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	mapTags2Properties: function(o) {
		...
		return Object.keys(o).reduce(function(r, tag) {
			...
			r[property] = (typeof o[tag] === 'object') ? _this.mapTags2Properties(o[tag]) : o[tag];
			return r;
		}, {});
	},
	sdataFilterToJson: function(filter) {
		return this.mapTags2Properties(filterHelpers.sdataFilterToJson(decodeURIComponent(filter)));
	},
	query: function(_, qs) {
		...
		if (qs.key) {
			...
			if (keyItems.length > 1) {
				...
				var k = 0;
				var args = qs.orderBy.split(',').reduce(function(r, item) {
					var tag = item.split('%20')[0];
					var property = _this.tag2Property(tag);
					r.push({
						property: property,
						value: keyParts[k++]
		...
		var resources = this.class.queryInstances(_, select, itemsPerPage + 1).map_(_, function(_, instance) {
