...
exports.constructor = util.defineClass(function(superv, repr, facet, path, clas) {
	...
	query: function(_, httpContext, qs) {
		...
		var fromInstanceToResource = function(_, instance) {
			var resource = this.supervisor.new(_, 'Resource', this, instance);
		...
		if (/^\$(query|lookup)$/.test(this.facet)) {
			var resources = this.class.queryInstances(_, null, select, itemsPerPage + 1).map(fromInstanceToResource).toArray(_);
			...
			this.class.queryInstances(_, null, select, qs.count || Infinity).map(fromInstanceToResource).pipe(_, dst);
		}
	},
