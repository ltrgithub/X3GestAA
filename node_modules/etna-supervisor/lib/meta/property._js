...
var Property = exports.constructor = util.defineClass(function(superv, parent, data) {
	this.superv = superv;
	this.data = data;
	this.parent = parent;
}, null, {
	...
	clone: function(parent) {
		var p = new Property(this.superv, parent, this.data);
		p.type = this.type;
	...
	targetClass: function(_) {
		...
		return this.superv.load(_, 'Class', this.data.LNKCLA);
	},
	prototypeNode: function(_) {
		var self = this;
		var node = {
			$title: this.superv.loadText(_, this.data.INTFLD),
			$shortTitle: this.superv.loadText(_, this.data.INTSHTFLD),
		}
		var keyParams = this.parent.class.data.PARAM_FLDS.filter(function(paramData) {
...
/* && keyParams.some(function(param) {
		...
		if (this.targetResourceFactory) {
			...
			this.superv.fillTypeNode(_, this.data, node);
		} else {
	...
	prototypeRefNode: function(_, simpleNode) {
		...
		//console.error("CREATING REF FOR: " + this.parent.name + "." + this.name);
		var item = node.$item;
		...
		item.$key = targetClass.instanceFactory.keyExpression(_);
		...
		keyParams.forEach_(_, function(_, paramData) {
			...
			this.superv.fillTypeNode(_, targetProp.data, item.$properties[paramData.CODPAR]);
...
/* for info, typical result
	...
	column: {
		get: function() {
			return this.parent.class.table.column(this.data.FLDCLA);
		}
	},
});