...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, newMaker, collection) {
	...
	updateText: function(_, instance, value) {
		// Use meta ATEXTRA :
		var self = this;
		...
		this.keyParams.forEach_(_, function(_, paramData) {
			var map = self.addFieldTransform(_, paramData.VALEUR);
			atextra.set(_, paramData.CODPAR, map(_, instance), true);
		});
		// Add the text:
		...
		try {
			...
			// The insert failed :
			// => try to update the instance of atextra 
			atextra.update(_, options);
		}
	},
	deleteText: function(_, instance) {
		// Use meta ATEXTRA :
		var self = this;
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, cache, raw, sqlOpts) {
		...
		if (this.isText) {
			...
			var self = this;
			var atextra = this.keyParams.reduce_(_, function(_, r, paramData) {
				r[paramData.CODPAR] = self.addFieldTransform(_, paramData.VALEUR);
				return r;
			}, {});
			...
			if (this.isReference(_)) {
				...
				cache.references[this.name] = {
					$name: this.name,
					$ids: {},
					cacheReference: function(_, property) {
						var getReferenceKey = property.makeGetReferenceKey(_);
						return function(_, cache, record) {
							var id = getReferenceKey(_, record);
							if (id) cache.references[property.name].$ids[id] = null;
							return record;
						};
					}(_, this)
				};
		...
		if (value === '""') {
			...
			return function(_, column) {
				return function(_, record) {
					...
					if (record.class && record.class && record.class.$class && record.class.$class.class.properties && record.class.$class.class.properties[column]) { // instance
						s = record.class.$class.slot(_, column);
						return s._get(_);
					}
	...
	loadReference: function(_, record) {
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			...
			function addTextTransform(_, _this, superv, attribute, reference, column, refReader) {
				...
				if (reference.properties[column].isText) {
			...
			try {
				...
				tracerJs.error && tracerJs.error(e.message, e.stack);
			} finally {
		...
		var refId = typeof getReferenceKey === "function" && getReferenceKey(_, record) || "";
