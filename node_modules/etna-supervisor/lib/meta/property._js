...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	set: function(value) {
		...
		//return variables.types[this.etnaType].set(this.name, this.set.key, value); //GH 5922
	...
	prototypeNode: function(_) {
		...
		var title = "";
		if (this.exportedProperty && this.exportedProperty.data.INTPRO) title = this.supervisor.loadText(_, this.exportedProperty.data.INTPRO);
		else if (this.data.INTFLD) title = this.supervisor.loadText(_, this.data.INTFLD);
		else if (this.collection && this.collection.data && this.collection.data.INTCOL) title = this.supervisor.loadText(_, this.collection.data.INTCOL);
		node.$title = title;
		...
		if (this.targetResourceFactory) {
			if (!(this.collection.data.MAXCOL === 1 && this.collection.data.MINCOL === 2)) {
				...
				//if (this.collection.data.MINCOL===2) node.$minItems = 1;
				//else 
				if (this.collection.data.MINCOL === 3 && this.collection.data.MAXCOL > 0) node.$minItems = this.collection.data.MAXCOL;
			}
			...
			if (this.collection && (
				(this.collection.mappingData && [2, 4].indexOf(this.collection.mappingData.TYPLNK) >= 0) ||
				(this.collection.class && this.collection.class.data && this.collection.class.data.TYPCLA === 5) ||
				(this.collection.class && this.collection.class.data && this.collection.ofIndexedProperties === true)
			)) {
				if (node.$item.$properties) node.$item.$instanceUrl = "{$instanceUrl}/" + this.name + "('{$uuid}')";
			}
		} else if (this.data.CODTYP != null) {
			if (this.type && this.type.$x3Format && this.resourceFactory.facet === '$edit') {
				node.$x3Format = this.type.$x3Format;
			}
			if ((!this.isExtra && this.column !== undefined) || (this.class && this.class.data && this.class.data.TYPCLA && this.class.data.TYPCLA !== 2) || (this.type && this.type.data.CODCLA === 'ATEXTRA')) {
				...
				if (this.type && this.type.data.CODCLA === 'ATEXTRA') {
					...
					node.$capabilities = (this.resourceFactory.facet === '$edit' ? "sort" : "sort,filter,filter_upper");
				} else if (this.type.data.TYPTYP !== 9 && this.type.data.TYPTYP !== 10) {
					node.$capabilities = (this.resourceFactory.facet === '$edit' ? "sort" : "sort,filter");
				}
			...
			if (this.links && this.links["ALOB"]) {
				...
				//if (this.isExtra === true) repr = this.class;
				//else repr = this.representation;
				...
				if (this.type.atypepro.lisflg !== 2) {
					...
					if (this.isExtra === true && this.class.data.PARAMS) {
						acceptedTypes = this.class.data.PARAMS.filter(function(paramData) {
						...
						acceptedTypes = this.class.data.PARAM_FLDS.filter(function(paramData) {
				...
				if (this.resourceFactory.facet === "$edit") node.$url = "{$instanceUrl}/" + this.name;
					...
					var sep = "~%C2%A8~";
					if (repr.class.curpath === this.class.curpath && repr.class.data.CODREP)
						node.$url = node.$url + "R_" + repr.class.data.CODREP;
					else {
						//node.$url = node.$url + "C_" + repr.class.data.CODCLA;
						node.$url = node.$url + "C_" + this.class.data.CODCLA;
					}
	...
	calculateRef: function(_, item) {
		...
		var keyParams = [];
		if (this.isExtra === true && this.class.data.PARAMS) {
			keyParams = this.class.data.PARAMS.filter(function(paramData) {
			...
			keyParams = this.class.data.PARAM_FLDS.filter(function(paramData) {
	...
	prototypeRefNode: function(_, simpleNode) {
		...
		if (link === undefined) {
			...
			if (link.data.REPLNK === repr.name) {
				f = repr.resourceFactory(_, facet);
			} else {
					...
					link.keys.forEach_(_, function(_, paramData) {
						if (keyitem.prop === paramData.CODPAR) {
							...
							if (item.$properties[keyitem.alias].$value && item.$properties[keyitem.alias].$value.$type) {
								...
								delete item.$properties[keyitem.alias].$value;
							}
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, cache, raw, sqlOpts) {
		...
		if (/^AX[123X]/.test(this.data.CODTYP)) {
			...
			var keyNames = this.class.table.indexes[0].sqlNames().slice(0, 2).map(function(key) {
			...
			cache.texts.getKeyText = cache.texts.getKeyText || function(_, record) {
				tracerJs.debug && tracerJs.debug("getKeyText " + atextra.IDENT1(_, record) + '~' + (atextra.IDENT2(_, record) || " "));
				return atextra.IDENT1(_, record) + '~' + (atextra.IDENT2(_, record) || " ");
			};

			cache.texts.properties[this.name] = {
				ids: [],
				texts: null,
				cacheTextKey: function(_, property, keys) {
					return function(_, cache, record) {
						record.$id = cache.texts.getKeyText(_, record);
						cache.texts.properties[property].ids.push(record.$id);
						return record;
					};
				}(_, this.name, keyNames),
				loadText: function(_this, zone, atextra) {
					...
					return function(_, textId) {
						...
						if (_this.class.cache.texts.properties[zone].texts === null) {
							...
							params.push(atextra.CODFIC(_, {}));
							wheres.push("CODFIC_0=" + param(params.length - 1));
							params.push(atextra.ZONE(_, {}));
							wheres.push("ZONE_0=" + param(params.length - 1));
							params.push(atextra.LANGUE(_, {}));
			...
			if (this.isReference(_)) {
				...
				cache.references[this.name] = {
					...
					cacheReference: function(_, name, $getReferenceKey) {
						return function(_, cache, record) {
							var id = $getReferenceKey(_, record);
	...
	addFieldTransform: function(_, value, index, refReader) {
		...
		if (value === '""') {
			...
			}(glob.context.x3session.actx.getProperty(_, value.substring(5)));
		} else if (regNum$.test(value) || regVal$.test(value)) {
			var match = regNum$.exec(value);
			return function(fct) {
				return function(_, record) {
					return fct(_, record).toString();
				};
			...
			return function(_, column) {
				return function(_, record) {
					var s;
					if (record.class && record.class.properties && record.class.properties[column]) { // instance
						s = record.slot(_, column);
						return s.get(_);
					}
					if (record.class && record.parent && record.parent.$parent && record.parent.$parent.class.properties && record.parent.$parent.class.properties[column]) { // instance
						s = record.parent.$parent.slot(_, column);
						return s.get(_);
					}
					if (record.resourceFactory && column.indexOf('.') < 0) {
						s = record.slot(_, column);
						return s.get(_);
					}
					if (record.resourceFactory && record.instance && column.indexOf('.') >= 0) {
						var path = column.split('.');
						s = record.instance.slot(_, path[1]);
						return s.get(_);
					}
	...
	loadReference: function(_, record) {
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			...
			if (!this.isExtra) {
				...
				if (!this.column || this.class.group)
					cache.references[this.name].$ids[id] = null; //VVV don't manage cache for the moment
			} else {
				cache = this.resourceFactory.cache;
				cache.references[this.name].$ids[id] = null; //VVV don't manage cache for the moment
			}
			try {
				if (!Object.keys(cache.references[this.name].$ids).length) return;

				// First create empty references :
				var refs = cache.references[this.name];
				Object.keys(refs.$ids).forEach(function(id) {
					refs[id] = {};
				});

				cache.references[this.name][id] = null;
				...
				if (refReader.columns) {
					...
					var params = [];
					var wheres = [];
					var ids = Object.keys(cache.references[this.name].$ids);
				...
				if (titleTransform || descTransform) {
					if (!reader) {
						...
						}(_, reference, Object.keys(cache.references[this.name].$ids));
				...
				var references = cache.references[this.name];
				...
				cache.references[this.name].$ids = {};

			} catch (e) {
				tracerJs.debug && tracerJs.debug(JSON.stringify(e));
			} finally {
				tracerJs.debug && tracerJs.debug("loadReferenceData:" + JSON.stringify(cache.references[this.name][id]));
				return cache.references[this.name][id];
			}
		};

		var refId = typeof this.getReferenceKey === "function" && this.getReferenceKey(_, record) || "";
		tracerJs.debug && tracerJs.debug("loadReference " + this.name + " id:" + refId);
		if (!refId.length) return {};

		var reference;
		if (!this.isExtra) {
			reference = this.class.cache.references[this.name][refId];
			if (reference === undefined) {
				this.class.cache.references[this.name][refId] = reference = this.loadReferenceData(_, refId);
			}
		} else {
			reference = this.resourceFactory.cache.references[this.name][refId];
			if (reference === undefined) {
				reference = this.loadReferenceData(_, refId);
				this.resourceFactory.cache.references[this.name][refId] = reference;
			}
		}
		return reference;
	},
	readBDOC: function(_, ident1, ident2, ident3) {
		...
		var self = this;
		try {
			var rs = sqlDriver.withConnection(_, function(_, cnx) {
				var i = 0;
				return self.supervisor.sqlReader(_, cnx, "select C." + field + "_0 From " + table + " C Where" +
