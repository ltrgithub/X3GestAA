...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	loadReference: function(_, record) {
		...
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
					...
					var atextra = reference.data.PARAM_FLDS.filter(function(paramData) {
						...
						if (paramData.VALEUR === '""') {
							...
							if (index.indexOf(paramData.VALEUR) < 0) {
								...
								refReader.columns.push(paramData.VALEUR + "_0");
							}
						}
						return r;
					}, {});
			...
			try {
				this.class.cache.references[this.name][id] = {};
				if (!id.length) return;
				...
				console.log("refReader:" + refReader);

				var reader;
				if (titleTransform || descTransform) {
					if (!reader && !refReader.columns) {
		...
		//dbg:console.log("loadReference refId:"+refId);
