...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	loadReference: function(_, record) {
		...
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			console.log(this.name + " loadReferenceData :" + id);
			if (!id.length) return {};
			...
			var refFields = ["SYMBOL", "COLFMT", "COLDEC", "COLLNG"].reduce(function(r, field) {
				if (reference.table.data[field]) {
					r[field] = reference.table.data[field];
					countRefFields++;
				}
				return r;
			}, {});

			tracerJs.debug && tracerJs.debug(this.name + " table:" + reference.table.name);
			...
			// HACK:
			return this.class.cache.references[this.name][refId] = {};

			//TODO : AX1,AX3...

			var fTitle = reference.table.data.INTITC;
			var fDescription = reference.table.data.INTIT;

			tracerJs.debug && tracerJs.debug(this.name + " loadReferenceData fTitle:" + fTitle);
			tracerJs.debug && tracerJs.debug(this.name + " loadReferenceData fDescription:" + fDescription);

			false && fTitle && names.push(fTitle + '_0');
			false && fDescription && names.push(fDescription + '_0');
			names.concat(Object.keys(refFields).map(function(field) {
			...
			// 	console.log("ref record:"+JSON.stringify(record));
