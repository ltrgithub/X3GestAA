		...
		if (value === '""') {
			...
			return function(column) {
				return function(record) {
					...
					else if (record.record && record.record[column]) return record.record[column];
					else if (record.slots && record.slots[column]) return record.slots[column].value;
					else if (column.indexOf('.') >= 0 && record.instance) {
						var path = column.split('.');
						return record.instance.record[path[1]];
					}
	...
	loadReference: function(_, record) {
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			...
			if (!this.isExtra) {
				...
				if (!this.column)
					cache.references[this.name].$ids[id] = null; //VVV don't manage cach for the moment
			} else {
