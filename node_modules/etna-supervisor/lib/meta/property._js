...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	init: function(_) {
		...
		if (this.data.CODTYP) {
			...
			this.type.data.NOLIB = this.data.NOLIB;
	...
	prototypeNode: function(_) {
		...
		if (this.type && (this.type.data.CODCLA === 'ATEXTRA') //
			...
			if (this.exportedProperty.data.CODPRO.match(reg1)) {
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, cache, raw) {
		...
		if (/^AX[123X]/.test(this.data.CODTYP)) {
			/*dbg*/ //console.log("property.getSql AXX:"+this.name+" cache.texts:"+JSON.stringify(cache.texts));

			cache.texts = cache.texts || {
				ids: []
			};
			cache.texts.properties = cache.texts.properties || {};
			...
			cache.cacheTextKey = cache.cacheTextKey || function(keys) {
				if (keys.length == 1) return function(cache, record) {
					record.$id = record[keys[0]];
					cache.texts.ids.push(record.$id);
					return record;
				};
				...
				loadText: function(_this, codfic, zone, lan) {
					...
					return function(_, textId) {
						...
						if (_this.class.cache.texts.properties[zone].texts === null) {
							...
							wheres.push("IDENT1_0 in (" + _this.class.cache.texts.ids.map(function(id) {
								params.push(id);
			...
			if (this.isReference(_)) {
				...
				if (paramRefKeys.length === 1) {
					cache.references = cache.references || {};
					cache.references[this.name] = {
						$name: this.name,
						$ids: {},
						cacheReference: function(name, fldcla) {
							return function(cache, record) {
								var id = record[fldcla];
								if (id !== undefined) {
									cache.references[name].$ids[record[fldcla]] = null;
								}
								return record;
							};
						}(this.name, paramRefKeys[0].FLDCLA)
					};
				}
			}
	...
	isReference: function(_, value) {
		if (value === undefined) {
			...
			reference = !! reference;
			//!reference && console.log(this.name + " is a reference");
			return reference;
		}
	...
	loadReference: function(_, instance) {
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			...
			var getRecordId, getReferenceId;
			if (id.length === 1) {
			...
			//console.log("this.class.cache.references[this.name]:" + JSON.stringify(this.class.cache.references[this.name]));
		...
		var reference = this.class.cache.references[this.name][id];
		if (reference === undefined) {
			this.class.cache.references[this.name][id] = reference = this.loadReferenceData(_, id);
		}
		return reference;
	}
});