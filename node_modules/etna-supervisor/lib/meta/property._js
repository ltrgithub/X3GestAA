...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	prototypeNode: function(_) {
		...
		if (this.targetResourceFactory) {
			...
			if (!this.isExtra) { // VVVVV TODO exclude added properties of classes (not linked to a table field)
				...
				if (this.type && this.type.data.CODCLA === 'ATEXTRA' && this.resourceFactory.facet === '$edit') {
					node.$capabilities += ",localize";
				}
			}
			this.supervisor.fillTypeNode(_, this.data, node);
			if (this.exportedProperty.data.UOMPRO !== undefined) {
				node.$value = {
					$title: "{$value} {$unit}",
					$type: node.$type,
			...
			if (this.links && this.links["ALOB"]) {
				...
				if (repr.resourceFactory.facet === "$edit") node.$url = "{$instanceUrl}/" + this.name;
				else {
	...
	prototypeRefNode: function(_, simpleNode) {
		...
		if (!link) {
			//We should not pass there but still leaves the code
				...
				loadText: function(_this, zone, atextra) {
					...
					return function(_, textId) {
						...
						if (_this.class.cache.texts.properties[zone].texts === null) {
							...
							var getTextId;
							if (atextra.IDENT2 === null) {
								getTextId = function(record) {
									return record.IDENT1;
								};

								wheres.push("IDENT1_0 in (" + _this.class.cache.texts.properties[zone].ids
									.filter(function(id) {
										return id !== undefined;
									})
									.map(function(id) {
