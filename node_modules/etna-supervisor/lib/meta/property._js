...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, newMaker, collection) {
			...
			cache.texts.properties[this.name] = {
				...
				cacheTextKey: function(_, property, keys) {
					return function(_, cache, record) {
						var keyText = cache.texts.getKeyText(_, record);
						cache.texts.properties[property].ids.push(keyText);
						return record;
					};
	...
	addFieldTransform: function(_, value, index, refReader) {
		...
		if (value === '""') {
			...
			return function(_, column) {
				return function(_, record) {
					if (record.class) {
						if (record.class.properties && record.class.properties[column]) { // instance
							return record.slot(_, column)._get(_);
						}
						if (record.class.codclaPath.indexOf('.') > 0) {
							// Class associated to a collection
							var path = column.split('.');
							if (path.length > 1) {
								column = path[path.length - 1];
								if (record.class.properties[column]) {
									return record.slot(_, column)._get(_);
								}
							} else if (record.$parent && record.$parent.$parent) {
								var instance = record.$parent.$parent;
								var slot = instance.slot(_, column);
								if (slot) return slot._get(_);
							}
						}
					}

					if (record.resourceFactory) {
						if (column.indexOf('.') < 0) {
							return record.slot(_, column)._get(_);
						}
						if (record.instance && column.indexOf('.') >= 0) {
							var path = column.split('.');
							return record.instance.slot(_, path[1])._get(_);
						}
					}
	...
	loadAtextra: function(_, instance) {
		...
		var cache = instance.class.cache;

		if (!cache.texts) return "";

		if (cache.texts.properties[zone].texts === null) {
			...
			wheres.push("((" +
				cache.texts.properties[zone].ids.reduce(function(r, id) {
					var parts = id.split('~');
					params.push(parts[0]);
					params.push(parts[1]);
			...
			cache.texts.properties[zone].texts = this.supervisor.sqlReader(_, sql, params) //
			...
			cache.texts.properties[zone].ids = [];
		}
		//dbg:tracerJs.debug && tracerJs.debug("loadText zone:"+zone+' textId:'+textId+" ="+self.class.cache.texts.properties[zone].texts[textId]);
		return cache.texts.properties[zone].texts[textId] || "";
	},
