...
var Property = exports.constructor = util.defineClass(function(superv, clas, data, repr) {
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, cache, raw) {
		...
		if (/^AX[123X]/.test(this.data.CODTYP)) {
			...
			if (this.isReference(_)) {
				this.getReferenceKey = function($this) {
					var self = $this;
					var name = self.name;
					var refKeys = self.class.data.PARAM_FLDS.reduce(function(r, paramData) {
						paramData.FLDCLA === name && paramData.TYPKEY === 2 && r.push(paramData);
						return r;
					}, []);


					return function(record) {
						return refKeys.reduce(function(r, k) {
							r.push(record[k.VALEUR]);
							return r;
						}, []).join('~');
	...
	loadReference: function(_, record) {
		...
		this.loadReferenceData = this.loadReferenceData || function(_, id) {
			...
			function addTextTransform(_, superv, attribute, reference, column, refReader) {
				if (!column) return;

				if (/^(AX1|AX3)/.test(reference.properties[column].type.data.CODTYP)) {
					...
					var atextra = reference.data.PARAM_FLDS.filter(function(paramData) {
						return paramData.FLDCLA === column && paramData.TYPKEY === 2;
					}, this).reduce_(_, function(_, r, paramData) {
						if (paramData.VALEUR === '""') {
							r[paramData.CODPAR] = null;
						} else if (paramData.VALEUR.charAt(0) === '"') {
							r[paramData.CODPAR] = function(constant) {
								return function(record) {
									return constant;
								};
							}(paramData.VALEUR.substring(1, paramData.VALEUR.length - 1));
						} else if (typeof paramData.VALEUR === 'number') {
							r[paramData.CODPAR] = function(constant) {
								return function(record) {
									return constant;
								};
							}(paramData.VALEUR);
						} else if (paramData.VALEUR.substring(0, 5) === 'ACTX.') {
							r[paramData.CODPAR] = function(constant) {
								return function(record) {
									return constant;
								};
							}(superv.context.get(_, paramData.VALEUR.substring(5)));
						} else {
							r[paramData.CODPAR] = function(column) {
								return function(record) {
									return record[column];
								};
							}(paramData.VALEUR);
							if (index.indexOf(paramData.VALEUR) < 0) {
								console.log("need column:" + paramData.VALEUR);
								refReader.columns = refReader.columns || [];
								refReader.columns.push(paramData.VALEUR);
							}
						}
						return r;
					}, {});

					if (atextra.IDENT2 === null) {
						return function(superv, atextra, attribute) {
							return function(_, reader, writer) {
								...
								//dbg:console.log("rs:"+JSON.stringify(rs));
								...
								if (rs && rs.length) {
									...
									wheres.push("IDENT1_0 in (" + rs.map(function(record) {
										var textId = atextra.IDENT1(record.record);
										params.push(textId);
										record.reference[attribute] = textId;
										return param(params.length - 1);
									}).join(',') + ")");

									var sql = "select IDENT1_0, TEXTE_0 from ATEXTRA  where (" + wheres.join(" and ") + ")";
									console.log("sql:" + sql);
									...
									var texts = superv.sqlDriver.withConnection(_, function(_, cnx) {
										...
										r[text.IDENT1] = text.TEXTE;
										return r;
									}, {});

									//dbg:console.log("texts:"+JSON.stringify(texts));
			...
			try {
				...
				var ctx = this.supervisor.currentContext(_);
					...
					reader = this.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
						return self.supervisor.sqlReader(_, cnx, sql, params)
							.map(function(_, record) {
								...
								refReader.maps.forEach(function(map) {
									map(item);
								});
								return item;
							});
				...
				if (this.class.cache.references[this.name][id] === null) {
					// Create empty references :
					var references = this.class.cache.references[this.name];
					var ids = Object.keys(references.$ids);
		...
		//!tracerJs.debug && tracerJs.debug("loadReference refId:"+refId);
