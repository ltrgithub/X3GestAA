...
exports.defineClass = function(constructor, parent, members) {
	...
	members.getScriptArgs = function(_, action, curpro, curpth) {
		// Add instance's keys
		var self = this;
		var index = this.index;
		return Object.keys(index).reduce_(_, function(_, args, i) {
			var name = index[i].name.split(' ')[0];
			args[name] = {
				type: index[i].etnaType,
				value: self.instance ? self.instance.get(_, name, true) : self.get(_, name, true)
			};
			return args;
		}, {
			THIS: {
