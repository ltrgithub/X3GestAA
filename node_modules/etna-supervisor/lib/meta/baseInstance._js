...
exports.defineClass = function(constructor, parent, members) {
	...
	function addPropertyEvent(action) {
		return function(_, property, oldValue) {
			tracerEvents.debug && tracerEvents.debug("$PROPERTIES:" + property + " ACTION:" + action);
			...
			while (instance !== undefined) {
				...
				if (["INIT", "CONTROL"].indexOf(action) >= 0) return status;

				args.CURPRO.value = (instance.name ? (instance.name + ".") : "") + args.CURPRO.value;
