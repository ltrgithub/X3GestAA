...
exports.defineClass = function(constructor, parent, members) {
	...
	members.deleteDiagnoses = function() {
	...
	members.getDiagnoses = function(sort) {
		var self = this;
		...
		return Object.keys(this.slots).reduce(function(r, property) {
			var diags = self.slots[property].getDiagnoses();
	...
	*/
	...
	members.ADELETEERROR = function(_, property) {
		...
		owner.deleteDiagnoses();
		return constants.CST_AOK;
	};

	members.ADELETEERRORALL = function(_) {
		Object.keys(this.slots).forEach(function(p) {
			this.slots[property].deleteDiagnoses();
		});
		return constants.CST_AOK;
	};
	...
	members.AGETMAXERROR = function(_) {
		var self = this;
		var err = Object.keys(this.slots).filter(function(pro) {
			return self.slots[pro].collection === undefined;
		}).reduce(function(r, property) {
			var diags = self.slots[property].getDiagnoses();
			if (diags) r = r.concat(diags);
			return r;
		}, (this.diagnoses || [])).sort(function(d0, d1) {
			return d0.sortOnSeverity(d1);
		});

		return err[0] ? err[0].severity : 0;
	};

	members.AGETMAXERRORALL = function(_) {

		var err = this.getDiagnoses("sortOnSeverity");
		return err[0] ? err[0].severity : 0;
	};

	members.AGETMAXERRORTXT = function(_, type, property) {
		// HACK : each time an error is added this.AERRORS is recomputed
		property = "toto";
