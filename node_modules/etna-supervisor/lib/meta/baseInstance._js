...
exports.defineClass = function(constructor, parent, members) {
	...
	*/
	...
	members.getHighestErrors = function(_, etype) {

		var self = this;
		switch (etype) {
			case constants.CST_ALL:
				return self.getDiagnoses(_, "sortOnSeverity");
	...
	members.AGETMAXERROR = function(_) {
		var err = this.getHighestErrors(_, constants.CST_ACURRENT)[0];
		return err ? err.severity : 0;
	};

	members.AGETMAXERRORALL = function(_) {

		var err = this.getHighestErrors(_, constants.CST_ALL)[0];
		return err ? err.severity : 0;
	};
	...
	members.ASEARCHINFOS = function(_, local, property, status, selstatus) {
		var errt = this.getHighestErrors(_, local).filter_(_, function(_, e) {
			var ftr = true;
	...
	members.xmetAGETMAXERRORTXT = function(_, args) {
		...
		// args[0] : err_type
		var err = this.getHighestErrors(_, args[0])[0];
