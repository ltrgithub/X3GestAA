...
var traceGET = require("syracuse-trace/lib/helper").getTracer("etna.accessorsGet");
var traceSET = require("syracuse-trace/lib/helper").getTracer("etna.accessorsSet");
...
exports.defineClass = function(constructor, parent, members) {
	...
	members.walkToProperty = function(_, path, f) {
	...
	members.AGETTEXTRA = function(_, property) {
		return this.get(_, property);
	};

	members.AGETTEXTRALAN = function(_, property, lan) {
		var slot = this.slot(_, property);
		if (slot) return slot.getAtextra(_, lan);
		return "";
	};

	members.ASETTEXTRALAN = function(_, property, text, lan) {
		var slot = this.slot(_, property);
	...
	members.actionControl = function(_, operation, type) {
		return this.action(_, ("A" + [operation, "CONTROL", type].join('_')).toUpperCase());
	};

	members.actionBeforeControl = function(_, operation) {
		return this.actionControl(_, operation, "BEFORE");
	};

	members.actionAfterControl = function(_, operation) {
	...
	members.AINIT = function(_) {
		traceSup.debug && traceSup.debug("baseInstance.AINIT");
		var self = this;
	...
	members.propertiesEvent = function(_, action) {
		...
		for (var i = 0; i < properties.length; i++) {
			if (!properties[i].FLDGRP) {
				status = Math.max(status, this.propertyInitValue(_, properties[i].FLDCLA));
			}
		}
		return status;
	};
	...
	function addPropertyEvent(action) {
		return function(_, property, oldValue) {
			tracerEvents.debug && tracerEvents.debug(action + " on " + property + " " + this.$$type + " (" + this.$$dbgId + ")");
			...
			while (instance !== undefined) {
				if (instance.runScripts) {
					status = instance.runScripts(_, "$PROPERTIES", args);
				}
	...
	members.propertyInitValue = addPropertyEvent("INIT");
	...
	members.deleteDiagnoses = function(_, etype) {
		var self = this;
	...
	members.$AREAD = function(_) {
		...
		key = key.join('~');
		traceSup.debug && traceSup.debug("baseInstance.AREAD(" + key + ")");
		util.checkInstance(this);
	...
	members.$AINSERT = function(_) {
		traceSup.debug && traceSup.debug("baseInstance.AINSERT(" + this.key + ")");
	...
	members.$AUPDATE = function(_) {
		traceSup.debug && traceSup.debug("baseInstance.AUPDATE(" + this.key + ")");
	...
	members.$ADELETE = function(_) {
		traceSup.debug && traceSup.debug("baseInstance.ADELETE(" + this.key + ")");
	...
	members.updateTexts = function(_) {
		var instance = this;
		Object.keys(this.slots).forEach_(_, function(_, name) {
			var slot = instance.slots[name];
			slot.beforeUpdate && slot.beforeUpdate(_);
		});
		var parent = instance.$parent;
		if (parent && parent.updateTexts) parent.updateTexts(_);
	};
	...
	*/
	...
	members.AGETTEXT = function(_, property) {
		return 0;
	};
	members.ASETTEXT = function(_, property, intit, val) {
		return 0;
	};
	members.AGETTEXTLAN = function(_, property, lan) {
		return 0;
	};
	members.ASETTEXTLAN = function(_, property, intit, val, lan) {
		return 0;
	};
	...
	members.toX3son = function(_) {
		var self = this;
		return "O!C_" + this.class.name + "{" + Object.keys(this.slots).map_(_, function(_, name) {
			return name + ':' + X3SON.stringify(_, self.slots[name]._get(_));
		}).join('') + "};";
	};
