...
exports.defineClass = function(constructor, parent, members) {
	...
	members.applyData = function(_, context, data, path) {
	...
	members.applyDelta = function(_, resourceFactory, delta) {
		// take snapshot before assigning
		this.snapshots();
		var self = this;
		Object.keys(delta).forEach_(_, function(_, tag) {
			...
			if (prop.collection) {
				// see later
				//dbg:console.log("applyDelta " + tag + " is a collection");
				...
				v.forEach_(_, function(_, deltaLine) {
					...
					if (line === null) {
						//dbg:console.log("add Line");
						collection.add(_, deltaLine.$index ? (deltaLine.$index + 1) : constants.CST_ALASTPOS, uuid);
					} else {
						//dbg:console.log("applyDelta to Line " + line.$uuid.x3ToString());
						line.applyDelta(_, prop.targetResourceFactory, deltaLine);
					}
				});

			} else if (!/.*_REF$/.test(name)) { // See later for references :
				//dbg:console.error("applyDelta " + prop.isExtra + " instance.set " + name + " v:" + v);
	...
	members.$AINSERT = function(_) {
		...
		return this.withControl(_, 'Insert', function Insert(_) {
			return this.withTransaction(_, function(_) {
				tracerJs.debug && tracerJs.debug("Insert " + this.key);
	...
	members.$AUPDATE = function(_) {
		...
		return this.withControl(_, 'Update', function Update(_) {
			return this.withTransaction(_, function(_) {
				tracerJs.debug && tracerJs.debug("Update " + this.key);
	...
	members.$ADELETE = function(_) {
		...
		return this.withControl(_, 'Delete', function Delete(_) {
			return this.withTransaction(_, function(_) {
				tracerJs.debug && tracerJs.debug("Delete " + this.key);
