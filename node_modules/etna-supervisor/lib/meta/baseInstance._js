...
exports.defineClass = function(constructor, parent, members) {
	...
	members.set = function(_, name, value, raw) {
		...
		if (s) s._set(_, value, (raw === undefined) ? false : raw);
		else {
			try {
				//External attributes ?
	...
	members.getDiagnoses = function(_, sort) {
		...
		var local = (this.diagnoses || []).map(function(d) {
			d.PRO = "";
			return d;
		});
	...
	*/
	...
	members.getHighestError = function(_, etype) {

		var self = this;
		switch (etype) {
			case constants.CST_ALL:
				return self.getDiagnoses(_, "sortOnSeverity")[0];
			case constants.CST_ACURRENT:
				var err = Object.keys(self.slots).filter(function(pro) {
					...
					return d0.sortOnSeverity(d1);
				});

				return err[0];
			default:
				throw new X3Error(67, "ERR_TYPE");
		}
	},
	...
	members.AGETMAXERROR = function(_) {
		var err = this.getHighestError(_, constants.CST_ACURRENT);
		return err ? err.severity : 0;
	};

	members.AGETMAXERRORALL = function(_) {

		var err = this.getHighestError(_, constants.CST_ALL);
		return err ? err.severity : 0;
	};

	members.AGETERRORSFROM = function(_, iorigin, env) {

		var errc = iorigin.getDiagnoses(_).map(function(e) {
			// clone the error or not ??
			var c = e.clone();
			c.copy = 1;
			c.environment = env;
			return c;
	...
	members.ASEARCHINFOS = function(_) {


		return constants.CST_AOK;
	};

	members.AGETNEXTINFOS = function(_) {
	...
	members.xmetAGETMAXERRORTXT = function(_, args) {
		...
		// args[0] : err_type
		var err = this.getHighestError(_, args[0]);

		if (!err) return constants.CST_ANOREC;

		args[1] = (err.PRO) ? err.PRO : ""; // PROPERTY
		args[2] = err.severity; // ASTATUS
		args[3] = (err.Label) ? err.Label : ""; // LABEL
		args[4] = err.message; // MESSAGE
		args[5] = err.category; // CATEGORY
		args[6] = err.environment; // ENVIRONMENT		
		return constants.CST_AOK;
	};
