...
exports.constructor = util.defineClass(function(superv, table, data) {
	...
	sqlNames: function(_, prefix, fn) {
		fn = fn || function( name ) { return name ;} ;
	...
	sqlNamesSelect : function(_, prefix) {
		...
		{
			case 4 :
				return sqlNames(_, prefix, superv.sqlDriver.decimalCast ) ;
			case 11 :
				return sqlNames(_, prefix, superv.sqlDriver.uuidCast ) ;
			default :
				return sqlNames(_, prefix ) ;
		}
	},
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, raw) {
		...
		if (/^AX[123X]/.test(this.data.CODTYP)) {
			...
			wheres.push(abbr + ".ZONE_0=" + param(params.length));
			params.push(this.supervisor.LAN);
			wheres.push(abbr + ".LANGUE_0=" + param(params.length));
