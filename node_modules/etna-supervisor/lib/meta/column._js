...
exports.constructor = util.defineClass(function(superv, table, data) {
	...
	sqlNames: function(prefix, alias) {
		prefix = prefix || '';
		var fn_alias = function(name) {
			return (alias) ? " As " + alias + "$" + name : "";
		};
		var z;
		var s = prefix + (z = this.data.CODZONE + '_0') + fn_alias(z);
		for (var i = 1; i < this.data.DIME; i++) s += ',' + prefix + (z = this.data.CODZONE + '_' + i) + fn_alias(z);
		return s;
	},

	sqlNamesAssign: function() {
		var z;
		var s = this.data.CODZONE + '_0 = @?';
		for (var i = 1; i < this.data.DIME; i++) s += ',' + prefix + (z = this.data.CODZONE + '_' + i) + " = @?";
		return s;
	},
	...
	getSql: function(_, colNames, tableNames, wheres, params, descs, raw) {
		...
		if (this.type && this.type.data.TYPTYP === 9) {
			...
			colNames.push(this.sqlNames(this.table.abrev + '.'));
