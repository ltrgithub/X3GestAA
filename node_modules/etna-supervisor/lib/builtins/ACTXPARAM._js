...
exports.constructor = util.defineClass(function(superv) {
	...
	addParamCache: function(levcod, code, parameter, value) {
	...
	AGETUSERVALNUM: function(_, parameter) {
		return parseInt(this.getParamUser(_, parameter, "NUM"));
	},
	AGETUSERVALCHAR: function(_, parameter) {
		return this.getParamUser(_, parameter, "CHAR");
	},
	AGETUSERVALDATE: function(_, parameter) {
		return this.getParam(_, parameter, "DATE");
	},
	_leg: function(_) {
	...
	getParam: function(_, levcod, code, parameter, action) {
		...
		return this.addParamCache(levcod, code, parameter, this.getParamBase(_, levcod, code, parameter));
	...
	getParamUser: function(_, parameter, action) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			// read param in ADOPAR to have the definiton
			params = [parameter];
			var wadopar = sqlDriver.execute(_, cnx, "select A.CHAPITRE_0,A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0 from ADOPAR A Where A.PARAM_0=" + param(0), params).map(trim);
			if (wadopar[0] === undefined) throw new HttpError(401, "Incorrect param");
			...
			var wadovalaus = sqlDriver.execute(_, cnx, "select A.CODUSR_0,A.PARAM_0,A.VALEUR_0 from ADOVALAUS A Where A.CODUSR_0=" + param(0) + " and A.PARAM_0=" + param(1), params);
			wadovalaus = wadovalaus.map(trim);
			...
			if (wadopar[0].VALDEF === 2) {
				var profil = "";
				params = [actx.$USER(_)];
				var waus = sqlDriver.execute(_, cnx, "select A.CODMET_0,A.PRFFCT_0 from AUTILIS A Where A.USR_0=" + param(0), params).map(trim);
				if (waus[0] === undefined) throw new HttpError(401, "Incorrect user");
				profil = waus[0].PRFFCT;
				if (profil === "") {
					params = [waus[0].CODMET];
					var wame = sqlDriver.execute(_, cnx, "select A.PRFFCT_0 from AMETUTI A Where A.CODMET_0=" + param(0), params).map(trim);
					if (wame[0] === undefined) throw new HttpError(401, "Incorrect profil");
				}
				if (profil) {
					var module = 0;
					params = [wadopar[0].CHAPITRE];
					var wadi = sqlDriver.execute(_, cnx, "select A.N1_0 from ATABDIV A Where A.NUMTAB_0=901 and A.CODE_0=" + param(0), params).map(trim);
					...
					if (wadi[0].N1 > 0 && wadi[0].N1 <= 20) {
						...
						params = [profil];
						var waft = sqlDriver.execute(_, cnx, "select A.FCYDEF_" + nummdl + " from AFCTFCT A Where A.PRFCOD_0=" + param(0), params).map(trim);
						if (waft[0] !== undefined) {
							value = self.getParam(_, 3, waft[0].FCYDEF, parameter, action);
							if (value !== undefined) return;
						}
						value = self.getParam(_, 1, "", parameter, action);
					}
				}
