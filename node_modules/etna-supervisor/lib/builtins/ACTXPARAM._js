...
exports.constructor = util.defineClass(function(superv) {
	...
	_leg: function(_) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			var wadoval = self.supervisor.sqlReader(_, cnx, "select A.VALEUR_0 from ADOVAL A Where A.CMP_0=' ' and A.FCY_0=' ' and A.PARAM_0='DOSLEG'").toArray(_);
	...
	_cpy: function(_) {
		...
		var sqlDriver = self.supervisor.sqlDriver;
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			self.tabcpy = self.supervisor.sqlReader(_, cnx, "select C.CPY_0 from COMPANY C").toArray(_).reduce(function(wcpy, cpy) {
				wcpy.push(cpy.CPY);
				return wcpy;
			}, []);
		});
	...
	_fcy: function(_) {
		...
		var sqlDriver = self.supervisor.sqlDriver;
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			self.tabfcy = self.supervisor.sqlReader(_, cnx, "select F.FCY_0 from FACILITY F").toArray(_).reduce(function(wfcy, fcy) {
				wfcy.push(fcy.FCY);
				return wfcy;
			}, []);
		});
	...
	_getLegFromCpy: function(_, folder, cpy) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			params = [cpy];
			var wparam = self.supervisor.sqlReader(_, cnx, "select A.LEG_0 from COMPANY A Where A.CPY_0=" + param(0), params).toArray(_);
			if (wparam[0]) {
				value = wparam[0].LEG;
			}
		});
	...
	_getCpyFromSite: function(_, folder, site) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			params = [site];
			var wparam = self.supervisor.sqlReader(_, cnx, "select A.LEGCPY_0 from FACILITY A Where A.FCY_0=" + param(0), params).toArray(_);
			if (wparam[0]) {
	...
	_requestAdovalBase: function(_, levcod, code, parameter) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			...
			var wparam = self.supervisor.sqlReader(_, cnx, "select A.VALEUR_0 from ADOVAL A Where A.CMP_0=" + param(0) + " and A.FCY_0=" + param(1) + " and A.PARAM_0=" + param(2), params).toArray(_);
			if (wparam[0]) {
	...
	_getParamBase: function(_, levcod, code, parameter) {
		var sqlDriver = this.supervisor.sqlDriver;
		var param = sqlDriver.param;
		var params;
		var value;
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			...
			params = [parameter];
			var wadopar = self.supervisor.sqlReader(_, cnx, "select A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0 from ADOPAR A Where A.PARAM_0=" + param(0), params).toArray(_);
	...
	_getParam: function(_, levcod, code, parameter, action) {
		...
		// TODO optimize _getParamBase in order to limit the number of select on ADOPAR 
		return this._addParamCache(levcod, code, parameter, this._getParamBase(_, levcod, code, parameter));
	...
	_getParamUser: function(_, parameter, action) {
		...
		var result = sqlDriver.withConnection(_, function(_, cnx) {
			// read param in ADOPAR to have the definiton
			params = [parameter];
			var wadopar = self.supervisor.sqlReader(_, cnx, "select A.CHAPITRE_0,A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0 from ADOPAR A Where A.PARAM_0=" + param(0), params).toArray(_);
			if (wadopar[0] === undefined) throw new HttpError(401, "Incorrect param");
			...
			var wadovalaus = self.supervisor.sqlReader(_, cnx, "select A.CODUSR_0,A.PARAM_0,A.VALEUR_0 from ADOVALAUS A Where A.CODUSR_0=" + param(0) + " and A.PARAM_0=" + param(1), params);
			...
			if (wadopar[0].VALDEF === 2) {
				var profil = "";
				params = [actx.$USER(_)];
				var waus = self.supervisor.sqlReader(_, cnx, "select A.CODMET_0,A.PRFFCT_0 from AUTILIS A Where A.USR_0=" + param(0), params).toArray(_);
				if (waus[0] === undefined) throw new HttpError(401, "Incorrect user");
				profil = waus[0].PRFFCT;
				if (profil === "") {
					params = [waus[0].CODMET];
					var wame = self.supervisor.sqlReader(_, cnx, "select A.PRFFCT_0 from AMETUTI A Where A.CODMET_0=" + param(0), params).toArray(_);
					if (wame[0] === undefined) throw new HttpError(401, "Incorrect profil");
				}
				if (profil) {
					var module = 0;
					params = [wadopar[0].CHAPITRE];
					var wadi = self.supervisor.sqlReader(_, cnx, "select A.N1_0 from ATABDIV A Where A.NUMTAB_0=901 and A.CODE_0=" + param(0), params).toArray(_);
					...
					if (wadi[0].N1 > 0 && wadi[0].N1 <= 20) {
						...
						params = [profil];
						var waft = self.supervisor.sqlReader(_, cnx, "select A.FCYDEF_" + nummdl + " from AFCTFCT A Where A.PRFCOD_0=" + param(0), params).toArray(_);
						if (waft[0] !== undefined) {
