...
exports.CHECK_EQUAL = function(_, v1, v2, title) {
	if (typeof v1 === "number" && typeof v2 === "number") {
		// Use the same precision that the one used by operators :
		deepEqual( operators.round(v1-v2), 0,title || v1);
	} else if ( bcd.isBCD(v1) && typeof v2 === "number") {
		deepEqual( bcd.toString(v1), ""+ v2, title || v1) ; 
	} else if ( bcd.isBCD(v2) && typeof v1 === "number") {
		deepEqual(  ""+v1, bcd.toString(v2), title || v1) ;
	} else if ( bcd.isBCD(v1) && bcd.isBCD(v2)) {
		deepEqual( bcd.toString(v1) , bcd.toString(v2), title || v1) ;
	} else if (date.isDate(v2) && date.isDate(v1)) {
		deepEqual(  v1.toString("yyyMMdd"), v2.toString("yyyMMdd"), title || v1) ;
	}
	else {
		deepEqual(v1, v2, title || v1);	
	}
}
