...
exports.RUN_TESTSUITE = function(_, title, description) {
	...
	QUnit.module(moduleId, {
		teardown: function() {
			...
			if (isLast) process.nextTick(function() {
				runtime.exit(0);
			});
		},
	});
...
exports.CHECK_EQUAL = function(_, v1, v2, title) {
	...
	if (typeof v11 === "number" && typeof v22 === "number") {
		deepEqual( operators.round(v11-v22), 0,title || v11);
	} else if ( bcd.isBCD(v11) && typeof v22 === "number") {
		deepEqual( bcd.toString(v11), ""+ v22, title || v11) ; 
	} else if ( bcd.isBCD(v22) && typeof v11 === "number") {
		deepEqual(  ""+v11, bcd.toString(v22), title || v11) ;
	} else if ( bcd.isBCD(v11) && bcd.isBCD(v22)) {
		deepEqual( bcd.toString(v11) , bcd.toString(v22), title || v11) ;
...
exports.CHECK_NOTEQUAL = function(_, v1, v2, title) {
    ...
    if ( bcd.isBCD(v11) && typeof v22 === "number") {
    	ok(  (bcd.compare(v11, bcd.fromInt32(v22))===0)?1:0, title || v11) ;
	}else if ( bcd.isBCD(v22) && typeof v11 === "number") {
		ok(  (bcd.compare(bcd.fromInt32(v11), v22)===0)?1:0, title || v11) ;
	} else if ( bcd.isBCD(v11) && bcd.isBCD(v22)) {
		ok(  (bcd.compare(v11, v22)===0)?1:0, title || v11) ;
	} else if (date.isDate(v22) && date.isDate(v11)) {
