...
exports.constructor = util.defineClass(function(superv) {
	...
	get: function(_,name){
		if (this.desactGet)     return this[name].get(_);
		if (this[name])			return this[name].get(_);
		...
		if (dime){
			this[name] = new Value([1,  this.class.properties[name].type.engineType(), [],1,dime-1]); 
		} else {
			this[name] = new Value(this.class.properties[name].type.defaultValue());		
		}
		var trtini;
		...
		switch (name) {
		case 'USER':
			if (typeof this.USER != 'undefined')	    this.USER.set(_,( process.env.USER || "user")) ;
			break ;
		case 'AFOLDER':
			if (typeof this.AFOLDER != 'undefined')	this.AFOLDER.set(_,this.supervisor.folderName);
			break ;
		case 'LAN':
		...
		this.desactGet = null ;
		return this[name].get(_);
	},
	setAttribute: function(_,property,attribut,value) {
		this[property].setAttribute(attribut,value);
	},
	getAttribute: function(_,property,attribut) {
