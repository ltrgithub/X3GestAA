...
var builtinList = ["ASOLUTION", "AFOLDER", "LANISO", "LAN", "LANDEF", "USER", "LOGIN", "CODMET"];
	...
	return {
		...
		$actxSetLOGIN: function(_, login) {
			...
			var rUser = self.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return self.supervisor.sqlReader(_,
					cnx, "select USR_0 ,CODMET_0 from AUTILIS where (LOGIN_0=:1)", [login.toUpperCase()]).limit(1).toArray(_);
			});
			if (rUser[0]) {
				this.$LOGIN(_, login);
				this.$USER(_, rUser[0].USR);
				this.$CODMET(_, rUser[0].CODMET);
				//ADXUSR=LOGIN
...
exports.constructor = function() {
	var construct = util.defineClass(function(superv) {
		...
		this.builtinProperties = {
			...
			CODMET: function(codmet) {
				if (codmet !== undefined) {
					this.CODMET.value = codmet;
				}
				var self = this;
				return {
					getValue: function(_) {
						return self.CODMET.value = self.CODMET.value || "";
					}
				};
			},
