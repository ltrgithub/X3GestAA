...
exports.init = function(_, session, superv) {
	...
	if (sessions[session.id]) {
		...
		tracerJs.debug && tracerJs.debug("load rights " + mysession.actx.$USER(_));
		mysession.actx._x3Rights.getJsonRigths(_, mysession.actx.$USER(_));
...
function _actxmember() {
	return {
		...
		/*
	$LAN: function(_, lan) {
		return this.builtinProperties.LAN(lan).getValue(_);
	},
	$LANDEF: function(_,landef) {
		return this.builtinProperties.LANDEF(landef).getValue(_);
	},
	*/
		...
		ACTX_SET_LANISO: function(_, laniso) {
			...
			var rTablan = self.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return self.supervisor.sqlReader(_,
					cnx, "select LAN_0,LANISO_0 from TABLAN where (LOWER(LANISO_0)=:1)", [laniso.toLowerCase()]).limit(1).toArray(_);
			...
			if (rTablan[0] !== undefined) {
				...
				var rTablan = self.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
					return self.supervisor.sqlReader(_,
						cnx, "select LAN_0,LANISO_0 from TABLAN where (LAN_0=:1)", [self.$LANDEF(_)]).limit(1).toArray(_);
	...
	$LANISO: function(_, laniso) {
		return this.builtinProperties.LANISO(laniso).getValue(_);
	},
	$AFOLDER: function(_,folder) {
		return this.builtinProperties.AFOLDER(folder).getValue(_);
	},
	$ASOLUTION: function(_,solution) {
		return this.builtinProperties.ASOLUTION(solution).getValue(_);
	},
	*/
		$actxSetLOGIN: function(_, login) {
			...
			var rUser = self.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return self.supervisor.sqlReader(_,
					cnx, "select USR_0 from AUTILIS where (LOGIN_0=:1)", [login.toUpperCase()]).limit(1).toArray(_);
			});
			if (rUser[0]) {
				this.$LOGIN(_, login);
	...
	$USER: function(_,user) {
		return this.builtinProperties.USER(user).getValue(_);
	},
	$LOGIN: function(_,login) {
		return this.builtinProperties.LOGIN(login).getValue(_);
	},
	*/
...
exports.constructor = function() {
	var construct = util.defineClass(function(superv) {
					...
					getValue: function(_) {
						...
						var rAdossier = glob.context.x3session.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
							return glob.context.x3session.supervisor.sqlReader(_,
								cnx, "select LANDEF_0 from ADOSSIER where (DOSSIER_0=:1)", [self.AFOLDER().getValue(_)]).limit(1).toArray(_);
						});
		...
		this._x3Rights = new X3Rights(_, superv);
