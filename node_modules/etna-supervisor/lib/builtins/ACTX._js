...
exports.init = function(_, session, superv) {
	...
	if (sessions[session.id]) {
		...
		mysession.actx.$actxInitSysVar(_);

		sessions[session.id] = mysession;

	}
	var self = this;
	// Initialise system variables related to the context :
	Object.keys(mysession.actx.sys).forEach(function(v) {
		glob.context.x3frame.context.sys.values[v] = mysession.actx.sys[v];
	});
};
...
function _actxmember() {
	return {
		...
		setProperty: function(_, name, val) {
			this[name].value = val;
			// TODO : set readonly throw new Error("SET ACTX property " + name);
		},
		getProperty: function(_, name) {
		...
		$actxSetLOGIN: function(_, login) {
			...
			var rUser = self.supervisor.sqlDriver.withConnection(_, function(_, cnx) {
				return self.supervisor.sqlReader(_,
					cnx, "select USR_0 ,CODMET_0, PRFFCT_0, ENAFLG_0, USRCONNECT_0 from AUTILIS where (LOGIN_0=" + self.supervisor.sqlDriver.param(0) + ")", [login.toUpperCase()]).limit(1).toArray(_);
			});
			if (rUser[0]) {
				if (rUser[0].ENAFLG !== 2) {
		...
		$actxInitSysVar: function(_) {
			...
			for (var i = 1; i <= 30; i++) {
				try {
					wparamTUL = this.$APARAM(_).AGETVALCHAR(_, 1, this.$actxFolder, "TUL" + ("00" + i).slice(-2));
					var arTUL = wparamTUL.split(":");
...
exports.constructor = function() {
	...
	['APARAM'].forEach(function(name) {
		Object.defineProperty(proto, "$" + name, {
			value: function(_) {
				return this.getProperty(_, name);
			},
			enumerable: true
		});
	});
