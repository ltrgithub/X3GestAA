...
function _actxLoadContextProp(_,ctx,name){
	...
	if(ctx.builtinProperties && ctx.builtinProperties[name]) {
		...
		if (!recDb) {
			...
			ctx[name] = new InstanceProperty(chapter);
		}
		var wprop = {
				"FLDCLA": name,
		        "CODTYP" : recDb.CODTYP,
		...
		if (dime){
			ctx[name] = new InstanceProperty([1,  ctx.class.properties[name].type.engineType(), [],1,dime-1],ctx); 
		} else {
			ctx[name] = new InstanceProperty(ctx.class.properties[name].type.defaultValue(),ctx);		
		}
		var trtini;
...
var Chapter =  util.defineClass(function(parent,builtinProperties) {
	...
	getValueAttribute:function(_,property,attribut,value) {
...
exports.constructor = util.defineClass(function(superv) {
	...
	this.builtinProperties = {
		...
		USER	: function(){return process.env.USER || "user";},
	};
