...
exports.constructor = util.defineClass(function(superv) {
	...
	get: function(_,name){
		...
		if (dime){
			this[name] = new Value([1,  this.class.properties[name].type.engineType(), [],1,dime-1],this); 
		} else {
			this[name] = new Value(this.class.properties[name].type.defaultValue(),this);		
		}
		var trtini;
		...
		case 'USER':
			if (typeof this.USER != 'undefined'){
				this.USER.value = process.env.USER || "user";
				//set this.USER as readonly ...
				this.USER.$isReadonly  = true;
				// ... and protect the $isReadonly attribute :
				this.USER.$_isReadonly = true;
			}
			break ;
		case 'AFOLDER':
			if (typeof this.AFOLDER != 'undefined')	this.AFOLDER.value = this.supervisor.folderName;
			break ;
		case 'LAN':
