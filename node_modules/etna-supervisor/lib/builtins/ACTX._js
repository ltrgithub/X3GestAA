...
function _actxLoadContextProp(_, ctx, name) {
	...
	if (ctx.builtinProperties && ctx.builtinProperties[name]) {
		...
		var recDb = ctx.supervisor.currentContext(_).loadContextProp(_, name);
		if (!recDb) {
			recDb = ctx.supervisor.currentContext(_).loadContextChapter(_, name);
...
exports.constructor = util.defineClass(function(superv) {
	...
	this._x3Rights = new X3Rights(_, this.supervisor);
	this.isContext = true;
	this.builtinProperties = {
		AFOLDER: function() {
			return {
				getValue: function(_) {
					return superv.AFOLDER(_);
				}
			};
		},
		LAN: function() {
		...
		USER: function() {
			return {
				getValue: function(_) {
					return glob.context.x3frame.context.sys.values.ADXUSR;
				}
			};
		},
		LOGIN: function() {
