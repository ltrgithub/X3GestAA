...
function AQUERY_TRANS_AFTER(_, resource) {
	...
	var codact = instance.getProperty(_, "CODACT");
	if ((instance.getProperty(_, "TYPCLA") !== 2) || (codact && activ.getActiv(_, superv, codact) <= 0)) {
		instance.setProperty(_, "FLGSEARCH", 1);
	} else if (instance.getProperty(_, "FLGSEARCH") === 2) {
		var rs = ez.devices.generic.empty.reader.transform(function(_, reader, writer) {
			...
			CODCLA: instance.getProperty(_, "CODCLA")
		})
