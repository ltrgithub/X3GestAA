...
function tds_connect(cfg, callback) {
	...
	return c.on('connect', function(err) {
		...
		tracer.debug && tracer.debug("connection callback ");
...
function tds_query(cfg, cnx, sql, args, callback) {
	...
	var req = new tds.Request(sql, function(err, rowCount) {
		if (err)
			tracer.debug && tracer.debug("Request end error :" + err);
		else {
			records.updateCount = rowCount;
			tracer.debug && tracer.debug("Request end rowcount :" + rowCount);
		}
...
module.exports = function(config) {
	...
	return {
		...
		execute: function(_, cnx, sql, args) {
			tracer.debug && tracer.debug("Pre Execute state" + cnx.state.name);
			var result = tds_query(cfg, cnx, sql, args, ~_);
			tracer.debug && tracer.debug("Post Execute state" + cnx.state.name);
			return result;
		},

		reader: function(_, cnx, sql, args, ord) {
			tracer.debug && tracer.debug("Pre Reader state" + cnx.state.name);
			var r = eztedious.reader(_, cnx, sql, args, ord);
			tracer.debug && tracer.debug("Post Reader state" + cnx.state.name);
			return r;

		},
		writer: function(_, cnx, sql, columnDefs) {
			return eztedious.writer(cnx, sql, columnDefs);
		},
		...
		readTableSchema: function(_, tableDef, tracker) {
			...
			this.withConnection(_, function(_, cnx) {
				...
				" LEFT JOIN " + _this.getFullTableName(tableDef.schemaName, "ATABZON") + " z ON z.CODFIC_0 = o.name AND c.name LIKE CONCAT(z.CODZONE_0, '[_]%')" +
					" LEFT JOIN " + _this.getFullTableName(tableDef.schemaName, "ATYPE") + " t ON z.CODTYP_0 = t.CODTYP_0" +
					" LEFT JOIN " + _this.getFullTableName(tableDef.schemaName, "ATABLE") + " tbl ON z.CODFIC_0 = tbl.CODFIC_0" +
				...
				_this.reader(_, cnx, sqlQuery).toArray(_).forEach(function(row) {
					...
					tableDef.columns[row.col_name] = column;
					...
					switch (row.col_type) {
						...
						case 108: // numeric
							...
							// i.e. ATABZON.OPTION_0 or ATYPE.OPTION_0 contains the character 'F'
					...
					if (row.zone_option && row.zone_option.trim())
						column.typeOption = row.zone_option.trim();
					else if (row.type_option && row.type_option.trim())
						column.typeOption = row.type_option.trim();
				...
				if (!options.skipCreate) {
					...
					tableDef.columns.forEach(function(colDef) {
						...
						switch (colDef.type) {
							...
							case "decimal":
								var fmt;
								if (colDef.decimalPrecision && colDef.decimalScale)
									fmt = getDecimalFormat(colDef.decimalPrecision - colDef.decimalScale, colDef.decimalScale, colDef.useFloatingPointFormat);
								else
