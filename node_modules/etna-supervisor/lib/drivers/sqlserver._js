...
module.exports = function (config) {
    ...
    var cnxStr = "DRIVER=SQL Server Native Client 11.0;SERVER=" + config.hostname + ";UID=" + config.user + ";PWD=" + config.password + ";DATABASE="+config.database+";AutoTranslate=no;QuotedId=no;AnsiNPW=no;"
    var connections = [];    
    return {
        ...
        withConnection: function (_, body) {
            ...
            if (!cnx) {
                trace && trace("connecting on"+cnxStr+"...");            
                var cnx = sqlserver.open(cnxStr, ~_);
            }
            try {
                return body(_, cnx);
            } finally {
