	...
	return {
		...
		readTableSchema: function(_, tableDef) {
			...
			this.withConnection(_, function(_, cnx) {
				var sql = "SELECT c.name, c.xtype, c.length, c.colid, c.isnullable, c.xprec, c.xscale, c.cdefault " +
					"FROM syscolumns c LEFT JOIN sysobjects o ON c.id = o.id WHERE o.name='" + tableDef.name + "'";
				tableDef.columns = _this.reader(_, cnx, sql).toArray(_).map(function(sql) {
					...
					switch (sql.xtype) {
						...
						case 231: // nvarchar(n)
							...
							break;
						default:
							throw new Error('unknown column type ' + sql.xtype + ' for column ' + tableDef.name + '.' + sql.name);
					}
					return column;
				});
			});
