...
exports.createTableReader = function(_, driver, tableDef, filters) {
	...
	driver.withConnection(_, function(_, cnx) {
		reader = driver.reader(_, cnx, sql);
	});
	return reader;
};
...
exports.createPermissions = function(_, driver, tableDef) {
	...
	// First, we have to query ATABLE to get the SECURE flag of the table
	driver.withConnection(_, function(_, cnx) {
		var folderName = "X3";
		var row = driver.reader(_, cnx, "SELECT SECURE_0 FROM ATABLE WHERE CODFIC_0 = '" + tableDef.tableName + "'").toArray(_)[0];

		var isOpenAccess = !row || (row["SECURE_0"] == 2);
		isOpenAccess = false;
		...
		if (isOpenAccess) {
			...
			// - give all permissions to FolderName_H
			grantPermissions(_, cnx, folderName + "_H");
			...
			// Now, we have to parse the folders chain ...
			var loopCount = 0; // just a security to avoid endless loops
			while (loopCount < 20) {
				...
				var row = driver.reader(_, cnx, "SELECT DOSREF_0 FROM ADOSSIER WHERE DOSSIER_0 = '" + folderName + "'").toArray(_)[0];
				if (!row)
					break;
				if (row['DOSREF_0'] == folderName)
					break;
				folderName = row['DOSREF_0'];
				loopCount++;
			}
		}
	});
