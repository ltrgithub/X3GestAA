...
module.exports = function(config) {
	...
	return {
		...
		getTableDef: function(_, tablename, schemaName) {
			var tableDef = {
				tableName: tablename,
				schemaName: schemaName,
			};
		...
		createTableFromTableDefinition: function(_, tableDef, options) {
			...
			this.withConnection(_, function(_, cnx) {
				...
				if (!options.skipCreate) {
					...
					var sql = "CREATE TABLE " + getFullTableName(tableDef) + " (";
		...
		createTableReader: function(_, tableDef, filters) {
			...
			var sql = "SELECT " + columnNames.join(',') + " FROM " + tableDef.tableName + " T";
