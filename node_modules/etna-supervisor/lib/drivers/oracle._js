	...
	return {
		...
		readTableSchema: function(_, table) {
			...
			this.withConnection(_, function(_, cnx) {
				...
				table.columns = _this.reader(_, cnx, sql, [table.name]).toArray(_).map(function(sql) {
					var column = {
					...
					switch (sql.DATA_TYPE) {
						...
						case 'RAW':
							...
							break;
						default:
							throw new Error('unknown column type ' + sql.DATA_TYPE + ' for column ' + table.name + '.' + column.name);
					}
					return column;
				});
			});
