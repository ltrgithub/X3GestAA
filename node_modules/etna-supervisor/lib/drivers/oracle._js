	...
	return {
		...
		allocConnection: function(_) {
			...
			if (!cnx) {
				...
				if (counters.alloc > 5) throw new Error("TOO MANY CONNECTIONS ALLOCATED");
		...
		withConnection: function(_, body, reuseCnx) {
			var self = this;
			var cnx = reuseCnx || self.allocConnection(_);
			try {
				return body(_, cnx);
			} finally {
				if (!reuseCnx) self.releaseConnection(cnx);
			}
		},
		...
		reader: function(_, cnx, sql, args, opts) {
			...
			opts = opts || {};
			if (opts.stop) throw new Error("internal error: opts.stop != null");
			setFetchInfo(opts);
			if (cnx == null) {
				var self = this;
