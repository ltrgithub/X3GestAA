	...
	return {
		...
		withConnection: function(_, body) {
			...
			if (!cnx) {
				...
				var cnx = oracle.getConnection(config, ~_);
		...
		execute: function(_, cnx, sql, args) {
			tracerJs.debug && tracerJs.debug("SQL: " + sql + ",args=\n" + (args || []).map(function(arg, i) {
				return ":" + (i + 1) + "=" + arg;
			}).join('\n'));
			var result = cnx.execute(sql, args || [], {
		...
		reader: function(_, cnx, sql, args, ord) {
			return ezoracle.reader(cnx, sql, args || [], ord);
		},
		...
		describeQuery: function(_, qry, folder) {
			...
			this.withConnection(_, function(_, cnx) {
				_this.execute(_, cnx, "Create view " + _tmpname + " as " + qry);
			});

			try {
				var desc = this.withConnection(_, function(_, cnx) {
					...
					return _this.reader(_, cnx, sql).toArray(_).map(function(row) {
						var col = {};
				...
				/* delete view*/
				this.withConnection(_, function(_, cnx) {
					_this.execute(_, cnx, "Drop view " + _tmpname);
				});
			}
			return desc;
		},
		...
		readTableSchema: function(_, tableDef, tracker) {
			...
			this.withConnection(_, function(_, cnx) {
				...
				_this.reader(_, cnx, sqlQuery, [tableDef.tableName]).toArray(_).forEach(function(row) {
					...
					tableDef.columns[row.COLUMN_NAME] = column;
					switch (row.DATA_TYPE) {
						case 'DATE':
							column.type = "date";
							break;
					...
					tableDef.columns.forEach(function(colDef, colIdx) {
						...
						switch (colDef.type) {
							...
							case "binary":
								...
								break;
							default:
								throw new Error('unknown column type ' + colDef.type + ' for column ' + _this.getFullTableDefName(tableDef) + '.' + colDef.name);
						}
