...
exports.defineClass = function(constructor, parent, members, options) {
	...
	members.getActionArgs = function(_, action, key, curpro, curpth) {
		var args = {
			THIS: {
				type: "LY",
				value: this
	...
	members._action = function(_, action) {
		tracerEvents.debug && tracerEvents.debug("action:" + action);
		return this.runScripts(_, "$METHODS", this.getActionArgs(_, action));
	};


	members.runScripts = function(_, handler, args) {
		var self = this;
		...
		scripts.forEach_(_, function(_, script) {
			...
			try {
				if (script.body.$isBuiltin) {
					...
					tracerEvents.debug && tracerEvents.debug(new Array(actx.$$dbgScriptLevel).join(' ') + "-> " + script.name + (args.CURPRO !== undefined ? (" CURPRO:" + (args.CURPRO.value || "")) : "") + (args.CURPTH !== undefined ? (" CURPTH:" + (args.CURPTH.value || "")) : ""));
					astatus = runtime.executeProg(_, script, handler, args);
					actx.$$dbgScriptLevel -= 1;
				}
			} catch (e) {
				tracerJs.error && tracerJs.error("exception in util.runScripts:" + e.message);
				tracerJs.error && tracerJs.error(e.stack.split("\\n")[5]);
			}
		}, self);
		return astatus;
	};
