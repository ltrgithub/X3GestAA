...
exports.invokeScripts = function(_, self, action, instance, $curpro, $curpth, values) {
	var curpro = $curpro || "";
	var curpth = $curpth || "";
	...
	self.scripts(_).forEach_(_, function(_, script) {
		if (script.body.$isBuiltin) {
			try {
				...
				astatus = script.body[handler](_, action, instance, curpro, curpth) || 0;
			...
			for (var v = 0; values !== undefined && v < values.length; v++) {
				args[values[v].name] = {
					letter: values[v].letter,
					abrev: values[v].abrev,
					type: values[v].type,
