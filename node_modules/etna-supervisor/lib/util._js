...
exports.defineClass = function(constructor, parent, members, options) {
	...
	members._action = function(_, action) {
		tracerJs.debug && tracerJs.debug("=> " + this.$$type + ".action " + action);
	...
	members.runScripts = function(_, handler, args) {
		...
		//!tracerJs.debug && tracerJs.debug("runScripts:" + JSON.stringify(scripts));

		if (!scripts) return astatus;

		scripts.forEach_(_, function(_, script) {
			// Doesn't execute the next script if status !== CST_AOK
			if (astatus > self.supervisor.constants.CST_AOK) return;

			tracerJs.debug && tracerJs.debug("invoke " + script.$name);
