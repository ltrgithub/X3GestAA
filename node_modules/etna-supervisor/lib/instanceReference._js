...
InstanceReference.prototype._get = function(_) {
	var reference = this.$parent._get(_, this.referenceName);
	if (reference != this.reference) {
		// Load reference's attributes:
		if (this.$parent.class)
		//this._value = this.$parent.class.properties[this.name].loadReference(_, this.$parent.record);
			this._value = this.$parent.class.properties[this.name].loadReference(_, this.$parent);
		else
			this._value = this.$parent.resourceFactory.properties[this.name].loadReference(_, this.$parent);
...
InstanceReference.prototype._set = function(_, value, raw) {
	throw new X3Error(147, (this.name ? this.name : '') + "is readonly");
};

InstanceReference.prototype.fromRecord = function(_, record, colnIndex) {};

InstanceReference.prototype.toRecord = function(record) {};

InstanceReference.prototype.clone = function(parent) {};

InstanceReference.prototype.revertTo = function() {};
