...
Object.defineProperty(InstanceProperty.prototype, "isUndefined", {
	get: function() {
		return this._value === undefined || this._value.x3IsUndefined();
	},
	enumerable: false,
});

Object.defineProperty(InstanceProperty.prototype, "deleteDiagnoses", {
	value: function() {
...
Object.defineProperty(InstanceProperty.prototype, "getDiagnoses", {
	value: function(parent) {
		if (this.diagnoses) {
			var self = this;
			return this.diagnoses.map(function(d) {
				d.PRO = (parent ? parent + '.' : '') + self.name;
				return d;
			});
		}
	},
...
Object.defineProperty(InstanceProperty.prototype, "addDiagnose", {
	value: function(_, severity, message) {
		this.diagnoses = this.diagnoses || [];
		this.diagnoses.push(this.$parent.supervisor.new(_, 'Diagnosis', severity, message));
		return severity;
	},
	enumerable: false,
});
...
Object.defineProperty(InstanceProperty.prototype, "controlFormat", {
	value: function(_) {
		...
		if (fmt) {
			// string
			var formattedValue = format.format(fmt, this.get(_, true));
		...
		var status = 0;
		if (isFormatInvalid) {
			tracerJs.error && tracerJs.error(this.name + " isFormatInvalid $x3Format=" + this.$x3Format);
			tracerJs.error && tracerJs.error(this.name + "          value='" + value + "'");
			tracerJs.error && tracerJs.error(this.name + " formattedValue='" + formattedValue + "'");

			status = this.addDiagnose(_,
				constants.CST_AERROR,
				this.$parent.supervisor.loadMess(_, 100, 149));
...
Object.defineProperty(InstanceProperty.prototype, "controlOthers", {
	value: function(_) {
		if (false) // TODO
			this.addDiagnose(_,
				constants.CST_AERROR,
...
Object.defineProperty(InstanceProperty.prototype, "get", {
	value: function(_, raw) {
		...
		//dbg:console.error("property " + this.name + " GET -> " + this._value + " (" + typeof this._value + "):"+this._value);
		return this._value;
	},
	enumerable: false,
});
...
Object.defineProperty(InstanceProperty.prototype, "propagateValue", {
	value: function(_, oldValue) {
		//dbg:console.error(this.name + " propagate old:'"+oldValue.value+"' new:'"+this._value+"'");
		// 1. bonds ?
		if (this.property.proMaps) {
			var self = this;
			this.property.proMaps.forEach_(_, function(_, property) {
				var slot = self.$parent.slot(_, property);
				if (slot) {
					slot.onPropagate(_, self);
				}
			});
		}
		// 2. Call scripts :
...
Object.defineProperty(InstanceProperty.prototype, "setAsMandatory", {
	value: function(_) {
		this.$isMandatory = true;
		if (this.property.type.data.TYPTYP === 1 && this.property.data.NOLIB) {
