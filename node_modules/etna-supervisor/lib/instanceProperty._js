...
Object.defineProperty(InstanceProperty.prototype, "get", {
	value: function(_, raw) {
		...
		if (!raw && this.property.data && this.property.data.FLGACCGET === 2) {
			...
			//dbg:console.log("DBG property " + this.name + " GET -> this._getCalls:" + this._getCalls);
			if (this._getCalls === 1) this.$parent.propertyGetValue(_, this.name);
			this._getCalls -= 1;
		}
		//dbg:console.log("property " + this.name + " GET -> " + this._value + " (" + typeof this._value + ")");
		return this._value;
	},
	enumerable: false,
});
...
Object.defineProperty(InstanceProperty.prototype, "set", {
	value: function(_, value, raw) {
		//dbg:console.log("property " + this.name + " SET -> " + this._value + " (type :"+ this.property.type.etnaType+ ")");
		...
		if (!raw) {
			...
			//dbg:console.log("DBG property " + this.name + " SET -> this._setCalls:" + this._setCalls);
			if (this._setCalls === 1) {
				// Control the new value:
				//dbg:console.log("DBG property " + this.name + " CONTROL :"+JSON.stringify(oldValue));

				if (!this.$parent.propertyControlValue(_, this.name, oldValue) && this.$parent.propertyPropagetValue && oldValue.value !== this._value) {
					//dbg:console.log(" !!! DBG property " + this.name + " PROPAGATE old:'"+oldValue.value+"' new:'"+this._value+"'");
					// Propagate the change : 
					this.$parent.propertyPropagetValue(_, this.name, oldValue);
				}
			}
			this._setCalls -= 1;
		}
...
Object.defineProperty(InstanceProperty.prototype, "fromRecord", {
	value: function(_, record, colnIndex) {
		//dbg:console.log("InstanceProperty.fromRecord:" + JSON.stringify(record));
...
Object.defineProperty(InstanceProperty.prototype, "toRecord", {
	value: function(record) {
		if (this.column) record[this.column] = this._value;
	},
	enumerable: false,
});
