...
Object.defineProperty(InstanceProperty.prototype, "isFormatInvalid", {
	value: function(_) {
		...
		if (fmt) {
			...
			return menu && menu.length && (this._value < menu[0].$value || this._value > menu[menu.length - 1].$value);
		}
		return false;
	},
	enumerable: false,
...
Object.defineProperty(InstanceProperty.prototype, "areControlsInvalid", {
	value: function(_) {
		return false;
	},
	enumerable: false,
	...
	value: function(_, value, raw) {
		...
		if (!raw) {
			...
			if (this._setCalls === 1) {
				...
				if (this.isFormatInvalid(_)) {
					this.$parent.setStatus(_,
						this.name,
						this.$parent.supervisor.constants.CST_AERROR,
						this.$parent.supervisor.loadMess(_, 100, 149));
				} else if (this.areControlsInvalid(_)) {
					this.$parent.setStatus(_,
						this.name,
						this.$parent.supervisor.constants.CST_AERROR,
						this.$parent.supervisor.loadMess(_, 123, 83));
				} else {
					var status = this.$parent.propertyControlValue(_, this.name, oldValue);

					if (status !== this.$parent.supervisor.constants.CST_AERROR) {
						// Propagate the value : 
