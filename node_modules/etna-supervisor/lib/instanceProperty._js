...
Object.defineProperty(InstanceProperty.prototype, "get", {
	value: function(_, raw) {
		...
		if (!raw && this.property.data && this.property.data.FLGACCGET === 2) {
			...
			if (this._getCalls === 1) this.$parent.x3Properties(_, "GET", this.name);
			this._getCalls -= 1;
		}
	...
	value: function(_, value, raw) {
		...
		if (!raw) {
			...
			if (this._setCalls === 1) {
				...
				if (!this.$parent.x3Properties(_, "CONTROL", this.name, oldValue) && oldValue !== this._value) {
					//dbg:console.log("DBG property " + this.name + " PROPAGATE");
					// Propagate the change : 
					this.$parent.x3Properties(_, "PROPAGATE", this.name, oldValue);
				}
			}
			this._setCalls -= 1;
		}
