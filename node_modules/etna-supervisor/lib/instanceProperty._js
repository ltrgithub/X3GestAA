...
Object.defineProperty(InstanceProperty.prototype, "get", {
	value: function(_, raw) {
		...
		// Call the GET accessor if and only if it is set in the dictionary
		this._gets = this._gets ? this._gets : 1;
		if (!raw && this.property.data && (this.property.data.FLGACCGET === 2) && (this._gets === 1)) {
			this.$parent.class.invokeScripts(_, "GET", this.$parent, this.name); // this.$parent.class.curpath + "." +
		}
		this._gets = -1;
...
Object.defineProperty(InstanceProperty.prototype, "set", {
	value: function(_, value, raw) {
		...
		// TODO : CCL look that when we look the edit mod.
		/*if (!raw) {
			if (this.$parent.class.invokeScripts(_, "$CONTROL", this.$parent,  this.name, ""))return;
			this.$parent.class.invokeScripts(_, "$PROPAGATE", this.$parent,  this.name, "");
		}*/
	},
	enumerable: false,
});
