...
exports.query = function(_, httpContext, request_code, query) {
	...
	if (requid === "" && (forced_exec || ifshared === 3)) {
		...
		var tbsize = 0;
		var tableName = runtime.executeProg(_, mod, "GET_MAINTABLE");
		//TODO:if (actx.$APARAM(_).AGETVALNUM(_, 1, "", "TYPDBA") === 1) {
		var params = [tableName];
		if (1) {
			var rs = superv.executeSql(_, "select num_rows from user_tables where table_name =" + param(0), params);
			tbsize = parseFloat(rs[0].NUM);
		} else {
			var rs = superv.executeSql(_, "select top 1 rows from sysindexes where id = object_id('" + param(0) + "') and indid < 2", params);
			//TODO:?
		}
		tracerJs.debug && tracerJs.debug("tbsize:" + tbsize);
