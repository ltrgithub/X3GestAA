...
asyncTest("replacements: slash, $$, $R ", 3, function(_) {
	var result = p.replacements(" ab/c c\\ab|$$ $$$R$$$/", "REPO");
	strictEqual(result[0], "ab"+path.sep+"c")
	strictEqual(result[1], "c"+path.sep+"ab")
	strictEqual(result.command, "$ $REPO$"+path.sep);
	start();
})

asyncTest("replacements: $1 $S ", 2, function(_) {
	var result = p.replacements(" x y z | $1$3$2$S", "REPO");
	strictEqual(result.join(" "), "x y z")
	strictEqual(result.command, "xzy"+path.normalize(process.cwd()))
	start();
})

// Test for commitData

asyncTest("commitData", 11, function(_) {
	var result;
	result = p.commitData("1234567890123456789012345678901234567890 RELEASE 3234567890123456789012345678901234567890 1 Info 1", true);
	strictEqual(result.rollout, "1234567890123456789012345678901234567890", "rollout hash")
	strictEqual(result.release, true, "is release")
	strictEqual(result.source, "3234567890123456789012345678901234567890", "source hash")
	strictEqual(result.relNumber, "1", "rel number")
	strictEqual(result.comment, "Info 1", "comment")
	result = p.commitData("1234567890123456789012345678901234567890 PATCH 3234567890123456789012345678901234567890 1 Info 1", true);
	strictEqual(result, null, "not found");
	result = p.commitData("1234567890123456789012345678901234567890 PATCH 3234567890123456789012345678901234567890 1 Info 1", false);
	strictEqual(result.rollout, "1234567890123456789012345678901234567890", "rollout hash")
