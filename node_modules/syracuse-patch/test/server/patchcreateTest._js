...
var isFibersFast = require('streamline/lib/globals').runtime == "fibers-fast";
...
function performTest(_, async, orig_module, transformed_module, parameter, consumer, text) {
	var stack_orig;
	try {
		console.error("ERGEBNIS" + async ? orig_module.test(parameter, _) : orig_module.test(parameter));
	} catch (e) {
	...
	var stack_trans;
	try {
		console.error("ERGEBNIS" + async ? transformed_module.test(parameter, _) : transformed_module.test(parameter));
	} catch (e) {
...
asyncTest("analyze stack trace Firefox (partial)", 8, function(_) {
...
asyncTest("source map with uglify", 6, function(_) {
	...
	try {
		...
		performTest(_, false, orig_module, transformed_module, 0, consumer, "ugl parameter 0");
		performTest(_, false, orig_module, transformed_module, 1, consumer, "ugl parameter 1");
...
// test for source maps
isFibersFast && asyncTest("source map with streamline", 6, function(_) {
	...
	try {
		...
		var orig_module = require(tempdir + "/sourcemap0._js");
		...
		fs.writeFile(tempfile, result[0], _);
		tracer && tracer("TRANSFORRM " + result[0]);
		try {
			var transformed_module = require(tempfile);
		} catch (e) {
			tracer && tracer("ZZZ " + e);
		}
		performTest(_, true, orig_module, transformed_module, 0, consumer, "str parameter 0");
		performTest(_, true, orig_module, transformed_module, 1, consumer, "str parameter 1");
...
//test for source maps
isFibersFast && asyncTest("source map with streamline and uglify", 6, function(_) {
	...
	try {
		...
		performTest(_, true, orig_module, transformed_module, 0, consumer, "strugl parameter 0");
		performTest(_, true, orig_module, transformed_module, 1, consumer, "strugl parameter 1");
