...
it('collect files', function(_) {
	...
	try {
		...
		error = true;
		tracer && tracer("No patch content " + e);
	};
...
it('collect files with streamline configuration', function(_) {
	// write some patch files
	try {
		fs.mkdir("patchunittest/" + patchtools.TEMP_DIRECTORY, _);
	} catch (e) {
		tracer && tracer("Cannot create directory " + e);
	}
	writeDummyPatchFile("patchunittest/patch1.dat", 1, 0, 1, 1, false, false, _);
...
it('collect files with streamline configuration over release border', function(_) {
	// write some patch files
	try {
		fs.mkdir("patchunittest/" + patchtools.TEMP_DIRECTORY, _);
	} catch (e) {
