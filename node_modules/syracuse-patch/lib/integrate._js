	...
	if (startIndex === 0 || contents.charAt(startIndex - 1) === '\n') {
		...
		if (endIndex >= 0) {
			...
			if (startIndex > 0) {
				...
				if (endIndex > 0) {
					// tracer("ST " + startIndex + " E " + endIndex + " StS <<<" + contents.substring(startIndex + 16, endIndex) + ">>>");
...
function collectFiles(targetDirectory, filename, currentVersion, currentRelease, _, content) {
	...
	tracer && tracer("cont" + contentAvailable + "curr vers fits " + currentVersionFits + " curr release fits " + currentReleaseFits + " curr vers fits end " + currentVersionFitsEnd);
		...
		if (line.substr(0, 9) === "syracuse ") { // patch meta information
			...
			if (parts[1] === "patch") {
				...
				if (patchOccurred) {
					...
					try {
						...
						tracer("Remove special directory " + e);
					}
					if (!metadataParts) {
						metadataParts = parts.slice(2);
					}
					tracer("Before checkVersions " + util.format(parts) + " metadata " + util.format(metadataParts));
			...
			if (versionInformation) {
				...
				if (parts[1] === "streamline") {
					var r = /\{.*\}/.exec(line);
					tracer("Streamline config change in " + line);
					if (r) {
						tracer("Streamline config change content " + r[0]);
						try {
							versionInformation.streamline = JSON.parse(r[0]);
						} catch (e) {
							tracer("Error during parsing streamline configuration " + e);
						}
					}
				}
	...
	if (versionInformation) {
		...
		if (!versionInformation.streamline) {
			...
			try {
				...
				tracer("No version information available " + e);
			}
		}
