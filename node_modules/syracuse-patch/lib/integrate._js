...
function _write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, access_rights, _) {
	if ((status & LOADED) != 0) {
		...
		tracer("Write file "+targetDirectory+"/"+file_to);		
...
function _checkVersions(parts, _) {
	...
	var oldHash = parts[7];
	var newHash = parts[8];
	var baseDir = write.BASE_DIRECTORY;
	...
	if (!write.exists(write.BASE_DIRECTORY+"/"+write.RELEASE_DIRECTORY, _)) {
		...
		versionInformation = write.readVersionFile(write.BASE_DIRECTORY, _);
		if (!versionInformation.init)
			throw new Error(locale.format(module, "noRelease"))
		tracer("Copy recursive . to "+write.RELEASE_DIRECTORY);
		write.copyRec(write.BASE_DIRECTORY, write.BASE_DIRECTORY+"/"+write.RELEASE_DIRECTORY, false, _);
		tracer("End copy recursive . to "+write.RELEASE_DIRECTORY);
	} else {
		if (oldPatchNumber === "0")
			baseDir = write.BASE_DIRECTORY+"/"+write.RELEASE_DIRECTORY;
	...
	tracer("Fast copy recursive "+baseDir+" to "+write.TEMP_DIRECTORY);
	try {
		write.copyRec(baseDir, write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY, true, _);
		// copy nodelocal.js into temp directory
		var buf = fs.readFile("nodelocal.js", _);
		fs.writeFile(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY+"/nodelocal.js", buf, _);
		...
		tracer("Full copy recursive "+baseDir+" to "+write.TEMP_DIRECTORY);
		write.copyRec(baseDir, write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY, false, _);
	}
	try {
		// delete semaphore file in temp directory
		fs.unlink(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY+"/"+write.SEMAPHORE)
	} catch (e) {
		tracer("Semaphore does not exist in temp directory")
	}
	if (!newPatchNumber) { // extra check when installing new release
		errors = write.checkChecksumsV(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY, _);
...
function clusterPatch(filename, contents, options, _) {
	...
	tracer("Base directory "+write.BASE_DIRECTORY)
	try {
		fs.mkdir(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY);		
	} catch (e) {} // ignore
	var integratePatch = true;
	var currentVersion = write.readVersionFile(write.BASE_DIRECTORY, _);
	var releaseVersion;
	var markIntegration = false; // write into database that new patch is in temp directory
	try {
		releaseVersion = write.readVersionFile(write.BASE_DIRECTORY+"/"+write.RELEASE_DIRECTORY, _);		
	} catch (e) {
		console.log("release directory version file "+e)
	...
	if (filename) {
		...
		var contents = collectFiles(write.BASE_DIRECTORY, filename, currentVersion.commit, releaseVersion.commit, _);
	...
	if (!filename || serverCount > 1) { // patch has to be integrated on several hosts
		...
		if (filename) { // notify other servers
			var opt = { path: "/nannyCommand/notifyOtherNannies/notifyOne/patchIntegration", method: "PUT"};
		...
		if (firstServer !== os.hostname()) {
			...
			console.log("No patch integration: different host is first "+firstServer)
		} else {
			var tempVersion = write.readVersionFile(write.BASE_DIRECTORY+'/'+write.TEMP_DIRECTORY, _);
			console.log("Parts "+patchMetaData+" "+util.format(metadataParts))
			console.log("temp version "+tempVersion.commit+" "+metadataParts[6]);
			if (tempVersion.commit === metadataParts[6]) { // patch already integrated
				integratePatch = false;				
				console.log("No patch integration: patch already integrated")
			}			
		}
	}
	if (integratePatch) {
		options.content = true;
		patch(write.BASE_DIRECTORY, contents, options, _);	
	}
	if (integratePatch || markIntegration) { // write into database that new patch has been integrated in temp directory
		var db = adminHelper.getCollaborationOrm(_);
		var hostData = db.fetchInstance(_, db.model.getEntity("host"), { hostname: os.hostname() });
	...
	if (filename && (integratePatch || markIntegration)) {
		...
		if (config.mockServer) {
			var options = { path: "/nannyCommand/notifyNannies/stopSessions", method: "GET", headers: {}};
...
function _wait(time, callback) {
	setTimeout(callback, time);
}

function collectFiles(targetDirectory, filename, currentVersion, currentRelease, _) {
	...
	var j = patchfiles.length;
	console.log("FFFF "+util.format(patchfiles))
	while (--j >= 0) {
		var file = patchfiles[j];
		console.log("FFF2 "+file)
		var startIndex = 0;
		var metadata = [];
		...
		for (var i=0; i<lines.length; i++) {
			...
			if (i === startIndex && line.substr(0, 15) === 'syracuse patch ') {
				...
				tracer && tracer("Syracuse patch "+line.substr(15));
				console.log("MMMMMMMMMMM"+line.substr(15));
				console.log("MEEEEE "+util.format(metadata))
				console.log("METADATA "+metadata[5]+" "+metadata[6])
				console.log("METADATt "+currentVersion+" "+currentRelease)
	...
	console.log("A "+currentVersionFits+" B "+currentReleaseFits+" C "+currentVersionFitsEnd )
	var startVersion = (currentVersionFits ? currentVersion : (currentReleaseFits ? currentRelease : null));
	console.log("STARTVER "+startVersion)
	if (!startVersion) {
	...
	// fs.writeFile(targetDirectory+"/"+write.TEMP_DIRECTORY+"/lastpatch.bbb", result, _);
	return result;
}
...
function patch(targetDirectory, patchdata, options, _) {
	if (LOGFILE) {
		try {
			console.log("Delete log file "+LOGFILE);
			fs.unlink(LOGFILE, _)
	...
	if (options.tryagain) {
		try {
			...
			var baseDirectory = ".";
			if (errors.length > 0) {
				errors = write.checkChecksumsV(write.RELEASE_DIRECTORY, _);
				tracer("Check checksums in release directory: "+util.format(errors));
				if (errors.length === 0)
					baseDirectory = write.RELEASE_DIRECTORY;
				else
			...
			write.copyRec(baseDirectory, write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY, false, _);
		}
	}
	return _patchIntern(targetDirectory, lines, options, _);
}
...
function _patchIntern(targetDirectory, lines, options, _) {
	...
	for (i=0; i<lines.length; i++) {
		...
		if (line.substr(0, 9) === "syracuse ") { // patch meta information
			...
			if (parts[1] === "patch") {
				...
				targetDirectory = write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY;
			}
		...
		if (line.substr(0, 8) === "literal " || line.substr(0, 6) === "delta ") {
			...
			if (status) {
				...
				tracer("Write binary file "+file_to);
			}
	...
	if (write.exists(write.EXTRA_FUNCTIONS, _)) {
		tracer("Copy "+write.EXTRA_FUNCTIONS+" to "+targetDirectory+"/"+write.EXTRA_FUNCTIONS_OLD)
		...
		if (!("." in checksums))
			checksums["."] = write.getChecksumContent(targetDirectory, ".", _) 
	...
	if (versionInformation) {
		...
		fs.writeFile(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY+"/"+write.VERSION_FILE, JSON.stringify(versionInformation), "utf8", _);
		// start new process
		if ("restart" in options) {
			tracer("Kill node")
			return { kill: function() { write.exchangeProcess(write.BASE_DIRECTORY+"/"+write.TEMP_DIRECTORY, LOGFILE, options.restart ? "PATCH" : "PATCH0"); }};
