...
function patch(targetDirectory, filename, options, _) {
	...
	if (options.tryagain) {
		try {
			...
			write.copyRec(baseDirectory, write.TEMP_DIRECTORY, false, _);
		}
	}
	return _patchIntern(targetDirectory, lines, options, _);
}
...
function _patchIntern(targetDirectory, lines, options, _) {
	...
	if (versionInformation) {
		...
		if ("restart" in options) {
			tracer("Kill node")
			return { kill: function() { write.exchangeProcess(write.TEMP_DIRECTORY, LOGFILE, options.restart ? "PATCH" : "PATCH0"); }};
