...
function _write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, access_rights, _) {
	if ((status & LOADED) != 0) {
		...
		if (!write.equal_sha1(sha1, sha1_to)) {
			...
			throw new Error("Wrong check sum for "+file_to+" expected "+sha1_to);
		}
...
function _checkVersions(parts, _) {
	...
	if (versionInformation.commit !== oldHash) 
		throw new Error("Base version mismatch. Expected "+versionInformation.relNumber+"-"+versionInformation.patchNumber+" ("+versionInformation.comment+"), actual "+oldRelNumber+"-"+oldPatchNumber);
	...
	if (!newPatchNumber) { // extra check when installing new release
		...
		if (errors.length > 0)
			throw new Error("Inconsistencies in temp directory when installing new release");
	}
...
// apply a patch
// targetDirectory must be "." if syracuse metadata are present
// options: tryagain: make full copy if there are errors during patch integration
//          restart: restart node after integrating the patch
		...
		try {
			...
			if (errors.length > 0) {
				...
				if (errors.length === 0)
					...
					throw new Error("Patch cannot be integrated because directories are corrupt");				
			}
...
function _patchIntern(targetDirectory, lines, options, _) {
	...
	for (i=0; i<lines.length; i++) {
		...
		if (line.substr(0, 6) === 'index ') {
			r = /^index ([0-9a-f]+)\.\.([0-9a-f]+)(?:\s+(\d+))?/.exec(lines[i]);
			if (r) {			
				sha1_from = r[1];
				sha1_to = r[2];
				var tmp_rights = r[3] || "";
