...
function collectFiles(targetDirectory, filename, currentVersion, currentRelease, _, content) {
	...
	while (--j >= 0) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (i === startIndex + 1 && line.substr(0, 22) === 'syracuse patchcomment ') metadata[7] = line;
			...
			if (line.substr(0, 8) === '7,"ASR",') {
				...
				if (metadata.length > 7) {
					metadata[8] = lines.slice(startIndex + startContent, i).join("\n");
	...
	header += metadata[2] + " " + metadata[3] + " " + metadata[4] + " " + startVersion + " " + metadata[6];
	header += "\n" + metadata[7] + "\n";
	var streamlineConfig = metadata[9];
	var result = "";
	var tempVersion = startVersion;
	while (tempVersion != lastVersion) {
		var metadata = patchContents[tempVersion];
		if (metadata[9]) streamlineConfig = metadata[9];
		result += metadata[8] + "\n";
