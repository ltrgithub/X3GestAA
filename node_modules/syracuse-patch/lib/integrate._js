...
function _write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, access_rights, _) {
	if ((status & LOADED) != 0) {
		...
		if (!patchtools.equal_sha1(sha1, sha1_to)) {
			tracer && tracer("Wrong check sum for " + file_to + " expected " + sha1_to);
			throw new Error(locale.format(module, "wrongChecksum", file_to));
		}
		tracer && tracer("Write file " + targetDirectory + "/" + file_to + " access rights " + access_rights);
...
function _checkVersions(parts, _, lastHash) {
	...
	tracer && tracer("checkVersions: Data: oldHash " + oldHash + " old release number " + oldRelNumber + " new hash " + newHash + " new release number " + newRelNumber);
	// consistency checks
	...
	if (!patchtools.exists(patchtools.BASE_DIRECTORY + "/" + patchtools.RELEASE_DIRECTORY, _)) {
		tracer && tracer("No release directory available");
		...
		tracer && tracer("Copy recursive . to " + patchtools.RELEASE_DIRECTORY);
		patchtools.copyRec(patchtools.BASE_DIRECTORY, patchtools.BASE_DIRECTORY + "/" + patchtools.RELEASE_DIRECTORY, false, _);
		tracer && tracer("End copy recursive . to " + patchtools.RELEASE_DIRECTORY);
	} else {
	...
	tracer && tracer("Version information " + util.format(versionInformation));
	...
	tracer && tracer("Fast copy recursive " + baseDir + " to " + patchtools.TEMP_DIRECTORY);
	try {
		...
		tracer && tracer("Error in fast copy " + e);
		var errors = patchtools.checkChecksumsV(baseDir, _);
		if (errors.length > 0) {
			tracer && tracer("Errors in checkChecksumsV " + errors.join(", "));
			throw new Error(locale.format(module, "invalidBaseDirectory", baseDir));
		}
		// base directory OK. Therefore full copy
		tracer && tracer("Full copy recursive " + baseDir + " to " + patchtools.TEMP_DIRECTORY);
	...
	if (!newPatchNumber) { // extra check when installing new release
		...
		tracer && tracer("Check temp directory " + errors.join(";"));
		if (errors.length > 0) throw new Error(locale.format(module, "inconsistenciesTemp"));
	}
	tracer && tracer("End checkVersion " + util.format(versionInformation));
	...
	if (startIndex === 0 || contents.charAt(startIndex - 1) === '\n') {
		...
		if (endIndex >= 0) {
			...
			if (startIndex > 0) {
				...
				if (endIndex > 0) {
					// tracer && tracer("ST " + startIndex + " E " + endIndex + " StS <<<" + contents.substring(startIndex + 16, endIndex) + ">>>");
...
function clusterPatch(filename, contents, options, _) {
	...
	tracer && tracer("Base directory " + patchtools.BASE_DIRECTORY);
	try {
	...
	tracer && tracer("Current version " + util.format(currentVersion) + " " + util.format(releaseVersion));
	if (filename) {
	...
	if (!filename || serverCount > 1) { // patch has to be integrated on several hosts
		...
		if (filename) { // notify other servers
			var opt = {
				path: "/nannyCommand/notifyOtherNannies/notifyOne/patchIntegration",
				method: "PUT",
		...
		if (firstServer !== os.hostname()) {
			...
			tracer && tracer && tracer("No patch integration: different host is first " + firstServer);
		} else {
			var tempVersion = patchtools.readVersionFile(patchtools.BASE_DIRECTORY + '/' + patchtools.TEMP_DIRECTORY, _);
			tracer && tracer && tracer("Meta data parts " + patchMetaData + " " + util.format(metadataParts));
	...
	if (filename && (integratePatch || markIntegration)) {
		...
		require('syracuse-session/lib/dispatcher').notifyEnd(_);
		if (config.mockServer) {
			var options = {
...
exports.batchIntegration = function(_, filename) {
	...
	if (!contents) {
		console.error(locale.format(module, "integrated"));
		return 10;
	}
...
function normalPatch(targetDirectory, patchdata, options, _, metadataParts) {
	...
	if (options.tryagain) {
		try {
			...
			tracer && tracer("Exception when integrating patch: " + e);
			// exception when integrating patch: maybe temp directory is corrupt. Full copy
			var errors = patchtools.checkChecksumsV(".", _);
			tracer && tracer("Check checksums: " + util.format(errors));
			var baseDirectory = ".";
			if (errors.length > 0) {
				errors = patchtools.checkChecksumsV(patchtools.RELEASE_DIRECTORY, _);
				tracer && tracer("Check checksums in release directory: " + util.format(errors));
				if (errors.length === 0) baseDirectory = patchtools.RELEASE_DIRECTORY;
				else throw new Error(locale.format(module, "corruptDirectories"));
			}
			tracer && tracer("Second attempt after full copying directory " + baseDirectory + " into temp directory");
...
function _patchIntern(targetDirectory, lines, options, _, metadataParts) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (line.substr(0, 9) === "syracuse ") { // patch meta information
			...
			tracer && tracer("T " + util.format(parts));
			...
			if (parts[1] === "patch") {
				...
				if (patchOccurred) {
					...
					try {
						...
						tracer && tracer("Remove special directory " + e);
					}
					if (!metadataParts) {
						metadataParts = parts.slice(2);
					}
					tracer && tracer("Before checkVersions " + util.format(parts) + " metadata " + util.format(metadataParts));
				...
				tracer && tracer("Set targetDirectory");
			...
			if (versionInformation) {
				...
				if (parts[1] === "streamline") {
					var r = /\{.*\}/.exec(line);
					tracer && tracer("Streamline config change in " + line);
					if (r) {
						tracer && tracer("Streamline config change content " + r[0]);
						try {
							versionInformation.streamline = JSON.parse(r[0]);
						} catch (e) {
							tracer && tracer("Error during parsing streamline configuration " + e);
						}
					}
				}
		...
		if (line.substr(0, 8) === "literal " || line.substr(0, 6) === "delta ") {
			...
			if (status) {
				...
				tracer && tracer("Write binary file " + file_to + " access rights " + access_rights);
			}
				...
				switch (m[1]) {
					case ' ':
						...
						if (remainingExistingLines < 0 || remainingTargetLines < 0) {
							tracer && tracer("Too many lines in diff area at " + lines[i]);
							status = 0;
						} else if (line !== file_lines[cur_line]) {
							tracer && tracer("Line mismatch: >" + line + "< expected: >" + file_lines[cur_line] + "<");
							status = 0;
						}
						cur_line++;
						break;
					...
					case '+':
						...
						if (remainingExistingLines < 0) {
							tracer && tracer("Too many lines deleted at " + lines[i]);
							status = 0;
						} else {
					...
					case '-':
						...
						remainingExistingLines--;
						if (remainingExistingLines < 0) {
							tracer && tracer("Too many lines deleted at " + i + " " + lines[i]);
							status = 0;
						} else if (line !== file_lines[cur_line]) {
							tracer && tracer("Line mismatch: >" + line + "< expected: >" + file_lines[cur_line] + "<");
							status = 0;
		...
		if (line.substr(0, 10) === 'diff --git') {
			...
			if (m = /^diff --git a\/(.*) b\/\1$/.exec(line)) {
				...
				status = 0;
				tracer && tracer('wrong file names in ' + line);
			}
			if (status) {
				var parent = path.dirname(file_from);
				if (!(parent in checksums)) checksums[parent] = patchtools.getChecksumContent(targetDirectory, parent, _);
		...
		if (line.substr(0, 6) === 'index ') {
			...
			if (r) {
				...
				status = 0;
				tracer && tracer('no index available for ' + file_from);
			}
		}
...
function _lastActions(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, access_rights, versionInformation, _) {
	...
	if (patchtools.exists(patchtools.EXTRA_FUNCTIONS, _)) {
		tracer && tracer("Copy " + patchtools.EXTRA_FUNCTIONS + " to " + targetDirectory + "/" + patchtools.EXTRA_FUNCTIONS_OLD);
	...
	if (versionInformation) {
		...
		tracer && tracer("Write version information " + JSON.stringify(versionInformation));
		if (!versionInformation.streamline) {
			...
			try {
				...
				tracer && tracer("No version information available " + e);
			}
		}
