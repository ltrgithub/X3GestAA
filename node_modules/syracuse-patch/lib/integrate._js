...
function patch1(filename, contents, options, _) {
	...
	if (filename) {
		...
		var contents = collectFiles(write.BASE_DIRECTORY, filename, _);
	...
	if (!filename || serverCount > 1) { // patch has to be integrated on several hosts
		...
		if (firstServer !== os.hostname()) {
			...
			console.log("temp version "+tempVersion.commit+" "+patchMetaData[5]);
			if (tempVersion.commit === patchMetaData[5]) { // patch already integrated
				integratePatch = false;
