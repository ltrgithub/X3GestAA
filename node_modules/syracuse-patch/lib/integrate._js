...
function _write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, _) {
	if ((status & LOADED) != 0) {
...
function _patchIntern(targetDirectory, lines, options, _) {
	...
	for (i=0; i<lines.length; i++) {
		...
		if (line.substr(0, 5) === "FILE ") {
			_write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, _);				
			m = /^FILE (\w) \"(.*?)\" \"(.*?)\" (\w*)\-(\w*)/.exec(line);
			if (m) {
				sha1_from = m[4];
				sha1_to = m[5];
				file_from = m[2];
				file_to = m[3] || file_from;
				status = DIFF_ENTRIES;
				if (m[1] === "N") {
					file_from = "";
					file_lines = [];
