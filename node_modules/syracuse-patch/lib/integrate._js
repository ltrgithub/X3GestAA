...
function _checkVersions(parts, _) {
	...
	try {
		...
		fs.writeFile(patchtools.BASE_DIRECTORY+"/"+patchtools.TEMP_DIRECTORY+"/nodelocal.js", buf, _);
	...
	try {
		...
		fs.unlink(patchtools.BASE_DIRECTORY+"/"+patchtools.TEMP_DIRECTORY+"/"+patchtools.SEMAPHORE)
	} catch (e) {
...
function clusterPatch(filename, contents, options, _) {
	...
	try {
		fs.mkdir(patchtools.BASE_DIRECTORY+"/"+patchtools.TEMP_DIRECTORY);		
	} catch (e) {} // ignore
	...
	if (integratePatch) {
		...
		patch(patchtools.BASE_DIRECTORY, contents, options, _);	
	}
...
function _patchIntern(targetDirectory, lines, options, _) {
	...
	if (patchtools.exists(patchtools.EXTRA_FUNCTIONS, _)) {
		...
		fs.writeFile(targetDirectory+"/"+patchtools.EXTRA_FUNCTIONS_OLD, extraOld, _);
	...
	if (versionInformation) {
		...
		fs.writeFile(patchtools.BASE_DIRECTORY+"/"+patchtools.TEMP_DIRECTORY+"/"+patchtools.VERSION_FILE, JSON.stringify(versionInformation), "utf8", _);
	}
	return {};
}
