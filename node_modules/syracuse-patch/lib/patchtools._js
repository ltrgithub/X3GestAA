...
function writeFile(targetFile, content, _) {
	// tracer && tracer("WRITEFILE "+targetFile)
	try {
...
function waitfunction(_) {
	tracer && tracer("Start of wait function");
	...
	try {
		...
		tracer && tracer("vote " + localVersion);
		var result = {
			timestamp: timestamp,
			data: data,
			version: version,
		...
		tracer && tracer("VOTES " + util.format(hosts));
		...
		tracer && tracer("Highest version " + highestVersion);
		...
		hosts.forEach(function(host) {
			...
			var patchStatus = host.patchStatus || "";
			tracer && tracer("Host " + host.hostname + " patchStatus " + patchStatus);
			var parts = patchStatus.split(";");
			if (!host.deactivated && parts[0] === highestVersion && parts[1] && !parts[2]) {
				var otherTimeStamp = parts[1] * 1;
				tracer && tracer("Timestamps " + firstTimeStamp + " " + otherTimeStamp + " diff " + (firstTimeStamp - otherTimeStamp));
				if (firstTimeStamp - otherTimeStamp < ballot.EXPIRE_TIME && (otherTimeStamp < firstTimeStamp || (otherTimeStamp === firstTimeStamp && host.hostname < firstServer))) {
					...
					tracer && tracer("Server changed " + firstServer);
				}
		...
		if (relNumberCmp(localVersion, highestVersion) < 0) {
			tracer && tracer("Local version (" + localVersion + ") is less than highest version: " + highestVersion);
			process.exit(0);
			return null;
		}
		if (localHostData.deactivated) {
			tracer && tracer("Local host is inactive");
			process.exit(0);
			return null;
		}
		if (firstServer === localhostname) {
			...
			tracer && tracer("Winner of database voting");
			if (stillStartedServer) {
				tracer && tracer("Wait some seconds to allow other servers to stop");
				ballot.wait(5000, _);
			}
			result.winner = true;
		} else {
...
function _polling(hostsCollection, firstServer, searchString, _) {
	tracer && tracer("Poll for server " + firstServer + " and '" + searchString + "'");
	// wait for first server (polling)
	var dbDriver = require('syracuse-load/lib/dbDriver');
	var ballot = require('syracuse-patch/lib/ballot');
	...
	while (totalPolling >= 0) {
		...
		tracer && tracer("Polling finished? " + finished);
		if (finished) return true;
	}
	tracer && tracer("Polling exceeded");
...
exports.finalIntegration = function(_) {
	tracer && tracer("Final patch integration " + util.format(waitData));
	if (!waitData) return false; // no waitData: do nothing
	var dbDriver = require('syracuse-load/lib/dbDriver');
	var db;

	function _markEnd(suffix, _) {
		tracer && tracer("Mark end " + suffix);
	...
	try {
		...
		if (waitData.vote) {
			...
			if (firstServer !== localhostname) { // wait until first server has finished (polling)
				...
				tracer && tracer("Target version " + util.format(targetVersion));
		...
		if (copying) {
			...
			while (i < newActions.length) {
				...
				if (parts.length != 2) tracer && tracer("Invalid action " + action);
				else {
					tracer && tracer("Perform action " + action);
					var mod = require(exports.BASE_DIRECTORY + "/" + parts[0]);
					if (parts[1] in mod) {
						var result = mod[parts[1]](_);
						tracer && tracer("Action " + action + " performed, result " + result);
					} else {
						tracer && tracer("Action " + action + " not available");
					}
				}
			}
			...
			if (version.patchNumber === "0") {
				...
				tracer && tracer("Special directory available");
		...
		if (checksums) {
			...
			if (exists(checksumsFile, _)) {
				try {
					...
					tracer && tracer("Error when deleting checksum file " + e);
				}
			}
		}
