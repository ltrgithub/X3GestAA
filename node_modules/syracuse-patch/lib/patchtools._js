...
function checkChecksumsAll(_) {
	...
	if (exists(exports.BASE_DIRECTORY+"/"+exports.RELEASE_DIRECTORY, _))
		future[1] = checkChecksumsV(exports.BASE_DIRECTORY+"/"+exports.RELEASE_DIRECTORY, void _);
	if (exists(exports.BASE_DIRECTORY+"/"+exports.TEMP_DIRECTORY, _))
		future[2] = checkChecksumsV(exports.BASE_DIRECTORY+"/"+exports.TEMP_DIRECTORY, void _);
	var result = { work: _output(future[0], _), rel: _output(future[1], _), temp: _output(future[2], _)};
	return result;
}
	...
	while (list && directory !== "" && list.length === 0) {
		if (checksums) {
			...
			if (exists(checksumsFile, _)) {
				...
				} catch (e) { console.log("Error when deleting checksum file "+e)}		
			}			
		}
