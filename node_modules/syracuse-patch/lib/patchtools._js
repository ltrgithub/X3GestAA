...
// chmod 666 for file before deleting/writing
var setWritePermissions = false;
...
function writeFile(targetFile, content, _) {
	...
	if (setWritePermissions) {
		try {
			fs.chmod(targetFile, '666', _);
		} catch (e) { // ignore 
			console.log("Error chmod "+e)
		}
	}
	try {
...
function _copyContents(start, target, exclude, fast, _) {
	...
	while (--i >= 0) {
		...
		if (fs.stat(startFile, _).isDirectory()) { // start is directory: copy recursively
			...
			var permissions = (stat.mode*1 & 07777).toString(8);
