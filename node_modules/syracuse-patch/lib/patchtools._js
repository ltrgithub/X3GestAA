...
function _copyContents(start, target, exclude, fast, _) {
	...
	while (--i >= 0) {
		...
		if (exclude && (filename in exclude || filename.lastIndexOf('.bbb') === filename.length-4))
...
// returns object with database instance, voting time, voting data in case of success, null in case that the current server has not correct version
function waitfunction(_) {
	...
	var result = {db: db, timestamp: timestamp, data: data, hostsCollection: hostsCollection, version: version, winner: false};
	dbDriver.update(hostsCollection, {hostname: localhostname}, {patchStatus: localVersion+";"+timestamp}, _);
	console.log("vote2"+util.format(Object.keys(ballot)))
	ballot.wait(1000, _);
	...
	hosts.forEach(function(host) {
		...
		if (host.active && relNumberCmp(parts[0], highestVersion) > 0) {
	...
	hosts.forEach(function(host) {
		...
		var patchStatus = host.patchStatus || ""; 
		var parts = patchStatus.split(";");
		if (host.active && parts[0] === highestVersion) {
			...
			if (highestVersion === parts[0] && host.started) stillStartedServer = true;
		}
	});
	if (!localHostData) throw new Error("noLocal$#$Local host does not occur in hosts table "+localhostname);
	if (!firstServer) throw new Error("noWinner$#$The system could not find a first server. Code installation is not OK.");
	...
	if (firstServer === localhostname) {
		...
		if (stillStartedServer) ballot.wait(5000, _);
		result.winner = true;
	} else {
		// this is not the server which has voted first
		// wait for first server (polling)
		_polling(result, firstServer, ";FIRST", _);
	}
...
function _polling(waitData, firstServer, searchString, _) {
	// wait for first server (polling)
	var ballot = require('syracuse-patch/lib/ballot');
	...
	while (totalPolling >= 0) {
		...
		var hosts = waitData.dbDriver.find(waitData.hostsCollection, {hostname: firstServer}, _);
		var finished = false;
		hosts.forEach(function(host) {
	...
	throw new Error("firstTooLong$#$First server takes too much time to finish patch integration - giving up");	
}
...
exports.patchintegration = function (_) {
	var ballot = require('syracuse-patch/lib/ballot');
	var dbDriver = require('syracuse-load/lib/dbDriver');
	if (!waitData) return false; // no waitData: version does not match: do nothing
	var version = waitData.version;
	if (waitData.winner) { // the winner of the database voting notifies about end of database update
		var patchStatus = version.relNumber+"."+version.patchNumber+";"+waitData.timestamp+";FIRST";
		dbDriver.update(waitData.hostsCollection, {hostname: localhostname}, {patchStatus: patchStatus}, _);		
	}
	...
	if (firstServer !== localhostname) { // wait until first server has finished (polling)
		_polling(waitData, firstServer, ";END", _);
		return true;
	}
	...
	var oldActions;
	var newActions;
	if (exports.exists(exports.BASE_DIRECTORY+"/"+exports.EXTRA_FUNCTIONS_OLD, _))
		oldActions = fs.readFile(exports.BASE_DIRECTORY+"/"+exports.EXTRA_FUNCTIONS_OLD, "utf8", _).split(/\r\n|\r|\n/);
	else
		oldActions = [];
	if (exports.exists(exports.BASE_DIRECTORY+"/"+exports.EXTRA_FUNCTIONS, _))
		newActions = fs.readFile(exports.BASE_DIRECTORY+"/"+exports.EXTRA_FUNCTIONS, "utf8", _).split(/\r\n|\r|\n/);
	else
		newActions = [];
	var oldActionsHash = {};
	...
	// mark as finished
	var patchStatus = version.relNumber+"."+version.patchNumber+";"+waitData.timestamp+";END";
	dbDriver.update(waitData.hostsCollection, {hostname: localhostname}, {patchStatus: patchStatus}, _);
	return true;
}
