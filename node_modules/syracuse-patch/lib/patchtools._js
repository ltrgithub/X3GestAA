...
function waitfunction(_) {
	...
	try {
		...
		var result = {timestamp: timestamp, data: data, version: version, winner: false, vote: true};
		...
		if (firstServer === localhostname) {
			...
			_polling(hostsCollection, firstServer, ";END", _);
		}
...
function _polling(hostsCollection, firstServer, searchString, _) {
	...
	while (totalPolling >= 0) {
		...
		var hosts = dbDriver.find(hostsCollection, {hostname: firstServer}, _);
		var finished = false;
		hosts.forEach(function(host) {
...
exports.finalIntegration = function (_) {
	console.log("Final patch integration "+waitData);
	...
	try {
		...
		if (waitData.vote) {
			...
			if (waitData.winner) { // the winner of the database voting notifies about end of database update
				...
				dbDriver.update(hostsCollection, {hostname: localhostname}, {patchStatus: patchStatus}, _);		
			}
			...
			if (firstServer !== localhostname) { // wait until first server has finished (polling)
				_polling(hostsCollection, firstServer, ";END_END", _);
				return true;
			}
		...
		if (waitData.vote) {
			...
			var patchStatus = version.relNumber+"."+version.patchNumber+";"+waitData.timestamp+";END_END";
			dbDriver.update(hostsCollection, {hostname: localhostname}, {patchStatus: patchStatus}, _);			
		}
		return true;		
	} finally {
