...
exports.dispatcher = function(config) {
	var routes = {
		integration: function(_, request, response) {
			...
			if (path === "/batch") {
				try {
					var result = require('@sage/syracuse-lib/src/collaboration/entities/patch').batchIntegration(patchdata, _);
					answer = result;
				} catch (e) {
					response.writeHead(500, {
						"content-Type": "text/plain"
				...
				require('@sage/syracuse-lib/src/collaboration/entities/patch').setLock(_, false);
				answer = "Unlocked";
			} else {
				require('./integrate').clusterPatch(null, patchdata, {}, _);
		...
		consistency: function(_, request, response) {
			var result = require('./patchtools').checkChecksumsAll(_);
