...
var os = require('os');
var encrypter = require('syracuse-lic/lib/encrypt');
...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, globalConfig, _) {
	...
	if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
		...
		if (newRelease && latestPatch) {
			...
			_execute('git checkout -f R'+relNumberNew, globalConfig.patch.rolloutRepo, _);
		}
	...
	for (var i=0; i<gitDiff.length; i++) {
		...
		if (/^(literal|delta) /.test(line)) {
			...
			if (!binary) {
				...
				if (i < gitDiff.length-1 && gitDiff[i+1].test(/^[A-Za-y]\S+$/))
					i++;
				continue;				
			}
		}
		...
		if (line.charAt(0) === '+' && line.length > 251) { // split very long lines
			var i = 1;
			while (i+250 < line.length) {
				target.push("="+line.substr(i, 250));
			    i += 250;
			}
			target.push("+"+line.substr(i));
			continue;
		}
		target.push(line);
	}
...
function _applyRule(rulename, filename, mode, config, _) {
	if (rulename === "ignore")
		return;
	var targetFileName = filename;
	if (rulename === "encrypt" || rulename === "hide") // change file name when encrypting
		targetFileName = encrypter.encryptName(filename);
	else if (rulename === "uglify" || rulename === "streamline") // change streamline file name when uglifying
		targetFileName = filename.replace(/\._js$/, ".js");
	...
	if (mode === "D") { // delete and mark file for commit
		...
		var buff = fs.readFile(write.BASE_DIRECTORY+"/"+filename, _);
		if ((rulename === "uglify" || rulename === "encrypt" || rulename === "streamline" || rulename === "hide") && /\._?js$/.test(filename)) {
			...
			if (rulename !== "streamline") {
			    // 1. parse
			    var toplevel = null;
			    toplevel = uglifyjs.parse(code, { filename: filename });
			...
			buff = new Buffer(code, "utf8");
				...
				tracer("Exception when preprocessing "+filename+": "+e);
			}
			if (rulename === "encrypt" || rulename === "hide")
				buff = encrypter.encrypt(filename, buff, rulename === "hide");
		} 		
...
function _collect(oldCommit, newCommit, config, _) {
	...
	// has configuration file been changed?
	if (diff.indexOf('\tnode_modules/syracuse-patch/lib/'+CONFIG_FILE) >= 0) {
		// delete all .js, ._js, .jsc files within node_modules directory:
		tracer("Configuration file has changed. Delete JavaScript files in roll-out repository");
		write.rmdirRec(config.rolloutRepo, _, /\._?jsc?$/);
		diff = _execute('git diff --name-status '+FIRST_COMMIT+' '+newCommit, write.BASE_DIRECTORY, _)
	}
	var oneTimeActions = executeDiff(diff, _applyRule, config, _);
