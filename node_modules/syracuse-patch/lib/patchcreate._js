...
function createPatch(track, newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _) {
	...
	try {
		...
		if (!patchtools.exists(globalConfig.patch.rolloutRepo, _))
		...
		if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
			...
			if (!globalConfig.patch.testMode && checkSource && _execute('git status -s', patchtools.BASE_DIRECTORY, _))
			...
			var sourceCommitCurrent = _execute('git log -1 --pretty=format:%H', patchtools.BASE_DIRECTORY, _);
			...
			// commit the changes
			track("Commit changes", "git commit", 50);
			try {
		...
		track("Create patch file", "git diff", 80);
		if (!patchfile)
		...
		// remove 
		track("Create patch file", "process git diff", 90);
		shrink(target, gitDiff);
		track("Create patch file", "write file", 95);
		if (x3Information) {
...
function createCustomerImage(track, patchConfig, _, release, releaseHash, releaseComment) {
	...
	if (!patchtools.exists(patchConfig.rolloutRepo, _))
	...
	try {
		...
		track && track("Create customer image", "", 60);
		...
		if (patchtools.exists(patchConfig.customerImage, _)) {
			track && track("Create customer image", "Delete existing customer image", 60)
		...
		track && track("Create customer image", "clone repository", 65);
		_execute('git clone -l -s -c core.eol=lf -c core.autocrlf=input --branch T'+release+' '+patchConfig.rolloutRepo+' '+patchConfig.customerImage, patchtools.BASE_DIRECTORY, _);
		...
		track && track("Create customer image", "write checksums", 75);
		...
		track && track("Create customer image", "double check checksums", 80);
...
function _execute(cmdline, directory, _) {
	...
	if (r = pattern.exec(line)) {
		...
		while (i < config.transformationRules.length) {
			...
			if (rule[0].test(filename)) {
				// changes in filename
				if (!/\._?js$/.test(filename) && (ru === "streamline" || ru === "uglify" || ru === "encrypt" || ru === "hide"))
					ru = "copy";
				else if (ru === "streamline" && filename.substr(filename.length-3) == ".js")
					ru = "copy";
				else {
...
function executeDiff(diff, oldCommit, config, _) {
	...
	if (oldCommit !== FIRST_COMMIT && diff.indexOf(relative) > 0) {
		...
		var oldConfig = _execute('git show '+oldCommit+':'+relative, patchtools.BASE_DIRECTORY, _);		
		...
		if (!deepEqual(oldConfig.streamline, config.streamline)) {
			...
			changedRuleConfigurations.hide = changedRuleConfigurations.encrypt =
				changedRuleConfigurations.uglify = changedRuleConfigurations.streamline = /\._js$/;
		}
		...
		var fullDiffLines = _execute('git diff --name-status '+FIRST_COMMIT+' HEAD', patchtools.BASE_DIRECTORY, _).split(/[\r\n]+/);
		var i = fullDiffLines.length;
		while (--i >= 0) {
			if ((fullDiffLines.length-i-1) % 200 < 1) {
				var processed = fullDiffLines.length-i-1; 
	...
	i = lines.length;
	while (--i >= 0) {
		if ((lines.length-i-1) % 200 < 1) {
			var processed = lines.length-i-1; 
...
function _collect(oldCommit, newCommit, config, _) {
	// log differences
	var diff = _execute('git diff --name-status '+oldCommit+' '+newCommit, patchtools.BASE_DIRECTORY, _);
