...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, globalConfig, checkSource, _) {
	if (LOGFILE) {
		try {
	...
	if (mode === "D") { // delete file
		...
		if ((data.rule === "uglify" || data.rule === "encrypt" || data.rule === "streamline" || data.rule === "hide") && /\._?js$/.test(data.f)) {
			...
			if (data.rule !== "streamline") {
			    ...
			    var sq = uglifyjs.Compressor({"hoist_funs": false
			    	/* ,
			    	unused: false, 
			    	side_effects: false */
			    });
