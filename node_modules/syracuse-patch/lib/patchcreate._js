...
function createPatch(track, newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _) {
	...
	try {
		...
		if (baseVersion.relNumber !== targetVersion.relNumber && targetVersion.patchNumber > 0) {
			...
			tracer("Release commit comment " + result);
...
function _analyzeStack(stack) {
	...
	lines.forEach(function(line) {
		...
		var reg = /^\s*at\s+(\S+)\s+\((.*)\:(\d+)\:(\d+)\)$/.exec(line);
		var comp;
		if (reg) {
			comp = reg[2];
			comp = comp.replace(/\\/g, "/");
			...
			if (reg) {
				...
				console.log("Cannot analyze line <" + line + ">");
				return;
			}
		}
		if (comp) {
			if (comp.indexOf("/") > 0) { // only paths are interesting, no internal node.js code files
				if (workdir && comp.indexOf(workdir) === 0) {
					// OK
				} else {
	...
	result.toString = function() {
		...
		var res = this.errortext ? this.errortext + "\n" : "";
		this.forEach(function(line) {
			if (line.func) {
				res += "1  at " + line.func + " (" + line.comp + ":" + line.line + ":" + line.column + ")\n";
			} else {
				res += "1  at " + line.comp + ":" + line.line + ":" + line.column + "\n";
			}
		});
		return res;
	};
	// console.log("ANA "+stack+"\nXXXXXXXXXXXXXXXXXXXX\n"+util.format(result))
	return result;
}
...
exports.translateStackTrace = function(_, sha1, originalTrace) {
	...
	// TODO sha1
	var cfg = fs.readFile(__dirname + "/" + CONFIG_FILE, "utf8", _);
	// var relative = path.relative(patchtools.BASE_DIRECTORY, __dirname + "/" + CONFIG_FILE).replace(/\\/g, "/");
	cfg = _readConfig(cfg);

	var analyzed = _analyzeStack(originalTrace);
	if (!analyzed.workdir) { // no working directory found: cannot translate anything
		return originalTrace;
	}
	console.log("ANA " + util.format(analyzed));
	var loop = true;
	while (loop) {
		loop = false;
		for (var i = 0; i < analyzed.length; i++) {
			...
			var relative = path.relative(analyzed.workdir, line.comp).replace(/\\/g, "/");
			if (line.comp.indexOf("/") < 0 || !/^\.jsc?$/.test(line.comp)) {
				// internal component or no Javascript - no translation necessary
				console.log("weg" + line.comp);
				line.comp = "x" + path.join(patchtools.BASE_DIRECTORY, relative);
				continue;
			}
			...
			if (transformed[1]) {
				var consumer = exports.consumer(transformed[1]);
				_translate(line, consumer);
			}
		}
		console.log("ende");
	}
...
function _translate(element, consumer) {
	console.log("TRANSLATE " + util.format(element));
...
function _transform(buff, filename, rule, config, genMap) {
	...
	if ((rule === "hidepretty" || rule === "encryptpretty" || rule === "encrypt" || rule === "hide" || rule === "uglify" || rule === "streamline") && /\._?js$/.test(filename)) {
		...
		try {
			...
			if (doStreamlining) // streamline ._js files
			{
				...
				if (genMap) {
					streamlineConfig.lines = "sourcemap";
					streamlineConfig.sourceMap = true;
				}
				code = transformModule(buff.toString("utf8"), filename, streamlineConfig);
				if (genMap && "toStringWithSourceMap" in code) {
					// get a real source map out of the source map generator
					var res = code.toStringWithSourceMap({
						file: filename.replace("._js", ".js")
					});
					code = res.code;
					map = res.map;
					map = map.toJSON();
			...
			if (rule !== "streamline" && rule !== "hidepretty" && rule !== "encryptpretty") {
				// 1. parse
				var toplevel = null;
			...
			tracer("Exception when preprocessing " + filename + ": " + util.format(e.stack));
		}
	}
	return [buff, map];
}
// for unit tests
exports.consumer = require('streamline/node_modules/source-map').SourceMapConsumer;
...
function _applyRule(data, mode, config, _) {
	...
	if (mode === "D") { // delete file
		...
		/*
		if ((data.rule === "hidepretty" || data.rule === "encryptpretty" || data.rule === "encrypt" || data.rule === "hide" || data.rule === "uglify" || data.rule === "streamline") && /\._?js$/.test(data.f)) {
			// code transformations for customer site
			var code = buff.toString("utf8");
			try {
				var uglifyjs = require('uglifyjs2');
				...
				if (data.rule !== "streamline" && data.rule !== "hidepretty" && data.rule !== "encryptpretty") {
					// 1. parse
					var toplevel = null;
					toplevel = uglifyjs.parse(code, {
						filename: data.f
	...
	if (r = pattern.exec(line)) {
		...
		while (i < config.transformationRules.length) {
			...
			if (rule[0].test(filename)) {
				// changes in filename
				if (!/\._?js$/.test(filename) && (ru === "encryptpretty" || ru === "hidepretty" || ru === "encrypt" || ru === "hide" || ru === "streamline" || ru === "uglify")) ru = "copy";
...
function executeDiff(diff, oldCommit, config, _) {
	...
	if (streamlineVersionChanged) {
		tracer("Streamline version change");
	}
	...
	if (oldCommit !== FIRST_COMMIT && (diff.indexOf(relative) > 0 || streamlineVersionChanged)) {
		...
		if (!deepEqual(oldConfig.streamline, config.streamline) || streamlineVersionChanged) {
			if (!streamlineVersionChanged) tracer("Streamline configuration change from " + JSON.stringify(oldConfig.streamline) + " to " + JSON.stringify(config.streamline));
