...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, globalConfig, _) {
		...
		if (startFromRelease) {
			if (newPatch) { // a new patch/release will be created. Then old version is latest already available release
				if (latestPatch.release) // latest current patch is release
					...
					previousVersion = gitlog.next(true, _);
			...
			if (newPatch) // a new patch/release will be created. Then old version is latest already available patch/release
				...
				previousVersion = gitlog.next(false, _);
		}
	}
	...
	if (latestPatch) { // create patch file
		...
		var relOld = previousVersion.release ? previousVersion.relNumber : "-";
		var relNew = newRelease ? relNumberNew : "-";
		createPatchFile(relOld, previousVersion.rollout, relNew, rolloutCommitCurrent, commitComment, patchfile, _);
	}
...
function createPatchFile(relOld, sha1Old, relNew, sha1New, comment, patchfile, globalConfig,  _) {
	patchlogger("-------------- Create patch file "+patchfile);
	if (!patchfile)
		throw new Error("Patch file name missing");
	if (!sha1Old || !sha1New)
		throw new Error("Commit hashes missing");
	var patchElement = '"SYR","PATCH","'+comment+'"\n';
	if (!relOld) {
		var infoOld = commitData(execute('git log -1 --pretty=oneline '+sha1Old, globalconfig.rolloutRepo, _), false);
		if (infoOld) 
			relOld = infoOld.release ? infoOld.relNumber : "-";
	}
	if (!relNew) {
		var infoNew = commitData(execute('git log -1 --pretty=oneline '+sha1New, globalconfig.rolloutRepo, _), false);
		if (infoNew) 
			relNew = infoNew.release ? infoNew.relNumber : "-";
		if (!comment)
			comment = infoNew.comment;
	}
	var header = [ "syracuse patch", sha1Old, relOld, sha1New, relNew, comment];
	var patchElement = '"SYR","PATCH","'+comment+'"\n';
	executeF('git diff --irreversible-delete --minimal --binary --no-color --unified=0 '+sha1Old+' '+sha1New, globalConfig.rolloutRepo, patchfile, '1,"","ENG","1","","X3","1",\n2,'+patchElement+header.join(" ")+"\n", '**********\n7,'+patchElement+'8,""\n', _);
...
function createCustomerImage(globalConfig, _, release, releaseHash, releaseComment) {
	...
	if (!releaseHash || !releaseComment) {
		...
		var latestPatch = gitlog.next(false, _);
		if (!latestPatch)
			throw new Error("Inconsistency: Tags available but no release");
...
function GitLog(directory) {
	...
	this.next = function(release, _) {
		while (!this.finished) {		
			...
			var currentCommit = this._logs.length > 0 ? this._logs[this._logs.length-1].substr(0, 40) : "HEAD";
			this._logs = execute('git log -2 --pretty=oneline '+currentCommit, directory, _).split(/[\r\n]+/);
...
function commitData(line, release) {
	this.patternRel = /(\w{40}) (RELEASE) (\w{40}) ([\d\.]+) (.*)/;
	this.patternPat = /(\w{40}) (PATCH|RELEASE) (\w{40}) ([\d\.]+) (.*)/;
	var pattern = release ? this.patternRel : this.patternPat;
