...
function _collect(oldCommit, newCommit, config, _) {
	...
	if (diff.indexOf('\tnode_modules/syracuse-patch/lib/'+CONFIG_FILE) >= 0) {
		...
		tracer("Configuration file has changed. Delete files in node_modules directory of roll-out repository");
		write.rmdirRec(config.rolloutRepo+"/node_modules", _, !/\.node$/);
