...
exports.translateStackTrace = function(_, sha1, originalTrace) {
	...
	if (/\D\:\d+\s*$/.test(originalTrace)) {
		...
		if (sha1) {
			...
			list = ["syracuse-rtf", "syracuse-sdata", "syracuse-x3", "syracuse-main", "syracuse-ui", "syracuse-core", "streamline", "jsurl", "jsxml"].map_(_, function(_, name) {
...
function copyDirectory(_, source, target, config) {
	...
	while (--i >= 0) {
		...
		if (fs.stat(patchtools.BASE_DIRECTORY + "/" + currFile, _).isDirectory()) {
			...
			copyDirectory(_, currFile, target + "/" + files[i]);
		} else {
...
function executeDiff(diff, oldCommit, config, _) {
	...
	if (oldCommit !== FIRST_COMMIT && (diff.indexOf(relative) > 0 || streamlineVersionChanged)) {
		...
		var fullDiff = _execute(config.git + ' diff --name-status ' + FIRST_COMMIT + ' HEAD', patchtools.BASE_DIRECTORY, _);
		var fullDiffLines = fullDiffLines.split(/[\r\n]+/);
		if (changedRuleConfigurations.streamline) {
			// configuration has changed
			var submodules = _execute(config.git + ' submodules').split(/[\r\n]+/);
			for (var i = 0; i < submodules.length; i++) {
				var submodulepath = submodules[i].split(" ")[2];
				if (fullDiff.indexOf(submodulepath) < 0) {
					// very simple method: when there is any configuration change, re-generate all submodules
					fullDiffLines.push("M " + submodulepath);
				}
			}
		}
		fullDiff = undefined;
...
function cmdLinePatch(config, _) {
	...
	switch (mode) {
		...
		case 'new-branch':
			globalConfig = config;
			return createPatch(null, true, startFromRelease, true, newRelease, description, baseRelease, patchFile, startVersion, null, x3Information, checkSource, _);
		case 'customer-image':
