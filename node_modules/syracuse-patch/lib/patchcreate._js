...
function _taglist(globalConfig, testRelease, _) {
	var tags = [];
	var tmptags = _execute('git tag', globalConfig.rolloutRepo, _).trim().split(/[\r\n\s]+/);
...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, globalConfig, _) {
	...
	var tags = _taglist(globalConfig, baseRelease, _);
	...
	if (baseRelease) {
		var branches = _execute('git branch -v --no-abbrev', globalConfig.rolloutRepo, _);		
		if (newPatch) { 
			...
			if (!r || r[1] !== 'R'+baseRelease) // not on correct branch
				_execute('git checkout R'+baseRelease, globalConfig.rolloutRepo, _);
		} else {
	...
	if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
		...
		if (newRelease && latestPatch) {
			...
			_execute('git branch R'+relNumberNew, globalConfig.rolloutRepo, _);
			_execute('git checkout -f R'+relNumberNew, globalConfig.rolloutRepo, _);
		}
		...
		_execute('git add --all .', globalConfig.rolloutRepo, _);
		// commit data
		var comment = [];
		if (newRelease) {
		...
		// commit the changes
		try {
			_execute('git commit --no-status -q -m "'+commentString+'"', globalConfig.rolloutRepo, _);			
		} catch (e) {
			var status = _execute('git status --porcelain', globalConfig.rolloutRepo, _);
			if (!status) {
				if (currentBranchNo && latestPatch && newRelease) { // delete newly created branch
					_execute('git checkout '+currentBranchNo, globalConfig.rolloutRepo, _);
					_execute('git branch -d R'+relNumberNew, globalConfig.rolloutRepo, _)
				}
		...
		if (newRelease) {
			...
			// create tag
			_execute('git tag T'+relNumberNew, globalConfig.rolloutRepo, _);
		}
	...
	var gitDiff = _execute('git diff --irreversible-delete --minimal --binary --no-color --unified=0 '+baseVersion.rollout+' '+targetVersion.rollout, globalConfig.rolloutRepo, _).split(/[\r\n]+/);
	// filter the output
	var target = [];
	if (x3Information)
...
function createCustomerImage(globalConfig, _, release, releaseHash, releaseComment) {
	...
	try {
		...
		var tags = _taglist(globalConfig, release, _);
		if (tags.length === 0)
		...
		_execute('git clone -l -s -c core.eol=lf -c core.autocrlf=input --branch T'+release+' '+globalConfig.rolloutRepo+' '+globalConfig.customerImage, ".", _);
...
function _applyRule(rulename, filename, mode, config, _) {
	...
	if (mode === "D") { // delete and mark file for commit
		...
		if (rulename === "encrypt") // encrypt file 
...
function allPatches(globalConfig, _) {
	...
	var tmpBranches = _execute('git branch -v --no-abbrev', globalConfig.rolloutRepo, _).split(/[\r\n]+/);
	var releases = [];
	var releasesHash = {};
	var i = 0;
...
function commitDataFromHash(sha1, globalConfig, _) {
	var result = commitData(_execute('git log -1 --pretty=format:"%H %cd %s" --date=short '+sha1, globalConfig.rolloutRepo, _), false);
	if (!result)
