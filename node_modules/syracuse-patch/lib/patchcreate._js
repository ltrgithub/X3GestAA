	...
	lines.forEach(function(line) {
		...
		if (comp) {
			...
			if (comp.indexOf("/") > 0) { // only paths are interesting, no internal node.js code files
				...
				if (comp.lastIndexOf("/node_modules/") < 0 && /^\/?(syracuse-\w+|jsurl|jsxml|streamline(?:-\w+)?)\//.test(comp)) {
...
exports.translateStackTrace = function(_, sha1, originalTrace) {
	...
	var SourceMapConsumer = require('streamline/node_modules/source-map').SourceMapConsumer;
	var cfg;
	if (sha1) {
		var relative = path.relative(patchtools.BASE_DIRECTORY, __dirname + "/" + CONFIG_FILE).replace(/\\/g, "/");
		cfg = _execute('git show ' + sha1 + ':' + relative, patchtools.BASE_DIRECTORY, _);
	} else {
		cfg = fs.readFile(__dirname + "/" + CONFIG_FILE, "utf8", _);
	}
	cfg = _readConfig(cfg);
	...
	for (var i = 0; i < analyzed.length; i++) {
		...
		var consumer;
		if (transformed[1]) {
			consumer = new SourceMapConsumer(transformed[1]);
		}
