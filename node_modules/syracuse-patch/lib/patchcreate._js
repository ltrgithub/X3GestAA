...
var encrypter;
var globalConfig = require('syracuse-main/lib/nodeconfig').config;
...
function _taglist(patchConfig, testRelease, _) {
	var tags = [];
	var tmptags = _execute('git tag', patchConfig.rolloutRepo, _).trim().split(/[\r\n\s]+/);
...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _) {
	if (LOGFILE) {
		try {
	...
	if (!encrypter)
		encrypter = require('syracuse-license').load("encrypt");
	var stdlogger = tracer;
	tracer = patchlogger;
	try {
	if (!globalConfig || !globalConfig.patch) 
		throw new Error("No patch without patch settings in global configuration object");
	if (!write.exists(globalConfig.patch.rolloutRepo, _))
		throw new Error("Roll-out repository path does not exist: "+globalConfig.patch.rolloutRepo)
	...
	if (baseRelease) {
		var branches = _execute('git branch -v --no-abbrev', globalConfig.patch.rolloutRepo, _);		
		if (newPatch) { 
			...
			if (!r || r[1] !== 'R'+baseRelease) // not on correct branch
				_execute('git checkout R'+baseRelease, globalConfig.patch.rolloutRepo, _);
		} else {
	...
	if (tags.length > 0) { // get log data from git
		if (sha1Old) {			
			...
			var gitlog = new GitLog(globalConfig.patch.rolloutRepo, latestSha1);
	...
	if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
		...
		config.streamline = globalConfig.streamline;
		...
		if (newRelease && latestPatch) {
			...
			_execute('git branch R'+relNumberNew, globalConfig.patch.rolloutRepo, _);
			_execute('git checkout R'+relNumberNew, globalConfig.patch.rolloutRepo, _);
		}
		if (!globalConfig.patch.testMode && checkSource && _execute('git status -s', write.BASE_DIRECTORY, _))
			throw new Error("No new patch with uncommitted changes in source tree");
		var sourceCommitOld = (latestPatch ? latestPatch.source : FIRST_COMMIT);
		var sourceCommitCurrent = _execute('git log -1 --pretty=format:%H', write.BASE_DIRECTORY, _);
		_collect(sourceCommitOld, sourceCommitCurrent, config, _);
		// add changes to staging area
		if (globalConfig.patch.testMode) // faster execution in test mode - just to show which files would be in rollout repository
			return ""; 
		_execute('git add --all .', globalConfig.patch.rolloutRepo, _);
		// commit data
		var comment = [];
		if (newRelease) {
		...
		// commit the changes
		try {
			_execute('git commit --no-status -q -m "'+commentString+'"', globalConfig.patch.rolloutRepo, _);			
		} catch (e) {
			var status = _execute('git status --porcelain', globalConfig.patch.rolloutRepo, _);
			if (!status) {
				if (currentBranchNo && latestPatch && newRelease) { // delete newly created branch
					_execute('git checkout '+currentBranchNo, globalConfig.patch.rolloutRepo, _);
					_execute('git branch -d R'+relNumberNew, globalConfig.patch.rolloutRepo, _)
				}
		...
		if (newRelease) {
			...
			// create tag
			_execute('git tag T'+relNumberNew, globalConfig.patch.rolloutRepo, _);
		}
		...
		if (!latestPatch) {			
			createCustomerImage(globalConfig.patch, _, relNumberNew, targetVersion.rollout, commitComment);
			return; // do not create patch file 
		} 
	} else {
		if (sha1New)
			targetVersion = commitDataFromHash(sha1New, globalConfig.patch, _);
		else
			targetVersion = commitDataFromHash("HEAD", globalConfig.patch, _);
	}
	// create patch file
	...
	var gitDiff = _execute('git diff --irreversible-delete --minimal --binary --no-color --unified=0 '+baseVersion.rollout+' '+targetVersion.rollout, globalConfig.patch.rolloutRepo, _).split(/[\r\n]+/);
	// filter the output
	var target = [];
	if (x3Information)
		target.push('1,"","ENG","1","","X3","2",', "2,"+patchElement); 
		...
		if (line.substr(0, 6) === 'index ') {
			...
			if (r) {
				...
				if (file_to === file_from)
					file_to = "";
				if (r[3])
					access_rights = r[3].substr(r[3].length-3);
				if (!/^[0-7]{3}$/.test(access_rights))
					access_rights = "644";
				target.push("FILE "+mode+' '+access_rights+' "'+file_from+'" "'+file_to+'" '+r[1]+'-'+r[2]);
			} else 
				throw new Error("Unnormal index line "+line);
			continue;
		}
...
function createCustomerImage(patchConfig, _, release, releaseHash, releaseComment) {
	...
	if (!write.exists(patchConfig.rolloutRepo, _))
		throw new Error("Roll-out repository path does not exist: "+patchConfig.rolloutRepo)
	...
	try {
		...
		_execute('git clone -l -s -c core.eol=lf -c core.autocrlf=input --branch T'+release+' '+patchConfig.rolloutRepo+' '+patchConfig.customerImage, write.BASE_DIRECTORY, _);
		tracer("Clone finished");
		if (!releaseHash || !releaseComment) {
			var gitlog = new GitLog(patchConfig.customerImage);
		...
		write.rmdirRec(patchConfig.customerImage+"/.git", _);
		// 		create checksum files
		tracer("Write checksum files")
		var	sha1 = write.makeChecksums(patchConfig.customerImage, _);
		...
		write.writeFile(patchConfig.customerImage+"/"+write.VERSION_FILE, initialVersion, _);
		tracer("Double-check checksums");
		var errors= write.checkChecksumsV(patchConfig.customerImage, _) 
		if (errors.length > 0) {
	...
	while (i < lines.length){
		...
		if (r) {
			if (section === "files") { // associations of file paths to actions
				if (r[1] === "noCrypt") {
					config.noCrypt = r[2];
					console.log("nocrypt "+r[2])
					continue;
				}
				if (r[1].indexOf('_') === 0 && !(r[1] in config.commands))
					throw new Error("Command "+r[1]+ " is not defined");				
				config.transformationRules.push(makeRule(r[1], r[2], config.noCrypt));
...
function _execute(cmdline, directory, callback) {
...
function _applyRule(data, mode, config, _) {
	if (!data || data.rule === "ignore" || data.rule === "norule")
		return;
	if (data.rule.charAt(0) === '_')
		return data.rule;
	mode = mode || data.mode;
	...
	if (mode === "D") { // delete file
		...
		var buff = fs.readFile(write.BASE_DIRECTORY+"/"+data.f, _);
		if ((data.rule === "uglify" || data.rule === "encrypt" || data.rule === "streamline" || data.rule === "hide") && /\._?js$/.test(data.f)) {
			...
			if (doStreamlining) // streamline ._js files
				code = streamlineTransform(buff.toString("utf8"), config.streamline)
			if (data.rule !== "streamline") {
			    // 1. parse
			    var toplevel = null;
			    toplevel = uglifyjs.parse(code, { filename: data.f });
			    ...
			    var map = null;
			    var inMap = null;
			    var stream = uglifyjs.OutputStream({ indent_level:0, space_colon:false, semicolons: false });
			    toplevel.print(stream);
			    code = stream + "";
			}
			buff = new Buffer(code, "utf8");
			} catch (e) {
				tracer("Exception when preprocessing "+data.f+": "+e);
			}
...
function makeRule(name, pattern, noCrypt) {
	...
	if (noCrypt && (name === "hide" || name === "encrypt"))
		name = "uglify";
	if (noCrypt > 1 && (name === "uglify"))
		name = "streamline";
	if (noCrypt > 2 && (name === "streamline"))
		name = "copy";	
...
function findRule(line, config, suppressMessage) {	
	...
	if (r = pattern.exec(line)) {
		var i = 0
		var filename = r[2];
		var result = { mode: r[1], f: filename}
		while (i < config.transformationRules.length) {
		    var rule = config.transformationRules[i++];
			var ru = rule[1];
			if (rule[0].test(filename)) {
				if (!suppressMessage)
					tracer(ru+"("+(i+1)+") "+r[1]+" "+filename);
				// changes in filename
				if (ru === "streamline" || ru === "uglify")
					filename = filename.replace(/\._js$/, ".js");
				else if (ru === "encrypt" || ru === "hide") {
					filename = filename.replace(/\._?js$/, ".jsc");
				}					
				if (ru === "extra") {
					result.extra = true;
					continue;
...
function _extra(oneTimeActions, data, _) {
	if (data.mode != "D") {
		var contents = fs.readFile(write.BASE_DIRECTORY+"/"+data.f, "utf8", _);
		var functionPattern = /(?:^|[\r\n])\s*exports\.(patch\w*)/g;
		var regexpResult;
...
function executeDiff(diff, oldCommit, config, _) {
	...
	var relative = path.relative(write.BASE_DIRECTORY, __dirname+"/"+CONFIG_FILE).replace(/\\/g, "/");
	var oneTimeActions = {}
	var r;
	...
	if (oldCommit !== FIRST_COMMIT && diff.indexOf(relative) > 0) {
		...
		for (var i = 0; i<lines.length; i++) {
			if (r = /[A-Z]+\s+(.*)/.exec(lines[i]))
				currentDiffContents[r[1]] = lines[i];
		}
		var oldConfig = _execute('git show '+oldCommit+':'+relative, write.BASE_DIRECTORY, _);
		oldConfig = _readConfig(oldConfig);
		var fullDiffLines = _execute('git diff --name-status '+FIRST_COMMIT+' HEAD', write.BASE_DIRECTORY, _).split(/[\r\n]+/);
		var i = fullDiffLines.length;
		while (--i >= 0) {
			var line = fullDiffLines[i];
			var resNew = findRule(line, config, true);
			var resOld = findRule(line, oldConfig, true);
			if (resOld && resNew) {
				if (resOld.rule !== resNew.rule || resOld.extra !== resNew.extra) { // different rule
...
function _collect(oldCommit, newCommit, config, _) {
	// log differences
	var diff = _execute('git diff --name-status '+oldCommit+' '+newCommit, write.BASE_DIRECTORY, _);
	...
	while (--i >= 0) { // one time action as a batch file
		...
		// execute the command
		try {
			_execute(result, write.BASE_DIRECTORY, _);
		} catch (e) {
	...
	this.next = function(release, _) {
		while (!this.finished) {		
			...
			if (this._logs.length > 0) {
				...
				this._i = 0;
			this._logs = _execute('git log -'+this._count+' --pretty=format:"%H %cd %s" --date=short '+currentCommit, directory, _).split(/[\r\n]+/);
			this._count *= 2;
...
function allPatches(patchConfig, _) {
	...
	var tmpBranches = _execute('git branch -v --no-abbrev', patchConfig.rolloutRepo, _).split(/[\r\n]+/);
	var releases = [];
	var releasesHash = {};
	var i = 0;
	...
	var result = [];
	var patch = null;
	var gitlog = new GitLog(patchConfig.rolloutRepo, releasesHash[releases[i-1]]);
	while (--i >= 0) {
		...
		while (patch) {
			...
			if (i > 0 && patch.rollout !== releasesHash[releases[i-1]]) {
				gitlog = new GitLog(patchConfig.rolloutRepo, releasesHash[releases[i-1]]);
				patch = null;
			}
			break;
		}
...
function commitDataFromHash(sha1, patchConfig, _) {
	var result = commitData(_execute('git log -1 --pretty=format:"%H %cd %s" --date=short '+sha1, patchConfig.rolloutRepo, _), false);
	if (!result)
