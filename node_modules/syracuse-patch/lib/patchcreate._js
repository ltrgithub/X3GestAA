...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, globalConfig, _) {
	...
	if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
		...
		var sourceCommitCurrent = _execute('git log -1 --pretty=format:%H', ".", _);
...
function createCustomerImage(patchConfig, _, release, releaseHash, releaseComment) {
	...
	try {
		...
		_execute('git clone -l -s -c core.eol=lf -c core.autocrlf=input --branch T'+release+' '+patchConfig.rolloutRepo+' '+patchConfig.customerImage, ".", _);
...
function _applyRule(rulename, filename, mode, config, _) {
	...
	if (rulename === "uglify") // change streamline file name when uglifying
	...
	if (mode === "D") { // delete and mark file for commit
		...
		if ((rulename === "uglify" || rulename === "encrypt") && /\._?js$/.test(filename)) {
			// code transformations for customer site
			var code = buff.toString("utf8");
			if (/\._js/.test(filename)) // streamline ._js files
				code = streamlineTransform(buff.toString("utf8"), config.streamline)
			var jsp = require("uglify-js").parser;
			var ug = uglifyjs.uglify;
			var ast = uglifyjs.parser.parse(code); // parse code and get the initial AST
			ast = ug.ast_mangle(ast); // get a new AST with mangled names
			ast = ug.ast_squeeze(ast); // get an AST with compression optimizations
			code = ug.gen_code(ast); // compressed code here
			buff = new Buffer(code, "utf8");
			if (rulename === "encrypt")
				buff = encrypter.encrypt(filename, buff);
		} 		
...
function _collect(oldCommit, newCommit, config, _) {
	// log differences
	var diff = _execute('git diff --name-status '+oldCommit+' '+newCommit, '.', _);
