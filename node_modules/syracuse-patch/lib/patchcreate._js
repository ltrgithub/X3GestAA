...
function _analyzeStack(stack) {
	...
	result.toString = function() {
		...
		this.forEach(function(line) {
			if (line.name) {
				res += "1  at " + line.name + " (" + line.comp + ":" + line.line + ":" + line.column + ")\n";
			} else {
...
exports.translateStackTrace = function(_, sha1, originalTrace) {
	...
	for (var i = 0; i < analyzed.length; i++) {
		...
		console.log("ORIGIAL " + originalName);
		try {
			...
			console.log("OR2IGIAL " + originalName);
			try {
				originalContent = fs.readFile(originalName, _);
			} catch (e) {
		...
		// console.log("RULE "+util.format(rule))
		var transformed = _transform(originalContent, rule.f, rule.rule, cfg, true);
		if (transformed[1]) {
			var consumer = new exports.consumer(transformed[1]);
	...
	if ((rule === "hidepretty" || rule === "encryptpretty" || rule === "encrypt" || rule === "hide" || rule === "uglify" || rule === "streamline") && /\._?js$/.test(filename)) {
		...
		try {
			...
			if (doStreamlining) // streamline ._js files
			{
				...
				if (genMap && code.toStringWithSourceMap) {
...
// for unit tests
exports.consumer = require('uglifyjs2/node_modules/source-map').SourceMapConsumer;
