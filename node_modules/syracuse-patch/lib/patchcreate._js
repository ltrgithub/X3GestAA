...
var deepEqual = require('@sage/syracuse-lib/src/load-balancer/certTools').deepEqual;


/// !doc
/// # Patch creation  
/// ```javascript
/// var patchcreate = require('./patchcreate')  
/// ```
/// 
...
function directImage(releaseNumber, commitComment, configuration, checkSource, _) {
	...
	try {
		...
		var config = _readConfig(contents, configuration.symbols);
		...
		var initialVersion = JSON.stringify({
			...
			src: sourceCommitCurrent + (configuration.symbols ? "-" + configuration.symbols.join(",") : "")
		});
...
function createPatch(track, newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _, rebuild) {
	...
	try {
		...
		if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
			...
			var config = _readConfig(contents, globalConfig.patch.symbols);
			// take rolloutRepo and streamline options from globalConfig so that globalConfig need not be passed to all functions
...
function createCustomerImage(track, patchConfig, _, release) {
	...
	try {
		...
		oldConfig = _readConfig(oldConfig, patchConfig.symbols);
...
function _readConfig(contents, symbols) {
	symbols && tracer && tracer("Symbols: " + symbols.join(","));
	...
	while (i < lines.length) {
		var line = lines[i++];
		r = /^(?:if\s*\(\s*(\!)?\s*(\w+)\s*\)\s*)?(\w+)\s+(.*)/.exec(line);
		if (r) {
			if (r[2]) { // conditional rule
				if (!r[1] ^ !(symbols && symbols.indexOf(r[2]) >= 0)) {
					tracer && tracer("Line " + i + ": Include rule " + r[3] + " " + r[4]);
				} else {
					tracer && tracer("Line " + i + ": Exclude rule " + r[3] + " " + r[4]);
					continue;
				}
			}
			if (section === "options") {
				if (r[3] === "streamline") {
					try {
						config.streamline = JSON.parse(r[4]);
					} catch (e) {
						tracer && tracer("Error in parsing streamline options " + e);
					}
				} else if (r[3] === "uglify") {
					try {
						config.uglify = JSON.parse(r[4]);
					} catch (e) {
						tracer && tracer("Error in parsing uglify options " + e);
					}
				} else console.log("Unknown settings " + r[3]);
			} else if (section === "files") { // associations of file paths to actions
				if (r[3] === "noCrypt") {
					config.noCrypt = r[4];
					tracer && tracer("nocrypt " + r[4]);
					continue;
				}
				if (r[3].indexOf('_') === 0 && !(r[3] in config.commands)) throw new Error("Command " + r[3] + " is not defined");
				config.transformationRules.push(makeRule(r[3], r[4], config.noCrypt, i));
			} else if (section === "commands") { // definitions of one time actions
				config.commands[r[3]] = r[4];
			}
		} else {
			r = /^\[(\w+)\]/i.exec(line);
...
function _analyzeStack(stack, list) {
	...
	lines.forEach(function(line) {
		...
		line = line.replace(/\bFunction\.(\w+) \[as fiberized-\d+\]/, '$1');
		var reg = /^\s*at\s+((?:\[[\w ]+\])?\S+)\s+\((.*)\:(\d+)\:(\d+)\)(\[[\w ]+\])?$/m.exec(line);
		var comp;
		var element;
		if (reg) {
			comp = reg[2];
		...
		if (comp) {
			...
			if (comp.indexOf("/") > 0 && element.column >= 0) { // only paths are interesting, no internal node.js code files
				...
				if (comp.lastIndexOf("/node_modules/") < 0 && /^\/?(syracuse-\w+|jsurl|js-xml|streamline(?:-\w+)?)\//.test(comp)) {
					...
					if (!workdir && index0 > 0 || workdir && workdir.substr(0, index) === comp.substr(0, index)) {
						workdir = comp.substr(0, index);
						// OK
					} else {
						console.error("maybe shadow-module " + workdir + " <" + comp + "> ");
						result.error = line;
					}
				}
			}
...
exports.translateStackTrace = function(_, sha1, originalTrace) {
	...
	var symbols;
	if (sha1) {
		var symbolIndex = sha1.indexOf("-");
		if (symbolIndex >= 0) {
			symbols = sha1.substr(symbolIndex + 1).split(/,/);
			sha1 = sha1.substr(0, symbolIndex);
		}
	...
	if (/\D\:\d+\s*$/.test(originalTrace)) {
		...
		if (sha1) {
			...
			list = ["@sage/rtf-converter", "syracuse-x3", "syracuse-main", "syracuse-ui", "syracuse-core", "streamline", "jsurl", "js-xml"].map_(_, function(_, name) {
				return _getFileList(_, patchtools.BASE_DIRECTORY, "node_modules/" + name);
			}).concat(["sdata"]).map_(_, function(_, name) {
				return _getFileList(_, patchtools.BASE_DIRECTORY, "src/" + name);
			}).join("");
		}
	}
	...
	for (var i = 0; i < analyzed.length; i++) {
		...
		relative = relative.replace(/\.(?:jsc|es5)$/, ".js");
		var originalContent;
...
function _transform(buff, filename, rule, config, genMap) {
	...
	if ((rule === "hidepretty" || rule === "encryptpretty" || rule === "encrypt" || rule === "hide" || rule === "uglify" || rule === "streamline" || rule === "babel") && /\.(_?js|ts)$/.test(filename)) {
		...
		try {
			...
			if (/\.(?:_j|t)s$/.test(filename)) {
				...
				streamlineConfig.sourceRoot = path.resolve(path.dirname(filename));
				streamlineConfig.filename = filename;
				streamlineConfig.sourceMaps = genMap;
				streamlineConfig.babel = {
					plugins: ['transform-flow-comments', 'transform-class-properties', 'streamline'],
					presets: ['es2015'],
					compact: false
				};
				var transformSync = require('streamline/lib/transformSync');
				var transformed = transformSync.transform(code, streamlineConfig)
				...
				var babelOptions = {
					...
					plugins: [require('babel-plugin-transform-flow-comments'), require("babel-plugin-transform-class-properties")]
				};
				// lazy load babel to reduce startup time and memory footprint
			...
			throw e; // non preprocessed files may be invalid because of missing babel transformation
		}
	}
	return [buff, map];
}
...
function _applyRule(data, mode, config, _) {
	if (!data) return;
	if (!/^[\t\x20-\x7F]*$/.test(data.file)) {
		console.log("Ignore path with strange characters " + data.file);
		return null;
	}
	...
	if (mode === "D") { // delete file
		...
		try {
			...
			if (data.rule === "encrypt" || data.rule === "hide" || data.rule === "encryptpretty" || data.rule === "hidepretty") {
				if (!encrypter) encrypter = require('@sage/syracuse-lib/src/license/index').load("encrypt");
			...
			if (/\.es5$/.test(fullTargetFileName)) {
				try {
					...
					if (e.code !== "ENOENT") {
						...
						try {
							patchtools.unlinkP(fullTargetFileName.replace(/\.es5$/, ".ts"), _);
							tracer && tracer("Delete obsolete file " + fullTargetFileName.replace(/\.es5$/, ".ts"));
						} catch (e) {
							if (e.code !== "ENOENT") {
								tracer && tracer("Cannot delete " + fullTargetFileName.replace(/\.es5$/, ".ts"));
							}
						}
					}
				}
	...
	if (r = pattern.exec(line)) {
		...
		while (i < config.transformationRules.length) {
			...
			if (rule[0].test(filename)) {
				// changes in filename
				if (!/\.(?:_?j|t)s$/.test(filename) && (ru === "encryptpretty" || ru === "hidepretty" || ru === "encrypt" || ru === "hide" || ru === "streamline" || ru === "uglify" || ru === "babel")) ru = "copy";
					...
					if (ru === "streamline" || ru === "uglify") filename = filename.replace(/\.(?:_j|t)s$/, ".es5");
					else if (ru === "encryptpretty" || ru === "hidepretty" || ru === "encrypt" || ru === "hide") filename = filename.replace(/\.(?:_?j|t)s$/, ".jsc");
				}
				if (!suppressMessage) {
			...
			default:
				...
				switch (argument) {
					...
					case '--config-file':
						...
						break;
					case '--symbols':
						if (!/^(\w+,)*\w+$/.test(args[++i])) throw new Error("Symbols must be comma separated: " + args[i]);
						config.patch.symbols = args[i].split(/,/);
						break;
					default:
