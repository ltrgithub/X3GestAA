...
function directImage(releaseNumber, commitComment, configuration, checkSource, _) {
	...
	try {
		...
		configuration.configFile = configuration.configFile || __dirname + "/" + CONFIG_FILE;
...
function createPatch(track, newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _) {
	...
	try {
		...
		var target = [];
		if (x3Information) target.push('1,"","ENG","1","","Syracuse","2",');
...
function createCustomerImage(track, patchConfig, _, release) {
	...
	try {
		...
		if (!newVersion) throw new Error("Inconsistency: Tags available but no branch");
		var relative = path.relative(patchtools.BASE_DIRECTORY, __dirname + "/" + CONFIG_FILE).replace(/\\/g, "/");
...
function cmdLinePatch(config, _) {
	...
	switch (mode) {
		...
		case 'patch-file':
			globalConfig = config;
			return createPatch(null, false, startFromRelease, false, null, null, baseRelease, patchFile, startVersion, endVersion, x3Information, checkSource, _);
		case 'direct':
