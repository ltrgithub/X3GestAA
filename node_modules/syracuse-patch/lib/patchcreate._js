...
function createPatch(track, newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, x3Information, checkSource, _) {
	...
	try {
		...
		if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
			...
			var difference = _execute('git diff --no-color --unified=0 ' + baseVersion.source + ' ' + targetVersion.source + ' -- ' + relative, patchtools.BASE_DIRECTORY, _);
			if (difference) {
		...
		if (relNumberCmp(baseVersion.relNumber, targetVersion.relNumber) > 0 || baseVersion.relNumber === targetVersion.relNumber && baseVersion.patchNumber - targetVersion.patchNumber > 0) throw new Error("No downgrade with patch from " + baseVersion.relNumber + "-" + baseVersion.patchNumber + " to " + targetVersion.relNumber + "-" + targetVersion.patchNumber);
...
function createCustomerImage(track, patchConfig, _, release, releaseHash, releaseComment) {
	...
	try {
		...
		var initialVersion = JSON.stringify({
			...
			init: true,
			sha1: sha1,
			streamline: patchConfig.streamline
		});
	...
	if (mode === "D") { // delete file
		...
		if ((data.rule === "uglify" || data.rule === "encrypt" || data.rule === "streamline" || data.rule === "hide") && /\._?js$/.test(data.f)) {
			...
			try {
				...
				console.log("CONFIG " + util.format(config.streamline));
				var streamlineConfig = {};
				for (var key in Object.keys(config.streamline)) streamlineConfig[key] = config.streamline[key];
				console.log("CONFIG2 " + util.format(streamlineConfig));
			...
			if (data.rule === "encrypt" || data.rule === "hide") buff = encrypter.encrypt(data.file, buff, data.rule === "hide", require);
		}
