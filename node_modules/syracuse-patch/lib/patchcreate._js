...
function patch(newPatch, startFromRelease, newRelease, relNumberNew, commitComment, baseRelease, patchfile, sha1Old, sha1New, globalConfig, _) {
	...
	if (baseRelease) {
		var branches = execute('git branch', globalConfig.rolloutRepo, _);
		patchlogger("Branches "+branches);
		if (branches.indexOf("* R"+baseRelease) < 0) // not on right branch		
	...
	patchlogger("Base version "+baseVersion);
	patchlogger("Latest patch "+util.format(latestPatch));
	if (newPatch) { // collect data for new patch or release (newPatch will be set if newRelease is set)
		...
		execute('git add .', globalConfig.rolloutRepo, _);
		// commit data
		var comment = [];
		if (newRelease) {
	...
	var header = "syracuse patch "+baseVersion.relNumber+" "+baseVersion.patchNumber+" "+targetVersion.date+" "+targetVersion.relNumber+" "+targetVersion.patchNumber+" "+baseVersion.rollout+" "+targetVersion.rollout+"\n";
	header += "syracuse patchcomment "+targetVersion.comment+"\n";
...
function execute(cmdline, directory, callback) {
	...
	child_process.exec(cmdline, {maxBuffer: 1024*1024, cwd: directory }, function(error, stdout, stderr) {
		if (error !== null) {
			...
			return callback(stderr ? stderr : error);
		}
		if (stderr)
	...
	this.next = function(release, _) {
		while (!this.finished) {		
			...
			if (this._logs.length > 0) {
				...
				this._i = 0;
			this._logs = execute('git log -100 --pretty=format:"%H %cd %s" --date=short '+currentCommit, directory, _).split(/[\r\n]+/);
...
function commitData(line, release) {
	...
	if (r) {
		return { release: (r[4] === "0"), source: r[5], rollout: r[1], relNumber: r[3], patchNumber: r[4], comment: r[7], date: r[2] };
	}
	return null;
}
