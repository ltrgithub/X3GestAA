...
function copyRecInt(start, target, exclude, fast, _) {
	...
	return copyContents(start, target, exclude, fast, _); // copy contents of directory
}

function copyRec(start, target, fast, _) {
	return copyRecInt(start, target, EXCLUDES_COPY, fast, _); // copy contents of directory
}
...
///
function copyContents(start, target, exclude, fast, _) {
	var checksumsStart;
	var checksumsTarget = {};
	var changes = false;
	var startFiles;
	...
	while (--i >= 0) {
		...
		if (fast) {
			...
			if (checksumsStart[filename].substr(0, 2) === "D ") { // fast copy subdirectory
				copyRecInt(startFile, targetFile, null, true, _);
				delete targetFilesHash[filename];
				continue;
			}
		}
		...
		if (fs.stat(startFile, _).isDirectory()) { // start is directory: copy recursively
			copyRecInt(startFile, targetFile, null, false, _);
...
function unlink(path, callback) {
	fs.unlink(path, function(ex) { return callback(ex, null)});
}
...
function rmdir(path, callback) {
	fs.rmdir(path, function(ex) { return callback(ex, null)});
}

//asynchronous rename
function rename(path, callback) {
	fs.rename(path, function(ex) { return callback(ex, null)});
}
exports.rename = rename;

//asynchronous mkdir
function mkdir(path, callback) {
	fs.mkdir(path, function(ex) { return callback(ex, null)});
}
...
function chmod(path, mode, callback) {
	fs.chmod(path, mode, function(ex) { return callback(ex, null)});
}
...
function checkChecksumsV(path, _) {
	...
	try {
		...
		if (sha1 != version.sha1)
			errors.push("Checksum mismatch with version file");
	} catch (e) {
		errors.push("Error when reading version file: "+e);
	}
	return errors;
}
			...
			if (currFile.isDirectory()) { // Recursive function back to the beginning
				if (contents[file].substr(0, 2) != "D ")
					...
					if (contents[file] !== "D "+sha1) {
						errors.push("Changes in "+filePath+". Expected checksum "+contents[file].substr(2)+" indeed "+sha1);
					}					
				}
			} else {
				var buffer = fs.readFile(filePath, _)
...
function exchangeProcess(dir, logfile, parameter) {
	...
	var child = child_process.spawn(dir+"/nodejs/node", [process.argv[1], parameter ], { detached: true, stdio: [ 'ignore', out, err ], cwd: dir });
	child.unref(); 
	// kill node process		
	 console.log("kill");
