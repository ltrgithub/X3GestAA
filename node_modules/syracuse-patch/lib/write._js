...
function copyContents(start, target, exclude, fast, _) {
	...
	if (fast) {
		...
		if (exclude && exists(start+"/"+exports.VERSION_FILE, _)) // root directory: copy version file
			startFiles.push(exports.VERSION_FILE);
		try {
...
// optional 3rd argument: regular expression for file name patterns to be deleted (will be applied to basename of file)
function rmdirRec(path, _, pattern) {
...
function exchangeProcess(dir, logfile, parameter) {
    ...
    if (!file)
    	throw new Error("node not found for detached process")
