...
var RELEASE_DIRECTORY = "rel";
var TEMP_DIRECTORY = "temp";
...
function write_data(targetDirectory, status, file_from, file_to, sha1_to, file_lines, checksums, _) {
	...
	if ((status & LOADED) != 0) {
		if (file_from && file_from !== file_to) {
			write.unlink(targetDirectory + "/" + file_from, _);
		...
		// just renaming
		write.rename(targetDirectory+"/"+file_from, targetDirectory+"/"+file_to, _);
...
function checkVersions(parts, _) {
	var oldHash = parts[2];
	var oldRelNumber = parts[3];
	var newHash = parts[4];
	var newRelNumber = parts[5];
	var comment = parts.slice(6).join(" ");
	var baseDir = ".";
	patchlogger("checkVersions: Data: oldHash "+oldHash+" old release number "+oldRelNumber+" new hash "+newHash+" new release number "+newRelNumber+" comment "+comment);				
	// consistency checks
	var versionInformation = {};
	// check whether there is release directory
	if (!write.exists(RELEASE_DIRECTORY, _)) {
		patchlogger("No release directory available");
		versionInformation = JSON.parse(fs.readFile(write.VERSION_FILE, "utf8", _));
		if (!versionInformation.init)
			throw new Error("No release directory although version is not initial")
		patchlogger("Copy recursive . to "+RELEASE_DIRECTORY);
		write.copyRec(".", RELEASE_DIRECTORY, false, _);
		patchlogger("End copy recursive . to "+RELEASE_DIRECTORY);
	} else {
		if (oldRelNumber !== "-")
			baseDir = RELEASE_DIRECTORY;
		versionInformation = JSON.parse(fs.readFile(baseDir+"/"+write.VERSION_FILE, "utf8", _));
	}
	...
	versionInformation.comment = comment;
	if (newRelNumber !== "-") {
		versionInformation.relNumber = newRelNumber;
		versionInformation.release = true;
	} else {
		versionInformation.release = false;
	}
	// copy everything into temporary directory
	patchlogger("Fast copy recursive "+baseDir+" to "+TEMP_DIRECTORY);
	try {
		write.copyRec(baseDir, TEMP_DIRECTORY, true, _);
		...
		patchlogger("Full copy recursive "+baseDir+" to "+TEMP_DIRECTORY);
		write.copyRec(baseDir, TEMP_DIRECTORY, false, _);
	}
...
function patch(targetDirectory, filename, _) {
	...
	for (i=0; i<lines.length; i++) {
		...
		if (line.substr(0, 9) === "syracuse ") { // patch meta information
			...
			patchlogger("Before checkVersions")
				...
				targetDirectory = TEMP_DIRECTORY;
			}
		}
		...
		if (m = /^\@\@ \-\d+(?:,(\d+))? +\+(\d+)(?:,(\d+))?/.exec(line)) {
			...
			if ((status & (DIFF_ENTRIES+LOADED)) === DIFF_ENTRIES) {
				if (!write.equal_sha1(checksums[path.dirname(file_from)][path.basename(file_from)], sha1_from))
					throw new Error("READ file:Wrong check sum for "+file_from+" expected "+sha1_from);
	...
	if (versionInformation) {
		...
		fs.writeFile(TEMP_DIRECTORY+"/"+write.VERSION_FILE, JSON.stringify(versionInformation), "utf8", _);
		// start new process
/*	     out = fs.openSync('./out.log', 'a'),
	     err = fs.openSync('./out.log', 'a');
		var child = child_process.spawn(TEMP_DIRECTORY+"/nodejs/node", ['index.js', 'PATCH' ], { detached: true, stdio: [ 'ignore', out, err ] });
		child.unref(); */
		// kill node process		
		 console.log("kill");
