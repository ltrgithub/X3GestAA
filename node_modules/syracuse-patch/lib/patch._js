...
function checkVersions(parts, _) {
	...
	var copyRestrictions = {};
	copyRestrictions[RELEASE_DIRECTORY] = "";
	copyRestrictions[TEMP_DIRECTORY] = "";
	...
	if (!write.exists(RELEASE_DIRECTORY, _)) {
		...
		write.copyRec(".", RELEASE_DIRECTORY, copyRestrictions, _);
	...
	patchlogger("Copy recursive . to "+TEMP_DIRECTORY);
	write.copyRec(baseDir, TEMP_DIRECTORY, copyRestrictions, _);
...
/*		// start new process
		var child = child_process.spawn(TEMP_DIRECTORY+"/nodejs/node", ['index.js', process.pid ], { detached: true });
		child.unref();
		*/
		// kill node process		
