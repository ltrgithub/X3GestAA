...
module.exports = function(_, context, instance) {
    ...
    var loopCounter = 0;
    var application;
    while (typeof application === 'undefined'){

        var result = helpers.client(_, collabUrl + 'applications');
        // if (result.statusCode !== 200) return helpers.error('get applications', result);
        application = result.body.$resources ? result.body.$resources.filter(function(res) {
            return res.application === 'x3' && res.contract === 'erp';
        })[0] : {};
        loopCounter++;
        if (loopCounter >= 500){
            // 500 tries have occured the database isn't going to be built by now.
            return helpers.error('get applications', result)
        }
    }
    ...
    while (groupIds.length <1){
        ...
        if (typeof sadmingroup !== 'undefined'){
            var sagroup = {
        ...
        if (typeof cadmingroup !== 'undefined'){
        ...
        if (loopCounter >= 500){
            // 500 tries have occured the database isn't going to be built by now.
            return helpers.error('get groups', result);
        }
    }
    ...
    while (typeof cadmin === 'undefined'){
        ...
        if (loopCounter >= 500){
            // 500 tries have occured the database isn't going to be built by now.
            return helpers.error('get users', result);
        }
    }
    ...
    loopCounter = 0;
    var sadmin;
    while (typeof sadmin === 'undefined'){
        result = helpers.client(_, collabUrl + 'users');
        if (result.statusCode !== 200) return helpers.error('get users', result);
        sadmin = result.body.$resources ? result.body.$resources.filter(function(res) {
            return res.login === 'admin';
        })[0] : null;

        if (sadmin) {
            // Change to option from nodelocal
            var sageIdConfig = config.sage_id || {};
            sadmin.email = sageIdConfig.devOpsEmail || 'SageERPX3DevOps@sage.com';
