...
function activate(_, context, site) {
	...
	var s3 = new AWS.S3();
	var siteName = site.site(_);
	var seiSiteName = siteName + '-sei';
	...
	collection.update({site:siteName}, {'$set':{sei:1}});


	//Reset AWS region to create Cloudwatch alarm
	AWS.config.update(config.aws);

	var params = {
		  AlarmName: site.site(_) + '_seiResponseTime_' + config.alerts.env,
		  ComparisonOperator: 'GreaterThanOrEqualToThreshold',
		  EvaluationPeriods: config.alerts.consecutivePeriods,
		  MetricName: 'seiResponseTime',
		  Namespace: 'Site',
		  ...
		  AlarmDescription: site.site(_) + '_Alert for SEI',
		  Dimensions: [
		        {
		          Name:'Site ID',
		          Value:site.site(_)
