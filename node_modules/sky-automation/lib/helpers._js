...
exports.client = function(_, url, data, method) {
...
exports.clientCert = function(_, url, data, method) {
	method = method || data ? 'POST' : 'GET';
	exports.log(method, 'url:', url);
	var response = ez.devices.http.client({
		rejectUnauthorized: false,
		url: url,
		method: method,
		cert: require('fs').readFileSync(config.xmSymphony.certPath),
		key: require('fs').readFileSync(config.xmSymphony.keyPath),		
		passphrase: config.xmSymphony.passphrase,
		headers: {
			'content-type': 'text/json',
		}
	}).end(JSON.stringify(data)).response(_);
