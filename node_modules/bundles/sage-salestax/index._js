...
// Using this interface for testing our connection with Avalara.  This is recommended by Avalara for testing
// connections with the REST API.
exports.sstEstimateTax = function(_, auth, urlPath, saleAmount) {
	//console.log('\n\n  URL = ' + urlPath);
	var authenticate = Base64.encode(auth);
	//console.log('\n\n  Authenticate = ' + authenticate);
	var request = httpClient.httpRequest(_, {
		url: urlPath + '?' + 'saleamount=' + saleAmount,
		method: 'GET',
		headers: {
...
function buildAddr(addrType, addrPiece) {
	if (!addrPiece) return;
	addrString = (addrString ? addrString + '&' : '') + addrType + '=' + addrPiece.split(' ').join('+');
}
...
exports.sstPost = function(_, auth, urlPath, values) {
	...
	var reqBuffer = new Buffer(req, 'utf8');
	//console.log('\n\n  req = ' + req);
	var request = httpClient.httpRequest(_, {
		url: urlPath,
		method: 'POST',
