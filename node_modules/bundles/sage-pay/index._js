...
exports.httpDispatch = function(_, request, response) {
	...
	while (i < notificationCache_.length) {
		...
		if (redir.id === id) {
			deleteFromNotificationCache(id);
	...
	if (reqJson && reqJson.VPSTxId) {
 	...
 	if (spsData && reqJson.Status) {
		...
		if (isValidMD5) {
			...
			if (trace_){
				console.log("MD5 validation failed");
...
function getNotificationPostUrl() {
	...
	setTimeout.call(cacheValue, cacheValue.timeout, SPS_TIMEOUT_VALUE, id);
	...
	if (spsCache_ && spsCache_.length > 0) {
		for (var ii = 0, jj = spsCache_.length; ii < jj; ii++) {
			if (spsCache_[ii].VPSTxId == VPSTxId) {
				if (trace_) {
					console.log("getSPSResponse.spsCache_.VPSTxId=" + spsCache_[ii].VPSTxId);
					console.log("getSPSResponse.spsCache_.VendorName=" + spsCache_[ii].VendorName);
					console.log("getSPSResponse.spsCache_.SecurityKey=" + spsCache_[ii].SecurityKey);
					console.log("getSPSResponse.spsCache_.NextURL=" + spsCache_[ii].NextURL);
					console.log("getSPSResponse.spsCache_.WaitStatus=" + spsCache_[ii].WaitStatus);
				}
				return spsCache_[ii];
			}
		}
...
function deleteFromNotificationCache(id) {
	if (notificationCache_ && id) {
		for (var ii = 0, jj = notificationCache_.length; ii < jj; ii++) {
			if (notificationCache_[ii].id === id) {
				...
				if (trace_) {
					console.log("deleteFromNotificationCache for id: " + id);
					console.log("Notification cache size: " + notificationCache_.length);
				}
				break;
			}
		}
...
function deleteFromSpsCache(VPSTxId) {
	if (spsCache_ && VPSTxId) {
		for (var ii = 0, jj = spsCache_.length; ii < jj; ii++) {
			if (spsCache_[ii].VPSTxId === VPSTxId) {
				...
				if (trace_) {
					console.log("deleteFromSpsCache for VPSTxId: " + VPSTxId);
					console.log("Sps cache size: " + spsCache_.length);
				}
