...
exports.seRequest = function(_, values, responseProperty, reqOptions) {
	var req = buildReq(values);
	// console.log('\n\n ' + responseProperty + ' request = ' + req);
	req = "request=" + encodeURIComponent(req);

  	var options = reqOptions || {};
	options.url = options.url || 'https://www.sageexchange.com/VirtualPaymentTerminal/frmPayment.aspx';
	...
	var request = httpClient.httpRequest(_, reqOptions);
...
exports.getRedirectUrl = function(_, values, ttl) {
	...
	var redirUrl = globals.context.session.host + '/bundles/sage-exchange/redirect?id=' + id;
	// console.log('\n redirURL = ' + redirUrl);
	return redirUrl;
};
