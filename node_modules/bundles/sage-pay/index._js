...
exports.createTokenRequest = function(_, tokenReqJson, registrationOptions) {
	...
	var notificationURL = _getNotificationPostUrl();
	tokenReqJson.NotificationURL = notificationURL.url;

	var spsRawResponse = _postToSPS(_, tokenReqJson, registrationOptions);

	if (spsRawResponse) {
		_storeSagePaySession(_, notificationURL.sid, notificationURL.host, spsRawResponse, tokenReqJson.Vendor, registrationOptions.timeout || SPS_TIMEOUT_VALUE);
	}
...
exports.createPaymentRequest = function(_, paymentReqJson, registrationOptions) {
	...
	if (spsRawResponse) {
		_storeSagePaySession(_, notificationURL.sid, notificationURL.host, spsRawResponse, paymentReqJson.Vendor, registrationOptions.timeout || SPS_TIMEOUT_VALUE);
	}
	return spsRawResponse;
};
...
exports.getSPSResponse = function(_, VPSTxId) {
	...
	if (spsData && waitStatus !== SPS_WAIT_STATUS_WAIT) {
		console.log("Deleting SagePay session: " + VPSTxId);
		//_deleteSagePaySession(_, VPSTxId)
	}
...
function _getNotificationPostUrl() {
...
function _deleteSagePaySession(_, vpsTxId) {
...
function _storeSagePaySession(_, sid, host, rawResponse, vendorName, spsTimeout) {
	...
	console.log("Timeout: " + spsTimeout);
