...
exports.httpDispatch = function(_, request, response) {
	console.log("httpDispatch, request: " + request);
	console.log("httpDispatch, response: " + response);
	...
	var notification = _getSagePaySession(_, {
		nid: id
	});
	if (notification) {
...
exports.createTokenRequest = function(_, tokenReqJson, registrationOptions) {
	...
	var sessionInfo = _getNotificationPostUrl2(registrationOptions.timeout || SPS_TIMEOUT_VALUE);
	console.log("createTokenRequest - sessionInfo:" + JSON.stringify(sessionInfo));
	tokenReqJson.NotificationURL = sessionInfo.redirectUrl;

	//tokenReqJson.NotificationURL = _getNotificationPostUrl(registrationOptions.timeout || SPS_TIMEOUT_VALUE);

	//console.log("createTokenRequest, tokenReqJson: " + JSON.stringify(tokenReqJson));

	var spsRawResponse = _postToSPS(_, tokenReqJson, registrationOptions);
	console.log("createTokenRequest - spsRawResponse:" + JSON.stringify(spsRawResponse));

	//console.log("createTokenRequest, spsRawResponse: " + JSON.stringify(spsRawResponse));

	if (spsRawResponse) {
		_storeSagePaySession(_, sessionInfo.nid, sessionInfo.host, spsRawResponse, tokenReqJson.Vendor, registrationOptions.timeout || SPS_TIMEOUT_VALUE);
...
exports.getSPSResponse = function(_, VPSTxId) {
	...
	var responseX3;
	var spsData = _getSagePaySession(_, {
		vpsTxId: VPSTxId
	});
	...
	// 	if (spsData.WaitStatus === SPS_WAIT_STATUS_OK || spsData.WaitStatus === SPS_WAIT_STATUS_INVALID) {
	// 		responseX3 = "WaitStatus=" + spsData.WaitStatus + '\r\n';
...
function _cacheSPSData(_, rawResponse, vendorName, spsTimeout) {
	...
	console.log("_cacheSPSData, values: " + JSON.stringify(values));
	if (values.Status === SPS_STATUS_OK) {
		...
		cacheValue.timeout = function(VPSTxId) {
			 ...
			 */
			console.log("Deleting SPSCache ID: " + VPSTxId);
			_deleteFromSpsCache(VPSTxId);
		};
...
function _notificationResponse(_, request, response) {
	...
	console.log("_notificationResponse, requestJson: " + JSON.stringify(requestJson));

	var spsResponseJson = _replyToSPSResponse(_, request, requestJson, response, spsData);

	console.log("_notificationResponse, spsResponseJson: " + JSON.stringify(spsResponseJson));
	...
	// 	requestJson.VPSTxId = decodeURIComponent(requestJson.VPSTxId).replace(/{/g, '').replace(/}/g, '');
	// 	console.log("_notificationResponse: " + requestJson.VPSTxId);
...
function _getNotificationPostUrl2(spsTimeout) {
...
function _getNotificationPostUrl(spsTimeout) {
	...
	console.log("_getNotificationPostUrl, id: " + id);
...
/*
function _createSagePaySession(_, values) {
	...
	//Check if session instance already
	var sessionIns = db.fecthInstance(_, sessionInt, filter);
	if(!sessionIns) {
		//Instance does not exist - create a new instance
		sessionIns = sessionInt.createInstance(_, db);
	} 
	
	// write (update) instance properties
	values.nid && sessionIns.nid(_, values.nid);
	values.host && sessionIns.host(_, values.host);
...
function _storeSagePaySession(_, nid, host, sessionInfo, rawResponse, vendorName, spsTimeout) {
...
function _getSagePaySession(_, jsonWhere) {
	var db = adminHelper.getCollaborationOrm(_);
	var sessionEnt = db.getEntity(_, "sagePaySession");
	...
	return db.fecthInstance(_, sessionInt, filter);
