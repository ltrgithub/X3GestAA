...
exports.createTokenRequest = function(_, tokenReqJson, registrationOptions) {
	tokenReqJson.NotificationURL = getNotificationPostUrl(); // Look at this again!!!

	var tokenReqQueryStr = buildQueryReq(tokenReqJson);

	console.log("createTokenRequest = " + tokenReqQueryStr);
...
exports.createPaymentRequest = function(_, paymentReqJson, registrationOptions) {
	...
	var paymentReqQueryStr = buildQueryReq(paymentReqJson);

	console.log(paymentReqQueryStr);

  	var options = registrationOptions || {};
	options.url = options.url || 'https://live.sagepay.com/gateway/service/vspserver-register.vsp';
	options.method = options.method || 'POST';
	options.headers = options.headers || {
	    'content-type': 'application/x-www-form-urlencoded',
	    'content-length': paymentReqQueryStr.length,
	};

	var request = httpClient.httpRequest(_, options);
	request.write(_, paymentReqQueryStr, 'utf8');
...
function getNotificationPostUrl() {
	...
	var redirUrl;
	if (test_) {
		redirUrl = 'http://54.148.78.219/bundles-non-auth/sage-pay/notification?id=' + id; //9999';
	} else {
