...
exports.removeTokenRequest = function(_, tokenReqJson, registrationOptions) {
	return postToSPS(_, tokenReqJson, registrationOptions);
};
...
exports.getSPSResponse = function(_, VPSTxId) {
	...
	if (spsData && spsData.WaitStatus) {
		if (spsData.WaitStatus == SPS_WAIT_STATUS_OK) {
			var responseX3 = "WaitStatus=" + SPS_WAIT_STATUS_OK + "&";
			...
			return "WaitStatus=" + spsData.WaitStatus; // WAIT
		}
	} else {
		/*
...
function replyToSPSResponse(_, request, requestJson, response) {
	...
	if (test_) {
		console.log("replyToSPSResponse.postValidationJson = " + postValidationJson);
	}
...
function validateNotificationPost(_, reqJson) {
	...
	var validationJson = {};
 	if (reqJson && reqJson.Status) {
		...
		var spsData = getSpsCacheData(reqJson.VPSTxId);
 		...
 		var isValidMD5 = validateMD5(_, 
				...
				reqJson.Status, 
				spsData.VendorName || "sagetest2015",
...
function getNotificationPostUrl() {
	...
	var redirUrl;
	if (test_) {
		redirUrl = 'http://54.148.78.219/bundles-non-auth/sage-pay/notification?id=' + id; 
		//redirUrl = 'http://54.148.78.219/bundles-non-auth/sage-pay/notification?id=' + 9999;
	} else {
		redirUrl = globals.context.session.host + '/bundles-non-auth/sage-pay/notification?id=' + id;
	}
	return redirUrl;
}
