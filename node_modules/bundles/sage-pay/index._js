...
var tracer = require('syracuse-trace/lib/helper').getTracer('sagePay');
...
exports.httpDispatch = function(_, request, response) {
	...
	var i = 0;
	while (i < notificationCache_.length) {
		var redir = notificationCache_[i];
		if (redir.id === id) {
			_deleteFromNotificationCache(id);
...
exports.createTokenRequest = function(_, tokenReqJson, registrationOptions) {
	...
	tokenReqJson.NotificationURL = _getNotificationPostUrl(registrationOptions.timeout || SPS_TIMEOUT_VALUE);

	var spsRawResponse = _postToSPS(_, tokenReqJson, registrationOptions);

	if (spsRawResponse) {
		_cacheSPSData(_, spsRawResponse, tokenReqJson.Vendor, registrationOptions.timeout || SPS_TIMEOUT_VALUE);
	}
...
exports.createPaymentRequest = function(_, paymentReqJson, registrationOptions) {
	...
	paymentReqJson.NotificationURL = _getNotificationPostUrl(registrationOptions.timeout || SPS_TIMEOUT_VALUE); 

	var spsRawResponse = _postToSPS(_, paymentReqJson, registrationOptions);

	if (spsRawResponse) {
		_cacheSPSData(_, spsRawResponse, paymentReqJson.Vendor, registrationOptions.timeout || SPS_TIMEOUT_VALUE);
	}
	return spsRawResponse;
};
...
exports.getSPSResponse = function(_, VPSTxId) {
	...
	debug_ && tracer.debug && tracer.debug("calling getSPSResponse with VPSTxId=" + VPSTxId);

	var responseX3;
	var spsData = _getSpsCacheData(VPSTxId);
	if (spsData && spsData.WaitStatus) {
		if (spsData.WaitStatus === SPS_WAIT_STATUS_OK) {
			...
			debug_ && tracer.debug && tracer.debug("getSPSResponse return value = " + responseX3);
		} else {
...
function _postToSPS(_, requestJson, registrationOptions) {
	...
	debug_ && tracer.debug && tracer.debug(requestQueryStr);
	...
	if (response.statusCode != 200) {
		...
		debug_ && tracer.debug && tracer.debug("rawResponse: " + rawResponse);
		return rawResponse;
	}
}

function _cacheSPSData(_, rawResponse, vendorName, spsTimeout) {
	var values = {};
	...
	if (values.Status === SPS_STATUS_OK) {
		...
		setTimeout.call(cacheValue, cacheValue.timeout, spsTimeout, VPSTxId);

		spsCache_.push(cacheValue);

		if (debug_ && tracer.debug) {
			Object.keys(cacheValue).map(function(key){
				tracer.debug("cache value: " + key + " = " + cacheValue[key]);
			});
		}
	}
}
...
function _notificationResponse(_, request, response) {
	...
	debug_ && tracer.debug && tracer.debug("_notificationResponse.requestRaw = " + requestRaw);

 	var spsResponseJson = _replyToSPSResponse(_, request, requestJson, response);

 	/*
 	 * We've responded to SPS, so cache our notification POST request to the SPS cache.
 	 */
 	 if (spsCache_ && requestJson.VPSTxId) {

		requestJson.VPSTxId = decodeURIComponent(requestJson.VPSTxId).replace(/{/g, '').replace(/}/g, '');

 		for (var ii = 0, jj = spsCache_.length; ii < jj; ii++) {
			if (spsCache_[ii].VPSTxId === requestJson.VPSTxId) {
				var spsData = spsCache_[ii];
				/*
				...
				spsData.notificationPostData = _buildDecodedQueryReq(requestJson);
...
function _replyToSPSResponse(_, request, requestJson, response) {
	/*
	 * get the content from the POST
	 */
	var postValidationJson = _validateNotificationPost(_, requestJson);

	debug_ && tracer.debug && tracer.debug("_replyToSPSResponse.postValidationJson = " + JSON.stringify(postValidationJson));
...
function _validateNotificationPost(_, reqJson) {
	/*
	...
	if (reqJson && reqJson.VPSTxId) {
		spsData = _getSpsCacheData(decodeURIComponent(reqJson.VPSTxId).replace(/{/g, '').replace(/}/g, ''));// remove the outer {}; // could happen if the timeout has been reached and deleted the cache entry

		debug_ && tracer.debug && tracer.debug("_validateNotificationPost.reqJson = " + JSON.stringify(reqJson));
	}
 	...
 	if (spsData && reqJson.Status) {
		...
		if (isValidMD5) {
			...
			if (debug_ && tracer.error){
				
				tracer.error("_validateNotificationPost.reqJson.VPSSignature = " + reqJson.VPSSignature);
				tracer.error("_validateNotificationPost.reqJson.VPSTxId = " + reqJson.VPSTxId);
				tracer.error("_validateNotificationPost.reqJson.VendorTxCode = " + reqJson.VendorTxCode);
				tracer.error("_validateNotificationPost.reqJson.Status = " + reqJson.Status);
				tracer.error("_validateNotificationPost.spsData.VendorName = " + spsData.VendorName);
				tracer.error("_validateNotificationPost.reqJson.Token = " + decodeURIComponent(reqJson.Token));
				tracer.error("_validateNotificationPost.spsData.SecurityKey = " + spsData.SecurityKey);
			}
...
function _validateMD5(_,  reqJson, spsData) { 
	...
	debug_ && tracer.debug && tracer.debug("MD5 hash of " + rawConcatenation + " = " + hashMD5);

	return hashMD5 && (hashMD5 === reqJson.VPSSignature);
}

function _getNotificationPostUrl(spsTimeout) {
	var id = require('syracuse-core/lib/uuid').generate('');
	var cacheValue = { id: id };
	cacheValue.timeout = function (id) {
			/*
			 * Setup a timer that will clear the cache entry for this VPSTxID after the specified timeout
			 */
			 _deleteFromNotificationCache(id);
	...
	notificationCache_.push(cacheValue);

	setTimeout.call(cacheValue, cacheValue.timeout, spsTimeout, id);

	var host = _getHost();
	var redirUrl = host + '/bundles-non-auth/sage-pay/notification?id=' + id;
	return redirUrl;
}

function _getSpsCacheData(VPSTxId) {
	if (spsCache_ && spsCache_.length > 0) {
		for (var ii = 0, jj = spsCache_.length; ii < jj; ii++) {
			if (spsCache_[ii].VPSTxId === VPSTxId) {
				if (debug_ && tracer.debug) {
					tracer.debug("getSPSResponse.spsCache_.VPSTxId=" + spsCache_[ii].VPSTxId);
					tracer.debug("getSPSResponse.spsCache_.VendorName=" + spsCache_[ii].VendorName);
					tracer.debug("getSPSResponse.spsCache_.SecurityKey=" + spsCache_[ii].SecurityKey);
					tracer.debug("getSPSResponse.spsCache_.NextURL=" + spsCache_[ii].NextURL);
					tracer.debug("getSPSResponse.spsCache_.WaitStatus=" + spsCache_[ii].WaitStatus);
				}
				return spsCache_[ii];
			}
		}
...
function _deleteFromNotificationCache(id) {
	if (notificationCache_ && id) {
		for (var ii = 0, jj = notificationCache_.length; ii < jj; ii++) {
			if (notificationCache_[ii].id === id) {
				notificationCache_.splice(ii, 1);

				if (debug_ && tracer.debug) {
					tracer.debug("_deleteFromNotificationCache for id: " + id);
					tracer.debug("Notification cache size: " + notificationCache_.length);
				}
				break;
			}
		}
...
function _deleteFromSpsCache(VPSTxId) {
	if (spsCache_ && VPSTxId) {
		for (var ii = 0, jj = spsCache_.length; ii < jj; ii++) {
			if (spsCache_[ii].VPSTxId === VPSTxId) {
				spsCache_.splice(ii, 1);

				if (debug_ && tracer.debug) {
					tracer.debug("_deleteFromSpsCache for VPSTxId: " + VPSTxId);
					tracer.debug("Sps cache size: " + spsCache_.length);
				}
