...
var http = require('ez-streams').devices.http,
	...
	url = require('url'),
	jsxml = require('jsxml'),
	config = require('syracuse-main/lib/syracuse').config,
...
exports.getRedirectUrl = function(_, values, ttl) {
	...
	if (config.hosting) {
		if (config.hosting.https) redirUrl.protocol = 'https';
		if (config.hosting.host) redirUrl.host = config.hosting.host;
	}

	redirUrl = url.format(redirUrl);
	console.log('\n URL = ' + redirUrl);
	return redirUrl;
};
