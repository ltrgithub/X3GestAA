...
exports.replyToOrtems = function(_, response, commsObj, doReply) {
	commsObj.future && commsObj.future(function() {});

	if (commsObj.data && doReply) {
		_sendResponse(_, response, commsObj.data, 200, commsObj.parsedUrl, readTransform.noTransform);
	} else {
		console.log("202");
		_sendResponse(_, response, {}, 202, commsObj.parsedUrl, readTransform.noTransform);
	}
};
...
exports.addToBuffer = function(_, parsedUrl, commsObj) {
	...
	var data = readTransform.transformData(_, parsedUrl, commsObj.data);
