	...
	if (commsObj.trackingDelay <= 0) {
		...
		if (commsObj.data) {
			...
			if ($diagnoses && $diagnoses.length > 0) {
				_replyError(_, response, commsObj, $diagnoses);
			} else {
...
function _replyError(_, response, commsObj, $diagnoses) {
	var wrapper = {};
	wrapper.$diagnoses = $diagnoses;
	tracer && tracer("_replyError: " + JSON.stringify(wrapper.$diagnoses, null, 2));
	commsObj.buffer = [];
	commsObj.buffer.push(wrapper.$diagnoses);
