...
const TRACKING_TIMER_DELAY = 4000;
const FUTURE_TIMER_DELAY = 500;

exports.$exported = true;

exports.httpDispatch = function(_, request, response) {
	return _httpDispatch(_, request, response);
};

// exports.httpDispatch.prepare = function(_, request) {
// 	var parsedUrl = utils.parseRequestUrl(_, request.url);
// 	var facet = utils.getFacet(_, parsedUrl.representation);
// 	request.useSessionPool = (facet === "$bulk");
// };

function _httpDispatch(_, request, response) {
...
function _processReadRequest(_, context, parsedUrl) {
	...
	commsObj.trackingDelay = TRACKING_TIMER_DELAY;
	commsObj.timer = commsObj.timer || setTimeout(_commsTimer, FUTURE_TIMER_DELAY, _, context.response, parsedUrl, commsObj);
}
...
function _commsTimer(_, response, parsedUrl, commsObj) {
	if (commsObj.trackingDelay <= 0) {
		...
		if (commsObj.data) {
			var $diagnoses = errorHelper.getErrorObjects(commsObj.data);
			if ($diagnoses && $diagnoses.length > 0) {
				_replyError(_, response, parsedUrl, commsObj, $diagnoses);
			} else {
			...
			commsObj.trackingDelay -= FUTURE_TIMER_DELAY;
			commsObj.timer = setTimeout(_commsTimer, FUTURE_TIMER_DELAY, _, response, parsedUrl, commsObj);
		}
	}
}
...
function _replyPartial(_, response, commsObj) {
	...
	chunkHelper.setNextChunkKey(_, commsObj);
...
function _replyError(_, response, parsedUrl, commsObj, $diagnoses) {
	var wrapper = {};
	wrapper.$diagnoses = $diagnoses;
	tracer && tracer("_replyError: " + JSON.stringify(wrapper, null, 2));
	commsObj.data = wrapper;
	comms.addToBuffer(_, parsedUrl, commsObj);
	comms.replyToOrtems(_, response, commsObj, 500);
	comms.deleteCommsSession(_, commsObj.trackingId);
}
...
function _processUpdateRequest(_, context, parsedUrl, request) {
	...
	var sendData = JSON.parse(request.readAll(_));

	commsObj.chunks = commsObj.chunks || chunkHelper.getChunkData(_, parsedUrl, sendData);

	tracer && tracer("_processUpdateRequest - chunks length: " + commsObj.chunks.length);

	if (commsObj.chunks && commsObj.chunks.length) {
		var chunk = writeTransform.transformData(_, parsedUrl, commsObj.chunks[0]);
		...
		var $diagnoses = errorHelper.getErrorObjects(res.body);
		if ($diagnoses && $diagnoses.length > 0) {
			var wrapper = {};
			wrapper.$diagnoses = $diagnoses;
			context.reply(_, 500, readTransform.transformData(_, parsedUrl, wrapper));
			comms.deleteCommsSession(_, commsObj.trackingId);
		} else {
			commsObj.chunks.splice(0, 1);
			if (commsObj.chunks.length) {
				context.reply(_, 206, readTransform.transformData(_, parsedUrl, res.body));
			} else {
				context.reply(_, 200, readTransform.transformData(_, parsedUrl, res.body));
				comms.deleteCommsSession(_, commsObj.trackingId);
			}
		}
	}
