...
function send(_, cookie, method, url, data, statusCode, step) {
	...
	var response = streams.httpRequest(opts);
	...
	if (response.statusCode === 301) {
		...
		response = streams.httpRequest(opts);
...
function basicGetTests(_, cookie) {
	send(_, cookie, "get", baseUrl + "/help/en-us/index.htm", null, 200, "Get online help main page");
	send(_, cookie, "get", baseUrl + "/help/en-us/ADXHelp_main.css", null, 200, "Get online help main CSS");
	send(_, cookie, "get", baseUrl + "/help/en-us/CANNOT_LOCATE.htm", null, 404, "Get bad page : TEMPORARY 500 INSTEAD OF 404");
...
asyncTest("strings", function(_) {
	...
	// This proxy doesn't work if the host in proxy config cannot be resolved
	// addProxyConf(_, cookie, url, "proxyTest");

	start();
});

asyncTest("GET Help index.htm", function(_) {
	cookie = getCookie(_, baseUrl);
