...
exports.dispatcher = function(config) {
	return function(_, request, response) {
		...
		if (helpConfig.local) {
			...
			if (!p) {
				return plainReply(response, 404, 'local help not correctly configured');
			} else {
				provider.push(p);
			}
		} else {
			...
			if (ep && ep.helpBaseUrl(_)) {
				...
				provider.push(helpProxyProvider(ep.helpBaseUrl(_), {}));
			}

			// Configuration provides the next base URL
			if (helpConfig.url) {
				provider.push(helpProxyProvider(helpConfig.url, {}));
			}

			// And finally fall back to the server in the cloud
			provider.push(helpProxyProvider("http://online-help.sageerpx3.com", {}));
		}
		...
		for (var k = 0; k < provider.length; k++) {
			for (var j = 0; j < urls.length; j++) {
				...
				request.url = urls[j];
				var helpResponse = provider[k](_, request);
