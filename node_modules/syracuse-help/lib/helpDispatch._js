...
var helpTrace = require("syracuse-trace/lib/helper").getTracer("help");
...
function helpProxyProvider(rootUrl, authent) {
	...
	return function(_, request) {
		if (authent) {
			...
			helpTrace.debug() && helpTrace.debug("helpProxyProvider: token=" + JSON.stringify(token, null, "\t"));
		...
		helpTrace.debug() && helpTrace.debug("helpProxyProvider: original request.url=" + request.url);
		var options = {
		...
		helpTrace.debug() && helpTrace.debug("helpProxyProvider: rootUrl=" + rootUrl + ", request.url=" + request.url + "\n>>> options=" + sys.inspect(options));
...
exports.dispatcher = function(config) {
	...
	return function(_, request, response) {
		if (!lic) {
			...
			helpTrace.debug() && helpTrace.debug("dispatcher: init serials=" + authent.serial);
		}
		var rel;
		...
		if (helpConfig.local) {
			...
			if (ep && ep.helpBaseUrl(_)) {
				helpTrace.debug() && helpTrace.debug("helpDispatcher: endpoint help url=[" + ep.helpBaseUrl(_) + "]");
		...
		helpTrace.debug() && helpTrace.debug("helpDispatcher: urls=[" + urls + "]");

		for (var k = 0; k < provider.length; k++) {
			for (var j = 0; j < urls.length; j++) {
				helpTrace.debug() && helpTrace.debug("helpDispatcher: Request number: " + j);
				request.url = urls[j];
				var helpResponse;
				try {
				...
				helpTrace.debug() && helpTrace.debug("helpDispatcher: is status code ok: " + (helpResponse && helpResponse.statusCode < 400));
				helpTrace.debug() && helpTrace.debug("helpDispatcher: is last request: " + (k === (provider.length - 1) && j === (urls.length - 1)));
