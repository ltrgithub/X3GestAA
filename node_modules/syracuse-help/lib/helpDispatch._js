...
function helpProxyProvider(rootUrl, authent) {
	...
	return function(_, request) {
		...
		tracer.debug && tracer.debug("helpProxyProvider: original request.url=" + request.url);
		var options = {
		...
		tracer.debug && tracer.debug("helpProxyProvider: rootUrl=" + rootUrl + ", request.url=" + request.url + "\n>>> options=" + sys.inspect(options));
