...
var helpTrace = require("syracuse-trace/lib/helper").getTracer(_, "help");
...
function helpProxyProvider(rootUrl, authent) {
	...
	return function(_, request) {
		if (authent) {
			...
			helpTrace.debug(_) && helpTrace.debug(_, "helpProxyProvider: token=" + JSON.stringify(token, null, "\t"));
		...
		helpTrace.debug(_) && helpTrace.debug(_, "helpProxyProvider: original request.url=" + request.url);
		var options = {
		...
		helpTrace.debug(_) && helpTrace.debug(_, "helpProxyProvider: rootUrl=" + rootUrl + ", request.url=" + request.url + "\n>>> options=" + sys.inspect(options));
...
exports.dispatcher = function(config) {
	...
	return function(_, request, response) {
		if (!lic) {
			...
			helpTrace.debug(_) && helpTrace.debug(_, "dispatcher: init serials=" + authent.serial);
		}
		var rel;
		...
		if (helpConfig.local) {
			...
			if (ep && ep.helpBaseUrl(_)) {
				helpTrace.debug(_) && helpTrace.debug(_, "helpDispatcher: endpoint help url=[" + ep.helpBaseUrl(_) + "]");
		...
		helpTrace.debug(_) && helpTrace.debug(_, "helpDispatcher: urls=[" + urls + "]");

		for (var k = 0; k < provider.length; k++) {
			for (var j = 0; j < urls.length; j++) {
				helpTrace.debug(_) && helpTrace.debug(_, "helpDispatcher: Request number: " + j);
				request.url = urls[j];
				var helpResponse;
				try {
				...
				helpTrace.debug(_) && helpTrace.debug(_, "helpDispatcher: is status code ok: " + (helpResponse && helpResponse.statusCode < 400));
				helpTrace.debug(_) && helpTrace.debug(_, "helpDispatcher: is last request: " + (k === (provider.length - 1) && j === (urls.length - 1)));
