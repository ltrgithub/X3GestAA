...
var tracer = require("syracuse-trace/lib/helper").getTracer("help");
...
function helpProxyProvider(rootUrl, authent) {
	...
	return function(_, request) {
		if (authent) {
			...
			tracer.trace.debug() && tracer.trace.debug("helpProxyProvider: token=" + JSON.stringify(token, null, "\t"));
		...
		request.headers['host'] = baseUri.host || request.headers['host'];

		tracer.trace.debug() && tracer.trace.debug("helpProxyProvider: original request.url=" + request.url);
		var options = {
		...
		tracer.trace.debug() && tracer.trace.debug("helpProxyProvider: rootUrl=" + rootUrl + ", request.url=" + request.url + "\n>>> options=" + sys.inspect(options));
...
exports.dispatcher = function(config) {
	...
	return function(_, request, response) {
		if (!lic) {
			...
			tracer.trace.debug() && tracer.trace.debug("dispatcher: init serials=" + authent.serial);
		}
		var rel;
		...
		if (helpConfig.local) {
			...
			if (ep && ep.helpBaseUrl(_)) {
				tracer.trace.debug() && tracer.trace.debug("helpDispatcher: endpoint help url=[" + ep.helpBaseUrl(_) + "]");
			...
			provider.push(helpProxyProvider("http://online-help.sageerpx3.com/erp", authent));
			providerName.push("http://online-help.sageerpx3.com/erp");
		}
		...
		tracer.trace.debug() && tracer.trace.debug("helpDispatcher: urls=[" + urls + "]");

		for (var k = 0; k < provider.length; k++) {
			for (var j = 0; j < urls.length; j++) {
				tracer.trace.debug() && tracer.trace.debug("helpDispatcher: Request number: " + j);
				request.url = urls[j];
				var helpResponse;
				try {
				...
				tracer.trace.debug() && tracer.trace.debug("helpDispatcher: is status code ok: " + (helpResponse && helpResponse.statusCode < 400));
				tracer.trace.debug() && tracer.trace.debug("helpDispatcher: is last request: " + (k === (provider.length - 1) && j === (urls.length - 1)));
