...
var PAGES = {
	...
	home: {
		...
		$links: {
			movies: {
				$title: "Movies",
				$url: "/sdata/movies/default/movies/movies?representation=movies.$query"
			},
			people: {
				$title: "People",
				$url: "/sdata/movies/default/movies/persons?representation=persons.$query"
			},
		}
	}
}
...
var protoBuilders = {
		...
		var proto = {
			...
			$links: {
				$create: {
					$title: "Create",
					$method: "POST",
					$url: "/sdata/movies/default/movies/" + name + "/$workingCopies?representation=" + name + ".$edit",
				},
			},
		};
		return proto;
	...
	$details: function(_, name) {
		...
		entity.$links.$query = {
			$title: "List",
			$url: "/sdata/movies/default/movies/" + name + "?representation=" + name + ".$query",
		};
		entity.$links.$edit = {
			$title: "Edit",
			$method: "POST",
			$url: "/sdata/movies/default/movies/" + name + "/$workingCopies/{$uuid}?representation=" + name + ".$edit",
		};
		return entity;
	},
	...
	$save: function(_, entity, wc, result) {
		...
		result.$diagnoses.push({
			...
			$links: {
				$details: {
					$title: "details",
					$url: "/sdata/movies/default/movies/" + entity + "/" + wc.$uuid + "?representation=" + entity + ".$details",
				}
			}
		});
	},
...
function workingCopyDispatcher(_, request, response, entity, path) {
	var uuid = request.parsedUrl.segments[7];
	var wc;
	...
	case 'POST':
		...
		wc.$url = "/sdata/movies/default/movies/" + entity + "/$workingCopies/" + uuid + "?representation=" + entity + ".$edit";
...
function dataDispatcher(_, request, response) {
	...
	var entity = segments[2].replace(/%27/g, "'");
	if (!entity) return notFound(_, request, response);
	var m = /^\$prototypes\('([^'.]*)\.([^'.]*)'\)/.exec(entity);
	if (m) {
		var builder = protoBuilders[m[2]];
		if (!builder) throw notFound(_, request, response);
		return jsonReply(_, response, 200, null, builder(_, m[1]));
	}
	var key = segments[3];
...
function sdataDispatcher(_, request, response) {
	var segments = request.parsedUrl.segments.slice(5);
	var entity = segments[0].replace(/%27/g, "'");
	if (!entity) return notFound(_, request, response);
	var m = /^\$prototypes\('([^'.]*)\.([^'.]*)'\)/.exec(entity);
	if (m) {
		var builder = protoBuilders[m[2]];
