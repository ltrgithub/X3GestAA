...
function registerTest(jsonFile, refPatchFile, includeInGlobalPatch, activityCodes) {
	unitTests.push({
		...
		includeInGlobalPatch: includeInGlobalPatch,
	});
}


registerTest("CONTENTTYPES/ATYPE0.json", "ATYP_ATYPE0.dat", true);

registerTest("ACTIVITYCODES/ABI.json", "ACV_ABI.dat", true);

// tables
registerTest("TABLES/ABANK.json", "ATB_ABANK.dat", true);

registerTest("CLASSES/TCAWRKHISSUI.json", "ACLA_TCAWRKHISSUI.dat", true);
registerTest("CLASSES/ACTXPARAM.json", "ACLA_ACTXPARAM.dat", true);
registerTest("WINDOWS/OACLA.json", "AWI_OACLA.dat", true);
registerTest("CONTEXTS/AFOLDER.json", "ACTX_AFOLDER.dat", true);
registerTest("CONTEXTS/AHISTO.json", "ACTX_AHISTO.dat", true);
registerTest("FUNCTIONS/ABATPAR.json", "AFC_ABATPAR.dat", true);
registerTest("GLOBVARS/G300CPFI.json", "AGB_G300CPFI.dat", true);
registerTest("INQUIRIES/AJE.json", "ACN_AJE.dat", true);
registerTest("OBJECTS/AAA.json", "AOB_AAA.dat", true);
registerTest("PARAMETERS/AASAUTNUM.json", "ADP_AASAUTNUM.dat", true);
registerTest("PROCESSES/1099MISC.json", "ADC_1099MISC.dat", true);
registerTest("REPORTS/ABLOB.json", "ARP_ABLOB.dat", true);
registerTest("REPORTS/ACODIF.json", "ARP_ACODIF.dat", true);

// Representations
registerTest("REPRESENTATIONS/ABANK.json", "ASW_ABANK.dat", true);

// Representations
registerTest("REPRESENTATIONS/SQUOTE.json", "ASW_SQUOTE.dat", true);

// Representations : validate ASHWMENU sub table + validate ASHWLNK with empty CODLNK
registerTest("REPRESENTATIONS/CONTACTCRM.json", "ASW_CONTACTCRM.dat", true);

// Representations : validate ASHWMET sub table
registerTest("REPRESENTATIONS/PPRICSEARCHM.json", "ASW_PPRICSEARCHM.dat", true);

// Representations : validate ASHWOPT sub table
registerTest("REPRESENTATIONS/SPRICCONF.json", "ASW_SPRICCONF.dat", true);

// Screens
registerTest("SCREENS/AAB1.json", "AMK_AAB1.dat", true);

// Screens : validate AMSKPAR sub table
registerTest("SCREENS/PTH1.json", "AMK_PTH1.dat", true);

// Validate partial activityCodes
// registerTest("SCREENS/PTH1.json", "AMK_PTH1_PAL_PP1.dat", false, ["PAL", "PP1"]);

registerTest("VIEWS/ADICADX.json", "AVW_ADICADX.dat", true);

registerTest("MISCTABLES/6.json", "ADV_6.dat", true);

registerTest("ACTIONS/ABIIMP.json", "ACT_ABIIMP.dat", true);
...
asyncTest('Generation of single patches, one patch file per JSON file', 2 * unitTests.length, function(_) {
	...
	unitTests.forEach_(_, function(_, unitTest) {
		var patchFile;
		try {
			patchFile = pg.generatePatches(_, [unitTest.json], {
				activityCodes: unitTest.activityCodes
			})[0];
			ok(true, "Generation of patch from JSON file : " + unitTest.json + ", result = " + patchFile);
		} catch (err) {
			ok(false, "Generation of patch from JSON file : " + unitTest.json + ", reason = " + err.stack);
			throw err;
		}
...
asyncTest('Generation of unique patch file, one patch file for many JSON file', 2, function(_) {
	...
	var files = unitTests.map(function(unitTest) {
		if (!unitTest.includeInGlobalPatch)
			return null;
		return unitTest.json;
	}).filter(function(file) {
		// Skip empty entries (those whose unitTest.includeInGlobalPatch == false)
		return file;
	});
