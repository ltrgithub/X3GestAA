...
exports.rebuildMetadataFile = function(_, conflictObject) {

	function transformObject(obj, key, textsHolder) {
		if (obj.$type == 'object') {
			var result = {};
...
exports.parseForConflicts = function(_, superv, entityType, jsonFile, options) {
	...
	function _findDifferences(_, superv, entity, propName, path, valLocal, valRemote, options) {
		...
		function indexArray(entity, array) {
			...
			array.forEach(function(instance) {
				var values = getOrderBy(entity).split(',').map(function(propName) {
				...
				if (indexedArray.hasOwnProperty(key)) {
					console.log(array);
					console.log(require("util").inspect(entity, false, null));
		...
		function getOrderBy(entity) {
			return entity.orderBy || entity.primaryKey || entity.patchPK;
		}
		...
		if (Array.isArray(valLocal)) {
			...
			if (entity.children && entity.children.hasOwnProperty(propName)) {
				...
				result = {
					$type: "sub-objects",
					$pk: getOrderBy(subEntity),
					$path: path,
					$values: _findDifferences(_, superv, subEntity, propName, path, arrLocal, arrRemote, options),
				};
			} else {
				// TODO !!!
				...
				for (var i = 0; i < Math.min(valLocal.length, valRemote.length); i++) {
					var subResult = _findDifferences(_, superv, entity, propName, path + "[" + i + "]", valLocal[i], valRemote[i], options);
					if (subResult) {
			...
			Object.keys(valLocal).filter(function(key) {
				...
				var subResult = _findDifferences(_, superv, entity, childPropName, path + "/" + childPropName, propLocal, propRemote, options);
				if (subResult) {
	...
	var result = _findDifferences(_, superv, entity, "", "", localObject, remoteObject, options);
