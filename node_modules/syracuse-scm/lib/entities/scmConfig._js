...
var supervisor = require("etna-supervisor/lib/supervisor"),
	...
	date = require("syracuse-core/lib/types/date"),
	...
	$services: {
		getSyncInfos: {
			...
			$execute: function(_, context, instance) {
				...
				var etnaConfig = data.etnaConfig; // TEMP HACK
				...
				try {
					...
					var tableDef = superv.sqlDriver.getTableDef(_, etnaConfig.folderName, settingsTableName);
				...
				try {
					superv.sqlDriver.withConnection(_, function(_, cnx) {
						var row = superv.sqlDriver.reader(_, cnx, "select VALUE from " + superv.sqlDriver.getFullTableName(etnaConfig.folderName, settingsTableName) + " where ID = " + superv.sqlDriver.param(0), ["PATCH.SYNC.LAST.GIT.HEAD"]).toArray(_)[0];
		...
		importPatch: {
			...
			$execute: function(_, context, instance) {
				...
				patch.type(_, "X3");
				patch.description(_, "Patch for SCM");
				patch.path(_, data.patchName);
				...
				try {
					update.applyUpdate(_, context, update, true, options);
				} catch (err) {
					console.log("XXXXXXXXXXXX Error : " + err.message);
					context.tracker.addError(err.message);
				}
			...
			$execute: function(_, context, instance) {
				...
				if (result.atLeastOneConflictFixed && !result.hasConflicts) {
					...
					if (data.autoSaveFixedFile) {
						...
						// We have to transofmr it bak to a metadataObject to be able to save it
