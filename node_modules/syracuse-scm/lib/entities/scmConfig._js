	...
	$services: {
		...
		saveResolvedFile: {
			...
			$execute: function(_, context, instance) {
				var data = JSON.parse(context.request.readAll(_));
				var endPointName = data.endPoint;
				var entityType = jsonHelper.parseMetaFilename(data.filename).type;
				var json = jsonHelper.sortJsonObject(_, data.json, entityType);
				var endPoint = _getEndpoint(_, context, endPointName);
				console.log("SAVING FILE");
				fs.writeFile(data.filename, JSON.stringify(json, null, '\t'), "utf8", _);
				return {
					result: "OK"
				};
			},
		...
		parseForConflicts: {
			...
			$execute: function(_, context, instance) {
				...
				return {
					json: conflictHelper.parseForConflicts(_, superv, entityType, filename),
