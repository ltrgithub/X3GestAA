	...
	$services: {
			...
			$execute: function(_, context, instance) {
				...
				try {
					superv.sqlDriver.withConnection(_, function(_, cnx) {
						...
						if (!row)
							throw new Error("Could not detect the product");
						productId = parseInt(row.VALEUR_0);
					});
				} catch (err) {
					console.log(err.stack);
					throw new Error("Could not retrieve the product, reason = " + err.message);
				}
				superv.release(_);
		...
		importPatches: {
			...
			$execute: function(_, context, instance) {
				...
				var description = "SCM Patch, imported @" + new Date().toISOString() + " (" + data.fromSha1 + " -> " + data.toSha1 + ")";
				...
				var applyResult = update.applyPatches(_,
					...
					data.endPoint, [{
						...
						type: "SCM",
						description: data.fromSha1 + " -> " + data.toSha1,
