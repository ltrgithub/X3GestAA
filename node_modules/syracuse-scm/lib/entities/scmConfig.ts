...
import { Driver, DriverConfig, TableDefinition } from '@sage/etna-sql';
const { date } = types;
...
interface Endpoint extends Instance {
	dataset(_: _): string;
	getEtnaConfig(_: _): DriverConfig;
...
function _getProductId(_: _, sqlDriver: Driver) {
	var productId: any;
	try {
		sqlDriver.withConnection(cnx => {
			var row: any = sqlDriver.reader(cnx, "SELECT VALEUR_0 FROM ADOVAL WHERE PARAM_0=" + sqlDriver.param(0), ['TYPDOS']).toArray()[0];
			if (!row)
...
function createSqlDriver(etnaConfig): Driver {
	return require('@sage/etna-' + etnaConfig.sql.driver).create(etnaConfig.sql);
}
	...
	$services: {
		syracuseIsReady: {
			...
			$execute(_: _, context: Context, instance: Instance): any {
				try {
					...
					var sqlDriver = createSqlDriver(etnaConfig);
					var result = true;
					try {
						sqlDriver.withConnection(cnx => {
							sqlDriver.reader(cnx, "select CONTROLFILE_TYPE from v$database").toArray()[0];
						});
					} catch (err) {
						result = false;
					}
		...
		getColumnsLocalizedLabelCodes: {
			...
			$execute(_: _, context: Context, instance: Instance) {
				...
				try {
					sqlDriver.withConnection(cnx => {
						const rows = sqlDriver.reader(
							cnx,
							"select CODZONE_0, NOLONG_0 from ATABZON where CODFIC_0 = " + sqlDriver.param(0), [data.tablename]).toArray();
						if (!rows)
		...
		getProductId: {
			...
			$execute(_: _, context: Context, instance: Instance) {
				...
				const result = {
					productId: _getProductId(_, sqlDriver),
				};
				sqlDriver.release();
				return result;
			}
		...
		getSyncInfos: {
			...
			$execute(_: _, context: Context, instance: Instance) {
				...
				try {
					...
					var tableDef = sqlDriver.getTableDef(etnaConfig.folderName, "ASCMSETTINGS");
				...
				if (createTable) {
					...
					sqlDriver.createTableFromTableDefinition(tableDef);
				}
				...
				try {
					sqlDriver.withConnection(cnx => {
						var row: any = sqlDriver.reader(cnx, "SELECT VALUE FROM ASCMSETTINGS WHERE KEY=" + sqlDriver.param(0), ['PATCH.SYNC.LAST.GIT.HEAD']).toArray()[0];
						if (row)
				...
				result.productId = _getProductId(_, sqlDriver);
		...
		setLastSyncdGitHead: {
			...
			$execute(_: _, context: Context, instance: Instance) {
				...
				try {
					sqlDriver.withConnection(cnx => {
						sqlDriver.execute(cnx, "delete from ASCMSETTINGS where KEY = " + sqlDriver.param(0), ["PATCH.SYNC.LAST.GIT.HEAD"]);
						var valueToWrite = JSON.stringify({
							sha1: sha1,
							date: new Date().toISOString()
						});
						var sql = "insert into ASCMSETTINGS (KEY, VALUE) values (" + sqlDriver.param(0) + "," + sqlDriver.param(1) + ")";
						var params = ["PATCH.SYNC.LAST.GIT.HEAD", valueToWrite];
						sqlDriver.execute(cnx, sql, params);
					});
				} catch (err) {
