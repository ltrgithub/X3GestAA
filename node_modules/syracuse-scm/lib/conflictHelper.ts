...
export function parseForConflicts(_: _, sqlDriver: Driver, entityType: string, jsonFile: string, options?: Options) {

	var atLeastOneConflictFixed = false;

	var localizedLabels = {} as Dict<string>;

	function _findDifferences(_: _, sqlDriver: Driver, entityDescriptor: Descriptor, propName: string, path: string, valLocal: any, valRemote: any, options?: Options) {
		...
		function getLocalizedLabel(_: _, tableName: string, columnName: string) {
			if (!localizedLabels[tableName]) {
				...
				sql += " WHERE z.CODFIC_0=" + sqlDriver.param(0) + " AND t.LAN_0 = " + sqlDriver.param(1);
				type Row = {
					ZONE: string;
					TEXT: string;
				}
				sqlDriver.withConnection(cnx => {
					sqlDriver.execute(cnx, sql, [tableName, options.langage || "FRA"]).forEach((row: Row) => {
		...
		if (Array.isArray(valLocal)) {
			...
			if (entityDescriptor.children && entityDescriptor.children.hasOwnProperty(propName)) {
				...
				result = {
					...
					$path: path,
					$values: _findDifferences(_, sqlDriver, subEntityDescriptor, propName, path, arrLocal, arrRemote, options),
				};
			} else {
				// TODO !!!
				...
				for (var i = 0; i < Math.min(valLocal.length, valRemote.length); i++) {
					var subResult = _findDifferences(_, sqlDriver, entityDescriptor, propName, path + "[" + i + "]", valLocal[i], valRemote[i], options);
					if (subResult) {
			...
			Object.keys(valLocal).filter(function (key) {
				...
				var subResult = _findDifferences(_, sqlDriver, entityDescriptor, childPropName, path + "/" + childPropName, propLocal, propRemote, options);
				if (subResult) {
	...
	var result = _findDifferences(_, sqlDriver, entityDescriptor, "", "", localObject, remoteObject, options);
