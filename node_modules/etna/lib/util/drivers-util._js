...
exports.tdsAddParameters = function(req, args) {
	...
	for (var i = 0; args && i < args.length; i++) {
		if (args[i] instanceof Buffer)
			processParam("p" + i, tds.TYPES.Binary, args[i]);
		else if (args[i] instanceof Blbfile)
			processParam("p" + i, tds.TYPES.VarBinary, args[i].value);
		else if (args[i] instanceof Date)
			processParam("p" + i, tds.TYPES.DateTime, args[i]);
		else if ((typeof args[i]) === "number")
			processParam("p" + i, tds.TYPES.Int, args[i]);
		else
			processParam("p" + i, tds.TYPES.NVarChar, args[i]);
	}
