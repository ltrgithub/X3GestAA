	...
	var superv = {
		...
		loadScript(_, name, clas) {
			...
			if (!script) {
				...
				script = tryX3js(_, fsp.join(__dirname, '../../test/x3js/fixtures', clas.jsName + ".x3js"));
				// second x3js try with local directory
				if (!script && superv.directories.jsLocal && clas) 
					script = tryX3js(_, fsp.join(superv.directories.jsLocal, moduleName, clas.jsName + '.x3js'));
				// third x3js try with shared directory
				if (!script && clas) 
					script = tryX3js(_, fsp.join(superv.directories.jsShared, moduleName, clas.jsName + '.x3js'));
