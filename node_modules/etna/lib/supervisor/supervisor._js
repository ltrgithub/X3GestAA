	...
	var superv = {
		...
		load(_, className, keyValue, dataChildren) {
			...
			if (!data && className === 'Class') {
				...
				// second try from source
				if (typeof Proxy !== 'undefined') {
					data = require('etna/lib/x3js/classMeta').loadData(_, this, keyValue);
					//console.error("LOADDATA RESULT", data);
				}
			}
			if(data) {
		...
		loadClassByJsName(_, jsName) {
			...
			var code = util.jsNameToCode(this, jsName);
			if (!code) {
