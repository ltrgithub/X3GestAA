	...
	var superv = {
		...
		loadClassByJsName(_, jsName) {
			...
			if (script.default.dictCode) {
				...
				const data = require('../x3js/classMeta').loadData(_, this, script.default);
		...
		loadJsScript(_, subdir, jsName) {
			...
			const path = segs.map(util.camelToKebab).join(fsp.sep);
			var script = null;
			...
			if (superv.directories.jsLocal) 
				script = tryX3js(_, fsp.join(superv.directories.jsLocal, path + '.x3js'));
			if (!script)
				script = tryX3js(_, fsp.join(__dirname, '../../test/x3js/fixtures', path + ".x3js"));
			if (!script) 
				script = tryX3js(_, fsp.join(superv.directories.jsShared, path + '.x3js'));
		...
		loadJsScriptSync(subdir, jsName) {
			...
			if (superv.directories.jsLocal) 
				script = tryX3js(fsp.join(superv.directories.jsLocal, path + '.x3js'));
			if (!script)
				script = tryX3js(fsp.join(__dirname, '../../test/x3js/fixtures', path + ".x3js"));
			if (!script) 
				script = tryX3js(fsp.join(superv.directories.jsShared, path + '.x3js'));
			if (script && !script.default) throw new Error(`script ${subdir}.${jsName} does not export default`);
			return script;
		},
		loadScript(_, name, clas) {
			...
			if (!script) {
				...
				const moduleNames = x3Modules[this.product];
				if (!moduleNames) throw new Error("bad contract: " + config.contract);
				const moduleName = clas ? moduleNames[clas.data.MODULE] : null;
				if (moduleName === undefined) throw new Error("bad module: " + clas.data.MODULE);
				const jsName = (clas && /_CSTD$/.test(name)) ? moduleName + '$' + clas.jsName : name;
				// x3js try with local directory
				if (clas) {
					script = this.loadJsScript(_, 'classes', jsName);
					if (script)	{
						clas.jsModules.push(script.default);
						clas.fixJsNames(_, script.default);
					}
		...
		initSysVar:function(_,actx) {
			...
			var tuls = actx.$APARAM(_)._getParams(_, 1, this.folderName, parameters, "AUTO");
