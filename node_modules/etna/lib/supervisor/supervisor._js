	...
	const superv = {
		...
		languages(_) {
			if (!this._languages) {
				...
				this._languages = this.sqlReader(_, "select " + columns.join(',') + " from ADOSSIER where (DOSSIER_0=" + this.sqlDriver.param(0) + ")", [this.folderName]).limit(1).toArray(_).reduce_(_, function (_, r, record) {
