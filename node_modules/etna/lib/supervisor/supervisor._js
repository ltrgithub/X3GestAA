...
exports.create = function(_, config) {
	...
	var superv = {
		...
		loadClassByJsName: function(_, jsName) {
		...
		loadScript: function(_, name, clas) {
			this.loadBuiltins(_);
			var script = cache.scripts[name];
			if (!script) {
				var path = config.solutionPath + "/" + config.folderName + "/scripts/" + name + ".x3js";
				// temporary hack to load scripts from test fixtures directory
				if (this.api && !fs.exists(path, _, _)) 
					path = fsp.join(__dirname, '../../test/x3js/fixtures/' + name + ".x3js");
				if (this.api && fs.exists(path, _, _)) {
					script = require(path);
					clas.jsModules.push(script.default);
				} else {
					path = config.solutionPath + "/" + config.folderName + "/TRT/" + name + ".src";
					tracerJs.debug && tracerJs.debug("Loading script: " + path);
					var source = fs.readFile(path, "utf8", _);
					var parsed = new(require("etna/lib/engine/parser").Parser)(source, path).parse();
					var transformed = require("etna/lib/engine/jsgen").generate(parsed.node, path);
		...
		loadFolderCache:function(_) {
			...
			var data =  this.mongoStore.collection('X3FoldersCache', _).find({
				folderName: this.folderName
			}).toArray(_)[0];
			if (data) {
				tracerJs.debug && tracerJs.debug("find in mongo folder: " + folderName + "  " + data.etag);
				return this.folderCache = data.folder;
			}	
			return {};
		},
		updateFolderCache:function(_,folder) {
		...
		initSysVar:function(_,actx) {
			if(this.sys && this.sys.ADXTUL) return; // sys is defined
	...
	function _createLocalFolder(_) {
		const root = config.etna && config.etna.localRoot;
		if (!root) return;
		superv.productName = config.etna.product;
		if (!superv.productName) throw new Error("etna.product key missing in nodelocal.js");
		superv.modulesNames = x3Modules[superv.productName];
		if (!superv.modulesNames) throw new Error("invalid etna.product key: " + superv.productName);
		superv.directories.jsLocal = fsp.join(root, superv.supervisor.solutionName, 'folders', superv.supervisor.folderName, 'node_modules');
