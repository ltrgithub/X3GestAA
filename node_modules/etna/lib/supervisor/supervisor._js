	...
	var superv = {
		...
		loadScript(_, name, clas) {
			...
			if (!script) {
				...
				if (clas && !moduleName) throw new Error("bad module: " + clas.data.MODULE);
				// first x3js try with fixtures hack
				script = tryX3js(_, fsp.join(__dirname, '../../test/x3js/fixtures/classes', clas.kebabName + ".x3js"));
				// second x3js try with local directory
				if (!script && superv.directories.jsLocal && clas) 
					script = tryX3js(_, fsp.join(superv.directories.jsLocal, moduleName, 'classes', clas.kebabName + '.x3js'));
				// third x3js try with shared directory
				if (!script && clas) 
					script = tryX3js(_, fsp.join(superv.directories.jsShared, moduleName, 'classes', clas.kebabName + '.x3js'));
