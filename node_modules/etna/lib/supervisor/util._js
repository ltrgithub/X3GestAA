...
function loadMappings(supervisor) {
	if (supervisor.jsMappings) return;
	if (supervisor.directories.jsLocal) {
		supervisor.jsMappings = require(supervisor.directories.jsLocal + '/name-mappings.json'); 
	} else {
		supervisor.jsMappings = require('etna/test/x3js/fixtures/name-mappings.json');
	}
	const jsMappings = supervisor.jsMappings;
	supervisor.invJsMappings = Object.keys(jsMappings).reduce((r, k) => {
		r[jsMappings[k][0]] = k;
		return r;
	}, {}); 
}

exports.fixJsName = function(supervisor, s1, s2, def) {
	loadMappings(supervisor);
	const jsMappings = supervisor.jsMappings;
	if (s2) return (jsMappings[s1] && jsMappings[s1][1] && jsMappings[s1][1][s2]) || def;
	else return (jsMappings[s1] && jsMappings[s1][0]) || def;
}

exports.jsNameToCode = function(supervisor, name) {
	loadMappings(supervisor);
	return supervisor.invJsMappings[name];
}

exports.camelToKebab = function(str) {
	const s = str.replace(/[A-Z]/g, ch => '-' + ch.toLowerCase());
