...
const jsMappings = require('etna/test/x3js/fixtures/name-mappings.json'); 
...
const invJsMappings = Object.keys(jsMappings).reduce((r, k) => {
	r[jsMappings[k][0]] = k;
	return r;
}, {}); 

exports.jsNameToCode = function(name) {
	return invJsMappings[name];
}

exports.camelToKebab = function(str) {
	return str.replace(/(.)([A-Z])/g, (all, ch1, ch2) => ch1 + '-' + ch2.toLowerCase());
}
