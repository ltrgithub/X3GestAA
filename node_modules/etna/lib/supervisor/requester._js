...
var evalue = require("etna/lib/engine/runtime/evaluate").evalue;
var datetime = require('etna/lib/engine/runtime/tdatetime');
...
exports.query = function(_, httpContext, request_code, query) {
	...
	*/
	var getExecutionTime = function(frq, forced_exec) {
	...
	var getPortalValue = function(record, cod, fields) {
	...
	var executeProg = function(_, fct, args) {
		...
		var result = runtime.executeProg(_, mod, fct, args);
	...
	var getTblSize = function(_) {
		var sqlDriver = superv.sqlDriver;
		var param = sqlDriver.param;
		var tableName = executeProg(_, "GET_MAINTABLE");
		var tblSize = sqlDriver.evalueRowCount(_, tableName);
	...
	var ifbulk = 0;
	var ifcube = (qs.cube && qs.cube === "true") ? 1 : 0;
	...
	var forced_exec = (qs.forcedExecution && qs.forcedExecution.toLowerCase() === "true") || false;
	var where = qs.Where || "";
	...
	var requid = qs.requestID ? qs.requestID.substring(1, qs.requestID.length - 1) : "";
	var startIndex = qs.startIndex ? parseInt(qs.startIndex) : 0;
	var startRecord = qs.startRecord ? parseInt(qs.startRecord) : 0;

	var firstline = Math.max(startRecord, startIndex, 1);
