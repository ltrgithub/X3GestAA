...
class BatchContext extends Helper {
	...
	constructor(resource: types.ResourceNode, superv: Supervisor, qs: any) {
		super();
...
const routes = [{
	...
	handler(_: _,  context: HttpContext, codcla: string) {
		...
		const data: types.Feed = JSON.parse(context.request.readAll(_));
		const batchResponse: types.Feed = { $resources: [] };
