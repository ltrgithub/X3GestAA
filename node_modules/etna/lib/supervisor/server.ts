...
import { ResourceNode, Feed } from './protocol/data';
import * as fs from 'fs';
import { FACETS } from './meta/facets';
import { WorkingCopy } from './dyn/workingCopy';
import { Class } from './meta/class';
import { Representation } from './meta/representation';
const glob = require('streamline/lib/globals');
import * as tuuid from '../engine/runtime/tuuid';
import * as supervisor from './supervisor';
import * as util from './util';
const Helper = util.Helper;
import * as bactx from '../supervisor/builtins/ACTX';
...
function loadType<T>(_: _,  context: HttpContext, type: string, code: string) {
	if (!code) throw new util.HttpError(400, type + " parameter missing");

	const clas = context.supervisor.load<T>(_, type, code);

	if (!clas) throw new util.HttpError(404, type + " not found: " + code);
	return clas;
}

function loadClass(_: _,  context: HttpContext, codcla: string) {
	return loadType<Class>(_, context, 'Class', codcla);
}

function loadRepresentation(_: _,  context: HttpContext, codrep: string) {
	return loadType<Representation>(_, context, 'Representation', codrep);
}
...
class HttpContext extends Helper {
	...
	request: any; // refine later
	response: any; // refine later
	qs: any;
	codrep: string;
	facet: string;
	constructor(request: any, response: any, superv?: Supervisor, qs?: any) {
		super();
	...
	get private() {
		if (!this.request.session) throw new util.HttpError(500, "session missing");
		return this.request.session.private = this.request.session.private || {
			workingCopies: {}
		};
	}
	workingCopy(trackingId: string) {
	...
	reply(_: _,  status: number, data?: any, headers?: Headers) {
		headers = headers || {};
	...
	jsonReply(_: _,  status: number, data: any, headers?: Headers) {
		headers = headers || {};
		headers["content-type"] = headers["content-type"] || "application/json";
		this.response.writeHead(status, headers);
...
class BatchContext extends Helper {
	...
	constructor(resource: ResourceNode, superv: Supervisor, qs: any) {
		super();
		this.resource = resource;
		this.request = {
			method: resource.$httpMethod,
			readAll: (_: _) => JSON.stringify(resource),
		};
	...
	checkMethod(method: string) {
		if (this.getMethod() !== method) throw new util.HttpError(405, "bad method " + method);
	}
	get private() {
		if (!this.request.session) throw new util.HttpError(500, "session missing");
		return this.request.session.private = this.request.session.private || {
...
function postWorkingCopy(_: _,  context: HttpContext, codcla: string, key?: any) {
	...
	const wc = workingCopies[qs.trackingId] || (workingCopies[qs.trackingId] = context.supervisor.new<WorkingCopy>(_, 'WorkingCopy', res, qs.trackingId));
...
const routes = [{
	pattern: /^(\$prototypes\(''\))?$/,
	handler(_: _,  context: HttpContext) {
		const lis = context.supervisor.sqlReader<{
			CODREP: string;
			CODCLA: string;
			INTREP: number;
		}>(_, "select R.MODULE_0, R.CODREP_0, R.CODCLA_0, R.INTREP_0, C.TYPCLA_0 " + //
	...
	handler(_: _,  context: HttpContext, keyword: string, keyvalue: string, facet: string) {
	...
	handler(_: _,  context: HttpContext, codrep: string, facet: string) {
		context.checkMethod("GET");
		var proto = {};
	...
	handler(_: _,  context: HttpContext, codcla: string, property: string, codblb: string, ident1: string, ident2: string) {
	...
	handler(_: _,  context: HttpContext, codcla: string, property: string, codblb: string, ident1: string) {
		const cl = loadClass(_, context, codcla);
	...
	handler(_: _,  context: HttpContext, codcla: string, property: string, codblb: string) {
		const cl = loadClass(_, context, codcla);
	...
	pattern: /^(\w*)(\('[^']*'\))?$/,
	handler(_: _,  context: HttpContext, codcla: string, key: string) {
		key = parseKey(key);
		if (codcla === 'QUERY') {
			context.checkMethod("GET");
			...
			context.checkMethod("GET");
			let res = loadModule("statistics").query(_, context, key, context.codrep);
		...
		if (facet !== "$search") {
			...
			if (['$query', '$edit'].indexOf(facet) >= 0 && method === 'DELETE') {
				...
				if (method === 'POST') {
					...
					var wc = context.supervisor.new<WorkingCopy>(_, 'WorkingCopy', res, tuuid.generate());
	...
	handler(_: _,  context: HttpContext, codcla: string) {
	...
	handler(_: _,  context: HttpContext, user: string) {
	...
	handler(_: _,  context: HttpContext, trackingId: string) {
		const method = context.getMethod();
	...
	handler(_: _,  context: HttpContext, trackingId: string, path: string) {
	...
	handler(_: _,  context: HttpContext, codcla: string, key: string, service: string) {
		key = parseKey(key);
		const codrep = context.codrep,
		      facet = context.facet,
		      qs = context.qs;
	...
	handler(_: _,  context: HttpContext, codcla: string) {
		...
		const data: Feed = JSON.parse(context.request.readAll(_));
		const batchResponse: Feed = { $resources: [] };

		data.$resources && data.$resources.forEach_(_, (_, resource) => {
			const m = /^\/sdata\/(?:etna|x3)\/erp\/(\w+)\/([^?]*)\??(.*$)/.exec(resource.$url);
			if (!m) throw new util.HttpError(400, "invalid $batch URL: " + resource.$url);
			...
			for (var i = 0; i < routes.length; i++) {
				...
				var r: any[] = routes[i].pattern.exec(path);
				if (r) {
					try {
...
// TODO: refine typing
export function httpDispatch(_: _,  config: any, request: any, response: any) {
	try {
		...
		if (ex.statusCode >= 400 && ex.statusCode < 500) console.error('http status: ' + ex.statusCode + ': ' + ex.message);
