...
function loadType(_: _,  context, type, code) {
...
function loadClass(_: _,  context, codcla) {
	return loadType(_, context, 'Class', codcla);
}

function loadRepresentation(_: _,  context, codrep) {
...
class HttpContext extends Helper {
	...
	jsonReply(_: _,  status, data, headers) {
		headers = headers || {};
		headers["content-type"] = headers["content-type"] || "application/json";
		this.response.writeHead(status, headers);
...
function postWorkingCopy(_: _,  context, codcla, key) {
...
const routes = [{
	...
	handler(_: _,  context, keyword, keyvalue, facet) {
	...
	handler(_: _,  context, codrep, facet) {
		context.checkMethod("GET");
		var proto = {};
		const headers = {};
	...
	handler(_: _,  context, codcla, property, codblb, ident1, ident2) {
	...
	handler(_: _,  context, codcla, property, codblb, ident1) {
		const cl = loadClass(_, context, codcla);
	...
	handler(_: _,  context, codcla, property, codblb) {
		const cl = loadClass(_, context, codcla);
	...
	pattern: /^(\w*)(\('[^']*'\))?$/,
	handler(_: _,  context, codcla, key) {
		key = parseKey(key);
		if (codcla === 'QUERY') {
			context.checkMethod("GET");
	...
	handler(_: _,  context, trackingId) {
		const method = context.getMethod();
	...
	handler(_: _,  context, trackingId, path) {
	...
	handler(_: _,  context, codcla, key, service) {
		key = parseKey(key);
		const codrep = context.codrep,
		      facet = context.facet,
		      qs = context.qs;
...
export function httpDispatch(_: _,  config, request, response) {
	try {
