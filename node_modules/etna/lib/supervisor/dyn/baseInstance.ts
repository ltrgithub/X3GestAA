...
import { Expression, InstanceIndex } from '../../engine/runtime/types';
...
import { trace } from './diagnosis';
import { BaseClass } from '../meta/baseClass';
import { Instance } from './instance';
import { Property } from '../meta/property';
import { Diagnosis, Slot, SlotAtextra, SlotText, SlotClob, SlotBlob, SlotReference, SlotInstance, SlotCollection } from './types';
import { ReferenceNode } from '../protocol/data';
import { Supervisor } from '../supervisor';
const traceSup = core.getTracer("etna.supervisor");
const traceGET = core.getTracer("etna.accessorsGet");
const traceSET = core.getTracer("etna.accessorsSet");
const tracerEvents = core.getTracer("etna.events");
...
export interface SysProperties {
	AERROR?: SysProperty;
	ACTX?: SysProperty;
	APARENT?: SysProperty;
	ASTALIN?: SysProperty;
...
export type PropertyWalker = (_: _, instance: BaseInstance, property: string) => void;
...
export class BaseInstance extends util.Helper {
	...
	createSlot(property: Property): Slot {
		switch (property.slotType) {
			case 'reference':
				// typing hack until we fix SlotReference inheritance
				return new SlotReference(this as any, property) as any;
			case 'instance':
				return new SlotInstance(this, property);
			case 'collection':
				const collectionSlot = new SlotCollection(this, property.collection);
				collectionSlot.proName = property.name;
				collectionSlot.proJsName = property.jsName;
				// typing hack until wi fix SlotCollection inheritance
				return collectionSlot as any;
			default:
				if (property.isAtextra) {
					return new SlotAtextra(this, property);
				} else if (property.isText) {
					return new SlotText(this, property);
				} else if (property.type && property.type.isClob) {
					return new SlotClob(this, property);
				} else if (property.type && property.type.isBlob) {
					return new SlotBlob(this, property);
				} else {
					return new Slot(this, property);
				}
		}
	}
	...
	ensureSlot(name: string) {
		...
		if (this.properties[name]) {
			slot = this.slots[name] = this.createSlot(this.properties[name]);
		...
		if (!this.sysProperties[name]) {
			if (name === 'AERROR') {
				...
				this.sysProperties.ACTX = {
					get: (_) => {
						return util.currentContext().x3session.actx;
					},
					set: (_, val) => {
				...
				this.sysProperties.ASTALIN = {
					...
					set: (_, astalin) => {
						...
						if (this.$parent && (this.$parent as any).collection && !(this.$parent as any).collection.mappingData) {
	...
	setColumnAttribute(_: _, collection: string, column: string, attribute: string, value: any) {
		// TODO: eliminate cast to anyinstanceOf creates circular dependency here
		const slot: any = this.slot(_, collection);
		if (slot.setColumnAttribute) return slot.setColumnAttribute(column, attribute, value);
	}
	...
	withControl(_: _, fname: string, f: (_: _) => number) {
		try {
			...
			var isAllowed = (this as any)["assert" + fname + "Allowed"];
	...
	propertyEvent(action: string) {
		return (_: _, prop: Slot | Property, oldValue: any, newValue: any) => {
			...
			var status = constants.CST_AOK;
			if (prop.type && (prop.type as any)[action]) {
				// The slot has to be created in this case :
				var slot = (prop as any).property ? prop : this.slot(_, prop.name);
				status = (prop.type as any)[action](_, slot);
				if (status === constants.CST_AERROR) return status;
			}

			// TODO: cleanup any cast hack
			const args = this.getActionArgs(_, "ARULE", action, null, prop as any);
	...
	addDiagnose(_: _, severity: number, message: string, category?: number) {
		...
		this.diagnoses.push(this.supervisor.new<Diagnosis>(_, 'Diagnosis', severity, message, "", category));
		return severity;
	}
	...
	getDiagnoses(_: _, sort?: string) {
		...
		return Object.keys(this.slots).reduce_(_, (_, r, property) => {
			...
			return r;
		}, local).sort((d0, d1) => (d0 as any)[sortMethod](d1));
	}
	$AREAD(_: _) {
	...
	manageSlots(_: _, operation: string) {
		Object.keys(this.slots).forEach_(_, (_, name) => {
			(this.slots[name] as any)[operation] && (this.slots as any)[name][operation](_);
		});
	...
	collectionSlot(_: _, collection: string): SlotCollection {
		return this.slot(_, collection) as any;
	}
	/* 
 ...
 */
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				...
				.reduce_(_, (_, r, property) => {
					...
					if (diags) r = r.concat(diags);
					return r;
				}, this.diagnoses || []).sort((d0, d1) => (d0 as any)[sort](d1));
			case constants.CST_ALL:
			default:
	...
	ASEARCHINFOS(_: _, local: number, property: string, status: number, selstatus: number) {
		const errt = (this as any).getSortedDiagnoses(_, local).filter_(_, (_: _, e: any) => {
			var ftr = true;
	...
	xmethMethods(_: _, name: string, args: Expression<any>[]) {
		const vals = basic.instructions.A(args)(_);
		const r = (this as any)[name](_, vals);
