...
import { ProgArg } from '../../engine/runtime/types';
...
import { Property, Type } from '../meta/types';
import { Attributes, AttributesHolder } from '../attributes';
const tracerSup = core.getTracer("etna.supervisor");

export class Slot implements AttributesHolder {
	initialized: boolean;
	property: Property;
	...
	snapshots: any; // refine later
	_setCalls: number;
	$isMandatory: boolean;
	bindAttributes: (parent: any) => void;
	snapshotof: Slot;
	// below come from ACTX
	$isReadOnly: boolean;
	$_isReadOnly: boolean;
		...
		if (this.column) {
			if (record[this.column] !== undefined) {
				...
				if (this.property.type.readMedia) {
					...
					this._value = this._value.map_(_, (_: _, val: any) => this.property.type.fromSql(_, val));
				} else {
	...
	getDeltaAttributes(_: _,  from: AttributesHolder): any {
		throw new Error("invalid call: abstract method");
	}

	getAttributes(_: _): any {
		throw new Error("invalid call: abstract method");
	}
