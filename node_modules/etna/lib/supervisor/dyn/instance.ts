...
import { Property, Reference } from '../meta/property';
...
const traceCRUD = core.getTracer("etna.crud");
...
export class Instance extends BaseInstance implements AttributesHolder {
	...
	get APARENT(): Instance {
		return this.$aparent || this.$parent as any;
	}
	...
	afterCreate(_: _,  record?: Record, colnIndex?: number) {
	...
	revertTo() {
		...
		attributes.revertTo(this as any, to as any);
		return to;
	}
	getProKeys(_: _) {
		const dataChildren = this.class.data.dataChildren;
	...
	AREAD(_: _, arg?: any) { // arg used by AVOLFIL??
	...
	crud(_: _,  fname: string, f: (_: _, instance: Instance) => number, key?: any) {
		traceCRUD.debug && traceCRUD.debug(this.class.name + "." + fname + ' key=' + key);
		...
		try {
			if (util.currentContext().x3session._crudActions === undefined) {
				util.currentContext().x3session._crudActions = [];
				//Define execute at the top level only
				execute = (_, event) => {
					var status = constants.CST_AOK;
					util.currentContext().x3session._crudActions.reverse().forEach_(_, (_: _, f: (_: _, ev: string) => number) => {
			...
			util.currentContext().x3session._crudActions.push((_: _, event: string) => this.action(_, fname + "_" + event, this.key));
			...
			if (execute) util.currentContext().x3session._crudActions = undefined;
		}
	...
	action(_: _,  action: string, key?: any, member?: any, frame?: Frame) {
		...
		while (instance != null) {
			...
			instance = instance.$parent as any;
		}
		return status;
	}
	afterQuery(_: _) {
		return this.action(_, "AQUERY_TRANS_AFTER", null, null, util.currentContext().queryFrame);
	}
	onControl(_: _,  type: string, operation: DOMStringMap) {
		return (this as any)[type](_, operation);
	}
	...
	executeControl(_: _,  type: string) {
		...
		const status = (this as any)[control] && (this as any)[control](_, type) || constants.CST_AOK;
			...
			Object.keys(this.properties).filter(property => !!this.properties[property].toControl).forEach_(_, (_, property) => {
				...
				if (slot) {
					...
					if (this.properties[property].isReference()) {
						...
						slot = this.createSlot(this.properties[property + "_REF"]);
						status = Math.max(status, slot.controlOthers(_));
					}
				}
				// Send "CONTROL"
				status = Math.max(status, this.propertyControlValue(_, this.createSlot(this.class.properties[property])));
			});
			if (status === constants.CST_AERROR) return status;
	...
	supportsTable(operation: string) {
		...
		const collection = this.$parent && (this.$parent as any).collection;
	...
	initMaps(_: _,  collection: Collection, parent: any) {
		const defMap = (_: _, map: ClassLinkMapMeta) => {
			...
			if (map.VALDEFMAP.toUpperCase().substring(0, 10) === "THIS.ACTX.") {
				value = util.currentContext().x3session.actx.get(_, lVALDEFMAP.substring(10));
			}
