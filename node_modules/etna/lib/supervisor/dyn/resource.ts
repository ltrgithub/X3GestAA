...
import { DataNode } from '../protocol/data';
...
import { ResourceFactory } from '../meta/resourceFactory';
import { Representation } from '../meta/representation';
import { Slot } from './slot';
import { SlotCollection } from './slotCollection';
import { Supervisor } from '../supervisor';

export class Resource extends BaseInstance {
	supervisor: Supervisor;
	resourceFactory: ResourceFactory;
	meta: Representation;
	$exported: boolean;
	jsClassName: string;
	enableSyssnapshot: () => void;
	disableSyssnapshot: () => void;
	$syssnapshot: Resource;
	constructor(superv: Supervisor, resourceFactory: ResourceFactory, instance: Instance, slots?: { [name: string]: Slot }) {
		super(superv);
		...
		Object.keys(this.properties).filter(name => !this.resourceFactory.properties[name].isExtraRef).forEach_(_, (_, name) => {
			...
			if (prop.collection) {
				if (prop.isInstance) {
					...
					let rChild = this.supervisor.new<Resource>(_, 'Resource', prop.targetResourceFactory, child).dataNode(_, full);
					...
					let snapshot = instance.$syssnapshot ? instance.$syssnapshot.get(_, name) : undefined;
					var delta = !full && !prop.isArray && snapshot !== undefined && snapshot !== collection;
					var lines: Instance[] = delta ? collection.getDelta(_, snapshot) : collection.lines;

					var lineIndex = 0;

					let rChild = lines.filter(line => {
						...
						const resLine = this.supervisor.new<Resource>(_, 'Resource', prop.targetResourceFactory, child).dataNode(_, prop.isArray ? true : full);
						...
						result[prop.tag] = rChild.map(child => {
							...
							if (val.length === 1) return val[0];
								...
								if (prop.targetResourceFactory.properties[childDest] !== undefined) {
									r[prop.targetResourceFactory.properties[childDest].destLookup] = val[0];
									r = Object.keys(val[1]).reduce((o, name) => {
										o[name] = val[1][name];
										return o;
									}, r);
								}
				...
				let snapshot = instance.$syssnapshot;
				...
				if (tag[0] !== '$') {
					...
					for (var i = 0, len = keys.length; i < len; i++) {
						if (this.properties[keys[i]].isInstance) {
							...
							var rChild = this.supervisor.new<Resource>(_, 'Resource', prop.targetResourceFactory, child);
					...
					v.forEach_(_, (_, deltaLine) => {
						...
						if (line !== null) {
							...
							if (deltaLine.$isDeleted) {
								...
								var rChild = this.supervisor.new<Resource>(_, 'Resource', prop.targetResourceFactory, line);
	...
	read(_: _,  key: any) {
		...
		this.instance = this.supervisor.new<Instance>(_, 'Instance', this.resourceFactory.representation.class, this);
...
export const Constructor = Resource;