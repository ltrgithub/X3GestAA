...
export class Resource extends BaseInstance {
		...
		Object.keys(this.properties).filter(name => !this.resourceFactory.properties[name].isExtraRef).forEach_(_, (_, name) => {
			...
			if (prop.collection) {
				if (prop.isInstance) {
					...
					let rChild = this.supervisor.new(_, 'Resource', prop.targetResourceFactory, child).dataNode(_, full);
					...
					let rChild: Resource[] = lines.filter(line => {
	...
	applyDelta(_: _,  delta: types.DataNode) {
		...
		Object.keys(delta).forEach_(_, (_, tag) => {
			...
			const v: types.DataNode[] = delta[tag];
