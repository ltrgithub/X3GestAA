...
var ETag = 0;
export class WorkingCopy extends util.Helper {
	...
	update(_: _,  context: HttpContext, delta: ResourceNode, path?: string) {
		tracerJs.debug && tracerJs.debug("updating working copy: cdelta=" + delta + ', path=' + path);
	...
	read(_: _,  context: HttpContext, path: string) {
		tracerJs.debug && tracerJs.debug("reading working copy: path=" +  path);
		this.current.walkToProperty(_, path, (_, instance, property) => {
	...
	readLocalization(_: _,  context: HttpContext, path: string) {
		tracerJs.debug && tracerJs.debug("reading working copy localization: path=" + path);
