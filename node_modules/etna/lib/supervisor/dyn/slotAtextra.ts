...
export class SlotAtextra extends Slot {
	...
	clone(parent: any) {
		...
		if (this._atextra) {
			(to as any)._atextra = Object.keys(this._atextra).reduce((r, lan) => {
	...
	getAtextra(_: _,  lan: string) {
		...
		return this._atextra[lan] = lan === util.currentContext().x3session.actx.$LAN(_) ? this.get(_) : this.property.getAtextra(_, this.$parent, lan);
	}
	...
	setAtextra(_: _,  text: string, lan: string) {
		...
		if (lan === util.currentContext().x3session.actx.$LAN(_)) {
			this.set(_, text);
		}
	}
}
