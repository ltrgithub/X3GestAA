...
import { Diagnosis, trace as diagTrace } from './diagnosis';
import { Property, Collection, BaseClass, Class } from '../meta/types';
import { Instance } from './instance';
import { Slot } from './slot';
import { Supervisor } from '../supervisor';

export class SlotCollection /*extends Slot*/ {
	...
	constructor(parent: any, collection: Collection) {
		//super(parent, collection.property);
		...
		const bindOperation = (flag: string, fct: string) => {
			(this as any)[fct] = !this.collection.mappingData || this.collection.mappingData[flag] === 2 
			? (this as any)['_' + fct] 
	...
	find(key: any, val: any) {
		for (var i = 0, len = this.lines.length; i < len; i++) {
			...
			if (line !== null && (line as any)[key].x3Compare(val) === 0) {
				return line;
			}
		}
		return null;
	...
	clone() {
		...
		// TODO: eliminate this cast after we fix Slot -> SlotCollection inheritance
		attributes.clone(this as any, to as any);
		to.snapshotof = this;
		return to;
	}
	...
	revertTo() {
		...
		// TODO: eliminate this cast after we fix Slot -> SlotCollection inheritance
		attributes.revertTo(this as any, this.snapshotof as any);
		return to;
	}
	...
	onMapChange(_: _,  property: string, value: any) {
		return this.applyToEach(_, (_, line, operation) => (line as any)[operation](_, property, value), "onMapChange");
	}

	onControl(_: _,  type: string, operation: string) {
		if (!/^Update$/.test(operation)) return this.applyToEach(_, (_, line, operation) => (line as any)[type](_, operation), operation);

		// Update:
		return this.applyToEach(_, (_, line, typeOperation) => (line as any)[type](_, typeOperation.toUpperCase()));
	}

	executeControl(_: _,  type: string) {
		return this.applyToEach(_, (_, line, typeOperation) => (line as any)[typeOperation](_, type), "executeControl");
	}

	_insert(_: _) {
		return this.applyToEach(_, (_, line, operation) => (line as any)[operation](_), "insert");
	}

	_update(_: _) {
		return this.applyToEach(_, (_, line, operation) => (line as any)[operation](_));
	}

	_delete(_: _) {
		return this.applyToEach(_, (_, line, operation) => (line as any)[operation](_), "delete");
	}

	_activeLoading(_: _) {
		return this.applyToEach(_, (_, line, operation) => (line as any)[operation] && (line as any)[operation](_), "activeLoading");
	...
	getDelta(_: _,  snapshot: SlotCollection) {
		...
		var lineIndex = 0;
		const diffLineNumbers = snapshot.lines.reduce((r, line) => {
		...
		lines.forEach_(_, (_, line) => {
			...
			if (!diffLineNumbers[uuid] || line.$syssnapshot !== undefined) {
