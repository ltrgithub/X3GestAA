...
class Slot {
		...
		if (!raw) {
			...
			if (this._setCalls === 1) {
				...
				if (status !== constants.CST_AERROR) {
					...
					if (status !== constants.CST_AERROR) {
						status = this.$parent.propertyControlValue(_, this.property, oldValue);
						if (status !== constants.CST_AERROR) {
							// Propagate the value : 
	...
	fromRecord(_, record, colnIndex) {
		//dbg:console.error("Slot.fromRecord:",record);
		if (this.column) {
	...
	propagateValue(_, oldValue) {
		...
		if (this.$parent.propertyPropagateValue && oldValue.value.x3Compare(this._value) !== 0) {
			this.$parent.propertyPropagateValue(_, this.property, oldValue);
		}
