...
function loadType(_,context,type,code) {
	if (!code) throw new HttpError(400, type+" parameter missing");

	var clas = context.supervisor.load(_, type, code);
	if (!clas) throw new HttpError(404, type+ " not found: " + code);
	return clas;
}
...
function readLob(_, context, codcla, property, ident1, ident2, ident3) {
	var cl = loadClass(_, context, codcla);
	return cl.readLob(_,context, property, ident1, ident2, ident3);
}
...
class HttpContext extends Helper {
	...
	workingCopy(trackingId) {
		var workingCopies = this.private.workingCopies;
...
class BatchContext extends Helper {
	constructor(resource, superv, qs) {
		...
		this.request = {
			...
			readAll:function(res) {
				return function(_) { return JSON.stringify(resource);}
			}(resource)
		};
	...
	checkMethod(method) {
		if (this.getMethod() === method) return;
		else throw new HttpError(405, "bad method " + this.method);
	}
	get private() {
		if (!this.request.session) throw new HttpContext(500, "session missing");
		return this.request.session.private = this.request.session.private || {
...
var routes = [{
	...
	handler: function(_, context, codcla, property, ident1, ident2, ident3) {
		readLob(_, context, codcla, property, ident1, ident2, ident3);
	},
}, {
	...
	handler: function(_, context, codcla, property, ident1, ident2) {
		readLob(_, context, codcla, property, ident1, ident2);
	},
}, {
	//sdata/x3/erp/etnaOracle/BDOC('C_AVOLFIL~%C2%A8~CONTENTS~%C2%A8~TMP~%C2%A8~000002.jpg')
	pattern: /^(?:B|C)DOC\('C_(\w*)~(?:¨|%C2%A8)~(\w*)~(?:¨|%C2%A8)~(\w*)/,
	handler: function(_, context, codcla, property, ident1) {
		readLob(_, context, codcla, property, ident1);
	},
}, {
	pattern: /^(\w*)(\('[^']*'\))?$/,
	...
	handler: function(_, context, user) {
		...
		var rights = glob.context.x3session.actx._x3Rights;
		var res = rights.getJsonRights(_, user);
		var rightsEtag = rights._etag;
	...
	handler: function(_, context, codcla, key, service) {
		...
		var codrep = context.codrep,
			...
			qs = context.qs;
		var repr = loadRepresentation(_, context, codrep);
		if (!FACETS[facet] || facet === '$edit') throw new HttpError(400, "bad facet: " + facet);
		if (repr.data.CODCLA !== codcla) throw new HttpError(400, "class/representation mismatch: " + repr.data.CODCLA + " <> " + codcla);
		repr.check(_, facet);
	...
	handler: function(_, context, codcla) {
		context.checkMethod("POST");
		var clas = loadClass(_,context,codcla);
		var repr = loadRepresentation(_, context,context.codrep);
		var data = JSON.parse(context.request.readAll(_));


		var batchResponse = {$resources: []};
		data.$resources && data.$resources.forEach_(_,function(_, resource) {
			var m = /^\/sdata\/(?:etna|x3)\/erp\/(\w+)\/([^?]*)\??(.*$)/.exec(resource.$url);
			var endpointName = m[1],
				path = m[2],
				qs = parseQueryString(m[3]);

			for (var i = 0; i < routes.length; i++) {
				path = path.replace("{$baseUrl}",context.supervisor.baseUrl)
						   .replace(/^http[s]{0,1}:\/\/\w*(?::\d+)/,"");
