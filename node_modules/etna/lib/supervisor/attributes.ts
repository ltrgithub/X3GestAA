...
const extAttributes = Object.keys(extAttributesObject).reduce((r, name) => {
...
Attributes.prototype.getDeltaAttributes = function (_: _,  from) {
	const result = {};
...
function addIntAttributes(_prototype) {
	...
	_prototype.getDeltaAttributes = function (_: _,  from) {
...
function addResources(_prototype) {
	Object.keys(resAttributes).forEach(resource => {
		_prototype["set" + resource] = function (_: _,  value, lan) {
			this.$attributes.setResource(_, resource, value, lan);
		};
		_prototype["get" + resource] = function (_: _,  lan) {
