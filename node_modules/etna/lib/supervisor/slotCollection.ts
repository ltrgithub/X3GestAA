...
import * as Diagnosis from '../supervisor/meta/diagnosis';
...
export class SlotCollection {
	...
	constructor(parent, collection) {
		...
		const bindOperation = (flag, fct) => {
			this[fct] = !this.collection.mappingData || this.collection.mappingData[flag] === 2 
			? this['_' + fct] 
			: (_) => constants.CST_AOK;
		}
	...
	get(_: _) {
		if (this.$lazyLoading) {
			this.lines = this.collection.class
	...
	onMapChange(_: _,  property, value) {
		return this.applyToEach(_, (_, line, operation) => line[operation](_, property, value), "onMapChange");
	}

	onControl(_: _,  type, operation) {
		if (!/^Update$/.test(operation)) return this.applyToEach(_, (_, line, operation) => line[type](_, operation), operation);

		// Update:
		return this.applyToEach(_, (_, line, typeOperation) => line[type](_, typeOperation.toUpperCase()));
	}

	executeControl(_: _,  type) {
		return this.applyToEach(_, (_, line, typeOperation) => line[typeOperation](_, type), "executeControl");
	}

	_insert(_: _) {
		return this.applyToEach(_, (_, line, operation) => line[operation](_), "insert");
	}

	_update(_: _) {
		return this.applyToEach(_, (_, line, operation) => line[operation](_));
	}

	_delete(_: _) {
		return this.applyToEach(_, (_, line, operation) => line[operation](_), "delete");
	}

	_activeLoading(_: _) {
		return this.applyToEach(_, (_, line, operation) => line[operation] && line[operation](_), "activeLoading");
...
export function newSlotCollection(property) { 
	return (parent => {
