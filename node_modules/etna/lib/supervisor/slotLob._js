...
class SlotLob extends Slot {
	...
	readRecord(_,lobTable,lobField) {
		...
		var rs = this.$parent.supervisor.sqlReader(_, "select "+lobField+"_0 From "+lobTable+" Where " +
	...
	updateRecord(_,lobTable,lobField,record, reader) {
		...
		var executeSql = function(_,self, sql, i, values) {
			var name = param(i++);
			sql += " returning "+lobField+"_0 into "+name;
			var result = self.$parent.supervisor.executeSql(_, sql, values,{
				outputVars: [{ 
		...
		// Does the lob exist ?
		var i = 0;
		var lobPlaceholder = driver.lobPlaceholder(this.property.type.sqlType());

		var columns = Object.keys(record);
		var columns2 = columns.filter(k => k !== lobField);
		var rs = this.$parent.supervisor.sqlReader(_, "select "+columns2.map(p  => p+'_0').join(',')+" From "+lobTable+" Where " +
				columns2.slice(0,4).map(p => p+'_0='+param(i++)).join(" and "),
				columns2.slice(0,4).map(p => record[p]    	 )).toArray(_);

		if(!rs.length) {
			i = 0;
			var sql = "insert into "+lobTable+" (" + columns.map(p => p+'_0').join(',') + ") values (" + 
				columns.map(p => p === lobField ? lobPlaceholder : param(i++)).join(',') + ")";
			return executeSql(_, this, sql, i, columns2.map(p => record[p]));
		}
		columns2.forEach(p => {if(rs[0][p]) record[p] = rs[0][p];});
		record.UPDUSR = actx.$USER(_);
		record.UPDDATTIM = tdatetime.now();
		...
		var sql = "update "+lobTable+" SET "+ columns.slice(4).map(p => p+'_0='+(p === lobField ? lobPlaceholder : param(i++))).join(',') 
				+" where " + columns.slice(0,4).map(p => p+'_0='+param(i++)).join(" and ");
