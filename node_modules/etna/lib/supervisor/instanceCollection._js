...
var InstanceCollection = helpers.defineClass(function(parent, collection) {
	...
	applyToEach: function(_, f, operation) {
		...
		tracerJs.debug && tracerJs.debug("col "+this.name+".applyToEach lines:"+ lines.length);

		if (operation === undefined) {
			// Filter and sort the lines
			var self = this;
			lines = this.lines.filter(function(line) {
				tracerJs.debug && tracerJs.debug("col "+self.name+".applyToEach filter $astalin:"+ line.$astalin);
				return line !== null && line.$astalin > constants.CST_ANEWDEL;
			}).sort(function(line1, line2) {
				return line2.$astalin - line1.$astalin;
			});
		}
		//console.error(this.name+".applyToEach ("+operation+") this.lines:"+this.lines.length);
		for (var i = 0, len = lines.length; i < len; i++) {
			var line = lines[i];
			if (line !== null) {
				var op = operation || operations[line.$astalin];
				if (op) {
					tracerJs.debug && tracerJs.debug("col "+this.name+".applyToEach "+ op + " on line " + i);
	...
	insert: function(_) {
		tracerJs.debug && tracerJs.debug("col "+ this.name+".insert");
		return this.applyToEach(_, function(_, line, operation) {
			return line[operation](_);
		}, "insert");
	},
	update: function(_) {
		tracerJs.debug && tracerJs.debug("col "+ this.name+".update");
		return this.applyToEach(_, function(_, line, typeOperation) {
			return line[typeOperation](_);
		});
	},
	delete: function(_) {
		tracerJs.debug && tracerJs.debug("col "+ this.name+".delete");
		return this.applyToEach(_, function(_, line, operation) {
			return line[operation](_);
		}, "delete");
	},
