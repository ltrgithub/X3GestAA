...
import { Reader, Writer } from 'ez-streams';
import { Dict } from '../engine/runtime/types';
import { Property } from './meta/types';
import { BaseInstance, Instance, Resource } from './dyn/types';
...
export class Cache {
	texts: Dict<TextEntry>;
	references: Dict<ReferenceEntry>;
	_isActivated: boolean;
	max: number;
	instanceset: BaseInstance[];
	...
	register(_: _,  instance: BaseInstance) {
	...
	transform(): (_: _, reader: Reader<Instance>, Writer: Writer<Instance>) => void {
		...
		function flush(_: _,  _writer: Writer<BaseInstance>) {
		...
		return function (_: _,  _reader: Reader<Instance>, _writer: Writer<Instance>) {
