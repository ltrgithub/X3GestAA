...
exports.$EVENTS = function (_: _,  action, resource) {
	switch (action) {
		case "AQUERY_OPEN":
...
function AQUERY_TRANS_AFTER(_: _,  resource) {
	...
	if (instance.get(_, "TYPCLA") !== 2 || codact && activ.getActiv(_, superv, codact) <= 0) {
		...
		var rs = ez.devices.generic.empty.reader.transform(function (_: _,  reader, writer) {
			session.$queryCtx.forEach_(_, (_, record) => {
