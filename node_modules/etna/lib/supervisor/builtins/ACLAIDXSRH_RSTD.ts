...
exports.$EVENTS = function (_: _,  action: string, resource: Resource): any {
	switch (action) {
		case "AQUERY_OPEN":
...
function AQUERY_OPEN(_: _) {
	...
	const superv = util.currentContext().x3frame.context.superv;

	session.$queryCtx = superv.mongoStore.collection("ASHW").find({
		DEFREP: 2
	}).toArray().then(_, _);
}
