...
import { Resource } from '../meta/resource';
...
function AQUERY_TRANS_AFTER(_: _,  resource: Resource) {
	...
	if (instance.get(_, "TYPCLA") !== 2 || codact && activ.getActiv(_, superv, codact) <= 0) {
		...
		var rs = ez.devices.generic.empty.reader.transform(function (_: _,  reader: ez.Reader<void>, writer: ez.Writer<types.Row>) {
			session.$queryCtx.forEach_(_, (_: _, record: types.Row) => {
