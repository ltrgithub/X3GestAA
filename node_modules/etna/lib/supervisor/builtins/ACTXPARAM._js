"use strict";
var date = require('etna/lib/engine/runtime/tdate');
...
class ACTXPARAM extends Helper {
	...
	_addParamUserCache(parameter, value) {
	...
	_addParamCache(levcod, code, parameter, value) {
	...
	_getParams(_, levcod, code, parameters, action) {
		if (!parameters) throw new HttpError(401, "Incorrect param");
		var values = {};
		if (this.cache.levcod[levcod] !== undefined && this.cache.levcod[levcod].code[code] !== undefined) {
			...
			values = parameters.reduce(function(r, p) {
				if (cache[p] === undefined) ok = false;
				else r[p] = cache[p] ? cache[p].value : null;
				return r;
			}, {});
			if (ok) return values;
		}
		...
		var select = "select A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0 from ADOPAR A Where A.PARAM_0";
		if (parameters.length === 1) {
			select += "=" + param(0);
		} else {
			select += " in (" + parameters.map(function(val, i) {
				return param(i);
			}).join(',') + ") order by A.PARAM_0";
		}
		//dbg:console.error("select:"+select+":",parameters);
		var wadopar = self.supervisor.sqlReader(_, select, parameters).toArray(_);
		for (var i = 0; i < wadopar.length; i++) {
			var parameter = wadopar[i].PARAM;
			if (action !== undefined) self.controlType(action, wadopar[i]);

			if (wadopar[i].NIVDEF === 4 && wadopar[i].VALDEF !== 2) throw new HttpError(401, parameter + " parameter can only be defined at User level");
			...
			switch (levcod) {
				...
				case 5: // looking for value at "L�gislation" level
					...
					if (wadopar[i] === 1) { // case 1 : Parameter defined at "Dossier" level
						value = self._requestAdovalBase(_, " ", " ", parameter);
				...
				case 2: // looking for value at "Soci�t�" level
					...
					if (wadopar[i].NIVDEF === 1) { // case 1 : Parameter defined at "Dossier" level
						value = self._requestAdovalBase(_, " ", " ", parameter);
						if (value !== undefined) break;
					} else if (wadopar[i].NIVDEF === 5) { //Parameter defined at "L�gislation" level						
				...
				case 3: //looking for value at "Site" level
					...
					if (wadopar[i].NIVDEF === 1) { // case 1 : Parameter defined at "Dossier" level
						value = self._requestAdovalBase(_, " ", " ", parameter);
						if (value !== undefined) break;
					} else if (wadopar[i] === 5) { // case 2Parameter defined at "L�gislation" level
					...
					} else if (wadopar[i].NIVDEF === 2) { // case 3 : Parameter defined at "Soci�t�" level
			...
			values[parameter] = this._addParamCache(levcod, code, parameter, value);
			values[parameter] = values[parameter] || null;
		}
		return values;
	}
	controlType(action, record) {
		switch (action) {
			case "CHAR": //1:alpha 4:local menu
				if ([1, 4].indexOf(record.TYPVAL) < 0) throw new HttpError(401, record.PARAM + " parameter is not of the right type : alpha (" + record.TYPVAL + ")");
				break;
			case "NUM":
				if (record.TYPVAL !== 2 && record.TYPVAL !== 4) throw new HttpError(401, record.PARAM + " parameter is not of the right type : numeric (" + record.TYPVAL + ")");
				break;
			case "DATE":
				if (record.TYPVAL !== 3) throw new HttpError(401, record.PARAM + " parameter is not of the right type : date (" + record.TYPVAL + ")");
				break;
		}
	}
	_getParamUser(_, parameter, action) {
		...
		//manage CACHE
		if (this.userCache.parameter[parameter]) return this.userCache.parameter[parameter].value ;
