...
class ACTXPARAM extends Helper {
				...
				switch (action) {
					...
					case "AUTO":
						switch (wadopar[i].TYPVAL) {
							case 1: value = ''; break; // string
							case 2: value = 0; break; // 
							case 3: value = 0; break; // date
							case 4: value = 0; break;
							default: throw new Error(`invalid TYPVAL: ${wadopar[i].TYPVAL}`);
						}
						break;
					default:
	...
	_getParamUser(_, parameter, action) {
		...
		// read param in ADOPAR to have the definiton
		params = [parameter];
		var wadopar = self.supervisor.sqlReader(_, "select A.CHAPITRE_0,A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0,A.NOLIB_0 from ADOPAR A Where A.PARAM_0=" + param(0), params).toArray(_);
