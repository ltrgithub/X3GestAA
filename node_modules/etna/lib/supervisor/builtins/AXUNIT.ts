...
export function RUN_TESTSUITE(_: _,  title: string, description: string) {
 ...
 recorder.start();*/
	...
	const frame = runtimeUtil.currentFrame();
	...
	testCases.forEach((tc, i) => {
		...
		asyncTest(tc.title, tc.count, function() {
			return _.run(_ => {
				try {
					var prog: any;
					if (!i && (prog = frame.prog.module["SETUP"])) // First element
						flowControl.callProg(_, prog, [], [], runtimeUtil.currentFrame().sub.loc);
...
export function CHECK_EQUAL(_: _,  v1: any, v2: any, title: string) {
	...
	if (typeof v11.x3ToString !== 'function') {
		console.error(v11);
		return deepEqual(-1, 0, `CHECK_EQUAL: EXPECTED ${ v22 }, GOT invalid value: typeof=${ typeof v11 }, val=${ v11 }`);
	}
