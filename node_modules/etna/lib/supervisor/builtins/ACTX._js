...
function initSession(_,session, superv) {
	var context = glob.context.x3frame.context;
	if(superv.sys) {
			Object.keys(superv.sys).forEach(function(v) {
			context.sys.values[v] = superv.sys[v];
		});
	}

	//dbg:console.error("************** ACTX ***************");
	var globals = context.globals;
	util.declVar(globals, 'GACTX', 'VY', sessions[session.id].actx);
	util.declVar(globals, 'AVOID', 'VY', new AVOID());
	util.declVar(globals, 'GADONIX', 'VI', sessions[session.id].actx.$ADONIX(_));
	util.declVar(globals, 'GDIMFIC', 'VI', 250);
	util.declVar(globals, 'GLEGFIL', 'VS', sessions[session.id].actx.$LEGFIL(_));
