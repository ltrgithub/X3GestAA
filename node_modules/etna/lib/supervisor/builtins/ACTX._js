...
exports.init = function(_, session, superv) {
	if (sessions[session.id]) {
		//dbg:console.log("restore session:" + session.id);
		glob.context.x3session = sessions[session.id];
		// Wait for the actx to be created
		sessions[session.id].funnel(_,function(_) {
			// set LANISO+LAN
			sessions[session.id].actx.$LANISO(_, session.locale);
			sessions[session.id].actx.initSession(_);
		});
	} else {
		tracerJs.debug && tracerJs.debug("new session:" + session.id);
		//dbg:console.log("new session:" + session.id);
		glob.context.x3session = sessions[session.id] = {
			dbConnection:null,
			supervisor:superv,
		...
		sessions[session.id].funnel(_, function(_) {
			sessions[session.id].actx = new ACTX(superv).init(_,session.userName, session.locale);
			sessions[session.id].actx.initSession(_);
		});
	}
};
...
class ACTX extends Chapter {
	...
	initSession(_) {
		//dbg:console.error("ACTX.initSession");
		var context = glob.context.x3frame.context;
		var sys = this.supervisor.sys;
		if(sys) {
			//dbg:console.error("ACTX.initSession => sys:",sys);
			Object.keys(sys).forEach(function(v) {
				context.sys.values[v] = sys[v];
			});
		}
	...
	$LANISO(_, laniso) {
		if(laniso !== undefined  && (!this.laniso || this.laniso !== laniso)) {
			var self = this;
	...
	$LOGIN(_, login) {
		if(login !== undefined) {
			...
			if (rUser[0]) {
				...
				tracerJs.debug && tracerJs.debug("load rights " + login);
				this._x3Rights.getJsonRights(_, login);
	...
	$ADONIX(_) {
		return this.adonix = (this.adonix || this.$APARAM(_).AGETVALNUM(_, 1, this.$AFOLDER(_), "ADONIX"));
	}
	...
	initSysVar(_) {
		if (!this.supervisor.sys) {
			...
			this.supervisor.sys = Object.keys(values).reduce(function(r, v) {
				r[v] = [1, variables.types["S"].def, values[v], 0, values[v].length];
				return r;
			}, {});
			//dbg:console.error("initSysVar values:",values);
