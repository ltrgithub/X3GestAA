...
class ACTX extends Chapter {
	...
	$LANISO(_, laniso) {
		if (laniso !== undefined && (!this.laniso || this.laniso.toLowerCase() !== laniso.toLowerCase())) {
			var self = this;
			var rTablan = self.supervisor.sqlReader(_, "select LAN_0,LANISO_0 from TABLAN where (LOWER(LANISO_0)=" + self.supervisor.sqlDriver.param(0) + ")", [laniso.toLowerCase()]).limit(1).toArray(_);

			if (rTablan[0] !== undefined) {
				this.laniso = rTablan[0].LANISO;
				this.$LAN(_, rTablan[0].LAN);
			} else {
				var rTablan = self.supervisor.sqlReader(_, "select LAN_0,LANISO_0 from TABLAN where (LAN_0=" + self.supervisor.sqlDriver.param(0) + ")", [self.$LANDEF(_)]).limit(1).toArray(_);
				this.laniso = rTablan[0].LANISO;
				this.$LAN(_, rTablan[0].LAN);
			}
