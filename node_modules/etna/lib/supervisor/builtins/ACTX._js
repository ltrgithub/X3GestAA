...
var ACTXCACHE = require("etna/lib/supervisor/builtins/ACTXCACHE");
var ACTXPARAM = require("etna/lib/supervisor/builtins/ACTXPARAM");
...
exports.init = function(_, session, superv) {
	...
	util.declVar(glob.context.x3frame.context.globals, 'GADOVERS', 'VS', "L");
	util.declVar(glob.context.x3frame.context.globals, 'GSILENCE', 'VI', 0);
	util.declVar(glob.context.x3frame.context.globals, 'GSERVEUR', 'VI', 0);
	util.declVar(glob.context.x3frame.context.globals, 'GWEBSERV', 'VI', 0);
	util.declVar(glob.context.x3frame.context.globals, 'GIMPORT', 'VI', 1);// For test ?
	util.declVar(glob.context.x3frame.context.globals, 'GTRACE', 'VS', "");
...
function _actxLoadContextProp(_, ctx, name) {
	...
	if (ctx.builtinProperties && ctx.builtinProperties[name]) {
		...
		if (dime) {
			ctx[name] = new Slot(ctx, [1, ctx.class.properties[name].type.defaultValue, [], 1, dime - 1]);
		} else {
			ctx[name] = new Slot(ctx, ctx.class.properties[name].type.defaultValue);
		}
		var trtini;
...
class Chapter extends Helper {
	constructor(parent, builtinProperties) {
		super();
		this.parent = parent;
					...
					return {
						getValue: function(_) {
							...
							if (!self.AMODULE.value) {
								...
								self.AMODULE.value = [1, 0, modules, 1, modules.length];
							}
							return self.AMODULE.value;
						},
					};
...
var proto = ACTX.prototype;
["ASOLUTION", "AFOLDER", "LANISO", "LAN", "LANDEF", "USER", "LOGIN"].forEach(function(name) {
