...
const check = require("syracuse-license/lib/check");
...
exports.init = function (_, session, superv) {
	// console.log(glob.context.session.x3license);
...
class AFOLD extends Chapter {
	...
	$AMODULE(_) {
		...
		if (!this.amodule) {
			...
			var modules = (values.MODULE1 + values.MODULE2).split('').map(c => parseInt(c));
			// Add restriction set by the license
			const license = glob.context.session.x3license; 
			if(license && license.modules) {
				modules.forEach((module,i)=> license.modules[i] = (license.modules[(i+1).toString()] === false) ? 1 : license.modules[i]);
			}
...
class ACTX extends Chapter {
	...
	initSession(_) {
		...
		if(glob.context.session.x3license === undefined) {
			// ASYRLIC requires a context and GPRODUIT
			const asyrlic = runtime.requireScript(_, "ASYRLIC");
			const ctlfic  = runtime.executeProg(_,asyrlic, "CTLLIC");
			const product = runtime.executeProg(_,asyrlic, "GET_CHPRODUCTSTDCODE");
			const aproduct = runtime.requireScript(_, "APRODUCT");
			const version  = runtime.executeProg(_,aproduct, "AGETVERSLIC");
			// console.log("ctlfic:" ,ctlfic)
			// console.log("product:",product)
			// console.log("version:",version)
			let diags = [];
			glob.context.session.x3license = ctlfic ? check.getX3LicenseInfo(_, product, version, glob.context.session, diags) :null;
		}
	}	
	...
	checkAccess(_,keyFunction,type) {
		// First check the license
		const license = glob.context.session.x3license; 
		if(license && license.keyFunctions && license.keyFunctions[keyFunction] === false) {
			throw new util.HttpError(403, this.supervisor.loadMess(_, 1,103)+" "+keyFunction); 
 		}
	}
	...
	$LANISO(_, laniso) {
		if (laniso !== undefined && (!this.laniso || this.laniso.toLowerCase() !== laniso.toLowerCase())) {
			var rTablan = this.supervisor.sqlReader(_, "select LAN_0,LANISO_0 from TABLAN where (LOWER(LANISO_0)=" + this.supervisor.sqlDriver.param(0) + ")", [laniso.toLowerCase()]).limit(1).toArray(_);

			if (rTablan[0] !== undefined) {
				this.laniso = rTablan[0].LANISO;
				this.$LAN(_, rTablan[0].LAN);
			} else {
				var rTablan = this.supervisor.sqlReader(_, "select LAN_0,LANISO_0 from TABLAN where (LAN_0=" + this.supervisor.sqlDriver.param(0) + ")", [this.$LANDEF(_)]).limit(1).toArray(_);
				this.laniso = rTablan[0].LANISO;
				this.$LAN(_, rTablan[0].LAN);
			}
	...
	$LAN(_, lan) {
		this.lan = (lan !== undefined) ? lan : (this.lan || this.$LANDEF(_));
		const license = glob.context.session.x3license; 
		if(license && license.languages && license.languages[this.lan] === false) {
			throw new util.HttpError(401, this.lan+" not supported by the license"); // TODO : add a message
		}
		return this.lan;
	}
	...
	$LOGIN(_, login) {
		if (login !== undefined) {
			this.login = login;
			var rUser = this.supervisor.sqlReader(_, "select USR_0 ,CODMET_0, PRFFCT_0, ENAFLG_0, USRCONNECT_0 from AUTILIS where (LOGIN_0=" + this.supervisor.sqlDriver.param(0) + ")", [login.toUpperCase()]).limit(1).toArray(_);
