...
class ACTX extends Chapter {
	...
	$LANISO(_, laniso) {
		if(laniso) laniso = laniso.toLowerCase();
		if(laniso !== undefined  && (!this.laniso || this.laniso !== laniso)) {
			var self = this;
			var rTablan = self.supervisor.sqlReader(_,
				"select LAN_0,LANISO_0 from TABLAN where (LOWER(LANISO_0)=" + self.supervisor.sqlDriver.param(0) + ")", [laniso]).limit(1).toArray(_);
			...
			this.laniso = this.laniso.toLowerCase();
