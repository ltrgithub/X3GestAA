...
export class ACTXPARAM extends Helper {
	...
	getJsValue(_: _,  levcod, code, parameter) {
		var param;
	...
	_leg(_: _) {
		...
		const wadoval = this.supervisor.sqlReader(_, "select A.VALEUR_0 from ADOVAL A Where A.CMP_0=' ' and A.FCY_0=' ' and A.PARAM_0='DOSLEG'").toArray(_);
	...
	_cpy(_: _) {
		...
		this.tabcpy = this.supervisor.sqlReader(_, "select C.CPY_0 from COMPANY C").toArray(_).reduce((wcpy, cpy) => {
			wcpy.push(cpy.CPY);
			return wcpy;
		}, []);
		return this.tabcpy;
	...
	_fcy(_: _) {
		...
		this.tabfcy = this.supervisor.sqlReader(_, "select F.FCY_0 from FACILITY F").toArray(_).reduce((wfcy, fcy) => {
			wfcy.push(fcy.FCY);
			return wfcy;
		}, []);
		return this.tabfcy;
	}
	_getLegFromCpy(_: _,  folder, cpy) {
		const sqlDriver = this.supervisor.sqlDriver;
		...
		var value = "";
		params = [cpy];
		const wparam = this.supervisor.sqlReader(_, "select A.LEG_0 from COMPANY A Where A.CPY_0=" + param(0), params).toArray(_);
	...
	_getCpyFromSite(_: _,  folder, site) {
		const sqlDriver = this.supervisor.sqlDriver;
		const param = sqlDriver.param;
		...
		var value = "";
		params = [site];
		const wparam = this.supervisor.sqlReader(_, "select A.LEGCPY_0 from FACILITY A Where A.FCY_0=" + param(0), params).toArray(_);
	...
	_getLegFromSite(_: _,  folder, site) {
		var cpy = "";
		var leg = "";
	...
	_requestAdovalBase(_: _,  levcod, code, parameter) {
		const sqlDriver = this.supervisor.sqlDriver;
		const param = sqlDriver.param;
		...
		const wparam = this.supervisor.sqlReader(_, "select A.VALEUR_0 from ADOVAL A Where A.CMP_0=" + param(0) + " and A.FCY_0=" + param(1) + " and A.PARAM_0=" + param(2), params).toArray(_);
	...
	_getParam(_: _,  levcod, code, parameter, action) {
	...
	_getParams(_: _,  levcod, code, parameters, action) {
		...
		const wadopar = this.supervisor.sqlReader(_, select, parameters).toArray(_);
	...
	_getParamUser(_: _,  parameter, action) {
		...
		params = [parameter];
		const wadopar = this.supervisor.sqlReader(_, "select A.CHAPITRE_0,A.VALDEF_0,A.NIVDEF_0,A.PARAM_0,A.TYPVAL_0,A.NOLIB_0 from ADOPAR A Where A.PARAM_0=" + param(0), params).toArray(_);
		const values = {};
		...
		var wadovalaus = this.supervisor.sqlReader(_, "select A.CODUSR_0,A.PARAM_0,A.VALEUR_0 from ADOVALAUS A Where A.CODUSR_0=" + param(0) + " and A.PARAM_0=" + param(1), params);
		...
		if (wadopar[0].VALDEF === 2) {
			var profil = "";
			params = [actx.$USER(_)];
			var waus = this.supervisor.sqlReader(_, "select A.CODMET_0,A.PRFFCT_0 from AUTILIS A Where A.USR_0=" + param(0), params).toArray(_);
			...
			if (profil) {
				...
				var wadi = this.supervisor.sqlReader(_, "select A.N1_0 from ATABDIV A Where A.NUMTAB_0=901 and A.CODE_0=" + param(0), params).toArray(_);
				...
				if (wadi[0].N1 > 0 && wadi[0].N1 <= 20) {
					...
					params = [profil];
					var waft = this.supervisor.sqlReader(_, "select A.FCYDEF_" + nummdl + " from AFCTFCT A Where A.PRFCOD_0=" + param(0), params).toArray(_);
					if (waft[0] !== undefined) {
