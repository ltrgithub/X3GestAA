...
import { Class } from '../meta/types';
import { Instance } from '../dyn/types';

export class ACTXCACHE extends util.Helper {
...
export function getInstance(_: _,  clas: Class, key: string) {
	...
	const cache = clas.instancesCache || (clas.instancesCache = {
		instances: {},
		funnel: _.funnel(1)
	});
	var instance = cache.instances[key];
	if (instance === undefined) {
		cache.funnel(_, _ => {
			instance = clas.createInstance(_);
			var status = instance.read(_, [key]);
			if (status === constants.CST_AERROR) instance = null;
			cache.instances[key] = instance;
		})
	}
	return instance;
}
...
export const cacheMethods = {
	...
	AGETVALDAT(_: _,  key: string, prop: string): DateValue {
