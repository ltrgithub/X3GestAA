...
export function init(_: _,  session: X3Session, superv: Supervisor) {
	util.currentContext().session = util.currentContext().session || {
		// for unit tests
		userName: "AQCORE",
		locale: "fr-FR"
	};
	util.currentContext().session.etnaContexts = util.currentContext().session.etnaContexts || {};
	var x3Session = util.currentContext().session.etnaContexts[superv.endpointName];
	if (!x3Session) {
		util.currentContext().session.etnaContexts[superv.endpointName] = x3Session = {
			dbConnection: null,
			supervisor: superv,
			funnel: _.funnel(1)
		};
	}
	util.currentContext().x3session = x3Session;

	// Initialize util.currentContext().x3frame
...
export class Chapter extends util.Helper {
...
class AINTL extends Chapter {
	...
	adxfmt(idx: number) {
		return util.currentContext().x3frame.context.sys.values.ADXFMT2[2][idx];
	}
	$TWODIGITYEAR(_: _) {
		return util.currentContext().x3frame.context.sys.values.ADXDCS;
	}
	$SHORTDATE(_: _) {
...
export class ACTX extends Chapter {
	...
	initSession(_: _) {
		...
		const context = util.currentContext().x3frame.context;
	...
	$ASOLUTION(_: _,  solution: string) {
		return this.solution = solution !== undefined ? solution : this.solution || util.currentContext().x3session.supervisor.solutionName;
	}

	$AFOLDER(_: _,  folder: string) {
		return this.folder = folder !== undefined ? folder : this.folder || util.currentContext().x3session.supervisor.folderName;
	}
	...
	$LANISO(_: _,  laniso?: string) {
		if (laniso !== undefined && (!this.laniso || this.laniso.toLowerCase() !== laniso.toLowerCase())) {
			...
			util.currentContext().x3frame.context.sys.values.MESSNAME = this.$LAN(_);
			var newlaniso = this.$LANISO(_);

			require('streamline-locale').setCurrent(_, newlaniso);
			const values = util.currentContext().x3frame.context.sys.values;
	...
	$USER(_: _,  user?: string) {
		return this.user = user !== undefined ? user : this.user || util.currentContext().x3frame.context.sys.values.ADXUSR.toUpperCase();
	}
	...
	$LOGIN(_: _,  login?: string) {
		if (login !== undefined) {
			...
			if (rUser[0]) {
				...
				//ADXUSR=LOGIN
				util.currentContext().x3frame.context.sys.values.ADXUSR = login;

				this.getRights(_);
			} else throw new util.HttpError(401, this.supervisor.loadMess(_, 130, 70));
		} else {
			this.login = this.login || util.currentContext().x3frame.context.sys.values.ADXUSR.toUpperCase();
		}
		return this.login;
	}
