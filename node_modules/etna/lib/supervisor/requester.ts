...
import { evalue } from '../engine/runtime/evaluate';
import * as datetime from '../engine/runtime/tdatetime';
...
import { Parser } from '../engine/compilers/parser';
import * as type from '../supervisor/meta/type';
import * as util from '../supervisor/util';
import { Table } from '../supervisor/meta/types';
import { ASYR } from '../supervisor/builtins/ASYR';
import { ASYRSND } from '../supervisor/builtins/ASYRSND';
import { ASYRRCV } from '../supervisor/builtins/ASYRRCV';
...
function loadScript(_: _,  script: string) {
	try {
		var mod = runtime.requireScript(_, "WF" + script);
		mod.$4gl = true;
		return mod;
	} catch (e) {
		if (e.errn === 20) throw new HttpError(404, "query not found: " + script);
		else throw e;
	}
}

export function proto(_: _,  httpContext: HttpContext, script: string, query: string, facet: string) {
	const mod = loadScript(_, script);
	const psyr = new ASYR(httpContext);
	const args: ProgArgs = {
		ACTX: {
			type: "AY",
			value: util.currentContext().x3session.actx
		},
		PSYR: {
			type: "AY",
			value: psyr
...
export function query(_: _,  httpContext: HttpContext, request_code: string, query: string): any {
 ...
 */
	...
	const getExecutionTime = (frq: number, forced_exec: boolean) => {
		...
		switch (frq) {
			...
			case 4:
			case 6:
				execution_time = (frq !== 6 ? 1 : 0) * +now
				- (frq === 2 ? 1 : 0) * (3600 * 1000)
				- (frq === 3 ? 1 : 0) * 24 * 3600 * 1000
				- (frq === 4 ? 1 : 0) * 7 * 24 * 3600 * 1000;
				break;
			case 5:
	...
	const getPortalValue = (record: any, cod: string, fields: any[]) => {
		const k = record.CMPPARCOD.indexOf(cod);

		if (k < 0) return;

		var formula: string = record.CMPPARFOR[k];
		const res = "";
		//Replace , per ; and remove ""

		formula = formula.replace(/,/g, ';').replace(/"/g, '');
		return formula.split(';').reduce((r, exp) => {
	...
	const executeProg = (_: _, fct: string, args: ProgArgs) => {
		args && tracerJs.debug && Object.keys(args).filter(key => args[key].type[1] !== 'Y').forEach(key => {
	...
	const rqtBatch = (_: _) => {
		...
		runtime.executeProg(_, asyrmngtreq, "CRERQT", args);
		tracerJs.debug && tracerJs.debug(args['MESSAGE'].value);
	};

	const getTblSize = (_: _) => {
		const sqlDriver = superv.sqlDriver;
		const param = sqlDriver.param;
		// TODO: typing is completely broken here !!!
		// executeProg takes one more arg and returns a status, not a result
		const tableName: any = executeProg(_, "GET_MAINTABLE", {});
	...
	const psyr = new ASYR(httpContext);
	const asnd = new ASYRSND(httpContext);
	const arcv = new ASYRRCV(httpContext.supervisor, httpContext.qs);

	//   Forced request id (only for batch computations)

	var calendarPrototype: any;
	var ifbulk = 0;
	const ifcube = qs.cube && qs.cube === "true" ? 1 : 0;
	const uuireq = "";
	const nblig: any[] = [1, 0, [], 1, 20]; // NBLIG(1..20)

	const valbeg: any[] = [1, "", [], 1, 100]; // VALBEG(250)(1..100)

	const valend: any[] = [1, "", [], 1, 100]; // VALEND(250)(1..100)

	const strictbeg: any[] = [1, 0, [], 1, 100]; // STRICTBEG(1..100)

	const strictend: any[] = [1, 0, [], 1, 100]; // STRICTEND(1..100)

	const ifline: any[] = [1, 0, [], 1, 2]; //IFLINE(1..2)

	// Let's analyze the parameters sent in the URL

	const limits: any[] = [1, 0, [], 1, 2]; // LIMITS(1..2)
	...
	if (rights._PRFCOD !== superadmin) {
		var alh = superv.load<Table>(_, 'Table', 'ALISTEH');
		let rs = alh.loadRecords<{
			ACS: string;
			ALLUSR: number;
			FCTLNK: string;
		}>(_, alh.indexes[0], [request_code]);
	...
	if (qs.portview) {
		var apv = superv.load<Table>(_, 'Table', 'APTLVW');
		let rs = apv.loadRecords<{
			SRCPARCOD: string;
			SRCPARFOR: string[];
			CMPCODTYP: string;
		}>(_, apv.indexes[0], [qs.portview]);
		...
		if (rs.length) {
			...
			if (["FCA", "FCC"].indexOf(portView.CMPCODTYP)) {
				...
				var nbfields = args['NBFIELDS'].value;
	...
	if (qs.where) {
		...
		//try {
			for (var i = 0; i < param[2].length; i++) {
				var etnaType = type.typTyp2EtnaType[partype[2][i]];
			...
			var expressions: any[] = filter.$and ? filter.$and : [filter];
			expressions.forEach(exp => {
				const i = param[2].indexOf(exp);

				if (i) {
					if (typeof exp === 'object') {
						Object.keys(expressions[exp]).forEach(operator => {
							switch (operator) {
								case '$eq':
