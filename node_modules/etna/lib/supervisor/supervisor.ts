...
export class Supervisor {
	...
	sqlReader(_: _, sql: string, args?: any[], opts?: types.SqlReaderOptions) {
		...
		const maptrim = opts.typesmap ? utilDriver.maptrim(this.sqlDriver, opts.typesmap, opts.dispatch) : 
			(_: _, record: types.Row) => utilDriver.trim(record);
		//debugger;

		return this.sqlDriver.reader(_, glob.context.x3session.dbConnection, sql, args || [], opts).map(maptrim);
	}
	...
	*/
	...
	languages(_: _) {
		if (!this._languages) {
			...
			this._languages = this.sqlReader(_, "select " + columns.join(',') + " from ADOSSIER where (DOSSIER_0=" + this.sqlDriver.param(0) + ")", [this.folderName])
