...
import { ACTX } from './builtins/actx'
...
export interface TABLAN extends Instance {
	LAN?: string;
}
...
export class Supervisor {
	...
	sqlReader<T>(_: _, sql: string, args?: any[], opts?: SqlReaderOptions): Reader<T> {
		opts = opts || {};
	...
	*/
	reader<T>(_: _, className: string, filter: any): Reader<T> {
		const loader = subModule(className);
		const coln = this.mongoStore.collection(loader.collectionName).find(filter || {});

		return devices.generic.reader(_ => {
			while (true) {
	...
	*/
	...
	loadScripts(_: _, clas: BaseClass):  Prog[] {
		if (clas.data.CODTRT) {
			clas._scripts = clas.data.CODTRT.filter(s => !/^\s*$/.test(s)).map_(_, (_, codtrt) => {
				return {
					name: codtrt,
