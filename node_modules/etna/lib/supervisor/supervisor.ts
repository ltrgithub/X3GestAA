...
const x3Modules = {
	...
	erp: util.arrayOfAny([/* see later*/]),
	hrm: util.arrayOfAny([/* see later*/]),
};
...
function compile(superv: Supervisor, source: string, name: string, filename: string) {
...
export class Supervisor {
	...
	constructor(_: _, config: Config) {
		...
		const types = this.mongoStore.collection('ATYPE', _).find({
			TYPTYP: 7
		}).toArray(_);

		this.constants = Object.assign(util.any({}), constants);
	...
	executeSql(_: _, sql: string, args?: any[], opts?: types.SqlQueryOptions) {
		...
		const result = this.sqlDriver.execute(_, glob.context.x3session.dbConnection, sql, sqlargs, opts);
	...
	sqlReader(_: _, sql: string, args?: any[], opts?: types.SqlReaderOptions) {
		opts = opts || {};
		tracerDbms.debug && tracerDbms.debug("sqlReader: " + sql + ", args=" + args);
		//if (!/^select /.test(sql) && !conf.etnaSqlStore.writable) throw new Error("cannot modify: SQL store is read-only");

		const maptrim = opts.typesmap ? utilDriver.maptrim(this.sqlDriver, opts.typesmap, opts.dispatch) : utilDriver.trim;
		const ftrim = function (_: _, record: types.Row) {
			return opts.typesmap ? maptrim(_, record) : utilDriver.trim(record);
		};
		//debugger;

		return this.sqlDriver.reader(_, glob.context.x3session.dbConnection, sql, args || [], opts).map(ftrim);
	}

	new(_: _, className: string, ...args: any[]) {
	...
	load(_: _, className: string, keyValue: string, dataChildren?: any) {
		...
		if (keyValue !== undefined) {
			...
			data = this.mongoStore.collection(loader.collectionName, _).find(filter).toArray(_)[0];
		}
	...
	defineClass(_: _, def: ClassDefinition) {
	...
	*/
	reader(_: _, className: string, filter: any) {
		const loader = subModule(className);
		const coln = this.mongoStore.collection(loader.collectionName, _).find(filter || {});
	...
	loadText(_: _, num: number, lan?: string) {
		...
		this.mongoStore.collection('ATEXTE', _).ensureIndex({
			LAN: 1,
			TEXTE: 1
		}, {
			background: true
		...
		const data = this.mongoStore.collection('ATEXTE', _).find({
			LAN: lan,
			NUMERO: num
		}).toArray(_)[0];
	...
	loadEnum(_: _, lib: number, lan?: string) {
		...
		var menu = this.cache.LAN[lan] !== undefined ? this.cache.LAN[lan].APLSTD[lib] : undefined;
		...
		menu = this.mongoStore.collection('APLSTD', _).find({
			LAN: lan,
			LANCHP: lib
		}).sort({
			LANCHP: 1,
	...
	fillTypeNode(_: _, data: any, node: types.NodeProto) {
	...
	*/
	...
	moduleName(mod: number) {
		return util.any(x3Modules)[this.product][mod];
	}
	/*
	...
	*/
	loadScript(_: _, name: string, clas?: BaseClass): types.Prog {
		this.loadBuiltins(_);
	...
	loadScripts(_: _, clas: BaseClass):  types.Prog[] {
		if (clas.data.CODTRT) {
	...
	moduleCode(name: string) {
		const i = util.any(x3Modules)[this.product].indexOf(name);
