...
export class SlotLob extends Slot {
	...
	readRecord(_: _,  lobTable, lobField) {
	...
	updateRecord(_: _,  lobTable, lobField, record, reader) {
		...
		const executeSql = (_, sql, i, values) => {
			const name = param(i++);
...
export function newSlotLob(property) {
	return (parent => new SlotLob(parent, property));
}
