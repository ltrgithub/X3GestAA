...
import * as dummy from 'syracuse-core/uuid'; // for definitions
import * as runtimeUtil from '../engine/runtime/util';
import { Frame, InstanceIndex, Prog, ProgArgMembers, ProgArgs, ProgArgsVars, VariableDict, X3Context } from '../engine/runtime/types';
...
export class Helper {
	...
	getActionArgs(_: _,  type: string, action: string, key?: any, member?: ProgArgMembers) {
		var args: ProgArgs = {
			THIS: {
				type: "LY",
				value: this
		...
		if (key) {
			// TODO: clean any cast 
			var index: InstanceIndex = (this as any).index;
	...
	_action(_: _,  action: string, frame: Frame) {
	...
	jsDispatch(_: _,  handler: string, args: ProgArgs, _frame: Frame) {
		...
		this.class.jsModules.forEach_(_, (_, def) => {
			...
			status = Math.max(status, require('../x3js/js-invoke').invoke(_, def, handler.substring(1).toLowerCase(), args, _frame));
		});
		return status;
	}
	
	runScripts(_: _,  handler: string, args: ProgArgs, _frame?: Frame) {
			...
			try {
				if (script.body.$isBuiltin) {
					...
					try {
						...
						const db = require('../engine/runtime/db');
						db.closeFiles([], sl);
					...
					var actx = util.currentContext().x3session.actx;
					...
					try {
						...
						: "") + (args.CURPTH !== undefined ? " CURPTH:" + (args.CURPTH.value || "") : ""));
							const runtime = require('../engine/runtime/runtime');
	...
	toJSON() {
		// why is toJSON not exposed to typescript??
		return (Object.prototype as any).toJSON.call(this);
	}
}
...
export function find<T>(arr: T[], key: string, val: any): T {
	for (var i = 0, len = arr.length; i < len; i++) {
		var elt = arr[i];
		if ((elt as any)[key] === val) return elt;
	}
	return null;
};

export function declVar(dict: VariableDict, name: string, type: string, value: any) {
	dict.values[name] = value;
	dict.types[name] = type;
};

export const uuid = core.uuid.generate;

export function clone<T extends Dict<any>>(obj: T, shallow?: boolean): T {
	return Object.keys(obj).reduce((r, name) => {
		var v = (obj as any)[name];
...
export function path(instance: any, name: string): string {
...
export function escapeJson(input: any): any {
	...
	//
	const out = {} as Dict<any>;

	core.helpers.object.forEachKey(input, function (key, value) {
		var escKey = key;
		var escVal = value;
		if (value !== null) {
...
export function inverseMap(map: Dict<string>) {
	return Object.keys(map).reduce((r, k) => {
		r[map[k]] = k;
		return r;
	}, {} as Dict<string>); // TODO: improve typing
}
...
export function currentContext() {
	return runtimeUtil.currentContext();
}