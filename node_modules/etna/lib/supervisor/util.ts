...
export class Helper {
	...
	getActionArgs(_: _,  type, action, key, member) {
		var args = {
			THIS: {
				type: "LY",
				value: this
	...
	jsDispatch(_: _,  handler, args, _frame) {
	...
	runScripts(_: _,  handler, args, _frame) {
	...
	deleteErrors(_: _,  group, property) {
		this.diagnoses = [];
...
export function declVar(dict, name, type, value) {
...
export function escapeArray(input) {
...
export function checkDataProperty(data, property) {
...
export function decodeValueMongoString(s) {
	return s.replace(new RegExp(String.fromCharCode(255), 'g'), ',');
};

export function traceException(e) {
...
export function upcase(str: string, sep?: string) {
	sep = sep || '_';
	const s = exports.camelize(str).replace(/[A-Z]/g, (ch: string) => sep + ch);
