import { _ } from 'streamline-runtime';
import * as core from 'syracuse-core';
import { Record } from '../engine/drivers/types';
import * as util from '../supervisor/util';
import { Index, Table } from './meta/types';
import { ClassLinkMapMeta } from './metadata/class';
import { Supervisor } from '../supervisor/supervisor';

const tracerJs = core.getTracer("etna.supervisor");

export interface MappingData { // ClassLinkData
	CLELNK: string;
	EXPLNK: string; 
	TABLNK: string;
	MAPS: ClassLinkMapMeta[];
}

export function getIndex(_: _,  superv: Supervisor, mappingData: MappingData) {
	tracerJs.debug && tracerJs.debug("mappingData.TABLNK " + mappingData.TABLNK);
	const childTable = superv.load<Table>(_, 'Table', mappingData.TABLNK);

	return mappingData.CLELNK ? util.find(childTable.indexes, 'name', mappingData.CLELNK) : childTable.indexes[0];
};

export function getKeys(_: _,  mappingData: MappingData, record: Record) {
	const keys = mappingData.EXPLNK.split(';').map_(_, (_, exp) => {
		// for now ignore [F:XXX] part - see later
		const lexp = exp.toUpperCase();

		if (exp.indexOf(']') > 0) {
			var p = exp.substring(exp.indexOf(']') + 1);
			return record[p];
		} else if (lexp.substring(0, 6) === "GACTX.") {
			return util.currentContext().x3session.actx.get(_, lexp.substring(6));
		} else {
			return record[exp] !== undefined ? record[exp] : exp;
		}
	});

	tracerJs.debug && tracerJs.debug("relation keys " + JSON.stringify(keys));
	return keys;
};