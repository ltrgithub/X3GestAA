...
import { Reader, devices } from 'ez-streams';
import { Record } from '../../engine/drivers/types';
import * as util from '../util';
import { Dict, Expression, ProgArgs, ProgArgsVars } from '../../engine/runtime/types';
...
import { Activ, Property, Collection, Column, ColumnDesc, Table, Index, SearchResourceFactory, Type, TypeLob } from './types';
...
function asTypeLob(x: Type): TypeLob { return x as any; }

function bulkWriter(httpContext: http.HttpContext) {
	var sep: string;
	return devices.generic.writer((_: _, resource: Resource) => {
		if (!sep) {
...
export type TypeMapper = (_: _, row: Record) => Instance;
...
export interface MethodParam {
	...
	get(args: any[]): any;
	set?: (_: _, args: Expression<any>[], value: any) => Expression<any>;
}
...
export interface Method {
	...
	buildArgs?: (_: _,  instance: Instance) => ProgArgs;
	getServiceArgs?: (_: _,  key: string, qs: any) => any;
	getServiceReturn?: (_: _,  res: any) => ServiceReturn;
	//index(args: any[]): number;
	//get(args: any[]): any;
	//set?: (_: _, args: Expression<any>[], value: any) => any;
}
...
export class Class extends BaseClass {
	...
	codclaPath: string;
	data: ClassMeta;
	reader: (_: _, parent: Class, select: Select, max: number, mapper?: TypeMapper) => Reader<Instance>;
	read: (_: _, instance: Instance, key: string) => number;
	jsModules: any[];
	jsInterface: JsInterface;
	jsName: string;
	isInterface: boolean;
	queryInstances: (_: _, select: Select, max: number) => Reader<Instance>;
	_scripts: any[];
	mandatoryProperties: string[];
	stdMethods: StdMethods;
	methods: Methods;
	instancesCache: {
		funnel: (_: _, body: (_: _) => void) => void;
		instances: Dict<Instance>;
	}
	
	
	static cast(clas: BaseClass): Class { return clas as any; }

	constructor(superv: Supervisor, data: ClassMeta, group: string) {
		super(superv, data);
		if (data.PROPERTIES) data.PROPERTIES.sort((a, b) => a.NUMLIG - b.NUMLIG);
		this.relations = {};
		this.parent = null;
	...
	init(_: _) {
		...
		if (this.data.dataChildren) {
			if (this.data.dataChildren.ownMapping) {
				...
				this.data.table = this.supervisor.load<Table>(_, 'Table', this.data.dataChildren.ownMapping.TABLNK);
		...
		if (this.data.TYPCLA === 2) {
			...
			this.table = this.table || this.supervisor.load<Table>(_, 'Table', tableCode);
		...
		const supervPropertiesFields = systemProperties.map(property => property.FLDCLA);
		...
		this.properties = supervProperties.reduce_(_, (_, r, propData) => {
			...
			if (!((this.group || "") !== (!propData.LNKCLA && propData.FLDGRP || "")) || supervPropertiesFields.indexOf(name) >= 0) {
				...
				r[name] = this.supervisor.new<Property>(_, 'Property', this, propData);
				...
				if (r[name].isReference()) {
					...
					r[name] = this.supervisor.new<Property>(_, 'Property', this, propData, 'reference');
		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, (_, data) => {
			...
			const coln: Collection = this.relations[data.CODCOL] = this.supervisor.new<Collection>(_, 'Collection', this, data);
			const name = data.CODCOL;

			var p = this.properties[name];
			if (!p) {
				this.relations[name] = coln;
				p = this.properties[name] = this.supervisor.new<Property>(_, 'Property', this, {
					FLDCLA: name
				}, 'collection', coln);

				// generate synthetic class for group of indexed properties
				coln.ofIndexedProperties = true;
				coln.class = this.supervisor.new<Class>(_, 'Class', {
				...
				if (data.ACTGRP && data.ACTGRP !== "") {
					var activ = this.supervisor.load<Activ>(_, 'Activ', data.ACTGRP);
					coln.count = activ.dimension;
				} else if (data.MAXCOL > 0) {
					coln.count = data.MAXCOL;
				}
				const colnClass: Class = coln.class as any;
				if (!coln.count) {
					coln.count = 1;
						...
						if (coln.class.table) {
							...
							if (col) {
								...
								if (col.CODACT && col.CODACT !== "") {
									var activcol = this.supervisor.load<Activ>(_, 'Activ', col.CODACT);
				...
				var clas = this.supervisor.load<Class>(_, 'Class', this.codclaPath + "." + targetName, {
				...
				if (coln.mappingData && coln.mappingData.TYPLNK === 3) {
					...
					this.properties[name] = this.supervisor.new<Property>(_, 'Property', this, {
						FLDCLA: name
					}, 'instance', coln);
				} else {
					this.properties[name] = this.supervisor.new<Property>(_, 'Property', this, {
			...
			r[method.CODMET] = {
				...
				params: method.PARAM_DEFS ? method.PARAM_DEFS.reduce_(_, (_, r, param) => {
					...
					if (param.MODPAR === 1) {
						...
						r[param.CODPAR].set = (_: _, args: Expression<any>[], value: any) => {
							const arg = args[np];
				...
				buildArgs(_: _,  instance: Instance) {
					...
					for (var inst: any = instance; inst.$parent; inst = inst.$parent.$parent) {
						(args as any).path.unshift(inst.$parent.jsName);
					}
					return args;
				},
				...
				getServiceReturn(_: _,  res: any) {
					const newReturn: ServiceReturn = {
						$type: variables.types[typDonmet2EtnaType[method.DONMET]].jsonDefaultType,
					};
					if (method.DONMET === 6) {
		...
		if (this.data.FLGBUFFER === 2) {
			Object.keys(cacheMethods).forEach(method => {
				(this as any)[method] = (cacheMethods as any)[method];
			});
		}
		return this;
	}
 ...
 */
	addMethods(instance: Instance) {
		Object.keys(this.stdMethods).forEach(stdMethod => {
			(instance as any)[stdMethod] = (instance as any)["$" + stdMethod];
		});
	...
	createInstance(_: _,  qs?: void) {
		return this.supervisor.new<Instance>(_, 'Instance', this).afterCreate(_);
	}
	...
	queryInstancesSql(_: _,  select: Select, max: number): Reader<Instance> {
		return this.reader(_, null, select, max, null);
	}

	queryInstancesInt(_: _,  select: Select, max: number): Reader<Instance> {
		...
		const lnk = util.currentContext().queryFrame.context.tables['LNK_'];

		if (lnk) {
			//ORDER('KEY1',[ORDERITEM(SQLCV('F','BPAZ','CRY'),'+')],null, null, C(null))
			var frame = util.currentContext().x3frame;
			util.currentContext().x3frame = util.currentContext().queryFrame;

			var where = selectToWhere(select);
			var order = selectToOrder(select);
			var filter = z.FILTER("LNK_", where, order);
			filter && filter(_);
			util.currentContext().x3frame = frame;

			const cursor = lnk.x3OpenFor(_, lnk.findKey(""), 0, null, false);
			reader = devices.generic.reader(_ => {
			...
			reader = devices.generic.reader(_ => undefined);
		}
		return reader.limit(max).map(this.class.fromRecordMapper(_));
	}
	sqlReader(_: _,  parent: Class, select: Select, max: number, $recordMapper: TypeMapper): Reader<Instance> {
		...
		const recordMapper = $recordMapper || ((_, record): Instance => {
			return this.supervisor.new<Instance>(_, 'Instance', this, parent).afterCreate(_, record);
		});
		...
		if (this.data.ACCSTR) {
			...
			params.push(util.currentContext().x3session.actx.$USER(_));
		...
		var reader = this.supervisor.sqlReader<Record>(_, sql, params, sqlOpts).limit(max).map(recordMapper);
		if (cache.isActivated) reader = reader.transform(cache.transform());
		return reader;
	}
	jsReader(_: _,  parent: Class, select: Select, max: number, $recordMapper: TypeMapper): Reader<Instance> {
		...
		const recordMapper = $recordMapper || ((_: _, record: any): Instance => {
			return this.supervisor.new<Instance>(_, 'Instance', this, null).afterCreate(_, record);
		});
	...
	rsReader(_: _,  recordset: Record[], select: Select): Reader<Record> {
		if (select && select.$orderBy) {
		...
		var reader = devices.generic.empty.reader.transform((_, reader, writer) => {
	...
	loadInstances(_: _,  parent: Class, index: Index, vals: any[], max?: number, $recordMapper?: TypeMapper): Reader<Instance> {
		return this.reader(_, parent, vals.reduce((o, val, i) => {
			if (index.columns[i]) {
				if (val !== undefined) {
					o[index.columns[i].name] = val;
				} else {
					o[index.columns[i].name] = index.columns[i].type.defaultValue;
				}
			}
			return o;
		}, {} as Dict<any>), max, $recordMapper);
	}
	fromRecordMapper(_: _,  instance?: Instance) {
		return (_: _, record: Record) => {
			...
			const ins = instance ? instance : this.supervisor.new<Instance>(_, 'Instance', this, null);
	...
	sqlRead(_: _,  instance: Instance, key: any) {
		...
		const instances = this.reader(_, (instance as any).parent, this.keyToSelect(key), 1, mapper).toArray(_);
	...
	searchResourceFactory(_: _) {
		return this.supervisor.new<SearchResourceFactory>(_, 'SearchResourceFactory', this);
	}
	searchProto(_: _) {
	...
	search(_: _,  httpContext: http.HttpContext, qs: any) {
		...
		const fromInstanceToResource = (_: _, instance: Instance) => {
			return this.supervisor.new<Resource>(_, 'Resource', factory, instance).bindInstance(instance);
		};
	...
	readLob(_: _,  context: http.HttpContext, property: string, codblb: string, ident1?: string, ident2?: string, ident3?: string) {
		...
		if (lob) {
			if (lob.type.readMediaLazy) {
				...
				const param = this.supervisor.sqlDriver.param;
				var i = 0;
				var rs = this.supervisor.sqlReader<Record>(_, "select " + lob.lobField + "_0 From " + lob.lobTable + " Where " + 
				Object.keys(keys).map(p => p + '_0=' + param(i++)).join(' and '), 
				Object.keys(keys).map(p => (keys as any)[p])).toArray(_);
