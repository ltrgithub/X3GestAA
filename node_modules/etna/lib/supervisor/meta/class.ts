...
import { BaseClass, BaseClassData } from './baseClass';
...
import { newSlotInstance, Instance } from './instance';
...
export interface ParamDefsData {
	CODE: string;
	TYPKEY: number;
	NOPAR: number;
}

export interface OwnMapping {
	TABLNK: string;
}
export interface ClassDataChildren {
	ownMapping: OwnMapping;
	LINKS: ClassLink[];
	MAPS: ClassMap[];
	codclaPath: string;
	parent: Class;
...
export class Class extends BaseClass {
	...
	codclaPath: string;
	data: ClassData;
	reader: (_: _, parent: Class, select: string[], max: number, mapper: TypeMapper) => ez.Reader<Instance>;
	read: (_: _, instance: Instance, key: string) => void;
	membersByJsName: { [name: string]: any };
	jsModules: any[];
	jsInterface: Helper;
	jsName: string;
	isInterface: boolean;
	queryInstances: (_: _, select: string, max: number) => ez.Reader<Instance>;
	_scripts: any[];
	
	constructor(superv: Supervisor, data: ClassData, group: string) {
		super(superv, data);
	...
	init(_: _) {
		...
		const supervPropertiesFields = _supervProperties.reduce((r, property) => {
		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, (_, data) => {
			...
			const coln = this.relations[data.CODCOL] = this.supervisor.new(_, 'Collection', this, data);
			...
			if (!p) {
				...
				if (!coln.count) {
					coln.count = 1;
					coln.class.data.PROPERTIES.forEach_(_, (_, propData) => {
						if (coln.class.table) {
				...
				if (coln.class.data.PROPERTIES) {
					coln.class.data.PROPERTIES.forEach_(_, (_, prop) => {
						if (coln.class.data.PARAM_FLDS) {
							coln.class.data.PARAM_FLDS.forEach_(_, (_, fld) => {
			...
			r[method.CODMET] = {
				...
				params: method.PARAM_DEFS ? method.PARAM_DEFS.reduce_(_, (_, r, param) => {
					...
					if (param.MODPAR === 1) {
						...
						r[param.CODPAR].set = (_, args, value) => {
							const arg = args[np];
				...
				buildArgs(_: _,  instance) {
					...
					args.path = [];
					// set path for x3js
					for (var inst = instance; inst.$parent; inst = inst.$parent.$parent) {
						args.path.unshift(inst.$parent.jsName);
					}
					return args;
				},
				getServiceArgs(_: _,  key, qs) {
		...
		if (this.data.FLGBUFFER === 2) {
			Object.keys(cacheMethods).forEach(method => {
				this[method] = cacheMethods[method];
			});
		}
		return this;
	}
 ...
 */
	addMethods(instance) {
		...
		const xmeth = (clas, method, methodType) => {
	...
	queryExpToSelect(_: _,  exp, params, forJs) {
		const map = {
			'$eq': '=',
			'$ne': '!=',
			'$gt': '>',
	...
	queryToSelect(_: _,  query, params, forJs) {
		const operator = query.$or ? "or" : query.$and ? "and" : undefined;

		if (operator) {
			return query['$' + operator].reduce_(_, (_, r, exp) => {
	...
	queryInstancesSql(_: _,  select, max) {
		return this.reader(_, null, select, max, null);
	}

	queryInstancesInt(_: _,  select, max) {
		const rt = require('etna/lib/engine/runtime');
		const z = rt.instructions;
		const expToWhere = exp => {
	...
	sqlReader(_: _,  parent, select, max, $recordMapper) {
	...
	jsReader(_: _,  parent, select, max, $recordMapper) {
	...
	loadInstances(_: _,  parent, index, vals, max, $recordMapper) {
	...
	keyToSelect(key) {
		...
		return Object.keys(this.index).reduce((r, i) => {
			r[this.index[i].name] = key[i];
			return r;
		}, {});
	}
	sqlRead(_: _,  instance, key) {
		const mapper = this.fromRecordMapper(_, instance);

		instance.snap && instance.snap.enable();
		const instances = this.reader(_, instance.parent, this.keyToSelect(key), 1, mapper).toArray(_);
	...
	search(_: _,  httpContext, qs) {
		...
		const fromInstanceToResource = (_, instance) => {
	...
	readLob(_: _,  context, property, codblb, ident1, ident2, ident3) {
		const ident = s => s || " ";
		...
		if (lob) {
			if (lob.type.readMediaLazy) {
				...
				let contentType = lob.type.readMediaCntLazy ? lob.type.readMediaCntLazy(_, lob, [codblb, ident1, ident2, ident3]) : "text/plain";
				...
				const param = this.supervisor.sqlDriver.param;
				var i = 0;
				var rs = this.supervisor.sqlReader(_, "select " + lob.lobField + "_0 From " + lob.lobTable + " Where " + Object.keys(keys).map(p => p + '_0=' + param(i++)).join(' and '), Object.keys(keys).map(p => keys[p])).toArray(_);

				if (rs.length) {
					context.response.writeHead(200, {
						'content-type': lob.type.contentType(_),
						'Transfer-Encoding': 'chunked',
						'X-FRAME-OPTIONS': 'DENY',
						'content-disposition': 'attachement;filename=' + keys.IDENT1
					});
					// Pipe the data to HTTTP:
					return lob.type.reader(_, rs[0][lob.lobField]).pipe(_, context.response.writer);
				}
			}
		}
...
export const systemProperties = _supervProperties;
