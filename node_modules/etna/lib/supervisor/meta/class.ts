...
export type TypeMapper = (_: _, row: types.Row) => Instance;
...
export interface JsInterface {
	AQUERY: (_: _,  select: Select) => types.Row[];
	//[name: string]: (_: _,  select: Select) => types.Row[];
}

export interface StdMethods {
	AREAD?: StdMethodData;
	AINSERT?: StdMethodData;
	AUPDATE?: StdMethodData;
	ADELETE?: StdMethodData;
}
 ...
 */
	...
	rsReader(_: _,  recordset: types.Row[], select: Select) {
		if (select && select.$orderBy) {
			...
			var sort = (o1: types.Row, o2: types.Row) => {
				var cmp = 0;
