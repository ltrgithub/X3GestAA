...
import { Collection, CollectionData, ClassLinkData } from './collection';
...
export const systemProperties: PropertyData[] = [{
	FLDCLA: "ASTALIN",
	CODTYP: "C",
	LONG: 4
}, {
...
export interface ParamFldData {
	FLDCLA: string;
	VALEUR: string;
}

export interface StdMethodData {
	ENAMETSTD: number;
	ACTMETSTD: string;
	CODMETSTD: string;
}

export interface MethodParamDefsData {
	CODPAR: string;
	TYPINTPAR: number;
	MODPAR: number;
	TYPKEY: number;
...
export interface MethodData {
	CODMET: string;
	FLGOPE: number;
	PARAM_DEFS: MethodParamDefsData[];
	DONMET: number;
}
...
export interface ClassData extends BaseClassData {
	...
	PARAM_FLDS: ParamFldData[];
	PROPERTIES: PropertyData[];
	COLLECTIONS: CollectionData[];
	METHODS: MethodData[];
	STD_METHODS: StdMethodData[];
	LINKS: ClassLinkData[];
	MAPS: ClassMap[];
	KEYINT: string;
	INDREF: string;
...
export interface JsInterface {
	AQUERY: (_: _,  select: Select) => ez.Reader<types.Row>;
}
			...
			if (!p) {
				...
				if (!coln.count) {
					...
					colnClass.data.PROPERTIES.forEach_(_, (_, propData) => {
						if (coln.class.table) {
							var col: ColumnData = util.find(coln.class.table.data.COLUMNS, 'CODZONE', propData.FLDCLA);
							if (col) {
								var dime = 0;
			...
			r[method.CODMET] = {
				...
				params: method.PARAM_DEFS ? method.PARAM_DEFS.reduce_(_, (_, r, param) => {
					...
					if (param.MODPAR === 1) {
						...
						r[param.CODPAR].set = (_: _, args, value) => {
							const arg = args[np];
 ...
 */
	...
	queryExpToSelect(_: _,  exp, params, forJs?: boolean) {
		const map = {
			'$eq': '=',
			'$ne': '!=',
			'$gt': '>',
	...
	queryToSelect(_: _,  query: any, params, forJs?: boolean) {
	...
	sqlReader(_: _,  parent, select: Select, max: number, $recordMapper) {
	...
	intRead(_: _,  instance: Instance, key) {
		return this.intCrud(_, 'AREAD', instance, key);
	}
	intCrud(_: _,  f_name: string, instance: Instance, key) {
