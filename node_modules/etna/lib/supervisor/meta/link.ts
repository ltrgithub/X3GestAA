...
import { LinkNodeProto, LinkParamsNodeProto, ParamPropertiesNodeProto } from '../protocol/prototype';
import * as types from '../../engine/runtime/types';
import { Supervisor } from '../supervisor';
import * as util from '../util';
import * as activ from '../meta/activ';
import { LinkMeta, LinkParamMeta } from '../metadata/representation';
import { Representation, ResourceFactory } from './types';

const tabValActx: { [name: string]: string; } = {
	"ACTX.USER": "$actxUser",
	"GACTX.USER": "$actxUser",
...
export const enumStdLinks: { [id: number]: StdLink } = {
...
export class Link {
	...
	data: LinkMeta;
	keys: LinkParamMeta[];
	params: LinkParamMeta[];
	links: { [name: string]: Link; };
	constructor(superv: Supervisor, anchor: Anchor, data: LinkMeta) {
	...
	prepareKeyparams(repr: Representation) {
		const fvalkey = (value: any, typint: number) => {
   ...
   *		}
   */
		const fvalpar = fvalkey;

		const prepParam = (repr: Representation, paramData: LinkParamMeta, key: any) => {
			...
			const propData = util.find(repr.data.EXPORTED_PROPERTIES, 'CODPRO', paramData.VALEUR);
		...
		this.keys = [];
		if (this.data.PARAMS) {
			var keyParams = this.data.PARAMS.filter(paramData => {
				//return paramData.TYPPAR === 3 && paramData.TYPKEY === 2 && paramData.CODLNK === this.data.oCODLNK && paramData.AFFLNK === this.data.oAFFLNK && paramData.CODFLD === this.data.ANCLNK;
				return paramData.TYPPAR === 3 && paramData.TYPKEY === 2;
			}).map((paramData) => prepParam(repr, paramData, true));
			this.keys = keyParams;
			this.params = this.data.PARAMS.filter(paramData => {
				//return paramData.TYPPAR === 3 && paramData.TYPKEY === 1 && paramData.CODLNK === this.data.oCODLNK && paramData.AFFLNK === this.data.oAFFLNK && paramData.CODFLD === this.data.ANCLNK;
				return paramData.TYPPAR === 3 && paramData.TYPKEY === 1;
			}).map((paramData) => prepParam(repr, paramData, false));
		}
	}
	prototypeNode(_: _) {
		const node: LinkNodeProto = {
		...
		switch (this.data.TYPLNK) {
			case 1:
				...
				switch (this.data.CMPLNK) {
					...
					case 7:
						...
						var params: LinkParamsNodeProto = {};
						params.$method = "POST";
						...
						var props: ParamPropertiesNodeProto = {};
						props.dummy = {};
						props.$title = "dummy";
					...
					case 17:
						...
						Object.keys(this.links).filter(name => name == "$details").forEach_(_, (_, name) => {
			...
			case 3:
				...
				node.$parameters = {};
				// TODO: clean type of node.$parameters
				node.$url = "{$baseUrl}/" + this.data.CLALNK + "('" + this.keyExpression() + "')/" + //
				"$services/" + this.data.CODLNK + "?representation=" + repr.name + "." + this.anchor.facet + //
					this.params3Expression(_, util.any(node.$parameters));
				break;
			case 4:
				// convergence
				node.$type = "application/json;vnd.sage=syracuse";
				node.$target = "main";
				//node.$method = "POST";
				if (this.data.CMPLNK === 1) node.$url = this.supervisor.transUrl + "/$sessions?f=" + this.data.CODFNCLNK + '/2//C/';
				else {
	...
	paramsFilterExpression(_: _) {
		const getSep = (type: number) => {
			switch (type) {
				case 7:
				case 11:
					return "'";
		...
		var filter = "";
		this.params.filter(paramData => paramData.PARFIL === 2 && (!!paramData.alias || !!paramData.value)).forEach(paramData => {
			filter = (filter !== "" ? filter + ' and ' : "") + "(" + paramData.CODPAR + " eq " + getSep(paramData.TYPINT) + (paramData.alias ? '{' + paramData.alias + '}' : paramData.value === '""' ? '' : paramData.value) + getSep(paramData.TYPINT) + ")";
		});
		if (filter) return '&where=' + filter;
		else return "";
	}
	paramsEntExpression(_: _) {
		return this.params.filter(paramData => paramData.PARENT === 2 && (!!paramData.alias || !!paramData.value)).map(paramData => {
			return '&' + paramData.CODPAR + "=" + (paramData.alias ? '{' + paramData.alias + '}' : paramData.value);
		}).join('');
	}
	params3Expression(_: _,  parameters: { [name: string]: string }) {
		return this.params.map(paramData => {
			if (paramData.alias) parameters[paramData.CODPAR] = '{' + paramData.alias + '}';
			else parameters[paramData.CODPAR] = "" + paramData.value; //convert to string
			return '&' + paramData.CODPAR + "={" + paramData.CODPAR + "}";
		}).join('');
	}
	params4Expression(_: _) {
		const isAfcObjGES3 = (_: _, link: Link) => {
			//VVV when ETL will be OK replace withMongoDB read
			const param = link.supervisor.sqlDriver.param;
			const params: string[] = [];
			const wheres: string[] = [];

			params.push(link.data.CODFNCLNK);
			wheres.push("CODINT_0=" + param(params.length - 1));
			const sql = "select ACTION_0,CODPAR_0,VALPAR_0 from AFONCTION  where (" + wheres.join(" and ") + ")";
			const recordset = link.supervisor.sqlReader<{
				ACTION: string;
				CODPAR: string;
				VALPAR: string;
			...
			if (recordset.length > 0) {
				if (recordset[0].ACTION === "GOBJET" && recordset[0].CODPAR === "OBJET" && recordset[0].VALPAR !== "") {
					...
					var recordset2 = link.supervisor.sqlReader<{
						TYPGES: number;
					}>(_, sql2, params2).toArray(_);
		...
		if (this.keys.length === 2 && isAfcObjGES3(_, this) === true) {
			var expKeys = this.keys.map(paramData => {
				return paramData.alias ? '{' + paramData.alias + '}' : paramData.value;
			}, this).join('~') || "";
		} else {
			var expKeys = "";
			var no = 1;
			this.keys.forEach(paramData => {
				if (no === 2) {
		...
		const params = this.params.map(paramData => {
	...
	keyExpression() {
		return this.keys.map(paramData => {
			return paramData.alias ? '{' + paramData.alias + '}' : paramData.value;
		}, this).join('~');
	}
	url(_: _,  facet: string, withKey?: boolean, withWorkingCopy?: boolean, withTemplate?: boolean) {
		//console.log("data:",this.data);
		const key = withKey ? "('" + this.keyExpression() + "')" : "";
		const workingCopy = withWorkingCopy ? "/$workingCopies" : "";
		const template = withTemplate ? "/$template" : "";
		const url = "{$baseUrl}/" + this.data.CLALNK + key + template + workingCopy + "?representation=" + this.data.REPLNK + "." + facet;

		return url;
	}
};
export const Constructor = Link;