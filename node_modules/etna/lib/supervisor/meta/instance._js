...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	action: function(_, action, key, crupro, curpth, frame) {
		...
		var dargs = this.getActionArgs(_, "AEVENT", action, key, crupro, curpth);
	...
	afterQuery: function(_) {
		return this.action(_, "AQUERY_TRANS_AFTER", null, null, null, glob.context.queryFrame);
	},
		...
		try {
			...
			Object.keys(properties)
				...
				.forEach_(_, function(_, i) {
					...
					// Send "CONTROL"
					status = Math.max(status,instance.propertyControlValue(_, property));
				});
			if (status === constants.CST_AERROR) return status;
