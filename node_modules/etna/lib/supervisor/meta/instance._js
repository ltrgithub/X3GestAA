...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	action: function(_, action, key, member) {
		tracerEvents.debug && tracerEvents.debug("$EVENTS:" + action + " KEY:" + (key || ""));
		...
		while (instance !== undefined) {
			if (instance.runScripts) {
				status = Math.max(status, instance.runScripts(_, "$EVENTS", dargs));
			}
