...
class Instance extends BaseInstance {
	...
	revertTo() {
		...
		Object.keys(from.slots).filter(function (property) {
	...
	getProKeys(_) {
		...
		if (this.$parent && dataChildren && dataChildren.ownMapping) {
			var instance = this;
			dataChildren.ownMapping.MAPS.filter(function (map) {
	...
	read(_, key, options) {
		...
		const AREAD = function (_, instance) {
	...
	insert(_, options) {
		...
		const AINSERT = function (_, instance) {
	...
	update(_, options) {
		...
		const AUPDATE = function (_, instance) {
			const actx = glob.context.x3session.actx;
	...
	delete(_, options) {
		...
		const ADELETE = function (_, instance) {
	...
	action(_, action, key, member, frame) {
		const newCurPth = function (p1, p2) {
			return [p1, p2].filter(function (p) {
				return !!p;
			}).join('.');
		};
	...
	executeControlOthers(_, type) {
		...
		try {
			...
			Object.keys(this.properties).filter(property => {
				return this.properties[property].toControl;
