...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	executeControlOthers: function(_, type) {
		/*dbg*/console.log("executeControlOthers:"+type);
		...
		try {
			...
			Object.keys(properties)
				.filter(function(property) {
					return !properties[property].FLDGRP;
				})
				.forEach_(_, (_, i) => {
					...
					// Send "CONTROL"
					status = Math.max(status,instance.propertyControlValue(_, this.properties[property]));
				});
			if (status === constants.CST_AERROR) return status;
