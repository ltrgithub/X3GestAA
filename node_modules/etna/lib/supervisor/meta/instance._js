...
var traceCRUD = require('syracuse-core').getTracer("etna.crud");
var tracerEvents = require('syracuse-core').getTracer("etna.events");


var baseInstance = require("etna/lib/supervisor/meta/baseInstance");
...
var Instance = exports.constructor = baseInstance.defineClass(function(superv, clas, parent, slots) {
	...
	setInstance: function(_, record) { // Manages instruction setInstance
		var instance = this;
		if (record) {
			if (record.AUUID) this.$uuid = record.AUUID;
			Object.keys(this.properties).forEach_(_, function(_, p) {
				if(record[p]) {
					var slot = this.slot(_, p);
					slot && slot._set(_,record[p]);
				}
			}, this);
		}
		return this;
	...
	action: function(_, action, key, member, frame) {
		var newCurPth = function(p1,p2) {
			return [p1,p2].filter(function(p) {return !!p;}).join('.');
		}
		...
		var dargs = this.getActionArgs(_, "AEVENT", action, key, member);
		...
		while (instance !== undefined) {
			...
			if (["AQUERY_TRANS_AFTER"].indexOf(action) < 0) {
				...
				if (typeof dargs !== 'function') {
					...
					dargs.path.unshift(instance.proJsName || instance.proName || "");
				}
			}
	...
	executeControlOthers: function(_, type) {
		...
		try {
			...
			Object.keys(this.properties)
				.filter((property) => { return this.properties[property].toControl;})
				.forEach_(_, (_, property) => {
					var slot = this.slots[property];
					if(slot) {
						if(slot.controlOthers)  status = Math.max(status, slot.controlOthers(_));
						if(this.properties[property].isReference() && !this.properties[property].isAtextra){
							// Add control on the reference :
							slot = this.properties[property+"_REF"].new(_, this);
							status = Math.max(status, slot.controlOthers(_));
						}
					}
					// Send "CONTROL"
					status = Math.max(status,this.propertyControlValue(_, this.properties[property]));
				});
			if (status === constants.CST_AERROR) return status;
	...
	initMaps: function(_, collection, parent) {
		function defMap(_,map) {
			var value="";
			var lVALDEFMAP = map.VALDEFMAP.toUpperCase();
		...
		if(collection.mappingData && collection.mappingData.MAPS) {
			collection.mappingData.MAPS.forEach_(_, function(_, map) {
