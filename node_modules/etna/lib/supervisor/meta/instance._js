		...
		while (instance !== undefined) {
			...
			if (["AQUERY_TRANS_AFTER"].indexOf(action) < 0) {
				...
				if (typeof dargs !== 'function') {
					...
					//if (!instance.proJsName) console.error("BAD PROJSNAME", instance.proName);
		...
		try {
			...
			Object.keys(properties)
				...
				.forEach_(_, (_, i) => {
					...
					// Send "CONTROL"
					status = Math.max(status, instance.propertyControlValue(_, instance.class.properties[property]));
				});
			if (status === constants.CST_AERROR) return status;
