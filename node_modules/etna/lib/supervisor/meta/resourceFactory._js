...
class ResourceFactory extends Helper {
	...
	itemProto(_, proto, facet) {
		...
		Object.keys(self.properties).filter(function (name) {
			...
			const prop = self.properties[name];
	...
	deleteResource(_, httpContext, $key, qs) {
		...
		const reply = {
			$diagnoses: resource.instance.getDiagnoses(_).reduce(function (r, diag) {
	...
	filtersTabCor(_) {
		const self = this;
		const list = Object.keys(this.properties).reduce(function (r, property) {
	...
	filterRep(_, name) {
		if (this.representation._filters === undefined || this.representation._filters[name] === undefined) {
			...
			this.representation.data.FILTERS && this.representation.data.FILTERS.forEach_(_, function (_, elt) {
				// GH 6167		
				if (elt.OPTCOD === name) option = elt;
				if (name == undefined) {
					if (elt.OPTDEF === 2) {
						if (elt.OPTACT !== undefined && activ.getActiv(_, self.supervisor, elt.OPTACT) <= 0) {
							// GH 6167																		
							setdefaut = 1; // GH 6167
						} // GH 6167
						else {
	...
	loadResources(_, select, max) {
		const fromInstanceToResource = function (self) {
			return function (_, reader, writer) {
				var instance;
				var astatus;
				const cache = self.cache ? self.cache.clone() : undefined;


				while (instance = reader.read(_)) {
					var resource = self.supervisor.new(_, 'Resource', self, instance).bindInstance(instance);
	...
	query(_, httpContext, qs) {
		...
		select.$params = Object.keys(qs).reduce(function (r, tag) {
			const property = self.tag2PropertyName[tag] || self.properties[tag] && self.properties[tag].tag;
	...
	keyExpression() {
		...
		if (this.keyItems) return this.keyItems.map(function (keyitem) {
