...
exports.defineClass = function(constructor, parent, members) {
	...
	members.ASETTEXT = function(_, property, text) {
		console.log(property+".ASETTEXT:"+text)
		this.set(_,property,text);
		return constants.CST_AOK;
	};
	members.ASETTEXTLAN = function(_, property, text, lan) {
		var slot = this.slot(_, property);
	...
	function addPropertyEvent(action) {
		return function(_, property, oldValue) {
			tracerEvents.debug && tracerEvents.debug(action + " on " + property + " " + this.$$type + " (" + (this.meta.name+" ") + this.$$dbgId + ")");

			var args = this.getActionArgs(_, "ARULE", action, null, property);
			if (oldValue) {
