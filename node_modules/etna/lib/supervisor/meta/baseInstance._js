...
class BaseInstance extends Helper {
	...
	propertyEvent(action) {
		...
		return function(_, prop, oldValue) {
			...
			// console.log(action + " on " + prop.name + " " + self.$$type + " (" + (self.meta.name+" ") + self.$$dbgId + ")");

			var status = constants.CST_AOK;
			if(prop.type && prop.type[action]) {
				status = prop.type[action](_,self,undefined);
			}
	...
	propertyInitValue(_, prop, oldValue) { 
		return this.propertyEvent("INIT")(_, prop, oldValue);
	}

	propertyGetValue(_, prop, oldValue) { 
		return this.propertyEvent("GET")(_, prop, oldValue);
	}

	propertyControlValue(_, prop, oldValue) {
		return this.propertyEvent("CONTROL")(_, prop, oldValue);
	}

	propertyPropagateValueEvent(_, prop, oldValue) {
		return this.propertyEvent("PROPAGATE")(_, prop, oldValue);
	}
	...
	propertyPropagateValue(_, prop, oldValue) {
