...
exports.defineClass = function(constructor, parent, members) {
		...
		if(properties) {
			for (var i = 0; i < properties.length; i++) {
				if (!properties[i].FLDGRP) {
					status = Math.max(status, this.propertyInitValue(_, this.properties[properties[i].FLDCLA]));
				}
			}
		}
		return status;
	...
	function addPropertyEvent(action) {
		return function(_, prop, oldValue) {
			tracerEvents.debug && tracerEvents.debug(action + " on " + prop.name + " " + this.$$type + " (" + this.$$dbgId + ")");
			//console.error("PROP EVENT", action, prop && prop.name, oldValue);
			var args = this.getActionArgs(_, "ARULE", action, null, prop);
			if (oldValue) {
			...
			while (instance !== undefined) {
				...
				args.path.unshift(instance.proName);
	...
	members.propertyPropagateValue = function(_, prop, oldValue) {
		// Mark the instance as changed :
		this.setASTALIN(constants.CST_AUPD);
		return this.propertyPropagateValueEvent(_, prop, oldValue);
	};
