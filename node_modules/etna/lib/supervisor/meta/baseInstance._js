...
exports.defineClass = function(constructor, parent, members) {
	...
	members.withControl = function(_, fname, f) {
		try {
			...
			traceSup.error && traceSup.error(e.message);
	...
	members.AINIT = function(_) {
		...
		status = this.propertiesEvent(_, "INIT")
		if(status === constants.CST_AERROR) return status;
		if (this.instance) {
			status = this.instance.propertiesEvent(_, "INIT");
			if(status === constants.CST_AERROR) return status;
		}
		// call AINIT event 
	...
	function addPropertyEvent(action) {
		return function(_, prop, oldValue) {
			tracerEvents.debug && tracerEvents.debug(action + " on " + prop + " " + this.$$type + " (" + (this.meta.name+" ") + this.$$dbgId + ")");
	...
	*/
	...
	members._activeLoading = function(_) {
		var self = this;
