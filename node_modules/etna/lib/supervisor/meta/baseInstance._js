...
class BaseInstance extends Helper {
			...
			if (name === 'AERROR') {
				...
				this.sysProperties.ASTALIN = {
					...
					set: (_, astalin) => {
						...
						if (this.$parent && this.$parent.collection && !this.$parent.collection.mappingData) {
	...
	propertyEvent(action) {
		return (_, prop, oldValue, newValue) => {
			...
			if (prop.type && prop.type[action]) {
				...
				var slot = prop.property ? prop : this.slot(_, prop.name);
				status = prop.type[action](_, slot);
				if (status === constants.CST_AERROR) return status;
			}

			const args = this.getActionArgs(_, "ARULE", action, null, prop);
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				...
				.reduce_(_, (_, r, property) => {
					const diags = this.slots[property].getDiagnoses(_, sort);

					if (diags) r = r.concat(diags);
					return r;
				}, this.diagnoses || []).sort((d0, d1) => d0[sort](d1));
			case constants.CST_ALL:
			default:
