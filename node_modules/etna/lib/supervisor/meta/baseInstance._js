...
class BaseInstance extends Helper {
	...
	deleteDiagnoses(_, etype) {
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				this.diagnoses = [];
				Object.keys(self.slots).filter(pro => self.slots[pro].collection === undefined).forEach_(_, function (_, property) {
	...
	getDiagnoses(_, sort) {
		...
		const local = (this.diagnoses || []).map_(_, function (_, d) {
 ...
 */
	...
	getSortedDiagnoses(_, etype, sort) {
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				return Object.keys(self.slots).filter(pro => self.slots[pro].collection === undefined).reduce_(_, function (_, r, property) {
	...
	AGETERRORSFROM(_, iorigin, env) {
		const errc = iorigin.getSortedDiagnoses(_).map_(_, function (_, e) {
	...
	ASEARCHINFOS(_, local, property, status, selstatus) {
		const errt = this.getSortedDiagnoses(_, local).filter_(_, function (_, e) {
			var ftr = true;
