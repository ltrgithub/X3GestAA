...
const X3SON = require("etna/lib/engine/x3son");
const mapping = require("etna/lib/supervisor/mapping");
const constants = require("etna/lib/supervisor/constants");
const Errors = require("etna/lib/supervisor/errors").Errors;
const Transaction = require("etna/lib/supervisor/transaction").Transaction;
const snapshots = require("etna/lib/supervisor/snapshots");
const Snapshot = require("etna/lib/supervisor/snapshot").Snapshot;
...
class BaseInstance extends Helper {
	...
	ensureSlot(name) {
		...
		if (this.properties[name]) {
			if (this.properties[name].newSlot['fiberized-0']) console.error("FIBERIZED", name, this.properties[name].newSlot['fiberized-0']);
					...
					return {
						get(_) {
							...
							return {
								collection: true,
								index(_, i) {
									if (i >= 1 && i <= self.meta.data.PROPERTIES.length) return self.meta.data.PROPERTIES[i - 1].FLDCLA;
	...
	AGETATTRIBUTE(_, property, attribut) {
		...
		if (attributes.isAttributeBoolean(attribut)) return this.getPropertyAttribute(_, property, attribut) === true ? "true" : "false";
		const value = variables.x3Val(this.getPropertyAttribute(_, property, attribut));
	...
	ASETATTRIBUTE(_, property, attribut, value) {
		...
		if (attributes.isAttributeBoolean(attribut)) return variables.x3Val(this.setPropertyAttribute(_, property, attribut, value === "true"));
		if (attributes.isAttributeNumber(attribut)) return variables.x3Val(this.setPropertyAttribute(_, property, attribut, value.x3ToInteger()));
	...
	propertyEvent(action) {
		...
		return function (_, prop, oldValue, newValue) {
			...
			const args = self.getActionArgs(_, "ARULE", action, null, prop);
	...
	propertyInitValue(_, prop, oldValue) {
		const status = this.propertyEvent("INIT")(_, prop, oldValue);
	...
	propertyControlValue(_, slot, oldValue, newValue) {
		return this.propertyEvent("CONTROL")(_, slot, oldValue, newValue);
	}

	propertyPropagateValueEvent(_, slot, oldValue, newValue) {
	...
	propertyPropagateValue(_, slot, oldValue, newValue) {
	...
	deleteDiagnoses(_, etype) {
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				this.diagnoses = [];
				Object.keys(self.slots).filter(pro => self.slots[pro].collection === undefined).forEach_(_, (_, property) => {
					const diags = self.slots[property].deleteDiagnoses(_);
				});
				break;
			case constants.CST_ALL:
			default:
				Object.keys(self.slots).forEach_(_, (_, property) => {
					const diags = self.slots[property].deleteDiagnoses(_);
				});
		}
		this.diagnoses = undefined;
	...
	getDiagnoses(_, sort) {
		...
		const local = (this.diagnoses || []).map_(_, (_, d) => {
		...
		return Object.keys(this.slots).reduce_(_, (_, r, property) => {
			const diags = self.slots[property].getDiagnoses(_);

			if (diags) r = r.concat(diags);
			return r;
		}, local).sort((d0, d1) => d0[sortMethod](d1));
	}
	$AREAD(_) {
 ...
 */
	...
	getSortedDiagnoses(_, etype, sort) {
		...
		switch (etype) {
			case constants.CST_ACURRENT:
				return Object.keys(self.slots).filter(pro => self.slots[pro].collection === undefined).reduce_(_, (_, r, property) => {
					const diags = self.slots[property].getDiagnoses(_, sort);

					if (diags) r = r.concat(diags);
					return r;
				}, self.diagnoses || []).sort((d0, d1) => d0[sort](d1));
			case constants.CST_ALL:
			default:
	...
	ADELLINE(_, collection, position) {
		const deletedAt = this.slot(_, collection).del(_, position);
	...
	ASETERROR(_, path, message, severity) {
		...
		this.errors = undefined;
		const owner = path && this.properties[path] ? this.slot(_, path) : this;
	...
	AGETERRORSFROM(_, iorigin, env) {
		const errc = iorigin.getSortedDiagnoses(_).map_(_, (_, e) => e.clone(_, env));
	...
	ASEARCHINFOS(_, local, property, status, selstatus) {
		const errt = this.getSortedDiagnoses(_, local).filter_(_, (_, e) => {
			var ftr = true;
	...
	_activeLoading(_) {
		...
		this.class.relations && Object.keys(this.class.relations).forEach_(_, (_, relation) => {
			const slot = self.slot(_, relation);
