...
class SearchResourceFactory extends Helper {
	...
	init(_) {
		...
		this.properties = Object.keys(this.class.properties).filter(property => self.class.properties[property].isSearchable && !/.*_REF$/.test(property) || self.class.properties[property].collection).reduce_(_, (_, r, property) => {
	...
	searchItemProto(_, facet) {
		...
		Object.keys(self.properties).forEach_(_, (_, name) => {
