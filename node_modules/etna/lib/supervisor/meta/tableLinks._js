...
class TableLinks extends Helper {
	...
	init(_) {
		...
		this.links = this.supervisor.mongoStore.collection('ATABLE', _).find({
			...
			CODFIC: 1
		}).toArray(_).filter(table => table.CODFIC !== self.table).reduce_(_, (_, r, table) => {
			//tracerJs.debug && tracerJs.debug("table:"+table.CODFIC);
			return table.COLUMNS.filter(column => column.LIEN && column.LIEN == self.table && column.ANNUL !== 4).reduce_(_, (_, r, column) => {
				try {
	...
	execute(_, instance, operation) {
		...
		if (this.links && this.links[operation.type]) {
			...
			Object.keys(self.links[operation.type]).forEach_(_, (_, groupKey) => {
				const group = self.links[operation.type][groupKey];
