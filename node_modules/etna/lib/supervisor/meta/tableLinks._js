...
class TableLinks extends Helper {
	...
	init(_) {
		...
		this.links = this.supervisor.mongoStore.collection('ATABLE', _).find({
			...
			CODFIC: 1
		}).toArray(_).filter(table => table.CODFIC !== self.table).reduce_(_, function (_, r, table) {
			//tracerJs.debug && tracerJs.debug("table:"+table.CODFIC);
			return table.COLUMNS.filter(column => column.LIEN && column.LIEN == self.table && column.ANNUL !== 4).reduce_(_, function (_, r, column) {
				try {
			...
			Object.keys(self.links[operation.type]).forEach_(_, function (_, groupKey) {
				...
				if (group.links) {
					...
					var sql = keys.reduce_(_, function (_, r, $key) {
						...
						group.links.filter_(_, function (_, link) {
