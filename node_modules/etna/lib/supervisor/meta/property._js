...
var FACETS = require("etna/lib/supervisor/meta/representation").FACETS;
var type = require("etna/lib/supervisor/meta/type");

var Slot = require("etna/lib/supervisor/slot").Slot;
var SlotText = require("etna/lib/supervisor/slotText").SlotText;
var SlotClob = require("etna/lib/supervisor/slotClob").SlotClob;
var SlotBlob = require("etna/lib/supervisor/slotBlob").SlotBlob;
var SlotAtextra = require("etna/lib/supervisor/slotAtextra").SlotAtextra;
var SlotReference = require("etna/lib/supervisor/slotReference").SlotReference;
...
/*
 ...
 - slotConstructor, collection are optionals
*/
class Property extends Helper {
	constructor(superv, clas, data, slotConstructor, collection) {
		super();
	...
	init(_) {
		...
		if(!this.slotConstructor) {
			if(this.isAtextra) {
				this.slotConstructor = SlotAtextra;
			} else if(this.isText) {
				this.slotConstructor = SlotText;
			} else if(this.type && this.type.isClob) {
				this.slotConstructor = SlotClob;
			} else if(this.type && this.type.isBlob) {
				this.slotConstructor = SlotBlob;
			} else {
				this.slotConstructor = Slot;
			}
		}
		return this;
	}
	...
	get params(){
		...
		return this.data.PARAM_FLDS || this.data.PARAMS;
	}
	get keyParams(){
		var self = this;
	...
	typeRulesMaps(_) {
		return this._typeRulesMaps = this._typeRulesMaps || this.params
			.filter( p => p.TYPPAR === 1 && p.TYPKEY === 1)
			.reduce_(_, (_,r,p) => {
				r[p.CODPAR]  = {
	...
	getAtextra(_, instance, lan) {
		...
		var saveLan = glob.context.x3session.actx.$LAN(_);
		glob.context.x3session.actx.$LAN(_, lan);
		// Read the text:
		var text = "";
		var atextra = this.setAtextra(_, instance);
		var status = atextra.read(_, atextra.getKeyTab(_), {
			crud: false,
			raw: true
		});
	...
	clone() {
		var p = new Property(this.supervisor, this.class, this.data, this.slotConstructor, this.collection);
		...
		this.loadReferenceData = this.loadReferenceData || function(_, id, instance) {
			...
			function addTextTransform(_, self, superv, attribute, reference, column, refReader) {
				...
				if (reference.properties[column].isAtextra) {
					var index = reference.tableIndex.sqlNames().map(function(key) {
						return key.substring(0, key.length - 2);
					});
					var self = self;
					var property = util.find(reference.data.PROPERTIES, 'FLDCLA',column);
					var atextra = property.PARAM_FLDS.filter(function(paramData) {
						return paramData.TYPPAR === 1 && paramData.TYPKEY === 2;
					}).reduce_(_, function(_, r, paramData) {
							...
							if (rs && rs.length) {
								...
								wheres.push("((" +
									rs.reduce_(_, function(_, r, record) {
										var id1 = ident(atextra.IDENT1(_, record.record));
										var id2 = ident(atextra.IDENT2(_, record.record));
										params.push(id1);
										params.push(id2);
