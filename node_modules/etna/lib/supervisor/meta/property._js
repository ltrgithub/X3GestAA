...
class Property extends Helper {
	...
	prototypeNode(_) {
		...
		if (this.targetResourceFactory && this.collection) {
			...
			if (this.links && this.links.ALOB) {
				var acceptedTypes = this.type.atypepro.lisflg !== 2 ? [this.type.atypepro.protyp] : this.typeParams.reduce_(_, function (_, r, paramData) {
					const typpro = self.supervisor.load(_, 'Typepro', paramData.VALEUR);
				...
				if (isFactoryEditFacet) node.$url = "{$instanceUrl}/" + this.name;
					...
					if (this.links.ALOB.params) urlKey = urlKey.concat(this.links.ALOB.params.map(function (paramData) {
						return paramData.alias ? alias(paramData.alias) : paramData.value;
					}, this));
	...
	calculateRef(_, item) {
		...
		keyParams = this.keyParams.reduce(function (r, paramData) {
			const propData = self.findExportedProperty(path + paramData.VALEUR);
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_, simpleNode) {
		...
		const factoryFacet = this.resourceFactory.facet;
		const isFactoryEditFacet = factoryFacet === '$edit';
		const node = ["$title", "$shortTitle", "$capabilities", "$isReadOnly", "$isHidden", "$isMandatory", "$isExcluded"].reduce(function (r, a) {
	...
	makeGetReferenceKey(_) {
		...
		const self = this;
		const refKeys = self.keyParams.reduce_(_, function (_, r, paramData) {
			r[paramData.CODPAR] = self.addFieldTransform(_, paramData.VALEUR);
			return r;
		}, {});
	...
	loadAtextra(_, instance) {
		...
		const gIds = ids.reduce(function (r, id) {
		...
		wheres.push("((" + Object.keys(gIds).reduce(function (r, ident1) {
			params.push(ident1);
		...
		const getTextId = function (record) {
		...
		const texts = this.supervisor.sqlReader(_, sql, params).toArray(_).reduce(function (r, record) {
					...
					return function (superv, atextra, attribute) {
						return function (_, reader, writer) {
							...
							if (rs && rs.length) {
								...
								wheres.push("((" + rs.reduce_(_, function (_, r, record) {
			...
			try {
				...
				if (refReader.columns) {
					...
					if (names.length === 1) {
						...
						wheres.push("(" + idsKeys.map(function (id) {
							var n = 0;
				...
				if (titleTransform || descTransform) {
					if (!reader) {
						...
						reader = function (_, reference, ids) {
							const index = reference.tableIndex.sqlNames().map(function (key) {
								return key.substring(0, key.length - 2);
							});
