...
class Resource extends BaseInstance {
	...
	dataNode(_, full) {
		...
		Object.keys(this.properties).filter(name => !self.resourceFactory.properties[name].isExtraRef).forEach_(_, (_, name) => {
			...
			if (prop.collection) {
				if (prop.isInstance) {
					...
					var rChild = lines.filter(line => {
						...
						const resLine = self.supervisor.new(_, 'Resource', prop.targetResourceFactory, child).dataNode(_, prop.isArray ? true : full);
