...
class Resource extends BaseInstance {
	...
	dataNode(_, full) {
		...
		var uuid = ((this.instance) ? this.instance.$uuid : this.$uuid);
				...
				if (prop.isInstance) {
					...
					var rChild = lines.filter(function(line) {
						...
						.map_(_, function(_, child) {
							...
							var resLine = self.supervisor.new(_, 'Resource', prop.targetResourceFactory, child).dataNode(_, prop.isArray ? true : full);
	...
	applyDelta(_, delta) {
		...
		Object.keys(delta).forEach_(_, function(_, tag) {
			//dbg:console.log("applyDelta properties:" + Object.keys(self.properties));
			...
			if (!prop) {
				if (tag[0] !== '$') {
					...
					for (var i = 0, len = keys.length; i < len; i++) {
						if (self.properties[keys[i]].isInstance) {
							name = keys[i];
							//dbg:console.error("isInstance "+name+" isInstance");
							prop = self.properties[name];
							var child = prop.instance(this).get(_, name);
							var rChild = self.supervisor.new(_, 'Resource', prop.targetResourceFactory, child);
			...
			if (prop.collection) {
				...
				if (prop.isArray) {
					...
					v.forEach_(_, function(_, deltaLine) {
						...
						var i = collection.add(_, constants.CST_ALASTPOS);
						var line = collection.index(_, i);
						var propCol = prop.collection.class.data.PROPERTIES[0];
	...
	checkAccess(_,type) {
		glob.context.x3session.actx.checkAccess(_,this.meta.data.AFCRIGHT,type);
	}

	insert(_) {
		this.checkAccess(_,"C");
		return this.instance.$AINSERT(_);
	}
	update(_) {
		this.checkAccess(_,"U");
		return this.instance.$AUPDATE(_);
	}
	read(_, key) {
		this.checkAccess(_,"R");
