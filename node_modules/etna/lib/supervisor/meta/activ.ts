...
import { ActivMeta } from '../metadata/activ';
import { Supervisor } from '../supervisor';

export class Activ extends util.Helper {
	supervisor: Supervisor;
	data: ActivMeta;
	constructor(superv: Supervisor, data: ActivMeta) {
		super();
...
export function getActiv(_: _, supervisor: Supervisor, cod: string) {
	if (!cod || cod === '') return 1;
	const activ = supervisor.load<Activ>(_, 'Activ', cod);
