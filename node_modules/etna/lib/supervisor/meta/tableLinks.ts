...
export interface LinkOperation {
	type: number;
	execute(_: _,  instance: IInstance, superv: Supervisor, sql: LinkSql): number;
}

const linkOperations = {
	control: {
		type: 1,
		execute(_: _,  instance: IInstance, superv: Supervisor, sql: LinkSql) {
...
export class TableLinks extends Helper {
	...
	init(_: _) {
		...
		this.links = this.supervisor.mongoStore.collection('ATABLE').find({
			...
			CODFIC: 1
		}).toArray().then(_, _).filter((table: TableData) => table.CODFIC !== this.table)
		.reduce_(_, (_: _, r: { [name: string]: SqlLink }, table: TableData) => {
			//tracerJs.debug && tracerJs.debug("table:"+table.CODFIC);
			return table.COLUMNS.filter((column: ColumnData) => column.LIEN && column.LIEN == this.table && column.ANNUL !== 4).reduce_(_, (_, r, column) => {
					var type = r[column.ANNUL] = r[column.ANNUL] || util.any({});
	...
	execute(_: _,  instance:IInstance, operation: LinkOperation) {
		var status = constants.CST_AOK;
	...
	control(_: _,  instance: IInstance) {
