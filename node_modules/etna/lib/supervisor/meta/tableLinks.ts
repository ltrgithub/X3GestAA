...
export class TableLinks extends Helper {
	...
	init(_: _) {
		...
		.reduce_(_, (_: _, r: { [name: string]: SqlLink }, table: TableMeta) => {
			//tracerJs.debug && tracerJs.debug("table:"+table.CODFIC);
			return table.COLUMNS.filter((column: ColumnMeta) => column.LIEN && column.LIEN == this.table && column.ANNUL !== 4).reduce_(_, (_, r, column) => {
