...
export class WorkingCopy extends Helper {
	...
	addAttributes(node: types.ResourceNode) {
	...
	update(_: _,  context: HttpContext, delta: types.ResourceNode, path: string) {
		...
		try {
			...
			var diagnoses: types.DiagnosisProto[];
			...
			if (delta && delta.$actions && delta.$actions.$save && delta.$actions.$save.$isRequested === true) {
				...
				var status = util.any(this.current)[this.save](_);
	...
	initLocalizeData(context: HttpContext): types.DataNode {
		return {
	...
	updateLocalization(_: _,  context: HttpContext, delta: types.DataNode, path: string) {
		if (typeof delta === 'string') delta = JSON.parse(util.any(delta)); // ????
		const data = this.initLocalizeData(context);
