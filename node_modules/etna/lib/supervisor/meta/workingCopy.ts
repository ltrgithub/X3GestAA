...
export class WorkingCopy extends Helper {
	...
	update(_: _,  context, delta, path) {
		...
		if (path !== undefined) {
			this.current.walkToProperty(_, path, (_, instance, property) => {
				const slot = instance.set(_, property, delta);
	...
	read(_: _,  context, path) {
		tracerJs.debug && tracerJs.debug("reading working copy:", path);
		this.current.walkToProperty(_, path, (_, instance, property) => {
			const slot = instance.slot(_, property);
	...
	readLocalization(_: _,  context, path) {
	...
	updateLocalization(_: _,  context, delta, path) {
