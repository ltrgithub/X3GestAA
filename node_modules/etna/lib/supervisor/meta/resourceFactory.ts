...
export class ResourceFactory extends Helper {
	...
	links: { [name: string]: Link }
	page: Page;
	constructor(superv: Supervisor, repr: Representation, facet: string, path: string, clas: Class) {
		super();
	...
	deleteResource(_: _,  httpContext, $key, qs) {
		const key = $key.split('~');
		const resource = this.supervisor.new(_, 'Resource', this, null);
