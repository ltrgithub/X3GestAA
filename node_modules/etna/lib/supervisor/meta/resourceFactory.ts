...
import { NodeProto, X3FormatProto } from '../protocol/prototype';
import { Feed } from '../protocol/data'; 
import { PageProto } from '../protocol/page'; 
import * as util from '../util';
import * as Jmql from '../../engine/compilers/jmqlgen';
const filterHelpers = require("syracuse-sdata-parser/lib/json-converter");
import * as constants from '../constants';
import { Cache } from '../cache';
import { Class, Property, Link, Representation, Select } from './types';
import { Diagnosis, Instance, Resource } from '../dyn/types';
import * as activ from '../meta/activ'; // GH 6167
import * as http from '../http';
import { Supervisor } from '../supervisor';

import { Reader, Writer, devices } from 'ez-streams';
...
export class ResourceFactory extends util.Helper {
	...
	fillTypeNode(_: _, node: NodeProto) {
	...
	itemProto(_: _, proto: NodeProto, facet?: string) {
	...
	pageProto(_: _) {
		...
		if (this.facet === "$edit") {
			var sysValues = util.currentContext().x3frame.context.sys.values;
			...
			pageProto.$x3FormatExt = adxtul.reduce((r: { [name: string]: X3FormatProto; }, tul: string, i: number) => {
				r[tul] = {
					stdType: adxtuc[i].split(''),
					chars: adxtut[i].split('')
				};
				return r;
			}, {} as Dict<X3FormatProto>);
		}
	...
	loadResource(_: _,  key: string, qs: any) {
		// TODO qs ?
		const k = key.split('~');
		const resource = this.supervisor.new<Resource>(_, 'Resource', this, null);
	...
	createResource(_: _,  qs: any) {
		...
		const resource = this.supervisor.new<Resource>(_, 'Resource', this, instance).bindInstance(instance);
	...
	loadResources(_: _,  select: Select, max: number): Reader<Resource> {
		const fromInstanceToResource = (_: _, reader: Reader<Instance>, writer: Writer<Resource>) => {
			var instance: Instance;
			var astatus: number;
			const cache = this.cache ? this.cache.clone() : undefined;

			while (instance = reader.read(_)) {
				var resource = this.supervisor.new<Resource>(_, 'Resource', this, instance).bindInstance(instance);
