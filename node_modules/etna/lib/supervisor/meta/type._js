...
class TypeLob extends Type {
	...
	insertLob(_,slot, value) {
		...
		if(this.rules && this.rules.INSERT_MEDIA) {
			...
			var sqlDriver = this.supervisor.sqlDriver;
			var param = sqlDriver.param;
			var keys = slot.getKeys(_);
			//dbg:console.log("insertLob name:"+slot.name+" keys:",keys);
			var params = [keys.CODBLB, keys.IDENT1, keys.IDENT2, keys.IDENT3, value];

			var i = 0;
			var sql = "insert into " + slot.table + " (" + ['CODBLB_0','IDENT1_0','IDENT2_0','IDENT3_0',slot.field].join(',') + //
			") values (" + params.map((p)=> param(i++)).join(',') + ")";
			//TODO return this.supervisor.executeSql(_, sql, params);
		}
	}
	...
	updateLob(_,slot,value) {
		...
		if(this.rules && this.rules.UPDATE_MEDIA) {
			...
			var sql = "update " + slot.table + " T set " + slot.field + " = " + param(i++) + " where " + updateWheres;
			//TODO return this.supervisor.executeSql(_, sql, params);
		}
	}
	...
	deleteLob(_,slot, value) {
		...
		if(this.rules && this.rules.DELETE_MEDIA) {
			...
			var i = 0;
			var sql = "delete From " + slot.table + " Where" +
				" CODBLB_0=" + param(i++) + " and IDENT1_0=" + param(i++) + " and IDENT2_0=" + param(i++) + " and IDENT3_0=" + param(i++);
			return this.supervisor.executeSql(_, sql, params);
		}
	}
}
