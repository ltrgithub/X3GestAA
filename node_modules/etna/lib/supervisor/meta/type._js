...
class Type {
	...
	init(_) {
		var makeTypeRule = function(type,rule) {
			return function(_,instance, value) {
				...
				return  type.applyRule(_, rule, args);
			}
		}
		...
		if(this.data.RULES) {
			...
			// if(this.rules.INIT) 		this.INIT 		 = makeTypeRule(this,this.rules.INIT);
			// if(this.rules.CONTROL)		this.CONTROL	 = makeTypeRule(this,this.rules.CONTROL);
			// if(this.rules.PROPAGATE)	this.PROPAGATE	 = makeTypeRule(this,this.rules.PROPAGATE);
			// if(this.rules.GET)			this.GET 		 = makeTypeRule(this,this.rules.GET);
			if(this.rules.READ_MEDIA)   this.readMedia   = makeMediaRule(this.rules.READ_MEDIA); 
	...
	getX3Format(propertyLength) { return this.$x3Format = undefined;}
...
class TypeB extends TypeLob {
	...
	toJson(_, slot , val) { return undefined;}
}
...
class TypeT extends TypeLob {
	...
	toJson(_, slot , val) { return this.contentType(_,slot, val);}
}
