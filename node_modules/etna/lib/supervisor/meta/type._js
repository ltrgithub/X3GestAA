...
class Type {
	...
	init(_) {
		const makeTypeRule = function (type, name) {
		...
		const makeMediaRule = function (rule) {
		...
		const makeMediaLazyRule = function (rule) {
		...
		if (this.data.RULES) {
			this.rules = this.data.RULES.filter_(_, (_, rule) => !/^WMTYP/.test(rule.TRTRUL) && rule.ENARUL === 2 && activ.getActiv(_, this.supervisor, rule.ACTRUL) > 0).reduce_(_, (_, r, rule) => {
				r[mapTypeRule[rule.TYPRUL]] = {
					...
					prog: rule.PRGRUL,
					params: rule.PARAMETERS ? rule.PARAMETERS.reduce_(_, function (_, r, param) {
						r[param.PARCOD] = {
...
module.exports = {
	...
	constructor: Type,
	fillTypeNode(_, superv, data, node) {
