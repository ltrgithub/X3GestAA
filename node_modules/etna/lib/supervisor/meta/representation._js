...
const Sdata = require("etna/lib/engine/sdatagen");
const tuuid = require('etna/lib/engine/runtime/tuuid');
const DBTableVAR = require('etna/lib/engine/runtime//dbtablevar').DBTableVAR;
const activ = require("etna/lib/supervisor/meta/activ");
const constants = require("etna/lib/supervisor/constants");
const extAttributesObject = require("etna/lib/supervisor/attributes").extAttributesObject;
const enumStdLinks = require("etna/lib/supervisor/meta/link").enumStdLinks;
const BaseClass = require("etna/lib/supervisor/meta/baseClass").BaseClass;
...
class Representation extends BaseClass {
	...
	init(_) {
		...
		this.properties = this.data.PROPERTIES ? this.data.PROPERTIES.filter(prop => !prop.FLDGRP).reduce_(_, function (_, r, prop) {
			const name = prop.CODFLD;

			r[name] = self.supervisor.new(_, 'Property', self, prop);
			return r;
		}, {}, this) : {};

		this.relations = this.data.COLLECTIONS && this.data.COLLECTIONS.reduce_(_, function (_, r, col) {
			if (col.CODCOL.indexOf('.') < 0) {
				r = r || {};
				var name = col.CODCOL;

				var coln = r[name] = self.supervisor.new(_, 'Collection', this, col);
				coln.class = self.supervisor.new(_, 'Class', {
					CODCLA: self.data.CODREP + "." + name,
					TYPCLA: 1, //5,
					PROPERTIES: self.data.PROPERTIES && self.data.PROPERTIES.reduce((r, propData) => {
				...
				var prop = self.properties[name] = self.supervisor.new(_, 'Property', self, {
					CODFLD: name
		...
		this.exportedProperties = this.data.EXPORTED_PROPERTIES ? this.data.EXPORTED_PROPERTIES.sort((ep1, ep2) => ep1.ORDPRO - ep2.ORDPRO).map_(_, function (_, data) {
		...
		this.data.LINKS = this.data.LINKS.filter(link => link.ENALNK === 2).sort(trierLink);
		return this;
	}
	prepareLinks(_) {
		...
		const linksRemRef = self.data.LINKS.filter(link => link.REMSTDLNK !== undefined && link.REMSTDLNK !== "" && link.FLGSTDLNK === 2).reduce((r, lk1) => {
			const tablnk = self.data.LINKS.filter(lk2 => lk2.CODLNK === lk1.REMSTDLNK && lk2.ANCLNK === lk1.ANCLNK && lk2.AFFLNK === lk1.AFFLNK);
		...
		this.links = self.data.LINKS.filter(link => {
			...
			if (link.ATTLNK > 1 && link.FLGSTDLNK === 2) {
				var tablnk = self.data.LINKS.filter(lk2 => lk2.ATTLNK === link.ATTLNK && lk2.FLGSTDLNK === 1 && lk2.AFFLNK === link.AFFLNK && lk2.ANCLNK === link.ANCLNK && lk2.CODLNK !== link.CODLNK && lk2.CODLNK !== lk1.CODLNK);
				if (tablnk && tablnk[0]) {
					link.ATTLNK = 1;
					link.FLGSTDLNK = 1;
				}
	...
	isFacetEnabled(facet) {
		return this.data.ENAFAC[FACETS[facet].id] === 2;
	}
	isBehaviourEnabled(no7968) {
	...
	resourceFactory(_, facet) {
		...
		const bindProperty = function (property, factory) {
			var factProperty;
			if (property) {
		...
		this.exportedProperties.filter(expprop => expprop.data[facetFlagPRO] === 2).forEach_(_, function (_, exppro) {
			...
			if (this.relations && this.relations[path[0]]) {
				if (!(prop = factory.properties[path[0]])) {
					...
					prop.tag = path[0];
					prop.targetResourceFactory = this.supervisor.new(_, 'ResourceFactory', this, facet, path[0], this.relations[path[0]].class);
			...
			if (!prop.hasLookup) {
				...
				// TODO META (links)
				prop.hasLookup = self.links.filter(linkData => linkData.ANCLNK === prop.exportedProperty.data.CODPRO && linkData.AFFLNK === 1 && linkData.ATTLNK === 3 && !(prop.isKey && linkData.REPLNK === self.name)).some(linkData => {
		...
		if (facet === "$edit") {
			var linksSave = {};
			var linksAbort = {};
			this.links.filter(link => link.FLGSTDLNK === 2 && link.TYPLNK === 1 && link.AFFLNK > 3).forEach_(_, function (_, link) {
				var l;
		...
		this.links.filter(link => link[facetFlagLNK] === 2).forEach_(_, function (_, link) {
	...
	filtersProto(_, resourceFactory) {
		const tabcor = resourceFactory.filtersTabCor(_);
		const self = this;
		const myopt = {};

		this.data.FILTERS && this.data.FILTERS.forEach_(_, function (_, opt) {
			var cond = true;
			var where;
			if (opt.OPTFLGCLA === 2) {
				var id = self.class.data.FILTERS.indexOf(opt.OPTCOD);
				if (!id) cond = false;
				else {
					myopt[opt.OPTCOD] = {};
					myopt[opt.OPTCOD].$title = this.supervisor.loadText(_, self.class.data.FILTERS.OPTLIB);
					where = Sdata.x3ToSdata(tabcor, util.decodeValueMongoString(self.class.data.FILTERS.OPTCND));
			...
			if (cond === true) {
				...
				if (opt.OPTACT !== undefined && activ.getActiv(_, self.supervisor, opt.OPTACT) <= 0) {
					// GH 6167
	...
	articleLayout(_, facet) {
		return {
			$layout: {
				$items: this.data.SECTIONS.sort((s1, s2) => s1.NIVSEC - s2.NIVSEC).map_(_, function (_, section) {
					return {
						$category: "section",
						$title: this.supervisor.loadText(_, section.INTSEC),
						$layout: {
							$items: section.BLOCKS ? section.BLOCKS.sort((b1, b2) => b1.NIVBLC - b2.NIVBLC).map_(_, function (_, block) {
								//
		...
		function fillMenuMen(_, facet, self, tabMenus, curMenu, links1, links2, items) {
			...
			if (tabMenus) {
				tabMenus.filter_(_, function (_, menu) {
					...
					if (okMenu === 1 && menu.ACTMENU && activ.getActiv(_, self.supervisor, menu.ACTMENU) <= 0) okMenu = 0;
					return okMenu === 1;
					...
					const hasBind1 = fillMenu(facet, self, links1, menu.CODMENU, items);
					const hasBind2 = fillMenu(facet, self, links2, menu.CODMENU, items);
	...
	service(_, keys, $service, facet, qs) {
		const reply = {};
		...
		reply.$diagnoses = ins.getDiagnoses(_).map(diagnose => diagnose && diagnose.dataNode() || null);
