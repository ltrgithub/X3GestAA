...
class Representation extends BaseClass {
		...
		this.relations = this.data.COLLECTIONS && this.data.COLLECTIONS.reduce_(_, (_, r, col) => {
			if (col.CODCOL.indexOf('.') < 0) {
				...
				coln.class = this.supervisor.new(_, 'Class', {
					...
					TYPCLA: 1, //5,
					PROPERTIES: this.data.PROPERTIES && this.data.PROPERTIES.reduce((r, propData) => {
	...
	resourceFactory(_, facet) {
		...
		if (facet === "$edit") {
			this.resourceFactories[facet].$attributes = (node, property, attributes) => {
				// Manage attributes :
			...
			this.resourceFactories[facet].$diagnoses = (node, property, diagnoses) => {
		...
		const bindProperty = (property, factory) => {
			var factProperty;
			if (property) {
		...
		this.data.FILTERS && this.data.FILTERS.forEach_(_, (_, opt) => {
			...
			if (opt.OPTFLGCLA === 2) {
				...
				if (!id) cond = false;
					...
					myopt[opt.OPTCOD] = {};
					myopt[opt.OPTCOD].$title = this.supervisor.loadText(_, this.class.data.FILTERS.OPTLIB);
					where = Sdata.x3ToSdata(tabcor, util.decodeValueMongoString(this.class.data.FILTERS.OPTCND));
					...
					return {
						...
						$layout: {
							$items: section.BLOCKS ? section.BLOCKS.sort((b1, b2) => b1.NIVBLC - b2.NIVBLC).map_(_, (_, block) => {
								...
								//
								const findCol = (targetResourceFactory, facet) => {
									var condCol = {
										cond: false,
										ordre: 0
									};
								...
								//
								//
								const getColTarget = (targetResourceFactory, facet, r, condTarget) => {
								...
								const getCol = (targetResourceFactory, facet, r) => {
									Object.keys(targetResourceFactory.properties).forEach(prop => {
										var condBind = {
											cond: false,
											ordre: 0
	...
	article(_, facet, f) {
		...
		//Menu
		const fillMenu = (facet, tabLinks, curMenu, items) => {
			var hasElt = false;
			if (tabLinks) {
				Object.keys(tabLinks).filter(name => {
					...
					if (this.data.DEFLNKFAC[FACETS[facet].id] !== 2) {
						if (this.data.AFFLNKFAC[FACETS[facet].id] === tabLinks[name].data.oAFFLNK && this.data.LNKMENFAC[FACETS[facet].id] === tabLinks[name].data.oCODLNK) {
							new_item.$kind = "main";
						}
		...
		const fillMenuMen = (_, facet, tabMenus, curMenu, links1, links2, items) => {
			var hasElt = false;
