...
class Representation extends BaseClass {
	...
	init(_) {
		...
		this.properties = this.data.PROPERTIES ? this.data.PROPERTIES.filter(function (prop) {
			return !prop.FLDGRP;
		...
		this.relations = this.data.COLLECTIONS && this.data.COLLECTIONS.reduce_(_, function (_, r, col) {
			if (col.CODCOL.indexOf('.') < 0) {
				...
				coln.class = self.supervisor.new(_, 'Class', {
					...
					TYPCLA: 1, //5,
					PROPERTIES: self.data.PROPERTIES && self.data.PROPERTIES.reduce(function (r, propData) {
		...
		this.exportedProperties = this.data.EXPORTED_PROPERTIES ? this.data.EXPORTED_PROPERTIES.sort(function (ep1, ep2) {
	...
	prepareLinks(_) {
		...
		const linksRemRef = self.data.LINKS.filter(function (link) {
			return link.REMSTDLNK !== undefined && link.REMSTDLNK !== "" && link.FLGSTDLNK === 2;
		}).reduce(function (r, lk1) {
			const tablnk = self.data.LINKS.filter(function (lk2) {
	...
	resourceFactory(_, facet) {
			...
			this.resourceFactories[facet].$diagnoses = function (node, property, diagnoses) {
				if (diagnoses === null || diagnoses.length) {
					...
					if (property) {
						...
						target.$diagnoses = diagnoses === null ? null : diagnoses.map(function (diagnose) {
							return diagnose && diagnose.dataNode() || null;
						});
					} else {
						var diags = diagnoses.filter(function (diagnose) {
		...
		this.exportedProperties.filter(function (expprop) {
			...
			if (!prop.hasLookup) {
				...
				// TODO META (links)
				prop.hasLookup = self.links.filter(function (linkData) {
					return linkData.ANCLNK === prop.exportedProperty.data.CODPRO && linkData.AFFLNK === 1 && linkData.ATTLNK === 3 && !(prop.isKey && linkData.REPLNK === self.name);
				}).some(function (linkData) {
					linkData.PARAMS && linkData.PARAMS.forEach(function (paramData) {
			...
			$layout: {
				$items: this.data.SECTIONS.sort(function (s1, s2) {
					...
					return {
						...
						$layout: {
							$items: section.BLOCKS ? section.BLOCKS.sort(function (b1, b2) {
	...
	service(_, keys, $service, facet, qs) {
		...
		const f = this.resourceFactory(_, facet);
		const links = this.links.filter(function (link) {
		...
		reply.$diagnoses = ins.getDiagnoses(_).map(function (diagnose) {
			return diagnose && diagnose.dataNode() || null;
		});
