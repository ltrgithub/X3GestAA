...
export class Table extends Helper {
	...
	executeSql(_: _, instance, sql, values, lobVars) {
		let output;
		if(lobVars.length) {
			const param = this.supervisor.sqlDriver.param;
	...
	loadRecords(_: _,  index, vals, max, where, raw) {
	...
	getRecordDescription(_: _,  record) {
		// Set a default value *
		var description = "";
		if (this.data.INTIT) {
			...
			if (this.data.INTITTYPE) {
				if (this.data.INTITTYPE === "ATX") {
					...
					rs = this.supervisor.sqlReader(_, sql, params).toArray(_);
