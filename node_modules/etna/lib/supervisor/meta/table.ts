...
import { Column, ColumnDesc, Index, TableLinks, Type } from './types';
...
export class Table extends Helper {
	...
	constructor(superv: Supervisor, data: TableMeta) {
		super();
		if (data.COLUMNS) data.COLUMNS.sort((a, b) => a.NUMLIG - b.NUMLIG);
	...
	reader<T extends Record>(_: _,  index: Index, vals: any[], where?: TableWhere, raw?: boolean, options?: SqlQueryOptions): Reader<T> {
	...
	executeSql(_: _, instance: Instance, sql: string, values: any[], lobVars: LobVar[]) {
		...
		if (result.updateCount) {
			for(let i=0; i < lobVars.length; i++) {
				...
				const reader: Reader<any> = lobVars[i].type.sqlType() === "CLOB" 
				? devices.string.reader(lob.value) 
				: devices.buffer.reader(lob.value);
	...
	loadRecords<T>(_: _,  index: Index, vals: any[], max?: number, where?: TableWhere, raw?: boolean) {
		if (max == null || max < 0) max = Infinity;
		return this.reader<T>(_, index, vals, where, raw).limit(max).toArray(_);
	}
	...
	writer(_: _,  cnx: Connection) {
		...
		const wr = (writer.pre as any).map((_: _, data: any) => {
			wr.count++;
