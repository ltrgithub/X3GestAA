...
// avoid circular dependency in constructors
type IInstance = any; 
...
export class Table extends Helper {
	...
	indexes: Index[];
	_links: TableLinks;
	constructor(superv: Supervisor, data: TableData) {
		super();
	...
	reader(_: _,  index: Index, vals: any[], where?: TableWhere, raw?: boolean, options?: types.SqlQueryOptions) {
	...
	executeSql(_: _, instance: IInstance, sql: string, values: any[], lobVars: LobVar[]) {
		let output;
		if(lobVars.length) {
			const param = this.supervisor.sqlDriver.param;
			...
			sql += " returning "+lobVars.map(v => {
				...
				output.outputVars.push({name: name.substring(1),type: v.type.sqlType()});
		...
		if (result.updateCount) {
			for(let i=0; i < lobVars.length; i++) {
				const reader = lobVars[i].type.sqlType() === "CLOB" ? ez.devices.string.reader : ez.devices.buffer.reader;
				let lob = instance.get(_,lobVars[i].name);
				reader(lob.value).pipe(_, result.outputVars[i]);
			}
		}

		return result;
	}
	insert(_: _,  instance: IInstance) {
	...
	writer(_: _,  cnx: types.Connection) {
		this.computeColumnNames(_);
		const sql = "insert into " + this.name + " (" + this.sql.columnNames.join(',') + //
		") values (" + this.sql.insertVars + ")";
	...
	getRecordDescription(_: _,  record: types.Row) {
		// Set a default value *
		var description = "";
		if (this.data.INTIT) {
			...
			if (this.data.INTITTYPE) {
				if (this.data.INTITTYPE === "ATX") {
					...
					const rs = this.supervisor.sqlReader(_, sql, params).toArray(_);
