...
import { FACETS } from '../meta/representation';
...
export class Property extends Helper {
	...
	updateAtextra(_: _,  instance, value, lan) {
		// Save the current language:
		var saveLan;
		if (lan) {
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_: _,  simpleNode) {
		const factoryFacet = this.resourceFactory.facet;
		const isFactoryEditFacet = factoryFacet === '$edit';
	...
	addFieldTransform(_: _,  value, index, refReader) {
		const regNum$ = /=num\$\(([\w\.]*)\)/;
		const regVal$ = /=val\(([\w\.]*)\)/;
