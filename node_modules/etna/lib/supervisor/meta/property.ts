...
export type KeyMap = (_: _, instance: Instance) => any;
...
export class Property extends Helper {
	...
	_keyMaps: KeyMaps;
	_typeRulesMaps: { [name: string]: (_: _, instance: Instance) => any };
	_key: string;
 ...
 "C06P02_REF": {
	...
	makeGetReferenceKey(_: _) {
		...
		return (_: _, record: Record) => Object.keys(refKeys).map_(_, (_, k) => refKeys[k](_, record) || "").join('~');
	}
	...
	addFieldTransform(_: _,  value: any, index?: any[], refReader?: { columns: string[] }): (_: _, instance?: Instance) => any {
		const regNum$ = /=num\$\(([\w\.]*)\)/;
		const regVal$ = /=val\(([\w\.]*)\)/;
		var property: string;
	...
	loadReference(_: _,  instance: Instance) {
		this.loadReferenceData = this.loadReferenceData || function (_: _,  id: string, instance: Instance) {
			tracerJs.debug && tracerJs.debug("loadReferenceData(" + this.name + "):" + id);

			//dbg:console.error("loadReferenceData(" + this.name + "):" + id);

			const addTextTransform = (_: _, superv: Supervisor, attribute: string, reference: Instance, column: string, refReader: ez.Reader<Instance>) => {
