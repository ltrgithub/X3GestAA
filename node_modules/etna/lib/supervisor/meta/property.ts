...
import { Record, DriverReaderOptions } from '../../engine/drivers/types';
import { NodeProto, PropertyRefProto } from '../protocol/prototype';
...
import { Slot } from '../dyn/slot';
import { ExportedProperty } from './exportedProperty';
import { Collection } from './collection';
import { ColumnDesc, Column } from './column';
...
import { PropertyMeta, PropertyParamMeta } from '../metadata/class';
import { ExportedPropertyMeta } from '../metadata/representation';
import { ReferenceNode } from '../protocol/data';
...
*/
export type SlotMaker = (prop: Property) => (parent: any) => Slot;
...
export type InstanceMapper = (_: _, instance: Instance) => any;
export type RecordMapper = (_: _, record: Record) => any;

export interface KeyMaps {
	IDENT1: InstanceMapper;
	IDENT2: InstanceMapper;
	CODFIC: InstanceMapper;
	ZONE: InstanceMapper;
	LANGUE: InstanceMapper;
	[name: string]: InstanceMapper;
}

export interface RuleMap {
	type: string;
	value: InstanceMapper;
}
...
export interface RefReader {
	columns: number[];
	maps: ReferenceMapper[];
}

export type ReferenceMapper = (instance: Instance) => any;

export class Property extends util.Helper {
	...
	$x3Format: string;
	jsName: string;
	_params: PropertyParamMeta[];
	_keyMaps: KeyMaps;
	_typeRulesMaps: Dict<RuleMap>;
	_key: string;
	resourceFactory: ResourceFactory;
	exportedProperty: ExportedProperty;
	targetResourceFactory: ResourceFactory;
	links: PropertyLinks;
	proMaps: any[]; // refine later
	toControl: boolean;
	tag: string; //??
	reference: any; // ??
	loadReferenceData: (_: _,  id: string, instance: Instance) => ReferenceNode;
	belongsToKey: boolean;
	isKey: boolean;
	hasLookup: boolean;
	destLookup: string;
	repLookup: string;
	...
	constructor(superv: Supervisor, clas: BaseClass, data: PropertyMeta, slotType: string, collection: Collection) {
		super();
		...
		if (collection) collection.property = this;
	...
	init(_: _) {
		...
		if (this.data.CODTYP) {
			this.type = this.supervisor.load<Type>(_, 'Type', this.data.CODTYP);
	...
	setAtextra(_: _,  instance: Instance) {
	...
	getAtextra(_: _,  instance: Instance, lan: string) {
		// Little trick
		// Save the current language:
		const saveLan = util.currentContext().x3session.actx.$LAN(_);

		util.currentContext().x3session.actx.$LAN(_, lan);
		// Read the text:
		var text = "";
		...
		util.currentContext().x3session.actx.$LAN(_, saveLan);
		return text;
	}
	updateAtextra(_: _,  instance: Instance, value: string, lan?: string) {
		// Save the current language:
		var saveLan: string;
		if (lan) {
			saveLan = util.currentContext().x3session.actx.$LAN(_);
			util.currentContext().x3session.actx.$LAN(_, lan);
		}
		try {
			...
			if (saveLan) util.currentContext().x3session.actx.$LAN(_, saveLan);
		}
	}
	deleteAtextra(_: _,  instance: Instance) {
		// Create an instance of atextra
		const atextra = this.setAtextra(_, instance);
	...
	clone() {
		const p = new Property(this.supervisor, this.class, this.data, this.slotType, this.collection);
	...
	targetClass(_: _) {
		...
		return this.supervisor.load<Class>(_, 'Class', this.data.LNKCLA);
	}
	findExportedProperty(name: string) {
	...
	prototypeNode(_: _) {
		...
		if (this.targetResourceFactory && this.collection) {
			if (!(this.collection.data.MAXCOL === 1 && this.collection.data.MINCOL === 2)) {
				...
				if (this.collection.data.FLGAPDCOL === 2) capabilities.push("append");
				if (this.collection.data.FLGINSCOL === 2) capabilities.push("insert");
				if (this.collection.data.FLGSUPCOL === 2) capabilities.push("delete");
				if (this.collection.data.FLGTRICOL === 2) capabilities.push("reorder");
				node.$capabilities = capabilities.join(',');
			...
			if (this.collection.mappingData && [2, 4].indexOf(this.collection.mappingData.TYPLNK) >= 0 || 
				this.collection.class.data && (this.collection.class.data.TYPCLA === 5 || this.collection.ofIndexedProperties === true)) {
			...
			if (!this.isExtra && this.column !== undefined || this.class.data.TYPCLA && this.class.data.TYPCLA !== 2 || this.type.data.CODCLA === 'ATEXTRA') {
				...
				if (this.type.data.CODCLA === 'ATEXTRA') {
					...
					capabilities = isFactoryEditFacet ? ["sort"] : ["sort", "filter", "filter_upper"];
				} else if ([9, 10].indexOf(this.type.data.TYPTYP) < 0) {
					capabilities = isFactoryEditFacet ? ["sort"] : ["sort", "filter"];
				}
				if (this.type.data.CODCLA === 'ATEXTRA' && isFactoryEditFacet) {
					capabilities.push("localize");
				}
				node.$capabilities = capabilities.join(',');
			}
			...
			if (expProData.UOMPRO) {
				...
				var propData: ExportedPropertyMeta = this.findExportedProperty(this.exportedProperty.data.UOMPRO);
			...
			if (this.links && this.links.ALOB) {
				var acceptedTypes = this.type.atypepro.lisflg !== 2 ? [this.type.atypepro.protyp] : this.typeParams.reduce_(_, (_, r, paramData) => {
					const typpro = this.supervisor.load<TypePro>(_, 'Typepro', paramData.VALEUR);
	...
	calculateRef(_: _,  item: PropertyRefProto) {
		...
		const repr = this.supervisor.load<Representation>(_, 'Representation', codrep);
		...
		keyParams = this.keyParams.map(paramData => {
			const propData: ExportedPropertyMeta = this.findExportedProperty(path + paramData.VALEUR);
		...
		// TODO: following is weakly typed because we cannot import ResourceFactory - improve later
		item.$value = reprResFact.keyItems.map((keyitem: any) => alias(keyitem.alias), this).join('~');

		item.$properties = {};
		reprResFact.keyItems.forEach_(_, (_: _, keyitem: any) => {
			...
			keyParams.forEach_(_, (_, paramData) => {
				if (keyitem.prop === paramData.CODPAR) {
					...
					if (paramData.alias) {
						(item as any)[keyitem.alias] = alias(paramData.alias);
						itemKey.push(alias(keyitem.alias));
					} else {
						(item as any)[keyitem.alias] = paramData.value;
						itemKey.push(paramData.value);
					}
				}
			});
			item.$key = itemKey.join('~');
		}, this);
	}
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_: _,  simpleNode: NodeProto) {
		const factoryFacet = this.resourceFactory.facet;
		const isFactoryEditFacet = factoryFacet === '$edit';
		const node: NodeProto = ["$title", "$shortTitle", "$capabilities", "$isReadOnly", "$isHidden", "$isMandatory", "$isExcluded"]
		.reduce((r, a) => {
		...
		if (link === undefined) {
			...
			this.calculateRef(_, item as any);
		} else {
			...
			if (link.data.REPLNK !== repr.name) {
				repr = this.supervisor.load<Representation>(_, 'Representation', link.data.REPLNK);
			...
			var itemKey: string[] = [];
			item.$value = [];
			f.keyItems.forEach_(_, (_: _, keyitem: any) => {
				item.$properties[keyitem.alias] = {};
	...
	makeGetReferenceKey(_: _) {
		const name = this.name;
		const refKeys = this.keyParams.reduce_(_, (_, r, paramData) => {
			r[paramData.CODPAR] = this.addFieldTransform(_, paramData.VALEUR);
			return r;
		}, {} as Dict<InstanceMapper>);

		return (_: _, record: Instance) => Object.keys(refKeys).map_(_, (_, k) => refKeys[k](_, record) || "").join('~');
	}
	getAtextraKey(_: _,  instance: Instance) {
		const keyMaps = this.keyMaps(_);
	...
	cache: any, raw?: boolean, sqlOpts?: DriverReaderOptions): void {
	...
	addFieldTransform(_: _,  value: any, index?: any[], refReader?: RefReader): (_: _, instance?: Instance) => any {
		const regNum$ = /=num\$\(([\w\.]*)\)/;
		const regVal$ = /=val\(([\w\.]*)\)/;
		var property: string;
		...
		if (value === '""' || value === 'AVOID.ACHAR') {
			...
			property = value.substring(5);
			return (_: _) => util.currentContext().x3session.actx.get(_, property);
			...
			return (_, instance) => {
				...
				if (instance.properties && instance.properties[property]) {
					...
					value = (instance as Dict<any>)[property];
				}
	...
	loadAtextra(_: _,  instance: Instance) {
		...
		wheres.push("((" + Object.keys(gIds).map(ident1 => {
			params.push(ident1);
			...
			if (gIds[ident1].length === 1) {
				...
				select += "IDENT2_0 in (" + gIds[ident1].map((ident2: any) => {
					params.push(ident2);
		...
		const texts = this.supervisor.sqlReader<{
			TEXTE: string;
			IDENT1: string;
			IDENT2: string;
		}>(_, sql, params).toArray(_).reduce((r, record) => {
			//tracerJs.debug && tracerJs.debug("record	:" + JSON.stringify(record));
			r[record.IDENT1 + '~' + (record.IDENT2 || " ")] = record.TEXTE || "";
			return r;
		}, {} as util.Dict<string>);
	...
	loadReference(_: _,  instance: Instance) {
		this.loadReferenceData = this.loadReferenceData || ((_: _,  id: string, instance: Instance) => {
			tracerJs.debug && tracerJs.debug("loadReferenceData(" + this.name + "):" + id);

			//dbg:console.error("loadReferenceData(" + this.name + "):" + id);

			const addTextTransform = (_: _, superv: Supervisor, attribute: string, reference: Class, column: number, refReader: RefReader) => {

				if (!column) return;

				if (reference.properties[column].isAtextra) {
					var index = reference.tableIndex.sqlNames().map((key: string) => key.substring(0, key.length - 2));
					var property: PropertyMeta = util.find(reference.data.PROPERTIES, 'FLDCLA', column);
					var atextra = property.PARAM_FLDS.filter(paramData => paramData.TYPPAR === 1 && paramData.TYPKEY === 2).reduce_(_, (_, r, paramData) => {
						...
						return r;
					}, {} as Dict<RecordMapper>);

					return (_: _, reader: ez.Reader<Reference>, writer: ez.Writer<Reference>) => {
						const param = superv.sqlDriver.param;
						const params: string[] = [];
						const wheres: string[] = [];
						const all: any = reader.readAll(_);
						const rs: Reference[] = all;
						...
						if (rs && rs.length) {
							...
							wheres.push("((" + rs.map_(_, (_, record) => {
								...
								return "IDENT1_0=" + param(params.length - 2) + " and " + "IDENT2_0=" + param(params.length - 1);
							}).join(") or (") + "))");

							var sql = "select IDENT1_0, IDENT2_0, ZONE_0, TEXTE_0 from ATEXTRA  where (" + wheres.join(" and ") + ")";

							var texts = superv.sqlReader<{
								TEXTE: string;
								IDENT1: string;
								IDENT2: string;
							}>(_, sql, params).toArray(_).reduce((r, text) => {
								r[text.IDENT1 + '~' + (text.IDENT2 || " ")] = text.TEXTE;
								return r;
							}, {} as util.Dict<string>);
					...
					const anyReader = refReader; // TODO: clean this hack
					anyReader.columns = anyReader.columns || [];
					anyReader.columns.push(column);
			...
			var reference: Class = this.supervisor.load<Class>(_, 'Class', this.type.data.CODCLA);
			if (!reference) return;

			//TODO : "COLDEC", "COLLNG"
			var refMaps: Dict<(column: string) => ReferenceMapper> = {
				SYMBOL(column) {
			...
			var reader: ez.Reader<Reference>;
			if (refReader.columns) {
				...
				var names: string[] = reference.tableIndex.sqlNames();

				const fields = names.map(name => name.substring(0, name.length - 2));
				var getRecordId = (record: any) => fields.map(field => record[field]).join('~');
				...
				// add other colums :
				names = names.concat(refReader.columns.map((column: number) => column + '_0'));
				...
				.map((_: _, record: any) => {
					...
					refReader.maps && refReader.maps.forEach((map: any) => {
						map(item);
					});
					return item;
				});
			...
			if (titleTransform || descTransform) {
				if (!reader) {
					...
					const index = reference.tableIndex.sqlNames().map((key: string) => key.substring(0, key.length - 2));
					reader = ez.devices.generic.empty.reader.transform<Reference>((_, reader, writer) => {
			...
			var references = instance.cache && instance.cache.references 
				? instance.cache.references[this.name] 
				: {
					instances: {} as Dict<Instance>,
					ids: {} as Dict<any>
				};
			reader.pipe(_, ez.devices.generic.writer(function(_: _, record: Reference) {
				if (record) references.instances[record.id] = record.reference;
				return this;
			}));

			// clear the list of $ids:
			references.ids = {};
			return references.instances[id];
		});
		...
		var reference = instance.cache && instance.cache.references ? instance.cache.references[this.name].instances[refId] : undefined;
		reference = reference || this.loadReferenceData(_, refId, instance);
		if (instance.cache && instance.cache.references) instance.cache.references[this.name].instances[refId] = reference;
		return reference;
	}
};
