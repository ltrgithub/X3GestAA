...
import { Slot, newSlot } from '../dyn/slot';
import { newSlotText } from '../dyn/slotText';
import { newSlotClob } from '../dyn/slotClob';
import { newSlotBlob } from '../dyn/slotBlob';
import { newSlotAtextra } from '../dyn/slotAtextra';
...
export class Property extends Helper {
	...
	tag: string; //??
	reference: any; // ??
	loadReferenceData: (_: _,  id: string, instance: Instance) => Instance;
	...
	constructor(superv: Supervisor, clas: BaseClass, data: PropertyMeta, newMaker: SlotMaker, collection: Collection) {
		super();
		...
		reprResFact.keyItems.forEach_(_, (_: _, keyitem: any) => {
			...
			keyParams.forEach_(_, (_, paramData) => {
				if (keyitem.prop === paramData.CODPAR) {
					...
					if (paramData.alias) {
						util.any(item)[keyitem.alias] = alias(paramData.alias);
						itemKey.push(alias(keyitem.alias));
					} else {
						util.any(item)[keyitem.alias] = paramData.value;
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_: _,  simpleNode: NodeProto) {
		...
		if (link === undefined) {
			...
			this.calculateRef(_, util.any(item));
		} else {
		...
		this.loadReferenceData = this.loadReferenceData || ((_: _,  id: string, instance: Instance) => {
			...
			const addTextTransform = (_: _, superv: Supervisor, attribute: string, reference: Class, column: number, refReader: RefReader) => {
				...
				if (reference.properties[column].isAtextra) {
					...
					return (_: _, reader: ez.Reader<Instance>, writer: ez.Writer<Instance>) => {
						const param = superv.sqlDriver.param;
						const params: string[] = [];
						const wheres: string[] = [];
						const all: any = reader.readAll(_);
						const rs: Instance[] = all;
					...
					const anyReader: any = refReader; // TODO: clean this hack
			...
			if (refReader.columns) {
				...
				// add other colums :
				names = names.concat(refReader.columns.map((column: string) => column + '_0'));
