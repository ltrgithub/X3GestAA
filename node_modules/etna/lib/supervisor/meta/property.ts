...
export class Property extends Helper {
	...
	_keyMaps: { [name: string]: (_: _, instance: IInstance) => any };
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_: _,  simpleNode: types.NodeProto) {
		const factoryFacet = this.resourceFactory.facet;
		const isFactoryEditFacet = factoryFacet === '$edit';
		const node: types.NodeProto = ["$title", "$shortTitle", "$capabilities", "$isReadOnly", "$isHidden", "$isMandatory", "$isExcluded"].reduce((r, a) => {
	...
	getSql(_: _,  colNames: string[], tableNames: string[], wheres: string[], params: string[], descs: Column[], 
	...
	loadReference(_: _,  instance: IInstance) {
		this.loadReferenceData = this.loadReferenceData || function (_: _,  id, instance) {
