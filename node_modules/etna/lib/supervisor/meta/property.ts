...
// reader is not the right place to store columns - fix this hack later
export interface RefReader extends ez.Reader<Reference> {
	columns?: string[];
}
...
export class Property extends Helper {
 ...
 "C06P02_REF": {
	...
	loadAtextra(_: _,  instance: Instance) {
		...
		const getTextId = (record: any) => record.IDENT1 + '~' + (record.IDENT2 || " ");
	...
	loadReference(_: _,  instance: Instance) {
		this.loadReferenceData = this.loadReferenceData || ((_: _,  id: string, instance: Instance) => {
				...
				if (reference.properties[column].isAtextra) {
					...
					return (_: _, reader: ez.Reader<Instance>, writer: ez.Writer<Instance>) => {
						...
						if (rs && rs.length) {
							...
							var getTextId = function (record: any) {
			...
			//TODO : "COLDEC", "COLLNG"
			var refMaps: { [name: string]: (column: string) => (instance: Instance) => any; } = {
				SYMBOL(column) {
