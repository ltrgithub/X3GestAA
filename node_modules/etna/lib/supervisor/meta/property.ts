...
import { Slot, newSlot } from '../slot';
import { newSlotText } from '../slotText';
import { newSlotClob } from '../slotClob';
import { newSlotBlob } from '../slotBlob';
import { newSlotAtextra } from '../slotAtextra';
import { newSlotReference } from '../slotReference';
import { newSlotCollection } from '../slotCollection';
import { ExportedPropertyData } from './exportedProperty';
...
type IInstance = any;
type IResource = any;
type IResourceFactory = any;
type IExportedProperty = any;
...
*/
export interface PropertyParamData {
	NUMPAR: number;
	TYPPAR: number;
	FLDCLA: string;
	CODFLD: string;
...
export interface PropertyData {
	...
	FLDGRP?: string;
	CODFLD?: string;
	PARAMS?: PropertyParamData[];
	PARAM_FLDS?: PropertyParamData[];
	LOBTAB?: string;
	LOBFLD?: string;
	LNKCLA?: string;
	NUMFLD?: number;
...
export type KeyMap = (_: _, instance: IInstance) => any;
...
export class Property extends Helper {
	...
	$x3Format: string;
	jsName: string;
	_params: PropertyParamData[];
	_keyMaps: KeyMaps;
	_typeRulesMaps: { [name: string]: (_: _, instance: IInstance) => any };
	_key: string;
	resourceFactory: IResourceFactory;
	exportedProperty: IExportedProperty;
	targetResourceFactory: IResourceFactory;
	links: PropertyLinks;
	proMaps: any[]; // refine later
	toControl: (_: _) => number; // may refine further later
	tag: string; //??
	reference: any; // ??
	loadReferenceData: (_: _,  id: string, instance: IInstance) => IInstance;
	...
	isExtraRef: boolean;
	constructor(superv: Supervisor, clas: BaseClass, data: PropertyData, newMaker: SlotMaker, collection: Collection) {
		super();
	...
	setAtextra(_: _,  instance: IInstance) {
	...
	getAtextra(_: _,  instance: IInstance, lan: string) {
		// Little trick
	...
	updateAtextra(_: _,  instance: IInstance, value: string, lan: string) {
		// Save the current language:
		var saveLan: string;
		if (lan) {
	...
	deleteAtextra(_: _,  instance: IInstance) {
		// Create an instance of atextra
		const atextra = this.setAtextra(_, instance);
	...
	prototypeNode(_: _) {
		...
		if (this.targetResourceFactory && this.collection) {
			...
			if (expProData.UOMPRO) {
				...
				var propData: ExportedPropertyData = this.findExportedProperty(this.exportedProperty.data.UOMPRO);
	...
	calculateRef(_: _,  item: types.PropertyRefProto) {
		...
		keyParams = this.keyParams.map(paramData => {
			const propData: ExportedPropertyData = this.findExportedProperty(path + paramData.VALEUR);
 ...
 "C06P02_REF": {
	...
	prototypeRefNode(_: _,  simpleNode: types.NodeProto) {
		const factoryFacet = this.resourceFactory.facet;
		const isFactoryEditFacet = factoryFacet === '$edit';
		const node: types.NodeProto = ["$title", "$shortTitle", "$capabilities", "$isReadOnly", "$isHidden", "$isMandatory", "$isExcluded"]
		.reduce((r, a) => {
	...
	makeGetReferenceKey(_: _) {
		...
		return (_: _, record: types.Row) => Object.keys(refKeys).map_(_, (_, k) => refKeys[k](_, record) || "").join('~');
	}
	getAtextraKey(_: _,  instance: IInstance) {
		const keyMaps = this.keyMaps(_);
	...
	cache: any, raw?: boolean, sqlOpts?: types.DriverReaderOptions): void {
	...
	addFieldTransform(_: _,  value: any, index?: any[], refReader?: { columns: string[] }): (_: _, instance?: IInstance) => any {
		const regNum$ = /=num\$\(([\w\.]*)\)/;
		const regVal$ = /=val\(([\w\.]*)\)/;
	...
	loadAtextra(_: _,  instance: IInstance) {
	...
	loadReference(_: _,  instance: IInstance) {
		this.loadReferenceData = this.loadReferenceData || function (_: _,  id: string, instance: IInstance) {
			tracerJs.debug && tracerJs.debug("loadReferenceData(" + this.name + "):" + id);

			//dbg:console.error("loadReferenceData(" + this.name + "):" + id);

			const addTextTransform = (_: _, superv: Supervisor, attribute: string, reference: IInstance, column: string, refReader: ez.Reader<IInstance>) => {
				...
				if (reference.properties[column].isAtextra) {
					...
					var property: PropertyData = util.find(reference.data.PROPERTIES, 'FLDCLA', column);
					...
					return (_: _, reader: ez.Reader<types.Row>, writer: ez.Writer<types.Row>) => {
					...
					return (_: _, reader: ez.Reader<IInstance>, writer: ez.Writer<IInstance>) => {
						var record: IInstance;
			...
			try {
				...
				//TODO : "COLDEC", "COLLNG"
				var refMaps: { [name: string]: (column: string) => (item: IInstance) => any; } = {
					SYMBOL(column) {
				...
				reader.pipe(_, ez.devices.generic.writer((_: _, record: any) => {
