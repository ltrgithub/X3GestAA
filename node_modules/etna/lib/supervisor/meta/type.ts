...
import { Reader } from 'ez-streams';
import * as util from '../util';
import { Dict, Prog, ProgArgs } from '../../engine/runtime/types';
...
import { PropertyMeta } from '../metadata/class';
import { TypeMeta } from '../metadata/type';
import { Class, Property, TypePro } from './types';
...
export const etnaType2TypTyp = Object.keys(typTyp2EtnaType).reduce((r, k) => (r[typTyp2EtnaType[k as any]] = +k, r), {} as Dict<number>);
...
export type MediaRule = (_: _,  slot: Slot, value?: any) => any;
export type MediaLazyRule = (_: _,  property: Property, value: any) => { value: number };

export interface TypeRuleParam {
	type: string;
}
...
export class Type {
	...
	constructor(superv: Supervisor, data: TypeMeta) {
		this.supervisor = superv;
		if (data.LNGTYP && typeof data.LNGTYP === 'string') {
			if (!isNaN(data.LNGTYP) && bcd.toDouble(bcd.fromString(data.LNGTYP as any)) > 0) {
				data.LNGTYP = bcd.toDouble(bcd.fromString(data.LNGTYP as any));
			}
	...
	init(_: _) {
		...
		this.protyp = this.data.TYPPROSYR ? this.supervisor.load<TypePro>(_, 'Typepro', this.data.TYPPROSYR) : undefined;
		if (this.data.RULES) {
			this.rules = this.data.RULES
			.filter_(_, (_, rule) => !/^WMTYP/.test(rule.TRTRUL) && rule.ENARUL === 2 && 
			activ.getActiv(_, this.supervisor, rule.ACTRUL) > 0)
			.reduce_(_, (_, r, rule) => {
				r[mapTypeRule[rule.TYPRUL]] = {
					...
					params: rule.PARAMETERS ? rule.PARAMETERS.reduce_(_, (_, r, param) => {
						...
						return r;
					}, {} as Dict<TypeRuleParam>) : {}
				};
				return r;
			}, {} as Dict<TypeRuleDesc>);
	...
	targetClass(_: _) {
		if (this.data.CODCLA) return this.supervisor.load<Class>(_, 'Class', this.data.CODCLA);
	}
 ...
 */
	...
	sqlType(colData?: ColumnMeta): string {
	...
	applyRule(_: _,  rule: TypeRuleDesc, args: ProgArgs) {
	...
	fillNode(_: _,  data: PropertyMeta, node: NodeProto) {
		if (this.atypepro === undefined) {
			var typpro: TypePro = this.supervisor.load<TypePro>(_, 'Typepro', this.data.TYPPROSYR);
...
//TYPTYP = 1
export class TypeJ extends Type {
	constructor(superv: Supervisor, data: TypeMeta) {
		super(superv, data);
	}
	get jsType() {
...
export class TypeLob extends Type {
	static promote(type: Type): TypeLob { return type as any; }
	...
	contentType(_: _,  slot?: Slot) {
		...
		if (slot !== undefined && this.rules && this.rules.READ_MEDIA_CNT) {
			...
			// TODO: investigate typing issue on args
			var astatus = this.applyRule(_, rule, args as any);
			contentType = args.AVALCNT.value;
		}
		return contentType || "";
	}
	reader(_: _,  val: any): Reader<any> {
...
export function	getConstructor(data: TypeMeta) {
	switch (data.TYPTYP) {
		case 1:
			return TypeJ;
		case 2:
...
export function	fillTypeNode(_: _,  superv: Supervisor, data: ColumnMeta, node: NodeProto) {
	const typ = superv.load<Type>(_, 'Type', data.CODTYP);
