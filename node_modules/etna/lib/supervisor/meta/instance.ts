...
export class Instance extends BaseInstance {
	methods: { [name: string]: {}}
	constructor(superv: Supervisor, clas: Class, parent: SlotCollection, slots: { [name: string]: Slot }) {
		super(superv);
	...
	afterCreate(_: _,  record, colnIndex) {
	...
	action(_: _,  action, key, member, frame) {
	...
	propagateControl(_: _,  type, operation) {
	...
	propagateControlExecution(_: _,  type) {
		var status = constants.CST_AOK;
	...
	executeControlMandatory(_: _,  type) {
		const instance = this;
	...
	initMaps(_: _,  collection, parent) {
		const defMap = (_, map) => {
			var value = "";
