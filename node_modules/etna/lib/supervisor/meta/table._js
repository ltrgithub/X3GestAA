...
class Table extends Helper {
	...
	insertRecord(_, record) {
		...
		var values = this.sql.columnDescs.reduce_(_, (_, r, desc) => {
			var val = record[desc.name];
	...
	create(_, exists) {
		...
		var sql = "create table " + this.name + " (UPDTICK_0 NUMBER(10) DEFAULT 0," + Object.keys(this.columns).map(function(name) {
			var col = this.columns[name];
			return col.sqlDef();
