...
class Table extends Helper {
	...
	init(_) {
		this.columns = {};
		this.data.COLUMNS.filter(function (data) {
	...
	executeSql(_,instance, sql, values, lobVars) {
		let output;
		if(lobVars.length) {
			const param = this.supervisor.sqlDriver.param;
	...
	insert(_, instance) {
		...
		let lobVars = [];
		const values = this.sql.columnDescs.reduce_(_, function (_, r, desc) {
			if(desc.type.isLob) lobVars.push(desc)
			else r.push(instance.getSql(_, desc));
			return r;
		}, [], this);
		
		let sql = "insert into " + this.name + " (" + this.sql.columnNames.join(',') + //
		") values (" + this.sql.insertVars + ")";
	...
	delete(_, instance) {
		this.computeColumnNames(_);
		const values = this.indexes[0].columns.map_(_, function (_, column) {
			return instance.slot(_, column.name).getSql(_);
		}, this);
	...
	writer(_, cnx) {
		...
		const self = this;
		const wr = writer.pre.map(function (_, data) {
			wr.count++;
	...
	create(_, exists) {
		...
		const sql = "create table " + this.name + " (UPDTICK_0 NUMBER(10) DEFAULT 0," + Object.keys(this.columns).map(function (name) {
			const col = this.columns[name];
