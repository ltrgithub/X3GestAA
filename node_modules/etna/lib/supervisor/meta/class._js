...
module.exports = {
	...
	constructor: baseClass.defineClass(function(superv, data, group) {
			...
			this.properties = supervProperties.reduce_(_, (_, r, propData) => {
					...
					if (!((self.group || "") !== ((!propData.LNKCLA && propData.FLDGRP) || "")) || supervPropertiesFields.indexOf(name) >= 0) {
						...
						if (r[name].isReference()) {
							...
							r[name].jsName = util.fixJsName(self.data.CODCLA, name + '_REF', r[name].data._proxyName + 'Ref');
							self.membersByJsName[r[name].jsName] = r[name];
						}
					}
					return r;
		...
		*/
		...
		readInstance: function(_, parent, key, qs) { // x3js API, key is value, array or object
		...
		queryInstancesSql: function(_, select, max) {
			return this.reader(_, null, select, max, null);
		},
		queryInstancesInt: function(_, select, max) {
		...
		readLob: function(_, context, property, ident1, ident2, ident3) {
			var key =[ident1];
