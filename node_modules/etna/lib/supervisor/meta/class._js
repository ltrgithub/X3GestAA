...
const utilRuntime = require('etna/lib/engine/runtime/util');
...
const newSlotInstance = require("etna/lib/supervisor/meta/instance").newSlotInstance;
...
const cacheMethods = require('etna/lib/supervisor/builtins/ACTXCACHE').cacheMethods;
...
class Class extends BaseClass {
	...
	init(_) {
		...
		const supervPropertiesFields = _supervProperties.reduce(function (r, property) {
		...
		this.properties = supervProperties.reduce_(_, (_, r, propData) => {
			...
			if (!((self.group || "") !== (!propData.LNKCLA && propData.FLDGRP || "")) || supervPropertiesFields.indexOf(name) >= 0) {
				...
				self.membersByJsName[r[name].jsName] = r[name];
		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, function (_, data) {
			...
			const coln = self.relations[data.CODCOL] = self.supervisor.new(_, 'Collection', this, data);
			...
			if (!p) {
				...
				if (mapping && mapping.MAPS) {
					...
					mapping.MAPS.filter(function (map) {
						return map.PROMAP && map.KEYMAP;
			...
			self.membersByJsName[self.properties[name].jsName] = self.properties[name];
		}, this);
		...
		this.methods = this.data.METHODS ? this.data.METHODS.reduce_(_, function (_, r, method) {
			...
			r[method.CODMET] = {
				...
				params: method.PARAM_DEFS ? method.PARAM_DEFS.reduce_(_, function (_, r, param) {
					param.NOPAR = noPar++;
					r[param.CODPAR] = {
				...
				buildArgs: function ($method) {
					return function (_, instance) {
						...
						if ($method.FLGOPE && $method.PARAM_DEFS) {
							$method.PARAM_DEFS.filter(function (param) {
								return param.TYPKEY === 2;
				...
				getServiceArgs: function ($method) {
					return function (_, key, qs) {
						...
						const newArgs = $method.PARAM_DEFS ? $method.PARAM_DEFS.reduce_(_, function (_, r, param) {
 ...
 */
	addMethods(instance) {
		...
		const xmeth = function (clas, method, methodType) {
			...
			return function (_) {
				...
				const meth = self.methods[method];
				...
				const vals = utilRuntime.instructions.A(args)(_);
		...
		Object.keys(this.methods).forEach(function (method) {
			...
			instance.methods["xmet" + method] = xmeth(self, method, self.methods[method].type);
		});
	}
	...
	queryExpToSelect(_, exp, params, forJs) {
		const map = {
			'$eq': '=',
			'$ne': '!=',
			'$gt': '>',
	...
	queryToSelect(_, query, params, forJs) {
	...
	queryInstancesInt(_, select, max) {
		...
		const expToWhere = function (exp) {
			// console.log("expToWhere:",exp);
			const wheres = Object.keys(exp).reduce(function (r, prop) {
				const propExp = exp[prop];
				const hasExp = typeof propExp === 'object' && propExp !== null && Object.keys(propExp).some(function (key) {
					return key.charAt(0) === '$';
				}) || false;
		...
		const selectToWhere = function (select) {
			if (!select || !select.$query) return;
			return z.WHERE(expToWhere(select.$query));
		};

		const selectToOrder = function (select) {
		...
		if (lnk) {
			...
			reader = ez.devices.generic.reader(function (_) {
				return;
			});
		}
	...
	sqlReader(_, parent, select, max, $recordMapper) {
		...
		const self = this;
		const recordMapper = $recordMapper || function () {
			return function (_, record) {
				const instance = self.supervisor.new(_, 'Instance', self, parent).afterCreate(_, record);
	...
	rsReader(_, recordset, select) {
		if (select && select.$orderBy) {
			var sort = function (orderBy) {
				const fcts = Object.keys(orderBy).reduce(function (r, property) {
	...
	loadInstances(_, parent, index, vals, max, $recordMapper) {
		return this.reader(_, parent, Object.keys(vals).reduce(function (o, c) {
			if (index.columns[c]) {
				if (vals[c] !== undefined) {
	...
	search(_, httpContext, qs) {
		...
		const fromInstanceToResource = function (_, instance) {
	...
	readLob(_, context, property, codblb, ident1, ident2, ident3) {
		const ident = function (s) {
			return s || " ";
		};
		...
		if (lob) {
			if (lob.type.readMediaLazy) {
				...
				const param = this.supervisor.sqlDriver.param;
				var i = 0;
				var rs = this.supervisor.sqlReader(_, "select " + lob.lobField + "_0 From " + lob.lobTable + " Where " + Object.keys(keys).map(p => {
					return p + '_0=' + param(i++);
				}).join(' and '), Object.keys(keys).map(p => {
					return keys[p];
				})).toArray(_);
