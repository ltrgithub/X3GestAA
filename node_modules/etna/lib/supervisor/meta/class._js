...
class Class extends BaseClass {
	...
	init(_) {
		...
		this.jsName = util.fixJsName(this.data.CODCLA, null, this.data._proxyName || this.name);
		...
		this.properties = supervProperties.reduce_(_, (_, r, propData) => {
				...
				if (!((self.group || "") !== ((!propData.LNKCLA && propData.FLDGRP) || "")) || supervPropertiesFields.indexOf(name) >= 0) {
					...
					r[name].jsName = util.fixJsName(self.data.CODCLA, name, propData._proxyName || name);
		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, function(_, data) {
			...
			self.properties[name].jsName = util.fixJsName(self.data.CODCLA, name, data._proxyName);
