...
module.exports = {
	...
	constructor: baseClass.defineClass(function(superv, data, group) {
		...
		init: function(_) {
			...
			var supervPropertiesFields = supervProperties.reduce(function(r, property) {
				r.push(property.FLDCLA);
				return r;
			}, []);

			this.properties = supervProperties.concat(this.data.PROPERTIES)
			...
			this.methods = this.data.METHODS ? this.data.METHODS.reduce_(_, function(_, r, method) {
						...
						r[param.CODPAR] = {
							...
							index: function(nopar) {
								return function(args) {
									return ((args && args.length >= nopar) ? nopar - 1 : -1);
								};
							}((param.TYPKEY === 2 ? 0 : nbKeys) + param.NOPAR),
							get: function(nopar) {
								return function(args) {
									return ((args && args.length >= nopar) ? args[nopar - 1] : variables.types[type.typTyp2EtnaType[param.TYPINTPAR]].default);
								};
							}((param.TYPKEY === 2 ? 0 : nbKeys) + param.NOPAR),
						};
				...
				r[method.CODMET]["Service"] = {
					getArgs: function() {
						return function(_, keys, qs) {
							var newArgs;
							var key = keys.split('~');
							newArgs = self.data.PARAM_DEFS ? self.data.PARAM_DEFS
								.filter(function(param) {
									return param.CODE === method.CODMET;
								})
								.reduce_(_, function(_, r, param) {
									if (param.TYPKEY === 2) {
										r[param.NOPAR - 1] = {
											value: key[param.NOPAR - 1]
										};
									} else {
										var varObj;
										if (qs[param.CODPAR]) {
											var val = qs[param.CODPAR];
											if (val === '""') val = variables.types[type.typTyp2EtnaType[param.TYPINTPAR]].default;
											varObj = variables.types[type.typTyp2EtnaType[param.TYPINTPAR]].set(param.CODPAR, param.CODPAR, val);
										} else varObj = variables.types[type.typTyp2EtnaType[param.TYPINTPAR]].default;
										r[nbKeys + param.NOPAR - 1] = {
											value: varObj,
											noset: true
										};
									}
		...
		*/
		addMethods: function(instance) {
			...
			var xmeth = function(clas, type, method) {
				return function(_) {
					...
					if (vals.length > 0 && (Object.keys(meth).length - 3) > vals.length) {
					...
					methArgs[(type === "$OPERATIONS") ? "AOPERATION":"AMETHOD"] = {
						type: "LS",
						value: method
					};

					methArgs = Object.keys(meth).reduce(function(r, p) {
						if (meth[p].type) {
							r[p] = {
								type: meth[p].type,
					...
					var status = this.runScripts(_, type, methArgs);
					// Manage variable arguments :
					var retValue;
					Object.keys(meth).filter(function(arg) {
						return meth[arg].set;
					}).forEach_(_, function(_, arg) {
						var index = meth[arg].index(args);
			...
			instance.methods["xmetAGETMAXERRORTXT"] = xmeth(self, "$OPERATIONS","AGETMAXERRORTXT");
			instance.methods["xmetAGETNEXTINFOS"] = xmeth(self, "$OPERATIONS","AGETNEXTINFOS");

			Object.keys(this.methods).forEach(function(method) {
				instance.methods = instance.methods || {};
				instance.methods["xmet" + method] = 
					xmeth(self,self.methods[method].$type,method);
			});
		},
		isSearchable: {
			get: function() {
