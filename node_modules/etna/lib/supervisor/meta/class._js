		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, function (_, data) {
			...
			if (!p) {
				...
				coln.class = self.supervisor.new(_, 'Class', {
					...
					TYPCLA: 1, //5,
					PROPERTIES: self.data.PROPERTIES.filter(propData => propData.FLDGRP === name),
				...
				buildArgs: function ($method) {
					return function (_, instance) {
						...
						if ($method.FLGOPE && $method.PARAM_DEFS) {
							$method.PARAM_DEFS.filter(param => param.TYPKEY === 2).forEach_(_, function (_, param) {
								args[param.CODPAR] = {
	...
	addMethods(instance) {
		...
		const xmeth = function (clas, method, methodType) {
			...
			return function (_) {
				...
				Object.keys(meth.params).filter(arg => meth.params[arg].set).forEach_(_, function (_, arg) {
