...
class Class extends BaseClass {
	...
	init(_) {
		...
		this.jsName = util.fixJsName(this.supervisor, this.data.CODCLA, null, this.data._proxyName || this.name);
		this.kebabName = util.camelToKebab(this.jsName);
		...
		this.properties = supervProperties.reduce_(_, (_, r, propData) => {
				...
				if (!((self.group || "") !== ((!propData.LNKCLA && propData.FLDGRP) || "")) || supervPropertiesFields.indexOf(name) >= 0) {
					...
					r[name].jsName = util.fixJsName(self.supervisor, self.data.CODCLA, name, propData._proxyName || name);
		...
		this.data.COLLECTIONS && this.data.COLLECTIONS.forEach_(_, function(_, data) {
			...
			self.properties[name].jsName = util.fixJsName(self.supervisor, self.data.CODCLA, name, data._proxyName);
	...
	*/
	...
	readInstance(_, parent, key) { // x3js API, key is value, array or object
		var instances = this.reader(_, parent, key, 1).toArray(_);
		return instances[0];
	}
	...
	queryExpToSelect(_, exp, params, forJs) {
		...
		return Object.keys(exp).reduce_(_, (_, r, k) => {
			...
			var prop = forJs ? this.membersByJsName[k] : this.properties[k];
			if (!prop) throw new Error("Missing property " + this.name + '.' + k);
			var column = prop.column.sqlName();


			var hasExp = (typeof propExp === 'object' && propExp !== null) && 
			Object.keys(propExp).some(key => key.charAt(0) === '$') || false;
	...
	queryToSelect(_, query, params, forJs) {
		...
		if (operator) {
			return query['$' + operator].reduce_(_, (_, r, exp) => {
				var select = this.queryToSelect(_, exp, params, forJs);
	...
	readLob(_, context, property, codblb, ident1, ident2, ident3) {
		...
		throw new util.HttpError(404, this.name+'.'+property + '('+key.join('~')+ ") couldn't be loaded");
	}
};
