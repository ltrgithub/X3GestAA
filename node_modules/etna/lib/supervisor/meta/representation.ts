...
import { Dict, Frame } from '../../engine/runtime/types';
import * as ez from 'ez-streams';
import { Article, Menu } from '../protocol/page';
import { ServiceProto } from '../protocol/prototype';
import { PageProto } from '../protocol/page';
import * as util from '../util';
import * as Sdata from '../../engine/compilers/sdatagen';
...
import { Property, Collection, Class, ExportedProperty, ResourceFactory, Link, Table } from './types';
...
export class Representation extends BaseClass {
	...
	init(_: _) {
		...
		this.class = this.supervisor.load<Class>(_, 'Class', this.data.CODCLA, {
		...
		this.properties = this.data.PROPERTIES ? this.data.PROPERTIES.filter(prop => !prop.FLDGRP).reduce_(_, (_, r, prop) => {
			...
			r[name] = this.supervisor.new<Property>(_, 'Property', this, prop);
			return r;
		}, {} as Dict<Property>) : {};
		...
		this.relations = this.data.COLLECTIONS && this.data.COLLECTIONS.reduce_(_, (_, r, col) => {
			if (col.CODCOL.indexOf('.') < 0) {
				...
				var coln = r[name] = this.supervisor.new<Collection>(_, 'Collection', this, col);
				...
				var prop = this.properties[name] = this.supervisor.new<Property>(_, 'Property', this, {
					CODFLD: name
				}, 'collection', coln);
			}
			return r;
		...
		.map_(_, (_, data) => this.supervisor.new<ExportedProperty>(_, 'ExportedProperty', this, data), this)
		 : [];
	...
	prepareLinks(_: _) {
		...
		this.links = this.data.LINKS.filter(link => {
			...
			if (link.TYPLNK === 1) {
				...
				repr = link.REPLNK === this.name ? this : this.supervisor.load<Representation>(_, 'Representation', link.REPLNK);
	...
	resourceFactory(_: _,  facet: string) {
		...
		factory = this.supervisor.new<ResourceFactory>(_, 'ResourceFactory', this.class, this, facet, []);
		factory.ETag = this.data._etag + "-" + util.currentContext().x3session.actx.$LAN(_);
		...
		this.exportedProperties.filter(expprop => expprop.data[facetFlagPRO] === 2).forEach_(_, (_, exppro) => {
			...
			if (this.relations && this.relations[path[0]]) {
				if (!(prop = factory.properties[path[0]])) {
					...
					prop.tag = path[0];
					prop.targetResourceFactory = this.supervisor.new<ResourceFactory>(_, 'ResourceFactory', this.relations[path[0]].class, this, facet, path[0]);
				...
				for (; i < path.length - 1; i++) {
					...
					container = im;
					f = prop.targetResourceFactory || (prop.targetResourceFactory = this.supervisor.new<ResourceFactory>(_, 'ResourceFactory', im, this, facet, path.slice(0, i + 1)));
				}
			...
			if (prop._isReference === true && prop.repLookup) {
				...
				else r = this.supervisor.load<Representation>(_, 'Representation', prop.repLookup);
		...
		if (facet === "$edit") {
			...
			this.links.filter(link => link.FLGSTDLNK === 2 && link.TYPLNK === 1 && link.AFFLNK > 3).forEach_(_, (_, link) => {
				...
				if (link.AFFLNK === 4 && link.TYPLNK === 1 && link.CMPLNK === 5) {
					//Query
					l = this.supervisor.new<Link>(_, 'Link', factory, link);
					//prepare keys+params
		...
		this.links.filter(link => link[facetFlagLNK] === 2).forEach_(_, (_, link) => {
			...
			const l = this.supervisor.new<Link>(_, 'Link', anchor, link);
			//prepare keys+params
	...
	articleLayout(_: _,  facet: string): Article {
		return {
			$layout: {
	...
	article(_: _,  facet: string, f: ResourceFactory) {
		...
		const trimLayout = (obj: Article) => {
			var hasElt = false;
			var hasBind = false;
			const new_items: Article[] = [];

			if (obj.$layout.$items && obj.$layout.$items.length > 0) {
		...
		//Menu
		const fillMenu = (facet: string, tabLinks: { [name: string]: Link; }, curMenu: string, items: Article[]) => {
			var hasElt = false;
			if (tabLinks) {
		...
		const fillMenuMen = (_: _, facet: string, tabMenus: MenuMeta[], curMenu: string, 
			links1: { [name: string]: Link; }, links2: { [name: string]: Link; }, items: Menu[]) => {
			var hasElt = false;
	...
	action(_: _,  action: string, frame?: Frame) {
	...
	beforeQuery(_: _) {
		...
		const prev = util.currentContext().x3frame;

		util.currentContext().queryFrame = {
			values: {},
			types: {},
		...
		return this.action(_, "AQUERY_OPEN_AFTER", util.currentContext().queryFrame);
	}
	afterQuery(_: _) {
		const status = this.action(_, "AQUERY_CLOSE");

		util.currentContext().queryFrame = undefined;
		return status;
	}
	service(_: _,  keys: string, $service: string, facet: string, qs: any) {
		const reply: ServiceProto = {};
		...
		this.deleteErrors(_);
		const cl = this.supervisor.load<Class>(_, 'Class', codcla);
