...
class SlotCollection{
	...
	clone() {
		var to = new SlotCollection(this.$parent, this.collection);
	...
	getDelta(_, snapshot) {
		...
		var lineIndex = 0;
		var diffLines = snapshot.lines.reduce(function(r, line) {
	...
	initFromParent(_, parent) {
		...
		if (instanceCollection.PROCOL) {
			...
			if (procol) {
				...
				procol._get = function(_, raw) {
					tracerJs.debug && tracerJs.debug(instanceCollection.PROCOL + "._get " + instanceCollection.proName + ".x3Maxtab()");
					var instance = this.$parent;
					var collection = instance.slot(_, instanceCollection.proName);
...
exports.newSlotCollection = function(property) {
	return function(parent) {
		var instanceCollection = new SlotCollection(parent, property.collection);
