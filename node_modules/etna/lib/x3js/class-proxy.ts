...
import * as ACTXCACHE from '../supervisor/builtins/ACTXCACHE';
...
export interface StringPropertyDef extends PropertyDef {
	maxLength?: number;
}

export interface EnumPropertyDef extends PropertyDef {
	enum?: any; // see later
	chapter?: number;
}

export interface DecimalPropertyDef extends PropertyDef {
	precision?: number;
	scale?: number;
}

export interface ClassIndexDef {
	name: string;
	properties: string;
	activityCode?: string;
	is?: string;
...
export interface ClassTableDef {
	name: string;
	abbrev: string;
	indexes: ClassIndexDef[];
}
...
export class ClassHandler {
	...
	create(values: any) {
		return x3jsUtil.wait(_ => {
			...
			const instance = clas.supervisor.new<Instance>(_, 'Instance', clas);
			...
			if (values) Object.keys(values).forEach(name => {
				(handler as any)[name] = values[name];
	...
	read(key: any) {
		return x3jsUtil.wait(_ => {
			const clas = this._class;
			const instance = clas.readInstance(_, null, resolveKey(clas, key));
	...
	cache(key: any) {
		return x3jsUtil.wait(_ => {
			const clas = this._class;
			const acache = util.currentContext().x3session.actx.get(_, 'ACACHE');
...
export function legacyClass(_: _,  code: string) {
	const clas = x3jsUtil.currentSupervisor(_).load<Class>(_, 'Class', code);
	return new ClassHandler(clas);
};


export function define(_: _,  def: ClassDef) {
