...
class ClassWrapper {
	...
	create(_, values) {
		...
		if (values) Object.keys(values).forEach_(_, (_, name) => {
			...
			if (!member) throw new Error("invalid class member: " + this.class.jsName + '.' + name);
	...
	cache(_, key) {
		...
		const cl = acache.get(_, this.class.name);
		const instance = cl.getInstance(Array.isArray(key) ? key.join('~') : key);
		if (!instance) throw new Error("cached lookup failed: " + this.class.jsName + '/' + key);
