...
///  const query = MYTABLE.query(); // query object
///  ```
/// 
...
class TableQuery {
	...
	/// #### Building a query
	/// 
	/// * `query.join(tableName, alias, condition)`: adds a join to another table.  
	...
	execute(_) {
		const tables = this.tables.map_(_, (_, t) => {
			return {
				...
				meta: this.supervisor.load(_, 'Table', t.name),
			};
		});
		const sqlOpts = {
			typesmap: [],
		...
		return this.supervisor.sqlReader(_, sql, args, sqlOpts).map(function(_, rec) {
