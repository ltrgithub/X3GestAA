...
exports.fromClassScript = (_, supervisor, script) => {
	...
	return {
		...
		COLUMNS: Object.keys(script.properties).map((name, i) => {
			...
			return {
				CODFIC: CODFIC,
				CODZONE: prop.columnName || name.toUpperCase(),
		...
		INDEXES: (table.indexes || []).map((ind, i) => {
			if (!ind.columns) throw new Error('bad index: columns missing: ' + ind.name);
			return {
				CODFIC: CODFIC,
				CODACT: ind.activityCode || '',
				CODIND: ind.name || CODFIC + '_IDX' + (i + 1),
				DESCRIPT: ind.columns.split(',').map(s => s.substring(1)).join('+'), // see later about asc/desc encoding
				HOMONYM: /unique/.test(ind.is) .unique ? 2 : 1,
