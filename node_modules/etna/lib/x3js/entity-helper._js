...
const TYPE_PROPS = ['enum', 'typeCode', 'maxLength', 'precision', 'scale', 'classCode'];
		...
		if (descCol.mapping === 'denormalized') {
			...
			items.forEach_(_, (_, row, i) => {
				...
				if (descCol.key || descCol.specialKey) {
					const key = descCol.key ? (row[descCol.key] || ('#' + (i + 1))) : specialKeyToCanonical(row, descCol.specialKey);
					dest[name] = dest[name] || {};
					dest[name][key] = metaToCanonical(_, superv, descCol, row);
				} else {
					dest[name] = dest[name] || [];
