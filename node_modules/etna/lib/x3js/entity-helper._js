...
entityHelper.updateSource = updateSource;
...
entityHelper.fillFromMeta = function(_, instance, descriptor, superv, meta, prefix) {
		...
		try {
			...
			switch (descProp.type) {
				...
				case 'flags':
					...
					break;
				case 'object':
					entityHelper.fillFromMeta(_, instance, descProp, superv, meta, name);
					break;
				case 'event':
					break;
				case 'type':
	...
	Object.keys(descriptor.collections || {}).forEach_(_, (_, k) => {
		...
		(meta[descCol.metaCode] || []).forEach_(_, (_, row) => {
			...
			entityHelper.fillFromMeta(_, rowInst, descCol, superv, row);
		});	
	});
}
...
entityHelper.updateAll = function(_, desc, instance, saving) {
	...
	if (instance.status(_) === '4gl') {
		instance.source(_, `// ${instance.dictCode(_)}: source not available. X3JS name has not been set.`);
		// do not save to file.
		return;
	}
	...
	if (fs.exists(filePath, _, _)) {
		parsed = entityParser.parseFile(_, filePath, desc);
	} else {
		const source = codeGen.template(_, desc);
