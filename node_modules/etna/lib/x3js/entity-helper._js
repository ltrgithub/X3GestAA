...
exports.deserializedToData = function(desc, d2, prefix = '') {
	const merged = {};
	...
	Object.keys(desc.properties).forEach(k => {
		...
		if (prop.type === 'object') {
			merged[k] = exports.deserializedToData(prop, d2, name);
		} else {
			merged[k] = d2[name];
		}
	});
	Object.keys(desc.collections || {}).forEach(k => {
		...
		if (!coln.key) {
			...
			merged[k] = d2[k].map(elt => exports.deserializedToData(coln, elt));
		} else {
			d2[k].forEach(elt => {
				const key = elt[coln.key];
				if (key) {
					merged[k] = merged[k] || {};
					merged[k][key] = exports.deserializedToData(coln, elt);
				}
			});
		}
	});
...
exports.mergeData = function(d1, desc, d2) {
	...
	Object.keys(desc.properties).forEach(k => {
		...
		switch (prop.type) {
			...
			case 'event':
				...
				break;
			case 'object':
				merged[k] = exports.mergeData(d1[k] || {}, prop, d2[k]);
				break;
			case 'flags':
	...
	Object.keys(desc.collections || {}).forEach(k => {
		...
		if (!coln.key) {
			...
			Object.keys(d2[k]).forEach(key => {
				merged[k][key] = exports.mergeData(merged[k][key] || {}, coln, d2[k][key]);
			});
		}
	});
	return merged;
