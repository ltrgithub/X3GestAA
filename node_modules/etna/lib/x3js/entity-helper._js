...
entityHelper.fillEntity = function(entity, descriptor, prefix) {
	...
	if (descriptor.hasSourceFile) {
		...
		entity.$services.saveSource = {
			...
			$execute(_, context, instance) {
				entityHelper.fromInstance(_, instance, descriptor).saveInstanceSource(_, instance);
			},
		};
	}
	// add this one systematically, to simplify create
	entity.$init = function(_, wc, context) {
		wc.$fillFromParameters(_, context.query);
	};
...
function canonicalToMeta(_, superv, descriptor, data, parent) {
	const dest = {};
...
class Helper {
	...
	canonicalToMeta(_, data) {
		return canonicalToMeta(_, this.supervisor, this.descriptor, data);
	}
	...
	sourceFilename(module, name) {
		...
		return fsp.join(root, util.kebabify(module || 'undefined'), this.descriptor.subdir, util.kebabify(name) + '.x3js');
	}
	...
	synchronize(_, context) {
		...
		if (desc.metaName) {
			...
			reader.forEach(_, (_, metaData) => {
				...
				var oldData = context.db.db.collection(util.pascalize(desc.entityName), _).find({
		...
		allParsed.forEach_(_, (_, parsed) => {
			...
			if (fileData[dictCodeOrId]) {
				...
				else throw new Error(`${fileData.name}: invalid dictCode: ${fileData[dictCodeOrId]}`);
			}
...
entityHelper.fromInstance = function(_, instance, descriptor) {
	if (instance.endpoint(_) == null) {
		instance.endpoint(_, globals.context.session.getUserProfile(_).selectedEndpoint(_));
	}
	return entityHelper.fromEndpoint(_, instance.endpoint(_), descriptor);
}
