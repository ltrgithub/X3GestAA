		...
		if (descCol.mapping === 'denormalized') {
			...
			items.forEach_(_, (_, row, i) => {
				...
				if (descCol.key || descCol.specialKey) {
					...
					const key = descCol.key ? genName(row[descCol.key], obj, allNames) : specialKeyToCanonical(row, descCol.specialKey);
					dest[name] = dest[name] || {};
					dest[name][key] = obj;
				} else {
					dest[name] = dest[name] || [];
...
class Helper {
	...
	sourceFilename(module, name) {
		...
		return fsp.join(root, module || 'undefined', this.descriptor.subdir, util.kebabify(name) + '.x3js');
	}
	...
	synchronize(_, context, endpoint) {
		...
		const processed = {};
		ezm.reader(superv.mongoStore.collection(desc.metaName, _).find({})).forEach(_, (_, metaData) => {
			var oldData = context.db.db.collection(util.pascalize(desc.entityName), _).find({
				'endpoint._uuid': endpoint.$uuid,
				dictCode: metaData[dictKey],
			}).toArray(_)[0]
