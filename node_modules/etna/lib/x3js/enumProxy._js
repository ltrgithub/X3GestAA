...
/// #### Defining an enum
/// 
/// An enum is defined with the `x3js.enum(...)` function:
/// 
/// ``` javascript
/// import * as x3js from 'x3js';
/// 
/// export default x3js.enum({
/// 	name: 'HighLow',
/// 	dictCode: 6,
/// 	values: {
/// 		HIGH: 1,
...
class EnumHandler{
	...
	$name(val) {
		var keys = Object.keys(this.def.values);
	...
	/// * `MyEnum.$names()`: returns the list of names, as an array.
	$names() {
	...
	$text(_, val) {
		const text = runtime.functions.MESS.fn(_, val, this.def.dictCode, 1);
...
exports.create = def => {
	...
	return new Proxy(def, {
		get(def, name) {
			if (name[0] === '$') {
				const handler = new EnumHandler(def);
			...
			const val = def.values[name];
			if (val == null) throw new Error(`invalid enum value: ${name}, expected one of ${Object.keys(def.values).join(', ')}`);
			return val;
		},
		ownKeys(def) {
			return Object.keys(def.values);
		},
	});
}
