...
const templateString = function (desc) {
...
const specials = {
	property(node, data) {
		node.properties = node.properties.filter(n => n.key.name !== 'type');
...
const enumNode = (val, collect) => {
	const pair = val.split('.');
	if (pair.length !== 2) throw new Error(`invalid enum specification: ${val}`);
	collect.imports[pair[1]] = '../../' + util.kebabify(pair[0]) + '/enums/' + util.kebabify(pair[1]);
	return types.identifier(pair[1]);
}

function genAst(desc, name, data, dest, collect) {
	...
	switch (desc.type) {
		...
		case 'object':
			...
			if (desc.codeGen !== 'enum' && desc.codeGen !== 'message') {
				var empty = true;
			...
			genProp(dest, name).value = desc.codeGen 
				? specials[desc.codeGen](objExp, obj)
				: objExp;
			break;
		case 'type':
			if (!data[name]) return false;
...
function addMissingImports(ast, imports) {
	var last = -1;
	const found = {};
	...
	ast.program.body.forEach((node, i) => {
		if (types.isImportDeclaration(node)) {
			last = i;
			node.specifiers.filter(types.isImportDefaultSpecifier).forEach(specifier => {
...
exports.generateSourceCode = function (_: _,  parsed, descriptor) {
	const properties = [];
	const collect = {
		events: [],
		imports: {},
	...
	parsed.ast.comments.forEach(c => c.ignore = false);
