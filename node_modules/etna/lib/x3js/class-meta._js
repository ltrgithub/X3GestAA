...
exports.fromClassScript = (_, supervisor, script, table) => {
	...
	if (script.tableDef) {
		...
		meta.KEYINT = script.tableDef.indexes[0].columns.split(',').map(c => c.substring(1)).join('~');
	}
...
exports.loadData = (_, supervisor, script) => {
	...
	if (script.tableRef) {
		...
		// we need an index and supervisor requires that we name it.
		if (!script.tableDef.indexes || !script.tableDef.indexes.length) throw new Error(`${script.name}: tableDef has no index`);
		if (!script.tableDef.indexes[0].name) script.tableDef.indexes[0].name = "X3JS_I_" + script.name.toUpperCase();
