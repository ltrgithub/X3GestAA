...
function specialKeyToCanonical(row: types.Row, keyCode: string) {
...
export class Helper<T> {
	...
	generateSource(_: _,  parsed: codeGen.Parsed) {
...
export function fromEndpoint(_: _,  endpoint: Endpoint, descriptor: Descriptor) {
	const superv = getSupervisor(_, endpoint);
	return new Helper(superv, descriptor);
};

export function fromCurrentSession(_: _,  descriptor: Descriptor) {
	const endpoint = globals.context.session.getUserProfile(_).selectedEndpoint(_);
	return fromEndpoint(_, endpoint, descriptor);
};

export function create(superv: Supervisor, descriptor: Descriptor) {
	return new Helper(superv, descriptor);
};