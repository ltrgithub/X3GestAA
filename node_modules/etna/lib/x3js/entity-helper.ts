...
function eventName(_: _,  instance, name) {
	...
	Object.keys(descriptor.properties).forEach(k => {
		...
		if (descProp.type === 'flags') {
			...
			entity.$properties[name + 'Enabled'] = {
				...
				$title: title,
				$default: false,
				$isDisabled(_: _,  instance) {
					return !instance.name(_);
				},
				$propagate(_: _,  instance, value) {
...
entityHelper.fillEntity = function (entity, descriptor, prefix) {
	...
	entity.$init = function (_: _,  wc, context) {
...
function metaToCanonical(_: _,  superv, descriptor, meta) {
	const dest = {};
...
function canonicalToMeta(_: _,  superv, descriptor, data, parent, jsName) {
	const dest = {};
...
function fillInstance(_: _,  superv, descriptor, instance, data, prefix = '') {
...
class Helper {
	...
	saveInstanceSource(_: _,  instance) {
	...
	fillInstance(_: _,  instance, metaData, parsed) {
		var data;
		if (parsed) {
			data = parsed.data;
...
entityHelper.getSupervisor = function (_: _,  endpoint) {
...
entityHelper.fromEndpoint = function (_: _,  endpoint, descriptor) {
	const superv = entityHelper.getSupervisor(_, endpoint);
	return new Helper(superv, descriptor);
};

entityHelper.fromCurrentSession = function (_: _,  descriptor) {
