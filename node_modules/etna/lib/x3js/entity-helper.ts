...
function rawEnum(name: string) {
	...
	const enu = (systemEnums as any)[enumName];
...
const crudCodes = Object.keys(crudNames).reduce((r, k) => (r[crudNames[k]] = k, r), {} as Dict<string>);
...
function genName(name: string, obj: any, allNames: Dict<boolean>) {
			...
			switch (descProp.type) {
				...
				case 'type':
					...
					if (!typeCode) {
						...
						const metaType = superv.load<Type>(_, 'Type', typeCode);
...
export class Helper<T> {
	...
	synchronize(_: _,  context: OrmContext, opts: SynchronizeOptions) {
		...
		const userProfile = util.currentContext().session.getUserProfile(_);
		const desc = this.descriptor;
		const superv = this.supervisor;
...
export function getSupervisor(_: _,  endpoint: Endpoint) {
	...
	const conf = endpoint.getEtnaConfig(_, util.currentContext().request.session);
...
export function fromCurrentSession<T>(_: _,  descriptor: FullDescriptor<T>) {
	const endpoint = util.currentContext().session.getUserProfile(_).selectedEndpoint(_);
