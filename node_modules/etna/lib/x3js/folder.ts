...
(function () {
	if (typeof Proxy === 'undefined') {
		...
		exports.create = function(_: _,  supervisor: Supervisor): any {
			return null;
		};
		return;
	}
	...
	function fixNames(_: _,  supervisor: Supervisor, colnName: string, codeProp: string, titleProp: string, capitalize: boolean, relations: FixInfo[]) {
		const coln = supervisor.mongoStore.collection(colnName);
		...
		ezmongo.reader(coln.find({
			...
			elt._proxyName = name;
			const subNames: Dict<any> = {}; // same namespace for properties and collections!

			relations.forEach_(_, (_, rel) => {
				if (!elt[rel.name]) return;
				//console.error("RELATION", rel.name, elt[rel.name]);
				elt[rel.name].forEach_(_, (_: _, sub: any) => {
	...
	function checkContext(supervisor: Supervisor) {
		if (util.currentContext().x3frame.context.superv !== supervisor) throw new Error("internal error: supervisor mismatch");
	}
	...
	class Folder {
		...
		runUnitTest(cb: Callback<any>, file: string) {
			return flows.withContext(() => {
				require("etna/lib/supervisor/builtins/ACTX").init((err: Error) => {
					...
					require('module')._cache = Object.keys(moduleCache).reduce((r: any, k: string) => {
				...
				session: {
					...
					getUserProfile(_: _) {
						return {
							selectedEndpoint(_: _) {
								const AdminHelper = require("syracuse-collaboration/lib/helpers").AdminHelper;
								const dataset = config.unit_test && config.unit_test.x3endpoint && config.unit_test.x3endpoint.dataset;
								if (!dataset) throw new Error("unit_test.x3endpoint.dataset not configured in nodelocal.js");
								return AdminHelper.getEndpoint(_, { dataset: dataset });
							},
							selectedLocale(_: _) {
								return {
									// todo
							...
							user(_: _) {
								return {
									getEndpointLogin(_: _) {
										return "unit-test-login";
									}
								};
							},
						};
	...
	exports.create = function (_: _,  supervisor: Supervisor) {
