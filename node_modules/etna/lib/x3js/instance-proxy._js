...
class Invoker {
	...
	$property(_, name) {
		const slot = this.instance.slot(_, name);
		if (slot == null) throw new Error("property not found: " + name);
		return new PropertyHandler(this.instance, name, slot);
	}
	/// Note: instances are read via the [class manager](x3js-class-manager).
};

x3jsUtil.fiberize(Invoker.prototype);
...
function addAttributeAccessors(atb, type) {
	Object.defineProperty(PropertyHandler.prototype, atb, {
		get() {
			const slot = this.instance.ensureSlot(this.key);
...
function propertyProxy(instance, name) {
	const handler = new PropertyHandler(instance, name);
