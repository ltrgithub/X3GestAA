...
export type TableQueryOrder = Dict<number>;
...
export class TableQuery<T> {
	...
	execute(_: _): Reader<T> {
		const tables = this.tables.map_(_, (_, t) => {
			return {
				name: t.name,
				alias: t.alias || t.name,
				meta: t.meta || this.supervisor.load<Table>(_, 'Table', t.name)
			};
		});
		...
		return this.supervisor.sqlReader<T>(_, sql, args, sqlOpts).map((_, rec) => mapRecord(_, rec));
	}
}
