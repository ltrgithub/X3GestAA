...
class Invoker {
	...
	/// ## Properties
	/// 
	/// * `instance.$classes`: [class manager](x3js-class-manager) for `instance`. 
	get $classes() {
		return this.instance.supervisor.api.classes;
	}
	/// * `instance.$tables`: [table manager](x3js-table-manager) for `instance`. 
	get $tables() {
		return this.instance.supervisor.api.tables;
	}
	/// * `instance.$cache`: [cache manager](x3js-cache-manager) for `instance`. 
	get $cache() {
		return this.instance.supervisor.api.cache;
	}
	/// * `instance.$context`: [context manager](x3js-context-manager) for `instance`. 
	get $context() {
	...
	/// * `instance.$snapshot`: `instance`'s snapshot. 
	get $snapshot() {
		return instanceProxy.create(this.instance.snapshot());
	}
	/// * `instance.$status`: `instance`'s modification status, one of 'original', 'modified', 'new', 'deleted'
	get $status() {
	...
	/// 
	/// * `instance.$error(_, text)`:  adds an error to `instance`.
	$error(_, text) {
		this.instance.ASETERROR(_, this.args.CURPTH, text, constants.CST_AERROR);
	}
	/// * `instance.$warn(_, text)`:  adds a warning to `instance`.
	$warn(_, text) {
		this.instance.ASETERROR(_, this.args.CURPTH, text, constants.CST_AWARNING);
	}
	/// * `instance.$info(_, text)`:  adds an information message to `instance`.
	$info(_, text) {
	...
	$save(_) {
		if (instance.$astalin === constants.CST_ANEW)
			return this.instance.$AINSERT(_);
		else
...
class PropertyHandler {
	...
	/// 
	/// * `val = instance.prop$`: gets the value of `instance`'s property _prop_.
	get(_) {
	...
	/// 
	/// * `val = instance.prop.error(_, text)`: adds error `text` to `instance`'s property _prop_.
	error(_, text) {
		this.instance.ASETERROR(_, this.path, text, constants.CST_AERROR);
	}
	/// * `val = instance.prop.warn(_, text)`: adds warning `text` to `instance`'s property _prop_.
	warn(_, text) {
		this.instance.ASETERROR(_, this.path, text, constants.CST_AWARNING);
	}
	/// * `val = instance.prop.info(_, text)`: adds info `text` to `instance`'s property _prop_.
	info(_, text) {
