...
import { Type } from '../supervisor/meta/types';
import { Diagnosis } from '../supervisor/dyn/types';
import { OrmInstance, OrmInstanceMethods, OrmInstanceProperties } from './orm';
...
export function toJs(val: any, TYPTYP: number, NOLIB?: number) {
	if (val == null) return val;
	switch (TYPTYP) {
		case 1:
			// menu
			if (NOLIB === 1) return NFT[val];
			else {
		...
		default:
			throw new Error("invalid type code: " + TYPTYP);
	}
};
...
export function fromJs(val: any, typtyp: number, NOLIB: number) {
	switch (typtyp) {
		case 1:
			// menu
			if (NOLIB == null && typeof val === 'boolean') {
				// we lack type enum vs. boolean info on X3 side, so we rely on JS type.
				return val ? 2 : 1;
			} else if (NOLIB === 1) {
		...
		case 3:
			// integers
			if ((val | 0) as any === val) return val;
...
export const x3jsTypeIds = Object.keys(x3jsTypeNames).reduce((r, id) => (r[x3jsTypeNames[+id]] = +id, r), {} as Dict<number>);
...
export function wait<T>(fn: (_: _) => T): T {
	const fiberized = (fn as any)['fiberized-0'];
...
export function currentSupervisor(_: _): supervisor.Supervisor {
	const session = util.currentContext().session;
