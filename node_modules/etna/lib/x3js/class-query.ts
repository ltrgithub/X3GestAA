...
import { ColumnDesc } from '../supervisor/meta/column';
...
export class ClassQuery {
	...
	execute() {
		return x3jsUtil.wait(_ => {
			...
			const recordMapper = (_: _, record: types.Row) => instanceProxy.create(clas.supervisor.new(_, 'Instance', clas).afterCreate(_, record));
			...
			if (clas.data.ACCSTR) {
				...
				params.push(util.currentSession().userName);
