...
import * as readerProxy from './reader-proxy';
...
export class ClassQuery {
	...
	execute(_: _) {
		...
		const recordMapper = (_, record) => instanceProxy.create(clas.supervisor.new(_, 'Instance', clas).afterCreate(_, record));
		...
		const reader = this.class.supervisor.sqlReader(_, sql, params, sqlOpts).map(recordMapper);
...
ClassQuery.prototype = x3jsUtil.fiberize(ClassQuery.prototype);
