...
import * as classModule from '../supervisor/meta/class';
import * as constants from '../supervisor/constants';
import { Property } from '../supervisor/meta/property';
import { BaseClass } from '../supervisor/meta/baseClass';
import { Instance } from '../supervisor/dyn/instance';
import { Slot } from '../supervisor/dyn/slot';
import { SlotCollection } from '../supervisor/dyn/slotCollection';
import * as collectionProxy from './collection-proxy';
import * as attributes from '../supervisor/attributes';

import { PropertyMeta } from '../supervisor/metadata/class';
 
type Dict<T> = { [name: string]: T; };

const instructions = {
	TRBEGIN: db.instructions.TRBEGIN(),
	COMMIT: db.instructions.COMMIT(),
	ROLLBACK: db.instructions.ROLLBACK()
};
...
function instanceError(message: string, instance: Instance) {
	const error = new Error(message);
	// TODO: introduce proper error class with diagnoses
	util.any(error).diagnoses = instance.diagnoses;
	return error;
}
...
class InstanceHandler {
	...
	get $parent() {
		return create(this.instance.APARENT);
	}
	/// * `instance.$snapshot`: the instance's snapshot.
	get $snapshot() {
		return this.instance._insideInit ? null: create(this.instance.snapshot());
	}
	...
	$data(full: Boolean) {
		return x3jsUtil.wait(_ => {
			return Object.keys(this.instance.slots).reduce_(_, (_: _, r: any, name: string) => {
				const slot = this.instance.slots[name];
				if (slot instanceof SlotCollection) {
					const colnSlot: any = slot;
					if (full) r[name] = colnSlot.lines.map_(_, (_: _, child: Instance) => new InstanceHandler(child).$data(full));
				} else {
					if (slot.property == null) throw new Error("invalid slot: no property");
					r[name] = x3jsUtil.toJs(slot.value, slot.property.type.data.TYPTYP, slot.property.data.NOLIB);
				}
				return r;
			}, {});
		});
...
const sysProperties: Dict<PropertyMeta> = classModule.systemProperties.reduce((names, prop) => {
...
class PropertyHandler {
	...
	_path: string;
	jsName: string;
	constructor(instance: Instance, prop: Property, slot: Slot) {
		this.instance = instance;
		this.prop = prop;
		this.slot = slot;
		this._path = null;
...
/// * `title`: string
/// * `description`: string
function addAttributeAccessors(atb: string, type: string) {
...
function propertyProxy(instance: Instance, prop: Property, slot: Slot) {
	const handler = new PropertyHandler(instance, prop, slot);
	return new Proxy(handler, {
		get(handler, name) {
			if (typeof name !== 'string') throw new Error('invalid proxy access: ' + typeof name);
			else {
				if (!(name in PropertyHandler.prototype)) throw new Error("invalid property method or attribute: " + name);
				const member = util.any(handler)[name];
...
function lookupJsMembers(instance: Instance, key: string, name: string) {
	const bases: any[] = instance.APARENT ? lookupJsMembers(instance.APARENT, 'relations', instance.$parent.proJsName) // instance.$parent is the collection!
...
class ClassHandler {
	...
	constructor(clas: BaseClass) {
		this.class = clas;
	}
	get(instance: Instance, name: string): any {
		return x3jsUtil.wait(_ => {
			//console.error("PROPERTY GET", this.clas.name, name);
			const prop = this.class.membersByJsName[name];
			if (prop) {
				const slot = instance.slot(_, prop.name);
				const val = slot.get(_);
				if (util.isCollection(val)) return collectionProxy.create(val);
				else return x3jsUtil.toJs(val, slot.property.type.data.TYPTYP, slot.property.data.NOLIB);
			}
			const methods = lookupJsMembers(instance, 'methods', name);
			if (methods.length > 0) return methods.pop().bind(create(instance)); // todo: reuse proxy
			if (typeof name === 'symbol' || /^(inspect|toString|stack)$/.test(name)) {
				const anyInstance = util.any(instance);
				return typeof anyInstance[name] === 'function' ? anyInstance[name].bind(instance) : anyInstance[name];				
			}
			throw new Error("member not found: " + this.class.name + "." + name);
		});
	}
	set(instance: Instance, name: string, val: any) {
		return x3jsUtil.wait(_ => {
			//console.error("PROPERTY SET", this.clas.name, name, val);
			const prop = this.class.membersByJsName[name];
			...
			try {
				slot.set(_, x3jsUtil.fromJs(val, slot.property.type.data.TYPTYP, slot.property.data.NOLIB));
			} finally {
...
// internal
export function create(instance: Instance) {
	...
	return new Proxy(instance, {
		get(instance, name) {
			if (typeof name !== 'string') throw new Error('invalid proxy access: ' + typeof name);
				...
				if (name[0] === '$') {
					...
					var member = util.any(instanceHandler)[name];
		...
		ownKeys(instance) {
			instance.meta.x3jsHandler = instance.meta.x3jsHandler || new ClassHandler(instance.meta);
			return instance.meta.x3jsHandler.ownKeys();
		}
	});
};
