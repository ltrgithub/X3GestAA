...
function instanceError(message: string, instance: Instance) {
	...
	(error as any).diagnoses = instance.diagnoses;
	return error;
}
...
class InstanceHandler {
	...
	$user() {
		return x3jsUtil.wait(_ => {
			return util.currentContext().x3session.actx.$USER(_);
		});
	}
	...
	return new Proxy(handler, {
		get(handler, name) {
			if (typeof name !== 'string') throw new Error('invalid proxy access: ' + typeof name);
				...
				const member = (handler as any)[name];
		...
		get(instance, name) {
			if (typeof name !== 'string') throw new Error('invalid proxy access: ' + typeof name);
				...
				if (name[0] === '$') {
					...
					var member = (instanceHandler as any)[name];
