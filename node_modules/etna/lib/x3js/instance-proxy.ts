...
InstanceHandler.prototype = x3jsUtil.fiberize(InstanceHandler.prototype);
...
PropertyHandler.prototype = x3jsUtil.fiberize(PropertyHandler.prototype);
...
class ClassHandler {
	...
	get(_: _,  instance, name) {
		...
		if (typeof name === 'symbol' || /^(inspect|toString|stack)$/.test(name)) 
			return typeof instance[name] === 'function' ? instance[name].bind(instance) : instance[name];
		throw new Error("member not found: " + this.class.name + "." + name);

	}
	set(_: _,  instance, name, val) {
...
ClassHandler.prototype = x3jsUtil.fiberize(ClassHandler.prototype);
