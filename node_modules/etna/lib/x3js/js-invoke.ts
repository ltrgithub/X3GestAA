...
import { etnaType2TypTyp } from '../supervisor/meta/type';
...
function labelValue(label: string, args: types.ProgArgs) {
	switch (label) {
		case 'properties':
...
export default function (_: _,  def: any, label: string, args: types.ProgArgs) {
	...
	const arg1 = args._ANEWVAL !== undefined && util.any(args).member ? x3jsUtil.toJs(args._ANEWVAL, util.any(args).member) : undefined;
		const supervisor = util.currentFrame().context.superv;
	//try {
		const jsKeys = Object.keys(args).filter(k => args[k] && args[k].isParam);
		const jsArgs = jsKeys.map(k => {
			const arg = args[k];
			return x3jsUtil.toJs(arg.value, supervisor.new(_, 'Type', { TYPTYP: arg.typ }));
		});
		...
		if (args.ARET_VALUE && result !== undefined) {
			...
			args.ARET_VALUE.value = x3jsUtil.fromJs(result, supervisor.new(_, 'Type', { TYPTYP: typ }));
			return undefined;
		} else {
			// ??
			return result;
