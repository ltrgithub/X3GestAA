...
import { Column, Table } from '../supervisor/meta/types';
...
class TableHandler {
	...
	table(_: _) {
		...
		const supervisor = util.currentContext().x3session.supervisor;
		...
		this._handle = supervisor.load<Table>(_, 'Table', segs[1]);
	...
	readRecord<T extends Record>(_: _,  key: any, opts?: TableReadOptions) {
		...
		if (rec) {
			...
			Object.keys(rec).forEach_(_, (_, name) => {
				(rec as any)[name] = table.columns[name].type.fromSql(_, (rec as any)[name]);
			});
