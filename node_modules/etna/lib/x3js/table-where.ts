...
function getColumn(converter, key) {
	const column = converter.findColumn(key);
	if (column.meta.dim !== 1) throw new Error("cannot compare on column " + key + ": dim=" + column.dim);
	return {
...
function convertLiteral(converter, str) {
...
function comparisonOp(sqlOp) {
	return function (_: _,  converter, key, exp) {
...
export class WhereConverter {
	...
	toSql(_: _,  cond) {
		...
		if (Array.isArray(cond)) return ops1.$and(_, this, cond);
		return ops1.$and(_, this, Object.keys(cond).map_(_, (_, k) => {
			if (k[0] === '$') {
				...
				if (arg != null && typeof arg === 'object') {
					return ops1.$and(_, this, Object.keys(arg).map_(_, (_, kk) => {
