...
class TableWrapper {
	...
	query(alias) {
		return new TableQuery(this.table, alias);
	}
...
class TablesHandler {
	...
	get(name) {
		if (name[0] === '$') throw new Error("invalid table name: " + name);
		return this.getters[name] || { get : _ => {
			const table = this.supervisor.load(_, 'Table', name);
			if (table == null) throw new Error("table not found: " + name);
