...
export function define(def: MessagesDef) {
	...
	return proxies[key] || (proxies[key] = new Proxy(new MessageHandler(def), {
		get(handler, name) {
			if (typeof name !== 'string') throw new Error('invalid proxy access: ' + typeof name);
				...
				return (...args: any[]) => {
					const fmt: string = x3jsUtil.wait(_ => flowControl.functions.MESS.fn(_, id, def.dictId, 1));
...
export function legacyMessage(chapter: number, id: number) {
	return x3jsUtil.wait(_ => flowControl.functions.MESS.fn(_, chapter, id, 1));
}