...
import { ClassMeta, PropertyMeta } from '../supervisor/metadata/class';
...
export interface PropertyEventDef {
}
export interface PropertyEventsDef {
	get: PropertyEventDef;
}

export interface PropertyDef {
	can: string;
	events: PropertyEventsDef;
}
...
export interface TableDefDef {
	name: string;
	is: string;
	indexes: IndexDef[];
}

export interface TableRefDef {
	name: string;
}
...
export interface ClassDef {
	module: string;
	name: string;
	properties: { [name: string]: PropertyDef };
	tableDef: TableDefDef;
	tableRef: TableRefDef;
	scripts: ScriptDef[];
}

export const descriptor: FullDescriptor<ClassMeta> = {
	type: 'object',
	...
	collections: {
		properties: {
			...
			dictIgnore(meta: PropertyMeta) {
				return x3jsUtil.isSysProperty(meta.FLDCLA);
			},
			orderCode: 'NUMLIG',
			dictFinish(meta: PropertyMeta) {
			...
			collections: {
				parameters: {
					...
					// enum 59: property: 1, displayed property: 2, link: 3
					properties: {
						dictCode: {
							type: 'string',
							dictCode: 'CODPAR'
