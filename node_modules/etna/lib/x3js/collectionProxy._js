...
/// # x3js collection
/// 
/// A collection holds a list of child instances, for example the lines of a sales order instances.
///
...
const methods = {
	...
	/// 
	/// * `coln.reader()`: returns a new reader on the collection.  
	///   The reader implements the `ez-streams` reader API and can be used to filter, transform and iterate on 
	///   collection elements.
	reader() {
		var i = 0;
		return ez.devices.generic.reader(
			(_) => i < this.length ? this.index(_, ++i) : undefined, 
			(_) => i = 0);
	},

	/// * `coln.toArray(_)`: returns the collection's elements as an array of instances.
	toArray(_) {
		return this.reader().toArray(_);
	},
	...
	return new Proxy(collection, {
		get(collection, name) {
			if (name === 'length') {
				...
				var i = parseInt(name);
				if (!isNaN(i)) 
					return x3jsUtil.toJs(collection.lines[i]);
					...
					return ez.devices.generic.empty.reader[name].bind(methods.reader.call(collection));
				else
