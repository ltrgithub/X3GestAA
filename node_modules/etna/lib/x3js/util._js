...
exports.x3jsType = function(type, nolib) {
	const result = {
	...
	switch (type.data.TYPTYP) {
		case 1:
			...
			break;
		case 4:
			Object.assign(result, type.precisionAndScale());
			break;
		case 7:
...
exports.controlCamelCase = function(propName) {
	return function(_, instance, value) {
		if (value && !/^[a-z_]\w*$/.test(value)) {
			instance.$addError(`invalid property name: camelCase expected`, propName);
		}
	};
};
...
exports.controlPascalCase = function(propName) {
	return function(_, instance, value) {
		if (value && (!/^[A-Z]\w+$/.test(value) || value === value.toUpperCase())) {
			instance.$addError(`invalid class name: PascalCase expected`, propName);
		}
	};
};
