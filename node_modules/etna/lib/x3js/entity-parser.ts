			...
			switch (desc.type) {
				...
				case 'type':
					...
					if (enumProp) {
						...
						if (!/TODO_/.test(enumProp.value.name))
				...
				case 'object':
					...
					dest[name] = Object.keys(desc.properties).reduce((obj, key) => {
						...
						if (node && !n) {
							if (desc.properties[key].isMandatory) {
								//throw new Error(`mandatory attribute missing: ${key}`);
								// TODO: warnings only go to console for now - improve later
								console.error(`mandatory attribute missing: ${key}`);
							}
						}
