	...
	parse(desc, dest, parent, name, node) {
		...
		try {
			switch (desc.type) {
				...
				case 'object':
					let result;
					if (node && desc.codeGen) {
						[node, result] = specials[desc.codeGen](types, node);
						if (result != null) {
							dest[name] = result;
							break;
						}
...
exports.parseFile = function (_: _,  filePath, descriptor) {
	try {
...
exports.parseSource = function (_: _,  source, filePath, descriptor) {
	...
	traverse(ast, {
		ImportDeclaration(path) {
			...
			path.node.specifiers.filter(types.isImportDefaultSpecifier).forEach(specifier => {
...
exports.parseSourceFiles = function (_: _,  subdir, descriptor, superv, tracker) {
