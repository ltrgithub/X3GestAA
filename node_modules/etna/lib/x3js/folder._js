...
(function() {
	...
	class Folder {
		...
		runUnitTest(cb, file) {
			...
			mocha.addFile(file);
			if (require.cache[file]) throw new Error(`unit test is already running - retry later: ${file}`);
			mocha.run().on('end', function() {
				// clear require cache entry so that we can run the test again
				if (!require.cache[file]) throw new Error(`cannot reset unit test require cache entry: ${file}`);
				delete require.cache[file];
				cb(null, this.testResults);				
			});
		}
	}
