...
import * as bcd from '../engine/runtime/tbcd';
...
import * as uuid from '../engine/runtime/tuuid';
...
function format(t: string, v: any) {
...
export function parse(_: _,  s: string) {

	function extractVar(c: string) {
			...
			switch (t) {
				...
				case "O":
					...
					if (clas.length > 0) {
						...
						var cl = frame.context.superv.load<Class>(_, 'Class', clas.substring(2));
...
export function stringify(_: _,  v: any) {
	var s = "";
	if (Array.isArray(v)) {
		if (!v.length) return "[];";
		var t = x3sonLetter(v[0]);
		s = t + "[" + v.map_(_, (_: _, e: any) => e.toX3son ? e.toX3son(_) : len(t, e) + format(t, e)).join('') + "];";
