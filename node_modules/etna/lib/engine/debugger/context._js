...
const tracer = require('syracuse-core').getTracer("etna.debugger");
const log = {
	info(s) {
...
function DebugContext(socket) {
	...
	this.write = function (_: _,  data, opts) {
		opts = opts || {};
		if (opts.json) {
...
const _commandsMap = {
	...
	// step_into
	step_into(_: _,  resp, options) {
		this.leave = true;
		this.setStatus(resp);
	},

	// step_over
	step_over(_: _,  resp, options) {
		// this.leave = true;
		this.markFrame();
		this.setStatus(resp, "break");
		this.status = "running";
	...
	// stop
	stop: tmp = function (_: _,  resp, options) {
		this.leave = true;
	...
	breakpoint_set(_: _,  resp, options) {
	...
	//     </breakpoint>
	// </response>	
	breakpoint_get(_: _,  resp, options) {
	...
	// -h	HIT_VALUE
	// -o	HIT_CONDITION	
	breakpoint_update(_: _,  resp, options) {
		const bp = this.getBreakpointById(options.d);
	...
	// breakpoint_remove
	// -d BREAKPOINT_ID	the unique session breakpoint id returned by breakpoint_set.
	breakpoint_remove(_: _,  resp, options) {
		const bp = this.getBreakpointById(options.d);
	...
	// breakpoint_list
	breakpoint_list(_: _,  resp, options) {
	...
	// context_names
	// -d	stack depth (optional)
	context_names(_: _,  resp, options) {
		resp.context = [{
			name: "Local",
			id: 0
		}
	...
	stdout(_: _,  resp, options) {
		resp.$.success = 1;
	},

	// stderr
	// -c	[0|1|2] 0 - disable, 1 - copy data, 2 - redirection
	stderr(_: _,  resp, options) {
		resp.$.success = 1;
	},
	...
	// stdin
	stdin(_: _,  resp, options) {
		resp.$.success = 1;
	},

	// break
	break(_: _,  resp, options) {
		throw new Error("NIY");
	},

	// eval
	eval(_: _,  resp, options) {
		var expr = options["-"],
		    frame = this.frame;
...
function _getProperty(_: _,  parents, name, value, context) {
	parents = parents || [];
	...
	if (type === "Array") {
		...
		prop.property = value.map_(_, (_, e, i) => _getProperty(_, descendents, i, e, context));
