...
function handleError(_, frame, e) {
...
exports.instructions = {
	...
	ONERRGO(ns, label) {
		const target = label && exports.instructions.GOSUB(ns, label);
	...
	FORV(varName, from, to, step, p, end, block) {
		return function forv$do(_) {
			...
			const condition = stepI >= 0 ? function () {
				return idx <= toI && frame.loopLevel === level;
			} : function () {
				return idx >= toI && frame.loopLevel === level;
			};

			const setIndex = (field => {
				const setValue = getSetter(field);
