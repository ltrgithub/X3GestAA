...
exports.functions = {
	...
	CLAVAR: {
		...
		fn: function(classe, n) {
			var i = 1;
			var name = "";
			Object.keys(classe.types).forEach(function(field) {
				if (i === n) name = field;
				i++;
			});
			return name;
...
function rowIn(_, row, descs) {
	return descs.reduce_(_, function(_, obj, desc) {
...
var sqlFunctions = {
	...
	LEN: function() {
		...
		thesql += ("(" + Array.prototype.map.call(arguments, function(arg) {
			return arg.sql;
		}).join(',') + ")");
		return {
			sql: thesql,
...
exports.instructions = {
	...
	SQLDECL: function(type, name, ranges) {
	...
	VTBCV: function(letter, abbrev, field, silent) {
	...
	FILEVAL: function(abb, vars, frv, where, order) {
		return function dot$do(_) {
			...
			var tbvar = frv.map_(_, function(_, e) {return e(_, 1); });
	...
	COLUMNS: function(abbrev, columns, ext) {
		return function columns$do(_) {
			...
			if (columns) {
				...
				table.columns = columns.reduce_(_, function(_, r, citem) {
	...
	REWRITEBYKEY: function(key, op, values) {
		var avalues = util.instructions.A(values);
