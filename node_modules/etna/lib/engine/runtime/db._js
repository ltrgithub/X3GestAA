...
var DBTableVAR = require('./dbtablevar').DBTableVAR;
...
exports.functions = {
	...
	FILEABRE: {
		...
		fn(n) {
			...
			var tb = (frame.context.tableSlots) ? frame.context.tableSlots.get(n) : undefined;
	...
	CLAVAR: {
		...
		fn(classe, n) {
			var i = 1;
			var name = "";
			Object.keys(classe.types).forEach((field) => {
				if (i === n) name = field;
				i++;
			});
			return name;
...
function binaryOp(op, sqlOp, type, numOp, numType) {
	return function(op1, op2) {
		return function binaryOp$do(_) {
			...
			var colu = (unary) ? (v1.columns || []) : (v1.columns || []).concat(v2.columns || []);
			return {
				sql: sql,
				type: type,
				constants: vals,
...
function _closeFile(frame, abbrev) {
	...
	if (frame.context.tables[abbrev].inFrontOf) {
		...
		if (frame.context.gclass[abbrev].inFrontOf)
			frame.context.gclass[abbrev] = frame.context.gclass[abbrev].inFrontOf;
	} else {
...
function sqlFn(name, type) {
	return function() {
		var thesql = name + "(" + Array.prototype.map.call(arguments, (arg) => {
			return arg.sql;
		}).join(',') + ")";
		return {
			sql: thesql,
...
var sqlFunctions = {
	...
	LEN() {
		...
		thesql += ("(" + Array.prototype.map.call(arguments, (arg) => {
			return arg.sql;
		}).join(',') + ")");
		return {
			sql: thesql,
	...
	*/
	...
	FIND(arg) {
		...
		return {
			sql: "(" + arg.sql + not + " IN (" + vals.map((val) => {
				return val.sql;
			}).join(',') + "))",
			type: "number",
		};
...
exports.instructions = {
	...
	NBRECORD(abbrev) {
		return function nbrecord$do(_) {
			...
			var res = frame.context.superv.executeSql(_, qry, [], {
				typesmap: [3],
				dispatch: true,
			});
	...
	KEY(letter, abbrev, name, indices) {
		return function key$do(_) {
			return {
				...
				abbrev: abbrev,
				name: name,
				indices: indices && indices.map_(_, (_, exp) => {
					return exp(_);
				})
			};
		};
	...
	FORSQL(fsql, abv, vars, p, end, block) {
		return function forsql$do(_) {
			...
			var fieldList = []; /* declare field */
			for (var i = 0; i < vars.length; i++) {
				fieldList[i] = vars[i](_, abv);
			}

			var cf = table.x3OpenFor(_, fieldList);
	...
	FILEVAL(abb, vars, frv, where, order) {
		return function dot$do(_) {
			...
			var nslt = frame.context.tableSlots.getSlot();

			// extracting values
			var tbvalues = util.instructions.A(frv)(_);
			// extracting table var
			var tbvar = frv.map_(_, (_, e) => {return e(_, 1); });

			if (tbvar.length !== vars.length) throw new X3Error(69, abb) ;

			var table = new DBTableVAR(abb, frame.context.superv, frame.stackLevel, tbvar, tbvalues );
			var gclas = new GClass(table);
	...
	FILESQL(abbrev, name, where, order) {
		return function file$do(_) {
			...
			var ctx = {};
			if (frame.context.tables[abb] && frame.context.tables[abb].local === frame.stackLevel)
				throw new X3Error(28, abb);
			...
			// init keys
			var kslot = -1;
			var tkeys = Object.keys(desc.indexes).map((name) => {
			...
			// where
			var table = new DBTable(driver, abb, frame.context.superv, desc, tkeys, frame.stackLevel).init(_);
			var gclas = new GClass(table);
	...
	LINK(abbrev, as, joins, where, order) {
		return function link$do(_) {
			...
			lktable.joins = joins.map_(_, (_, jitm) => {
				return jitm(_, ctx);
			});
	...
	LOOK(key, op, values, lock, hint) {
		...
		return function look$do(_) {
			...
			var fst = ka.table.x3Read(_, ka.nkey, ka.kln, ka.wkey, ka.krevert, lock, hint);
	...
	READ(key, op, values, lock, hint) {
		...
		return function read$do(_) {
			...
			var fst = ka.table.x3Read(_, ka.nkey, ka.kln, ka.wkey, ka.krevert, lock, hnt);
			if ((fst === 0) && ((op === 'GE') || (op === 'LE'))) {
				if (ka.table.keyCompare(_, ka.nkey, vals) === 0) frame.context.sys.values.FSTAT = 0;
				else frame.context.sys.values.FSTAT = 2;
	...
	REWRITE(key, op, values) {
		...
		return function rewrite$do(_) {
			...
			// ----------------------------------------------------------
			var vassig = [];
			var abv = (ka.table.master || ka.table).abbrev;
			var lst = ka.table.columns && ka.table.columns[abv];
	...
	DELETE(key, where, op, values) {
		...
		return function delete$do(_) {
			...
			var rs = ka.table.x3Delete(_, ka.nkey, ka.wkey, where);
	...
	UPDATE(abbrev, where, assign) {
		return function update$do(_) {
			...
			var consts = [];
			var asssql = "" + assign.map_(_, (_, ass) => {
				var asg = ass(_, ctx);
				consts = consts.concat(asg.constants || []);
				return asg.sql;
			}).join(",");

			var rs = table.x3Update(_, where, asssql, consts);
