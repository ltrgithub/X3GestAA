...
function sqlFn(name, type) {
	return function() {
		var thesql = name + "(" + Array.prototype.map.call(arguments, arg => arg.sql).join(',') + ")";
		return {
			sql: thesql,
			type: type,
			tooComplex: false
...
var sqlFunctions = {
	...
	LEN() {
		...
		thesql += ("(" + Array.prototype.map.call(arguments, arg => arg.sql).join(',') + ")");
		return {
			sql: thesql,
			type: "number",
			tooComplex: false
...
exports.instructions = {
	...
	FILEVAL(abb, vars, frv, where, order) {
		return function dot$do(_) {
			...
			var tbvar = frv.map_(_, (_, e) => e(_, 1));
