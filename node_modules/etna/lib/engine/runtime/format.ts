...
import { x3Trim } from '../../engine/runtime/util';
import * as util from './util';
const locale = require('streamline-locale');
const resources = locale.resources(module);

function _isLower(ch: string) {
	return ch >= 'a' && ch <= 'z';
}

function _isUpper(ch: string) {
	return ch >= 'A' && ch <= 'Z' || ch == ' ';
}
...
function _isHexa(ch: string) {
	return ch >= 'A' && ch <= 'F' || ch >= 'a' && ch <= 'f' || ch >= '0' && ch <= '9';
}

function _fmtInt(val: number, rep: number) {
	var s = val.toString();
...
function _monthName(month: number, rep: number) {
...
interface KFormatters {
	[letter: string]:(vch: string) => string;
}

export function format($fmt: string, $val: any) {
		...
		K() {
			...
			for (; rep > 0; rep--) {
				...
				if (fk) vch = fk.call(this, vch);
					...
					} else throw new errors.X3Error(5, $fmt + " (215) bad format character: " + ch);
				}
			...
			switch (ch) {
				case '#':
					if (cur === integ) {
						...
						rep += n3;
						if (cur.length < rep) {
							var fill: string = options['*'] ? special[5] : options['0'] ? '0' : ' ';
							if (fill === '0' && cur[0] === '-') {
								result += '-';
								cur = cur.substring(1);
								rep--;
						...
						throw new errors.X3Error(5, "too many # directives in format");
					}
					break;
				case '.':
					result += special[3];
				...
				default:
					throw new errors.X3Error(5, $fmt + " (271) bad format character: " + ch);
			}
		},
		D() {
			...
			switch (ch) {
				...
				case 's':
					...
					break;
				default:
					throw new errors.X3Error(5, $fmt + ": (309) bad format character: " + ch);
			}
		},
		L() {
			result += "?";
...
interface MapFormat {
	K: { [letter2: string]: string };
	[letter1: string]: { [letter2: string]: string };
}
...
export function fromX3FormatToRegex(format: string) {
	const parts = format.split(':');
	const map = mapFormat[parts[0][0]];
	const values = util.currentFrame().context.sys.values;
	const adxtul = values.ADXTUL ? values.ADXTUL[2] : "";
	const adxtuc = values.ADXTUC ? values.ADXTUC[2] : "";
	const adxtut = values.ADXTUT ? values.ADXTUT[2] : "";
