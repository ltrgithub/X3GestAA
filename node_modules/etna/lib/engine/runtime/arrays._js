...
function getOneDimNbValues(arr) {
	//sub or not
...
const _isSliced = exports.isSliced = function (arr) {
		...
		return function index$do(_, assign, val) {
			...
			if (Array.isArray(targetVal)) {
				...
				if (assign) {
					...
					if (val !== undefined) {
						if (vinfo.vtype === 'C') throw new X3Error(62, frame.prog.name + ": '" + field + "' Variable not modifiable");
		...
		return function sorta$do(_) {
			...
			function compare(i, j) {
				...
				for (var k = 0; k < orderDepth && !delta; k++) {
					...
					else if (util.x3IsDate(elt1) || util.x3IsBCD(elt1)) delta = elt1.x3Compare(elt2);else throw util.badArgumentType(elt1);
				}
				return delta * dirVal;
			}
