...
import { ContextFn, Expression } from './types';
...
const builders: { [name: string]: (Node: nodes.ParseNode) => ContextFn<any>; } = {
	STRING: buildLiteral,
	...
	OP(node: nodes.ParseOpNode) {
		return (db.instructions as any)["SQL" + node.op].apply(null, node.args.map(_build));
	},
...
export const instructions = {
	EVALUE(req: (name: string) => any, ns: any, arg: Expression<any>) {
