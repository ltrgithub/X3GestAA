...
class X3path {
	constructor(_, superv, dir, nam, ext, app, vol, hst) {
		...
		if (self.directory === undefined && dir !== "") self.directory = dir;
		...
		if (self.volume === undefined && !fca) {
			...
			if (p >= 0) {
				...
				app = app.substr(p + 1);
			} else if (vol !== "") self.volume = vol;
		}

		// application
		if (self.application === undefined && !fca) self.application = app;

		// extention
		if (self.extention === undefined && ext !== "") self.extention = ext;
		...
		if (self.application === "") self.application = superv.folderName;
	...
	getFolderVolumes(superv) {
		...
		const vol = dir.reduce(function (vol, obj) {
			if (obj.volume) {
