...
export class DBTableSQL extends DBTableParent {
	query: string;
	constructor(driver: types.Driver, abbrev: string, superv: Supervisor, loc: number, qry: string) {
	...
	x3OpenFor(_: _,  fieldList: types.SqlColumnDesc[]) {
		/* query describe  */

		const desc = this.driver.describeQuery(_, this.query, this.superv.folderName);
		const descobj: types.SqlColumnsDesc = {};
