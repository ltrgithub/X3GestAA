...
import { Frame, Expression } from './types';
...
function findFile(frame: Frame, abbrev: string) {
...
function closeFile(frame: Frame, abbrev: string) {
	if (abbrev === "") {
...
function findFileS(frame: Frame, abbrev: string) {
	var file: File;
...
function findFileW(frame: Frame, abbrev: string) {
	var file: File;
...
function findFileR(frame: Frame, abbrev: string) {
	var file: File;
	if (abbrev === "") file = frame.context.files["###"] || frame.context.files["#"];
	else file = frame.context.files[abbrev];
...
export const instructions = {
	OPEN(abbrev: string, name: Expression<string>, mode: string, seek: Expression<number>) {
	...
	IOMODE(abbrev: string, mode: string, value: Expression<string | number>) {
		...
		return function iomode$do(_: _) {
			...
			(file as any)[mode] = val === "" || val === 0 ? undefined : val;
		};
	},
	SEEK(abbrev: string, mode: string, _direction: Expression<number> | string, value: Expression<number> | string) {
	...
	WRSEQ(abbrev: string, exps: Expression<any>) {
		return function wrseq$do(_: _) {
			...
			if (vals[vals.length - 1] !== null) newline = true;
			else {
				vals.pop();
				newline = false;
			}
	...
	RDSEQ(abbrev: string, vars: Expression<any>[]) {
		return function rdseq$do(_: _) {
			...
			if (vals.length === 0) {
				util.currentContext().x3frame.context.sys.values.FSTAT = 1;
				while (vars[i]) {
					const f = vars[i++];
				...
				return 0;
			} else util.currentContext().x3frame.context.sys.values.FSTAT = 0;
			...
			while (vars[i]) {
				...
				if (Array.isArray(s)) {
					...
					for (var j = min; j <= max; j++) {
						const fidx = arrays.instructions.INDEX(vars[i - 1], [basic.instructions.C(j)]);
						fidx(_, true, vals[index++]);
						if (index >= vals.length) break;
					}
				} else {

					f(_, variables.vOperations.SET, vals[index++], false);
					if (index >= vals.length) break;
				}
			}
			return 0;
	...
	PUTSEQ(abbrev: string, nbr: Expression<number>, vars: Expression<any>[]) {
		return function putseq$do(_: _) {
			...
			while (vars[i]) {
				...
				const inf = f(_, variables.vOperations.GETINFO);
	...
	GETSEQ(abbrev: string, nbr: number, vars: Expression<any>[], sized: Expression<number>) {
		return function putseq$do(_: _) {
			const frame = util.currentContext().x3frame;
			const file = findFileR(frame, abbrev);
			const size: number[] = [];
			...
			while (vars[i]) {
				...
				if (Array.isArray(v)) {
					...
					for (var j = min; j <= max; j++) {
						...
						var fidx = arrays.instructions.INDEX(vars[i - 1], [basic.instructions.C(j)]);
						fidx(_, true, val);
						if (++n >= nbr) break;
					}
					if (n >= nbr) break;
					...
					// set value
					f(_, variables.vOperations.SET, val, false);
					if (++n >= nbr) break;
				}
			}
			util.currentContext().x3frame.context.sys.values.FSTAT = 0;
			return 0;
		};
	}
};