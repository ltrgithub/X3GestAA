...
export const instructions = {
	OPEN(abbrev: string, name: types.Expression<string>, mode: string, seek: types.Expression<number>) {
	...
	IOMODE(abbrev: string, mode: string, value: types.Expression<string | number>) {
		...
		return function iomode$do(_: _) {
			...
			util.any(file)[mode] = val === "" || val === 0 ? undefined : val;
		};
	},
	SEEK(abbrev: string, mode: string, _direction: types.Expression<number> | string, value: types.Expression<number> | string) {
	...
	WRSEQ(abbrev: string, exps: types.Expression<any>) {
	...
	RDSEQ(abbrev: string, vars: types.Expression<any>[]) {
	...
	PUTSEQ(abbrev: string, nbr: types.Expression<number>, vars: types.Expression<any>[]) {
	...
	GETSEQ(abbrev: string, nbr: number, vars: types.Expression<any>[], sized: types.Expression<number>) {
		return function putseq$do(_: _) {
			const frame = util.currentFrame();
			const file = findFileR(frame, abbrev);
			const size: number[] = [];
