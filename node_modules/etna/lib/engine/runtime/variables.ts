...
exports.instructions = {
	...
	CA(field, exps) {
		const decl = exports.instructions.DECL('LX', field, util.instructions.A([util.instructions.C(exps.length)]));
		const aexps = util.instructions.A(exps);
	...
	SET(lhs, rhs, silent) {
		if (config.optimize && typeof rhs != "function") return function set$do(_) {
   ...
   */
	...
	SETINSTANCE(target, abbrev) {
		return function setinstance$do(_, silent) {
			const t = target(_);
			const frame = glob.context.x3frame;
