...
export const vOperations = {
	GET: {
		fn(frame: tpes.Frame, dict: tpes.VariableDict, field: string, val: any, silent: boolean, strict: boolean) {
		...
		error(_: _,  frame, field, val, silent, strict) {
			if (silent) return;
...
export const instructions = {
	...
	CLALEV(arg) {
		return function clalev$do(_: _) {
			const a = arg(_, false, undefined, true);
   ...
   */
	...
	ATT(target, property, attribut) {
		if (property) return function att$do(_: _,  assign, val, silent) {
			const t = target(_);
		...
		};else return function att$do(_: _,  assign, val, silent) {
			const t = target(_);
	...
	AGETATTR(target, property, attribut) {
		if (property) return function agetattr$do(_: _,  silent) {
			const t = target(_);
		...
		};else return function agetattr$do(_: _,  silent) {
			const t = target(_);
	...
	ASETATTR(target, property, attribut, val) {
		if (property) return function asetattr$do(_: _,  silent) {
			const t = target(_);
		...
		};else return function asetattr$do(_: _,  silent) {
			const t = target(_);
	...
	SETINSTANCE(target, abbrev) {
		return function setinstance$do(_: _,  silent) {
			const t = target(_);
			const frame = glob.context.x3frame;
