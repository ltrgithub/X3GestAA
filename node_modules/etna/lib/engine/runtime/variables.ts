			...
			const vsz = function () {
				...
				switch (type[1]) {
					...
					case "T":
						...
						return l;
					case "D":
						return type[0] === " " && dict !== util.currentFrame().context.sys ? 3 : 4;
					case "Z":
						return 16;
					case "E":
						return 8;
	...
	DECL(key: string, field: string, using: string, lens0: Expression<number[]>, ranges0: Expression<Range[]>) {
		...
		if (config.etna.optimize && lens0 === undefined && ranges0 === undefined) {
			if (letter === "V") return function decl$do(_: _) {
				util.currentFrame().context.globals.values[field] = exports.clone(def);
				util.currentFrame().context.globals.types[field] = key;
			};else if (letter === "L") return function decl$do(_: _) {
				util.currentFrame().values[field] = exports.clone(def);
				util.currentFrame().types[field] = key;
			};
		}
		...
		return function dot$do(_,  assign, val, silent) {
			...
			try {
				if (assign) {
					...
					if (typeof t.get === "function") return x3Val(t.get(_, name, util.currentDebug()));
					else {
						var v = t[name];
						if (v === undefined) {
