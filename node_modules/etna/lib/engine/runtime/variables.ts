...
import { Dict, Expression, Frame, FrameContext, VariableDict, VariableOp } from './types';
...
const tracerJs = core.getTracer("etna.engine");
...
export function initStack(superv: Supervisor) {
	const cx: FrameContext = {
		globals: {
			values: {
				$exported: true,
			} as any,
			types: {} as Dict<string>,
		},
		sys: {
			values: {} as Dict<string>,
			types: {} as Dict<string>,
		},
		tables: {}  as Dict<DBTableParent>,
		gclass: {} as Dict<GClass>,
		superv: superv,
	...
	function declVar(dict: VariableDict, name: string, type: string, value: any) {
...
function autoType(dict: VariableDict, field: string, v: any) {
			...
			const vsz = function () {
				...
				switch (type[1]) {
					...
					case "T":
						...
						return l;
					case "D":
						return type[0] === " " && dict !== util.currentContext().x3frame.context.sys ? 3 : 4;
					case "Z":
						return 16;
					case "E":
						return 8;
...
export const instructions = {
	...
	V(field: string): Expression<any> {
		const consts = constants as any;
		if (typeof consts[field] !== 'undefined') {
			var fn: Expression<number> = function v$do(_: _) {
				return consts[field];
			};
		...
		return function v$do(_,  operation, val, silent, strict) {
			...
			if (tables) {
				var keys = Object.keys(frame.tables);
				//for (var j = 0; !found && j < keys.length; j++) {
				for (var j = 0; j < keys.length; j++) {
					var abb = keys[j];
					let table = tables[abb];
					var values = table.values;
					if (values[field]) {
						return fdo.fn(frame, table, field, val, silent, strict);
					}
				}
			}
	...
	DIM(letter: string, field: string, rhs: Expression<number>) {
		var fv: Expression<number>;
	...
	CV(letter: string, abbrev: string, field: string, silent?: boolean): Expression<any> {
		const consts = constants as any;
		if (typeof consts[field] !== 'undefined') {
			var fn: Expression<any> = function v$do(_: _) {
				return consts[field];
			};
			fn.value = consts[field];
			return fn;
		...
		return function cv$do(_,  operation, val, silent, strict) {
			...
			switch (letter) {
				...
				case 'F':
				case '?':
					var tdict = frame.context.tables;
			...
			if (!dict) {
				if (false && frame.tryingSql) {
					...
					(err as any).tooComplexForSql = true;
					throw err;
				}
			} else {
	...
	DECL(key: string, field: string, using: string, lens0: Expression<number[]>, ranges0: Expression<Range[]>) {
		...
		if (config.etna.optimize && lens0 === undefined && ranges0 === undefined) {
			if (letter === "V") return function decl$do(_: _) {
				util.currentContext().x3frame.context.globals.values[field] = exports.clone(def);
				util.currentContext().x3frame.context.globals.types[field] = key;
			};else if (letter === "L") return function decl$do(_: _) {
				util.currentContext().x3frame.values[field] = exports.clone(def);
				util.currentContext().x3frame.types[field] = key;
			};
		}
	...
	CA(field: string, exps: Expression<any>[]) {
	...
	SET(lhs: Expression<any>, rhs: Expression<any>, silent?: boolean) {
	...
	APPEND(lhs: Expression<any>, rhs: Expression<any>, silent?: boolean) {
		return function append$do(_: _) {
			lhs(_, exports.vOperations.SET, lhs(_).x3Add(rhs.value !== undefined ? rhs.value : rhs(_)), silent);
		};
	},
	ASSIGN(lhs: Expression<any>, rhs: Expression<any>, silent?: boolean) {
   ...
   */
	...
	SNAP(target: Expression<any>): Expression<void> {
	...
	PROP(target: Expression<any>, name: string): Expression<any> {
		return function dot$do(_,  assign, val, silent) {
			const t = target(_);
			...
			try {
				if (assign) {
					...
					if (typeof t.get === "function") return x3Val(t.get(_, name, util.currentContext().debug));
					else {
						var v = t[name];
						if (v === undefined) {
	...
	ATT(target: Expression<any>, property: string, attribute: string): Expression<any> {
	...
	AGETATTR(target: Expression<any>, property: string, attribute: Expression<string>): Expression<any> {
	...
	ASETATTR(target: Expression<any>, property: string, attribute: Expression<string>, val: Expression<any>): Expression<void> {
	...
	XMET(target: Expression<any>, name: string, args: Expression<any>[]): Expression<any> {
		// fmet and callmet
	...
	CLA(letter: string, abbrev: string): Expression<void> {
	...
	AADD(target: Expression<any>, position: Expression<number>): Expression<any> {
	...
	ADEL(target: Expression<any>, position: Expression<number>): Expression<any> {
	...
	ASETCOLUMNATTR(target: Expression<any>, collection: string, column: Expression<string>, 
	attribute: Expression<string>, val: Expression<any>): Expression<any> {
	...
	AGETCOLUMNATTR(target: Expression<any>, collection: string, column: Expression<string>, attribute: Expression<any>): Expression<any> {
	...
	AGETTEXT(target: Expression<any>, property: string): Expression<string> {
		return function agettext$do(_) {
			const t = target(_);
			return t.AGETTEXT(_, property);
		};
	},
	ASETTEXT(target: Expression<any>, property: string, intit: Expression<string>): Expression<void> {
		return function asettext$do(_) {
			const t = target(_);
			return t.ASETTEXT(_, property, intit(_));
		};
	},
	AGETTEXTLAN(target: Expression<any>, property: string, lan: string): Expression<string> {
		return function agettextlan$do(_) {
			const t = target(_);
			return t.AGETTEXTLAN(_, property, lan);
		};
	},
	ASETTEXTLAN(target: Expression<any>, property: string, intit: string, lan: string): Expression<void> {
		return function asettextlan$do(_) {
			const t = target(_);
			return t.ASETTEXTLAN(_, property, intit, lan);
		};
	},
	AGETTEXTRA(target: Expression<any>, property: string): Expression<string> {
		return function agettextra$do(_) {
			const t = target(_);
			return t.AGETTEXTRA(_, property);
		};
	},
	AGETTEXTRALAN(target: Expression<any>, property: string, lan: string): Expression<string> {
		return function agettextralan$do(_) {
			const t = target(_);
			return t.AGETTEXTRALAN(_, property, lan);
		};
	},
	ASETTEXTRALAN(target: Expression<any>, property: string, intit: string, lan: string): Expression<void> {
		return function asettextralan$do(_) {
			const t = target(_);
			return t.ASETTEXTRALAN(_, property, intit, lan);
		};
	},
	SETINSTANCE(target: Expression<any>, abbrev: string): Expression<void> {
