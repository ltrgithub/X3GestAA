...
// -----------------------------------------------------------
function _fromString(can: X3path, canstr: string) {
...
export class X3path {
	...
	host: string;
	application: string;
	extention: string; // TODO fix typo
	name: string;
	string: string;
	volumes: X3pathVolume[];
	constructor(_: _,  superv: Supervisor, dir: string, nam: string, ext: string, app?: string, vol?: string, hst?: string) {
		var p: number, fca : boolean;
		var result = "";
		...
		fca = this.directory && _patabs(this.directory);
	...
	toString(_: _) {
		...
		if (this.directory && _patabs(this.directory)) {
			...
			pat = vol ? vol : _defaultDir; // should nt exist
		} else {
	...
	getFolderVolumes(superv: any) {
		const dir: any[] = config.sandbox.directories;
		const result: X3pathVolume[] = []
		const vol = dir.reduce((vol, obj) => {
			if (obj.volume) {
				var path = obj.path.replace(/\{(\w+)\}/, function (str: string, p1: string) {
					return superv[p1];
				});
				vol.push({
					name: obj.volume,
	...
	getApplicationVolumes(_: _,  app: string) {
		for (var i = 0; i < this.volumes.length; i++) {
			try {
