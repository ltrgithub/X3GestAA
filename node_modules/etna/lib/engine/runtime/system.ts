...
export const functions = {
	...
	VER$: {
		...
		fn(exp_num: number) {
			...
			throw new errors.X3Error(50, "Function not defined for the given value");
		}
	},
	NOMAP: {
		count: 0,
...
function sysFor(_: _,  where: any, p: types.Expression<any>[], end: number, block: types.Expression<any>) {
	console.error("System FOR loop - skipping for now");
	p[end](_);
}

export const instructions = {
	FILESYS(abbrev: types.Expression<any>, vars: types.Expression<any>[], from: void) {
		const avars = basic.instructions.A(vars);

		return function dot$do(_: _) {
			// TODO: fix typing aon true arg
			const table = abbrev(_, util.any(true), {
				types: {},
				values: {},
				doFor: sysFor
			});
