...
type TableColumns = { [name: string]: TableColumn[] }

interface TableKeyColumn {
	abbrev: string;
}
interface TableKey2 {
	columns: TableKeyColumn[];
	sql: string;
}
function _addColumns(dest: TableColumns, origin: TableKey2) {
...
function _splitKeyWhere(kw: KeyWhere) {
...
function _isPostEvaluated(wh: Where[]) {
...
function _buildQuery(_: _, clause: string, dbtable: DBTable, key: TableKey2, wkey: TableKey[], 
fwhere: types.Expression<Where>, 
fwfrom: types.Expression<Where>, 
fwto: types.Expression<Where>, lock: boolean, hint: Hint) {
...
export class DBTable extends DBTableParent {
	...
	keys: TableKey[];
	joins: TableJoin[];
	columns: { [colName: string]: TableColumn[] };
	values: { [colName: string]: any };
	types: { [colName: string]: string }
	qrewrite: string;
	constructor(driver: types.Driver, abbrev: string, superv: types.Supervisor, desc: TableDesc, keys: TableKey[], loc: string) {
	...
	init(_: _) {
		...
		Object.keys(columns).forEach_(_, (_, colName) => {
			...
			if (columns[colName].data.DIME <= 1) {
				...
				activ = activ || require("etna/lib/supervisor/meta/activ");
	...
	// operation on table --------------------------------------
	// ---------------------------------------------------------
	x3Read(_: _, key: TableKey2, lvl: number, wkey: TableKey[], krevert: boolean, lock: boolean, hint: Hint) {
		var rkey: TableKey2;
	...
	x3Rewrite(_: _, wkey: TableKey[], values: any[]) {
	...
	x3OpenFor(_: _, key, lvl, hint, lock, where, wfrom, wto) {
	...
	x3Update(_: _, fwhere, asssql, consts) {
		const ctx = {};
		ctx.tables = [this.abbrev];
		ctx.noabbrev = true;
		const dwhere = fwhere && fwhere(_, ctx);
	...
	x3Rowcount(_: _) {
		...
		if (loop) {
			...
			const qry = _buildQuery(_, "rowcount", this, undefined, undefined, undefined, false);
			const q = qry[0];
