...
interface WhereColumns {
	
}

export type ContextFn<T> = (_: _, cx: QueryContext) => T;

function _buildQuery(_: _, clause: string, dbtable: DBTable, key: TableKey, wkey: WKey[], 
...
function _transformKey(orik: TableKey, mod: KeyModified) {
	//copy key
	const newk: TableKey = {
		name: "",
		sql: "",
...
export class DBTable extends DBTableParent {
	...
	constructor(driver: types.Driver, abbrev: string, superv: Supervisor, desc: Table, keys: TableKey[], loc: number) {
	...
	// operation on table --------------------------------------
	// ---------------------------------------------------------
	x3Read(_: _, key: TableKey, lvl: number, wkey: WKey[], krevert: boolean, lock: boolean, hint: types.Hint) {
		var rkey: TableKey;
	...
	x3OpenFor(_: _, key: TableKey, lvl: number, hint: types.Hint, lock: boolean, where: ContextFn<WKey[]>, wfrom: ContextFn<WKey>, wto: ContextFn<WKey>) {
	...
	x3Delete(_: _, key: TableKey, wkey:WKey[], fwhere: ContextFn<WKey[]>) {
		const ctx: QueryContext = {};
	...
	// misc ----------------------------------------------------
	// ---------------------------------------------------------
	x3SetNewKey(_: _, newkey: TableKey) {
		if (newkey.sql) {
