...
import { Driver, Hint, Record, SqlQueryOptions } from '../drivers/types';
import { ContextFn, Expression, QueryContext, QueryKey, QueryKeys, KeyModified, TableKey, KeyExpression, WKey } from './types';
...
import { Table, Index, Column } from '../../supervisor/meta/types';
...
const tracer = core.getTracer("etna.dbms");
...
function _addColumn(r: QueryKey[], t: QueryKey) {
	const n = r.length;
	for (var i = 0; i < n; i++) {
...
function _addColumns(dest: QueryKeys, origin: { columns?: QueryKey[]}) {
...
export type Predicates = Expression<boolean[]>[];
...
fwto?: ContextFn<WKey[]>, lock?: boolean, hint?: Hint) {
	// buikld query
	...
	// columns is too weird (extended??) - will sort out typing later
...
function _writeLobs(_: _,  row: Record, lobVars: LobVar[], outputVars: OutputVars) {
...
function _lobsReturningClause(driver: Driver, lobColumns: Column[], lobVars: LobVar[], currind: number) {
	var sql = " RETURNING ";
...
export class DBTable extends DBTableParent {
	...
	constructor(driver: Driver, abbrev: string, superv?: Supervisor, desc?: Table, keys?: KeyExpression[] | TableKey[], loc?: number) {
		if (driver instanceof DBTable) {
			const tbl: DBTable = driver as any;
			...
			this.reader = undefined;
			this.desc = desc;
			this.keys = keys as any; // todo: clean this up
		}
	}
	init(_: _) {
		const table = this;
	...
	x3Read(_: _, key: KeyExpression, lvl: number, wkey: WKey[], krevert: boolean, lock: boolean, hint: Hint) {
		...
		const cnx = util.currentContext().x3session.dbConnection;
	...
	x3OpenFor(_: _, key: KeyExpression, lvl: number, hint: Hint, lock: boolean, where: ContextFn<WKey[]>, wfrom: ContextFn<WKey[]>, wto: ContextFn<WKey[]>) {
		const master: DBTable = (this.master as any) || this;
