...
export function requireScript(_: _,  name) {
...
function initTraces() {
	...
	function traceCall(_: _,  action, arg) {
		const frame = glob.context.x3frame;
	...
	function traceFunc(_: _,  action, arg) {
		const frame = glob.context.x3frame;
...
export function executeProg(_: _,  mod, prog, args, _frame) {
...
function _initDebugger(_: _) {
	...
	Object.keys(exports.instructions).filter(k => trueInst.indexOf(k) != -1).forEach(k => {
		...
		exports.instructions[k] = function debugWrapper() {
			...
			if (typeof fn !== "function") return;
			const loc = helpers.object.clone(basic.sourceLoc);
