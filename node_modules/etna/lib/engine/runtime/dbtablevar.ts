...
function _buildKeyGuide(_: _,  dbtable: DBTableVAR, key: KeyExpression) {
	...
	key.guide = base.sort_(_, (_, l0, l1) => {
		const row0: types.Row = {};
		const row1: types.Row = {};
...
export class DBTableVAR extends DBTableParent {
	...
	tables: types.Expression<DBTableParent>[]; //{ [name: string]: DBTableParent };
	bases: number[];
	lines: number;
	curpos: { [name: string]: any };
	constructor(abbrev: string, superv: Supervisor, loc: number, tbvar: types.Expression<DBTableParent>[], tbvalues: any[]) {
	...
	x3OpenFor(_: _,  key: KeyExpression, lvl: number, hint: types.Hint, lock: boolean, 
