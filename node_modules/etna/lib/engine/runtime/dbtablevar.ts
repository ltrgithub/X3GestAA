...
import { DBTableParent, KeyExpression } from './dbtableparent';
...
function _buildWhere(_: _,  dbtable, wkey, fwhere, fwfrom, fwto) {
	const tpwhere = [];
	const ctx = {};
...
function _buildKeyGuide(_: _,  dbtable, key) {
	const base = [];
...
export class DBTableVAR extends DBTableParent {
	...
	constructor(abbrev: string, superv: Supervisor, loc: number, tbvar, tbvalues) {
	...
	x3OpenFor(_: _,  key, lvl, hint, lock, fwhere, wfrom, wto) {
		const maptrim = (_: _, row: types.Row) => {
			const rec: types.Row = {};
			const row1 = Object.keys(row).reduce((r: types.Row, name: string) => {
	...
	x3Read(_: _,  key, lvl, wkey, krevert, lock, hint) {
		const maptrim = (_, row) => {
			const rec = {};
