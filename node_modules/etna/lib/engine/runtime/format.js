...
var x3Trim = require('etna/lib/engine/runtime/util').x3Trim;
...
exports.format = function($fmt, $val) {
	...
	var isDigit;
	var values = glob.context.x3frame.context.sys.values;

	var special = values.ADXSCA;
	var adxtul = values.ADXTUL ? values.ADXTUL[2] : "";
	var adxtuc = values.ADXTUC ? values.ADXTUC[2] : "";
	var adxtut = values.ADXTUC ? values.ADXTUT[2] : "";
	...
	var formatters = {
		...
		D: function() {
			var _year = val.utcYear || val.year;
			var _month = val.utcMonth || val.month;
			var _day = val.utcDay || val.day;
			var _hour = val.utcHour || val.hour;
			var _minute = val.utcMinute || val.minute;
			var _second = val.utcSecond || val.second;
...
exports.fromX3FormatToRegex = function(format) {
	...
	for (var i = 0; i < adxtul.length; i++) {
		mapFormat.K[adxtul[i]] = adxtut[i].split('').reduce(function(r, c) {
	...
	if (map) {
		var rep = 0;
		exp = '^' + parts[1].split('').reduce(function(r, ch) {
			if (_isDigit(ch)) {
				rep = 10 * rep + parseInt(ch);
			} else {
				var e = map[ch];
				if(!e) console.error("fromX3FormatToRegex ch:"+ch+" map",map);
				if (e.indexOf('-') >= 0) e = '[' + e + ']';
				r.push(e + '{0,' + (rep || 1) + '}');
				rep = 0;
			}
