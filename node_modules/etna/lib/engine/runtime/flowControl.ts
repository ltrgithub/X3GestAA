...
const trace = config.etna.traceError ? console.log : null;
...
export function handleError(_: _,  frame, e) {
	...
	if (!(e instanceof X3Error)) console.error(`EXCEPTION: ${e && e.stack}`);
...
function callProg(_: _,  prog, args, argVals, loc) {
...
export function callProg(_: _,  prog, args, argVals, loc) {
	return callProg(_, prog, args, argVals, loc);
};

export function labelProg(ns, name) {
	return instructions.PROG({}, "stub", [], instructions.GOSUB(basic.instructions.C(ns), name));
};

function callJS(_: _,  ns, prog, argVals, frame) {
...
export const instructions = {
	...
	F(name: string, args: types.Expression<any>[]) {
		const fn = require('./index').functions[name];

		if (fn == null) throw new X3Error(4, "bad function name: " + name);
		const argc = args ? args.length : 0;

		if (fn.count >= 0 && fn.count != argc) throw new X3Error(69, _locPrefix(basic.sourceLoc) + "bad arg count: function'" + name + "' expected " + fn.count + ", got " + argc);
		const aargs = args ? basic.instructions.A(args) : basic.instructions.C();
	...
	FORV(varName, from, to, step, p, end, block) {
		return function forv$do(_: _) {
			...
			if (stepI === 0) {
				...
				// p[end] :
				handleError(_, frame, new X3Error(41, "invalid step value: " + stepI));
				p[end](_);
				return;
			}
	...
	BREAK(p, level) {
		return function break$do(_: _) {
			...
			if (frame.loopLevel === null || frame.loopLevel < l) throw new X3Error(50, "invalid break level: " + l + " frame.loopLevel:" + frame.loopLevel);
			frame.loopLevel -= l;
		};
	},
	MAIN(e, _) {
