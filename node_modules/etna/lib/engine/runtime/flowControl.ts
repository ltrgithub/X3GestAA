...
function currentSupervisor(_: _) {
	const session: any = util.currentSession();
...
export const functions = {
	...
	MESS: {
		...
		fn(_: _,  id: number, chapter: number, swtch?: number) {
			...
			// --------------------------------------------------------------------------
			if (frame.context.sys.values["MESSNAME"] !== util.currentSession().actx.$LAN(_)) return frame.context.superv.loadMess(_, swtch !== 0 ? chapter : chapter + 9001, id, frame.context.sys.values["MESSNAME"]);
...
export const instructions = {
	...
	MAIN(e: Expression<any>, _: _) {
		...
		const frame = util.setCurrentFrame(variables.initStack(superv));
