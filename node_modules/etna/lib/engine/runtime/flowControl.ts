...
import { Expression, Frame, Location, Prog, WKey } from './types';
...
export function handleError(_: _,  frame: Frame, e: any) {
	//!console.log("_handleError:"+e);
	// unwind stack
	(_.context as any).frame = frame;
	if (e.fill) {
	...
	if (!(e && e instanceof errors.X3Error) || e.errn === 26) console.error(`EXCEPTION: ${e && e.stack}`);
...
function _locPrefix(loc: Location) {
...
export function callProg(_: _,  prog: Prog, args: Expression<any>[], argVals: any[], loc: Location) {
...
function callJS(_: _,  prog: (_: _) => any, argVals: any[], frame: Frame) {
...
function getSetter(field: string, val?: any) {
	...
	for (var i = 0, len = dicts.length; i < len; i++) {
		if (dicts[i].values[field] !== undefined) {
...
function currentSupervisor(_: _) {
	const session: any = util.currentContext().x3session;
	...
	const superv = require('../../supervisor/supervisor').create(_, conf);
	require('../../supervisor/builtins/ACTX').init(_, conf.session, superv);
	return superv;
}
...
export const functions = {
	...
	MESS: {
		...
		fn(_: _,  id: number, chapter: number, swtch?: number) {
			...
			// --------------------------------------------------------------------------
			if (frame.context.sys.values["MESSNAME"] !== util.currentContext().x3session.actx.$LAN(_)) return frame.context.superv.loadMess(_, swtch !== 0 ? chapter : chapter + 9001, id, frame.context.sys.values["MESSNAME"]);
...
export const instructions = {
	REQ(req: (_: _, name: string) => any, v: Expression<string>) {
	...
	GOSUB(ns: Expression<any>, label: string) {
	...
	GOTO(ns: Expression<any>, label: string) {
		return function gosub$do(_: _) {
			const frame = util.currentContext().x3frame;
	...
	ONERRGO(ns: Expression<any>, label: string) {
	...
	PROG(e: { [name: string]: Prog }, name: string, params: string[], body: Expression<any>) {
		return e[name] = {
			name: name,
			params: params,
			body: body,
	...
	CALL(ns: Expression<any>, name: Expression<string> | string, args: Expression<any>[]) {
	...
	F(name: string, args: Expression<any>[]): Expression<WKey> {
		...
		(fdo as any).instName = name;
		return fdo;
	},
	IF(cond: Expression<boolean>, p: Expression<any>[], branch: number, block: Expression<any>) {
	...
	CASE(exp: Expression<any>, p: Expression<any>[], branch: number) {
	...
	WHEN(exps: Expression<any>[], p: Expression<any>[], branch: number, block: Expression<any>) {
	...
	WHILE(cond: Expression<boolean>, p: Expression<any>[], end: number, block: Expression<any>, isRepeat: boolean) {
	...
	FORV(varName: string, from: Expression<number>, to: Expression<number>, step: Expression<number>, 
	...
	FORIN(varName: string, items: Expression<any>[], p: Expression<any>[], end: number, block: Expression<any>) {
		return function forv$do(_: _) {
			const frame = util.currentContext().x3frame;
			const level = ++frame.loopLevel;
	...
	BREAK(p: void, level: Expression<number>) {
	...
	MAIN(e: Expression<any>, _: _) {
		if (gotMain) return;
		gotMain = true;
		
		const superv = currentSupervisor(_);
		const frame = util.currentContext().x3frame = variables.initStack(superv);
