	...
	const tqry = twhere.map(function(whereSqls, idx) {
		...
		switch (clause) {
			case 'rowcount':
				...
				break;
			case 'read':
				sql = "Select " + self.driver.hintClause0(hint, true) + " " + self.driver.rowIdGet(self.abbrev + ".ROWID") + " As " + self.abbrev + "$ROWID, ";
				break;
			case 'for':
				sql = "Select " + self.driver.hintClause0(hint, false) + " " + self.driver.rowIdGet(self.abbrev + ".ROWID") + " As " + self.abbrev + "$ROWID, ";
				break;
			default:
