...
function _buildQuery(_, clause, dbtable, key, wkey, fwhere, fwfrom, fwto, lock, hint) {
	...
	if (wkey && wkey[0].rowid) {
		...
		columns = columns && ((origin) => {
			const cpy = {};
			Object.keys(origin).forEach((abv) => {
				cpy[abv] = origin[abv];
			});
			return cpy;
		})(columns);
	...
	const tqry = twhere.map((whereSqls, idx) => {
		...
		const middleSql = (tablesSql + whereSql).replace(/@\?/g, (val) => {
			return dbtable.driver.param(currtag++);
		});
		var orderBySql = "";
...
function _transformKey(orik, mod) {
	...
	// treat items and sql
	newk.directions = orik.directions.map((d) => {
	...
	newk.sql = newk.items.map((it, idx) => {
...
class DBTable extends DBTableParent {
	...
	x3Rewrite(_, wkey, values) {
		...
		if (lst) {
			...
			sql = sql.replace(/@\?/g, (val) => {
				return this.driver.param(currtag++);
			});
	...
	x3Delete(_, key, wkey, fwhere) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) || (this.where && _isPostEvaluated(this.where)) || (this.filter.where && _isPostEvaluated(this.filter.where))) {
			...
			var currtag = 0;
			var sql = ("Delete " + this.desc.name + whereSql).replace(/@\?/g, (val) => {
				return this.driver.param(currtag++);
			});;
	...
	x3Update(_, fwhere, asssql, consts) {
		...
		if ((dwhere && _isPostEvaluated(dwhere)) || (this.where && _isPostEvaluated(this.where)) || (this.filter.where && _isPostEvaluated(this.filter.where))) {
			...
			var currtag = 0;
			const dqry = ("Update " + this.desc.name + " Set " + asssql + " Where ROWID = @?").replace(/@\?/g, (val) => {
				return this.driver.param(currtag++);
			});
			...
			var currtag = 0;
			var sql = ("Update " + this.desc.name + " Set " + asssql + whereSql).replace(/@\?/g, (val) => {
				return this.driver.param(currtag++);
			});;
			sql = sql.replace(new RegExp(this.abbrev + "\\.", "g"), "");
