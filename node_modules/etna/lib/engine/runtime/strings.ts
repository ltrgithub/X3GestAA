...
export const functions = {
	...
	CTRANS: {
		count: -1,
		fn(src: string, from: string, to: string) {
			var dst = "";
			...
			if (from) {
				...
				if (from.length < src.length) {
					...
					for (var i = 0, l = src.length; i < l; i++) {
						var tch = map[c = src.charCodeAt(i)];
						if (tch !== -1) dst += String.fromCharCode(tch === undefined ? c : tch);
					}
				} else {
					let ch: string;
					for (var i = 0, l = src.length; i < l; i++) {
   ...
   if (!fn) throw new X3Error(50, "bad mode: " + mode);*/
	...
	STRING$: {
		count: 2,
		fn(n: number, c: number | string) {
			let ch = typeof c === 'number' ? String.fromCharCode(c) : c;
			return repeat(ch, n);
		}
	},
	FORMAT$: {
		count: 2,
		fn(fmt: string, val: any) {
	...
	INSTR: {
		count: 3,
		fn(start: number, str: string, s: string) {
