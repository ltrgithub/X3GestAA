...
export type VariableOp = (frame: Frame, dict: Dict, field: string, val: any, silent: boolean, strict: boolean) => any;
...
// throw wrapper to avoid problem with ts type checking
export function throwError(err: any): any {
	throw err;
}
/* 
 * Line numbers
...
export function throwBadOperand(x: any) {
	//return new X3Error(10, "bad operand" + x? (": "+x + " (type " + x.constructor.name + ")"):"");
	throwError(new X3Error(10, "bad operand" + (x ? ": " + x + " (type " + x.constructor.name + ")" : "")));
};
...
export const instructions = {
	...
	A(items: Expression<any>[], forget?: boolean) {
