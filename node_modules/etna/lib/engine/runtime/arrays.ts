...
function _insDela(isDel: boolean) {
	return function (ints: util.Expression<number>[], vars: util.Expression<any>[]) {
...
exports.instructions = {
	RANGE(lo: util.Expression<number>, hi: util.Expression<number>) {
		hi = hi || util.instructions.C(null);
		return function range$do(_: _) {
			return [lo(_), hi(_)];
		};
	},
	INDEX(target: util.Expression<any>, args: util.Expression<number>[]) {
	...
	SORTA(rep: util.Expression<number>, vars: util.Expression<any>[], orders: util.Expression<any>[], direction: util.Expression<number>) {
		const avars = util.instructions.A(vars);
		const aorders = util.instructions.A((orders || []).map(_evalOrder));
