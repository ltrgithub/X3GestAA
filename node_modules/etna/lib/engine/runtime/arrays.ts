...
function _insDela(isDel: boolean) {
	return function (ints: Expression<number>[], vars: Expression<any>[]) {
...
export const instructions = {
	RANGE(lo: Expression<number>, hi: Expression<number>) {
		hi = hi || basic.instructions.C(null);
		return function range$do(_: _) {
			return [lo(_), hi(_)];
		};
	},
	INDEX(target: Expression<any>, args: Expression<number>[]) {
		const aargs = basic.instructions.A(args);

		return function index$do(_: _, assign?: any, val?: any) {
			const frame = util.currentContext().x3frame;
	...
	SORTA(rep: Expression<number>, vars: Expression<any>[], orders: Expression<any>[], direction: Expression<number>) {
		const avars = basic.instructions.A(vars);
...
Object.defineProperty(Array.prototype, 'x3Compare', {
	value(arr: any, strict?: boolean) {
		if (!Array.isArray(arr)) {
			// investigate: we should always throw but this breaks x3Deltas unit test (arr is a number 102)
			if (strict) throw util.badOperand(arr);
			else return +1;
		}
		var cmp = this.length.x3Compare(arr.length);
		if (cmp !== 0) return cmp;
		for (var i = 0; i < this.length; i++) {
			var v1 = this[i];
			var v2 = arr[i];
			if (v1 != null) return v1.x3Compare(v2);
			else return v2 == null ? 0 : 1;
		}
		return 0;
	},
	enumerable: false
