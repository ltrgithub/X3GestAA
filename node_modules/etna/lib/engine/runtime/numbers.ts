...
export const functions = {
	...
	SQR: {
		...
		fn(x: any) {
			...
			if (d < 0) throw new errors.X3Error(11, "Negative root for a number");
	...
	CNP: {
		...
		fn(n: any, p: any) {
			...
			const result = fac(n) / fac(n - p) / fac(p);
			const bcdResult = bcd.fromInt32(result);
	...
	LOG: {
		...
		fn(x: any) {
			...
			if (d <= 0) {
				if (d === 0) throw new errors.X3Error(52, "ERROR log(0)");
				throw new errors.X3Error(14, "Logarithm for a negative number");
			}
	...
	SETBIT: {
		count: 3,
		fn(val: number, pos: number, bit: number) {
			return bit == 1 ? val | 1 << pos - 1 : val & ~(1 << pos - 1);
		}
	},
	GETBIT: {
		count: 2,
		fn(val: number, pos: number) {
