...
function _buildWhere(_, self, wkey, fwhere, fwfrom, fwto ) {
	...
	var wheres = (wkey || []).concat(self.where, self.filter.where);
...
class DBTableVAR extends DBTableParent {
	...
	x3OpenFor(_, key, lvl, hint, lock, fwhere, wfrom, wto) {
		...
		const maptrim = function(_, row) {
			var rec = { } ;
			var row1 = Object.keys( row ).reduce(function( r, name) { 
			...
			rec[self.abbrev] = row1;
			return rec ;
		};

		var pwhere =_buildWhere(_, self, undefined, fwhere, wfrom, wto );
		var tables = {};
		tables[self.abbrev] = self ;

		self.curpos["for"] = 0 ;
		const rdr = vtb.reader( self, key, "for" );
	...
	x3Read(_, key, lvl, wkey, krevert, lock, hint) {
		...
		var pwhere =_buildWhere(_, self, wkey );
		var tables = {};
		tables[self.abbrev] = self ;

		//console.log("--"+JSON.stringify(self.curpos)+"--");

		if (wkey && wkey[0].rowid ) self.curpos["read"] = self.curpos[self.curpos.curr] ;
		else self.curpos["read"] = 0 ;

		const rdr = vtb.reader( self, rkey, "read" );
		...
		if (cf.x3GetRecord(_)) {
			...
			if (krevert) {
				self.curpos["read"] = self.lines - (self.curpos["read"] - 1)-1 ;
			}
