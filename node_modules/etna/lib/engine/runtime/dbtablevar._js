...
function _buildKeyGuide(_, self, key) {
	const base = [];
	const names = Object.keys(self.values);

	for (var i = 0; i < self.lines; i++) base[i] = i;
	...
	key.guide = base.sort_(_, (_, l0, l1) => {
		...
		const sav = self.values;

		self.values = row0;
		const kvals = self.getKeyValues(_, key, key.func.length);

		self.values = row1;
		const r = self.keyCompare(_, key, kvals);
		// restore data
