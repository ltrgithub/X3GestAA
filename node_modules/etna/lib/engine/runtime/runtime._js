...
const version = require('etna/lib/engine/version').version;
const scriptDebugger = require("etna/lib/engine/debugger/server");
const db = require('./db');
const tracer = require('syracuse-core').getTracer("etna.engine");
const debugTracer = require('syracuse-core').getTracer("etna.debugger");
// require jsInvoke lazily to avoid circular dependency bug
var jsInvoke;

const modules = ['arrays', 'dates', 'db', 'evaluate', 'fileio', 'flowControl', 'misc', 'numbers', //
'operators', 'stats', 'strings', 'system', 'util', 'variables', //
'tbcd', 'uuid', 'tblbfile', 'tclbfile'].map(name => require('./' + name));
...
exports.requireScript = function (_, name) {
	...
	if (!exports.requireScript.builtins) {
		...
		files.filter(file => file.length == 4 + file.lastIndexOf("._js")).forEach_(_, function (_, file, i) {
	...
	if (!exports.requireScript.nodeScript) {
		...
		files.filter(file => file.length == 4 + file.lastIndexOf(".src")).forEach_(_, function (_, file, i) {
...
const runtime = require('streamline-runtime').globals.runtime;
...
function initTraces() {
	...
	Object.keys(exports.instructions).forEach(k => {
		const instruction = exports.instructions[k];
...
function _dumpInstructionCounters() {
	...
	const counters = Object.keys(instCounters).map(k => instCounters[k]).filter(c => c.hits).sort((c1, c2) => c2.nanos - c1.nanos).map(c => 'total (ms): ' + millis(c.nanos) + '\thits: ' + padL(c.hits, 9) + '\tcost (ns):' + padL(Math.round(c.nanos / c.hits, 2), 9) + '\t' + c.name);
...
exports.executeProg = function (_, mod, prog, args, _frame) {
	...
	glob.context.x3frame = frame = _frame || {
		values: {},
		types: {},
		context: cx,
		prev: prev,
	...
	try {
		if (prog[0] === '$' && !mod.$4gl && !mod.body.$4gl) {
			jsInvoke = jsInvoke || require('etna/lib/x3js/js-invoke');
			result = jsInvoke(_, mod, prog.substring(1).toLowerCase(), args);
		} else {
			args && Object.keys(args).forEach_(_, (_, name) => {
			...
			args && Object.keys(args).forEach(name => {
				if (!/^[A-Z]/.test(name)) return;
...
function _initDebugger(_) {
	...
	Object.keys(exports.instructions).filter(k => trueInst.indexOf(k) != -1).forEach(k => {
		const instruction = exports.instructions[k];
		...
		exports.instructions[k] = function debugWrapper() {
			...
			if (typeof fn !== "function") return;
			const loc = helpers.object.clone(util.sourceLoc);
