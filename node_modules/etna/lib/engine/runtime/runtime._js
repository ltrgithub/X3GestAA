...
exports.requireScript = function(_, name) {
	...
	if (!exports.requireScript.builtins) {
		...
		files = fs.readdir(__dirname + "/../../../etna/lib/supervisor/builtins/", _);
		files.filter(function(file) {
			return file.length == (4 + file.lastIndexOf("._js"));
		}).forEach_(_, function(_, file, i) {
			exports.requireScript.builtins[file.substring(0, file.lastIndexOf("._js"))] = __dirname + "/../../../etna/lib/supervisor/builtins/" + file;
	...
	if (!exports.requireScript.nodeScript) {
		...
		files = fs.readdir(__dirname + "/../../../etna/test/engine/server/", _);
		files.filter(function(file) {
			return file.length == (4 + file.lastIndexOf(".src"));
		}).forEach_(_, function(_, file, i) {
			exports.requireScript.nodeScript[file.substring(0, file.lastIndexOf(".src"))] = __dirname + "/../../../etna/test/engine/server/" + file;
		});
	}
