...
function _iter(args: any[], fn: (v: any) => boolean) {
	for (var i = 0; i < args.length; i++) {
		...
		if (Array.isArray(val)) {
			...
			if (sliced) {
				var _doSlice = function (dim: number, off: number, mult: number) {
...
export const functions = {
	...
	FINDOBJ: {
		async: true,
		count: -1,
		fn(_: _,  val: any, target: any, property: string, range: NumberArray[]) {
	...
	MAXCOL: {
		async: true,
		count: -1,
		fn(_: _,  target: any, property: string, range: NumberArray[]) {
...
export const instructions = {
	SIGMA(field: string, args: any[]) {
		return function sigma$do(_: _) {
