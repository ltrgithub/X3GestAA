...
var flowControl = require('etna/lib/engine/runtime/flowControl');
...
var tracerJs = require('syracuse-core').getTracer("etna.engine");
...
exports.instructions = {
	...
	DECL(key, field, using, lens0, ranges0) {
		...
		return function decl$do(_) {
			...
			var arr;
			if (/^[LV]$/.test(letter))
				values[field] = undefined; // Kill : the value		
			...
			if (rangesVals && values[field] && values[field].lines) { // case instanceCollection				
	...
	XMET(target, name, args) { // fmet and callmet
