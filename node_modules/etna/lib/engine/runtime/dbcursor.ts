...
import { DBTableParent, KeyExpression, QueryKeys, WKey } from './dbtableparent';
...
function _save(tables: { [name: string]: DBTableParent }, columns?: QueryKeys) {
	const rec: any = {};
...
function _dispatch(tables: { [name: string]: DBTableParent }, row: Record, columns?: QueryKeys) {
...
export class DBCursor {
	...
	maptrim:  (_: _,  obj: Record) => any, pwhere: Expression< boolean[]>[], key: KeyExpression, lvl: number, typ: string, columns?: QueryKeys) {
		this.reader = reader;
		this.tables = tables;
