...
import { DBTableParent, KeyExpression, TableColumns, WKey } from './dbtableparent';
...
function _save(tables: { [name: string]: DBTableParent }, columns?: TableColumns) {
	const rec: any = {};
...
function _dispatch(tables: { [name: string]: DBTableParent }, row: types.Row, columns?: TableColumns) {
...
export class DBCursor {
	...
	tables: { [name: string]: DBTableParent };
	maptrim: (_: _,  obj: types.Row) => any;
	pwhere: types.Expression<boolean[]>[];
	key: KeyExpression;
	klvl: number;
	level: number;
	currkeyValues: any[];
	...
	pocket: any;
	lastRecord: types.Row;
	finished: boolean;
	typecursor: string;

	constructor(reader: ez.Reader<types.Row>, tables: { [name: string]: DBTableParent }, 
	maptrim:  (_: _,  obj: types.Row) => any, pwhere: types.Expression< boolean[]>[], key: KeyExpression, lvl: number, typ: string, columns?: TableColumns) {
		this.reader = reader;
		this.tables = tables;
	...
	x3GetRecord(_: _,  sv?: boolean) {
		...
		// save last record
		const vsave = _save(tables, this.columns);
