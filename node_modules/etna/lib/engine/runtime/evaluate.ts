...
const builders = {
	...
	DIM(node) {
		...
		if (field != null) return instructions.DIM(letter || "", field || "", rhs);
		else return instructions.DYNDIM.apply(null, node.args.map(_build));
	},
	TYPE(node) {
		if (node.field != null) return instructions.TYPE(node.letter, node.field);
		else return instructions.DYNTYPE(_build(node.exp));
	}
};
