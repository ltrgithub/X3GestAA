...
import { Expression, Instruction } from './types';
...
//var insts: { [name: string]: Instruction<any> };

import * as util from './util';
import * as errors from './errors';
const trace = core.getTracer("etna.engine");
...
type Builder = (Node: nodes.ParseNode) => Expression<any>;
const builders: { [name: string]:  Builder } = {
	...
	PROP(node: nodes.ParseMemberNode) {
	...
	OP(node: nodes.ParseOpNode) {
		return (operators.instructions as any)[node.op].apply(null, node.args.map(_build));
	},
	...
	SIGMA(node: nodes.ParseFieldArgsNode) {
		return stats.instructions.SIGMA(node.field, node.args.map(_build));
	},
	FUNC(node: nodes.ParseFuncNode) {
		//dbg:console.error("FUNC node.from:",node.from)
		//dbg:console.error("FUNC node.name:",node.name)
		if (node.from) {
			return flowControl.instructions.CALL(flowControl.instructions.REQ(_buildRequire, _build(node.from)), node.name as any, (node.args || []).map(_build));
		} else {
	...
	DIM(node: nodes.ParseArgsNode) {
		var letter: string, field: string, rhs: Expression<any>;
