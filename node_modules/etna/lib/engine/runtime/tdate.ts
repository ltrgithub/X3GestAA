...
const base = core.types.date;
const NULLORIGIN = base.make(1599, 12, 31);
export const MIN = base.make(1600, 1, 1);
export const MAX = base.make(9999, 12, 31);
export const NULL = base.make(0, 0, 0);
...
_prototype.x3ToInteger = function () {
	throw util.badTypeConversion('date to integer');
};
...
_prototype.x3ToDouble = function () {
	throw util.badTypeConversion('date to double');
};
...
_prototype.x3ToDatetime = function () {
	throw util.badTypeConversion('date to datetime');
};

_prototype.x3ToBCD = function () {
	throw util.badTypeConversion('date to decimal');
};

_prototype.x3Serialize = function (l: any) {
...
_prototype._addOrSub = function (v: any, op: number) {
	if (v.x3IsNumeric()) {
...
_prototype.x3Nday = function (origin: number) {
	return this.x3IsZero() ? new Number(-1) : new Number(this.daysDiff(origin));
};

_prototype.x3AddDays = function (n: number) {
	if (n.x3IsNumeric()) {
		if (n < 0) return NULL;
...
export const dayName = base.dayName;
export const today = base.today;
export const makeInWeek = base.makeInWeek;
export const monthName = base.monthName;
export const fromJsDate = base.fromJsDate;
export const isDate = base.isDate;

export type Date = core.types.date.DateValue;