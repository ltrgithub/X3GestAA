...
export interface FrameContext {
	...
	loginName: string;
	messages: string[];
	traceCall?: (_: _, name: string, arg: any) => void;
	traceFunc?: (_: _, name: string, arg: any) => void;
	table?: Dict<{}>; // check??
}
...
export interface SubFrame {
	...
	// do we need both ret and result?
	ret?: any;
	result?: any;
}
...
export interface Frame extends SubFrame {
	...
	loginName?: string;
	fcall?: string;
	tables?: Dict<VariableDict>; // really used??
}
...
export interface X3Session {
	...
	locale: string;
	actx: any; // fix later
	$queryCtx?: any; // RepresentationMeta?
	supervisor: Supervisor;
	_crudActions: ((_: _, event: string) => number)[];
}
...
export interface X3Context {
	x3frame: Frame;
	x3session: X3Session;
	debug: boolean;
	queryFrame: Frame; // TODO: review: why do we save restore frame???
	request: any; // see later - http request
	session: any; // http session
	endpointDataset: string;
}
...
export type QueryKeys = Dict<QueryKey[]>
