...
export class VGClass {
	table: DBTableParent;
	inFrontOf: string; // Q: should this be both on VGClass and GClass?
	constructor(tbl: DBTableParent) {
		this.table = tbl;
		this.inFrontOf = undefined;
	}

	get ADXFNAME() {
		const tb = Object.keys(this.table.desc.columns).reduce((o, kn) => {
			const c = this.table.desc.columns[kn];
	...
	get KEYNAME() {
		return this.table.keys.reduce((t, k) => {
	...
	get CURRIND() {
		const ck = this.table.currkey;

		for (var i = 0, j = 1; i < this.table.keys.length; i++) {
	...
	get CURRLEN() {
		const ck = this.table.keys[this.table.currkey];
