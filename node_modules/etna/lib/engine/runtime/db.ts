...
function closeFiles(abbrevs: string[], local: number) {
...
function rowIn(_: _, row: types.Row, descs: types.RowDesc[]) {
	const result: types.Row = {};
...
const sqlFindPat = {
	FIND(_: _, args: types.Expression<any>[], ctx: types.FrameWhereContext) {
	...
	PAT(_: _, args:  types.Expression<any>[], ctx: types.FrameWhereContext) {
...
function findTable(frame: types.Frame, abbrev: string) {
	...
	//debugger;
	const table = frame.context.tables[abbrev || frame.abbrev];
...
function keyAnalysis(_: _, cmd: string, key: types.Expression<any>, op: string, vals: any[]) {
	...
	switch (op) {
		...
		case 'EQ':
			...
			var keyw = exports.instructions.SQLKEY(k.name, op, k.indice ? basic.instructions.A([basic.instructions.C(k.indices)]) : basic.instructions.C(null), vals);
			break;
		case 'FIRST':
			break;
		case 'LAST':
...
export const instructions = {
	...
	KEY(letter: string, abbrev: string, name: string, indices) {
	...
	SQLDECL(type, name, ranges) {
		return function sqldecl$do(_: _,  abv) {
			const frame = util.currentFrame();
	...
	VAR(type, name, len) {
		return function var$do(_: _, abv) {
			const frame = util.currentFrame();
			const table = frame.context.tables[abv];
	...
	VTBV(field) {
		return function vtbv$do(_: _, analyse) {
	...
	FILEVAL(abb, vars, frv, where, order) {
		return function dot$do(_: _) {
			...
			const tbvar = frv.map_(_, (_: _, e) => e(_, 1));
	...
	FILESQL(abbrev, name, where, order) {
		return function file$do(_: _) {
			const frame = util.currentFrame();
			const driver = frame.context.superv.sqlDriver;
			const nameVal = name(_);
			const desc = frame.context.superv.load(_, 'Table', nameVal);
			...
			// init keys
			var kslot = -1;
			const tkeys = Object.keys(desc.indexes).map(name => {
				const idx = desc.indexes[name];
	...
	COLUMNSITEM(abbrev, field, index) {
		return function columnsitem$do(_: _, def) {
			const frame = util.currentFrame();
			const abv = abbrev || def;
	...
	COLUMNS(abbrev, columns, ext) {
		return function columns$do(_: _) {
			//debugger ;
			const frame = util.currentFrame();
			const table = findTable(frame, abbrev);
			const master = table.master || table;

			if (columns) {

				var _addColumn = function (r, t) {
					const n = r.length;
	...
	LINKITEM(key, outer, values) {
		return function linkitem$do(_: _, ctx) {
			const frame = util.currentFrame();
			const k = key(_);
	...
	LINK(abbrev, as, joins, where, order) {
		return function link$do(_: _) {
			...
			const lktable = frame.context.tables[as] = new DBTable(table, table.abbrev).init(_);
	...
	LOOK(key, op, values, lock, hint) {
		...
		return function look$do(_: _) {
			...
			const fst = ka.table.x3Read(_, ka.nkey, ka.kln, ka.wkey, ka.krevert, lock, hint);

			if (fst === 0 && (op === 'GE' || op === 'LE')) {
				if (ka.table.keyCompare(_, ka.nkey, vals) === 0) frame.context.sys.values.FSTAT = 0;
	...
	READ(key, op, values, lock, hint) {
		...
		return function read$do(_: _) {
			...
			const ka = keyAnalysis(_, 'read', key, op, vals);
			const fst = ka.table.x3Read(_, ka.nkey, ka.kln, ka.wkey, ka.krevert, lock, hnt);
	...
	REWRITE(key, op, values) {
		...
		return function rewrite$do(_: _) {
			...
			const vassig = [];
			const abv = (ka.table.master || ka.table).abbrev;
			const lst = ka.table.columns && ka.table.columns[abv];
			...
			if (lst) {
				...
				Object.keys(ka.table.desc.columns).forEach(cname => {
			...
			const fs = ka.table.x3Rewrite(_, ka.wkey, vassig);
	...
	DELETE(key, where, op, values) {
		...
		return function delete$do(_: _) {
			...
			const rs = ka.table.x3Delete(_, ka.nkey, ka.wkey, where);
	...
	ASSIGNUPDATE(lhs, rhs) {
		return function assignupdate$do(_: _,  ctx) {
			const frame = util.currentFrame();
	...
	FORF(key, hint, lock, where, from, to, p, end, block) {
		...
		return function forf$do(_: _) {
			...
			if (!cf) {
				...
				var wfrm = frmv.length !== 0 ? exports.instructions.SQLKEY(k.name, 'GE', basic.instructions.A([basic.instructions.C(table.currlen)]), frmv) : undefined;
				var wto = tov.length !== 0 ? exports.instructions.SQLKEY(k.name, 'LE', basic.instructions.A([basic.instructions.C(table.currlen)]), tov) : undefined;
	...
	SQLINDEX(field, args) {
		...
		return function sqlindex$do(_: _) {
			const frame = util.currentFrame();
			const driver = frame.context.superv.sqlDriver;
			const ctx = frame.whereContext;
