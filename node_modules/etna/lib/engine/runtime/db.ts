...
import { ContextFn, Expression, Frame, QueryContext, RowDesc, VariableDict, X3Function, WKey, TableKey, 
	QueryKey, TableJoin, KeyExpression, KeyModified } from './types';
...
export const functions = {
	...
	CLANBS: {
		count: 2,
		fn(cla: VariableDict, kind: number) {
	...
	CLASIZ: {
		count: 2,
		fn(dict: VariableDict, exp_buf: void) {
			return 16;
		}
	},
	CLANAM: {
		count: 1,
		fn(dict: VariableDict) {
			return 0;
		}
	},
	CLAVAR: {
		count: 2,
		fn(dict: VariableDict, n: number) {
			var i = 1;
			var name = "";
...
function binaryOp(op: string, sqlOp: string, type: string, numOp?: string, numType?: string) {
	return function (op1: any, op2: any) {
		return function binaryOp$do(_: _) {
			...
			if (!frame.tryingSql) return (operators.instructions as any)[op](op1, op2)(_);
			var v1 = genSql(_, op1);
...
function _closeFile(frame: Frame, abbrev: string) {
...
export function closeFiles(abbrevs: string[], local: number) {
	const frame: Frame = util.currentContext().x3frame;
...
function rowIn(_: _, row: Record, descs: RowDesc[]) {
	const result: Record = {};
...
type SqlPatFn = (_: _, args: Expression<any>[], ctx: QueryContext) => WKey;
...
function findTable(frame: Frame, abbrev: string) {
	// use frame's default abbrev by default
	//debugger;
	const table: DBTable = frame.context.tables[abbrev || frame.abbrev] as any;
...
function keyAnalysis(_: _, cmd: string, key: Expression<any>, op: string, vals: any[]) {
	const frame = util.currentContext().x3frame;
	const session = util.currentContext().x3session;
	const driver = frame.context.superv.sqlDriver;
	const k = key(_);
	...
	const nkey = table.findKey(k ? k.name : "");
...
export const instructions = {
	...
	KEY(letter: string, abbrev: string, name: string, indices: Expression<any>[]) {
	...
	EXECSQL(fsql: Expression<string>) {
	...
	ANASQL(fsql: Expression<string>, fvar: Expression<number[]>) {
	...
	FORSQL(fsql: Expression<string>, abv: string, vars: ((_: _, abv: string) => SqlColumnDesc)[], 
	p: Expression<any>[], end: number, block: Expression<any>) {
	...
	VAR(type: string, name: string, len: Expression<number>) {
	...
	VTBINDEX(target: Expression<any>, args: Expression<number>[]) {
	...
	FILESQL(abbrev: string, name: Expression<string>, where: ContextFn<any>, order: ContextFn<any>): Expression<DBTable> {
		return function file$do(_) {
			const frame = util.currentContext().x3frame;
			const driver = frame.context.superv.sqlDriver;
			const nameVal = name(_);
			const desc = frame.context.superv.load<Table>(_, 'Table', nameVal);
	...
	CLOSEFILE(scope: string, abbrevs: string[]) {
		return function closefile$do(_: _) {
			closeFiles(abbrevs, scope === 'LOCAL' ? util.currentContext().x3frame.stackLevel : 0);
		};
	},
	LOGICCLOSE(scope: string, abbrevs: string[]) {
		return function logicclose$do(_: _) {
			closeFiles(abbrevs, util.currentContext().x3frame.stackLevel);
		};
	},
	...
	COLUMNSITEM(abbrev: string, field: string, index: Expression<number>): Expression<QueryKey> {
		return function columnsitem$do(_, def) {
			const frame = util.currentContext().x3frame;
			const abv = abbrev || def as any; // weird typing for def paramater!
	...
	COLUMNS(abbrev: string, columns: ContextFn<QueryKey>[], ext: boolean): Expression<void> {
		return function columns$do(_) {
			//debugger ;
			const frame = util.currentContext().x3frame;
			const table = findTable(frame, abbrev);
			const master = (table as any).master || table;
			...
			if (columns) {
				...
				// typing??
				(table.columns as any).extended = ext;
			} else {
	...
	HINT(nohint: any, key: Expression<any>) {
	...
	LINKITEM(key: Expression<LinkItem>, outer: boolean, values: Expression<any>[]): ContextFn<TableKey> {
	...
	LINK(abbrev: string, as: string, joins: ContextFn<TableJoin>[], where: ContextFn<WKey[]>, order: ContextFn<KeyExpression>) {
		return function link$do(_: _) {
			...
			const lktable = frame.context.tables[as] = new DBTable(table as any, table.abbrev).init(_);
	...
	LOOK(key: Expression<any>, op: string, values: Expression<any>[], lock: boolean, hint: Hint) {
	...
	READ(key: Expression<any>, op: string, values: Expression<any>[], lock: boolean, hint: Expression<Hint>) {
		const avalues = basic.instructions.A(values);
	...
	REWRITEBYKEY(key: void, op: void, values: Expression<any>[]) {
		const avalues = basic.instructions.A(values);
	...
	REWRITE(key: Expression<any>, op: string, values: Expression<any>[]) {
		const avalues = basic.instructions.A(values);
	...
	DELETE(key: Expression<any>, where: ContextFn<WKey[]>, op: string, values: Expression<any>[]) {
		const avalues = basic.instructions.A(values);
	...
	ASSIGNUPDATE(lhs: Expression<any>, rhs: Expression<any>): ContextFn<any> {
	...
	UPDATE(abbrev: string, where: ContextFn<WKey[]>, assign: ContextFn<WKey>[]) {
		return function update$do(_: _) {
			const frame = util.currentContext().x3frame;
			const session = util.currentContext().x3session;
			const table: DBTable = findTable(frame, abbrev) as any;
			const ctx: QueryContext = {};
	...
	FORF(key: Expression<any>, hint: Expression<Hint>, lock: boolean, where: ContextFn<WKey[]>, 
	from: Expression<any>[], to: Expression<any>[], p: Expression<any>[], 
	end: number, block: Expression<any>) {
	...
	WHERE(wheres: Expression<WKey>[]) {
		return function (_: _,  ctx: QueryContext) {
			const frame = util.currentContext().x3frame;
			const sqls: string[] = [];
	...
	ORDER(keyName: string, by: ContextFn<WKey>[], withExp: Expression<string>, direct: number, klen: Expression<number[]>): ContextFn<WKey> {
		return function (_,  ctx) {
			//debugger;
			const frame = util.currentContext().x3frame;
	...
	SQLINDEX(field: Expression<string>, args: Expression<any>[]) {
	...
	SQLCV(letter: string, abbrev: string, field: string): Expression<WKey[]> {
	...
	SQLF(field: string, args: Expression<any>[]) {
	...
	SQLEVALUE(req: (name: string) => any, ns: any, arg: Expression<any>) {
	...
	SQLKEY(kyname: string, op: string, np: Expression<any>, args: Expression<any>[]): Expression<WKey> {
		//
		return function sqlsql$do(_) {
			...
			if (op === 'EQ' || op === "NE" || npk === 1) {
				...
				for (var i = 0; i < npk; i++) {
					...
					consts = consts.concat(s.constants || []);
					var fequal = (instructions as any)['SQL' + op](key.func[i], s.func);
				...
				for (var i = 0; i < npk; i++) {
					...
					for (var j = 0; j < kp; j++) {
						...
						if (j < kp - 1) {
							...
							f = (instructions as any)['SQL' + op](key.func[j], s.func);
						}
						consts = consts.concat(s.constants || []);
