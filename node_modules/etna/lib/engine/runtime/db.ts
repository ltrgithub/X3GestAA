...
import { DBTableParent, WKey, TableKey, QueryKey, TableJoin } from './dbtableparent';
...
const sqlFindPat = {
	FIND(_: _, args: types.Expression<any>[], ctx: types.QueryContext) {
	...
	PAT(_: _, args:  types.Expression<any>[], ctx: types.QueryContext) {
...
export const instructions = {
	...
	SQLDECL(type: string, name: string, ranges: void): types.SqlColumnDesc {
		return function sqldecl$do(_: _,  abv: string) {
			const frame = util.currentFrame();
	...
	FORSQL(fsql: types.Expression<string>, abv: string, vars: ((_: _, abv: string) => types.SqlColumnDesc)[], 
	p: types.Expression<any>[], end: number, block: types.Expression<any>) {
		return function forsql$do(_: _) {
			...
			const fieldList: types.SqlColumnDesc[] = []; /* declare field */
	...
	FILEVAL(abb: string, vars: ContextFn<any>[], frv: types.Expression<any>[], 
	...
	FILESQL(abbrev: string, name: types.Expression<string>, where: ContextFn<any>, order: ContextFn<any>): types.Expression<TableKey> {
	...
	COLUMNS(abbrev: string, columns: ContextFn<QueryKey>[], ext: any) {
	...
	LINKITEM(key: types.Expression<QueryKey>, outer: boolean, values: types.Expression<any>[]): ContextFn<TableKey> {
	...
	LINK(abbrev: string, as: string, joins: ContextFn<TableJoin>[], where: ContextFn<WKey[]>, order: ContextFn<TableKey>) {
	...
	LOOK(key: types.Expression<any>, op: string, values: types.Expression<any>[], lock: boolean, hint: types.Hint) {
	...
	READ(key: types.Expression<any>, op: string, values: types.Expression<any>[], lock: boolean, hint: types.Expression<types.Hint>) {
		const avalues = basic.instructions.A(values);
