...
import * as dates from '../runtime/dates';
...
export function generate(tabcor: { [name: string]: string; }, node: ParseNode) {
	...
	const VALUES = ["STRING"];
	const opValue = function (operator: string, op0: any, op1: any) {
		return function () {
			var l: string, r: string;
	...
	const addOpValue = function (op: string) {
	...
	function findValues(operator: string, find: ParseNode, value: ParseNode) {
		return function () {
			const l = gen(find.args[0])();
			const vals: any[] = Array.prototype.slice.call(find.args, 1);
	...
	const operators: { [name: string]: (node: ParseNode) => () => any; } = {
		EQ: addOpValue("eq"),
		NE: addOpValue("ne"),
		GE: addOpValue("ge"),
		LE: addOpValue("le"),
...
export function x3ToSdata(tabcor: { [name: string]: string; }, expression: string) {
