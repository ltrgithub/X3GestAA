...
exports.generate = function(node, fname) {
	...
	function error(node, message) {
		...
		return new X3Error(26, fname + ':' + node.line + ': '+ message, fname, node.line, scopeName);
	}
	...
	var generators = {
		...
		FCALL: function(node, variant) {
			...
			if (node.name === "MAX" && node.args && node.args[0].tag === "PROP") {
				...
				if (decl.tag === 'PARAM') {
					...
					if (fctRANGE.indexOf(node.type) >= 0 && decl.ranges && decl.ranges.length === 1 && !decl.ranges[0].args) {
						...
						if (typeof decl.lens[0].value == 'number') {
							...
							Object.keys(decl.lens[0]).forEach(function(k) {
		...
		CONSIGN: function(node) {
			used.CONSIGN = true;
			return insert(node, "CONSIGN(" + JSON.stringify(node.data) + ")");
		},
		CHECK: function(node) {
			used.pragma = true;
			var callPragmaFunction = "";
			if(node.label) {
				callPragmaFunction = insert(node, "pragma(" + JSON.stringify(node.label) + ")");
			}
			return callPragmaFunction;
		}
	};
	...
	var file = head + ";z.BEGIN(module);NL(0);" + result;
	return file;
};