			...
			if (result.rows) return result.rows;
				...
				return {
					...
					outputVars: opts.outputVars && opts.outputVars.map((v) => {
						...
						return ez.devices.generic.writer((_, data) => {
...
// console.error("ORACLE READER: ", sql, args);
		...
		readTableSchema(_, tableDef, tracker) {
			...
			var sqlQuery = "SELECT c.COLUMN_NAME, c.DATA_TYPE, c.DATA_LENGTH, c.CHAR_USED, c.CHAR_LENGTH, c.DATA_PRECISION, c.DATA_SCALE, c.NULLABLE," +
				...
				" LEFT JOIN " + this.getFullTableName(tableDef.schemaName, "ATABZON") + " z ON z.CODFIC_0 = c.TABLE_NAME AND c.COLUMN_NAME LIKE CONCAT(z.CODZONE_0, '\\_%') ESCAPE '\\'" +
				" LEFT JOIN " + this.getFullTableName(tableDef.schemaName, "ATYPE") + " t ON z.CODTYP_0 = t.CODTYP_0" +
				" WHERE table_name = :1" +
		...
		evalueRowCount(_, tableName) {
			var rs = this.reader(_, "select num_rows from user_tables where table_name =" + this.param(0), [tableName]).toArray(_);
