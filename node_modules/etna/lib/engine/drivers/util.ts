...
export function fromSql(_: _,  val: any, type: number, driver: types.Driver) {
...
export function toSql(params: any[], driver: types.Driver) {
...
export function maptrim(driver: types.Driver, typesmap, dispatch) {

	return function (_: _,  obj) {
		if (dispatch) {
			var result = Object.keys(obj).reduce_(_, (_, record, k, idx) => {
				const fromSql = exports.fromSql;
...
export function mapFromSql(driver: types.Driver, abv: string, mapsql) {
	return function (_: _,  obj: types.Row) {
		const result = {};
