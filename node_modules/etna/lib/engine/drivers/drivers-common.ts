...
export function createTableReader<T>(_: _,  driver: Driver, tableDef: TableDefinition, filters: TableFilter[]): Reader<T> {
	...
	return driver.reader<T>(_, sql, filterValues);
};
...
export type ReaderMaker<T> = (_: _, cnx: Connection, sql: string, args: any[], opts?: DriverReaderOptions) => ez.Reader<T>;
export type WriterMaker<T> = (_: _, cnx: Connection, sql: string, opts?: SqlExecuteOptions) => ez.Writer<T>;

export function reader<T>(creator: ReaderMaker<T>): ReaderMaker<T> {
...
export function writer<T>(creator: WriterMaker<T>): WriterMaker<T> {
