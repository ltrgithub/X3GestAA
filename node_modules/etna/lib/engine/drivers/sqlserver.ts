...
import { BaseDriver, ColumnDefinition, Connection, Driver, DriverConfig, DriverReaderOptions, Hint, 
	IndexDefinition, Record, SqlColumnInfo, SqlExecuteOptions, TableDefinition } from '../drivers/types';
...
function fillParameters(req: tds.Request, args: any[]) {
	function processParam(name: any, type: tds.TediousType, value: any) {
		const param = (req as any).parametersByName[name];
...
function describeColumns(request: tds.Request, columnDefs: { [name: string]: ColumnDefinition }) {
...
function getConfig(config: DriverConfig): tds.ConnectionConfig {
...
module.exports = function (config: DriverConfig): Driver {
	...
	const driver: BaseDriver = {
		...
		nullDate() {
			return new Date(Date.UTC(1753, 0, 1));
		},
		...
		reader: common.reader((_: _, cnx: Connection, sql: string, args: any[], opts: DriverReaderOptions) => {
			return eztedious.reader(cnx, sql, args, {
				fillParameters: fillParameters
			}).map((_: _, row: any) => rowIn(row));
		}),
		// opts parameter is strange here...
		writer: common.writer((_: _, cnx: Connection, sql: string, opts: any) => {
		...
		readTableSchema(_,  tableDef, tracker) {
			...
			var currentIndex: IndexDefinition;
		...
		createTableWriter(_,  tableDef) {
			...
			const columnDefs: { [name: string]: ColumnDefinition } = {};
			...
			return this.withConnection(_, (_: _, cnx: Connection) => {
