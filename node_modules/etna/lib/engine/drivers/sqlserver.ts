...
//var sbuf = require('etna/lib/engine/drivers/sqlbuffer') ;
...
function tds_query(cfg, cnx, sql, args, callback) {
	...
	const records = [];
	const req = new tds.Request(sql, (err, rowCount) => {
...
module.exports = function (config) {
	...
	return {
		...
		beginTransaction(_: _,  cnx) {
			cnx.beginTransaction(_);
		},
		commitTransaction(_: _,  cnx) {
			cnx.commitTransaction(_);
		},
		rollbackTransaction(_: _,  cnx) {
		...
		closeConnections(_: _) {
			connections.forEach_(_, (_, cnx) => {
				cnx && tds_close(cnx, _);
			});
		},

		execute(_: _,  cnx, sql, args, opts) {
			...
			const result = tds_query(cfg, cnx, sql, args, _);
		...
		tableExists(_: _,  tableName, schemaName) {
		...
		getTableDef(_: _,  schemaName, tableName) {
			const tableDef = {
		...
		readTableSchema(_: _,  tableDef, tracker) {
			// ----------------------------------------
			...
			tableDef.columns = this.reader(_, sqlQuery).toArray(_).map(row => {
				if (!tableId) {
			...
			var currentIndexId;
			tableDef.indexes = [];
			this.reader(_, sqlQuery).toArray(_).forEach(row => {
		...
		createTableFromTableDefinition(_: _,  tableDef, options) {
			options = options || {};
		...
		readTables(_: _,  schemaName) {
			...
			this.reader(_, sql).forEach(_, (_, item) => {
				tables.push({
					schemaName: schemaName,
					tableName: item.name
				});
		...
		createTableReader(_: _,  tableDef, filters) {
			return common.createTableReader(_, this, tableDef, filters);
		},

		createTableWriter(_: _,  tableDef) {
			const columnNames = [];
			const paramNames = [];
			const columnDefs = {};
			...
			var writer;
			this.withConnection(_, (_, cnx) => {
				writer = this.writer(_, cnx, sql, columnDefs);
			});
			return writer;
		},
		...
		dropSequence(_: _,  tableDef) {
			return common.dropSequence(_, this, tableDef);
		},
		evalueRowCount(_: _,  tableName) {
			this.reader(_, "select top 1 rows from sysindexes where id = object_id('" + param(0) + "') and indid < 2", [tableName]).toArray(_);
			return 0 / 0; //TODO ?
		},
		release(_: _) {}
	};
