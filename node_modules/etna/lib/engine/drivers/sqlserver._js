...
module.exports = function(config) {
	...
	return {
		...
		blobReader: function(_,val) {
			return ez.devices.buffer.reader(val);
		},
		clobReader: function(_,val) {
		...
		releaseConnection: function(cnx) {
		...
		doubleType: function() { // for now handle as char to keep full precision
			return "float"; // float is double (8 bytes), and real is single (4 bytes)
		},

		tableExists: function(_, tableName, schemaName) {
		...
		readTableSchema: function(_, tableDef, tracker) {
			...
			var columnsById = [];
			tableDef.columns = _this.reader(_, sqlQuery).toArray(_).map(function(row) {
				if (!tableId) {
			...
			var currentIndexId;
			tableDef.indexes = [];
			_this.reader(_, sqlQuery).toArray(_).forEach(function(row) {
		...
		readTables: function(_, schemaName) {
			...
			_this.reader(_, sql).forEach(_, function(_, item) {
				tables.push({
					schemaName: schemaName,
					tableName: item.name
				});
		...
		clearTable: function(_, tableDef) {
			var _this = this;
			_this.execute(_, "DELETE FROM " + _this.getFullTableDefName(tableDef));
		},
		...
		evalueRowCount: function(_, tableName) {
			var _this = this;
			_this.reader(_, "select top 1 rows from sysindexes where id = object_id('" + param(0) + "') and indid < 2", [tableName]).toArray(_);
			return 0 / 0; //TODO ?
		},
		release: function(_) {
		},
