...
function tds_query(cfg, cnx, sql, args, callback) {
	...
	var records = [];
	var req = new tds.Request(sql, (err, rowCount) => {
...
module.exports = function(config) {
	...
	return {
		...
		execute(_, cnx, sql, args, opts) {
			...
			console.error("EXECUTE", sql, args && args
				.map(arg => typeof arg + " isbuf=" + Buffer.isBuffer(arg) + " len=" + (arg && arg.length) + ': ' + (arg && (arg.length > 100 
					? arg.slice(0, 100) + "..." 
		...
		decimalType() { // for now handle as char to keep full precision
			return "numeric";
		},
		doubleType() { // for now handle as char to keep full precision
			return "float"; // float is double (8 bytes), and real is single (4 bytes)
		},

		tableExists(_, tableName, schemaName) {
			return !!this.reader(_, 
				"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", 
				[schemaName, tableName]).toArray(_)[0];
		},
		...
		readTableSchema(_, tableDef, tracker) {
			...
			var row = this.reader(_, sqlQuery, [tableDef.tableName]).toArray(_)[0];
		...
		createTableWriter(_, tableDef) {
			...
			tableDef.columns.forEach((column, index) => {
				...
				var paramName = this.param(index);
				paramNames.push(paramName);
				columnDefs[paramName] = column;
			});

			var sql = "INSERT INTO " + this.getFullTableDefName(tableDef) + " (" + columnNames.join(',') + ") VALUES (" + paramNames.join(',') + ")";
			var writer;
