...
module.exports = function (config: types.DriverConfig): types.Driver {
	...
	const driver: types.BaseDriver = {
		...
		readTables(_,  schemaName) {
			...
			return this.reader(_, sql).map((_: _, item: types.Row) => {
				return {
		...
		release(_) {
			...
			// Reset counters:
			Object.keys(counters).forEach(key => any(counters)[key] = 0);
		}
	};
	return any(driver);
};