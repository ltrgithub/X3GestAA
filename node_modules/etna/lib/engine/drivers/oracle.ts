...
import { BaseDriver, Connection, Driver, DriverConfig, Hint, Record, SqlColumnInfo, SqlExecuteOptions, TableDefinition } from '../drivers/types';
import * as ez from 'ez-streams';
import * as crypto from 'crypto';
const ezoracle = require('ez-oracle');
const oracle = require('oracledb');
import * as common from './drivers-common';
const tracerJs = core.getTracer("etna.dbms");

//tracerJs = { debug: console.error }

const _months = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];

function setFetchInfo(opts: SqlExecuteOptions) {
...
type OracleOptions = any; // refine later
...
module.exports = function (config: DriverConfig): Driver {
	...
	const driver: BaseDriver = {
		...
		toSqlDate(val) {
			var date: any;
			// very strange: Date.parse returns a number!
		...
		execute(_: _,  cnx?: Connection, sql?: string, args?: any[], opts?: SqlExecuteOptions) {
			...
			if (cnx == null) {
				return this.withConnection(_, (_: _, cnx: Connection) => this.execute(_, cnx, sql, args, opts));
			}
			tracerJs.debug && tracerJs.debug("execute " + sql + ",args=\n" + (args || []).map((arg, i) => ":" + (i + 1) + "=" + arg).join('\n'));
			const oraOpts: OracleOptions = {
		...
		describeQuery(_,  qry, folder) {
			...
			try {
				...
				return this.reader(_, sql).toArray(_).map((row: DescribeRow) => {
					const col: SqlColumnInfo = {};
					...
					switch (row.DATA_TYPE) {
						...
						case 'TIMESTAMP(3)':
							...
							break;
						case 'VARCHAR':
							col.type = 10 + +row.DATA_LENGTH;
							col.typtyp = 7;
							break;
						case 'VARCHAR2':
							col.type = 10 + +row.CHAR_LENGTH;
							col.typtyp = 7;
							break;
						case 'RAW':
							...
							break;
						case 'NUMBER':
							var precision = +row.DATA_PRECISION;
							var scale = +row.DATA_SCALE;
		...
		readTables(_,  schemaName) {
			...
			return this.reader(_, sql).map((_: _, item: Record) => {
				return {
		...
		readTableSchema(_,  tableDef, tracker) {
			...
			tableDef.columns = this.reader(_, sqlQuery, [tableDef.tableName]).toArray(_).map((row: any) => {
				const column: any = {
				...
				switch (row.DATA_TYPE) {
					...
					case 'NUMBER':
						...
						if (10 === precision && !scale) {
							...
							var precision: number = row.ZONE_LENGTH;
							if (precision == 0) {
			...
			var currentIndex: any; // refine later
			var currentIndexName: string;
			tableDef.indexes = [];
			this.reader(_, sqlQuery, [tableDef.tableName]).toArray(_).forEach((row: any) => {
			...
			this.reader(_, sqlQuery, ["SEQ_" + tableDef.tableName]).toArray(_).forEach((row: any) => {
		...
		createTableWriter(_,  tableDef) {
			...
			return this.withConnection(_, (_: _, cnx: Connection) => this.writer(_, cnx, sql));
		},
		...
		release(_) {
			...
			// Reset counters:
			Object.keys(counters).forEach(key => (counters as any)[key] = 0);
		}
	};
