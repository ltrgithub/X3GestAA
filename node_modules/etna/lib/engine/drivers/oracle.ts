...
import { BaseDriver, Driver, DriverConfig, Record } from '../drivers/types';
...
function setFetchInfo(opts: types.SqlExecuteOptions) {
...
module.exports = function (config: DriverConfig): Driver {
	...
	const driver: BaseDriver = {
		...
		hintClause0(hnt: types.Hint, def: any) {
			// rf : read or for
			if (hnt) {
		...
		getTableDef(_,  schemaName, tablename) {
			const tableDef: types.TableDefinition = {
				schemaName: schemaName,
				tableName: tablename
			};
		...
		createTableWriter(_,  tableDef) {
			...
			return this.withConnection(_, (_: _, cnx: types.Connection) => this.writer(_, cnx, sql));
		},
