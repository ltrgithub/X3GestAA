...
if (!Object.assign) Object.assign = function(d, s) { Object.keys(s || {}).forEach(function(k) { d[k] = s[k] }); return d; };
...
export function mkdirsSync(path) {
	var p = "",
	    i = 0;
...
export function cachedPath(path, options) {
	...
	require.extensions['.ts'] = function (module, filename) {
		if (isX3js(filename)) {
			const transformed = cache.get(filename, {
				...
				if (!/^import x3js\b/.test(source)) throw new Error(`${filename} is not an X3JS file`);
