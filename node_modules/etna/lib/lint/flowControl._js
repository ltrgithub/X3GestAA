...
exports.instructions = {
	REQ:function(req, v) {
		...
		return function req$do(_) {
			try {
				var lint = glob.context.lint;
				lint.from = v ? v(_) : undefined;
				return lint.require(_,lint.from,loc);
			} catch(e) {
				console.log(e.message);
				console.log(e.stack);
				
				lint.modulesNotFound[lint.from] = true;
				loc.file = req ? (loc.file || lint.from) : lint.from;
				
				var message = e.message.split('\n')[0];

				if(/^Cannot find module/.test(message)) {
					message = message.replace(/^.*[\\\/]/, '').replace(/'/g,'');
					lint.addDiagnose("fatal","Cannot find module "+message,loc);
					...
					lint.addDiagnose("fatal","Parsing error " + message,loc);
				}
				else lint.addDiagnose("fatal","REQ "+ message +" ("+lint.from+")",loc);
			}
			return;
		};
	},
		...
		return function gosub$do(_) {
          	...
          	var nsVal = load(_,ns);
	   		if(nsVal && checkLabel(nsVal,label,loc)){
	            var script = lint.getLocFilename(nsVal[label].loc);
	            var key    = script + "." + label;
	            if(lint.labels[key] !== undefined) return;
	            lint.labels[key] = true;               
