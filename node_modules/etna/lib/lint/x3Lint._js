...
function clearCacheAccordingToSize(size, cache, nb) {

	var sourceFiles = Object.keys(cache);
	if(sourceFiles.length === size) {
		console.log("reached the size : " + size);
		...
		console.log("deleting "+ nb + " cache item");
	}
}
...
class X3Lint {
	...
	require(_, name, loc) {
	  	...
	  	this.from = name;
		clearCacheAccordingToSize(100,this.cache,25);
		try {
