...
export class X3Lint {
	constructor(root, options) {
		...
		this.cache = {};
		util.currentContext().lint = this;
		variables.initStack({});
		util.currentContext().x3frame.context.undecl = { types: {} };

		const sys = util.currentContext().x3frame.context.sys;
	...
	getDiagnoses() {
		const types = util.currentContext().x3frame.context.undecl.types;
		...
		module && Object.keys(module).forEach_(_, (_, prog) => {
			if (module[prog].name && module[prog].name[0] !== '$') {
				if (module[prog].name !== 'wrapper') {
					...
					util.currentContext().x3frame.checkV = false;
