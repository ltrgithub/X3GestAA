...
exports.get = function(_, cookie, url, statusCode, facet) {
	...
	if (response.statusCode >= 500 && response.statusCode !== statusCode) ok(false, 'STATUS 500 on GET: ' + url);
...
exports.post = function(_, cookie, url, data, statusCode, returnFullResponse) {
	...
	if (response.statusCode >= 500 && response.statusCode !== statusCode) ok(false, 'STATUS 500 on POST: ' + url);
	if (typeof(statusCode) != 'undefined') {
...
exports.delete = function(_, cookie, url, data, statusCode) {
	try {
		...
		if (response.statusCode >= 500 && response.statusCode !== statusCode) ok(false, 'STATUS 500 on DELETE: ' + url);
	} catch (e) {
...
exports.put = function(_, cookie, url, data, statusCode, returnFullResponse) {
	...
	if (response.statusCode >= 500 && response.statusCode !== statusCode) ok(false, 'STATUS 500 on PUT: ' + url);
	if (typeof(statusCode) != 'undefined') {
		/*dbg*/
