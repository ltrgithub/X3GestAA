...
config.unit_test.x3endpoint = config.unit_test.x3endpoint || {
	dataset: "x3_unit_test",
	user: "QLF",
	folder: "SUPERV",
	x3SolutionName: "SUPDVLP",
	serverHost: "AWS-X3-DEVSUP",
	serverPort: 17000,
	localPort: 3004
};

var port = config.unit_test.x3endpoint.localPort;
...
exports.url = function(_url) {
	return "/sdata/x3/erp/" + config.unit_test.x3endpoint.dataset + "/" + (_url || "");
};

exports.x3Url = function(_url) {
	return baseUrl + "/sdata/x3/erp/" + config.unit_test.x3endpoint.dataset + "/" + (_url || "");
};
...
exports.get = function(_, cookie, url, statusCode, facet) {
	...
	if (0) { //config.QLF && config.QLF.logPath)
		var logpath = config.unit_test.x3endpoint.logPath;
		var now = new Date();
...
function _createObjects(_) {
	...
	// x3solution    ("serverHost": "172.28.16.106",)
	body = this.post(_, cookie, "x3solutions", {
		code: "X3 Unit Test Server",
		description: "X3 Unit Test Server",
		solutionName: config.unit_test.x3endpoint.x3SolutionName,
		serverHost: config.unit_test.x3endpoint.serverHost,
		serverPort: config.unit_test.x3endpoint.serverPort,
		proxy: false,
		serverTimeout: 60000,
		$actions: {
			$save: {
	...
	body = this.post(_, cookie, "endPoints", {
		...
		dataset: config.unit_test.x3endpoint.dataset,
		enableSearch: false,
		protocol: "syracuse",
		...
		x3ServerFolder: config.unit_test.x3endpoint.folder,
		groups: [{
			$uuid: grpId
		}]
	}, 201);
	...
	body = this.put(_, cookie, "users(login eq 'admin')?representation=user.$edit", {
		endpoints: [{
			$index: 0,
			login: config.unit_test.x3endpoint.user,
			endpoint: {
				$uuid: epId
			}
		}]
	...
	strictEqual(body.endpoints[0].login, config.unit_test.x3endpoint.user, "Endpoint added to user");
...
exports.runTestCase = function(_, script, test) {
	exports.runTestCase.cookie = exports.runTestCase.cookie || exports.getCookie(_);

	var results = exports.get(_, exports.runTestCase.cookie, exports.x3Url("$service/QLF?script=" + script + "&test_case=" + test), 200);

	if (Array.isArray(results)) {
		results.reverse().forEach(function(result) {
			ok(result.ok, script + "." + test + ":" + result.message);
		});
	} else {
		ok(false, script + "." + test + " failed");
	}
};