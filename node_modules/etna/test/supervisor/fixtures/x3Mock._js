...
function X3Mock() {
	...
	const connectionRequest = (request) => {
		...
		this.challenge = helpers.uuid.generate();
		var respBody = {
			id: "123456",
			auth: {
				method: "X3RSA",
	...
	this.onSessionRequest = request => {
		...
		verifier.update(_makeToSignChallenge(request.body.auth.user, this.challenge));
		var respHead;
	...
	const dispatchRequest = request => {
		...
		if (request.head.url && (request.head.url == "/$connections")) {
			return connectionRequest(request);
		}
		if (request.head.url && (request.head.url == "/$sessions")) {
			return this.onSessionRequest(request);
		}
		return {
			head: {
				status: 404
	...
	const writeResponse = (_, c, response) => {
	...
	this.close = () => {
		if (this.crtSocket) this.crtSocket.end();
	};

	this.server = ez.devices.net.server((c, _) => {
		try {
			this.crtSocket = c;
			var data = c.read(_, 4);
