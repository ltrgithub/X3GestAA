...
module.exports = {
  ...
  operations: {
    clean: function(_) {
      this.$tables.AQCLNK01$.clear(_);
    },
    setup: function(_) {
      const table = this.$tables.AQCLNK01$;
      this.$withTransaction(_, function(_) {
        for (var i = 1; i <= 100; i++) {
          const j = 1 + (i - 1) % 10;
          const k = i % 5;
          var rec = {
    ...
    op1: function(_, f1) {
      if (parseInt(f1.substring(3)) < 50) {
        const rec = this.$tables.AQCLNK01$.readRecord(_, f1);
    ...
    op5: function(_, f1) {
      if (parseInt(f1.substring(3)) < 50) {
        const watb = this.$classes.TableDictionary$.read(_, "AQCLNK01");
    ...
    op6: function(_, f1) {
      if (parseInt(f1.substring(3)) < 50) {
        const aql01 = this.$classes.Aqclnk01$.read(_, f1);
        const waus = this.$classes.Autilis$.read(_, aql01.user$);
        return waus.photo$;
      } else {
        return x3js.blob.fromBase64("R0lGODlhBgAJAJECAPj3+NXt/wAAAAAAACH5BAEAAAIALAAAAAAGAAkAAAINlC+AqQfREmygOrYiKgA7");
      }
    },
    opp1: function(_, f1, p1, p2, p3, p4, p7, p8, p11, p12) {
    ...
    opp2: function(_, f1, p1, p2, p3, p4, p7, p8, p11, p12) {
      ++p1;
      ++p2;
      ++p3;
      ++p4;
  ...
  methods: { 
    ...
    mgetdat: function(_) {
      if (parseInt(this.field01$.substring(3)) < 50) {
        var res;
        const rec = this.$tables.AQCLNK01$.readRecord(_, this.field01$);
