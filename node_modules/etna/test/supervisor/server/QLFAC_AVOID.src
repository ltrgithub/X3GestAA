...
Call ADD_TESTCASE("AVOID_TEST","AVOID test",27) From AXUNIT
...
Setlob MY_CLOB2 With "ceci est le test de CLOB"
Call CHECK_EQUAL(num$(MY_CLOB2<>MY_CLOB)-"MY_CLOB2 not empty","1 MY_CLOB2 not empty") From AXUNIT

Local Blbfile MY_BLOB2(1)
Local File ABLOB [AAA]
For [AAA] Where CODBLB<>""
 If [F:AAA]BLOB<>MY_BLOB
  MY_BLOB2=[F:AAA]BLOB
  Break
 Endif
Next
Close Local File [AAA]
Call CHECK_EQUAL(num$(MY_BLOB2<>MY_BLOB)-"MY_BLOB2 not empty","1 MY_BLOB2 not empty") From AXUNIT
...
####NOT OK FOR THE MOMENT #### AVOID.AINS=ASHW
