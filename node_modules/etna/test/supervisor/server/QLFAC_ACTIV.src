...
Funprog TESTSUITE()
  ...
  Call ADD_TESTCASE("ACTIV_AINSERT","Creation of ACTIV record ZUTST",10)         From AXUNIT
  Call ADD_TESTCASE("ACTIV_AREAD","Read ACTIV record ZUTST",10)                 From AXUNIT
  Call ADD_TESTCASE("ACTIV_AUPDATE","Update ACTIV record ZUTST",6)              From AXUNIT
...
Subprog ACTIV_AINSERT()
  ...
  Call CHECK_EQUAL("Check the AINSERT method - should fail in this scenario=["+num$([L]ASTATUS)+"]","Check the AINSERT method - should fail in this scenario=["+num$([V]CST_AERROR)+"]") From AXUNIT
  Gosub WRITE_ERRORS
  ...
  [L]ASTATUS = fmet MYINSTANCE.AINSERT
  Call CHECK_EQUAL("Insert valid record=["+num$([L]ASTATUS)+"]","Insert valid record=["+num$([V]CST_AOK)+"]") From AXUNIT
  Gosub WRITE_ERRORS
  #
  FreeGroup MYINSTANCE
End
...
For I=1 To maxtab(MYINSTANCE.AERROR)
 If  MYINSTANCE.AERROR(I)<>null #& MYINSTANCE.AERROR(I).STA=[V]CST_AERROR & MYINSTANCE.AERROR(I).DEL<>2
   Call CHECK_NOTEQUAL(MYINSTANCE.AERROR(I).PRO-":"-MYINSTANCE.AERROR(I).MES-":"-num$(MYINSTANCE.AERROR(I).STA),"") From AXUNIT
 Endif
Next I
Return
