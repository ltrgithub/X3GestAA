...
asyncTest("ADOPAR", 4, (_) => {
	_checkSearch(_, "ADOPAR");
	start();
});

asyncTest("PARSTA1", 4, (_) => {
	_checkSearch(_, "PARSTA1");
	start();
});
asyncTest("PARSTA2", 4, (_) => {
	_checkSearch(_, "PARSTA2");
	start();
});
asyncTest("TABCOEFF", 4, (_) => {
	_checkSearch(_, "TABCOEFF");
	start();
});
asyncTest("TABCOUNTRY", 4, (_) => {
	_checkSearch(_, "TABCOUNTRY");
	start();
});
asyncTest("TABCUR", 4, (_) => {
	_checkSearch(_, "TABCUR");
	start();
});
asyncTest("TABFOR", 4, (_) => {
	_checkSearch(_, "TABFOR");
	start();
});
asyncTest("TABUNIT", 4, (_) => {
