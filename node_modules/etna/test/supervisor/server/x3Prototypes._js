...
var rewriteFixturesProto = true; // to rewrite fixtures json from SUPERV

module(config.unit_test.x3endpoint.folder + ".x3Prototypes");

if (!config.unit_test || !config.unit_test.x3endpoint) {
	test('X3 PROTOTYPES UNIT TESTS DISABLED FOR NOW', 1, function() {
				...
				for (var p in o1) {
					...
					switch (p) {
						...
						case "$url":
							...
							if (config.unit_test.x3endpoint.dataset !== "SUPERV") o1[p] = o1[p].replace("/" + config.unit_test.x3endpoint.dataset + "/", "/SUPERV/");
							break;
					}
	...
	*/
	...
	testDevice();
	testPrototypes();
	if (rewriteFixturesProto === true && config.unit_test.x3endpoint.dataset === "SUPERV") {
		testCompare(true);
	}
	testCompare(false);
	testLinks();
