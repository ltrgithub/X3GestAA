...
console.log("endpoint:",config.unit_test.etnaEndPoint);

const x3Helper = require("etna/test/supervisor/fixtures/x3Helper");
// const helper = require('etna/test/engine/fixtures/helper');
// const endpoint = helper.getEtnaConfigForUnitTest(_, config.unit_test.etnaEndPoint);

const tracer = console.error;
const rewriteFixturesProto = false; // to rewrite fixtures json from SUPERV
...
*/
	...
	if (_facet in {
		...
		if (prototype.$properties.$resources.$item) _checkLinks(prototype.$properties.$resources.$item);
	}
...
function _compareJSON(_, paramRewrite, _representation, _facet) {
	var name = _representation + ".$" + _facet;
...
*/
	/*
    "AQCLNK01/query/Type/$properties.$resources.$item.$properties/REP_CREUSR_REF", // Not a reference on query facet
    ...
    "AQCLNK01/bulk/Type/$properties.$resources.$item.$properties/REP_CREUSR_REF", // Not a reference on bulk facet
	"AQCLNK01/bulk/Type/$properties.$resources.$item.$properties.REP_CREUSR/$isReadOnly",
	"AQCLNK01/bulk/Type/$properties.$resources.$item.$properties.REP_CREUSR/$capabilities",
*/
	/*
...
*/
	...
	function _replaceObjects(o1) {
		function _replaceObj(o1, path) {
			tracer && tracer("_replaceObj o1:", toString.call(o1));
			for (var p in o1) {
				tracer && tracer("_replaceObjects:", p, toString.call(o1[p]), o1[p]);
				var exception = false;
				switch (p) {
					case "$actxUser":
						o1[p] = "AQC";
					...
					case "$actxLogin":
						...
						break;
					case "$baseUrl":
						wdebutURL = o1[p].substr(0, o1[p].indexOf("/sdata"));
						o1[p] = "/sdata/x3/erp/SUPERV";
						break;
					case "$url":
						o1[p] = o1[p].replace(wdebutURL, "");
						if (config.unit_test.etnaEndPoint !== "SUPERV") o1[p] = o1[p].replace("/" + config.unit_test.etnaEndPoint + "/", "/SUPERV/");
						break;
				}
				...
				if (["$baseUrl", "$collaborationUrl", "$localization", "$title", "$shortTitle", "$actxLan", "$actxLanDef", "$actxLanIso", "$actxUser", "$actxLogin", "$actxLegCur", "$actxNbLeg", "$actxFolder", "$actxSolution"].indexOf(p) < 0) {
					...
					if (exception === false) {
						...
						if ((toString.call(o1[p]).indexOf("Object") >= 0) || (toString.call(o1[p]).indexOf("Array") >= 0)) {
							o1[p] = _replaceObj(o1[p], path + (path ? "." : "") + p);
						}
					}
				}
			}
	...
	function trierBind(a, b) {
		var info1 = (a.$bind ? a.$bind : a.$CODMENU);
		var info2 = (b.$bind ? b.$bind : b.$CODMENU);
		if (info1 < info2) return -1;
		else if (info1 == info2) return 0;
		else return 1;
	}
	...
	function _compareObjects(o1, o2) {
		...
		function _compare(o1, o2, path, message) {
			...
			for (var p in o1) {
				...
				//if (["$baseUrl", "$localization", "$title", "$shortTitle", "capabilities"].indexOf(p) < 0) {
				//if (["$x3Format", "$baseUrl", "$collaborationUrl", "$localization", "$title", "$shortTitle", "$actxLan", "$actxLanDef", "$actxLanIso", "$actxUser", "$actxLogin", "$actxLegCur", "$actxNbLeg", "$actxFolder", "$actxSolution"].indexOf(p) < 0) {
				if (["$baseType","$baseUrl", "$collaborationUrl", "$localization", "$title", "$shortTitle", "$actxLan", "$actxLanDef", "$actxLanIso", "$actxUser", "$actxLogin", "$actxLegCur", "$actxNbLeg", "$actxFolder", "$actxSolution"].indexOf(p) < 0) {
					tracer && tracer("_compare:", p, toString.call(o1[p]), toString.call(o2[p]));
					if (toString.call(o2[p]).indexOf("Undefined") >= 0) {
						if (_facet === "bulk" && p === "$links" && (/^\$properties.*$/.test(path) || path === "")) {
							// in etna, no links on properties on bulk facet
							exception = true;
						} else if (p === "$capabilities" && /^\$properties.*$/.test(path) && ["details", "edit", "summary"].indexOf(_facet) >= 0) {
							exception = true;
						} else if (exceptions.indexOf(_representation + "/" + _facet + "/" + "Type/" + path + "/" + p) >= 0) {
							exception = true;
						} else {
					...
					if (exception === false) {
						...
						tracer && tracer("indexOf", toString.call(o1[p]).indexOf("Object"));
						if ((toString.call(o1[p]).indexOf("Object") >= 0) || (toString.call(o1[p]).indexOf("Array") >= 0)) {
							if (["$article.$menus.$layout.$items"].indexOf((path + (path ? "." : "") + p)) >= 0) {
								if (toString.call(o1[p]).indexOf("Array") >= 0 && o1[p].length > 1) {
									o1[p] = o1[p].sort(trierBind);
								}
								if (toString.call(o2[p]).indexOf("Array") >= 0 && o2[p].length > 1) {
									o2[p] = o2[p].sort(trierBind);
								}
							}
	...
	try {
		...
		p2 = _replaceObjects(p2);
	} catch (e) {
		ok(false, "prototypes failed with error " + e);
		return test;
	}

	if (paramRewrite === true) {
		console.log("rewriteFixturesProto " + __dirname + "/../fixtures/" + _representation.toLowerCase() + "_" + _facet + ".json");
		try {
			fs.writeFile(__dirname + "/../fixtures/" + _representation.toLowerCase() + "_" + _facet + ".json", JSON.stringify(p2, null, 2), "utf8");
		} catch (e) {
			ok(false, _representation.toLowerCase() + "_" + _facet + ".json writeFile failed with error " + e.message);
			return test;
		}
		var test = _compareObjects(p2, p2);
	} else {
		var p1 = {};
		try {
			p1 = require("etna/test/supervisor/fixtures/" + _representation.toLowerCase() + "_" + _facet);
		} catch (e) {
...
function _compareJSONs(_, paramRewrite, _representation, Detail, Edit, Query, Lookup, Summary, Bulk) {
	var facets = [];
	...
	if (Bulk === true) facets.push("bulk");

	for (var i in facets) {
		_compareJSON(_, paramRewrite, _representation, facets[i]);
	}
}
...
function testPrototypes() {
	...
	asyncTest("AQCPROC01   prototype", (_) => {
	...
	asyncTest("AQCPROC02   prototype", (_) => {
	...
	asyncTest("AQCPROC03   prototype", (_) => {
	...
	asyncTest("AQCPROC04   prototype", (_) => {
	...
	asyncTest("AQCPROC05   prototype", (_) => {
	...
	asyncTest("AQCPROC06   prototype", (_) => {
	...
	asyncTest("AQCPROC07   prototype", (_) => {
	...
	asyncTest("AQCPROC08   prototype", (_) => {
	...
	asyncTest("AQCPROC09   prototype", (_) => {
	...
	asyncTest("AQCPROC10   prototype", (_) => {
	...
	asyncTest("AQCPROC11   prototype", (_) => {
	...
	asyncTest("AQCPROC12   prototype", (_) => {
	...
	// asyncTest("AQCLNK01   prototype", (_) => {
	...
	// 	start();
	// });
	// asyncTest("AQCPROT1   prototype", (_) => {
...
function testCompare(paramRewrite) {
	...
	asyncTest("ACLAIDX    compare", 6, (_) => {
		_compareJSONs(_, paramRewrite, "ACLAIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("AFCTIDX    compare", 6, (_) => {
		_compareJSONs(_, paramRewrite, "AFCTIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});

	asyncTest("APSADX     compare", 6, (_) => {
		_compareJSONs(_, paramRewrite, "APSADX", Detail, false, Query, false, false, Bulk);
		start();
	});
	asyncTest("AREPIDX    compare", 6, (_) => {
		_compareJSONs(_, paramRewrite, "AREPIDX", false, false, Query, Lookup, false, Bulk);
		start();
	});
	// asyncTest("ATABLE     compare", 12, (_) => {
	// 	_compareJSONs(_, paramRewrite, "ATABLE", Detail, Edit, Query, Lookup, Summary, Bulk);
	// 	start();
	// });
	asyncTest("ATABDIV    compare", 8, (_) => {
		_compareJSONs(_, paramRewrite, "ATABDIV", Detail, false, Query, Lookup, false, Bulk);
		start();
	});
	asyncTest("ATRACE     compare", 6, (_) => {
		_compareJSONs(_, paramRewrite, "ATRACE", Detail, false, Query, false, false, Bulk);
		start();
	});
	asyncTest("AVOLUME    compare", 10, (_) => {
		_compareJSONs(_, paramRewrite, "AVOLUME", Detail, Edit, Query, Lookup, false, Bulk);
		start();
	});
	// asyncTest("COMPANY    compare", 12, (_) => {
	// 	_compareJSONs(_, paramRewrite, "COMPANY", Detail, Edit, Query, Lookup, Summary, Bulk);
	// 	start();
	// });
	asyncTest("POSCOD     compare", 12, (_) => {
		_compareJSONs(_, paramRewrite, "POSCOD", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("TABLAN     compare", 12, (_) => {
		_compareJSONs(_, paramRewrite, "TABLAN", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});
	asyncTest("TABUNIT    compare", 12, (_) => {
		_compareJSONs(_, paramRewrite, "TABUNIT", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});

	//	asyncTest("ZQREQH     compare", 8, (_) => {
	//		_compareJSONs(_, paramRewrite, "ZQREQH",Detail,false,Query,Lookup,false,Bulk);
	//		start();
	//	});
	//	asyncTest("ZZSERVICE  compare", 8, (_) => {
	//		_compareJSONs(_, paramRewrite, "ZZSERVICE",Detail,Edit,Query,false,false,Bulk);
	//		start();
	//	});

	//TODO check representation later
	/*
	asyncTest("AQCPROT1  compare", 12, (_) => {		
		_compareJSONs(_, paramRewrite, "AQCPROT1", Detail, Edit, Query, Lookup, Summary,Bulk);				  
		start();
	});  ////// Failing at 402 EDIT, so add all EDITS to Detail properties.
*/

	asyncTest("TABCOUNTRY compare", 12, (_) => {
		_compareJSONs(_, paramRewrite, "TABCOUNTRY", Detail, Edit, Query, Lookup, Summary, Bulk);
		start();
	});

	// asyncTest("AQCLNK01  compare", 12, (_) => {
	// 	_compareJSONs(_, paramRewrite, "AQCLNK01", Detail, Edit, Query, Lookup, Summary, Bulk);
	// 	start();
	// });
}

function testLinks() {
	asyncTest("AQCPROC06" + x3Helper.gitHubIssue(1309), (_) => {
	...
	asyncTest("AQCPROC09" + x3Helper.gitHubIssue(1309), (_) => {
...
function testUS89048() {
	asyncTest("User story 89048/1", (_) => {
		...
		// if (config.unit_test.etnaEndPoint !== 'etnaOracle')
...
function testFilter() {
	asyncTest("AQCPROT1 - default filter" + x3Helper.gitHubIssue(6044), (_) => {
...
function testDevice() {
	asyncTest("Device test on AQCPROC01", (_) => {
	...
	asyncTest("Device test on AQCPROC01M" + x3Helper.gitHubIssue(1621), (_) => {
	...
	asyncTest("Device test on AQCPROC01T", (_) => {
	...
	asyncTest("Device test on AQCPROC10", (_) => {
...
function testIssue5013() {
	asyncTest("AQCPROC01" + x3Helper.gitHubIssue(5013), (_) => {
		var prototype = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('AQCPROC01.$query')"), 200);
...
function testIssue4971() {
	asyncTest("AQCPROC01" + x3Helper.gitHubIssue(4971), (_) => {
		var prototype = x3Helper.get(_, _getCookie(_), x3Helper.x3Url("$prototypes('AQCPROC01.$query')"), 200);
...
function testIssue5554() {
	asyncTest("QUERIES" + x3Helper.gitHubIssue(5554), (_) => {
...
testDevice();
testPrototypes();
if (rewriteFixturesProto === true ) {//&& config.unit_test.etnaEndPoint === "SUPERV") {
	testCompare(true);
}
testCompare(false);
testLinks();
