...
QUnit.module(config.unit_test.x3endpoint.folder + ".x3Query");

if (!config.unit_test || !config.unit_test.x3endpoint) {
	test('X3 UNIT TESTS DISABLED FOR NOW', 1, function() {
	...
	*/
	...
	function testAQCLNK01(_) {
		...
		asyncTest("AQCLNK01 - OPERATIONS Test", function(_) {
			var representation = new Representation("AQCLNK01");
			var count = 100;
			...
			//ok(true, "DBG query:" + JSON.stringify(query));
			var wcount = 0;
			query.$resources.forEach_(_, function(_, resource) {
				wcount = wcount + 1;
			});
	...
	asyncTest("stop server", function(_) {
		ok(true, "start");
