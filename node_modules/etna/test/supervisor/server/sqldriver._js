...
function sqlInsert(_, driver, cnx, table, value) {
	...
	var columns = Object.keys(value).reduce((previousValue, currentValue, index, array) => {
	...
	var sql = "insert into " + driver.escape(table) + " (" + columns.join(',') + ") values (" + Object.keys(columns).map((column) => {
		return driver.param(parseInt(column));
	}).join(',') + ")";
	//DBG:ok(true,"insert:"+sql);

	var values = Object.keys(value).reduce((previousValue, currentValue, index, array) => {
...
function testConnect(_) {
	try {
		...
		sqlDriver.withConnection(_, (_, cnx) => {
			ok(true, "Connected");
...
asyncTest("scm ", (_) => {
	...
	for(var i=0;i<max;i++) {
		...
		try {
			superv.sqlDriver.withConnection(_, (_, cnx) => {
