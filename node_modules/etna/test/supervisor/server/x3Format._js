...
const dataModel = require("syracuse-orm/lib/dataModel");
...
function testAQCQRY02() {
	...
	asyncTest("AQCQRY02 - Create NOK (integer <> decimal)", _ => {
		var FIELD01 = 1001;
		var FIELD02 = 101.01;
		var representation = new Representation("AQCQRY02");
		representation.delete(_, FIELD01);
	...
	asyncTest("AQCQRY02 - Create NOK (integer <> alphanumeric)", _ => {
		var FIELD01 = 1002;
		var FIELD02 = "102";
		var representation = new Representation("AQCQRY02");
		representation.delete(_, FIELD01);
	...
	asyncTest("AQCQRY02 - Create NOK (integer <> alphanumeric)", _ => {
		...
		// type incompat generates a 500 - difficult to distinguish client side error (bad input) from 
		// server-side programming error in this case as X3 errn 10 may be raised by either.
		representation.setProperties(_, {
			"FIELD02": FIELD02
		}, null, 500);
		var response = representation.save(_);
