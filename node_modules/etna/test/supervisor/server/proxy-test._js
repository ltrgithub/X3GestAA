...
if (typeof Proxy !== "undefined" && config.unit_test && config.unit_test.etnaEndPoint) {
	...
	asyncTest("connect ", 1, (_) => {
		...
		dataset = require('etna/lib/supervisor/proxies/dataset').create(_, supervisor);
	...
	asyncTest("query", 1, (_) => {
		const ABANK = dataset.classes.ABANK(_);
		var count = 0;
		ABANK.query(_)
		.filter({ BAN: /^3/ })
		.forEach(_, (_, b) => {
			console.error(count, b.BAN(_));
			count++;
		});
		ok(count > 2 && count < 10, "query worked");
		start();
	});

	asyncTest("basic class operations", 2, (_) => {
		const ABANK = dataset.classes.ABANK(_);
