...
const Datatypes = x3js.class('system.Datatypes')
...
if (typeof Proxy !== "undefined" && config.unit_test && config.unit_test.etnaEndPoint) {
	...
	function checkUserParam(_, name, type) {
		const val = x3js.parameters.user(_, name);
		if (type !== 'date')
	...
	function checkFolderParam(_, name, type) {
		const val = x3js.parameters.folder(_, name);
		if (type !== 'date')
			strictEqual(typeof val, type, `user param ${name}: value=${val}`);
		else
			ok(x3js.date.isDate(val), `user param ${name}: value=${val}`);
	...
	function checkLegislationParam(_, leg, name, type) {
		const val = x3js.parameters.legislation(_, leg, name);
		if (type !== 'date')
			strictEqual(typeof val, type, `user param ${name}: value=${val}`);
		else
			ok(x3js.date.isDate(val), `user param ${name}: value=${val}`);
	...
	asyncTest("user parameters of various types", 5, _ => {
		checkUserParam(_, 'AQCUSRBOOL', 'boolean');
		checkUserParam(_, 'AQCUSRBOOL', 'boolean'); // twice, to test cache
		checkUserParam(_, 'AQCUSRNUM', 'number');
		checkUserParam(_, 'AQCUSRCHAR', 'string');
		checkUserParam(_, 'AQCUSRDATE', 'date');
		start();
	});
	asyncTest("folder parameters of various types", 6, _ => {
		checkFolderParam(_, 'AQC1', 'boolean');
		checkFolderParam(_, 'AQC1', 'boolean'); // twice, to test cache
		checkFolderParam(_, 'ADMUSR', 'string');
		checkFolderParam(_, 'CHGTYP', 'number');
		checkFolderParam(_, 'CPYMAXLEN', 'number');
		checkFolderParam(_, 'AQCFOLDATE', 'date');
		start();
	});
	...
	asyncTest("product parameter", 1, _ => {
		const typdos = x3js.parameters.folder(_, 'TYPDOS');
