...
export default {
  ...
  $relations: {
    rates: {
      $properties: {
        ...
        currency: {
          $propagate(_, val) {
            this.monetarySymbol$ = this.$classes.CurrencyTable$.cached(_, this.currency$).monetarySymbol$;
            this.getRate(_);
          },
        },
        diviso: {
  ...
  $events: {
    $read(_) {
      this.monetarySymbol$ = this.$classes.CurrencyTable$.cached(_, this.destinationCurrency$).monetarySymbol$;
      this.rateDate$ = x3js.date.today();
      this.loadRates(_);
    },
    ...
    loadRates(_) {
      ...
      .join('TABCUR', 'TCU', {
        ...
        const data = Object.assign(rec.TCH, rec.TCU, {
          ...
          EQUAL: '=',
          CURSYM: this.$classes.CurrencyTable$.cached(_, rec.TCH.CUR).monetarySymbol$,
        });
