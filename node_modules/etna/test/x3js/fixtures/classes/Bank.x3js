...
export default {
  ...
  $properties: {
    code: {
      $control(_, val) {
        const len = this.$cache(_, 'Country', this.countryCode$, 'lengthOfBank');
        if (val.length !== len) this.code.error(_, this.$message(_, 111, 123, 1) + ' ' + this.countryCode$ + ' (' + len + ')');
      },
    },
    countryCode: {
        $control(_, val) {
          if (this.$cache(_, 'Country', val, 'lengthOfBank') < 1 || 
            this.$cache(_, 'Country', val, 'bankControl') !== x3js.YES) {
  ...
  $events: {
    ...
    $deleteControlBefore(_) {
      var len = this.$cache(_, 'Country', this.countryCode$, 'lengthOfBank');
