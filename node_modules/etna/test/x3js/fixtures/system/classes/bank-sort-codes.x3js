...
const BankIdStatement = x3js.class('system.BankIdStatement')
...
export default x3js.defineClass({
  ...
  properties: {
    code: {
      dictCode: 'BAN',
      control(_, val) {
        const len = Country.cache(_, this.countryCode$).lengthOfBank$;
    ...
    countryCode: {
      dictCode: 'CRY',
      control(_, val) {
          if (Country.cache(_, val).lengthOfBank$ < 1 || 
            Country.cache(_, val, 'bankControl') !== x3js.YES) {
  ...
  events: {
    updateAfter(_) {
      if (this.$snapshot.validFrom$ != this.validFrom$ || 
        ...
        const len = Country.cache(_, this.countryCode$).lengthOfBank$;
        BankIdStatement.query(_).execute(_).filter({
    ...
    deleteControlBefore(_) {
      var len = Country.cache(_, this.countryCode$).lengthOfBank$;
      if (BankIdStatement.query(_).execute(_).some(_, {
        countryCode: this.countryCode$,
        bankIdNumber: x3js.predicates.startsWith(this.code$.substring(0, len)),
      })) {
