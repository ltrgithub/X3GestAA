...
export default x3js.class({
  ...
  relations: {
    rates: {
      ...
      properties: {
        ...
        currency: {
          propagate(_, val) {
            this.monetarySymbol$ = this.$classes.system$CurrencyTable.cache(_, this.currency$).monetarySymbol$;
            this.getRate(_);
          },
        },
        diviso: {
      ...
      methods: {
        getRate(_) {
          if (this.rateType$ != '' && 
            ...
            if (this.$classes.system$CurrencyRate.read(_, [this.rateType$, this.destinationCurrency$, this.currency$, this.rateDate$])) {
  ...
  events: {
    read(_) {
      this.monetarySymbol$ = this.$classes.system$CurrencyTable.cache(_, this.destinationCurrency$).monetarySymbol$;
      this.rateDate$ = x3js.date.today();
      this.loadRates(_);
    },
    ...
    loadRates(_) {
      ...
      .join('TABCUR', 'TCU', {
        ...
        const data = Object.assign(rec.TCH, rec.TCU, {
          ...
          EQUAL: '=',
          CURSYM: this.$classes.system$CurrencyTable.cache(_, rec.TCH.CUR).monetarySymbol$,
        });
      ...
      if (this.rates$.some(_, (_, rate) => {
        return this.rates$.some(_, (_, r) => {
          if (r.rateType$ == rate.rateType$ && 
            ...
            return true;
          else if (this.$classes.system$CurrencyRates.$read(_, [r.rateType$, r.destinationCurrency$, r.currency$, r.rateDate$])) 
            return true;
          else
            return false;
        });
