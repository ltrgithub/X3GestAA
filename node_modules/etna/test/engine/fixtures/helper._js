...
exports.getEtnaConfigForUnitTest = function(_, endpointName, locale) {
	...
	try {
		...
		if (!endPoint.x3solution) throw new Error("no x3solution in endpoint: " + endpointName);
		var solution = db.collection('X3solution', _).find({
			_id: endPoint.x3solution._uuid,
		}).toArray(_)[0];
		if (!solution) throw new Error("solution not found for endpoint: " + endpointName);
		...
		return {
			...
			mongo: {
				...
				database: solution.solutionName + '-' + endPoint.x3ServerFolder
			},
			sql: {
			...
			session: sessionsByLocale[locale || 'fr-FR'], // TODO: default should be 'en-US'
		};
	} finally {
		db.close();
	}
...
exports.getTestSupervisor = function(_) {
	const etnaConfig = exports.getEtnaConfigForUnitTest(_, config.unit_test.etnaEndPoint, 'en-US');
...
exports.getTestFolder = function(_) {
	return getTestSupervisor(_).api;
}

exports.x3jsTest = function(body) {
	if (typeof Proxy !== "undefined" && config.unit_test && config.unit_test.etnaEndPoint) {
		body();
	} else if (typeof Proxy === "undefined") {
		test("TEST SKIPPED: Proxies are not available. Start node.js with --harmony-proxies option", 0, () => {});
	} else {
		test("TEST SKIPPED: ETNA endpoint is not configured", 0, () => {});
	}
}