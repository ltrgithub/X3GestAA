...
exports.validate = function(value, constraints, errors){
    ...
    if (!(value instanceof Datetime)) {
        errors.push("value must be a datetime");
        return;
    }
...
function _utc(dt){
    ...
    if (!utc) {
        ...
        utc = d.getUTCFullYear() * 10000 * 100000 +
        (d.getUTCMonth() + 1) * 100 * 100000 +
        d.getUTCDate() * 100000 +
        d.getUTCHours() * 3600 +
        d.getUTCMinutes() * 60 +
        d.getUTCSeconds();
...
function _local(dt){
    ...
    if (!local) {
        var d = new Date(dt._value);
        local = d.getFullYear() * 10000 * 100000 +
        (d.getMonth() + 1) * 100 * 100000 +
        d.getDate() * 100000 +
        d.getHours() * 3600 +
        d.getMinutes() * 60 +
        d.getSeconds();
...
exports.Datetime = helpers.defineClass(Datetime, null, {
    date: {
        get: function(){
    ...
    timezoneOffset: {
        get: function(){
            if (this._tzOffset == null) 
                this._tzOffset = this.toJsDate().getTimezoneOffset();
                ...
                d: function(repeat){
                    switch (repeat) {
                        ...
                        case 4:
                            ...
                            break;
                        default:
                            throw new Error("bad format directive d: " + repeat);
                    }
                },
                M: function(repeat){
                    switch (repeat) {
                ...
                y: function(repeat){
                    switch (repeat) {
                        ...
                        case 4:
                            ...
                            break;
                        default:
                            throw new Error("bad format directive y: " + repeat);
                    }
                }
            });
            return result;
...
function _parseIso(str){
    var year = 0, month = 0, day = 0, hours = 0, minutes = 0, seconds = 0, ms = 0, offset = 0, convertError = true;
...
function _make(year, month, day, hour, minute, second, millis){
    ...
    jsDate.setSeconds(second || 0);
    jsDate.setMilliseconds(millis || 0);
    return new Datetime(jsDate.getTime(), jsDate.getTimezoneOffset());
}
...
function _makeUtc(year, month, day, hour, minute, second, millis){
    ...
    jsDate.setUTCHours(hour || 0);
    jsDate.setUTCMinutes(minute || 0);
    jsDate.setUTCSeconds(second || 0);
    jsDate.setUTCMilliseconds(millis || 0);
    return new Datetime(jsDate.getTime(), jsDate.getTimezoneOffset());
}
...
function _parse(str, format){
    ...
    function parseName(names){
        for (var k = 0; k < names.length; k++) {
            var name = names[k], len = name.length;
            if (str.substring(j, j + len).toUpperCase() == name.toUpperCase()) {
                j += len;
                return k;
            }
        }
        throw new Error("unknown date: " + str.substring(j));
    }
    
    _walkFormat(format, {
        literal: function(literal){
            if (str.substring(j, j + literal.length) !== literal) 
                throw new Error("format mismatch: expected " + literal + ", got " + str.substring(j));
            j += literal.length;
        },
        d: function(arg){
            switch (arg) {
                ...
                case 4:
                    ...
                    break;
                default:
                    throw new Error("bad day format: " + format.substring(i));
            }
        },
        M: function(arg){
            switch (arg) {
        ...
        y: function(arg){
            switch (arg) {
                ...
                case 4:
                    ...
                    break;
                default:
                    throw new Error("bad year format: " + format.substring(i));
            }
        },
        H: function(arg){
            switch (arg) {
    ...
    if (timeMode != null || abbrTimeMode != null) {
        if (hour > 12) 
            throw new Error("bad hour value: expected 1<hour<12, got :" + hour + " for mode :" + timeMode);
...
/*var utc = format.indexOf("Z") >= 0;

 var date = utc ? self.utcDate : self.date;

 var time = utc ? self.utcTime : self.time;
