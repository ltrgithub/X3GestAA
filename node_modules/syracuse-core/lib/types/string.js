...
exports.validate = function(value, constraints, errors, instance) {
	if (value == null) {
	...
	if (constraints.$pattern != null) {
		...
		if (typeof constraints.$pattern === "function") {
			pattern = constraints.$pattern(function(err, result) {
				if (err) throw err;
				return result;
			}, instance);
		}
		if (constraints.$patternModifiers) {
			pattern = new RegExp(pattern, constraints.$patternModifiers);
		} else {
			pattern = new RegExp(pattern);
		}
		if (!value.match(pattern)) errors.push(constraints.$patternMessage || locale.format(module, "regexp", value, pattern));
	}
