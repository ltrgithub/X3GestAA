...
helpers.defineClass(DateObj, null, {
    year: {
        get: function(){
    ...
    at: function(time, millisecond){
        throw new Error("method not installed. You must require the datetime module to get it")
    }
})
...
exports.monthFromName = function(name){
    var n = _resources().monthNames, m = _resources().abbreviatedMonthNames, s = name.toLowerCase();
...
exports.weekDayFromName = function(name){
    var n = _resources().dayNames, m = _resources().abbreviatedDayNames, o = _resources().shortestDayNames, s = name.toLowerCase();
    for (var i = 0; i < n.length; i++) {
        if (n[i].toLowerCase() == s || m[i].toLowerCase() == s) {
            return i;
        }
...
exports.parse = function(str, format){
    ...
    if (format == null) {
        // RFC 3339 by default -- very fast
        if (str.length != 10) 
            throw new Error("bad date format, expected 'yyyy-MM-dd', got '" + str + "'");
