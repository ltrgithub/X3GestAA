...
const _resources = locale.resources(module);
...
const _monthOffsets = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
...
class DateObj {
	...
	get yearDay() {
		...
		const leap = month > 2 && _isLeap(this.year) ? 1 : 0;
		return _monthOffsets[month - 1] + leap + this.day;
	}
	get week() {
		// ISO 8601 with week 1
	...
	weekNumber(firstDayOfWeek) {
		const begOfWeek1 = _make(this.year, 1, 4).begOfWeek(firstDayOfWeek);
	...
	addYears(years) {
		...
		const month = this.month;
		const day = Math.min(this.day, _daysInMonth(year, month));
	...
	addMonths(months: number) {
		...
		const monthLen = _daysInMonth(year, month);
