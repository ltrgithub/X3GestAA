...
exports.format = function(val, format) {
	...
	if (isNaN(val)) throw new Error(locale.format(module, "notNumber", val));
...
function _format(val, format) {
	...
	switch (flen) {
		...
		case 3:
			...
			break;
		default:
			throw new Error(flen === 0 ? locale.format(module, "noFormat") : locale.format(module, "manyFormat"));
	}
}
...
function _parseFormats(str, formatsList, negative) {
	...
	switch (flen) {
		case 1:
			...
			if (splitted.length > 2) throw new Error(locale.format(module, "oneSep", formatNumber));
		...
		case 3:
			...
			switch (i) {
				...
				case 2:
					...
					break;
				default:
					throw new Error(locale.format(module, "cannotParse"));
			}
			break;
		default:
			throw new Error(flen === 0 ? locale.format(module, "noFormat") : locale.format(module, "manyFormats"));
	}
	return num;
}