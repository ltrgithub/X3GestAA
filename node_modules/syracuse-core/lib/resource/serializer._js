...
function _serialize(_, dbHandle, meta, options, isRoot) {
	...
	if (options.isPrototype) {
		if (isRoot) {
			...
			result.$baseHelpUrl = options.baseUrl.replace("/sdata/", "/help/" + locale.current.replace('-', '_') + "/");
		}
	...
	var $ = result.$properties = helpers.object.clone(dbHandle.$properties) || {};
	...
	if (options.isPrototype && options.representation.type === "$query") {
		...
		result = {
			...
			$baseHelpUrl: result.$baseHelpUrl,
			$url: "{$baseUrl}/" + entity.plural,
			$type: result.$type,
			$title: entity.plural,
			$properties: {
