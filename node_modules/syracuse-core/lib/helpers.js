...
exports.defineClass = function(constructor, parent, members) {
	// Generators fast: constructor is a read-only function* so we have to wrap it 
...
exports.string = {
	pluralize: function(str /*:string*/ ) /*:string*/ {
	...
	makeLabel: function(str /*:string*/ ) /*:string*/ {
	...
	formatValue: function(item /*:any*/ , val /*:string*/ ) /*:string*/ {
	...
	parseValue: function(item /*:{ type: string; }*/ , str /*:any*/ ) /*:any*/ {
		if (str == null || str == "")
			return str;
		switch (item.type) {
			case "integer":
	...
	padLeft: function(str /*:?string*/ , len /*:number*/ , ch /*:string=" "*/ ) /*: string*/ {
	...
	/* flow found it buggy (match returns an array) - I'm disabling it because it is not used
	checkName: function(name) {
		...
		if (name.match(/[A-Z][A-Z0-9_\-]*\b/i) != name)
	...
	checkPath: function(path) {
		...
		if (path.match(/[A-Z][A-Z0-9_]*(?:[.][A-Z][A-Z0-9_]*)*\b/i) != path)
			throw new Error("path does not match required pattern: " + path);
	},*/
	sqlQuote: function(str /*:string*/ ) /*:string*/ {
		return str && ("'" + str.replace(/'/g, "''") + "'");
	},
	compare: function(s1 /*:string*/ , s2 /*:string*/ , ignoreCase /*:boolean=false*/ ) /*:number*/ {
		if (ignoreCase) {
			s1 = s1.toUpperCase();
			s2 = s2.toUpperCase();
		}
	...
	labelSeparator: ' :',
	htmlEscape: function(str /*:string*/ ) /*:string*/ {
		var htmlEscaper = /[&<>"'\/]/g,
			htmlEscapes = {
				'&': '&amp;',
				'<': '&lt;',
...
exports.resource = {
	...
	sdataDelta: function(oldResource, newResource) {
		return _sdataDiff(oldResource, newResource, true, true, false);
	},
	sdataMerge: function(oldResource, delta) {
...
exports.url = {
	...
	}*/
	{
		...
		var path = _split.splice(0, 1)[0];
		var rawQuery = _split.join('?');
		var _segments = path.split('/').map(function(seg) {
		...
		return {
			...
			path: path,
			rawQuery: rawQuery,
			query: exports.url.parseQueryString(rawQuery),
			walkUrl: function() {
	...
	decodeDetailSegment: function(segment /*:string*/ ) /*:?UrlSegment*/ {
...
exports.http = {
	...
	checkIfIE: function(userAgent) {
		if (!userAgent) return false;
		var reIEOld = /MSIE ([0-9]{1,}[\.0-9]{0,})/; // new RegExp("MSIE ([0-9]{1,}[\.0-9]{0,})");
		var reIENew = /Trident\/.*rv:([0-9]{1,}[\.0-9]{0,})/; // new RegExp("Trident/.*rv:([0-9]{1,}[\.0-9]{0,})");
