...
exports.fusionJsonPrototypes = true;
...
function _deltaDollar(oldResource, newResource, dollar, delta) {
	...
	var oldIsNull = (oldResource.$ == null);
	var newIsNull = (newResource.$ == null);
	// add undefined values && deleteMissing
	if (dollar.length) {
		delta.$ = delta.$ || {};
		dollar.forEach(function(patch) {
			delta.$[patch.name] = delta.$[patch.name] || {};
			delta.$[patch.name][patch.key] = patch.value;
		});
	}
	...
	if (oldIsNull) {
		...
		hasDiff = true;
		Object.keys(newResource.$).forEach(function(key) {
			if (delta.$[key]) helpers.object.extend(delta.$[key], newResource.$[key]);
			else delta.$[key] = newResource.$[key];
		});
	} else if (newIsNull) {
		if (!hasDiff) {
			delta.$ = null;
		...
		var cdelta = _sdataDiff(oldResource.$, newResource.$, false, false, true);
		if (cdelta) {
			if (hasDiff) {
				Object.keys(delta.$).forEach(function(key) {
					...
					if (typeof dvalue == "undefined") {
						...
						cdelta[key] = delta.$[key];
						var val = cdelta[key];
						Object.keys(oldResource.$[key]).forEach(function(key) {
							val[key] = null;
						});
						//
					} else {
						helpers.object.extend(dvalue, delta.$[key]);
					}
				});
				delta.$ = cdelta;
			} else {
...
function _sdataMerge(resource, delta, isResource) {
	...
	if (isResource) {
		_mergeDollar(resource.$, resDollar, newResource);
	}
	if (dollar) delta.$ = dollar;
	return newResource;
}
