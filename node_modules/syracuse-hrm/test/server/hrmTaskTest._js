...
asyncTest("initialize data for hrm task test ", function(_) {
	var diag = [];
	...
	//jsonImport.jsonImport(_, db, "syracuse-admin-license-hrm-init-unittest.json", {
	//    $diagnoses: diag
	//});
	ok(onlyInfo(diag), "hrm license import ok");
	start();
});


asyncTest("initialization for hrm task test ", function(_) {
	...
	var body = post(_, cookie, "x3solutions", {
		"code": "HRMT",
		"description": "MRMT",
		"solutionName": "HRMDVLP",
		"serverHost": "aws-x3-devhrm.sagefr.adinternal.com",
		"serverPort": 16100,
		"proxy": false,
		application: {
			$uuid: app.$uuid
		...
		runtimes: [{
			...
			$value: "aws-x3-devhrm.sagefr.adinternal.com:16100",
			autoConfig: true,
			banTimeout: 60,
			banned: false,
			description: "aws-x3-devhrm.sagefr.adinternal.com:16100",
			disabled: false,
			errorTry: 0,
			exclusive: false,
			serverHost: "aws-x3-devhrm.sagefr.adinternal.com",
	...
	var endpointUUID = body.$uuid;
	ok(!hasErrors(body), "End point created");
...
asyncTest("Test x3 task behavior ", function(_) {
	...
	ok(!onlyInfo(resp.body.$diagnoses), "Entity does not exist for endpoint");
...
asyncTest("Test hrm task call ", function(_) {
	...
	strictEqual(resp.body.$diagnoses[0].$message, "=>>My SYRACUSRMNG AddT DCB1+TDCB2=TDCB=0", "message  ok");
