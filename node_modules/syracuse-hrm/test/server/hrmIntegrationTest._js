...
asyncTest("round robin load balancer tests", 14, function(_) {
	...
	trace && trace("First attempt on site0 --> must be the first site's asociated server");
	...
	trace && trace("Second attempt on same site0 --> must be the second site's asociated server");
	...
	trace && trace("First attempt on site3 --> must not use the first site's asociated server because it was already used once");
	...
	trace && trace("First attempt on site1 --> must use the first site's asociated server because it's the only one");
	...
	trace && trace("First attempt on site2 --> must fail because of no server associated");
	...
	trace && trace("Second attempt on site3 --> must use the third site's asociated server");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	...
	trace && trace("Third attempt on site3 --> must use the first site's asociated server");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 1
	...
	trace && trace("Fourth attempt on site3 --> must use the same server because because forced");
	...
	try {
		trace && trace("Second attempt on site1 (without ignoreAvailability --> must fail because server will be banned");
	...
	trace && trace("Fifth attempt on site3 --> must use the third site's asociated server because the second was banned just before");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	...
	trace && trace("Third attempt on site0 --> must use the first site's asociated server");
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	strictEqual(server && server.toString(_), ws[0].host + ":" + ws[0].http, "Third selected server for site0 OK [" + (server && server.toString(_)) + "]");

	try {
		trace && trace("Sixth attempt on site3 (without ignoreAvailability --> must fail because all servers will be banned");
	...
	trace && trace("Fourth attempt on site0 --> must use the first site's asociated server because the other must be banned");
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	...
	try {
		trace && trace("Seventh attempt on site3 --> must fail because all servers have been banned");
		server = lb.selectServer(_, context, optSite3, opt);
	} catch (e) {
		// no server
		ok(true, "Server selection failed for site3 OK [" + e.message + "]");
