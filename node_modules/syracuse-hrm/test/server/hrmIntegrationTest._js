...
asyncTest("check installation data", function(_) {
	...
	strictEqual(servers[2] && servers[2].toString(_), ws[2].host + ":" + ws[2].http, "Server OK [" + (servers[2] && servers[2].toString(_)) + "]");
...
asyncTest("round robin load balancer tests", 14, function(_) {
	...
	// server 0
	ok(server && server.toString(_) === ws[0].host + ":" + ws[0].http, "First selected server for site0 OK [" + (server && server.toString(_)) + "]");

	// Second attempt on same site0 --> must be the second site's asociated server
	server = lb.selectServer(_, context, optSite0, opt);
	// server 1
	ok(server && server.toString(_) === ws[1].host + ":" + ws[1].http, "Second selected server for site0 OK [" + (server && server.toString(_)) + "]");
	...
	// server 2
	ok(server && server.toString(_) === ws[2].host + ":" + ws[2].http, "First selected server for site3 OK [" + (server && server.toString(_)) + "]");
	...
	// server 2
	ok(server && server.toString(_) === ws[2].host + ":" + ws[2].http, "First selected server for site1 OK [" + (server && server.toString(_)) + "]");
	...
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	ok(server && server.toString(_) === ws[3].host + ":" + ws[3].http, "Second selected server for site3 OK [" + (server && server.toString(_)) + "]");

	// Third attempt on site3 --> must use the first site's asociated server
	server = lb.selectServer(_, context, optSite3, opt);
	// server 1
	ok(server && server.toString(_) === ws[1].host + ":" + ws[1].http, "Third selected server for site3 OK [" + (server && server.toString(_)) + "]");
	...
	// server 1
	ok(server && server.toString(_) === ws[1].host + ":" + ws[1].http, "Fourth forced selected server for site3 OK [" + (server && server.toString(_)) + "]");
	...
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	ok(server && server.toString(_) === ws[3].host + ":" + ws[3].http, "Fifth selected server for site3 OK [" + (server && server.toString(_)) + "]");

	// Third attempt on site0 --> must use the first site's asociated server
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	ok(server && server.toString(_) === ws[0].host + ":" + ws[0].http, "Third selected server for site0 OK [" + (server && server.toString(_)) + "]");
	...
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	ok(server && server.toString(_) === ws[0].host + ":" + ws[0].http, "Fourth selected server for site0 OK [" + (server && server.toString(_)) + "]");
