...
// Unit test traces
var trace = require("syracuse-trace/lib/helper").getTracer("hrm.unittest");
...
var data_mongo = {
	...
	"$items": [{
		...
		"$type": "x3solution",
		"code": "SOLPAIEV6_1",
		"description": "SOLPAIEV6_1",
		"solutionName": "SOLPAIEV6_1",
		"serverHost": "dummyhost",
		"serverPort": 17000,
		"application": {
			"application": "x3",
			"contract": "hrm"
		...
		"$type": "x3solution",
		"code": "SOLPAIEV6_2",
		"description": "SOLPAIEV6_2",
		"solutionName": "SOLPAIEV6_2",
		"serverHost": "dummyhost",
		"serverPort": 17000,
		"application": {
			"application": "x3",
...
asyncTest("data setup", function(_) {
	...
	console.log("import demo db diags (134): " + require('util').inspect(diag));
...
function getSiteOptions(_, siteName, _ep) {
	return {
		site: siteName,
		solution: _ep.getSolutionName(_),
...
asyncTest("round robin load balancer tests", 14, function(_) {
	...
	trace.info && trace.info("First attempt on site0 --> must be the first site's asociated server");
	...
	trace.info && trace.info("Second attempt on same site0 --> must be the second site's asociated server");
	...
	trace.info && trace.info("First attempt on site3 --> must not use the first site's asociated server because it was already used once");
	...
	trace.info && trace.info("First attempt on site1 --> must use the first site's asociated server because it's the only one");
	...
	trace.info && trace.info("First attempt on site2 --> must fail because of no server associated");
	...
	trace.info && trace.info("Second attempt on site3 --> must use the third site's asociated server");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	...
	trace.info && trace.info("Third attempt on site3 --> must use the first site's asociated server");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 1
	...
	trace.info && trace.info("Fourth attempt on site3 --> must use the same server because because forced");
	...
	try {
		trace.info && trace.info("Second attempt on site1 (without ignoreAvailability --> must fail because server will be banned");
	...
	trace.info && trace.info("Fifth attempt on site3 --> must use the third site's asociated server because the second was banned just before");
	server = lb.selectServer(_, context, optSite3, opt);
	// server 3
	...
	trace.info && trace.info("Third attempt on site0 --> must use the first site's asociated server");
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	strictEqual(server && server.toString(_), ws[0].host + ":" + ws[0].http, "Third selected server for site0 OK [" + (server && server.toString(_)) + "]");

	try {
		trace.info && trace.info("Sixth attempt on site3 (without ignoreAvailability --> must fail because all servers will be banned");
	...
	trace.info && trace.info("Fourth attempt on site0 --> must use the first site's asociated server because the other must be banned");
	server = lb.selectServer(_, context, optSite0, opt);
	// server 0
	...
	try {
		trace.info && trace.info("Seventh attempt on site3 --> must fail because all servers have been banned");
		server = lb.selectServer(_, context, optSite3, opt);
	} catch (e) {
		// no server
		ok(true, "Server selection failed for site3 OK [" + e.message + "]");
...
asyncTest("maxTry test ", , function(_) {

	var serverinst = hrmHelper.getHrmServer(_, db, ws[0]);
	serverinst.banned(_, false);
	// check if the errorTry is set to 0
	strictEqual(serverinst.errorTry(_), 0, "error try reset ");
	// test ping 5 time and check if after 5 try the server is correctly banned
	for ()
		serverinst.ping(_);
	strictEqual(serverinst.errorTry(_), 1, "error try first attempt ");
