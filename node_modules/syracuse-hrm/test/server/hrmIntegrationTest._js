...
asyncTest("installation with wizard helper API", 5, function(_) {
	...
	var siteName = "SITE_0";
	// servers 0 + 1
	var diags = wizardHelper.install(_, siteName, ws.slice(0, 2), true).$diagnoses;
	ok(onlyInfo(diags), "HRM Site " + siteName + " installation ok");

	siteName = "SITE_0_BIS";
	// no servers --> This Site will be used only for proxy selection tests
	diags = wizardHelper.install(_, siteName, [], false).$diagnoses;
	ok(onlyInfo(diags), "HRM Site " + siteName + " installation ok");

	setSessionContext(_, ep[1]);
	siteName = "SITE_1";
	// server 2
	diags = wizardHelper.install(_, siteName, ws.slice(2, 3), true).$diagnoses;
	ok(onlyInfo(diags), "HRM Site " + siteName + " installation ok");

	setSessionContext(_, ep[2]);
	siteName = "SITE_2";
	// no servers
	diags = wizardHelper.install(_, siteName, [], true).$diagnoses;
	ok(onlyInfo(diags), "HRM Site " + siteName + " installation ok");

	setSessionContext(_, ep[3]);
	siteName = "SITE_3";
	// servers 1 + 2 + 3
	diags = wizardHelper.install(_, siteName, ws.slice(1), true).$diagnoses;
	ok(onlyInfo(diags), "HRM Site " + siteName + " installation ok");
