...
var tracer = require("syracuse-trace/lib/helper").getTracer("hrmProxy");
...
function initHrm(_, context, suburl) {
	...
	tracer.trace.debug() && tracer.trace.debug("Init context: " + JSON.stringify(params, null, 2));
	...
	// Instanciate session
	var s = new Session(_, sid, _site, server, tracer.trace);
...
function forward(_, context, path, solution, folder, site) {
	...
	var s = hrmSession.getSession(_, context, tracer.trace, solution, folder, site, host, port);
	if (s) {
		...
		if (!s) tracer.trace.info() && tracer.trace.info("No session proxy found !!! New Tomcat session will be created.");
		var query = {};
		...
		switch (name) {
			case "page":
				...
				if (!lang) {
					...
					tracer.trace.info() && tracer.trace.info("Propagate Syracuse language: " + params.LANG);
				...
				// else -> propagate portal language to Syracuse user profile
					...
					tracer.trace.info() && tracer.trace.info("Change Syracuse language: " + lang);
