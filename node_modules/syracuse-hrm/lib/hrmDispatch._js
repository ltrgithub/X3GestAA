...
var lb = require('./hrmLoadBalancer');
var hrmSession = require('./hrmSession');
var hrmHelper = require('./hrmHelper');
var Session = hrmSession.Session;

var tracer = require('@sage/syracuse-core').getTracer("hrm.proxy");
...
function initHrm(_, context, suburl) {
	...
	tracer.debug && tracer.debug("Init context: " + JSON.stringify(params, null, 2));
	...
	// Instanciate session
	var s = new Session(_, sid, _site, server, tracer);
...
function forward(_, context, path, solution, folder, site) {
	...
	var s = hrmSession.getSession(_, context, tracer, solution, folder, site, host, port);
	if (s) {
		...
		if (!s) tracer.info && tracer.info("No session proxy found !!! New Tomcat session will be created.");
		var query = {};
		...
		switch (name) {
			case "page":
				...
				if (!lang) {
					...
					tracer.info && tracer.info("Propagate Syracuse language: " + params.LANG);
				...
				// else -> propagate portal language to Syracuse user profile
					...
					tracer.info && tracer.info("Change Syracuse language: " + lang);
