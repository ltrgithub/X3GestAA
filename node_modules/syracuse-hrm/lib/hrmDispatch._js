...
var trace = require("syracuse-trace/lib/helper").getTracer("hrmProxy");
...
function initHrm(_, context, suburl) {
	...
	trace.debug() && trace.debug("Init context: " + JSON.stringify(params, null, 2));
...
function forward(_, context, path, solution, folder, site) {
	...
	if (s) {
		...
		if (!s) trace.info() && trace.info("No session proxy found !!! New Tomcat session will be created.");
		var query = {};
		...
		switch (name) {
			case "page":
				...
				if (!lang) {
					...
					trace.info() && trace.info("Propagate Syracuse language: " + params.LANG);
				...
				// else -> propagate portal language to Syracuse user profile
					...
					trace.info() && trace.info("Change Syracuse language: " + lang);
