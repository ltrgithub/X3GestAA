...
function initHrm(_, context, suburl) {
	function authenticate(_, webBaseUrl, solution, folder, siteName, certificate, suburl) {
	...
	try {
		...
		var server = lb.selectServer(_, siteOptions, {
			host: params.SRVHOST,
			port: params.SRVPORT
		});
		...
		var result = authenticate(_, webServerBaseUrl, solution, folder, site, null, suburl);
		var resp = result.resp;
		var JSESSIONID = result.id;

		// Store proxy in httpSession	
		p = proxyHelper.storeInHttpSession(context, {
			solution: solution,
			folder: folder,
			site: site,
			jsid: JSESSIONID,
...
function forward(_, context, path, solution, folder, site, JSESSIONID) {
	try {
		...
		var p = proxyHelper.getProxyInstance(context, solution, folder, site, host, port, JSESSIONID);
		if (p) {
		...
		switch (name) {
			case "page":
				...
				if (!lang) {
					...
					trace && trace("Propagate Syracuse language: " + lang);
					context.rawQuery = querystring.stringify(params);
				}
				// else -> propagate portal language to Syracuse user profile
				else if (lang && lang !== locale.x3Language(up.selectedLocale(_).code(_))) {
			...
			default:
				return _dispatch(_, context, _hrmMap);
		}
	},
