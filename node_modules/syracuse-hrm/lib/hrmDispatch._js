...
function initHrm(_, context, suburl) {
	...
	trace.debug(_) && trace.debug(_, "Init context: " + JSON.stringify(params, null, 2));
...
function forward(_, context, path, solution, folder, site) {
	...
	if (s) {
		...
		if (!s) trace.info(_) && trace.info(_, "No session proxy found !!! New Tomcat session will be created.");
		var query = {};
		...
		switch (name) {
			case "page":
				...
				if (!lang) {
					...
					trace.info(_) && trace.info(_, "Propagate Syracuse language: " + params.LANG);
				...
				// else -> propagate portal language to Syracuse user profile
					...
					trace.info(_) && trace.info(_, "Change Syracuse language: " + lang);
