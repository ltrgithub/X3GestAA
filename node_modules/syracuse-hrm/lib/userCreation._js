...
function title(item) {
	...
	if (!(currentLocale in item)) {
		currentLocale = currentLocale.replace("-", "_");
	} else if (!(currentLocale in item)) {
		currentLocale = "en-US";
	} else if (!(currentLocale in item)) {
		currentLocale = currentLocale.replace("-", "_");
	} else if (!(currentLocale in item)) {
 ...
 *    index 1 and 2 will be filled with endpoint login name and description in case of error
 * return values: 0 OK, 1: Syracuse reference user does not exist
 *    2: no selected endpoint available
 *    3:
...
function user(_, login, reflogin, title, firstname, lastname, email, usr, tel, fax, ret_val) {
	...
	var i = items.length;
	// direct loop to make exit of function more easy
	while (--i >= 0) {
		var item = items[i];
		// get endpoint
		...
		if (!x3RefUser) {
			// error: finish, put more information into return values 
			ret_val[1] = item.login(_);
			ret_val[2] = title(endpoint.description(_));
