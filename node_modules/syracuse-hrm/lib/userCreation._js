...
function user(_, login, reflogin, usr_code, usr, title, firstname, lastname, email, tel, fax, range) {
	...
	items.forEach_(_, function(_, item) {
		...
		if (!x3RefUser) {
			...
			return _addError(locale.format(module, "noX3Ref", item.login(_), description), description);
		}
...
function reactivate(_, user) {
	...
	if (diagnoses.length === 0) {
		_addMessage(diagnoses, "success", locale.format(module, "userReactivated", user));
	}
	return makeResult(diagnoses);
