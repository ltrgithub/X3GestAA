 ...
 * - title, firstname, lastname, email: title, first name, last name, email of the newly created Syracuse user (and of the newly created X3 user too)
...
function user(_, login, reflogin, usr_code, usr, title, firstname, lastname, email, tel, fax, range) {
	...
	var ref_user = db.fetchInstance(_, users, {
		jsonWhere: {
			'userEndpoint.login': reflogin,
			'userEndpoint.endpoint': up.$uuid
		}
	});
	if (!ref_user) {
	...
	if (newSyraUser) {
		...
		var authent = authHelper.getStandardSetting(_).authentication(_);
