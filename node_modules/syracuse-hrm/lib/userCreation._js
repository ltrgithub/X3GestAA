...
function user(_, login, reflogin, usr_code, usr, title, firstname, lastname, email, tel, fax, range) {
	...
	function _addMessage(severity, message) {
		diagnoses.push({
			$severity: severity,
			$message: message
		});
	...
	var up = globals && globals.context && globals.context.sdataContext && globals.context.sdataContext.endpoint;

	if (!up) {
		up = globals && globals.context && globals.context.session && globals.context.session.getUserProfile(_);
		if (up) up = up.selectedEndpoint(_);
		if (!up) {
	...
	console.log("HRM user creation " + up.description(_));
	var x3RefUser2;
	// Syracuse user
	...
	var ref_users = db.fetchInstances(_, users, {
		jsonWhere: {
			...
			'endpoints.endpoint._uuid': up.$uuid
		}
	});
	// the query above will also find users where 'reflogin' is the login of one user endpoint and 'up.$uuid' is the UUID of another user endpoint.
	...
	ref_users.some_(_, function(_, ref_user0) {
		...
		if (endps.some_(_, function(_, endp) {
			if (endp.login(_) === reflogin && endp.endpoint(_).$uuid === up.$uuid) return true;
			return false;
		})) {
			ref_user = ref_user0;
			return true;
	...
	items.forEach_(_, function(_, item) {
		...
		if (endpoint.$uuid === up.$uuid) {
			...
			if (range < 3 && up.applicationRef(_).$uuid === endpoint.applicationRef(_).$uuid)
				return;
		}
		var orm = endpoint.getOrm(_);
		var ent = orm.getEntity(_, "ASYRAUS");
		if (!ent) {
			_addError(locale.format(module, "noEntity", endpoint.description(_)));
			return;
		}
