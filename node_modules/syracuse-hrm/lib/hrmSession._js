...
var trace = require("syracuse-trace/lib/helper").getTracer(_, "hrmProxy");
...
exports.getSession = function(_, context, solution, folder, site, host, port) {
	...
	trace.debug(_) && trace.debug(_, "Searching context: " + JSON.stringify({
		...
		if (s) {
			if (host && port) {
				...
				if (s && s.baseUrl.indexOf(baseUrl) === -1) {
					...
					trace.info(_) && trace.info(_, "Keep current session configuration");
				}
			}
			if (s.jsid !== JSESSIONID) {
				trace.info(_) && trace.info(_, "JSESSIONID is not corresponding !");
				s.resetCookie();
			}
		}
	} else {
		s = context.httpSession && context.httpSession.hrm && context.httpSession.hrm.lastSessionUsed;
		if (s) trace.info(_) && trace.info(_, "Select latest session configuration used because of special resource: " + context.request.url);
	}
	if (s) trace.info(_) && trace.info(_, "***\nSelected session configuration base url: " + JSON.stringify(s, null, 2) + "\n***");
	return s;
};
...
exports.Session = helpers.defineClass(function(_, sid, site, server, trace) {
	...
	connect: function(_, context, certificate, suburl) {
		...
		function getPortal(_) {
			...
			self.trace.info(_) && self.trace.info(_, "get portal request: " + JSON.stringify(options, null, 2));
			...
			self.trace.debug(_) && self.trace.debug(_, "get portal response:" + JSON.stringify({
		...
		function connection(_) {
			...
			self.trace.debug(_) && self.trace.debug(_, "auth request ($connection): " + JSON.stringify(options, null, 2));
			var request = httpClient.httpRequest(_, options);
		...
		function session(_, body, headers) {
			...
			self.trace.debug(_) && self.trace.debug(_, "auth request ($session): " + JSON.stringify(opt, null, 2));
		...
		try {
			...
			this.trace.debug(_) && this.trace.debug(_, "auth response ($connection):" + JSON.stringify({
			...
			this.trace.debug(_) && this.trace.debug(_, "auth response ($session):" + JSON.stringify({
				status: resp.statusCode,
				headers: resp.headers
			}, null, 2));

			if (resp.statusCode !== 200) {
				this.trace.error(_) && this.trace.error(_, "$session response: " + resp.readAll(_));
			...
			this.trace.error(_) && this.trace.error(_, "An error occured while authentication :\n" + JSON.stringify(connectBody, null, 2) + "\n" + e.stack);
			resp = getPortal(_);
	...
	disconnect: function(_, context) {
		try {
			...
			this.trace.debug(_) && this.trace.debug(_, "disconnect ($session): " + JSON.stringify(opt, null, 2));
			var request = httpClient.httpRequest(_, opt);
			...
			this.trace.debug(_) && this.trace.debug(_, "disconnect ($connections): " + JSON.stringify(opt, null, 2));
			request = httpClient.httpRequest(_, opt);
			resp = request.end().response(_);
			this.trace.debug(_) && this.trace.debug(_, "auth response ($connections):" + JSON.stringify({
				status: resp.statusCode,
				headers: resp.headers
			}, null, 2));
