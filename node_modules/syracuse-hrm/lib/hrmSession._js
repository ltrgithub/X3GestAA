...
exports.getSession = function(_, context, solution, folder, site, host, port) {
	trace && trace("Searching context: " + JSON.stringify({
	...
	if (solution && folder && site) {
		...
		if (s) {
			if (host && port) {
				...
				if (s && s.baseUrl !== baseUrl) {
					throw new Error(locale.format(module, "sessionAlreadyExist", s.baseUrl));
				} else {
					trace && trace("Keep current session configuration");
				}
			}
			if (s.jsid !== JSESSIONID) {
				trace && trace("JSESSIONID is not corresponding !");
				s.resetCookie(context);
			}
		}
	} else {
		s = context.httpSession && context.httpSession.hrm && context.httpSession.hrm.lastSessionUsed;
		if (s) trace && trace("Select latest session configuration used because of special resource: " + context.request.url);
	}
	if (s) trace && trace("***\nSelected session configuration base url: " + JSON.stringify(s, null, 2) + "\n***");
	return s;
};

exports.Session = helpers.defineClass(function(_, sid, solution, folder, site, server, trace) {
	this.sid = sid;

	this.mustResetCookies = false;
	this.solution = solution;
	this.folder = folder;
	this.site = site;
	this.server = server;
	this.host = server.host(_);
	this.port = server.port(_);
	var baseUrl = "http://" + this.host + ":" + this.port;
	proxy.Proxy.call(this, _, baseUrl, trace);
	...
	connect: function(_, context, certificate, suburl) {
		...
		function getPortal(_) {
			...
			self.trace && self.trace("get portal request: " + JSON.stringify(options, null, 2));
			...
			self.trace && self.trace("get portal response:" + JSON.stringify({
		...
		function connection(_) {
			...
			self.trace && self.trace("auth request ($connection): " + JSON.stringify(options, null, 2));
			var request = httpClient.httpRequest(_, options);
		...
		function session(_, body, headers) {
			...
			self.trace && self.trace("auth request ($session): " + JSON.stringify(opt, null, 2));
		...
		try {
			...
			this.trace && this.trace("auth response ($connection):" + JSON.stringify({
			...
			this.trace && this.trace("auth response ($session):" + JSON.stringify({
				status: resp.statusCode,
				headers: resp.headers
			}, null, 2));
			...
			this.trace && this.trace("An error occured while authentication :\n" + JSON.stringify(connectBody, null, 2) + "\n" + e.stack);
			resp = getPortal(_);
	...
	disconnect: function(_) {
		try {
			...
			this.trace && this.trace("disconnect ($session): " + JSON.stringify(opt, null, 2));
			console.log("disconnect ($sessions): " + JSON.stringify(opt, null, 2));
			var request = httpClient.httpRequest(_, opt);
			...
			this.trace && this.trace("disconnect ($connections): " + JSON.stringify(opt, null, 2));
			console.log("disconnect ($connections): " + JSON.stringify(opt, null, 2));
			request = httpClient.httpRequest(_, opt);
			resp = request.end().response(_);
			this.trace && this.trace("auth response ($connections):" + JSON.stringify({
				status: resp.statusCode,
				headers: resp.headers
			}, null, 2));
	...
	toJSON: function() {
		return {
			...
			xadxid: this.xadxid,
			originalSetCookie: this["original-set-cookie"],
