...
var trace = require("syracuse-trace/lib/helper").getTracer("hrmLoadBalancer");
var lbList = [];
...
exports.selectServer = function(_, context, site, options) {
	...
	try {
		lbFunnel(_, function(_) {
			...
			trace.info() && trace.info("Begin server selection for site [" + site.name(_) + "]");
			trace.debug() && trace.debug("Selection options: " + JSON.stringify(options, null, 2));
			...
			trace.info() && trace.info("Servers linked to site :" + JSON.stringify(realList, null, 2));
			trace.info() && trace.info("Banned servers :" + JSON.stringify(bannedList, null, 2));
			// ***
			// Synchronize list and real list
			// ***
			// Remove servers from list if banned
			trace.debug() && trace.debug("*** LB List before remove banned :" + JSON.stringify(lbList, null, 2));
			lbList = lbList.filter(function(item) {
				return bannedList.indexOf(item) === -1;
			});
			// Add never balanced servers
			trace.debug() && trace.debug("*** LB List before add new servers :" + JSON.stringify(lbList, null, 2));
			...
			trace.debug() && trace.debug("*** LB List before apply round robin :" + JSON.stringify(lbList, null, 2));
			...
			trace.debug() && trace.debug("*** LB List after apply round robin :" + JSON.stringify(lbList, null, 2));
			trace.info() && trace.info("Selected server : " + server.$uuid);
			if (server.$uuid !== selectedUuid) server = getServer(selectedUuid);
		});
	} catch (e) {
		//helpers.log.error(module, e);
		trace.error() && trace.error("HRM load balancer error: " + e.stack);
		throw e;
	}
	var endTime = new Date().getTime();
	trace.info() && trace.info("Selection time: " + (endTime - startTime) + "ms");
	return server;
};