...
exports.selectServer = function(_, context, site, options) {
	...
	try {
		lbFunnel(_, function(_) {
			...
			trace && trace("#######################################");
			trace && trace("Begin server selection for site [" + site.name(_) + "]");
			...
			var realList = servers.filter_(_, function(_, s) {
				...
				trace && trace("Le site est securisé");
				trace && trace("Le serveur est securisé : " + s.sslEnabled(_));
				if (site.secure(_)) {
			...
			if (options.host && options.port) {
				...
				for (var j in servers) {
					var port = site.secure(_) ? servers[j].portssl(_) : servers[j].port(_);
			...
			if (!options.ignoreAvailability) {
				...
				if (!server.isServerRunning(_) && !options.host && !options.port) {
