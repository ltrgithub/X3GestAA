...
var tracer = require("syracuse-trace/lib/helper").getTracer("hrmLoadBalancer");
...
exports.selectServer = function(_, context, site, options) {
	...
	try {
		lbFunnel(_, function(_) {
			...
			tracer.trace.info() && tracer.trace.info("#######################################");
			tracer.trace.info() && tracer.trace.info("Begin server selection for site [" + site.name(_) + "]");
			tracer.trace.debug() && tracer.trace.debug("Selection options: " + JSON.stringify(options, null, 2));
			...
			tracer.trace.info() && tracer.trace.info("Servers linked to site :" + JSON.stringify(realList, null, 2));
			tracer.trace.info() && tracer.trace.info("Banned servers :" + JSON.stringify(bannedList, null, 2));
			// ***
			// Synchronize list and real list
			// ***
			// Remove servers from list if banned
			tracer.trace.debug() && tracer.trace.debug("*** LB List before remove banned :" + JSON.stringify(lbList, null, 2));
			lbList = lbList.filter(function(item) {
				return bannedList.indexOf(item) === -1;
			});
			// Add never balanced servers
			tracer.trace.debug() && tracer.trace.debug("*** LB List before add new servers :" + JSON.stringify(lbList, null, 2));
			...
			tracer.trace.debug() && tracer.trace.debug("*** LB List before apply round robin :" + JSON.stringify(lbList, null, 2));
			...
			tracer.trace.debug() && tracer.trace.debug("*** LB List after apply round robin :" + JSON.stringify(lbList, null, 2));
			tracer.trace.info() && tracer.trace.info("Selected server : " + server.$uuid);
			if (server.$uuid !== selectedUuid) server = getServer(selectedUuid);
		});
	} catch (e) {
		//helpers.log.error(module, e);
		tracer.trace.error() && tracer.trace.error("HRM load balancer error: " + e.stack);
		throw e;
	}
	var endTime = new Date().getTime();
	tracer.trace.info() && tracer.trace.info("Selection time: " + (endTime - startTime) + "ms");
	return server;
};