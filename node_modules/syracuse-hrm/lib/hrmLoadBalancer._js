...
var trace = require("syracuse-trace/lib/helper").getTracer(_, "hrmLoadBalancer");
var lbList = [];
...
exports.selectServer = function(_, context, site, options) {
	...
	try {
		lbFunnel(_, function(_) {
			...
			trace.info(_) && trace.info(_, "#######################################");
			trace.info(_) && trace.info(_, "Begin server selection for site [" + site.name(_) + "]");
			trace.debug(_) && trace.debug(_, "Selection options: " + JSON.stringify(options, null, 2));
			...
			trace.info(_) && trace.info(_, "Servers linked to site :" + JSON.stringify(realList, null, 2));
			trace.info(_) && trace.info(_, "Banned servers :" + JSON.stringify(bannedList, null, 2));
			// ***
			// Synchronize list and real list
			// ***
			// Remove servers from list if banned
			trace.debug(_) && trace.debug(_, "*** LB List before remove banned :" + JSON.stringify(lbList, null, 2));
			lbList = lbList.filter(function(item) {
				return bannedList.indexOf(item) === -1;
			});
			// Add never balanced servers
			trace.debug(_) && trace.debug(_, "*** LB List before add new servers :" + JSON.stringify(lbList, null, 2));
			...
			trace.debug(_) && trace.debug(_, "*** LB List before apply round robin :" + JSON.stringify(lbList, null, 2));
			...
			trace.debug(_) && trace.debug(_, "*** LB List after apply round robin :" + JSON.stringify(lbList, null, 2));
			trace.info(_) && trace.info(_, "Selected server : " + server.$uuid);
			if (server.$uuid !== selectedUuid) server = getServer(selectedUuid);
		});
	} catch (e) {
		//helpers.log.error(module, e);
		trace.error(_) && trace.error(_, "HRM load balancer error: " + e.stack);
		throw e;
	}
	var endTime = new Date().getTime();
	trace.info(_) && trace.debug(_, "Selection time: " + (endTime - startTime) + "ms");
	return server;
};