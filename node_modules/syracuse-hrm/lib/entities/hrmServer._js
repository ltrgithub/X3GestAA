...
var maxTryBeforeBanish = (config && config.hrm && config.hrm.maxTry || 5) - 1;
var trace = config && config.hrm && config.hrm.proxy && config.hrm.proxy.trace;

exports.entity = {
	$titleTemplate: "HRM Server",
	$descriptionTemplate: "Configure HRM server",
	$valueTemplate: "{host}:{http}",
	$helpPage: "Administration-reference_HRM-Web-Servers",
	$properties: {
		host: {
	...
	$functions: {
		...
		bannish: function(_) {
			...
			if (errTry >= maxTryBeforeBanish) {
				self.banned(_, true);
			} else {
				self.errorTry(_, errTry + 1);
			}
			var s = self.save(_);
			var err = hrmHelper.hasErrors(s);
			if (err) {
