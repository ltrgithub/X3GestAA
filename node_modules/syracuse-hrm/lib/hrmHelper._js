...
exports.formatDiagnosis = function(sev, msg, stack) {
	var diag = {
		$severity: sev,
		$message: msg
	};
...
var updateHrmSite = exports.updateHrmSite = function(_, db, inst, options) {
	...
	inst.devmode(_, options.devmode || false);
	inst.secure(_, options.secure || false);
...
exports.getHrmSite = function(_, db, options) {
	...
	var sites = db.fetchInstances(_, db.model.getEntity(_, "hrmSite")).filter_(_, function(_, s) {
		return s.name(_) === options.site && s.endpoint(_).x3ServerFolder(_) === options.folder && s.endpoint(_).x3solution(_).solutionName(_) === options.solution;
	});
	return sites && sites.length > 0 && sites[0];
};
...
var updateHrmServer = exports.updateHrmServer = function(_, inst, options) {
	...
	inst.http(_, options.http);
	inst.sslEnabled(_, options.sslEnabled || false);
	inst.https(_, options.https);
	var err = hasErrors(inst.save(_));
	if (err) throw new Error(err);
	return inst;
};
...
var getHrmServer = exports.getHrmServer = function(_, db, options) {
	var filter = {
		sdataWhere: "host eq '" + options.host + "' and http eq '" + options.http + "'"
	};
