	...
	if (!encryptFunctionDefined)
	{
		...
		if (!script_exec(script, &result0, req))
  		...
  		if (result0->IsNull())
			return false;
#ifdef NEWAPI
		encryptFunction.Reset(Isolate::GetCurrent(), Local<Function>::Cast(result0));
#else
...
#ifdef NEWAPI
	Local<Function> fn = Local<Function>::New(Isolate::GetCurrent(), encryptFunction);
	return callSafe(fn, argc, argv, output);
#else
...
static RETURNTYPE encrypt(const ARGUMENTS &args)
{
	HandleScope scope;
	if (args.Length() < 2) 
	{
...
/* decrypt passphrases
 */
static RETURNTYPE decrypt(const ARGUMENTS &args)
{
	HandleScope scope;
	if (args.Length() < 2) 
	{
