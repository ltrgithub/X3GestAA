...
#if NODE_MAJOR_VERSION > 0 || (NODE_MAJOR_VERSION == 0 && NODE_MINOR_VERSION >= 12)
#define V12
...
bool cryptWindows(char* input, Local<Value>* result)
{
	...
	{
		BUFFER_TYPE resultb = Buffer::New((char*) data_out.pbData, (size_t) data_out.cbData);
...
bool decryptWindows(Local<Object> input, Local<Value>* result)
{
	...
	{
		*result = STRING_NEW((char*) data_out.pbData, data_out.cbData);
		LocalFree(data_out.pbData);
		return true;
	}
	else
...
static bool callEncryptFunction(Local<Value> input, Local<Value>* output, BUFFER_TYPE key, BUFFER_TYPE iv, bool encrypt)
{
	const unsigned argc = 4;
	Local<Value> argv[argc] = { BUFFER_TO_HANDLE(key), input, BOOLEAN_NEW(encrypt)->ToBoolean(), BUFFER_TO_HANDLE(iv) };
...
bool makeKey(BUFFER_TYPE* result, char* iv)
{
  char key[KEY_LENGTH];
  int length;
  int i;
...
bool cryptLinux(char* input, Local<Value>* result, Local<Value> req)
{
	...
	for (int i=0; i<16; i++)
		ch_iv[i] = rand() % 256;
	BUFFER_TYPE iv = Buffer::New(ch_iv, 16);
	BUFFER_TYPE key;
	...
	if (!makeKey(&key, ch_iv))
		return false;
	BUFFER_TYPE toEncrypt = Buffer::New(inputlength);
	...
	if (!callEncryptFunction(BUFFER_TO_HANDLE(toEncrypt), &encrypted, key, iv, true))
		return false;
	size_t encryptedLength = Buffer::Length(encrypted->ToObject());
	// add iv to encrypted data
	BUFFER_TYPE encryptedIv = Buffer::New(encryptedLength+IV_LENGTH);
...
bool decryptLinux(Local<Object> input, Local<Value>* result, Local<Value> req)
{
	...
	BUFFER_TYPE iv = Buffer::New(input_data, IV_LENGTH);
	BUFFER_TYPE input1 = Buffer::New(input_data+IV_LENGTH, input_length-IV_LENGTH);
	if (!callEncryptFunction(BUFFER_TO_HANDLE(input1), &decrypted, key, iv, false))
		return false;
...
{
	target->Set(STRING_NEW_SYMBOL("encrypt"), FUNCTION_TEMPLATE_NEW(encrypt)->GetFunction());
	target->Set(STRING_NEW_SYMBOL("decrypt"), FUNCTION_TEMPLATE_NEW(decrypt)->GetFunction());
}
