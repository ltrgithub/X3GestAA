...
#if NODE_MAJOR_VERSION > 0 || (NODE_MAJOR_VERSION == 0 && NODE_MINOR_VERSION >= 11)
#define NEWAPI
#endif
...
#define FUNCTION_TO_LOCAL(pfn)		Local<Function>::New(Isolate::GetCurrent(), pfn)
#else
...
#define FUNCTION_TO_LOCAL(pfn)		pfn
#endif
...
static bool callEncryptFunction(Local<Value> input, Local<Value>* output, BUFFER_TYPE key, BUFFER_TYPE iv, bool encrypt)
{
	...
	Local<Value> argv[argc] = { BUFFER_TO_HANDLE(key), input, Boolean::New(encrypt)->ToBoolean(), BUFFER_TO_HANDLE(iv) };
	Local<Function> encryptFunctionLocal = FUNCTION_TO_LOCAL(encryptFunction);
