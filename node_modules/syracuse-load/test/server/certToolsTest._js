...
asyncTest('Certificate operations in file system', 89, function(_) {
	var directory = 'certificatetest/';
	try {
		fs.mkdirSync(directory);
	} catch (e) {
	...
	strictEqual(key, fileContent, "correct key in file system");
	var phrases = certTools.readPassphrases(directory, _);
	strictEqual(phrases["xyz#server"], "server", "correct passphrase");
	var data = [{
		name: "server",
		tenantId: "xyz",
		del: true
	...
	strictEqual(existent, false, "key deleted");
	var phrases = certTools.readPassphrases(directory, _);
	strictEqual(phrases["xyz#server"], undefined, "deleted passphrase");
	try {
