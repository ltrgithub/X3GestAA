...
asyncTest('CA Certificate operations in file system', 26, function(_) {
	...
	var h = {missingCA: ["ca", "ca2", "ca1"], connectionData: [{active: true, serverCert:{name: 'bla', caCertificates: [{name: "ca"}]}}]};
...
test('deep equal', 10, function() {
	...
	strictEqual(certTools.deepEqual({a:5, b:7}, {b:7, a:5}), true, "two equal simple objects");
	strictEqual(certTools.deepEqual({a:5, b:7, c:8}, {b:7, a:5}), false, "extra property in first argument");
	strictEqual(certTools.deepEqual({a:5, b:7}, {b:7, a:5, c:8}), false, "extra property in second argument");
	...
	strictEqual(certTools.deepEqual({a:{c: 77}, b:[5,{t:8, r:4}]}, {b:[5,{r:4, t:8}], a:{c: 77}}), true, "two equal structured objects");
})
 ...
 asyncTest('Certificate operations in file system', 74, function(_) {
	...
	// put certificate
	var h = {missingCert: ["ca", "server"], connectionData: [{active: true, serverCert: { name: "server"}}]};
	var data = [{
		name: "server",
		cert: crt,
		key: key,
	...
	var changed = {};
	var h = {missingCert: [], connectionData: [{active: false, serverCert: {name: "server"}}]};
	var res = certTools.parseRequestCert(_, data, certificates, caCertificates, directory, h, changed);
	...
	var changed = {};
	var h = {missingCert: [], connectionData: [{active: true, serverCert: {name: "server"}}]};
	var res = certTools.parseRequestCert(_, data, certificates, caCertificates, directory, h, changed);
