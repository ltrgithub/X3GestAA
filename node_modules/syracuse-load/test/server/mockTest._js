...
asyncTest('copy test', 5, function(_) {
	var result = mocks.extractDataFromRequest({url: 5, method:"get", foo: 18, connection: {authorized: true, _peerCertificate: 55 } });
...
console.log("=============================================================")
	...
	strictEqual(server.numberRequests(), 0, "no open request on server")
	strictEqual(client.numberRequests(), 0, "no open request on client")
...
asyncTest('Stream destroy test', 19, function(_) {
	...
	serverResClosed = 0;
	try {
		mocks.simpleRequest(client, { path: "/destroyRequest"}, "", _);
	...
	serverResClosed = 0;
	try {
		mocks.simpleRequest(client, { path: "/destroyResponse"}, "", _);
		throw new Error("No close detected")
	} catch (e) {
	...
	serverResClosed = 0;
	try {
		mocks.simpleRequest(client, { path: "/destroyResponse2"}, "", _);
		throw new Error("No close detected")
	} catch (e) {
	...
	function closeClient(mode, callback) {
		var req = client.request({ path: "/" }, function(response) {
			var data = "";
			var answered = false;
			response.on('close', function() {
				if (!answered) {
...
asyncTest('Timeout test', 4, function(_) {
	var toServer = new AsyncPipeStream();
	var toClient = new AsyncPipeStream();
	var client = new mocks.Mock(toServer, toClient, null, {timeout: 1});
	// server with request handler
	var server = new mocks.Mock(toClient, toServer, function(request, response) {
		var data = "";
		console.log("URL "+request.url)
		var time = request.url.replace(/[^\d]/g, "");
		console.log("URL2 "+time)
		setTimeout(function() {
			response.end("OK");
		}, 1*time)
	});
	...
	var result = mocks.simpleRequest(client, { path: "/0"}, "", _);
	strictEqual(result, "OK", "within timeout");
	// slow request not within timeout limit
	try {
		mocks.simpleRequest(client, { path: "/1500"}, "", _);
		throw new Error("No timeout detected")
	} catch (e) {
		strictEqual(""+e, "Timeout", "with timeout");		
	}
	strictEqual(server.numberRequests(), 0, "no open request on server")
	strictEqual(client.numberRequests(), 0, "no open request on client")
	start();
