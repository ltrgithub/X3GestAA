...
exports.MockStreamServer = function(disp, outputStream, inputStream, clientOptions, options) {
	...
	self.listen = function() {
		...
		try {
			...
			console.log("LISTEN PREP", options, clientOptions);
			if (options.extraIn) {
				console.log("LISTEN PREP2 ");
				var extraInputStream = new DescriptorReadStream(options.extraIn);
				self._xx = new Mock(new DescriptorWriteStream(options.extraOut), extraInputStream, dispatcher, clientOptions, true);
				console.log("LISTEN PREP30");
				extraInputStream.resume();
				console.log("LISTEN PREP3");
			}
			if (self._mockEngine) {
...
function DescriptorReadStream(fd) {
	var self = this;
	this._paused = true;
	this._chunk;
	this._ended = false;
	this._hasChunk = false;
	this.readable = true;
	self.pause = function() {
		this._paused = true;
	};
	self.resume = function() {
		...
		process.nextTick(function() {
			if (!self._paused && self.readable) {
				if (self._hasChunk) self.emit('data', buffer);
				_readByte();
			}
		});
	};
...
DescriptorReadStream.prototype = new events.EventEmitter();

exports.DescriptorWriteStream = DescriptorWriteStream;
exports.DescriptorReadStream = DescriptorReadStream;