...
exports.extractDataFromRequest = function(source, target) {
	...
	if ("connection" in source) {
		target.connection = {
			...
			localPort: (source.connection.localPort || source.connection.__syra_localPort) // support for SSL, because there is no connection.localPort
		};
...
exports.attachEngineIO = function(client, findClient) {
	var engine = require('socket.io/node_modules/engine.io')(client, {
		path: "/socket.io"
	});
	engine.on('connection', function(socket) {
		// find out which process belongs to this session
		var mockClient = findClient(socket.request);
		if (!mockClient) socket.close();
...
Mock.prototype.registerEngine = function(socket) {
	...
	socket.on('close', function(reason) {
		console.log("Fehlt: close " + reason);
	...
	socket.on('message', function(data) {
		console.log("Fehlt: message " + data);
	...
	socket.on('error', function(error) {
		...
		console.log("Fehlt: error " + JSON.stringify(error));
...
MockSocket.prototype.write =
	MockSocket.prototype.send = function(data) {
		console.log("engine.io MOCKSOCKET Send " + data);
		if (data) {
			var mode = IO_SEND;
			if (Buffer.isBuffer(data)) {
				mode = IO_SENDB;
	...
	if (headerType & SPECIAL) {
		switch (headerType) {
			...
			case IO_CONN:
				...
				tracer && tracer("Receive engine.io connection header for " + headerNumber + " response " + !! responseHeader);
				if (!self._mockEngine) {
			...
			case IO_CLOSE:
				// receive close event
				tracer && tracer("Receive engine.io close header for " + headerNumber + " response " + !! responseHeader);
				var reg = (responseHeader ? self._registry : self._registryForeign);
				if (headerNumber in reg) {
					var entry = reg[headerNumber];
					var socket = entry[4];
			...
			case IO_SENDB:
				// receive message
				tracer && tracer("Receive engine.io header for " + headerNumber + " response " + !! responseHeader);
				tracer && tracer("!!! " + data);
				console.log("Receive engine.io header for " + headerNumber + " response " + !! responseHeader);
				console.log("!!! " + data);
				var reg = (responseHeader ? self._registry : self._registryForeign);
				if (headerNumber in reg) {
					var entry = reg[headerNumber];
