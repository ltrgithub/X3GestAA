...
function sign(_, cert, algorithm, data, options) {
	...
	sign.update(data, options ? options.data_encoding : undefined);
	return sign.sign(key, options ? options.output_encoding : undefined);	
...
function verify(_, cert, algorithm, data, signature, options) {
	...
	verify.update(data, options ? options.data_encoding : undefined);
	return verify.verify(certificate, signature, options ? options.signature_encoding : undefined);
}
...
function getPEMCertificate(_, name, ca) {
	...
	try {
		...
		throw new Error("noCert$#$Certificate "+name+" not available");
	}
	return certificate;
}
...
function parseRequestCert(_, data, certificates, caCertificates, directory, host, changed) {
	...
	data.forEach_(_, function(_, part) {
		...
		if (part.del) {
			...
			if (host) {
				var array = part.ca ? host.missingCA : host.missingCert;
				var index = array.indexOf(part.name);
				if (index >= 0) {
					array.splice(index, 1);
					// changes in list of missing certificates
					if (changed) changed.missing = true;
				}
			}
		} else {
			if (part.cert || part.key) {
				// update
				// check data
				...
				var integ = jsx509.integrity(checkCert, checkKey, part.pass);
				if (integ.error) {
				...
				try {
					if (part.cert) {
						var filename = directory+part.name+crtExtension;
				...
				if (!part.ca) {
					try {
						if (part.key) {
							...
							updatePassphrases = true;
							var filename = directory+part.name+".key";
						...
						host.connectionData.forEach(function(data) {
							if (data.active) {
								if (_changes(data.serverCert, part.name, part.ca)) {
									if (newData) changed.start = true 
									else changed.restart = true;
								}									
							}
						})
					}
	...
	if (updatePassphrases) writePassphrases(directory, passphrases, _)
	return response;
}
...
function _changes(cert, name, ca) {
	...
	if (!ca) return cert.name === name;
	// test CA certificates
	if (!cert.caCertificates) return false;
...
function readPassphrases(directory, _) {
	try {
		return JSON.parse(fs.readFile(directory+lcHostName+".pass", "utf8", _));
	} catch (e) {
...
function writePassphrases(directory, phrases, _) {
	var filename = directory+lcHostName+".pass"
	fs.writeFile(filename, JSON.stringify(phrases), "utf8", _)
	fs.chmod(filename, 0x180, _); // read and write permissions: 0600
}
