...
function parseRequestCert(_, data, certificates, caCertificates, directory, host) {
	...
	data.forEach_(_, function(_, part) {
		...
		if (part.del) {
			...
			if (part.cert || part.key) {
				...
				try {
					if (part.cert) {
						fs.writeFile(directory+part.name+crtExtension, part.cert, "utf8", _);					
					}
				} catch (e) {
				...
				if (!part.ca) {
					try {
						if (part.key) {
							...
							updatePassphrases = true;
							fs.writeFile(directory+part.name+".key", part.key, "utf8", _);					
						}
					} catch (e) {
				...
				if (list) {
					...
					if (part.key) localEntry.key = integ.key; // decrypted key					
				}
				if (host) {
					var array = part.ca ? host.missingCA : host.missingCert;
					var index = array.indexOf(part.name);
				...
				if (!list.some_(_, function(_, cert) {
					...
					console.log("Not found "+part.name);
				}
			}
		}
	})
...
function writePassphrases(directory, phrases, _) {
	fs.writeFile(directory+lcHostName+".pass", JSON.stringify(phrases), "utf8", _)
}
