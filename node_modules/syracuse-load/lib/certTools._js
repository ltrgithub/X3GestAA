		...
		if (part.del) {
			...
			if (part.cert || part.key || part.cas) {
				...
				if (list) {
					...
					var change = false;
					var newData = !localEntry.certificate || !localEntry.key;
					...
					if (change && host && changed && host.connectionData) {
						host.connectionData.forEach(function(data) {
							if (data.active) {
								if (data.ssl && (_changes(data.serverCert, part.name, part.ca) || data.clientAuth && _changes(data.clientCert, part.name, part.ca))) {
									if (newData) {
										changed.start = true;
									} else {
...
function _getCryptoModule() {
	...
	var filename = __dirname + '/../bin/' + arch + '/' + v8 + '/crypt.node';
	if (fs.existsSync(filename)) return require(filename);
	else {
		filename = __dirname + '/../bin/' + arch + '/default/crypt.node';
		if (fs.existsSync(filename)) return require(filename);
	}
