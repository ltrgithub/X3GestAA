...
//  
exports.setPassphrase = function(_, name, passphrase) {
...
function streamHttpRequest(_, name, options, cas, tenantId) {
	...
	if (cas && cas.length) {
		cas = cas.map_(_, function(_, caName) {
			if (caName != "ca" && tenantId) caName = tenantId + "#" + caName;
...
function parseRequestCert(_, data, certificates, caCertificates, directory, host, changed) {
	...
	data.forEach_(_, function(_, part) {
		...
		var array;
		var i;
		var fullName = (part.tenantId ? part.tenantId + "#" + part.name : part.name);
		if (part.del) {
			// delete
			if (list) {
			...
			// delete file
			try {
				fs.unlink(directory + fullName + crtExtension, _);
			} catch (e) {
				console.log("Deletion error certificate " + e);
			}
			if (!part.ca) {
				try {
					fs.unlink(directory + fullName + ".key", _);
				} catch (e) {
					console.log("Deletion error key file " + e);
				}
				if (fullName in passphrases) {
					delete passphrases[fullName];
			...
			if (part.cert || part.key || part.cas) {
				...
				if (!list) { // for usage outside of cluster 
					if (!part.cert) {
						try {
							checkCert = fs.readFile(directory + fullName + crtExtension, "utf8", _);
						} catch (e) {
					...
					if (!part.key && !part.ca) {
						try {
							var keyfile = fs.readFile(directory + fullName + ".key", "utf8", _);
							checkKey = jsx509.stripEncryption(keyfile, passphrases[fullName]);
						} catch (e) {
							if (part.keyTest) {
				...
				if (part.cas) {
					for (var i = 0; i < part.cas.length; i++) {
						...
						if (caCertificates) {
							...
							try {
								caCerts.push(fs.readFile(directory + (name === "ca" ? name : (part.tenantId ? part.tenantId + "#" : "") + name) + ".cacrt", "utf8", _));
							} catch (e) {
								// ignore non existing file
							}
						}
				...
				try {
					if (part.cert) {
						filename = directory + fullName + crtExtension;
				...
				if (!part.ca) {
					try {
						if (part.key) {
							passphrases[fullName] = part.pass;
							updatePassphrases = true;
							filename = directory + fullName + ".key";
				...
				if (!list.some_(_, function(_, cert) {
					if (cert.name === part.name) {
						var encryptedKey;
						if (!part.ca && cert.key) {
							encryptedKey = fs.readFile(directory + fullName + ".key", "utf8", _);
						}
						response.push({
							name: part.name,
							ca: part.ca,
...
function readPassphrases(directory, _, exc) {
	try {
		...
		if (e.code === 'ENOENT') {
			...
			console.log("Error in passphrase file " + e);
...
function writePassphrases(directory, phrases, _) {
	...
	for (var i = 0; i < 5; i++) {
		...
		if (test !== encrypted) {
			console.log("Write passphrases again");
			continue;
		}
		return;
	}
