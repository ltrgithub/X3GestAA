		...
		if (part.del) {
			...
			if (part.cert || part.key || part.cas) {
				...
				if (!list) { // for usage outside of cluster 
					...
					if (!part.key) {
						try {
							...
							if (part.keyTest) {
								throw new Error("keyCorrupt$#$Private key file corrupt on server for "+part.name+": "+e);
							}
						// ignore non existing file
						}
					} 
				...
				if (part.keyTest && !checkKey) {
					throw new Error("keyMissing$#$Private key file missing on server for "+part.name);
				}
				var caCerts = [];
				if (part.cas) {
...
function _getCryptoModule() {
	return require("syracuse-load/"+os.platform()+'_'+os.arch()+"/crypt")		
}
...
function readPassphrases(directory, _) {
	try {
		...
		return JSON.parse(cr.decrypt(new Buffer(crypted, "base64")));
	} catch (e) {
...
function writePassphrases(directory, phrases, _) {
	...
	var cr = _getCryptoModule();
	fs.writeFile(filename, cr.encrypt(clear).toString("base64"), "utf8", _);
	fs.chmod(filename, 0x180, _); // read and write permissions: 0600
}
