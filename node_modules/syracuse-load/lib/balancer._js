...
function start(conf, _) {
	...
	if (process.argv[2] === "install") {
		try {
			...
			if (localHosts.length > 0) {
				...
				// new host
				var newHost = {_id: uuid.generate(), hostname: config.servername, 
						connectionData: [{port: port, active: true, ssl: false, clientAuth: false, serverCert: null, clientCert: null, _uuid: uuid.generate()}],
	...
	case "passphrase":
		...
		try {
			...
			} catch (e) { console.log("Could not contact nanny " + e+" "+e.stack+" "+e.trace);
			}
			return 0;
		} catch (e) {
...
function _loadCertificates(_) {
	...
	if (!caCertificates.some(function(certificate) { 
		...
		var newCertificate = {_id: uuid.generate(), name: "ca", internal: true, _creUser: "setup", _updUser: "setup", _updDate: new Date() };
	...
	if (ownCaCertificate) {
		...
		if (!certificates.some(function(certificate) {
			...
			var newCertificate = {_id: uuid.generate(), name: lcHostName, internal: true, keyExists: true, caCertificates: [dbDriver.setRelatedInstance(ownCaCertificate)],
...
function updateHostData(startChildren, _, init) {
	...
	if (localHost.stop) { // data for local host have changed
		...
		shutdown(undefined, undefined, true);		
	} else {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {		
		...
		case 'notifyOne': // send request only to local server with lowest number of sessions (without '/nannyCommand/notifyOne')
			...
			try {
				...
				future.extra = {port: data[1], hostname: localHost.hostname};
				futures.push(future);
			} catch (e) {
...
function stopSessions(sessionCollection, exclude, _, notify) {
	return (function(callback) {
...
function checkFileCertificates(_, certificate, ca, passphrases, newPassphrase, cas, files) {
	...
	if (files) {
		var index = files.indexOf(basename);
		if (index < 0) {
	...
	try {
		var cert = fs.readFile(certDirectory+basename, "utf8", _);		
	} catch (e) {
	...
	if (!ca && (certificate.keyExists || certificate.internal)) {
		var basename = certificate.name+".key";
		if (files) {
			var index = files.indexOf(basename);
			if (index < 0) {
...
function doRequest(streamRecorder, response, hostPort, _, sessionId) {
	return (function(callback) {
