...
function start(conf, _) {
	switch (process.argv[2]) {
		...
		case "--help":
			...
			process.exit(0); // Process will not terminate otherwise any more
			return 0;
	}
	config = conf;
		...
		if (process.argv[2] === "install") {
			...
			try {
				...
				try {
					...
					process.exit(1); // some blocking process is still running - therefore process.exit();
					return 1;
				} finally {
					server.close(_);
				}
			...
			case "patch":
				...
				if (process.argv[3] === "--unlock") {
					...
					if (!process.argv[3]) {
						...
						process.exit(1); // Process will not terminate otherwise any more
						return 1;
					}
