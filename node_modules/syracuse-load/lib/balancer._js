...
function get(hostdata, method, path, data, _) {
	...
	try {
		...
		if (hostdata.status === STATUS_NOT_REACHABLE) {
...
function start(conf, _) {
	...
	case undefined:
		...
		// localHost.status = (localHost.active ? STATUS_INIT : STATUS_INACTIVE);
...
function requestListener(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {		
		...
		var notifyNannies = false; // "notifyNannies" in the path?
		...
		case 'children': // ping to children
			...
			if (notifyNannies) { // extra information only from current host
			...
			try {
				...
				for (var attempt = 0; attempt < 2; attempt++) {
					for (var i = 0; i<hosts.length; i++) {
						...
						if (!hosts[i].local && hosts[i].hostname === hostData.hostname) {
					...
					if (found < 0) updateHostData(false, _);					
				}
				if (found < 0) {
		...
		case 'down': // nanny has stopped
			...
			for (var i = 0; i<hosts.length; i++) {
				if (!hosts[i].local && hosts[i].hostname === host) {
	...
	if (localHost.status < STATUS_READY) {
		...
		case STATUS_START: temporary = true; text = "Child processes are being started. Try again later"; break;
		case STATUS_INIT: temporary = true; text = "Child processes not yet started. Try again later"; break;
