...
function startChildProcesses(force, _) {
	if (localHost.status > STATUS_INIT || force) {
		startChildrenFunnel(_, function(_) {
			...
			var newBatchCount = dbDriver.count(batchCollection, {
				active: true
			}, _) > 0 ? 1 : 0;
			...
			console.error("Start child processes " + oldChildCount + " " + newChildCount + " " + oldWsChildCount + " " + newWsChildCount + " " + oldBatchCount + " " + newBatchCount);;
