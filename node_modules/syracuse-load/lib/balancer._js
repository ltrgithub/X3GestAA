...
function hostOK(host) {
	return host && !host.local && host.active && host.started && host.version === hosts.mainVersion;
}
...
function getIntern(hostdata, command, callback, forget) {
	...
	var result = "";
	var options = {method: 'GET', hostname: hostdata.local ? "localhost" : hostdata.hostname , port: hostdata.nannyPort, path: command};
...
/** wrapper function for asynchronous calls.
 * See `getIntern` for more details
 * */
function get(hostdata, command, _, forget) {
	return getIntern(hostdata, command, _, forget);
}
...
//	console.log("START function")
		...
		try {
			...
			if (hosts[0]) {
				...
				if (process.argv[5] === "force") {
					try {
						get(hosts[0],  "/nannyCommand/shutdown", _, true);
					} catch (e) {
	...
	case "stop":
		...
		var result = get(hosts[0], "/nannyCommand/stop", _, true);
...
function startSignal(host, _) {
	...
	data = get(host, "/nannyCommand/started/"+hosts[0].hostname+"/"+hosts[0].version, _, true);
	// check versions
...
function requestListener(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		case 'notifyAll': // notify all
			...
			if (clientId) {
				...
				for (var i=1; i<hosts.length; i++) {
					...
					if (host.active && host.started && host.version === hosts.mainVersion) {
						...
						doRequest(recorderStream, dummyResponse, [host, host.nannyPort], _);
						console.log("Answer from host "+i+" "+dummyResponse.content);
					}
				}
			}
			...
			console.log("Keys "+util.format(keys))
			for (var i=0; i< keys.length; i++) {
				var port = keys[i];
				if (port !== clientId) {
					var dummyResponse = new mock.MemoryStream();
					console.log("Response "+util.format(dummyResponse))
					console.log("Request to local app "+port)
					doRequest(recorderStream, dummyResponse, [hosts[0], port], _);
					console.log("Answer from app "+port+" "+dummyResponse.content);
				}
			}
			response.end("OK3");
			break;
		...
		case 'stop': // request to stop server
			...
			hosts.forEach(function(host) {
				if (host && host.started && host.active && !host.local) {
					futures.push(get(host, "/nannyCommand/down/"+hosts[0].hostname, null, true));
				}
			})
			killChildren();
			response.end("OK1");
...
function doRequest(recorderStream, response, hostPort, callback, sessionId) {
	...
	var options;
	var port = ""+hostPort[1];
	if (hostPort[0] === hosts[0] && port.substr(0, 1) === "N") {
