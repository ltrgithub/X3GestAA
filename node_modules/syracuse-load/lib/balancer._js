...
function refreshSessions(balanceData, local, _) {
	...
	var sessionsL = {};
	if (config.system && config.system.keepSessions) {
		Object.keys(sessions).forEach(function(prop) {
			var hp = sessions[prop];
			if (hp && hp[0] && +hp[0].status >= STATUS_READY)
				sessionsL[prop] = sessions[prop];
		})
		tracer && tracer("Keep sessions " + Object.keys(sessions).join(","));
	}
