...
function findClient(request) {
	...
	if (!hostport) {
		console.log("Session not available: " + newSession + " " + request.connection.localPort);
		return undefined;
	}
	...
	if (config.hosting.multiTenant) {
		if (hostHeader) {
			...
			if (!tenantDbObject || tenantId !== tenantDbObject.tenantId) {
				var threshold = now - config.session.timeout * 60000;
...
function healthLoop(_) {
	...
	for (var i = 0; i < children.length; i++) {
		if (children[i].syratime != undefined) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		switch (command[0]) {
			...
			case 'info':
				...
				for (i = 0; i < children.length; i++) {
					var child = children[i];
					childResults.push("N" + i + ": " + child.mockClient.numberRequests() + " " + (child.syralast ? Math.max(0, child.syralast - now) : "-") + ";" + ("" + child.syratime).substr(0, 7));
				}
				for (i = 0; i < wsChildren.length; i++) {
...
function _balanceInternal(_, local) {
	...
	if (!local) { // query other servers
		...
		hosts.forEach(function(host) {
			...
			_balanceHost(host.loaddata, host);
		})
	}
	...
	if (goodchild.syralast) goodchild.syralast += CHILD_PING2_POLLING;
...
function _balanceload(_, sessionId, local) {
	...
	if (array) {
		...
		var timeoutvalue = now - (config.session.timeout || 20) * 60000;
		for (var key in sessions) {
			if (sessions[key][2] < timeoutvalue) {
				tracer && tracer("Delete old session " + key)
				delete sessions[key];
			}
		}
		var data;
