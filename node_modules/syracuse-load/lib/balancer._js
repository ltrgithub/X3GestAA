...
function _getHostByBalancing(localBalancer, cookie, _) {
	...
	if (!sessionCollection) sessionCollection = dbDriver.createCollection(db, 'SessionInfo', _);
	if (!cookie) {
		var data = refreshSessions(sessionCollection, localBalancer, _);
		return data;
	}
...
function startChildProcesses(force, _) {
	if (localHost.status > STATUS_INIT || force) {
		startChildrenFunnel(_, function(_) {
			...
			if (oldChildCount === newChildCount) return;
...
function createProcess(code, port) {
	...
	if (code === undefined) {
		_startMessage("Start process N" + port);
	} else {
		if (code === 5) {
			_startMessage("License problem: no license N" + port);
...
function requestListener0(request, response, _) {
	...
	if (sid) { // session available
		...
		data = _getHostByBalancing(localBalancer, sid, _);
	...
	for (i = 0; i < 2; i++) {
		...
		if (!data) {
			balancerKey = "" + request.connection.localPort + "." + request.headers.cookie;
			data = _getHostByBalancing(localBalancer, balancerKey, _);
			if (!data) return differentVersionError(response);
		}
		try {
...
var doRequest = _(function(streamRecorder, response, hostPort, callback, sessionId, cookieValue) {
	var client;
	var options;
	var sessionHostName;
	...
	var req = client.request(options, function(res) {
		...
		if (cookies) {
			...
			if (cookies && streamRecorder.originalStream.connection && streamRecorder.originalStream.connection.localPort) {
				if (Array.isArray(cookies)) cookies = cookies.join(" ");
