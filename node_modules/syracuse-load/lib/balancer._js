	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
			...
			case 'notifyAll':
				...
				for (i = 0; i < children.length; i++) {
					...
					if (port !== clientId) {
						...
						var future = doRequestWithAnswer(streamRecorder, [localHost, port], !_);
						future.extra = {
							port: port
						};
						futures.push(future);
		...
		switch (command[0]) {
			...
			case 'notifyOne':
				...
				try {
					var future = doRequestWithAnswer(streamRecorder, data, !_);
					future.extra = {
						port: data[1],
			...
			case 'children':
				...
				if (localHost.status >= STATUS_START) {
					...
					for (i = children.length - 1; i >= 0; i--) {
						var future = children[i].mockClient.ping(!_, 1000);
						future.extra = {
							hostname: localHost.hostname,
							port: "N" + i,
			...
			case 'details':
				...
				if (localHost.status === STATUS_READY) {
					for (var i = children.length - 1; i >= 0; i--) {
						var future = children[i].mockClient.detail(!_, 1000);
						future.extra = {
							hostname: localHost.hostname,
							port: "N" + i
						};
