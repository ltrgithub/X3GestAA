...
function updateHostData(startChildren, _) {
	...
	newHosts.forEach_(_, function(_, host) {
		console.log("DATA START "+host.hostname)
		...
		console.log("DATA END "+host.hostname)
	})
...
function _updateHosts(oldHosts, hosts, name) {
	...
	LOOP: while (--i >= 0) {
		...
		if (!host.connectionData || !host.connectionData.length || host.connectionData[0].deactivated) {
			throw "errorConnectionData$#$Inconsistency in connection data of host "+host.hostname;
		}
		...
		if (mock.BALANCER_HEADER in streamRecorder.originalStream.headers) {
			...
			if (sslHeader) {
				try {
					...
					writeError(response, "sslHeaderError$#$Error in internal SSL header: "+e+" Header: "+sslHeader);
				}
			} else {				
				options.connection = {};				
			}			
