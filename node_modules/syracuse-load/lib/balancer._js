...
function get(hostdata, method, path, data, _) {
	...
	try {
		...
		if (!hostdata.local) hostdata.status = STATUS_NOT_REACHABLE; 
		throw e;		
	}
}
...
function start(conf, _) {
	...
	case undefined:
		...
		// hosts[0].status = (hosts[0].active ? STATUS_INIT : STATUS_INACTIVE);
	...
	case "stop":
		...
		db.close();
		try {
			var result = get(hosts[0], "PUT", "/nannyCommand/stop", null, _);
...
function startSignal(host, _) {
	if (host.local && host.status <= STATUS_NOT_REACHABLE) return;
	tracer && tracer("Host with start signal "+util.format(host))
	var data = get(host, "POST", "/nannyCommand/started", JSON.stringify(hosts[0]), _);
	// check versions
...
function checkVersions() {
	console.log("CHECK "+hosts[0].status+" "+hosts.mainVersion)
	if (hosts[0].status !== STATUS_INIT) return;
	var commonVersion;
	if (hosts.mainVersion) {
		...
		hosts.forEach(function(host) {
			console.log("STA "+host.status+" "+host.hostname)
	...
	if (commonVersion) {
		...
		hosts[0].status = STATUS_WRONG_VERSION;
	}
}


function startChildProcesses(force) {
	if (hosts[0].status >= STATUS_START || force) {
		// new number of children
		var newChildCount = (hosts[0].status >= STATUS_INIT) ? hosts[0].children : 0;
		...
		if (newChildCount > 0 && hosts[0].status < STATUS_START) hosts[0].status = STATUS_START;
		if (hosts[0].status === STATUS_START) {
		...
		console.log("Start process on port N"+port)
			...
			if (child && respawnTime && startTime-child.startTime < respawnTime) {
				...
				if (child.startCount > hosts[0].respawnCount) {
					...
					hosts[0].status = STATUS_RESPAWN;
					hosts.forEach(function(host) {
						if (host && host.status >= 0 && !host.local) {
							// do not wait for execution of these calls
							get(host, "PUT", "/nannyCommand/down/"+hosts[0].hostname, null, undefined);
						}
					})
					killChildren();
				}
...
function _updateHosts(oldHosts, hosts, name) {
	...
	LOOP: while (--i >= 1) {
		...
		if (host.started && host.status === STATUS_NOT_STARTED || host.active && host.status === STATUS_INACTIVE) host.status = undefined;
		if (!host.local && !host.started) host.status = STATUS_NOT_STARTED;
		else {
			if (!host.active) host.status = STATUS_INACTIVE;
		}
		if (host.status === undefined) { // status not yet set: a foreign host is not reachable unless it is known to be reachable
			host.status = (host.local ? STATUS_INIT : STATUS_FOREIGN);
		}
	}
...
function requestListener(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		case 'stop': // request to stop server
			...
			for (i = 0; i<hosts.length; i++) {
				if (host && host.status >= 0 && !host.local) {
					futures[i] = get(host, "PUT", "/nannyCommand/down/"+hosts[0].hostname, null, undefined);
				}
			}
			killChildren();
			resultText = "OK1"
		...
		case 'children': // ping to children
			if (hosts[0].status >= STATUS_START) {
				for (var i = children.length-1; i>=0; i--) {
					...
					future.extra = {hostname: hosts[0].hostname, port: "N"+i, requests: children[i].mockClient.numberRequests()};
					futures.unshift(future)
				}				
			} else {
				resultText += JSON.stringify({hostname: hosts[0].hostname, message: "Children not yet started"});
			}
			break;
		...
		case 'started': // nanny has started
			try {
				...
				for (var attempt = 0; attempt < 2; attempt++) {
					for (var i = 1; i<hosts.length; i++) {
						...
						if (hosts[i].hostname === hostData.hostname) {
							hosts[i].status = hostData.status;
				...
				if (!found) {
					response.end("??;"+hosts[0].version+";"+hosts[0].status)
					return;
				}
				checkVersions();
				// maybe update status of other host
				console.log("HOST2 "+util.format(hosts[found]))
				if (found && hosts.mainVersion !== hosts[found].version) hosts[found].status = STATUS_WRONG_VERSION;
				var returnValue = hosts.mainVersion+";"+hosts[0].version+";"+hosts[0].status
		...
		case 'down': // nanny has stopped
			...
			for (var i = 1; i<hosts.length; i++) {
				if (hosts[i].hostname === host) {
					...
					hosts[i].status = STATUS_NOT_STARTED;
	...
	if (hosts[0].status < STATUS_READY) {
		var text;
		switch (hosts.status) {
		case STATUS_INIT: text = "No child processes started"; break;
		case STATUS_TIME_DIFFERENCE: text = "Host has been deactivated because of time difference"; break;
		case STATUS_WRONG_VERSION: text = "Version mismatch: main version "+hosts.mainVersion+", local version "+hosts[0].version;
		case STATUS_RESPAWN: text = "Respawn limit exceeded. Applications cannot be started";
		case STATUS_INACTIVE: text = "Host is not active";
		default: text= "Wrong status "+hosts[0].status;
		}
		writeError(response, text);
		return;
	}
	...
	if (now < lastDatabaseCheck.time || now-lastDatabaseCheck.time > TIME_THRESHOLD || ++lastDatabaseCheck.requests > REQUEST_THRESHOLD*hosts.length) {		
		...
		if (diff > TIME_THRESHOLD || -diff > TIME_THRESHOLD) {
			hosts[0].status = STATUS_TIME_DIFFERENCE;
