...
function _getBalanceData(_, hostHeader) {
	if (hostHeader && config.hosting.multiTenant) {
        ...
        if (tenantId !== tenantDbId || !tenantDb) {
            ...
            var expiredConnectionIndexes = [];
            // checks if tenant is in the tenantDbCollection
            dbObject = tenantDbCollection.filter(function(db,index){
                ...
                // checks if timestaps are older than timeout specified in the session timeout. if older add them to a collection to be removed later
                if (db.timeStamp < (Date.now() - config.session.timeout *60000){
                    expiredConnectionIndexes.splice(0,0,index)
                }
                return 	db.tenantId == tenantId
            })[0];  // tenantID is uniquie so there should only be one element in the array
            // removes expired connections from tenantDbCollection
            expiredConnectionIndexes.forEach(function(index){
               	dbDriver.close(tenantDbCollection[index].tenantDb);
                tenantDbCollection.splice(index,1);
            });
            // db object is not found in tenatnDbCollection
            if (!dbObject)
            {
                tenantDb = null;
                ...
                tenantDbCollection.push(dbObject)
            }
            else{
                tenantDb = dbObject.tenantDb
            }
            tenantDbId = tenantId;
        }
