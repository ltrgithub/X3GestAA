...
function _balanceInternal(_, local) {
	...
	function _balanceHost(array, host) {
		...
		while (--i >= 0) {
			var child = array[i];
			tracer && tracer(host.hostname + " " + i + " " + child.syratime + " " + child.syralast + " " + now + " minimaltime " + minimaltime + " minimalload " + minimalload);
...
function doRequestWithAnswer(streamRecorder, hostPort, _, client) {
	...
	tracer && tracer("Answer from host " + hostPort[0].hostname + ":" + hostPort[1] + " " + dummyResponse.content);
