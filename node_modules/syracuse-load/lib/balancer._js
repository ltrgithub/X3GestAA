...
//	console.log("START function")
	...
	var hosts = hostsCollection.find().toArray(_);
	// data for local host
	hosts.unshift(null);
	var i = hosts.length;
	while (--i >= 1) {
...
function proxy(hosts, sessionCollection) {
	http.createServer(function(request, response) { return requestListener(request, response, hosts, sessionCollection, function(error, data) {
		if (error)
...
function requestListener(request, response, hosts, sessionCollection, _) {
	request.pause();
	...
	if (cookies) {
		...
		if (r) {
			...
			if (session in sessions) {
				...
				var data = refreshSessions(sessionCollection, hosts, _);
...
function refreshSessions(sessionCollection, hosts, _) {
	// console.log("Refresh")
...
function doRequest(request, response, hostPort, callback) {
...
// console.log("2 part res")
		...
		// console.log(request.url+ "  Cookies "+cookies);
		res.on("data", function(chunk) {
