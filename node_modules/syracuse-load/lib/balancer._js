...
function findClient(request) {
	...
	if (!request.connection.__syra_localPort) {
		var re = /\:(\d+)$/.exec(request.headers.host);
		if (re) {
			request.connection.__syra_localPort = +re[1];
			console.log("Set local port " + re[1]);
		}
	}
...
function _getBalanceData(_, hostHeader) {
	if (config.hosting.multiTenant) {
		if (hostHeader) {
			...
			if (tenantId !== tenantDbId || !tenantDb) {
				try {
					dbDriver.close(tenantDb);
				} catch (e) {
					console.error("Error during closing tenant db for " + tenantDbId + ": " + e.stack);
				}
				tenantDb = null;
...
WritableIOStream.prototype.write = function(data, enc) {
	...
	return false; // TODO: drain!! Damit funktioniert es derzeit nicht
	*/
	return true;
};
...
function ReadableIOStream(socket) {
	...
	socket.on('close', function() {
		console.error("CLSE " + this.socket);
		if (self.socket) {
			console.error("CLSE2");
			self.emit('close');
			self.readable = false;
...
function transferSocket(hostname, socket) {
	...
	console.log("----------------Set mock client " + hostname);
	...
	host.mockClient.on('close', function() {
		console.log("---------==remove mock client " + (host.mockClient || {})._ident);
...
function makeEngineIOClient(host) {
	...
	console.log("Make engine IO client " + host.mockClient);
	if (host.hostname < localHost.hostname) {
		var uri = (host.tcpHostName || host.hostname) + ":" + host.connectionData[0].port;
		console.log("Verbinden " + uri);
