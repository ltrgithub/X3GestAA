		...
		for (var attempts = 0; attempts < 2; attempts++) {
			...
			if (data.length === hosts.length + 1) {
				for (i = data.length - 1; i > 0; i--) {
					if (hosts[i - 1] && !hosts[i - 1].local) {
						hosts[i - 1].status = data[i];
						if (hosts[i - 1].inactive && data[i] > STATUS_INACTIVE) {
							hosts[i - 1].status = STATUS_INACTIVE;
							oldData = true;
							console.log("OLD data 1");
						};
						if (!hosts[i - 1].inactive && data[i] === STATUS_INACTIVE) {
							oldData = true;
							console.log("OLD data 2");
						}
					}
				}
			} else if (data.length > 1) {
				oldData = true;
				console.log("OLD data 3");
			}
			if (oldData) {
				oldData = false;
				if (!attempts) {
					// first run: update local data
					console.log("OLD data 4");
					updateHostData(true, _);
				} else {
