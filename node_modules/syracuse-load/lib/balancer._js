...
function startSignal(host, _) {
	...
	var data = get(host, "POST", "/nannyCommand/started", JSON.stringify(hosts[0]), _, true);
	// check versions
				...
				if (child.startCount > hosts[0].respawnCount) {
					...
					hosts.forEach(function(host) {
						if (host && host.started && host.active && !host.local) {
							...
							get(host, "PUT", "/nannyCommand/down/"+hosts[0].hostname, null, undefined, true);
						}
					})
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		case 'stop': // request to stop server
			...
			for (i = 0; i<hosts.length; i++) {
				if (host && host.started && host.active && !host.local) {
					futures[i] = get(host, "PUT", "/nannyCommand/down/"+hosts[0].hostname, null, undefined, true);
				}
			}
			killChildren();
			resultText = "OK1"
