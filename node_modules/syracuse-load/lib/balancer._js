...
function firstStartSignal(_) {
	...
	hosts.forEach(function(host) {
		if (!host.local && host.started) {
			var signal = startSignal(host, void _, true);
...
function startChildProcesses(force, _) {
	if (localHost.status > STATUS_INIT || force) {
		...
		if (localHost.status === STATUS_START) {
			...
			var futures = [];
			children.forEach(function(child) {
				futures.push(child.mockClient.ping(void _, PING_TIMEOUT));
			});
...
function createProcess(code, port) {
	...
	if (!child || !child.terminate) { // restart processes (unless terminating all processes is intended)
		var child = child_process.spawn(process.argv[0], ['.', "N" + port, localHost.returnRequestTimeout], {
...
function updateHostData(startChildren, _, init) {
	...
	if (localHost.stop) { // data for local host have changed
		...
		shutdown(void _, undefined, true);
	} else {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
			...
			case 'notifyAll':
				...
				if (clientId) {
					...
					hosts.forEach(function(host) {
						if (!host.local && host.status === STATUS_READY) {
							...
							futures.push(doRequestWithAnswer(streamRecorder, [host, null], void _));
						}
					});
				}
				...
				for (i = 0; i < children.length; i++) {
					...
					if (port !== clientId) {
						...
						var future = doRequestWithAnswer(streamRecorder, [localHost, port], void _);
						future.extra = {
							port: port
						};
						futures.push(future);
			...
			case "notifyNannies":
				...
				hosts.forEach(function(host) {
					if (!host.local && host.status >= STATUS_INACTIVE) {
						tracer && tracer("Request to server " + host.hostname);
						var fut = doRequestWithAnswer(streamRecorder, [host, null], void _);
		...
		switch (command[0]) {
			case 'stop':
				...
				for (i = 0; i < hosts.length; i++) {
					if (host && host.status >= STATUS_INACTIVE && !host.local) {
						futures[i] = get(host, "PUT", "/nannyCommand/down/" + localHost.hostname, null, void _);
					}
				}
				killChildren();
				resultText = "OK1";
			...
			case 'notifyOne':
				...
				try {
					var future = doRequestWithAnswer(streamRecorder, data, void _);
					future.extra = {
						port: data[1],
			...
			case 'children':
				...
				if (localHost.status >= STATUS_START) {
					...
					for (i = children.length - 1; i >= 0; i--) {
						var future = children[i].mockClient.ping(void _, 1000);
						future.extra = {
							hostname: localHost.hostname,
							port: "N" + i,
					...
					if (destination === "*") { // send request to all servers
						...
						hosts.forEach(function(host) {
							if (!host.local && host.status >= STATUS_READY && host.dhKey) {
								...
								futures.push(secureRequest(host, "POST", "/nannyCommand/updateCertificate", d, null, void _));
							}
						});
					} else {
			...
			case 'details':
				...
				if (localHost.status === STATUS_READY) {
					for (var i = children.length - 1; i >= 0; i--) {
						var future = children[i].mockClient.detail(void _, 1000);
						future.extra = {
							hostname: localHost.hostname,
							port: "N" + i
						};
		...
		try {
			...
			if (localHost.status < STATUS_READY) {
				...
				for (var i = children.length - 1; i >= 0; i--) {
					...
					if ("N" + i !== exclude) futures.push(mock.simpleRequest(children[i].mockClient, options, void _));
				}
			}
