...
function healthLoop(_) {
	...
	for (var i = 0; i < children.length; i++) {
		if (children[i] && children[i].syratime != undefined) {
			var future = children[i].mockClient2.ping2(!_, CHILD_PING2_TIMEOUT);
			future.extra = ["N" + i, children[i]];
			futures.push(future);
		}
	}
	restrictedServers.forEach(function(server) {
		var future = server.mockClient2.ping2(!_, CHILD_PING2_TIMEOUT);
		if (children[i]) {
	...
	futures.forEach_(_, function(_, future) {
		try {
			...
			if (result[1].substr(0, 1) !== "0" && future.extra[0])
