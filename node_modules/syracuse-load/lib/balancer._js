	...
	if (process.argv[2] === "install") {
		try {
			...
			if (localHosts.length > 0) {
				...
				// new host
				var newHost = {_id: generateUUID(), hostname: config.servername, nannyPort: nannyPort, children: ports, _creUser: "setup", _updUser: "setup", _updDate: new Date(), active: true, started: false, respawnCount: 10, respawnTime: 120, returnRequestTimeout: 30, tcpHostName: tcpHostName}
	...
	case undefined:
		...
		if (!process.argv[2]) {
			hosts.forEach(function(host) {
				...
				if (host.hostname !== localHost.hostname && host.active && relNumberCmp(parts[0], localHost.version) > 0) {
		...
		case "notifyNannies": // send the command (without '/notifyOtherNannies' in URL) to all nannies which have status at least inactive.
			...
			hosts.forEach(function(host) {
				if (!host.local && host.status >= STATUS_INACTIVE) {
					tracer && tracer("Request to server "+host.hostname)
					var fut = doRequestWithAnswer(streamRecorder, [host, host.nannyPort]);
...
function stopSessions(sessionCollection, exclude, callback) {
	...
	function pollfunction() {
		...
		console.log("Poll for sessions, count "+count+", remaining "+remainingTime);
