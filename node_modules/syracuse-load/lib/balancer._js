...
var EngineioClient = require('socket.io/node_modules/socket.io-client/node_modules/engine.io-client');
...
var CHILD_PING2_LIMIT = 10000; // default value for child status pinging time so that when exceeded, the child will not be balanced unless 1 remains (millis)
var CHILD_PING2_TIMEOUT = 30000; // default value for timeout for child status pinging (millis)
...
function findClient(request) {
	...
	var client = children[hostport[1].substr(1)].mockClient;
...
function startServer(_) {
	...
	for (i = 0; i < localHost.connectionData.length; i++) {
		...
		if (!httpServers[i] && connectionData.active) {
			...
			httpServers[i].timeout = Math.max(120000, 1000 * (localHost && localHost.returnRequestTimeout || 0) + 5000);
			tracer && tracer("Server timeout", httpServers[i].timeout);
...
function start(conf, _) {
	...
	CHILD_PING2_LIMIT = config.nanny.childPingStatusLimit || CHILD_PING2_LIMIT;
	...
	try {
		...
		switch (process.argv[2]) {
			...
			case "stop":
				...
				process.exit(0); // some blocking process is still running - therefore process.exit();
				return 0;
			case "patch":
				var action = "batch";
...
function _getBalanceData(_, hostHeader) {
	if (config.hosting.multiTenant) {
		if (hostHeader) {
			...
			var now = Date.now();
			if (!tenantDbObject || tenantId !== tenantDbObject.tenantId) {
				var threshold = now - (30 + (config.session.timeout || 20)) * 60000; // keep old sessions a bit longer
				var i = tenantDbCollection.length;
				tenantDbObject = null;
				while (--i >= 0) {
					var checkDb = tenantDbCollection[i];
					// updates last used timestamp of collection if tenantID is part of the collection
					if (checkDb.tenantId == tenantId) {
						tenantDbObject = checkDb; // tenant database found
					} else
					// checks if timestaps are older than timeout specified in the session timeout. if older remove them from colection
					if (checkDb.timeStamp < threshold) {
						dbDriver.close(checkDb.tenantDb);
						// removes expired connection from tenantDbCollection
						tenantDbCollection.splice(i, 1);
					}
				}
				// db object is not found in tenantDbCollection
				if (!tenantDbObject) {
					tenantDbObject = {
						tenantId: tenantId,
					...
					tenantDbCollection.push(tenantDbObject);
				}
			}
			tenantDbObject.timeStamp = now;
			var shiftValue = Math.abs(_hashCode(tenantDbObject.tenantId));
			var sc = dbDriver.createCollection(tenantDbObject.tenantDb, 'SessionInfo', _);
			if (!sc) {
			...
			throw new Error("No host header in multitenant mode");
		}
	} else {
		if (!sessionCollection) sessionCollection = dbDriver.createCollection(db, 'SessionInfo', _);
...
WritableIOStream.prototype.write = function(data, enc) {
	...
	if (this.writable) {
		...
		console.log("Write data after end " + util.format(data));
	return true;
};
	...
	if (localHost.status > STATUS_INIT || force) {
		startChildrenFunnel(_, function(_) {
			...
			if (localHost.status === STATUS_START) {
				...
				var futures = [];
				children.forEach(function(child) {
					futures.push(_childping(!_, child));
				});
				wsChildren.forEach(function(child) {
...
// do the ping2 function for second mock client when it fails for first mock client
function _ping2(_, child, id) {
	try {
		return child.mockClient2.ping2(_, CHILD_PING2_TIMEOUT);
	} catch (e) {
		console.log("Error in ping2 for " + id + ", try again: " + e);
		return child.mockClient.ping2(_, CHILD_PING2_TIMEOUT);
	}
}
...
function healthLoop(_) {
	...
	for (var i = 0; i < children.length; i++) {
		if (children[i] && children[i].syratime != undefined) {
			var future = _ping2(!_, children[i], "N" + i);
			future.extra = ["N" + i, children[i]];
			futures.push(future);
		}
	}
	restrictedServers.forEach(function(server) {
		var future = _ping2(!_, server, "restricted");
		future.extra = [undefined, server];
		futures.push(future);
	});
	...
	futures.forEach_(_, function(_, future) {
		try {
			var result = future(_);
			var tmp = "" + future.extra[1].syratime;
			var index = tmp.indexOf("|");
			var syratime = tmp.substr(0, index);
			var syraload = tmp.substr(index + 1);
			future.extra[1].syratime = ("" + (+result[0] + (+syratime - result[0]) * CHILD_SLIDING_MEAN)).substr(0, 7) + "|" + result[1].substr(2);
			tracer && tracer("Load computation " + result[0] + " " + result[1] + " " + future.extra[1].syratime);
			if (result[1].substr(0, 1) !== "0" && future.extra[0]) // only non-restricted servers
	...
	setTimeout(function() {
		return healthLoop(_ >> function(err) {
			if (err) throw err;
		});
	}, CHILD_PING2_POLLING);
}
...
function createProcess(code, port, extraArgument) {
	...
	if (!child || !child.terminate) { // restart processes (unless terminating all processes is intended)
		...
		child = child_process.spawn(process.argv[0], cmdline, {
			stdio: ['pipe', process.stdout, process.stderr, 'pipe', 'pipe', 'pipe']
		});
		...
		// special client for pingStatus requests so that they are not disturbed by communication on other streams
		child.mockClient2 = new mock.Mock(child.stdio[4], child.stdio[5], function(request, response, _) {
			request.fromNanny = true; // mark these requests as internal requests
			return requestListener(request, response, _);
		});
...
function updateHostData(startChildren, _, init) {
	...
	if (localHost.returnRequestTimeout != null) {
		children.forEach(function(child) {
			if (child) {
				child.mockClient.setTimeout(localHost.returnRequestTimeout);
			}
		});
		// it does not make sense to change the http server timeout, because it seems to be effective only after restarting nanny.
	}
...
function requestListener0(request, response, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
		...
		switch (command[0]) {
			...
			case 'info':
				...
				for (i = 0; i < children.length; i++) {
					var child = children[i];
					if (child) childResults.push("N" + i + ": " + child.mockClient.numberRequests() + " " + (child.syralast ? Math.max(0, child.syralast - now) : "-") + ";" + child.syratime);
				}
				for (i = 0; i < wsChildren.length; i++) {
	...
	if (request.url.substring(0, 6) === "/soap-" && request.method === "POST") {
		...
		if (config.hosting.multiTenant) {
			var tenantId = request.headers.host;
...
function _balanceInternal(_, local) {
	...
	function _balanceHost(array, host) {
		...
		while (--i >= 0) {
			...
			var index = child.syratime.indexOf("|");
			var syratime = +child.syratime.substr(0, index);
			var syraload = +child.syratime.substr(index + 1);
			...
			// consider child with minimal load value. Consider child processes with long response time
			// (longer than CHILD_PING2_LIMIT) only when there is nothing else, then take child process
			// with shortest response time (above CHILD_PING2_LIMIT), because there must be one child
			// process chosen
			if (syraload <= minimalload && (syratime < CHILD_PING2_LIMIT || syratime < minimaltime)) {
				minimaltime = syratime;
				minimalload = syraload
				goodhost = host;
				goodport = i;
				goodchild = child;
			}
	...
	var minimaltime = 1000000000000000;
	var minimalload = 1000000000000000;
...
var doRequest = _(function(streamRecorder, response, hostPort, callback, sessionId, cookieValue, client) {
	...
	if (hostPort[0].hostname === localHost.hostname) {
		...
		if (!client) client = (hostPort[1][0] === "W" ? wsChildren : children)[hostPort[1].substr(1)].mockClient;
	} else {
		if (/^[NW]/.test(port)) {
	...
	var req = client.request(options, function(res) {
		...
		if (res.statusCode === 500 && res.headers[mock.RESTRICT_HEADER] === 'on' && hostPort[0].hostname === localHost.hostname) {
			...
			_startMessage("Ask to restrict (status code:500) " + hostPort[1]);
	...
	req.on("error", function(err) {
		...
		var foundHost = hostsByName[hostPort[0].hostname];
		console.log("MARK " + util.format(foundHost) + " hostport " + util.format(hostPort) + " Error " + err + " Options " + util.format(options) + " Response " + util.format(ress));
...
function _restrict(pseudoport, kill) {
	...
	_startMessage("Restricted process " + pseudoport + " " + kill);
