...
function startSignal(host, _, first) {
	...
	if (first) {
		if (ownCertificate) {
			...
			if (index < 0) localHost.untrusted.push(host.hostname); // mark host as untrusted first
		} else {
...
function requestListener0(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {		
		...
		case 'updateCertificate': // send/receive certificate information
			...
			if (!ownCertificate) {
				writeError(locale.format(module, "noBaseCert"));
				return;
			}				
			...
			if (d) {
				var changes = {};
				try {
					var answer = certTools.parseRequestCert(_, JSON.parse(d), certificates, caCertificates, certDirectory, localHost, changes);					
				} catch (e) {
			...
			try {
				...
				if (foundHost.status >= STATUS_INIT && hostData.extra) { // first invocation: important for security information
					...
					if (security && foundHost) {
						var certs = [];
						var index = localHost.untrusted.indexOf(foundHost.hostname);
						if (index >= 0) {
							localHost.untrusted.splice(index, 1); // remove host from list of untrusted hosts
						...
						var changes = {};
						var certs = certTools.parseRequestCert(_, parsed, certificates, caCertificates, certDirectory, localHost, changes);
						if (changes.restart) { // restart server
							console.log("Connection data changed - restart server");
							setTimeout(function() { process.exit(10); }, 1000);									
						} else {
						...
						// add host to list of untrusted hosts
						if (foundHost) {
							var index = localHost.untrusted.indexOf(foundHost.hostname);
							if (index < 0) {
								localHost.untrusted.push(foundHost.hostname);
								untrustedChange = true;
							}
						}
					}
...
function _securePack(data, host, auth) {
	try {
		...
		return {data: encrypted, iv: iv.toString("base64"), sender: host ? localHost.hostname : undefined, auth: auth, sign: signature};
	} catch (e) {
...
function _secureUnpack(parsed, _) {
	...
	if (parsed.sender) {
		...
		key = host.dhKey;
		// verify signature
		if (!_verifyText(parsed.data, host.certificate, parsed.sign))
			throw new Error(locale.format(module, "wrongSign"));
	} else {
		key = caHash;
	}
