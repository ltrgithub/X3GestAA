	...
	for (i = 0; i < localHost.connectionData.length; i++) {
		...
		if (!httpServers[i] && connectionData.active) {
			var listener = function(request, response) {
				return requestListener(request, response, sessionCollection, function(error, data) {
					if (error) {
...
function _fillData(certInstance, certObject) {
	...
	if (_deepEqual(certInstance.issuer, value)) {
		certInstance.issuer = value;
		changed = true;
	}
	value = certObject.subject;
	if (_deepEqual(certInstance.subject, value)) {
	...
	caCertificates.forEach_(_, function(_, certificate) {
		...
		if (certificate.internal) {
			...
			if (_fillData(certificate, checked.cert)) {
				dbDriver.update(caCertificateCollection, {name: "ca"}, {issuer: checked.cert.issuer, subject: checked.cert.subject, notBefore: certificate.notBefore, notAfter: certificate.notAfter }, _);
			}
	...
	if (ownCaCertificate) {
		...
		for (i = certificates.length-1; i >= 0; i--) {
			...
			if (certificate.internal) {
				...
				if (_fillData(certificate, checked.cert)) {
					dbDriver.update(certificateCollection, {name: lcHostName}, {issuer: checked.cert.issuer, subject: checked.cert.subject, notBefore: certificate.notBefore, notAfter: certificate.notAfter }, _);
				}
			}
		}
	}
...
function requestListener0(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {		
		...
		case 'updateCertificate': // send/receive certificate information
			console.log("UPDATE CERT"+streamRecorder)
			...
			try {
				...
				if (foundHost.status >= STATUS_INIT && hostData.extra) { // first invocation: important for security information
					...
					if (security && foundHost) {
						...
						if (foundHost && _markTrusted(foundHost, false))
								untrustedChange = true;
					}
	...
	console.log("URLLLLL"+util.format(request.headers));
...
function doRequest(streamRecorder, response, hostPort, callback, sessionId) {
	...
	var req = client.request(options, function(res) {
		...
		if (res.statusCode === 303) {
			...
			var date = _clearRedirects();
			if (redirectUrl !== "/") {
				redirects[redirectUrl] = [hostPort, date];
				console.log("ADD REDIRECT FOR "+redirectUrl+" "+sessionId);				
			}
		}
