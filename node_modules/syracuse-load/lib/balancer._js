...
function _stringifyHosts(h) {
	return JSON.stringify(h.map(function(host) { return _minifyHost(host)}));
}

function _minifyHost(h, extra) {
	var result = { hostname: h.hostname, extra: extra};
...
function _fillData(certInstance, certObject) {
	var changed = false;
	var value = certObject.issuerDn;
	if (certInstance.issuerDn !== value) {
		certInstance.issuerDn = value;
		changed = true;
	}
	value = certObject.subjectDn;
	if (certInstance.subjectDn !== value) {
		certInstance.subjectDn = value;
		changed = true;
	}
...
function start(conf, _) {
	...
	if (process.argv[2] === "adjust") {
		...
		try {
			dbDriver.updateAll(hostsCollection, {}, {patchStatus: "0"}, _);			
			return 0;
		} finally {
			dbDriver.close(db);
		}
	...
	if (process.argv[2] === "install") {
		try {
			...
			if (!/^\d+$/.test(process.argv[3]))
				throw new Error(locale.format(module, "invalidPort", process.argv[3]));
			if (!/^\d\d?$/.test(process.argv[4]))
				throw new Error(locale.format(module, "invalidChildCount", process.argv[4]));
			...
			if (localHosts.length > 0) {
				...
				// new host
				var newHost = {_id: generateUUID(), hostname: config.servername, 
	...
	case undefined:
		...
		var update = { started: true, pid: process.pid};
		if (tcpName) update.tcpHostName = tcpName;
		dbDriver.update(hostsCollection, {hostname: config.servername}, update, _);
		startServer(_);
	...
	case "passphrase":
		...
		try {
			...
			try {
				var result = get(localHost, "PUT", "/nannyCommand/reloadCertificates", null, _);
			} catch (e) { console.log("Could not contact nanny "+e);
			}
			return 0;
		} catch (e) {
	...
	case "remove":
		try {
			...
			dbDriver.remove(certificateCollection, { name: config.servername.toLowerCase()}, _);			
		} finally {
			dbDriver.close(db);			
		}		
...
function _loadCertificates(_) {
	...
	if (!caCertificates.some(function(certificate) { 
		...
		var newCertificate = {_id: generateUUID(), name: "ca", internal: true, _creUser: "setup", _updUser: "setup", _updDate: new Date() };
	...
	caCertificates.forEach_(_, function(_, certificate) {
		...
		try {
			checked = checkFileCertificates(_, certificate, true);
		} catch (e) {
		...
		if (certificate.internal) {
			...
			if (_fillData(certificate, checked.cert)) {
				dbDriver.update(caCertificateCollection, {name: "ca"}, {issuerDn: certificate.issuerDn, subjectDn: certificate.subjectDn, notBefore: certificate.notBefore, notAfter: certificate.notAfter }, _);
			}
			// console.log("JJJJ"+util.format(certificate))
			var hash = crypto.createHash("SHA256");
			hash.update(checked.cert.publicKey);
			caHash = new Buffer(hash.digest("binary").substr(0, 16), "binary");
		}
	});
	if (ownCaCertificate) {
		...
		if (!certificates.some(function(certificate) {
			...
			var newCertificate = {_id: generateUUID(), name: lcHostName, internal: true, keyExists: true, caCertificates: [dbDriver.setRelatedInstance(ownCaCertificate)],
				_creUser: "setup", _updUser: "setup", _updDate: new Date(), server: dbDriver.setRelatedInstance(localHost)};
		...
		for (var i = certificates.length-1; i >= 0; i--) {
			...
			if (certificate.internal && (!cas || !cas.length)) {
				...
				dbDriver.update(certificateCollection, {name: certificate.name}, {caCertificates: cas}, _);
			}
			if (cas) {
				for (var j = cas.length-1; j >= 0; j--) {
					cas[j] = dbDriver.findInstance(cas[j], caCertificates);
				}
			}
			try {
				checked = checkFileCertificates(_, certificate, false, passphrases, null, cas);
			} catch (e) {
			...
			if (certificate.internal) {
				...
				if (_fillData(certificate, checked.cert)) {
					dbDriver.update(certificateCollection, {name: lcHostName}, {issuerDn: certificate.issuerDn, subjectDn: certificate.subjectDn, notBefore: certificate.notBefore, notAfter: certificate.notAfter }, _);
				}
			}
		};
	};
...
function startSignal(host, _, first) {
	...
	if (first) {
		if (ownCertificate) {
			...
			var signature = _signText(ownChallenge, _);
			extra = { time: time, sign: signature, challenge: challenge, cert: ownCertificate.certificate,
	...
	// console.log("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEXTRA "+util.format(extra))
	...
	if (certificates) {
		hosts.forEach(function(host) {
			host.connectionData.forEach(function(conn) {
				if (host.hostname === localHost.hostname) { // add only server certificates of localhost
...
function updateHostData(startChildren, _, init) {
	...
	if (!init) addCertificateData(newHosts);
...
function requestListener(request, response, sessionCollection, _) {
	try {
		return requestListener0(request, response, sessionCollection, _);
	} catch (e) {
		writeError(response, (e instanceof Error ? e.message : ""+e));
	}
}

function requestListener0(request, response, sessionCollection, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {		
		...
		case 'children': // ping to children
			...
			break;
		case 'reloadCertificates': // reload local certificate data
			_loadCertificates(_);
			firstStartSignal(_);
			break;			
		case 'updateCertificate': // send/receive certificate information
			...
			// console.log("DDAATTAA "+util.format(d));
			if (destination) {
				if (destination === "*") { // send request to all servers
					delete request.headers[mock.BALANCER_HEADER];
					// localHost.dhKey = new Buffer(32);
					...
					hosts.forEach(function(host) {
						if (!host.local && host.status >= STATUS_READY && host.dhKey) {
							...
							futures.push(secureRequest(host, "POST", "/nannyCommand/updateCertificate", d, null, undefined));
						}
					});
				} else {
					var foundHost = hostsByName[destination];
					if (foundHost) {
						if (!foundHost.local) {
							if (foundHost.status >= STATUS_READY)
								
								secureRequest(foundHost, "POST", "/nannyCommand/updateCertificate", d, null, _);
							else {
				...
				var unpacked = _secureUnpack(d, _);
				d = unpacked[0];
				sender = hostsByName[unpacked[1]];
			}
			if (d) {
				try {
					var answer = certTools.parseRequestCert(_, JSON.parse(d), certificates, caCertificates, certDirectory, localHost);					
				} catch (e) {
				...
				if (!destination) {
					response.end(_securePack(answerJson, sender));
				}
			}
			break;
		...
		case 'started': // nanny has started
			try {
				...
				if (foundHost.status !== STATUS_WRONG_VERSION && hostData.extra) { // first invocation: important for security information
					var security = _checkFirstData(null, hostData.extra, foundHost, _);
					if (security && foundHost) {
						// !!!!! Zertifikate abgleichen
						var certs = [];
						console.log("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
						certTools.pushMissing(foundHost, certs);
						console.log("Missing foreign certificates "+util.format(certs));
						certs = certTools.parseRequestCert(_, certs, certificates, caCertificates, certDirectory, foundHost);
						console.log("Answer "+util.format(certs));
						// add missing own certificates
						certTools.pushMissing(localHost, certs);
						console.log("Answer2 "+util.format(certs));

						var data = secureRequest(foundHost, "POST", "/nannyCommand/updateCertificate", JSON.stringify(certs), security, _);
						console.log("Answer3 "+util.format(data));
						var parsed = JSON.parse(data);
						var certs = certTools.parseRequestCert(_, parsed, certificates, caCertificates, certDirectory, localHost);
					}
				}
			} catch (e) {
	...
	if (localHost.status < STATUS_READY) {
		...
		case STATUS_START: temporary = 503; text = locale.format(module, "beingStarted"); break;
		case STATUS_INIT: temporary = 503; text = locale.format(module, "notYetStarted"); break;
...
function _checkFirstData(ownChallenge, data, host, _) {
	try {
		...
		if (!ownChallenge) {
			...
			var challenge = localHost.hostname+" "+data.time;			
		} else {
			var challenge = ownChallenge;
		}
		// verify the foreign challenge
		tracer && tracer("Verify challenge");
		if (!_verifyText(challenge, data, data.sign, _)) return null;
		...
		if (data.challenge) {
			...
			// sign the challenge
			signature = _signText(data.challenge, _);
		}
		return { sign: signature, cert: ownCertificate.certificate, dh: diffieHellman.getPublicKey("base64") };

	} catch (e) {
		console.log("Exception in check data: "+e);
		return null;
	}
}

//sign text with own private key. Output: base64
function _signText(text, _) {
	return certTools.sign(_, ownCertificate, "RSA-SHA1", text, { data_encoding: "utf8", output_encoding: "base64"});
}

//verify text with given certificate. Signature must be in base64
function _verifyText(text, cert, sign, _) {
	return certTools.verify(_, cert, "RSA-SHA1", text, sign, {data_encoding: "utf8", signature_encoding: "base64"});
}
...
/// * cas: object with CA certificates
///
/// Result: object with attributes
/// * key: decrypted private key
/// * cert: certificate object
/// * certificate: certificate text (certificate file content) in PEM format
function checkFileCertificates(_, certificate, ca, passphrases, newPassphrase) {
	if (!certificate) {
		certificate = { name: (ca ? "ca" : lcHostName), internal: true }
	}
	var filename = certDirectory+certificate.name;
	// read certificate from file
	try {
		var cert = fs.readFile(filename+(ca ? ".cacrt" : ".crt"), "utf8", _);		
	} catch (e) {
		console.error("Cannot read certificate file "+e);
		throw new Error(locale.format(module, "noCertFile", certificate.name));
	}
	...
	if (!ca && (certificate.keyExists || certificate.internal)) {
		try {
			key = fs.readFile(filename+".key", "utf8", _);
		} catch (e) {
			console.error("Cannot read certificate key "+e);
			throw new Error(locale.format(module, "noKeyFile", certificate.name));			
		}
	};
	// prepare CA certificates
	var cas;
	if (certificate.caCertificates) {
		cas = certificate.caCertificates.map(function(cert) { return cert.certificate});
	}
	// integrity check
...
function _securePack(data, host, auth) {
	try {
		...
		var cipher = crypto.createCipheriv("AES256", host.dhKey, iv);
		// console.log("EN "+data)
		var encrypted = cipher.update(data, "utf8", "base64");
		encrypted += cipher.final("base64");
	} catch (e) {
		console.log("Error in _securePack "+e)
	}
	return JSON.stringify({data: encrypted, iv: iv.toString("base64"), sender: localHost.hostname, auth: auth});
}

//decrypts the data using the common Diffie-Hellman key
function _secureUnpack(data, _) {
	if (!ownCertificate) throw new Error(locale.format(module, "noCertificate"))
	// console.log("Secure unpack:"+data)
	var parsed = JSON.parse(data);
	...
	var decipher = crypto.createDecipheriv("AES256", host.dhKey, iv);
	var decrypted = decipher.update(parsed.data, "base64", "utf8");
...
function secureRequest(hostdata, method, path, data, auth, _) {
	...
	var result = get(hostdata, method, path, encryptedData, _);
	tracer && tracer("Result "+util.format(result))
	return _secureUnpack(result, _)[0];
}
...
function writeError(response, reason, temporary) {
	...
	response.writeHead(temporary ? temporary : 500, "Error", { "Content-Type": "text/plain" });
	response.end(reason);
}
