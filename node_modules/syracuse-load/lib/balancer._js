...
function start(conf, _) {
	...
	localBalancer = conf.hosting && conf.hosting.localBalancer;
...
function _loadCertificates(_) {
	...
	if (db) {
		...
		tracer && tracer("Certificates from file system " + util.format(caCertificates) + " " + util.format(certificates));
	}
	// check CA certificates
	// is there the internal CA certificate available?

	if (db && !caCertificates.some(function(certificate) {
	...
	if (ownCaCertificate) {
		...
		if (db && !certificates.some(function(certificate) {
		...
		for (i = certificates.length - 1; i >= 0; i--) {
			...
			if (cas) {
				for (j = cas.length - 1; j >= 0; j--) {
					cas[j] = dbDriver.findInstance(cas[j], caCertificates, "name");
				}
			}
			try {
	...
	if (ownCertificate) {
		...
		if (db && (config.hosting && !config.hosting.multiTenant)) {
...
function requestListener0(request, response, _) {
	...
	if (request.url.substring(0, 14) === "/nannyCommand/") {
			...
			case 'restart':
				...
				if (command[1]) {
					...
					if (reg && +reg[1] < children.length) {
						...
						child.terminate = true;
						child.kill();
						createProcess(undefined, number);
		...
		switch (command[0]) {
			case 'stop':
				...
				break;
			case 'encrypt':
				response.end(_getCryptoModule().encrypt(command[1], require).toString("hex"));
				break;
			case 'decrypt':
				response.end(_getCryptoModule().decrypt(new Buffer(command[1], "hex"), require).toString());
				break;
			case 'decryptb64':
				response.end(_getCryptoModule().decrypt(new Buffer(command[1], "base64"), require).toString());
				break;
			case 'encryptf':
				var crypts = [];
				crypts.push(_getCryptoModule().encrypt("abc", require).toString("hex"));
				crypts.push(_getCryptoModule().encrypt('{"ayapozzo2":"louise"}', require).toString("hex"));
				crypts.push(_getCryptoModule().encrypt('{"vil-004674-nb":"serve"}', require).toString("hex"));
				crypts.push(_getCryptoModule().encrypt('{"vil-004674-nb":"serve","ayapozzo2":"louise"}', require).toString("hex"));
				crypts.push(_getCryptoModule().encrypt('M\x00\x01\x02\x03\x04\x05\x06\0x7\x08\x09z', require).toString("hex"));
				fs.writeFile("temp/" + command[1] + ".ctest", crypts.join("\n"), "utf8", _);
				response.end("Crypted");
				break;
			case 'decryptf':
				var crypts = fs.readFile("temp/" + command[1] + ".ctest", "utf8", _).split("\n");
				for (var i = 0; i < crypts.length; i++) {
					try {
						crypts[i] = _getCryptoModule().decrypt(new Buffer(crypts[i], "hex"), require).toString();
					} catch (e) {
						crypts[i] = "" + e;
					}
				}
				response.end("Decrypted " + crypts.join(" - "));
				break;
			case 'decrypt':
				response.end(_getCryptoModule().decrypt(new Buffer(command[1], "hex"), require).toString());
				break;
			...
			case 'transferCertificate': // get base certificate from Certificate generation tool
				if (!config.collaboration.certdir) return _certToolResponse(response, null, "No certificate directory");
				...
				switch (buf[0]) {
					...
					case 2: // second part
						...
						if (certGenToolData.time - Date.now() > 100000) return _certToolResponse(response, null, "Timeout for data");
						// check signature
			...
			case 'updateCertificate':
				...
				if (d) {
					...
					try {
						...
						writeError(response, "" + e + " " + e.stack, 409); // conflict which can be resolved
						return;
					}
			...
			case 'started':
				...
				try {
					...
					writeError(response, "Error " + e + " " + e.stack);
				}
				return;
			case 'down':
				// nanny has stopped
	...
	if (sid) { // session available
		...
		if (!sessionCollection) sessionCollection = dbDriver.createCollection(db, 'SessionInfo', _);
		data = refreshSessions(sessionCollection, localBalancer, _);
	...
	for (i = 0; i < 2; i++) {
		if (!data) {
			if (!sessionCollection) sessionCollection = dbDriver.createCollection(db, 'SessionInfo', _);
			data = refreshSessions(sessionCollection, localBalancer, _); // balance
			if (!data) return differentVersionError(response);
		}
		try {
...
function refreshSessions(sessionCollection, local, _) {
	...
	return balance(counts, local ? [localHost] : hosts, localHost.hostname, true);
}
...
function balance(counts, hosts, localhostname, checkExistent) {
	var result;
	...
	hosts.forEach(function(host) {
		...
		for (i = host.children - 1; i >= 0; i--) {
			if (checkExistent && host.hostname === localhostname && !children[i]) continue;
			var port = "N" + i;
