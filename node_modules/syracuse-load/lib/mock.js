...
function Mock(outputStream, inputStream, requestHandler) {
	...
	tracer && tracer("MOCKMOCK "+outputStream)
	var self = this;
...
function ReadableMockStream(options, number, mock, resp) {
	...
	tracer && tracer("IIIIIIIIIIIII "+util.format(options))
	exports.extractDataFromRequest(options, self);
	self.headers = self.headers || {} 
	tracer && tracer("$$$$$$ "+util.format(self))
	tracer && tracer("<<<<<< "+self.statusCode)
...
function _frameHeader(type, number, length) {
	tracer && tracer("Header typ "+type+" nr. "+number+" laenge "+length)
...
function simpleRequest(client, options, content, callback) {
	...
	var req = client.request(options, function(res) {
		...
		res.on("data", function(chunk) {
			console.log("SQ CHUNK "+util.format(chunk))
			result += chunk;
		});
		res.on("end", function(chunk) {
			console.log("SIMPLE REQ result "+result)
