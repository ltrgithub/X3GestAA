...
function WritableMockStream(options, number, mock, resp) {
	...
	self.write = function(data, enc, end) {
		// console.log(_number+"---------- WRITE "+util.format(data)+" "+enc+" "+end+"--"+_used)
		...
		if (!data) data = new Buffer(0);		
		// console.log(_number+"DATA ????? "+util.format(data))
		...
		var list = [];
		if (_options) {
			// console.log(_number+"Send frame header for "+_number)
	...
	self.end = function(data, enc) {
		// console.log(_number+"ENDDATA --- <"+data+">")
		if (_finished) {
	...
	self._markFree = function() {
		...
		_used = false;
		// console.log(_number+"USED "+_used+" fin "+_finished)
