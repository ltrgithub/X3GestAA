...
exports.EclipseClientMock = helpers.defineClass(EclipseClientMock, null, {
	register: function(_) {
		...
		if (this.cookie) {
			...
			headers["Authorization"] = "Basic ZXJib3U6ZXJib3U=";
		}
	...
	submitCommand: function(_, cmd, args) {
		var data, response,
			headers = {
				"Accept": "application/json",
				"cookie": this.cookie
			...
			options = {
				method: "POST",
				headers: headers,
				url: this.baseurl + "/studio/$proxy/session"
			};
	...
	track: function(_, tracker) {
		...
		var response,
			...
			options = {
				method: "GET",
				headers: headers,
				url: this.baseurl + "/studio/$proxy/session?trackingId=" + tracker.$trackingId
			};
	...
	proxyinit: function(_) {
		var tracker = this.submitCommand(_, "proxyinit", ["-k", getToken(this.$uuid), "-m", "1"]);
