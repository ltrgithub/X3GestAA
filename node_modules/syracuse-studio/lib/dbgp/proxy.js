...
function startServer(cb, opt) {
	opt = opt || {};
	var dbgpPort = opt.port || 9514;
	tracer = tracer || (opt && opt.trace);
	tolerance = opt.tolerance;
	_setKey(opt.key);

	createLogger(opt.logDir || logDir, opt, function(err, res) {
		if (err)
			return cb(err);
		log = res;
		server = net.createServer(dbgpListener).listen(dbgpPort, function(err) { //'listening' listener
...
function processDebuggerInitMessage(matches, sender) {
	...
	if (ide) {
		bindSession(ide, sender, matches[0], msg.init || {});
	} else {
		sender.end();
	}
}
...
var _proxyCommand = {
				...
				item.sessions = Object.keys(e.sessions).map(function(k) {
					...
					return {
						$: {
							...
							thread: e.context && e.context.thread,
							appid: e.context && e.context.appid
						}
					};
				});
				sessions.push(item);
	...
	kill: function(options, sender) {
		// proxykill -k ide_key -u uuid
		var uuid = options["-u"],
			socket = _connections[uuid];

		log && log("[PXY] KILL sender.context=" + JSON.stringify(sender.context));
		if (sender.context && socket) {
			sendProxyResponse(sender, "kill");
			socket.end();
		}
		sender.end();
	},
	shutdown: function(options, sender) {
		// proxyshutdown -k ide_key -t now
		var delay = options["-t"];

		log && log("[PXY] SHUTDOWN delay=" + delay + "sender.context=" + JSON.stringify(sender.context));
		if (sender.context && delay) {
			delay = ~~delay || 10;
			sendProxyResponse(sender, "shutdown");
			setTimeout(function() {
				process.exit(0);
			}, delay);
		}
...
// Create and initialize an IDE session
// - An IDE session is bound to an Eclipse instance identified by the token.key
// - An IDE session maintain a list of debug sessions
...
function bindSession(ide, x3, data, init) {
	...
	x3.context.thread = initAttr.thread;
	x3.context.appid = initAttr.appid;
	log && log("[PXY] Binding debug session: idekey=" + ide.token.key + ", x3-pid=" + initAttr.thread + ", appid=" + initAttr.appid + //
		", ide=" + session.context.uuid + "[" + session.remoteAddress + ":" + session.remotePort + "]" + //
		", x3=" + x3.context.uuid + "[" + x3.remoteAddress + ":" + x3.remotePort + "]");
...
function sendProxyResponse(socket, type, diagnosis, data) {
	...
	resp += ' uuid="' + (socket.context && socket.context.uuid || "") + '"';
