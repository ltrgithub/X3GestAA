...
function parseRequest(message, sender) {
	try {
		...
		log("[PXY] FATAL ERROR during request parsing: " + e.message);
		sender.end();
	}
}
...
function consumeProxyMessage(message, sender) {
	...
	try {
		...
		log("[PXY] ERROR Failed to process proxy command '" + m[0] + "': " + e.message);
...
function bindSession(ide, x3, data) {
	...
	if (!session) {
		log("[PXY] No available ide session for idekey " + ide.token.key);
		x3.end();
	}
	...
	log("[PXY] Binding ide session for idekey " + ide.token.key + ": ide=" + session.context.uuid + ", x3=" + x3.context.uuid);
