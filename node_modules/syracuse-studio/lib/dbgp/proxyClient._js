...
exports.create = function _create(port, host, key) {
	tracer.debug && tracer.debug("DbgpClient.create: key=" + this.key + ", port=" + port + ", host" + host);
	return new DbgpProxyClient(port, host, key);
};
...
function DbgpProxyClient(port, hostname, key) {
	this.key = key;
	...
	this.close = function(_) {
		...
		tracer.debug && tracer.debug("DbgpClient.close: key=" + this.key + ", peer=" + stream.localAddress + ":" + stream.localPort);
		stream.close(_);
		stream = null;
	};
	...
	this._write = function(_, data) {
		_ensureConnect(_);
		tracer.debug && tracer.debug("DbgpClient.write: key=" + this.key + ", data=" + data);
		stream.write(_, data);
	};

	this._read = function(_, trackingId) {
		tracer.debug && tracer.debug("DbgpClient.read before: key=" + this.key + ", trackingId=" + trackingId + "\n" + //
		...
		tracer.debug && tracer.debug("DbgpClient.read after: key=" + this.key + ", trackingId=" + trackingId + "\n" + //
			"\taddress=" + stream.localAddress + ":" + stream.localPort + "\n" + //
			"\tdata=" + msg);
		return msg;
	};
...
exports.DbgpProxyClient = helpers.defineClass(DbgpProxyClient, null, {
	...
	poll: function(_, trackingId, options) {
		try {
			...
			if (options) {
				...
				tracer.debug && tracer.debug("DbgpClient.poll: [async] trackingId=" + trackingId + ", key=" + this.key);
				tracker = _makeReadTracker(_, this, trackingId);
				return _asyncCall(tracker, options);
			}
			tracer.debug && tracer.debug("DbgpClient.poll: [sync] trackingId=" + trackingId + ", key=" + this.key);
	...
	submitCommand: function(_, cmd, args, options) {
		try {
			...
			cmd = cmd.split(" ").concat(args || []).join(" ").split(/\0/)[0] + "\0";
			this._write(_, cmd);
...
function _asyncCall(tracker, options) {
	...
	var callback = function(e, r) {
		tracer.debug && tracer.debug("DbgpClient._asyncCall [future] callback: trackerId=" + tracker.id + "\n  >>> e=" + e + ", r=" + r);
		tracker.future = null;
		tracker.error = e;
		tracker.done = true;
		tracker.result = r;
		...
		tracer.debug && tracer.debug("DbgpClient._asyncCall [future] onComplete: trackerId=" + tracker.id + ", onComplete=" + typeof(tracker.onComplete) + "\n  >>> " + tracker.result);
		return tracker.onComplete && tracker.onComplete(tracker);
	};
