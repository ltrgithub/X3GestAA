...
function _wrapOnComplete(f) {
	return function(tracker) {
		tracer && tracer("DbgpClient._wrapOnComplete: id=" + tracker.id + ", future=" + tracker.future + //
...
function _asyncCall(_, tracker, options) {
	...
	tracker.future(_ >> function(e, r) {
		...
		if (tracker.future) {
			...
			return ctx.onComplete(tracker);
		}
		tracer && tracer("DbgpClient._asyncCall [future] unexpected: " + tracker.id + "\n  >>> " + tracker.result);
		return ctx.onError(new Error("DbgpClient asyncCall already completed"), tracker);
	});
}