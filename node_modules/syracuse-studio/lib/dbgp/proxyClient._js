...
function DbgpProxyClient(port, hostname, key) {
	...
	this.close = function(_) {
		...
		tracer && tracer("DbgpClient.close: key=" + this.key + ", peer=" + stream.localAddress + ":" + stream.localPort);
		stream.close(_);
		stream = null;
	};
	...
	this._read = function(_, trackingId) {
		tracer && tracer("DbgpClient.read before: key=" + this.key + ", trackingId=" + trackingId + "\n" + //
		...
		tracer && tracer("DbgpClient.read after: key=" + this.key + ", trackingId=" + trackingId + "\n" + //
			"\taddress=" + stream.localAddress + ":" + stream.localPort + "\n" + //
			"\tdata=" + msg);
		return msg;
	};
...
function _asyncCall(tracker, options) {
	...
	tracer && tracer("DbgpClient._asyncCall: tracker=" + tracker.id);
	...
	tracker.future && tracker.future(callback);
}