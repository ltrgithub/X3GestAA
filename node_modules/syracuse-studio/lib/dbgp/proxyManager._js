...
function _getConfig(_, resolve) {
	var hostname = (dbgpConfig.host || "").replace(/^(localhost|127\.0\.0\.1|\s*)$/i, os.hostname()),
		port = dbgpConfig.port || defaultPort,
		ip;

	if (resolve)
		ip = dns.lookup(hostname, 4, ~_);
