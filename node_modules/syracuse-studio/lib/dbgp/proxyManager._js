...
exports.startDbgpProxy = function _startDbgpProxy(_) {
	if (studioConfig.disabled) {
		tracer && tracer("startDbgpProxy: studio disabled");
		return {
			info: {
				$: {
					pid: 0,
...
function _createClient(_, key, opt) {
	...
	opt = opt || {};
	return proxyClient.create(opt.port || config.port, opt.hostname || config.hostname, opt.token || getToken(uuid.generate()), key);
}
