...
exports.touchStudioSessionInfo = function(_, context, data) {
	...
	if (instance.wsid(_) != data.$wsid) {
		var debugProxy = "";
		if (exports.canDebug(_)) {
			debugProxy = dbgpHelper.getDbgpProxyAddress(_);
		}
		...
		instance.debugProxy(_, debugProxy);
		instance.save(_);
		instance.getAllDiagnoses(_, diagnoses);
	}
...
function _globalSession(op) {
	var session = globals.context.session;
	if (!(session && (!op || session[op]))) {
...
function _initUserContext(_) {
	var studioContext = _globalSession("getData").getData("studioContext");
	if (studioContext == null) {
		...
		_globalSession("setData").setData("studioContext", studioContext);
	}
}

function _getStudioUserContext() {
	return _globalSession("getData").getData("studioContext") || {};
}
...
exports.setStudioSession = function(context, ses) {
	_authorized();
	_globalSession("setData").setData("studioSession", ses);
};

function _getStudioSession() {
	return _globalSession("getData").getData("studioSession");
}
...
exports.getStudioSession = function() {
	_authorized();
	_globalSession().touch();
	return _getStudioSession();
};
