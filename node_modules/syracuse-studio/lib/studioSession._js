...
var cfgTracer = studioConfig.trace;
var studioSessions = {};
var tracer = require("syracuse-trace/lib/helper").getTracer("studio.session");
...
exports.createStudioSession = function(_, context, data) {
	...
	var id = data.$uuid,
		ses = studioSessions[id] || (studioSessions[id] = new StudioSession(data));
...
exports.touchStudioSessionInfo = function(_, context, data) {
	_authorized(context);
	...
	if (instance.wsid(_) != data.$wsid) {
		instance.login(_, httpSession.getUserLogin());
		...
		instance.debugProxy(_, dbgpHelper.getDbgpProxyAddress(_));
		instance.save(_);
		instance.getAllDiagnoses(_, diagnoses);
	}
	...
	if (errors.length > 0) {
		tracer.error && tracer.error("Failed to update studio session info: " + JSON.stringgify(errors));
	}
	return diagnoses;
};
...
exports.setStudioSession = function(context, ses) {
	...
	if (!(session && session.setData)) {
		tracer.error && tracer.error("setStudioSession: Bad session context");
		throw new Error("Bad session context");
	}

	session.setData("studioSession", ses);
};

function _getStudioSession(context) {
	var session = context.request && context.request.session;
	if (!(session && session.getData)) {
		tracer.error && tracer.error("getStudioSession: Bad session context");
		throw new Error("Bad session context");
	}
	return session.getData("studioSession");
}
...
exports.getStudioSession = function(context) {
	...
	return _getStudioSession(context);
};
...
function _authorized(context) {
	...
	var studioSession = _getStudioSession(reqContext);
	if (studioSession) {
...
function precond(ses) {
	...
	var studioSession = _getStudioSession(context);
