...
/// 
exports.createStudioSession = function(context, data) {
	_authorized();
	...
	var hits = ++ses.$hits;
	tracer && tracer("  created session $uuid=" + (ses && ses.$uuid));
...
function _authorized() {
	...
	var request = glob.context.request,
...
exports.StudioSession = helpers.defineClass(StudioSession, null, {
	...
	update: function(_, endpoint, data) {
		...
		tracer && tracer(">>> update: " + JSON.stringify(res));
		return res;
	},
	...
	deleteFile: function(_, endpoint, data) {
		...
		tracer && tracer("StudioSession.deleteFile: path=" + data.path + ", dir=" + dir);
	...
	writeFile: function(_, endpoint, data) {
		...
		tracer && tracer(">>> StudioSession.writeFile: path=" + data.path + ", dir=" + dir);
		// Only allow deletion of files into the tmp folder
		if (["tmp", "TRT"].indexOf(dir) === -1)
			throw new Error("Not supported");
	...
	getFileList: function(_, endpoint, data) {
		...
		tracer && tracer("StudioSession.getFileList: cmd=" + cmd);
...
function _compile(_, sadfs, folder, filename) {
	...
	tracer && tracer(">>> _compile: cmd='" + cmd + "'");
	sadfs.exec(_, cmd);
	var output = sadfs.readFile(_, filedesc, {
		encoding: "ascii"
	});
	tracer && tracer(">>> _compile: out content=" + output);
	sadfs.unlink(_, filedesc);
...
function _getFileDesc(path, folder) {
	...
	tracer && tracer("studioSession._getFileDesc: " + JSON.stringify(desc));
	return desc;
}