...
exports.createStudioSession = function(_, context, data) {
	...
	var instance = _getStudioSessionInfo(_, id),
		httpSession = context.httpSession,
		sessionInfo = httpSession.sessionInfo,
		req = httpSession.getData("requestInfo") || {};

	if (!instance) {
		instance = _createStudioSessionInfo(_, id);
	}
	if (hits === 1 || instance.wsid(_) != data.$wsid) {
		instance.login(_, sessionInfo.userName(_));
		instance.lastAccess(_, datetime.now());
		instance.sid(_, sessionInfo.sid(_));
		instance.clientId(_, sessionInfo.clientId(_));
		...
		instance.peerAddress(_, req.peerAddress);
