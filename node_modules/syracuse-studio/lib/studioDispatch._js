...
var _datasetMap = {
	...
	operation: function(_, context) {
		...
		var data = JSON.parse(context.request.readAll(_));
		// console.log("studio-operation: " + (data || {}).$operation);
	...
	invoke: function(_, context, id) {
		...
		// console.log("studio-invoke: " + context.dataset + (id ? ", " + id : ""));
		...
		// 	console.log("studio-invoke: -- connected" + context.dataset + (id ? ", " + id : ""));
...
function _dispatch(_, context, map) {
	...
	if (seg == null) {
		...
		throw context.badRequest("Unexpected segment: " + seg);
	}
}
