...
var tracer = require('syracuse-core/lib/trace').getTracer("studio.dispatch");
...
var _proxyServiceOperationMap = {
	...
	disconnect: {
		...
		post: function(_, context, id) {
			...
			if (sid == null)
				throw context.badRequest("sid parameter expected");
			_closeDbgpClient(_, context, "session", sid);
			return context.reply(_, 200, {
				$sid: sid
			});
		}
...
function _closeDbgpClient(_, context, type, sid) {
...
function _rawReply(context, statusCode, result, headers) {
	...
	function callback(e, r) {
		tracer.error && tracer.error("Cannot send response: Request response is closed - " + context.url + "?" + context.rawQuery);
	}
	if (context.response.closed) {
		var session = studio.getStudioSession(context);
		if (session && session.$uuid && sid != session.$uuid)
			_closeDbgpClient(callback, context, "session", session.$uuid);
		if (sid) {
			_closeDbgpClient(callback, context, "session", sid);
		}
		return;
	}
