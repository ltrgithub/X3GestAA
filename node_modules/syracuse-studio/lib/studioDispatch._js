...
var _studioMap = {
	...
	register: function(_, context, id) {
		...
		tracer && tracer("register: id=" + id + ', data=' + data);
		context.reply(_, 200, studio.createStudioSession(context, data.$uuid || id));
	},
...
var _datasetMap = {
	...
	operation: function(_, context) {
		...
		if (!ses || !ses.$uuid) {
			tracer && tracer("studio-operation failed: data=" + JSON.stringify(data) + ", studioSession=" + ses + ', $uuid=' + (ses && ses.$uuid));
...
var _proxyServiceOperationMap = {
	...
	connect: {
		...
		post: function(_, context, id) {
			tracer && tracer("studio-proxy-connect: " + context.url + "?" + context.rawQuery);
	...
	session: {
		...
		get: function(_, context, id) {
			tracer && tracer("studio-proxy-poll: " + context.url + "?" + context.rawQuery);
			var sid = context.parameters.sid;
			if (sid == null)
				throw context.badRequest("sid parameter expected");
			var dbgpClient = _getDbgpClient(_, context, "session", sid),
		...
		post: function(_, context, id) {
			tracer && tracer("studio-proxy-command: " + context.url + "?" + context.rawQuery);
			var sid = context.parameters.sid;
			if (sid == null)
				throw context.badRequest("sid parameter expected");
...
function _getDbgpClient(_, context, type, sid) {
	...
	tracer && tracer("_getDbgpClient: " + context.url + "?" + context.rawQuery + //
		"\nkey=" + type + "_" + sid +
...
function _closeDbgpClient(context, type, sid) {
	...
	tracer && tracer("_closeDbgpClient: " + context.url + "?" + context.rawQuery + //
		"\nkey=" + type + "_" + sid +
		"\nclients=" + Object.keys(clients)
	);
...
function _rawReply(context, statusCode, result, headers) {
	var sid = context.parameters.sid,
		clients = context.httpSession.dbgpClients || {};
...
function _trackerReply(tracker, context, statusCode, result, headers) {
	tracer && tracer("StudioDispatcher._trackerReply: id=" + tracker.id + ", hasFuture=" + !! tracker.future + //
...
exports.dispatcher = function(config) {
	return function(_, request, response) {
		var context = new Context(request, response, config),
			...
			studioSession = studio.getStudioSession(context);

		request.context = context;

		tracer && tracer("dispatcher: " + context.url + "?" + context.rawQuery + //
			"\n\tcookie=" + request.headers["cookie"] + //
			"\n\tstudioSession=" + studioSession + ", $uuid=" + (studioSession && studioSession.$uuid) + //
			"\nclients=" + Object.keys(context.httpSession.dbgpClients || {})
		);
