...
var _proxyServiceMap = {
	walk: function(_, context, name, id) {
		...
		var op = _proxyServiceOperationMap[name],
...
var _proxyServiceOperationMap = {
	...
	state: {
		...
		get: function(_, context, id) {
			...
			var dbgpClient = proxyManager.createClient(),
				proxyState = null,
				headers = {
					"content-type": "text/xml"
				};

			proxyState = dbgpClient.getState(_);
			dbgpClient.close();
	...
	session: {
		// GET http://host/studio/$proxy/session
		// GET http://host/studio/$proxy/session?trackingId=aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee
		// This url is used to poll and read dbgp packets sent by the X3 debugger through the dbgp proxy
		get: function(_, context, id) {
			var dbgpClient = _getDbgpClient(context),
				timeout = context.parameters.timeout || dbgpConfig.longPollingMillis || 30000;
		...
		post: function(_, context, id) {
			...
			var dbgpClient = _getDbgpClient(context),
				timeout = context.parameters.timeout || dbgpConfig.trackingMillis || 4000;
...
function _getDbgpClient(context) {
	return context.httpSession.dbgpClient || (context.httpSession.dbgpClient = proxyManager.createClient());
}
