...
*/
		...
		function addComponent(opts) {
			...
			.append('<img src ="/syracuse-uml/html/images/s-page-menus-open.png"/>') //
		...
		var $dialog = $('<div>').text("this is a dialog").appendTo($designer).hide();
		plumbInstance.bind("click", function(connection, ev) {
			var r = $designer[0].getBoundingClientRect();
			$dialog.css({
				position: 'absolute',
				top: ev.clientY - r.top + $designer[0].offsetTop,
				left: ev.clientX - r.left + $designer[0].offsetLeft,
			});
			$dialog.show();
		});
