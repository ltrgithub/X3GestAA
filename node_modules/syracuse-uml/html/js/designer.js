...
(function(exports) {
	exports.create = function(config) {
		...
		function initClassComponent(parent, data) {
			...
			$('<input>').attr('data-prop-name', 'name').addClass('edi-field').attr('type', 'text').val(data.name).appendTo($form);
			...
			function addListWidget(name) {
				...
				function addLine(text) {
					var $lineDiv = $('<div>');
					$('<input>').attr('data-prop-name', 'line').attr('type', 'text').val(text).appendTo($lineDiv);
		...
		var $leftMenuTitle = $('<div>').addClass('edi-menuitem').text('Tools').appendTo($leftMenu);
		$leftMenuTitle.css({
			width: "100%",
			left: "0%",
		...
		$('<div>').addClass('edi-menuitem').text('Add Class').appendTo($leftMenu).click(function() {
		...
		function load(graph) {
			try {
				deserialize(graph);
				message('uml diagram successfully loaded!');
			} catch (ex) {
		...
		function addComponent(opts) {
			...
			var $compoRemove = $('<div>').addClass('edi-remove').append('<img src ="/syracuse-uml/html/images/s-page-menus-open.png"/>').appendTo($compo).click(function(elt) {
		...
		function showTools() {
			$leftMenu[0].style.display = "block";
			$leftMenu[0].style.width = "20%";
			$workbench[0].style.left = "20%";
			$workbench[0].style.width = "80%";
		}

		function hideTools() {
			//$leftMenu[0].style.display = "none";
			$leftMenu[0].style.width = "0%";
			$workbench[0].style.left = "0%";
			$workbench[0].style.width = "100%";
		}
