...
exports.jsonExport = function (_, instance,filename) {
    ...
    fs.writeFile(fsp.join(__dirname, "../../../import", filename), JSON.stringify(exportObj, null, 2));
    ...
    flows.eachKey(_, meta.$relations, function (_, rel, relation) {
        ...
        if (relation.isPlural) {
            ...
            if (relation.isChild) {
                ...
                console.log("Non Child relation");
                if (keysel && keysel.length === 1) {
                    console.log("KEYSELECTION: " + keysel[0].name(_) + " : " + val[keysel[0].name(_)](_));
                    resource[rel] = val[keysel[0].name(_)](_);
                }
                else {
                    resource[rel] = [];
                    keysel.forEach_(_, function (_, el) {
                        resource[rel].push(val[el.name(_)](_));
                    });
                }
            }
        }
