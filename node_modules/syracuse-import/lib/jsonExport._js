						...
						if (!elt[keysel[0]](_)) //notValuedkey
							...
							if (prop.$isLocalized) {
								...
								tracer && tracer("localization["+l+ "]: "+sys.inspect(localization[l]));
									localization[l]={} ;
								if (!localization[l][locCode]) 
									localization[l][locCode] = elt[keysel[0]](_);
							} else 
								resource[rel].push(elt[keysel[0]](_));
							...
							if (elt[k](_)) {
								...
								if (prop && prop.$isLocalized) {
									...
									if (!localization[l])
										localization[l]={} ;
									if (!localization[l][locCode]) 
										localization[l][locCode] = elt[k](_);
								} else it[k] = elt[k](_);
							}
						});
						resource[rel].push(it);
				...
				if (!val[keysel[0]](_)) profileInst.$addDiagnose("error", locale.format(module, "notValuedkey", keysel[0]));
					...
					if (prop && prop.$isLocalized) {
						...
						if (!localization[l])
							localization[l]={} ;
						if (!localization[l][locCode]) 
							localization[l][locCode] = val[keysel[0]](_);
					} else resource[rel] = val[keysel[0]](_);
					tracer && tracer("resource[" + rel + "]=" + sys.inspect(resource[rel]));
				}
			} else {
					...
					if (val[k](_)) {
						...
						if (prop && prop.$isLocalized) {
							...
							if (!localization[l])
									localization[l]={} ;
							if (!localization[l][locCode]) 
								localization[l][locCode] = val[k](_);
						} else it[k] = val[k](_);
						error = false;
					}
				});
