...
exports.jsonExport = function (_, instance, options) {
    ...
    items.forEach_(_, function (_, item, idx) {
        ...
        var exportedObjs = item.exportedObjects(_).toArray(_);
            ...
            exportItem = objSerialize(_, inst, propSelection, allItems, model, model.singularize(item.className(_)),instance);
...
function refSerialize(_, targetEntity, data, propSelection, allItems, model, type) {
    var resource={} ;
    tracer && tracer("jsonExport: serializing reference ");
    if (!data) 
        tracer && tracer("data null") ;

// console.log("targetEntity: "+ sys.inspect(targetEntity)) ;
...
//console.log("propSelection: "+JSON.stringify(propSelection)) ;
    ...
    console.log("parcours des propriétés") ;
    ...
    console.log("PROP: "+prop) 
        ...
        if (property.$isLazy || property.isExternalStorage()) {
            ...
            console.log("prop value: "+sys.inspect(data[prop](_)));
        ...
        if (relation.isPlural) {
            ...
            if (relation.isChild) {
                ...
                collData.forEach_(_, function (_, elt) {
                    ...
                    //console.log("SERIALIZE REFERENCE "+ sys.inspect(elt)) ;
                    //console.log("propselNames pour "+rel +' '+ sys.inspect(propselNames)) ;
                    console.log("ligne 303: calling refSerialize") ;
            ...
            if (relation.isChild) {
                ...
                resource[rel] = refSerialize(_, val._relMeta.targetEntity, val, propselNames, allItems, model);
                console.log("RELATION " + rel + " is serialized" + JSON.stringify(serialized, null, 2));
    ...
    flows.eachKey(_, meta.$relations, function (_, rel, relation) {
        ...
        if (relation.isPlural) {
            ...
            if (relation.isChild) {
                console.log("Child relation");
                collData.forEach_(_, function (_, elt) {
                    console.log("ligne 451: calling refSerialize") ;
            ...
            if (relation.isChild) {
                ...
                //instance ou  val ?????????
                //console.log("INSTANCE-DATA : "+sys.inspect(instance));
                var serialized = refSerialize(_, relation.targetEntity, /*instance*/val, propselNames, allItems, model);
                //console.log("RELATION "+rel+" is serialized"+ JSON.stringify(serialized,null,2)) ;
                ...
                if (!val[keysel[0].name(_)](_) )
                    profileInst.$addDiagnose("warning", "Key missing" );/*locale.format(module, "referenceMissing", inst._meta.name, relMeta.name, 
