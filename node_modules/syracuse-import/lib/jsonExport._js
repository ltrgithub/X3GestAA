...
exports.profileToJsonContent = function(_, profileInst, options) {
	var opt = options || {};
	tracer = tracer || opt.tracer || (require('config').export || {}).tracer;
	var localis = _get_supported_locales(_);
	tracer && tracer("localis: " + sys.inspect(localis));
	...
	items.forEach_(_, function(_, item) {
		...
		tracer && tracer("item: " + JSON.stringify(item.serializeInstance(_), null, 2));
	...
	items.forEach_(_, function(_, item) {
		...
		tracer && tracer("jsonExport profile item definition for " + profileIt + ' :' + JSON.stringify(profItemDefs[profileIt], null, 2));

		var entity = model.getEntity(_, profileIt);
		var itemProto = serializeProto(_, entity, profItemDefs, model, profileInst);
		tracer && tracer("jsonExport exported itemProto :" + JSON.stringify(itemProto, null, 2));
		...
		exportedObjs.forEach_(_, function(_, ob) {
			...
			//tracer && tracer("INST:  " +JSON.stringify(ob.serializeInstance(_),null,2));

			var inst = db.fetchInstance(_, entity, ob.$uuid);
			if (!inst) {
				tracer && tracer("inst not fetched " + ob.$uuid);
				return;
			}
...
exports.jsonExport = function(_, profileInst, options) {
	...
	if (!profileInst.$diagnoses || !profileInst.$diagnoses.some(function(d) {
		...
		opt.$diagnoses && opt.$diagnoses.push({
			$severity: "success",
			$message: locale.format(module, "exportCompleted", options.path, exportObj.$items.length)
		});
...
function serializeProto(_, entity, profItemDefs, model, profileInst) {
	tracer && tracer("proto serialize " + entity.name);
...
function refSerialize(_, targetEntity, data, profItemDefs, model, profileInst, localization) {
	...
	//tracer && tracer("refSerialize " + sys.inspect(data));
			...
			if (relation.getIsChild()) {
				...
				collData.forEach_(_, function(_, elt) {
					if (keysel && keysel.length === 1) {
						...
						if (prop.$isLocalized) {
							tracer && tracer("RELATION " + rel + " is localised");
...
function objSerialize(_, instance, profItemDefs, model, type, profileInst, localization) {
	...
	tracer && tracer(" objSerialize: type: " + type);

	function serializeProp(_, prop, property) {
		tracer && tracer("jsonExport PROP : " + prop);
		if (propSelection.length > 0 && propSelection.indexOf(prop) === -1) {
		...
		if (!propVal) {
			tracer && tracer("jsonExport: " + prop + "is null or undefined");
			return;
		}
	...
	flows.eachKey(_, meta.$relations, function(_, rel, relation) {
		...
		if (relation.isPlural) {
			...
			if (!collData.length) {
				tracer && tracer("collection is empty");
				return null;
			}

			tracer && tracer("jsonExport: Collection " + collData.length);
			resource[rel] = [];
			if (relation.getIsChild()) {
				tracer && tracer("jsonExport: child relation");
				collData.forEach_(_, function(_, elt) {
				...
				collData.forEach_(_, function(_, elt) {
					if (keysel && keysel.length === 1) {
						if (!elt[keysel[0]](_)) //notValuedkey
							...
							if (prop.$isLocalized) {
								tracer && tracer("RELATION " + rel + " is localized");

								var locCode = keysel[0] + '_' + elt.$uuid;
								tracer && tracer("locCode: " + locCode);

								resource[rel].push(locCode);
								tracer && tracer("resource[" + rel + "]: " + JSON.stringify(resource[rel], null, 2));

								var l = locale.current.toLowerCase();
								tracer && tracer("localization[" + l + "]: " + sys.inspect(localization[l]));
								localization[l] = {};
								if (!localization[l][locCode]) localization[l][locCode] = elt[keysel[0]](_);
							} else resource[rel].push(elt[keysel[0]](_));
							tracer && tracer("resource: " + JSON.stringify(resource));
						}
					} else {
						var it = {};
						keysel.forEach_(_, function(_, k) {
			...
			if (relation.getIsChild()) {
				...
				if (!val[keysel[0]](_)) profileInst.$addDiagnose("error", locale.format(module, "notValuedkey", keysel[0]));
					...
					if (prop && prop.$isLocalized) {
						var locCode = keysel[0] + '_' + val.$uuid;
						tracer && tracer("property " + keysel[0] + " of relation " + rel + " is localised; code: " + locCode);
						resource[rel] = locCode;
		...
		//tracer && tracer("exported RELATION: " + rel) ;
	});
...
function _serialize(_, result, localization, stdProfile, entity, item, options) {
	...
	function _onRelationItem(_, r, relDef, relItem, cb) {
		tracer && tracer("jsonExport.OnRelationItem enter " + relItem.$uuid);
	...
	//
	tracer && tracer("jsonExport._serialize enter " + item.$uuid + " has profile: " + (stdProfile != null) + "; " + sys.inspect(stdProfile, null, 4));
	var res = {
	...
	(std.$properties || []).forEach_(_, function(_, pName) {
		...
		tracer && tracer("jsonExport.serialize property " + res.$type + "." + pName + ": " + res[pName]);
	});
	...
	Object.keys(std.$relations || {}).some_(_, function(_, rName) { // use some instead forEach to be able to stop the loop
		...
		if (r.isPlural) {
			...
			item[rName](_).toArray(_).forEach_(_, function(_, relItem) {
				tracer && tracer("exporting plural relation item: " + relItem._meta.name + "." + relItem.$uuid + "; has relDef: " + (relDef != null) + "; has filter " + ff);
				var ff = filter;
				var rd = relDef;
		...
		tracer && tracer("jsonExport.serialize relation " + res.$type + "." + rName + ": " + sys.inspect(res[rName], null, 4));
		return false;
	});
	//
