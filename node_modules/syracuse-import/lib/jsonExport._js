...
exports.jsonExport = function (_, instance,filename) {
    ...
    items.forEach_(_, function (_, item, idx) {
        ...
        serializeRelProto(_,entity, proto, allItems,model);
    ...
    fs.writeFile(fsp.join(__dirname, "../../../import", filename+".json"), JSON.stringify(exportObj, null, 2));

}

function serializeRelProto(_,entity, proto, allItems,model) {
    flows.eachKey(_, entity.$relations, function (_, rel, relation) {
        ...
        if (relation.isChild) {
            ...
            serializeRelProto(_,entityRel, proto[rel], allItems,model) ;
...
function refSerialize(_, instance,elt ,propSelection,  allItems, model, type) {
    var resource={} ;
console.log("serializeing reference ");
...
function objSerialize(_, instance, propSelection,  allItems, model, type) {
    ...
    flows.eachKey(_, meta.$properties, function (_, prop, property) {
        console.log("PROP:" + prop);
        if (propSelection.length > 0 && propSelection.indexOf(prop) == -1) return;
        if (property.isExternalStorage() && !(instance[prop](_) && instance[prop](_).fileExists(_))) {
            console.log("PROP ExternalStorage:" + prop);
            return;
        }
