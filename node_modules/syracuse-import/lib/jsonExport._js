...
/*        return {
    ...
    db_file: function(_, exportObj, options) {
        if(!options.storageDescription || !(options.storageVolume && options.storageVolume.$uuid))
...
/*        return {
    ...
    server: function(_, exportObj, options) {
        if(!options.friendServer || !options.friendServer.$uuid)
...
//    console.log("LOCALIZATION: "+sys.inspect(localis)) ;
    ...
    items.forEach_(_,function(_,item){
        ...
        item.entityAttribute(_).toArray(_).forEach_(_, function (_, el) {
            if (selection.props.indexOf(el.name(_)==-1) )
                selection.props.push(el.name(_));
        });
...
function serializeProto(_, entity,  profItemDefs,  model, profileInst) {
    ...
    if ( profItemDefs[clnam].keys.length === 1) {
        ...
        if (prop && prop.$isLocalized)
            proto["$key"] = keys[0]+'.'+locale.current;
        else if ( prop || entity.$relations[keys[0]])
            proto["$key"] = keys[0] ;
    }
        ...
        var lockeys=keys.map_(_,function(_,k){
            ...
            if (prop && prop.$isLocalized)
                return  k+'.'+locale.current;
            else if (prop || entity.$relations[k] )
                return k ;
        });
        proto["$key"] = lockeys ;
    }
        ...
        if (relation.isChild) {
            ...
            //tracer && tracer("jsonExport: exporting relation proto: " + sys.inspect(proto[rel]));
                ...
                if ( relkeys.length === 1) {
                    ...
                    if (prop && prop.$isLocalized)
                        proto[rel]["$key"] = relkeys[0]+'.'+locale.current;
                    else if ( prop || entityRel.$relations[relkeys[0]])
                        proto[rel]["$key"] = relkeys[0] ;
                }
                    ...
                    var keys=relkeys.map_(_,function(_,k){
                        ...
                        if (prop && prop.$isLocalized)
                            return  k+'.'+locale.current;
                        else if (prop || entityRel.$relations[k] )
                            return k ;
                    });
...
function refSerialize(_, targetEntity, data, profItemDefs, model, profileInst, localization) {
    ...
    flows.eachKey(_, targetEntity.$relations, function (_, rel, relation) {
        ...
        if (relation.isPlural)
            reltype=model.singularize(relation.$type) ;
        else 
                    ...
                    if (keysel && keysel.length === 1) {
                        ...
                        var prop=  relation.targetEntity.$properties[keysel[0]] ;
                          if (prop.$isLocalized){
                               ...
                               //flows.eachKey(_,localization , function (_, lang ) {
                                if (!localization[locale.current][locCode])
                                    localization[locale.current][locCode]=elt[keysel[0]](_) ;
                           //});
                        }
                        else
                            resource[rel].push(elt[keysel[0]](_));
                           ...
                           // it[k] = elt[k](_);
                             var prop=relation.targetEntity.$properties[k] ;
                                if ( prop && prop.$isLocalized){
                                  ...
                                  it[k] =  locCode ; 
                                  if (!localization[locale.current][locCode])
                                        localization[locale.current][locCode]=elt[k](_) ;  
                                }else
                            it[k] = elt[k](_);
                        });
            ...
            if (relation.isChild) {
                ...
                if (prop.$isLocalized){
                    ...
                    resource[rel]=locCode ;
                     //resource[rel].push(locCode);
                    if (!localization[locale.current][locCode])
                        localization[locale.current][locCode]=val[keysel[0]](_) ;
                    }
                    else
                    ...
                    keysel.forEach_(_, function (_, k) {
                        if (val[k](_)){
                                ...
                                if ( prop && prop.$isLocalized){
                                  ...
                                  it[k] =  locCode ; 
                                  if (!localization[locale.current][locCode])
                                        localization[locale.current][locCode]=val[k](_) ;  
                                }else
                                    it[k] = val[k](_);
                        }
...
function objSerialize(_, instance,  profItemDefs, model, type,profileInst,localization) {
    ...
    flows.eachKey(_, meta.$relations, function (_, rel, relation) {
       ...
       if(relation.isPlural)
            ...
            reltype=relation.$type ; //helpers.string.pluralize(relation.$type) // targetEntity.name ;
        ...
        if (relation.isPlural) {
            ...
            if (relation.isChild) {
                ...
                if (error) 
                    profileInst.$addDiagnose("error",  locale.format(module, "controlKeys", keysel, type) );
...
function _serialize(_, result, stdProfile, entity, item, options) {
    ...
    function _onRelationItem(_, r, relDef, relItem, cb) {
        ...
        if(r.isChild) {
            ...
            cb(_serialize(_, result, relDef, relItem.getEntity(), relItem, options));
        } else {
            // export keys
