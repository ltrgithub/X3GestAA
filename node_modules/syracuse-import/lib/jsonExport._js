...
var _exportTargetsMap = {
    download: function(_, exportObj, options) {
        console.log("****_exportTargetsMap download******exportObj: "+sys.inspect(exportObj)) ;
    ...
    file: function(_, exportObj, options) {
        console.log("***_exportTargetsMap file") ;
        if (!options.path) throw new Error(locale.format(module, "missingParameter", "path"));
        fs.writeFile(fsp.join(__dirname, "../../../import", options.path + ".json"), JSON.stringify(exportObj, null, options.stringifySpace));
        return {
    ...
    items.forEach_(_, function (_, item, idx) {
        ...
        if ((filter && filter !== "") || all){
            ...
            while(inst=cursor.next(_)) {
                ...
                exportItem = objSerialize(_, inst, propSelection, keys ,profItemDefs, model, clnam,profileInst,exportObj.$localization['en-us']);
                if (exportItem){
           ...
           return ; 
            ...
            exportedObjs.forEach_(_, function (_, ob, idx) {
                ...
                var exportItem ;
                exportItem = objSerialize(_, inst, propSelection, keys,profItemDefs, model, model.singularize(profItName),profileInst,exportObj.$localization['en-us']);
            ...
            console.log("all items exported") ;
            });
    });
console.log("profile to json content returns: "+ sys.inspect(exportObj) ) ;
    //
    return exportObj;
}
...
exports.jsonExport = function (_, profileInst, options) {
    ...
    console.log("*****jsonExport*******exportObj: "+sys.inspect(exportObj)) ;
            
    var opt = options || {};
    opt.stringifySpace = opt.beautify ? "\t" : null;
    var result;
    console.log("profileInst.$diagnoses: "+sys.inspect(profileInst.$diagnoses)) ;
    if(!profileInst.$diagnoses || !profileInst.$diagnoses.some(function(d) {
                       return d.severity === "error"
    })) {
        console.log("opt: "+sys.inspect(opt)) ;
...
function refSerialize(_, targetEntity, data, propSelection, keySelection ,profItemDefs, model, profileInst, localization) {
    ...
    tracer && tracer("jsonExport: exporting property: "+prop) ;
        ...
        if (property.$isLazy || property.isExternalStorage()) {
            ...
            var allLocales=inst.getPropAllLocales(_,prop) ;
            console.log("*relation*****ALL LOCALES: "+prop+" : "+JSON.stringify(allLocales,null,2)) ;      
        ...
        if (relation.isPlural) {
            ...
            if (relation.isChild) {
                ...
                collData.forEach_(_, function (_, elt) {
                     var serialized = refSerialize(_, val._relMeta.targetEntity, elt, propselNames,keysel, profItemDefs, model,profileInst,localization);
                    if (serialized){
                    ...
                    //DARY
                        var prop=  relation.targetEntity.$properties[keysel[0]] ;
                          if (prop.$isLocalized){
                                ...
                                if (!localization[locCode])
                                    localization[locCode]=elt[keysel[0]](_) ;
            ...
            if (relation.isChild) {
                ...
                var serialized=refSerialize(_, val._relMeta.targetEntity, val, propselNames,keysel, profItemDefs, model,profileInst,localization);
                if (serialized ){
                   ...
                   //DARY
                      var prop=relation.targetEntity.$properties[keysel[0]] ;
                       ...
                       if (prop.$isLocalized){
                                ...
                                if (!localization[locCode])
                                    localization[locCode]=val[keysel[0]](_) ;
                                }
                                else
...
function objSerialize(_, instance, propSelection,  keySelection ,profItemDefs, model, type,profileInst,localization) {
...
function serializeProp(_, prop, property) {
        ...
        if (property.$isLazy || property.isExternalStorage()) {
            ...
            if (!(localization[resource[prop]])){
                //console.log("localization["+resource[prop]+"] is null or undefined" ) ;
                localization[resource[prop]]=locales['en-us'] ;
                //console.log("locales['en-us']: "+locales['en-us']) ;
            }
            console.log("LocalizationCode :"+resource[prop]) ;
            console.log("Localization :"+JSON.stringify(localization)) ;
...
console.log("à l'extérieur: "+JSON.stringify(localization)) ;
...
function _serialize(_, result, stdProfile, entity, item, options) {
    function _getProp(_, pName, inst) {
        var p = inst.getEntity().$properties[pName];
        if(!p) return null;
        if(p.$isLocalized)
            return inst.getPropAllLocales(_, pName);
        else
            return resourceHelpers.formatValue(p, inst[pName](_));
    }
