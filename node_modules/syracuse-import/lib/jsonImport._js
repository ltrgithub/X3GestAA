...
function _importCollection(_, inst, relName, relMeta, relProto, value, localization) {
	...
	var db = inst._db;
	tracer && tracer("jsonImport._importCollection: relation: " + relName);
	...
	if (Array.isArray(value)) value.forEach_(_, function(_, relItemValue) {
		...
		if (relMeta.getIsChild(varType)) {
			...
			//references
			relInst = relItemValue ? db.fetchInstance(_, te, _getFilter(_, db, relKey, relItemValue, te, relProto, localization)) : null;
...
function _importReference(_, inst, relName, variantName, relMeta, instProto, value, localization) {
	var key = relName;
	var db = inst._db;
	...
	if (relMeta.getIsChild(variantName) || relMeta.$inlineStore) {
		...
		if (!relInst) {
			...
			_importInstance(_, relInst, value, rp, localization);
			inst[key](_, relInst);
		} else _importInstance(_, relInst, value, rp, localization);
	} else {
		var relInst = null;
...
function _importInstance(_, instance, data, itemProto, localization) {
	...
	flows.eachKey(_, data, function(_, key, value) {
		...
		if (entity.$relations && (relMeta = entity.$relations[key])) {
			...
			if (relMeta.isPlural) {
				_importCollection(_, inst, key, relMeta, relProto, value, localization);
			} else {
				var varType = relMeta.$variants && value && (value.$variantType || value.$type);
				_importReference(_, inst, key, varType, relMeta, itemProto, value, localization);
			}
		} else //
