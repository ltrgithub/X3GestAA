...
function _simplifyItem(_, entity, item, itemProto, _pointerTypes) {
	...
	tracer && tracer("_simplifyItem: item=" + sys.inspect(item) + "\nentity=" + sys.inspect(entity));
	itemProto = itemProto || {};
	flows.eachKey(_, item, function(_, key, value) {
		...
		tracer && tracer("_simplifyItem: key=" + key);

		var relMeta, propMeta;
		if (entity.$relations && (relMeta = entity.$relations[key])) {
			// relation
			tracer && tracer("_simplifyItem relation: key=" + key + ", value=" + sys.inspect(value));
			var relProto = itemProto[key];
			if (relMeta.isPlural) {
				tracer && tracer("_simplifyItem: key=" + key + ", value=" + sys.inspect(value));
				tracer && tracer("entity=" + sys.inspect(entity));
				tracer && tracer("itemProto=" + sys.inspect(itemProto));
			...
			// tracer && tracer("entity=" + sys.inspect(entity));
			// tracer && tracer("itemProto=" + sys.inspect(itemProto));
			// tracer && tracer("propMeta=" + sys.inspect(propMeta));
...
function _simplifyCollection(_, entity, relName, relMeta, relProto, value, _pointerTypes) {
	...
	// tracer && tracer("_simplifyCollection: name=" + relName + ", relMeta=" + JSON.stringify(relMeta) + //
