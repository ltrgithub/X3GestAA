...
function _getFilter(_, db, key, value, entity, proto, localization) {
    ...
    if (typeof value == "object") {
        ...
        if ((typeof keyvalue == "string") && keyvalue.match(new RegExp(key + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
            if (entity.$properties && entity.$properties[key] && entity.$properties[key].$isLocalized) {
                console.log("****LOCALIZED prop key: " + key);
                console.log("LOCALIZATION value: " + localization['en-us'][keyvalue]);
                //throw Error("Format error: localized type but one part key: "+entity.name+"."+key);
                filter.jsonWhere[key] = localization['en-us'][keyvalue]; //value[parts[0]][parts[1]];
            }
        } else {
            console.log("non localized key " + key + ": " + value[key]);
            filter.jsonWhere[key] = value[key];
        }
    } else {
        if ((typeof value == "string") && value.match(new RegExp(key + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
            if (entity.$properties && entity.$properties[key] && entity.$properties[key].$isLocalized) {
                ...
                console.log("LOCALIZATION value: " + localization['en-us'][value]);
                filter.jsonWhere[key] = localization['en-us'][value];
            }
        } else {
...
function _importCollection(_, inst, relName, relMeta, relProto, value, localization) {
    ...
    console.log("IMPORT COLLECTION " + relName);
        ...
        value.forEach_(_, function (_, relItemValue) {
            ...
            if (relMeta.isChild) {
                ...
                relKeys.forEach(function (relKeyItem) {
                    console.log("ligne 74: " + relKeyItem);
                    if (relProto[relKeyItem]) {
                        //console.log("ligne 76: "+relKeyItem + "." + relProto[relKeyItem].$key) ;

                        var fullRelKey = relKeyItem + "." + relProto[relKeyItem].$key;
                        console.log("relMeta.isChild");
                        //console.log("relItemValue["+relKeyItem+"]["+relProto[relKeyItem].$key+"]") ;



                        opt.jsonWhere[fullRelKey] = relItemValue[relKeyItem] && relItemValue[relKeyItem][relProto[relKeyItem].$key];
                    } else {
                        console.log("ligne 80: " + relKeyItem + " ----- " + relItemValue[relKeyItem]);
                        ...
                        if (relProto.$localized && relProto.$localized.indexOf(relKeyItem) != -1) {
                        	//TODO hard coded
                            opt.jsonWhere[fullRelKey] = localization['en-us'][relItemValue[relKeyItem]];
                        } else
        ...
        if (entity.$relations && (relMeta = entity.$relations[key])) {
                ...
                if (relMeta.isChild || relMeta.$inlineStore) {
                    ...
                    if (value) {
                        ...
                        if (!isNull) {
                            ...
                            //console.log("fetched instance : "+sys.inspect(relInst)) ;
                            if (!relInst) {
            ...
            if ((typeof value == "string") && value.match(new RegExp(key + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
                if (propMeta.$isLocalized) {
                    //console.log(" assigning LOCALIZED prop key: " + key + " value: " + value);
    ...
    function _import(_, db, proto, data, localization, options) {
        ...
        try {
            ...
            data.forEach_(_, function (_, item) {
                ...
                var inst = db.fetchInstance(_, entity, _getFilter(_, db, key, item, entity, itemProto, localization));
                    ...
                    //for each language
                      flows.eachKey(_,localization, function (_, lang, values) {
                      	for (var l in include.$localization[lang]) {
                      		...
                      		//console.log("$localization["+lang+"]: " + sys.inspect(localization[lang]));
                     	 }
                      });
            });
        }
