...
function _getFilter(_, db, key, value, entity, proto, localization) {
	...
	if (Array.isArray(key)) {
		...
		typeof value[simpleKey] == "string" && value[simpleKey].match(new RegExp(simpleKey + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
			...
			var parts = key.split(".");
			if (parts.length < 2) throw Error("Format error: localized type but one part key: " + entity.name + "." + key);
	...
	try {
		...
		data.forEach_(_, function(_, item) {
			...
			diags.forEach_(_, function(_, d) {
				...
				// use locale['format'] rather than locale.format to defeat pre-commit error on dynamic message id
				if (path) d.message = locale['format'](module, "diagPrefix_" + d.$severity, item.$type, path.resource[path.property], d.$message);
