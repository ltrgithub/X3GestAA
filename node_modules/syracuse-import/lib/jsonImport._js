...
function _importFileName(filename) {
	var importFolder = (filename.indexOf("/") >= 0) ? "node_modules" : "import";
	return fsp.join(config.system.root, importFolder, filename);
}
...
exports.importTest = function(_, filename, ifNoneMatch) {
	var fname = _importFileName(filename);
	try {
...
exports.jsonImport = function(_, db, filename, options) {
	...
	var fname = filename;
	try {
		// check if the file exists, the path is then absolute
		fs.stat(fname, _);
	} catch (e) {
		// filename is supposed to be located in "import" folder (same level as node_modules)
		fname = _importFileName(filename);
	}
