...
function _getFilter(_, db, key, value, entity, proto, localization) {
   // tracer && tracer
   console.log ("jsonImport._getFilter for " + sys.inspect(key) + "=" + sys.inspect(value) + "; entity: " + (entity && entity.name));
    var filter = {
        jsonWhere: {}
    };
    ...
    if (typeof value == "object") {
        var simpleKey = key.split(".")[0];
         console.log("!!!!!!!!!!!!!! key:  "+sys.inspect(key)) ;
       ...
       // console.log("simpleKey "+simpleKey) ;
        if (entity.$properties && entity.$properties[simpleKey] && entity.$properties[simpleKey].$isLocalized //
           ...
           // console.log("keyvalue "+keyvalue) ;
                filter.jsonWhere[parts[0]] = { };
            filter.jsonWhere[parts[0]][parts[1]] = localization[parts[1]][keyvalue] ;
            //filter.jsonWhere[key]= localization[parts[1]][keyvalue] ;
             console.log("filter ligne 54 "+JSON.stringify(filter)) ;
        } else {
            //console.log("non localized key " + key + ": " + value[key]);
            filter.jsonWhere[key] = value[simpleKey];
            console.log("filter ligne 58 "+JSON.stringify(filter)) ;
        }
    }
     else {
        var parts=key.split(".") ;
        if (parts.length>1 &&  typeof value == "string" && value.match(new RegExp(parts[0] + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
            if (entity.$properties && entity.$properties[parts[0]] && entity.$properties[parts[0]].$isLocalized) {
                //console.log("****LOCALIZED prop key: " + key + " value: " + value);
               // console.log("LOCALIZATION value: " + localization[parts[1]][value]);
                filter.jsonWhere[parts[0]] = { } ;
                filter.jsonWhere[parts[0]][parts[1]]= localization[parts[1]][value];
                //filter.jsonWhere[key]=localization[parts[1]][value];
            }
        } else {
            //console.log("non localized key " + key + ": " + value);
            filter.jsonWhere[key] = value;
        }

    }
    //tracer && tracer
    console.log("jsonImport get filter: " + sys.inspect(filter, null, 4));
    return filter;
}
//
        ...
        value.forEach_(_, function (_, relItemValue) {
            ...
            if (relMeta.isChild) {
                ...
                relKeys.forEach(function (relKeyItem) {
                    console.log("ligne 98: " + relKeyItem);
                    

                    
                    if (relProto[relKeyItem] ) { //relProto[parts[0]]
                       // console.log("ligne 100: "+relKeyItem + "." + relProto[relKeyItem].$key) ;
                       // console.log("relMeta.isChild");
                        ...
                        if (parts.length>1)
                        {
                            console.log("localized key lang "+parts[1]) ;
                           
                           
                               // opt.jsonWhere[fullRelKey] = relItemValue[relKeyItem] && relItemValue[relKeyItem][relProto[relKeyItem].$key];
                                var fullRelKey =  relKeyItem + "." + relProto[relKeyItem].$key ;// parts[0];
                               // console.log("relKeyItem: "+relKeyItem+"******** relItemValue[relKeyItem]: "+JSON.stringify(relItemValue[relKeyItem],null,2)) ;
                               if (relItemValue[relKeyItem] && typeof relItemValue[relKeyItem]=="object"){
                               // console.log("******** CODE pour localisation: "+relItemValue[relKeyItem][parts[0]]) ;
                               // opt.jsonWhere[fullRelKey]=localization[parts[1]][relItemValue[relKeyItem][parts[0]]] ;
                               opt.jsonWhere[fullRelKey]=localization[parts[1]][relItemValue[relKeyItem][parts[0]]] ;
                            }
                            else
                                opt.jsonWhere[fullRelKey]=localization[parts[1]][relItemValue[relKeyItem]] ;

                        }
                        else{
                           //console.log(" non localized key") ;
                            opt.jsonWhere[fullRelKey] = relItemValue[relKeyItem];
                        }
                    } else {
                        // console.log("propriété et pas relation") ;
                        //console.log("ligne 80: " + parts[0] + " ----- " + JSON.stringify(relItemValue)+'  ' +relItemValue[parts[0]]);

                        var fullRelKey = relKeyItem;

                        //if ((relItemValue[relKeyItem] && typeof relItemValue[relKeyItem] == "string") && relItemValue[relKeyItem].match(new RegExp(relKeyItem + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
                        
                        //opt.jsonWhere[fullRelKey] = localization[parts[1]][relItemValue[relKeyItem][parts[0]]] ;
                       
                        
                        //console.log("relProto.$localized: " + sys.inspect(relProto.$localized));
                        //if (relProto.$localized && relProto.$localized.indexOf(relKeyItem) != -1) {
                ...
                //references
                console.log("references " +relMeta.targetEntity.name) ;
                //console.log("call getFilter with relItemValue "+JSON.stringify(relItemValue)+" relProto "+sys.inspect(relProto));
        ...
        if (entity.$relations && (relMeta = entity.$relations[key])) {
            ...
            if ((typeof value == "string") && value.match(new RegExp(key + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
                if (propMeta.$isLocalized) {
                    ...
                    var allLocales = {"default" : localization[locale.current] && localization[locale.current][value]};
...
function _import(_, db, proto, data, localization, options) {
    ...
    try {
        ...
        data.forEach_(_, function (_, item) {
            ...
            var filt=_getFilter(_, db, key, item, entity, itemProto, localization) ;
            var inst = db.fetchInstance(_, entity, filt);
            ...
            if (inst){
                if (inst._data.description){
                    ...
                    console.log("*******ALLS : "+JSON.stringify(alls)) ;
                }
            }
            //
            ...
            diags.forEach_(_, function (_, d) {
                ...
                var p = itemProto.$value || itemProto.$key || "description";
                var path = p ? helpers.object.walkPath(item, p) : null;
                if (path)
                    d.message = locale.format(module, "diagPrefix_" + d.severity, item.$type, path.resource[path.property], d.message);
