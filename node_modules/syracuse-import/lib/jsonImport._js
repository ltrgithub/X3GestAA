...
function _getFilter(_, db, key, value, entity, proto, localization) {
	...
	if (Array.isArray(key)) {
		...
		typeof value[simpleKey] == "string" && value[simpleKey].match(new RegExp(simpleKey + "_[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"))) {
			...
			var parts = key.split(".");
			if (parts.length < 2) 
			inst.$addDiagnose("error", locale.format(module, "formatError", entity.name, key)) ;
			return ;
			...
			if (!loclang)
				inst.$addDiagnose("error", locale.format(module, "localizationError", parts[1])) ;
	...
	try {
		...
		data.forEach_(_, function(_, item) {
			...
			diags.forEach_(_, function(_, d) {
				...
				var lkey="diagPrefix_" + d.$severity ;
				if (path) d.message = locale.format(module, lkey, item.$type, path.resource[path.property], d.$message);
