		...
		value.forEach_(_, function(_, relItemValue) {
			...
			if(relMeta.isChild) {
				...
				if(Array.isArray(relKey)) {
					relKey.forEach(function(relKeyItem) { 
	...
	flows.eachKey(_, data, function(_, key, value) {
		...
		if(entity.$relations && (relMeta = entity.$relations[key])) {
			...
			if(relMeta.isPlural) {
				...
				var relInst = db.fetchInstance(_, relMeta.targetEntity, _getFilter(_, db, relKey, value, relMeta.targetEntity, itemProto));
				inst[key](_, relInst);
			}
		} else
...
//			inst._initialize(_);
		...
		inst.save(_);
		//
		options && options.$diagnoses && inst.getAllDiagnoses(_, options.$diagnoses);
	});
}