...
function _import(_, db, proto, data, localization, options) {
	...
	try {
		...
		data.some_(_, function(_, item) {
			...
			if (inst && (importMode === "insert")) return (tracer.debug && tracer.debug("jsonImport will not import because in insert only mode")) && false;
...
exports.jsonImport = function(_, db, filename, options) {
	...
	if (options.ifNoneMatch) {
		...
		if (automaticData.updDate && (Math.abs(automaticData.updDate.getTime() - modTime.getTime()) < 600000)) {
			tracer.debug && tracer.debug("No modification " + fname);
			return true;
		}
		...
		if (h === automaticData.contentHash) {
			// no modification
			tracer.debug && tracer.debug("No content modification " + fname);
			return;
		}
	...
	tracer.debug && tracer.debug("Import diagnoses: " + sys.inspect(options.$diagnoses));
