...
function loadBindings() {
  var os = require('os');
  var v8 = /^\d+\.\d+/.exec(process.versions.v8)[0];
  var filename = __dirname + '/../bin/' + os.platform() + '-' + os.arch() + '-v8-' + v8 + '/oracle_bindings.node';
  if (require('fs').existsSync(filename)) return require(filename);
  return require("../build/Release/oracle_bindings");
}
...
Reader.prototype.discard = function() {
  if (this._handle)
    this._handle.discard.apply(this._handle);
}
