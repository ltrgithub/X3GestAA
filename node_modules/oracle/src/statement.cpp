...
void Statement::Init(Handle<Object> target) {
  ...
  uni::Deref(constructorTemplate)->SetClassName(STRING_NEW_SYMBOL("Statement"));

  NODE_SET_PROTOTYPE_METHOD(uni::Deref(constructorTemplate), "execute", Execute);
  target->Set(STRING_NEW_SYMBOL("Statement"), uni::Deref(constructorTemplate)->GetFunction());
}
...
uni::CallbackType Statement::Execute(const uni::FunctionCallbackInfo& args) {
  ...
  if (baton->busy) {
    UNI_THROW(Exception::Error(STRING_NEW("invalid state: statement is busy with another execute call")));
  }
  baton->busy = true;
