...
void Reader::Init(Handle<Object> target) {
  ...
  uni::Deref(constructorTemplate)->SetClassName(STRING_NEW_SYMBOL("Reader"));

  NODE_SET_PROTOTYPE_METHOD(uni::Deref(constructorTemplate), "nextRows", NextRows);
  NODE_SET_PROTOTYPE_METHOD(uni::Deref(constructorTemplate), "discard", Discard);
  target->Set(STRING_NEW_SYMBOL("Reader"), uni::Deref(constructorTemplate)->GetFunction());
}
...
uni::CallbackType Reader::Discard(const uni::FunctionCallbackInfo& args) {
  UNI_SCOPE(scope);
...
uni::CallbackType Reader::NextRows(const uni::FunctionCallbackInfo& args) {  
  ...
  if (baton->busy) {
    UNI_THROW(Exception::Error(STRING_NEW("invalid state: reader is busy with another nextRows call")));
  }
  baton->busy = true;
...
void Reader::EIO_AfterNextRows(uv_work_t* req, int status) {
  ...
  // Is this necessary? Feels like overkill
  ...
  node::MAKE_CALLBACK(CURRENT_CONTEXT()->Global(), cb, 2, argv);
}
