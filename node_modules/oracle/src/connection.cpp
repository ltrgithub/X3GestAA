...
void Connection::Init(Handle<Object> target) {
  ...
  Local<FunctionTemplate> t = FUNCTION_TEMPLATE_NEW(New);
  uni::Reset(constructorTemplate, t);
  uni::Deref(constructorTemplate)->InstanceTemplate()->SetInternalFieldCount(1);
  uni::Deref(constructorTemplate)->SetClassName(STRING_NEW_SYMBOL("Connection"));
  ...
  target->Set(STRING_NEW_SYMBOL("Connection"), uni::Deref(constructorTemplate)->GetFunction());
}
...
uni::CallbackType Connection::Execute(const uni::FunctionCallbackInfo& args) {
  ...
  if (baton->error) {
    Local<String> message = STRING_NEW(baton->error->c_str());
    delete baton;
...
uni::CallbackType Connection::Close(const uni::FunctionCallbackInfo& args) {
  ...
  try {
    ...
    UNI_THROW(Exception::Error(STRING_NEW(ex.what())));
  }
}
...
uni::CallbackType Connection::IsConnected(const uni::FunctionCallbackInfo& args) {
  ...
  if(connection && connection->m_connection) {
    UNI_RETURN(scope, args, BOOLEAN_NEW(true));
  } else {
    UNI_RETURN(scope, args, BOOLEAN_NEW(false));
  }
}
...
void Connection::CreateColumnsFromResultSet(oracle::occi::ResultSet* rs, ExecuteBaton* baton, vector<column_t*> &columns) {
  ...
  for (vector<oracle::occi::MetaData>::iterator iterator = metadata.begin(), end = metadata.end(); iterator != end; ++iterator) {
    ...
    int scale = metadata.getInt(oracle::occi::MetaData::ATTR_SCALE);
    int precision = metadata.getInt(oracle::occi::MetaData::ATTR_PRECISION);
    ...
    switch(type) {
      ...
      case oracle::occi::OCCI_TYPECODE_SMALLINT:
      	col->type = ((precision != 0) && (scale == 0))?VALUE_TYPE_INUMBER:VALUE_TYPE_NUMBER ;
...
void Connection::EIO_AfterCommit(uv_work_t* req, int status) {
  ...
  argv[0] = UNDEFINED();
  argv[1] = UNDEFINED();
  node::MAKE_CALLBACK(CURRENT_CONTEXT()->Global(), uni::Deref(baton->callback), 2, argv);
  delete baton;
  delete req;
}
...
void CallDateMethod(v8::Local<v8::Date> date, const char* methodName, int val) {
  Handle<Value> args[1];
  args[0] = NUMBER_NEW(val);
  Local<Function>::Cast(date->Get(STRING_NEW(methodName)))->Call(date, 1, args);
}
...
Local<Date> OracleDateToV8Date(oracle::occi::Date* d) {
  ...
  Local<Date> date = uni::DateCast(DATE_NEW(0.0));
...
Local<Object> Connection::CreateV8ObjectFromRow(ExecuteBaton* baton, vector<column_t*> columns, row_t* currentRow) {
  Local<Object> obj = OBJECT_NEW();
  uint32_t colIndex = 0;
  for (vector<column_t*>::iterator iterator = columns.begin(), end = columns.end(); iterator != end; ++iterator, colIndex++) {
    column_t* col = *iterator;
    void* val = currentRow->values[colIndex];
    if(val == NULL) {
      obj->Set(STRING_NEW(col->name.c_str()), NULL_VALUE());
    } else {
      switch(col->type) {
        case VALUE_TYPE_STRING:
          {
            string* v = (string*)val;
            obj->Set(STRING_NEW(col->name.c_str()), STRING_NEW(v->c_str()));
            delete v;
          }
          break;
        case VALUE_TYPE_INUMBER:
          {
            ...
            obj->Set(STRING_NEW(col->name.c_str()), NUMBER_NEW((double)(*v)));
            delete v;
          }
          break;
        case VALUE_TYPE_NUMBER:
          {
            ...
            if (baton->connection->getNumberStringFormat() == NULL)
              ...
              obj->Set(STRING_NEW(col->name.c_str()), STRING_NEW(v->toText(baton->connection->getEnvironment(), baton->connection->getNumberStringFormat()->c_str()).c_str()));
            delete v;
          }
          break;          
        case VALUE_TYPE_DATE:
          {
            ...
            obj->Set(STRING_NEW(col->name.c_str()), OracleDateToV8Date(v));
            delete v;
          }
          break;
        case VALUE_TYPE_TIMESTAMP:
          {
            ...
            obj->Set(STRING_NEW(col->name.c_str()), OracleTimestampToV8Date(v));
            delete v;
          }
          break;
        case VALUE_TYPE_CLOB:
          {
            ...
            v->close();
            obj->Set(STRING_NEW(col->name.c_str()), STRING_NEW2(columnVal.c_str(), totalBytesRead));
            delete v;
            delete [] buffer;
          }
          break;
        case VALUE_TYPE_BLOB:
          {
            ...
            uni::BufferType nodeBuff = node::BUFFER_NEW(buffer, blobLength, RandomBytesFree, NULL);
            v8::Local<v8::Object> globalObj = v8::CURRENT_CONTEXT()->Global();
            v8::Local<v8::Function> bufferConstructor = v8::Local<v8::Function>::Cast(globalObj->Get(v8::STRING_NEW("Buffer")));
            v8::Handle<v8::Value> constructorArgs[3] = { uni::BufferToHandle(nodeBuff), v8::INTEGER_NEW(blobLength), v8::INTEGER_NEW(0) };
            v8::Local<v8::Object> v8Buffer = bufferConstructor->NewInstance(3, constructorArgs);
            obj->Set(STRING_NEW(col->name.c_str()), v8Buffer);
            delete v;
            break;
          }
          break;
...
Local<Array> Connection::CreateV8ArrayFromRows(ExecuteBaton* baton, vector<column_t*> columns, vector<row_t*>* rows) {
  ...
  Local<Array> retRows = ARRAY_NEW(totalRows);
  uint32_t index = 0;
...
void Connection::EIO_AfterExecute(uv_work_t* req, int status) {
  ...
  try {
    ...
    Handle<Value> argv[2];
    argv[0] = Exception::Error(STRING_NEW(ex.what()));
    argv[1] = UNDEFINED();
    node::MAKE_CALLBACK(CURRENT_CONTEXT()->Global(), uni::Deref(baton->callback), 2, argv);
  }
...
void Connection::handleResult(ExecuteBaton* baton, Handle<Value> (&argv)[2]) {
  if(baton->error) {
failed:
    argv[0] = Exception::Error(STRING_NEW(baton->error->c_str()));
    argv[1] = UNDEFINED();
  } else {
    argv[0] = UNDEFINED();
    if(baton->rows) {
      ...
      Local<Object> obj = OBJECT_NEW();
      obj->Set(STRING_NEW("updateCount"), INTEGER_NEW(baton->updateCount));
      ...
      for (vector<output_t*>::iterator iterator = baton->outputs->begin(), end = baton->outputs->end(); iterator != end; ++iterator, index++) {
        ...
        switch(output->type) {
          case OutParam::OCCIINT:
            obj->Set(STRING_NEW(returnParam.c_str()), INTEGER_NEW(output->intVal));
            break;
          case OutParam::OCCISTRING:
            obj->Set(STRING_NEW(returnParam.c_str()), STRING_NEW(output->strVal.c_str()));
            break;
          case OutParam::OCCIDOUBLE:
            obj->Set(STRING_NEW(returnParam.c_str()), NUMBER_NEW(output->doubleVal));
            break;
          case OutParam::OCCIFLOAT:
            obj->Set(STRING_NEW(returnParam.c_str()), NUMBER_NEW(output->floatVal));
            break;
          case OutParam::OCCICURSOR:
            obj->Set(STRING_NEW(returnParam.c_str()), CreateV8ArrayFromRows(baton, output->columns, output->rows));
          ...
          case OutParam::OCCICLOB:
            {
              output->clobVal.open(oracle::occi::OCCI_LOB_READONLY);
      				...
      				obj->Set(STRING_NEW(returnParam.c_str()), STRING_NEW2(clobVal.c_str(), totalBytesRead));				
      				delete [] buffer;
      				break;
            }
          case OutParam::OCCIBLOB:
            {
              ...
              // convert to V8 buffer
              uni::BufferType nodeBuff = node::BUFFER_NEW(buffer, lobLength, RandomBytesFree, NULL);
              v8::Local<v8::Object> globalObj = v8::CURRENT_CONTEXT()->Global();
              v8::Local<v8::Function> bufferConstructor = v8::Local<v8::Function>::Cast(globalObj->Get(v8::STRING_NEW("Buffer")));
              v8::Handle<v8::Value> constructorArgs[3] = { uni::BufferToHandle(nodeBuff), v8::INTEGER_NEW(lobLength), v8::INTEGER_NEW(0) };
              v8::Local<v8::Object> v8Buffer = bufferConstructor->NewInstance(3, constructorArgs);
              obj->Set(STRING_NEW(returnParam.c_str()), v8Buffer);
              break;
            }
          case OutParam::OCCIDATE:
            obj->Set(STRING_NEW(returnParam.c_str()), OracleDateToV8Date(&output->dateVal));
            break;
          case OutParam::OCCITIMESTAMP:
            obj->Set(STRING_NEW(returnParam.c_str()), OracleTimestampToV8Date(&output->timestampVal));
            break;
          case OutParam::OCCINUMBER:
            obj->Set(STRING_NEW(returnParam.c_str()), NUMBER_NEW(output->numberVal));
            break;
          default:
            {
              ostringstream oss;
