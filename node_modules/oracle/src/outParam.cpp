...
void OutParam::Init(Handle<Object> target) {
  ...
  uni::Deref(constructorTemplate)->SetClassName(STRING_NEW_SYMBOL("OutParam"));
  target->Set(STRING_NEW_SYMBOL("OutParam"), uni::Deref(constructorTemplate)->GetFunction());
}
...
uni::CallbackType OutParam::New(const uni::FunctionCallbackInfo& args) {
  ...
  if (args.Length() >=2 && !args[1]->IsUndefined()) {
    ...
    if (opts->Has(STRING_NEW("in"))) {
      ...
      default:
        UNI_THROW(Exception::Error(STRING_NEW("Unhandled OutPram type!")));
      }
    }
  }
