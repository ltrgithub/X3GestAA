...
double CallDateMethod(v8::Local<v8::Date> date, const char* methodName) {
  ...
  Local<Value> result = Local<Function>::Cast(date->Get(STRING_NEW(methodName)))->Call(date, 0, args);
...
void ExecuteBaton::CopyValuesToBaton(ExecuteBaton* baton, v8::Local<v8::Array>* values) {
  ...
  for(uint32_t i=0; i<(*values)->Length(); i++) {
    ...
    // output
    else if(val->IsObject() && val->ToObject()->FindInstanceInPrototypeChain(uni::Deref(OutParam::constructorTemplate)) != v8::NULL_VALUE()) {
