...
exports.sendToEDI = function(_, options){
	try{
		if (options != null){
			...
			var proxyServerHost = helpers.proxy.server != null ? helpers.proxy.server : null;
			var proxyServerPort = helpers.proxy.port != null ? helpers.proxy.port : null;

			var debug = helpers.sageup.debug != null ? helpers.sageup.debug : false;
			var format = helpers.sageup.format != null ? helpers.sageup.format : 'xml';
			var params = helpers.sageup.params != null ? helpers.sageup.params : {};
			...
			// Put authentication informations in header
			sdata_options.auth = getAuthentication(options.auth);
			// Put host informations in header
...
function checkOptions(options){
	...
	if (options.method == null){
		...
		if (options.sdata == null && (options.method === 'POST' || options.method === 'PUT'))
			throw new Error("SData informations are required in 'options' parameter for 'POST' and 'PUT' requests.");
	}
	if (options.auth == null)
		throw new Error("Authentication informations are required in 'options' parameter.");
