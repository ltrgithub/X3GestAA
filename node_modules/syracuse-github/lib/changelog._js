...
var _formatters = {
	"html": function(json) {
		var cnt = '<!DOCTYPE html><html><head><meta charset="UTF-8" /></head><body>';
		json.forEach(function(tag) {
			cnt += '<h1>' + tag.name + '</h1>';
			tag.issues.forEach(function(issue) {
				cnt += '#' + issue.issue.number + ':' + '<a href="https://github.com/Sage-ERP-X3/Syracuse/issues/' + issue.issue.number + '" target="blank">' + issue.issue.title + '</a>';
				cnt += '<ul>';
				issue.commits.forEach(function(commit) {
					cnt += '<li>' + commit.date + ' ' + commit.committerName + ': ' + commit.message + '</li>';
				});
				cnt += '</ul>';
			});
		});
...
//		console.log("name: "+tag.name+", index: "+index);
		var locIssues = {};
...
//			console.log("Commit: "+c.sha+"; Date: "+r.date+" Message: "+r.message);
			// issue
...
exports.makeChangelog = function(_, opt) {
