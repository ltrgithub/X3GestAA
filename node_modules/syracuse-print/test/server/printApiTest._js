...
asyncTest("Connection", 5, function(_) {
	var cli = PrintClient.createClient(_, hostname, port);
...
asyncTest("ChangePriority and ListJobs",  function(_) {
	...
	var seqMap = [];
	for (var i = 0 ; i < 10; i++) {
		jobData1.__wJobID = helpers.uuid.generate();
		var seqId = cli.launchReport(_, jobData1);
		seqMap.push(seqId);
	...
	for (var j = 0; j < jobs.length; j++) {
		...
		if (j === 2)
			strictEqual(job.order, 2, "Check priority OK for rank "+job.rank+" and order '+'");
		if (j === 5)
			strictEqual(job.order, 7, "Check priority OK for rank "+job.rank+" and order '-'");
		if (j === 9)
			strictEqual(job.order, 1, "Check priority OK for rank "+job.rank+" and order 'MAX'");
		if (j === 0)
			strictEqual(job.order, 10, "Check priority OK for rank "+job.rank+" and order 'MIN'");
