...
/// 
exports.getClient = function(_, hostname, port) {
	var client = clients[hostname + ":" + port];
	if (client) {
		client._setSocketTimeout(_, true);
	} else {
		client = new PrintClient(_, hostname, port);
		clients[client.printServer] = client;
	}
	return client;
};
...
var PrintClient = exports.PrintClient = helpers.defineClass(function(_, hostname, port, standAlone) {
	...
	checkPrinterPaper: function(_, pName, pPort, pDriver, ppSizeId, ppSourceId) {
		...
		try {
			...
			if (pParams) {
				if (pParams.bac && (nbElem = pParams.bac.length) > 0 && (ppSourceId !== null && ppSourceId !== undefined)) {
					for (ii = 0; ii < nbElem; ii++) {
						if (pParams.bac[ii].$value == ppSourceId) {
							break;
						}
					}
					ret.paperSource = {
						"id": ii < nbElem ? ppSourceId : pParams.bac[0].$value
					};
				}
				if (pParams.paperSize && (nbElem = pParams.paperSize.length) > 0 && (ppSizeId !== null && ppSizeId !== undefined)) {
					for (ii = 0; ii < nbElem; ii++) {
						if (pParams.paperSize[ii].$value == ppSizeId) {
							break;
						}
					}
					ret.paperSize = {
						"id": ii < nbElem ? ppSizeId : pParams.paperSize[0].$value
					};
					try {
	...
	_setSocketTimeout: function(_, prevent) {
		...
		this.socketTimeout = setTimeout(function(_) {
			try {
				...
				delete clients[self.printServer];
			}
		}, (config.x3print && config.x3print.timeout) || 300000);
	}
});