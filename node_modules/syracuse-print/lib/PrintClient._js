...
var PrintComm = require('syracuse-print/lib/PrintComm').printComm;
var streams = require('streamline/lib/streams/streams');
var mime = require('streamline-static/lib/utils').mime;

var stack = [];
function getStackedJobs() {
	return stack.reduce(function(job, desc, index) {
		job[desc['jobId']] = {
			exportFormat: desc['exportFormat'],
			seqId: index
		};
		//console.log("Stacked Jobs: "+JSON.stringify(job,null,2));
		return job;
	}, {});
}
...
var typesMap = [
	undefined, //0
	".rpt",//1  Etat Crystal report (*.rpt)  
	undefined,//2  ?inactivf en V5 - Data interchange format  
	".doc",//3  Microsoft Word (*.doc)  
	undefined,//4  ?inactivf en V5 - Colonnes de valeurs  
	".rtf",//5  Format RTF (*.rtf)  
	undefined,//6  ?inactivf en V5 - Valeurs séparées par des virgules (CSV)  
	undefined,//7  ?inactivf en V5 - Valeurs séparées par des TAB  
	undefined,//8  ?inactivf en V5 - Valeurs séparées par un caractère  
	".txt",//9  Texte ASCII  
	".txt",//10  Texte ASCII paginé  
	undefined,//11  ?inactivf en V5 - Texte ASCII séparé par des TAB  
	undefined,//12  ?inactif depuis la version 136  
	undefined,//13  ?inactif depuis la version 136  
	undefined,//14  ?inactif depuis la version 136  
	undefined,//15  ?inactif depuis la version 136  
	undefined,//16  ?inactif depuis la version 136  
	undefined,//17  ?inactif depuis la version 136  
	undefined,//18  ?inactif depuis la version 136  
	undefined,//19  ?inactif depuis la version 136  
	undefined,//20  ?inactif 140 ODBC  
	undefined,//21  ?inactif 140 HTML 3.2  
	undefined,//22  ?inactif depuis la version 136  
	undefined,//23  ?inactif depuis la version 136  
	undefined,//24  ?HTML 4 non supporté par Crystal XI  
	undefined,//25  ?inactif depuis la version 136  
	undefined,//26  ?inactif depuis la version 136  
	".xls",//27  Microsoft Excel 97-2000 (*.xls)  
	".xls",//28  Microsoft Excel 97-2000 données uniquement (*.xls)  
	".pdf",//29  Adobe Acrobat (*.pdf)      
];
function getMimeFromExport(exportFormat) {
	var ef = parseInt(exportFormat, 10);
	var ext = typesMap[ef];
	if (!ext) throw new Error("The export format ["+ef+"] is not supported.");
...
var PrintClient = exports.PrintClient = helpers.defineClass(function(_, hostname, port) {
	...
	launchReport: function(_, properties) {
		var seqId = this.communicator.send_ND_NDOUVIMP(_, properties);
		stack[seqId] = {
			jobId: properties.__wJobID,
			exportFormat: properties._FormatExport,
		};
		return seqId;
	},
	...
	getReport: function(_, jobId) {
		var res = this.communicator.send_ND_GETRESULTREPORT(_, jobId);
		var job = getStackedJobs()[jobId];
		if (!job) throw new Error("Job ["+jobId+"] was not found on stack");
		res.mimeType = getMimeFromExport(job.exportFormat);
		delete stack[job.seqId];
		return res;
	},
