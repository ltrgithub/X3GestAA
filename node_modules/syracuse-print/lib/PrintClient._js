...
var stack = {};
function getStackedJobs(host) {
	return stack[host].reduce(function(job, desc, index) {
		job[desc.jobId] = {
			seqId: index,
...
var PrintClient = exports.PrintClient = helpers.defineClass(function(_, hostname, port, standAlone) {
	...
	launchReport: function(_, properties, alreadyLaunched) {
		var seqId = !alreadyLaunched ? this.communicator.send_ND_NDOUVIMP(_, properties) : 0;
		var ef = parseInt(properties._FormatExport, 10), fn = (properties.__REPORT || "???").split(".");
