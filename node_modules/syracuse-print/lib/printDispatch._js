...
function _doIt(_, context) {
	try {
		...
		if (context.printClient)
			context.printClient.disconnect(_);
		var headers = {
			"content-type": "application/json"
		};
		if (ex.httpStatus) {
			return context.reply(_, ex.httpStatus, getErrorDiagnose(ex), headers);
		} else {
			return context.reply(_, 500, getErrorDiagnose(ex), headers);
		}
	}
}
