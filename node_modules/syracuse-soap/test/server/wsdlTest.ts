...
var wsdlGenerator = require('../../lib/server/wsdlGenerator');

var syrEntities = require('@sage/syracuse-lib/src/collaboration/contract').contract.entities;
...
describe(module.id, () => {
	...
	it('initialize syracuse test server', function(_) {
		testAdmin.initializeTestContract(_, testData, "unit_test_data");
	...
	function get(_, cookie, url, statusCode, headers) {
		...
		url = url.indexOf("http") === 0 ? url : baseUrl + "/sdata/qunit/sdataTest/unit_test_data/" + url;
	...
	function executeCompleteTest(entityName, representation, baseDatasetUrl) {
		...
		it(representation + ": Web service creation", function(_) {
			try {
		...
		//	
		it(representation + ": Get prototype", function(_) {
			try {
				if (baseDatasetUrl) {
					protoUrl = baseUrl + "/sdata" + baseDatasetUrl + "$prototypes('" + representation + ".$edit')";
				} else {
					protoUrl = baseUrl + "/sdata/qunit/sdataTest/unit_test_data/$prototypes('" + representation + ".$edit')";
				}
				cookie = getCookie(_, baseUrl);
		...
		it(representation + ": WSDL Generation", function(_) {
			try {
				if (baseDatasetUrl) {
		...
		it(representation + ": Check definitions", function(_) {
			try {
		...
		it(representation + ": Check messages", function(_) {
			try {
		...
		it(representation + ": Check portType", function(_) {
			try {
		...
		it(representation + ": Check binding", function(_) {
			try {
		...
		it(representation + ": Check service", function(_) {
			try {
		...
		it(representation + ": Check types", function(_) {
			try {
	...
	//
	executeCompleteTest("groups", "group", "/syracuse/collaboration/unit_test/");
	executeCompleteTest("users", "user", "/syracuse/collaboration/unit_test/");
	executeCompleteTest("endPoints", "endPoint", "/syracuse/collaboration/unit_test/");
	executeCompleteTest("documents", "document", "/syracuse/collaboration/unit_test/");
	//
	// X3 entities
	//
