...
function execCompleteTests(_, useProtocol){
	
	protocol = useProtocol;

	asyncTest("Protocol: " + protocol + " --> strings", 17, function(_){
		try{
	...
	asyncTest("Protocol: " + protocol + " --> integers", 22, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> reals", 16, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> dateTimes", 12, function(_){
		try{	
			...
			//console.log("COMPARE: "+body2.datetimeDefNow+" AND "+datetimeBefore.toString());
	...
	asyncTest("Protocol: " + protocol + " --> dates", 11, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> times", 12, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> uuids", 11, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> jsons", 10, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> parents", 71, function(_){
		try{		
	...
	asyncTest("Protocol: " + protocol + " --> paging", 36, function(_){
		...
		var body = postSoap(_, "pageds", "listPaged", {_count: "1", _pageInfo: "2", _orderBy: {orderBy:{binding: "name"}}});
		...
		var body = postSoap(_, "pageds", "listPaged", {_count: "3", _pageInfo: "1", _orderBy: {orderBy:{_binding: "name",_descending: "true"}}});
		...
		for (var i in body.pagedList.paged){
			var subbody = postSoap(_, "pageds", "deletePaged", {
	...
	asyncTest("Protocol: " + protocol + " --> delete test", 98, function(_) {
...
execCompleteTests(_, "REST");
execCompleteTests(_, "ORM");
