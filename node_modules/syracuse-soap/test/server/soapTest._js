...
function get(_, cookie, url, statusCode, headers) {
	...
	if (statusCode)
		strictEqual(response.statusCode, statusCode, "status verified");
	
	var resp = response.readAll(_);
	if (response.statusCode !== (statusCode || 200)){
		console.log("GET Request URL: "+ url);
		console.log("Bad response: "+JSON.stringify(resp,null,2));
	}
	try {
...
function addApplication(_, cookie, url, application, contract){
	...
	if (!appUuid[application]){
		...
		if (appBody.$resources && appBody.$resources[0] && appBody.$resources[0].$uuid){
			...
			//console.log("appBody="+JSON.stringify(appBody,null,2));
...
function addX3UserEndpoint(_, cookie, url, epUuid){
	...
	if (!x3UserEndpoint){
		...
		var epBody = put(_, cookie, url + "users('"+guestUuid+"')", {endpoints: [{
			login: "TC",
		    endpoint: {
		    	$uuid: epUuid
		    }
		...
		//console.log("BODY="+JSON.stringify(epBody,null,2));
...
function addWebService(_, application, contract, dataset, name, description, entities){
	...
	for (var ent in entities) {
		...
		var reps = [];
		for (var rep in entity.representations){
			var representation = entity.representations[rep];
		...
		// Create soapEntities
		var entBody = post(_, cookie, url + "soapEntities", {
			name: entity.name,
			title: entity.title,
			representations: reps
		});

		strictEqual(entBody.name, entity.name, "entity name test roundtrip");
		strictEqual(entBody.title, entity.title, "entity title test roundtrip");
		ents.push(entBody)	
	...
	//console.log("wsBody="+JSON.stringify(wsBody,null,2));
	strictEqual(wsBody.name, name, "web service name test roundtrip");
	strictEqual(wsBody.description, description, "web service description test roundtrip");
	...
	//console.log("savBody="+JSON.stringify(savBody,null,2));
...
function postSoap(_, service, soapAction, data, errorExpected){
	...
	var response = streams.httpRequest({
		method: "post",
		url: baseUrl + "/soap/"+application+"/"+contract+"/"+dataset+"/" + service,
		headers: {
	...
	if (errorExpected){
			if (result["soap:Envelope"]["soap:Body"]["soap:Fault"]){
				...
				return {fault: result["soap:Envelope"]["soap:Body"]["soap:Fault"].faultstring.$value.toString()};
			}else{
				return {fault: null};
			}
...
function createWebService(_, name, description, entities){
	...
	addWebService(_, application, contract, dataset, name, description, entities ? entities : def);
}
...
asyncTest("strings", 26, function(_){
	try{
		...
		var body2 = postSoap(_, "testStrings", "readString", {_key: body1._key}).string;
...
asyncTest("integers", 31, function(_){
	try{	
		...
		var body2 = postSoap(_, "testIntegers", "readInteger", {_key: body1._key}).integer;
...
asyncTest("reals", 25, function(_){
	try{		
		...
		var body2 = postSoap(_, "testReals", "readReal", {_key: body1._key}).real;
...
asyncTest("datetimes", 21, function(_){
	try{	
		...
		var body2 = postSoap(_, "testDatetimes", "readDatetime", {_key: body1._key}).datetime;
...
asyncTest("dates", 20, function(_){
	try{		
		...
		var body2 = postSoap(_, "testDates", "readDate", {_key: body1._key}).date;
...
asyncTest("times", 21, function(_){
	try{		
		...
		var body2 = postSoap(_, "testTimes", "readTime", {_key: body1._key}).time;
...
asyncTest("uuids", 20, function(_){
	try{		
		...
		var body2 = postSoap(_, "testUuids", "readUuid", {_key: body1._key}).uuid;
...
asyncTest("jsons", 19, function(_){
	try{		
		...
		var body2 = postSoap(_, "testJsons", "readJson", {_key: body1._key}).json;
...
asyncTest("parents", 92, function(_){
	try{		
		...
		var body = postSoap(_, "testMultiple", "createRefer", {
			name: "ref1"
		});
		var ref1 = body._key;
		...
		var body = postSoap(_, "testMultiple", "createAssociate", {
			name: "associate1"
		});
		...
		// CREATE
		var body = postSoap(_, "testMultiple", "createParent", {
			name: "parent1",
			children: { 
				child: [{
		...
		// READ
		var body = postSoap(_, "testMultiple", "readParent", {_key: parent}).parent;
		...
		// UPDATE
		var body = postSoap(_, "testMultiple", "updateParent", {
			_key: parentKey,
			name: "parent1",
			children: {
				_deleteMissing: true,
...
asyncTest("paging", 45, function(_){
	...
	var body = postSoap(_, "testPageds", "listPaged", {_count: "1", _orderBy: {orderBy:{binding: "name"}}});
	...
	var body = postSoap(_, "testPageds", "listPaged", {_count: "1", _pageInfo: "2", _orderBy: {orderBy:{binding: "name"}}});
	...
	var body = postSoap(_, "testPageds", "listPaged", {_count: "3", _pageInfo: "1", _orderBy: {orderBy:{_binding: "name",_descending: "true"}}});
...
asyncTest("delete test", 127, function(_) {

	var entities = [{
		name: "deleteTestParents", 
		title: "deleteTestParents", 
		representations: [{
			name: "deleteTestParent", 
			title: "deleteTestParent"
		}]},{
		name: "deleteTestRefOnes", 
		title: "deleteTestRefOnes", 
		representations: [{
			name: "deleteTestRefOne", 
			title: "deleteTestRefOne"
		}]},{
		name: "deleteTestRefManies", 
		title: "deleteTestRefManies", 
		representations: [{
			name: "deleteTestRefMany", 
			title: "deleteTestRefMany"
		}]},{
		name: "deleteTestMultiRefes", 
		title: "deleteTestMultiRefes", 
		representations: [{
			name: "deleteTestMultiRefe", 
			title: "deleteTestMultiRefe"
		}]},{
		name: "deleteTestCascadeMasters", 
		title: "deleteTestCascadeMasters", 
		representations: [{
			name: "deleteTestCascadeMaster", 
			title: "deleteTestCascadeMaster"
		}]},{
		name: "deleteTestCascadeDetails", 
		title: "deleteTestCascadeDetails", 
		representations: [{
