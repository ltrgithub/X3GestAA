...
describe(module.id, () => {
	...
	function addEndpoint(_, cookie, url, application, contract, dataset) {
		...
		if (!endpointUuid[application]) {
			...
			if (endpointBody.$resources && endpointBody.$resources[0] && endpointBody.$resources[0].$uuid) {
				...
				if (application === "x3") {
					addX3Solution(_, cookie, url, application);
					endPointData.x3solution = {
						$uuid: x3solutionUuid
					};
	...
	function addX3Solution(_, cookie, url, application) {

		if (!x3solutionUuid) {

			var x3solutionBody = post(_, cookie, url + "x3solutions", {
				code: "SUPDVLP",
				description: "SUPDVLP",
			...
			x3solutionUuid = x3solutionBody.$uuid;
	...
	function addWebService(_, application, contract, dataset, description) {
		...
		var url = baseUrl + "/sdata/syracuse/collaboration/unit_test/";
		cookie = getCookie(_);
	...
	function addPool(_, alias, size, endpoint, localePref, user) {
		...
		pool.x3serverTags(_, "MAIN");
		pool.user(_, user);
		var res = pool.save(_);
		//console.error("SAVE= " + JSON.stringify(res, null, 2));
		globals.context.request = globals.context.request || {};
		globals.context.request.headers = globals.context.request.headers || {};

		globals.context.request.headers.unittest = recMode + "~" + recFileName;
	...
	it('init collaboration', function(_) {
	...
	function checkStatus(value, expected, operationName) {
		strictEqual(parseInt(value.$value, 10), expected, operationName + " status [" + value.$value + "] OK");
	}
	...
	function checkProcessReport(techInfo) {
		ok(techInfo.processReport && techInfo.processReport.$cdata != null, "process report [" + techInfo.processReport && techInfo.processReport.$cdata + "] OK");
	}
	...
	it("Call generic ws operation 'run' on 'QLFWSADD'", function(_) {
...
<FLD NAME=\"R\" >0</FLD>\
		...
		//console.log("RETURN: " + JSON.stringify(body["wss:runResponse"].runReturn, null, 2));

		checkStatus(body["wss:runResponse"].runReturn.status, 1, "run");

		resultXml = jsxml.parse(body["wss:runResponse"].runReturn.resultXml.$cdata);
...
<FLD NAME=\"R\" >0</FLD>\
		...
		body = postSoap(_, "CAdxWebServiceXmlCC", "run", callParams, recFileName);
		checkStatus(body["wss:runResponse"].runReturn.status, 0, "run");
		messages = body["wss:runResponse"].runReturn.messages;
		checkMessage(messages.type, 3, messages.message, "Bad input XML parameters.");
		checkProcessReport(body["wss:runResponse"].runReturn.technicalInfos);

	});


	it("Call generic ws operation 'query' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
		...
		checkStatus(body["wss:queryResponse"].queryReturn.status, 1, "query");

		resultXml = jsxml.parse(body["wss:queryResponse"].queryReturn.resultXml.$cdata);
	...
	it("Call generic ws operation 'read' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
		...
		checkStatus(body["wss:readResponse"].readReturn.status, 1, "read");
		resultXml = jsxml.parse(body["wss:readResponse"].readReturn.resultXml.$cdata);
	...
	it("Call generic ws operation 'save' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
	...
	<TAB DIM=\"20\" ID=\"AUS5_2\" SIZE=\"2\">\
		...
		checkStatus(body["wss:saveResponse"].saveReturn.status, 1, "read");
		resultXml = jsxml.parse(body["wss:saveResponse"].saveReturn.resultXml.$cdata);
	...
	it("Call generic ws operation 'modify' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
		var body, resultXml, messages;
		// Tests read
...
<LIN NUM=\"3\">\
	...
	<FLD NAME=\"BPR\" TYPE=\"Char\">TESTX3</FLD>\
		...
		checkStatus(body["wss:modifyResponse"].modifyReturn.status, 1, "delete");
		resultXml = jsxml.parse(body["wss:modifyResponse"].modifyReturn.resultXml.$cdata);
		var tab5_2 = resultXml.RESULT.TAB[1];
		strictEqual(tab5_2.LIN.length, 3, "Tab 5_2 lines count OK [" + tab5_2.LIN.length + "]");
		strictEqual(tab5_2.LIN[0].FLD[0].$value, "AQC", "Line 0 ROL OK [" + tab5_2.LIN[0].FLD[0].$value + "]");
		strictEqual(tab5_2.LIN[0].FLD[1].$value, "Tests unitaires", "Line 0 ZROL OK [" + tab5_2.LIN[0].FLD[1].$value + "]");
	...
	it("Call generic ws operation 'deleteLines' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
		...
		checkStatus(body["wss:deleteLinesResponse"].deleteLinesReturn.status, 1, "delete");
		resultXml = jsxml.parse(body["wss:deleteLinesResponse"].deleteLinesReturn.resultXml.$cdata);
		var tab5_2 = resultXml.RESULT.TAB[1];
	...
	it("Call generic ws operation 'delete' on 'OAUS'", function(_) {
		// force new cookie
		cookie = getCookie(_, true);
		var body, resultXml, messages;
		// Tests read
		...
		//console.error("BODY2: " + JSON.stringify(body, null, 2));
		checkStatus(body["wss:deleteResponse"].deleteReturn.status, 1, "delete");
		resultXml = jsxml.parse(body["wss:deleteResponse"].deleteReturn.resultXml.$cdata);
	});

	it('cleanup', function(_) {
		if (x3MockServer) {
			try { x3MockServer.close(_); } catch(e) {};
		}
	});
});