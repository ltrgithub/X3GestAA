...
function addWebService(_, cookie, application, contract, dataset, name, description, entities){
...
function executeCompleteTest(entityName, representation, baseDatasetUrl){
		...
		function checkComplexType(complexType){
			...
			if (name.indexOf("read") === 0){
				if (name.indexOf("Request") !== -1){
					...
					strictEqual(element.$.type, representation + ":" + entityName + "--type", "ComplexType element[0] type ["+element.$.type+"] ok");
				}
			}
				...
				if (name.indexOf("Request") !== -1){
					...
					strictEqual(element.$.type, representation + ":" + entityName + "--type", "ComplexType element type ["+element.$.type+"] ok");
				}else if (name.indexOf("Response") !== -1){
				...
				if (name.indexOf("Request") !== -1){
					...
					strictEqual(element[7].$.type, representation + ":" + entityName + "--list", "ComplexType element type ["+element[3].$.type+"] ok");
				}
			}
					...
					if (cp.$.name === name){
						...
						for (var elt in elements){
							...
							if (elt.$ && elt.$.type && elt.$.type.indexOf(representation + ":") === 0){
								...
								if (!valid){
									trace("Simple type invalid: "+ JSON.stringify(elt,null,2));
									return false;
								}
								...
								trace("Element: '"+elt.$.name+"' has no valid type !");
								// SCHEMA INVALID
								return false;
							}
						}
	...
	asyncTest(representation+": Web service creation", 9, function(_){
		try{
...
//	
	...
	asyncTest(representation+": WSDL Generation", 2, function(_){
		try{
			if (baseDatasetUrl){
				serviceUrl = baseUrl+ "/soap" + baseDatasetUrl + representation;
			}else{
				serviceUrl = baseUrl + "/soap/"+application+"/"+contract+"/"+dataset+"/" + representation;
			}
			cookie = getCookie(_, baseUrl);
