	...
	function checkComplexType(complexType){
		...
		if (name.indexOf("read") === 0){
			...
			if (name.indexOf("Request") !== -1){
				...
				strictEqual(element[3].$.type, "tns:orderBy--list", "ComplexType element type ["+element[3].$.type+"] ok");
				...
				strictEqual(element[7].$.type, "tns:" + entityName + "--list", "ComplexType element type ["+element[3].$.type+"] ok");
			}
		}
			...
			for (var cp in complexTypes){
				...
				if (cp.$.name === name){
					...
					for (var elt in elements){
						...
						if (elt.$ && elt.$.type && elt.$.type.indexOf("tns:") === 0){
...
function executeCompleteTest(entityName, baseHostUrl, baseDatasetUrl){
	...
	asyncTest(entityName+": Check types", 80, function(_){
		try{
