	...
	function replaceRefs(obj, parentKey) {
		...
		if (typeof obj === "object") {
			...
			if (obj.$ && Object.keys(obj.$).some(function(k) {
				return k === "soapenc:arrayType" || obj.$[k] === "soapenc:Array";
			})) {
				isArrayType = true;
			}
...
exports.translateResponse = function(soapBody, protoResp, useSoapStyle) {
	...
	function replaceArray(_key, _array, _property, _parent) {
		...
		if (_parent[_key].$ && _property.$soapType) {
			_parent[_key].$["soapenc:arrayType"] = "wss:" + _property.$soapType + "[" + (Object.keys(_parent[_key]).length - 1) + "]";
		}
	...
	function parseObject(obj) {
		forEachKey(obj, function(_key, _value) {
			if (Array.isArray(_value) && _value.length > 0) {
				if (useSoapStyle) replaceArray(_key, _value, protoResp[_key], obj);
...
exports.getNamespacePrefix = function(envelope, url, strict) {
	var xmlns = "xmlns:";
	for (var key in envelope.$) {
		if (key.indexOf(xmlns) === 0 && envelope.$[key].indexOf(url) === 0) return key.substring(xmlns.length);
	}
