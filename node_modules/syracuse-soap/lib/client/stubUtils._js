...
exports.getSoapStub = function(_, stubName, context, isIncomingChild){
	...
	if (isIncomingChild){
		...
		if (instances.length > 1)
			throw context.notFound(locale.format(module, "severalStubs", stubName));
	
		if (instances.length !== 1)
			throw context.notFound(locale.format(module, "stubNotFound", stubName));
...
exports.convertValue = function(name, type, value){
	...
	try{
		...
		throw new Error(locale.format(module, "convertValueErr", util.inspect(value), type, e.stack));
