...
var stubHelper = helpers.defineClass(function(_, name, wsdl, url, options) {
	...
	parseWsdl: function(_){
		...
		function addPort(_, port){
			...
			function addOperation(_, bindingOperation){
				...
				function createPrototype(_, msgName, store, isInput){
					...
					function buildProperty(name, type, constraints, schema){
						...
						function handleElement(elt, nature){
							
							function defineProperty(aElt, aProp){

								var splited = splitNs(aElt.$.type);
								var eltNs = splited.ns;
								var eltType = splited.type;

								setProperty(aProp, aElt.$.name, eltNs, eltType, aElt.$);
							}
							
							function handleSequence(sequence){
								var subElements = getSubNode(sequence, xsdNs, _element);
								if (Array.isArray(subElements)){
									subElements.forEach(function(subElement){
										defineProperty(subElement, prop);
									});
								}else if (subElements){
									defineProperty(subElements, prop);
								}
							}
							...
							function handleRestriction(restriction){
								...
								function handleEnums(){
									var enums = getSubNode(restriction, xsdNs, _enumeration);
									// Handle enumerations
									if (enums){
										console.log("Enum OK");
							...
							if (elt.$.type){
								...
								if (nature === _complexType){
									...
									var seq = getSubNode(elt, xsdNs, _sequence);
									if (seq){
										handleSequence(seq);
									}else{
										throw new Error("Sequence not found for element: "+elt.$.name);
									}

									
									
								}else if (nature === _simpleType){
									console.log("TODO: simpleTypes");
									
									var restrict = getSubNode(elt, xsdNs, _restriction);
									if (restrict){
										handleRestriction(restrict);
									}else{
										throw new Error("Restrictions not found for element: "+elt.$.name);
									}
									

								}else if (nature === _element){
									var complexT = getSubNode(elt, xsdNs, _complexType);
									var seq = getSubNode(complexT, xsdNs, "sequence");
									if (seq){
										handleSequence(seq);
									}else{
										throw new Error("Sequence not found for element: "+elt.$.name);
						...
						if (schema){
							...
							if (constraints.maxOccurs && constraints.maxOccurs !== "1"){
					...
					function setProperty(proto, name, ns, type, constraints){
					...
					function readMsgPart(part){
						...
						if (partType){
							...
							setProperty(proto, name, partNs, partType, part.$);
				...
				// Set URL of Tester
				operationInstance.tester(_, globals.context.session.host + "/syracuse-main/html/main.html?url=/soap-stubs/syracuse/collaboration/syracuse/" + self.name + "/" + port.$.name + "/" + operationName + "?representation=" + operationName + ".$request");
			...
			}else if (protocol.indexOf(__http) === 0 || protocol.indexOf(__http2) === 0){
