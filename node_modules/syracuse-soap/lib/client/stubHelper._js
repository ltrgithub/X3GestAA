...
var soapUtils = require('syracuse-soap/lib/server/soapUtils');
			...
			function addOperation(_, bindingOperation) {
				...
				function createPrototype(_, content, store, facet) {
					...
					function buildProperty(name, element, nature, constraints, schema) {
						...
						tracer.trace("-> Parameters : " + JSON.stringify(params, null, 2));
						...
						function defineProperty(aElt, aProp, aNature, isEmbedded) {
							...
							if (aElt.$ && aElt) {
								...
								if (aElt.$.type) {
									...
									tracer.trace("---> Define property ref: " + aElt.$.ref);
									aType = aElt.$.ref;
								}
								var splited = splitNs(aType);
								var eltNs = splited && splited.ns;
								var eltType = splited && splited.type;
						...
						function handleSequence(sequence) {
							...
							if (typeof sequence === 'object') forEachKey(sequence, function(key, value) {
								if (key === xsdPrefix + _element || key === xsdNs + ":" + _element) {
						...
						function handleRestriction(restriction) {
							...
							if (typeof restriction === 'object') forEachKey(restriction, function(key, value) {
								if (key === xsdPrefix + _enumeration || key === xsdNs + ":" + _enumeration) {
									// Handle enumerations
								...
								// Handle pattern
								else if (key === xsdPrefix + _pattern || key === xsdNs + ":" + _pattern) {
									prop.$pattern = value;
								}
								// Handle length
								else if (key === xsdPrefix + _length || key === xsdNs + ":" + _length) {
									var len = parseInt(value, 10);
									prop.$minLength = len;
									prop.$maxLength = len;
								}
								// Handle minLength
								else if (key === xsdPrefix + _minLength || key === xsdNs + ":" + _minLength) {
									prop.$minLength = parseInt(value, 10);
								}
								// Handle maxLength
								else if (key === xsdPrefix + _maxLength || key === xsdNs + ":" + _maxLength) {
									prop.$maxLength = parseInt(value, 10);
								}
								// Handle whiteSpace
								else if (key === xsdPrefix + _whiteSpace || key === xsdNs + ":" + _whiteSpace) {
									// TODO
									nyi(key);
								}
								// Handle minInclusive
								else if (key === xsdPrefix + _minInclusive || key === xsdNs + ":" + _minInclusive) {
									prop.$minimum = parseInt(value, 10);
									prop.$exclusiveMinimum = false;
								}
								// Handle maxInclusive
								else if (key === xsdPrefix + _maxInclusive || key === xsdNs + ":" + _maxInclusive) {
									prop.$maximum = parseInt(value, 10);
									prop.$exclusiveMaximum = false;
								}
								// Handle minExclusive
								else if (key === xsdPrefix + _minExclusive || key === xsdNs + ":" + _minExclusive) {
									prop.$minimum = parseInt(value, 10);
									prop.$exclusiveMinimum = true;
								}
								// Handle maxExclusive
								else if (key === xsdPrefix + _maxExclusive || key === xsdNs + ":" + _maxExclusive) {
									prop.$maximum = parseInt(value, 10);
									prop.$exclusiveMaximum = true;
								}
								// Handle fractionDigits
								else if (key === xsdPrefix + _fractionDigits || key === xsdNs + ":" + _fractionDigits) {
									format = "0.";
								...
								// Handle totalDigits
								else if (key === xsdPrefix + _totalDigits || key === xsdNs + ":" + _totalDigits) {
								...
								// Handle attribute
								else if (key === xsdPrefix + _attribute || key === xsdNs + ":" + _attribute) {
									handleAttribute(value);
								}
								// take care about attributes
								else if (key === '$') {
									if (value.base) {
										prop.$type = convertType(value.base);
										delete prop.$item;
									}
								} else {
									nyi(key);
						...
						function handleComplexContent(_cc) {
							...
							if (typeof _cc === 'object') forEachKey(_cc, function(key, value) {
								if (key === xsdPrefix + _annotation || key === xsdNs + ":" + _annotation) {
									nyi(key, element);
								} else if (key === xsdPrefix + _restriction || key === xsdNs + ":" + _restriction) {
									handleRestriction(value);
								} else if (key === xsdPrefix + _extension || key === xsdNs + ":" + _extension) {
									nyi(key);
								}
							});
						...
						function handleComplexType(_cp) {
							...
							if (typeof _cp === 'object') forEachKey(_cp, function(key, value) {
								...
								if (key === xsdPrefix + _annotation || key === xsdNs + ":" + _annotation) {
									// TODO
									nyi(key, element);
								} else if (key === xsdPrefix + _simpleContent || key === xsdNs + ":" + _simpleContent) {
									// TODO
									nyi(key);
								} else if (key === xsdPrefix + _complexContent || key === xsdNs + ":" + _complexContent) {
									handleComplexContent(value);
								} else if (key === xsdPrefix + _group || key === xsdNs + ":" + _group) {
									// TODO
									nyi(key);
								} else if (key === xsdPrefix + _all || key === xsdNs + ":" + _all) {
									// TODO
									nyi(key);
								} else if (key === xsdPrefix + _choice || key === xsdNs + ":" + _choice) {
									// TODO
									nyi(key);
								} else if (key === xsdPrefix + _sequence || key === xsdNs + ":" + _sequence) {
								...
								} else if (key === xsdPrefix + _attributeGroup || key === xsdNs + ":" + _attributeGroup) {
									// TODO
									nyi(key);
								} else if (key === xsdPrefix + _anyAttribute || key === xsdNs + ":" + _anyAttribute) {
									// TODO
									nyi(key);
								} else if (key !== '$') {
									nyi(key);
						...
						function handleSimpleType(_sp) {
							...
							if (typeof _sp === 'object') forEachKey(_sp, function(key, value) {
								if (key === xsdPrefix + _restriction || key === xsdNs + ":" + _restriction) {
									handleRestriction(value);
								} else if (key !== '$') {
									nyi(key);
								}
							...
							if (typeof _att === 'object') {
								if (Object.keys(_att).length === 1 && Object.keys(_att)[0] === "$") {
									...
									forEachKey(_att, function(key, value) {
										if (key === xsdPrefix + _simpleType || key === xsdNs + ":" + _simpleType) {
											handleSimpleType(value);
										} else if (key === xsdPrefix + _annotation || key === xsdNs + ":" + _annotation) {
											// TODO
											nyi(key);
										} else if (key !== '$') {
											nyi(key);
							...
							if (typeof _elt === 'object') {
								...
								if (Object.keys(_elt).length === 1 && Object.keys(_elt)[0] === "$") {
									...
									forEachKey(_elt, function(key, value) {
										if (key === xsdPrefix + _complexType || key === xsdNs + ":" + _complexType) {
											handleComplexType(value);
										} else if (key === xsdPrefix + _simpleType || key === xsdNs + ":" + _simpleType) {
											handleSimpleType(value);
										} else if (key !== '$') {
											nyi(key);
										}
					...
					function setProperty(prop, element, ns, nature) {
						//tracer.traceSubSection("-> Set property :",JSON.stringify(element,null,2));
						tracer.traceSubSection("-> Set property");
				...
				// Set URL of Tester
				operationInstance.tester(_, soapUtils.makeTesterSoap("/" + self.dispatcher + "/syracuse/collaboration/syracuse/" + self.name + "/" + port.$.name + "/" + operationName + "?representation=" + operationName + ".$request"));
