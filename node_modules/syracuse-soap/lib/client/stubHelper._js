	...
	parseWsdl: function(_){
		...
		function addPort(_, port){
			...
			function addOperation(_, bindingOperation){
				...
				var portTypeOperation = getPortTypeOperation(name);
				tracer.trace(" - Add Operation: "+name);
				
				var entity = model.getEntity("soapStubOperation");
				var operationInstance = entity.factory.createInstance(_, null, db);
				
				operationInstance.name(_, name);
				...
				operationInstance.input(_, getSubNode(portTypeOperation, wsdlNs, 'input').$.message);
				operationInstance.output(_, getSubNode(portTypeOperation, wsdlNs, 'output').$.message);
			...
			var entity = model.getEntity("soapStubPort");
			var portInstance = entity.factory.createInstance(_, null, db);
