		...
		function addPort(_, port){
			...
			function addOperation(_, bindingOperation){
				...
				function createPrototype(_, msgName, store, isInput){
					...
					function setProperty(proto, element, ns, nature){
					...
					function readMsgPart(part){
						...
						if (part.$.type){
							...
							if (xsdComplexType){
								setProperty(proto, xsdComplexType, partNs, _complexType);
							}else{
								var xsdSimpleType = getSimpleType(sc, partType);
								if (xsdSimpleType){
									setProperty(proto, xsdSimpleType, partNs, _simpleType);
								}else{
									throw new Error("XSD type '"+partType+"' not found.");
								}
							}
							...
							if (xsdElement){
								setProperty(proto, xsdElement, partNs, _element);
							}else{
