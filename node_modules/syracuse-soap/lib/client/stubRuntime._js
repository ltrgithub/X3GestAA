...
var Runtime = helpers.defineClass(function(_, stubName, portName, operationName, data, context, stubType) {
	this.data = data;
	...
	execute: function(_, context) {
		...
		function send(_, envelope) {
			...
			if (isChild && syracuse.server instanceof mock.MockStreamServer) {
				...
				opt.port = context.request.connection.localPort;
				response = config.mockServer.mockClient.simpleRequest(opt, envelope, _);
				console.error("RES: " + JSON.stringify(response, null, 2));
				return {
					statusCode: 200,
					body: response
				};
			...
			if (resp.statusCode === 200 && resp.body) {
				...
				try {
					...
					if (r && r.$diagnoses && r.$diagnoses[0]) {
						throw new Error(r.$diagnoses[0].$message);
					} else {
...
exports.execute = function(_, stubName, portName, operationName, data, context, stubType) {
	var response;
	var runtime = new Runtime(_, stubName, portName, operationName, data, context, stubType);
