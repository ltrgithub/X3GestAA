...
var Runtime = helpers.defineClass(function(_, stubName, portName, operationName, data, context, stubType, localizations) {
	...
	execute: function(_, context) {
		...
		function send(_, envelope) {
			...
			location = soapUtils.makeTesterSoap(location);
			var opt = {
			...
			if (isChild && syracuse.server instanceof mock.MockStreamServer) {
				...
				opt.headers.host = context.request.headers['x-forwarded-host'] || context.request.headers.host;
				opt.connection = {
			...
			if (prototype.$type === "application/x-object") {
				forEachKey(prototype.$item.$properties, function(key, value) {
					...
					if (value.$type === "application/x-object") {
						...
						if (subTarget.length !== 0) target[prefix ? prefix + ":" + key : key] = subTarget;
					} else {
						val = data && data[key];
						...
						if (val) {
							...
							var k = prefix ? prefix + ":" + key : key;
							target[k] = {};
							target[k].$value = val.toString();
						}
					}
				});

			} else if (prototype.$type === "application/x-array") {
				tracer.trace("Ignore field: " + name + " - " + locale.format(module, "rootArrayNYI"));
		...
		function buildResponseProperty(properties, name, node, ns) {
			...
			if (typeof node === 'object' && Object.keys(node).length !== 0) {
				...
				forEachKey(obj, function(key, value) {
					if (value !== '$') {
						var nodeName = value.indexOf(':') !== -1 ? value.split(':')[1] : value;
							...
							var handleArray = function(aVal, aTarget) {
								...
								if (typeof aVal === 'object') {
									...
									forEachKey(aVal, function(k, nod) {
										var subNodeName = k.indexOf(':') !== -1 ? k.split(':')[1] : k;
		...
		timer.start();
		var protoInData = this.data && this.data.header && this.data.header.prototype;
		var proto = this.requestPrototype.$properties.header || protoInData;

		if (proto) {
			calcNamespace(proto.$xmlns);
			var header = protoInData ? (this.data.header.data || {}) : this.data && this.data.header;
			envelope["soap:Envelope"]["soap:Header"] = buildRequestProperty('header', proto, header, true);
		}
