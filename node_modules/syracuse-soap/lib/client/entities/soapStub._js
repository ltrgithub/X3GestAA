...
var stubHelper = require('../stubHelper');
var genericStubHelper = require('../../generic/stubHelper');
	...
	$properties: {
		...
		type: {
			...
			$enum: [{
				...
				$value: "standard-in",
				$title: "Standard incoming tester"
			}, {
				$value: "generic-in",
				$title: "Generic incoming tester"
			}],
			$isMandatory: true,
			$default: "stub",
		},
	...
	$functions: {
		...
		$generate: function(_, instance) {
			...
			if (instance.$diagnoses.length === 0) {
				...
				// Case of generic web services.
				// We duplicated the code to not be dependant
				if (instance.type(_) === "generic-in") {
					genericStubHelper.create(_, instance, wsdl);
				} else {
