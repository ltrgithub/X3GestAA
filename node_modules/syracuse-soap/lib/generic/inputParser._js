...
exports.InputParserXml = helpers.defineClass(function(wsCall) {
	...
	parse: function() {
		...
		if (this.wsCall.report.errors.length > 0) {
			...
			throw new CvgWsException(locale.format(module, "cantLoadInputXml", "XML"));
		}
		...
		grps && grps.forEach(function(grp) { // check if the group exists in the
			...
			if (descrGrp) {
				if (descrGrp.$.DIM && descrGrp.$.DIM > 1) {
					self.wsCall.report.addError(locale.format(module, "grpDimGt", descrGrp.$.NAM));
				}
	...
	_checkTables: function(paramNode) {
		...
		tabs && tabs.forEach(function(tab) {
			...
			if (descrGrp) {
				if (descrGrp.$.DIM && descrGrp.$.DIM === 1) {
					self.wsCall.report.addError(locale.format(module, "tabDimEq", descrGrp.$.NAM));
				}
...
exports.InputParserJson = helpers.defineClass(function() {
	InputParser.call(this);
