...
var CvgOutputBuilderXml = outputBuilder.CvgOutputBuilderXml;
var CvgOutputBuilderJson = outputBuilder.CvgOutputBuilderJson;
...
exports.WsCall = helpers.defineClass(function(techInfo, lang, params, requestConfig) {
	...
	_computeResult: function(params) {
		...
		function getParamFieldOutputXml(resultParamMap) {
			...
			var outputBuilder, output, beautify;
			if (self.requestConfig["adxwss.optreturn"] && self.requestConfig["adxwss.optreturn"].toLowerCase() === "json") {
				outputBuilder = new CvgOutputBuilderJson(self.description, self.lang, resultParamMap);
				beautify = self.requestConfig["adxwss.beautify"] === "true";
				output = outputBuilder.build(self.action);
				return beautify ? JSON.stringify(output, null, "\t") : JSON.stringify(output);
			} else {
				outputBuilder = new CvgOutputBuilderXml(self.description, self.lang, resultParamMap);
				output = outputBuilder.build(self.action);
				var opt = {};
				beautify = self.requestConfig["adxwss.beautify"] === "true";
