...
var _entityMap = {
	post: function(_, context) {
		...
		var index = serviceName.indexOf('?'); // remove query parameter
		if (index >= 0) serviceName = serviceName.substr(0, index);
		// All real ws calls are intercepted here !!!
		var result = executor.intercept(_, serviceName, context);
		var head = Object.keys(config.security.cors).reduce(function(prev, curr) {
		...
		context.response.writeHead(200, head);
...
var _portMap = {
	...
	get: function(_, context) {
		...
		var index = portName.indexOf('?'); // remove query parameter
		if (index >= 0) portName = portName.substr(0, index);
...
var _operationMap = {
	...
	get: function(_, context) {
		...
		operationName = soapUtils.removeTesterSoap(operationName);
...
function _dispatch(_, context, map) {
	...
	if (seg == null) {
		...
		if (map[method]) return map[method](_, context);
			...
			console.error("Bad method (" + context.path + "): " + context.method) //TODO better logging;
...
exports.dispatcher = function(config) {
	return function(_, request, response) {
		...
		}, soapUtils.notTesterSoap(request));
