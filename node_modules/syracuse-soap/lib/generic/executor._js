...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	intercept: function(_, context) {
		...
		//console.error("wsCall.context.pool.metadata: "+JSON.stringify(wsCall.context.pool.metadata,null,2));
		var limitNotReached = licenseCounter.step(_, bufResp.length, wsCall.context.pool.metadata);
