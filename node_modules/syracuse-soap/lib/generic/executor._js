...
var WsCall = require('./wsCall').WsCall;
var check = require('@sage/syracuse-lib/src/license/check');
var tracer = require('@sage/syracuse-core').getTracer("soap-generic.request");
...
function validateProperties(properties, params) {
	function validateProperty(_property, _param, key) {
		...
		if (_property.$type === "application/x-object") {
			...
			var _itm;
			if (_param) {
				var pKeys = Object.keys(_param);
				if (pKeys.length === 1) {
					_itm = _param[pKeys[0]];
				} else if (pKeys.length === 2) {
					_itm = [];
					var arrKeys = typeof _param[pKeys[0]] === "object" ? _param[pKeys[0]] : [_param[pKeys[0]]];
					var arrValues = typeof _param[pKeys[1]] === "object" ? _param[pKeys[1]] : [_param[pKeys[1]]];
...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	intercept: function(_, context) {
		var pool, wsClient, response, responseRoot, useSoapStyle, wsCall, responseProperties, responseNodeName, responseNodeNameNs, returnNodeName;
		try {
			wsCall = new WsCall();
			// start total timer
			...
			responseNodeNameNs = "wss:" + responseNodeName;
			responseRoot = {};
			responseRoot[responseNodeNameNs] = {
				$: {
			...
			pool = poolManager.getPool(poolAlias);
			if (!pool) {
				// this can return true only in multi tenant mode
				var succesStart = poolManager.tryInitializePool(_, poolAlias);
				if (succesStart) {
					pool = poolManager.getPool(poolAlias);
					if (!pool) throw new Error("No Pool: " + poolAlias);
				} else {
					throw new Error("No Pool: " + poolAlias);
				}
			}
			...
			wsClient = pool.alloc(_, context, wsCall);
			...
			//
			//
			response = responseRoot[responseNodeNameNs][returnNodeName] = respMap[this.serviceName](wsCall);
		} catch (e) {
			...
			response = responseRoot[responseNodeNameNs][returnNodeName] = respMap[this.serviceName](wsCall);
			if (wsClient) {
				console.error("Set free");
				wsClient.setFree(wsCall.context);
			}
		}

		response.$ = response.$ || {};
		response.$["xsi:type"] = "wss:CAdxResultXml";
