...
// scoring system
function getMatchingClient(endpoint, user, lang) {
	var client = WSCLIENTS.filter_(_, function(_, cli) {
...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	_setCallContext: function(_, callContext) {
		...
		if (callContext) {
			...
			throw new Error("noCallContext!#!callContext parameter is mandatory");
		}
	},
	...
	execute: function(_, context, method, params) {
		...
		var wsClient = findWsClient(_, this.endpoint, this.user, this.lang, this.localePref, this.context);
		//		wsClient.execWS(_, {
		//			user: this.user,
		//			lang: this.lang
		//		}, transformed);
