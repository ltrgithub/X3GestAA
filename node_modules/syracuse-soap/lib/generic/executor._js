...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	intercept: function(_, context) {
		...
		if (wsCall.context && wsCall.context.poolMetadata) {
			console.error("wsCall.context.poolMetadata: " + JSON.stringify(wsCall.context.poolMetadata, null, 2));
			var limitNotReached = licenseCounter.step(_, bufResp.length, wsCall.context.poolMetadata);
		}
		return bufResp;
	},
