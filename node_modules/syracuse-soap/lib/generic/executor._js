...
var licenseCounter = require('syracuse-soap/lib/generic/licenseCounter');
...
function canReadInstance(_, context, instance) {
	var sp = context.httpSession && context.httpSession.getSecurityProfile(_);
	if (sp) {
...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	intercept: function(_, context) {
		...
		try {
			...
			console.error("TEST1: " + JSON.stringify(reqBody, null, 2));
			...
			console.error("TEST2: " + JSON.stringify(opRequest, null, 2));
		...
		if (wsCall.context && wsCall.context.poolMetadata) {
			var licenseCheck = licenseCounter.step(_, bufResp.length, wsCall.context.poolMetadata);
