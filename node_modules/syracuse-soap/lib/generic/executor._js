...
var callMap = {
	CAdxWebServiceXmlCC: {
		"run": function(action, params, response) {
			...
			return {
				name: params.publicName.$value,
				flow: params.inputXml.$value || params.inputXml.$cdata,
				action: action
			};
		},
...
var AckcallExecutor = helpers.defineClass(function(_, context, serviceName) {
	...
	_setCallContext: function(_, callContext) {
		...
		if (callContext) {
			...
			var epName = callContext.poolAlias.$value;
			...
			if (callContext.codeLang) {
				var x3lang = callContext.codeLang.$value;
