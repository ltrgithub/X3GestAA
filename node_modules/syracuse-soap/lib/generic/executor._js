...
function validateProperties(properties, params) {
			...
			if (_param.item) {
				if (Array.isArray(_param.item)) {
					_param.item.forEach(function(it) {
						arr.push(validateProperties(_property.$item.$properties, it));
					});
				} else {
					arr.push(validateProperties(_property.$item.$properties, _param.item));
				}
			}
			return arr;
		} else {
	...
	forEachKey(properties, function(key, value) {
		result[key] = validateProperty(value, params[key]);
	});
	return result;
}
...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	execute: function(_, context, method, params, techInfo) {
		...
		// Execute ackcall
		var res = wsClient.execWS(_, context, transformed, techInfo, true, 2);
		tracer.debug && tracer.debug("# Ack call result : " + JSON.stringify(res, null, 2));
		response.xmlResult = {
			$cdata: res
		};
		return response;
