...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	execute: function(_, context, method, params, techInfo) {
		...
		var responseNodeName = Object.keys(this.respProto[method].$properties.body.$item.$properties)[0];
		var returnNodeName = Object.keys(this.respProto[method].$properties.body.$item.$properties[responseNodeName].$item.$properties)[0];
		...
		var clientManager = cliManager.getManager(context.httpSession.genericWsCtx.dataset);
		if (!clientManager) {
			clientManager = new ClientManager({
				dataset: context.httpSession.genericWsCtx.dataset,
				poolSize: 50
			});
		}


		var wsClient = clientManager.alloc(_, context);
		techInfo.poolDistribDuration.stop();

		// Execute ackcall
		var wsCall = wsClient.execWs(_, context, transformed, techInfo, options);
