...
var AckcallExecutor = helpers.defineClass(function(serviceName) {
	...
	execute: function(_, context, method, params, wsCall) {
		...
		// Check if license authorize this call
		this.isCountLimitReached(_, pool.metadata);
		...
		// return the pool metadata to be able to retrieve information like licence data for instance
	...
	intercept: function(_, context) {
		...
		try {
			...
			// execute request
			poolMetadata = this.execute(_, context, opRequestName, opRequest[opRequestName], wsCall);
		...
		// WE SHOULD INCREASE COUNT HERE !!
		// soapResponse is the real response, so we can compute the size if needed...


		return soapResponse;
	},
	isCountLimitReached: function(_, poolMetadata) {
