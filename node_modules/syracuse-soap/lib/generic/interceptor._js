...
var Interceptor = helpers.defineClass(function(serviceName, context) {
	...
	run: function(_, context, envelope) {
		...
		var useSoapStyle = reqBody.indexOf("encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"") !== -1;
		...
		var soapRequest = soapTools.translateRequest(reqBody, useSoapStyle);
		...
		if (useSoapStyle) envelope = soapTools.translateResponse(envelope);

		// set technical informations in response flow
		responseRoot[responseNodeName][returnNodeName].technicalInfos = wsCall.techInfo.stringify();
