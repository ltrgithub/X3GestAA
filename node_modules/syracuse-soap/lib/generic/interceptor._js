...
function removeNamespaces(node) {
	...
	forEachKey(node, function(key, value) {
		if (key.indexOf(':') !== -1) {
			...
			obj[keyParts[1]] = typeof value === "object" ? removeNamespaces(value) : value;
		} else {
			obj[key] = typeof value === "object" ? removeNamespaces(value) : value;
		}
	});
	return obj;
}
...
var Interceptor = helpers.defineClass(function(serviceName, context) {
	...
	run: function(_, context, envelope, techInfo) {
		...
		console.log("BEFORE: " + JSON.stringify(soapBody, null, 2));
		var opRequest = removeNamespaces(soapBody);
		console.log("AFTER: " + JSON.stringify(opRequest, null, 2));

		var opRequestName = Object.keys(opRequest)[0];
		console.log("opRequestName: " + JSON.stringify(opRequestName, null, 2));
		// remove namespaces
		...
		// execute request
		var response = this.executor.execute(_, context, opRequestName, opRequest[opRequestName], techInfo);
