...
exports.createClient = function(_, context) {
	...
	if (context && context.request && context.request.headers && context.request.headers.unittest) { // for unit test 
		var recOptions = {
			...
			fileName: "genericSoapTest_UNIT.json"
		};
		if (recOptions.recMode === "REC") recOptions.overwrite = true; // always override file
		console.error("UNIT TEST INSTANCIATE CLIENT\n\n\n");
		newCli = new CvgClientWS().init(_, context, recOptions);
	} else {
		newCli = new CvgClientWS().init(_, context);
	}
...
exports.disconnect = function(uuid) {
	if (clients[uuid]) {
		tracer.info && tracer.info("Client destroyed: " + uuid);
		delete clients[uuid];
	}
};