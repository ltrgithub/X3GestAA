...
var tracer = require('@sage/syracuse-core').getTracer("soap-generic.stub");
...
var plainTextProperties = ["objectXml", "lineXml", "inputXml", "traceRequest", "processReport"];
var xmlProperties = ["resultXml"];

function getForcedType(elt) {
	if (plainTextProperties.indexOf(elt && elt.$ && elt.$.name) !== -1) {
		return "text/plain";
	} else if (xmlProperties.indexOf(elt && elt.$ && elt.$.name) !== -1) {
		return "text/xml";
	}
}
				...
				function createPrototype(_, content, store, facet) {
					...
					function setProperty(prop, element, ns, nature) {
						...
						if (ns && getNamespaceUrl(ns).indexOf(__XMLSchema) === 0) {
							...
							loc.$properties[name].$xsdType = element.$.type;
						}
						...
						// Special code for soapenc elements
							...
							if (element && element.$ && element.$.ref === ns + ":arrayType") {
								...
								if (arrTypeKey) {
									...
									// hack for generic soap web services prototypes translations
									if (tmpName.indexOf("{") === 0) tmpName = tmpName.substring(2, tmpName.length - 1);
									//
									var tmpProp;
