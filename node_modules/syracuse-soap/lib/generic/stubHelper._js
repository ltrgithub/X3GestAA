...
var plainTextProperties = ["resultXml", "objectXml", "lineXml", "inputXml"];
...
var StubHelper = helpers.defineClass(function(_, instance, wsdl) {
	...
	parseWsdl: function(_) {
		...
		function getNamespaceUrl(prefix) {
			return soapTools.getNamespaceUrl(definitions, prefix, true);
		}

		function getNamespacePrefix(url) {
			return soapTools.getNamespacePrefix(definitions, url, true);
		}
			...
			function addOperation(_, bindingOperation) {
				...
				function createPrototype(_, content, store, facet) {
					...
					var proto = {
						...
						$baseUrl: host + "/" + self.dispatcher + "/syracuse/collaboration/syracuse/" + self.name + "/" + port.$.name,
						$value: documentation,
						$title: operationName,
