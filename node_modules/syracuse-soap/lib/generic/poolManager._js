...
exports.ClientPool = helpers.defineClass(function(options) {
	...
	stop: function(_, context, all, diags) {
		...
		for (var i = min; i < max; i++) {
			...
			this._addTrackerDiagnose(context, "info", locale.format(module, "clientStopped", cli.cliCfg.x3Pid));
	...
	alloc: function(_, context, wsCall) {
		...
		var res = this.readFunnel(_, function(_) {
			...
			if (self.avail > 0) {
				...
				tracer.debug && tracer.debug("[" + self.alias + "] Reuse free client: " + cli.uuid);
				return cli;
	...
	createClient: function(_, context, callContext) {
		...
		return this.createFunnel(_, function(_) {
			...
			self.created++;
			tracer.debug && tracer.debug("[" + self.alias + "] New client created: " + newCli.cliCfg.x3Pid);
			self._addTrackerDiagnose(context, "info", locale.format(module, "newClientCreated", newCli.cliCfg.x3Pid));
			return newCli;
		});
	},
	getInfo: function() {
