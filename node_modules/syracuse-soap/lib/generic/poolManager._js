...
exports.ClientPool = helpers.defineClass(function(options) {
	...
	_addTrackerDiagnose: function(context, severity, message) {
		if (context.tracker && context.tracker.$diagnoses) {
			context.tracker.$diagnoses.push({
				$severity: severity,
				$message: message
			});
		}
	...
	createClient: function(_, context, callContext) {
		...
		return this.createFunnel(_, function(_) {
			...
			self._addTrackerDiagnose(context, "info", "New soap classic client created");
			return newCli;
		});
	},
	getInfo: function() {
