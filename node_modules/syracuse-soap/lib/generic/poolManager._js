		...
		return this.createFunnel(_, function(_) {
			...
			try {
				...
				if (context && context.request && context.request.headers && context.request.headers.unittest) { // for unit test 
					...
					newCli = new CvgClientWS(this.protocolVersion).init(_, context, callContext, recOptions, {
						tags: self.tags
					});
				} else {
					try {
						newCli = new CvgClientWS(this.protocolVersion).init(_, context, callContext, null, {
							tags: self.tags
						});
					} catch (e) {
