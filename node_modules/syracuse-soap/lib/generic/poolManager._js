...
var tracer = require('@sage/syracuse-core').getTracer("soap-generic.pool");
...
exports.ClientPool = helpers.defineClass(function(options) {
	...
	_getlicenseInfos: function(_, context, endpoint, user, localePref) {
		try {
			...
			// hack for unit test
			if (context && context.request && context.request.headers && context.request.headers.unittest) {
				config = {
					x3solution: endpoint.x3solution(_),
					x3serverTags: "UNITTEST",
					certificate: endpoint.x3solution(_).certificate(_)
				};
			}
	...
	// Resolves with an array of diagnoses
	start: function(_, force, context, poolInstance, oneSyncOnly) {
		...
		return this.startStopFunnel(_, function(_) {
			...
			if (poolInstance.autoStartDisabled(_)) {
				...
				// Strange : params 'parameter' and 'option' are mandatory to avoid error 'undefined is not a function' in NannyMode
				// The pool doesn't stop if ignoreRestrictions = false
	...
	free: function(res) {
		if (!this.started) {
			...
			res.disconnectClient(function(err) {
				if (err) throw err;
