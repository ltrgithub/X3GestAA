...
exports.OutputBuilderXml = helpers.defineClass(function(wsCall, resultParamMap, options) {
	...
	this.typesIgnoredNull = ["Date", "Integer", "Blob"];
		...
		if (_params) {
			if (field.$.DIM > 1) {
				...
				if (_p && _p.resu != null) {
					...
					} else if (field.$.TYP === "Clob" || field.$.TYP === "Char") {
						_fld.$value = encode(_p.resu);
					} else {
						_fld.$value = _p.resu;
					}
				...
				else if ((!force || (force && field.$.MEN == null)) && this.specialFieldsIdx.indexOf(this.currentIdx) < 0) {
	...
	_createField: function(elt, field, id) {
		...
		if (_params) {
			...
			if (field.$.DIM > 1) {
				...
				} else if (_p.resu != null && (field.$.TYP === "Clob" || field.$.TYP === "Char")) {
					elt[field.$.NAM] = encode(_p.resu);
				} else {
...
function toUnicode(char, charCode) {
	charCode = charCode || char.charCodeAt(0);
	var res = charCode.toString(16).toUpperCase();
	while (res.length < 4) {
		res = '0' + res;
	}
	return '\\u' + res;
