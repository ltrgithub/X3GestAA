...
var binary = require('@sage/syracuse-core').types.binary;
...
var OutputBuilder = helpers.defineClass(function(wsCall, resultParamMap, options) {
		...
		for (var i = 0; i < menus.length && !found; i++) {
			...
			if (menu.$.NO === idMenu && this.lang) {
				...
				for (var j = 0; j < vals.length && !found; j++) {
					if (vals[j] && vals[j].$ && vals[j].$.IND && parseInt(vals[j].$.IND, 10) === parseInt(resu, 10)) {
	...
	_computeTableSize: function(grp) {
		var self = this;
		var fldCount = Array.isArray(grp.FLD) ? grp.FLD.length : 1;
		// the first 12 parameters are technical
		var firstIdx = 12 + self.currentIdx;
		// filter params attended by grp
		var tabResParams = this.resultParamMap.filter(function(p) {
			var num = p && p[0] && p[0].num;
			return num >= firstIdx && num < firstIdx + fldCount;
		});
		// maybe some lines don't contain all params : SAM 114480
		return tabResParams.reduce(function(prec, curr) {
			var curSize = 0;
			var biggerPosteKey = Object.keys(curr).sort(function(a, b) {
				return b - a;
			}).slice(0, 1);
			var node = curr[biggerPosteKey];
			if (node.resu != null) curSize = node.poste + 1;
			if (curSize > prec) {
				return curSize;
			}
			return prec;
...
exports.OutputBuilderXml = helpers.defineClass(function(wsCall, resultParamMap, options) {
			...
			if (field.$.DIM > 1) {
				...
				if (_p && _p.resu != null) {
					if (field.$.TYP === "Blob") {
						...
						_fld.$.MIMETYPE = binary.calcMimeType(buf);
	...
	_createTable: function(elt, grp) {
		...
		if (this.action !== "EXEC") {
			...
			size = this._computeTableSize(grp);
		}
		...
		//
		if (this.action === 'EXEC' || (_tab.LIN && _tab.LIN.length > 0)) elt.TAB.push(_tab);
		this.currentIdx += flds.length;
	},
...
exports.OutputBuilderJson = helpers.defineClass(function(wsCall, resultParamMap, options) {
		...
		if (_params && field.$.UNSEL !== "true") {
			...
			if (field.$.DIM > 1) {
				...
				if (field.$.TYP === "Blob" && !isResuNull) {
					...
					elt[field.$.NAM + "_MIME"] = binary.calcMimeType(buf);
				} else {
	...
	_createTable: function(elt, grp) {
		...
		if (this.action !== "EXEC") {
			...
			// for subprograms, we need to compute the table size
			size = size = this._computeTableSize(grp);
		}
