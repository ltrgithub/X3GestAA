...
var lbMode = undefined;

var getBaseUrl = exports.getBaseUrl = function(_, authNeeded) {
	var ep = adminHelper.getCollaborationEndpoint(_);
	var baseUrl = authNeeded ? "/soap-generic/" : "/soap-wsdl/";
	return baseUrl + ep.application(_) + "/" + ep.contract(_) + "/" + ep.dataset(_);
};

var getWsdl = function(_, serviceName) {
	var wsdl = fs.readFile(fsp.join(__dirname, "../wsdl") + "/" + serviceName + ".wsdl", "utf8", _);
	return wsdl.replace("{baseUrl}", getBaseUrl(_, true));
};
...
exports.entity = {
	...
	$helpPage: "Administration-reference_Soap-generic",
	$canEdit: false,
	$canDelete: false,
	$canCreate: false,
	$properties: {
	...
	$relations: {
		stub: {
			...
			$isHidden: function(_, instance) {
				if (lbMode === undefined) {
					var syra = require('syracuse-main/lib/syracuse');
					var mock = require('syracuse-load/lib/mock');
					lbMode = (syra.server instanceof mock.MockStreamServer);
				}
				return lbMode;
			},
			*/
	...
	$functions: {
		regenerate: function(_) {
			...
			this.$diagnoses.push({
				$severity: "info",
				$message: locale.format(module, "stubRegenerated")
			});
		}
	},
	$services: {
