		...
		start: {
			...
			$execute: function(_, context, instance, parameters) {
				...
				if (lbMode) {
					...
					forEachKey(result, function(port, message) {
						var isErr = message !== "OK";
						instance.$diagnoses.push({
							$severity: isErr ? "error" : "info",
							$message: port + " : " + (isErr ? message : locale.format(module, "poolStopped"))
						});
					});
				} else {
...
exports.updateExternal = function(_, uuid) {
	_fetchPool(_, uuid).start(_, true);
};
