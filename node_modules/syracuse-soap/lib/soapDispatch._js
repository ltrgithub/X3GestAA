...
var isGenerationForced = helpers.soap && helpers.soap.forceGen != null ? helpers.soap.forceGen : false;


function getEntityWsdl(_, context) {
	// url is like '/soap/contract/application/dataset/entities',
	var path = context.request.url.split("?")[0].split("/");
	path[5] = context.model.singularize(path[5]);
	path[1] = 'descriptions';
	var wsdlPath = require("path").resolve(__dirname, "../") + "/" + path.slice(1, 6).join("/") + ".wsdl";
	var wsdl;
	if (fs.existsSync(wsdlPath)){
		wsdl = fs.readFile(wsdlPath, "utf8", _);
	} else {
		if (isGenerationForced){
				wsdlGenerator.generate(_, context);
				wsdl = fs.readFile(wsdlPath, "utf8", _);	
		}else{
			throw context.badRequest("SOAP Web Service doesn't exist for entity : "+path[5]);
		}
	}

	return wsdl.toString('utf8');
}


function replyWsdl(_, context) {
	var content = getEntityWsdl(_, context)
	var header = {
