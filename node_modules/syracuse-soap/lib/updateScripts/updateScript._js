...
_scripts[10] = function(_, db) {
	exports.tracer && exports.tracer("Executing update script to version: 10; Remove bad notification event for U8 only");
	var patchtools = require('syracuse-patch/lib/patchtools');
	try {
		var relNumber = patchtools.readVersionFile(patchtools.BASE_DIRECTORY, _).relNumber;
		var update = parseInt(relNumber.split('.')[0], 10);
		if (update === 8) {

			db.db.collection("NotificationEvent", _).remove({
				code: "license_web_grace"
			}, {
				safe: true
			}, _);

			db.db.collection("NotificationEvent", _).remove({
				code: "license_web_max"
			}, {
				safe: true
			}, _);

			db.db.collection("NotificationEvent", _).remove({
				code: "license_web_warn"
			}, {
				safe: true
			}, _);
		}
...
_scripts[11] = function(_, db) {
	exports.tracer && exports.tracer("Executing update script to version: 11; regenerate stub prototypes");
	regenerateStub(_, db);
};
