...
var Generator = helpers.defineClass(function(context) {
	...
	write: function(_, content){
		...
		var path = require("path").resolve(__dirname, "../") + "/descriptions/";
		var appPath = path + this.application;
		if (!fs.existsSync(appPath))
			fs.mkdir(appPath, 0777, _);
		var contractPath = appPath + "/" + this.contract;
		if (!fs.existsSync(contractPath))
			fs.mkdir(contractPath, 0777, _);
		var datasetPath = contractPath + "/" + this.dataset;
		if (!fs.existsSync(datasetPath))
			fs.mkdir(datasetPath, 0777, _);
		var wsdlPath = datasetPath + "/" + this.entity + ".wsdl";

		
		fs.writeFile(wsdlPath, content, 'utf8', _);
	},
