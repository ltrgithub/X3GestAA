...
var SoapHelper = helpers.defineClass(function(app, schema) {
	...
	convertValue: function(type, value){
		...
		try{
			...
			console.error("error in convertValue: "+e);
			return;
		}
	},

	isChild: function(title, elt, parentType){
		//console.log(title + " IS CHILD of "+parentType + " ? "+this.getEntityType(title, elt));
		if (elt.$item && this.getEntityType(title, elt.$item) === parentType){
			return true;
		}
		return false;
	},
	...
	formatValueForXml: function(_, title, element, instance, getBinaryFunc){
				...
				if (typeof(val) !== 'string'){
					...
					if (title.indexOf("$") === 0){
						if (type === "xs:date"){
								...
								return types.time.fromJsDate(new Date(val), true).toString();
						}else if (type === "xs:dateTime"){
								return types.datetime.fromJsDate(val).toString();
						}
					}
					if (type === "b64"){
						return getBinaryFunc(_, title);
					}else{
						try{
		...
		function formatReference(_){
			...
			for( var key in element.$ ) {
				if (val != null){
					var formatted = self.formatValueForXml(_, key, element.$[key], val, getBinaryFunc);
					if (formatted)
						ref[key] = formatted;
				}
			}
		...
		function formatCollection(_){
			...
			for (var i in val){
				...
				for( var key in element.$item.$ ) {
					var formatted = self.formatValueForXml(_, key, element.$item.$[key], val[i], getBinaryFunc);
					if (formatted)
						item[key] = formatted;
				}
			}					
		...
		function formatJson(_){
			...
			for( var key in element.$item.$ ) {
				if (val != null){
					var formatted = self.formatValueForXml(_, key, element.$item.$[key], val, getBinaryFunc);
					if (formatted)
						ref[key] = formatted;
				}
			}
...
exports.ckeckDiagnoses = function(obj){
	...
	if (obj && obj.$){
		Object.keys(obj.$).forEach(function(elt){
			if (obj.$[elt] && obj.$[elt].$diagnoses != null && obj.$[elt].$diagnoses.length !== 0){
