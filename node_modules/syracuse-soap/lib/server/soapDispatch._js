...
function replyWsdl(_, serviceName, context, entities) {
	var content = wsdlGenerator.generate(_, serviceName, context, entities);
	var header = {
...
var _syracuseDatasetMap = {
	walk: function(_, context, name, id) {
		if (context.model) { // TEST to allow stubs;
			var singular = context.model.singularize(name);
			context.entity = context.model.getEntity(singular);
...
var _x3DatasetMap = {
		walk: function(_, context, name, id) {
			...
			if (id) {
				...
				return _dispatch(_, context, _entityX3Map);
			}
		}
	}
...
var _entityMap = {
	get: function(_, context) {
		...
		var entityName = parts[0].split("/")[5];
		var param = parts[1];
		var entities = [];
//		
//		var filter = {sdataWhere: "name eq "+entityName};
//		var instances = context.db.fetchInstances(_, self.context.entity, filter);
//		
		...
		if (param && param.toUpperCase() === 'WSDL'){
			replyWsdl(_, entityName, context, entities);
		}
	},
	post: function(_, context) {
		var entityName = context.request.url.split("/")[5];
		var result = soapInterceptor.intercept(_, entityName, context);
		var header = {
...
// TEMP: Only for X3 while entities list not available
var _entityX3Map = {
		get: function(_, context) {
			var parts = context.request.url.split("?");
			var entityName = parts[0].split("/")[5];
