...
function replyWsdl(_, entityName, context) {
	var content = wsdlGenerator.generate(_, entityName, context);
	var header = {
...
var _contractMap = {
	walk: function(_, context, name, id) {
		...
		return _dispatch(_, context, context.applicationRef && (context.applicationRef.protocol(_) !== "syracuse") ? _x3DatasetMap : _syracuseDatasetMap);
	}
	,
	get: function(_, context) {
...
var _entityMap = {
	get: function(_, context) {
		...
		if (param && param.toUpperCase() === 'WSDL'){
			replyWsdl(_, entityName, context);
		}
	},
