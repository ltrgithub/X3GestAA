...
var ormExecutor = helpers.defineClass(function(_,entityName, context) {
	...
	execute: function(_, method, params){
		...
		function getPrototype(_, facetName){
			...
			if (self.app === "x3"){
				return self.context.entity.getPrototype(_, facetName); 

			}else{
				return self.context.getPrototypeResource(_, self.entityName + "." + facetName);
			}
				...
				for( var key in schema.$ ) {
					...
					if (serializedInstance[key] != null){
						if(serializedInstance[key].length === 0){
							...
							var formatted = sHelper.formatValueForXml(_, key, schema.$[key], serializedInstance, getBinary);
							if (formatted){
								if (key.indexOf('$') === 0)
		...
		function $edit(_, isUpdate){
			...
			if (!schema)
				throw new Error("An error has occured while retrieving prototype: " + self.entityName + ".$edit");
			//console.log("Schema = "+JSON.stringify(schema,null,2));
		...
		function $query(_){
			...
			if (!schema)
				throw new Error("An error has occured while retrieving prototype: " + self.entityName + ".$query");
			...
			if (self.app !== "x3"){
				...
				//console.log("INSTANCES="+util.inspect(instances));
...
//				}
			...
			for (var i in instances){
				...
				for( var key in schema.$.$resources.$item.$ ) {
					if (instance[key]){
						var formatted = sHelper.formatValueForXml(_, key, schema.$.$resources.$item.$[key], serializedInstance, getBinary, "syracuse");
						if (formatted){
							if (key.indexOf('$') === 0)
								key = '_' + key.substring(1);
		...
		var self = this;
		var namespace = this.entityName + "." + this.dataset + "." + this.contract + "." + this.application;
		var schema;
...
exports.create = function(_, entityName, context){
	return new ormExecutor(_, entityName, context);
}
