...
//			console.log("INSTANCE="+util.inspect(instance));
		...
		function $details(_){
			...
			if (params.$key === "")
				throw new Error(locale.format(module, "keyNeededForOperation", "READ"));
			
			schema = getPrototype(_, "$details");
			if (!schema)
				throw new Error(locale.format(module, "errGetProto", self.entityName, "details"));
		...
		function $edit(_, isUpdate){
					...
					function writeInstanceRelation(_, relationName){
						...
						function getInstance(_, subParams, entityType, isChild){
							...
							if (isChild){
								...
								if (subParams.$key){
									if (!subInst)
										throw new Error(locale.format(module, "entityTypeNotFound", entityType, subParams.$key));
								}else{
									return;
								}
							}
			...
			if (!schema)
				throw new Error(locale.format(module, "errGetProto", self.representation, "edit"));
			//console.log("Schema = "+JSON.stringify(schema,null,2));
			...
			if (isUpdate){
				...
				if (!primaryInstance)
					throw new Error(locale.format(module, "entityTypeNotFound", self.representation, params[self.representation].$key));
		...
		function $delete(_){
			if (params.$key === "")
				throw new Error(locale.format(module, "keyNeededForOperation", "DELETE"));
		...
		function $query(_){
			...
			// count
			if (params.$count)
				fetchParams.count = parseInt(params.$count, 10);
			...
			if (self.app !== "x3"){
				if (params.$pageInfo)
					fetchParams.startIndex = parseInt(params.$pageInfo, 10);
			}else{
			...
			if (!schema)
				throw new Error(locale.format(module, "errGetProto", self.representation, "query"));
