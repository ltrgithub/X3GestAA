...
var OrmExecutor = helpers.defineClass(function(_, entityName, representation, context) {
	...
	if (!context.entity && context.db) {

		context.entity = context.db.getEntity(_, entityName);
		if (!context.entity) throw new Error(locale.format(module, "entityNotFound", entityName));
	}
	...
	execute: function(_, context, method, params) {
		...
		function getPrototype(_, facetName) {
			...
			if (self.app === "x3") {
				return context.entity.getPrototype(_, context.entity.name, facetName);

			} else {
				return context.getPrototypeResource(_, self.representation + "." + facetName);
			}
		...
		function $details(_) {
			...
			var instance = context.db.fetchInstance(_, context.entity, params.$key);
		...
		function $edit(_, isUpdate) {
				...
				function writeInstanceParam(_, paramName, param) {
					...
					function writeInstanceRelation(_, relationName) {
						...
						function getInstance(_, subParams, entityType, isChild) {
							...
							var entity = context.model.getEntity(_, entityType);
							var subInst = context.db.fetchInstance(_, entity, subParams.$key);

							if (isChild) {
								if (!subInst) {
									var factory = entity.factory;
									tracer.traceSubSection("Create Child Instance with $uuid :" + subParams.$key, "");
									subInst = factory.createInstance(_, null, context.db);
						...
						function writeInstanceRelationCollection(_) {
							...
							var instType = context.model.singularize(inst._relMeta.type);
			...
			var primaryInstance;
			if (isUpdate) {
				primaryInstance = context.db.fetchInstance(_, context.entity, params[self.representation].$key);
				...
				var factory = context.entity.factory;
				primaryInstance = factory.createInstance(_, null, context.db);
			}
		...
		function $query(_) {
			...
			var instances = context.db.fetchInstances(_, context.entity, fetchParams);
			...
			if (self.app !== "x3") {
				...
				response._totalResults = context.db.count(_, context.entity, fetchParams).toString();
			} else {
