...
var Interceptor = helpers.defineClass(function(_, entityName, context) {
	...
	// Force implementation for unit tests
	if (this.app === "qunit")
		this.app = "syracuse";
	...
	if (this.app === "syracuse"){
		...
		// By default, use ORM mode
		if (config.soap && config.soap.promote.toUpperCase() === "REST"){
			this.executor = restExecutor.create(_, this.entityName, context);
		}else{
			this.executor = ormExecutor.create(_, context);
		}
...
exports.intercept = function(_, entityName, context){
	...
	var soapResponse;
	try{
		tracer.traceSection("MODE: " + interceptor.executor.mode);
