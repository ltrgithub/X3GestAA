...
var tracer = helpers.soap && helpers.soap.request.tracer != null ? helpers.soap.request.tracer : false;
...
var Interceptor = helpers.defineClass(function(context) {
	...
	run: function(_){
			...
			trace("Header : " + JSON.stringify(reqHeader,null,2));
	...
	execute: function(_, method, params){
		...
		function mustWrite(param, isUpdateReq){
			if (param == null){
				...
				return false;
			}else if (param.$key != null && param.$key != "" && !isUpdateReq){
				return true;
			}else if (isUpdateReq){
				return true;
			}else{
		...
		if (method === "read" + this.title){
			...
			if (instance != null){
				...
				//trace("Schema = "+JSON.stringify(schema,null,2));
				var serializedInstance = instance.serializeInstance(_);
				//trace("Instance = " +JSON.stringify(serializedInstance,null,2));
				...
				for( var key in schema.$ ) {
					...
					if (serializedInstance[key] != null){
						...
						response[this.entity][key] = {$: {"xsi:nil": "true"}};
					}
						...
						if (!inst || !inst._relMeta || !inst._relMeta.isPlural){
							...
							if (param.$key){
								...
								if (!inst){
									...
									throw new Error("TODO: No '" + entity.name + "' instance found with identifier '" + param.$key + "'");
								}
