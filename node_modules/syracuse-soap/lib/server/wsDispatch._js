...
function _replyWsdl(_, serviceName, context) {
	var ws = soapUtils.getSoapWebService(_, serviceName, context);
	//tracer && tracer("SERIALIZED SERVICE="+JSON.stringify(ws.serializeInstance(_),null,2));
	if (ws) {
...
//URL is http://host/soap-ws/app/contract
...
// URL is http://host/soap-ws/app/contract/dataset
...
var _syracuseDatasetMap = {
	...
	get: function(_, context) {
		...
		return context.replyDictionaryPage(_, soapUtils.getSoapWebServices(_), "soapWebService", function(name, ws) {
			return {
...
//URL is http://host/soap-ws/app/contract/dataset/entity (or deeper)
var _entityMap = {
	get: function(_, context) {
	...
	post: function(_, context) {
		...
		var result = soapInterceptor.intercept(_, serviceName, context);
		var header = {
			"content-type": "text/xml; charset=utf-8",
...
//http://host/soap-stubs/app/contract/dataset/entity/port
var _portMap = {
	walk: function(_, context, name, id) {
	...
	get: function(_, context) {
		...
		return context.replyDictionaryPage(_, stubUtils.getSoapStubPortOperations(_, stubName, portName, context, true, "standard-in"), "soapStubPortOperations", function(name, operation) {
			return {
...
// http://host/soap-stubs/app/contract/dataset/entity/port/operation
var _operationMap = {
	...
	get: function(_, context) {
		...
		if (match) {
			...
			if (paramName === "representation") {
				var proto = stubUtils.getOperationPrototype(_, stubName, portName, operationName, facet, context, "standard-in");
				proto.$links = {
					$invoke: {
						$title: "Invoke",
...
//http://host/soap-stubs/app/contract/dataset/entity/port/$prototypes
var _invokeMap = {
	post: function(_, context) {
		var parts = context.request.url.split("/");
		var stubName = parts[5];
		...
		var response = stubRuntime.execute(_, stubName, portName, operationName, data, context, "standard-in");
		//tracer && tracer("Request: "+JSON.stringify(resp,null,2));
...
//http://host/soap-stubs/app/contract/dataset/entity/port/$prototypes('id')
var _prototypeMap = {
	get: function(_, context) {
		var parts = context.request.url.split("/");
		var stubName = parts[5];
...
function _doIt(_, context) {
	try {
		...
		if (protocol !== "soap-ws") throw new Error(locale.format(module, "badUrl", protocol));
