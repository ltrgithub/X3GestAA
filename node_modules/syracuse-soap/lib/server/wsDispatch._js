...
function _replyWsdl(_, serviceName, context){
	...
	//console.log("SERIALIZED SERVICE="+JSON.stringify(ws.serializeInstance(_),null,2));
	if (ws){
		...
		throw context.notFound(locale.format(module, "Web Service not found", serviceName));
	}
}
...
var _syracuseDatasetMap = {
	...
	get: function(_, context) {
		...
		return context.replyDictionaryPage(_, soapUtils.getSoapWebServices(_, context), "soapWebService", function(name, ws) {
			return {
	...
	get: function(_, context) {
		...
		if (match){
			...
			if (paramName === "representation"){
				var proto = stubUtils.getOperationPrototype(_, stubName, portName, operationName, facet, context, true);
				proto.$links = {
					$invoke: {
						$title: "Invoke",
				...
				throw context.badRequest("unexpected parameter: '" + paramName + "'");
			}
		}else{
...
var _invokeMap = {
	post: function(_, context) {
		...
		var response = stubRuntime.execute(_, stubName, portName, operationName, data, context, true)
...
function _doIt(_, context) {
	try {
		...
		if (protocol != "soap-ws") throw new Error(locale.format(module, "badUrl"));
