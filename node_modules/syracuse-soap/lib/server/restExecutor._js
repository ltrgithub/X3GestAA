...
var restExecutor = helpers.defineClass(function(entityName, representation, context) {
	...
	this.baseUrl = context.httpSession.host + "/soap/" + this.application + "/" + this.contract + "/" + this.dataset;
			...
			function prepareData(data, parameters){
				...
				function prepareDataParam(paramName, param){
					...
					function prepareDataRelation(relationName){
						...
						function prepareDataRelationReference(){
							if (mustWrite(param, isUpdate)){
								...
								if (param.$ && param.$["xsi:nil"] === "true"){
									...
									if (sHelper.isChild(relationName, schema.$properties[relationName], self.representation)){
									...
									for (var key in param){
										if (key !== "$key"){
											...
											var converted = sHelper.convertValue(node.$properties[key].$type, param[key]);
											if (converted != null){
												if (converted !== "b64"){
						...
						if (schema.$properties[paramName].$type === "application/x-collection"){
...
//							tracer.traceSubSection("Schema: "+JSON.stringify(schema.$properties[paramName],null,2));
							if (schema.$properties[paramName].$type === "application/x-collection" || schema.$properties[paramName].$type === "application/x-reference"){
								prepareDataRelation(paramName);
							}else if (schema.$properties[paramName].$item && schema.$properties[paramName].$item.$type === "application/json"){
								prepareDataRelation(paramName);
							}else{
			...
			if (!isUpdate){
				...
				method = "PUT";
				url = self.url + "('" + params[self.representation].$key + "')";
			}
		...
		function $query(_){
			...
			if (httpResp.statusCode !== 200)
				throw new Error("An error has occured while retreiving $query representation for entity: "+ self.representation);
			...
			for (var i in instances){
				...
				response[self.representation + "List"][self.representation][i] = {};
				response[self.representation + "List"][self.representation][i]._key = computeKey(instance, schema.$properties.$resources.$item);
