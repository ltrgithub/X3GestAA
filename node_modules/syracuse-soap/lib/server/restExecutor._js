						...
						function prepareDataRelationReference(){
							if (mustWrite(param, isUpdate)){
								...
								if (param.$ && param.$["xsi:nil"] === "true"){
									...
									if (sHelper.isChild(relationName, schema.$[relationName], self.representation)){
...
//							tracer.traceSubSection("Schema: "+JSON.stringify(schema.$[paramName],null,2));
		...
		function $query(_){
			...
			for (var i in instances){
				...
				response[self.representation + "List"][self.representation][i] = {};
				response[self.representation + "List"][self.representation][i]._key = computeKey(instance, schema.$.$resources.$item);
