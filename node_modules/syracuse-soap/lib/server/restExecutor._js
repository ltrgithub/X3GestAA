...
var RestExecutor = helpers.defineClass(function(entityName, representation, context) {
	...
	execute: function(_, context, method, params) {
		...
		function $details(_) {
			...
			schema = restUtils.getPrototype(_, context.request.headers, self.baseUrl, self.representation, "$details");
			...
			var httpResp = restUtils.getRepresentation(_, context.request.headers, self.baseUrl, schema.$url, resource);
		...
		function $edit(_, isUpdate) {
			...
			schema = restUtils.getPrototype(_, context.request.headers, self.baseUrl, self.representation, "$edit");
			...
			//var resp = restUtils.sendRequest(_, context.request.headers, method, url, principalData);
			restUtils.useWorkingCopy(_, context.request.headers, self.baseUrl, self.url, self.entityName, self.representation, principalData, key);
		...
		function $delete(_) {
			...
			var resp = restUtils.sendRequest(_, context.request.headers, "DELETE", url);
		...
		function $query(_) {
			...
			var fetchParams = {};
			schema = restUtils.getPrototype(_, context.request.headers, self.baseUrl, self.representation, "$query");
			//tracer.trace("Schema="+JSON.stringify(schema,null,2));
			var sHelper = soapHelper.create(self.application, schema);
			...
			var httpResp = restUtils.getRepresentation(_, context.request.headers, self.baseUrl, schema.$url, null, fetchParams);
