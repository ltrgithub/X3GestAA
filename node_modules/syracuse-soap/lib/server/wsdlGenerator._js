...
var Generator = helpers.defineClass(function(_, service, entities) {
	...
	makeWsdl: function(_){
		...
		function addSchema(_, entityName, representationName, schema, generics){
				...
				switch(element.$type){
						...
						if (type != null){
							...
							if (elementType != null ){
								...
								if (sHelper.isParent(title,element, elementName)){
									formatted.$.type = ns + elementType + "--ref-list";
									if (!complexTypeExists(elementType + "--ref")){
										
										addComplexType(elementType + "--ref", element.$item);
									}
									if (!complexTypeExists(elementType + "--ref-list")){
										addComplexList(elementType, elementType !== elementName, true);
									}
							...
							if (listType && listType.name){
								if (listType.isRef){
									formatted.$.type = ns + listType.name + "--ref";
								}else{
									formatted.$.type = ns + listType.name + "--type";
								}
					...
					case "application/x-choice":
						...
						if (type != null){					
							...
							for( var key in element.$value.$enum ) {
								var xsenum = {
									$: {
										value: element.$value.$enum[key].$value.toString()
									}
								}
					...
					case "application/x-reference":
						...
						if (sHelper.isParent(title, element, elementName)){
							if (!complexTypeExists(elementType + "--ref")){
								formatted.$.type = ns + elementType + "--ref";
								addComplexType(elementType + "--ref", element);
								return formatted;
							}else{
								return;
							}
							
						}else if (!complexTypeExists(elementType + "--type")){
			...
			function addComplexList(cpTitle, withDeleteMissing, isParent){
				...
				listCp.$properties[cpTitle].$item.$properties[elementName] = {
					$nokey: true,
					$type: isParent ? (ns + elementName +"--ref") : (ns + elementName +"--type")
				}
				
				addComplexType(isParent ? (cpTitle + "--ref-list") : (cpTitle + "--list"), listCp);
			}
			...
			function addComplexType(title, element){
				if (!complexTypeExists(title)){
					...
					if (title.indexOf("--list") === (title.length - 6)){
						listType = {
							name: title.substring(0,title.length - 6),
							isRef: false
						}
					}
					if (title.indexOf("--ref-list") === (title.length - 10)){
						listType = {
							name: title.substring(0,title.length - 10),
							isRef: true
						}
					}
					for( var key in element.$properties) {
						tracer.trace("Add Sequence : "+key);
						var formatted = format(key, element.$properties[key], listType);
						if (formatted)
