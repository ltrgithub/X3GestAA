	...
	makeWsdl: function(_){
		...
		function getPrototype(_, name, facet){
			try{
				...
				if (!sc)
					throw new Error("An error has occured while retrieving "+facet+" protoype: " +name + "." + facet);
		...
		try{
			...
			if (this.representations.length === 0)
				throw new Error("The web service '"+this.serviceName+"' must contains at least one representation.");
