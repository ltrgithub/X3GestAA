	...
	makeWsdl: function(_){
		...
		function getPrototype(_, name, facet){
			try{
				...
				if (self.application === "x3" && facet === "$edit")
					facet = "$details";
					
				var sc = restUtils.getPrototype(_, self.context.request.headers, self.baseUrl, name, facet);
				if (!sc)
					throw new Error(locale.format(module, "errGetProto", facet, name, facet, ""));
		...
		function addSchema(_, entityName, representationName, generics){
			...
			function format(title, element, listType, facet){
				...
				tracer.traceSubSection("Format : "+ entityName + " with facet "+facet);
				...
				switch(element.$type){
					...
					case "application/x-reference":
						...
						if (!elementType)
							throw new Error("An error has occured while retrieving element type for reference ["+title+"]");
						if (sHelper.isParent(title, element, elementName)){
			...
			function addComplexType(title, element, facet){
				...
				if (!complexTypeExists(title)){
					...
					if (title.indexOf("--list") === (title.length - 6)){
						var len = facet ? title.length - 6 - facet.length - 1 : title.length - 6;
						listType = {
							name: title.substring(0,len)
						}
					}
					if (title.indexOf("--ref-list") === (title.length - 10)){
						var len = facet ? title.length - 10 - facet.length - 1 : title.length - 10;
						listType = {
							name: title.substring(0,len),
							kind: 'ref'
						}
					...
					if (title.indexOf("--enum-list") === (title.length - 11)){
						var len = facet ? title.length - 11 - facet.length - 1 : title.length - 11;
						listType = {
							name: title.substring(0,len),
							kind: 'enum'
						}
					...
					for( var key in element.$properties) {
						...
						var formatted = format(key, element.$properties[key], listType, facet);
			...
			if (!elementName)
				throw new Error("An error has occured while retrieving element name.");
			
			var editCP = addComplexType(elementName + "-edit--type", schemas.$edit, "edit");
			...
			var detailsCP = addComplexType(elementName + "-details--type", schemas.$details, "details");
			
			sHelper = soapHelper.create(self.application, schemas.$query);
			elementName =  sHelper.getEntityType(entityName, schemas.$query);
			var queryCP = addComplexType(elementName + "-query--type", schemas.$query, "query");
		...
		try{
			...
			if (this.representations.length === 0)
				throw new Error(locale.format(module, "atLeastOneRepr", this.serviceName));
