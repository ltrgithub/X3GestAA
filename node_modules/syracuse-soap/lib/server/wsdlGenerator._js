...
var Generator = helpers.defineClass(function(entityName, application, contract, dataset) {
	...
	runSyracuse: function(_, context){
		var schema = context.getPrototypeResource(_, this.entityName + "." + requestType);
		//tracer.traceSubSection("Entity Schema : " + this.entityName, JSON.stringify(schema,null,2));
		return makeWsdl(_, this.entityName, this.namespace, schema, "syracuse", context.url);
	},
	runX3: function(_, context){	
		var schema = restUtils.getPrototype(_, context, this.entityName, requestType);
		//tracer.traceSubSection("Entity Schema : " + this.entityName, JSON.stringify(schema,null,2));		
		return makeWsdl(_, this.entityName, this.namespace, schema, "x3", context.url);
	}
...
function makeWsdl(_, entityName, namespace, schema, protocol, wsUrl){
...
//			formatted.$.minOccurs = "1";
		...
		switch(element.$type){
			case "application/x-collection":
				...
				if (type != null){
					...
					var entityType = getEntityType(title, element.$item);
			...
			case "application/x-reference":
				...
				var entityType = getEntityType(title, element);
	...
	entityName =  getEntityType(entityName, schema);
