...
osFileProto.createWritableStream = function(_) {
	tracer && tracer("OsFileStore.createWritableStream for file: " + this.fileName);
	return new WritableStream(fs.createWriteStream(this.fileName));
};
...
var x3FileProto = X3FileStore.prototype;
x3FileProto.getProperties = function(_) {
...
x3FileProto.fileExists = function(_) {
	// TODO: check exists against X3. For now just check filename
	return this.fileName != null;
};
x3FileProto.setFile = function(_, fileName) {
	// fileName is x3 url here
	this.fileName = fileName;
};
x3FileProto.read = function(_, len) {};
x3FileProto.write = function(_, buffer, options) {};
x3FileProto.deleteFile = function(_) {};
x3FileProto.close = function(_) {};
...
var _propertyStoreMap = {
	...
	"x3": function(instance, property) {
		return new X3FileStore((instance._data[property.name] && instance._data[property.name].$uuid) || "");
	}
};
