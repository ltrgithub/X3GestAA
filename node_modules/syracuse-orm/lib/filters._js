...
function _toFilterValue(value, type) {
	switch (type || typeof value) {
		...
		case "object":
			return null;
			// TODO add the right condition regarding what we received
...
function _createSdataFilterFromJson(jsonWhere) {
	...
	var res = "";
	Object.keys(jsonWhere).map(function(k) {
		var v = _toFilterValue(jsonWhere[k]);
		var cond = (v ? k + " eq " + v : "");
		res += (res !== "" ? " and " + cond : cond);
	});
	return res ? "(" + res + ")" : "";
...
exports.jsonToSdata = function(params) {
	...
	if (p.jsonWhere && Object.keys(p.jsonWhere).length) exp.push(_createSdataFilterFromJson(p.jsonWhere));
