...
//			config.tracer && config.tracer("mongodbDbHandle.create where.children[0]: " + sys.inspect(where.children[0].value));
			...
			if (where.value.code === "=") {
				...
				if(oper === "between") {
					result[_left(where.children[0])] = {$gte:_format(where.children[1].value, isDate), $lte:_format(where.children[2].value, isDate)};
				} else {
					var options = null;
...
/*					result[where.children[0].value] = {};
					result[where.children[0].value][oper] = right;
					if(options) result[where.children[0].value].$options = options;*/
					result[l] = {};
					result[l][oper] = right;
					if(options) result[l].$options = options;
				}
			}
		}
		break;
...
exports.jsonToSdata = function(params) {
	...
	return exp.length ? exp.map(function(term) { 
		return "(" + term + ")"; 
	}).join(" AND ") : "";
}
