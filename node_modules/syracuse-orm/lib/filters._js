...
//			config.tracer && config.tracer("mongodbDbHandle.create where.children[0]: " + sys.inspect(where.children[0].value));
			...
			if (where.value.code === "=") {
				...
				if(oper === "between") {
					result[where.children[0].value] = {$gte:_format(where.children[1].value, isDate), $lte:_format(where.children[2].value, isDate)};
				} else {
					var options = null;
					var right = _format(where.children[1].value, isDate);
