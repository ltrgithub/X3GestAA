...
exports.Relation = helpers.defineClass(Relation, BaseProperty, {
	...
	getPropertyPrototype: function(_, stringRes, facetName, isChild, childPrefix, visited) {
		...
		function _updateItem(_, res, item, ent, variantMeta) {
			...
			if (isChild) {
				if (visitedEntities && visitedEntities.indexOf(variant.targetEntity.name) >= 0) {
					...
					item.$type = "application/x-pointer";
					item.$prototype = "#" + variant.targetEntity.name;
				} else {
		...
		if (self.isPlural && facetName === "$edit") {
			...
			($r.$links = $r.$links || {}).$select = self.getLink("$select");
		}
		//
		return result;
	},
	getLink: function(facetName, variantName, params) {
		var self = this;
		switch (facetName) {
			case "$select":
				var lp = self.$select && (self.$select.parameters || self.$select.$parameters);
				...
				if (self.$variants) {
					...
					if (variantName) {
						...
						res.$variants[variantName] = _getLink("$select", null, pars.join("&") + "&variant=" + variantName, vEnt.name);
					} else {
						for (var vName in self.$variants) {
							if (!self.getIsChild(vName)) {
								...
								res.$variants[vName] = _getLink("$select", null, pars.join("&") + "&variant=" + vName, vEnt.name);
							}
						}
					}
					return res;
					...
					return _getLink("$select", locale.format(module, "createTableActionTitle"), pars.join("&"), ent.name);
				}
			default:
				return _getLink(facetName, null, pars.join("&"), self._entity.name); // TODO ?
		}
	}
});
...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix, visited) {
	...
	flows.eachKey(_, entity.$relations, function(_, name, rel) {
		...
		var $r = $[name] = rel.getPropertyPrototype(_, stringRes, facetName, isChild, childPrefix, visitedEnts);
	});
...
exports.Entity = helpers.defineClass(Entity, null, {
	...
	getPrototype: function(_, reprName, facetName, isChild, childPrefix, visited) {
		return (this.$getPrototype && this.$getPrototype(_, reprName, facetName)) || _getEntityPrototype(_, this._model, this, reprName, facetName, isChild, childPrefix, visited);
	},
	...
	tick: function(_, db) { // atomically increases the tick and sets the local endpoint for the digest of this entity 
		// not yet finished - atomic counter extension missing
		var entity = this;
		...
		syncInfo.endpoint(_, this.endpoint(_));
	...
	getLink: function(facet, title, params) {
		return _getLink(facet, title, params, this.name);
	},
	...
	getCounterValue: function(_, db, code, withIncrement) {
		return db.getCounterValue(_, this.name, code, withIncrement);
	}
});