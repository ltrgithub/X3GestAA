	...
	if ((facetName !== "$child") && !isChild) {
		...
		if (queryRepr.indexOf(facetName) >= 0) {
			...
			resource = {
				...
				$type: resource.$type,
				$title: _getTranslatedString(stringRes, [entity.name, "$listTitle"]) || entity.$listTitle || locale.format(module, "listOf", entity.plural),
				$properties: {
					$resources: {
						$type: "application/x-array",
						$item: resource
			...
			if (entity.$queryLinks) {
				resource.$links = entity.$queryLinks;
			}
...
function Entity(model, name, meta) {
	...
	if (_isEntityCapableOf(entity, "mailTemplate")) require('syracuse-etl/lib/entities/mailTemplate').addMailTemplateService(entity);
	// 
	entity._$properties = {};
...
exports.Entity = helpers.defineClass(Entity, null, {
	...
	addServicesLinks: function(reprType, resource, isChild) {
		...
		forEachKey(self.$services, function(key, service) {
			var s = null;
			if (service.$facets && service.$facets.indexOf(reprType) < 0) return;
			...
			s.$title = _getTranslatedString(stringRes, [entity.name, "$services", key, "$title"], true) || service.$title || key;
			s.$description = _getTranslatedString(stringRes, [entity.name, "$services", key, "$description"], true) || service.$description || null;
			s.$confirm = _getTranslatedString(stringRes, [entity.name, "$services", key, "$confirm"], true) || service.$confirm || null;
