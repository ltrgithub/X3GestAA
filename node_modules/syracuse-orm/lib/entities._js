	...
	if (prop.$capabilities == null) {
		...
		if (facetName === "$edit") {
			if (prop.$isArray || prop.isPlural) {
				...
				if ((i_add < 0) && prop.$isChild) cap.push("append");
		...
		switch (facetName) {
			case "$select":
				...
				if (self.$variants) {
					...
					if (variantName) {
						res.$variants[variantName] = _getLink("$select", locale.format(module, "createTableActionTitle"), pars.join("&"), ent.name);
					} else {
			...
			default:
				return _getLink(facetName, null, pars.join("&"), self._entity.name);
		}
	}
});
