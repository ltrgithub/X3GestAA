			...
			case "$select":
				...
				if (self.$variants) {
					...
					if (variantName) {
						...
						res.$variants[variantName] = _getLink("$select", null, pars.join("&"), vEnt.name);
					} else {
						for (var vName in self.$variants) {
							if (!self.getIsChild(vName)) {
								...
								res.$variants[vName] = _getLink("$select", null, pars.join("&"), vEnt.name);
							}
						}
					}
					return res;
				} else //
					return _getLink("$select", locale.format(module, "createTableActionTitle"), pars.join("&"), self._entity.name);
			default:
