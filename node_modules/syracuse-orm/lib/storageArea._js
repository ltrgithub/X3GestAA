...
// -------------
/// ## getWritableStream function :
/// ``` javascript
/// var document = sa.getWritableStream(_, properties, writeFunction, filter, volume);  
/// ```
/// Update document in storage area if filter is valid, else Create it. 
/// 
/// * The `properties` parameter represents the document properties in JSON format :  
...
/// 
/// 
/// Returns a object with 2 properties : stream and documents.  
///  
exports.getWritableStream = function(_, properties, filter, volume, entityName) {
	return new Helper(_, entityName).getWritableStream(_, properties, filter, volume)
};
/// -------------
/// ## remove function :
/// ``` javascript
...
var Helper = helpers.defineClass(function(_, entityName) {
	...
	writeAll: function(_, props, buf, filter, volume) {
		...
		checkDiagnoses(document);
		return document;
	},

	getWritableStream: function(_, props, filter, volume) {
		var document = this.getDocumentInstance(_, filter, volume);
		// Manage volume only for 'document' entity
		if (this.entityName === "document") {
			var docVolume = document.volume(_);
		...
		props = props && props.content != null ? props.content : {};
		props.uploadDone = function(_,instance) {
			document.save(_);
			checkExpirationDocument(_, document);
			checkDiagnoses(document);
		}
		...
		return {"stream":stream,"document":document}	
