...
///  
exports.open = function(_, filter, volume){
	return new Helper(_).open(_, filter, volume);
}
...
var Helper = helpers.defineClass(function(_) {
	...
	open : function(_, filter, volume){
		
		var ddUuid = uuid.generate('-');
		var self = this;
		var StoWriter = helpers.defineClass(function(_, filter) {
			this.document = self.getDocumentInstance(_, filter, volume);
