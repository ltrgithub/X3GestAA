...
///  
exports.listDocuments = function(_, filter, volume, serialize, expectedProperties){
	return new Helper(_).listDocuments(_, filter, volume, serialize, expectedProperties);
};
...
var Helper = helpers.defineClass(function(_) {
	...
	getVolumeInstance: function(_, volume) {
		...
		return this.db.fetchInstance(_, this.db.model.getEntity("storageVolume"), {
			"jsonWhere": {
	...
	listDocuments: function(_, filter, volume, serialize, expected) {
		if (expected && !Array.isArray(expected))
			throw new Error("'expected' parameter must be an array.");
		...
		if (volume && volume.length > 0) {
			...
			if (!vol)
				throw new Error("Volume '" + volume + "' doesn't exist");
			if (filter && filter.jsonWhere) {
				filter.jsonWhere.volume = vol.$uuid;
			} else {
				filter = {
					jsonWhere: {
						volume: vol.$uuid
			...
			for (var i in instances){
				...
				if (expected && expected.length > 0) {
					...
					for (var j in expected) {
						if (serializedInstance[expected[j]] !== null)
							inst[expected[j]] = serializedInstance[expected[j]];
					}
