...
/// 
exports.getDocumentInstance = function(_, filter){
	return new Helper(_).getDocumentInstance(_, filter);
}
...
var Helper = helpers.defineClass(function(_) {
	...
	getDocumentInstance : function(_, filter){
		...
		if (filter != null){
			...
			document.volume(_, this.db.fetchInstance(_, this.db.model.getEntity("storageVolume"), {
				code: "STD"
			}));
		}
		return document;
...
/*		var props = store.getProperties(_);
		...
		return store.createReadableStream(_).readAll(_);
// crnit <<<
	},
...
// crnit: change of storage area write interface: use a "real" WritableStream >>>
