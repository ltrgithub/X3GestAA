...
function Factory(entityMeta) {
	...
	function Instance(connection) {
		...
		if (inst._meta.name !== factorySelf._meta.name) {
			console.error("FATAL ERROR !!!!!!!!!!!!! entityMeta differ from factory._meta");
			console.error("Meta names: inst=" + inst._meta.name + "; factory=" + factorySelf._meta.name);
		}
		// DEBUG <<<
	}
	...
	_proto.createChild = function(_, relationName, key, data, loadOnly, variantType) {
		...
		if ((self._meta.name === "menuBlock") && (elt._meta.name === "menuBlock")) {
			// ERROR CONDITION !!!
			console.error("FATAL ERROR !!!!!!!!!! - Wrong created item meta");
			console.error("Variant types : data=" + (data && data.$variantType) + "; param=" + variantType + "; result=" + varType);
			console.error("r name=" + r.name);
			console.error("type: data=" + (data && data.$type) + "; result=" + t);
			console.error("data url: " + (data && data.$url));
			console.error("target type: " + sys.inspect(tgType));
			console.error("target type factory: " + sys.inspect(tgType && tgType.factory));
		}
		// DEBUG <<<
	...
	factorySelf.syncCreateInstance = function(_, data, connection, context) {
		var inst = new Instance(connection);
		if ((data && data.$type === "menuSubblock") && ((entityMeta.name === "menuBlock") || (factorySelf._meta.name === "menuBlock") || (inst._meta.name === "menuBlock"))) {
			console.error("FATAL ERROR : syncCreateInstance(1) !!!!!!!!!!!!");
			console.error("Meta: entityMeta=" + entityMeta.name + "; factoryMeta=" + factorySelf._meta.name + "; inst=" + inst._meta.name);
		}
		data && inst.load(_, data);
		if ((data && data.$type === "menuSubblock") && ((entityMeta.name === "menuBlock") || (factorySelf._meta.name === "menuBlock") || (inst._meta.name === "menuBlock"))) {
			console.error("FATAL ERROR : syncCreateInstance(2) !!!!!!!!!!!!");
			console.error("Meta: entityMeta=" + entityMeta.name + "; factoryMeta=" + factorySelf._meta.name + "; inst=" + inst._meta.name);
		}
		//
		return inst;
