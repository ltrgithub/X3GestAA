...
_collProto.load = function(_, value) {
	function _makeInst(_, elt) {
		if (elt.$uuid || elt.$key || elt.$url) return instance.createChild(_, self._relMeta.name, elt.$uuid || elt.$key || elt.$url, elt);
		else throw new Error(locale.format(module, "loadElementNoData", self._relMeta.name));
	}
	//
	...
	if (Array.isArray(value)) this._array = value.map_(_, function(_, elt) {
		...
		if ((key[0] !== "$") && (key[0] !== "_")) {
			self._array.push(_makeInst(_, value[key]));
		}
	}
};
/// -------------
...
function Factory(entityMeta) {
	...
	_proto.syncCreateChild = function(_, relationName, key, data) {
		...
		// DEBUG
		if (targetType) {
			if (targetType.factory._meta.name !== targetType.factory._entityMeta.name) {
			...
			if (r.targetEntity && targetType.name !== r.targetEntity.name) {
				console.error("FATAL FACTORY (2528) targetType: " + targetType.name + " !== r.targetEntity " + r.targetEntity.name);
			}
		}
		// DEBUG
		...
		if (relationName === "vignettes" && elt && elt._meta.name === "dashboardVariant") {
			console.error("FATAL FACTORY (2530) targetType: " + sys.inspect(targetType));
			console.error("FATAL FACTORY (2530) targetType.factory: " + sys.inspect(targetType.factory._meta));
		}
		// DEBUG
