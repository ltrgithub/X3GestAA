...
function _replacePredefinedVars(_, sdata) {
	...
	r = r.replace("{$profile.locales}", "(" + up.locales(_).toUuidArray(_).map(function(rId) {
		return "\"" + rId + "\"";
	}).join(",") + ")");
	return r;
}
...
function _canUpdateInstance(_, instance) {
	...
	if(sp) {
		...
		var r = sp.canUpdateClass(_, instance.getEntity().name);
		if(typeof r === "boolean") {
			if(r === false) {
				return false;
			}
...
function _canDeleteInstance(_, instance) {
	...
	if(sp) {
		...
		var r = sp.canDeleteClass(_, instance.getEntity().name);
		if(typeof r === "boolean") {
			if(r === false) {
				return false;
			}
...
function _getPropertiesResourceMeta(_, resource, instance) {
	...
	if(sp && !config.showRights) {
		...
		if(!sp.canUpdateClass(_, instance._meta.name)) {
			resource.$links = resource.$links || {};
			var l = resource.$links.$edit = resource.$links.$edit || {};
			l.$isHidden = true;
			//
			resource.$actions = resource.$actions || {};
		...
		if(!sp.canDeleteClass(_, instance._meta.name)) {
			resource.$links = resource.$links || {};
			var l = resource.$links.$delete = resource.$links.$delete || {};
			l.$isHidden = true;
		}
	}
}
...
//			filter[collection._relMeta.$inv + (collection._relMeta.targetEntity.$relations[collection._relMeta.$inv].isPlural?".$keys":".$uuid")] = collection._parent.$uuid;
	...
	_collProto.deleteInstance = function(_, instanceId, isInverse) {
		...
		if(relInst) {
			var invRel = this._relMeta.$inv && relInst.getEntity().$relations[this._relMeta.$inv];
...
//		config.tracer && config.tracer("factory.Instance().connection:"+sys.inspect(connection));
		...
		if(data) {
				...
				if (value && r && r.isPlural && Array.isArray(value)) {
					...
					if (value && r) {
						...
						if (rel.isPlural) 
							...
							var e = _getTargetEntity(self._db.model, rel, value.$type);
			...
			//
			if(!data.$isSnapshot && self._meta.$signed) {
				if(!checksum.verify(data, ["_id", "$uuid", "$key", "$loaded"]))
...
//		if(!this._meta.$relations[relationName]) console.log("factory.syncCreateChild: "+relationName+";"+sys.inspect(data));
	...
	function _fireBeforeSave(_, self) {
		...
		self._meta.$events && self._meta.$events.$beforeSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		...
		// can save ?
		self._meta.$events && self._meta.$events.$canSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		...
		flows.eachKey(_, self._meta.$relations, function(_, relName, rel) {
			if(rel.isChild) {
	...
	function _fireAfterSave(_, self) {
		self._meta.$events && self._meta.$events.$afterSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		...
		// notify child instances
		flows.eachKey(_, self._meta.$relations, function(_, relName, rel) {
			if(rel.isChild && rel.targetEntity.$events) {
				var relInst = self[relName](_);
				if(!relInst) return;
				if(rel.isPlural) { 
...
//		op.$title = "Save";
		...
		switch(expression.type) {
			case "operator":
				switch(expression.value.code) {
					...
					case "in":
						var child1 = _evaluateChild(_, expression.children[0], instance);
						return expression.children.slice(1).some_(_, function(_, child) {
							return child1 === _evaluateChild(_, child, instance);
						});
					default:
	...
	_proto.deleteSelf = function(_){
		...
		if(!_canDeleteInstance(_, self)) {
			...
			self.$addError(locale.format(module, "deleteForbidden", self.getEntity().name));
			return;
		}
		try {
...
exports.updateWorkingCopy = function(_, context, instance){
	// rights management
	if(!_canUpdateInstance(_, instance))
		return context.reply(_, 403, locale.format(module, "updateForbidden", instance.getEntity().name));
	// lock TODO !!!
	try {
		instance.lockInstance(_);
	} catch(ex) {
