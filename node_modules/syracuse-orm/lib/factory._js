...
function _serializeReference(_, instance, relMeta) {
	...
	// TEMP tweek : if the reference isn't there any more (like deleted) there is an instance but no data.
	// TODO: instance should be null in this case !
	if(!instance._data) return null;
	if(relMeta.isChild || relMeta.$serializeAll) {
		config.tracer && config.tracer("factory._serialize child reference: "+relMeta.name);
	...
	function _getRel(_, instance, name, relation){
		...
		if (relation.isPlural) {
			...
			if (!relation.isChild && val && val.$key && !val.$loaded) {
				val.ensureLoaded(_);
				// if not loaded, the reference might not exists anymore (like it was deleted)
				if(!val.$loaded)
					val = null;
			}
