...
function _instanceParentOf(testParent, instance) {
	...
	while(parent._parent) { 
		if(parent._parent === testParent)
			return true;
		if(visited.indexOf(parent._parent.$uuid) >= 0)
			throw new Error("INTERNAL ERROR: Looping detected in getTopInstance");
				...
				if(sp && !sp.canExecuteService(_, instance._meta.name, name)) {
					...
					if (s) {
						if((s.$invocationMode === "async") && action.$trackingId) {
							...
							if(s.$capabilities && (s.$capabilities.indexOf("abort") >= 0)) 
								tracker.canAbort = true;
							tracker.start(_);
...
//		instance._data[this.property.name] = {};
	...
	function InstanceCollection(_, instance, name, relMeta, value) {
		...
		var self = this;
		if (Array.isArray(value)) 
			this._array = value.map_(_, function(_, elt) {
...
//			filter[collection._relMeta.$inv + (collection._relMeta.targetEntity.$relations[collection._relMeta.$inv].isPlural?".$keys":".$uuid")] = collection._parent.$uuid;
	...
	_collProto.deleteInstance = function(_, instanceId, isInverse) {
		...
		if(relInst) {
			...
			if(!isInverse && (this._relMeta.isComputed || (invRel && !invRel.isComputed))) {
				...
				if(invRel.isPlural)
					relInst[this._relMeta.$inv](_).deleteInstance(_, this._parent.$uuid, true);
				else
					relInst[this._relMeta.$inv](_, null);
				if(!this._relMeta.isChild)
	...
	_collProto.set = function(_, value, isInverse) {
		if(!value) 
			throw new Error("Cannot set an undefined value for a collection; use deleteInstance instead.");
	...
	_collProto.filter = function(_, options) {
		...
		//
		options = options || {};
		if(options.sdataWhere)
			options.jsonWhere = filterHelpers.sdataFilterToJson(options.sdataWhere);
		...
		self._array.forEach_(_, function(_, inst){
			...
			flows.eachKey(_, filter, function(_, filterKey, filterValue) {
				...
				while(term = filterItem.shift()) {
					...
					if((term !== "$uuid") && !tempInst._meta.$properties[term] && !tempInst._meta.$relations[term])
						throw new Error(locale.format(module, "unknownFilterTerm", term, tempInst._meta.name));
					if(term === "$uuid")
						add = add && (tempInst[term] === filterValue);
					else
...
//		config.tracer && config.tracer("factory.Instance().connection:"+sys.inspect(connection));
		...
		if(data) {
			...
			flows.eachKey(_, data, function(_, key,value) {
				...
				if (value && r && r.isPlural && Array.isArray(value)) {
					...
					if (value && r) {
						var rel = r;
						if (rel.isPlural) 
							self._data[key] = new InstanceCollection(_, self, key, rel, value);
						else {
							...
							if (!id && !value.$url)
								...
								self._data[key] = self.syncCreateChild(_, key, id, value);
						}
					}
					else {
						if (key[0] === "$") 
							...
							if(p) {
								if(p.isExternalStorage()) {
									...
									if(p.$isArray && Array.isArray(value))
										...
										self._data[key] = resourceHelpers.parseValue(p, _fixLocalized(p, value, self._meta));
								}
							} else
								self._data[key] = value;
						}
			...
			if(!data.$isSnapshot && !data.$childRecord && self._meta.$signed) {
				...
				if(!checksum.verify(toCheck))
					throw new Error(locale.format(module, "checksumVerifyError", self._meta.name, self.$uuid));
			}
		}
		//
		//this.$key = this.$uuid;
	...
	function _getRel(_, instance, name, relation){
		...
		if (relation.isPlural) {
			...
			if (!instance._data[name]) 
				instance._data[name] = new InstanceCollection(_, instance, name, relation);
...
/*		instance._controlling = property.name;
	...
	function _afterSetValue(_, instance, name, property, val, validateFunction) {
		...
		try {
			...
			// validation
			if (instance._meta.$properties[name] && !instance._disableValidate)
				validateFunction && validateFunction(_, instance, property, name, val);
			// control rule
			if (property.$control) 
				property.$control(_, instance, val);
		}
		...
		flows.eachKey(_, instance._meta.$relations, function(_, name, relation) {
			if (relation.hasOwnProperty("defaultValue")) {
				...
				if (typeof relation.defaultValue === "function") 
					defVal = relation.defaultValue(_, instance);
				else 
		...
		function _propagateDelete(_, inst) {
			Object.keys(inst._meta.$relations || {}).forEach_(_, function(_, rName) {
				...
				if(r.isChild || r.$cascadeDelete || r.$nullOnDelete) {
					if(r.isPlural)
						inst[rName](_).toArray(_).forEach_(_, function(_, item) {
							_onRelationDelete(_, r, item);
							if(r.$nullOnDelete)
								inst[rName](_).deleteInstance(_, item.$uuid);
						});
					else {
...
function _createWorkingCopy(_, context, factory, wcId) {
	...
	if(sp) {
		...
		if(!context.instanceId && !sp.canCreateClass(_, context.entity.name)) 
			return context.reply(_, 403, locale.format(module, "createForbidden", context.entity.name));
	}
...
exports.replyInstances = function(_, context, entity, parameters) {
	...
	// add rights
	if(!_addRights(_, ent, params))
		return context.reply(_, 403, locale.format(module, "readForbidden", ent.name));
	//
...
exports.fetchInstance = function(_, context) {
	...
	if(!_addRights(_, context.entity, context.parameters))
		return null;
	//
...
function _replyInstance (_, context, notFoundCallback) {
	...
	context.parameters = context.parameters || {};
	if(!_addRights(_, context.entity, context.parameters))
		return context.reply(_, 403, locale.format(module, "readForbidden", context.entity.name));
	//
