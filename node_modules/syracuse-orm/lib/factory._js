...
//		if(!this._meta.$relations[relationName]) console.log("factory.syncCreateChild: "+relationName+";"+sys.inspect(data));
		...
		var targetType = _getTargetEntity(this._db.model, r, data && data.$type);
...
exports.replyInstances = function(_, context, entity, parameters) {
	...
	if(ent.$fetchInstances) {
		...
		instances = (params.key && params.key.split(".")[0] === "gt") ? preliminary.slice(start, Math.min(start+count, listCount)) : preliminary.slice(-count);
	} else {
