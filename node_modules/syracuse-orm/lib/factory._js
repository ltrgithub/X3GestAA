...
function _getTargetEntity(model, relation, type) {
	return (relation.$isDynamicType && type) ? model.getEntity(type) : relation.targetEntity;
}
...
function _serializeReference(_, instance, relMeta) {
	if(relMeta.isChild || relMeta.$serializeAll) {
		...
		var val = instance[relMeta.name](_);
		return val && _serialize(_, val);
	} else {
		config.tracer && config.tracer("factory._serialize reference: "+relMeta.name);
		// try to avoid loading
		var d = instance._data[relMeta.name];
		if(!d) return null;
		//
		var res = {
			$uuid: d.$uuid,
		...
		var t = relMeta.targetEntity.$valueTemplate || new Template(relMeta.targetEntity.$key);
		config.tracer && config.tracer("factory._serialize reference: "+relMeta.name+"; value template is: "+sys.inspect(t));
		if(t && t.matches) {
			var matches = t.matches.map(function(e) {
				return e.substring(1, e.length - 1);
			});
			var mustLoad = false;
			matches.forEach_(_, function(_, match) {
				var v = res[match] = (match[0] === "$") ? d[match] : d[match](_);
				if(!v && !d.$loaded)
					mustLoad = true;
			});
			if(mustLoad) {
				config.tracer && config.tracer("factory._serialize reference: "+relMeta.name+"; must load instance");
				// ensure load
				var i = instance[relMeta.name](_);
				matches.forEach_(_, function(_, match) {
					res[match] = i[match](_);
				});
			}
		...
		if (relation.isPlural) {
			...
			if(collData.length)
				resource[key] = collData.map_(_, 
					function(_, elt) {
						var r = relation.$assoType ? _serialize(_, val.getAsso(_, elt.$uuid), false) : 
							_serialize(_, elt, !relation.isChild);
			...
			resource[key] = _serializeReference(_, instance, relation);
		}
	});
	//
				...
				value.forEach_(_, function(_, newElt, idx){
					...
					if (newElt.$isDeleted) {
						...
						if(isDelta) {
							...
							if(isCreated) oldElts.splice(newElt.$index, 0, actualItem);
						} else
	...
	if(newColl.some_(_, function(_, newElt, i) {
		...
		delta[key] = newColl.map_(_, function(_, e) {
			if(e.$isDeleted)
				...
				return _serialize(_, e, !relation.isChild);
		});
...
//		instance._data[this.property.name] = {};
	...
	function InstanceCollection(instance, name, relMeta, value) {
		...
		var self = this;
		if (Array.isArray(value)) 
			value.forEach(function(elt, i){
				var e = self._data[elt.$uuid] = instance.syncCreateChild(self._relMeta.name, elt.$uuid, elt);
				e.$index = i;
			});
		else 
...
//			filter[collection._relMeta.$inv + (collection._relMeta.targetEntity.$relations[collection._relMeta.$inv].isPlural?".$keys":".$uuid")] = collection._parent.$uuid;
	...
	_collProto.toArray = function(_, excludeDeleted) { 
		...
		flows.eachKey(_, self._data, function(_, key, value){
			//console.log("(1145) key: "+key+"; value: "+sys.inspect(value));
		...
		result.sort(function(a,b) {
			...
			if(!self._relMeta.$canReorder && self._relMeta.defaultOrder && self._relMeta.defaultOrder.length)
							...
							if(p) {
								if(p.isExternalStorage()) {
									...
									if(p.$isArray && Array.isArray(value))
										...
										self._data[key] = resourceHelpers.parseValue(p, value);
								}
							} else
								self._data[key] = value;
						}
...
//		if(!data) child._initialize(_, this._context);
	...
	_proto.syncCreateChild = function(relationName, key, data, forAsso) {
		...
		var targetType = _getTargetEntity(this._db.model, this._meta.$relations[relationName], data && data.$type);
		...
		// !!! TODO: for compat set $uuid=$key but it's not right for some cases !!!
...
exports.replyInstances = function(_, context, entity, parameters) {
	...
	instances.forEach_(_, function(_, instance){
		resources.push(_serialize(_, instance, (["$query","$lookup","$select","$bulk"].indexOf(context.representation.type) >= 0)));
	});
...
function _createInstance(_, context, factory, batchRequest) {
	...
	if (context.batchResult) {
		instance = _instanceById(_, context);
		if (instance || instance.instanceId && (instance.instanceId== context.instanceId)) {
...
exports.createInstance = _createInstance;
...
exports.updateInstance = _updateInstance;

function _deleteInstance(_, context) {
	if (context.instanceId) {
		var instance = _instanceById(_, context);
		//var instance = context.db.fetchInstance(_, context.entity, context.instanceId, (context.representation && context.representation.type));
		if (context.batchResult && (!instance || !instance.instanceId)) {
			context.reply(_, 410);
			return;
		}
		if (!instance) {
