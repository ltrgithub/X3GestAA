...
function _securityError(request, code) {
	console.error("[SECURITY]", "[" + code + "]", request.method, request.url, request.headers);
...
function _safeParseRequestBody(_, request, payload) {
	...
	if (request && request.headers && request.headers["content-type"] !== "application/json") {
		throw _securityError(request, "x-03928c8394a4");
	}
	// if (payload) {
	// 	return jsurl.parse(payload);
	// }	
	return JSON.parse(request.readAll(_));
}
