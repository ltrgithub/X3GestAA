			...
			if (Array.isArray(value)) {
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (newElt.$isDeleted) {
						...
						// #4928: Factory security
						var _inst = collection.get(_, k);
						if (_inst) {
							if (!hasFactoryRights(_, _inst, key)) {
								allowedDelta = false;
								return;
							}
...
function _instanceById(_, context) {
	...
	if (context.parameters && context.parameters.where) {
		...
		if (context.isExpressionId) {
			...
			if (context.entity.$withCache !== false) {
				var cached = _fromCache(context.instanceId) || _toCache(_, context.db, context.entity, context.instanceId);
