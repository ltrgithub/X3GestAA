	...
	if(relMeta.isChild || relMeta.$serializeAll) {
		...
		if(t && t.matches) {
			...
			matches.forEach(function(match) {
				var v = match[0] === "$" ? d[match] : d._data[match];
//				console.log("matching "+match+"=" + v);
				if(v)
					res[match] = v;
				else
					mustLoad = true;
...
function _applyDelta(_, instance, delta){
	...
	flows.eachKey(_, delta, function(_, key, value){
		...
		if (r && r.isPlural && Array.isArray(value)) {
...
//		op.$title = "Save";
		...
		// TODO: this kind of action should be part of an event framework (onAfterSave)
		var indexer = new searchIndex.ElasticIndex(self.getEndpoint(_));
		indexer.updateInstance(null, self, {
			tracer: config.tracer
		});
		flows.eachKey(_, self._reindexInst, function(_, k, inst) {
			indexer.updateInstance(null, inst, {
				tracer: config.tracer
			});
		});
		//
