...
function _canReadInstance(_, instance) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	if (sp) {
		factoryTracer.debug && factoryTracer.debug("factory._canReadInstance found security profile: " + sp.code(_));
		...
		if (typeof r === "boolean") {
			...
			r = sp.replacePredefinedVars(_, r);
			//
			factoryTracer.debug && factoryTracer.debug("filters.canReadInstance add where: " + r);
			return instance.match(_, parser.Parser.parse(r));
		}
	}
	//
