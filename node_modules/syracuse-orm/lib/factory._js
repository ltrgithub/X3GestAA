		...
		if (filter.jsonWhere) {
			...
			if (testInstance) {
				if (propNames.length === 1) {
					instance.$addError(locale.format(module, "uniqueKeyViolation", instance._meta.name), property.name, undefined, undefined, "UNIQUE_KEY_VIOLATION");
				} else {
					instance.$addError(locale.format(module, "uniqueKeysViolation", instance._meta.name, propNames.join(",")), property.name, undefined, undefined, "UNIQUE_KEY_VIOLATION");
				}
				return instance.$validated[name] = false;
			}
		}
...
function _createInstance(_, context, factory, batchRequest) {
	...
	if (sp) {
		config.tracer && config.tracer("filters._createWorkingCopy found security profile: " + sp.code(_));
		if (!context.instanceId && !sp.canCreateClass(_, context.entity.name, true)) return context.reply(_, 403, locale.format(module, "createForbidden", context.entity.name));
	}
	// create instance
	var instance = null;
