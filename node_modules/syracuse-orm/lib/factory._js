...
var nocache = nodeconfig && nodeconfig.config && nodeconfig.config.hosting && nodeconfig.config.hosting.nocache;
...
var traceFactorySecurity; // = console.log;
function hasFactoryRights(_, _inst, key) {
	traceFactorySecurity && traceFactorySecurity("========");
	traceFactorySecurity && traceFactorySecurity("Check right on current instance [" + _inst._meta.name + "] for relation [" + key + "]");

	var hasRight = true;
	// #4928: Factory security
	if (key) {
		if (_inst._relation && _inst._parent) {
			traceFactorySecurity && traceFactorySecurity("Check right on parent for relation [" + _inst._relation.name + "]");
			hasRight = hasFactoryRights(_, _inst._parent, _inst._relation.name);
			traceFactorySecurity && traceFactorySecurity("Rights on parent : " + hasRight);
			if (!hasRight) {
				_inst.$diagnoses = helpers.object.clone(_inst._parent.$diagnoses, true);
				return false;
			}
		}
		if (hasRight && (_inst._meta.$properties.hasOwnProperty(key) || (_inst._meta._$relations && _inst._meta._$relations[key]))) {
			...
			traceFactorySecurity && traceFactorySecurity("Excludes: " + JSON.stringify(factoryExcludes, null, 2));
			if (factoryExcludes && factoryExcludes.indexOf(key) === -1) {
				if (_inst.$factory && !_canModifyFactoryInstance(_, _inst)) {
					_inst.$addError(locale.format(module, "updateFactoryPropertyForbidden", _inst._meta._allProperties[key].title, _inst.getEntity(_).title, _inst.$factoryOwner));
					traceFactorySecurity && traceFactorySecurity("REJECTED");
					return false;
				}
			}
		}
		...
		if (key === "$factory") {
			if (_inst.$factory == null && !_canModifyFactoryInstance(_, _inst, true)) {
				_inst.$addError(locale.format(module, "createFactoryForbidden"));
				traceFactorySecurity && traceFactorySecurity("CAN'T CREATE FACTORY");
				return false;
			} else if (_inst.$factory && !_canModifyFactoryInstance(_, _inst)) {
				_inst.$addError(locale.format(module, "ownFactoryForbidden", _inst.getEntity(_).title, _inst.$factoryOwner));
				traceFactorySecurity && traceFactorySecurity("CAN'T APPROPRIATE FACTORY");
				return false;
			}
		}
	} else if (_inst.$factory && !_canModifyFactoryInstance(_, _inst)) {
		_inst.$addError(locale.format(module, "updateFactoryForbidden", _inst.getEntity(_).name, _inst.$factoryOwner));
		hasRight = false;
	}
	traceFactorySecurity && traceFactorySecurity(hasRight ? "GRANTED" : "REJECTED");
	traceFactorySecurity && traceFactorySecurity("==========!");
	return hasRight;
}
...
*/
	...
	// #4928: Create snapshot before applying delta
	// create sys snapshot
			...
			if (Array.isArray(value)) {
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (newElt.$isDeleted) {
						...
						if (isDelta || isCreated) {
							// #4928: Factory security
							if (!hasFactoryRights(_, actualItem, key)) {
								allowedDelta = false;
								return;
							}
						}
...
_collProto.deleteInstance = function(_, instanceId, isInverse) {
	...
	if (relInst) {
		...
		if (!hasFactoryRights(_, relInst, this._relName)) {
			return;
		}
...
function Factory(entityMeta) {
	...
	_proto.deleteSelf = function(_) {
		...
		var self = this;
		if (self.$factory && !_canModifyFactoryInstance(_, self)) {
...
exports.updateWorkingCopy = function(_, context, instance, delta, opt) {
	...
	var deltaApplied = _applyDelta(_, options.updInstance || instance, delta);
...
function _updateInstance(_, context, data, instance, opt) {
	...
	// #4928
	//	if (instance.$factory && !_canModifyFactoryInstance(_, instance)) {
	//		config.tracer && config.tracer("factory.updateFactoryInstance forbidden will reply 403");
	//		return context.reply(_, 403, locale.format(module, "updateFactoryForbidden", instance.getEntity(_).name, instance.$factoryOwner));
	//	}
	// rights management
	...
	// save
	try {
		var resource = instance.save(_, params);
