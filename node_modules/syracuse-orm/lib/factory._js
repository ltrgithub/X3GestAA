...
function canUpdateRelation(_, _inst, _rel, _key, isInv) {
	...
	if (_rel && _inst && _inst._meta._$relations && _relInfo) {
		...
		if (sp && _inst.$factoryOwner === protectOnly && _inst.$factoryOwner === _rel.$factoryOwner && sp.factoryOwner(_) !== protectOnly) {
			var _instTpl = _inst._meta.$valueTemplate && _inst._meta.$valueTemplate.resolve(_inst.serializeInstance(_));
...
function Factory(entityMeta) {
	...
	function _prepareSetValue(_, instance, name, property, val, validateFunction) {
		...
		if (oldVal && instance._meta.$relations && instance._meta.$relations[name]) {
			...
			if (oldVal.$uuid !== val.$uuid && !canUpdateRelation(_, instance, oldVal, name, true)) return false;
		}
