...
exports.replyInstances = function(_, context, entity, parameters) {
	...
	if (ent.$fetchInstances) {
		var instancesFecthed = ent.$fetchInstances(_, context, params);
		if (instancesFecthed.$diagnoses) {
			context.replyResource(_, 500, instancesFecthed);
		} else {
			var preliminary = (ent.$sortInstances || context.sortInstancesArray)(_, context.filterInstancesArray(_, instancesFecthed), params, ent.defaultOrder);
