...
function _getPropertiesResourceMeta(_, resource, instance) {
	...
	if (instance._meta.$links) {
		if (typeof instance._meta.$links === "function") {
			resource.$links = instance._meta.$links(_, instance);
		} else {
...
exports.updateWorkingCopyLocalizedProp = function(_, context, instance, propertyName, delta) {
	var newDelta = {};
	newDelta[propertyName] = ((delta && delta.$values) || []).reduce(function(prev, locVal) {
		prev[locVal.$locale.toLowerCase()] = locVal.$value;
		return prev;
	}, {});
	exports.updateWorkingCopy(_, context, instance, newDelta, {
		forceReply: [propertyName]
	});
};

exports.updateWorkingCopy = function(_, context, instance, delta, options) {
	// rights management
