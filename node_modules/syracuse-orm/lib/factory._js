...
function _serializeReference(_, instance, relMeta) {
	...
	if (relMeta.$variants) {
		...
		varType = instance.$variantType || _getVariantType(relMeta.$variants, instance.getEntity(_).name);
	}
	if (relMeta.getIsChild(varType) || relMeta.$serializeAll) {
		config.tracer && config.tracer("factory._serialize child reference: " + relMeta.name);
		var res = _serialize(_, instance);
...
function _internalSerialize(_, instance, shallow, sync) {
	...
	if (sync > 1) {
		...
		resource.$endpoint = instance.$endpoint;
...
*/
		...
		if (resource.$url) {
			...
			result = _valueToInstance(_, instance._db, relation.getTargetEntity(variantType), resource);
			if (!result) {
				instance.$addDiagnose("warning", locale.format(module, "referenceNotFound", relation.name), relation.name);
				options.resolved = false;
				return;
			...
			if (Array.isArray(value)) {
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (rel.$variants) {
						...
						if (Object.keys(newElt).length > 1) throw new Error(locale.format(module, "invalidVariantCount", Object.keys(newElt).length));
					...
					if (newElt.$isDeleted) {
						...
						if (collection._relMeta.getIsChild(actualItem.$variantType)) _applyDelta(_, actualItem, newElt);
						// order management
						if (isDelta) {
				...
				if (rel) {
					if (value) {
						...
						if (rel.getIsChild(varType)) {
							...
							if (!instance[key](_)) {
								instance[key](_, instance.createChild(_, key, value.$uuid, null, false, varType));
							}
							...
							// reference change
								instance[key](_, instance.createChild(_, key, k, null, false, varType));
						}
...
function _findError(_, instance) {
	...
	if ((obj = instance._meta.$relations) && Object.keys(obj).some_(_, function(_, key) {
		...
		if (rel.getIsChild()) // TODO: getIsChild parameter ? Important if $variants //
			var content = instance[key](_);
		if (rel.isPlural) {
			return content.toArray(_, true).some_(_, function(_, item) {
...
function _validateProperty(_, instance, property, name, value) {
	...
	if (!instance._parent) {
		...
		if (filter.jsonWhere) {
			...
			if (testInstance) {
				instance.$addError(locale.format(module, "uniqueKeyViolation", instance._meta.name, propNames.join(",")), property.name, undefined, undefined, "UNIQUE_KEY_VIOLATION");
				return instance.$validated[name] = false;
			}
		}
	}
...
function Factory(entityMeta) {
	...
	function _setRel(_, instance, name, property, val, isInverse, variantName) {
		...
		if (property.isPlural) {
			...
			if (val && property.$variants) val.$variantType = val.$variantType || variantName || _getVariantType(property.$variants, val.getEntity(_).name);
			//
			var oldVal = instance._data[name];
			instance._data[name] = val;
			if (!isInverse && val) {
	...
	forEachKey(entityMeta.$relations, function(name, relation) {
		_proto[name] = function(_, val, isInverse, variantName) {
			if (arguments.length === 1) {
				...
				_setRel(_, this, name, relation, val, isInverse, variantName);
			}
		};
	});
			...
			if (this.$save) {
				...
				if (this._noIncreaseTick) {
					...
					if (syncInfo) {
						...
						this.$endpoint = this._meta.endpoint(_);
	...
	_proto.deleteSelf = function(_) {
		...
		var syncInfo = this._meta._syncInfo;
		if (syncInfo && this.$syncUuid && !this._noIncreaseTick) {
