...
function Factory(entityMeta) {
	...
	*/
	...
	_proto.createChild = function(_, relationName, key, data, loadOnly) {
		...
		var targetType = _getTargetEntity(_, md, r, t);
		// DEBUG
		if (targetType) {
			...
			if (!r.$isDynamicType && r.targetEntity && targetType.name !== r.targetEntity.name) {
				console.error("FATAL FACTORY (2528) targetType: " + targetType.name + " !== r.targetEntity " + r.targetEntity.name);
			}
		}
		// DEBUG
		var elt = targetType.factory.syncCreateInstance(_, data, self._db, self._context);
		// DEBUG
