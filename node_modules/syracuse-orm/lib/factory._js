...
var config = {};
var factoryTracer = require("syracuse-trace/lib/helper").getTracer("orm.factory");
...
function hasFactoryRights(_, _inst, key, noDiag) {
	...
	function checkInstanceRights(_) {
		...
		if (hasRight && instmeta && _inst.$factory && instmeta.$properties.hasOwnProperty(key)) {
			...
			var isLocalized = _inst._meta.$properties.hasOwnProperty(key) && _inst._meta.$properties[key].$isLocalized;
...
function _canUpdateInstance(_, instance) {
	...
	if (sp) {
		...
		if (typeof r === "boolean") {
			...
			var res = instance.match(_, parser.Parser.parse(r));
			if (!res) {
...
function _safeCall(_, instance, propName, fn, field, val) {
	...
	if (!instance[field]) {
		...
		if (instance[field].indexOf(id + propName) >= 0) {
...
_collProto.toArray = function(_, excludeDeleted) {
	...
	if (!self._relMeta.getIsChild() && !self._ordered && !self._relMeta.$canReorder && self._relMeta.defaultOrder && self._relMeta.defaultOrder.length) {
		factoryTracer.debug && factoryTracer.debug("factory._toArray sort by: " + sys.inspect(self._relMeta.defaultOrder) + "; original array: " + sys.inspect(self._array.map(function(r) {
			return {
				$uuid: r.$uuid,
				_data: r._data,
				$index: r.$index,
...
streamerProto.init = function(_, fileName) {
	...
	if (fileName != null && fileName !== "") this._store.setFile(_, fileName);
};
...
streamerProto.createWorkingCopy = function(_, isDeleted) {
	...
	if (isDeleted && this.fileExists(_)) this.deleteFile(_);
...
streamerProto.createWritableStream = function(_, options) {
	...
	options.fsName = this._store.fileName;
...
exports.updateWorkingCopy = function(_, context, instance, delta, opt) {
	...
	function manageNonPersistentCollectionItem(_) {
		...
		Object.keys(instance._meta._$relations).forEach_(_, function(_, key) {
			if (delta[key] && Array.isArray(delta[key])) {
				...
				delta[key].forEach(function(item) {
					mapDeltaUuid.push(item.$uuid);
				});

				if (instance._meta._$relations[key].$isPlural) {
	...
	if (delta.$actions && delta.$actions.$save && delta.$actions.$save.$isRequested) {
		...
		if (!instance._meta.$isPersistent) { // SAM 110110 - Search Index Management run for language connection also
			manageNonPersistentCollectionItem(_);
		}
