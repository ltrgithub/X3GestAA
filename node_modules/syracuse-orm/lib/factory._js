...
function _getTranslatedString(stringResources, parts, combineParts) {
	...
	console.log(">>>>>>GTS " + require('util').format(parts) + " " + combineParts);
...
function hasFactoryRights(_, _inst, key, noDiag) {
	...
	function checkInstanceRights(_) {
		...
		if (hasRight && _inst && (_inst._meta.$properties.hasOwnProperty(key) || (_inst._meta._$relations && _inst._meta._$relations[key]))) {
...
function _getPropertiesResourceMeta(_, resource, instance) {
	...
	function _getTranslatedString2(parts) {
		if (!stringRes) {
			stringRes = entity.contract && entity.contract.resources2 && entity.contract.resources2();
			if (!stringRes) {
				stringRes = entity.contract.resources && entity.contract.resources();
				if (!stringRes) return "";
				else stringRes = [stringRes];
			}
		}
		console.log(">>>>>>GTS " + require('util').format(parts));
		for (var i = 0; i < parts.length; i++) {
			var j = stringRes.length;
			var key = parts.slice(i).join(".");
			while (--j) {
				var str = stringResources[j][key];
				//console.log("resource for : "+parts.slice(i).join(".")+"="+str);
				if (str) return str;
			}
		}
		...
		if (typeof instance._meta.$links === "function") {
			...
			Object.keys(lks).forEach(function(lk) {
				...
				if (resource.$links[lk]) {
					resource.$links[lk].$title = _getTranslatedString2([entity.alias, "$links", lk, "$title"]) || resource.$links[lk].$title || lk;
					resource.$links[lk].$description = _getTranslatedString2([entity.alias, "$links", lk, "$description"]) || resource.$links[lk].$description || null;
				}
			});
		} else {
				...
				if (typeof link === "function") {
					...
					if (l) {
						...
						if (resource.$links[key]) {
							resource.$links[key].$title = _getTranslatedString2([entity.alias, "$links", key, "$title"]) || resource.$links[key].$title || key;
							resource.$links[key].$description = _getTranslatedString2([entity.alias, "$links", key, "$description"]) || resource.$links[key].$description || null;
						}
					}
				}
			});
