...
//		if(!this._meta.$relations[relationName]) console.log("factory.syncCreateChild: "+relationName+";"+sys.inspect(data));
	...
	function _fireBeforeSave(_, self) {
		self._meta && self._meta.$events && self._meta.$events.$beforeSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		self._db.model && self._db.model.$events && self._db.model.$events.$beforeSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		// can save ?
		self._meta && self._meta.$events && self._meta.$events.$canSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		self._db.model && self._db.model.$events && self._db.model.$events.$canSave.forEach_(_, function(_, h) {
			h(_, self);
		});
	...
	function _fireAfterSave(_, self) {
		self._meta && self._meta.$events && self._meta.$events.$afterSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		self._db.model && self._db.model.$events && self._db.model.$events.$afterSave.forEach_(_, function(_, h) {
			h(_, self);
		});
		// notify child instances
		self._meta && flows.eachKey(_, self._meta.$relations, function(_, relName, rel) {
