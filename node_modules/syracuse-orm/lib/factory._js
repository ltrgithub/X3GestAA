...
function _replacePredefinedVars(_, sdata) {
	var r = sdata;
	...
	r = r.replace("{$profile.user.$uuid}", up.user(_).$uuid);
...
function _addRights(_, ent, params) {
	...
	if (sp) {
		...
		if (typeof r === "boolean") {
			...
			r = _replacePredefinedVars(_, r);
			// 
			config.tracer && config.tracer("filters.addRights add where: " + r);
...
function _canDeleteInstance(_, instance) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	if (sp) {
		config.tracer && config.tracer("factory._canDeleteInstance found security profile: " + sp.code(_));
...
function _getTargetEntity(_, model, relation, type) {
	var targetType;
...
function _getPropertiesResourceMeta(_, resource, instance) {
	function _internalGetMeta(_, key, prop, isRelation) {
		...
		if (prop.$filterRepresentation && (typeof prop.$filterRepresentation === "function")) {
			...
			if (res_repr) resource.$properties[key].$links = {
				$prototype: res_repr
			};
		}
	}
...
function _createSnapshot(_, instance, type) {
	...
	//	var snapshot = new Instance(_, copy, _clone(instance.$properties), instance._db, instance._context);
	...
	flows.eachKey(_, delta, function(_, key, value) {
		...
		if (r && r.isPlural) {
			if (Array.isArray(value)) {
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (!collection._relMeta.isChild) {
						if (newElt.$url) {
							if ((!newElt.$uuid && !newElt.$key) || collection._relMeta.$isDynamicType) {
						...
						} else if (!newElt.$uuid && !newElt.$key) {
							// try to fetch the instance by $value
							resolvedInstance = _valueToInstance(_, instance._db, collection._relMeta.targetEntity, newElt);
							if (!resolvedInstance) {
								instance.$addDiagnose("warning", locale.format(module, "referenceNotFound", collection._relMeta.name), collection._relMeta.name);
								return;
							}
					...
					if (newElt.$isDeleted) {
						...
						if (!actualItem) {
							actualItem = resolvedInstance || instance.createChild(_, key, k, r.$inlineStore && newElt);
							if (isDelta) actualItem.$index = newElt.$index;
							collection.set(_, actualItem);
							isCreated = true;
						}
						if (actualItem.$isDeleted) actualItem.$isDeleted = false;
						if (collection._relMeta.isChild) _applyDelta(_, actualItem, newElt);
						// order management
						if (isDelta) {
			...
			if (key[0] === '$') {
				...
				if (key === "$properties") for (var d in delta.$properties)
				...
				if (rel) {
					if (value) {
						if (rel.isChild) {
							...
							if (!value.$uuid) {
								value = _valueToInstance(_, instance._db, rel.targetEntity, value);
								if (!value) {
...
function _cleanupDeleted(_, instance, shallow) {
	...
	flows.eachKey(_, instance._meta.$relations, function(_, key, rel) {
		if (instance._data[key]) if (rel.isPlural) {
			...
			if (instance._data[key]._data) flows.eachKey(_, instance._data[key]._data, function(_, elemKey, elemInst) {
				...
				} else if (!shallow || rel.isChild) _cleanupDeleted(_, elemInst, true);
			});
					...
					if ((s.$invocationMode === "async") && action.$trackingId) {
						...
						var tracker = sdataAsync.create(context, function(_, context) {
							try {
								...
								tracker.addError(e.message, e.stack);
							}
						}, true);
...
function _computeDelta(_, instance, snapshotType, snapshotValue, fullLocale, forceDelta) {
	...
	if (snapshot) {
		...
		if (instance.hasOwnProperty("$index")) if (instance.$index !== snapshot.$index) delta.$index = instance.$index;
	}
...
function _errorCount(instance) {
	if (instance.$properties && (typeof instance.$properties === "object")) return (Object.keys(instance.$properties).reduce(function(prev, current) {
...
function _existsError(_, instance) {
	...
	//
	var hasErr = false;
	if (instance.$properties) flows.eachKey(_, instance.$properties, function(_, key, item) {
	...
	// recurse relations
	flows.eachKey(_, (instance._meta.$relations || {}), function(_, key, rel) {
		if (rel.isChild) if (rel.isPlural) {
...
function _safeCall(_, instance, propName, fn, field, val) {
	if (!instance[field]) {
		...
		if (instance[field].indexOf(propName) >= 0) throw new Error(field.substring(1) + " loop on " + propName);
	}
...
function _validateRelation(_, instance, relation, name, value) {
	...
	if (mandatory) if (!value) instance.$addError(name + " is mandatory", name);
}
...
function _validateProperty(_, instance, property, name, value) {
	...
	if (type) {
		...
		else type.validate(value, c, errors);
		if (errors.length) {
...
function Factory(entityMeta) {
	...
	function InstanceCollection(_, instance, name, relMeta, value) {
		this._data = null;
		...
		if (Array.isArray(value)) this._array = value.map_(_, function(_, elt) {
			return instance.syncCreateChild(_, self._relMeta.name, elt.$uuid, elt);
		});
		else for (var key in value) {
			if ((key[0] !== "$") && (key[0] !== "_")) {
				self._array.push(instance.syncCreateChild(_, self._relMeta.name, key, relMeta.isChild ? value[key] : null));
			}
		}
	}
	//
	...
	_collProto.toArray = function(_, excludeDeleted) {
		...
		//
		if (!self._relMeta.isChild && !self._ordered && !self._relMeta.$canReorder && self._relMeta.defaultOrder && self._relMeta.defaultOrder.length) {
			...
			if (!self._relMeta.isChild) self._array.forEach_(_, function(_, v) {
	...
	_collProto.deleteInstance = function(_, instanceId, isInverse) {
		...
		if (relInst) {
			...
			if (!isInverse && (this._relMeta.isComputed || (invRel && !invRel.isComputed))) {
				...
				if (!this._relMeta.isChild) this._parent.addRelatedInstance(relInst);
	...
	_collProto.set = function(_, value, isInverse) {
		...
		if (!isInverse && this._relMeta.$inv && value._meta.$relations[this._relMeta.$inv]) {
			...
			// to persist
			if (!this._relMeta.isChild && !value._meta.$relations[this._relMeta.$inv].isComputed) this._parent.addRelatedInstance(value);
		...
		self._array.forEach_(_, function(_, inst) {
			...
			flows.eachKey(_, filter, function(_, filterKey, filterValue) {
				...
				while (term = filterItem.shift()) {
					...
					if (term === "$uuid") add = add && (tempInst[term] === filterValue);
					else if (tempInst._meta.$properties[term]) {
						// is final prop
						...
						//
						break;
					} else if (tempInst._meta.$relations[term] && !tempInst._meta.$relations[term].isPlural) {
						// walk
	...
	// instance prototype
	function Instance(_, data, $, connection, context, setLoaded) {
		...
		var self = this;
		//
		forEachKey(self._meta.$functions, function(name, fn) {
			self[name] = fn.bind(self); // can bind it
		});
		forEachKey(self._meta.$methods, function(name, fn) {
		...
		if (data) {
			...
			flows.eachKey(_, data, function(_, key, value) {
				...
				if (value && r && r.isPlural && Array.isArray(value)) {
					self._data[key] = new InstanceCollection(_, self, key, r, value);
				} else if (value && r) {
					var rel = r;
					if (rel.isPlural) self._data[key] = new InstanceCollection(_, self, key, rel, value);
					else {
						var e = _getTargetEntity(_, self._db.model, rel, value.$type);
						var id = e && e.$key ? (new Template(e.$key)).resolve(value) : value.$uuid;
						// check null relation
						if (!id && !value.$url) self._data[key] = null;
						else self._data[key] = self.syncCreateChild(_, key, id, value);
					}
				} else {
					if (key[0] === "$") self[key] = value;
						...
						if (p) {
							if (p.isExternalStorage()) {
								...
								if (p.$isArray && Array.isArray(value)) self._data[key] = value.map(function(v) {
	...
	function _getRel(_, instance, name, relation) {
		...
		if (relation.$compute) {
			// FDB
			if (!instance._data[name]) instance._data[name] = new InstanceCollection(_, instance, name, relation);
		...
		if (relation.isPlural) {
			...
			if (instance._snapshotType && val) {
				...
				if (relation.isChild) _createSnapshot(_, val, instance._snapshotType);
			}
			// lazy load associations 
	...
	_proto.createChild = function(_, relationName, key, data, forAsso) {
		var child = this.syncCreateChild(_, relationName, key, data, forAsso);
		if ((!data && (this._meta.$relations[relationName].isChild || forAsso)) || !key) {
			child._initialize(_, this._context);
			child.$created = true;
		}
		//		if(!data) child._initialize(_, this._context);
		return child;
	};
	_proto.syncCreateChild = function(_, relationName, key, data, forAsso) {
		...
		var targetType = _getTargetEntity(_, this._db.model, r, t);
		var elt = targetType.factory.syncCreateInstance(_, data, this._db, this._context);
	...
	function _runEvents(_, events, self) {
		if (!events) return;
		events.forEach_(_, function(_, ev) {
			if (typeof ev === "function") ev(_, self);
			else if (ev.handler) ev.handler(_, self);
		});
	}

	function _fireBeforeSave(_, self) {
		_runEvents(_, self._meta && self._meta.$events && self._meta.$events.$beforeSave, self);
		_runEvents(_, self._db.model && self._db.model.$events && self._db.model.$events.$beforeSave, self);
		// can save ?
		_runEvents(_, self._meta && self._meta.$events && self._meta.$events.$canSave, self);
		_runEvents(_, self._db.model && self._db.model.$events && self._db.model.$events.$canSave, self);
		...
		self._meta && flows.eachKey(_, self._meta.$relations, function(_, relName, rel) {
			if (rel.isChild) {
				...
				if (!relInst) return;
				if (rel.isPlural) {
					relInst.toArray(_, false).forEach_(_, _fireBeforeSave);
				} else {
					_fireBeforeSave(_, relInst);
				}
			}
		});
	}
	...
	function _fireAfterSave(_, self) {
		_runEvents(_, self._meta && self._meta.$events && self._meta.$events.$afterSave, self);
		_runEvents(_, self._db.model && self._db.model.$events && self._db.model.$events.$afterSave, self);
		// notify child instances
		self._meta && flows.eachKey(_, self._meta.$relations, function(_, relName, rel) {
			if (rel.isChild && rel.targetEntity && rel.targetEntity.$events) {
				var relInst = self[relName](_);
				if (!relInst) return;
				if (rel.isPlural) {
					relInst.toArray(_, false).forEach_(_, _fireAfterSave);
				} else {
					_fireAfterSave(_, relInst);
				}
			}
		});
	}
	_proto.save = function(_, parameters, options) {
		...
		// global validation
		!self._meta.$ignoreValidateOnSave && _validateBeforeSave(_, this);
		// compute checksum
		var r;
		...
		if (!hasErrors) try {
			...
			if (this.$save) this.$save(_, op, parameters);
			else {
			...
			//
			if (!hasRelatedErrors) op.$diagnoses.splice(0, 0, {
				$severity: "success",
		...
		op.$isRequested = false;
		//
		var result = opt.shallowSerialize ? {} : _serialize(_, this);
		...
		// fire after save
		if (!hasErrors) _fireAfterSave(_, this);
		//
		this._relatedInst = {};
		//
		return result;
	...
	_proto.$resolvePath = function(_, path) {
		...
		while (seg = segs.shift()) {
			...
			if (self._meta.$properties[seg]) return self[seg](_);
	...
	_proto.$canDelete = function(_) {
		...
		return (!self._meta.$relations || !Object.keys(self._meta.$relations).filter(function(relName) {
			...
			if (rel.isPlural) return self[relName](_).toArray(_).some_(_, function(_, item) {
				return !item.$canDelete(_);
			});
			else return (!self[relName](_) || !self[relName](_).$canDelete(_));
	...
	_proto.deleteSelf = function(_) {
		function _onRelationDelete(_, rel, inst) {
			if (rel.isChild) _propagateDelete(_, inst);
			if (rel.$cascadeDelete) inst.deleteSelf(_);
		}
		...
		function _propagateDelete(_, inst) {
			Object.keys(inst._meta.$relations || {}).forEach_(_, function(_, rName) {
				...
				if (r.isChild || r.$cascadeDelete || r.$nullOnDelete) {
	...
	this.createInstance = function(_, data, connection, context) {
		var inst = new Instance(_, data, null, connection, context, data != null);
		//		var inst = new Instance(_, null, null, connection, context);
		if (!data) inst._initialize(_, context);
		//		else inst._load(_, data);
		return inst;
	};
	this.syncCreateInstance = function(_, data, connection, context) {
		return new Instance(_, data, null, connection, context);
	};
}
...
exports.createWorkingCopy = function(_, context, factory) {
	...
	if (instance._meta.$canSave !== false) {
		...
		res.$actions.$save.$isDisabled = hasSave;
...
exports.updateWorkingCopy = function(_, context, instance) {
	...
	_applyDelta(_, instance, delta);
	// execute actions
	var actions = _executeActions(_, context, instance);
	//
	instance.$etag++;
	instance.$updUser = (context && context.getUser(_) && context.getUser(_).login(_));
	//
	if (delta.$actions && delta.$actions.$save && delta.$actions.$save.$isRequested) {
		// save
		try {
			var resource = instance.save(_, delta.$actions.$save.$parameters);
...
function _replyInstance(_, context, notFoundCallback) {
	...
	// add rights
	context.parameters = context.parameters || {};
	if (!_addRights(_, context.entity, context.parameters)) return context.reply(_, 403, locale.format(module, "readForbidden", context.entity.name));
	//
	var instance = context.instance || _instanceById(_, context);
	if (instance) context.replyResource(_, httpHelpers.HttpStatus.OK, _serialize(_, instance));
...
function _updateInstance(_, context, batchRequest) {
	if (context.instanceId) {
		...
		if (!_canUpdateInstance(_, instance)) {
			...
			return context.reply(_, 403, locale.format(module, "updateForbidden", context.entity.name));
		}
		// apply delta
		instance.$sysSnapshot = null;
		instance._snapshotEnabled = true;
	...
	try {
		...
		brequest.$resources.forEach_(_, function(_, entry) {
			...
			try {
				...
				switch (op.method) {
					case "get":
						_replyInstance(_, context, entry);
						break;
					case "post":
