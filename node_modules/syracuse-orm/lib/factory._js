...
function Factory(entityMeta) {
	...
	this.syncCreateInstance = function(_, data, connection, context) {
		...
		if (createMetaName !== afterLoadMetaName) {
			console.error("FATAL ERROR !!!!! Meta after create " + createMetaName + " different of after load meta name " + afterLoadMetaName);
		}
		if (data && data.$type === "menuSubblock" && inst._meta.name !== "menuSubblock") {
			console.error("FATAL ERROR !!!!! Meta after create different of subblock " + data.$type + " different of after load meta name " + inst._meta.name);
		}
		//
		return inst;
	};
...
exports.createWorkingCopy = function(_, context, factory) {
	var delta = _safeParseRequestBody(_, context.request);
...
function _createInstance(_, context, factory, batchRequest) {
	...
	//
	var delta = batchRequest ? batchRequest : _safeParseRequestBody(_, context.request);
	//
