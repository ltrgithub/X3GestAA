...
// avoid infinite recursions when calling function. When function is called for the first time, an object for this function call is created which will always be deleted
// as soon as the function has finished. The value for the corresponding key is set to 'null' first.
// When function 'fn' should be called again before first invocation has finished, the pending invocation of 'fn' can be observed in the object. The value will be set to an array which contains
// a timeout, the function 'fn' and the callback and the value. Further invocations will append additional tripels of 'fn', value, callback. When after 2 seconds, the first invocation has not 
// finished, the timeout will send an error response to all callbacks. 
// When 'fn' has finished, all stored callbacks will receive the same result or error, so that 'fn' need not be called again for them. Only when there are pending invocations for other argument
// values, 'fn' will be called again.
// Consequence: When the function 'fn' calls itself recursively, the first invocation will wait for the second one by recursion, and the second invocation will wait for the first
// one by implementation of _safeCall. Therefore the second one will be stopped with an error after a timeout.
// The implementation allows 10 concurrent invocations of the computation function for the same field - when there are more, all callbacks will receive an error message.
function _safeCall(callback, instance, propName, fn, field, val) {

	// call callback not directly but within next tick
	function _nextTick(cb, error, result) {
		process.nextTick(function() {
			return cb(error, result);
		});
	}

	// allow 2 seconds to execute computation function when there are other invocations of computation function, otherwise stop other invocations with error
	function _setTimeout() {
		return setTimeout(function() {
			var refid = ref[id];
			if (refid) {
				ref[id] = null; // do not delete ref[id] because the computation is still in progress
				var error = new Error("Timeout for executing " + fn.name + " for " + propName);
				factoryTracer.error && factoryTracer.error("Timeout for executing " + fn.name + " for " + propName);
				for (var i = 3; i < refid.length; i += 3) {
					_nextTick(refid[i], error);
				}
			}
		}, 2000);
	}

	// when computation function has finished, return the result to invocations with same parameters and maybe invoke computation function again
	function _safeCallCbFunction(val0, callback0, fn0) {
		return function(err, result) {
			var refid = ref[id];
			// clear timeout first because invocation has finished
			if (refid) {
				clearTimeout(refid[0]);
			}
			if (err) {
				try { // try-catch block because otherwise callbacks may get lost
					instance.$addError(err.message + "\n" + err.safeStack, propName);
					factoryTracer.error && factoryTracer.error("Error executing " + fn.name + " for " + propName, err);
					result = undefined;
				} catch (e) {
					console.error("Error in _safeCall " + (e.safeStack || e));
				}
			}
			// finish calls with same argument
			if (refid) {
				for (var i = 3; i < refid.length; i += 3) {
					if (refid[i - 1] === val0 && refid[i - 2] === fn0) { // finish calls with same value of argument and same function
						_nextTick(refid[i], undefined, result);
						refid.splice(i - 2, 3); // remove these entries
						i -= 3;
					}
				}
				if (refid.length > 1) { // still an invocation of 'fn' with other value left
					var fn1 = refid[1];
					var val1 = refid[2];
					var cb1 = refid[3];
					refid.splice(1, 3);
					if (refid.length > 1) { // pending other functions: new timeout
						refid[0] = _setTimeout();
					} else ref[id] = refid = null; // only one invocation left
					fn1(_safeCallCbFunction(val1, cb1, fn1), instance, val1);
				} else
					delete ref[id]; // no invocations of 'fn' left (all have been handled): therefore no need to store key any more 
			} else
				delete ref[id]; // no further invocations of 'fn' available at all: therefore no need to store key any more 
			return callback0(undefined, result);
		};
	}


	var id = ((globals.context && globals.context.requestId) || 0) + propName;
	var ref = instance[field];
	if (!ref) {
		ref = instance[field] = {};
	}
	if (id in ref) { // function call in progress - store function, value, callback
		var refid = ref[id];
		if (refid) { // too many references
			if (refid.length > 31) {
				clearTimeout(refid[0]);
				ref[id] = null; // do not delete ref[id] because the computation is still in progress
				var error = new Error("Too many concurrent attempts to compute " + field.substring(1) + " for " + propName);
				for (var i = 3; i < refid.length; i += 3) {
					_nextTick(refid[i], error);
				}
				return callback(error);
			} else {
				refid.push(fn, val, callback);
				return;
			}
		} else {
			// first array element. Add timeout so that it will not block forever (e. g. infinite recursion)
			var to = _setTimeout();
			ref[id] = [to, fn, val, callback];
			return;
		}
	} else {
		ref[id] = null; // set marker that function call is in progress
	}
	fn(_safeCallCbFunction(val, callback, fn), instance, val);
}

exports._sc = _safeCall; // for unit tests
