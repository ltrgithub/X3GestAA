...
function canUpdateRelation(_, _inst, _rel, _key, mustThrow) {
	var _relInfo = _inst._meta._$relations[_key];
	if (_rel && _inst && _inst._meta._$relations && _relInfo) {
		var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
		// protect links between SAGE factory elements
		// for this instance related
		if (_inst.$factoryOwner === protectOnly && _inst.$factoryOwner === _rel.$factoryOwner && sp.factoryOwner(_) !== protectOnly) {
			var msg = locale.format(module, "protectedFactoryRef", _key, _inst.getEntity(_).name, _inst.$factoryOwner);
			if (mustThrow) throw new Error(msg);
			// Put diagnose
			_inst.$addError(msg);
			return false;
		}
...
function Factory(entityMeta) {
	...
	function _prepareSetValue(_, instance, name, property, val, validateFunction) {
		...
		if (oldVal) {
			...
			if (!canUpdateRelation(_, instance, oldVal, name, true)) return false;
		}
