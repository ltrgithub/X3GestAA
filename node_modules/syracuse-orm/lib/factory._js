	...
	//			collection._isDelta = (receivedUuids == null);
				...
				value.forEach_(_, function(_, newElt, idx){
					...
					if (newElt.$isDeleted) {
						...
						if (!actualItem) {
							actualItem = collection.set(_, resolvedInstance || instance.createChild(_, key, newElt.$uuid));
							isCreated = true;
						}
...
/*		var result = [];
	...
	_collProto.set = function(_, value, isInverse) {
		...
		if(this._relMeta.$isDynamicType) {
//			value.$type = value.getEntity().name;
//			config.tracer && config.tracer("factory.collection.set: dynamicType $type: "+value.$type);
//			config.tracer && config.tracer("factory.collection.set: dynamicType $url: "+value.$url);
		}
		//
...
//		if (property.$propagate) {
	...
	function _setRel(_, instance, name, property, val, isInverse) {
		...
		if (property.isPlural) {
			...
			instance._data[name] = val;
			if(property.$isDynamicType && val) {
//				val.$type = val.getEntity().name; 
//				config.tracer && config.tracer("factory._setRel dynamicType $type is : "+val.$type);
//				config.tracer && config.tracer("factory._setRel $url is : "+val.$url);
			}
			// replication
