...
function _safeCall(_, instance, propName, fn, field, val) {
	...
	try {
		...
		factoryTracer.debug && factoryTracer.debug("Error executing " + fn.name + " for " + propName + ": " + e.message + "\n" + e.stack);
	} finally {
...
function Factory(entityMeta) {
	...
	function Instance(connection) {
		//		factoryTracer.debug && factoryTracer.debug("factory.Instance().connection:"+sys.inspect(connection));
	...
	_proto.save = function(_, parameters, options) {
		...
		if (!hasErrors) try {
			...
			hasErrors = true;
			factoryTracer.debug && factoryTracer.debug("Save error stack: " + ex.stack);
		}
		//		op.$title = "Save";
		op.$isDisabled = true;
		op.$isRequested = false;
...
function _createWorkingCopy(_, context, factory, wcId, initialInstanceId) {
	...
	if (context.instanceId) {
		...
		try {
			...
			if (ex.httpStatus) {
				factoryTracer.debug && factoryTracer.debug("factory.createWC http error: " + ex.httpStatus + "=" + ex.message);
...
function _deleteInstance(_, context) {
	if (context.instanceId) {
		...
		factoryTracer.debug && factoryTracer.debug("factory.deleteInstance fetched instance: " + instance.$inspect(true));
