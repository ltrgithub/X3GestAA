...
*/
	...
	function hasFactoryRights(_, _inst, key) {
		if (factoryExcludes && factoryExcludes.indexOf(key) === -1) {
			if (_inst.$factory && !_canChangeFactoryInstance(_, _inst)) {
				_inst.$addError(locale.format(module, "updateFactoryPropertyForbidden", key, _inst.getEntity(_).name, instance.$factoryOwner));
				return false;
			}
		}
		return true;
	...
	var factoryExcludes = instance.getEntity(_).$factoryExcludes || [];
