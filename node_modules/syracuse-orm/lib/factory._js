...
function _serializeReference(_, instance, relMeta) {
	...
	if (relMeta.getIsChild(varType) || relMeta.$serializeAll) {
		...
		res = _serialize(_, instance, false, false, relMeta);
...
function _internalSerialize(_, instance, shallow, sync, relation) {
	if (!instance) return null;
...
function _serialize(_, instance, shallow, sync, relation) {
	if (!instance) return null;
	...
	//
	return _internalSerialize(_, instance, shallow, sync, relation);
}
...
function Factory(entityMeta) {
	...
	_proto.computeUrl = function() {
		if (this.$trackingId) return [this._db.baseUrl, "$workingCopies('" + this.$trackingId + "')"].join("/") + "?representation=" + this._meta.name + ".$edit";
...
exports.updateWorkingCopy = function(_, context, instance, delta, opt) {
	...
	_applyDelta(_, options.updInstance || instance, delta);
	// execute actions
	...
	if (delta.$actions && delta.$actions.$save && delta.$actions.$save.$isRequested) {
		...
		var res = options.fullSerialize ? instance.serializeInstance(_) : _computeDelta(_, instance, "$sysSnapshot", null, false);
		// activer save
...
exports.updateLocalizedProperty = function(_, context, instance, relInstance, propertyName, delta) {
	...
	_updateInstance(_, context, wcInstance, newDelta, {
