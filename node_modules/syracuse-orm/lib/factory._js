...
function hasFactoryRights(_, _inst, key, noDiag) {
	...
	function checkInstanceRights(_) {
		...
		var instmeta = _inst._meta;
		// condition "_inst.$factory" has been put in front because hasPropRights will only return false if this condition is true
		if (hasRight && _inst.$factory && (instmeta.$properties.hasOwnProperty(key) || (instmeta._$relations && instmeta._$relations[key]))) {
				...
				if (rel) {
					if (value) {
						...
						if (rel.getIsChild(varType)) {
							...
							// if (!instance[key](_)) { // always create a new child in order to call $propagate
							// #4928: Factory security
							...
							// remove and delete previous child assign
							instance[key](_) && instance[key](_).deleteSelf(_);
...
exports.updateWorkingCopy = function(_, context, instance, delta, opt) {
	...
	function _getObject(collElem) {
		if (collElem && Object.keys(collElem).length) {
			if (collElem.$key) // fist level
				return collElem;
			else
	...
	function _manageNonPersistentCollectionItem(_) {
		...
		Object.keys(instance._meta._$relations).forEach_(_, function(_, key) {
			if (delta[key] && Array.isArray(delta[key])) {
				if (delta[key].length > 0 && !_getObject(delta[key][0]).$index) {
