...
function hasFactoryRights(_, _inst, key) {
	...
	if (key && (_inst._relation || _inst._meta.$properties.hasOwnProperty(key)) || (key[0] === '$' && _inst[key])) {
		...
		if (factoryExcludes && factoryExcludes.indexOf(key) === -1) {
			// Relations
			if (_inst._relation && _inst._parent && _inst._parent.$factory && !_canModifyFactoryInstance(_, _inst._parent)) {
				_inst._parent.$addError(locale.format(module, "updateFactoryPropertyForbidden", _inst._relation.title, _inst._parent.getEntity(_).title, _inst._parent.$factoryOwner));
				return false;
			}
			// Properties
			else if (_inst.$factory && !_canModifyFactoryInstance(_, _inst)) {
				var propTitle = (key[0] === '$') ? key : _inst._meta._allProperties[key].title;
				_inst.$addError(locale.format(module, "updateFactoryPropertyForbidden", propTitle, _inst.getEntity(_).title, _inst.$factoryOwner));
				return false;
			}
		}
	} else if (_inst.$factory && !_canModifyFactoryInstance(_, _inst)) {
...
var _canModifyFactoryInstance = function(_, instance) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	return sp && sp.canUpdateFactoryInstance(_, instance);
};
