...
function _internalSerialize(_, instance, shallow, sync) {
	if (!instance) return null;
	...
	//
	if (sync) {
		var uuid = instance.$syncUuid;
		var url = instance.$url || instance.computeUrlShort();
	} else {
		var uuid = (instance.$syncUuid || instance.$uuid);
		var url = instance.$url || instance.computeUrl();
	}
	var resource = {
		$uuid: uuid,
		$key: instance.$key,
...
function _serialize(_, instance, shallow, sync) {
	if (!instance) return null;
	...
	//
	return _internalSerialize(_, instance, shallow, sync);
}
...
exports.setSyncUuid = function(_, context, replaceUuid) {
	...
	if (!replaceUuid) {
		...
		if (instance.$syncUuid !== context.syncUuid) {
			if (instance.$syncUuid) return context.reply(_, httpHelpers.HttpStatus.BadRequest, locale.format(module, "differentUuid", instance.) $syncUuid + " of the instance");
			var oldInstance = context.db.fetchInstance(_, context.entity, {
				jsonWhere: {
			...
			if (oldInstance) {
				...
				for (var i = 1; i < 10; i++) { // local UUID is not unique: try to find a better one
					...
					if (!oldInstance) context.syncUuid = tempUuid;
					break;
				}
		...
		var erg = context.reply(_, 200, erg);
		return erg;
	}
};
...
function Factory(entityMeta) {
	...
	_proto.computeUrlShort = function() {
		return [this._db.baseUrl, this._meta.plural + "('" + this.computeKey() + "')"].join("/");
	};
	...
	_proto.$addError = function(message, propName, links, stackTrace) {
	...
	/*	_proto.createChild = function(_, relationName, key, data) {
		var child = this.syncCreateChild(_, relationName, key, data);
		if ((!data && this._meta.$relations[relationName].isChild) || !key) {
	...
	*/
	//	_proto.syncCreateChild = function(_, relationName, key, data) {
	_proto.createChild = function(_, relationName, key, data, loadOnly) {
		...
		var tgType = _getTargetEntity(_, md, r, t);
		// DEBUG
		if (tgType) {
		...
		elt._relation = r;
		//
		if (!loadOnly && ((!data && r.isChild) || !key)) {
	...
	_proto.save = function(_, parameters, options) {
		...
		if (!hasErrors) try {
			...
			if (this.$save) {
				...
				// increase the tick				
				var syncInfo = this._meta._syncInfo;
				if (syncInfo) {
					if (this.$created) {
						var syncUuid;
						var i = 0;
					...
					// TODO: here must be the atomic counter
					var syncEnt = syncInfo.getEntity(_);
					// var syncInfo = syncInfo._db.fetchInstance(_, syncEnt, { jsonWhere: { name: this._meta.name}});
					var oldValue = syncInfo.tick(_);
					syncInfo.tick(_, oldValue + 1);
					syncInfo.save(_);
					this.$tick = oldValue;
					this.$endpoint = syncInfo.endpoint(_);
				}
			...
			summary = Object.keys(ent.$relations).reduce_(_, function(_, prev, item) {
				...
				if (prop.isPlural) {
					val = self[item](_).toArray(_).map_(_, function(_, it) {
						return prop.isChild ? it.$getSummary(_, true) : (it.description ? it.description(_) : "?");
					}).join(", ") || "";
				} else {
					var it = self[item](_);
					val = prop.isChild ? it.$getSummary(_, true) : (it.description ? it.description(_) : "?");
				}
				var res = [];
				if (prev) res.push(prev);
				if (val) res.push(prop.getTitle() + "=(" + val + ")");
...
exports.createWorkingCopy = function(_, context, factory) {
	...
	if (delta) {
		...
		if (delta.$actions && delta.$actions.$save && delta.$actions.$save.$isRequested) {
			hasSave = true;
			// save
			console.log("save instance " + instance._meta.$titleTemplate);
...
exports.replyInstances = function(_, context, entity, parameters) {
	...
	instances.forEach_(_, function(_, instance) {
		resources.push(_serialize(_, instance, (queryFacets.indexOf(context.representation.type) >= 0), context.useSyncUuid));
	});
...
function _replyInstance(_, context, notFoundCallback) {
	...
	if (instance) {
		...
		if (context.sendUrlOnly) {
			...
			resource = _serialize(_, instance, null, context.useSyncUuid); // useSyncUuid: force SData compliant $uuid
		}
		context.replyResource(_, httpHelpers.HttpStatus.OK, resource);
...
function _createInstance(_, context, factory, batchRequest) {
	...
	console.log("create instance " + instance._meta.$titleTemplate);
