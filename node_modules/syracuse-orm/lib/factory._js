...
function Factory(entityMeta) {
	...
	_collProto.load = function(_, value) {
		...
		if (Array.isArray(value)) this._array = value.map_(_, function(_, elt) {
			return instance.createChild(_, self._relMeta.name, elt.$uuid, elt);
		});
		else for (var key in value) {
			if ((key[0] !== "$") && (key[0] !== "_")) {
				self._array.push(instance.createChild(_, self._relMeta.name, key, self._relMeta.isChild ? value[key] : null));
			}
		}
	};
	/// -------------
			...
			flows.eachKey(_, data, function(_, key, value) {
				...
				if (value && r && r.isPlural && Array.isArray(value)) {
					...
					if (rel.isPlural) {
						...
						else self._data[key] = self.createChild(_, key, id, value);
					}
				} else {
	...
	_proto.syncCreateChild = function(_, relationName, key, data) {
		...
		// DEBUG
		if((relationName === "vignettes") && (targetType.factory._meta.name !== targetType.factory._entityMeta.name)) {
			console.error("FATAL FACTORY VIGNETTES FACTORY TYPEOF META ("+typeof targetType.factory._entityMeta.name+") != _META ("+typeof targetType.factory._meta.name+") - (2603)");
			console.error("FATAL FACTORY VIGNETTES FACTORY META ("+targetType.factory._entityMeta.name+") != _META ("+targetType.factory._meta.name+") - (2604)");
		}
		if((relationName === "vignettes") && targetType && (targetType.name !== "dashboardVignette")) {
			console.error("FATAL FACTORY VIGNETTES TYPE (2605): "+sys.inspect(targetType));
			console.error("FATAL FACTORY VIGNETTES FACTORY META (2606): "+sys.inspect(targetType.factory._meta));
		}
		// DEBUG
