...
//	console.log("before format: "+val+"; after format: "+resourceHelpers.formatValue(prop, val));
		...
		if (Array.isArray(value)) {
			...
			if (key[0] == '$') {
				...
				if (instance._meta.$relations && instance._meta.$relations.hasOwnProperty(key)) {
					if (value) {
						if(instance._meta.$relations[key].isChild) {
				...
				if(instance._data[key]._data)
					flows.eachKey(_, instance._data[key]._data, function(_, elemKey, elemInst) {
						if(elemInst.$isDeleted) {
							...
							console.log("resulting data:"+sys.inspect(instance._data[key]._data));
						} else
			...
			flows.eachKey(_, acts, function(_, name, action) {
				...
				if (action.$isRequested && (name !== "$save")) {
					if (name === "$create") {
						...
						if(action.$properties && action.$properties.$select) {
							action.$properties.$select.forEach_(_, function(_, item) {
...
//								config.tracer && config.tracer("factory._executing "+name+".$create; setting "+rel.$select.$target+" on "+rel.targetEntity.name+"."+rel.targetEntity.$relations[rel.$select.$target].name);
								if(rel.targetEntity.$relations[rel.$select.$target]) {
									var inst = instance._db.fetchInstance(_, rel.targetEntity.$relations[rel.$select.$target].targetEntity, item.$uuid);
									inst && child[rel.$select.$target](_, inst);
								} else
									if(rel.targetEntity.$properties[rel.$select.$target]) {
										rel.$select.$selectField && item[rel.$select.$selectField] && child[rel.$select.$target](_, item[rel.$select.$selectField]);
									}
							});
						} else {
							var child = instance.createChild(_, key);
...
//				instance.$addError(property.name+":"+error, property.name);
...
//		op.$title = "Save";
	...
	_proto.getAllDiagnoses = function(_, diag) {
		var self = this;
		if(self.$diagnoses) 
			...
			if(rel.isChild) {
				...
				if(rel.isPlural)
					relData.toArray(_).forEach_(_, function(_, item) {
						item.getAllDiagnoses(_, diag);
					});
				else
					relData && relData.getAllDiagnoses(_, diag);
			}
		});
		//
...
exports.fetchInstances = function(_, context, entity, parameters) {
	...
	var instances;
	//console.log("ent: "+sys.inspect(ent));
