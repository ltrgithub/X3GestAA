...
//		op.$title = "Save";
	...
	_proto.deleteSelf = function(_){
		...
		try {
			...
			config.tracer && configtracer("factory.deleteSelf error, $uuid: "+this.$uuid+"; message: "+e.message);
			return false;
		}
		return true;
	}
...
function _addRights(_, ent, params) {
	...
	if(sp) {
		...
		if(typeof r === "boolean") {
			...
			r = r.replace("{$profile.roles}", "(" + globals.context.session.getUserProfile(_).roles(_).toUuidArray(_).map(function(rId) {
				return "\"" + rId + "\"";
			}).join(",") + ")");
			// 
