	...
	if (instance._meta.$links) {
		if (typeof instance._meta.$links === "function") {
			...
			Object.keys(lks).forEach(function(lk) {
				...
				resource.$links[lk].$description = _getTranslatedString(stringRes, [entity.name, "$links", lk, "$description"], true) || resource.$links[lk].$description || lk;
			});
		} else {
			flows.eachKey(_, instance._meta.$links, function(_, key, link) {
				if (typeof link === "function") {
					...
					if (l) {
						...
						resource.$links[key].$description = _getTranslatedString(stringRes, [entity.name, "$links", key, "$description"], true) || resource.$links[key].$description || key;
					}
				}
			});
		}
