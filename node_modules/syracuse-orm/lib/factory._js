...
function canUpdateRelation(_, _inst, _rel, _key) {
	if (_rel && _inst && _inst._meta._$relations && _inst._meta._$relations[_key]) {
		var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
		// protect links between SAGE factory elements
		if (_inst.$factoryOwner === "SAGE" && _inst.$factoryOwner === _rel.$factoryOwner && sp.factoryOwner(_) !== "SAGE") {
			// Put diagnose
			_inst.$addError(locale.format(module, "protectedFactoryRef", _key, _inst.getEntity(_).name, _inst.$factoryOwner));
			return false;
		}
		return true;
	}
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (newElt.$isDeleted) {
						...
						if (!actualItem) {
							...
							if (!canUpdateRelation(_, instance, collection.get(_, k), key)) {
								collection.deleteInstance(_, k);
								allowedDelta = false;
								return;
							}
