...
function hasFactoryRights(_, _inst, key) {
	...
	if (key && (_inst._parent || _inst._meta.$properties.hasOwnProperty(key))) {
		var excludesLocation = _inst._parent || _inst;
		var factoryExcludes = excludesLocation.getEntity(_).$factoryExcludes || [];
		if (factoryExcludes && factoryExcludes.indexOf(key) === -1) {
			if (_inst.$factory && !_canChangeFactoryInstance(_, _inst)) {
				_inst.$addError(locale.format(module, "updateFactoryPropertyForbidden", key, _inst.getEntity(_).name, _inst.$factoryOwner));
				return false;
			}
		}
	} else if (_inst.$factory && !_canChangeFactoryInstance(_, _inst)) {
...
var _canChangeFactoryInstance = exports._canChangeFactoryInstance = function(_, instance) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	return sp && sp.canChangeFactoryInstance(_, instance);
};
		...
		if (r && r.isPlural) {
			if (Array.isArray(value)) {
				...
				value.forEach_(_, function(_, newElt, idx) {
					...
					if (newElt.$isDeleted) {
						...
						// #4928: Factory security
						if (!hasFactoryRights(_, collection.get(_, k), key)) return;
						...
						if (isDelta || isCreated) {
							// #4928: Factory security
							if (!hasFactoryRights(_, actualItem, key)) return;
						}
					}
				});
				// rebase indexes
				...
				if (receivedUuids) {
					...
					collUuids.forEach_(_, function(_, collEltUuid) {
						if (receivedUuids.indexOf(collEltUuid) < 0) {
							...
							// #4928: Factory security
							if (!hasFactoryRights(_, collection.get(_, collEltUuid), key)) return;
			...
			if (key[0] === '$') {
				...
				if (key === "$uuid" && value && instance.$uuid && instance.$uuid !== value) {
					...
					if (key === "$factory" && !hasFactoryRights(_, instance)) return;
					instance[key] = value;
				}
				...
				if (rel) {
					if (value) {
						...
						if (rel.getIsChild(varType)) {
							...
							if (!instance[key](_)) {
								...
								if (!hasFactoryRights(_, child, key)) return;
					...
					if (p.$isArray && Array.isArray(value)) instance[key](_, value.map(function(v) {
						...
						// #4928: Factory security
						if (instance[key](_) !== resourceHelpers.parseValue(p, value) && !hasFactoryRights(_, instance, key)) return;
...
function Factory(entityMeta) {
	...
	_proto.deleteSelf = function(_) {
		...
		var self = this;
		if (self.$factory && !_canChangeFactoryInstance(_, self)) {
...
function _createWorkingCopy(_, context, factory, wcId, initialInstanceId) {
	...
	if (context.instanceId) {
		...
		//		// rights management
		//		if (instance.$factory && !_canChangeFactoryInstance(_, instance)) {
