...
/*
	...
	_initDbHandle: function(_, dbHandle, resource, execContext, delta) {
		...
		var meta = resource ? (resource.$properties || {}) : null;
	...
	_extractDbHandleErrors: function(path, entity, dbHandle, errors) {
		...
		if (d) {
			forEachKey(entity.allProperties, function(name) {
				var d = dbHandle.$properties[name];
	...
	_setMetaProperty: function(dbHandle, propertyName, preKey, key, value, defValue) {
		dbHandle.$properties = dbHandle.$properties || {};
		var meta = dbHandle.$properties;
		if (propertyName) meta = dbHandle.$properties[propertyName] = dbHandle.$properties[propertyName] || {};
	...
	addError: function(dbHandle, propertyName, message) {
		...
		if (propertyName) {
			dbHandle.$properties = dbHandle.$properties || {};
			meta = dbHandle.$properties[propertyName] = dbHandle.$properties[propertyName] || {};
	...
	_clearDbHandleErrors: function(dbHandle, propertyName) {
		...
		if (propertyName) {
			meta = null;
			if (dbHandle && dbHandle.$properties) meta = dbHandle.$properties[propertyName];
		}
	...
	_validateProperty: function(dbHandle, property, name) {
		...
		if (type) {
			var errors = [];
			type.validate(value, property, errors);
			if (errors.length) {
