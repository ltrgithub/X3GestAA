...
function _fetchPrototype(_, handle, reprName, facetName) {
	if(!facetName)
	...
	var pr = client.jsonSend(_, {
		head: {
			...
			method: "GET",
			url: handle._baseUrl + "/$prototypes('" + reprName + "." + facetName + "')"
		},
		body: {}
	});
	//
...
helpers.defineClass(X3Entity, null, {
	...
	getPrototype: function(_, reprName, facetName) {
...
//	console.log("item proto: " + sys.inspect(itProto));
	if(itProto.$url) {
		...
		if(prop.$type) {
			...
			switch(prop.$type) {
				case "application/x-reference":
					...
					var className = (prop.$url && prop.$url.match(/.*representation=(.*).*/)[1].split("&")[0].split(".")[0]) || entityName+"."+pName;
					p = {
							name: pName,
							$type: className,
				...
				case "application/x-array":
					...
					if(prop.$item.$type === "application/x-json")
						prop.$item.$type = "application/json";
			...
			//
			if(prop.$searchFacets)
				ent.$searchFacets[pName] = prop.$searchFacets.split(",");
		}
	});
...
helpers.defineClass(X3Model, null, {
	...
	getIndexedEntities: function(_) {
		...
		var clsIndexEnt = this.getEntity(_, "ACLAIDX", "$query");
		var cursor = this.getHandle(_).createCursor(_, clsIndexEnt, {
			jsonWhere: {
				FLGSEARCH: true
			}
		}, "$bulk");
	...
	registerEvent: function(eventName, handler, entityName) {
		var self = this;
...
//		p.push("count=100");
	...
	saveInstance: function(_, instance) {
		...
		var data = instance.serializeInstance(_);
		var client = this.getClient(_);
		console.log("saveInstance (405): "+sys.inspect(data, null, 4));
		return;
		var r = client.jsonSend(_, {
			head: {
				"accept":"application/json;vnd.sage=syracuse",
				...
				url: this._baseUrl + "/" + entity.name + (instance.$created ? "" : "('" + data.$key + "')") + "?representation=" + entity.name + ".$edit"
			},
			body: data
		});
		...
		(Object.keys(r.body.$properties) || []).forEach(function(pName) {
			...
			(p.$diagnoses || []).forEach(function(d) {
				instance.$addDiagnose(d.severity, d.message, pName);
			});
		});
		//
		return 1;
