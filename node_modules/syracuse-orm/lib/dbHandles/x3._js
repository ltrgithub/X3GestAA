...
helpers.defineClass(X3DbHandle, null, {
	...
	fetchInstances: function(_, entity, params) {
		...
		tracer && tracer("X3DbHandle.fetchInstances: " + JSON.stringify(r, null, 2));
	...
	getUserRights: function(_, actualRights) {
		var client = this.getClient(_);
		var handle = this;
		if (client && client.$links && client.$links.$rights) {
			var url = client.$links.$rights.$url;
			tracer && tracer("_fetchPrototype: url=" + url + ", locale=" + locale.current);
			var head = {
				"accept": "application/json;vnd.sage=syracuse",
				"accept-language": locale.current || "en-US",
			...
			if (actualRights && actualRights.$etag) head["if-none-match"] = actualRights.$etag;
	...
	batch: function(_, entity, resources, options) {
		...
		tracer && tracer("X3DbHandle.batch: properties=" + JSON.stringify(entity.$properties, null, 2) + ", data=" + JSON.stringify(data, null, 2));
		...
		tracer && r.body.monitoring && tracer("X3DbHandle.batch: monitoring=" + JSON.stringify(r.body.monitoring, null, 2));
		// diags ?
