...
helpers.defineClass(X3DbHandle, null, {
	...
	postAction: function(_, operation, entity, facet, params, representation) { //TODO test
		...
		var url = this._baseUrl + "/" + entity.name + "/$template/$workingCopies?representation=" + (representation ? representation : entity.name) + "." + facet + "&trackingId=" + trackingId;
		console.log("URL" + url);
		...
		if (create.head.status >= 400) {
			if (create.body && create.body.$diagnoses) {
				var diagnosis;
				if (create.body.$diagnoses.some(function(diag) {
					if (diag.$severity === "error") {
						diagnosis = diag;
						return true;
					}
				})) {
					throw new Error("Error creating instance: " + diagnosis.$message);
				}
			} else
				throw new Error("Error creating instance " + body);

		}
		console.error("XXXXX " + require('util').format(create) + " " + require('util').format(create.body.$diagnoses));



		x3Tracer.debug && x3Tracer.debug("X3DbHandle.sendOperation: " + entity.name + "; ");
		if (operation) { // execute operation if it's defined
			url = create.$url.replace('{baseUrl}', this._baseUrl) + "&trackingId=" + trackingId;
			var body = {
				$actions: {},
				$etag: 1,
				$url: url,
		...
		if (del.head.status >= 400) {
			if (del.body && del.body.$diagnoses) {
				var diagnosis;
				if (del.body.$diagnoses.some(function(diag) {
					if (diag.$severity === "error") {
						diagnosis = diag;
						return true;
					}
		...
		// TODO Check if deletion of working work fine
	...
	_service: function(_, verb, service, parameters) {
	...
	postService: function(_, service, parameters) {
		return this._service(_, "post", service, parameters);
	},
