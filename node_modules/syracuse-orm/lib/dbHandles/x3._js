...
var tracer = ((config.orm || {}).x3 || {}).trace;
...
function _fetchQueryResources(_, handle, entity, parameters) {
	...
	var r = client.jsonSend(_, {
		head: {
			...
			"accept-language": locale.current || "en-US",
			method: "GET",
			url: handle._baseUrl + "/" + entity.$className + "?representation=" + entity.name + ".$query" + (sdataWhere ? "&where=" + encodeURIComponent(sdataWhere) : "") + (params.key ? "&key=" + params.key : "") + (params.count ? "&count=" + params.count : "")
		},
		body: {}
	});
