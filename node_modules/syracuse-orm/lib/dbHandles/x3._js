...
helpers.defineClass(X3DbHandle, null, {
	...
	getUserRights: function(_, actualRights) {
		...
		if (rl) {
			var url = rl.$url;
			x3Tracer.debug() && x3Tracer.debug("getUserRights: url=" + url + ", locale=" + locale.current);
			var head = {
				"accept": "application/json;vnd.sage=syracuse",
				"accept-language": locale.current || "en-US",
				method: "GET",
	...
	batch: function(_, entity, resources, options) {
		...
		x3Tracer.debug() && r.body.monitoring && x3Tracer.debug("X3DbHandle.batch: monitoring=" + JSON.stringify(r.body.monitoring, null, 2));
		// diags ?
