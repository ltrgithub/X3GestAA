...
function _fetchPrototype(_, handle, reprName, facetName) {
	if (!facetName) throw new Error("_fetchPrototype: facetName is required");
	// fetch prototype
		...
		if (prop.$type) {
			...
			switch (prop.$type) {
				case "application/x-reference":
					...
					if (p.targetEntity) ent.$relations[pName] = p;
					break;
...
function _fetchQueryResources(_, handle, entity, parameters) {
	...
	var r = client.jsonSend(_, {
		head: {
			...
			"accept-language": locale.current || "en-US",
			method: "GET",
			url: handle._baseUrl + "/" + entity.name + "?representation=" + entity.name + ".$query" + (sdataWhere ? "&where=(" + encodeURIComponent(sdataWhere) + ")" : "") + (params.count ? "&count=" + params.count : "")
		},
		body: {}
	});
...
helpers.defineClass(X3DbHandle, null, {
	...
	createCursor: function(_, entity, params, bulkFacet) {
		...
		return new X3Cursor(this, entity, this._client.createBulkReader(_, {
			url: [
				[this._endpoint.getBaseUrl(_), entity.name].join("/"), p.join("&")].join("?")
		}));
	},
