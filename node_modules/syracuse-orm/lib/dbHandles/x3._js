...
function _jsonToEntity(_, proto, localization, handle, entityName, facet) {
	...
	// useful functions
	ent.createInstance = (function(_, db, initial) {
		return this.factory.createInstance(_, initial, db);
	}).bind(ent);
	ent.fetchInstance = (function(_, db, options) {
		return db.fetchInstance(_, this, options);
	}).bind(ent);
	ent.fetchInstances = (function(_, db, options) {
		return db.fetchInstances(_, this, options);
	}).bind(ent);
	ent.count = (function(_, db, options) {
		return db.count(_, this, options);
	}).bind(ent);
	//
	// add standard links resources for the entity
	ent.fillLinksResource = (function(reprType, resource) {
...
function _makeEntity(_, handle, entityName, facet) {
	...
	// convert to entity
	var ent = _jsonToEntity(_, pr.body, pr.body.$localization, handle, entityName, facet);
	//
