...
function X3Entity(handle, entityName) {
	this._handle = handle;
...
helpers.defineClass(X3Entity, null, {
	...
	getPrototype: function(_, facetName) {
		return this.$orgPrototypes[facetName] || (this.$orgPrototypes[facetName] = _fetchPrototype(_, this._handle, this.name, facetName));
	}
});

function _jsonToEntity(_, proto, localization, handle, entityName, facet) {
	var loc = localization;
	var ent = handle._entities[entityName] || (new X3Entity(handle, entityName));
	//
	ent.$orgPrototypes[facet || "$details"] = proto;
	var pr = proto.$.$resources ? proto.$.$resources.$item.$ : proto.$;
	//
			...
			switch(prop.$type) {
				case "application/x-reference":
					...
					p = ent.$relations[pName] = {
							...
							targetEntity: _jsonToEntity(_, prop, localization, handle, className, facet),
							isPlural: false,
							$isPlural: false
						}
					break;
				case "application/x-collection":
					...
					p.$type = className;
					p.targetEntity = _jsonToEntity(_, prop.$item, localization, handle, className, facet);
					break;
...
function _makeEntity(_, handle, entityName, facet) {
	var pr = _fetchPrototype(_, handle, entityName, (facet || "$details"));
	//
	if(!pr || !pr.$) return null;
	// convert to entity
	var ent = _jsonToEntity(_, pr, pr.$localization, handle, entityName, facet);
	//
	return handle._entities[entityName] = ent;
}
...
exports.create = function(_, endpoint) {
	var handle = new X3DbHandle(endpoint);
	handle.connect(_);
	return handle;
}
