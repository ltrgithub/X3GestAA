...
function _fetchPrototype(_, handle, reprName, facetName, device, checkDiagnoses) {
	...
	tracer && tracer("_fetchPrototype: url=" + url + ", locale=" + locale.current);
...
function _jsonToEntity(_, proto, localization, model, entityName, facet) {
	...
	tracer && tracer("_jsonToEntity - proto: " + sys.inspect(proto));
	tracer && tracer("_jsonToEntity - item proto: " + sys.inspect(itProto));
	if (itProto.$url) {
...
function _fetchQueryResources(_, handle, entity, parameters) {
	...
	var r = client.jsonSend(_, {
		head: {
			...
			"accept-language": locale.current || "en-US",
			method: "GET",
			url: handle._baseUrl + "/" + entity.name + "?representation=" + entity.name + ".$query" + (sdataWhere ? "&where=" + encodeURIComponent(sdataWhere) : "") + (params.key ? "&key=" + params.key : "") + (params.count ? "&count=" + params.count : "")
		},
		body: {}
	});
