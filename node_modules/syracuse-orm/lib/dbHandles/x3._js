...
var x3Tracer = require('syracuse-core').getTracer("orm.x3");
...
helpers.defineClass(X3DbHandle, null, {
	...
	postAction: function(_, operation, entity, facet, params, representation) {
	...
	getUserRights: function(_, actualRights, loginChanged) {
		if (loginChanged) this.getClient(_).disconnect(_);
		var client = this.getClient(_);
		var handle = this;
		...
		if (rl) {
			...
			if (!loginChanged && !nocache && actualRights && actualRights.$etag) head["if-none-match"] = actualRights.$etag;
