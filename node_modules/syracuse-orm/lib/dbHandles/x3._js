...
helpers.defineClass(X3Entity, null, {
	...
	getPrototype: function(_, facetName) {
		return this.$orgPrototypes[facetName] || (this.$orgPrototypes[facetName] = _fetchPrototype(_, handle, this.name, facetName));
	}
});
...
function _jsonToEntity(_, proto, localization, handle, entityName, facet) {
	...
	var pr = proto.$;
	var ent = _entities[entityName] || (new X3Entity(entityName));
	//
...
helpers.defineClass(X3DbHandle, null, {
	...
	getEntity: function(_, entityName, facet) {
		if(_entities[entityName] && _entities[entityName].$orgPrototypes[facet])
