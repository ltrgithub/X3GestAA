	...
	function _getDirtyPlural(_, data, pull, $r, value, path) {
		...
		value.forEach_(_, function(_, v) {
			flows.eachKey(_, v, function(_, key, val) {
				var propMeta = $r.targetEntity.$properties[key];
				if (propMeta) {
					if (propMeta.$encrypt) {
						base64 = base64 || require('syracuse-license').load('license');
						// console.log("ENCRYPT " + val.toString());
						if (val.toString().length >= 64) throw new Error(locale.format(module, "cannotEncrypt", val.toString().length));
						v[key] = base64.license(0, val.toString(), new Boolean(true));
						if (v[key] === null || v[key] === undefined) throw new Error("Old version of license module");
						// console.log("CRYPTED " + v[key]);
					}
				}
			});
		});
	...
	function _internalSave(_, instance, db) {
		...
		if (instance.$factory) {
			...
			if (!instance.$factoryOwner) {
				data.$set._factoryOwner = instance.$factoryOwner = adminHelper.getFactoryOwner(_);
			}
