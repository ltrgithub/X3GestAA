...
function MongoDbHandle(model, dataset) {
	...
	function _synchronizeDb(_, model, db, actualVersion, updateMeta) {
		...
		console.log("mongodbDbHandle.synchronize; ActualVersion: " + actualVersion + "; ModelVersion: " + updateMeta.version);
		// avoid dead-lock
		if (self._isSynchronizing) return;
		self._isSynchronizing = true;
		try {
	...
	/// withIncrement: if true atomically increments by one, if integer increments by it's value
	/// the new counter value is returned or unchanged counter value if !withIncrement
	/// 
	self.getCounterValue = function(_, counterDomain, counterName, withIncrement) {
		var counter;
		...
		if (withIncrement) {
			counter = cntColl.findAndModify(key, null, {
				$inc: {
					value: typeof withIncrement === "number" ? withIncrement : 1
				}
			}, {
				upsert: true,
				"new": true
