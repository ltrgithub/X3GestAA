...
storeProto.createReadableStream = function(_) {
	...
	return ez.devices.node.reader((new mongodb.GridStore(this.db, this.fileName, "r")).open(_).stream(true));
};
				...
				if (meta.$relations[key].isPlural) {
					...
					if (rel.getIsChild(value && value.$variantType) || rel.$inlineStore) {
						...
						if (value) {
							...
							if (value.$factory) data.$set[locPath + escKey + "." + syracuseEscapeMap["$factory"]] = value.$factory;
							if (value.$factoryOwner) data.$set[locPath + escKey + "." + syracuseEscapeMap["$factoryOwner"]] = value.$factoryOwner;
