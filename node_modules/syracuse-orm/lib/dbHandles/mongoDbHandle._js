...
function MongoDbHandle(model, dataset) {
	...
	self.connect = function(_) {
		...
		if (params && params.length) {
			...
			if (actualVersion !== (model.dbMeta.version || 0)) sync = true;
		} else sync = true;
		if (sync) {
			...
			try {
				...
				if (params && params.length) {
					...
					var actualVersion = ((p[model.name] || {}).dbVersion != null) ? (p[model.name] || {}).dbVersion : -1;
					config.tracer && config.tracer("mongodbDbHandle.connect ActualVersion: " + actualVersion);
					if (actualVersion !== (model.dbMeta.version || 0))
						_synchronizeDb(_, model, db, actualVersion);
					else {
						_setExpireEntities(_, model, db);
					}
				} else {
					_synchronizeDb(_, model, db, -1);
				}
			} finally {
	...
	function _ensureExpireIndex(_, entity, db) {
		if (entity.$expire) {
	...
	function _setExpireEntities(_, model, db) {
		...
		if (entities) {
			for (var name in entities) _ensureExpireIndex(_, entities[name], db);
		}
	}
	...
	function _synchronizeDb(_, model, db, actualVersion) {
		...
		// db init
		if ((actualVersion === -1) && (model.dbMeta && model.dbMeta.initScript)) {
				...
				if (meta.$relations[key].isPlural) {
					...
					if (rel.isChild || rel.$inlineStore) {
						...
						if (value) {
							...
							else data.$set[locPath + escKey + "." + syracuseEscapeMap["$uuid"]] = value.$uuid;
