...
var mongoTracer = require("syracuse-trace/lib/helper").getTracer("orm.mongodb");
...
function MongoDbHandle(model, dataset) {
	...
	self.checkExpire = function(_, model, db) {
		/*/TODO purge non purge data that have decribe a ttl (entity with $expire like sessionInfo, CvgSession)
        ...
        if (entities) {
            for (var name in entities) {
                ...
                var list =entities[name].fetchInstances(_);
                ...
                for(var i=0; i< list.length;i++){
                    ...
                    if( list[i].$ttl ){
                        try {
                            if (currentTime > (new Date(data.$ttl).getTime())) {
                                data.deleteSelf();
                            }
                        } catch (e) {
	...
	self.connect = function(_) {
		...
		if (connectionPool && connectionPool[key]) {
			...
			mongoTracer.debug && mongoTracer.debug("Mongodb connect - pool connection for : " + host + ":" + port + "/" + dataset.database + " in state: " + self.db.state);
			if (self.db.state != "connected") self.db.open(_);
			return;
		}
		// create a connection
		//		mongoTracer.debug && mongoTracer.debug("Mongodb connect : "+dataset.hostname+":"+dataset.port);
		...
		if (params && params.length) {
			var p = params[0];
			mongoTracer.debug && mongoTracer.debug("mongodbDbHandle.connect ModelName: " + model.name + "; found dbParam: " + sys.inspect(p));
		...
		self.checkExpire(_, self.model, self.db);
	...
	self.updateEntityIndex = function(_, entity) {
		...
		if (entity.$indexes)
			for (var idxName in entity.$indexes) {
				...
				db.collection(entity.className, _).ensureIndex(fields, {
	...
	function _initializeDb(_, model, db) {
		...
		try {
			...
			if (!(config.system && config.system.protectSettings) && model.dbMeta && model.dbMeta.initScript) {
				//mongoTracer.debug && mongoTracer.debug("mongodbDbHandle.apply initialize script: " + model.dbMeta.initScript);
				var diag = [];
		...
		while (data = cursor.next(_)) {
			...
			if (entity.$expire) {
				try {
					...
					mongoTracer.debug && mongoTracer.debug("$ttl doesn't contain a date for entity " + entity.name);
				}
			}
			if (!isDelete)
				instances.push(data);
	...
	function _internalSave(_, instance, db) {
		...
		mongoTracer.debug && mongoTracer.debug("mongodb.save data result: opRef=" + _opRef + "; " + result);
		return result;
	}
	...
	self.lockDatabase = function(_) {
		...
		try {
			...
			if (res != 0) {
				mongoTracer.debug && mongoTracer.debug("Mongodb lock database: success");
				return {
					status: "success",
					id: lockId
				};
