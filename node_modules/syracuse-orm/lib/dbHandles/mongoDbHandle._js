...
function MongoDbHandle(model, dataset) {
	...
	self.dropAndensureExpireIndex = function(_, entity) {
		var db = self.db;
		if (entity.$expire) {
	...
	function _dropIndexAndExpireEntities(_, model, db) {
		var entities = model.getEntities();

		if (entities) {
			for (var name in entities) self.dropAndensureExpireIndex(_, entities[name]);
		}
	}
	...
	self.connect = function(_) {
		...
		if (sync) {
			...
			try {
				...
				_dropIndexAndExpireEntities(_, model, db);
