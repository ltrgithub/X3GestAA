...
function getSelectFields(entity, shallow) {
	...
	fields.splice(0, 0, "_creUser", "_creDate", "_updUser", "_updDate", "_signature", "_syncUuid", "_tick", "_endpoint", "_stamp");
	//
	return fields;
}
...
function MongoDbHandle(model, dataset) {
	...
	self.connect = function(_) {
		...
		var self = this;
		var automaticUpdIndex = model.dbMeta.automaticImport ? model.dbMeta.automaticImport.length : -1;
		if (automaticUpdIndex > 0) {
			importHandler = require("syracuse-import/lib/jsonImport");
		}
		...
		if (params && params.length) {
			...
			if (!sync && automaticUpdIndex > 0) {
				if (!pm.automatic) {
					sync = true;
				} else {
					// make object out of array
					var automaticData = {};
					pm.automatic.forEach(function(entry) {
						automaticData[entry[0] || ""] = entry;
					});
					...
					while (--automaticUpdIndex >= 0) {
						var fname = model.dbMeta.automaticImport[automaticUpdIndex];
						var data;
						if (!(fname in automaticData) || importHandler.importTest(_, fname, automaticData[fname])) { // some change
							automaticUpdIndex++;
							sync = true;
							break;
						}
		...
		if (sync) {
			...
			try {
				...
				if (pm && pm.dbVersions) {
					...
					var diag = [];
					var automaticData = {};
					if (automaticUpdIndex > 0 && pm.automatic) {
						pm.automatic.forEach(function(entry) {
							automaticData[entry[0] || ""] = entry;
						});
					}
					...
					while (--automaticUpdIndex >= 0) {
						var fname = model.dbMeta.automaticImport[automaticUpdIndex];
						if (!(fname in automaticData)) {
							automaticData[fname] = [fname, ""];
						}
						if (!importHandler.jsonImport(_, self, fname, {
							tracer: null,
							importMode: "update",
							$diagnoses: diag,
							data: automaticData[fname]
						})) {
							...
							Object.keys(automaticData).forEach(function(key) {
								if (model.dbMeta.automaticImport.indexOf(key) >= 0) {
									tracer && tracer("New data for file " + automaticData[key]);
									automaticArray.push(automaticData[key]);
								}
							});
							upd.$set[model.name + ".automatic"] = automaticArray;
