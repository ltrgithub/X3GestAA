...
var mongoTracer = require('syracuse-core').getTracer("orm.mongodb");
...
var lockWaitTimeout = (config.collaboration && config.collaboration.lockWaitTimeout) || 600000;
...
function MongoDbHandle(model, dataset) {
	...
	function _ensureCollectionExpire(_, collection, action) {
		...
		if (!found && (action === "create")) {
			syncTracer && syncTracer("Creating expire index on " + collection.collectionName);
		...
		if (found && (action === "drop")) {
			syncTracer && syncTracer("Dropping expire index on " + collection.collectionName);
	...
	self.connect = function(_) {
		...
		*/
		...
		try {
			...
			if (sync) {
				mongoTracer.debug && mongoTracer.debug("Lock the database");
				...
				// pm = _getModelParams(_); // don't read again, lock is before initial read
