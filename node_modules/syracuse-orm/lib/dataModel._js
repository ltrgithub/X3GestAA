...
/*	var result = {
	...
	// if precedence set to 0, omit payload
	if ((!selected && !needsThumb) || parameters.precedence == 0) return result;

	//	result.$facets = entity.$facets || {};
	// facets : copy entity facets, generate standard missing facets
	//	_createMissingFacets(entity, result.$facets);
	// 
	forEachKey(entity.$properties, function(name, prop) {
		if (parameters.precedence > 0 && (prop.precedence == null || prop.precedence > parameters.precedence)) return;
		if (parameters.select && !parameters.select[name]) return;
		result.$properties[name] = prop;
		// facets
		//		_pushPropertyToFacets(result.$facets, name);
	});

	function _down(parameters, name) {
		return {
			precedence: parameters.precedence,
			include: parameters.include && parameters.include[name],
			select: parameters.select && parameters.select[name]
		}
	}

	var includeChildren = forceChildren || (parameters.include && parameters.include.$children);
	forEachKey(entity.$relations, function(name, rel) {
		var relSelected = ((includeChildren && (rel.$isChild || rel.isChild)) || (parameters.include && parameters.include[name]) || (parameters.select && parameters.select[name]));
		var needsThumb = !relSelected && rel.relType == "reference";
		if ((entity.name != rel.targetEntity.name) || (name != metaName)) {
			var relMeta = (result.$relations[name] = _getMeta(rel.targetEntity, name, _down(parameters, name), relSelected, needsThumb, forceChildren, depth++));
			relMeta.isPlural = rel.isPlural;
			relMeta.isChild = rel.isChild || rel.$isChild;
			relMeta.relType = rel.relType;
			relMeta.targetEntity = rel.targetEntity;
			relMeta.title = rel.title;
		} else result.$relations[name] = rel;
		result.$relations[name].isComputed = rel.isComputed || rel.$isComputed;
		// facets
		//		_pushRelationToFacets(result.$facets, name, rel.isPlural);
	});
	//
	return result;
	*/
