...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix, visited) {
	...
	function _addExcelLinks(_, resource, entityName, facetName) {
		...
		resource.$links.$excelreport = {
			...
			"$officeAddinSetup": "/msoffice/lib/general/addIn/SyracuseOfficeAddinsSetup.EXE",
			"$parameters": {
				"$url": adminBaseUrl + "/msoReportModes/$template/$workingCopies?representation=msoExcelReportMode.$edit&role={$role}" + urlParams,
				"$method": "POST",
				"$properties": {
					"dummy": {
						"$title": "dummy",
	...
	if ((facetName !== "$child") && !isChild) {
		...
		if (queryRepr.indexOf(facetName) >= 0) {
			...
			if (_isEntityCapableOf(entity, "excel")) {
...
exports.getOrm = function(_, model, dataset) {
	...
	if (model.contract && model.contract.entities) {
		...
		try {
			Object.keys(ents).forEach_(_, function(_, name) {
				...
				if (!conflictPriority) {
					...
					if (settingsEnt) {
						...
						// for test cases
						conflictPriority = 5;
						endpoint = require('os').hostname();
					}
				}
				var entInstance = metaEnt.createInstance(_, orm, {
					name: name,
					tick: 2,
			...
			if (locked) orm.unlockDatabase(_);
		}
	}
