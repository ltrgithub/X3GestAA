...
exports.make = function(contract, dataset) {
	...
	// UPDATE: TCA: TODO is partially completed... To be reviewed !!!
	...
	var _model = contract.models[dataset];
	if (_model) {
		forEachKey(contract.entities, function(name, ent) {
			if (!_model._entities[name]) {
				_model._entities[name] = ent;

				if (_model.pluralMap[ent.plural]) throw new Error(ent.name + ": duplicate plural: " + ent.plural);
				_model.pluralMap[ent.plural] = name;
			}
		});
	}
