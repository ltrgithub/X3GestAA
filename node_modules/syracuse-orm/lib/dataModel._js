						...
						if (facetName === "$edit") {
							...
							if (rel.$select) {
								...
								a.$parameters.$actions.$select = {
									...
									$url: "{$baseUrl}/" + selectEnt.plural + "?representation=" + selectEnt.name + ".$select" + (rel.$select.$parameters ? "&" + rel.$select.$parameters : "")
								}
	...
	function _initProperty(entity, prop) {
		...
		prop.getPropertyPrototype = (function(_, stringRes, facetName, isChild) {
			...
			if (prop["$enum"] && Array.isArray(prop["$enum"])) {
				...
				if (prop.isExternalStorage()) if (facetName === "$edit") $p.$url = "{$baseUrl}/$workingCopies('{$trackingId}')/" + name;
		...
		prop.getDefaultValue = (function(_, instance) {
			...
			if (property.hasOwnProperty("defaultValue")) {
				if (typeof property.defaultValue === "function") return property.defaultValue(_);
...
function DataModel(contract) {
	...
	self.getRepresentation = function(callback, name, facet) {
		...
		if ("function" === typeof callback) return callback(null, contract.entities[name]);
		else
	...
	self.registerEvent = (function(eventName, handler, entityName) {
		...
		if (target && target.$events && target.$events[eventName]) target.$events[eventName].push(handler);
	}).bind(self);
}
