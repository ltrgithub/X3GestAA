...
function DataModel(contract) {
	...
	var entities = contract.entities = sanitize(contract.entities);
	var repr = contract.represenatations = sanitize(contract.representations);
	var rules = contract.$rules = sanitize(contract.$rules);
	//
	...
	self.initEntities = function(_, db) {
		Object.keys(_entInstances).forEach_(_, function(_, name) {
			var entity = _entInstances[name];
