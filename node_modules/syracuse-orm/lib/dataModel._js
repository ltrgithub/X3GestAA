...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix) {
	...
	function _updateChildRef(_, resource, rel, variant, facetName, isChild, childPrefix, isCollection) {
		resource.$type = "application/x-object";
			...
			if(isCollection) {
				...
				if(variant.$lookup) {
					...
					$item.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "selectItemActionTitle"), 
							"trackingId={$trackingId}" + ((rel && rel.name) ? "&binding=" + thumbRelName : "") + 
		...
		if (rel.isPlural) {
			...
			case "children":
				if(rel.$variants) {
					...
					_updateChildRef(_, $item, rel, rel, facetName, true, thumbRelName, true);
...
function DataModel(contract) {
	...
	forEachKey(entities, function(name, entity) {
		...
		forEachKey(entity.$relations, function(relName, rel) {
			...
			rel.$canReorder = rel.$capabilities && (rel.$capabilities.indexOf("reorder") >= 0);
