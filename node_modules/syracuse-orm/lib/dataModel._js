...
function _copyConstraints(src, dst) {
	...
	'$isReadOnly', '$isDisabled', '$scale', '$precision'].forEach(function(key) {
...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix) {
	...
	function _fillThumb($thumb, e, rel, prefix) {
		...
		$thumb.$title = (e.$valueTitleTemplate && (_getTranslatedString(stringRes, [e.name, "$valueTitleTemplate"]) 
				|| e.$valueTitleTemplate.expression));
		...
		if((facetName === "$edit") || (queryRepr.indexOf(facetName) >= 0)) {
			...
			var relLookupParams = (rel.$lookup && (rel.$lookup.parameters || rel.$lookup.$parameters)) || 
					(rel.$select && (rel.$select.parameters || rel.$select.$parameters));
			$thumb.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "createTableActionTitle"), 
					"trackingId={$trackingId}" + ((rel && rel.name) ? "&binding=" + prefix : "") + 
					(relLookupParams ? "&" + relLookupParams : ""), e.name);
		}
	}
	...
	function _addWordLinks(_, resource, entityName, facetName)
	{
		...
		var templateParams = {
			...
			"$msoLocale": "",   //require("streamline/lib/globals").context.locale,
			"$msoCpy": "",
			"$msoLeg": "",
			"$msoActiv": "",
		...
		if (facetName === "$edit") {
			if(isCollection) {
				...
				if(variant.$select) {
					...
					var selectEnt = model.getEntity(variant.$select.$type);
					...
					if(!rel.$capabilities || rel.$capabilities.indexOf("append") >= 0)
						resource.$actions.$create = resource.$actions.$create || {
							$title: locale.format(module, "createTableActionTitle"),
							$method: "PUT"
						}
				}
				...
				if(variant.$lookup) {
					...
					$item.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "selectItemActionTitle"), 
							"trackingId={$trackingId}" + ((rel && rel.name) ? "&binding=" + childPrefix : "") + 
							(lp ? "&" + lp : ""), variant.targetEntity.name);
				}
				if($item.$actions && $item.$actions.$delete)
...
//		}
	...
	flows.eachKey(_, entity.$relations, function(_, name, rel) {
		...
		$r.$description = _getTranslatedString(stringRes, [entity.name, name, "$description"], true) || rel.$description;
		...
		if (rel.isPlural) {
				...
				if(rel.$variants) {
					...
					Object.keys(rel.$variants).forEach(function(vName) {
						...
						if(facetName === "$edit") {
							...
							($v.$links = $v.$links || {}).$select = _getLink("$select", locale.format(module, "createTableActionTitle"), 
									"trackingId={$trackingId}" + (rel.name ? "&binding=" + thumbRelName : "") +
									(lp ? "&" + lp : ""), variant.targetEntity.name);
						}
					});
				} else {
			...
			case "children":
				if(rel.$variants) {
					...
					_updateChildRef(_, $r, rel, rel, facetName, true, thumbRelName, true);
			...
			case "child":
				...
				if(rel.$variants) {
					...
					_updateChildRef(_, $r, rel, rel, facetName, true, thumbRelName);
				break;
			case "reference":
				if(rel.$variants) {
					$r.$type = "application/x-variant";
	...
	if ((facetName !== "$child") && !isChild) {
		...
		if(_isEntityCapableOf(entity, "wordReport"))
		...
		if (queryRepr.indexOf(facetName) >= 0) {
			...
			// word links is "opt-in"
			if(_isEntityCapableOf(entity, "mailMerge"))
				_addWordLinks(_, resource, entity.name, facetName);
			// removed ppt since reporting in ppt is not supported yet
			// _addPptLinks(resource, facetName);
...
//		return (!(this.$storage === "inline") && (["image", "binary"].indexOf(this.type) >= 0));
	...
	prop.getPropertyPrototype = (function(_, stringRes, facetName, isChild) {
		...
		res.$description = _getTranslatedString(stringRes, [entity.name, name, "$description"], true) || prop.$description;
	...
	prop.getDefaultValue = (function(_, instance) {
		...
		if(property.hasOwnProperty("defaultValue")) {
			if(typeof property.defaultValue === "function")
				return property.defaultValue(_, instance);
			else
		...
		entity.fillLinksResource = (function(reprType, resource) {
			...
			if(self.$actions) {
				...
				Object.keys(self.$actions).forEach(function(lName) {
					...
					if(!a.$facets || (a.$facets && a.$facets.indexOf(reprType) >= 0))
						var p = resource.$actions[lName] = resource.$actions[lName] || {};
			...
			forEachKey(self.$services, function(key, service) {
				...
				if (service.$method && (service.$method !== "GET") && (reprType === "$edit")) {
					...
					s = resource.$links[key] = {
						...
						$url: "{$baseUrl}/{$pluralType}('{$uuid}')/$service/" + key + "?representation={$representation}." + reprType
					}
				...
				if(service.$parameters)
					s.$parameters = service.$parameters;
...
//			rel.type = rel.type || rel.$type || relName;
			...
			if (rel.isPlural) {
				...
				if(!rel.defaultOrder) {
					if(rel.$capabilities && (rel.$capabilities.split(",").indexOf("reorder") >= 0))
						...
						rel.defaultOrder = rel.targetEntity && rel.targetEntity.defaultOrder;
				}
			} else {
				if(rel.type) {
	...
	forEachKey(entities, function(name, entity) {
		forEachKey(entity.$relations, function(relName, rel) {
			...
			if (inv) {
//				console.log("inv (1193): "+sys.inspect(inv.targetEntity));
...
//		entity.deepMeta = entity.getMeta(null, true);
	...
	self.getRepresentation = function(callback, name, facet) {
		// TODO: return the representation as the associated entity with filtered properties
		if("function" === typeof callback) 
			return callback(null, contract.entities[name]);
		else
			// name is the first parameter (so callback is in fact the name)
			return contract.entities[callback];
	}
	self.getEntity = function(callback, name) {
		if("function" === typeof callback)
			return callback(null, contract.entities[name]);
		else
			// name is the first parameter (so callback is in fact the name)
