...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix, visited) {
	...
	flows.eachKey(_, entity.$relations, function(_, name, rel) {
		...
		var $r = $[name] = rel.getPropertyPrototype(_, stringRes, facetName, isChild, childPrefix, visited);
	});
...
function DataModel(contract) {
	...
	forEachKey(entities, function(name, entity) {
		...
		entity.getPrototype = (function(_, reprName, facetName, isChild, childPrefix, visited) {
			return (this.$getPrototype && this.$getPrototype(_, reprName, facetName)) || _getEntityPrototype(_, self, entity, reprName, facetName, isChild, childPrefix, visited);
		}).bind(entity);
		entity.makeDigest = (function(_, db) {
			if (!this._syncInfo) return null;
			// get latest data, therefore reload instance
			var ent = db.getEntity(_, "entity");
			this._syncInfo = db.fetchInstance(_, ent, {
				jsonWhere: {
					name: this.name
				}
			});
			return this._syncInfo.makeDigest(_);
		}).bind(entity);
		entity.endpoint = (function(_) {
			if (!this._syncInfo) return null;
			return this._syncInfo.endpoint(_);
		}).bind(entity);
		entity.tick = (function(_, db) {
			var syncInfo = this._syncInfo;
			if (!syncInfo) throw new Error(locale.format(module, "noSyncInfo", entity.name));
			// TODO: here must be the atomic counter
			var newValue = syncInfo.tick(_) + 1;
			syncInfo.tick(_, newValue);
			var res = db.saveInstance(_, syncInfo);
			if (!res) {
				var ent = db.getEntity(_, "entity");
				syncInfo = db.fetchInstance(_, ent, {
					jsonWhere: {
						name: this.name
					}
				});
				newValue = syncInfo.tick(_) + 1;
				syncInfo.tick(_, newValue);
			}
		...
		entity.saveDigest = (function(_, db, digest) {
			var syncInfo = this._syncInfo;
			if (!syncInfo) throw new Error(locale.format(module, "noSyncInfo", entity.name));
			syncInfo.putDigest(_, digest);
			var res = db.saveInstance(_, syncInfo);
			if (!res) {
				var ent = db.getEntity(_, "entity");
				syncInfo = db.fetchInstance(_, ent, {
		...
		entity.getLink = function(facet, title, params) {
			return _getLink(facet, title, params, entity.name);
		};
	});
	forEachKey(entities, function(name, entity) {
		forEachKey(entity.$relations, function(relName, rel) {
	...
	forEachKey(entities, function(name, entity) {
		...
		var _props = entity._$properties = {};
		Object.keys(entity.$properties).forEach(function(propName) {
			_props[propName] = new entityClasses.Property(entity, entity.$properties[propName]);
			entity.allProperties[propName] = _props[propName];
		});
		...
		//
		var _rels = entity._$relations = {};
		Object.keys(entity.$relations).forEach(function(relName) {
			_rels[relName] = new entityClasses.Relation(entity, entity.$relations[relName]);
			entity.allProperties[relName] = _rels[relName];
		});
		...
		// allProperties
		entity._allProperties = entity.allProperties;
		delete entity.allProperties;
		Object.defineProperty(entity, "allProperties", {
			get: function() {
				var p = {};
				Object.keys(entity._allProperties).forEach(function(pk) {
					p[pk] = entity._allProperties[pk];
				});
				return p;
			},
			set: function(val) {
