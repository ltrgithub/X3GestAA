			...
			if (reprType === "$edit") {
				...
				if (self.$canSave !== false) {
					resource.$actions.$save = resource.$actions.$save || {
						...
						$isDisabled: true,
						$links: {
							$details: _getLink("$details", locale.format(module, "ok"), "", self.plural),
							$query: _getLink("$query", locale.format(module, "backToList"), "", self.plural),
							$create: _getLink("$create", _getTranslatedString(stringRes, [self.name, "$createActionTitle"]) || locale.format(module, "newLinkTitle", self.name), "", self.plural)
						}
					}
