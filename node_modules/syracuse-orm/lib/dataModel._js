...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix) {
	...
	function _addWordLinks(resource)
	{
		var self = this;
		...
		if (rel.isPlural) {
			...
			case "link":
				if (rel.$assoType) {
					...
					($r.$links = $r.$links || {}).$select = _getLink("$select", "$select", locale.format(module, "createTableActionTitle"), 
...
//							$r.$actions.$create.$parameters.$actions.$select.$selectField = rel.$select.$selectField;
	...
	if ((facetName !== "$child") && !isChild) {
		...
		//
		_addWordLinks(resource, facetName);
