...
function _copyConstraints(src, dst) {
	['$pattern', '$minLength', '$maxLength', '$minValue', '$maxValue', //
	...
	function _fillThumb($thumb, e, rel, prefix) {
		...
		if ((facetName === "$edit") || (queryRepr.indexOf(facetName) >= 0)) {
			...
			$thumb.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "createTableActionTitle"),
				"trackingId={$trackingId}" + ((rel && rel.name) ? "&binding=" + prefix : "") + (relLookupParams ? "&" + relLookupParams : ""), e.name);
		}
	}
			...
			if (isCollection) {
				...
				if (variant.$lookup) {
					...
					$item.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "selectItemActionTitle"),
						"trackingId={$trackingId}" + ((rel && rel.name) ? "&binding=" + childPrefix : "") + (lp ? "&" + lp : ""), variant.targetEntity.name);
				}
					...
					if (rel.$variants) {
						...
						Object.keys(rel.$variants).forEach(function(vName) {
							...
							if (facetName === "$edit") {
								...
								($v.$links = $v.$links || {}).$select = _getLink("$select", locale.format(module, "createTableActionTitle"),
									"trackingId={$trackingId}" + (rel.name ? "&binding=" + thumbRelName : "") + (lp ? "&" + lp : ""), variant.targetEntity.name);
							}
						});
					} else {
						...
						if (facetName === "$edit") {
							...
							($r.$links = $r.$links || {}).$select = _getLink("$select", locale.format(module, "createTableActionTitle"),
								"trackingId={$trackingId}" + (rel.name ? "&binding=" + thumbRelName : "") + (lp ? "&" + lp : ""), rel.targetEntity.name);
						}
					}
					break;
				case "children":
