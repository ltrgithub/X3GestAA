...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix) {
	...
	function _fillThumb($thumb, e, rel, prefix) {
		...
		var relLookupParams = (rel.$lookup && rel.$lookup.parameters) || (rel.$select && rel.$select.parameters);
			...
			case "link":
				if (rel.$assoType) {
					...
					($r.$links = $r.$links || {}).$select = _getLink("$select", locale.format(module, "createTableActionTitle"), 
							...
							(rel.$select && rel.$select.parameters ? "&" + rel.$select.parameters : ""), rel.targetEntity.name);
				}
				break;
			case "children":
...
/*					$r.$actions = $r.$actions || {};
					...
					if(rel.$lookup) {
						...
						$item.$links[lookupLinkName] = _getLink(lookupLinkName, locale.format(module, "selectItemActionTitle"), 
								...
								(rel.$lookup && rel.$lookup.parameters ? "&" + rel.$lookup.parameters : ""), rel.targetEntity.name);
					}
	...
	prop.getDefaultValue = (function(_, instance) {
		...
		if(property.hasOwnProperty("defaultValue")) {
			if(typeof property.defaultValue === "function")
				return property.defaultValue(_);
			else
