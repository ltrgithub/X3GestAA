...
function _getEntityPrototype(_, model, entity, reprName, facetName, isChild, childPrefix) {
	...
	flows.eachKey(_, entity.$relations, function(_, name, rel) {
		...
		var $r = $[name] = rel.getPropertyPrototype(_, stringRes, facetName, isChild, childPrefix);
	});
...
function DataModel(contract) {
	...
	forEachKey(entities, function(name, entity) {
		...
		entity.getPrototype = (function(_, reprName, facetName, isChild, childPrefix) {
			return (this.$getPrototype && this.$getPrototype(_, reprName, facetName)) || _getEntityPrototype(_, self, entity, reprName, facetName, isChild, childPrefix);
		}).bind(entity);
...
exports.getOrm = function(_, model, dataset) {
	...
	else orm = require("syracuse-orm/lib/mysqlDbHandle").create(_, model, dataset);
