		...
		if (Array.isArray(value)) {
			...
			if (key === "$uuid") {
				...
				if (crtRel) {
					...
					if ((keysLen > 1) && (sKey[keysLen - 1]) && rels[keysLen - 2] && !rels[keysLen - 2].isChild) {
						...
						// case sensitive
						if (targetEnt.$properties && targetEnt.$properties[lastPart] && targetEnt.$properties[lastPart].$caseInsensitive) if (typeof value === "string") filter[key] = value = {
							$regex: "^" + value + "$",
							$options: "i"
						};
						// localization
...
function getSelectFields(entity, shallow) {
	...
	fields.splice(0, 0, "_creUser", "_creDate", "_updUser", "_updDate", "_signature");
	//
	return fields;
}
...
function MongodbCursor(db, entity, cursor) {
	...
	this._entity = entity;
	this._db = db;
	this._dataFuture = _nextObject(this._cursor, void _);
}
...
cursorProto.next = function(_) {
	...
	if (data = self._dataFuture(_)) {
		self._dataFuture = _nextObject(self._cursor, void _);
		data.$key = data._id;
		data.$uuid = data._id;
		data.$loaded = true;
...
function MongoDbHandle(model, dataset) {
	...
	self.connect = function(_) {
		...
		//		var host = "localhost";
		//		var port = 27017;
		var key = host + '/' + port + '/' + dataset.database;
		...
		var dbname = config.multitenant ? globals.context.tenantId + '-' + dataset.database : dataset.database;
	...
	function _synchronizeDb(_, model, db, actualVersion) {
		...
		if ((actualVersion === -1) && (model.dbMeta.initScript)) {
			...
			var diag = [];
			require("syracuse-import/lib/jsonImport").jsonImport(_, self, model.dbMeta.initScript, {
