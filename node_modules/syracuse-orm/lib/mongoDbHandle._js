...
gdWsProto.end = function(buffer, enc, _) {
	buffer && this.store.write(buffer, _);
	this.store.close(_);
}
...
storeProto.createWritableStream = function(_) {
	return (new GridWritableStream(new mongodb.GridStore(this.db, this.fileName, "w").open(_)));
}
//
...
storeProto.read = function(_, len) {
	if(!this.readStore) 
		throw new Error("Error: File store not opened");
	//
