...
//						newKey = key + "." + syracuseEscapeMap["$keys"];
						newKey = key + "." + syracuseEscapeMap["$uuid"];
					...
					if(targetEnt.$properties && targetEnt.$properties[lastPart] && targetEnt.$properties[lastPart].$isLocalized) {
						//sKey.push(locale.current.toLowerCase());
						//newKey = sKey.join(".");
						if(value != null) {
							newKey = "$or";
							value = [_makeTerm(sKey, locale.current.toLowerCase(), value), _makeTerm(sKey, "default", value)];
						}
					} else {
						// is last part the locale code of a localized prop ?
						if(targetEnt.$properties && targetEnt.$properties[sKey[sKey.length - 2]] && targetEnt.$properties[sKey[sKey.length - 2]].$isLocalized) {
...
/*		parameters.jsonWhere && forEachKey(parameters.jsonWhere, function(key, value) {
		...
		filter = decorateFilter(entity, filter);
	...
	self.getEntity = function(_, entityName) {
		return this.model.getEntity(entityName) || this.model.getEntity(this.model.singularize(entityName));
	}
			...
			if ($p.$isLocalized) {
				...
				if($p.isExternalStorage()) {
					...
					if(value && value._uuid)
						data.$set[path + "._uuid"] = value._uuid; 
					else
						data.$set[path] = {};
				} else
					data.$set[path] = value;
...
/*	function _getDirtyPlural(_, data, pull, $r, value, path) {
		...
		flows.eachKey(_, value, function(_, listKey, listValue) {
			if (listValue.$isDeleted) {
				data.$unset = data.$unset || {};
				data.$unset[path + "." + value[listKey].$uuid] = 1;
				// keys array allow reverse relation loading
				if (!$r.isChild) {
					//pull = pull || {};
					pull.$pullAll = pull.$pullAll || {};
					var pullPath = path + "." + syracuseEscapeMap["$keys"];
					pull.$pullAll[pullPath] = pull.$pullAll[pullPath] || [];
					pull.$pullAll[pullPath].push(value[listKey].$uuid);
				}
			} else {
				if ($r.isChild) 
					_getDirtyProps(_, data, pull, listValue, $r.targetEntity, path + "." + value[listKey].$uuid);
				else {
					var setKey = path + "." + syracuseEscapeMap["$keys"];
					if ($r.targetAssoEntity) 
						_getDirtyProps(_, data, pull, listValue, $r.targetAssoEntity, path + "." + value[listKey].$uuid);
					else 
						data.$set[path + "." + value[listKey].$uuid] = 1; // dummy payload to define property
					data.$addToSet = data.$addToSet || {};
					data.$addToSet[setKey] = data.$addToSet[setKey] || {
						$each: []
					};
					data.$addToSet[setKey].$each.push(value[listKey].$uuid);
				}
			}
		});
	}*/
		...
		flows.eachKey(_, delta, function(_, key, value) {
			...
			if (rel && !rel.$compute && !rel.isComputed) {
				if (meta.$relations[key].isPlural) {
					...
					if (rel.isChild || rel.$inlineStore) {
						if (!value) {
							//										throw new Error("Cannot save null child for relation: "+key);
							data.$set[locPath + escKey] = {};
						} else {
							_getDirtyProps(_, data, pull, value, meta.$relations[key].targetEntity, locPath + escKey);
