	...
	serialize: function(_, instance, options, withMeta) {
		...
		flows.eachKey(_, meta.$relations, function(_, key, relation){
			if (relation.isPlural) {
				...
				serializer.canSerializeRelation(_, instance, relation) && serializer.serializeReference(_, instance, relation, resource);
		});
...
exports.SaveSerializer = helpers.defineClass(SaveSerializer, Serializer, {
	...
	serializePlural: function(_, instance, relation, resource) {
		...
		resource[key] = instance[key](_).toArray(_, true).map_(_, function(_, e) {
			var r;
			if(relation.$isDynamicType && !relation.isChild) {
				r = { $url: e.$url || e.computeUrl() }
			} else {
				r = relation.isChild ? self.serialize(_, e) : { $uuid: e.$uuid };
				if(r && (relation.$isDynamicType || relation.$variants))
					r.$type = e.getEntity().name;
			}
			return r;
		});
	},
	serializeReference: function(_, instance, relation, resource) {
		...
		if(relation.isChild || relation.$inlineStore) {
			var v = instance[key](_);
			resource[key] = self.serialize(_, v);
			if(v && resource[key] && (relation.$isDynamicType || relation.$variants))
				resource[key].$type = v.getEntity().name;
		} else 
			...
			if(instance._data[key] && instance._data[key].$uuid) {
				...
				if(relation.$isDynamicType || relation.$variants)
					resource[key].$url = instance._data[key].$url || instance._data[key].computeUrl();
			}
	}
});
	...
	serializePluralResource: function(instance, relation, resource) {
		...
		resource[key] = instance[key].map(function(e) {
			...
			if(relation.$isDynamicType && !relation.isChild) {
				...
				r = relation.isChild ? self.serializeResource(relation.targetEntity, e) : { $uuid: e.$uuid };
			}
			return r;
		});
	},
	serializeReferenceResource: function(instance, relation, resource) {
		...
		if(relation.isChild || relation.$inlineStore) {
			...
			if(v.$uuid) {
				...
				if(relation.$isDynamicType || relation.$variants)
					resource[key].$url = v.$url;
			}
	},
	serializeResource: function(entity, instance, options) {
		if(!instance) return null;
		...
		Object.keys(meta.$relations).forEach(function(key){
			...
			if (relation.isPlural) {
				...
				instance[key] && serializer.serializeReferenceResource(instance, relation, resource);
		});
		if(Object.keys(resource).length === 0)
			return null;
		else {
		...
		//
		tracer && tracer("serializer.serializeResource exit : " + sys.inspect(resource, null, 4));
		return resource;
	}
});
