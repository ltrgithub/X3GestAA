...
asyncTest("Initialize", 1, function(_) {
	...
	var model = x3Handle.makeModel(_, {
		getBaseUrl: function(_) {
			return "";
		}
	});
	// create with endpoint stub
	handle = x3Handle.create(_, {
		getBaseUrl: function(_) {
			return "";
		},
		getModel: function(_) {
...
asyncTest("Entity creation test", 13, function(_) {
	...
	deepEqual(entity.$properties.MODULE.$enum, [1, 2, 3], "MODULE $enum ok");
	deepEqual(entity.$properties.MODULE.$enumCaptions, ["Tronc commun", "Interne superviseur", "Interface compta"], "MODULE $enumCaptions ok");
	...
	strictEqual(detProto.$.CODFIC.$title, "{@6}", "$details original proto ok");
	var detProto = entity.getPrototype(_, "$query");
	strictEqual(detProto.$.$resources.$item.$.CODFIC.$title, "{@6}", "$query original proto ok");
	//
	start();
});
