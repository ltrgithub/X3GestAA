...
function readAll(_,data){
	try{
		...
		strictEqual(new Buffer(data, 'utf8').toString(),new Buffer(result, 'utf8').toString(),"Buffer Content ok");
...
asyncTest('readDocumentAfterCreate', 1, function(_){
	readAll(_,data1);
});
...
asyncTest('readDocumentAfterUpdate', 1, function(_){
	readAll(_,data2);
});
...
asyncTest('readStreamDocument', 8, function(_){
	try{
		var filter = {jsonWhere:{$uuid: uuidRef}};
		...
		checkDocument(_, document, options1);
		strictEqual(new Buffer(data1, 'utf8').toString(),new Buffer(data3, 'utf8').toString(),"Buffer Content ok");
