...
asyncTest("sdata matches test", 23, function(_) {
	...
	strictEqual(country.match(_, parser.Parser.parse("((code like '%F%') and (description like '%ra%'))")), true, "Complex multiple expression 1 Ok");
	...
	tracer && tracer("expression (99): " + sys.inspect(parser.Parser.parse("address.country.code eq 'FR'"), null, 6));
	...
	t.bool(_, true);
	strictEqual(t.match(_, parser.Parser.parse("bool eq true")), true, "Bool 'eq' true 1 Ok");
	strictEqual(t.match(_, parser.Parser.parse("bool eq false")), false, "Bool 'eq' false 1 Ok");
	t.bool(_, false);
	strictEqual(t.match(_, parser.Parser.parse("bool eq true")), false, "Bool 'eq' true 2 Ok");
	strictEqual(t.match(_, parser.Parser.parse("bool eq false")), true, "Bool 'eq' false 2 Ok");
	//
	start();
});
