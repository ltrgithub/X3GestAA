...
asyncTest("$actions test", 19, function(_) {
	...
	//
	body = put(_, cookie, baseUrl + body.$links.$abort.$url, {}, 200);
	// give him time to exit loop
	setTimeout(_, 500);
	//
	body = get(_, cookie, trkUrl);
	strictEqual(body.phase, "Completed", "Got tracker completed ok");
	//
...
asyncTest("OneToMany references", 17, function(_) {
	...
	// add refC, method 2
	body = put(_, cookie, "refAs('" + refA + "')", { refCList: [{$uuid:refC2, $index:1}, {$uuid:refC3,$index:2}] }, 200);
	...
	// delete refC1
	body = put(_, cookie, "refAs('" + refA + "')", { refCList: [{$uuid:refC1, $isDeleted:true, $index:1}, {$uuid:refC3, $isDeleted:true, $index:2}] }, 200);
