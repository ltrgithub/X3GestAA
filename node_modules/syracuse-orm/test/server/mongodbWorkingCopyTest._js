...
config.session.auth = "streamline-auth/lib/basic";
...
asyncTest("simple object creation", 21, function(_) {
	...
	strictEqual(body.$.code.$diagnoses[0].message, "country code must be unique (FR)", "set code to existing one ok");
	// insist ;-)
	body = put(_, cookie, body.$url, {
		$key: body.$uuid,
		$etag: body.$etag,
	...
	strictEqual(body.$.code.$diagnoses[0].message, "country code must be unique (FR)", "try to save existing code ok");
...
asyncTest("Various controls et diagnoses", 29, function(_) {
	...
	ok(!onlyInfo(body.$actions.$save.$diagnoses), "Save error ok(1)");
