...
function hasErrors(resource, includeWrn) {
	...
	if(!result) 
		result = resource.$ && Object.keys(resource.$).some(function(prop) {
			return hasErrors(resource.$[prop], includeWrn);
		});
	if(!result)
...
asyncTest("simple object creation", 21, function(_) {
	...
	strictEqual(body.$.code.$diagnoses[0].message, "country: the property(ies) value(s) (code) must be unique", "set code to existing one ok");
	// insist ;-)
	body = put(_, cookie, body.$url, {
		$key: body.$uuid,
		$etag: body.$etag,
	...
	strictEqual(body.$.code.$diagnoses[0].message, "country: the property(ies) value(s) (code) must be unique", "try to save existing code ok");
...
asyncTest("Various controls et diagnoses", 29, function(_) {
	...
	strictEqual(body.$.firstName.$diagnoses.length, 1, "Error count should be 1");
	strictEqual(body.$.firstName.$diagnoses[0].severity, "error", "Severity should be error (1)");
	...
	ok(!body.$.firstName || (body.$.firstName.$diagnoses.length==0), "Errors cleared");
	...
	strictEqual(body.$.firstName.$diagnoses.length, 1, "Error count should be 1");
	strictEqual(body.$.firstName.$diagnoses[0].severity, "error", "Severity should be error");
	strictEqual(body.$.firstName.$diagnoses[0].message, "firstName cannot start with $", "Error message check");
	//	console.log("!!!!!!!!!!body:"+sys.inspect(body.$.firstName.$diagnoses));
	// mandatory lastName
	body = put(_, cookie, body.$url, {
		$key: createdUser,
		$etag: body.$etag,
	...
	strictEqual(body.$.lastName.$diagnoses.length, 1, "Error count should be 1");
	strictEqual(body.$.lastName.$diagnoses[0].severity, "error", "Severity should be error (2)");
	// clear error
	body = put(_, cookie, body.$url, {
		$key: createdUser,
		$etag: body.$etag,
	...
	ok(!body.$.lastName || (body.$.lastName.$diagnoses.length==0), "Errors cleared");
...
asyncTest("Plural references", 11, function(_) {
	var cookie = getCookie(_);
	var body;
...
asyncTest("OneToMany references", 12, function(_) {
	var cookie = getCookie(_);
	var body;
	...
	// add refC, method 2
	body = put(_, cookie, "refAs('" + refA + "')", { refCList: [{$uuid:refC2}] }, 200);
	...
	strictEqual(body.refCList.length, 2, "refC2 added Ok");
	// delete refC1
	body = put(_, cookie, "refAs('" + refA + "')", { refCList: [{$uuid:refC1, $isDeleted:true}] }, 200);
