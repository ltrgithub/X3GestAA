...
var adminEndpoint = testAdmin.modifyCollaborationEndpoint("mongodb_example");
//add model
var testEp = {
...
config.sdata.endpoints.push(testEp);
...
asyncTest("init database", 1, function(_) {
	var server = new mongodb.Server(adminEndpoint.datasets["mongodb_example"].hostname, adminEndpoint.datasets["mongodb_example"].port, {});
	var _db = testAdmin.newMongoDb(adminEndpoint.datasets["mongodb_example"].database, server, {});
	_db = _db.open(_);
	_db.dropDatabase(_);
	var server = new mongodb.Server(testEp.datasets["mongodb_example"].hostname, testEp.datasets["mongodb_example"].port, {});
	var _db = testAdmin.newMongoDb(testEp.datasets["mongodb_example"].database, server, {});
	_db = _db.open(_);
	_db.dropDatabase(_);
...
asyncTest("start server", 1, function(_) {
...
asyncTest("simple object creation", 21, function(_) {
	...
	tracer && tracer("Create count end :" + sys.inspect(body));
	...
	tracer && tracer("body(217): " + sys.inspect(body, null, 4));
	...
	tracer && tracer("body(226): " + sys.inspect(body, null, 4));
	strictEqual(body.$properties.code.$diagnoses[0].$message, "This value has already been used for this field (code) of entity 'country'");
...
var createdUser = "";
asyncTest("create user test", 9, function(_) {
	cookie = getCookie(_);
	// Create user
	var body = post(_, cookie, "userTests/$template/$workingCopies?trackingId=" + uuid.generate(), {});
	...
	tracer && tracer("body(323): " + sys.inspect(body, null, 4));
	...
	tracer && tracer("body(340): " + sys.inspect(body, null, 4));
...
asyncTest("add/modify/delete childrens - user test", 18, function(_) {
	...
	body = post(_, cookie, "userTests('" + createdUser + "')/$workingCopies?trackingId=" + uuid.generate() + "&representation=user.$edit", {});
	//	tracer && tracer("add/modify/delete childrens=================================");
	...
	ok(onlyInfo(body.$actions.$save.$diagnoses), "save diags ok");
	// Fetch user WC
	body = post(_, cookie, "userTests('" + createdUser + "')/$workingCopies?trackingId=" + uuid.generate(), {});
...
asyncTest("child adress test", 18, function(_) {
	...
	tracer && tracer("address test (446) body: " + sys.inspect(body, null, 4));
	ok(onlyInfo(body.$actions.$save.$diagnoses), "save diags ok");
	body = post(_, cookie, "userTests('" + createdUser + "')/$workingCopies?trackingId=" + uuid.generate(), {});
	...
	tracer && tracer("address mod(450): " + sys.inspect(body, null, 4));
...
asyncTest("Various controls et diagnoses", 29, function(_) {
	...
	tracer && tracer("body(mod firstname):" + sys.inspect(body, null, 4));
	...
	//	tracer && tracer("!!!!!!!!!!body:"+sys.inspect(body.$properties.firstName.$diagnoses));
	// mandatory lastName
	body = put(_, cookie, body.$url, {
		$key: createdUser,
		$etag: body.$etag,
	...
	//	tracer && tracer("body(392): "+sys.inspect(body,null,4));
...
asyncTest("$actions test", 19, function(_) {
	cookie = getCookie(_);
	var body;
	// Fetch user WC
	body = post(_, cookie, "userTests('" + createdUser + "')/$workingCopies?trackingId=" + uuid.generate(), {});
	...
	tracer && tracer("body actions: " + sys.inspect(body, null, 4));
	// async actions
	var wcUrl = body.$url;
...
asyncTest("Propagate and error management", 11, function(_) {
	var body;
	...
	tracer && tracer("body (788):" + sys.inspect(body, null, 4));
...
asyncTest("Batch test", 2, function(_) {
	// Create BB
	...
	tracer && tracer("(911) body: " + sys.inspect(body));
