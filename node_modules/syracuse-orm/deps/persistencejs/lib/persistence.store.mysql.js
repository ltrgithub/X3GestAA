...
exports.config = function(persistence, hostname, port, db, username, password) {
  exports.getSession = function(cb) {
    ...
    session.transaction = function (explicitCommit, fn) {
      ...
      if (explicitCommit) {
        tx.executeSql("START TRANSACTION", null, function(err){
          fn(err, tx);
        });
      }
      else 
    ...
    session.conn = conn;
    conn.connect(cb && function(err, r) {
		cb(err, session);
	});
    return session;
  };
  ...
  function transaction(conn){
    ...
    that.executeSql = function(query, args, callback){
      function cb(err, result){
        if (err) {
          // mysql does not support IF NOT EXISTS on CREATE INDEX, so we hack it here
          if (query.indexOf("CREATE INDEX") === 0 && err.message.indexOf("Duplicate key name") >= 0)
    ...
    that.commit = function(session, callback){
      session.flush(that, function(err){
    ...
    that.rollback = function(session, callback){
      that.executeSql("ROLLBACK", null, function(err) {
        session.clean();
        callback && callback(err);
      });
    };
  ...
  exports.mysqlDialect = {
    ...
    createTable: function(tableName, columns) {
      ...
      const TEMPORARY = persistence.temporary ? 'TEMPORARY ' : '';
      var sql = "CREATE "+ TEMPORARY + "TABLE IF NOT EXISTS `" + tableName + "` (";
      var defs = [];
