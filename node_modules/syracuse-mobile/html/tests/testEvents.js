...
(function( ) {
    ...
    $(document).ready(function() {
 		...
 		$('input[data-jqm="true"]').bind(allEvts.join(' '), function(evt){ 		
 			log($(evt.target).attr('id') + " : " + evt.type);
 			evt.stopPropagation();
 		});
 		var noJquery = $("#nojquery")[0];
 		noJquery.addEventListener ("keydown", function (evt) {
 			log("nojquery - keydown - " + evt.keyCode);
 			evt.stopPropagation();
 		}, false);
 		noJquery.addEventListener ("keyup", function (evt) {
 			log("nojquery - keyup - " + evt.keyCode);
 			evt.stopPropagation();
 		}, false);
 		noJquery.addEventListener ("keyup", function (evt) {
 			log("nojquery - keypress - " + evt.keyCode);
 			evt.stopPropagation();
 		}, false);
 		noJquery.addEventListener ("blur", function (evt) {
 			log("nojquery - blur");
 			evt.stopPropagation();
 		}, false);
 		$('#clickbtn').bind("click", function(evt){ 		
 			log($(evt.target).attr('id') + " : " + evt.type);
 			evt.stopPropagation();
 		});
