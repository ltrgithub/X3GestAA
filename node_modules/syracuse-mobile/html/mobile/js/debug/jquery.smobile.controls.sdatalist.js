...
(function($) {
	...
	that.controls.SDataList = {
		...
		navFooter: function($page, create) {
			var controller = this,
			...
			if (create) $page.append(html.join(''));
		...
		clickAll: function(e) {
			...
			if (e && e.target) {
				if (controller.multiSelOn()) {
					var l = $(e.target).closest(".s-m-slist-item");
					if (l) {
						l.toggleClass("multisel");
						// We remove this class for better ui
						l.removeClass("ui-btn-up-c");
						// Return false to stop evt processing - detail link is canceled 
						return false;
					}
				} else {
			...
			// No return to let JQM process action
		},
		...
		events: function($c, c) {
			...
			if ($sdata.capabilities.showSearch) {
				...
				$('#' + ceid + "_search_form").submit(that.controls.SDataList._execSearch.bind({
					id: ceid,
					controller: controller
				}));
			}
