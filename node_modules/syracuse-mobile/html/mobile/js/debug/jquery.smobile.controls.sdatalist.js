...
(function($) {
	...
	that.controls.SDataList = {
		...
		navFooter: function($page, create) {
			...
			isVisible = hasNext || hasPrev,
			nState = (hasPrev?((hasNext?3:1)):(hasNext?2:0));
			if (create) {
				var html = [];
				html.push('<div id="'+ controller.prefix + 'nav" data-role="footer" data-position="fixed" class="s-m-navig-parent ui-bar'+(isVisible?'':' s-m-hidden') + '">');
				...
				if (a === "setfavorite") {
					...
					if (elem) {
						...
						$sdata.switchFav(url, elem, function(res) {
							...
							controller.showHideFav(null, fav);
						});
