...
(function($) {
	...
	that.controls.SDataList = {
		...
		createItems: function(controller, options, html) {
			...
			var detail = null;
			if (cp.$links) {
				detail = $article.$detailsLink ? cp.$links[$article.$detailsLink] : null;
				if (!detail) {
					// #5837 - Authoring property to force detail link
					detail = cp.$links.$details;
				}
				if (detail && detail.$url && !detail.$isHidden) {
					detail = detail.$url;
				} else {
					detail = null;
				}
			...
			if (!options.$lookup) {
				...
				if (filters && filters.length && controller.config && controller.config.url) {
					...
					filters.forEach(function(filter, i) {
						...
						if (cf.$isHidden != true && cf.$isMandatory != true) {
							addHeader = true;
						}
					});
					if (addHeader === true) {
		...
		_execSearch: function(e) {
			...
			e.preventDefault();
			var purl = config.url;
			var proto = $.sdata.url2proto(purl);
			var cv = ($('#' + id + "_search")[0].value || "");
			var $ul = $('#' + id + "_list");
			var sv = (purl.query ? (purl.query.search || "") : "");
			var where = purl.query && proto && purl.query.filter && proto.$filters && proto.$filters[purl.query.filter] && proto.$filters[purl.query.filter].$where;

			if (!where && proto.$filters) { // if no filter selected, check for mandatory filter
				Object.keys(proto.$filters).forEach(function(filter, i) {
					var cf = proto.$filters[filter];
					if (cf.$isMandatory === true) {
						where = cf.$where;
					}
				});
				if (!where) {
					var df = $proto.defaultFilter(proto);
					where = df && proto.$filters[df] && proto.$filters[df].$where;
				}
			}
			...
			if (sv !== cv) {
				if (!cv) {
					...
					delete purl.query.noHighlight;
					purl.query.where = config.originUrl && config.originUrl.query && config.originUrl.query.where;
					if (!purl.query.where) {
					...
					purl.query.noHighlight = "true"; // Avoid display of <em>...match...</em>
					if (where) {
						if (config.originUrl && config.originUrl.query && config.originUrl.query.where) {
							purl.query.where = "(" + config.originUrl.query.where + ") and (" + that.controls.SDataList._tweakFilterNullChecks(where) + ")";
						} else {
							purl.query.where = "(" + that.controls.SDataList._tweakFilterNullChecks(where) + ")";
						}
					}
				}
				/*
		...
		applyFilter: function(controller, after, init) {
			var originWhere = controller.config.originUrl && controller.config.originUrl.query ? controller.config.originUrl.query.where : null;
			var purl = controller.config.url;
			...
			if (filters) {
				filters.forEach(function(filter) {
					...
					newFilter = newFilter.length == 0 ? cf : newFilter + ' and ' + cf;
				});
			}
			if (init || lastFilter != newFilter) {
				...
				if (hasFilter) {
					purl.query = purl.query || {};
					purl.query.where = (originWhere ? originWhere + ' and ' : '') + newFilter;
				} else if (purl.query) {
					purl.query.where = originWhere || "";
				}
		...
		_execFilter: function(e) {
			...
			var proto = $.sdata.url2proto(purl);
			var where = proto && proto.$filters && proto.$filters[cv] && proto.$filters[cv].$where;
			if (sv !== cv) {
				if (!cv) delete purl.query.filter;
				else {
					purl.query = purl.query || {};
					purl.query.filter = cv;
					if (where && purl.query.search) {
						if (config.originUrl && config.originUrl.query && config.originUrl.query.where) {
							purl.query.where = "(" + config.originUrl.query.where + ") and (" + that.controls.SDataList._tweakFilterNullChecks(where) + ")";
						} else {
							purl.query.where = "(" + that.controls.SDataList._tweakFilterNullChecks(where) + ")";
		...
		// sdata where may included checks for empty strings
		// empty strings are not indexed in the search index
		// that's why we also allow null as valid value in addition to ""
		_tweakFilterNullChecks: function(where) {
			// ((VISA eq "") and (STA ne 3)) ->
			// (((VISA eq "") or VISA eq null) and (STA ne 3))

			// ((VISA ne "") and (STA ne 3)) ->
			// (((VISA ne "") and VISA ne null) and (STA ne 3))
			return where.replace(/\((\w*)\seq\s""\)/g, function(m) {
				var isNull = m.replace('""', "null");
				return "(" + m + " or " + isNull + ")";
			}).replace(/\((\w*)\sne\s""\)/g, function(m, p, p1) {
				var isNull = m.replace('""', "null");
				return "(" + m + " and " + isNull + ")";
			});
		}
