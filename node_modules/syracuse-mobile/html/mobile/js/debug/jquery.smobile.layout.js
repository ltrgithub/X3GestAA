...
(function($) {
	...
	var layout = {
		CollapsibleLayout: function(layoutData) {
			...
			var b = ['<div id="' + layoutData.id + '" class="s-m-cl' + (options.$inquiryRoot ? ' inquiryroot' + options.$inquiry : '') + (addpadding ? '' : ' s-m-nopad') + '" data-role="collapsible"  data-iconpos="right" data-collapsed="' + (options.collapsed ? "true" : "false") + '"'],
				res = {
					childs: [],
					before: "",
					after: "",
			...
			b.push('>');
			if (options.text) b.push('<h' + (layoutData.size || 3) + (!isChild ? ' class="uppercase">' : '><div>') + $helpers.htmlEncode(options.text || "") + '</h' + (layoutData.size || 3) + '>');

			if (false && options.$inquiryRoot) {
				// Add a link inside the page to result/param 
				// -> It's a try to deal with param/result blocks
				b.push('<a id="inquiry');
				b.push(options.$inquiry);
				b.push('" class="s-m-inquiry-link" href="#inquiry');
				var target = options.$inquiry === "result" ? "param" : "result";
				b.push(target);
				b.push('">');
				b.push(target);
				b.push('</a>');
		...
		Block: function(layoutData) {
			...
			if (options.$isHeader) {
				...
				var app = controller.getApplication();
				if (app && app.icon) {
					b.push(app.icon + ' ');
				}
			}
		...
		AvancedBlock: function(layoutData) {
			...
			res.before = b.join('');
			b = [];
			b.push('</div><div id="' + id + '_expand" class="s-m-avanced-expand">');
			if (res.options.$opened) {
		...
		RowLayout: function(layoutData) {
			...
			if (options.$themeLayout) rclass.push("theme" + options.$themeLayout.toUpperCase());
			...
			options.widths.forEach(function(width, index) {
				...
				if (cc) rcss.push(cc);
				if (options.$emptycell == "empty") {
					// #3598 - Set Empty by default according to authoring $emptycell property
					// Cell is filled (gray) by default
					// We have to do this because of cells with no field attached - row layout 33/33/33 with only 2 fields declared
					// -> No control will be attached to the mast cell to set $emptycell options
					rcss.push("empty");
				}
	...
	that.addLayout = function(parentLayout, layoutData) {
		...
		if (layout[layoutData.layout]) {
			layoutData.data = layoutData.data || {};
			var isInquiryRoot = layoutData.data.$inquiry;
			if (parentLayout.$inquiry) {
				// Propagate to children
				layoutData.data.$inquiry = parentLayout.$inquiry;
			}
			if (isInquiryRoot) {
				// THis is the main bloc that will be opened 
				layoutData.data.$inquiryRoot = isInquiryRoot;
			}
