	...
	var layout = {
		CollapsibleLayout: function(layoutData) {
			...
			if (options.notCollapsable) {
				...
				if (isChild) {	
					options.jmdata["data-expanded-icon"] = "s-plus";
					options.jmdata["data-collapsed-icon"] = "s-minus";
				}  else {
					options.jmdata["data-expanded-icon"];
					options.jmdata["data-collapsed-icon"];
			...
			b.push('>');
			if (options.text) b.push('<h' + (layoutData.size || 3) + ' class="s-m-style uppercase">' + $helpers.htmlEncode(options.text || "") + '</h' + (layoutData.size || 3) + '>');
			res.before = b.join('');
			res.after = '</div>';
			return res;
