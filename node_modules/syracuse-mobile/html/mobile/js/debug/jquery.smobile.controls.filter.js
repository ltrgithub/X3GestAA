...
(function($) {
	...
	that.controls.SDataFilter = {
		...
		uidlg: function(config, controller) {
			var locale = $helpers.locale();
			var opvalues = config.opvalues;
			config.opvalues = null;
			var cp = that.controls.SDataList.field2proto(controller.parent, config.fieldValue);
			if (!cp) return;
			var ui = {
				...
				actions: {
					savesettings: function(after) {
						...
						var title = cd.valueTitle ? cd.valueTitle : cd.value;
						switch (cp.$type) {
							case "application/x-choice":
								config.valueTitle = null;
								cp.$value.$enum.forEach(function(item) {
									if (item.$value === cd.value) {
										config.valueTitle = $proto.execExpression(item.$title, cp);
									}
								});
								break;
							case "application/x-date":
								try {
									title = $helpers.date.toDateLocalized(Date.parseISO8601(cd.value));
								} catch (ex) {
									title = cd.value;
								}
								break;
							case "application/x-time":
								try {
									title = $helpers.date.toTimeLocalized(Date.parseISO8601(cd.value));
								} catch (ex) {
									title = cd.value;
								}
								break;
							case "application/x-datetime":
								try {
									title = $helpers.date.toDateTimeLocalized(Date.parseISO8601(cd.value));
								} catch (ex) {
									title = cd.value;
								}
								break;
			...
			$.smobile.createDetail(ui, config, controller);
		},
		events: function($c, c) {
			var controller = this;
			var $ul = $c;
