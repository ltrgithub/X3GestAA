...
(function($) {
	...
	that.controls.Lookup = {
		...
		createLookup: function(controller, c) {
			...
			//  Issue #2298 - X3 Errors are returned on $rvalue field 
			var errField = cp.$rvalue ? cp.$rvalue : options.bind;
			//var protoRef = $proto.getProto(page, options.reference);
			var protoRef = $proto.getProto(page, errField);
			that.ui.addErrors(controller, errField, c.id, html);
			...
			if (!options.reference || options.$isReadOnly) html.push('readonly="true" ');
