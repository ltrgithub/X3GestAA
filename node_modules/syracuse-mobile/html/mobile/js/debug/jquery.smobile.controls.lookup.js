...
(function($) {
	...
	that.controls.Lookup = {
		...
		createLookup: function(controller, c) {
			...
			//  Issue #2298 - referenceField is the filed that contains the value (given by cp.$value) and the error
			...
			html.push('</label><div  class="ui-input-text s-m-editfld "><input type="text" name="' + c.id + '" id="' + c.id + '_i" ');
			...
			var protoRef = that.controls.Lookup.getFieldProto(controller, options, referenceField);
			...
			$input.blur(function(evt) {
				...
				try {
					if (c.bind) {
						...
						//var cp = $proto.getProto(controller.getPageData(), c.options.bind);
						var cp = that.controls.Lookup.getFieldProto(controller, c.options);
