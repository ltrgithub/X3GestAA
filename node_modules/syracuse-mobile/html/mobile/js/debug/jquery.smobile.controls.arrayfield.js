...
(function($) {
	...
	_controls.types._table_row = function(html, fields, $prototype, value, controller, edit, index, noDetailLink, parentBind) {
		if (!fields) return;
		html.push('<tr>');
		var prop = $prototype.$properties;
		var first = true;
		fields.forEach(function(field) {
			...
			if (proto.$type === "application/x-reference") {
				...
				if (!edit && (proto.$type === "application/x-array")) {
					if (value && value[field] && value[field].length) {
	...
	_controls.ArrayField = {
		...
		_cards: function(controller, id, options, html) {
			var ui = that.detailUiData(controller, options.cacheData.$proto, options.cacheData.data[options.$index], options.cacheData.layout, {inlinePrefix: options.$bind + "_", readOnly: true});
			var dhtml = {
				root: {
					before: "",
					after: "",
			...
			that.renderPageHTML(controller, ui.uiData.content, dhtml, {pageData: detailpagedata, prefix:options.$bind, data: options.cacheData.data[options.$index]});
			var dcontent = [];
							...
							switch (field.$proto.$type) {
								...
								case "application/json":
									...
									if ((layout.$layout && !layout.$format) || (layout.$format === "grid")) {
										_controls.types._table_header(html, layout.$layout.$items, field.$proto, layout.$cards?false:true);
										...
										if (layout.$format === "cards") {
											if (fieldData && fieldData.length) {
												...
												html.push('</div>')
												that.controls.navFooter(((options.$index+1) < options.cacheData.data.length), (options.$index>0), c.id+'_nav', true, {
													"jmdata": { "data-role":"footer"}}, html)											
				...
				if (c.options.$cards) {
					$c.on("swipeleft", function(event) {
						if ((c.options.$index + 1) < c.options.$len) {
							...
							that.controls.navFooter(((c.options.$index+1) < c.options.cacheData.data.length), (c.options.$index>0), c.id+'_nav', false);	
