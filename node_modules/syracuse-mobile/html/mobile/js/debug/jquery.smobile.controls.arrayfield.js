...
(function($) {
	...
	// TODO - Call addStylesReference in table_row
	_controls.types.addStylesReference = function(controller, html, refOptions, $article) {
		if (!refOptions) return;
		html.push('<li>');
		if ($article && $article.$refDescription && $article.$isRrefDescrHidden !== true) {
			// #3872 - With styles - Optimization - If c (center style) in style value we should add it in resfDescription....
			...
			if (refOptions.title) {
				...
				before = $article.$refDescription.$before === true;
				$proto.auth.mobileExtentions(cssl, $article.$refDescription);
			}
			var rval = refOptions.value;
			refOptions.value = $helpers.htmlEncode(before ? refOptions.title : rval);
			var rttl = $helpers.htmlEncode(before ? rval : refOptions.title);
			delete refOptions.title;
			that._addLink(html, refOptions, controller);
			html.push('</span>');
			if (cssl && rttl) {
				html.push(' <span class="');
				html.push(cssl.join(' '));
	...
	// Adjust display mode (grid or stacked) according to available width
	// tale.class is set with ui-responsive by default - widest mode
	_controls.types.table_checkResponsive = function(parent) {
		if (!parent) return;
		if (!parent.jquery) parent = $(parent);
		if (!parent.is("div.s-m-table-wrapper")) {
			parent = parent.find("div.s-m-table-wrapper");
		}
		if (parent.length == 0) return;
		var table = parent.find("table.s-m-table");
		// Works only if table is visible it's why we call the method on show/hide collapsible blocks
		...
		// I tried to deal with overflow and overflow changed event but it's not really cross browsers
		...
		if (table.length > 0 && table.attr("data-mode") == "reflow") {
			var overflowDiv = parent.get(0);
			if (overflowDiv && parseInt(overflowDiv.offsetWidth) < parseInt(overflowDiv.scrollWidth)) {
				// Disable grid mode -> stacked fields
				table.removeClass("ui-responsive");
			}
		}
	};
	_controls.types.table_refresh = function($parent) {
		if (!$parent) return;
		// Force responsive (mode with largest width)
		var table = $parent.find("table.s-m-table").addClass("ui-responsive");
		// Create table widget after html update
		table.parent().trigger('create');
		_controls.types.table_checkResponsive($parent);
	};
	_controls.types._table_header = function(html, $layout, $prototype, noDetailLink, edit) {
		...
		html.push('<div class="s-m-table-wrapper">');
		// ui-responsive by default - Display mode is check after table has been created in dom - see table_checkResponsive
	...
	_controls.types._table_row = function(html, $layout, $prototype, rowVal, controller, edit, index, noDetailLink, parentBind) {
		...
		fields.forEach(function(fieldArticle) {
			...
			if (!fieldHidden) {
				...
				if (value == null && rValue && rowVal) {
					//  #4000 - Try to read row val
					value = rowVal[rValue];
				}
				...
				if (!fieldEmpty) {
					...
					htmlVal.push('"');
					htmlVal.push('>');
					if (fieldArticle.$expression) {
						var defValue = ($proto.htmlValue(rowVal, $prototype, proto, {
							$type: proto.$type,
							$bind: field
						}) || "").trim();
						var v = fieldArticle.$expression.replace(/\{(.*?)\}/g, function(match, prop) {
							if (prop === field) return defValue;
							var cppp = $prototype.$properties[prop];
							if (!cppp) return "";
							return $proto.htmlValue(rowVal, $prototype, cppp, {
								$bind: prop,
								$type: cppp.$type
							}, '', {
								list: true
					...
					} else if (proto.$type === "application/x-reference") {
						// TODO - Use addStylesReference function
						styles.$refDescription = fieldArticle.$refDescription || $layout.$refDescription || {};
						styles.$isRrefDescrHidden = fieldArticle.$isRrefDescrHidden === true || $layout.$isRrefDescrHidden === true;
						var refOpts = _controls.types.addReference(proto, [value, rowVal], controller, field, styles.$refDescription.$withTitle);
						fieldEmpty = refOpts.value == null || refOpts.value.trim().length == 0;
						var before = false;
						var cssl = null;
						if (refOpts.title && styles.$isRrefDescrHidden !== true) {
							cssl = ["s-m-style", "s-m-linkdesc"];
							if (styles.$refDescription) {
								before = styles.$refDescription.$before === true;
								$proto.auth.mobileExtentions(cssl, styles.$refDescription);
							}
						}
						...
						//#5818 - Add parentData option to allow resolution of image url expression
			...
			if (first) {
				...
				if (proto.$externalLinks) {
					Object.keys(proto.$externalLinks).forEach(function(link) {
						htmlCells.push('<div class="s-m-style s-m-value-link">');
						var text = $proto.parseExpression(proto.$externalLinks[link].$title, {
							data: controller.exprData(dd),
							$prototype: proto.$protoparent
						}, "$title", {
							html: true
						});
						var href = $proto.parseExpression(proto.$externalLinks[link].$url, {
							data: controller.exprData(rowVal),
							$prototype: proto.$protoparent
						}, "$url", {
							html: false
						});
						htmlCells.push('<a href="' + href + '" target="_blank">' + text + '</a>');
						htmlCells.push("</div>");
					});
				}
				htmlCells.push('</td>');
	...
	_controls.ArrayField = {
		...
		handler: function(c, after) {
			...
			var isTitleHidden = $article.$isTitleHidden;
				...
				switch (cpt) {
					...
					case "application/x-array":
						...
						if (ct) {
							...
							html.push('<li  class = "s-m-arrayfld-ttl');
							var ttlcss = [];
							$proto.auth.mobileExtentionsLbl(ttlcss, $article);
							if (ttlcss.length > 0) {
								html.push(' s-m-style ' + ttlcss.join(' '));
							}
							html.push('" data-role="list-divider">' + ct + '</li>');
							...
							switch (field.$proto.$type) {
								case "application/x-reference":
									if (fieldData.length) fieldData.forEach(function(value) {
										...
										// #3872
										_controls.types.addStylesReference(controller, html, toptions, $article);
									});
									break;
								case "application/x-object":
								case "application/json":
									...
									if ((layout.$layout && !layout.$format) || (layout.$format === "grid")) {
										// Propagate from grid layout to  $item layout
										$helpers.propagateMobileExt(layout.$layout, layout);
										...
										if (layout.$format === "cards") {
											if (fieldData && fieldData.length) {
												...
												$helpers.propagateMobileExt(options.cacheData.layout, layout);
		...
		refresh: function(controller, options, $c, id, cbAfter) {
			// Refresh table - Adjust reflow mode    
			if (cbAfter) {
				// cbAfter provided if page is not already built
				// -> To refresh grid we need page dom elmt to calculate overflow
				cbAfter.push(function() {
					_controls.types.table_refresh($c);
				});
			} else {
				_controls.types.table_refresh($c);
			}
		...
		events: function($c, c) {
			var controller = this;
			var opts = c.options;
			var edit = opts.$article && opts.$article.$inquiry === "result" ? false : (opts.facet === "$edit") || (opts.facet === "$create");
			if (!edit) {
				if (opts.$cards) {
					$c.on("swipeleft", function(event) {
						if ((opts.$index + 1) < opts.$len) {
							opts.$index++;
							var html = [];
							_controls.ArrayField._cards(controller, c.id, opts, html);
							var $ci = $('#' + c.id + "cards");
							$ci.html(html.join(''));
							$ci.trigger('create');
							controller.setHtmlEvents();
							that.controls.navFooter(false, ((opts.$index + 1) < opts.cacheData.data.length), (opts.$index > 0), c.id + '_nav', false);
					...
					$c.click(function(e) {
						...
						if (p) {
							if (p === "detail") {
								...
								cp = $proto.getProto(page, opts.$bind);
								cd = controller.getValue(opts.$bind, null, []);
								if (cd && Array.isArray(cd) && (p >= 0)) {
									cd = cd[p];
									//TODO
									$layout = $proto.auth.findSlayout(opts.$article, opts.$bind, false, true);
									pp = cp;
									nd = cd;
										...
										if (cd && Array.isArray(cd) && (p.$index >= 0)) {
											...
											if (cd) {
												...
												if (cd && cd[p.$bind]) {
													...
													pp = cp;
													var $layout = $proto.auth.findSlayout(opts.$article, p.$bind, true);
													if (!$layout) {
														$layout = {};
														$helpers.propagateMobileExt($layout, opts.$article);
													}
			...
			// Refresh table - Adjust reflow mode
			_controls.types.table_checkResponsive($c);
		}
	};
})(jQuery);