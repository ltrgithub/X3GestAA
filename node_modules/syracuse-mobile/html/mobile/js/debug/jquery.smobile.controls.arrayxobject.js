...
(function($) {
	...
	_controls.ArrayObjectRef = {
		// #5818 - add parentBind
		_renderContent: function(controller, options, cp, id, html, fieldData, parentBind) {
			...
			if (options.$article) {
				$helpers.propagateMobileExt($lyt, options.$article);
			}
			_controls.types._table_header(html, $lyt, cp, false, true);
			fieldData.forEach(function(value, index) {
				_controls.types._table_row(html, $lyt, cp, value, controller, true, index, false, parentBind);
			});
		...
		refresh: function(controller, options, $c, id, cbAfter) {
			...
			var html = [];
			// #5818 - add parentBind
			var uuid = _controls.ArrayObjectRef._renderContent(controller, options, cp, id, html, fieldData, options.bind);
			...
			if (cbAfter) {
				...
				cbAfter.push(function() {
					_controls.types.table_refresh($("#" + uuid));
				});
			} else {
				_controls.types.table_refresh($("#" + uuid));
			}
		...
		createArrayObjectRef: function(controller, c, fieldData) {
			if (fieldData == null)
				fieldData = controller.getValue(c.data.bind, null, []);
			var options = c.data || {};
			var page = controller.getPageData();
			var cp = $proto.getProto(page, options.bind);
			var arrayproto = $proto.getProto(page, options.bind, true);
			...
			html.push('>');
			// #5818 - add parentBind
			_controls.ArrayObjectRef._renderContent(controller, options, cp, c.id, html, fieldData, c.data.bind);
			if (options.hasButton)
				html.push(_controls.ArrayObjectRef._addLast(c.id, options));
			html.push('</ul>');
			...
			$ul.click(function(e) {
				if (e && e.target) {
					...
					if (p === "edit") {
						...
						if (layout.$layout) {
							// Like we crate a page from layout we need to have styles in the first $layout instead of root
