...
(function($) {
	...
	_controls.ArrayObjectRef = {
		_renderContent: function(controller, options, cp, id, html, fieldData, createList) {
			...
			html.push('>');
			_controls.types._table_header(html, (options.$gridItems ? options.$gridItems : options.$article.$layout.$items), cp, false, true);

			fieldData.forEach(function(value, index) {
				_controls.types._table_row(html, options.$gridItems ? options.$gridItems : options.$article.$layout.$items, cp, value, controller, true, index);
			});
