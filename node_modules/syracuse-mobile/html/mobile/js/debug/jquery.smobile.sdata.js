...
(function($) {
	...
	that.localStorage = {
		...
		_isAvailable: function() {
			if (that.localStorage._available !== null) {
				return that.localStorage._available;
			}
			that.localStorage._available = false;
			try {
				window.localStorage.setItem("__test_availability__", "__test_availability__");
				that.localStorage._available = true;
			} catch (e) {}
			return that.localStorage._available;
		},
		getItem: function(key, removeAfter) {
			if (!that.localStorage._isAvailable()) {
				that.localStorage._log && console.log("local.unsupported.getItem(" + key + "):" + val);
				return;
			}
			var val = window.localStorage.getItem(key);
			that.localStorage._log && console.log("local.getItem(" + key + "):" + val);
			if (true === removeAfter) {
				that.localStorage.removeItem(key);
			}
			return val;
		},
		setItem: function(key, value) {
			...
			that.localStorage._log && console.log("local.setItem(" + key + "):" + value);
			return window.localStorage.setItem(key, value);
		},
		removeItem: function(key) {
			if (!that.localStorage._isAvailable()) {
				that.localStorage._log && console.log("local.unspported.removeItem(" + key + ")");
				return;
			}
			that.localStorage._log && console.log("local.removeItem(" + key + ")");
	...
	that.sessionStorage = {
		...
		_log: false,
		_isAvailable: function() {
			if (that.sessionStorage._available !== null) {
				return that.sessionStorage._available;
			}
			that.sessionStorage._available = false;
			try {
				window.sessionStorage.setItem("__test_availability_session__", "__test_availability_session__");
				that.sessionStorage._available = true;
			} catch (e) {}
			return that.sessionStorage._available;
		},
		getItem: function(key) {
			if (!that.sessionStorage._isAvailable()) {
				that.sessionStorage._log && console.log("session.fallback.getItem(" + key + "):" + val);
				return that.sessionStorage._backupMap[key];
			}
			var val = window.sessionStorage.getItem(key);
			that.sessionStorage._log && console.log("session.getItem(" + key + "):" + val);
			return val;
		},
		setItem: function(key, value) {
			if (!that.sessionStorage._isAvailable()) {
				that.sessionStorage._log && console.log("session.fallback.setItem(" + key + "):" + value);
				that.sessionStorage._backupMap[key] = value;
				return;
			}
			that.sessionStorage._log && console.log("session.setItem(" + key + "):" + value);
			return window.sessionStorage.setItem(key, value);
		},
		removeItem: function(key) {
			if (!that.sessionStorage._isAvailable()) {
				that.sessionStorage._log && console.log("session.fallback.removeItem(" + key + ")");
				delete that.sessionStorage._backupMap[key];
				return;
			}
			that.sessionStorage._log && console.log("session.removeItem(" + key + ")");
	...
	that.loadRepData = function(representation) {
		...
		var value = $.sdata.localStorage.getItem(that.config.id);
		if (value) {
	...
	that.saveRepData = function(representation, value) {
		...
		var dd = $.sdata.localStorage.getItem(that.config.id) || "{}";
		try {
			dd = JSON.parse(dd);
		} catch (ex) {
			dd = {};
				...
				success: function(data, headers) {
					...
					that.createDraft(draftData, function(draft) {
						...
						odata.$info = {
							...
							// #3369 - We need to know if current mode is creation - It's given by original_url because of local working copies
							isCreation: draft.original_url.indexOf("/$template") > 0,
							inquiryResponse: odata.$info && odata.$info.inquiryResponse
						};
					...
					if (cd.$resources) {
						$helpers.each(cd.$resources,
							...
							function(value, index, next) {
								...
								// #3468 - Temporarilly fix this bug - Add send $action in response
								if (send.$actions && value && !value.$actions && (send.$httpMethod == "PUT" || send.$httpMethod == "POST")) {
									value.$actions = $.extend(true, {}, send.$actions);
								}
	...
	// #3468 - Temporarilly fix this bug - don't remove $action in sent data
	var _mobileInfo = ["$info", "$nbFieldErrors", "$localUrl", "$properties", "$baseUrl", "$prototype", "$representation", "$type", "$diagnoses"];
	...
	that._success = function(cd, hack, opts) {
		if (!opts) opts = {};
		...
		if (cd && opts.noDiagnoses !== true) {
			return $helpers.pushDiagnoses(cd, opts);
		}
		return null;
	};
	that.post = function(url, data, next, headers, opts) {
		...
		var doPost = function(retry) {
			...
			$.ajax({
				...
				success: function(cd) {
					try {
						that._success(cd, url != curl, opts);
						next(cd);
					} catch (e) {
						if (opts.errHandler) {
							opts.errHandler(e);
			...
			$.ajax({
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					if (status == 200 && (status.responseText == null || status.responseText.trim() == 0)) {
						errMsg = "Unexpected empty server response";
					}
					...
					$helpers.pushAjaxError(jqXHR, textStatus, errorThrown, url, "GET", (status === 401) ? $helpers.locale().errorauth_title : errMsg, (status === 401) ? $helpers.locale().errorauth : null);
					if (status === 401) that.capabilities.resetInterval = true;
					var rheaders = {
						status: status,
						location: jqXHR ? jqXHR.getResponseHeader("Location") : ""
	...
	that.execGetSData = function(uri, options) {
		...
		if (!local && !usefav) {
			that.authenticate(function(ad, errors, offline) {
				...
				if (uri.query && uri.query.localstorage) {
					// Used for actionmethod calls
					// Data is stored in local storage and page is rebuilt with these data in order to take into account META6DATA
					// -> Controller.Refresh method just refresh values
					// Try to read data in localstorage - If no data or failed -> Read sdata url
					var data = $.sdata.localStorage.getItem(uri.query.localstorage, true);
					if (data) {
						try {
							data = JSON.parse(data);
							// To improve - for other needs
							// clearErrors to display only diagnoses contained by data
							$helpers.clearErrors();
							// setDisplayErrorAsMessage=true for method call - evntually add a new param in query displayErrorAsMessage:true
							$helpers.setDisplayErrorAsMessage();
							// Tells controller that it's a method call response - Todo add param in query
							if (!data.$info) data.$info = {};
							data.$info.inquiryResponse = true;
							// Call get/post/put std success method
							that._success(data, false, {
								checkFieldsError: true,
								prototype: that.url2proto(uri)
							});
							options.success(data, null);
							return;
						} catch (e) {
							console.log("execGetSData - Error parsing Localstorage Data [" + uri.query.localstorage + "]\n" + e.message);
						}
					} else {
						console.log("execGetSData - Localstorage not found [" + uri.query.localstorage + "]");
					}
				}
					...
					if (uri.query.where || filter) {
						...
						if (filter) {
							if (proto && proto.$filters && proto.$filters[filter]) {
								...
								if (fwhere) {
									// SAM 96900
									// Remove X3 prefixes results in a valid where clause that is always false
									// resulting in no favorites displayed at all.
									// This prevents visibel error and can be corrected by app devs
									// by using a filter clause that relies on class properties that are available on the client
									fwhere = fwhere.replace(/\[(.*?)\]/g, "");
									str = (str ? (str + " and " + fwhere) : fwhere);
								}
							}
						}
	...
	that.getAutocompleteList = function(value, opts, cb) {
		var matches = [];
		var stored = $.sdata.localStorage.getItem("s-search-terms-array");
		if (stored) {
			try {
	...
	that.updateAutocompleteList = function(value, opts, cb) {
		...
		var stored;
		if (value) {
			stored = $.sdata.localStorage.getItem("s-search-terms-array");
			if (stored) {
				try {
					stored = JSON.parse(stored);
				} catch (e) {
			...
			$.sdata.localStorage.setItem("s-search-terms-array", JSON.stringify(newList));
		}
		cb();
	};
