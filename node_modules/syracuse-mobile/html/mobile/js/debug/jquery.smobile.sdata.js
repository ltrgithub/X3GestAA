					...
					if (uri.query.where || filter) {
						...
						if (filter) {
							if (proto && proto.$filters && proto.$filters[filter]) {
								...
								// SAM 96900
								// https://github.com/Sage-ERP-X3/Syracuse/issues/3537
								// filter where clauses contain sometimes X3 expressions which can only be
								// evaluated serverside. Applying the client side triggers serveral error or
								// causes the where clause to be false all the time.
								// Because of this, the where clause is disabled for now to show favorites in filtered
								// lists all the time - This will show all favorites picked in all filtered lists no
								// matter if they match the expression or not.
								fwhere = null;
								
								if (fwhere) {
									
									// SAM 96900
									// only removing X3 prefix results in a valid where clause that is always false
									// resulting in no favorites displayed at all -> no valid option
