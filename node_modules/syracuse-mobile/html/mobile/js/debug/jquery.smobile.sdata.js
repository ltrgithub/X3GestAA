...
(function($) {
	...
	that.sdataAdmin = function() {
		...
		return window.location.protocol + "//" + window.location.host + that.capabilities.installUrl;
	};
	...
	that.createGetDataId = function(purl, data, proto) {
		proto = proto || that.url2proto(purl);
		if (!proto) return data.$uuid || "";
		var isuuid = $proto.useUuid(proto);
		// #4043 - Force uuid for createActivity sdata id's because using $key doesn't work if we don't have the data
		// -> sometime the fields of the key are calculated after SAve action (see expenses)
		if (isuuid || true) {
		...
		var doPost = function(retry) {
			...
			$.ajax({
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					if (!$helpers.handleHttp401(status, jqXHR, doNext)) {
			...
			$.ajax({
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					if (opts.errHandler) {
						...
						var status = jqXHR ? jqXHR.status : 200;
						if (!$helpers.handleHttp401(status, jqXHR, next)) {
							if ((status === 401) && canRetry) {
								--retry;
								return _retry(retry);
							}
	...
	that.GETSYNC = function(url, headers, options) {
		var _retry;
		var result;
		var doGet = function(retry) {
			if ($helpers.trace.ajax) {
			...
			$.ajax({
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					if (!$helpers.handleHttp401(status, jqXHR, null)) {
						if ((status === 401) && canRetry) {
							--retry;
							return _retry(retry);
						}
	...
	that.getApplication = function(id) {
		if (!that.applications) return;
		var app = null;
		that.applications.some(function(a) {
			if (a.id === id) {
				app = a;
				return true;
			}
