...
(function($) {
	...
	that.extractErrors = function(data, defaultMsg, types) {
		return $helpers.dataReadErrors(data, defaultMsg, true, "", null, types);
	};
					...
					if (cd.$resources) {
						...
						function(value, index, next) {
							...
							if (send.$httpMethod == "PUT") {
								// GLUTE - Issue #2306 - Set 400 status if errors - To remove once bug fixed
								errors = that.extractErrors(value, value.$httpMessage, ["error", "fatal"]);
								if (value.$httpStatus == 200 && errors && errors.errors.length > 0) value.$httpStatus = 400;
								switch (value.$httpStatus) {
									...
									case 400:
										...
										if (options.editActivity && errors.onlyFieldsErrors) {
											// pushFieldsErrors just set a status thatwill be checked later
											$helpers.pushFieldsErrors();
										} else {
											$helpers.pushException({
								...
								switch (value.$httpStatus) {
									case 200:
										// GLUTE - Issue #2306 - Set 201 status if status=200 - To remove once bug fixed
										// GLUTE - X3 RETURNS A 200	instead of a 201 and no $httpLocation
									...
									case 201:
										...
										that._checkBatchMsgs(value);
										newurl = value.$httpLocation ? that.createUrl(value.$httpLocation, presUrl, value) : null;
										return $.sdata.delData(resUrl, function() {
											var nfunc = function(data, error) {
												if (error) {
													return next(data, error);
									...
									case 400:
										...
										// if editActivity we don't generate global error if only fields errors are returned by server
										if (options.editActivity && errors.onlyFieldsErrors) {
											// pushFieldsErrors just set a status that will be checked later
											$helpers.pushFieldsErrors();
										} else {
											$helpers.pushException({
												message: errors.message
	...
	that._success = function(cd, hack) {
		...
		if (cd) $helpers.pushDiagnoses(cd);
	};
