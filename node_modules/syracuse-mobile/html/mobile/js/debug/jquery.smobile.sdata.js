...
(function($) {
	...
	var _mobileInfo = ["$info", "$nbFieldsError", "$localUrl"];
	...
	that._removeMobileInfo = function(data) {
		_mobileInfo.forEach(function(prop) {
			delete data[prop];
		});
	};
	...
	that.post = function(url, data, next, headers, errHandler) {
		var _retry;
		var doPost = function(retry) {
			if ($helpers.trace.ajax) {
			...
			$.ajax({
				...
				error: function(jqXHR, textStatus, errorThrown) {
					...
					if (errHandler) {
						errHandler(jqXHR, textStatus, errorThrown);
					} else {
						var status = jqXHR ? jqXHR.status : 200;
						if ((status === 401) && canRetry) {
							--retry;
