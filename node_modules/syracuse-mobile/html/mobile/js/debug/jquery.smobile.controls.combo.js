...
(function($) {
	...
	that.controls.Combo = {
		...
		_createSelect: function(controller, c) {
			...
			if (key == null) {
				// #7479 Add a blank line if no value provided by the server
				html.push("<option value=\"\" selected=\"true\"></option>");
			}
			options.items.forEach(function(item) {
				html.push("<option value=\"" + item.value + "\"" + ((key && key == item.value) ? " selected=\"selected\"" : "") + ">" + item.title + "</option>");
			});
			html.push('</select>');
		...
		_createRadio: function(controller, c, choiceLayout) {
			...
			var html = [];
			var errors = that.ui.addFieldContainer(controller, html, c.id, options.bind, "ui-icon-alt");
			html.push('<fieldset class="s-m-radiocombo" data-role="controlgroup" data-mini="true" id="');
			html.push(c.id);
			html.push('_i" ');
			if (nbCols == -1) {
