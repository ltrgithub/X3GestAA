...
(function($) {
	...
	_controls.ArrayObjectRef = {
		_renderContent: function(controller, options, cp, id, html, fieldData, createList) {
			...
			//options.cards = false;
			if (!options.cards) {
				_controls.types._table_header(html, options.items , cp);
			}
			fieldData.forEach(function(value, index) {
				if (options.cards) _controls.types._cards(html, {$items: options.items || []}, cp, value, controller, true, index, options.bind);
