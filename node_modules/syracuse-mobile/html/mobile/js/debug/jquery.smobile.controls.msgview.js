	...
	that.controls.MsgView = {
		...
		handler: function(c, after) {
			...
			var errormsgs = data && data.filter(function(msg) {
				...
				return ($helpers.error2Type(msg.type) === "error" || $helpers.error2Type(msg.type) === "fatal" || $helpers.error2Type(msg.type) === "warning");
			}) || [];
			var infomsgs = data && data.filter(function(msg) {
				return ($helpers.error2Type(msg.type) === "info" || $helpers.error2Type(msg.type) === "success");

				$helpers.error2Type(msg.type) === "error" || $helpers.error2Type(msg.type) === "fatal" || $helpers.error2Type(msg.type) === "warning";
			}) || [];
			...
			var errPage = options.errorPage,
				success = data.length == 1 && data[0].type == $helpers.ERRORS.success;
			html.push('<ul id="' + c.id + '" data-theme="g" data-role="listview"');
			html.push(' data-inset="true" class="s-m-error-view"');
			html.push('>');
			controller.errorViewHasErrors = errormsgs.length; //data && data.length;
			...
			if (controller.errorViewHasInfo && !controller.errorViewHasErrors) {
				...
				var mail = [];
				// add close button
				if (!options.errorPage && !success) that.controls.MsgView._setCloseBox(data, c.id, html, buttons);
				// add errors
				data.forEach(function(cd) {
					that.controls.MsgView._additem(cd, html, mail, buttons);
				});
				// add send mail

				if (mail.length > 0) that.controls.MsgView._addMail(html, mail, c.id, buttons);
				//$helpers.clearErrors();
				if (success) that.controls.MsgView.autoClose(c.id);
			}
			html.push('</ul>');
		...
		refresh: function(controller, options, $c, id) {
			...
			if (controller.errorViewHasErrors) {
				...
				if (!success) that.controls.MsgView._setCloseBox(data, id, html, buttons);
				data.forEach(function(cd) {
					that.controls.MsgView._additem(cd, html, mail, buttons);
				});
				if (mail.length > 0) that.controls.MsgView._addMail(html, mail, id, buttons);
				$c.html(html.join(''));
				...
				if (success) that.controls.MsgView.autoClose(id);
			} else {
				$c.html('');
			}
