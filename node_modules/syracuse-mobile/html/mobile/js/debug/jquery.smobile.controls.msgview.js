	...
	that.controls.MsgView = {
		...
		_renderMessage: function(controller) {
			...
			if (!$helpers.displayErrorAsMessage() && (infos > 0 && errors == 0)) {
				// We force display as top message if info only if not already enabled qsdqsd qsqsf qsdqsdqs sdqsfdqs sfqsfqsf
			...
			if ($helpers.displayErrorAsMessage()) {
				...
				timeout += Math.min(Math.round(nbChars / 40) * 500, 5000);
