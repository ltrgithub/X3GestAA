...
(function($) {
	...
	that.controls.SyncView = {
		...
		create_items: function(controller, data, html, mainId) {
			var lineIdx = 0;
			var _addBtn = function(opts) {
				html.push('<div class="ui-grid-a">');
				for (var p in opts) {
					html.push('<div class="ui-block-' + p + '">');
					if (opts[p] != null) {
						var bid = mainId + '_' + lineIdx + opts[p].id;
						html.push('<a  data-iconpos="top" id="');
						html.push(bid);
						html.push('" data-role="button" data-theme="f" ');
						html.push('data-mini="true" ');
						for (var d in opts[p].jmdata) {
							html.push(d);
							html.push('="');
							html.push(opts[p].jmdata[d]);
							html.push('" ');
						}
						html.push('>');
						html.push('</a>');
			...
			Object.keys(data).forEach(function(value) {
				...
				list.forEach(function(cd, index) {
					...
					html.push('>');
					var coptions = [];
					coptions.push("<p class=\"s-m-synch-error\"><strong>");
					coptions.push("Key : " + $helpers.htmlEncode(cd.$info.title || "..."));
					coptions.push("</strong></p>");
					if (!cd.$info.last_error) cd.$info.last_error = "This is the error message";
			...
			var opts = {
				success: function() {
					...
					if (reload) {
						...
						var appid = controller.applicationId;
						$.smobile.removeAllPages([controller.currentDoomId()]);
