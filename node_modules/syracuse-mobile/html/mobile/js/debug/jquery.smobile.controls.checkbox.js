...
(function($) {
	...
	that.controls.CheckBox = {
		createCheckBox: function(controller, c) {
			var options = c.data || {};
			var html = options.noform ? [] : ['<form id="' + c.id + '">'];
			...
			if (!options.noform) html.push('</form>');
			return {
				html: html.join(''),
				bind: options.bind,
		...
		refresh: function(controller, options, $c, id) {
			var $input = $('#' + id + "_i");
			$input.attr('checked', controller.getValue(options.bind, null, false)).checkboxradio("refresh");
