	...
	that.controls.ErrorView = {
		...
		handler: function(c, after) {
			...
			if (controller.errorViewHasErrors) {
				var mail = [];
				// add close button
				if (!options.errorPage && !success) that.controls.ErrorView._setCloseBox(data, c.id, html, buttons);
				// add errors
				...
				//$helpers.clearErrors();
				if (success) that.controls.ErrorView.autoClose(c.id);
			}
			html.push('</ul>');
		...
		refresh: function(controller, options, $c, id) {
			...
			if (controller.errorViewHasErrors) {
				...
				if (!success) that.controls.ErrorView._setCloseBox(data, id, html, buttons);
				data.forEach(function(cd) {
					that.controls.ErrorView._additem(cd, html, mail, buttons);
				});
				...
				if (success) that.controls.ErrorView.autoClose(id);
			} else {
				$c.html('');
			}
