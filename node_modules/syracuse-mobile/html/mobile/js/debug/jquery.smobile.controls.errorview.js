...
(function($) {
	...
	that.controls.ErrorView = {
		_additem: function(cd, html, mail, buttons, isInfoMsg) {
			isInfoMsg = isInfoMsg === true;
			cd.data = cd.data || {};
			html.push('<li');
			var css = [],
			...
			if (!isInfoMsg) {
				if (mail && (cd.type != $helpers.ERRORS.info) && (cd.type != $helpers.ERRORS.success)) {
					mail.push(cd.title);
				}
		...
		handler: function(c, after) {
			...
			// Tempo only to display a success message or a global message if server has returned field errors 
			var isInfoMsg = ($helpers.hasFieldsErrors() && data.length > 0) || (data.length == 1 && data[0].type == $helpers.ERRORS.success);
			...
			if (controller.errorViewHasErrors) {
				var mail = [];
				// add close button
				if (!options.errorPage && !isInfoMsg) {
					that.controls.ErrorView._setCloseBox(data, c.id, html, buttons);
				}
				// add errors
				data.forEach(function(cd) {
					that.controls.ErrorView._additem(cd, html, mail, buttons, isInfoMsg);
				});
				// add send mail
				if (!isInfoMsg && mail.length > 0) {
					that.controls.ErrorView._addMail(html, mail, c.id, buttons);
				}
				// We don't clear field Errors
				$helpers.clearErrors(true);
			}
			if (isInfoMsg) that.controls.ErrorView.autoClose(c.id);
			html.push('</ul>');
		...
		refresh: function(controller, options, $c, id) {
			...
			if (controller.errorViewHasErrors) {
				var html = [],
					mail = [],
					buttons = [];
				var isInfoMsg = $helpers.hasFieldsErrors() || (data.length == 1 && data[0].type == $helpers.ERRORS.success);
				if (!isInfoMsg) {
				...
				if (!isInfoMsg && mail.length > 0) {
					that.controls.ErrorView._addMail(html, mail, id, buttons);
				}
				$c.html(html.join(''));
				...
				if (isInfoMsg) {
					that.controls.ErrorView.autoClose(id);
				}
			} else {
				$c.html('');
			}
