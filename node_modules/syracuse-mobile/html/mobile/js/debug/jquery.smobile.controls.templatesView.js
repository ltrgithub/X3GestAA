...
(function($) {
	...
	that.controls.TemplatesView = {
		...
		handler: function(c, after) {
			var controller = this;
			var html = [];
			controller._tpl = that.controls.TemplatesView._getStoredTemplate();
			$.sdata.localStorage.removeItem("s-template-transfer");
			that.controls.TemplatesView._buildContainers(c, html, controller._tpl);
			var actions = {};
			after({
				html: html.join(''),
				bind: " ",
		...
		_buildContainers: function(c, html, tpl) {
			var locale = $helpers.locale();			
		...
		_refreshTemplates: function(id, controller, cb) {
			var html = [];
			var keys = that.controls.TemplatesView._getKeys(controller);
		...
		_createNewTemplate: function($c, c, controller) {
			var locale = $helpers.locale();
			if (!controller || !controller._tpl) return;
			var $label = $("#" + c.id + "_input");
			var template = {
				$uuid: $helpers.uuid(),
				label: $label[0].value,
				data: controller._tpl.data
			};
			var keys = that.controls.TemplatesView._getKeys(controller);
		...
		_selectTemplate: function($c, c, controller, uuid, lineid) {
			var locale = $helpers.locale();
			that.controls.TemplatesView._getMatchingTemplate(uuid, controller, function(template) {
				if (template == null) {
					...
					$.sdata.localStorage.setItem("s-template-transfer", JSON.stringify(template));
					$.smobile.doBack();
				}
			});
		},
		...
		_getMatchingTemplate: function(uuid, controller, cb) {
			var keys = that.controls.TemplatesView._getKeys(controller);
		...
		_getStoredTemplate: function() {
			...
			try {
				...
				console.log("Error parsing template data");
			}
		},
