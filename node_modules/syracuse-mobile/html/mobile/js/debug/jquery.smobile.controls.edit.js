...
(function($) {
	...
	that.controls.Edit = {
		...
		_getvalue: function(options, controller) {
			var cv = null;
			var nullDateReg = /(0{4})-(0{2})-(0{2})([T\s]?)(0{2})?(:)?(0{2})?(:)?(0{2})?(\.(0{3}))?(Z)?(([+-])(0{2}))?(.*$)/;
			if (options.type === "date") {
				cv = controller.getValue(options.bind, null, null);

				if (cv != null) {
					if (nullDateReg.exec(cv)) {
						cv = null;
					} else {
						try {
					...
					if (options.$isMandatory && controller.options.inCreate) {
						cv = new Date();
					}
				}
		...
		createEdit: function(controller, c) {
			...
			if (!$article.$isTitleHidden) {
				...
				var labelAfter = $proto.auth.mobileExtentionsLbl(cssl, $article);
				if (!labelAfter) {
					html.push('<label class="' + cssl.join(' ') + '">');
					html.push($helpers.htmlEncode(options.label) + (options.$isMandatory ? "*" : ""));
					html.push('</label>');
				}
				if (false) html.push('<label class="ui-input-text" for="' + c.id + '_i">' + $helpers.htmlEncode(options.label) + (options.$isMandatory ? "*" : "") + '</label>');

			}

			var css = ["s-m-editfld s-m-style s-m-value"];
			var readonly = options.$isReadOnly || (valproto && valproto.$isReadOnly) || options.$isEditMode === false || (valproto && valproto.$isEditMode === false);
			var disabled = options.$isDisabled || (valproto && valproto.$isDisabled);
			if (disabled) {
			...
			html.push('<div class="'); // class="s-m-editfld ');
			html.push(css.join('')); // class="s-m-editfld ');
			html.push('" ><input ');
		...
		_checkDateTime: function(c, controller) {
			...
			if (c.options.type === "date") {
				...
				$input.val(s);
				if (ovalue !== value) {
					// issue #3377 The milliseconds are problematic for X3 
					var d = /(\d{4})-(\d{2})-(\d{2})([T\s]?)(\d{2})?(:)?(\d{2})?(:)?(\d{2})?(\.(\d{3}))?(Z)?(([+-])(\d{2}))?(.*$)/.exec(value);
					if (d && d[10]) value = value.replace(d[10], "");
					controller.checkChange(c.id, value, _aftercchange, null);
				}
			}
