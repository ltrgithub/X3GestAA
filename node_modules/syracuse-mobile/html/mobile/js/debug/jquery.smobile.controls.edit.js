...
(function($) {
	...
	that.controls.Edit = {
		...
		createEdit: function(controller, c) {
			var options = c.data;
			var errors = controller.getDiagnoses(options.bind);
			var html = ['<div data-role="fieldcontain"'];
			if (errors)
				html.push(' class="s-m-flderr"');
			html.push(' id="' + c.id + '">');
			...
			*/
			if (itype === "string" && options.$maxLength) html.push(' maxlength="' + options.$maxLength + '"');
			html.push('/>');
		...
		checkValue: function($c, c) {
			...
			var e = $c.find('#' + that.ui.getErrId(c.id));
		...
		_updNumber: function(controller, $input, c) {
			...
			if (c.options.type === "range") {
				if (nnvalue < c.options.$minimum) throw new Error($helpers.format($helpers.locale().ui.range_overflow_number, [c.options.$minimum, c.options.$maximum]));
				//nnvalue = c.options.$minimum;
				else if (nnvalue > c.options.$maximum) throw new Error($helpers.format($helpers.locale().ui.range_overflow_number, [c.options.$minimum, c.options.$maximum]));
				//nnvalue = c.options.$maximum;
			}
			var expectedDecimals = c.options.decimals;

			if (c.options.decimals == null && c.options.bindDecimals) expectedDecimals = controller.getValue(c.options.bindDecimals, null, 0);
			if (expectedDecimals !== null) {
				var gotDecimals;
				gotDecimals = decimalPlaces(nnvalue);
				if (gotDecimals > expectedDecimals) throw new Error($helpers.format($helpers.locale().ui.number_precision_error, [expectedDecimals]));
				else nnvalue = parseFloat(nnvalue.toFixed(expectedDecimals));
			}
		...
		events: function($c, c) {
			...
			$input.blur(function(evt) {
				var _aftercchange = controller.afterChangeHnd(c);
				// Parent div of input and error msg
				var parent = $(evt.target).closest(".s-m-flderr"); //(".s-m-editfld");

				//if (parent && parent.length)
				var e = parent.find('#' + that.ui.getErrId(c.id));
				try {
					if (c.bind) {
						if (c.options.$isMandatory && trim($input.val()) === "") throw new Error($helpers.format($helpers.locale().ui.input_required));
						if (c.options.$pattern && !c.options.$pattern.exec($input.val())) throw new Error($helpers.format($helpers.locale().ui.incorrect_format));
					...
					//$(evt.target).addClass("ui-focus");
					//$(evt.target).focus();
					controller.pushError(error);
					if (!e.length) {
						// Error view is created by addErrors in and should be present
						//console.log("Error elemt expected[" + that.ui.getErrId(c.id) + "]\n" + error.message);
						//modified by Darina : create error field on the fly if needed 

						//$(evt.target).after($('<div  data-role="none" class="s-m-state-block s-m-error" id="' + that.ui.getErrId(this.id) + '">'+
						//error.message+'</div>'));
						e = $('<div  data-role="none" class="s-m-state-block s-m-error" id="' + that.ui.getErrId(c.id) + '">' +
							error.message + '</div>').insertAfter($(evt.target).parent());
					}
					//$(evt.target).parent()
					parent.addClass("s-m-flderr");
					//parent.find('#' + that.ui.getErrId(c.id));

					e.html(error.message);
					// Check an error - Styles for error and no error are managed by css selector
					//parent.addClass("s-m-flderr");
					return;
				}
				//if (e.length) e.empty();
				// Uncheck error
				//$(evt.target).parent().removeClass("s-m-input-error");
				parent.removeClass(".s-m-flderr");
				e.remove();
