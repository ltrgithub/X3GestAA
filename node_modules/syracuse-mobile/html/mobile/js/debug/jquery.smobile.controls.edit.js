	...
	that.controls.Edit = {
		...
		_checkDateTime: function(c, controller) {
			...
			if (c.options.type == "date") {
				value = $input.val();
				if (!value) return controller.checkChange(c.id, value, _aftercchange);
				try {
				...
				if (ovalue != nnvalue) controller.checkChange(c.id, nnvalue, _aftercchange);
				...
				value = value.toISO();
				if (value != ovalue) controller.checkChange(c.id, value, _aftercchange);
				...
				$input.val(s);
				if (ovalue != value) controller.checkChange(c.id, value, _aftercchange);
			}
