...
(function($) {
	...
	Controller.prototype = {
		...
		actionmethod: function(options, params) {
			...
			params = params && params.id && controller.alinks ? $.extend(true, {}, controller.alinks[params.id]) : null;
			if (!params || !$helpers.online() || !params.url) {
				return _after(false);
			}
			var sDataUrl = $helpers.decodeURIComponent(this.data.$info.original_url); //$sdata.parseSdataUri();
			var workingCopyUrl;
			if (controller.data.$info.isCreation) {
			...
			var url = $proto.parseExpression(params.url, {
				data: controller.exprData(actParams.$parameters),
				$prototype: $proto.getProto(page, params.bind)
			}, "$url");
			...
			if (params.$method == "POST") {
				$sdata.authenticate(function(ad, error, offline) {
					...
					var formData = controller.getParentEditData();
					var edit = formData != null;
					if (edit) {
