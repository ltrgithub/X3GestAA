		...
		actionmethod: function(options, params) {
			...
			var _after = function(data) {
				if (data) {
					...
					// Like thpage is rebuilt, meta data are processeed like enamble/disabme fields or actions
			...
			if (!$helpers.online() || !self.editActivity() || !params || !params.url) {
				$helpers.pushError('Bad params or offlinemode');
				return _after();
			}
			if (!self.checkData(true)) {
				if (options.success) {
			...
			if (!self.data.$info.original_url) {
				throw new Error("Can't launch action\nUnexpected empty original_url");
			}
			...
			if (self.data.$info.isCreation) {
				...
				// sDataUrl 	http://pc101329:8124/sdata/x3/erp/SUPERV/AQMDEVICE('1')?representation=AQMCRUDM.%24edit
				...
				$sdata.authenticate(function(ad, error, offline) {
					...
					$sdata.post(url, postData, function(data) {
						...
						if (false && data) {
							...
							// Set $properties - Maybe we can use extend (controller.data , data)
							if (data.$properties) {
								var curProps = formData.$properties;
								if (!curProps) {
									curProps = {};
								}
								var np = $.extend(true, curProps, data.$properties);
								self.setData("$properties", np);
							}
							// Set $actions - Maybe we can use extend (contreller.data , data)
							if (data.$actions) {
								var curActs = formData.$actions;
								if (!curActs) {
									curActs = {};
								}
								self.setData("$actions", $.extend(curActs, data.$actions));
							}
							self.notifyChange();
						}
					}, null, {
