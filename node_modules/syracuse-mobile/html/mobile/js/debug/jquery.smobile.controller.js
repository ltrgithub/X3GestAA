...
(function($) {
	...
	/**
	 * #4670 Scans the prototype and extracts image fields paths
	 * We just take into account level 1
	 * No needs to manage images in level > 2 for mobile version
	 */
	var _scanImgFieldsPath = function(list, protoObj, propname, path) {
		if (protoObj && protoObj.$properties) {
			if (path == null) path = [];
			if (propname) path.push(propname);
			Object.keys(protoObj.$properties).forEach(function(property) {
				var pv = protoObj.$properties[property];
				if (pv.$type === "image") {
					list.push(path.length > 0 ? path.concat([property]) : [property]);
				}
			});
	...
	/**
	 * #4670 Returns the list of image fields paths
	 */
	var _getImgFieldsPath = function(controller) {
		if (controller._imageFields == null) {
			controller._imageFields = _scanImgFieldsPath([], $proto.getProto(controller.getPageData()));
		}
		return controller._imageFields;
	};
	/**
	 * @constructor
	 */
	...
	Controller.prototype = {
		...
		init: function() {
			...
			self.config.originUrl = $.extend(true, {}, self.options.url);
		...
		exprData: function(propData) {
			...
			// #3789 - There is a detData that contains the data for array rows
			// We add detData in the stack if exists
			if (self.detData && self.detPrefix && self.detData[self.detPrefix]) {
				res.push(self.detData[self.detPrefix]);
			}
			while (true) {
				res.push(cc.data);
		...
		checkData: function(removeDiags) {
			var self = this;
			var $firstErr;
			self.clearErrors();
			if (removeDiags === true) {
				var page = self.getPageData();
				var props = self.getValue();
			...
			Object.keys(self.controls).forEach(function(name) {
				...
				var $c = $('#' + control.id);
				if (f && $c) {
					if (removeDiags === true && props && control.bind) {
						var bind = control.bind;
						var proto = $proto.getProto(page, bind);
						if (proto && proto.$rvalue) {
							// x-reference fields
							bind = proto.$rvalue;
						}
						if (props[bind] && props[bind].$diagnoses) {
							delete props[bind].$diagnoses;
						}
					}
		...
		getDiagnoses: function(bind, remove) {
			...
			if (remove === true) delete cd.$diagnoses;
		...
		setData: function(bind, value) {
			if (bind) {
				...
				if (value && value.$type && value.$type == "image") {
					cd[values[0]] = value.$value;
					cd.$properties = cd.$properties || {};
					if (!cd.$properties[values[0]])
						cd.$properties[values[0]] = {};
					cd.$properties[values[0]].$contentType = value.$contentType;
					cd.$properties[values[0]].$fileName = value.$fileName;
					cd.$properties[values[0]].$type = value.$type;
				} else {
			...
			if (self.editActivity()) {
				...
				$sdata.getSyncData(function(res) {
					...
					if (res) {
						...
						$helpers.confirmDlog.call(opts, self);
					} else {
						// Delete the draft associated to edit mode
						$sdata.deleteDraft(self.draftId(), function() {
							options.success();
		...
		/**
		 * #4670 work arround
		 * Set the value of image field in local DB (sdata objectstore) after a save error
		 * X3 doesn't return the image value inentity data save faild (bad filed value) so the image is not displayed
		 * This method set image field's value with the right data and update the sdata object in local DB (to be able to display the image in draft mode)
		 *	editData: 	controller data
		 *				contains the original data (url) or the new data (base64) if a file has been selected
		 *	sDataUrl	key for sdata object in local DB
		 */
		_setImgValAfterSaveErr: function(editData, sDataUrl, cb) {
			var self = this;
			// Get image field list
			var imsFields = _getImgFieldsPath(self);
			if (imsFields.length === 0) return cb();
			// If there is at least one image field, update image data in local DB
			// -1- read data in DB
			$.sdata.getSyncData(function(dbData, error) {
				if (!dbData) return cb();
				dbData = dbData[self.data.$representation];
				if (dbData == null || dbData.length === 0) return cb();
				dbData = dbData[0];
				var doUpdate = false;
				// -2- Scans image fields
				imsFields.forEach(function(path) {
					// We can have multiple image fields in the record at different level
					// I just tested the first level because we beed a representation woth multiple image fields (LAM)
					var parentEditData = editData;
					var parentDbData = dbData;
					for (var i = 0; i < path.length - 1; i++) {
						if (parentEditData && parentDbData) {
							parentEditData = parentEditData[path[i]];
							parentDbData = parentDbData[path[i]];
						}
					}
					var fldName = path[path.length - 1];
					if (parentEditData && parentDbData && parentEditData[fldName]) {
						// -3- Set the data
						parentDbData[fldName] = parentEditData[fldName];
						doUpdate = true;
					}
				});
				if (doUpdate) {
					// -4- Update the workingcopy in local DB
					// So we are able to display the image in draft mode
					$.sdata.updData(sDataUrl, {
						data: dbData,
						status: dbData.$info.status
					}, function() {
						cb();
					});
				} else {
		...
		actionsave: function(options) {
			...
			if (self.editActivity()) {
				...
				var after = function(cbOpts, errors, sDataUrl) {
					if ($helpers.hasErrors() && !$helpers.displayErrorAsDialog()) {
					...
					if (sDataUrl && $helpers.hasMessages() && $helpers.displayErrorAsMessage()) {
						//#4670 update local workingcopy with image fields data
						self._setImgValAfterSaveErr(editData, sDataUrl, function() {
						...
						if (!$helpers.online()) {
							...
							// #3630 Remove query page if any to force refresh and display new/modified record
							self.removeCachePages(function(page) {
								var url = decodeURIComponent(decodeURIComponent($(page).attr("data-url")));
								if (url.indexOf(self.data.$representation + ".$query") > 0) {
									// #3630 Remove query page to force refresh
									return true;
								}
								return false;
							});
						...
						// We pass forceWindowHistory to true to force browser history to go to detail - Issue 3298
						// If not we have to click 2 times back to retreive initial state List -> Detail -> Edit -> Save -> Detail -Back-> List
						// It's not possible to set changeHash=false in edit link to not change page - no data-changeHash for links
		...
		doRefresh: function(clearErrors, cbAfter) {
			var self = this;
			var $firstErr;
			if (self.controls) {
				Object.keys(self.controls).forEach(function(cn) {
					...
					if (c.uiClass && c.id) {
						...
						var $c = $('#' + c.id);
						if (f && $c) {
							var $err = $.smobile.ui.updateError(self, $c, c.id, f(self, c.options, $c, c.id, cbAfter));
							if (!$firstErr) {
								$firstErr = $err;
							}
						}
		...
		actionsyncdrafts: function(options) {
			...
			$sdata.getSyncCount(function(count) {
				...
				$.smobile.changePage("#" + $smobile.PAGES.send.id + $smobile.hash_sep + $helpers.encodeURIComponent(controller.applicationId), {
					changeHash: true
				});
			});
		},
		...
		_findControl: function(bind) {
			var self = this;
			if (!self.controls) return null;
			var c;
		...
		/** #7512 Add specific process after action
		 *   "$doAfterAction": {
		 *		params.name: "back" or "refresh"
		 *	}
		 */
		_authGetDoAfterAction: function(params) {
			if (!params || !params.name) return;
			var $article = this.getPageData();
			$article = $article ? $article.$article : null;
			if (!$article) return null;
			var $doAfterAction = $article.$doAfterAction ? $article.$doAfterAction[params.name] : null;
			if (!$doAfterAction) {
				//$refreshPageActions has been deprecated and replaced by $doAfterAction[params.name]: "refresh"
				$doAfterAction = $article.$refreshPageActions && $article.$refreshPageActions.indexOf(params.name) >= 0 ? "refresh" : null;
			};
			return $doAfterAction;
		},
		// Call an object's method
		actionmethod: function(options, params) {
			var self = this;
			var controller = this;
			params = params && params.id && self.alinks ? $.extend(true, {}, self.alinks[params.id]) : null;
			self._clearErrors();
			$('#' + self.prefix + 'rpanel').panel("close");
			var _after = function(data, msg) {
				self.disabled = false;
				if (msg) {
					$.helpers.pushInfoMessage(msg);
				}
				//#7512
				var $doAfterAction = controller._authGetDoAfterAction(params);
				if ($doAfterAction && !$helpers.hasErrors() && data != null) {
					// Only if no error
					if ($doAfterAction === "back") {
				...
				if (data) {
					// We store data in localstorage and call the same URL with localstorage param
					// --> In that case data url will not be sent but data will be read in localstorage
					// --> This allows to rebuild the page like if data have been received from server
					// --> Like the page is rebuilt, meta data are processeed like enable/disable fields or actions
					$.sdata.localStorage.setItem("s-methodcall-transfer", JSON.stringify(data));
					var url = '#' + $smobile.PAGES.page.id + $smobile.hash_sep + $helpers.encodeURIComponent(self.applicationId) + $smobile.hash_sep + $helpers.uuid();
					//url += $helpers.encodeJsmQuery('?url=' + $helpers.encodeURIComponent(self.data.$info.original_url + "&localstorage=s-methodcall-transfer"));
					var dataUrl = self.options.url.origUri;
					dataUrl = dataUrl.replace(/\((.*?)\)/g, "");
					//dataUrl = self.data.$info.original_url;
					if (dataUrl.indexOf("s-methodcall-transfer") === -1) {
						dataUrl += "&localstorage=s-methodcall-transfer";
					}
					url += $helpers.encodeJsmQuery('?url=' + $helpers.encodeURIComponent(dataUrl));
					// #3700 - We force changeHash to false to replace current page by updated page
					// --> If we open detail of a row the back button has been hacked to go back to last updated page
					// --> Like we have changeHash=false page is not store in history and without hack we go back to orginal page (empty one)
					// If we put true in changeHash back works fine but all updated pages are stored in hiwtory :-(
					//...
					$.smobile.changePage(url, {
						changeHash: true
					});
					...
					self.doRefresh();
					if (options.success) {
						options.success(null, $helpers.hasErrors(), true);
					}
				}
			};
			if (!self.editActivity()) {
				return _after(null, 'Method call failed - Edit facet only');
			}
			if (!params || !params.url) {
				return _after(null, 'Method call failed - Bad paramameters');
			}
			if (!$helpers.online()) {
				return _after(null, 'Method call failed - Online mode only');
			}
			if (!self.checkData(true)) {
				if (options.success) {
					options.success(null, false, true);
			...
			if (!self.data.$info.original_url) {
				return _after(null, "Can't launch action\nUnexpected empty original_url");
			}
			var sDataUrl = $helpers.decodeURIComponent(self.data.$info.original_url);
			var workingCopyUrl;
			//if (self.data.$info.isCreation) {
			// sDataUrl 	http://pc101329:8124/sdata/x3/erp/SUPERV/AQMDEVICE/$template?representation=AQMCRUDM.%24edit
			// workingcopie http://pc101329:8124/sdata/x3/erp/SUPERV/AQMDEVICE/$template/$workingCopies?representation=XSYRGBINTM.$edit&trackingId=98bd59fb-f293-4b1a-8726-c7d0302c31af
			// workingcopie http://pc101329:8124/sdata/x3/erp/SUPERV/AQMDEVICE('1')/$workingCopies?representation=AQMCRUDM.$edit&trackingId=d4d18cc5-f26c-44c3-8fed-05ef9dce240e 	
			workingCopyUrl = sDataUrl.replace("?representation", "/$workingCopies?representation");
			//	}
			var r = /(.*)\/sdata\//; // new RegExp("(.*)/sdata/");
			var postData = {
				$data: {},
				$actionParams: {},
				$workingCopyUrl: workingCopyUrl.replace(r, "/sdata/")
			};
			//#5814 - use _getServiceUrl
			var methUrl = self._getServiceUrl(params);
			var url = methUrl.url;
			if (url && params.$method == "POST") {
				postData.$actionParams[params.name] = {
					$isRequested: true,
					$parameters: methUrl.paramData
				};
				$sdata.authenticate(function(ad, error, offline) {
					if (error || offline) {
						_after(null, "Authentication error");
					}
					var formData = self.getParentEditData();
					var page = self.getPageData();
					var proto = $proto.getProto(page);
					var properties = proto.$properties;
					for (var p in formData) {
						if (p == "$etag" || p == "$uuid" || properties[p] != null) {
							postData.$data[p] = formData[p];
						}
					}
					$.mobile.loading('show', {
						theme: "a",
					...
					$sdata.post(url, postData, function(data) {
						$.mobile.loading('hide');
						if (data) {
							data.$info = formData.$info;
							_after(data);
						} else {
							_after(null, "Empty result");
						}
					}, null, {
		...
		addAlink: function(link) {
			var self = this;
			var cdid = $helpers.uuid();
			self.alinks = self.alinks || {};
			self.alinks[cdid] = {
				url: link.$url,
				$method: link.$method,
		...
		actionrepresentation: function(options, params) {
			var controller = this;
		...
		_actionservice: function(options, params, post) {
			...
			var _after = function(error) {
				...
				if (options.success) {
					...
					if (!params) return;
					//#7512
					if (controller._authGetDoAfterAction(params) !== "refresh") return;
					var sDataUrl = controller.config && controller.config.url ? controller.config.url.origUri : null;
					if (!sDataUrl) return;
					var url = $.smobile.ui.urlHref($sdata.detailUrl(sDataUrl), controller);
					// 500 - Just to let time to user to read the message
					setTimeout(function() {
						$.smobile.changePage(url, {
							changeHash: false
						});
			...
			if (!params.url)
				return _after();
			//#5814 - use _getServiceUrl
			var svcUrl = this._getServiceUrl(params);
			var url = svcUrl.url;
			...
			if (url && post) {
				$sdata.authenticate(function(ad, error, offline) {
					...
					$sdata.post(url, postData, function(data) {
						$.mobile.loading('hide');
						// FDB - Display all Errors/Warning/info as a top message
		...
		_getServiceUrl: function(params) {
			...
			return {
				url: $proto.parseExpression(params.url, {
		...
		_sendLocalData: function(after, sDataUrl, noCheckErrs, isBatchAction) {
			var self = this;
		...
		removeCachePages: function(cb) {
			...
			$.smobile.removeAllPages(exclude, cb);
		},
		...
		actiontemplates: function(options) {
			var self = this;
			var page = self.getPageData();
			var formData = self.getParentEditData();
			var $prototype = $proto.getProto(page);
			var $properties = $prototype.$properties;
			var templateData = {
				page: self.page,
				appid: self.applicationId,
				label: $proto.parseExpression($prototype.$title || $prototype.$description, {
					data: self.exprData(),
					$prototype: $prototype
				}, "$title"),
				data: {}
			...
			for (var p in formData) {
				if (p && p.indexOf("$") < 0) {
					templateData.data[p] = formData[p];
				}
			}
			$.sdata.localStorage.setItem("s-template-transfer", JSON.stringify(templateData));
			$.smobile.changePage("#templates", {
				changeHash: true
			});
			if (options.success) {
				options.success(null, false, true);
		...
		// Call onWindowResize method for controls
		_onWindowResize: function() {
			var self = this;
			window.setTimeout(function() {
				for (var p in self.controls) {
					var control = self.controls[p];
					var f = $smobile.controls[control.uiClass].onWindowResize;
					if (!f) continue; // control don't care about onWindowResize
					var $c = $('#' + control.id);
					if (!$c) continue;
					f(self, $c, control);
				}
		...
		orientationChanged: function(orientation) {
			var self = this;
			if ($.helpers.isTablet()) {
				var panel = $('#' + self.prefix + 'rpanel');
				if (!panel) return;
				if (orientation == "landscape") {
					// Force push mode - ajdust content to width
		...
		// Only right panel manages push data-display
		onRPanelBeforeClose: function(evt, ui) {
			var self = this;
			if ($.helpers.isTablet()) {
				var t = $(evt.target);
				if (t && $(t).attr("data-display") == "push") {
					// Force overlay mode if push mode - push enabled only on orientationchanged to landscape
					t.attr("data-display", "overlay");
					t.panel({
						display: "overlay"
					});
					// When we close from push mode we resize controls because width changed -> table can change its mode reflow or not
					self._onWindowResize();
				}
			}
		},
		applyTemplate: function(template) {
			...
			for (var p in data) {
				...
				/*
				 *  Never set internal properties ($uuid, ...)
				 */
				if (p && p.indexOf("$") < 0) {
					$prop = $properties[p];

					if ($prop) {
						self.setData(p, data[p]);
					}
				}
			}
			self.notifyChange();
		...
		getApplication: function() {
			return $.sdata.getApplication(this.applicationId);
		}
	};
	/* 
	 * **************************
	...
	$.extend(Controller.prototype, {
		...
		multiselActStop: function(error) {
			...
			var locale = $helpers.locale();
			self.getHeaderTab().find(".s-m-header-cell-ttl").show().html(nbok + " " + locale.success + " - " + nbko + " " + locale.error);
		},
	...
	$.extend(Controller.prototype, {
		...
		setButtons: function(btns) {
			if (!btns) return;
			var self = this;
			var foot = self._getFooter(true);
			foot.html(self.footerHtmlBtns(btns));
			var exprData = self.exprData();
			var pageData = self.getPageData();
			btns.forEach(function(btn) {
				var $$b = $('#' + btn.linkId);
				if (btn.action === "nolink" && btn.url) {
					//#5814 - add this new type of button that open a page
					$$b.attr("href", $.smobile.ui.urlHref(self._getServiceUrl(btn).url, self));
				}
				$$b.button();
			});
		},
		...
		footerHtmlBtns: function(buttons) {
			...
			var btnHtml = [];
			// Use JQM Grid layout - http://api.jquerymobile.com/grid-layout/
			var a = 'a'.charCodeAt(0);
			// Max 5 buttons - See JQuery Mobile grid 
			var gridClass = buttons.length > 1 ? "ui-grid-" + String.fromCharCode(a + (buttons.length - 2)) : null;
			var blockClass = gridClass ? "ui-block-" : null;
			btnHtml.push('<div class="s-m-footer-grid-btn ');
			if (gridClass) {
				btnHtml.push(gridClass);
			}
			btnHtml.push('">');
			buttons.forEach(function(btn, idx) {
				if (idx < 5) {
					...
					if (blockClass) {
						btnHtml.push(blockClass + String.fromCharCode(a + idx));
					}
					if (btn.$isHidden) {
						btnHtml.push(' s-m-hidden');
					}
					btnHtml.push('">');
					$.extend(btn.jmdata, {
						"data-iconpos": "left",
					...
					console.log("5 buttons max allowed in footer [" + btn.title + "]");
				}
			});
			return btnHtml.join('');
		}
