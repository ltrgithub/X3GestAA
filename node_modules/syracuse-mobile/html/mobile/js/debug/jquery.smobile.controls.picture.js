...
(function($) {
	...
	that.controls.Picture = {
			...
			if (value == null) {
				...
				if (value.$value) {
					...
					if (url.indexOf("$workingCopies") > -1) {
						url = that.controls.Picture._fixUrl(controller, $c, c, value, cp, page);
					}
					$image[0].src = url;
					_showImg();
				} else _doNodata();
		...
		createPicture: function(controller, c, article, proto) {
			var options = c.data;
			var html = [];
			var value = controller.data[options.bind];

			if (value == null) value = "";
			var exprData = controller.data ? $.proto.exprData(controller.data) : null;
			if (proto) {
				//var url = proto.$url ;
				var url = $.proto.parseExpression(proto.$url, {
					data: exprData,
					$prototype: proto
				}, "$url");
			}
			...
			html.push(curl + '?etag=' + controller.data.$etag);
			html.push('"/></center>');
			html.push('</div>');
		...
		// because of this, url is repaced by the url of tje $details facet
		_fixUrl: function(controller, $c, c, value, cp, page) {
			try {
				var property = cp.$url.split("/")[1];
				var rep = cp.$protoparent.$representation + ".$details";
