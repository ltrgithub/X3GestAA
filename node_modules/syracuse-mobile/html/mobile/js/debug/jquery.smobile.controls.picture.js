...
(function($) {
	...
	that.controls.Picture = {
		...
		createPicture: function(controller, c) {
			...
			if (options.label) html.push($helpers.htmlEncode(options.label));
			...
			html.push('"/></center>');
			html.push('</div>');
			html.push('<a id="' + c.id + '_file" class="s-m-min40 ui-btn-right" data-theme="f" data-role="button" data-mini="true" data-icon="plus" data-iconpos="left" data-inline="false">' + $helpers.locale().ui.picture_file + '</a>');
			html.push('</div>');
			return {
				html: html.join(''),
				bind: options.bind,
		...
		checkValue: function($c, c) {
			...
			if (c.options.$isMandatory) {
				...
				if (!value) {
					var error = {
						field: $c,
						message: $helpers.format($helpers.locale().ui.mandatory_field, [c.options.label])
					};
