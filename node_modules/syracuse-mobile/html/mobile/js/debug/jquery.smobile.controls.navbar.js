		...
		syncHandler: function(c, left) {
			...
			if (c.options.links) {
				c.options.links.forEach(function(linkid) {
					//#5814 - Check if it's actually necessary, because data.url seems to be the same as the one sent by actionlink
					...
					switch (item.code) {
						...
						case "actions":
							if (!linksloaded) {
								...
								// #5814 pass controller.applicationId
								links = $proto.extractLinks(proto, proto, controller.getCurrentData(), edit, page, controller.applicationId);
