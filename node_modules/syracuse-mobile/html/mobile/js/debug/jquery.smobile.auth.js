...
(function($) {
	...
	function _defaultListArticle(proto, imageProp, info) {
		...
		if (!proto.$title && proto.$value) {
			$title = '$value';
		} else if (proto.$title && (proto.$title === proto.$description)) $description = null;
	...
	function _execStackLayout($layout, html, options, controller, cp, cd, exprData, info) {
		...
		$layout.$items.forEach(function(item, index) {
			if (!addContainer) info.defContainer = "p";
				...
				var css = ["s-m-l-stack"];
				if ((index === 0) && (index < len)) css.push("first");
				if ((index > 0) && (index === len)) css.push("last");
	...
	var _addCssPrefix = function(ev, html, item, cd, cp, cover, addTitle) {
		if (item.$cssPrefix && (typeof ev != "object")) ev = {
			value: ev
		};
		if (item.$cssPrefix && (typeof ev === "object")) {
			...
			if (!item.$isTitleHidden && ev.title) {
				...
				} else if (addTitle && cp && cp.$properties && cp.$properties[item.$bind]) {
	...
	function _articleArrayComposition2Layout($article, cp, filter, list, grid) {
		var fn = [];
		if (cp && cp.$properties) {
			...
			if ($article.$format != "grid") {
				if (grid) {
					...
					if ($article.$layout && $article.$layout.$items && $article.$layout.$items.length) {
						for (var i = 0, len = Math.min(maxlen, $article.$layout.$items.length); i < len; i++) {
							grid.$items.push({
								$bind: $article.$layout.$items[i].$bind
							});
						}
					} else {
						var ff = [];
	...
	function _execField(item, html, options, controller, cp, cd, exprData, info) {
		...
		if (!(item.$isTitleHidden === false)) item.$isTitleHidden = true;
		var title = "",
			cssl,
			pi;
		...
		if (!isExpression && cp && cp.$properties && cp.$properties[item.$bind]) {
			...
			if (cpi.$type === "application/x-reference") {
				doparse = false;
				var toptions = $.smobile.controls.types.addReference(cpi, [cd[item.$bind], cd], controller, item.$bind, true);
				ev = $helpers.htmlEncode(toptions.value);
				if (toptions.title) {
					cssl = ["s-m-style", "s-m-linkdesc"];
					if (item.$refDescription) $proto.auth.mobileExtentions(cssl, item.$refDescription);
					ev = ev + ' <span class="' + cssl.join(' ') + '"> ' + $helpers.htmlEncode(toptions.title) + '</span>';
				}
		...
		that.mobileCssPrefix(ev, html, item, cd, cp, cover, true);
	...
	function _genContainerId(options) {
		...
		if (options.prefix) return options.prefix + s;
		else return 'c_' + s;
		...
		if (p) {
			p.forEach(function(item) {
				...
				if (pi) {
					...
					var g = $proto.uiCanGroupField(pi);
					if (g) {
						content[cid] = {
							parent: parentId,
							"uiClass": "SingleField",
		...
		if (item.$items && item.$items.length) {
			if (item.$items[0].$bind) {
				...
				item.$items.forEach(function(ci) {
					return _layoutItem(ci, content, options, cp, exprData, info, parentId, item, false);
				});
			}
		}
	}
	...
	function _layoutItem(item, content, options, cp, exprData, info, parentId, parent, onlyChildren) {
		...
		if (item.$bind && item.$layout) {
			if (!$proto.acceptSubLayout(cp)) item.$layout = null;
		}
		...
		if (item.$category) {
			...
			if (item.$bind) {
				if (!item.$layout && !item.$cards) {
					...
					if (pp) {
						if (parent) _showfieldsInParent(item, content, options, cp, exprData, info, parentId);
					}
