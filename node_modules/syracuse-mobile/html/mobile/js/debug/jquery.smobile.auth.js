...
(function($){
    ...
    function _execField(item, html, options, controller, cp, cd, exprData, info){
        ...
        var exp = item.$bind, isExpression = false;
        if (item.$expression) {
            isExpression = true;
            exp = item.$expression;
        }
        ...
        if (!isExpression && cp && cp.$properties && cp.$properties[item.$bind]) {
            ...
            if (cpi.$type === "application/x-reference") {
                ...
                var rtitle = $helpers.htmlEncode(before ? toptions.value : toptions.title), rvalue = $helpers.htmlEncode(before ? toptions.title : toptions.value);
                ev = rvalue;
                if (cssl) {
