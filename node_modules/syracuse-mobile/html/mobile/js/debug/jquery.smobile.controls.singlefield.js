...
(function($) {
	...
	var _checkMetaData = function(controller, cp, options) {
		var meta = controller.getProtoDataValue(options.$bind);
		var res = {
			$isHidden: meta && meta.$isHidden
		};
		if (cp.$rtype === "application/x-reference") {
			/* Temporarily - To see with Marc -  Issue #2298 
			 * In $properties metadata a reference field is identified by the ID of the linked field
			 *  $properties ={CUR:$isHidden:true} means CUR_REF field is hidden
			 */
			for (var p in cp.$properties) {
				var meta = controller.getProtoDataValue(p);
	...
	that.controls.SingleField = {
		...
		initData: function(cp, ft, title, opts, $article) {
			...
			switch (ft) {
				...
				case "application/x-password":
					...
					if (ft === "application/x-password") {
						opts.type = "password";
						if (cp.$capabilities && (cp.$capabilities.indexOf("confirm") >= 0)) opts.$confirm = true;
					}
			...
			if (cp.$format) opts.$format = cp.$format;
			return res;
		...
		handler: function(c, after) {
			...
			if (edit) {
				...
				if (pp.$isHidden || pp.$isExcluded || _checkMetaData(controller, cp, options).$isHidden) return after(null);
