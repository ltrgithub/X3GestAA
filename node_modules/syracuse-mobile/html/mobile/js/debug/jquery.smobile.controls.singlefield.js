	...
	that.controls.SingleField = {
		...
		handler: function(c, after) {
			...
			if (edit) {
				...
				} else if (valproto && valproto.$isHidden) {
					html.push(" s-m-hidden");
				}
				html.push('">');
				...
				var _createparenDiv = function(isImage) {
					html.push('<div class="');
					html.push(css.join(' '));
				...
				if (cp.$rtype === "application/x-reference") {
					...
					if (!added) {
						_createparenDiv(updproto.$type === "image");
						...
						if ($article && $article.$expression) {
							var nvalue = $article.$expression.replace(/\{(.*?)\}/g, function(match, prop) {
