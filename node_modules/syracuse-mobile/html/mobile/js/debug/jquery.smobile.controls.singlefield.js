	...
	that.controls.SingleField = {
		...
		handler: function(c, after) {
			...
			if (edit) {
				...
				if (cp.$links) {
					Object.keys(cp.$links).forEach(function(link) {
						if (cp.$links[link].$url && cp.$links[link].$url.indexOf("http") === 0) {
							html.push('<div class="s-m-style s-m-value-link">');
							var text = $proto.parseExpression(cp.$links[link].$title, {
								data: controller.exprData(dd),
								$prototype: pp
							}, "$title", {
								html: true
							});
							var href = $proto.parseExpression(cp.$links[link].$url, {
								data: controller.exprData(dd),
								$prototype: pp
							}, "$url", {
								html: false
