...
(function($) {
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel) {
		...
		if (controller.data.isError) {
			type = "error"; //_error2Type(controller.data.diagnose.type) ;
			html.push('<div class="s-m-header-tab s-error-' + type + '">');
		} else
			html.push('<div class="s-m-header-tab ">');
		// No leftpanel for logout page
		...
		// -2- Add title / Message
		/*if (ui.css && ui.css.length)
			html.push('<div' ) ;
			ui.css.forEach(function(style,index){
			html.push('class="'+style+'"><h1 class="s-m-header-ttl" ');	
			});
			
			html.push('>' ) ;*/
		if (controller.data.isError) {
			//s-error-ttl",'s-error-' + type
			html.push('<div class="s-m-header-cell-ttl s-error-' + type + '"><h1 class="s-error-ttl" ');
		} else {
...
(function($) {
	...
	Controller.prototype = {
		...
		actionmultiswitch: function(options, params) {
			...
			if (this.multiSelOn()) {
				var nb = 0;
				ttl = $helpers.format($helpers.locale().ui.multinbsel, [nb == null || nb <= 0 ? 0 : nb]);
			} else {
				ttl = this.pgtitle;
			}
			$('#' + this.prefix + 'headerTtl').html(ttl);
