...
(function($) {
	...
	var _createHeaderRight = function(controller, uiData, nopanel) {
		...
		if ((uiData && uiData.roptions && uiData.roptions.nopanel) || nopanel) {
			...
			html.push('<div class="s-m-header-cell-icon" id="');
			...
			//html.push('<a href="#' + controller.prefix + 'rpanel" data-role="button" class="s-m-hbutton" data-theme="a"  data-inline="true" data-iconpos="notext" data-icon="s-rpanel" data-corners="false" data-iconshadow="false" data-shadow="false"> </a>');
			html.push('</div">');
		}
		//html.push('</div>');
		return html.join('');
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel, isLocalData) {
		...
		// #2489 - we use tab-cell for each icons
		html.push('<div class="s-m-header-tab "><div class="s-m-header-cell-icon">');
		if (!controller.pageInfo.page.isLogOut) {
		...
		// -A- Add title
		html.push('<div class="s-m-header-cell-ttl">');
		html.push('<h1 class="s-m-header-ttl">');
									...
									success: function(cbOpts, error, noback) {
										...
										if (!error && !noback) {
											...
											if (cbOpts && cbOpts._cburl) {
												window.location.hash = that.ui.urlHref(cbOpts._cburl, ctrl);
											} else {
												$.smobile.doBack(ctrl);
											}
										}
	...
	function _load_page(page, params, uiData, pageData, cb) {
		...
		if ((page.page.useAppId == 2) && (!$sdata.config || ($sdata.config.id != page.application))) {
			$sdata.getAppConfig(page.application, function(config, error) {
				if (!config) throw new Error("Application not found in local database.\nPlease try to reinstall it\nhostname " + window.location.host + "\nappid[" + page.application + "]\n");
				$sdata.config = config;
...
(function($) {
	...
	Controller.prototype = {
		...
		activeBtnsRestore: function() {
			var self = this;
			for (var p in this.activeButtons) {
		...
		getFilterFields: function(withDetails) {
			...
			if ((page.indexOf(".$query") > 0) || (page.indexOf(".$lookup") > 0) || (page.indexOf(".$select") > 0)) {
				...
				Object.keys(proto.$properties).forEach(function(name) {
					var prop = proto.$properties[name];
					if (prop.$isHidden || prop.$isExcluded) return;
					var tt = prop.$type;
					var fv = name;
		...
		actioncancel: function(options) {
			...
			if (self.editActivity()) {
				$sdata.deleteDraft(self.draftId(), function() {
					if (!controller.data.$localUrl) {
						options.success();
						return;
					}
					var url = $proto.parseExpression(controller.data.$url, {
						data: $proto.exprData(controller.data)
					}, "$url");
					var sdataUrl = $sdata.parseSdataUri(url);
					var localUrl = $sdata.url2local(sdataUrl);
					if (localUrl.indexOf("appid=") < 0) localUrl = localUrl + "&appid=" + controller.applicationId;
					console.log(localUrl);

					var ourl = controller.data.$info.original_url;
					var osdataUrl = $sdata.parseSdataUri(ourl);
					if (false && osdataUrl.template) {
						osdataUrl.template = false;
						osdataUrl.id = $sdata.data2id(osdataUrl, odata);
					}
					osdataUrl.createActivity = false;
					osdataUrl.editActivity = false;
					var olocalUrl = $sdata.url2local(osdataUrl);

					if (olocalUrl.indexOf("appid=") < 0) olocalUrl = olocalUrl + "&appid=" + controller.applicationId;
					console.log(olocalUrl);
					console.log(controller.data.$localUrl);
					localUrl = controller.data.$localUrl;
		...
		actionlogout: function(options) {
			...
			$sdata.post(url, {}, function(data) {
				if (data) {
					console.log(host + loc.pathname + "#logout");
					window.location = host + loc.pathname + "#logout";
				} else {
