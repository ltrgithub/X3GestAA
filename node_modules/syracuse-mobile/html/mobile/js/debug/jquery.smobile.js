...
(function($) {
	...
	var _hasErrorControl = function(controls) {
		for (var name in controls) {
			if (controls[name].uiClass === "ErrorView")
				return true;
		}
		return false;
	};
				...
				.forEach(
					function(id, index) {
						...
						if (c.layout) {
							...
							// Calls 'handler' method to initcontrol
							// (create html)
							$.smobile.addControl
								.apply(
									controller, [
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel) {
		...
		// -2- Add title / Message
		html
			.push('<div class="s-m-header-cell-ttl"><h1 class="s-m-header-ttl" ');
		html.push('id="');
		...
		if (cpage) {
			...
			if (ctrl) {
				...
				if (target) {
					...
					if (p) {
						if (p === "filter") {
							if (that.openFilterDlg) { // declared in
								// jquery.smobile.controls.filter.js
								that
									.openFilterDlg(ctrl,
										ctrl.extraData.filters);
								e.preventDefault();
							}
						}
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info,
		...
		var errfieldsprevchildren = $content.find(".s-m-error").prev()
			.children();
		...
		getCtxItems: function(facet, representation, data, items) {
			...
			if (page) {
				...
				if (query || (facet === "$lookup")) {
					...
					if (query) {
						/*
						 * Icons for multti selection displayed if multisel
						 * class in header div
						 */
						items.push({
							id: "multiinvalid",
				...
				if (query) {
					...
					if (page.$prototype.$links && page.$prototype.$links.$create && !page.$prototype.$links.$create.$isHidden) {
						url = $proto.parseExpression(
							page.$prototype.$links.$create.$url, {
								data: null,
	...
	function _afterEnhance(page, params, uiData, pageData, panel, rpanel, cb) {
		...
		if (!page.page.db) {
			...
			uiData = cd;
			/*
			 * var _afterData= _private.enhanceHeader.bind({ ui: uiData, panel:
			 * panel, rpanel: rpanel });
			 */
			var obj = {
				ui: uiData,
		...
		if ((page.page.useAppId === 2) && (!$sdata.config || ($sdata.config.id != page.application))) {
			$sdata.getAppConfig(page.application,
				function(config, error) {
					if (!config)
						throw new Error(
							"Application not available for hostname " + window.location.host + "\nappid[" + page.application + "]");
					$sdata.config = config;
	...
	that.removePage = function(page) {
		...
		if (controller) {
			$helpers
				.log("Remove page" + (controller ? (": " + (controller.pageInfo && controller.pageInfo.page && controller.pageInfo.page.id) + "-" + (controller.pageInfo && controller.pageInfo.pageId || "")) : ""));
			controller.remove();
		}
		var $t = $(page);
...
(function($) {
	...
	Controller.prototype = {
		...
		// restore uf any - o.restoreShow:retsore the class on page Show
					...
					.forEach(
						function(name) {
							...
							if (prop.$capabilities && (prop.$capabilities
								.indexOf('filter') >= 0)) {
								if (prop.$type === 'application/x-reference') {
									if (!prop.$item || !prop.$item.$value)
										return;
									if (!$proto.useUuid(prop.$item)) {
										fv = prop.$item.$value
											.substring(
												1,
												prop.$item.$value.length - 1);
										if (!proto.$properties[fv])
											return;
		...
		notifyChange: function() {
			...
			if (self.editActivity()) {
				if (self.parent) {
					...
					$sdata.updateDraft(self.draftId(), self.getValue());
			}
		},
		...
		multiSelClick: function(li) {
			...
			// Pop/Push line
			// we have one multiSel array by controler which is reseted when
			// list is refreshed by multiSelReset (case with multiple filters in
			// query)
		...
		_updateHeaderTtl: function() {
			...
			if (self.multiSelOn()) {
				...
				if (nbSel <= 1) {
					...
					$("#" + self.prefix + "headerTab").find(
						".s-m-header-cell-icon.multisel").each(
						function(i, e) {
		...
		actionsave: function(options) {
			...
			if (self.editActivity()) {
				// EDIT/CREATE Mode
				// Override standard _sendLocalData success callback to deal
				// with field error
				// sDataUrl is the sdataUrl (ID for local db) of current page
				// data returned by saveDraft - Used to reload page if fields
				// errors
				var after = function(cbOpts, errors, sDataUrl) {
					if ($helpers.hasErrors()) {
						...
						var url = $.smobile.ui.urlHref(
							$sdata.editUrl(sDataUrl), self);
						window.location.replace(url);
					} else {
						// Std process
						if (sDataUrl) { // set when a new record is created and
							// we are in online mode
							var url = $.smobile.ui.urlHref($sdata
								.editUrl(sDataUrl), self);
							window.location.replace(url);
						} else {
				...
				return $sdata
					.saveDraft(
						...
						function(result) {
							...
							// only this data (not sent pending/draft
							// data)
							// --> We send only current data to
							// catch/display fields error (see
							// hasFieldsErrors below)
		...
		// sDataUrl=null -> Send all pending draft data Else send one data
		// identified by sDataUrl
