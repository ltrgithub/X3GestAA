...
(function($) {
	...
	var _createHeaderLeft = function(controller, ui) {
		...
		if (controller.parent) {
			...
			nh.fpanel.data.action = "back";
			nh.fpanel.data.jmdata["data-icon"] = "arrow-l";
		} else {
			// Display left panel
			nh.fpanel.data.hash = controller.prefix + "panel";
			nh.fpanel.data.jmdata["data-icon"] = "s-bars";
		}
		for (var p in ui.header) {
			nh[p] = ui.header[p];
		}
		ui.header = nh;
		var content = [],
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel) {
		var html = [];
		html.push('<div style="display:table;table-layout:fixed;width:100%">');
		html.push('<div style="display:table-cell;width:34px;">');
		var htmLeft = _createHeaderLeft(controller, ui);
		console.log(htmLeft);
		html.push(htmLeft);
		html.push('</div>');
		html.push('<div style="display:table-cell">');
		html.push('<h1 style="width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" class="s-m-header">');
		html.push($helpers.htmlEncode(pgtitle));
		html.push('</h1>');
		html.push('</div>');
		html.push('<div style="display:table-cell;">');
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		var _afterSetTitle = function() {
			// Set page header Html with right icons
			...
			if (hasPanel) {
				...
				$panel.on("panelclose", function(event, ui) {
			...
			if (hasrPanel) {
				...
				$rpanel.on("panelclose", function(event, ui) {
					that.docEvents.enableSwipe();
				});
			}
