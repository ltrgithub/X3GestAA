		...
		Object.keys(controls).forEach(function(id, index) {
			...
			if (c.layout) {
				...
				if (l && l.event) {
					...
					controller.layouts[c.id] = {id: c.id, event: l.event};
					l.event = null;
				}
				html[id] = l;
...
(function($) {
	...
	Controller.prototype = {
		...
		checkChange: function(cid, value, next, cp) {
			...
			if (ctrl && ctrl.bind) {
				...
				if (isLookup) {
					...
					if (isuuid) {
						if (!value) {
							self.data[ctrl.bind] = null;
						} else {
							self.data[ctrl.bind] = {$value: value};
						}
		...
		remove: function() {
			...
			if (this.cleanupHnd) this.cleanupHnd(this);
