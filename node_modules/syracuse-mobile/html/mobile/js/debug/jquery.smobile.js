...
(function($) {
					...
					$.smobile.addControl.apply(controller, [parent, c, function(ctrl) {
						if (ctrl && Array.isArray(ctrl)) {
							ctrl.forEach(function(cc) {
								html[id + cc.scid] = cc;
								if (cc.bind || cc.action) controller.registerControl({
									id: c.id + cc.scid,
									bind: cc.bind,
									action: cc.action,
									options: cc.options,
							...
							if (ctrl) {
								html[id] = ctrl;
								if (ctrl.bind || ctrl.action) controller.registerControl({
									id: c.id,
									bind: ctrl.bind,
									action: ctrl.action,
	...
	var _createHeaderRight = function(controller, uiData, nopanel) {
		...
		if (uiData && uiData.hbuttons) {
			uiData.hbuttons.forEach(function(btn) {
				...
				if (btn.id === "filter") {
					...
					btn.controlId = controller.prefix + btn.id;
					...
					if (fav) {
						...
						if (controller.extraData == null || controller.extraData.bookmarks == null || controller.extraData.bookmarks.length == 0) {
							if (!btn.css) btn.css = [];
							btn.css.push("s-m-hidden");
						}
					}
				} else if (btn.id === "sortorder") {
					btn.controlId = controller.prefix + btn.id;
					controller.extraData.sortorderBtnId = btn.controlId;
					btn.$isHidden = true;
				}
		...
		if ((uiData && uiData.roptions && uiData.roptions.nopanel) || nopanel) {} else {
	...
	var _createHeaderLeft = function(controller, ui) {
		var hhtml = {
			root: {
				before: "",
				after: "",
		...
		var nh = {
			fpanel: {
				...
				data: {
					link: true,
					hash: controller.prefix + "panel",
					css: ["s-m-hbutton", "left"],
					jmdata: {
						"data-inline": "true",
						"data-iconpos": "notext",
						"data-icon": "s-bars",
						...
						if (hnd) {
							...
							return hnd.call(ctrl, {
								success: function(data, error, noback) {
									...
									if (!error && !noback) {
										if (data && data.cburl) {
											window.location.hash = that.ui.urlHref(data.cburl, ctrl);

										} else $.smobile.doBack(ctrl);
									}
								}
							},
							actp
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		$header.html(_createHeaderLeft(controller, ui));
		var $ttl = $header.find("h1");
		var _afterSetTitle = function() {
			if (pageInfo.page.useAppId || true) {
				$header.append(_createHeaderRight(controller, ui, !pageInfo.page.useAppId));
			}
			var content = [];
			...
			} else if (controller.pageOptions.detail) $.jqmData($page[0], "detail", 'detail');
			if (ui.actions) {
			...
			if ($template) $page.insertBefore($template);
			else $page.appendTo($("body"));
			$header.click(_clickOnHeader);
			$content.click(_clickOnContent);
			cb();
		};
		if (pgtitle && $ttl && $ttl.length) $ttl.html(pgtitle);
		_afterSetTitle();
	};
	...
	var _private = {
		...
		representation2ui: function(params, data) {
			...
			if (params.facet == "$dashboard") {
				...
				if (data && data.$prototype && data.$prototype.$title) cd.$title = $proto.parseExpression(data.$prototype.$title, {
					data: null,
				...
				if (!article) article = $proto.auth.defaultDetailArticle(data.$prototype, {
					list: false,
					facet: params.facet
	...
	function _afterEnhance(page, params, uiData, pageData, panel, rpanel, cb) {
		...
		if (!page.page.db) {
			...
			else return cb(params.representation, uiData, pageData, _afterData);
		} else {
			var uimap = null,
	...
	var _loadDataAndDesign = function(query, page, cb) {
		var pageData = null,
			uiData = {}, params = _parsePageURI(query, page.pageName);
...
(function($) {
	...
	Controller.prototype = {
				...
				Object.keys(proto.$properties).forEach(function(name) {
					...
					if (prop.$capabilities && (prop.$capabilities.indexOf('sort') >= 0)) {
						...
						if (prop.$type === "application/x-reference") {
							var cn = $proto.getReferenceField(cn, prop.$item, proto);
							if (!cn) return;
							if (cn != name) {
			...
			if (ctrl && ctrl.bind) {
				...
				if (isLookup) {
					...
					if (isuuid) {
						...
						//by reference
						var display = ctrl.options.reference;
						if (display) {
							dd[display] = value;
						}
					}
		...
		getDiagnoses: function(bind) {
			...
			cd = cd.$properties || cd.$;
			if (!cd) return null;
			// Issue #2298 - Property name must point to refrence field named CUR_REF in X3 instead of CUR (excluded field)
			cd = cd[bind] ? cd[bind] : cd[bind + "_REF"] ? cd[bind + "_REF"] : null;
			if (!cd) return null;
			cd = cd.$diagnoses;
			if (!cd) return null;
			var res = [];
		...
		actionsave: function(options) {
			...
			if (self.editActivity()) {
				...
				var success = function(data, error, noback) {
					if (sDataUrl && $helpers.hasFieldsErrors()) {
						...
						// Std process - If Error other than field error goto draft view (syncView)
						options.success(data, error, noback);
					}
				};
				...
				return $sdata.saveDraft(self.draftId(), self.getValue(), function(result) {
					...
					sDataUrl = result && result.sDataUrl ? result.sDataUrl : null;
					// Send sdata http request 
					self._sendLocalData(success, sDataUrl);
				});
			} else {
				options.success();
			}
		...
		_actionservice: function(options, params, post) {
			var controller = this;
			if (params && params.id && controller.alinks) params = controller.alinks[params.id];
			else params = null;
			var _after = function(error) {
				if (error) controller.doRefresh();
				if (options.success) options.success(null, error, true);
			};
			...
			if (post) {
				$sdata.authenticate(function(ad, error, offline) {
					...
					var postData = controller.serviceData(),
						edit = true;
					if (!postData) {
						postData = {};
						if (params.$parameters) {
		...
		installApplication: function(options, data) {
			...
			$.smobile.load(data, function(index, count, msg) {
				if ((index == 0) && count) $.mobile.loading('show', {
					theme: "a",
		...
		_sendLocalData: function(after, sDataUrl) {
			var self = this;
				...
				if (!error && data) {
					var opts = {
						...
						success: function(options) {
							// if editActivity=true hasErrors returns false
							if ($helpers.hasErrors()) {
								// Display Draft (synchro) page
							...
							// Call std callback
							if (after) after((options && options._cburl) ? {
								cburl: options._cburl
							} : null, $helpers.hasErrors());
						}
					};
		...
		actiondelete: function(options) {
			...
			if (cp && cp.$links && cp.$links.$delete && cp.$links.$delete.$confirm) {
				...
				if (conf) if (!window.confirm(conf)) {
					if (options.success) options.success(null, false, true);
					return;
				}
			}
			...
			if (self.editActivity()) {
				...
				if (self.config && self.config.url && self.config.url.origUri) return $sdata.deleteDataByUrl(self.config.url.origUri, self.getValue(), function() {
					if ($helpers.hasErrors()) {
						return options.success(null, true);
					}
					var after = function() {
