...
(function($) {
	...
	var _addMenus = function(controller, html, options) {
		options = options || {};
		html.push('<div class="s-m-header-cell-select');
		if (options.transparent) html.push(' transparent');
		if (options.rightAligned) html.push(' right');
		html.push('">');
		var id = controller.prefix + options.id;
		html.push('<select id ="' + id + '" ');
		options.native = false;
		if (!options.native) html.push('data-native-menu="false" data-theme="a" data-overlay-theme="a" ');
		html.push('>');
	...
	var _addMultiselMenus = function(controller, ui, html) {
		...
		if (multiSelLinks.length > 0) {
			...
			var id = _addMenus(controller, html, {
				id: 'multisel-acts',
				items: multiSelLinks,
				rightAligned: true
			});
	...
	var _selectMenuHeader = function() {
		var $select = $(this);
			...
			if ($.mobile.activePage && $option) {
				...
				if (value) {
					...
					if (lnk) {
						...
						var ppurl = $sdata.parseSdataUri(url);
					}
				}
			}
		} finally {
