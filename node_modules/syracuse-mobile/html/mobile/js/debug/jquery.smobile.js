...
(function($) {
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel) {
		...
		// 			// -A- Add title
		// 			//html.push('</div><div class="s-m-header-cell-ttl"><h1 class="s-m-header-ttl">');
		// 			html.push('</div>');
		// 		}
		...
		html.push('<div class="s-m-header-tab "><div class="s-m-header-cell-icon nosel">');
		if (controller.parent) {
			// Back arrow to return to parent - Ex : detail of a grid line, object reference
			opts["data-action"] = "back";
			opts["data-icon"] = "arrow-l";
		...
		// -2- Add title / Message
		// -> nosel to hide this in in multiseletion mode
		html.push('<div class="s-m-header-cell-ttl nosel"><h1 class="s-m-header-ttl">');
		// pgtitle Could be HTML (home page sage in green)
		html.push(pgtitle);
		html.push('</h1></div>');
		// -2- Add Message nbselected for multiselection
		// -> nosel to hide this in in multiseletion mode
		html.push('<div class="s-m-header-cell-ttl multisel"><h1 class="s-m-header-ttl" ');
		html.push('id="');
		html.push(controller.prefix + 'multiselttl');
		html.push('">');
		// Empty by default - updated by program
		html.push('</h1></div>');
		// -3- Create rights icons - actions
		var htmRight = _createHeaderRight(controller, ui, nopanel);
		html.push(htmRight);
		// >>>>>>> 9af1ed45b5b16c142e82b0a2a06de67a43d97a16
		html.push('</div>');
		return html.join('');
	};
...
(function($) {
	...
	Controller.prototype = {
		...
		multiSelOn: function() {
			var h = $(".s-m-header-tab");
			return h && h.hasClass("multisel");
		},
		_multiSelSetTtl: function(nb) {
			$('#' + this.prefix + 'multiselttl').html($helpers.format($helpers.locale().ui.multinbsel, [nb == null || nb <= 0 ? 0 : nb]));
		},
		// Query multiselection - Switch to multilines selection / no selection
		actionmultiswitch: function(options, params) {
			$(".s-m-header-tab").toggleClass("multisel");
			this._multiSelSetTtl("");
		...
		actionmultidel: function(options, params) {
			console.log("actionmultidel");
			if (options && options.success) options.success(null, false, true);
		},
		// Query multiselection - Apply action on  selected lines
		actionmultivalid: function(options, params) {
			console.log("actionmultivalid");
			if (options && options.success) options.success(null, false, true);
		},
