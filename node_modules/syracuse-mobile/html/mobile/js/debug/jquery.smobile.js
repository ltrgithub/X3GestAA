...
(function($) {
	...
	var _addMenus = function(controller, html, options, ui) {
		options = options || {};
	...
	var _addMultiselMenus = function(controller, ui, html) {
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel, isLocalData) {
		...
		html.push('</div>');
		var multisel = ui.$facet == '$query' && _addMultiselMenus(controller, ui, html);
	...
	var _updtLineInfo = function(lineData, ok, message) {
		var message = message || "No message returned by server";
		lineData.elmt.addClass(ok ? "ok" : "ko");
		lineData.elmt.find("a.ui-link-inherit").append('<div class="s-m-multisel-msg ' + (ok ? "ok" : "ko") + '">' + message + "</div>");
	};

	var _selectMenuHeader = function($select, controller) {
		...
		try {
			if ($.mobile.activePage && $option) {
				...
				var list = $('#' + controller.controlByClass('SDataList').id);
				list.find(".s-m-multisel-msg").remove();
				list.find("li.multisel").removeClass("ok ko");
				if (value == "unselall" || value == "selall") {
					controller.extraData.multiSel = [];
					list.find("li.multisel").removeClass("multisel");
					if (value == "selall") {
						list.find("li").addClass("multisel").each(function(x) {
							var x = $(x);
							//remove black frame
							x.removeClass("ui-btn-up-c");
							controller.extraData.multiSel.push(x[0]);
						});
					}
					controller._multiSelUpdtHead();
				} else if (value) {
					try {
						$.mobile.loading('show', {
							theme: "a",
						...
						if (lnk) {
							...
							data.forEach(function(lineData) {
								...
								console.log("_selectMenuHeader send action " + value + "\n\t" + url);
								$sdata.post(url, {}, function(resdata) {
									console.log("\tResult OK");
									var ok, message;
									if (value == "UNLOC") {
										_updtLineInfo(lineData, false, "It's just an error test on UNLOC action");
									} else if (resdata != null) {
										_updtLineInfo(lineData, true, null);
									} else {
										var infos = $helpers.dataReadErrors(resdata);
										console.log(JSON.stringify(infos, null, 2));
										_updtLineInfo(lineData, true, infos.message);
									}
								}, null, function(jqXHR, textStatus, errorThrown) {
									console.log("\tResult KO - " + jqXHR.status);
									var message;
									...
									_updtLineInfo(lineData, false, message);
								});
							});
						}
					} catch (e) {
				...
				$('#' + id).bind("change", function() {
					...
					if (controller == null) {
						...
						window.setTimeout(function() {
							...
							var controller = that.pageController($.mobile.activePage);
							_selectMenuHeader($select, controller);
						}, 200);
					} else {
...
(function($) {
	...
	Controller.prototype = {
		...
		draftId: function() {
			var self = this;
			console.log(self.options.url.id);
		...
		multiSelReset: function() {
			...
			if (multiSel) {
				multiSel.forEach(function(e) {
					e = $(e);
					e.removeClass("multisel ok ko");
					e.find(".s-m-multisel-msg").remove();
				});
				self.extraData.multiSel = null;
				self._multiSelUpdtHead();
			}
		},
		...
		actioncancel: function(options) {
			...
			if (self.editActivity()) {
				$sdata.deleteDraft(self.draftId(), function() {
					var localUrl = controller.data.$localUrl;
					...
					$sdata.getSyncData(function(res) {
						...
						if (res) {
							var goToDetail = function() {
								...
								$helpers.confirmDlog.historyBack(function(url) {
									// Find details page in history
									return url && url.indexOf("details") > 0;
								});
							};
							var locale = $helpers.locale();
							var opts = {
			...
			if (self.editActivity()) {
				...
				var after = function(cbOpts, errors, sDataUrl) {
					if (sDataUrl && $helpers.hasFieldsErrors()) {
						...
						// -> If fields error we display general error in the top of the page and don't go to draft page
						...
						// Force stay on same page
						options.success(null, true, true);
						// Reload edited page to refresh error messages by using
						// mobile client workingcopies standard url
						var url = $.smobile.ui.urlHref($sdata.editUrl(sDataUrl), self);
						window.location.replace(url);
					} else {
						//  Issue #2811
						options.success(cbOpts, $helpers.hasErrors(), false);
					}
				};
					...
					.saveDraft(
						...
						function(result) {
							...
							if (result && result.status == 404) {
								$helpers.pushInfo($helpers.locale().unmodified);
							}
							...
							// catch/display fields error (see hasFieldsErrors below)
			...
			if (controller.hasMsgView) {
				if (controller.controls[controller.prefix + "errors"]) {
					...
					if (c.uiClass) {
						...
						if (f)
							f(controller, c.options, $('#' + c.id), c.id);
					}
				}
			}
			if (clearErrors)
		...
		_clearErrors: function() {
			...
			$helpers.clearErrors();
			var errctrl = this.controlByClass("MsgView");
			if (errctrl)
				$('#' + errctrl.id).empty();
		},
