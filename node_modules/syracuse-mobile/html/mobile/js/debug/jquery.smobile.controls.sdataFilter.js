		...
		uidlg: function(config, filterController, page) {
			...
			var ui = {
				...
				actions: {
					savesettings: function(after) {
						...
						if (!dlogController.checkData()) {
							dlogController.disabled = false;
							return;
						}
						...
						switch (valFieldProto.$type) {
							...
							case "application/x-choice":
								title = null;
								valFieldProto.$value.$enum.some(function(item) {
									if (item.$value == value) {
										title = $proto.execExpression(item.$title, valFieldProto);
										return true;
									}
									return false;
								});
							...
							case "application/x-boolean":
								title = value === true ? $helpers.locale().yes : $helpers.locale().no;
								break;
							default:
								break;
						}
			...
			switch (valFieldProto.$type) {
				...
				case "application/x-boolean":
					...
					break;
				case "application/x-reference":
					var lnks = valFieldProto.$item.$links;
					if (lnks && lnks.$lookup) {
						// Only if the link is declared in prototype
						edtCtrl.uiClass = "Lookup";
						edtCtrl.data.bind = "value";
						edtCtrl.data.$lookupurlparsed = true;
						edtCtrl.data.$lookupurl = $proto.parseExpression(lnks.$lookup.$url, {
							data: null,
			...
			that.controls.SingleField.initData(valFieldProto, valFieldProto.$type, config.field, edtCtrl.data, null);
