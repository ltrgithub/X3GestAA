...
(function($) {
	...
	function _errorUi(title, diag, last) {
		var res = {
			"$css": "dlogErrors",
			"$title": title,
			"content": {
	...
	function _processDlog(controller, start) {
		var idx = 0;
		var allmsgs = $helpers.allMessages() || ["Unexpected empty error message"];
		var diag = allmsgs[idx];
		var ui = _errorUi((idx + 1) + '/' + allmsgs.length + " " + $helpers.locale().ui.messages + " ", diag, (idx + 1) == allmsgs.length);
