...
(function($) {
	...
	that.controls.navFooter =  function(hasNext, hasPrev, id, create, options, html) {
		...
		if (create) {
			...
			html.push('class="s-m-navig-parent ui-bar'+(isVisible?'':' s-m-hidden') + '">');
	...
	that.ui.addLinks = function(id, controller, html, cp, bind) {
		...
		var links = $proto.extractLinks(cp, proto, controller.getCurrentData(), edit);
	...
	that._addLink = function(html, options, controller, oncontent) {
		...
		if (options.value != null) {
			...
			if (options.title) {
				...
				html.push('<span class="s-m-style s-m-linkdesc theme'+ settings.preferences.linkDescTheme+'"> ' + $helpers.htmlEncode(options.title) + '<span>');
			}
