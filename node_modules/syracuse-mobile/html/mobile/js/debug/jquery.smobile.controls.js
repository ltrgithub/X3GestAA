...
(function($) {
	...
	that.ui.addErrors = function(controller, bind, id, html) {
		var errors = controller.getDiagnoses(bind);
		html.push('<div  data-role="none" class="s-m-state-block s-m-error" id="' + that.ui.getErrId(id) + '">');
		if (errors) {
	...
	that._addLink = function(html, options, controller, oncontent) {
		...
		if (options.value != null) {
			...
			if (options.title) {
				...
				if (options.$article && options.$article.$refDescription) $proto.auth.mobileExtentions(cssl, options.$article.$refDescription);
