...
(function($) {
	...
	that.ui.menuItem = function(title, description, icon) {
		...
		// SAM 105484 - replace equalIgnoreCase by toLowerCase comparison
		if (title.toLowerCase() == description.toLowerCase()) description = "";
	...
	that.ui.panelItem = function(title, icon, description, count, item) {
		...
		if (item && item.type === 'templates') {
			res += '<span class="s-m-notify-templates ui-li-count s-m-hidden">0</span>';
		}
		return res;
	};
	...
	// #5814 - To not modify existing code we accept a controller (object) or an applicationID (string) as second parameter 
	that.ui.urlHref = function(url, controllerOrAppId) {
		var appid = controllerOrAppId == null ? null : typeof controllerOrAppId === "string" ? controllerOrAppId : controllerOrAppId.applicationId;
		var res = [($helpers.hashFirst() ? ('#' + that.linkDetailPage(appid)) : "")];
	...
	that.ui.refreshErrors = function(controller, bind, id) {
		var diags = controller.getDiagnoses(bind);
		if (diags && diags.length) {
			var err = diags.join('\n');
			that.ui.updateError(controller, $('#' + id), id, err);
			return err;
		}
		return null;
	};
		...
		if (options.value != null) {
			...
			if (options.title && options.$article.$isRrefDescrHidden !== true) {
				...
				if (options.$article.$refDescription) {
					before = options.$article.$refDescription.$before === true;
					$proto.auth.mobileExtentions(cssl, options.$article.$refDescription);
				}
			}
			var title = $helpers.htmlEncode(before ? options.value : options.title),
				value = $helpers.htmlEncode(before ? options.title : options.value);
			html.push(value);
			if (cssl) {
				html.push('<span class="' + cssl.join(' ') + '"> ' + title + '</span>');
			}
		} else if (oncontent) {
			oncontent();
		}
