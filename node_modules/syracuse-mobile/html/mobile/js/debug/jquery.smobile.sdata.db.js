...
(function($) {
	...
	var _addResource = function(row, data, resources, purl, values, map, searchmap) {
		//#3744
		//after Virginie's fix on the server
		// needed prototype to transform boolean values "true" ->true and "false" ->false
		var proto = null;
		if (purl.contract == 'erp' && purl.application == 'x3')
			proto = $.sdata.config.pages[purl.query.representation].$prototype;
		if (data) {
			/* apply where */
			if (!$sdata.execWhere(data, purl.pwhere, proto)) return false;
	...
	var _indexDB = {
		...
		_openDB: function(ccb) {
			...
			var request;
			try {
				request = indexedDB.open(dbName, _indexDB.version);
			} catch (ex) {
				if (_log) _log("_openDB.indexedDB.open - ERROR");
			...
			request.onerror = function(e) {
				if (_log) _log("_openDB - ERROR");
				if (request && request.error && request.error.message) {
					var msg = "_openDB - ERROR - errName: " + request.error.name + "\nerrMsg: " + request.error.message;
					console.log(msg);
					if (!e.message) e = new Error(msg);
				}
				$helpers.pushdbError(e);
				cb(null, e);
			};
		...
		_getSDataByUrl: function(url, objectstore, cb) {
			...
			sdr.onerror = function(event) {
				if (_log) _log("_getSDataByUrl - ERROR");
				cb(null, event);
			};
			sdr.onsuccess = function(event) {
				if (_log) _log("_getSDataByUrl - SUCCESS");
		...
		_rmvSDataByUrl: function(url, objectstore, cb) {
			...
			sdr.onerror = function(event) {
				if (_log) _log("_rmvSDataByUrl - ERROR");
				cb(null, event);
			};
			sdr.onsuccess = function(event) {
				if (_log) _log("_rmvSDataByUrl - SUCCESS");
				cb(null);
			};
		},
			...
			request.onsuccess = function(event) {
				if (event.target.result) {
					...
					if (purl.template) {
						createActivity = true;
						purl.template = false;
						purl.id = $sdata.createGetDataId(purl, cdata.data);
					}
		...
		_markSdataDeleted: function(o_url, sdataObj, odata, cb) {
			...
			if (purl.template) {
				createActivity = true;
				purl.template = false;
				purl.id = $sdata.createGetDataId(purl, odata);
			}
			purl.createActivity = false;
			purl.editActivity = false;
			var url = $sdata.url2local(purl);
		...
		_getModifiedData: function(db, purl, cb) {
			_indexDB._scanDb(db, "sdata", null, function() {
				if (!_checkDBHandler(db, cb)) return;
				...
				crequest.onsuccess = function(event) {
					...
					if (cr) {
						idx++;
						if (_log && cr.value.appid === appid) {
							_log("[" + idx + "] Status->" + cr.value.status + " - entity_url->" + cr.value.entity_url);
						}
						...
						if (_log) _log("_getModifiedData-> found=" + found + " - nb=" + idx);
						cb(res);
					}
				};
		...
		_installApp: function(db, data, cb) {
			...
			var doInsert = function() {
				...
				reqt.onerror = function(event) {
					if (_log) _log("_installApp.doInsert - ERROR");
					$helpers.pushdbError(event);
					cb();
				};
				reqt.onsuccess = function(event) {
					if (_log) _log("_installApp.doInsert - SUCCESS");
					cb();
				};
			...
			request.onerror = function(event) {
				if (event.target.errorCode == IDBDatabaseException.NOT_FOUND_ERR) {
					if (_log) _log("_installApp.openCursor - Not found");
					doInsert();
				} else {
					if (_log) _log("_installApp.openCursor - ERROR");
					$helpers.pushdbError(event);
					cb();
				}
			};
		...
		_removeFromDB: function(db, id, cb) {
			...
			_indexDB._uninstallDraft(draft, sdata, sdatafav, id, function() {
				...
				rmvapp.onsuccess = function(event) {
					if (_log) _log("_removeFromDB - SUCCESS");
					cb();
				};
				rmvapp.onerror = function(event) {
					if (_log) _log("_removeFromDB - ERROR");
		...
		_getAppConfig: function(db, appId, cb) {
			...
			request.onerror = function(event) {
				if (_log) _log("_getAppConfig - ERROR");
				$helpers.pushdbError(event);
				cb(null, event);
			};
		},
		_saveAppConfig: function(db, data, cb) {
			...
			var savedata = null;
			request.onsuccess = function(event) {
				if (_log) _log("_saveAppConfig - GET - SUCCESS");
				var cr = event.target.result;
				delete data.$info;
				cr.data = data;
				...
				if (cr) {
					...
					pr.onsuccess = function(event) {
						if (_log) _log("_saveAppConfig - PUT - SUCCESS");
						cb(null);
					};
					pr.onerror = function(event) {
						if (_log) _log("_saveAppConfig - PUT - ERROR");
						cb(null);
					};
			...
			request.onerror = function(event) {
				if (_log) _log("_saveAppConfig - GET - ERROR");
				$helpers.pushdbError(event);
				cb(null, event);
			};
		},
		...
		_getInstalledApps: function(db, grouped, cb) {
			...
			request.onsuccess = function(event) {
				...
				if (cr) {
					var data = cr.value;
					if (_log) _log("\t" + data.title);
					if (_log) _log("\t" + data.baseURL);
					results.push({
						id: data.id,
						url: data.url,
						title: data.title,
					...
					if (_log) _log("_getInstalledApps - SUCCESS - Nb=" + results.length);
					cb(results);
				}
			};
			request.onerror = function(event) {
				if (_log) _log("_getInstalledApps - ERROR");
				if (event.target.errorCode == IDBDatabaseException.NOT_FOUND_ERR) {
					cb(results);
				} else {
					$helpers.pushdbError(event);
		...
		_updData: function(db, oldUrl, data, cb) {
			if (_log) _log("_updData " + JSON.stringify(oldUrl, null, 2));
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction(["sdata"], IDBTransaction.READ_WRITE);
			var sdata = tx.objectStore("sdata");
			data.url = oldUrl;
	...
	var _private = {
		...
		_openDB: function(cb) {
			if (window.openDatabase) {
				var error = function(tx, error) {
					$helpers.alert($helpers.locale().ui.no_db_support_priv + "\n\n" + error.message, function() {}, true);
					cb(null);
				};
				var maxsize = 200000;
				try {
					var db = window.openDatabase(dbName, "1.0", "HTML 5 Database for Mobile", maxsize);
				} catch (ex) {
					$helpers.pushException({
						title: $helpers.locale().ui.no_db_support_sol,
						message: ex.message,
		...
		_installApp: function(db, data, cb) {
			if (_log) {
				_log("_installApp");
				_log("\ttitle=" + (data ? data.title : "Null data"));
				_log("\tbaseURL=" + (data ? data.baseURL : "Null data"));
			}
			if (!_checkDBHandler(db, cb)) return;
			var _failTran = function(tx, error) {
				if (_log) _log("_installApp.transaction - ERROR");
				$helpers.pushdbError(error);
				cb();
			};

			db.transaction(function(tx) {
				if (_log) _log("\ttransaction - SUCCESS");
				var _after = function() {
					tx.executeSql("INSERT INTO apps(id, url, title, description, icon, baseURL, version, data, dsdescription) values(?,?,?,?,?,?,?,?,?)", [data.id, data.url, data.title, data.description, data.icon, data.baseURL, data.version, JSON.stringify(data.data), data.dsdescription],
						function(tx, result) {
							if (_log) _log("\tinsert - SUCCESS");
							cb();
						}, function(tx, error) {
							if (_log) _log("\tinsert - ERROR");
							$helpers.pushdbError(error);
							cb();
						});
				...
				tx.executeSql("delete from apps where url=? and baseURL= ?", [data.url, data.baseURL],
					function(tx, result) {
						if (_log) _log("\tdelete - SUCCESS");
						_after();
					},
					function(tx, error) {
						if (_log) _log("\tdelete - ERROR");
						_after();
					});
			});
		},
		//ok
		_clearSdata: function(db, id, tx, cb) {
			if (_log) _log("_clearSdata");
			if (!_checkDBHandler(db, cb)) return;
			if (tx) {
				return _private._execSQL(db, tx, 'DELETE from sdata where appid=?', [id], {
					success: function() {
						if (_log) _log("_clearSdata.delete - SUCCESS");
						cb();
					},
					fail: function() {
						if (_log) _log("_clearSdata.delete - ERROR");
						cb();
					}
				});
			} else {
				var _failTran = function(tx, error) {
					if (_log) _log("_clearSdata.transaction - ERROR");
					$helpers.pushdbError(error);
					return cb();
				};
				db.transaction(function(tx) {
		...
		_removeFromDB: function(db, id, cb) {
			...
			if (!_checkDBHandler(db, cb)) return;
			var _failTran = function(tx, error) {
				if (_log) _log("_removeFromDB.transaction - ERROR");
				$helpers.pushdbError(error);
				return cb();
			};
			db.transaction(function(tx) {
				if (_log) _log("_removeFromDB.transaction - SUCCESS");
				var _removeFav = function() {
					return _private._execSQL(db, tx, 'DELETE from sdatafav where appid=?', [id], {
						success: function() {
							if (_log) _log("_removeFromDB._removeFav - SUCCESS");
							cb();
						},
						fail: function() {
							if (_log) _log("_removeFromDB._removeFav - ERROR");
							cb();
						}
					});
				};
				var _removeApp = function() {
					return _private._execSQL(db, tx, 'DELETE from apps where id=?', [id], {
						success: function() {
							if (_log) _log("_removeFromDB._removeApp - SUCCESS");
							_removeFav();
						},
						fail: function() {
							if (_log) _log("_removeFromDB._removeApp - ERROR");
							_removeFav();
						}
					});
				};
				...
				var _removeDrafts = function() {
					return _private._execSQL(db, tx, 'DELETE from draft where appid=?', [id], {
						success: function() {
							if (_log) _log("_removeFromDB._removeDrafts - SUCCESS");
							_removeApp();
						},
						fail: function() {
							if (_log) _log("_removeFromDB._removeDrafts - ERROR");
							_removeApp();
						}
					});
				};
			...
			_private._execOpen(db, 'SELECT * FROM apps where id=?', [appId], function(rows, error) {
				...
				if (rows && rows.length) {
					...
					try {
						...
						console.log("_getAppConfig- PARSING JSON ERROR\nappId=" + appId);
					}
					cdata = cdata || {};
					cdata.id = cr.id;
					cdata.$info = {
					...
					if (_log) _log("_getAppConfig - SUCCESS - Empty");
					cb(null);
					return;
				}
			});
		...
		_saveAppConfig: function(db, data, cb) {
			...
			_private._execOpen(db, 'SELECT * FROM apps where id=?', [data.id], function(rows, error) {
				...
				if (rows && rows.length) {
					if (_log) _log("_saveAppConfig - SUCCESS");
					var cr = rows.item(0);
					delete data.$info;
					var cid = data.id;
					...
					if (_log) _log("_saveAppConfig - SUCCESS - Empty");
					cb(null);
					return;
				}
			});
		...
		_getInstalledApps: function(db, grouped, cb) {
			...
			_private._execOpen(db, 'SELECT * FROM apps ORDER BY url', [], function(rows, error) {
				...
				if (_log) _log("_getInstalledApps - SUCCESS - Count=" + results.length);
				cb(results);
			});
		},
		//ok
		...
		_saveTemplate: function(db, keys, template, cb) {
			...
			db.transaction(function(tx) {
				var _insert = function() {
					tx.executeSql("INSERT INTO template(id, appid, page, user, data) values(?, ?, ?, ?, ?)", [template.$uuid, keys.appid, keys.page, keys.user, JSON.stringify(template)],
						function(tx, result) {
							cb();
						},
						function(tx, error) {
