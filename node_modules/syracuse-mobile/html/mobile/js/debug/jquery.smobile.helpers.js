	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method, userMsgTitle, userMsgDetail) {
		...
		if (diags.length > 0) {
			diags.forEach(function(d) {
				...
				// First line in title
				if (userMsgTitle == null && userMsg.length > 0) userMsgTitle = userMsg[0];
				if (userMsgDetail) {
					detail = userMsgDetail + "\n" + detail;
				}
				if (userMsgTitle == null) userMsgTitle = that.locale().ui.unknownError;
				if (d.$severity === "info") {
...
(function($) {
	...
	$.helpers.confirmDlog = {
		...
		call: function(controller, opts) {
			...
			if (opts.yesCb || opts.yesBack === true) htm += '<a href="#" class="dlog-yes" data-role="button" data-theme="d"' + (opts.yesBack ? ' data-rel="back"' : "") + '>' + locale.yes + '</a>';
		...
		historyBack: function(callBack) {
			...
			hist.clearForward();
			if (dst) {
				console.log("startIdx=" + startIdx + " - prevIdx =" + prevIdx);
				// Works fine
				window.history.go(-1 * (startIdx - prevIdx));
				/*
