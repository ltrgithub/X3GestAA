...
(function($) {
	...
	that.extractLinks = function(cp, proto, data, edit, pagedata, controller) {
		var exclude = ["$lookup"];
		if (!cp) return null;
		if (!data) data = {};
		...
		Object.keys(scan).forEach(function(ln) {
			...
			if (!lv.$url) {
			...
			} else if (controller && (lv.$url.indexOf("$workingCopies?") != -1)) {
				...
				cl.$url = $.smobile.ui.urlHref(url, controller);
			} else {
