...
(function($) {
	...
	that.extractLinks = function(cp, proto, data, edit) {
		...
		Object.keys(scan).forEach(function(ln) {
			...
			if (edit && $proto.support$actions) {
			} else if (!lv.$url || !lv.$title) return;
	...
	that.canAdd = function(arrayproto, arrayData) {
		var res = (arrayproto.$capabilities.indexOf('append')>=0) || (arrayproto.$capabilities.indexOf('insert') >=0);
		if (res && arrayproto.$maxItems &&  (arrayData.length >=  arrayproto.$maxItems)) res=false;
		return res;
	};
	that.canDel = function(arrayproto) {
		var res = (arrayproto.$capabilities.indexOf('delete')>=0);
		return res;
	};
	...
	that.htmlValue = function(data, proto, cp, field, value, options) {
		...
		switch (field.$type) {
			...
			case "application/x-datetime":
				// todo
				if (value) value = $scroller.formatDate(
				$helpers.dateSettings.dateFormat, Date.parseISO8601(value));
				return $helpers.htmlEncode(value || "");
			...
			case "application/x-boolean":
				...
				if (options.list) return '<div class="ui-checkbox" style="display:inline"><center class="ui-icon ui-icon-checkbox-' + (value ? "on" : "off") + '"> </center></div>';
	...
	that.getReferenceField = function(cn, cp, proto) {
		if (that.useUuid(cp)) return cn;
		var display = cp.$value || cp.$key;
		if (display && (display.charAt(0) === '{')) {
			display = display.substring(1, display.length-1);
			if (display && cp[display]) {
				display = cp[display];
				if (display && (display.charAt(0) === '{')) {
					display = display.substring(1, display.length-1);
				}						
		...
		if (display && proto.$properties[display])	return display;
		return "";
...
(function($) {
	...
	that.cleanUp = function(obj, norecursive) {
		var keys = Object.keys(obj);
		keys.forEach(function(p) {
			var o = obj[p];
			if (o && (typeof o === "object") && !Array.isArray(o) && !norecursive) {
				that.cleanUp(o, false);
			}
			obj[p] = null;
		});
	...
	var _compat = function() {
		...
		if (supportISO) {
			...
			Date.parseISO8601 = function(s) {
				...
				if (m === null) throw new Error("Invalid ISO String");
				var d = new Date;
				if (!d[6]) {
					return new Date(+m[1], m[3] ? (m[3] >> 0) - 1 : 0, m[5] >> 0);
				}
		...
		function(pn) {
			...
			if (Array.isArray(o)) {
				for (var i = 0, len = o.length; i < len; i++) {
					var co = o[i];
					if (typeof co === "object") {
						_extractErrors(co, "", false, path ? (path + '.' + pn + '[' + i + ']') : pn,
						res, type);
					}
				}
			} else if (typeof o === "object") {
				_extractErrors(o, "", false,
	...
	that.extractErrors = function(data, defaultMsg) {
		// Extract all msgs (warn/err/info) and create errors to display
		return that._extractAll(data, defaultMsg);
	};
	that._extractAll = function(data, defaultMsg) {
		var res = _extractErrors(data, defaultMsg || "", true, "", null, null);
...
(function($) {
		...
		format: function() {
			...
			if (that.dateSettings.ampmText) {
				...
				return format.replace(/ii/, mms).replace(/i/, ms).replace(
					/ss/, sss).replace(/s/, ss).replace(/hh/, HHs)
					.replace(/h/, Hs).replace(/A/, ampm);
			} else return format.replace(/ii/, mms).replace(/i/, ms).replace(
				/ss/, sss).replace(/s/, ss).replace(/HH/, HHs)
				.replace(/H/, Hs);
		}
	};
	...
	that.lang2DateSettings = function(shortDate, shortTime) {
		...
		if (shortTime) that.dateSettings.timeFormat = shortTime.replace(/tt/g, 'A')
			.replace(/m/g, 'i');
