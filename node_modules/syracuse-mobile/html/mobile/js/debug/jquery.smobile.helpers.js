...
(function() {
	...
	// SAM 105484 - remove equalIgnoreCase
...
(function($) {
	...
	// #5814 replace controller by applicationId 
	that.extractLinks = function(cp, proto, data, edit, pagedata, applicationId) {
		var exclude = ["$lookup"];
		if (!cp) return null;
		if (!data) data = {};
		...
		Object.keys(scan).forEach(function(ln) {
			...
			} else if (applicationId && (lv.$url.indexOf("$workingCopies?") != -1)) {
				// #3719 links of type "Representation" in X3  are generated with url contain the pattern "$workingCopies?"
				// they are implemented as  a new type of action - actionrepresentation, the url is processed
				cl.actionType = "representation";
				cl.icon = "service " + cn;
				var url = that.parseExpression(lv.$url, {
					data: ped,
					$prototype: proto
				}, "$url");

				// detect presense of a key in brakets, if present, 
				// remove workingcopies and do not add $templates to avoid creation of new record
				// e.g. sdata/x3/erp/SUPERV/AQTCRUD('10')/$workingCopies?... ->
				// sdata/x3/erp/SUPERV/AQTCRUD('10')?...
				if (url.match(/\/.*\(.*\)\/\$workingCopies/)) {
					//  #5578 - Nothing - url = url.replace("$workingCopies?", "?");
				} else {
					url = url.replace("$workingCopies?", "$template?");
				}
				cl.$url = $.smobile.ui.urlHref(url, applicationId);
			} else {
	...
	//#5818 - Returns $detail url for $bind
	//Does'nt take into account arrays embeded into array (just images in first level array)
	that._getImageUrl = function(fieldProto, $bind, parentBind) {
		var url = fieldProto.$url;
		var property = url.split("/")[1];
		var proto = fieldProto;
		while (proto && !proto.$representation) {
			proto = proto.$protoparent;
		}
		if (!parentBind || !proto) return url;
		var rep = proto.$representation + ".$details";
		proto = $.sdata.config.pages[rep].$prototype;
		if (!proto || !$bind) return url;
		["$properties", parentBind, "$item", "$properties", $bind].some(function(prop) {
			proto = proto[prop];
			return proto == null;
		});
		if (!proto) return url;
	...
	//#5818 - Add options.parentData to allow resolution of image url expression
	// 		  Add options.edit and option.parentBind to allow resolution of image url expression for edit mode (hack)
	that.htmlValue = function(data, proto, cp, field, value, options) {
		...
		if (exprData && options.parentData) {
			exprData.push(options.parentData);
		}
		...
		switch (field.$type) {
			...
			case "image":
				...
				if (value && typeof value === "string") {
					...
					src = "data:" + ct + ";base64," + value.$value;
				} else if (value && proto) {
					var url = value.$url;
					if (!url && cp.$url) {
						url = options.edit ? that._getImageUrl(cp, field.$bind, options.parentBind) : cp.$url;
						url = that.parseExpression(url, {
							data: exprData,
							$prototype: cp
						}, "$url");
					}
					if (url) {
						...
						src = curl + '?' + (data.$etag ? 'etag=' + data.$etag : "") + "&salt=" + ((new Date()).getTime());
					}
				}
				return '<image class="s-image-detail" src="' + src + '"/>';
				break;
			default:
				break;
	...
	that.proto2Instance = function(cp) {
		...
		Object.keys(cp.$properties).forEach(function(propName) {
			...
			var cpProp = cp.$properties[propName];
			that.proto2Value(cpProp, propName, res);
			...
			// #4229 we just copy needed properties instead of copying all props - Some props like $prototype/$rprototype... cause bug when object is stringified
			var props = res.$properties[propName] = {};
			for (var p in _dataProps) {
				p = _dataProps[p];
				if (cpProp[p] != null) {
					props[p] = cpProp[p];
				}
		...
		if (prop.charAt(0) == "@") {
			...
			while (p) {
				if (p.hasOwnProperty(prop)) return {
					value: p[prop],
					found: true,
					//#5818 - $instaceUrl resolution for $edit
					proto: p.$protoparent
				};
				p = p.$protoparent;
			}
				...
				if (!level && proto && !proto.noRef && proto.$protoparent && (proto.$protoparent.$type === "application/x-reference")) {
					...
					if (!pvalue.found) {
						...
						if (pvalue.value.indexOf("{") >= 0) {
							//#5818 - use pvalue.proto
							var cp = proto;
							if (pvalue.value.indexOf("{" + prop + "}") >= 0) {
								cp = pvalue.proto || proto.$protoparent;
							}
							if (options.html) {
...
(function($) {
	...
	// prototype = null -> Count Fields Error
	that.dataReadErrors = function(data, defaultMsg, root, path, res, type, prototype) {
		...
		if (data.$properties) {
			...
			that._extractDiag(data.$properties, path, res, type);
			var o = {};
				...
				if (o.$diagnoses && o.$diagnoses.length) {
					...
					if (prototype && prototype.$properties && !prototype.$properties[pn]) {
						...
						o.$diagnoses.forEach(function(d) {
							if (d.$message) {
								d.message = d.$message = d.$message + " (" + pn + ")";
							} else if (d.message) {
								d.$message = d.message = d.message + " (" + pn + ")";
							}
	...
	that.pushDiagnoses = function(data, opts) {
		if (!data) return null;
		if (!opts) opts = {};
		// checkFieldsError = true  + prototype != null -> Fields error are not stored as general message
		var checkFieldsError = opts.checkFieldsError && opts.prototype != null;
		var res = that.dataReadErrors(data, opts.defaultMsg || "", true, "", null, null, checkFieldsError ? opts.prototype : null);
		if (checkFieldsError && res.hasFieldsErrors) {
			that.setFieldsErrors();
		} else if (res && res.errors && res.errors.length > 0) {
			var tmp = [],
				msg;
			that._sortDiagnoses(res.errors);
			res.errors.forEach(function(d, idx) {
				msg = d.message + (d.stackTrace || "");
				if (tmp.indexOf(msg) === -1) {
					// Skip duplicated msgs
					tmp.push(msg);
					if (d.severity == "error") {
						if (idx == 0) {
							that.pushException(d);
						} else {
							that.pushErrorMessage(d.$message);
						}
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method, userMsgTitle, userMsgDetail) {
		...
		if (jqXHR.status === 403 && diags && diags.length > 0) {
			var msg = [];
			diags.forEach(function(d) {
				msg.push(d.message);
			});
			that.setDisplayErrorAsDialog();
			return that.pushError(msg.join('\n'));
		}
			...
			diags.forEach(function(d) {
				...
				if (userMsg.length >= 1) {
					...
					if (userMsg.length > 0) {
						...
						// No title (application exception) for messages and warnings
						if (userMsgTitle == null || (d.$severity === "info" || d.$severity === "warning")) {
	...
	that._pushMsgItem = function(error) {
		...
		var errors = $.sdata.sessionStorage.getItem(key);
		errors = errors ? JSON.parse(errors) : [];
		errors.push(error);
		$.sdata.sessionStorage.setItem(key, JSON.stringify(errors));
	};
	...
	that._clearAllMsgItems = function(type) {
		_errKeys.forEach(function(typ) {
			...
			$.sdata.sessionStorage.removeItem(key);
		});
	};
	...
	// #3817 - propagate only fields style - not layout styles ($themeLayout)
	var _toPropagate = ["$styles", "$theme", "$label", "$emptycell", "$inquiry", "$refDescription"];
	that.propagateMobileExt = function(to, from, exclude) {
		if (!to || !from) return false;
		var done = false,
			prop, x;
		for (var i = 0; i < _toPropagate.length; i++) {
			prop = _toPropagate[i];
			if (exclude == null || exclude.indexOf(prop) === -1) {
				x = from[prop];
				if (x != null && to[prop] == null) {
					done = true;
					// #3817 Temporarilly - We need to know if property has been propagated or not - Conflict with A/B theme between rowLayout and fields 
					if (!to.$propagated) to.$propagated = [];
					to.$propagated.push(prop);
					if (typeof x == "object") {
						to[prop] = $.extend(true, Array.isArray(x) ? [] : {}, x);
					} else {
						to[prop] = x;
					}
				}
...
(function($) {
	...
	var _IOS = null;
	$.helpers.isIOS = function() { // detect iOS in general (display apps on first start after creating a desktop link requires refresh)
		if (_IOS == null) {
			_IOS = /iPhone|iPad|iPod/.test(navigator.platform) && navigator.userAgent.indexOf("AppleWebKit") > -1;
		}
		return _IOS;
	};
	var _isIOSWithManifestIssue = null;
	$.helpers.isIOSWithManifestIssue = function() { // Detect iOS version not supporting html cache manifest correctly
		if (_isIOSWithManifestIssue == null) {
			_isIOSWithManifestIssue = /iPhone|iPad|iPod/.test(navigator.platform) && navigator.userAgent.indexOf("AppleWebKit") > -1 && (true || navigator.userAgent.indexOf("OS 7_0 ") > -1 || navigator.userAgent.indexOf("OS 7_1 ") > -1);
			//  ^-- true because no version is fixed right now!
		}
		return _isIOSWithManifestIssue;
	};
	$.helpers.isOfflineAvail = function() {
		return !$.helpers.isIOS();
	};
	$.helpers.handleHttp401 = function(status, jqXHR, next) {
		if (status !== 401) {
			// Return false to continue in original flow
			return false;
		}
		try {
			var res = JSON.parse(jqXHR.responseText);
			var url = res.$diagnoses[0].$links.$redirect.$url;
			if (url) {
				window.location.href = url;
			}
			return true;
		} catch (e) {
			// Continue as usually since response was not understood
		}
		return false;
	};
	$.helpers.getLabel = function(path) {
		if (!path || typeof path !== "string") return "";
		path = path.split(".");
		var l = that.locale();
		for (var i = 0; i < path.length && l != null; i++) {
			l = l[path[i]];
		}
		return l != null && typeof l === "string" ? l : "";
	};
	var _sdataHost = (window.location.protocol.toLowerCase() === "file:") ? "" : (window.location.protocol + "//" + window.location.host);
})(jQuery);