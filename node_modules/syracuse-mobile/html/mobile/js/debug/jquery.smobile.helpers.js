...
(function($) {
	...
	that.online = function() {
		...
		return !$(document.body).hasClass("offline");
	};
	that.setOffline = function(yes) {
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method, userMsgTitle, userMsgDetail) {
		...
		var diags = that._sortDiagnoses(diag.$diagnoses);
		diags.forEach(function(d) {
			...
			var stack = d.$stackTrace;
			if (stack) {
				var p = stack.indexOf("<<< async stack >>>");
				if (p > 0) stack = stack.substring(0, p);
			}
			if (userMsg.length > 1) {
				// Copy lines except first one to details
				detail += userMsg.join('\n');
			}
			if (userMsgDetail) {
				detail = userMsgDetail + "\n" + detail;
			}
			if (d.$severity === "info") {
				that.pushInfo(userMsgTitle || userMsg || detail);
			} else if (d.$severity === "warning") {
				that.pushWarning(userMsgTitle || userMsg || detail);
			} else {
				// We just display first line
				that.pushError({
					origin: _errors.http,
					title: userMsgTitle || userMsg,
					data: {
						message: detail,
		...
		if (!diags.length) {
			var detail = "METHOD : " + method + " - STATUS : " + jqXHR.status + "\n" + url;
			var userMsg = that.locale().ui.unknownError;
			if (errorThrown) {
				detail += "\n" + errorThrown;
	...
	that.clearErrors = function(withoutFieldErrors) {
		if (withoutFieldErrors !== true) that._rmvFieldsErrors();
...
(function($) {
	...
	$.helpers.confirmDlog = {
		...
		call: function(controller, opts) {
			...
			if (opts.yesCb) htm += '<a href="#" class="dlog-yes" data-role="button" data-theme="d"' + (opts.yesBack ? ' data-rel="back"' : "") + '>' + locale.yes + '</a>';
			// noBack= true -> histpry.back on cancel otherwise back is done by noCb
			if (opts.noCb) htm += '<a href="#" class="dlog-no" data-role="button" data-theme="d"' + (opts.noBack ? ' data-rel="back"' : "") + '>' + locale.no + '</a>';
			// cancel do back
