...
(function($) {
	...
	$sdata.checkVersionInCache = function(versionMobile) {
		...
		window.console.log(clientVersion);
		window.console.log(serverVersion);

		if (clientVersion && // Basic sanity checks to ensure we got some valid information
			clientVersion.length > 5 &&
			serverVersion &&
			serverVersion.length > 5) {
			//if (location.href && location.href.indexOf("mobile.html") > -1) {	// Only check version in compressed client???
			if (clientVersion !== serverVersion) {
				//					alert($.helpers.format($.helpers.locale().newVersion, [serverVersion, clientVersion]));
