...
$(function() {
	...
	that.changePage = function(url, options) {
	...
	that.docEvents = {
		...
		"pagebeforechange": function(e, data) {
			if (!window.location.hash || !data || !e) {
				e.preventDefault();
				return;
			}
					...
					$.smobile.loadPage(u.hash, page, function(uiId, ui, error, afterdata) {
						...
						$.smobile.parseURL(u, page, function(href, resData, error, info) {
							...
							$.smobile.buildPage(uiId, activePage, $page, ui, resData, info, page, $ptemplate, function() {
								...
								//if(navigator.userAgent.match(/iPad/i) || ( navigator.userAgent.match(/iandroid/i)  && !navigator.userAgent.match(/imobile/i) )){
								if (navigator.userAgent.toLowerCase().indexOf("ipad") > -1 || (navigator.userAgent.toLowerCase().indexOf("android") > -1 && navigator.userAgent.toLowerCase().indexOf("mobile") == -1)) {
									...
									if (window.orientation == 90 || window.orientation == -90) {
                                        ...
                                        /*
										if (controller.pageInfo.leftPanel) {
										    var lpanel=$("#"+controller.prefix + "panel");
										    lpanel.panel("open");
										}
										*/
									}
		...
		$(window).bind("orientationchange", function(event) {
			...
			if (navigator.userAgent.toLowerCase().indexOf("ipad") > -1 || (navigator.userAgent.toLowerCase().indexOf("android") > -1 && navigator.userAgent.toLowerCase().indexOf("mobile") == -1)) {
				if (event.orientation == "landscape") {
					...
					panel = $('#' + controller.prefix + 'rpanel');

					panel && panel.attr("data-display", "push");
					panel && panel.panel({
						display: "push"
					});
					//alert(panel.attr("data-display") );

					//panel && panel.attr("data-display","overlay");
