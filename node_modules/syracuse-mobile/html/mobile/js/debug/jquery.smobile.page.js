								...
								try {
									$helpers.each(data, function(lineData, ii, doNext) {
										...
										$sdata.post(url, {}, function(resdata) {
											...
											if (value == "UNLOC") {
												ok = false;
												message = "It's just an error test on UNLOC action";
											...
											controller.multiselActUpdt(lineData.$elmt, true, infos.message, ii + 1, data.length);
											doNext();
										...
										controller.multiselActStop($lst);
									});
								} catch (e) {
									controller.multiselActStop($lst);
									throw e;
