...
(function($) {
	...
	var _createFooter = function($footer, controller, ui) {
		...
		if (ui.footerActions && $helpers.online()) {
			...
			var foot = controller._getFooter(true);
	...
	var _private = {
		...
		getCtxItems: function(page, facet, representation, data, items) {
			...
			if (query) {
				...
				items.length = 0;
				if (acts.$abort && !acts.$abort.$isHidden) {
					items.push({
				...
				if (acts.$delete && !acts.$delete.$isHidden) {
					items.push({
						title: $helpers.locale().ui.remove_item,
						action: "delete",
						"jmdata": {
				...
				if (acts.$save && !acts.$save.$isHidden) {
					items.push({
			...
			if (uiData && uiData.$facet && $sdata.config) {
				...
				if (page) {
					...
					if (auth && auth.$footeractions) {
						...
						links = $proto.extractLinks(proto, proto, null, uiData.$facet == "$create" || uiData.$facet == "$edit", page);
						if (links) {
