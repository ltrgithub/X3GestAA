...
(function($) {
			...
			if (installed && installed.length) {
				...
				if ($.helpers.isIOS()) {
					...
					if ($.sdata.localStorage.getItem(x) === "no") {
						// $.helpers.logOnServer("START - NO FORCE RELOAD");
						$.sdata.localStorage.removeItem(x);
					} else {
						$.sdata.localStorage.setItem(x, "no");
						// $.helpers.logOnServer("START - FORCE RELOAD");
	...
	that.linkDetailPage = function(controller) {
		if (controller.applicationId) {
			...
			return that.PAGES.page.id + that.hash_sep + $helpers.encodeURIComponent(controller.applicationId) + that.hash_sep + $helpers.uuid();
		} else {
			...
			if (uiData && uiData.$facet && $sdata.config) {
				...
				if (page) {
					...
					if (auth && auth.$footeractions) {
						...
						links = $proto.extractLinks(proto, proto, data, uiData.$facet == "$create" || uiData.$facet == "$edit", page);
						if (links) {
