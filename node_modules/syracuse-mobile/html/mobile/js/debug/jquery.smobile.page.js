...
(function($) {
				...
				if (id === "root") {
					...
					/*if (myParent){
						for (var i=0; i < propagate.length; i++){
							var prop = propagate[i];
							if (myParent[prop] && !c.$styles[prop]){
								c[prop] = myParent[prop];
							}
						}
					}*/
	...
	that.doBack = function(ctrl) {
		if (ctrl && ctrl.pageOptions.detail && ctrl.parent.data && ctrl.parent.data.$info && ctrl.parent.data.$info.isMethodCall) {
			// Method call
			// Like the result of a call generates a page not stored in browser history (changeHash=false) we force to display this page by doing a changepage on the dom element
			// This page is well displayed and back works in that case
			// Inconvenient : the original page stays in browser history and a back from parent go to this page instead of application home (for inquiries reps)
