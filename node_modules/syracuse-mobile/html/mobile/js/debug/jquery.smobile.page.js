...
(function($) {
	...
	that.PAGES = {
		...
		/* logout screen */
		"langchanged": {
			ui: "langchanged.$static",
			useAppId: false,
			isHome: false,
			noLeftPanel: true,
			db: false
		},
		/* language changed screen */
		"templates": {
			ui: "templates.$static",
			useAppId: true,
			db: false,
	...
	that.PAGES.start.enhanceHandler = function(pdata, after) {
		...
		$sdata.getInstalledApps(false, function(installed) {
			...
			if (installed && installed.length) {
				...
				if ($.helpers.isIOS()) {
					// #4036 - SAM 98951 - Fix IOS 7.1 bug with web application
					// Create new shortcut + Install + click button iphone + login -> No more application - use a new shortcut to reproduce the bug
					// We force reload if start page is empty otherwise the installed applications are not actually stored in local db and are not displayed on reload
					// If we reload the empty page it works and we can see installed applications in db with the debugger :-(
					var x = "s-no-start-reload";
					if ($.sdata.localStorage.getItem(x) === "no") {
						$.sdata.localStorage.removeItem(x);
					} else {
						$.sdata.localStorage.setItem(x, "no");
	...
	// Override standard doAfterData method - doAfterData is bind to an
	...
	that.PAGES.settings.doAfterData = function(data) {
		var self = this,
			autoLogon = null,
			logon;
	...
	that.PAGES.language.enhanceHandler = function(pdata, after) {
		pdata.content.locale.data.items = [];
		$.smobile.userProfile.queryLocalePreferences(function(locales) {
			// Entity -> UI data structure
			locales.forEach(function(locale) {
				pdata.content.locale.data.items.push({
					value: locale.code,
	...
	that.PAGES.about.enhanceHandler = function(pdata, after) {
		after(pdata);

		// Set background image class
		$("div[data-url='#about']").addClass("s-m-app-bkg about");

		// Tab switching of about dialog
		var $ul = $("#aboutTabs");
		$ul.delegate('li', 'click', function() {
			var tab = $(this).children("a").attr("data-href");
			var $li = $ul.find("li");
			for (var i = 0; i < $li.length; i++) {
				var ctab = $($li[i]).children("a").attr("data-href");
				var $tab = $("#" + ctab);
				if (ctab === tab) {
					$tab.removeClass("s-m-hidden");
				} else {
					$tab.addClass("s-m-hidden");
				}
			}
		});

		if ($helpers.online()) {
			var $aboutVersionInfo = $("#aboutVersionInfo");
			$.smobile.userProfile.getUserProfileVersionInfo(function(data) {
				if (data && data.versionInfo && data.versionInfo.versionMobile) {
					$aboutVersionInfo.removeClass("s-m-hidden");
					var clientVersion = $helpers.buildDateString();
					var serverVersion = data.versionInfo.versionMobile.buildDateString;
					if (clientVersion != serverVersion) {
						var $update = $aboutVersionInfo.find("a");
						$update.text($helpers.format($helpers.locale().about.serverVersion, [serverVersion]));
						$update.removeClass("s-m-hidden");
	...
	// Page hash - #5814
	that.linkDetailPage = function(applicationId) {
		if (applicationId) {
			// #3412 - add uid (pageId) in detail link
			// Instead of adding it on pageload we add it into the link (se pageNAme function)
			//-> The navigator history stores the application id AND pageId when we click the link
			//-> When  we goBack to the parent page we retrieve the page (back after validating the sort criterion)
			//   Otherwise we can't retrieve the page and create a new one each time we go back
			return that.PAGES.page.id + that.hash_sep + $helpers.encodeURIComponent(applicationId) + that.hash_sep + $helpers.uuid();
		} else {
	...
	that.scrollIntoView = function(element, offset) {
		if (element == null)
			return;
		if (!element.jquery)
			element = $(element);
		window.scrollTo(0, Math.max(offset != null ? offset : element.offset().top - parseInt($(window).height() / 2), 0));
	};
	// Used to retrieve representation/facet in page hash
	var _parsePageURI = function(query) {
		var params = {};
	...
	var _createPage = function(controller, controls, html, options) {
		...
		try {
			...
			var emptyCellOption = null,
				propagate = ["$styles", "$theme", "$label"];
			Object.keys(controls).forEach(function(id, index) {
				var c = controls[id];
				...
				if (id === "root") {
					...
					var myParent = controls[c.parent];
					if (myParent) $helpers.propagateMobileExt(c.data, myParent.data);
					emptyCellOption = c.layout == "RowLayout" ? c.data && c.data.$emptycell ? c.data.$emptycell : "withtitle" : null;
					// Layout controls - see jquery.smobile.layout.js
					var l = $.smobile.addLayout.apply(controller, [parent, c]);
					if (l.$inquiry != null && !controller.pageInfo.$inquiry) {
						// Set inquiry mode if there is at least one inquiry bloc param or result
						// Used later to open 
						controller.pageInfo.$inquiry = true;
					}
					...
					// Do not display empty cells in RowLayout #3128 - details only
					var parentLyt = controls[c.parent];
					if (!c.data) c.data = {};
					if (!parentLyt && html[c.parent]) {
						// Tabs layout - Only one control c_001 per tab - Each tab has c_001_0, _1 id but refers to c_001
						parentLyt = controls[html[c.parent]["parent"]];
					}
					if (parentLyt) {
						if (!c.data.$article) c.data.$article = {};
						$helpers.propagateMobileExt(c.data.$article, parentLyt.data);
					}
					c.emptyCellOption = emptyCellOption;
					$.smobile.addControl.apply(controller, [parent, c,
						function(ctrl) {
							...
							if (ctrl && Array.isArray(ctrl)) {
								...
								if (c.emptyCellOption != null) {
									// We have to modify the class in html string :-(
									var p = html[c.parent];
									if (p && p.before) {
										// Cell css is initialize by layout.RowLayout method according to $emptycell option
										// If a control is attached to the cells we reset $emptycell option to calculate the it according ctrl value
										p.before = p.before.replace(" empty", "");
										// Issue #3128 - do not display empty cells or empty rows for RowLayouts
										if (ctrl.isEmpty || ctrl.html == null || ctrl.html.length == 0) {
											if (p.before.indexOf("s-m-l-row") >= 0) {
												// We add empty class in cell
												if (c.emptyCellOption == "empty") {
													p.before = p.before.replace("s-m-l-row", "s-m-l-row empty");
												}
												p = html[p.parent];
												if (p && p.childs && (p.nbEmpty != null || p.before.indexOf("s-m-l-table") >= 0)) {
													p.nbEmpty = p.nbEmpty == null ? 1 : p.nbEmpty + 1;
													if (p.nbEmpty == p.childs.length) {
														// We add empty class in row
														p.before = p.before.replace("s-m-l-table", "s-m-l-table empty");
														delete p.nbEmpty;
													}
												}
											}
	...
	var _createHeader = function(controller, ui, pgtitle, nopanel, isLocalData) {
		...
		// -2- Add title / Message
		html
			.push('<div class="s-m-header-cell-ttl"><h1 class="s-m-header-ttl" id="');
		...
		html.push('</div>');
		var multisel = ui.$facet == '$query' && $helpers.online() && that.multisel.addMultiselMenus(controller, ui, html);
	...
	var _createFooter = function($footer, controller, ui) {
		// Add footer buttons comming from authoring
		if (ui.footerActions && $helpers.online()) {
			var cp = $proto.getProto(controller.getPageData());
			var btns = [];
			var sorted = [];
			var doSort = true;

			for (var p in ui.footerActions) {
				var lnk = ui.footerActions[p];
				doSort = doSort && lnk.$footeraction && lnk.$footeraction.$order != null;
				sorted.push(p);
			}
			if (doSort) { // sort only if all elements have an $order property
				sorted.sort(function(a, b) {
					var la = ui.footerActions[a];
					var lb = ui.footerActions[b];
					return la.$footeraction.$order > lb.$footeraction.$order;
				});
			}
			for (var i = 0; i < sorted.length; i++) {
				...
				if (ttl.length > 0) {
					var cdid = controller.addAlink(lnk);
					btns.push({
						$isHidden: lnk.$isHidden,
						linkId: "footer_act_" + p,
						title: ttl,
						//#5814 - add new type of button that open a page
						url: lnk.$url,
						$parameters: lnk.$parameters,
						bind: lnk.bind,
						//icon: lnk.$footeraction.$icon,
						action: lnk.$method === "GET" ? "nolink" : lnk.actionType || "service",
						actionData: '{"id":"' + cdid + '"}',
						"jmdata": {
							"data-role": "button",
							"data-icon": "s-" + lnk.$footeraction.$icon,
							"data-theme": lnk.$footeraction.$theme || "v"
						}
					});
				}
			}
		...
		if (cpage) {
			...
			if (ctrl) {
				...
				if (target) {
					...
					if (p) {
						if (p === "filterheader" && that.openFilterDlg) {
						...
						if (hnd) {
							...
							return hnd.call(ctrl, {
									success: function(cbOpts, error, noback) {
										ctrl.disabled = false;
										if (!error && !noback) {
											// if cbOpts._cburl -> Goto this page -
											// Else back
											if (cbOpts && cbOpts._cburl) {
												window.location.hash = that.ui
													.urlHref(cbOpts._cburl,
														ctrl);
											} else {
												$.smobile.doBack(ctrl);
											}
						...
						var eTarg = $(e.target);
						if (eTarg.hasClass('s-m-header-cell-icon')) {
							// #3999 improve click on header cell - trigger a click on anchor if click on header cell (enlarge active zone)
							var a = eTarg.find("a");
							if (a.length > 0) {
								a.trigger('click');
								e.stopPropagation();
	...
	// Click events on content
	var _clickOnFooter = function(e) {
		return _clickOnHeader(e, true);
	};

	var _bindEvents = function(controller, ui, $header, $content, $footer) {
		$header.click(_clickOnHeader);
		$content.click(_clickOnContent);
		$footer.click(_clickOnFooter);
		// Expanding and collapsing boxes sometimes causes charts to not be centered correctly.
		// This prevents the effect
		$content.find('div:jqmData(role="collapsible")').bind("expand", function(e, ui) {
			var $container = e && e.target && $(e.target);
			if ($container) {
				var $charts = $container.find(".s-m-gauge-container");
				...
				if (len > 0) {
					...
					for (i = 0; i < len; i++) {
						...
						ct && ct.setSize($chart[0].parentElement.clientWidth, ct.chartHeight);
					};
				}
			}
		});
		$content.bind("updatelayout", function(e) {
			// Triggered when open/close collapsible layouts
			var t = $(e.target);
			if (t.is(':visible')) {
				// UPdate table to check display mode repsonsive (grid) or not
				t.find("div.s-m-table-wrapper").each(function(i, e) {
					$.smobile.controls.types.table_refresh($(e));
				});
			}
		});
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info,
		...
		var $content = $page.children(":jqmData(role=content)").children(".content-primary");
		var $panel = $page.children(":jqmData(sagerole=panel)");
		var $rpanel = $page.children(":jqmData(sagerole=rpanel)");
		...
		pageInfo.leftPanel = hasPanel;
		pageInfo.rightPanel = hasrPanel;
		...
		if (hasrPanel) {
			...
			var overlay = $rpanel.attr("data-display") === "overlay";
			$rpanel.on("panelbeforeopen", function(event, ui) {
				if (!overlay) that.docEvents.disableSwipe();
				var cpage = $.mobile.activePage;
				if (cpage) {
					var ctrl = that.pageController(cpage);
					if (ctrl)
			...
			$rpanel.on("panelbeforeclose", function(event, ui) {
				that.docEvents.enableSwipe();
				// onRPanelBeforeClose -> To manage push data-display
				controller.onRPanelBeforeClose(event, ui);
			});
		}
		// $page.page();
		...
		// Set page footer Html (add buttons) after the page has been created
		_createFooter($footer, controller, ui);
		// Set events
		_bindEvents(controller, ui, $header, $content, $footer);
		cb();
	};
	// Retun static page descriptions
	that.getStaticUI = function(name, page) {
		if (name === "apppanel") {
			return that.staticui[name](page && (page.page.id === "page" || page.page.id === "send"));
		} else {
	...
	var _private = {
		...
		representation2ui: function(params, data) {
			...
			if (params.facet == "$dashboard") {
				...
				// #4516 - Allows display for summary facets
				// - > No Edit for $summary because $create opens edit facet (not an EditSummary facet) and back from edit returns to detail
			} else if ((params.facet === "$summary") || (params.facet === "$details") || (params.facet == "$edit") || (params.facet == "$create")) {
				iaddErrorView = false;
				cd = {};
				var map = {};
		...
		getCtxItems: function(page, facet, representation, data, items) {
			if (!data) data = {};
			var isHidden = function(id, isAction, isEnabledInOffline, forceVisible) {
				if (forceVisible === true)
					return false;
				if (isEnabledInOffline === true) {
					if ($helpers.online() === false) {
						if (['$save', '$abort'].indexOf(id) >= 0)
							return false;
					}
				}
				var meta = data[isAction === true ? "$actions" : "$links"];
				if (meta && meta[id] && meta[id].$isHidden != null) return meta[id].$isHidden;
				var proto = page.$prototype[isAction === true ? "$actions" : "$links"];
				if (proto && proto[id] && proto[id].$isHidden != null) return proto[id].$isHidden;
				return false;
			};
			var url, ppurl;
			var inDraftMode = data.$localUrl != null;
			...
			if (query) {
				...
				if (links && links.$create) {
					url = $proto.parseExpression(links.$create.$url, {
						data: null,
				...
				var cfg = $sdata.config ? $sdata.config.pages : null;
				if (cfg && cfg[representation + ".$query"]) {
					var pq = cfg[representation + ".$query"].$prototype;
					if (pq.$links && pq.$links.$create) {
						...
						ppurl = $sdata.parseSdataUri(url);
						if (ppurl && ppurl.query && ppurl.query.representation && cfg[ppurl.query.representation]) {
							items.push({
								id: "$create",
								$isHidden: isHidden("$create"),
								title: $helpers.locale().ui.add_item,
				...
				if (links && links.$edit) {
					url = $proto.parseExpression(links.$edit.$url, {
						data: $proto.exprData(data),
						$prototype: page.$prototype
					}, "$url");
					ppurl = $sdata.parseSdataUri(url);
					if (ppurl && ppurl.query && ppurl.query.representation && cfg && cfg[ppurl.query.representation]) {
						items.push({
							id: "$edit",
							$isHidden: isHidden("$edit"),
			...
			} else if (page.$prototype.$actions && ((facet === "$edit") || (facet === "$create"))) {
				var acts = page.$prototype.$actions;
				items.length = 0;
				if (acts.$abort) {
					items.push({
						id: "$abort",
						$isHidden: isHidden("$abort", true, true, inDraftMode),
				...
				if (acts.$delete) {
					items.push({
						id: "$delete",
						$isHidden: isHidden("$delete", true, false, inDraftMode),
						title: $helpers.locale().ui.remove_item,
						action: "delete",
						"jmdata": {
							"data-icon": "s-delete",
				...
				if (acts.$save) {
					items.push({
						id: "$save",
						$isHidden: isHidden("$save", true, true, inDraftMode),
		...
		// Set headers icon actions  - #5814 add page param
		doAfterData: function(data, pageInfo) {
			var uiData = this.ui,
			...
			if (uiData && uiData.$facet && $sdata.config) {
				var page = $sdata.config.pages[uiData.$representation + "." + uiData.$facet];
				if (page) {
					// Add right panel actions in hbuttons
					uiData.hbuttons = _private.getCtxItems(page, uiData.$facet, uiData.$representation, data, uiData.hbuttons || []);
					// Check footer actions
					uiData.footerActions = null;
					var auth = page ? page.$article : null;
					if (auth && auth.$footeractions) {
						var proto = page.$prototype;
						// #5814 pass application ID
						links = $proto.extractLinks(proto, proto, data, uiData.$facet == "$create" || uiData.$facet == "$edit", page, pageInfo ? pageInfo.application : null);
						if (links) {
							links.forEach(function(link) {
								if (auth.$footeractions[link.name] != null) {
									// Add property to link
									link.$footeraction = $.extend(true, {}, auth.$footeractions[link.name]);
									if (!uiData.footerActions) {
										uiData.footerActions = {};
									}
									// Add footer action to page
									uiData.footerActions[link.name] = link;
								}
							});
						}
					}
				}
				if ($helpers.online() && rightPanel && uiData.$facet === "$query") {
					var links = page.$prototype.$links;
					if (links) {
						var rpanel = uiData.rpanel.right_panel.data ? uiData.rpanel.right_panel.data : null;
						for (var link in links) {
							if (link.indexOf("$") != 0) {
								var lnk = links[link];
								if (lnk.$footeraction == null) {
									var ttl = $proto.execExpression(lnk.$title || "", page.$prototype);
									if (ttl.length > 0) {
										// If there's at least one link in right panel we enable multiselection
										if (!rightPanel.data.items) rightPanel.data.items = [];
										rightPanel.data.items.unshift({
											code: "topactions",
											load: true,
											items: [{
												title: $helpers.locale().ui.multilines,
												icon: "multilines",
												action: "multiswitch"
											}]
										});
	...
	that.detailUiData = function(controller, cp, data, $layout, options) {
		...
		if (cl != $layout) {
			$helpers.propagateMobileExt(cl, $layout);
		}
	...
	function _afterEnhance(page, params, uiData, pageData, panel, rpanel, cb) {
		...
		if (!page.page.db) {
			...
			var _stdAfterData = _private.doAfterData.bind(obj);
			var _pageAfterData = page.page.doAfterData ? page.page.doAfterData.bind(obj) : null;
			...
			if (uimap && uimap[rep]) {
				...
				if (uiData) {
					_afterData = _private.doAfterData.bind({
						ui: uiData,
						panel: panel,
						rpanel: rpanel
					});
					...
					console.log("Representation [" + rep + "] -no ui data");
					return cb("", null, null, null);
				}
			} else {
				console.log("Representation [" + rep + "] not found");
				return cb("", null, null, null);
			}
		}
	};
	...
	function _addappItems(items, ritems, params, page) {
		...
		if (params.facet && $helpers.online()) {
			...
			if ((params.facet === "$query") || (params.facet === "$lookup")) {
				...
				// no action in right panel for query and lookup
				// For query Actions are available in multiselection
				// TODO - Filter actions according to the presence of a key in url
				item = {
					code: "actions",
					load: true,
		...
		if (params.facet && (params.facet === "$edit" || params.facet === "$create")) {
			item.items.push({
				"type": "templates"
			});
		}
			...
			$sdata
				.getAppConfig(
					...
					function(config, error) {
						if (!config) {
							throw new Error("Application not found in local database\nPlease try to reinstall it\nhostname " + window.location.host + "\nappid[" + page.application + "]");
						...
						_addappItems(panel.data.items,
							rpanel.data.items, params, page);
						_afterEnhance(page, params, uiData, pageData,
							panel, rpanel, cb);
					});
	...
	var _loadDataAndDesign = function(query, page, cb) {
		...
		var params = _parsePageURI(query);
	...
	that.removePage = function(page) {
		var controller = $.jqmData(page, "controller");
		if (controller) {
			// $helpers.log("Remove page" + (controller ? (": " +
			// (controller.pageInfo && controller.pageInfo.page &&
			// controller.pageInfo.page.id) + "-" + (controller.pageInfo &&
			// controller.pageInfo.pageId || "")) : ""));
			controller.remove();
		}
		var $t = $(page);
	...
	// remove all pages
	that.removeAllPages = function(exclude, cb) {
		exclude = exclude || [];
	...
	that.parsePageURI = _parsePageURI;
	...
	that.refreshPage = function() {
		...
		window.setTimeout(function() {
			return $.smobile.changePage(window.location.hash, {
				// changeHash: true,
	...
	that.showSends = function(controller) {
		if ($sdata.config && $sdata.config.id) {
			$.smobile.changePage("#send-" + $sdata.config.id, {
				allowSamePageTransition: true
			});
		}
	};
	...
	that.createDialog = function(uiData, pageData) {
		...
		$.smobile.buildPage(uiId, $.mobile.activePage, cp.$page, uiData,
			pageData, {
				...
				$.smobile.changePage(cp.$page, options);
				controller.bindData();
			});
	...
	that.parseURL = function(u, page, cb) {
		...
		var params = _parsePageURI(u.search || u.hash);
		var filters = null;
		...
		//if (params.dataurl && params.dataurl.indexOf("localstorage") == 0) {
		if (params.url && params.url.indexOf("&localstorage=") >= 0) {
			var json = $.sdata.localStorage.getItem("s-methodcall-transfer");
			var data;
			try {
				data = JSON.parse(json);
				if (!data.$info) data.$info = {};
				data.$info.isMethodCall = true;
			} catch (e) {
				throw new Error("Error parsing localstorage data");
			}
			var href = '?url=' + $helpers.encodeURIComponent(params.dataurl);
			href = base + (doEncodeHref ? $helpers.encodeJsmQuery(href) : href) + ($helpers.hashFirst() ? "" : ('#' + page.longPageName));
			return cb(href, data, false, options);
		} else if (params.url && (params.facet !== "$dashboard")) {
				...
				success: function(data, headers) {
					...
					if (headers.status > 300) {
						...
						if ((headers.status == 404) && page.application) {
							...
							if (purl.id) {
								// Issue #3076 - Goto query if found in history
								if ($.helpers.historyBack(function(url) {
									return url && url.indexOf("query") > 0;
								}, false)) {
									return;
								}
								// if not found force to go to this page
								...
								purl.id = '';
								var qurl = purl.base + purl.application + '/' + purl.contract + '/' + purl.dataset + '?' + param.representation + '.$query';
								// redirect to $query
								options.stop = true;
								var hash = that.PAGES.page.id + that.hash_sep + $helpers
									.encodeURIComponent(page.application) + $helpers
									.encodeURIComponent('?url=' + $helpers
										.encodeURIComponent(qurl));
								// Issue #3076 - ad a # to make it working
								$.smobile.changePage('#' + hash, {
									changeHash: true
								});
								return;
								...
								// redirect to home
								options.stop = true;
								// Issue #3076 - add a # to make it working
								var hash = "";
								if (that.PAGES.home.id.indexOf('#') != 0)
									hash += '#';
								hash += that.PAGES.home.id + that.hash_sep + $helpers
	...
	that.multisel = {
		...
		_addMenus: function(controller, html, options, ui) {
			...
			if (options.type)
				html.push(' ' + options.type);
			html.push('">');
			var id = controller.prefix + options.id;
			html.push('<select id ="' + id + '" ');
			options.nativeDlog = false;
			if (!options.nativeDlog)
				html
					.push('data-native-menu="false" data-theme="a" data-overlay-theme="a" ');
			html.push('>');
			var itms = options.items || [];
			itms.forEach(function(itm, i) {
				html.push('<option ');
				if (i == 0)
					html.push('data-placeholder="true" ');
		...
		_confirm: function(controller, cb, action, nbSel) {
			...
			$helpers.confirmDlog.call(opts, controller);
		},
		_selectMenuHeader: function($select, controller) {
			...
			try {
				if ($.mobile.activePage && $option) {
					...
					} else if (value === "selinv") {
						controller.multiSelSelectInv();
										...
										try {
											...
											$helpers
												.each(
																		...
																		try {
																			...
																			_log && _log(JSON
																				.stringify(
																					...
																					2));
																			message = $helpers
																				.dataReadErrors(resData).messages;
																		} catch (e) {
													...
													function() {
														controller
															.multiselActStop(parallelException);
													});
										} catch (e) {
											controller
												.multiselActStop(e);
						...
						$('#' + id)
							.bind(
								...
								function() {
									var $select = $(this);
									var controller = that
										.pageController($.mobile.activePage);
									if (controller == null) {
										// If action list is too
										// large the widget displays
										// a page with the list
										// -> Active page is not the
										// $query page
										// -> We call
										// _selectMenuHeader with
										// controller as parameter
										// witn timeout to cal after
										// current page is closed
										// TODO - Temporailly -> To
										// see how to make sur the
										// page is closed
										window
											.setTimeout(
												function() {
