...
exports.schedule = function(_, event, key, executionTime, parameters, mode) {
	...
	if (mode) {
		...
		var fullKey = event ? event.getEntity(_).name + "_" + event.code(_) + "_" + key : key;
...
exports.scheduleAll = function(_, event, data, mode) {
	...
	var description = event ? event.getEntity(_).name + "_" + event.code(_) + "_" : "";
	...
	while (instance = cursor.next(_)) {
		...
		var instanceDescription = instanceEvent ? instanceEvent.getEntity(_).name + "_" + instanceEvent.code(_) + "_" : "";
...
// fill values of db and entity

function _fillValues(_) {
	if (!db) {
		db = adminHelper.getCollaborationOrm(_);
	}
	if (!entity) {
		entity = db.model.getEntity(_, "eventTime");
	}
}
...
function _getInstances(_, time, key) {
	...
	tracer && tracer("vorher" + util.format(whereClause));
	var result = db.fetchInstances(_, entity, {
		jsonWhere: whereClause
	});
	tracer && tracer("nachher" + result.length);
	return result;
}
