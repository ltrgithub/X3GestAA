...
exports.connectX3Client = function(_, x3sol, options) {
	...
	client.createSession(_, sessionOpt);
	return client;
};
...
exports.getOldX3BatchConfig = function(_, batchSrvInst) {
	...
	try {
		...
		var client = exports.connectX3Client(_, batchSrvInst.x3solution(_), connOpt);
...
exports.sdataRequest = function(_, diagnoses, client, url, alias, method, body, options) {
	function _inspectResponse(_, resp) {
		if (resp) {
			if (!options.noTraces) tracer.debug && tracer.debug(options.prefix + alias + " call result: " + JSON.stringify(resp, null, 2));
				...
				if (resp.head.status !== 200) {
					...
					if (diags && diags.length > 0) {
						diags.forEach(function(d) {
							...
							tracer.error && tracer.error(options.prefix + alias + " call result: " + resp.head.status + " - " + d.$message + "\n" + d.safeStack);
						});
					...
					if (!options.noTraces) tracer.info && tracer.info(options.prefix + alias + " call result: " + resp.head.status);
					return resp.body;
				}
			}
		}
	...
	if (!options.noTraces) {
		...
		tracer.debug && tracer.debug(options.prefix + alias + " call params: " + JSON.stringify(params, null, 2));
	}
