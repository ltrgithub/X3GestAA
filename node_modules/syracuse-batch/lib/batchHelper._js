...
var x3FolderHelper = require('syracuse-x3/lib/entities/lookupX3Folder');
...
var host0 = (config.x3batch && config.x3batch.host);

var host;

function checkMandatoryFields(_, inst) {
	var err = false;
	inst.$diagnoses = inst.$diagnoses || [];
	if (!inst.user(_)) {
		inst.$diagnoses.push({
			$severity: "error",
			$message: locale.format(module, "userMandatory")
		});
		err = true;
	}
	if (!inst.role(_)) {
		inst.$diagnoses.push({
			$severity: "error",
			$message: locale.format(module, "roleMandatory")
		});
		err = true;
	}
	if (!inst.refEndpoint(_)) {
		inst.$diagnoses.push({
			$severity: "error",
			$message: locale.format(module, "epMandatory")
		});
		err = true;
	}
	return err;
...
exports.connectX3Client = function(_, x3sol, options) {
	...
	// it works only when 'start' service is called manually or config.x3batch.host is set in nodelocal.js
	if (host0)
		host = host0.replace("{{tenantId}}", globals.context.tenantId || "");
	else
		host = host || globals.context && globals.context.session && globals.context.session.host;
	...
	if (sessionOpt.license) tracer.info && tracer.info("License is used for connection");
...
exports.getOldX3BatchConfig = function(_, batchSrvInst) {
	...
	try {
		if (!checkMandatoryFields(_, batchSrvInst)) {
			var tracePrefix = "[" + batchSrvInst.code(_) + "] ";
			var connOpt = {
				x3server: batchSrvInst.x3solution(_).selectX3Server(_),
				localePref: batchSrvInst.localePref(_),
				user: batchSrvInst.user(_).getEndpointLogin(_, batchSrvInst.refEndpoint(_).$uuid) || "ADMIN",
				folder: batchSrvInst.refEndpoint(_).x3ServerFolder(_) || "X3",
				adxtyp: 35
			};
			var client = exports.connectX3Client(_, batchSrvInst.x3solution(_), connOpt).client;
			var resp = exports.execRpc(_, batchSrvInst.$diagnoses, client, batchSrvInst.refEndpoint(_).getBaseUrl(_), "ASYRSRVBATCH", "ABATCH_MNGT_ALLPARGET", "POST", {}, {
				throwErr: true,
				prefix: tracePrefix
			});
			...
			if (batchSrvInst.$diagnoses) {
				batchSrvInst.$diagnoses.forEach(function(d) {
					if (d.$severity === "error") tracer.error && tracer.error(tracePrefix + d.$message);
					if (d.$severity === "warning") tracer.warn && tracer.warn(tracePrefix + d.$message);
					if (d.$severity === "info") tracer.info && tracer.info(tracePrefix + d.$message);
				});
			}
		}
	} catch (e) {
...
exports.initBatchServer = function(_, s) {
	...
	function startController(_) {
		bs.start(_);
		var tracePrefix = "[" + bs.code(_) + "] ";
		bs.$diagnoses.forEach(function(d) {
			if (d.$severity === "error") tracer.error && tracer.error(tracePrefix + d.$message);
			if (d.$severity === "warning") tracer.warn && tracer.warn(tracePrefix + d.$message);
			if (d.$severity === "info") tracer.info && tracer.info(tracePrefix + d.$message);
		});
		if (bs.status(_) === "running") {
			if (retryInterval) {
				tracer.info && tracer.info("Batch controller [" + bs.code(_) + "] auto start succeeded after " + nbRetries + " tries");
				clearInterval(retryInterval);
			}
		}
	}
	var retryInterval, nbRetries = 0; // used for auto start retries
	var db = adminHelper.getCollaborationOrm(_);
	...
	if (!bs) {
		...
		if (s) {
			try {
				...
				tracer.error && tracer.error("An error occured when synchronising x3 solution and batch server '" + s.code(_) + "': " + e.message);
			}
		}
	...
	if (!bs.role(_)) {
		var rolEntity = db.getEntity(_, "role");
		var rol = db.fetchInstance(_, rolEntity, {
			jsonWhere: {
				code: multiTenant ? "ADMCA" : "ADMIN"
			}
		});
		if (rol) bs.role(_, rol);
	}
	...
	if (bs.auto(_)) {
		if (config && config.batch && config.batch.auto && config.batch.auto.retries) {
			retryInterval = flows.setInterval(function(_) {
				startController(_);
				nbRetries++;
				if (nbRetries >= config.batch.auto.retries) {
					tracer.warn && tracer.warn("Batch controller [" + bs.code(_) + "] can't be started even after " + nbRetries + " retries");
					clearInterval(retryInterval);
				}
			}, config.batch.auto.intervalMs || 30000);
		} else {
			startController(_);
		}
	}
