...
exports.entity = {
	...
	$functions: {
		...
		qRemove: function(_, uuids) {
			var bc = this.getClient(_, false, true);
			if (bc) bc.qRemove(_, this.$diagnoses, uuids);

		},
		qAdd: function(_, confs) {
			var bc = this.getClient(_, false, true);
			if (bc) bc.qAdd(_, this.$diagnoses, confs);
		},
		qUpdate: function(_, confs) {
			var bc = this.getClient(_, false, true);
			if (bc) bc.qUpdate(_, this.$diagnoses, confs);
		},
			...
			for (var i = 0; i < Object.keys(maxQueriesMap).length; i++) {
				...
				if (!helpers.object.areEqual(conf.old, conf.new)) {
					...
					if (conf.new.maxQueries > 0) {
						...
						this._changesOnFly.qUpdate.push({
