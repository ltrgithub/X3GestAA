	...
	$functions: {
		...
		stop: function(_, diagnoses) {
			...
			try {
				...
				diagnoses && diagnoses.push({
					$severity: "info",
					$message: locale.format(module, "alreadyStopped")
				});
				console.error("Batch controller stop error [" + this.code(_) + "]: " + e);
				this.status(_, "stopped");
				this.save(_);
			}
			if (bc) bc.stop(_, diagnoses, this, undefined);
		},
		stopAll: function(_, diagnoses) {
			...
			try {
				...
				console.error("Batch controller stopAll error [" + this.code(_) + "]: " + e);
				this.status(_, "stopped");
				this.save(_);
			}
			if (bc) bc.stopAll(_, diagnoses, this, undefined);
		},
				...
				try {
					...
					lines.forEach(function(line) {
						if (line.charAt(0) === '{') {
							...
							if (childData.port === "B0") b0Found = true;
					...
					if (!b0Found) {
						self.$diagnoses.push({
							$severity: "error",
							$message: locale.format(module, "noBatchServer")
						});
					}
				} catch (e) {
