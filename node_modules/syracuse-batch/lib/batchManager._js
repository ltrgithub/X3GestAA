...
exports.X3BatchClient = helpers.defineClass(function() {}, null, {
	...
	stop: function(_, context, batchSrvInst, stopPendings) {
		try {
			...
			if (stopPendings) {
				this._mngtStopServReq(_, context);
			}

			var servers = this.srvQueue.contents();
			for (var i = 0; i < servers.length; i++) {
				this.srvQueue.read(_);
				var cli = this._getAdminClient(_, i, true);
	...
	_getAdminClient: function(_, idx, ignoreIfNotExist) {
		idx = idx != null ? idx : 0;
		...
		if (cli && !ignoreIfNotExist && !cli.isAlive()) {
	...
	_mngtStopServReq: function(_, context) {
		for (var i = 0; i < Object.keys(this.adminClients).length; i++) {
			var cli = this._getAdminClient(_, i);
			if (cli) {
				this._execRpc(_, context, cli, "ASYRSRVBATCH", "ABATCH_STOP_SERVREQ", "POST", {
	...
	_mngtAllErr: function(_, context, numreq, diags) {
		var self = this;
		diags.forEach_(_, function(_, d) {
			self._execRpc(_, context, self._getAdminClient(_), "ASYRSRVBATCH", "ABATCH_MNGT_ALLERR", "POST", {
				numreq: numreq,
				$severity: d.$severity,
				$message: d.$message
			});
	...
	_execQuery: function(_, srvConf, query, options) {
		...
		if (query.MONO) {
			...
			var pendingOnFolder = this._listQueries(_, {
				where: "(FLAG in (1, 2, 5) and DOSSIER eq '" + query.DOSSIER + "' and NUMREQ lt " + query.NUMREQ + ")",
				filter: "ACTIVE"
			}, {
				noTraces: true
			}).filter(function(q) {
				return q.EXENOW === true;
			});
		...
		try {
			...
			if (e instanceof X3BatchException) {
				...
				tracer.error && tracer.error("[" + this.code + "] Query launch failed '" + query.NUMREQ + ": " + e.message);
		...
		if (_srvConf._queries.length > 0) {
			...
			queries.forEach_(_, function(_, query) {
				...
				if (query.FLAG === 1 || query.FLAG === 2 || query.FLAG === 5) {
					...
					if (query.MONO) {
						if (self.monoPendings[query.DOSSIER]) {
	...
	_executeControlLoop: function(_) {
		...
		if (!this.loopRunning) {
			...
			//
			// console.error("Servers queue contents: " + JSON.stringify(this.srvQueue.contents().map(function(s) {
			//	 return s.descr;
			// }), null, 2));
			//
			try {
				...
				queries = queries.filter(function(q) {
					...
					return q.FLAG === 1 && q.EXENOW === true && (!self.monoPendings[q.DOSSIER] || self.monoPendings[q.DOSSIER] === q.NUMREQ);
				});
