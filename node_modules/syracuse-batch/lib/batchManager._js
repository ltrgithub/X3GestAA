...
exports.X3BatchClient = helpers.defineClass(function() {}, null, {
	_init: function(_, context, batchSrvInst) {
		...
		confEnabled.forEach_(_, function(_, conf) {
			if (srvDescriptions.indexOf(conf.description(_)) === -1) {
				...
				// put metadata on instance itself
				if (srv) {
					serversConf.push({
		...
		console.error("serversConf: " + JSON.stringify(serversConf, null, 2));
		serversConf.forEach_(_, function(_, srvConf) {
			if (srvConf._maxQueries && srvConf._maxQueries > 0) {
				self.totalMaxQueries += srvConf._maxQueries;
	...
	start: function(_, context, batchSrvInst) {
		...
		for (var i = 0; i < servers.length; i++) {
			var srv = this._getX3Server(_, servers[i].descr);
	...
	_removeSrvFromQueue: function(srvInst) {
		this.srvQueue.adjust(function(_, arr) {
			return arr.filter_(_, function(_, s) {
				return s.descr === srvInst.description(_);
			});
		});
	},
	...
	_sdataRequest: function(_, context, client, url, alias, method, body, options) {
		...
		try {
			...
			// TODO handle reconnection mecanism
			throw e;
		}
	},
	...
	_listQueries: function(_, params) {
		...
		var resp = this._sdataRequest(_, null, this.refClients[0], url, "ASYRREQ", "GET", {}
			//,{noTraces: true}
		);
