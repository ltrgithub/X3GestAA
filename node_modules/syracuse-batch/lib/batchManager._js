...
exports.X3BatchClient = helpers.defineClass(function() {}, null, {
	...
	_sdataRequest: function(_, context, client, url, alias, method, body, options) {
		function _inspectResponse(_, resp) {
			if (resp) {
				tracer.debug && tracer.debug("[" + self.code + "] " + alias + " call result: " + JSON.stringify(resp, null, 2));
				if (resp.head && resp.head.status) {
					if (resp.head.status !== 200) {
						...
						tracer.info && tracer.info("[" + self.code + "] " + alias + " call result: " + resp.head.status);
						return resp.body;
					}
				}
			}
		...
		tracer.info && tracer.info("[" + this.code + "] Execute sdata request with client: " + (client && client.pid) + " - alive: " + (client && client.isAlive()) + " (url=" + url + ")");
	...
	_mngtServReq: function(_, context) {
		for (var i = 0; i < Object.keys(this.adminClients).length; i++) {
			...
			if (cli) this._execRpc(_, context, cli, "ASYRSRVBATCH", "ABATCH_MNGT_SERVREQ", "POST", {
	...
	_execQuery: function(_, srvConf, query, options) {
		...
		try {
			...
			this._sdataRequest(!_, null, cli, url, "Batch request " + query.NUMREQ, "POST", {}, options);
			// add the query number in server instance
			srvConf._queries.push(query);
			tracer.info && tracer.info("[" + this.code + "] Execute query '" + query.NUMREQ + "' ended");
		...
		function _selectServer(_, mainOnly) {
			function _refreshPendingQueries(_, _srvConf) {
				...
				if (_srvConf._queries.length > 0) {
					tracer.info && tracer.info("[" + self.code + "] Refresh pending queries statuses");
					...
					queries.forEach_(_, function(_, query) {
						...
						if (query.FLAG === 1 || query.FLAG === 2) {
							// keep in array if still running or not yet really launched
				...
				tracer.info && tracer.info("[" + self.code + "] Pending queries: " + JSON.stringify(_srvConf._queries.map(function(q) {
					return q.NUMREQ;
				})));
			}
			...
			if (mainOnly) {
				...
				if (mainsSrv.length > 0) {
					...
					if (mainSrv.disabled(_) || mainSrv.banned(_)) {
						...
						self._removeSrvFromQueue(_, mainSrv);
						...
						tracer.info && tracer.info("[" + self.code + "] Select (MAIN) '" + srvConf.descr + "' server: pending: " + srvConf._queries.length + " - max: " + srvConf._maxQueries);
						// Here we do not take care about changing server's order in the queue.
						return srvConf;
					} else {
						tracer.warn && tracer.warn("[" + self.code + "] Ignore (because main is full) '" + srvConf.descr + "' server (MAIN): pending: " + srvConf._queries.length + " - max: " + srvConf._maxQueries);
					}
				} else {
				...
				for (var i = 0; i < self.srvQueue.contents().length; i++) {
					...
					if (srv.disabled(_) || srv.banned(_)) {
						...
						// if max number of concurrent queries is not reached for this server, return the server
						// the server instance will be put again in the queue after the launch of the query
						tracer.info && tracer.info("[" + self.code + "] Select (LB) '" + srvConf.descr + "' server: pending: " + srvConf._queries.length + " - max: " + srvConf._maxQueries);
						return srvConf;
					} else {
		...
		try {
			...
			var foldersToIgnore = queries.filter(function(q) {
				if (q.MONO === true && q.FLAG === 1) {
					self.monoPendings[q.DOSSIER] = q.NUMREQ;
					return true;
				}
				return false;
			}).map(function(q) {
			...
			foldersToIgnore.forEach(function(f) {
				tracer.info && tracer.info("[" + self.code + "] Mono is pending on folder " + f + " so ignore all request with this target");
			});
