		...
		if (!cli) {
			try {
				var srvConf = this.srvQueue.contents().filter(function(conf) {
					return conf.descr === cliList[idx];
				})[0];
				cli = this._createAdminClient(_, null, srvConf);
			} catch (e) {
				tracer.warn && tracer.warn("[" + this.code + "] Create administrative client failed on '" + cliList[idx] + "'.\n" + e.stack);
			}
			...
			try {
				...
				this.adminClients[cliList[idx]] = undefined;
