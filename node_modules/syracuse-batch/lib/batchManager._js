...
exports.X3BatchClient = helpers.defineClass(function(code) {
	...
	_createClient: function(_, diagnoses, options) {
		try {
			...
			this.qRemove(_, diagnoses, [options.x3server.$uuid]);
		}
	},
		...
		if (cliList.length === 0) {
			...
			if (cli && !cli.isAlive()) {
				...
				try {
					...
					this.qRemove(_, null, [cli.x3server.$uuid]);
				}
			}
		}
	...
	qRemove: function(_, diagnoses, uuids) {
		...
		try {
			...
			for (var i = 0; i < Object.keys(servers).length; i++) {
				var descr = Object.keys(servers)[i];
	...
	qUpdate: function(_, diagnoses, confs) {
		tracer.debug && tracer.debug("[" + this.code + "] Try to update existing configurations: " + JSON.stringify(confs, null, 2));
		// just waiting 3 seconds to be sure x3solution has been saved (with future)
		var self = this;
		flows.setTimeout(function(_) {
			for (var j = 0; j < confs.length; j++) {
				...
				if (conf.server && conf.maxQueries) {
					// if server is uuid --> fetch it
					var server = typeof conf.server === "string" ? self._getX3Server(_, conf.server) : conf.server;

					var alreadyExists = false;
					for (var i = 0; i < self.srvQueue.contents().length; i++) {
						var s = self.srvQueue.contents()[i];
						if (s.$uuid === server.$uuid) {
							// conf already exists into queue
							alreadyExists = true;
							if (conf.maxQueries != null) {
								self.totalMaxQueries -= s._maxQueries;
								self.totalMaxQueries += conf.maxQueries;
							...
							if (conf.reconnect) {
								...
								if (self.adminClients[oldDescr] && self.adminClients[oldDescr].isAlive()) {
									self.adminClients[oldDescr].disconnect(_);
									var msgDisc = locale.format(module, "clientDisconnected", self.adminClients[oldDescr].pid, oldDescr);
									tracer.info && tracer.info("[" + self.code + "] " + msgDisc);
									diagnoses && diagnoses.push({
										$severity: "info",
										$message: msgDisc
									});
								}
								delete self.adminClients[oldDescr];
								// create new administrative connection
								try {
									self._createAdminClient(_, diagnoses, s, {
										throwErr: true
									});
								} catch (e) {
									tracer.error && tracer.error("[" + self.code + "] Change connection failed: " + e.message);
									return;
								}
							}

							var msg = locale.format(module, "hostUpdated", server.description(_), conf.maxQueries);
							tracer.info && tracer.info("[" + self.code + "] " + msg);
							diagnoses.push({
								$severity: "info",
								$message: msg
							});
