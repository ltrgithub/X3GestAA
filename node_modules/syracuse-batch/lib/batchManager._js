...
exports.X3BatchClient = helpers.defineClass(function(code) {
	...
	stop: function(_, diagnoses, batchSrvInst, stopPendings) {
		...
		try {
			...
			// else launch shutdown loop and keep only one administrative client
				...
				this._launchShutdownLoop(_);
			}

			// disconnect all administrative clients
			for (var i = firstClientIndex; i < Object.keys(this.adminClients).length; i++) {
	...
	listPendingQueries: function() {
		return this.srvQueue.contents().reduce(function(prev, curr) {
			curr._queries.forEach(function(q) {
				return prev.push(q);
			});
			return prev;
		}, []);
	...
	_getAdminClient: function(_, idx) {
		...
		var cli;
		if (cliList.length === 0) {
			throw new Error("[" + this.code + "] No administrative client found.");
		} else {
	...
	applyChanges: function(_, diagnoses, batchSrvInst, changes) {
		...
		for (var j = 0; j < Object.keys(changes).length; j++) {
			var func = Object.keys(changes)[j];
	...
	_launchShutdownLoop: function(_) {
		// execute one first time to not have to wait for scheduling...
		this._executeShutdownLoop(_);
				...
				for (var i = 0; i < queries.length; i++) {
					...
					if (srvConf) {
						try {
							...
							this.srvQueue && this.srvQueue.put(srvConf);
						}
					} else {
