...
exports.X3BatchClient = helpers.defineClass(function(code) {
	...
	_createAdminClient: function(_, diagnoses, srvConf) {
	...
	_updateSrvInQueue: function(_, diagnoses, descr, maxQueries) {
		for (var i = 0; i < this.srvQueue.contents().length; i++) {
			...
			if (s.descr === descr) {
				...
				var msg = locale.format(module, "hostUpdated", descr, maxQueries);
				tracer.info && tracer.info("[" + this.code + "] " + msg);
				diagnoses.push({
					$severity: "info",
					$message: msg
