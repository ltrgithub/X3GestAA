...
exports.getBatchClient = function(code, start) {
	var bc;
	if (multiTenant) {
...
exports.X3BatchClient = helpers.defineClass(function(code) {
	...
	start: function(_, diagnoses, batchSrvInst) {
		...
		try {
			...
			for (var i = 0; i < servers.length; i++) {
				this._createAdminClient(_, diagnoses, batchSrvInst, servers[i]);
			}
	...
	stopAll: function(_, diagnoses, batchSrvInst) {
		return stop(_, diagnoses, batchSrvInst, true);
	},
	...
	_createAdminClient: function(_, diagnoses, batchSrvInst, srvConf) {
	...
	_getAdminClient: function(_, idx) {
		...
		var cli;
		if (cliList.length === 0) {
			tracer.warn && tracer.warn("No administrative client found. Restart of batch controller will be done !!!");
			this.stop(_);
			this.start(_);
		} else {
			...
			if (cli && !cli.isAlive()) {
				tracer.info && tracer.info("[" + this.code + "] Admin client on '" + cli.x3server.description(_) + "' fall... Try to reconnect it");
	...
	_sdataRequest: function(_, diagnoses, client, url, alias, method, body, options) {
		...
		tracer.info && tracer.info("[" + this.code + "] Execute sdata request on " + client.x3server.description(_) + " with adonix: " + (client && client.pid) + " - alive: " + (client && client.isAlive()) + " (url=" + url + ")");
	...
	_refreshPendingQueries: function(_, _srvConf) {
		...
		Object.keys(this.monoPendings).forEach_(_, function(_, k) {
			var m = this.monoPendings[k];
			var _queries = self._listQueries(_, {
				where: "(NUMREQ eq " + m.NUMREQ + ")"
			}, {
				noTraces: true
			});
			if (_queries.length === 0) delete this.monoPendings[k];
		});
		...
		if (!this.loopRunning) {
			...
			try {
				...
				var queries = this._listQueries(_, {
					...
					// - not concerned by folder occupied by mono requests
					// - EXENOW eq true
					representation: "ASYRREQPEND"
				});
