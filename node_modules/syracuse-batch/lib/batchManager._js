...
function removeBatchClient(_, code) {
	if (multiTenant) {
		var tenantId = globals.context.tenantId;
		var bClients0 = bClients[tenantId];
...
exports.X3BatchClient = helpers.defineClass(function() {}, null, {
	...
	stop: function(_, context, batchSrvInst, stopPendings) {
		...
		try {
			...
			if (this.countPendingRequests() === 0) {
				this.status === "stopped";
				this._setBatchInstanceStopped(_);
			}
		} catch (e) {
			console.error(e.stack);
	...
	_setBatchInstanceStopped: function(_) {
		var bs = this.db.fetchInstance(_, this.db.model.getEntity(_, "batchServer"), {
			jsonWhere: {
				code: this.code
			}
		});
	...
	_execQuery: function(_, srvConf, query, options) {
		...
		if (query.MONO) {
			...
			var pendingOnFolder = this._listQueries(_, {
				where: "(DOSSIER eq '" + query.DOSSIER + "' and NUMREQ lt " + query.NUMREQ + ")",
				filter: "ACTIVE"
			}, {
				noTraces: true
			}).filter(function(q) {
