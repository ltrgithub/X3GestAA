...
exports.X3BatchClient = helpers.defineClass(function() {}, null, {
	...
	_createClient: function(_, context, options) {
		try {
			...
			var sessionOpt = {
				...
				baseUrl: host + ep.getBaseUrl(_), //context && context.baseUrl,
				collaborationBaseUrl: adminEP ? host + adminHelper.getCollaborationEndpoint(_).getBaseUrl(_) : "",
				peerAddress: peer
			};
	...
	_execQuery: function(_, srvConf, query, options) {
		...
		try {
			...
			if (e instanceof X3BatchException) {
				...
				tracer.error && tracer.error("[" + this.code + "]  _execQuery error: " + e.stack);
			}
		}
	},
		...
		if (_srvConf._queries.length > 0) {
			...
			queries.forEach_(_, function(_, query) {
				...
				if (query.FLAG <= 2) {
					...
					if (query.MONO) {
						var idx = self.monoPendings[query.DOSSIER].indexOf(query.NUMREQ);
						if (idx !== -1) {
							self.monoPendings[query.DOSSIER].splice(idx, 1);
						}
						if (self.monoPendings[query.DOSSIER].length === 0) {
	...
	_executeControlLoop: function(_) {
		...
		function _selectServer(_, mainOnly) {
			...
			for (var i = 0; i < nbSrvConcerned; i++) {
				...
				if (srv.disabled(_) || srv.banned(_)) {
					...
					debugTrace && debugTrace("MAX QUERIES REACHED SO PUT THE SRV CONF IN THE QUEUE");
		...
		//
		debugTrace && debugTrace("Servers queue contents: " + JSON.stringify(this.srvQueue.contents().map(function(s) {
			return s.descr;
		}), null, 2));
		//
		try {
			...
			debugTrace && debugTrace("Active queries: " + JSON.stringify(queriesMap, null, 2));

			// Mono queries management section
			var monoToExecute = [];
			var folders = Object.keys(queriesMap);
			folders.forEach_(_, function(_, folderName) {
				var _queries = queriesMap[folderName].filter(function(q) {
					return q.MONO === true && q.FLAG === 1 && q.EXENOW === true;
				});

				// is at least one mono is waiting
				if (_queries.length > 0) {
					// consider the first one only
					var mono = _queries[0];
					// if not already taken in the past
					if (!self.monoPendings[folderName] || self.monoPendings[folderName].indexOf(mono.NUMREQ) === -1) {
						self.monoPendings[folderName] = self.monoPendings[folderName] || [];
						self.monoPendings[folderName].push(mono.NUMREQ);

						// call X3 service ABATCH_STOP_RECURRENT to ask to stop all recurrent requests
						self._stopRecurrent(_, null, folderName);
					}

					// if no other request is pending on same folder, execute the mono
					if (queriesMap[folderName].filter(function(q) {
						return q.FLAG === 2;
					}).length === 0) {
						tracer.info && tracer.info("[" + self.code + "] No requests are pending on folder '" + folderName + "', then the mono '" + mono.NUMREQ + "' can be executed");
						monoToExecute.push(mono.NUMREQ);
					} else {
						tracer.info && tracer.info("[" + self.code + "] Some requests are pending on folder '" + folderName + "', then the mono '" + mono.NUMREQ + "' can't be executed now");
					}
			...
			debugTrace && debugTrace("monoPendings: " + JSON.stringify(Object.keys(this.monoPendings)));
			debugTrace && debugTrace("monoToExecute: " + JSON.stringify(monoToExecute));
			...
			queries = queries.filter(function(q) {
				return q.FLAG === 1 && q.EXENOW === true && (Object.keys(self.monoPendings).indexOf(q.DOSSIER) === -1 || monoToExecute.indexOf(q.NUMREQ) !== -1);
			});
			...
			for (var i = 0; i < queries.length; i++) {
				...
				if (srvConf) {
					try {
						...
						debugTrace && debugTrace("Put server configuration into the queue");
