...
var client;
var lockExpiration = 30 * 1000;
var lockRetryInterval = lockExpiration + 30 * 1000;
...
exports.registerBatchServer = function(_) {
	...
	var lock = orm.lock(_, "batch", lockExpiration, false);
	if (lock.status == "success") {
		startBatchServers(_);
	} else var timer = setInterval(function() {
		(_ => {
			lock = orm.lock(_, "batch", lockExpiration, false);
			if (lock.status == "success") {
				startBatchServers(_);
				clearInterval(timer);
			}
...
function _subscribeBatchServer(_) {
	...
	tracer.debug && tracer.debug("Subscribe batch server: tenantId='" + globals.context.tenantId);
	client = getBroker(_).subscribe(_, "batch", function(_, message) {
		var diagnoses = [];
		var batchSrvInst;
		try {
			var db = adminHelper.getCollaborationOrm(_);
			tracer.info && tracer.info("Proceed with message: tenantId='" + globals.context.tenantId + "', db=" + db.db.databaseName + ", message=" + JSON.stringify(message));
