...
test("temporarily disabled", 1, function() {
  ok(true);
});
return; // will fix and re-enable later
...
module("JediFile");
asyncTest("TC2001 (csv file)",2,function(_) {
  var sTC2001 = {
     "$name":"sTC2001",
     "$properties": {
      "operations"   :{
  ...
  var rflow  = new JediFile(__dirname + "/../fixtures/TC2001.csv",'r');
...
asyncTest("TC2002 (csv file)",3,function(_) {
  var sTC2002 = {
     "$name":"sTC2002",
     "$properties": {
      "operations"   :{
  ...
  var rflow  = new JediFile(__dirname + "/../fixtures/TC2002.csv",'r');
...
asyncTest("TC2003 (csv file)",2,function(_) {
  var sTC2003 = {
     "$name":"sTC2003",
     "$properties": {
      "entete"   :{
  ...
  var rflow  = new JediFile(__dirname + "/../fixtures/TC2003.csv",'r');
...
asyncTest("TC2004 (afb120 file)",function(_) {
  var sAFB120 = jsonfile.read(_, __dirname + "/../../lib/prototypes/AFB120.json");
  
  var rflow = new JediFile(__dirname + "/../fixtures/TC2004.afb120", 'r');
  
  var bankStatement = rflow.parse(_, sAFB120);
  
  var solde = 0;
  for (var i in bankStatement.statements) {
  	solde += bankStatement.statements[i].movement.amount;
  }
  var balance = bankStatement.oldBalance.amount + solde;
  strictEqual(bankStatement.newBalance.amount,balance,"balance="+balance);
  start();
});
