...
var JediFile = function(_file, _mode, _options) {
	/*dbg*/
	tracer && tracer.log("new JediFile(", _file, ",", _mode, ")");
	this.file = _file;
	this.mode = _mode;
	...
	/*dbg*/
	tracer && tracer.log("file stream", this.file, " encoding:", this.encoding);
	if (_mode == 'w') {
		/*dbg*/
		tracer && tracer.log("createWriteStream:", this.file);
		this.wStream = fs.createWriteStream(this.file);
		this.writableStream = ez.devices.node.writer(this.wStream);
	}
	if (_mode == 'r') {
		/*dbg*/
		tracer && tracer.log("createReadStream:", this.file);
		this.rStream = fs.createReadStream(this.file);
		this.readableStream = ez.devices.node.reader(this.rStream);
	}
};
...
JediFile.prototype.parse = function(_, _structure) {
	var jedi = new Jedi(this.options);
	/*dbg*/
	tracer && tracer.log("JediFile.parse");
...
JediFile.prototype.write = function(_, _value, _structure) {
	/*dbg*/
	tracer && tracer.log("JediFile.write");
	...
	/*dbg*/
	tracer && tracer.log("JediFile.write buffer:" + buffer);
	var encoding;
