...
exports.entity = {
	...
	$services: {
		...
		asyncChangeLastName: {
			$method: "PUT",
			$isMethod: true,
			$title: "Async change last name",
			$invocationMode: "async",
			$capabilities: "abort",
			$execute: function(_, context, instance) {
				var maxLoop = 100;
				if (context.tracker) context.tracker.phase = "Running";
				while (maxLoop--) {
					if (!context.tracker) break;
					if (context.tracker.abortRequested) {
						console.log("abort requested");
						context.tracker.acceptAbort();
						instance.$addDiagnose("info", "aborted");
						break;
					}
					setTimeout(_, 10);
				}
				instance.lastName(_, "asyncChangedLastName");
			}
		}
	},
	$facets: {
