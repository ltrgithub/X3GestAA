		...
		query:function(_, request, response, qs) {
			...
			callback:function(_, request, response, qs) {
				...
				function consoleWriter() {
					var id=0;
					return ez.devices.generic.writer(function(_, tweet) {
						tweet && console.log((++id)+" "+JSON.stringify(tweet));
					});
				};
				...
				try {
					...
					var src = function($params){
							...
							function nextTweet(_) {
								if(tweets && t < tweets.length) return tweets[t++];
