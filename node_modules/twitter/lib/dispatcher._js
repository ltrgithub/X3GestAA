		...
		query:function(_, request, response, $qs) {
			...
			callback:function(_, request, response, $qs) {
				...
				try {
					...
					var params = {q:qs.keywords};
					if(qs.since_id) params.since_id = qs.since_id;

					console.log("count:"+qs.count);
					...
					var src = function($params){
							...
							function nextTweet(_) {
								if(tweets && t < tweets.length) return  {tweet:tweets[t++],keywords:qs.keywords,entity:qs.entity,key:qs.key};
								...
								return {tweet:tweets[t++],keywords:qs.keywords,entity:qs.entity,key:qs.key};
							}
					...
					src.limit(qs.count).transform(alchemy()).pipe(_, dst);
