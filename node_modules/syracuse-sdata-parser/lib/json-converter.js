...
function _left(where) {
	...
	switch (where.type) {
		...
		case "function":
			switch (where.value) {
				case "upper":
					result = "upper(" + _left(where.children[0]) + ")";
					break;
				default:
					result = where.value;
			}
...
function _createFilter(where) {
	...
	//
	config.tracer.debug && config.tracer.debug("mongodbDbHandle.create filter enter: ", where);
	if (!where) return {};
	//
	var result = {};
	switch (where.type) {
							...
							if (oper === "between") {
								...
								switch (oper) {
									...
									case "like":
										...
										right = right.replace(/([\\\^\$\.\(\)\[\]\{\}\*\?\+])/g, "\\$1");
										if (right.charAt(0) === "%") right = right.substr(1);
										else right = "^" + right;
										if (right.charAt(right.length - 1) === "%") right = right.substr(0, right.length - 1);
										else right += "$";
	...
	config.tracer.debug && config.tracer.debug("create filter exit: ", result);
	return result;
}
...
function _createSdataFilterFromJson(jsonWhere) {
	...
	return Object.keys(jsonWhere).map(function(k) {
		...
		return v ? "(" + k + " eq " + v + ")" : undefined;
	}).filter(function(item) {
		return item != undefined;
	}).join(" and ");
}

exports.sdataToJson = function(where, options) {
