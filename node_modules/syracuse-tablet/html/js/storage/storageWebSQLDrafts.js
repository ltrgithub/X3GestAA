...
var _DRAFT_INSERT = "INSERT INTO sdata_drafts (id, endpoint, representation, comment, status, dataSet, restoredContext, saveLink, title, creation_date) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
var _DRAFT_READ_LIST = "SELECT id, endpoint, representation, comment, status, title, creation_date FROM sdata_drafts";
		...
		_draftSave: function(ctx) {
			...
			if (!ctx || !ctx.id) {
				...
				self._draftDelete(ctx.id).then(function() {
					return self._executeSql(_DRAFT_INSERT, [ctx.id, ctx.endpoint, ctx.representation, ctx.comment, ctx.status, JSON.stringify(ctx.dataSet), JSON.stringify(ctx.restoredContext), JSON.stringify(ctx.saveLink), ctx.title, ctx.creation_date]);
				}).then(function() {
