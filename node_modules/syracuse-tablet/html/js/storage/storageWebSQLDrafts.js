...
var _DRAFT_INSERT = "INSERT INTO sdata_drafts (id, endpoint, representation, comment, status, reason, dataSet, restoredContext, title, creation_date) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	function() {
		...
		_readDraftInfo: function(item) {
			...
			try {
				return {
					...
					title: item.title || "",
					creation_date: this._formatDate(item.creation_date)
				};
			} catch (e) {
		...
		_draftSave: function(ctx) {
			...
			return self._draftDelete(ctx.id).then(function() {
				return self._executeSql(_DRAFT_INSERT, [ctx.id, ctx.endpoint, ctx.representation, ctx.comment, ctx.status, ctx.reason, JSON.stringify(ctx.dataSet), JSON.stringify(ctx.restoredContext), ctx.title, ctx.creation_date])
					.then(function() {
