...
exports.Klass = utils.defineClass(
	...
	Base, {
		 ...
		 * Resolves with object store or rejects
		 */
		//		_tmplObjectStorex: function(readOpt) {
		//			var self = this;
		...
		templateOperation: function(op) {
			log && log("templateOperation", op)
			var promise;
			switch (op) {
				case "save":
					promise = this._tmplSave(arguments[1], true);
					break;
				case "delete":
					promise = this._tmplDelete(arguments[1]);
					break;
					break;
				case "deleteAll":
					promise = this._tmplDeleteAll(arguments[1], arguments[2]);
					break;
				case "read":
					promise = this._tmplRead(arguments[1]);
					break;
				case "update":
					promise = this._tmplUpdate(arguments[1], arguments[2]);
					break;
				case "query":
					promise = this._tmplQuery(arguments[1], arguments[2], arguments[3]);
					break;
				case "readall":
					promise = this._tmplQuery(arguments[1]);
					break;
				case "count":
					promise = this._tmplCount();
					break;
				default:
					promise = $.smReject(storageInterface.buildErrResult("Unknown template operation[" + op + "]"));
					break;
			}
			return promise;

			//			.then(function(res){
			//				log && log("templateOperation OK", op)
			//				return res;
			//			}).fail(function(e){
			//				log && log("templateOperation KO", op)
			//				return e;
			//			})
		}
	});