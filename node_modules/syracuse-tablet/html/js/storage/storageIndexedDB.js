...
var _storageInterfaceIndexedDBClass = utils.defineClass(
	...
	storageInterface.StorageInterface, {
		...
		/**
		 * Simplestorage is used internally for test
		 * Stores key/data
		 *
		 */
		_simpleStorageReject: function(deferred, event) {
			deferred.reject(storageInterface.buildErrResult("database error", event));
		},
		simpleStorageWrite: function(id, value) {
			var deferred = $.Deferred();
			if (!id) {
			...
			var self = this;
			try {
				var tx = self.db.transaction(["simpleStorage"], READ_WRITE);
				var objectStore = tx.objectStore("simpleStorage");
				var request = objectStore.index("read_write_one").openCursor(IDBKeyRange.only(id));
				var doInsert = function() {
					var reqt = objectStore.add({
						"id": id,
						"data": value
					});
					reqt.onerror = function(event) {
						log && log("simpleStorageWrite insert", id, "ko");
						self._simpleStorageReject(deferred, event);
					};
					reqt.onsuccess = function(event) {
						log && log("simpleStorageWrite insert", id, "ok", value);
				...
				self._simpleStorageReject(deferred, e);
			}
			return deferred.promise();
		},
		simpleStorageRead: function(id) {
			var deferred = $.Deferred();
			if (!id) {
				deferred.resolve(null);
				return;
			...
			try {
				...
				request.onsuccess = function(event) {
					try {
						var cr = event.target && event.target.result && event.target.result.value;
						log && log("simpleStorageRead", id, "ok", cr);
						deferred.resolve(cr ? cr.data : null);
					} catch (e) {
						self._simpleStorageReject(deferred, e);
					}
				};
				request.onerror = function(event) {
					log && log("simpleStorageRead", id, "ko");
					if (event.target.errorCode == IDBDatabaseException.NOT_FOUND_ERR) {
						deferred.resolve(null);
					} else {
						self._simpleStorageReject(deferred, event);
		...
		simpleStorageRemove: function(id) {
			var deferred = $.Deferred();
			if (!id) {
			...
			try {
				...
				request.onsuccess = function(event) {
					try {
						...
						if (cr) {
							cr["delete"]();
							log && log("simpleStorageRemove", id, "ok");
							deferred.resolve(true);
						} else {
							log && log("simpleStorageRemove", id, "ko");
							deferred.resolve(false);
						}
					} catch (e) {
						self._simpleStorageReject(deferred, e);
