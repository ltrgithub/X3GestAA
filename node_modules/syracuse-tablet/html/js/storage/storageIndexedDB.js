...
var _objectStores = {
	"sdata": {
		...
		"indexes": {
			"read_write_one": {
				"key": "key_collection_context",
				"options": {
					"unique": true
				}
			},
			"read_write_all": {
				"key": "collection_context",
				"options": {
					"unique": false
				}
			}
	...
	storageInterface.StorageInterface, {
		...
		put: function(opts) {
			...
			try {
				...
				var key = [opts.$key, opts.$collection, context].join("_");
				...
				var doInsert = function() {
					var obj = {
						...
						"key": opts.$key,
						"key_collection_context": key,
						"collection_context": [opts.$collection, context].join("_"),
						"data": opts.$data,
					};
				...
				request.onsuccess = function(event) {
					try {
						...
						if (!cr) {
							...
							deferred.resolve(storageInterface.buildResult(storageInterface.StatusCodes.OK, cr.data));
						}
					} catch (e) {
		...
		query: function(opts) {
			...
			try {
				...
				var key = [opts.$collection, context].join("_");
				...
				request.onsuccess = function(event) {
					try {
						...
						if (cr) {
							...
							deferred.resolve(storageInterface.buildResult(storageInterface.StatusCodes.OK, results));
						}
					} catch (e) {
						deferred.reject(storageInterface.buildResult(storageInterface.StatusCodes.ERROR, null, "error reading from database", e));
					}
