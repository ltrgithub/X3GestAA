...
var _Klass = utils.defineClass(
	...
	Base, {
		 ...
		 */
		_objectStore: function(type, readOpt) {
			var self = this;
			...
			try {
				...
				_logError('_objectStore - ' + _objName(type), e);
				deferred.reject(e);
			}
		 ...
		 */
		_getObject: function(type, id, fail) {
			var self = this;
			var deferred = $.Deferred();
			self._objectStore(type, READ_ONLY).then(function(objectStore) {
				var request = objectStore.get(id);
				request.onsuccess = function(event) {
					try {
						var result = request.result && request.result.data ? JSON.parse(request.result.data) : null;
						deferred.resolve(result);
						log && log("_getObject - " + _objName(type), "OK");
					} catch (e) {
						log && log("_getObject - " + _objName(type), "Error parsing data");
						if (fail) {
							deferred.reject({
								$diagnoses: [{
									$message: "_getObject - " + _objName(type) + " - Error parsing data"
								}]
							});
						} else {
				...
				request.onerror = function(event) {
					_logError("_getObject - " + _objName(type), event);
					if (fail) {
						deferred.reject({
							$diagnoses: [{
		 ...
		 * fail = true to reject if error
		 */
		_getAllObjects: function(type, fail) {
			var self = this;
			var deferred = $.Deferred();
			self._objectStore(type, READ_ONLY).then(function(objectStore) {
				...
				request.onsuccess = function(event) {
					...
					if (cursor) {
						try {
							...
							log && log("_getAllObjects - " + _objName(type), "Error parsing data");
						}
						cursor.continue ();
					} else {
						log && log("_getAllObjects - " + _objName(type), "OK");
						deferred.resolve(result);
					}
				};
				request.onerror = function(event) {
					_logError("_getAllObjects - " + _objName(type), event);
					if (fail) {
						deferred.reject({
							$diagnoses: [{
								$message: _objName(type) + " - IndexedDB.openCursor error",
		 ...
		 * fail = true to reject if error
		 */
		_putObject: function(type, id, data, fail) {
			var self = this;
			var deferred = $.Deferred();
			if (!data || !id) {
				_logError("_putObject - " + _objName(type), "Invalid parameters");
				if (fail) {
					deferred.reject({
						$diagnoses: [{
							$message: "_putObject - " + _objName(type) + " - Invalid parameters"
						}]
					});
				} else {
					deferred.resolve(null);
				...
				self._objectStore(type, READ_WRITE).then(function(objectStore) {
					var _put = function(created) {
						...
						request.onsuccess = function(event) {
							log && log("_putObject - " + _objName(type), "OK");
							deferred.resolve(data, created);
						};
						request.onerror = function(event) {
							_logError("_putObject - " + _objName(type), event);
							if (fail) {
								deferred.reject({
									$diagnoses: [{
		 ...
		 * fail = true to reject if error
		 */
		_deleteObject: function(type, id, fail) {
			var self = this;
			var deferred = $.Deferred();
			self._objectStore(type, READ_WRITE).then(function(objectStore) {
				var request = objectStore["delete"](id);
				request.onsuccess = function(event) {
					log && log("_deleteObject - " + _objName(type), "OK");
					deferred.resolve(true);
				};
				request.onerror = function(event) {
					_logError("_deleteObject - " + _objName(type), event);
					if (fail) {
						deferred.reject({
							$diagnoses: [{
		 ...
		 * fail = true to reject if error
		 */
		_deleteAllObjects: function(type, id, fail) {
			var self = this;
			var deferred = $.Deferred();
			self._objectStore(type, READ_WRITE).then(function(objectStore) {
				var request = objectStore.clear();
				request.onsuccess = function(event) {
					log && log("_deleteAllObjects" + _objName(type), "OK");
					deferred.resolve(true);
				};
				request.onerror = function(event) {
					_logError("_deleteAllObjects" + _objName(type), event);
					if (fail) {
						deferred.reject({
							$diagnoses: [{
		 ...
		 * fail = true to reject if error
		 */
		_countAllObjects: function(type, fail) {
			var self = this;
			var deferred = $.Deferred();
			self._objectStore(type, READ_ONLY).then(function(objectStore) {
				var request = objectStore.count();
				request.onsuccess = function(event) {
					log && log("_countAllObjects" + _objName(type), "OK", request.result);
					deferred.resolve(request.result);
				};
				request.onerror = function(event) {
					_logError("_countAllObjects" + _objName(type), event);
					if (fail) {
						deferred.reject({
							$diagnoses: [{
		...
		_wrkcpyRead: function(id) {
			return this._getObject("w", id, false);
		},
		/**
		...
		_wrkcpyDelete: function(id) {
			return this._deleteObject("w", id, false);
		},
		/**
		 * Resolve with true/false
		 */
		_wrkcpyDeleteAll: function() {
			return this._deleteAllObjects("w", false);
		},
		/**
		...
		_wrkcpyUpdate: function(dataSet) {
			...
			return this._putObject("w", dataSet.$uuid, dataSet, false);
		},
		...
		_draftReadList: function() {
			return this._getAllObjects("d", false);
		},
		/**
		 * Resolve with data/null
		 */
		_draftRead: function(id) {
			return this._getObject("d", id, false);
		},
		/**
		 * Resolve with  true/false
		 */
		_draftDelete: function(id) {
			var self = this;
			return self._deleteObject("d", id, false).then(function(result) {
		...
		_draftSave: function(ctx) {
			...
			var self = this;
			return self._putObject("d", ctx.id, ctx, false).then(function(data, created) {
				if (created) {
					self._draftNotifyChanged();
				}
				return data;
		...
		_draftCount: function(notify) {
			return this._countAllObjects("d", false).then(function(count) {
				if (notify) {
