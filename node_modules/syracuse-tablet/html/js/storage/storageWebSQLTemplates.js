...
var _Klass = utils.defineClass(
	function() {
		...
		_tmplQuery: function(representation, endpoint) {
			var self = this;
			return self._executeSql(_TMPL_QUERY)
		...
		_tmplRead: function(id) {
			var self = this;
			return self._executeSql(_TMPL_READ, [id])
				.then(function(result) {
					if (!result || !result.rows || result.rows.length == 0) return null;
					log && log("_tmplRead", id, "OK");
					return self._tmplReadInfo(result.rows.item(0));
				}).fail(function(status) {
		...
		_tmplDelete: function(id) {
			var self = this;
			return self._executeSql(_TMPL_DELETE, [id]).then(function(result) {
		...
		_tmplDeleteAll: function(representation, endpoint) {
			var self = this;
			return self._executeSql(_TMPL_DELETE_ALL).then(function(result) {
		...
		_tmplCount: function(endpoint, representation) {
		...
		templateOperation: function(op) {
			switch (op) {
				case "save":
					return this._tmplSave(arguments[1]);
				case "delete":
				...
				case "query":
					return this._tmplQuery(arguments[1], arguments[2]);
				case "count":
					return this._tmplCount(arguments[1], arguments[2]);
				default:
