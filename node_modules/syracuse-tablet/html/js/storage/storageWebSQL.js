...
var _tables = [
	...
	"data text " +
	")",
	"DROP INDEX IF EXISTS sdata_pk",
	"CREATE UNIQUE INDEX IF NOT EXISTS sdata_pk ON sdata (key, collection, context)"
];
	...
	storageInterface.StorageInterface, {
		...
		put: function(opts) {
			...
			try {
				...
				self._executeSql("SELECT * FROM sdata where key = ? and collection = ? and context = ?", params)
					.then(function(result) {
						if (result.rows.length > 0) {
							return self._executeSql("DELETE FROM sdata where key = ? and collection = ? and context = ?", params);
						}
					}).
				then(function() {
					params.push(JSON.stringify(opts.$data));
					return self._executeSql("INSERT INTO sdata (key, collection, context, data) values (?, ?, ?, ?)", params);
				}).
				then(function() {
		...
		query: function(opts) {
			...
			try {
				...
				self._executeSql("SELECT * FROM sdata where collection = ? and context = ?", params)
					.then(function(result) {
						var ret = [];
						try {
		...
		remove: function(opts) {
			...
			try {
				...
				var context = JSON.stringify(opts.$context);
				var params = [opts.$key, opts.$collection, context];
				self._executeSql("DELETE FROM sdata where key = ? and collection = ? and context = ?", params)
					.then(function(result) {
						try {
		...
		_openDB: function() {
			...
			try {
				...
				self.db = openDatabase(self.databaseName, self.databaseVersion, 'Database', self.databaseSize);
				self._readCurrentVersion().then(function(version) {
					log && log("Current version: " + version);
					if (version === self.databaseVersion) {
						deferred.resolve(storageInterface.buildResult(storageInterface.StatusCodes.OK));
					} else {
