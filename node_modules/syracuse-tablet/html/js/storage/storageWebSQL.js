...
var _sqlInterfaces = [require('syracuse-tablet/html/js/storage/storageWebSQLSData'),
	require('syracuse-tablet/html/js/storage/storageWebSQLDrafts')
];

//Increase this number to upgrade database to schema described in _objectStores
var _databaseVersion = 17;
...
var _storageInterfaceWebSQLClass = utils.defineClass(
	...
	storageInterface.getStorageInterface(), {
				...
				self.db.transaction(function(tx) {
					try {
						...
						tx.executeSql(sql, params, function(tx, result) {
							...
							deferred.reject(storageInterface.buildErrResult("DB error initializing database: " + _sqlErrorToMessage(error)));
						});
					} catch (e) {
						deferred.reject(storageInterface.buildErrResult("error running SQL statement", e));
					}
				}, function(error) {
					deferred.reject(storageInterface.buildErrResult("DB transaction error: " + _sqlErrorToMessage(error) + ". Try to reload the page"));
				});
			} catch (e) {
				deferred.reject(storageInterface.buildErrResult("error running SQL statement", e));
			}
		...
		_createTables: function() {
			...
			var nbExec = 0;
			var tables = {};
			_sqlInterfaces.forEach(function(module) {
				$.extend(tables, module.tables);
			});
...
function _sqlErrorToMessage(error) {
	return "CODE: " + error.code + " MESSAGE: " + error.message;
}
exports.getStorageInterface = function() {
	var klass = _storageInterfaceWebSQLClass;
	_sqlInterfaces.forEach(function(module) {
		utils.addClassMembers(klass, module.members);
	});
	return _storageInterfaceWebSQLClass;
};
