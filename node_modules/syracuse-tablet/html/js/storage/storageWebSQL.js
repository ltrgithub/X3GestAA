...
var _tables = [
	...
	// Storage table
	"DROP TABLE IF EXISTS simple_storage",
	"CREATE TABLE simple_storage ( " +
	"id text, " +
	"data text " +
	")",
	"DROP INDEX IF EXISTS simple_storage_pk",
	"CREATE UNIQUE INDEX simple_storage_pk ON simple_storage (id unique)",
];
...
var _storageInterfaceWebSQLClass = utils.defineClass(
	...
	storageInterface.StorageInterface, {
		...
		simpleStorageRead: function(id) {
			...
			try {
				var db = self.db;
				self._executeSql("SELECT * FROM simple_storage where id = ?", [id])
					.then(function(result) {
						if (result && result.rows && result.rows.length > 0) {
							try {
								deferred.resolve(JSON.parse(result.rows[0].data));
							} catch (e) {
								deferred.resolve(null);
							}
						} else {
		...
		simpleStorageWrite: function(id, data) {
			var deferred = $.Deferred();
			if (!id) {
				deferred.resolve(null);
				return;
			...
			try {
				...
				self.simpleStorageRemove(id).then(function() {
					self._executeSql("INSERT INTO simple_storage (id, data) values (?, ?)", [id, data])
						.then(function() {
				...
				deferred.reject(storageInterface.buildErrResult("error writing data", e));
			}
			return deferred.promise();
		},
		simpleStorageRemove: function(id) {
			...
			var self = this;
			try {
				self.simpleStorageRead(id)
					.then(function(data) {
						if (!data) {
							deferred.resolve(false);
						} else {
							self._executeSql("DELETE FROM simple_storage where id = ?", [id])
								.then(function() {
