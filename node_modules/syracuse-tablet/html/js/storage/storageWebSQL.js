	...
	storageInterface.StorageInterface, {
		...
		query: function(opts) {
			...
			try {
				...
				self._executeSql("SELECT * FROM sdata_objects o INNER JOIN sdata_meta m ON (o.id = m.id and m.collection = ? and m.endpoint = ? and m.context = ?)", params)
					.then(function(result) {
						var ret = [];
						try {
		...
		_writeMetaData: function(metaData) {
			...
			try {
				...
				if (!metaData.$id) {
					...
					self._executeSql("INSERT INTO sdata_meta (key, collection, endpoint, context, id, etag, last_read) values (?, ?, ?, ?, ?, ?, ?)", [
						metaData.$key,
						metaData.$collection,
						metaData.$endpoint,
