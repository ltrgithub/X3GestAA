...
exports.rejectHttpResult = function(title, result, url) {
	if (result && result.responseJSON && result.responseJSON.$diagnoses) {
		...
		// Sometimes responseText contains html
		return $.smReject({
			$title: title,
			$diagnoses: [{
				$message: (result.descrHttpStatus || "Http request error") + " - Status=" + result.status,
				$stackTrace: result.responseText ? result.responseText.replace(/<\/(\w*)>|<(\w*)>/g, "&lt;$1$2&gt;") : null
			}]
		});
	}
	return $.smReject({
