...
exports.Layout = utils.defineClass(Layout, null, {
	...
	createLayout: function($layout, $category, $title) {
		...
		if ($category && $title) {
			layoutHeader.textContent = $title;
		}
 ...
 *
 * if ($item.$bind) { var widgetDom = document.createElement("div");
 * widgetDom.className = "sm-widget thumbnail"; return widgetDom; }
 *
 * if ($item.$layout) { return createLayout($item.$layout); } }
 *
 * function createLayout($layout, $category, $title) { var layoutSlot =
 * document.createElement("div"); layoutSlot.className = "sm-layout-slot";
 *
 * var layoutHeader = document.createElement("header"); layoutHeader.className =
 * "sm-layout-header"; layoutSlot.appendChild(layoutHeader);
 *
 * var layoutCore = document.createElement("div"); layoutCore.className =
 * "sm-layout-core"; layoutSlot.appendChild(layoutCore);
 *
 * if ($category && $title) { layoutHeader.textContent = $title; }
 *
 * var $layoutType = $layout.$layoutType || "stack";
 *
 * if ($layout.$items) {
 *
 * if ($layoutType == "row") { utils.toggleClass(layoutCore, "row", true); if
 * (!$layout.$widthsDesktop || !$layout.$widthsTablet || !$layout.$widthsMobile) {
 * throw new Error("Columns widths not specified"); } // desktop only for test
 * purpose var $widthsDesktop = $layout.$widthsDesktop.split(",");
 *
 * var isRowLayout = true; }
 *
 * for (var ii = 0, jj = $items.length; ii < jj; ii++) {
 *
 * var coreItem = document.createElement("div"); if (isRowLayout) {
 * utils.toggleClass(coreItem, "col-md-" + $widthsDesktop[ii], true); }
 *
 * coreItem.appendChild(createItem($layout.$items[ii])); } }
 *
 * return layoutSlot; }
 */