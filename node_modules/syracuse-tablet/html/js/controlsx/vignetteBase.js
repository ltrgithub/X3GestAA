...
var Base = require('syracuse-tablet/html/js/controlsx/ctrlBase').Klass;
...
var _Klass = utils.defineClass(function(controller, article, prototype) {
	...
	buildHtml: function(controllerDao, buildOptions) {
		var context = {};
		context.id = this.id;
		return this.templates.execSync("tmplVignette", context).replace(/\n/g, "");
	},
	/**
	 * Loaded by dashboard.afterRender Get page html and update $elmt innerHtml
	 */
	load: function() {
		var self = this;
		var state;
		...
		var rejectError = function(e) {
			...
			log && log(msg, e);
			// !! TODO - deferred - this deferred is not correctly rejected -
			// dash-board reject callback is not called - To test
			alert(msg);
			deferred.reject(e);
		};
		try {
			state = self._getPageInfo();
			if (state.name.smEndsWith("$query")) {
	...
	_getPageInfo: function() {
		...
		if (gadget.$type === "$representation") {
			...
			var page = pageHelper.getPageInfo(name, {
				vignette: true
			});
			if (page)
				return page;
		}
		return reqPage.getPageInfo("vignetteerror", {
			vignette: true
		});
	}
});
