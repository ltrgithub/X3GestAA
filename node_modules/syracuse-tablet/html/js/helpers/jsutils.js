...
var _convertToDiagnoses = function(e, html) {
	...
	if (typeof e === "string") {
		...
		if (e.$exception && (_isError(e.$exception) || _isException(e.$exception))) {
			// $exception returned by storage in addition to $message
			res.$diagnoses.push({
				$message: html ? exports.stringToHtml(e.$exception.message) : e.$exception.message,
				$stackTrace: _cleanStack(e.$exception.stack, null, html),
				$severity: "error"
			});
		}
		return res;
