 ...
 */
var _getOpenLinkUrl = function(ctx) {
	var url = exports.getCurrentUrl();
	// We open open_link.html that will remove the context from url and store it in sessionStorage (see _setUrlContext)
	var file = url.file;
	url.file = "open_link.html";
	url.path = url.path.replace(file, url.file);
	if (ctx) {
		if (!url.query) url.query = {};
		url.query.context = JSON.stringify(ctx);
	}
...
var _getUrlContext = function() {
	...
	if (sessStorage) {
		context = sessStorage.getItem("__urlContext__");
		sessStorage.removeItem("__urlContext__");
	}
	if (!context) {
		// If no context check url context
		var url = exports.getCurrentUrl();
		context = url.quey ? url.query.context : null;
		if (!context) return;
	}
	try {
	...
	if (!context || !$.isPlainObject(context) || $.isEmptyObject(context)) return null;
	if (!context.endpoint || !context.currentState || !context.applicationName) {
		if (globals.getModal()) {
			globals.getModal().actionResult(_localeModule.text("urlcontext.bad"), _localeModule.text("urlcontext.cantopen"));
		}
...
var _setUrlContext = function() {
	...
	try {
		var context = url.query && url.query.context;
		if (sessStorage && context && context.trim().length > 0) {
			// Stores the context
			sessStorage.setItem("__urlContext__", context);
			// Remove it from url
...
var _convertToDiagnoses = function(e, html) {
	...
	if (typeof e === "string") {
		...
		return {
			$diagnoses: [{
				$severity: "error",
				$message: html ? exports.stringToHtml(e) : e
			}]
		};
		...
		return {
			$diagnoses: [{
				$message: html ? exports.stringToHtml(e.message) : e.message,
		...
		diag.$message = e.message || e.$message || e.$title || "No error message";
		if (html) {
			diag.$message = exports.stringToHtml(diag.$message);
		}
		diag.$severity = "error";
		var detail = e.detail;
		if (detail) {
...
exports.getUrlContext = _getUrlContext;
exports.setUrlContext = _setUrlContext;
