...
exports.updateMobileDetect = function() {
	var agt = navigator.userAgent;
	_globals.mobileDetect = window.MobileDetect && new window.MobileDetect(agt);
	// our own detection (poor)
	var match = (agt.match(/(iphone|ipod|ipad)/) || agt.match(/(android)/i) || agt.match(/(iemobile)/) || agt.match(/iphone/i) || agt.match(/ipad/i) || agt.match(/ipod/i) || agt.match(/blackberry/i) || agt.match(/bada/i));
	_globals.isMobile = match != null && match.length > 0;
	if (!_globals.isMobile && _globals.mobileDetect) {
		// A better one
		match = _globals.isMobile = _globals.mobileDetect.mobile();
		_globals.isMobile = match != null && match.length > 0;
	}
...
exports.isMobileWrapper = function() {
	return window.smNativeInterface != null;
};
...
/**
 * Just to optimize the managementof events
 */
exports.isDebugDeviceSimlator = function() {
	return exports.isMobile() && exports.isMobileWrapper() === false;
};
exports.mustHideDevFeatures = function() {
...
exports.setUserProfile = function(up) {
	if (!up || !up.user || !up.selectedRole || !up.selectedLocale || !up.selectedEndpoint) {
		throw new Error("Unexpected " + (up ? "bad" : "null") + " user profile");
	}
	exports.setEndpoint(up.selectedEndpoint.application + "." + up.selectedEndpoint.contract + "." + up.selectedEndpoint.dataset);
...
exports.noConnectionModal = function() {
	...
	return _globals.modal.noConnectionModal();
};