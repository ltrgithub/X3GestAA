...
(function() {
	try {
		_baseLocation.host = window.location.protocol + "//" + window.location.host;
		var path = window.location.pathname.split('/').splice(0, 2).join('/');
		_baseLocation.withPath = _baseLocation.host + path;
		_baseLocation.requirePath = _baseLocation.host + "/requireJS" + path;
		_baseLocation.htmlRoot = _baseLocation.withPath + "/html";
		_stackRegexp = {
			r1: new RegExp(_baseLocation.withPath, "g"),
			r2: new RegExp(_baseLocation.requirePath, "g")
		};
	} catch (e) {
		alert(e);
		return null;
	...
	o.forEach(function(diag) {
		...
		if (d.exception) {
			...
			if (d.exception.stack) {
				d.stackTrace = _cleanStack(d.exception.stack, null, html);
			}
			delete d.exception;
		}
 ...
 */
exports.baseLocation = _baseLocation;
exports.UUID = _UUID;
...
exports.isTestEnvironment = _isTestEnvironment;
