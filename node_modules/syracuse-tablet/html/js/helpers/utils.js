...
var _sanitizeArticle = function(article) {
	...
	function _walk(value) {
		...
		Object.keys(value).some(function(key) {
			if (key != "$title" && key !== "$titleOrig") {
				if ($.type(value[key]) !== "string") {
					_walk(value[key]);
				}
			} else {
				if (value[key] != "" && !_isExpression(value[key])) {
					delete value[key];
				}
			}
		});
...
/**
 * Call open_link.html page taht opens the index page without the contex in url
 */
exports.getOpenLinkUrl = function(ctx) {
...
/**
 * Context has been stored by open_link.html page and removed from url (see getOpenLinkUrl)
 * {endpoint:, applicationName:, history[state]}
 */
exports.getUrlContext = function() {
	var context = window.sessionStorage.getItem("__urlContext__");
	window.sessionStorage.removeItem("__urlContext__");
	if (!context) return;
	try {
