...
exports.isResolve = function(value) {
	return !/{.*}/.test(value)
};
...
exports.setLocalization = function($localization, uuid, text) {
	var localization = exports.getLocalization($localization, locale.getCurrentLocale());
	delete($localization[locale.getCurrentLocale()]);
	$localization[locale.getCurrentLocale().toLowerCase()] = localization || {}
	$localization[locale.getCurrentLocale().toLowerCase()][uuid] = text;
	return $localization;
};
exports.getLocalization = function($localization, lang) {
	if (!$localization || !lang) return null;
	var localization = ($localization[lang] && !$.isEmptyObject($localization[lang])) ? $localization[lang] : null;
	return (localization || $localization[lang.toLowerCase()])
};
exports.getLocalizedText = function($localization, lang, uuid) {
	var localization = exports.getLocalization($localization, lang);
	return (localization ? localization[uuid] : "");
};
