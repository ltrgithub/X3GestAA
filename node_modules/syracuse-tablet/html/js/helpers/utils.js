 ...
 *  	dataset: a DaoSdata object (with value method to chain with a parent) or a simple JSON object
...
var _execExpression = function(expression, dataset, prototype, level) {
	...
	if (level === 0 && expression.indexOf("{") < 0) {
		var value = dataset.value ? dataset.value(expression) : dataset[expression];
	...
	var res = expression.replace(_exprRegExp,
		function(match, prop) {
			...
			var val = dataset.value ? dataset.value(prop) : dataset[prop];
