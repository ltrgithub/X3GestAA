...
var NumberFormatter = utils.defineClass(function($type, $format) {
	...
	formatValue: function(value, editMode) {
		...
		if (editMode == true && ((+value) === 0)) {
			// Valora - we return 0 instead of 0.000
			return 0;
		}
	...
	parseValue: function(value, errors) {
		...
		try {
			var rawValue = numberFormat.parse(value, this.format);
			// Valora - We format the value before in order to get the right number of decimals
			// Eg: 123,45354444444445 -> 123,4535 if $scale=4 -> 123.4535
			// I don't want to modify numberFormat.parse that doesn't format the decimal part according to $scale	
			// We should review/fix the formatter based on tests driven 
			value = this.formatValue(rawValue);
			return numberFormat.parse(value, this.format);
		} catch (e) {
			errors.push(locale.text("field.invalidInput", [this.format]));
			return value;
