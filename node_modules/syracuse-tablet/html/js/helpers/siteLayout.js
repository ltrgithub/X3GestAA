...
function _layoutMainContainers() {
	...
	var forcePreview = _internalParameters.pageParameters.forcePreview,
		$$wrapper;
	if (forcePreview) {
		var wrapBorder = 30;
		// Wrapp the application into device container to better display the devise structure with round corners...
		$$wrapper = $('<section class="s-m-device-wrapper"></section>"');
		$$wrapper.css({
			top: (appView.top - wrapBorder) + "px",
			left: (appView.left - wrapBorder) + "px",
			width: (appView.width + 2 * wrapBorder) + "px",
			height: (appView.height + 2 * wrapBorder) + "px"
		});
		$$app.wrap($$wrapper);
	...
	if (forcePreview && _internalParameters.pageParameters.scale !== 1) {
		$$app.css({
