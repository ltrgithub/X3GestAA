...
/*
 * Triggered by browser window resize - only on desktop environment
 * This event is trigered also on mobile devices even if we can't change the size
 * We have the case on android/nexus when we go from fullscreen display to non-fullscreen (with address bar) display (click onstart button)
 */
exports.onWindowResize = function(evt, opts) {
	...
	if (utils.isDesktop() && _internalParameters.pageParameters.forcePreview !== true) {
	...
	// Just do the resize - Orientation is done by onOrientationchanged
...
exports.onWindowResizeInteral = function(evt, opts) {
	...
	if (params && params.device && params.forcePreview) {
		...
		typ = typ ? _deviceClassToType(typ.deviceClass) : "";
...
function _layoutMainContainers(resizeEvt, opts) {
	...
	console.log("orientationChanged", info.orientationChanged, "deviceChanged", info.deviceChanged);
...
var _getDeviceType = function() {
	return _deviceClassToType(_getDeviceClass());
};
var _deviceClassToType = function(deviceClass) {
