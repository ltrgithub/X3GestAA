 ...
 *
 */
var _isOnDesktop = false;
try {
	var _url = jsutils.getCurrentUrl();
	_isOnDesktop = _url.query && (_url.query.testNativeWrapper === "true" || _url.query.isDesktop === "true");
} catch (e) {}
/**
 * _isOnDesktop allows to use the test device function provided by the browser
 * With this feature the user agent is the one of a mobile device but the orientationChanged event is not triggered (just the size changes)
 * We add the query param to force to calculate the orientation on window resize
 */
var _isDeviceSimlator = function() {
	return !utils.isDesktop() && _isOnDesktop === true;
};
...
exports.onWindowResize = function(evt, opts) {
	...
	if (_isDeviceSimlator() || (utils.isDesktop() && _internalParameters && _internalParameters.pageParameters.forcePreview !== true)) {
		...
		if (_isDeviceSimlator() && (_lastUserAgent == null || _lastUserAgent !== navigator.userAgent.toLowerCase())) {
...
function _layoutMainContainers(resizeEvt, opts) {
	...
	var appView;
	if (_isDeviceSimlator()) {
		// Force no preview if we simulate a device from the browser (chrome)
		pageParams.forcePreview = false;
	}
	...
	// Force deviceChanged if device changed or first notification or preview changed - deviceChanged modified the behavior of the client
	info.deviceChanged = opts.deviceChanged || _lastParams == null || pageParams.forcePreview != _lastParams.forcePreview;
	// If device has changed we force oreinetationChange
	info.orientationChanged = opts.deviceChanged || opts.orientationChanged;
