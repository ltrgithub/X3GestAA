...
function _layoutMainContainers() {
	...
	var appView;
	var forcePreview = _internalParameters.pageParameters.forcePreview === true;
	...
	if (forcePreview) {
		...
		//Center
		appView.left = Math.max((applicationWidth - wrapW * _internalParameters.pageParameters.scale) / 2, 0);
		appView.top = Math.max((applicationHeight - wrapH * _internalParameters.pageParameters.scale) / 2, 0);
		...
		if (pageParams.scale !== 1) {
			wrapCss["transform"] = "scale(" + _internalParameters.pageParameters.scale + ", " + _internalParameters.pageParameters.scale + ")",
	...
	notifications.publish("sm.main.layout.changed", {
		application: appView
	}, _internalParameters.pageParameters.orientation, _getDeviceClass());
