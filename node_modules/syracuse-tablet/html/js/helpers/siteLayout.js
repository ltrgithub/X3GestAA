...
exports.onWindowResize = function(evt, opts) {
	if (!evt) return;
	// Updates mobile/desktop detector - Eg: Used when we test with the mobile simularor on chrome (user agent changes when we select a device)
	globals.updateMobileDetect();
	opts = opts || {};
	opts.orientationChanged = false;
	opts.deviceChanged = false;
	// serverLog.log("onWindowResize");
	if (globals.isDebugDeviceSimlator() || (globals.isDesktop() && _internalParameters && _internalParameters.pageParameters.forcePreview !== true)) {
		...
		if (_lastUserAgent == null || _lastUserAgent !== navigator.userAgent.toLowerCase()) {
...
function _layoutMainContainers(resizeEvt, opts) {
	...
	var appView;
	if (globals.isDebugDeviceSimlator()) {
