...
function _checkOrientationChange(evt) {
	var orientation = _getWindowOrientation();
...
/*
 * Triggered by browser window orientationchanged
 */
exports.onWindowOrientation = function(evt, opts) {
	serverLog.logObject(evt);
	if (!evt || evt.orientation == null) return;
	if (_checkOrientationChange(evt)) {
		opts = opts || {};
		opts.resizeEvent = evt;
		_layoutMainContainers(opts);
	}
};
/*
 * Triggered by browser window reisze
 */
exports.onWindowResize = function(evt, opts) {
	...
	if (_checkOrientationChange(evt) || utils.isDesktop()) {
		_layoutMainContainers(opts);
	}
};
