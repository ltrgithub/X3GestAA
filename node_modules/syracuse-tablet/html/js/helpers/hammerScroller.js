...
var _velocityToDist = function(velocity) {
	...
	var v = Math.abs(Math.round(velocity * 7));
...
var _Klass = utils.defineClass(function hammerScroller($$elmt, options, parentMgr, $$nativeScroll) {
	...
	getValMin: function() {
		...
		// 8 pixel more
		return Math.min(this.valMax - x + this.viewRect[prop] - 8, this.valMax);
	},
	/**
	 * Adjust the scroller after modification of DOM to ma this.$elmt visible
	 * By example after an action which hide lines we need to call this method to ajdjust scrollin because the visible are can be outside the viewRect
	 */
	adjust: function() {
		// Panel has not been initialized
		if (this.viewRect == null) return;
		var elmtRect = uiRect.elmtRect(this.$$elmt);
		if (elmtRect.contains(this.viewRect)) return;
		var prop = this.dir == "v" ? "height" : "width";
		this.val = this.viewRect[prop] - elmtRect[prop];
		this.refresh();
	},
	// Move to beginning 
	 ...
	 */
	checkViewRect: function(viewRect) {
		var reset = true;
