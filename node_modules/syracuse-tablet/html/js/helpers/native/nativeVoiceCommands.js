...
var _VoiceCommandsMgr = function(bookmarks) {
	...
	this.add = function(mobileCommand) {
		...
		this.availableCommands.push(mobileCommand);
		return true;
	};
	...
	this.executeVoiceCommand = function(data) {
		if (this.isReady) {
			...
			nativeTestUtils.callAppNativeInterface("voiceCommands", "executeVoiceCommand", cmd);
			return;
		}
		// Stores the command
...
if (!(window.smVoiceJS)) {
	window.smVoiceJS = {
		...
		executeVoiceCommand: function(data) {
			if (!_voiceCommandsManager) return;
			_voiceCommandsManager.executeVoiceCommand(data);
		}
	};
}
/**
 ...
 */
exports.createCommandFromPage = function(pinnedPage) {
	if (!pinnedPage || !pinnedPage.title || !pinnedPage.voiceCommandContext) return null;
	return {
		type: "openBookmark",
