 ...
 * Returns the login token (reconnectToken) and the context to restore (context)
 * {
 * 		reconnectToken:
 * 		context:
 * }
...
exports.getReconnectContext = function() {
	if (true) {
		try {
			var ctx = localStorage.getItem("reconnectcontext");
			return ctx ? JSON.parse(ctx) : null;
		} catch (e) {
			return null;
		}
	}
	return window.smNativeApp && window.smNativeApp.getReconnectToken && window.smNativeApp.getReconnectContext();
};
exports.setReconnectContext = function(context) {
	context = context != null ? JSON.stringify(context, null, 2) : null;
	if (true) {
		return localStorage.setItem("reconnectcontext", context);
	}
	return window.smNativeApp && window.smNativeApp.setReconnectToken && window.smNativeApp.setReconnectContext(context);
};
