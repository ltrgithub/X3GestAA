...
var nativeTestX3WUPApp = require('syracuse-tablet/html/js/helpers/native/test/nativeTestX3WUPApp');
...
if (!(window.smWrapperJS)) {
	window.smWrapperJS = {
		...
		backButtonPressed: function() {
			return nativeHelpers.callAppNativeInterface("X3WUPApp", "backButtonPressed") ? "true" : "false";
		},
		...
		applicationClosing: function() {
			var currentHistoryStack = nativeHelpers.callAppNativeInterface("X3WUPApp", "applicationClosing");
...
exports.supports = function(capability) {
	if (capability === "X3WUPApp") {
		return nativeExtCall.isSupported("smNativeApp", "NativeWrapperSupported");
	}
	return false;
};
...
exports.getReconnectToken = function() {
	var token = nativeExtCall.callMethod("smNativeApp", "GetReconnectToken");
	if (token) {
		try {
...
exports.setReconnectToken = function(token) {
	nativeExtCall.fireMethod("smNativeApp", "SetReconnectToken", token);
};
/**
 * Management of reconnection
...
exports.getReconnectContext = function() {
	return nativeExtCall.callMethod("smNativeApp", "GetReconnectContext");
};
/**
 * Management of reconnection
...
exports.setReconnectContext = function(context) {
	nativeExtCall.fireMethod("smNativeApp", "SetReconnectContext", context);
};
exports.configServer = function() {
	nativeExtCall.fireMethod("smNativeApp", "ConfigServer");
};
exports.selectServer = function() {
	nativeExtCall.fireMethod("smNativeApp", "SelectServer");
};
...
exports.notifLoaded = function() {
	nativeExtCall.fireMethod("smNativeApp", "WebappLoaded");
};
...
exports.notifUnloaded = function(historyContext) {
	...
	if (window && window.smNativeApp && window.smNativeApp.onWebappUnloaded) {
		return window.smNativeApp.onWebappUnloaded(historyContext ? JSON.stringify(historyContext, null, 2) : null);
	}
};
...
exports.notifStartFail = function(msg) {
	nativeExtCall.fireMethod("smNativeApp", "WebappLoadFail", msg);
};

exports.init = function(testMode) {
	if (window && window.smNativeApp) return;
	if (!testMode) return;
	log && log("Creating native X3WUPApp test wrapper");
	window.smNativeApp = nativeTestX3WUPApp.create();
};