...
if (!(window.smWrapperJS)) {
	window.smWrapperJS = {
		...
		backButtonPressed: function() {
			return nativeTestUtils.callAppNativeInterface("X3WUPApp", "backButtonPressed") ? "true" : "false";
		},
		...
		applicationClosing: function() {
			var currentHistoryStack = nativeTestUtils.callAppNativeInterface("X3WUPApp", "applicationClosing");
...
exports.supports = function(capability, testMode) {
	if (capability === "X3WUPApp") {
		return testMode ? true : window ? window.smNativeApp != null : false;
	}
	return false;
};
 ...
 * The value returned by this function is set before by "applicationClosing"
 */
exports.getHistoryContext = function() {
	try {
 ...
 * Management of reconnection
 */
exports.setHistoryContext = function(context) {
	try {
		window.smNativeApp && window.smNativeApp.setHistoryContext && window.smNativeApp.setHistoryContext(context);
	} catch (e) {
	}
};
...
exports.notifStartFail = function(msg) {
	...
	return window &&
		window.smNativeApp &&
		window.smNativeApp.onWebappLoadFail &&
		window.smNativeApp.onWebappLoadFail(msg);
};
