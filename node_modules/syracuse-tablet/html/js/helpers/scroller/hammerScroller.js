...
var _Klass = utils.defineClass(function HammerScroller($$elmt, options, parentMgr) {
	...
	makeVisible: function($$e) {
		if (!$$e || $$e.length == 0 || !this.isInitialized() || this.isVisible($$e)) return;
		if (this.slowingDown) {
			this._stopSlowingDown();
			// Last click is avoided - The click evt that is always triggered after press a event
			this.cancelClick = true;
	...
	getScrollMin: function() {
		...
		var scale = globals.getSiteLayout().getTransformScale();
		var viewProp = this.viewRect[prop];
		if (scale != 1 && scale > 0) {
			x = x / scale;
			viewProp = viewProp / scale;
		}
		// 30 pixel more
		return Math.min(this.scrollMax - x + viewProp - 30, this.scrollMax);
	},
