	...
	if ((forceReadFromServer || uuid) && globals.isOnline()) {
		return sdataCommonResources.queryWelcomeDashboard(uuid)
			...
			.then(function(dashboardMeta) {
				if (dashboardMeta && dashboardMeta.$gadgets) { // at the very first time no dashboard
					var meta = globals.getMetaData();
					return meta.getApplications(false)
						.then(function(apps) {
							_cleanApplications(dashboardMeta);
							apps.forEach(function(appInfo) {
								if (appInfo.applicationName !== "$welcomeApplication") { // main app is not a gadget
									var bind = "$welcome_" + appInfo.applicationName;
									dashboardMeta.$gadgets[bind] = _gadget(appInfo);
									dashboardMeta.$vignettes[bind] = _vignette(appInfo);
								}
							});
...
function _getRoles(currentRoles) {
	var ActiveRoles = {};
	if (currentRoles) {
...
function _setMetaWelcomeDashboard(dashboard) {
	...
	if (welcomeDashboard) {
		...
		welcomeDashboard.description = dashboard.description;
...
function _defaultDashboard(dashboard) {
	...
	if (dashboard.$article) {
		if (dashboard.$article.$items) {
			var groups = [];
			dashboard.$article.$items.forEach(function(g) {
				if (g && (globals.showTestApplications() || g.$groupType !== "sageAppli")) {
					groups.push(g);
				}
			})
			dashboard.$article.$items = groups;
		}
		var meta = globals.getMetaData();
		...
		step = _updateDefaultDashboard(dashboard, {
			"defaultArticle": true
		}); // empty dashboard, set a default dashboard
	};
	return step.then(function(dashboard) {
		dashboard.$views = dashboard.$views || [];
		if ((dashboard.$views.length > 0 && !dashboard.$views[0].$isPersonal) || dashboard.$views.length === 0) {
			// save dashboard as dashboard user because user have no own dashboard
			_unLocalizeHubGroups(dashboard.$article);
			dashboard.$roles = [];
			dashboard.description = locale.text("welcome.mydashboard")
			dashboard.$factoryOwner = "";
			dashboard.$views.forEach(function(view) {
				view.$selected = false
			});
			// create the new view with no uuid
			dashboard.$views.unshift({
				$description: dashboard.description,
				$factoryOwner: dashboard.$factoryOwner,
				$isPersonal: true,
				$selected: true
			});
...
function _cleanApplications(dashboard) {
	Object.keys(dashboard.$gadgets).forEach(function(key) {
		if (!dashboard.$gadgets[key].pinPageId) { // not a pinned tile or page
			delete dashboard.$gadgets[key];
			delete dashboard.$vignettes[key];
		}
	});
	return dashboard;
};
...
function _updateDefaultDashboard(dashboard, opts) {
	...
	return $.smResolve()
		...
		.then(function() {
			return _addApps(dashboard, appList, voiceBookmarks, opts);
		})
		.then(function() {
...
function _isPinnedPage(pinPageId) {
	...
	// Some pages like login will not have an id, so better do not ask to init the welcome dashboard
	// since the whole application is not up and running yet, this can cause several issues like
	// no endpoint or user profile in general
	if (pinPageId == null) {
		return $.smResolve(false);
	}

	return globals.getMetaData().getWelcomeDashboard().then(function(dashboard) {
		if (dashboard.$factoryOwner) return 0; /*Template*/
		if (!pinPageId) return 1;
		if (!dashboard || !dashboard.$gadgets) return 1;
		var isPinnedPage = Object.keys(dashboard.$gadgets).some(function(key) {
...
function _getPinnedPageContext(pinPageId) {
	var self = this;
	if (!pinPageId) return $.smResolve(false);
	return globals.getMetaData().getWelcomeDashboard().then(function(dashboard) {
		if (!dashboard || !dashboard.voiceCommandContext) return null;
		var context = null;
		Object.keys(dashboard.$gadgets).some(function(key) {
			if (dashboard.$gadgets[key].pinPageId === pinPageId) {
				var voiceCommandContext = dashboard.voiceCommandContext[key]
				if (voiceCommandContext && voiceCommandContext.currentState) {
					context = voiceCommandContext.currentState.savedContext;
					return true;
				} else {
					return false;
				}
...
exports.getPinnedPageContext = _getPinnedPageContext;
...
function _addGroup(dashboard, text) {
	var personal = !dashboard.$factoryOwner || dashboard.$factoryOwner === "";
	var uuid = personal ? null : utils.UUID(); //if perso no translation
	var titleContent = uuid ? "{@" + uuid + "}" : text;
	var grp = _hubGroup(_groupTypes.quickLinks, titleContent);
	if (uuid) {
		dashboard.$article.$localization = dashboard.$article.$localization || {};
		utils.setLocalization(dashboard.$article.$localization, uuid, text)
	}
	return grp;
}
...
function _addPinnedPage(pageData) {
	...
	return globals.getMetaData().getWelcomeDashboard().then(function(dashboard) {
		...
		if (!quikLinksGroup) {
			var grp = _addGroup(dashboard, locale.text("welcome.myquicklinks"));
...
function _addPinnedTile(article, prototype, localization) {
	...
	return globals.getMetaData().getWelcomeDashboard().then(function(dashboard) {
		...
		if (!pinnedTilesGroups) {
			var grp = _addGroup(dashboard, locale.text("welcome.pinnedTiles"))
			var quikLinksGroup = _getHubGroup(dashboard.$article, _groupTypes.quickLinks);
			if (quikLinksGroup) {
...
function _unLocalizeHubGroups($article) {
	function _walk(items) {
		if (!items) return;
		items.forEach(function(item) {
			if (item.$layoutType === "hubGroup") {
				var uuid = item.$title.match(/\{\@(\S+)\}/);
				item.$title = localization[(uuid && uuid[1])]
				utils.removeLocalization($article.$localization, (uuid && uuid[1]));
			}
			_walk(item.$items);
		});
	};
	var localization = utils.getArticleLocalization($article);
	if (!localization) return $article;
	_walk($article.$items);
	return $article;
};
//if group empty : delete the group
function _removePinnedTile($article, $bind, groupType) {
	var grp = _getTileHubGroup($article, $bind);
	if (!grp) return;
...
function _getDashboard(dashboard) {
	...
	delete cleaneddashboard.roles;
	delete cleaneddashboard.$pageInfo;
	delete cleaneddashboard.$update;
	delete cleaneddashboard.description;
	return cleaneddashboard;
}

function _addApps(dashboard, apps, voiceBookmarks, opts) {
	var apps;
	var groups = {
	...
	for (var p in groups) {
		if (globals.showTestApplications() || p !== "sageAppli") {
...
function _vignette(appInfo) {
	var vignette = {
		"$uuid": "$welcome_" + appInfo.applicationName
	}
	return vignette;
}
...
function _checkTilesStatus(dashboard) {
	...
	return meta.getApplications(forceReadApps)
		.then(function(apps) {
			_cleanApplications(dashboard)
			apps.forEach(function(appInfo) {
				var bind = "$welcome_" + appInfo.applicationName;
				dashboard.$gadgets[bind] = _gadget(appInfo);
				dashboard.$vignettes[bind] = _vignette(appInfo);
				_checkTileStatus(appInfo, _getTileByBind(dashboard.$article, bind));
			});
			return dashboard;
		})
...
function _writeWelcomeDashboard(dashboard, localOnly) {
	if (dashboard.$views[0].$isPersonal && dashboard.$views[0].$selected) { // if personal is selected (be careful $views is not an array) 
	...
	return storage.put({
		...
		if (!localOnly && result.$status === storageModule.StatusCodes.OK && globals.isOnline()) {
			...
			var data = {
				...
				"description": dashboard.description || locale.text("welcome.mydashboard"),
				"clientId": "",
...
function _saveEntityWelcomeDashboard(data) {
	return sdataCommonResources.updateWelcomeDashboard(data)
		...
		.fail(function(e) {
			modal.error(locale.text("dashboard.welcome.updating.error"), e, function() {
				return data;
			});
		})
};

function _writeCommonWelcomeDashboard(dashboard, opts) { //factory or group : template)
	var currentView;
	Object.keys(dashboard.$views).some(function(key) {
		if (dashboard.$views[key].$selected) {
			currentView = dashboard.$views[key];
			return true;
		}
	});
	var uuid = opts ? opts.uuid : (currentView ? currentView.$uuid : null);
	var roles = dashboard.roles;
	var cleanedDashboard = _getDashboard(dashboard);
	var data = {
		"uuid": uuid,
		"user": {
		...
		"role": {
			"$uuid": globals.getSeletedRoleUuid(), // the current user role
		},
		"roles": roles, // the destination roles
		"description": dashboard.description || "Dashboard template",
		"clientId": "",
		"content": cleanedDashboard
	};
	return _saveEntityWelcomeDashboard(data).then(function(data) {
...
function _deleteCommonWelcomeDashboard(uuid) { //factory or group : template)
	return sdataCommonResources.deleteWelcomeDashboard(uuid)
		.then(function(data) {
			return data
		})
		.fail(function(e) {
			modal.error(locale.text("dashboard.welcome.deleting.error"), e, function() {
				return {};
			});
		})
};
...
function _getTileHubGroup($article, $bind) {
	var grp;
...
function _removeTileByBind($article, $bind) {
	var remove;
	...
	function _walk(items) {
		...
		items.forEach(function(item) {
			if (remove) return
			if (item.$layoutType === "tile" && !remove) {
				if (item.$items[0].$bind === $bind) {
					remove = {
						collection: items,
						idx: idx
	...
	_walk($article.$items);
	if (remove) {
		remove.collection.splice(remove.idx, 1);
	}
	return $article;
};
