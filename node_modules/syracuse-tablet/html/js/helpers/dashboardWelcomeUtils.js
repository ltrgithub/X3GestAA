...
function _updateDashboard(dashboard) {
	...
	return _getWelcomeDashboards(true).then(function(dashboard) {
...
function _getWelcomeDashboards(forceReadFromServer) {
	if (forceReadFromServer && globals.isOnline()) {
		return sdataCommonResources.queryWelcomeDashboards().
		then(function(dashboards) {
			...
			var welcomeDashboard = (dashboards[0] && dashboards[0].content);
			welcomeDashboard.$views = views;
...
function _defaultDashboard(dashboard) {
	...
	return step.then(function(dashboard) {
		if (JSON.stringify(globals.getApplication().welcomeDashboard) !== JSON.stringify(dashboard)) {
			return _writeWelcomeDashboardUser(dashboard, true);
		} else {
...
function _addPinnedPage(pageData) {
	...
	return _writeWelcomeDashboardUser(dashboard).then(function() {
		//dashboard saved
	});
};
...
function _removePinnedPage(pageId, groupType) {
	...
	delete dashboard.voiceCommandContext[gadgetId];
};
...
function _writeWelcomeDashboardUser(dashboard, localOnly) {
	...
	return storage.put({
		...
		console.log("_writeWelcomeDashboardUser " + (new Date().getTime() - t));
