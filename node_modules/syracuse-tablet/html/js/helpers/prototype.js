...
var _addFacetLink = function(info, facet) {
	...
	switch (facet) {
		...
		case "$create":
			...
			break;
		default:
			console.log("Facet " + facet + " not implemented yet");
			info.title = "???";
			break;
	}
};
...
var _Prototype = utils.defineClass(function(json, parent) {
	 ...
	 * filter 		visible				actions displeyd in footer and visible ppage
	 *				hidden				actions displeyd in panel
	...
	getLinks: function(controllerDao, filter, options) {
		...
		var result = [];
		// TODO manage $links properties attributes in data $isHidden....
		if ($links && Object.keys($links).length > 0) {
			$.each($links, function(name, value) {
				try {
					...
					value = value || {};
					console.log("getLinks - invalid link - Title: " + self.checkExpression(value.$title || 'no title') + "\n\t" + (value.$url || 'no url'));
				}
			});
		}
		return result;
