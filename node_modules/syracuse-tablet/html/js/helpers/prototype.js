...
var _Prototype = utils.defineClass(function(json, parent) {
	...
	initRefField: function(protoField, name) {
		...
		if (item.$myLookupUrl == null) {
			this._setMyLookupUrl(item);
		}
	},
	// #7811 Init $$$myLookupUrl - Trick to resolve $lookup urls with keys - Adda where clause
	// Tested with 3 level of key ( LAM)
	// TODO - Finalize
	_setMyLookupUrl: function(item) {
		item.$myLookupUrl = item.$links && item.$links.$lookup ? item.$links.$lookup.$url : "";
		// Put false here to disabled the adding of where clause
		if (item.$myLookupUrl && item.$key && item.$value != item.$key) {
			var key = item.$key.split('~');
			var where = [];
			key.forEach(function(k) {
				if (!k || k == item.$value) return;
				// check if it's a field  #7830
				if (!k.smStartsWith('{') && !k.smEndsWith('}')) return;
				where.push("(" + k.replace(/[{}]/g, "") + "%20eq%20%27" + k + "%27)");
			});
			if (where.length > 0) {
				if (item.$myLookupUrl.indexOf("?") < 0) {
					item.$myLookupUrl += "?";
				} else {
					item.$myLookupUrl += "&";
				}
				item.$myLookupUrl += "where=" + where.join("%20and%20");
			}
		}
	},
	/**
