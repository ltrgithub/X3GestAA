...
var _Prototype = utils.defineClass(function(json, parent) {
	...
	getFieldTypeForValue: function(name) {
		var protoField = this.property(name);
		var item = protoField && protoField.$item;
		// Type of bind field - Same as $type for non reference type
		return item && item.$refValueType ? item.$refValueType : protoField.$type;
	},
	/**
	...
	getRepDotFacet: function() {
		var self = this;
		return self.data("$type").split(".").shift().join(".");
	},
	isValid: function() {
