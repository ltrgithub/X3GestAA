...
exports.validate = function(value, constraints, errors) {
	if (value == null) {
		if (constraints.$isMandatory) return errors.push(locale.text("real.valMan"));
		if (!constraints.$isNullable) return errors.push(locale.text("real.valNull"));
		return;
	}
	if (typeof value !== "number" || isNaN(value)) {
		errors.push(locale.text("real.valNumber"));
		return;
	}
	...
	})) errors.push(locale.text("real.valEnum"));
	if (constraints.$minimum != null) {
		if (constraints.$minimumCanEqual) {
			if (value < constraints.$minimum) errors.push(locale.text("real.valGe", constraints.$minimum));
		} else {
			if (value <= constraints.$minimum) errors.push(locale.text("real.valGt", constraints.$minimum));
		}
	}
	if (constraints.$maximum != null) {
		if (constraints.$maximumCanEqual) {
			if (value > constraints.$maximum) errors.push(locale.text("real.valLe", constraints.$maximum));
		} else {
			if (value >= constraints.$maximum) errors.push(locale.text("real.valLt", constraints.$maximum));
		}
	}
};
