...
exports.validate = function(value, constraints, errors) {
	if (value == null) {
		if (constraints.$isMandatory) return errors.push(locale.text("valMan"));
		if (!constraints.$isNullable) return errors.push(locale.text("valNull"));
		return;
	}
	if (!(value instanceof Time)) {
		errors.push(locale.text("valTime"));
		return;
	}
};
...
exports.parse = function(str, format) {
	...
	if (format == null) {
		...
		// hh:mm
		if (str.length != 5 && str.length != 8) throw new Error(locale.text("badFormat", str));
...
function _parse(str, format) {
	...
	_walkFormat(format, {
		literal: function(literal) {
			if (str.substring(j, j + literal.length) !== literal) throw new Error(locale.text("wrongFormat", literal, str.substring(j)));
			j += literal.length;
		},
		H: function(arg) {
			switch (arg) {
				...
				case 2:
					...
					break;
				default:
					throw new Error(locale.text("badHour", format.substring(j))); // replaced i with j
			}
		},
		h: function(arg) {
			switch (arg) {
