...
exports.validate = function(value, constraints, errors) {
	if (value == null) {
		if (constraints.$isMandatory)
			return errors.push(locale.text("valMan"));
		if (!constraints.$isNullable)
			return errors.push(locale.text("valNull"));
		return;
	}
	if (!(value instanceof DateObj)) {
		errors.push(locale.text("valDate"));
		return;
	}

	// month value check
	if (!(value.month >= 1 && value.month <= 12)) {
		errors.push(locale.text("invMonth"));
		return;
	}

	// day value check
	if (!(value.day >= 1 && value.day <= _daysInMonth(value.year, value.month))) {
		errors.push(locale.text("invDay"));
		return;
	}
...
utils.defineClass(DateObj, null, {
	...
	at: function(time, millisecond) {
		throw new Error(locale.text("notInstalled"));
	},
...
function _parse(str, format) {
	...
	function parseInteger(max) {
		if (!/\d/.test(str[j])) {
			throw new Error(locale.text("notNumber", str.substring[j]));
		}
		var beg = j++;
	...
	_walkFormat(format, {
		literal: function(literal) {
			if (str.substring(j, j + literal.length) !== literal)
				throw new Error(locale.text("wrongFormat", literal, str.substring(j)));
			j += literal.length;
		},
		d: function(arg) {
			switch (arg) {
...
exports.parse = function(str, format) {
	if (str == null)
		throw new Error(locale.text("dateNull"));

	if (format == null) {
		// RFC 3339 by default -- very fast
		if (str.length != 10)
			throw new Error(locale.text("badFormat", str));
