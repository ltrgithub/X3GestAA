...
var _dateInfo = locale.getDateInfo;

exports.validate = function(value, constraints, errors) {
	if (value == null) {
		if (constraints.$isMandatory) return errors.push(locale.text("valMand"));
		if (!constraints.$isNullable) return errors.push(locale.text("valNull"));
		return;
	}
	if (!(value instanceof Datetime)) {
		errors.push(locale.text("valDT"));
		return;
	}

	// month value check
	if (!(value.month >= 1 && value.month <= 12)) {
		errors.push(locale.text("valMon"));
		return;
	}

	// day value check
	if (!(value.day >= 1 && value.day <= date.daysInMonth(value.year, value.month))) {
		errors.push(locale.text("valDay"));
		return;
	}
};
				...
				H: function(repeat) {
					switch (repeat) {
						...
						case 2:
							...
							break;
						default:
							throw new Error(locale.text("formatH", repeat));
					}
				},
				h: function(repeat) {
					switch (repeat) {
				...
				m: function(repeat) {
					switch (repeat) {
						case 2:
							...
							break;
						default:
							throw new Error(locale.text("formatm", repeat));
					}
				},
				s: function(repeat) {
					switch (repeat) {
						case 2:
							...
							break;
						default:
							throw new Error(locale.text("formats", repeat));
					}
				},
				t: function(repeat) {
					switch (repeat) {
						...
						case 2:
							...
							break;
						default:
							throw new Error(locale.text("formata", repeat));
					}
				},
				d: function(repeat) {
					switch (repeat) {
						...
						case 4:
							...
							break;
						default:
							throw new Error(locale.text("formatd", repeat));
					}
				},
				M: function(repeat) {
					switch (repeat) {
				...
				y: function(repeat) {
					switch (repeat) {
						...
						case 4:
							...
							break;
						default:
							throw new Error(locale.text("formaty", repeat));
					}
				},
				S: function(repeat) {
					if (repeat === 3) result += _pad3(self.millisecond);
					else new Error(locale.text("formatS", repeat));
				}
			});
			return result;
		}
...
function _parseIso(str) {
	...
	if (convertError) throw new Error(locale.text("noParse", str));
}
...
exports.parse = function(str, format) {
	if (str == null) throw new Error(locale.text("isNull"));
...
function _parse(str, format) {
	...
	function parseInteger(max) {
		if (!/\d/.test(str[j])) {
			throw new Error(locale.text("noNumber", str.substring[j]));
		}
		var beg = j++;
	...
	function parseName(names) {
		...
		throw new Error(locale.text("unknown", str.substring(j)));
	}

	_walkFormat(format, {
		literal: function(literal) {
			if (str.substring(j, j + literal.length) !== literal) throw new Error(locale.text("formatMis", literal, str.substring(j)));
			j += literal.length;
		},
		d: function(arg) {
			switch (arg) {
				...
				case 4:
					...
					break;
				default:
					throw new Error(locale.text("badDay", format.substring(j))); // replaced i with j
			}
		},
		M: function(arg) {
			switch (arg) {
		...
		y: function(arg) {
			switch (arg) {
				...
				case 4:
					...
					break;
				default:
					throw new Error(locale.text("badYear", format.substring(j))); // replaced i with j
			}
		},
		H: function(arg) {
			switch (arg) {
				...
				case 2:
					...
					break;
				default:
					throw new Error(locale.text("badHour", format.substring(j)));
			}
		},
		h: function(arg) {
			switch (arg) {
		...
		m: function(arg) {
			switch (arg) {
				case 2:
					...
					break;
				default:
					throw new Error(locale.text("badMin", format.substring(j)));
			}
		},
		s: function(arg) {
			switch (arg) {
				case 2:
					...
					break;
				default:
					throw new Error(locale.text("badSec", format.substring(j)));
			}
		},
		t: function(arg) {
			switch (arg) {
				...
				case 2:
					...
					break;
				default:
					throw new Error(locale.text("badMarker", format.substring(j)));
			}
		}
	});
	...
	if (timeMode != null || abbrTimeMode != null) {
		if (hour > 12) throw new Error(locale.text("badHourVal", hour, timeMode));
