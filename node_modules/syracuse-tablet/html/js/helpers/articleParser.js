	...
	function(page) {
		...
		parse: function(article, prototype, opts) {
			...
			try {
				...
				if (loc && article.$localization && article.$localization[globals.fallbackLocaleCode]) {
					loc.$fallback = $.extend(true, {}, article.$localization && article.$localization[globals.fallbackLocaleCode]);
				} else if (!loc) {
					if (article.$localization && article.$localization[globals.fallbackLocaleCode]) {
						loc = article.$localization && article.$localization[globals.fallbackLocaleCode];
					}
				}
