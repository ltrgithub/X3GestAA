...
var _resources = [];
_countries.forEach(function(ctry, idx) {
	_resources.push({
		"level": 1,
		"CRY": ctry.code,
		"$uuid": "uuid" + ctry.code,
...
exports.entity = {
	$entityName: "$localStatVEN42",
	$value: "{date}",
	$proto: {
		"$baseUrl": "/mobile1/x3/erp/SEEDTEST",
		"$url": "{$baseUrl}/STAT('VEN42')?representation=STAT~VEN42.$cube&role={$role}",
		"$title": "Sales by Region",
		"$type": "application/json; vnd.sage=syracuse.presentation=x3.erp.superv/STAT('VEN42')",
		"$methods": "GET",
		"codeStat": "VEN42",
		"$properties": {
			"datstart1": {
			...
			"$resources": {
				...
				"$item": {
					...
					"$links": {
						"levelDown": {
							"$url": "{$baseUrl}/STATS('VEN42')?representation=STATS~VEN42.$cube&{linkDown}",
							"$title": "Lower Level",
							"$method": "GET"
						},
						"levelUp": {
							"$url": "{$baseUrl}/STATS('VEN42')?representation=STATS~VEN42.$cube&{linkUp}",
							"$title": "Higher Level",
							"$method": "GET"
						}
					}
