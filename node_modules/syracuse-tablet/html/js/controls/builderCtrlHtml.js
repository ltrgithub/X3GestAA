...
var _BuilderCtrlHtml = utils.defineClass(
	function(ctrl) {
		...
		buildHtml: function(html, controllerDao, buildOptions) {
			...
			var property = this.prototype.property(this.article.$bind);

			var context = {};

			if (!this.article.$isTitleHidden) {
				context.$title = property.$title;
			}
			if (controllerDao) {
				if (!controllerDao.getDisplayValue) throw new Error("BuildHtml - dao is expected for data parameter");
			}
			context.value = controllerDao.getDisplayValue(this.article.$bind);

			context.id = this.id;
			//debugger;

			return this.ctrl.templates.execSync(uiUtils.getFieldTmplId(property.$type), context).replace(/\n/g, "");

			//uiUtils.getFieldTmplId(property.$type)
