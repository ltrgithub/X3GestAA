...
/**
 * Used by ctrlCubeCHart and ctrlArrayChart (control)
 * Opens chart detail page
 */
var _openChartDetailPage = function(control) {
	var attrs = {
		"data-control-id": control.id,
		"data-parent-id": control.controller.id,
		"data-nav": "chartdetail",
		"data-nav-target": "application"
	};
	eventListener.triggerNavigation(control.$$elmt, attrs);
};
/**
 * Used by ctrlCubeCHart and ctrlArrayChart (control)
 * Save detail article in control's article
 */
var _setChartDetailAuthoring = function(control, article) {
	control.article.$chartDetail = article;
 ...
 */
var _childPageGetPageInfo = function(control, typePage, currentPage) {
	var pageInfo = {
		home: false,
		type: typePage,
		cached: false,
	...
	var article = control.article.$chartDetail;
	if (!article) {
		var opt = {
			openedFromArrayChart: control.$type === "tablet/x-array-chart"
		};
		article = _getChartDetailArticle(opt, control.$bind, control.article);
	}
	var protoJson = $.extend(true, {}, control.controller.prototype.json);
	//todo : get title in case of ctrlArrayChart
	protoJson.$title = control.prototype.getDataByPath("$cube.$title") || locale.text("chartdetail.page.name");
	pageInfo.prototype = control.prototype.create(protoJson, true);
	...
	var parentVignette = control.controller.getParentVignette();
 ...
 * Return the page info proto/article for the chart detail page
 */
var _notifChartHosted = function(control) {
	// Notifies cell/tile to improve display when a chart is nested
	if (control.controller.isVignette && control.controller.getParentVignette()) {
		var parent = control.controller.getParentVignette().parent;
...
var _Klass = utils.defineClass(function CtrlCubeChart(controller, article, prototype, options) {
	...
	_setChartHeight: function(show) {
		...
		this.highchartObject.setSize(this._$$chartSlot.width(), height, false);
		if (show) {
	...
	_actChartDetail: function() {
		_openChartDetailPage(this);
	},
	childPageGetPageInfo: function(typePage) {
		return _childPageGetPageInfo(this, typePage);
	},
	setChartDetailAuthoring: function(article) {
		_setChartDetailAuthoring(this, article);
	},
	/**
	 * #6843
...
exports.arrayChartInterface = {
	openChartDetailPage: _openChartDetailPage,
	childPageGetPageInfo: _childPageGetPageInfo,
	setChartDetailAuthoring: _setChartDetailAuthoring,
	notifChartHosted: _notifChartHosted
};