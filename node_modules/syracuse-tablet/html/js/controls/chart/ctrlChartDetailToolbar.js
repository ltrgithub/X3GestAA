...
var _Klass = utils.defineClass(
	...
	function CtrlChartDetailToolbar($type, controller, options) {
		...
		_actToolbar: function(action) {
			var toolbarStat = this._toolbarInfoGet() || {};
			...
			if (action === "togglechart") {
				...
				toolbarStat["chartVisible"] = chart.$$elmt.is(":visible");
				this._toolbarInfoSet(toolbarStat, true);
				return;
			}
			if (action === "togglearray") {
				...
				uiUtils.triggerResizeInternal();
				toolbarStat["arrayVisible"] = array.$$elmt.is(":visible");
				this._toolbarInfoSet(toolbarStat, true);
				return;
			}
		},
		setToolbarStat: function() {
			var toolbarStat = this._toolbarInfoGet() || {};
			var chart = this.controller.getChart();
			var array = this.controller.getArray();
			if (toolbarStat.chartVisible !== undefined && !toolbarStat.chartVisible) {
				this._actToolbar("togglechart");
			};
			if (toolbarStat.arrayVisible !== undefined && !toolbarStat.arrayVisible) {
				this._actToolbar("togglearray");
			}
		},
		_toolbarInfoGet: function() {
			var info = this.controller.toolbarRead("toolbar", this);
			return info;
		},
		_toolbarInfoSet: function(info, refresh) {
			this.controller.toolbarWrite("toolbar", info, this, refresh);
		}
	});
