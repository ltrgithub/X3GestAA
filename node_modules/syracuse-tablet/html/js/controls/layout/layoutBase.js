...
var _Klass = utils.defineClass(function LayoutBase(controller, type, article, opts) {
	...
	buildTitle: function($$rootLyt, $$parent, controllerDao, destroyAfter) {
		this._ensureHeader();
		var value = this.getArticleText("$title");
		if (this.isEditable) {
			var uuid = this.getArticleTextUUID("$title");
			this.$$header.find("input.title").remove();
			this.$$header.prepend('<input readonly class="title" ' + ((uuid) ? ' data-old-uuid="' + uuid + '" data-old-value="' + value + '"' : '') + ' value="' + value + '"/>');

		} else {
			this.$$header.find("span.title").remove();
			this.$$header.prepend('<span class="title">' + value + '</span>');
		}
	},
