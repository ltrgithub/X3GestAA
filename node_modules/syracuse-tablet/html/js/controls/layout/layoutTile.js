...
var _Klass = utils.defineClass(function LayoutTile(controller, type, article, opts) {
	...
	buildHtml: function($$parent, controllerDao, buildOptions) {
		...
		self.$$elmt.attr("data-empty-tile", self.article.$items ? self.article.$items[0].$bind : "")
		self.$$body = self.$$elmt;
		self._setCellColorClass();
		self.buildChildrenHtml(self.$$body, controllerDao, buildOptions);
		if (self.isEmpty) {
			self._buildEmptyTile(self.$$body, buildOptions);
		}
	},
	_buildEmptyTile: function($parent, buildOptions) {
		$parent.append('<section class="s-m-icon"><div><div class="s-m-sage-app-icon ' + fontUtils.dashboardIcon("emptyTile") + '"></div></div></section>');
		$parent.append('<section class="s-m-title"><div>' + locale.text("welcome.dashboard.emptyTile") + '</div></section>')
