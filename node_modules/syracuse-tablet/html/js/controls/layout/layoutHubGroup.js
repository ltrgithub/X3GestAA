...
var _Klass = utils.defineClass(
	function LayoutHubGroup(controller, type, article, opts) {
		...
		_buildCellStructure: function(controllerDao, buildOptions) {
			...
			if (self.children) {
				self.children.forEach(function(layout) {
					//layout.children[0]._gadget._data.$title=layout.children[0]._gadget._data.$title.substr(0,10)+" "+index
					//layout.children[0]._gadget._data.icon="";
			...
			if (buildOptions.showAddTiles) {
				...
				self._addActionTile(opts);
			}
			// True if the group contains only links
			self._linksOnly = notLinksCpt === 0;
		},
		_addActionTile: function(opts) {
			var self = this;
			var cell = self._createCell(opts.size);
			cell.domItem = uiUtils.createDomElement("div", null, null, {
				"data-action": opts.action,
				"data-params": self.id
			}, self.$$elmt);
			self.cells.push(cell);
			var addTile = uiUtils.createDomElement("div", "s-m-tile-action s-m-color-tile-action medium", null, null, cell.domItem);
		...
		calculateLayout: function(options) {
			...
			self._width = 4 * self.hubGroupOptions.baseSize
