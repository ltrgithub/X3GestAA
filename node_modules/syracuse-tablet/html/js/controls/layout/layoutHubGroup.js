	...
	function LayoutHubGroup(controller, type, article, opts) {
		...
		calculateLayout: function(options) {
			...
			if (self.hubGroupOptions.orientation === "landscape") {
				self._calculateLayoutHorizontal(options);
			} else {
				self._calculateLayoutVertical(options);
			}
		 ...
		 */
		_calculateLayoutHorizontal: function(options) {
			var self = this;
			...
			self._sizes = {
				...
				full: {
					width: self.hubGroupOptions.baseSize * 4 + self.hubGroupOptions.padX * 3,
					height: self.cells.length == 1 ? options.maxHeight : self.hubGroupOptions.maxFullTileHeight
				},
				all: {
		 ...
		 * Calc tile positions in horizontal scrolling model
		 */
		_calculateLayoutVertical: function(options) {
			var self = this;
			...
			self._sizes = {
				...
				full: {
					width: self.cells.length == 1 ? options.maxWidth : self.hubGroupOptions.baseSize * 4 + self.hubGroupOptions.padX * 3,
