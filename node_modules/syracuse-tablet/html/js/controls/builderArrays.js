...
var _defDevSz = globals.getDeviceSize(true);
/**
...
var _getWidths = function(gridBuidlder, nbCells, maxCols, sz, justNum) {
	...
	for (var i = 0; i < nbCells; i++) {
		...
		sum += nb;
		self[cache].push(justNum === true ? nb : "col-" + sz.toLowerCase() + "-" + nb);
	}
	return self[cache];
};
/* Temporarily*/
...
var _getPropsToDisplay = function(gridBuilder) {
	var props = gridBuilder.ctrl.itemProto.data("$properties");
...
var _GridArticleBuilder = utils.defineClass(
	function(gridCtrl) {
		var self = this;
		Base.call(self, gridCtrl);
		self.maxCols = 12;
	}, Base, {
		buildHtml: function(html, controllerDao, buildOptions) {
			var self = this;
			if (self.article.$isTitleHidden !== true) {
				html.push("<p><b>" + self.prototype.property(self.article.$bind, "$title") + "</b></p>");
			}
		...
		buildTtlHtml: function(html, controllerDao, buildOptions) {
			...
			if (!self._titleHtml) {
				...
				var root = articleParser.article2Controls(self.ctrl.controller, info.article, info.proto);
				self._titleHtml = [];
				/* Generate Html */
				self._titleHtml.push(root.buildHtml(info.data, buildOptions).outerHTML);
		...
		buildContentHtml: function(html, controllerDao, buildOptions) {
			...
			if (!self._contentHtml) {
				var rows = self.ctrl.getRows(controllerDao);
				self._contentHtml = [];
				...
				rows.forEach(function(rowDao, idx) {
					...
					/* Create the control structure from article */
					var root = articleParser.article2Controls(self.ctrl.controller, article, self.ctrl.itemProto);
					/* Generate Html */
					self._contentHtml.push(root.buildHtml(rowDao, buildOptions).outerHTML);
				});
				/* Add article html to parent html array */
		...
		_getTtlInfos: function() {
			...
			for (var colIdx = 0; colIdx < keys.length; colIdx++) {
				if (nbCols < self.maxCols) {
					...
					res.data[key] = self.ctrl.itemProto.property(key, "$title") || self.ctrl.itemProto.property(key, "$subtitle") || key;
				}
			};
			...
			for (var colIdx = 0; colIdx < keys.length; colIdx++) {
				if (nbCols < self.maxCols) {
					...
					if (key === reqProto.rowIdxProperty) {
						var detLnk = self.ctrl.itemProto.data("$links");
						if (detLnk && detLnk.$details) {
							// Add info to generate <a> html tag
							fieldDescr.displayOptions.link = sdataUtils.getLinkInfo(detLnk.$details.$url, rowDao);
						}
					}
	...
	function(gridCtrl) {
		...
		buildTtlHtml: function(html, controllerDao) {
			...
			for (var idx = 0; idx <= keys.length; idx++) {
				...
				if (idx < self.maxCols) {
					...
					var p = self.ctrl.itemProto.property(key, "$title") || self.ctrl.itemProto.property(key, "$subtitle") || key;
					html.push(p);
					html.push('</div>');
				}
			};
		...
		buildContentHtml: function(html, controllerDao) {
			...
			rows.forEach(function(rowDao, idxRow) {
				...
				html.push(self.ctrl.id + "r-" + idxRow);
				html.push('">');
				for (var idxCell = 0; idxCell <= keys.length; idxCell++) {
					...
					if (idxCell < self.maxCols) {
						...
						if (idxCell === 0) {
							...
							if (detLnk && detLnk.$details) {
								html.push('<a href="#" ');
								html.push(sdataUtils.getLinkAttrs(detLnk.$details.$url, rowDao));
								html.push('>');
								detLink = true;
							}
						}
