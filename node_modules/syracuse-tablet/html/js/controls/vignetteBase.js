...
var _Klass = utils.defineClass(
	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		onMainPageResize: function(info, orientation, deviceClass) {
			...
			// Subscribes only if full page - No need to unsubscribe - notifications.subscribe prvents multiple subscriptions
		...
		_initActionDashboard: function(dstPage) {
			try {
				...
				if (dstPage.isChild) {
					...
					this._typeAction = dstPage.isMultiSelectionEnabled() ? "multiselection" : dstPage.hasFooterHeaderActions(true) ? "footerHeader" : null;
		...
		_doTogglePinVignette: function(show) {
			try {
				...
				this.controller.toggleHeaderFooter(this.getTypeActionControl(), controlToggle, status === "on");
		...
		_getActionControl: function(create) {
			// _actionControlsCache beacuse we can have multiple pages ina vignette
		...
		destroyActionControls: function() {
			if (!this._actionControlsCache) return;
			if (this.controller && this._actionControl) {
			...
			for (var p in this._actionControlsCache) {
				this._actionControlsCache[p].destroy();
			}
		...
		_actVignetteHome: function() {
			...
			return self._loadPage(state, null, "home").then(function(page, error) {
		...
		_actVignetteBack: function() {
			...
			return self._loadPage(self._stateStack[self._stateStack.length - 1], null, "back").then(function(page, error) {
				self._updateActionControl();
				return $.smResolve(page);
			});
		},
