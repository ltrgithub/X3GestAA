...
var _Klass = utils.defineClass(
	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		destroy: function() {
			...
			if (self._savedFooter) {
				self._savedFooter.destroy();
				self._savedFooter = null;
			}
			if (self._actionFooter) {
				self._actionFooter.destroy();
				self._actionFooter = null;
			}
			...
			self._pageLoader = new PageLoader({
				...
				succeeded: function(dstPage, pageToClose, parentNotifInfo, back, loadOptions) {
					...
					if (dstPage.state.options.$displayStyle === "$full") {
						...
						self._initActionFooter(dstPage);
						setTimeout(function() {
		...
		_initActionFooter: function(dstPage) {
			// TODO Check authoring
			if (!dstPage) return;
			var tile = this.getParentTile();
			if (!tile) return;
			if (this.$$actionIcon) {
				this.$$actionIcon = tile.toggleActionIcon("togglePinVignette");
				return;
			}
			if (!this._actionFooter) {
				var mgr = actionManager.create(dstPage, "footer", {
					enableMultiSelectionOnly: true
				});
				if (!mgr.hasLinks() && !dstPage.isMultiSelectionEnabled()) return;
				this._actionFooter = ctrlFactory.createVignetteFooter("footer", this);
			}
			this.$$actionIcon = tile.iconActionAdd("togglePinVignette", fontUtils.getPinVignetteIcon("off"), this);
		...
		_doTogglePinVignette: function(id) {
			if (!this._actionFooter || (id != this.id && !this._isPinned)) return;
			var tile = this.getParentTile();
			if (!tile) return;
			...
			this._savedFooter = this.controller.toggleFooter(this._isPinned ? this._actionFooter : this._savedFooter);
			this.$$actionIcon.removeClass(fontUtils.getPinVignetteIcon("all")).addClass(fontUtils.getPinVignetteIcon(this._isPinned ? "on" : "off"));
		},
