...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, opts) {
		...
		onResize: function() {
			/**
			 * Work around for asynchonous loading of the page ($full)
			 * When the page main page is loaded it calls onResize for als control of the page including vignettes (sm.main.layout.changed)
			 * -> If vignette is already loaded we call _currentPage.onResize()
			 * -> If not already loaded we need to call the _currentPage.resize after the page has been loaded
			 * It's whey we need this.hasBeenResized
			 * !! Like we can change orientation/devioce class calling reisze is mandatory to well display the page and make scrolling working
			 * TODO Optimize once all the process will be finalized
			 */
			if (this.hasBeenResized === true) {
				this.hasBeenResized = false;
				log && log("vignette has already been resized - onResize skipped " + this.id);
				return;
			}
			...
			if (this._loaded && this._currentPage && this._currentPage.state.options.$displayStyle === "$full") {
				...
				log && log("vignette not loaded or no page - onResize skipped " + this.id);
			}
		},
		/**
