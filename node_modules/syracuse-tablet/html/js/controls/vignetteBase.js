...
var _actionTitleMgr = {
	...
	iconActionAdd: function(vignetteCtrl) {
		...
		iconBarCtrl.$$header.append($('<icon data-action="' + _actionTitleMgr.action + '" data-control-id="' + vignetteCtrl.id + '" class="s-m-tile-act ' + icon + '" style="float:right"></span>'));
	...
	iconActionToggleStatus: function(vignetteCtrl, status) {
		...
		iconBarCtrl.$$elmt.toggleClass("s-m-enabled", status === "on");
		var $$res = iconBarCtrl.$$header.find('[data-action="' + _actionTitleMgr.action + '"]');
...
var _Klass = utils.defineClass(
	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		onMainPageResize: function(info, orientation, deviceClass) {
			if (this._actionsEnabled && this._actionControl) {
				// If multiselection is enable force off if device changed because _actionControl changed footer/header
				// If current actionControl is a header and we want a footer we destroy it
				var typeControl = this.getTypeActionControl();
				if (this._actionControl.getPageControlType() !== typeControl) {
					this._doTogglePinVignette(null, true);
				}
			}
		...
		_loadPage: function(state, pageOptions, action, loadOptions) {
			var self = this;
			self._error = null;
			if (action === "reload" || (!self._openLinkInsideVignette(state) && self._stateStack.length > 0)) {
		...
		_initPageLoader: function() {
			...
			self._pageLoader = new PageLoader({
				...
				setCurrentPage: function(page) {
					...
					if (page.$activity === "edit" || page.$activity === "create") {
		...
		_initActionDashboard: function(dstPage) {
			try {
				...
				if (!dstPage) return;
				this._typeAction = dstPage.isMultiSelectionEnabled() ? "multiselection" : dstPage.hasFooterHeaderActions() ? "footerHeader" : null;
		 ...
		 */
		_doTogglePinVignette: function(id, forceOff) {
			try {
				if (!this._actionsEnabled) return;
				// Disables the vignette that have the focus and switchs current status
				var status = id != this.id || forceOff === true ? "off" : this._actionControl ? "off" : "on";
				var typeControl = this.getTypeActionControl();
				var controlToggle;
				if (status === "on") {
					if (this._actionControlDisabled) {
						this._actionControl = this._actionControlDisabled;
					} else {
						this._actionControl = ctrlFactory.createActionControl(typeControl, this, {
							multiSelectionOnly: this._typeAction === "multiselection"
						});
					}
					this._actionControlDisabled = null;
					controlToggle = this._actionControl;
				} else {
					this._actionControlDisabled = this._actionControl;
					this._actionControl = null;
					controlToggle = this._actionControlDisabled;
				}
				// this._actionControl = null forces to restore the regular footer/header
				this.controller.toggleHeaderFooter(typeControl, controlToggle, this._actionControl === controlToggle);
		...
		destroyActionControl: function() {
			var ctrl = this._actionControl || this._actionControlDisabled;
			if (!ctrl) return;
			if (this.controller) {
				// remove from controller and restore regular one (null)
				this.controller.toggleHeaderFooter(ctrl.getPageControlType(), null);
			}
			ctrl.destroy();
			this._actionControl = this._actionControlDisabled = null;
		},
