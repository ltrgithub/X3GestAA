			...
			if (!self._pageLoader) {
				...
				self._pageLoader = new PageLoader({
					...
					setHistory: function(state) {
						if (self._stateStack.length == 0 && self._currentPage.isEditMode()) {
							// We force openLinkInVignette to open the lookups in the vignette if first page in edit mode
							// It's to avoid error in test pages that occur when we click on a lookup in a vignette
							self.openLinkInVignette = true;
						}
