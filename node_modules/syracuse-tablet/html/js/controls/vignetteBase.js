...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype) {
		...
		buildHtml: function(parent, controllerDao, buildOptions) {
			var self = this;
			var $$elmt = uiUtils.self(["s-m-control", "s-m-vignette-ctrl"]);
			...
			$(icons).appendTo($$elmt);
			$(self.content).appendTo($$elmt);
			self.rootElmt.appendTo(parent);
		},
		...
		_loadRepresentation: function(gadget) {
			...
			globals.getApplication().getPageInfo(name, {
				...
				if (pageInfo && pageInfo.options && pageInfo.options.$displayStyle === "$full") {
					...
					self.rootElmt.appendChild(picker);
					deferred.resolve();
				}
			});
		...
		_loadExternal: function(gadget) {
			...
			if (displayStyle === "$link") {
				...
				self.rootElmt = document.createElement("div");
				...
				// append line break
				self.rootElmt.appendChild(document.createElement("br"));

				// append iframe
				self.rootElmt.appendChild(uiUtils.createDomElement("iframe", null, null, {
