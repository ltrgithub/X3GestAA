	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		_initActionDashboard: function(dstPage) {
			try {
				...
				if (!tile) {
					console.log("vignette._initActionDashboard - Current version needs a parent tile to work");
					return;
				}
				...
				// It would be better to create an icon bar for the vignette in case there's no tile
		...
		_doTogglePinVignette: function(id) {
			try {
				...
				var typeControl = globals.getSiteLayout().getDeviceType() === "smartphone" ? "header" : "footer";
				this.controller.toggleHeaderFooter(typeControl, this._isPinned ? this._actionControl : null);
			} catch (e) {
