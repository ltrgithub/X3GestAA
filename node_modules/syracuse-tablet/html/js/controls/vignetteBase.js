...
var _Klass = utils.defineClass(
	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		_loadPage: function(state, options, action) {
			...
			then(function() {
				...
				if (typeof state === "string") {
					...
					articleCopy = options.vignetteArticle; // Backup of original article, this MUST NOT be cloned
				...
				.then(function(pageInfo) {
					if (pageInfo.options && pageInfo.options.$displayStyle === "$full") {
						...
						var icon = self._getCssIcon(options.gadget, "s-m-sage-gadget-icon");
						var attrs = {
							"data-nav": pageInfo.name,
							"data-gadget-id": options.gadget.data("$uuid"),
							"data-application-name": options.gadget.data("applicationName"),
							"title": self.article && self.article.$article && self.article.$article.$title ? self.article.$article.$title : options.gadget.data("$title"),
							"icon": icon
						};
		...
		_initPageLoader: function() {
			...
			self._pageLoader = new PageLoader({
				...
				loadingError: function(deferred, state, e) {
					self._displayError(locale.text("error.page.loading", state.name), e);
		...
		_loadRepresentation: function(gadget, name, options) {
			...
			options.vignetteArticle = self.article.$article; // sub article in case the layout of a vignette is defined in the dashboard layout
			...
			try {
				...
				if (gadget && gadget.isValid()) {
					...
					switch (gadget.data("$type")) {
						...
						case "$stats":
						case "$request":
							self._loadRepresentation(gadget, name, options)
