	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			if (this._stateStack.length > 0) {
				...
				if (this.prototype.getVignetteGadget().$type !== "$application") {
					if (this.hammer) {
						this.hammer.destroy();
					}
		...
		_onPress: function(evt) {
			...
			var self = this;
			var step;
			if (!self._currentPage) return;
			dashboardWelcomeUtils.isPinnedTile().then(function(pinned) {
				if (!pinned) {
					var message = locale.text("pin.tile.add.confirm");
					globals.getModal().simpleConfirmPromise({
							message: message
						})
						.then(function(result) {
							if (result.action !== "yes") return;
							dashboardWelcomeUtils.addPinnedTile(self.article, self.prototype)
								.then(function() {
									var notify = {
					...
					var message = locale.text("pin.tile.pinned");
