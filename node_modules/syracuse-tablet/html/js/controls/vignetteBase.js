...
var _Klass = utils.defineClass(
	...
	function VignetteBase(controller, article, prototype, opts) {
		...
		onMainPageResize: function(info, orientation, deviceClass) {
			if (this._multiSelectionEnabled && this._actionControl) {
		...
		reloadPage: function(loadOptions) {
			if (!this.$displayStyle === "$full" || !this._currentPage) return;
		...
		_initActionDashboard: function(dstPage) {
			try {
				...
				this._multiSelectionEnabled = _actionTitleMgr.iconActionAdd(this);
			} catch (e) {
		...
		_doTogglePinVignette: function(id, forceOff) {
			try {
				if (!this._multiSelectionEnabled) return;
				...
				// Recreated because device can change
				this.destroyActionControl();
				if (status === "on") {
					var typeControl = this.getTypeActionControl();
					...
					this.controller.toggleHeaderFooter(typeControl, this._actionControl);
				}
		...
		load: function(name, pageOptions, loadOptions) {
			...
			var _finalize = function(page, error) {
				...
				if (page != null) {
					...
					notifications.notifyController("sm.vignette.loaded", self.controller, self.id, page, error);
				}
			};
