...
var _types = {
	...
	arrayChart: "tablet/x-array-chart"
};
/**
...
var _createControl = function(controller, parentStructElmt, article, prototype, options) {
	...
	switch (type) {
		...
		case _types.reference:
			...
			break;
		case _types.array:
			var pfacet = prototype.parent && prototype.parent.getFacet();
			if (pfacet !== "cube") {
				// Array or request
				if (prototype.data("$cube") && article.$forceArray !== true) {
					// Trick to open a chart from an array 
					if (!article.$display || article.$display == "chart") {
						c = new _ctor.CubeChart(controller, article, prototype, options);
					} else {
						c = new _ctor.ArrayChart(_types.arrayChart, controller, article, prototype, options);
					}
				} else {
				...
				// Statistics
				var forceArray = (article.$forceArray === true) || (article.$display === "table");
				if (prototype.data("$cube") && forceArray !== true) {
					c = new _ctor.CubeChart(controller, article, prototype, options);
				} else {
					c = new _ctor.Array(controller, article, prototype, options);
				}
