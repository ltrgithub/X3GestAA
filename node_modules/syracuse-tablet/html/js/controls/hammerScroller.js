...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("scroll", true);
...
var _Klass = utils.defineClass(function hammerScroller($$elmt, options, parentMgr, $$nativeScroll) {
	...
	_enableEvents: function(yes) {
		...
		// Object has bee notified to not enable scroll
		...
		// IN ANY CASES
		if (this.$$elmt && this.$$nativeScroll) {
			if (wasDisabled) {
				// Nothing
			} else {
				this.panStart = false;
			...
			var css = {
				overflow: this._evtsEnabled === false ? "auto" : "hidden"
			};
			// Set the width/height with right values
			var prop = this.dir === "h" ? "width" : "height";
			css[prop] = (this._evtsEnabled === false ? this.viewRect[prop] : this._originalRect[prop]) + "px";
	...
	_toggleRect: function(remove) {
		...
		if (this._$$viewRect) {
			...
			this._$$docRect = uiRect.elmtRect(globals.getCurrentPage().$$elmt).showInDocument("green");
	...
	notifScrollerToggle: function(status) {
		this._enableEvents(status == null ? !this._evtsEnabled : status === true);
	}
});
