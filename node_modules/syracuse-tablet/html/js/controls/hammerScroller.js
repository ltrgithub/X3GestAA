...
var _Klass = utils.defineClass(function hammerScroller($$elmt, options, parentMgr, $$nativeScroll) {
	 ...
	 */
	init: function(viewRect, dir, valMax) {
		...
		// !! it's important to have the right height/width and to call scroll.init when the page is entirely displayed 
		// !! It's why we need to fix the height/width of images (which are loaded asynchronously)
		...
		// Relocate $elemt if orientation changed
		// If we go from V to H we need to reset the old V scrolling (top=0) and vice versa
		var css = {};
		css[this.dir == "v" ? "left" : "top"] = 0 + "px";
		this.$$elmt.css(css);
	...
	_toggleRect: function(remove) {
		...
		if (this._$$viewRect) {
			...
			var o = this.isPageScroller ? {
				top: 0,
				left: 0
			} : globals.getCurrentPage().getScrollOffset();
			this._$$viewRect.css({
				marginTop: "+=" + o.top,
				marginLeft: "+=" + o.left
			});
	...
	onPan: function(evt) {
		...
		try {
			if (!this.viewRect.isEvtInside(evt.srcEvent, this.isPageScroller ? null : globals.getCurrentPage().getScrollOffset())) {
				log && log(globals.getCurrentPage().getScrollOffset());
	...
	makeVisible: function($$e) {
		...
		val = Math.max(Math.min(0, -1 * val), this.valMin);
		var rel = this.val - val;
		if (rel < 0 || rel > (this.viewRect[prop] - 20)) {
			// We put the field at the top of the parent element to let enough room at the bottom to display the keyboard on a tablet
			// !!! If there's a bug in the calculation and the field is not actually visible, the set focus on the element will scroll the page
			// --> The footer will be scrolled also 
			this.val = val + 5;
			this.refresh();
		}
	}
