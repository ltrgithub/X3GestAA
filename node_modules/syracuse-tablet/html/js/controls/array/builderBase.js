...
var _Klass = utils.defineClass(
	function(control, disp, options) {
		...
		buildHtml: function(arrayData, buildOptions, classes) {
			if (!this.$article) {
				// $article is expected - see articlePArse which set the default article
				this.insertError("<b>No $article found for array " + this.control.$bind + "<b>");
			}
			this._refreshHeader(buildOptions);
		},
		/**
		...
		onResize: function(context) {
			 ...
			 */
			this._refreshHeader();
			this._initGestureMgr(context && context.preserveScroll);
		},
		 ...
		 * We prevent build if no change in $filtersAuthoring or headerAuthoring
		 */
		_refreshHeader: function(buildOptions) {
			...
			if (headerOpts && headerOpts.$filters && headerOpts.$filters.length > 1) {
				...
				if (this.$filtersAuthoring && $filtersAuth == this.$filtersAuthoring) {
					// Prevent to build if no change
					$filtersAuth = null;
				}
			}
			var headerAuth = this.control.getAuthoring("$headerOptions", this.control.controller.isVignette ? "none" : "all");
			if (this.headerAuthoring && headerAuth == this.headerAuthoring) {
				// Prevent to build if no change
				headerAuth = null;
			}
			if (headerAuth && (headerAuth != "none" || $filtersAuth === "list")) {
				this.buildArrayHeader(this.control.getArrayData(), buildOptions, headerAuth, $filtersAuth);
			}
			if ($filtersAuth && $filtersAuth != "none" && $filtersAuth !== "list") {
				this.build$Filters(this._$$stdHeader, headerOpts.$filters, $filtersAuth);
			}
		},
		buildArrayHeader: function(arrayData, buildOptions, headerAuth, $filtersAuth) {
			this.headerAuthoring = headerAuth;
			...
			if (headerOpts.paginEnabled === true && (headerAuth === "all" || headerAuth === "paginOnly")) {
				this.buildPagin(this._$$stdHeader, arrayData);
			}
			if ((headerOpts.filterEnabled === true || headerOpts.sortEnabled === true) && (headerAuth === "all" || headerAuth === "sortFilterOnly")) {
				this.buildSortFilter(this._$$stdHeader);
			}
			if ($filtersAuth === "list") {
				this.build$Filters(this._$$stdHeader, headerOpts.$filters, $filtersAuth);
			}
		},
		build$Filters: function($$stdHeader, filters, $filtersAuth) {
			this.$filtersAuthoring = $filtersAuth;
			this.control.$$header.find(".s-m-filtertabs").remove();
			this.control.$$header.find(".s-m-filterslist").remove();
			if (!$filtersAuth) return;
			var ctrlr = this.control.controller;
			if ($filtersAuth == "none") {
				return;
			}
			if ($filtersAuth == "list") {
		...
		buildPagin: function($$parent, arrayData) {
			var self = this;
		...
		buildSortFilter: function($$parent) {
		...
		sortInfoUpdate: function(order) {
			...
			var $$icon = this._$$stdHeader.find(".s-m-sorted");
		...
		newScroller: function(type, $$elmt) {
			...
			if (this._gestureMgr) {
				 ...
				 */
				this._$$nativeScroll = $$elmt.wrap('<div class="s-m-scroll-wrapper"/>').parent();
			}
			...
			}, globals.getMainPageGestureMgr(), this._$$nativeScroll);
		},
		/**
