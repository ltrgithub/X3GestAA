...
var _Klass = utils.defineClass(
	function(control, disp, options) {
		...
		_refreshHeader: function(buildOptions) {
			...
			var headerAuth = this._getHeaderAuth();
			if (this._isHeaderVisible(headerAuth, $filtersAuth)) {
				this.control.$$header.children(".s-m-stdheader").remove();
				this.control.$$headerTop.children(".s-m-search").remove();
				this._buildArrayHeader(this.control.getArrayData(), buildOptions, headerAuth, $filtersAuth);
			}
			if ($filtersAuth === "tabs") {
		...
		_getHeaderAuth: function() {
			var $headerOptions = this.control.getAuthoring("$headerOptions", {});
			var defaults;
			...
			$headerOptions = $.extend(defaults, $headerOptions);
			return $headerOptions;
		},
		_isHeaderVisible: function($headerOptions, $filtersAuth) {
			return $headerOptions.showPagination ||
				$headerOptions.showSortFilter ||
				$headerOptions.showSearch ||
				$filtersAuth === "list";

		},
		_buildArrayHeader: function(arrayData, buildOptions, headerAuth, $filtersAuth) {
			...
			if (headerOpts.paginEnabled === true && headerAuth.showPagination === true) {
				this._buildPagin(this._$$stdHeader, arrayData);
			}
			if ((headerOpts.filterEnabled === true || headerOpts.sortEnabled === true) &&
				headerAuth.showSortFilter === true) {
				this._buildSortFilter(this._$$stdHeader);
			}

			if (headerOpts.searchEnabled === true && headerAuth.showSearch === true) {
				this.control.getSearcher().getHtml().prependTo(this.control.$$headerTop);
			}
			if ($filtersAuth === "list") {
				this._build$Filters(headerOpts.$filters, $filtersAuth);
			}
