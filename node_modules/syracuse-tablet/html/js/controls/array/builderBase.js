...
var _Klass = utils.defineClass(
	function(control, disp, options) {
		...
		onResize: function(context) {
			 ...
			 */
			if (this.$filtersAuthoring) {
				var headerOpts = this.control.getHeaderOptions ? this.control.getHeaderOptions() : null;
				if (headerOpts && headerOpts.$filters && headerOpts.$filters.length > 0) {
					var auth = this.control.controller.$filtersGetAuthoring ? this.control.controller.$filtersGetAuthoring() : null;
					if (auth != this.$filtersAuthoring) {
						this.build$Filters(this._$$stdHeader, headerOpts.$filters);
					}
				}
			}
		},
		...
		buildArrayHeader: function(arrayData, buildOptions) {
			if (this.control.controller.isVignette) {
				// No header in vignettes
				return;
			}
			...
			this.control.$$header.empty();
			this._$$stdHeader = $('<nav class="s-m-stdheader"/>').appendTo(this.control.$$header);
			if (headerOpts.paginEnabled === true) {
			...
			if (headerOpts.$filters && headerOpts.$filters.length > 0) {
				this.build$Filters(this._$$stdHeader, headerOpts.$filters);
			}
		},
		build$Filters: function($$stdHeader, filters) {
			...
			var auth = ctrlr.$filtersGetAuthoring ? ctrlr.$filtersGetAuthoring() : null;
			if (!auth) return;
			this.$filtersAuthoring = auth;
			if (auth == "none") {
				return;
			}
			if (auth == "list") {
