...
var _templates = {
	...
	options: '\
		<div class="s-m-array-options">\
			...
			{{/if}}\
			<a href="#" data-action="filterSortPanel" draggable="false" data-params="toggle" data-control-id="{{ctrlId}}">\
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		buildPagingLinks: function(arrayData, buildOptions) {
			...
			// build optionshtml
			// TODO check if list is sorted / filtered
			var optionsHtml = _getHtml("options", {
				ctrlId: self.id
			});

			// build array header
			var arrayHeader = _getHtml("arrayHeader", {
				paginHtml: self._$$links && self._$$links[0].outerHTML,
				optionsHtml: optionsHtml
			});

			// append to dom
			self.$$value.before($(arrayHeader));
		},
		/**
		...
		_getRowById: function(rowId) {
			return this.$$elmt.find('tbody .' + globals.RECORDCLASS + '[data-params="' + rowId + '"]');
		},
		...
		getSelectedRowId: function() {
			return this.$$elmt.find("." + globals.RECORDCLASS + ".s-m-last-selected").attr('data-params');
		},
		...
		_actFilterSortPanel: function(actionName) {
			//debugger;
		},
