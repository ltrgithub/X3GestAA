...
var _templates = {
	...
	$filters: '\
		<nav class="s-m-proto-filters">\
			<select	class="form-control" data-action="actionArray" data-params="$filters">\
				{{#each filters}}\
					<option value="{{idx}}" {{#if $isDefault}}selected=""{{/if}}>{{$title}}</option>\
				{{/each}}\
			</select>\
		</nav>'
};
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		buildArrayHeader: function(arrayData, buildOptions) {
			...
			if (buildOptions.$filtersEnabled === true) {
				this.build$Filters();
			}
		},
		build$Filters: function() {
			...
			this._$filters = [];
			var filters = this.control.prototype.data("$filters");
			if (!filters) return;
			var def = false;
			for (var p in filters) {
				var ttl = this.control.prototype.resolveExpression(filters[p].$title);
				if (ttl) {
					var f = $.extend(true, {}, filters[p]);
					f.$title = ttl;
					f.idx = this._$filters.length;
			...
			var $e = $(_getHtml("$filters", {
				filters: this._$filters
			}));
			this.control.$$header.append($e);
		},
