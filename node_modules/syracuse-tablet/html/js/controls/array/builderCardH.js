	...
	function(control, disp, options) {
		...
		_build: function(buildOptions, resizeContext) {
			...
			setTimeout(function() {
					...
					if (visibleTds.indexOf($$td) >= 0) {
						...
						if (idx === 0) {
							...
							if (visibleTds.length * realW < parentW) {
								...
								var cardPerWidth = Math.floor(parentW / realW);
								if (visibleTds.length <= numberOfRows * cardPerWidth) {
									// Dispatch the cards by filling the lines from the top to bottom
									nbCardPerRow = cardPerWidth;
								}
							}
						}
					}
				...
				if (buildOptions.refresh === true || resizeContext) {
