...
var _Klass = utils.defineClass(function(control, disp, options) {
	...
	notifyArrayDataChanged: function(refresh, arrayData, isEmpty) {
		...
		if (arrayData && arrayData.$links) {
			...
			Object.keys(arrayData.$links).forEach(function(key) {
				if (arrayData.$links[key].$isHidden === false) self._links[key] = key;
			});
		}
	},
	_settingsFromValue: function() {
		...
		if ($cube && $cube.$style === "spiderweb") {
			...
			dataset && dataset.reduce(function(res, current, idx) {
				...
				if (categoryName != null) {
					...
					$.smForEachKey(self._meta.series, function(key, s) {
						...
						self._pushData(serie, categoryName, key, current, idx);
					});
				}
				return res;
			}, settings);
	...
	_pushData: function(serie, categoryName, key, data, idx) {
	...
	_getOptions: function(proto, $$parent, options) {
		...
		if (this.currentMetaData.$axisLabels && this.currentMetaData.$axisLabels[this.getCurrentDrillLevel()]) {
			axeTitle = this.$prototype.$properties[this.currentMetaData.$axisLabels[this.getCurrentDrillLevel()]].$title;
		}
		var margin = {
					...
					point: {
						events: {
							click: function(event) {
								return self._onMeasureClick(this.options.idx);
							}
						}
					},
	...
	_getMeasureLabel: function(measure, code) {
		...
		var articleLabelCode = (this.currentMetaData.$axisLabels) ? this.currentMetaData.$axisLabels[this.getCurrentDrillLevel()] : null;
		code = articleLabelCode || code;
	...
	getCurrentDrillLevel: function() {
		return (this._meta && this._meta.drill) ? this._meta.drill.currLevel : 0;
	},
	...
	_onMeasureClick: function(idx) {
		var opt = {};
		if (this._links && this._links.levelUp) {
			opt.linkUp = this._dataset[idx].linkUp;
			opt.level = this._dataset[idx].level;
