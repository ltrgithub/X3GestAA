...
var _Klass = utils.defineClass(function(control, disp, options) {
					...
					point: {
						events: {
							click: function(event) {
								return self._onMeasureClick(this.options.rowId);
							}
						}
					},
	...
	_getMeasureCode: function(code) {
		var measureCode, level;
		level = this.control.controller.state.level;
		measureCode = (level > 1) ? this.$prototype.$axes[0].$hierarchies[level - 1][0] : null;
	...
	_onMeasureClick: function(rowId) {
		if (!this._links || !this._links.$drillDown) return false;
		this.control.actDrillDown(rowId);
		return true;
	}
});
