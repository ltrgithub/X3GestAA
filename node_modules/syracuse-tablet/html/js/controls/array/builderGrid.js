...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		_createColumnInfo: function(options) {
			var self = this;
			var article = self.getGridAuthoring();
			if (!article || !article.$items) throw new Error("Bad array article");
			self.columnInfo = [];
		...
		isExcluded: function(proto, idx) {
			var self = this;
			if (Base.prototype.isExcluded(self, proto)) return true;
			return idx + 1 >= self.maxCols || _isExcluded.indexOf(proto.$type) >= 0;
		},
		...
		_buildTableHead: function(root, options) {
			...
			self.columnInfo.forEach(function(col) {
				var title = col.proto ? utils.parseExpression(col.proto.data('$title') || "", null, self.control.prototype) : "";
