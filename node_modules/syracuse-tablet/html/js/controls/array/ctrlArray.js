...
var _Klass = utils.defineClass(
	...
	function CtrlArray(controller, article, prototype, options) {
		...
		_initArrayAuththoring: function() {
			...
			} else if (this.controller.isChartDetailPage) {
				defaults = {
					showPagination: false,
					showSortFilter: false,
					showSearch: false
		...
		_newBuilder: function(options) {
			...
			Builder = Builders[this.$display == "card" ? options.displayAsCarousel ? "carousel" : "card" : this.$display];
			if (!Builder) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (this.builder.$display !== this.$display) {
			...
			try {
				...
				for (var i = 0; i < pathBind.length; i++) {
					...
					if (!res) {
						...
						res = {
							row: daoSdata.getArrayEmptyRow(this.controller, proto, pathBind[i], dao)
						};
					}
					// Current dao	
					dao = res.row;
				...
				var rowDetailDao = daoSdata.getDaoRowDetail(dao, self.controller, proto);
		...
		readArrayData: function(controllerDao) {
			...
			if (this.prototype.isQuery()) {
				return controllerDao.getQueryData(this.prototype, this._localPaginator);
			} else if (this.prototype.isLookup()) {
				return controllerDao.getLookupData(this.prototype, this._localPaginator);
			} else {
				return controllerDao.getArrayData(this.$bind, this.prototype, this._localPaginator);
			}
		},
		hasLocalPagination: function() {
			return this._localPaginator != null;
		},
		...
		_actLocalPagin: function(action) {
			if (!this._localPaginator) return;
			this._localPaginator.doAction(action);
		...
		savedCtxCreate: function() {
			...
			var res = {
				$builder: this.builder.savedCtxCreate()
			};
			if (this._search) {
				// Stores the search context to restore it on back - detail -> list #6734
				res.$search = this._search.savedCtxCreate();
			}
			return res;
		},
		/**
		...
		*/
		...
		getHeaderOptions: function() {
			...
			opts.paginEnabled = opts.searchEnabled = queryOrLookup || this.hasLocalPagination();
