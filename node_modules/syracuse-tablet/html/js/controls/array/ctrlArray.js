...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		refresh: function(controllerDao, options) {
			var self = this;
			if (self.builder == null) throw new Error("Null builder");
			var selRowId = self.builder.getSelectedRow();
			self.builder.refresh(self._setArrayData(controllerDao, true), options);
			if (selRowId) {
				self.builder.selectLastSelectedRow(selRowId, true);
			}
		},
		onResize: function() {
		...
		_actRowDetail: function(rowId) {
			...
			try {
				...
				for (var i = 0; i < pathBind.length; i++) {
					...
					if (!info.isCreate) {
						// !! it creates a dao chain but the parents are not the same dao object as the dao of array control (same data but not same object)
						// !! When we will update the parent dao on a SAVE action we will take the actual parent instead of rowDetail dao parent (see pageRegular.notifRowdetailUpdate)
				...
				} else if (self.controller.prototype.getDataByPath("$links.$delete")) {
