	...
	function CtrlArray(controller, article, prototype, options) {
		...
		_setArrayData: function(controllerDao, refresh) {
			...
			if (!refresh) {
				...
				var bgColor = this.article.$arrayBgColor;
				if (bgColor == null) {
					bgColor = "s-m-default";
		...
		*/
		...
		addCardContext: function(ctx) {
			if (!ctx) return;
			if (!this._cardContexts) {
				this._cardContexts = [];
			}
			this._cardContexts.push(ctx);
		},
		destroyCardContexts: function() {
			if (!this._cardContexts) return;
			this._cardContexts.forEach(function(ctx) {
				if (ctx.carousel) {
