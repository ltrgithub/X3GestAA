...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		refresh: function(controllerDao, options) {
			...
			var selRowId = this.builder.getSelectedRow();
			...
			try {
				...
				for (var i = 0; i < pathBind.length; i++) {
					...
					if (!info.isCreate) {
						...
						// We need a way to retreive the right row dao when we clik on a row Detail link (there are complex use cases with cards arrays inside cards arrays)
						// -> We store the path of row ids and path of $bind in data attributes of html
						// To retrive the row dao we scan the rootDao from the root according to pathBinds and pathRowIds
