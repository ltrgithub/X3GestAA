...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		_newBuilder: function(options) {
			...
			if (disp.indexOf("card") === 0) {
				Builder = Builders["card"];
			} else {
				Builder = Builders.table;
			}
			this.builder = new Builder(this, disp, options);
		},
		getMode: function() {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (this.article.$display && this.article.$display.smStartsWith("card")) this.article.$display = "card";
			var disp = this.article.$display || "table";
			if (this.builder.$display !== disp) {
			...
			if (this._displayArray()) {
				...
				if (this.prototype.isSingleArray() && this.builder.isEmptySingleArray()) {
					...
					this.isEmpty = true;
				} else if (hasFilerSort) {
					// Update icons
					this._filterSortUpdateHeader();
				}
			} else if (this.isEmpty) {
				$('<span>' + locale.text("array.empty") + '</span>').appendTo(this.$$value);
			}
			...
			if (info == null) {
				...
				if (props) {
					...
					$.each(props, function(key, value) {
						if (!value.$isExcluded && _sortFilterExclude.indexOf(value.$type) === -1) {
							info.push({
		...
		*/
		...
		filterMapGet: function() {
			var self = this;
			var map = [];
			var proto = self.prototype.create(self.prototype.getDataByPath("$item"));
			var props = proto.data("$properties");
			if (props) {
				$.each(props, function(key, value) {
					if (!value.$isExcluded && _sortFilterExclude.indexOf(value.$type) === -1 && value.$capabilities && value.$capabilities.indexOf("filter") >= 0) {


						var isReference = value.$type.indexOf("x-reference") >= 0;
						var addItem = true;

						// if reference field, we will not consider the fields that have combined primary keys
						if (isReference && value.$item.$properties && Object.keys(value.$item.$properties).length > 1) {
							addItem = false;
						}

						if (addItem) {
							map.push({
								fieldName: proto.resolveExpression(value.$title),
								operators: ctrlfactory.getOperators(value.$type),
								id: proto.getFieldNameForValue(key),
								prop: value,
		...
		getHeaderOptions: function() {
			...
			// Manual sort/filters
			opts.filterEnabled = opts.sortEnabled = opts.paginEnabled = queryOrLookup;
		...
		$filtersGetAuthoring: function() {
			var auth = this.getAuthoring("$filters", "list");
		...
		multiSelSetMgr: function(mgr) {
			...
			if (mgr) {
				this._multiselHandler = utils.bindFn(mgr.callbackArray, mgr);
			}
		},
		/**
		...
		authOnSlideMinSize: function(slideEvt, action) {
			if (this.builder.$display != "card" || !this._arrayData || this._arrayData.length > 0) return;
			var value = this.builder.authMinSize2Pixels(slideEvt.value);
			...
			if (action == "slide") {
				...
				this._$$cardSize.children("span").text("Min size " + value);
			}
		}
