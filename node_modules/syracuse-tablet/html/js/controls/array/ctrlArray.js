...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
			...
			if (this._displayArray()) {
				if (!this.isArrayField()) {
					...
					this._filterSortPanel = this.controller.panelCreate(_filterSortPanelId, {
						parentArray: this,
						filter: false, //buildOptions.filterEnabled,
						sort: buildOptions.sortEnabled
					});
				}
				this.builder.buildHtml(this._arrayData, buildOptions);
				if (buildOptions.sortEnabled === true) {
					this.sortUpdateHeader();
				}
			}
		},
		/**
		...
		sortInfoGet: function() {
			if (this._sortInfo) return this._sortInfo;
			this._sortInfo = sessStorage.getItem(this.sDataUrl);
			if (this._sortInfo) {
				try {
					this._sortInfo = JSON.parse(this._sortInfo);
			...
			var props = proto.getDataByPath("$item.$properties");
			if (props) {
			...
			this.sortInfoSet(sortInfo);
			return this._sortInfo;
		},
		sortInfoSet: function(info) {
			this._sortInfo = null;
			if (info) {
				this._sortInfo = $.extend([], info);
				sessStorage.setItem(this.sDataUrl, JSON.stringify(this._sortInfo));
				this.sortUpdateHeader();
			}
		},
		sortUpdateHeader: function() {
			var sortInfo = this.sortInfoGet();
			var order = null;
			// take the first sorted column
			// Currently we support sorting on only one column
			// -> We can display a asc/dsc icon in the header
			// Sort panel can manage sorting on multiple columns but it's disabled
			sortInfo.some(function(i) {
				if (i.sort != "none") {
					order = i.sort;
					return true;
				}
			});
			var $$icon = this.$$header.find(".s-m-sorted");
