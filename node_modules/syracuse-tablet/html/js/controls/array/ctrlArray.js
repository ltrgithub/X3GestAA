	...
	function(controller, article, prototype, options) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (this._displayArray()) {
				if (!this.isArrayField()) {
					buildOptions.$filtersEnabled = buildOptions.filterEnabled = buildOptions.sortEnabled = buildOptions.paginEnabled = true;
		...
		*/
		filterMapGet: function() {
			...
			if (props) {
				$.each(props, function(key, value) {
					if (!value.$isExcluded && _sortFilterExclude.indexOf(value.$type) === -1 && value.$capabilities.indexOf("filter") >= 0) {
						map.push({
							fieldName: proto.resolveExpression(value.$title),
							operators: self.getOperators(value.$type),
							id: proto.getFieldNameForValue(key),
							prop: value,
