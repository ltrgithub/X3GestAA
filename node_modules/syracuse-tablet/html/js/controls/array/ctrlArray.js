	...
	function CtrlArray(controller, article, prototype, options) {
		...
		multiSelGetSelectedData: function() {
			...
			this.$$content.find(".s-m-record.s-m-multi-selected").each(function(idx, record) {
				...
				if (rowId.length > 0) {
					rowId = rowId[1];
					var rowdata = self.arrayCongroller.dataset.getRowByUuid(rowId);
					if (rowdata) {
						data.push({
							rowData: rowdata,
							rowId: rowId,
		...
		onDeltaRow: function(action, arrayDataset, $uuid, idx) {
			...
			var rowCtrl;
			this.arrayController.forEachControl(function(row, idx) {
			...
			var append = idx == null || idx >= (rowDataset.parent.getNumRows() - 1);
			var rowctrl;
			if (append) {
