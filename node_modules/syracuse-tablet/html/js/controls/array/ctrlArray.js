...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		sortInfoGet: function() {
			var info = this.controller._filterSortRead("sort");
			if (info) return info;
			info = [];
			...
			this.controller._filterSortWrite("sort", info);
			return info;
		},
		sortInfoSet: function(info) {
			var self = this;
			if (!info) return;
			self.controller._filterSortWrite("sort", $.extend([], info));
			//this._filterSortUpdateHeader("sort");
			setTimeout(function() {
		...
		filterInfoGet: function() {
			var info = this.controller._filterSortRead("filter");
			if (info) return info;
			info = [];
			// TODO
			return info;
		},
		filterInfoSet: function(info) {
			if (!info) return;
			this.controller._filterSortWrite("filter", $.extend([], info));
			this._filterSortUpdateHeader("filter");
		}
	});
