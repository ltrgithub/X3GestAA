...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		_newBuilder: function(options) {
			if (false && this.prototype.isQuery()) {
				this.article.$display = "cardH";
			}
		...
		isMode: function(mode) {
			return this.builder && this.builder.$type === mode;
		},
		/**
		...
		destroy: function() {
			if (this._multiselHandler) {
				this._multiselHandler(null, this, "$stopmultisel");
			}
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (this._displayArray()) {
				...
				if (hasFilerSort) {
					this._filterSortPanel = this.controller.panelCreate("filtersortpanel", {
						parentArray: this,
						filter: true,
						sort: true
					});
		...
		*/
		...
		/**
		 * Returns filters map. Map is a JSON object with following structure
		 *  [{
		...
		multiSelSetMgr: function(mgr) {
			if (this._multiselHandler) {
				if (this.$$value) {
					this.$$value.off("click", ".s-m-record", this._multiselHandler);
				}
				this._multiselHandler = null;
			}
			if (mgr) {
				this._multiselHandler = utils.bindFn(mgr.multiSelCallBack, mgr, this);
				this.$$value.on("click", ".s-m-record", this._multiselHandler);
			}
		},

		_actMultiSelectionTrigger: function(params, $target) {
			if (!this._multiselHandler) return;
			this._multiselHandler(null, this, params);
		}
