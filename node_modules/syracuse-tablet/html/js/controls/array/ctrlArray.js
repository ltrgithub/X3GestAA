...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		_openDetailRow: function(rowInfofId) {
			...
			try {
				...
				for (var i = 0; i < pathBind.length; i++) {
					...
					if (!res) {
						if (info.activity != "create" || i != pathBind.length - 1) throw new Error("unexpected empty row");
						res = {
							row: daoSdata.getArrayEmptyRow(utils.UUID(), proto, pathBind[i], dao)
						};
					}
					// Current dao
					dao = res.row;
				...
				if (!dao) return;
				// here dao contains the data of selected row
				...
				if (self.controller.isEditMode()) {
					...
					if (true || self.controller.prototype.getDataByPath("$links.$delete")) {
				...
				proto = proto.getPrototype("$item", protoJson);
		...
		_actRowDelete: function(rowId) {
			...
			modal.simpleConfirm(locale.text("actions.del.confirm"), function(confirmed) {
				if (confirmed) {
					...
					if (pathBind.length == 0) throw new Error("unexpected null bind id");
					/**
					 ...
					 * --> Not implemented for the moment beacuse not tested
					 */
					if (pathBind.length > 1) throw new Error("Deleting in a child array is not implemented");
					// Name of the array field
					var field = pathBind.pop();
					// Remove the row in dao
					self.getDao().deleteRow(field, rowId);
					// Remove the row in _arrayData
					var array = self._arrayData;
					...
					self.builder.removeRowById(rowId);
				}
			});
		},
		...
		rowDetailActivity: function(create) {
			// Read/ Edit row detail
			return this._rowDetailInfo.activity;
		},
		/**
		...
		_getRowById: function(rowId, arrayData) {
			...
			if (!arrayData || !arrayData.$resources || arrayData.$resources.length == 0) return result;
			var idx = 0;
