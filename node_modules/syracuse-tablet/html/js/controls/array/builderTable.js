	...
	function(control, options) {
		...
		buildHtml: function(arrayData, options) {
			...
			if (this._createColumnInfo(options).length > 0) {
				var $$table = $('<table class="table table-condensed"/>').appendTo(this.control.$$value);
				this._buildTableHead($$table, options);
				if (arrayData.$resources.length > 0 || this.control.controller.isEditMode) {
					...
					if (scrollable) {
						$$table = $('<table class="table table-condensed"/>').appendTo(this.control.$$value);
					}
					this._buildTableBody($$table, arrayData, options);
					this._buildTableFoot($$table, arrayData, options);
		...
		_buildTableHead: function($$table, options) {
			var self = this;
			this.$$tabHead = $(uiUtils.createDomElement('thead', null, null, null, $$table));
		...
		_buildTableBody: function($$table, arrayData, options) {
			var tbody = $(uiUtils.createDomElement('tbody', null, null, null, $$table)).get(0);
			// Build rows
			for (var i = 0; i < arrayData.$resources.length; i++) {
				this._buildTableRow(tbody, arrayData.$resources[i], i, options);
			}
		},
		 ...
		 */
		sortInfoUpdate: function(order, field) {
			Base.prototype.sortInfoUpdate.call(this, order);
			if (order && field && this.$$tabHead) {
				...
				$$th = this.$$tabHead.find("." + field);
				if ($$th.length > 0) {
					var title = $$th.text();
					$$th.html(_getHtml("sortedHeader", {
						title: title,
						css: fontUtils.sortIcon(order)
					}));
					$$th.addClass("s-m-sorted " + order);
				}
			}
		}
	});
