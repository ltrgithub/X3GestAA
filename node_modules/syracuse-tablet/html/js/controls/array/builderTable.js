...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		onSwipePan: function(evt) {
			...
			if (panInfo.curIdx == null) panInfo.curIdx = 0;
			var lastidx = panInfo.curIdx;
			if (evt.type === "panup") {
				var newIdx = Math.max(0, Math.min(panInfo.rsrcLen - 1, lastidx + count));
			...
			if (!self.control.isArrayField() && self.$$tbody) {
				...
				if (self._panEvtInfo.overFlow) {
					...
					if (this._restoredScrollIdx != null) {
						...
						self._panEvtInfo.curIdx = self._restoredScrollIdx;
		...
		savedCtxCreate: function() {
			return this._panEvtInfo ? {
				scrollIdx: this._panEvtInfo.curIdx
			} : null;
		},
		/**
