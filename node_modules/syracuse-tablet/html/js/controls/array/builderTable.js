	...
	function(control, options) {
		...
		_getOtherColProto: function(colInfo, rowId, rowData, options) {
			...
			if (colInfo.$activity === "create") {
				...
				// TODO - Table into a card - we've to add the pathbind/pathid
				// We can create row only at the first level
				params = rowId;
			} else {
			...
			var json = {
				$action: {
					"data-action": action,
					// TODO - We recopy params here whiel rowid is already stored in s-m-grid-record elemt
					// See how to get rowid from this elemt instead of having to copy it 
					"data-params": params
				}
			};
		...
		_svggetOtherColProto: function(colInfo, rowId, rowData, options) {
			var self = this;
			var action = colInfo.$action;
			var params;
			var rowInfo = {
			...
			if (colInfo.$activity === "create") {
				...
				while (rd && rd.$bind && rd.$bind.length > 0) {
					rowInfo.pathId.unshift(rd.getValue("$uuid"));
					rowInfo.pathBind.unshift(rd.$bind);
					rd = rd.parent;
				}
				params = rowId;
