		...
		buildHtml: function(arrayData, options) {
			...
			if (this._createColumnInfo(options).length > 0) {
				...
				if (arrayData.$resources.length > 0 || this.control.controller.isEditMode) {
					...
					this._buildTableFoot(this.$$tbl, arrayData, options);
		...
		_buildTableFoot: function($$table, arrayData, options) {
			...
			if (self.control.controller.isEditMode()) {
				if (this.control.prototype.isSingleArray()) {
					// No 'add row' button for single arrays in readonly or hidden mode - TODO improve
					if ($$table.find(".s-m-record.readonly").length === arrayData.$resources.length) return;
					if ($$table.find(".s-m-record.hidden").length === arrayData.$resources.length) return;
				}
