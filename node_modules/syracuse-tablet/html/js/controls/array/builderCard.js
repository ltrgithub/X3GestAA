...
var _Klass = utils.defineClass(
	function(control, disp, options) {
		...
		buildHtml: function(arrayData, buildOpts) {
			...
			this.oneCardDisplay = buildOpts.htmlRootCtrl && buildOpts.htmlRootCtrl.$display === "card";
		...
		_build: function(buildOpts, resizeContext) {
			...
			if (!this._$$body) {
				...
				var $$table = $('<table/>').appendTo(this.control.$$value);
				if (this.oneCardDisplay) {
					$$table.width("100%");
				}
				this._$$body = $('<tbody>').appendTo($$table);
				var $$td;
		...
		buildCardH: function(visibleTds, bs, parentH, parentW, buildOpts) {
			...
			var nbVisiblecardsPerRow = this.oneCardDisplay ? 1 : buildOpts.cardAuthoring.$cardsPerRow;
			var minWidth = buildOpts.cardAuthoring.$minWidth;
			parentH = parentH - bs;
			var remaining = 0;
			var self = this;
