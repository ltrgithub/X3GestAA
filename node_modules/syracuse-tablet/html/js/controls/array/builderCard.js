	...
	function(control, disp, options) {
		...
		buildHtml: function(arrayData, buildOptions) {
			...
			if (self._layoutRoot == null) {
				...
				var article = self.$articleArray.$card;
				if (!article) {
					article = articleGen.genNewArticle({
						page: {
				...
				self._layoutRoot = articleParser.article2Controls(self.control.controller, article, self.control.prototype.getPrototype("$item"), options);
			}
		...
		_build: function(buildOptions, resizeContext) {
			...
			if (this._dirArray == null || this._dirArray != dirArray) {
