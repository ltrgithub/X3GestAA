	...
	function(control, disp, options) {
		...
		buildHtml: function(arrayData, buildOptions) {
			...
			arrayData.$resources.forEach(function(rowData, idx) {
				...
				self.callGenerateHtml(self._layoutRoot, $$root, rowData, false, buildOptions, idx);
				self._tds.push({
					"$$td": $$td,
					"$$card": $$cell
				});
			});
			if (this.isRefreshContext(buildOptions)) {
		...
		_build: function(buildOptions, resizeContext) {
			...
			this["buildCard" + dirArray.toUpperCase()](visibleTds, bs, parentH, parentW, buildOptions);
		...
		buildCardH: function(visibleTds, bs, parentH, parentW, buildOptions) {
			...
			if (remaining && buildOptions.cardAuthoring.$emptyCell !== "nodisplay") {
				var $$emptyCells = this.addEmptyCells(this._layoutRoot, $$row, remaining, buildOptions, this._$$cellTmpl);
				if (heightChanged) {
