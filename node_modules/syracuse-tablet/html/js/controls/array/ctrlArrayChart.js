...
var _checkChartArticle = function(article, $bind) {
	if (!article) return;
	_traverseArticleNodes(article, function(node) {
		if (node.$bind === $bind && node.$forceArray == null) {
			node.$forceArray = node.$display !== "chart";
		}
	});
	return article;
};
	...
	function CtrlArrayChart(controller, article, prototype, options) {
		...
		_getChartDetailPageInfo: function(typePage) {
			...
			if (this.controller.isVignette) {
				...
				pageInfo.name = this.controller.state.name + "_" + this.$bind + "_" + typePage;
			}
			return pageInfo;
		},
		/**
