...
var _findChartArticle = function(article, $bind) {
	...
	var chartNode = _traverseArticleNodes(article, function(node) {
		if (node.$bind === $bind && node.$display === "chart") {
			return true;
		}
	});
	return chartNode;
...
var _Klass = utils.defineClass(
	function CtrlArrayChart(controller, article, prototype, options) {
		 ...
		 */
		_findTitle: function() {
			var $title = this.prototype.data("$title");
			if ($title) {
				return $title;
			}
			var $cube = this.prototype.data("$cube");
			...
			if (this.prototype.parent) {
				return this._findTitle(this.prototype.parent);
			}
		},
		 ...
		 */
		triggerOpenChartDetail: function() {
			if (!this.$$elmt) return;
			var attrs = {
				...
				"data-sdata-url": this.controller.getOriginalSdataUrl(),
		...
		chartNeedsHeight: function() {
			return this.$display === "chart" && this._layouytType === "stack" && !this.isChartDetailPage();
		},
		/**
		...
		_actLocalPagin: function(action) {
			Base.prototype._actLocalPagin.call(this, action);
			if (this._paginSynchro) {
				this._paginSynchro.forEach(function(ctrl) {
					ctrl._actLocalPagin(action);
				});
			}
		},
		/**
