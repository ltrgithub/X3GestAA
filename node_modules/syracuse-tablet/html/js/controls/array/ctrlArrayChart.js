...
var _Klass = utils.defineClass(
	function CtrlArrayChart(controller, article, prototype, options) {
		...
		_displayTitle: function(title) {
			// be carefull, in some cases title could be numeric (???) no trim method in this case.
			if (title == null || (title.trim && title.trim().length === 0) || this.getAuthoring("$isTitleHidden") === true) {
				return false;
			}
		 ...
		 * 
		 */
		_getChartDetailSettings: function(level) {
			var mainPage = this._findMainPage()
			if (mainPage) {
				var settings = mainPage.getPageSettings();
				var levelSettings = settings.chartDetail && settings.chartDetail[level];
