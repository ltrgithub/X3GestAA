...
var _Klass = utils.defineClass(
	function CtrlArrayChart(controller, article, prototype, options) {
		...
		_newBuilder: function(options) {
			// No _actRowSelect Action except if isChartDetailPage (link to sublevel)
		...
		//called by builderChartBase
		actDrillDown: function(rowId) {
			this._actRowSelect(rowId, null);
		},
		...
		_actRowSelect: function(rowId, $$target) {
			var opt = {
				...
				currentSelection: this.getCurrentMeasureLabel(rowId)
			};
		...
		getCurrentMeasureLabel: function(rowId) {
			var hierarchieIndex = (this.controller.state.level || 1) - 1;
		...
		_getChartDetailUserPrefsId: function() {
			...
			if (controller.isVignette && controller.getParentVignette()) {
				...
				var level = this.controller.state.level ? this.controller.state.level + 1 : 1;
				if (level > 1) {
					id.push('-');
					id.push(level);
				}
		...
		triggerOpenChartDetail: function(opt) {
			this.currentSelection = (opt && opt.currentSelection) ? opt.currentSelection : null // for breadcrumb;
			if (!this.$$elmt) return;
			var attrs = {
		...
		_getChartDetailPageInfo: function(typePage) {
			...
			if (parentVignette) {
				...
				title = this.currentSelection || (locale.text("chartdetail.page.level") + " " + pageInfo.level);
			}
