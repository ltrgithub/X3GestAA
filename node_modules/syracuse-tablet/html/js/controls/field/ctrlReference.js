	...
	function CtrlReference(controller, article, prototype, options) {
		...
		addLink: function(parentElmt) {
			...
			if ($detaillink) {
				...
				return uiUtils.createDomElement("span", null, null, self._getLinkAttrs(sdataUtils.getLinkInfo($detaillink.$url, self.getDao()), {
		...
		_getLinkAttrs: function(link, options) {
			...
			if (self.controller.isVignette) {
				// We alway open a lookup in ful page
				// This old code set teh target according to the context - attrs["data-nav-target"] = self.controller.openLinkInVignette() ? "vignette" : "application";
