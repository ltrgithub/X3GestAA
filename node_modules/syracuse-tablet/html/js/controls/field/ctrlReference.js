...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype) {
		...
		getValue: function() {
			...
			var $item = self.prototype.data("$item");
			var $itemProp = $item["$value"] ? utils.extractCode($item["$value"]) : "$value";

			return val ? val[$itemProp] || val.$title || val.$value : "";
		},
		hasValue: function() {
		...
		hasDescription: function() {
			var self = this;
			var val = Base.prototype.getValue.call(self);
			return val != null && val.$description != null && val.$description.length > 0;
		},
		...
		buildFieldValue: function(parentSlot, buildOptions) {
			...
			if (self.$isEditMode) {
				...
				// build lookup link
				if (!(self.prototype.data("$item") && self.prototype.data("$item").$links && self.prototype.data("$item").$links.$lookup)) {
					throw new Error("Expected $lookup link");
				} else {

					// get dom $lookup link parameters/attributes
					var $link = self.prototype.data("$item").$links.$lookup;
					...
					self.picker = uiUtils.createDomElement("a", ["btn", "btn-default", "s-m-field-ref-lookup"], null, {
						"href": "#",
						"data-nav": link.page,
						"data-sdata-url": link.sDataUrl,
						"title": utils.parseExpression($link.$title, null, self.prototype) || "Lookup"
					}, span);
					//TODO localize ("Lookup")

					uiUtils.createDomElement("span", "glyphicon glyphicon-eye-open", null, null, self.picker);
				}
