...
var _Klass = utils.defineClass(
	...
	function CtrlReference(controller, article, prototype, options) {
		...
		buildDetailValue: function(parentElmt, value, descr, title) {
			...
			function setDescByOrder(el1, el2) {
				var res;
				if (el1) {
					res = el1;
				}
				if (el2) {
					res += el1 ? (el1 === el2 ? "" : "-" + el2) : el2;
				}
				return res;
			}
			...
			if (value.length > 0) {
				...
				switch ($refDescFormat) {
					case "title":
						...
						break;
					case "desctitle":
						descContent = setDescByOrder(descr, title);
						break;
					case "titledesc":
						descContent = setDescByOrder(title, descr);
						break;
				...
				switch ($refDescPosition) {
					...
					case "none":
						...
						break;
						// "right" will be considered as default
					default:
						arr = [$$value, $$desc];
						$(container).append($$value).append($$desc);
						$.each(arr, function(index, $$ee) {
