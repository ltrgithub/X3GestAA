...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (self.article.$gauge && !self.$isEditMode) {
				if (!self.buildGauge || !self._isNumberType(metaData)) {
					throw new Error(locale.text("field.gauge.typeError", [metaData.$type]));
				} else {
					self.buildGauge(fieldSlot, self.article.$gauge);
				}
			} else {
				// valueFirst see combo-box
				var valueFirst = buildOptions.valueFirst === true || self.getAuthoring("$value.$beforelabel", false);
				if (valueFirst) {
		...
		_isNumberType: function(metaData) {
			var self = this;
			...
			var types = ["integer", "decimal", "real"];
			for (var ii = 0, jj = types.length; ii < jj; ii++) {
				if (metaData.$type.indexOf(types[ii]) > -1) {
					return true;
				}
			}
			return res;
