...
var _Klass = utils.defineClass(
	...
	function CtrlFieldBase(controller, article, prototype, options) {
		...
		this.nativePenTextInput = native.hasCapability("penTextInput");
		this.nativeGPS = native.hasCapability("GPS");
		this.nativeBarcode = native.hasCapability("scanBarcode");
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (this.controller.isUnitTest) {
				this.displayUnitTestMsg(this.isEmpty ? this.$$elmt.parent() : this.$$elmt);
			}
		},
		/**
		...
		createMainInput: function(parent, value, type) {
			...
			var pen = this.nativePenTextInput && (!checkBox) && !($(parent).hasClass("input-group"));
			var gps = this.article && this.article.$scanGPS && this.nativeGPS && (!checkBox) && !($(parent).hasClass("input-group"));
			var barcode = this.article && this.article.$scanBarcode && this.nativeBarcode && (!checkBox) && !($(parent).hasClass("input-group"));

			var contener = (pen || gps) ? uiUtils.createDomElement("div", ["input-group"], null, null, parent) : parent;
			var css = ["s-m-meta"];
			var attrs = {
				"type": type
			};
			...
			this.domInput = uiUtils.createDomElement("input", css, null, attrs, contener);
			if (checkBox) this.domInput.checked = value;

			if (barcode) { // Barcode is most valuable option
				this._createBarcodeInput(contener);
			} else if (gps) { // GPS overrides pen input
				this._createGPSInput(contener);
			} else if (pen) {
				this._createPenInput(contener);
			}
		...
		_createPenInput: function(parent) {
			var self = this;
			if (!this.picker) {
				var span = uiUtils.createDomElement("span", ["input-group-btn"], null, null, parent);
				this.picker = uiUtils.createDomElement("button", ["btn", "btn-default", "s-m-meta"], null, null, span);
				uiUtils.createDomElement("span", fontUtils.penFieldIcon(), null, null, this.picker);
						...
						function(data) {
							if (data.action === "ok") {
								if (data.matches && data.matches.length) {
									self.setFieldValue(data.matches[0]);
								}
							}
						}
					);
		...
		_createGPSInput: function(parent) {
			var self = this;
			if (!this.picker) {
				var span = uiUtils.createDomElement("span", ["input-group-btn"], null, null, parent);
				this.picker = uiUtils.createDomElement("button", ["btn", "btn-default", "s-m-meta"], null, null, span);
				uiUtils.createDomElement("span", fontUtils.getIconByName("gps"), null, null, this.picker);
				$(this.picker).click(function() {
		...
		_createBarcodeInput: function(parent) {
			var self = this;
			if (!this.picker) {
				var span = uiUtils.createDomElement("span", ["input-group-btn"], null, null, parent);
				this.picker = uiUtils.createDomElement("button", ["btn", "btn-default", "s-m-meta"], null, null, span);
				uiUtils.createDomElement("span", fontUtils.getIconByName("scanBarcode"), null, null, this.picker);
				$(this.picker).click(function() {
					globals.getApplication().currentPage.waitWheelStart();
