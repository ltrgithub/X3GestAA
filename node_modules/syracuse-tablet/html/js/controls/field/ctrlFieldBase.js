...
var _Klass = utils.defineClass(
	...
	function CtrlFieldBase(controller, article, prototype, options) {
		...
		isNativeCapabilityEnabled: function(capability) {
		...
		createMainInput: function(parent, value, type, attrs) {
			type = type || "text";
			if (this._nativeCapabilities) {
				var nativeCapability;
				if (this.isNativeCapabilityEnabled("scanBarcode")) {
					nativeCapability = "scanBarcode";
				} else if (this.isNativeCapabilityEnabled("GPS")) {
					nativeCapability = "GPS";
				} else if (this.isNativeCapabilityEnabled("penTextInput")) {
					nativeCapability = "penTextInput";
				}
				if (nativeCapability) {
					this.addPicker(nativeCapability, fontUtils.getIconByName(nativeCapability + "Picker"));
				}
			}
			...
			this.$$input = $(uiUtils.createDomElement(element, css, null, attrs)).prependTo(this.$$inputGroup || this.$$value);
			if (checkBox) {
		...
		addPicker: function(type, icon) {
			if (this._pickers && this._pickers[type]) return;
			this._pickers = this._pickers || {};
			if (!this.$$inputGroup) {
				this.$$inputGroup = $('<div class="input-group"/>').appendTo(this.$$value);
			}
			if (!this.$$inputGroupBtn) {
				this.$$inputGroupBtn = $('<span class="input-group-btn"/>').appendTo(this.$$inputGroup);
			}
			this._pickers[type] = $('<button data-control-id="' + this.id + '" data-action="' + type + 'Picker" class="btn btn-default s-m-meta"><span class="' + icon + '"></span></button>');
			this._pickers[type].appendTo(this.$$inputGroupBtn);
		},
