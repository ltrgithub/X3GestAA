...
var _Klass = utils.defineClass(
	...
	function CtrlFieldBase(controller, article, prototype, options) {
		...
		this.nativePenInput = nativePen.supportsPenInput();
		...
		createMainInput: function(parent, value, type) {
			...
			var pen = this.nativePenInput && (!checkBox) && !($(parent).hasClass("input-group"));
			var contener = pen ? uiUtils.createDomElement("div", ["input-group"], null, null, parent) : parent;
			var css = ["s-m-meta"];
			var attrs = {
				"type": type
			};
			...
			if (pen) this._createPenInput(contener);
						...
						function(data) {
							if (data.action === "ok") {
								if (data.matches && data.matches.length) {
									self.setFieldValue(data.matches[0], true);
								}
							}
						}
					);
