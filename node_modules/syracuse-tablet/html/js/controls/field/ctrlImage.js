...
var _Klass = utils.defineClass(
	...
	function CtrlImage(controller, article, prototype, options) {
		...
		buildFieldValue: function(parentSlot, buildOptions) {
			var diplayInACell = buildOptions && (buildOptions.displayCtx === "table" || buildOptions.displayCtx === "card")
			if (!this.article.$imgHeight) {
				this.article.$imgHeight = diplayInACell ? "small" : "medium";
			}
			...
			if (this.$isEditMode && typeof FileReader !== "undefined") {
				...
				if (this.article.$imgDisplayIcon === true) {
					...
					var css = ["img-responsive"]
					if (!diplayInACell) {
						// No click in table or card mode
						css.push("ctrl-evt-click")
					}
					var e = uiUtils.createDomElement("a", css, null, {
						"href": "#"
					}, imgParent);
					// Image with link
	...
	*/
		...
		*/
		...
		_getResizeOptions: function() {
			var scale = _scaleMap[this.getAuthoring("$imgScale") || "original"]
			if (scale == null) {
				return null;
			}
			return {
