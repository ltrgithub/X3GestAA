...
var nativePen = require('syracuse-tablet/html/js/helpers/native/nativePenInput');
...
var _Klass = utils.defineClass(
	...
	function CtrlImage(controller, article, prototype, options) {
		...
		_buildEditImageField: function(parentSlot, docUrl, empty) {
			...
			var $$header = $("<header>");
			var clearBtn, editBtn, uploadBtn, core;
			clearBtn = uiUtils.createDomElement("a", ["ctrl-evt-click"], null, {
				"href": "#"
			}, $$header);
			$(clearBtn).css("top", 1);
			$(clearBtn).attr("action", '$delete');
			var classCss = ["s-m-clear"];
			classCss.push(fontUtils.getIconByName("$delete"));
			clearBtn = uiUtils.createDomElement("span", classCss, null, null, clearBtn);
			$(clearBtn).toggle(!empty);

			if (this.penInput) {
				editBtn = uiUtils.createDomElement("a", ["ctrl-evt-click"], null, {
					"href": "#"
				}, $$header);
				$(editBtn).css("right", 30);
				$(editBtn).attr("action", '$edit');
				classCss = ["s-m-edit"];
				classCss.push(fontUtils.getIconByName("$edit"));
				editBtn = uiUtils.createDomElement("span", classCss, null, null, editBtn);
			}
			$(editBtn).toggle(!empty);

			$$header.appendTo(this.$$layout);
		...
		*/
		_onClearClick: function(evt, dom) {
			...
			$(clearBtn).toggle();
			if (this.penInput) {
				var editBtn = this.$$elmt.find(".s-m-img-layout > header > a[action='$edit']")[0];
				$(editBtn).toggle();
			}
			// update upload button
			this.$$elmt.find(".s-m-img-layout > footer > a").text(locale.text("image.label.addimage"));
			this._setLocalUrlValue(null);
		},
		_onEditClick: function(evt, dom) {
			var self = this;
			nativePen.getAnnotatedImage({
				title: self.getTitle(),
				...
				function(data) {
					if (data && data.action === "ok") {
						if (data.matches && data.matches.length) {
							self.setValue(self._getValue(data.matches[0]));
							self._setImage(data.matches[0]);
						}
					}
				}
			);
	...
	*/
			...
			if (evt.target.nodeName.toLowerCase() === 'input') {
				...
				if (files && files.length) {
					...
					reader.onload = (function(theFile, cont) {
						return function(e) {
							cont.$value = self._getValue(e.target.result);
							delete cont.$url;
		...
		*/
		_setImage: function(src) {
			...
			// update upload button
			this.$$elmt.find(".s-m-img-layout > footer > a").text(locale.text("image.label.changeimage"));
			// toggle clearBtn
			this.$$elmt.find(".s-m-img-layout > header > a[action='$delete']").toggle(true);
			if (this.penInput) this.$$elmt.find(".s-m-img-layout > header > a[action='$edit']").toggle(true);
		},
