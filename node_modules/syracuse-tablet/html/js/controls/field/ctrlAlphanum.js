	...
	function CtrlUrlField(controller, article, prototype, options) {
		...
		buildFieldValue: function(fieldSlot, buildOptions) {
			...
			if (self.$isEditMode) {
				...
				var $urlLink = self.prototype.getUrlLink(self.$bind);
				var value = self.getFormattedValue(self.getValue()) || self.prototype.resolveExpression($urlLink.$url, self._dao.data, true);
				var title = ($urlLink.$title ? self.prototype.resolveExpression($urlLink.$title) : self.prototype.data("$shortTitle") || "...");
				var link = uiUtils.createDomElement("a", [], title, {
					"href": value,
					"target": "_blank",
