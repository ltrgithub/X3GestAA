	...
	function CtrlFooterBase($type, controller, options) {
		...
		_addActionMgrHtmlCtx: function(ctx, actionController, displayController) {
			...
			if (this._actionPanel && this._actionPanel.hasLinks()) {
				...
				rightLinks.push({
					action: "footerAction",
					icon: "fa fa-ellipsis-h", //fontUtils.pageIcon("sidePanelR"),
					$uuid: _actionPanelId,
					title: "Actions",
			...
			if (this.footerOptions.showMyDashboard) {
				ctx.mydashboard = _getHtml("tablet", "action", {
					action: "myDashboard",
					icon: fontUtils.pageIcon("mydashboard"),
					title: locale.text("action.mydashboard")
				});
			}
			// 0 = don't show
			// 1 = show inactive
