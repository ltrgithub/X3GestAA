...
var _templates = {
	...
	tablet: {
		...
		rightLinks: '\
			{{#each links}}\
				<a draggable="false" {{#if $isHidden}}style="display:none"{{/if}} href="#" class="s-m-link {{css}}{{#if $isDisabled}} disabled{{/if}}" data-action="{{#if action}}{{action}}{{else}}footerLink{{/if}}" data-params="{{$uuid}}" data-control-id="{{../ctrlId}}">\
					<i class="{{icon}}"/>\
					<span>{{{title}}}</>\
				</a>\
			{{/each}}\
...
var _Klass = utils.defineClass(
	function CtrlFooterBase($type, controller, options) {
		...
		_addActionMgrHtmlCtx: function(ctx, actionController, displayController) {
			...
			if (rightLinks) {
				// FD - ordered link removed to have save button on the right - >  'templates' - 'cancelEdit' - 'save'
				var rlnk;
				for (var i = rightLinks.length - 1; i >= 0; i--) {
					rlnk = rightLinks[i];
					if (displayController.isEditMode() && rlnk.name === "$save") {
						this._saveLinkId = rlnk.$uuid;
						rlnk.$isDisabled = true;
