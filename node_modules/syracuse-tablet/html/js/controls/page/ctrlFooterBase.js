...
var _templates = {
	...
	tablet: {
		...
		rightLinks: '\
			{{#each links}}\
				<a draggable="false"  href="#" class="s-m-link {{css}}" data-action="{{#if action}}{{action}}{{else}}footerLink{{/if}}" data-params="{{$uuid}}" data-control-id="{{../ctrlId}}">\
					<i class="{{icon}}"/>\
					<span>{{{title}}}</>\
				</a>\
			{{/each}}\
...
var _Klass = utils.defineClass(
	function($type, controller, options) {
		...
		_addActionMgrHtmlCtx: function(ctx, actionController, displayController) {
			...
			if (this.footerOptions.showNativeShare) {
				ctx.nativeShare = _getHtml("tablet", "action", {
					action: "nativeShare",
					icon: fontUtils.pageIcon("nativeShare"),
					title: locale.text("action.nativeShare")
				});
			}
		},
