...
var _Klass = utils.defineClass(
	function(controller, options) {
		...
		notifications.subscribe(this, ["sm.page.pin.state.change", "sm.multiSelection"]);
	}, Base, {
		...
		_build: function(deviceType) {
			...
			this._height = globals.getSiteLayout().getHeight("footer", deviceType);
			this._$$parent.css({
				left: 0 + "px",
		...
		_tabletHtml: function(opts) {
			...
			if (self.actionManager) {
				// Device type can change
				self.actionManager.updateLinks("footer");
				self.multiSelManager.updateLinks();
			} else {
				self.multiSelManager = multiSelectionMgr.create(self.controller);
				self.actionManager = actionManager.create(self.controller, "footer", {
					addCancelEditAction: "footerLink",
					addMultiSelection: self.multiSelManager != null ? multiSelectionMgr.actionId : null
				});
			}
			self.$$elmt.find(".s-m-footer-right").remove();
			var rightLinks;
			if (self.actionManager.hasLinks()) {
				rightLinks = self.actionManager.getLinks();
			}
			...
			if (self._actionPanel && self._actionPanel.hasLinks()) {
			...
			if (opts.showPageConfig) {
				configLink = _getHtml("tablet", "pageConfig", {
		...
		/**
		 * When multiselManager is disabled
		 */
		notifMultiSelection: function(status) {
			if (!this.multiSelManager) return;
			if (status === "stop" && this.multiSelManager.isEnabled()) {
				this._multiSelToggle();
			}
		},
		_multiSelToggle: function() {
			var enable = this.multiSelManager.toggle(this.$$elmt.find(".s-m-footer-right"));
			this.$$elmt.find("a:not(.s-m-ismultisel)").toggle(enable !== true);
		}
	});
