...
var _Klass = utils.defineClass(
	function($type, controller, options) {
		...
		_actMultiSelection: function(uuid, $$target) {
			this.multiSelManager.toggle(this, this.$$elmt.find(".s-m-footer-right"), "dropup");
		},
		...
		_build: function(deviceType) {
			...
			var status = this.options.showHide ? "closed" : "opened";
			this._height = globals.getSiteLayout().getFooterHeight(deviceType, status);
			this._$$parent.css({
				left: 0 + "px",
		...
		_onSwipe: function(evt) {
			...
			if ((status === "closed" && action === "open") || (status === "opened" && action === "close")) {
				...
				this._height = globals.getSiteLayout().getFooterHeight(this._deviceType, status === "closed" ? "opened" : "closed");
		...
		_tabletHtml: function(opts) {
			var self = this;
			var _mainCtx = {};
			var mgrCreated = self.actionManager == null;
			if (mgrCreated) {
