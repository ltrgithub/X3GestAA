...
var _templates = {
			...
			<section class="panel panel-default create">\
				...
				<div class="panel-body">\
					<table style="width:100%"><tr>\
						<td>{{name}}</td>\
						<td>\
							<div class="input-group {{#if smartphone}}input-group-sm{{/if}}" style="padding-left:10px;padding-right:10px">\
								<input id="{{CREATE_BTN}}" class="ctrl-evt-input form-control" aria-describedby="create-clear-addon" type="text" style="font-weight:bold;width: 100%;"/>\
								<span  data-action="clearname" data-control-id="{{ctrlId}}" class="input-group-addon" id="{{CREATE_BTN_ADDON}}" style="background-color:inherit">X</span>\
							</div>\
						</td>\
						<td>\
							<span style="float:right;{{#if smartphone}}font-size:12px;{{/if}}" class="btn btn-default disabled" data-action="create" data-control-id="{{ctrlId}}">\
								<i class="{{createicn}}"></i>\
								<span>{{create}}</span>\
							</span>\
						</td>\
	...
	items: '\
		{{#each items}}\
			<li class="list-group-item" data-params="{{tmplId}}">\
				<table style="width:100%;"><tr>\
					<td style="width:100%;padding-right:5px;{{#if ../smartphone}}font-size:12px;{{/if}}">\
						<div style="font-weight:bold;" class="{{../ITEM_TTL}}" data-action="changeItemTitle" data-control-id="{{../ctrlId}}">{{title}}</div>\
						<div style="overflow: hidden"><small>{{date}}</small>{{#if endpoint}}<small style="float:right;max-width:50%;">{{endpoint}}</small>{{/if}}</div>\
					</td>\
					<td style="float:right">\
						<table style="width:100%;">\
							<tr>\
								<td style="padding:1px">\
									<span style="font-size:11px" class="btn btn-default {{#unless applicable}}disabled{{/unless}}" data-action="apply" data-control-id="{{../ctrlId}}">\
										<i class="{{../applyicn}}"></i>\
										<span>{{../apply}}</span>\
									</span>\
								</td>\
								<td style="padding-left:1px;">\
									<span style="font-size:11px" class="btn btn-default {{#unless applicable}}disabled{{/unless}}" data-action="update" data-control-id="{{../ctrlId}}">\
										<i class="{{../updateicn}}"></i>\
									</span>\
								</td>\
								<td style="padding:1px">\
									<span style="font-size:11px" class="btn btn-default" data-action="delete" data-control-id="{{../ctrlId}}">\
										<i class="{{../deleteicn}}"></i>\
									</span>\
								</td>\
							</tr>\
	...
	filtersMenu: '\
		<select style="float:right;{{#if smartphone}}font-size:12px;{{/if}};max-width:40%;" id="FILTER_MENU" data-action="filter" data-control-id="{{ctrlId}}">\
			{{#each filterOptions}}\
...
var _Klass = utils.defineClass(
	...
	function(controller, $type, panelId, options) {
		...
		_initLabels: function() {
			...
			this._filterOptions = [{
				value: "current",
				title: locale.text("edit.templates.dlog.currendpoint")
			}, {
				value: "all",
				title: locale.text("edit.templates.dlog.allendpoint")
			}];
		},
		destroy: function() {
			...
			globals.getStorage().templateOperation("count").then(function(count) {
				...
				templates.forEach(function(tmpl, idx) {
					context.items.push({
						...
						tmplId: tmpl.id,
						applicable: tmpl.representation === self.representation && tmpl.endpoint === ep,
						endpoint: self._selectedFilter === "all" ? tmpl.endpoint : null
					});
				});
