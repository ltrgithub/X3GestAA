...
var _panelLinks = {
	...
	templates: {
		label: "header.links.page.templates",
		icon: "templates",
		action: "templates",
		target: "page"
	}
...
var _Klass = utils.defineClass(
	...
	function(controller, $type, panelId, options) {
		...
		notifications.subscribe(this, ["sm.page.pin.state.change", "sm.drafts.changed", "sm.templates.changed"]);
		...
		onMainPageResize: function(info) {
		...
		_initPanelItems: function(page, options) {
			...
			this._panelLinks["templates"].hidden = !toolBars.showTemplates;
		 ...
		 * Manage the count info in 'List of draft' menu
		 */
		notifTemplatesChanged: function(count) {
			if (!this.$$elmt) {
				// Postpone the change if control not built
				this._afterBuildPush(jsutils.bindFn(this.notifTempatesChanged, this, count));
			} else {
				ctrlHelpers.notifTempatesChanged(this, count);
			}
		}
	});
