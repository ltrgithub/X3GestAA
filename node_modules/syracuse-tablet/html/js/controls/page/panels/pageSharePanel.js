...
var _Klass = utils.defineClass(
	...
	function(controller, $type, panelId, options) {
		...
		_colorChange: function(value) {
			...
			this.liveTile.backgroundColor = value ? getHexColor(value.bgColor) : null;
			// Color is set even if not needed - Code review
			this.liveTile.color = value ? getHexColor(value.color) : null;
		},
		/**
		...
		_actReminderCreate: function() {
			...
			if (schedule.date && schedule.time) {
				...
				notifications.publish("sm.page.share", "schedule", content);
				_scheduleMsgItems.forEach(function(item) {
		...
		_actSharePageLink: function() {
			notifications.publish("sm.page.share", "link");
			this.close("close");
		},
		_actLiveTileCreate: function() {
			...
			if (content) {
				...
				notifications.publish("sm.page.share", "liveTile", content);
			}
			this.close("close");
		}
	});
