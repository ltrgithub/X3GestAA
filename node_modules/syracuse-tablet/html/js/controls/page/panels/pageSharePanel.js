	...
	main: '\
			<nav class="navbar s-m-side-panel {{side}} navbar-inverse" role="navigation">\
				...
				{{#if share}}\
					...
					<div class="btn-group btn-group-justified" role="group">\
						<a type="button" href="#" class="btn btn-default" data-action="{{share.action}}">\
							<i class="{{share.icon}}"/>\
...
var _Klass = utils.defineClass(
	...
	function(controller, $type, panelId, options) {
		...
		_getContext: function() {
			...
			var context = page.getPageShareContext();
			var ctx = {
				ctrlId: this.id,
				side: this.side
			};
			...
			context.vignettes.forEach(function(vignette) {
		...
		_setData: function() {
			...
			if (context.vignettes) {
				...
				btnCreate.toggle(self.$$elmt.find("#vignette").val() !== "");
			}
		},
		...
		_actChoice: function(id) {
			...
			var value = this.$$elmt.find("#vignette").val();
			...
			if (itemDescription && !itemDescription.disabled && itemDescription.defaultValue !== undefined) {
				this.$$elmt.find("#description").val(value ? this.$$elmt.find("#vignette option:selected").text() : "");
			}
			btnCreate.toggle(value !== "");
		},
		_actDatePicker: function(id) {
			selectorFactory.createSelector(new _Field("application/x-date", this.$$panel.find("#" + id), locale.text("schedule.message." + id + ".label")));
		},
		_actTimePicker: function(id) {
			selectorFactory.createSelector(new _Field("application/x-time", this.$$panel.find("#" + id), locale.text("schedule.message." + id + ".label")));
		},
		_actReminderCreate: function() {
			...
			if (content) {
				notifications.publish("sm.page.share.changed", content);
				_scheduleMsgItems.forEach(function(item) {
			...
			_liveTile.forEach(function(item) {
				if (!item.disabled) {
					if (self.liveTile && self.liveTile[item.id]) {
						...
						content.liveTile[item.id] = self.$$panel.find("#" + item.id).val();
					}
				}
			});
