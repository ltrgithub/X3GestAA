...
var FormKlass = utils.defineClass(
	function(proto, dao, gestureMgr, filtersMap) {
				...
				switch (ctrl.$bind) {
					case "FIELDS":
						...
						if ($$target.val() == 0) {
							...
							// set $isHidden: false for value ctrl to display
							// in case of reference field, key is different
							var keyToShow = ""; //"VALUE-"+self.filtersMap[$$target.val() - 1].id;
							if (fm.isReference) {
								keyToShow = fm.prop.$item.$refValue;
							} else {
								keyToShow = "VALUE-" + fm.id;
							}
		...
		validateForm: function() {
			var self = this;
			var res = true;
			// check control of displayed ctrls (except OPERATORS choice list)
...
var _Klass = utils.defineClass(
	function(parentTab) {
		...
		create: function($$container) {
			...
			//console.log("protoJson : " + JSON.stringify(protoJson, null, 2));
			console.log("dataJson : " + JSON.stringify(dataJson, null, 2));

			/*var refFieldData = {};
			var filtersMap = self._getFiltersMap();
			for (var ii=0,jj=filterMaps.length;ii<jj;ii++){
				var ff = filtersMap[ii];
				if (ff.isReference){
					refFieldData[ff.id] = {
						$isHidden: true
					}
			...
			//dataJson = $.extend(dataJson,refFieldData);
		...
		_buildProtoJson: function() {
			...
			for (var ii = 0, jj = self.filtersMap.length; ii < jj; ii++) {
				...
				if (ff.isReference) {
					var kk = ff.prop.$item.$refValue;
					var kkk = (ff.prop.$item.$key.replace("{", "")).replace("}", "");
					protoJson.$properties[kk] = {
						$type: ff.prop.$item.$properties[kkk].$type,
						$isHidden: true,
						$isExcluded: true
					};
					//debugger;
			...
			if (self.controller.validateForm()) {
				...
				*/
				...
				var info = {
					...
					value: typeof value === 'object' ? value.$value || value.$description || value.$title : value,
					operator: operator,
		...
		*/
		setInitData: function(info, initIdx) {
			...
			self.initData["VALUE-" + self.filtersMap[filterMapIdx].id] = info.originalValue;
			self.initData.$properties["VALUE-" + self.filtersMap[filterMapIdx].id] = {
				$isHidden: false
			};


			// set $isHidden property to true for all the value ctrls
			/*for (var ii = 0, jj = self.filtersMap.length; ii < jj; ii++) {
				var ff = self.filtersMap[ii];
				var key = "VALUE-" + ff.id;
				self.initData.$properties[key] = {
					$isHidden: true
				};
				if (ff.isReference) {
					var kk = ff.prop.$item.$refValue;
					var kkk = (ff.prop.$item.$key.replace("{", "")).replace("}", "");
					self.initData.$properties[kk] = {
						$isHidden: true
					};
					//debugger;
				}
