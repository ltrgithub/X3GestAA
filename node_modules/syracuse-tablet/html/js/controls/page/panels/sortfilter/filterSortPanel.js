...
var _Form = require('syracuse-tablet/html/js/controls/page/panels/sortfilter/tabForm').Klass;
...
var _Klass = utils.defineClass(
	function(controller, $type, panelId, options) {
		...
		buildHtml: function(side) {
			...
			if (self.$$panel && self.$$panel.length) {
				...
				if (visibleTabs.length > 0) {
					...
					self.$$btnBar.on('click', '[data-params]', utils.bindFn(this.onBtnBarClick, this));
				} else {
					this.close();
				}
			}
		...
		/**
		 * Notification of a lookup selection when the panel contains a x-reference field (filters)
		 */
		notifLookupSelection: function(controlId, rowData) {
			if (this._activeTab) {
				this._activeTab.notifLookupSelection(controlId, rowData);
			}
		},
		getControl: function(id) {
			return this._activeTab ? this._activeTab.getControl(id) : null;
		}
	});
