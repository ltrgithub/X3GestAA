...
var _mustHideSoftwareBackButton = function() {
	if (_hideSoftwareBackButton == null) {
		_hideSoftwareBackButton = native.hasCapability("hardwareBackButton") == true;
	}
...
var _Klass = utils.defineClass(
	...
	function CtrlHeaderBase($type, controller, options) {
		...
		_build: function() {
			...
			if (this.headerOptions.showDashboardList) {
				notifications.subscribe(this, ["sm.view.change"]);
			}
			this._updateWidth();
		},
		/**
		...
		_tabletHtml: function() {
			...
			var _mainCtx = {
				...
				headerLabel: self.headerOptions.title || self.getProtoTitle(),
				ctrlId: self.id
			};
		...
		_smartPhoneHtml: function(opts, displayController, actionController) {
			...
			var linksContent = "";
			// build label content. 
			labelContent = self.headerOptions.title || self._getLabelContent(opts);
			// build links content.
