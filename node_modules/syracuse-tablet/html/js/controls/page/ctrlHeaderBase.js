...
var _templates = {
	...
	tablet: {
		...
		userInfo: '\
			...
			<span style="font-size:32px; vertical-align: 5px;" class="' + fontUtils.pageIcon("switchContext") + '" />\
			',
		infoOnly: '\
			<div>{{{userInfo}}}</div>',
		dropDown: '\
	...
	smartphone: {
		...
		actionLink: '\
			<a href="#" draggable="false" class="s-m-link {{css}}" {{#if $isHidden}}style="display:none"{{/if}} data-action="{{#if action}}{{action}}{{else}}headerLink{{/if}}" data-params="{{$uuid}}" data-control-id="{{ctrlId}}">\
				<i class="{{icon}}"></i>\
			</a>',
		actionPanelToggle: '\
...
var _mustHideDevFeatures = function() {
	...
	_hideDevFeatures = native.hasCapability("nativeApp");
};
...
var _Klass = utils.defineClass(
	...
	function CtrlPageHeader($type, controller, options) {
		...
		getHeaderOptions: function(buildOptions) {
			...
			opts.showDevelopment = _mustHideDevFeatures() === true ? false : globals.isDvlpMode();
			opts.showAuthoring = (utils.isDesktop() === true && _mustHideDevFeatures() === false) ? opts.showAuthoring : false; // Show full feature authoring only if on PC
			// Authoring of chart detail page needs to be done in parent dashboard
			if (this.controller.state.type === "chartdetail") {
				opts.showAuthoring = false;
			}
			return opts;
		},
		...
		_build: function() {
			...
			this.$$header.find(".s-m-freshness").hide();
			if (this.controller.freshness) {
				//trigger DataFreshnessChange for regular page that are not in dashboard because data 
				//are loader before header is available
				this.notifDataFreshnessChange(this.controller.freshness);
			}
		},
		/**
		...
		_smartPhoneHtml: function(opts, displayController, actionController) {
			...
			if (opts.showUserpanel) {
				self._smphUserpanel = displayController.panelCreate(_userPanelId, opts);
			}
			var showActions = opts.showActions !== false;
			if (showActions && opts.showActionpanel) {
				self._smphPageactionpanel = displayController.panelCreate(_actionPanelId, opts);
			}
			var backDom = "";
			var labelContent = "";
			var linksContent = "";
		...
		_getSmartphoneLinksContent: function(displayController, actionController, hasPanelLinks, showActions) {
			...
			this._actionManager = actionManager.create(actionController, "header", {
				...
				addMultiSelection: this._multiSelManager && !this.headerOptions.multiSelectionOnly && actionController.isMultiSelectionEnabled(),
				multiSelectionOnly: this.headerOptions.multiSelectionOnly
			});
		...
		_actMultiSelection: function(uuid, $$target) {
			...
			var enabled = this._multiSelManager.toggle(this, $$hLinks, "dropdown");
		...
		notifDataFreshnessChange: function(data) {
			var $$freshness = this.$$header.find(".s-m-freshness");
			this.dataFreshness = this.dataFreshness || {
				oldest: data.dateTime,
				recent: 0
			};
			this.dataFreshness.oldest = Math.min(this.dataFreshness.oldest, data.dateTime);
			this.dataFreshness.recent = Math.max(this.dataFreshness.recent, data.dateTime);
			var diffDays = Math.round((new Date().getTime() - this.dataFreshness.oldest) / (24 * 60 * 60 * 1000));
			var freshness = "verytainted";
			if (diffDays <= 7) freshness = "tainted";
			if (diffDays <= 3) freshness = "ratherfresh";
			if (diffDays <= 1) freshness = "fresh";
			this.dataFreshness.value = freshness;
			$$freshness.removeClass("s-m-fresh s-m-ratherfresh s-m-tainted s-m-verytainted");
			$$freshness.addClass("s-m-" + freshness);
			$$freshness.show();
		},
		_actShowFreshness: function() {
			var formatter = formatApi.getFormatter("application/x-datetime");
			var dateTime = formatter.formatValue(utils.getCurISODateTime(new Date(this.dataFreshness.oldest)), false);
			var notify = {
				severityClass: this.dataFreshness.value,
				title: locale.text("page.data.freshness.last") + " : " + dateTime,
				body: locale.text("page.data.freshness." + this.dataFreshness.value),
				onlyWebapp: true
			};
