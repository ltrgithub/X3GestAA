		...
		dropDownMenu: '\
			<ul class="dropdown-menu" role="menu" aria-labelledby="{{dropdownId}}">\
				{{#each links}}\
					...
					{{/if}} \
					<li>\
						<a href="#" {{#if data-nav}}data-nav="{{data-nav}}"{{else}}data-action="linkClicked" data-params="{{data-params}}"{{/if}} draggable="false" data-control-id="{{ctrlId}}" class="list-group-item {{css}}">\
							<span class="{{icon}}" />\
...
var _Klass = utils.defineClass(
	...
	function CtrlPageHeader($type, controller, options) {
		...
		_build: function() {
			...
			this._$$freshness = this.$$header.find(".s-m-freshness").hide();
		},
		/**
			...
			if (showActions && linksArr && linksArr.length > 0) {
				...
				linksArr.forEach(function(ll, idx) {
					cc.push(_getHtml("smartphone", "actionLink", $.extend(ll, {
						ctrlId: self.id,
						$isHidden: ll.action === "multiSelection" || ll.$isHidden === true
					})));
				});
			}
