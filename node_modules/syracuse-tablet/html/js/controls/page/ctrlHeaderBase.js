...
var nativeTestVoiceCommmands = native.isTestMode() ? require('syracuse-tablet/html/js/helpers/native/native').getModule("voiceCommands") : null;
...
var _Klass = utils.defineClass(
	...
	function CtrlPageHeader($type, controller, options) {
		...
		_build: function(info) {
			...
			if (nativeTestVoiceCommmands) {
				nativeTestVoiceCommmands.appendTestVoiceCommandsMenu(this.$$header.find(".s-m-header-nav"));
			}
		...
		_updateHeaderStyle: function(deviceType) {
			...
			var $$label = self.$$elmt.find(".s-m-header-labels");
			var offset = 10;
			...
			// smartphone
				...
				if (false && !self.controller.state.isWelcomePage) {
		...
		_tabletHtml: function() {
			...
			var _mainCtx = {
				...
				headerLabel: "Texte très long qui doit dépasser les limites du panel alloué à l'affichage de cette information vraiment très importante" //self.controller.prototype.data("$title")
			};
		...
		_smartPhoneHtml: function(opts, displayController, actionController) {
			...
			// build label content. 
			labelContent = "Texte très long qui doit dépasser les limites du panel alloué à l'affichage de cette information vraiment très importante" //self.controller.prototype.data("$title");
