...
var _Klass = utils.defineClass(
	...
	function CtrlPageHeader($type, controller, options) {
		...
		notifications.subscribe(this, ["sm.page.data.loaded"]);
	}, Base, {
		...
		_build: function() {
			...
			this.$$header.find(".s-m-freshness").toggle(false);
		},
		...
		notifPageDataLoaded: function(data) {
			...
			var colorClass = "s-m-verytainted";
			if (diffDays <= 7) colorClass = "s-m-tainted";
			if (diffDays <= 3) colorClass = "s-m-ratherfresh";
			if (diffDays <= 1) colorClass = "s-m-fresh";
			var $$freshness = this.$$header.find(".s-m-freshness");
			$$freshness.removeClass("s-m-fresh s-m-ratherfresh s-m-tainted s-m-verytainted");
			$$freshness.addClass(colorClass);
			$$freshness.toggle(true);
			console.log("notifPageDataLoaded : " + new Date(this.dataFreshness.oldest).toString() + " " + new Date(utils.getUTCTime(new Date())).toString() + " " + diffDays);
		},
		/**
