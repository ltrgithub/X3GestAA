...
var _templates = {
	main: '\
			<div class="s-m-header container-fluid {{device}}">\
	...
	dropDownMenu: '\
		<ul class="dropdown-menu" role="menu" aria-labelledby="{{dropdownId}}">\
			{{#each links}}\
				...
				{{/if}} \
				<li>\
					<a href="#" data-action="linkClicked" data-params="{{data-params}}" data-control-id="{{ctrlId}}" class="list-group-item {{css}}">\
						<span class="{{icon}}" />\
						<span>{{label}}</span>\
					</a>\
				</li>\
...
var _Klass = utils.defineClass(
	...
	function(controller, options) {
			...
			for (var p in _linksCtx) {
				...
				if (opts[p] === true) {
					...
					l.items.forEach(function(x, idx) {
						...
						if (idx === 0 && linksCpt > 1) x.divider = l.divider;
		...
		_buildSm: function() {
			...
			var getHtml = function(name, ctx, templs) {
				var key = name + "compiled";
				var tmpl = templs[key];
				if (!tmpl) {
					tmpl = templs[key] = Handlebars.compile(templs[name]);
				}
				return tmpl(ctx);
			};
			...
			if (opts.showHome && globals.getLookAndFeel() === "ios") {
				backDom = getHtml("backDom", {}, templates);
			}

			// build label content. 
			// Label depends whether user is located in home page, app dashboard page, or anywhere else

			// home page --> brand name
			if (!opts.showHome) {
				labelContent = getHtml("brandDom", {
					labelBrand: opts.labelBrand || "Sage",
					labelProduct: opts.labelProduct || "ERP X3",
				}, templates);
			} else {
				// app dashboard page --> app title
				// other case --> page title
				labelContent = getHtml("labelDom", {
					label: self.controller.state.type === "isHomeDashboard" ? globals.getApplication().getTitle() : self.controller.prototype.data("$title")
				}, templates);
			...
			self.$$header = $(getHtml("main", ctx, templates));
			self.$$header.appendTo(self.$$elmt);
		},
