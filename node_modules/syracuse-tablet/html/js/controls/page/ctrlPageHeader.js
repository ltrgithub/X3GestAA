...
var _Klass = utils.defineClass(
	...
	function($type, controller, options) {
		...
		_smartPhoneHtml: function(opts) {
			...
			// build links content.
			linksContent = self._getSmartphoneLinksContent(self._smphPageactionpanel && self._smphPageactionpanel.hasLinks());
			var ctx = {
				backDom: backDom,
		 ...
		 */
		_getSmartphoneLinksContent: function(hasPanelLinks) {
			var linksContent = "";
			this._multiSelManager = multiSelectionMgr.create(this.controller, ".s-m-header-nav, .s-m-header-labels, a:not(.s-m-ismultisel)");
			notifications.subscribe(this, ["sm.panel.multiselection.toggle"]);
			this._actionManager = actionManager.create(this.controller, "header", {
				addCancelEditAction: "headerLink",
				addMultiSelection: this.controller.isMultiSelectionEnabled()
			});
