...
var _templates = {
	main: '\
		<div class="s-m-header container-fluid {{typeDevice}}">\
			{{{contentHtml}}}\
		</div>\
	',
	tablet: {
	...
	smartphone: {
		...
		labelDom: '\
			<span>{{label}}</span>',
		actionLink: '\
			<a href="#" class="{{css}}" data-action="headerLink" data-params="{{$uuid}}" data-control-id="{{ctrlId}}">\
				<i class="{{icon}}"></i>\
			</a>',
		actionPanelToggle: '\
	...
	function(controller, options) {
		...
		_updateHeaderStyle: function(deviceClass) {
			...
			if (typeDevice === "tablet") {
				...
				// for pages without brand name for the moment
				// update label width
		...
		_getLinksContent: function() {
			...
			function getLinksToDisplay($facet, links) {
				var allowed = {
					details: {
						$edit: true,
						$delete: true
				...
				for (var ii = 0, jj = links.length; ii < jj; ii++) {
					if (allowed[$facet][links[ii].name]) {
						arr.push(links[ii]);
					}
				}
				return arr;
			...
			var allowedLinks = getLinksToDisplay(this.controller.$facet, linksArr);

			if (allowedLinks && allowedLinks.length > 0) {
				var cc = [];
				allowedLinks.forEach(function(allowed, idx) {
					cc.push(_getHtml("smartphone", "actionLink", $.extend(allowed, {
						ctrlId: self.id
					})));
				});
