...
var _Klass = utils.defineClass(
	...
	function ctrlDashboardConfigHeader($type, controller, options) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			//keep personal dashboard description is case of user replace its own dashbord by a template.
			...
			if (this.options.views) {
				Object.keys(this.options.views).forEach(function(key) {
					...
					if (self.options.views[key].$isPersonal) {
						personalDescription = self.options.views[key].$description;
					}
				});
			}
			...
			// 		options.savePublish : user can save a template
			options.buttonSaveAction = options.personal ? "savePersonalDashboard" : (options.savePublished ? "saveTemplateDashboard" : "replacePersonalDashboard");
			options.displaySaveButton = "none"
				// If user want to replace its own dashboard by a templtae display by default the save button and keep its own title
			if (options.buttonSaveAction === "replacePersonalDashboard") {
				options.displaySaveButton = ""
				options.description = personalDescription;
				options.factoryOwner = "";
			};
