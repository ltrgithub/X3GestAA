	...
	main: '\
			<div class="header">\
				<buttons class="pull-left">\
					<button type="button" class="btn btn-default fa fa-arrow-left" data-controller-id="{{controllerId}}" data-control-id="{{ctrlId}}" data-nevent="" data-naction="cancel" />\
				</buttons>\
				<buttons class="pull-right">\
					<button type="button" class="btn btn-default ' + fontUtils.sortFilterTab("memo") + '" data-controller-id="{{controllerId}}" data-control-id="{{ctrlId}}" data-nevent="" data-naction="memo"/>\
					<button type="button" class="btn btn-default ' + fontUtils.sortFilterTab("apply") + '" data-controller-id="{{controllerId}}" data-control-id="{{ctrlId}}" data-nevent="" data-naction="apply"/>\
				</buttons>\
...
exports.PanelFilterSort = utils.defineClass(
	function PanelFilterSort(controller, $type, article, options) {
		...
		destroy: function() {
			this.$$elmt.closest("aside").removeClass("sortfilter");
		...
		buildHtml: function(side) {
			...
			var ctx = {
				...
				ctrlId: self.id,
				controllerId: self.controller.id,
				side: side,
		...
		applyTabSettings: function() {
			...
			this.parentArray.filterSort.applyFilterSort();
			return self.savePagePrefs().then(function(close) {
				if (close) {
					self.hide();
				}
			}).fail(function(error) {
		...
		unselectMemo: function() {
			this._tabs["memo"].unSelectCurrent()
		},
		...
		show: function() {
			...
			this.$$elmt.closest("aside").addClass("sortfilter");
			this._makeVisible();
		...
		favoriteChange: function(favorite) {
			this._tabs["filter"].filterInfo = $.extend(true, [], favorite.filter)
			this._tabs["sort"].sortInfo = $.extend(true, [], favorite.sort)
		},
		_actCancel: function() {
			this.hide();
		},
