...
exports.PanelShare = utils.defineClass(
	function PanelShare(controller, $type, article, options) {
		...
		_liveTile: function() {
			...
			if (this._publishOptions.liveTile.pages.length > 1) {
			...
			if (self._context.vignette) {
				if (self._context.singleVignette) {
					...
					_liveTile.forEach(function(item) {
						if (!item.disabled) {
							if (!item.component) {
								self.$$panelContent.find(".s-m-livetile #" + item.id).val("");
							}
						}
					});
				}
			...
			_liveTile.forEach(function(item) {
				if (!item.disabled) {
					if (self.liveTile && self.liveTile[item.id]) {
						...
						content[item.id] = self.$$panelContent.find(".s-m-livetile #" + item.id).val();
					}
				}
			});
			if (content) {
...
function _getShareDeepLink(page) {
	return sharedContext.getContextUrlByPage(page)
		.then(function(url) {
			var link = {
