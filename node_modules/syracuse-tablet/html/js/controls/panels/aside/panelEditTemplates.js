...
function _readTemplates(sdataPage, filter) {
	var storage = modules.get("storage").getStorage();
	return storage.templateOperation("count").then(function(totalCount) {
		var queryRep = filter === "all" ? null : sdataPage.getRepresentation();
		var queryEp = filter === "all" ? null : _getEndPoint();
		return storage.templateOperation("query", queryRep, queryEp, true).then(function(templates) {
			return $.smResolve(totalCount, templates || []);
		})
	})
}
var _templates = {
	...
	main: '\
		<article style="height:100%">\
			<section class="s-m-memo create">\
				<header>\
					<span>{{createSection}}</span>\
		 		</header>\
				<div class="panel-body">\
					<table style="width:100%"><tr>\
						<td class="s-m-input-label"><span style="padding-right:10px;">{{name}}</span></td>\
						<td>\
			...
			<section class="s-m-memo avail">\
				<header>\
					<span>{{availSection}}</span>\
				</header>\
				<div class="panel-body s-m-filter">\
					<table style="width:100%;">\
					<tr>\
					<td class="s-m-input-label"><span>{{label_endpoint}}</span></td>\
					<td class="s-m-filter-container"></td>\
					</tr>\
					</table>\
				</div>\
				<div class="{{LST_WRAP}}">\
...
exports.PanelEditTemplates = utils.defineClass(
	function PanelEditTemplates(controller, $type, article, options) {
		...
		buildHtml: function() {
			...
			this.$$list = this.$$content.find(".s-m-memo.avail ul");
		},
		/**
		...
		_buildContent: function() {
			var context = {
				...
				name: _txt("name"),
				label_endpoint: _txt("label.endpoint"),
				create: _txt("create"),
		...
		_buildItems: function() {
			...
			var self = this;
			_readTemplates(this.page, this._selectedFilter).then(function(totalCount, templates) {
				...
				if (!self._$$filter) {
					self._$$filter = $(self._buildFilterMenu()).appendTo(self.$$content.find(".s-m-memo.avail > .s-m-filter  .s-m-filter-container"));
				}
				self._$$filter.val(self._selectedFilter);
				var filterContainer = $(".edittemplates  .s-m-filter");
				filterContainer.toggle(self._selectedFilter === "all" || totalCount > templates.length);
				filterContainer.toggle(templates.length > 0);
		...
		_actApply: function(param, event) {
			...
			this.getStorage().templateOperation("read", tmplId).then(function(data) {
				self.hide();
				if (data) {
					self.page.getDataset().setEditTemplateData(self.page, data.dataSet);
				}
			}).fail(function(e) {
		...
		_walkCtrls: function(controller, data) {
			controller.forEachControl(function(c) {
		...
		_actUpdate: function(param, event) {
			...
			this.getStorage().templateOperation("update", tmplId, {
				dataSet: self.page.getDataset().getEditTemplateData(self.page.controller)
			}, false).then(function() {
				self.hide();
		...
		_actCreate: function(param, event) {
			var ctx = {
				...
				dataSet: this.page.getDataset().getEditTemplateData(this.page.controller)
			};
			var self = this;
		...
		_actChangeItemTitle: function(param, event) {
			var tmplId = this._getTmplId(event); // Id current clicked
			if (!tmplId) return;
			if (!this._$$changeItemTitle) { // create input
				...
				$(".s-m-tmpl-itm-ttl", this._$$changeItemTitle.parent()).show();
			}
			...
			this.refreshSaveItemTitleBtn(this._$$changeItemTitle.find("#" + ITEM_INPUT_TTL).val($$target.text())); //set the input value
		},
		/**
		...
		onClickSaveItemTitle: function(event) {
			...
			var tmplId = self._getTmplId(event, true);
			if (!tmplId) return;
		...
		refreshCreateBtn: function(val) {
			if (!this._$$create) {
				this._$$create = this.$$content.find('.s-m-memo.create .btn[data-naction="create"]');
				this._$$input = this.$$content.find('.s-m-memo.create input[type="text"]');
			}
			if (val != null) {
		...
		hide: function() {
			Base.prototype.hide.call(this);
			if (this._$$changeItemTitle) {
				this._$$changeItemTitle.find("span").off('click');
				this._$$changeItemTitle.empty();
				this._$$changeItemTitle = null;
			}
			exports.publishEditTemplateStatus(this.page);
		}
	});
/**
 * Notifies subscribers that status has changed (button in header/footer to change the icon)
 */
exports.publishEditTemplateStatus = function(pageSdata) {
	if (!pageSdata) {
		return;
	}
	_readTemplates(pageSdata).then(function(totalCount, templates) {
		notifications.publish("sm.edit.templates.changed", templates.length === 0);
	}).fail(function(e) {
		console.log("publishEditTemplateStatus Error");
