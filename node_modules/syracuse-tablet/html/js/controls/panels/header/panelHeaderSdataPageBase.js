	...
	mainContainer: '\
		<div class="s-m-header {{device}}">\
			...
			<div class="s-m-header-links">\
				...
				{{/if}}\
				{{#if showActionMultiSelectionPanel}}\
				<a class="s-m-header-link" href="#" draggable="false" \
					data-nevent\
					data-naction="openActionsMultiSelectionPanel">\
					<i class="fa fa-ellipsis-v"></i>\
				</a>\
				{{/if}}\
			</div>\
...
exports.PanelHeaderSdataPageBase = utils.defineClass(
	function PanelHeaderSdataPageBase(controller, type, article, opts) {
		...
		notifications.subscribe(this, ["sm.all.vignettes.loaded"]);
		notifications.subscribe(this, ["sm.page.loaded"]);
	}, Base, {
		...
		_headerLinksVisible: function() {
			...
			var titleWidth = this.$$title.outerWidth() || 0;
			...
			$$list.reverse().each(function(idx) {
				var show = true;
				var forceHeader = $(this).is("[data-forceheader]") || ($(this).attr("data-menuitem-id") == "search")
			...
			if (titleWidth != initialWidth) this.$$title.css("max-width", titleWidth + "px");
			if (showSearch) {
				this._showSearch();
			}
			return lists;
		...
		notifPageLoaded: function(page) {
			if (page.isDashboard() && !this.allVignettesLoader) {
				$(".s-m-header-nav > a", this.$$elmt).attr("disabled", true);
			}
		},
		notifAllVignettesLoaded: function() {
			this.allVignettesLoader = true; // to prevent the case where this notification is sent before page loaded notif
			$(".s-m-header-nav > a", this.$$elmt).removeAttr("disabled");
		}
	});