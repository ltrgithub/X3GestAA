 ...
 * 
 */
var _availableSlots = ["main", "right", "unclassified"]
...
var _html = {
	mainContainer: '\
			...
			{{/if}}\
			<div class="s-m-header-links" style="height:{{height}}">\
				{{#if showActionPanel}}\
...
exports.PanelHeaderSdataPageBase = utils.defineClass(
	function PanelHeaderSdataPageBase(controller, type, article, opts) {
		...
		buildHtml: function() {
			...
			this.$$links.append($$itemsBlocksHtml.html());
			this._checkBack();
		},
		...
		_responsiveHeader: function() {
			...
			var $$links = this.$$elmt.find(".s-m-header-links");
			var actionPanelButton = $('[data-naction="openActionsPanel"]', $$links);
		...
		_headerLinksVisible: function() {
			...
			var $$title = this.$$elmt.find(".s-m-header-labels");
			$$title.css("display", "");
			var titleWidth = $$title.width();
			var initialWidth = titleWidth
			var $$links = this.$$elmt.find(".s-m-header-links");
			var $$btnAction = $(".s-m-header-link", $$links);
			var availableWidth = $$links.width() - $$btnAction.outerWidth();
			var $$list = $(".s-m-link", $$links);
			$$list.each(function(idx) {
				var show = true;
				var forceHeader = $(this).is("[data-forceheader")
				if ((availableWidth - $(this).outerWidth()) < 0) {
					// can reduce title ?
					if (titleWidth > $(this).outerWidth()) {
