...
exports.PanelSdataPage = utils.defineClass(
	function PanelSdataPage(controller, type, article, opts) {
		...
		notifications.subscribe(this, ["sm.pin.status.change"]);
	}, Base, {
		destroy: function() {
			Base.prototype.destroy.call(this);
			notifications.unsubscribe(this, ["sm.pin.status.change"]);
		},
		///
		/// START: Rendering
		///
		...
		hideAction: function(itemId) {
			var $$item = this.$$elmt.find('[data-menuitem-id="' + itemId + '"]');
			$$item.addClass("hidden");
		},
		showAction: function(itemId) {
			var $$item = this.$$elmt.find('[data-menuitem-id="' + itemId + '"]');
			$$item.removeClass("hidden");
		},
		notifPinStatusChange: function(state) {
			...
			var actionHide = (state) ? "pinPage" : "unpinPage";
			var actionShow = (state) ? "unpinPage" : "pinPage";
			this.hideAction(actionHide);
			this.showAction(actionShow);
