...
exports.PanelBase = utils.defineClass(function PanelBase(controller, type, article, opts) {
	...
	enableAction: function(action, yes) {
		var $$act = this.$$elmt.find('[data-naction="' + action + '"]');
		if ($$act.length == 0) {
			return;
		}
		$$act.toggleClass("disabled", yes === false);
		if (yes) {
	...
	showAction: function(action, yes) {
		var $$act = this.$$elmt.find('[data-naction="' + action + '"]');
		if ($$act.length == 0) {
			return;
		}
		$$act.toggle(yes !== false);
	}
});