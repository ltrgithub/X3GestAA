...
var _Klass = utils.defineClass(
	function StructElmt(controller, $type, article, options) {
		...
		generateHtml: function($$parent, controllerDao, htmlRootCtrl) {
			...
			/**
			 * We store the dao  (see getDao)
			 */
			this._htmlGeneratorDao = controllerDao;
			if (htmlRootCtrl) {
			...
			if (this.afterRender) {
				// To apply meta to the current elmt
				this.afterRender();
			}
			this._htmlGeneratorDao = null;
			return this.$$elmt;
		},
		/**
		...
		getDao: function() {
			/**
			 * We pass controllerDao in buildhtml but some methods use getDao in case controllerDao is not passed as a parameter (row data in arrays)
			 * _htmlGeneratorDao is different from this.controller.dao in that mode (rowdada)
			 */
			return this._htmlGeneratorDao ? this._htmlGeneratorDao : this.controller.dao;
		},
		/**
