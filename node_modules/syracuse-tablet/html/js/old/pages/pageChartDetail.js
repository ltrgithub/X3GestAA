...
var _Page = utils.defineClass(
	...
	function ChartDetailPage($parent, state, prototype, article, parentPage, options) {
		...
		this._mainParentPage = this._findMainPage();
	}, Base, {
		/*
		 * This page has no ID for settings
		 * like this, no user preferences are read since for chart detail pages, the settings
		 * for every chart level are stored in the vignette of the parent dashboard
		 */
		...
		_setDisplay: function() {
			...
			var prefs = this._getChartDetailPrefs();
			var opts = {
		...
		/*
		 *  For all levels of drill down, user settings are stored in the settings of the main page
		 *  the main page is the first page upwards the tree that is not a chart details page
		 */
		_findMainPage: function() {
			var parent = this.parentPage;
			while (jsutils.isInstanceOf(parent, "ChartDetailPage")) {
				parent = parent.parentPage;
			}
			return parent;
		},
		_getChartDetailPrefs: function() {
			var settings = this._mainParentPage.getPageSettings();
			var levelSettings = settings.chartDetail && settings.chartDetail[this.state.level];
			return levelSettings || {};
		},
		_setChartDetailPrefs: function(prefs) {
			var settings = this._mainParentPage.getPageSettings();
			settings.chartDetail = settings.chartDetail || {};
			settings.chartDetail[this.state.level] = prefs;
			settings.isDirty = true;
		},
		actToolbarSetDisplay: function(opts) {
			this._setChartDetailPrefs(opts);
			this._setDisplay();
			this._mainParentPage.writePageSettings()
				.then(function() {
