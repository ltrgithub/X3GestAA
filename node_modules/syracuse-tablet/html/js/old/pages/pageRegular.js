...
var filterHelper = require('syracuse-tablet/html/js/sdata/filterHelper');
...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		this._originalSdataUrl = utils.checkUrlHostAndPort(this._originalSdataUrl)
			//this._init$Filters();
		this._initSdataInfo();
		...
		_draftEnabled: function() {
			if (!this.isEditMode() || this.isChild) {
				return false;
			}
			return this.prototype && this.prototype.getDataByPath("$links.$save");
		},
		...
		pageInitAsync: function() {
			...
			self.pagePrefs = {};
			var step;
			if (self.gadget && self.gadget.data("pinPageId") != null) {
				// Read the savedContext of the pinned page that contains filters/sort/search in to restore them
				step = dashboardWelcomeUtils.getPinnedPageContext(self.gadget.data("pinPageId"))
			} else {
				step = $.smResolve()
			}
			return step.then(function(savedContextPinnedPage) {
				if (savedContextPinnedPage) {
					// Restore saved context
					self.state.savedContext = savedContextPinnedPage;
				}
				return self.readPageSettings();
			}).then(function() {
				// Init after having read the context
				self._init$Filters();
			})
		},
		/* 
		 ...
		 * When the key is empty, the page is assumed to have not settings and nothing is read
		 */
		...
		_initSdataInfo: function(sdataUrl) {
			...
			sdataUrl = utils.checkUrlHostAndPort(sdataUrl)
		...
		activate: function(pageToClose, cb) {
			...
			notifications.publish("sm.data.freshness.change", this.dao && this.dao.dataFreshness());
		},
		/**
		...
		getRestoreContext: function(reason) {
			...
			if (!ctx) return ctx;
			if (!$.isEmptyObject(this.state.options.sdataParameters)) {
		...
		savedCtxCreate: function(type) {
			...
			if (type != "link" || !this._filterSortAllowed()) {
				// We don't save filterSortInfo becaus it is stored in user preferences
				return ctx;
			}
			// For a link context we save the filterSortInfo
			var sortInfo = this.getPageSort();
			if (sortInfo && sortInfo.some(function(i) {
					if (i.sort != "none") {
						return true;
					}
				})) {
				// Save only there's a not none entry
				ctx.sortInfo = sortInfo
			}
			var filterInfo = this.getPageFilter();
			if (filterInfo != null && filterInfo != "nofilter") {
		...
		_filterSortUpdateUrl: function() {
			...
			var filterClause = filterHelper.getFilterWhereClause(info);
			if (!filterHelper.isValidWhere(filterClause)) {
				filterClause = null;
				filterMsg = true;
			}
			// build search clause
			var searchInfo = self.searchInfoRead();
			var searchClause = filterHelper.getSearchClause(searchInfo);
			if (!filterHelper.isValidWhere(searchClause)) {
				searchClause = null;
				filterMsg = true;
			}
			if (filterMsg) {
				var notify = {
					...
					title: locale.text("error.filter.invalid")
				};
				globals.getModal().notify(notify);
			}
