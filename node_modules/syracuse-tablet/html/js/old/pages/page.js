...
var _templates = {
	...
	panels: '\
		<div class="s-m-side-panel-dismiss" data-action="toggleSidePanel" data-params="close,all" data-nevent data-naction="closePanel"></div> \
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		this.views = options ? options.views : null;
		if (this.isChild) {
		...
		_initRootElmt: function($$parent) {
			...
			if (self.isVignette) {
				...
				// Parent vignette containing this page may be destoryed because of navigation event
				// so check first here
				if (!self.state.options.parentVignette.destroyed) {
		...
		loadStructure: function(loadOptions) {
			...
			return $.smResolve()
				.then(function() {
					self.createInsidePageControls(loadOptions);
		...
		createInsidePageControls: function(loadOptions) {
			var self = this;
		...
		buildHtmlOption: function(loadOptions) {
			...
			return dashboardWelcomeUtils.isPinnedPage(self.getPinPageId())
				.then(function(isPinned) {
					resultOpts.toolBars.statusPinPage = !self.isChild ? isPinned : 0;
				})
				.then(function() {
					if (self.isChild) {
		...
		savedCtxCreate: function(type) {
			...
			if (self.lastSelectedRow) {
				ctx.lastSelectedRow = self.lastSelectedRow;
			}
			if (type !== "link" && self._gestureMgr) {
			...
			if (self.ctrlSaveCtxCalls && self.ctrlSaveCtxCalls.length > 0) {
				...
				self.ctrlSaveCtxCalls.forEach(function(c) {
					...
					if (ctrlCtx) ctxCtrls[c.$bind] = ctrlCtx;
				});
				if (!$.isEmptyObject(ctxCtrls)) {
					ctx.ctxCtrls = ctxCtrls;
				}
			}
			return ctx;
		},
		...
		getRestoreContext: function(reason) {
			...
			if (self.state.options) {
				...
				["dataParams"].forEach(function(p) {
					var val = self.state.options[p]
					if (val != null) {
						ctx.currentState.options[p] = val;
					}
				});
			}
			return ctx;
		...
		_sharePageLink: function() {
			return this.getTileLinkContext().then(function(ctx) {
		...
		_shareAddLiveTile: function(data) {
			...
			var options = {},
				self = this;
			return self.getTileLinkContext().then(function(ctx) {
