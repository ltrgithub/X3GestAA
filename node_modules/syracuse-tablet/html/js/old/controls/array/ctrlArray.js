...
var _Klass = utils.defineClass(
	...
	function CtrlArray(controller, article, prototype, options) {
		...
		_actRowDelete: function(rowId) {
			...
			if (!info) return;
			modal.simpleConfirm(locale.text("actions.del.confirm"), function(result, confirmed) {
				if (confirmed) {
		...
		savedCtxCreate: function(type) {
			...
			var res = {};
			var subCtx = this.builder.savedCtxCreate(type);
			if (subCtx) {
				res.$builder = subCtx
			}
			subCtx = this._search ? this._search.savedCtxCreate(type) : null;
			if (subCtx) {
		...
		getScrollViewRect: function() {
			if (this.page.options.isVignette && this.page.parentVignette) {
				return this.page.parentVignette.getScrollViewRect(): null;
			} else {
		...
		sortInfoGet: function() {
			var info = this.controller.getPageSort();
		...
		sortInfoSet: function(info, refresh) {
			this.controller.setPageSort(info);
			if (refresh) {
				this.controller.refreshPageFilterAndSort();
			}
		},
		/**
		...
		filterInfoGet: function() {
			var info = this.controller.getPageFilter();
			return info;
		},
		/**
		...
		*/
		filterInfoSet: function(info, refresh) {
			this.controller.setPageFilter(info);
			if (refresh) {
				this.controller.refreshPageFilterAndSort();
			}
		},
