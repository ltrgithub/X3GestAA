...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("initLogin", false);
...
var _notifLoggedIn = function(userProfile, offlineLogin, fromInitProcess) {
	...
	return _readRestoredContext(fromInitProcess).then(function(contextToRestore) {
		...
		if (fromInitProcess !== true && nativeApp) {
			...
			return auth.reconnectGetToken().then(function(token, userLogin, sid, loginCookieName, sidCookieName) {
				// Stores the new reconnection token
				log && log("Getting reconnect token: " + token, userLogin, sid, loginCookieName, sidCookieName);
...
exports.init = function() {
	...
	var nativeApp = native.getModule("nativeApp");
	if (nativeApp) {
		// Call to native API are asynchronous (ios)
		step = nativeApp.getReconnectToken().then(function(loginCookie, sidCookie) {
			// Call a request that sets the session cookie to reconnect Syracuse server
			return (loginCookie || sidCookie) ? auth.reconnectSetToken(loginCookie, sidCookie) : null;
		})
	} else {
		step = $.smResolve();
	}
