...
var _notifLoggedIn = function(userProfile, contextToRestore, offlineLogin, fromInitProcess) {
	...
	// When we open the application from a link in a email
	var urlContext = utils.getUrlContext();
	return step.then(function(token) {
		...
		return initContext.init(false, contextToRestore, offlineLogin, urlContext);
	}).then(function(userContext, urlCtx) {
		// urlContext passed in initContext has been checked (endpoint, role..) and set to null if not valid
		urlContext = urlCtx;
		...
		} else if (fromInitProcess === true && nativeX3WUPApp) {
			// A saved context comming from native wrapper has to be restored (if any)
			historyContext = nativeX3WUPApp.getHistoryContext();
			nativeX3WUPApp.setHistoryContext(""); // Clear context. So it will only be restored once
		}
		notifications.publish("sm.user.logged.in", fromInitProcess, historyContext);
		return true;
	});
};
/**
	...
	return step.then(function() {
		...
		if (nativeX3WUPApp) {
			if (reconnectToken && userProfile == null) {
				...
				nativeX3WUPApp.setReconnectToken(null);
			}
			return nativeX3WUPApp.getReconnectContext();
		} else {
			return null;
		}
