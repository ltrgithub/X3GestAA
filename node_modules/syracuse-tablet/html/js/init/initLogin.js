...
exports.init = function() {
	...
	//Instead of executing the existing chain, we exit and create a new one
	//Not doing this caused a strange behaviour failing with another message than the one that
	//Is put here
	...
	step.then(function() {
		...
		.fail(function(e) {
			...
			if (e) {
				...
				if (!message && e.$diagnoses && e.$diagnoses.length > 0) {
					message = e.$diagnoses[0].$message || "No error message";
				}
			} else {
			...
			def.reject({
				message: message,
				$redirectPage: "login",
				$noConnectionError: (e && e.$noConnectionError) || false
			});
		});
