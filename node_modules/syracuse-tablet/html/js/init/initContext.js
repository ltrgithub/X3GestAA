	...
	return settings.getGlobalSettings()
		...
		.then(function() {
			if (endpoints.length < 1 || roles.length < 1) {
				errorText = locale.text("login.roleendpoint.error");
				return $.smReject();
			}
		})
		.then(function() {
		...
		.then(function(context) {
			...
			if (errorText) {
				globals.getApplication().logout()
					.then(function() {
						globals.getModal().error(errorText);
					});
			} else if (e && e.$redirectPage === "login") {
				globals.getApplication().logout()
					.then(function() {
						globals.getModal().error(e.$message);
					});
			} else {
				globals.getModal().error(locale.text("login.context.error"), e);
			}
		});
};
