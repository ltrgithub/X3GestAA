 ...
 * urlContext		Context provided by url to connect an application/page from email
 * Resolve with
 * 					userContext - Null if login needed
 * 					urlContext	- Null if no url context or if not valid (bad endpoint...)
 */
exports.init = function(fromUserAction, contextToRestore, offlineLogin, urlContext) {
	if (contextToRestore) {
	...
	return settings.getGlobalSettings().then(function() {
		...
		return $.smResolve().then(function() {
			...
			if (urlContext) {
				var ar = urlContext.endpoint.split(".");
				urlEndpoint = {
					ok: false,
					protocol: ar[0],
					contract: ar[1],
			...
			if (urlEndpoint && !urlEndpoint.ok) {
				// Cancel urlContext
				globals.getModal().actionResult(locale.text("urlcontext.bad"), locale.text("urlcontext.badendpoint", [urlContext.endpoint]));
				urlContext = null;
			}
			...
			rsrcRoles.forEach(function(rl) {
				if (userProfile.roles && userProfile.roles.some(function(r) {
					...
					if (urlContext && !urlRole && rl.code === urlContext.role) {
						urlRole = rl;
					}
					roles.push(rl);
				}
		...
		if (urlContext && urlContext.endpoint && urlContext.role && languages) {
			...
			if (navLang && languages.some(function(l) {
				if (utils.sameLanguage(l.code, navLang)) {
					urlContext.language = l.code;
					return true;
				};
			})) {
				// urlContext can be used as currentContext
				return $.smResolve(urlContext);
			}
		}
		...
		if (displayDialog) {
			...
			// When we open from a link we hide the endpoints because it's given by urlContext
			// We hide the roles if there's a role provided by url context
			// We don't hide the language because it's not given by urlContext
			var opts = {
				hideEndpoints: urlContext != null,
				hideRoles: urlContext != null && urlContext.role != null
			};
			...
			new modalSelectContext.Modal(currentContext, endpoints, roles, languages, opts).show()
				.then(function(selectContext) {
					...
					if (selectContext.action === "logout") {
						...
						deferred.reject(new Error("$logout$"));
					} else {
						deferred.resolve(selectContext.action === "ok" ? selectContext : null);
					}
				}).fail(function(e) {
					deferred.reject(e);
				});
		...
		if (urlContext) {
			// Force the endpoint/role with the one provided by url context if any
			// Endpoint is mandatory
			currentContext.endpoint = urlContext.endpoint;
			if (urlContext.role) {
				// Role is not mandatory - If null we keep the current one
				currentContext.role = urlContext.role;
			}
		}
		...
		if (native.hasCapability("X3WUPApp")) {
			...
			native.getModule("X3WUPApp").setReconnectContext({
		...
		// !! urlContext can be null if not valid
		return $.smResolve(currentContext, urlContext);
	});
};
