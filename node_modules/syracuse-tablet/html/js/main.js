	...
	return initStyles.init()
		...
		.then(function() {
			...
			if (globals.getApplication()) {
				globals.getApplication().logout(message, true, offlineHint)
					.then(function() {
				...
				var locale = require('syracuse-tablet/html/js/helpers/locale');
				var st = globals.getStorage() || storage.getStorage();
				// Check if it's versionError error
				diags.clearLocalDb = locale && JSON.stringify(diags).indexOf(locale.text("db.err.version")) > 0 && st && st.clearDatabaseSupported();
