...
var _fatalError = '\
		<body style="font-family:Segoe UI Light,Segoe UI,Segoe WP,Helvetica Neue,sans-serif;font-size: 14px;">\
			<h2>{{title}}</h2>\
			<p>\
				{{#each $diagnoses}}\
...
require('syracuse-tablet/html/deps/mobile-detect');
	...
	return initStyles.init()
		...
		.then(function() {
			...
			if (globals.getApplication()) {
				...
				diags.title = locale ? locale.text("fatalerror.title") : 'A fatal error occurred during initialization process';
				document.write(Handlebars.compile(_fatalError)(diags));
			}
		});
}
