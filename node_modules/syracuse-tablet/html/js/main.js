...
function _startup() {
	...
	return initStyles.init()
		...
		.then(function(userLogedIn) {
			...
			if (!userLogedIn) {
				// #9088
				// _forceLogin does a logout which kills the http session
				// because of this, the context for sage id stored in the
				// http session is lost and redirection does not work
				// Not clear why there is a logout here at all if the
				// status is not logged in already
				// so removed it and just go to login page here
				//_forceLogin(_text("login.authentication"), false);

				globals.getApplication().gotoLogin(_text("login.authentication"));
			}
		}).fail(function(e) {
