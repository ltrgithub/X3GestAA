...
function _startup() {
	...
	return initStyles.init()
		...
		.then(function() {
			/*var deferred = $.Deferred();
			initLogin.init().then(function(){
				initLogin.resolve(true);
			}).fail(function(){
		...
		.then(function(userLogedIn) {
			...
			if (globals.getApplication()) {
				...
				diags.clearLocalDb = locale && JSON.stringify(diags).indexOf(_text("db.err.version")) > 0 && st && st.clearDatabaseSupported();
