...
var _template = '\
	...
	<div class="tab-content"> \
		<div id="authPanelControl" class="tab-pane panel-body"> \
			...
			<textarea  id="authJSONControl" type="text"></textarea> \
			<div class="s-m-auth-panel-footer"> \
				<button type="button" id="authPanelBtnControlApply" class="btn pull-right">{{label_apply}}</button> \
			</div> \
		</div> \
		<div id="authPanelEdit" class="tab-pane panel-body active"> \
			<label id="authEditName"></label> \
			<textarea  id="authJSON" type="text"></textarea> \
			<div class="s-m-auth-panel-footer"> \
				<button type="button" id="authPanelBtnApply" class="btn pull-right">{{label_apply}}</button> \
			</div> \
		</div> \
...
var AuthInterface = utils.defineClass(function() {
	...
	_ensureAuthPanel: function() {
		...
		self.$$authJSONControl = $("#authJSONControl");
	...
	_updateLayout: function() {
		...
		self.$$authJSON.css("height", (window.innerHeight - 220) + "px");
		self.$$authJSONControl.css("height", (window.innerHeight - 220) + "px");
