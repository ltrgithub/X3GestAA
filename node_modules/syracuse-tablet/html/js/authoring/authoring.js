...
var AuthInterface = utils.defineClass(function AuthoringInterface() {
	...
	_authCloseAuthoring: function(articlePage) {
		...
		var _end = function() {
			...
			self.disable();
			var page = globals.getApplication().currentPage;
			if (page && (page.prototype.isQuery() || page.prototype.isLookup())) {
				// We want to reload the data and with forceUseCache!=true in order to restore the right pagination
				// Pagination in query doesn't work fine with forceUseCache=true (when we refreshAuthoring) but it's ok for design (pagination disabled)
				// -> so we restore pagination when we exit authoring - see _queryCutAndAddLinks
				page.reload({
					updateLayout: false
				});
			}
