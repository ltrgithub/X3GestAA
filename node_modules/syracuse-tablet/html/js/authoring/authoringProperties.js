...
exports.getProperties = function(sel, data) {
	...
	var $$elmt = sel.$$elmt;
	var props = [];
	/* fdb - with are now able to retreive the controller from $$elmt */
	...
	if ($$elmt.hasClass("s-m-layout")) {
		...
		var numberOfRows = _getArrayNumberOfRows(sel, data);
		var displayEmptyCells = _getArrayEmptyCells(sel, data);
		var queryFiltersOpts = null;
		if (controller && (controller.prototype.isLookup() || controller.prototype.isQuery()) && controller.$filtersHasAuthoring()) {
			queryFiltersOpts = _getArrayQueryFilters(sel, data);
		}
		var props = [_properties.isTitleHidden, _properties.labelPosition, labelAlign, _properties.theme, _properties.arrayDisplay, cardsPerRow, numberOfRows, displayEmptyCells];
		if (queryFiltersOpts) {
...
//Only for cardH
function _getArrayNumberOfRows(sel, data) {
	var valPos = _getValueOrDefault(_properties.arrayDisplay, sel, data);
	if (valPos !== "cardH") {
		return _propertyValuesHideAll(_properties.arrayNumberOfRows, true);
	} else {
		return _propertyValuesHideAll(_properties.arrayNumberOfRows, false);
	}
}
