...
var _properties = {
	...
	// only card
	"arrayCardLink": {
		applyMarkup: _arrayCardLinkButton
	},
...
exports.getProperties = function(sel, data) {
	...
	if ($$elmt.hasClass("s-m-layout")) {
		...
		var props = [_properties.isTitleHidden, _properties.labelPosition, labelAlign, _properties.theme, _properties.arrayDisplay];
		_arrayCardCheck(_properties.arrayCardsPerRow, sel, data, props);
		_arrayCardCheck(_properties.arrayCardsMinWidth, sel, data, props);
		_arrayCardCheck(_properties.arrayCardsEmptyCells, sel, data, props);
		if (queryFiltersOpts) {
		...
		authoringPropertiesChart.setChartConfig(data, props);

	}


	_arrayCardCheck(_properties.arrayCardLink, sel, data, props);
...
//Only for card
function _arrayCardCheck(prop, sel, data, props) {
	var display = _getValueOrDefault(_properties.arrayDisplay, sel, data);
	if (prop !== _properties.arrayCardLink) {
		if (display === "card") {
			props.push(_propertyValuesHideAll(prop, false));
		}
		return;
	}
	if (data.control && data.control.controller.state.type == "carddesign" || display === "card") {
		props.unshift(prop);
	}
}
...
function _destroyMarkupCardsMinWidthSlider($$parent, prop) {
	...
	if ($$slider.length > 0 && $$slider.slider && $$parent.find(".slider").length > 0) {
...
function _arrayCardLinkButton($$accord, prop, sel, data) {
	var ctrl = data.control;
	var closeDesign = ctrl.controller.state.type == "carddesign";
	var html = $('<button id="' + "auth_opt_" + prop.name + '" type="button" class="btn btn-default">' + (closeDesign ? "Save card design" : "Open card design") + '</button>');
	$$accord.prepend(html);
	html.on("click", function(evt) {
		if (closeDesign) {
			eventListener.triggerAction(ctrl.$$elmt, "saveCardDesign");
		} else {
			// Trigger a navigation to carddesign page
			var attrs = {
				"data-control-id": ctrl.id,
				"data-parent-id": ctrl.controller.id,
				"data-nav": "carddesign",
				"data-nav-target": "application"
			};
			eventListener.triggerNavigation(ctrl.$$elmt, attrs);
		}
	});
}
