...
function _setNumDisplayConfig(data, props) {
	if (data.type === "integer" || data.type === "real" || data.type === "decimal") {
		...
		if (isGauge) {
			...
			// set and add "gBorderValuesType" property if "gSetBorderValues" is set to "yes"
			if (data.articleSelection["$gSetBorderValues"] === "yes") {
				data.articleSelection["$gBorderValuesType"] = typeof data.articleSelection["$gauge"]["$bindMin"] == "string" ? "property" : "input";
...
function _createMarkupGSegmentsNumbers($$parent, prop, sel, data) {
	...
	var ctx = {
		...
		label_apply: "Apply",
		value: data.articleSelection["$gauge"].segments ? data.articleSelection["$gauge"].segments.length : ""
	};
