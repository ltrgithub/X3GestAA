...
exports.getProperties = function(sel, data) {
	...
	if ($$elmt.hasClass("s-m-layout")) {
		...
		var props = [_properties.isTitleHidden, _properties.labelPosition, labelAlign, _properties.theme, _properties.arrayDisplay, cardsPerRow, displayEmptyCells];
		if (queryFiltersOpts) {
...
function _createMarkupGBorderValues($$parent, prop, sel, data) {
	...
	if (isProperty) {
		...
		if (data.control.controller.prototype && data.control.controller.prototype.json && data.control.controller.prototype.json.$properties) {
			...
			_err("Gauge Authoring Error - Border value type", "There is a problem with the prototype");
			return;
		}
	...
	$("button", $$panel).on("click", function(e) {
		...
		if (!isNaN(parseInt(vvMin), 10) && !isNaN(parseInt(vvMax), 10)) {
			...
			if (vvMin === vvMax) {
				err = _err("Gauge Authoring Error - Border values", "Min and max can not come from the same property");
			}
