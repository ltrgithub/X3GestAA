...
var _properties = {
	 ...
	 */
	"gaugeSetBorderValues": {
		"key": "$gaugeSetBorderValues",
		"default": "no",
		values: [{
			value: "yes"
		}, {
	 ...
	 */
	"gaugeBorderValuesType": {
		"key": "$gaugeBorderValuesType",
		"default": "input",
		values: [{
			value: "input"
		}, {
...
exports.getProperties = function(sel, data) {
	...
	if ($$elmt.hasClass("s-m-layout")) {
		...
		if (controller && controller.prototype.isQuery()) {
			detailsLinkOpts = _getDetailsLinkOptions(controller, sel, data);
		}
...
function _setNumDisplayConfig(data, props) {
	if (data.type === "integer" || data.type === "real" || data.type === "decimal") {
		...
		if (isGauge) {
			...
			// set the value of "gaugeSetBorderValues" property
			if (data.articleSelection["$gauge"]["$bindMin"] && data.articleSelection["$gauge"]["$bindMax"]) {
				data.articleSelection["$gaugeSetBorderValues"] = "yes";
			}
			// add "gaugeSetBorderValues" property
			props.push(_properties.gaugeSetBorderValues);

			// set and add "gaugeBorderValuesType" property if "gaugeSetBorderValues" is set to "yes"
			if (data.articleSelection["$gaugeSetBorderValues"] === "yes") {
				data.articleSelection["$gaugeBorderValuesType"] = typeof data.articleSelection["$gauge"]["$bindMin"] == "string" ? "property" : "input";
				props.push(_properties.gaugeBorderValuesType);
			}
		}
	}
}
...
//Only for array in queries
function _getDetailsLinkOptions(controller, sel, data) {
...
function _createMarkupNumDisplay($$parent, prop, sel, data) {
	var article = data.articleSelection;
	var ctx = {};
	ctx.label_prop_title = "Display :";
	ctx.label_prop_value = article["$gauge"] ? "Gauge" : "Normal";
	ctx.btnsHtml = authHtml.execute("numDisplayEditAction", {
		id_btn_edit: "s-m-auth-numdisp-edit",
		label_button_edit: "Edit"
	});

	var html = authHtml.execute("authPanelPropertyNumDisplay", ctx);
	var $$panel = $("<div class='s-m-auth-num'>").append($(html));


	// events handlers

	/*
	 *	if click on 'edit' button, do the following :
	 *	- create view with 'save' and 'cancel' buttons, and with edit view
	 *	- hide detail view
	 */
	$$panel.delegate("button[id='s-m-auth-numdisp-edit']", "click", function(e) {

		// build 'save' 'cancel' buttons
		var savecancelbtns = authHtml.execute("numDisplaySaveCancelAction", {
			id_btn_cancel: "s-m-auth-numdisp-cancel",
			label_button_cancel: "Cancel",
			id_btn_save: "s-m-auth-numdisp-save",
		...
		// build edit view
		var $$editView = $("<div class='s-m-auth-num-edit'>");
		$$editView.append($(savecancelbtns));

		// hide previous content (detail view)
		$("div.s-m-auth-num-detail", $$panel.parent()).hide();

		// append edit view
		$$panel.append($$editView);
	});
	 ...
	 *	- restore detail view
	 */
	$$panel.delegate("button[id='s-m-auth-numdisp-cancel']", "click", function(e) {
		var $$pp = $$panel.parent();
		$("div.s-m-auth-num-edit", $$pp).remove();
		$("div.s-m-auth-num-detail", $$pp).show();
	});
