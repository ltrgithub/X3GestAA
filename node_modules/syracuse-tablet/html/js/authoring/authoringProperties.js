...
function _createMarkupCardsMinWidthSlider($$parent, prop, sel, data) {
	...
	slider.on("slide", utils.bindFn(ctrl.authOnSlideMinSize, ctrl, "slide"));
	slider.on("slideStart", utils.bindFn(ctrl.authOnSlideMinSize, ctrl, "start"));
	slider.on("slideStop", utils.bindFn(_cardsMinWidthSliderSetVal, null, $$parent, ctrl, prop, sel, data));
}

function _cardsMinWidthSliderSetVal(slideEvt, $$parent, ctrl, prop, sel, data) {
...
function _createMarkupGBorderValues($$parent, prop, sel, data) {
	...
	if (isProperty) {
		...
		if (data.control.controller.prototype && data.control.controller.prototype.json && data.control.controller.prototype.json.$properties) {
			...
			ctx.markupMin = authHtml.execute("gaugeBorderSelect", {
				id: idMin,
				options: optionsMin
			});

			ctx.markupMax = authHtml.execute("gaugeBorderSelect", {
				id: idMax,
				options: optionsMax
			});
		} else {
	...
	$("button", $$panel).on("click", function(e) {
		...
		var errTitle = locale.text("Gauge Authoring Error - Border values");
