...
var _templates = {
	...
	palettePicker: '\
		<section class="auth-palettePicker {{dropDirection}}">\
			<div class="input-group" data-toggle="dropdown" aria-expanded="false">\
				<div class="input-group-btn"">\
					<span class="form-control btn-default auth-paletteBtn"></span>\
				</div>\
				<span class="form-control auth-paletteColor">\
			</div>\
...
var _components = {
	...
	"palettePicker": function(options, name) {
		options = options || {};
		this.options = options;
		this.name = name || "palettePicker";
		this.createMarkup = function($$parent) {
			var css = options.css || "";
			this.$$panel = $(_getHtml("palettePicker", {
				id: options.id,
				css: css,
				dropDirection: options.dropUp === true ? "dropup" : "dropdown"
			})).appendTo($$parent);
			this.$$rootPalette = this.$$panel.find(".auth-paletteRoot");
			this.$$button = this.$$panel.find(".auth-paletteBtn");
		...
		this.updatePalette = function(palette) {
			...
			palette.forEach(function(item) {
				html.push(_getHtml("paletteItem", item));
			});
		...
		this.destroyMarkup = function() {
			...
			this.options = null;
			this.$$button = this.$$input = this.$$panel = null;
		};
		...
		this.setValue = function(color) {
			...
			this.$$button.text(color ? color.name : locale.text("auth.components.palette.nocolor"));
			this.$$input.removeClass().addClass("form-control");
			if (color) {
