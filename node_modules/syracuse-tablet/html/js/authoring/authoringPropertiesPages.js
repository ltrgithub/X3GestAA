...
var _properties = {
	"variants": function(controller, sel, data) {
		...
		this.views.forEach(function(view, idx) {
			...
			if (view.$selected) currentSelection = view.$url;
		});
		this.applyMarkup = function($$parent, prop, sel, data) {
			...
			this.viewSelect.createMarkup($$parent);
		...
		this.valueChange = function(event, viewSelect, sel, data, prop) {
			...
			if (variantSelected) {
				...
				var articleVariant = JSON.parse(variantSelected.pageData.content).$article;
				notifications.publish(["sm.auth.change.prop.ui"], JSON.stringify(articleVariant));
			}
		};
	}
};
