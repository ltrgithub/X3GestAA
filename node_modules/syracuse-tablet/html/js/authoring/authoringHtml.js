...
var _templates = {
	authPanelHeader: '\
		...
		<div class="btn-group"> \
			<button class="btn btn-auth-header disabled" id="s-m-auth-remove-id" type="button"><label class="glyphicon glyphicon-trash"></label>{{label_remove}}</button> \
			<button class="btn btn-auth-header" id="s-m-auth-remove-all-id" type="button"><label class="glyphicon glyphicon-erase"></label>{{label_remove_all}}</button> \
		</div> \
	...
	authPanelLeftStructure: '\
		<div class="s-m-site-auth-panel-content"> \
			<ul class="nav nav-pills" role="tablist"> \
				<li><a href="#s-m-auth-tree-box" role="tab" data-toggle="tab">{{label_content}}</a></li> \
				<li><a href="#s-m-auth-insert-box" role="tab" data-toggle="tab">{{label_insert}}</a></li> \
			</ul> \
			<div class="tab-content"> \
				<div id="s-m-auth-tree-box" class="tab-pane active"> \
					<ul class="s-m-auth-tree s-m-scroll-element" id="auth-structure-root-id"></ul> \
				</div> \
				<div id="s-m-auth-insert-box" class="tab-pane"> \
					<div class="panel panel-primary s-m-panel-auth-prop"> \
						...
						<div id="auth-lp-cnt-containers-clpse" class="panel-collapse collapse in" \
							...
							<div class="s-m-auth-btn-container s-m-draggable fa fa-th" data-auth-bind="group" data-auth-drag-action="add_layout"></div> \
							<div class="s-m-auth-btn-container s-m-draggable fa fa-square-o" data-auth-bind="tile" data-auth-drag-action="add_layout"></div> \
							<div class="s-m-auth-btn-container s-m-draggable fa fa-bars" data-auth-bind="stack" data-auth-drag-action="add_layout"></div> \
						</div> \
					</div> \
					<div class="panel panel-primary s-m-panel-auth-prop"> \
	...
	authPanelPropertyGroup: ' \
		<div class="panel panel-primary s-m-panel-auth-prop s-m-auth-prop-type-{{type}}"> \
			<div class="panel-heading" role="tab" id="heading_{{id}}"> \
				<a data-toggle="collapse" data-parent="#accordion" href="#collapse_{{id}}" {{#if expanded}} aria-expanded="true"{{/if}} aria-controls="collapse_{{id}}" {{#unless expanded}}class="collapsed"{{/unless}}>\
					<h4 class="panel-title"> \
						{{label}}\
					</h4> \
				</a> \
	...
	authPanelPropertyJSON: ' \
			<div class="form-group"> \
				...
				<button class="btn btn-default pull-right" data-action="authJsonApply">{{label_apply}}</button> \
			</div><div style="clear: both;"></div>',
	authPanelPropertyPROTOTYPE: ' \
			<div class="form-group"> \
				<textarea readonly class="form-control" id="{{id}}" rows="10">{{value}}</textarea> \
			</div>',
	authPanelPropertyNumDisplay: ' \
		<div class="s-m-auth-num-detail">\
			<div class="s-m-auth-num-actions">\
				{{{btnsHtml}}}\
			</div>\
			<div class="s-m-auth-num-content">\
				<div class="s-m-auth-num-prop">\
					<label class="s-m-auth-num-prop-title">{{label_prop_title}}</label>\
					<label class="s-m-auth-num-prop-value">{{label_prop_value}}</label>\
				</div>\
			</div>\
		</div>\
	',
	numDisplayEditAction: '\
		<button type="button" class="btn btn-primary btn-block" id={{id_btn_edit}}>{{label_button_edit}}</button> \
	',
	numDisplaySaveCancelAction: '\
		<button type="button" class="btn btn-danger btn-block" id={{id_btn_cancel}}>{{label_button_cancel}}</button> \
		<button type="button" class="btn btn-success btn-block" id={{id_btn_save}}>{{label_button_save}}</button> \
	',
	numDisplaySelect: '\
		<div class="s-m-auth-num-prop">\
			<label class="s-m-auth-num-prop-title">{{label_prop_title}}</label>\
			<select>\
				{{#each options}}\
					<option value="{{value}}" selected="{{selected}}">{{label_option}}</option>\
				{{/options}}\
			</select>\
		</div>\
	'
