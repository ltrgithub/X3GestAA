...
exports.getModuleProperties = function($$elmt, controller, props, sel, data) {
	...
	// ctrlFieldBase
	if (sel.$$elmt.is(".s-m-control.s-m-field")) {
		[_ctrlFieldsProperties.isTitleHidden,
			_ctrlFieldsProperties.labelPosition,
			labelAlign,
			// Themes are added as css class by widgest but since there is no dedicated css yet, we disable this option here
			//_ctrlFieldsProperties.theme
		].forEach(function(p) {
			props.push(p);
		});
	...
	} else if (sel.$$elmt.is(".s-m-control.s-m-array")) {
	...
	} else if ($$elmt.is(".s-m-control.s-m-chart")) {
...
exports.getThemeProperties = function() {
	return _ctrlFieldsProperties.theme;
};