...
var _ctrlFieldsProperties = {
	 ...
	 * Title hidden
	 */
	"emptyImageIcon": function(controller, sel, data) {
		this.name = "emptyImageIcon";
		this.label = null;
		this["default"] = null;
		this.key = "$emptyImageIcon";
		this.destroyMarkup = function() {
			_destroyMarkup.call(this);
		};
		this.createMarkup = function($$parent, prop, sel, data) {
			_createMarkup.call(this, $$parent, prop, sel, data, fontUtils.getEmptyImageIconList(), fontUtils.getEmptyImageDefIcon());
		};
	},
	// only CLOB field (text plain, text html, text rtf)
	"emptyIconSize": {
	...
	"urlIcon": function(controller, sel, data) {
		this.name = "urlIcon";
		this.label = null;
		this["default"] = null;
		this.key = "$urlIcon";
		...
		this.createMarkup = function($$parent, prop, sel, data) {
			_createMarkup.call(this, $$parent, prop, sel, data, fontUtils.getUrlIconList(), fontUtils.getUrlDefIcon());
		};
	},
	"urlIconSize": {
		"key": "$urlIconSize",
...
function _destroyMarkup() {
	if (this.paletteIcon) {
		this.paletteIcon.destroyMarkup();
		this.paletteIcon = null;
	}
};

function _createMarkup($$parent, prop, sel, data, iconList, defIcon) {
	var self = this;
	var opts = {
		palette: iconList,
		dropUp: true,
		icon: data.articleSelection[self.key] || defIcon,
		onSelectIcon: function(value) {
			authPropsGeneral.setValue(self, sel, data, value);
			authPropsGeneral.notifyChangePropUI($$parent, sel, data);
		}
	};
	var authStatus = data.articleSelection[self.key] || [];
	self.paletteIcon = authComponents.newComponent("iconPalettePicker", opts);
	self.paletteIcon.createMarkup($$parent);
};
