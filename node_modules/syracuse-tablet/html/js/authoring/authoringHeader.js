...
exports.Panel = utils.defineClass(function() {
	...
	_onChooseDevice: function(elmt) {
		...
		modal.show().then(function(result) {
			if (result != null) {
				...
				// fdb - notify authoring
				notifications.publish(["sm.auth.context.changed"], "device");
			}
		});
	},
	_onScaleDevice: function(elmt) {
		...
		uiutils.triggerResizeInternal(params);
		// fdb - notify authoring
		notifications.publish(["sm.auth.context.changed"], "scale");
	},
	_onOrientation: function(elmt) {
		...
		if (orient) {
			...
			// fdb - notify authoring
			notifications.publish(["sm.auth.context.changed"], "orientation");
		}
	},
});