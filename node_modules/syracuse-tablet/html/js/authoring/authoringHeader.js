...
exports.Panel = utils.defineClass(function AuthoringPanelHeader() {
	...
	notifAuthSetSelection: function(id) {
		var self = this;
		var canRemove = self._canBeRemoved(id);
		if (id && canRemove) {
			self._enableRemoveBtn(true);
			self._currentSelectionId = id;
		} else {
			self._enableRemoveBtn(false);
			self._currentSelectionId = null;
		}
	},

	_enableRemoveBtn: function(state) {
		var self = this;
		if (state !== false) {
	...
	_onRemoveItem: function() {
		...
		var parent = data.control.parent;
		if (!parent) {
			return;
		}
		...
		self._enableRemoveBtn(false);
		// Change widget tree
		parent.removeChild(data.control);
	...
	_canBeRemoved: function(id) {
		var self = this;
		var sel = authHelpers.getSelectionById(id);
		var data = sel && authHelpers.getSelectionData(sel);
		if (!data || !data.control) {
			return false;
		}
		var parent = data.control.parent;
		if (jsutils.isInstanceOf(parent, "Page")) { // Never allow to remove root layout node, in page and/or in vignette
			return false;
		}
		return true;
	}
