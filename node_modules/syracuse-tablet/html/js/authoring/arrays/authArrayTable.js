...
var _tableConfigKeys = ["$bind", "$type", "$titleOrig", "$widthType", "$widthVal", "$refValueDisplay", "$refDescFormat", "$refDescPosition", "$imgIcon", "$imgDisplayIcon", "$imgHeight"];
...
var _TableColumnPanel = function(parent, colInfo) {
	...
	this.colXrefAdd = function() {
		...
		var displayValue = (this.colInfo["$refValueDisplay"] === false ? false : true);
		this._xrefDisplayValue = this._createXRefSelect("dispVal", [true, false], displayValue);
		...
		this.$$xrefroot.find("#selectDescrpos").toggle(displayValue);
	};
	this._createXRefSelect = function(type, codes, def) {
			var selectOpts = [];
			var defIdx = codes.indexOf(def);
			if (defIdx == -1) defIdx = 0;
			var textRoot = "auth." + (type == "fmt" ? "refDescFormat" : (type == "dispVal" ? "refValueDisplay" : "refDescPosition"));
		...
		this.colXrefChange = function(event, prop, type) {
			...
			var ref = type === "fmt" ? this._xrefDescrFmt : (type === "dispVal" ? this._xrefDisplayValue : this._xrefDescrPos);
			ref = ref[idx].code;
			if (type == "dispVal") {
				// hide pos if !dispVal
				this.$$xrefroot.find("#selectDescrpos").toggle(ref);
			}
			if (type == "pos") {
				// hide fmt if pos == none
				this.$$xrefroot.find("#selectDescrfmt").toggle(ref !== "none");
			}
			this.colInfo["$ref" + (type === "fmt" ? "DescFormat" : (type == "dispVal" ? "ValueDisplay" : "DescPosition"))] = ref;
		};
	// COL IMAGE
