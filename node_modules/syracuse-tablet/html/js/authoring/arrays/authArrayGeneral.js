...
var _properties = {
	...
	"chartDetailButton": function(controller, sel, data) {
		...
		this.createButtonMarkup = function() {
			var level = (this.controller.state.level || 0);
			//var label="auth."+(!this.control.isSublevel()?"chartDetail":"subLevel")+".openDesign";
		...
		this.onClickButton = function(evt) {
			...
			if (this.control.hasSublevel()) {
				level = (this.controller.state.level || 0) + 1;
				opt = {
					controlOriginId: this.controller.state.options.controlOriginId,
					rowId: this.control.rowIdx2Id(0),
					level: level
				};
				if (level > 1) {
...
exports.getModuleProperties = function($$elmt, controller, props, sel, data) {
	if ((controller.state.type !== "chartdetail" && data.control.isArrayChart()) || data.control.hasSublevel()) {
