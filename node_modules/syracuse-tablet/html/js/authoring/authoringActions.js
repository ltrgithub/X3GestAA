...
var _optionsProperties = ["footerHeader", "refreshPage", "multiselection"];
...
var _properties = {
	...
	"actionSettings": function(controller, sel, data) {
		...
		this.validateRecord = function(errorMsg) {
			...
			var opts = this.optionsList.getValue() || [];
			rec.options = {};
			var nbChecked = 0;
			opts.forEach(function(o) {
				if (o.checked) nbChecked++;
				rec.options[o.id] = o.checked === true;
			});
		...
		this._updateParams = function(record) {
			...
			var opts = [];
			if (record.options) {
				for (var p in record.options) {
					opts.push({
						id: p,
						checked: record.options[p] === true
					});
				}
			}
			this.optionsList.updateValue(opts);
