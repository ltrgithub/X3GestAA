	...
	serieTitle: ' \
		<div class="panel panel-primary s-m-panel-auth-prop s-m-auth-prop-type-{{type}}"> \
			<div style="overflow:hidden;" class="panel-heading" role="tab" id="heading_{{id}}"> \
				<a class="serieTitleCollapsible" style="width:100%;display:table;" data-toggle="collapse" data-parent="#{{#if parentId}}{{parentId}}{{/if}}{{#unless parentId}}accordion{{/unless}}" href="#collapse_{{id}}" {{#if expanded}} aria-expanded="true"{{/if}} aria-controls="collapse_{{id}}" {{#unless expanded}}class="collapsed"{{/unless}}>\
	...
	var _scan = function(items, array) {
		...
		items.forEach(function(item) {
			if (item.items) {
				...
				array.push({
					title: item.title ? locale.text("auth.seriesOptions." + item.title) : null,
					items: items
				});
			} else if (typeof item === "string") {
				array.push({
					name: locale.textNoFail("seriesOptions.color." + item) || item.smCapitalize(),
					css: "s-m-color-" + item
				});
			}
		});
...
var _SeriePanel = function(parent, serieInfo, name) {
	this.name = name;
...
var _properties = {
	...
	"chartStyle": function(controller, sel, data) {
		...
		if (data.control.prototype.chartGetMeasureCount() === 1) {
			this.values.push({
				value: "pie"
			});
		};
	...
	"chartOptions": {
		...
		createMarkup: authPropsGeneral.createMarkupCheckboxes
	},
	...
	"seriesOptions": function(controller, sel, data) {
		...
		this._initProps = function(props, createEntry) {
			...
			Object.keys(props).forEach(function(key) {
				self._createSerieProperty(props[key], createEntry, key);
			});
		};

		this._createSerieProperty = function(prop, createEntry, key) {
			var prop = new _SeriePanel(this, prop, key);
		...
		this._createSerieEntry = function(prop, sel, data) {
			var id = "seriesEntry" + prop.name.smCapitalize();
			var $$parent;
		...
		this.paramColorAdd = function($$root, sel, data, prop) {
			...
			this._addParam("color", this.paletteColor, true, $$root, false);
			this.paletteColor.setValue(this._getColorValue(prop.bind));
		};
		this._getColorValue = function(bind) {
			var res = null;
			this._serieProperties.forEach(function(prop) {
				if (prop.bind === bind) {
					res = prop.color;
		...
		this._addParam = function(prop, component, title, $$parent, mandatory) {
			...
			var $$root;
			if (title) {
				var res = this._addSubSection(prop.name, $$parent, mandatory);
				$$root = res.$$content;
				$$title = res.$$panel;
			} else {
				$$root = $$parent;
		...
		this._addSubSection = function(textId, $$parent, mandatory) {
			var $$panel = $(_getHtml('subtitle', {
				...
				margin: 10,
				id: "seriesColor" + textId.smCapitalize()
...
exports.getModuleProperties = function($$elmt, controller, props, sel, data) {
	props.push(_properties.chartStyle, _properties.chartOptions, _properties.seriesOptions);
};