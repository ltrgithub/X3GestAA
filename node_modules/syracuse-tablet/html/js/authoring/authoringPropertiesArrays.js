...
/*
		...
		"auth.arrayCardStatus": "Manage a status",
		"auth.arrayCardStatus.fiels": "Field",
		"auth.arrayCardStatus.value": "Value",
		"auth.arrayCardStatus.bgcolor": "Color", 
 * */
...
var _properties = {
	"arrayCardStatusEnabled": {
		"key": "$cardStatusEnabled",
		"default": false,
		"values": [{
			"value": true
		}, {
	...
	"arrayCardStatus": function(controller, sel, data) {
		this.name = "arrayCardStatus";
		this.label = null;
		this["default"] = null;
		this.key = "$cardStatus";
		...
		this._myProperties = [];
		/** 
		 	data structure data.articleSelection.$cardStatus
		 		[{
					field: "BPCNUM",
		...
		*/
		this.initMarkup = function(controller, sel, data) {
			...
			var authCrits = data.articleSelection[this.key] || [];
			this.crits = [];
			var self = this;
			authCrits.forEach(function(c) {
				if (self.getField(c.field) != null) {
					c = $.extend({}, true, c);
					c.id = utils.UUID();
					self.crits.push(c);
				}
		...
		this.getField = function($bind) {
			var field = null;
			this._protoValue.some(function(x) {
				if (x.$bind === $bind) {
					field = x;
					return true;
				}
		...
		this.createMarkup = function($$parent, prop, sel, data) {
			...
			this.$$parent = $$parent;
			this.componentListAdd(sel, data);
			this.editSectionAdd();
		};
		this.componentListAdd = function(sel, data) {
			var opts = {
				css: "critlist",
				doAction: jsutils.bindFn(this.listCallBackAction, this, sel, data),
				appendTitle: "Append criteria"
			};
			this.listComponent = components.newComponent("editableList", opts);
			this._addParam("critlist", this.listComponent, true);
			this.critUpdateRecords();
		};
		this.editSectionAdd = function() {
			this.editcritSection = this._addSubSection("editcrit");
			this.editcritSection.$$panel.hide();
			this.editcritSection.$$content.css("padding-left", "7px");
			this.editcritSection.$$title.css("font-weight", "bold");
			var $$rootComponents = this.editcritSection.$$content;
			this.paramFieldAdd($$rootComponents);
			this.paramOperatorAdd($$rootComponents);
			this.paramValueAdd($$rootComponents);
			this.paramBgcolorAdd($$rootComponents);
		...
		this.listCallBackAction = function(action, recordId, listComponent, sel, data) {
			...
			if (action === "validate") {
				var newRecord = this.validateRecord();
				var validateAppend = recInfo == null;
				if (newRecord) {
					if (validateAppend) {
						this.crits.push(newRecord);
					} else {
						this.crits.splice(recInfo.idx, 1);
						this.crits.splice(recInfo.idx, 0, newRecord);
					}
					this.critUpdateRecords(true, sel, data);
				}
				this._resetParams();
				return true;
			}
		...
		this.validateRecord = function() {
			var rec = {};
			rec.field = this.fieldComponent.getValue();
			rec.operator = this.operatorComponent.getValue();
			rec.value = this.$$valueElmt.val() || "";
			rec.bgcolor = this.bgcolorComponent.getValue() || "";
			rec.color = this.colorComponent.getValue() || "";
			rec.id = utils.UUID();
			return rec.field != _noValue && rec.operator != _noValue && rec.bgcolor.length > 0 ? rec : null;
		};
		this._updateParams = function(record) {
			var edit = record != null;
			record = record || {};
			this.editcritSection.$$panel.show();
			this.fieldComponent.setValue(record.field || _noValue);
			this.operatorComponent.setValue(record.operator || _noValue);
			this.bgcolorComponent.setValue(record.bgcolor || "");
			this.colorComponent.setValue(record.color || "");
			this.$$valueElmt.val(record.value || "");
		};
		this._resetParams = function() {
			this.editcritSection.$$panel.hide();
			this.fieldComponent.setValue(_noValue);
			this.operatorComponent.clear();
			this.bgcolorComponent.setValue("");
			this.colorComponent.setValue("");
			this.$$valueElmt.val("");
		};
		this.critUpdateRecords = function(updateAuthoring, sel, data) {
			var clearBefore = true;
			if (clearBefore) {
				this.listComponent.clear();
			}
			var records = [];
			this.crits.forEach(function(c) {
				records.push({
					titleHtml: '<span style="float:left">' + c.field + ' ' + c.operator + ' ' + c.value + '</span><span style="float:right;width:20px;height:20px;background-color:' + c.bgcolor + ';margin-right:5px;"></span>',
					id: c.id
				});
			});
		...
		this.paramFieldAdd = function($$root) {
			var selectOpts = [{
				"title": locale.text('auth.arrayCardStatus.novalue'),
				"value": _noValue
			}];
			...
			this._addParam("field", this.fieldComponent, true, $$root);
		};
		...
		this.paramOperatorAdd = function($$root) {
			var opts = {
				...
				css: "cardStatusOperator",
				onChange: jsutils.bindFn(this.onParamOperatorSelect, this),
				isHidden: false
			};
			this.operatorComponent = components.newComponent("select", opts);
			this._addParam("operator", this.operatorComponent, true, $$root);
			this.updateOperators();
		};
		this.paramValueAdd = function($$root) {
			this.$$valueElmt = $('<input class="form-control cardStatusValue"></input>');
			this._addParam("value", this.$$valueElmt, true, $$root);
		};
		this.paramBgcolorAdd = function($$root) {
			var opts = {
				value: null,
				css: "cardStatusBgColor",
				isHidden: false
			};
			this.bgcolorComponent = components.newComponent("colorPicker", opts);
			this._addParam("bgcolor", this.bgcolorComponent, true, $$root);
		};
		this.paramColorAdd = function($$root) {
			...
			this.colorComponent = components.newComponent("colorPicker", opts);
			this._addParam("color", this.colorComponent, true, $$root);
		};

		this.onParamFieldSelect = function(event, prop) {
			//TODO Update operators according to the type
			var field = this.getField($(event.target).val());
			this.updateOperators(field.$type, "eq");
		};
		this.updateOperators = function(typeField, defValue) {
			typeField = typeField || "application/x-string";
			defValue = defValue == null ? _noValue : defValue;
			var ops = ctrlfactory.getOperators(typeField, true);
			var selectOpts = [{
				"title": locale.text('auth.arrayCardStatus.operator'),
				"value": _noValue
			}];
		...
		this._toggleColorValue = function(show) {
			this.$$parent.find("#arrayCardStatusValue, #arrayCardStatusColor").toggle(show === true);
		};
		this._addParam = function(prop, component, title, $$parent) {
			var $$parent = $$parent || this.$$parent;
			var $$title;
			var $$root;
			if (title) {
				var res = this._addSubSection(prop, $$parent);
				$$root = res.$$content;
				$$title = res.$$panel;
			} else {
				$$root = $$parent;
		...
		this._addSubSection = function(textId, $$parent) {
			var $$panel = $(_getHtml('subtitle', {
				title: locale.text('auth.arrayCardStatus.' + textId),
				margin: 10,
				id: "arrayCardStatus" + textId.smCapitalize()
...
exports.getModuleProperties = function($$elmt, controller, props, sel, data) {
	...
	if (_checkDisplay(sel, data, "card")) {
		...
		if (!controller.prototype.isLookup()) {
			props.push(_properties.arrayCardStatusEnabled);
			if (data.control.getAuthoring("$cardStatusEnabled") == true) {
				props.push(_properties.arrayCardStatus);
			}
		}
