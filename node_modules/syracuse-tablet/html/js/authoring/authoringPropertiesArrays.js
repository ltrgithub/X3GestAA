...
var _tableDesignProperties = {
	...
	"arrayTableColumn": function(parent, colInfo) {
		...
		this.colNameChoiceAdd = function() {
			...
			this._add2PropHeading(locale.text('auth.tableDesign.selColName'), new _tableDesignProperties.select(opts, "selectName"));
		};
		...
		this.colWidthTypeAdd = function() {
			this._add2PropHeading(locale.text("auth.tableDesign.width"), new _tableDesignProperties.flatRadio({
				btns: [{
					key: "auto",
					label: locale.text("auth.tableDesign.auto")
				}, {
					key: "fixed",
					label: locale.text("auth.tableDesign.fixed")
				}, {
					key: "percent",
					label: "%"
				}],
		...
		this.colWidthValAdd = function() {
			...
			this.propWidthVal = this._add2PropHeading(locale.text("auth.tableDesign.selWidth"), new _tableDesignProperties.genericSlider(opts));
...
var _properties = {
	...
	"arrayCardButton": function(controller, sel, data) {
		...
		this.createButtonMarkup = function() {
			...
			this.$$Button = $('<button id="' + "auth_opt_" + this.name + '" type="button" class="btn btn-default">' + text + '</button>');
	...
	"arrayTableDesign": function(controller, sel, data) {
		...
		this.addPropActionButtons = function(sel, data) {
			var btns = [{
				key: "save",
				label: locale.text("auth.tableDesign.save")
			}];
