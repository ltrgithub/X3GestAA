...
/*
		...
		"auth.arrayCardStatus.color": "Color", 
 * */
...
var _properties = {
	...
	"arrayCardStatus": function(controller, sel, data) {
		...
		this.existField = function($bind) {
			return this._protoValue.some(function(x) {
				return x.$bind === $bind;
			});
		};
		...
		this.createMarkup = function($$parent, prop, sel, data) {
			...
			this.$$parent = $$parent;
			this.componentListAdd();
			this.editSectionAdd();
		};
		this.componentListAdd = function() {
			var opts = {
				css: "critlist",
				doAction: jsutils.bindFn(this.listCallBackAction, this),
		...
		this.listCallBackAction = function(action, recordId, listComponent) {
			if (action === "append") {
		...
		this.validateRecord = function() {
			...
			rec.id = utils.UUID();
			return rec.field != _noValue && rec.operator != _noValue && rec.color.length > 0 ? rec : null;
		};
		...
		this._resetParams = function() {
			...
			this.operatorComponent.setValue(_noValue);
			this.colorComponent.setValue("");
			this.$$valueElmt.val("");
		};
		this.critUpdateRecords = function(updateAuthoring) {
			...
			this.crits.forEach(function(c) {
				records.push({
					titleHtml: '<span style="float:left">' + c.field + ' ' + c.operator + ' ' + c.value + '</span><span style="float:right;width:20px;height:20px;background-color:' + c.color + ';margin-right:5px;"></span>',
					id: c.id
				});
			});
			this.listComponent.insertRecords(records);
			if (updateAuthoring == true) {
				alert("todo authoring");
			}
		};
		...
		this.updateOperators = function(typeField, defValue) {
			...
			var ops = ctrlfactory.getOperators(typeField || "application/x-string", true);
			var selectOpts = [{
		...
		this.onParamOperatorSelect = function(event, prop) {
			var op = prop.getValue();
			//this._toggleColorValue(op !== _noValue);
		};
