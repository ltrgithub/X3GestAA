...
var _templates = {
	...
	subtitle: '\
		<section style="{{#if notHidden}}overflow:hidden;{{/if}}white-space:nowrap;text-overflow:ellipsis;{{#if margin}}margin-bottom:{{margin}}px;{{/if}}">\
...
var _dictionary = {
	selByWidth: "Select column width",
	selColName: "Select column name",
	width: "Width",
	auto: "Auto",
	fixed: "Fixed",
	characters: "Characters",
	saveDesign: "Save card design",
	openDesign: "Open card design",
	save: "Save"
};
var _dico = function(id) {
...
var _tableDesignProperties = {
	...
	"arrayTableColumn": function(parent, colInfo) {
		...
		this.colNameChoiceAdd = function() {
			...
			this._add2PropHeading(_dico('selColName'), new _tableDesignProperties.select(opts, "selectName"));
		};
		...
		this.colWidthTypeAdd = function() {
			this._add2PropHeading(_dico("width"), new _tableDesignProperties.flatRadio({
				btns: [{
					key: "auto",
					label: _dico("auto")
				}, {
		...
		this.colWidthValAdd = function() {
			...
			this.propWidthVal = this._add2PropHeading(_dico("selByWidth"), new _tableDesignProperties.genericSlider(opts));

		};
		this.colWidthValSlide = function(val, prop) {
			return this.colInfo.$widthType === "percent" ? val + "%" : val + " " + _dico("characters");
		};
...
var _properties = {
	...
	"arrayCardButton": function(controller, sel, data) {
		...
		this.createButtonMarkup = function() {
			...
			this.$$Button = $('<button id="' + "auth_opt_" + this.name + '" type="button" class="btn btn-default">' + _dico(saveDesign ? "saveDesign" : "openDesign") + '</button>');
		...
		this._createColEntry = function(prop) {
			...
			if (!this.isSingleArray) {
				var $$propGroup = $(_getHtml("subPropTitle", {
					...
					type: "child columns",
					label: prop.label,
					info: prop.colInfo ? prop.colInfo.$key : null,
					expanded: false,
