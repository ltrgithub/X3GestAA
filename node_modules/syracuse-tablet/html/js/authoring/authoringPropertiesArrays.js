...
var _tableDesignProperties = {
	arrayTableColumn: function(parent, colInfo) {
		...
		if (!this.colInfo.$fieldArticle) {
			// Article of field (x-reference description...)
			this.colInfo.$fieldArticle = {};
		}
		this._myProperties = [];
		...
		this.colXrefAdd = function() {
			...
			this._xrefDescrFmt = this._createXRefSelect("fmt", ["desc", "title", "desctitle", "titledesc"], this.colInfo.$fieldArticle["$refDescFormat"] || "desc");
			this._xrefDescrPos = this._createXRefSelect("pos", ["bottom", "right", "left", "top", "none"], this.colInfo.$fieldArticle["$refDescPosition"] || "none");
		};
		...
		this.colXrefChange = function(event, prop, type) {
			...
			this.colInfo.$fieldArticle["$refDesc" + type.smCapitalize()] = ref[idx].code;
		};
...
var _properties = {
	...
	"arrayTableDesign": function(controller, sel, data) {
		...
		this._protoValue = [];
		this._tableConfigKeys = ["$key", "$title", "$type", "$titleOrig", "$widthType", "$widthVal", "$fieldArticle"];
