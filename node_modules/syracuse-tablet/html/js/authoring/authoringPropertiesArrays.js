...
var _tableDesignProperties = {
	...
	"arrayTableColumn": function(parent, colInfo) {
		...
		this.colXrefAdd = function() {
			...
			// Description format
			this._xrefDescrFmt = this._createXRefSelect("fmt", ["desc", "title", "desctitle", "titledesc"], this.colInfo["$xrefDescrFmt"] || "desc");
			this._xrefDescrPos = this._createXRefSelect("pos", ["bottom", "right", "left", "top", "none"], this.colInfo["$xrefDescrPos"] || "none");
			/*
			
			var def = this.colInfo["$xrefDescrFmt"] || "desc";
			var defIdx = 0;
			["desc", "title", "desctitle", "titledesc"].forEach(function(code, idx) {
					if (code == def) defIdx = idx;
					self._xrefDescrFmt.push({
						code: code,
						$title: locale.text("auth.refDescFormat." + code),
						$key: idx
					});
				});
			var opts = {
				value: defIdx,
				options: this._xrefDescrFmt,
				onChange: jsutils.bindFn(this.colXrefChange, this, "fmt")
			};
			this._add2PropHeading(locale.text("auth.refDescFormat"), new _tableDesignProperties.select(opts, "selectDescrFmt"));
			// Description position
			this._xrefDescrPos = [];
			defIdx = 4;
			def = this.colInfo["$xrefDescrPos"] || "none";
			["bottom", "right", "left", "top", "none"].forEach(function(code, idx) {
				if (code == def) defIdx = idx;
				self._xrefDescrPos.push({
					code: code,
					$title: locale.text("auth.refDescPosition." + code),
					$key: idx
				});
			});
			opts = {
				value: defIdx,
				options: this._xrefDescrPos,
				onChange: jsutils.bindFn(this.colXrefChange, this, "pos")
			};
			this._add2PropHeading(locale.text("auth.refDescPosition"), new _tableDesignProperties.select(opts, "selectDescrPos"));
			*/
		};
		this._createXRefSelect = function(type, codes, def) {
			...
			this._add2PropHeading(locale.text(textRoot), new _tableDesignProperties.select(opts, "selectDescr" + type), this.$$xrefroot);
			return array;
		},
		this.colXrefChange = function(event, prop, type) {
			...
			this.colInfo["$xrefDescr" + type.smCapitalize()] = ref[idx].code;
		};
		this.destroyMarkup = function() {
			this.parent = null;
			this.colInfo = null;
...
var _properties = {
	...
	"arrayCardsMinWidth": function(controller, sel, data) {
		...
		this.createMarkup = function($$parent, prop, sel, data) {
			...
			this._slider = new _tableDesignProperties.genericSlider(opts);
	...
	"arrayTableDesign": function(controller, sel, data) {
		...
		this._protoValue = [];
		this._tableConfigKeys = ["$key", "$title", "$type", "$titleOrig", "$widthType", "$widthVal", "$xrefDescrFmt", "$xrefDescrPos"];
