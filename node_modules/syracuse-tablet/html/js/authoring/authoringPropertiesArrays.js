		...
		*/
		...
		this.componentListAdd = function(sel, data) {
			var opts = {
				...
				appendTitle: locale.text("auth.arrayCardStatus.append")
			};
		...
		this.listCallBackAction = function(action, recordId, listComponent, sel, data) {
			...
			if (action === "validate") {
				...
				if (newRecord) {
					...
					var self = this;
					self.listComponent.toggleMessage("auth.arrayCardStatus.invalid", "error");
					setTimeout(function() {
						self.listComponent.toggleMessage(null);
					}, 2500);
					// Keeps validate/cancel
					return false;
				}
			}
		...
		this.paramBgcolorAdd = function($$root) {
			...
			this._addParam("bgcolor", this.bgcolorComponent, true, $$root, true);
		};
		...
		this._addSubSection = function(textId, $$parent, mandatory) {
			var $$panel = $(_getHtml('subtitle', {
				title: locale.text('auth.arrayCardStatus.' + textId) + (mandatory === true ? "*" : ""),
				margin: 10,
