...
var _maxCharWidth = 100;
var _defCharWidth = 50;
...
var _html = {
	...
	subtitle: '\
		<section style="{{#if notHidden}}overflow:hidden;{{/if}}white-space:nowrap;text-overflow:ellipsis">\
...
var _getHtml = function(name, ctx) {
	var c = Handlebars.compile(_html[name]);
	return c(ctx);
};
...
var _tableDesignProperties = {
	...
	"genericSlider": function(options, name) {
		...
		this.createMarkup = function($$parent) {
			var displayValue = this.options.displayValue || this.options.val2Text != null;
			this.$$panel = $(_getHtml("genericSlider", {
				value: this.options.val,
				min: this.options.min,
				max: this.options.max,
				step: this.options.step || 1,
				valText: this.options.val2Text ? this.options.val2Text(this.options.val) : this.options.val,
		...
		this.onSlider = function(slideEvt, action) {
			if (action === "start") {
				this.options.startHandler(slideEvt.value);
				return;
			}
			if (action === "slide") {
				this.$$textValue.text(this.options.val2Text ? this.options.val2Text(slideEvt.value) : slideEvt.value);
				return;
			}
			if (action === "stop") {
				if (this.options.val != slideEvt.value) {
					this.options.val = slideEvt.value;
		...
		this.update = function(opts) {
			...
			if (opts.value != null) {
				...
				this.$$textValue.text(this.options.val2Text ? this.options.val2Text(opts.value) : opts.value);
			}
		};
	},
	"arrayTableColumn": function(parent, colInfo) {
		this.name = "arrayTableColumn";
		this.label = colInfo.$title || colInfo.$key;
		this.parent = parent;
		this.colInfo = colInfo;
		if (!this.colInfo.$widthType) {
		...
		this.colNameChoiceAdd = function() {
			if (false) {
				var idx = this.parent.getProtoValByKey(this.colInfo.$key).idx;
				var opts = {
					val2Text: jsutils.bindFn(this.colNameSlide, this),
					stopHandler: jsutils.bindFn(this.colNameChange, this),
					val: idx + 1,
					min: 1,
					max: this.parent.getProtoValLength(),
					margin: 0
				};
				this._add2PropHeading("Select column name", new _tableDesignProperties.genericSlider(opts));

			} else {
				var idx = this.parent.getProtoValByKey(this.colInfo.$key).idx;
				var opts = {
					value: this.colInfo.$key,
					options: this.parent._protoValue,
					onChange: jsutils.bindFn(this.colNameSelect, this),
				};
				this._add2PropHeading("Select column name", new _tableDesignProperties.select(opts, "selectName"));
			}
		};
		this.colNameSelect = function(event, prop) {
			this.colInfo = this.parent.getProtoValByKey($(event.target).val()).value;
			prop.$$panel.closest(".panel-primary").find(".panel-title").text(this.colInfo.$title);
			this.$$xrefroot.toggle(this.colInfo.$type === "application/x-reference");
		};
		this.colNameChange = function(rank, prop) {
			this.colInfo = this.parent.getProtoValByIdx(rank - 1);
			prop.$$panel.closest(".panel-primary").find(".panel-title").text(this.colInfo.$title);
			this.$$xrefroot.toggle(this.colInfo.$type === "application/x-reference");
		};
		this.colNameSlide = function(rank) {
			var p = this.parent.getProtoValByIdx(rank - 1);
			return p.$title + ' (' + p.$key + ')';
		};
		// COL TYPE WIDTH	
		this.colWidthTypeAdd = function() {
			this._add2PropHeading("Type width", new _tableDesignProperties.flatRadio({
				btns: [{
					key: "auto",
					label: "Auto"
				}, {
		...
		this.colWidthValAdd = function() {
			var opts = {
				val2Text: jsutils.bindFn(this.colWidthValSlide, this),
				stopHandler: jsutils.bindFn(this.colWidthValChange, this),
				val: this.colInfo.$widthVal || 0,
				min: 0,
				max: this.colInfo.$widthType === "percent" ? 100 : _maxCharWidth,
				margin: 0
			};
			this.propWidthVal = this._add2PropHeading("Select column width", new _tableDesignProperties.genericSlider(opts));

		};
		this.colWidthValSlide = function(val) {
			return this.colInfo.$widthType === "percent" ? val + "%" : val + " Characters";
		};
		this.colWidthValChange = function(val, prop) {
			this.colInfo.$widthVal = val;
		};
		this.colWidthValUpdt = function() {
			...
			this.propWidthVal.update({
				value: this.colInfo.$widthType === "percent" ? _defPercentWidth : _defCharWidth,
				max: this.colInfo.$widthType === "percent" ? 100 : _maxCharWidth,
			});
		};
		// COL X-REFERENCE
		this.colXrefAdd = function() {
			...
			// Description format
			this._xrefDescrFmt = [];
			var def = this.colInfo["$xrefDescrFmt"] || "desc",
				defIdx = 0;
			["desc", "title", "desctitle", "titledesc"].forEach(function(key, idx) {
					if (key == def) defIdx = idx;
					self._xrefDescrFmt.push({
						label: locale.text("auth.refDescFormat." + key),
						key: key
					});
				});
			var opts = {
				val2Text: jsutils.bindFn(this.colXrefSlide, this, "fmt"),
				stopHandler: jsutils.bindFn(this.colXrefChange, this, "fmt"),
				val: defIdx,
				min: 0,
				max: this._xrefDescrFmt.length - 1,
				margin: 0
			};
			this._add2PropHeading(locale.text("auth.refDescFormat"), new _tableDesignProperties.genericSlider(opts), this.$$xrefroot);
			// Description position
			this._xrefDescrPos = [];
			defIdx = 4;
			def = this.colInfo["$xrefDescrPos"] || "none";
			["bottom", "right", "left", "top", "none"].forEach(function(key, idx) {
				if (key == def) defIdx = idx;
				self._xrefDescrPos.push({
					label: locale.text("auth.refDescPosition." + key),
					key: key
				});
			});
			var opts = {
				val2Text: jsutils.bindFn(this.colXrefSlide, this, "pos"),
				stopHandler: jsutils.bindFn(this.colXrefChange, this, "pos"),
				val: defIdx,
				min: 0,
				max: this._xrefDescrPos.length - 1,
				margin: 0
			};
			this._add2PropHeading(locale.text("auth.refDescPosition"), new _tableDesignProperties.genericSlider(opts), this.$$xrefroot);
		};
		this.colXrefSlide = function(val, type) {
			var ref = type === "fmt" ? this._xrefDescrFmt : this._xrefDescrPos;
			return ref[val].label;
		};
		this.colXrefChange = function(val, event, type) {
			var ref = type === "fmt" ? this._xrefDescrFmt : this._xrefDescrPos;
			this.colInfo["xrefDescr" + type.smCapitalize()] = ref[val].key;
		};
		this.destroyMarkup = function() {
			this.parent = null;
			this.colInfo = null;
...
var _properties = {
	...
	"arrayCardsMinWidth": function(controller, sel, data) {
		...
		this.createMarkup = function($$parent, prop, sel, data) {
			...
			var $$panel = $(authHtml.execute("authPanelSliderStep", {
	...
	"arrayTableDesign": function(controller, sel, data) {
		...
		this.label = null;
		this.key = "$tableOptions";
		this.controller = controller;
		this.ctrlArray = data.control;
		this._generalProperties = [];
		this._columnProperties = [];
		this._protoValue = [];
		this._tableConfigKeys = ["$key", "$title", "$titleOrig", "$widthType", "$widthVal", "$xrefDescrFmt", "$xrefDescrPos"];

		this.initMarkup = function(controller, sel, data) {
			this._protoValue = this.ctrlArray.prototype.arrayTableGetColInfo();
			this._orginalValue = data.articleSelection.$tableOptions;
		...
		this.addPropNbColumns = function() {
			var opts = {
				val2Text: function(val) {
					return val + " columns";
				},
				stopHandler: jsutils.bindFn(this.updateColumns, this),
				val: this._columnProperties.length,
				min: 1,
				max: this._protoValue.length,
				displayValue: "above"
			};
			this._generalProperties.push(new _tableDesignProperties.genericSlider(opts, "nbcolumns"));
		};
		...
		this.addPropActionButtons = function(sel, data) {
			...
			this._generalProperties.push(new _tableDesignProperties.actionBtns({
				btns: btns,
		...
		this.onClickAction = function(evt, sel, data) {
			...
			if (action === "save") {
				...
				this._columnProperties.forEach(function(prop) {
					entry = {};
					self._tableConfigKeys.forEach(function(p) {
						if (prop.colInfo && prop.colInfo[p] != null) {
							entry[p] = prop.colInfo[p];
						}
					});
					value.push(entry);
				});
		...
		this._createColEntry = function(prop) {
			var id = utils.UUID();
			var $$propGroup = $(authHtml.execute("authPanelPropertyGroup", {
				id: id,
				type: "child",
				label: prop.label,
				expanded: false,
			...
			// create sub-property
			prop.createMarkup($$propGroup.find("#collapse_" + id));
		...
		this._createColumnProperty = function(config, createEntry) {
			config.$title = this.controller.prototype.resolveExpression(config.$title) || config.$key;
		...
		this.getProtoValByKey = function(key) {
			var res = null;
			...
			if (res == null) throw new Error("unexpected null value - key=[" + key + "]");
			return res;
		};
...
exports.getModuleProperties = function($$elmt, controller, props, sel, data) {
	...
	if (_checkDisplay(sel, data, "card")) {
		...
		props.push(_properties.arrayTableHeader, _properties.arrayTableDesign);
	}
