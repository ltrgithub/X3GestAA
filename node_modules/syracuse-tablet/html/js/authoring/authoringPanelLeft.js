...
exports.Panel = utils.defineClass(function AuthoringPanelLeft() {
	...
	_buildPageStructTree: function() {
		...
		function _walkNode(node, $$parent, bindParent) {
			...
			var typeClass;
			var label;
			var isHidden = node.prototype && node.prototype.isHidden();
			...
			var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-' + typeClass + (isHidden ? ' s-m-auth-hidden' : '') + '">').appendTo($$parent);
			var $$p = $('<p data-ctrl-id="' + id + (isHidden ? '" ' : '" class="s-m-draggable s-m-droppable" data-auth-drag-action="move"') + '>' + label + '</p>');
			$$item.append($$p);
			if (node.isVignette()) {
