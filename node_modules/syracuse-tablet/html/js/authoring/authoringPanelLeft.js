...
exports.Panel = utils.defineClass(function() {
	...
	_buildPageStructTree: function(article) {
		...
		function _walkNode(node, $$parent, bindParent) {
			...
			var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-' + typeClass + ' s-m-draggable" data-auth-drag-action="move">').appendTo($$parent);
	...
	_addVignette: function(page, vignette, $$parent) {
		...
		var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-vignette" data-auth-bind="' + vignette + '"  data-auth-drag-action="add">').appendTo($$parent);
		...
		props.forEach(function(key) {
			if (proto.property(key, "$isExcluded") !== true) {;
				...
				if (self._bindsOnPage.indexOf(bind) < 0) {
					...
					var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-' + typeClass + ' s-m-draggable" data-auth-bind="' + key + '" data-auth-bind-parent="' + (bindParent || '') + '" data-auth-drag-action="add">').appendTo($$parent);
					$$item.append('<p>' + label + '</p>');
				}
			}
		});
	...
	_initDrag: function() {
		...
		// In structure tree, all that can be dragged is also a drop target (to move fields in the tree)
		$(".s-m-draggable", self.$$structureRoot).droppable({
			greedy: true,
			...
			drop: function(event, ui) {
				...
				return authDrag.dropDrop($(this), $$helper, $$target);
			},
			accept: function($$dragable) {
				return authDrag.dropAccept($(this), $$dragable);
			},
			over: function(event, ui) {
				var $$helper = ui.helper;
				var $$target = $(event.target);
				return authDrag.dropOver($(this), $$helper, $$target);
			}
		});
	},
	...
	_createDragHelper: function($$source) {
		...
		var text = $(">p", $$dragSource).text();
		var $$helper = $('<p class="s-m-auth-drag-helper">');
		$$helper.text(text);
		$$helper.attr("data-auth-drag-action", $$dragSource.attr("data-auth-drag-action"));
		return $$helper[0];
	},
});