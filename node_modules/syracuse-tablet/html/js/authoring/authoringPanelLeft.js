...
exports.Panel = utils.defineClass(function() {
	...
	_buildPageStructTree: function(article) {
		...
		function _walkNode(node, $$parent, bindParent) {
			...
			var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-' + typeClass + ' s-m-draggable" data-ctrl-id="' + id + '" data-auth-drag-action="move">').appendTo($$parent);
	...
	_addVignette: function(page, vignette, $$parent) {
		...
		var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-vignette s-m-draggable" data-auth-bind="' + vignette + '"  data-auth-drag-action="add_bind">').appendTo($$parent);
		...
		if (ctrlVignette) { // may be null, if vignette is not loaded yet
			...
			// May be null if page is inside the vignette and the vignette is not displaying the content but only a link
			if (page) {
		...
		props.forEach(function(key) {
			if (proto.property(key, "$isExcluded") !== true) {;
				...
				if (self._bindsOnPage.indexOf(bind) < 0) {
					...
					var $$item = $('<li class="s-m-auth-tree s-m-auth-tree-' + typeClass + ' s-m-draggable" data-auth-bind="' + key + '" data-auth-bind-parent="' + (bindParent || '') + '" data-auth-drag-action="add_bind">').appendTo($$parent);
					$$item.append('<p>' + label + '</p>');
				}
			}
		});
