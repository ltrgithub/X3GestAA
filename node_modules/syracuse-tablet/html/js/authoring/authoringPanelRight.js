...
var authArrays = require('syracuse-tablet/html/js/authoring/authoringPropertiesArrays');
...
exports.Panel = utils.defineClass(function AuthoringPanelRight() {
	...
	_ensurePanel: function() {
		...
		self.$$panelRightStruct = $(authHtml.execute("authPanelRightStructure", ctx)).appendTo(self.$$panel).find(".s-m-auth-prop-list");
	...
	destroyPropertyPanel: function(justSelection) {
		if (justSelection !== true) {
			if (this.topButtons) {
	...
	notifAuthSetPage: function(page) {
		...
		if (page.state.type == "carddesign") {
			...
			notifications.publish("sm.auth.top.button", "add", authArrays.getCardDesignBackButton(page));
		}
	},
	...
	notifAuthSetSelection: function(id) {
		...
		self.destroyPropertyPanel(true);
		if (!sel) {
			return;
		}
		...
		if (data) {
			...
			if (sel.$$elmt && globals.getMainPageGestureMgr()) {
				globals.getMainPageGestureMgr().makeVisible(sel.$$elmt.closest(".s-m-tile"));
			}
		}
	},
	...
	notifAuthTopButton: function(action, prop) {
		if (!this.$$panelRightStruct || !prop.buttonMarkup) return;
		this.topButtons.push(prop);
		this.$$panelRightStruct.after(prop.buttonMarkup(this.getPage()));
	}
});