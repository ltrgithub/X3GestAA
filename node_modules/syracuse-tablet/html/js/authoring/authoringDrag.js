...
exports.dropAccept = function($$drop, $$draggable) {
	var data = _getDropData($$drop, $$draggable);
	return true; //_isDropAllowed(data);
};
...
function _isDropAllowed(data) {
	...
	if (data.action === "add_layout") {
		if (data.bind == "stack") {
			...
			if (targetLayout !== "row" && targetLayout !== "hubGroup") {
				return false;
			}
		}
...
function _addLayout(data) {
	...
	var ctrl = ctrlFactory.createLayout(data.toCtrl.controller, data.toContainer, article, {});
