...
function _isDropAllowed($$drop, $$draggable, $$helper) {
	...
	if (data.action === "add_bind") {
		...
		if (data.parent != null && data.parent != data.toParent) {
			return false;
		}
...
function _getDropData($$drop, $$draggable, $$helper) {
	...
	var toParent;
	if (ctrl) {
		if (ctrl.controller.isVignette) {
			var v = ctrl.controller.getParentVignette();
			if (v) {
				toParent = v.$bind; // Name of the vignette that is bound to the controler of the current element
			}
		}
	}
	return {
