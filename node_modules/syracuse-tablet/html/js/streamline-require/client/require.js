...
(function(exports, global) {
	...
	function _load(key, arg, callback) {
		var url;
		var ms_appx_web = window && window.location && window.location.href.indexOf('ms-appx-web://') > -1;
		if (ms_appx_web) {
			if (key === "sources") {
				if (_sources[arg]) {
					return callback();
		...
		// Here, in require.js, only non CORS resources are loaded, this is why $.ajax is valid
	...
	function _sandbox(path, isMain) {
		...
		try {
			...
			throw new Error("module initialization failed: " + path + ":" + ex.lineNumber + ":" + ex.columnNumber + ": " + (ex.message || ex));
		}
	}
