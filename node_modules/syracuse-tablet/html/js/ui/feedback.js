...
exports.bindActions = function($$elmt) {
	$$elmt.on("mousedown", ":not(select)[data-nav], :not(select)[data-nav-refresh], :not(select)[data-action], :not(select)[data-disable-navigation]", _armMouseDown);
...
function _armMouseDown(evt) {
	...
	var $$actionElmt = $$target.closest("[data-nav], [data-nav-refresh], [data-action] , [data-disable-navigation]");
...
function _armMouseUp(evt) {
	...
	for (i = 0; i < _armedItems.length; i++) {
		_armedItems[i].removeClass("s-m-armed s-m-draggable");
	}
	_armedItems = [];
}