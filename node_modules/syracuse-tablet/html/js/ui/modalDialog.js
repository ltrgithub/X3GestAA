...
var _Klass = utils.defineClass(
	...
	function BaseModal() {
		...
		_ensureParent: function() {
			var self = this;
			var $$parent = $("#s-m-modal-parent");
			if ($$parent.length < 1) {
				$$parent = $("<div>");
				$$parent.attr("id", "s-m-modal-parent");
		...
		_onShow: function(evt) {
			...
			self._calcSize();
			/* todo
			self.$$content = this.$$elmt.find(".modal-scroll-content");
			if (self.$$content.length > 0) {
				self.$$scrollWrapper = $('<div class="s-m-scroll-wrapper">');
				self.$$scrollWrapper.appendTo(self.$$content.parent());
				self.$$content.wrap(self.$$scrollWrapper);
				self.$$content.css({
					height: "auto",
					position: "relative"
				});
				...
				self._gestureMgr = new HammerScroller(self.$$content, {
					direction: "v",
					valMax: 0,
					name: "MODAL",
				...
				var scrollRect = uiRect.elmtRect(this.$$content, "outer");
				...
				if (viewRect && !viewRect.contains(scrollRect)) {
					self._gestureMgr.init(viewRect);
				} else {
		...
		*/
		...
		show: function() {
			...
			self.html = outerTpl({
				header: self.template.header(self.data),
				content: self.template.content(self.data),
				footer: self.template.footer(self.data),
		...
		_calcSize: function() {
			...
			if (df.modalClass === "half_right" || df.modalClass === "full") {
				...
				var header = uiRect.elmtRect($$header, "outer");
				var footer = uiRect.elmtRect($$footer, "outer");
