...
var eventListener = require('syracuse-tablet/html/js/ui/eventListener');

var _doAction = function(object, act, params, controlId) {
	var fail = function(e) {
		log && log("_doAction Error", e);
		modal.error("Bind action error", e);
	};
	try {
		var method = "_act" + $.camelCase("-" + act);
		if (controlId.length > 0) {
			var object = object.getControl ? object.getControl(controlId) : null;
			if (object == null) throw new Error("Can't exec action[" + act + "] on control id[" + controlId + "] - COntrol not found");
		}
		if (!object[method]) { /* We check if it isn't an action of main application */
			object = globals.getApplication();
			if (!object[method]) {
				throw new Error("_doAction - Object's method " + method + " not found");
			}
		}
		if (object === globals.getApplication() || object.isActive || controlId.length > 0) {
...
var _bindActions = function(object, selector, $$elmt) {
	...
	$$elmt.delegate(selector || "[data-nav], [data-action]", "click select change", function(evt) {
		try {
			...
			if (clickedElmt.length > 0) {
				...
				globals.getApplication().changePage(attr, options)
			...
			if (clickedElmt.length > 0) {
				...
				_doAction(object, clickedElmt.attr("data-action"), param, controlId);
				return;
			}
		} catch (e) {
	...
	// global handling of events on the page
	eventListener.bindEvents($$elmt, object);
};
