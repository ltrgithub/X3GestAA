...
var _Klass = utils.defineClass(
	...
	function() {
		...
		appendTimeSelector: function() {
			...
			// empty container
			uiUtils.empty(this.parentSlot);

			// fill with time selector
			this.parentSlot.appendChild(timeSelectorDom);
		...
		onTimeClick: function(picker) {
			this.field.setFieldValue(this.dateValue + " " + _timeApi.parse(picker.textContent).toString());
		...
		bindEvents: function(bind) {
			...
			if (bind) {
				...
				this.$$modalElmt.delegate("td, a", "click", function(evt) {
					self.onClick(evt);
					evt.preventDefault();
					evt.stopPropagation();
				});
			} else {
				this.$$modalElmt.undelegate();
