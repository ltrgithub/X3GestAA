...
var _Klass = utils.defineClass(
	...
	function() {
		...
		onClick: function(evt) {
			...
			while (evt.target) {
				if (evt.target.syraOnClick) {
					close = this[evt.target.syraOnClick](evt.target, evt);
					if (close) {
						uiUtils.closeModal(self.field.id);
					}
					return;
				} else {
		...
		onTodayClick: function() {
			...
			this.field.setFieldValue(_dateApi.fromInternalValue(this._currentDate._value).toString(), this._currentDate._value);
			return true;
		},
		onDayClick: function(picker) {
			this.field.setFieldValue(_dateApi.fromInternalValue(picker.syraValue).toString(), picker.syraValue);
			return true;
		},
		...
		onMonthClick: function() {
			...
			if (!self._month) {
				...
				self._month.create(self.parentSlot, self.field, self._currentDate, function(newDate) {
		...
		create: function(parentSlot, field, options) {
			...
			_dateApi = require('syracuse-tablet/html/js/helpers/types/date');
		...
		_bindEvents: function(bind) {
			var self = this;
			if (bind) {
				$(self.domItem).delegate("td, a", "click", function(evt) {
					self.onClick(evt);
					evt.preventDefault();
					evt.stopPropagation();
				});
