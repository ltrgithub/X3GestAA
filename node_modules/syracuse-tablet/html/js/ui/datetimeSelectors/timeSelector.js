...
var _Klass = utils.defineClass(
	...
	function() {
		...
		create: function(field, options) {
			...
			this.curTime = field.getValue() ? _timeApi.parse(field.getValue()) : _timeApi.now();
			this.field = field;
			...
			for (var rr = 0; rr < rowCount; rr++) {
				...
				for (var col = 0; col < this.columns; col++) {
					...
					if (cellIndex < cellCount) {
						...
						link.textContent = timeHour.toString(this.field.getFieldFormat());
		...
		onTimeClick: function(picker) {
			this.field.setFieldValue(_timeApi.parse(picker.textContent, this.field.getFieldFormat()).toString());
			return true;
		},
		bindEvents: function(bind) {
			var self = this;
			var $$timeSelector = $('.s-time-choice');
			if (bind) {
				$$timeSelector.delegate("a", "click", function(evt) {
					self.onClick(evt);
					evt.preventDefault();
					evt.stopPropagation();
				});
			} else {
				$$timeSelector.undelegate();
			}
		},
		destroy: function() {
			//TODO to complete
