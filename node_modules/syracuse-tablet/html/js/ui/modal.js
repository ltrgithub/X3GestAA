...
var _info = function(title, text, cb, fieldId) {
	_modal("info", {
		close: locale.text("modal.btn.close"),
		title: title || "Warning",
		body: text,
...
var _modal = function(id, context, cb, _nbTries) {
	...
	/* Always to retry read template and display aletr if not found */
	$.when(templates.execSync(mid, context)).always(
		function() {
			if (arguments[0] == null || utils.isError(arguments[0])) {
				...
				h.on("shown.bs.modal", function() {
					...
					uiutils.addFieldModal(h, context.fieldId);
				});
				h.on("hidden.bs.modal", function() {
					...
					uiutils.removeFieldModal(context.fieldId);
				});
