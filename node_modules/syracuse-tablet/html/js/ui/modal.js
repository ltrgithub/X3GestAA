				...
				<section>\
					<div class="s-m-modal-content row">\
						...
						<div class="col-md-4">\
							...
							{{#each diags}} \
								<div class="alert alert-{{this.severity}}" role="alert"> \
									{{this.message}} \
								</div> \
							{{/each}} \
							<footer><button type="button" class="btn btn-default" data-dismiss="modal">{{close}}</button></footer>\
...
var _diagnoses = function(title, diag, cb) {
	...
	ctx.title = title || (ctx.severity === "error" ? titlelocale.text("modal.error.ttl") : titlelocale.text("modal.info.ttl"));
	return _modalStatic("diagnoses", ctx, cb);
};
...
var _action = function(title, text, success) {
	...
	$$actMsgSlot = $$actMsgSlot.length == 0 ? (isTablet ? $("#s-m-app-id") : $("section.s-m-main-content")).prepend($("<div>")).addClass("s-m-act-msg-slot") : $$actMsgSlot;
