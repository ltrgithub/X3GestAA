...
var _error = function(title, e, cb) {
	var diag;
	diag = _convertToDiagnoses(title, e);
...
var _convertToDiagnoses = function(title, e) {
	var o = e;
	if (e == null) {
		// -> Transform title string to error diagnoses since there is no more information given
		o = title;
	} else if (typeof e === "string") {
		// -> Transform string to error diagnoses
		o = e;
	...
	o.forEach(function(diag) {
		...
		if (d.exception) {
			if (d.exception.message) {
				d.$message = (d.$message ? d.$message + "\n" : "") + d.exception.message;
			}
			if (d.exception.stack) {
				d.$stackTrace = (d.$stackTrace ? d.$stackTrace + "\n" : "") + jsutils.cleanStack(d.exception.stack);
			}
			delete d.exception;
		}
		if (d.detail) {
			if (jsutils.isError(d.detail)) {
				if (d.detail.message) {
					d.$stackTrace = (d.$stackTrace ? d.$stackTrace + "\n----\n" : "") + d.detail.message;
				}
				if (d.detail.stack) {
					d.$stackTrace = (d.$stackTrace ? d.$stackTrace + "\n----\n" : "") + jsutils.cleanStack(d.detail.stack);
				}
			} else {
				d.$stackTrace = (d.$stackTrace ? d.$stackTrace + "\n" : "") + d.detail;
			}
