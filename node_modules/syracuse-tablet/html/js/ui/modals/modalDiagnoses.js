...
var _templateAll = {
	...
	footer: '\
		<buttons>\
		{{#if showmore}}\
			<button type="button" id="s-m-msg-stacks" class="btn btn-default" data-action="more">{{stackShow}}</button>\
		{{/if}}\
			<button type="button" class="btn btn-default" data-action="$cancel">{{close}}</button>\
		</buttons>\
		'
};
...
var _Klass = utils.defineClass(
	function ModalDiagnoses(title, diag) {
		...
		/*var diagAdd={
				...
				$severity:"error",
				$stackTrace:"Stack trace message 2 ↵ligne ↵ligne ↵linge",
		}
		diag.$diagnoses.push(diagAdd);
		 diagAdd={
					$message:"test message 3",
					$severity:"error",
					$stackTrace:"Stack trace message 3 ↵ligne ↵ligne ↵linge",
			}
		diag.data.$diagnoses.push(diagAdd)
		*/
		...
		_getDataContext: function() {
			var self = this;
			var ctx = {
				close: locale.text("modal.btn.close").toUpperCase() || "CLOSE",
				stackShow: locale.text("modal.diagnoses.btnstack_show").toUpperCase(),
				severity: "info",
				diags: [],
				showmore: false
			};
		...
		_more: function() {
			var self = this;
			var $$btn = $("#s-m-msg-stacks", self.$$elmt);
			var $$parent = $("#s-m-msg-block-" + self.curMsg);
			var $$stackSection = $("section", self.$$elmt);
			if ($$stackSection.css("display") === "none") {
				$$stackSection.css("display", "block");
				$$btn.text(locale.text("modal.diagnoses.btnstack_hide").toUpperCase());
				self._handleGesture();
			} else {
				$$stackSection.css("display", "none");
				$$btn.text(locale.text("modal.diagnoses.btnstack_show").toUpperCase());
			}
		},
		...
		_checkSize: function() {
			...
			var maxHeight = this.$$elmt.height() - (parseInt($(".modal-dialog", this.$$elmt).css("marginTop"), 10) + $(".modal-dialog", this.$$elmt).height() + 10)
			$$stacks.css({
