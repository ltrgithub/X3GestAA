	...
	function BaseModal() {
		...
		_checkShowNextModal: function() {
			...
			if (_openModalsPromises.length > 0) {
				_openModalsPromises[0].resolve(this);
			}
		},
		/*
		 * =======================================
		...
		_compileTpl: function() {
			...
			var tpl = {
				...
				headerContent: htmlTpl.headerContent ? Handlebars.compile(htmlTpl.headerContent) : "",
		...
		_ensureParent: function() {
			...
			if ($$parent.length < 1) {
				...
				if (globals.isAuthoringActive()) {
					...
					$("#s-m-app-id").show().addClass("s-m-modal-parent " + site.getDeviceType() + " " + site.getPageOrientation()).append($$parent);
				}
			}
			self.$$parent = $$parent;
			self.$$parent.addClass(site.getDeviceType() + " " + site.getPageOrientation());
		},
		...
		show: function(forceDisplay) {
			...
			startShow.then(function(previousDlog) {
					return $.smResolve().then(function() {
						return self._getDataContext();
					}).then(function(data) {
						self.data = data;
						if (previousDlog && utils.areEqual(self.data, previousDlog.data)) {
							// Is same info displayed we skip the dialog
							// Can occurs if a process posts multiple times the same dialog
							return true;
						}
						return false;
					})
				...
				.then(function(stop) {
					...
					var headerContent = self.template.headerContent ? self.template.headerContent(self.data) : "";
					if (header.length > 0) {
					...
					if (headerContent.length > 0) {
						headerContent = '<div class="modal-header-content">' + headerContent + '</div>';
					}
					self.html = outerTpl({
						header: header,
		...
		_calcSize: function() {
			...
			var $$headerContent = self.$$elmt.find(".modal-header-content");
			...
			var headerContent = $$headerContent.length > 0 ? uiRect.elmtRect($$headerContent, "outer").height : 0;
			$$headerContent.css({
				"top": header
			})
			$$body.css({
				"top": header + headerContent,
				"height": $$content.height() - header - footer - headerContent
			});
