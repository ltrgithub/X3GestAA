...
var _templateAll = {
	header: '\
		...
		<div class="modal-title">{{subtitle}}</div> \
			',
	content: ' \
			<div class=""> \
				<div class="form-group"> \
					<table class="s-m-pinandlivetile"><tr> \
					{{#each items}}\
					<td><div class="s-m-title">{{this.title}}</div>\
					<div class="s-m-value"><input id="{{this.id}}" type="checkbox" data-value="{{this.value}}"></div></td>\
					{{/each}}\
					</tr> \
					</table> \
				</div> \
...
var _Klass = utils.defineClass(
	function ModalPinAndLiveTile(ctx) {
		...
		_getDataContext: function() {
			...
			data.subtitle = locale.text("pin.page.subtitle");
			return data;
		},
		_onShow: function() {
			var self = this;
