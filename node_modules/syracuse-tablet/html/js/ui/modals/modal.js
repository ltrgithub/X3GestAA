...
var nativeMsg = require('syracuse-tablet/html/js/helpers/native/nativeMessages');
...
var ModalPinAndLiveTile = require('syracuse-tablet/html/js/ui/modals/ModalPinAndLiveTile').Modal;
...
exports.modalPinAndLiveTile = function(options) {
	var deferred = $.Deferred();
	var modal = new ModalPinAndLiveTile(options, null, true);
...
var _actionResult = function(title, obj, forceToast) {
	...
	if (!hasStack || forceToast === true) {
		...
		if (!options.onlyWebapp && nativeMsg.supportsShowToastMessage()) {
			// Adds the message on the toast message stack on windows
			nativeMsg.showToastMessage(options);
		}
