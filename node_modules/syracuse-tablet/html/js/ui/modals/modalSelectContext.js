...
var _Klass = utils.defineClass(
	function ModalSelectContext(context, settings, displayOpts, isOffline) {
		if (settings == null || !settings.roles || settings.roles.length == 0) {
		...
		if (isOffline) {
			...
			self.displayOpts.hideRememberMe = true;
		}
		...
		_getDataContext: function() {
			...
			return settings.getLoginRememberMe().then(function(rememberMe) {
				...
				var showRememberMe = self.displayOpts.hideRememberMe !== true,
					rememberme_opts = [];
				if (showRememberMe) {
					[{
		...
		onRoleChanged: function(event) {
			...
			var html = [];
			this._getEndPoints().forEach(function(ep) {
				...
				html.push('>');
				html.push(ep.description || ep.code);
				html.push('</option>');
			});
			this._$$endPoints.html(html.join('')).show();
		},
