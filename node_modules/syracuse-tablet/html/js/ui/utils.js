...
var _doAction = function(object, act, params) {
	...
	try {
		...
		if (!object[act]) {
			throw new Error("_doAction - Object's method " + act + " not found");
		}
		$.when(object[act].call(object, act, params)).then(function(todo) {
			if (todo) {
				if (todo.page) {
					window.smApp.changePage(todo.page);
				}
			}
		}, function(e) {
			fail(e);
	...
	$elmt.delegate(selector || "[data-nav], [data-action]", "click", function(evt) {
		try {
			...
			if (clickedElmt.length > 0) {
				...
				evt.preventDefault();
				evt.stopPropagation();
				window.smApp.changePage(attr, {}).then(function(dstPage) {
			...
			if (clickedElmt.length > 0) {
				evt.preventDefault();
				evt.stopPropagation();
				_doAction(object, clickedElmt.attr("data-action"), clickedElmt.attr("data-aparams"));
				return;
			}
		} catch (e) {
