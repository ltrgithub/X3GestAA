...
var _createPage = function($parent, state, options) {
	...
	try {
		if (type === "html" && state.name === "login") {
			...
			globals.getApplication().dao.getRegularPageInfo(state)
				.then(function(info) {
...
define('syracuse-tablet/html/js/application/authentication',['require','exports','module','syracuse-tablet/html/js/helpers/logger','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/ajax/ajax','syracuse-tablet/html/js/sdata/sdataCommonResources'],function (require, exports, module) {
...
function _initApplications(context) {
	return settings.getMyApplications()
		.then(function(apps) {
			...
			globals.setValidApplications(matchedApps);
			return context;
		}).fail(function(e) {
...
define('syracuse-tablet/html/js/authoring/properties',['require','exports','module','syracuse-tablet/html/js/helpers/locale'],function (require, exports, module) {
...
var _template = '\
	...
	<div class="tab-content"> \
		<div id="authPanelControl" class="tab-pane panel-body active"> \
			...
			<div class="tab-content"> \
				<div id="authPanelControlSimple" class="tab-pane panel-body active"> \
				</div> \
...
var AuthInterface = utils.defineClass(function() {
	...
	selectionChangeHandler: function(sel) {
		...
		self._updateAuthProps(props, article);
	},
	_updateAuthProps: function(props, article) {
		var self = this;
		if (!props) {
			return;
		}
		var $$arc = $('<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">');
		var id = 0;
				...
				<div class="panel panel-default"> \
					<div class="panel-heading" role="tab" id="heading' + id + '"> \
						<h4 class="panel-title"> \
							<a data-toggle="collapse" data-parent="#accordion" href="#collapse' + id + '" ' + (id === 0 ? 'aria-expanded="true"' : '') + ' aria-controls="collapse' + id + '">' + prop.label + '</a> \
						</h4> \
					</div> \
					<div id="collapse' + id + '" class="panel-collapse collapse' + (id === 0 ? ' in' : '') + '" role="tabpanel" aria-labelledby="heading' + id + '"> \
					</div> \
				</div>');
			$$arc.append($$block);
			var $$content = $('#collapse' + id, $$block);
			prop.createMarkup(prop, article && article[prop.key], $$content);
			id++;
		});
		self.$$authPanelControlSimple.append($$arc);
	},
...
var _Page = utils.defineClass(
	function($$parent, state, prototype, article, options) {
		...
		onResize: function(evt) {
			...
			if (this.rootLayout && this.rootLayout.onResize) this.rootLayout.onResize(evt);
		},
		/**
		...
		addControl: function(c) {
			...
			if (globals.smInstanceOf(c, "chart")) {
				this.chartsCtrls.push(c);
			}
		},
		/**
...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		_beforeAddContent: function() {
			var self = this;
			if (!self.isVignette) self.addTitle(globals.getApplication().getTitle() + " - " + self.prototype.data("$title"));
		},
...
var _Dao = utils.defineClass(function(data, prototype, parent, options) {
	...
	getArrayEmptyRow: function(protoArray) {
		var self = this;
		var rsrc;
		if (protoArray.isSingleArray()) {
		...
		return new _Dao(rsrc, protoArray, self);
	},
	/**
	 * Returns query info
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, options) {
		...
		getTitle: function() {
			return (this.article && this.article.$title) || this.prototype.getPropTitle(this.$bind);
		},
		/**
		 * Applies meta-data
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, options) {
		...
		afterRender: function() {
			...
			self.applyMetaData(self.getMetaData());
		},
		/**
...
var _Klass = utils.defineClass(
	function(control, type, options) {
		 ...
		 */
		buildHtml: function($$parent, arrayData, buildOptions, classes) {
			// For convenience - root control has been created by arrayBas ctrl
			this.$$elmt = this.control.$$elmt;
			if (classes) this.$$elmt.addClass(classes.join(' '));
			return this.$$elmt;
		},
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		isExcluded: function(item, idx) {
			...
			return idx + 1 >= self.maxCols || _isExcluded.indexOf(item.data("$type")) >= 0;
		},
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		buildHtml: function($$parent, arrayData, buildOptions) {
			var self = this;
			Base.prototype.buildHtml.call(self, $$parent, arrayData, buildOptions, ["s-m-grid s-m-horizontal"]);
	...
	function(controller, type, article, opts) {
		...
		_setHeight: function() {
			...
			if (self.displayOptions && self.displayOptions.rowHeight) {
				self.$$elmt.css("height", self.displayOptions.rowHeight);
			}
		}
	}
);
...
var _Klass = utils.defineClass(
	function(controller, type, article, opts) {
		...
		onResize: function(evt) {
			...
			var rel = w / h;
			var orientation = (rel > 1.2 && h >= 400) ? "landscape" : "portrait";
			self.$$elmt.height(h);
			var posX = 0;
			var posY = 0;
			var opts;
...
var _Klass = utils.defineClass(function(controller, type, article, opts) {
	...
	_setCellColorClass: function() {
		var self = this;
		var color = self.article.$cellColor;
		if (!color) {
			color = "lime";
		}
		self.$$cell.addClass("cell-" + color);
	},
	...
	function(controller, article, prototype, opts) {
		...
		onResize: function(evt) {
			...
			if (this._loaded && this._currentPage && this._currentPage.state.options.$displayStyle === "$full") {
				// console.log("vignette onResize " + this.id)
				this._currentPage.onResize(evt);
			} else {
				// console.log("vignette onResize skipped " + this.id)
			}
		},
		/**
			...
			if (!self._pageLoader) {
				...
				self._pageLoader = new PageLoader({
					...
					succeeded: function(dstPage, parentNotifInfo) {
						...
						self._loaded = true;
						// console.log("vignette loaded" + self.id)
						// Call resize handler each time a page has been loaded for calculations
						self.onResize(null);
					},
					loadingError: function(deferred, state, e) {
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			this.$$elmt.text("Unknown type: " + this.$type);
		}
	}
);
...
define('syracuse-tablet/html/js/controls/chart/ctrlCubeChart',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/controls/chart/ctrlCubeChartBase','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/helpers/formatApi','syracuse-tablet/html/js/helpers/locale'],function (require, exports, module) {
			...
			if (evt.target.nodeName.toLowerCase() === 'input') {
				...
				if (files && files.length) {
					...
					if (typeof FileReader !== "undefined") {
						...
						alert(locale.text("image.err.noFileReader"));
					}
				}
			}
		},
