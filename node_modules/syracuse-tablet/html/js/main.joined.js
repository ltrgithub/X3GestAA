...
var _error = function(title, e, cb) {
	...
	if (e == null) {
		...
		body = JSON.stringify(e, null, 2).replace(/\\n/g, '\n');
			...
			globals.getApplication().dao.getRegularPageInfo(state)
				.then(function(info) {
					if (info.prototype.isLookup()) {
						...
						res = new _ctor.RowDetailPage($parent, state, info.prototype, info.article);
					} else {
	...
	$$elmt.delegate("[data-nav], [data-nav-refresh], [data-action]", "click select change", function(evt) {
		try {
			...
			if (clickedElmt.length > 0) {
				...
				// Parameters are passed in page options
				attr = _attr(clickedElmt, "data-params");
				if (attr.length > 0) {
	...
	function(callerInterface) {
		...
		_pageActivate: function(pageToOpen, cb, back) {
			...
			pageToOpen.activate(self.getCurrentPage(), function() {
				self.setCurrentPage(pageToOpen);
				if (cb) cb();
			});
		},
...
var _Prototype = utils.defineClass(function(json, parent) {
	...
	isEditMode: function() {
		var url = this.data("$url");
		return url && (url.indexOf("$create") > -1 || url.indexOf("$edit") > -1);
	},
...
define('syracuse-tablet/html/js/authoring/authoringHelpers',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/locale'],function (require, exports, module) {
...
exports.enableAuthCss = function() {
	...
	$(".s-m-control.s-m-auth").on("click", _OnSelectControl);
}

exports.disableAuthCss = function() {
	$(".s-m-control.s-m-auth").off("click", _OnSelectControl);
	$(".s-m-control").removeClass("s-m-auth");
}

function _OnSelectControl() {
	$(".s-m-control.s-m-auth").removeClass("s-m-auth-selected");
	$(this).addClass("s-m-auth-selected");
	return false;
}
});

define('syracuse-tablet/html/js/authoring/authoring',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/locale','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/authoring/authoringSData','syracuse-tablet/html/js/authoring/authoringHelpers','syracuse-tablet/html/js/ui/modal'],function (require, exports, module) {
			...
			<div class="modal-body"> \
				...
				<div class="tab-content"> \
					<div id="authPanelControl" class="tab-pane panel-body active"> \
						<div id="authControlName" class="form-control">Comming soon</div> \
					</div> \
...
var _createControl = function(controller, parentStructElmt, article, prototype, options) {
	...
	switch (type) {
		...
		case _types.doc:
		case _types.binary:
			c = new _ctor.Binary(controller, article, prototype);
			break;
		case _types.icon:
			c = new _ctor.Icon(controller, article, prototype);
			break;
		default:
...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		isEditMode: function() {
			return this.prototype.isEditMode();
		},
...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		/**
		 * Load the page
		 * options.parentPage:		parentPage from where we opened rowdetail page
		 * 							!!! Do not store parentPage
		 */
		load: function(options) {
			var self = this;
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, options) {
		...
		self.$isEditMode = prototype.isEditMode();
	}, Base, {
		destroy: function() {
			var self = this;
			self.prototype = null;
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, options) {
		...
		/**
		 *  buildOptions.valueFirst === true : Display label before
		 */
		buildHtml: function($$parent, controllerDao, buildOptions) {
			var self = this;
			...
			// valueFirst see combo-box
			var valueFirst = buildOptions.valueFirst === true || self.getAuthoring("$value.$valueFirst", false);
			if (valueFirst) {
...
var _PhoneField = utils.defineClass(
	...
	function(controller, article, prototype) {
		var self = this;
		_AlphaNum.call(self, controller, article, prototype);
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			buildOptions.valueFirst = self.$isEditMode;
			Base.prototype.buildHtml.call(self, $$parent, controllerDao, buildOptions);
		},
...
var _Klass = utils.defineClass(
	function(control, type) {
		...
		appendField2Cell: function($$parent, fieldName, fieldProto, data) {
			var self = this;
			if (fieldName == "$MORE$") {
				$('<div class="s-m-control s-m-field"><div class="s-m-value"><span class="glyphicon glyphicon-remove"></span></div></div>');
			} else {
	...
	function(control) {
		...
		_buildTableRow: function(tabBody, rowData, idx, options) {
			...
			self.columnInfo.forEach(function(col, idx) {
				rowIdx = options.displayRowIdx && col.$bind === reqProto.rowIdxProperty;
...
var _Klass = utils.defineClass(
	function(controller, article, prototype) {
		var self = this;
		Base.call(self, controller, article, prototype);
		self.setBuilder(new builder(self));
	}, Base, {
			...
			<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">\
				<div class="container-fluid">\
					<div class="navbar-header">\
						...
						<a href="#" data-action="gotoWelcomeApplication" class="navbar-brand">\
...
var _Klass = utils.defineClass(
	...
	function(controller, $type) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			var user = up && up.user && ((up.user.firstName ? up.user.firstName + " " : "") + up.user.lastName) || "unknown";

			var _mainCtx = {
				title: "Sage ERP X3 - Mobile",
				user: user,
				ctrlId: self.id,
				showAuthoring: (self.controller && self.controller.state && self.controller.state.disableAuthoring) !== true
			};
...
var _Klass = utils.defineClass(
	...
	function(controller, $type) {
		var self = this;
		Base.call(self, controller, $type, "right", "actions");
	}, Base, {
	...
	function(controller, article, prototype) {
		...
		buildFieldValue: function(parentSlot, buildOptions) {
			...
			if (this.$isEditMode) {
				...
				if (self.isImage) {
					var clearBtn = uiUtils.createDomElement("a", ["s-m-clear-binary-field ctrl-evt-click"], null, {
						"href": "#"
					});
					var uploadBtn = uiUtils.createDomElement("a", ["s-m-upload-binary-field ctrl-evt-click"], locale.text("image.label.addimage"), {
						"href": "#"
					});
...
define('syracuse-tablet/html/js/controls/ctrlIcon',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/ui/fontUtils','syracuse-tablet/html/js/controls/field/ctrlFieldBase','syracuse-tablet/html/js/ui/uiUtils'],function (require, exports, module) {
