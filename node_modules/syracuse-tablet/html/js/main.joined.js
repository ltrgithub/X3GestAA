...
var _templates = {
	...
	authPanelPropertyRadioColor: ' \
		<div class="btn-group" data-toggle="buttons"> \
			{{#each options}} \
				<div class="btn-group s-m-btn-col-2"> \
					<label class="tile-{{color}} btn btn-primary" id="{{this.id}}_lab" data-s-m-auth-value="{{this.value}}"> \
						<input type="radio" name="{{group}}" id="{{this.id}}">{{this.label}} \
					</label> \
				</div> \
			{{/each}} \
...
var AuthInterface = utils.defineClass(function() {
	...
	_updateLayout: function() {
		...
		if (self.$$authPanelRight.is(":visible")) {
			...
			self.$$authPanelControlSimple.css("height", (window.innerHeight - 270) + "px");
			...
			if ($article.$layoutType) {
				...
				if (lyt && $article.$items) {
					$article.$items.forEach(function($itm) {
						// Important: Do not clone article here since full tree has to be preserverd						
