...
var _defineClass = function(constructor, parent, members, enableInstanceOf) {
	...
	if (enableInstanceOf === true) {
		// If we need to jsutils.instanceOf we need to register the class name
		// Otherwise we don't addInheritanceInfo because it slows down application
...
var _Prototype = utils.defineClass(function(json, parent) {
	 ...
	 * fltrs			filters - string or array
	 * 					visible				actions displayed in footer and visible page
	 *					hidden				actions displayed in action panel
	 * 					all or empty		all actions
	 * 					nopagin				exlclude pagin links
	 */
	getLinks: function(controllerDao, fltrs, options) {
		var self = this;
		var result = [];
		if (!controllerDao) {
		...
		options = options || {};
		if (typeof fltrs === "string") fltrs = [fltrs];
		var filters = {
			all: fltrs == null || fltrs.indexOf("all") >= 0
		};
		if (!filters.all) {
			filters.visible = fltrs.indexOf("visible") >= 0;
			if (filters.visible) {
				filters.noPagin = fltrs.indexOf("nopagin") >= 0;
			} else {
				filters.hidden = fltrs.indexOf("hidden") >= 0;
			}
		}
		...
		if ($links && Object.keys($links).length > 0) {
			$.each($links, function(name, value) {
				try {
					...
					if (visible && _isOfficeLink(name)) return;
					if (visible && filters.noPagin && _isPaginLink(name)) return;
					if (filters.all || (filters.visible && visible) || (filters.hidden && !visible)) {
						var info = $.extend({}, value);
						info.page = link.page;
						info.name = name;
						info.sdataurl = link.sDataUrl;
						if (visible && (filters.all || filters.visible)) {
							_addFacetLink(info, name);
						} else if (!visible && (filters.all || filters.hidden)) {
...
var _templates = {
	...
	authPanelLeftStructure: '\
		<div class="s-m-site-auth-panel-content"> \
			...
			<div class="btn-group btn-group-justified" data-toggle="buttons" id="auth_page_design"> \
				<div class="btn-group"> \
					<label class="btn btn-primary active"> \
						<input type="radio" name="page_design" id="page_design_vertical" checked>{{label_vertical}} \
					</label> \
				</div> \
				<div class="btn-group"> \
					<label class="btn btn-primary"> \
						<input type="radio" name="page_design" id="page_design_horizontal">{{label_horizontal}} \
					</label> \
				</div> \
			</div> \
...
function _createMarkupJSON($$parent, prop, sel, data) {
	...
	$("textarea", $$panel).blur(function() {
		...
		if (obj) {
			...
			if (authVal.validateJSON(true, authVal.toSortedJSON(data.articlePage)) != null) {
				_notifyChangePropUI($$parent, sel, data);
			}
		}
	});
...
define('syracuse-tablet/html/js/authoring/authoringPanelLeft',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/locale','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/ui/modal','syracuse-tablet/html/js/authoring/authoringHelpers','syracuse-tablet/html/js/authoring/authoringArticleGen','syracuse-tablet/html/js/authoring/authoringHtml','syracuse-tablet/html/js/authoring/authoringVal'],function (require, exports, module) {
...
exports.Panel = utils.defineClass(function() {
	...
	_ensurePanel: function() {
		...
		$("#auth_page_design .btn", self.$$panel).click(function() {
			var $$this = $(this);
			var id = $("input", $$this).attr("id");
			var segs = id.split("_");
			self._setPageDesign(segs[segs.length - 1]);
		});
	...
	_buildPageStructTree: function(article) {
		...
		function _walkNode(node, $$parent, path, $$pageParent) {
			...
			if (node.$layoutType) {
				...
				label = title ? title + " (" + node.$bind + ")" : node.$bind;
		...
		_walkNode(article, self.$$structureRoot, "0", self.$$page);
	},
...
define('syracuse-tablet/html/js/authoring/authoringPage',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/notifications'],function (require, exports, module) {
...
exports.AuthPage = utils.defineClass(
	function AuthPage() {
		...
		notifAuthSetSelection: function(path) {
			...
			/*
			$(".s-m-main-content > .s-m-layout", self.$$page).animate({
				scrollTop: $$item.offset().top,
...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		 ...
		 */
		notifMainLayoutChanged: function(info, orientation) {
			var self = this;
			...
			if (self.currentPage) {
				self.currentPage.onMainPageResize(info, orientation);
			}
		},
				...
				self._pageLoader = new pageLoader.Klass({
					...
					succeeded: function(dstPage, parentNotifInfo) {
						...
						if (parentNotifInfo && parentNotifInfo.notifId) {
							 ...
							 */
							var notif = dstPage.id === parentNotifInfo.parentId;
							if (!notif && dstPage.isDashboard() != null) {
								// We can display a lookup full page for an edit displayed in a vignette
								notif = dstPage.findVignettePage(parentNotifInfo.parentId);
							}
							if (notif) {
						...
						// Async call to let display the page - I noticed that page height is not Ok if no setTimeout
						setTimeout(function() {
							dstPage.enableScroll(true);
						});
					}
				});
			}
...
var _Klass = utils.defineClass(function rect(top, left, height, width) {
	...
	setLeft: function(l) {
		this.left = Math.min(l, this.right);
		this.width = this.right - this.left;
	},
	setTop: function(t) {
		this.top = Math.min(t, this.bottom);
		this.height = this.bottom - this.top;
	},
	setRight: function(r) {
		this.right = Math.max(r, this.left);
		this.width = this.right - this.left;
	},
	setBottom: function(b) {
		this.bottom = Math.max(b, this.top);
		this.height = this.bottom - this.top;
	},
	toString: function() {
...
define('syracuse-tablet/html/js/controls/hammerScroller',['require','exports','module','syracuse-tablet/html/js/helpers/logger','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/ui/rect','syracuse-tablet/html/js/helpers/jsutils'],function (require, exports, module) {
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		_initGestureMgr: function() {
			...
			this._gestureMgr = new HammerScroller(this.getScrollElement(), typeScroll, valMax);
		},
		...
		getScrollOffset: function() {
			// Page never scroll
			// Only dashboard hub layout scrolls - see getScrollOffset od dashBoardPage
			return {
				top: 0,
				left: 0
			};
...
define('syracuse-tablet/html/js/pages/pageDashboard',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/pages/page','syracuse-tablet/html/js/helpers/logger','syracuse-tablet/html/js/ui/rect'],function (require, exports, module) {
...
var _Page = utils.defineClass(
	...
	function DashboardPage($parent, state, prototype, article) {
		...
		/**
		 * Scrolling is handled by layout hub
		 */
		scrollAllowed: function() {
			return false;
		},
		getScrollOffset: function() {
			return this.rootLayout.getScrollOffset ? this.rootLayout.getScrollOffset() : {
				top: 0,
				left: 0
			};
		}
...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		scrollAllowed: function() {
			...
			return true; //Base.prototype.scrollAllowed.call(this);
		},
		onMainPageResize: function(info, orientation) {
			Base.prototype.onMainPageResize.call(this, info, orientation);
...
var _noDataHtml = '	<br><br><br>\
					...
					<a draggable="false" href="#" data-action="vignetteBack" class="fa fa fa-arrow-circle-o-left" style="display: inline-block;"/>\
					</div>';
/**
...
var _Dao = utils.defineClass(function(data, prototype, parent, options, $bind) {
	...
	getArrayData: function($bind, protoArray) {
		...
		if (protoArray.isSingleArray()) {
			...
			rsrcs.forEach(function(rsrc) {
				res.$resources.push(new _Dao(rsrc, protoRow, self, null, $bind));
			});
		}
		return res;
	},
...
define('syracuse-tablet/html/js/controls/array/builderBase',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/ui/rect','syracuse-tablet/html/js/ui/fontUtils','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/controls/hammerScroller'],function (require, exports, module) {
		...
		buildHtml: function(arrayData, options) {
			...
			if (this._createColumnInfo(options).length > 0) {
				...
				if (arrayData.$resources.length > 0 || this.control.controller.isEditMode) {
					...
					if (scrollable) {
						// TODO optimize
						this.$$tbl = $('<table class="table table-condensed"/>');
						$('<div style="position:relative;overflow:hidden;width:100%;height:100%"/>').appendTo(this.$$value).append(this.$$tbl);
					}
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		singleCardBuild: function(buildOptions) {
			...
			this._gestureMgr = new _OneCardSwiper(this, buildOptions).init().triggerSwipe();
		},
		/**
		...
		multipleCardsBuild: function(arrayData, buildOptions) {
			...
			if (false && cardsPerRow > 1) {
				...
				//var $$tr = $(_tr).appendTo(_thead).appendTo(self.$$tbl);//.children(":first-child");
				for (var i = 0; i < cardsPerRow; i++) {
					$(_td).css({
...
var _KlassOld = utils.defineClass(
	function(control, options) {
		...
		buildHtml: function(arrayData, buildOptions) {
			...
			self.onResizeWindow();
			self.buildPagingLinks(arrayData, buildOptions);
		},
...
define('syracuse-tablet/html/js/controls/array/ctrlArray',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/controls/ctrlBase','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/ui/modal','syracuse-tablet/html/js/application/eventListener','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/application/daoSdata','syracuse-tablet/html/js/controls/array/builderTable','syracuse-tablet/html/js/controls/array/builderCardV','syracuse-tablet/html/js/controls/array/builderCardH'],function (require, exports, module) {
...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		refresh: function(controllerDao, options) {
			var self = this;
			if (self.builder == null) throw new Error("Null builder");
			self.builder.refresh(self._setArrayData(controllerDao, true), options);
		},
		onResize: function() {
			if (this.builder.afterShow) {
				this.builder.afterShow();
			}
		},
		...
		/**
		 * handle selection of a row - list -> detail
		 */
		_actSelect: function(rowId) {
			...
			self._notifSelectedRow(rowId, rowData);
			// Open the link
		 ...
		 */
		_actSelectLookup: function(rowId) {
		...
		_actRowDetail: function(rowId) {
			...
			try {
				...
				notifications.pageNotify("sm.select.row", self.controller, self.id, rowId, null, true);
				eventListener.triggerNavigation(self.$$elmt, attrs);
			} catch (e) {
...
define('syracuse-tablet/html/js/controls/layout/layoutHub',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/controls/layout/layoutBase','syracuse-tablet/html/js/controls/layout/layoutHubGroup','syracuse-tablet/html/js/ui/rect','syracuse-tablet/html/js/controls/hammerScroller'],function (require, exports, module) {
...
define('syracuse-tablet/html/js/controls/page/actionManager',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/sdata/sdataDispatcher','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/ui/fontUtils','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/helpers/jsutils','syracuse-tablet/html/js/application/eventListener'],function (require, exports, module) {
		...
		triggerLink: function(uuid, page) {
			...
			try {
				...
				if (self.getLinks().some(function(l) {
					...
					if (link.sdataurl && (link.$isAction === true || link.sdataurl.indexOf("$services") > 0)) {
...
var _ActionManager = function(vignette, controller) {
	var self = this;
	var vignette = vignette;
	self.actionManager = new ActionManager({
		$links: controller.prototype.getLinks(controller.dao, "hidden"),
		noMessage: false
	});
	var context = {
		title: "Actions",
					...
					succeeded: function(dstPage, parentNotifInfo) {
						...
						if (self.enableLeftPanelAcions) {
							self._actionManager = new ActionManager({
								$links: dstPage.prototype.getLinks(dstPage.dao, "hidden"),
								noMessage: false
							});
						}
						if (self.hasBtnsBar()) {
...
var _templates = {
	...
	headerLinks: '\
		<div class="list-group">\
			{{#if showAuthoring}} \
				<a href="#" data-action="designPage" class="list-group-item">\
					<span class="' + fontUtils.pageIcon("designPage") + '" />\
					' + locale.text("header.links.designPage") + '\
				</a>\
			{{/if}} \
			{{#if showLinks}} \
				<a href="#" data-action="clearCache" class="list-group-item">\
					<span class="' + fontUtils.pageIcon("clearCache") + '" />\
					' + locale.text("header.links.clearCache") + '\
				</a>\
				<a href="#" data-action="switchContext" class="list-group-item"> \
					<span class="' + fontUtils.pageIcon("switchContext") + '" />\
					' + locale.text("header.links.switchContext") + '\
				</a>\
			{{/if}} \
		</div>'
};
...
var _Klass = utils.defineClass(
	...
	function(controller, $type) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			/*
		// build header links dom
		var $$headerLinks = $(_getHtml("headerLinks",_mainCtx));
		self.$$elmt.append($$headerLinks);
		$$headerLinks.toggle();

		// bind click on user icon to open header links list
		var $$linksPicker = self.$$elmt.find(".s-m-header-links");
		$$linksPicker.on("click", function(){
			if (!$$headerLinks.is(":visible") && !self.hasLinks){
				$$headerLinks.position({
					my:"right top",
					at:"right bottom",
...
define('syracuse-tablet/html/js/controls/page/ctrlPageFooter',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/controls/structElmt','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/sdata/sdataDispatcher','syracuse-tablet/html/js/sdata/sdataUtils','syracuse-tablet/html/js/controls/page/actionManager','syracuse-tablet/html/js/ui/fontUtils','syracuse-tablet/html/js/helpers/jsutils'],function (require, exports, module) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			if (self.controller.isEditMode()) {
				...
				links.some(function(l) {
					// Ack for #5175 - Save action - Cache issue - Save button is not displayed
					if (l.name == "$save") {
						l.$isHidden = false;
						return true;
					}
...
var _templates = {
	navigLink: '<li class=""><a draggable="false"  href="#" data-action="switchApp" data-params="{{appName}}" data-control-id="{{ctrlId}}"><i class="{{icon}}"></i>&nbsp;<span>{{title}}</span></a></li>'
};
