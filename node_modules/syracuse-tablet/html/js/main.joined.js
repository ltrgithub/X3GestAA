...
var _action = function(title, text, success) {

	var isTablet = screen.width > 480;
	var options = {
		title: title,
		body: text,
		severityClass: success ? "success" : "error",
		deviceType: isTablet ? "tablet" : "phone",
		toggleTrigger: isTablet ? function($$elt) {
			$$elt.animate({
				width: 'toggle'
			});
		} : function($$elt) {
		...
		$$parentSlot: isTablet ? $("#s-m-app-id") : $("section.s-m-main-content")
	};
...
- toggleTrigger : functions that toggles content. function depends on device type
...
var _actionHandler = function(options) {
	...
	function toggle() {
		options.toggleTrigger($$actionModal);
		options.toggleTrigger($$closer);
	}
...
define('syracuse-tablet/html/js/authoring/authoringVal',['require','exports','module','syracuse-tablet/html/js/helpers/locale'],function (require, exports, module) {var locale = require('syracuse-tablet/html/js/helpers/locale');
...
define('syracuse-tablet/html/js/authoring/authoringProperties',['require','exports','module','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/locale','syracuse-tablet/html/js/authoring/authoringHtml','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/authoring/authoringVal'],function (require, exports, module) {
...
var _Klass = utils.defineClass(
	function(controller, $type, article, options) {
		 ...
		 * options	buildOptions optional - use constructor options parameter (self.options) by default
		 * Returns $$elmt
		 */
		generateHtml: function($$parent, controllerDao, destroyAfter, options) {
			var self = this;
			if (self._needReuse) {
				// If control 
				self._initReuse();
			...
			self.buildHtml($$parent, controllerDao, options || self.options);
		...
		setRootElement: function(e, $$parent) {
			...
			e.attr("data-layout-path", self.getAuthoringPath());
			return e;
		},
		log: function() {
...
var _Klass = utils.defineClass(
	...
	function(controller, article, prototype, options) {
		...
		applyLabelAuthoring: function() {
			...
			if ($labelAlign) {
				self.setLabelPosition($labelAlign);
			}
		},
		/*
		 *	Set field label position (top,right,bottom,left)
		 */
		setLabelPosition: function($lp) {
			...
			function left() {
				self.$$elmt.prepend($$title);
			}

			function right() {
				self.$$elmt.prepend($$value);
			}

			function top() {
				left();
				//TODO add CSS attribute ?
			}
	...
	function(control, options) {
		...
		_buildTableRow: function(tabBody, rowData, idx, options) {
			...
			self._columnInfo.forEach(function(col, idx) {
				...
				col.control.generateHtml(cell, rowData, col.control.options, false);
			});
		},
		...
		buildCellHtml: function($$parent, fieldName, fieldProto, rowData) {
			...
			ctrl.generateHtml($$parent, rowData, ctrl.options, true);
		}
	});
			...
			if (true || cardsPerRow > 1) {
				...
				arrayData.$resources.forEach(function(rowData, idx) {
					...
					$$cell = $(self.buildDetailLink(rowData, {
						tag: "td",
						parent: $$row, //$('<td>').appendTo($$row),
						createIfNoLink: true
					}));
					self._layoutRoot.generateHtml($$cell, rowData, false);
				});
				...
				"$items": [{
					...
					"$items": [{
						...
						"$article": {
							...
							"$items": [{
								"$bind": "$resources",
								"$title": "$display = cardV - $cardsPerRow = 1",
								"$cardsPerRow": 1,
								"$article": {
									"$display": "cardV",
									"$items": [{
				...
				"$items": [{
					...
					"$items": [{
						...
						"$article": {
							...
							"$items": [{
								"$bind": "$resources",
								"$title": "$display=cardV - $cardsPerRow=3 - $displayEmptyCells=true",
								"$article": {
									"$cardsPerRow": 3,
