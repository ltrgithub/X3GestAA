...
var _Klass = utils.defineClass(
	...
	function(controller, $type, options) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			var self = this;
			self.actionManager = new ActionManager({
				$links: self.controller.prototype.getLinks(self.getControllerDao(), "hidden"),
				noMessage: false
			});
			var context = {
				title: "Actions",
...
var _Klass = utils.defineClass(function(controller, article, prototype, opts) {
	...
	_renderGauge: function($c) {
		...
		if (gauge.$bindMin != null && gauge.$bindMax != null) {
			if (typeof gauge.$bindMin === "string") valMin = self.getControllerDao().getValue(gauge.$bindMin);
			else valMin = gauge.$bindMin;
			if (typeof gauge.$bindMax === "string") valMax = self.getControllerDao().getValue(gauge.$bindMax);
