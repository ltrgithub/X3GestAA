...
define('syracuse-tablet/html/js/helpers/siteLayout',['require','exports','module','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/notifications'],function (require, exports, module) {
...
var _templates = {
	authPanelMainStructure: '\
		<splitter id="s-m-site-auth-splitter"></splitter> \
		<div id="s-m-site-auth-panel-content"> \
			...
			<div class="tab-content"> \
				<div id="authPanelControl" class="tab-pane panel-body active"> \
					...
					<textarea  id="authJSONSelection" style="display:none" type="text"></textarea> \
...
define('syracuse-tablet/html/js/authoring/authoring',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/helpers/globals','syracuse-tablet/html/js/helpers/locale','syracuse-tablet/html/js/helpers/notifications','syracuse-tablet/html/js/authoring/authoringHelpers','syracuse-tablet/html/js/ui/modal','syracuse-tablet/html/js/authoring/authoringProperties','syracuse-tablet/html/js/authoring/authoringHtml','syracuse-tablet/html/js/ui/uiUtils'],function (require, exports, module) {
...
var AuthInterface = utils.defineClass(function() {
	...
	_ensureAuthPanel: function() {
		...
		$("#authPanelBtnSelectionApply").on("click", self._onApplySelection.bind(self));
		$("#authPanelBtnSelectionApply").on("click", self._onApplySelection.bind(self));
	...
	_initSplitter: function() {
		function getPageX(e) {
			var pageX = e.pageX || e.originalEvent && e.originalEvent && e.originalEvent.touches && e.originalEvent.touches[0] && e.originalEvent.touches[0].pageX;
			return pageX;
		}
		...
		function getRelX(e) {
			...
			var relX = pageX - splitter.pageX;
			splitter.pageX = pageX;
			return relX;
		}

		function start(e) {
			self.$$splitter.addClass("s-m-splitter-active");
			splitter.pageX = getPageX(e);
			splitter.mouseDown = true;
			return false;
		}

		function end(e) {
			if (splitter.mouseDown === true) {
				update(e);
				self.$$splitter.removeClass("s-m-splitter-active");
				var sel = authHelpers.getSelection();
				self._updateLayoutPage(sel);
				splitter.mouseDown = false;
				return false;
			}
			splitter.mouseDown = false;
		}
		...
		function update(e) {
			self.$$splitter.addClass("s-m-splitter-active");
			var relX = getRelX(e);
			var width = self.$$authPanelRight.width() - relX;
			uiutils.triggerResizeInternal({
				layout: {
					authPanelRight: {
						width: width
		...
		var self = this;
		self.$$splitter = $("#s-m-site-auth-splitter");
		...
		$(document.documentElement)
			.on("mousemove", move).on("touchmove", move)
			.on("mouseup", end).on("touchend", end);
		self.$$splitter.on("mousedown", start).on("touchstart", start);
	}
});
});
...
var StaticControls = function() {
	...
	this.addToControlMap = function(page) {
		for (var id in _map) {
			page.addControl(_map[id].control);
		}
	};
};
/**
	...
	function(control, options) {
		...
		_createColumnInfo: function(options) {
			...
			_additionalCols.forEach(function(col) {
				if ((edit && col.$activity === "edit") || (!edit && overflow && col.$activity === "display")) {
					self.columnInfo.push($.extend(true, {}, col));
				}
			});
			if (unfound.length > 0) {
				// Add unfound columns
		...
		_buildTableRow: function(tabBody, rowData, idx, options) {
			...
			self.columnInfo.forEach(function(col, idx) {
				rowIdx = self.displayRowIdx && col.$bind === reqProto.ROWIDXPROP;
				...
				if (!col.control) {
					col.control = self.createCellFieldCtrl(cell, col.$bind, fieldProto);
				}
				col.control.detachElmt(true);
				col.control.buildHtml(cell, rowData, col.control.options);
			});
		},
		...
		_getOtherColProto: function(colInfo, rowId) {
			...
			if (self.control.controller.isVignette) {
				...
				$action["data-nav-target"] = self.control.controller.openLinkInVignette() ? "vignette" : "application";
			}
			var params = {
				rowId: rowId,
		...
		appendField2Cell: function($$parent, fieldName, fieldProto, rowData) {
			var self = this;
			var ctrl = self.createCellFieldCtrl($$parent, self.control.controller, articleParser.createArticle(fieldName, fieldProto), fieldProto);
			ctrl.buildHtml($$parent, rowData, ctrl.options);
		 ...
		 * 	TODO Destroy the ctrl after getting html - no needs to store it
		 */
		createCellFieldCtrl: function($$parent, fieldName, fieldProto) {
			var self = this;
			if (!fieldProto) fieldProto = self.$itemProto.getPrototype(fieldName);
			if (!fieldProto) return null;
			return ctrlFactory.createCellFieldCtrl($$parent, self.control.controller, articleParser.createArticle(fieldName, fieldProto), fieldProto);
		}
	});
...
define('syracuse-tablet/html/js/controls/array/builderCardV',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/controls/array/builderBase','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/helpers/articleParser'],function (require, exports, module) {
...
define('syracuse-tablet/html/js/controls/array/builderCardH',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/controls/array/builderBase','syracuse-tablet/html/js/ui/uiUtils'],function (require, exports, module) {
...
var _Klass = utils.defineClass(
	function(control, options) {
		...
		buildHtml: function($$parent, arrayData, buildOptions) {
			var self = this;
			Base.prototype.buildHtml.call(self, $$parent, arrayData, buildOptions, ["s-m-card s-m-horizontal"]);
...
define('syracuse-tablet/html/js/controls/array/ctrlArray',['require','exports','module','syracuse-tablet/html/js/helpers/utils','syracuse-tablet/html/js/controls/ctrlBase','syracuse-tablet/html/js/ui/uiUtils','syracuse-tablet/html/js/ui/modal','syracuse-tablet/html/js/controls/array/builderTable','syracuse-tablet/html/js/controls/array/builderCardV','syracuse-tablet/html/js/controls/array/builderCardH'],function (require, exports, module) {
...
var _Klass = utils.defineClass(
	function(controller, article, prototype, options) {
		...
		buildHtml: function($$parent, controllerDao, buildOptions) {
			...
			Base.prototype.buildHtml.call(self, $$parent, controllerDao, buildOptions, ["s-m-array", "s-m-" + self.article.$article.$display]);
