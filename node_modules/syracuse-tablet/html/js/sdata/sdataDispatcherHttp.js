...
exports.Dispatcher = utils.defineClass(function() {}, null, {
	...
	"query": function(op, dataToSend, forceUseCache) {
		var self = this;
		var cacheRead;
		var usedCache;
		...
		if ((self._useCache(op, "read") !== true) || (self._isOnline() && forceUseCache !== true)) {
			...
			cacheRead = self._queryReadCache(op);
			usedCache = true;
		}

		return cacheRead.then(function(cacheResult) {
			var step;
			if (cacheResult != null) {
				...
				return self._queryDoHttp(op)
					.then(function(httpResult) {
						...
						if (!usedCache) {
							...
							return self._queryReadCache(op)
	...
	// Side effect: If we forceUseCache=true in a query (by example when we refresh the page in authoring mode) the pagination is not OK
	// 				In that case we force the page to relaod (forceUseCache=false) when we exit authoring - see _authCloseAuthoring
