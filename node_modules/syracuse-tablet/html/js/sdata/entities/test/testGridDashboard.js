"use strict";

exports.dashboards = {
	"testGridDashboard": {
		"$dashboardName": "testGridDashboard",
		"$title": "Arrays - Test dashboard",
		"$description": "Arrays - Test dashboard",
		"$vignettes": {
			"testAQMDEVICEQueryGrid": {
				"$uuid": "testAQMDEVICEQueryGrid",
				"$displayStyle": "$full"
			},
			"testAQMDEVICEDetailGrid": {
				"$uuid": "testAQMDEVICEDetailGrid",
				"$displayStyle": "$full"
			}
		},
		"$article": {
			"$layoutType": "stack",
			"$items": [{
				"$layoutType": "row",
				"$rowHeight": "385px",
				"$items": [{
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEQueryGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "$resources",
								"$title": "QUERY - $display=table",
								"$article": {
									"$display": "table",
									"$items": [{
										"$bind": "CODE"
									}, {
										"$bind": "CODECHR"
									}, {
										"$bind": "TEXTDES0"
									}, {
										"$bind": "CUR_REF"
									}]
								}
							}]
						}
					}]
				}, {
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEQueryGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "$resources",
								"$title": "QUERY - $display=cardH - $cardsPerLine=1 - $emptycell=withtitle",
								"$article": {
									"$display": "cardV",
									"$emptycell": "withtitle",
									"$cardsPerRow": 1,
									"$items": [{
										"$bind": "CODE"
									}, {
										"$bind": "CODECHR"
									}, {
										"$bind": "TEXTDES0"
									}, {
										"$bind": "CUR_REF"
									}]
								}
							}]
						}
					}]
				}]
			}, {
				"$layoutType": "row",
				"$rowHeight": "385px",
				"$items": [{
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEQueryGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "$resources",
								"$title": "$display=cardV - $cardsPerRow=2 - $emptycell=filled",
								"$article": {
									"$display": "cardV",
									"$emptycell": "filled",
									"$cardsPerRow": 2,
									"$items": [{
										"$bind": "CODE"
									}, {
										"$bind": "CODECHR"
									}, {
										"$bind": "TEXTDES0"
									}, {
										"$bind": "CUR_REF"
									}]
								}
							}]
						}
					}]
				}, {
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEDetailGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "CODE"
							}, {
								"$bind": "CPY_REF"
							}, {
								"$bind": "SITE_REF"
							}, {
								"$bind": "CRY_REF"
							}, {
								"$bind": "ADEVCOLREF1",
								"$article": {
									"$display": "table",
									"$items": [{
										"$bind": "~data~"
									}]
								}
							}, {
								"$bind": "ADEVCOLREF2",
								"$article": {
									"$display": "table",
									"$items": [{
										"$bind": "~data~"
									}]
								}
							}, {
								"$bind": "ADEVCTINT",
								"$article": {
									"$display": "table",
									"$items": [{
										"$bind": "~data~"
									}]
								}
							}, {
								"$bind": "ADEVADEVICEL",
								"$article": {
									"$display": "table",
									"$items": [{
										"$bind": "ADEV_ADEVICEL_CODE"
									}, {
										"$bind": "SUBCODE"
									}, {
										"$bind": "TEXTDES"
									}, {
										"$bind": "ADEV_TEXTAXX"
									}, {
										"$bind": "ADEV_SITE_REF"
									}, {
										"$bind": "ADEV_UNIT_REF"
									}, {
										"$bind": "STATE"
									}, {
										"$bind": "DIVERS"
									}, {
										"$bind": "ADEVADEVICELADEVICELL"
									}]
								}
							}, {
								"$bind": "ADEVADEVICEL",
								"$article": {
									"$display": "cardV",
									"$items": [{
										"$bind": "ADEV_ADEVICEL_CODE"
									}, {
										"$bind": "SUBCODE"
									}, {
										"$bind": "TEXTDES"
									}, {
										"$bind": "ADEV_TEXTAXX"
									}, {
										"$bind": "ADEV_SITE_REF"
									}, {
										"$bind": "ADEV_UNIT_REF"
									}, {
										"$bind": "STATE"
									}, {
										"$bind": "DIVERS"
									}, {
										"$bind": "ADEVADEVICELADEVICELL"
									}]
								}
							}]
						}

					}]
				}]
			}, {
				"$layoutType": "row",
				"$rowHeight": "385px",
				"$items": [{
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEQueryGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "$resources",
								"$title": "$display=cardH - $cardsPerLine=1 - $addEmptyCells=true",
								"$article": {
									"$cardsPerLine": 1,
									"$addEmptyCells": true,
									"$display": "cardH",
									"$items": [{
										"$bind": "CODE"
									}, {
										"$bind": "CODECHR"
									}, {
										"$bind": "TEXTDES0"
									}, {
										"$bind": "CUR_REF"
									}]
								}
							}]
						}
					}]
				}, {
					"$layoutType": "cell",
					"$widthXs": "6",
					"$items": [{
						"$bind": "testAQMDEVICEQueryGrid",
						"$article": {
							"$layoutType": "stack",
							"$items": [{
								"$bind": "$resources",
								"$title": "$display=cardH - $cardsPerLine=3 - $emptycell=withtitle",
								"$article": {
									"$cardsPerLine": 3,
									"$emptycell": "withtitle",
									"$display": "cardH",
									"$items": [{
										"$bind": "CODE"
									}, {
										"$bind": "CODECHR"
									}, {
										"$bind": "TEXTDES0"
									}, {
										"$bind": "CUR_REF"
									}]
								}
							}]
						}
					}]
				}]
			}]
		}
	}
};

exports.gadgets = {
	"testAQMDEVICEQueryGrid": {
		"$type": "$representation",
		"$title": "Query AQMCRUD",
		"entity": "AQMDEVICE",
		"action": "$query",
		"representation": "AQMCRUDM",
		"facet": "$query"
	},
	"testAQMDEVICEDetailGrid": {
		"$type": "$representation",
		"$title": "Detail AQMCRUD",
		"entity": "AQMDEVICE",
		"action": "$details",
		"representation": "AQMCRUDM",
		"facet": "$details",
		"keyParameter": "1"
	}

};