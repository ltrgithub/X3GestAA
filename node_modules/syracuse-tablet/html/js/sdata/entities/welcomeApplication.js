...
var _voiceCommandAddGadget = function(commands, title, voiceCommandContext) {
	if (!commands || !voiceCommandContext || !title) return commands;
	commands.push({
		title: title,
		context: $.extend(true, {}, voiceCommandContext)
	});
	return commands;
};
		...
		.then(function(pp) {
			...
			pp.forEach(function(page) {
				if (appNames.indexOf(page.applicationName) >= 0) {
					...
					_voiceCommandAddGadget(voiceBookmarks, gadget.$title, page.voiceCommandContext);
		...
		.then(function(hiddenAppsList) {
			...
			apps.forEach(function(appInfo) {
				...
				if (appInfo.applicationName !== _welcomeApplication.$application.applicationName && hiddenApps.indexOf(name) < 0) {
					...
					_voiceCommandAddGadget(voiceBookmarks, gadget.$title, {
						applicationName: appInfo.applicationName,
...
function _updateWelcomeDashboard(dashboard) {
	...
	var voiceBookmarks = native.hasCapability("voiceCommands") ? [] : null;
	...
	return $.smResolve()
		...
		.then(function() {
			...
			if (voiceBookmarks) {
				native.getModule("voiceCommands").setVoiceBookmarks(voiceBookmarks);
			}
			return dashboard;
		})
		.fail(function(e) {
