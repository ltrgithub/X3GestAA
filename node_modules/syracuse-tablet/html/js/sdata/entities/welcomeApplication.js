...
var _voiceCommandAddGadget = function(commands, gadget) {
	var ep = globals.getEndpoint();
	if (!commands || !gadget || !gadget.applicationName || !gadget.$title) return commands;
	commands.push({
		title: gadget.$title,
		context: {
			endpoint: gadget.endpoint || globals.getEndpoint(),
		...
		.then(function(pp) {
			...
			pp.forEach(function(page) {
				if (appNames.indexOf(page.applicationName) >= 0) {
					...
					_voiceCommandAddGadget(voiceBookmarks, gadget);
