...
var _getAppList = function() {
	...
	try {
		...
		var url = _consts.appListUrl;
		if (globals.isDvlpMode()) {
			// Ignore roles
			url += "?allApps";
		}
		_get(url).then(function(result) {
			if (result.isSuccess && result.responseJSON) {
				_cache.appsList = result.responseJSON.$resources;
				deferred.resolve(_cache.appsList);
			} else {
				throw new Error("Loading application list error\n" + JSON.stringify(result, null, 2));
			}
		}, function(e) {
			log && log("_getAppList rejected", e);
			return deferred.reject(e);
		});
	} catch (e) {
...
var _getAppDetail = function(appInfo) {
	...
	try {
		...
		_get(appInfo.$metaDataUrl).then(function(result) {
			if (result.isSuccess && result.responseJSON) {
				_cache.appsDetails[appInfo.$uuid] = result.responseJSON;
				deferred.resolve(_cache.appsDetails[appInfo.$uuid]);
			} else {
				return deferred.reject(new Error("Loading application detail error\n" + JSON.stringify(result, null, 2)));
			}
		}, function(e) {
			log && log("_getAppDetail rejected", e);
			return deferred.reject(e);
		});
	} catch (e) {
