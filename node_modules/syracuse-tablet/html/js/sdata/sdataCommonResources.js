 ...
 */
var _appListUrl = "/sdata/syracuse/collaboration/syracuse/mobileApplications/$service/availableApplications?endpoint=x3.erp.GX3APP";
...
exports.queryMyApplications = function() {
	...
	var endPoint = globals.baseLocation().query["endpoint"];
	// expected endpoint=GX3APP in url
	if (!endPoint) {
		deferred.reject(new Error("Endpoint expected in tablet application url like ?endpoint=GX3APP"));
		return deferred.promise();
	}
	var params = {
		method: "GET",
		url: _appListUrl + "?endpoint=x3.erp." + endPoint
	};
	...
	sdataHttp.send(params)
		.then(function(result) {
			if (result.isSuccess && result.responseJSON) {
				...
				deferred.reject(new Error("Loading user profile error\n" + JSON.stringify(result, null, 2)));
			}
		}, function(e) {
			deferred.reject(e);
		});
