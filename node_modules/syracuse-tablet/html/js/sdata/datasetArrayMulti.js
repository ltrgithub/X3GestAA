...
exports.MultiArrayDataset = utils.defineClass(
	...
	Base, {
		...
		getDataDelta: function(forceUpdateAll) {
			var deltaRows = [];
			var dirtyRows = forceUpdateAll === true ? this.getAllRows() : this._getOrderedDirtyRows();
			...
			for (var i = 0; i < dirtyRows.length; i++) {
				...
				if (row) { // modified row
					deltaRow = row.getDataDelta(forceUpdateAll);
		...
		applyDelta: function(delta, context) {
			...
			if (changedLinesOnly) {
				...
				this._applyDeltaIndexed(delta, context);
			} else {
				this._applyDeltaFull(delta, context);
			}
		},
		_applyDeltaIndexed: function(delta, context) {
			var self = this;
			...
			for (var ii = 0, jj = delta.length; ii < jj; ii++) {
				...
				if (!row) { // new row
					...
					row.applyDelta(deltaRow, context);
				}
			}
		...
		_applyDeltaFull: function(delta, context) {
			var self = this;
			var oldUuids = this.json.map(function(row) {
				return row.$uuid;
			});
		...
		_deltaSetValue: function($bind, value, context) {
		...
		buildTemplateData: function() {
			var result = [];
			for (var ii = 0, jj = this.json.length; ii < jj; ii++) {
				var row = this.getRowByUuid(this.json[ii].$uuid);
				if (row) {
					result.push(row.buildTemplateData())
				}
			}
			return result;
		}
