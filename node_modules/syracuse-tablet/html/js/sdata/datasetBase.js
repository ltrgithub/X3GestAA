...
exports.DatasetBase = utils.defineClass(
	...
	null, {
		...
		/**
		 * Returns the value given by json without any transformation
		 */
		getJsonValue: function($bind) {
			return this.json[$bind];
		},
		/**
		...
		setProperty: function($bind, prop, value) {
			if (!this.json.$properties) {
				this.json.$properties = {};
			}
			if (!this.json.$properties[$bind]) {
				this.json.$properties[$bind] = {};
			}
			this.json.$properties[$bind][prop] = value;
		},
		/**
		 * Return the value of the property or the whole object if prop == null
		 */
		getProperty: function($bind, prop) {
			var props = this.json.$properties ? this.json.$properties[$bind] : null;
			if (!props) {
				return null;
			}
			return props && prop ? props[prop] : props;
		},
		/**
		 * Accept prop1.prop2.prop3
		 */
		getClientContext: function(path) {
			var ctx = this.json.$clientContext || {};
			if (!path) {
				return ctx;
			}
			return utils.getPropByPath(ctx, path, null);
		},
		...
		_deltaSetValue: function($bind, value, deltaNotifications, context) {
			...
			if ($bind === "$properties") {
				...
				if (!this.prototype.propExists($bind)) {
					...
					if (context === "draft") {
						// Set the encoded B64 saved value
						this.json[$bind] = value;
					}
				}
			}
		...
		/**
		 * Returns only the functional data
		 */
		buildTemplateData: function() {
			var result = {};
			var properties = Object.keys(this.json);
			var self = this;
			Object.keys(this.json).forEach(function($bind) {
				if ($bind === "$properties" || $bind === "$uuid") {
					// Keep $properties meta and $uuid to renew them
					result[$bind] = self.json[$bind];
					return;
				}
				if ($bind.charAt(0) === "$" || !self.prototype.propExists($bind)) {
					return;
				}
				if (self.prototype.propIsSimple($bind)) {
					result[$bind] = self.json[$bind];
				} else if (self.prototype.propIsReference($bind)) {
					result[$bind] = self.json[$bind];
				} else if (self.prototype.propIsArray($bind)) {
					var array = self.getChild($bind);
					result[$bind] = array.buildTemplateData(self.getChild($bind));
				}
			});
			return result;
		},
		 ...
		 */
		getEditTemplateData: function() {
			return $.extend(true, {}, this.buildTemplateData());
		},
		/**
		 * Update the current dataSet with the data coming from template
		 */
		setEditTemplateData: function(page, data) {
			this.applyDelta(datasetUtils.templateAddRowId(data), "template");
		...
		getDraftsData: function() {
			// Contrary to templates we keep all the properties of dataset
			// TODO check if it's actually true
			return $.extend(true, {}, this.json);
		},
		/**
		 * Set data from saved draft as for edit templates 
		 */
		setDraftsData: function(data) {
			// Contrary to templates we restore all the properties of saved dataset
			// TODO check if it's actually true
			this.applyDelta(data, "draft");
		},
		removeDiagnoses: function() {
			sdataUtils.scanDiagnoses(this.json, {
				remove: true
			});
		}
	}
