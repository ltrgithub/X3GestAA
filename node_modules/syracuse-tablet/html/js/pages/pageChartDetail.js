...
var _Page = utils.defineClass(
	...
	function ChartDetailPage($parent, state, prototype, article, parentPage, options) {
		...
		this._userPrefs = chartUserPrefs.createChartDetailPrefs(state.options["dataParams"]);
	}, Base, {
		...
		_findArrayCtrl: function(chartDisplay) {
			var self = this;
			var key = "_ctrlArrayChart_" + (chartDisplay ? "chart" : "array");
			var res = self[key];
			if (!res) {
				this.forEachControl(function(id, ctrl) {
					if (jsutils.isInstanceOf(ctrl, "CtrlArrayChart") && (chartDisplay ? ctrl.isHighChart() : true)) {
						res = self[key] = ctrl;
						return true;
					}
				});
