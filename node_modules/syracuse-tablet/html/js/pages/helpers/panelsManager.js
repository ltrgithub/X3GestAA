...
var _PanelManager = function(page) {
	...
	this.panelToggle = function(where, panel, action) {
		...
		if (!$$root) return;
		if (!panel) panel = self.controller.getPageControl(where);
		...
		var visible = $$root.is(':visible') === true;
		var newWidth = visible ? $$root.width() : 0;
		if (panel.closeBox != true) {
			self.$$dismissPanel[visible ? "addClass" : "removeClass"](where).css(where, newWidth);
		}
	...
	this.closeAll = function(context) {
		var self = this;
