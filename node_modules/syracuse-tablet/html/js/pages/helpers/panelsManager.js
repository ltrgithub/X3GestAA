...
var _PanelManager = function(page) {
	...
	this.panelCreate = function(panelId, options) {
		...
		return ctrlFactory.createPanel(this.controller, panelId, options);
	};
	/**
	 * Call resize method of current panels
	 */
	this.panelResize = function(context) {
		this.forEachPanel(function(where, panel) {
			if (panel.onResize) {
	...
	/**
	 * Manages orientationChanged and deviceChanged
	 */
	this.onMainPageResize = function(info, orientation, deviceType) {
		var self = this;
		setTimeout(function() {
			if (self.checkDestroyed()) return;
			// Timeout to delay in order to be able to shift element (overlap = false - see panelToggle)
			self.forEachPanel(function(where, panel) {
				if (info.deviceChanged) {
					self.panelToggle(where, panel, "close");
				} else if (info.orientationChanged) {
					self.panelToggle(where, panel, "resize");
				}
				// call panel's method
				if (panel.onMainPageResize) {
					panel.onMainPageResize(info);
				}
			});
		});
	};
	this.panelMainPageResize = function(info) {
		if (info.deviceChanged || info.orientationChanged) {
			["right", "left"].forEach(function(w) {
				var p = controller.getPageControl(w);
				if (p && p.onMainPageResize) {
					p.onMainPageResize(info);
				}
			});
		}
	};
