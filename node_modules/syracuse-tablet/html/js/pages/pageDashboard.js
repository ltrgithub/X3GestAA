	...
	function DashboardPage($parent, state, prototype, article, options) {
		...
		savedCtxCreate: function(type) {
			...
			if (this.isDashboard("hub") && this.rootLayout) {
				var hubCtx = this.rootLayout.savedCtxCreate(type);
				if (hubCtx) {
		...
		getPinPageId: function() {
			var self = this;
			var parameters = JSON.stringify(self.state.options.sdataParameters);
			return self.state.dashboardName + (parameters ? "_" + parameters : "");
		},
		...
		getPinPageData: function() {
			...
			var sdParam = this._sdataParam2String();
			if (sdParam != null && sdParam.trim().length > 0) {
				// In case of a filtered dashboard, may be null
				pageData.sdataParameters = sdParam;
			}
		...
		getRestoreContext: function(reason) {
			...
			if (!ctx.currentState.options.vignettesPageStates) {
				ctx.currentState.options.vignettesPageStates = {}
			}
