...
var _Page = utils.defineClass(
	...
	function DashboardPage($parent, state, prototype, article, options) {
		...
		countChartLoaded: function(chart) {
			if (this._chartsCounter == null || !globals.getCurrentPage().isDashboard("hub") || !jsutils.checkCurrentBrowser("chrome")) return;
			this._chartsCounter++;
		...
		getPinPageData: function() {
			...
			pageData.sdataParameters = this._sdataParam(); // In case of a filtered dashboard, may be null
		...
		getRestoreContext: function(reason) {
			...
			var params = this._sdataParam();
			if (params) {
				ctx.currentState.options["sdataParameters"] = params;
			}
			...
			console.log(JSON.stringify(ctx.currentState, null, 2));
			return ctx;
		},
		/**
		...
		isDashboard: function(typeDashboard) {
			return typeDashboard ? this.rootLayout && this.rootLayout.$type === typeDashboard : true;
		},
		getGestureMgr: function() {
			return this.isDashboard("hub") ? this.rootLayout.getGestureMgr() : Base.prototype.getGestureMgr.call(this);
		},
		...
		checkBack: function() {
			...
			if (this._editModeVignettes.length === 1) {
				// If there's only one pagein edit mode this is thos page that manages the chekBack
				return this._editModeVignettes[0].checkBack();
			} else {
				// If there eara more than one pages in edit mode we check thglobal dirty
				var isDirty = this._editModeVignettes.some(function(page) {
					return page && page.dao && page.dao.isDirty;
				});
				if (!isDirty) return true;
				return this.adkDiscardChanges();
			}
		},
		adkDiscardChanges: function() {
			var opts = {
				buttons: [],
				defaultAction: "no"
			};
