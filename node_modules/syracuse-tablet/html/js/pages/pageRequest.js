...
var _Klass = utils.defineClass(
	function RequestPage($parent, state, prototype, article, options) {
		...
		getPagePrefsId: function() {
			var id = "";
			// FDB - We test destroyed because if there's a page request pinned on a dashboard when we open a page from a context
			// the dashboard is closed before the vignette is initialized (to open the page given by the context) 
			if (this.isVignette && !this.destroyed) {
				var vignette = this.getParentVignette();
				if (vignette && !vignette.destroyed) {
