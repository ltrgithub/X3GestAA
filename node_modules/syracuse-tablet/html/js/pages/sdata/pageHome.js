	...
	Base, {
		...
		renderRootLayout: function() {
			...
			if (this.controller.isConfigActive()) {
				self.rootLayout.setBuildOption("LayoutHubGroup", "showAddTiles", true);
				self.rootLayout.setBuildOption("LayoutTile", "tileActions", self._defineTileActions());
			} else {
				self.rootLayout.setBuildOption("LayoutHubGroup", "showAddTiles", null);
				self.rootLayout.setBuildOption("LayoutTile", "tileActions", null);
			}
		...
		_actChangeGroup: function(tileId) {
			...
			modal.show().then(function(groupId) {
				if (groupId) {
					//TODO: Move tile to this group
				}
			});
		},
			...
			_getAvailabeNotUsedApps(this.pageData.page.$article, this.pageData.dataset.json)
				...
				.then(function(result) {
					...
					if (result && result.length > 0) {
						self._addAppsToGroup(result, hubGroupId);
					}
				})
				.then(function() {
					var states = [];
		...
		_addAppsToGroup: function(apps, hubGroupId) {
			...
			apps.forEach(function(app) {
				var vignetteUuid = _addVignetteAndGadget(self.pageData.dataset.json, self.pageData.prototype.json, app)
				if (vignetteUuid) {
					_bindVignetteToHubGroupCtrl(hubGroupCtrl, vignetteUuid);
				}
			});
		},
	}
