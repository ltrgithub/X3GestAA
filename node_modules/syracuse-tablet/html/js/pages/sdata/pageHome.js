...
exports.Page = utils.defineClass(
	function PageHome(pageData, options) {
		...
		this.actionAdapter.showGlobalPanelButton = true;
	...
	Base, {
		...
		_actChangeGroup: function(tileId) {
			...
			var groupIdTile = ctrlTile.parent.id;
			var groups = _getOtherGroups(this.rootLayout, groupIdTile);
			...
			modal.show().then(function(groupIdDest) {
				if (groupIdDest) {
					if (self.rootLayout.children.some(function(group, idx) {
						...
						if (self._changeGroup(grpIdxDest, groupIdTile, ctrlTile.id)) {
							self._refresh(true);
						}
					}
				}
		...
		_changeGroup: function(grpIdxDest, groupIdTile, tileId) {
			var self = this,
				grpIdx, tileIdx;
			var grpIdx = self._getIdxGroup(groupIdTile)
			if (grpIdx > -1) {
		...
		_getIdxGroup: function(groupId) {
			var grpIdx = -1
			this.rootLayout.children.some(function(group, idx) {
				if (group.id === groupId) {
					grpIdx = idx;
					return true;
				}
		...
		_actConfigTile: function(tileId) {
			...
			var grpIdxDest;
			if (ti) {
				var ctrlTile = self.controller.getControl(tileId);
				var groupId = ctrlTile.parent.id;
				var groups = _getAllGroups(self.rootLayout, groupId);
				var modal = new ModalConfigTile({
					tile: ti,
					groups: groups
				});
				modal.show().then(function(result) {
					if (result) {
						var groupDest = ""
						self._setTileInfo(result.tile);
						if (result.group === "$new") {
							self._addGroup();
							grpIdxDest = self.rootLayout.children.length //Always the last one
						}
						if (result.group && groupId !== result.group) {
							grpIdxDest = grpIdxDest || self._getIdxGroup(result.group)
							if (self._changeGroup(grpIdxDest, groupId, tileId)) {
								self._refresh(true);
							}
						} else {
		...
		_getTileInfo: function(tileId) {
			...
			var ctrlVignette = ctrlTile
			if (ctrlTile.isLayout()) {
				ctrlVignette = ctrlTile.getChildByIndex(0);
			}
			var $$vignette = $("#" + tileId);
			var $$tile = $$vignette.closest(".s-m-tile");
			if (ctrlVignette && ctrlVignette.$bind) {
				var article = ctrlTile.isLayout() ? ctrlTile.getArticle() : ctrlTile.parent.getArticle();
				...
				tileInfo = {
					...
					title: ctrlVignette.getTitle(),
					stackTitle: (ctrlTile && ctrlTile.$$elmt) ? $(".s-m-stack header", ctrlTile.$$elmt).text() : null,
					size: article.$size,
		...
		_setTileInfo: function(tileInfo) {
			var article = tileInfo.ctrl.article = tileInfo.ctrl.article || {};
		...
		_moveTile: function(opts) {
			...
			if (opts.where === "append") {
				layoutGroupTarget = self._getGroupLayout(opts.dropId);
		...
		_getGroupLayout: function(groupId) {
			return this.getRootLayout().children.find(function(layout) {
				return layout.id === groupId
			})
		}
	}
);
...
function _getAllGroups(rootLayout, groupId) {
	var groups = [];
	rootLayout.children.forEach(function(group) {
		groups.push({
			title: group.getArticleText("$title"),
