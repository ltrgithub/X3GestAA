...
exports.Page = utils.defineClass(
	function PageSdata(pageData, options) {
		this.pageData = this._adaptPageData(pageData); // provide entry point to change something in page data if necessary
		options = options || {};
	...
	Base, {
			...
			this.rootLayout.children.forEach(function(elmt, index) {
				if (elmt.$type === "stack") {
					if (idx === index || idx === undefined) {
						elmt.show()
						notifications.publish(["sm.stack.show"]);
					} else {
						elmt.hide()
					}
				}
		...
		_adaptPageData: function(pageData) {
			if (pageData.page && pageData.page.$article && pageData.page.$article.$localization) {
				var langToRemove = [];
				Object.keys(pageData.page.$article.$localization).forEach(function(lang) {
					if (lang !== lang.toLowerCase()) {
						langToRemove.push(lang);
						pageData.page.$article.$localization[lang.toLowerCase()] = pageData.page.$article.$localization[lang];
					}
				});
				langToRemove.forEach(function(lang) {
					delete pageData.page.$article.$localization[lang]
				})
			}
			return pageData;
		}
