	...
	Base, {
		...
		_initMultiSelItems: function() {
			...
			if ((items && items.length > 0) || this.page.isDownloadEnabled()) {
				...
				this._addMultiSelectionToggle(items, "left");
		...
		getMultiSelectionItems: function(panelControllerId) {
			var items = this.getItems("multiSelection").filter(function(item) {
				return (!item.options || item.options.footerHeader)
			});
		...
		getMultiSelectionSidePanelItems: function() {
			var items = this.getItems("multiSelection");
			return items.filter(function(item) {
				return (item.options && item.options.multiselection);
			})
		},
		...
		getActionsPanelItems: function(options) {
			options = options || {};
			this.visibleLinks = options.visibleLinks || this.visibleLinks;
			options.visibleLinks = this.visibleLinks;
			options.recompute = true; // We compute items all time, if we do not, dynamic metadata like hidden/disabled is lost
		...
		getItems: function(panelName, options) {
			...
			if (!items || options.layoutChange || options.recompute) {
		...
		_addFooterHeaderActions: function(items, slot) {
			...
			actions.forEach(function(act) {
				if (act.options && act.options.footerHeader === true && act.options.multiselection !== true) {
					if (knownLinks[act.name] && utils.isResolve(knownLinks[act.name].$url)) {
						self._addSdataLinkItem(items, act.name, controller, $.extend(true, {
							slot: slot
						}, act));
					} else if (knownActions[act.name] && utils.isResolve(knownActions[act.name].$url)) {
		...
		_computeActionsPanelItems: function(visibleLinks) {
			...
			for (var $linkName in $links) {
				if (!$linkName.smStartsWith("$") && utils.isResolve($links[$linkName].$url)) {
			...
			for (var $actionName in $actions) {
				if (!$actionName.smStartsWith("$") && utils.isResolve($actions[$actionName].$url) && !visibleLinks[$actionName]) {
		...
		_pushItem: function(items, item, overrides) {
			...
			if ((item.options && item.options.multiselection) || item.name == "multiSelTriggerAction") {
