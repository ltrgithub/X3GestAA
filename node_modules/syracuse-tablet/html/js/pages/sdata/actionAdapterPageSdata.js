	...
	ActionAdapter, {
		...
		_initMultiSelItems: function() {
			...
			if ((items && items.length > 0) || this.sdataPage.isDownloadEnabled()) {
				...
				items.unshift({
					"action": this.sdataPage.isDownloadEnabled() ? "multiSelActionsWithDownload" : "multiSelActions"
				});
		...
		_getPageActionSettings: function() {
			...
			if (!actions) {
				if (this.sdataPage.isDownloadEnabled()) {
		...
		_computeHeaderItems: function() {
			...
			if (this.showAccept) { // what is it ?
				this._addAcceptButton(items, controller, {
					slot: "right"
				});
			}
			if (this.showCancel) { // what is it ?
