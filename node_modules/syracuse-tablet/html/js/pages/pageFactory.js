...
var _classes = {
	...
	PageDrafts: require("syracuse-tablet/html/js/pages/drafts/pageDrafts").Page,
	PageSdataDraftable: require("syracuse-tablet/html/js/pages/drafts/pageSdataDraftable").Page,
	...
	return $.smResolve()
		.then(function() {
			if (url.indexOf("html://") === 0) {
				if (pageCreateData.name === "HOME") {
					return _fetchSDataPageHomeData(options)
...
function _fetchSDataPageHomeData(options) {
	return $.smResolve()
		.then(function() {
			return wpHelpers.fetchWelcomePageData(options);
		})
		.then(function(pageData) {
...
function _createSdataPage(pageData, pageCreateData, options) {
	...
	pageData.name = pageCreateData.name;
	if (facet === "$edit" && pageData.dataset.isStateLessMode()) {
		return new _classes.PageSdataDraftable(pageData, options);
	}
	switch (facet) {
		case "$query":
