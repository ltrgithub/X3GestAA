...
var _Page = utils.defineClass(
	...
	function WelcomeDashboardPage($parent, state, prototype, article, options) {
		...
		_getHeaderFooterOptions: function() {
			...
			opts.onlyTemplate = optList.onlyTemplate
			return opts;
		},
		 ...
		 * The goal is to provide a list of available template ,
		 * The user could take on of these to replace its own dashboard.
		 */
		_actTemplateList: function() {
			var self = this;
			var opts = this._optDashboardList();
			var modal = new modalChooseWelcomeDashboard.Modal(opts);
			modal.show().then(function(result) { // result : $uuid of the dashboard selected
				if (result) {
					dashboardWelcomeUtils.getWelcomeDashboard(true, result)
						.then(function(dashboard) {
							self._updateWelcomeDashboard(dashboard, {
									dirty: true
								})
		...
		/* from the footer only for authorized user and NOT in editing mode
					...
					dashboardWelcomeUtils.getWelcomeDashboard(true, result)
						.then(function(dashboard) {
							self._updateWelcomeDashboard(dashboard, {
								...
								.then(function() {
									//Need to recompute because change article but not only, proto changed too
									//TODO : avoid it. This will change with the new process to shwith dashboard.
		...
		_optDashboardList: function(ownerOnly) {
			...
			//If its a personal dashboard and the user don't have the right to create or modified a template
			// Only the liste if template
			var onlyTemplate = optSave.personal && !optSave.publish && !optSave.savePublish;
			var optList = {
				"title": locale.text(onlyTemplate ? "welcome.select.template.title" : "welcome.select.dashboard.title"),
		...
		_actReplacePersonalDashboard: function(title) {
			var self = this;
