	...
	function WelcomeDashboardPage($parent, state, prototype, article, options) {
			...
			modal.show().then(function(result) {
				if (result && result.length > 0) {
					$.smForEachPromise(result, function(uuid) {
						...
						globals.getApplication().welcomeDashboard.dirty = true
		...
		_actCloseDashboardConfig: function() {
			...
			if (!opts.publish) { // if user not authorized to publish set restaure personal dashboard
				...
				step = $.smResolve(globals.getApplication().welcomeDashboard);
			};
		...
		_actSavePersonalDashboard: function() {
			var self = this;
			dashboardWelcomeUtils.writeWelcomeDashboardUser(globals.getApplication().welcomeDashboard)
				.then(function(dashboard) {
