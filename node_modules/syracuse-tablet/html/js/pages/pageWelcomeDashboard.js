...
var _Page = utils.defineClass(
	...
	function WelcomeDashboardPage($parent, state, prototype, article, options) {
		...
		_afterChangePage: function() {
			Base.prototype._afterChangePage.call(this);
		...
		_actRemoveGroup: function(id) {
			var self = this;
			Base.prototype._actRemoveGroup.call(self, id).then(function(dashboard) {
				dashboard.dirty = true;
				globals.getMetaData().updateWelcomeDashboard(dashboard).then(function() {
					self._afterChangePage();
				});
		...
		_moveTile: function(opts) {
			...
			if (idxTarget >= 0) {
				if (opts.where === "bottom" || opts.where === "right") {
					idxTarget++;
					if (idxTarget >= layoutGroupTarget.article.$items) {
						idxTarget = -1
					}
				};
		...
		_actDashboardAddGroup: function() {
			var self = this;
			globals.getMetaData().getDashboard(self.prototype.getDataByPath("$dashboardName")).then(function(dashboard) {
				dashboard.$article.$items.push(dashboardWelcomeUtils.hubGroup(dashboardWelcomeUtils.groupType("default"), "My applications"));
				dashboard.dirty = true
				globals.getMetaData().updateWelcomeDashboard(dashboard).then(function() {
					self._afterChangePage();
				});
