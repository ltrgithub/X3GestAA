	...
	function WelcomeDashboardPage($parent, state, prototype, article, options) {
		...
		_actRemoveTile: function(vignetteId) {
			...
			if (groupType === dashboardWelcomeUtils.groupType("default") || groupType === dashboardWelcomeUtils.groupType("sageAppli")) {
				step = globals.getMetaData().getDashboard("$welcomeDashboard").then(function(dashboard) {
		...
		_applyTileChanges: function(tileInfo, tile) {
			...
			globals.getMetaData().getDashboard("$welcomeDashboard").then(function(dashboard) {
				dashboard.$article = self.article;
		...
		_actCloseDashboardConfig: function() {
			...
			var uuid = null;
			if (!opts.publish) { // if user not authorized to publish set restaure personal dashboard
				step = dashboardWelcomeUtils.getWelcomeDashboard(false, self.views[0].$uuid)
			} else {
				step = globals.getMetaData().getDashboard("$welcomeDashboard")
			}
			step.then(function(dashboard) {
		...
		_actSavePersonalDashboard: function() {
			var self = this;
			globals.getMetaData().getDashboard("$welcomeDashboard").then(function(dashboard) {
				dashboardWelcomeUtils.writeWelcomeDashboardUser(dashboard)
					.then(function(dashboard) {
						globals.getMetaData().updateWelcomeDashboard(dashboardWelcomeUtils.setMetaWelcomeDashboard(dashboard)); // update views
						self.afterChangePage();
						//return $.smResolve();
					}).then(function() {
						var notify = {
