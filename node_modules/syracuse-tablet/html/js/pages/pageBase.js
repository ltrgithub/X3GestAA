...
exports.Page = utils.defineClass(
	...
	null, {
		...
		_getAttachedControlInfo: function(name) {
			...
			var defaults = {
				...
				tablet: {
					...
					editTemplatesPanel: {
						selector: "aside.s-m-panel.s-m-right",
						$type: "application/x-panel-edit-templates"
					}
				}
			};
		...
		getPinPageData: function() {
			var NameType = this.pageName.split(".");
			var clientContext = this.pageData.dataset.getValueByPath("$clientContext");
			var represention = this.pageData.prototype.getRepresentation() || null;
			var urlParsed = sdataUtils.parseSDataURL(this.pageData.$url);
			return {
				$title: this.getPageTitle() + (urlParsed.id ? " (" + urlParsed.id + ")" : ""),
				$type: represention || NameType[1],
