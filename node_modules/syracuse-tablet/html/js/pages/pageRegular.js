		...
		_doActSave: function(success, link, responseJSON, options) {
			...
			if (success) {
				...
				if (responseJSON && responseJSON.$properties) {
					// We update dao only with $properties that contain potential diagnoses
					// We don't update all data because X3 doesn't return input data (if we input a bad SITE (llokup) 3 returns empty value
					self.dao.update({
