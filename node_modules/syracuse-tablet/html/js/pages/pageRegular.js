	...
	function RegularPage($parent, state, prototype, article, options) {
		...
		loadData: function(loadOptions) {
			...
			return self.loadFromSData(loadOptions).then(function(data) {
				...
				self.freshness = self._dataFrechness(data);
				notifications.publish("sm.data.freshness.change", self.freshness);
				//notif necessary here in case of next/previous datas in grid
			});
		},
		/*data.$cacheHint:{}
		 *data.$cache: Object
		  ...
		  data.$resources[0].$cache
				...
				$lastUpdated: "2015-11-10-12-04-50
		 */
		_dataFrechness: function(data) {
			var freshNess = {
				dateTime: 0
			};
			var getResourcesTime = function($resources) {
				var utcTime = utils.getUTCTime(new Date());
				$resources.forEach(function(item) {
					utcTime = Math.min(utils.getUTCTime(item.$cache.$lastUpdated), utcTime);
				});
				return utcTime;
			};
			freshNess.dateTime = utils.getUTCTime(new Date());
			if (data.dateTime) {
				//nothing for moment
				freshNess.dateTime = Math.min(freshNess.dateTime, utils.getUTCTime(new Date()));
			}
			if (data.$cacheHint) {
				if (data.$cache) {
					freshNess.dateTime = Math.min(freshNess.dateTime, utils.getUTCTime(data.$cache.$lastUpdated));
				} else {
					freshNess.dateTime = Math.min(freshNess.dateTime, getResourcesTime(data.$resources));
				}
			};
			return freshNess;
		},
