			...
			if (self._multiSelectionLinks == null) {
				...
				if ($links) {
					...
					$.each($links, function(name, value) {
						...
						actSettings = self.actionGetSettings(name, "options");
						if (actSettings == null || actSettings.multiselection !== true) {
							return;
						}
		...
		getToolbarLinks: function(toolbar) {
			...
			if ($links) {
				...
				$.each($links, function(name, value) {
					...
					// multiselection action are not display in panel or footer/header - Managed by multiselection manager
					if (authSettings.options && authSettings.options.multiselection === true) return;
					forceFooterHeader = authSettings.options && authSettings.options.footerHeader === true;
