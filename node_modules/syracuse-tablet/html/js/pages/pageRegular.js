...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		_dataFrechness: function(data) {
			...
			if (data.$cacheHint) {
				if (data.$cache) {
					freshNess.dateTime = Math.min(freshNess.dateTime, utils.getTimeFromString(data.$cache.$lastUpdated));
				} else {
		...
		loadFromSData: function(loadOptions) {
			...
			return dispatcher.dispatch({
				...
				globals.getOfflineManager().setOffline(data && (data.$cache || data.$cacheHint) && !forceUseCache);
