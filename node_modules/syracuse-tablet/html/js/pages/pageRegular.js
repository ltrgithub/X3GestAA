...
var modalYesNoCancel = require('syracuse-tablet/html/js/ui/modals/modalYesNoCancel');
...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		loadData: function(loadOptions) {
			...
			if (enableWorkingCopy) {
				step = self.loadCheckWorkingCopy(loadOptions);
			} else if (loadOptions.reload !== true && this.state.options["draft-id"]) {
				step = self.loadFromDraft(this.state.options["draft-id"]);
			} else {
				step = $.smResolve(null);
			}
			return step.then(function(data) {
				if (data) return $.smResolve(data, true);
				return self.loadFromSData(loadOptions);
			}).then(function(data, isWorkingCopy) {
				if (self.destroyed) {
				...
				if (loadOptions.refresh === true) {
					// Only on page refresh othewise see activate method - next/previous datas in grid...
		...
		loadCheckWorkingCopy: function(loadOptions) {
			if (loadOptions.reload === true) {
				...
				return globals.getStorage().draftOperation("wrkcpyRead", id);
			}
			// Nothing
			return $.smResolve(null);
		},
		...
		loadFromSData: function(loadOptions) {
			...
			return dispatcher.dispatch({
				...
				if (!forceUseCache) {
					// if forceUseCache don't know if we on/off line
		...
		_doActSave: function(success, link, responseJSON, options) {
			...
			if (success) {
				...
				if (!success && (responseJSON == null || (!responseJSON.$diagnoses || responseJSON.$diagnoses.length == 0))) {
					reason = responseJSON.$diagnoses.length[0].$message;
					// Unexpected
				...
				if (this._draftMgr) {
					this._draftMgr.setStatus("saveError", reason);
				}
			}
		},
		/**
		...
		checkBack: function() {
			if (this._draftMgr) {
				return this._draftMgr.checkBack();
			}
			return true;
		}
	});
...
var _DraftMgr = function(page) {
	...
	this.setStatus = function(status, reason) {
		...
		this._statusReason = reason || "none";
	};
	...
	this.checkBack = function() {
		var self = this;
		if (!this._page.dao || !this._page.dao.isDirty) return true;
		var modal = new modalYesNoCancel.Modal(locale.text("draft.save"), locale.text("draft.save.info"));
	...
	this.save = function() {
		var uuid = jsutils.getPropByPath(this._page, "dao.data.$uuid");
		if (!uuid) return;
		var restoredCtx = this._page.getRestoreContext();
		...
		var draftContext = {
			...
			"representation": this._page.prototype.data("$representation"),
			"status": this._status,
			"reason": this._statusReason,
			"title": this._page.prototype.data("$title") || "",
			"comment": "",
			"restoredContext": restoredCtx,
			"dataSet": this._page.dao.data,
