		...
		loadData: function(loadOptions) {
			...
			return step.then(function(data, loadedFrom) {
				...
				if (loadedFrom === "draft") {
					...
					self.dao.setDirty();
					// Remove the draft id - A new one will be created if the draft will be saved again
...
var _DraftMgr = function(page) {
	...
	this.checkBack = function(forcePromise) {
		...
		/**
		 * Discard changes Cancel/SaveAsDraft/No
		 */
		var modal = new modalYesNoCancel.Modal(locale.text("prompt.title.discardchanges"), locale.text("prompt.info.saveasdraft"), {
			no: locale.text("prompt.button.saveasdraft")
		});
		return modal.show().then(function(result) {
			if (result === "yes") {
				...
				/**
				 * Input a comment - yes/no
				 */
				var modal = new modalSaveDraft.Modal(self._comment || "");
				return modal.show().then(function(result) {
					result = result || {};
					if (result.action === "yes") {
						return self.save(result.comment).always(function() {
							return true;
						});
					}
					return false;
	...
	this.loadFromDraft = function(draftId) {
		...
		return globals.getStorage().draftOperation("draftRead", draftId).then(function(ctx) {
			return globals.getStorage().draftOperation("draftDelete", draftId).then(function() {
				...
				if (ctx && ctx.dataSet) {
					data = ctx.dataSet;
					// Used if we save the draft again
