	...
	function RegularPage($parent, state, prototype, article, options) {
		...
		loadData: function(loadOptions) {
			...
			return step.then(function(data, loadedFrom) {
				...
				if (self._draftMgr) {
					self._draftMgr.notifDataLoaded(loadOptions, loadedFrom);
				}
			});
		},
		/**
				...
				switch (link.name) {
					...
					case "$canceledit":
						self.checkBack(true).then(function(kontinue) {
							...
							var loadedFromDraft = self._draftMgr != null && self._draftMgr.pageLoadedFromDraft();
							// If data loaded from draft we go back to draft list
							if (self.$activity === "create" && !loadedFromDraft) {
								self.reload();
							} else {
								...
								self.goBack().then(function(newPage) {
									if (newPage == null || loadedFromDraft) return;
		...
		_doActSave: function(success, link, result, options) {
			...
			if (success) {
				if (self._draftMgr) {
					self._draftMgr.notifSaveSucceeded();
				}
				self.dao.resetDirty();
