...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		getOpenLinkAttrs: function() {
			...
			res["data-sdata-url"] = this._getSdataUrlToSend();
		 ...
		 *  options.force  True to force refresh
		 *  callBackInterface
		...
		_filterSortWrite: function(id, info) {
			if (!this._filterSortInfo) this._filterSortInfo = {};
			this._filterSortInfo[id] = null;
			if (info) {
				this._filterSortInfo[id] = info;
				sessStorage.setItem(this.sDataUrl + "?" + id, JSON.stringify(info));
			}
		},
		_getSdataUrlToSend: function() {
			var sortClause = null;
			var info = this._filterSortRead("sort");
			...
			info = this._filterSortRead("filter");
			if (info != null && info.length != 0) {
				// TODO
			}
			if (!sortClause && !filterClause) return this.sDataUrl;
			var url = this.sDataUrl;
			if (sortClause) {
				// TODO - improve with IRL object that parses url and rebuild url after modifs
				if (url.indexOf('?') === -1) {
					url += '?';
				} else {
					url += '&';
				}
				url += "orderBy=" + encodeURIComponent(sortClause.field + ' ' + sortClause.order);
			}
			if (filterClause) {
				// Todo
			}
