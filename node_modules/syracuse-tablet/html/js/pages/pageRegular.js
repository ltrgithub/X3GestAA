...
var _Page = utils.defineClass(
	function($parent, state, prototype, article, jsonInfo) {
		...
		buildHtmlContentBefore: function() {
			var self = this;
			if (self.vignette) {
				//	No title -> Displayed by vignette - html.push('<h5 class="s-m-page-title">' + self.state.name + (self.isCreateAtion() ? ".create" : "")  + "</h5>");
				return false;
			} else {
				return Base.uiutils.createDomElement("h3", ["s-m-page-title"], globals.getApplication().getTitle() + " - " + self.state.name);
			}
		},
		/**
