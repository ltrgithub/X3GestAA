...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		notifValidateActionLink: function() {
			var self = this,
				ok = true,
				focusControl;
			Object.keys(self.controlsMap).forEach(function(key) {
				var c = self.controlsMap[key];
				if (c && c.checkData) {
					ok = ok && c.checkData();
					if (!ok && focusControl == null) focusControl = c;
				}
			});
			if (focusControl) {
		...
		_doActSave: function(success, link, responseJSON, options) {
			var self = this;
			if (!success && (!responseJSON.$diagnoses || responseJSON.$diagnoses.length == 0)) {
