...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		notifications.subscribe(self, ["sm.lookup.selection", "sm.rowdetail.update"], 1);
		 ...
		 */
		validateBeforeActionLink: function() {
		...
		notifRowdetailUpdate: function(childDao, arrayId) {
			if (!childDao) return;
			// !! we force the parent with the actual this.dao object updateParentRow(this.dao)
			// !! childDao.parent is not the same object as this.dao (same data but not same object)
			// --> If we update childDao.parent nothing happends - see ctrlArray._actRowDetail
			if (childDao.updateParentRow(this.dao)) {
