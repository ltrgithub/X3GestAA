...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		filterSortWrite: function(type, info, ctrl, refresh) {
			...
			if (info && info.length > 0) {
				...
				if (old == null || !this._filterSorEquals(type, info, old)) {
					...
					sessStorage.setItem(id, JSON.stringify(info));
					if (refresh === true) {
						var attrs = {
		...
		_filterSortUpdateUrl: function() {
			if (!this._filterSortAllowed(true)) return this.sDataUrl;
			// Add filter/sort clause for Lookup and Query if any
			var sortClause = null;
			var info = this.filterSortRead("sort");
			...
			info = this.filterSortRead("filter");
			if (info != null && info.length != 0) {
				// TODO
			}
			if (!sortClause && !filterClause) return this.sDataUrl;
			var url = utils.parseURL(this.sDataUrl);
			// url.query.key != null menas it's a pagination request
