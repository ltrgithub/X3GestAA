...
var _getQueryOriginalUrl = function(url) {
	// We keep only the representation (no pagination info...)
	...
	function RegularPage($parent, state, prototype, article, options) {
			...
			try {
				dispatcher.dispatch({
					...
					if (data) {
						...
						if (data.$cacheHint && !self.isVignette) {
							var notify = {
		...
		filterSortWrite: function(type, info, ctrl, refresh) {
			...
			if (info && info.length > 0) {
				...
				if (old == null || !this._filterSorEquals(type, info, old)) {
					save = true;
				}
			} else {
			...
			if (save) {
				...
				if (refresh === true) {
					...
					// Set original URL - Remove pagination - Ex we cha,nge the fiter/sort criteria in a pagination 
					this.sDataUrl = _getQueryOriginalUrl(this.sDataUrl);
					setTimeout(function() {
						eventListener.triggerRefreshPage(ctrl.$$elmt, attrs);
					});
				}
			}
		},
		...
		_filterSortAllowed: function(log) {
			if (this.prototype.isLookup() || this.prototype.isQuery()) return true;
			if (log) {
				console.log("!! filterSort is allowed only for facets lookup and query");
			}
			return false;
		},
		_filterInfoSaveId: function(type, ctrl) {
			if (!this._filtersortUrlId) {
				this._filtersortUrlId = _getQueryOriginalUrl(this.sDataUrl);
			}
		...
		// true if info == old
		_filterSorEquals: function(type, info, old) {
			...
			if (type == "filter") {
				// TODO - test filterInfo -> true is equals
			}
		},
		_filterSortUpdateUrl: function() {
			var self = this;

			if (!self._filterSortAllowed(true)) return self.sDataUrl;
			...
			if (info != null && info.length != 0) {
				$.each(info, function(idx, filter) {
					var cf = '(' + filter.id + ' ' + (filter.operator === "like_s" ? "like" : filter.operator) + ' ' + self._setFilterRightValue(filter) + ')';
					filterClause = filterClause.length == 0 ? cf : filterClause + ' and ' + cf;
				});
			}

			// apply sort clause
			if (!sortClause && !filterClause) return self.sDataUrl;
			var url = utils.parseURL(self.sDataUrl);
			// url.query.key != null means it's a pagination request
			...
			if (sortClause && url.query.key == null) {
				var old = url.query.orderBy;
				url.query.orderBy = sortClause.field + ' ' + sortClause.order;
				if (old) url.query.orderBy = ", " + old;
			}
			...
			// -> The pagination request contains already the sort criteria so we don't add them
			// -> Otherwise we add sort criteria to the request
			if (filterClause && url.query.key == null) {
				// Todo
				var old = url.query.where;
				url.query.where = filterClause;
				if (old) url.query.where = old + ' and ' + url.query.where;
			}
		 ...
		 */
		_setFilterRightValue: function(filter) {
			var cp = filter.prop;
