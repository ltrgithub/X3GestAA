...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		refresh: function(options, callBackInterface) {
			...
			var _wait = function(status) {
				if (callBackInterface) {
					var meth = "wait" + status.smCapitalize();
		...
		getRestoreContext: function() {
			var ctx = Base.prototype.getRestoreContext.call(this);
			if (!ctx) return ctx;
		...
		loadFromSData: function(loadOptions) {
			...
			dispatcher.dispatch({
				...
				if (self.prototype.isQueryLikeFacet() && e != null && e.$redirectPage == null && e.$httpStatus !== 401) {
					// We display the message and resolve with an empty array to display the page
					// If the user entered a bad filter(where clause) he/she will be able to modify it otherwise he/she will get stuck
					globals.getModal().error(locale.text("error.page.loading", [""]), e, function() {
						deferred.resolve([]);
					});
				} else {
					deferred.reject(e);
		...
		notifActionLink: function(success, link, result, options) {
			...
			try {
				...
				switch (link.name) {
					//#119222 detail to query ($query link)
					case "backToQueryList":
						this.goBack();
						break;
					case "$save":
		...
		getToolbarLinks: function(toolbar, options) {
			...
			if ($links) {
				...
				$.each($links, function(name, value) {
					...
					if (name === "$query" && value.$url) {
						//#119222 - $query link in a detail/edit facet
						// If the previous page is a $query we go back
						// json["$repUrl"] to not resolve the url
						if (value.$url.indexOf(self.prototype.json["$repUrl"] + "?representation=" + self.prototype.data("$representation") + ".$query") >= 0) {
							var history = globals.getApplication().history;
							var prevPage = history.length > 1 ? history[history.length - 2] : null;
							if (prevPage && prevPage.name === value.page) {
								// call notifActionLink to go back
								value.name = "backToQueryList";
								value.isClientAction = true;
							}
						}
					}
