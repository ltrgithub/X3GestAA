...
var draftMgr = require('syracuse-tablet/html/js/pages/draftManager');
...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		loadData: function(loadOptions) {
			...
			return step.then(function(data, loadedFrom) {
				...
				if (loadedFrom === "draft") {
					...
					// We enable only save button with getDirtyInitStatus and keep dao not dirty
				}
				if (self._draftMgr) {
		...
		/**
		 * Returns the status to enable/disable save button when header/footer is created
		 * We force true if the page is loaded from a draft because the user can save without having modified the page
		 * See ctrlHelpers.checkDaoDirty
		 */
		getDirtyInitStatus: function() {
			return this.dao && (this.dao.isDirty || this._dataLoadedFrom === "draft");
		},
		/**
			...
			try {
				...
				switch (link.name) {
					...
					case "$delete":
						...
						break;
					case "$canceledit":
						self.checkBack(true).then(function(kontinue) {
							if (kontinue != true) {
		...
		_doActSave: function(success, link, result, options) {
			...
			if (success) {
				...
				if (self._draftMgr) {
					...
					self._draftMgr.notifSaveFailed(result);
				} else {
