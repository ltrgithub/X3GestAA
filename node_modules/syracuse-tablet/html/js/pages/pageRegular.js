...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		// Init working copy
		this.enableWorkingCopy = (this.$activity === "edit" || this.$activity === "create") && native.hasCapability("nativeApp");
	}, Base, {
		...
		destroy: function() {
			...
			} else if (this.isVignette && !keepWorkingCopy && this.dao && this.dao.getWorkingCopyId()) {
				// in a dashboard with multiple create/edit facest we just delete the current WorkingCopy because we don't know if other pages will be restored
				globals.getStorage().draftOperation("wrkcpyDelete", this.dao.getWorkingCopyId());
			}
		...
		getRestoreContext: function(reason) {
			...
			if (this.dao && reason === "nativeAppClosing" && this.enableWorkingCopy === true) {
				ctx.currentState.options["workingCopy-Id"] = this.dao.getWorkingCopyId();
			}
			return ctx;
		},
		...
		loadData: function(loadOptions) {
			...
			return step.then(function(data) {
				...
				self.setDao(data ? factory.createDaoSdata(data, self.prototype, {
					...
					createWorkingCopy: enableWorkingCopy && isWorkingCopy !== true
		...
		loadCheckWorkingCopy: function(loadOptions) {
			if (loadOptions.reload === true) {
				...
				if (this.dao && this.dao.getWorkingCopyId()) {
					globals.getStorage().draftOperation("wrkcpyDelete", this.dao.getWorkingCopyId());
				}
