...
var _Page = utils.defineClass(
	function RegularPage($parent, state, prototype, article, options) {
		...
		getRestoreContext: function(reason) {
			var workingCopyId;
			if (reason === "nativeAppClosing" && this._draftMgr && this.dao.data) {
				workingCopyId = this.dao.data.$uuid;
			...
			if (workingCopyId) {
				ctx.currentState.options["workingCopy-Id"] = workingCopyId;
			}
			return ctx;
		},
		...
		loadData: function(loadOptions) {
			...
			var step;
			if (this.state.options["workingCopy-Id"] && this._draftMgr) {
				step = self.loadFromWorkingCopy(this.state.options["workingCopy-Id"]);
			} else {
				step = $.smResolve(null);
			}
			return step.then(function(data) {
				return data || self.loadFromSData(loadOptions);
			}).then(function(data) {
				if (self.destroyed) {
		...
		loadFromWorkingCopy: function(id) {
			return this._draftMgr.readWorkingCopy(id).then(function(data) {
				return data;
			}).fail(function() {
				return null;
			});
