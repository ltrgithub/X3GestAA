	...
	function RegularPage($parent, state, prototype, article, options) {
		...
		loadData: function(loadOptions) {
			...
			try {
				...
				dispatcher.dispatch({
					$url: self._filterSortAllowed() ? self._filterSortUpdateUrl() : self.sDataUrl,
					...
					if (!data) {
						// If no json returned - needed for some requests
						data = daoSdata.emptyDao(self.prototype);
					}
			...
			try {
				...
				switch (link.name) {
					...
					default:
						...
						var settings = this.actionGetSettings(link.name);
		...
		scrollAllowed: function() {
			if (this.prototype.isLookup() ||
				this.prototype.isQuery() ||
				this.prototype.isRequest() ||
				this.prototype.isStatistics()) {
		...
		_filterSortUpdateUrl: function() {
			...
			if (!self._filterSortAllowed()) return utils.urlToString(sdataUrl);
			...
			if (this.prototype.isLookup() || this.prototype.isQuery()) {
				...
				info = self.getControlByBind("$resources");
				// chart bind is also $resources
				if (info && info.searchInfoGet) {
					info = info.searchInfoGet();
					if (info != null && info.length != 0) {
					...
					if (info.$type == null || info.$type.length !== 0 || info.$type === "application/json;vnd.sage=syracuse") {
						if (info.$url) {
							info.$parameters && $.each(info.$parameters, function(paramName, paramValue) {
								info.$url = info.$url.replace("{" + paramName + "}", encodeURIComponent(paramValue));
							});
							var urlParsed = self.prototype.resolveExpression(info.$url, self.dao);
