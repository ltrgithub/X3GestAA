...
var _templates = {
	...
	panels: '\
		...
		<section class="s-m-panel s-m-left"></section> \
		<section class="s-m-panel s-m-right"></section>'
};
...
/**
 * Page controls not children of rootLayout - header/footer and left/right panel
 */
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		this.options = $.extend({}, _defOpts, options);
		...
		notifications.subscribe(this, ["sm.action.link", "sm.select.row"], 1);
		...
		_initRootElmt: function($$parent) {
			...
			if (self.isVignette) {
				...
				self.$$dismissPanel = this.$$elmt.children(".s-m-side-panel-dismiss");
		...
		destroy: function() {
			...
			if (self._pageControls) {
				self._pageControls.destroy();
				self._pageControls = null;
			}
			if (self._waiting) {
		...
		loadStructure: function(updateLayout) {
			...
			try {
				...
				this.rootLayout = articleParser.article2Controls(this, this.article, this.prototype, {}, this.state.options.layoutPath);
				deferred.resolve();
			} catch (e) {
				deferred.reject(e);
			} finally {
		...
		render: function(updateLayout, buildOpts) {
			try {
				...
				if (this.$$ctrlsRootElmt) {
					...
					this.rootLayout.buildHtml(this.$$ctrlsRootElmt, this.dao, buildOpts);
				}
			} catch (e) {
				return $.smReject(e);
			}
		...
		buildHtmlOption: function() {
			return {
				...
				toolBars: {
					...
					statusPinPage: 1
					//					statusPinPage: settings.isPinnedPage(self._getPinPageId()) ? 2 : 1
				}
			};
		},
		/**
		 ...
		 * where	"left" or "right"
		 * id		predefined panel id
		 * 				pageactionpanel 	for action panel
		 * 				filtersortpanel		for query filter/sort
		 * 				userpanel			for left panel
		 */
		panelAttach: function(where, id, action) {
			if (!this.$$dismissPanel) return;
			if (where != "right" && where != "left") return;
			var panel = this.panelGet(id);
			if (!panel) return null;
			if (this._pageControls.get(where) && this._pageControls.get(where) != panel) {
				this.panelDetach(where);
			}
			if (!panel.isAttached(where)) {
				var $$parent = this.$$elmt.children("section.s-m-panel.s-m-" + where);
				if ($$parent.length != 1) return;
				// Add the control to the list
				this.controlsMap[panel.id] = panel;
				this._pageControls.add(where, panel).attach($$parent, where);
			}
			return this._panelToggle(where, panel, action);
		},
		_panelToggle: function(where, panel, action) {
			var self = this;
			if (!panel) panel = this._pageControls.get(where);
			if (!panel) return null;
			...
			if (panel.closeBox != true) {
				self.$$dismissPanel[visible ? "addClass" : "removeClass"](where);
				self.$$dismissPanel.css(where, newWidth);
			}
			if (panel.overlap != true) {
				["header", "footer"].forEach(function(id) {
					var c = self._pageControls.get(id);
					if (c) {
						c.$$parent.find(".navbar").css(where, newWidth);
					}
				});
			}
			panel.notifyToggled(where, action);
			return panel;
		},
		/**
		...
		panelDetach: function(where) {
			if (!this.$$dismissPanel) return;
			var p = this._pageControls.remove(where);
			if (!p) return;
			this._panelToggle(where, p, "close");
			// Not undefined because there are loops on controlsMap
			delete this.controlsMap[p.id];
			return p.detach();
		},
		/**
		 * return the panel and create it if not exists
		 * Responsability of the page - Ex: regular pages has is own pageactionpanel...
		 */
		panelGet: function(id, options) {
			return null;
		},
		/**
		...
		_actSidePanel: function(action, side) {
			...
			side.forEach(function(where) {
				self._panelToggle(where, null, action);
			});
		},

		_actPinPage: function() {
			var self = this;
			var pageData = self._getPinPageData(pageData); // allow to add information by subclasses
			settings.addPinnedPage(pageData).then(function() {
				// TODO: We could display a message here..
			}).fail(function(e) {
