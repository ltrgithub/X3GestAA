	...
	function Page($$parent, state, prototype, article, options) {
		...
		onMainPageResize: function(info, orientation, deviceType) {
			...
			if (!this.isVignette) {
				...
				if (self._panelsManager) {
					self._panelsManager.onMainPageResize(info, orientation, deviceType);
				}
			} else {
				/**
		...
		notifUpdtChartDetailArticle: function(notifData, controlId) {
			...
			chart.setChartDetailAuthoring(notifData.article, (this.state.level ? this.state.level + 1 : 1)); //because page current is the parent of page updated
		},
		/**
