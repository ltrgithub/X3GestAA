...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		notifications.subscribe(this, ["sm.action.link", "sm.select.row", "sm.panel.lookup.selection"], 1);
		...
		getControl: function(id) {
			var ctrl = Base.prototype.getControl.call(this, id);
			if (ctrl) return ctrl;
			if (this._pageControls) {
				var p = this._pageControls.get("left");
				if (p && p.getControl(id)) return p.getControl(id);
				p = this._pageControls.get("right");
				if (p && p.getControl(id)) return p.getControl(id);
			}
			return null;
		},
		...
		notifPanelLookupSelection: function(panelId, controlId, rowData) {
			if (!this._panelsManager) return;
			var panel = this._pageControls.get(panelId = "RIGHTPANEL" ? 'right' : 'left');
			if (!panel) return;
			panel.notifLookupSelection(controlId, rowData);
		}
	}
);
