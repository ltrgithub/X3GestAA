	...
	function Page($$parent, state, prototype, article, options) {
		...
		buildHtmlOption: function(loadOptions) {
			...
			var resultOpts = {
				...
				toolBars: {
					...
					showActions: true,
					showNativeShare: nativeShare.supportsShare()
				}
			};
			return settings.isPinnedPage(self.getPinPageId())
				.then(function(isPinned) {
					resultOpts.toolBars.statusPinPage = !self.isChild ? ((isPinned && !self.supportsLiveTile()) ? 2 : 1) : 0;
				})
				.then(function() {
					if (self.isChild) {
		...
		getNavigationPageData: function() {
			...
			var pageData = {
				...
				htmlRoot: globals.getGlobalCtx().baseLocation.htmlRoot,
				endPoint: globals.getEndpoint(),
				language: globals.getUserCtx().$lang,
			};
			return pageData;
		},
		/**
		...
		supportsLiveTile: function() {
			return this._nativeSupportsLiveTile && nativeLiveTile.supportsLiveTile();
		}
	}
);
