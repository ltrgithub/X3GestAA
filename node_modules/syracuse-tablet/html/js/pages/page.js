...
var _authActionProperties = ["options", "icon"];
var _authActionOptsProperties = ["footerHeader", "refreshPage", "multiselection"];
...
var _templates = {
	fullpage: '\
		<section class="s-m-page s-m-full {{type}} {{subtype}}" id="{{pageid}}" {{#if parentId}}data-parent-id="{{parentId}}"{{/if}} class="ctrl-event-click" style=""> \
			<header></header> \
			<section class="s-m-breadcrumbs"></section> \
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		notifications.subscribe(this, ["sm.select.row", "sm.updt.card.article", "sm.page.updt.scroll", "sm.vignette.loaded"], 1);
	}, Base, {
		resetMaps: function() {
		...
		_initRootElmt: function($$parent) {
			...
			if (self.isVignette) {
				...
				self.$$breadcrumbsElmt = self.$$elmt.children("section.s-m-breadcrumbs");
			...
			return $.smResolve().then(function() {
				...
				if (self._pageControls && loadOptions.skipPageControls !== true) {
					...
					if (self.options.header === true) {
						...
						self._pageHeader = self._pageControls.add("header", ctrlFactory.createPageControl("header", self));
					};
					if (self.options.footer === true) {
						// Save orginal footer beacuse we can change it
						self._pageFooter = self._pageControls.add("footer", ctrlFactory.createPageControl("footer", self));
					}
				}
		...
		authUpdateLayout: function(article) {
			...
			var updatePageControls = article.$pageActionSettings != null && this._pageControls != null;
			if (updatePageControls) {
				this._pageControls.destroyControls();
			}
			// Clear dom
			...
			var options = {
				updateLayout: true,
				skipPageControls: updatePageControls === false
			};
			return self.load(options).then(function() {
				// Add page controls to maps
				if (options.skipPageControls) {
		...
		/**
		 * a paged has been loaded in a child vignette
		 * #6401 - sm.vignette.loaded - Notified by vignette to adjust scroll
		 * In stacked/row layout the height of the page is given by the hieght of the content of the vignette
		 */
		notifVignetteLoaded: function(vignetteId, pageLoaded, error) {
			...
			// Init scroll because hight has changed
			this._initScroll();
		},
		/**
		 * Scroll the page
		...
		toggleHeaderFooter: function(typeControl, newControl) {
			var currentControl = this._pageControls.get(typeControl);
			if (newControl == null) {
				// Default control
