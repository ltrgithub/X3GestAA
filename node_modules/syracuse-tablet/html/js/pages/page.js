...
var _templates = {
	...
	panels: '\
		<div class="s-m-side-panel-dismiss" data-action="toggleSidePanel" data-params="close"></div> \
...
var PanelManager = function(page) {
	...
	this.panelAttach = function(where, panel, action) {
		...
		if (controller._pageControls.get(where) && controller._pageControls.get(where) != panel) {
		...
		if (!panel.isAttached(where)) {
			...
			controller.controlsMap[panel.id] = panel;
	...
	this._panelDetach = function(where) {
		...
		// Not undefined because there are loops on controlsMap
		delete controller.controlsMap[p.id];
		return p.detach();
	};
	/**
	 * Returns a panel - Just to have ctrlFactory declared here
	 * id	pageactionpanelregular	 	for action panel
	 * 		userpanelsmartphone			for left panel
	 */
	this.panelCreate = function(panelId, options) {
		// controller of the control is the current page because we could need to acced to this page from the panel (Ex for a panel attached to a vignette)
...
var PageControls = function(page) {
	...
	this.addToControlMap = function() {
		for (var id in this.map) {
			this.page.addControl(this.map[id], true);
		}
	};
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		notifications.subscribe(this, ["sm.select.row"], 1);
	}, Base, {
		_initMaps: function() {
			// Contains all the controls of the page including page controls
			this.controlsMap = {};
			...
			if (self.isVignette) {
				...
				if (self.options.header === true) {
					...
					self.$$contentElmt.css({
						top: globals.getSiteLayout().getHeight("header")
					});
				}
			}
		...
		load: function(updateLayout) {
			...
			try {
				self.buildHtmlOption()
					.then(function(opts) {
						self.buildOpts = opts;
					})
					.then(function() {
						return self.loadStructure(updateLayout);
					})
					.then(function() {
						if (updateLayout) {
							return $.smResolve();
				...
				/*setTimeout(function() {
					...
					for (var ii = 0, jj = 3; ii < jj; ii++) {
						$diagnoses.push({
							"$severity": "error",
							"$message": "X3 engine error: CST_ANFO : Variable Non-existent\n Sed tamen haec cum ita tutius observentur, quidam vigore artuum inminuto rogati ad nuptias ubi aurum dextris manibus",
							"$stackTrace": ii == 0 ? null : "X3 engine error: CST_ANFO : Variable Non-existent\n@SUPERV.TRT/AQMDEVICE_CSTD$adx(381) : If fmet this.ASETERROR(\"\",\"Operation Simulation-message Err X3=\"+num$(CODE),[V]CST_ANFO) : Endif\n@SUPERV.TRT/AQMDEVICE_CSTD$adx(20):THISINSTANCE\n@SUPERV.TRT/AQMDEVICE_CSTD$adx(5):METHODS\n@SUPERV.TRT/ASYRSUB$adx(361):MET_PRO_ACTION\n@SUPERV.TRT/ASYRSUB$adx(385):ACTION_MET\n@SUPERV.TRT/WMC0AQMDEVICE$adx(84):GETCOUNT\n@SUPERV.STC/C_AQMDEVICE$adx(1776):GETCOUNT\n@SUPERV.TRT/WMR0AQMCRUDM$adx(256):EXE_OPERATION\n@SUPERV.TRT/ASYROPE$adx(80):OPE\n@SUPERV.TRT/ASYROPE$adx(16):HANDLER_POST\n@SUPERV.TRT/ASYRMAIN$adx(448):MANAGE_REQUEST\n@SUPERV.TRT/ASYRMAIN$adx(146):(No Label)\n"
						});
					}
					modal.diagnoses("Diagnoses test", {
						$diagnoses: $diagnoses
					});
				}, 1000);*/
			} catch (e) {
		...
		/ * * * Create layout /
		control tree structure according to article * /;
		loadStructure: function(updateLayout) {
			...
			try {
				...
				if (this._pageControls && updateLayout !== true) {
					if (this.options.header === true) {
						this._pageControls.add("header", ctrlFactory.createPageControl("pageheader", this));
					};
					if (this.options.footer === true) {
						this._pageControls.add("footer", ctrlFactory.createPageControl("pagefooter", this));
					}
				}
				/ / Creates control / fields tree(from article)
		this.$$elmt.attr("data-layout-path", (this.state.options.layoutPath || 0) + "#page");
		this.buildControlsTree(this.state.options.layoutPath);
		deferred.resolve();
	} catch (e) {
		deferred.reject(e);
	} finally {
	...
	render: function(updateLayout, buildOpts) {
		try {
			...
			if (this._pageControls && updateLayout !== true) {
				this._pageControls.buildHtml(this.dao, buildOpts);
			}
			if (this.$$ctrlsRootElmt) {
				this.beforeAddContent();
				// Renders tree controls (article)
				Base.prototype.render.call(this, this.$$ctrlsRootElmt, buildOpts);
			}
		} catch (e) {
			return $.smReject(e);
		}
	...
	buildHtmlOption: function() {
		var self = this;
		var allowPin = true;
		var $typ = self.prototype && self.prototype.data("$typ");
		if ($typ == "rowdetail") {
			allowPin = false;
		}
			...
			.then(function(isPinned) {
				return {
					...
					toolBars: {
						...
						statusPinPage: allowPin ? (isPinned ? 2 : 1) : 0
					}
				};
			});
	},
	...
	onResize: function() {
		...
		if (this._panelsManager) {
			this._panelsManager.panelResize();
		}
	},
	/**
	 ...
	 *  isPageControl: 	true is it's a page control - Panel, Header, Footer...
	 **/
	...
	notifSelectRow: function(arrayId, rowId, rowdata, updateUI) {
		...
		this.lastSelectedRow = {
			array$Bind: this.getControl(arrayId).$bind,
			rowId: rowId
		};
		if (updateUI) {
	...
	makeVisible: function(c) {
		if (!c || !this._gestureMgr || !c.$$elmt) return;
		this._gestureMgr.makeVisible(c.$$elmt);
	},
	/**
	 * Adds a panel to left or right sections - sidePanel class
	 */
