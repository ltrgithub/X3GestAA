...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		savedCtxRestore: function(ctx) {
			...
			if (ctx.scroll && self._gestureMgr) {
		 ...
		 * 	$$container 	element into which the page is nested
		 * 				this elmt gives the fixed boundaries H/W
		 */
		enableScroll: function(enable, $$container) {
			if (!this.isNew) {
				// Cached page which has been activated
				if (this._gestureMgr) this._gestureMgr.refresh();
				return;
			}
			if (this.scrollAllowed() && enable === true) {
				// Enable _gestureMgr
				var viewRect = this.getScrollViewRect($$container);
				if (viewRect != null) {
				...
				if (this._gestureMgr) this._gestureMgr.reset();
			}
		},
		getScrollViewRect: function($$container) {
			// Enable _gestureMgr
			var viewRect;
			if (this.isVignette) {
				// Scroll should be avoided - just for test vignette which need scroll
				viewRect = uiRect.elmtRect($$container, "computed");
				// To allow scrolling
		...
		getScrollOffset: function() {
			...
			return this._gestureMgr.$$elmt.position();
			return this._gestureMgr.$$elmt.css(["top", "left"]);
			return {
				top: 0,
				left: 0
			};
