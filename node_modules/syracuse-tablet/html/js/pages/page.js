...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		this.nativeCapabilities = pageHelpers.getNativeCapabilities(this.state.type, this.state.subtype.split(" ")[0]);
		if (this.isChild) {
		...
		_initRootElmt: function($$parent) {
			...
			if (self.scrollAllowed()) {
				...
				self.$$scrollElmt = $('<div class="s-m-scroll-elmt"/>').appendTo(self.$$contentElmt);
				self.$$scrollElmt.css({
					height: "auto"
				});
				self.$$ctrlsRootElmt = self.$$scrollElmt;
		...
		isSamePage: function(pageName, pageOptions) {
			return !this.destroyed && this.isActive() && this.state.name === pageName;
		},
		...
		_initScroll: function(preserveScroll) {
			...
			if (this.scrollAllowed()) {
				...
				// Force $$scrollElmt to full heigth of childrens
