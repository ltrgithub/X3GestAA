...
var _nativeCapabilities = {
	liveTile: {
		enabled: native.hasCapability("liveTile"),
		regular: {
			query: true
		}
	},
	scheduleMsg: {
		enabled: native.hasCapability("scheduleMessage"),
		regular: {
			query: true,
			details: true,
			edit: false
	...
	share: {
		enabled: native.hasCapability("share"),
		regular: {
			query: true,
			details: true
		},
	...
	function Page($$parent, state, prototype, article, options) {
		...
		getNavigationPageData: function() {
			...
			var pageData = {
				...
				DeepLink: {
					Title: this.state.$title,
					Url: globals.getGlobalCtx().baseLocation.url + "?context=" + encodeURIComponent(JSON.stringify(query)),
				}
			};
			return pageData;
		},
