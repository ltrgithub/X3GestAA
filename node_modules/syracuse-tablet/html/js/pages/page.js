...
var _topPageControls = ["breadcrumbs", "topToolbar"];
...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		notifications.subscribe(this, ["sm.select.row", "sm.updt.card.article", "sm.updt.chart.article", "sm.scroller.update"], 1);
	}, Base, {
		resetMaps: function() {
						...
						if (self.options[id] === true) {
							...
							if (ctrl) {
								...
								if (_topPageControls.indexOf(id) >= 0) {
									...
									self["_$$" + id] = self.$$elmt.children("section.s-m-" + id).show();
								}
							}
						};
					});
		...
		reload: function(options) {
			...
			var updatePageControls = self.article.$pageActionSettings != null && self._pageControls != null;
			...
			return self.load(options).then(function() {
				...
				if (options.skipPageControls && self._pageControls) {
			...
			if (!this.isVignette) {
				...
				if (self._pageControls) {
					...
					self._pageControls.forAllCtrls(function(id, ctrl) {
						...
						if (ctrl.isVisible() && (_insidePageControls.indexOf(id) >= 0)) {
							var ch = ctrl.getHeight();
							contentHeight -= ch;
							if (id === "header") {
								top += ch;
							} else if (_topPageControls.indexOf(id) >= 0) {
								var $$root = self["_$$" + id];
								if ($$root) {
									$$root.css({
										top: top
		 ...
		 * In particular cases we need to restore the context before activating the page (when we create the control)
		 ...
		 * This._currentSavedContext property is set by the pageLoader just after having created the page
		 * In regular process savedContext is passed as a parameter of page.activate method
		 * Returns the context for 'bind' contrl if any
		 */
		saveCtxGetCtrlValue: function(bind) {
			if (!bind || !this._currentSavedContext || !this._currentSavedContext.ctxCtrls) return;
			return this._currentSavedContext.ctxCtrls[bind];
		},
		...
		notifUpdtChartArticle: function(notifData, controlId) {
		...
		isMultiSelectionEnabled: function() {
			var allowDownload = this.getAuthoring("$allowDownload") === true;
			return (this.getMultiSelectionLinks() != null || allowDownload);
		},
		...
		toggleHeaderFooter: function(typeControl, newControl, show) {
			...
			if (currentControl) {
				this._pageControls.remove(typeControl);
				if (currentControl.$$elmt) {
					currentControl.$$elmt.detach();
				}
				currentControl.notifToggled(false);
