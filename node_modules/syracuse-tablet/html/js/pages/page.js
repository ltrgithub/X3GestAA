...
var _Page = utils.defineClass(
	function($parent, state) {
		...
		buildHtml: function(data) {
			...
			if (self.vignette) {
				html.header = null;
				html.content = templates.execSync("contructionVignetteContentTmpl", {
					name: self.state.name,
					type: self.state.type
				});
			} else {
				html.header = templates.execSync("contructionHeadTmpl", {
					title: "Sage X3 Mobile",
					pageId: self.id,
					logout: "logout"
				});
				html.content = templates.execSync("contructionContentTmpl", {
					name: self.state.name,
					type: self.state.type
				});
			}
		...
		getPageRepresentation: function($uuid) {
			var gadgetUuid = window.$sm.$homeDashboard.$vignettes[$uuid].$uuid;
			var pageNodeName;
			if (window.$sm.$gadgets[gadgetUuid].$type == "$representation") {
				pageNodeName = window.$sm.$gadgets[gadgetUuid].representation + "." + window.$sm.$gadgets[gadgetUuid].facet;
			} else {
				//TODO
				throw new Error("The process for this type of gadget is not implemented yet !");
			}
			return window.$sm.$pages[pageNodeName].$page;
		},
...
var _pageInfo = function(state, options) {
	...
	if (typeof state === "string") {
		...
		if (!state.transition) {
			// Default transition
			state.transition = globals.application().$config("transition");
		}
		// Readable id
