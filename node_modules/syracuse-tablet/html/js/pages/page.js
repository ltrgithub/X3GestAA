...
var _Page = utils.defineClass(
	function($parent, state, prototype, article) {
		...
		_createPageHeader: function() {
			var self = this;
			return factory.createPageHeader(self, self.article, self.prototype);
		},
		...
		/**
		 * TODO - Optimization - resolved deferred eventually
		 */
		buildHtml: function() {
			var deferred = $.Deferred();
			var self = this;
		...
		buildHtmlContent: function() {
			...
			try {
				var contentHtml = '';
				var htmlBefore = self.buildHtmlContentBefore() && self.buildHtmlContentBefore().outerHTML;
				contentHtml += (htmlBefore ? htmlBefore : "") + self.rootLayout.buildHtml(self.dao).outerHTML;
				/*
				var contentHtml = [];
				self.buildHtmlContentBefore(contentHtml);
				self.rootLayout.buildHtml(contentHtml, self.dao));
				self.buildHtmlContentAfter(contentHtml);*/
				deferred.resolve(contentHtml);
			} catch (e) {
				deferred.reject(e);
			} finally {
				return deferred.promise();
		 ...
		 * For child classes
		 */
		buildHtmlContentBefore: function() {
			return uiutils.createDomElement("h3", ["s-m-page-title"], globals.getApplication().getTitle());
		},
		/**
		 * TODO - Optimization - revoved deferred eventually
		 */
		...
		afterRender: function() {
			...
			try {
				...
				// I noticed that if we throw an Error in the for loop it does	n't reject correctly - Error dialog not displayed
				// TODO - TEST
				log && log("afterRender error", e);
				deferred.reject(e);
			} finally {
				return deferred.promise();
			}
