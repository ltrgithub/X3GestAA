	...
	function($parent, state, prototype, article) {
		...
		_createHtmlRoot: function() {
			...
			if (self.vignette) {
				...
				var header = uiutils.createDomElement("header", ["s-m-header"]);
				var content = uiutils.createDomElement("section", ["s-m-content"]);
				var footer = uiutils.createDomElement("footer", ["s-m-footer"]);
		...
		render: function() {
			...
			// Add page header to DOM
			self.header = ctrlFactory.createPageHeader(self, self.article, self.prototype);
			var hc = self.$elmt.children("header");
			hc = hc[0];
			self.header.buildHtml(hc);

			// Add page content to DOM
			var cc = self.$elmt.is("section") ? self.$elmt : self.$elmt.children("section");
			cc = cc[0];
			self.rootLayout.buildHtml(cc, self.dao, null);
