...
var _pageSettingPanelId = globals.PANELPAGESETTINGS;
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		this.nativeCapabilities = _getNativeCapabilities(this.state.type, this.state.subtype);
		if (this.isChild) {
		...
		this.nbCharts = 0;
		this._pageSettingsPanel = this.panelCreate(_pageSettingPanelId);
		...
		panelAttachPageSettings: function(ctx, opt) {
			this.pageSettings = {
				ctx: ctx,
				opt: opt
			};
			this.panelAttach("right", this._pageSettingsPanel, "toggle");
		},
		/**
		...
		getPinPageData: function() {
			return {
				...
				title: this.prototype.data("$title"),
		...
		getRestoreContext: function() {
			...
			if (self.state.options) {
				var opts = ctx.currentState.options = {};
				["sdata-url", "dataParams"].forEach(function(p) {
					opts[p] = self.state.options[p];
				});
			}
			return ctx;
		},
		getTileLinkContext: function() {
			var ctx = this.getRestoreContext();
			return {
				Title: this.state.$title,
				Description: "",
				DeepLink: {
					Title: this.state.$title,
					Url: globals.getGlobalCtx().baseLocation.url + "?context=" + encodeURIComponent(JSON.stringify(ctx)),
				}
			};
		},
		/**
