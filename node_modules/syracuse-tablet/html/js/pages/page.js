...
var _Page = utils.defineClass(
	function Page($$parent, state, prototype, article, options) {
		...
		/**
		 * Change the current footer with newFooter
		 * Return the ol footer
		 * newFooter =  null -> Restores the original footer
		 */
		toggleFooter: function(newFooter) {
			if (newFooter == null) {
				// Restore original footer
				newFooter = this._pageFooter;
			}
			if (!newFooter) return;
			var $$root = this.$$elmt.children("footer");
			if (this._pageControls.get("footer") === newFooter) {
				return;
			}
			var oldFooter = this._pageControls.remove("footer");
			if (oldFooter) {
				if (oldFooter.$$elmt) {
					oldFooter.$$elmt.detach();
				}
				oldFooter.notifToggled(false);
				this.removeControl(oldFooter);
			}
			this._pageControls.add("footer", newFooter);
			if (!newFooter.$$elmt) {
				newFooter.buildHtml($$root, this.dao);
			} else {
				$$root.append(newFooter.$$elmt);
				// JQuery work around for a tags - the JQuery.defaultDisplay function returns 'inline' for a and we need 'inline-block'
				// if we attach a footer and reattach it, call show/hide restore the defaultDisplay instead the orginal one 
				// if we do'nt detach the footer it works ok so we force inline-block
				newFooter.$$elmt.find("a").css("display", "inline-block");
			}
			this.addControl(newFooter);
			newFooter.notifToggled(true);
			return oldFooter;
		}
	}
);
