...
var nativeScheduleMsg = require('syracuse-tablet/html/js/helpers/native/nativeScheduleMsg');
var nativeMessages = require('syracuse-tablet/html/js/helpers/native/nativeMessages');
...
var _nativeCapabilities = {
	liveTile: {
		enabled: nativeLiveTile.supportsLiveTile(),
		regular: {
			query: true
		}
	},
	scheduleMsg: {
		enabled: nativeScheduleMsg.supportsScheduleMsg(),
		regular: {
			query: true,
			details: true,
			edit: false
	...
	share: {
		enabled: nativeShare.supportsShare(),
		regular: {
			query: true,
			details: true
		},
	...
	function Page($$parent, state, prototype, article, options) {
		...
		buildHtmlOption: function(loadOptions) {
			...
			var resultOpts = {
				...
				toolBars: {
					...
					showPageSettings: self._showPageSettings()
				}
			};
			return settings.isPinnedPage(self.getPinPageId())
				.then(function(isPinned) {
					resultOpts.toolBars.statusPinPage = !self.isChild ? ((isPinned && !self.nativeCapabilities.liveTile) ? 2 : 1) : 0;
				})
				.then(function() {
					if (self.isChild) {
		...
		activate: function(pageToClose, cb) {
			...
			self._show(pageToClose, function() {
				if (self._getSavedContext()) {
					self.savedCtxRestore(self._getSavedContext());
		 ...
		 */
		_getSavedContext: function(propName) {
		...
		getCtrlSavedContext: function(bind) {
			var ctx = this._getSavedContext('ctxCtrls');
		...
		getNavigationPageData: function() {
			var query = {
				endpoint: globals.getEndpoint(),
				applicationName: globals.getApplication().uuid,
			...
			var pageData = {
				...
				DeepLink: {
					Title: this.state.$title,
					Url: globals.getGlobalCtx().baseLocation.url + "?context=" + encodeURI(JSON.stringify(query)),
				}
			};
			return pageData;
		},
		...
		_setNativeCapabilities: function() {
			var self = this;
			this.nativeCapabilities = {};
			Object.keys(_nativeCapabilities).forEach(function(capability) {
				self.nativeCapabilities[capability] = (_nativeCapabilities[capability].enabled && _nativeCapabilities[capability][self.state.type] && _nativeCapabilities[capability][self.state.type][self.state.subtype]) ? true : false;
			});
		},
