...
var PanelManager = function(page) {
	...
	this.panelAttach = function(where, panel, action) {
		if (!_controller.$$dismissPanel || !panel || (where != "right" && where != "left")) return;
		if (_controller._pageControls.get(where) && _controller._pageControls.get(where) != panel) {
			this._panelDetach(where);
		}
		if (!panel.isAttached(where)) {
			var $$parent = _controller.$$elmt.children("section.s-m-panel.s-m-" + where);
			if ($$parent.length != 1) return;
			// Add the control to the list
			_controller.controlsMap[panel.id] = panel;
			_controller._pageControls.add(where, panel).attach($$parent, where);
		}
	...
	this.panelToggle = function(where, panel, action) {
		if (!panel) panel = _controller._pageControls.get(where);
		...
		if (panel.closeBox != true) {
			_controller.$$dismissPanel[visible ? "addClass" : "removeClass"](where);
			_controller.$$dismissPanel.css(where, newWidth);
		}
		if (panel.overlap != true) {
			var shiftElmts = [_controller.$$contentElmt];
			["header", "footer"].forEach(function(id) {
				var c = _controller._pageControls.get(id);
				if (c) {
	...
	this._panelDetach = function(where) {
		if (!_controller.$$dismissPanel) return;
		var p = _controller._pageControls.remove(where);
		if (!p) return;
		this.panelToggle(where, p, "close");
		// Not undefined because there are loops on controlsMap
		delete _controller.controlsMap[p.id];
		return p.detach();
	};
	/**
	...
	this.panelCreate = function(panelId, options) {
		return ctrlFactory.createPanel(_controller, panelId, options);
	};
};
...
var PageControls = function(page) {
	...
	this.buildHtml = function(dao, buildOptions) {
		for (var id in this.statics) {
			this.statics[id].buildHtml(_controller.$$elmt.children(id), dao, buildOptions);
		}
	};
	...
	this.addToControlMap = function() {
		for (var id in this.map) {
			_controller.addControl(this.map[id], true);
		}
	};
	...
	function Page($$parent, state, prototype, article, options) {
		...
		_initRootElmt: function($$parent) {
			...
			if (self.isVignette) {
				...
				// Panels are managed by main page
				self._panelsManager = globals.getCurrentPage()._panelsManager;
			} else {
				self.$$elmt = $(_getHtml("fullpage", ctx));
				self.$$contentElmt = self.$$elmt.children("section.s-m-main-content");
				// Panel manager only for main page
