			...
			<section id="{{id}}" data-action="selectRecord" class="list-group-item s-m-draft-item {{status}}">\
				<div>\
					<div>\
						...
						<div class="draft-date"><b><small>{{creation_date}}</small></b></div>\
					</div>\
					<div>\
					...
					<div>\
						...
						<div class="draft-upload">\
							<span data-action="upload" class="s-m-link fa fa-upload"></span>\
						</div>\
					</div>\
				</div>\
...
var _Page = utils.defineClass(
	function DraftsPage($parent, state, options) {
		...
		_updateSelectMenu: function() {
			...
			this._$$selectMenu.find('a[data-action]').toggle(selected === true);
			...
			this._$$selectMenu.find('a[data-params="hideunselected"]').toggle(hiddenUnselRecords == 0);
			this._$$selectMenu.find('a[data-params="showunselected"]').toggle(hiddenUnselRecords > 0);
			this._$$selectMenu.find('.badge').text(selectedRecords);
		},
		_selectRecords: function(filter) {
			var selector = "";
			if (filter != "all") {
				selector += filter;
			}
		...
		_actSelectMenu: function(action) {
			...
			switch (action) {
				case "selectall":
				case "unselectall":
					self._selectRecords(action === "selectall" ? "all" : ".selected").each(function() {
						self._toggleRecordSelection($(this));
					});
					break;
				case "hideunselected":
		...
		_actSelectRecord: function(params, $$elmt) {
			var $$rec = $$elmt.closest(".s-m-draft-item");
			this._toggleRecordSelection($$elmt.closest(".s-m-draft-item"));
			this._updateSelectMenu();
		},
		_toggleRecordSelection: function($$rec) {
			if (!$$rec || $$rec.length == 0) return;
			$$rec.toggleClass("selected");
			$$rec.find('[data-action="upload"]').toggleClass("disabled", $$rec.is(".selected"));
		},
		_startOperation: function(name, ids) {
		...
		_upload: function(ids) {
			var self = this;
			self._startOperation("upload", ids);
			var join = $.Deferred();
			self._uploadPop(ids).then(function(status) {
				self._stopOperation("upload");
				join.resolve(status);
			}).fail(function(e) {
				self._stopOperation("upload");
				join.reject(e);
