...
var _Page = utils.defineClass(
	function DraftsPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			self._buildList().then(function(ok) {
				if (ok !== true) {
					$$parent.html("<b>" + locale.text("error") + "</b><br><br><b>" + locale.text("contact.admin") + "</b>");
					return;
				}
				...
				self._gestureMgr = new HammerScroller(self._$$list, {
					isPageScroller: true,
					direction: "v"
				});
			});
		...
		_buildList: function() {
			...
			globals.getStorage().draftOperation("draftReadList").then(function(resources) {
				...
				globals.getModal().error("Error loading draft list", e, function() {
		...
		onMainPageResize: function(info, orientation, deviceClass) {
			...
			setTimeout(function() {
				if (!self._gestureMgr) return;
				var mainRect = uiRect.elmtRect(self.$$contentElmt, "outer");
				var viewRect = uiRect.elmtRect(self.$$myScrollWrapper, "outer");
				var scrollRect = mainRect.intersectRect(viewRect);
				if (scrollRect.height < self._gestureMgr.$$elmt.height()) {
		...
		_uploadAll: function() {
			...
			self._upload(self._selectRecords(".selected").map(function() {
				...
				globals.getModal().notify({
					title: "Drafts synchronization " + (status === "stopped" ? "stopped" : "completed"),
					body: "Processed: " + count + " - Errors: " + nbErrors + " - Success: " + nbSuccess,
		...
		_removeAll: function() {
			...
			globals.getStorage().draftOperation("draftDeleteAll").then(function() {
				...
				globals.getModal().error("Error removing drafts", e);
			});
		},
