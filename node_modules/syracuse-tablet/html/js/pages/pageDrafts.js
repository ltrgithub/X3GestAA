					...
					<div style="display:table-row;width:100%">\
						<div style="display:table-cell">\
							...
							<div style="clear: both;">\
								<b>{{endpoint}} - {{representation}}</b>\
							</div>\
							<div>\
								<span style="float:left">{{reason}}</span>\
								<span style="float:right">{{comment}}</span>\
							</div>\
						</div>\
						<div style="clear:both;display:table-cell;width:5em;font-size:1.5em;position:relative;padding-left: 15px;">\
							<div style="display:table;width:100%;position:absolute;top:-6px;">\
								<div style="display:table-cell;width:33%;text-allign:center;">\
									<div data-action="delete" class="s-m-link fa fa-times"></div>\
								</div>\
								<div style="display:table-cell;width:33%;text-allign:center;">\
									<div data-action="editDraft" class="s-m-link fa fa-pencil-square-o"></div>\
								</div>\
								<div style="display:table-cell;width:33%;text-allign:center;">\
									<div data-action="upload" class="s-m-link fa fa-upload"></div>\
								</div>\
							</div>\
						</div>\
					</div>\
	...
	content: '\
			<div class="s-m-drafts-header" style="display:table;width: 100%;margin-bottom:20px;margin-top:20px;">\
				<h3 style="display:table-cell">\
					<span style="padding-right: 5px;">{{table-title}}</span>\
					<span class="badge" style="top:-3px;position: relative;">{{count}}</span>\
				</h3>\
			...
			{{#if isEmpty}}\
				<div>EMPTY LIST</div>\
			{{else}}\
				<div class="list-group s-m-drafts-list" style="position:relative">\
					{{{recordsHtml}}}\
					<div id="spinner" style="margin:5px;position:absolute;width:100%;display:none;opacity:.5;z-index:1000;background-color:aliceblue;">\
	...
	function DraftsPage($parent, state, options) {
		...
		_buildList: function(justRefred) {
			...
			step.then(function() {
				...
				self._$$spinner = self.$$htmlRoot.find("#spinner");
			}).fail(function(e) {
			...
			self._uploadIdBegin(id).then(function(ctx) {
				...
				return dispatcher.dispatch({
					...
					if (!result.isSuccess) {
						...
						next = self._uploadIdEnd(id, "saveFailed", msg, {
							draftCtx: draftCtx
						});
					} else {
						// OK
						next = self._uploadIdEnd(id, "saveSucceeded");
					}
					return next;
				});
			}).then(function() {
		...
		_uploadIdEnd: function(id, status, message, opts) {
			...
			if (status === "saveSucceeded") {
				return globals.getStorage().draftOperation("draftRead", id).then(function() {
		...
		_uploadAll: function() {
			...
			self._upload(this.$$elmt.find(".s-m-draft-item").map(function() {
				...
				var count = self._logData.count;
				var nbFails = count - nbSuccess;
				if (count === nbSuccess) {
					globals.getModal().info("Uploading draft succeeds - " + nbSuccess + "/" + count);
				} else {
					globals.getModal().warning("Uploading uploading failed - Failed:" + nbFails + "/" + count + " - Success:" + nbSuccess + "/" + count);
				}
