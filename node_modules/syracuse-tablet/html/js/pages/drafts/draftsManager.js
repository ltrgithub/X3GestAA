 ...
 */
exports.DraftsManager = function DraftsManager(page) {
	this._page = page;
	...
	this.onBackAction = function() {
		if (this._offlineDraftId) {
			//  Ask user to remove offline draft which has been created automatically on connection error
			return this.offlineDraftAskRemove();
		}
		if (!this._page.pageData.dataset.isDirty()) {
	...
	this.askSaveDraft = function() {
		...
		return modal.show().then(function(result) {
			if (result === "drafts.dlog.savedraft.button") {
				return this.save(false, true).then(function(saved, draftCtx) {
	...
	this.save = function(offline, confirm) {
		...
		step.then(function(result) {
			...
			modules.get("modal").error(locale.text("drafts.error.saving"), e, function() {
	...
	this.notifSaveSucceeded = function(result) {
		...
		if (result && result.responseJSON && result.responseJSON.$uuid) {
			return modules.get("storage").getStorage().draftOperation("draftDelete", result.responseJSON.$uuid);
		} else {
			return $.smResolve();
		}
	};
	...
	this.notifSaveFailed = function(result) {
		...
		if (responseJSON.$noConnectionError === true) {
			...
			var self = this;
			this.save(true, this._offlineDraftId == null && !this._pageLoadedFromDraft()).then(function(saved, draftCtx) {
				if (saved) {
		...
		this._offlineDraftId = null;
		// No need to wait the promise
		this.offlineDraftRemove();
		this._draftStatus = "error";
		this._errMsg = responseJSON && responseJSON.$diagnoses && responseJSON.$diagnoses.length > 0 ? responseJSON.$diagnoses[0].$message : 0;
		return true;
	};
	/**
	...
	this.offlineDraftRemove = function() {
		...
		if (!this._offlineDraftId) {
			...
			var self = this;
			modules.get("storage").getStorage().draftOperation("draftDelete", this._offlineDraftId).always(function() {
				self._offlineDraftId = null;
				deferred.resolve();
			});
		}
