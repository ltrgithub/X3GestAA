...
exports.Page = utils.defineClass(
	...
	Base, {
		...
		onGotoUrlAction: function(isBack, parsedUrl) {
			...
			if (parsedUrl && (parsedUrl.protocol === "child" || (jsutils.getPropByPath(parsedUrl, "query.representation") || "").indexOf(".$lookup") > 0)) {
				return $.smResolve(true);
			}
			// Save new errors if draft has been saved
			if (!this._isDirty() || this._draftStatus == "success") {
				return $.smResolve(true);
			}
			if (this._draftId) {
		...
		_askSaveDraft: function() {
			...
			["saveAsDraft", "continue", "stayOnCurrentPage"].forEach(function(act) {
				opts.buttons.push({
					action: act,
		...
		_deleteDraft: function(actionName, success, beforeContext) {
			...
			this._draftStatus = "success";
			return modules.get("storage").getStorage().draftOperation("draftDelete", this._draftId);
		},
		/**
