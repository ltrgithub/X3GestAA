...
var _getTemplate = function(state, ctxHeader, ctxContent) {
	...
	try {
		...
		$.when(templates.exec(root + _consts.tmplMain, ctxContent), templates.exec(root + _consts.tmplHead, ctxHeader, {
			failNotFound: false
...
var _getData = function(state) {
	var deferred = $.Deferred();
	var url = utils.baseLocation.htmlRoot + state.name + _consts.dataFile;
	if (state.detailid) {
