...
var _Page = utils.defineClass(
	...
	function CustomizedDashboard($parent, state, prototype, article, options) {
		...
		_actChangeGroupTile: function(vignetteId) {
			...
			var groups = self.rootLayout.children.filter(function(group) {
		...
		_afterChangePage: function(opts) {
			this._toggleConfigPage();
			opts = opts || {}
			_initConfigPage = true; // Force auto start of page config on reload
			var gesture = this.getGestureMgr();
			_scroll = opts.scroll == "end" ? 99999 * (gesture.isNative() ? 1 : -1) : (opts.keepScrollValue ? _scroll : this.getGestureMgr().getScroll());
		},
		...
		_dragUI: function(ev) {
			if ($(ev.currentTarget).hasClass("s-m-move-group")) {
		...
		enabledConfig: function(options) {
			...
			for (var i = 0; i < $$tiles.length; i++) {
				...
				$$tile.append($('<div class="insert top" data-param="top" data-vignette-id="' + $$vignette.attr('id') + '"></div>'));
				$$tile.append($('<div class="insert bottom" data-param="bottom" data-vignette-id="' + $$vignette.attr('id') + '"></div>'));
				$$tile.append($('<div class="insert left" data-param="left" data-vignette-id="' + $$vignette.attr('id') + '"></div>'));
				$$tile.append($('<div class="insert right" data-param="right" data-vignette-id="' + $$vignette.attr('id') + '"></div>'));
				$$tile.append($('<btn class="cfg changegroup fa fa-truck" data-action="changeGroupTile" data-params="' + $$vignette.attr('id') + '">'));
			...
			$$hubGroups.each(function() {
				...
				if (layout.isEditable) {
					//layout.$$header.prepend($('<btn class="cfg move fa fa-ellipsis-v s-m-move-group" id="' + $(this).attr("id") + '">'));
					if (!$(this).is(":first-child")) layout.$$header.prepend($('<btn class="cfg move fa fa-arrow-left s-m-move-group" data-action="moveGroup" data-params="left,' + $(this).attr("id") + '">'));
					layout.$$header.append($('<btn class="cfg remove fa fa-pencil" data-action="editGroup" data-params="' + $(this).attr("id") + '">'));
					if (!$(this).is(":last-child")) layout.$$header.append($('<btn class="cfg move fa fa-arrow-right s-m-move-group" data-action="moveGroup" data-params="right,' + $(this).attr("id") + '">'));
				}
		...
		notifVignettePressed: function(vignetteId) {
			var tileInfo = this._getTileInfo(vignetteId);
			tileInfo.$$tile.toggleClass("s-m-draggable", true);
			notifications.publish("sm.drag.state.change", true);
		},
		...
		_dragManagerCreate: function() {
			var dragOpts = {
				selector: ".s-m-hub-group>header>.s-m-move-group,.s-m-tile",
				enable: false,
				notify: {
					start: "sm.drag.start",
					stop: "sm.drag.stop",
			...
			this.dragManager = new dragManager(dragOpts, dropOpts, self.$$elmt);
			notifications.subscribe(this, this.dragManager.getNotifs());
		},
		disableConfig: function() {
			...
			$(".s-m-tile div.insert", self.$elmt).remove();
			$(".s-m-tile", self.$elmt).removeAttr("data-disable-navigation");
			$("[data-action='editGroup']").remove();
			$("[data-action='removeGroup']").remove();
			notifications.unsubscribe(self, ["sm.dashboard.title.change"]);
		},
		notifDragStart: function($$dragElt, ui) {
			if ($$dragElt.is(".s-m-hub-group>header>.s-m-move-group")) {
		...
		notifDragStop: function($$dragElt, ui) {
			notifications.publish("sm.drag.state.change", false)
		...
		notifElementDrop: function($$dropElt, ui, params) {
			...
			if ($$dragElt.is(".s-m-hub-group > header > .s-m-move-group")) this._groupDrop($$dropElt, $$dragElt, params)
		},
		...
		_actMoveGroup: function(where, id) {
			...
			var dep = parseInt(this._prevPageInfo.orientation === "landscape" ? layoutTarget.$$elmt.width() : layoutTarget.$$elmt.height(), 10);
			dep = dep * (this.getGestureMgr().isNative() ? 1 : -1) * ((where === "right") ? 1 : -1);
			_scroll += dep
				/*====================================*/
		...
		notifElementOver: function($$dropElt, ui) {
			...
			if ($$dragElt.is(".s-m-hub-group>header > .s-m-move-group")) this._groupOver($$dropElt, $$dragElt)
		},
