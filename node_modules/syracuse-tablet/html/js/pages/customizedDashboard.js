...
var _Page = utils.defineClass(
	...
	function CustomizedDashboard($parent, state, prototype, article, options) {
		...
		_titleChange: function(event, opt) {
			var self = this;
			var uuid = this._setLocalization(opt.title.val(), opt.title.attr("data-old-uuid")).
			then(function(uuid) {
				var titleContent = uuid ? "{@" + uuid + "}" : opt.title.val()
				self._changeHubGroupTitle(opt.layout, titleContent);
			});
		},
		_changeHubGroupTitle: function(layout, titleContent) {
			...
			return globals.getMetaData().getDashboard(self.prototype.getDataByPath("$dashboardName")).then(function(dashboard) {
				...
				if (idx >= 0) {
					self.article.$items[idx].$title = titleContent;
