...
var nativePenInput = require('syracuse-tablet/html/js/helpers/native/nativePenInput');

var _pageTemplate = '\
		{{#if nativeApp}} \
			<button class="btn btn-primary s-m-login-back fa fa-arrow-circle-o-left" data-action="selectServer" type="button"> \
			</button> \
			<button class="btn btn-primary s-m-login-back fa fa-pencil" data-action="penInput" type="button"> \
			</button> \
		{{/if}} \
	...
	function LoginPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			var _pageContext = {
				...
				"nativeApp": nativeApp.isSageX3WUPApp()
			};
		...
		_actSelectServer: function() {
			nativeApp.configServer();
		},
		_actPenInput: function() {
			if (nativePenInput.supportsPenInput()) {
				nativePenInput.getPenTextInput({
					title: "some field"
				})
