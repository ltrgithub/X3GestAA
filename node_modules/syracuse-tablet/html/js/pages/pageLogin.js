...
var _pageTemplate = '\
		<div class="s-m-scroll-wrapper">\
			<div class="s-m-loginform" style="display:none;position: relative;">\
				...
				<div id="s-m-login-withcookie" class="row" style="display: none;">\
					<div class="col-xs-offset-1 col-xs-10 col-sm-offset-3 col-sm-6 col-md-offset-4 col-md-4 col-lg-offset-4 col-lg-4">\
						<div class="panel panel-default">\
							<div class="panel-body s-m-login-body">\
								<button  data-action="forgetme" type="button" class="btn btn-primary s-m-signin-btn">Forget me on this computer</button>\
							</div>\
							<div class="panel-body s-m-login-body" id="s-m-login-changeuser">\
								<button  data-action="changeuser" type="button" class="btn btn-primary s-m-signin-btn">Login with a different user</button>\
							</div>\
						</div>\
					</div>\
				</div>\
...
var _Page = utils.defineClass(
	...
	function LoginPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			self._updateStatus(msg, !globals.isOnline());
		...
		_checkMessage: function(e) {
			...
			if (e && e.$redirectPage === "login") {
				self.message = e.message || e.$message;
				if (!self.message) {
			...
			var $$fail = $("#login-fail");
			if (self.message) {
				self._displayText($$fail, locale.text("login.failed") + "<p>" + self.message + "</p>" + (self.stack ? self.stack : ""));
			} else {
				$$fail.empty().hide();
			}
		},
		_updateStatus: function(e, forceOffLine) {
			...
			if (self.isOffLine()) {
				...
				step = auth.reconnectGetToken().then(function(token) {
					return $.smResolve(token);
				});
			};
			step.then(function(loginCookie) {
				...
				} else if (false && loginCookie) {
					// FDB temporaryly disabled
					$("#s-m-login-offline").hide();
					$("#s-m-login-online").hide();
					$("#s-m-login-nooffline").hide();
					$("#s-m-login-withcookie").show();
					var un = globals.getUserName();
					$("#login-reason").text(un ? "You are still connected as '" + un + "'" : "");
				} else {
