					...
					<div class="panel panel-default">\
						...
						<div  id="s-m-login-withoutcookie">\
							...
							<div class="panel-body s-m-login-body" id="s-m-external-body">\
								{{#each externals}} \
									<button style="margin-bottom:10px;" data-action="externalSignin" data-params="{{this.href}}" type="button" class="btn btn-primary s-m-signin-btn">{{this.title}}</button>\
								{{/each}} \
							</div>\
							<div id="s-m-login-noauth">\
								<div class="form-group">\
									<center>\
										{{no_auth_method}} \
										<br> \
...
var _Page = utils.defineClass(
	...
	function LoginPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			var externals = [];
			if (auth.isAuthenticationAvail("saml2")) {
				Array.prototype.push.apply(externals, auth.getAuthData("$saml2"));
			}
			if (auth.isAuthenticationAvail("oauth2")) {
				Array.prototype.push.apply(externals, auth.getAuthData("$oauth2"));
			}
			...
			// Sage-id button
			var sageId = auth.isAuthenticationAvail("sage-id");
			var basic = auth.isAuthenticationAvail("basic");
			$$parent.find("#s-m-login-online").find("#s-m-sageid-header, #s-m-sageid-body").toggle(sageId);
			$$parent.find("#s-m-login-online").find("#s-m-login-basic-fields-id").toggle(basic);
			$$parent.find("#s-m-login-online").find("#s-m-login-noauth").toggle(!sageId && !basic && !(externals.length > 0));
			$$parent.find("#s-m-login-online").find("#s-m-external-header, #s-m-external-body").toggle(externals.length > 0);
		...
		_updateStatus: function(e, forceOffLine) {
			...
			if (self.isOffLine()) {
				...
				step = auth.reconnectGetToken().then(function(loginCookie, loginCookieUser, sidCookie, loginCookieName, sidCookieName) {
					return $.smResolve(loginCookie, loginCookieUser, sidCookie);
				});
			};
			step.then(function(loginCookie, loginCookieUser, sidCookie) {
		...
		_actForgetme: function() {
			...
			var self = this;
			if (userProfile) {
				initContext.removeLastUsedContext(userProfile.user.$value);
			}
			// logout to make sure that forgeMe logout the user - forgetme removes only the security token 
			auth.logout().then(function() {
				return auth.forgetme();
			}).always(function() {
				// Refresh UI
				self.reload();
			});
		...
		_actExternalSignin: function(href) {
