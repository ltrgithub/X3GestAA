...
var _Page = utils.defineClass(
	...
	function LoginPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			$$parent.find("#s-m-login-online").find("#s-m-login-noauth").toggle(!sageId && !basic);
		...
		_updateStatus: function(e, forceOffLine) {
			...
			if (self.isOffLine()) {
				...
				step = auth.reconnectGetToken().then(function(loginCookie, loginCookieUser, sidCookie, loginCookieName, sidCookieName) {
					return $.smResolve(loginCookie, loginCookieUser, sidCookie, loginCookieName, sidCookieName);
				});
			};
			step.then(function(loginCookie, loginCookieUser, sidCookie, loginCookieName, sidCookieName) {
