					...
					<div class="col-xs-offset-1 col-xs-10 col-sm-offset-3 col-sm-6 col-md-offset-4 col-md-4 col-lg-offset-4 col-lg-4">\
						<div class="panel panel-default">\
							...
							<div class="panel-body s-m-login-body">\
								...
								<br>\
								<button id="login-login" data-action="login" type="submit" class="btn btn-primary s-m-signin-btn">{{act_login}}</button>\
							</div>\
	...
	function LoginPage($parent, state, options) {
		...
		_actLogin: function() {
			...
			return settings.loginRememberMe().then(function(rememberMe) {
				...
				if (!loggedIn) {
					// Not expected
					self._updateStatus(locale.text("login.issue"));
				}
		...
		_actOfflinesignin: function() {
			var self = this;
			self.$$elmt.find("#login-reason").hide();
			initLogin.notifLoggedIn(self._offLineProfile, true).then(function(loggedIn) {
				if (!loggedIn) {
					// Not expected
					self._updateStatus(locale.text("login.issue"));
				}
