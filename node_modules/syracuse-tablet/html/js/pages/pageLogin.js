...
var _Page = utils.defineClass(
	...
	function LoginPage($parent, state, options) {
		...
		appendHtml: function($$parent, loadOpts) {
			...
			if (self.state.options.reason) {
				if (self.state.options.isError === true) {
					this.message = self.state.options.reason;
				} else {
					this._displayText($$parent.find("#login-reason"), self.state.options.reason);
				}
			}
			if (!globals.isOnline()) {
		...
		_updateStatus: function() {
			...
			var $$fail = $("#login-fail");
			if (self.message) {
				this._displayText($$fail, locale.text("login.failed") + "<p>" + self.message + "</p>" + (self.stack ? self.stack : ""));
			} else {
				$$fail.empty().hide();
			}
		...
		_actOfflinesignin: function() {
			var self = this;
			self.$$elmt.find("#login-reason").hide();
			initLogin.notifLoggedIn(self.profile, true).then(function(loggedIn) {
				if (!loggedIn) {
					// Not expected
					self._checkMessage(locale.text("login.issue"));
				}
