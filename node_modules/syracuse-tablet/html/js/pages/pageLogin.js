...
var _Page = utils.defineClass(
	function($parent, state) {
		...
		buildHtml: function(data) {
			...
			try {
				dispatcher.queryLocalePreferences().then(function(locales) {
					var html = [];
					(locales || []).forEach(function(locale) {
						html.push('<option value="');
						html.push(locale.code);
						html.push('"');
					...
					pageTemplates.getTemplate(self.state, _tmplCtx.header, _tmplCtx.content).then(function(result) {
		...
		_actLogin: function() {
			...
			auth.login(usr, pwd).then(function(ok, userProfile, message) {
				if (ok) {
					success.show().html("Login succeeded");
					window.setTimeout(function() {
						// self.getProp("redirect") - not implemented
				...
				fail.show().html("Login failed<p>" + e.message + "</p>");
				uiutils.waitStop();
			});
		}
	});
