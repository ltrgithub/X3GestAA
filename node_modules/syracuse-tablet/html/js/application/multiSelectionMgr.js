...
var _templates = {
	...
	processMenu: '\
		<a draggable="false"  href="#" class="s-m-ismultisel" data-action="multiSelectionTrigger" data-params="$exitrun" data-control-id="{{ctrlId}}">\
			<i class="{{icon}}"/>\
		</a>'
};
var _selAction = [{
...
var _Klass = utils.defineClass(
	...
	function(controller, opts) {
		...
		toggle: function(ctrl, $$parent) {
			...
			var enabled = this._$$ownerParent != null;
			// Hide/Show all action other than multisel ones
			ctrl.$$elmt.find("a:not(.s-m-ismultisel)").toggle(enabled !== true);
			return this._$$ownerParent != null;
		},
		/**
		 ...
		 * Called from array
		 */
		callbackArray: function(event, ctrlArray, action, $target) {
			if (event) {
				var record = $(event.currentTarget);
				record.toggleClass(_selectedClass);
				if (record.hasClass(_selectedClass)) {
		...
		_startMultiSel: function($$parent, array) {
			...
			this._$$All(array).attr("data-disabled", "");
		...
		_stopMultiSel: function(array) {
			...
			this._$$All(array).removeAttr("data-disabled");
		...
		triggerAction: function(array, uuid, $target) {
			...
			if (uuid == "$exitmultisel") {
				// Notify controlOwner
				notifications.publish("sm.multiSelection", uuid);
				return;
			}
		...
		_processAll: function(array, uuid) {
			...
			self._processStart(array, uuid).then(function(link, data) {
				console.log("process start", "nbRecords", data.length);
				var deferred = $.Deferred();
				var _run = function(idx) {
					if (idx >= data.length) {
						deferred.resolve(link, data);
					} else {
						self._processOne(link, data[idx], idx).then(function(ok) {
							_run(idx + 1);
						});
					}
				};
				...
				console.log("process stop", "ok", ok, "ko", data.length - ok);
				self._processEnd(false);
			}).fail(function(e) {
		...
		_processEnd: function(ok) {
			console.log("_processEnd");
		},
		_processExit: function(array) {
			console.log("_processExit");
			this._$$All(array).show().removeClass("error success " + _selectedClass);
			this._$$ownerParent.find('[data-params="$exitrun"]').remove();
		...
		_processOne: function(link, data, idx) {
			var deferred = $.Deferred();
			try {
				if (idx % 2 == 0) throw new Error("test");
				console.log("process " + idx);
				data.ok = true;
				data.$$record.addClass("success");
				deferred.resolve(true);
			} catch (e) {
				data.ok = false;
				data.$$record.addClass("error");
