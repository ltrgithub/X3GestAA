...
var _templates = {
	buttonLinks: '\
		{{#each links}}\
			<a draggable="false"  href="#" class="s-m-multisel-link {{css}}" data-action="multiSelectionAction" data-params="{{$uuid}}" data-control-id="{{../ctrlId}}">\
				<i class="{{icon}}"/>\
				<span>{{{title}}}</>\
			</a>\
		{{/each}}',
	menuActions: '\
		<div class="btn-group s-m-ismultisel">\
	...
	menuLinks: '\
		<div  id="{{id}}" class="btn-group s-m-ismultisel s-m-multisel-link">\
			<div class="{{dropDirection}}">\
				<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false">\
...
var _Klass = utils.defineClass(
	...
	function(controller, cssToToggle) {
		...
		_stopMultiSel: function(array) {
			...
			this._$$ownerParent.find(".s-m-multisel-link").remove();
		...
		_processOne: function(array, link, data, idx) {
			...
			try {
				if (link.$url) {
					...
					var payload = data && data.rowData ? data.rowData.getActionPayload(link, self.controller) : null;
					dispatcher.dispatch({
						$url: sdataUrl,
		...
		_updateUI: function(array, nb) {
			...
			this._$$ownerParent.find(".s-m-multisel-link").toggleClass("disabled", noSel);
			if (noSel || allSel) {
