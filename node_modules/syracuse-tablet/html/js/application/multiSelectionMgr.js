	...
	function(controller, cssToToggle) {
		...
		_processStart: function(array, uuid) {
			...
			try {
				...
				//	if(array.$display == "table"){
				unSelected.hide();
				//	} else {
				unSelected.css("visiblity", "hidden");
				//	}
				this._resizeArray(array);
		...
		_processExit: function(array) {
			this._$$All(array).show().removeClass("error success " + _selectedClass).removeAttr("data-disabled").css("visiblity", "hidden");
