	...
	function(controller, cssToToggle) {
		...
		_startDownloadAction: function(ctrlArray) {
			...
			modal.show().then(function(result) {
				if (result === "done") {
					//#8773 - Don't stop multi selection
					// self._stopMultiSel(ctrlArray);
				}
			});
		},
		/**
		 * Disable tablet header + table header
		 * Some menu (filters) are still enabled
		 * It's better than nothing (replaced by woco)
		 */
		blockEvents: function(yes, array) {
			var $$elmts = array.controller.$$elmt.find(".s-m-header.tablet").add(array.$$header)
			yes = yes === true;
			$$elmts.css("opacity", yes ? 0.5 : "inherit");
			if (yes) {
				$$elmts.click(function(evt) {
					evt.preventDefault();
					evt.stopPropagation();
				})
			} else {
