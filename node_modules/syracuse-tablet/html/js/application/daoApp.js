...
var _Dao = utils.defineClass(
	function(appli) {
		...
		getPageInfo: function(state, options) {
			...
			if (pi) {
				return self._extendPageInfo(pi);
			}
			var name = state;
			if (state === "home") {
			...
			/**
			 *  We pass a fallBack object for test applications pages are loaded dynamically - see getRegularPageInfo
			 **/
			pi = pageHelper.getPageInfo(name, options, {
				home: false,
				type: "regular",
		...
		_extendPageInfo: function(state) {
				...
				clientContract.getRegularPageInfo(state.name)
					...
					.then(function(info) {
						if (!info) {
							...
							registerPage = true;
							return self.fetchRemotePrototype(state.name);
						}
						return info;
					})
					.then(function(info) {
					...
					.then(function(info) {
						...
						// 3- generate s.th.
						var article = state.options.article || info.$page.$article || prototype.proto2Article(proto);
						deferred.resolve({
							article: state.options.article || info.$page.$article || prototype.proto2Article(proto),
							prototype: prototype.create(info.$page.$prototype)
						});
					}).fail(function(e) {
						deferred.reject(e);
					});
