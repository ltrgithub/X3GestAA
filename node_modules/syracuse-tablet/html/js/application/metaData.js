...
exports.getPageDetails = function(pageInfo) {
	...
	if (pageInfo.type === "rowdetail") {
		return _getRowDetailsPageDetails(pageInfo);
	}
...
function _getRowDetailsPageDetails(pageInfo) {
	return $.smResolve()
		.then(function() {
			var pageDetails = {
				prototype: pageInfo.rowDetailPrototype
			};
			return _ensureRegularPageDetailsArticle(pageInfo, pageDetails)
				.then(function() {
					return pageDetails;
...
function _ensureRegularPageDetailsArticle(pageInfo, pageDetails) {
	...
	var article = vignetteArticle != null && !$.isEmptyObject(vignetteArticle) ? vignetteArticle : pageDetails.article;
	var genArticle;
...
// To delete if after Mathias code review
function _ensureRegularPageDetailsArticle_old(pageInfo, pageDetails) {
	var proto = pageDetails.prototype;
	var article = pageInfo.options.article;
	var genArticle;

	// If the representation is shown in a dashboard, article is given by dashboard
	// If not, generate one and add it to the dashboard article
	if (article && Object.keys(article).length === 0) {
		genArticle = articleGen.genNewArticle({
			page: {
				prototype: proto
			}
		})
			.then(function(protoArticle) {
				// Inject vignette article into dashboard
				Object.keys(protoArticle).forEach(function(p) {
					article[p] = protoArticle[p];
				});
			});
	} else {
		// Check if the dashboard is giving an article (pageInfo.options.article)
		// If not, use the article defined this representation in fullscreen (not as vignette)
		// Generate an article if there is none of the both
		article = pageInfo.options.article || pageDetails.article;
		if (article) {
			genArticle = $.smResolve(article);
		} else {
			genArticle = $.smResolve()
				.then(function() {
					article = article || pageInfo.article;
					if (!article) {
						return articleGen.genNewArticle({
							page: {
								prototype: proto
