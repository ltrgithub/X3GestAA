...
exports.getWelcomeApplication = function() {
	//get welcome app in cache else get default (never stored server side)
			...
			if (dashboard) {
				if (dashboard && dashboard.$update) {
					return dashboard.$update(dashboard).then(function(dashboard) {
						/*set the welcomedashboard to avoid reading each time we need it.
						 For example, we need it to check if page or tile is pinned, etc ...
						 don't use updatePageAuthoring because not only article can change and changing 
						 updatePageAuthoring according to that could be make lot of regressions.
						 */
						application.setWelcomeDashboard(dashboard);
						return dashboard;
					});
				} else {
					return dashboard;
