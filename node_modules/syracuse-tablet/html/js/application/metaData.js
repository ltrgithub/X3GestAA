...
function _initApplications(forceReadFromServer) {
	...
	return _readApplications(forceReadFromServer).then(function(apps) {
		...
		return $.smForEachPromise(_applications, function(app) {
			return _getApplication(app.applicationName, true)
...
function _initWelcomeApplication() {
	return _getWelcomeApplication().then(function(welcomeApp) {
...
exports.checkApplicationUpdate = function(appName) {
	...
	return _getApplicationHeaderByName(appName)
		.then(function(_appHeader) {
			...
			return _getApplication(appName, true);
		})
...
function _getWelcomeApplication(switchContext) {
	return _getApplication("$welcomeApplication", true, false)
		.then(function(welcomeAppp) {
			if (welcomeAppp && !switchContext) {
				return welcomeAppp;
			} else {
				return dashBoardWelcomeUtils.defaultWelcomeApplication();
			}
		});
};
 ...
 * Name of application
 */
function _getApplication(name, localOnly, forceUpdate) {
	...
	return _getApplicationHeaderByName(name)
		.then(function(header) {
			if (header) {
				// #8849 When we open a link with context _currentApplicationContext is empty
				var ctx = _currentApplicationContext == null || $.isEmptyObject(_currentApplicationContext) ? _checkApplicationCtx() : _currentApplicationContext
				return metaDataCache.getApplication(header, ctx, localOnly, forceUpdate)
					.then(function(app) {
						if (app) {
...
function _updateWelcomeDashboard(newDashboard) {
	if (!newDashboard) {
	...
	return $.smResolve().then(function() {
		...
		var appMetaData = app && app.dao && app.dao.appMetaData;
		if (!appMetaData) {
			throw new Error("Unknown appMetaData " + newDashboard.$dashboardName);
		};
		appMetaData = $.extend(true, {}, appMetaData);
		var pageInfo = _getDashboardByName(newDashboard.$dashboardName, appMetaData);
		if (!pageInfo) throw new Error("Unknown page " + newDashboard.$dashboardName);
		pageInfo.$article = $.extend(true, {}, newDashboard.$article);
		pageInfo.$vignettes = $.extend(true, {}, newDashboard.$vignettes);
		pageInfo.$gadgets = $.extend(true, {}, newDashboard.$gadgets);
		pageInfo.$factoryOwner = newDashboard.$factoryOwner;
		pageInfo.$views = $.extend(true, {}, newDashboard.$views);
		pageInfo.appList = $.extend(true, {}, newDashboard.appList);
		pageInfo.description = newDashboard.description;
		pageInfo.roles = newDashboard.roles;
		pageInfo.user = newDashboard.user;
		pageInfo.voiceCommandContext = newDashboard.voiceCommandContext;
		pageInfo.$update = null;
		pageInfo.$title = newDashboard.$title;
...
exports.getWelcomeDashboard = function() {
	return _getWelcomeApplication().then(function(app) {
...
exports.getDashboard = function(name) {
	...
	var step;
	if (dashboard.$update) {
		step = dashboard.$update(dashboard).then(function(newDashboard) {
			dashboard = newDashboard
			return _initWelcomeApplication();
		}).then(function() {
			return _updateWelcomeDashboard(dashboard);
		})
	} else {
		if (dashboard.$pageInfo && dashboard.$pageInfo.isWelcomePage) {
			step = dashBoardWelcomeUtils.checkTilesStatus(dashboard)
		} else {
			step = $.smResolve(dashboard);
		}
	}
	return step.then(function(dashboard) {
		if (dashboard) {
			...
			// LV : Don't understand, but this code was here before welcome enhancement so I keep it ! 
...
exports.getApplication = _getApplication;
exports.getWelcomeApplication = _getWelcomeApplication;
exports.updateWelcomeDashboard = _updateWelcomeDashboard;