	...
	function($$elmt, factory) {
		...
		notifMainLayoutChanged: function(info, orientation, deviceClass) {
			...
			if (self.currentPage) {
				...
				// Async call is better to make sure page is displayed
				setTimeout(function() {
					// Re-initialization of scrolls because the size of dom elmts changed
					self.currentPage.enableScroll(true);
				});
			}
		},
		/**
				...
				self._pageLoader = new pageLoader.Klass({
					...
					succeeded: function(dstPage, parentNotifInfo) {
						...
						if (dstPage.isNew) {
							...
							// isNew = false means that the page has been reactivated from cache -> We just do nothing
		...
		getDashboards: function() {
			return this.dao && this.dao.appMetaData && this.dao.appMetaData.$dashboards || [];
		}
	});
