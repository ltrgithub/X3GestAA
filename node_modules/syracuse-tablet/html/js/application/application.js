...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		notifPageSettingsChanged: function(data) {
			...
			this._nativeAddRemoveLiveTile(data.liveTile);
		...
		_nativeAddRemoveLiveTile: function(add) {
			if (add === undefined) return;
			...
			if (add) {
				...
				provider = liveTileModule.addLiveTile;
				action = "add";
			} else {
				options = pageData.id;
				provider = liveTileModule.removeLiveTile;
				action = "remove";
			}
			provider(options)
				.then(function(result) {
		...
		_nativeScheduleMessage: function(data) {
			...
			if (!scheduleMessageModule) return;
