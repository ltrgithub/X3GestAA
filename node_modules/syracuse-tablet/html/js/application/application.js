...
var _subscribe = function(self) {
	notifications.subscribe(self, ["sm.switch.app", "sm.modal.open", "sm.modal.close", "sm.main.layout.changed", "sm.user.logged.in", "sm.page.settings.changed"], 1);
};
/**
	...
	function($$elmt, factory) {
		...
		onBeforeUnload: function() {
			if (native.hasCapability("X3WUPApp")) {
				native.getModule("X3WUPApp").notifUnloaded();
			}
		},
		/**
			...
			setTimeout(function() {
				self.changePage(self.homePageId).then(function() {
					if (self.isWelcomeApplication()) {
						notifications.publish(["sm.welcomeApplication"]);
					}
					deferred.resolve();
				}).fail(function(e) {
					deferred.reject(e);
			...
			settings.isPinnedPage(page.getPinPageId())
				.then(function(isPinned) {
					...
					if (native.hasCapability("liveTile")) {
						return native.getModule("liveTile").getLiveTile(page.getPinPageId())
		...
		notifPageSettingsChanged: function(data) {
			...
			this._addRemoveLiveTile(data.liveTile);
			this._scheduleMessage(data.schedule);
		},
			...
			if (add) {
				...
				settings.addPinnedPage(pageData)
					.then(function() {
						...
						notifications.publish("sm.page.pin.state.change", nativeLiveTile !== null ? 1 : 2);
					});
			} else {
		...
		_addRemoveLiveTile: function(add) {
			if (add === undefined) return;
			var options = {}, provider, self = this,
				action;
			var pageData = self.currentPage.getPinPageData() // allow to add information by subclasses;
			if (add) {
				options = $.extend(true, pageData, self.currentPage.getNavigationPageData());;
				options.applicationName = self.dao.getApplicationName();
				options.endpoint = pageData.endpoint || globals.getEndpoint();
				provider = native.getModule("liveTile").addLiveTile;
				action = "add";
			} else {
				options = pageData.id;
				provider = native.getModule("liveTile").removeLiveTile;
				action = "remove";
			}
			provider(options)
				.then(function(result) {
		...
		_scheduleMessage: function(data) {
			/*
				data: {
                    ScheduleTime: "2015-10-10 09:00:00"
                    Severity:"reminder"
			...
			data.DeepLink = this.currentPage.getNavigationPageData().DeepLink;
			native.getModule("scheduleMessage").scheduledMsg(data);
		},
		/**
		 * Open about page
		 */
		...
		_actPinPage: function() {
			...
			settings.isPinnedPage(self.currentPage.getPinPageId())
				.then(function(isPinned) {
					unpin = isPinned;
					return nativeLiveTile != null ? nativeLiveTile.getLiveTile(self.currentPage.getPinPageId()) : $.smResolve(false);
				}).then(function(isliveTile) {
					liveTileRemove = isliveTile;
					if (nativeLiveTile != null) {
						return globals.getModal().modalPinAndLiveTile({
							pageTitle: self.currentPage.state.$title,
							pinWelcome: unpin,
							liveTile: isliveTile
						}, {
							liveTile: true,
					...
					var message = locale.text("pin.page." + (unpin ? "remove" : "add") + ".confirm");
				...
				.then(function(result) {
					...
					if (!result.content || result.content.togglepinWelcome) {
						self._addRemovePinPage(!unpin);
					}
					var action;
					if (result.content && result.content.toggleliveTile) {
						self._addRemoveLiveTile(!liveTileRemove);
					}
				})
				.fail(function(e) {
		...
		_actNativeShare: function() {
			...
			native.getModule("share").sharePageLink(options)
				.then(function(result) {
					...
					// The notification is not neccessary since there is a UI popup
					// globals.getModal().notify(notify);
				});
		},
		...
		_getHistoryContext: function() {
			...
			self.historyScan(function(state) {
				...
				if (context.currentState == null) {
					// We store only name and options which is enough to call changePage
					// It also minimize the size of a page 's link
		 ...
		 * historyContext	historyContext to restore if any (url context in email link or context provided by nativeWrapper)
		 */
		notifUserLoggedIn: function(fromInitProcess, historyContext) {
			this._restoreHistoryContext(historyContext);
		},
		/**
		 * historyContext	historyContext to restore if any (url context in email link or context provided by nativeWrapper)
		 */
		_restoreHistoryContext: function(historyContext) {
			var self = this;
			...
			self.gotoWelcomeApplication().then(function() {
				if (historyContext == null || historyContext.applicationName == null) return;
				var step;
				if (historyContext.applicationName !== self.applicationName || historyContext.applicationName !== self.uuid) {
					// Check application name
					step = globals.getMetaData().getApplication(historyContext.applicationName).then(function(app) {
						var def = $.Deferred();
						if (app) {
							def.resolve(historyContext);
						} else {
							// Cancel historyContext
							globals.getModal().actionResult(locale.text("restorecontext.bad"), locale.text("restorecontext.badapp", [historyContext.applicationName]));
							def.resolve(null);
						}
						return def.promise();
					});
		...
		nativeWrapperJSInterface: function(moduleName, action) {
			...
			if (moduleName === "X3WUPApp" && action === "applicationClosing") {
				...
				var ctx = self._getHistoryContext(true);
			...
			if (moduleName === "voiceCommands" && action === "executeVoiceCommand") {
				return self._nativeExecuteVoiceCommmand(arguments[2]);
			}
			if (moduleName === "share" && action === "handleLink") {
				var historyContext = arguments[2];
				self.gotoWelcomeApplication().then(function() {
					if (historyContext) {
						self._restoreHistoryContext(historyContext);
					}
				});
				return;
			}
		},
		_nativeExecuteVoiceCommmand: function(data) {
			if (!data || !typeof data === "object") return;
			if (data.command === "openBookmark") {
				if (!data.text) return;
				alert(data.text);
			}
		}
