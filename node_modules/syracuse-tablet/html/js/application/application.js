	...
	function($$elmt, factory) {
		...
		_actClearCache: function() {
			...
			then(function(e) {
				self.waitWheelStop();
				var notify = {
					severityClass: e === "no" ? "info" : "success",
					title: "",
					body: e === "no" ? locale.text("notify.clear.cache.no") : locale.text("notify.clear.cache.ok")
				};
		...
		changePage: function(pageName, options) {
			...
			return changeApp
				.then(function() {
					var curPage;
					if (options && options.parentId && options.parentId.length > 0) {
						// Open a child page infullpage mode
						curPage = globals.getController(options.parentId);
						if (!curPage) throw new Error("changePage - Can't find parent page - " + options.parentId);
					} else {
						curPage = self.currentPage;
					}
					return pageRegistry.getPageInfo(pageName, options, curPage);
				})
