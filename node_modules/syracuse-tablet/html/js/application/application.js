...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		_nativeAddRemoveLiveTile: function(add) {
			...
			if (add === undefined || !liveTileModule) return;
			var options = {}, self = this,
				action;
			var pageData = self.currentPage.getPinPageData() // allow to add information by subclasses;
			var step;
			if (add === true) {
				step = self.currentPage.getTileLinkContext().then(function(ctx) {
					options = $.extend(true, pageData, ctx);
				...
				step = $.smResolve().then(function() {
					options = pageData.id;
					action = "remove";
					return liveTileModule.removeLiveTile(options);
				});
			}
