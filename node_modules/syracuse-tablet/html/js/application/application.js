...
var _subscribe = function(self) {
	notifications.subscribe(self, ["sm.modal.open", "sm.modal.close", "sm.main.layout.changed", "sm.user.logged.in", "sm.welcomeDashboard.changed"], 1);
};
/**
...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		setWelcomeDashboard: function(welcomeDashboard) {
		...
		notifWelcomeDashboardChanged: function() {
			dashboardWelcomeUtils.writeWelcomeDashboardUser(this.welcomeDashboard)
		},
		/**
		...
		_actSwitchContext: function() {
			...
			initContext.askNewContext()
				.then(function(canceled) {
					...
					self.welcomeDashboard = null; // not very pretty but need to clean it to avoid to keep same dashboard
					self.gotoWelcomeApplication();
				})
				.fail(function(e) {
					//#8000  needed to stop waitWheel trigger by changeProfile if failure
					self.waitWheelStop();
					self.welcomeDashboard = null;
		...
		gotoWelcomeApplication: function() {
			...
			if (globals.isLoggedIn()) {
				step = globals.getMetaData().getWelcomeApplication().then(function(appDetails) {
