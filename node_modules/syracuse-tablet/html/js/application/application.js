...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		gotoHome: function() {
			...
			setTimeout(function() {
				self.changePage(self.homePageId, {
					...
					//	notifications.publish(["sm.welcome.application"]);
					//}
					deferred.resolve();
				}).fail(function(e) {
					deferred.reject(e);
		...
		notifSwitchApp: function(appName) {
			this.switchToApplication(appName);
		},
		switchToApplication: function(appName) {
			var self = this;
		...
		_nativeExecuteVoiceCommmand: function(data) {
			var self = this;
			log && log("Voice command: " + JSON.stringify(data || {}));
			if (!data || !typeof data === "object") return;
			if (data.command === "openBookmark") {
				if (!data.text) return;
				if (native.hasCapability("voiceCommands")) {
					var bookmark = native.getModule("voiceCommands").getVoiceBookmark(data.text);
					log && log("Bookmark: " + JSON.stringify(bookmark || {}));
					if (!bookmark || !bookmark.context) return;
					var step, context = bookmark.context;
					if (context.endpoint && context.endpoint !== globals.getEndpoint()) {
						...
						// No need to switch endpoint - force gotoWelcomeApplication is not the current page
					...
					step.then(function(applicationName, forceGotoWelcomeApp) {
						...
						return step.then(function() {
							if (applicationName) {
								return self.switchToApplication(applicationName);
							}
						});
					}).fail(function(e) {
						globals.getModal().error(e);
