...
var _subscribe = function(self) {
	notifications.subscribe(self, ["sm.switch.app", "sm.modal.open", "sm.modal.close", "sm.main.layout.changed", "sm.user.logged.in", "sm.history.pop", "sm.history.push"], 1);
};
/**
...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		onUnload: function() {},
		// Called on page onBeforeUnload - We have time to cal processes
		onBeforeUnload: function() {
			if (false && nativeApp.isSageX3WUPApp() && nativeApp.notifBeforeUnload) {
				nativeApp.notifBeforeUnload();
			}
			this.destroy();
		},
		/**
		...
		logout: function(msgId) {
			...
			self.waitWheelStart();
			auth.logout(settings.logoutForgetMe()).then(function(ok, message) {
				self.waitWheelStop();
				if (ok) {
					_loggedOut();
				} else {
		...
		notifHistoryPush: function() {
			if (false && nativeApp.isSageX3WUPApp()) {
				// To test reconnection outside a native wrapper
				nativeApp.testClosingApplication(this._getHistoryContext());
			}
		},
		notifHistoryPop: function() {
			if (false && nativeApp.isSageX3WUPApp()) {
				// To test reconnection outside a native wrapper
				nativeApp.testClosingApplication(this._getHistoryContext());
			}
