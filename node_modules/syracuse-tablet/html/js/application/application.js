			...
			settings.isPinnedPage(page.getPinPageId())
				.then(function(isPinned) {
					...
					return nativeLiveTile.getLiveTile(page.getPinPageId())
						.then(function(liveTileGot) {
							if (page.state.type !== "regular" || !nativeLiveTile.supportsLiveTile()) {
								return globals.getModal().simpleConfirmPromise({
									message: message
								});
							} else {
								removelivetile = (liveTileGot && liveTileGot.content !== null);
								return globals.getModal().modalPinAndLiveTile({
									pageTitle: page.state.$title,
									pinwelcome: isPinned,
									livetile: (liveTileGot && liveTileGot.content !== null)
								});
							}
						});
				})
							...
							.then(function(result) {
								...
								if (result && result.error) {
									notify = {
										...
										title: locale.text("livetile.page." + result.action + ".notok"),
									...
									notify = {
										severityClass: "success",
										title: "",
										body: locale.text("livetile.page." + result.action + ".ok"),
									};
								}
								globals.getModal().notify(notify);
								notifications.publish("sm.page.pin.state.change", 2);
