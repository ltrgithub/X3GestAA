...
var _Application = utils.defineClass(
	function($elmt) {
		...
		init: function() {
			...
			self._bindEvents();
			self.dao = factory.createDaoApp(self);
		...
		_activate: function() {
			...
			var json = self.dao.getJsonDescr();
			if (json) {
		...
		notifSwitchApp: function(appInfo) {
			...
			self.dao.setJsonDescr(appInfo);
			self.removeAllPages();
			reqPage.registerAppPages(appInfo);
			setTimeout(function() {
				self.changePage(appInfo.$application.$homeDashboard.$uuid);
			});
		},
		...
		// Optio noverride standard pageInfo and allows to store a context
