...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		back: function(state) {
			...
			if (self.homePageId === "$welcomeDashboard.$dashboard") {
				return;
			}
			if (!state) {
				throw new Error("Back", "Unexpected null history state");
			}
			if (state.id === "$welcomeDashboard.$dashboard") {
				self.gotoWelcomeApplication();
			} else {
		...
		gotoWelcomeApplication: function() {
			...
			settings.getMyApplicationDetail(welcomeApp).then(function(appDetail) {
				self.dao.setApplicationDesc(appDetail);
			...
			if (!self._pageLoader) {
				...
				self._pageLoader = new pageLoader.Klass({
					...
					succeeded: function(dstPage, parentNotifInfo) {
						if (parentNotifInfo) alert(JSON.stringify(parentNotifInfo, null, 2));
					}
				});
			}
		...
		/**
		 Set application ChartsTheme
		 @param ct ChartTheme object
		*/
		setChartsTheme: function(ct) {
			this.chartsTheme = ct;
		},
		/*
		Get application ChartsTheme
		*/
		getChartsTheme: function() {
			return this.chartsTheme;
		}
