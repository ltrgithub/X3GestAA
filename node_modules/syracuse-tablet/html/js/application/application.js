...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		_addRemoveLiveTile: function(add) {
			...
			var pageData = self.currentPage.getPinPageData() // allow to add information by subclasses;
			if (add) {
				options = $.extend(true, pageData, self.currentPage.getTileLinkContext());;
		...
		_scheduleMessage: function(data) {
			...
			data.DeepLink = this.currentPage.getTileLinkContext().DeepLink;
		...
		_actNativeShare: function() {
			var options = {
				pageData: this.currentPage.getTileLinkContext()
			};
		...
		notifUserLoggedIn: function(fromInitProcess, restoredContext) {
			this._restoreHistoryContext(restoredContext);
		},
		/**
		...
		_restoreHistoryContext: function(restoredContext) {
			...
			self.gotoWelcomeApplication().then(function() {
				...
				return self.changePage(restoredContext.currentState.name, opts);
			});
		},
		/**
		...
		nativeWrapperJSInterface: function(moduleName, action) {
			...
			if (moduleName === "X3WUPApp" && action === "applicationClosing") {
				...
				var ctx = self._getSavedContext(true);
		...
		_nativeExecuteVoiceCommmand: function(data) {
			...
			if (data.command === "openBookmark") {
				if (!data.text) return;
				if (native.hasCapability("voiceCommands")) {
					return native.getModule("voiceCommands").tmpExecuteVoiceCommmand(this, data.text);
				}
			}
		}
	});
