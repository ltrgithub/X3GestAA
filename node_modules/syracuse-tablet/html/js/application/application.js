...
var _Application = utils.defineClass(
	function($elmt, factory) {
		...
		_actOpenlinksbar: function(linksBar) {
			this.currentPage.$$linksBar = this.currentPage.$$linksBar || $("div.sm-links-bar");
			this.currentPage.$$linksBar.toggleClass("hidden");
		},
		...
		_gotoPage: function(state, back) {
			...
			var failed = function(e) {
				uiutils.waitStop();
				modal.error("Application page [" + state.name + "] loading error", e, function() {
					deferred.reject();
				});
			};
			var succeeded = function(dstPage) {
				...
				if (back !== true) {
					...
					window.history.pushState(dstPage.state, dstPage.state.uuid, null);
					log && log("pushState", dstPage.state.uuid, "history.length=" + window.history.length);
			...
			try {
				...
				self.factory.createPage(self.$elmt, state)
					.then(function(p) {
						dstPage = p;
		...
		changePage: function(state, options) {
			...
			return self.getPageInfo(state, options)
				.then(function(info) {
					return _currentApp._gotoPage(info);
				});
		},
