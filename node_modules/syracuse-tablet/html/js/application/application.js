...
var nativeLifeTile = require('syracuse-tablet/html/js/helpers/native/nativeLifeTile');
...
var _subscribe = function(self) {
	notifications.subscribe(self, ["sm.switch.app", "sm.modal.open", "sm.modal.close", "sm.main.layout.changed"], 1);
};
/**
		...
		_actPinPage: function() {
			...
			settings.isPinnedPage(page.getPinPageId())
				.then(function(isPinned) {
					...
					return nativeLifeTile.getLifeTile(page.getPinPageId())
						.then(function(lifeTileGot) {
							if (page.state.type !== "regular" || !nativeLifeTile.supportsLifeTile()) {
								return globals.getModal().simpleConfirmPromise({
									message: message
								});
							} else {
								removelifetile = (lifeTileGot.content !== null);
								return globals.getModal().modalPinAndLifeTile({
									pageTitle: page.state.$title,
									pinwelcome: isPinned,
									lifetile: (lifeTileGot.content !== null)
								});
							}
						});
				})
				.then(function(result) {
					...
					if (result.content && result.content.togglelifetile) {
						var options = {}, provider;
						if (removelifetile) {
							options = pageData.id;
							provider = nativeLifeTile.removeLifeTile;
						} else {
							options = pageData;
							options.applicationName = self.dao.getApplicationName();
							options.endpoint = pageData.endpoint || globals.getEndpoint();
							provider = nativeLifeTile.addLifeTile;
						}
						provider(options)
							.then(function(result) {
								var notify = {};
								if (result && result.error) {
									notify = {
										...
										title: locale.text("lifetile.page." + result.action + ".notok"),
										body: result.error.no + " " + result.error.text
									};
								} else {
									notify = {
										severityClass: "success",
										title: "",
										body: locale.text("lifetile.page." + result.action + ".ok"),
									};
								}
								globals.getModal().notify(notify);
								notifications.publish("sm.page.pin.state.change", 2);
