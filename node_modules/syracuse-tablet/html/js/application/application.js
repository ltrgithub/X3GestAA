...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		/**
		 * Pin/Unpin the current on the native wrapper liveTile
		 */
		_pinLiveTile: function(message, isPinned) {
			var self = this;
			var unpin = isPinned !== true;
			var nativeModule = native.getModule("liveTile");
			if (!nativeModule) return;
			var removeliveTile = false;
			return nativeModule.getLiveTile(self.currentPage.getPinPageId())
				.then(function(isliveTile) {
					var opt = {
						liveTile: true,
						pinWelcome: true
				...
				.then(function(result) {
					...
					if (!result.content || !result.content.toggleliveTile) return;
					var pageData = self.currentPage.getPinPageData() // allow to add information by subclasses;
					var action;
					var options = {}, provider;
					if (removeliveTile) {
						options = pageData.id;
						provider = nativeModule.removeLiveTile;
						action = "remove";
					} else {
						...
						provider = nativeModule.addLiveTile;
						action = "add";
					}
					provider(options)
						.then(function(result) {
		...
		/**
		 * Pin/Unpin the current on the welcome dashboard
		 */
		_pinWelcomeDashboard: function(message, isPinned) {
			var self = this;
			var unpin = isPinned !== true;
					...
					if (unpin) {
						...
						settings.addPinnedPage(pageData)
							.then(function() {
								...
								globals.getModal().notify(notify);
								notifications.publish("sm.page.pin.state.change", self.currentPage.capabilites.liveTile ? 1 : 2);
							});
					}
				});
		},
		_actPinPage: function() {
			...
			settings.isPinnedPage(self.currentPage.getPinPageId())
				.then(function(isPinned) {
					...
					if (self.currentPage.nativeCapabilities.liveTile) {
						self._pinLiveTile(message, isPinned);
					} else {
						self._pinWelcomeDashboard(message, isPinned);
					}
				})
				.fail(function(e) {
					globals.getModal().error(e);
		...
		_restoreHistoryContext: function(historyContext) {
			...
			self.gotoWelcomeApplication().then(function() {
				...
				var step;
				if (historyContext.applicationName !== self.uuid) {
				...
				step.then(function(context) {
					...
					self.changePage(gotoState.name, opts);
				});
			});
		},
		/**
		...
		nativeWrapperJSInterface: function(moduleName, action) {
			...
			if (moduleName === "X3WUPApp" && action === "applicationClosing") {
				...
				var ctx = self._getHistoryContext();
