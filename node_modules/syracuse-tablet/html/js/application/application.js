...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		isLoadingPage: function() {
			...
			return this._pageLoader != null && this._pageLoader.isLoading();
		},
		...
		_actOpenDraft: function(draftId) {
			...
			return globals.getStorage().draftOperation("draftRead", draftId).then(function(ctx) {
				...
				self._switchToContext(ctx.restoredContext);
			});
		},
		/**
		...
		_actHistoryBack: function() {
			// #9519 call directly historyPop window.history.back();
			this.historyPop();
		},
		...
		_actSwitchContext: function() {
			...
			initContext.askNewContext()
				.then(function(canceled) {
					...
					self.gotoWelcomeApplication(true); //true : switchcontext
				})
				.fail(function(e) {
		...
		_addRemovePinPage: function(add) {
			...
			if (add) {
				...
				dashboardWelcomeUtils.addPinnedPage(pageData)
					.then(function() {
						var notify = {
				...
				dashboardWelcomeUtils.removePinnedPage(pageData.id, dashboardWelcomeUtils.groupType("quickLinks"), true).then(function() {
					var notify = {
						severityClass: "success",
						title: "",
		 ...
		 */
		gotoWelcomeApplication: function(switchContext) {
			var self = this,
				step;
			if (globals.isLoggedIn()) {
				step = globals.getMetaData().getWelcomeApplication(switchContext).then(function(appDetails) {
		...
		_setCurrentApplication: function(appInfo, options) {
			...
			try {
				...
				if (self.uuid !== appInfo.$application.$uuid || options.forceChange) {
					// Application changed
					...
					self.welcomeApplication = appInfo.$application.welcomeApplication || false;
				}
		...
		_gotoPage: function(state, back, loadOptions) {
			...
			if (self.applicationName && state.applicationName != self.applicationName) {
				// #8791 - Force application if not current one - Needed if we call _gotoPage history
				step = self._switchToApplication(state.applicationName, {
					noPageChange: true
				});
			} else {
				step = $.smResolve();
			...
			return step.then(function() {
				if (state.isLogin === false && !globals.isLoggedIn()) {
					...
					return $.smResolve();
				} else if (self._pageLoader.isLoading()) {
		...
		historyPop: function(event, pageFilter) {
			...
			if (this._pageLoader && this._pageLoader.isLoading()) {
				self._pageLoader.stopLoading("back");
			...
			step.then(function(kontinue) {
				...
				if (state) {
					setTimeout(function() {
						var step;
						// Test !self.currentPage.isLookup()
						// -> create/edit facet in a vignette pinned on welcomePage
						// -> When we select a lookup we want to go to the welcome page (regular lookup selection process) (not gotoWelcomeApplication)
						if (state.isWelcomePage && !self.currentPage.isLookup()) {
		...
		changePage: function(pageName, pageOptions, changePageOpts, savedContext) {
			...
			var step;
			if (pageName == "$welcomeDashboard.$mobileDashboard") {
				// Force application if not current one
				pageOptions.applicationName = "$welcomeApplication"
			}
			...
			return step
				.then(function() {
					...
					// we know that we go to a welcome dashboard only because of welcomeApplication on application 
		...
		logout: function(msgOrLocaleId, isError, offlineHint) {
			...
			var _loggedOut = function() {
				...
				globals.getMetaData().removeApplications();
				// Do an sdata request here, no matter what's the outcome
				// intention of this is to allow the dispatcher on server side
				// to catch the current url for later use on sageid redirect
		...
		_actPinPage: function() {
			...
			dashboardWelcomeUtils.isPinnedPage(self.currentPage.getPinPageId())
		...
		_restoreHistoryContext: function(restoredContext) {
			...
			return step.then(function() {
				...
				globals.getModal().error("Error restoring page context", e);
			});
		},
		/**
		...
		nativeWrapperJSInterface: function(moduleName, action) {
			...
			if (moduleName === "voiceCommands" && action === "executeVoiceCommand") {
				...
				if (cmd.type === "openBookmark") {
					...
					self._switchToContext(cmd.context);
					return;
				}
			}
		 ...
		 * }
		 */
		_switchToContext: function(context) {
			var self = this;
			if (!context) return;
			var step, context;
