...
var _Application = utils.defineClass(
	function($$elmt, factory) {
		...
		notifUserLoggedIn: function(fromInitProcess, historyContext) {
			var self = this;
			var step;
			if (historyContext == null || historyContext.applicationName == null) {
				// Nothing to restore -> Goto home page
				step = $.smResolve(null);
			} else if (historyContext.applicationName !== self.uuid) {
				...
				step = globals.getMetaData().getApplication(historyContext.applicationName).then(function(app) {
					...
					if (app) {
						...
						globals.getModal().error("Restore context error", "Application " + historyContext.applicationName + "not found", function() {
							// Cancel history -> Goto home page
							def.resolve(null);
						});
					}
					return def.promise();
			...
			return step.then(function(context) {
				try {
					// Restores the context
					// -1- gotoWelcomeApplication is mandatory (this page need to be displayed to continue)
					// -2- Push context states in history (except the last one)
					// -3- Goto to the page given by the last state
