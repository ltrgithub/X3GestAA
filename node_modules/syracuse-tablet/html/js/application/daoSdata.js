	...
	this._walkPlainObject = function(o, operation, result) {
		...
		for (var ii = 0, jj = p.length; ii < jj; ii++) {
			...
			if (operation.removeprop) {
				walkInside = this.removeprop(operation.removeprop, o, name, result, operation.test);
			}
...
var _Dao = utils.defineClass(function(data, prototype, options) {
	...
	getActionPayload: function(link, isEditMode) {
		...
		// For test- 
		if (operations.test && result) {
			console.log("getActionPayload - operations - Elsaped=", new Date().getTime() - result.startTime);
