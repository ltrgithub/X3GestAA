...
var _getArrayEmptyRow = function(controller, proto, bind, parentDao) {
	...
	return new _Dao(rsrc, _getProtoRowDetail(controller, proto), parentDao, bind);
};
var _getProtoRowDetail = function(controller, proto) {
	var protoJson = {
		$url: "",
		$typ: "rowdetail",
		$links: {}
	...
	if (controller.isEditMode()) {
		...
		if (true || controller.prototype.getDataByPath("$links.$delete")) {
	...
	if (proto.isSingleArray()) {
		...
		return proto.getPrototype("$item", protoJson, controller.$activity === "read" ? "details" : "edit");
	}
};
/**
 ...
 */
var _getDaoRowDetail = function(parentDao, controller, protoArray) {
	var prototype = _getProtoRowDetail(controller, protoArray);
	// parentDao is needed to resolve some properties in prototype taht needs parent data (url's of images...)
	return new _Dao($.extend({}, true, parentDao.data), prototype, parentDao.parent, parentDao.$bind);
};
...
var _Dao = utils.defineClass(function(data, prototype, parent, $bind) {
	...
	getArrayData: function($bind, protoArray) {
		...
		if (protoArray.isSingleArray()) {
			...
			rsrcs.forEach(function(rsrc, idx) {
				...
				var diag = null;
				var errEntry;
				if (protoRow.property(globals.SINGLEARRAYPROP).$type === "application/x-reference") {
					// Add the field's value in data object {CUR_REF:{}, CUR:"USD"} -> {"~data~":{...}, CUR:"USD"}
					var valFld = protoRow.getFieldNameForValue(globals.SINGLEARRAYPROP);
					if (valFld) {
	...
	getActionPayload: function(link, controller) {
		// !! We remove the diagnoses from dataset before sending a request
		var result = this._processData({
			removeprop: ["$diagnoses"]
		});
		// For test- console.log("removeprop", JSON.stringify(result, null, 2));
		if (!link) return;
		var payload;
	...
	_walkPlainObject: function(o, operation, result) {
		...
		for (var ii = 0, jj = p.length; ii < jj; ii++) {
			name = p[ii];
			e = o[name];
			if (operation.removeprop && operation.removeprop.indexOf(name) >= 0 && e !== undefined) {
				o[name] = undefined;
				if (result.nbRemoveprop == null) {
					result.nbRemoveprop = 0;
				}
				result.nbRemoveprop++;
			}
			if (typeof(e) === 'object') {
...
exports.getDaoRowDetail = _getDaoRowDetail;
