...
var _remove$Index = function(payload) {
	if (payload != null) {
...
var _Dao = utils.defineClass(function(data, prototype, options) {
	...
	setValue: function(name, value, notify) {
		...
		} else if (this.prototype.isDate(name)) {
			if (value === "") {
				value = null;
			}
	...
	getActionPayload: function(link, controller) {
		...
		// Remove $index by dedicated function to not introduce regression by modification of "DataProcessor" which
		// seems to not remove the listed properties at all.
		// But making "DataProcessor" work also removes things like $uuid which breaks all sdata operations
		_remove$Index(payload);
		return payload;
	},
	clone: function() {
