...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("daoSData", true);
	...
	this._walkPlainObject = function(o, operation, result) {
		...
		for (var ii = 0, jj = p.length; ii < jj; ii++) {
			...
			if (operation.removeprop) {
				walkInside = this.removeprop(operation.removeprop, o, name, result);
			}
...
var _Dao = utils.defineClass(function(data, prototype, options) {
	...
	publishDirty: function(dirty, forceNotify) {
		// Only page dao
		if (this.$bind == null || this.$bind.length > 0) return;
	...
	getQueryData: function(protoArray, localPaginator) {
		...
		var res = {
			...
			displayLineNum: this.getValue("displayLineNum"),
			ifCube: this.getValue("ifCube"),
			level_c: this.getValue("level_c"),
			level_m: this.getValue("level_m"),
			level_p: this.getValue("level_p"),
			$links: this._getlinks(protoArray)
		};
		res.$resources = this._createArrayData("$resources", protoArray, localPaginator);
		return res;
	},
	_getlinks: function(protoArray) {
		var linksNameTampering = function($link) {
		...
		var protoLinks = linksNameTampering(protoArray.getDataByPath("$item.$links"));
		var dataLinks = linksNameTampering(this.getPathValue("$properties.$resources.$item.$links"));
		return $.extend(true, dataLinks, protoLinks);
	},
	/**
	 * Lookup info
	 */
