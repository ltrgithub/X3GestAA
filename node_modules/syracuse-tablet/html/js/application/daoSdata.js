...
var _Dao = utils.defineClass(function(data, prototype, parent, $bind) {
	 ...
	 */
	updateParentRow: function(create, forceParent) {
		forceParent = forceParent || this.parent;
		if (!forceParent || !this.data || $.isEmptyObject(this.data)) return false;
		var id = this.data.$uuid;
		var grid = forceParent.data[this.$bind];
		if (!grid) {
			if (!create) return false;
			grid = forceParent.data[this.$bind] = [];
		}
		if (create) {
			grid.push($.extend({}, this.data));
		} else {
			var idx = -1;
			...
			if (utils.areEqual(this.data, grid[idx])) return false;
			grid[idx] = $.extend({}, this.data);
		}
		return true;
	},
	deleteRow: function(field, rowid) {
	...
	getFacet: function() {
		return this.prototype && this.prototype._facet;
	}
});
// for override only
exports.Dao = _Dao;
