...
// We don"t take into account $isAction and $method which should have the same value as prototype 
var _linksMeta = ["$isDisabled", "$isHidden", "$style", "$isRequested"];
...
var _Dao = utils.defineClass(function(data, prototype, parent, $bind) {
	...
	getDisplayValue: function(name, defValue) {
		var val = this.getValue(name, defValue);
	...
	getMetaData: function(name) {
		if (this.prototype.isReference(name)) {
			...
			name = this.prototype.getFieldNameForValue(name);
		}
		var meta = this._cache.metaData[name];
		if (meta) return meta;
		meta = this._cache.metaData[name] = $.extend({}, this.prototype.getMetaData(name), this.data.$properties ? this.data.$properties[name] : null);
		return meta;
	},
	/**
	 * Return the meta-data for $links including the ones given by prototype
	 */
	getLinksMeta: function() {
		...
		if (meta) return meta;
		meta = {};
		var proto = this.prototype.data(name);
		var data = this.data[name];
		for (var p in _linksMeta) {
			if (data[p] != null) {
				meta[p] = data[p];
		...
		meta = this._cache.metaData[name] = meta;
		return meta;
	},
	...
	/**
	 * Update dao with $isActions results
	 */
	update: function(json) {
		return;
		/*
		var copy = function(dst, data) {
			
		}
		var self = this;
		if (!json) return;
		self.resetCache ();
		["$properties","$links"].forEach(function(p){
			copy(self.data[p], json[p],)
		})*/
	}
});
// for override only
