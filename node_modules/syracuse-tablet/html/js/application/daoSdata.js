...
var _getArrayEmptyRow = function(controller, proto, bind, parentDao) {
	...
	return new _Dao(rsrc, _getProtoRowDetail(controller, proto), {
		parent: parentDao,
		$bind: bind
	});
};
...
var _getDaoRowDetail = function(parentDao, controller, protoArray) {
	...
	return new _Dao($.extend(true, {}, parentDao.data), prototype, {
...
var _Dao = utils.defineClass(function(data, prototype, options) {
	...
	this.resetCache();
	this.enableWorkingCopy = options.enableWorkingCopy === true;
	if (this.enableWorkingCopy === true && options.createWorkingCopy === true) {
		var self = this;
		//CReate working copy if not alrady exists
		globals.getStorage().draftOperation("wrkcpyCreate", self.data).then(function(data) {
	...
	setValue: function(name, value, notify) {
		...
		if (done && this.enableWorkingCopy === true) {
			//Manage a the workingcopy
			globals.getStorage().draftOperation("wrkcpyUpdate", this.data);
		}
		return done;
	},
	getWorkingCopyId: function() {
		return this.enableWorkingCopy === true && this.data != null ? this.data.$uuid : null;
	},
	/**
	 * Get field's value
