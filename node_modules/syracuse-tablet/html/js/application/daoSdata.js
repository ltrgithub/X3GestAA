...
var _Dao = utils.defineClass(function(data, prototype, parent, options) {
	...
	getArrayData: function(name, protoArray) {
		...
		if (protoArray.isSingleArray()) {
			// $resources contains an array of single type $resource:[2,6,8,9]
			// We create the same field structure with a hard coded field name singleArrayProp {$data:1}
			rsrcs.forEach(function(rsrc) {
				var data = {};
				data[reqProto.singleArrayProp] = rsrc;
				res.$resources.push(new _Dao(data, protoArray, self));
			});
		} else {
			rsrcs.forEach(function(rsrc) {
