...
var _findControlById = function(app, ctrlId, $$target) {
	...
	if (controller && controller.length > 0) {
		...
		if (controller) {
			ctrl = controller.getControl(ctrlId);
		}
	}
	if (ctrl) return ctrl;
	if (app.currentPage.isDashboard()) {
		// May be unused since I added data-controller-id
		var res = utils.findCtrlInDashboard($$target, ctrlId);
		if (res) ctrl = res.control;
		if (ctrl) return ctrl;
	}
...
var _bindActions = function(app) {
	...
	var handler = utils.bindFn(_handleActionEvt, null, app);
