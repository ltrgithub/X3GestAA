...
var _handleActionEvt = function(evt, app) {
	...
	try {
		...
		if ($$actionElmt.is("[data-action]")) {
			...
			if (_isNavigationAction(act) && $$actionElmt.closest("[data-disable-navigation]").length > 0) return
			if ($$actionElmt.is("select")) {
				if ($$actionElmt.closest("[data-disable-navigation]").length > 0) return
				param = $(this).val();
			} else {
			...
			if (_attr($$actionElmt, "data-disable-navigation"))
				return
				// NAVIGATION
...
var _isNavigationAction = function(act) {
	return act === globals.ACTCHARTDETAIL || act === globals.ACTROWDETAIL
}
...
var _bindEvents = function(app) {
	...
	if (mobileDetect.isMobile() && mobileDetect.isOs("android")) {
		_disablePullToRefresh();
	}
};
// Disable the pull down event that refreshes the page on android #8588
var _disablePullToRefresh = function() {
		...
		window.document.addEventListener('touchmove', touchmoveHandler, false);
	}
	/**
	 * Trigger an action
