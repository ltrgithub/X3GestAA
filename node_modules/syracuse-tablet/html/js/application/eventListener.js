	...
	$$elmt.delegate("[data-nav], [data-nav-refresh], [data-action]", "click select change", function(evt) {
		try {
			...
			if (clickedElmt.length > 0) {
				...
				var attr = _attr(clickedElmt, "data-sdata-method");
				if (attr.length > 0) {
...
/**
 * Trigger an action
 * 		$$elmt: JQL element - $$$$elmt a control by example
 */
var _triggerAction = function(control, act, params, controlId) {
	if (!control || !control.$$elmt) return;
	_doAction(control.$$elmt, globals.getApplication(), act, params, controlId);
};
