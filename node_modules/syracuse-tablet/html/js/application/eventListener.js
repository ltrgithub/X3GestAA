...
var _findParentCtrlId = function($$target, selector) {
	// Find a control arra with an ID
...
var _findParentVignette = function($$target, app, fail) {
	app = app || globals.getApplication();
...
/** NAVIGATION 
 * clickedElmt:	jQuery element or object that provides attr() method
 * app			global application
 * target		$$target event for click in page
 * 				Null or vignette control if navigation is triggered
 * **/
var _doNavigation = function(clickedElmt, app, targetOrVignette) {
	var options = {}, attr;
	// Page to open
	...
	if (targetOrVignette) {
		if (attr === "vignette") {
			...
			navTarget = targetOrVignette.jquery ? _findParentVignette(targetOrVignette, app, true) : targetOrVignette;
			...
			navTarget = targetOrVignette.jquery ? _findParentVignette(targetOrVignette, app, false) : targetOrVignette;
		}
	...
	$$elmt.delegate("[data-nav], [data-nav-refresh], [data-action]", "click select change", function(evt) {
		try {
			...
			if (clickedElmt.length > 0) {
				_doNavigation(clickedElmt, app, $$target);
				return;
			}
...
/**
 * Trigger a navigation (open link) according to attributes params
 * attributes		SAme attributes as $$element
 * vignetteCtrl		Null for navigation in application
 * 					Vignette  for navigation in a vignette
 */
var _triggerNavigation = function(attributes, vignetteCtrl) {
	...
	_doNavigation(new attrs(attributes), globals.getApplication(), vignetteCtrl);
};
