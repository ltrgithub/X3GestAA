...
var _reconnectGetToken = function() {
	...
	ajax("GET", _config.svcReconnectGetToken, null, {
		...
		if (result && result.status === 200 && result.headers) {
			...
			var loginCookieName = result.headers["x-reconnect-login-cookie-name"];
			var sidCookieName = result.headers["x-reconnect-sid-cookie-name"];
			log && log("_reconnectGetToken", token, "sid", sid, "loginCookieName", loginCookieName, "sidCookieName", sidCookieName);
...
exports.getAuthData = function(schema) {
	return _availableAuthentications[schema] || [];
}