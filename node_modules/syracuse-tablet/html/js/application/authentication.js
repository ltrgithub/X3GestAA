...
var _check = function() {
	return ajax("GET", _config.checkAuth, null, null, {
		...
		if (result && result.status === 200 && result.responseJSON) {
			...
			if (nativeApp) {
				_reconnectGetToken().then(function(token, userLogin, sid) {
					if (token && userLogin) {
						nativeApp.setReconnectToken(token, userLogin, sid);
					}
...
var _reconnectGetToken = function() {
	...
	ajax("GET", _config.svcReconnectGetToken, null, {
		...
		if (result && result.status === 200 && result.headers) {
			...
			log && log("_reconnectGetToken", token, "sid", sid);
			var login = _parseReconnectCookie(token);
			deferred.resolve(login ? token : null, login, sid);
		} else {
