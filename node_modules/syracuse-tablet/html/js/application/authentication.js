...
var notif = require('syracuse-tablet/html/js/helpers/notifications');
...
var _check = function() {
	...
	try {
		var checkurl = utils.baseLocation.host + _config.authParams;
		...
		log && log("Check login request error", e);
...
var _login = function(user, password) {
	...
	try {
		...
		return ajax("GET", _config.svcDoLogin, {
			...
			if (result.isSuccess) notif.publish("sm.login", result.responseJSON);
		...
		log && log("Do login request error", e);
		deferred.resolve(false, null, "JavaScript error [" + e.message + "]");
	} finally {
		return deferred.promise();
	}
};
...
var _logout = function() {
	var deferred = new $.Deferred();
	try {
		var url = utils.baseLocation.host + _config.svcLogout;
		// temporarily - To improve
		return ajax("GET", url, {
			device: "phone"
		}).always(function(result) {
			...
			notif.publish("sm.logout");
			deferred.resolve(result.isSuccess, msg);
		});
	} catch (e) {
		log && log("Do logout request error", e);
		deferred.resolve(ok, "JavaScript error [" + e.message + "]");
	} finally {
		return deferred.promise();
	}
};
