...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("auth", false);
...
var _config = {
	...
	checkAuth: "/sdata/syracuse/collaboration/syracuse/userProfiles/$service/current",
 ...
 */
var _check = function() {
	return ajax("GET", _config.checkAuth, null, null, {
		...
		if (result && result.status === 200 && result.responseJSON) {
			...
			if (nativeApp) {
				_reconnectGetToken().then(function(token, userLogin, sid, loginCookieName, sidCookieName) {
					if (token && userLogin) {
...
var _reconnectGetToken = function() {
	...
	ajax("GET", _config.svcReconnectGetToken, null, {
		...
		if (result && result.status === 200 && result.headers) {
			...
			// We return the cookies names because the client doesn't know the port number used to build the name in case of reverse proxy like on server http://sage-x3-mobile.sage-demo.fr
			...
			log && log("_reconnectGetToken", token, "sid", sid, loginCookieName, sidCookieName);
...
/**
 * Call a service that set the login and session cookies loginCookie, sidCookie
 * No need to pass cookies name because the server know the port to build the name when it sets the cookies in http response headers
 */
var _reconnectSetToken = function(loginCookie, sidCookie) {
	var deferred = new $.Deferred();
	loginCookie = loginCookie || "";
	sidCookie = sidCookie || "";
	log && log("_reconnectSetToken - Login: ", loginCookie, " - sid: ", sidCookie)
	ajax("GET", _config.svcReconnectSetToken, null, {
		"accept": "application/json",
		"x-reconnect-token": loginCookie,
		"x-reconnect-sid": sidCookie
	}).then(function(result) {
		log && log("_reconnectSetToken succeeded", loginCookie, sidCookie);
		deferred.resolve();
