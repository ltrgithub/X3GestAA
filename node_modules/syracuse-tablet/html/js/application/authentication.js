...
var _logout = function(forgetMe) {
	var deferred = $.Deferred();
	ajax("POST", globals.baseLocation().host + _config.svcLogout, {
		forgetMe: forgetMe === false ? false : true
	}).then(function(result) {
		var msg;
		// There's a bug in Syracuse which returns sometimes 401 on logout - Aurelien is aware
		// We are offline (status 0) we consider logout is ok and do not display a message
