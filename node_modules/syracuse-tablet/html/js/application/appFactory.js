...
var _createPage = function($parent, state) {
	...
	if (type == "html" && state.name == "login") {
		...
		var info = globals.getApplication().dao.getRegularPageInfo(state.name);
		res = new _ctor.RegularPage($parent, state, info.prototype, info.article, info.jsonInfo);
	} else {
...
var _createViewCtrlr = function(page, type) {
	...
	switch (type) {
		...
		case "dashboardpage":
		case "externallink":
			ctrlr = new _ctor.ViewCtrlr(page);
			break;
		default:
