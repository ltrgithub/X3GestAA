			...
			if (link.page) {
				...
				if (self.options.noRedirection === true) {
					...
					globals.getApplication().changePage(link.page, pageOpts, opts).then(function() {
						...
						var name = link.page ? link.page.split('.') : [];
						name = (name.length > 3 ? name.slice(3) : name).join('.');
						globals.getModal().actionResult(locale.text("error.page.loading", name ? [name] : null), e);
						deferred.resolve();
					});
				}
				return;
