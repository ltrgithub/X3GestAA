	...
	function(callerInterface) {
		...
		load: function(state, back) {
			...
			try {
				...
				if (pageToClose && state.options && state.options["linkarticle"]) {
					...
					var v = pageToClose.getControl(state.options["linkarticle"]);
					$forceLinkArticle = v && v.article ? v.article.$linkarticle : null;
				}
