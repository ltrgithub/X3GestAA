...
var _PageLoader = utils.defineClass(
	function PageLoader(callerInterface) {
		...
		destroy: function() {
			...
			this.factory = null;
			if (this._loadingContext) {
				if (this._loadingContext.pageToLoad) {
					this._loadingContext.pageToLoad.destroy();
				}
				this._loadingContext = null;
			}
		},
			...
			try {
				...
				self.factory.createPage(self.getRootElmt(), state, parentPage, $forceLinkArticle)
					.then(function(p) {
						...
						self._loadingContext.pageToLoad = pageToOpen = p;
						// Page created
						pageToOpen.isNew = true;
						// Context to restore			
						if (pageToOpen.gadget && pageToOpen.gadget.data("pinPageId") != null) {
							// Restores the context of a pinned page to preserve filters/sort...
							// The context has been read by the page in pinnedPageData
						} else if (!back && pageToOpen.isVignette && pageToClose == null) {
							...
							state.savedContext = state.savedContext;
						} else {
							// No restoration
							state.savedContext = null;
						}
						// If vignette was destroyed, no need to load further
						// In most cases the self.destroyed at the beginning of this
						// function will abort before this
						if (pageToOpen.isVignette &&
							pageToOpen.state.options.parentVignette &&
							pageToOpen.state.options.parentVignette.destroyed) {
							return;
						}
		...
		isLoading: function() {
			return this._loadingContext != null;
		},

		stopLoading: function(reason) {
			if (reason != "back" || this.isLoading() !== true) {
				return;
			}
			this.waitWheelStop();
			if (this._loadingContext) {
				...
				if (this._loadingContext.deferred) {
					this._loadingContext.deferred.resolve(null);
				}
				this._loadingContext = null;
			}
		},
