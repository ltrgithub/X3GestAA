...
var _PageLoader = utils.defineClass(
	function(callerInterface) {
		...
		_pageActivate: function(pageToOpen, cb, back, isNewPage) {
			var self = this;
			var pageToClose = self.getCurrentPage();
			...
			if (isNewPage) {
				...
				if (!back && pageToOpen.isVignette) {
					// Restore the context of a page nested in a vignette after it has been loaded
			...
			pageToOpen.activate(pageToClose, savedContext, function() {
				...
				self.getCurrentPage().isNew = isNewPage;
				if (cb) cb();
			});
		},
