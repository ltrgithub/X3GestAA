...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("settings", false);
...
function _readSettingsUser() {
	if (_settingsUser == null) {
		...
		return storage.read({
			...
			log && log(JSON.stringify(_settingsUser));
		});
	} else {
		return $.smResolve();
	}
...
/*
 * ======================================================
 * Settings belonging to a specific page
 * ======================================================
 */
exports.readUserPagePrefs = function(pageId) {
	...
	return storage.read({
		...
		log && log("read user page setting: " + pageId);
...
exports.writeUserPagePrefs = function(pageId, prefs) {
	...
	return exports.readUserPagePrefs(pageId)
		...
		.then(function(newPrefs) {
			log && log("write user page setting: " + pageId);
			log && log(JSON.stringify(newPrefs));
			return storage.put({
				$context: ctx,
				$collection: "$settings",
				$endpoint: "$local",
