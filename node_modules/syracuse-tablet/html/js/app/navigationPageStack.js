...
var NavigationPageStack = utils.defineClass(function NavigationPageStack($$elmt) {
	...
	isFirstPageHome: function() {
		if (this._pageStack.length > 0) {
			var stackItem = this._pageStack[0];
			return stackItem.pageType === "home";
		}
	},
	isEmpty: function() {
		return this.count() == 0;
	...
	goBackToId: function(id) {
		...
		function _closeTopPage(stackItem) {
			...
			var step;
			if (page) {
				step = page.onGotoUrlAction(true);
			} else {
				step = $.smResolve(true);
			}
			var closeAllowed;
				...
				getPage = modules.get("pageFactory").createPage(top.pageCreateData)
					.then(function(page) {
						...
						modules.get("modal")
							...
							.always(function() {
								modules.get("navHelper").gotoUrl("html://login")
							})
					})
			} else {
