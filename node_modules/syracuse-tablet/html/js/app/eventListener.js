...
exports.registerEvents = function($$container) {
	...
	// General click handler for not select tag
	$$container.on("click", "[data-nevent]:not(select):not(.disabled):not([disabled])", function(e) {
		_eventHandler(e, "click");
	});
	// General click handler for select tag
	$$container.on("change", "select[data-nevent]:not(.disabled):not([disabled])", function(e) {
		_eventHandler(e, "click");
	});
	...
	["blur", "focus", "change", "click", "input"].forEach(function(eventName) {
...
function _eventHandler(event, eventName) {
	...
	if (ctrl && controller) { // data related actions are only triggered if there was a control and controller found attached to the event
		...
		//disable click on edit homePage
		var page = ctrl.page.parentPage || ctrl.page;
		if (page.controller && page.controller.configActive && !$$target.attr("data-naction") && !ctrl.isPanel && (!ctrl.$type || (ctrl.$type && ctrl.$type.toLowerCase() !== "hubgroup"))) return;
...
exports.getDataActionAttrs = function(controllerId, action, params, asHtml) {
	...
	return asHtml ? uiutils.objectToHtmlAttrs(attrs) : attrs;
};
