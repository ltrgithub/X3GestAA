...
exports.MultiSelectionPanelController = utils.defineClass(
	...
	Base, {
		...
		_multiSelUpdateStatus: function() {
			this.panel.$$elmt.find('a.s-m-link:not([data-menuitem-id="multiSelToggle"])').toggleClass("disabled", this._nbSelected == 0);
			var $$menuActions = this.panel.$$elmt.find("#multiSelMenuActions");
			...
			this._multiSelAction("$download").toggle(this.panel.page.isDownloadEnabled() && this._nbSelected > 0);
		},
		/**
		 * Returns unselected records
		 */
		_multiSelUnselected: function() {
			return this.panel.page.$$contentElmt.find(".s-m-array.s-m-control > .s-m-value .s-m-record:not(.s-m-multi-selected)");
		},
		/**
		 * Returns selected records
		 */
		_multiSelSelected: function() {
			return this.panel.page.$$contentElmt.find(".s-m-array.s-m-control > .s-m-value .s-m-record.s-m-multi-selected");
		},
		/**
		 * Returns all the records
		 */
		_multiSelAll: function() {
			return this.panel.page.$$contentElmt.find(".s-m-array.s-m-control > .s-m-value .s-m-record");
		},
		/**
		...
		_multiSelAction: function() {
			var res = $();
			var args = arguments;
			this.panel.$$elmt.find('#multiSelMenuActions a[data-params]').each(function(id) {
		...
		_multiSelToggleUnselected: function(show) {
			...
			if (this.panel.page.$$contentElmt.find(".s-m-array.s-m-card").length > 0) {
				// Hide empty rows to remove the height of empty tr elements
				this.panel.page.$$contentElmt.find(".s-m-array.s-m-card tr").each(function() {
					var e = $(this);
				...
				this._array.notifyScrollerUpdate();
			}
		},
		/**
