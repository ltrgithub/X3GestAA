...
exports.MultiSelectionMainController = utils.defineClass(
	...
	Base, {
			...
			return $.smResolve().then(function() {
				...
				var _exec = function(idx) {
					if (idx >= info.data.length) {
						...
						var rowData = info.data[idx];
						self._processOne(info.link, rowData, idx).then(function(status) {
							if (status === "stop") {
		...
		_processOne: function(link, data, idx) {
			var step;
			if (link.$url === "$UnitTesturl") {
				step = this._procesUnitTestUrl(link, data, idx);
			} else {
				step = this._processUrl(link, data, idx)
			}
			var self = this;
		...
		/**
		 * Execute action link on data dtaSet
		 */
		_processUrl: function(link, data, idx) {
			return $.smResolve("success");
		},
		/**
		 * Used to test ui process.
		 * Don't send an url
		 */
		_procesUnitTestUrl: function(link, data, idx) {
			var deferred = $.Deferred();
			setTimeout(function() {
				if (idx % 2 == 0) {
