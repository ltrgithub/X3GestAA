...
exports.request = function(url, method, data, headers, options) {
	...
	$.ajax(ajaxData).always(function(jqXHR, textStatus, jqXHRSuccess) {
		...
		try {
			...
			if (status === 0) {
				result.offline = true;
				diags = helpers.createDiag(null, "error", locale.text("client.offline"));
			}
			// Not found
			else if (status === 404) {
				diags = helpers.createDiag(null, "error", locale.text("http404"), "url:" + url);
			}
			// not authorized
			else if (status === 401) {
				result.unauthenticated = true;
				diags = helpers.createDiag(result, "error", locale.text("user.not.authenticated"));
