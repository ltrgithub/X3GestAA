...
exports.request = function(url, method, data, headers, options) {
	...
	$.ajax(ajaxData).complete(function(jqXHR, textStatus) {
		var result;
		try {
			...
			if (status === 0) {
				result.offline = true;
				deferred.reject(helpers.createDiag(result, "error", "Client is offline")); // TODO: NLS
			}
			// not authorized
			else if (status === 401) {
				result.unauthenticated = true;
				deferred.reject(helpers.createDiag(result, "error", "User is not authenticated")); // TODO: NLS
				...
				if (result.data && result.data.$diagnoses) {
					...
					// ALWAYS REJECT WITH A DIAGNOSE
					diags = helpers.createDiag({
						success: result.success,
						status: result.status
					}, "error", "Error http " + status + " reading resource", "url:" + url + "\n" + jqXHR.responseText);
				}
