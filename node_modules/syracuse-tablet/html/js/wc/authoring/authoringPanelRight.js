...
exports.Panel = utils.defineClass(function AuthoringPanelRight() {
	...
	_ensurePanel: function() {
		...
		var ctx = {
			...
			label_closeCardDesign: locale.text("auth.closeCardDesign")
		};
	...
	_getDomObjects: function() {
		...
		self.$$panelControlBack = $("#s-m-auth-child-design-close-id");
	...
	_initDomEvents: function() {
		...
		self.$$panelControlBack.on("click", self._onBackClick.bind(self));
	},
	...
	_checkDetailNavigation: function(sel, data) {
		var self = this;
		var page = data && data.control && data.control.page;
		if (page && page.isCardDetail()) {
			self.$$panelControlBack.show();
		} else {
			self.$$panelControlBack.hide();
		}
		if (page && !page.isCardDetail() && data && data.control.$display === "card") {
	...
	_onBackClick: function() {
		notifications.publish("sm.auth.close.card.design");
	}
});