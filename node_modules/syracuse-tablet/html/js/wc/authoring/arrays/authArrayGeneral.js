...
var _properties = {
	...
	"arrayOptions": function(controller, sel, data) {
		...
		this.values = [];
		if (ctrl.$display === "card" && ctrl.displayAsCarousel) {
			...
			if (ctrl.$display === "table" && !ctrl.prototype.isSingleArray() && !ctrl.isArrayChart() && !controller.dataset.prototype.isQueryLikeFacet()) {
				this.values.push({
	...
	*/
	...
	if (data.control.$display === "separator") {
		...
		if (!sel.$$elmt.is(".s-m-control.s-m-chart")) {
			...
			if (!data.control.isArrayField() && data.control.$filtersGet().length > 1) {
