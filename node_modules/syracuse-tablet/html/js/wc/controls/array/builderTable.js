...
exports.BuilderTable = utils.defineClass(
	function builderTable(control) {
		...
		buildOnResize: function(data, $$parent, context) {
			if (!this.destroyed && this._tableColumnInfo && this._tableColumnInfo.length > 0) {
		...
		_buildTableRow: function($$body, recordDataset, index) {
			var rowCtrl = this.control.createRecordCtrl($$body, recordDataset, index);
			if (rowCtrl) {
		...
		savedCtxCreate: function(type) {
			...
			ctx.selectedRowUuid = this.control.$$content.find(".s-m-last-selected").attr("id");
			return ctx;
		},
