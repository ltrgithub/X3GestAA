	...
	function builderTable(control) {
		...
		_buildTableRow: function($$body, recordDataset, index) {
			...
			if (this.controller.root.isLookupController) {
				rowLinkDomAttrs = this.control.getDataActionAttrs("lookupAccept");
			} else {
				var $linkName = this.control.getAuthoring("$detailsLink");
				if ($linkName == null) {
					$linkName = this.control.page.options.isVignette ? "$queryfullpage" : "$details";
				}
				var $links = this.$itemProto.data("$links");
				if ($linkName == "$queryfullpage" || ($links && $links[$linkName])) {
					rowLinkDomAttrs = this.control.getSdataLinkAttrs($linkName);
				} else {
