	...
	function builderTable(control) {
		...
		buildHtml: function(data, $$parent) {
			...
			if (this.prototype.hasCapability("append")) {
				this.$$parent.append($('<a class="btn btn-primary" href="#" data-nevent data-naction="addRow">Add row</a>'));
			}
		},

		_buildTable: function(data, $$parent) {
		...
		_buildTableHead: function($$table) {
			...
			this._tableColumnInfo.forEach(function(col) {
				var th = uiUtils.createDomElement('th', col.cssHeader || col.css, col.title || col.$bind, null, tr);
		...
		_buildTableRow: function($$body, recordDataset, index) {
			...
			if (environment.isAutoUITestMode()) {
				environment.getUnitTestMgr().arrayAddRowIndex(rowCtrl, index);
			}
		},
