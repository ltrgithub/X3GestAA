	...
	function builderCard(control) {
		...
		buildCards: function(data, $$container, context) {
			...
			if (dirArray === "h") {
				...
				this.buildCardV(visibleTds, bs, parentH, parentW);
			}
		...
		buildCardV: function(visibleTds, bs, parentW) {
			...
			visibleTds.some(function(o, idx) {
				...
				if (idx === 0) {
					var padding = Math.round((o.$$card.outerWidth() - o.$$card.width()) / 2);
					realW = o.$$card.width();
					var minWidth = Math.min(realW, self.authMinSize2Pixels(self.article.$minWidth));
					while (nbCardPerRow > 1) {
						var cellW = Math.round(((realW - ((nbCardPerRow + 1) * bs)) / nbCardPerRow) - padding);
						if (cellW >= minWidth) {
							realW = cellW
							break;
						}
