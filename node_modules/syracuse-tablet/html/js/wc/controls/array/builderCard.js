...
exports.BuilderCard = utils.defineClass(
	function builderCard(control) {
		...
		buildCards: function(data, $$container, context) {
			...
			if (!this._$$body) {
				...
				data.forEach(function(rowData, idx) {
					self._tds.push({
						"$$td": $('<td s-m-record><section class="s-m-card" style="background-color:#DDDDDD;width:100%;height:100%"/></td>'),
						rowData: rowData
					});
				});
			}
			...
			context.cardAuthoring = {
				$emptycell: this.article.$emptycell || "nodisplay",
				$minWidth: this.authMinSize2Pixels(this.article.$minWidth),
				$cardsPerRow: parseInt(this.article.$cardsPerRow, 10) || 2
			};
			...
			if (dirArray === "h") {
				this.buildCardH(visibleTds, bs, parentH, parentW, context);
			} else {
				this.buildCardV(visibleTds, bs, parentH, parentW, context);
			}
		...
		createCardContent: function($$card, idx, rowData, buildOpts) {
