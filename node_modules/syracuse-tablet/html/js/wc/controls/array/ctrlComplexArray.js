...
exports.CtrlComplexArray = utils.defineClass(
	function CtrlComplexArray(controller, article, prototype, options) {
		...
		_checkAuthoring: function() {
			...
			if (isChartDetail || page.options.isVignette === true) {
		...
		createTableRowCtrl: function($$container, recordDataset, index) {
			...
			if (this.controller.root.isLookupController) {
				...
				if ($linkName == null) {
					$linkName = this.page.options.isVignette ? "$queryfullpage" : "$details";
				}
		...
		removeRow: function(rowCtrl, rowUuid) {
			var self = this;
		...
		$filtersGetAuthoring: function() {
			var auth = this.getAuthoring("$filters", this.page.options.isVignette ? "none" : "list");
		...
		notifyVignetteArrayHosted: function() {
			if (this.isArrayField() || this.prototype.isLookup() || !this.page.options.isVignette || !this.page.parentVignette) {
				return;
			}
