...
var RowDataset = require('syracuse-tablet/html/js/wc/sdata/dataset').RowDataset;
...
var CtrlArrayCardCell = require('syracuse-tablet/html/js/wc/controls/array/ctrlArrayCardCell').CtrlArrayCardCell;
...
exports.CtrlComplexArray = utils.defineClass(
	function CtrlComplexArray(controller, article, prototype, options) {
		...
		this.filterSearch = new SearchArray(this);
		this.filterX3 = new FiltersSort.Filters(this); // Predefined x3 filters
		...
		_newBuilderHeader: function() {
			...
			// $filters
			headerOpts.$filtersAuthoring = this.filterX3 && this.filterX3.$filtersAllowed() ? this.$filtersGetAuthoring() : "none";
		...
		_actActionArray: function(id, param) {
			switch (id) {
				case "panelfiltersort":
					...
					break;
				case "filterSelected":
					this.filterX3.filtersSelect(param);
					break;
				case "search":
		...
		getFilterSortOptions: function() {
			var opt = {
				...
				"$filters": this.filterX3.$filterGetInfo(),
		...
		$filtersGet: function() {
			return (this.filterX3 && this.filterX3.$filterGetInfo()) || [];
		},
		/**
		...
		authOnSlideMinSize: function(size, action) {
			...
			if (this.$display != "card" || !this._arrayData || this._arrayData.length == 0) {
				return;
			}
			var scale = this.getTransformScale();
			...
			value += "px";
			if (action == "start") {
				var height = this.$$content.find(this.$$content.find("#rowid-" + this._arrayData[0].getValue("$uuid"))).closest(".s-m-record").height();
				height = height
				this.$$content.css({
					opacity: "0.3"
				});
		...
		getCardDetailPageData: function() {
			var dataset;
			var num = this.arrayDataset.getNumRows();
			if (num > 0) {
				dataset = this.arrayDataset.getRowByIndex(0);
			} else {
				dataset = this.getDummyRowDataset();
			}
			return {
		...
		getDummyRowDataset: function() {
			var prototype = this.arrayDataset.rowPrototype.clone();
			return new RowDataset({}, prototype, null, "$dummy");
		},
		/**
		...
		getNbRecords: function() {
			return this._arrayData ? this._arrayData.length : 0;
		},
		/**
		...
		multiSelGetSelectedData: function() {
			...
			this.$$content.find(".s-m-record.s-m-multi-selected").each(function(idx, record) {
				...
				var rowId = record.attr("id").split("rowid-");
				if (rowId.length > 0) {
					rowId = rowId[1];
					var rowdata = self.arrayDataset.getRowByUuid(rowId);
					if (rowdata) {
						data.push({
							rowData: rowdata,
							rowId: rowId,
		...
		multiSelAddDiagnoses: function(data) {
			...
			if (data.success !== true) {
				var $$root = null;
				var colSpan;
				var isTable = this.$display = "table";
				if (isTable) {
					colSpan = data.$$record.children('td').length;
				} else {
					$$root = $('<section class="s-m-multisel-diag" data-nevent data-naction="multiSelTriggerAction" data-params="showdiagnoses"/>').prependTo(data.$$record);
					$$root.width(data.$$record.width());
					$$root.smData('diagnoses', data.$diagnoses);
				}
				data.$diagnoses.forEach(function(d, idx) {
					var $$diag = $('<div data-idx="' + idx + '" class="alert alert-' + (d.$severity === "error" ? "danger" : "success") + '" role="alert">' + d.$message + '</div>');
					if (isTable) {
			...
			this.builderContent.makeVisible(data.$$record)
		},
		/**
		 * Show the diagnose attached to  diagnose and check the row/card as error
		 */
		multiSelShowDiagnoses: function() {
			if (!eventListener.currentEvent) {
				return;
			}
			var $$elmt = $(eventListener.currentEvent.target).closest(".s-m-multisel-diag");
