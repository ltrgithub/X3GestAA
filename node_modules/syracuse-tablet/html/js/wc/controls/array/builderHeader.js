...
exports.BuilderHeader = utils.defineClass(
	function builderHeader(control, headerOpts) {
		...
		this.options.showFiltersTabs = this.options.$filtersAuthoring === "tabs";
		...
		buildHtml: function(arrayData, refresh) {
			...
			var showSearch = this.options.showSearch && (arrayData.length > 0 || this.control._search.hasSearchCriteria());
			...
			if (this.options.showFiltersList || this.options.showFiltersTabs || this.options.showSearch || this.options.showSortFilter || this.options.showPagination) {
		...
		update: function(showSearch) {
			...
			if (this.control.filterSort) {
				this.control.filterSort.filterSortUpdate(this.$$elmt);
			}
