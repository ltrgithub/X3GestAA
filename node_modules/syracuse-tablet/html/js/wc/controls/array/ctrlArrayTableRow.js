...
exports.CtrlArrayTableRow = utils.defineClass(
	function CtrlArrayTableRow(rowDataset, arrayController, article, prototype, options) {
		// first
		this.rowController = new SdataController(rowDataset, arrayController);
		// Second
		...
		buildHtml: function(columnsInfo) {
			...
			this.$$elmt.attr({
				"data-controller-id": this.rowController.id
			});
			var self = this;
			columnsInfo.forEach(function(col, idx) {
				if (col.isProtoItem) {
					var ctrlCell = ctrlFactory.createControl(self.rowController, col.article,
						self,
						self.page, {
							displayCtx: "table",
		...
		_actLookupAccept: function() {
			...
			var parentController = this.rowController.findController(lookup.controllerId);
			var value = this.rowController.getValue(lookup.$lookupKeyName);
		...
		onSdataLinkClicked: function($linkName, $linkBind) {
			this.setRowSelected();
			this.rowController.onSdataLinkClicked($linkName, $linkBind);
		},

		_actRowDetail: function() {
			this.setRowSelected();
			this.rowController.gotoUrl("child://SdataRowDetail", null, {
				isChild: true,
				childContext: {
					controllerId: this.rowController.id
				}
			});
		},
		...
		getUuid: function() {
			return this.rowController.dataset.json.$uuid;
		},
