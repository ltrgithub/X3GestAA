...
exports.SearchArray = utils.defineClass(
	...
	function searchArray(control) {
		...
		getHtml: function() {
			if (this.firstRendering === true) {
				// Restore the search keywords - Done at build stage
				this.firstRendering = false;
				var ctx = this.control.savedCtxGetSearch();
				this.searchValue = ctx ? ctx.searchValue || "" : "";
				this.searchInfos = this.searchValue ? ctx.searchInfos || [] : [];
			}
			var self = this;
		...
		removeSearch: function(ctrl) {
			if (this.searchValue !== "") {
				this.searchValue = "";
				this.searchInfos = [];
				var $url = filterHelper.filterSortUpdateUrl(ctrl.getFilterSortOptions());
				ctrl.notifyApplyFilter($url)
			}
		...
		hasSearchCriteria: function() {
			return this.searchInfos && this.searchInfos.length > 0;
		},
		search: function(ctrl) {
			var $$value = $(":input", this.$$search);
			if (this.searchValue !== $$value.val()) {
				this.searchValue = $$value.val();
				this.searchInfos = this.searchValue.length > 0 ? this._filterInfosGet(ctrl) : [];
				var ctx = {
		...
		_filterInfosGet: function(ctrl) {
			var columnInfo = ctrl.getColumnsInfo(true);
			var proto = ctrl.prototype.getProto$Item();
		...
		savedCtxRestore: function(ctx) {
			// No need because it's done at the creation of the array - Not the regular process	
		}
	});