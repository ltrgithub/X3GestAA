	...
	function builderBase(control) {
		...
		savedCtxCreate: function(type) {
			var res = {
				...
				selectedRowUuid: this.control.$$content.find(".s-m-last-selected").attr("id")
			};
			if (type === "link" || !this._gestureMgr) {
				// if send mail or pinnedpage (link) we don't restore _gestureMgr and _lastSelectedRow
				return null;
			}
			res.gestureMgr = this._gestureMgr ? this._gestureMgr.savedCtxCreate(type) : null;
			return res;
		},
		savedCtxRestore: function(ctx) {
			if (ctx.selectedRowUuid) {
				this.control.$$content.find('#' + ctx.selectedRowUuid).addClass("s-m-last-selected");
			}
