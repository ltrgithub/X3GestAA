...
var _templates = {
	layout: ' \
		<div class="s-m-img-layout">\
			<section>\
				<a class="img-responsive s-m-field-img ctrl-evt-click" href="#" draggable="false">\
				</a>\
			</section>\
			{{#if actions}}\
			<footer> \
				{{#each actions}}\
					{{#if active}}\
						<a class="ctrl-evt-click s-m-meta"\
							href = "#"\
							draggable="false"\
								data-nevent=""\
								data-naction="{{id}}"\
...
exports.CtrlImage = utils.defineClass(
	...
	function CtrlImage(controller, article, prototype, options) {
		...
		_isBtnActive: function(btnId) {
			var isSignature = false //this.article.$isSignature ? true : false;
			var isActive = false;
			switch (btnId) {
				case "$select":
		 ...
		 */
		/*
		 * TODO : check the image size (nb Ko) because we can crash X3 engine when the file is too big
		 */
