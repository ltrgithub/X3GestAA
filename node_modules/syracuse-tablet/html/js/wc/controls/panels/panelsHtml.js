			...
			<div id="{{id}}" class="btn-group s-m-link">\
				<div class="{{dropDirection}}">\
					...
					<ul class="dropdown-menu dropdown-menu-{{dropPosition}}" role="menu">\
						{{#each links}}\
							<li><a  {{#if style}}style="{{style}}"{{/if}} draggable="false" data-nevent data-naction="{{action}}" data-params="{{params}}" href="#">{{title}}</a></li>\
						{{/each}}\
					</ul>\
				</div>\
			</div>'
...
var _getMultiSelMenuActions = function(panelName) {
...
exports.getItemHtml = function(panelName, item) {
	...
	if (item.action) {
		if (item.action === "multiSelActions") {
			return _getMultiSelMenuActions(panelName);
