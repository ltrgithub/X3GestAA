...
exports.SdataController = utils.defineClass(
	...
	Base, {
		...
		onSdataLinkClicked: function($linkName) {
			this.triggerLink($linkName).fail(function(result) {
				modules.get("modal").error(result);
			});
		},
		/**
		...
		onSdataActionClicked: function(actionName) {
			...
			if (this.uiAdapter && this.uiAdapter.beforeOnSdataActionClicked) {
				...
				first = $.smResolve(true);
			}
			var self = this;
			first.then(function(goAhead) {
				if (goAhead) {
					return self.triggerAction(actionName).then(function(result) {
						if (self.uiAdapter && self.uiAdapter.afterOnSdataActionClicked) {
							return self.uiAdapter.afterOnSdataActionClicked(actionName, self, result);
						}
						return result;
					})
				} else {
		...
		getShareContext: function() {
			var self = this;
			var settings = {
				opt: self._nativeCapabilities || {},
				ctx: {
					pageTitle: self.uiAdapter.getPageTitle()
				},
				vignettes: []
			};
			Object.keys(self._controlsById).forEach(function(key) {
				if (self._controlsById[key].isVignette()) {
					settings.vignettes.push({
						id: self._controlsById[key].id,
						opt: self._controlsById[key].getNativeCapabilities(),
						ctx: {
							pageTitle: self._controlsById[key].getTitle()
						}
					})
				}
			});
		 ...
		 */
		share: function(action, data) {
			var result;
			switch (action) {
				case "link":
					result = this._sharePageLink();
					break;
				case "liveTile":
					result = this._shareAddLiveTile(data);
					break;
				case "schedule":
					result = this._shareScheduleMessage(data);
					break;
			}
			if (result) {
				result.fail(function(error) {
					window.smNativeLogger && window.smNativeLogger.log("notifPageShare Error", error.message, error.stack)
		...
		_sharePageLink: function() {
			var page = this.uiAdapter;
			var options = {}
			options.DeepLink = this._getDeepLink(page);
			options.Title = this.dataset.prototype.getValueByPath("$title");
			options.Description = this.dataset.prototype.getValueByPath("$description");
		...
		_shareAddLiveTile: function(data) {
			...
			/*options.description
			options.bind*/
			var page = data.vignette ? this._controlsById[data.vignette].getTopPage() : this.uiAdapter;
			options.id = page.pageData.$originalUrl;
			options.title = data.title;
			options.backgroundColor = data.backgroundColor;
			options.sDataUrl = page.pageData.$url;
			options.DeepLink = this._getDeepLink(page);
			options.userLogin = this._getUserLogin()
			liveTileModule.addLiveTile(options).then(function(result) {
		...
		_shareScheduleMessage: function(data) {
			...
			data.DeepLink = this._getDeepLink(this.uiAdapter);
			scheduleMessageModule.scheduledMsg(data);
		},
		_getDeepLink: function(page) {
			var deepLink = {}
			var url = page.pageData && page.pageData.$url;
			var method = page.pageData && page.pageData.$method || "GET";
			...
			deepLink.Title = page.getPageTitle();
			deepLink.Url = href;
			return deepLink
		},
