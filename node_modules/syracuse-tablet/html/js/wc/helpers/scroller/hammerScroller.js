...
var _Klass = utils.defineClass(function HammerScroller($$elmt, options, parentMgr) {
	...
	init: function(viewRect, dir, scrollMax, preserveScroll) {
		if (viewRect && modules.get("siteLayout").getTransformScale() != 1) {
			// Developement mode - scaling 
			viewRect = viewRect.applyTransformScale(modules.get("siteLayout").getTransformScale(), true);
		}
	...
	makeVisible: function($$e) {
		if (!$$e || !this.isInitialized() || this.isVisible($$e)) return;
		if (this.slowingDown) {
			this._stopSlowingDown();
			// Last click is avoided - The click evt that is always triggered after press a event
			this.cancelClick = true;
		...
		*/
		...
		var o = $$e.offset();
		var offset = modules.get("siteLayout").getTransformScale() * $$e[size]() / 2;
	...
	getScrollMin: function() {
		...
		if (false && modules.get("siteLayout").getTransformScale() != 1) {
			// disabled because doesn't work fine - Like we scaled viewRect we should scale x 
			x = x * modules.get("siteLayout").getTransformScale();
		}
		// 30 pixel more
	...
	isVisible: function($$e) {
		var elementRect = $$e.get(0).getBoundingClientRect();
		var viewportRect = this.$$elmt.parent().get(0).getBoundingClientRect();
		return (
