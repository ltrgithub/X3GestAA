...
var _Klass = utils.defineClass(function HammerScroller($$elmt, options, parentMgr) {
	...
	makeVisible: function($$e, smoothOffset) {
		...
		if (!$$e || !this.isInitialized()) return;
		if (this.slowingDown) {
			this._stopSlowingDown();
			// Last click is avoided - The click evt that is always triggered after press a event
			this.cancelClick = true;
		...
		*/
		...
		var o = $$e.offset();
		var offset = smooth ? smoothOffset : $$e[size]() / 2;
		if (scale != 0 && scale != 1) {
			// !! JQuery 3.11 returns offset by taking into account the scale transform
			// !! We just cancel this transform and it works fine
		...
		var stopPos = smooth ? this.viewRect[prop] + this.viewRect[size] - $$e[size]() : this.viewRect[prop] + this.viewRect[size] / 2;
