...
var bigint = require("syracuse-tablet/html/js/wc/helpers/types/bigint");
...
exports.validate = function(value, constraints, errors) {
	if (value == null) {
		if (constraints.$isMandatory) return errors.push(locale.text("decimal.valMan"));
		if (!constraints.$isNullable) return errors.push(locale.text("decimal.valNull"));
		return;
	}
	if (!(value instanceof BigDecimal)) {
		errors.push(locale.text("decimal.valDec"));
		return;
	}
	if (constraints.$minimum != null) {
		if (constraints.$minimumCanEqual) {
			if (value.compare.compareTo(_make(constraints.$minimum)) < 0) errors.push(locale.text("decimal.valGe", constraints.$minimum));
		} else {
			if (value.compare.compareTo(_make(constraints.$minimum)) <= 0) errors.push(locale.text("decimal.valGt", constraints.$minimum));
		}
	}
	if (constraints.$maximum != null) {
		if (constraints.$maximumCanEqual) {
			if (value.compare.compareTo(_make(constraints.$maximum)) > 0) errors.push(locale.text("decimal.valLe", constraints.$maximum));
		} else {
			if (value.compare.compareTo(_make(constraints.$maximum)) >= 0) errors.push(locale.text("decimal.valLt", constraints.$maximum));
		}
	}
};
...
utils.defineClass(BigDecimal, null, {
	_zero: function(size) {
