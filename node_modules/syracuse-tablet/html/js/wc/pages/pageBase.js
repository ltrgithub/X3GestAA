...
exports.Page = utils.defineClass(
	...
	null, {
		...
		/**
		 * wasCached: 	True means the page was cached and as just to be made visible (no compute)
		 * 				Back from a child page
		 */
		show: function(wasCached) {
			if (!this.$$elmt) {
				return $.smResolve();
			}
			this.$$elmt.show();
			...
			return $.smResolve().then(function() {
				 ...
				 */
				if (wasCached != true && !self.isVignette()) {
					self.computeLayout();
				}
				/**
		...
		savedCtxCreate: function(type) {
			...
			var ctx = {
				type: type,
				pageName: this.pageName,
				ctxCtrls: this.controller.savedCtxCreate(type),

			};
			if (type !== "link" && this._gestureMgr) {
				// Save page scrolling if is not a link (by email, pinned page...)
				ctx.scroll = this._gestureMgr.savedCtxCreate(type);
			}
			return ctx;
		},
		savedCtxCtrlRead: function(indexCtrl) {
			if (this.options.savedCtx && this.options.savedCtx.ctxCtrls) {
				return this.options.savedCtx.ctxCtrls[indexCtrl];
			}
			return null;
		},
