...
exports.createPage = function(pageCreateData) {
	...
	pageCreateData.name = _getPageName(url);

	if (options.isVignette && options.$vignetteArticle) {
		// Force vignette's article
		fetchOptions.article = $.extend(true, null, options.$vignetteArticle);
	} else if (options.$mockArticle) {
		// used by mockup
		fetchOptions.article = $.extend(true, null, options.$mockArticle);
	}
	return $.smResolve()
		.then(function() {
			if (url.indexOf("/mobile2/") > -1) {
				return _fetchSDataPageData(url, fetchOptions, method)
					.then(function(pageData) {
						pageData.$url = url;
						pageData.$method = method;

						var page = _createSdataPage(pageData, options);
						if (pageData.name) {
							pageCreateData.name = pageData.name;
						} else {
 ...
 * 
 */
function _fetchSDataPageData(url, options, method) {
	var pageSettings;
	return $.smResolve()
		.then(function() {
			return settings.readUserPagePrefs(url)
		})
 ...
 * 
 */
function _createSdataPage(pageData, options) {
...
function _createChildPage(pageName, options) {
	return $.smResolve()
		.then(function() {
			...
			var pageData = {}; // Page data will be created by child page since it's usually just pointing to the parent page
			options = options || {};
...
function _getPageName(url) {
	...
	if (urlLC.indexOf("/mobiledashboard") >= 0) {
		return protoHelpers.getKeyFromUrl(url);
	} else if (urlLC.indexOf("html://") === 0 || urlLC.indexOf("child://") === 0) {
		return url.split('/')[2];
	} else {
		var rep = protoHelpers.getReprFromUrl(url);
		return rep || "unknown";
	}
}