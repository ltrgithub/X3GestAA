...
exports.Page = utils.defineClass(
	...
	Base, {
		...
		onVignetteLoaded: function(vignette, success) {
			...
			if (vignette.getTopPage()) {
				// Set the saved context for nested page (scroll/last selected row...)
				vignette.getTopPage().options.savedCtx = this.getSavedCtxVignette(vignette);
			}
			...
			if (this._vignettes && this._nbVignettesLoaded >= this._vignettes.length) {
				...
				setTimeout(function() {
					...
					self._vignettes.forEach(function(v) {
						v.afterAllVignettesLoaded();
					})
					self.onContentChanged();
					notifications.publish(["sm.all.vignettes.loaded"], self);
				})
			}
		},
		/*
		...
		savedCtxCreate: function(type) {
			...
			ctx.vignettes = [];
			var p;
			this._vignettes && this._vignettes.forEach(function(v) {
				ctx.vignettes.push(v.getTopPage() ? v.getTopPage().savedCtxCreate(type) : null);
			});
		...
		getSavedCtxVignette: function(v) {
			...
			var idx = this._vignettes.indexOf(v);
