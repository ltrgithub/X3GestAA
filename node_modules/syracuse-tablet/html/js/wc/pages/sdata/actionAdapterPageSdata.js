 ...
 */
var _skipExlude = ["multiSelection"];
/**
 * 
 */
exports.ActionAdapterPageSdata = utils.defineClass(
	...
	ActionAdapter, {
		 ...
		 */
		_initMultiSelItems: function(items, exclude) {
			...
			actions.forEach(function(act) {
				if (act.options && act.options.multiselection === true) {
					self._addSdataLinkItem(self._items.multiSelection, act.name, self.sdataPage.controller, act)
				}
			})
		},
		 ...
		 * Returns items to be shown in the footer panel
		 */
		getMultiSelectionItems: function() {

			if (this._items.multiSelection && this._items.multiSelection.length > 0) {
				this._addSdataActionItem(this._items.multiSelection, "multiSelection", this.sdataPage.controller, "right");
			}
			return this.getItems("multiSelection");
		},
		...
		_addFooterHeaderActions: function(items, slot) {
			...
			if (this._items.multiSelection.length > 0) {
				this._addSdataActionItem(items, "multiSelection", controller, {
					slot: slot
				});
			}
		},
		...
		_addSdataLinkItem: function(items, $linkName, controller, overrides) {
			...
			this._pushItem(items, $.extend(true, {}, item, overrides));
		},
		...
		_addSdataActionItem: function(items, $actionName, controller, overrides) {
			if (_skipExlude.indexOf($actionName) < 0 && this._isExcluded($actionName)) {
				return;
			}
			var dataset = controller.dataset;
			var title;
			...
			var item = {
				...
				"$action": $actionName,
				"icon": $actionName,
				"title": dataset.resolveExpression(title)
			}
			this._pushItem(items, $.extend(true, {}, item, overrides));
		},
		_pushItem: function(items, item) {
			items.push(item);
			if (!this._items.excluded) {
				this._items.excluded = [];
			}
			this._items.excluded.push(item.name);
		},
		_isExcluded: function(name) {
			return this._items.excluded != null && this._items.excluded.indexOf(name) >= 0;
		}
	});