...
exports.AttachedControlsMgr = utils.defineClass(
	function AttachedControlsMgr(page) {
		this.page = page;
		this.controls = [];
		this.visibleControlsByName = {};
		this.hiddenControlsByName = {};
	},
	null, {
		...
		createControl: function(info, options, hidden) {
			...
			if (ctrl) {
				...
				if (hidden === true) {
					self.hiddenControlsByName[info.name] = c;
				} else {
					self.visibleControlsByName[info.name] = c;
				}
			}
			return ctrl;
		},
		...
		findByName: function(name) {
			return (this.visibleControlsByName[name] && this.visibleControlsByName[name].control) || (this.hiddenControlsByName[name] && this.hiddenControlsByName[name].control);
		},
		buildHtml: function() {
			var self = this;
		...
		toggleControls: function(name1, name2, show) {
			var wasVisible = this.visibleControlsByName[name1] != null;
			...
			ctrls.forEach(function(i) {
				if (i.show) {
					var ctrl = self.hiddenControlsByName[i.name];
					if (ctrl) {
						self.visibleControlsByName[i.name] = ctrl;
						self.hiddenControlsByName[i.name] = null;
						ctrl.control.$$elmt.show();
						ctrl.control.computeLayout();
					}
				} else {
					var ctrl = self.visibleControlsByName[i.name];
					if (ctrl) {
						self.hiddenControlsByName[i.name] = ctrl;
						self.visibleControlsByName[i.name] = null;
						ctrl.control.$$elmt.hide();
					}
				}
			})
			return show;
		...
		isVisible: function(nameOrCtrl) {
			if (!nameOrCtrl || !this.visibleControlsByName) {
				return false;
			}
			if (typeof nameOrCtrl === "string") {
				return this.visibleControlsByName[nameOrCtrl] != null;
			}
			for (var name in this.visibleControlsByName) {
				if (this.visibleControlsByName[name] && this.visibleControlsByName[name].control === nameOrCtrl) {
					return true
				}
			}
			return false;
