...
exports.AttachedControlsMgr = utils.defineClass(
	...
	null, {
		...
		createControl: function(info, options, hidden) {
			...
			if (self.controlsByName[info.name]) {
			...
			if (ctrl) {
				...
				if (hidden === true) {
					...
					self.controlsByName[info.name] = c;
				}
			}
			return ctrl;
		},
		...
		findByName: function(name) {
			return (this.controlsByName[name] && this.controlsByName[name].control) || (this.hiddenControlsByName[name] && this.hiddenControlsByName[name].control);
		},
		buildHtml: function() {
			var self = this;
		 ...
		 * Toggle the control name2 to the reverse status od name1
		 */
		toggleControls: function(name1, name2, show) {
			var wasVisible = this.controlsByName[name1] != null;
			...
			ctrls.forEach(function(i) {
				if (i.show) {
					var ctrl = self.controlsByName[i.name] = self.hiddenControlsByName[i.name];
					self.hiddenControlsByName[i.name] = null;
					ctrl.control.$$elmt.show();
					ctrl.control.computeLayout();
				} else {
					var ctrl = self.hiddenControlsByName[i.name] = self.controlsByName[i.name];
					self.controlsByName[i.name] = null;
