...
var sdataRequesterLocal = require("syracuse-tablet/html/js/wc/sdata/sdataRequesterLocal");
var sdataRequesterOffline = require("syracuse-tablet/html/js/wc/sdata/sdataRequesterOffline");
...
var log = require('syracuse-tablet/html/js/wc/helpers/logger').getLogger("SDataRequester", true);
...
exports.fetchPageData = function($link, options) {
	...
	return _getPrototype(protoHelpers.getPrototypeUrl($url), options)
		.then(function(proto) {
			prototypeJson = proto;
			return _getData($url, $method, prototypeJson, options);
		})
		.then(function(data) {
			datasetJson = data;
			return _getArticle($url, $method, prototypeJson, datasetJson, options);
		})
		.then(function(article) {
			articleJson = article;
			return protoHelpers.checkTransform($url, prototypeJson, datasetJson, articleJson);
		})
		.then(function(pageDataTransformed) {
			...
			pageData.article = pageDataTransformed.articleJson;
			...
			if (options.useMetaFromCache === true) {
				...
				sdataRequesterOffline.getPrototype($prototypeUrl)
					...
					.fail(function(resultOffline) {
						// If offline fails, we use a client side test application
						// so this is a fallback for tesing only and will never be used in real life
						ajax.request($prototypeUrl)
							.then(function(result) {
								fetchProto.resolve(result.data);
							})
							.fail(function(result) {
								fetchProto.reject(resultOffline);
							});
					});
			} else {
				ajax.request($prototypeUrl)
...
function _getArticle($url, $method, prototypeJson, datasetJson, options) {
	if (options && options.article) {
		log && log("Skip reading article");
		return $.smResolve(options.article);
	}
				...
				sdataRequesterOffline.getArticle($url, datasetJson)
					...
					.fail(function(resultOffline) {
						...
						protoHelpers.getPageUrl($url, datasetJson)
							...
							.fail(function(result) {
								fetchPage.reject(resultOffline);
							})
							.then(function(result) {
								fetchPage.resolve(result.data.$article);
							});
					});
			} else {
				protoHelpers.getPageUrl($url, datasetJson)
	...
	return fetchPage.then(function($article) {
		if (!$article) {
...
function _getStaticArticle($url, $method, prototypeJson, datasetJson) {
	if (prototypeJson.$representation === "mobileApplication") {
		...
		// Instead, the home dashboard of the mobile application is rendered
		// This is why we create a fake article for the mobile application (which will not be used) and 
		// embed the dashboard's article which is alway included by the server in the dashboard payload
		var $article = datasetJson.$homeDashboard.$article;
		return $.smResolve({
...
exports.triggerAction = function($actionName, dataset, workingCopy) {
	return $.smResolve()
		...
		.then(function($action) {
			...
			// TODO: Check errors here: null action data means nothing to do right now, e.g. $abort in stateless mode
			if (actionData) {
