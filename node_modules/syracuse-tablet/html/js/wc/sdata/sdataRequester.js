...
var log = require('syracuse-tablet/html/js/wc/helpers/logger').getLogger("SDataRequester", false);
...
exports.fetchPageData = function($link, options) {
	...
	return _getPrototype(protoHelpers.getPrototypeUrl($url), options)
		.then(function(proto) {
			prototypeJson = proto;
			console.log("prototypeJson", prototypeJson)
			return _getData($url, $method, prototypeJson, options);
		})
		.then(function(data) {
			if (options.isHome) {
				return _getDefaultDataHome()
					.then(function(dataHome) {
						datasetJson = dataHome
						return _appendTestHomeDataset(datasetJson)
					})
					.then(function(dataset) {
						datasetJson = dataset
						return _getHomePage(data.$resources, datasetJson);
					})
			} else {
				datasetJson = data;
...
function _appendTestHomeDataset(dataset) {
	return sdataRequesterLocal.fetchDataset('HOME')
		.then(function(datasetLocal) {
			if (datasetLocal.data.vignettes) {
				datasetLocal.data.vignettes.forEach(function(vi) {
					vi.local = true
					dataset.vignettes.push(vi)
				})
				dataset.$mobileGadgets = $.extend({}, dataset.$mobileGadgets, datasetLocal.data.$mobileGadgets);
			}
			return dataset
		})
}

function _getDefaultDataHome() {
	return metaDataCache.getApplicationsList().then(function(applist) {
		return protoHelpers.checkTransformDataset(applist);
	});
}

function _getHomePage($resources, datasetJson) {
	return {
		$article: $resources.content ? $resources.content.$article : protoHelpers.defaultHomeArticle(datasetJson),
