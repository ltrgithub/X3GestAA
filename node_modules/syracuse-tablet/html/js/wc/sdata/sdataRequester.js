...
exports.fetchPageHomeData = function(url, options) {
	...
	return _getPrototype(protoHelpers.getPrototypeUrl($url), options)
		.then(function(proto) {
			prototypeJson = proto;
			return _getData($url, "GET", prototypeJson, null, options);
		})
		.then(function(data) {
		...
		.then(function(pageDataTransformed) {
			if (!$resources.content) exports.savePersonalHomeDashboard({
...
function _appendTestHomeDataset(dataset) {
	...
	return sdataRequesterLocal.fetchDataset('MOCK')
		.then(function(datasetLocal) {
			if (datasetLocal.data.vignettes) {
				datasetLocal.data.vignettes.forEach(function(vi) {
					vi.local = true // identify local : avoid to store them when saving the dashboard
...
function _getDefaultDataHome(content) {
	return metaDataCache.getApplicationsList().then(function(applist) {
		...
		return protoHelpers.checkHomeDataset(applist, content);
	});
}
...
function _getHomePage($resources, datasetJson) {
	...
	return {
		$article: protoHelpers.homeArticle(article, datasetJson),
...
exports.savePersonalHomeDashboard = function(pageData) {
	var opts = protoHelpers.getSaveHomeDashboardOpts(pageData);
