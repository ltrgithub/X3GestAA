 ...
 */
exports.fetchUserWelcomePageData = function() {
	var up = modules.get("appController").App.getCurrentUserProfile();
	return exports.fetchWelcomePageData(up.user.$uuid, up.selectedRole.$uuid, true);
}

/**
 * 
 */
exports.fetchWelcomePageData = function(userUID, roleUID, isPersonal) {
	var prototypeJson;
	var datasetJson;
	var $resources;
	var $url = _getHomeDashboardUrl(userUID, roleUID);
	...
	ajax.request($url, "GET")
		...
		//	})
		.then(function(result) {
			fetchDashboard.resolve(result.data.$resources);
		})
		.fail(function(result) {
			// When we are not up to date with the server, we do not auto save a personal dashboard later in the process
			noAutoSave = true;

			if (isPersonal && result.offline) {
				_readWelcomeDashboardClient(userUID, roleUID)
					.then(function(result) {
	...
	return fetchDashboard.then(function(resources) {
			$resources = resources;
			return _getDefaultDataHome($resources.content, isPersonal)
		})
		...
		.then(function(pageDataTransformed) {
			...
			pageData.meta = meta;
			pageData.isPersonal = isPersonal;

			if (!$resources.content) { // Did we create a new default one
				if (noAutoSave !== true && isPersonal) {
 ...
 */
function _computeWelcomePageData(content, isPersonal, roleUuid, meta) {
	var data = {
...
function _computeWelcomePageContent(pageData) {
	...
	pageData.dataset.json.$vignettes.forEach(function(vi) {
		if (!vi.local) {
			vignettes.push(vi);
			$gadgets[vi.persistentId] = pageData.dataset.json.$mobileGadgets[vi.persistentId];
		}
	});
...
function _getHomeDashboardUrl(userUID, roleUID) {
 ...
 */
exports.createWelcomeDashboardFromPageData = function(pageData, isPersonal, roleUuid) {
	var content = _computeWelcomePageContent(pageData);
	var data = _computeWelcomePageData(content, isPersonal, roleUuid, pageData.meta);
	return data;
}
...
exports.saveWelcomeDashboard = function(data) {
	return $.smResolve()
		...
		.always(function() { // Try to save on server even if client side save failed eventually
			return _saveWelcomeDashboardServer(data);
		});
}
