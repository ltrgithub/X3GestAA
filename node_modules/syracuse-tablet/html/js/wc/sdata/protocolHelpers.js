...
exports.getHomeDashboardUrl = function(userUID, roleUID) {
	...
	return exports.getHostPort() + url;
};
/**
 * 
...
exports.computeActionResult = function($actionName, response) {
	...
	result.$diagnose = {
		$severity: "success",
		$message: locale.text("action.succeeded")
	};
	result.success = true;
	return result;
}
...
exports.checkHomeDataset = function(appList, dashboardContent) {
	var dataset = exports.ensureHomeDatasetCompatibility(dashboardContent, appList)
	if (dataset) return dataset // for moment but later manage delta between dataset and avalibled applist
	var datasetJson = {
		vignettes: [],
		$mobileGadgets: {}
	};
	...
	appList.forEach(function(app) {
		...
		datasetJson.$mobileGadgets[app.$uuid] = {
			...
			"$url": _mobileApplicationUrl(app.applicationName),
			"$method": "GET"
		};
	})
	return datasetJson;
}
exports.ensureHomeDatasetCompatibility = function(datasetJson, appList) { // datasetJson could be null, content V11 or content V12(woco)
	var compatibleDatasetJson = {
		$mobileGadgets: {},
		vignettes: []
	};
	if (!datasetJson || datasetJson.$mobileGadgets) {
		return datasetJson
	}
	var appMap = {};
	appList.forEach(function(app) {
	...
	Object.keys(datasetJson.$vignettes).forEach(function(key) {
		if (datasetJson.$gadgets[key].$type !== "$application" || (datasetJson.$gadgets[key].$type === "$application" && appMap[datasetJson.$gadgets[key].applicationName])) {
			...
			compatibleDatasetJson.$mobileGadgets[key].$url = _mobileApplicationUrl(datasetJson.$gadgets[key].applicationName);
...
function _mobileApplicationUrl(appName) {
	var endpoint = modules.get("appController").App.getCurrentEndpoint();
	return "{$hostPort}/mobile2/syracuse/collaboration/syracuse/mobileApplications(applicationName%20eq%20'" + appName + "')?representation=mobileApplication.$details&$mobileEndpoint=" + endpoint;
}
exports.homeArticle = function($article, datasetJson) {
	if (!$article) return _defaultHomeArticle(datasetJson);
	// Append test vignette
	var $items = [];
