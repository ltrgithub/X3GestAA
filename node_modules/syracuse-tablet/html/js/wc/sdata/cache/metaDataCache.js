...
var log = require('syracuse-tablet/html/js/wc/helpers/logger').getLogger("metaData", true);
		...
		.then(function() {
			...
			return $.smForEachPromise(Object.keys($pages), function(page) {
				...
				return $.smResolve()
					.then(function() {
						if (article) {
							log && log("Saving article: " + page);
							return _putMetaData("article", "$local", page, article);
						}
					})
					.then(function() {
						if (prototype) {
...
exports.getArticle = function(pageName) {
	log && log("Read meta data (page): " + pageName);
	return _getMetaData("article", "$local", pageName)
