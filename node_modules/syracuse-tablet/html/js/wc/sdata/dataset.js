...
var Dataset = utils.defineClass(
	...
	null, {
		...
		_getValueText: function($bind, useWorkingCopy) {
			/*Strangely,  for text this.json[$bind] is always null
			 * It will be better is it's null only when text is empty (like image). 
			 * not necessary to get an empty text, it take time for nothing 
			 * */
			var prop = "$url";
			if (useWorkingCopy !== true && this.prototype.root.isWorkingCopyFacet()) {
				prop = "$urlStateless"
		...
		getLink: function($linkName, $bind) {
			if ($linkName == "$queryfullpage") {
				...
				if (!this.prototype.root || !this.prototype.root.isFacet("$query")) {
					throw new Error("$qeury facte expected for $queryfullpage link");
				}
				return {
					"$title": "$queryfullpage",
					"$target": "",
					...
					"$method": "GET",
					"$url": this.prototype.root.data("$url")
				}
			}
			...
			var link = $.extend(true, {}, linkP, linkD);
			return link;
		},
		...
		_deltaSetActions: function(value, deltaNotifications) {
			...
			for (var ii = 0, jj = actions.length; ii < jj; ii++) {
				...
				this.json.$actions[action] = value[action];
...
var ArrayDataset = utils.defineClass(
	...
	Dataset, {
		...
		createRowDataset: function(json, prototype, $uuid, index) {
			// Note that a row's $bind is the rows $uuid which allows to perfectly identify it later
...
var MultiArrayDataset = utils.defineClass(
	...
	ArrayDataset, {
		...
		applyDelta: function(delta) {
			...
			var changedLinesOnly = delta[0].$index !== undefined;
			if (changedLinesOnly) {
		...
		// be careful it's not the right way to get col info, just a hack to display the sort panel
		getColumnsInfo: function() {
			var self = this;
			var colInfos = [];
			var $properties = this.rowPrototype.getValueByPath("$properties");
			Object.keys($properties).forEach(function(key) {
				var colinfo = $.extend(true, {}, $properties[key])
				colinfo.$bindValue = colinfo.$item && colinfo.$item.$refValueName ? colinfo.$item.$refValueName : key // !ok when reference get $referenceValueName
				colinfo.$title = self.prototype.root.resolveExpression(colinfo.$title)
