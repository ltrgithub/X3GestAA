...
var RowDataset = utils.defineClass(
	...
	Dataset, {
		...
		getLinkDrillDown: function() {
			...
			var $link = this.prototype.getValueByPath(path);
			if (!$link || !$link.$url) return null
			return this.resolveExpression($link.$url);
		},
		getLinkLevelUp: function() {
			var path = "$links.levelUp";
			var $link = this.prototype.getValueByPath(path);
			if (!$link || !$link.$url) return null
			return protoHelpers.getHostPort() + this.resolveExpression($link.$url);
		},
	}
);
