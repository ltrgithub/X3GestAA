...
exports.fetchPageData = function($link, options) {
	...
	if (url.path.indexOf("/mobileDashboard") >= 0) {
		...
		if (defBaseUrl && prototypeUrl == null) {
			prototypeUrl = defBaseUrl + "proto.json"
		}
	...
	return $.smResolve()
		.then(function() {
			return ajax.request(prototypeUrl + forceNoCache);
		})
		.then(function(result) {
			...
			return ajax.request(datasetUrl + forceNoCache);
		})
		.then(function(result) {
			...
			if (options.article) {
				...
				return ajax.request(articleUrl + forceNoCache, null, null, null, {
		...
		.then(function(result) {
			...
			return protoHelpers.checkTransform($link.$url, prototypeJson, datasetJson, articleJson);
		})
