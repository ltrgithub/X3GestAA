...
var breadcrumb = require('syracuse-tablet/html/js/site/breadcrumb');
var factory = require('syracuse-tablet/html/js/factories/siteFactory');
...
var Site = utils.defineClass(
	function($elmt) {
		var self = this;
		self.breadCrumb = new BreadCrumb();
		self.$elmt = $elmt;
		self.currentPage = null;
		tselfhis.currentApplication = null;
		self._config = {
			transition: null
		};
		self._bindEvents();
		// Set the current site - Only one site is allowed
		_currentSite = self;
	}, null, {
...
var _changePage = function(state, options) {
	...
	if (typeof state === "string") {
		...
		if (!state.transition) {
			// Default transition
			state.transition = _currentSite.config("transition");
		}
		// Readable id
		state.uuid = utils.readableuid("page", state.type, state.name, state.appId || "site");
	}
	return _currentSite._gotoPage(state);
};
