...
<script>
	var _IOSNativeLogger = function(){
		this.log = function(msg){
			this.log = webkit.messageHandlers.log.postMessage(msg)
		}
		this.logError = function(where, e){
			this.log = (where, e.message)
		}
	}
	var _IOSNativeInterface = function(){
		...
		this.isSupported = function(domain, method){
			try{
				var res = webkit.messageHandlers.isSupported.postMessage(this._serialize(arguments))
				smNativeLogger.log("isSupported result = " + res)
				return res;
			}catch(e){
				smNativeLogger.logError("Native call isSupported", e)
			}
		}
		this.callMethod = function(domain, method, param){
			try{
				var res = webkit.messageHandlers.callMethod.postMessage(this._serialize(arguments))
				smNativeLogger.log("callMethod result = "+ res)
				return res;
			}catch(e){
				smNativeLogger.logError("Native call callMethod", e)
			}
		}
		this.fireMethod = function(domain, method, param){
			try{
				var res = webkit.messageHandlers.fireMethod.postMessage(this._serialize(arguments))
				smNativeLogger.log("fireMethod result = "+ res)
				return res;
			}catch(e){
				smNativeLogger.logError("Native call fireMethod", e)
			}
		}
	}
	function initIOSWrapper (){
