...
function checkFile(path, buf) {
	...
	if (path.indexOf('/native/') > 0) return;
	if (path.indexOf('/mockup/') > 0) return;
	if (path.indexOf('/syracuse-tablet/html/js/main.min.js') > 0) return;
	if (path.indexOf('/syracuse-tablet/html/js/main.joined.js') > 0) return;
...
function depCheck(path, code) {
	...
	if (reqVarsHit.length > 0) {
		console.log("Unused require(s) in: " + path);
		console.log("  " + reqVarsHit.join("\n  "));
		console.log("");
	}
}
