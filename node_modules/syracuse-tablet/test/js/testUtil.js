...
var _updtCounters = function(_self, ok) {
	_self.counts++;
	if (ok) _self.nbSuccess++;
	else _self.nbErrors++;
...
var _updtPanel = function(_self, ok) {
	_updtCounters(_self, ok);
	if (_self.nbErrors > 0) {
...
var _testCase = function(testSet, $html, data) {
	...
	_self.addResult = function(title, ok, errMsg) {
		// Tests
		if (false && _self.counts % 2 == 0) {
			ok = false;
			errMsg = "error " + _self.counts;
		}
		_updtPanel(_self, ok);
		var $html = $(_createHtml("teststatus", {
			icon: ok ? "ok" : "remove",
			title: title,
			error: ok ? "&nbsp;" : (errMsg ? ("<p>" + errMsg + "</p>") : "&nbsp;")
		}));
		$html.appendTo(_self.$resultGroup.show());
		_self.testSet.update(ok);
		return {
			$root: $html
		};
	};
...
var _testSet = function($html, data) {
	...
	_self.addTestcase = function(title) {
		var data = {
			id: _getId(),
			title: title,
