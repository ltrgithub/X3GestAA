				...
				var runTest = function() {
					...
					if (module) {
						module.test().then(function() {
							...
							modal.error("Test module '" + module.modulename + "' ERROR", e);
						});
					}
				};
				if (modulename != 'all') {
					var idx = modules.smIndexOf(function(m) {
						return m && m.modulename == modulename;
					});
					if (idx >= 0) {
						modules[idx].test();
					} else {
						alert("setTimeout");
						modal.error("Run test error", modulename + " test module not found");
					}
				} else {
					runTest();
				}
