...
var modal = require('syracuse-tablet/test/js/modal');
...
/**
 * NOTIFICATIONS
 * 	publish calls the subscriber asynchronously
 * 	-> We need setTimout(f,0) after a publish to check if object has been notified
 */
		...
		tests: [{
			run: function(testset, testCase, test) {
				...
				setTimeout(function() {
					...
					// check each event
					var check = notifications.check(subscriber);
					testCase.addResult("Unsubscribe sm.login", check["sm.login"] === 0 && check["sm.logout"] === 1);
 ...
 * SDATAUTILS
 */
var _baseUrl = "http://pc101329.sagefr.adinternal.com:8124/mobile1/x3/erp/SUPERV";
var _dataSdataUtils = {
	...
	testCases: [{
		...
		tests: [{
			...
			key: "5678",
			data: {
				"create": "{$baseUrl}/AQTCRUD/$template?representation=AQTCRUDM.$create",
...
var _runTestSet = function(dataTestSet) {
	...
	var _run = function(dataTestCase, i) {
		var dataTest = dataTestCase.tests[i];
		if (dataTest) {
			var prom = dataTest.run();
		}
	};
	window.setTimeout(function() {
		// This process allows Test.run to return a deferred
		var _runTestCase = function(j) {
			var dataTestCase;
			try {
				dataTestCase = dataTestSet.testCases[j];
				if (!dataTestCase) return;
				var testCase;
				try {
					testCase = testset.addTestcase(dataTestCase.title, dataTestCase);
					var _runTest = function(i) {
						var dataTest = dataTestCase.tests[i];
						if (dataTest) {
							var prom = dataTest.run(testset, testCase, dataTest);
							if (prom) {
								prom.then(function() {
									_runTest(i + 1);
								}, function(e) {
				...
				modal.error("uncaught javascript running test [" + (dataTestCase.title || "no title") + "]", e);
			}
		};
		_runTestCase(0);
	}, 10);
