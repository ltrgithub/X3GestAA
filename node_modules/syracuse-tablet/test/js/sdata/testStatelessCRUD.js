	...
	tests: [{
		...
		response: {
			callBack: function(testCase, test, send, result) {
				result.callBackMessage = "Nb records=" + ((result.responseJSON && result.responseJSON.$resources) || []).length;
			}
		}
	}, {
		...
		response: {
			...
			callBack: function(testCase, test, send, result) {
				if (result.isSuccess) {
					...
					testCase.context.testId = result.responseJSON.CODE;
					result.callBackMessage = "New id is " + result.responseJSON.CODE;
				}
			}
		}
	}, {
		...
		response: {
			...
			callBack: function(testCase, test, send, result) {
				// Check if modify ok
				if (result.isSuccess) {
					if (result.responseJSON.TEXTDES0 == send.send.TEXTDES0 && result.responseJSON.TEXTE == send.send.TEXTE) {
						result.callBackMessage = "Record id[" + result.responseJSON.CODE + "] saved - Fields updated";
					} else {
						result.status = "error";
		...
		response: {
			...
			callBack: function(testCase, test, send, result) {
				// Check if modify ok
				if (result.isSuccess) {
					if (result.responseJSON.TEXTDES == send.send.TEXTDES && result.responseJSON.TDCB == send.send.TDCB) {
		...
		response: {
			...
			callBack: function(testCase, test, send, result) {
				if (result.isSuccess) {
					var TDCB = parseFloat(result.responseJSON.TDCB);
					console.log(TDCB);
					var TDCB1 = parseFloat(result.responseJSON.TDCB1);
					var TDCB2 = parseFloat(result.responseJSON.TDCB2);
...
var _execSdata = function(testCase, test) {
	...
	try {
		...
		sdataHttp.send(http).then(function(result) {
			...
			try {
				...
				if (result.responseJSON && result.responseJSON.$diagnoses && result.responseJSON.$diagnoses.length > 0) {
					result.responseJSON.$diagnoses.forEach(function(d) {
				...
				testCase.context.prevJsonData = result.responseJSON;
			} catch (e) {
			...
			testCase.addResult("execSdata Rejected", "error", e.message);
		});
	} catch (e) {
		...
		var exec = function(i) {
			if (i < kase.tests.length) {
				...
				_tempo(i == 0 ? 0 : 100).then(function() {
					_execSdata(testCase, kase.tests[i]).then(function() {
						exec(i + 1);
					}, function(e) {
						log && log("_execTestCase - rejected - " + kase.title, e);
						testCase.addResult("_execSdata rejected", "error", e);
					});
				}, function(e) {
					log && log("_execTestCase - rejected - " + kase.title, e);
					testCase.addResult("_tempo rejected", "error", e);
				});
			} else {
				log && log("_execTestCase - Stopped - " + kase.title);
				deferred.resolve();
			}
		};
		exec(0);
	} catch (e) {
		log && log("_execTestCase Exception" + kase.title, e);
		testCase.addResult("_execSdata Exception", "error", e);
		/* To not break execution of other tests*/
		deferred.resolve(e);
	}
	return deferred.promise();
};

exports.modulename = "sdataStatelessCRUD";
exports.test = function() {
	var deferred = new $.Deferred();
	try {
		var testset = libTests.addTestset("sdata StateLess Protocol Unit Tests (CRUD)");
