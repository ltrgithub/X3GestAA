...
var log = require('syracuse-tablet/html/js/helpers/logger').getLogger("testStorage");
...
exports.test = function() {
	...
	try {
		...
		var storage;
		storage.init("testStorage")
			.then(function(status) {
				log && log("Init DB OK");
				var sequence = $.Deferred();
				try {
					var ok = status.$status === storageModule.StatusCodes.OK;
					if (!ok) throw new Error("storage init error " + e.$message);
					testcase.addResult("Check status", ok, status.$message);
			...
			.then(function() {
				testcase = testset.addTestcase("No operation on put with same etag");
				return storage.put({
					"$context": context,
					"$collection": "testcase",
					"$key": 'etag1',
			...
			.then(function(status) {
				testcase.addResult("Check status (insert)", status.$status === storageModule.StatusCodes.OK);
				return storage.read({
					"$context": context,
					"$collection": "testcase",
					"$key": 'etag1'
			...
			.then(function(status) {
				testcase.addResult("Check status (read)", status.$status === storageModule.StatusCodes.OK);
				// change data but not etag -> must not be stored because of etag equality
				return storage.put({
					"$context": context,
					"$collection": "testcase",
					"$key": 'etag1',
			...
			.then(function(status) {
				testcase.addResult("Check status (update)", status.$status === storageModule.StatusCodes.OK);
				return storage.read({
					"$context": context,
					"$collection": "testcase",
					"$key": 'etag1'
			...
			.then(function(status) {
				...
				var deferred = $.Deferred();
				try {
					testcase.addResult("Check value (read)", status.$data.key === "A");
					deferred.resolve();
				} catch (e) {
					deferred.reject(e);
				}
			...
			.then(function() {
				testcase = testset.addTestcase("Overwrite operation on put with changed etag");
				return storage.put({
					"$context": context,
					"$collection": "testcase",
					"$key": 'etag1',
			...
			.then(function(status) {
				...
				var deferred = $.Deferred();
				try {
					testcase.addResult("Check value (read)", status.$data.key === "C");
					deferred.resolve();
				} catch (e) {
					deferred.reject(e);
				}
			...
			.then(function() {
				...
				log && log(e);
				if (!testcase) {
					testcase = testset.addTestcase("Javascript Error");
				}
				testcase.addResult("Javascript exception", "error", e);
				deferred.resolve();
			});
	} catch (e) {
