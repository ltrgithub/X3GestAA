	...
	try {
		...
		storage.init(true)
			...
			.then(function() {
				var sequence = $.Deferred();
				try {
					testcase = testset.addTestcase("Read single records with wrong endpoint");
					records.forEach(function(rec, idx) {
						storage.read({
							"$context": context,
							"$endpoint": "not_here",
							...
							testcase.addResult("Check status (NOT_FOUND)", status.$status === storageModule.StatusCodes.NOT_FOUND);
							if (idx == (records.length - 1)) {
								sequence.resolve();
							}
						}, function(e) {
