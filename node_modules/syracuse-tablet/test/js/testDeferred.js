	...
	try {
		...
		$.smResolve("xxx")
			...
			.then(function(param) {
				...
				testcase.addResult("Rejected as expected", param === es);
			});
	} catch (e) {
		// Ok to be here
		testcase.addResult("Exception as expected", e === es);
	...
	$.smResolve("xxx")
		.then(function(param) {
			...
			setTimeout(function() {
				try {
					d.resolve("yyy");
					// we end up here because exception is catched and chain is rejected
					testcase.addResult("Next statement as expected", true);
				} catch (e) {
		...
		.then(function(param) {
			...
			// we end up here because of throw is catched internally and sent to reject handler
...
exports.test = function() {
	...
	if (jQuery.smDeferred !== jQuery.Deferred) {
		simpleExceptionOld();
		asyncExceptionOld();
	} else {
