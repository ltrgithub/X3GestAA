...
describe("Array row detail navigation", function() {
	// Open browser, login, ensure browser is closed once all tests were executed
	UT.defaultBeforeAndAfter();

	it("Array row detail navigation - Navigate to test page", function() {
		this.timeout(20000);
		return UT.App.navToTestPage("testDefaultAQMCRUDM.$mobileDashboard")
			...
			.then(function($$grid) {
				return UT.Dom.click(UT.Sel.Row(0), $$grid, UT.App.waitPage$("AQMCRUDM.$query"));
			})
			...
			.then(function($$grid) {
				return UT.Dom.click(UT.Sel.Row(0), $$grid, UT.App.waitPage$("AQMCRUDM.$details"));
			});
	});

	it("Array row detail navigation - Open and apply authoring", function() {
		this.timeout(15000);
	...
	it("Array row detail navigation - Navigate to row detail page", function() {
		this.timeout(10000);
		return UT.Dom.waitLocated(UT.Sel.X3Field("ADEVADEVICEL"), 2000)
			.then(function() {
			...
			.then(function($$row) {
				return UT.Dom.click(UT.Sel.DataAction("rowDetail"),
					$$row,
					UT.App.waitPage$("AQMCRUDM.$details.ADEVADEVICEL", 5000));
			})
			.then(function() {
