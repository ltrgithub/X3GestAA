...
describe("Check array row detail navigation", function() {
	...
	it("Navigate to row detail page", function() {
		...
		return UT.DOM.find(UT.SEL.$$Field("ADEVADEVICEL"))
			...
			.then(function(row) {
				return UT.DOM.click(UT.SEL.$$DataAction("rowDetail"), row, UT.App.waitPage$("x3.erp.SUPERV.AQMCRUDM.$query", 5000));
			})
			.then(function() {
				UT.assertOk("Navigation to row detail successful");
			})
			.thenCatch(function() {
				UT.assertFail("Navigation to row detail failed");
			});
	});
});