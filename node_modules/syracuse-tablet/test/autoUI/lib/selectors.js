...
exports.PageTile = function(pageName) {
	pageName = _prependEndpoint(pageName);
	return {
...
function _prependEndpoint(expr) {
	// Do not change if starting with dot
	if (expr.indexOf(".") === 0) {
		return expr.substring(1);
	}
	// Mobile dashboard pages never contain endpoint in their name
	if (expr.indexOf(".$mobileDashboard") > -1) {
		return expr;
	}
	if (expr.split(".").length < 5) {
		return UT.Config.endpoint + "." + expr;
	}
	return expr;
}