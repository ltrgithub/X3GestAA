...
// ===================== Login with SageId ===============================================
exports.ensureLoginSageId = function(options) {
	var defaultPage = "html://HOME";
	var page = _getPageNameFromUrl(options && options.testPageUrl || defaultPage);
	UT.log("==>ensureLoginPageWait, options.pagewait: " + options.pagewait);
	return UT.Dom.waitLocated([
		...
		.then(function(isLogin) {
			if (isLogin) {
				//--- Link sageID click -----------------------------------------------------------------
				return UT.Dom.click(UT.Sel.Xpath("//button[@id='s-m-sageid-signin']")).then(function() {
					UT.log("$$$==>ensureLoginSageId, options.pagewait: " + options.pagewait);
					return UT.Dom.waitLocated(UT.Sel.Xpath("//div[@id='contentOneCol']"), 2000).then(function() {
						return UT.Dom.setValue(UT.Sel.Id('sso_Email'), UT.Config.emailSageId)
							.then(function() {
								return UT.Dom.setValue(UT.Sel.Id('sso_Password'), UT.Config.passwordSageId);
							})
							.then(function() {
								return UT.Dom.click(UT.Sel.Xpath("//input[@type='submit']"));
							})
					})
				});
			}
