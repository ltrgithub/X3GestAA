...
(function(VS) {
	...
	function processMetadata(metadata, thisClass, baseClass) {
		...
		if (hasMetadata) {
			Object.defineProperty(thisClass, "_metadata", {
				value: classMetadata,
				enumerable: false
			});
		}
			...
			function RemoveElementsAction_ctor() {
				...
				execute: function() {
					...
					if (typeof this.elementsToRemove !== "object") {
						Array.prototype.forEach.call(document.querySelectorAll(this.elementsToRemove), function(actualTarget) {
							executeFunc(actualTarget);
						}, this);
					} else {
			...
			function RemoveChildrenAction_ctor() {
				...
				execute: function() {
					...
					if (typeof this.parentElement !== "object") {
						Array.prototype.forEach.call(document.querySelectorAll(this.parentElement), function(actualTarget) {
							executeFunc(actualTarget);
						}, this);
					} else {
			...
			function ToggleClassAction_ctor() {
				...
				execute: function() {
					...
					if (this.targetSelector) {
						Array.prototype.forEach.call(document.querySelectorAll(this.targetSelector), function(actualTarget) {
							executeFunc(actualTarget, this);
						}, this);
					} else {
	...
	VS.Namespace.defineWithParent(VS, "Actions", {
		...
		LoadPageAction: VS.Class.derive(VS.Actions.ActionBase,
			function LoadPageAction_ctor() {
				...
				pageLoaded: {
					type: Array,
					elementType: VS.Actions.ActionBase
				}
			}
		)
	});
			...
			function TimerBehavior_ctor(configBlock, attachment) {
				...
				_attachImpl: function(attachment) {
					...
					if (!this._timerIdPerAttachment[attachment.uniqueID]) {
						this._timerIdPerAttachment[attachment.uniqueID] = window.setInterval(function() {
							tickHandler(that, attachment);
			...
			function EventTriggerBehavior_ctor(configBlock, attachment) {
				...
				_addSourceImpl: function(source) {
					...
					if (this.event) {
						...
						that._eventListener = function(event) {
							that.execute(source, event);
						};
...
(function(VS) {
			...
			function RequestAnimationFrameBehavior_ctor(configBlock, attachment) {
				...
				_attachImpl: function(attachment) {
					...
					if (!this._requestPerAttachment[attachment.uniqueID]) {
						this._requestPerAttachment[attachment.uniqueID] = window.requestAnimationFrame(function() {
							callBack(that, attachment);
						});
					}
				},
	...
	function callBack(requestAnimationFrameBehavior, attachment) {
		...
		requestAnimationFrameBehavior._requestPerAttachment[attachment.uniqueID] = window.requestAnimationFrame(function() {
			callBack(requestAnimationFrameBehavior, attachment);
		});
	}
})(VS);
...
(function(VS, global) {
	...
	dataWinControlInfo["WinJS.UI.ListView"] = {
		event: "loadingstatechanged",
		key: "loadingState",
		value: "complete"
	};
	dataWinControlInfo["WinJS.UI.Hub"] = {
		event: "loadingstatechanged",
		key: "loadingState",
		value: "complete"
	};
	dataWinControlInfo["WinJS.UI.FlipView"] = {
		event: "pagecompleted",
		key: null,
		value: null
	};
	dataWinControlInfo["WinJS.UI.Repeater"] = {
		event: "itemsloaded",
		key: null,
		value: null
	};
