...
var _consts = {
	...
	appListUrl: "/sdata/syracuse/collaboration/syracuse/mobileApplications/$service/availableApplications",
...
var _getUserProfile = function() {
	...
	try {
		if (_cache.userProfile) deferred.resolve(_cache.userProfile);
		_get(_consts.userProfileUrl).always(function(result) {
			if (result.isSuccess) {
				_cache.appsList = result.responseJSON.$resources;
			} else {
				log && log("_getUserProfile loading error - Status: " + result.status);
			}
			deferred.resolve(result.isSuccess, _cache.appsList);
		});
	} catch (e) {
		log && log("_getUserProfile Exception", e);
		return deferred.reject(false, e);
	} finally {
...
var _queryLocalePreferences = function() {
	...
	try {
		if (_cache.localePreferences) {
			...
			var url = utils.baseLocation.host + _consts.localePreferencesUrl;
			// Temporarily - It's not a sdata url - We use requireJS
			ajax("GET", url).always(function(result) {
				if (result.isSuccess) {
					var locales = [];
...
/**
 *  List of Syracuse Mobile Applications
 * */
var _getAppList = function() {
	var deferred = $.Deferred();
	try {
		if (_cache.appsList) deferred.resolve(_cache.appsList);
		_get(_consts.appListUrl).always(function(result) {
			if (result.isSuccess) {
				_cache.appsList = result.responseJSON.$resources;
			} else {
				log && log("_getAppList loading error - Status: " + result.status);
			}
			deferred.resolve(result.isSuccess, _cache.appsList);
		});
	} catch (e) {
		log && log("_getAppList Exception", e);
		return deferred.reject(false, e);
	} finally {
		return deferred.promise();
	}
...
/**
 * Syracuse Mobile Applications Detail
 * */
var _getAppDetail = function(appInfo) {
	var deferred = $.Deferred();
	try {
		if (!_cache.appsDetails) _cache.appsDetails = {};
		if (_cache[appInfo.$uuid]) deferred.resolve(_cache[appInfo.$uuid]);
		_get(appInfo.$metaDataUrl).always(function(result) {
			if (result.isSuccess) {
				_cache.appsDetails[appInfo.$uuid] = result.responseJSON;
			} else {
				log && log("_getAppDetail loading error - Status: " + result.status);
			}
			deferred.resolve(result.isSuccess, _cache.appsDetails[appInfo.$uuid]);
		});
	} catch (e) {
		log && log("_getAppDetail Exception", e);
		return deferred.reject(false, e);
	} finally {
		return deferred.promise();
	}
...
exports.getAppList = _getAppList;
exports.getAppDetail = _getAppDetail;
