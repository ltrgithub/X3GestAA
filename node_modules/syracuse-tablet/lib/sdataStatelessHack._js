...
exports.prototypes = function(_, context, name, id) {
	...
	try {
		...
		if (proto && proto.length > 0) {
			//#7157 - Dataset is used to read details facet prototype to transform urls in edit mode
			proto = _transformProto(_, JSON.parse(proto), repr, facet, x3Facet, context.endpoint && context.endpoint.dataset(_));
		}
...
//dataset is the endpoint ID
function _transformProto(_, proto, repr, newFacet, oldFacet, dataset) {
	tracer && tracer("Transforming proto: ", repr + " " + newFacet + "->" + oldFacet + " - Dataset:" + dataset);
...
function _transformLazyPropertyUrls(_, proto, repr, newFacet, oldFacet, dataset, detailsProto) {
	...
	var getProtoDetail = function(_, repr) {
		var ep;
		var entity;
		var model;
		var protoDet;
	...
	Object.keys(proto.$properties).forEach_(_, function(_, key) {
		...
		if (prop.$url) {
			if (!protoDetails) {
				protoDetails = getProtoDetail(_, repr);
			}
