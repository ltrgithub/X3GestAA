...
exports.transformProto = function(_, proto, repr, newFacet, oldFacet, dataset, apiPrefix) {
	...
	_transformLazyPropertyUrls(_, proto, repr, newFacet, oldFacet, dataset, null, apiPrefix);
 ...
 */
function _transformLazyPropertyUrls(_, proto, repr, newFacet, oldFacet, dataset, detailsProto, apiPrefix) {
	...
	if (!dataset) {
		dataset = _urlGetDataset(proto.$baseUrl, apiPrefix);
	}
	if (!dataset) {
		return;
	}
	...
	Object.keys(proto.$properties).forEach_(_, function(_, key) {
		...
		if (prop.$item) {
			...
			if (details$Item) {
				_transformLazyPropertyUrls(_, prop.$item, repr, newFacet, oldFacet, dataset, details$Item, apiPrefix);
			}
		}
	});
}
...
// return the dataset
function _urlGetDataset(url, prefix) {
	if (!url) return null;
	var segs = url.split("/");
	var idx = segs.indexOf(prefix || _mobileProtocolUrlSeg);
