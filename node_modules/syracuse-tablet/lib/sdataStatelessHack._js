...
function _transformActions(actions, links, facet, representation, instanceUrl) {
	console.log("BEFORE");
	console.log(actions ? JSON.stringify(actions, null, 3) : "");
	var res = {};
...
function _handleUrl(_, context, name, id) {
	...
	try {
		...
		if (url.indexOf("/$services/") > -1 && method === "get") {
			...
			if (responsTime) responsTime.send = new Date().getTime();
			// console.log(responsTime.send , "send")
			x3pool.send(_, context);
			if (responsTime) responsTime.x3response = new Date().getTime();
			// console.log(responsTime.x3response , "x3response")
		...
		if (responsTime) console.log("request=" + name, "send=" + (responsTime.send - responsTime.start), "x3response=" + (responsTime.x3response - responsTime.send), "reply=" + (responsTime.reply - responsTime.x3response), "elapsed=" + (responsTime.reply - responsTime.start));
	} finally {
		context.response = savedResponse;
	}
}
