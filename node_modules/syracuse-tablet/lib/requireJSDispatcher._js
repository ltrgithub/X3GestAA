...
var _getAuthConfig = function(_) {
	...
	if (authHelper.isAllowed("saml2") && saml2s.length) {
		result["saml2"] = true
		result["$saml2"] = saml2s;
	} else {
		result["saml2"] = false;
	...
	if (authHelper.isAllowed("oauth2") && oauth2s.length) {
		result["oauth2"] = true;
	...
	switch (action) {
		case "getToken":
			...
			if (_checkLoginCookie(_, loginCookie)) {
				...
				headers['x-reconnect-login-cookie-name'] = loginCookieName;
				headers['x-reconnect-sid-cookie-name'] = sidCookieName;
			} else {
				status = 404;
			}
			break;
