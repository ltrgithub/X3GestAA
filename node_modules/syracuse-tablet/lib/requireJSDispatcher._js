...
/**
 * Implements 2 services - tabletCheckLogin - tabletDoLogin used by tablet
 * applications Temporarily services while waiting for more genral services
 */
var _log = null;
var _loginSvc = function(_, service, url, request, response) {
	// Full url sent by client - we need same host/port as the one used by
	// client - Cookies are host/port dependent
	...
	// We need no-encoding (no gzip) data otherwise we get an error on client
	// side
	...
	if (!check) {
		...
		// TODO - Change lang if login ok params.lang = query.lang;
	}
	...
	response.writeHead(testResponse.statusCode === 401 ? 403 : 200,
...
var _locales = {
	"$resources": [{
		...
		"code": "zh-CN",
		"description": "中文(中华人民共和国)"
	}]
};

/**
 * Returns the list of languages for login dialog
 */
var _localePreferences = function(_, response) {
	response.writeHead(200, {
		'content-type': 'application/json'
	});
	// globals.context.localePreferences can't be used
	// sdata request needs an authenticated user
	// Temporarily hard coded
	response.writeHead(200, {
		'content-type': 'application/json'
	});
	response.write(_, JSON.stringify(_locales));
};
exports.dispatcher = function(config) {
	...
	return function(_, request, response) {
		...
		} else if (service && (service === "tabletLocalePreferences")) {
			_localePreferences(_, response);
		} else {
