...
exports.entity = {
	...
	$relations: {
		...
		homeDashboard: {
			...
			$isMandatory: function(_, instance) {
				return (!instance.useHomeGadget(_));
			},
	...
	$functions: {
		$serialize: function(_) {
			...
			var mobileGadget = db.getEntity(_, "mobileGadget");

			var result = self._internalSerialize(_);
			if (self.useHomeGadget(_) && self.homeGadget(_)) {
				var gadgetData = db.fetchInstance(_, mobileGadget, self.homeGadget(_).$uuid);
				result.$homeGadget = _getGadgetData(_, gadgetData);
			} else if (self.homeDashboard(_)) {
...
function _getApplicationInfo(_, context, mobileApplication) {
	...
	var data = {
		...
		useHomeGadget: mobileApplication.useHomeGadget(_),
		roles: roles,
		endpoints: endpoints,
		versionNumber: mobileApplication.versionNumber(_),
		onlineOnly: mobileApplication.onlineOnly(_)
	};
	if (mobileApplication.useHomeGadget(_)) {
		var mobileGadget = db.getEntity(_, "mobileGadget");
		var gadgetData = db.fetchInstance(_, mobileGadget, mobileApplication.homeGadget(_).$uuid);
		data.$homeGadget = _getGadgetData(_, gadgetData);
	} else {
		var mobileDashboard = db.getEntity(_, "mobileDashboard");
		var dashboardData = db.fetchInstance(_, mobileDashboard, mobileApplication.homeDashboard(_).$uuid);
		data.$homeDashboard = dashboardData.$serialize(_);
	}
	return data;
}
...
function _gatherMetaData(_, instance, helpers, applicationData) {
	...
	if (instance.useHomeGadget(_)) {
		tracer && tracer("Home gadget:" + instance.homeGadget(_).code(_));
		_processGadget(_,
			helpers,
			applicationData,
			instance.homeGadget(_)
		);
	} else {
...
function _processGadget(_, helpers, applicationData, gadget) {
	...
	var data = _getGadgetData(_, gadget);
