	...
	$services: {
		...
		applicationMetaData: {
			...
			$execute: function(_, context, instance) {
				...
				if (errors.length > 0) {
					var errs = [];
					Object.keys(applicationData.$pageErrors).forEach(function(key) {
						errs.push(key + ": " + applicationData.$pageErrors[key].msg + "\nSource: " + applicationData.$pageErrors[key].ref);
					});
...
function _processDashboard(_, helpers, applicationData, dashboard, ref) {
	...
	dashboard.vignettes(_).toArray(_).forEach_(_, function(_, vignette) {
		...
		if (gadget) {
			 ...
			 */
			var id = vignette.persistentId(_) || vignette.$uuid;
			output.$vignettes[id] = data;
		}
	});
	...
	gadgets.forEach_(_, function(_, gadget) {
		_processGadget(_, helpers, applicationData, gadget, ref);
	});
}
...
function _processGadget(_, helpers, applicationData, gadget, ref) {
	...
	switch (gt) {
		case "$dashboard":
			...
			if (dashboard) {
				...
				_processDashboard(_, helpers, applicationData, dashboard, ref);
			} else {
				data = null;
			}
			break;
		...
		case "$representation":
			...
			_processPage(_, helpers, applicationData, pageName, ref);
			break;
...
function _processPage(_, helpers, applicationData, pageName, ref) {
		...
		Object.keys(applicationData.$pages).forEach_(_, function(_, pageName) {
			...
			if (page.$notParsedYet) { // only traverse each page once!
				...
				if (pageName.indexOf(".QUERY~") > -1) {
					...
					_parseProto(_, helpers, applicationData, page.$prototype, pageName, null, ref);
				}
				done = false;
			}
		});
...
function _parseProto(_, helpers, applicationData, prototype, pageName, allowedLinks, ref) {
	var repr = pageName.split(".");
	var representation = repr[3];
	var facet = repr[4];
	var $links;
	...
	if ($links) {
		_parseLinks(_, helpers, applicationData, $links, allowedLinks, prototype, pageName, ref);
	}
	...
	if ($links) {
		_parseLinks(_, helpers, applicationData, $links, allowedLinks, prototype.$properties.$resources.$item, pageName, ref);
	}
	if (prototype.$properties) {
		_parseProperties(_, helpers, applicationData, prototype.$properties, pageName, ref);
	}
}

function _parseProperties(_, helpers, applicationData, properties, pageName, ref) {
	var repr = pageName.split(".");
	var representation = repr[3];
	var facet = repr[4];
		...
		switch (prop.$type) {
			case "application/x-array":
				if (prop.$item.$type === "application/x-reference") {
					...
					_parseProto(_, helpers, applicationData, prop.$item, pageName, allowedLinks, pref);
				}
				break;
...
function _parseLinks(_, helpers, applicationData, links, allowedLinks, prototype, pageName, ref) {
	var repr = pageName.split(".");
	var representation = repr[3];
	var facet = repr[4];
		...
		if (!allowedLinks || allowedLinks.indexOf(linkName) > -1) { // If a list of allowed links is given, ignore other ones!
			...
			if (url.indexOf("$mobileDashboards") > -1) {
				...
				if (url.indexOf("{$baseUrl}") != 0 || representation == null) {
					...
					_processPage(_, helpers, applicationData, endpoint + "." + rep[0] + "." + rep[1], ref);
				}
			}
		}
	});
