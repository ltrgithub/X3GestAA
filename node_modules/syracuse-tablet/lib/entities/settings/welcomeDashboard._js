	...
	$services: {
		...
		updateDashboard: {
			...
			$execute: function(_, context) {
				...
				isPersonal = (data.user != "" && data.roles && data.roles.length === 1) ? true : false;
				isGlobal = (!isPersonal && data.roles && data.roles.length >= 1 && _isAutorizedGlobal(_)) ? true : false;
				isFactory = (!isPersonal && !isGlobal && _isAutorizedFactory(_)) ? true : false;;
				if (!isPersonal && !isGlobal && !isFactory) {
					error = true;
				}
				if (!error) {
					var wd = isPersonal ? _getPersonalDashboard(_, wds, data.user, data.roles) : _getGlobalOrFactoryDashboard(_, wds, isGlobal, isFactory);
					if (wd) {
				...
				context.reply(_, error ? 500 : 200, data.content);
			}
		},
		//Hack for test
...
//get dashboard with user and all roles match
function _getPersonalDashboard(_, dashboards, user, roles) {
