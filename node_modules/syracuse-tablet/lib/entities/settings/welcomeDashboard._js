...
exports.entity = {
	...
	$services: {
		availableDashboards: {
			...
			$method: "GET",
			$isMethod: false,
			$title: "Get available welcome dashboards",
			$overridesReply: true,
			$execute: function(_, context) {
				//console.log("availableDashboards"+sys.inspect(context));
				var self = this;
				var dashboards = [];
				...
				var selectedDashboards = _getRelevantWelcomeDashboards(_, wcdashboards, user, role);
				selectedDashboards.forEach_(_, function(_, dashboard) {
					dashboards.push(dashboard.dashboard);
				});
				context.reply(_, 200, {
		...
		availableDashboard: {
			...
			$execute: function(_, context) {
				...
				var db = context.db;
				var wcdashboards = db.fetchInstances(_, db.getEntity(_, "welcomeDashboard"), {});
				var selectedDashboard = _getRelevantWelcomeDashboard(_, wcdashboards, user, role, uuid);
				context.reply(_, 200, {
		...
		listOfRoles: {
			...
			$method: "GET",
			$isMethod: false,
			$title: "Delete all welcome dashboard",
			$overridesReply: true,
			$execute: function(_, context) {
				var roles = [];
				var db = context.db;
		...
		updateDashboard: {
			...
			$execute: function(_, context) {
				...
				var error, wds, isPersonal, isGlobal, isFactory;
				var db = context.db;
				var entityWelcomeDashboard = db.getEntity(_, "welcomeDashboard");
				wds = db.fetchInstances(_, entityWelcomeDashboard, {});
				...
				if (!error) {
					var wd = isPersonal ? _getPersonalDashboard(_, wds, data.user, data.role) : _getGlobalOrFactoryDashboard(_, wds, isGlobal, isFactory);
					if (wd) {
						console.log("updateDashboard update");
					} else {
						console.log("updateDashboard create");
						wd = entityWelcomeDashboard.factory.createInstance(_, null, db);
						wd.$factory = isFactory;
		...
		//Hack for test
		allWelcomeDashboards: {
			$isHidden: true,
			$method: "GET",
			$isMethod: false,
			$title: "Get all welcome dashboard",
			$overridesReply: true,
			$execute: function(_, context) {
				var dashboards = [];
				var db = context.db;
				var allWds = db.fetchInstances(_, db.getEntity(_, "welcomeDashboard"), {});
				allWds.forEach_(_, function(_, wds) {
					dashboards.push(_getMobileWelcomeDashboardInfo(_, wds));
				});
				context.reply(_, 200, {
					$resources: dashboards
				});
		...
		deleteAll: {
			...
			$method: "GET",
			$isMethod: false,
			$title: "Delette all welcome dashboards",
			$overridesReply: true,
			$execute: function(_, context) {
				var db = context.db;
				var count = 0,
					error = false;
				var wds = db.fetchInstances(_, db.getEntity(_, "welcomeDashboard"), {});
				wds.forEach_(_, function(_, wd) {
					wd.deleteSelf(_);
					count++;
				});
				context.reply(_, error ? 500 : 200, {
					"count": count + " deleted"
				});
			}
		}
...
function _getPersonalDashboard(_, dashboards, user, role) {
	...
	dashboards.forEach_(_, function(_, wd) {
		...
		var wrRoles = wd.roles(_).toArray(_);
		wrRoles.forEach_(_, function(_, rl) {
			if (role.$uuid === rl.$uuid) {
				wdf = wd;
			}
		});
	});
...
function _getGlobalOrFactoryDashboard(_, welcomeDashboards, isGlobal, isFactory) {
	if (!welcomeDashboards) return null;
	var factoryOwner = _getFactoryOwner(_);
	var dashboard;
	welcomeDashboards.forEach_(_, function(_, wd) {
		if (dashboard) return;
		if (wd.user(_) && wd.user(_).$uuid) return;
		if (isGlobal && (wd.$factory || wd.roles(_).length() === 0)) return;
		if (isFactory && (!wd.$factory || (wd.$factoryOwner !== factoryOwner))) return;
		dashboard = wd;
	});
	return dashboard;
};
...
function _getRelevantWelcomeDashboard(_, welcomeDashboards, user, role, uuid) {
	...
	welcomeDashboards.forEach_(_, function(_, mobileWelcomeDashboard) {
		if (mobileWelcomeDashboard.$uuid === uuid) {
			score = 0;
			dashboard = {
				"score": score,
		...
		if (mobileWelcomeDashboard.user(_) && mobileWelcomeDashboard.user(_).$uuid === user && _isRoleMathes(_, mobileWelcomeDashboard, role)) {
			score = 1;
		...
		} else if (mobileWelcomeDashboard.$factory && mobileWelcomeDashboard.$factoryOwner != "SAGE") {
			score = 3;
			if (!dashboard || dashboard.score > score) { // we take only the first one we found
				dashboard = {
					"score": score,
					"dashboard": _getMobileWelcomeDashboardInfo(_, mobileWelcomeDashboard)
				};
...
//Get the most relevant dashboard (math user end role, or role)
function _getRelevantWelcomeDashboards(_, welcomeDashboards, user, role) {
	var dashboards = [];
	var isAutorizedFactory = _isAutorizedFactory(_);
	var isAutorizedGlobal = false;
	if (!isAutorizedFactory) {
		isAutorizedGlobal = _isAutorizedGlobal(_);
	}
	var isPersonal = (!isAutorizedFactory && !isAutorizedGlobal) ? true : false;
	welcomeDashboards.forEach_(_, function(_, mobileWelcomeDashboard) {
		var dashboard;
		var views = [];
		if (mobileWelcomeDashboard.user(_) && mobileWelcomeDashboard.user(_).$uuid === user && _isRoleMathes(_, mobileWelcomeDashboard, role)) {
		...
		if (_isRoleMathes(_, mobileWelcomeDashboard, role)) {
			dashboard = {
				"score": 2,
				"dashboard": _getMobileWelcomeDashboardInfo(_, mobileWelcomeDashboard)
			};
		...
		if (mobileWelcomeDashboard.$factory && mobileWelcomeDashboard.$factoryOwner != "SAGE") {
			dashboard = {
				"score": 3,
				"dashboard": _getMobileWelcomeDashboardInfo(_, mobileWelcomeDashboard)
			};
		...
		if (isPersonal && dashboard && dashboard.score === 1) return;
	});
	dashboards.sort_(_, function(_, a, b) {
		return (a.score - b.score);
	});
	return isPersonal ? (dashboards[0] ? [dashboards[0]] : []) : dashboards;
};

function _getAvailableViews(_, welcomeDashboards, user, role, uuidSelected) {
	var views = [];
	welcomeDashboards.forEach_(_, function(_, mobileWelcomeDashboard) {
		var view;
		if (mobileWelcomeDashboard.user(_) && mobileWelcomeDashboard.user(_).$uuid === user && _isRoleMathes(_, mobileWelcomeDashboard, role)) {
			view = {
				"score": 1,
				"view": _getMobileWelcomeDashboardViewInfo(_, mobileWelcomeDashboard, uuidSelected)
			}
			view.$isPersonal = true;
			views.push(view);
			return;
		};
		if (_isRoleMathes(_, mobileWelcomeDashboard, role)) {
...
function _isAutorizedGlobal(_) {
	var sp = globals.context.session && globals.context.session.getSecurityProfile(_);
	//console.log("authoringLevel :" + sp.authoringLevel(_));
