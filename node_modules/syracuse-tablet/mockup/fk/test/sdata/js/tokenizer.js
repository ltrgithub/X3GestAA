...
exports.Tokenizer = new function() {
	...
	function _parseDateTime(line, chars, i, tokens) {
		...
		return end + 1;
		/*
		 * var end = i + 1; while (end < chars.length && chars[end] !== '@') {
		 * end++; } if (end === chars.length) { throw new Error('date constant
		 * not terminated:' + line.substring(i, line.length)); } var str =
		 * chars.substr(i + 1, end - i - 1); var dt = str.length > 0 ?
		 * (str.length == 10 ? date.parse(str) : datetime.parse(str)) : null; //
		 * crnit: we accept date=@@ syntax, so empty string should be interpret
		 * as null date // if (dt != null) { tokens.push(new
		 * token.Token(token.tokenType.literal, line, i, end + 1 - i, dt)); // }
		 * return end + 1;
		 */
	}
