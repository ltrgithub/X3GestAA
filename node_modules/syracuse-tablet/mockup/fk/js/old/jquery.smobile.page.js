...
(function($) {
	...
	that.linkDetailPage = function(controller) {
		if (controller.applicationId) {
			...
			// Instead of adding it on pageload we add it into the link (se
			// pageNAme function)
			// -> The navigator history stores the application id AND pageId
			// when we click the link
			// -> When we goBack to the parent page we retrieve the page (back
			// after validating the sort criterion)
			// Otherwise we can't retrieve the page and create a new one each
			// time we go back
	...
	var _createPage = function(controller, controls, html, options) {
		...
		try {
			...
			Object.keys(controls).forEach(function(id, index) {
				...
				// Control id : controller.prefix + prefix - Ex: F1_items for a
				// query list - representation2ui
				...
				if (id === "root") {
					...
					/*
					 * if (myParent){ for (var i=0; i < propagate.length; i++){
					 * var prop = propagate[i]; if (myParent[prop] &&
					 * !c.$styles[prop]){ c[prop] = myParent[prop]; } } }
					 */
					...
					if (l.$inquiry != null && !controller.pageInfo.$inquiry) {
						// Set inquiry mode if there is at least one inquiry
						// bloc param or result
						// USed later to open
					...
					// UI Controls with handler, refresh, events methods - Class
					// defined in $.smobile.controls[c.uiClass]
					// Calls 'handler' method to initcontrol(create html);
					// Do not display empty cells in RowLayout #3128 - details
					// only
					var parentLyt = controls[c.parent];
					if (!c.data) c.data = {};
					if (!parentLyt && html[c.parent]) {
						// Tabs layout - Only one control c_001 per tab - Each
						// tab has c_001_0, _1 id but refers to c_001
					...
					$.smobile.addControl.apply(controller, [parent, c,
						function(ctrl) {
							// ctrl -> is not a class object it contains { html,
							// action, options}
							if (ctrl && Array.isArray(ctrl)) {
								...
								if (c.emptyCellOption != null) {
									// We have to modify the class in html
									// string :-(
									var p = html[c.parent];
									if (p && p.before) {
										// Cell css is initialize by
										// layout.RowLayout method according to
										// $emptycell option
										// If a control is attached to the cells
										// we reset $emptycell option to
										// calculate the it according ctrl value
										p.before = p.before.replace(" empty", "");
										// Issue #3128 - do not display empty
										// cells or empty rows for RowLayouts
								...
								if (ctrl.async) {
									// View control with asynchronous handler
									// called by bindData - See appView and
									// syncView
	...
	var _createHeaderRight = function(controller, uiData, multisel) {
		var html = [];
	...
	var _createHeader = function(controller, ui, pgtitle, isLocalData) {
		var html = [],
			opts = {};
		if ($.isWinJs) {
			html.push('<div id="createNavBar" data-win-control="WinJS.UI.NavBar" data-win-options="{sticky: true}">');
			html.push('<div data-win-control="WinJS.UI.NavBarContainer">');
			// TEST DISPLAY ICONS
			var x = 0x100;
			for (var i = 0; i < 0; i++) {
				var z = (x + i).toString(16).toUpperCase();
				html.push('<div data-win-control="WinJS.UI.NavBarCommand" data-win-options="{ label: \'' + "uE" + z + '\', icon: \'\\uE' + z + '\' }"></div>');
			}
			html.push('<a  href="#start" data-win-control="WinJS.UI.NavBarCommand" data-win-options="{ label: \'Home\', icon: \'' + $.helpers.winJsIconChar('home') + '\' }"></a>');
			html.push('<a href="#about" data-win-control="WinJS.UI.NavBarCommand" data-win-options="{ label: \'About\', icon: \'' + $.helpers.winJsIconChar('contactinfo') + '\' }"></a>');
			html.push('<a href="#apps" data-win-control="WinJS.UI.NavBarCommand" data-win-options="{ label: \'Set up\', icon: \'' + $.helpers.winJsIconChar('download') + '\' }"></a>');
			html.push('<a href="#settings" data-win-control="WinJS.UI.NavBarCommand" data-win-options="{ label: \'Settings\', icon: \'' + $.helpers.winJsIconChar('settings') + '\'}"></a>');
			html.push('</div>');
			html.push('</div>');
		} else {
			html.push('<div class="navigation-bar dark fixed-top">');
			html.push('<div class="navigation-bar-content container">');
			html.push('<span class="element"><img src="images/s-site-sage.png"></span>');
			html.push('<span class="element-divider"></span>');
			html.push('<span class="element"><a href="#start"><span class="icon-home"></span>  Home</a></span>');
			html.push('<span class="element-divider"></span>');
			html.push('<span class="element"><a href="#about"><span class="icon-info-2"></span>  About</a></span>');
			html.push('<span class="element-divider"></span>');
			html.push('<span class="element"><a href="#apps"><span class="icon-download"></span>  Setup</a></span>');
			html.push('<div class="element place-right" title="Settings"><a href="#settings"><span class="icon-cog"></span></a></div>');
			html.push('</div>');
			html.push('</div>');
		...
		var htmRight = _createHeaderRight(controller, ui, multisel);
		html.push(htmRight);
		html.push('</div>');
		return html.join('');
	};
	...
	var _bindEvents = function(controller, ui, $header, $content) {
		...
		// Expanding and collapsing boxes sometimes causes charts to not be
		// centered correctly.
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info,
		...
		var $content = $page.find("section.s-m-content");
	...
	function _afterEnhance(page, params, uiData, pageData, cb) {
		var _afterData;
		if (!page.page.db) {
			...
			$sdata
				.getAppConfig(
					...
					function(config, error) {
						...
						_afterEnhance(page, params, uiData, pageData, cb);
					});
	...
	that.doBack = function(ctrl) {
		if (ctrl && ctrl.pageOptions.detail && ctrl.parent.data && ctrl.parent.data.$info && ctrl.parent.data.$info.isMethodCall) {
			// Method call
			// Like the result of a call generates a page not stored in browser
			// history (changeHash=false) we force to display this page by doing
			// a changepage on the dom element
			// This page is well displayed and back works in that case
			// Inconvenient : the original page stays in browser history and a
			// back from parent go to this page instead of application home (for
			// inquiries reps)
			$.smobile.changePage(ctrl.parent.$page, {
				changeHash: true
			});
		} else {
			$.mobile.back();
	...
	that.createPage = function(page) {
		...
		var cls = $page.attr("class").split(" ");
		var klass = [];
		for (var i = 0; i < cls.length; i++) {
			if (cls[i].indexOf("ui-") == -1) klass.push(cls[i]);
		}
		return {
			$page: $page,
			$template: $ptemplate
	...
	that.parseURL = function(u, page, cb) {
		...
		if (params.url && params.url.indexOf("&localstorage=") >= 0) {
			...
			return cb(params.dataurl, data, false, options);
	...
	that.multisel = {
		...
		addMultiselMenus: function(controller, ui, html) {
			...
			var multiSelLinks = [];
			for (var p in links) {
				// TODO - Filter actions according to the presence of a key in
				// url
				if (p.indexOf("$") !== 0) {
					var lnk = links[p];
