...
var _endsWith = function(str, s) {
	if (str == null || s == null || str.length < s.length) return false;
	s = s.toString();
	return s.length === 1 ? str.charAt(str.length - 1) === s : str.lastIndexOf(s) === str.length - s.length;
};
var _startsWith = function(str, s) {
	if (str == null || s == null || str.length < s.length) return false;
 ...
 * Old code Mathias
 */
var _oldReplaceStringMultilines = function(code){
	// Replace multiline strings with a dummy code line
	return code.replace(/.*'.*\\\s+((.*?)\\\s+)*.*'[;,\)\s].*/gmi, function(m) {
		var lc = [];
...
/**
 * Replace multilines strings by concatenation
 * Keeps the content of the string 
 * Preserve the line number if error
 */
var _replaceStringMultilines = function(code, fileName){
	var _log = null; //console.log
	var res = [], str, found = false, startComment = false;
	(code || "").split('\n').forEach(function(line){
		...
		if (_startsWith(line, "/*")){
			...
			res.push(line)
			return;
		} else if (_endsWith(line, "*/")){
			startComment = false;
			res.push(line)
			return;
		} else if (_startsWith(line, "//") || startComment){
			res.push(line)
			return;
		}
		if (line.length === 0 || !_endsWith(line, "\\") ){
			if(str){
				if(line.indexOf("'") < 0){
					_log("!!!", fileName, "Unexpected not EOS", line)
				}
				str.push("'" + line);
				res.push(str.join("+\n"));
				str = null;
		...
		var pugedLine = line.replace(/(\'\s*\+.*\+\s*\')|(\\\')/g, function(a, b, c){return ""})
		var a = pugedLine.indexOf("'");
		if (a === -1){
			str.push("'" + line + "'");
			return;
		}	
		if(str != null){
			_log("!!!", fileName, "not null str", line)
		}
		str = [];
		str.push(line + "'");
	})	
	if(str != null){
		_log("!!!", fileName, "end not null str")
	}
	res = res.join('\n')
	if (false && found){
		_log("\n\n", fileName, "\n\n");
...
/*
 * http://upshots.org/javascript/javascript-regexp-to-remove-comments#solution
 */
var A_BLOCKS = [
	'(' +
	/"[^"\\]*(?:\\[\S\s][^"\\]*)*"|'[^'\\]*(?:\\[\S\s][^'\\]*)*'/.source,
...
function checkFile(path, buf) {
	...
	code = code.replace(/^\#\!.*/, '');
	var orgCode = code;
	code = '(function(){' + _replaceStringMultilines(code, path) + '})';
	try {
		...
		runLint(code, function(lintRes) {
			var duplMethods = checkDoubleMethods(orgCode);
...
function depCheck(path, code) {
	...
	code = code.replace(requireRegExp, function(dep, p1, p2) {
		reqVars[p1] = dep;
	...
	scannedFileNames.forEach(function(file) {
		...
		fileData.requires.forEach(function(req) {
			...
			var match = scannedFileNames.some(function(known) {
				if (known.indexOf('../') == 0){
					known = known.replace("../", "syracuse-tablet/")
				}
			...
			else if (!match && !matchCI && req.indexOf("/deps/") < 0 
					&& file.indexOf("/html-escape/") < 0
					&& file.indexOf("/syracuse-rtf/") < 0) {
				console.log("Missing module: " + req + " in file: " + file);
			}
		});			
	});	
}
