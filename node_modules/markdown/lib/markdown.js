...
expose.toHTML = function toHTML( source , dialect ) {
  ...
  // BRJOU fix: remove <br></br>
  return expose.renderJsonML( input ).replace(/<br><\/br>/g, '<br>');
}
...
Markdown.dialects.Gruber.inline = {
    ...
    "`": function inlineCode( text ) {
      ...
      // BRJOU added ([^`]*\n) to swallow language keyword after ```
      var m = text.match( /(`+)([^`]*?\n)?(([\s\S]*?)\1)/ );

      if ( m && m[3] )
        return [ m[1].length + (m[2] || "").length + m[3].length, [ "inlinecode", m[4] ] ];
      else {
