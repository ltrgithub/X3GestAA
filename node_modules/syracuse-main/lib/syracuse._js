...
function disp(request, response, _) {
	...
	try {
		...
		if (config.hosting.multiTenant) {
			...
			if (!/^\/(?:license|logging|patch)\//.test(request.url)) {
...
function initAsync(_, simplified) {
	...
	if ((config && config.port === "N0") || !(exports.server instanceof mock.MockStreamServer)) {
		var _localhost = os.hostname() + ":" + config.port;
		var dbInfo = adminHelpers.AdminHelper.getCollaborationOrm(_).db.admin().serverInfo(~_);
		if (dbInfo && dbInfo.version) {
			ga.hitEvent(_, "Syracuse System", "Database", "MongoDB " + dbInfo.version, 1, {
				host: _localhost
			});
		}
		var cpus = os.cpus();
		if (cpus && cpus.length > 0) {
			ga.hitEvent(_, "Syracuse System", "CPU model", cpus[0].model, 1, {
				host: _localhost
			});
			ga.hitEvent(_, "Syracuse System", "CPU number", cpus.length, 1, {
				host: _localhost
			});
		}
		ga.hitEvent(_, "Syracuse System", "Memory", os.totalmem(), 1, {
			host: _localhost
		});
		ga.hitEvent(_, "Syracuse System", "OS", os.type() + "-" + os.platform() + "-" + os.arch() + "-" + os.release(), 1, {
			host: _localhost
		});
	}
