...
function _redir(login, status, userAuthentication) {
...
function _userCheck(session, login, password, passwordCheck, method, standardSetting, _) {
	...
	if (user) {
		...
		if (!user.active(_)) // no login for inactive users!
			return _redir(login, 4, userAuthentication);
		...
		if (localConfig.method !== method && (localConfig.method.indexOf('oauth') >= 0 || method.indexOf('oauth') >= 0)) {
			...
			return _redir(login, 7, userAuthentication);
		...
		if (user) {
			...
			if (!groups.length) return _redir(login, 5, userAuthentication);
			...
			if (!role) {
				return _redir(login, 6, userAuthentication); // no role
			}
...
function createPerfmonSnapshot(_) {
	...
	if (path.dirname(path.resolve(logDir, "")) === "bin") {
		// console.log("perfmon.createSnapshot: " + logDir);
	...
	// logDir = path.join(logDir, d.toString("yyyy-mm") + "/" + d.toString("dd"));
	// console.log("perfmon.createSnapshot: " + logDir);
	var snapshot = {
...
exports.startServers = function(_, port) {
	...
	var snapshotCollectingInterval = ((config.system || {}).perfmon || {}).snapshotInterval;
	if (snapshotCollectingInterval > 0) {
		console.log("Creation of perfmon snapshot every " + snapshotCollectingInterval + " seconds");
		snapshotInterval = setInterval(function() {
			createPerfmonSnapshot(_ >> function(err) {
				if (err) throw err;
			});
		}, snapshotCollectingInterval * 1000);
	}
...
process.on('exit', function() {
	snapshotInterval && clearInterval(snapshotInterval);
