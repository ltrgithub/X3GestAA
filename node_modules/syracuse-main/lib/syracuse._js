...
function ensureAuthenticated(dispatcher) {
	return function(request, response) {
		return flows.withContext(function() {
			(function(_) {
				var useSessionPool;
				if (dispatcher.prepare && typeof(dispatcher.prepare) === "function") {
					dispatcher.prepare(_, request);
				}
				useSessionPool = request.useSessionPool || /^\/(api\d+|soap-generic)\//.test(request.url);
...
function unezify(listener) {
	var fn = ez.devices.http.listener(function(request, response, _) {
	...
	if (listener.prepare && typeof(listener.prepare) === "function") {
		fn.prepare = listener.prepare;
	}
	return fn;
}
