...
var MEMORY_THRESHOLD1 = (config.system.memoryThreshold1 || 1500) * 1048576;
var MEMORY_THRESHOLD2 = (config.system.memoryThreshold2 || 2500) * 1048576;
...
function patchHeaders(request, response) {
	...
	response.writeHead = function(status, headers) {
		...
		if (hd) {
			...
			if (!hd["cache-control"])
				hd["cache-control"] = "max-age=0, s-maxage=0, no-cache, no-store, must-revalidate, private, proxy-revalidate";
			else if (/no-cache/.test(hd["cache-control"])) {
				// ensure we have all the recommended values but without duplicates
				hd["cache-control"] = hd["cache-control"].split(/\s*,\s*/).concat(["no-store", "must-revalidate", "private"]).filter(function(e, i, arr) {
					return arr.indexOf(e) == i;
				}).join(", ");
			}
			if (/no-cache/.test(hd["cache-control"]))
				hd["pragma"] = "no-cache";
			...
			if (hd["set-cookie"]) {
				// ensure cookies are secured on https
				hd["set-cookie"] = hd["set-cookie"].map(c => sessionManager.secureCookie(secure, c));
			}
		}
		...
		return flows.withContext(function() {
			(function(_) {
				...
				return sessionManager.ensureSession(_, request, response, useSessionPool) //
					&& authModule.ensureAuthenticated(_, request, response, params) //
...
var requireDispatcher = require('streamline-require/lib/server/require').dispatcher({
	...
	whiteList: [
		/^@sage\/(syracuse-core|syracuse-sdata-parser|html-sanitizer|rtf-converter|syracuse-ui|ui-chart|ui-d3-graph|ui-schedule|ui-sql-designer|ui-text|ui-visual-process|uml-designer)\b/,
		/^(syracuse-(main\/html|core|rtf|office|msoffice)|streamline|jsbn|jsurl|js-xml|test-runner)(\/|$)/,
...
var uiModules = ["syracuse-react", "syracuse-ui", "ui-chart", "ui-d3-graph", "ui-schedule", "ui-sql-designer", "ui-text", "ui-visual-process", "uml-designer"];
var uiBasePath = basePath = fsp.join(fsp.dirname(module.filename), "../../@sage");
var uiStaticDispatcher = require('streamline-static').dispatcher({
	root: uiBasePath
});

uiModules.forEach(function(uiModule) {
	var dispatcher;
	dispatcher = (function(_, req, resp) {
		if (!uiStaticDispatcher(_, req, resp)) {
			return notFound(_, req, resp);
		}
	});
	authenticatedRoutes[uiModule] = dispatcher;
});
...
*/
	try {
		...
		if (/^\/[^\/]+\/public\//.test(request.url)) {
			return rawStaticDispatcher(request, response);
		} else if (/^\/syracuse-react\/(dist|devTemp)\//.test(request.url)) {
			if (config && config.react && config.react.devMode === true) {
				request.url = request.url.replace("/syracuse-react/dist/", "/syracuse-react/devTemp/")
			}
			if (!uiStaticDispatcher(_, request, response)) {
