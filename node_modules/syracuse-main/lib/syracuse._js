...
uiModules.forEach(function(uiModule) {
	var dispatcher;
	if (uiModule === "syracuse-react" && config && config.react && config.react.devMode === true) {
		dispatcher = (function(_, req, resp) {
			req.url = req.url.replace("/syracuse-react/dist/", "/syracuse-react/devTemp/")
...
*/
	try {
		...
		} else if (/^\/syracuse-react\/dist\//.test(request.url)) {
			return uiStaticDispatcher(_, request, response);
		}
