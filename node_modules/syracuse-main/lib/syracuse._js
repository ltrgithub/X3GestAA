...
function staticDispatcher(_, request, response) {
	...
	if (request.url.indexOf("/notificationAll") >= 0 && url.parse(request.url).pathname === "/notificationAll") {
		...
		console.log("NOTIFY END");
		require('syracuse-collaboration/lib/entities/notificationTime').scheduleAll(_); // function which updates local notification array
		response.end('OK');
		return;
	}
...
function helpProxyProvider(rootUrl, authent) {
	...
	return function(_, request, response) {
		...
		if (helpResponse.statusCode === 404) return false;
...
function helpDispatcher(_, request, response) {
	...
	// *** REMINDER ***: remove hardcoded url
	var configUrl = config.help && config.help.url || "http://172.28.105.4/";
	...
	if (provider(_, request, response)) return;

	// second chance with x3 language code
	// request.url = '/' + locale.isoLanguageMap[locale.current.toLowerCase()] + '/' + rel;
	// if (provider(_, request, response)) return;
