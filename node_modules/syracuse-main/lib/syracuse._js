...
function startSnapshotCollector(_, interval) {
	if (!interval) return;

	var snapshotInterval, lastSeqId = -1;
		...
		if (sids.length > 0) {
			sids.forEach_(_, function(_, k) {
				...
				snapshot.perfmon[s.id] = {
					timings: s.timings.filter(filterTimings)
				};
				lastSeqId = (s.timings || []).reduce(maxTimingId, lastSeqId);
			});
...
var _map = {
	...
	xtend: function(_, request, response) {
		return allowOrigin(request, response) && sessionManager.sessionManagement(_, request, response, {
			authRequired: true,
			forceAuth: true
		}) && hrmDispatcher(_, request, response);
	},
...
exports.startServers = function(_, port) {
	...
	startSnapshotCollector(_, ((config.system || {}).perfmon || {}).snapshotInterval);
