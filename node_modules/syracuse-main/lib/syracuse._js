...
var perfmonLogger = require('../../../src/perfmon/logger');
...
var api1 = require('syracuse-sdata/lib/api1Dispatch');
...
var shadowRoot = path.join(path.dirname(require.resolve('streamline-runtime')), '..');
...
function staticDispatcher(_, request, response) {
	...
	if (!staticProvider(_, request, response, opts) && !staticShadowProvider(_, request, response, opts)) return notFound(_, request, response);
}
	...
	return function(request, response) {
		return flows.withContext(function() {
			(function(_) {
				...
				if (useSessionPool) {
					ignoreCookie = require('syracuse-soap/lib/server/soapUtils').notTesterSoap(request);
				}
				return sessionManager.ensureSession(_, request, response, useSessionPool, ignoreCookie) //
...
var requireDispatcher = require('streamline-require/lib/server/require').dispatcher({
	...
	whiteList: [
		/^@sage\/syracuse-(core|sdata-parser)\b/,
		/^(syracuse-(main\/html|ui|core|rtf|office|msoffice|sdata\/lib\/(parser|httpHelpers))|streamline|jsbn|jsurl|js-xml|test-runner|html-escape)(\/|$)/,
		/^syracuse-sdata(-parser|\/lib\/httpHelpers)/,
		/^msoffice\/lib\//,
...
var sessionDispatcher = require('../../../src/session/dispatcher').dispatcher(config);
var eventDispatcher = require('syracuse-event/lib/dispatcher').dispatcher(config);
var widgetRegistryDispatcher = require('syracuse-main/lib/widget-registry').dispatcher(config);
...
var authenticatedRoutes = {
	...
	perfmon: require('../../../src/perfmon/dispatcher').dispatcher(config),
...
if (config.react) {
	var reactBasePath = path.join(basePath, (config.react.path || "../../syracuse-react/build"));
	var staticReactProvider = require('streamline-static').dispatcher({
		root: reactBasePath,
		maxAge: 0,
	});
	authenticatedRoutes.react = (function(_, req, resp) {
		var opts = {};
		// don't cache main.html to trigger authentication through refetch.
		if (/\/react\/index\.html/.test(req.url)) opts.nocache = true;
		if (!staticReactProvider(_, req, resp, opts)) return notFound(_, req, resp);
	});
};
...
if (enableDevelopmentFeatures) {
	...
	var staticTestProvider = require('streamline-static').dispatcher({
		root: path.join(path.dirname(module.filename), "../../test-contract")
	});
...
*/
	try {
		...
		if (config.hosting.multiTenant) {
			...
			if (!/^\/(?:license|batch|logging|patch|soap\-wsdl)\//.test(request.url)) {
...
registry.scanExtensions(function(extensions, sub, pack) {
	if (extensions.startup && extensions.startup.module) {
		var startup = require(extensions.startup.module);
		if (startup) {
			try {
	...
	if (extensions.http) extensions.http.forEach(function(dispatcher) {
		...
		var api = require(dispatcher.module);
		...
		var unauthPath = dispatcher.unauthenticatedPath && ('/' + sub + '/' + dispatcher.unauthenticatedPath + '/');
		mainRoutes[sub] = function(request, response) {
...
function initBatchMulti(_, stopMode) {
	...
	if (tenants) tenants.forEach_(_, function(_, tenant) {
...
function initAsync(_, simplified, stopMode) {
	...
	if (!stopMode) {
		...
		if (!/^[NW]\d+$/.test(process.argv[2])) {
			require('syracuse-batch/lib/batchHelper').initBatchServers(flows.check);
		}
