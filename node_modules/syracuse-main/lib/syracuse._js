...
function allowOrigin(request, response) {
	...
	response.writeHead = function(status, headers) {
		...
		var key; // consider lower case and uppercase (not yet general lower casing as in V8)
		if (headers["content-disposition"]) { // this header may contain characters outside the allowed range
			key = "content-disposition";
		} else if (headers["Content-Disposition"]) { // this header may contain characters outside the allowed range
			key = "Content-Disposition";
		}
		if (key) {
			var cont = headers[key];
			...
			if (cont.indexOf('filename=') > 0 && cont.indexOf('filename*=') < 0 && !/^[\x20-\x7F]+$/.test(cont)) {
				...
				if (res) {
					...
					// use rfc5987
					headers[key] = cont.substr(0, res.index) + 'filename="' + filename2 + '"; filename*=UTF-8\'\'' + filename2 + " " + cont.substr(res.index + res.input.length);
				}
			}
		}
