...
function _makeHttpServerOptions(config) {
...
exports.server = new streams.createHttpServer(function(request, response, _) {
	try {
		// TEMP hack: in the code we use socket.secure wich doesn't exists. Fix the code later
		request.socket.secure = (config.session || {}).secure;
