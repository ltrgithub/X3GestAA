...
var api1 = require('../../../src/sdata/api1Dispatch');
...
var mongodb = require('../../../src/orm/dbHandles/mongoDbHandle');
...
var staticPublicProvider = require('streamline-static').dispatcher({
	root: basePublicPath,
	maxAge: 3600 * 1000,
});
...
var authModule = require('../../../src/auth/dispatcher');
...
var requireDispatcher = require('streamline-require/lib/server/require').dispatcher({
	...
	whiteList: [
		/^@sage\/(syracuse-core|syracuse-sdata-parser|html-sanitizer)\b/,
...
var licenseDispatcher = require('../../../src/license/dispatcher').dispatcher(config.license);
...
if (config.react) {
	var reactBasePath = fsp.join(basePath, (config.react.path || "../../syracuse-react/build"));
...
var mainRoutes = {
	...
	auth: authDispatcher,
	"public": staticDisp(staticPublicProvider),
...
function initAsync(_, simplified, stopMode) {
	...
	if (process.argv[2] === "--reset" || process.argv[2] === "--resetpass") { // set admin user to DB authentication; test the password
		try {
			...
			if (process.argv[2] === "--resetpass") {
				// test passphrase
				if (!require('../../../src/load-balancer/certTools').checkPassphrase(_, null, passphrase)) {
	...
	for (var i = 2; i < process.argv.length; i++) {
		...
		if (arg.substr(0, 8) === "licfile=") {
			...
			require('../../../src/license/check').licenseChange(lic, diagnoses, _);
	...
	if (!simplified) {
		...
		require('../../../src/flamegraph/index').autoStart(_);
	}
