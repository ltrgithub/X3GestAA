...
var badgeutil = require('syracuse-license/lib/badge')
...
function _userCheck(session, login, password, method, standardSetting, _) {
	...
	if (user) {
		...
		// License check
		if (user && user.preferences(_)) {
			
			
			// find out badges for current user
			
			var role = user.preferences(_).lastRole(_);
			
			
			if (!check.checkConcurrent(session, role, _)) {
				user = null;
...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
	...
	if (request.url.indexOf("/licenseupdate") >= 0 && url.parse(request.url).pathname === "/licenseupdate")
	{
		...
		var updateResult = check.updateLicense(_);
