...
function _userCheck(session, login, password, passwordCheck, method, standardSetting, _) {
	...
	if (user) {
		...
		else
		{
			...
			case "db":
				localConfig.method = config.session.auth || "digest";
				localConfig.source = "db";
				break;
			case "ldap":
				if (method === "digest") throw new Error("No HTTP digest authentication with LDAP") 
			...
			default:
				...
				throw new Error(locale.format(module, "wrongAuth", userAuthentication));
			}
		}
		if (localConfig.method !== method) {
			...
			if (passwordCheck ? !passwordCheck(user.password(_)) : password !== user.password(_)) user = null;
		...
		// License check
		var p;
		if (user && (p = user.preferences(_))) {			
			
			// find out badges for current user
			
			var role = p.lastRole(_);
			if (!check.checkConcurrent(session, role, user.login(_), _)) {
				return 2;
			} 
...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
	...
	// TODO only for testing purposes - will be removed later
	if (request.url.indexOf("/kill") >= 0 && url.parse(request.url).pathname === "/kill") {
