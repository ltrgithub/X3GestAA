...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
	...
	if (request.url.indexOf("/auth/pwd--") === 0)
	{
		...
		if (tmpLogin) {
			...
			if (request.method === "GET") {
				...
				if (db) {
					...
					var user = db.fetchInstance(_, db.model.getEntity("user"), {sdataWhere: whereClause});
					if (user) {
						var up = db.model.getEntity("userProfile").factory.createInstance(_, null, db);
				...
				answer = answer.replace("{changePasswdHeader}", locale.format(module, "changePasswdHeader", tmpLogin.user));
				...
				answer = answer.replace("{login}", tmpLogin.user)
				...
				if (content) { 				
					...
					if (user) {
						...
						var token = changePasswordModule.setTempLogin({user: query.login})
		...
		response.writeHead(303, { "Content-Type": "text/html", "Location": newUrl });
		response.end('<html><a href="/test">Link</a></html>');
		return;
	}
	...
	if (request.url.indexOf("/patchIntegration") >= 0 && url.parse(request.url).pathname === "/patchIntegration")
	{
...
function disp(request, response, _) {
	var timing = perfmon.start(module, "main.httpDispatch", request.url);
	try {
...
exports.startServers = function() {
	...
	exports.server.listen(_(function() {
		console.log('Server running at http://localhost:' + port + '/');
	}), port);
	exports.integrationServer && exports.integrationServer.listen(_(function() {
		console.log('Integration server running at http://localhost:' + config.integrationServer.port + '/');
	}), config.integrationServer.port);
}
...
if (process.argv[2] && process.argv[2].substr(0, 1) === "N") {
	...
	exports.server = new mock.MockStreamServer(disp, new mock.DescriptorWriteStream(3), process.stdin, {timeout: 1*(process.argv[3] || 0)});
...
// final part of patch integration (must be at the end, after initialization)
