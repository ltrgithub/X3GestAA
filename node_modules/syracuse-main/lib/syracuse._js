...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
	...
	if (request.url.indexOf("/auth/pwd--") === 0)
	{
		...
		var newUrl = "/";
		console.log("TOKEN "+token+" "+request.method);
		var changePasswordModule = require('streamline-auth/lib/changePassword');
		var tmpLogin = changePasswordModule.getTempLogin(token, request.method === "GET"); // do not delete token for get request
		console.log("Login found for user "+sys.format(tmpLogin));
		if (tmpLogin) {
			if (request.method === "GET") {
				...
				answer = answer.replace("{action}", "/auth/pwd"+token)
				response.writeHead(200, { "Content-Type": "text/html"});
				response.end(answer);
				return;
			} else {
				...
				if (content) { 				
					...
					console.log("3."+sys.format(query));
					var user;
