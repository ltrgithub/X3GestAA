...
function mainHtmlDispatcher(_, request, response) {
	var path = request.url.split('?')[0];
	if (/\/syracuse-main\/html\/main(-\w+)?\.html$/.test(path)) return staticDispatcher(_, request, response);
...
function ensureSession(dispatcher, noChallenge) {
	return function(_, request, response) {
		if (request.url.split('/')[1] == "sage-id")
...
var sageIdDispatcher = require('sage-id').create(config).dispatch;

var authenticatedMap = {
	...
	"syracuse-main": mainHtmlDispatcher,
	require: requireDispatcher,
	"sage-id": sageIdDispatcher,
};

var mainMap = {
	// authenticated routes that fail with forbidden instead of sending auth challenge
	upload: ensureSession(upload.dispatcher, false),
	// Following routes are *NOT* authenticated
	"index2.html": authDispatcher,
	"index3.html": authDispatcher,
	auth: authDispatcher,
...
Object.keys(authenticatedMap).forEach(function(key) {
	mainMap[key] = ensureSession(authenticatedMap[key]);
});
...
function disp(request, response, _) {
	...
	try {
		...
		var fs = request.url.split('/')[1];
		if (fs) fs = fs.split('?')[0]; // example /proxy?url=...
		var dispatch = mainMap[fs] || staticDispatcher;
		//		flows.callWithTimeout(_, function(_) {
		//dispatch(_, request, response);
		var authTest = dispatch(_, request, response);
		if (authTest == true) {
			var host = request.headers.host;
