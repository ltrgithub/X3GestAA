...
var getAuthModule = function(request, redirect, _) {
	...
	regexResult = /^\/auth\/(std|basic|digest|oauth2-(\w*)|sage-id)(\-\-[0-9a-f]*)?/.exec(pathname);
	...
	if (authmethod === "basic") {
		...
		return require("sage-id").create({
			...
			StartSignOnAttemptRequest: {
				SuccessUri: "http://localhost:8124/sage-id/startSignOnAttemptSuccess/{0}",
				FailureUri: "http://localhost:8124/sage-id/startSignOnAttemptFailure",
				CancelAllowed: "true",
				State: "test",
...
var staticProvider = require('streamline-static/lib/staticProvider').staticProvider({
	roots: [basePath, path.join(basePath, "../../node_modules")],
	maxAge: 3600 * 1000
});
...
function staticDispatcher(_, request, response) {
	...
	if (defaultPageExp.exec(request.url)) {
		if (!config.hosting.multiTenant) {
			return require('syracuse-health/lib/suspend').getSiteStatus(request, response, _);
		} else {
	...
	if (/suspend\.html$/.exec(request.url)) {
		var html = fs.readFile(__dirname + "../../../suspend.html", "utf8", _);
		response.writeHead("200", {
			"Content-Type": "text/html"
		});
...
function debugDispatcher(_, request, response) {
...
function _dispatch(_dispatcher) {
	return function(_, request, response) {
...
var _map = {
	...
	api: function(_, request, response) {
		return allowOrigin(request, response) && sessionManager.sessionManagement(_, request, response, {
			authRequired: true,
			forceAuth: true
		}) && sdataDispatcher(_, request, response);
	...
	xtend: _dispatch(hrmDispatcher),
	shrmportal: _dispatch(hrmDispatcher),
	...
	//"sage-id": require('streamline-auth/lib/sage-id').dispatcher,
	// Force two steps of authentication to ensure successful auth through Sage ID portal
	// Add authType to check for 2nd auth step in sessionManager.sessionManagement
	"sage-id": function (_, request, response) {
		return sessionManager.sessionManagement(_, request, response, {
			authRequired: true,
			forceAuth: false,
			forceAuth2: true,
	...
	logout: function(_, request, response) {
		// Pass sageIdBase, pfx cert, and pass as options for Sage ID logout
...
// hack to work around circular require problem when registering in urlMap
Object.keys(exports.urlMap).forEach(function(key) {
	_map[key] = exports.urlMap[key];
})
exports.urlMap = _map;
