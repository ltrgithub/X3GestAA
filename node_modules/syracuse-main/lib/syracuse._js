...
function helpDispatcher(_, request, response) {
	...
	if (!configUrl) {
		if (helpProvider) {
			...
			if (helpProvider(_, request, response)) return;
			// second chance with en_US language
			request.url = '/en_US/' + rel;
			if (helpProvider(_, request, response)) return;
		}
		return plainReply(response, 404, helpProvider ? 'help file not found: ' + rel : 'help not configured');
	}
	var url = configUrl + '/' + lang + '/' + rel;
