...
if ((config.system || {}).enableDeveloppementFeatures || helpers.enableDeveloppementFeatures) {
...
function helpProxyProvider(rootUrl, authent) {
	return function(_, request, response) {
		...
		if (helpResponse.statusCode === 404 || helpResponse.statusCode === 500) return false;
...
function helpDispatcher(_, request, response) {
	...
	var configUrl = config.help && config.help.url || "http://online-help.sageerpx3.com";
	...
	var urls = [];
	langParam && urls.push(langParam.replace('-', '_') + '/' + rel);
	langSeg && urls.push(langSeg.replace('-', '_') + '/' + rel);
	urls.push(locale.current.replace('-', '_') + '/' + rel);
	// last chance with en_US language
	urls.push('en_US/' + rel);

	helpTrace && helpTrace("helpDispatcher: urls=[" + urls + "]");

	if (urls.some_(_, function(_, url) {
		request.url = url;
		return provider(_, request, response);
	})) {
		return;
	}
