...
function setupHttpSecurity() {
	...
	var defaultHeaders = {
		...
		"content-security-policy": {
			...
			// 	'sha256-PC/JaatOIxSsFjtJ7S/uH5NZsi4WRfDYbKY9H+b7nIg=' //is required because of inline script in main.html.
			// ],
...
function _timerFn(request, nextTimer) {
	if (nextTimer === 0) nextTimer = (config.system || {}).requestReportThreshold || 10000;
	else console.log("Long request pending : ", nextTimer / 2, request.url);
	request.timer = setTimeout(function() {
		_timerFn(request, nextTimer * 2);
	}, nextTimer);
}
...
function disp(request, response, _) {
	...
	// Accept OPTIONS request to handle Ajax requests (for web services call from JQuery for instance
