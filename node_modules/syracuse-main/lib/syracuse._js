...
function staticDispatcher(_, request, response) {
	...
	if (/\/$|(\/default)\.(html|htm)/.exec(request.url)) {
...
var authDispatcher = require('streamline-auth/lib/dispatcher').dispatcher(config);
var sessionDispatcher = require('syracuse-session/lib/dispatcher').dispatcher(config);

var _map = {
	sdata: ensureSession(sdataDispatcher),
	trans: ensureSession(x3Dispatcher),
	"soap-ws": ensureSession(wsDispatcher),
	"soap-stubs": ensureSession(stubDispatcher),
	print: ensureSession(printDispatcher),
	xtend: ensureSession(hrmDispatcher),
	shrmportal: ensureSession(hrmDispatcher),
	upload: ensureSession(upload.dispatcher, false),
	tests: testServer && ensureSession(testServer.dispatch),
	search: ensureSession(searchEngine.dispatch),
	xsearch: ensureSession(searchEngine.dispatch),
	// Exact search, no wildcards
	fsearch: ensureSession(searchEngine.dispatch),
	// Fuzzy like this search
	bundles: ensureSession(require('bundles/lib/dispatcher').dispatcher(config, staticDispatcher)),
	perfmon: ensureSession(require('syracuse-perfmon/lib/dispatcher').dispatcher(config)),
	help: ensureSession(require('syracuse-help/lib/helpDispatch').dispatcher(config)),
	"main.html": ensureSession(staticDispatcher),
	"mobile.html": ensureSession(staticDispatcher),
	"config.html": ensureSession(staticDispatcher),

	// Following URLs are *NOT* authenticated
	"index2.html": authDispatcher,
	auth: authDispatcher,
	oauth2: authDispatcher,
	memwatch: require('syracuse-memwatch/lib/dispatcher').dispatcher(config),
	dump: require('syracuse-trace/lib/dispatcher').dispatcher(config),
	patchIntegration: patchDispatcher,
	consistency: patchDispatcher,
	licensetest: licenseDispatcher,
	licensecheck: licenseDispatcher,
	licenseupdate: licenseDispatcher,
	bo: x3Dispatcher,
...
function disp(request, response, _) {
	...
	try {
		...
		if (request.url && request.url.indexOf('/perfmon/') !== 0) timing.end();
