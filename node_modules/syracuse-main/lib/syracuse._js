...
config.mongodb.multitenant = config.mongodb.multitenant || config.multitenant;
...
config.searchEngine.multitenant = config.searchEngine.multitenant || config.multitenant;
...
exports.main = function() {
	exports.startServers(_ >> function(err) {
		if (err) throw err;
	});
};
...
function initAsync(_) {
	if (initializedTenants[globals.context.tenantId]) return;
	initializedTenants[globals.context.tenantId] = true;
