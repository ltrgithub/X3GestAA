...
/** check user and password against database (no password check if password is undefined)
*/
function _userCheck(session, login, password, _) {
    ...
    if (typeof(password) !== "undefined")
      whereClause += " and (password eq \"" + password + "\")";
...
if (config.session.auth === "basic") {
  if (config.session.useLdap == true) {
  authModule = require("streamline-auth/lib/basic").create(function (session, login, password, _) {
    ...
    authModule = require("streamline-auth/lib/basic").create(_userCheck);
}
else if (config.session.auth === "oauth2") {
  authModule = require("streamline-auth/lib/oauth2").create(_userCheck);
};
...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
    ...
    if (request.url.indexOf("/oauth2/redirect") >= 0 && url.parse(request.url).pathname === "/oauth2/redirect") {
      ...
      response.writeHead(303, { "Content-Type": "text/html", "Location": authModule.originalPath });
