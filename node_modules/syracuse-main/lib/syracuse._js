...
function patchHeaders(request, response) {
	if (config.traces && config.traces.http) console.log(new Date().toISOString(), "SYRACUSE REQUEST ", ++requestTraceId, request.url);
	var writeHead = response.writeHead;
	var t0 = Date.now();
	response.writeHead = function(status, headers) {
		if (config.traces && config.traces.http) console.log(new Date().toISOString(), "SYRACUSE RESPONSE", ++requestTraceId, status, (Date.now() - t0) + ' ms', request.url);
		headers = headers || {};
		// normalize headers
