...
function staticDispatcher(_, request, response) {
	...
	if (request.url.indexOf("/logging/") >= 0 && url.parse(request.url).pathname.substr(0, 9) === "/logging/") {
		var traceRecord = require('syracuse-trace/lib/entities/traceRecord');
		var parsed = url.parse(request.url, true);
		var parts = parsed.pathname.split("/");
		console.error("Parts " + require('util').format(parsed) + " " + require('util').format(parts));
		var result = "OK";
		try {
			traceRecord.handleChange(_, parts[2], parsed.query.seq, parsed.query.uuid);
		} catch (e) {
			result = "" + e;
			console.error("Handle change error " + e.stack);
		}
		response.writeHead(200, {
			"Content-Type": "text/plain"
		});
