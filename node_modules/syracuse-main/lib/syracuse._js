...
var print = require('syracuse-print/lib/printDispatch');
 ...
 * Return codes: 0 OK, 1 authentication failure, 2 no license, 3 change password
 */
...
function staticDispatcher(_, request, response) {
	...
	if (defaultPageExp.exec(request.url)) {
		...
		console.log("301 301 " + defaultPageExp);
		response.writeHead("301", {
...
function helpProxyProvider(rootUrl, authent) {
	if (rootUrl && rootUrl[rootUrl.length - 1] === '/') rootUrl = rootUrl.slice(0, -1);
	return function(_, request, response) {
		if (authent) {
			var token = {
				"serial": authent.serial || "10237",
				"date": datetime.now.toString(),
				"once": helpers.uuid.generate()
			},
				secret = authent.secret || "dc62db3f-26d7-48a3-9789-d6aa5ea99655",
				md5sum = crypto.createHash('md5');

			md5sum.update(token.serial + '/' + token.date + '/' + token.once + '/' + secret, "ascii");
			token.signature = md5sum.digest("hex");

			request.headers.Authorization = "X3Doc " + JSON.stringify(token).toString('base64');
		}
		var options = {
		...
		var helpRequest = streams.httpRequest(options);
		var buf;
		console.log(">>>> helpProxyProvider: request.headers=" + sys.inspect(request.headers));
		while (buf = request.read(_)) helpRequest.write(_, buf);
		var helpResponse = helpRequest.end().response(_);
		console.log(">>>> helpProxyProvider: before rewrite helpResponse.headers=" + sys.inspect(helpResponse.headers) + "\nhelpResponse.statusCode=" + helpResponse.statusCode);
		response.writeHead(helpResponse.statusCode, helpResponse.headers);
		while (buf = helpResponse.read(_)) response.write(_, buf);
		response.end();
		console.log(">>>> helpProxyProvider: after rewrite helpResponse.statusCode=" + helpResponse.statusCode);
		return helpResponse.statusCode !== 404;
	};
}

function helpDispatcher(_, request, response) {
	var rel;
	var matches = /^\/help\/([^\/]*)\/([^\/]*)\/([^\/]*)\/([^\?]*)(.*)/.exec(request.url);
	if (!matches) {
		matches = /^\/help\/([^\?]*)(.*)/.exec(request.url);
		if (matches) rel = matches[1];
		else return plainReply(response, 400, "invalid help URL: " + request.url);
	} else {
		var application = matches[1];
		// ignore contract and dataset
		var remain = matches[4];
	...
	var lang = locale.current.replace('-', '_');
	var configUrl = config.help && config.help.url;
	// var provider = configUrl ? redirectProvider(configUrl) : helpProvider;
	...
	request.url = '/' + lang + '/' + rel;
	// request.url = '/' + "ENG" + '/' + rel;
	console.log(">>>> helpDispatcher: url=" + request.url);
	var resp = provider(_, request, response);
	console.log(">>>> helpDispatcher: url=" + request.url + " => " + resp);
	if (resp) return;

	// second chance with x3 language code
	request.url = '/' + locale.isoLanguageMap[locale.current] + '/' + rel;
	console.log(">>>> helpDispatcher: url=" + request.url);
	resp = provider(_, request, response);
	console.log(">>>> helpDispatcher: url=" + request.url + " => " + resp);
	if (resp) return;

	// last chance with en_US language
	request.url = '/en_US/' + rel;
	console.log(">>>> helpDispatcher: url=" + request.url);
	resp = provider(_, request, response);
	console.log(">>>> helpDispatcher: url=" + request.url + " => " + resp);
	if (resp) return;
...
function disp(request, response, _) {
	if (!_initialized) return response.end(locale.format(module, "initializing"));
	//
...
if (process.argv[2] && process.argv[2].substr(0, 1) === "N") {
	...
	exports.server = new streams.createHttpServer(disp, _makeHttpServerOptions((config.session || {}).secure));
