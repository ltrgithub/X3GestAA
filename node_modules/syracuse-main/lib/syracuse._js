...
function _enableCompression(request, response) {
	...
	response.writeHead = function(status, headers) {
		if (!headers["content-encoding"] && //
			...
			var zipper = require('zlib').createGzip({
			...
			zipWriter = require('ez-streams').devices.node.writer(zipper);
			var zipReader = require('ez-streams').devices.node.reader(zipper);
...
function disp(request, response, _) {
	...
	try {
		...
		response = _enableCompression(request, response);
