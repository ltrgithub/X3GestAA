...
var os = require('os');
require('syracuse-core').error.configure(config.system || {}); // to get safeStack
...
var staticShadowProvider = require('streamline-static').dispatcher({
	root: path.join(basePath, '../shadow-modules/node_modules'),
	maxAge: 3600 * 1000,
});
...
function patchHeaders(request, response) {
	...
	response.writeHead = function(status, headers) {
		...
		hd["x-frame-options"] = hd["x-frame-options"] || config.security.http['x-frame-options'] || 'DENY';
	...
	return function(request, response) {
		return flows.withContext(function() {
			(function(_) {
				...
				var useSessionPool = /^\/api\d+\//.test(request.url);
...
var requireDispatcher = require('streamline-require/lib/server/require').dispatcher({
	...
	whiteList: [
		/^(syracuse-(main\/html|ui|core|rtf|office|msoffice|sdata\/lib\/(parser|httpHelpers))|streamline|jsurl|js-xml|test-runner|html-escape)(\/|$)/,
...
var authenticatedRoutes = {
	...
	"syracuse-uml": htmlStaticDispatcher,
...
function initBatchMulti(_) {
	...
	if (tenants) tenants.forEach_(_, function(_, tenant) {
		console.log("Init tenant " + tenant.tenantId);
		// initialization of tenant database
		exports.initializeTenant(_, tenant.tenantId);
	});
	// restore tenantId
	globals.context.tenantId = tenantId;
}

exports.initializeTenant = function(_, tenantId) {
	globals.context.tenantId = tenantId;
	initAsync(_, true);
};
...
function initAsync(_, simplified) {
	...
	Object.keys(sdataRegistry.applications).forEach_(_, function(_, appKey) {
		var _app = sdataRegistry.applications[appKey];
		Object.keys(_app.contracts).forEach_(_, function(_, contractKey) {
			var _contract = _app.contracts[contractKey];
			if (_contract.initialize) _contract.initialize(_);
		});
	});
