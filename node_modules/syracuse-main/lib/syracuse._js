...
var helpPath = path.join(path.dirname(module.filename), "../../../../online-doc/DOCV7-X3");
...
function helpDispatcher(_, request, response) {
	// replace /help segment by locale (/la_CO) and default to /index.htm 
	var path = '/' + locale.current.replace('-', '_') + (request.url.split('?')[0].substring(5) || '/index.htm');
	var url = config.help && config.help.url;
	if (!url) {
		if (helpProvider) {
			request.url = path;
		...
		return response.end(helpProvider ? 'file not found: ' + path : 'help not configured', 'utf8');
	}
	url += path;
...
var _map = {
	...
	help: function(_, request, response) {
		return helpDispatcher(_, request, response);
	},
