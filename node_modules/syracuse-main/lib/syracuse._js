...
/** check user (and password) for different authentication methods. User name must be in 
*/
function _userCheck(session, login, password, method, standardSetting, _) {
	...
	if (users && users.length) {
      user = users[0];
	  if (user.inactive(_)) // no login for inactive users!
		user = null;
	}
	if (user) {
      var localConfig = {};
      ...
      if (localConfig.method !== method) {
        ...
        if (localConfig.ldap.inactive) { // server inactive: no authentication
