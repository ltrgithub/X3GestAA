...
function _redir(login, status) {
	...
	login = "$" + login;
	if (login in userRedirect) {
		delete userRedirect[login];
		return 1;
	} else {
		userRedirect[login] = now;
		return status;
	}
}
 ...
 * Return codes: 0 OK, 1 authentication failure, 2 no license, 3 change password, 4 user inactive, 5 no group, 6 no role
 */
	...
	if (user) {
		...
		if (user) {
			...
			if (!role) {
				...
				if (!groups.length) return _redir(login, 5);
			...
			if (!role) {
				return _redir(login, 6); // no role
			}
			if (role && !check.checkConcurrent(session, role, user.login(_), _, getDevice(), diagnoses, true)) { // try to find new session
				return 2;
			}
			...
			if (request.method === "GET") {
				...
				if (content) {
					...
					if (user) {
						...
						newUrl = "/syracuse-main/html/main.html?url=%3Frepresentation%3Dhome.%24navigation";
					}
				}
			}
		}
