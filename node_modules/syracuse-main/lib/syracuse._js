...
//				Location: request.url.replace(defaultPageExp, "/syracuse-main/html/main.html?url=" + encodeURIComponent("?representation=home.$dashboard"))
	...
	if (request.url.indexOf("/auth/changePassword") === 0)
	{
		...
		if (content) { 				
			...
			var tmpLogin = changePasswordModule.getTempLogin(query.auth);
			console.log("Login found for user "+tmpLogin);
			var user;
			if (tmpLogin && tmpLogin === query.login) {
				var db = adminHelpers.AdminHelper.getCollaborationOrm(_);
				// fetch user
			...
			if (user) {
				...
				var newUrl;
				var tmpLogin = changePasswordModule.setTempLogin(query.login)
				...
				if (query.url.substr(0,6) === "/auth/") {
					newUrl = query.url.replace(/^(\/auth\/(?:std|digest|basic|oauth2-\w*))(?:\-\-[0-9a-fA-F]*)?/, "$1"+tmpLogin);				 
				} else {
					newUrl = "/auth/std"+tmpLogin+query.url;
				}
		...
		response.writeHead(303, { "Content-Type": "text/html", "Location": "/" });
		response.end('<html><a href="/test">Link</a></html>');
	}
