...
(function($) {
	...
	that.controls.SDataFilter={
		...
		uidlg : function(data, controller) {
			...
			if (cp.$type === "application/x-boolean") {
				ui.content.value.uiClass = "CheckBox";
				ui.content.value.data.label = data.field;
				
			} else if (cp.$type === "application/x-choice") {
				var vals = [];
				cp.$value.$enum.forEach(function(value) {
					vals.push({value:value.$value, title:value.$title});
				});
				ui.content.value.uiClass = "Combo";
				ui.content.value.data.items = vals;
			}
