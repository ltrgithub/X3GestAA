...
(function($) {
			...
			saveFilter: function(after) {
				...
				if (controller && controller.parent) {
					...
					pcontroller.saveFilter({
						filters: controller.data.filters
					});
	...
	that.controls.SDataSorter = {
		...
		createSDataSorter: function(sortPageCtrl, c, filters) {
			...
			html.push('>');
			html.push('<a href="#" data-role="button" data-action="resetSort" data-iconpos="left" data-theme="f" data-inline="false"  data-mini="false" data-icon="delete">');
			html.push("Annuler le tri"); //$helpers.locale().ui.edit_item);
			html.push('</a>');	
			var options = c.data || {}, cd = sortPageCtrl.data;
			cd.items = cd.items || [];
			...
			$c.click(function(e) {
				...
				if (p == "resetSort") {
					...
					if (!status){
						...
						status.lastClicked = null;
					}else if(!status.lastClicked && status.orderBy && status.orderBy.length>0){
						// First click on item list
						status.lastClicked = target.closest("ul").find("a[data-action=" + status.orderBy + "]").closest("li");						
					}
					if (status.orderBy === p) {
					...
					if (status.lastClicked)  status.lastClicked.buttonMarkup({'icon' : ''});
					status.lastClicked = target.closest("li");
					sortPageCtrl.sortStatus = status;
					status.lastClicked.buttonMarkup({icon: that.controls.SDataSorter._statusIcon(status.asc, '')});
				}
			});
		},				
		validateSort: function(sortPageCtrl) {
			...
			$sdata.execURL(url.uri, {
				...
				success: function(cd, headers) {
					var queryPage = sortPageCtrl.parent;
					queryPage.extraData.bookmarks = headers ? headers.favorites : null;
					queryPage.extraData.sortOrderInfo = order;
					$.mobile.loading('hide');
					if (cd) {
						// refresh sdatalist 
						queryPage.data = cd;
					...
					savesettings: function(after) {
						...
						if (cd.newFilter) {
							filters.push({
								...
								op: cd.op,
								value: data.value,
								valueTitle: data.valueTitle ? data.valueTitle : data.value,
								refresh: true
							});
