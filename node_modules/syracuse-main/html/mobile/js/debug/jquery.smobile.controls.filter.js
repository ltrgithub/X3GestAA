...
(function($) {
	...
	that.controls.SDataSorter = {
		...
		createSDataSorter: function(sortPageCtrl, c, filters) {
			var html = [], options = c.data || {}, cd = sortPageCtrl.data;
			cd.items = cd.items || [];
			...
			if (ord.asc!=null){
				...
				html.push($helpers.locale().ui.cancel_sort);
				html.push('</a>');
			}
		...
		events: function($c, c) {
			...
			$c.click(function(e) {
				...
				if (p == "resetSort") {
					...
					if (status.lastClicked) status.lastClicked.buttonMarkup({
						'icon': ''
					});
					status.lastClicked = target.closest("li");
					sortPageCtrl.sortStatus = status;
					status.lastClicked.buttonMarkup({
