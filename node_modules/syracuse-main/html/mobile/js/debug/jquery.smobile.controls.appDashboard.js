...
(function($) {
	...
	that.controls.AppDashboard={
		...
		handler: function(c, after) {
			...
			html.push('<center>');
			html.push('<div id="' + c.id + '" class="s-m-dash-table" data-theme="b">');
			...
			items.forEach(function(v,i) {
				...
				itmHtml.push('<center>');
				// Image	
				var icon  =  v.data.icon || _consts.default_menu_name, css = icon.toLowerCase().split(".");
				...
				itmHtml.push('</div>');		
				if (v.data.dsdescription){
					itmHtml.push('<div class="s-m-dash-ds">');
					itmHtml.push(v.data.dsdescription);
					itmHtml.push('</div>');
				}
				itmHtml.push('</center>');
				v.data.url = that.ui.sdata_link(null, v.data);
				//v.data.jmdata={"data-theme":"b","data-role":"button"};
