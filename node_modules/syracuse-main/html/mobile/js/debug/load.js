...
$(function(){
	...
	function setJMobileEvents() {
		if (window.location.hash.indexOf("#apperrors") != 0 ){
			// Avoid clear errors on error page
			$.helpers.clearErrors();
		}
			...
			if (force) {
				...
				if (page) {
					...
					if ($foundPage && $foundPage.length) {
						...
						if ($sdata.config && page.application && (page.application != $sdata.config.id)) 
							$sdata.config = null;
						if (!$sdata.config) {
							e.preventDefault();
						...
						$.smobile.parseURL(u, page, function(href, resData, error, info) {
							...
							$.smobile.buildPage(uiId, activePage, $page, ui, resData, info, page, $ptemplate, function() {
								...
								if (controller.hasErrorView && controller.errorViewHasErrors) {
									...
									if (errctrl) {
										controller.errorInView({$element: $("#"+errctrl.id), id: errctrl.id, uiClass: "ErrorView"});
									}
								}
							});
						});
    ...
    window.onerror = function(errorMsg, url, lineNumber, error){
    	var msg = "A javascript error occured\nPlease contact the administrator\nErrorMsg : " + errorMsg + "\n"  + (error ? + JSON.stringify(error, null, 2) : "Url : " + url + "\nLineNumber : " +  lineNumber);
    	try{
    		if($ && $.mobile && $.mobile.activePage) {
    			$.helpers.pushException ({message: msg});
