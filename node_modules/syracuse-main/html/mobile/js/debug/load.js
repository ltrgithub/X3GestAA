...
$(function() {
	...
	function setJMobileEvents() {
		$(document).on("swipeleft", function(event) {
			...
			if (page) {
				var controller = $.smobile.pageController(page);
		...
		$(document).bind("pagebeforechange", function(e, data) {
			var $helpers = $.helpers; // hasErrors = $helpers.hasErrors();
			var $sdata = $.sdata;
						...
						if (!$sdata.config) {
							...
							if (npcontroller) {
								...
								if (npcontroller.needRefresh) {
									...
									if (npcontroller.child) npcontroller.doRefresh();
								}
					...
					$.smobile.loadPage(u.hash, page, function(uiId, ui, error, afterdata) {
						if (!ui) {
							...
							if ((page.page.id != $.smobile.PAGES.errors.id)) {
								if ($helpers.hasErrors()) $.smobile.showErrors();
								else {
									if (error && (typeof error === "object") && error.action) $.smobile.silentAction(error);
									else $.smobile.showError(options, error || "");
								}
							}
							return;
						}
						$sdata.startLoad();
						//if (hasErrors) $helpers.clearErrors();
						$.smobile.parseURL(u, page, function(href, resData, error, info) {
							...
							$.smobile.buildPage(uiId, activePage, $page, ui, resData, info, page, $ptemplate, function() {
								...
								controller.bindData();
								$sdata.endLoad();
								if (controller.hasErrorView && controller.errorViewHasErrors) {
									var errctrl = controller.controlByClass("ErrorView");
									if (errctrl) {
										controller.errorInView({
											$element: $("#" + errctrl.id),
											id: errctrl.id,
											uiClass: "ErrorView"
										});
		...
		$(document).bind("pagehide", function(fromPage, data) {
			...
			if (fromPage.target != data.nextPage[0]) {
				...
				if (currentController) {
					if (currentController && currentController.config && currentController.config.ui && (currentController.config.ui.indexOf(".$query") > 0)) return;
		...
		$(document).bind("pageshow", function(curPage, data) {
			var $helpers = $.helpers;
			if (!window.location.hash) return;
			var controller = $.jqmData(curPage.target, "controller");
			if (!controller) return;
			if (controller.extraData.sortorderBtnId) {
				var $c = $("#" + controller.extraData.sortorderBtnId);
				var ordInfo = controller.extraData.sortOrderInfo;
				if (ordInfo != null) {
