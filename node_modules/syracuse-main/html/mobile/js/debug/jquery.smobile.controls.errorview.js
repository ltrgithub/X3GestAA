...
(function($) {
	...
	that.controls.ErrorView = {
		...
		handler: function(c, after) {
			...
			html.push('>');
			if (false !== c.noCloseBox){
				html.push('<div class="navBar"><div id="' + c.id + '-close" class="closeBox"></div></div>');
			}
			controller.errorViewHasErrors = false;
			if (data) {
				data.forEach(function(cd){
					that.controls.ErrorView._additem(cd, html);
					controller.errorViewHasErrors = true;	
				});
				if (this.pageInfo.pageName.indexOf("apperror") !=0 ) $helpers.clearErrors();
			}
			html.push('</ul>');
		...
		refresh: function(controller, options, $c, id) {
			...
			var data = $helpers.lastMessages(), html=[], hasmsgs=false;
			controller.errorViewHasErrors = false;
			if (data) {
				data.forEach(function(cd){
					hasmsgs = true;
			...
			return hasmsgs?{$element: $c, id: id, uiClass: "ErrorView"}:null;
		},
		events: function($c, c) {
			if (true !== c.noCloseBox){
				var controller = this;
				var cb = $("#" + c.id + '-close');
				cb.click(function(e) {
					$c.empty();			
