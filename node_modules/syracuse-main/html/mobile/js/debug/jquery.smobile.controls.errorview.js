...
(function($) {
	...
	that.controls.ErrorView = {
		_additem: function(cd, html) {
			...
			html.push('>');
			html.push('<div class="'+$helpers.ui.consts.normal_icon_size+ ' ' + 's-error '+ _error2css(cd.type) + '"></div>');
		...
		refresh: function(controller, options, $c, id) {
			var data = $helpers.lastErrors(), html=[], haserrors=false;
			controller.errorViewHasErrors = false;
			if (data) {
				data.forEach(function(cd){
					haserrors = true;
			...
			return haserrors?{$element: $c, id: id, uiClass: "ErrorView"}:null;
		}
	}
})(jQuery);    
