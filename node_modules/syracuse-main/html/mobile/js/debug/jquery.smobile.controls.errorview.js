...
(function($) {
	...
	that.controls.ErrorView = {
		...
		_addCloseBox: function(id, html){
			var html = html || [];
			html.push('<div class="navBar"><div id="' + id + '-close" class="closeBox"></div></div>');
			return html;
		},
		sync: true,
		handler: function(c, after) {
			var controller = this, html = [], options= c.data || {};
			var data = $helpers.lastErrors(), errPage = this.pageInfo.pageName.indexOf("apperror")==0;
			...
			if (controller.errorViewHasErrors) {
				if (!errPage) that.controls.ErrorView._addCloseBox(c.id, html);
		...
		refresh: function(controller, options, $c, id) {
			...
			if (controller.errorViewHasErrors) {
				...
				if ($c.data('closebox')) {
					var cb = $("#" + id + '-close');
					if (cb.length==0) that.controls.ErrorView._addCloseBox(id, html);
				}
				data.forEach(function(cd){
					that.controls.ErrorView._additem(cd, html);
				});
