...
(function($) {
	...
	function _execField(item,html, options, controller, cp, cd, exprData, info) {
		...
		if ((item.$isTitleHidden === false) && item.$bind) {
			if (cp && cp.$properties && cp.$properties[item.$bind])
				title = $proto.parseExpression(cp.$properties[item.$bind].$title, {data: null, $prototype: cp},"$title", {html:true});
		...
		if (item.$cssPrefix && (typeof ev === "object")) {
			...
			if (ev.title) {
				...
				if (cp && cp.$properties && cp.$properties[item.$bind])
					html.push($proto.parseExpression(cp.$properties[item.$bind].$title, {data: null, $prototype: cp},"$title", {html:true}));
				...
				if (pi) {
					...
					if ($proto.uiCanGroupField(pi)) {
						...
						if (pi.$type === "application/x-array") 
							content[cid] = {parent: parentId,  "uiClass": "ArrayField", data:{jmdata: $proto.jqmDataArray(), facet: info.facet,  field:{$bind: item.$bind, $type: pi.$type, $title: pi.$title, $proto: pi.$item}, $article: $.extend(true, {},item)}};
					}
				}; 
			});
		}
	...
	function _layoutTabItem(item, content, options, cp, exprData, info, parentId) {
		...
		item.$items.forEach(function(ii) {
			tabs.push($proto.parseExpression(item.$title || "", exprData,"$title", {html:true}));
		});
	...
	function _layoutRowItem(item, content, options, cp, exprData, info, parentId) {
		...
		var ncid = _genContainerId(options);
		content[ncid] = {parent:parentId,  layout:"RowLayout", data:{widths: widths, responsive: item.$responsive}};
		info.root = false;
		item.$items.forEach(function(ci, index) {
			return _layoutItem(ci, content, options, cp, exprData, info, ncid + '_' + index, item);
		});
