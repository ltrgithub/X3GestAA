...
(function($) {
	...
	function  _findLayout(article, name) {
	...
	function  _defaultListArticle(proto, imageProp, info) {
		...
		if (!proto.$title && proto.$value) {
			$title = '$value'; 
		} else if (proto.$title && (proto.$title===proto.$description)) 	
			$description = "";
		...
		if ($description) 
			article.$layout.$items[0].$cards.$layout.$items.push({$bind: $description, $lines: 2});
		if (imageProp) {
			if (info.design) {
				article.$layout.$items[0].$cards.$layout.$items.push({$bind: imageProp});
			} else {
	...
	function _execRowlayout($layout,html, options, controller, cp, cd, exprData, info) {
		...
		if ($layout.$theme) pclass.push("ui-body-" + $layout.$theme.toLowerCase());
	...
	function _articleArrayComposition2Layout($article, cp) {
		var res = {cards:false,  items: []};
		var fn = [], fp = {};
		if (cp && cp.$properties) {
		...
		if ($article && ($article.$cards || $article.$layout)) {
			...
			_scanObj($article.$cards.$layout || $article.$layout, function(o) { 
				if (o.$bind) {
	...
	function _execField(item,html, options, controller, cp, cd, exprData, info) {
		...
		if ((item.$isTitleHidden === false) && item.$bind) {
			if (cp && cp.$properties && cp.$properties[item.$bind]) {
				...
				title = $proto.parseExpression(pi.$shortTitle ||  pi.$title, {data: null, $prototype: cp},"$title", {html:true});
			}
			if (title) html.push(title + ": ");
		}
		var ev =  $proto.parseExpression(exp, exprData,item.$bind || "", {html:true, list: true, onlyValue:false, keepBrackets: true, cssPrefix: item.$cssPrefix});
		if (item.$cssPrefix && (typeof ev === "object")) {
			html.push('<span class="s-indicator '+ item.$cssPrefix + '-' + ev.value+'"></span>');
			if (ev.title) {
				html.push(ev.title);
			} else {
				if (cp && cp.$properties && cp.$properties[item.$bind]) {
					pi = cp.$properties[item.$bind];
					html.push($proto.parseExpression(pi.$shortTitle ||  pi.$title, {data: null, $prototype: cp},"$title", {html:true}));
				}
	...
	function _showfieldsInParent(item, content, options, cp, exprData, info, parentId) {
		...
		if (item.$items) {
			if (_showfieldsInParent)
			item.$items.forEach(function(item) {
				var pi = cp.$properties[item.$bind];
				if (pi) {
					var cid = _genContainerId(options);
					if ($proto.uiCanGroupField(pi)) {
						content[cid] = {parent: parentId,  "uiClass": "SingleField", data:{facet: info.facet, $bind: item.$bind, $article: $.extend(true, {},item)}};
					} else {	
						if (pi.$type === "application/x-array") 
							content[cid] = {parent: parentId,  "uiClass": "ArrayField", data:{jmdata: $proto.jqmDataArray(), facet: info.facet,  field:{$bind: item.$bind, $type: pi.$type, $title: pi.$shortTitle || pi.$title, $proto: pi.$item}, $article: $.extend(true, {},item)}};
					}
				}; 
			});
		}
	...
	function _detailArticle2Design($article, content, cp, layoutInfo) {
		var options = {iid: 0};
		var expData = {data: null, $prototype: cp};
	...
	function _layoutTabItem(item, content, options, cp, exprData, info, parentId) {
		...
		item.$items.forEach(function(ii) {
			tabs.push($proto.parseExpression(ii.$title || "", exprData,"$title", {html:true}));
		});
		var ncid = _genContainerId(options);
		content[ncid] = {parent:parentId,  layout:"TabsLayout", data:{tabs: tabs}};
		info.root = false;
	...
	function _layoutRowItem(item, content, options, cp, exprData, info, parentId) {
		...
		var ncid = _genContainerId(options);
		content[ncid] = {parent:parentId,  layout:"RowLayout", data:{widths: widths, responsive: item.$responsive, theme: item.$theme}};
		info.root = false;
		item.$items.forEach(function(ci, index) {
			return _layoutItem(ci, content, options, cp, exprData, info, ncid + '_' + index, item);
		});
	...
	function _layoutItem(item, content, options, cp, exprData, info, parentId, parent) {
		...
		var layoutType = item.$layoutType, dd, ncid;
		...
		if (item.$category){
			...
			dd.text = $proto.parseExpression(item.$title, exprData,"$title", {html:true});
			if (item.$collapsed) dd.collapsed = true;
			ncid = _genContainerId(options);
			content[ncid] = {parent:parentId,  layout:"CollapsibleLayout", data:dd};
			info.root = false;
