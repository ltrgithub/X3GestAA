	...
	_controls.ArrayObjectRef={
		_renderContent: function(controller, options, cp, id, html, fieldData, createList) {
			...
			fieldData.forEach(function(value, index){
				if (options.cards)
					_controls.types._cards(html, options.items|| [], cp, value, controller,true,index, options.bind);
				else 
					_controls.types._table_row(html, options.items|| [], cp, value, controller, true,index);
			});
			if (!options.cards) {
