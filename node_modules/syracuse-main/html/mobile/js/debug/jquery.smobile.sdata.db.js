...
(function($) {
	...
	var _private = {
		...
		_installApp: function(db, data, cb) {
			...
			db.transaction(function(tx) {
				var _after = function() {
					tx.executeSql("INSERT INTO apps(id, url, title, description, icon, baseURL, version, data) values(?,?,?,?,?,?,?,?)", [data.id, data.url, data.title, data.description, data.icon, data.baseURL, data.version, JSON.stringify(data.data)],
