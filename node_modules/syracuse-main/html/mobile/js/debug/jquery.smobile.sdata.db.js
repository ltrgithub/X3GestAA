...
(function($) {
	...
	var _getKey=function(representation,data){
		var proto = $.sdata.config.pages[representation].$prototype;
		...
		var key = $.proto.parseExpression(proto.$key, {data: $.proto.exprData(data), $prototype:proto},"$key");
		return key;
	}
	...
	var _indexDB = {
			...
			request.onsuccess= function(event) {
				if (event.target.result) {	
					...
					if (purl.template) {
						...
						purl.template = false;
						debugger;
						purl.id = _getKey(purl.query.representation, cdata.data);
					}
		...
		_markSdataSeleted: function(o_url, sdataObj, odata, cb) {
			...
			if (purl.template) {
				createActivity = true
				purl.template = false;
				purl.id = _getKey(purl.query.representation, odata);
			}
			delete purl.query;
			purl.createActivity = false;
			purl.editActivity = false;
