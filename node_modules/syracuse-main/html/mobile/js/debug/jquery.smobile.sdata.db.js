	...
	*/	 
	...
	var _indexDB = {
		...
		_getSDataByUrl: function(url, objectstore, cb) {
			var sdr = objectstore.get(url +"?appid=" + $.sdata.config.id);
		...
		_rmvSDataByUrl: function(url, objectstore, cb) {
			var sdr = objectstore["delete"](url +"?appid=" + $.sdata.config.id);
			sdr.onerror=function(event) {cb(null, event);};
		...
		_deleteDataByDraft:  function(db, id, cb) {
			var options = {success: cb};
			if (!_checkDBHandler(db, options)) return;
			tx =  db.transaction(["draft", "sdata"], IDBTransaction.READ_WRITE); 
			try {
				var objectStore = tx.objectStore("draft");  
				var request = objectStore.get(id); 
				request.onsuccess = function(event) {  	
		...
		//oK
		_uninstallDraft:  function(draft, sdata,id, options) {
			options = options || {};
			try {	
				...
				request.onsuccess = function(event) {  
					if (event.target.result) {
						...
						_indexDB._uninstallSdata(sdata, id, options);
				};
				request.onerror = function(event) {  
					_indexDB._uninstallSdata(sdata, id, options);
				}
		...
		_uninstallSdata:  function(sdata,  id, options) {
			options = options || {};
		...
		_removeFromDB: function(db, id, options) {
			if (!_checkDBHandler(db, options)) return;
			...
			_indexDB._uninstallDraft(draft, sdata,  id, {
				success: function(){
	...
	var _private = {
		...
		_getSDataByUrl: function(url, tx, cb) {
			tx.executeSql('SELECT * FROM sdata where url=?', [url +"?appid=" + $.sdata.config.id],
				function (tx, results) {
		...
		_rmvSDataByUrl: function(url, tx, cb) {
			...
			tx.executeSql('DELETE FROM sdata where url=?', [url +"?appid=" + $.sdata.config.id],
				function (tx, results) {
					 cb(null);
				}, 
				function(tx, error) {
									...
									if (data) {
										...
										if ((data.status === $.sdata.STATUS.ORIGINAL) || (data.status === $.sdata.STATUS.MODIFIED)) {
											...
											if (JSON.stringify(insData.original_data) === JSON.stringify(insData.data)) {
												...
												if (!$.sdata.store(insData.url)) {
													// delete sdata record & delete draft
													return _private._rmvSDataByUrl(insData.url, sdataObj, function(data, error) {	
		...
		_loadDraft : function(db, id, cb) {
			var options = {success: cb};
			if (!_checkDBHandler(db, options)) return;
			_private._execOpen(db, 'SELECT * FROM draft where id=?', [id], { success: function (rows) {
				if (rows.length) {
					var row = rows.item(0);
					cb({
							appid: row.appid,
		...
		_getModifiedData:  function(db, purl, cb) {
			...
			_private._execOpen(db, 'SELECT * FROM sdata where status > ? and appid = ? and entity_url = ?', 
				[$.sdata.STATUS.ORIGINAL, appid, eu], { 
					success: function (rows) {
						if (rows && rows.length) {
	...
	IndexedDBHandler.prototype = {
		...
		removeFromDB: function(id, options) {
			var self = this;
			self.check(function(db) { _indexDB._removeFromDB(db, id, options);});
		},
	...
	DataBaseHandler.prototype = {
		...
		removeFromDB: function(id, options) {
			var self = this;
			self.check(function(db) { _private._removeFromDB(db, id, options);});
		},
	...
	if (indexedDB && !window.openDatabase) {
		...
		$.sdata.removeFromDB = function(id, options) { idb.removeFromDB(id, options); };
		...
		$.sdata.removeFromDB = function(id, options) { mobiledb.removeFromDB(id, options); };
	} 
