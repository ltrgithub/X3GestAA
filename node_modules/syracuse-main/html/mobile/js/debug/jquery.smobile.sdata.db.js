...
(function($) {
	...
	var _indexDB = {
		...
		_loadFavoriteList: function(db, options, cb) {
			...
			//console.log("_loadFavoriteList" + JSON.stringify(options, null, 2));
		//_indexDB._scanFavs(db);
			if (options.entity_url) {
				var favs = [];
				index = fav.index("entity_url");
				var url = options.entity_url + "&appid=" + options.appid;
				//console.log("\t-> entity_url -> " + url);
		...
		_insertFav: function(db, appid, entity_url, data, cb) {
			try {
				//console.log("_insertFav" + JSON.stringify(data, null, 2));
				...
				request.onsuccess = function(event) {
					//console.log("\Ok " + JSON.stringify(event.target.result, null, 2));
					if (event.target.result) {
						var cr = event.target.result;
		...
		_scanFavs:function(db,cb){
			if (!_checkDBHandler(db)) return;
			var tx = db.transaction(["sdatafav"], IDBTransaction.READ_ONLY);
			var fav = tx.objectStore("sdatafav");
		    //console.log("START _scanFavs");
			var c =fav.openCursor();
			c.onsuccess = function(event) {
				  var cursor = event.target.result;
				  //console.log(JSON.stringify(cursor,null,2));
				  if (cursor) {
					    //console.log(cursor.key + " - " + JSON.stringify(cursor.value,null,2));
					    cursor["continue"]()
				  }
				  else {
				    //console.log("STOP _scanFavs OK");
				  }
				};	
			c.onerror = function(event) {
			    //console.log("STOP _scanFavs ERROR " + + event.target.errorCode);
				};		
		},
		_switchFav: function(db, url, data, cb) {
			//console.log("_switchFav" + JSON.stringify(data, null, 2));
			//_indexDB._scanFavs(db);
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction(["sdatafav"], IDBTransaction.READ_ONLY);
			var fav = tx.objectStore("sdatafav");
			...
			request.onsuccess = function(event) {
				//console.log("\treadOK" + JSON.stringify(event.target.result, null, 2));
				if (event.target.result) {
					...
					rd.onsuccess = function(event) {
						//console.log("\tdeleteOK" + _extractkey(cd.url));
						cb({
							op: 2,
					...
					rp.onsuccess = function(event) {
						//console.log("\taddOK" + JSON.stringify(event.target.result, null, 2));
						cb({
							op: 1,
							key: _extractkey(cd.url)
						}); //inserted
				...
				if (purl.id || purl.template) {
					...
					request.onsuccess = function(event) {
						...
						if (cr) {
							...
							if (purl.pgroupBy) for (var ii in gvalues) result.$resources.push(gvalues[ii]);
		...
		_updData: function(db, oldUrl, data, cb) {
			...
			var _updSdata = function() {
				if ((data.status === $sdata.STATUS.ORIGINAL) && !$sdata.store(oldUrl)) return _indexDB._rmvSDataByUrl(oldUrl, sdata, function(dd, error) {
					cb(null, error);
				});
				else return _indexDB._updSDataByUrl(oldUrl, data, sdata, function(dd, error) {
					cb(null, error);
				});
		...
		_createDraft: function(db, data, cb) {
			...
			try {
				...
				request.onsuccess = function(event) {
					if (event.target.result) {
						var cr = event.target.result;
						if (cr.value.appid === data.appid) cr["delete"]();
						cr["continue"]();
					} else _indexDB._insertDraft(db, tx, data, cb);
				};
				request.onerror = function(event) {
					if (event.target.errorCode == IDBDatabaseException.NOT_FOUND_ERR) {
	...
	var _private = {
			...
			if (window.openDatabase) {
				...
				if (!db) {
					$helpers.pushException({
						message: $helpers.locale().ui.no_db_support
					}, $helpers.locale().ui.no_db_support_sol);
					return cb(null);
				}
		...
		_getSyncData: function(db, count, cb) {
			...
			if (count) {
				...
				_private._execOpen(db, 'SELECT * FROM sdata where status > ? and appid = ?', [$sdata.STATUS.ORIGINAL, appid], function(rows, error) {
					if (error) return cb(null);
					if (rows && rows.length) {
						for (var i = 0, len = rows.length; i < len; i++) {
							var row = rows.item(i);
	...
	IndexedDBHandler.prototype = {
		...
		getSyncData: function(cb) {
			var self = this;
			self.check(function(db) {
				_indexDB._getSyncData(db, false, cb);
			});
		},
	...
	DataBaseHandler.prototype = {
		...
		getSyncData: function(cb) {
			var self = this;
			self.check(function(db) {
				_private._getSyncData(db, false, cb);
			});
		},
		getSyncCount: function(cb) {
			var self = this;
