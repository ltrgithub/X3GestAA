...
(function($) {
	...
	*/	 
			...
			try {	
				if (purl.id ||  purl.template) {
					...
					request.onsuccess = function(event) {  	
						var cr = event.target.result;
						if (cr) {
							if ((cr.value.appid === $sdata.config.id) && (cr.value.status < $sdata.STATUS.DELETED)) 
		...
		_getSData:  function(db, purl, cb) {
			...
			try {	
				...
				if (purl.id ||  purl.template) {
					...
					db.readTransaction(function(tx) {
						var smap = $sdata.url2search(purl);
						tx.executeSql('SELECT * FROM sdata where entity_url=? and  appid=? and status<?', [su, $sdata.config.id, $sdata.STATUS.DELETED],function (tx, results) {
							var rows = results.rows;
							if (rows.length)  {
	...
	IndexedDBHandler.prototype = {
		...
		getSData:  function(purl, cb) {
			var self = this;
			self.check(function(db) {
				_indexDB._getSData(db, purl, cb);
			});
		},
	...
	DataBaseHandler.prototype = {
		...
		getSData:  function(purl, cb) {
			var self = this;
			self.check(function(db) { _private._getSData(db, purl, cb);});
		},
		initData:  function(config, data, doclean, cb) {
			var self = this;
