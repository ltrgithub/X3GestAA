...
(function($) {
	...
	var _addResource = function(row, data, resources,purl, values, map) {
		if (data) {
			/* apply where */
					...
					request.onsuccess = function(event) {  	
						...
						if (cr) {
							if ((cr.value.appid === $.sdata.config.id) && (cr.value.status < $.sdata.STATUS.DELETED)) 
								_addResource(cr.value, cr.value.data, result.$resources, purl, gmap, gvalues);
							cr["continue"]();
						} else {
							if (purl.pgroupBy) 
						...
						tx.executeSql('SELECT * FROM sdata where entity_url=? and  appid=? and status<?', [su, $.sdata.config.id, $.sdata.STATUS.DELETED],function (tx, results) {
							...
							if (rows.length)  {
								for (var jj =0, ll= rows.length; jj<ll; jj++) {
									...
									_addResource(row, JSON.parse(row.data), result.$resources, purl, gmap, gvalues);
								}
								if (purl.pgroupBy) 
									for (var ii in gvalues) result.$resources.push(gvalues[ii]);	
								_applySort(purl,result.$resources); 
