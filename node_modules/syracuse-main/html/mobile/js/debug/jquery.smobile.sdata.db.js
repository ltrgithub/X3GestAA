...
(function($) {
	...
	var _tables = {
		...
		apps: {columns:{
					...
					description:  {def: "description TEXT"},  /* application description  */
					dsdescription:  {def: "dsdescription TEXT"},  /* endpoint desscription */
	...
	*/	 
	...
	var _indexDB = {
		...
		_createObjectStore: function(db) {
			...
			objectStore = db.createObjectStore("sdatafav", { keyPath: "url" });
			objectStore.createIndex("appid", "appid", { unique: false }); 
			objectStore.createIndex("entity_url", "entity_url", { unique: false }); 
		...
		_loadFavoriteList:  function(db, options, cb) {
			...
			if (options.entity_url) {
				...
				keyRange = IDBKeyRange.only(options.entity_url);
				request = index.openCursor(keyRange);  
				request.onsuccess = function(event) {  
					if (event.target.result) {
						var cr = event.target.result;
						if (cr.value.appid === options.appid);
			...
			request.onsuccess= function(event) {
				if (event.target.result) {
					...
					rd.onsuccess=function(event) {
						cb({op:2, key: _extractkey(cd.url)}); //deleted
					};			
				} else {
					//do insert 
					tx = db.transaction(["sdatafav"], IDBTransaction.READ_WRITE);
					...
					rp.onsuccess=function(event) {
						cb({op:1, key: _extractkey(cd.url)}); //inserted
					};			
				}
			}
		}, 
		...
		_getSData: function(db, purl, fav, cb) {	
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction([(fav?"sdatafav":"sdata")], IDBTransaction.READ_ONLY), request; 
			var gmap=[], gvalues = {};
				...
				if (purl.id ||  purl.template) {
					...
					request.onsuccess = function(event) {  	
						...
						if (cr) {
							...
							cb(result, {status:200, favorites: favs});
						}
					};
					request.onerror = function(event) {  
		...
		_getAppConfig: function(db, appId, cb) {
			...
			request.onsuccess = function(event) {  	
				...
				if (cr) {
					...
					res.$info = {url: cr.url, version: cr.version, title:cr.title, icon: cr.icon,  description:cr.description, dsdescription:cr.dsdescription};
	...
	var _private = {
					...
					if (rows && rows.length) {
						...
						_private._rmvFav(tx, data.url, function(d, error){
							if (error)
								...
								cb({op:2, key: _extractkey(data.url)}); 
						});
						...
						_private._insFav(tx, data, function(d, error){
							if (error)
								...
								cb({op:1, key: _extractkey(data.url)}); 
						});
		...
		_createtables: function(db, tx, cb) {
			if (!_checkDBHandler(db, cb)) return;
			var _exec = function(tx) {
				$helpers.each(["draft", "sdata", "apps", "sdatafav"], function(table, index, next) {
		...
		_getSData:  function(db, purl, fav, cb) {
			...
			try {	
				...
				if (purl.id ||  purl.template) {
					...
					db.readTransaction(function(tx) {
						var smap = $sdata.url2search(purl), favs = fav?[]:null;
		...
		_installApp: function(db, data, cb) {
			...
			db.transaction(function(tx) {
				var _after = function() {
					tx.executeSql("INSERT INTO apps(id, url, title, description, icon, baseURL, version, data, dsdescription) values(?,?,?,?,?,?,?,?,?)", 
						[data.id, data.url, data.title, data.description, data.icon, data.baseURL, data.version, JSON.stringify(data.data), data.dsdescription],
						function (tx, result) { 
							cb();
						}, function(tx, error) {
							$helpers.pushdbError(error);
			...
			db.transaction(function(tx) {
				...
				var _removeApp =  function() {
					return _private._execSQL(db, tx, 'DELETE from apps where id=?', [id], {
						success: function(){ _removeFav();},
						fail: function(){ _removeFav();}
					});
				};
			...
			_private._execOpen(db, 'SELECT * FROM apps where id=?', 
				[appId], function (rows, error) {
					...
					if (rows && rows.length) {
						...
						cdata = cdata || {};
						cdata.id = cr.id;
						cdata.$info = {url: cr.url, version: cr.version, title:cr.title, icon: cr.icon, description:cr.description, dsdescription:cr.dsdescription};
	...
	IndexedDBHandler.prototype = {
		...
		insData:  function(url, data, cb) {
			var self = this;
			self.check(function(db) { _indexDB._insData(db, url, data, cb);});
		},
		insertFav: function(appid, entity_url, data, cb) { 
			var self = this;
			self.check(function(db) { _indexDB._insertFav(db, appid, entity_url, data, cb);});
		},
		switchFav:  function(url, data, cb) {
			var self = this;
			self.check(function(db) { _indexDB._switchFav(db, url, data, cb);});
		},
		loadFavoriteList: function(options, cb) { 
			var self = this;
			self.check(function(db) { _indexDB._loadFavoriteList(db, options, cb);});
		}
	...
	DataBaseHandler.prototype = {
		...
		getSData:  function(purl, useFav, cb) {
			var self = this;
			self.check(function(db) { _private._getSData(db, purl, useFav, cb);});
		},
		initData:  function(config, data, doclean, cb) {
			var self = this;
		...
		switchFav:  function(url, data, cb) {
			var self = this;
			self.check(function(db) { _private._switchFav(db, url, data, cb);});
		},
		insertFav: function(appid, entity_url, data, cb) { 
			var self = this;
			self.check(function(db) { _private._insertFav(db, appid, entity_url, data, cb);});
		},
		
		loadFavoriteList: function(options, cb) { 
			var self = this;
			self.check(function(db) { _private._loadFavoriteList(db, options, cb);});
		}
	};
	if (indexedDB && !window.openDatabase) {
		...
		$sdata.getSData = function(url, useFav,  options) { mobiledb.getSData(url, useFav, options); };
		$sdata.initData = function(config, data, doclean, cb) {
