...
(function($) {
	...
	var _tables = {
		...
		sdatacache: {columns:{
					id:  {def: "id TEXT"}, /*key path */
					appid:  {def: "appid TEXT"}, /*application id */
					entity:  {def: "entity TEXT"}, /* used for select */
					data:  {def: "data TEXT"}
				}
		},
		apps: {columns:{
	...
	var _acceptStatut = function(row) {
		if (row.status >= $sdata.STATUS.DELETED)
			return false;
		return true;
	};	
	...
	var _addResource = function(row, data, resources,purl, values, map, searchmap) {
		if (data) {
			/* apply where */
			if (!$sdata.execWhere(data, purl.pwhere)) return;
			if (searchmap && !$sdata.execSearch(data, searchmap, purl.query.search)) return;
			/* get selected */
			var selected= data; 
			if (purl.pgroupBy) {
	...
	*/	 
	...
	var _addInfoPlus = function(purl, row, data, proto) {
		...
		data.$info.title = $proto.parseExpression(proto.$value, {data: $proto.exprData(data), $prototype: proto}, "$value");
		return data;
	};
	...
	var _indexDB = {
		...
		_createObjectStore: function(db) {
			...
			if(db.objectStoreNames.contains("sdatacache")) db.deleteObjectStore("apps");
			...
			objectStore = db.createObjectStore("sdatacache", { keyPath: "id" });
			objectStore.createIndex("appid", "appid", { unique: false }); 
		...
		_openDB: function(ccb) {
			...
			try {
				...
				$helpers.pushException(ex,$helpers.locale().ui.no_db_support_sol);
				return cb(null, ex);			
			}
			try {
		...
		_insData:  function(db, url, data, cb) {	
			...
			return _indexDB._insertSdata({data: data, url: url}, sdata, cb)
		}, 
		...
		_deleteDataByUrl:  function(db, url, odata, cb) {
			...
			_indexDB._markSdataDeleted(url,sdataObj, odata, function(data, error){
				cb(null);
			});
			...
			try {
				...
				request.onsuccess = function(event) {  	
					if (event.target.result) {	
						...
						var sdataObj = tx.objectStore("sdata");
						_indexDB._markSdataDeleted(url,sdataObj, (cdata.original_data?cdata.original_data:cdata.data), function(data, error){
		...
		//ok
		_uninstallSdataCache: function(sdata, sdatacache,id, cb) {
			try {	
				var index = sdatacache.index("appid"); 
		...
		//ok
		_uninstallDraft:  function(draft, sdata, sdatacache,id, cb) {
			try {	
				...
				request.onsuccess = function(event) {  
					if (event.target.result) {
						...
						_indexDB._uninstallSdataCache(sdata, sdatacache, id, cb);
				};
				request.onerror = function(event) {  
					_indexDB._uninstallSdataCache(sdata, sdatacache, id, cb);
				}
		...
		//ok
		_getSData: function(db, purl, cache, cb) {	
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction([(cache?"sdatacache":"sdata")], IDBTransaction.READ_ONLY), request; 
			var gmap=[], gvalues = {};
			var su = $sdata.url2local(purl, cache);
			var objectStore = tx.objectStore((cache?"sdatacache":"sdata")); 
			try {	
				if (purl.id ||  purl.template) {
					request =  objectStore.get(su);
					request.onsuccess = function(event) {
						if (event.target.result   && (event.target.result.status < $sdata.STATUS.DELETED)) 
							cb(_addInfo(event.target.result, event.target.result.data),  {status:200});
						else
							cb(null, {status:404});
					};
					request.onerror= function(event) {	
					...
					var index = objectStore.index(cache?"entity":"entity_url"); 
					var keyRange = IDBKeyRange.only(su);
					request = index.openCursor(keyRange);  
					request.onsuccess = function(event) {  	
						var cr = event.target.result;
						if (cr) {
							if ((cr.value.appid === $sdata.config.id) && (cache || (cr.value.status < $sdata.STATUS.DELETED))) 
								_addResource(cr.value, cr.value.data, result.$resources, purl, gmap, gvalues, smap);
							cr["continue"]();
						} else {
							if (purl.pgroupBy) 
				...
				request.onsuccess = function(event) {
					...
					if (cr) {
						if (cr.value.appid === appid) {
							...
							_addInfoPlus(purl, cr.value, rdata, $sdata.url2proto(purl));
							list.push(rdata);
						}
						cr["continue"]();  
					} else {
		...
		_removeSdataCache: function(sdatacache, entity, appid, cb) {
			try {	
				var index = sdatacache.index("entity"); 
				var keyRange = IDBKeyRange.only(entity);
				var request = index.openCursor(keyRange);  
				request.onsuccess = function(event) {  
					if (event.target.result) {
						var cr = event.target.result;
		...
		_addCache: function(db, appid, entity, data, cb){
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction(["sdatacache"], IDBTransaction.READ_WRITE);
			...
			var fail = false;
			var sdatacache = tx.objectStore("sdatacache"); 
			return _indexDB._removeSdataCache(sdatacache,entity, appid, function(){
				data.forEach(function(value) {
					if (fail) return;
					var cd = {data: value, appid: appid, entity: entity, id: $helpers.uuid()};
					var request = sdatacache.add(cd);
					request.onerror = function(event) {
						fail = true;
					};
				});
		...
		_clearSdata: function(db, id, tx, cb) {
			if (!_checkDBHandler(db, cb)) return;
			if (!tx) 
				tx = db.transaction(["sdata"], IDBTransaction.READ_WRITE);
			var sdata = tx.objectStore("sdata");
			_indexDB._uninstallSdata(sdata, id, cb);
		},
		//okk
		_removeFromDB: function(db, id, cb) {
			if (!_checkDBHandler(db, cb)) return;
			var tx = db.transaction(["apps", "draft", "sdata", "sdatacache"], IDBTransaction.READ_WRITE);
			var sdata = tx.objectStore("sdata");
			var draft = tx.objectStore("draft");
			var sdatacache = tx.objectStore("sdatacache");
			_indexDB._uninstallDraft(draft, sdata, sdatacache,  id, function(){
		...
		_initData:  function(db, config, data, doclean, cb) {	
			...
			data.forEach(function(value, index) {
				if (fail) return;
				var cd = {data: value, appid: config.id, url: $sdata.data2url(config.url, value, config.$prototype)};
				_checkSdataData(cd);
	...
	var _private = {
		...
		_insData:  function(db, url, data, cb) {	
			...
			db.transaction(function(tx) {
				return _private._insertSdata({data: data, url: url}, tx, cb)
			}, function(tx, error) {		
		...
		//ok
		_rmvEntityCache: function(appid, entity, tx, cb) {
			tx.executeSql('DELETE from sdatacache where appid=? and entity=?', [appid, entity],
				function (tx, results) {
					cb();
				}, 
				function(tx, error) {
		...
		_addEntityCache: function(appid, entity, resources, tx) {
			_private._rmvEntityCache(appid, entity, tx,  function() {
				var sql = 'INSERT INTO sdatacache(id, appid, entity, data) values(?, ?,?,?)';
				resources.forEach(function(data) {
					tx.executeSql(sql, 
						[
							$helpers.uuid(), appid, entity, (data?JSON.stringify(data):"")
						],
						function (tx, results) {
						}, 
						function(tx, error) {
		...
		_addCache: function(db, appid, entity, data, cb){
			if (!_checkDBHandler(db, cb)) return;
			db.transaction(function(tx) {
				_private._addEntityCache(appid, entity, data, tx);
			}, function(error) {
				cb();
			}, function() {
				cb();
		...
		_createtables: function(db, tx, cb) {
			if (!_checkDBHandler(db, cb)) return;
			var _exec = function(tx) {
				$helpers.each(["draft", "sdata", "apps", "sdatacache"], function(table, index, next) {
		...
		_openDB: function(cb) {
			if (window.openDatabase) {
				...
				if (!db)  {
					$helpers.pushException({message: $helpers.locale().ui.no_db_support}, $helpers.locale().ui.no_db_support_sol);
					return cb(null);			
				}
		...
		_insertDraft : function(db, tx, data, cb) {	
			if (!_checkDBHandler(db, cb)) return;
			_private._execSQL(db, tx, 'INSERT INTO draft (id, appid, original_url, original_data,  data) values(?,?,?,?,?)', 
				[data.id, data.appid  , data.original_url, 
		...
		_deleteDataByUrl:  function(db, url, odata, cb) {
			...
			db.transaction(function(tx) {
				_private._markSdataDeleted(url,tx, odata, function(data, error){
					cb(null);
				});
			}, _failTran);
		},
			...
			db.transaction(function(tx) {
				tx.executeSql('SELECT * FROM draft where id=?', [id],function (tx, rows) {
					if (rows && rows.rows && rows.rows.length) {
						...
						var url = cdata.original_url;
						_private._markSdataDeleted(url,tx, (cdata.original_data?cdata.original_data:cdata.data), function(data, error){
		...
		_getSData:  function(db, purl, cache, cb) {
			...
			try {	
				...
				var su = $sdata.url2local(purl, cache);
				if (purl.id ||  purl.template) {
					db.readTransaction(function(tx, cache) {
						tx.executeSql('SELECT * FROM sdata where url=? and status<?', [su, $sdata.STATUS.DELETED],function (tx, results) {
							var rows = results.rows;
							if (rows.length) 
								cb(_addInfo(rows.item(0), JSON.parse(rows.item(0).data)), {status: 200});		
							else
								cb(null, {status: 404});
						});
					}, function() {
					...
					db.readTransaction(function(tx) {
						...
						if (cache) {
							sql = 'SELECT * FROM sdatacache where entity=? and  appid=?';
						...
						tx.executeSql(sql, params,function (tx, results) {
							...
							if (rows.length)  {
								for (var jj =0, ll= rows.length; jj<ll; jj++) {
									...
									_addResource(row, JSON.parse(row.data), result.$resources, purl, gmap, gvalues, smap);
								}
								if (purl.pgroupBy) 
									for (var ii in gvalues) result.$resources.push(gvalues[ii]);	
								_applySort(purl,result.$resources); 
					...
					[$sdata.STATUS.ORIGINAL, appid], function (rows, error) {
						...
						if (rows && rows.length) {
							for (var i = 0, len=rows.length; i<len; i++) {
								...
								_addInfoPlus(purl, row, rdata, $sdata.url2proto(purl));
								list.push(rdata);
							}
							cb(data);
						} else cb(null);
		...
		_removeFromDB: function(db, id, cb) {	
			...
			db.transaction(function(tx) {
				var _removeCache =  function() {
					return _private._execSQL(db, tx, 'DELETE from sdatacache where appid=?', [id], {
						success: function(){ cb();},
						fail: function(){ 
							cb();
						}
				...
				var _removeApp =  function() {
					return _private._execSQL(db, tx, 'DELETE from apps where id=?', [id], {
						success: function(){ _removeCache();},
						fail: function(){ _removeCache();}
					});
				};
		...
		_saveAppConfig: function(db, data, cb) {
			if (!_checkDBHandler(db, cb)) return;
			_private._execOpen(db, 'SELECT * FROM apps where id=?', 
				[data.id], function (rows, error) {
					if (error) cb(null);
					if (rows && rows.length) {
						var cr = rows.item(0);
						delete data.$info;
						var cid = data.id;
						_private._execSQL(db, null, 'UPDATE apps set data=? where id=?', 
							[JSON.stringify(data), cid], {success: cb});
					} else 
						cb(null);
				}
			);
	...
	IndexedDBHandler.prototype = {
		...
		check: function(dbcall, open) {
			...
			if (open) {
				return _indexDB._openDB(function(db, errors) {
					if (!db) 
						$helpers.pushdbError({message: $helpers.locale().ui.open_db_error});
					dbcall(db);
				});
			}
			if (!self.db)  {
		...
		getSData:  function(purl, cache, cb) {
			var self = this;
			self.check(function(db) {
				_indexDB._getSData(db, purl, cache, cb);
			});
		},
		...
		clearSdata: function(id, cb) {
			var self = this;
			self.check(function(db) { _indexDB._clearSdata(db, id, null, cb);});
		},
		addCache: function(appid, entity, data, cb) { 
			var self = this;
			self.check(function(db) { _indexDB._addCache(db, appid, entity, data, cb);});
		...
		saveAppConfig: function(data, cb) {
			var self = this;
			self.check(function(db) { _indexDB._saveAppConfig(db, data, cb);});
		},
		...
		insData:  function(url, data, cb) {
			var self = this;
			self.check(function(db) { _indexDB.insData(db, url, data, cb);});
		}
	...
	DataBaseHandler.prototype = {
		...
		check: function(dbcall, open) {
			...
			if (open) {
				return _private._openDB(function(db,errors) {
					if (!db) $helpers.pushdbError({message: $helpers.locale().ui.open_db_error});
					dbcall(db);
				});
			}
			if (!self.db)  {
				_private._openDB(function(db,errors) {
					self.db = db;
					if (!self.db) $helpers.pushdbError({message: $helpers.locale().ui.open_db_error});
					dbcall(self.db);
				});
			} else dbcall(self.db);
		},
		...
		getSData:  function(purl, cache, cb) {
			var self = this;
			self.check(function(db) { _private._getSData(db, purl, cache, cb);});
		},
		initData:  function(config, data, doclean, cb) {
			var self = this;
			self.check(function(db) { _private._initData(db, config, data, doclean, cb);});
		},
		delData:  function(url, cb) {	
			var self = this;
		...
		clearSdata: function(id, cb) {
			var self = this;
			self.check(function(db) { _private._clearSdata(db, id, null, cb);});
		},
		addCache: function(appid, entity, data, cb) { 
			var self = this;
			self.check(function(db) { _private._addCache(db, appid, entity, data, cb)});
		},
		getAppConfig: function(appId, cb) {
			var self = this;
		...
		saveAppConfig: function(data, cb) {
			var self = this;
			self.check(function(db) { _private._saveAppConfig(db, data, cb);});
		},
		getSyncData:  function(cb) {	
			var self = this;
		...
		insData:  function(url, data, cb) {
			var self = this;
			self.check(function(db) { _private._insData(db, url, data, cb);});
		}		
	...
	if (indexedDB && !window.openDatabase) {
		...
		$sdata.getSData = function(url, cache, options) { 
			idb.getSData(url, cache, options); 
		};
		...
		$sdata.saveAppConfig = function(data, cb) { idb.saveAppConfig(data, cb); };
		$sdata.removeFromDB = function(id, cb) { idb.removeFromDB(id, cb); };
		$sdata.clearSdata = function(id, cb) { idb.clearSdata(id, cb); };
		...
		$sdata.getSData = function(url, cache,  options) { mobiledb.getSData(url, cache, options); };
		$sdata.initData = function(config, data, doclean, cb) {
		...
		$sdata.saveAppConfig = function(data, cb) { mobiledb.saveAppConfig(data, cb); };
		$sdata.removeFromDB = function(id, cb) { mobiledb.removeFromDB(id, cb); };
		$sdata.clearSdata = function(id, cb) { mobiledb.clearSdata(id, cb); };
		$sdata.addCache = function(appid, entity, data, cb) { 
