(function($) {
	...
	(function ($sdata) {
		...
		*/
		...
		$.helpers.extend(Operator.prototype, {toString: function() { return this.text;} });
		/**
		* @constructor
		*/
		function Operators(){
		...
		*/
		function _Parser(){
			...
			self.parse = function(str, entity, ecache){
				if (str == null || str.length == 0) {
					return null;
				}
				var tokens = Tokenizer.tokenize(str);
				tokens.forEach(function(token) {
					if ((token.type == tokenType.identifier) && (token.value.indexOf(".") > 0)) 
						$.sdata.parseField(token.value, entity, ecache);
				});
				var tokenIndex = [];
				tokenIndex[0] = 0;
		...
		$sdata.parseWhere= function(str, entity, ecache) {
			return Parser.parse(str, entity, ecache);
		};
		function _exec(data, exp){
					...
					if (exp.type == "operator") {
						switch (exp.value.code) {
							...
							case "=":
								...
								break;
							case ".":
								_exec(data, exp.children[0]);
								_exec(exp.children[0].property, exp.children[1]);
		...
		$sdata.execWhere= function(dataContext, exp) {
			if (exp == null) return true;
			_exec(dataContext, exp);
			return exp.property;
		}
