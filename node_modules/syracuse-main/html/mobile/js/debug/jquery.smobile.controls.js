(function($) {
		...
		if (options.url || options.hash || options.action) {
			...
			if (options.action){
				...
				if (options.linkType == "$external") {
					...
					//alert($.helpers.encodeJsmQuery('?url='+encodeURIComponent(options.url)));
	...
	that._createLink = function(html, options, controller, afterCreatelink) {
		var  layout = options.layout, ct, render;
		var _continue = function() {	
			if (options.addend) {
				if (layout) {
					if (layout.type) { 
						render = that.renders[layout.type];
						if (render) {
							render.addDataAndCss(options.data, controller, html, options);
						}
					} else {
						$.helpers.addJqmData(layout,html); 
						var css = [];
			...
			var ccss = [];
			if (options.disabled) ccss.push("ui-disabled")
			if (options.ui || options.hash || options.action) {
				var after = [];
				var sep = "?";
				if (options.ui) {
					after.push( sep+'ui=' + options.ui); 
					sep = "&";
					html.push('<a href="' + ($.helpers.hashFirst()?('#' +$.smobile.linkDetailPage(controller)):""));
					if (options.ui && !options.entity && options.useDefault)   {
						if (controller.data && controller.data.$url) {
							var url = $.sdata.parseSdataUri($.sdata.$url({$baseURI: $.sdata.config.baseURL}, controller.data.$url))
							options.entity = url.entity;
							options.entityId = url.id;
						}
					}
				...
				if (options.entity) {
					after.push(sep + 'entity='+ (options.entity || controller.options.entity));
					sep = "&";
					var entityAction = options.entityAction || $.sdata.sdataActions.view.id;
					if (options.entityId) after.push('&entityId='+ options.entityId);
					if (options.orderBy) after.push('&orderBy='+  encodeURIComponent(options.orderBy));
					if (options.selected) after.push('&selected='+  encodeURIComponent(options.selected));
					if (options.groupBy) after.push('&_groupBy='+  encodeURIComponent(options.groupBy));
					if (options.filter) after.push('&filter='+  encodeURIComponent(options.filter));
					after.push('&entityAction='+ entityAction);
					after.push('&type=mobile.' + entityAction);
					if (options.init) {
						after.push('&init=' + encodeURIComponent(JSON.stringify(options.init)));
					}
				}
				if (after.length) html.push($.helpers.encodeJsmQuery(after.join('')));
				if (options.ui) html.push(($.helpers.hashFirst()?(""):'#' +$.smobile.linkDetailPage(controller)));
				html.push('"');
				if (options.navBar) {
					if (options.hash) {
			...
			var _addA = function() {
				if (options.ui || options.hash || options.action) html.push('</a>');
				if (afterCreatelink) afterCreatelink();
			};
			if (layout && layout.type) {
				render = render || that.render[layout.type];
				if (render) {
					return render.render(options.data, controller, html, options, _addA);
				}
			} else {
				if (layout && options.template && layout.bind ) {
					var value = layout.value || "";
					return that._template(value, options.template, options.data, controller, function(tv) {
						html.push(tv);
						_addA();
					});
				} else 	{
					if ((options.text || '').indexOf("$") >= 0) {
						ct = that.parseTemplate(options.text);
						return that._template("", ct, null, controller, function(tv) {
							html.push(tv);
							_addA();
						});
					} else
						html.push(options.text || '');
				}
			}
			_addA();
		};
		var _prefetchTemplate = function() {
			if (options.prefetch && options.prefetchTemplate) {
				that.execTemplate(options.prefetchTemplate, options.data, controller, function(prefetch) {
					options.prefetch = prefetch;
					_continue();
				});
			} else _continue();
		};
		var _filterTemplate = function() {
			if (options.filter && options.filterTemplate) {
				that.execTemplate(options.filterTemplate, options.data, controller, function(filter) {
					options.filter = filter;
					_prefetchTemplate();
				});
			} else _prefetchTemplate();
		};
		if (options.ui && options.uiTemplate) {
			that.execTemplate(options.uiTemplate, options.data, controller, function(ui) {
				options.ui = ui;
				_filterTemplate();
			});
		} else _filterTemplate();
	...
	that.controls.ListView={
		handler: function(c, after) {
			...
			return $.helpers.each(items, function(ii, index, next) {
				var cd = ii.data;
				html.push('<li');
				...
				if (cd.divider) {
					...
					cd.layout = ii.layout || layout;
					cd.addend = true;
					return that._createLink(html, cd, controller, function() {
						html.push('</li>');
						next();
					});
				}
