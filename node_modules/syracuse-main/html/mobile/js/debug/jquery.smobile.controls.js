...
(function($) {
	...
	that.ui.panelItem = function(title, icon) {
		icon  =  icon || consts.default_panel_name;
		var css = icon.split(".");
		if (css.length == 1) css.splice(0, 0, consts.default_panel_class);
		return '<div class="'+consts.panel_icon_size+ ' ' + css.join(" ") + '"></div>'+$helpers.htmlEncode(title) ;
	...
	that.ui.addLinks=function(id, controller, html, cp, bind) {
		...
		if (links &&  links.length)  {
			...
			links.forEach(function(link){
				...
				controller.alinks[cdid] = {bind: bind, $title:  link.$title, url:link.$url, $method: link.$method, $parameters: link.$parameters};
		...
		if (options.url || options.hash || options.action) {
			...
			if (options.action){
				...
				if ((controller.applicationId) &&  (options.hash.indexOf(that.hash_sep) < 0)) {
					...
					if (pc && (pc.useAppId == 2)) 
						options.hash = options.hash + that.hash_sep + $helpers.encodeURIComponent(controller.applicationId);
				}
