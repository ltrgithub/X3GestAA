			...
			Object.keys(cd.$properties).forEach(function(property) {
				...
				if (pv.$type === "application/x-array") {
					...
					if (pv.$item) {
						...
						// never: for old compatibility (no $item in prototype)
						pv.$rtype="application/x-reference"; 
						list.push(pv); 
					}
				} else if (pv.$type === "application/x-object") {
					if (pv.$item) {
						list.push(pv.$item); 
						...
						// never: for old compatibility (no $item in prototype)
						list.push(pv); 
						pv.$rtype = "application/x-object";
					}
				} else if (pv.$type === "application/json") {
					// for old compatibility (no x-object)
					// remove this code
					if (pv.$item) {
						list.push(pv.$item); 
						pv.$item.$rtype = "application/x-object";
			...
			if (cp.$type === "application/x-array") {
				if (data.$bind=="$resources") {
					...
					if (cp.$item.$type === "application/x-reference") {
						...
						|| (cp.$item.$type === "application/json") //TODO remove this line (for old compatibility
						) {
						var spmap = {}, cf = [];
						prototypes.push(cp.$item);
