(function($) {
	...
	that.extractLinks = function(cp, proto) {
		...
		Object.keys(cp.$links).forEach(function(ln){
			...
			var cl = {name: ln};
			cl.$url= that.parseExpression(cp.$url, {data: null, $prototype: proto},"$url");
			cl.$title= that.parseExpression(cp.$url, {data: null, $prototype: proto},"$title");
			cl.$type = ul.$type;
			cl.$method = ul.$method;
			links.push(cl);
		});
