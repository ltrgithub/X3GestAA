	...
	that.execExpression = function(expression, proto, data, level, options) {
		...
		var res = expression.replace(/\{(.*?)\}/g, function(match, prop) {
			...
			if (!level && proto && !proto.noRef && proto.$protoparent && (proto.$protoparent.$type === "application/x-reference")) {
				...
				if (!pvalue.found) {
					dvalue = data ? that._dataProp(data, prop, proto) : {
						found: false
					};
					if (!dvalue.found) {
				...
				pvalue = (proto ? that._protoProp(proto, prop) : {
					found: false
				});
				if (!pvalue.found) {
					if ($helpers.trace.expressions) {
...
(function($) {
	...
	var _errKeys = [_errors.success, _errors.info, _errors.warning, _errors.error, _errors.fatal];
	...
	var _extractErrors = function(data, defaultMsg, root, path, res, type) {
		...
		if (data.$actions) {
			var o = {};
			Object.keys(data.$actions).forEach(function(pn) {
	...
	that.applySDataSort = function(sort, res) {
		var sh = null;
		if (sort.length === 1) sh = function(a, b) {
			var so = sort[0],
				v1 = a[so.name],
				v2 = b[so.name];
			if (v1 === v2) return 0;
...
(function($) {
	...
	that.Time.prototype = {
		...
		format: function() {
			...
			} else return format.replace(/ii/, mms).replace(/i/, ms).replace(/ss/, sss).replace(/s/, ss).replace(/HH/, HHs).replace(/H/, Hs);
		}
	};
