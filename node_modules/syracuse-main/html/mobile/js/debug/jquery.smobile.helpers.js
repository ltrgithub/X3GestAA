(function($) {
				...
				if (value) {
					if (value.$url) {
						...
						if (that.isExpression(value.$url)) {
							curl = that.parseExpression(value.$url, {data: exprData, $prototype: proto},"$url"); 
						}
	...
	that.layoutParser = function(map, data, propname, protos, fields) {
		...
		if (data.$category && data.$title) {
			...
			obj.title = that.parseExpression(data.$title, {data:null, $prototype: cp},"$title"); 
		...
		if (data.$bind) {
			...
			if (cp.$type === "application/x-array") {
				if (data.$bind=="$resources") {
					...
					} else if (cp.$item.$type === "application/x-object") {
						var spmap = {}, cf = [];
						prototypes.push(cp.$item);
	...
	that.getPropertyValue = function (prop, data) {
		// base url
		var value,cd;
		if (prop === "$baseUrl") return $.sdata.config.baseUrl;
		if (data) {
			if (prop.charAt(0) == "@") {
				value =  prop;
	...
	that.setArrayRef=function(cv, proto, data, bind) {
		return cv;
	};
	that.setRef=function(cv, proto, data, bind) {
		var protoRef = proto.$protoparent;
		var dprops = proto.$properties;
		Object.keys(proto.$properties).forEach(function(propName) {
			var pi = proto.$properties[propName];
			var cp = protoRef[propName];
			if (cp) {
				//cp = {prop_name}
				var len = cp.length;
				// faster than regexp
				if ((cp.charAt(0) === "{") && (cp.charAt(len-1) === "{")) {	
					var v = cv[propName];
					delete cv[propName];
					cp = cp.substring(0, len-2);
					
					if (!pi.isReadOnly) {
						var pp = cp.$rprotoparent;
						var pd = data.data();
						while (pp && pd) {
							if (pp.$properties[cp]) {
								pd[cp] = v;
	...
	that.parseExpression = function(expression, data, propName, level) {
		level = level || 0;
		...
		var result = expression && expression.replace(/\{(.*?)\}/g, function(match, prop) {
			var value, pvalue;
			if (data.$prototype ) pvalue = data.$prototype[prop];
		    if (pvalue && (typeof pvalue  === "string") && (pvalue.indexOf('{') >= 0))
				value = that.parseExpression(pvalue, data, level+1);
			 else {
				value=that.getPropertyValue(prop, data);
			}
