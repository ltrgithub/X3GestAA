(function($) {
	...
	that.parseExpression = function(expression, data, propName) {
		return that.execExpression(expression, data.$prototype, data.data, 0, {url: propName==="$url"});
	};
...
(function($) {
	...
	that._extractDiag = function(data, prop, res, warns) {
		...
		var s = (warns?"warning":"error");
		$d.forEach(function(d){	
			if (d.severity === s)   {
				res.errors.push({propName:prop, message: d.message}); 
				if (res.defMsg) {
					res.defMsg = false;
	...
	that.pushWarning = function(ex, solution) {
		that.pushError({
			...
			title: that.messages.app_warning, data: {message:ex.message, solution: solution}
		});
