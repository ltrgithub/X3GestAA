...
(function($) {
	...
	that.htmlValue = function(data,proto,cp,field, value, useValue) {	
		if (!useValue) 
			value = data[field.$bind];
		var exprData = that.exprData(data);
		if (cp.$isHidden) {
			return "";
		}
		if (value === null) value ="";
		switch (field.$type) {
			...
			case "application/x-string": 	
				if (cp.$format && value) {
					if (cp.$format ==="$email") {
						return '<a class="s-m-email"  href="mailto:'+value+'">' + value + '</a>';
					} else if (cp.$format==="$phone") {
						return '<a class="s-m-phone" href="tel:'+value+'">' + value + '</a>';
					}
				}
	...
	that.layoutParser = function(map, data, propname, protos, fields) {
		...
		if (data.$bind) {
			var $bind = (typeof data.$bind === "object")?data.$bind.$bind:data.$bind;
			cp = (cp && cp.$properties)?cp.$properties[$bind]:null;
			...
			if (cp.$type === "application/x-array") {
				if ($bind=="$resources") {
					...
					if (!cp.$item) return;
					cmap.fields =  cmap.fields || [];
					var proto = {$type: cp.$type, $bind: $bind, $title: cp.$title, $proto: cp.$item};
				...
				cp = cp.$item;
				cmap.fields =  cmap.fields || [];
				var proto = {$type: "application/x-reference", $bind: $bind, $title: cp.$title, $proto: cp };
				cmap.fields.push(proto);
				return;
				...
				cmap.fields =  cmap.fields || [];
				var proto = {$type: cp.$type, $bind: $bind};
				cmap.fields.push(proto);
				return;
			}
		}
	...
	function _formatOutValue(value, options) {
		return value;
	};
	/*
	* options
		...
		if (!level) {
			if (expression.indexOf("{") < 0) {
				if (data) {
					...
					if (value.found) {
						var vf =  value.value || "";
						return _formatOutValue(vf, options);
					}
				}
			}
		}
		var res =  expression.replace(/\{(.*?)\}/g, function(match, prop) {
			...
			if (!level  && proto && !proto.noRef && proto.$protoparent && (proto.$protoparent.$type === "application/x-reference")) {
				...
				if (!pvalue.found) {
					...
					cv =  dvalue.value || "";
					return _formatOutValue(cv, options);
				} else {
					if (pvalue.stop) {
						cv = pvalue.value || "";; 
						return _formatOutValue(cv, options);
...
(function($) {
	...
	that.online = function() {
		if (navigator.connection && (navigator.connection.type != null)) {
	...
	that._extractDiag = function(data, prop, res, warns) {
		var $d = data.$diagnoses;
		var s = (warns?["warning", "success"]:["error"]);
		$d.forEach(function(d){	
			if (s.indexOf(d.severity) >= 0)   {
				res.errors.push({propName:prop, message: d.message, severity: d.severity}); 
				if (res.defMsg) {
					res.defMsg = false;
		...
		Object.keys(data).forEach(function(pn){	
			...
			if (Array.isArray(o)){
				for (var i = 0, len=o.length; i<len; i++) {
					var co = o[i];
					if (typeof co === "object") {
						_extractErrors(co, "", false, path?(path +'.'+pn + '['+ i+']'):pn, res, warns);
					}
				}
			} else if (typeof o === "object") {
				_extractErrors(o, "", false, path?(path +'.'+pn):pn, res, warns);
				//
			}
		});
		return res;
	...
	that.extractErrors = function(data, defaultMsg) {
		return _extractErrors(data, "", true, "", null);
	};
	that.extractWarns = function(data, defaultMsg) {
		return _extractErrors(data, "", true, "", null, true);
	};
	that.getErrorMessage = function(data, propname) {
		if (data.$properties && data.$properties[propname] && data.$properties[propname].$diagnoses) {
			var o = data.$properties[propname].$diagnoses;
			if(o.length) {
				for (var i=0, len=o.length; i<len; i++) 
					if (o[i].severity === "error") return o[i].message;
			}
		}
		return "";
	};
	...
	that.pushdbError = function(ex) {
		if (ex) that.pushError({type: _errors.db, title: that.locale().ui.db_exception, data: {message:ex.message}});
	},
	that.pushException = function(ex, solution) {
		that.pushError({
			type: _errors.exception, 
			title: that.locale().ui.app_exception, data: {message:ex.message, solution: solution}
		});
		
	};
	that.pushWarning = function(ex, solution) {
		var success =  (ex && ex.severity && (ex.severity==="success"));
		that.pushError({
			type: _errors.warning, 
			title: success?that.locale().ui.app_success:that.locale().ui.app_warning, data: {message:ex.message, solution: solution}
		});
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method) {
		...
		if (!serror) serror = that.locale().ui.unknownError;
		if ((jqXHR.status == 200) || (jqXHR.status == 201)) {
			that.pushError({type: _errors.exception, title: that.locale().ui.app_exception, data: {message:serror}});
		} else 
...
(function($) {
	...
	that.lang2DateSettings = function(langSettings) {	
		var date = new Date();
		var langDate = {
			"amDesignator": "AM",
			"pmDesignator": "PM",
		...
		var res = {
			...
			yearText: that.locale().datetime.year,
			monthText: that.locale().datetime.month,
			dayText: that.locale().datetime.day,
			minuteText: that.locale().datetime.minutes,
			hourText: that.locale().datetime.hour,
			secText: that.locale().datetime.seconds
	...
	that.addLanguageHeader = function(header) {
