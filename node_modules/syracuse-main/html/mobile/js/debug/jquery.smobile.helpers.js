...
(function($) {
	...
	that.pushdbError = function(ex) {
		if (ex) that.pushError({type: _errors.db, title: $.smobile.locale().ui.db_exception, data: {message:ex.message}});
	},
	that.pushException = function(ex, solution) {
		that.pushError({
			type: _errors.exception, 
			title: $.smobile.locale().ui.app_exception, data: {message:ex.message, solution: solution}
		});
	...
	that.pushWarning = function(ex, solution) {
		...
		that.pushError({
			...
			title: success?$.smobile.locale().ui.app_success:$.smobile.locale().ui.app_warning, data: {message:ex.message, solution: solution}
		});
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method) {
		...
		if ((jqXHR.status == 200) || (jqXHR.status == 201)) {
			that.pushError({type: _errors.exception, title: $.smobile.locale().ui.app_exception, data: {message:serror}});
		} else 
