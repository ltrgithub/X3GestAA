...
(function($) {
	...
	var _msgType= ["info","warning","error"];	
	var consts= {
			...
			if (res && res.errors.length>0) {
				res.errors.forEach(function(d){
					if (d.severity=="error") 
						...
						that.pushWarning({
							title: d.message, 
							data: {message: d.stackTrace}
						});
					else that.pushInfo(d.message);
				});	
				return true;
			}
		}
	...
	that._errkey=function(type) {		
		return "mobile_" + (_errors.info == type ? "infos" : _errors.warning == type ? "warnings" : "errors");
	};
	...
	that._clearAllMsgItems=function(type) {
		[_errors.info, _errors.warning, _errors.error].forEach(function(typ){
	...
	that.pushError = function(error) { error.type = _errors.error ; that._pushMsgItem(error); };
	that.hasErrors = function() { return that._hasMsgItem(_errors.error); };
	that.clearErrors=function() { that._clearAllMsgItems();};
	that.lastErrors=function() { return that._lastMsgItem(_errors.error);};
	...
	that.pushInfo = function(msg) { 
		...
		msg.type = _errors.info;
		that._pushMsgItem(msg);
	};	
	that.hasInfos = function() 	{ return that._hasMsgItem (_errors.info);};
	that.lastInfos = function() { return that._lastMsgItem(_errors.info);};
	that.pushWarning = function(warn) { 
		if (typeof warn === "string") warn = {title: warn};
		warn.type = _errors.warning;
		that._pushMsgItem(warn);
	};	
	that.hasWarnings = function() 	{ return that._hasMsgItem (_errors.warning);};
	that.lastWarnings = function()  { return that._lastMsgItem(_errors.warning);};
	that.allMessages = function() {
		var res=[], msgs = that.lastErrors()
		if (msgs) res.push.apply(res, msgs);
		msgs = that.lastWarnings()
		if (msgs) res.push.apply(res, msgs);
		msgs = that.lastInfos()
		if (msgs) res.push.apply(res, msgs);
		return  res;
