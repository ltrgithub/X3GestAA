(function($) {
	...
	that.execExpression = function(expression, proto, data, level) {
		...
		var res =  expression.replace(/\{(.*?)\}/g, function(match, prop) {
			var dvalue, pvalue;
			if (!level  && proto && (proto.$rtype === "application/x-reference")) {
				...
				if (!pvalue.found) return ""; //return "{"+ prop+ "}";
				if (pvalue.stop) return pvalue.value;
				if (pvalue.value.indexOf("{") >= 0) {
					return that.execExpression(pvalue.value, proto, data, level+1);
				} else 
		...
		if (map.fields) {
			map.fields.forEach(function(field) {
				if (field.$isSection) {
					...
					var o= null;
					if (field.title) 
						o = {layout:"CollapsibleLayout", data:{facet: facet, jmdata: {"data-inset": root?"false":"true"}, text: field.title, collapsed: false, size: 6}};
					if (o) {
						if (cp) o.parent = cp;
						var np = "c"+cid;
						cid++;
