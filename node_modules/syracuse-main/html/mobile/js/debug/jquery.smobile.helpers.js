...
(function($) {
	...
	that._extractDiag = function(data, prop, res, warns) {
		...
		$d.forEach(function(d){	
			if (s.indexOf(d.severity) >= 0)   {
				res.errors.push({propName:prop, message: (d.$message || d.message), severity: d.severity}); 
				if (res.defMsg) {
					res.defMsg = false;
					res.message = (prop?(prop + ": "):"") + (d.$message || d.message);
				}
			}
		});
	};
	...
	that.getErrorMessage = function(data, propname) {
		if (data.$properties && data.$properties[propname] && data.$properties[propname].$diagnoses) {
			...
			if(o.length) {
				for (var i=0, len=o.length; i<len; i++) 
					if (o[i].severity === "error") return (o[i].$message || o[i].message);
			}
		}
		return "";
	};
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method) {
		...
		if (jqXHR.status === 0) {
			// offline
			return that.pushError({type: _errors.exception, title: that.locale().ui.exception_offline, data: {message:that.locale().ui.check_net}});
		} else if (textStatus === 'timeout') {
			return that.pushError({type: _errors.exception, title: that.locale().ui.exception_timeout, data: {message:that.locale().ui.check_net}});
		}
		...
		if (diag && diag.$diagnoses && diag.$diagnoses.length) {
			serror = (diag.$diagnoses[0].$message || diag.$diagnoses[0].message);
