(function($) {
	var that = $.proto || {};
	$.proto = that;
	var _basicTypes = ["application/x-string", "application/x-integer", "application/x-real", 
	...
	that.htmlValue = function(data,proto,cp,field) {
		...
		var meta = (data.$properties?data.$properties[data.$properties]:null);
		...
		switch (field.$type) {
			...
			case "application/x-boolean": 
				return '<span class="ui-icon ui-icon-shadow ui-icon-checkbox-' + (value?"on":"off") + '"></span>';
	...
	that.generateForm = function(map, cid, content, parentId, root) {
		...
		if (map.fields) {
			map.fields.forEach(function(field) {
				if (field.$isSection) {
					...
					var o= null;
					if (field.title) 
						o = {layout:"CollapsibleLayout", data:{jmdata: {"data-inset": "false"}, text: field.title, collapsed: false, size: 6}};
					if (o) {
						if (cp) o.parent = cp;
						var np = "c"+cid;
						cid++;
						...
						if (field.fields && field.fields.length) 
							cid = that.generateForm(field, cid, content, np, false);
						sections.push(o);
					}
				} else {
					...
					if (canGroup) {
						if (!groupControl) {
							e = {uiClass:"MultiField", data:{fields:[]}};
							if (cp) e.parent = cp;
							var eid = "e" + cid;
							cid++;
							content[eid] = e;
						...
						groupControl = null;
						e = {uiClass:"ArrayField", data:{jmdata: {"data-inset": "true", "data-mini": "true", "data-theme":"d", "data-divider-theme":"e"}, field: field}};
...
(function($) {
	...
	that._diag = function(data, path , res) {
		...
		Object.keys(data).forEach(function(value) {
			...
			if ((typeof o == "object") && (o.$)) {
				that._diag(o.$, (path?(path+"."+value):value), res);
			}
		});
	};
	that.http = {
	...
	that.FUNCS = {
		...
		BIG_ICON: function(title, description, iconName, iconIndex) {
			return '<div class="ui-m-home ui-m-icon-45 ui-m-li-left ui-m-' + iconName + '-45 u-m-h-'+iconIndex+'"></div><h3>'+title +'</h3><p><strong>'+description+'</strong></p>';
		}
	};
	//remove
	...
	that.pushAjaxError = function(jqXHR, textStatus, errorThrown, url, method) {
		...
		if (!serror) serror = $.helpers.messages.unknownError;
	...
	*/	 
	...
	that.date = {
		...
		checkString: function(val, orig) {
			...
			try {
				var cv = val+suffix;
				date = Date.parseISO8601(val+suffix);
				if (cv != date.toISOString()) {
					if (orig) return orig;
					date = new Date();
				}
			} catch(ex) {
...
(function($) {
	...
	that.messages = {
		...
		field_requires: "\"%0\" is required.",
		sdata_notfound: "\"%0\" - Resource not found.",
		sdata_apost_notimplemented: "Asynchronous POST - not implemented.",
		sdata_m_before: "Modify the request before resubmitting",
		sdata_412: "Update canceled",
		sdata_d_412: "Delete canceled",
		sdata_410: "Record deleted",
		sdata_u_500: "Update canceled",
		sdata_503: "Retry after ...",
		sdata_c_canceled: "create canceled",
		sdata_d_canceled: "delete canceled",
		action_syncronize: "Synchronize your application data.",
		cancel_delete: "Undo Delete",
		cancel_update: "Undo Changes",
		cancel_create: "Undo Create",
		edit_save: "Save",
		edit: "Edit",
		edit_cancel: "Cancel",
		edit_rmv: "Remove",
		sync_data: "Resynchronize local database",
		resync_data: "Download",
		cancel_all: "Undo All changes",
		send_all: "Send Changes",
	...
	if ($.scroller) {
		that.msFormat = function() {
			return {
				dateFormat: $.helpers.messages.dateFormat,
				dateOrder: $.helpers.messages.dateOrder
			};
		};
		that.msSettings = function() {
			return {
				preset: 'date',
				dateFormat: $.helpers.messages.dateFormat,
				dateOrder: $.helpers.messages.dateOrder
