(function($) {
	...
	that.controls.Picture = {
		_setPhoto: function(controller, $c, c, value) {
			...
			var value = value || (controller?controller.getValue(c.options.bind, null, null):null);
			if (value == null) {
				_doNodata();
			} else {
				if (value.$url) {
					var curl = value.$url;
					if (curl && $.proto.isExpression(curl) && controller) {
						var page = controller.getPageData();
						var entityProto = $.proto.getProto(page);
						if (!page)  	
							curl =""
						else {	
							curl = $.proto.parseExpression(value.$url, {data: controller.exprData(),  $prototype: entityProto},"$url"); 
						}
					}
					$image[0].src = curl;
					_showImg();
		...
		createPicture: function(controller, c) {
			...
			html.push('<input id="'+c.id+'_input" class="s-hidden-file" accept="image/*" type="file"  data-ajax="false" name="'+c.id+'_input"/>');
			html.push('<center><img class="s-m-image s-m-hidden" id="'+c.id+'_image" src=""/></center>');
			html.push('</div>'); 
			...
			$photo.bind( "click", function(event, ui) {
				if (navigator.camera && navigator.camera.getPicture) {
					navigator.camera.getPicture(function(data) {
						...
						that.controls.Photo._setPhoto(controller, $c, c, res);
					}, function() {}, {
						quality : 50
					});				
				} else {
					alert(that.locale().ui.picture_nocamera);
				}
			});
