...
(function($) {
	...
	that.controls.Picture = {
		...
		createPicture: function(controller, c) {
			...
			html.push('"/></center>');
			html.push('</div>'); 
			html.push('<a id="'+c.id+'_file" data-theme="f" data-role="button" data-mini="true" data-icon="file" data-inline="true">'+$helpers.locale().ui.picture_file+'</a>');
			html.push('<a id="'+c.id+'_photo" data-theme="f" data-role="button" data-mini="true" data-icon="photo" data-inline="true">'+$helpers.locale().ui.picture_photo+'</a>');
			html.push('</div>'); 
			return { 
				html: html.join(''),
				bind: options.bind,
		...
		checkValue:  function($c, c) {
			...
			if (c.options.$isMandatory) {
				...
				if (!value) {
					var error = {field: $c, message: $helpers.format($helpers.locale().ui.mandatory_field,[c.options.label])};
		...
		events: function($c, c) {
			...
			$photo.bind( "click", function(event, ui) {
				if (navigator.camera && navigator.camera.getPicture) {
					...
					$helpers.alert($helpers.locale().ui.picture_nocamera, function(){}, true);
				}
			});
