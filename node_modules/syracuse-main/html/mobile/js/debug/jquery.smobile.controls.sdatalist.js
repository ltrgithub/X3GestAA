	...
	that.controls.SDataList={
		handler: function(c, after) {
			...
			resources.forEach(function(value, index) {
				...
				if (options.$lookup) {
					var o = {$uuid: value.$uuid, $key: value.$key};
					o.$value = $.proto.parseExpression(cp.$value, {data: value, $prototype: cp});
					cd.action = "validate"
					cd.actionParams = JSON.stringify(o);
				}
		...
		events: function($c, c) {
			...
			$c.click(function(e) {
				...
				if (e && e.originalEvent && e.originalEvent.target) {
					...
					var p = $(e.originalEvent.target).attr("data-action");
					if (p) {
						p = JSON.parse(p);
						if (controller.parent) {
							if (controller.config && controller.config.url && controller.config.url.query && controller.config.url.query.control) {
								var c = controller.parent.controls[controller.config.url.query.control];
								if (c) {
									var cc = that.controls[c.uiClass];
									if (cc.setValue) return cc.setValue(controller.parent, c, p, that.doBack);
								}
							}
						}						
					}
