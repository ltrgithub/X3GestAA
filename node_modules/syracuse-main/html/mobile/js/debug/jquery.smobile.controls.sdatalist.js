(function($) {
	...
	that.controls.SDataList={
		...
		createItems: function(controller, options, html) {
			...
			var coptions = {};
			$article = $auth.checkQueryArticle(cp, $article);
			var detail = null;
			...
			resources.forEach(function(value, index) {
				var cd = {};
				var chtml = [];
				$proto.auth.sdi($article, chtml, coptions, controller, cp, value);
	...
	that.controls.SdataSort = {
		...
		handler: function(c, after) {
			...
			if (options.items) {
				html.push('<a href="#'+c.id+'_sortpopup" id="'+c.id +'" class="ui-btn-right" xdata-iconpos="notext" data-inline="true" data-icon="arrow-d" data-rel="popup">'+$helpers.locale().ui.sort+ '</a>');
