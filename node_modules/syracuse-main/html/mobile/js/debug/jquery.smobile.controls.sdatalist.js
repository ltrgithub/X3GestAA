...
(function($) {
	...
	that.controls.SDataList={
		...
		createItems: function(controller, options, html) {
			...
			var coptions = {}, info={list:true, noCache: $proto.auth.designMode, addFav: true};
			...
			resources.forEach(function(value, index) {
				var cd = {jmdata:{"data-iconshadow":false, "data-corners":false, "data-shadow": false, "data-icon": "s-noicon"}};
				var chtml = [], licss = ["s-m-slist-item"];
				...
				if (info.addFav) {
					var favi = 'off';
					if (favs) {
						var key = $proto.parseExpression(cp.$key, {data: controller.exprData(value), $prototype: cp},"$key");
						if (favs.indexOf(key) >= 0)  favi = 'on';
					}
					
					html.push('<div class="s-m-fav-item" data-action="setfavorite" data-action-param="'+index+'"><div data-action-param="'+index+'" data-action="setfavorite" class="ui-icon ui-icon-s-fav-'+favi+'"></div></div>');
			...
			if (e && e.target) {
				...
				if (a === "setfavorite") {
					...
					if (elem) {
						...
						var url = $proto.parseExpression(cp.$url, {data: controller.exprData(elem), $prototype: cp},"$url");
						...
						$sdata.switchFav(url, elem, function(res){
							...
							var favs = controller.extraData.bookmarks = controller.extraData.bookmarks || [], oldc, newc, i;
							if (res.op === 2) {
