(function($) {
	...
	that.controls.SDataList={
		...
		createItems: function(controller, options, html) {
			...
			var defArticle = {
				icon: {propName: "$icon", protoValue:true}, 
				hint: {propName: "$hint", protoValue:true},
		...
		handler: function(c, after) {
			...
			if (!options.$lookup) {
				var filters = cp.$filters?Object.keys(cp.$filters):null;
				if (filters && filters.length && controller.config && controller.config.url) {
					...
					filters.forEach(function(filter, i) {
						var cf = cp.$filters[filter];
						html.push('<li><a data-sagefilter="'+ filter +'" href="#"');
						if (sf && (filter == af)) {
							sf = false;
							html.push(' class="ui-btn-active"');
						}
						html.push('>');
						html.push($.helpers.htmlEncode($.proto.parseExpression(cf.$title, {data: null, $prototype: cp}, "$title")));	
						html.push('</a></li>');
					});
					html.push('</ul></div>');
				}
		...
		events: function($c, c) {
			...
			if  (c.options.$lookup) {
				...
				$('#' + ceid + "_filter").click(function(e) {
					that.controls.SDataList._execFilter(e, ceid, controller);
				});
			}
			if ($.sdata.capabilities.showSearch) 
				$('#'+ceid + "_sbutton").click(function(e) {that.controls.SDataList._execSearch(e, ceid, controller)});
			else
				$('#' + ceid + "_search_form").submit(function(e) {that.controls.SDataList._execSearch(e, ceid, controller);});
		},
		_execUrl: function(purl, controller) {
			$.sdata.getUri(purl);
		...
		_execSearch: function(e, id, controller) {
			e.preventDefault();
			var purl = controller.config.url;
		...
		_execFilter: function(e, id, controller) {
			e.preventDefault();
			var target = e.target;
			while (target && target.tagName && (target.tagName.toUpperCase() != 'A'))
				target = target.parentNode;
			...
			var purl = controller.config.url;
			var cv = $(target).attr("data-sagefilter") || "";
