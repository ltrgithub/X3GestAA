...
(function($) {
	...
	_controls.types.addArray = function(bind, parentbind, index) {
		return  {value:$helpers.locale().ui.array_detail, action:"show_detail", actionParams:JSON.stringify({"action": "show_detail", "$bind":bind, "$parentbind": parentbind,"$index":index})};
	...
	_controls.types.addReference = function(proto, data, controller, field, addtitle) {
		...
		if (pp && (pp.$title || pp.$description)) res.title = (pp.$title || pp.$description);
		...
		if (proto.$links && proto.$links.$details) {
			...
			if ($sdata.url2proto(purl))
				res.url = url;
		}
		return res;
	}
	_controls.types._table_header = function(html, fields, $prototype, edit) {
		html.push('<table data-role="table" data-mode="reflow" class="s-m-table ui-responsive table-stroke"');
		html.push('>');
		html.push('<thead><tr>');
		...
		fields.forEach(function(field, index){
			...
			html.push($helpers.htmlEncode($proto.parseExpression(prop[field].$shortTitle ||  prop[field].$title, {data: null, $prototype: $prototype},"$title")));
			html.push('</th>');
		});
		if (edit) {
			html.push('<th data-priority="' + fields.length +'">'+ $helpers.htmlEncode($helpers.locale().ui.array_detail) +'</th>');
	...
	_controls.types._table_row = function(html, fields, $prototype, value, controller, edit, index) {
		...
		fields.forEach(function(field){
			...
			if (proto.$type === "application/x-reference") {
				//ok
				var options = _controls.types.addReference(proto, [value[field], value], controller, field, true);
				that._addLink(html, options, controller);
			}  else {
				html.push($proto.htmlValue(value,$prototype, proto,{$type:proto.$type, $bind:field}));
			}
			if (first) {
				html.push('</th>');
				first = false;
		...
		if (edit) {
			html.push('<td>');
			html.push('<a href="#" data-role="button"  data-index="'+index+'" data-action="edit" data-iconpos="left" data-theme="f" data-inline="true"  data-mini="true" data-icon="bars">');
	...
	_controls.types._cards = function(html, fields, $prototype, value, controller, edit, index, parentBind) {
		var id, options;
		html.push('<table data-role="table" data-mode="reflow" class="s-m-table tv ui-responsive table-stroke"');
		html.push('>');
		html.push('<thead><tr>');
		...
		fields.forEach(function(field){
			...
			html.push('<tr>');
			html.push('<th>');
			html.push($helpers.htmlEncode($proto.parseExpression(proto.$shortTitle ||  proto.$title, {data: null, $prototype: $prototype},"$title")));
			html.push('</th><td>');
			if (proto.$type === "application/x-reference") {
				//ok
				options = _controls.types.addReference(proto, [value[field.$bind], value], controller,field.$bind, true);
				that._addLink(html, options, controller);
			}  else {
				//
				field.$type = proto.$type;	
				if (!edit && (proto.$type === "application/x-array")) {
					if (value && value[field.$bind] && value[field.$bind].length) {
						options = _controls.types.addArray(field.$bind, parentBind, index);
						that._addLink(html, options, controller);
					} else 
						html.push(' ');
				} else {
		...
		if (edit) {
			html.push('<tr>');
			html.push('<td colspan=2 style="text-align: right;">');
			//html.push('<a class="ui-link s-m-rlink" href="#" data-index="'+index+'" data-action="edit" data-theme="c"');
			//html.push('>'+$helpers.locale().ui.edit_item+'<span class="ui-icon ui-icon-s-bars-dark">&nbsp;</span></a>');
			
			html.push('<a href="#" class="" data-role="button"  data-index="'+index+'" data-action="edit" data-iconpos="left" data-theme="f" data-inline="true"  data-mini="true" data-icon="bars">');
			html.push($helpers.locale().ui.edit_item);
			html.push('</a>');
			
			
			//html.push('<a href="#" class="ui-link">Retourner en haut<span class="ui-icon ui-icon-edit">&nbsp;</span></a>');
			//html.push('</td>');
			//html.push('<td>');
			//html.push('<a href="#" data-role="button"  xdata-iconpos="notext"  data-index="'+index+'" data-action="delete"  data-theme="f" data-mini="true" data-icon="delete"');
			//html.push('>'+$helpers.locale().ui.remove_item+'</a>');
			html.push('</td>');
			html.push('</tr>');
	...
	_controls.ArrayField={
		...
		handler: function(c, after) {
			...
			var $article = options.$article, isTitleHidden = false;
			...
			if (edit) {
				...
				switch (cpt) {
					case "application/x-reference":
						opts.type = "text";
						opts.label = isTitleHidden?"":$proto.parseExpression(protoProp.$shortTitle ||  protoProp.$title, {data: null, $prototype: cp},"$title");
					...
					case "application/x-array": 
						switch (field.$proto.$type) {
							case "application/x-reference":
								opts.title =isTitleHidden?"":$proto.parseExpression(protoProp.$shortTitle ||  protoProp.$title, {data: null, $prototype: cp},"$title");
							...
							case "application/json": // for old compatibility
								...
								opts.jmdata = $proto.jqmDataArray();
								var clayout = $proto.auth.compositionLayout($article, (field.$proto.$item || field.$proto), null, true);
								opts.cards = clayout.cards;
								opts.items = clayout.$items;
						...
						if (fieldData) {
							...
							switch (field.$proto.$type) {
								...
								case  "application/json":
									// array composition 
									var layout = $proto.auth.compositionLayout($article, (field.$proto.$item || field.$proto), null, true);
									if (layout.cards) {
										fieldData.forEach(function(value, index){
											html.push('<li>'); 
											if (layout.$items) 
												_controls.types._cards(html, layout.$items, field.$proto, value, controller,false, index, field.$bind);
											html.push('</li>'); 
										});
									} else {
										if (layout.$items) {
											html.push('<li>'); 
											//html.push('<div>'); 
											_controls.types._table_header(html, layout.$items, field.$proto);
											fieldData.forEach(function(value, index){
												_controls.types._table_row(html, layout.$items, field.$proto, value, controller, false, index);
				...
				$c.click(function(e) {
					...
					if (p) {
						try {
							p = JSON.parse(p);
							var page = controller.getPageData();
							var cd = controller.getValue(p.$parentbind, null, []);
							if (cd) {
								var  cp = $proto.getProto(page, p.$parentbind);
								if (cd && Array.isArray(cd) && (p.$index >= 0)) {
									...
									if (cd) {
										...
										if (cd && cd[p.$bind]) {
											...
											var ui = that.detailUiData(controller, pp, nd, $layout, {readOnly: true});
