...
(function($) {
	...
	_controls.types._table_row = function(html, fields, $prototype, value, controller, edit, index) {
		...
		fields.forEach(function(field){
			...
			if (proto.$type === "application/x-reference") {
				//ok
				var options = _controls.types.addReference(proto, [value[field.$bind], value], controller);
	...
	_controls.types._cards = function(html, fields, $prototype, value, controller, edit, index, parentBind) {
		...
		fields.forEach(function(field){
			...
			if (proto.$type === "application/x-reference") {
				//ok
				options = _controls.types.addReference(proto, [value[field.$bind], value], controller);
				that._addLink(html, options, controller);
			}  else {
				//
			...
			if (edit) {
				...
				switch (cpt) {
					case "application/x-reference":
						...
						// lookup 
						toptions = _controls.types.addReference(field.$proto, fieldData, controller);
						html.push('<li>'); 
										...
										if (cd && Array.isArray(cd) && (p.$index >= 0)) {
											...
											if (cd) {
												...
												if (cd) {
													...
													var $layout = $proto.auth.findSlayout(c.options.$article);
													var uidata= $proto.auth.compositionLayout($layout, cp);
													//console.log(uidata);
													/*
													*
													*/
