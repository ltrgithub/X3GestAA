		...
		events: function($c, c) {	
			...
			if (!edit) {
				$c.click(function(e) {
					...
					if (!target) return;
					var p = $(target).attr("data-action"), page, cp,cd, pp, $layout, ui, nd;
					if (p) {
						if (p === "detail") {
							...
							if (cd && Array.isArray(cd) && (p >= 0)) {
								cd = cd[p];
								$layout = $proto.auth.findSlayout(c.options.$article, c.options.$bind, false);
								pp = cp;
								nd = cd;
								if (!$layout) $layout= $proto.auth.compositionLayout(null, pp, null, false);
								ui = that.detailUiData(controller, pp, nd, $layout, {readOnly: true});
