(function($) {
	...
	that.controls.Combo = {
		handler: function(c, after) {
			...
			if (options.$mandatory) html.push(' data-placeholder="true"');
			html.push('>');
			var _afterGetValue = function(cv) {
				var key = cv || options.value;
				if (options.$mandatory) 
					html.push("<option value=\"\""+ (key?"":" selected=\"selected\"") +">"+ options.label + "</option>");
				options.items = options.items || [];
				options.items.forEach(function(item, index) {
					var value = index, title = item;
					...
					if (typeof item == "object") {
						...
						if (!options.useIndex) {
							...
							key = options.items.indexOf(key);
						}
					}
					html.push("<option value=\"" + value +"\""+ ((key == value)?" selected=\"selected\"":"") + ">" + title+ "</option>");
				});
				if (controller.config.inCreate && options.autoInit && (cv == null)) {
					var selvalue = key;
					if (options.useIndex ) {
						if (key >= 0) 
							selvalue = options.items[key];
						else
							selvalue = "";
					} 
					controller.setData(options.bind, selvalue);
				}
				html.push('</select>');
				html.push('</div>');
				after({ 
			...
			if (options.bind) {
				controller.getData(options.bind, null, null, _afterGetValue);
			} else _afterGetValue();
		...
		events: function($c, c) {
			...
			var $select = $c.find( '#'+ c.id + "_i");
			//OK
			controller.getData(c.bind, null, null, function(oldValue) {
				$select.bind( "change", function(event, ui) {
					var value = $select.find('option:selected').val();
					var _aftercchange = controller.afterChangeHnd(c);
					if (c.useIndex) value = c.items[parseInt(value, 10)];
					if (oldValue != value) {
