(function($) {
	...
	that.controls.Lookup={
		createLookup: function(controller, c) {
			...
			if (cv) {
				...
				if (!val && cp.$value) {
					...
					var key = cv.$key;
					if (!key && cp.$key) 
						key = $.proto.parseExpression(cp.$key, {data: controller.exprData(cv), $prototype: cp},"$key");
					val = key
				}
		...
		setValue: function(controller, c, value, next) {
			...
			var ov = controller.getValue(c.options.bind, null, null), oldkey;
			if (ov) {	
				// calculate old key
				oldkey = ov.$key;
				if (!oldkey && cp.$key) {
			...
			if (setvalue) {
				...
				var ivalue = cv.proto.parseExpression(cp.$value, {data: controller.exprData(controller.getValue(c.options.bind, null, null)), $prototype: cp},"$value") || "";
				$input.val(ivalue);
			}
			return next();
		},
