(function($) {
	...
	that.controls.ArrayRef={
		_createItem: function(html, cp, value, index) {
			html.push('<li>'); 
			html.push('<a href="#">'); 
			var ioptions = that.controls.types.addReference(cp, value);
		...
		createArrayRef: function(controller, c, fieldData) {
			...
			fieldData.forEach(function(value, index){
				that.controls.ArrayRef._createItem(html, cp, value, index)
			});
