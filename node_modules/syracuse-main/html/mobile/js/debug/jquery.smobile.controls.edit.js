	...
	that.controls.Edit={
		...
		createEdit: function(controller, c) {
			...
			if (options.type == "date") {
				...
				if (controller.config.inCreate) {}
				cv = controller.getValue(options.bind, null, 0);
			} else {
				cv = controller.getValue(options.bind, null, "");
			}
			html.push($.helpers.htmlEncode(that.controls.Edit._nativeToString(cv, options.type)));
			html.push('"');
			if (options.$pattern) 
		...
		checkValue:  function($c, c) {
			...
			if (opts.$isMandatory && ($input[0].value === "")) {
				error = {field: $c, message: $.helpers.format(that.locale().ui.mandatory_field,[opts.label])};
				controller.pushError(error);
				e.html(error.message);
			}  else if (opts.$minLength && ($input[0].value.length < opts.$minLength)) {
				error = {field: $c, message: $.helpers.format(that.locale().ui.minlength_field,[opts.label])};
				controller.pushError(error);
				e.html(error.message);
			}
