(function($) {
	var that = $.smobile || {};
	$.smobile = that;
	var $helpers = $.helpers;
	var $sdata = $.sdata;
	that.controls.SyncCheck = {
		handler : function(c, after) {
			var self = this;
			c.data = {label: $helpers.locale().ui.sync_sync_data, value: false, action: " "};
			return that.controls.CheckBox.handler.apply(self, [c, after]);
			
		},
		events : function($c, c) {
			var controller = this;
			var $input = $c.find( '#'+ c.id + "_i");
			$input.change(function(target) {	
				var value = $input[0].checked;
				controller.resync = value;
			});
		}
		
	};
	var _status2css = function(value) {
		switch (value) {
			case $sdata.STATUS.CREATED: 
				return "created";
				break;
			case $sdata.STATUS.DELETED:
				return "deleted";
				break;
			case $sdata.STATUS.MODIFIED:
				return "modified";
				break;
		}
		return "";
	};
	var _status2undoHint = function(value) {
		switch (value) {
			case $sdata.STATUS.CREATED: 
				return $helpers.locale().cancel_create;
				break;
			case $sdata.STATUS.DELETED:
				return $helpers.locale().ui.cancel_delete;
				break;
			case $sdata.STATUS.MODIFIED:
				return $helpers.locale().cancel_update;
				break;
		}
		return "";
	};
	
	that.controls.SyncFooter = {
		sync: false,
		handler : function(c, after) {
			var controller = this;
			c.data={ 
				"items": [
					{ title: $helpers.locale().ui.sync_cancel_all, action: "undo" , "jmdata": {"data-icon":"delete", "data-transition": "none"}},
					{ title: $helpers.locale().ui.sync_send_all, action: "sync", "jmdata": {"data-icon":"arrow-u", "data-transition": "none"}},
					{ title:$helpers.locale().ui.sync_resync_data, action: "resync", "jmdata": {"data-icon":"arrow-d", "data-transition": "none"} }
				]
			};
			var ff = function(o) {
				o.uiClass = "SyncFooter";
				return after(o);
			};
			return that.controls.NavBar.handler.apply(controller, [c, ff]);
				
		},
		updSync: function() {
			var controller = this;
			var syncFooter = null;
			Object.keys(controller.controls).forEach(function(cn) {
				if (!syncFooter) {
					var o = controller.controls[cn];
					if (o.uiClass === "SyncFooter") {
						syncFooter = o;
					}
				}
			});		
			if (syncFooter && syncFooter.options && syncFooter.options) {
				var hasdata = controller.data?true:false;
				var $nav = $("#" + syncFooter.id + "_ul");
				$nav.attr("class", hasdata?"ui-grid-a":"ui-grid-solo"); 
				
				$nav = $("#" + syncFooter.id + "_resync");
				if (hasdata) {
					$nav.remove();
				} else {
					$nav.attr("class","ui-block-a"); 
					$("#" + syncFooter.id + "_sync").remove();
					$("#" + syncFooter.id + "_undo").remove();
					
				}
			}			
			
		},
		refresh: function(controller, options, $c, id) {
			var html = [];
		},
		
		events : function($c, c) {
			var controller = this;
			return that.controls.NavBar.events.apply(controller, [$c, c]);
		}
		
	};
	that.controls.SyncView = {
		create_items: function(controller, data, html) {
			Object.keys(data).forEach(function(value) {
				var meta = $sdata.entityMeta(value);
				if (!meta) return ;
				html.push('<li data-role="list-divider">');
				html.push(meta.title|| value);
				html.push('</li>');
				var list  = data[value];
				list.forEach(function(cd) {
					html.push('<li');
					var css = [];
					css.push("ui-li-has-icon");
					css.push($helpers.ui.consts.normal_icon);
					if (css.length) html.push(' class="' + css.join(" ")+'"');
					html.push('>');
					var coptions = [];
					coptions.push("<div class=\"ui-li-icon ui-li-thumb s-m-status "+ _status2css(cd.$info.status) +"\"></div>");
					coptions.push($helpers.htmlEncode(cd.$info.title || "..."));
					if (cd.$info.last_error) {
						coptions.push("<p class=\"s-m-error\"><strong>");
						coptions.push($helpers.htmlEncode(cd.$info.last_error));
						coptions.push("</strong></p>");
					}
					if (cd.$info.status == $sdata.STATUS.DELETED) {
						html.push('<a href="#">');
						html.push(coptions.join(""));
						html.push("</a>");
						html.push("<a href=\"#action-cancel"+ $helpers.encodeJsmQuery("?url=" + $helpers.encodeURIComponent(cd.$info.url)) +"\">"+  $helpers.htmlEncode(_status2undoHint(cd.$info.status)) + "</a>");
					} else {
						var  opts =  {url: $sdata.editUrl(cd.$info.url)};
						that._addLink(html, opts, controller, function() {
							html.push(coptions.join(""));
						});
						html.push("<a href=\"#action-cancel"+ $helpers.encodeJsmQuery("?url=" + $helpers.encodeURIComponent(cd.$info.url)) +"\">"+  $helpers.htmlEncode(_status2undoHint(cd.$info.status)) + "</a>");
					}
					html.push('</li>');
				});
			});
		
		},
		sync: false,
		asyncHandler: function() {
			var obj = this;
			var controller  = obj.controller;
			var id = obj.id;
			$sdata.getSyncData(function(data, error) {
				var html = [];
				if (!error) {
					controller.data = data;
					if (data) that.controls.SyncView.create_items(controller, data, html);
				}
				var $c = $('#' + id);
				$c.html(html.join(''));
				$c.listview("refresh");
				var syncFooter = null;
				Object.keys(controller.controls).forEach(function(cn) {
					if (!syncFooter) {
						var o = controller.controls[cn];
						if (o.uiClass === "SyncFooter") {
							var hnd = that.controls.SyncFooter.updSync.bind(controller);
							hnd();
							
						}
					}
				});
			});
		},
		handler: function(c, after) {
			var controller = this, html = [];
			controller.hasSyncView = true;
			html.push('<ul id="'+c.id + '" data-role="listview" data-split-icon="back" data-split-theme="d"');
			html.push(' data-inset="true"');
			html.push('>');
			html.push('</ul>');
			after({ 
				html: html.join(''),
				bind: " ",
				async: that.controls.SyncView.asyncHandler.bind({controller: controller, id: c.id})
				
			});
			
		},
		refresh: function(controller, options, $c, id) {
			var html = [];
			$sdata.getSyncData(function(data, error) {
				if (!error) {
					controller.data = data;
					if (data) that.controls.SyncView.create_items(controller, data, html);
				}
				$c.html(html.join(''));
				$c.listview("refresh");
			});
		}
		
	};
})(jQuery);    


