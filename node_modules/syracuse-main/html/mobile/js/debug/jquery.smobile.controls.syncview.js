...
(function($) {
	...
	that.controls.SyncView = {
		create_buttons: function(controller, b1html,b2html , buttons, id) {
			...
			if (hasdata) {
				var del_itm = {linkId: id+ "_undo", title:$helpers.locale().ui.sync_cancel_all, action: "undo" , "jmdata": {"data-iconpos":"top", "data-icon":"delete", "data-transition": "none"}};
				that._addLink(b1html, del_itm, controller, function() {	
					b1html.push($helpers.htmlEncode(del_itm.title));
				});	
				buttons.push(del_itm.linkId);
				var send_itm = {linkId: id+ "_sync", title:$helpers.locale().ui.sync_send_all, action: "sync", "jmdata": {"data-iconpos":"top","data-icon":"arrow-u", "data-transition": "none"}};
				...
				var rinstall_itm = {linkId: id+ "_install", title:$helpers.locale().ui.sync_resync_data, action: "resync", "jmdata": {"data-iconpos":"top", "data-icon":"arrow-d", "data-transition": "none"}};
			...
			Object.keys(data).forEach(function(value) {
				...
				list.forEach(function(cd, index) {
					...
					if (cd.$info.status == $sdata.STATUS.DELETED) {
						...
						html.push('>');
						html.push($helpers.htmlEncode($helpers.locale().ui.cancel_item));
						html.push('</button>');
						html.push('</div></div>');
						...
						html.push('>');
						html.push($helpers.htmlEncode($helpers.locale().ui.edit_item));
						html.push('</a>');
						html.push('</div><div class="ui-block-b">');
						bid = id + '_'+ ci + "_cancel";
						buttons.push(bid);
		...
		events : function($c, c) {
			...
			$c.click(function(e) {
				// find parent by tag 
				var target = e.target;
				while (target && target.tagName && ((target.tagName.toUpperCase() != 'BUTTON') && (target.tagName.toUpperCase() != 'A')))
		...
		asyncHandler: function() {
			...
			$sdata.getSyncData(function(data, error) {
				var html = [], buttons= [], b1html= [], b2html= [];
				if (!error) {
		...
		handler: function(c, after) {
			...
			html.push('<ul id="'+c.id + '_ul" data-role="listview"');
			html.push(' data-inset="true"');
			html.push('>');
			html.push('</ul>');
			
			var check= {id : controller.prefix + "reinstall"};
			check.data = {label: $helpers.locale().ui.sync_sync_data, value: false, action: " ", noform: true};
