...
(function($) {
	...
	that.capabilities = {
		...
		authRequired: false,
		showSearch: true,
		installUrl: "/sdata/syracuse/collaboration/syracuse/dashboardDefs/$service/mobilePortlets",
		auth: null
	};
				...
				success: function(data, headers) {
					...
					if (headers.status > 300) {
						if ((headers.status == 404) && sendURL.template) {
							$.helpers.pushException({message: $.helpers.format($.smobile.locale().sdata.S404, [newUrl.uri])}, $.smobile.locale().actions.syncronize);
						}	
	...
	that._execBatch = function(syncData, options, resync) {
		$.mobile.loading('show', {theme: "a", text: $.smobile.locale().ui.sending});
						...
						function(value, index, next) {
							...
							if (send.$httpMethod == "PUT") {
								if (value.$httpStatus == 200) {
									...
									$.helpers.pushException({message: errors.message}, $.smobile.locale().sdata.modify_before);
									...
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message}, $.smobile.locale().sdata.S410);
									that._cleanHeaders(value);
									...
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message},$.smobile.locale().sdata.S412);
									that._cleanHeaders(value);
									return $.sdata.updData(resUrl, {data:value, original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
									...
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message}, $.smobile.locale().sdata.S503);
									return next();
								} else {
									$.helpers.pushException({message: $.smobile.locale().sdata.invalid_response}, $.smobile.locale().sdata.invalid_response);
									return next();
								}
								...
								if (value.$httpStatus == 201) {
									...
									$.helpers.pushException({message: $.smobile.locale().post_ni});
								} else if (value.$httpStatus == 400) {
									...
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message},  $.smobile.locale().sdata.SC_canceled);
									// delete
								...
								if ((value.$httpStatus == 200) || (value.$httpStatus == 410)) {
									...
									//Consumer is not authorized to create the resource.
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message}, $.smobile.locale().sdata.SD_canceled);
									return $.sdata.undoUpdate(resUrl, next);		
								} else if (value.$httpStatus == 412) {
									//The If-Match header was provided but does not match the state of the resource on the provider side.
									errors = $.helpers.extractErrors(value, value.$httpMessage);
									$.helpers.pushException({message: errors.message}, $.smobile.locale().sdata.SD412);
									that._cleanHeaders(value);
									return $.sdata.updData(resUrl, {data:value, original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
								} else if (value.$httpStatus == 500) {
									//Generic status for consumer side error.
