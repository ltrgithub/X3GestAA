			...
			_private._execSDATA(sendURL, {
				success: function(data, headers) {
					...
					if (!data && (headers.status > 400) && sendURL.template && that.config) {
						if (that.config && that.config.pages && sendURL.query  && that.config.pages[sendURL.query.representation]) {
							var eproto = that.config.pages[sendURL.query.representation].$prototype;
					...
					if (sendURL.template) {
						...
						var tproto = null;
						if (that.config && that.config.pages && sendURL.query  && that.config.pages[sendURL.query.representation]) {
							tproto = that.config.pages[sendURL.query.representation].$prototype;
						}
						isTemplate = true;
						if (tproto) {
						...
						function(value, index, next) {
							...
							if (send.$httpMethod == "PUT") {
								if (value.$httpStatus == 200) {
									...
									return $.sdata.updData(resUrl, {data:value, representation: "", original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
									...
									return $.sdata.updData(resUrl, {$load: true, data:value, representation: info.representation, status:$.sdata.STATUS.MODIFIED, last_error:errors.message}, next);
			...
			return that.getModifiedData(url, function(data) {
				if (url.id) {
					...
					if (!url.template && data && Object.keys(data).length)  {
						var isuuid  = true;
						var proto = (url.query && url.query.representation)?$.sdata.config.pages[url.query.representation]:null;
						if (proto) {
							proto = proto.$prototype.$properties.$resources.$item;
