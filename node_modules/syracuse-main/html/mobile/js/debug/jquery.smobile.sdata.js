...
(function($) {
	...
	$.helpers.FUNCS.STATUS=function(value) {	
		var ii = 0;
		switch (value) {
			case _status.CREATED: 
				ii= 0;
				break;
			case _status.DELETED:
				ii= 1;
				break;
		...
		return "<span class=\"ui-m-icon ui-m-li-left ui-m-status ui-m-o-"+ ii +"\"></span>"
	};
	...
	var _private = {
		...
		viewURI : function(entity, id, filter, orderBy, selected, groupBy, initData) {
			//return {uri: "xxxx"};
			return _private._viewResource(entity, id, filter, orderBy, selected, groupBy);
		},
		
		newURI : function(entity, id, filter, orderBy, selected, groupBy, initData) {
			return _private._editResource(entity, '', null);
		},
		editURI : function(entity, id, filter, orderBy, selected, groupBy, initData) {
			return _private._editResource(entity, id, null);
		},
		
		_viewResource: function(entity, id, filter, orderBy, selected, groupBy) {
			if (id)  {
				return {
					uri: that.config.baseUrl + entity + "('"+ id +"')",
					method: "GET",
				...
				var param = '';
				if (filter)
					param = param + (param?"&":"?") + "where="+encodeURIComponent(filter);
				if (orderBy) 
					param = param + (param?"&":"?") + "orderBy="+encodeURIComponent(orderBy);
				if (selected) 
					param = param + (param?"&":"?") + "selected="+encodeURIComponent(selected);
				if (groupBy)
					param = param + (param?"&":"?") + "_groupBy="+encodeURIComponent(groupBy);
				return {
					uri: that.config.baseUrl + entity + param,
					method: "GET",
					accept: "application/json"
				}
			}
		},
		_editResource: function(entity, id, data) {
			if (id)  
				return {
					uri: that.config.baseUrl + entity + "('"+ id +"')" + "/$template/"+that.sdataKey.workingCopies+"?trackingId=" + $.helpers.uuid(),
					method: "POST",
					accept: "application/json",
					data: data
				}
				...
				return {
					uri: that.config.baseUrl + entity + "/$template/"+that.sdataKey.workingCopies+"?trackingId=" + $.helpers.uuid(),
					method: "POST",
					accept: "application/json",
					data: data
				}
		...
		makeURL: function (url) {
			...
			if (url.editActivityRequest) u.push("/" + that.sdataKey.workingCopies);
			if (url.query) {
		...
		*/	 
		_startEditActivity: function(url, options) {
			...
			_private.logSdata("Virtual operation: Creating a working copy from a template: ", url);
			if (that.options.stateless) {
				/*  
				*   No statefull activity  -- > Simulate  a stateful activity 
				**  Simulate :  http://host/sdata/app/-/-/salesOrders/$template/$workingCopies?trackingId=xxx or 
				*** SEND  ---> GET  http://host/sdata/app/-/-/salesOrders/$template  or  GET  http://host/sdata/app/-/-/salesOrders('xxx')
				*       
				*/ 
				//SEND ---> GET  http://host/sdata/app/-/-/salesOrders/$template  or  GET  http://host/sdata/app/-/-/salesOrders('xxx')
				var newUrl = {query: null};
				$.extend(newUrl, url);
				newUrl.editActivityRequest = false;
				newUrl.method = 'GET';
				newUrl.accept= 'application/json';
				if (newUrl.id) 
					newUrl.template = false;
				delete newUrl.query;
				_private.makeURL(newUrl);
				var oldSuccess = options.success;
				options.success = function(data, headers) {
					if (headers.status > 300) {
						if ((headers.status == 404) && newUrl.template && !newUrl.id) {
							$.helpers.pushException({message: $.helpers.format($.helpers.messages.sdata_notfound,[newUrl.uri])}, $.helpers.messages.action_syncronize);
						}	
						return oldSuccess(null, headers);
					}
					/* save response in draft */
					if (!data.$key && !data.$uuid) data.$uuid = $.helpers.uuid();
					newUrl.entity=that.sdataKey.workingCopies;
					newUrl.template=false;
					newUrl.id = url.query.trackingId;
					_private.makeURL(newUrl);
					if (!data.$key) delete data.$url; 
					if (options.$init) {
						$.extend(data, options.$init);
						
					}
					var draftData = {
						data: data,
						id: $.helpers.uuid(), 
						original_url: url.uri, 
						original_type: options.$type,
						ui_url: options.$ui, 
						ui_type: 'application/json', 
						title: options.$title, 
						workingCopy_url: newUrl.uri,
						workingCopy_type: options.$type
					};
					that.createDraft(draftData, {
						success: function(draft) {
							var response = draft.data;
							response.$links ={
								$ui: {
							...
							oldSuccess(response, {status:201, location: newUrl.uri});
						}
						
					});
					
				};
				that.sendSDataRequest(newUrl, options);
			} else if (options.success) 
				options.success("Statefull Protocol not implemented.", {status: 500});
		},	
		...
		parseSDataURL:function(url) {
			...
			if (a.length == 2) {
				...
				if (a.length > 4) {
					s = a[4]
					if (s == '$template')  res.template=true;
				}
				if (a.length > 5) {
					s = a[5];
					if (res.template && (s == that.sdataKey.workingCopies)) 
						res.editActivityRequest = true;
				}
			} 
			return res;
		}
	};
	that.url=function(entityAction, entity, id, filter, orderBy, selected, groupBy, initData)  {
		if (entityAction && entity) {
			var hnd = _private[entityAction+'URI'];
			if (hnd) {
				return hnd(entity, id, filter, orderBy, selected, groupBy, initData);
			}
			return null;
		} 
		return null;
							...
							*/	 
							...
							if (send.$httpMethod == "PUT") {
								...
								if (value.$httpStatus == 201) {
									...
									return $.sdata.undoUpdate(that.$url({$baseURI: that.config.baseUrl}, send.$url), {	
										success: next,
										fail: next
									});									
									// delete
	...
	that.createDraft = function(data, options) {
		options.success(data);
	};
	that.saveDraft = function(workingCopy_url, data, options) {
		options.success();
	};
	that.deleteDraft = function(workingCopy_url, options) {
		options.success();
	};
	that.undoUpdate = function(url, options) {
		options.success();
	};
	that.deleteDataByDraft = function(url, options) {
		options.success();
	};
	that.deleteSDataByURL = function(url, options) {
		options.success();
	};
	that.updateDraft = function(draftURL, data, options) {
	};
	that.loadDraft = function(draftURL, options) {
	...
	that.getSyncData = function(options) {
		options.success(null);
	};
	...
	that.getSyncDataCount = function(options) {
		options.success(0);
	};
	...
	*/
	...
	that.removeFromDB = function(urls, options) {
		options.success();
	};
	...
	that.execURL=function(url, options)  {
		...
		var sdataURL = _private.parseSDataURL(url), cacheid = false;
		var olds = null, rmvcache = false;
		if (that.loadCache && sdataURL.id && !sdataURL.template ) {
			cacheid = sdataURL.id;
			if (that.loadCache[cacheid]) {
				return options.success(that.loadCache[cacheid].data, that.loadCache[cacheid].data.headers);
			}
		}
		if (sdataURL ) { 
			options.$type = 'application/json;vnd.sage=sdata';
			sdataURL.accept = options.$type;
			sdataURL.method = options.$method;
			var opts = options;
			if (cacheid) {
				opts = $.extend({}, options);
				opts = {
					success: function(data, headers) {
						that.loadCache[cacheid] = {data : data, headers: headers};
						options.success(data, headers);
					}
				};
			}
			_private._execSDATA(sdataURL, opts);
		} else if (options.success) {
