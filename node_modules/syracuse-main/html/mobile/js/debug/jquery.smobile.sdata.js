...
(function($) {
	...
	that.createUrl = function(newurl, sendUrl, data) {
		newurl = $proto.parseExpression(newurl, {
			data: $proto.exprData(data),
			$prototype: that.url2proto(sendUrl)
		}, "$url");
				...
				success: function(data, headers) {
					...
					if (headers.status > 300) {
						if ((headers.status == 404) && sendURL.template) {
							...
							}, $helpers.locale().actions.syncronize);
						}
			...
			if (a.length == 2) {
				...
				if (a.length > 4) {
					...
					if (s == '$template') res.template = true;
						...
						if (res.id) res.editActivity = true;
						else {
	...
	that._execBatch = function(syncData, options, resync) {
		...
		$helpers.each(Object.keys(syncData), function(entityKey, ii, doNext) {
			...
			res.forEach(function(value) {
				var d;
				/*
				 * if (false && value.$info.status == $.sdata.STATUS.CREATED) {
				...
				*/
				// FDB - STATUS.CREATED - Same as save - Need a PUT to work with X3 server like in desktop client
				if (value.$info.status == $.sdata.STATUS.MODIFIED || value.$info.status == $.sdata.STATUS.CREATED) {
					infos.push(value.$info);
					d = value;
					d.$httpMethod = "PUT";
								...
								if (send.$httpMethod == "PUT") {
									switch (value.$httpStatus) {
										...
										case 400:
											...
											}, $helpers.locale().sdata.modify_before);
										...
										case 503:
											...
											$helpers.pushException({
												message: errors.message
											}, $helpers.locale().sdata.S503);
											return next();
										default:
											$helpers.extractErrors(value, value.$httpMessage);
											$helpers.pushException({
												message: $helpers.locale().sdata.invalid_response
											}, value.$httpStatus);
											return next();
									}
									...
									switch (value.$httpStatus) {
										case 201:
											...
											that._checkBatchMsgs(value);
											var newurl = value.$httpLocation ? that.createUrl(value.$httpLocation, presUrl, value) : null;
										...
										case 202:
											...
											$helpers.pushException({
												message: $helpers.locale().post_ni
											});
											return next();
										case 400:
										...
										case 403:
											...
											$helpers.pushException({
												message: errors.message
											}, $helpers.locale().sdata.SC_canceled);
											// delete
									...
									switch (value.$httpStatus) {
										...
										case 403:
											...
											$helpers.pushException({
												message: errors.message
											}, $helpers.locale().sdata.SD_canceled);
											return $.sdata.undoUpdate(resUrl, next);
										case 412:
											//The If-Match header was provided but does not match the state of the resource on the provider side.
											errors = $helpers.extractErrors(value, value.$httpMessage);
					...
					if (url.id) {
						if (data && data[url.id]) {
							if (data[url.id].status === $.sdata.STATUS.DELETED) {
								...
								return options.success(data[url.id].data, {
									status: 200
								});
							}
						}
								...
								success: function(sdata, headers) {
									if (sdata && sdata.$resources) {
										...
										while (ii--) {
											...
											if (data[key]) {
												if (data[key].status === $.sdata.STATUS.DELETED) sdata.$resources.splice(ii, 1);
												else sdata.$resources[ii] = data[key].data;
											}
										}
									}
									headers = headers || {};
