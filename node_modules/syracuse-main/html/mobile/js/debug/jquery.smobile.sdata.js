...
(function($) {
	...
	that.store = function(url) {
		if (typeof url !== "object")
			url = that.parseSdataUri(url);
		var rep = _purl2rep(url);
		if (rep) {
			rep = _repPart(rep);
			if (that.config && that.config.entities && that.config.entities[rep] && that.config.entities[rep].sync)
				return true;
		}
		return false;	
	};
	
	that.cache = function(url) {
		if (typeof url !== "object")
			url = that.parseSdataUri(url);
		var rep = _purl2rep(url);
		if (rep) {
	...
	that.url2proto = function(purl) {
		var rep = _purl2rep(purl);
		if (rep.indexOf(".") < 0) rep = rep + "." + _consts.store_facet;
		if (that.config && that.config.pages && that.config.pages[rep]) return that.config.pages[rep].$prototype; 
		return null;
	};
	...
	that.data2id=function(purl,data, proto){   
		...
		return $proto.parseExpression(proto.$key, {data: $proto.exprData(data), $prototype:proto},"$key");
	},
	that.data2url=function(purl,data, proto){   
		...
		if (proto) {
			...
			if (isuuid)  {
				...
				key = $proto.parseExpression(proto.$key, {data: $proto.exprData(data), $prototype:proto},"$key");
		}
	...
	that.bestProto = function(purl) {
		...
		var facets = [{name:"$sync", detail:true}, {name:"$edit", detail:true}, {name:"$details", detail:true}, {name:"$query", detail:false}, {name:"$lookup", detail:false}];
		var proto = null;
	...
	that.url2local=function(purl, cache) {
		var query = purl.query;
		if (cache) return query.representation;
		purl.query = {} 
		if (query)
			purl.query.representation = _repPart(query.representation || ""); 
		var res = $.sdata.getUri(purl);
		purl.query = query;
		if (purl.id || purl.template) 
			res = res + "&appid=" + $.sdata.config.id;
		return res;
	};
	...
	that.createUrl = function(newurl, sendUrl, data) {	
		newurl =  $proto.parseExpression(newurl, {data: $proto.exprData(data), $prototype:that.url2proto(sendUrl)},"$url");
				...
				success: function(data, headers) {
					...
					if (headers.status > 300) {
						if ((headers.status == 404) && sendURL.template) {
							$helpers.pushException({message: $helpers.format($helpers.locale().sdata.S404, [newUrl.uri])}, $helpers.locale().actions.syncronize);
						}	
					...
					that.createDraft(draftData, function(draft) {
						...
						odata.$info = {original_url: draft.original_url};
						return options.success(odata, {status:201, location: sendURL.uri});
					});
				}
			});
	...
	that._execBatch = function(syncData, options, resync) {
		$.mobile.loading('show', {theme: "a", text: $helpers.locale().ui.sending});
		var _dosync = function(opt) {
			resync({$reinstall: true}, null, function() {
				opt.success();
			});
			return;
		};
		...
		$helpers.each(Object.keys(syncData), function(entityKey, ii, doNext) {
			...
			if (toSend.$resources.length) {
				var ch = {"Accept": "application/json;vnd.sage="+ _consts.vnd_sage};
				var retryOn401 = $helpers.addCustomHeaders(ch, false);
				$.ajax({
				headers: ch,
				type: "POST",
				dataType: "json",
							...
							if (send.$httpMethod == "PUT") {
								switch(value.$httpStatus) {
									case 200: 
										...
										return $.sdata.updData(resUrl, {data:value, original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
									case 400: 
										...
										$helpers.pushException({message: errors.message}, $helpers.locale().sdata.modify_before);
										that._cleanHeaders(send);
										return $.sdata.updData(resUrl, {$load: true, data:value, status:$.sdata.STATUS.MODIFIED, last_error:errors.message}, next);
									case 410: 
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message}, $helpers.locale().sdata.S410);
										that._cleanHeaders(value);
									...
									case 412: 
										...
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message},$helpers.locale().sdata.S412);
										that._cleanHeaders(value);
										return $.sdata.updData(resUrl, {data:value, original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
									case 500: 
										errors = $helpers.extractErrors(value, value.$httpMessage);
										return $.sdata.updData(resUrl, {$load: true, last_error:errors.message}, next);
									case 503: 
										//Transient provider error. Client MAY retry the request.
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message}, $helpers.locale().sdata.S503);
										return next();
									default:
										$helpers.pushException({message: $helpers.locale().sdata.invalid_response}, $helpers.locale().sdata.invalid_response);
										return next();
								}
								...
								switch(value.$httpStatus) {
									case 201:
										...
										var newurl = that.createUrl(value.$httpLocation, presUrl, value);
										return $.sdata.delData(resUrl, function(){
											if (that.store(newurl)) {	
												return $.sdata.insData(newurl, value, next);
											} else
												next();
										});
									case 202: 
										//An asynchronous POST was accepted. 
										$helpers.pushException({message: $helpers.locale().post_ni});
										return next();
									case 400:
									...
									case 403:
										...
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message},  $helpers.locale().sdata.SC_canceled);
										// delete
								...
								switch(value.$httpStatus) {
									...
									case 403: 
										//Consumer is not authorized to create the resource.
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message}, $helpers.locale().sdata.SD_canceled);
										return $.sdata.undoUpdate(resUrl, next);		
									case 412: 
										//The If-Match header was provided but does not match the state of the resource on the provider side.
										errors = $helpers.extractErrors(value, value.$httpMessage);
										$helpers.pushException({message: errors.message}, $helpers.locale().sdata.SD412);
										that._cleanHeaders(value);
										return $.sdata.updData(resUrl, {data:value, original_data:null, status:$.sdata.STATUS.ORIGINAL, last_error:""}, next);
									case 500: 
										//Generic status for consumer side error.
	...
	that.post = function(url,data, next, headers, options) {
		...
		var doPost = function(retry) {	
			...
			retryOn401 = $helpers.addCustomHeaders(h, retry);
			...
			$.ajax({
			  ...
			  error: function(jqXHR, textStatus, errorThrown) {
				...
				if (options && options.Authenticate && (status === 401)) {
					...
					return next(null, {status:status,"WWW-Authenticate": s});
				}
				$helpers.pushAjaxError(jqXHR, textStatus, errorThrown, url, "GET");
				if (status === 401) that.capabilities.resetInterval = true;
				var rheaders =  {status: status, location: jqXHR?jqXHR.getResponseHeader("Location"):""};
				next(null, rheaders);
			  }
			});
		};
	...
	*/
							...
							if (sdata && sdata.$resources) {
								...
								while (ii--) {
									...
									if (isuuid) 
										...
										key = $proto.parseExpression(proto.$key, {data: $proto.exprData(res), $prototype: proto}, "$key");
									if (data[key]) {
	...
	that.execGetSData = function(uri, options) {
		...
		var cachequery = (!local  && that.cache(uri));
		var usecache = cachequery && !$helpers.online();
		usecache = cachequery;
		if (!local && !usecache) {
			that.authenticate(function(ad, errors, offline) {
				if (errors || offline) return options.success();
				that.GET(uri.uri, function(data, headers) {
					if (that.addCache && cachequery && !uri.query.where) {
						var cdata = (data?data.$resources:[]) || [];
						return that.addCache( that.config.id, uri.query.representation, cdata, function(){
			...
			if (proto && proto.$properties.$resources && proto.$properties.$resources.$item) {
				if (uri.query.where || uri.query.filter) {
					var str = uri.query.where || "";
					if (uri.query.filter) {
						if (proto && proto.$filters && proto.$filters[uri.query.filter]) {
							var fwhere = proto.$filters[uri.query.filter].$where || "";
			...
			that.getSData(uri, usecache, options.success);
		}
	};
		

	that.getSData = function(uri, cache,  cb) {
		if (cb) cb("Not found", {status: 404});
	};
	that.addCache = function(appid, entity, data, cb) {
		if (cb) cb();
	};
	...
	that.downloadLocale = function(locale, next) {
		var lang = $helpers.extractLang(locale),  nl = "";
		if (lang) {
			return that.GET("./mobile/js/locale/mobile-"+lang + ".json", function(data, headers) {
				if (data) {
