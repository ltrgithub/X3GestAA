...
(function($) {
	...
	that.url2local=function(purl, fav) {
		...
		if (query) {
			if (fav) 
				purl.query.representation = query.representation; 
			else
		...
		$helpers.each(Object.keys(syncData), function(entityKey, ii, doNext) {
			...
			res.forEach(function(value) {
				...
				if (value.$info.status == $.sdata.STATUS.CREATED) {
					...
					d.$url = value.$info.url  + "." + _consts.store_facet;;
	...
	that.translateURL =  function (url) {
		...
		if (url.indexOf("device=") < 0) {
			...
			// url = url + sep + "device=desktop";
			// !! device = phone for mobile devices
			url = url + sep + "device=phone";
		}	
		return url;
	};
	that._success=function(cd, hack){
		...
		if (cd && cd.$diagnoses) $helpers.pushDiagnoses(cd);
	};
