...
(function($) {
	...
	that.cache = function(url) {
		...
		if (rep) {
			rep = _repPart(rep);
			if (that.config && that.config.entities && that.config.entities[rep] && that.config.entities[rep].cache)
				return true;
		}
		return false;	
	};
	...
	*/
	...
	that.execGetSData = function(uri, options) {
		...
		var cachequery = (!local && $.sdata.offline && that.cache(uri));
		if (!local && !cachequery) {
			that.authenticate(function(ad, errors, offline) {
			...
			that.getSData(uri, options.success);
		}
	};
		

	that.getSData = function(uri, cb) {
