...
(function($) {
	...
	that._checkBatchMsgs = function(data){
		if (data && data.$actions && data.$actions){
			for (var p in data.$actions){
				if (data.$actions[p].$diagnoses) $helpers.pushDiagnoses(data.$actions[p]);
			}
		}
	...
	that.translateURL =  function (url) {
		...
		var deviceName = settings.debug.deviceName?settings.debug.deviceName:"phone";
		
		if (url.indexOf("device=") < 0) {
			var sep = (url.indexOf("?") < 0)?"?":"&";
			url = url + sep + "device=" + deviceName;
		}	
		return url;
	};
	...
	that.applicationDetails = function(appData, cb) {
		...
		var settings = $helpers.settings();		
		settings.debug = settings.debug || {};
		cd.ignoreErrors = settings.debug.installWithErrors?true:false;;
