...
(function($) {
	...
	that.loadRepData = function(representation) {
		if (!$sdata.config && $sdata.config.id) return null;
		var value = window.localStorage.getItem($sdata.config.id);
		if (value) {
	...
	that.saveRepData = function(representation, value) {
		if (!$sdata.config && $sdata.config.id) return;
		var dd = window.localStorage.getItem($sdata.config.id) || {};
		dd[representation] = value;
		
		window.localStorage.setItem($sdata.config.id, JSON.stringify(dd));
	}
	that.loadFilter = function(representation) {
		var value = that.loadRepData(representation);
	...
	that.favorite = function(representation, value) {
		...
		if (value != null) {
			rep = rep || {};
			rep.favorite = value;
			that.saveRepData(that.saveRepData, rep);
			return  value;
		} else {
		...
		if (uiData && uiData.hbuttons) {
			uiData.hbuttons.forEach(function(btn) {
				...
				if (btn.id === "filter") {
					...
					var fav = that.favorite(controller.ui);
	...
	var _buildPage = function (pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		ui.header = ui.header  || {}
		var nh = {
			fpanel: { uiClass: "Button", data: { link: true, hash: controller.prefix + "panel", css :["s-m-hbutton"],
	...
	*/
	...
	Controller.prototype = {
		...
		actionfavorite: function(options) {
			var self = this;
			var rep = self.ui;
			var fav = $smobile.favorite(rep);
			fav = !fav;
			$smobile.favorite(rep, fav);
		...
		clearFilter: function() {
			var self = this;
			var repdata = $smobile.loadRepData(this.ui);
			if (repdata) {
				repdata.filter = null;
				$smobile.saveRepData(this.ui, repdata);
			}
		},
