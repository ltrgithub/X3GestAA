'use strict';

(function($) {
	var that = $.smobile || {};
	var q_parser = /([^&=;]*)=?([^&;]*)/g; //  query parser
	that._uiCache= {};
	
	that.PAGES = {
		"start": {ui: "start.$static", useAppId: false, db: false, addBack: false}, /* start page */
		"errors": {ui: "errors.$static", useAppId: 1, db: false, addBack: true}, /*error page  */
		"home": {ui: "home.$static", useAppId: 2, isHome: true, db: true, addBack: true, cache: true}, /*home page of application  */
		"about": {ui: "about.$static", useAppId: false, db: false, addBack: true, home: true}, /*about sage*/
		"send": {ui: "send.$static", useAppId: 2, home: true, addBack: true}, /*send page  */
		"page": {useAppId: 2, db: true, addBack: true, home: true},  /* regular  page */
		"dialog": {useAppId: 1, db: true, addBack: true, home: false},  /*regular  page */
		"apps": {ui: "applications.$static", useAppId: 0, db: false, addBack: true, home: true}  /*application manager* */
	};
	Object.keys(that.PAGES).forEach(function(id){
		that.PAGES[id].id = id;
	});
	
	that.lastDomId = null;
	that._pageStack=[];
	that._pageStackById=[];
	that.pushPage = function(object) {
		if (object.id) {
			that._pageStack.push(object);
			that._pageStackById.push(object.id);
		}
	};
	that.popPage = function() {
		that._pageStackById.pop();
		return that._pageStack.push();
	};
	that.lastPage = function() {
		if (that._pageStack.length) return that._pageStack[that._pageStack.length-1];
		return null;
	};
	that.prevPage = function(id) {
		var idx = that._pageStackById.indexOf(id);
		if (idx > 0) 
			return  that._pageStack[idx-1];
		else if (idx === 0) return null
		return that.lastPage();
	};
		
	
	
	that.mainFooter = function() { return "mainfooter";};
	that.pageIsDialog = function(page) {
		return  (page.attr( "data-" + $.mobile.ns + "role") === "dialog");
	};
	that.pageController = function(page) {
		return  $.jqmData(page[0], "controller");
	};
	
	that.pageIsDetail = function(page) {
		return ($.jqmData(page[0], 'detail') === "detail");
	};
	
	
	
	
	that.hash_sep = "-";
	$.smobile = that;
	
	that.CONSTS = {
		error_json : "error"
	}
	that.PAGES.start.enhanceHandler = function(pdata, after) {
		// add installed applications
		var items = (pdata && pdata.content && pdata.content.menu && pdata.content.menu.data && pdata.content.menu.data.items)?pdata.content.menu.data.items:null;
		if (!items) return after(pdata);
		
		$.sdata.getInstalledApps(false,{
			success: function(installed) {
				var locale = that.locale;
				if (installed && installed.length) {
					installed.forEach(function(app) {
						items.push({data: 
							{
								title:app.title, 
								description: app.description, 
								icon: app.icon,  
								hash: that.PAGES.home.id + that.hash_sep + encodeURIComponent(app.id)
							}});
					});
				} else {
					items.push({data:{title: locale.applicationmanager.title, divider:true}});
					items.push({data:{
							title: locale.applicationmanager.title, 
							description: locale.applicationmanager.description,
							icon: "sage.appmanager",  //TODO
							hash: that.PAGES.apps.id
						}});
				
				}
				after(pdata);
			}
		});
		
	};
	that.PAGES.apps.enhanceHandler = function(pdata, after) {
		var locale = that.locale;
		if (pdata) pdata.$title = locale.applicationmanager.title;
		after(pdata);
	};
	
	that.linkDetailPage = function(controller) {
		if (controller.applicationId)  {
			return that.PAGES.page.id + that.hash_sep + encodeURIComponent(controller.applicationId);
		} else return that.PAGES.page.id;
	};
	var _parseRepresentation = function(param) {
		if (param.representation) {
			var a = param.representation.split(".");
			if (a.length >= 2) {
				param.representation = a[0];
				param.facet = a[1];
			}
		}
	};
	var _parsePageURI = function(query, page) {
		var params= {}, pRegex = new RegExp("#"+page, "");
		$.helpers.decodeJsmQuery(query.replace(pRegex, "")).replace(/^\?/, "").replace(q_parser, function($0, $1, $2 ) {
			if ($1) params[$1] = decodeURIComponent($2);
		});
		if (params.url) {
			params.dataurl = "";
			var ii = params.url.indexOf("?");
			if (ii >= 0) {
				params.dataurl = params.url.substr(0, ii);
				var qq = params.url.substr(ii+1);
				qq.replace(q_parser, function($0, $1, $2 ) {
					if ($1) params[$1] = decodeURIComponent($2);
				});
			}
		}	
		_parseRepresentation(params);
		return params;
	};
	
	var _pageName = function (hash) {
	    if (!hash) return null;
		var isEncoded = (hash.indexOf("=") < 0);
		//hashFormat:   [#]pagename[-appid[-pageId]][encodeURIComponent(?query)]
	    var ch = (hash[0] === "#")?hash.substring(1):hash;
		var idx = ch.indexOf((isEncoded?$.helpers.encodeJsmQuery("?"):"?")); 	
		if (idx > 0) ch = ch.substring(0, idx);
		var res = { page: null };
		idx = ch.indexOf(that.hash_sep);
		if (idx > 0)  {
			res.page = that.PAGES[ch.substring(0, idx)];
			res.application = decodeURIComponent(ch.substr(idx+1));
			// 36 is 
			if (res.application.length > 36) {
				res.pageId = res.application.substr(36+1)
				res.application = res.application.substr(0, 36)
			}

		} else {
			res.page = that.PAGES[ch];
			res.application ="";
		}
		if (!res.page) return null;
		if ((res.page.useAppId===2) && !res.application) return null; // no appid found
		if ((res.page.useAppId===1) && !res.application) {
			res.application = ($.sdata.config?$.sdata.config.id:"");
		}
		if (!res.pageId) res.pageId = $.helpers.uuid();
		res.pageName = res.page.id + (res.application?(that.hash_sep+encodeURIComponent(res.application)):"");
		res.domid =(that.PAGES.home.id === res.page.id)?res.application:res.pageId;
		return res;
	};
	that.ACTIONS = {
		"action-cancel" : {
			name: "action-cancel",
			execute: function(params, after) {
				var fafter = function() {
					$.sdata.getSyncDataCount({
						success: function(count) {
							if (!count) {
								that.doBack(null); 
								after(false);
							} else after(true);
						}
					});
				};
				$.sdata.undoUpdate(params.url, {	
					success: fafter,
					fail: fafter
				});
			}
		}
		
		
	}
	var _actionName = function (hash) {
	    if (!hash) return null;
		var a = Object.keys(that.ACTIONS);
		var pn = "", i = a.length;
		while (i--) {
			var s=a[i];
			var ii = hash.indexOf(s);
			if ((ii === 0) || (ii === 1)) 
				return that.ACTIONS[s];
		}
		return null
	};
	var _execAction = function (hash, action, after) {
		var params = _parsePageURI(hash, action.name);
		return action.execute(params, after);
		
	};
	
	var _createHtml = function (cl, res) {
		if (cl.before) res.push(cl.before);
		if (cl.html) res.push(cl.html);
		if (cl.childs) {
			cl.childs.forEach(function(child){
				_createHtml(child, res);
			});
		}
		if (cl.after) res.push(cl.after);
	};
	var _hasErrorControl = function(controls) {
		for (var name in controls) 
			if (controls[name].uiClass === "ErrorView") return true;
		return false;
	};
	var _createPage = function(controller, controls, html, after) {
		$.helpers.each(Object.keys(controls), function(id, index, next) {
			var c = controls[id];
			var parent = html[c.parent] || html.root;
			c.id = controller.prefix+id;
			if (c.layout) { 
				var l = $.smobile.addLayout.apply(controller, [parent, c]);
				html[id] = l;
				if (l.childs && l.childs.length) l.childs.forEach(function(child) { html[child.id] = child;});
				return next();
			} else if (c.uiClass) {
				$.smobile.addControl.apply(controller, [parent, c, function(ctrl) {
					if (ctrl) {
						html[id] = ctrl;
						if (ctrl.bind || ctrl.action) 
							controller.registerControl({id: c.id, bind: ctrl.bind, action:ctrl.action,  options: ctrl.options, uiClass: c.uiClass, refresh: ctrl.refresh});
					}
					next();
					
				}]);
			} else return next();
		}, after);
	};
	var _buildPage = function (pagename, $currentPage, $page, ui, data, draftURL, info, pageInfo, $template, cb) {
		var sd = new Date();
		var page = $page[0], html = {root: {before: "", after:"", childs: []}}, parentController = null; 
		var hhtml = {root: {before: "", after:"", childs: []}}
		var fhtml = {root: {before: "", after:"", childs: []}}
		if ($currentPage && $currentPage[0]) {
			parentController = $.jqmData($currentPage[0], "controller");
		}
		$page.uiController({id: pagename, ui: pagename, entity: info.entity, entityAction: info.entityAction, inCreate: info.$createActivity, page: pageInfo});
		var controller = $.jqmData(page, "controller");
		controller.pageInfo = pageInfo;
		if (pageInfo.page.useAppId === 2) {
			controller.applicationId = pageInfo.application; 
			controller.homeRef = '#' + that.PAGES.home.id + that.hash_sep + encodeURIComponent(controller.applicationId);
		} else  if (pageInfo.page.useAppId == false) {
			controller.applicationId = "";
			controller.homeRef = "#"+that.PAGES.start.id;
		}
		
		if (ui.$page) {
			controller.page = ui.$page;
			delete ui.$page;
		}
		controller.data = data || {};
		controller.draftURL = draftURL;
		controller.parent = parentController;
		var $header = $page.children( ":jqmData(role=header)" ),
			$content = $page.children( ":jqmData(role=content)").children(".content-primary"),
			$footer = $page.children( ":jqmData(sagerole=footer)" );
		ui.header = ui.header  || {title : {uiClass: "Html", data: {html: "<h1></h1>"}}};
		ui.footer = ui.footer  || {};
		ui.content = ui.content || {};
		controller.pageOptions = ui.options || {};
		
		_createPage(controller, ui.footer, fhtml, function(){
			_createPage(controller, ui.header, hhtml, function() {
				if (!_hasErrorControl(ui.content)) 
					$.helpers.clearErrors();
				else 
					controller.hasErrorView = true;
				_createPage(controller, ui.content, html, function(){
					var content = [], pgtitle;
					_createHtml(hhtml.root, content);
					$header.html(content.join(''));
					var $h = $header.find( "h1" );
					if ($h && $h.length)  pgtitle =  ui.$title || (controller.data?controller.data.$title:"") || "";
					var _afterSetTitle = function() {
						if (pageInfo && pageInfo.page && pageInfo.page.home) { 
							$header.append('<a href="'+ (controller.homeRef?controller.homeRef:pageInfo.page.home)+'" data-'+ $.mobile.ns +'icon="home" data-'+ $.mobile.ns +'iconpos="notext" data-'+ $.mobile.ns +'direction="reverse" class="ui-btn-right jqm-home">'+that.locale.maintoolbar.home+'</a>');
						} else	if (pageInfo && pageInfo.page && (pageInfo.page.id == that.PAGES.home.id)) { 
							$header.append('<a href="#'+ that.PAGES.start.id+'" data-'+ $.mobile.ns +'icon="gear" data-'+ $.mobile.ns +'iconpos="notext" data-direction="reverse" class="ui-btn-right jqm-home">'+that.locale.ui.root_menu_tip+'</a>');
						}
						content = [];
						_createHtml(html.root, content);
						$content.html(content.join(''));
						content = [];
						_createHtml(fhtml.root, content);
						$footer.html(content.join(''));
						//$page.page();
						if (controller.pageOptions.dialog) {
							$page.removeAttr('data-'+ $.mobile.ns +'role');
							$page.attr('data-'+ $.mobile.ns +'role', 'dialog');
							controller.pageOptions.detail = false;
						} else if (controller.pageOptions.detail) 
							$.jqmData($page[0], "detail", 'detail');
						if (ui.actions) {
							for (var action in ui.actions) controller["action" +action] =  ui.actions[action];
						}
						//$page.attr( "data-" + $.mobile.ns + "external-page", true ).one( 'pagecreate', $.mobile._bindPageRemove );
						$page.insertAfter($template);
						var delta = new Date() - $.smobile.startDate;
						//console.log("--- PERFS ---------");
						//console.log("Render Time : " + delta + " (ms)");
						cb();
					};
					if (pgtitle){
						if (that.hasTemplate(pgtitle)) {
							var ct = that.parseTemplate(pgtitle);
							return that._template("", ct, controller.data, controller, function(tv) {
								$h.html(tv);
								_afterSetTitle();
							});
						
						} else 
							$h.html(pgtitle);
					} 
					_afterSetTitle();
					
				}); 
			}); 
		});
	};
	that._loadStaticUI = function(name) { return that.staticui[name](); }
	var _loadDialogData = function (ui, cb) {
		var hash = encodeURIComponent("?ui="+encodeURIComponent(ui));
		var page = _pageName( '#' + that.PAGES.dialog.id);
		return _loadDataAndDesign(hash, page, function(name, uiData) {
			if (uiData) delete uiData.footer;
			cb(name, uiData);
		});
	};
	var _menu2link = function(menuItem, mi) {	
		if (menuItem.linkType == "$representation") {
			mi.entity = menuItem.entity;
			mi.representation = menuItem.representation;
			mi.facet = menuItem.facet || "$query";
		}
	};
	var _representation2ui = function(params, data) {	
		var cd = {};
		if (params.facet ==  "$dashboard") {
			cd =  {
				"content": {
					"menu": { "uiClass":"MainMenu", "data" :{"items": []}} 
				}
			};
			data.forEach(function(menuItem){
				var mi = { title: menuItem.title, description:menuItem.description, icon: menuItem.icon};
				_menu2link(menuItem, mi);
				cd.content.menu.data.items.push({data:mi});
			});
		} else if (params.facet ==  "$query") {
			cd =  {
				"content": {
					"items": { "uiClass":"SDataList", "data" :{"bind": "$resources"}} 
				}
			};
			if (data && data.$prototype && data.$prototype.$title)
				cd.$title = data.$prototype.$title;
			cd.$page = params.representation + "." +  params.facet;
		} else if ((params.facet ==  "$details") || (params.facet ==  "$edit")) {
			cd =  {
				"content": {
				}
			};
			var map = {};
			$.proto.layoutParser(map, data.$article || data.$prototype.$article, "$article", [data.$prototype]);
			$.proto.generateForm(map, 0, cd.content, "", false);
			if (data && data.$prototype && data.$prototype.$title)
				cd.$title = data.$prototype.$title;
			cd.$page = params.representation + "." +  params.facet;
		} 		
		else cd = data;
		return cd;
		
	};
	var _loadDataAndDesign = function (query, page, cb) {
		var pageData=null, uiData={}, params = _parsePageURI(query, page.pageName);
		if (!params.representation && page.page.ui) {
			params.representation=page.page.ui;
			_parseRepresentation(params);
		}
		if (!params.representation) return cb("", null, null);
		
		var _afterEnhance = function(footer) {
			if (!page.page.db) {
				var cd = that._loadStaticUI(params.representation);
				if (!cd) return cb(params.representation, uiData, pageData);
				uiData = cd;
				if (uiData.footer && (uiData.footer.type == "edit")) {
					uiData.footer = that.editFooter();	
				} else if (uiData.footer && (uiData.footer.type == "sync")) {
					uiData.footer = that.syncFooter();	
				}  else if (footer) {
					uiData.footer = footer;
				}
				if (page.page.enhanceHandler) 
					page.page.enhanceHandler(uiData, function(newUiData) {
						return cb(params.representation, newUiData, pageData);
					});
				else
					return cb(params.representation, uiData, pageData);
			} else {
				var uimap = null, rep = params.representation;
				if (page.page.isHome) {
					params.representation = $.sdata.config.home;
					rep = params.representation;
					params.facet = "$dashboard";
				}
				if (params.facet == "$dashboard") {
					uimap = $.sdata.config.dashboards;
				} else if (params.facet) {
					rep = params.representation + "." + params.facet;
					uimap = $.sdata.config.pages;
				}
				if (uimap && uimap[rep]) {
					uiData = uimap[rep];
					uiData = _representation2ui(params, uiData);
					if (uiData) {
						if (uiData.footer && uiData.footer.type) {
							uiData.footer = that[uiData.footer.type + "Footer"](uiData.footer.data);	
						}  else if (footer) 
							uiData.footer = footer;
						if (page.page.isHome) {
							uiData.$title = $.sdata.config.title;
						}
						if (page.page.enhanceHandler) 
							page.page.enhanceHandler(uiData, function(newUiData) {
								return cb(rep, newUiData, pageData);
							});
						else {
							return cb(rep, uiData, pageData);
						}
					}  else {
						return cb("", null, null);
					}
				} else
					return cb("", null, null);
			}
		};
		var _loadRootFooter = function(next) {
			var data = that._loadStaticUI(that.mainFooter());
			next(data);
		
		};
		var _loadAppFooter = function(next) {	
			if ($.sdata.config) {
				var max_items = 4;
				var footer =  that.staticui.appFooter();
				var items = footer.footer.data.items;
				var footers = $.sdata.config.footer;
				var len = Math.min(footers.length, max_items-1);
				var ni = [], i;
				for (i=0; i<len; i++){
					var ci = footers[i]
					var mi = {text:ci.title, icon:ci.icon|| "gear"};
					_menu2link(ci, mi);
					ni.push(mi); 
				}
				footer.footer.data.items =  ni.concat(items);
				// cache footer
				$.sdata.config.cacheFooter = $.extend(true, {},footer);
				next(footer);
			}  else  next(null);
		};
		var _after = function(footer) {
			if (footer) {
				return _afterEnhance(footer);
				
			} else  return _afterEnhance(null);
				
		};
		var _load_page = function() {
			if ((page.page.useAppId == 2) && (!$.sdata.config || ($.sdata.config.id != page.application))) {
				$.sdata.getAppConfig(page.application, {
					success: function(config) {
						$.sdata.config = config;
						if ($.sdata.config.pages) {
							Object.keys($.sdata.config.pages).forEach(function(pagename){
								var cp = $.sdata.config.pages[pagename];
								if (cp.$prototype){
									$.proto.linkParents(cp.$prototype, null);
								}
							})
						}
						_loadAppFooter(_after);
					}
				});
				
			} else {
				if (!page.page.useAppId) {
					delete $.sdata.config;
					 $.sdata.useCacheCount = false;
					_loadRootFooter(_after);
				} else 
					_after(($.sdata.config?$.sdata.config.cacheFooter:null));
				
			}
		
		}
		_load_page();
	};
	that.removePage=function(page) {
		if (page.id) {
			var idx = that._pageStackById.indexOf(page.id); 
			if (idx >= 0) {
				that._pageStack.splice(idx, 1);
				that._pageStackById.splice(idx, 1);
				
			}
		}
		var controller = $.jqmData(page, "controller");
		//console.log("Remove page" + (controller?(": " + controller.pageInfo.page.template +"-"+ (controller.pageInfo.pageId || "")):""));
		var $t = $(page);
		if (controller) controller.remove();
		$.removeWithDependents($t);
	
	};
	that.removeAllPages=function() {
		while (that._pageStack.length) {
			var pid = that._pageStackById.pop();
			that._pageStack.pop();
			var $page = $('#' + pid.id);
			if ($page && $page.length) that.removePage($page[0]);
		}
	};
	that.pageName=_pageName;
	that.actionName=_actionName;
	that.loadPage = function (query, page, cb) {
		_loadDataAndDesign(query, page, cb);
	};
	that.loadDialog = function (query, cb) {
		_loadDialogData(query, cb);
	};
	that.execAction = _execAction;
	that.buildPage = _buildPage;
	that.refreshPage = function() {
		return $.mobile.changePage(window.location.hash, 
				{	
					changeHash: true,
					allowSamePageTransition: true,
					transition: "none"
				}
			);	
	};
	that.goHome = function(controller, appid) { 
		var hash = "#"+that.PAGES.start.id;
		if (controller && controller.applicationId) appid = controller.applicationId;
		if (appid)
			hash = '#' + that.PAGES.home.id + that.hash_sep + encodeURIComponent(appid);
		$.mobile.changePage(hash, 
			{	
				changeHash: true,
				allowSamePageTransition: true,
				reverse: true
			}
		);
	};
	that.doBack = function(controller) {
		var back = $.mobile.urlHistory.getPrev();
		if (back) {
			window.history.go(-1);
		} else that.goHome(controller);
	};
	
	that.showErrors = function(controller) {
		if (controller && controller.hasErrorView) {
			return that.refreshPage();
		}
		$.mobile.changePage("#errors", 
			{	
				changeHash: true,
				allowSamePageTransition: true
			}
		);
	};
	that.createPage = function(page) {
		var  $ptemplate = $("#page-template");
		var $page=$ptemplate.clone().attr('data-'+$.mobile.ns +'url', page.pageName).attr('id', page.domid)
		if (page.page.addBack) $page.attr( "data-" + $.mobile.ns + "add-back-btn", "true");
		return {$page: $page, $template: $ptemplate};
	};
	that.createDialog = function(uiData, pageData) {
		var page = _pageName('#dialog'), uiId = "dialog";
		var cp = that.createPage(page);
		var options =  {transition: 'pop', role: 'dialog', changeHash: true};
		$.smobile.buildPage(uiId, null, cp.$page, uiData, pageData, "", {$createActivity: true}, page, cp.$template, function() {
			var controller = $.jqmData(cp.$page[0], "controller");
			if (uiData.actions) {
				controller.pageActions = uiData.actions;
				delete uiData.actions;
			}
			if ($.mobile.activePage) {
				var oldController = $.smobile.pageController($.mobile.activePage);
				if  (oldController) {
					$.smobile.pushPage({id: oldController.pageInfo.pageId, detail:oldController.pageOptions.detail, dialog: oldController.pageOptions.dialog, href:oldController.options.dataUrl});
				}
			}
			$.mobile.changePage(cp.$page, options);
			controller.bindData();
		});
		
	};
	that.showError = function(settings, msg) {
		$( "<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>"+ (msg || $.mobile.pageLoadErrorMessage) +"</h1></div>" )
							.css({ "display": "block", "opacity": 0.96, "top": $(window).scrollTop() + 100 })
							.appendTo( settings.pageContainer || document.body )
							.delay( 800 )
							.fadeOut( 1000, function() {
								$( this ).remove();
							});	
	};
	that.addLayout = function(parentLayout, layoutData) { return null; };
	that.addControl = function(parentLayout, c, after) { after();};
	that.setEvents = function(c, $c) { return null;};
	that.parseURL = function(u, page, cb) {
		var doEncodeHref = true;
		var filter = null, initData=null, url={method:"GET"};
		var params = _parsePageURI(u.search || u.hash, page.pageName);
		var base = u.domain+ u.pathname + ($.helpers.hashFirst()?('#' + page.pageName):"");
		params.entityAction =  params.entityAction || $.sdata.sdataActions.view.id;
		if (!params.representation && page.page.ui) params.representation=page.page.ui;
		
		var options = {};
		if (params.url)  {
			
			var opts = {$title:params.title, $method:url.method, $init: initData, $filter:filter, $entityAction:params.entityAction,  $ui: params.ui, $type: params.type, 
				success: function(data, headers) {
					var href =  u.domain+ u.pathname, isDraft=(params.entityAction != $.sdata.sdataActions.view.id);  
					var newURI = headers.location?headers.location:params.url;
					if (headers.status > 300) {
						if (page.application)
							href = href + "#" +that.PAGES.home.id + that.hash_sep + page.application;
						else
							href = href + "#" +that.PAGES.start.id;
						return cb(href , data, '', true, options);
					} else if (headers.status == 100) {
						return cb("", data, "", false, {stop: true});
					} else {				
						if ((headers.status == 201 ) && (params.entityAction ==  $.sdata.sdataActions["new"].id)) {
							options.$createActivity = true;
						}
						var search = '?url=' + encodeURIComponent(newURI);
						search = base + (doEncodeHref?$.helpers.encodeJsmQuery(search):search) + ($.helpers.hashFirst()?"":('#' + page.pageName));
						return cb(search, data, (isDraft?newURI:""), false, options);
					}
					
				}
			};
			if (url && url.options) $.extend(opts, url.options);
			return $.sdata.execURL(params.url, opts);
		}
		return cb(base +   ($.helpers.hashFirst()?"":('#' + page.pageName)), null, '', false, options ); 
	};
	var _initSDATA = function(config, data, options) {
		$.sdata.initData(config, data, options);
		
	};
	that.load = function(appData, progress, after) {
		$.sdata.loadAuth(function() {
			$.sdata.authModule.checkConnected({
				success: function() {
					return that._load(appData, progress, after);
				},
				fail : function() {
					$.sdata.authModule.connect({
						success: function() { 
							return that._load(appData, progress, after); 
						},
						fail: function() { if (after) after();}
					});
				}
			});
		});
	};
	
	that._saveAppsInDB = function(after, apps, fail, progress, count, len) {
		$.helpers.each(apps, function(capp, index, next) {
			if (fail || $.helpers.hasErrors()) {
				fail = true;
				return next();
			}
			$.sdata.installApp(capp, {
				success: function() {
					count++;
					if (progress) progress(count, len, "Done");
					next();
				},
				fail: function() {
					next();
				}	
			});
		}, function() {
			if (progress) progress(len, len, "Done");
			if (after) after();
		});
	};
	
	that._load = function(appData, progress, after) {	
		var apps = [], urls = [];
		var datasets = appData.datasets;
		delete appData.datasets;
		datasets.forEach(function(dataset) {
			if (dataset.selected === false) return;
			var cd = $.extend(true, {},appData);
			cd.id = $.helpers.uuid();	
			cd.baseURL = cd.baseURL + dataset.dataset;
			cd.description = cd.description + " ("+dataset.description + ")";
			cd.version = cd.version || "0.000";
			apps.push(cd);
		});
		var len=apps.length*2, count = 0;
		$.helpers.each(apps, function(capp, index, next) {
			$.sdata.applicationDetails(capp, function(data) {	
				count++;
				if (progress) progress(count, len, "Done");
				capp.data = data;
				data.title = capp.title;
				data.description = capp.description;
				data.baseUrl = capp.baseURL;
				return next();
			});
		},function() {
			that._saveAppsInDB(after, apps, false, progress, count, len); 
		});
	};
	
	
	
	
	that.editFooter= function() {
		return {
			footer : {
				uiClass:"NavBar", 
				data:{ 
					"items": [
						{ text: $.helpers.messages.edit_cancel, action: "cancel" , "jmdata": {"data-icon":"refresh", "data-transition": "none"} },
						{ text: $.helpers.messages.edit_rmv, action: "delete", "jmdata": {"data-icon":"delete", "data-transition": "none"} },
						{ text: $.helpers.messages.edit_save, action: "save", "jmdata": {"data-icon":"check", "data-transition": "none"} }
					]
				}
			}
		}
	};
	that.editViewFooter= function(data) {
		var res = {
			footer : {
				uiClass:"NavBar", 
				data:{ 
					"items": [
						{ text: $.helpers.messages.edit_cancel, action: "cancel" , "jmdata": {"data-icon":"refresh", "data-transition": "none"} },
						{ text: $.helpers.messages.edit_rmv, action: "delete", "jmdata": {"data-icon":"delete", "data-transition": "none"} }
					]
				}
			}
		};
		if (data && data["ui-edit"])
			res.footer.data.items.push({ useDefault: true, text: $.helpers.messages.edit,  entityAction: $.sdata.sdataActions.edit.id, ui:  data["ui-edit"], "jmdata": {"data-icon":"grid"} });
		return res;
	};
	that.syncFooter= function() {
		return {footer : {uiClass:"SyncFooter" }};
	};
	
})(jQuery);    

(function($){
	/**
	* @constructor
	*/
	var Controller = function($page, options){
		var self = this;
		self.$page = $page;
		self.page = $page[0];
		if (options.id) options.id = options.id.replace(/\./g, "_").replace(/\$/g, "_");;
		var old = $.jqmData(self.page, "controller");
		if (old) old.remove();
		$.jqmData(self.page, "controller", this);
		self.options = options;
		self.controls = {};
		return self.init();
	};
	Controller.prototype = {
		defaults: {id: "empty"},
		init: function() {
			var self = this;
			self.config = $.extend({}, self.defaults, self.options);
			self.prefix = self.config.id + "_";
			self.ui = self.config.ui;
			self.hasErrorView = false;
			self.errors = [];
			return this;
		}, 
		clearErrors: function() {
			var self = this;
			self.errors.length=0;
		},
		hasErrors: function() {
			return this.errors.length >0;
		},
		pushError: function(error) {
			var self = this;
			self.errors.push(error);
		},
		getPageData: function() {
			var self = this;
			if (!self.page) return null;
			if (!$.sdata.config || !$.sdata.config.pages || !$.sdata.config.pages[self.page]) return null;
			var data = $.sdata.config.pages[self.page];
			
			return data;
		},	
		afterChangeHnd: function(c) {
			var self = this, f;
			if (c.options && c.options.action && self["action"+c.options.action]) {
				f = self["action"+c.options.action];
				return f.bind(self);
			}
			return null;
		},
		
		checkData: function() {
			var self = this;
			self.clearErrors();
			Object.keys(self.controls).forEach(function(name){
				var control = self.controls[name];
				var f = $.smobile.controls[control.uiClass].checkValue;
				if (f) {
					var $c=$('#' + control.id);
					if ($c) {
						f.apply(self, [$c, control]);
					}
				}
				
			});
			if (self.hasErrors()) return false;
			return true;
			
		},
		checkChange: function(cid, value, next) {
			var self = this;
			var ctrl = self.controls[cid];
			if (ctrl && ctrl.bind) {
				//OK
				return self.getData(ctrl.bind, null, "", function(oldValue) {
					if (oldValue != value) {
						self.setData(ctrl.bind,value);
						self.notifyChange();
					}
					if (next) next();
				});
			}
			if (next) next();
			
		},
		notifyChange: function() {
			var self = this;
			if (self.draftURL) $.sdata.updateDraft(self.draftURL, self.data);
		},
		getValue: function(bind, data, defValue) {
			var cd =  data || this.data, name;
			var values = bind.split('.');
			while (values.length) {
				if (!cd) break;
				name = values.shift();
				cd = cd[name];
			}
			if (cd==null) cd = defValue;
			return cd;
		},
		
		getData: function(bind, data, defValue, after) {
			var cd =  data || this.data;
			var values = bind.split('.');
			var doContinue = false;
			$.helpers.each(values, function(name, idx, next) {
				if (doContinue) 
					return next();
				var old = cd;
				cd = old[name];
				if (cd == null) {
					if (old.$url && (name != '$key') && (name != '$uuid')) {
						return $.sdata.execURL(old.$url,  {success: function(data, headers) {
							if (data) {
								cd = data[name];
								if (cd == null) doContinue = true;
							} else 
								doContinue = true;
							 next(); 
							
						}});
					} else
						doContinue = true;
				}
				 next(); 
			}, function() {
				if (cd==null) cd = defValue;
				return after(cd);
			});
		},
		setData: function(bind, value, data) {
			var cd =  data || this.data;
			var values = bind.split('.');
			for (var i=0, len=values.length-1; i<len; i++) {
				var nv = cd[values[i]];
				if (!nv) {
					nv = {};
					cd[values[i]] = nv
				};
				cd = nv;
			}
			cd[values[values.length-1]]=value;
		},
		actioncancel: function(options) {
			var self = this;
			if (self.draftURL) {
				return $.sdata.deleteDraft(self.draftURL, options);
			}
			if (options.success) options.success();
		},
		actionsave: function(options) {
			var self = this;
			if (!self.checkData()) return false;
			if (self.draftURL) {
				return $.sdata.saveDraft(self.draftURL, self.data, options);
			}
			if (options.success) options.success();
		},
		installApplication: function(options, data) {
			var hasDatasets = false;
			if (!data) {
				if (options.fail) return options.fail();
				return;
			}
			$.helpers.clearErrors();
			if (data.datasets) {
				for (var i=0, len = data.datasets.length; i<len; i++) {
					hasDatasets = !(data.datasets[i].selected === false);
					if (hasDatasets) break;
				}
			} 
			if (!hasDatasets) {
				if ( options.fail) {
					return options.fail();
				}
				return;
			}
			var self = this;
			$.smobile.load(data, function(index, count, msg) {
				if ((index == 0) && count) $.mobile.showPageLoadingMsg("a", $.helpers.messages.installing);
				else if (index == count) $.mobile.hidePageLoadingMsg();
			}, function() {
				if ($.helpers.hasErrors()) {
					$.smobile.showErrors(self);
				} else if (options.success) options.success();
			});
		},
		_getRemoveList: function(remove, data) {
			var res = [];
			if (remove.data.entities) {
				Object.keys(remove.data.entities).forEach(function(en){
					var entity = remove.data.entities[en];
					if (entity.template) res.push({url: remove.baseURL + en + "/$template", type: "template"});
					res.push({url: remove.baseURL + en, type: "entity"});
				});
			}
			return res;
			
		},
		removeApplication: function(options, id) {
			$.helpers.clearErrors();
			var self = this;
			var toRemove = self.data.remove[id];
			
			var _after = function() {
				if ($.helpers.hasErrors()) {
					$.smobile.showErrors(self);
				} else if (options.success) options.success();
			
			};
			
			$.sdata.getAppConfig(id, {
				success: function(data) {
					toRemove.data = data;
					var urlList = self._getRemoveList(toRemove);
					urlList.push({url: toRemove.id, type: "application"});
					$.sdata.removeFromDB(urlList, {
						success: _after,
						fail: _after
					})
				},
				fail: _after
			});
			
		},
		
		actionsync: function(options) {
			$.helpers.clearErrors();
			var self = this, errors=[];
			var opts = {
				success: function() {
					if ($.helpers.hasErrors()) { 
						$.smobile.showErrors(self);
						return;
					}
					if (options.success) options.success();
				}
			}
			return $.sdata.execBatch(self.data, opts, self.resync);
		},
		actionundo: function(options) {
			$.helpers.clearErrors();
			var self = this, errors=[];
			if (self.data) {
				$.helpers.each(Object.keys(self.data), function(value, index, next) {
					var list = self.data[value];
					if (list && list.length) {
						$.helpers.each(list, function(value, index, next) {
							var url = value.$info.url;
							$.sdata.undoUpdate( value.$info.url, {success: next, fail: next})
						}, next);
					} else next();
				}, function() { if (options.success) options.success();})
			} else 
				if (options.success) options.success();
		},
		actiondelete: function(options) {
			var self = this;
			$.helpers.clearErrors();
			var opts = {
				success: function() {
					if ($.helpers.hasErrors()) { 
						$.smobile.showErrors();
						return;
					}
					if (options.success) options.success();
				}
			}
			if (self.draftURL) {
				return $.sdata.deleteDataByDraft(self.draftURL, opts);
			} else if (self.data && self.data.$url) {
				var url = $.sdata.$url({$baseURI: $.sdata.config.baseURL}, self.data.$url);
				return $.sdata.deleteSDataByUrl(url, opts);
			}
			if (options.success) options.success();
		},
		remove: function() {
			$.jqmRemoveData(this.page, "controller");
			delete this.$page;
			delete this.page;
			delete this.controls;
		},
		registerControl: function(data) {
			var self = this;
			self.controls[data.id] = data;
		},
		controlByClass: function(uiClass) {
			var self = this;
			for (var p in self.controls) {
				var o = self.controls[p];
				if (o.uiClass == uiClass) return o;
			}
			return null;
		},
		bindData: function(cb) {
			var self = this;
			Object.keys(self.controls).forEach(function (id) {
				var c = self.controls[id];
				var $c=$('#' + c.id);
				if ($c) $.smobile.setEvents.apply(self, [$c, c]);
			});
			if (cb) cb();
		}
	};
	$.fn.uiController = function(options){
		new Controller(this, options);
		return this;
	};
})(jQuery);

/*
	that._load = function(appData, progress, after) {	
		$.sdata.applicationDetails(appData, function(data) {
			var apps = [], urls = [];
			var datasets = appData.datasets;
			delete appData.datasets;
			datasets.forEach(function(dataset) {
				if (dataset.selected === false) return;
				var cd = $.extend(true, {},appData);
				cd.id = $.helpers.uuid();	
				cd.baseURL = cd.baseURL + dataset.dataset + "/";
				cd.description = cd.description + " ("+dataset.description + ")";
				cd.version = cd.version || "0.000";
				
				cd.data = {entities:{}, resources: {}, baseURL:cd.baseURL, homePage: data.homePage, appFooter:data.appFooter};
				data.entities.forEach(function(entity){
					cd.data.entities[entity.name] = {titleProp: entity.titleProp, template: entity.hasTemplate, localStore: (entity.localStore || (entity.localStore == null))};
					if (!cd.data.entities[entity.name].localStore) return;
					var obj = {type: "entity"};
					if (entity.hasTemplate) {
						obj.url =  cd.baseURL + entity.name + '/$template';
						obj.entity = entity.name ;
						obj.onlyOne = true;
						urls.push(obj);
						obj = {type: "entity"};
					}
					obj.url = cd.baseURL  + entity.name;
					urls.push(obj);
					obj = {type: "schema", ref: cd.data.entities[entity.name]};
					obj.url = cd.baseURL  + entity.name + "/$schema";
					urls.push(obj);
					
				});
				data.resources.forEach(function(resource){
					if (typeof resource.content == "object") 
						cd.data.resources[resource.name] = JSON.stringify(resource.content);
					else
						cd.data.resources[resource.name] = resource.content;
				});
				apps.push(cd);
			});
			var count = 0, len= urls.length + 1 + apps.length;
			var fail = $.helpers.hasErrors();
			if (progress) progress(count, len, "Done");
			var _createApplications = function() {
				$.helpers.each(apps, function(appData, index, next) {
					if (fail || $.helpers.hasErrors()) {
						fail = true;
						return next();
					}
					$.sdata.installApp(appData, {
						success: function() {
							count++;
							if (progress) progress(count, len, "Done");
							next();
						},
						fail: function() {
							next();
						}	
					});
				}, function() {
					if (progress) progress(len, len, "Done");
					if (after) after();
				});
			};
			var insertData= [], ci=null;
			//console.log(urls);
			$.helpers.each(urls, function(value, index, next) {
				if (fail || $.helpers.hasErrors()) {
					fail = true;
					return next();
				}
				var opt = ((value.type=="schema")?{ignoreErrors:true}:null);
				$.sdata.GET(value.url, function(data) {
					count++;
					if (fail || $.helpers.hasErrors()) {
						fail = true;
						return next();
					}
					if (progress) progress(count, len, "Done");
					if (!data) return next();
					if (value.type === "schema") {
						// add schema to application 
						value.ref.$schema = data;
						return next();
					} 
					if (!ci) {
						ci = value;
						var idata = data;
						var ppurl = $.sdata.parseSdataUri(value.url); 
						$.sdata.config = {baseURL :  $.sdata.getBaseUrl(ppurl)};						
						return _initSDATA(value, idata, {
							success: function() {
								ci = null;
								next(); 
							},
							fail: function() {
								fail = true;
								ci = null;
								next();
							}
						});				
					} else {
						var toadd = {value: value, data: data};
						if (data.$baseUrl) toadd.$baseURI = data.$baseUrl;
						insertData.push(toadd);
						return next();
					}
				}, {"Cache-Control":"no-cache", "Pragma": "no-cache"}, opt);
			}, function() {
				if (!fail) {
					return _initSDATA(insertData, null, {
						success: function() {
							_createApplications();
						},
						fail: function() {
							if (progress) progress(len, len, "Done");
							if (after) after();
						},
						setConfig: true
					});				
					
				} else {
					if (progress) progress(len, len, "Done");
					if (after) after();
				}
			},3);
			
		});
	};
*/