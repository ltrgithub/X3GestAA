(function($) {
	...
	var _pageName = function (hash) {
		...
		if (that.PAGES.home.id === res.page.id) 
			...
			res.longPageName = res.page.id + (res.application?(that.hash_sep+encodeURIComponent(res.application+ '-'+ res.pageId)):"");
		return res;
	};
	that.ACTIONS = {
		"action-cancel" : {
	...
	var _loadDialogData = function (ui, cb) {
		var hash = encodeURIComponent("?url="+encodeURIComponent(ui));
	...
	that.parseURL = function(u, page, cb) {
		...
		return cb(base +   ($.helpers.hashFirst()?"":('#' + page.longPageName)), null, false, options ); 
	};
