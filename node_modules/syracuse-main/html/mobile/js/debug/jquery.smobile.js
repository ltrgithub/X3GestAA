(function($) {
			...
			if (page) {
				if (facet === "$details")  {
					...
					if (params && params.url) {
						...
						res.exclude = true;
						var links = $.proto.extractLinks(page.$prototype, page.$prototype);
						//Object.keys(page.$prototype.$links).forEach =  
						res.items = [
						...
						if (true || links && links.length){
							res.items.push({ title: that.locale().ui.actions_item, action:"popup", links: links, "jmdata": {"data-icon":"gear", "data-transition": "none"} });
						}
					}
				}
			}
	...
	that.parseURL = function(u, page, cb) {
		...
		if ((params.facet === "$dashboard") && params.url) {
			...
			ch = ch + "?url=" +  $.helpers.encodeURIComponent(params.url);
		}
		return cb(ch, null, false, options); 
	};
	...
	that._load = function(appData, progress, after) {	
		var apps = [], reinstall = null;
		if (appData.$reinstall) {
			delete appData.$reinstall;
			reinstall = $.sdata.config.$info;
			if ($.sdata.config && $.sdata.config.id) {
...
(function($){
	...
	*/
	...
	Controller.prototype = {
		...
		_sendCurrentData: function(after, link) {
			var self = this;
			var data = self.data;
		},
