			...
			if (page) {
				if (facet === "$details")  {
					...
					if (page.$prototype.$links.$edit && !page.$prototype.$links.$edit.$isHidden) {
						...
						url = $.proto.parseExpression(url, {data: $.proto.exprData(data), $prototype: page.$prototype}, "$url");	
						ppurl = $.sdata.parseSdataUri(url); 
						if (ppurl.query && ppurl.query.representation && $.sdata.config.pages[ppurl.query.representation]) {
							res.items.push({title:that.locale().ui.edit_item, icon: "edit", url: page.$prototype.$links.$edit.$url, bind:true});
						}
	...
	Controller.prototype = {
		...
		actionnext: function(options) {
			...
			if (ccd.$links && ccd.$links.$next) {
				...
				$.mobile.changePage($.smobile.ui.urlHref(url, self), 
					{	
						changeHash: false,
						allowSamePageTransition: true,
						transition: "none"
