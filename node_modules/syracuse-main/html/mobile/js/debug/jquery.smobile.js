...
(function($) {
	...
	that.hash_sep = "-";
	$.smobile = that;
	that.filterkey = function(representation) {
		var res = [];
		if (!$sdata.config) return "";
		res.push($sdata.config.id);
		res.push(representation);
		res.push("filter");
		return res.join(".");
			
	};
	function _favoritekey(representation) {
		var res = [];
		if (!$sdata.config) return "";
		res.push($sdata.config.id);
		res.push(representation);
	...
	that.loadFilter = function(representation) {
		var key = that.filterkey(representation);
		if (!key) return null;
		var value = window.sessionStorage.getItem(key);
		return value?JSON.parse(value):null;
	};
	that.favorite = function(representation, value) {
		var key = _favoritekey(representation);
		if (!key) return false;
		if (value != null)
...
(function($){
	...
	*/
	...
	Controller.prototype = {
		...
		_filterkey: function() {
			return $smobile.filterkey(this.ui);
			
		},
		clearFilter: function() {
			var self = this;
			var key = self._filterkey();
			if (!key) return;
