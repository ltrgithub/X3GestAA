...
(function($) {
	...
	var _clickonheader = function(e) {
		...
		if (cpage) {
			...
			if (ctrl) {
				...
				var target = e.target;
				while (target && target.tagName && (target.tagName.toUpperCase() != 'A') && (target.tagName.toUpperCase() != 'BUTTON'))
	...
	var _buildPage = function (pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		_createPage(controller, ui.footer, fhtml);
	...
	var _private =  { 
		...
		enhanceFooter: function(data) {
			var max_items = 4, uiData = this.ui, footer = this.footer, panel = this.panel, rpanel = this.rpanel;
	...
	that.detailUiData = function(controller, cp, data, $layout, options) {
		...
		if (options) {
			...
			if (options.canDelete) {
				res.uiData.footer.footer.data.items.push({ 
					title: $helpers.locale().ui.remove_item, 
					action: "delete_detail", 
					"jmdata": {"data-icon":"delete", "data-transition": "none"} 
				});
			}
		}
	...
	function _afterEnhance(page, params, uiData, pageData, footer, panel, rpanel, cb) {
		var  _afterData; 
		if (!page.page.db) {
			var cd = that._loadStaticUI(params.representation);
			if (!cd) return cb(params.representation, uiData, pageData, null);
			uiData = cd;
			_afterData = _private.enhanceFooter.bind({ui: uiData, footer: footer, panel: panel, rpanel: rpanel});
	...
	function _load_page(page, params, uiData, pageData, cb ) {
		...
		if ((page.page.useAppId == 2) && (!$sdata.config || ($sdata.config.id != page.application))) {
			$sdata.getAppConfig(page.application, function(config, error) {
				...
				_afterEnhance(page, params, uiData, pageData, footer, panel, rpanel, cb);
			});
	...
	Controller.prototype = {
		...
		refreshPanel: function(left) {
			...
			if (left) {
				...
				if (pl) {
					if (pl && pl.options.syncHandler) pl.options.syncHandler.bind(self)(pl);
				}
			} else {
