...
(function($){
    ...
    var _buildPage = function(pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb){
        ...
        var $header = $page.children(":jqmData(role=header)"), $content = $page.children(":jqmData(role=content)").children(".content-primary"), $panel = $page.children(":jqmData(sagerole=panel)"), $rpanel = $page.children(":jqmData(sagerole=rpanel)");
        ...
        var _afterSetTitle = function(){
            ...
            if (controller.pageOptions.dialog) {
                ...
                if (controller.pageOptions.detail) 
    ...
    var _private = {
        menu2link: function(menuItem, mi){
            if (menuItem.linkType == "$representation") {
                ...
                if (menuItem.linkType == "$dashboard") {
        ...
        representation2ui: function(params, data){
            ...
            if (params.facet == "$dashboard") {
                ...
                if ((params.facet === "$query") || (params.facet === "$lookup")) {
                    ...
                    if ((params.facet == "$details") || (params.facet == "$edit") || (params.facet == "$create")) {
                        iaddErrorView = false;
                        cd = {};
                        var map = {};
    ...
    function _load_page(page, params, uiData, pageData, cb){
        ...
        if ((page.page.useAppId == 2) && (!$sdata.config || ($sdata.config.id != page.application))) {
            $sdata.getAppConfig(page.application, function(config, error){
                if (!config) 
                    throw new Error("Application not available for hostname " + window.location.host + "\nappid[" + page.application + "]");
                $sdata.config = config;
    ...
    that.parseURL = function(u, page, cb){
        ...
        var filter = null, initData = null, url = {
            method: "GET"
        };
...
(function($){
    ...
    Controller.prototype = {
        ...
        changeFav: function(fav){
            var self = this;
            var oldc = "ui-icon-s-fav-on", ncc = "ui-icon-s-fav-off";
            if (fav) {
                ncc = oldc;
