(function($) {
	...
	var _loadDialogData = function (ui, cb) {
		var hash = $.helpers.encodeURIComponent("?url="+$.helpers.encodeURIComponent(ui));
			...
			if (page) {
				if (facet === "$details")  {
					...
					if (page.$prototype.$links.$edit && !page.$prototype.$links.$edit.$isHidden) {
						url = $.proto.parseExpression(page.$prototype.$links.$edit.$url, {data: null, $prototype: page.$prototype},"$url");
	...
	that.detailUiData = function(controller, cp, data, items) {
		var res = {};
	...
	Controller.prototype = {
		...
		installApplication: function(options, data) {
			...
			$.smobile.load(data, function(index, count, msg) {
				if ((index == 0) && count) $.mobile.loading('show', {theme:"a", text: $.smobile.locale().ui.installing});
