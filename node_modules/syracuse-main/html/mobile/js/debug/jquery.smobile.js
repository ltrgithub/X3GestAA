...
(function($) {
	...
	var _buildPage = function (pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		var phtml = {root: {before: "", after:"", childs: []}}, hasPanel = false;
		...
		ui.header = ui.header  || {}
		var nh = {fpanel: { uiClass: "Button", data: { link: true, hash: controller.prefix + "panel", 
	...
	var _private =  { 
		...
		enhanceFooter: function(data) {
			var max_items = 4, uiData = this.ui, footer = this.footer, panel = this.panel;
	...
	var _loadDataAndDesign = function (query, page, cb) {
		...
		var _afterEnhance = function(footer, panel) {
			var  _afterData; 
			if (!page.page.db) {
				var cd = that._loadStaticUI(params.representation);
				if (!cd) return cb(params.representation, uiData, pageData, null);
				uiData = cd;
				var _afterData = _private.enhanceFooter.bind({ui: uiData, footer: footer, panel: panel});
