...
(function($) {
	...
	var _createHeaderRight = function(controller, uiData) {
		...
		html.push('<div id="'+controller.prefix+'headerright" data-type="horizontal" data-role="controlgroup" class="ui-btn-right">');
		...
		if (uiData && uiData.roptions && uiData.roptions.nopanel) {
		} else
			html.push('<a id="'+controller.prefix+'right_panel" href="#'+ controller.prefix +'rpanel" data-theme="a" data-role="button" data-inline="true" data-iconpos="notext" data-icon="bars" data-corners="false" data-iconshadow="false" data-shadow="false"> </a>');
		html.push('</div>');
		return html;
	};
	var _clickonheader = function(e) {
	...
	var _buildPage = function (pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		ui.header = ui.header  || {}
		var nh = {
			fpanel: { uiClass: "Button", data: { link: true, hash: controller.prefix + "panel", 
				jmdata: {"data-inline":"true", "data-iconpos":"notext", "data-icon":"bars", "data-corners":"false", "data-iconshadow":"false", "data-shadow":"false"}
			}},
			title: {uiClass: "Html", "data": {"html": "<h1></h1>"}}
			
		};
		if (controller.parent) delete nh.fpanel;
		...
		var _afterSetTitle = function() {
			if (pageInfo.page.useAppId) {
				var headerRight = _createHeaderRight(controller, ui);
	...
	var _private =  { 
		...
		getCtxItems: function(facet, representation, data, items) {
			...
			if (page) {
				if (facet === "$query")  {
					if (page.$prototype.$links.$create && !page.$prototype.$links.$create.$isHidden) {
						...
						if (ppurl && ppurl.query && ppurl.query.representation && $sdata.config.pages[ppurl.query.representation]) {
							items.push({title:$helpers.locale().ui.add_item, url: url, "jmdata": {"data-icon":"add", "data-transition": "none"}});
						}
					}
					...
					if (page.$prototype.$links.$edit && !page.$prototype.$links.$edit.$isHidden) {
						...
						ppurl = $sdata.parseSdataUri(url); 
						if (ppurl && ppurl.query && ppurl.query.representation && $sdata.config.pages[ppurl.query.representation]) {
							items.push({title:$helpers.locale().ui.edit_item, url: url, "jmdata": {"data-icon":"edit", "data-transition": "none"}});
						}
					}
					if (page.$prototype.$links.$delete && !page.$prototype.$links.$delete.$isHidden) {
						items.push({title:$helpers.locale().ui.remove_item, action: "delete", "jmdata": {"data-icon":"delete", "data-transition": "none"}});	
					}
				} else if ((facet === "$edit") || (facet === "$create")) {
					items.length = 0;
					items.push({ title: $helpers.locale().ui.cancel_item, action: "cancel" , "jmdata": {"data-icon":"refresh", "data-transition": "none"}});
					items.push({ title: $helpers.locale().ui.remove_item, action: "delete", "jmdata": {"data-icon":"delete", "data-transition": "none"}});
					items.push({ title: $helpers.locale().ui.save_item, action: "save", "jmdata": {"data-icon":"check", "data-transition": "none"}});
				}
			}
		},
	...
	that.detailUiData = function(controller, cp, data, $layout, options) {
		...
		res.uiData.hbuttons = [
			{ title: $helpers.locale().ui.ok_item, action: "save_detail", "jmdata": {"data-icon":"check", "data-transition": "back"}},
			{title: $helpers.locale().ui.cancel_item, action: "cancel_detail", "jmdata": {"data-icon":"refresh", "data-transition": "back"}}
		];
		if (options) {
			if (isReadOnly) {
	...
	function _addappItems(items, ritems, params) {
		...
		if (params.facet) {
			...
			item = {code: "actions", load:true, title: locale.ui.services, items: []};
			ritems.push(item);
		}
		...
		item = {title: locale.ui.sync_item, items: []};
	...
	that.createPage = function(page) {
		...
		if (page.page.isHome && !page.page.useAppId) {
			$page.children( ":jqmData(sagerole=panel)" ).attr('data-'+$.mobile.ns +'theme', "c");
			$page.children( ":jqmData(sagerole=rpanel)" ).attr('data-'+$.mobile.ns +'theme', "c");
		...
		$helpers.each(urls, function(url, index, next) {
			...
			$sdata.GET(url.url, function(data, headers) {
				...
				if (data && data.$resources  && data.$resources.length) {
					...
					if (url.cache) {
						return $sdata.addCache(url.appid, purl.query.representation, data.$resources, next)
					} else {
				...
				if (capp.data && capp.data.entities) {
					Object.keys(capp.data.entities).forEach(function(representation) {
						...
						if (config.sync || $proto.forceSync) {
							...
							if (capp.data.pages && capp.data.pages &&  capp.data.pages[representation+ ".$lookup"]) {
								url = {
									appid: capp.id, 
									url: capp.baseURL + "/" + config.pluralType + "?representation=" + representation+".$lookup" + "&sync=cache&role=" , 
									entity: capp.baseURL + "/" + config.pluralType,
									config: capp.data,
									representation: representation,
									cache: true
