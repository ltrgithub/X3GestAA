			...
			if ((page.indexOf(".$query") > 0) || (page.indexOf(".$lookup") > 0) || (page.indexOf(".$select") > 0)) {
				...
				Object.keys(proto.$properties).forEach(function(name) {
					...
					if (prop.$capabilities && (prop.$capabilities.indexOf('filter') >= 0)) {
						...
						if (details)  
							res.push({value: fv, title: $proto.parseExpression(prop.$title, {data: null, $prototype: proto},"$title"), type:tt});
						else
			...
			switch(event) {
				case "pageload": 
					if (self.ui && $sdata.config && $sdata.config.pages[self.ui]) {
						...
						design = design.$article || design.$prototype.$article;
						$proto.auth.notifyParent("pageload", design);
					}
					break;
			}
		},
