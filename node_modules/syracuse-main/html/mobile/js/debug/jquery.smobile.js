		...
		getCtxItems: function(facet, representation, data) {
			...
			if (page) {
				if (facet === "$details")  {
					...
					res.exclude = true;
					var links = $.proto.extractLinks(page.$prototype, page.$prototype, data, true);
					res.items = [
	...
	that.detailUiData = function(controller, cp, data, items, options) {
		...
		res.uiData.footer = {
			"footer" : {
				...
				"data":{ 
					"items": [
						{ title: that.locale().ui.ok_item, action: "saveDetail", "jmdata": {"data-icon":"check", "data-transition": "none"} }
					]
				}
			}
		};
		if (options && options.canDelete) {
			res.uiData.footer.footer.data.items.push({ title: that.locale().ui.remove_item, action: "deleteDetail", "jmdata": {"data-icon":"delete", "data-transition": "none"} });
		}
	...
	*/
	...
	Controller.prototype = {
		...
		actionservice: function(options, params) {
			...
			var url = $.proto.parseExpression(params.url, {data: controller.exprData(), $prototype: $.proto.getProto(page, params.bind)},"$url");
			var purl = $.sdata.parseSdataUri(url); 
			$.helpers.clearErrors();
			$.sdata.post(url, controller.allData(), function(data) {
				var cd  = data;
		...
		bindData: function(cb) {
			...
			self.$page.click(function(e){
				if (e && e.target) {
					...
					if (target && target.href.indexOf("#actionservice") >= 0) {
						e.preventDefault();
						var p = $(target).attr("data-action");
						if (p && self.alinks && self.alinks[p]) {
							self.actionservice({success:{}}, self.alinks[p]);
						}
					}
				}
			});
