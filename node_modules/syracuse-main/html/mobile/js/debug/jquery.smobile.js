...
(function($) {
	...
	var _buildPage = function(pagename, $currentPage, $page, ui, data, info, pageInfo, $template, cb) {
		...
		if (!_hasErrorControl(ui.content)) $helpers.clearErrors();
		else controller.hasErrorView = true;
	...
	function _addappItems(items, ritems, params, page) {
		...
		if (apps) {
			apps.forEach(function(app) {
				if (true || app.id != page.application || true !== page.page.isHome){
					itms.items.push({
						title: app.title,
						icon: app.icon,
						hash: that.PAGES.home.id + that.hash_sep + $helpers.encodeURIComponent(app.id)
	...
	that.refreshErrorView = function(controller){
		var ctrl = controller.controlByClass("ErrorView");
		if (ctrl) {
			$.smobile.controls["ErrorView"].refresh(controller, ctrl.options, $('#' + ctrl.id), ctrl.id);
		}
	};
	...
	Controller.prototype = {
			...
			if (ctrl && ctrl.bind) {
				...
				if (isLookup) {
					...
					if (value) {
						...
						if ((value[ctrl.options.$display] === "") && !value.$key && !value.$uuid) value = null;
					}
					if (value === null) {
						if (oldValue != null) {
		...
		notifyChange: function() {
			...
			if (self.editActivity()) {
				...
				} else $sdata.updateDraft(self.draftId(), self.data);
			}
		},
		...
		openSortDialog: function(sortFields) {
			var controller = this;
			if ($.smobile.openSortDlg) { 
				// declared in jquery.smobile.controls.filter.js
				var order = $.smobile.controls.SDataSorter.getOrderBy(controller);
				$.smobile.openSortDlg(controller, order, sortFields || controller.getSortFields());
				return true;
			}
			return false;			
		},
		...
		actiondelete: function(options) {
			...
			if (self.editActivity()) {
				...
				if (self.config && self.config.url && self.config.url.origUri) return $sdata.deleteDataByUrl(self.config.url.origUri, self.data, function() {
					if ($helpers.hasErrors()) {
						return options.success(null, true);
					}
					var after = function() {
