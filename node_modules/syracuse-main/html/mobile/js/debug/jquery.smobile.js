...
(function($) {
	...
	var _private =  { 
		...
		getCtxItems: function(facet, representation, data, params) {
			var res = {exclude: false, items: []}, url, ppurl;
			...
			if (page) {
				if (facet === "$details")  {
					...
					if (page.$prototype.$links.$edit && !page.$prototype.$links.$edit.$isHidden) {
						url =  page.$prototype.$links.$edit.$url;
						url = $.proto.parseExpression(url, {data: data, $prototype: page.$prototype});	
						ppurl = $.sdata.parseSdataUri(url); 
						if (ppurl.query && ppurl.query.representation && $.sdata.config.pages[ppurl.query.representation]) {
							res.items.push({title:that.locale().ui.edit_item, icon: "grid", url: page.$prototype.$links.$edit.$url, bind:true});
						}
						
					}
				} else if (facet === "$edit")  {
					if (params && params.url) {
						ppurl = $.sdata.parseSdataUri(params.url); 
						res.exclude = true;
						res.items = [
							{ title: that.locale().ui.cancel_item, action: "cancel" , "jmdata": {"data-icon":"refresh", "data-transition": "none"} },
							{ title: that.locale().ui.remove_item, action: "delete", "jmdata": {"data-icon":"delete", "data-transition": "none"} },
							{ title: that.locale().ui.save_item, action: "save", "jmdata": {"data-icon":"check", "data-transition": "none"} }
						];
					}
				}
			}
		...
		enhanceFooter: function(uiData, footer, params, page, pageData) {
			var max_items = 4;
			...
			if (footer && uiData && uiData.$facet && $.sdata.config) {	
				...
				var context = _private.getCtxItems(uiData.$facet, uiData.$representation, pageData, params); // context items
	...
	var _loadDataAndDesign = function (query, page, cb) {
		...
		var _afterEnhance = function(footer) {
			if (!page.page.db) {
				...
				uiData = cd;
				_private.enhanceFooter(uiData, footer, params, page.page);
	...
	*/
	...
	Controller.prototype = {
		...
		notifyChange: function() {
			var self = this;
			if (self.editActivity()) $.sdata.updateDraft(self.draftId(), self.data);
		},
		...
		actionsave: function(options) {
			...
			if (self.editActivity()) {
				return $.sdata.saveDraft(self.draftId(), self.data, options.success);
			}
			options.success();
		},
