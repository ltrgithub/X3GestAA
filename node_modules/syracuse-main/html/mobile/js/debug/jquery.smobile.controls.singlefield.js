	...
	that.controls.SingleField={
		...
		handler: function(c, after) {
			...
			if (edit) {
				...
				if (cp.$isHidden || $article.$isHidden) return after({html:"", bind:false});
				...
				if (cp.$rtype === "application/x-reference") {
					...
					if ($proto.isNumeric(cp.$type)) {
						...
						if ((updproto.$maximum != null) && (updproto.$minimum != null)) {
							...
							html.push($proto.htmlValue(controller.data,entityProto, updproto,{$bind: options.$bind, $type: cp.$type},value, {useValue: true}));
