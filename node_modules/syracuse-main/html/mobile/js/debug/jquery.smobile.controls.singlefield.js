	...
	that.controls.SingleField={
		...
		handler: function(c, after) {
			...
			if (edit) {
				...
				var title = $proto.parseExpression(pp.$title, {data: null, $prototype: pp},"$title");
				switch (ft) {
				...
				if (!$article.$isTitleHidden) {
					var cssl = ["s-m-style", "s-m-label"];
						+$helpers.htmlEncode($proto.execExpression(cp.$title, cp))
					if (cp.$type === "image") {
					...
					html.push($proto.parseExpression(pp.$title || "", {data: null, $prototype: pp},"$title", {html:true}));
					html.push('</label>');
				}
				...
				if (cp.$rtype === "application/x-reference") {
					...
					html.push($proto.htmlValue(controller.data,entityProto, cp,{$bind: options.$bind, $type: cp.$type}));
				}
				html.push('</div>');
				html.push('</div>');
				return after({ 
