...
const locale = require('syracuse-core').locale;
...
exports.getConnectedAppToken = function(_, clientId, params) {
	...
	return globals.context.session ? globals.context.session.withElevatedPrivilege(_, getToken) : getToken(_);
};
...
exports.formatConnectedAppUrl = function(_, clientId, path, params) {
	...
	return globals.context.session ? globals.context.session.withElevatedPrivilege(_, getUrl) : getUrl(_);
};