...
exports.create = function(_, reader) {
	...
	/// 
	function sign(_, certName, certdir){
		...
		try{
		    ...
		    var	filter = {jsonWhere: {name: certName}};
		    
		    var certInst = db.fetchInstance(_, entity, filter);
			if (!certInst)
				throw pdfUtils.error(locale.format(module, 'errCertInstNotFound', certName));
...
//			console.log("Expected length :"+util.inspect(guessSigSize));
			...
			var options = {output_encoding: "hex"};
			if (certdir != null) {
				options.certdir = certdir;
			}
			var signature = certInst.sign(_, 'sha1', documentHash, {output_encoding: "hex"});		
			try{
				// VERIFY SIGNATURE
				valid = certInst.verify(_, 'sha1', documentHash, signature, {signature_encoding: "hex"});
				if (!valid)
					throw pdfUtils.error(locale.format(module, 'errSigNotValid', reader.file, certName));
			}catch(e3){
