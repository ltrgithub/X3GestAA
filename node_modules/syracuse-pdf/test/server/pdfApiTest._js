...
var createCertificateInstance = function(_, name, passphrase) {
    ...
    var model = db.model;
    var entity = model.getEntity("certificate");
    ...
    var caString = fs.readFileSync("node_modules/jsx509/test/server/fixtures/"+name+".crt", "utf8");
    var caBuf = new Buffer(caString);
	var caStore = certInst.certificate(_);
	var stream = caStore.createWritableStream(_, {length: caBuf.length});
	stream.write(_, caBuf.toString(), "binary");
	stream.write(_, null);
	caStore.close(_);

	
    var keyString = fs.readFileSync("node_modules/jsx509/test/server/fixtures/"+name+".key", "utf8");
    var keyBuf = new Buffer(keyString);
	var keyStore = certInst.key(_);
	var stream2 = keyStore.createWritableStream(_, {length: keyBuf.length});
...
asyncTest("initialize syracuse test server", 1, function(_) {
	...
	syracuse.initializerStatus.on("initialized", function() {
		...
		syracuse.server.listen(null, 3004);
		start();
	});
});
...
asyncTest('check nodelocal config', 1, function(_){
	strictEqual(config && config.collaboration && config.collaboration.certdir && fs.existsSync(config.collaboration.certdir), true, "Certificate directory exists.");
	start();
});
